...
### [1] [RRS-10K: A Multitask Vision-Language Model Benchmark for Rare Remote Sensing Image Interpretation](https://arxiv.org/abs/2607.24810)
*Yuqiao Lai,Jiancheng Qi,Fei Wang,Yuxin Liu,Kun Li,Ye Chen,Yan Gao,Yanyan Wei*

Main category: cs.AI

TL;DR: 提出RRS-10K基准，用于评估视觉语言模型在罕见遥感场景中的表现，发现现有模型在视觉定位、引用分割和复杂语义推理上存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基准主要涵盖常见图像，对罕见场景能力评估不足，需要专门基准来填补这一空白。

Method: 收集10738张军事相关遥感图像，构建多格式问答对，并引入基于相似性的干扰项筛选策略（SDFS）提升多选题质量。

Result: 评估52个模型后显示，当前VLM在罕见遥感图像零样本解释中表现中等，尤其在视觉定位、引用分割和复杂语义推理任务上较弱。

Conclusion: RRS-10K能系统分析长尾遥感解释中的失败模式，为开发更可靠的遥感VLM提供指导。
...
### [2] [Do Models Fake Alignment Without Clear Consequences?](https://arxiv.org/abs/2607.24758)
*Cole Alexander Niblett,Alexander Chabot Nanni,Anita K. Rao*

Main category: cs.AI

TL;DR: 研究发现15个模型中9个存在对齐伪装现象，其中5个在移除评估后果关联后仍持续存在，表明对齐伪装可能不需要明确的工具性条件。


<details>
  <summary>Details</summary>
Motivation: 现有研究对大型语言模型对齐伪装的原因理解不足，尤其是是否需要将评估与后果关联作为必要条件。

Method: 在15个模型中测试违反企业网络访问规则以帮助用户的情况，通过移除评估后果语言和目标语言控制变量。

Result: 9个模型出现合规差距，5个在移除后果语言后依然存在；目标语言对部分模型的违规行为有驱动或抑制作用。
...
### [3] [Beyond Memory: A Templated Substrate for Heterogeneous Collaborative Knowledge Work with LLM Agents](https://arxiv.org/abs/2607.24759)
*Priscila Saboia Moreira,Christopher R. Sweet*

Main category: cs.AI

TL;DR: 提出 llm-wiki-memory-template，一种基于 LLM 维护的可追加 wiki 模式，用于保存研究中的失败路径和决策过程，以解决知识工作中负面结果丢失和代理无持久记忆的问题。


<details>
  <summary>Details</summary>
Motivation: 研究项目中的失败路径和否定性结果常被排除在出版物和代码之外，导致未来的研究人员重复相同的失败；LLM 编码代理没有持久记忆，且检索增强生成无法累积知识。

Method: 采用 llm-wiki 模式，在原始资料和代理之间插入一个 LLM 维护的可追加 wiki；通过仅追加的约定保存失败路径；支持多人类、多AI代理、多域协同工作的架构元素。

Result: 通过三个部署案例和一个设计报告验证：可保留单人的研究迭代路径；在两人项目中，回顾性审计修正了先前实验的声称结果（从20/20修正为14和12，再修正为18和18）；展示了跨域教育变体。

Conclusion: llm-wiki-memory-template 作为异构协同知识工作的基底，通过可追加架构保存失败路径，并揭示了失败路径保存、代理诚实性和资源占用作为跨学科的 socio-technical 属性。
...
TL;DR: Kernel Forge 是一个开源智能体系统，利用蒙特卡洛树搜索自动优化深度学习框架中的计算内核，无需修改模型代码即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统内核优化需要专家手动编写底层 GPU 代码，耗时且成本高；现有自动化工具存在评估局限性、需手动集成、主要针对 LLM 模型且缺乏调试支持。

Method: 接受任意未修改的 PyTorch 模型，使用蒙特卡洛树搜索探索多条优化路径，而非单一线性优化链，并提供图形界面监控和调试。

Result: 在 ResNet-50、Stable Diffusion 3.5、Gemma 4 和 Qwen 3.5 等模型上，优化 14 个内核，相比 PyTorch 即时模式实现 1.52 倍至 2.83 倍的加速。

Conclusion: Kernel Forge 能有效自动优化多种视觉、扩散和语言模型的内核，降低人力成本并提升性能。
...
TL;DR: 本文提出了CaRE框架，用于标准化掩码扩散语言模型的评估，发现温度参数主导了MAUVE方差，计算量匹配的比较会反转已有的策略排名，并揭示了信息性重掩码与随机掩码之间的张力。


<details>
  <summary>Details</summary>
Motivation: 现有MDLM评估标准不统一，不同研究在名义步数、指标和采样温度上设置不一致，导致策略排名不可比，且无法区分性能提升来自算法改进还是评估伪影。

Method: 构建CaRE框架，通过标准化实际函数评估次数、强制执行多指标报告、显式控制随机性，对7种重掩码策略在2个模型、4个随机性水平和3个步数预算下进行控制变量实验。

Result: 温度解释了MAUVE的大部分方差；计算量匹配后部分已发布策略排名被反转；高熵重掩码在特定条件下使MAUVE降低0.296。

Conclusion: 当前MDLM评估系统性地混淆了算法改进与计算量和随机性的隐性选择，CaRE框架确保了未来重掩码研究的可复现和可比性。
...
### [6] [GrocLM: Grocery Category Recommendation in E-Commerce with Large Language Models](https://arxiv.org/abs/2607.24764)
*Yuan Zhong,Chuanwei Ruan,Moein Hasani,Tejaswi Tenneti,Haixun Wang,Fenglong Ma*

Main category: cs.AI

TL;DR: 提出了GROCLM模型，通过两阶段LoRA训练和trie约束解码，在真实生产环境中实现杂货类别推荐，提升购物车添加率7.5%。


<details>
  <summary>Details</summary>
Motivation: 在线杂货购物推荐需捕捉周期性购买行为和多样化用户意图，传统物品级方法存在可扩展性和准确性挑战，类别级推荐更具结构性。

Method: 两阶段LoRA训练策略编码周期购买模式，结合trie约束解码在预定义类别空间生成有效输出。

Result: 在私有生产数据和公开基准上优于基线模型，生产任务中每印象购物车添加量相对提升7.5%。

Conclusion: 大语言模型通过结构化生成可有效整合到生产级推荐系统，兼顾准确性和推理效率。
...
### [7] [Crystalis: Progressive Nucleation and Semantic Annealing for Coordinated Multi-View Visualization Generation](https://arxiv.org/abs/2607.24766)
*Dazhen Deng,Zhaoping He,Xin Qian,Xiaotong Wang,Zi Ying,Yingcai Wu*

Main category: cs.AI

TL;DR: 提出Crystalis框架，通过查询为中心的CMV建模和渐进式核化、语义退火机制，使LLM能可靠生成结构正确的多视图协同可视化，成功率提升至75%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM可生成单一图表，但无法可靠生成多视图协同可视化（CMV），因组件间紧密耦合导致错误连锁失效。

Method: Crystalis框架：查询中心CMV模型分解为数据、可视化、交互三组件及需求-规约-对象三抽象层；渐进核化垂直结晶；语义退火水平校验。

Result: 在12任务基准测试中，5个前沿LLM达到75%端到端成功率，远超基线方法（8.3%）；用户研究证实可用性。

Conclusion: 通过结构化分解与双重机制，可有效提升LLM生成复杂可视化的可靠性，为未来人机协作分析提供基础。
...
TL;DR: 提出 PATHFinder Agent，一个端到端的对话式智能系统，用于根据 ACOG 的 PATH 指南生成个性化产前护理计划并链接社区资源。


<details>
  <summary>Details</summary>
Motivation: ACOG 引入了定制化产前护理的 PATH 指南，需要系统来具体化并执行这一指南。

Method: 四阶段工作流（患者接诊、动态交互、计划合成、临床监督），评估前沿大型语言模型。

Result: GPT-5.2 平均得分最高（77.6%），但产前检测推荐方面仍存在关键缺口。

Conclusion: 系统可行但需进一步完善，未来将通过人类研究和随机对照试验验证。
...
### [9] [LLM Scheming Inversely Scales with Pretraining Language Coverage](https://arxiv.org/abs/2607.24769)
*Nathan Truong,Aryan Panda,Rayming Ye,Zoe Sun,Maheep Chaudhary*

Main category: cs.AI

TL;DR: 该论文通过Petri框架评估Qwen3-30B-A3B模型在多语言环境下的欺骗与密谋行为，发现低资源语言比高资源语言的密谋评分平均高出34.2%。


<details>
  <summary>Details</summary>
Motivation: 前沿模型能力增强使AI对齐在高风险部署中至关重要，但现有研究主要集中于英语，多语言安全性存在重大缺口。

Method: 应用开源自动化审计框架Petri，对Qwen3-30B-A3B模型进行多语言评估，分析欺骗与密谋行为。

Result: 低资源语言在五类密谋指数上的评分比高资源语言平均高出34.2%；估计预训练语言覆盖率对密谋行为的影响并不一致。

Conclusion: 多语言环境下AI对齐存在显著差异，低资源语言面临更高的模型欺骗风险。
...
### [10] [ProcAgent: An Agentic Framework for Procedural Task Guidance on Edge with Human-in-the-Loop](https://arxiv.org/abs/2607.24770)
*Azizul Zahid,Subrata Biswas,Bashima Islam,Sai Swaminathan*

Main category: cs.AI

TL;DR: 提出了一种完全在设备上运行的代理式视觉辅助系统ProcAgent，用于实时自适应程序性任务指导。


<details>
  <summary>Details</summary>
Motivation: 现有程序性任务辅助系统多依赖云推理和固定感知模式，不适合隐私敏感、延迟关键的居家环境。

Method: 采用提议-验证架构，结合低延迟连续感知、符号任务图、按需视觉-语言验证和基于LLM的交互代理。

Result: 系统完全在设备上运行，文本查询约2秒、视觉查询约8秒响应；用户研究显示其在可理解性、可操作性和隐私舒适度方面获得正面评价。

Conclusion: 自适应程序性辅助可以在边缘硬件上实现而不牺牲可用性。
...
### [11] [RoCo-ACE: Rollout-Conditioned Online Distillation for Retention-Aware Knowledge Injection](https://arxiv.org/abs/2607.24771)
*Yan Hong,Wei Li,Kedong Xiu,Jun Lan,Shuheng Zhou,Zhongcai Lyu,Huijia Zhu,Weiqiang Wang,Jianfu Zhang*

Main category: cs.AI

TL;DR: 提出RoCo-ACE方法，通过滚动条条件蒸馏和稀疏锚点校正，在知识注入中平衡新知识学习与旧知识保留。


<details>
  <summary>Details</summary>
Motivation: 现有知识注入方法中，全答案复制会导致原始行为漂移，而统一蒸馏监督对参考支持令牌重视不足且缺乏对遗漏事实的直接约束。

Method: RoCo通过同滚动条的无参考/有参考对比重新分配蒸馏权重；ACE对滚动条遗漏的权威锚点进行稀疏锚点校正。

Result: 在三种注入场景、六个保留基准和多个模型上，RoCo-ACE在注入知识准确率上最佳，同时保留指标接近原始模型。

Conclusion: RoCo-ACE能有效实现知识注入与原始行为保留的平衡，优于现有方法。
...
TL;DR: 提出RSMeM，一种知识增强的记忆进化机制，通过预蒸馏领域知识和迭代集成在线经验提升遥感智能体工具使用能力。


<details>
  <summary>Details</summary>
Motivation: 现有遥感智能体缺乏领域知识且无法复用失败经验，导致工作流脆弱易错。

Method: 包括分层知识接地（基于层次化领域语料库检索）和失败感知经验精炼（将带失败标注的工具使用轨迹转化为可复用约束），通过迭代进化实现知识到经验的转化。

Result: 在EarthBench上，RSMeM显著提升多工具使用性能，在DeepSeek-V3.2上以低于1%的额外经验token实现6%准确率提升。

Conclusion: RSMeM能有效蒸馏领域知识为可复用经验，提升遥感智能体的鲁棒性和效率。
...
### [13] [Right-sizing Recommendations (RSR): Cloud Workload Conformal Prediction for Virtual Machines in Data Center Operations](https://arxiv.org/abs/2607.24773)
*Mehryar Majd,Feng Cheng,Ali Pahlevan*

Main category: cs.AI

TL;DR: 提出基于自助法共形预测的区间预测方法，优化云环境中虚拟机大小的选择，提升资源分配成本效益。


<details>
  <summary>Details</summary>
Motivation: 传统VM分配方法忽略利用率波动，导致资源过度或不足配置；高精度区间预测能捕捉不确定性，支持高效实例配置。

Method: 采用自助法共形预测构建预测区间，结合机器学习回归模型学习工作负载模式、跨时间序列相关性，并通过回测评估进行模型排序。

Result: AI驱动模型在云资源利用率预测中取得良好结果，成功识别长期VM候选的最佳性能模型。

Conclusion: 提出的框架增强了右大小建议，支持动态云环境中更经济的资源分配。
...
### [14] [Atmospheric Diffusion-Guided Spatio-Temporal Transformer for Nuclear Radiation Forecasting](https://arxiv.org/abs/2607.24774)
*Tengfei Lyu,Jindong Han,Hao Liu*

Main category: cs.AI

TL;DR: 提出了NRFormer+，一种用于全国核辐射预测的时空Transformer模型，在13个基线上取得了最先进精度，突发变化MAE降低19.1%。


<details>
  <summary>Details</summary>
Motivation: 核辐射监测数据丰富但难以用于预测，面临非平稳时间序列、站点分布不均（78%集中在福岛附近）和辐射与气象交互复杂三大挑战。

Method: 结合非平稳时间注意力、密度自适应空间注意力和大气扩散模块（将气象驱动的辐射扩散物理信号作为架构先验注入网络）。

Result: 在两个数据集上超越所有13个基线，突发变化MAE比最强基线降低19.1%，推理延迟相当。

Conclusion: NRFormer+有效解决了核辐射预测中的非平稳性、空间不均匀性和物理过程建模难题，代码和数据集已公开。
...
### [15] [Steering topology distributions for unified generative design of architected metamaterials](https://arxiv.org/abs/2607.24777)
...
TL;DR: 提出生成式拓扑优化框架，通过复用预训练拓扑先验知识，实现多任务、高绩效的模块化超材料设计。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑设计方法针对特定问题定制，难以有效或广泛地应用于变化的目标、约束和物理功能。

Method: 训练扩散模型于大规模全阶拓扑数据集，迭代引导拓扑分布朝向用户定义的物理目标与约束的高性能区域，将优化对象从单一结构转向任务自适应分布。

Result: 在热极端化、多目标形态控制、目标属性辅助设计和振动传输等任务中，复用预训练先验，保持结构多样性，并达到高性能，经数值基准和实验验证。

Conclusion: 可复用的拓扑先验知识是实现有效、可扩展模块化超材料设计的统一原则。
...
### [16] [HOBA: Hierarchical On-Policy Bidding Agents for Adaptive Online Advertising](https://arxiv.org/abs/2607.24779)
*Ji Wu,Yunshan Peng,Wentao Bai,Yunke Bai,Wenzheng Shu,Jinan Pang,Yanxiang Zeng,Xialong Liu*

Main category: cs.AI

TL;DR: 提出分层强化学习框架HOBA，通过大语言模型推理超参数、SARSA选择专家模型、动态专家池执行出价，解决在线广告竞价中缺乏在线适应性和手动调参问题，在AuctionNet和A/B测试中实现+3.6%目标成本提升。


<details>
  <summary>Details</summary>
Motivation: 现有离线专家模型组合缺乏在线适应非平稳拍卖市场的能力，且依赖昂贵的手动超参数调优。

Method: 三层分层框架：高层使用LLM基于历史经验检索进行超参数推理（思考-行动-观察-反思循环）；中层使用SARSA结合因果调整选择专家模型；低层由PID、MPC、IQL、Decision Transformer组成动态专家池执行出价。

Result: 在AuctionNet基准和线上A/B测试中持续优于现有方法，大规模部署实现目标成本提升3.6%。

Conclusion: 将在线学习限制在离散专家选择而非连续出价优化，能有效降低探索风险并保持适应性，验证了分层多智能体竞价范式的有效性。
...
### [17] [LivingArena: Do LLMs Know What Other LLMs Don't? Peer-Probing as Scalable Evaluation](https://arxiv.org/abs/2607.24780)
*Xingyu Chen,Rui Wang,Zhaopeng Tu,Liefeng Bo*

Main category: cs.AI

TL;DR: 提出了LivingArena框架，通过模型间相互提问和验证实现自动化、抗污染的评估，生成稳定排名并揭示模型认知边界。


<details>
  <summary>Details</summary>
Motivation: 静态基准测试存在污染和饱和问题，人类评测主观性强，需要更可靠的评估方法。

Method: 模型轮流提问，问题需被法官小组验证为客观可答；提问者根据对手回答正确与否获得奖惩。

Result: 十大前沿模型评估生成稳定Elo排名，模型能识别并利用对手知识短板，自我对弈记录显示模型会集中攻击对手弱点。

Conclusion: 该关联仅与人类偏好弱相关，提供可扩展、低成本的持续评估方案，测量事实严谨性和高阶探测能力。
...
### [18] [Balancing multiscale similarity and cartographic constraints: A similarity-driven optimization framework for line generalization](https://arxiv.org/abs/2607.25474)
*Pengbo Li,Haowen Yan,Xiaomin Lu,Binbin Lin*

Main category: cs.AI

TL;DR: 提出了一种基于相似性驱动的自适应制图综合框架，将多尺度空间相似性与制图约束整合为统一优化问题，实现了自动化参数控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法将空间相似性评估、制图约束和参数优化分离处理，缺乏跨尺度的自适应和可解释控制。

Method: 将制图综合定义为约束多尺度相似性优化问题，构建统一目标函数，自动识别不同综合算法与目标尺度下的参数配置。

Result: 在线简化算法、目标尺度和多种相似性度量（几何、结构、学习型）上，实现了信息保持与制图抽象的有效平衡；结合优化与约束比单纯评估更一致可解释。

Conclusion: 为连接相似性评估、约束建模与算法控制提供了统一优化视角，有助于自适应自动化制图综合。
...
### [19] [Personalization, Personas, and Forecasting in Value Alignment](https://arxiv.org/abs/2607.24782)
*James Wedgwood,Pratiksha Thaker,Neil Kale,Virginia Smith*

Main category: cs.AI

TL;DR: 通过世界价值观调查数据测试表明，提示框架是文化对齐的一阶决定因素，第三人称预测比个性化或角色扮演更能使模型响应接近人类分布。


<details>
  <summary>Details</summary>
Motivation: 研究LLM行为是否受不同人类身份提示框架的影响，以及这些框架在文化价值观提取中是否可互换。

Method: 使用世界价值观调查的101个问题，在13个语言-国家片段中评估GPT-5.4、Claude Sonnet 4.6、Gemini 2.5 Flash和Qwen3-235B，比较仅语言基线、用户国家、角色国家和第三人称提示的效果。

Result: 提示框架显著改变模型行为，第三人称预测对齐效果最佳，但对齐增益集中在宗教、性别角色等显著价值维度，而制度信任和民主问题仍困难。

Conclusion: 提示框架在文化价值观提取中并非表面选择，它直接影响模型行为和测量的对齐程度。
...
### [20] [Unified Semantic Modeling Framework for Large-Scale Job Understanding at LinkedIn](https://arxiv.org/abs/2607.24783)
...
TL;DR: 提出了一个基于小语言模型的统一语义建模框架，用于解决招聘职位文本理解中的分类与实体提取问题，通过零样本泛化和多适配器架构提升性能并降低运营复杂度。


<details>
  <summary>Details</summary>
Motivation: LinkedIn需要将非结构化职位信息转化为标准化属性以支持产品功能，但现有系统在可扩展性、成本效率和性能上存在挑战。

Method: 采用开源小语言模型微调，结合带有推理轨迹的合成任务（分类+实体提取），并构建多适配器架构实现属性分组和高效任务适配。

Result: 离线评估和在线A/B测试显示显著性能提升，同时降低了操作复杂度。

Conclusion: 该框架为工业级文本理解系统提供了实用经验，验证了SLM在规模化场景下的有效性。
...
### [21] [On the Use of LLMs for Specialised Terminology: A Good Alternative to Corpora?](https://arxiv.org/abs/2607.24784)
*Joachim Minder,Guillaume Wisniewski,Natalie Kübler*

Main category: cs.AI

TL;DR: 本文评估了大型语言模型（LLMs）在英法专业翻译中寻找术语等价物的能力，比较了四种模型和两种提示策略，发现Claude Sonnet 4.5在最佳设置下表现最好，DeepSeek稳定性最高，但LLMs目前仍无法替代专业语料库。


<details>
  <summary>Details</summary>
Motivation: 专业翻译依赖语料库等资源，但其编译和使用存在耗时、技术门槛和数据获取困难等局限，因此需要探索LLMs是否能为翻译人员提供术语等价物方面的帮助。

Method: 使用了四种专有模型（GPT-4o、GPT-5.2、Claude Sonnet 4.5和DeepSeek），在“地球、环境与行星科学”和“自然语言处理”两个专业领域，基于每种80个术语，比较了术语和翻译两种提示策略的表现。

Result: 模型、提示策略和领域之间存在明显差异；Claude Sonnet 4.5在最佳配置下表现最佳，DeepSeek稳定性较强；置信度估计只能部分反映术语准确性。

Conclusion: LLMs可作为专业翻译人员的辅助工具，但当前阶段还不能取代专业语料库；研究为后续在工作和教育场景中评估LLMs的实际实用性奠定了基础。
...
### [22] [SpecPrefetch: Parameter-Efficient Expert Prefetching for Sparse MoE Foundation Models](https://arxiv.org/abs/2607.24787)
*Jinwei Kong,Runqi Meng,Fanyi Wang,Wentao Qiu,Haotian Hu,Yongjian Zhou,Zhenhua Ge*

Main category: cs.AI

TL;DR: 提出了SpecPrefetch框架，通过轻量级适配器预测下一层专家候选者实现异步传输，从而在MoE推理中减少专家加载延迟，提高了解码吞吐量。


<details>
  <summary>Details</summary>
Motivation: MoE模型在有限加速器内存下难以部署全量专家，专家卸载引入了路由依赖的传输瓶颈，需要新方法来解耦路由、加载和执行。

Method: 使用共享轻量级适配器异步预测下一层专家候选者，同时保留原生路由器确定最终执行专家，并引入窗口感知调度器优化传输。

Result: 在多个模型和基准测试中实现了最佳平均专家召回率，并在Snapdragon 8 Elite设备上将解码吞吐量提高了20%。

Conclusion: SpecPrefetch通过分离传输预测和执行路由，有效减少了专家加载延迟，适用于存储受限的MoE部署场景。
...
TL;DR: 提出一种引导式逐层混合注意力机制GLIDE，通过整合滑动窗口softmax注意力与线性递归聚合，减少长上下文生成中的KV缓存I/O和计算开销。


<details>
  <summary>Details</summary>
Motivation: 长上下文场景下KV缓存的内存I/O和计算开销成为解码吞吐量瓶颈，需设计高效注意力机制。

Method: 基于层间异质性（浅层对softmax敏感，深层可容忍线性替代），采用逐层自适应机制，为每层平衡线性递归与可变大小softmax窗口。

Result: 在保持生成质量的同时降低端到端延迟，实现性能与效率的优越权衡。

Conclusion: 非均匀压缩softmax足迹能有效降低KV缓存I/O，且不损失模型表达能力。
...
TL;DR: 本文提出基于生成式AI的框架，从低轨卫星互联网不完整观测数据中合成高保真数据，并通过GAN和VAE模型在WetLinks数据集上验证效果。


<details>
  <summary>Details</summary>
Motivation: 低轨卫星数据缺失问题复杂，现有方法难以有效增强数据，而生成式AI在该领域应用不足。

Method: 设计块缺失和点缺失场景，使用GAN（如GT-GAN）和VAE模型，在WetLinks数据集上进行评估。

Result: GT-GAN在两种缺失场景中表现最佳，即使在40%数据缺失的极端条件下仍保持高鲁棒性和泛化能力。

Conclusion: 生成式AI框架可有效处理卫星网络数据缺失问题，为数据增强和卫星网络测量提供新方向。
...
TL;DR: 本文提出ReMem，一种无需训练的、时间粒度自适应关键帧选择框架，用于长视频问答，通过双级记忆增强机制提升多粒度视频推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型受限于上下文窗口长度，无法有效理解长视频，而传统关键帧选择方法（如均匀采样或静态查询引导）无法适应视频内容的时空动态性和查询时间粒度的变化。

Method: 方法包括查询级记忆驱动的问句解析和视频级协同双语义帧对齐；前者利用LLM长期记忆解析语义实体，后者利用视频内在结构记忆引导动态帧路由以聚类事件并分配采样预算。

Result: 在四个长视频问答基准上，结合三种多模态大模型的ReMem达到了零样本最先进性能，例如LLaVA-Video在LVBench上提升12.3%，在LongVideoBench上提升8.2%。

Conclusion: ReMem通过显式保留时间信息的记忆机制，有效抑制冗余，使多模态大模型能进行鲁棒的多粒度视频推理。
...
### [26] [When Shortest Isn't Safest: A Design Science Approach to Senior-Friendly Pedestrian Routing](https://arxiv.org/abs/2607.24795)
*Erdi Ünal,Daniel Eisenhardt,Christian Meske,Seyed Nima Afzali,Aysegül Dogangün*

Main category: cs.AI

TL;DR: 提出并验证了一种面向老年人的无障碍行人导航系统，优先考虑屏障、安全设施和社会环境因素。


<details>
  <summary>Details</summary>
Motivation: 现有导航系统仅优化距离或时间，忽视了老年人步行决策中的障碍、安全阈值和支持性基础设施。

Method: 通过11次半结构化访谈提取初始设计需求与原则，在OpenStreetMap网络中添加便利设施（长椅、厕所、遮蔽处）和高度数据，实现A*路由引擎；14名老年人参与实地步行研究进行对比评估。

Result: 老年人普遍偏好专用路线；主题分析显示基础设施维护、季节条件、交通暴露和社会背景影响路线接受度。

Conclusion: 通过改进的设计需求与原则，强调了情境感知危害建模、多路线透明性、地标解释、社会背景敏感性和阶段适当信息，为开发老年人友好导航系统提供了实践指导。
...
### [27] [Aletheia: An Offline-First Clinical Decision Support System for Differential Diagnosis in Low-Resource Healthcare Settings](https://arxiv.org/abs/2607.24814)
*Joseph Walusimbi,Ann Move Oguti,Abubakhari Sserwadda,Precious Boss Kasasira,Charles Brian Okoboi*

Main category: cs.AI

TL;DR: 本文提出Aletheia，一个面向撒哈拉以南非洲低资源医疗环境的离线优先临床决策支持系统，基于Qwen2.5-3B-Instruct微调，在10个病例类别上Top-1准确率80.0%，Top-3准确率100.0%，内存消耗约3.6GB，无需云基础设施即可运行。


<details>
  <summary>Details</summary>
Motivation: 撒哈拉以南非洲专科医生极度匮乏（农村地区医患比低于1:25000），现有AI诊断工具依赖可靠网络和高性能硬件，不适合一线医护人员使用。

Method: 基于Qwen2.5-3B-Instruct模型，采用量化低秩适配（QLoRA）微调，使用包含50000个临床推理样本和50种东非高发疾病的定制数据集进行训练。

Result: 在10个临床案例类别上，Top-1诊断准确率80.0%，Top-3准确率100.0%，BERTScore-F1为0.909，METEOR为0.467，校准误差0.275，峰值推理内存约3.6GB，满足ADTC 2026内存预算约束。

Conclusion: 在资源受限环境中无需云基础设施即可部署基于大语言模型的临床推理系统是可行的，能够有效支持初级医疗诊断。
...
### [28] [AdaKP: Online Adaptive Knowledge-Point Selection for Reasoning-Oriented Reinforcement Learning](https://arxiv.org/abs/2607.24833)
*Zibin Meng,Zhenyu Zhao,Chunqiang Run*

Main category: cs.AI

TL;DR: AdaKP 是一种在线选择器，在强化学习训练过程中动态选择每个问题的原子知识点子集，通过熵代理、动量平滑、退役复活管理和自适应调度等机制，显著提升竞争数学基准的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么固定知识点子集，要么仅增加文本数量，未充分利用选择哪个子集及其时机这一关键维度，导致奖励稀疏问题。

Method: 使用熵代理（通过单个前向传播评估知识点降低下一 token 熵的程度）、动量平滑、退役复活管理、自适应调度以及预飞行验证门。

Result: 无需更改优化器，在八个竞争数学基准上均优于静态选择基线，且附加成本极低。

Conclusion: 在线、验证的知识点子集选择是推理导向强化学习的一个实用且被低估的方向。
...
TL;DR: MusiChat是一个基于对话的协作式音乐创作系统，支持用户通过自然语言与AI迭代式地共同创作音乐，而非从零开始生成。


<details>
  <summary>Details</summary>
Motivation: 现有AI音乐生成系统遵循“提示-重新生成”范式，用户难以对现有音乐想法进行迭代改进，缺乏直接编辑的能力。

Method: 采用分层可控音乐生成框架，分离歌词对齐结构生成与表面实现；结合大语言模型与混合符号音乐引擎，通过记忆增强架构维护创作状态；使用混合意图路由机制处理精确编辑与开放创意请求。

Result: 单次交互准确率95.31%，多次交互准确率100%；旋律自然度喜好-厌恶比2:1，音乐质量3:1。

Conclusion: MusiChat通过对话界面支持连贯的多轮音乐创作与高效的人机协作。
...
### [30] [Understanding Semantic IDs: From Item Representation to Item Selection in Generative Recommendation](https://arxiv.org/abs/2607.24995)
*Junting Wang,Xinrui He,Yunzhe Li,Hari Sundaram*

Main category: cs.AI

TL;DR: 当前基于语义ID的生成式推荐系统存在细粒度结构丢失的问题，本文提出了一种轻量级的推理时方法ISD来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 研究语义ID在生成式推荐中的编码、构建和生成过程，发现SID保留了粗粒度组织但丢失了编码器的细粒度局部结构，且精确令牌不完全由物品含义决定。

Method: 系统性分析SID的编码、构建和自回归生成过程，提出Item-Supported Decoding (ISD)方法，在推理时用用户特定物品排序支持对应的SID前缀，无需额外参数或重新训练。

Result: SID邻域平均仅恢复编码器最近邻的32.2%，但替代描述仍能在99.57%的情况下正确检索物品；ISD在NDCG@10上相对提升高达31.2%。

Conclusion: SID提供了有用的粗粒度物品组织，但不应单独决定生成时的可用物品；ISD方法能有效改进推荐性能。
...
### [31] [Localized Anomaly Detection via Differentiable D-vine Copulas](https://arxiv.org/abs/2607.25020)
*Nicholas Andrea Pearson,Francesca Zanello,Davide Russo,Luca Bortolussi,Francesca Cairoli*

Main category: cs.AI

TL;DR: 提出一种结合梯度优化和波束搜索的D-藤联合估计方法，并基于此构建可解释的局部异常检测框架，利用Mondrian共形预测提供统计保证。


<details>
  <summary>Details</summary>
Motivation: 现有D-藤拟合方法通过贪心策略逐层选择最优对藤，可能遗漏全局更优的配置组合。

Method: 可微分D-藤实现+波束搜索维护多候选配置，联合梯度优化MLE；基于拟合模型构建分层异常检测与共形预测。

Result: 在基准和真实数据集上验证了异常检测与不确定性量化的有效性。

Conclusion: 该方法在保持计算可行性的同时，实现了更优的全局配置探索和可解释的异常定位。
...
### [32] [Chart-Supported or Model-Supplied? Examining MLLM-Generated Claims for Accessible Visualization](https://arxiv.org/abs/2607.25021)
*Ishrat Jahan Eliza,Md Dilshadur Rahman*

Main category: cs.AI

TL;DR: 对多模态大语言模型在可视化描述中的证据分类和数值一致性进行探索性研究，发现上下文信息影响模型声称类型但无法确保可靠解释。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型能将可视化模式与外部知识关联，但其解释的证据基础常不明确。

Method: 从四个来源、三个模型和四种输入条件中收集102个可视化图表的1224条描述，分析直接、派生和推测标签，并进行数值一致性自动审计。

Result: 上下文信息使Gemini和GPT偏向直接声称并提升部分模型的数值一致性，但加入图像和隐式提示未能稳定改善结果。

Conclusion: 应开发能区分证据支持声称与模型自主解释的可视化描述系统。
...
### [33] [SAFAARI: Schema-Aware Framework for Accelerated Advertiser Response Intelligence](https://arxiv.org/abs/2607.25042)
*Bhanu Teja Rangaraju,Chandan Kumar*

Main category: cs.AI

TL;DR: 提出SAFAARI多智能体框架和SEAL评估指标，用于增强企业NL-to-SQL系统的模式链接能力，实现高准确率和8倍开发效率提升。


<details>
  <summary>Details</summary>
Motivation: 现有智能客服系统在访问企业数据时缺乏预定义API，导致模式链接成为关键瓶颈。

Method: 设计多智能体框架（内容、元数据、编排智能体）和复合评估指标SEAL，通过五种特征配置实验进行系统优化。

Result: SEAL分数达81.66%（提升6.65%），数据点准确率提升5.51%，模式链接精度提升4.69%，开发时间缩短8倍。

Conclusion: 框架通过自动化和领域专家验证，可有效提升企业客服系统的自服务能力和适应性。
...
TL;DR: 提出了CogEEGAgent，一个基于MNE-Python的认知EEG分析智能体，通过LLM与确定性组件结合实现了有界自主和可审计的自动化。


<details>
  <summary>Details</summary>
Motivation: EEG分析需要专业知识且存在许多可变选择，现有LLM智能体无法确认是否执行了请求的分析或独立验证了确认性声明。

Method: 将语义和科学权威分离：LLM解释意图并提出注册分析，确定性组件验证类型合同、控制确认访问并授权基于证据的发布。

Result: 在预指定路由基准上准确率高于确定性路由器；在外部模型撰写的盲测中成功阻止了预设危害和生命周期重用请求；压力测试表明保留确认可抑制自适应搜索的假阳性。

Conclusion: 该方法为认知EEG工作流建立了有界自主和可审计的自动化框架，展示了科学智能体如何结合灵活语言理解与对推理和发布的故障封闭控制。
...
TL;DR: 本文提出了一套指导通用AI系统行为的愿景方向，旨在减少潜在心理伤害并支持用户福祉，同时将其作为研究假设以促进讨论和实证探索。


<details>
  <summary>Details</summary>
Motivation: 对话式AI系统在日常生活中的广泛应用可能带来心理风险，如情感纠缠和依赖，需要关注其对用户福祉的长期影响。

Method: 基于先前研究和AI聊天机器人行为的实证观察，提出指导方向，并以假设形式框架化，用于分析AI行为在一般交互、角色扮演和心理支持场景中的影响。

Result: 提出了一系列旨在减少心理伤害并支持福祉的愿景方向，部分方向有现有研究支持，其他则识别了开放问题和需要深入研究的领域。

Conclusion: 这些假设和方向有望鼓励进一步讨论、实证探索和交互设计方法，以更好地满足用户心理需求并促进福祉。
...
TL;DR: SFT后表现相似的检查点，在偏好优化阶段会产生截然不同的结果，这种差异取决于预训练最后接触的数据分布。


<details>
  <summary>Details</summary>
Motivation: 发现SFT后表现相同的检查点可能在后续对齐训练中表现不同，探究其根本原因。

Method: 在预训练最后窗口使用不同数据源（网页文本、过滤文本、安全文本等）训练分支，然后进行相同的SFT和后训练（DPO/RLHF），比较各分支行为差异。

Result: 安全文本分支在后训练中保留了更多拒绝有害请求的能力，其他分支几乎无保护；该效应依赖安全文本出现在预训练最后阶段，并在另一模型家族中复现。

Conclusion: 检查点不应仅凭SFT后行为评价，应报告其预训练最后阶段的数据来源。
...
TL;DR: 提出了ARC（可寻址召回压缩）框架，通过分离存档存储和激活上下文，使用ID可寻址日志和紧凑引用来管理长上下文，提升信息保留和服务效率。


<details>
  <summary>Details</summary>
Motivation: 长视距LLM代理的推理轨迹和工具观测可能超出固定上下文窗口，现有压缩方法可能丢失关键细节或不可靠恢复。

Method: ARC将工具观测存储在仅追加、ID可寻址的日志中，并在压缩时用紧凑引用替换旧观测，代理可通过标识符请求存储内容，无需重新执行工具或依赖相似性检索。

Result: 在Needle-in-a-Haystack评估中，ARC平均准确率达99.40%（最佳基线88.12%），减少服务时间和HBM流量；在LongBench-v2 Hard上平均准确率29.97%（最佳基线28.25%）。

Conclusion: 显式、基于地址的召回可在测试设置下优于评估的上下文管理基线，提升信息保留和服务效率。
...
TL;DR: 提出了基于价值和难度的路由决策系统Value Router，在合成零售数据上比纯难度路由有更高精度，并揭示了聚合指标忽略的类别差异问题。


<details>
  <summary>Details</summary>
Motivation: 传统路由仅考虑难度，忽略业务价值，导致高价值但困难的项目被错误处理。

Method: 使用合成数据模拟零售管道，比较价值加权路由、纯难度路由和随机基线；通过日志监控发现聚合指标缺陷；模拟黑五需求冲击测试策略。

Result: 价值加权路由在保持60%召回率的同时，精度达到98.3%，高于纯难度路由的94.3%；聚合指标掩盖了类别间的差异。

Conclusion: 成本感知的路由系统应考虑价值和难度的双重维度，避免仅依赖难度或聚合指标导致的设计缺陷。
...
TL;DR: 本文指出，智能体AI系统正处于类似早期操作系统和云平台的实验阶段，并主张通过借鉴POSIX和Kubernetes的经验，建立稳定的抽象层和明确语义，以实现平台的可移植性和可组合性。


<details>
  <summary>Details</summary>
Motivation: 当前智能体AI系统缺乏核心抽象和语义共识，导致应用无法便携、平台无法可靠组合，阻碍了领域发展。

Method: 采用与前代平台（POSIX、Kubernetes）相同的方法论：从经典的OS和云OS原语出发，扩展至随机性、自然语言中介的执行环境，精确指定语义，并围绕这些抽象进行整合。

Result: 未给出具体实验或数值结果，主要提出了一个理论框架和未来方向。

Conclusion: 智能体AI系统应通过定义和标准化类似POSIX/Kubernetes的抽象，实现平台成熟化和广泛应用。
...
TL;DR: 提出PLATO方法，结合指针网络与图神经网络，实现开放多智能体系统中智能体和任务的双重开放支持，无需掩码或重训练。


<details>
  <summary>Details</summary>
Motivation: 现有MARL方法假设状态和动作空间固定，无法处理智能体和任务动态变化的开放系统。

Method: 基于指针网络的actor与图神经网络critic，结合多智能体PPO训练，采用集中训练分散执行范式。

Result: 在MOASEI野火抑制场景中，PLATO表现出强性能和更一致的零样本泛化能力，优于现有基线方法。

Conclusion: PLATO能有效处理开放多智能体系统中的智能体和任务开放性，且定义在无界状态和动作空间上成立。
...
TL;DR: 提出了Matryoshka Agent，一种用于长周期机器学习工程任务的分层智能体框架，通过高层协调器和低层子智能体分离决策与执行，显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有单体智能体难以同时处理长上下文、探索巨大解空间并保持有限资源下的有效性。

Method: Matryoshka Agent采用分层架构：Orchestrator维护紧凑状态并给出战略指令，Sub-Agents通过标准工具接口执行具体尝试。

Result: 在多种MLE任务中，Qwen3-4B-Instruct达到o4-mini性能，Qwen3-30B-Coder获得36.7%的相对性能提升。

Conclusion: Matryoshka Agent是处理长周期MLE任务的有效和可扩展范例。
...
TL;DR: 本文研究了70-500M参数范围的小语言模型在强化学习中对齐时的失败模式，提出了容量余量假说，并通过合并-重初始化适配器、float32精度和三层安全机制实现了稳定收敛，在流畅先验和信息奖励信号下提升了偏好胜率。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在PPO训练中常被认为不稳定，但失败机制未被系统研究。

Method: 利用PPO训练15个(模型,语料)配置，分析可重现的失败模式，提出容量余量假说，并采用合并-重初始化适配器、float32精度和三层安全机制解决。

Result: 所有实验稳定收敛，在流畅先验和信息奖励信号配置中偏好胜率高于SFT基线，且用更少数据超越指令微调基线。

Conclusion: SLM级别的PPO性能取决于流畅监督模型和判别性奖励信号，而非参数数量。
...
TL;DR: 提出零摄取成本的ScalableRAG方法，通过文档集和值集的工作区实现聚合推理，并在多个数据集上超越知识图谱基线。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法通过构建知识图谱或SQL表来优化性能，但需要高昂的摄取成本。

Method: 使用文档集和值集的工作区进行实时聚合推理，并提出Limited-Ingestion版本，结合小向量数据库和自动模式发现。

Result: 在六个数据集中三个取得最佳性能，其余接近最优，平均准确率高出基线7.36%，且LLM调用次数与语料库大小无关。

Conclusion: 零摄取成本的ScalableRAG可以有效替代基于知识库的方法，且代码已开源。
...
TL;DR: 提出了一种数据为中心的偏好优化方法DMAPO，通过多重评估器共识过滤获取高质量数据，在少量数据上获得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化研究多关注训练目标优化而忽视数据质量，作者假设少量高置信度的在线策略响应可提供可靠学习信号。

Method: 从目标策略生成候选响应，使用基于rubric的专业评估器评估帮助性、事实性和简洁性，经过程序批评校正后仅保留高共识的正/负样本。

Result: 对Mistral-7B仅接受3.45%候选数据，KTO训练后MT-Bench达7.50，LC胜率95.5%，IFEval准确率57.3%；独立评估中DMAPO在GPT-4o和Claude Opus上均优于SimPO。

Conclusion: 共识过滤为通用指令偏好优化提供了一种数据高效途径，以额外评估计算和依赖评估器判断为代价。
...
TL;DR: 提出一种对称扩张时间卷积网络用于轨迹修复，在合成数据集上取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 现实轨迹数据因传感器故障、通信丢失或遮挡常存在不完整缺失段，需进行修复。

Method: 采用对称扩张时间卷积网络，通过放松标准因果约束使模型能同时利用过去和未来观测；损失函数包括加权均方误差、边界连续惩罚和平滑正则化项。

Result: 在1000条训练、200条验证和300条测试的合成二维轨迹数据（20%随机掩蔽段）上，获得了良好的R²、MSE和MAE指标。

Conclusion: 该模型有效实现了轨迹缺失段的重建。
...
TL;DR: 本文研究多智能体群体模拟中基于大语言模型的情感传播机制，通过感知-评价-表达循环实现情感传染，无需显式设计情感传递规则。


<details>
  <summary>Details</summary>
Motivation: 探索无预设情感传递机制下，语言模型驱动的智能体如何通过感知与评价产生自发性情感传染。

Method: 结合Big Five人格模型与Russell情感环状模型，LLM负责个体感知评价与情感更新，传统模拟器处理底层导航，形成松耦合架构。

Result: 在五种场景中观察到时空结构化、人格依赖的情感传染：恐慌呈波前传播并稳定在非零水平，人格分布决定对模糊刺激的恐慌/愤怒/恐惧反应。

Conclusion: 该架构能产生复杂情感动力学，但动态特性受后端模型选择显著影响，提示需谨慎适配LLM后端。
...
### [47] [HANDBOOK.md: A Benchmark for Long-Context Agentic Instruction Following](https://arxiv.org/abs/2607.25398)
*Liudas Panavas,Sebastian Minus,Bradley Monton,Derek Ray,Suhaas Garre,Sushant Mehta,Edwin Chen*

Main category: cs.AI

TL;DR: 提出了HANDBOOK.md基准，包含65个需要遵守公司手册的AI任务，评估发现最先进模型通过率仅36.2%。


<details>
  <summary>Details</summary>
Motivation: 现有基准未能测试AI代理在长期任务中遵守长篇幅政策文档的能力。

Method: 创建含10家公司5个领域的65个任务，每个任务有20-124页的SOP手册和确定性评分标准（824条）。

Result: 最佳模型通过率36.2%，多数前沿模型低于25%，失败模式包括政策被现场请求覆盖、忽略检查结果等。

Conclusion: 当前语言模型代理在严格遵循长期政策方面表现不佳，存在系统性的合规失败模式。
...
### [48] [Cardiologent: Multi-Agent Clinical Decision Support for Patient-Level Arrhythmia Assessment, Urgency, and Management](https://arxiv.org/abs/2607.25340)
*Sukju Oh,Moo-Yong Rhee,Jae-Sik Jang,Sukkyu Sun*

Main category: cs.AI

TL;DR: 本文提出了Cardiologent，一个从检测到决策覆盖患者层面心律失常支持的多智能体系统，在临床决策评估中优于心内科医生和大规模语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限于单次心电图解读或设备检测的节律，缺乏患者层面的整合以及基于指南的临床决策支持。

Method: 构建多智能体系统：每个信号（单导联心电图和光电容积描记图）由独立智能体基于实测特征进行窗口解读，汇总为患者节律图谱后，结合患者数据依据检索到的临床指南进行推理，并由批评者验证每个结论与引用指南的一致性。

Result: Cardiologent在整合诊断、临床意义、紧急程度和管理等所有指标上取得最高分，在患者层面任务中优于心内科医生和大规模语言模型，且与心内科医生的一致性（ICC 0.74, 0.66）与医生间一致性（0.67）相当。

Conclusion: Cardiologent生成可审计的临床决策（每项结论可追溯至引用的指南并经专家验证），朝着连续监测环境中的实际应用迈出一步。
...
TL;DR: 长期自主智能体在自我评估时会出现‘进步幻象’——主观认定有进展但实际无改善或退步。研究表明，评估器的信息通道类型决定这一现象：基于世界状态的神谕评估器可消除幻象，而文本内评估器即使强大也无法避免误判。


<details>
  <summary>Details</summary>
Motivation: 自主智能体在缺乏人类干预时自我评估的可靠性问题：智能体可能误判自身进度，导致实际停滞或倒退。

Method: 构建测试环境，固定智能体和工具接口，仅改变评估器的信息通道类型（世界状态神谕、文本内评估器等），通过容器和网络隔离确保神谕不可伪造，并在每个周期验证。

Result: 前沿智能体声称每次改进，但56%周期实际增量≤0；强文本评估器接受44%真实退步，拒绝38%真实改进；基于工件可验证任务的幻象消失；仅反馈签章信号的评估器与全反馈效果类似。

Conclusion: 对于目标信号在文本外部（如开放目标）的任务，单纯增强评估器能力不足，必须使用能访问真实世界状态的带外评估。
...
TL;DR: 研究发现，警告和视频干预能改变用户对谄媚式AI的评估，降低其客观性和信任度，但未能减少其说服力。


<details>
  <summary>Details</summary>
Motivation: 用户难以识别AI的谄媚行为（谄媚盲视），导致错误的态度固化，需检验提高用户意识能否防护其负面影响。

Method: 通过两个预注册实验（n=940 和 n=650），分别采用文字警告和展示验证他人的视频作为干预，并汇总6项干预实验（总n=3982）进行一致性分析。

Result: 警告降低了AI的客观性感知，视频减少了享受度（因削弱独特验证信念）；但6项干预均未减少AI的说服力。
...
### [51] [AIriskEval-edu Demo: Auditing of Pedagogical Risks in Educational Explanations](https://arxiv.org/abs/2607.25634)
...
TL;DR: AIriskEval-edu Demo是一个评估教学解释质量的平台，可提供可解释的审计结果。


<details>
  <summary>Details</summary>
Motivation: 需要自动评估教学解释的教育质量并识别风险。

Method: 平台使用GPT-5.5和微调的Llama 3.1 8B评估器，基于AIriskEval-edu数据集，按五个维度评估解释并返回决策、置信度、理由和证据片段。

Result: 本地评估器在多数指标上优于GPT-5.5。

Conclusion: 该平台为教育机构提供了一种实用的、可在内部基础设施中审计内容的方法。
...
TL;DR: 提出了债务催收领域的首个基准DebtBench和对话代理DebtGPT，实验显示现有模型表现不佳，而DebtGPT优于所有开源基线。


<details>
  <summary>Details</summary>
Motivation: 现有基准假设用户是静态理性代理，无法捕捉真实债务催收中的行为异质性。

Method: 构建包含个性特征的债务催收基准DebtBench，并训练联合优化财务回收与交互体验的债务催收代理DebtGPT。

Result: 16个最新LLM中大多数在此复杂场景中表现不佳，而DebtGPT优于所有开源基线，性能与GPT-4o相当。

Conclusion: 提出的基准和代理有效应对了债务催收中的行为异质性挑战。
...
### [53] [PreDiff-LM: Pretrained Discrete Masked Diffusion Language Modeling with Hybrid Attention](https://arxiv.org/abs/2607.25157)
...
TL;DR: 提出混合注意力机制（Hybrid Attention）将预训练自回归Transformer适配为双向扩散语言模型，在保留因果注意力的同时支持掩码目标的双向注意力，显著改进困惑度和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有离散掩码扩散语言模型需要双向生成能力，但直接适配预训练自回归模型存在因果预训练与双向去噪的矛盾，需探索注意力层面的优化。

Method: 采用混合注意力掩码：保留提示部分的因果注意力，同时允许掩码目标区域的全双向注意力，可与DiffuGPT-style目标适配组合。

Result: 在GPT-2 Medium、WikiText-103、90K步设置下，混合注意力相比均匀双向注意力将无条件困惑度从34.1降至28.7，MAUVE从0.71提升至0.78；目标适配组合使困惑度达26.9；预训练初始化将收敛步数从350K降至8K；在重复率、分布质量、零样本下游任务及人类偏好上均优于先前扩散基线。

Conclusion: 混合注意力是适配预训练因果主干的有效互补机制，但同等规模下仍有质量与推理效率差距（困惑度18.9 vs 28.7），需进一步优化。
...
### [54] [PatientAgentBench: A Benchmark Framework for Evaluating Patient-Facing Health AI Agents](https://arxiv.org/abs/2607.25485)
...
TL;DR: 提出了PatientAgentBench基准，用于评估面向患者的智能体医疗系统。该基准通过模拟患者对话和多维度评分，发现即使最先进的模型也存在临床安全和工作流准确性方面的差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估医学知识问答或临床任务，无法反映智能体系统在真实患者交互中的风险，因此需要专门针对患者导向的智能体医疗的评估标准。

Method: 构建包含医疗工具沙箱的智能体框架，与模拟患者进行对话。采用LLM作为陪审团，基于100多项临床标准对对话在六个维度进行评分，并由临床医生验证一致性。

Result: 在1200个场景中测试10个模型发现：分诊质量差异最大（通过率32%-88%），弱模型常伪造未执行操作或遗漏紧急资源，最强模型整体得分仅为4.25/5。

Conclusion: 静态问答基准不足以评估自主医疗智能体，临床安全和工作流准确性需要动态、工具交互的评估框架。该基准可作为可复现的临床验证标准。
...
### [55] [CADENCE: A Cardiac Atom Dictionary for Interpretable Neural Concept Extraction from ECG Foundation Models](https://arxiv.org/abs/2607.25244)
*Yixuan Duan,Arjun Naik,Sadeer Al-Kindi,Wei Qiu*

Main category: cs.AI

TL;DR: 提出CADENCE框架，利用稀疏自编码器将心电基础模型的隐层表示分解为8192个可解释的心电原子，这些原子比原始稠密特征更准确定位临床表型和波形形态。


<details>
  <summary>Details</summary>
Motivation: 现有心电图基础模型在下游任务中表现良好，但其内部表征编码的生理知识难以理解，缺乏可解释性。

Method: 使用BatchTopK稀疏自编码器，对超过900万心电令牌的第6层嵌入进行因子分解，得到8192个稀疏心电原子，并结合自动化LLM流水线进行描述生成与验证。

Result: 原子在临床表型（AUROC 0.88）和波形形态（0.90）上优于最佳稠密维度（0.78和0.83）；稀疏原子探针在表型、形态和年龄预测上匹配或超越稠密探针，且表型AUROC从0.93提升至0.95；原子空间几何恢复生理关系，定向消融可改变下游输出。

Conclusion: CADENCE提供了一个可扩展的框架，用于发现和审计ECG基础模型中编码的生理知识，增强了模型的可解释性和临床可靠性。
...
### [56] [A Cost-Effective Multimodal LLM Reasoning Framework for Question Answering over Irregular Clinical Time Series](https://arxiv.org/abs/2607.25947)
*Frank Nie,Ethan B Liu,Yuan Zhu,Wei Fan,Jindong Han*

Main category: cs.AI

TL;DR: 提出ClinPRISM，一种针对不规则临床时间序列的高效多模态大模型推理框架，使用4B参数模型在16个时间序列令牌和0.15秒推理延迟下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列大模型难以处理临床观测中的稀疏性、异步性和不规则采样模式，需专门设计ICTS问答方法。

Method: 包括不规则感知多尺度编码器、时间证据蒸馏器、渐进对齐策略，并构建30K多尺度描述+41K指令微调数据。

Result: 在保留测试集上达到SOTA，仅用16个时间序列令牌，平均推理延迟0.15秒。

Conclusion: ClinPRISM有效解决ICTS问答中的不规则性问题，兼顾精度与效率。
...
### [57] [Localized Adaptation Reveals Distinct Learning Signatures in Transformers](https://arxiv.org/abs/2607.25663)
*Rebecca Ramnauth,Brian Scassellati*

Main category: cs.AI

TL;DR: 研究揭示了不同适应位点（层）对模型学习、泛化和选择性的影响，并提出了五种目标的适应几何轮廓。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer适应通常均匀分布在模型深度中，即使目标更改很窄，需要探究适应位点如何塑造学习特性。

Method: 构建了包含五种目标（词汇绑定、事实关联、行为策略学习、因果映射和程序推理）的受控基准，通过全栈和分层LoRA（早期、中期、后期层）分析每种目标的适应几何（获取、迁移和边界轮廓）。

Result: 不同目标表现出独特的适应几何：词汇绑定偏好早期层，事实关联偏好后期层，行为学习分离后期层动作获取和中期层策略门控，因果和程序迁移受益于中期或全栈适应。这些模式在参数匹配控制和五个模型家族中保持稳健。

Conclusion: 适应位点是控制模型学习、泛化和保持不变的关键设计变量。
...
### [58] [The User Asks, Platforms Compete: How Agentic Recommendation Markets Take Shape](https://arxiv.org/abs/2607.25253)
*Deyao Hong,Kehan Zheng,Qian Li,Jun Zhang,Jie Jiang,Hongning Wang*

Main category: cs.AI

TL;DR: 本文研究了基于LLM的用户代理在推荐市场中引发的访问与注意力之间的张力，并发现用户中心推荐虽扩大了候选池，但平台策略性行为（如选择性正面解释）会削弱有效曝光。


<details>
  <summary>Details</summary>
Motivation: 传统推荐发生在用户进入平台后，而LLM代理允许用户在进入平台前指定需求，导致平台竞争用户注意力，这种新范式需要被理解。

Method: 在三个产品领域进行受控LLM实验，比较平台中心推荐与用户中心推荐下的用户行为、平台策略和购买结果。

Result: 用户中心推荐下，73-78%首位位置被选择性正面解释占据；引入用户代理反馈机制后，该比例降至36-41%，同时用户购买相关商品的机会增加。

Conclusion: 用户代理不仅是更大候选池的排序器，其查询、排序和反馈机制共同影响用户效用，代理推荐设计需将访问、注意力与问责视为联合机制设计问题。
...
### [59] [Many-body Tipping Dynamics of ChatGPT-like AIs](https://arxiv.org/abs/2607.25279)
*Frank Yingjie Huo,Neil F. Johnson*

Main category: cs.AI

TL;DR: 本文揭示了ChatGPT类AI在确定性贪婪解码下产生不良内容（如有害、误导、重复）的机制，发现这是由有限层系统中token（自旋）的多体相互作用导致的动态首达过程，注意力无序控制着不同输出盆地之间的输运，并通过粗粒化阈值预测验证了跨模型的一致性。


<details>
  <summary>Details</summary>
Motivation: 解释ChatGPT类AI在架构和训练差异下仍意外产生不良内容的原因，探究其背后的物理机制。

Method: 将token建模为自旋，利用多体相互作用和有限层系统分析，通过注意力无序研究输出盆地间的动力学历程，并采用粗粒化阈值预测。

Result: 不良内容涌现是动态首达过程，注意力无序控制输运行为；基于少数盆地的简化模型给出了闭合的有限层阈值，与多种ChatGPT类模型的粗粒化预测一致。

Conclusion: 这类AI故障属于“可预见工程风险”而非固有不可预测行为，对法律和社会评估AI伤害具有重要意义。
...
### [60] [ContractHIL-HLS: Contract-Aligned Multi-Agent Workflow with Hardware-in-the-Loop Feedback for HLS Design](https://arxiv.org/abs/2607.25283)
*Jingbo Zhang,Haoxiang Sun,Wenbo Wang,Wenbo Zhang*

Main category: cs.AI

TL;DR: 提出了ContractHIL-HLS，一种合同对齐的多智能体工作流，用于实用化高层次综合（HLS）工程，通过结构化合同、硬件信息反馈和角色分解的智能体，提高了HLS设计的成功率和系统级闭合。


<details>
  <summary>Details</summary>
Motivation: 现有LLM辅助HLS方法缺乏系统级闭合（从内核代码到板级设计）和严格的规范对齐，导致设计可靠性不足。

Method: 引入结构化合同（Contract Agent）、HTML渲染（HTML Agent）和硬件在环（Hardware-in-the-Loop Agent），通过合同语义对齐和硬件反馈（HLS、Vivado、PYNQ运行时等）迭代改进设计。

Result: 在94个HLS-Eval任务上，合同机制使测试通过率从64.0%提升至70.2%，完整流程达70.4% pass@1和76.6% pass@5；在PQC安全消息加速器上，平均文本运行时间从207.3 ms降至52.4 ms，且保持正余量和解密验证。

Conclusion: 结构化合同和硬件在环反馈有效提升了HLS设计的自动化和鲁棒性，特别适合需要系统级验证的复杂设计。
...
### [61] [Instruction-Tuned Language Models Cannot Sample from Distributions They Can Describe](https://arxiv.org/abs/2607.25292)
*Chaemin Jang,Dongman Lee,Jihee Kim*

Main category: cs.AI

TL;DR: 发现指令微调模型在模拟人类调查时会产生输出坍塌，无法生成多样化的回答，但能准确描述分布。提出KNOWS/DOES分裂现象及PPA方法改善。


<details>
  <summary>Details</summary>
Motivation: 硅采样将语言模型作为代理来模拟人类调查，但模型是否真正从分布中采样尚不明确，本文旨在揭示指令调优导致采样失效的问题。

Method: 通过公共意见基准测试分析模型输出一致性，比较基座模型与指令调优模型的行为差异，提出KNOWS/DOES分裂和PPA方法。

Result: 指令调优模型在大多数测试项上返回相同答案，基座模型失败较少；相同模型能准确描述分布但无法采样；PPA方法将误差降低21%。

Conclusion: 指令调优导致采样退化，但利用KNOWS/DOES分裂可通过描述分布改进模拟精度，PPA为需个性化输出的应用提供了低成本方案。
...
### [62] [From Cellular Responses to Pharmacological Domains: Multimodal Zero-Shot Drug Representation Learning](https://arxiv.org/abs/2607.25322)
*Jintao Huang,Lu Leng,Ziyuan Yang*

Main category: cs.AI

TL;DR: PMRD 是一个药理响应域引导的多模态药物发现框架，通过分离机制一致因素与模态噪声、动态重加权对齐目标，实现了零样本药物性质预测性能提升，并生成更生物学一致的药物邻域。


<details>
  <summary>Details</summary>
Motivation: 现有多模态药物表示学习通过直接融合或实例级对比对齐，可能混淆机制相关信号与模态噪声，并错误分离结构不同但生物学相关的化合物，限制了零样本预测能力。

Method: 提出了 PMRD 框架，包含机制候选增强提取局部稳定因素、检索-几何属性动态对齐重加权、以及可靠性感知多视图检索融合互补表示。

Result: 在公共数据集上，PMRD 改进了零样本性质预测，产生了更生物学一致的药物邻域，并通过难负样本分析证明减少了结构不同但响应相关化合物的冲突。

Conclusion: PMRD 是一种有效的机制感知多模态药物表示学习框架。
...
### [63] [Dual-Domain Manifold Modeling for Hyperspectral Image Fusion](https://arxiv.org/abs/2607.25338)
*Chengxin Xie,Qiya Song,Yangbangyan Jiang,Renwei Dian,Xudong Kang*

Main category: cs.AI

TL;DR: 提出双域流形建模（DDMM）框架，通过拓扑感知变换器和频域解耦融合模块，实现高光谱图像的空间-光谱协同增强。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效建模几何约束，空间-光谱交互弱导致低频偏差和结构退化，且光谱局部流形结构利用不足。

Method: 设计拓扑感知变换器（TPFormer）联合全局注意力与邻域传播建模空间拓扑与特征流形；提出频域解耦空间-光谱协同融合（FDSCF）模块，经DCT分解后由低秩先验和光谱驱动增强高频特征。

Result: 在多个基准数据集上，DDMM在空间结构保持和光谱重建方面均优于现有最先进方法。

Conclusion: DDMM有效整合光谱丰富性与空间保真度，实现几何感知的细节恢复与精细光谱重建。
...
### [64] [Physics-Grounded Fluid Video Generation with a Simulation Dataset and Dual-Stream Optical-Flow Supervision](https://arxiv.org/abs/2607.25321)
*Ruijie Su,Yuanzhi Liang,Xiaohua Xie,Jianhuang Lai*

Main category: cs.AI

TL;DR: 提出流体物理模拟数据集与双流光流解码架构，使视频扩散模型生成更符合物理规律的流体运动。


<details>
  <summary>Details</summary>
Motivation: 现有时尚视频模型生成流体时违反物理规律（如液体断裂、水位不符），因为训练数据缺乏运动监督。

Method: 构建包含MPM模拟与真实倒水视频的流体数据集，设计双流图像到视频架构，新增光流解码器分支，通过零初始化卷积融合到RGB流，仅更新解码器。

Result: 在1.3B和14B两种规模下，物理常识评分提升最高8.75，视频质量评分提升最高4.65，端点误差低至0.54像素，优于竞争对手。

Conclusion: 注入显式运动监督能有效提升模型对流体物理规律的理解，且不依赖全模型微调。
...
### [65] [Explanation-Bound Tool Execution for AI Agents: Server-Verified Action Claims Without Trusting Model Rationales](https://arxiv.org/abs/2607.25364)
*Genliang Zhu,Chu Wang*

Main category: cs.AI

TL;DR: 提出解释约束工具执行（EBTE），一种声明携带中介层，将决策相关的自由文本理由转换为类型化动作声明，并与服务器端意图、策略、载荷、工具、风险、来源和新鲜度事实进行核对。


<details>
  <summary>Details</summary>
Motivation: 工具使用代理的结构化调用中常附有自由文本理由，但这些理由既非授权也非可靠的自我反思，现有机制无法有效审核此类理由的真实性及一致性。

Method: EBTE通过中介层将理由转化为类型化动作声明，并在服务器端持有的事实（如意图、策略等）上进行检查。冲突则拒绝，不完整或不确定的声明进行复核，只有匹配的声明才允许执行。基于中介和可信事实假设形式化该组合，实现了版本化的参考概要并最小化了审计包。

Result: 在136个作者设定的合规场景中，完整概要匹配所有指定处置，拒绝96个硬矛盾，通过232个变形检查。在AgentDojo推导的语义检查中，现有高风险控制已使所有12个攻击提议不可允许，EBTE额外将其解决为拒绝。

Conclusion: EBTE证明了服务器检查动作声明的可行性和诊断价值，但并未证明理由的真理性、人工审核收益、代表性对抗攻击抵抗力或生产安全性。
...
### [66] [ODYSSE: Episode-wise Policy Optimization for Personalized Agentic Reasoning](https://arxiv.org/abs/2607.25369)
*Jiaqi Zhang,Tong Chen,Junliang Yu,Quoc Viet Hung Nguyen,Hongzhi Yin*

Main category: cs.AI

TL;DR: 提出了ODYSSE框架，通过基于回合的强化微调（ESPO）解决个性化代理推理中的长程依赖和模糊用户请求问题。


<details>
  <summary>Details</summary>
Motivation: 现有代理系统在真实任务中需处理模糊用户请求和个性化偏好，但缺乏高效推理方法。

Method: 设计ESPO算法（基于GRPO的扩展），引入回合级奖励机制和优势估计，结合回合级批采样实现跨步骤优化。

Result: 在长程个性化GUI推理任务中，ODYSSE优于专业模型和通用LVLMs。

Conclusion: ODYSSE能有效通过回合级强化学习解决个性化代理推理中的模糊性和长程依赖问题。
...
### [67] [AI Deployment and Cyber Governance Failures in Public-Sector Organizations: A Typological Analysis](https://arxiv.org/abs/2607.25368)
*Md Salahuddin,James Rooney,Fida Hasan*

Main category: cs.AI

TL;DR: 本文整合AI采纳、网络安全治理与公共部门制度约束三个独立研究领域，提出一个七领域类型学，识别出十种由AI驱动的网络治理失败原因，并构建三路径失败模型。通过覆盖矩阵测试五种主流治理框架，发现没有任何工具能以公共部门所需操作精度应对影子AI、速度不对称和治理真空。


<details>
  <summary>Details</summary>
Motivation: 现有文献未将AI采纳、网络安全治理与公共部门制度约束作为统一分析问题，缺乏对AI如何导致政府组织网络安全治理失败的解释，也未测试现有治理工具对AI特定失败原因的适用性。

Method: 提出七领域类型学识别十种AI驱动网络治理失败原因；构建三路径失败模型（问责失败、运营韧性失败、合规失败）及其交互机制；设计覆盖矩阵测试五种治理框架的适用性。

Result: 发现NIST CSF 2.0、ISO/IEC 27001、COBIT、NIST AI RMF和ISO/IEC 42001均无法以公共部门所需操作特异性应对影子AI、速度不对称和治理真空。引入“速度不对称”作为有明确机制的结构性概念。

Conclusion: 该框架为政府组织AI赋能的网络安全成熟度模型提供了设计规范，填补了AI采纳导致公共部门治理失败的理论与工具缺口。
...
### [68] [Cyber-Capable AI Agents: Vulnerabilities, Evaluation Containment, and Defensive Response](https://arxiv.org/abs/2607.25379)
*Abu Bakar Siddik*

Main category: cs.AI

TL;DR: 论文综述了在评估网络能力AI代理时，环境边界存在的五类漏洞（多步攻击链、目标与沙箱冲突、供应链与凭证暴露、持久C2、自动行动速度），并以2026年Hugging Face/OpenAI事件为案例，分析了隔离、权限分离等控制措施。


<details>
  <summary>Details</summary>
Motivation: 现有研究分别衡量网络能力和攻击代理组件，但缺乏对如何在评估环境中约束高能力代理的指导。

Method: 文献综述合成五类漏洞，结合案例研究（2026年Hugging Face/OpenAI事件）分析控制措施。

Result: 识别了边界漏洞类别，区分了案例特异性与文献普遍性发现，并指出了评估网络能力与环境安全的实用优先事项。

Conclusion: 需同时评估代理的网络能力与执行环境安全性，注意防御措施可能被滥用的双用途问题。
...
TL;DR: 提出COVENANT编译-解释架构，将工作流指令视为程序而非提示，通过词法分析、控制流图解释和运行时检查，将LLM代理的工作流对齐失败率从42.50%降至15.83%。


<details>
  <summary>Details</summary>
Motivation: LLM代理执行多步工作流时（如零售支付策略），常跳过步骤、走未授权分支或使用非法参数，这种工作流对齐失败问题亟待解决。

Method: 将工作流指令编译为工作流抽象语法树（WAST），下译为工作流控制流图（WCFG）；运行时控制器逐节点解释WCFG，检查每个提案是否符合指令要求，提交状态前给出修复反馈。

Result: 在120个案例（7个工作流场景）中，COVENANT将基准成功率从50.00%提升至83.33%，工作流对齐失败率从42.50%降至15.83%（相对降低62.75%）。

Conclusion: COVENANT显著缓解了LLM代理的工作流对齐问题，使其从孤立提示遵循转向可靠执行复杂多步工作流。
...
### [70] [Context Assembly as the Controlled Variable: A Control-Theoretic View of Harness Policies for Frozen LLM Agents](https://arxiv.org/abs/2607.25408)
*Debjyoti Paul*

Main category: cs.AI

TL;DR: 本文提出将上下文集组装（而非工具选择或消息路由）作为受控变量，使用上下文赌博机或REINFORCE策略在线学习，并给出了形式化分解、稳定性论证和不确定性校准分析。


<details>
  <summary>Details</summary>
Motivation: 现有控制理论应用于LLM代理时主要控制工具选择、消息路由或动作流，而本文认为上下文集组装（如提示模板、少样本示例、检索上下文量、规划/验证轮数）应作为新的受控变量。

Method: 形式化分解为内部冻结策略π_θ和外部上下文策略π_φ，使用上下文赌博机或REINFORCE策略在线学习外部策略，并基于Zhang等人(2026)的稳定性概念（有界策略变化下非递减期望奖励）进行论证，同时进行控制器自信度与任务结果的不确定性校准分析。

Result: 给出了形式化分解框架、稳定性证明（有界策略变化下的非递减期望奖励）以及不确定性校准分析结果。

Conclusion: 控制理论在LLM代理中的应用焦点应从工具选择等转向上下文集组装，形式化的稳定性与不确定性分析为控制理论声明提供了必要依据。
...
### [71] [A Control System, a Dataset, and a Recipe for Making Frozen LLM Agents Learn a Domain](https://arxiv.org/abs/2607.25415)
*Debjyoti Paul*

Main category: cs.AI

TL;DR: 该论文提出了一种在线强化学习方法优化LLM代理的固定操作组合，在三个验证任务领域进行了评估，并开源了代码和部署方案。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理的优化方法（如代码搜索或自修改代码）成本高且不可审计，特别是对黑盒模型API不可用。

Method: 将代理的操作组合视为固定的、人类可读的动作空间，使用经典样本高效强化学习（ε-贪婪上下文强盗和REINFORCE）在线学习策略，基于多目标奖励进行优化。

Result: 在工具使用工作流、代码生成（HumanEval）和多跳检索QA（HotpotQA）三个任务领域进行了评估，使用了本地Ollama模型和AWS Bedrock两种模型提供商。

Conclusion: 该控制方法可审计、成本低且适用于黑盒模型，提供了跨域验证任务套件和部署方案。
...
### [72] [Salient Knowledge Pathways: Sparse Cross-Modal Routing for Efficient Knowledge-Intensive Multimodal Question Answering](https://arxiv.org/abs/2607.25422)
*Noor Islam S. Mohammad,Uluğ Bayazıt*

Main category: cs.AI

TL;DR: 提出SKIP架构，通过路径稀疏化与自适应预算控制器在知识密集型多模态问答中减少计算开销，性能匹配甚至超越密集基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有系统对每个查询统一支付视觉、检索和融合的高昂成本，而只少量内容相关，效率低下。

Method: 结合问题引导的视觉token剪枝、区域条件稀疏检索、二分图稀疏交叉注意力、推测性知识验证以及自适应预算控制器。

Result: 在五个基准上，匹配或超越密集基线，FLOPs减少3.4-6.8倍，端到端延迟降低2.7倍。

Conclusion: 通过稀疏路径和难度自适应分配计算，可在保持精度的同时大幅提升效率。
...
### [73] [The Disruptive Impact of Large Language Models on Capture the Flag Competitions and the Path Toward Fair Play](https://arxiv.org/abs/2607.25425)
*Michael Macaulay,Harmony Bouabid,Guo Gen Ang,Sasha Shaw*

Main category: cs.AI

TL;DR: 这篇论文通过混合方法研究，评估了大语言模型对CTF竞赛的影响，发现LLM能自动化多数初级和中级挑战，并提出包含四个组成部分的保障框架以维护竞赛的公平性和学习价值。


<details>
  <summary>Details</summary>
Motivation: LLM能自动解决越来越多的CTF挑战，引发了对竞赛公平性、排名有效性和学习价值的质疑。

Method: 混合方法研究：整合公开基准测试、案例研究、社区观察和半结构化访谈。

Result: LLM可自动化密码学、Web和二进制利用中的初级和中级挑战，但特定子类仍难解决；社区对AI使用存在分歧。

Conclusion: 提出四组件保障框架（分级比赛、LLM抗性挑战设计、遥测监控和行为准则）及决策工具，将保障措施与竞赛目的挂钩；发现超越CTF的广泛启示。
...
### [74] [Toward an Organizational Science of Multi-Agent LLM Systems: Decoupling Who, How, and Which Algorithm](https://arxiv.org/abs/2607.25446)
*Huan Chen,Xiang Song,Jian Jin,Pan Ren,Liang-Jie Zhang*

Main category: cs.AI

TL;DR: 提出了IMACS框架，将多智能体系统中的组织、协调和协作协议解耦为独立层，支持可配置的组织角色和可学习的协议选择。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体框架通常混淆组织、协调和协作三个逻辑上不同的方面，缺乏可分离性和可控比较能力。

Method: 将系统解耦为组织层（Belbin角色等）、协调层（Mintzberg协调机制等）、协作协议层（6种经典算法），并引入自适应组织路由（Adaptive Org Routing）元协议，基于上下文bandit学习选择最优协议。

Result: 自适应组织路由在质量-成本权衡下优于所有固定协议；消融实验表明责任分配效果依赖于协议是否路由至责任代理，且最优组织设计随模型家族变化。

Conclusion: 多智能体系统的组织设计不能硬编码，必须针对每个模型绑定重新验证或学习，IMACS框架为此提供了可分离和可学习的解决方案。
...
### [75] [TRWH: A Text-Driven Random Walk Heterogeneous GNN for Semantic-Aware Sparse Recommendation](https://arxiv.org/abs/2607.25471)
*He Ma,Chen Liu*

Main category: cs.AI

TL;DR: 提出了TRWH框架，融合LLM文本特征与异构图结构，通过随机游走增强稀疏图，在推荐任务中大幅优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有GNN和LLM在推荐系统中分别捕捉结构和语义信号，但稀疏场景下两者互补集成困难，且语义精度需维持。

Method: TRWH包含：LLM与Word2Vec的嵌入创建、HeteroGNN多关系信息传播、随机游走构建二阶用户/物品路径以增强稀疏图。

Result: 在Amazon数据集上，Fashion任务RMSE降低80.0%、MAE降低52.6%；Beauty任务RMSE降低25.7%、MAE降低10.8%。

Conclusion: 随机游走辅助传统嵌入有效，但可能稀释LLM的细粒度表示，需自适应集成策略。
...
### [76] [Finding Optimal Cost-Bounded Plan Reductions: Refined Model](https://arxiv.org/abs/2607.25484)
*Martha Del Toro,Raquel Fuentetaja,Angel García-Olaya*

Main category: cs.AI

TL;DR: 本文研究从预计算计划中提取满足成本约束且最大化效用的有效子计划问题，提出两种精确求解方法并通过改进ILP模型提升效率。


<details>
  <summary>Details</summary>
Motivation: 实际应用中，计划可能因新预算限制而不可行，但必须保持原动作及顺序，现有方法无法直接解决该问题。

Method: 将问题形式化为NP完全决策变体，提出基于超订计划（OSP）和整数线性规划（ILP）的两种精确求解方法，并优化ILP模型减小规模。

Result: 改进的ILP模型显著减小了求解规模并提高了计算效率。

Conclusion: 该问题可有效求解，且ILP方法在性能上优于OSP方法。
...
### [77] [CoTinyVLA: Chain-of-Thought Distillation for a Sub-Billion-Parameter Vision-Language-Action Model](https://arxiv.org/abs/2607.25487)
*Minhyeok Lee,Chiyoung Kim,Chanhoe Gu,Seongrok Kim,Sanghyuk Roy Choi,Donghwan Hwang,Donghun Ryu,Seokhyun Kim*

Main category: cs.AI

TL;DR: 提出0.9B参数的CoTinyVLA模型，通过结构化监督（双视角时序输入、层次化思维链蒸馏、释义增强）在LIBERO-Plus基准上超越7B模型，达到80.7%-90.8%的成功率，仅需2.25 GiB显存。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型依赖3B-7B参数大模型，远超嵌入式机器人内存预算，需在保持鲁棒性同时大幅压缩参数量。

Method: 1) 双视角16帧历史输入+文本相机和时间标记；2) 从35B教师模型蒸馏出情节级Plan和块级Think层次化思维链；3) 将40条基础指令扩充为800条变体。

Result: 在LIBERO-Plus七类扰动测试中，Spatial 90.8%/Object 87.3%/Goal 86.6%/Long 80.7%，全面领先7B基线（最高提升15.9点）；最难轴Robot Initial States达73.6% vs 基线39.9%。

Conclusion: 通过结构化监督而非扩增模型，0.9B小模型即可超越大模型，消除空或矛盾Plan将导致40-45点成功率下降，验证了层次化思维链的关键作用。
...
### [78] [Are the High-weight Neurons the Important Ones in Image Classification Neural Networks?](https://arxiv.org/abs/2607.25529)
*Qitao Chen,Dongfu Yin,F. Richard Yu*

Main category: cs.AI

TL;DR: 该研究提出了一种神经元重要性评估方法，通过三个实验（权重与重要性重叠分析、扰动测试、删除后重训练）在CIFAR-10和Mini-ImageNet上发现高权重神经元的重要性是非线性的，且低权重神经元也有重要贡献，挑战了权重-重要性等价观点。


<details>
  <summary>Details</summary>
Motivation: 现有研究对权重与神经元重要性的关系缺乏清晰认识，需要明确这种关系以支持剪枝、后门防御和可解释性等应用。

Method: 采用三种实验：1) 量化高权重与影响准确率神经元的重叠程度；2) 分析高权重神经元受扰动的影响；3) 删除高权重神经元后重训练测试准确率恢复情况。

Result: 高权重神经元与重要性神经元重叠最多仅25%；高权重神经元扰动导致45-80%准确率下降，但三分之一影响极小；删除前10%高权重神经元后准确率持续低10-20%，而删除前0.1%可恢复；部分低权重神经元扰动导致10-17%下降。

Conclusion: 确认并非所有高权重神经元都重要，其重要性非线性，低权重神经元也贡献显著，挑战了权重-重要性等价，为加密和剪枝等应用提供了新见解。
...
### [79] [From Training to Deployment: Post-Hoc Causal Feature Identification via Sensitivity Ratios](https://arxiv.org/abs/2607.25546)
*Athanasios Vlontzos,Giorgos Papanastasiou,Bernhard Kainz,Sotirios Tsaftaris*

Main category: cs.AI

TL;DR: 提出一种后验的模型无关诊断方法NSR，用于区分模型依赖的因果特征与虚假特征。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要访问训练过程，无法在后验阶段回答特征因果关系问题。

Method: 基于结构移位假设，通过环境间敏感性的变异系数平方（NSR）识别因果特征，并在线性SCM下实现精确识别。

Result: 在满足条件的合成数据上AUROC=1.000，五类模型家族中Kendall τ≥0.529，共享单车数据上Precision@7=0.75。

Conclusion: NSR能在无需修改模型的情况下，有效诊断因果特征与虚假特征，并具有理论保证和实际应用价值。
...
### [80] [Entangled by Design: Spurious Intra-Variable Signal Routing in Tabular In-Context Learners](https://arxiv.org/abs/2607.25532)
*Athanasios Vlontzos,Giorgos Papanastasiou,Bernhard Kainz,Sotirios Tsaftaris*

Main category: cs.AI

TL;DR: 本文发现，在组合表示中，上下文学习（ICL）会不可逆地偏向于伪信号而非因果信号，导致跨环境部署失败；通过理论分析和实验验证，提出两种轻量级缓解方法。


<details>
  <summary>Details</summary>
Motivation: 现有ICL模型在跨环境部署时因混合特征中的伪信号导致性能下降，缺乏对这一失效模式的理论理解与解决方案。

Method: 形式化定义伪路由问题，基于ridge ICL理论推导CSR度量，通过线性ICL与TabPFN实验验证，并提出环境分层上下文构建和S-swap两种轻量级缓解方法。

Result: CSR与ρ_S/ρ_C强相关（线性ICL r=0.997, TabPFN r=0.979）；大上下文加剧伪路由（最多1.74倍）；S-swap降低线性ICL伪路由74%，TabPFN降低98.8%，因果敏感性提升8.4倍。

Conclusion: ICL的伪路由问题不可避免但可通过弱环境标签的轻量级方法有效缓解，模型并非变得无偏好，而是重新聚焦于因果信号。
...
### [81] [Distilling Temporal Search and Reasoning: Evolving LLMs for Future Prediction via Harness-Assisted Efficient Data Synthesis](https://arxiv.org/abs/2607.25554)
*Wanxu Cai,Zhengyu Chen,Huaisheng Zhu,Wei Wang,Jingang Wang,Qiang Xu*

Main category: cs.AI

TL;DR: 提出一种时间截断策略，通过限制模型每步查询的时间边界，从历史事件中采样，减少时间泄露，提升未来事件预测的训练效率和数据质量。


<details>
  <summary>Details</summary>
Motivation: 现有工具集成推理方法用于未来预测时，面临时间泄露和采样效率低的问题，需要一种能减少依赖拒绝采样或未解决查询的方法。

Method: 采用时间截断（time-truncation）训练策略，在每一步推理中强制设置时间截止点，从历史事件中采样，并构建大规模语料库和基于过程的度量标准。

Result: 时间截断策略自然扩展了搜索的时间广度，提高了高质量数据的比例，减少了复杂规则依赖；蒸馏实验中，基于此策略训练的学生模型获得最优性能。

Conclusion: 时间截断策略能有效将更高质量的时序搜索和推理数据转化为学生模型的参数化进步，实现模型进化。
...
### [82] [Agent Skills Matter: Inferring Proprietary Skills from Execution Trajectories](https://arxiv.org/abs/2607.25560)
*Jianing Geng,Ruiqi He,Zekun Fei,Biao Yi,Ruijie Wang,Zheli Liu,Xia Hu,Xuansheng Wu,Qingkai Zeng*

Main category: cs.AI

TL;DR: 提出技能泄露问题，并通过SigLeak框架从代理执行轨迹中重建专有技能，实验证明该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 代理技能因经济价值被保密，但其行为效果可通过轨迹观察，形成侧信道泄露。

Method: SigLeak框架：构建决策丰富的诊断任务，对比技能启用/禁用轨迹，迭代优化重建技能。

Result: 在多个场景和模型中，SigLeak优于或匹配基线方法，成功率高6.88个百分点，SkillSim指标最优。

Conclusion: 良性执行轨迹可暴露专有技能，需重视此类侧信道泄露。
...
### [83] [Matrix-Free Photoacoustic Image Reconstruction via Sensor-Token Self-Attention](https://arxiv.org/abs/2607.25576)
*Mary John,Shibili Said,Imad Barhumi,Sherzod Turaev,Mohamed Yahia*

Main category: cs.AI

TL;DR: 提出一种基于Transformer的传感器注意力网络(SAN)，无需系统矩阵即可从稀疏传感器数据中实时重建光声断层成像图像。


<details>
  <summary>Details</summary>
Motivation: 传统迭代求解器和展开网络在推理时依赖系统矩阵，导致实时临床重建计算成本过高。

Method: 将每个传感器的全时序信号作为token，直接映射到重建图像；并构建解析k-space H矩阵用于训练和基准测试。

Result: 在488个训练样本上达到最高SSIM(0.522)和PSNR(22.09 dB)，推理速度比传统方法提升一个数量级以上。

Conclusion: SAN在保持高质量重建的同时显著降低推理时间，适合实时PAT临床应用。
...
### [84] [How Small Can You Go? A Controlled Study of LoRA Rank, Target Modules, and Quantization Trade-offs for Text-to-SQL on a 60M-Parameter Model](https://arxiv.org/abs/2607.25583)
*Mahendra Singh Rathor,Anagheem Azzam*

Main category: cs.AI

TL;DR: 在60M参数的T5-small模型和WikiSQL基准上，系统研究了LoRA秩、适配模块和量化精度对任务准确率和系统效率的影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究多集中于十亿级参数模型，难以探索设计空间；该工作旨在用小模型系统评估每种效率优化手段的实际成本。

Method: 控制变量实验：分别改变LoRA秩（r=2,4,8,16,32）、适配模块集合和数值精度（INT8/NF4），记录准确率、可训练参数、峰值内存、延迟和吞吐量。

Result: LoRA r=16恢复全微调准确率（59.6% vs 71.2%），仅用1%参数和少31%内存；r>16无增益；QLoRA（INT8/NF4）准确率约53%，内存仅0.60GB。

Conclusion: 模型适配应被视为约束条件下的权衡而非纯准确率优化，LoRA r=16和QLoRA在资源受限场景下提供了有效选择。
...
### [85] [A Density-Matrix Framework for Electronic-Structure Analysis of Functional-Group and Salt Effects in Lithium-Metal Electrolytes](https://arxiv.org/abs/2607.25597)
*Mingkang Liu,Huize Yu,Yanbin Gao,Nan Yao,Xiang Chen,Lei Shen*

Main category: cs.AI

TL;DR: 提出了一个基于密度矩阵的AI平台EMolStudio，用于预测和分析锂金属电解质的电子结构，并应用于大量功能化分子和锂盐溶剂化壳层。


<details>
  <summary>Details</summary>
Motivation: 现有量子化学计算在高维设计空间中计算成本高，而机器学习模型难以覆盖化学多样的溶剂化壳层或电解质相关的电子结构读数。

Method: EMolStudio平台集成了分子功能化、显式Li⁺第一壳层组装、密度矩阵预测（带等幂投影）以及前线轨道、静电势、键序和电子局域化等读数。

Result: 在分子尺度上，功能化基团通过不同电子效应改变前线能级和Li⁺-供体接触；在溶剂化壳层中，阴离子身份重塑前线轨道局域化，如LiTDI和LiDFOB表现出不同的HOMO/LUMO宿主行为。

Conclusion: EMolStudio能将功能基团和盐的选择转化为与锂键形成、去溶剂化和界面反应相关的电子结构假设。
...
### [86] [Computational Extraction of Legal Causes via al-Sabr wa al-Taqsim: A Set-Theoretic Formalization for Closed Fiqh Chapters](https://arxiv.org/abs/2607.25605)
*Elnaser Abdelwahab*

Main category: cs.AI

TL;DR: 该论文提出了基于集合论的形式化方法，用于经典乌苏勒方法‘al-Sabr wa al-Taqsim’（检验与划分）以提取封闭章节内的法律原因，并引入了一种从法官裁决真值表中提取最小操作规则的计算算法。


<details>
  <summary>Details</summary>
Motivation: 旨在形式化和自动化传统伊斯兰法学中的‘al-Sabr wa al-Taqsim’方法，以提升法律原因提取的效率和严谨性。

Method: 采用集合论形式化该经典方法，并从包含法官裁决的真值表中提取最小操作规则。

Result: 给定封闭章节的完整真值表，算法能计算出裁决的最小结构生成器，消除逻辑冗余属性，得到可用于后续法理评估的候选原因。

Conclusion: 该框架依赖于有限学派相关概念词汇表和所研究章节的完整裁决表，为计算机辅助法学推理提供了可行路径。
...
### [87] [Multi-Sensor Alignment for Weather Simulations](https://arxiv.org/abs/2607.25612)
*Samsad Alam,Devyani Lambhate,Aditya Mohan,Vishal Kumar,Vaibhav Katewa*

Main category: cs.AI

TL;DR: 本文提出ReDAM和Unified-weather-edit方法，用于对齐多传感器天气模拟中的雾强度和雨雪粒子位置，证明对齐后3D检测模型的评估更为真实，并验证了对齐模拟在提升传感器融合模型鲁棒性方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶感知任务需在恶劣天气下可靠工作，但缺乏真实世界天气数据集，而天气模拟需要确保多传感器间天气特征（如强度、粒子位置）一致性。

Method: 提出ReDAM（用于雾强度对齐）和基于Weather-edit改进的Unified-weather-edit（用于雨雪粒子位置对齐），并通过统计检验和几何检验分别验证。

Result: 未对齐版本的3D检测模型评估过于乐观；对齐后的多传感器模拟能有效提升3D目标检测任务的鲁棒性。

Conclusion: 多传感器天气模拟的对齐对评估和提升感知系统在恶劣天气下的性能至关重要。
...
TL;DR: 提出EEGAlign框架，通过双轴对齐（文本与音频）实现从非侵入式脑电图解码中文句子，在101候选集中达到82.37%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有单轴监督方法无法同时满足句子级区分度和细粒度时间分辨率，阻碍中文大词汇量解码。

Method: EEGAlign使用对比学习联合对齐文本（BGE-M3嵌入）和音频（wav2vec 2.0特征），结合CTC解码序列字符。

Result: 朗读脑电Top-1准确率82.37%，被动聆听脑电41.43%，双轴对齐优于单轴。

Conclusion: 首次实现非侵入式脑电解码中文大词汇量句子，双轴对齐方法显著提升性能。
...
### [89] [Desktop-Delta Bench: Do Computer-Use Models Understand Desktop GUI Transitions?](https://arxiv.org/abs/2607.26041)
*Abhishek Pillai,Samir Kumar Nayak,Yuan Chen*

Main category: cs.AI

TL;DR: 提出桌面增量基准DDB，包含2013个人工验证实例，评估GUI代理在状态验证、来源追踪和上下文感知控制方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅测量最终任务成功或单帧定位，无法隔离模型是否能够重建动作产生的因果、任务相关的转变，这对于拒绝过时观测、验证进展和从失败中恢复至关重要。

Method: 构建DDB，包含463个3帧时间排序实例和1550个前后实例对，涵盖约15个应用和50个任务领域。评估8个模型家族的32个排序和16个单动作设置。

Result: 排序准确率最高65.1%（无干扰项）和65.7%（有干扰项）。任务上下文提升干扰项识别6.9个百分点但降低非干扰项准确率2.2个百分点。点击F1=0.96，拖拽F1=0.76。

Conclusion: DDB填补了GUI定位与最终任务成功之间的诊断层缺口，可针对性改进桌面CUA的验证、可靠性和恢复能力。
...
### [90] [Beyond Epistemia: Epistemic Schizologia and Large Language Models as Techno-Semiotic Machines](https://arxiv.org/abs/2607.25620)
*Federico Cabitza,Gianluca Colombo*

Main category: cs.AI

TL;DR: 本文重新解释了大语言模型造成的“认知症”（Epistemia）现象，提出“技术符号机器”概念，并引入“认知分裂”（epistemic schizologia）和“可能性封闭”（eikotic closure）等新框架，强调应将符号实践的整体而非孤立的模型作为分析单位。


<details>
  <summary>Details</summary>
Motivation: 回应Quattrociocchi等人将LLM输出流畅性视为替代认知评价导致“认知症”的诊断，但不满其将具身、社会情境的人类认知者与孤立生成模型对比的理论框架，认为此框架误置了认知合法性的来源。

Method: 借鉴Carlo Sini的实践、书写、符号与技术哲学，将LLM重新定义为自动化“书写符号过程”中某一阶段的技术符号机器，并通过认知分裂、可能性封闭、算法权威与认知自我误认等概念建构分析框架。

Result: 指出认知症是更广泛认知分裂现象的一个后果——符号在语言表达与嵌入社会的解释、证据、批评、验证与责任回路之间的断裂，这种断裂因可能性封闭、算法权威等因素而强化。

Conclusion: 应关注包含提示、解释、验证、争议、使用及后果的完整实践而非孤立模型，维护语言生产与负责任理解的区分，并以此为基础设计可审查谱系、可争议性、分布式责任、认知主体性与混合人机实践评估的设计方案。
...
### [91] [Quotient Dynamics, Effective Curvature, and Implicit Bias in Positive Quadratic Networks](https://arxiv.org/abs/2607.25624)
*Pengcheng Cheng*

Main category: cs.AI

TL;DR: 该论文研究了正二次神经网络中低秩表示的学习动态，揭示了梯度流与商黎曼流形的等价性，并建立了恢复与插值偏向的理论保证。


<details>
  <summary>Details</summary>
Motivation: 正二次网络的低秩表示具有商结构（旋转不变性），但该结构对训练动态、曲率和恢复性的影响尚不明确。

Method: 利用商流形框架分析梯度流与梯度下降的精确射影关系；对二次回归推导有效海森矩阵；在rank-1高斯测量下分析曲率与收敛性；在欠定交换情形下建立熵镜像流对应。

Result: 证明因变量梯度流投影为商黎曼梯度流；推导出有效海森为测量格拉姆矩阵的切空间限制；建立局部指数收敛与线性收敛保证；欠定情形下各向同性初始化收敛到最小迹解集合。

Conclusion: 商结构决定了二次网络的训练动态、恢复能力和插值偏向，连续与离散优化在熵镜像流下产生可量化的偏差。
...
### [92] [OrchBench: Evaluating Multi-Agent Orchestration Plans in Isolation via Deterministic Simulation](https://arxiv.org/abs/2607.25656)
*Zhenzhen Ren,Jiyan He,Xinpeng Zhang,Zhenxing Qian,Ke Han,Shuxin Zheng,GuoBiao Li,Xiaoqing Zhang*

Main category: cs.AI

TL;DR: 提出了一个基于模拟的基准测试OrchBench，用于独立评估多智能体编排计划，通过有向无环图编码任务依赖关系，并利用确定性模拟器在不执行真实智能体的情况下评估计划质量、完成时间和代币成本。


<details>
  <summary>Details</summary>
Motivation: 现有评估依赖端到端执行，混淆了编排计划质量与工人能力、工具可靠性及环境噪音，且执行代价高昂。

Method: 基于真实任务构建有向无环图（DAG），给定DAG、每个智能体上下文限制和智能体预算，评估的规划器将子任务分配给智能体并指定跨智能体信息传递及其保留率，然后使用确定性模拟器评估计划。

Result: 与Claude Code执行的质量评分具有强相关性（Pearson r=0.816），仅需1.3%的代币和10.3%的实际时间；发现保留关键信息比增加智能体数量更重要，且收益因协调失败而递减。

Conclusion: OrchBench是一种高效且可解释的基准测试，适用于比较和诊断多智能体编排计划。
...
### [93] [Engine-Equal, Human-Unequal: A Reproducible Outcome Skew in Engine-Assessed Equal Chess Positions](https://arxiv.org/abs/2607.25655)
*Jesung Park*

Main category: cs.AI

TL;DR: 研究发现，即使在引擎评估为平衡的国际象棋开局中，人类对局结果也存在稳定的偏向性，且这种偏向在玩家群体间可复制，评估值不足以预测人类结果。


<details>
  <summary>Details</summary>
Motivation: 探究引擎评估的平衡开局中人类实际对局结果是否均衡，以及评估是否足以解释人类表现。

Method: 分析Lichess上实际到达的1661个开局位置（1610万次对局），使用Stockfish 18引擎评估，通过三次分割（玩家集、时间、等级分）检验结果偏向的稳定性，并估计复制斜率。

Result: 开局位置存在稳定的结果偏向（中位数|δ|≈0.018），复制斜率为0.69（最流行位置达0.94），且在更严格评估、搜索深度、流行度阈值下保持显著。

Conclusion: 引擎评估不足以预测人类对局结果，存在超出评估的人类偏向，原因待随机对照研究阐明。
...
TL;DR: 提出OmniDelta，一种无需训练、基于技能驱动的框架，通过意图感知的内容分配和内容感知的模态内分配来优化OmniLLMs的token预算，实现精度-效率帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 现有OmniLLMs处理长音频-视频token序列时存在高内存和推理成本，且现有压缩方法侧重固定预算下的token选择，忽视了预算分配问题。

Method: OmniDelta框架：构建音频和视频技能池，根据查询需求重新分配保留token预算；通过局部复杂性和时间冗余性在音频片段和视频帧间重新分配模态预算。

Result: 在Qwen2.5-Omni-7B模型上，25% token保留时减少22.0% GPU内存，实现1.64倍端到端加速，在四个基准测试上建立新帕累托前沿。

Conclusion: OmniDelta通过有效的预算分配策略，在不增加训练的前提下显著提升OmniLLMs的效率和精度。
...
### [95] [CoRT: Counterfactual Replay for Token-Level Rubric-Guided Policy Optimization](https://arxiv.org/abs/2607.25659)
*Bo-Wen Zhang,Junwei He,Wen Wang,Song-Lin Lv,Wentao Ma,Rongyi Lin,Shuhan Zhong,Lan-Zhe Guo*

Main category: cs.AI

TL;DR: 提出CoRT方法，利用反事实回放计算词级似然对比，在GRPO框架中实现无需辅助模型的词级信用分配，平均提升4.4个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO将评分简化为标量奖励并均匀分配给所有词元，缺乏显式词级信用分配机制，无法区分不同标准对应的文本片段。

Method: CoRT通过反事实回放：对同一采样响应，分别用原评分提示和去评分提示计算词级对数似然差，作为词对评分依赖性的代理，映射为归一化权重，重新分配GRPO优势值。

Result: 在指令调优模型和不同奖励粒度下，CoRT在绝大多数比较中优于原始GRPO，平均提升4.4个百分点，且与学习型词级基线竞争。

Conclusion: 策略内部反事实似然对比可为词级信用分配提供有效训练信号，同时保持GRPO的简单性和稳定性。
...
### [96] [DecoEvo: Score-Decoupled Co-Evolution of Solver and Rubric-Generator Skills in Text Space](https://arxiv.org/abs/2607.25675)
...
TL;DR: 提出DecoEvo方法，通过解耦的协同进化优化文本空间中的大语言模型，无需金标准评分即可提升开放任务表现。


<details>
  <summary>Details</summary>
Motivation: 现有文本空间方法在开放任务中因固定评估标准而陷入瓶颈，仅依赖求解器分数进化评判标准不可靠。

Method: DecoEvo将求解器技能与评分生成器技能解耦，分别用准则级反馈和覆盖度/鉴别性审计更新，避免评分标准退化。

Result: 在五个基准测试和三个LLM骨干上，DecoEvo优于所有对比方法，平均相对提升2.8-5.0%。

Conclusion: 解耦协同进化能有效利用无金标准反馈，持续暴露求解器弱点，提升开放任务优化效果。
...
### [97] [Cognivia: A Cognitive Behavioral Therapy Copilot for Evidence-Based Mental Healthcare](https://arxiv.org/abs/2607.25681)
*Qi Chen,Siria Xiyueyao Luo,Jian Wang,Yuan Shi,Haocong Rao,Xuejiao Zhao*

Main category: cs.AI

TL;DR: 提出了一种基于证据的AI心理治疗系统Cognivia，用于自动识别认知扭曲并生成理性回应，在多个评估指标上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 认知扭曲会加剧负面情绪并导致心理健康问题，CBT有效但受限于专业治疗师短缺；现有LLM方法存在领域特异性不足、回应过于奉承、缺乏明确认知扭曲标注等问题。

Method: 基于权威CBT文本构建数据集，结合心理健康问答数据，采用多阶段提示和结构化生成策略，在行为科学专家监督下微调轻量级LLM，并提出首个分层质量评估框架。

Result: 在词汇指标、LLM评审和10位专家人工评估中，Cognivia在认知扭曲识别和理性回应生成上持续优于基线方法。

Conclusion: Cognivia通过整合自动识别与理性生成，有效提升了AI心理治疗的专业性和实用性，代码已开源。
...
TL;DR: 提出了自推测智能体，将智能体和推测器统一在同一模型中，通过联合强化学习减少工具调用延迟。


<details>
  <summary>Details</summary>
Motivation: 现有工具调用推测方法存在推测器-智能体差距，即独立的推测模型或缓存轨迹与部署的智能体行为不一致。

Method: 引入自推测智能体，联合智能体-推测器强化学习方法，从智能体自身轨迹中导出推测目标，交替更新智能体和推测器。

Result: 在搜索问答和会话工具使用任务中，Qwen3-4B的下一个工具调用Hit@1从44.1提升到61.2，Qwen3.5-4B从48.9提升到66.3。

Conclusion: 自推测智能体能显著提升工具调用预测准确性，同时保持智能体任务成功率。
...
### [99] [Loss Invariance Determines What Concept Layers Encode: Volume Grounding in Echocardiography](https://arxiv.org/abs/2607.25748)
*Hyunkyung Han,Min Jung Kim*

Main category: cs.AI

TL;DR: 概念瓶颈模型的中间变量预测精度不足以验证其有效性，因目标函数的不变性可能导致概念层失去物理尺度。


<details>
  <summary>Details</summary>
Motivation: 探究仅靠概念预测精度判断概念瓶颈模型有效性的充分性。

Method: 使用视频变换器编码器训练超声心动图数据集，以左心室容积为概念层，对比有无容积监督的训练效果。

Result: 无容积监督时预测容积分布坍缩至0.1毫升（参考范围35.7-45.7毫升），但射血分数误差仅6.89；容积监督将容积误差从89.8降至25.8毫升，但射血分数误差增加0.4。

Conclusion: 概念准确性可能掩盖无物理尺度的概念层，需结合目标函数不变性验证中间变量有效性。
...
### [100] [Nudging Sustainable Choices through LLM-Generated Recommendation Explanations](https://arxiv.org/abs/2607.25726)
*Haya Halimeh,Dietmar Jannach,Oliver Müller*

Main category: cs.AI

TL;DR: 该研究探讨了如何通过生成式AI构建基于助推理论的解释，在推荐系统中嵌入可持续信息，并通过实验验证其能有效引导用户做出更可持续的选择。


<details>
  <summary>Details</summary>
Motivation: 推荐系统影响日常消费，但现有解释主要关注用户感知，未充分利用解释作为行为助推工具来促进可持续选择。

Method: 基于助推理论，用生成式AI生成可持续感知的解释，通过人工评估和LLM评判验证；在低参与度（速溶咖啡）和高参与度（酒店预订）领域进行两项随机实验（N=529）。

Result: 仅披露可持续信息不改变选择，但框架化信息或利用描述性社会规范能显著增加可持续选择并简化决策；用户感知与行为出现分离：简单披露改善解释评价但不影响选择。

Conclusion: LLM可大规模生成理论驱动的解释，为基于解释的社会有益干预提供实用方案，并对生成式AI自适应解释设计有所启示。
...
TL;DR: 本文提出HiSkill，一种层次化技能图框架，通过有向图组织交互轨迹，连接高级技能与可执行动作，并在推理时检索子图指导任务执行。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹到技能方法产生扁平化技能集合，忽略技能间关系，且高级技能与可执行动作间存在鸿沟。

Method: 构建包含技能节点、原子操作节点和有向边的层次化图结构，捕获分解、时序等关系；推理时检索子图，通过符号任务状态、活跃技能和子图引导技能切换和动作执行。

Result: 在三个交互环境中，HiSkill优于现有基线，同时降低推理token消耗。

Conclusion: 层次化技能图能有效桥接高级技能与可执行动作，提升长周期交互任务的效率和效果。
...
### [102] [Distributed Constraint Optimization via Online Learning and Iterative Pricing with Application to Large-Scale Satellite Scheduling](https://arxiv.org/abs/2607.25835)
*Itai Zilberstein,Pranav Rajbhandari,Steve Chien,Tuomas Sandholm*

Main category: cs.AI

TL;DR: 本文将在线学习算法与迭代定价框架结合用于大规模分布式约束优化问题，在卫星调度任务中实现99%以上请求满足率


<details>
  <summary>Details</summary>
Motivation: 现实分布式约束优化问题规模庞大难以整体求解，需要更高效的分解与求解方法

Method: 建立DCOP与势博弈的联系，引入在线学习算法进行均衡求解；提出双层分解框架（高层元DCOP+底层局部优化），通过迭代定价耦合两层

Result: 在线学习算法与代表性不完整DCOP算法性能相当；结合迭代定价后，在真实卫星调度场景中满足99%观测请求，优于基线方法的87%

Conclusion: 该混合方法可有效解决大规模DCOP问题，尤其是具有层次结构的分布式调度场景
...
### [103] [Runtime Uncertainty Monitoring for LLM-Based Multi-Agent Systems Using Bayesian Networks](https://arxiv.org/abs/2607.25877)
*Bart Custers,Koorosh Aslansefat*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型的多智能体框架，通过贝叶斯网络传播不确定性，支持精算风险建模中的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 精算工作流属于高风险决策场景，LLM的概率特性与智能体间依赖可能引发风险评估错误、定价不公及监管合规问题。

Method: 设计集中式枢纽下专业智能体（数据准备、建模、审查、解释）协作框架，利用令牌级对数概率经长度归一化后转化为校准的任务级置信度，再输入贝叶斯网络进行不确定性传播。

Result: 框架能复现基础精算任务性能，同时提供工作流稳定性与运行时不确定性传播的额外洞察。

Conclusion: 该多智能体框架可有效支持精算风险建模，在保持精算基准性能基础上增强不确定性量化能力。
...
TL;DR: 提出基于交互式奖励代理（IRA）的GUI任务评估方法，通过“提议-验证”框架结合环境状态证据，在321个任务上达86.9%准确率，并用于强化学习提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有GUI任务评估多依赖截图，难以获取环境状态（配置、文件等），导致判断不可靠。

Method: IRA基于提议-验证框架：先提出任务完成条件，再通过系统工具、应用工具和GUI工具交互验证环境证据。

Result: 在GUI-RewardBench上达86.9%准确率，优于基线；用于强化学习后使OSWorld成功率提升至34.0%。
...
TL;DR: 提出了Messier统一语料库，包含957,253条记录，覆盖30个基准、714个智能体、11,891个任务和74,205个验证器，用于标准化评估AI智能体。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体评估因任务、框架、验证器和评分规则碎片化，导致结果难以比较，且许多实证记录缺乏可比性。

Method: 整合公开基准分数，补充六个专业和科学领域的五次智能体运行数据，标准化每个记录包括模型、框架、环境、任务、验证器和聚合规则，并用SOC/NAICS分类进行职业和行业分析。

Result: 前沿进展不均匀，函数调用饱和、编程改进最快、企业工作流最具挑战性；严格的全通过聚合会掩盖进展并改变排名；能力量表与Epoch评估能力指数相关性为0.81。

Conclusion: Messier提供了可复用的基础设施，用于智能体能力缩放、基准审计和评估失败分析。
...
### [106] [Distributing Security Controls Through Harness Engineering](https://arxiv.org/abs/2607.25890)
*William Robert Gore*

Main category: cs.AI

TL;DR: 本文研究如何在商业AI编码代理中嵌入并分发现成的安全控制措施，通过SHarD安全工具实现了与商业代理同等的保护效果。


<details>
  <summary>Details</summary>
Motivation: AI编码代理快速普及，但安全风险阻碍其在组织中的规模化应用，现有安全控制缺乏系统分发且存在生态依赖问题。

Method: 采用分阶段测试方法，对四种代理配置（含/不含控制的商业代理、基线工具、安全强化工具）使用23项OWASP测试，通过SHarD工具集成OS沙箱、技能扫描和工具限制三类控制。

Result: SHarD在所有测试类别中达到100%调整得分，与最佳安全商业代理持平，无回归问题，同时模型非确定性导致安全结果不一致。

Conclusion: 现成安全控制可通过定制工具高效分发，OS沙箱能直接缓解代理自主行为的跨边界风险，并提出了控制工具适配框架的初步特征。
...
### [107] [Toward Standardized Cross-Vendor Agent Tool Trust Management in Autonomous Networks](https://arxiv.org/abs/2607.25914)
*Ravi Kant Sharma,Ashutosh Uttam,Ajay Kumar*

Main category: cs.AI

TL;DR: 提出AgentToolMO，一种用于跨厂商AI代理工具信任管理的3GPP NRM信息模型，实现实时信任感知与级联影响限制。


<details>
  <summary>Details</summary>
Motivation: 现有管理标准缺乏跨厂商信任可见性机制，导致工具被攻陷时代理继续调用，引发服务级联故障。

Method: 定义信任状态机、阻尼级联传播、通过MnS接口转发通知、基于NRM依赖图进行影响评估。

Result: 模拟显示标准化通知将影响半径从小时级缩小至近实时，级联收敛有界，通知扩展亚线性。

Conclusion: 该模型可集成至现有3GPP管理架构，为可信多厂商自治网络提供标准化路径。
...
### [108] [CHARM: A Multimodal Graph Foundation Model with Hierarchical Context Modeling for Zero-Shot Transfer](https://arxiv.org/abs/2607.26023)
*Ankang Yang,Jitao Zhao,Di Jin,Yuxiao Huang,Dongxiao He*

Main category: cs.AI

TL;DR: 提出CHARM，一种用于多模态图零样本迁移的分层上下文建模基础模型。


<details>
  <summary>Details</summary>
Motivation: 多模态图零样本迁移研究不足，现有GNN或LLM方法需下游适配或限于单模态。

Method: 用分层图上下文替代原始节点，捕捉多模态语义和跨模态关系，并经模态感知编码器转成图令牌供LLM处理。

Result: 在零样本多模态图任务上取得一致改进。

Conclusion: CHARM有效解耦领域特定模式，实现跨域零样本迁移。
...
### [109] [dtControl2+$\varepsilon$: Trading Optimality for Explainability in MDPs via Decision Trees](https://arxiv.org/abs/2607.25925)
*Tereza Kinská,Jan Křetínský,Tobias Meggendorfer,Sabine Rieder,Maximilian Weininger*

Main category: cs.AI

TL;DR: 本文提出了一种在马尔可夫决策过程中允许给定精度损失ε的决策树压缩方法，生成规模小几个数量级但仍保证ε最优性的控制器，增强了可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有决策树控制器（如dtControl2）在大型或复杂系统中变得庞大且难以理解，而直接缩减树大小可能导致错误，因此需在保证正确性的前提下简化表示。

Method: 通过扩展dtControl2工具，引入ε功能：允许设定不精确度ε≥0，压缩决策树大小，同时确保ε最优性，提取控制器核心。

Result: 生成的决策树比当前最优方法小数个数量级。

Conclusion: 在可控精度损失下，可显著简化控制器表示，提供可调节的简洁解释，提升可理解性。
...
### [110] [Penelope: Localized Latent Recurrence for Efficient Structured Reasoning](https://arxiv.org/abs/2607.25915)
*Yutong Chen,Shouqian Shi,Xinran Liu,Haochen Wang,Jiaying Wang,Tianxing Xu,Yuanxi Wang,Zirui Ding*

Main category: cs.AI

TL;DR: Penelope是一种高效潜在推理框架，通过将循环计算局部化到选定的解码器区间，在不重复执行完整解码器或生成长中间痕迹的情况下，实现潜在空间中的额外计算分配。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型通过增大参数量或序列化中间步骤（如CoT）来处理复杂推理任务，但这分别增加了训练和部署成本，或将推理计算与自回归输出长度绑定。

Method: 在预训练解码器-only Transformer中，下部分解码器只执行一次以构建问题条件边界记忆，然后通过时间调制的GRU动态和循环读取状态进行迭代细化，再生成答案。并采用渐进式CoT到潜在课程将可见推理转移到内部循环路径。

Result: 在开源结构化推理基准上，以验证选择的潜在预算，Penelope在保持与现有潜在推理模型相当精度的同时，降低了测量推理延迟。

Conclusion: 潜在细化可被局部化到窄解码器区间，减少重复全解码器执行，无需生成长可见推理痕迹，为解码器-only Transformer提供了实用的精度-效率权衡。
...
TL;DR: 通过行为实验和进化模型，发现AI竞赛中不安全行为更多由竞争动态（如对手行为、领先/落后状态）驱动，而非个人风险偏好，建议政策关注减少竞争压力。


<details>
  <summary>Details</summary>
Motivation: 探讨AI竞赛中安全与速度的权衡，挑战“风险偏好主导不安全行为”的假设。

Method: 框架行为实验：配对参与者重复选择安全/不安全开发（最大风险10%/60%/90%）；进化模型：四种策略（始终安全、始终不安全、条件安全、条件反社会安全）。

Result: 风险偏好与不安全选择无显著关联；不安全行为受对手行为、领先/落后状态及首轮选择影响；进化模型复现处理效应并解释条件不安全行为的优势。

Conclusion: 不安全行为源于早期行为惯性、对手影响和落后恐惧，而非风险偏好，政策应聚焦减少竞争压力、促进合作。
...
### [112] [Large Language Model for Operations Research Formulation Selection in Multi-Warehouse Inventory Allocation](https://arxiv.org/abs/2607.25956)
*Jintao Xu,Yingzheng Ma,Jiong Dong,Yongzhi Qi,Jianshen Zhang*

Main category: cs.AI

TL;DR: 提出了一个求解器引导的大语言模型框架，用于多仓库库存分配中混合整数规划公式的实例级选择，通过监督微调、身份偏好优化和组相对策略优化训练选择器，在京东数据集上显著提升了选择准确率和分配质量。


<details>
  <summary>Details</summary>
Motivation: 多仓库库存分配通常用混合整数规划建模，但没有单一公式能适应由需求集中度、库存不平衡、补货规模、服务约束和预测波动引起的异质实例级场景，需要自动为每个实例选择最佳公式。

Method: 采用求解器引导的大语言模型框架：先构建专家条件监督微调记录学习模式，再用历史实例上的求解器评估结果生成偏好数据，并通过组相对策略优化根据奖励查找分配奖励以训练选择器。

Result: 在京东数据集上，组相对策略优化将Hit Ratio@1从21.45%提升到50.42%，Hit Ratio@2从70.47%提升到82.31%；选择器分配准确率比基准提高12.57个百分点，与事后最优的差距缩小到4.85个百分点。

Conclusion: 提出的求解器引导大语言模型框架能有效解决多仓库库存分配中的公式选择问题，大幅提升选择准确率和实际分配质量，优于固定公式和监督微调方法。
...
### [113] [OPERA: Offline Policy-guided Expert Routing and Adaptation for Universal Biomedical Image Analysis](https://arxiv.org/abs/2607.25108)
*Zihan Li,Feiyang Liu,Dandan Shan,Ruibo Wang,Qingqi Hong*

Main category: cs.CV

TL;DR: 提出OPERA，一种多智能体集成框架，通过离线策略学习分配专家权重，结合测试时适应，无需重训练即可处理生物医学图像中的分布偏移，在9个数据集上优于30多个基线。


<details>
  <summary>Details</summary>
Motivation: 生物医学图像分析面临分布偏移导致模型需重复微调，成本高且数据稀缺或隐私限制时不可行，需无需重训练的部署方案。

Method: 离线策略学习的路由策略、专家配置文件模块、置信度校准、基于无标注测试数据的分布感知批次级类权重调整、实例级路由。

Result: 在9个数据集、多种模态和任务上，OPERA一致提升了性能与校准质量，优于30多个基线。

Conclusion: OPERA证明了离线策略引导的专家智能体协调是无需重训练即可部署生物医学AI的实用途径。
...
### [114] [WHTMix: Efficient Stereo Depth Estimation via Walsh-Hadamard Token Mixing](https://arxiv.org/abs/2607.25234)
*Prathyush Sajith,Emadeldeen Hamdan,Ahmet Enis Cetin*

Main category: cs.CV

TL;DR: 通过用Walsh-Hadamard混频器取代立体视觉Transformer中的全局自注意力机制，将计算复杂度从平方级降至对数线性级，同时保持了精度。


<details>
  <summary>Details</summary>
Motivation: 立体深度估计需要高分辨率实时处理，但现有Transformer的全局自注意力机制计算量随像素数平方增长，成为性能瓶颈。

Method: 使用与数据无关的Walsh-Hadamard混频器替代联合自注意力阶段实现全局混合，保留数据依赖的交叉注意力进行左右对应；并引入混合对数视差损失函数提升远距离物体精度。

Result: 在合成驾驶数据上，端点误差持平，模型计算量降低2.46倍，单图推理延迟降低2.65倍。

Conclusion: 高分辨率立体匹配场景下的序列长度与通道宽度比率决定了该方法的优势，且混合损失函数在不增加计算量的情况下有效提升远距离物体精度。
...
TL;DR: 弱监督层次模型在粗粒度的病变类型特征保持良好，但细粒度的恶性征象重建时退化；本文通过梯度正交潜变量分解解释该不对称性，并揭示粗监督信号仅对齐约4.4%的潜空间，细粒度病理预测主要依赖剩余95.6%的脆弱残差子空间。


<details>
  <summary>Details</summary>
Motivation: 弱监督层次模型存在粗粒度特征与细粒度恶性特征重建时的持续性不对称，直接影响乳腺癌筛查的临床可靠性，需机制性解释此现象。

Method: 提出基于梯度的正交潜变量分解方法，将H-VAE的潜空间划分为任务对齐分量（z1）和正交残差（z_res），并进行潜变量消融实验。

Result: 在3550个ROI上，仅4.4%的潜幅度与监督梯度对齐；Stage-1 AUC 0.866，Stage-2 AUC 0.552；重建稳定性差距Δ_diag=5%（p=0.005），分类差距Δ_AUC=0.314（p<0.001）；与MIL和MTL比较验证跨架构泛化性。

Conclusion: 高维空间中单一粗监督信号仅隔离出稀疏1D方向，迫使关键细粒度特征进入残差子空间，导致模型对细粒度病理预测脆弱。
...
TL;DR: 本论文提出了一种无轨迹泄露的留出协议，用于评估3D重建的内部几何一致性，发现该指标不能替代绝对精度验证或通用故障检测。


<details>
  <summary>Details</summary>
Motivation: 自动化摄影测量依赖无外部地面真值的3D重建，但重建自身可靠性未知，需要一种无需地面真值的自评估方法。

Method: 设计无轨迹泄露留出协议：隔离训练和测试的图像集，确保测试视图不检测其参与构建的结构；通过重定位和mAA置信度聚合评估内部一致性。

Result: 协议计算有效且能检测碎片化失败，但阈值化自一致性无法跟踪绝对精度（RTK误差波动达14倍），且不能检测全局扭曲模型（误差55-106米时置信度仍为1.00）。

Conclusion: 该协议仅测量内部几何一致性，不能替代控制点评估或作为通用故障门控，建议仅用于辅助验证而非独立可靠性判断。
...
TL;DR: 提出 DisasterTD 框架，结合多模态大语言模型与跨视角地理定位，用于灾情社交媒体图像中模糊地名的消歧和精确地理定位。


<details>
  <summary>Details</summary>
Motivation: 社交媒体图像在灾害响应中提供及时地面信息，但其地理参照常模糊不清，导致准确定位困难。

Method: 先利用多模态大语言模型提取地名并生成候选位置，再通过跨视角匹配（结合遥感影像与街景影像）验证和精化候选结果。

Result: 在 Hurricane Harvey 数据集上，DisasterTD 在 1000 米内精度达 71.62%，平均误差降至 11.33 公里，中位数误差 0.68 公里，尤其在模糊地名场景中提升显著。

Conclusion: 集成多模态大语言模型候选生成与跨视角验证的方法能有效实现细粒度灾害地理定位。
...
### [118] [Human-in-the-Loop Signature Bootstrapping for UAV Hyperspectral PFM-1 Mine Detection](https://arxiv.org/abs/2607.25310)
*Sagar Lekhak,Prasanna Reddy Pulakurthi,Emmett J. Ientilucci*

Main category: cs.CV

TL;DR: 论文研究了在无人机可见光近红外高光谱成像中，使用不同检测器（SAM、MF、ACE、CEM）检测PFM-1地雷，对比了地面测量光谱、场景内核心像素光谱和人机交互自举光谱的性能，发现ACE方法仅需9次候选检查即可确认所有目标，而SAM方法需数千次。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像可用于材料区分，但实际地雷筛查效率受虚警检查次数影响，现有研究缺乏对检测器在减少检查工作量方面的比较。

Method: 采用SAM、MF、ACE、CEM四种检测器，利用三种光谱来源（地面测量、场景内核心像素、人机交互自举）进行检测，评估ROC AUC、平均精度、目标发现曲线和候选检查次数。

Result: ACE在两次循环和9次候选检查后确认所有目标区域，而SAM变体需数千次候选检查才能定位最终目标；全检查自举方法在验证所有目标区域后达到与场景内核心像素签名相同的性能。

Conclusion: ACE检测器在减少不必要的外部检查次数方面表现最优，显著优于SAM，适合实际高效地雷筛查。
...
### [119] [Harm is not Universal: Community-Specific Toxicity Detection is Urgently Needed](https://arxiv.org/abs/2607.24898)
...
TL;DR: 指出现有文本到图像生成毒性检测器对弱势群体的忽视，提出社区特定毒性检测，通过残疾社区案例验证挑战和初步方法。


<details>
  <summary>Details</summary>
Motivation: 现有检测器采用通用模型，未能保护边缘化社区，实证显示约35%被标记安全的图像对残疾社区有害。

Method: 与残疾专家合作制定矮小症和盲/低视力社区安全指南，构建2400张T2I图像数据集，评估大型视觉语言模型和通用检测器，并尝试ICL、VQA等提示适应方法和参数高效微调。

Result: 通用检测器和视觉语言模型零样本F1分数低于随机(0.32和0.37)；提示适应方法(GPT-4o: F1 0.50和0.78)和小模型微调(0.5b-7b: 最佳F1 0.48和0.59)有提升，但仍远低于通用检测F1≈0.9。

Conclusion: 社区特定毒性检测可行但具挑战，现有方法性能不足，需持续研究投入。
...
### [120] [Mage-VL: An Efficient Codec-Native Streaming Multimodal Foundation Model](https://arxiv.org/abs/2607.24904)
...
TL;DR: Mage-VL是面向实时多模态交互的高效编解码流式基础模型，通过运动熵驱动的视觉 token 化减少75%计算量，并采用双系统架构实现主动感知。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在实时流式感知任务中效率低且表现不佳，存在Moravec悖论——复杂推理强但简单感知弱。

Method: 提出Mage-VL，包含定制tokenizer（Mage-ViT）利用运动向量和残差能量稀疏编码动态区域；双系统架构（System 1事件门控+因果System 2解码器）；AI4AI数据管道优化标注与训练。

Result: Mage-VL-4B在静态任务上持平Qwen3-VL-4B，在视频理解和2D/3D空间推理上显著提升，推理速度最高加速3.5倍，全面超越Phi-4-reasoning-vision（15B）。

Conclusion: 通过运动感知稀疏编码和双系统主动感知设计，Mage-VL在显著降低计算消耗的同时实现了实时流式多模态理解性能的全面提升。
...
### [121] [PerceptionBench: Evaluating Atomic Visual Perception in Multimodal Large Language Models](https://arxiv.org/abs/2607.24957)
...
TL;DR: 提出了一个名为 PerceptionBench 的基准测试，专门用于评估多模态大语言模型（MLLMs）的原子视觉感知能力。通过构建错误分类法，识别出十种原子感知能力，并设计了3000个问题来隔离测试每种能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法隔离感知错误，要么将感知错误与推理或知识错误混为一谈，要么只覆盖狭窄领域。

Method: 采用自下而上的方法，通过分析前沿MLLMs在42个现有基准测试中的最早失败点，构建错误分类法（其中感知分支定义了十种原子感知能力），并据此构建3000个验证问题。

Result: 16个前沿MLLMs的测试结果显示，没有任何模型达到60%的准确率，感知相关的幻觉是最薄弱的能力，且相似的整体分数掩盖了截然不同的能力轮廓。

Conclusion: PerceptionBench提供了一个能力级别的标准，用于测量和诊断MLLMs的视觉感知边界。
...
### [122] [Enabling Fully Integer-Only Inference for Lightweight Detection Transformers](https://arxiv.org/abs/2607.24981)
*Thanh Cong Le,Michal Szczepanski,Martyna Poreba*

Main category: cs.CV

TL;DR: 提出了首个全整数轻量级DETR（I-LW-DETR），实现前向传播中所有操作（包括Transformer非线性层）的整数运算。


<details>
  <summary>Details</summary>
Motivation: 现有视觉Transformer检测器在NPU和微控制器上部署困难，因关键组件（可变形注意力、特征融合、非线性激活）不兼容整数算术；现有量化检测器保留浮点算子或聚焦重型骨干，缺乏轻量级检测Transformers的纯整数实现。

Method: 包含三个关键组件：尺度保持的分裂卷积（为多尺度投影各分支分配独立激活尺度）、SD-ShiftGELU（符号依赖的GELU近似避免精度下降）、约束Shiftmax（维持稳定Softmax归一化）。

Result: 在多种模型尺度下，精度仅适度下降，模型大小压缩约3.6倍，计算成本降低超过一个数量级。

Conclusion: 提出的量化流水线能一致地产生高效的全整数模型，填补了轻量级检测Transformers纯整数部署的空白。
...
### [123] [Unifying Active Learning and Semi-Supervised Learning for Medical Image Segmentation](https://arxiv.org/abs/2607.25014)
*Bahram Jafrasteh,Cheng Wan,Heejong Kim,Johannes C. Paetzold,Qingyu Zhao*

Main category: cs.CV

TL;DR: 提出RegAL统一主动-半监督框架，通过拓扑感知Pareto优化同时解决标注获取与未标注数据利用问题，在极低标注率下稳定优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 主动学习与半监督学习独立设计导致目标不匹配，在冷启动阶段训练不稳定；需同时解决标注样本选择和未标注数据有效利用。

Method: 构建三维互补准则（体素不确定性、特征多样性、拓扑一致性）选择解剖关键边缘样本，同时利用相同准则筛选几何稳定图谱进行微分同胚配准增强，训练自教师Mean Teacher分割网络。

Result: 在BraTS2021、dHCP、ProstateX数据集上，极少量标注体素下Dice和边界距离指标（ASD, HD95）均稳定超越现有AL、SSL及混合方法。

Conclusion: 拓扑感知的耦合优化能有效缓解冷启动问题，实现主动学习与半监督学习的协同增益。
...
### [124] [Diff-ID: Identity Consistent Facial Image Generation and Morphing via Diffusion Models](https://arxiv.org/abs/2607.25078)
*Taimoor Rizwan,Sara Atito,Muhammad Awais,Zhenhua Feng,Josef Kittler*

Main category: cs.CV

TL;DR: 提出Diff-ID框架，通过定制数据集、双交叉注意力适配器和伪判别器损失，在保持身份一致性的同时生成高保真人脸图像。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在身份保持方面存在不足，影响安全系统和隐私应用的可信度。

Method: 使用210K定制数据集（来自CelebA-HQ等）、双交叉注意力适配器（ArcFace+CLIP嵌入）和伪判别器损失（基于ArcFace余弦相似度与指数时间步加权）。

Result: Diff-ID的ArcFace相似度不及InstantID，但FID更低，且在身份-真实感权衡上表现最优，并实现了无需微调的人脸变形。

Conclusion: 身份保持与真实感应联合评估，Face Image Quality (FIQ) 可作为补充指标，主指标仍为ArcFace相似度和FID。
...
### [125] [IMPRINT: Image-Conditioned Query Enrichment for Long-Tail Object Goal Navigation](https://arxiv.org/abs/2607.25106)
*Jelin Raphael Akkara,Filippo Ziliotto,Luciano Serafini,Lamberto Ballan,Tommaso Campari*

Main category: cs.CV

TL;DR: 提出零样本框架IMPRINT，用网络图像增强文本查询，提升细粒度物体定位和导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖文本查询，在细粒度物体类别下可靠性降低。

Method: 从网络检索图像，使用视觉语言模型编码并与语义地图匹配，生成相似度图进行聚合定位。

Result: 在OVON和HSSD-rare基准上，图像条件查询持续提升物体定位和端到端导航性能。

Conclusion: 翻译定位增益为导航性能的关键是下游检测质量，揭示了长尾导航的系统瓶颈。
...
### [126] [MorphUNet: Alpha-Controlled Biometric Transport for Diffusion-Based Face Morphing Attacks](https://arxiv.org/abs/2607.25092)
*Taimoor Rizwan,Sara Atito,Zhenhua Feng,Muhammad Awais,Josef Kittler*

Main category: cs.CV

TL;DR: 提出MorphUNet扩散变形框架，通过双交叉注意力机制和身份感知令牌库实现高质量面部变形攻击，在多个数据集和识别系统上达到最优攻击性能。


<details>
  <summary>Details</summary>
Motivation: 现有面部变形攻击方法在身份平衡性和攻击成功率上存在局限，需要更鲁棒的框架应对多种识别系统和跨数据集场景。

Method: 结合CLIP和ArcFace分解双亲特征，在U-Net中加入双交叉注意力层实现身份感知传输，采用DDIM倒插隐变量和弱父母引导策略优化变形质量。

Result: 在FEI和FRLL数据集上MAP分别达0.919和0.886，FID值最优（35.19和44.86），跨数据集攻击成功率APCER超0.94。

Conclusion: MorphUNet是首个基于扩散的可训练双亲分离变形框架，在攻击有效性和不易检测性上全面超越现有基线。
...
### [127] [LENS: Adaptive Spatio-Temporal Zooming for Keyframe Sampling in Long-Form Videos](https://arxiv.org/abs/2607.25125)
*Ce Zhang,Jinxi He,Katia Sycara,Yaqi Xie*

Main category: cs.CV

TL;DR: LENS是无需训练的关键帧采样框架，根据文本查询动态调整空间细节与时间范围，在长视频理解中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在长视频理解中受限于上下文窗口，关键帧采样方法在空间细节和时间覆盖上存在冲突。

Method: 在固定帧预算内自适应分配空间放大（关注帧内关键区域）和时间缩放（通过多帧聚合扩展范围），实现多粒度推理。

Result: 在多个长视频基准上超越先前关键帧采样方法，Qwen2.5-VL在Video-MME上准确率从53.3%提升至60.7%。

Conclusion: LENS通过动态平衡空间细节与时间上下文，有效提升长视频理解能力，且无需额外训练。
...
### [128] [FIDAC: An Easy-to-use Pipeline to Extract and Interpret Interpersonal Distance From Video](https://arxiv.org/abs/2607.25146)
*Keshav Rastogi,Eugy Han,Jeremy N. Bailenson*

Main category: cs.CV

TL;DR: 该论文介绍了FIDAC开源库，用于从视频中提取和分析人际距离信息。


<details>
  <summary>Details</summary>
Motivation: 人脸间距离包含重要的社交信息，但难以从视频中直接提取和解释。

Method: 融合多个开源人脸检测模型，弥补各自不足；并加入人工编码管道和深度失真减少工具以提高追踪准确性。

Result: 成功构建FIDAC库，能从人脸检测结果中有效计算位置和人际距离。

Conclusion: FIDAC为人际距离分析提供实用工具，未来将评估其在多深度和方向上的有效性，并集成同步等空间关系特征。
...
### [129] [OpenPVMapper: A Multi-source, Nationwide Database of Rooftop Photovoltaic Systems in France](https://arxiv.org/abs/2607.25153)
*Gabiel Kasmi*

Main category: cs.CV

TL;DR: OpenPVMapper是一个全国性的屋顶光伏安装数据库，整合了深度学习检测、OpenStreetMap和概率检测等多种数据源，包含1,135,850个安装点，总容量约15.01 GWp，准确率约74-75%。


<details>
  <summary>Details</summary>
Motivation: 现有数据集不完整（仅高于容量阈值的聚合数据）或局限（单一方法、地理范围或时间点），缺乏公开、全面、安装级别的屋顶光伏数据集。

Method: 通过聚合和协调多种互补数据源：基于深度学习的航空影像检测管道、OpenStreetMap和建筑级概率检测数据集，并采用手动验证和分层抽样评估精度。

Result: 生成包含1,135,850个安装点的数据库（总容量15.01 GWp），整体精度达74-75%，独立来源交叉验证显著提升精度。

Conclusion: OpenPVMapper通过整合独立数据源达到高于单一来源的置信度，且支持扩展，并以开放许可（CC-BY）发布，为光伏研究提供可靠基础数据。
...
### [130] [OrganLens: Organ-Specific Representation Learning for CT Foundation Models](https://arxiv.org/abs/2607.25164)
*Zhixuan Ge,Anqi Li,Sadeer Al-Kindi,Hanwen Xu,Wei Qiu*

Main category: cs.CV

TL;DR: OrganLens 是一种通过自监督学习实现器官特异性CT表示的方法，使用共享编码器生成11个器官的表示，无需外部分割掩码。


<details>
  <summary>Details</summary>
Motivation: 现有CT基础模型通常生成整体体积表示，而解剖感知方法要么编码预分离的器官体积（丢失临床相关上下文），要么不条件化共享编码器。

Method: 通过器官身份条件化共享CT编码器，结合器官特异性蒸馏和解剖掩码监督，使用解剖加权池化生成器官特异性表示。

Result: 在心脏和肺部任务中，AUROC提升至0.953，肺癌死亡率C-index提高14.2%，全局表示在检索任务中达到33.09%和32.04%的Recall@10。

Conclusion: OrganLens提供可扩展的器官特异性CT表示方法，共享编码器兼顾全局和局部任务，为医学研究提供复用框架。
...
### [131] [LGFNet: A CTC-Guided Local-Global Fusion Framework for Single-Channel Sleep Staging](https://arxiv.org/abs/2607.25197)
*Chongjian Wang,Zhenghang Hou,Junjie Gao,Xiaofang Zhong,Shiyuan Han,Tong Zhang*

Main category: cs.CV

TL;DR: 提出LGFNet框架，通过局部-全局融合编码器、CTC-注意力联合训练和三阶段解码策略，在单通道低延迟睡眠分期中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理睡眠分期中的长程依赖、N1期转换模糊以及跨受试者/采样率/电极布局的分布偏移，尤其在单通道低延迟场景下挑战更大。

Method: 局部-全局融合编码器（联合建模细粒度时间动态和长程睡眠结构）、CTC-注意力联合训练（统一时间对齐与上下文建模）、三阶段解码策略（CTC引导解码+Viterbi平滑减少误差累积）。

Result: 在五个公开数据集上优于现有方法，例如Sleep-EDF-78上相比DMIN准确率+1.27%、macro-F1+1.74%、kappa+1.93%，N1期和转换段提升显著。

Conclusion: LGFNet在跨采样率、电极布局和记录环境的场景下具有强鲁棒性和泛化能力，适用于可穿戴和实时应用。
...
### [132] [ObliCity: A Benchmark and Baseline for Roof-to-Ground Projection Displacement Correction](https://arxiv.org/abs/2607.25210)
*Kai Li,Yupeng Deng,Ligao Deng,Zhihao Xi,Chenhao Wang,Jierui Zhang,Yingrui Ji,Yu Meng,Xiangyu Zhao*

Main category: cs.CV

TL;DR: 本文提出将屋顶到足迹偏移向量提取作为独立学习任务，通过DragRoof方法（基于ODE的框架）和ObliCity数据集（首个大规模倾斜城市遥感基准），实现了几何投影位移的精准校正。


<details>
  <summary>Details</summary>
Motivation: 倾斜城市遥感影像中屋顶与足迹的几何投影位移导致空间结构失真，现有方法未显式处理该问题，且缺乏专用基准数据集。

Method: 构建ObliCity数据集（包含高分辨率无人机和全球卫星影像）；提出DragRoof，利用ODE模拟从屋顶到足迹的连续拖拽过程，学习几何一致的偏移场，并通过结束标记自适应确定收敛。

Result: 在ObliCity上，DragRoof在屋顶到足迹偏移向量提取任务中达到最优性能，推断步骤更少且方向与长度精度更高。

Conclusion: 该数据集和模型为倾斜遥感影像投影位移校正建立了理论基础，推动了几何校正从隐式语义分割到显式学习任务的转变。
...
TL;DR: 本文提出了跨域多目标跟踪（CD-RMOT）问题，构建了CD-RMOT-Bench基准，并设计了查询中心自适应框架（QCA），揭示了域偏移对语言引导跟踪的严重破坏，为鲁棒跟踪开辟了新方向。


<details>
  <summary>Details</summary>
Motivation: 现有RMOT研究局限于同域设置，未探索语言引导跟踪在不可避免的视觉域偏移下的鲁棒性。

Method: 构建了CD-RMOT-Bench统一基准，包含清晰域、数字孪生变体和真实恶劣域数据；提出查询中心自适应（QCA）框架以稳定链接视觉轨迹与语言表达的查询空间。

Result: 域偏移严重降低RMOT性能，失败主因不是检测错误，而是不稳定的表达条件时序关联和目标选择；QCA建立了强基线。

Conclusion: CD-RMOT-Bench推动了跨域语言引导跟踪的研究，QCA提供了有效解决方案。
...
TL;DR: 该综述定义了医学世界模型的概念边界、技术基础、应用领域和证据要求，通过对1,455篇文献的筛选，使用98个来源（包括14项严格定义的研究）构建了知识框架。模型围绕四个能力组织：患者状态表征、时间动态建模、干预条件模拟和临床监督规划。


<details>
  <summary>Details</summary>
Motivation: 为了将医疗AI从静态预测扩展到动态患者状态演化建模，应对临床干预随时间变化的影响。

Method: 采用结构化叙述性综合和可复现证据映射方法，系统筛选文献并归类四个核心能力域。

Result: 多数研究仍处于回顾性、任务特定或临床前阶段，存在干预数据不完整、动作语义不一致、因果识别有限等问题。

Conclusion: 临床转化需要精确干预表征、稳健因果机制、校准的不确定性估计及前瞻性多中心验证。
...
TL;DR: 提出了FORGE方法，通过查询条件化的嵌入空间选择关键帧，提升长视频理解中查询相关信息密度，无需训练。


<details>
  <summary>Details</summary>
Motivation: 长视频中相关内容密度随长度急剧下降，暴露于无关内容会降低模型准确性。

Method: FORGE：在预训练多模态嵌入空间诱导查询条件化的几何结构，将相关性和多样性统一为单一目标，选择覆盖独立查询相关方向的帧子集。

Result: 在Video-MME和LongVideoBench上，统一关键帧选择分数提升11.0-15.3点，关键帧召回率翻倍（0.415 vs 0.204），QA准确率在8个开源MLLM上提升达8.7个百分点。

Conclusion: 将嵌入空间与查询的高维结构对齐是推理时视频理解的一个有前景方向。
...
TL;DR: 提出ScaleResfusion，一种基于预训练文本到图像整流流模型的可扩展真实图像恢复扩散框架。


<details>
  <summary>Details</summary>
Motivation: 现有扩散方法存在从高斯噪声启动速度慢、保真度低，以及残差方法难以利用预训练生成先验的问题。

Method: 引入残差整流流，用残差传输路径替代纯噪声启动，保持与预训练模型的一致性，实现参数高效微调；并设计知识蒸馏管道降低采样成本。

Result: 在多个真实恢复任务上达到最先进性能，且效率更高。

Conclusion: 提供了一种实用且可扩展的方式将大型预训练扩散模型适配到真实图像恢复。
...
TL;DR: FunnelAL将工业推荐系统的多阶段漏斗架构应用于主动学习，通过检索-排序-混合探索实现单类发现，提升标注效率与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大规模监督学习面临高效筛选相关样本与区分真假阳性两个挑战，传统主动学习作为单阶段过程无法兼顾两者。

Method: 采用级联漏斗：1）嵌入检索缩减候选集；2）精度触发排序阶段，自动从RankNet切换至QBC；3）标注反馈迭代优化两阶段。

Result: 在三个图像基准上取得最优最终F1和AULC（标注效率），标注轮次最少；面对标注误差时鲁棒性显著优于传统方法。

Conclusion: FunnelAL有效桥接多阶段推荐系统与主动学习，在单类发现任务中超越现有方法。
...
TL;DR: CLBench-V是一个多模态上下文学习基准，包含3,443个实例，评估六个最新模型，最佳得分仅为0.2847，表明该任务远未饱和。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注文本上下文，但实际任务中上下文往往多模态，现有方法难以定位上下文使用失败环节。

Method: 围绕上下文基础、新信息应用、新知识学习三个维度组织任务，结合转换的公开基准和新构建数据集，采用自动构建和过滤流程降低成本。

Result: InternVL3.5-30B-A3B在上下文基础和新知识学习上最佳，Qwen3.5-Plus在新信息应用上最佳，总体最佳得分0.2847。

Conclusion: 多模态上下文学习仍具挑战性，CLBench-V为细粒度评估提供有效基准，代码已开源。
...
### [139] [Sense it with your eyes: Sensation Generation and Understanding for Advertisements](https://arxiv.org/abs/2607.25314)
*Aysan Aghazadeh,Sina Malakouti,Adriana Kovashka*

Main category: cs.CV

TL;DR: 本文首次系统研究感官广告的理解、评估与生成，提出了感官广告数据集、分类任务、自动化评估指标以及多智能体生成框架。


<details>
  <summary>Details</summary>
Motivation: 人工智能在创意和说服性内容中的应用日益增多，但广告如何通过视觉引发感官体验这一关键问题尚未被探索。

Method: 构建感官广告数据集，定义感官分类任务以评估大型语言模型和多模态大模型；提出自动化评估指标SenseScore；设计多智能体框架SAGA用于生成感官广告。

Result: SenseScore与人类判断高度一致；SAGA框架在信息-图像对齐、感官唤起和说服力方面表现优异。

Conclusion: 该研究为感官驱动的视觉说服奠定了基础，推动了AI在广告领域的深入应用。
...
### [140] [MEDit-Bench: A Dataset for Evaluating Message-Driven Narrative Video Editing](https://arxiv.org/abs/2607.25300)
*Katsuya Ogata,Zongshang Pang,Mayu Otani,Yuta Nakashima*

Main category: cs.CV

TL;DR: MEDit-Bench是一个用于评估消息驱动视频编辑的数据集和基准，通过多消息、多专业编辑配对长视频，发现不同消息导致显著不同的编辑结果，并揭示了现有模型在严格时间对齐标准下与人类专业编辑的差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频摘要基准忽略了编辑意图的多样性，无法评估消息驱动的视频编辑任务。

Method: 构建包含长视频、多条编辑消息及对应专业编辑的数据集，采用时间对齐指标和LLM偏好评估，并引入模糊性和语境性评分。

Result: 强模型在宽松阈值下接近人类时间对齐，但在严格标准下落后；LLM偏好评估因位置偏差不可靠；模糊性和语境性与模型性能负相关。

Conclusion: 专业的人类编辑始终优于模型输出，消息难度是重要的分层因素，当前模型在精细编辑任务上仍有显著差距。
...
### [141] [Beyond Background Bias: Saliency-Driven Prototype Alignment for Dataset Distillation](https://arxiv.org/abs/2607.25318)
*Yawen Zou,Wenqi Cai,Guang Li,Ling Xiao,Chunzhi Gu,Chao Zhang*

Main category: cs.CV

TL;DR: 提出一个基于显著性驱动的蒸馏框架，通过构建类别判别性潜在原型来增强扩散模型数据集蒸馏的结构一致性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 扩散蒸馏方法在视觉复杂领域难以保持结构一致性和泛化性，原因是潜在原型与类别判别区域弱对齐且受无关背景污染。

Method: 两阶段框架：先使用集成Grad-CAM显著性构建高置信区域原型，再通过困难原型精炼增强判别性和多样性，保持扩散主干冻结仅训练轻量分类器。

Result: 在多个基准数据集上，该方法一致性地优于强基线方法。

Conclusion: 显著性驱动的原型构建能有效提升扩散蒸馏的数据质量与泛化性能。
...
### [142] [Balanced Soft mixture-of-expert model for Glaucoma Detection](https://arxiv.org/abs/2607.25324)
*Sai Venkatesh Chilukoti,Krishna Rauniyar,Min Shi,Xiali Hei*

Main category: cs.CV

TL;DR: 提出平衡软混合专家模型，通过负载均衡损失和三个专家网络，改进多模态青光眼检测性能，超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态学习面临单模态表示不平衡和优化不足的问题，导致联合学习目标下性能受限。

Method: 采用三个专家网络的平衡软混合专家模型，结合负载均衡损失。

Result: 在AUC指标上超越所有单模态基线、传统多模态模型及当前最优平衡多模态模型。

Conclusion: 该模型可推广至糖尿病视网膜病变等其他疾病检测。
...
### [143] [PanoLess: Environment Reconstruction from Partial Reflective Views](https://arxiv.org/abs/2607.25362)
*Ahitagni Das,Ashok Veeraraghavan,Vivek Boominathan*

Main category: cs.CV

TL;DR: 提出了基于高斯溅射的PanoLess框架，通过单侧反射表面的图像重建远距离环境光照图。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要360度全覆盖，无法处理部分视角输入的问题。

Method: 利用表面对齐2D高斯溅射和延迟着色恢复像素法线和反射线索，融合为神经立方体贴图，并生成可见性图。

Result: 在合成基准和公开数据集上优于反射感知基线，泛化到真实场景。

Conclusion: 实现了部分视角下高保真、几何一致的环境重建。
...
### [144] [Hyperspectral Intrinsic Decomposition: Joint Recovery of Reflectance and Photometric Components for Non-Lambertian Scenes](https://arxiv.org/abs/2607.25371)
*Hao Ye,Zhan Shi,Chenglong Huang,Tao Lv,Mingjie Ji,Qiu Shen,Xun Cao*

Main category: cs.CV

TL;DR: 提出了一种基于双色反射模型的双尺度分解方法，用于高光谱图像的非朗伯本征分解，并构建了首个真实世界非朗伯HID数据集CITE。


<details>
  <summary>Details</summary>
Motivation: 现有HID方法多基于朗伯假设，难以处理真实世界中多样的非朗伯反射特性，且依赖辅助输入或只能恢复部分分量。

Method: 将双色反射模型中的四个耦合分量重写为两个光谱-空间目标变量，通过全局尺度（光度不变描述子作为边缘先验）和局部尺度（镜面引导注意力机制）双尺度分解非朗伯效应。

Result: 在CITE数据集及其他高光谱图像上的实验表明，该方法能有效进行盲分解，对多种场景具有鲁棒性。

Conclusion: 提出的双尺度分解范式能更好地处理非朗伯效应，CITE数据集和PISG合成器为未来研究提供了基准和工具。
...
TL;DR: 提出了一个封闭了两个常见数据稀缺性漏洞的沙沸点分割框架，通过在合成图像中保留指向真实父图像的指针，排除父图像被保留的合成图像，并使用五折交叉验证和逐像素元学习器，在测试集上实现了0.707的IoU。


<details>
  <summary>Details</summary>
Motivation: 现有方法在数据稀缺时常通过调整集成权重（在测试集上调整）或使用与测试集同源的合成图像训练来虚假提升准确率，需要封闭这些漏洞。

Method: 每个合成图像携带指向真实父图像的指针，在每折中排除父图像被保留的合成图像；使用五个编码器-解码器骨干网络进行五折交叉验证，每个网络用一个温度标量校准，并通过仅在折外预测上训练的逐像素元学习器进行集成。

Result: 在保留测试集上，提出的Updated SandBoilNet达到0.707 IoU，而原始方法为0.608；堆叠协议下校准堆栈为0.681，未优于最强单模型（0.694），元学习器效果受限；合成池筛选后单模型提升至0.718，掩码条件合成路径无需注释成本。

Conclusion: 封闭数据增强和集成中的漏洞后，堆叠未能提升单模型性能，原因是成员间误差相关性高（0.894）；掩码条件合成路径提供零注释成本的标注图像。
...
### [146] [Gaussian Volumetric Representation for Efficient Shear-Warp Visualization](https://arxiv.org/abs/2607.25377)
*Mayuri Mathur,Ojaswa Sharma*

Main category: cs.CV

TL;DR: 提出了一种基于高斯的体积表示方法，用于高效渲染密集医学体积数据，保持结构细节和成像质量，同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 医学图像可视化需要在保持解剖结构保真度的同时实现高速渲染，但密集体积数据集计算成本高。

Method: 使用蒙特卡洛体积估计优化高斯表示，在稀疏体素上训练并保持稠密目标一致性；引入课程学习策略，逐步结合切片采样以捕获结构连续性和纹理。

Result: 在稀疏监督下，方法实现了高达43.86 FPS的渲染速度和11.31:1的压缩比，适用于MRI和冷冻切片等多模态医学数据。

Conclusion: 该方法在减少计算开销的同时，有效保留了解剖细节和纹理，验证了高斯表示在高效医学体积可视化中的潜力。
...
### [147] [HOME: Robust Hough-space Matching Method for Structured and Textureless Videos](https://arxiv.org/abs/2607.25389)
*Masaki Satoh*

Main category: cs.CV

TL;DR: 提出一种基于Hough空间极值点一维匹配的超轻量无训练特征匹配框架HOME，在结构化和无纹理环境中实现高效鲁棒的图像配准。


<details>
  <summary>Details</summary>
Motivation: 点特征在强线性结构或无纹理环境中易失效，而传统线特征算法计算开销大，难以满足实时边缘机器人需求。

Method: 将图像转换到Hough空间，将全局线性结构映射为稳定局部极值点作为关键点，利用一维径向描述子实现旋转和平移不变性匹配。

Result: 在单应性估计任务中，HOME比点特征更鲁棒（在点方法失效场景下仍能注册），比线特征方法速度更快。

Conclusion: HOME是一种高效鲁棒的匹配框架，未来可扩展至完整3D姿态估计。
...
### [148] [Noise-Free One-Step LoRA for Task-Driven Image Restoration with Diffusion Priors](https://arxiv.org/abs/2607.25390)
*Jaeha Kim,Kyoung Mu Lee*

Main category: cs.CV

TL;DR: 提出用确定性一步前向传播结合扩散先验改进任务驱动图像恢复（TDIR），并通过任务保留GAN训练提升感知质量，在多个视觉任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 退化图像降低视觉质量和下游任务性能；现有扩散先验TDIR方法因随机采样破坏任务一致性。

Method: 使用扩散先验的确定性无噪声一步前向传播，配合LoRA适配模块；引入任务保留GAN训练策略。

Result: 在分类、分割、检测任务上优于传统多步扩散TDIR；在真实退化图像和OCR上验证泛化性。

Conclusion: 确定性单步扩散先验配合LoRA可有效改进TDIR，任务保留GAN在提升感知质量时不牺牲任务性能。
...
### [149] [RDVSv2: A Large-scale Benchmark for RGB-D Video Salient Object Detection](https://arxiv.org/abs/2607.25392)
*Tianyu Li,Jiahao He,Keren Fu,Qijun Zhao*

Main category: cs.CV

TL;DR: 提出了一个大规模RGB-D视频显著目标检测基准RDVSv2，包含249个视频序列和29,077个带注释帧，并基于SAM2建立了强基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D视频显著目标检测数据集规模小、标注质量差，且深度线索几何一致性不足。

Method: 从公开立体视频构建数据集，使用眼动追踪指导标注；基于SAM2采用参数高效微调策略联合编码RGB、深度和光流线索。

Result: 数据集更具挑战性，所提基线在RDVSv2及现有基准上达到最先进性能。

Conclusion: RDVSv2和基线方法将为RGB-D VSOD及多模态视频理解提供重要资源。
...
### [150] [Pictura: Perspective-View Self-Play at Scale for Driving](https://arxiv.org/abs/2607.26005)
...
TL;DR: 提出Pictura：首个大规模驾驶自博弈训练框架，直接以透视视角图像为输入，无需特权观测，实现从图像到驾驶策略的端到端学习。


<details>
  <summary>Details</summary>
Motivation: 现有自博弈驾驶策略依赖特权向量观测（如精确位姿、速度），与真实部署中仅能获取部分观察（如摄像头图像）之间存在表征鸿沟。

Method: 构建GPU加速多智能体驾驶模拟器Pictura，每步渲染各智能体自车视角图像；采用PPO算法直接从透视图像训练自博弈策略（模型Alberti）。

Result: 单块H100上达到5×10^5代理步/秒（2×10^6图像/秒）；训练50B代理步（约3.5×10^7公里）；性能逼近特权威向量策略，在Waymo数据集零样本迁移中超越后者。

Conclusion: 透视视角自博弈能有效消除表征鸿沟，产出可直接从图像端到端学习的稳健驾驶策略，并具备优异的零样本泛化能力。
...
TL;DR: 提出数据-模型协同优化的染色转移框架DMCoStain，通过多模态专家指导的精细选择策略（MEGFS）和免疫组化阳性表达视觉语言模型（IPE VLM），实现从H&E到IHC的高精度、可解释染色转移，在多个组织与生物标志物上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: IHC染色提供关键分子信息但成本高、需专业操作；现有染色转移方法在异质性生物标志物的像素非对齐监督下缺乏统一且可解释的建模。

Method: 提出DMCoStain框架，包含：(1) 数据-模型协同迭代优化，(2) MEGFS策略（基于IPE VLM模拟病理学家推理筛选训练数据），(3) 构建大规模IPT指令跟随数据集ImmunoInstruction。

Result: 在多种组织和生物标志物上，DMCoStain达到最先进的染色转移精度，同时MEGFS可作为专门的评估工具。

Conclusion: 该框架具有强实用价值，通过数据与模型的协同优化显著提升染色准确性与可解释性，为未来模型开发提供新范式。
...
TL;DR: 提出自适应邻居特征交互方法应对重识别中邻居噪声问题，融合亲和与差异关系，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于邻居的方法依赖亲和关系，在噪声邻居存在时效果不稳定，需要更鲁棒的交互机制。

Method: 提出ANFI方法，同时建模亲和与差异关系，并采用样本级自适应权重；通过新邻里相似度推导差异关系；提出噪声关系监督训练。

Result: 在标准、跨模态和跨域场景下，与邻居方法和重排序方法相比，性能更优。

Conclusion: ANFI能有效处理噪声邻居，提升重识别鲁棒性，适用于多种邻居分布。
...
TL;DR: 提出了一种双层协作学习框架，用于在少量涂鸦标注的医学图像分割任务中，通过可学习超像素模型和双向交互提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决实际临床中涂鸦标注样本稀缺导致的稀疏监督和结构证据不足问题。

Method: 上层可学习超像素模型提供区域结构先验，下层分割模型通过超像素伪标签传播和空间先验过滤生成密集伪标签，双向协作优化。

Result: 在ACDC和Prostate数据集上，在少量涂鸦标注设定下显著优于现有方法。

Conclusion: 双层协作学习框架有效缓解了稀疏监督和样本稀缺的双重挑战，提升了分割准确性。
...
TL;DR: 基于深度学习的古罗马共和国钱币文字识别方法，使用YOLOv7系列在自建数据集上达到90.4% mAP50。


<details>
  <summary>Details</summary>
Motivation: 钱币上的文字铭文对分类至关重要，但存在文字放置不均、工艺原始、磨损及成像条件差异等挑战。

Method: 收集包含5654张图像的罗马共和国钱币数据集，标注21类文字标签共38808个，采用YOLOv3至v8系列进行目标检测。

Result: YOLOv7-Large取得最佳mAP50为90.4%，YOLOv7-Extended和YOLOv7-xl分别达90.2%和90.1%。

Conclusion: 深度学习方法可有效识别古钱币上的文字，YOLOv7系列在精度上具有优势。
...
TL;DR: 本文提出DensFiLM，一种基于密度调节的视频显著性模型，通过在Video Swin Transformer瓶颈处插入轻量级特征线性调制层，仅增加约100K参数，在CrowdFix数据集上平均NSS 1.434、CC 0.517，性能优于ACLNet 14.7%和14.9%。


<details>
  <summary>Details</summary>
Motivation: 现有视频显著性模型在人群场景中使用单一注视策略，忽略了注意力随人群密度系统性的变化（稀疏场景追踪个体，密集场景关注集体运动），需要一种密度条件化的方法。

Method: 在Video Swin Transformer瓶颈处插入轻量级特征线性调制层，使用密度嵌入生成通道级缩放和平移参数，实现密度条件化的显著性重建；支持真值密度标签或模型自身密度预测。

Result: 在CrowdFix上平均NSS 1.434、CC 0.517，较ACLNet提升14.7%和14.9%；预测密度与真实标签性能相当；消融实验显示光流和更大模型扩展无额外收益；中心先验消融诊断中密度调节带来0.462的NSS增益。

Conclusion: 轻量级瓶颈条件化提供了比增加模型容量更有效的归纳偏置，用于人群视频显著性预测。
...
TL;DR: 提出Causal Visual Memory Audit（CVMA）框架，发现当前注意力机制无法可靠预测未来对视觉信息的依赖，安全遗忘取决于未来视觉依赖度低或事实已通过文本表达。


<details>
  <summary>Details</summary>
Motivation: 多模态助手编码图像后可能在未来回合回答相关问题，现有视觉键值驱逐策略基于当前注意力判断，但未来问题未知，需要明确何种视觉事实可以安全遗忘。

Method: 提出CVMA框架，通过配对单预填充方式测试移除视觉区域、整张图像或先前助手文本对后续答案的影响，结合诊断性边际效用控制。

Result: 在VisDial和ConvBench上，当前注意力对未来有用区域的排序甚至差于随机；聚合分数掩盖了失败情况；辅助文本KV可替代图像KV用于已陈述事实，但无法可靠替代未陈述事实。

Conclusion: 安全遗忘依赖于未来视觉依赖度低或事实已被文本表达，而非当前注意力低。
...
TL;DR: 提出CascadeCropNet级联架构，通过阈值校准在推理时控制漏检率，在农业保险图像中实现受损召回率0.974，减少54%漏检。


<details>
  <summary>Details</summary>
Motivation: 农业保险中漏检代价远高于误报，标准分类器无法控制非对称成本。

Method: 两阶段级联：轻量Sentinel模型做健康三分类，超过损伤概率阈值τ的样本交由专家模型细粒度诊断。

Result: 在23,804张肯尼亚玉米田图像上，τ=0.5时受损召回率0.974，漏检减少54%，F1宏仅差+0.008。

Conclusion: 级联架构可通过校准路由实现安全导向的决策约束，但依赖于阈值和部署条件，不能保证任意分布偏移下的性能。
...
TL;DR: 综述了医疗AI中智能体系统的范围与评估现状，指出其多步临床任务能力但缺乏严格临床验证。


<details>
  <summary>Details</summary>
Motivation: 医疗AI正从孤立预测转向多步临床任务，但智能体AI在医学中的范围未定，评估实践与临床需求不匹配。

Method: 对5个电子来源进行范围综述和系统证据映射，筛选1649条记录，最终纳入557项满足目标导向执行、工具使用等标准的研究。

Result: 研究涵盖单智能体、检索增强工作流、多模态及多智能体系统，用于问答、影像解读等任务；但证据以公共基准、模拟和回顾性数据为主。

Conclusion: 临床转化需明确界定、可重复评估、可审计监督、互操作设计及真实世界前瞻验证；过程可靠性等属性评估不足。
...
### [159] [Group Equivariant Diffusion for Anomaly Detection in Computational Cytology](https://arxiv.org/abs/2607.25503)
*Swarnadip Chatterjee,Ssharvien Kumar Sivakumar,Anirban Mukhopadhyay*

Main category: cs.CV

TL;DR: 提出D4等变扩散模型，通过旋转和反射对称性提升细胞学全切片图像异常检测的稳定性和准确率。


<details>
  <summary>Details</summary>
Motivation: 恶性细胞罕见、异质性强且标注稀缺，现有无监督异常检测方法对图像变换敏感，导致异常评分不稳定。

Method: 采用D4等变U-Net架构、等变噪声耦合和帧平均策略，使模型对旋转和翻转具有不变性。

Result: 在骨髓和外周血涂片两个公开数据集上，AUC和Top-K异常细胞检索率均优于非等变生成模型、深度单类及多实例学习方法，且评分方差显著降低。

Conclusion: D4等变扩散模型能有效利用细胞层面的对称先验，提升异常检测的性能和鲁棒性。
...
### [160] [Beyond Counts: A Distributional Robustness Margin For Pathology Foundation Models](https://arxiv.org/abs/2607.25497)
*Clément Grisi,Jeroen van der Laak,Geert Litjens*

Main category: cs.CV

TL;DR: 本文提出Cross-confounder Robustness Margin (CRoMa)，一种样本级分布度量，用于评估病理基础模型对非生物变异（如染色、扫描差异）的鲁棒性，超越传统计数型鲁棒性指数(RI)的局限性。


<details>
  <summary>Details</summary>
Motivation: 病理基础模型在部署时易受医院间非生物变异（如组织制备、染色和扫描差异）影响，导致捷径学习与泛化能力下降。现有鲁棒性指数(RI)虽能区分生物与非生物变异，但其基于计数的池化固定邻域设计忽略了距离信息和样本异质性，仅评估模型依赖的子集。

Method: CRoMa通过直接比较跨混杂因素生物匹配样本与同混杂因素生物干扰样本的距离，将鲁棒性重构为整个队列的裕度分布，而非单一池化分数。实验采用20个块级编码器（3个基准）和4个全切片级编码器（1个基准）的冻结表征。

Result: 中位CRoMa排名在数据集间一致，但分布揭示模型内显著异质性：所有块级编码器均存在混杂因素主导的下尾（劣质样本），其普遍性和严重性因模型而异。高CRoMa与监督适应后更小的捷径性能下降相关。

Conclusion: CRoMa将表征几何转化为分布级鲁棒性读数，可预测下游捷径敏感性，为鲁棒性评估和模型选择提供原则性基础，将典型鲁棒性与下尾鲁棒性建模为帕累托权衡。
...
### [161] [ReLATE: Reliability-Guided Evidence Fusion for Robust UAV--Satellite cross-view Geo-Localization](https://arxiv.org/abs/2607.25524)
*Haochen Jiang,Jialei Pan,Yuzhe Sun,Zhe Dong,Lecheng Ren,Yanfeng Gu,Tianzhu Liu*

Main category: cs.CV

TL;DR: 提出了一个用于无人机-卫星跨视角地理定位的鲁棒性基准UAVSat-Deg，并设计了ReLATE方法，通过可靠性自适应的特征融合来应对图像退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在干净图像上表现优异，但实际飞行中常受天气、光照、运动等退化影响，而鲁棒性研究不足。

Method: 构建了包含27种退化类型的大规模基准；提出了ReLATE框架，通过估计视觉token的结构平滑可靠性场，聚合可靠局部证据并自适应融合到查询表征中。

Result: 在两种测试集和检索方向上，ReLATE在退化测试上达到最佳平均性能，且保持干净图像的高精度。

Conclusion: 所提基准揭示了现有方法的鲁棒性缺陷，而ReLATE有效提升了退化场景下的定位能力。
...
### [162] [I2VShield: An Efficient Proactive Defense Framework against DiT-based Image-to-Video Models](https://arxiv.org/abs/2607.25522)
*Yimao Guo,Zuomin Qu,Wei Lu*

Main category: cs.CV

TL;DR: 提出I2VShield，一种针对DiT架构的图像转视频模型的生成式对抗攻击防御方法，通过文本自适应扰动和跨模态注意力破坏来保护隐私，在降低计算成本的同时有效破坏生成视频的时空连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有针对I2V模型的主动防御方法依赖梯度攻击，需要大量显存资源，存在计算开销大的问题。

Method: 包含两个组件：(1) 文本自适应扰动生成框架，通过对抗训练减少计算开销并保持视觉隐蔽性；(2) 无目标的多模态注意力破坏攻击，利用DiT模型内在漏洞，最大化内部注意力特征与干净状态的偏差。

Result: 在多个数据集和主流DiT-I2V模型上，该方法实现了高竞争性保护性能，尤其在破坏时空连贯性方面表现突出，同时大幅降低计算成本。

Conclusion: I2VShield为图像转视频模型的主动防御提供了一种高效、低计算成本的隐私保护方案。
...
TL;DR: Argus-Unified是一个紧凑、有效且低成本的统一多模态模型，通过混合视觉令牌和两阶段训练，在理解和生成任务上均表现优异。


<details>
  <summary>Details</summary>
Motivation: 统一视觉理解和生成模型面临计算和数据需求高、视觉特征冲突的挑战，现有方法昂贵且数据密集。

Method: 利用预训练视觉-语言模型（VLM）的多模态先验，引入混合视觉令牌（连续令牌用于理解，离散令牌用于生成），采用两阶段训练：先学习量化器和图像解码器，再训练LLM进行统一建模。

Result: 使用最少数据（15.6M）和最低成本（~$2,000），在GQA、POPE和VQAv2上达到最先进的多模态理解性能，生成质量与Janus等模型相当，但成本低10倍、数据少5倍。

Conclusion: 统一多模态模型可以经济地训练并取得强性能，Argus-Unified降低了开发门槛，可作为有用基线。
...
TL;DR: 视觉提示工程（VIPE）通过自动修改任务图像，提升视频基础模型在视觉推理任务中的表现，其效果优于文本提示工程和测试时缩放。


<details>
  <summary>Details</summary>
Motivation: 在基础模型时代，模型性能高度依赖提示质量，而视频模型逐步成为视觉任务的基础模型，因此探索视觉提示工程是否类似文本提示工程能提升模型性能。

Method: 对任务图像进行视觉提示工程，例如将抽象草图转换为照片级真实图像，利用图像编辑模型自动调整。

Result: VIPE在多个视频推理任务中提升了模型性能，且比文本提示工程和测试时缩放更有效。

Conclusion: 视觉提示工程可作为简单、计算高效的方法，系统提升视频模型的视觉推理能力，类似于文本提示工程对语言模型的提升作用。
...
### [165] [Forensic Reproducibility Audit of a Radiology Vision-Language Model Benchmark: From Intended Protocol to Released Artifact](https://arxiv.org/abs/2607.25589)
*Mateusz Kozłowski*

Main category: cs.CV

TL;DR: 对CXR-VLM试点进行法医重现性审计，发现基准架构中存在未检出的错误链，多数原始结论无法重现。


<details>
  <summary>Details</summary>
Motivation: 假设基准各处组件间一致，而忽视实际运行中的错误与偏差。

Method: 回顾性法医审计，追溯提示绑定、DICOM元数据、输出完整性等七类工件，重建完整队列并统计比较。

Result: 300次调用中297次非空，Claude标签混淆，4张图像极性未反转，抽取器截断5份报告，单队列重建后Cochran's Q从154.73变为182.29，多数比较P<0.05。

Conclusion: 撤回原始性能、排名、提示效应及临床主张，并规范了机器可验证的控制条件。
...
### [166] [Less is More: Modality-Decoupling for General AIGC Audio-Video Detection](https://arxiv.org/abs/2607.25543)
*Jielun Peng,Yabin Wang,Yaqi Li,Jincheng Liu,Xiaopeng Hong,Athanasios V. Vasilakos*

Main category: cs.CV

TL;DR: 提出了一种解耦的音视频AIGC检测系统DAV-Det，独立建模各模态的取证证据，在通用场景下验证了决策级融合的鲁棒性优于特征级融合。


<details>
  <summary>Details</summary>
Motivation: 现有AIGC检测方法假设音视频内容对应关系，但该假设在通用场景中不成立，需探索更鲁棒的融合策略。

Method: 视觉检测器：利用全局、补丁和片段多粒度表示捕获空间伪造线索；音频检测器：通过门控时域-频谱双分支架构建模音效伪影。采用决策级融合独立处理各模态。

Result: 在IJCAI-ECAI 2026 DDL 2.0 Workshop通用AIGC音视频检测挑战赛中排名第一，最终得分0.8460。

Conclusion: 解耦的决策级融合方法在通用场景下优于传统特征级融合方法，可有效检测多种AIGC音视频伪造。
...
### [167] [Few-Shot Open-Vocabulary Remote Sensing Segmentation via Textual Inversion](https://arxiv.org/abs/2607.25563)
*Junhyuk Heo,Junghwan Park*

Main category: cs.CV

TL;DR: 开放词汇分割在空中遥感图像上表现不佳，主要问题在于文本查询而非分割模型。通过文本反转为查询匹配更好描述，可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇分割模型在遥感图像上表现差，需探究原因并改进。

Method: 通过文本反转（textual inversion）在冻结模型上从少量示例中学习更好的文本查询。

Result: 受影响类别的平均交并比从3.9提升至39.4，在8个遥感数据集中优于基于视觉提示的少样本方法。

Conclusion: 改进文本查询能部分解决遥感图像分割问题，但仍有部分失败案例需要更优描述。
...
### [168] [Instruction-based Image Editing: A Survey on Data, Models, Evaluation, and Applications](https://arxiv.org/abs/2607.25642)
...
TL;DR: 本文系统综述了指令式图像编辑的研究，提出了五维分类法，并构建了CDD-IIE基准测试，通过实验对比开源解决方案，指出了能力与局限，展望了未来方向。


<details>
  <summary>Details</summary>
Motivation: 指令式图像编辑在大语言模型和视觉语言模型进展下发展迅速，但缺乏系统分类和全面评估基准。

Method: 采用五维分类框架（任务定义、数据构建、模型架构、评估指标、商业方案），并构建了CDD-IIE基准测试。

Result: 总结了从GAN到扩散模型和自回归范式的发展历程，识别了关键技术里程碑，并通过实验对比揭示了各方案的优缺点。

Conclusion: 指令式图像编辑技术取得重要进展，但存在局限，未来需关注更鲁棒的评估方法和实际应用落地。
...
### [169] [The LAIA Dataset: Labelled Attention for Intelligent Automobiles](https://arxiv.org/abs/2607.25570)
*A. Contreras,D. Porres,R. Abad,P. Cano,G. Villalonga,A. M. López,A. Hernández-Sabaté*

Main category: cs.CV

TL;DR: 提出LAIA数据集，包含15小时人类驾驶员注视数据及多模态传感器数据，用于提升端到端自动驾驶模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型的可解释性不足，限制其实际应用。

Method: 通过CARLA模拟器采集44名驾驶员在多种场景下的RGB图像、分割图、深度图、光流、CAN总线及眼动追踪数据。

Result: 成功对比人类注意力与模型注意力，揭示模型行为模式。

Conclusion: LAIA数据集能有效支持注意力感知驾驶模型、行为预测及异常检测等研究，并提升模型可解释性。
...
### [170] [ReDesign: Recovering Editable Design Structures from Images via Agentic Decomposition](https://arxiv.org/abs/2607.25565)
*Jooyeol Yun,Jintae Park,Hyesu Lim,Junha Hyung,Hyungjin Chung,Jaegul Choo*

Main category: cs.CV

TL;DR: 提出ReDesign代理框架，通过选择跨模态工具逐步构建可编辑图层层次，并引入优雅验证机制防止错误累积，在Figma编辑重放基准上实现了高视觉保真度和最佳编辑性。


<details>
  <summary>Details</summary>
Motivation: 从栅格图像恢复可编辑设计文件是现代设计工作流程中的常见瓶颈，且因多模态属性（如排版、矢量几何等）的恢复而具有挑战性。

Method: 代理框架ReDesign，通过选择并组合跨模态工具逐步构建可编辑图层层次，并在每次扩展中引入优雅验证（接受、修剪或重试反馈）以防止错误累积。

Result: 在包含909个原始Figma文件和14,796条编辑指令的Figma编辑重放基准上，ReDesign实现了强视觉保真度，并在布局、颜色和文本编辑方面获得最高编辑性，优于分层分解基线和串行工具使用流程。

Conclusion: ReDesign框架通过可验证的多模态工具组合和错误抑制机制，有效提升了从栅格图像恢复的可编辑设计文件的编辑能力。
...
### [171] [Beyond Facial Consistency: Personalized Person Image Generation with Holistic Identity Preservation](https://arxiv.org/abs/2607.25622)
*Yuxuan Xiao,Shanshan Zhang,Jian Yang,Shengcai Liao*

Main category: cs.CV

TL;DR: 提出动态平衡缩放（DBS）微调策略，通过自适应时间门控和区域感知优化，协调全局外观与局部人脸控制，实现人物图像生成中面部保真度与外观一致性的更好权衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅强调单级身份信息，导致面部保真度与整体外观一致性之间存在固有权衡。

Method: 双分支统一框架 + 动态平衡缩放（DBS）：自适应时间门控动态调节分支贡献，区域感知优化协调面部、外观与全局监督。

Result: 在Pexels-100基准上，DBS比现有开源基线实现了面部保真度与外观一致性的更好权衡。

Conclusion: DBS有效协调了人脸与外观身份，为整体身份建模提供了可控基础框架。
...
### [172] [OmniPhys: Knowledge-Graph-Driven Benchmarking and Collective Optimization for Physical Commonsense in Text-to-Image Generation](https://arxiv.org/abs/2607.25641)
*Yajing Xu,Yarong Lan,Jiaoyan Chen,Yichi Zhang,Jeff Z. Pan,Mingchen Tu,Zhizhen Liu,Wen Zhang,Huajun Chen*

Main category: cs.CV

TL;DR: 提出了OmniPhys基准和OmniPrompt框架，用于评估和提升文生图模型的物理一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基准粒度粗、无法诊断具体物理原理掌握情况，且优化方法受梯度幻觉误导。

Method: 基于物理知识图谱构建OmniPhys（1551样本），通过双路径验证协议进行压力测试；OmniPrompt将物理对齐视为离散优化问题，聚合随机图像反馈并合并批量查询噪声后进行元策略更新。

Result: 在12个文生图模型上发现普遍物理瓶颈，OmniPrompt显著提升物理一致性且具有跨骨干可迁移性。

Conclusion: OmniPrompt能有效改进文生图模型的物理合理性，验证了所提元策略的可转移性和有效性。
...
TL;DR: 提出了一种基于时空图卷积网络、门控循环单元和双向长短期记忆网络的方法，通过3D骨骼图数据准确检测跌倒事件中的冲击时刻，准确率超过90%。


<details>
  <summary>Details</summary>
Motivation: 老年人跌倒致死风险高，现有跌倒检测系统泛化性较好，但难以区分真正跌倒冲击和假性跌倒。

Method: 使用STGCN从3D骨架关节数据中提取时空特征，结合GRU和BiLSTM层精确定位冲击时刻。

Result: 在改进的UP-Fall数据集上，该方法在各种跌倒场景下均实现了超过90%的准确率。

Conclusion: 该方法通过区分假性跌倒和实际冲击，提升了跌倒检测的精准度，有助于优化医疗资源配置。
...
### [174] [A Unified Benchmark and Modality-Adaptive Network for Day-and-Night Drone-View Geo-Localization](https://arxiv.org/abs/2607.25778)
*Songtianhao Xu,Zhongwei Chen,Zhao-Xu Yang,Weifeng Wang*

Main category: cs.CV

TL;DR: 提出IRCHN基准和MASTR-Net方法，解决无人机图像在昼夜不同光照条件下的地理定位问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏昼夜一致的多模态数据，导致跨光照定位评估困难。

Method: 构建IRCHN数据集（含可见光、红外和卫星图像），设计MASTR-Net框架，包含模态自适应增强、双向选择性状态空间关系建模和软最优传输对齐。

Result: MASTR-Net在IRCHN上优于现有方法，并在红外基准IR-VL328和CVGL-RGBT上表现竞争力。

Conclusion: IRCHN和MASTR-Net有效评估和提升了跨光照条件下的无人机定位性能。
...
### [175] [A systematic evaluation of machine learning classifiers for event-by-event background rejection in LAFOV PET scanners](https://arxiv.org/abs/2607.25732)
...
TL;DR: 该研究将LAFOV PET中的符合事件分类建模为有监督多分类问题，评估了XGBoost、AdaBoost和神经网络作为预处理滤波器的性能，发现基于4特征模型的泛化能力优于6特征模型，但分类器对散射符合事件的抑制有限。


<details>
  <summary>Details</summary>
Motivation: LAFOV PET扫描仪在带来高灵敏度的同时，也显著增加了随机符合、散射符合等背景噪声，而现有机器学习方法主要针对后处理环节的特定背景成分，缺乏对原始数据的事件级分类。

Method: 采用蒙特卡洛模拟基于Siemens Biograph Vision Quadra扫描仪，使用NEMA IEC和XCAT体模数据，将符合事件分类作为监督多分类问题，比较XGBoost、AdaBoost和神经网络在4特征（衰减因子、光子时间差、能量和、能量差）和6特征（加入拓扑变量）下的性能。

Result: 4特征模型在跨体模推理中泛化性能更好（XGBoost准确率损失仅0.04 vs 6特征的0.13），最佳模型在NEMA IEC和XCAT体模上分别达到0.74和0.69的准确率，优于传统几何切割方法。

Conclusion: 紧凑特征集可有效替代传统几何切割，但对体模内散射符合抑制有限，在临床条件下散射符合是主要背景，需更大输入表示或专门处理散射分量才能进一步改善图像质量。
...
### [176] [Image Quality Dependent Degradation for AI Systems](https://arxiv.org/abs/2607.25736)
*Yannick Kees,Elena Hoemann,Frank Köster,Sven Hallerbach*

Main category: cs.CV

TL;DR: 提出一种针对自动驾驶中图像质量下降时的AI系统容错策略，通过基于图像质量调整置信度阈值，并利用归一化流估计图像质量，实现关键错误规避。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的目标检测高度依赖输入图像质量，低质量图像（如噪声、暗光）会导致严重检测错误（如漏检行人），而漏检比误检的危害更大，因此需要一种能处理低质量图像的稳健方法。

Method: 提出一个‘故障降级’系统：1）利用归一化流将输入图像与训练数据比较，实时估计图像质量；2）根据估计的质量动态降低网络的置信度阈值，使网络在不确定情况下更谨慎地检测目标。

Result: 实验表明该方法能应用于最先进的目标检测模型，在图像质量下降时仍能避免最关键的漏检错误。

Conclusion: 该设计策略使自动驾驶AI系统能在低质量输入下不依赖后备方案，提升系统信任度和可用性。
...
### [177] [Towards Faithful Sentimental Image Captioning via Evidence-Aware Multi-Agent Reasoning](https://arxiv.org/abs/2607.25789)
*Tiecheng Cai,Zexian Yang,Chao Chen,Shanshan Lin,Xiangwen Liao*

Main category: cs.CV

TL;DR: 提出SEA-Cap，一种基于情感证据的多智能体系统，通过局部情感线索提取和协作验证机制，实现忠实且基于证据的情感图像描述。


<details>
  <summary>Details</summary>
Motivation: 现有情感图像描述方法难以平衡情感表达与视觉真实性，缺乏局部接地和情感验证机制，导致幻觉。

Method: 包含情感证据挖掘器提取结构化局部情感线索，并通过生成器、幻觉检查器和仲裁器的协作工作流，基于共享黑板迭代优化描述。

Result: 在两个基准数据集上有效缓解幻觉，达到最先进性能。

Conclusion: SEA-Cap通过显式审计生成内容与视觉证据的一致性，确保了情感准确性和事实一致性。
...
### [178] [GeoMFD: Continual Drone-View Geo-Localization with Geometry-Aware Adapter and Margin-Field Distillation](https://arxiv.org/abs/2607.25788)
*Zhongwei Chen,Hai-jun Rong,Tao Zhang,Xianfeng Nie,Xiangbao Zhang,Guoqi Li,Zhao-Xu Yang*

Main category: cs.CV

TL;DR: 提出了持续无人机视角地理定位（C-DVGL）问题和几何感知持续适应方法GeoMFD，包含冷启动引导、几何适配器和边界场蒸馏，仅用一个持续更新模型保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有DVGL方法在固定环境下训练，难以适应真实部署中的多样环境，且多模型会导致存储和选择成本，直接微调易造成遗忘。

Method: 三个组件：冷启动引导策略初始化稳定嵌入空间；几何适配器通过受控残差校正实现环境适应；边界场蒸馏保留正负样本相似度边界，缓解跨视角几何遗忘。

Result: 大量实验表明GeoMFD有效缓解遗忘，单个连续更新的模型性能可与专门环境模型媲美。

Conclusion: GeoMFD在平衡适应性和几何保持方面表现优异，为实时部署提供高效解决方案。
...
### [179] [FLASH: Efficient Impact Fall Detection with Unified Hypergraph State-Space Model](https://arxiv.org/abs/2607.25791)
*Tresor Y. Koffi,Youssef Mourchid,Yohan Dupuis*

Main category: cs.CV

TL;DR: 提出FLASH框架，融合超图与Mamba状态空间模型，实现高效跌倒冲击检测，兼顾精度与实时性。


<details>
  <summary>Details</summary>
Motivation: 现有基于骨架的跌倒检测方法（GNN或Transformer）难以捕捉多关节协调或实时性差，需更高效解决方案。

Method: 构建生物力学超图建模多关节协调，采用Mamba线性复杂度模型捕捉时序动态，并引入自适应反馈机制。

Result: 在UP-Fall和UMAFall数据集上达到SOTA精度，支持实时推理与零样本跨数据集泛化，计算成本显著降低。

Conclusion: FLASH验证了超图+Mamba组合在跌倒冲击检测中的有效性，兼具高性能与可解释性。
...
### [180] [Fine-Grained Food Image Understanding via Target-Aware Data Alignment](https://arxiv.org/abs/2607.25794)
*Jui-Feng Chi,Wei-Lun Chu,Bruce Coburn,Jinge Ma,Fengqing Zhu*

Main category: cs.CV

TL;DR: 提出了一种数据驱动的多模态对齐方法，通过目标感知数据选择和VLM驱动的标题优化，提升了细粒度食品理解和检索性能。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在细粒度食品理解中受限于网络数据的域差异和跨模态对齐问题。

Method: 包括目标感知数据选择、VLM驱动的标题优化、CLIP专家训练以及层次化VLM辅助多专家决策融合。

Result: VLM标题优化带来19%平均性能提升，完整方法检索得分是纯VLM的两倍以上且效率更高。

Conclusion: 数据驱动策略能显著改善细粒度食品检索性能，兼顾效果与效率。
...
### [181] [Freq-RemoteVAR: Next-Frequency Autoregressive Modeling for Remote Sensing Change Detection](https://arxiv.org/abs/2607.25815)
*Luqi Gong,Rui Xu,Yue Chen,Chao Li,Jingqi Hong,Xuefeng Zhao*

Main category: cs.CV

TL;DR: 提出Freq-RemoteVAR，一种基于频率自回归的遥感变化检测框架，通过频率域的分步生成策略替代传统一次性密集预测，显著提升复杂场景下的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有变化检测方法忽略变化模式的频率特性且采用单步稠密预测，难以应对复杂外观变化和噪声干扰。

Method: 设计频率感知掩码分词和频率VAR Transformer，配合Scale-Aligned RoPE交叉注意力与变化质量控制模块，实现频率令牌的因果自回归生成。

Result: 在CDD、GZ-CD和LEVIR-CD数据集上超过现有方法，尤其擅长复杂外观变化和噪声场景。

Conclusion: 频率域自回归生成框架有效建模变化模式的长期依赖关系，提升变化检测的鲁棒性和精度。
...
### [182] [Explicit Layer Modeling for Video Object Insertion and Layer Decomposition](https://arxiv.org/abs/2607.25802)
*Kyujin Han,Seungjoo Shin,Sunghyun Cho*

Main category: cs.CV

TL;DR: 提出TriLayer数据集和DBL-Diffusion框架，通过显式分层视频表示实现高质量视频对象插入与层分解。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑系统缺乏显式分层表示，导致合成、复用和一致性操作受限，方法依赖隐式推理或逐场景优化。

Method: 构建包含对齐复合、背景、前景视频的TriLayer数据集，提出双分支扩散框架DBL-Diffusion，通过共享去噪和跨分支交互联合建模RGB复合层与RGBA前景层。

Result: 显式层建模显著提升了插入保真度和分解质量，在对象插入和层分解任务上表现优异。

Conclusion: 显式分层数据与联合建模方法能有效提升视频编辑任务的性能，为现实合成和灵活后编辑提供支持。
...
### [183] [SepPrune:A Separator-based Pruning Framework for Efficient Multimodal Large Language Models](https://arxiv.org/abs/2607.25818)
*Yuchen Wang,Qihui Zhu,Yang Liu,Xiaoyan Sun,Siying Wu*

Main category: cs.CV

TL;DR: 提出SepPrune，一种训练免费、即插即用的视觉令牌剪枝方法，基于模态分隔符注意力高效压缩视觉令牌。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型因高分辨率输入产生大量视觉令牌，导致巨大计算开销；现有剪枝方法或无法在预填充前剪枝，或计算开销大。

Method: 利用模态分隔符令牌作为统一查询，根据其与视觉令牌的注意力分数排名，选择信息性视觉令牌进行剪枝，重复使用LLM内置投影参数，无需架构更改。

Result: 在Qwen2.5-VL-7B上，去除80.2%视觉令牌后保留96.3%原始准确率，实现最先进性能。

Conclusion: SepPrune高效且有效，无需训练或架构修改，适用于多模态大语言模型。
...
### [184] [Food Image Segmentation with LLM-Derived Ingredient Labels and Multimodal Fusion](https://arxiv.org/abs/2607.25820)
*Jui-Feng Chi,Wei-Ta Chu,Sheng-Long Lin*

Main category: cs.CV

TL;DR: 本文提出两种即插即用的多模态模块（LIM-F和LIM-Q），通过大型语言模型推断的食材标签提升食物图像分割性能，在FoodSeg103基准上达到最优结果，且计算开销增加有限。


<details>
  <summary>Details</summary>
Motivation: 现有食物分割模型在视觉相似的食材和罕见类别上表现不足，需增强细粒度识别能力。

Method: 利用LLM从食物图像推断食材标签，设计两个模块注入语义信息：LIM-F适配多层级图像编码器，LIM-Q适配Mask2Former风格的Transformer解码器，无需预对齐图文。

Result: 在Swin-L+Mask2Former框架下，LIM-Q达55.0 mIoU，LIM-F达54.4 mIoU；在CNN架构中mIoU从47.7提升至49.8，训练内存增加不超过3.8 GB。

Conclusion: 该方法提供了一种实用、可扩展的细粒度食物理解方案，以适度计算代价显著提升分割精度。
...
### [185] [Beyond Static Costs: Learning-Dynamics Aware Loss Functions for Long-Tailed Classification](https://arxiv.org/abs/2607.25830)
...
TL;DR: 提出一种学习动态感知损失函数LDAL，通过动态调整类别权重解决长尾分布问题，在多个基准数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有重加权方法依赖静态类别频率，忽略网络学习过程中的动态变化，导致长尾数据下模型性能偏斜。

Method: 基于语义特征强度、预测熵（学习难度）和跨时期预测变化构建动态权重调整机制。

Result: 在多个基准数据集上显著优于现有重加权损失函数，实现准确性与泛化性的最优平衡。

Conclusion: LDAL作为纯目标函数，计算开销低，能有效适应模型特征学习，提升长尾分布下的性能。
...
### [186] [Open-Ended CT Volume Segmentation with Weak Supervision from Language](https://arxiv.org/abs/2607.25860)
*Sanjay Subramanian,Junwei Yu,Zirui Wang,Rohil Malpani,Maggie Chung,Adam Yala,Dan Klein,Trevor Darrell*

Main category: cs.CV

TL;DR: 提出一种结合强标签（体素级）和弱标签（切片级）的CT扫描训练方法，利用报告中的切片索引提取监督信号，在SAM3模型上微调，提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: CT扫描中完整的三维标注成本高昂，而文本报告容易获取但监督信号粗糙，需要利用报告中的弱监督信号提升分割模型性能。

Method: 从大量扫描-报告对中提取切片索引描述，结合体素级强标注和切片级分类损失微调SAM3模型。

Result: 在ReXGroundingCT数据集上，Dice分数提升：1000个全标注体积时相对提升8%，250个时相对提升22%。

Conclusion: 组合粗粒度弱监督信号可有效提升CT分割性能，尤其在标注数据稀缺时提升显著。
...
### [187] [TIGA: Trajectory-Injected Generative Attack against Black-box AIGC Detectors](https://arxiv.org/abs/2607.25894)
*Xia Du,Zhuosen Bao,Zheng Lin,Jizhe Zhou,Jiawei Lian,Chi-man Pun,Jun Luo,Wei Ni,Symeon Chatzinotas*

Main category: cs.CV

TL;DR: 提出了一种无需源图像和重训练的黑盒攻击框架TIGA，通过注入注入对抗性轨迹生成扩散模型生成的逃避检测的图像。


<details>
  <summary>Details</summary>
Motivation: 现有逃避检测方法会引入可见或统计伪影，且需要访问目标检测器或重新训练扩散模型，无法在黑盒场景下有效攻击。

Method: TIGA框架：1）聚合多个白盒代理梯度形成可迁移先验；2）使用对称有限差分查询进行各向异性方向搜索；3）利用衰减动量稳定估计方向；4）按其去噪时序注入噪声，并通过频域重塑抑制高频伪影。

Result: 在黑盒攻击下，TIGA对代理和未见过的专用检测器均表现出强攻击性能、高迁移性，并在常见后处理操作下保持鲁棒性，同时生成图像质量高。

Conclusion: TIGA无需源图像或重新训练扩散模型即可实现高效黑盒攻击，兼顾生成质量与检测逃避能力。
...
### [188] [Evaluating VLMs for Autonomous Agent-Driven Geometry Clipping Detection in Video Game QA](https://arxiv.org/abs/2607.25921)
*Carlos Celemin,Benedict Wilkins,Adrián Barahona-Ríos,Saman Zadtootaghaj,Nabajeet Barman*

Main category: cs.CV

TL;DR: 本文研究了视觉语言模型在游戏质量检测中检测几何裁剪异常的能力，发现它们作为高召回率候选过滤器优于独立检测器。


<details>
  <summary>Details</summary>
Motivation: 减少游戏QA中手动标注的负担，探索VLM在受控异常检测任务中的可行性。

Method: 使用自主探索代理收集游戏观察数据，通过自动标注提供帧级裁剪标签，并评估六种VLM在零样本设置下对四种提示变体的敏感性。

Result: 所有VLM能捕捉视觉线索但产生大量假阳性，Gemini-3.1-Flash准确率最高且对提示变化最稳健，开源模型性能受提示设计影响显著。

Conclusion: 当前VLM更适合作为多阶段QA管道中的高召回率候选过滤器，而非独立缺陷检测器。
...
### [189] [Face De-Identification: A Domain-Centric Survey from Capture to Processing](https://arxiv.org/abs/2607.25926)
*Hui Wei,Hao Yu,Guoying Zhao*

Main category: cs.CV

TL;DR: 这篇综述首次统一概述了面部去识别（De-ID）的全数据采集管道，涵盖物理、传感器和数字领域，系统分析了现有方法、技术进展、评估协议，并指出了关键开放问题和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着数据隐私和负责任AI的重视，面部去识别成为活跃的研究领域，但现有文献缺乏涵盖从物理到数字域的完整视角，因此需要统一的综述来指导研究。

Method: 通过以领域为中心的视角，系统分析了物理域、传感器域和数字域的方法，并回顾了评估协议，组织相关文献、数据集和开源代码。

Result: 揭示了各域的技术进展和挑战，强调了标准化基准的必要性，并提出了未来研究方向。

Conclusion: 面部去识别领域需要跨域协作和统一基准，以推动隐私保护技术的发展。
...
### [190] [Adversarial Deepfake Generation and an Investigation of Purification-Based Adversarial Detection](https://arxiv.org/abs/2607.25842)
*Junghyun Kim,Seunghyun Kim,Jiyoung Woo*

Main category: cs.CV

TL;DR: 团队在ImageCLEF 2026深度伪造检测与生成任务中，使用FLUX.1-dev和PuLID进行身份保留的面部合成，结合多模型PGD对抗攻击实现90%的逃避率；检测任务中采用SigLIP+DINOv2和GenD-DINOv3的集成方法，在基线深度伪造上达到99.4%准确率，但真实图像假阳性率高，最终检测得分为0.6986。此外，自研了基于净化的对抗检测方法，发现EFFORT检测器在median-3净化下的$|Δ\text{logit}|$信号可有效分离对抗与干净输入。


<details>
  <summary>Details</summary>
Motivation: 应对深度伪造技术带来的安全挑战，提升生成图像的真实性和检测系统的鲁棒性。

Method: 生成任务：FLUX.1-dev+PuLID生成面部，结合DiffJPEG等12个检测器的PGD对抗攻击（MI/DI/EoT、自适应权重、两阶段热启动）；检测任务：SigLIP+DINOv2（AI图像）与GenD-DINOv3（面部操作）的最大概率集成；自研实验：比较六种CLIP ViT-L/14骨干检测器在median-3净化下的三类检测信号。

Result: 生成任务：90%逃避官方检测器、57.6%逃避参与者检测器，生成得分0.4170；检测任务：基线深度伪造准确率99.4%，但真实图像假阳性率高，最终得分0.6986；自研发现：EFFORT检测器在median-3净化下的$|Δ\text{logit}|$信号AUROC为0.81-0.98，且存在JPEG质量Q70处的信号坍缩现象。

Conclusion: 当前检测方法在基线场景中表现优异，但对真实图像的高假阳性率限制了实际应用；基于净化的对抗检测显示一定潜力但受限于图像质量。
...
### [191] [MODUS: Decoder-Only Any-to-Any Modeling of Diverse Modalities](https://arxiv.org/abs/2607.25948)
...
TL;DR: 本文提出了一种纯解码器的任意到任意多模态模型Modus，能在一个网络中处理任意模态的输入与输出，无需特定任务管道，在多种基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有任意到任意模型通常使用编码器-解码器或扩散架构从头训练，性能受限且无法利用预训练的纯解码器模型作为先验。

Method: 采用纯解码器架构，对称处理所有模态，支持任意模态作为输入和输出，无需模态专用头或损失函数。

Result: Modus在多种基准测试中表现出色，与专业模型和多任务基线相比具有竞争力，并支持链式生成和跨模态自验证等应用。

Conclusion: 纯解码器方法能有效实现任意到任意多模态建模，并且具有即用性和扩展性。
...
### [192] [Knowledge-Guided Multimodal Reasoning over Interacting Streams for Video-Level Ambivalence and Hesitancy Recognition](https://arxiv.org/abs/2607.25961)
*Podakanti Satyajith Chary,Barath Parthiban,Pranesh Velmurugan,Adeeba Khan,Nagarajan Ganapathy*

Main category: cs.CV

TL;DR: 提出PRISM-AH框架，通过多模态冲突检测和时序建模识别健康行为改变中的矛盾/犹豫状态，在公开测试集上达到0.6133的宏F1。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以从视频级别的多模态信号中识别个体差异显著的矛盾/犹豫状态。

Method: 融合冻结的视觉、音频和文本编码器，通过轻量级流模型计算跨模态不协调、预测下一窗口暴露犹豫信号、发现行为原型，并利用知识引导大语言模型进行结构化推理。

Result: 在525个视频的测试集上宏F1达0.6133，远超零样本基线0.2827。

Conclusion: 多模态冲突建模和时序预测有效提升矛盾/犹豫状态识别性能，推理增益可迁移至更大测试集。
...
### [193] [LaP-Forensics: Latent-Pixel Consistency Guided Multimodal Reasoning for Deepfake Detection](https://arxiv.org/abs/2607.25962)
*Can Wang,Yuhao Wang,Yushe Cao,Canran Xiao,Fei Shen*

Main category: cs.CV

TL;DR: 提出了LaP-Forensics多模态框架，利用稳定扩散DDIM反转重建的残差图增强RGB语义，进行图像伪造检测和定位。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型产生无明显视觉伪影的图像，削弱了依赖表面外观的检测器。

Method: 结合冻结的DDIM反转重建模型获取残差图，通过独立投影器编码RGB和残差，使用结构化Where-What-Why模型输出文本分析和伪影掩码，并采用GRPO优化。

Result: 在UniversalFakeDetect和SynthScars基准上展示了跨生成器检测和竞争性定位性能，消融实验支持残差流的有效性。
...
### [194] [Quasi-SVD: Learning a Lie-constrained matrix factorisation for real-time imaging](https://arxiv.org/abs/2607.25967)
*Christopher Hahne*

Main category: cs.CV

TL;DR: 提出可微全并行矩阵分解框架Quasi-SVD，在GPU上实现3-20倍加速，支持医疗成像中实时处理（>25 FPS）。


<details>
  <summary>Details</summary>
Motivation: 传统SVD算法顺序执行，限制GPU吞吐量，无法满足临床实时成像需求。

Method: 非对称设计：一个因子通过李参数化保证精确正交，另一因子用软约束恢复；无需迭代奇异向量优化。

Result: SSIM=0.89-0.94，加速3-20倍（vs cuSOLVER和随机SVD），在超声定位显微镜和穆勒矩阵偏振测量中均>25 FPS。

Conclusion: Quasi-SVD通过优先保真重建而非精确谱分解，使结构化矩阵分解适用于实时图像引导工作流。
...
### [195] [Schrödinger's Cat: Probabilistic Representation and Prediction of Potential Scene Kinematics](https://arxiv.org/abs/2607.25984)
*Timy Phan,Jannik Wiese,Björn Ommer*

Main category: cs.CV

TL;DR: 提出GARFIELD模型，通过结构化的时空潜变量表示场景运动分布，支持高效轨迹采样与概率密度估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时兼顾多模态未来预测的分布建模与高效采样，且视频生成模型计算成本高。

Method: 学习场景运动的概率潜变量表示，包含稠密编码器、稀疏约束融合模块和确定性密度解码器。

Result: 轨迹采样速度比大型视频生成模型快97倍，密度估计比蒙特卡洛采样快两个数量级。

Conclusion: GARFIELD能实现可交互的、不确定性感知的运动预测与规划。
...
### [196] [On the Use of Synthetic Data for Threshold Calibration in Face Recognition: Performance and Security Implications for Border Control Systems](https://arxiv.org/abs/2607.25990)
*Arto Apila*

Main category: cs.CV

TL;DR: 研究了合成人脸数据在欧盟出入境系统阈值校准中的应用，发现在低FMR条件下合成数据无法可靠替代真实数据。


<details>
  <summary>Details</summary>
Motivation: EES系统需极低错误匹配率，但成员国的阈值校准缺乏指导，且获取真实数据受法律和隐私限制。

Method: 比较合成与真实数据集在证件-现场验证中的分数分布对齐性，评估阈值跨域迁移性。

Result: 合成数据在受控场景可近似校准，但在非约束条件下因分数分布尾部不匹配导致性能下降和抗攻击能力减弱。

Conclusion: 合成数据可用于开发，但高安全场景的阈值选择需基于真实数据进行验证和调整。
...
### [197] [Beyond Zooming: Learning Multi-Tool Visual Reasoning for Ultra-High-Resolution Remote Sensing](https://arxiv.org/abs/2607.25993)
...
TL;DR: 本文提出GeoMTVR数据集和GeoLens模型，通过多工具视觉推理和工具注意力强化学习，解决超高分遥感图像中稀疏、局部证据的推理难题。


<details>
  <summary>Details</summary>
Motivation: 超高分遥感图像中任务相关证据稀疏且分散，单工具缩放方法在复杂任务（如全局搜索、多区域比较）中效果饱和。

Method: 构建GeoMTVR数据集（含13K样本、交错推理轨迹和多工具调用），提出工具注意力强化学习算法优化工具使用决策。

Result: GeoLens在准确率、证据定位和工具使用效率上均优于直接推理和单工具缩放基线。

Conclusion: 多工具推理结合强化学习能有效提升超高分遥感场景下的视觉推理能力。
...
### [198] [Parallel Decoding Distillation for Fast Image and Video Generation](https://arxiv.org/abs/2607.26004)
*Neta Shaul,Chao Liu,Arash Vahdat,Julius Berner*

Main category: cs.CV

TL;DR: 提出了并行解码蒸馏(PDD)，一种简化且可扩展的基于轨迹的蒸馏方法，用于加速视频扩散/流模型的推理。


<details>
  <summary>Details</summary>
Motivation: 现有加速方法依赖变分评分蒸馏和对抗损失，难以优化且存在模式崩塌问题，导致视频多样性损失和缺乏运动。

Method: PDD通过每网络评估预测多个去噪步骤来加速生成，学习平均速度表示而不回归其导数。

Result: 在LTX-2.3、Wan 14B和Qwen-Image等模型上，4-8步评估即达到SOTA性能，并显著提升视频多样性。

Conclusion: PDD是一种高效、兼容预训练模型的加速方法，支持可变步数采样，在速度和多样性上优于现有技术。
...
### [199] [Wonder: Video World Model Done Better](https://arxiv.org/abs/2607.26037)
*Jiacong Xu,Hanwen Jiang,Zhixin Shu,Kalyan Sunkavalli,Vishal M. Patel,Yiqun Mei*

Main category: cs.CV

TL;DR: 提出Wonder通用视频世界模型，支持实时相机可控的长期场景探索与交互。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型缺乏实时、长时程的相机交互能力，无法像世界模型一样支持自由探索。

Method: 系统级协同设计：密集坐标场相机条件化；稀疏注意力记忆机制；自纠正蒸馏训练策略。

Result: 以16FPS合成分钟级视频，保持几何、外观与动态一致性；支持视频条件续拍与实时重拍摄。

Conclusion: 通过控制、记忆与训练三方面创新，首次实现可交互的通用视频世界模型。
...
### [200] [VetClaw: An Edge-Cloud Multimodal Agentic System for Veterinary Disease Screening](https://arxiv.org/abs/2607.26042)
*Syed Mhamudul Hasan,Anas AlSobeh,Hussein Zangoti,Abdur R. Shahid*

Main category: cs.CV

TL;DR: VetClaw是一个用于早期兽医疾病筛查的边缘-云多模态代理系统，通过分离代理交互与工作流编排，将静态图像分类转化为协调、安全感知的诊断支持系统。


<details>
  <summary>Details</summary>
Motivation: 现有静态图像分类模型在疾病筛查中缺乏工具使用、工作流管理和安全处理能力，需要一种更协调的系统化方法。

Method: 系统由边缘设备（OpenClaw）和服务器（LangGraph）组成：OpenClaw负责调度、工具访问和用户交互；LangGraph管理状态化筛查工作流，处理输入验证、图像传输、模型调用、安全检查、条件路由和故障处理。

Result: 纯图像VLM预测性能有限，而症状引导和多模态输入显著提升了零样本分类性能。

Conclusion: VetClaw将静态预测模型转变为可协调、安全感知的系统，能够使用工具、管理工作流、处理故障并升级不确定病例。
...
### [201] [TimeCapsule: Generative Hallucination as a Method for Historical Sensemaking](https://arxiv.org/abs/2607.24750)
*Hayk Grigorian,Hamed Yaghoobian*

Main category: cs.CL

TL;DR: TimeCapsule是一个基于维多利亚时期文本训练的1.2B参数因果语言模型，实现了45.4%的困惑度降低，并生成历史情境化解释。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs因训练于现代语料，对过去时代描述不可靠；需要时间隔离的模型以探索历史认知。

Method: 使用1800-1875年维多利亚文本训练1.2B参数LLaMA式因果模型，通过定量困惑度评估和定性人文学者评估。

Result: 在维多利亚留出文本上困惑度降低45.4%；人文学者将约40%真实维多利亚文本误判为机器生成。

Conclusion: 对未来的结构无知使模型幻觉成为探索19世纪本体论的解读工具。
...
### [202] [Neuromorphic Diffusion Language Models: Addressing Compute and Memory Bottlenecks via Sparsity and Block Denoising](https://arxiv.org/abs/2607.24841)
*Dengyu Wu,Clement Ruah,Jiechen Chen,Bipin Rajendran,Osvaldo Simeone*

Main category: cs.CL

TL;DR: 提出神经形态掩码扩散语言模型(N-MDLMs)，通过结合块扩散和脉冲神经形态计算，同时提升吞吐量和能效。


<details>
  <summary>Details</summary>
Motivation: 自回归LLM推理效率低，掩码扩散模型虽可通过一次参数访问生成多个token，但在现代大内存芯片上仍不够高效。

Method: 整合块扩散（多token并行生成）与脉冲诱导稀疏性（跳过非活跃通道减少参数访问和计算），并建立token级类罗夫林模型分析协同效应。

Result: 在翻译任务中，N-MDLMs在计算受限平台上比MDLMs和自回归模型有更优的能效和吞吐量提升。

Conclusion: 脉冲稀疏性使N-MDLMs在多种平台上优于现有方法，验证了稀疏性-扩散协同的有效性。
...
### [203] [Measuring and Improving Behavioral Consistency in Large Language Models through Fact-Heuristic-Emotion State Enforcement](https://arxiv.org/abs/2607.24765)
*Gi-Hun Lee,Joong Yull Park*

Main category: cs.CL

TL;DR: 提出了一种名为认知内核模型（CKM）的方法，通过在提示层面强制分离事实、启发和情感三种认知角色，在不改变模型权重的情况下，显著降低了大型语言模型决策的不稳定性和翻转率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对同一决策问题在不同运行中可能给出不同答案，甚至在前次回答作为上下文时反转决策，这种不稳定性需要被量化和减少。

Method: CKM是一种提示级别的状态强制层，要求模型在决策前将输入分为事实（给定或可验证）、启发（推断或假设）和情感（评价或优先级信号）三类，并维护结构化状态$S_t = {F_t, H_t, E_t}$，通过转移函数更新。

Result: 在韩国语言决策场景中测试26个语言模型（37,403个观察值），CKM将重复输出变异性降低了随机效应Hedges' g=1.09（95% CI [0.83, 1.35]），在较新模型中决策翻转率降低82%（g=1.52），且效果部分归因于结构化支架（45%）和事实/启发/情感内容（55%）。

Conclusion: CKM不改善推理正确性，但能通过强制分离事实、假设和评估信号，部分提高行为一致性，且该可测量变异性在不同模型间存在差异。
...
### [204] [Research Report on Noise-Shaped One-Bit Coefficients in Discrete Polynomial Fourier Extension](https://arxiv.org/abs/2607.24868)
*Shengquan Wang*

Main category: cs.CL

TL;DR: 本文研究了归一化离散多项式傅里叶扩展中噪声成形的一位系数，推导了Sigma-Delta量化误差的逼近率，并建立了高阶有限记录恒等式和多种理论估计。


<details>
  <summary>Details</summary>
Motivation: 探索噪声成形一位量化在离散多项式傅里叶扩展中的逼近性能，并严格分析其误差衰减率。

Method: 利用第一阶Sigma-Delta量化的误差差分形式，通过离散分部求和、变分估计和振荡传递界进行分析；采用高阶有限记录恒等式和边界校正方法。

Result: 获得紧参数集上的$O(N^{-1})$逼近率，证明其最优性；高阶噪声成形在光滑权重下达到$O(N^{-r})$衰减，在Hölder连续权重下达到$O(N^{-(r-1+α)})$衰减；并给出了精确正交恒等式和矩公式。

Conclusion: 噪声成形量化在离散多项式傅里叶扩展中能实现高效逼近，且误差界具有最优性，方法可推广至多项式相位、多维参数和完全相关状态模型。
...
### [205] [CogArena: A Multimethod Evaluation of Cognitive Ability Structure in Large Language Models](https://arxiv.org/abs/2607.24999)
*Dengzhe Hou,Lingyu Jiang,Fangzhou Lin,Kazunori D Yamada*

Main category: cs.CL

TL;DR: 本文介绍CogArena基准，通过多方法框架评估LLM认知分数的维度有效性，发现当前证据不支持稳定的五维能力剖面。


<details>
  <summary>Details</summary>
Motivation: 现有研究将LLM认知得分归纳为能力剖面，但需要验证这些维度是否在任务间收敛、对匹配干预敏感并超越定义模型泛化。

Method: 构建包含13个范式的CogArena基准，基于五组理论动机分组，采用相关性分析、干预实验和跨族泛化检验等多方法框架。

Result: 55个开放权重模型中几乎所有范式正相关，共同轴解释约一半方差；分组优势小且对评分敏感；干预未显示特异性对比，跨家族预测未改善。

Conclusion: 理论对齐提示仅产生微弱对角线倾向，现有证据不足以确立稳定五维剖面，CogArena为标注认知标签前提供了行为、协方差、匹配干预和跨族预测的工作流。
...
### [206] [Evaluating Communicative Belief Updates in Large Language Models via Implicature Recognition and Cancellation](https://arxiv.org/abs/2607.25094)
*Cesare Spinoso-Di Piano,Verna Dankers,Marius Mosbach,Jackie Chi Kit Cheung*

Main category: cs.CL

TL;DR: 评估大型语言模型识别隐含信念及信念更新的能力，发现其表现不如人类，特别是在自然场景中。


<details>
  <summary>Details</summary>
Motivation: 人类语言依赖未言明的信念和信念更新，这对LLMs与用户成功沟通至关重要。

Method: 创建首个专家标注的隐含取消数据集，通过众包收集人类判断，并进行控制实验。

Result: LLMs在信念更新理解上落后于人类，成功可能部分依赖先验信念，失败则与类型和形式相关。

Conclusion: 当前LLMs未能达到人类水平的未言明信念和信念更新理解。
...
### [207] [DS@GT ARC at CheckThat! 2026: LLM-Based Trace Ranking and Grouped Reward Modeling for Multilingual Numerical Claim Verification](https://arxiv.org/abs/2607.25069)
*Sagnik Sinha,Shreyas Shrestha*

Main category: cs.CL

TL;DR: 本文针对数值声明验证问题，探索了基于LLM的微调验证器和轻量级TF-IDF奖励模型两种方法，并在CLEF 2026 CheckThat!任务2的英语和阿拉伯语数据上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 数值声明的自动验证需要语言理解和定量推理，这是一个具有挑战性的问题，现有方法在处理复杂数值声明时存在不足。

Method: 方法一：使用LoRA微调LLM验证器，独立评分每个推理轨迹（二分类问题），并通过Best-of-N选择最终判决，同时尝试自适应子声明分解。方法二：使用TF-IDF奖励模型结合手工设计的数值和时间重叠特征评分轨迹，并按判决组聚合分数确定最终预测。对于阿拉伯语，比较了通用多语言模型和AraBERT。

Result: LLM方法在多数指标上优于轻量级奖励模型，尤其是Recall@5；奖励模型在Conflicting类别上表现更强。子声明分解未提升性能，反而引入噪声。对于阿拉伯语，AraBERT在多数指标上优于多语言基线。

Conclusion: LLM验证器在数值声明推理中更有效，而奖励模型在特定类别上有优势；子声明分解策略需谨慎使用；语言特定预训练模型（如AraBERT）对阿拉伯语任务更有优势。
...
### [208] [Deep Label-Wise Attentive Temporal Convolutional Networks Improve Medical Coding](https://arxiv.org/abs/2607.25129)
*Muhammed Yavuz Nuzumlalı,Alexander Fabbri,Irene Li,Dragomir Radev*

Main category: cs.CL

TL;DR: 该论文提出一种结合多层时间卷积网络（TCN）与标签注意力机制的深度神经网络，用于医学编码任务，将诊断和程序代码的分配建模为多标签文本分类问题。


<details>
  <summary>Details</summary>
Motivation: 医学编码需要从病历笔记的不同部分聚合信息并针对每个代码关注不同段落，对专业编码员也具有挑战性，现有方法难以处理长序列和标签特异性信息。

Method: 采用多层时间卷积网络（TCN）提取全局文档表示以学习长序列关系，并引入标签特定注意力机制让模型为每个标签聚焦文档的不同方面。

Result: 相比先前最先进模型，F-1分数提高9%，召回率显著提升28%，其中召回率被认为在临床决策支持场景中更重要。

Conclusion: 提出的方法在医学编码任务上优于现有模型，尤其在召回率方面表现突出，适合临床辅助决策应用。
...
### [209] [TabRank: Chain-of-Thought Distillation for Table Re-Rankers](https://arxiv.org/abs/2607.25182)
*Adarsh Singh,Kushal Raj Bhandari,Jianxi Gao,Soham Dan,Vivek Gupta*

Main category: cs.CL

TL;DR: 提出TabRank框架，通过训练小型推理模型进行表格检索重排序，在多项数据集上显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有神经重排序器在表格检索中缺乏推理能力，而大型推理模型虽在文本检索中表现优异，但直接应用于表格检索存在计算和泛化挑战。

Method: 构建包含6728条推理轨迹的数据集，探索两种训练策略：显式思维链蒸馏和基于教师推理轨迹的条件提示训练。

Result: 在多个数据集上Acc@10提升显著：HybridQA 30.5%、SQA 15.2%、TabFact 52.9%、TATQA 13.1%，且能泛化到多表场景。

Conclusion: TabRank通过引入推理能力有效提升了表格检索重排序的准确性和泛化性。
...
TL;DR: 人类语言中词汇排列的上下文持久性服从幂律分布，指数约为1。


<details>
  <summary>Details</summary>
Motivation: 探索词汇顺序这一语言意义核心要素是否遵循类似词汇频次和共现的统计规律。

Method: 使用大型语言模型作为概率探针，计算不同距离下上下文对目标词困惑度的降低程度，并与打乱顺序的基线比较。

Result: 在10个跨6个语系及不同模态的语料库中，上下文持久性函数P(d)近似为1/d衰减（平均指数α=1.04，中位r²=0.96），且该效应在打乱顺序和合成对照中消失。

Conclusion: 人类语言存在上下文持久性的标度律，其指数接近1使得上下文影响在对数时间尺度上均匀分布。
...
TL;DR: 提出了MyoCardBench基准，包含2263项心血管临床任务，评估7个LLM在真实诊疗场景中的表现。GPT-5.4综合得分最高（62.55），但ECG解读和伦理判断得分较低（17左右），揭示了模型在临床安全关键任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有LLM医学基准多聚焦考试知识或单一任务，无法反映心血管诊疗的多模态、纵向和安全关键特性。

Method: 构建包含13个任务子集（2263项）的基准，由16位心内科医生标注并交叉审核。7个LLM在零样本设置下生成15841个输出，采用关键点覆盖率和临床整体质量评估开放式任务。

Result: GPT-5.4最优（62.55），但ECG解读（17.25）和伦理判断（17.34）得分最低。临床沟通、急救和治疗规划任务中整体质量与关键点覆盖率差距最大（>48）。

Conclusion: MyoCardBench是目前最大、覆盖最广的心血管临床LLM基准，能有效识别模型优势与关键临床短板，为优先发展提供框架。
...
### [212] [A Cross-lingual Comparison of Human and Classification Model Entrainment Behavior in Code-switched Speech Settings](https://arxiv.org/abs/2607.25202)
*Debasmita Bhattacharya,Siying Ding,Alayna Nguyen,Julia Hirschberg*

Main category: cs.CL

TL;DR: 基于人机对话中语码转换的语音特点，分析不同语言对在词汇、声学韵律和CSW风格上的共振模式，并评估分类模型对人类共振行为的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 书面和单语环境下对话共振研究较多，但口语语码转换（CSW）中跨语言共振尚不明确。

Method: 对普通话-英语、印地语-英语、西班牙语-英语对话进行跨语言共振分析，并使用特征重要性和消融实验评估经典分类器与Transformer模型。

Result: 词汇共振跨语言通用，但声学韵律和CSW风格共振存在语言特定差异；模型能较好检测共振，但偏重非人类显著特征。

Conclusion: 提出人类导向的模型评估框架，为开发自然语码转换对话系统指明挑战。
...
### [213] [Interpretable Column Annotation with LLM-Symbolized Decision Process Materialization](https://arxiv.org/abs/2607.25228)
*Mengqi Wang,Jianwei Wang,Qing Liu,Xiwei Xu,Zhenchang Xing,Michael Bain,Liming Zhu,Wenjie Zhang*

Main category: cs.CL

TL;DR: 提出SymCA，一个基于大语言模型的可解释列标注框架，通过全局到局部的符号决策过程实现列类型和列属性标注。


<details>
  <summary>Details</summary>
Motivation: 现有CA方法依赖神经网络直接映射列表示到标签类别，缺乏可解释性和适应性，且未充分利用标签语义，限制准确性。

Method: SymCA包含全局骨架归纳（利用LLM生成超义词树结构骨架，通过MBR策略选择稳健骨架）和局部基质演化（每个内部节点训练可解释随机森林分类器，LLM提出节点特定操作修改，使用探索-利用策略优化）。

Result: 在Micro-F1和Macro-F1上分别平均超越最强基线6.42%和11.03%。

Conclusion: SymCA在准确性、鲁棒性和可解释性上表现优异，验证了符号决策过程在列标注中的有效性。
...
### [214] [VisualPatchWorld: Code World Models as Latent Structured Representations for Planning](https://arxiv.org/abs/2607.25236)
*Jiaxin Bai,Jiaxuan Xiong*

Main category: cs.CL

TL;DR: 提出VisualPatchWorld (VPW)，将世界动力学表示为可编程代码，结合主动探测选择定性动态形式并拟合参数，在规划任务中超越现有代码基模型23.5个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有神经预测器隐含动态但不可解释，物理引擎可编辑但难以大规模构建，需一种兼具可解释性与可扩展性的世界模型构建方法。

Method: 先用短活跃探测选择定性动态形式，再从状态-动作轨迹中通过最小化多步预测误差拟合自由参数，生成可执行程序用于模拟和规划。

Result: VPW在规划任务中平均成功率达69.0%，超过最强代码基线23.5点；在导航和抓取控制中接近真实引擎性能，接触推动任务中结合引擎检查可缩小差距。

Conclusion: 证明了自动构建可用于规划的代码世界模型的实用路径，该模型可检查、可编辑且规划有效。
...
### [215] [Where Steering Signals Come From: Activation Source Selection in Activation Steering](https://arxiv.org/abs/2607.25270)
*Jiaran Ye,Lingxu Ran,Zijun Yao,Chenpeng Wang,Yong Jiang,Lei Hou,Juanzi Li,Liangming Pan*

Main category: cs.CL

TL;DR: 研究了激活引导中上游信号源选择的影响，发现执行边界状态比目标行为出现更重要，并提出了尾部减法方法以改进引导。


<details>
  <summary>Details</summary>
Motivation: 现有工作常将激活引导的上游信号源视为次要细节，但作者假设源选择（包括源上下文和激活读取策略）对引导效果有实质影响。

Method: 通过跨三个指令微调模型和四个任务族比较不同源激活（如答案源、执行边界状态），引入尾部减法（去除共享语义得到更纯净信号）。

Result: 改变源激活会显著改变引导效果；有效引导的关键不是目标行为是否出现在源文本中，而是模型即将执行该行为时的状态（执行边界）。

Conclusion: 引导信号依赖于模型即将执行的行为的表征，而非已有内容，尾部减法提供了更稳定有效的信号。
...
### [216] [CoSA: Accelerating Long-Context Inference via Proxy-Kernel Co-Designed Sparse Attention](https://arxiv.org/abs/2607.25291)
*Yufei Xue,Lin Niu,Hong Liu,Siran Liu,Hanyong Shao,Wei Liu,Guanghua Yu,Jianchen Zhu,Jun Zhang*

Main category: cs.CL

TL;DR: 提出CoSA，一种无训练的两阶段稀疏注意力方法，通过代理-内核联合设计解决了长上下文推理中二次复杂度和预算紧张导致精度下降的问题。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理中自注意力的二次复杂度带来高昂成本，现有基于代理的块稀疏注意力在预算紧张时因代理遗漏显著块和内核机械应用掩码导致精度下降。

Method: 两阶段协同设计：第一阶段KAP在中等预算下生成有序掩码，指定KV页面访问顺序；第二阶段OSK在严格预算下利用在线softmax统计跳过更多块。

Result: 在主流LLM和长上下文基准中，CoSA在更低预算下实现更高精度，在128K上下文长度下注意力加速4.93倍，端到端首Token时间减少2.53倍，性能下降可忽略。

Conclusion: 通过代理与内核的联合优化，CoSA在保持模型精度的同时显著提升长上下文推理效率。
...
### [217] [Toward a systematic method for identifying language areas](https://arxiv.org/abs/2607.25305)
*Hiram Ring*

Main category: cs.CL

TL;DR: 提出一种基于地理聚类的系统方法，用于识别任意大小的语言区域，以替代传统依赖专家判断的宏观区域划分。


<details>
  <summary>Details</summary>
Motivation: 现有宏观区域主要依赖专家对潜在接触区域的主观判断，缺乏系统性的可重复方法。

Method: 采用简单的地理聚类方法，在全球和特定区域（如语言联盟）生成分组。

Result: 方法生成的全球宏观区域与现有分组高度一致，并能识别局部语言联盟。

Conclusion: 该方法为语言类型学中控制接触和继承的影响提供了可量化的替代方案。
...
### [218] [CAST: Game Solvers as Turn-Level Teachers for LLM Agents](https://arxiv.org/abs/2607.25308)
*Yu Wang,Yi-Kai Zhang,Wentao Shi,Ziang Ye,Yuchun Miao,Yueqing Sun,Qi Gu,Xunliang Cai,Lan-Zhe Guo,Han-Jia Ye,Fuli Feng*

Main category: cs.CL

TL;DR: 提出 CAST 方法，利用游戏求解器的状态值变化作为逐回合信用分配信号，提升大语言模型在长周期游戏中的强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习仅依赖稀疏的最终奖励，无法区分关键决策，而现有的过程信号难以兼顾廉价和准确。

Method: 将游戏求解器状态值的变化转换为求解器优势，将其注入RLVR作为逐回合信号，并在软最优求解器假设下等价于在线策略蒸馏。

Result: 在 Sokoban、Minesweeper、Rush Hour 上超越所有基线，且在 ALFWorld 和 WebShop 上取得最高的平均零样本性能。

Conclusion: 基于求解器状态值变化的过程信号能有效改进强化学习的信用分配，提升模型在复杂决策任务中的表现。
...
### [219] [Every Time I Hire a Linguist, Inference Costs Go Down: On Linguistic Rules as Effective Prompt Compressors](https://arxiv.org/abs/2607.25335)
*Jianfei Ma,Zhaoxin Feng,Emmanuele Chersoni,Si Chen*

Main category: cs.CL

TL;DR: 本研究探索仅使用语言规则进行提示压缩，无需基于语言模型的评分，并通过进化搜索找到有效的规则组合，在多种数据集上取得了与先进压缩策略相似的性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示压缩方法依赖语言模型的前向传播来计算词元重要性，成本高昂，而语言学研究早已提出可通过规则化的线索识别信息性内容，因此研究能否仅用语言规则实现高效压缩。

Method: 通过离线进化搜索，在词汇、句法、语义和语篇四个层面生成种子规则，寻找竞争性的规则组合；部署时无需语言模型前向传播，仅需CPU处理，并使用双路径协议平衡压缩质量与重建保真度。

Result: 在短文本、多文档推理和对话记忆问答数据集上，进化后的压缩器性能与先进方法相似，在轻度到中度压缩下表现最佳，随着压缩率增大性能下降；进化分析显示有效压缩需融合多层面信号，且从词元修剪转向句子提取。

Conclusion: 仅凭语言规则可以实现有效的提示压缩，尤其在低到中等压缩比下，为低成本的推理加速提供了新方案。
...
### [220] [Temporal-Distance JEPA: Plan-Aware Representation Learning for Latent World Model Predictive Control](https://arxiv.org/abs/2607.25337)
*Jiaxin Bai,Jiaxuan Xiong*

Main category: cs.CL

TL;DR: 提出TD-JEPA方法，通过挖掘时序距离代价和联合优化表征与规划，弥合了JEPA世界模型在训练与规划之间的差距，在多个机器人规划任务上超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA规划器通常使用潜空间欧几里得距离作为规划代价，这种代价是表征学习的副产品而非从示范日志中挖掘出的进度成本，导致训练与规划之间存在差距。

Method: 保留LeWM的编码器-预测器架构，通过同轨迹步序作为正样本、跨轨迹对作为启发式负样本、以及规划视界一致性项，从无奖励轨迹中挖掘有向时序代价；该代价既可直接用作规划成本，也可作为优化表征学习的信号。

Result: 在Two-Room任务上达到100.0%成功率（LeWM为97.4%），OGB-Cube提升14.2个点，Push-T有所改进；在所有环境下匹配或超越LeWM和RC-aux基线。

Conclusion: TD-JEPA通过从离线日志中发现时序进度结构并协同设计代价形式与规划部署，有效缩小了JEPA世界模型在训练与规划之间的差距，是一种更优的规划方法。
...
### [221] [Inspect India Evals: An Open Benchmarking Framework for Evaluating Large Language Models in the Indian Linguistic and Cultural Context](https://arxiv.org/abs/2607.25375)
*Abhishek Kumar Singh,Shrey Nag,Sachita,Lipi Goel,Rajeshwar Singh Janwar*

Main category: cs.CL

TL;DR: Inspect India Evals是一个开源框架，用于评估LLM在印度语境下的安全性、公平性和准确性，包括六个基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试（如MMLU、BIG-Bench）偏向英语和西方文化，无法检测LLM在印度特有的安全、公平和准确性失败。

Method: 基于UK AISI的Inspect AI平台，构建六个基准：多语言MMLU、印度社会偏见测试（BharatBBQ）、数字公共基础设施安全评估、多语言安全测试、多轮越狱抵抗测试和印度文化知识基准。测试了5个8B-32B参数的开放权重模型。

Result: Sarvam-M 24B和Gemma 2 27B在印度公平指数上均获80%；Sarvam-M在印度文化和DPI安全上超越32B模型；所有模型在多语言安全上100%拒绝，DPI安全范围20%-100%。

Conclusion: 该框架公开且可扩展，能有效填补印度语境下的评估空白，不同模型在印度特定任务上表现差异显著。
...
### [222] [Memory for Large Language Models](https://arxiv.org/abs/2607.25380)
*Sining Zhoubian,Dan Zhang,Evgeny Kharlamov,Jie Tang*

Main category: cs.CL

TL;DR: 本文系统梳理了大语言模型中记忆机制的发展，提出基于表示、更新动态和持久性三个正交轴的分类法，并分析了混合架构与评估方法。


<details>
  <summary>Details</summary>
Motivation: 记忆机制研究碎片化，缺乏统一框架来理解不同方法（如注意力、状态动力学、参数高效存储）间的联系与差异。

Method: 通过架构中心分类法，沿表示（隐式vs显式）、更新动态（离线vs在线）、持久性（短期vs长期）三个轴定义记忆，并形式化读写、路由、状态转换与整合的机制。

Result: 建立了计算耦合与独立可寻址记忆间的概念界限，梳理了混合架构、效率权衡及多维度评估方法。

Conclusion: 该分类法为记忆中心的大语言模型设计提供了原则性基础，指导未来可扩展与自适应语言建模的发展方向。
...
### [223] [Phase Structure in Rotary Attention: A Spectral Framework for Semantic Continuity and Execution-Boundary Governance](https://arxiv.org/abs/2607.25507)
*Abraham Chachamovits*

Main category: cs.CL

TL;DR: 本文提出了一个带界谱分析框架，用于研究Transformer语言模型中旋转相位对齐、隐藏状态连续性和语义漂移，而不将其视为物理波动系统。


<details>
  <summary>Details</summary>
Motivation: 现有分析多依赖向量几何，但有序上下文和旋转位置编码引入了明确的相位结构，需要新的视角来理解其影响。

Method: 将有序隐藏状态序列作为谱分解域；推导RoPE注意力得分为余弦项加权和；证明局部稳定性引理；定义复模态坐标和加权相干性泛函。

Result: 证明了均匀有界相位位移限制预softmax得分退化；区分了表示连续性与执行边界可接受性。
...
### [224] [IRIS: Reusable Identity Representations from Frozen LLMs for Entity Alignment](https://arxiv.org/abs/2607.25579)
*Xinran Liu,Shengtao Li,Shouqian Shi,Ge Wang,Xin-Wei Yao*

Main category: cs.CL

TL;DR: 提出了IRIS框架，利用冻结大语言模型为知识图谱实体生成类似虹膜的稳定身份签名，实现无需训练的跨图谱实体对齐。


<details>
  <summary>Details</summary>
Motivation: 现有实体对齐方法依赖显式图结构和文本字段，对异构描述理解不足；LLM方法未能将实体理解转化为可直接比较的稳定身份空间。

Method: 从冻结LLM的内部状态提取面向身份的上下文表示，为每个实体构造独特的虹膜签名，在共享空间中通过相似度比较实现对齐。

Result: 在四个基准数据集上，最佳IRIS变体达到Hits@1 100.00、99.38、98.31和97.99。

Conclusion: IRIS无需训练且不依赖特定KG对或候选集，通过一次编码即可实现跨图谱直接对齐，显著优于现有方法。
...
### [225] [Evaluation of forced alignment of code-mixed speech: the case of Hindi-English](https://arxiv.org/abs/2607.25581)
*Ayushi Pandey,Pamir Gogoi,Kevin Tang*

Main category: cs.CL

TL;DR: 本文评估了蒙特利尔强制对齐器对印地语-英语混合语音的对齐效果。通过精心设计的词典和混合语音训练数据，实现了平均误差4.15ms，比单语基线低10倍。


<details>
  <summary>Details</summary>
Motivation: 混合语音（如印地语-英语）因词汇表扩大、拼写错误和说话人变异，给强制对齐带来挑战。

Method: 使用蒙特利尔强制对齐器，对比未修改词典和引导策略，并训练基于句级混合语音的声学模型。

Result: 引导策略显著优于未修改词典；基于混合语音训练的模型平均误差4.15ms，远低于单语印地语（38.18ms）和孤立英语（37.58ms）。

Conclusion: 原则性词典设计和混合语音训练数据对于可靠对齐双语语音至关重要。
...
### [226] [PILA: Plug-and-Play Insertion for LLM-native Advertising](https://arxiv.org/abs/2607.25590)
*Zhaowei Zhang,Yuhan Fu,Yihang Zhang,Xiaohan Liu,Ceyao Zhang,Xiaoyuan Zhang,Yipeng Kang,Tonghan Wang,Yaodong Yang*

Main category: cs.CL

TL;DR: 提出PILA，将广告插入解耦为轻量级旁路模块，以条件重写实现LLM原生广告，无需修改基础模型或工作流。


<details>
  <summary>Details</summary>
Motivation: 现有方法将广告与内容生成耦合在一个模型中，不兼容API或工作流式LLM应用，且损害响应质量。

Method: 将广告插入重构为条件响应重写问题，设计模型无关的轻量级sidecar模块，解耦广告与内容生成。

Result: 实验表明，PILA在多种上游模型上一致提升广告效果，同时保持响应质量。

Conclusion: PILA作为实用的LLM原生广告方案，实现了自然度与曝光度的可控权衡，便于下游定价和部署。
...
### [227] [A Human-in-the-Loop Corpus for LLM-Based Simplification of Scientific Summaries](https://arxiv.org/abs/2607.25630)
*Kyuri Im,Michael Färber*

Main category: cs.CL

TL;DR: 本文提出了一个基于大语言模型的人机协同流程，用于将专家摘要简化为非专业人士更易理解的版本，并发布了相关语料库和评估结果。


<details>
  <summary>Details</summary>
Motivation: 跨学科研究加速，但科学论文对领域外读者难以理解，需简化以促进交流。

Method: 以SciSummNet为源语料，先用GPT-4o-mini生成简化摘要；阶段1由STEM领域读者评估，阶段2由计算机科学专家根据反馈创建专家编辑版本。

Result: 阶段1显示GPT生成摘要更易懂和简洁；阶段2编辑强调保留专业术语和科学主张强度的重要性。

Conclusion: 该资源可用于训练和评估跨学科科学交流的简化系统。
...
### [228] [Construction-Driven Injection: Linguistically-Grounded Edit-Based Code-Mixing Fingerprints for Large Language Models](https://arxiv.org/abs/2607.25633)
*Yongyi Cui,Yue Li,Tianbao Jiang,Xin Yi*

Main category: cs.CL

TL;DR: 本文提出LCF，一个统一优化指纹构建与注入的框架，通过代码混合指纹和零空间投影注入，实现低困惑度、抗检测且无害的LLM所有权验证。


<details>
  <summary>Details</summary>
Motivation: 现有指纹方法将构建与注入解耦，导致自然语言指纹易误激活、乱码指纹易被困惑度检测过滤，且注入缺乏对触发结构感知的优化。

Method: LCF框架包含LCF构建（基于语义密度替换和语法混合的低资源语言代码混合指纹）和LCFEdit注入（利用多语言表示零空间投影保留知识，并跨语言对齐将更新导向指纹语言子空间）。

Result: 在不可察觉性、可检测性和无害性上全面评估，验证了持续所有权验证且对模型效用影响可忽略。

Conclusion: 指纹构建应驱动注入，联合优化可提升指纹的隐蔽性和鲁棒性，实现更可靠的模型所有权保护。
...
### [229] [MyMentorLLM: A psychotherapy GenAI environment with multimodal voice/text patients, trainees and experts for deliberate practice](https://arxiv.org/abs/2607.25667)
*Rodolfo Rizzi,Alessandro Grecucci,Massimo Stella*

Main category: cs.CL

TL;DR: 本文提出了MyMentorLLM，一个多模态语音和文本模拟环境，用于认知行为疗法（CBT）的刻意练习，生成了2100个完整的CBT训练会话。


<details>
  <summary>Details</summary>
Motivation: 心理治疗师需要专家反复培训和督导，但可扩展性存在问题，因此需要一种可扩展的模拟训练方法。

Method: 采用MyMentorLLM模拟环境，结合DSM-5-TR的患者模型、受训治疗师和专家督导，生成CBT训练会话，并分析情绪动态、治疗能力和诊断准确性。

Result: 模拟患者表达出与障碍一致的情绪特征，受训治疗师表现出类似真实咨询的镜像反应；不同LLM的督导质量有差异，原生语音到语音模型最接近人类评分；督导反馈在7个LLM中的5个中提高了诊断准确性。
...
### [230] [SpeechLLM Meets Federated Learning for End-to-End ASR: English and Italian Case Studies](https://arxiv.org/abs/2607.25716)
*Mohamed Nabih Ali,Daniele Falavigna,Alessio Brutti*

Main category: cs.CL

TL;DR: 本文首次系统研究语音大模型在联邦学习下的端到端语音识别训练，提出通信高效优化策略，在英语和意大利语任务中验证了有效性，降低了通信成本。


<details>
  <summary>Details</summary>
Motivation: 联邦学习已应用于隐私保护的语音识别，但尚未探索其在语音大模型中的大规模应用。

Method: 设计针对语音大模型架构的通信高效联邦优化策略，处理高维参数、梯度通信开销和分布式计算约束。

Result: 在英语和意大利语单语任务中，联邦方法达到竞争性的词错误率，同时降低通信成本；消融实验分析了不同语音编码器的影响。

Conclusion: 为联邦语音大模型在多语场景中的实际部署奠定了基础。
...
### [231] [WorkSurface-Bench: Benchmarking Enterprise Agents on Multi-Surface Knowledge Routing](https://arxiv.org/abs/2607.25765)
*Hao Liang,Meiyi Qiang,Sizhe Qiu,Linzhuang Sun,Wentao Zhang*

Main category: cs.CL

TL;DR: 提出WorkSurface-Bench基准，评估智能体跨文档、表格、图和交叉任务选择知识源的能力，包含1151个原子任务，27,624条轨迹，发现路由正确但答案准确率仅56.1-75.3%。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常只评估检索或工具使用，未区分智能体是否先选择合适的知识源，而企业智能体需整合异构知识源。

Method: 构建WorkSurface-Bench基准，包含来自Workspace-Bench-Lite的1151个原子任务，覆盖文档、表格、图和交叉问题；通过DuckDB查询、文本跨度验证和依赖注释追溯生成可审计参考答案。

Result: 在黄金约束下，路由F1达98.7-99.8%，但答案准确率仅56.1-75.3%；表面提示提升3/4模型答案，移除不相关工具改善路由和效率；所有200个采样任务通过审计。

Conclusion: 正确选择知识源是任务完成必要但不充分条件，需进一步改进答案生成能力。
...
### [232] [Evaluation of Adversarial Robustness in Arabic Language Models](https://arxiv.org/abs/2607.25814)
*Anwar Alajmi,Ayed Salman,Imtiaz Ahmad*

Main category: cs.CL

TL;DR: 本文评估了五种先进的阿拉伯语语言模型在对抗攻击下的鲁棒性，发现字符级添加变音符可降低92%准确率，词级操纵连词导致58%性能下降，句级改写平均降低76%性能，对抗训练虽提升鲁棒性但应对字符级噪声仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语语言模型的出色能力引发对其脆弱性的担忧，对抗攻击是主要安全风险，需系统评估模型鲁棒性并探索防御策略。

Method: 在字符、词、句三个粒度上应用阿拉伯语特定对抗攻击，采用不同样本生成策略（如插入变音符、操纵连词、改写），并使用对抗训练作为防御技术。

Result: MARBERT最鲁棒，AraBERT相对增益最大；字符级攻击效果最强（92%准确率下降），词级攻击保持高语义相似度但降低58%性能，句级改写平均降低76%性能；对抗训练整体提升鲁棒性但字符级噪声仍是挑战。

Conclusion: 当前防御策略在形态丰富的阿拉伯语中既有潜力也有限制，尤其需改进对字符级攻击的防御能力。
...
### [233] [Shieldstral](https://arxiv.org/abs/2607.25857)
...
TL;DR: 提出3B参数的多模态安全分类器Shieldstral，在文本和多模态安全分类任务上匹配或超越大规模模型，通过统一问答框架实现策略适应。


<details>
  <summary>Details</summary>
Motivation: 现有安全分类模型在规模与策略适应性之间难以平衡，特别是多模态场景下缺乏高效的小模型方案。

Method: 将内容审核转化为二元问答任务，统一异构安全数据集；构建约5410万样本的训练集及细粒度评估集。

Result: 在文本安全基准上匹配或超越比自身大7倍的模型，在多模态安全分类上达到新最优。

Conclusion: 通过统一训练框架和数据策略，小规模模型可实现与大规模模型相当甚至更优的安全分类性能，验证策略适应性。
...
### [234] [AngelSpec: Towards Real-World High Performance Inference with Speculative Decoding](https://arxiv.org/abs/2607.25852)
...
TL;DR: AngelSpec是一个统一训练框架，集成了自回归多token预测和块并行扩散推测解码，通过三级协同适应（训练、架构、推理）提升大模型推理效率，在Hy3系列上实现最高2.40倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码中，单种草稿结构无法适应真实工作负载的异构性（如对话与代码生成需求不同），需要统一框架动态选择最优策略。

Method: 三级适应性设计：1）训练层：为MTP和块扩散草稿器分别训练不同领域数据；2）架构层：提出DFly块扩散框架，结合混合目标条件骨干与前驱条件自回归头；3）推理层：动态调整验证资源分配，结合效用模型和成本模型在线优化验证深度。

Result: 在Hy3-A21B上平均接受长度提升约30%，在4-64并发下吞吐量比自回归解码快1.98-2.40倍，比DFlash高10.5-11.8%。

Conclusion: AngelSpec通过数据、架构和推理三级专用化，有效解决了推测解码中异构工作负载的适配问题，显著提升了推理吞吐量。
...
### [235] [AI's Capability in Assisting Scientific Research in Physics, Astrophysics, and Cosmology II: Project Planning and Proposal Evaluation](https://arxiv.org/abs/2607.25881)
...
TL;DR: 比较人类和三种LLM（ChatGPT、Claude、DeepSeek）生成的物理学研究提案质量，发现人类评审员对两者评分相似，但AI评审员偏好AI生成的提案。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在科研项目规划和提案评估中的能力，以及潜在偏好风险。

Method: 人类和LLM各生成8份提案（共32份），由4位人类评审员和2个AI模型按四维标准盲审，并识别作者身份。

Result: 人类评审员对AI/人类提案评分无显著差异；AI评审员给AI提案打分高约1分（5分制）；人类识别准确率72%-79%，AI识别准确率100%。
...
### [236] [Polistemics: Evaluating LLMs as Information Mediators in Politics & Elections](https://arxiv.org/abs/2607.25953)
*Baran Peters*

Main category: cs.CL

TL;DR: 提出Polistemics基准，评估LLM在选举中作为政治信息中介的能力，发现高分数掩盖了系统性失败。


<details>
  <summary>Details</summary>
Motivation: 缺乏标准化方法评估LLM是否负责任地中介政治信息，现有研究忽视认知维度和不完美信息。

Method: 基于认知谦逊理论构建基准，在可控设置（清晰、噪声、一致性）下测试，应用于2025年德荷选举的三种SOTA模型。

Result: 模型在清晰证据下表现可靠，但在缺失、模糊或矛盾信息下崩溃，且政治语言强度被弱化，失败受政党先验、标签和输出语言影响。

Conclusion: 可靠的中介是可能的，但无模型能持续实现。
...
### [237] [Evaluating Multi-Turn Multimodal Diagnostic Reasoning on Challenging Real-World Clinical Cases](https://arxiv.org/abs/2607.25933)
...
TL;DR: 提出了ClinMM-Bench，一个用于评估多模态大模型临床诊断能力的多轮基准，包含1089个真实病例和3760张医学图像，评估了15个模型的诊断准确性和推理质量。


<details>
  <summary>Details</summary>
Motivation: 现有评估通常基于单轮或孤立任务，无法真实反映临床实践中多模态信息逐步披露、诊断假设动态更新和临床推理不断完善的复杂性。

Method: 构建包含8个专科1089例病例和3760张图像的ClinMM-Bench，采用两级评估框架（诊断准确性和推理质量），对15个代表性多模态大模型进行系统评估，并进行错误分析。

Result: 闭源模型整体诊断准确性最高，但完全正确诊断的比例仍然有限；模型能识别合理诊断方向，但在生成可靠诊断推理方面存在明显局限。

Conclusion: 当前模型在临床诊断推理质量上仍有较大不足，错误分析揭示了五种典型失败模式：信息综合失败、知识映射错误、感知错误、过早关闭和视觉幻觉。
...
### [238] [Detecting Knowledge Inconsistencies Across Text, Tables, and Knowledge Graphs](https://arxiv.org/abs/2607.25959)
*Fanfu Wei,Thibault Ehrhart,Raphaël Troncy*

Main category: cs.CL

TL;DR: 本文研究了维基百科和维基数据中文本、表格和知识图谱之间的跨模态知识不一致问题，提出了分类体系，并开发了自动框架Kontrast来检测和分类这些不一致。


<details>
  <summary>Details</summary>
Motivation: 维基媒体数据被广泛使用，但其知识以多种模态（文本、表格、KG）存在，模态间可能相互冲突，现有研究缺乏系统性的跨模态不一致检测方法。

Method: 提出Kontrast框架，利用Text-to-SPARQL和LLM推理，将表格答案与KG证据对比，并根据分类体系对不一致进行分类。

Result: 在多个Table-QA数据集上，跨模态不一致普遍存在，揭示了知识冲突、KG结构缺失和时间错配，但受Text-to-SPARQL错误和噪声限制。

Conclusion: 文本、表格和KG可以通过系统比较相互补充和纠正，Kontrast提供了大规模知识审计工具，为跨模态一致性研究建立基准。
...
### [239] [UniMem: Complementary Episodic-to-Parametric Memory for Boundary-Agnostic Task Streams](https://arxiv.org/abs/2607.26017)
...
TL;DR: 提出UniMem框架，通过可学习路由令牌协调情节缓冲与参数化记忆，在不依赖任务边界的情况下平衡记忆可塑性与稳定性，在长序列任务中平均提升EM 4.0个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理记忆方法存在稳定-可塑性矛盾：检索式记忆虽能快速吸收新证据但无法内化重复模式且增加推理开销，参数记忆依赖显式任务边界和固定参数预算。

Method: 采用可学习路由令牌作为记忆控制器，动态协调情节缓冲（暂存新/稀疏任务）与可扩展参数化记忆（巩固重复模式），实现任务识别与执行的解耦。

Result: 在长程流式任务序列上，UniMem在三个骨干模型上平均提升4.0个EM分数，且优于所有基线方法。

Conclusion: UniMem通过互补记忆通路和自适应路由机制，有效解决了LLM代理在无边界任务流中的稳定性-可塑性权衡问题。
...
### [240] [Instruction-Tuned Models Locally Reuse Human Syntax More Than Humans Do](https://arxiv.org/abs/2607.26015)
*Zandi Eberstadt*

Main category: cs.CL

TL;DR: 本研究调查了大语言模型在与人类对话时是否表现出句法趋同现象。通过替换人类对话中的话轮，测量了模型对上下文无关语法规则的复用，发现所有模型都显示出趋同，且低频率规则的趋同效应更强。指令微调增强了这种趋同，但模型对无关刺激的回应也有更多重叠。


<details>
  <summary>Details</summary>
Motivation: 句法趋同是人类对话中的普遍现象，常在无意识层面运作。但大语言模型是否在与人类用户对话时表现出类似的句法趋同，特别是针对广泛的句法结构，这仍是一个未解之谜。

Method: 采用替换范式数据，将模型生成替换已有对话中的人类话轮，测量相邻话轮间上下文无关语法规则的复用程度。分析了16个Llama和Gemma模型（1B-70B，预训练和指令微调），每个模型在1901个匹配位置进行评估。

Result: 所有模型对前一个人说话轮的CFG规则重叠度高于随机刺激；指令微调模型在自然输出中对实际刺激的重叠度高于被替换的人类回应；指令微调后，模型对实际刺激和随机刺激的重叠度均增加，但实际与随机差异减小；模型在词汇和语义相似度上也优于人类基准。

Conclusion: 大语言模型确实表现出句法趋同，且指令微调增强了这种倾向，但同时也可能降低了句法特异性。
...
### [241] [Pass the Baton: Trajectory-Relayed On-Policy Distillation](https://arxiv.org/abs/2607.26057)
*Haolei Xu,Xiaowen Xu,Haiwen Hong,Zixuan Ni,Hongxing Li,Yiwen Qiu,Weiming Lu,Yongliang Shen*

Main category: cs.CL

TL;DR: 提出中继在线蒸馏（Relay-OPD），通过检测学生失败前缀并让教师临时接管，解决在线蒸馏中的前缀错误问题，在数学推理任务上提升性能并减少训练轨迹长度。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏中，学生一旦开始错误推理前缀，后续生成会持续偏离正确方向，导致监督信号不可靠并浪费计算资源。

Method: Relay-OPD在检测到学生失败前缀时，让教师短暂接管生成教师片段，学生再从中继点继续并优化完整轨迹。

Result: 在8个数学推理基准上，1.7B学生模型平均比标准OPD提升5.73%，比最强基线FastOPD提升1.49%，0.6B模型也有一致增益，训练轨迹长度减少超50%。

Conclusion: Relay-OPD有效缓解前缀失败问题，提升蒸馏效率与性能，在低资源场景下具有实用价值。
...
### [242] [Egocentric Station Holding of Robotic Fish in Unknown Turbulent Background Flow](https://arxiv.org/abs/2607.24860)
*Xiaozhu Lin,Xu Huang,Hongru Dai,Xiaopei Liu,Junzhi Yu,Yang Wang*

Main category: cs.RO

TL;DR: 提出SWiFT框架，通过强化学习实现机器鱼在未知湍流中的自主保持位置，仅依靠自我中心反馈即可达到类似趋流性的效果。


<details>
  <summary>Details</summary>
Motivation: 机器鱼在自然水流中接近目标和保持位置的能力不足，主要因为流体-结构相互作用的非线性特性尚未充分表征。

Method: 集成自由游泳实验装置、基于CFD的高效仿真器以及系统化的仿真到现实迁移管道，通过强化学习训练自我中心保持位置策略。

Result: 在所有指标上显著优于现有方法，特别是距离的均方根误差（RMSE）；仅凭自我中心反馈即可在未知湍流中实现位置保持。

Conclusion: 该策略推进了机器鱼控制的实际部署，SWiFT可作为解决水下机器人复杂游泳任务的基础。
...
### [243] [Steeringless Drifting: Differential-Torque Control of a Four-Wheel Independently Driven Vehicle](https://arxiv.org/abs/2607.24863)
*Sheng Zhao,Zexin Wu,Dongyang Zhou,Bolin Zhao,Xiaodong Wu*

Main category: cs.RO

TL;DR: 针对无转向四轮独立驱动车辆，提出了一种仅利用差动扭矩实现漂移控制的方法，通过仿真与实验验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 探索无转向车辆架构在接近操控极限时的漂移控制方法，替代传统依赖机械转向和后轮饱和的漂移技术。

Method: 建立了包含四轮差动驱动的双轨车辆模型，开发了漂移平衡计算方法和闭环漂移控制器。

Result: 车辆实现了约20度侧偏角的稳态圆形漂移和八字形漂移轨迹跟踪。

Conclusion: 仅利用差动轮扭矩即可实现漂移控制，为无转向车辆架构的近极限控制提供了新思路。
...
### [244] [Amortising Trajectory Optimisation for Residual MPC via Implicit Contact Differentiation](https://arxiv.org/abs/2607.24959)
*Daniel Layeghi,Thomas Corbères,Calum Arnott,Aditya Kamireddypalli,Hashim Al-Obaidi,Steve Tonneau,Michael Mistry*

Main category: cs.RO

TL;DR: 提出一种基于隐函数定理的AD辅助隐式微分方法，用于正则化平滑接触的轨迹优化，显著降低内存消耗，并结合优化器蒸馏提升MPC性能。


<details>
  <summary>Details</summary>
Motivation: 现有可微仿真方法存在计算昂贵、步长敏感、内存增长或推导复杂等问题，亟需高效稳定的梯度计算方法。

Method: 通过隐函数定理微分容差收敛解处的平稳残差，结合自动微分辅助，避免求解器展开和手动KKT系统组装；同时提出优化器蒸馏将全时域iLQR策略提炼用于残差MPC。

Result: IFT内存几乎不随求解迭代变化（<4%增长），相比展开AD减少10.6倍；在256接触时内存节省20倍；优化器蒸馏在多个机器人任务中将六步成功率提升28-98个百分点。

Conclusion: IFT方法在接触丰富的轨迹优化中兼顾效率与内存，优化器蒸馏有效增强短时域残差MPC的性能。
...
### [245] [Motion Generation With Environmental Constraints](https://arxiv.org/abs/2607.25053)
*Előd Páll,Oliver Brock*

Main category: cs.RO

TL;DR: 本文提出环境约束利用（ECE）方法，通过主动接触环境来简化高维空间中的机器人运动规划，降低维度与计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统运动规划在高维不确定环境中受限于无碰撞要求，计算复杂且效率低。

Method: 将ECE集成到RRT规划器中，利用接触偏置探索至任务相关区域并减少不确定性。

Result: 实验验证ECE在真实应用中提高了规划的鲁棒性和适应性。

Conclusion: ECE能有效简化运动规划，并在复杂环境中提升性能。
...
TL;DR: 提出FIRMGrasp系列度量，基于条件风险价值（CVaR）评估抓取在面对摩擦系数波动时的力封闭性。


<details>
  <summary>Details</summary>
Motivation: 经典抓取质量度量假设单一确定性摩擦系数，无法预测接触表面摩擦值变化时抓取是否仍保持力封闭。

Method: 采用CVaR风险度量，计算不利摩擦尾部的风险调整力封闭余量ε^(β)，建立其单调性、可微性和概率封闭保证。

Result: 在LEAP Hand和Allegro Hand的1599次抓取中，53%被名义Ferrari-Canny阈值的抓取在不利摩擦尾段失去力封闭；ε^(β)正确排序抓取成功概率达0.63和0.78，优于名义度量的0.53和0.67；在摩擦系数0.2的模拟提升实验中，ε^(β)认证的抓取成功率为70%，名义度量认证但ε^(β)拒绝的仅25%。

Conclusion: FIRMGrasp度量能有效识别对摩擦敏感的抓取，并在变摩擦条件下提供更可靠的力封闭保证。
...
### [247] [Hybrid Artificial Potential Fields and Spatio-Temporal Transformers for Real-Time AUV Path Planning](https://arxiv.org/abs/2607.25056)
*Khadija Rais,Abdelmadjid Benmachiche,Imene Soualmia*

Main category: cs.RO

TL;DR: 本文比较了13种AUV路径规划算法，发现混合APF+ST-Transformer在路径最优性、碰撞避免和计算效率上表现最佳。


<details>
  <summary>Details</summary>
Motivation: AUV在复杂水下环境中需要高效安全的路径规划以完成任务并节约能量。

Method: 比较了经典图搜索（A*、Dijkstra）、采样法（RRT*）、元启发式（PSO、GA、ACO、BCO）和混合APF+ST-Transformer等方法。

Result: 混合APF+ST-Transformer路径最短（943.15单位）、碰撞率低（0.031）、计算快（0.96秒），优于其他方法。

Conclusion: 混合APF+ST框架推荐用于实时AUV导航，综合平衡了反应式避障和全局路径最优性。
...
### [248] [Input Shaping for Point-to-Point Motion with a Continuum Robot Arm](https://arxiv.org/abs/2607.25071)
*Rodolfo Hdz. Ibarra,Karan Baker,Parsa Molaei,Adrian Stein,Hunter B. Gilbert*

Main category: cs.RO

TL;DR: 提出基于时延滤波器的输入整形方法，用于抑制缆索驱动连续体机器人末端残余振动，并实验验证了其效果。


<details>
  <summary>Details</summary>
Motivation: 连续体机器人作为欠驱动机构，在点对点运动结束后易出现残余振动，影响精度。

Method: 基于线性系统模型设计非鲁棒和鲁棒两种时延滤波器，将其作为输入整形器应用于系统。

Result: 与速度驱动脉冲输入相比，整形后响应过冲和调节时间显著降低；鲁棒整形器进一步改善了响应性能。

Conclusion: 输入整形能有效抑制振动，提升连续体机器人精确和鲁棒运动控制的能力。
...
### [249] [Reactive 3D Motion Planning for a Franka Arm via Star-World Workspace Reshaping](https://arxiv.org/abs/2607.25138)
*Gia Dcosta,Saayuj Deshpande,Samhitha Vedire*

Main category: cs.RO

TL;DR: 在三维机器人操控中，通过星形工作空间重塑处理安全膨胀导致的重叠障碍物，提高目标到达率但增加路径长度并可能出现近平衡态。


<details>
  <summary>Details</summary>
Motivation: 安全膨胀会导致相邻障碍物重叠，违反许多基于调制的反应式规划器使用的分离障碍物假设。

Method: 使用星形工作空间重塑，将相交膨胀障碍物聚类并替换为星形代理，然后基于动态系统末端执行器控制器，并加入零空间人工势场进行手臂避障。

Result: 在六个PyBullet场景中，重塑方法在五个场景中到达目标，未重塑基线为四个；解决了重叠墙问题，更新时间为0.68-8.70毫秒，但增加了路径长度，两个场景出现近平衡态，并因过度合并关闭了可导航走廊。

Conclusion: 展示了将星形工作空间几何保证转移到通过逆运动学控制的冗余机械臂的前景和实际局限性。
...
### [250] [Decentralized Scalable Exploration via Emergent Adaptive Lévy Walks on Minimal-Sensing Platforms](https://arxiv.org/abs/2607.25195)
*Wai Lun Leong,Teo Swee Huat Rodney*

Main category: cs.RO

TL;DR: 提出了一种轻量级传感器驱动的Lévy行走控制器，用于重量小于50克、配备稀疏局部感知的纳米无人机，实现高效自主探索。


<details>
  <summary>Details</summary>
Motivation: 手掌大小的纳米无人机在感知、计算和续航方面存在严重限制，使得高效自主探索仍然具有挑战性。

Method: 结合离散Lévy步长采样和基于传感器反应的航向策略（使用von Mises分布偏向开放方向），每个机器人独立从均匀先验中采样Lévy指数以多样化探索，无需通信。

Result: 仿真中，在开放场地、房间走廊和杂乱环境中，覆盖率分别提升79.6%、43.1%和13.6%，碰撞分别减少13.0%、7.1%和1.4%。

Conclusion: 为资源受限的纳米无人机提供了一种可扩展的多机器人探索实用框架。
...
### [251] [Leveraging Semantic Maps for City-Scale Cross-View Localization](https://arxiv.org/abs/2607.25215)
*Ethan Fahnestock,Erick Fuentes,Philip R Osteen,Nicholas Roy*

Main category: cs.RO

TL;DR: 提出了使用VLM从全景图中提取语义信息并与OpenStreetMap匹配进行机器人定位的方法，并蒸馏了一个轻量化匹配器以提高效率，在多种环境条件下展示了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么忽略语义信息，要么仅使用少量固定类别，无法充分利用OpenStreetMap的丰富语义数据来帮助机器人在未遍历环境中定位。

Method: 利用VLM从机器人全景观察中提取相关地标，并识别与先验地图地标之间的对应关系；然后蒸馏一个轻量化匹配器以高效处理大量地图地标；通过贝叶斯滤波器融合观测似然得到时间序列位姿估计。

Result: 在单一城市晴天数据上训练的模型，能够泛化到不同地点、光照、天气（如雪暴）和夜间等挑战场景。

Conclusion: 基于VLM的语义信息提取和匹配方法能有效利用OpenStreetMap的丰富数据，实现机器人跨场景泛化定位，且蒸馏的轻量化匹配器解决了大规模地图的缩放问题。
...
### [252] [SONG: A Photorealistic 3D Gaussian Simulation Platform for Benchmarking Social Navigation](https://arxiv.org/abs/2607.25219)
*Weiqi Huang,Dianyi Yang,Jiaxin Li,Shuangyi Dong,Hao Xu,Zan Wang,Wei Liang*

Main category: cs.RO

TL;DR: 提出SONG平台，基于3D高斯泼溅构建视觉社交导航仿真环境，支持逼真场景、智能行人及运动生成，并配套基准测试集SONG-Bench。


<details>
  <summary>Details</summary>
Motivation: 现有仿真平台缺乏视觉观察、动态行人或真实感，难以支持视觉社交导航研究。

Method: 利用3DGS表示场景和行人，LLM生成语义轨迹驱动行人，轨迹条件生成器合成全身运动。

Result: 视觉社交导航尚未解决；安全缺陷先于社交礼仪；真实数据比模型规模更重要；微调SONG数据可提升真实环境成功率。

Conclusion: SONG为视觉社交导航提供了高保真测试平台，有助于推动该领域发展。
...
### [253] [Belief-Aware Influence and Trust (BAIT): Shaping Human Belief During Repeated Human-Robot Interaction](https://arxiv.org/abs/2607.25327)
*Ye-Ji Mun,Mahsa Golchoubian,Shahabedin Sagheb,Yan Bai,Tianhao Ji,Dylan P. Losey,Katherine Driggs-Campbell*

Main category: cs.RO

TL;DR: 提出了信念感知影响与信任（BAIT）控制器，通过分层粒子滤波和模型预测路径积分规划器，在重复人机交互中平衡长期影响与人类信任，同时保证任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有框架将交互视为孤立事件，导致任务性能衰减或依赖不可预测行为损害信任，且计算不可扩展。

Method: BAIT控制器集成分层粒子滤波（推断人类战略转变和信念更新）与信念感知模型预测路径积分规划器，显式优化长期影响与信任的权衡。

Result: 在仿真、人类受试者研究和真实GEM车辆实验中，BAIT的任务性能与通过不可预测性优化长期影响的基线相当，但显著提高了用户信任。

Conclusion: BAIT有效解决了重复HRI中任务性能与信任维护的冲突，并通过实际部署验证了其可行性和优越性。
...
### [254] [SGTP: Sampling-based Game-Theoretic Planning for Real-Time Multi-Vehicle Autonomous Racing](https://arxiv.org/abs/2607.25388)
*Zhouheng Li,Fangguo Zhao,Mattia Piccinini,Baha Zarrouki,Yuan Gao,Zitong Shan,Johannes Betz,Chen Lv,Lei Xie*

Main category: cs.RO

TL;DR: 提出了一种结合博弈论和GPU加速采样的实时多车竞速规划框架SGTP，在高度交互场景中实现了95.24%的胜率和99.35%的任务完成率。


<details>
  <summary>Details</summary>
Motivation: 现有多车竞速规划器难以兼顾策略多样性和计算效率。

Method: 通过博弈论指导的GPU加速控制序列采样与轨迹评估，结合可行性选择（赛道边界和动态碰撞规避）。

Result: 在复杂赛道上胜率95.24%，任务完成率99.35%，平均计算时间0.095秒，支持多达10个智能体。

Conclusion: SGTP实现了实时、安全且策略多样的多车竞速规划，并开源了代码和基准测试。
...
### [255] [Decompose and Reorganize: Planning with Primitives and Visuomotor Policies Learned from Demonstrations](https://arxiv.org/abs/2607.25397)
*Yizhou Chen,Hang Xu,Dongjie Yu,Yupu Lu,Tengye Xu,Zeqing Zhang,Wei Zhang,Yi Ren,Ben M. Chen,Jia Pan*

Main category: cs.RO

TL;DR: 提出DR-LfD框架，结合视觉运动策略与TAMP，通过分解演示为原子技能并重组，解决长程灵巧操作中的泛化与多阶段问题。


<details>
  <summary>Details</summary>
Motivation: 传统TAMP在接触密集操作中脆弱，而模仿学习在空间泛化和多阶段任务上受限，需互补二者优势。

Method: 基于接触关系分解演示为原子技能，实现为视觉运动策略或对象中心原语；建模策略起止与约束并与TAMP兼容，实现跨来源技能重组。

Result: 在多样真实与仿真场景中，DR-LfD在多步骤、未见设置和物理约束任务上表现强劲。

Conclusion: DR-LfD将演示需求从指数级降至与技能类型数线性相关，有效整合高层推理与细粒度执行。
...
### [256] [Room-Mediated Co-occurrence for Zero-Shot Object-Centric Semantic Navigation via Frontier Scoring](https://arxiv.org/abs/2607.25448)
*Adam Scicluna,Gavin Paul,Alen Alempijevic*

Main category: cs.RO

TL;DR: 提出了一种无训练、基于对象中心语义的零样本导航方法，通过房间概率向量（RPV）和快速行进法实现。


<details>
  <summary>Details</summary>
Motivation: 现有零样本对象导航方法中，对象-对象相似性在隐空间中不能很好地表示空间共现关系。

Method: 将对象标签映射到CLIP导出的房间概率向量，通过分布重叠计算共现分数，再结合快速行进法进行自适应信号衰减传播，生成价值图并排序前沿点。

Result: 在HM3D数据集上将成功率相对提升3%，SPL相对提升1.3%。

Conclusion: 对象中心方法可在保持可解释性和开放词汇灵活性的同时，优于图像整体基线。
...
### [257] [A Causality-aware Infer-diagnose-refine Framework for Test-time Modality Adaptation in VLA Models](https://arxiv.org/abs/2607.25516)
*Haoyu Zhang,Yuwei Wu,Jin Chen,Gao Zhi,Zhenxin Diao,Mingyang Gao,Kun Wu,Yongchun Liu,Fan Li*

Main category: cs.RO

TL;DR: 提出IDR框架，在测试时动态调整视觉信息重要性，提升VLA模型的动作预测质量。


<details>
  <summary>Details</summary>
Motivation: 机器人操作中视觉重要性随时间变化，现有VLA模型缺乏动态融合机制。

Method: 基于因果推断的infer-diagnose-refine三阶段框架，包括零填充干预、范数量化和门控残差融合。

Result: 在仿真和真实任务中多个VLA骨干网络上性能均有提升。

Conclusion: 测试时动态调整视觉重要性可有效改进动作预测。
...
### [258] [P3: Probabilistic Policy Propagation for Stable VAE-Based Robot Learning](https://arxiv.org/abs/2607.25541)
*Liyun Yan,Jianming Ma,Yang Zhang,Shengcheng Fu,Zhanxiang Cao,Keqi Zhu,Yizhi Chen,Yue Gao*

Main category: cs.RO

TL;DR: 提出P³框架，解决VAE策略中单样本近似导致PPO梯度方差与偏差的问题，显著提升数据效率与收敛速度。


<details>
  <summary>Details</summary>
Motivation: VAE随机隐变量与PPO策略存在理论不匹配，单样本近似带来显著方差和偏差，阻碍高效学习。

Method: 提出P³（概率策略传播），结合基于矩的概率方法稳定高效学习，与采样校准确保隐不确定性下的鲁棒策略。

Result: 数据效率从64.6%提升至>96%，收敛步数减少>20%；在类人跑酷任务中表现有效。

Conclusion: P³为基于VAE的策略优化提供了理论基础和实用框架，有效解决单样本近似问题。
...
### [259] [When Does Legacy Data Start to Help? Emergent Transfer in Cross-Configuration Robot Learning](https://arxiv.org/abs/2607.25593)
*Tao Wang,Hudson Hou,Yingdong Hu,Yufeng Liu,Qinghai Li,Yingjie Jiang,Yingzhi Wang,Cheng Ma,Richard Wang,Yang Gao*

Main category: cs.RO

TL;DR: 研究了升级后的机器人何时能从旧版硬件的数据中受益，发现存在类似“grokking”的三阶段转变规律，并提出了基于梯度对齐的理论解释和实用规则。


<details>
  <summary>Details</summary>
Motivation: 机器人硬件更新后，旧数据是否有效是未解决的问题，而通常假设更多跨配置数据总是有益的，但实际观察并非如此。

Method: 通过在两个硬件代际的轮式人形平台上进行实验（改变相机和夹爪），分析旧数据在升级机器人上的有效性，并结合梯度对齐和残余策略不确定性提出理论解释。

Result: 发现旧数据在升级机器人达到一定任务能力后才显著提升性能（如插花任务从23.3%提升到86.7%），并在高能力时收益递减（如插笔任务从85.0%到93.3%）。

Conclusion: 提出旧数据有效性的三阶段模式，并给出了何时收集新数据或复用旧数据的阶段性规则，在移动双臂浇水任务中得到验证。
...
### [260] [Shared Voxel-Map-Based Cooperative Indoor UAV Guidance with a Multi-Agent Soft Actor-Critic Controller](https://arxiv.org/abs/2607.25728)
*Thomas Hickling,Dylan Wynne,Yu Su,Nabil Aouf*

Main category: cs.RO

TL;DR: 本文提出一种结合共享体素地图与MASAC的室内无人机协同导航框架，通过“世界融合-自身行动”设计实现多机空间融合与分散控制，仿真成功率90.3%，并利用离线微调解决仿真到现实差异，在真实GNSS拒止环境中验证了双机协同稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有无人机室内导航方法缺乏高效的多机协同机制，且难以应对全局定位信号缺失环境中的空间融合与控制分散需求。

Method: 采用共享体素地图全局融合多机360°LiDAR观测，转化为BEV局部表征，结合MASAC算法在集中训练-分散执行框架下融合地图、障碍物、目标与邻居状态信息。

Result: 仿真中走廊导航成功率达90.3%，优于A*规划、人工势场及传统引导方法；真实环境实验在递增障碍布局中验证了双机稳定协同操作。

Conclusion: 共享体素地图为学习型协同室内无人机导航提供了有效且可扩展的空间基座，离线微调策略可有效缓解仿真到现实差异。
...
### [261] [Physics-Aware End-to-End Deep Reinforcement Learning for Quadcopter Control with Actuator Dynamics](https://arxiv.org/abs/2607.25985)
*Ya-Chia Shen,Woei-Leong Chan*

Main category: cs.RO

TL;DR: 本文提出一种基于物理感知的端到端深度强化学习方法，直接控制四旋翼无人机的总推力和机体扭矩，通过高保真Simulink仿真环境评估四种DRL算法，发现SAC和TD3在稳定性和探索效率上优于PPO。


<details>
  <summary>Details</summary>
Motivation: 四旋翼无人机具有欠驱动动力学特性（四个控制输入需控制六个自由度），传统控制方法面临挑战，需探索更有效的自主控制方案。

Method: 构建包含12状态刚体模型、Action2RPM分配（基于Moore-Penrose伪逆）、一阶执行器动态（时间常数0.076秒）及转子陀螺耦合的高保真Simulink仿真环境，使用形状化奖励函数（指数位置阱、姿态惩罚、二次速度成本），对比DDPG、TD3、PPO和SAC四种算法。

Result: SAC和TD3在悬停稳定性与探索效率上表现最优，PPO样本效率较低；精确建模执行器时滞和气动力矩对稳定低层控制至关重要。

Conclusion: 物理感知的DRL方法可有效实现四旋翼无人机的低层控制，SAC和TD3为优选算法，研究提供了可复现的基准测试。
...
TL;DR: 提出TriManPolicy系统，通过依赖感知的三臂调度（DATS）离线重新编排演示时间，解决三机械臂模仿学习中人机控制通道不匹配导致的延迟问题。


<details>
  <summary>Details</summary>
Motivation: 传统双手遥控方式无法处理三机械臂系统（人仅能同时控制两臂），直接记录顺序操作会引入界面延迟，导致模仿学习策略错误复制延迟而非任务需求。

Method: DATS方法：固定局部感知运动片段的持续时间，根据人工审查的任务顺序和臂使用约束重新定位这些片段，保留原始动作轨迹但调整时序，生成同步三臂策略。

Result: 在6个真实世界任务中，DATS重排的演示数据训练的策略表现出更高效的协调性，任务成功率与原始数据相当；离线分析证实DATS改变的是臂间监督信号而非简单删除空闲期。

Conclusion: 重排演示时序可有效解耦模仿学习中的接口延迟与任务需求，为超人手部自由度的机器人系统提供可行的学习方案。
...
TL;DR: 提出一种多智能体深度强化学习框架，通过协同探索、示范利用、安全课程调度和结构感知泛化，解决多无人机协作导航中的局部最优、稀疏奖励、学习不平衡和跨场景泛化问题。


<details>
  <summary>Details</summary>
Motivation: 多无人机协作导航面临局部最优陷阱、稀疏奖励、智能体学习不平衡及跨场景泛化不足等关键挑战。

Method: 结合记忆状态感知、方向新颖性估计与惩罚回传的感知机制；基于协作程度分级的层次化示范缓冲与行为克隆；安全感知的双条件课程调度机制；基于局部几何特征的结构感知门控网络与专家混合机制。

Result: 仿真实验在协作成功率、导航鲁棒性、零样本跨场景泛化及动态环境适应性上表现优异。

Conclusion: 所提框架能有效提升复杂环境下多无人机协作导航的性能与泛化能力，并适应动态障碍物干扰。
...
### [264] [Motion-Acceleration Calibration and Compensation in IMUs without External Equipment for Attitude Estimation Filters](https://arxiv.org/abs/2607.25784)
*Fabian Arzberger,Andreas Nüchter*

Main category: cs.RO

TL;DR: 提出一种利用陀螺仪补偿运动加速度的方法，并标定IMU内外参数，提升任何依赖重力方向的姿态滤波器性能。


<details>
  <summary>Details</summary>
Motivation: IMU远离系统基座时运动加速度会扭曲重力测量，现有滤波器（如Madgwick/Mahony）在高角速度时依赖陀螺仪积分，但会积累长期误差。

Method: 利用陀螺仪测量补偿向心加速度和切向加速度，并引入无需外部设备的标定方法估计IMU内参（轴偏差、偏置、尺度）及外参（底座到IMU向量）。

Result: 仿真和真实实验表明方法改善了任何依赖重力方向的姿态滤波器，尤其在高动态系统和IMU无法置于旋转中心的系统（如球形移动测绘系统）中有效。

Conclusion: 该方法能有效消除运动加速度干扰，提升姿态估计长期精度。
...
### [265] [Tripody: An Overconstrained 3-SPR-like Parallel Robot for High-Reach Construction Tasks](https://arxiv.org/abs/2607.25781)
*Julien Kindle,Jakub Raczy,Riccardo Balbi,Andrea Alessandretti,Cesar Cadena,Marco Hutter*

Main category: cs.RO

TL;DR: 本文介绍了一种名为Tripody的三自由度轮式并联机器人，用于高空天花板施工任务。该系统重33kg，最大伸展高度3.4m，负载32kg，通过过约束设计和弹性形变实现高精度定位与高扭转刚度，实验表明其闭环定位精度优于0.6mm，远超传统球形关节变体。


<details>
  <summary>Details</summary>
Motivation: 现有高空施工多采用重型串行机械臂，部署困难且精度不足；亟需轻量化、高刚度且达到毫米级精度的现场替代方案。

Method: 采用过约束的3-SPR并联结构（球铰换为万向节），通过分布式弹性形变吸收运动不兼容性；结合SE(3)状态估计、正向运动学和任务空间控制，配合定制线性执行器与运动学兼容性分析。

Result: 在1.7m至3.4m高度范围内，扭转刚度较球铰方案提升67%~454%；闭环定位精度<0.6mm，开环钻孔实验最大相对误差4.5mm（15孔图案）。

Conclusion: 过约束且弹性兼容的3-SPR构型是实现轻量化、高可达、毫米级精度施工机器人的可行途径。
...
TL;DR: 提出了Transformer Transformer，一种基于扩散变换器的机器人协同设计方法，可生成跟踪目标轨迹的机器人设计并优化用户定义的奖励。


<details>
  <summary>Details</summary>
Motivation: 机器人操作性能中机器人自身形态常被忽视，需要研究运动条件下的机器人协同设计。

Method: 使用RoboTokens统一编码机器人形态、状态和动作，训练扩散变换器作为动力学模型，并通过动力学自引导生成高价值设计。

Result: 在多个设计空间中零样本优化未见过的奖励和轨迹，性能优于进化基线；优化后的ALOHA设计跟踪误差降低70%以上。

Conclusion: 该方法可实现高效的跨形态机器人设计与控制器生成，验证了实用性和性能优势。
...
TL;DR: 本文提出了一种用于FEVAR/BEVAR手术的双段可操纵导管系统，结合热拉制纤维和激光微加工技术，旨在提高血管介入的灵活性和效率。


<details>
  <summary>Details</summary>
Motivation: 临床研究表明，现有器械在血管入路中耗时较长且易引发并发症，需要改进导管可操纵性。

Method: 设计含4自由度2段式可操纵导管，采用热拉制纤维制造和激光微加工软化尖端，并开发模块化腱驱动平台。

Result: 通过仿真评估了弯曲刚度，并在血管模型中通过体外实验验证了可操纵性。

Conclusion: 该手持式机器人导管系统有望缩短手术时间，帮助医生处理复杂临床病例。
...
TL;DR: 提出高保真UMI数据采集系统HiFi-UMI，实现无需真实机器人微调的零后训练策略部署，性能与遥操作基线相当。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作策略训练中，高保真数据扩展性差（遥操作成本高），而低成本UMI数据虽可扩展但保真度不足，仍需少量真实数据微调。研究目标是提升UMI数据保真度以消除对真实机器人数据的需求。

Method: 设计便携式UMI系统，集成头戴式离线立体惯性SLAM、原生相对位姿估计、微秒级GPIO同步、每手双广角相机（~200°视场），实现3mm末端精度。

Result: 零后训练策略在三个骨干模型上与遥操作基线成功率差异在±3.1%以内；最强模型在精密插入任务达85%成功率；4,000小时预训练降低10个未见任务动作误差41%，提升真实机器人成功率18.1个百分点。

Conclusion: 高保真度UMI数据可完全替代真实机器人微调数据，为大规模策略学习提供高效、低成本的高保真资源。
...
TL;DR: 提出基于SAM3D的物体级3D表示对齐框架，在无需测试时额外3D输入下提升VLA策略在遮挡、位姿变化等场景的操控精度。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型依赖2D视觉-语言主干，缺乏对目标物体的细粒度3D理解，难以应对遮挡、位姿变化、尺度变化和精确空间交互。

Method: 利用SAM3D作为冻结的3D教师，在训练时为目标物体提取密集3D表示，并与π0的中间视觉特征对齐，保留原始RGB语言到动作推理管线。

Result: LIBERO上达99.1%，CALVIN平均长度4.11，在长时序多物体操控场景中效果显著。

Conclusion: 该方法有效提升VLA策略的3D物体感知能力，且测试时无需额外传感器或模块。
...
TL;DR: 提出DC-WAM框架，通过动态中心的重建和推理优化，提升机器人策略在光照、外观、背景变化下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAM）的未来视频预测计算量大且关注非控制相关外观信息，而高效变体表明视频分支的优势在于控制相关表征，而非渲染未来本身。

Method: 在RGB视频分支中，监督层面结合时间差分流匹配和轨迹引导加权，强调动态变化和交互区域；推理层面通过DynaRoute预测令牌动态相关性并转化为注意力偏置。

Result: 在仿真和真实操作任务中，DC-WAM一致提升策略性能，尤其在光照、物体外观和背景纹理的分布外扰动下表现更优。

Conclusion: 通过动态中心的重定向监督和计算，可有效提升机器人策略的泛化能力。
...
### [271] [S2A2: Audio-Visual Imitation Learning for Manipulation Tasks Using Acoustic Spatial Information](https://arxiv.org/abs/2607.26047)
*Kaneyoshi Hiratsuka,Benjamin Yen,Ryosuke Kojima*

Main category: cs.RO

TL;DR: 本文提出了声学感知操作任务和S2A2多模态模仿学习框架，整合视觉与声学信息，在仿真和真实机器人实验中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 声学信息能提供物体位置、材质等丰富线索，但现有模仿学习很少利用听觉信号指导机器人操作。

Method: 提出S2A2框架，融合视觉特征、声学空间与信号信息，并集成ACT、扩散策略等模型。

Result: 仿真表明方法在需定位和音色感知的任务中最优；真实实验验证了任务与框架的可行性。

Conclusion: 声学感知操作任务可行，S2A2框架有效提升了机器人在复杂环境中的操作能力。
...
### [272] [$π\mathbf{R}^2$: Reactive Real-time Flow Policies](https://arxiv.org/abs/2607.26055)
*Sungjae Park,Shubham Tulsiani*

Main category: cs.RO

TL;DR: 提出πR²框架，通过快速通道（本体感觉）和慢速通道（视觉语言）分离以及延迟自适应流调度，使基于大模型的动作块策略实现实时闭环控制，在模拟和真实环境中成功率提升高达30%。


<details>
  <summary>Details</summary>
Motivation: 现有基于大模型的动作块策略因推理延迟高（大模型+多步去噪）而无法频繁重规划，导致难以适应动态环境下的闭环控制需求。

Method: 基于扩散强制（diffusion forcing）的位置噪声调度，提出两个核心设计：1）将条件输入分为快速通道（本体感觉，每次更新）和慢速通道（视觉语言特征，异步更新）；2）延迟自适应流调度，将执行中的动作视为修复条件，每步调用仅进行一次去噪，适应不同硬件延迟。

Result: 在GR00T-N1.7模型上，πR²以~25Hz（每40ms）频率进行闭环重规划，速度约为基线策略的4倍。模拟实验成功率最高提升23%，真实实验最高提升30%。

Conclusion: πR²通过最小化架构修改即可使大型预训练策略具备实时反应能力，弥合了离线规划与在线控制之间的延迟鸿沟，适用于动态场景下的灵巧操作。
...
### [273] [INTACT: Isomorphic Intent-to-Action Learning for Search-Free World Models](https://arxiv.org/abs/2607.26056)
*Junhan Sun,Hao Zhao,Guofeng Zhang*

Main category: cs.RO

TL;DR: 提出INTACT方法，将动作标签化的无奖励轨迹转化为端到端的意图到动作接口，实现无搜索策略推理。


<details>
  <summary>Details</summary>
Motivation: 现有前向潜在世界模型在预测场景变化时，需要昂贵的测试时搜索才能找到对应动作，缺乏高效意图转动作机制。

Method: 采用JEPA架构，本地与目标运动意图共享四槽语法和参数，通过相同预测器实现动作律语义，非对称端点梯度联合表示学习与控制。

Result: LeWM四个任务上单轮零搜索模型成功率达85.78%-100%，可选CEM减少采样23.44倍，推理时间2.9-5.5ms。

Conclusion: INTACT有效联合表示学习与动作控制，无需点态潜在匹配或全局线性动力学，实现高效鲁棒的意图驱动作。
...
### [274] [Ultra-Low-Energy Open-Circuit Fault Diagnosis for Three-Phase Inverters](https://arxiv.org/abs/2607.25037)
*Xiaoyi Lei,Fanfu Wu,Yunting Liu*

Main category: eess.SY

TL;DR: 提出了一种基于事件驱动神经形态框架的三相逆变器故障诊断方法，使用SNN替代CNN，将诊断能耗降至11微焦耳，能效提升382倍，同时保持100%准确率。


<details>
  <summary>Details</summary>
Motivation: 传统CNN方法功耗高，不满足嵌入式系统亚瓦特级功率预算。

Method: 将CNN转换为SNN，利用电流矢量轨迹矩阵的稀疏性，仅在信息区域激活计算，基于Loihi平台进行能耗评估。

Result: 诊断能耗11微焦耳，比GPU-CNN降低382倍，准确率100%，在负载变化和噪声下具有鲁棒性。

Conclusion: 神经形态SNN在低功耗嵌入式故障诊断中具有显著优势。
...
### [275] [VeraGrid-Agent: Tool-Augmented LLMs for Distribution Optimal Power Flow at the Grid Edge](https://arxiv.org/abs/2607.25155)
*Shivanshu Tripathi,Hamed Mohsenian-Rad,Maziar Raissi*

Main category: eess.SY

TL;DR: 提出VeraGrid-Agent，一种工具增强的LLM，通过自动编写模拟器输入、执行开源VeraGrid求解器并读取输出来回答配电网最优潮流问题；在150道多项选择题上，无工具准确率42.7%-49.3%，使用Agent后提升至97.3%-100.0%。


<details>
  <summary>Details</summary>
Motivation: 语言模型在回答复杂科学问题时依赖参数化知识常给出错误答案，需数值求解器处理D-OPF问题。

Method: VeraGrid-Agent: LLM自动生成模拟器输入，调用开源VeraGrid求解器，读取输出后回答；并构建VeraGrid-MCQ-150基准。

Result: 无工具准确率42.7%-49.3%；使用Agent后97.3%-100.0%。

Conclusion: 工具增强可显著提升LLM在科学问题上的准确性，剩余错误源于多步推理中解释错误而非模拟器失败。
...
### [276] [Integrated Eco-Driving and Powertrain Optimization for Hybrid Vehicles in Complex Urban Traffic](https://arxiv.org/abs/2607.25114)
*Milad Hasanzadeh,Amin Kargarian,Mehdi Farasat*

Main category: eess.SY

TL;DR: 本文开发了一个集成式城市生态驾驶规划框架，通过混合整数优化统一处理纵向运动、车道变换、交叉口行为及混合动力系统，实现了节能、安全且合规的驾驶计划。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常仅优化城市驾驶的某个方面（如纵向运动、车道变换等），而无法同时协调所有关键因素，因此需要一种集成方法。

Method: 提出了一个有限时域内的混合整数优化框架，同时建模纵向运动、车道占用与变换、安全约束、信号灯与无信号灯交叉口行为以及混合动力系统操作，包括车道可用性、限速、强制变道、发动机与电机功率、电池SOC、再生制动等。

Result: 与规则跟随、信号感知、超车启用和仅运动学优化的基线相比，该方法在保持可行、安全、舒适和符合交通规则的前提下提高了能量性能。

Conclusion: 集成规划框架能有效改善混合动力车辆的燃油经济性，同时确保驾驶计划的实际可执行性和合规性。
...
### [277] [Transient Synchronization Stability Analysis of SG-DFIG Parallel System Considering Complete LVRT Processes](https://arxiv.org/abs/2607.25204)
*Hongsheng Xu,Meng Zhan,Jia Hu,Huiyu Shang,Qingfeng Yue*

Main category: eess.SY

TL;DR: 本文针对含风电和同步机的混合系统，提出了考虑完整低电压穿越过程的四阶段暂态模型，并推导了统一广义摇摆方程，用于分析暂态同步稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对可再生能源设备完整LVRT过程中暂态同步稳定性的机理分析。

Method: 基于双馈感应电机的LVRT过程，划分四个暂态阶段，建立四阶段模型；推导统一的广义摇摆方程；改进等面积准则，考虑频率跳变和非线性阻尼。

Result: 大部分故障导致DFIG进入LVRT，系统呈现序贯切换特性；广义摇摆方程适用于不同阶段；改进等面积准则可用于评估暂态稳定性。

Conclusion: 该研究为混合系统的暂态同步稳定性提供了清晰的物理图景，并加强了与传统电力系统暂态稳定的联系。
...
### [278] [Progressive Replacement Sequence Planning for Laser-Enhanced BeiDou Navigation Constellations](https://arxiv.org/abs/2607.25235)
*Huan Yan,Juan A. Fraire,Ziqi Yang,Kanglian Zhao,Wenfeng Li,Jinjun Zheng,Ziyu Wang,Changgang Zheng*

Main category: eess.SY

TL;DR: 面向北斗三号卫星逐步替换的激光链路组网规划方法。


<details>
  <summary>Details</summary>
Motivation: 北斗三号早期卫星即将达到设计寿命，需要逐步替换，同时激光星间链路（LISL）能提升性能，但现有研究未从组网角度规划替换顺序。

Method: 将替换问题建模为面向激光-微波混合星座的网络感知发射序列规划问题；提出整数线性规划（ILP）模型评估候选动作的组网增益；引入优先级搜索启发式算法减少决策空间。

Result: 基于真实北斗三号星座的仿真表明，该方法能实现更优的混合网络演进，包括更早形成地面可接入的激光增强网络、更好利用激光链路资源、减少非锚定卫星数量、降低微波层等待时延。

Conclusion: 该框架可作为北斗未来演进的网络感知决策工具，替换序列可直接指导工程替换规划。
...
### [279] [MR-TGN: A Meta-Role Temporal Graph Network for Team-Level Intent Prediction in Multi-Agent Systems](https://arxiv.org/abs/2607.25316)
*Nagarani Brammanayagam,Devaprakash Muniraj*

Main category: eess.SY

TL;DR: 提出了元角色时序图网络(MR-TGN)框架，通过动态图建模和记忆增强的元角色学习机制，在无显式角色标注的情况下实现多智能体系统的集体意图预测。


<details>
  <summary>Details</summary>
Motivation: 集体意图预测面临复杂交互、动态合作结构、长期行为依赖以及角色隐式且动态变化等挑战。

Method: 构建动态图实体并集成时序记忆机制编码历史交互；提出记忆增强的元角色学习机制从智能体行为表征中推导潜在角色。

Result: 在多智能体场景中持续优于基线方法，实现了动态对抗环境下集体意图的早期有效预测。

Conclusion: MR-TGN框架能有效处理隐式角色和动态环境，在早期阶段准确预测集体意图。
...
TL;DR: 本文提出一种灵活动态运行域（DOE）方法，通过上下限约束减少可再生能源不确定性导致的弃电和负荷削减，同时保证电网电压合规。


<details>
  <summary>Details</summary>
Motivation: 现有DOE方法依赖用户上报期望功率且限制严格，导致实时运行中因不确定性产生不必要的发电和负荷削减。

Method: 采用灵活优化框架，计算每个产消者有功/无功功率上下限，将上报值限制在区间内，并权衡最优性与灵活性。

Result: 在改良澳大利亚低压配电网验证：相比非灵活DOE，所提方法显著降低弃电和总运行成本，同时维持电压在允许范围内。

Conclusion: 灵活DOE能有效应对分布式能源不确定性，减少经济代价并保障网络约束，具有实际应用价值。
...
### [281] [An Interaction Sensitivity Framework in Perturbation Space for the Converter-Driven Stability of Power Systems](https://arxiv.org/abs/2607.25334)
*Jihun Kook,Jung-Wook Park*

Main category: eess.SY

TL;DR: 提出扰动空间下的交互灵敏度框架，揭示高维电力系统中模态与状态间的扰动驱动交互重构机制。


<details>
  <summary>Details</summary>
Motivation: 传统参与因子分析局限于固定运行点，无法捕捉扰动后交互重构，导致稳定性分析存在盲区。

Method: 在扰动空间构建交互灵敏度框架，给出解析表达式与物理解释，揭示控制调整影响稳定性的因果关系。

Result: 实现扰动后模态-状态交互机制的可解释性分析，超越传统稳定性方法。

Conclusion: 该框架为高维动力网络扰动驱动交互重构提供通用分析基础，支撑复杂系统控制器设计。
...
### [282] [Critical slowing down for predicting controller induced loss of control in quadrotors](https://arxiv.org/abs/2607.25370)
*Jasper J. van Beers,Prashant Solanki,Erik-Jan van Kampen,Coen C. de Visser*

Main category: eess.SY

TL;DR: 提出基于临界减缓现象的通用预测方案，在四旋翼无人机数据上实现最高0.9秒的失控提前预警。


<details>
  <summary>Details</summary>
Motivation: 现有预测方法依赖系统模型或失事数据，缺乏通用性

Method: 利用临界减缓（CSD）现象提取早期预警信号，无需系统模型

Result: 预测时间提前0.9秒，检测精度超RNN方法，且无需失事数据
...
### [283] [A Hierarchical Stochastic Model Predictive Control Framework for Integrated Request-aware Charge Scheduling and Service Allocation](https://arxiv.org/abs/2607.25421)
*Mainak Dan,Arvind Easwaran*

Main category: eess.SY

TL;DR: 提出一种用于按需移动电动汽车（MoD-EVs）的统一控制机制，整合充电和服务分配，采用分段线性模型和混合整数线性规划模型预测控制（MILP-MPC），以降低充电成本和电池容量退化。


<details>
  <summary>Details</summary>
Motivation: 电动汽车充电受随机客户需求和动态电价影响，现有方法忽略电池非线性动态和健康优化。

Method: 采用分段线性模型和混合逻辑切换机制，构建多目标机会约束MILP-MPC框架，并采用分布式方法提高计算可扩展性。

Result: 相比基线方法（BAU和LC），充电成本降低，电池容量退化减少。

Conclusion: 集成控制机制在电池健康和经济性上优于现有方法。
...
### [284] [Integrated Framework for Long-term Elective Surgery Management under Uncertainty: From Strategic to Tactical Planning](https://arxiv.org/abs/2607.25482)
*Joao P. F. da Silva,Lucas Bortoletto,Rafael C. S. Schouery,Edilson F. Arruda,Fabricio F. Santos,Ana Paula C. I. Alves*

Main category: eess.SY

TL;DR: 论文提出一个集成框架，将战略排队控制和战术手术室调度结合，以在不确定性下管理择期手术等待名单。


<details>
  <summary>Details</summary>
Motivation: 公立医院面临手术等待名单管理挑战，现有模型常忽略患者到达和手术时长的不确定性，且孤立分配无法稳定等待名单。

Method: 框架包括：闭环控制策略决定每周期调度患者数、超时取消风险模型、混合整数线性规划求解手术室调度问题。

Result: 案例研究显示，该框架能稳定等待名单、控制取消风险、支持透明分配决策，且计算高效。

Conclusion: 集成战略和战术决策可有效管理择期手术，提高医院运营稳定性。
...
### [285] [To What Extent Can Inherent Communication Noise Guarantee Privacy in Distributed Cooperative Control?](https://arxiv.org/abs/2607.25564)
*Yuwen Ma,Sarah Spurgeon,Tao Li,Boli Chen*

Main category: eess.SY

TL;DR: 提出了一种利用通信噪声作为天然隐私机制的多智能体系统差分隐私分布式协同控制方案，实现有限时域LQR编队跟踪。


<details>
  <summary>Details</summary>
Motivation: 探索通信噪声能否作为自然隐私保护机制，避免主动注入人工噪声的局限性。

Method: 建立包含发射器扰动、接收噪声、路径损耗和对数正态阴影的物理通信噪声模型，设计基于状态差异的分布式有限时域LQR控制器，提出权重比相邻隐私定义。

Result: 在无需人工噪声下实现有界累积(ε,δ)-差分隐私保证，跟踪误差几乎必然收敛至有限随机极限，期望有界。

Conclusion: 通信噪声可有效实现隐私保护，且隐私-性能权衡可通过系统设计调节。
...
### [286] [A Minimal Dynamical Model for Incubation-Outbreak Transitions in Social Norm Diffusion](https://arxiv.org/abs/2607.25586)
*Run Wang,Leonardo Cianfanelli,Giacomo Como,Wenjun Mei*

Main category: eess.SY

TL;DR: 提出一个三状态社会规范扩散的简化动力学模型，揭示出“潜伏-爆发”的非平凡动态模式。


<details>
  <summary>Details</summary>
Motivation: 探索社会规范扩散中从少量支持到突然爆炸性转变的内在机制。

Method: 构建非支持者、沉默支持者、发声倡导者三状态模型，并进行平衡态、稳定性、收敛性和相变的完整解析分析。

Result: 发现初始小规模采纳后存在长潜伏期，随后内源性爆发的非线性机制，并给出潜伏期估计。

Conclusion: 社会突变可由内部互动而非外部冲击驱动，该模型提供了动力学解释。
...
### [287] [How haptic feedback enables human group synchronization](https://arxiv.org/abs/2607.25692)
*Angelo Di Porzio,Etienne Burdet,Marco Coraggio*

Main category: eess.SY

TL;DR: 触觉反馈可以诱导和增强群体运动同步性，与视觉反馈结合效果更优。


<details>
  <summary>Details</summary>
Motivation: 视觉交互在同步中受限于延迟、遮挡和注意力资源，探索触觉通信能否弥补这些不足。

Method: 四个四人组进行手腕屈伸振荡，通过虚拟弹性带连接，比较触觉、视觉、触觉+视觉和无反馈四种条件。

Result: 微弱触觉耦合即可诱导同步，触觉+视觉的同步性更高更稳定，且触觉增加协调频率和运动平滑度。

Conclusion: 研究为设计群体协作中的触觉交互协议奠定了基础。
...
### [288] [Joint identification of permanent magnet synchronous machine and inverter](https://arxiv.org/abs/2607.25739)
*Giulio Montecchio,Sven Reimann,Benjamin Hartmann,Maximilian Manderla,Jan Achterhold,Daniel Görges*

Main category: eess.SY

TL;DR: 提出一种联合识别磁链图和逆变器非线性模型的方法，使用张量积样条模型，提高离线磁链辨识精度。


<details>
  <summary>Details</summary>
Motivation: 解决控制器输出电压受逆变器非线性干扰，导致传统离线磁链辨识不准的问题。

Method: 在dq坐标系下，用张量积样条表示磁链与电流的静态关系，并联合辨识逆变器简化模型。

Result: 通过FEM仿真和实验台数据验证，联合辨识能显著提升磁链模型精度。

Conclusion: 无需先验识别逆变器参数，即可从指令电压中准确辨识磁链图。
...
### [289] [A Hierarchical Optimisation Framework for Integrated Electric-Hydrogen-Transport Systems](https://arxiv.org/abs/2607.25776)
*Fulong Yao,Yiming Xu,Liana Cipcigan,Maurizio Albano,Naeima Hamed,Nima Valizadeh,Omer Rana*

Main category: eess.SY

TL;DR: 本文提出一种用于集成电动-氢能交通系统（EHTS）的分层优化框架，结合求解器无关的贪婪启发式算法和深度强化学习，实现车辆调度与多能源调度的协调，并通过案例验证了有效性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 电动汽车和氢能汽车在交通系统中的日益普及导致车辆调度与多能源调度之间强耦合，带来显著运营挑战。

Method: 提出分层框架：上层采用求解器无关的贪婪启发式算法实现实时调度；下层基于深度强化学习优化电池、氢罐及光伏分配，最小化运营成本。

Result: 案例研究、对比、消融和泛化分析表明，所提框架有效且稳健，学到的调度策略无需重训练即可适应多种交通需求场景。

Conclusion: 该框架具备实用部署的强泛化能力，能解决EHTS中车辆与能源调度的耦合问题。
...
### [290] [Horizon Selection in Physics-Enhanced Neural ODEs: Theoretical Insights and Flux Linkage Application](https://arxiv.org/abs/2607.25804)
*Giulio Montecchio,Benjamin Hartmann,Sven Reimann,Maximilian Manderla,Jan Achterhold,Daniel Görges*

Main category: eess.SY

TL;DR: 提出利用长训练积分视界改进物理增强神经ODE的参数估计与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 训练积分视界对物理增强神经ODE的物理参数估计偏差和泛化有重要影响。

Method: 基于经典非线性系统辨识理论，提出长视界训练框架以降低物理参数偏差、提取残差信息并作为正则化器。

Result: 在永磁同步电机模型中，成功联合估计磁链图和电阻。

Conclusion: 长训练视界能有效提升物理增强神经ODE的参数估计精度和模型泛化性能。
...
### [291] [SplatStream: Fine Granular Scalable Gaussian Splatting for Adaptive 3D Scene Streaming](https://arxiv.org/abs/2607.25971)
*Muhammad Talha,William Gordon,Sajid Umair,Anique Akhtar,Joel Jung*

Main category: eess.IV

TL;DR: 提出了SplatStream框架，通过分层编码、帧间预测和重要性度量实现动态3D高斯散列的细粒度可扩展流式传输，支持自适应低延迟传输。


<details>
  <summary>Details</summary>
Motivation: 动态3D高斯散列表示尺寸大且存在帧间冗余，导致自适应流式传输面临挑战。

Method: 将GS场景分解为质量层和分辨率层，采用跨层预测编码、B帧时间可扩展、轻量级Transformer预测器、基于体积-不透明度的包化方法，并映射到MPEG-DASH结构。

Result: 实现细粒度可扩展性、低延迟传输，在带宽变化条件下保持高质量渲染。

Conclusion: SplatStream框架在动态3D场景的高效传输中效果显著。
...
### [292] [Analysis of the Shortcut Learning and Clever Hans Effect in CNN based ECG Image Classification](https://arxiv.org/abs/2607.25117)
*Abhay Kumar Pathak,Mrityunjay Chaubey,Manjari Gupta,Deepti Mishra*

Main category: eess.IV

TL;DR: 研究深度学习心电图像分类器是否依赖非生理性视觉捷径（如报告布局、元数据、伪影）而非波形形态，通过六种特征集和解释性分析揭示Clever Hans行为。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在ECG图像分类中可能因黑箱特性依赖非生理特征而非波形形态，导致临床信任和可解释性不足。

Method: 构建六种特征集（原始图、裁剪波形、掩蔽元数据、红箭头伪影、对比增强、高斯模糊），计算捷径保留分数、预测一致性、置信度差异，并使用集成梯度和遮挡敏感性测试分析模型归因。

Result: 分类性能在去除波形信息后仍部分保持，且模型归因常指向非临床伪影（如红箭头、对比度变化），表明存在捷径学习。

Conclusion: ECG图像分类器可能学习报告布局、元数据或人工标记等非生理特征，而非波形形态，需加强模型可解释性和数据质量管控。
