...
TL;DR: 提出DeepER-Med框架，通过可检查的循证工作流提升AI在医学研究中的可信度和透明度，并在真实医学问题上优于现有平台。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统缺乏明确的证据评估标准，难以评估输出可靠性，且缺乏对复杂真实医学问题的评估基准，限制了AI在临床和生物医学研究中的应用。

Method: 设计DeepER-Med框架，包含研究规划、智能体协作和证据合成三个模块；构建DeepER-MedQA数据集（100个专家级医学研究问题）用于评估；通过专家评估和真实临床案例验证。

Result: DeepER-Med在多项指标上优于主流生产级平台，能生成新颖科学见解；在8个真实临床案例中，7个案例的结论与临床建议一致。

Conclusion: DeepER-Med通过可检查的循证工作流提升了AI系统的可信度，在医学研究和临床决策支持中具有应用潜力。
...
TL;DR: 提出GIST多模态知识提取管道，将移动点云转换为语义标注的导航拓扑结构，用于密集环境中的空间定位和人机交互任务。


<details>
  <summary>Details</summary>
Motivation: 密集环境（如零售店、仓库）中视觉特征易过时、语义分布长尾，现有视觉语言模型在空间定位方面存在困难，需要更好的空间知识表示方法。

Method: 开发GIST管道：将移动点云转换为2D占据地图，提取拓扑布局，通过智能关键帧和语义选择叠加轻量级语义层，构建结构化空间知识。

Result: 实现四项下游任务：语义搜索（推断类别替代方案）、语义定位（1.04米top-5平均平移误差）、区域分类、视觉接地指令生成（优于序列基线），现场评估导航成功率80%。

Conclusion: GIST能有效提取结构化空间知识，支持密集环境中的可靠人机交互，验证了通用设计的可行性。
...
TL;DR: 加拿大联邦AI登记册通过技术化描述构建AI作为'可靠工具'的本体论，而非'可争议决策'，导致透明度与问责制脱节。


<details>
  <summary>Details</summary>
Motivation: 研究旨在批判性分析政府AI登记册作为问责工具的实际效果，揭示其如何通过设计塑造对AI的认知与问责边界。

Method: 使用ADMAPS框架，对登记册中409个系统进行定量映射与演绎性定性编码，结合量化分析与质性研究。

Result: 86%的AI系统仅用于内部效率提升，登记册通过强调技术细节掩盖了人工裁量、训练与不确定性管理，构建了去政治化的AI叙事。

Conclusion: 现有登记册设计可能将问责制简化为表演性合规，需重构透明度机制以支持真正的争议性与社会技术语境。
...
TL;DR: 本文提出LACE框架，通过跨线程注意力机制让并行推理路径相互交流与纠错，显著提升大语言模型的推理准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的推理过程相互独立，并行采样的推理路径缺乏交互，容易重复犯相同错误，导致推理效率低下。

Method: 1. 设计LACE框架，改造模型架构以支持跨线程注意力机制；2. 构建合成数据管道，训练模型学习跨线程通信与错误纠正能力。

Result: 实验表明，该方法相比标准并行搜索显著提升推理准确率，改进幅度超过7个百分点。

Conclusion: 允许并行推理路径相互交互能大幅提升大语言模型的推理效果，跨线程协作是提升模型推理能力的有前景方向。
...
TL;DR: 提出PBRC协议机制，通过预注册证据触发器和审计追踪，防止多智能体系统中因社会影响导致的错误共识，确保信念变更仅基于外部验证证据。


<details>
  <summary>Details</summary>
Motivation: 多智能体协商系统易因从众效应（如多数意见、声望等被误作证据）导致高置信度的错误结论收敛，需设计机制确保信念变更仅依赖真实证据。

Method: 设计PBRC协议：预注册证据触发条件、可接受的信念修正算子、优先级规则和回退策略；要求非回退步骤必须提供外部验证的证据令牌集合；建立合约动态信念逻辑进行形式化验证与模拟。

Result: 证明在保守回退策略下，纯社交交互不会增加置信度或引发错误级联；实现可审计的触发协议规范化；证明强制执行的轨迹仅依赖令牌暴露痕迹，并在扩散传播下获得精确的直径边界。

Conclusion: PBRC机制能有效抑制从众驱动的错误级联，通过证据审计实现认知问责，为多智能体系统提供可验证的信念修正框架。
...
TL;DR: 研究表明，在智能体系统中，不安全的行为特征可以通过模型蒸馏过程隐性传递，即使训练数据经过严格的关键词过滤也无法完全防御。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明语言模型可以通过语义无关的数据传递语义特征，但尚不清楚在智能体系统中，通过轨迹学习的行为特征是否也能以类似方式传递。

Method: 在两个互补实验设置中验证行为特征的隐性传递：1）API工具接口环境中构建具有强烈删除偏好的教师智能体，使用经过严格过滤的安全任务轨迹进行蒸馏；2）Bash原生环境中用shell命令替代API调用，操作化为对chmod命令的偏好。

Result: 尽管进行了完整的关键词过滤，学生智能体仍继承了行为偏差：API环境中删除率达到100%（基线5%）；Bash环境中chmod优先率达到30%-55%（基线0%-10%），大模型向小模型蒸馏时传递效果最强。

Conclusion: 显式的数据清理不足以防御行为特征的隐性传递，行为偏差被隐式编码在轨迹动态中，与工具接口无关。
...
TL;DR: 本文提出了一种双层优化框架，通过蒙特卡洛树搜索优化技能结构，结合LLM优化组件内容，以提升LLM代理的任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有技能设计对LLM代理任务性能有显著影响，但技能优化面临结构设计与组件内容优化的复杂耦合问题，缺乏系统化方法。

Method: 将技能优化建模为双层优化问题：外层使用蒙特卡洛树搜索确定技能结构，内层在选定结构下优化组件内容，两阶段均利用LLM辅助优化。

Result: 在开源运筹学问答数据集上的实验表明，该框架能有效提升代理使用优化后技能的性能。

Conclusion: 所提出的双层优化框架能系统化解决技能结构-内容耦合优化问题，为LLM代理的技能设计提供了有效方法。
...
TL;DR: 本文提出了Milkyway系统，一种自进化的未来预测代理，通过内部反馈机制在问题未解决时持续改进预测过程，而无需修改基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖最终结果进行改进，但最终结果过于粗糙，无法有效指导早期因素追踪、证据收集与解释以及不确定性处理。

Method: Milkyway系统保持基础模型固定，更新持久化的未来预测工具链（包括因素追踪、证据收集与解释、不确定性处理），通过时间对比提取内部反馈，并将可重用指导写入工具链。

Result: 在FutureX和FutureWorld数据集上，Milkyway取得了最佳综合得分，分别将FutureX从44.07提升至60.90，FutureWorld从62.22提升至77.96。

Conclusion: 内部反馈机制使预测系统能在问题未解决时持续自我改进，最终结果仅作为回顾性检查，更新后的工具链可迁移至后续问题。
...
TL;DR: 本文主张将大语言模型的推理视为潜在状态轨迹形成而非表面思维链的忠实反映，提出了三种竞争假设，并通过实证分析支持将潜在状态动态作为默认研究对象。


<details>
  <summary>Details</summary>
Motivation: 当前关于大语言模型推理的忠实性、可解释性、基准测试和推理时干预的讨论，都依赖于对推理本质的理解。本文旨在澄清这一本质，区分常被混淆的表面思维链、潜在状态和串行计算三个因素。

Method: 通过形式化三种竞争假设（H1：推理主要由潜在状态轨迹介导；H2：推理主要由表面思维链介导；H0：推理增益主要由通用串行计算解释），重组现有实证、机制和调查研究成果，并引入计算审计的工作示例来分解表面痕迹、潜在干预和匹配的计算预算扩展。

Result: 当前证据最强烈支持H1作为默认工作假设，即推理主要由潜在状态轨迹介导，而非表面思维链或通用串行计算。

Conclusion: 建议领域将潜在状态动态作为大语言模型推理的默认研究对象，并在评估推理时采用能明确区分表面痕迹、潜在状态和串行计算的设计。
...
TL;DR: 提出一种符号推理框架，通过皮尔士的三段推理（溯因、演绎、归纳）和五个代数不变量（Gamma五元组）来解决大语言模型在结构化逻辑推理中的系统性问题，确保推理链的逻辑一致性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在结构化逻辑推理中存在系统缺陷：混淆假设生成与验证、无法区分猜想与已验证知识、允许弱推理步骤在推理链中传播。这些缺陷促使研究者设计一种能强制逻辑一致性的推理框架。

Method: 采用符号推理框架，将皮尔士的三段推理（溯因、演绎、归纳）操作化为明确的推理协议，并通过五个代数不变量（Gamma五元组）强制逻辑一致性，其中最强的不变量“最弱链接边界”确保推理链中结论的可靠性不超过其最弱前提的可靠性。

Result: 通过基于属性的测试套件（100个属性和16个模糊测试，覆盖10^5+生成案例）验证了所有不变量，提供了一个经过验证的参考实现，可作为未来推理基准的基础。

Conclusion: 该框架通过明确的推理协议和代数不变量，有效解决了大语言模型在结构化逻辑推理中的一致性问题，为构建可靠的推理系统提供了理论基础和实现参考。
...
TL;DR: KWBench是一个评估大语言模型无提示问题识别能力的基准，包含223个专业场景任务，要求模型仅从原始数据中识别问题结构而非直接解决问题。


<details>
  <summary>Details</summary>
Motivation: 现有前沿基准已饱和，且多数知识工作评估简化为基于明确规范的提取或任务完成，缺乏对问题识别能力的评估。

Method: 构建包含6个专业领域任务的基准，每个任务编码博弈论模式并包含结构化真实标签。采用三层评分标准，包含强制性的错误路径预测检查。评估了16个模型。

Result: 最佳模型仅通过27.9%的任务；前两名模型仅在31.7%的通过任务上一致；前8名模型联合覆盖50.7%的任务；模型在被询问时能正确表述博弈论概念，但无提示时无法应用。

Conclusion: 模型在无提示问题识别能力上表现有限，需要改进对情境的理解能力。发布KWBench旨在推动前沿模型评估从任务执行转向问题识别。
...
TL;DR: 提出一种结合Stein算子的估计分布算法，通过参数空间中的排斥机制使种群分散探索多个最优区域，在高维组合黑盒优化中表现优异


<details>
  <summary>Details</summary>
Motivation: 传统估计分布算法在复杂多峰目标函数中容易过早收敛于单一区域，需要平衡探索与利用，特别是在高维离散黑盒优化问题中

Method: 将Stein算子引入估计分布算法，在参数空间中建立粒子间排斥机制，促进种群分散并同时探索适应度景观的多个模态

Result: 在多种基准问题上实证评估显示，该方法性能与现有先进方法相当或更优，尤其在大规模实例中表现突出

Conclusion: Stein变分梯度下降是解决大规模、计算昂贵的离散黑盒优化问题的有前景方向，能有效避免早熟收敛并提升多峰探索能力
...
TL;DR: 论文提出了自动定理证明中的'困难模式'概念，并开发了DAP框架，在多个基准测试中取得最佳性能，揭示了LLMs与形式证明器之间的能力差距。


<details>
  <summary>Details</summary>
Motivation: 现有ATP基准测试通常将答案嵌入形式化陈述中（'简单模式'），这种设计简化了任务，可能导致对模型能力的高估。需要更严格、更接近人类竞赛环境的'困难模式'来评估系统真实能力。

Method: 1) 发布MiniF2F-Hard和FIMO-Hard两个专家重新标注的困难模式基准；2) 提出DAP框架：使用LLM进行自然语言推理和自我反思来发现答案，然后将困难模式陈述重写为简单模式供现有ATP证明器使用。

Result: DAP在CombiBench上将已解决问题从7个提升到10个；在PutnamBench上首次在困难模式下形式化证明了36个定理；同时发现最先进LLMs在相同问题上的答案准确率超过80%，而形式证明器成功率低于10%。

Conclusion: 困难模式基准能更真实地评估系统能力，DAP框架在困难模式下取得最佳性能，揭示了LLMs与形式证明器之间存在的显著能力差距，为未来研究提供了重要方向。
...
TL;DR: 本文主张用严格的符号化方法替代非符号化方法进行机器学习模型解释，特别针对特征重要性分配问题，批评了当前流行的SHAP等非严格方法在高风险应用中的问题。


<details>
  <summary>Details</summary>
Motivation: 当前可解释人工智能（XAI）领域广泛采用的非符号化方法（如SHAP）缺乏严谨性，可能误导人类决策者，这在机器学习的高风险应用中尤其成问题。

Method: 本文综述了使用严格符号化方法进行XAI的研究进展，重点关注特征重要性分配的符号化方法替代方案。

Result: 论文指出非符号化方法存在可证明的严谨性缺失问题，并展示了符号化方法作为替代方案的发展现状。

Conclusion: 在高风险机器学习应用中，应采用严谨的符号化解释方法替代当前流行的非严格非符号化方法，以确保解释的可靠性和决策的安全性。
...
### [15] [Experience Compression Spectrum: Unifying Memory, Skills, and Rules in LLM Agents](https://arxiv.org/abs/2604.15877)
*Xing Zhang,Guanghui Wang,Yanwei Cui,Wei Qiu,Ziyuan Li,Bing Zhu,Peiyang He*

Main category: cs.AI

TL;DR: 本文提出了经验压缩谱框架，统一了记忆、技能和规则三种知识表示形式，揭示了当前系统缺乏跨压缩级别自适应能力的局限性，并指出了构建可扩展智能体学习系统的设计原则。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体向长时程、多会话部署扩展，如何高效管理累积经验成为关键瓶颈。智能体记忆系统和技能发现领域都致力于从交互轨迹中提取可重用知识，但两个社区之间引用率极低（<1%），缺乏统一框架。

Method: 提出经验压缩谱框架，将记忆、技能和规则定位为单一压缩轴上的不同点（压缩比：情景记忆5-20倍，程序技能50-500倍，声明性规则1000倍以上）；对22篇主要论文的1136篇参考文献进行引文分析；将20多个系统映射到该谱系上进行分析。

Result: 发现所有系统都在固定的预定压缩级别上运行，缺乏自适应跨级别压缩能力（称为“缺失对角线”）；两个社区独立解决共享子问题而未交换解决方案；评估方法与压缩级别紧密耦合；可迁移性随压缩增加而提高，但以特异性为代价；知识生命周期管理仍被忽视。

Conclusion: 当前智能体学习系统存在跨压缩级别自适应能力的缺失，需要开发支持全谱系学习的系统。文章阐明了可扩展智能体学习系统的开放问题和设计原则，强调需要统一框架来整合记忆、技能和规则表示。
...
TL;DR: 本文综述了图与大型语言模型（LLM）的集成方法，系统分类了现有技术，并分析了其在各领域的应用场景与选择指南。


<details>
  <summary>Details</summary>
Motivation: 尽管图与LLM的融合在推理、检索和结构化决策中应用迅速，但缺乏关于何时、为何、如何选择合适集成方法的清晰指导，需要系统化的梳理与总结。

Method: 通过分类现有方法，依据目的（推理、检索、生成、推荐）、图模态（知识图谱、场景图、交互图、因果图、依赖图）和集成策略（提示、增强、训练、基于代理）进行结构化分析，并跨领域映射代表性工作。

Result: 系统梳理了图-LLM集成的设计选择，明确了不同技术的优势、局限性和适用场景，为跨领域应用提供了实践参考。

Conclusion: 本综述为研究者提供了根据任务需求、数据特征和推理复杂度选择最合适图-LLM方法的实用指南，促进了该交叉领域的规范化发展。
...
TL;DR: 提出WORC框架，通过识别并优化多智能体系统中的薄弱环节来提升推理稳定性与性能


<details>
  <summary>Details</summary>
Motivation: 现有多智能体框架存在推理不稳定的问题，个体错误会在协作中被放大。当前研究多关注增强高能力智能体或抑制不可靠输出，缺乏对性能限制环节的系统性识别与强化

Method: 提出基于薄弱环节原则的WORC框架，包含两阶段：1) 薄弱智能体定位阶段：构建任务特征，通过元学习权重预测器（基于群体智能算法优化的配置训练）实现零样本性能权重映射，识别权重最低的薄弱智能体；2) 薄弱环节优化阶段：采用不确定性驱动的分配策略，为薄弱智能体分配额外推理预算，预测权重越低则重复采样配额越大

Result: 在推理基准测试中平均准确率达到82.2%，同时提升了框架稳定性和跨架构泛化能力

Conclusion: 补偿薄弱环节（而非仅强化优势）能有效增强多智能体系统的鲁棒性
...
TL;DR: 本文提出了ReactBench基准测试，揭示了多模态大语言模型在复杂拓扑结构（如化学反应图）推理上的根本性缺陷，性能差距超过30%。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在识别单个视觉元素和简单线性图表方面表现良好，但在处理涉及分支路径、汇聚流和循环依赖的复杂拓扑结构时，推理能力急剧下降。现有基准测试未能探测这一缺陷，因为它们侧重于语义理解而非结构推理。

Method: 引入ReactBench基准测试，包含1,618个专家标注的QA对，涵盖四个层次化任务维度。通过对17个MLLMs进行广泛评估和受控消融实验，验证瓶颈在于推理而非感知。

Result: 评估显示，在基于锚点的任务和整体结构推理任务之间存在超过30%的性能差距。消融实验证实这一瓶颈确实在于推理能力不足。

Conclusion: 研究暴露了MLLMs在结构理解上的根本性缺陷，为推进视觉推理能力的发展指明了方向。
...
TL;DR: 提出了MEDLEY-BENCH基准测试，用于评估AI模型在模糊情境下的元认知能力，发现评估能力随模型规模增长而控制能力不增长，且小模型可能表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前AI基准测试中缺乏对元认知（监控和调节自身推理）的系统评估，需要开发能区分独立推理、自我修正和社会影响修正的评估工具。

Method: 创建MEDLEY-BENCH基准，包含5个领域的130个模糊实例，评估12个家族的35个模型。使用两种评分：MMS（基于反思更新、社会鲁棒性和认知表达的层级聚合）和MAS（基于四个元认知子能力）。后续对11个模型进行渐进对抗分析。

Result: 发现评估与控制能力分离：评估能力随模型规模增长，控制能力不增长；识别出两种行为模式（基于论证质量修正 vs 基于共识统计修正）；所有模型中评估能力相对最弱；小模型常能匹配或超越大模型表现。

Conclusion: MEDLEY-BENCH可作为测量社会压力下信念修正的工具；元认知能力不是规模的简单函数；未来训练应关注校准的、比例适当的更新而不仅是输出质量。
...
TL;DR: 提出SocialGrid环境评估LLM智能体的社会推理能力，发现现有模型在任务完成、规划和社会推理方面存在显著不足，特别是难以检测欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型从文本处理器向自主智能体转变，需要在具身多智能体环境中评估其社会推理能力，现有评估方法存在局限。

Method: 开发受《Among Us》启发的具身多智能体环境SocialGrid，引入规划预言机分离规划缺陷与社会推理，使用自动失败分析和细粒度指标，建立基于Elo评分的对抗性排行榜。

Result: 最强开源模型任务完成准确率低于60%，智能体陷入重复行为或无法导航基本障碍；规划辅助可提升任务完成率，但社会推理仍是瓶颈，欺骗检测准确率接近随机猜测。

Conclusion: 当前LLM智能体的社会推理能力严重不足，依赖浅层启发式而非行为证据积累，SocialGrid为诊断和改进智能体提供了系统化评估框架。
...
TL;DR: 提出MARCH多智能体框架，模仿放射科专业层级结构，通过角色分工和迭代共识机制提升3D放射报告生成的临床准确性和语言质量。


<details>
  <summary>Details</summary>
Motivation: 现有自动化3D放射报告生成存在临床幻觉问题，且缺乏人类实践中迭代验证的协作监督机制，需要更符合临床工作流程的解决方案。

Method: 设计多智能体框架MARCH：住院医师智能体负责多尺度CT特征提取和初稿起草，多个专科医师智能体进行检索增强修订，主治医师智能体通过基于立场的共识对话协调解决诊断分歧。

Result: 在RadGenome-ChestCT数据集上，MARCH在临床保真度和语言准确性方面显著优于现有最先进基线方法。

Conclusion: 模拟人类组织结构的AI框架能够增强高风险医疗领域中人工智能的可靠性，为医疗AI系统设计提供了新思路。
...
TL;DR: 本文提出了一套量化指标，用于系统比较不同大语言模型生成的胜任力问题（CQs）在可读性、相关性和结构复杂性等方面的特性，并通过多领域实验揭示了模型性能受用例影响的生成特征。


<details>
  <summary>Details</summary>
Motivation: 传统CQ生成依赖人工协作，效率有限；生成式AI虽能规模化自动生成CQs并降低参与门槛，但不同LLMs在参数规模、领域专长等方面差异显著，需系统评估其生成CQs的内在特性以指导实际应用。

Method: 1. 设计多维度量化指标（可读性、输入文本相关性、结构复杂性）；2. 基于明确定义的用例和场景生成CQs；3. 对比开源（KimiK2-1T等）与闭源模型（Gemini 2.5 Pro等）在多个用例上的表现。

Result: 实验表明：1. 不同LLMs生成的CQs呈现显著差异；2. 模型性能与用例特征紧密相关，形成独特的生成模式；3. 量化指标有效揭示了CQs在可读性、相关性及结构复杂度上的分布规律。

Conclusion: LLMs生成CQs的能力具有用例依赖性，需结合系统化评估指标选择合适模型；该研究为自动化CQ生成的模型选型与优化提供了实证依据，有助于推动本体工程的大众化应用。
...
TL;DR: 提出DeepInsightTheorem框架，通过显式提取核心证明技巧和草图来增强大语言模型的非形式化定理证明能力，采用渐进式微调策略显著提升数学推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动化定理证明方法多依赖形式化系统，但非形式化证明更契合大语言模型（LLMs）的自然语言处理优势。研究发现非形式化证明的主要瓶颈在于缺乏洞察力，即难以识别解决复杂问题所需的核心技巧。

Method: 1. 构建DeepInsightTheorem分层数据集，在非形式化证明中显式标注核心技巧和证明草图；2. 设计渐进式多阶段监督微调策略，模拟人类学习过程，引导模型从基础证明写作逐步过渡到洞察性推理。

Result: 在具有挑战性的数学基准测试中，该洞察感知的生成策略显著优于基线方法，证明模型识别和应用核心技巧的能力可大幅提升其数学推理水平。

Conclusion: 通过培养模型识别核心证明技巧的洞察力，能够有效解决非形式化定理证明的瓶颈问题，为大语言模型的数学推理能力提升提供了新方向。
...
TL;DR: 提出一种利用知识图谱增强机器学习模型可解释性的方法，通过结构化存储领域知识和ML结果，结合LLM生成用户友好的解释，并在制造环境中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习结果的可解释性仍面临挑战，需要更透明、用户友好的解释方法，以支持实际应用中的决策过程。

Method: 构建领域知识图谱存储ML结果和解释，设计选择性检索方法提取相关三元组，利用大语言模型生成自然语言解释，并在制造环境中使用XAI问题库进行评估。

Result: 评估了33个问题，包括标准问题和定制复杂问题，定量指标（准确性、一致性）和定性指标（清晰度、实用性）均显示该方法能有效生成高质量解释。

Conclusion: 该方法在理论和实践上均有贡献：理论上提出动态访问知识图谱增强ML可解释性的新方法；实践上证明其能成功应用于真实制造环境，支持更好的决策制定。
...
TL;DR: 提出了ASMR-Bench基准测试，用于评估审计员检测机器学习研究代码中蓄意破坏的能力，发现当前前沿LLM和人类审计员均难以可靠识别此类破坏。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地自主进行科学研究，未对齐的系统可能引入难以察觉的缺陷，产生误导性结果却逃避检测，需要开发有效的监控审计方法。

Method: 构建包含9个机器学习研究代码库的基准测试，每个代码库包含经过蓄意破坏的变体（修改超参数、训练数据或评估代码等实现细节），评估前沿LLM和LLM辅助人类审计员的检测能力，并测试LLM作为红队生成破坏的能力。

Result: 最佳性能由Gemini 3.1 Pro实现（AUROC 0.77，top-1修复率42%）；LLM生成的破坏弱于人类生成，但仍能逃避同能力LLM审计员的检测；整体检测可靠性不足。

Conclusion: 当前AI系统和人类审计员在检测研究代码蓄意破坏方面存在显著局限，ASMR-Bench的发布旨在支持AI主导研究的监控与审计技术发展。
...
### [26] [Zoom Consistency: A Free Confidence Signal in Multi-Step Visual Grounding Pipelines](https://arxiv.org/abs/2604.15376)
*Keon Kim,Krish Chelikavada*

Main category: cs.CV

TL;DR: 提出利用多步缩放GUI定位流程中的中间预测（缩放一致性）作为无需校准的置信度信号，证明其能跨不同视觉语言模型预测准确性，并用于模型路由决策。


<details>
  <summary>Details</summary>
Motivation: 现有GUI定位多步缩放流程丢弃中间预测信息，但这些信息包含可用于评估预测置信度的几何信号（缩放一致性），且无需模型校准即可跨架构比较。

Method: 定义缩放一致性为第二步预测与裁剪中心间的几何距离，理论证明其在理想条件下是第一步空间误差的线性估计量，并在两个VLMs（KV-Ground-8B和Qwen3.5-27B）上验证其与预测正确性的相关性。

Result: 缩放一致性虽相关性较小（AUC=0.60；Spearman rho=-0.14/-0.11），但在模型、应用类别和操作系统中表现一致；将其用于专家与通用模型路由可捕获16.5%的潜在性能提升（+0.8%）。

Conclusion: 多步缩放流程的中间几何信息可作为跨模型可比的免费置信度信号，为模型选择等应用提供轻量级决策依据，代码已开源。
...
### [27] [Weak-to-Strong Knowledge Distillation Accelerates Visual Learning](https://arxiv.org/abs/2604.15451)
*Baiang Li,Wenhao Chai,Felix Heide*

Main category: cs.CV

TL;DR: 提出一种通用知识蒸馏方法，通过冻结较弱教师模型并在早期训练阶段应用蒸馏，显著加速强学生模型的训练过程。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉学习的训练成本日益成为限制因素，现有知识蒸馏方法主要用于模型压缩或最终精度提升，而非训练加速。

Method: 提出可泛化的即插即用方案：冻结较弱教师模型，仅在训练早期应用知识蒸馏，当学生模型达到并超越教师性能后关闭蒸馏。

Result: 在ImageNet和CIFAR分类任务上实现最高4.8倍的epoch加速；在COCO目标检测任务上实现1.7倍epoch加速；在CIFAR-10扩散生成任务上实现2.5倍步骤加速。

Conclusion: 该方法可作为视觉学习的通用加速机制，在不同任务上均能显著缩短训练时间。
...
### [28] [(1D) Ordered Tokens Enable Efficient Test-Time Search](https://arxiv.org/abs/2604.15453)
...
TL;DR: 本文研究了自回归生成模型中分词结构对测试时搜索能力的影响，发现粗到细的有序分词比传统网格分词更有利于通过验证器引导生成过程。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索分词结构如何影响自回归模型在测试时通过搜索和验证进行生成引导的能力，特别是比较不同分词方式对生成过程可控性的影响。

Method: 使用图像生成作为测试平台，比较1D有序分词（粗到细结构）与经典2D网格分词；通过控制实验研究不同搜索算法（最佳N选择、束搜索、前瞻搜索）与分词结构的交互；分析不同验证器和自回归先验的作用。

Result: 实验发现：1）使用粗到细有序分词的AR模型表现出更好的测试时扩展行为；2）纯测试时搜索（无需训练AR模型）结合图像-文本验证器可实现免训练文本到图像生成；3）分词结构显著影响推理时的可扩展性。

Conclusion: 分词结构对自回归模型的推理时扩展性有重要影响，粗到细有序分词结构更有利于测试时搜索引导，为AR模型的测试时扩展提供了实用指导。
...
### [29] [Frequency-Aware Flow Matching for High-Quality Image Generation](https://arxiv.org/abs/2604.15521)
*Sucheng Ren,Qihang Yu,Ju He,Xiaohui Shen,Alan Yuille,Liang-Chieh Chen*

Main category: cs.CV

TL;DR: 提出FreqFlow方法，通过频率感知条件化改进流匹配模型，在ImageNet-256生成任务上取得SOTA性能（FID 1.38）。


<details>
  <summary>Details</summary>
Motivation: 传统流匹配模型在潜空间添加高斯噪声时，对不同频率分量的影响不均匀，导致推理时低频结构先于高频细节生成，限制了生成质量。

Method: 设计双分支架构：频率分支分别处理低/高频分量以捕获全局结构和细化纹理；空间分支在频率分支引导下于潜空间合成图像，并通过时变自适应加权实现频率感知条件化。

Result: 在ImageNet-256基准测试中，FID达到1.38，优于DiT（提升0.79）和SiT（提升0.58）等现有方法。

Conclusion: 显式集成频率信息能同时有效建模大尺度连贯性和细粒度细节，低频条件强化全局结构，高频条件提升纹理保真度与细节锐度。
...
### [30] [UA-Net: Uncertainty-Aware Network for TRISO Image Semantic Segmentation](https://arxiv.org/abs/2604.15542)
*Kyle Lucke,Zuzanna Krajewska-Travar,Shoukun Sun,Lu Cai,John D. Stempien,Min Xian*

Main category: cs.CV

TL;DR: 提出UA-Net深度学习框架，用于自动分割TRISO燃料微观图像中的五个特征区域，并生成预测不确定性地图，以改进传统人工分析的效率与客观性。


<details>
  <summary>Details</summary>
Motivation: 传统TRISO包覆颗粒燃料的辐照后分析依赖专家手动评估数千个亚毫米样本的截面，过程繁琐且主观，需要自动化、高精度的图像分析工具。

Method: 采用多阶段预训练策略：先在ImageNet上学习通用图像表征，再在多种辐照实验和AGR-5/6/7颗粒截面的TRISO微观图像上微调；集成元模型进行不确定性预测以识别微小缺陷。

Result: 在102张测试图像上，UA-Net的平均交并比(mIoU)达95.5%，平均精度(mP)达97.3%；元模型特异性91.8%、敏感性93.5%，能有效检测误分类；在新图像定性评估中展现出高精度分层提取能力。

Conclusion: UA-Net能高效、准确地自动化分析TRISO燃料微观结构，其不确定性预测功能有助于识别微小缺陷，为核燃料性能评估提供了可靠工具。
...
### [31] [CXR-LT 2026 Challenge: Multi-Center Long-Tailed and Zero Shot Chest X-ray Classification](https://arxiv.org/abs/2604.15555)
...
TL;DR: CXR-LT 2026挑战赛建立了多中心胸部X光长尾分布数据集，包含超过14.5万张图像，重点关注已知30类疾病的多标签分类和6类未见罕见疾病的开放世界泛化能力评估。


<details>
  <summary>Details</summary>
Motivation: 现有胸部X光基准数据集通常基于单机构封闭类别，无法捕捉罕见疾病分布和临床环境中新发现的出现，需要建立更贴近真实临床长尾分布和开放世界特性的评估框架。

Method: 构建多中心数据集（PadChest和NIH Chest X-ray），所有开发集和测试集由放射科医生标注；定义两个核心任务：已知30类疾病的鲁棒多标签分类和6类未见罕见疾病的开放世界泛化；分析参赛团队的解决方案策略。

Result: 视觉语言基础模型提升了分布内和零样本性能，但在多中心偏移下检测罕见发现仍具挑战；研究揭示了头尾类别性能差异、校准问题和跨中心泛化差距。

Conclusion: 该研究为在真实长尾分布和开放世界临床条件下开发和评估AI系统提供了基础，强调了罕见疾病检测和多中心泛化仍是重要挑战。
...
### [32] [CLIMB: Controllable Longitudinal Brain Image Generation using Mamba-based Latent Diffusion Model and Gaussian-aligned Autoencoder](https://arxiv.org/abs/2604.15611)
*Duy-Phuong Dao,Muhammad Taqiyuddin,Jahae Kim,Sang-Heon Lee,Hye-Won Jung,Jaehoo Choi,Hyung-Jeong Yang*

Main category: cs.CV

TL;DR: 提出CLIMB框架，基于状态空间潜在扩散模型生成可控的纵向脑部MRI图像，用于模拟大脑结构随时间的变化。


<details>
  <summary>Details</summary>
Motivation: 预测患者大脑的演变有助于早期干预、预后和治疗规划，现有潜在扩散模型依赖自注意力模块计算成本高，需要更高效的时序建模方法。

Method: 使用状态空间模型架构替代自注意力模块降低计算开销；引入高斯对齐自编码器提取符合先验分布的潜在表示；整合基线MRI、年龄、性别、疾病状态、遗传信息和脑结构体积等多条件变量。

Result: 在ADNI数据集（6,306个MRI扫描）上评估，生成图像与真实MRI的结构相似性指数达0.9433，优于现有方法。

Conclusion: CLIMB框架能高效合成高质量纵向脑部图像，为疾病进展建模提供有效工具，状态空间架构在保持图像质量的同时显著降低计算成本。
...
### [33] [AdaVFM: Adaptive Vision Foundation Models for Edge Intelligence via LLM-Guided Execution](https://arxiv.org/abs/2604.15622)
...
TL;DR: AdaVFM是一个自适应框架，通过动态调整计算资源来优化语言对齐视觉基础模型在边缘设备上的推理效率，实现精度与效率的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 语言对齐视觉基础模型在边缘设备部署时面临严格的延迟和功耗限制，需要一种能根据场景上下文和任务复杂度动态调整计算的方法。

Method: 将神经架构搜索集成到VFM主干中，支持运行时轻量子网执行；利用云端多模态大语言模型作为上下文感知代理进行运行时控制。

Result: 在零样本分类和开放词汇分割任务中，AdaVFM在可比VFM尺寸下，IN1K准确率提升7.9%，ADE20K mIoU提升5.2%；同等精度下平均FLOPs减少高达77.9%。

Conclusion: AdaVFM通过任务自适应的运行时执行策略，显著提升了语言对齐视觉基础模型在边缘设备上的部署效率，同时保持了强大的准确性。
...
### [34] [SIMMER: Cross-Modal Food Image--Recipe Retrieval via MLLM-Based Embedding](https://arxiv.org/abs/2604.15628)
*Keisuke Gomi,Keiji Yanai*

Main category: cs.CV

TL;DR: 提出SIMMER模型，使用单模态大语言模型统一编码食物图像和食谱文本，在跨模态检索任务中取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖双编码器架构，需要复杂对齐策略和任务特定设计来弥合模态语义鸿沟，因此探索更统一的跨模态表示方法。

Method: 基于VLM2Vec构建单编码器模型，设计针对食谱结构化内容（标题、食材、步骤）的提示模板，并提出组件感知数据增强策略以提升对不完整输入的鲁棒性。

Result: 在Recipe1M数据集上，1k和10k评估设置均达到最优：1k图像到食谱R@1从81.8%提升至87.5%，10k图像到食谱R@1从56.5%提升至65.5%。

Conclusion: 单编码器MLLM方法能有效统一处理多模态食物数据，通过结构化提示和数据增强显著提升跨模态检索性能，为相关应用提供新范式。
...
### [35] [Causal Bootstrapped Alignment for Unsupervised Video-Based Visible-Infrared Person Re-Identification](https://arxiv.org/abs/2604.15631)
*Shuang Li,Jiaxu Leng,Changjiang Kuang,Mingpi Tan,Yu Yuan,Xinbo Gao*

Main category: cs.CV

TL;DR: 本文提出了一种用于无监督可见光-红外视频行人重识别（USL-VVI-ReID）的因果引导对齐框架（CBA），通过利用视频先验知识解决模态偏差和粒度不匹配问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VVI-ReID方法严重依赖昂贵的跨模态标注数据，限制了可扩展性。而直接将基于图像的无监督方法扩展到视频领域时，由于预训练编码器存在身份判别能力弱和模态偏差强的问题，导致伪标签可靠性差和跨模态对齐困难。

Method: 提出了因果引导对齐（CBA）框架：1）因果干预预热（CIW），利用时序身份一致性和跨模态身份一致性进行序列级因果干预，抑制模态和运动引起的虚假关联；2）原型引导的不确定性细化（PGUR），采用由粗到细的对齐策略，在可靠可见光原型的指导下重组欠聚类的红外表示。

Result: 在HITSZ-VCM和BUPTCampus基准测试上的大量实验表明，CBA在扩展到USL-VVI-ReID设置时，显著优于现有的USL-VI-ReID方法。

Conclusion: CBA框架通过显式利用视频先验知识，有效解决了无监督跨模态视频行人重识别中的模态偏差和聚类粒度不平衡问题，为全天候监控提供了可扩展的解决方案。
...
### [36] [SPLIT: Self-supervised Partitioning for Learned Inversion in Nonlinear Tomography](https://arxiv.org/abs/2604.15651)
*Markus Haltmeier,Lukas Neumann,Nadja Gruber,Gyeongha Hwang*

Main category: cs.CV

TL;DR: 提出SPLIT框架，一种自监督学习方法，用于解决非线性、不完整且含噪声的层析成像重建问题，无需真实图像样本。


<details>
  <summary>Details</summary>
Motivation: 传统监督学习方法需要配对的测量数据和真实图像，但这些数据通常难以获取，尤其是在非线性逆问题中。

Method: SPLIT框架通过强制跨分区一致性和测量域保真度，利用多个分区的互补信息；引入自动停止规则，当无参考图像质量指标饱和时停止优化。

Result: 在稀疏视图采集实验中，SPLIT展现出高质量重建和强噪声鲁棒性，优于经典迭代重建和近期自监督基线方法。

Conclusion: SPLIT为非线性层析成像提供了一种有效的自监督重建方案，理论证明其自监督目标在期望上等价于监督目标。
...
### [37] [Towards Realistic Open-Vocabulary Remote Sensing Segmentation: Benchmark and Baseline](https://arxiv.org/abs/2604.15652)
*Bingyu Li,Tao Huo,Haocheng Dong,Da Zhang,Zhiyuan Zhao,Junyu Gao,Xuelong Li*

Main category: cs.CV

TL;DR: 提出OVRSISBenchV2基准和Pi-Seg基线方法，用于开放词汇遥感图像分割，通过大规模数据集和噪声增强机制提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇遥感图像分割研究存在数据集碎片化、训练多样性不足、缺乏反映真实地理空间应用需求的评估基准等问题，需要更全面的基准和方法来评估现实世界的泛化能力。

Method: 构建包含95K图像-掩码对的OVRSIS95K数据集，整合形成170K图像的OVRSISBenchV2基准；提出Pi-Seg基线方法，采用正激励噪声机制，通过可学习的语义引导扰动扩展视觉-文本特征空间。

Result: Pi-Seg在OVRSISBenchV1、OVRSISBenchV2及下游任务上表现一致且强劲，尤其在更具挑战性的OVRSISBenchV2基准上效果显著，验证了基准设计的合理性和扰动迁移的有效性。

Conclusion: 现实基准设计对开放词汇遥感图像分割至关重要，基于扰动的迁移方法能有效提升模型泛化性能，为地理空间应用提供了更可靠的评估框架和解决方案。
...
### [38] [From Zero to Detail: A Progressive Spectral Decoupling Paradigm for UHD Image Restoration with New Benchmark](https://arxiv.org/abs/2604.15654)
*Chen Zhao,Yunzhe Xu,Zhizhou Chen,Enxuan Gu,Kai Zhang,Xiaoming Liu,Jian Yang,Ying Tai*

Main category: cs.CV

TL;DR: 提出ERR框架，通过渐进式频谱分解（零频增强、低频恢复、高频细化）和FW-KAN网络，结合新建的LSUHDIR数据集，实现超高清图像高质量恢复。


<details>
  <summary>Details</summary>
Motivation: 超高清图像恢复面临高分辨率、内容多样和结构精细的独特挑战，现有方法难以同时处理全局信息和细节纹理。

Method: 1. 渐进式频谱分解三阶段框架（ERR）；2. 三个协作子网络：ZFE（全局先验）、LFR（粗尺度重建）、HFR（FW-KAN细化细节）；3. 构建LSUHDIR数据集（82,126张UHD图像）。

Result: 在多种UHD图像恢复任务中表现优异，消融实验验证了各模块的必要性和贡献。

Conclusion: ERR框架通过频谱分解和专用子网络有效解决了UHD图像恢复的挑战，新建数据集推动了该领域研究。
...
### [39] [CPU Optimization of a Monocular 3D Biomechanics Pipeline for Low-Resource Deployment](https://arxiv.org/abs/2604.15665)
*Yan Zhang,Xiong Zhao*

Main category: cs.CV

TL;DR: 优化单目视频3D生物力学分析流程，使其能在CPU上高效运行，提升在消费级硬件上的部署能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究级3D生物力学分析流程依赖GPU加速，限制了在消费级硬件和低资源环境中的部署。

Method: 基于MonocularBiomechanics框架，通过模型初始化重构、消除磁盘I/O序列化和改进CPU并行化等系统优化方法。

Result: 在AMD Ryzen 7 9700X CPU上，处理吞吐量提升2.47倍，总运行时间减少59.6%，初始化延迟降低4.6倍，生物力学输出与基准实现高度一致。

Conclusion: 研究级视觉生物力学流程可在商品CPU硬件上部署，实现可扩展的运动评估。
...
### [40] [PixDLM: A Dual-Path Multimodal Language Model for UAV Reasoning Segmentation](https://arxiv.org/abs/2604.15670)
*Shuyan Ke,Yifan Mei,Changli Wu,Yonghan Zheng,Jiayi Ji,Liujuan Cao,Rongrong Ji*

Main category: cs.CV

TL;DR: 本文针对无人机影像的推理分割任务，提出了三维语义需求框架，构建了大规模基准数据集DRSeg，并开发了像素级多模态语言模型PixDLM作为统一基线。


<details>
  <summary>Details</summary>
Motivation: 现有推理分割研究主要针对地面场景，而无人机影像存在倾斜视角、超高分辨率和极端尺度变化等独特挑战，需要专门的研究框架和基准。

Method: 1) 将无人机推理分割的语义需求划分为空间、属性和场景三个维度；2) 构建包含1万张高分辨率航拍图像的DRSeg基准数据集，配备链式思维问答监督；3) 提出PixDLM像素级多模态语言模型作为统一基线方法。

Result: 在DRSeg数据集上的实验建立了强基线结果，验证了PixDLM的有效性，同时凸显了无人机推理分割任务的独特挑战。

Conclusion: 本研究为无人机推理分割提供了系统的任务定义、大规模基准数据集和有效的基线模型，为未来研究奠定了坚实基础。
...
### [41] [HyCal: A Training-Free Prototype Calibration Method for Cross-Discipline Few-Shot Class-Incremental Learning](https://arxiv.org/abs/2604.15678)
*Eunju Lee,MiHyeon Kim,JuneHyoung Kwon,Yoonji Lee,JiHyun Kim,Soojin Jang,YoungBin Kim*

Main category: cs.CV

TL;DR: 提出XD-VSCIL基准和HyCal方法，解决异构域不平衡数据下的少样本类增量学习问题，通过结合余弦相似度和马氏距离稳定原型表示，缓解领域引力效应。


<details>
  <summary>Details</summary>
Motivation: 现有FSCIL方法假设同质域和平衡数据分布，但现实场景中数据来自异构学科，存在样本不平衡和视觉复杂度差异，导致领域引力效应——数据不平衡引发嵌入空间表示偏差，影响模型性能。

Method: 提出训练无关的混合原型校准方法HyCal，结合余弦相似度（捕捉方向对齐）和马氏距离（协方差感知幅度），在冻结CLIP嵌入上操作，生成异构不平衡条件下的稳定原型。

Result: HyCal在跨域不平衡增量学习中有效缓解领域引力效应，在保持效率的同时实现持续的性能提升，优于现有方法。

Conclusion: HyCal通过几何互补的相似度度量解决了异构域不平衡增量学习中的原型漂移问题，为现实场景中视觉语言模型的持续学习提供了实用方案。
...
### [42] [Self-Supervised Angular Deblurring in Photoacoustic Reconstruction via Noisier2Inverse](https://arxiv.org/abs/2604.15681)
*Markus Haltmeier,Nadja Gruber,Gyeongha Hwang*

Main category: cs.CV

TL;DR: 提出一种基于Noisier2Inverse的自监督光声断层扫描重建方法，无需真实标签即可校正有限尺寸探测器导致的图像模糊问题。


<details>
  <summary>Details</summary>
Motivation: 传统光声断层扫描重建算法应用于有限尺寸探测器数据时会产生系统性模糊，而监督学习方法需要难以获取的真实标签数据。

Method: 将问题建模为角度去模糊，在极坐标域应用Noisier2Inverse框架，利用已知角度点扩散函数，并设计了基于统计的早停准则。

Result: 该方法在无监督方法中表现最佳，性能接近监督基准方法，适用于实际有限尺寸探测器数据采集。

Conclusion: 所提出的自监督方法能有效解决有限尺寸探测器效应，无需真实标签数据，具有实际应用价值。
...
### [43] [P3T: Prototypical Point-level Prompt Tuning with Enhanced Generalization for 3D Vision-Language Models](https://arxiv.org/abs/2604.15703)
*Geunyoung Jung,Soohong Kim,Kyungwoo Song,Jiyoung Jung*

Main category: cs.CV

TL;DR: 提出P³T方法，通过点级提示和文本提示实现3D视觉语言模型的高效微调，避免过拟合并提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 预训练3D点云模型在下游任务适配时，传统全微调方法计算和存储成本高，而现有提示调优方法易过拟合、泛化能力不足。

Method: 1) Point Prompter为输入点云生成实例感知的点级提示；2) Text Prompter将可学习提示注入文本输入；3) 引入原型损失减少类内方差，增强嵌入空间对齐。

Result: 在分类和少样本学习任务中达到或超越全微调性能，在跨数据集场景下对数据偏移具有鲁棒泛化能力。

Conclusion: P³T是一种参数高效的提示调优方法，能在保持3D视觉语言模型泛化能力的同时实现任务特定适配，为3D点云模型轻量化适配提供有效方案。
...
### [44] [LP$^{2}$DH: A Locality-Preserving Pixel-Difference Hashing Framework for Dynamic Texture Recognition](https://arxiv.org/abs/2604.15707)
*Ruxin Ding,Jianfeng Ren,Heng Yu,Jiawei Li,Xudong Jiang*

Main category: cs.CV

TL;DR: 提出LP²DH框架，通过局部保持的像素差异哈希编码全时空邻域信息，显著降低STLBP特征维度并提升动态纹理识别性能。


<details>
  <summary>Details</summary>
Motivation: STLBP特征维度极高，现有三正交平面方法牺牲了平面间相关性，需开发能保留完整时空信息且低维度的动态纹理描述符。

Method: 1. 将像素差异向量哈希为紧凑二进制码；2. 引入局部保持嵌入维持哈希前后局部结构；3. 在Stiefel流形上联合优化哈希矩阵与二进制码；4. 通过字典学习生成直方图特征。

Result: 在三个主流动态纹理数据集上达到最优性能：UCLA（99.80%）、DynTex++（98.52%）、YUPENN（96.19%），均超越现有最佳方法。

Conclusion: LP²DH通过联合编码全时空邻域信息，在显著降维的同时保持判别力，为动态纹理识别提供了高效解决方案。
...
### [45] [APC: Transferable and Efficient Adversarial Point Counterattack for Robust 3D Point Cloud Recognition](https://arxiv.org/abs/2604.15708)
*Geunyoung Jung,Soohong Kim,Inseok Kong,Jiyoung Jung*

Main category: cs.CV

TL;DR: 提出APC方法，通过生成实例特定的对抗性反扰动来净化3D点云输入，同时实现鲁棒性和可迁移性，无需重新训练即可防御多种攻击。


<details>
  <summary>Details</summary>
Motivation: 现有3D点云防御方法在鲁棒性和可迁移性之间存在权衡，难以同时兼顾。

Method: APC作为轻量级输入级净化模块，利用干净-对抗点云对，在数据空间强制几何一致性、特征空间强制语义一致性，并采用混合训练策略提升泛化性。

Result: 在两个3D识别基准测试中达到最先进的防御性能，跨模型评估验证了其优越的可迁移性。

Conclusion: APC能有效中和对抗攻击，以可忽略的时间和参数开销实现高效防御，且可直接迁移到未见过的模型。
...
### [46] [PLAF: Pixel-wise Language-Aligned Feature Extraction for Efficient 3D Scene Understanding](https://arxiv.org/abs/2604.15770)
*Junjie Wen,Junlin He,Fei Ma,Jinqiang Cui*

Main category: cs.CV

TL;DR: 提出PLAF框架，通过像素级语言对齐特征提取实现精确的开放词汇3D场景理解，并设计高效语义存储查询方案减少冗余。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时满足语言对齐、空间精确和3D可扩展性要求，且密集像素语义传播到3D时存在严重冗余问题。

Method: 1. 设计像素级语言对齐特征提取框架（PLAF）；2. 开发跨2D/3D域的高效语义存储与查询方案以减少冗余。

Result: 实验表明PLAF为开放词汇3D场景理解提供了准确高效的语义基础，代码已开源。

Conclusion: PLAF框架成功解决了现有方法在语义对齐精度和3D扩展效率方面的局限性，实现了更优的开放词汇3D场景理解。
...
### [47] [SSMamba: A Self-Supervised Hybrid State Space Model for Pathological Image Classification](https://arxiv.org/abs/2604.15711)
*Enhui Chai,Sicheng Chen,Tianyi Zhang,Xingyu Li,Tianxiang Cui*

Main category: cs.CV

TL;DR: 提出SSMamba框架，通过混合自监督学习解决病理图像分析中ROI级基础模型的三个核心限制，在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于ViT的ROI级基础模型存在三个主要问题：跨放大倍率域偏移、局部-全局关系建模不足、细粒度敏感性不够，限制了病理图像分析的性能。

Method: 提出SSMamba混合自监督学习框架，包含三个组件：MAMIM缓解域偏移、DMS模块平衡局部-全局建模、LPR模块增强细粒度敏感性；采用两阶段流程：目标数据集自监督预训练后监督微调。

Result: 在10个公共ROI数据集上超越11个SOTA病理基础模型，在6个公共WSI数据集上超越8个SOTA方法。

Conclusion: 任务特定的架构设计对病理图像分析具有优越性，SSMamba框架能有效解决现有模型的局限性。
...
### [48] [Continual Hand-Eye Calibration for Open-world Robotic Manipulation](https://arxiv.org/abs/2604.15814)
*Fazeng Li,Gan Sun,Chenxi Liu,Yao He,Wei Cong,Yang Cong*

Main category: cs.CV

TL;DR: 提出了一种持续手眼标定框架，通过空间感知回放策略和结构保持双重蒸馏，使机器人能够在开放世界场景变化中持续适应，有效缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的手眼标定模型在开放世界场景变化中面临灾难性遗忘问题，而简单的回放式持续学习策略无法有效缓解这一挑战。

Method: 采用空间感知回放策略构建几何均匀的回放缓冲区，确保每个场景姿态空间的全面覆盖；提出结构保持双重蒸馏，将定位知识分解为粗粒度场景布局和细粒度姿态精度分别进行蒸馏。

Result: 在多个公开数据集上的实验表明，该方法在保持对新场景适应能力的同时，显著提升了抗场景遗忘性能，维持了对过往场景的标定精度。

Conclusion: 该框架通过空间回放与结构化蒸馏的结合，使机器人能够持续积累多场景标定能力，为开放世界机器人操作提供了有效的持续适应解决方案。
...
### [49] [NeuroLip: An Event-driven Spatiotemporal Learning Framework for Cross-Scene Lip-Motion-based Visual Speaker Recognition](https://arxiv.org/abs/2604.15718)
*Junguang Yao,Wenye Liu,Stjepan Picek,Yue Zheng*

Main category: cs.CV

TL;DR: 本文提出NeuroLip，一种基于事件相机的唇部运动识别框架，通过捕捉精细的唇部动态特征实现跨场景的视觉说话人识别，并发布了首个综合的事件唇部运动数据集DVSpeaker。


<details>
  <summary>Details</summary>
Motivation: 传统视觉说话人识别方法依赖外观特征，易受环境变化影响；而基于唇部运动的生物特征具有行为稳定性，但传统相机因运动模糊和动态范围限制难以捕捉精细动态。事件相机的高时空分辨率为此提供了新可能。

Method: 1）提出时序感知体素编码模块，采用自适应事件加权；2）设计结构感知空间增强器，抑制噪声并保留垂直结构运动信息；3）引入极性一致性正则化机制，保持事件极性中的运动方向线索。在单场景训练、多场景测试的跨场景协议下验证。

Result: NeuroLip在匹配场景下接近完美准确率，在未见过的视角和低光条件下分别达到71%和76%的识别准确率，比现有代表性方法至少提升8.54%。

Conclusion: 事件相机能有效捕捉唇部精细运动特征，NeuroLip框架通过多模块协同实现了跨场景的鲁棒识别，为无声环境下的生物识别提供了新解决方案。公开的数据集和代码将推动该领域发展。
...
### [50] [SENSE: Stereo OpEN Vocabulary SEmantic Segmentation](https://arxiv.org/abs/2604.15946)
*Thomas Campagnolo,Ezio Malis,Philippe Martinet,Gaétan Bahl*

Main category: cs.CV

TL;DR: 提出首个基于立体视觉的开放词汇语义分割方法SENSE，通过融合立体图像对和视觉语言模型提升分割精度，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇语义分割方法依赖单视角图像，在遮挡和物体边界处空间精度不足，难以适应动态环境需求。

Method: 利用立体视觉提供几何线索，结合视觉语言模型进行语义-几何联合推理，在PhraseStereo数据集上训练。

Result: 在PhraseStereo上平均精度比基线提升2.9%；在Cityscapes和KITTI上mIoU分别相对提升3.5%和18%。

Conclusion: SENSE通过立体视觉增强空间推理能力，显著提升开放词汇语义分割精度，为自主机器人和智能交通系统提供更准确的场景理解。
...
### [51] [Diffusion Autoencoder for Unsupervised Artifact Restoration in Handheld Fundus Images](https://arxiv.org/abs/2604.15723)
*Mathumetha Palani,Kavya Puthumana,Ayantika Das,Ganapathy Krishnamurthi*

Main category: cs.CV

TL;DR: 提出一种无监督扩散自编码器，用于修复手持眼底图像中的伪影，仅使用高质量桌面眼底图像训练，提升诊断准确率至81.17%。


<details>
  <summary>Details</summary>
Motivation: 手持眼底成像设备虽便捷，但图像常受闪光反射、曝光变化和运动模糊等伪影影响，降低质量并阻碍分析；现有生成模型依赖配对监督或预定义伪影结构，难以适应手持图像中的非结构化退化。

Method: 开发无监督扩散自编码器，结合上下文编码器和去噪过程，学习语义表示以修复伪影；模型仅用高质量桌面眼底图像训练，并推断修复伪影影响的手持图像。

Result: 通过定量和定性评估验证修复效果，在未见数据集和多种伪影条件下，诊断准确率提升至81.17%。

Conclusion: 该方法能有效修复手持眼底图像中的非结构化伪影，提高图像质量和下游诊断准确性，无需配对监督或预定义退化结构。
...
### [52] [FineCog-Nav: Integrating Fine-grained Cognitive Modules for Zero-shot Multimodal UAV Navigation](https://arxiv.org/abs/2604.16298)
*Dian Shao,Zhengzheng Xu,Peiyang Wang,Like Liu,Yule Wang,Jieqi Shi,Jing Huo*

Main category: cs.CV

TL;DR: 提出FineCog-Nav框架，通过细粒度认知模块化实现无人机零样本视觉语言导航，并在新基准AerialVLN-Fine上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有零样本方法依赖大型基础模型、通用提示和松散协调模块，在复杂3D环境中执行多步模糊指令时存在局限，需要更精细的认知架构。

Method: 采用自上而下框架，将导航分解为语言处理、感知、注意力、记忆、想象、推理和决策等模块，每个模块使用中等规模基础模型配合角色特定提示和结构化输入输出协议。

Result: FineCog-Nav在指令遵循、长时程规划和未见环境泛化方面持续优于零样本基线方法。

Conclusion: 细粒度认知模块化对零样本空中导航具有有效性，模块化设计提升了协作效率和可解释性。
...
### [53] [Sketch and Text Synergy: Fusing Structural Contours and Descriptive Attributes for Fine-Grained Image Retrieval](https://arxiv.org/abs/2604.15735)
*Siyuan Wang,Hanchen Gao,Guangming Zhu,Jiang Lu,Yiyue Ma,Tianci Wu,Jincai Huang,Liang Zhang*

Main category: cs.CV

TL;DR: 提出STBIR框架，通过结合手绘草图的结构轮廓与文本的颜色纹理信息，实现细粒度图像检索，并构建了相应基准数据集验证有效性。


<details>
  <summary>Details</summary>
Motivation: 细粒度图像检索中，手绘草图缺乏颜色纹理，文本描述缺乏空间轮廓，二者具有互补性，但存在模态鸿沟问题。

Method: 1. 课程学习驱动的鲁棒性增强模块处理不同质量查询；2. 基于类别知识的特征空间优化模块提升表示能力；3. 多阶段跨模态特征对齐机制缓解对齐挑战；4. 构建细粒度STBIR基准数据集。

Result: 大量实验表明，STBIR框架在细粒度检索性能上显著优于现有先进方法。

Conclusion: 通过协同利用草图与文本的互补信息，并设计相应模块优化特征表示与对齐，STBIR能有效提升跨模态细粒度图像检索效果。
...
### [54] [MambaBack: Bridging Local Features and Global Contexts in Whole Slide Image Analysis](https://arxiv.org/abs/2604.15729)
*Sicheng Chen,Chad Wong,Tianyi Zhang,Enhui Chai,Zeyu Liu,Fei Xia*

Main category: cs.CV

TL;DR: 提出MambaBack混合架构，结合Mamba和MambaOut优势，通过Hilbert采样、分层结构和非对称分块设计解决WSI分析中的空间局部性破坏、细粒度特征建模不足和高内存消耗问题，在五个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于Mamba的多示例学习方法在WSI分析中存在三个关键问题：1D序列展平破坏2D空间局部性；细粒度局部细胞结构建模不足；在资源受限设备上推理时内存峰值过高。同时研究发现Mamba的SSM组件对局部特征提取冗余，而WSI分析需要同时具备自然图像的细粒度特征提取和NLP的全局上下文建模能力。

Method: 1. 提出Hilbert采样策略，在1D序列中保持瓦片的2D空间局部性；2. 设计分层结构：基于MambaOut的1D门控CNN块捕获局部细胞特征，BiMamba2块聚合全局上下文；3. 实现非对称分块设计，训练时并行处理，推理时分块流式累积以降低内存使用。

Result: 在五个数据集上的实验结果表明，MambaBack在性能上超越了七种最先进的方法。

Conclusion: MambaBack通过有效结合Mamba的全局上下文建模能力和MambaOut的局部特征提取优势，解决了WSI分析中的关键挑战，为计算病理学提供了高效且性能优异的解决方案。
...
### [55] [RefereeBench: Are Video MLLMs Ready to be Multi-Sport Referees](https://arxiv.org/abs/2604.15736)
*Yichen Xu,Yuanhang Liu,Chuhan Wang,Zihan Zhao,jinghan luo,Jianzhe Ma,Wenxuan Wang,Qin Jin*

Main category: cs.CV

TL;DR: 本文提出了首个大规模基准测试RefereeBench，用于评估多模态大语言模型在体育裁判任务中的表现，发现当前模型准确率仅约60%，远未达到可靠裁判水平。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在通用视频理解方面表现优异，但其在专业、基于规则的决策能力（如体育裁判）方面尚未得到充分探索，需要专门的评估基准来推动该领域发展。

Method: 构建了包含11项运动、925个视频和6,475个问答对的大规模基准测试RefereeBench，评估五种核心裁判能力：犯规存在性判断、犯规与处罚分类、犯规与处罚推理、实体感知和时间定位。所有数据均经过人工标注，确保基于真实裁判逻辑和多模态证据的高质量标注。

Result: 对最先进模型的评估显示，最强模型（如Doubao-Seed-1.8和Gemini-3-Pro）准确率仅约60%，最强开源模型Qwen3-VL仅达47%。模型虽能识别事件和实体，但在规则应用、时间定位方面存在困难，且常对正常片段误判为犯规。

Conclusion: 当前模型距离成为可靠的体育裁判仍有较大差距，未来需要开发能更好整合领域知识和多模态理解的模型，以推动可信的AI辅助裁判及更广泛的多模态决策系统发展。
...
### [56] [Concept-wise Attention for Fine-grained Concept Bottleneck Models](https://arxiv.org/abs/2604.15748)
*Minghong Zhong,Guoshuai Zou,Kanghao Chen,Dexia Chen,Ruixuan Wang*

Main category: cs.CV

TL;DR: 提出CoAt-CBM框架，通过概念级视觉查询和对比优化解决概念瓶颈模型中粒度错位和概念独立性假设问题，提升细粒度图像-概念对齐和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型存在预训练偏差（粒度错位/结构先验依赖）和二元交叉熵损失忽略概念互斥性的问题，导致次优对齐。

Method: 1. 使用可学习概念级视觉查询自适应获取细粒度概念视觉嵌入；2. 设计概念对比优化策略处理概念分数的相对重要性。

Result: 在多个实验中，CoAt-CBM持续优于现有最先进方法，实现了更准确的图像内容反映和对齐效果。

Conclusion: CoAt-CBM通过自适应细粒度对齐和对比优化有效解决了概念建模中的关键限制，为可解释性AI提供了改进方案。
...
### [57] [Seeing the Intangible: Survey of Image Classification into High-Level and Abstract Categories](https://arxiv.org/abs/2308.10562)
*Delfina Sol Martinez Pandiani,Valentina Presutti*

Main category: cs.CV

TL;DR: 本文系统综述了计算机视觉中高层视觉理解的研究，聚焦于抽象概念图像分类，通过多学科分析澄清了高层语义的模糊性，并探讨了相关任务、挑战及混合AI系统的应用前景。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉领域日益转向“高层”视觉理解任务，但这些任务的具体定义和内涵尚不明确且多为隐含知识，需要系统性的梳理和澄清。

Method: 采用系统性文献综述方法，结合多学科视角分析高层语义，将其分类为常识、情感、美学和归纳解释性语义等类别，并识别和归类相关计算机视觉任务。

Result: 明确了高层视觉语义的分类框架；揭示了抽象概念（如价值观、意识形态）在图像分类中的处理方式及挑战；指出大规模数据集在抽象概念分类中效果有限，需结合补充信息和中层特征。

Conclusion: 高层视觉理解是计算机视觉的重要发展方向，混合AI系统在应对抽象概念图像分类的复杂性方面具有潜力，本文为未来研究奠定了基础。
...
### [58] [SegMix:Shuffle-based Feedback Learning for Semantic Segmentation of Pathology Images](https://arxiv.org/abs/2604.15777)
*Zhiling Yan,Sicheng Chen,Tianyi Zhang,Nan Ying,Yanli Lei,Guanglei Zhang*

Main category: cs.CV

TL;DR: 提出一种基于洗牌反馈学习的方法，利用图像级标签生成更高质量的病理图像伪语义分割掩码，解决像素级标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 病理图像分割需要像素级标注，但标注成本高昂且依赖专家经验，限制了深度学习应用。使用图像级分类标签可扩大数据利用范围，但现有CAM方法生成的伪标注质量不足，仅能识别小区域。

Method: 提出基于课程学习思想的洗牌反馈学习方法：对病理图像进行块级洗牌，模型根据先前学习反馈自适应调整洗牌策略，以生成更优的伪语义分割掩码。

Result: 在三个不同数据集上的实验表明，该方法优于现有最先进方法。

Conclusion: 所提出的洗牌反馈学习方法能有效提升仅使用图像级标签时伪分割掩码的质量，为病理图像分析提供了更高效的弱监督解决方案。
...
### [59] [Fed3D: Federated 3D Object Detection](https://arxiv.org/abs/2604.15795)
*Suyan Dai,Chenxi Liu,Fazeng Li,Peican Lin*

Main category: cs.CV

TL;DR: 本文提出了一种名为Fed3D的新型联邦3D物体检测框架，旨在解决多机器人网络中3D物体检测的隐私保护、数据异构性和通信成本问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D物体检测方法在多机器人感知网络中部署时面临严重的隐私问题，且传统联邦学习方法难以应对3D数据异构性和有限通信带宽的挑战。

Method: 提出Fed3D框架，包括局部-全局类别感知损失以处理3D数据异构性，以及联邦3D提示模块以减少通信成本。

Result: 实验表明，Fed3D在联邦3D物体检测任务中显著优于现有算法，并在有限本地训练数据下实现了更低的通信成本。

Conclusion: Fed3D为分布式3D物体检测提供了一种有效的隐私保护解决方案，能够平衡数据异构性和通信效率。
...
### [60] [Robust Multispectral Semantic Segmentation under Missing or Full Modalities via Structured Latent Projection](https://arxiv.org/abs/2604.15856)
*Irem Ulku,Erdem Akagündüz,Ömer Özgür Tanrıöver*

Main category: cs.CV

TL;DR: 提出CBC-SLP模型，通过结构化潜在投影保留模态不变和模态特定信息，提升多模态遥感图像语义分割在完整和缺失模态场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态分割模型通过共享表示处理缺失模态，但会损失模态特定互补信息，导致所有模态可用时性能下降。

Method: 采用结构化潜在投影作为架构归纳偏置，将潜在表示分为共享和模态特定组件，根据模态可用性掩码自适应传输到解码器。

Result: 在三个多模态遥感数据集上，CBC-SLP在完整和缺失模态场景下均优于现有方法，并能恢复共享表示中可能丢失的互补信息。

Conclusion: CBC-SLP通过显式建模共享和模态特定表示，有效平衡了模态互补信息利用和缺失模态鲁棒性，为多模态分割提供了新思路。
...
TL;DR: 该研究提出了SGMRI-VQA基准，用于评估视觉语言模型在医学MRI图像上的空间推理和视觉定位能力，并通过实验证明有针对性的空间监督能有效提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型缺乏透明推理和空间证据，且现有基准测试仅关注2D图像，忽略了临床影像的立体特性，无法评估模型在跨帧空间推理上的能力。

Method: 基于fastMRI+数据集构建了包含41,307个问答对的SGMRI-VQA基准，包含放射科专家标注的链式思维轨迹和帧索引边界框坐标；将任务分层为检测、定位、计数/分类和描述；对10个视觉语言模型进行基准测试，并对Qwen3-VL-8B模型进行带边界框监督的微调。

Result: 监督微调的Qwen3-VL-8B模型在空间定位性能上持续优于强零样本基线模型，表明有针对性的空间监督能有效提升临床推理的定位能力。

Conclusion: SGMRI-VQA基准填补了医学视觉语言模型在立体影像空间推理评估上的空白，实验证明空间监督是提升临床推理定位能力的有效途径。
...
TL;DR: 本文提出一种基于令牌动态的方法来优化视觉语言模型的信息流，通过识别重要视觉令牌并减少无关视觉令牌的干扰，显著提升了模型在多项视觉语言任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在回答问题时，虽然能关注到正确的图像区域，却常给出错误答案。研究发现，这可能是由于文本令牌在解码过程中过度关注无关视觉令牌，导致信息流不理想。

Method: 提出一种基于令牌动态的视觉令牌重要性评估方法：在解码的不同阶段，激活模式差异显著的视觉令牌被视为重要令牌。通过调节推理时的信息流，使文本令牌仅与重要视觉令牌关联，减少无关区域的干扰。

Result: 在多个开源视觉语言模型上验证，涵盖视觉问答、视觉定位与计数、光学字符识别和物体幻觉等数据集。实验结果表明，该方法能显著提升基线模型的性能。

Conclusion: 通过调节视觉语言模型推理时的信息流，聚焦重要视觉令牌，可有效提升模型的感知能力，解决答案与视觉关注区域不匹配的问题。
...
### [63] [UniEditBench: A Unified and Cost-Effective Benchmark for Image and Video Editing via Distilled MLLMs](https://arxiv.org/abs/2604.15871)
*Lifan Jiang,Tianrun Wu,Yuhang Pei,Chenyang Wang,Boxi Wu,Deng Cai*

Main category: cs.CV

TL;DR: 提出了一个统一的图像和视频编辑基准UniEditBench，支持重建式和指令驱动式方法，并蒸馏了轻量级评估器以降低大模型评估成本。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编辑评估方法存在碎片化问题：不同范式难以公平比较，视频编辑缺乏可靠基准，自动评估指标与人类偏好不一致，而直接使用大模型评估成本过高。

Method: 构建了包含9种图像操作和8种视频操作的统一基准，并提出通过知识蒸馏将大模型（Qwen3-VL-235B）压缩为轻量级（4B/8B）评估器，从多个维度进行评分。

Result: 蒸馏后的评估器在保持与人类判断高度一致的同时，显著降低了部署成本，基准支持对复杂组合任务（如计数、空间重排）的评估。

Conclusion: UniEditBench为现代视觉编辑方法提供了实用且可复现的评估协议，其基准和奖励模型已开源，有助于推动领域标准化评估。
...
### [64] [Watching Movies Like a Human: Egocentric Emotion Understanding for Embodied Companions](https://arxiv.org/abs/2604.15823)
*Ze Dong,Hao Shi,Zejia Gao,Zhonghua Yi,Kaiwei Wang,Lin Wang*

Main category: cs.CV

TL;DR: 本文提出了首个面向自我中心屏幕视角电影情感理解的基准数据集ESE，并构建了多模态长上下文情感推理框架，以解决现有研究在真实观看场景中的领域泛化问题。


<details>
  <summary>Details</summary>
Motivation: 现有电影情感理解研究主要基于电影原生镜头，而现实中的具身机器人通常通过自我中心屏幕视角感知电影，存在视角畸变、尺度变化等域偏移问题，导致跨域泛化能力受限。

Method: 1. 构建ESE数据集：包含224个受控条件下采集的电影预告片，28,667个时序对齐关键帧，采用置信度感知的多标签标注协议；2. 提出多模态长上下文情感推理框架，整合时序视觉证据、叙事摘要、压缩历史上下文和音频线索。

Result: 跨域实验显示严重领域差距：在电影原生数据训练的模型在自我中心屏幕视角数据上Macro-F1从27.99降至16.69；使用ESE训练显著提升模型在真实观看条件下的鲁棒性；所提方法性能与强闭源多模态模型相当。

Conclusion: 领域特定数据和长上下文多模态推理对真实场景情感理解至关重要，ESE数据集和所提框架为具身智能的跨域情感理解提供了有效解决方案。
...
### [65] [SSFT: A Lightweight Spectral-Spatial Fusion Transformer for Generic Hyperspectral Classification](https://arxiv.org/abs/2604.15828)
*Alexander Musiat,Nikolas Ebert,Oliver Wasenmüller*

Main category: cs.CV

TL;DR: 提出轻量级光谱-空间融合Transformer（SSFT），通过分解光谱与空间路径学习并交叉注意力融合，在异构HSI基准上实现SOTA性能且参数量极少


<details>
  <summary>Details</summary>
Motivation: 高光谱成像面临高维、光谱冗余、标记数据稀缺及域偏移等挑战，尤其在非地球观测领域数据稀缺且不平衡，需紧凑模型适应多样化采集场景

Method: 设计双路径Transformer架构：光谱路径学习波长依赖特征，空间路径提取结构信息，通过交叉注意力融合互补信息，参数量小于先前方法的2%

Result: 在异构多数据集HSI-Benchmark上取得最优综合性能；在更大规模SpectralEarth基准迁移评估中保持竞争力；消融实验证实双路径必要性且空间建模贡献最大

Conclusion: SSFT通过高效的光谱-空间解耦与融合机制，实现了轻量化高光谱分类，在数据稀缺场景下具有强鲁棒性，为跨领域应用提供实用解决方案
...
### [66] [Beyond Text Prompts: Precise Concept Erasure through Text-Image Collaboration](https://arxiv.org/abs/2604.15829)
*Jun Li,Lizhi Xiong,Ziqiang Li,Weiwei Jiang,Zhangjie Fu,Yong Li,Guo-Sen Xie*

Main category: cs.CV

TL;DR: 提出TICoE框架，通过文本-图像协同机制实现精确的概念擦除，提升文本到图像生成的安全性和可控性


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型可能因训练数据中的隐含偏见产生不安全内容，而现有概念擦除方法存在擦除不彻底或过度擦除的问题

Method: 采用连续凸概念流形和分层视觉表示学习，构建文本-图像协同擦除框架，并引入面向保真度的评估策略

Result: 在多个基准测试中，TICoE在概念擦除精度和内容保真度方面优于现有方法

Conclusion: TICoE能够精确移除目标概念同时保留无关内容，为更安全可控的文本到图像生成提供了有效解决方案
...
### [67] [Learning to Look before Learning to Like: Incorporating Human Visual Cognition into Aesthetic Quality Assessment](https://arxiv.org/abs/2604.15853)
*Liwen Yu,Chi Liu,Xiaotong Han,Congcong Zhu,Minghao Wang,Sheng Shen*

Main category: cs.CV

TL;DR: 提出AestheticNet，一种受人类视觉认知启发的图像美学质量评估新范式，通过视觉注意力和语义感知双路径架构提升评估性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动化美学质量评估方法主要基于静态像素向量和语义感知，与人类动态视觉探索的审美认知过程存在差异，需要更贴近人类视觉认知的评估模型。

Method: 采用双路径架构：视觉注意力路径使用基于眼动数据预训练的GAVE编码器模拟人类视觉系统；语义路径使用CLIP等固定语义编码器；通过跨注意力融合两路径信息。

Result: 实验验证表明，该方法相比仅使用语义的基线模型有持续改进，视觉注意力模块可作为模型无关的校正器兼容多种AQA骨干网络。

Conclusion: 人类视觉认知对于美学质量评估具有必要性和模块化价值，视觉注意力提供的认知先验能有效增强语义感知，提升评估性能。
...
### [68] [AHS: Adaptive Head Synthesis via Synthetic Data Augmentations](https://arxiv.org/abs/2604.15857)
*Taewoong Kang,Hyojin Jang,Sohyun Jeong,Seunggi Moon,Gihwi Kim,Hoon Jin Jung,Jaegul choo*

Main category: cs.CV

TL;DR: 提出自适应头部合成（AHS）方法，用于处理全身图像中的头部替换，克服现有方法在头部姿态、表情和发型多样性上的限制。


<details>
  <summary>Details</summary>
Motivation: 当前头部替换方法主要基于面部裁剪数据，视角有限，难以处理真实场景中多样的头部姿态、表情和发型，且无法实现面部区域外的自然融合。

Method: 采用自适应头部合成（AHS）框架，结合头部重演合成数据增强策略，无需配对训练数据，提升对不同面部表情和方向的泛化能力。

Result: AHS在真实场景中表现优异，能生成视觉一致的结果，保持身份和表情保真度，并在剧烈表情变化和头部姿态变化时保持鲁棒性。

Conclusion: AHS方法有效解决了现有头部替换技术的局限性，在多样化的真实场景中实现了高质量的头部合成。
...
### [69] [Mind's Eye: A Benchmark of Visual Abstraction, Transformation and Composition for Multimodal LLMs](https://arxiv.org/abs/2604.16054)
*Rohit Sinha,Aditya Kanade,Sai Srinivas Kancheti,Vineeth N Balasubramanian,Tanuja Ganu*

Main category: cs.CV

TL;DR: 论文提出名为'Mind's Eye'的视觉认知基准测试，评估多模态大语言模型在视觉空间推理能力上的表现，发现其远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉语言基准测试中表现优异，但其视觉认知和视觉空间推理能力尚未得到充分理解，需要更符合认知科学的评估框架。

Method: 设计包含八项视觉认知任务的单选题基准测试，采用'A-R-T'分类法（抽象、关系、转换），评估多种闭源和开源MLLM模型，并与人类参与者进行对比。

Result: 人类平均准确率达80%，而表现最佳的MLLM模型低于50%。错误分析显示模型在视觉注意力分配、内部感知操作和视觉概念抽象方面存在缺陷。

Conclusion: 当前MLLM模型的视觉空间推理能力有限，与人类存在显著差距，需要开发更具认知基础的评估框架来推动该领域发展。
...
### [70] [Splats in Splats++: Robust and Generalizable 3D Gaussian Splatting Steganography](https://arxiv.org/abs/2604.15862)
...
TL;DR: 提出Splats in Splats++框架，在3D高斯泼溅表示中嵌入高容量3D/4D内容，实现安全隐写而不影响渲染质量


<details>
  <summary>Details</summary>
Motivation: 3DGS资产版权保护需求迫切，现有隐写方法难以兼顾安全嵌入、高容量和资产实用性，常破坏渲染流程或易受攻击

Method: 基于球谐系数频率分析的重要性分级加密方案；哈希网格引导的不透明度映射机制；梯度门控不透明度一致性损失

Result: 比现有方法消息保真度高6.28 dB，渲染速度快3倍，对GSPure等3D结构攻击具有强鲁棒性，可扩展到2D图像嵌入和4D动态场景

Conclusion: 该框架实现了管道无关的隐写，在保持原始表达能力的同时确保嵌入不可感知，为3DGS资产版权保护提供了有效解决方案
...
### [71] [Chain-of-Thought Degrades Visual Spatial Reasoning Capabilities of Multimodal LLMs](https://arxiv.org/abs/2604.16060)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于思维链的多模态推理模型在数学和逻辑问题上表现出色，但其在广义空间智能任务上的有效性尚未得到充分验证，需要评估这些模型在空间推理方面的实际能力。

Method: 对17个模型在13个空间基准测试上进行全面评估，使用思维链提示方法，并通过新颖的No-Image++消融实验分析模型行为。

Result: 思维链提示在视觉空间推理任务中持续降低模型性能；多模态推理模型和思维链提示的语言模型存在严重捷径学习问题；即使没有图像输入，模型也会从文本先验中幻觉出视觉细节。

Conclusion: 纯文本思维链方法在空间任务上效果有限，需要开发以视觉为中心的新型推理范式来解决空间智能挑战。
...
### [72] [CLOTH-HUGS: Cloth Aware Human Gaussian Splatting](https://arxiv.org/abs/2604.15875)
*Sadia Mubashshira,Nazanin Amini,Kevin Desai*

Main category: cs.CV

TL;DR: Cloth-HUGS是一种基于高斯溅射的神经渲染框架，用于逼真的着装人体重建，通过分离身体和服装层实现高质量渲染


<details>
  <summary>Details</summary>
Motivation: 现有方法将服装与身体融合为单一表示，难以处理宽松衣物和复杂变形，需要更精细的分离表示来提升真实感

Method: 使用共享规范空间中的独立高斯层分别表示身体和服装，结合SMPL驱动的关节变形、物理约束（模拟一致性、ARAP正则化、掩码监督）以及深度感知多通道渲染策略

Result: 在多个基准测试中优于现有方法，LPIPS降低达28%，实现超过60 FPS的实时渲染，并产生时间一致的服装动态效果

Conclusion: Cloth-HUGS通过显式分离身体与服装表示，结合物理约束和高效渲染策略，显著提升了着装人体重建的感知质量和几何保真度
...
### [73] [Early Detection of Acute Myeloid Leukemia (AML) Using YOLOv12 Deep Learning Model](https://arxiv.org/abs/2604.16082)
*Enas E. Ahmed,Salah A. Aly,Mayar Moner*

Main category: cs.CV

TL;DR: 使用YOLOv12深度学习模型对急性髓系白血病细胞进行多分类，结合细胞和细胞核特征分割方法，最高准确率达99.3%。


<details>
  <summary>Details</summary>
Motivation: 急性髓系白血病（AML）是一种危及生命的血癌，由于不同细胞类型间视觉相似性高，其准确分类一直具有挑战性。

Method: 采用YOLOv12深度学习模型，结合基于细胞和细胞核特征的两种分割方法，使用色调通道和Otsu阈值技术进行图像预处理。

Result: 实验表明，基于细胞分割的YOLOv12模型结合Otsu阈值处理，在验证集和测试集上均达到最高准确率99.3%。

Conclusion: YOLOv12模型结合适当的图像预处理技术，能够有效实现AML细胞的高精度多分类，为白血病诊断提供有力工具。
...
### [74] [PolarMAE: Efficient Fetal Ultrasound Pre-training via Semantic Screening and Polar-Guided Masking](https://arxiv.org/abs/2604.15893)
*Meng Lv,Yapeng Li,Hang Su,Juhua Liu,Bo Du*

Main category: cs.CV

TL;DR: 提出PolarMAE框架，针对胎儿超声图像特性进行无监督预训练，通过渐进视觉语义筛选、声学边界区域约束和极坐标纹理协同掩码，显著提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 胎儿超声智能解读需要大量标注且存在操作者差异，无监督预训练是可行方案，但现有方法未充分考虑超声图像的数据冗余、扇形局部性和极坐标波束形成等特性。

Method: 1. 渐进视觉语义筛选（PVSS）自适应提取高价值样本；2. 声学边界区域约束（ABRC）聚焦有效声学区域；3. 极坐标纹理协同掩码（PTCM）捕捉径向成像模式和组织结构。

Result: 在多个数据集和下游任务上的实验表明，该方法实现了最先进的性能，并具有强大的预训练可扩展性和效率。

Conclusion: PolarMAE通过针对性设计有效解决了超声图像预训练中的关键挑战，为胎儿超声智能解读提供了高效解决方案。
...
### [75] [Efficient Video Diffusion Models: Advancements and Challenges](https://arxiv.org/abs/2604.15911)
*Shitong Shao,Lichen Bai,Pengfei Wan,James Kwok,Zeke Xie*

Main category: cs.CV

TL;DR: 本文系统综述了高效视频扩散模型的研究进展，提出了四类主要范式，分析了算法趋势与设计选择，并讨论了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型虽能生成高质量视频，但推理成本高昂，时空token增长与迭代去噪导致注意力与内存成为实际部署的主要瓶颈，需要系统性的效率优化方法。

Method: 提出统一分类框架，将现有方法归纳为四类范式：步骤蒸馏、高效注意力、模型压缩、缓存/轨迹优化，并分析各类方法如何减少函数评估次数与降低单步开销。

Result: 建立了首个高效视频扩散模型的系统性综述框架，明确了不同优化范式的核心目标与设计逻辑，为研究者与工程师提供了结构化领域概览。

Conclusion: 未来需关注复合加速下的质量保持、软硬件协同设计、鲁棒实时长序列生成、标准化评估基础设施等方向，推动视频扩散模型的实用化部署。
...
### [76] [AeroDeshadow: Physics-Guided Shadow Synthesis and Penumbra-Aware Deshadowing for Aerospace Imagery](https://arxiv.org/abs/2604.15903)
*Wei Lu,Zi-Yang Bo,Fei-Fei Sang,Yi Liu,Xue Yang,Si-Bao Chen*

Main category: cs.CV

TL;DR: 提出AeroDeshadow框架，通过物理引导的阴影合成和半影感知修复，解决航空影像中阴影去除的配对数据缺乏和宽过渡带处理难题。


<details>
  <summary>Details</summary>
Motivation: 航空影像中的阴影会导致光谱失真和信息丢失，影响后续解译任务。现有深度学习方法面临两大挑战：缺乏严格配对的训练数据，以及传统均匀阴影假设无法处理航空场景中广泛存在的半影过渡带。

Method: 提出两阶段框架：1) PDSS-Net通过显式建模光照衰减和空间衰减，构建具有软边界过渡的大规模合成配对数据集AeroDS-Syn；2) PCDS-Net将输入解耦为本影和半影分量，通过渐进式修复减轻边界伪影和过度校正。

Result: 仅使用合成数据训练的模型在合成和真实航空影像数据集上均达到最先进的定量精度和视觉保真度，且无需真实配对标注即可泛化到真实场景。

Conclusion: AeroDeshadow通过物理引导的阴影合成和半影感知修复，有效解决了航空影像阴影去除的数据和过渡带处理问题，具有实际应用价值。
...
### [77] [Stylistic-STORM (ST-STORM) : Perceiving the Semantic Nature of Appearance](https://arxiv.org/abs/2604.16086)
*Hamed Ouattara,Pierre Duthon,Pascal Houssam Salmane,Frédéric Bernardin,Omar Ait Aider*

Main category: cs.CV

TL;DR: 提出ST-STORM自监督学习框架，通过解耦内容和风格分支，在保持语义不变性的同时有效捕捉外观特征，适用于天气分析和医学检测等任务。


<details>
  <summary>Details</summary>
Motivation: 传统自监督学习（如MoCo、DINO）通过忽略图像外观变化（如光照、几何变换）来学习鲁棒表示，但在外观本身是关键判别信号的应用（如天气分析、自动驾驶）中，这可能导致忽略重要信息（如雨雪纹理、大气散射），带来风险。

Method: ST-STORM采用混合自监督架构，包含两个解耦的潜在流：内容分支使用JEPA方案和对比目标，学习对外观变化不变的语义表示；风格分支通过特征预测和重建，在对抗约束下捕捉外观特征（如纹理、对比度、散射）。

Result: 在多个任务中评估：风格分支有效隔离复杂外观现象（在Multi-Weather上F1=97%，在ISIC 2024上使用10%标注数据F1=94%），内容分支保持语义性能（在ImageNet-1K上F1=80%），并改善了关键外观特征的保留。

Conclusion: ST-STORM框架成功解耦了内容和风格表示，在需要外观信息的应用中（如天气表征、医学检测）表现出色，同时不损害语义识别能力，为自监督学习提供了更灵活和安全的解决方案。
...
### [78] [SWNet: A Cross-Spectral Network for Camouflaged Weed Detection](https://arxiv.org/abs/2604.16147)
*Henry O. Velesaca,Luigi Miranda,Angel D. Sappa*

Main category: cs.CV

TL;DR: SWNet是一个双模态端到端跨光谱网络，专门用于在密集农业环境中检测伪装杂草，通过结合可见光和近红外信息以及边缘感知细化模块，在Weeds-Banana数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 植物伪装（入侵物种模仿主要作物的表型特征）给传统计算机视觉系统带来重大挑战，需要新技术来在复杂农业环境中准确检测杂草。

Method: 使用Pyramid Vision Transformer v2骨干网络捕获长距离依赖，通过双模态门控融合模块动态整合可见光和近红外信息，并采用边缘感知细化模块生成更清晰的对象边界。

Result: 在Weeds-Banana数据集上的实验表明，SWNet在分割精度上优于十种最先进的方法，证明了跨光谱数据和边界引导细化在复杂作物冠层中的有效性。

Conclusion: 跨光谱数据和边界引导细化的整合对于复杂作物冠层中的高精度分割至关重要，SWNet为农业环境中的伪装杂草检测提供了有效解决方案。
...
### [79] [Making Image Editing Easier via Adaptive Task Reformulation with Agentic Executions](https://arxiv.org/abs/2604.15917)
*Bo Zhao,Kairui Guo,Runnan Du,Haiyang Sun,Pengshan Wang,Huan Yang,Kun Gai,Yixin Cao,Wei Ji*

Main category: cs.CV

TL;DR: 提出自适应任务重构框架，通过MLLM智能体动态分析、路由、重构和反馈优化图像编辑任务，在不修改底层模型的情况下提升编辑性能。


<details>
  <summary>Details</summary>
Motivation: 现有指令引导的图像编辑方法在许多看似简单的案例中仍不可靠，研究发现失败主要源于任务表述不当（如小目标、隐式空间关系、指令不明确），而非模型能力不足。

Method: 将原始图像-指令对转化为由MLLM智能体通过分析、路由、重构和反馈驱动优化动态确定并执行的序列操作，形成自适应任务重构框架。

Result: 在ImgEdit、PICA、RePlan等多个基准测试中，针对Qwen Image Edit和Nano Banana等不同编辑骨干模型均取得一致改进，尤其在挑战性案例上提升显著。

Conclusion: 任务重构是影响编辑性能的关键但未被充分探索的因素，通过使编辑任务更好地匹配现有模型的有效操作范围，可实现实质性性能提升。
...
### [80] [AIFIND: Artifact-Aware Interpreting Fine-Grained Alignment for Incremental Face Forgery Detection](https://arxiv.org/abs/2604.16207)
...
Method: 设计基于语义锚点的AIFIND框架，包括：1) 利用低层伪造痕迹生成不变语义锚点的先验生成器；2) 通过注意力机制将视觉特征与语义锚点对齐；3) 保持语义锚点角度关系的自适应决策协调器。

Result: 在多种增量协议上的实验验证了AIFIND的优越性，能有效稳定增量学习过程。

Conclusion: AIFIND通过显式约束特征空间，实现了更稳定和有效的增量人脸伪造检测。
...
### [81] [Neural Gabor Splatting: Enhanced Gaussian Splatting with Neural Gabor for High-frequency Surface Reconstruction](https://arxiv.org/abs/2604.15941)
*Haato Watanabe,Nobuyuki Umetani*

Main category: cs.CV

TL;DR: 提出神经Gabor溅射方法，通过为每个高斯基元添加轻量级MLP来建模颜色变化，结合频率感知的致密化策略，解决3DGS在高频细节场景中基元数量激增的问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）在处理高频外观细节场景时，由于每个基元只能表示单一颜色，导致基元数量急剧增加，限制了其效率和实用性。

Method: 1. 神经Gabor溅射：为高斯基元增强轻量级多层感知机（MLP），使单个基元能建模更广的颜色变化范围；2. 频率感知致密化策略：基于频率能量选择不匹配的基元进行剪枝和克隆，控制基元数量。

Result: 方法在Mip-NeRF360和棋盘格等高频率数据集上实现了对挑战性高频表面的精确重建，并通过消融实验验证了有效性。

Conclusion: 所提方法通过增强基元的颜色建模能力和智能基元管理，显著提升了3DGS在高频细节场景中的表现，平衡了重建质量与计算效率。
...
### [82] [From Competition to Coopetition: Coopetitive Training-Free Image Editing Based on Text Guidance](https://arxiv.org/abs/2604.15948)
*Jinhao Shen,Haoqian Du,Xulu Zhang,Xiao-Yong Wei,Qing Li*

Main category: cs.CV

TL;DR: 提出CoEdit框架，通过合作竞争注意力控制实现零样本图像编辑，在空间和时间维度上协调编辑与重建分支，提升编辑质量和结构保持。


<details>
  <summary>Details</summary>
Motivation: 现有免训练图像编辑方法采用编辑与重建分支对抗的范式，导致语义冲突和不可预测结果，缺乏分支间协调。

Method: 1. 空间上：提出双熵注意力操纵，将注意力控制转化为和谐最大化问题；2. 时间上：提出熵潜在细化机制，动态调整潜在表示；3. 提出保真度约束编辑评分联合评估语义编辑和背景保真度。

Result: 在标准基准测试中，CoEdit在编辑质量和结构保持方面均取得优越性能。

Conclusion: CoEdit通过合作竞争范式实现编辑和谐，增强了视觉与文本模态间的有效交互，提升了多媒体信息利用效率。
...
### [83] [MMGait: Towards Multi-Modal Gait Recognition](https://arxiv.org/abs/2604.15979)
*Chenye Wang,Qingyuan Cai,Saihui Hou,Aoqi Li,Yongzhen Huang*

Main category: cs.CV

TL;DR: 提出多模态步态识别基准MMGait，包含五种传感器、十二种模态数据，并设计统一模型OmniGait处理单模态、跨模态和多模态任务。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别方法主要依赖RGB模态，难以应对真实场景中多模态协作与跨模态检索的需求，缺乏统一的多模态基准数据集。

Method: 构建包含RGB、深度、红外、LiDAR和4D雷达五种传感器的多模态数据集MMGait；提出OmniGait基线模型，学习跨模态共享嵌入空间。

Result: MMGait包含725名受试者的334,060个序列；OmniGait在单模态、跨模态和多模态任务中均表现出有竞争力的识别性能。

Conclusion: MMGait为多模态步态识别研究提供了系统基准，OmniGait证明了统一建模多模态步态识别任务的可行性，相关资源已开源。
...
### [84] [A Two-Stage, Object-Centric Deep Learning Framework for Robust Exam Cheating Detection](https://arxiv.org/abs/2604.16234)
*Van-Truong Le,Le-Khanh Nguyen,Trong-Doanh Nguyen*

Main category: cs.CV

TL;DR: 提出一种基于YOLOv8n和RexNet-150的两阶段考试作弊检测系统，在27万样本数据集上取得高精度与快速推理性能，并注重隐私保护。


<details>
  <summary>Details</summary>
Motivation: 传统人工监考效率低、成本高且易出错；现有AI监考系统常缺乏透明度或需复杂架构。需开发高效、可扩展且符合伦理的作弊检测方案。

Method: 采用两阶段框架：先用YOLOv8n定位考生区域，再通过微调的RexNet-150对裁剪图像进行行为分类（正常/作弊）。使用10个来源共273,897个样本的数据集进行训练。

Result: 系统准确率达0.95，召回率0.94，精确率0.96，F1分数0.95，较视频基线准确率提升13%；平均推理时间13.9毫秒/样本，适合大规模部署。

Conclusion: 该系统为实时、可扩展、开源且符合伦理的监考方案奠定基础，未来可通过加入音频、连续帧等进一步提升精度。
...
### [85] [IA-CLAHE: Image-Adaptive Clip Limit Estimation for CLAHE](https://arxiv.org/abs/2604.16010)
*Rikuto Otsuka,Yuho Shoji,Yuka Ogino,Takahiro Toizumi,Atsushi Ito*

Main category: cs.CV

TL;DR: 提出IA-CLAHE方法，通过自适应估计局部区域的对比度限制参数，避免传统CLAHE的过度增强问题，无需任务特定训练数据即可提升视觉质量和识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统CLAHE在计算机视觉任务和工业应用中广泛使用，但其固定的对比度限制参数常导致局部区域过度增强，影响效果。

Method: 训练轻量级剪裁限制估计器，结合可微分的CLAHE扩展实现端到端优化，通过将输入图像直方图映射到域不变均匀分布来学习自适应参数。

Result: 实验表明IA-CLAHE能一致提升识别性能，同时改善人类感知的视觉质量，且无需任务特定训练数据即可泛化到多样条件。

Conclusion: IA-CLAHE通过自适应参数估计解决了传统CLAHE的局限性，实现了零样本泛化，在提升视觉质量和任务性能方面具有优势。
...
### [86] [Breakout-picker: Reducing false positives in deep learning-based borehole breakout characterization from acoustic image logs](https://arxiv.org/abs/2604.16011)
*Guangyu Wang,Xiaodong Ma,Xinming Wu*

Main category: cs.CV

TL;DR: 本研究开发了一个名为Breakout-picker的深度学习框架，通过引入负样本训练和方位对称性验证，显著降低了钻孔崩落自动识别中的误报率。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的钻孔崩落自动识别方法常因非崩落特征（如天然裂缝、键槽等）的误分类导致高误报率，影响了地应力分析的可靠性。

Method: 1. 在训练中引入非崩落特征的负样本（天然裂缝、键槽、测井伪影）；2. 对候选崩落应用方位对称性准则进行二次验证，排除不对称的检测结果。

Result: 在三个不同区域的声波成像测井数据集上测试表明，Breakout-picker相比其他自动方法具有更高准确率和显著更低的误报率。

Conclusion: Breakout-picker通过降低误报率提高了声波成像测井中崩落自动识别的可靠性，有助于基于钻孔崩落的地应力分析。
...
### [87] [Ranking XAI Methods for Head and Neck Cancer Outcome Prediction](https://arxiv.org/abs/2604.16034)
*Baoqiang Ma,Djennifer K. Madzia-Madzou,Rosa C. J. Kraaijveld,Jin Ouyang*

Main category: cs.CV

TL;DR: 该研究首次全面评估了13种可解释AI方法在头颈癌预后预测中的表现，发现集成梯度和DeepLIFT在多个评估维度表现最佳。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在头颈癌预后预测中表现优异，但其可解释性不足阻碍了临床实际应用，现有研究缺乏对可解释AI方法的系统评估。

Method: 在HECKTOR多中心数据集上，使用24个指标从忠实性、鲁棒性、复杂性和合理性四个维度，对13种可解释AI方法进行全面评估和排名。

Result: 不同可解释AI方法在各评估维度存在显著差异，其中集成梯度和DeepLIFT在忠实性、复杂性和合理性方面持续获得高排名。

Conclusion: 研究强调了全面评估可解释AI方法的重要性，该评估框架可扩展至其他医学影像任务，促进AI在临床中的可信应用。
...
### [88] [Elucidating the SNR-t Bias of Diffusion Probabilistic Models](https://arxiv.org/abs/2604.16044)
*Meng Yu,Lei Sun,Jianhao Zeng,Xiangxiang Chu,Kun Zhan*

Main category: cs.CV

TL;DR: 本文发现扩散概率模型存在SNR-t偏差问题，并提出一种差分校正方法，通过分频处理显著提升多种扩散模型的生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在推理阶段存在信噪比与时间步长不对齐的问题（SNR-t偏差），导致误差累积和生成质量下降，需要解决这一偏差以提升模型性能。

Method: 提出差分校正方法，将样本分解为不同频率分量，并对每个分量分别进行校正，以匹配训练时信噪比与时间步长的严格对应关系。

Result: 在多种扩散模型（如IDDPM、ADM、DDIM等）和不同分辨率数据集上的实验表明，该方法能显著提升生成质量，且计算开销可忽略。

Conclusion: SNR-t偏差是扩散模型的重要问题，通过分频差分校正可有效缓解该偏差，提升模型生成性能，具有广泛适用性。
...
### [89] [VEFX-Bench: A Holistic Benchmark for Generic Video Editing and Visual Effects](https://arxiv.org/abs/2604.16272)
...
TL;DR: 该研究提出了VEFX-Dataset（大规模人工标注视频编辑数据集）、VEFX-Reward（专用视频编辑质量评估模型）和VEFX-Bench（标准化评测基准），以解决当前视频编辑领域缺乏高质量数据集和专用评估工具的问题。


<details>
  <summary>Details</summary>
Motivation: 随着AI辅助视频创作的普及，指令引导的视频编辑需求日益增长，但当前领域缺乏大规模人工标注的完整编辑示例数据集，以及标准化的编辑系统评估方法。现有资源规模小、缺少编辑输出或人工质量标签，而评估多依赖昂贵的人工检查或非专用的通用视觉语言模型。

Method: 1）构建VEFX-Dataset：包含5,049个视频编辑示例，涵盖9个主要编辑类别和32个子类别，每个示例在三个解耦维度（指令遵循、渲染质量、编辑排他性）上进行人工标注；2）开发VEFX-Reward：基于VEFX-Dataset训练的视频编辑质量奖励模型，通过联合处理源视频、编辑指令和编辑后视频，使用序数回归预测各维度质量分数；3）发布VEFX-Bench：包含300个精选视频-提示对的标准化评测基准。

Result: 实验表明：1）VEFX-Reward在标准IQA/VQA指标和分组偏好评估中，比通用VLM评估器和现有奖励模型更符合人类判断；2）使用VEFX-Reward评估代表性商业和开源视频编辑系统，揭示了当前模型在视觉合理性、指令遵循和编辑局部性方面仍存在明显差距。

Conclusion: 该研究通过构建大规模标注数据集、专用评估模型和标准化基准，为视频编辑领域提供了重要的资源和评估工具，有助于推动AI辅助视频编辑技术的发展和质量提升。
...
### [90] [The Amazing Stability of Flow Matching](https://arxiv.org/abs/2604.16079)
*Rania Briq,Michael Kamp,Ohad Fried,Sarel Cohen,Stefan Kesselheim*

Main category: cs.CV

TL;DR: 本文研究发现流匹配模型在数据剪裁、架构调整和训练配置变化下表现出惊人的稳定性，生成样本的质量和多样性得以保持，潜在表征变化微小。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型通常被认为依赖特定架构和大规模训练数据，本研究旨在探究这些因素对流匹配模型样本生成质量与多样性的实际影响。

Method: 在CelebA-HQ数据集上进行实验，通过剪裁50%数据集、改变模型架构和调整训练配置等方式，评估流匹配模型的稳定性。

Result: 即使剪裁一半数据，生成样本的质量和多样性仍得以保持；潜在表征变化微小，相同种子在不同训练条件下生成视觉相似的输出；模型在不同架构和配置变化下均表现出稳定性。

Conclusion: 流匹配模型具有强大的稳定性，能够抵御数据、架构和训练配置等多种扰动，这解释了其在实际应用中的可靠性。
...
### [91] [DINOv3 Beats Specialized Detectors: A Simple Foundation Model Baseline for Image Forensics](https://arxiv.org/abs/2604.16083)
*Jieming Yu,Qiuxiao Feng,Zhuohan Wang,Xiaochen Ma*

Main category: cs.CV

TL;DR: 提出基于DINOv3与LoRA适配的简单高效图像伪造定位基线方法，在多项基准测试中显著超越现有方法，且具有强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有伪造图像定位方法设计复杂且泛化能力有限，难以适应多种篡改类型和成像条件，需要更简单有效的解决方案。

Method: 采用冻结的ViT-L骨干网络，结合LoRA适配和轻量级卷积解码器，仅使用910万可训练参数构建模型。

Result: 在CAT-Net协议下平均像素级F1提升17.0点，MVSS-Net协议下达到0.774 F1（此前最佳为0.530），且对噪声、JPEG重压缩和模糊具有强鲁棒性。

Conclusion: 该方法可作为图像取证研究的可靠基线，预训练表征中的取证信息通过轻量适配得以更好保留，LoRA微调策略优于全参数微调。
...
### [92] [Do Vision-Language Models Truly Perform Vision Reasoning? A Rigorous Study of the Modality Gap](https://arxiv.org/abs/2604.16256)
*Yige Xu,Yongjie Wang,Zizhuo Wu,Kaisong Song,Jun Lin,Zhiqi Shen*

Main category: cs.CV

TL;DR: 本文提出了CrossMath基准测试，用于评估视觉语言模型（VLMs）是否真正依赖视觉信息进行推理。研究发现当前VLMs主要依赖文本推理，视觉输入反而会降低性能。通过微调训练集，可以提升模型的多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多种任务中表现出色，但尚不清楚其优异性能是源于真正的视觉推理能力，还是主要依赖其文本骨干网络的推理能力。需要系统性地测量模型在多模态推理中对视觉信息的实际依赖程度。

Method: 设计了CrossMath多模态推理基准测试，将每个问题构建为纯文本、纯图像和图像+文本三种格式，确保任务相关信息完全一致。通过人工标注验证对齐质量，从而隔离模态特异性差异。对先进VLMs进行广泛评估，并构建CrossMath训练集进行模型微调。

Result: 评估发现：文本推理与视觉推理之间存在显著性能差距；VLMs在纯文本输入下表现优异，但加入视觉数据（图像+文本）后性能经常低于纯文本基线。这表明当前VLMs主要在文本空间进行推理，对视觉证据的真实依赖有限。微调CrossMath训练集后，模型在所有单独和联合模态上的推理性能均显著提升，并在两个通用视觉推理任务上获得稳健增益。

Conclusion: 当前视觉语言模型尚未充分实现真正的视觉基础推理，主要依赖文本推理能力。CrossMath基准测试能有效评估多模态推理中的模态依赖问题，通过针对性微调可以提升模型对视觉信息的利用能力，推动更平衡的多模态推理发展。
...
### [93] [TableSeq: Unified Generation of Structure, Content, and Layout](https://arxiv.org/abs/2604.16070)
*Laziz Hamdi,Amine Tamasna,Pascal Boisson,Thierry Paquet*

Main category: cs.CV

TL;DR: TableSeq是一个仅使用图像的端到端框架，将表格结构识别、内容识别和单元格定位统一为单一序列生成任务，通过单个解码器生成HTML标签、单元格文本和坐标标记的混合流，在多个基准测试中取得竞争性或最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有表格识别方法通常依赖外部OCR、多个解码器或复杂的多阶段后处理流程，导致系统复杂且效率低下。本研究旨在开发一个统一、简洁的端到端框架，避免这些复杂性。

Method: 将表格识别任务建模为单一序列生成问题，使用轻量级高分辨率FCN-H16编码器、最小化结构先验头和单层Transformer编码器，通过自回归解码器生成包含HTML标签、文本内容和离散化坐标的混合序列。

Result: 在PubTabNet上达到95.23 TEDS/96.83 S-TEDS，FinTabNet上97.45 TEDS/98.69 S-TEDS，SciTSR上精度/召回率/F1分别为99.79/99.54/99.66，同时在PubTables-1M上保持竞争力。该框架还能泛化到基于索引的表格查询任务。

Conclusion: TableSeq提供了一个实用且可复现的单流基线框架，通过统一的序列接口实现了表格识别的简化，同时保持了高性能。多标记预测技术进一步降低了推理延迟，代码将开源以促进研究。
...
### [94] [DenTab: A Dataset for Table Recognition and Visual QA on Real-World Dental Estimates](https://arxiv.org/abs/2604.16099)
*Laziz Hamdi,Amine Tamasna,Thierry Paquet*

Main category: cs.CV

TL;DR: 提出了DenTab数据集，包含2000张牙科估算表格图像及HTML标注，用于评估表格识别和视觉问答，并设计了Table Router Pipeline提升算术问题的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有表格识别和视觉问答数据集多基于干净的数字源或渲染表格，未能充分反映实际行政文档中的噪声和复杂结构，需要更贴近真实场景的评估资源。

Method: 构建DenTab数据集（含2208个问题，覆盖11类任务）；评估16个系统（14个视觉语言模型+2个OCR基线）；提出Table Router Pipeline，结合视觉语言模型生成结构化表示与基于规则的执行器进行精确计算。

Result: 实验表明，即使使用真实HTML表格输入，现有模型在多步算术和一致性问题上仍存在推理失败；Table Router Pipeline无需训练即可显著提升算术可靠性。

Conclusion: DenTab填补了噪声行政表格评估数据的空白；结构恢复能力不能保证复杂推理的可靠性；Table Router Pipeline为提升表格问答的算术准确性提供了有效路径。
...
### [95] [Polyglot: Multilingual Style Preserving Speech-Driven Facial Animation](https://arxiv.org/abs/2604.16108)
...
TL;DR: 本文提出Polyglot，一种基于扩散模型的个性化多语言语音驱动面部动画方法，通过联合建模语言和个人风格特征，提升单语和多语场景下的动画质量。


<details>
  <summary>Details</summary>
Motivation: 现有语音驱动面部动画模型多基于单语数据训练，难以适应多语言现实场景；且现有方法通常单独处理语言或说话人风格，无法建模二者的交互影响，限制了生成动画的真实性。

Method: 提出统一扩散架构Polyglot：使用文本嵌入编码语言信息，从参考面部序列提取风格嵌入捕捉个人说话特征；无需预定义语言或说话人标签，通过自监督学习实现跨语言和说话人的泛化。

Result: 实验表明，该方法在单语和多语设置下均表现更优，能有效捕捉节奏、发音习惯和面部运动等表达特征，生成时序连贯且真实的面部动画。

Conclusion: Polyglot为语音驱动面部动画中的语言与个人风格建模提供了统一框架，通过联合条件建模实现了更真实的多语言个性化动画生成。
...
### [96] [From Articles to Canopies: Knowledge-Driven Pseudo-Labelling for Tree Species Classification using LLM Experts](https://arxiv.org/abs/2604.16115)
...
TL;DR: 提出一种融合生物学知识的半监督深度学习方法，结合高光谱成像与激光雷达数据，通过伪标签和物种共栖先验提升树种分类精度。


<details>
  <summary>Details</summary>
Motivation: 高光谱树种分类面临样本有限、类别不平衡、光谱混合及生态异质性等挑战，需超越单纯光谱分析，整合植被结构与生态交互特征。

Method: 基于预构建冠层图进行生物启发的伪标签生成；利用大语言模型从可靠来源自动提取物种共栖先验知识，构建共栖概率矩阵并融入伪标签策略。

Result: 在真实森林数据集上相比最佳基准方法提升5.6%分类精度；专家评估显示共栖先验准确率高，误差不超过15%。

Conclusion: 融合多源遥感数据与生态先验知识的半监督方法能有效应对复杂生态场景中的树种分类问题，以较低标注成本实现更高精度。
...
### [97] [Towards In-Context Tone Style Transfer with A Large-Scale Triplet Dataset](https://arxiv.org/abs/2604.16114)
*Yuhai Deng,Huimin She,Wei Shen,Meng Li,Ruoxi Wu,Lunxi Yuan,Xiang Li*

Main category: cs.CV

TL;DR: 本文提出ICTone框架和TST100K数据集，通过扩散模型和上下文学习实现照片色调风格迁移，解决了现有方法因数据缺乏和语义丢失导致的视觉质量问题。


<details>
  <summary>Details</summary>
Motivation: 现有色调风格迁移方法缺乏高质量大规模三元组数据集，且独立提取内容与参考特征的方式易导致语义丢失和不恰当的颜色迁移，限制了模型性能。

Method: 1) 构建包含10万组内容-参考-风格化三元组的TST100K数据集，使用训练的色调风格评分器确保风格一致性；2) 提出ICTone扩散框架，通过联合条件编码实现上下文色调迁移；3) 结合奖励反馈学习提升风格保真度和视觉质量。

Result: TST100K数据集验证有效，ICTone在定量指标和人工评估中均达到最先进性能，实现了语义感知的色调迁移。

Conclusion: 大规模高质量数据集与上下文扩散框架的结合显著提升了色调风格迁移的语义准确性和视觉美感，为相关任务提供了新解决方案。
...
### [98] [Motion-Adapter: A Diffusion Model Adapter for Text-to-Motion Generation of Compound Actions](https://arxiv.org/abs/2604.16135)
*Yue Jiang,Mingyu Yang,Liuyuxin Yang,Yang Xu,Bingxin Yun,Yuhe Zhang*

Main category: cs.CV

TL;DR: 提出Motion-Adapter模块，通过解耦交叉注意力机制解决文本生成复合动作中的灾难性遗忘和注意力崩溃问题，提升动作合成的连贯性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成动作的扩散模型在处理复合动作（如边走边挥手）时存在两个关键问题：灾难性遗忘（后生成动作覆盖前动作）和注意力崩溃（交叉注意力机制过度融合特征），导致需要依赖详细文本描述或大语言模型辅助，限制了自然复合动作的生成能力。

Method: 提出可插拔模块Motion-Adapter，在去噪过程中通过计算解耦的交叉注意力图作为结构掩码，引导文本到动作扩散模型生成复合动作，避免特征过度融合。

Result: 大量实验表明，该方法在不同文本提示下能持续生成更忠实、连贯的复合动作，性能超越现有最先进方法。

Conclusion: Motion-Adapter模块有效解决了文本生成复合动作中的关键限制，无需依赖详细描述或外部模型，显著提升了动作合成的语义表达和运动机制建模能力。
...
### [99] [neuralCAD-Edit: An Expert Benchmark for Multimodal-Instructed 3D CAD Model Editing](https://arxiv.org/abs/2604.16170)
*Toby Perrett,Matthew Bouchard,William McCarthy*

Main category: cs.CV

TL;DR: 提出了首个用于编辑专家CAD工程师创建的三维CAD模型的基准测试neuralCAD-Edit，通过录制专业设计师在CAD软件中的操作视频收集真实编辑请求，发现当前基础模型与人类专家存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有研究多基于文本条件进行CAD编辑，缺乏真实场景下专业设计师通过交互（说话、指向、绘制）表达的编辑请求数据，需要建立更贴近实际工作流程的基准测试。

Method: 招募十位专业设计师录制在CAD软件中操作时的视频（包含语音、指向和绘制动作），构建包含真实编辑请求的数据集；使用自动指标和人工评估对比主流基础模型与人类专家的编辑性能。

Result: 最佳基础模型（GPT 5.2）在人工接受度测试中比CAD专家低53%（绝对值），自动指标和人工评估均显示模型与人类专家存在巨大性能差距。

Conclusion: neuralCAD-Edit基准揭示了当前三维CAD编辑方法和基础模型的局限性，为未来相关技术发展提供了可靠的评估基础。
...
### [100] [Saturation-Aware Space-Variant Blind Image Deblurring](https://arxiv.org/abs/2604.16200)
*Muhammad Z. Alam,Larry Stetsiuk,Arooba Zeshan*

Main category: cs.CV

TL;DR: 提出了一种新颖的饱和感知空间变化盲图像去模糊框架，专门处理高动态范围和低光条件下饱和像素带来的挑战，通过分割图像区域并利用暗通道先验估计饱和区域真实辐射度，有效提升去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 在高动态范围和低光条件下，图像中的饱和像素会干扰传统去模糊方法，导致伪影（如振铃效应），因此需要一种能够专门处理饱和区域的盲图像去模糊方法。

Method: 基于模糊强度和接近饱和程度分割图像，利用预估计的光扩散函数减少杂散光影响，结合暗通道先验准确估计饱和区域的真实辐射度，避免引入伪影。

Result: 在合成和真实数据集上的实验表明，该框架在各种场景下均能改善去模糊结果，性能优于现有的饱和感知和通用去模糊方法。

Conclusion: 该框架能有效处理饱和像素问题，提升去模糊质量，并具备与现有及新兴盲图像去模糊技术集成的潜力。
...
### [101] [GAViD: A Large-Scale Multimodal Dataset for Context-Aware Group Affect Recognition from Videos](https://arxiv.org/abs/2604.16214)
*Deepak Kumar,Abhishek Pratap Singh,Puneet Kumar,Xiaobai Li,Balasubramanian Raman*

Main category: cs.CV

TL;DR: 本文介绍了GAViD数据集和CAGNet模型，用于解决野外场景中群体情感识别的数据和方法挑战。


<details>
  <summary>Details</summary>
Motivation: 野外场景中群体情感的计算建模面临大规模标注数据稀缺和多模态社交交互复杂性的挑战，现有数据集缺乏多模态和上下文信息的全面标注。

Method: 构建了包含5091个视频片段的GAViD数据集，提供视频、音频、上下文等多模态数据，并标注了情感效价和离散情绪标签；提出了基于上下文感知的群体情感识别网络CAGNet。

Result: CAGNet在GAViD数据集上达到63.20%的测试准确率，与当前最优性能相当；数据集和代码已开源。

Conclusion: GAViD数据集和CAGNet模型为多模态上下文感知的群体情感识别提供了新的资源和解决方案，有助于推动该领域的发展。
...
### [102] [CollideNet: Hierarchical Multi-scale Video Representation Learning with Disentanglement for Time-To-Collision Forecasting](https://arxiv.org/abs/2604.16240)
*Nishq Poorav Desai,Ali Etemad,Michael Greenspan*

Main category: cs.CV

TL;DR: 提出CollideNet，一种时空分层Transformer架构，用于多尺度视频碰撞时间预测，在三个公共数据集上实现SOTA性能。
...
Method: 设计时空双流架构：空间流同时聚合多分辨率帧信息；时间流通过多尺度特征编码，并解耦视频的非平稳性、趋势和季节性成分。

Result: 在三个常用公共数据集上超越先前工作，显著刷新SOTA；跨数据集评估显示良好泛化能力；可视化验证了趋势与季节性解耦的有效性。

Conclusion: CollideNet通过时空分层设计和成分解耦，有效提升了碰撞时间预测的精度与泛化能力，为多尺度视频理解提供了新思路。
...
### [103] [Find, Fix, Reason: Context Repair for Video Reasoning](https://arxiv.org/abs/2604.16243)
*Haojian Huang,Chuanyu Qin,Yinchuan Li,Yingcong Chen*

Main category: cs.CV

TL;DR: 提出一种基于冻结教师模型提供时空证据补丁的强化学习方法，通过最小干预提升视频推理能力，在多个基准测试中取得准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理方法存在知识边界瓶颈、混合策略需复杂正则化、动态上下文方法依赖两阶段调优且受限于小模型能力等问题，而大模型在多模态理解和指令跟随方面具有优势，可为小模型提供更丰富的上下文。

Method: 采用冻结的集成工具教师模型识别缺失的时空依赖关系，从原始视频中提取最小证据补丁（如时间戳、区域等），学生模型在增强上下文中重新回答，结合选择展开方案集成到GRPO中训练，并提出鲁棒改进奖励（RIR）来对齐答案正确性和证据依赖关系。

Result: 在多个相关基准测试中实现了一致的准确率提升和强泛化能力。

Conclusion: 该方法通过最小干预的观察级干预，在保持在线探索的同时沿因果有意义方向引导训练，有效提升了视频推理性能，且对训练框架改动最小。
...
### [104] [Dental Panoramic Radiograph Analysis Using YOLO26 From Tooth Detection to Disease Diagnosis](https://arxiv.org/abs/2604.16231)
*Khawaja Azfar Asif,Rafaqat Alam Khan*

Main category: cs.CV

TL;DR: 本研究首次应用YOLOv26实现全景X光片的自动牙齿检测、FDI编号及牙病分割，YOLOv26m-seg模型在牙齿计数任务中表现最佳，性能优于YOLOv8x基线模型。


<details>
  <summary>Details</summary>
Motivation: 全景X光片是牙科基础诊断工具，但人工判读耗时且易出错，临床亟需高效自动化解决方案。

Method: 使用DENTEX数据集，经Roboflow预处理（格式转换与增强）得到1082张牙齿计数图像和1040张疾病分割图像；在Google Colab上以800×800分辨率训练五种YOLOv26-seg变体模型，采用迁移学习策略。

Result: YOLOv26m-seg模型在牙齿计数任务中精度达0.976、召回率0.970、边界框mAP50为0.976，较YOLOv8x基线精度提升4.9%，mAP50提升3.3%；YOLOv26l-seg模型在疾病分割任务中边界框mAP50为0.591，掩码mAP50为0.547，其中阻生牙检测精度最高（0.943）。

Conclusion: YOLOv26模型为牙科影像自动分析提供了鲁棒且准确的框架，视觉显著性比标注数量更影响检测性能，该技术有望提升临床诊断效率与一致性。
...
### [105] [Where Do Vision-Language Models Fail? World Scale Analysis for Image Geolocalization](https://arxiv.org/abs/2604.16248)
*Siddhant Bharadwaj,Ashish Vashist,Fahimul Aleem,Shruti Vyas*

Main category: cs.CV

TL;DR: 本文系统评估了多种先进视觉语言模型在仅使用地面视角图像进行国家级别地理定位的零样本能力，揭示了模型间的显著性能差异及其在细粒度地理线索识别上的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统图像地理定位依赖检索式地点识别或几何视觉定位方法，而视觉语言模型在多模态任务中展现出强大的零样本推理能力，但其在地理推断方面的性能尚未得到充分探索。

Method: 采用零样本设置，基于提示词进行国家预测，不依赖图像匹配、GPS元数据或任务特定训练。在三个地理多样性数据集上测试了多种最先进的视觉语言模型，评估其鲁棒性和泛化能力。

Result: 不同模型间存在显著性能差异，表明语义推理在粗粒度地理定位中具有潜力，但当前视觉语言模型在捕捉细粒度地理线索方面存在局限。

Conclusion: 本研究首次对现代视觉语言模型在国家级别地理定位任务进行了集中比较，为多模态推理与地理理解交叉领域的未来研究奠定了基础。
...
### [106] [Information Router for Mitigating Modality Dominance in Vision-Language Models](https://arxiv.org/abs/2604.16264)
*Seulgi Kim,Mohit Prabhushankar,Ghassan AlRegib*

Main category: cs.CV

TL;DR: 提出MoIR方法，通过显式减少模态间信息差异来解决视觉语言模型中的模态主导问题，在多个基准测试中提升了模型鲁棒性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要通过调整注意力分配来解决模态主导问题，但现实中不同模态的信息密度和信噪比存在差异，仅调整注意力无法解决信息缺失或模糊的根本问题。

Method: 提出多模态信息路由器（MoIR），在融合前识别信息较少的token，从信息更强的模态路由补充信息，构建信息密集的token表示后再由大语言模型处理。

Result: 在三个广泛使用的多模态基准测试和多个模型架构上，MoIR始终表现出更平衡的模态贡献，提高了鲁棒性和下游性能，特别是在模态退化情况下表现更优。

Conclusion: 显式修改跨模态信息是缓解多模态推理模型中模态主导问题的有效补充策略，能够实现可靠的模态主导转移，即使某个模态质量下降时也能保持性能。
...
### [107] [Winner of CVPR2026 NTIRE Challenge on Image Shadow Removal: Semantic and Geometric Guidance for Shadow Removal via Cascaded Refinement](https://arxiv.org/abs/2604.16177)
*Lorenzo Beltrame,Jules Salzinger,Filip Svoboda,Jasmin Lampert,Phillipp Fanta-Jende,Radu Timofte,Marco Koerner*

Main category: cs.CV

TL;DR: 本文提出了一种用于CVPR2026 NTIRE WSRD+挑战赛的三阶段渐进式阴影去除流水线，基于OmniSR框架，通过迭代直接细化的方式去除阴影，结合RGB外观、冻结的DINOv2语义引导以及单目深度和表面法线的几何线索，并引入收缩约束目标以稳定多阶段优化。该方法在官方测试集上取得了最佳性能，赢得了NTIRE 2026图像阴影去除挑战赛。


<details>
  <summary>Details</summary>
Motivation: 现有的阴影去除方法在处理复杂阴影残留和伪影方面存在不足，特别是在多阶段优化过程中容易产生不稳定的重建误差。本研究旨在通过渐进式细化策略和稳定的训练机制，提升阴影去除的精度和鲁棒性。

Method: 1. 三阶段渐进式阴影去除流水线，基于OmniSR框架进行迭代直接细化；2. 结合RGB外观、冻结的DINOv2语义特征、单目深度和表面法线几何线索；3. 引入收缩约束目标，确保级联过程中重建误差非递增；4. 采用分阶段训练策略，从WSRD预训练过渡到WSRD+监督学习，最后进行WSRD+ 2026适应训练，并结合余弦退火检查点集成。

Result: 在WSRD+ 2026隐藏测试集上，最终集成模型取得了26.680 PSNR、0.8740 SSIM、0.0578 LPIPS和26.135 FID的优异指标，排名第一，赢得了NTIRE 2026图像阴影去除挑战赛。在ISTD+和UAV-SC+数据集上的进一步验证也显示了模型的强大性能。

Conclusion: 所提出的三阶段渐进式阴影去除方法通过结合多模态线索和稳定的优化策略，显著提升了阴影去除的精度和鲁棒性，在多个基准数据集上取得了最先进的性能，为图像阴影去除任务提供了有效的解决方案。
...
### [108] [Hero-Mamba: Mamba-based Dual Domain Learning for Underwater Image Enhancement](https://arxiv.org/abs/2604.16266)
*Tejeswar Pokuri,Shivarth Rai*

Main category: cs.CV

TL;DR: 提出Hero-Mamba网络，通过Mamba架构在空间域和频域并行处理，实现高效的水下图像增强，在PSNR和SSIM指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 水下图像因光线吸收和散射存在色彩失真、低对比度和细节模糊等问题；现有CNN方法难以建模长距离依赖，Transformer计算复杂度高，无法高效处理高分辨率图像。

Method: 1. 设计双域并行输入（空间域RGB图像+频域FFT分量）以解耦退化因素；2. 采用Mamba-based SS2D模块以线性复杂度捕获全局感受野；3. 引入基于背景光先验的ColorFusion模块恢复色彩。

Result: 在LSUI和UIEB数据集上超越SOTA方法，其中LSUI数据集PSNR达25.802，SSIM达0.913。

Conclusion: Hero-Mamba通过双域学习和线性复杂度的全局建模，有效解决了水下图像增强问题，兼具高性能与计算效率。
...
### [109] [Repurposing 3D Generative Model for Autoregressive Layout Generation](https://arxiv.org/abs/2604.16299)
*Haoran Feng,Yifan Niu,Zehuan Huang,Yang-Tian Sun,Chunchao Guo,Yuxin Peng,Lu Sheng*

Main category: cs.CV

TL;DR: LaviGen是一个将3D生成模型重新用于3D布局生成的框架，通过自回归过程在原生3D空间中直接生成符合几何关系和物理约束的布局，在LayoutVLM基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常从文本描述推断物体布局，缺乏对3D空间中几何关系和物理约束的显式建模，导致生成的布局可能不够连贯或物理上不合理。

Method: 提出在原生3D空间中操作的框架，将布局生成建模为自回归过程；引入适配的3D扩散模型，整合场景、物体和指令信息；采用双引导自展开蒸馏机制提升效率和空间准确性。

Result: 在LayoutVLM基准测试中，LaviGen的物理合理性比现有最佳方法高19%，计算速度快65%，实现了优越的3D布局生成性能。

Conclusion: LaviGen通过直接操作于3D空间并显式建模几何与物理约束，能够生成更连贯、物理合理的3D场景布局，为3D布局生成提供了高效准确的解决方案。
...
### [110] [Enhancing Hazy Wildlife Imagery: AnimalHaze3k and IncepDehazeGan](https://arxiv.org/abs/2604.16284)
*Shivarth Rai,Tejeswar Pokuri*

Main category: cs.CV

TL;DR: 该论文提出了AnimalHaze3k合成数据集和IncepDehazeGan架构，用于解决大气雾霾对野生动物图像质量的影响，提升计算机视觉在动物检测、跟踪和行为分析等保护应用中的性能。


<details>
  <summary>Details</summary>
Motivation: 大气雾霾严重降低了野生动物图像质量，阻碍了计算机视觉在动物检测、跟踪和行为分析等关键保护任务中的应用，因此需要开发有效的去雾方法。

Method: 通过基于物理的流程从1,159张清晰野生动物照片生成3,477张雾霾图像，构建AnimalHaze3k合成数据集；提出IncepDehazeGan架构，在GAN框架中结合初始块和残差跳跃连接。

Result: IncepDehazeGan在SSIM（0.8914）、PSNR（20.54）和LPIPS（0.1104）指标上达到最优性能，比竞争方法SSIM提高6.27%、PSNR提高10.2%；去雾图像使YOLOv11检测mAP提升112%、IoU提升67%。

Conclusion: 该方法为生态学家在恶劣环境条件下提供了可靠的种群监测工具，通过鲁棒的视觉分析显著增强了野生动物保护工作的潜力。
...
### [111] [Applied Explainability for Large Language Models: A Comparative Study](https://arxiv.org/abs/2604.15371)
*Venkata Abhinandan Kancharla*

Main category: cs.CL

TL;DR: 本文对三种可解释性技术（Integrated Gradients、Attention Rollout和SHAP）在微调DistilBERT模型上进行SST-2情感分类的实践比较研究，评估了它们在一致实验设置下的实际表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理任务中表现出色，但其决策过程难以解释，这种缺乏透明度给实际系统部署中的信任、调试带来了挑战。

Method: 在微调的DistilBERT模型上进行SST-2情感分类任务，系统比较了三种可解释性方法：梯度归因法（Integrated Gradients）、注意力展开法（Attention Rollout）和模型无关法（SHAP）。

Result: 梯度归因法提供更稳定直观的解释；注意力方法计算效率高但与预测相关特征对齐度较低；模型无关方法灵活性高但计算成本大且结果变异性高。

Conclusion: 研究揭示了不同可解释性方法间的关键权衡，强调它们应作为诊断工具而非确定性解释，为基于Transformer的NLP系统研究者和工程师提供了实用见解。
...
### [112] [Think Multilingual, Not Harder: A Data-Efficient Framework for Teaching Reasoning Models to Code-Switch](https://arxiv.org/abs/2604.15490)
*Eleanor M. Lin,David Jurgens*

Main category: cs.CL

TL;DR: 本文提出首个基于语言学和行为动机的微调框架，用于识别大型语言模型中有益的代码切换推理行为，并教导模型更有效地进行代码切换以提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将代码切换视为错误行为，或仅通过修改输入提示或解码过程来控制，且研究范围局限于特定语言、领域、任务和模型。本文旨在填补这些空白，系统研究代码切换在推理中的潜在价值。

Method: 1. 创建并系统分析多模型、多语言、多任务、多领域的推理轨迹数据集，以理解现有推理模型中的代码切换行为；2. 基于观察到的有益行为，开发微调干预方法，教导推理模型进行代码切换。

Result: 该框架能以数据高效的方式显著增加有益的代码切换推理行为；有趣的是，即使微调任务不直接涉及推理中的代码切换（如机器翻译），也能修改推理模型的代码切换行为。

Conclusion: 数据高效的干预方法可以在推理模型中植入有益的代码切换行为，表明代码切换在推理中具有潜在价值，且可通过微调进行引导和优化。
...
### [113] [Brain Score Tracks Shared Properties of Languages: Evidence from Many Natural Languages and Structured Sequences](https://arxiv.org/abs/2604.15503)
*Jingnong Qu,Ashvin Ranjan,Shane Steinert-Threlkeld*

Main category: cs.CL

TL;DR: 研究发现Brain Score指标可能不足以仅凭高分推断语言模型具有类人语言处理能力，因为不同训练数据的模型表现相似。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型处理与人类语言处理的相似性，特别是验证Brain Score指标是否真能反映类人处理能力。

Method: 通过在不同类型输入数据（多种自然语言、人类基因组、Python代码、嵌套括号结构）上训练语言模型，并评估它们在Brain Score上的表现。

Result: 不同自然语言训练的模型Brain Score表现非常相似；其他结构化数据训练的模型表现也相当好，某些情况下接近自然语言模型。

Conclusion: Brain Score能反映语言模型提取自然语言共同结构的能力，但该指标敏感度不足，不能仅凭高分推断模型具有类人语言处理。
...
### [114] [PolicyBank: Evolving Policy Understanding for LLM Agents](https://arxiv.org/abs/2604.15505)
*Jihye Choi,Jinsung Yoon,Long T. Le,Somesh Jha,Tomas Pfister*

Main category: cs.CL

TL;DR: 提出PolicyBank机制，通过交互和反馈自主修正LLM代理对模糊政策规范的理解，解决政策规范中的歧义和逻辑漏洞问题。


<details>
  <summary>Details</summary>
Motivation: LLM代理在组织政策下运行时，自然语言描述的政策规范存在歧义和逻辑漏洞，导致代理行为与真实要求系统性偏离，现有方法将政策视为不可变真理，强化了'合规但错误'的行为。

Method: 提出PolicyBank记忆机制，维护结构化、工具级的政策理解，通过预部署测试中的交互和纠正反馈迭代优化；建立系统化测试平台，在工具调用基准中引入受控政策漏洞以隔离对齐失败和执行失败。

Result: 现有记忆机制在政策漏洞场景下成功率接近零，而PolicyBank能将与人类专家标准的差距缩小高达82%。

Conclusion: PolicyBank通过自主细化政策解释能有效弥补规范漏洞，显著提升LLM代理的政策对齐能力。
...
### [115] [Consistency Analysis of Sentiment Predictions using Syntactic & Semantic Context Assessment Summarization (SSAS)](https://arxiv.org/abs/2604.15547)
*Sharookh Daruwalla,Nitin Mayande,Shreeya Verma Kathuria,Nitin Joglekar,Charles Weber*

Main category: cs.CL

TL;DR: 提出SSAS框架，通过语法和语义上下文评估摘要，解决LLM在情感预测中的随机性问题，提升预测一致性。


<details>
  <summary>Details</summary>
Motivation: LLM的随机性与企业级分析对一致性的要求存在冲突，且现代数据集噪声多，导致情感预测波动大，难以支持战略决策。

Method: 采用SSAS框架，通过分层分类结构（主题、故事、集群）和迭代的摘要之摘要（SoS）上下文计算架构，为LLM提供高信号、情感密集的提示。

Result: 在三个行业标准数据集上评估，SSAS能提升高达30%的数据质量，通过去噪和改进情感预测估计，显著提高一致性。

Conclusion: SSAS通过稳定的上下文估计能力，为决策提供了可靠证据基础，有效缓解了LLM的随机性和数据噪声问题。
...
### [116] [Why Fine-Tuning Encourages Hallucinations and How to Fix It](https://arxiv.org/abs/2604.15574)
*Guy Kaplan,Zorik Gekhman,Zhen Zhu,Lotem Rozner,Yuval Reif,Swabha Swayamdipta,Derek Hoiem,Roy Schwartz*

Main category: cs.CL

TL;DR: 本文提出一种基于自蒸馏的监督微调方法，通过正则化输出分布漂移来减少大语言模型在微调过程中的事实幻觉，并探讨了幻觉产生的机制。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在监督微调（SFT）过程中接触新事实信息时，容易产生与预训练知识相矛盾的事实幻觉，这源于训练过程中的知识退化问题。

Method: 1. 提出基于自蒸馏的SFT方法，通过正则化输出分布漂移来平衡新知识学习和已有知识保留；2. 在无需新知识获取的场景中，采用冻结参数组的方法抑制事实可塑性；3. 通过容量限制、行为克隆和局部干扰三个假设研究SFT诱导幻觉的机制。

Result: 实验表明：1. 自蒸馏方法能有效促进事实学习并减少对预训练知识的幻觉；2. 冻结参数组方法能在保持任务性能的同时减少幻觉；3. 幻觉的主要驱动因素是重叠语义表征之间的干扰，自蒸馏通过减轻这种干扰发挥作用。

Conclusion: SFT诱导的幻觉主要由语义表征干扰引起，自蒸馏方法通过缓解干扰能有效减少幻觉，为平衡知识更新与知识保留提供了实用解决方案。
...
### [117] ["Excuse me, may I say something..." CoLabScience, A Proactive AI Assistant for Biomedical Discovery and LLM-Expert Collaborations](https://arxiv.org/abs/2604.15588)
*Yang Wu,Jinhong Yu,Jingwei Xiong,Zhimin Tao,Xiaozhong Liu*

Main category: cs.CL

TL;DR: 提出了CoLabScience，一种主动式LLM助手，通过PULI框架在生物医学协作中实现适时干预，显著提升了干预精度和任务效用。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs的被动响应模式限制了其在需要预见性和自主参与的科学协作场景中的有效性，需要开发能够主动介入的AI助手以加速生物医学发现。

Method: 提出PULI框架（基于正未标记学习的强化学习干预机制），利用项目提案和长短期对话记忆决定干预时机与方式；构建了BSDD生物医学流式对话数据集作为基准。

Result: PULI在干预精度和协作任务效用上显著优于现有基线方法，验证了主动式LLM作为智能科学助手的潜力。

Conclusion: CoLabScience通过主动干预机制有效增强了人机协作效率，为LLMs在科学工作流中的深度整合提供了新范式。
...
### [118] [LLM attribution analysis across different fine-tuning strategies and model scales for automated code compliance](https://arxiv.org/abs/2604.15589)
*Jack Wei Lun Shi,Minghao Dang,Wawan Solihin,Justin K. W. Yeoh*

Main category: cs.CL

TL;DR: 本研究通过扰动归因分析比较不同微调策略和模型规模下大语言模型在代码合规检查中的解释行为差异，发现全微调产生更聚焦的归因模式，且模型增大时会发展特定解释策略。


<details>
  <summary>Details</summary>
Motivation: 现有研究将大语言模型视为黑箱，仅关注性能而忽略训练决策对其解释行为的影响，缺乏对模型可解释性的深入理解，特别是在建筑、工程和建造行业等关键监管任务中需要更透明的模型。

Method: 采用基于扰动的归因分析方法，比较全微调、低秩适应和量化低秩适应等不同微调策略，以及不同参数规模的模型在代码合规任务中的解释行为差异。

Result: 全微调产生的归因模式在统计上不同于参数高效微调方法且更聚焦；模型规模增大时，大语言模型会发展出优先处理数值约束和规则标识符等特定解释策略，但超过70亿参数后生成规则与参考规则的语义相似度性能提升趋于平缓。

Conclusion: 研究揭示了不同训练配置下大语言模型的解释行为差异，为构建更透明、适用于关键监管任务的大语言模型提供了重要见解，推动了建筑、工程和建造行业自动化代码合规检查的可解释性发展。
...
### [119] [DALM: A Domain-Algebraic Language Model via Three-Phase Structured Generation](https://arxiv.org/abs/2604.15593)
*Chao Li*

Main category: cs.CL

TL;DR: 提出DALM（领域代数语言模型），通过结构化去噪在领域格上生成文本，以解决大语言模型中不同领域知识相互干扰的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型将异构知识压缩到单一参数空间，导致不同领域事实在生成过程中相互干扰，影响生成内容的准确性和领域特异性。

Method: 采用三阶段生成路径：先解决领域不确定性，再解决关系不确定性，最后解决概念不确定性；基于领域格、类型化关系和纤维划分构建代数约束的编码器-解码器架构。

Result: DALM能将生成过程限制在特定领域纤维中，结构上防止跨领域污染，支持单查询生成多视角答案空间，并在CDC知识表示系统上进行了实例化验证。

Conclusion: DALM将语言生成重新定义为代数约束的结构化去噪过程，而非在平坦标记空间上的无约束解码，为可控、可审计的领域感知生成提供了新框架。
...
### [120] [LLMs Corrupt Your Documents When You Delegate](https://arxiv.org/abs/2604.15597)
*Philippe Laban,Tobias Schnabel,Jennifer Neville*

Main category: cs.CL

TL;DR: 该研究通过DELEGATE-52基准测试评估大语言模型在委托工作流中的可靠性，发现当前模型在长期委托任务中会损坏文档内容，即使前沿模型也会平均损坏25%的内容，且代理工具使用无法改善此问题。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在知识工作中的广泛应用，委托工作成为一种新兴交互范式（如氛围编程），但委托需要信任模型能忠实执行任务而不引入错误。当前缺乏对模型在长期委托工作流中可靠性的系统评估。

Method: 提出DELEGATE-52基准，模拟52个专业领域（如编程、晶体学、音乐记谱）中需要深度文档编辑的长期委托工作流。对19个大语言模型进行大规模实验，并额外测试了代理工具使用、文档大小、交互长度和干扰文件等因素的影响。

Result: 当前模型在委托过程中会损坏文档：前沿模型（Gemini 3.1 Pro、Claude 4.6 Opus、GPT 5.4）在长期工作流结束时平均损坏25%的文档内容，其他模型表现更差。代理工具使用未能提升性能，且文档大小、交互长度和干扰文件会加剧损坏程度。

Conclusion: 当前大语言模型是不可靠的委托代理：它们会引入稀疏但严重的错误，悄无声息地损坏文档，并在长期交互中不断累积。这揭示了现有模型在委托工作流中的根本局限性。
...
### [121] [GroupDPO: Memory efficient Group-wise Direct Preference Optimization](https://arxiv.org/abs/2604.15602)
*Jixuan Leng,Si Si,Hsiang-Fu Yu,Vinod Raman,Inderjit S. Dhillon*

Main category: cs.CL

TL;DR: 提出了一种内存高效的组级偏好优化算法，通过解耦样本减少内存开销，使大组训练成为可能，在离线和在线对齐中均优于单对训练。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法通常每个提示只使用一个正负对进行训练，忽略了偏好数据集中通常包含多个候选响应的额外监督信息。

Method: 提出内存高效的组级偏好优化算法，在反向传播过程中解耦样本以保留梯度，大幅降低峰值内存使用，并引入正响应的负对数似然项以提升性能和稳定性。

Result: 使用多个响应的组级训练在离线和在线对齐设置中均一致优于单对训练；正响应的负对数似然项对性能提升和训练稳定性至关重要。

Conclusion: 组级偏好优化能有效利用多响应监督信息，通过内存优化算法实现可扩展训练，结合负对数似然项可显著提升对齐效果和训练稳定性。
...
### [122] [FD-NL2SQL: Feedback-Driven Clinical NL2SQL that Improves with Use](https://arxiv.org/abs/2604.15646)
...
TL;DR: 介绍FD-NL2SQL系统，这是一个基于反馈驱动的临床自然语言转SQL助手，专为肿瘤学数据库设计，通过分解问题、检索示例和合成SQL来支持临床医生进行复杂查询。


<details>
  <summary>Details</summary>
Motivation: 临床医生在探索肿瘤学试验数据库时，经常需要进行多约束的临时查询，但编写SQL需要专业知识，因此需要一个易于使用的自然语言转SQL工具来降低使用门槛。

Method: 系统使用模式感知的LLM将自然语言问题分解为谓词级子问题，通过句子嵌入检索专家验证的NL2SQL示例，并基于分解、检索结果和模式合成可执行的SQL，同时包含后处理有效性检查。系统通过两种更新信号持续改进：临床医生编辑并批准的SQL被添加到示例库，以及基于逻辑的SQL增强生成变体并自动扩展示例库。

Result: FD-NL2SQL提供了一个演示界面，展示分解、检索、合成和执行结果，支持交互式细化和持续改进，使临床医生能够更轻松地进行复杂查询。

Conclusion: FD-NL2SQL通过反馈驱动的方法，有效降低了临床医生使用肿瘤学数据库的SQL查询门槛，并通过自动扩展示例库实现了系统的持续优化。
...
### [123] [Imperfectly Cooperative Human-AI Interactions: Comparing the Impacts of Human and AI Attributes in Simulated and User Studies](https://arxiv.org/abs/2604.15607)
...
TL;DR: 本研究通过模拟实验和真人实验，比较了人类人格特质与AI设计特性在目标不完全一致的人机协作场景中的相对及联合影响，发现模拟与真人实验结果存在差异，且AI透明度在真人互动中影响更大。


<details>
  <summary>Details</summary>
Motivation: 人类人格特质与AI设计特性均影响人机交互质量，但二者在不完全合作场景（双方目标仅部分一致）中的相对与共同作用尚未得到充分探索。

Method: 采用并行研究设计：1）基于2000次仿真的纯模拟数据集；2）涉及290名真人参与者的实验。场景包括招聘谈判和AI可能隐瞒信息以达成内部目标的交易任务。分析结合了用户外向性/宜人性人格特质、AI适应性/专业性/思维链透明度等设计特性，并通过因果发现方法整合场景结果、沟通分析和问卷测量。

Result: 模拟实验与真人实验结果存在分歧：模拟中人格特质与AI属性影响相当；但在真人实验中，AI属性（尤其是透明度）的影响远大于人格特质。这种差异在不同互动情境（招聘谈判 vs. 信息不对称交易）中也有所变化。

Conclusion: 模拟环境与真人互动中影响因素的权重存在显著差异，突显了在开发以人为本的AI代理时，需结合真实人类行为数据评估设计特性，并考虑不同交互情境的调节作用。
...
### [124] [CIG: Measuring Conversational Information Gain in Deliberative Dialogues with Semantic Memory Dynamics](https://arxiv.org/abs/2604.15647)
*Ming-Bin Chen,Jey Han Lau,Lea Frermann*

Main category: cs.CL

TL;DR: 提出对话信息增益框架，通过建模语义记忆评估对话质量，关注信息进展而非仅礼貌或结构。


<details>
  <summary>Details</summary>
Motivation: 现有公共讨论质量评估多关注礼貌性或论证结构，缺乏对信息进展的量化评估，需要新框架衡量对话如何促进集体理解。

Method: 构建对话语义记忆模型：从话语提取原子主张并整合为结构化记忆状态，基于记忆从新颖性、相关性和隐含范围三个维度评分话语。在电视辩论和社区讨论数据上标注验证。

Result: 记忆衍生的动态指标（如主张更新数量）比传统启发式方法（话语长度或TF-IDF）更显著相关于人类感知的信息增益；开发了有效的基于大语言模型的预测器。

Conclusion: 对话信息增益框架能有效评估对话信息质量，为聚焦信息进展的对话分析提供了新途径，有助于评估审议式对话的成功程度。
...
### [125] [HyperGVL: Benchmarking and Improving Large Vision-Language Models in Hypergraph Understanding and Reasoning](https://arxiv.org/abs/2604.15648)
...
TL;DR: 本文提出了首个评估大型视觉语言模型（LVLMs）在超图理解和推理能力的基准测试HyperGVL，包含12个任务共84,000个视觉语言问答样本，并提出了自适应表示学习框架WiseHyGR以提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 超图在生命科学、社交网络等领域有重要应用，但现有研究缺乏评估LVLMs处理超图能力的基准，导致其能力边界不清晰。

Method: 构建HyperGVL基准测试，涵盖多尺度合成结构及真实引用网络、蛋白质网络；设计12种文本与视觉超图表示方法；提出可泛化的路由框架WiseHyGR，通过学习自适应表示增强LVLMs的超图处理能力。

Result: 对12个先进LVLMs进行全面评估，发现其在超图任务（从基础组件计数到NP难问题推理）中表现存在显著差异；WiseHyGR能有效提升模型在超图任务上的性能。

Conclusion: 该研究填补了LVLMs超图能力评估的空白，推动了超图与LVLMs的结合，为后续研究提供了基准与优化方向。
...
### [126] [C-Mining: Unsupervised Discovery of Seeds for Cultural Data Synthesis via Geometric Misalignment](https://arxiv.org/abs/2604.15675)
...
TL;DR: 提出了C-Mining框架，通过无监督方法量化挖掘文化种子数据，用于提升大语言模型的文化对齐能力，显著降低了数据准备成本并提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的文化对齐依赖合成数据生成，但种子数据的选择缺乏量化标准，现有方法依赖不可扩展的人工筛选或易产生偏见的LLM提取，将文化特异性视为抽象概念而非可测量信号。

Method: 提出C-Mining无监督框架，利用预训练嵌入空间中文化概念的跨语言不对齐作为可量化发现信号，通过识别语言排他性和几何隔离区域，自动从原始多语言语料中提取高保真文化点，无需人工或LLM监督。

Result: 将数据准备成本降低150倍以上，在CulturalBench-Hard基准上获得+6.03分的提升，超越了现有最先进基线，显著增强了文化理解和推理能力。
...
### [127] [Preference Estimation via Opponent Modeling in Multi-Agent Negotiation](https://arxiv.org/abs/2604.15687)
...
TL;DR: 提出了一种结合大语言模型与贝叶斯推理的对手偏好估计方法，通过自然语言交互提升多议题谈判中的对手建模精度。


<details>
  <summary>Details</summary>
Motivation: 传统数值化对手建模方法无法捕捉自然语言中的定性信息，导致偏好估计不稳定且不完整；大语言模型虽能理解语义，但难以定量整合到一致的对手建模框架中。

Method: 利用大语言模型从对话中提取定性线索，将其转化为概率形式，并整合到结构化的贝叶斯对手建模框架中进行动态信念更新。

Result: 在多议题谈判基准测试中，该框架通过结合概率推理与自然语言理解，提高了完全协议达成率和偏好估计准确率。

Conclusion: 融合自然语言信息的贝叶斯对手建模能有效提升复杂多议题谈判中的对手偏好估计性能，为自动化谈判提供了更稳健的解决方案。
...
### [128] [Improving Reasoning Capabilities in Small Models through Mixture-of-Layers Distillation with Stepwise Attention on Key Information](https://arxiv.org/abs/2604.15701)
*Yao Chen,Jiawei Sheng,Wenyuan Zhang,Tingwen Liu*

Main category: cs.CL

TL;DR: 提出一种新的思维链蒸馏框架，通过转移教师模型在推理过程中的渐进注意力到学生模型，提升小模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有思维链蒸馏方法主要关注转移教师生成的推理过程，但未充分探索教师在推理中对关键信息的动态注意力模式。研究发现语言模型在推理过程中会逐步将注意力转移到关键信息上，这为结论推导提供了重要线索。

Method: 1. 提出新颖的思维链蒸馏框架，转移教师对关键信息的逐步注意力；2. 开发混合层模块，实现教师与学生模型不同层之间的动态对齐。

Result: 该方法在多个数学和常识推理数据集上实现了持续的性能提升。

Conclusion: 这是首个利用逐步注意力机制改进思维链蒸馏以提升小模型推理能力的方法。
...
### [129] [The Metacognitive Monitoring Battery: A Cross-Domain Benchmark for LLM Self-Monitoring](https://arxiv.org/abs/2604.15702)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: 该研究开发了一个跨领域的行为测试电池，基于Nelson-Narens元认知框架，用于评估大型语言模型的监控-控制耦合能力，包含524个项目，覆盖六个认知领域，并在20个前沿LLM上进行了测试。


<details>
  <summary>Details</summary>
Motivation: 将人类心理测量学方法应用于LLM评估，以系统性地量化模型的元认知能力（特别是监控与控制功能），并探索不同模型架构在元认知行为上的差异。

Method: 基于Nelson-Narens元认知框架设计测试电池，包含六个认知领域的任务；采用Koriat和Goldsmith的双重探测方法（KEEP/WITHDRAW和BET/decline）；计算关键指标“撤回差异”（错误与正确项目的撤回率差）；对20个前沿LLM进行10,480次评估。

Result: 识别出三种与Nelson-Narens架构一致的行为模式：盲目自信、盲目撤回和选择性敏感；准确率排名与元认知敏感度排名基本倒置；回顾性监控与前瞻性调控可能分离；元认知校准随模型规模的变化呈现架构依赖性（如Qwen递减、GPT-5.4递增、Gemma持平）；行为结果与独立的Type-2信号检测论方法结构一致。

Conclusion: 该测试电池能有效区分LLM的元认知行为特征，为模型评估提供了新的心理测量学工具；不同模型在元认知能力上存在显著差异，且与准确率不直接相关；研究支持了跨方法的初步结构效度，所有材料和代码已开源。
...
### [130] [Target-Oriented Pretraining Data Selection via Neuron-Activated Graph](https://arxiv.org/abs/2604.15706)
*Zijun Wang,Haoqin Tu,Weidong Zhou,Yiyang Zhou,Xiaohuan Zhou,Bingni Zhang,Weiguo Feng,Taifeng Wang,Cihang Xie,Fengze Liu*

Main category: cs.CL

TL;DR: 提出NAG-based Ranking框架，通过分析LLM神经元激活图来选择目标导向的预训练数据，无需额外训练且可解释，在多个基准测试中优于随机采样和现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有目标导向语言模型预训练缺乏可解释的数据选择方法，通常依赖黑盒表征。本研究旨在开发一种无需训练、可解释的框架，通过分析预训练大模型中与目标相关的关键神经元来优化数据选择。

Method: 提出Neuron-Activated Graph Ranking (NAG-based Ranking)：1) 量化神经元影响力，筛选各层中最重要的神经元构建紧凑的神经元激活图(NAG)；2) 通过计算候选数据与目标示例的NAG相似度进行排序；3) 支持单目标和多目标场景。

Result: 1) 在六个基准测试中，NAG-based Ranking比随机采样平均提升4.9%，在HellaSwag上比SOTA基线高5.3%准确率；2) 多目标设置下优于两个基线1.1%和4.1%；3) 消融实验显示，仅禁用NAG选中的0.12%神经元会导致性能下降23.5%，仅使用最后一层NAG会带来4.1%平均下降，证明NAG捕捉了学习目标特征的稀疏“功能骨干”。

Conclusion: NAG-based Ranking是一种有效且可解释的目标导向预训练数据选择方法，通过稀疏神经元激活图捕获关键特征，显著提升模型性能，并为理解LLM的内部机制提供了新视角。
...
### [131] [GTA-2: Benchmarking General Tool Agents from Atomic Tool-Use to Open-Ended Workflows](https://arxiv.org/abs/2604.15715)
*Jize Wang,Xuanxuan Liu,Yining Li,Songyang Zhang,Yijun Wang,Zifei Shan,Xinyi Le,Cailian Chen,Xinping Guan,Dacheng Tao*

Main category: cs.CL

TL;DR: 本文提出GTA-2基准测试，用于评估通用工具代理在原子工具使用和开放式工作流中的性能，发现当前前沿模型在复杂工作流任务上表现不佳，而执行框架设计对性能提升至关重要。


<details>
  <summary>Details</summary>
Motivation: 当前工具使用基准测试与真实世界需求脱节，依赖AI生成查询、虚拟工具和有限的系统级协调，无法有效评估复杂生产力工作流的完成能力。

Method: 构建GTA-2分层基准测试，包含GTA-Atomic（评估短视界封闭式工具使用精度）和GTA-Workflow（评估长视界开放式端到端任务）；提出基于递归检查点的评估机制，将目标分解为可验证子目标。

Result: 实验显示能力断层：前沿模型在原子任务上成功率低于50%，在工作流任务上最佳模型仅14.39%成功率；检查点引导反馈可提升性能，Manus和OpenClaw等高级执行框架显著改善工作流完成度。

Conclusion: 执行框架设计对工具代理性能的影响不亚于底层模型能力，为开发可靠个人和专业助手提供指导；数据集和代码已开源。
...
### [132] [Learning Uncertainty from Sequential Internal Dispersion in Large Language Models](https://arxiv.org/abs/2604.15741)
*Ponhvoan Srey,Xiaobao Wu,Cong-Duy Nguyen,Anh Tuan Luu*

Main category: cs.CL

TL;DR: 提出SIVR框架，利用隐藏状态的逐层方差特征检测大语言模型幻觉，无需特定假设且能避免信息损失，在泛化性和数据效率上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于模型内部状态的幻觉检测方法存在两个问题：对隐藏状态跨层演化有严格假设，以及仅关注最后或平均token导致信息损失。

Method: 提出SIVR监督框架，基于隐藏状态计算token-wise和layer-wise的方差特征，捕捉内部表示的离散程度作为不确定性指标，并聚合全序列的方差特征学习时间模式。

Result: 实验表明SIVR持续优于强基线方法，具有更强的泛化能力且无需大量训练数据，适合实际部署。

Conclusion: SIVR通过更基础的方差假设和全序列特征聚合，实现了模型和任务无关的幻觉检测，为实际应用提供了高效解决方案。
...
### [133] [Language, Place, and Social Media: Geographic Dialect Alignment in New Zealand](https://arxiv.org/abs/2604.15744)
*Sidney Wong*

Main category: cs.CL

TL;DR: 本研究通过分析新西兰相关Reddit社区，探讨了社交媒体中地理方言与地方认同的关联，结合定性与计算方法揭示了语言变异模式及语义演变。


<details>
  <summary>Details</summary>
Motivation: 探索社交媒体如何作为社会语言学的天然实验室，研究语言使用如何反映地方身份认同，以及地理方言社区与地方相关社区之间的复杂关系。

Method: 整合用户感知的定性分析与计算方法，基于用户提供的词汇、形态句法和语义变量，使用静态和历时Word2Vec语言嵌入模型进行语言建模，并构建了包含42.6亿未处理词汇的语料库。

Result: 用户普遍将语言与地方关联，地方相关社区形成连续言语社区；语义变异存在于不同地方社区中，新西兰英语内部出现有意义的语义演变；地理方言社区与地方社区的对齐关系复杂。

Conclusion: 社交媒体为研究语言变异和变化提供了宝贵资源，展示了其作为社会语言学研究的有效平台潜力，所构建的大规模语料库可为未来研究提供支持。
...
### [134] [TTL: Test-time Textual Learning for OOD Detection with Pretrained Vision-Language Models](https://arxiv.org/abs/2604.15756)
*Jinlun Ye,Jiang Liao,Runhe Lai,Xinhua Lu,Jiaxin Zhuang,Zhiyong Gan,Ruixuan Wang*

Main category: cs.CL

TL;DR: 提出TTL框架，通过动态学习未标记测试流中的OOD文本语义，无需外部OOD标签即可提升CLIP模型的OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的测试时适应方法依赖有限的外部OOD标签，无法适应真实开放且动态变化的OOD语义空间。

Method: 1. 动态学习可学习提示词以捕获新兴OOD知识；2. 设计OOD知识净化策略筛选可靠样本；3. 建立OOD文本知识库实现跨批次稳定校准。

Result: 在2个基准测试和9个OOD数据集上的实验表明，TTL持续达到最先进的性能水平。

Conclusion: 文本适应对提升测试时OOD检测的鲁棒性具有重要价值，TTL框架能有效应对开放语义空间的挑战。
...
### [135] [Skill-RAG: Failure-State-Aware Retrieval Augmentation via Hidden-State Probing and Skill Routing](https://arxiv.org/abs/2604.15771)
*Kai Wei,Raymond Li,Xi Zhu,Zhaoqian Xue,Jiaojiao Han,Jingcheng Niu,Fan Yang*

Main category: cs.CL

TL;DR: 提出Skill-RAG框架，通过诊断检索失败原因并选择相应技能（如查询重写、问题分解等）来纠正查询与证据间的错位，显著提升困难案例的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有自适应检索方法将检索后失败视为重试信号，而非诊断机会，未能解决查询与证据空间错位的结构性问题，导致大量持续检索失败。

Method: 提出Skill-RAG框架，包含轻量级隐藏状态探测器和基于提示的技能路由器。探测器在流水线两阶段控制检索；检测到失败状态时，路由器诊断原因并从四种检索技能（查询重写、问题分解、证据聚焦、退出技能）中选择以纠正错位。

Result: 在多个开放域QA和复杂推理基准测试中，Skill-RAG显著提升了多轮检索后困难案例的准确率，尤其在分布外数据集上表现突出；表示空间分析显示技能在失败状态空间中占据结构化、可分离区域。

Conclusion: 查询-证据错位是一种类型化而非单一现象，Skill-RAG通过诊断失败原因并应用针对性技能有效纠正错位，为处理持续检索失败提供了新思路。
...
### [136] [MemEvoBench: Benchmarking Memory MisEvolution in LLM Agents](https://arxiv.org/abs/2604.15774)
*Weiwei Xie,Shaoxiong Guo,Fan Zhang,Tian Xia,Xue Yang,Lizhuang Ma,Junchi Yan,Qibing Ren*

Main category: cs.CL

TL;DR: 提出了首个评估LLM智能体长期记忆安全性的基准MemEvoBench，用于测试对抗性记忆注入、噪声工具输出和偏见反馈下的行为漂移。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型配备持久记忆虽提升交互连续性，但可能因污染或偏见记忆积累引发异常行为，现有方法缺乏标准化评估框架来测量记忆错误演化现象。

Method: 构建包含7个领域36种风险类型的问答任务，以及20个Agent-SafetyBench环境改编的工作流任务，在混合良性/误导性记忆池中进行多轮交互模拟记忆演化，测试代表性模型。

Result: 实验显示偏见记忆更新会导致显著安全性退化，记忆演化是故障主因，静态提示防御效果不足。

Conclusion: 记忆演化对LLM智能体安全构成重大威胁，需开发针对性防护机制，当前防御手段亟待加强。
...
### [137] [PIIBench: A Unified Multi-Source Benchmark Corpus for Personally Identifiable Information Detection](https://arxiv.org/abs/2604.15776)
*Pritesh Jha*

Main category: cs.CL

TL;DR: 提出了PIIBench，一个用于自然语言文本中个人身份信息检测的统一基准语料库，整合了十个公开数据集，包含240万标注序列和335万个实体提及，涵盖48种PII实体类型。


<details>
  <summary>Details</summary>
Motivation: 现有的PII检测资源分散在领域特定的语料库中，标注方案互不兼容，阻碍了检测系统的系统比较。

Method: 整合十个公开数据集，开发了标准化流程，将80多个源特定标签映射到标准BIO标注方案，应用基于频率的实体类型抑制，并生成保留源分布的分层训练/验证/测试分割。评估了八个已发布的系统，包括规则引擎和通用及特定领域NER模型。

Result: 所有系统的跨度级F1分数均低于0.14，最佳系统（Presidio，F1=0.1385）在大多数实体类型上召回率为零。

Conclusion: PIIBench比任何现有单一源PII数据集提供了更困难、更全面的评估挑战，直接量化了领域孤岛问题。
...
### [138] [A Systematic Study of Training-Free Methods for Trustworthy Large Language Models](https://arxiv.org/abs/2604.15789)
*Wai Man Si,Mingjie Li,Michael Backes,Yang Zhang*

Main category: cs.CL

TL;DR: 本文系统评估了无需训练的LLM对齐方法，通过分类分析揭示了其在可信性、实用性和鲁棒性之间的权衡，并提出了实用建议。


<details>
  <summary>Details</summary>
Motivation: 随着LLM广泛应用，其生成有害内容、偏见和对抗攻击脆弱性等风险日益凸显。现有无需训练的对齐方法虽成本低，但评估不一致、可信维度覆盖有限，且可能引发实用性下降和脆弱性增加等副作用。

Method: 将无需训练方法按推理时干预位置分为输入层、内部层和输出层三类，系统评估不同LLM家族和规模下代表性方法的可信性、实用性、鲁棒性和计算开销。

Result: 分析揭示了当前方法在可信性提升与实用性保持、鲁棒性之间的多重权衡，并识别出未解决的技术挑战和局限性。

Conclusion: 无需训练对齐方法需更系统评估框架，未来应平衡可信性、实用性和鲁棒性，本文为此提供了分类体系、实证发现和实用建议。
...
### [139] [CHOP: Chunkwise Context-Preserving Framework for RAG on Multi Documents](https://arxiv.org/abs/2604.15802)
*Hyunseok Park,Jihyeon Kim,Jongeun Kim,Dongsik Yoon*

Main category: cs.CL

TL;DR: 提出了CHOP框架，通过迭代评估文档块相关性并重构文档来解决RAG系统中相似文档共存导致的检索准确性问题。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）系统在向量数据库中存在相似文档时，会导致检索准确性下降、产生不必要信息、幻觉和事实错误，需要解决这一问题。

Method: CHOP框架包含两个关键组件：CNM-Extractor（生成包含类别、关键名词和模型名称的紧凑块签名）和连续性决策模块（保持上下文连贯性）。通过为每个块添加上下文感知元数据来减少语义冲突。

Result: 在基准数据集上的实验表明，CHOP缓解了检索混淆，实现了90.77%的Top-1命中率，并在排序质量指标上取得显著提升。

Conclusion: CHOP为构建高质量知识库提供了可扩展的方法，能有效增强检索器区分能力并减少相似文档间的语义冲突。
...
### [140] [CoEvolve: Training LLM Agents via Agent-Data Mutual Evolution](https://arxiv.org/abs/2604.15840)
*Shidong Yang,Ziyu Ma,Tongwen Huang,Yiming Hu,Yong Wang,Xiangxiang Chu*

Main category: cs.CL

TL;DR: 提出CoEvolve框架，通过智能体与数据的协同进化解决强化学习中静态数据分布的问题，在多个基准测试中显著提升LLM智能体性能。


<details>
  <summary>Details</summary>
Motivation: 传统LLM智能体强化学习使用静态数据分布，无法适应智能体行为演化，导致复杂环境交互覆盖不足。

Method: CoEvolve框架：1）从轨迹中提取遗忘和不确定性等反馈信号识别易失败交互模式；2）基于LLM的任务合成；3）通过环境交互验证任务并更新数据分布，实现智能体与数据的联合适应。

Result: 在AppWorld和BFCL基准测试中，对Qwen2.5-7B、Qwen3-4B和Qwen3-30B-A3B模型分别实现19.43%、15.58%和18.14%的绝对性能提升。

Conclusion: CoEvolve通过闭环交互驱动训练实现了智能体与数据的协同进化，能有效提升LLM智能体在复杂环境中的适应能力。
...
### [141] [Qwen3.5-Omni Technical Report](https://arxiv.org/abs/2604.15804)
*Qwen Team*

Main category: cs.CL

TL;DR: 介绍了Qwen3.5-Omni模型，这是一个支持文本、视觉、音频多模态的大规模模型，在多项任务中达到SOTA水平，并引入了ARIA技术提升语音合成质量。


<details>
  <summary>Details</summary>
Motivation: 为了开发一个能够处理多种模态（文本、视觉、音频）且支持长上下文、高效推理的通用模型，以解决现有模型在跨模态理解、交互和生成方面的不足。

Method: 采用混合注意力专家混合（Hybrid Attention Mixture-of-Experts）框架，使用大规模异构文本-视觉对和音频-视觉数据进行训练，并引入ARIA技术动态对齐文本和语音单元以提升流式语音合成质量。

Result: 模型在215个音频和音频-视觉理解、推理和交互子任务中达到SOTA，在关键音频任务上超越Gemini-3.1 Pro，支持10小时音频理解和400秒720P视频处理，并展现出音频-视觉编程等新能力。

Conclusion: Qwen3.5-Omni在多模态理解、生成和交互方面取得显著进展，其架构和技术创新为大规模多模态模型的发展提供了新方向，并展示了跨模态编程等新兴能力。
...
### [142] [DiZiNER: Disagreement-guided Instruction Refinement via Pilot Annotation Simulation for Zero-shot Named Entity Recognition](https://arxiv.org/abs/2604.15866)
*Siun Kim,Hyung-Jin Yoon*

Main category: cs.CL

TL;DR: 提出DiZiNER框架，通过模拟人类标注中的分歧解决过程，利用大语言模型作为标注者和监督者，优化零样本命名实体识别任务指令，在18个基准测试中取得14个SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在零样本命名实体识别中仍存在系统性错误，性能远低于监督系统，这些错误与人类早期标注过程中的不一致性相似，因此借鉴人类通过试点标注解决分歧的方法来改进模型。

Method: DiZiNER框架模拟试点标注过程：让多个异构大语言模型对相同文本进行标注，再由监督模型分析模型间的分歧，据此优化任务指令。

Result: 在18个基准测试中，DiZiNER在14个数据集上达到零样本SOTA，F1值比之前最佳结果提升8.0，将零样本与监督系统的差距缩小超过11个百分点；且始终优于其监督模型GPT-5 mini，表明性能提升源于分歧引导的指令优化而非模型容量。

Conclusion: 模型间的一致性程度与命名实体识别性能强相关，验证了通过模拟人类标注分歧解决过程来优化指令的有效性，为提升零样本信息提取性能提供了新途径。
...
### [143] [Exploring the Capability Boundaries of LLMs in Mastering of Chinese Chouxiang Language](https://arxiv.org/abs/2604.15841)
*Dianqing Lin,Tian Lan,Jiali Zhu,Jiang Li,Wei Chen,Xu Liu,Aruukhan,Xiangdong Su,Hongxu Hou,Guanglai Gao*

Main category: cs.CL

TL;DR: 本文介绍了Mouse基准测试，用于评估大语言模型在抽象话（一种中文网络亚文化语言）上的表现，发现当前SOTA模型在多个任务上存在明显局限，但在涉及上下文语义理解的任务上表现良好。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在通用语言任务上取得了显著成功，但它们在中文互联网亚文化语言——抽象话上的表现尚未得到充分探索，这促使研究者开发专门的评估基准。

Method: 研究者构建了Mouse基准测试，包含六个任务来评估大语言模型处理抽象话的能力，并进一步分析了模型表现不佳的原因，检验了LLM-as-a-judge方法在翻译任务中与人类判断的一致性，以及影响抽象话翻译的关键因素。

Result: 实验结果显示，当前最先进的大语言模型在多个抽象话任务上存在明显局限性，但在涉及上下文语义理解的任务上表现较好。研究还发现LLM-as-a-judge方法在翻译评估中与人类判断存在偏差。

Conclusion: 本研究揭示了当前大语言模型在处理抽象话等网络亚文化语言方面的不足，旨在促进NLP社区对多元文化融合和动态演变的互联网语言的进一步研究。代码和数据已公开。
...
### [144] [AgentV-RL: Scaling Reward Modeling with Agentic Verifier](https://arxiv.org/abs/2604.16004)
...
TL;DR: 提出Agentic Verifier框架，通过双向代理（前向与后向）结合工具增强的审慎过程，改进LLM推理验证，在复杂任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统验证器在复杂领域面临挑战：错误中间推理导致误判，缺乏外部依据使其在计算或知识密集型任务中不可靠。

Method: 1. 设计双向代理框架：前向代理从前提推导结论，后向代理从结论反查前提；2. 提出AgentV-RL，通过强化学习自主交替使用工具与内部推理。

Result: 实验显示Agentic Verifier在并行与顺序TTS中均取得稳定提升，4B参数版本超越现有最佳ORM 25.2%。

Conclusion: 该框架为智能奖励建模提供了可靠、可解释的评估范式，显著增强了复杂领域下的验证能力。
...
### [145] [CiPO: Counterfactual Unlearning for Large Reasoning Models through Iterative Preference Optimization](https://arxiv.org/abs/2604.15847)
*Junyi Li,Yongqiang Chen,Ningning Ding*

Main category: cs.CL

TL;DR: 本文提出CiPO框架，通过迭代偏好优化实现大语言模型的反事实遗忘，有效消除目标知识同时保持推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法难以从大推理模型的思维链中完全消除隐私/版权信息，且容易干扰推理性能，需要解决这一困境。

Method: 提出反事实遗忘迭代偏好优化框架，针对目标遗忘答案生成逻辑有效的反事实推理轨迹，通过迭代更新偏好学习数据扩大与原模型的差异。

Result: 在挑战性基准测试中，CiPO能完全消除思维链中间步骤和最终答案中的知识，同时保持大推理模型的推理能力。

Conclusion: CiPO通过重新定义遗忘为对思维链推理的针对性干预，有效解决了大推理模型中的遗忘困境，实现了知识消除与性能保持的平衡。
...
### [146] [Disentangling Mathematical Reasoning in LLMs: A Methodological Investigation of Internal Mechanisms](https://arxiv.org/abs/2604.15842)
*Tanja Baeumel,Josef van Genabith,Simon Ostermann*

Main category: cs.CL

TL;DR: 本文通过分析大语言模型在算术任务中的内部处理机制，揭示了模型在推理过程中的分层处理模式及注意力与MLP模块的分工协作。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理密集型任务上表现出色，但其内部处理机制尚不明确，特别是算术运算这类需要逐步推理的任务。研究旨在揭示模型如何通过内部计算实现复杂推理。

Method: 采用早期解码技术追踪模型各层对下一个token预测的构建过程，通过对比不同算术能力的模型在注意力模块和MLP模块中的激活模式来分析内部机制。

Result: 研究发现：1）模型早期识别算术任务，但正确结果仅在最后几层生成；2）擅长算术的模型中，注意力模块负责传播输入信息，MLP模块负责信息聚合，两者分工明确；3）能力较弱的模型缺乏这种分工；4）成功模型对更复杂的算术任务表现出超越事实记忆的推理能力。

Conclusion: 大语言模型在算术任务中通过模块化分工实现推理，注意力与MLP的功能分离是模型算术能力的关键特征，这为理解模型内部推理机制提供了新视角。
...
### [147] [Where does output diversity collapse in post-training?](https://arxiv.org/abs/2604.16027)
*Constantinos Karouzos,Xingwei Tan,Nikolaos Aletras*

Main category: cs.CL

TL;DR: 研究发现语言模型后训练中的输出多样性崩溃主要由训练数据组成决定，而非生成格式或特定后训练方法，且崩溃嵌入模型权重中，无法仅通过推理时调整解决。


<details>
  <summary>Details</summary>
Motivation: 后训练语言模型输出多样性低于基础模型，这削弱了依赖多样样本的推理时扩展方法，并可能在创意和价值负载任务中导致输出同质化。先前研究未区分训练数据组成与后训练方法、生成格式与模型权重的作用。

Method: 通过Olmo 3的三个并行后训练谱系（Think-思维链蒸馏、Instruct-多源数据、RL-Zero），在15个任务和四个文本多样性指标上追踪输出多样性，分析数据组成影响，并在六个可验证任务中分解多样性损失为质量控制成分和残余成分。

Result: 多样性崩溃位置与数据组成共变：Think谱系在监督微调时失去最多语义多样性；DPO在Instruct中影响大于Think；抑制Think模型的思维链推理会降低困难任务准确率但不改变答案级多样性；Think模型比Instruct保留更多正确答案多样性。

Conclusion: 多样性崩溃由训练期间的数据组成决定，并嵌入模型权重中，无法仅通过推理时干预解决；崩溃中质量控制与真实窄化的比例依赖任务，且不同训练谱系保留的正确答案多样性不同。
...
### [148] [How Hypocritical Is Your LLM judge? Listener-Speaker Asymmetries in the Pragmatic Competence of Large Language Models](https://arxiv.org/abs/2604.15873)
*Judith Sieker,Sina Zarrieß*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在语用能力上存在听者（判断语言适当性）与说者（生成适当语言）之间的不对称表现，多数模型作为听者表现更佳。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常被同时用作语言生成器和语言输出评判者，但这两个角色之间的表现关联性很少被直接研究，不清楚一个角色的成功是否对应另一个角色的成功。

Method: 比较多个开源和专有大型语言模型在三种语用场景下的表现：作为语用听者（判断语言适当性）和作为语用说者（生成语用适当的语言）。

Result: 发现语用评价与语用生成之间存在显著不对称性：许多模型作为听者的表现明显优于作为说者的表现。

Conclusion: 当前大型语言模型中语用判断与语用生成仅弱相关，需要更综合的评估方法。
...
### [149] [Towards Intrinsic Interpretability of Large Language Models:A Survey of Design Principles and Architectures](https://arxiv.org/abs/2604.16042)
*Yutong Gao,Qinglin Meng,Yuan Zhou,Liangming Pan*

Main category: cs.CL

TL;DR: 本文系统综述了大型语言模型内在可解释性的最新进展，将现有方法分为五个设计范式，并讨论了该新兴领域的开放挑战和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能强大，但其不透明的内部机制阻碍了可信度和安全部署。现有可解释性研究多关注事后解释方法，而内在可解释性作为直接构建透明性的替代方案正在兴起。

Method: 采用系统性文献综述方法，对内在可解释性研究进行分类，提出五个设计范式：功能透明性、概念对齐、表示可分解性、显式模块化和潜在稀疏性诱导。

Result: 建立了内在可解释性研究的分类框架，识别了五种主要设计范式，并整理了相关论文资源库供研究社区使用。

Conclusion: 内在可解释性为提高LLM透明度提供了有前景的途径，但仍面临诸多挑战，需要未来研究在模型设计、评估方法和实际应用等方面进一步探索。
...
### [150] [MUSCAT: MUltilingual, SCientific ConversATion Benchmark](https://arxiv.org/abs/2604.15929)
*Supriti Sinhamahapatra,Thai-Binh Nguyen,Yiğit Oğuz,Enes Ugan,Jan Niehues,Alexander Waibel*

Main category: cs.CL

TL;DR: 提出了一个多语言语音识别基准数据集Muscat-Eval，用于评估ASR系统处理混合语言输入、专业术语和语码转换的能力，实验表明现有先进系统仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏能够评估多语言语音技术处理混合语言对话、专业词汇和语码转换能力的标准化基准数据集，这阻碍了跨语言无缝通信技术的发展。

Method: 构建了一个基于科学论文讨论的双语对话数据集，包含多位说话者使用不同语言交流的场景，并设计了超越词错误率的标准化评估框架。

Result: 实验结果表明，即使是当前最先进的自动语音识别系统，在该数据集上的表现仍存在显著困难，验证了该基准的挑战性。

Conclusion: Muscat-Eval数据集填补了多语言语音识别评估的空白，为未来研究提供了重要的基准测试平台，推动实现真正的跨语言无缝通信。
...
### [151] [RAGognizer: Hallucination-Aware Fine-Tuning via Detection Head Integration](https://arxiv.org/abs/2604.15945)
*Fabian Ridder,Laurin Lessel,Malte Schilling*

Main category: cs.CL

TL;DR: 提出RAGognize数据集和RAGognizer方法，通过联合优化语言建模和幻觉检测，在多个基准测试中实现最先进的幻觉检测并显著降低生成幻觉。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成模型仍会产生闭域幻觉，现有检测方法多为后处理方案，缺乏直接利用内部状态表示作为训练信号的机制。

Method: 构建带token级标注的闭域幻觉数据集RAGognize；提出RAGognizer方法，在LLM中集成轻量检测头，联合优化语言建模和幻觉检测目标。

Result: RAGognizer在多个基准测试中实现最先进的token级幻觉检测，显著降低生成幻觉率，且不损害语言质量或相关性。

Conclusion: 将幻觉检测作为直接训练信号能有效提升模型内部状态的可分离性，同时改善生成质量，为缓解RAG系统幻觉问题提供新途径。
...
TL;DR: 提出SCHK-HTC方法，通过层次知识提取和兄弟类对比学习解决少样本层次文本分类中语义相似兄弟类难以区分的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖标签层次结构约束保证父子预测一致性，但缺乏领域知识导致难以区分深层语义相似的兄弟类别，成为性能瓶颈。

Method: 1. 层次知识提取模块：编码层级结构信息；2. 兄弟类对比学习机制：增强模型对兄弟类细微差异的感知能力；引导模型学习各层级的判别性特征。

Result: 在三个基准数据集上取得优越性能，在多数情况下超越现有最先进方法。

Conclusion: SCHK-HTC通过增强模型对兄弟类差异的感知而非仅强制层次规则，有效提升了少样本层次文本分类中混淆类别的可区分性。
...
### [153] [Cut Your Losses! Learning to Prune Paths Early for Efficient Parallel Reasoning](https://arxiv.org/abs/2604.16029)
*Jiaxi Bi,Tongxu Luo,Wenyu Du,Zhengyang Tang,Benyou Wang*

Main category: cs.CL

TL;DR: 提出首个路径剪枝系统分类法，并基于此开发了STOP方法，通过可学习的内部信号提升大推理模型的并行推理效率。


<details>
  <summary>Details</summary>
Motivation: 并行推理虽能增强大推理模型，但早期错误导致的无效路径会产生高昂计算成本，而现有路径剪枝研究缺乏统一框架，特别是可学习的内部方法潜力未被充分探索。

Method: 提出路径剪枝分类法（按信号来源分为内部/外部，按可学习性分为可学习/不可学习），并设计STOP方法——利用可学习的内部超令牌进行前缀级路径剪枝。

Result: 在1.5B至20B参数的大推理模型上验证，STOP在效果和效率上均优于基线；例如在固定计算预算下，将GPT-OSS-20B在AIME25上的准确率从84%提升至近90%。

Conclusion: STOP证明了可学习内部剪枝方法的有效性，研究还总结了实证指南以促进实际部署，相关代码、数据和模型已开源。
...
### [154] [Stochasticity in Tokenisation Improves Robustness](https://arxiv.org/abs/2604.16037)
*Sophie Steger,Rui Li,Sofiane Ennadir,Anya Sims,Arno Solin,Franz Pernkopf,Martin Trapp*

Main category: cs.CL

TL;DR: 本文研究了随机分词对大型语言模型鲁棒性的影响，发现预训练和微调时使用均匀采样的随机分词能提升模型对随机和对抗性扰动的鲁棒性，且不增加推理成本。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型的广泛使用引发了对鲁棒性的担忧，确定性规范分词训练的模型易受对抗攻击影响，而随机分词可能提供更稳健的内部表示。

Method: 系统研究了不同学习机制（预训练、监督微调、上下文学习）、数据集和模型架构，使用均匀采样的随机分词进行训练和评估。

Result: 随机分词训练将Llama-1b模型在非规范分词评估中的准确率下降从29.8%减少到可接受水平，同时保持推理效率。

Conclusion: 随机分词是一种有效提升语言模型鲁棒性的方法，能抵御分词扰动攻击，且无需额外推理开销。
...
### [155] [Can LLMs Understand the Impact of Trauma? Costs and Benefits of LLMs Coding the Interviews of Firearm Violence Survivors](https://arxiv.org/abs/2604.16132)
*Jessica H. Zhu,Shayla Stringfield,Vahe Zaprosyan,Michael Wagner,Michel Cukier,Joseph B. Richardson*

Main category: cs.CL

TL;DR: 评估开源大语言模型在分析枪支暴力幸存者访谈中的表现，发现模型虽能识别部分重要编码，但整体相关性低、对数据处理敏感，且伦理护栏导致叙事擦除。


<details>
  <summary>Details</summary>
Motivation: 枪支暴力是紧迫的公共卫生问题，但幸存者经历的研究资金不足且难以规模化。传统质性分析方法耗时耗力，而大语言模型为自动化分析提供可能，但需验证其能否准确、伦理地处理弱势群体数据。

Method: 使用开源大语言模型对21名经历社区枪支暴力的黑人男性幸存者访谈进行归纳式编码，评估模型在不同数据处理配置下的表现。

Result: 模型某些配置能识别重要编码，但整体编码相关性较低，且结果对数据处理方式高度敏感；模型的伦理护栏机制导致大量叙事内容被擦除。

Conclusion: 大语言模型辅助质性编码具有潜力但存在明显局限，在边缘化群体研究中应用人工智能需谨慎应对伦理挑战。
...
### [156] [Sentiment Analysis of German Sign Language Fairy Tales](https://arxiv.org/abs/2604.16138)
*Fabrizio Nunnari,Siddhant Jain,Patrick Gebhard*

Main category: cs.CL

TL;DR: 该研究构建了一个德国手语童话情感分析数据集和模型，通过文本情感标注和视频特征提取，训练可解释模型预测手语情感，发现面部和身体动作对手语情感表达同等重要。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索德国手语（DGS）中情感的表达方式，特别是面部和身体动作在情感传达中的作用，以填补手语情感分析领域的空白。

Method: 首先使用四个大语言模型（LLMs）对德国童话文本片段进行情感标注（负向、中性、正向），并通过多数投票达成标注一致；然后使用MediaPipe从对应的DGS视频片段中提取面部和身体运动特征；最后训练基于XGBoost的可解释模型进行情感预测。

Result: 文本情感标注的标注者间一致性达到0.781 Krippendorff's alpha；视频情感预测模型的平均平衡准确率为0.631；特征重要性分析显示，除了眉毛和嘴部动作外，臀部、肘部和肩部的运动也对情感区分有重要贡献。

Conclusion: 研究证实了面部和身体动作在德国手语情感传达中具有同等重要性，为手语情感分析提供了数据集和可解释模型，并揭示了身体运动特征在情感表达中的关键作用。
...
### [157] [On the Rejection Criterion for Proxy-based Test-time Alignment](https://arxiv.org/abs/2604.16146)
*Ayoub Hammal,Pierre Zweigenbaum,Caio Corro*

Main category: cs.CL

TL;DR: 本文提出了一种基于保守置信度赌注的新拒绝准则，用于测试时对齐方法，改进了现有基于代理模型引导大模型生成的两种方法。


<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐方法依赖小对齐模型作为代理指导大基模型生成，但隐式奖励方法会扭曲大模型分布，而推动方法在大模型不自信时依赖小模型生成，其置信度准则因语言模糊现象而动机不足。

Method: 将两种现有方法统一为从相似图模型采样，仅拒绝准则不同；提出基于保守置信度赌注的新拒绝准则，通过实验验证其有效性。

Result: 新方法在多个数据集上优于先前工作，实验证明其性能更优。

Conclusion: 基于保守置信度赌注的拒绝准则能更有效地指导测试时对齐，解决了现有方法中置信度准则的缺陷，提升了生成质量。
...
### [158] [AtManRL: Towards Faithful Reasoning via Differentiable Attention Saliency](https://arxiv.org/abs/2604.16158)
*Max Henning Höth,Kristian Kersting,Björn Deiseroth,Letitia Parcalabescu*

Main category: cs.CL

TL;DR: 提出AtManRL方法，通过可微注意力操纵和强化学习，使大语言模型的思维链推理更忠实于最终答案的生成过程，提升模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型依赖思维链推理解决复杂任务，但难以确保推理过程真实影响最终答案，而非仅伴随生成，存在忠实性和可解释性挑战。

Method: 结合可微注意力操纵与强化学习，训练可加性注意力掩码识别思维链中对答案生成关键的分词，生成显著性奖励信号，并与基于结果的奖励在GRPO框架中联合优化。

Result: 在GSM8K和MMLU数据集上使用Llama-3.2-3B-Instruct的实验表明，该方法能有效识别关键推理分词，并训练出更具可解释性的推理模型。

Conclusion: AtManRL通过注意力机制和强化学习的结合，能够提升大语言模型推理过程的忠实性和透明度，为可解释AI提供新思路。
...
### [159] [Beyond Surface Statistics: Robust Conformal Prediction for LLMs via Internal Representations](https://arxiv.org/abs/2604.16217)
*Yanli Wang,Peng Kuang,Xiaoyu Han,Kaidi Xu,Haohan Wang*

Main category: cs.CL

TL;DR: 提出了一种基于大语言模型内部表示的校准预测框架，使用层间信息分数作为非一致性评分，在问答任务中实现了比文本层面基线更好的有效性-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在可靠性要求高的场景中部署时，基于输出层面的不确定性信号（如词元概率、熵、自一致性）在校准-部署不匹配情况下可能变得脆弱，需要更鲁棒的不确定性量化方法。

Method: 提出了一个用于LLM问答的校准预测框架，引入层间信息（LI）分数作为非一致性评分，该分数通过测量输入条件如何在不同模型深度上重塑预测熵来计算，并将其应用于标准分割校准流程中。

Result: 在封闭式和开放域问答基准测试中，特别是在跨域迁移场景下，该方法在相同名义风险水平下实现了比强文本层面基线更好的有效性-效率权衡，同时保持了竞争力的域内可靠性。

Conclusion: 研究结果表明，当表面层面的不确定性在分布迁移下不稳定时，内部表示可以提供信息量更丰富的校准评分，为LLM的可靠性部署提供了新思路。
...
### [160] [Optimizing Korean-Centric LLMs via Token Pruning](https://arxiv.org/abs/2604.16235)
*Hoyeol Kim,Hyeonwoo Kim*

Main category: cs.CL

TL;DR: 本文系统评估了通过词元剪枝（一种压缩技术）优化的多语言大语言模型在韩语中心NLP任务上的表现，发现该方法能显著提升生成稳定性并有效减少词汇量，适用于内存受限的领域特定部署。


<details>
  <summary>Details</summary>
Motivation: 针对多语言大语言模型在特定语言任务中存在语言混淆和资源冗余的问题，研究旨在通过词元剪枝技术优化模型，提升其在韩语中心应用中的效率和性能。

Method: 采用词元剪枝技术压缩模型，聚焦Qwen3、Gemma-3、Llama-3和Aya等架构，在原始、英韩、英韩中三种词汇配置下，使用通用能力、文化素养、指令跟随和机器翻译基准进行评估。

Result: 词元剪枝显著提高了生成稳定性，消除了语言混淆；在机器翻译任务中常提升韩语特定性能；指令跟随能力因架构和跨语言表征而异；词汇量大幅减少，但推理延迟改善有限。

Conclusion: 词元剪枝是内存受限、领域特定部署的高效优化策略，能有效提升多语言模型在目标语言任务中的专注度和稳定性，尽管推理加速效果不显著。
...
### [161] [BAGEL: Benchmarking Animal Knowledge Expertise in Language Models](https://arxiv.org/abs/2604.16241)
...
TL;DR: BAGEL是一个评估语言模型动物知识专业性的基准测试，通过封闭式评估衡量模型在分类学、形态学、栖息地、行为、发声、地理分布和物种互动等多方面的知识，无需外部检索。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在广泛领域的知识和推理基准上表现良好，但它们在专门动物知识方面的能力尚不明确，需要统一的封闭式评估协议来测试。

Method: 从bioRxiv、Global Biotic Interactions、Xeno-canto和Wikipedia等科学和参考来源构建BAGEL基准，结合精选示例和自动生成的封闭式问答对，覆盖动物知识的多个方面。

Result: BAGEL支持跨来源领域、分类群组和知识类别的细粒度分析，能够更精确地描述模型的优势和系统性失败模式。
...
### [162] [From Benchmarking to Reasoning: A Dual-Aspect, Large-Scale Evaluation of LLMs on Vietnamese Legal Text](https://arxiv.org/abs/2604.16270)
*Van-Truong Le*

Main category: cs.CL

TL;DR: 本文提出了一个双方面评估框架，用于全面评估大语言模型在越南法律文本简化任务中的表现，揭示了模型在准确性、可读性和一致性之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 越南法律文本的复杂性阻碍了公众获取司法公正，大语言模型虽然为法律文本简化提供了潜在解决方案，但需要超越表面指标的全面评估方法来衡量其真实能力。

Method: 建立了包含准确性、可读性和一致性三个维度的性能基准，对GPT-4o、Claude 3 Opus、Gemini 1.5 Pro和Grok-1四个先进模型进行评估；在60条复杂越南法律条款数据集上进行了大规模错误分析，采用了专家验证的错误分类法。

Result: 研究发现关键权衡：Grok-1在可读性和一致性方面表现优异，但在细粒度法律准确性上有所妥协；Claude 3 Opus虽然获得高准确性分数，但掩盖了大量微妙但关键的推理错误。错误分析显示'错误示例'和'误解'是最常见的失败类型。

Conclusion: 当前大语言模型的主要挑战不是摘要能力，而是受控、准确的法律推理能力。通过整合定量基准和定性深度分析，本研究为法律应用中的大语言模型提供了全面且可操作的评估。
...
### [163] [SwanNLP at SemEval-2026 Task 5: An LLM-based Framework for Plausibility Scoring in Narrative Word Sense Disambiguation](https://arxiv.org/abs/2604.16262)
*Deshan Sumanathilaka,Nicholas Micallef,Julian Hough,Saman Jayasinghe*

Main category: cs.CL

TL;DR: 提出基于大语言模型的框架，通过结构化推理机制评估叙事文本中同形异义词的语义合理性，并探索微调小参数模型与动态少样本提示对大参数模型的影响。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试虽显示大语言模型具备消歧能力，但其在真实叙事场景中的实际应用仍缺乏充分探索，SemEval-2026 Task 5 旨在填补这一空白，评估模型对人类感知的词义合理性的预测能力。

Method: 采用基于大语言模型的框架，结合结构化推理机制；对小参数模型进行多推理策略微调，对大参数模型使用动态少样本提示；通过模型集成提升性能。

Result: 商业大参数模型配合动态少样本提示能较好模拟人类合理性判断；模型集成可略微提升性能，更接近五名标注者的一致性模式。

Conclusion: 大语言模型在叙事文本的词义合理性评估任务中展现出潜力，动态少样本提示与模型集成有助于提升模型的人类对齐能力。
...
### [164] [No Universal Courtesy: A Cross-Linguistic, Multi-Model Study of Politeness Effects on LLMs Using the PLUM Corpus](https://arxiv.org/abs/2604.16275)
*Hitesh Mehta,Arjit Saxena,Garima Chhikara,Rohit Kumar*

Main category: cs.CL

TL;DR: 研究探索了不同礼貌程度对大型语言模型响应质量的影响，发现礼貌提示可提升响应质量约11%，但效果因语言和模型而异，并发布了多语言礼貌语料库PLUM。


<details>
  <summary>Details</summary>
Motivation: 探究用户提示的礼貌程度是否以及如何影响大型语言模型的响应质量，验证礼貌作为计算变量的可量化性及其跨语言、跨模型的普适性。

Method: 基于礼貌理论和冒犯性框架，在英语、印地语、西班牙语三种语言中，对五种LLM模型进行实验，使用三种交互历史（原始、礼貌、不礼貌），通过八因素评估框架（连贯性、清晰度等）分析22,500组提示-响应对。

Result: 礼貌提示平均提升响应质量约11%，不礼貌提示降低质量；效果受语言和模型影响：英语适合礼貌或直接语气，印地语适合恭敬间接语气，西班牙语适合坚定语气；Llama对语气最敏感，GPT对抗性语气更稳健。

Conclusion: 礼貌是可量化的计算变量，能影响LLM行为，但其影响具有语言和模型依赖性，非普适性；发布了多语言礼貌语料库PLUM以支持未来研究。
...
### [165] [Foundation Models in Robotics: A Comprehensive Review of Methods, Models, Datasets, Challenges and Future Research Directions](https://arxiv.org/abs/2604.15395)
...
TL;DR: 本文对机器人领域中的基础模型进行了全面系统的综述，分析了该领域从早期自然语言处理和计算机视觉模型到当前多感官泛化和现实世界部署的五个发展阶段，并对模型类型、架构、学习范式、知识整合阶段、机器人任务和应用领域进行了详细的分类研究。


<details>
  <summary>Details</summary>
Motivation: 近年来，机器人领域正从固定、单任务、特定领域的解决方案向适应性强、多功能、通用型智能体转变，能够在复杂、开放和动态环境中操作。这一巨大进步主要由基础模型的兴起推动，这些模型在多模态理解与推理、长时程规划和跨具身泛化方面展现出前所未有的能力。

Method: 采用系统性文献综述方法，首先通过五个研究阶段描述领域演进历程，随后对文献进行细粒度分类研究，涵盖基础模型类型（如LLMs、VFMs、VLMs、VLAs）、神经网络架构、学习范式、知识整合阶段、机器人任务和实际应用领域，并进行比较分析和关键见解总结。

Result: 研究提供了机器人领域基础模型的全面分类框架，识别了不同模型类型和架构的特点，总结了当前学习范式和知识整合方法，系统梳理了机器人任务和应用领域，并整理了公开可用的训练和评估数据集。

Conclusion: 基础模型正在推动机器人技术向通用智能体方向发展，但仍面临开放挑战。本文通过分层讨论指出了当前研究局限和未来有前景的研究方向，为领域发展提供了系统性参考框架。
...
### [166] [One-Shot Cross-Geometry Skill Transfer through Part Decomposition](https://arxiv.org/abs/2604.15455)
*Skye Thompson,Ondrej Biza,George Konidaris*

Main category: cs.RO

TL;DR: 提出了一种基于语义部件分解的机器人技能迁移方法，通过生成式形状模型将交互点从演示对象迁移到新对象，实现单次演示下的跨形状泛化。


<details>
  <summary>Details</summary>
Motivation: 现有机器人技能迁移方法难以适应形状陌生的物体，而组合式建模在提升迁移能力方面具有潜力。

Method: 将物体分解为语义部件，利用数据高效的生成式形状模型迁移交互点，并自主构建优化目标以对齐技能相关部件。

Result: 方法在模拟和真实环境中均能成功实现单次演示下的技能迁移，相比现有工作能泛化到更广泛的物体几何形状。

Conclusion: 基于部件分解和生成式形状模型的技能迁移方法能有效提升机器人对陌生形状物体的适应能力。
...
### [167] [Iterated Invariant EKF for Quadruped Robot Odometry](https://arxiv.org/abs/2604.15449)
...
TL;DR: 提出了一种基于迭代不变扩展卡尔曼滤波（IterIEKF）的新型开源状态估计算法，用于足式机器人，仅依赖本体感知测量，在准确性和一致性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统卡尔曼滤波算法依赖于线性和高斯噪声假设，难以在实际机器人系统中满足。虽然IEKF已扩展了系统动力学的非线性处理能力，但测量模型的非线性扩展仍需解决，以提升足式机器人在复杂环境中的状态估计性能。

Method: 基于IterIEKF框架，设计了一种仅使用本体感知测量的状态估计算法。利用足部接触时的速度运动学约束和基座速度约束，构建鲁棒的更新步骤。通过数值仿真和真实数据集进行验证。

Result: 实验表明，IterIEKF在准确性和一致性方面均优于原始IEKF、基于SO(3)的卡尔曼滤波及其迭代变体。算法对环境条件具有内在鲁棒性。

Conclusion: 所提出的IterIEKF算法为足式机器人提供了一种高效、鲁棒的状态估计解决方案，成功将卡尔曼滤波的优良特性扩展至非线性测量模型，在实际应用中表现出优越性能。
...
### [168] [NeuroMesh: A Unified Neural Inference Framework for Decentralized Multi-Robot Collaboration](https://arxiv.org/abs/2604.15475)
...
TL;DR: 提出NeuroMesh框架，解决异构机器人系统中多机器人模型部署的挑战，通过统一管道标准化观察编码、消息传递、聚合和任务解码，支持跨平台、模块化的分散式神经推理。


<details>
  <summary>Details</summary>
Motivation: 异构机器人的硬件差异、通信限制以及缺乏统一执行栈，使得学习到的多机器人模型在实际部署中面临挑战。

Method: 采用双聚合范式（基于归约和广播的信息融合）与并行化架构（解耦周期时间和端到端延迟），基于C++高性能实现，利用Zenoh进行机器人间通信，支持GPU/CPU混合推理。

Result: 在异构空中和地面机器人团队上验证了NeuroMesh在协作感知、分散控制和任务分配中的有效性，展示了其在多样化任务结构和负载大小下的鲁棒运行。

Conclusion: NeuroMesh作为一个开源框架，为多机器人系统提供了标准化、高效的神经推理解决方案，计划向社区开放以促进相关研究与应用。
...
### [169] [Trajectory Planning for Safe Dual Control with Active Exploration](https://arxiv.org/abs/2604.15507)
*Kaleb Ben Naveed,Manveer Singh,Devansh R. Agrawal,Dimitra Panagou*

Main category: cs.RO

TL;DR: 提出Dual-gatekeeper框架，在安全与预算约束下集成鲁棒规划与主动探索，平衡任务性能与不确定性降低。


<details>
  <summary>Details</summary>
Motivation: 传统鲁棒规划过于保守且忽视不确定性降低，现有双控制方法缺乏对探索时机与安全性的形式化保证。

Method: 基于预算约束的双控制问题建模，提出Dual-gatekeeper框架，仅在探索可验证改善且不违反安全与预算时执行探索，并提供两种安全机制实现。

Result: 在四旋翼导航和自动驾驶赛车案例中验证了框架在参数不确定性下的有效性，实现了安全、预算可行且性能平衡的轨迹规划。

Conclusion: 该框架为安全关键系统提供了原则性的双控制解决方案，能动态权衡即时任务性能与长期不确定性降低。
...
### [170] [ShapeGen: Robotic Data Generation for Category-Level Manipulation](https://arxiv.org/abs/2604.15569)
*Yirui Wang,Xiuwei Xu,Angyuan Ma,Bingyao Yu,Jie Zhou,Jiwen Lu*

Main category: cs.RO

TL;DR: 提出ShapeGen方法，通过无仿真器的3D方式生成形状多样化的操作数据，提升策略对类别内不同形状物体的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现实世界中同类物体几何形状差异大，需要类别级泛化能力。手动收集形状多样化的训练数据成本高、不现实

Method: 两阶段方法：1) 形状库构建：训练形状间的空间扭曲映射，建立功能对应点；2) 功能感知生成：利用形状库，通过少量人工标注生成物理合理、功能正确的新演示数据

Result: 真实世界实验表明ShapeGen能有效提升策略在类别内的形状泛化能力

Conclusion: ShapeGen提供了一种高效生成形状多样化操作数据的方法，解决了手动数据收集的瓶颈，增强了机器人策略的类别级泛化性能
...
### [171] [GaussianFlow SLAM: Monocular Gaussian Splatting SLAM Guided by GaussianFlow](https://arxiv.org/abs/2604.15612)
*Dong-Uk Seo,Jinwoo Jeon,Eungchang Mason Lee,Hyun Myung*

Main category: cs.RO

TL;DR: 提出GaussianFlow SLAM，一种基于3D高斯泼溅的单目SLAM系统，利用光流作为几何感知线索来优化场景结构和相机位姿，解决了单目输入缺乏可靠几何监督的问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在SLAM中展现出密集和逼真的场景建模能力，但应用于单目SLAM时，由于缺乏可靠的几何线索，容易陷入局部最优，导致结构退化和不准确。

Method: 1. 利用光流作为几何感知线索，通过使高斯投影运动（GaussianFlow）与光流对齐，为地图重建和位姿估计提供一致的结构约束；2. 引入基于归一化误差的致密化和剪枝模块，优化非活跃和不稳定的高斯分布。

Result: 在公开数据集上的实验表明，该方法在渲染质量和跟踪精度方面优于现有最先进算法。

Conclusion: GaussianFlow SLAM通过光流引导的优化策略，有效提升了单目3D高斯泼溅SLAM系统的性能，实现了更高质量的地图重建和更准确的相机跟踪。
...
### [172] [Factor Graph-Based Shape Estimation for Continuum Robots via Magnus Expansion](https://arxiv.org/abs/2604.15619)
...
<details>
  <summary>Details</summary>
Motivation: 现有连续体机械臂形状重建方法存在矛盾：参数化方法状态紧凑但缺乏概率结构，因子图方法能量化不确定性但状态维度随离散化增长。需要结合两者优势。

Method: 在因子图框架中估计低维几何可变应变（GVS）参数化系数，通过基于马格努斯展开推导的新型运动学因子，将应变系数与骨架位姿变量关联为约束先验。

Result: 在0.4米肌腱驱动连续体机器人仿真中，三种测量配置下平均位置误差均低于2毫米；仅使用位置测量时，方向误差比高斯过程回归基线降低六倍。

Conclusion: 该方法在保持因子图模块化、概率处理和计算效率的同时，实现了紧凑状态表示，可直接用于基于模型的控制，为连续体机械臂形状估计提供了新框架。
...
### [173] [Contact-Aware Planning and Control of Continuum Robots in Highly Constrained Environments](https://arxiv.org/abs/2604.15638)
*Aedan Mangan,Kehan Long,Ki Myung Brian Lee,Miheer Potdar,Nikolay Atanasov,Tania K. Morimoto*

Main category: cs.RO

TL;DR: 提出了一种接触感知的连续体机器人路径规划方法，能在受限解剖环境中区分危险与良性接触，通过硬件实验验证了其在三种解剖模型中的有效性，成功率达100%。


<details>
  <summary>Details</summary>
Motivation: 连续体机器人在血管或腔内解剖等受限脆弱环境中运动时，接触难以避免。不良接触可能导致可控性下降、运动学奇异或安全风险，需要区分接触类型以实现安全导航。

Method: 开发接触感知规划器，评估接触质量并惩罚危险交互；生成运动学可行轨迹和接触感知雅可比矩阵；在基于患者扫描的解剖模型上进行集成系统（规划、控制与机械设计）实验验证。

Result: 在三种解剖环境中均生成有效路径；硬件实验中机器人成功抵达目标且避免危险尖端接触（成功率100%）；平均跟踪误差分别为1.9±0.5mm、1.2±0.1mm、1.7±0.2mm；消融研究表明惩罚末端接触可提升可操纵性并防止硬件故障。

Conclusion: 该方法实现了高度受限环境中可靠、接触感知的导航，通过区分接触类型平衡了运动效率与安全性，为临床解剖环境中的机器人应用提供了解决方案。
...
### [174] [Long-Term Memory for VLA-based Agents in Open-World Task Execution](https://arxiv.org/abs/2604.15671)
*Xu Huang,Weixin Mao,Yinhao Li,Hua Chen,Jiabao Zhao*

Main category: cs.RO

TL;DR: 提出ChemBot框架，结合自主AI代理与进度感知VLA模型，通过双层记忆架构和异步推理机制提升化学实验室自动化中的长程决策能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在复杂化学实验室自动化中受限于长程推理能力不足和缺乏经验积累，导致多阶段实验协议中效率低下。

Method: 1. 设计双层闭环框架整合AI代理与Skill-VLA模型；2. 采用双层记忆架构存储成功轨迹；3. 通过MCP服务器协调子代理与工具；4. 实现基于未来状态的异步推理机制。

Result: 在协作机器人实验中，ChemBot在操作安全性、精度和任务成功率上均优于现有VLA基线模型。

Conclusion: ChemBot通过经验积累与分层执行机制，显著提升了复杂化学实验自动化中的长程决策性能。
...
### [175] [Fuzzy Logic Theory-based Adaptive Reward Shaping for Robust Reinforcement Learning (FARS)](https://arxiv.org/abs/2604.15772)
*Hürkan Şahin,Van Huyen Dang,Erdi Sayar,Alper Yegenoglu,Erdal Kayacan*

Main category: cs.RO

TL;DR: 本文提出了一种基于模糊逻辑的奖励塑形方法，将人类直觉融入强化学习奖励设计，在自主无人机竞速任务中实现了更稳定的学习和更快的收敛。


<details>
  <summary>Details</summary>
Motivation: 强化学习在高维状态空间和长时程任务中常因稀疏或固定奖励导致探索缓慢、陷入局部最优，需要更有效的奖励设计方法。

Method: 采用模糊逻辑奖励塑形方法，将专家知识编码为自适应且可解释的模糊规则，根据智能体状态动态调整奖励贡献。

Result: 在自主无人机竞速基准测试中，该方法表现出稳定的学习行为，在更具挑战性的环境中实现了更快的收敛和更低的性能方差，成功率比非模糊奖励方法提升约5%。

Conclusion: 模糊逻辑奖励塑形能有效整合人类直觉，促进强化学习在复杂导航任务中的稳定学习和性能提升，降低对超参数的敏感性。
...
### [176] [From Seeing to Simulating: Generative High-Fidelity Simulation with Digital Cousins for Generalizable Robot Learning and Evaluation](https://arxiv.org/abs/2604.15805)
...
TL;DR: 提出生成式框架，将真实全景图映射到高保真仿真场景，通过语义和几何编辑合成多样化场景，支持机器人交互任务和长视野导航，实验验证了仿真到真实的强相关性及数据扩展对泛化能力的提升。


<details>
  <summary>Details</summary>
Motivation: 真实世界机器人策略学习需要多样化数据增强，但物理数据收集成本高昂，因此将真实场景增强到仿真中成为高效学习和评估的实用方法。

Method: 建立生成式真实到仿真映射框架，从真实全景图生成高保真仿真场景，通过语义和几何编辑合成多样化“表亲场景”，结合高质量物理引擎和真实资产，并采用多房间拼接构建大规模一致环境。

Result: 实验显示仿真到真实具有强相关性，验证了平台保真度；大规模数据生成显著提升了对未见场景和物体变化的泛化能力。

Conclusion: Digital Cousins框架能有效支持可泛化的机器人学习和评估，通过仿真数据扩展提升机器人策略的适应性和鲁棒性。
...
### [177] [Limits of Lamarckian Evolution Under Pressure of Morphological Novelty](https://arxiv.org/abs/2604.15854)
*Jed R Muff,Karine Miras,A. E. Eiben*

Main category: cs.RO

TL;DR: 研究探讨了在机器人形态和控制器协同进化中，拉马克遗传在追求形态多样性时的表现，发现其优势在形态差异增大时显著下降。


<details>
  <summary>Details</summary>
Motivation: 拉马克遗传能加速机器人形态和控制器的进化，但其效果依赖于亲子形态相似性；当进化过程强调形态多样性时，这种相似性可能被破坏，从而影响性能。

Method: 使用模块化机器人系统，在解决运动任务中比较达尔文和拉马克进化；从单纯任务性能优化转向多目标选择（同时奖励形态新颖性），分析性能变化和亲子相似性。

Result: 仅优化任务性能时，拉马克进化优于达尔文进化；但引入形态多样性压力后，拉马克系统性能大幅下降，且降幅更大，因为多样性降低了亲子相似性，削弱了继承学习控制器的益处。

Conclusion: 拉马克进化存在局限性，揭示了基于遗传的利用和多样性驱动的探索之间的根本权衡，形态多样性会损害其优势。
...
### [178] [Environment-Adaptive Solid-State LiDAR-Inertial Odometry](https://arxiv.org/abs/2604.15864)
*Zhi Zhang,Chalermchon Satirapod,Bingtao Ma,Changjun Gu*

Main category: cs.RO

TL;DR: 提出一种环境自适应固态激光雷达-惯性里程计方法，通过局部法向量约束和退化感知地图维护，提升极端环境下的SLAM精度与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 固态激光雷达-惯性SLAM在极端环境中面临严重几何退化和不可靠观测问题，导致优化病态和地图不一致，需要提高其在这些场景下的准确性和稳定性。

Method: 1. 引入局部法向量约束以增强状态估计稳定性；2. 设计退化感知的地图更新策略，通过退化指导地图维护提升地图精度。

Result: 在极端和感知退化环境中，该方法相比基线方法平均RMSE降低达12.8%，实现了更优的建图精度和鲁棒性。

Conclusion: 所提出的环境自适应方法通过融合局部几何约束与智能地图维护，有效解决了固态激光雷达-惯性SLAM在退化环境中的挑战，显著提升了系统性能。
...
### [179] [DTEA: A Dual-Topology Elastic Actuator Enabling Real-Time Switching Between Series and Parallel Compliance](https://arxiv.org/abs/2604.15865)
*Vishal Ramesh,Aman Singh,Shishir Kolathaya*

Main category: cs.RO

TL;DR: 提出了一种新型双拓扑弹性驱动器（DTEA），能够在运行中实时切换串联弹性执行器（SEA）和并联弹性执行器（PEA）两种拓扑结构，并通过原型验证了其可行性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有串联和并联弹性执行器各有优势但无法在运行中切换拓扑结构，限制了执行器的适应性和性能优化。

Method: 设计了DTEA原型机，通过实验评估拓扑切换机制的鲁棒性、切换时间、静态刚度以及抗干扰性能。

Result: DTEA在负载下完成324次拓扑切换无损坏，切换时间小于33.33毫秒；PEA模式刚度是SEA的1.53倍，抗干扰时峰值偏转小2.26倍，稳定时间快3.45倍。

Conclusion: DTEA成功实现了运行中拓扑切换，兼具SEA和PEA的优势，为弹性执行器设计提供了新思路。
...
### [180] [Robust Fleet Sizing for Multi-UAV Inspection Missions under Synchronized Replacement Demand](https://arxiv.org/abs/2604.15890)
*Vishal Ramesh,Antony Thomas*

Main category: cs.RO

TL;DR: 本文针对多无人机巡检任务中备用无人机数量规划问题，提出了一种考虑电池同步耗尽导致需求聚集的闭式解舰队规模规划规则，相比传统方法显著提升了任务级可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有舰队规模规划方法通常假设稳态运行条件或需求事件统计独立，无法处理有限时间任务中因高效路由导致无人机电池同步耗尽、产生聚集性替换需求的情况，导致备用池耗尽风险。

Method: 推导出闭式充分性舰队规模规划规则 k = m(ceil(R) + 1)，其中 m 为活跃无人机数量，R 为恢复时间与活跃时间之比；通过蒙特卡洛模拟在五种场景下验证，并与基于 Erlang-B 模型的每请求阻塞保证方法进行对比。

Result: 在 R=3.39 的最严苛场景中，传统 Erlang-B 方法（ε=0.01）任务成功率降至 69.9%，且 95% 的备用耗尽事件集中在需求最高的 5 分钟窗口内；而所提规则在所有测试条件下（包括风速变异系数 CV=0.30）均保持 99.8% 的成功率，在最严苛场景仅需额外增加 4 架无人机。

Conclusion: 所提出的加性缓冲规则能有效吸收恢复周期边界处最坏情况同步需求，确保即使所有无人机同时耗尽电池时仍能维持任务级可靠性，为有限时间多无人机任务提供了鲁棒的舰队规模规划方法。
...
### [181] [A Reconfigurable Pneumatic Joint Enabling Localized Selective Stiffening and Shape Locking in Vine-Inspired Robots](https://arxiv.org/abs/2604.15907)
*Ayodele James Oyejide,Ustaz A. Yaqub,Samir Erturk,Eray A. Baran,Fabio Stroppa*

Main category: cs.RO

TL;DR: 提出一种可重构气动关节（RPJ）架构，用于增强藤蔓机器人的轴向刚度、负载能力和形状保持能力，使其能在自由空间执行操作任务。


<details>
  <summary>Details</summary>
Motivation: 传统藤蔓机器人虽能在受限环境中安全导航，但在自由空间中存在轴向刚度低、负载能力差、转向时难以保持形状等根本性限制，难以执行操作任务。

Method: 设计对称分布气动腔的RPJ模块，通过气压调节局部弯曲刚度；将其集成到肌腱驱动转向的软体生长机器人中，并开发紧凑的中空翻转基站。

Result: 实验验证显示：RPJ机器人实现了适中的翻转气压需求、与层压机构相当的局部刚化和转向性能；在弯曲时形状保持更好、负载下重力偏转减小、可级联收缩，并在自由空间可靠运输202克载荷。

Conclusion: RPJ机制为结构自适应藤蔓机器人提供了实用路径，使其能够胜任物体分拣和非受限环境自适应探索等操作导向任务。
...
### [182] [VADF: Vision-Adaptive Diffusion Policy Framework for Efficient Robotic Manipulation](https://arxiv.org/abs/2604.15938)
*Xinglei Yu,Zhenyang Liu,Shufeng Nan,Simo Wu,Yanwei Fu*

Main category: cs.RO

TL;DR: 提出VADF框架，通过自适应损失网络和分层视觉任务分割器，解决扩散策略在机器人操作中的训练收敛慢和推理超时问题。


<details>
  <summary>Details</summary>
Motivation: 扩散策略在机器人操作中因均匀采样和缺乏样本难度感知，导致训练收敛缓慢和推理频繁超时失败。

Method: 1. 训练阶段：使用自适应损失网络实时量化样本难度，通过加权采样优先处理高损失区域；2. 推理阶段：设计分层视觉任务分割器，根据视觉输入将任务分解为多阶段子指令，并为不同复杂度子任务分配不同的噪声调度和执行序列。

Result: 显著减少收敛步数，提高推理早期成功率，降低计算开销，且模型无关设计可无缝集成到任何扩散策略架构。

Conclusion: VADF框架通过视觉驱动的双自适应机制，有效解决了扩散策略在机器人操作中的关键瓶颈，提升了训练效率和推理性能。
...
### [183] [DENALI: A Dataset Enabling Non-Line-of-Sight Spatial Reasoning with Low-Cost LiDARs](https://arxiv.org/abs/2604.16201)
*Nikhil Behari,Diego Rivero,Luke Apostolides,Suman Ghosh,Paul Pu Liang,Ramesh Raskar*

Main category: cs.RO

TL;DR: 本文提出DENALI数据集，首次大规模采集低成本LiDAR的时空直方图数据，证明通过数据驱动方法可实现基于消费级LiDAR的非视距感知。


<details>
  <summary>Details</summary>
Motivation: 消费级LiDAR内部记录的时分辨直方图包含多路径光信号，能编码隐藏物体的非视距信息，但硬件限制使传统重建方法困难，需探索数据驱动的替代方案。

Method: 构建DENALI数据集，采集72,000个隐藏物体场景的时分辨LiDAR直方图，涵盖不同物体形状、位置、光照和空间分辨率；采用数据驱动推理方法进行非视距感知。

Result: 实验表明消费级LiDAR能实现准确的数据驱动非视距感知，同时识别了影响性能的关键场景与建模因素，以及当前仿真到真实迁移存在的保真度差距。

Conclusion: 研究验证了低成本LiDAR实现非视距感知的可行性，为未来消费级LiDAR的规模化非视距视觉应用提供了数据基础和研究方向。
...
### [184] [Semantic Area Graph Reasoning for Multi-Robot Language-Guided Search](https://arxiv.org/abs/2604.16263)
*Ruiyang Wang,Hao-Lun Hsu,Jiwoo Kim,Miroslav Pajic*

Main category: cs.RO

TL;DR: 提出SAGR框架，利用大型语言模型通过语义区域图协调多机器人探索和语义搜索，在保持探索竞争力的同时显著提升语义目标搜索效率。


<details>
  <summary>Details</summary>
Motivation: 传统多机器人协调策略依赖几何探索，无法融入高层任务意图（如搜索特定房间类型的物体），需要结合语义推理的协调方法。

Method: SAGR分层框架：增量构建语义区域图（编码房间实例、连通性、边界可用性和机器人状态），LLM进行高层语义房间分配，确定性边界规划和局部导航处理几何执行。

Result: 在Habitat-Matterport3D数据集的100个场景中，SAGR在保持与前沿探索方法竞争力的同时，语义目标搜索效率显著提升（大型环境中最高达18.8%）。
...
### [185] [Load Block Modeling in Distribution Systems: Network Reconfiguration for Load Restoration](https://arxiv.org/abs/2604.15480)
*David M. Fobes,Harsha Nagarajan,Manuel Garcia,Robert Ferrando,Russell Bent*

Main category: eess.SY

TL;DR: 本文重新审视了配电网恢复（DSR）问题中负荷削减、供电和恢复的建模方法，提出了更精确的数学模型。


<details>
  <summary>Details</summary>
Motivation: 配电网恢复问题在计算上具有挑战性，现有模型难以准确处理配电网特有的工程约束（如辐射状运行要求），需要更精确的负荷管理建模方法。

Method: 重新构建了负荷削减、供电和恢复过程的数学模型，开发了能更准确反映配电网实际运行要求的优化问题表述。

Result: 提出了改进的DSR问题表述，能够更精确地建模配电网中的负荷管理操作，解决了现有模型在辐射状运行等约束方面的不足。

Conclusion: 新的建模方法提高了配电网恢复问题求解的准确性和实用性，为更有效的配电网恢复策略提供了理论基础。
...
### [186] [Temporal Derivative Soft-Sensing and Reconstructing Solar Radiation and Heat Flux from Common Environmental Sensors](https://arxiv.org/abs/2604.15352)
*Neksha DeSilva*

Main category: eess.SY

TL;DR: 提出了一种基于物理的差分时间导数软传感方法，使低成本传感器阵列能够通过建模辐射热通量来推断环境能量交换。


<details>
  <summary>Details</summary>
Motivation: 现有环境监测方法只能测量温度、压力、湿度等参数，但无法捕捉能量流动的物理原因，缺乏对能量交换的量化能力。

Method: 采用差分时间导数软传感方法，结合新型配对传感器配置和惯性噪声降低算法，通过计算全球水平辐照度和对流热通量来建模环境能量流动。

Result: 实验测试使用斯里兰卡气象部门校准的参考辐射计，在微控制器不足2KB RAM条件下，8位嵌入式处理器结果与参考值的R²约0.9，RMSE约45W/m²。

Conclusion: 该方法成功实现了低成本传感器对环境能量交换的准确推断，为环境监测提供了新的物理基础解决方案。
...
### [187] [''It Is Much Safer to Be Sparse than Connected'': Safe Control of Robotic Swarm Density Dynamics with PDE-Optimization with State Constraints](https://arxiv.org/abs/2604.15516)
*Longchen Niu,Gennaro Notomista*

Main category: eess.SY

TL;DR: 提出了一种基于控制Lyapunov函数和控制屏障函数的安全关键优化控制策略，用于引导服从Fokker-Planck方程的机器人群体密度分布至目标分布，支持闭环控制和分布式部署，并通过实验验证了稀疏群体更易满足安全约束。


<details>
  <summary>Details</summary>
Motivation: 传统开环状态约束最优控制方法在机器人群体密度控制中存在局限性，需要一种能够保证安全性的闭环控制策略，并适应分布式部署需求。

Method: 结合控制Lyapunov函数和控制屏障函数构建安全关键优化控制器，基于Fokker-Planck方程建模群体动力学，提出Voronoi图变体实现分布式部署。

Result: 理论推导证明了控制器的安全性保证，数值仿真验证了控制性能，多机器人实验表明该方法在定位和运动噪声下具有实际应用可行性，且稀疏群体比密集群体更容易满足安全约束。

Conclusion: 所提出的闭环控制策略能有效安全地引导机器人群体密度分布，分布式变体增强了部署灵活性，实验证实了方法的实用性和稀疏群体在安全约束方面的优势。
...
### [188] [Safety Filtering with an Infinite Number of Constraints](https://arxiv.org/abs/2604.15477)
*Max H. Cohen,Pio Ong,Pol Mestres,Aaron D. Ames*

Main category: eess.SY

TL;DR: 本文扩展了控制屏障函数理论，将其应用于无限约束场景，解决了安全集定义中的基本理论问题，包括前向集不变性条件、CBF定义、控制器正则性以及约束简化方法。


<details>
  <summary>Details</summary>
Motivation: 现有CBF理论主要针对有限约束，但在备份CBF等应用中需要处理无限约束。目前对无限约束下安全集的理论基础存在缺失，包括不变性条件、CBF定义、控制器正则性等基本问题尚未解决。

Method: 通过扩展CBF理论到无限约束设置，识别使Nagumo定理简化为屏障不等式的正则条件，分析关联CBF控制器的连续性，并将结果与最优衰减CBF连接，最后用备份CBF案例验证理论。

Result: 建立了无限约束下CBF的理论框架，明确了保证前向集不变性的必要充分条件，证明了在特定正则条件下CBF控制器至少具有连续性，并展示了如何将无限约束集简化为有限约束。

Conclusion: 该研究填补了无限约束CBF的理论空白，为实际应用（如备份CBF）提供了理论基础，连接了不变性理论条件与控制器的实际实现。
...
### [189] [Perron-Frobenius Contractive Operator Matching for Data-Driven Reachable Fault Identification and Recovery](https://arxiv.org/abs/2604.15470)
*Joshua D. Ibrahim,Mahdi Taheri,Soon-Jo Chung,Fred Y. Hadaegh*

Main category: eess.SY

TL;DR: 提出一种基于概率密度空间的数据驱动故障检测、识别与恢复框架，用于非线性仿射控制系统，通过故障索引的Perron-Frobenius算子预测状态分布演化，并在10态航天器姿态控制系统上验证。


<details>
  <summary>Details</summary>
Motivation: 传统基于轨迹的故障处理方法难以处理非线性系统的随机性和故障多样性，需要一种能在概率密度空间统一处理故障检测、识别与恢复的鲁棒框架。

Method: 1) 利用Fokker-Planck方程的概率流表示构建确定性PF算子；2) 通过流图匹配从轨迹数据学习故障索引算子；3) 建立2-Wasserstein距离的可认证边界；4) 在线连续故障参数拟合；5) 结合可达密度传播和高斯混合协方差控制进行恢复。

Result: 1) 建立了故障驱动与标称密度演化差异的定量可认证边界；2) 为各类故障的可检测性和可识别性提供了量化条件；3) 学习算子的近似误差可由观测残差直接界定；4) 在四反作用轮航天器姿态控制系统中成功验证。

Conclusion: 该概率密度空间框架能统一处理非线性系统的故障检测、识别与恢复，通过数据驱动的算子学习和可认证边界，实现了对连续参数空间故障的泛化处理，特别适用于复杂随机控制系统。
...
### [190] [Safe and Energy-Aware Multi-Robot Density Control via PDE-Constrained Optimization for Long-Duration Autonomy](https://arxiv.org/abs/2604.15524)
*Longchen Niu,Andrew Nasif,Gennaro Notomista*

Main category: eess.SY

TL;DR: 提出了一种多机器人密度控制框架，保证空间安全和能量可持续性，通过Fokker-Planck PDE描述随机运动，结合控制Lyapunov和屏障函数实现目标跟踪、避障和能量管理。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统在存在定位和运动不确定性的情况下，需要同时保证空间安全（避障）和能量可持续性（多次充电循环），传统方法难以统一处理这些约束。

Method: 使用Fokker-Planck PDE在密度层面编码随机机器人运动；结合控制Lyapunov函数（目标密度跟踪）和控制屏障函数（避障和能量充足）；通过二次规划实现实时闭环控制。

Result: 多机器人实验和大量仿真表明，该控制器在定位和运动不确定性下能有效实现密度跟踪、避障和能量可持续性，且计算效率支持实时调整。

Conclusion: 该框架为多机器人系统提供了统一的密度控制方法，同时保障空间安全和能量可持续性，适用于存在不确定性的实际场景。
...
### [191] [Dual-Radio BLE-LoRa Hierarchical Mesh for Infrastructure-Free Emergency Communication](https://arxiv.org/abs/2604.15532)
*Andrii Vakhnovskyi*

Main category: eess.SY

TL;DR: 提出一种结合BLE和LoRa的双无线电分层网状架构，用于无基础设施应急通信，通过BLE处理大部分本地流量，显著降低LoRa能耗并提升网络规模。


<details>
  <summary>Details</summary>
Motivation: 应急场景下缺乏基础设施时，现有单一无线技术（如纯LoRa网状网络）能耗高、扩展性有限，需要一种能兼顾低功耗、远距离和大规模节点连接的通信方案。

Method: 采用nRF52840（BLE 5.0）和SX1262（LoRa）双模硬件，通过BLE广播式AODV路由组建本地集群，动态选举簇头节点，利用LoRa骨干网进行跨集群通信，并建立流量卸载模型优化能耗。

Result: 当局部流量偏置系数β≥0.76时，82-90%的流量通过BLE传输，LoRa能耗比纯LoRa网状网络降低79%；网络直径超过10公里，支持250-562个节点，集群内延迟低于50毫秒，仅需3.0 KB内存。

Conclusion: 该架构首次在商用硬件上实现了BLE广播式网状路由与多跳LoRa骨干网的结合，为无基础设施应急通信提供了高能效、可扩展的解决方案。
...
### [192] [Nonlinear Stochastic Density Steering via Gaussian Mixture Schrodinger Bridges and Multiple Linearizations](https://arxiv.org/abs/2604.15576)
*Mattia Mosso,George Rapakoulias,Yue Guan,Panagiotis Tsiotras*

Main category: eess.SY

TL;DR: 本文提出了一种多分布到分布线性化方法，用于解决非线性连续时间随机系统的最优密度控制问题，通过高斯混合模型分解和局部线性化策略，在不确定性较高的区域获得比单线性化更精确的近似。


<details>
  <summary>Details</summary>
Motivation: 传统单点线性化方法在非线性系统的高不确定性区域（显著偏离标称线性化点）难以准确捕捉系统动态，需要更精确的近似方法处理边界分布间的密度控制问题。

Method: 1. 使用高斯混合模型近似边界分布；2. 将原非线性问题分解为高斯分量对之间的高斯到高斯最优协方差控制子问题；3. 通过连接对应高斯分量均值的轨迹进行局部线性化求解各子问题；4. 根据条件密度组合基本策略。

Result: 1. 理论证明多线性化方法比单线性化具有更紧的近似误差界；2. 在地球到火星轨道转移场景的数值实验中验证了方法的有效性。

Conclusion: 所提出的多分布线性化框架能更精确地处理非线性随机系统的密度控制问题，特别适用于高不确定性区域，为复杂轨迹优化提供了新思路。
...
### [193] [A Common Lyapunov Matrix Approach to the Exponential Stability of Augmented Primal-Dual Gradient Flow as LPV Systems](https://arxiv.org/abs/2604.15624)
*Mengmou Li,Lijun Zhu,Masaaki Nagahara*

Main category: eess.SY

TL;DR: 本文证明了两个Hurwitz矩阵的凸组合存在公共Lyapunov矩阵的充要条件，并将其应用于带仿射不等式约束优化问题的增广原始-对偶梯度流收敛性分析。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于分析带约束优化问题的梯度流收敛性，特别是当系统可视为由主动约束选择器驱动的多面体线性参数变化（LPV）系统时，需要建立更通用的稳定性判据。

Method: 采用Lyapunov矩阵集合交集分析的方法，将问题转化为一个矩阵的严格Lyapunov矩阵集合与另一个矩阵的非严格Lyapunov矩阵集合的交集非空性判定，并扩展至LPV系统的积分二次约束（IQC）框架进行数值计算。

Result: 在松弛的强凸性条件下，证明了LPV系统的指数收敛性，并建立了可通过IQC框架数值搜索收敛率的分析框架。

Conclusion: 提出的Lyapunov矩阵交集条件为分析凸组合Hurwitz矩阵的稳定性提供了简洁判据，成功应用于约束优化梯度流的收敛性证明，且该框架可扩展至更一般的LPV系统分析。
...
### [194] [Verification of Autonomous Systems with Optimal Controllers](https://arxiv.org/abs/2604.15659)
*Dylan Le,Joel McCandless,Carlos Varela,Radoslav Ivanov*

Main category: eess.SY

TL;DR: 本文提出了一种针对采用最优控制器的控制系统进行可达性分析的方法，将梯度下降算法视为嵌入物理系统的数字动力系统，并在四旋翼和倒立摆系统上验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 最优控制器虽能通过成本最小化保证任务完成，但随着系统动态复杂度增加，实时求解优化问题变得困难。因此需要验证即使在未找到最优解的情况下，控制器仍能实现高级控制目标，确保系统安全性与正确性。

Method: 将梯度下降算法建模为独立的数字动力系统，嵌入原始物理动力系统中，并将控制量作为状态变量的一部分，设计相应的可达性分析算法。

Result: 所提方法在二维四旋翼和倒立摆两个控制系统上验证了可行性，证明了该框架能够分析含梯度下降优化过程的闭环系统的可达性。

Conclusion: 通过将梯度下降算法形式化为嵌入系统的数字动力系统，为复杂最优控制系统的安全验证提供了一种有效的可达性分析途径。
...
### [195] [DAE-Aware Bayesian Inference for Joint Generator-Network Parameter Estimation](https://arxiv.org/abs/2604.15686)
*Abdallah Alalem Albustami,Ahmad F. Taha,Sankaran Mahadevan*

Main category: eess.SY

TL;DR: 本文针对多机电力系统模型中的参数估计问题，提出了一个联合贝叶斯推断框架，利用电网微分代数方程模型同时估计发电机和网络参数，并通过物理感知统计建模与高效后验采样实现实用化。


<details>
  <summary>Details</summary>
Motivation: 电力系统模型通常由非线性微分代数方程描述，但发电机惯量、阻尼以及网络支路电阻、电抗等参数的辨识与估计研究相对不足。现有方法多依赖ODE近似，缺乏对完整DAE模型的利用。

Method: 开发了联合贝叶斯推断框架，结合物理感知统计建模与计算高效的后验采样方法，直接基于电网DAE模型进行发电机和网络参数的联合估计。

Result: 在IEEE 9总线系统上实现了准确的参数恢复，后验不确定性表现良好；在39总线系统的初步研究中，框架在更大规模的联合估计问题上仍保持有效性，且无需过于保守的先验假设。

Conclusion: 所提出的联合贝叶斯校准框架能够有效处理电力系统DAE模型的参数估计问题，为多机系统的参数辨识提供了实用且可扩展的解决方案。
...
### [196] [Integrating AI and Simulation for Teaching Power System Dynamics: An Interactive Framework for Engineering Education](https://arxiv.org/abs/2604.15697)
*Osasumwen Cedric Ogiesoba-Eguakun,Phani Kumar Inkollu,Rupesh Sah,Zia Rashid,Douglas Jussaume,Suman Rath*

Main category: eess.SY

TL;DR: 提出一个基于AI的交互式学习框架，结合仿真与智能反馈，以解决电气工程中学生对电力系统动态概念理解困难的问题。


<details>
  <summary>Details</summary>
Motivation: 电气工程与能源系统教学中，学生常因概念抽象、数学复杂且缺乏实践机会，难以掌握电力系统动态知识；AI与大型语言模型的发展为改善工程教育提供了新机遇。

Method: 构建三层框架：AI层（提供解释与指导）、仿真层（模拟系统行为）、用户层（实时交互）；设计闭环学习流程，支持学生探索系统、调整参数并获取反馈；提出教育者分步实施指南，包括概念分解、仿真应用与效果评估。

Result: 框架通过仿真与智能反馈的融合，增强了学生对抽象概念的理解与实践能力，提升了学习参与度；为工程教育中负责任地使用AI工具提供了可行路径。

Conclusion: 该AI交互式学习框架能有效促进电气工程教育的实践性与应用性，帮助学生将理论知识与实际系统结合，并为AI在工程教育中的合理应用提供实践范例。
...
### [197] [QGas: Interactive Gas Infrastructure Toolkit](https://arxiv.org/abs/2604.15876)
*Marco Quantschnig,Yannick Werner,Sonja Wogrin,Thomas Klatzer*

Main category: eess.SY

TL;DR: 提出了QGAS交互式工具包，用于可视化、创建和协作扩展地理参考的燃气基础设施数据集，支持多载体能源系统规划。


<details>
  <summary>Details</summary>
Motivation: 燃气基础设施数据分散在异构来源中，现有工具创建拓扑一致网络表示复杂、耗时且易出错，需要统一工具支持脱碳能源规划。

Method: 开发基于Python、JavaScript和Leaflet地图库的模块化架构，集成GIS几何编辑与拓扑保持图操作，提供网络数字化、编辑、属性管理和拓扑一致修改功能。

Result: 实现统一Web环境工具包，通过天然气数据集扩展至氢气和CO2基础设施的示例，证明其能支持一致的多载体燃气基础设施数据集准备。

Conclusion: QGAS工具包能有效简化燃气基础设施数据集成与扩展过程，为能源系统规划提供一致的多载体网络表示支持。
...
### [198] [Dispersion-Domain Detection for Mobile Molecular Communication Under Multiplicative Geometry Uncertainty](https://arxiv.org/abs/2604.15896)
*Shaojie Zhang,Ozgur B. Akan*

Main category: eess.SY

TL;DR: 该论文研究了移动分子通信中具有计数接收器的二进制检测问题，提出了一种针对未知几何增益和符号间干扰的色散域统计方法，通过高斯近似实现稳定的阈值检测。


<details>
  <summary>Details</summary>
Motivation: 移动分子通信中，发射器与接收器的几何关系不确定性会随机化接收强度，导致超出泊松分布的色散，影响检测性能，需要一种对几何增益变化鲁棒的检测方法。

Method: 提出了一种基于轮廓化色散域的统计量T_k^(Δ)，通过减去固有泊松分量并以轮廓化均值的平方归一化，结合活动门控技术，使用时序中心极限定理的高斯近似进行性能分析。

Result: 仿真表明，该方法在测试增益范围内具有经验阈值稳定性，在均值域差异较弱、不可靠或故意抑制时仍有效，支持所提机制的有效性。

Conclusion: 所提出的色散域统计方法能够处理移动分子通信中的几何不确定性和符号间干扰，提供稳定的检测性能，适用于均值域信息不足的场景。
...
### [199] [From Individual Consumers to Energy Communities: A Techno-economic Assessment of Swiss Local Electricity Communities](https://arxiv.org/abs/2604.15900)
*Na Li,Binod Koirala*

Main category: eess.SY

TL;DR: 本研究评估了瑞士新法律框架下合规本地能源社区（LEC）的技术经济表现，发现LEC参与能提高本地可再生能源利用率、减少电网输出，并为消费者和产消者带来经济效益。


<details>
  <summary>Details</summary>
Motivation: 随着瑞士能源社区从政策设计转向实施阶段，了解其实际运行表现变得日益重要，需要为决策和未来发展提供实践依据。

Method: 采用技术经济评估方法，对比无本地电力交换的基准案例与内部电力共享的LEC场景，并进行敏感性分析，重点关注内部电价机制的影响。

Result: LEC参与显著提高了本地可再生能源利用率，减少了电网输出，并为所有参与者带来经济效益；敏感性分析表明内部电价是平衡整体效率与利益分配公平性的关键因素。

Conclusion: 这项探索性研究为瑞士LEC的知情决策和未来发展提供了实践见解，强调了内部电价机制在优化社区运行效果中的重要作用。
...
### [200] [A Practical Guide to PID Controller Implementation](https://arxiv.org/abs/2604.15918)
*E. Sundström,M. Bauer,J. L. Guzmán,T. Hägglund,K. Soltesz*

Main category: eess.SY

TL;DR: 本文介绍了考虑实际应用问题的PID控制器参考实现，包括伪代码、实现选择讨论和重要测试案例的仿真。


<details>
  <summary>Details</summary>
Motivation: 虽然PID控制律本身简单且计算成本低，但在实际应用中由于执行器物理限制、测量噪声、不同结构实现、无扰切换和变采样时间等问题，基本形式无法正常工作，因此需要更实用的实现方案。

Method: 提出了一种考虑实际问题的PID控制器参考实现方法，包括提供伪代码、讨论实现选择，并通过精心选择的重要测试案例进行仿真验证。

Result: 开发了一个能够处理实际应用中常见问题的PID控制器实现方案，包括执行器限制、噪声抑制、不同控制结构实现等实际问题。

Conclusion: PID控制器的实际实现比表面看起来更困难，需要综合考虑各种实际问题，本文提供的参考实现为解决这些问题提供了实用方案。
...
### [201] [Uncertainty-based perturb and observe for data-driven optimization](https://arxiv.org/abs/2604.15922)
*Leontine Aarnoudse,Mark Haring,Nathan van de Wouw,Alexey Pavlov*

Main category: eess.SY

TL;DR: 提出了一种基于不确定性的扰动观测方法，减少扰动次数同时保持跟踪时变最优解的能力，在光伏阵列案例中表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于数据的自适应优化方法通常依赖持续扰动，这在工业等实际应用中不受欢迎，需要减少扰动频率同时保持优化性能。

Method: 开发了'仅在需要时扰动'的不确定性扰动观测方法，通过判断是否需要扰动来减少操作次数，并在温和条件下证明其收敛性。

Result: 方法在光伏太阳能阵列的仿真案例研究中，表现优于标准扰动观测方法及其他三种基于数据的优化方法。

Conclusion: 该方法有效解决了持续扰动问题，减少了实际应用中的扰动需求，同时保持了跟踪时变最优解的能力，具有实际应用价值。
...
### [202] [Robust predictive control design for uncertain discrete switched affine systems subject to an input delay](https://arxiv.org/abs/2604.15982)
*Gerson Portilla,Carolina Albea,Alexandre Seuret*

Main category: eess.SY

TL;DR: 本文针对具有单位输入延迟的不确定切换仿射系统，提出了鲁棒稳定条件，通过李雅普诺夫框架和最小切换状态反馈预测控制律实现，并证明了系统轨迹和预测的指数收敛性。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于需要处理具有输入延迟和参数不确定性的切换仿射系统的稳定控制问题，这类系统在实际工程中常见但控制设计复杂。

Method: 采用李雅普诺夫框架和最小切换状态反馈预测控制律，结合考虑名义系统参数的预测方案，并构建了包含预测误差的李雅普诺夫函数。

Result: 证明了系统轨迹和系统预测能够指数收敛到一个鲁棒极限环，并通过示例验证了所提方法的有效性。

Conclusion: 所提出的方法能够有效稳定具有输入延迟和参数不确定性的切换仿射系统，为这类系统的控制提供了理论保证和实用方案。
...
### [203] [Stealthy Cyber-Attacks on Vehicle Lateral Dynamics: A System-Theoretic Analysis](https://arxiv.org/abs/2604.15996)
*Ali Eslami,Jiangbo Yu,Mohammad Pirani*

Main category: eess.SY

TL;DR: 本文研究了车辆自行车模型在三种隐蔽网络攻击下的脆弱性：重放攻击、零动态攻击和隐蔽攻击，分析了不同传感器配置对攻击可检测性和系统安全性的影响。


<details>
  <summary>Details</summary>
Motivation: 随着车辆系统日益网络化，隐蔽网络攻击对车辆横向动力学的威胁日益凸显，需要系统性地分析不同攻击类型在车辆控制中的可行性和影响。

Method: 采用系统理论框架分析攻击特性，考虑横摆角速度、横向加速度和纵向加速度等不同测量配置，并通过CarSim-Simulink联合仿真验证理论结果。

Result: 重放攻击基本与模型无关；零动态攻击受输出选择等控制设计约束；隐蔽攻击在具备足够系统知识和访问权限时可实现持续隐蔽的横向状态偏离。执行器和轮胎饱和对攻击隐蔽性和有效性有不同影响。

Conclusion: 传感器配置选择可显著影响系统对特定攻击的脆弱性，控制导向的设计（如输出选择）能消除不稳定零动态，从而限制零动态攻击的影响，但隐蔽攻击在充分条件下仍构成严重威胁。
...
### [204] [Safe Deep Reinforcement Learning for Building Heating Control and Demand-side Flexibility](https://arxiv.org/abs/2604.16033)
*Colin Jüni,Mina Montazeri,Yi Guo,Federica Bellizio,Giovanni Sansavini,Philipp Heer*

Main category: eess.SY

TL;DR: 提出一种基于深度强化学习的安全控制框架，用于优化建筑供暖系统，在保证舒适度的同时提供需求侧灵活性，实现能源成本节约。


<details>
  <summary>Details</summary>
Motivation: 建筑能耗占全球40%，随着可再生能源比例增加，供暖系统的需求侧灵活性对电网稳定和能源效率至关重要，但现有方法在安全性和灵活性协调方面存在不足。

Method: 采用深度确定性策略梯度算法作为核心强化学习方法，结合实时自适应安全过滤器，确保系统在提供灵活性时始终满足运营商约束条件。

Result: 相比基于规则的控制器，节能达50%；在能源和成本指标上优于纯强化学习控制器，仅轻微增加舒适温度违规次数。

Conclusion: 所提出的安全强化学习框架能有效协调建筑供暖的灵活性服务、能耗优化与舒适度要求，为电网交互式建筑控制提供了安全可靠的解决方案。
...
### [205] [A numerical approach to the co-design of PID controllers and low-pass filters for time-delay systems](https://arxiv.org/abs/2604.16124)
*Diego Torres-García,Wim Michiels*

Main category: eess.SY

TL;DR: 提出了一种同时优化PID控制器参数和导数滤波器常数的数值设计框架，通过直接最小化滤波后闭环系统的谱横坐标来协调高频鲁棒性和低频性能。


<details>
  <summary>Details</summary>
Motivation: 传统PID控制器设计通常将导数滤波器视为后处理步骤，但滤波器会显著改变闭环系统的谱特性，影响稳定性和性能，因此需要将滤波器作为控制器设计的核心部分进行联合优化。

Method: 建立包含低通滤波器的PID控制器设计框架，将PID增益和滤波器常数作为优化变量，通过直接最小化滤波后闭环系统的谱横坐标进行数值优化，适用于带输入/状态延迟的线性时不变SISO/MIMO系统。

Result: 数值示例表明，所提出的控制器-滤波器协同设计方法能够有效协调高频鲁棒性（缓解近似恒等引起的脆弱性）和低频性能，同时抑制测量噪声放大。

Conclusion: 将导数滤波器作为控制器设计的有机组成部分进行联合优化，比传统后处理方式更能保证系统的稳定性和性能，该方法对带延迟的线性系统具有普适性。
...
### [206] [Real-Time Solution-Seeking for Game-Theoretic Autonomous Driving via Time-Distributed Iterations](https://arxiv.org/abs/2604.16184)
*Shaoqing Liu,Mushuang Liu*

Main category: eess.SY

TL;DR: 提出基于牛顿和牛顿-康托罗维奇方法的时间分布式迭代算法，解决博弈论模型预测控制中的计算复杂度问题，实现自动驾驶的实时决策。


<details>
  <summary>Details</summary>
Motivation: 博弈论模型预测控制在自动驾驶多智能体交互中面临实时计算纳什均衡的挑战，需要在快速采样率下完成复杂计算。

Method: 采用势博弈框架确保解的存在性，结合势函数优化和最优响应动态寻求纳什均衡，运用牛顿和牛顿-康托罗维奇方法将迭代计算分布到时间维度。

Result: 在交叉路口场景的数值实验中，所提方法实现了有效的实时性能，解决了传统方法计算负担重的问题。

Conclusion: 时间分布式求解迭代能有效降低博弈论模型预测控制的计算复杂度，为自动驾驶多智能体实时决策提供了可行方案。
...
### [207] [Optimization of Phase Change Material Integration for Active Cooling Control](https://arxiv.org/abs/2604.16199)
*Asmaou S. Ouedraogo,Donald J. Docimo*

Main category: eess.SY

TL;DR: 本文提出了一个相变材料冷却系统的统一优化框架，通过能量项建模解决热管理设计问题，并通过两个案例验证了被动和主动冷却配置的性能改进。


<details>
  <summary>Details</summary>
Motivation: 热管理在光伏模块、电池组和电力电子等应用中至关重要，过热会降低性能和寿命。现有研究缺乏统一多种性能目标的正式设计方法，因此需要开发一个综合优化框架。

Method: 开发了一个基于能量项的PCM设计问题公式化框架，包含静态和动态目标以捕捉物理设计和控制方面。通过两个案例研究验证：被动冷却探索和主动冷却配置实施。

Result: 比较了两种系统的设计和控制，显示两种选项在个体性能指标上均有改进。

Conclusion: 该框架成功统一了PCM冷却系统的多样化性能目标，为热管理设计提供了有效的优化方法。
...
### [208] [Data-Driven Distributed Stability Certification for Power Systems via Input-State Trajectories](https://arxiv.org/abs/2604.16212)
*Xiaohui Zhang,Liaoyuan Yang,Peng Yang*

Main category: eess.SY

TL;DR: 提出数据驱动框架验证互联电力系统稳定性条件，通过输出微分无源性指标量化各节点对系统稳定性的影响，无需显式物理模型即可从测量数据中提取指标。


<details>
  <summary>Details</summary>
Motivation: 传统稳定性分析方法依赖精确物理模型，难以适应复杂多变的实际电力系统。需要一种无需显式模型、能基于实测数据验证分布式稳定性条件的方法。

Method: 1. 建立输出微分无源性（ODP）条件作为系统稳定性判据；2. 推导基于输入-状态轨迹数据的线性矩阵不等式（LMI）判据；3. 将最优ODP指标提取问题转化为凸半定规划（SDP）问题求解。

Result: 仿真验证表明，该方法在单设备离线评估和系统级在线认证两种场景下均有效，能够通过实测数据可靠验证系统稳定性条件。

Conclusion: 所提数据驱动框架能有效验证互联电力系统的分布式稳定性条件，为缺乏精确模型的复杂系统稳定性分析提供了实用工具。
...
### [209] [Consensus Clustering for the Identification of Coherent Regions with Varied Generation Mix](https://arxiv.org/abs/2604.16222)
*Kiran Kumar Challa,Alok Kumar Bharati,Venkataramana Ajjarapu*

Main category: eess.SY

TL;DR: 本文提出了一种多视图共识算法，用于在电网运行条件变化和多种扰动下识别电力系统的同调区域，并在miniWECC 240节点测试系统中进行了验证。


<details>
  <summary>Details</summary>
Motivation: 随着逆变器技术大量接入电网，大型互联系统的频率响应变得难以预测，导致同调区域边界随扰动位置和运行条件显著变化。现有同调识别方法多基于单一发电机故障，而系统需要应对更广泛的扰动范围。

Method: 采用多视图共识算法，综合考虑电网不同运行条件和多种扰动场景，以识别动态变化下的同调区域边界。

Result: 在miniWECC 240节点测试系统中成功识别出不同运行条件和扰动下的同调区域，验证了所提算法的有效性。

Conclusion: 所提出的多视图共识算法能够适应电网运行条件和扰动的多样性，为含高比例逆变器的电力系统同调识别提供了有效工具。
...
### [210] [Simulating Arbitrage Optimization for Market Monitoring in Gas and Electricity Transmission Networks](https://arxiv.org/abs/2604.16229)
*Noah Rhodes,Sachin Shivakumar,Luke S. Baker,Kaarthik Sundar,Anatoly Zlotnik*

Main category: eess.SY

TL;DR: 本文研究能源运输网络中的市场结果，重点关注燃气发电厂在电力批发市场和天然气市场中的双重角色，提出识别市场操纵行为的优化方法。


<details>
  <summary>Details</summary>
Motivation: 燃气发电厂同时在电力市场和天然气市场运营，可能通过在一个市场的投标行为影响另一个市场的价格，现有市场监控机制需要更有效的工具来检测这种跨市场操纵行为。

Method: 使用DC最优潮流（OPF）和稳态最优天然气流（OGF）模型构建电力与天然气市场交互框架，开发基于优化的方法来识别电力市场中的市场势力，以及通过天然气市场投标操纵电力市场的条件。

Result: 提出了可解释的小型测试网络场景，展示了燃气发电厂如何通过天然气市场投标改变电力市场结果；建立了结构化价格和数量投标交换的双市场交互模型。

Conclusion: 需要更完善的市场监控机制来检测跨能源市场的操纵行为，所提出的优化框架能够帮助市场管理员识别燃气发电厂通过天然气市场投标行使市场势力的条件。
...
### [211] [Portable Medical Imaging in Modern Healthcare: Fundamentals, AI-Based Taxonomy, Image Quality, and Open Challenges](https://arxiv.org/abs/2604.15378)
*Yassine Habchi,Hamza Kheddar,Muhammad Ali Qureshi,Mohamed Seghier,Azeddine Beghdadi*

Main category: eess.IV

TL;DR: 本文系统综述了便携式医学影像（PMI）领域的最新进展，重点关注AI技术在提升图像质量、重建、评估及分析中的应用，并探讨了图像质量、AI鲁棒性与临床可用性之间的关系。


<details>
  <summary>Details</summary>
Motivation: 便携式医学影像在急诊、农村和资源有限环境中具有重要价值，但其图像质量易受运动伪影、环境干扰、硬件限制等因素影响，制约了临床诊断的可靠性和普及性。

Method: 提出了一种基于AI的PMI方法分类体系，涵盖机器学习、深度学习、迁移学习和Transformer方法；系统分析了图像增强、重建、质量评估、检测与分类等任务，并梳理了设备、传感流程、模态特异性失真、评估指标和公开数据集。

Result: 综述揭示了现有PMI技术中图像质量与AI模型鲁棒性的紧密关联，指出当前研究多集中于特定模态或应用，缺乏对质量-鲁棒性-临床部署协同关系的系统性探讨。

Conclusion: 未来需致力于开发更可靠、可解释且易于临床部署的PMI系统，填补当前研究在跨模态泛化、实时处理、临床验证等方面的空白。
...
### [212] [RelativeFlow: Taming Medical Image Denoising Learning with Noisy Reference](https://arxiv.org/abs/2604.15459)
*Yuxin Liu,Yiqing Dong,Wenxue Yu,Zhan Wu,Rongjun Ge,Yang Chen,Yuting He*

Main category: eess.IV

TL;DR: 提出RelativeFlow框架，通过相对流匹配解决医学图像去噪中缺乏绝对干净监督图像的问题，将绝对噪声-干净映射分解为相对噪声-噪声映射，在CT和MR去噪任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 医学图像去噪缺乏绝对干净的监督图像，现有方法（模拟监督判别学习、模拟监督生成学习）将噪声参考视为干净目标导致次优收敛或参考偏差学习，而自监督学习对噪声假设限制严格，难以适应真实医学成像场景。

Method: 1）提出相对流匹配框架，将绝对噪声-干净映射分解为相对噪声-噪声映射；2）设计一致传输（CoT）位移图，约束相对流逐步组合成统一绝对流；3）构建模拟基速度场（SVF），利用模态特定退化算子支持不同医学成像模态。

Result: 在计算机断层扫描（CT）和磁共振（MR）去噪的大量实验中，RelativeFlow显著优于现有方法，有效处理带噪声参考的医学图像去噪问题。

Conclusion: RelativeFlow通过相对流匹配框架成功解决了医学图像去噪中缺乏干净监督的根本限制，实现了从任意质量输入到统一高质量目标的映射，为医学图像去噪提供了新解决方案。
...
### [213] [CTSCAN: Evaluation Leakage in Chest CT Segmentation and a Reproducible Patient-Disjoint Benchmark](https://arxiv.org/abs/2604.15561)
*Anton Ivchenko*

Main category: eess.IV

TL;DR: CTSCAN是一个用于胸部CT分割的可重复多源基准测试框架，旨在解决因训练和测试数据混合同一患者切片而导致的性能虚高问题，通过患者分离评估提供更真实的性能测量。


<details>
  <summary>Details</summary>
Motivation: 现有胸部CT分割研究中，训练集和测试集常混合同一患者的不同切片，导致性能评估被严重夸大，无法反映模型在真实患者分离场景下的泛化能力。

Method: 提出CTSCAN基准测试框架，整合来自三个数据集的89个病例，设计患者分离的数据划分方案，使用FPN+EfficientNet-B0架构进行多种子协议扫描，对比切片混合与患者分离两种评估协议的性能差异。

Result: 切片混合协议获得0.6665前景Dice和0.5031前景IoU，而患者分离协议仅获得0.2066 Dice和0.1181 IoU，去除患者数据重用导致前景Dice绝对下降0.4599（相对下降69.00%），前景IoU绝对下降0.3850（相对下降76.52%）。

Conclusion: 患者数据重用会严重夸大分割性能，CTSCAN通过提供确定性数据划分、弱监督控制、多种子协议扫描和可重复图表生成，为胸部CT分割的可靠患者分离评估建立了可重复的基础框架。
...
### [214] [Topology-Driven Fusion of nnU-Net and MedNeXt for Accurate Brain Tumor Segmentation on Sub-Saharan Africa Dataset](https://arxiv.org/abs/2604.15964)
...
TL;DR: 本文针对低收入和中等收入国家脑肿瘤分割的挑战，通过拓扑优化模块改进现有分割模型，在BraTS-Africa数据集上取得了更好的分割性能。


<details>
  <summary>Details</summary>
Motivation: 低收入和中等收入国家由于缺乏标准化的成像协议、多样化的影像数据、广泛使用的低场强MRI扫描仪以及有限的医疗资源，导致脑肿瘤自动分割准确性面临挑战。

Method: 结合BraTS 2025数据集预训练分割模型，并在BraTS-Africa数据集上微调；在nnU-Net、MedNeXt及其组合模型基础上引入拓扑优化模块，以解决预测中的拓扑错误导致的形变问题。

Result: 通过拓扑优化模块，在Surrounding Non-Enhancing FLAIR Hyperintensity、Non-Enhancing Tumor Core和Enhancing tumor三个区域分别获得了0.810、0.829和0.895的归一化表面距离（NSD）值。

Conclusion: 拓扑优化模块有效提升了在低质量MRI图像上的脑肿瘤分割性能，为资源有限地区的医疗影像分析提供了可行的技术方案。
...
### [215] [Dual-Modal Lung Cancer AI: Interpretable Radiology and Microscopy with Clinical Risk Integration](https://arxiv.org/abs/2604.16104)
*Baramee Sukumal,Aueaphum Aueawatthanaphisut*

Main category: eess.IV

TL;DR: 提出了一种结合CT影像和H&E病理切片的双模态AI框架，用于肺癌诊断和亚型分类，通过可解释性技术提高模型透明度，实验显示性能优异。


<details>
  <summary>Details</summary>
Motivation: 传统CT影像在区分良恶性病变和提供可解释诊断方面存在局限，需要更准确的肺癌诊断方法。

Method: 使用卷积神经网络提取影像和病理特征，结合临床元数据，采用加权决策级融合机制分类肺癌亚型，并应用多种可解释AI技术（如Grad-CAM++）提供可视化解释。

Result: 模型准确率达0.87，AUROC超过0.97，宏观F1分数0.88；Grad-CAM++在忠实度和定位准确性上表现最佳，与专家标注区域高度一致。

Conclusion: 放射学与病理学的多模态融合可提升诊断性能并保持模型透明度，有望用于临床决策支持系统，推动精准肿瘤学发展。
