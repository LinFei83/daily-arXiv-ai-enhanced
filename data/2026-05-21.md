...
TL;DR: 提出SOLAR框架，通过参数级元学习和多层级强化学习实现终身自主适应，在多个推理任务上超越基线。


<details>
  <summary>Details</summary>
Motivation: 传统微调在动态环境中难以适应流式数据，存在灾难性遗忘和概念漂移问题。

Method: 利用参数级元学习初始化常识先验，结合多层级强化学习探索自适应策略，并维护演变知识缓存以平衡可塑性和稳定性。

Result: 在常识、数学、医学、编程、社交和逻辑推理任务中优于强基线方法。

Conclusion: SOLAR向能够在动态环境中终身自主适应的智能体迈出了重要一步。
...
TL;DR: 提出COSMO-Agent框架，通过强化学习训练LLM自动完成CAD-CAE闭环优化，显著提升小模型在约束驱动设计中的可行性、效率和稳定性。


<details>
  <summary>Details</summary>
Motivation: 工业设计中CAD-CAE迭代优化受限于语义鸿沟，即仿真反馈难以自动转化为有效的几何修改。

Method: 将CAD生成、CAE求解、结果解析和几何修正构建为交互式RL环境，设计多约束奖励函数，并构建包含25类组件的工业对齐数据集。

Result: COSMO-Agent训练的小型开源LLM在可行性、效率和稳定性上超越大型开源和强闭源模型。

Conclusion: 该框架有效地弥合了CAD-CAE语义鸿沟，提升了约束驱动设计的自动化和可靠性。
...
TL;DR: 提出了OSCToM方法，通过强化学习和领域特定语言生成训练数据，提升了LLM在复杂社会情境中的心智推理能力，在FANToM基准上准确率达76%，数据合成效率提高6倍。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在心智推理任务中表现不均，现有基准如ExploreToM未能充分测试递归信念和信息不对称带来的复杂性。

Method: 结合强化学习、扩展领域特定语言和组合替代模型，生成观察者与自身信念冲突的嵌套推理场景。

Result: OSCToM-8B在测试系统中表现最佳；在FANToM上准确率76%（ExploreToM仅0.2%），在Hi-ToM和BigToM上具有竞争力；数据合成效率提高6倍。

Conclusion: 针对性的训练数据合成方法能高效提升较小模型处理高级认知推理的能力，并达到或超越当前最优水平。
...
TL;DR: AgentCo-op通过检索合成与局部修复，自动构建多智能体工作流，在开放科学任务中无需重新设计现有工具即可实现高效协作。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖高质量训练集和明确评估指标，难以处理开放科学场景中缺乏标准化接口和多智能体协作的复杂性。

Method: 提出检索合成框架，通过类型化工件传递组合技能、工具和外部智能体，并对失败组件进行边界自引导局部修复。

Result: 在基因组学案例中成功协调专业智能体完成空间转录组学和跨模态分析；在6个基准测试中4项最优且平均成本更低。

Conclusion: 检索式合成可扩展至开放世界工作流，与全局搜索互补，为复杂科学任务提供自动化设计方案。
...
TL;DR: 本文提出并评估了多种生成逻辑语句嵌入的方法，通过三元组损失训练，并引入重复项锚点、平衡正负样本难度以及周期强化困难样本的策略，以提升下游推理任务的效率。


<details>
  <summary>Details</summary>
Motivation: 神经网络的逻辑推理中，搜索答案的效率依赖于有效的嵌入表示，而现有方法在生成适用于推理的嵌入时效果有限。

Method: 采用三元组损失训练嵌入，包括（1）生成包含重复项的锚点；（2）按易、中、难三类平衡生成正负样本；（3）训练中周期性地强调困难样本。

Result: 实验比较了不同知识库上的嵌入性能，验证了所提策略能带来更好的下游推理结果。

Conclusion: 合理设计嵌入生成方式（如重复锚点和难度平衡）可显著提升逻辑推理中神经网络的效率。
...
TL;DR: 提出了一套新的评估指标ECUAS，用于综合评估不确定性增强决策系统的预测和不确定性，并通过理论和实验验证其优势。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法无法全面衡量不确定性增强系统在高风险决策中的表现，需要统一度量预测和不确定性的综合成本。

Method: 基于适当评分规则构建ECUAS指标族，通过参数n控制预测错误与不确定性不完美的权衡。

Result: 在分类和生成数据集（包括TriviaQA）上实验证明ECUAS指标优于传统评估方法。

Conclusion: ECUAS指标能更有效评估不确定性增强系统的整体决策性能，适用于不同成本权衡场景。
...
TL;DR: 本文倡导了一种补充性的AI评估方法——开放世界评估，通过长期、复杂现实任务的小样本定性分析，以弥补传统基准测试的不足。


<details>
  <summary>Details</summary>
Motivation: 传统基准测试在评估AI前沿进展时存在局限性，可能高估或低估实际部署能力，因其偏好精确指定、自动评分、易优化、低成本且短周期的任务。

Method: 论文通过调查现有开放世界评估、识别其优缺点，并提出CRUX项目作为定期执行此类评估的框架。作为实例，让AI代理完成开发并发布简单iOS应用到App Store的任务。

Result: AI代理仅需一次可避免的人工干预即完成任务，表明开放世界评估能对即将广泛普及的能力提供早期预警。

Conclusion: 开放世界评估能补充传统基准测试，并建议了设计和报告此类评估的方法。
...
TL;DR: 提出了AgentAtlas评估框架，包含六态控制决策分类、九类轨迹失败分类、分类感知与无关方法论及基准覆盖审计，揭示了当前大语言模型智能体在无显式标签时性能显著下降至相似水平，且无模型在所有维度占优。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试碎片化，单个准确率指标不足以评估可部署智能体的复杂行为。

Method: 构建了AgentAtlas框架，包含四部分：控制决策六态分类、轨迹失败九类分类、对比分类感知与无关提示的方法、基准覆盖审计。通过八模型在两种提示模式下进行1342个生成项的实验。

Result: 移除显式标签菜单后，所有模型轨迹准确率下降14-40个百分点，收敛至0.54-0.62区间；无模型在控制准确性、轨迹诊断和工具上下文效用保留方面同时最优。

Conclusion: 当前智能体评估需超越单一准确率，AgentAtlas提供了更细粒度的诊断框架；提示中的显式标签对性能影响巨大，且模型间能力差异较小。
...
TL;DR: 本文提出人格工程方法，利用AI代理精确参数化、操纵和评估谈判者人格，以人际环状模型为基础，测试经典谈判理论并指导AI谈判代理设计。


<details>
  <summary>Details</summary>
Motivation: 传统谈判理论强调平衡共情与自我主张等矛盾，但人类难以管理这些张力，且研究者缺乏在受控条件下严格检验理论的能力。

Method: 引入人格工程方法，通过AI代理的精度、重复性、一致性和可扩展性，以人际环状模型的温暖和支配两个核心维度为坐标系，参数化、操纵并评估谈判者人格。

Result: 该方法为测试经典谈判理论提供了严格实验框架，并生成了设计AI谈判代理人格的实用指南。

Conclusion: 人格工程能够克服人类谈判者的局限性，为谈判理论验证和AI代理设计提供新的有效途径。
...
TL;DR: 提出一种面向4/5级自治网络的分层多智能体架构，通过双驱动编排器、专用执行智能体和公共记忆机制实现战略与恢复的统一，在5G核心网中验证可将平均修复时间降低86%。


<details>
  <summary>Details</summary>
Motivation: 现有网络运维依赖刚性脚本，缺乏处理非标称条件的认知能力，无法实现高级别自治。

Method: 设计分层多智能体参考架构，包含双驱动编排器协调专用执行智能体，公共记忆体管理统一领域知识，并引入智能体自我意识以平衡战略规划与故障恢复。

Result: 在5G核心网环境中，系统能在拥塞时维持关键吞吐量，并将平均修复时间降低86%。

Conclusion: 该架构有效统一了战略规划与运维弹性，是迈向L4/L5自治网络的关键一步。
...
TL;DR: 提出COAgents，一个基于合作多智能体的车辆路径问题求解框架，将搜索过程建模为图，通过节点、移动和跳跃智能体引导搜索，在CVRP和VRPTW上取得领先效果。


<details>
  <summary>Details</summary>
Motivation: 传统启发式方法依赖手工规则，难以泛化；端到端学习方法可解释性差，且难以适应不同任务。

Method: 构建部分搜索图（PSG），训练节点选择、移动选择和跳跃三个智能体，分别负责局部改进、精细搜索和跳出局部最优。

Result: 在CVRP上与传统方法竞争，在VRPTW上超越现有学习基线，相比POMO和ALNS显著缩小与最优解的差距（N=100降低14%和21%，N=50降低44%和40%）。

Conclusion: COAgents通过分离搜索控制与领域编码，实现了跨任务的可适应性，并在复杂VRPTW实例上达到新最优。
...
TL;DR: 本文针对工业资产运维工作流中的延迟敏感问题，提出了一种结合时间语义缓存和MCP工作流优化的双层加速方案，在AssetOpsBench基准上实现了显著的速度提升，同时揭示了纯语义缓存在参数丰富的工业查询中的失效模式。


<details>
  <summary>Details</summary>
Motivation: 工业资产运维工作流中的单个用户查询需要协调传感器数据、工单、故障模式、预测工具和领域特定代理，导致延迟敏感性问题；现有LLM缓存技术（如KV缓存复用和基于嵌入的语义缓存）针对聊天服务设计，在输出有效性依赖于时间、资产或传感器参数时失效。

Method: 提出了两种互补优化层：时间语义缓存，以及结合磁盘支持的工发现缓存和依赖感知的并行步骤执行的MCP工作流优化集合。

Result: MCP工作流优化实现1.67倍加速，中位端到端延迟降低约40.0%；时间语义缓存在缓存命中时实现中位30.6倍加速。

Conclusion: 所提优化方法显著提升了工业代理工作流的速度，同时暴露了纯语义缓存在参数丰富工业查询中的具体失败模式，为基于MCP的代理基准中的缓存选择与评估正确性交互提供了关键分析。
...
### [13] [VBFDD-Agent for Electric Vehicle Battery Fault Detection and Diagnosis: Descriptive Text Modeling of Battery Digital Signals](https://arxiv.org/abs/2605.20742)
*Joey Chan,Zhen Chen,Ershun Pan*

Main category: cs.AI

TL;DR: 提出VBFDD-Agent框架，将电池信号转换为自然语言描述，结合大语言模型实现可解释的故障诊断与维护建议。


<details>
  <summary>Details</summary>
Motivation: 电池系统复杂性和场景多样性导致传统故障诊断方法缺乏跨域适应性和人机协作能力。

Method: 将监测信号等转换为结构化文本，构建诊断语料库；集成文本描述、历史案例、维修手册和LLM推理生成诊断结果。

Result: 实验证明框架能准确监测异常，并提供灵活有效的维护建议；专家评估确认其实用价值。

Conclusion: VBFDD-Agent将电池诊断从标签预测扩展到可解释、面向维护的决策支持。
...
### [14] [Interaction Locality in Hierarchical Recursive Reasoning](https://arxiv.org/abs/2605.20784)
*Yosuke Miyanishi,Tetsuro Morimura*

Main category: cs.AI

TL;DR: 提出交互局部性框架，测量HRM/TRM中递归推理与MTU3D中模块边界的空间因果局部性模式。


<details>
  <summary>Details</summary>
Motivation: 空间推理需结合局部计算与全局结构，但缺乏量化信息流动局部性的统一框架。

Method: 基于稀疏自编码器消融与激活补丁分析，辅以结构雅可比矩阵和注意力检查。

Result: 递归模型在邻近单元内写信息，MTU3D在视觉-模块交互处显示因果局部性。

Conclusion: 交互局部性将直观的局部执行/全局规划故事转化为可复现的测量框架。
...
### [15] [Conditional Equivalence of DPO and RLHF: Implicit Assumption, Failure Modes, and Provable Alignment](https://arxiv.org/abs/2605.20834)
*Zhiqin Yang,Yonggang Zhang,Wei Xue,Dong Fang,Bo Han,Yike Guo*

Main category: cs.AI

TL;DR: 本文证明直接偏好优化（DPO）与RLHF的理论等价性是有条件的，当假设违背时DPO优化相对优势而非绝对对齐，导致策略偏好非偏好回应。提出约束偏好优化（CPO）以解决此问题，实验表明CPO达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: DPO被视为RLHF的简化替代方案，但其理论等价性依赖于可能被违反的隐含假设，需要研究这些假设失效时的行为并提出修正。

Method: 理论分析DPO与RLHF的等价条件，证明路径收敛问题；提出CPO方法，在RLHF中添加约束以实现可证明的对齐；通过软间隔排名给出几何解释。

Result: DPO在假设违反时优化目标与RLHF不同，导致偏好非偏好回应；CPO在标准基准上达到最先进性能。

Conclusion: DPO的保证是有条件的，CPO能在保持简单性的同时提供可证明的对齐，优于DPO。
...
### [16] [Governance by Construction for Generalist Agents](https://arxiv.org/abs/2605.20874)
*Segev Shlomov,Iftach Shoham,Alon Oved,Ido Levy,Sami Marreed,Harold Ship,Offer Akrabi,Sergey Zeltyn,Avi Yaeli,Nir Mashkif*

Main category: cs.AI

TL;DR: 提出CUGA策略系统，通过模块化策略即代码层，在LLM代理执行流程中嵌入五个检查点，实现可治理、可审计的企业代理部署。


<details>
  <summary>Details</summary>
Motivation: 企业代理需要自主操作但受治理约束，传统方法需要为每个领域重建代理，缺乏模块化和可审计性。

Method: 设计运行时治理架构，包含五个策略干预点：意图守卫、行动手册、工具指南、人工审批和输出格式化，无需模型微调。

Result: 通过医疗场景演示验证了动态行动手册注入、意图拦截和人工审批的有效性，提高了策略遵循和执行一致性。

Conclusion: 类型化治理原语能加速企业代理系统的安全部署，同时提升策略遵循和执行一致性。
...
### [17] [For How Long Should We Be Punching? Learning Action Duration in Fighting Games](https://arxiv.org/abs/2605.20911)
*Hoang Hai Nguyen,Kurt Driessens,Dennis J. N. J. Soemers*

Main category: cs.AI

TL;DR: 本论文提出了一种在格斗游戏中让强化学习代理同时学习动作和持续时间的方法，通过动态调整帧跳过策略来平衡响应性与性能。


<details>
  <summary>Details</summary>
Motivation: 传统的固定帧间隔决策限制了代理调整反应时间的能力，导致要么帧完美响应不现实，要么固定间隔响应性不足。

Method: 作者让代理联合预测动作和持续时间，使用FightLadder环境，在脚本化机器人上训练，测试不同帧跳过配置的影响。

Result: 学习到的时序能匹配精心选择的固定帧跳过性能，并促进重复动作模式，但高帧跳过值（低响应性）下表现最好，代理容易学习利用脚本弱点的策略。

Conclusion: 学习时序本身不足以保证鲁棒性，提高帧跳过值有助于学习利用性策略，但可能牺牲响应性。
...
### [18] [AutoRPA: Efficient GUI Automation through LLM-Driven Code Synthesis from Interactions](https://arxiv.org/abs/2605.21082)
*Minghao Chen,Xinyi Hu,Zhou Yu,Yufei Yin*

Main category: cs.AI

TL;DR: 提出AutoRPA框架，将ReAct风格的LLM代理决策逻辑自动转化为高效、可复用的RPA函数，以减少重复GUI任务中的推理开销。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在重复GUI任务中重复调用推理（ReAct范式）效率低，而传统RPA虽高效但需大量人工维护，缺乏自动化提炼能力。

Method: 设计翻译器-构建器流水线：翻译器将硬编码ReAct动作转为软编码程序，构建器通过检索增强生成合成鲁棒RPA函数；混合修复策略在代码验证中结合RPA执行与ReAct回退迭代优化。

Result: 在多个GUI环境中，AutoRPA生成的RPA函数成功解决相似任务，并将令牌使用量降低82%至96%，显著提升运行时效率和可复用性。

Conclusion: AutoRPA有效平衡了推理准确性与运行时效率，为自动生成可复用的GUI任务RPA函数提供新方法。
...
### [19] [Playing Devil's Advocate: Off-the-Shelf Persona Vectors Rival Targeted Steering for Sycophancy](https://arxiv.org/abs/2605.21006)
*Ishaan Kelkar,Nebras Alam,Vikram Kakaria,Madhur Panwar,Vasu Sharma,Maheep Chaudhary*

Main category: cs.AI

TL;DR: 研究发现，相比传统的对比激活添加（CAA）方法，使用现成的角色导向向量（如怀疑或审视角色）能有效减少模型对用户的盲从（sycophancy），且不影响用户正确时的准确性，效果可达CAA的68%至98%。


<details>
  <summary>Details</summary>
Motivation: 标准反盲从方法CAA需要人工标注数据，且可能牺牲模型在用户正确时的表现。因此，研究试图探索更简便、通用的替代方案。

Method: 在两种指令微调模型上，使用现成的角色导向向量（怀疑/审视型）进行激活干预，并与CAA方法对比；通过几何分析理解盲从与角色向量在激活空间中的关系。

Result: 怀疑/审视角色向量能显著减少盲从，效果约为CAA的68%-98%，且不降低准确率；而顺从型角色不会对称增加盲从；角色向量与盲从方向在几何上几乎正交。

Conclusion: 盲从更应被视为一种角色层级的属性，而非单一可干预的方向；现成角色向量可作为轻量级替代CAA的反盲从工具。
...
### [20] [ScenePilot: Controllable Boundary-Driven Critical Scenario Generation for Autonomous Driving](https://arxiv.org/abs/2605.21168)
*Qiyu Ruan,Yuxuan Wang,He Li,Zhenning Li,Cheng-zhong Xu*

Main category: cs.AI

TL;DR: 提出了ScenePilot框架，通过约束多目标强化学习，在保证物理可行性的前提下生成边界场景，显著提升自动驾驶系统的碰撞率。


<details>
  <summary>Details</summary>
Motivation: 现有场景生成方法要么产生物理不可行的极端碰撞，要么过度关注攻击性操作或控制器依赖，缺乏对物理可行但系统失效的边界场景的建模。

Method: 基于约束的多目标强化学习，结合RSS物理可行性分数和在线学习的AV风险预测器，并引入步级可行性感知屏蔽。

Result: 多个规划器测试中碰撞率提升6.2个百分点，且边界场景的对抗微调持续降低下游碰撞率。

Conclusion: ScenePilot能有效生成物理有效且挑战性高的边界场景，提升自动驾驶系统的安全评估与鲁棒性。
...
### [21] [Insights Generator: Systematic Corpus-Level Trace Diagnostics for LLM Agents](https://arxiv.org/abs/2605.21347)
...
TL;DR: 提出了一种多智能体系统Insights Generator，自动分析LLM代理执行轨迹语料库，生成带证据支撑的系统性行为洞察报告。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理故障诊断依赖人工检查少量轨迹，无法发现跨轨迹的模式，且无法扩展到生产级大规模语料。

Method: 采用侦察-调查架构的多智能体系统，通过提出并检验假设来生成证据支持的洞察报告。

Result: 人类专家使用IG报告后，脚手架性能提升30.4个百分点；编码代理利用IG洞察取得稳定增益；领域专家认为IG报告在深度和证据质量上领先。

Conclusion: IG系统在诊断覆盖范围与对比方法相当的基础上，提供了更优的深度和证据质量，证明了自动轨迹诊断的有效性。
...
### [22] [Towards Resilient and Autonomous Networks: A BlueSky Vision on AI-Native 6G](https://arxiv.org/abs/2605.21395)
*Liang Wu,Kelly Wan,Mayank Darbari,Liangjie Hong*

Main category: cs.AI

TL;DR: 本文提出了6G中原生集成AI的BlueSky愿景，从“为AI的网络”转向“为网络的AI”，利用基础模型和多智能体系统实现统一、多模态、多任务的网络管理。


<details>
  <summary>Details</summary>
Motivation: 新兴应用（如自动驾驶和沉浸式体验）要求蜂窝网络更快、更弹性和自主，而5G依赖的分散、单任务AI模型无法满足需求。

Method: 提出两个方向：1) 开发6G基础模型作为统一骨干，将任务特定知识蒸馏到紧凑模型中；2) 推进多智能体系统实现自主诊断、维护和恢复。

Result: 构建了6G作为智能、自维持通信基础设施的路线图。

Conclusion: 原生AI将推动6G演变为自主智能的网络，实现从“网络支持AI”到“AI支持网络”的范式转变。
...
### [23] [Teaching AI Through Benchmark Construction: QuestBench as a Course-Based Practice for Accountable Knowledge Work](https://arxiv.org/abs/2605.21413)
...
TL;DR: 本文提出通过基准测试构建来教授AI课程的方法，学生设计跨学科专家级问题评估AI系统，生成QuestBench数据集，发现当前AI系统平均通过率仅16.85%，最高57.58%。


<details>
  <summary>Details</summary>
Motivation: 现有AI教育主要将其作为生产力工具，缺乏让学生批判性评估AI输出、理解自身判断角色的教学实践。

Method: 课程中让学生将学科知识转化为可验证的专家问题，相互评审设计后用于评估AI系统，构建256道题的QuestBench基准。

Result: 在13个评估AI系统中，平均问题级通过率16.85%，GPT-5.5最高达57.58%，暴露出AI在查询准确性、源判断等方面的隐藏失败。

Conclusion: 基准构建教学法能帮助学生将专业知识作为评判AI的依据，培养其作为负责任知识行动者的能力。
...
### [24] [PALS: Power-Aware LLM Serving for Mixture-of-Experts Models](https://arxiv.org/abs/2605.21427)
*Can Hankendi,Rana Shahout,Minlan Yu,Ayse K. Coskun*

Main category: cs.AI

TL;DR: 提出一种功率感知的LLM服务运行时系统PALS，通过联合优化GPU功率上限与软件参数（如批大小），在满足吞吐目标的同时最大化能效。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理优化系统将GPU功率视为静态约束而非可控资源，忽略了功率调控对能效的潜在提升。

Method: 结合轻量级离线功率-性能模型与反馈驱动控制器，动态选择最佳配置；无需模型重训练或API变更，集成于vLLM框架。

Result: 在多GPU系统及密集/MoE模型上，能效提升最高26.3%，QoS违规降低4至7倍，并支持动态功率预算追踪。

Conclusion: 将功率控制直接集成到LLM运行时中，可实现能效比例与电网交互式AI系统，具有显著的实际应用潜力。
...
### [25] [Mind the Sim-to-Real Gap & Think Like a Scientist](https://arxiv.org/abs/2605.21458)
*Harsh Parikh,Gabriel Levin-Konigsberg,Dominique Perrault-Joncas,Alexander Volfovsky*

Main category: cs.AI

TL;DR: 研究何时及如何利用低成本但有偏差的模拟器与高成本但无偏的实验相结合，以优化序贯决策问题。


<details>
  <summary>Details</summary>
Motivation: 模拟器成本低但存在校准偏差和漂移问题，而真实实验无偏但成本高，需平衡两者以提高决策效率。

Method: 提出扩展仿真引理分解价值误差，分析价值差距的局部与可达性成分，并设计Fisher-SEP算法最小化后验预测方差。

Result: 模拟器最优策略与真实最优策略的价值差距无法通过纯被动学习消除；Fisher-SEP可有效引导实验探索，如供应链中提前实验在长时域下更优，HIV测试中主动探索能覆盖未监测区域。

Conclusion: 规划者应在长时域或需覆盖未达状态时，通过主动实验设计补充模拟器学习。
...
### [26] [AiraXiv: An AI-Driven Open-Access Platform for Human and AI Scientists](https://arxiv.org/abs/2605.21481)
*Junshu Pan,Panzhong Lu,Yixuan Weng,Qiyao Sun,Fang Guo,Zijie Yang,Qiji Zhou,Yue Zhang*

Main category: cs.AI

TL;DR: 提出了一个AI时代的学术出版平台AiraXiv，支持人类和AI科学家共同参与论文的提交、审阅和迭代。


<details>
  <summary>Details</summary>
Motivation: 传统学术出版系统面临投稿量激增、审稿人负担加重和会议规模扩大的挑战，需要可扩展的新范式。

Method: 基于开放预印本，采用AI增强分析和审阅，通过交互式UI支持人类科学家，并通过MCP协议支持AI科学家的互动。

Result: 通过实际部署（包括作为ICAIS 2025的投稿平台）验证了其作为快速、包容和可扩展研究基础设施的潜力。

Conclusion: AiraXiv展示了AI时代学术出版新范式的可行性，提供了更高效和包容的解决方案。
...
### [27] [DeepWeb-Bench: A Deep Research Benchmark Demanding Massive Cross-Source Evidence and Long-Horizon Derivation](https://arxiv.org/abs/2605.21482)
...
TL;DR: DeepWeb-Bench是专为深度研究任务设计的困难基准，通过检索、推导、推理和校准四类能力评估前沿模型，发现推导和校准失败是主要错误来源（>70%），且模型间存在显著领域专长差异。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究产品在高分基准上表现趋同，难以区分真实能力，需要更困难的基准来揭示模型弱点。

Method: 构建包含大规模证据收集、跨源协调和长期多步推导任务的DeepWeb-Bench，设计四类能力族（检索、推导、推理、校准），使用带来源溯源的参考答案和四种披露级别进行评分。

Result: 1) 检索仅占错误12-14%，推导和校准失败超70%；2) 强模型错误以不完整推导为主，弱模型错误以幻觉精度为主；3) 模型间跨域一致性低（ρ=0.61），个案差异达18.8个百分点。

Conclusion: 现有前沿模型在深度研究任务中面临的主要瓶颈是推导和校准能力，而非检索；模型存在真实领域专长差异，需针对性提升推理与证据整合能力。
...
### [28] [Leveraging Vision-Language Models to Detect Attention in Educational Videos](https://arxiv.org/abs/2605.20211)
*Gabriel Becquet,Sébastien Lallé,Vanda Luengo,Ali Abou-Hassan*

Main category: cs.CV

TL;DR: 本文探索使用视觉语言模型（VLM）分析教育视频中的眼动数据来检测注意力流失，但发现其性能未能超越传统统计基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于手工特征（如注视和扫视统计）的注意力检测方法难以捕捉学习者参与度的复杂时间特性，预测性能有限。

Method: 采用多模态基础模型方法，使用视觉语言模型（VLM）结合叠加注视数据直接分析视频内容，并基于Gemini 3尝试多种提示策略。

Result: 所有VLM提示策略均未能在检测注意力方面超越统计基线模型。
...
### [29] [Why Latent Actions Fail, and How to Prevent It](https://arxiv.org/abs/2605.20223)
*Jung Min Lee,Taehyun Cho,Li Zhao,Jungwoo Lee*

Main category: cs.CV

TL;DR: 本文通过扩展线性潜在动作模型，分析了外生状态对潜在动作学习的干扰，发现标准重建目标会编码外生信息，而关注内生成分的表示空间是关键。先前辅助目标如动作监督可减少外生状态的干扰。


<details>
  <summary>Details</summary>
Motivation: 现实视频中包含背景等外生状态，其变化与动作无关，阻碍了从无标签视频中学习可靠的潜在动作表示。

Method: 扩展线性潜在动作模型显式建模外生状态，进行理论分析，并通过线性和非线性模型实验验证。

Result: 标准重建目标使潜在动作编码未来观察中的外生信息；在内生成分表示空间中学习可减轻干扰；动作监督等辅助目标可确保潜在动作在不同外生状态下一致。

Conclusion: 外生状态通过引入无关变化干扰潜在动作学习，而关注内生表示和辅助目标能有效缓解该问题，为常用方法提供统一理论解释。
...
### [30] [AnimeAdapter: Fine-grained and Consistent Zero-shot Anime Character Generation](https://arxiv.org/abs/2605.20237)
*Yixuan Han*

Main category: cs.CV

TL;DR: 提出了一种轻量级外观适配器，基于单张参考图像实现可控且一致的动漫角色生成，无需大规模模型或微调。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大规模视觉-语言模型或每个主题的微调，计算成本高且灵活性不足。

Method: 利用CLIP突发的局部空间化特性，设计语义选择性局部注意力机制，并在适配器训练中引入姿态感知条件解耦外观与布局。

Result: 适配器紧凑、模块化，兼容Stable Diffusion社区工作流，部署时无需额外微调，并在多种编辑场景下取得良好效果。

Conclusion: 该方法在保持高性能的同时显著降低了计算和部署成本，并提供了高质量动漫数据集。
...
### [31] [AI-Assisted Competency Assessment from Egocentric Video in Simulation-Based Nursing Education](https://arxiv.org/abs/2605.20233)
*Hanchen David Wang,Yilin Liu,Madison J. Lee,Surya Chand Rayala,Gautam Biswas,Daniel T. Levin,Meiyi Ma*

Main category: cs.CV

TL;DR: 本研究通过三阶段框架从护理论坛视频提取动作时间线，发现识别准确率与能力呈负相关，表明能力强的学生工作流程更多样化且更难分类。


<details>
  <summary>Details</summary>
Motivation: 临床模拟中评估学生能力依赖于专家观察，耗时、难以扩展且存在评分者变异，因此需要自动化评估方法。

Method: 使用冻结视觉编码器（如DINOv2）和少样本学习提取动作时间线，通过HMM Viterbi解码进行序列识别，并分析序列特征与能力评分的关系。

Result: 在22个密集标注的会话中达到57.4% MOF，发现识别准确率（如mIoU）与能力呈负相关（rho=-0.524, p=0.012），更熟练的学生产生更多样化、难分类的工作流。

Conclusion: 识别准确率可作为一种教育学上有意义的信号，用于自动能力评估，尤其反映患者安全协议和团队沟通等行为。
...
### [32] [Generation of Heterogeneous PET Images from Uniform Organ Activity Maps Using a Pretrained Domain-Adapted Diffusion Model](https://arxiv.org/abs/2605.20267)
*Suya Li,Kaushik Dutta,Debojyoti Pal,Jingqin Luo,Kooresh I. Shoghi*

Main category: cs.CV

TL;DR: 提出一种预训练域自适应扩散模型（PAD），从均匀器官活性图生成临床相关的异质性PET图像，具有高定量精度和视觉真实性。


<details>
  <summary>Details</summary>
Motivation: 传统物理模拟方法计算成本高、解剖变异性有限且难以捕捉PET异质性摄取。

Method: 采用自然图像预训练文本到图像解码器，结合条件编码器和PET域适配器，通过两阶段训练（粗分布学习+细节精炼）从CT分割的均匀活性图合成PET。

Result: 器官SUV一致性相关系数>0.92，噪声和纹理接近真实PET；肿瘤分割性能相当；人眼辨别准确率≈50%。

Conclusion: PAD可基于临床分割或数字体模生成逼真PET，支持数据增强和下游成像研究。
...
TL;DR: NeuroQA是一个大规模3D脑部MRI视觉问答基准，包含56,953个QA对，覆盖12个数据集和5个临床领域，评估11种临床推理技能，并消除了纯文本捷径。


<details>
  <summary>Details</summary>
Motivation: 现有医学VQA仅基于2D切片或窄诊断标签，缺乏3D体积和系统性临床推理评估。

Method: 构建203个模板（131个图像锚定，72个图像辅助），通过答案分布精炼和38规则确定性流程确保质量，并采用双轮专家审核。

Result: 最佳零样本视觉语言模型在封闭格式上达到47.5%准确率，低于49.4%的文本多数模板基线，表明任务挑战性。

Conclusion: NeuroQA为3D医学VQA提供了严格基准，揭示了现有模型在临床推理上的不足。
...
### [34] [You Don't Need Attention: Gated Convolutional Modeling for Watch-Based Fall Detection](https://arxiv.org/abs/2605.20275)
*Sana Alamgeer,Ronish Kumar,Awatif Yasmin,Muhammad Irshad,Anne H. H. Ngu*

Main category: cs.CV

TL;DR: 提出Gated-CNN双流轻量架构，通过sigmoid门控机制精准定位跌倒冲击特征，在五个数据集上优于Transformer，并在智能手表上实现零漏检。


<details>
  <summary>Details</summary>
Motivation: 现有自注意力机制计算开销大且全局权重分配难以精准定位跌倒的短暂冲击特征。

Method: 双流1D-CNN提取加速度计和陀螺仪特征，经sigmoid门控抑制无关激活、全局平均池化压缩描述符，最后融合分类。

Result: 离线：五个数据集平均F1达93%、93%、90%、91%、90%；实时：智能手表测试F1 97%、准确率98%、零漏检。

Conclusion: sigmoid门控在结构对齐和计算效率上优于注意力机制，适用于商用智能手表跌倒检测。
...
### [35] [Regulating Anatomy-Aware Rewards via Trajectory-Integral Feedback for Volumetric Computed Tomography Analysis](https://arxiv.org/abs/2605.20277)
*Tianwei Lin,Zhongwei Qiu,Jie Cao,Jiang Liu,Wenjie Yan,Bo Zhang,Yu Zhong,Wenqiao Zhang,Yingda Xia,Ling Zhang*

Main category: cs.CV

TL;DR: 针对医疗视觉语言模型在3D CT分析中的评估幻觉问题，提出CABS基准和TIF-GRPO框架，通过控制理论优化临床真实性。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法使用词汇代理信号导致评估幻觉，模型优化语言流畅性而非临床正确性，产生诊断关键错误。

Method: 构建CABS分解报告为临床语义单元，提出TIF-GRPO利用积分反馈回路调节解剖感知奖励，惩罚遗漏并抑制幻觉。

Result: 在3D CT基准上显著提升异常检测和临床真实性。

Conclusion: TIF-GRPO建立了医疗VLM细粒度调控的新范式。
...
### [36] [JUDO: A Juxtaposed Domain-Oriented Multimodal Reasoner for Industrial Anomaly QA](https://arxiv.org/abs/2605.20284)
*Hyunju Kang,Woohyun Lee,Jaewon Kim,Hogun Park*

Main category: cs.CV

TL;DR: 该论文提出了JUDO框架，通过结合视觉比较和领域知识微调，提升了大模型在工业异常检测中的推理能力，并在MMAD基准上超越了Qwen2.5-VL-7B和GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 大语言模型缺乏工业领域的特定知识，导致在复杂工业场景中回答不准确。

Method: 通过视觉推理将查询图像与正常图像并置进行细粒度比较，并通过监督微调和强化学习注入领域知识。

Result: 在MMAD基准上取得了优于Qwen2.5-VL-7B和GPT-4o的性能。
...
### [37] [Can Vision Models Truly Forget? Mirage: Representation-Level Certification of Visual Unlearning](https://arxiv.org/abs/2605.20282)
*Zhenyu Yu,Yangchen Zeng,Chunlei Meng,Guangzhen Yao,Shuigeng Zhou*

Main category: cs.CV

TL;DR: 本文提出了Mirage框架，从表征层面评估纵向联邦学习中的机器遗忘效果，发现现有方法通过输出级认证但仍保留大量类别信息，揭示了遗忘差距、遗忘三难问题和类别样本不对称性。


<details>
  <summary>Details</summary>
Motivation: 现有纵向联邦学习中的机器遗忘方法仅使用输出级指标来认证遗忘效果，这种评估可能不充分，需要更深入的表征级评估。

Method: 提出了Mirage框架，包含四个互补诊断方法：线性探针恢复（LPR）、中心核对齐（CKA）、特征可分性评分和逐层恢复分析，在七个数据集和七种基线方法上进行实验。

Result: 三项关键发现：(i) 遗忘差距：通过输出级认证的方法在表征中仍保留类别结构，LPR比重训练基线高15.4分；(ii) 遗忘三难问题：没有方法能同时实现高效用、输出级遗忘和表征级遗忘；(iii) 类别样本不对称：类别级遗忘留下强表征痕迹（LPR高达97%），而样本级遗忘与随机无异（LPR约50%）。

Conclusion: 联邦遗忘研究需要引入表征层感知的评估标准，因为仅依赖输出级指标可能掩盖实质性的信息保留。
...
### [38] [MedCRP-CL: Continual Medical Image Segmentation via Bayesian Nonparametric Semantic Modality Discovery](https://arxiv.org/abs/2605.20297)
*Ziyuan Gao*

Main category: cs.CV

TL;DR: 提出MedCRP-CL框架，通过中餐馆过程在线发现任务结构，并采用语义模态特定LoRA适配器实现持续学习，在16个医疗分割任务中达到73.3% Dice分数和4.1%遗忘率。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法要么对所有任务施加统一约束导致灾难性遗忘，要么需要预定义任务分组而无法适应未来任务多样性。

Method: 利用中餐馆过程从临床文本提示动态推断任务分组，并维护语义模态特定LoRA适配器，通过模态内EWC正则化实现参数隔离与知识迁移。

Result: 在16个分割任务上，MedCRP-CL以6倍更少参数取得73.3% Dice分数和4.1%遗忘率，比最佳基线高8.0%。

Conclusion: MedCRP-CL有效解决了医疗图像持续分割中的结构发现和遗忘问题，无需重放数据且参数高效。
...
### [39] [Co-Fusion4D: Spatio-temporal Collaborative Fusion for Robust 3D Object Detection](https://arxiv.org/abs/2605.20301)
*Wenxuan Li,Qin Zou,Shoubing Chen,Chi Chen,Yingyi Yang,Shoubing Chen,Qingxiang Meng*

Main category: cs.CV

TL;DR: 提出Co-Fusion4D框架，通过当前帧主导与历史帧选择性融合、双注意力机制，解决BEV检测中的时空不一致问题，在nuScenes上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中BEV检测器因目标运动与自运动导致跨帧时空不一致和特征漂移。

Method: 采用当前帧为中心策略，结合时空过滤对齐与双注意力融合模块（空间注意+时间注意）。

Result: nuScenes上实现74.9% mAP和75.6% NDS，无需测试时增强或外部数据。

Conclusion: 所提方法有效提升BEV表征的时空一致性和判别能力，优于现有方法。
...
### [40] [SDM: A Powerful Tool for Evaluating Model Robustness](https://arxiv.org/abs/2605.20308)
*Xinlei Liu,Tao Hu,Jichao Xie,Peng Yi,Hailong Ma,Baolin Li*

Main category: cs.CV

TL;DR: 提出一种名为SDM的梯度攻击方法，通过重构目标为最大化非真实标签概率上界与真实标签概率之差，采用循环-阶段-步长的三层优化框架，在多个数据集上取得更强的攻击性能和更高的性价比。


<details>
  <summary>Details</summary>
Motivation: 现有梯度攻击方法（如APGD）难以取得显著突破，分析发现“高损失非对抗样本”问题源于不合适的对抗样本生成目标。

Method: 重构目标为最大化非真实标签概率上界与真实标签概率之差，提出SDM方法，包含循环-阶段-步长的三层优化，初始阶段使用负概率损失，后续阶段使用方向概率差异比损失，通过阶段式顺序优化逼近理想目标。

Result: 与当前最优方法相比，SDM在攻击性能和性价比上均更优。

Conclusion: SDM是一种更强、更高效的梯度攻击方法，代码已公开。
...
### [41] [WildRoadBench: A Wild Aerial Road-Damage Grounding Benchmark for Vision-Language Models and Autonomous Agents](https://arxiv.org/abs/2605.20306)
*Bingnan Liu,Chenhang Cui,Rui Huang,Jiani Luo,Zhirong Shen,Tinghao Wang,Xiande Huang,Lingbei Meng,Fei Shen,An Zhang*

Main category: cs.CV

TL;DR: 提出了WildRoadBench基准，用于评估无人机航拍道路损伤的视觉定位，包含VLM和Agent两种协议，发现当前模型在该场景下性能远未达到可靠水平。


<details>
  <summary>Details</summary>
Motivation: 现有道路损伤检测缺乏在无人机航拍这一野场景下的综合评估基准，尤其是结合VLM直接定位和LLM驱动智能体的自主研究工程能力。

Method: 构建了专业标注的无人机数据集，在相同图像集和AP_50指标下设计了VLM Track（统一提示、解码和解析）和Agent Track（自主搜索、适配组件、编写代码并提交预测）两种评估协议。

Result: 闭源前沿模型在VLM中领先但AP_50仍低于50%；开源模型表现更差且新版本未改善；小目标检测对开源模型失败；Agent性能低于最强VLM，部分无法在预算内完成提交。

Conclusion: 现有VLM和智能体在野场景道路损伤定位上可靠性不足，需进一步研究；代码和数据已开源。
...
### [42] [FullFlow: Upgrading Text-to-Image Flow Matching Models for Bidirectional Vision--Language Generation](https://arxiv.org/abs/2605.20316)
*Eric Tillmann Bill,Enis Simsar,Alessio Tonioni,Thomas Hofmann*

Main category: cs.CV

TL;DR: 本文提出FullFlow，一种参数高效的方法，通过仅训练LoRA适配器和轻量级文本头，将预训练的文本到图像扩散模型升级为双向视觉-语言生成器，无需完整的多模态预训练。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型仅支持单向生成，而双向视觉-语言模型需要大规模联合预训练或重训练，丢弃了预训练模型已有的强大图像先验。

Method: 基于预训练的整流流文本到图像模型（如SD3），固定大部分参数，仅插入LoRA适配器和轻量级文本头，将图像保持在连续流空间，同时为文本添加离散插入过程，通过独立的图像和文本时间步实现二维生成空间中的轨迹选择。

Result: 在SD3上，与先前最先进的LoRA等效方法相比，FullFlow将文本→图像FID从62.7降至31.6，图像→文本CIDEr从2.0提升至99.4，峰值VRAM从约84GB降至约38GB，吞吐量提升约8倍，仅训练约5%的参数，并在24小时内完成训练。该方法也可迁移至FLUX.1-dev模型。

Conclusion: 表明无需完整的多模态预训练，仅通过微调预训练的文本到图像流模型即可解锁强大的双向视觉-语言能力。
...
### [43] [ParaVT: Taming the Tool Prior Paradox for Parallel Tool Use in Agentic Video Reinforcement Learning](https://arxiv.org/abs/2605.20342)
*Zuhao Yang,Kaichen Zhang,Sudong Wang,Keming Wu,Zhongyu Yang,Bo Li,Xiaojuan Qi,Shijian Lu,Xingxuan Li,Lidong Bing*

Main category: cs.CV

TL;DR: 提出ParaVT，首个多智能体端到端强化学习框架，通过并行视频工具调用提升长视频理解，并针对工具先验悖论提出PARA-GRPO算法。


<details>
  <summary>Details</summary>
Motivation: 现有原生强化学习方法顺序调用工具导致错误传播、上下文污染和推理成本线性增长。

Method: 多智能体并行工具调用框架ParaVT，以及PARA-GRPO算法（包含结构化标记奖励和帧预算随机化）。

Result: 在六个长视频基准上平均提升7.9%，训练格式合规率从0.13提升至0.64。

Conclusion: RL需与LMM内化的工具先验协同，ParaVT提供通用解决方案。
...
TL;DR: 本文提出了测量视觉模型特征可解释性的框架，通过两个心理物理学协议（局部定位性、可命名性）评估模型。对六种视觉Transformer的分析表明，基础模型的可解释性低于监督模型，且该差距与任务性能无关，而是与特征激活的局部性和粗粒度语义对齐相关。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法可靠评估视觉模型特征的可解释性，而模型正被部署到高风险场景中，因此需要标准化框架来比较不同模型的可解释性。

Method: 采用稀疏自编码器提取特征，通过两个心理物理学协议（局部定位性和可命名性）收集人类行为响应，并用机会锚定评分函数将模型置于统一尺度。

Result: 收集了超过15,000份响应，分析377名有效参与者的13,400份数据。发现基础模型（如DINOv2、CLIP）的可解释性低于监督ViT，且可解释性与下游任务性能无相关性，但与激活局部性和粗粒度语义对齐正相关。

Conclusion: 可解释性是独立于任务性能的表示质量维度，基础模型在此方面落后于监督模型，提升可解释性需关注特征激活的局部性和粗粒度语义对齐，而非单纯增强能力。
...
### [45] [HAPS: Rethinking Image Similarity for Virtual Staining](https://arxiv.org/abs/2605.20362)
...
TL;DR: 提出了针对组织病理学图像的感知相似度指标HAPS，通过减少训练数据噪声提升了虚拟染色模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有通用图像质量指标（如SSIM、PSNR）无法准确评估组织病理学图像中形态结构与生物标志物的保留情况。

Method: 系统评估多种全参考指标，引入几何畸变分析，基于预训练组织病理学编码器提出HAPS指标，并用于训练数据质量过滤。

Result: 通过过滤MIST数据集中低质量训练对，虚拟染色模型性能优于未过滤数据的训练结果。

Conclusion: HAPS指标能有效对齐专家评估，且通过数据筛选可提升虚拟染色模型的训练效果。
...
### [46] [ConceptSeg-R1: Segment Any Concept via Meta-Reinforcement Learning](https://arxiv.org/abs/2605.20385)
...
TL;DR: 提出概念分割三层分类法（CI/CD/CR），并通过元强化学习框架ConceptSeg-R1实现统一的概念级分割。


<details>
  <summary>Details</summary>
Motivation: 现有提示分割方法虽转向概念理解，但概念定义不明确，缺乏对认知复杂性的泛化能力评估。

Method: 基于Meta-GRPO的元强化学习机制学习可迁移任务规则，通过代理推理验证状态，轻量级概念翻译模块生成分割提示，并采用短路路由策略保持效率。

Result: 在跨领域（自然/工业/医疗/推理）的CI/CD/CR基准上，ConceptSeg-R1在完整概念层级中保持高性能，且不损害原分割骨干能力。

Conclusion: 首次为任意概念分割建立了实用基线，推动分割从目标级向概念级理解进步。
...
### [47] [How You Move Tells What You'll Do: Trajectory-Conditioned Egocentric Prediction](https://arxiv.org/abs/2605.20388)
*Sejoon Jun,Hai Nguyen-Truong,Luigi Seminara,Lorenzo Torresani*

Main category: cs.CV

TL;DR: 本文提出TrajPilot模型,通过预测未来相机轨迹来指导第一人称动作预测,在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统动作预测方法因未来不确定性而表现不佳,而相机轨迹蕴含操作者意图,可提供有效约束。

Method: 利用上下文预测候选轨迹,在动作对齐嵌入空间进行动作预测,语言仅用于结构化嵌入而不作为输入。

Result: 在Ego-Exo4D等数据集上超越VLM和结构化规划基线,轨迹优势随预测时长增加而扩大,且支持无目标自由预测。
...
### [48] [STELLAR: Scaling 3D Perception Large Models for Autonomous Driving](https://arxiv.org/abs/2605.20390)
...
TL;DR: 研究了大规模训练在自动驾驶感知系统中的应用，提出了STELLAR模型，在Waymo Open Dataset上取得新最优结果。


<details>
  <summary>Details</summary>
Motivation: 模型缩放在其他领域成功，但在自动驾驶感知中因传感器融合和3D空间理解等挑战尚未验证。

Method: 基于稀疏窗口变换器，扩展输入模态包括LiDAR、雷达、摄像头和地图先验，使用5000万驾驶样本训练达5亿参数模型。

Result: 发现模型性能与规模、数据和计算之间的经验缩放趋势，在Waymo数据集上大幅超越先前方法。

Conclusion: 大规模训练是提升自动驾驶感知模型能力的有效途径。
...
### [49] [Lighting-aware Unified Model for Instance Segmentation](https://arxiv.org/abs/2605.20436)
*Qisai Liu,Alloy Das,Zhanhong Jiang,Joshua R. Waite,Aditya Balu,Adarsh Krishnamurthy,Soumik Sarkar*

Main category: cs.CV

TL;DR: 提出一种轻型卷积-注意力适配器模块，在不调整大模型参数的前提下显著提高分割模型对不同光照条件的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型（如SAM）在零样本泛化中表现优秀，但面对复杂真实光照时性能严重下降，尤其是在实例分割任务中。

Method: 采用双分支架构处理RGB特征与对比度图，通过成对训练策略显式惩罚干净图像与光照变体之间的差异。

Result: 在多个现有基准和新构建的Unity合成数据集上，该方法有效跨越域差距，实现了光照鲁棒的分割。

Conclusion: 该方法在不微调重载骨干网络的情况下显著提升了分割的鲁棒性，验证了物理驱动的光照适应能力。
...
### [50] [A Comprehensive Comparison of Deep Learning Architectures for COVID-19 Classification on CT & X-ray Imagery](https://arxiv.org/abs/2605.20445)
*Sarmad Khan,Arslan Shaukat,Umer Asgher,Basim Azam*

Main category: cs.CV

TL;DR: 本文提出基于CNN的CAD系统，使用多种预训练网络（如ResNet、VGG）对COVID-19与健康肺部X光和CT图像进行分类，准确率达95-98%。


<details>
  <summary>Details</summary>
Motivation: COVID-19大流行造成大量死亡，影像技术是筛查关键，但人工分析效率低，需自动化AI系统辅助诊断。

Method: 使用VGG、DenseNet、ResNet、MobileNet、Xception、EfficientNet、NasNet等多种预训练CNN模型，分别对两组X-ray和两组CT图像数据集进行分类。

Result: ResNet和VGG架构在X-ray和CT数据集上平均准确率分别为95-98%，性能优于此前文献报道。

Conclusion: 该CAD系统能有效区分COVID-19与正常肺部图像，具有临床应用潜力。
...
### [51] [ELEMENT: Multi-Modal Retinal Vessel Segmentation Based on a Coupled Region Growing and Machine Learning Approach](https://arxiv.org/abs/2605.20458)
*Erick O. Rodrigues,Aura Conci,Panos Liatsis*

Main category: cs.CV

TL;DR: 提出了一种名为ELEMENT的多模态视网膜血管分割框架，结合特征提取与基于连通性的像素分类，在多个数据集上取得了优于26种主流方法（包括6种深度学习方法）的结果。


<details>
  <summary>Details</summary>
Motivation: 人工或交互式视网膜血管分割耗时且易出错，亟需自动化方法。

Method: 融合灰度与血管连通性特征的机器学习框架，采用区域生长与像素分类技术。

Result: 在DRIVE等6个数据集上准确率达97.40%-98.35%，超越所有对比方法。

Conclusion: ELEMENT能有效提升分割一致性与速度，为多模态眼底诊断提供稳健方案。
...
### [52] [Do Vision--Language Models Understand 3D Scenes or Just Catalogue Objects?](https://arxiv.org/abs/2605.20448)
*Animesh Maheshwari,Divyansh Sahu,Nishit Verma*

Main category: cs.CV

TL;DR: 当前视觉语言模型在名称识别上表现出色，但在深度顺序遮挡、光学几何推理和体积重排规划等三维空间理解任务上表现极差，反映出严重的空间理解缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有模型依赖语言线索识别物体，但缺乏对场景真实三维布局的表征能力，需要系统评估其空间理解局限性。

Method: 构建含3034个样本的人工标注基准，从遮挡、反射、体积重排三方面评估；对6个前沿模型收集18204个反应，并由人工评分（避免LLM评估偏差）；对Qwen3-VL-8B进行白盒分析。

Result: 模型在可见布局重排任务准确率达53%-97%且不违反碰撞约束，但遮挡任务降至6%-45%，反射任务低于7%；白盒分析表明空间信息在视觉token合并后丢失。

Conclusion: 当前视觉语言模型缺乏可靠的三维空间理解能力，该缺陷源于视觉编码器与语言解码器之间的token合并过程，提示需改进空间表征机制。
...
### [53] [Pixel Wised Lesion Prediction on COVID-19 CT Imagery: A Comparative Analysis of Automated Image Segmentation Architectures](https://arxiv.org/abs/2605.20459)
*Sarmad Khan,Arslan Shaukat,Umer Asgher,Basim Azam*

Main category: cs.CV

TL;DR: 本研究系统评估了四种深度学习架构（Unet、PSPNet、Linknet、FPN）结合六种预训练编码器在COVID-19 CT图像分割中的性能，二元分割最高F1分数达98%，多类分割达75%-77%。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像分割领域缺乏标准化评估方法，且数据集不统一，限制了可靠性。本研究旨在建立一个基准评估框架，以准确预测CT图像中的COVID-19病灶。

Method: 整合Unet、PSPNet、Linknet、FPN四种架构与VGG19、DenseNet121、Inception ResNet V2、MobileNet V2、SeresNet 101、EfficientNet B0六种预训练编码器，进行二元和多类分割实验。

Result: 在三个COVID-19 CT数据集上，二元分割达到98% F1分数，多类分割在两个数据集中分别获得75%和77%的F1分数。

Conclusion: 深度学习架构能准确高效地分割COVID-19 CT图像；AI和深度学习可多维度提升大流行病的诊断过程。
...
### [54] [Understanding Model Behavior in Monocular Polyp Sizing](https://arxiv.org/abs/2605.20461)
*Xinqi Xiong,Andrea Dunn Beltran,Junmyeong Choi,Sarah K. McGill,Marc Niethammer,Roni Sengupta*

Main category: cs.CV

TL;DR: 本文审计了基于单目结肠镜的息肉尺寸二分类（≤5 mm vs >5 mm）方法，发现模型性能中等一致，依赖检查行为线索而非真实尺度，且尺度信息与掩膜鲁棒性是两个独立瓶颈。


<details>
  <summary>Details</summary>
Motivation: 准确的息肉尺寸分层指导随访决策，但单目结肠镜缺乏可靠的度量参考，现有方法未充分评估尺度信息与掩膜鲁棒性的影响。

Method: 跨多个公共多中心数据集、模型架构和输入模态（RGB、深度、光度学），使用患者分层交叉验证进行诊断审计，并通过真值尺度、深度估计和全局校准量化潜在改进，同时采用预测量掩膜替代和分布偏移分析。

Result: 当前深度估计和全局校准带来的改进有限；分布偏移下的分割错误消除了大多数潜在提升，而具备预测量掩膜的神谕尺度仅恢复基线性能。

Conclusion: 度量尺度和掩膜鲁棒性是息肉尺寸分类的两个独立瓶颈，提供了预测尺度梯子、捷径分区和掩膜替换等可复用评估工具。
...
### [55] [HalluCXR: Benchmarking and Mitigating Hallucinations in Medical Vision-Language Models for Chest Radiograph Interpretation](https://arxiv.org/abs/2605.20469)
*Haoyu Wang,Zitong Li*

Main category: cs.CV

TL;DR: 提出了HalluCXR基准测试，发现61.9%-82.3%的VLM输出存在幻觉，其中高达80.2%属于临床危险错误。正常胸片反而最易产生严重幻觉，常见发现被过度虚构而罕见发现被漏检，仅响应长度即可预测幻觉风险（AUC最高0.908）。六模型集成可将虚构降低84.8%，但以增加遗漏为代价。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学图像解释中频繁产生幻觉（临床可行但事实错误的发现），这直接威胁患者安全，因此需要系统评估和量化这些幻觉。

Method: 构建HalluCXR基准，包含856个分层MIMIC-CXR胸片和三种查询类型，共15,408次模型评估。提出八类别幻觉分类法（含临床严重性评级）和两层检测流水线（自动检测F1=0.959，LLM判断F1=0.907），并用250个人工标注验证。

Result: 发现61.9%-82.3%输出含幻觉，正常胸片最易引发严重幻觉，常见发现被过度虚构、罕见发现被漏检，响应长度预测幻觉风险（AUC达0.908）。六模型集成降低虚构84.8%但增加遗漏，三模型子集性能相当且成本减半。

Conclusion: 幻觉审计、基于冗长性的风险监控以及基于集成的安全层是临床部署的前提条件。
...
### [56] [EPC-3D-Diff: Equivariant Physics Consistent Conditional 3D Latent Diffusion for CBCT to CT Synthesis](https://arxiv.org/abs/2605.20470)
*Alzahra Altalib,Chunhui Li,Haytham Al Ewaidat,Khaled Alawneh,Ahmad Qendel,Alessandro Perelli*

Main category: cs.CV

TL;DR: 本文提出EPC-3D-Diff，一种基于3D潜空间扩散的CBCT到CT合成方法，通过投影域等变性损失提升HU精度，在体模和临床数据上PSNR分别提升7.4 dB和1.8 dB。


<details>
  <summary>Details</summary>
Motivation: CBCT的散射、噪声和伪影导致HU不准确，影响放疗定量分析。

Method: 在3D自编码器潜空间中进行条件扩散，加入基于投影域旋转等变性的物理一致性损失。

Result: 体模PSNR+7.4 dB，临床数据PSNR+1.8 dB，SSIM和HU精度均优于对比方法。

Conclusion: EPC-3D-Diff提升了合成图像的物理一致性和鲁棒性，支持HU感知的放疗工作流。
...
### [57] [Goodbye Drift: Anchored Tree Sampling for Long-Horizon Video-to-Video Generation](https://arxiv.org/abs/2605.20476)
*Matthew Bendel,Stephen W. Bailey,Mithilesh Vaidya,Sumukh Badam,Xingzhe He*

Main category: cs.CV

TL;DR: 提出了一种无需训练的推理时调度方法锚定树采样（ATS），通过稀疏到密集的树状插值框架解决长时视频生成中的漂移和连续性问题。


<details>
  <summary>Details</summary>
Motivation: 长时视频生成面临质量随时间退化的漂移问题和物体持续性问题的连续性挑战，现有方法依赖自回归蒸馏无法分离解决漂移问题。

Method: 采用稀疏到密集的锚定插值策略：根调用生成全时域稀疏锚点，递归优化生成中间锚点，最终在相邻锚点间合成叶子片段，将关键路径从K步自回归减少为L+1步树层次步骤。

Result: 在Wan 2.1+VACE模型上，ATS在五种条件模态（修复、扩展、边缘、姿态、深度）下均优于自回归基线，并能在LTX-2.3上稳定生成≥40分钟的视频。

Conclusion: ATS有效将漂移限制在锚点区间内，可通过扩展支持任意长度文本生成视频、动态摄像头及多镜头场景。
...
### [58] [Oracle Supervision Transfers for Hyperparameter Prediction in Model-Based Image Denoising](https://arxiv.org/abs/2605.20479)
*Jianmin Liao,Lixin Shen,Yuesheng Xu*

Main category: cs.CV

TL;DR: 本文提出HyperDn，一种跨配置的超参数预测方法，能从源配置转移监督到新目标配置，仅需极少目标标签即可达到近最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有超参数预测方法需要为每种新配置重新收集带标签的oracle训练集，扩展性差且成本高。

Method: HyperDn使用配置条件化的预测器，池化源配置的oracle监督，预测新配置的异质超参数。

Result: 在从TV/TGV迁移到DiffPIR时，仅用2个目标标签达到30.23 dB（接近oracle的0.90 dB内），且无标签时在未见混合噪声和跨图像分辨率迁移中表现优异。

Conclusion: oracle监督可从源配置转移到目标配置，大幅减少为每配置重建标签的需求。
...
### [59] [A Human-in-the-Loop Framework for Efficient Prompt Selection in Microscopy Vision-Language Models](https://arxiv.org/abs/2605.20495)
*Abhiram Kandiyana,Ankur Mali,Lawrence O. Hall,Peter R. Mouton,Dmitry Goldgof*

Main category: cs.CV

TL;DR: 提出目标驱动的主动学习框架，通过优先选择待验证图像来优化提示集构建，在显微图像分类中以更少人工标注达到目标性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示调优方法虽减少标注量，但未解决选择哪些未标注图像优先验证及所需验证样本数量的问题。

Method: 将提示集构建建模为目标驱动的主动学习问题，在低资源约束下研究三种互补的选择标准，基于小规模未标注池评估。

Result: 该方法以显著更少的专家验证图像达到目标性能，平均仅需20张标注图像即可实现100%测试准确率。

Conclusion: 人机协同框架在降低标注成本的同时，保持专家对模型输出的审核与优化，展现生成式AI在生物医学图像分析中的人本化应用。
...
### [60] [ShadeBench: A Benchmark Dataset for Building Shade Simulation in Sustainable Society](https://arxiv.org/abs/2605.20510)
*Longchao Da,Mithun Shivakoti,Xiangrui Liu,T Pranav Kutralingam,Yezhou Yang,Hua Wei*

Main category: cs.CV

TL;DR: 提出了ShadeBench数据集和基准测试，用于城市阴影理解，包含多模态数据并支持阴影生成、分割和3D重建等任务。


<details>
  <summary>Details</summary>
Motivation: 城市热暴露问题日益严重，但缺乏大规模数据和系统化框架来建模和分析城市阴影模式。

Method: 构建了包含多模态数据集（如阴影地图、卫星图像、建筑骨架和3D网格）的基准测试，并建立了标准化评估协议和基线方法。

Result: ShadeBench支持可扩展的阴影分析，为数据驱动城市气候研究和热适应规划提供基础。

Conclusion: ShadeBench为阴影理解和城市气候研究提供了有效工具，有助于未来热适应城市规划和决策。
...
### [61] [HADS-Net:A Hybrid Attention-Augmented Dual-Stream Network with Physics-Informed Augmentation for Breast Ultrasound Image Classification](https://arxiv.org/abs/2605.20536)
*Chinedu Emmanuel Mbonu,Blessing Nwamaka Iduh,Joseph Ikechukwu Odo,Doris Chinedu Asogwa*

Main category: cs.CV

TL;DR: 提出了一种结合物理信息增强与跨注意力融合的双流网络HADS-Net，在乳腺超声图像三分类（良性/恶性/正常）中达到96.58%准确率且无恶性被误判为正常。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法忽略超声成像物理特性，且未单独设计专门提取病灶边界特征的网络支路。

Method: 双流架构：纹理流采用物理感知数据增强+EfficientNet-B3提取特征，边界流经Sobel边缘图+轻量CNN处理，用交叉注意力模块融合特征后经MLP分类（自适应类加权focal loss）。

Result: BUSI数据集上准确率96.58%，宏ROC-AUC 0.9978，宏F1 0.9654，各类F1分别为0.970/0.951/0.976，无恶性被误判为正常。

Conclusion: 针对超声模态的特定增强与跨注意力特征融合策略能有效提升诊断性能。
...
### [62] [Continual Segmentation under Joint Nonstationarity](https://arxiv.org/abs/2605.20538)
*Prashant Pandey,Himanshu Kumar,Devineni Sri Venkatraya Chowdary,Brejesh Lall*

Main category: cs.CV

TL;DR: 该论文提出了一种在联合非平稳性下进行持续语义分割的方法，结合梯度自适应稳定化和原型锚定半监督学习，在类增量、域增量和少样本场景中均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习研究通常孤立处理类、域或标签转移，但实际系统中这些因素常同时出现，导致未探索的联合非平稳性问题。

Method: 采用梯度自适应稳定化（参数级正则化）和原型锚定监督（联合置信度与原型一致性验证伪标签）的半监督学习。

Result: 在类增量、域增量和少样本设置下的实验表明，该方法在异构结构化预测环境中持续优于先前方法。

Conclusion: 梯度自适应稳定化和原型锚定半监督学习能有效应对持续分割中的联合非平稳性，暴露了现有方法的固有问题，为动态环境下的鲁棒稠密预测提供了新见解。
...
### [63] [Faster or Stronger: Towards Flexible Visual Place Recognition via Weighted Aggregation and Token Pruning](https://arxiv.org/abs/2605.20551)
*Zichao Zeng,June Moh Goo,Junwei Zheng,Weijia Fan,Jiaming Zhang,Rainer Stiefelhagen,Jan Boehm*

Main category: cs.CV

TL;DR: 提出了WeiAD和WeiToP两种方法，分别通过加权聚类聚合和基于自蒸馏的token剪枝，提升视觉地点识别的准确性并降低计算延迟。


<details>
  <summary>Details</summary>
Motivation: 现有ViT-based VPR方法中，均匀聚类聚合忽略了不同聚类对性能的不平等贡献，且特征提取成本高，未被压缩策略充分优化。

Method: 利用加权聚类聚合（WeiAD）增强判别性，同时通过自蒸馏框架WeiToP，用聚合层token重要性指导早期Transformer层的剪枝模块，实现推理时剪枝。

Result: WeiAD提升了全局描述符的判别力；WeiToP在减少特征提取成本的同时优于通用视觉任务中的剪枝方法，并支持灵活的精度-效率权衡。
...
### [64] [Uncertainty-Guided Conservative Propagation for Structured Inference in Vessel Segmentation](https://arxiv.org/abs/2605.20543)
*Huan Huang,Michele Esposito,Chen Zhao*

Main category: cs.CV

TL;DR: 提出了一种不确定性引导的保守传播模块，用于血管分割，通过少量更新步骤和改进推理区域来优化分割结果。


<details>
  <summary>Details</summary>
Motivation: 现有深度模型多为单次预测，无法在推理中细化不确定或断开的区域，导致血管分割精度受限。

Method: 不确定性引导保守传播（UGCP），一种可插拔模块，在logit空间进行少量更新步骤，利用预测不确定性指导可靠区域支持模糊区域，并引入结构感知调制和源稳定机制来减少不可靠传播和过度漂移。

Result: 在四个公共血管分割数据集上（2D和3D任务），UGCP显著提升了Dice相似系数、中心线Dice和95% Hausdorff距离，并减少了血管断开，保持结构一致性，且计算开销小。

Conclusion: UGCP是一种有效的通用模块，能提升多种血管分割网络的性能，尤其改善结构连续性。
...
### [65] [Beyond Routing: Characterising Expert Tuning and Representation in Vision Mixture-of-Experts](https://arxiv.org/abs/2605.20610)
*Gene Tangtartharakul,Katherine R. Storrs*

Main category: cs.CV

TL;DR: 分析了视觉MoE模型中专家的特异化，发现专家不仅按类别路由，还编码连续视觉和语义特征，且生/非生物区分稳定。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅通过路由分析MoE专家角色，缺乏对专家编码内容的深入理解。

Method: 训练对比学习的稀疏门控卷积MoE模型，使用视觉神经科学工具、THINGS语义维度及表征相似性分析。

Result: 专家在动/非生物区分上稳定，路由稀疏但调谐广泛，特征调谐跨类别边界，专家间类别可分性相似。

Conclusion: 视觉MoE的专家特异化远超类别路由，需通过细粒度调谐和表征结构来理解。
...
### [66] [Accelerating Video Inverse Problem Solvers with Autoregressive Diffusion Models](https://arxiv.org/abs/2605.20624)
*Taesung Kwon,Jonghyun Park,Hyungjin Chung,Jong Chul Ye*

Main category: cs.CV

TL;DR: 提出一种基于自回归视频扩散模型的零样本逆问题求解器AVIS，实现流式视频恢复，显著降低延迟并提高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在零样本视频逆问题中表现优异，但存在高初始延迟和低吞吐量两大效率瓶颈。

Method: 利用自回归视频扩散模型进行流式恢复，通过测量一致性估计初始化反向扩散，减少采样步数；AVIS Flash仅在第一个块施加测量一致性。

Result: 相比非自回归方法，初始延迟从114秒降至4秒，吞吐量从0.71 FPS提升至1.18 FPS；AVIS Flash在RTX 4090上达5.91 FPS。

Conclusion: AVIS框架在保持恢复质量的同时大幅提升效率，为实现实时部署提供了可行方案。
...
### [67] [End-to-End Unmixing with Material Prompts for Hyperspectral Object Tracking](https://arxiv.org/abs/2605.20569)
*Xu Han,Mohammad Aminul Islam,Lei Wang,Zekun Long,Guanmanyi Fu,Wangshu Cai,Kuldip K. Paliwal,Jun Zhou*

Main category: cs.CV

TL;DR: 提出了一种端到端的材料感知高光谱跟踪框架，通过联合优化材料分解与目标定位，利用加权目标导向解混损失和对偶分支小波增强材料提示模块，实现了最先进的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖预训练的RGB跟踪器，忽略了高光谱图像中固有的材料信息，且材料感知方法通常将光谱解混与跟踪目标解耦，限制了材料表示对定位的优化。

Method: 包括一个材料表示分解模块（用于深度学习的自适应频谱分解）和一个对偶分支小波增强材料提示模块（在频域中学习低频和高频材料提示）。

Result: 在标准高光谱跟踪基准上取得了最先进性能，验证了端到端材料感知框架的有效性。

Conclusion: 提出的框架是模型无关的，能泛化到不同解混骨干网络，通过联合优化有效提升了跟踪鲁棒性。
...
### [68] [MAPS: A Synthetic Dataset for Probing Vision Models in a Controlled 3D Scene Space](https://arxiv.org/abs/2605.20549)
*Santiago Galella,Pamela Osuna-Vargas,Maren Wehrheim,Martina G. Vilas,Gemma Roig,Matthias Kaschube*

Main category: cs.CV

TL;DR: 提出MAPS，一个通过参数化3D场景评估视觉模型对场景因子依赖性的可扩展工具，发现所有架构对相机距离和仰角存在普遍敏感缺陷，且敏感模式更受微观设计而非CNN/Transformer类别影响。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法系统分离和归因模型预测的3D场景参数（如背景、光照、相机），限制了对模型行为背后驱动因素的理解。

Method: 构建含2618个可识别网格的资源库和Blender渲染管道，通过连续变化9个场景因子生成图像，并运用回归敏感性分析量化模型依赖度。

Result: 20个卷积与Transformer模型在相机距离和仰角上均表现出一致的识别失效；现代CNN与Transformer的敏感结构聚类相似，与旧架构显著不同。

Conclusion: 相机视角是模型鲁棒性的共性薄弱点，且模型敏感度由细粒度架构设计主导，而非粗粒度的CNN/Transformer分类。
...
### [69] [Pareto-Enhanced Portrait Generation: Vision-Aligned Text Supervision for Alignment, Realism, and Aesthetics](https://arxiv.org/abs/2605.20640)
*Yunlong Wang,Jinjin Shi,Wenbin Gao,Xuran Xu,Runyu Shi,Ying Huang*

Main category: cs.CV

TL;DR: 提出一种特征监督范式，通过交叉模态对齐和隐式美学信号提取，同时提升文本-图像对齐、逼真度和美学效果，且无额外推理开销。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在人像生成中存在三维困境：文本-图像对齐、逼真度和美学相互抑制。

Method: 针对多模态扩散变换器（MM-DiT）设计特征监督：利用SigLIP 2的轻量级交叉模态对齐提取多粒度文本表示，监督图像分支；同时从预训练视觉基础模型中挖掘隐式美学信号。

Result: 实验表明该方法推动了帕累托前沿，在文本-图像对齐、逼真度和美学上实现协同提升。

Conclusion: 该范式有效打破SFT的过拟合和性能退化问题，实现全面优化。
...
### [70] [$Δ$ynamics: Language-Based Representation for Inferring Rigid-Body Dynamics From Videos](https://arxiv.org/abs/2605.20576)
...
TL;DR: 提出一种名为ΔYNAMICS的视觉-语言框架，通过语言作为统一表示来推断刚体物理状态和属性，显著提升分割性能并在真实世界视频中表现出强迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖特定物理系统、物体类型和相机位姿，无法泛化到复杂真实场景。

Method: 使用语言作为刚体动力学的统一表示，通过生成结构化文本配置进行物理模拟；整合自然语言运动推理和光流作为语义无关输入。

Result: 在CLEVRER数据集上分割IoU达0.30，比领先VLM提升7倍；测试时采样和进化搜索分别进一步提升27%和120%的IoU；在235个真实刚体视频数据集中展示强泛化性。

Conclusion: 语言驱动的物理推理能有效桥接感知与模拟，并在复杂真实场景中具有潜力。
...
### [71] [Rethinking Cross-Layer Information Routing in Diffusion Transformers](https://arxiv.org/abs/2605.20708)
*Chao Xu,Maohua Li,Qirui Li,Yixuan Xu,Yanke Zhou,Yunhe Li,Cuifeng Shen,Hanlin Tang,Kan Liu,Tao Lan,Lin Qu,Shao-Qun Zhang*

Main category: cs.CV

TL;DR: 本文提出扩散自适应路由（DAR），一种可学习、时间步自适应且非增量的残差替代方法，用于解决扩散Transformer中跨层信息流的三个问题，并显著提升性能与训练效率。


<details>
  <summary>Details</summary>
Motivation: 传统残差连接在DiT中存在前向幅度膨胀、反向梯度衰减和逐块冗余三个症状，因此需要设计更优的跨层信息聚合机制。

Method: 提出DAR，对子层输出历史进行可学习、时间步自适应、非增量的聚合，作为残差连接的替代方案，并兼容REPA等现代增强方法。

Result: 在ImageNet 256×256上，DAR使SiT-XL/2的FID从9.67降至7.56，以8.75倍更少迭代达到基线质量；叠加REPA后早期训练加速2倍。

Conclusion: 跨层信息路由是扩散建模中未被充分探索的设计维度，与现有表示对齐目标正交，DAR在预训练和微调阶段均有效。
...
### [72] [QwenSafe: Multimodal Content Rating Description Identification via Preference-Aligned VLMs](https://arxiv.org/abs/2605.20584)
*Dishanika Denipitiyage,Aruna Seneviratne,Suranga Seneviratne*

Main category: cs.CV

TL;DR: 本文提出QwenSafe，一个视觉语言模型，用于自动识别苹果定义的内容评级描述符，通过联合推理应用元数据和截图，显著提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 移动应用市场中，确保内容评级描述符的准确性和一致性具有挑战性，因为应用内容涵盖文本描述和视觉界面等多模态信息。

Method: 使用metadata2CRD管道合成问答对，基于Qwen3-VL-8B进行监督微调和直接偏好优化（DPO），实现跨模态的对齐。

Result: 在12个苹果内容评级描述符上，QwenSafe在二元分类中正类召回率分别比Qwen3-VL、LLaVA-1.6和Gemini-2.5-Flash提升111.8%、36.1%和2.1%。

Conclusion: 描述符感知的多模态对齐可大幅改进自动内容分类，视觉语言模型有潜力支持可扩展且一致的应用内容评级。
...
### [73] [SAVER: Selective As-Needed Vision Evidence for Multimodal Information Extraction](https://arxiv.org/abs/2605.20713)
*Miaobo Hu,Shuhao Hu,Bokun Wang,Rui Chen,Xin Wang,Xiaobo Guo,Daren Zha,Jun Xiao*

Main category: cs.CV

TL;DR: 提出了一种选择性视觉按需框架SAVER，用于多模态命名实体识别和关系抽取，通过门控机制和子模选择在需要时激活视觉信息，提升了性能并降低了计算开销。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中多模态IE因图像与文本关系弱、冗余或误导而困难，传统的始终融合方式浪费计算且可能放大虚假视觉线索。

Method: SAVER框架包含Conformal Groundability Gate（CGG）估计视觉可接地性、子模相关-多样性选择器选取证据子集、Set Transformer聚合及能量启发联合评分头。

Result: 相比强文本基线及始终融合多模态方法，SAVER在F1上持续提升，同时降低了AURC、提高了风险固定下的激活覆盖率，并减少了FLOPs和P90延迟。

Conclusion: 按需激活视觉信息能有效提升多模态IE性能与效率。
...
### [74] [Head-Aware Key-Value Compression for Efficient Autoregressive Image Generation](https://arxiv.org/abs/2605.20600)
*Guotao Liang,Baoquan Zhang,Zhiyuan Wen,Yunming Ye*

Main category: cs.CV

TL;DR: 提出了一种面向自回归图像生成的新型头感知KV缓存压缩框架HeadKV，通过识别不同注意力头类型并分配差异化缓存预算，同时设计分层令牌驱逐策略，在保持图像质量的同时降低内存占用并提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有基于缓存行压缩的方法给每个注意力头分配固定预算，忽略了不同头之间注意力模式的异质性（部分头关注局部邻域，部分头关注全局上下文），导致内存分配次优。

Method: 首先观察层内各头在不同位置存在一致注意力模式，据此在生成早期识别头类型；为局部偏置头分配较小缓存预算，为全局头分配较大预算；并设计分层令牌驱逐策略保留长距离信息。

Result: 在多个自回归图像生成模型上实验，证明该方法能有效降低内存使用、提升吞吐量，同时保持高质量图像生成。

Conclusion: 通过感知注意力头异质性的动态缓存压缩，可以在不增加额外训练或数据集统计的前提下，实现高效且通用的自回归图像生成。
...
### [75] [Mind Your Margin and Boundary: Are Your Distilled Datasets Truly Robust?](https://arxiv.org/abs/2605.20606)
*Muquan Li,Yingyi Ma,Yihong Huang,Hang Gou,Ke Qin,Ming Li,Yuan-Fang Li,Tao He*

Main category: cs.CV

TL;DR: 提出C²R框架，通过攻击感知课程学习与对比鲁棒性目标，改善数据集蒸馏的精度-鲁棒性权衡。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒数据集蒸馏方法对所有对抗样本统一处理，未关注鲁棒风险主要由近零鲁棒边界主导，且未显式增强决策边界间隔。

Method: 基于鲁棒边界推导扰动分数近似样本的鲁棒铰链损失，优先处理最小边界对手；同时使用类别平衡的对比鲁棒性损失增强边界分离。

Result: 在CIFAR-10/100、Tiny-ImageNet及ImageNet-1K子集上，六种攻击下平均鲁棒准确率提升2.8%。

Conclusion: C²R有效平衡了精度与鲁棒性，为鲁棒数据集蒸馏提供了新思路。
...
### [76] [TASTE: A Designer-Annotated Multi-Dimensional Preference Dataset for AI-Generated Graphic Design](https://arxiv.org/abs/2605.20731)
*Haonan Zhu,Elad Hirsch,Alexandria Minetti,Allison Nulty,Purvanshi Mehta*

Main category: cs.CV

TL;DR: 发布了TASTE数据集和三项贡献：批评标准无关的信号测试框架、现有系统与设计师一致性不足（宏一致性低于0.55）、一个小型成对差异头模型达到0.611一致性，接近人类评分的0.741上限。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型生成的图形设计评估中，现有方法仅使用单一整体偏好标签，忽略了设计评估的多维度（如排版、色彩、布局等）需求。

Method: 构建TASTE数据集（10位设计师在9个标准上对4个模型的输出进行评分），提出标准无关的信号测试框架（使用Kendall's tau、多数概率和Condorcet环），并训练小型成对差异头模型。

Result: 设计师一致性介于食物与电影偏好之间；现有系统均不超过0.55宏一致性；小模型达到0.611，缩小了与人类评分的差距。

Conclusion: 需要多维度评估体系改进文本到图像模型，现有自动评估器与设计师差异显著，但小模型有潜力弥补。
...
### [77] [ArchSIBench: Benchmarking the Architectural Spatial Intelligence of Vision-Language Models](https://arxiv.org/abs/2605.20837)
*Qirui Shen,Wenda Wang,Jiachen Lu,Zilong Huang,Jin Bai,Lei He,Hongxuan Chen,Weixin Huang*

Main category: cs.CV

TL;DR: 本文提出了ArchSIBench，一个用于评估视觉-语言模型建筑空间智能的基准，涵盖感知、推理、导航、变换和配置五个核心维度，包含17个子任务和3000个问答对。


<details>
  <summary>Details</summary>
Motivation: 现有研究只评估了VLMs的基本空间技能，忽略了建筑空间的高级认知如布局、动线和功能分区。

Method: 基于建筑学、认知科学和心理学的视角，设计了ArchSIBench基准，包含五个维度和17个细粒度子任务，并通过专家手工标注构建了3000个问答对。

Result: 大多数VLMs的建筑空间智能与人类基线存在显著差异，不同能力维度间表现波动大；部分最先进模型接近未经建筑训练的人类水平，但与受过建筑训练的人类相比仍有差距，特别是在空间变换和配置推理方面。
...
### [78] [USV: Towards Understanding the User-generated Short-form Videos](https://arxiv.org/abs/2605.20838)
*Haoyue Cheng,Su Xu,Liwei Jin,Wayne Wu,Chen Qian,Limin Wang*

Main category: cs.CV

TL;DR: 提出了USV数据集，用于用户生成短视频的高层次语义理解，并建立了主题识别和视频-文本检索两个任务及相应基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频数据集和识别方法多关注实例级识别，不足以捕捉用户生成短视频的高层语义信息。

Method: 构建了包含约22.4万短视频的USV数据集，提出MMF-Net用于主题识别，VTCL用于视频-文本检索。

Result: 在USV上进行了综合基准测试，证明了方法的有效性。

Conclusion: USV数据集及其基线方法为短视频高层次语义理解提供了基础，便于未来研究。
...
### [79] [Seeing Through Fog: Towards Fog-Invariant Action Recognition](https://arxiv.org/abs/2605.20645)
*Enqi Liu,Liyuan Pan,Zhi Gao,Lingzhi Li,Qing Li*

Main category: cs.CV

TL;DR: 本文针对雾霾天气下动作识别面临的挑战，提出首个雾霾动作识别基准数据集FogAct和双流CLIP模型FogNet，通过提取雾不变语义信息实现鲁棒识别，在多个数据集上达到与SOTA相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现实应用中雾天频繁，但现有动作识别方法假设好天气和高清输入，雾天能见度降低和对比度下降阻碍语义提取，亟需专门解决方案。

Method: 构建包含近万段配对清晰/雾天视频的FogAct数据集，并提出FogNet双流CLIP模型，利用清晰视频引导学习雾天视频的鲁棒表示，捕捉共享的结构和运动线索。

Result: 在FogAct和三个公开数据集上的实验表明，该方法性能与现有最先进方法具有竞争力。

Conclusion: 通过专门数据集和模型，有效缓解了雾天动作识别的性能退化，为实际应用提供了可行方案。
...
TL;DR: 提出视频场景变化检测（VSCD）任务及大规模基准数据集，设计查询中心多参考模型，在非同步、自由视角视频中实现像素级变化检测，并在机器人应用中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有变化检测方法受限于固定视角、轻微错位或少量变化物体，无法处理自由视角和大量物体变化的实际场景。

Method: 构建包含110万帧标注的大规模基准和真实测试集；提出查询中心多参考模型，通过变化掩码监督学习时间匹配，利用局部块对应对齐参考特征，融合帧级和块级置信度解码高分辨率掩码。

Result: 在图像和视频基线上达到最优性能，并在移动机器人的视觉监控和目标增量学习任务中验证真实世界有效性。

Conclusion: 提出的VSCD方法和模型在复杂、非同步视频场景中有效检测变化，具有实际部署潜力。
...
### [81] [Gaze into the Details: Locality-Sensitive Enhancement for OCTA Retinal Vessel Segmentation](https://arxiv.org/abs/2605.20651)
*Tuopusen Huang,Ding Ma,Xiangqian Wu*

Main category: cs.CV

TL;DR: 提出了LSENet，一种改进的U-Net架构，用于OCTA血管分割，通过三个创新模块解决了低局部对比度导致的血管不连续和细节丢失问题。


<details>
  <summary>Details</summary>
Motivation: 现有OCTA血管分割方法多基于U-Net，但仅关注全局特征，无法处理OCTA特有的低局部对比度问题，导致血管不连续和细节丢失。

Method: 在U-Net基础上引入三个模块：PIE模块（补丁信息增强，替换跳跃连接进行补丁级注意力）、MFF模块（多尺度特征融合，提供丰富多尺度信息）、CRD模块（连接性细化解码器，用大卷积核减少碎片化）。

Result: 在三个公共数据集（OCTA-500、ROSE-1、ROSSA）上达到最先进性能，且参数量更少。

Conclusion: LSENet有效解决了OCTA血管分割中的低对比度问题，实现了高性能和高效率。
...
### [82] [LiteViLNet: Lightweight Vision-LiDAR Fusion Network for Efficient Road Segmentation](https://arxiv.org/abs/2605.21007)
*Daojie Peng,Bingtao Wang,Fulong Ma,Liang Zhang,Jun Ma*

Main category: cs.CV

TL;DR: 提出了LiteViLNet，一种轻量级多模态道路分割网络，在KITTI数据集上以14.04M参数达到96.36% MaxF，速度163.79 FPS（RTX 4060 Ti）。


<details>
  <summary>Details</summary>
Motivation: 现有基于transformer的多模态道路分割方法计算成本高，难以在资源受限的嵌入式设备上实现实时推理。

Method: 设计双流轻量级编码器和深度可分离卷积提取特征；提出多尺度特征融合模块实现跨模态交互；大核桥接模块捕获长距离依赖。

Result: 14.04M参数，MaxF 96.36%，在RTX 4060 Ti上163.79 FPS，Jetson Orin NX上22.18 FPS，性能优于多数CNN方法，与transformer方法相当。

Conclusion: LiteViLNet在精度与效率间取得优异平衡，验证了其在自动驾驶和智能机器人中实时嵌入式部署的潜力。
...
### [83] [VISTA: Technical Report for the Ego4D Short-Term Object Interaction Anticipation at EgoVis 2026](https://arxiv.org/abs/2605.20901)
*Qiaohui Chu,Haoyu Zhang,Yisen Feng,Meng Liu,Weili Guan,Dongmei Jiang,Liqiang Nie*

Main category: cs.CV

TL;DR: VISTA是一个用于Ego4D短期物体交互预测任务的视觉预测模型，在EgoVis 2026挑战中取得第一名。


<details>
  <summary>Details</summary>
Motivation: 为了解决Ego4D挑战中预测下一帧人-物交互的需求，包括物体包围框、类别、动作和接触时间等。

Method: 采用StillFast设计，结合COCO预训练的Faster R-CNN进行空间检测，以及冻结的V-JEPA 2.1提取时序上下文，通过特征调制和ROI融合实现多任务预测。

Result: 在Ego4D挑战服务器上获得第一名，展示出高效且鲁棒的性能。

Conclusion: VISTA验证了结合空间和时序特征在短期交互预测中的有效性，为未来研究提供基线。
...
### [84] [Grounding Driving VLA via Inverse Kinematics](https://arxiv.org/abs/2605.21061)
*Junsung Park,Hyunjung Shim*

Main category: cs.CV

TL;DR: 现有驾驶VLA模型因任务公式不恰当而忽略视觉令牌，本研究通过逆运动学视角重新设计，提出预测未来视觉状态和独立逆运动学网络，实现0.5B模型性能媲美7B-8B模型。


<details>
  <summary>Details</summary>
Motivation: 发现驾驶VLA模型忽略视觉令牌的根本原因并非训练不足，而是任务公式结构上有问题，导致模型依赖自我状态和文本指令捷径。

Method: 重新设计驾驶VLA为逆运动学求解器：1) 预测未来视觉状态作为目标，提供密集视觉监督；2) 使用交叉注意力条件扩散模型作为独立逆运动学网络，仅接受当前和未来视觉状态输入。

Result: 0.5B规模模型在闭环NAVSIM-v2和nuScenes基准上达到或超越7B-8B VLA的性能，尤其在动态驾驶场景如转弯中视觉特征利用能力显著提升。

Conclusion: 通过简单的逆运动学视角重新设计，有效恢复了驾驶VLA的视觉基础能力，并显著缩小模型规模差距。
...
### [85] [RoPeSLR: 3D RoPE-driven Sparse-LowRank Attention for Efficient Diffusion Transformers](https://arxiv.org/abs/2605.20659)
*Yuxi Liu,Zekun Zhang,Yixiang Cai,Renjia Deng,Yutong He,Kun Yuan*

Main category: cs.CV

TL;DR: 提出RoPeSLR框架，通过3D RoPE引导的稀疏低秩注意力机制解决DiTs在长序列视频生成中的计算瓶颈，实现近无损生成且大幅加速。


<details>
  <summary>Details</summary>
Motivation: DiTs的平方复杂度限制长视频生成，现有稀疏线性注意力方法在极端稀疏下因RoPE悖论性能下降。

Method: 基于注意力流形可分性（高频语义稀疏集+极低秩背景连续区），采用头级低秩参数化与可学习3D绝对位置编码注入，替代标准线性注意力。

Result: 90%稀疏度下Wan2.1-1.3B FLOPs减少10倍，HunyuanVideo-13B 100K+令牌序列端到端加速2.26倍，VBench退化低于1.3%。

Conclusion: RoPeSLR有效解决RoPE悖论，实现长视频生成的高效缩放，保持生成质量的同时显著降低计算成本。
...
### [86] [Comparative Analysis of Military Detection Using Drone Imagery Across Multiple Visual Spectrums](https://arxiv.org/abs/2605.21157)
*Sourov Roy Shuvo,Prajwal Panth,Rajesh Chowdhury,Sorup Chakraborty,Sudip Chakrabarty,Prasant Kumar Pattnaik*

Main category: cs.CV

TL;DR: 研究了YOLOv11-small在多种模拟环境（灰度、热成像、夜视、遮蔽视觉）下对军事目标的检测性能。


<details>
  <summary>Details</summary>
Motivation: KIIT-MiTA数据集未考虑真实世界的多变场景，需要评估模型在不同环境条件下的表现。

Method: 基于KIIT-MiTA数据集创建四种模拟真实场景的数据集，并使用YOLOv11-small模型进行目标检测训练。

Result: 模型在多种视觉条件下有效检测军事目标，提升了无人机操作的可靠性和性能。

Conclusion: 该研究通过开发适应复杂环境的先进检测系统，增强了无人机在防御和进攻任务中的应用效能。
...
### [87] [LER-YOLO: Reliability-Aware Expert Routing for Misaligned RGB-Infrared UAV Detection](https://arxiv.org/abs/2605.20667)
*Liming Hou,Yueping Peng,Hexiang Hao,Ji Wang,Xuekai Zhang,Wei Tang,Zecong Ye,Xin Ying,Yubo He*

Main category: cs.CV

TL;DR: 提出LER-YOLO框架，通过不确定性感知对齐和可靠性引导的稀疏MoE融合，实现RGB-红外双模小无人机检测，在MBU数据集上达到89.7±0.2% AP50。


<details>
  <summary>Details</summary>
Motivation: 现有双模检测器在跨传感器对齐中缺乏局部对应可靠性评估，导致误匹配伪影传播到检测头。

Method: 设计不确定性感知目标对齐模块生成空间可靠性图，再由可靠性引导的稀疏MoE融合模块动态选择RGB主导、红外主导和交互融合专家。

Result: 在YOLOv5s-family协议下MBU数据集上，三独立种子平均AP50为89.7±0.2%，最佳结果89.9%。

Conclusion: 性能提升主要来自可靠性引导的专家路由机制，而非模型容量增加。
...
### [88] [GSA-YOLO: A High-Efficiency Framework via Structured Sparsity and Adaptive Knowledge Distillation for Real-Time X-ray Security Inspection](https://arxiv.org/abs/2605.20669)
*Jiahao Kong*

Main category: cs.CV

TL;DR: 提出一种基于YOLOv8n的轻量级框架GSA-YOLO，通过组套索、稀疏结构选择和自适应知识蒸馏，在X光安检中实现高效、准确的违禁品检测。


<details>
  <summary>Details</summary>
Motivation: 现有模型难以平衡X光安检中的严重遮挡、复杂杂乱和严格速度要求。

Method: 在YOLOv8n基础上集成组套索（GL）应用于网络颈部、稀疏结构选择（SSS）应用于检测头、以及自适应知识蒸馏（Ada-KD）机制。

Result: 在HiXray和PIDray数据集上，推理速度达189.62 FPS，计算成本从8.7G降至8.0G，mAP50:95分别提升2.4%和1.8%。

Conclusion: GSA-YOLO在保持计算效率的同时提高了检测精度，是实用X光安检的理想解决方案。
...
### [89] [Focus-then-Context: Subject-Centric Progressive Visual Token Reduction for Vision-Language Models](https://arxiv.org/abs/2605.20950)
*Yulin Zhao,Yun Wang,Dehua Zheng,Borui jiang,Zheng Zhang*

Main category: cs.CV

TL;DR: 提出了一种基于类人视觉焦点-上下文机制的视觉标记渐进缩减方法，在显著降低计算成本的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记缩减方法未充分挖掘显著主体及其上下文关系，导致计算效率与表征完整性之间的权衡不理想。

Method: 构建焦点识别模块挖掘视觉显著性与语义相关性，开发上下文感知结构扫描模块聚合邻域上下文信息。

Result: 在Qwen2.5-VL上仅保留22.2%视觉标记实现2.53倍加速，在LLaVA上减少67%FLOPs且准确率仅降0.6%。

Conclusion: 该方法验证了模仿人类视觉感知机制可有效平衡计算效率与视觉表征质量。
...
### [90] [Hyper-V2X: Hypernetworks for Estimating Epistemic and Aleatoric Uncertainty in Cooperative Bird's-Eye-View Semantic Segmentation](https://arxiv.org/abs/2605.21309)
*Abhishek Dinkar Jagtap,Sanath Tiptur Sadashivaiah,Andreas Festag*

Main category: cs.CV

TL;DR: 本文提出了Hyper-V2X，一种基于超网络的方法，用于估计V2X协作感知中的认知不确定性和随机不确定性，而无需显著增加计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有V2X协作感知工作在多智能体融合方面取得进展，但不确定性量化尚未得到充分研究。

Method: 通过部分权重生成方案和V2X上下文嵌入模块，将贝叶斯超网络条件化于融合的多智能体特征，生成随机BEV分割的权重分布。

Result: 在OPV2V基准上，Hyper-V2X提供准确且校准良好的不确定性估计，提高了感知可靠性。

Conclusion: Hyper-V2X是一种架构无关的方法，可与现代协作骨干网（如CoBEVT）无缝集成，实现高效的不确定性估计。
...
TL;DR: 比较四种预训练CNN架构在GAN生成图像检测上的性能，VGG16达91%最高准确率。


<details>
  <summary>Details</summary>
Motivation: GAN生成图像检测面临挑战，需要评估不同CNN架构的检测能力。

Method: 统一预处理流程，使用VGG16、ResNet50等四种模型，通过准确率等指标评估。

Result: VGG16准确率最高（91%），EfficientNetB0对假图敏感但可靠性低。

Conclusion: 需平衡数据集、增强增广和公平训练以提升检测鲁棒性。
...
TL;DR: 本文发现LVLM在生成时会逐渐忽视视觉证据导致幻觉，提出基于层间注意力差异的无需训练方法，通过增强视觉注意力来缓解幻觉。


<details>
  <summary>Details</summary>
Motivation: LVLM虽表现优异但容易产生幻觉，且现有方法未充分利用模型内部注意力差异。

Method: 提出ILVAD方法，利用层间视觉注意力差异构建显著性图，增强生成过程中的视觉注意力并选择与视觉证据强关联的文本token。

Result: 在五个最新模型上验证，该方法能持续缓解幻觉，且无需训练可即插即用。

Conclusion: 该方法有效缓解LVLM幻觉，具有通用性。
...
### [93] [VISTAQA: Benchmarking Joint Visual Question Answering and Pixel-Level Evidence](https://arxiv.org/abs/2605.20676)
*Mozhgan Nasr Azadani,Yimu Wang,Yongpeng Zhu,Lihong Chen,Milan Ganai,Sean Sedwards,Marco Pavone,Krzysztof Czarnecki*

Main category: cs.CV

TL;DR: 提出了VISTAQA基准和GROVE指标，用于联合评估多模态大模型在视觉问答中的答案正确性和证据定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法将答案正确性和定位能力分开考虑，缺乏对推理与定位耦合的明确强制对齐。

Method: 构建VISTAQA数据集（含1157个专家样本，覆盖6种任务和6个视觉领域），并提出GROVE指标，通过逐样本几何平均结合文本准确性和定位质量。

Result: 最强系统在GROVE指标下表现有限，揭示了答案准确性与视觉证据对齐之间的显著差距。

Conclusion: 当前多模态模型在联合推理与定位任务中存在明显不足，需要进一步改进。
...
### [94] [Closed Loop Dynamic Driving Data Mixture for Real-Synthetic Co-Training](https://arxiv.org/abs/2605.21372)
*Hongzhi Ruan,Pei Liu,Weiliang Ma,Zhengning Li,Xueyang Zhang,Jun Ma,Dan Xu,Kun Zhan*

Main category: cs.CV

TL;DR: 提出了AutoScale，一种自动化的闭环数据引擎，通过动态优化真实与合成数据的混合比例，在有限训练预算下提升自动驾驶模型性能。


<details>
  <summary>Details</summary>
Motivation: 真实驾驶数据标注昂贵且存在场景偏差，而简单混合所有合成数据效率低下且导致分布偏移，缺乏在有限预算下优化数据混合的方法。

Method: 将数据混合建模为动态优化过程，使用Graph-RAE进行场景表示、Cluster-GA进行聚类重要性估计与重加权、聚类引导向量检索选择高价值样本，并通过闭环反馈迭代调整混合比例。

Result: 在NavSim上，AutoScale优于简单联合训练及跨域基线，在较少合成样本下取得更优性能。

Conclusion: 通过闭环反馈和场景感知的数据混合优化，可以有效提升数据效率，为自动驾驶中的真实-合成协同训练提供实用框架。
...
### [95] [DarkShake-DVS: Event-based Human Action Recognition under Low-light andShaking Camera Conditions](https://arxiv.org/abs/2605.20680)
*Jiaqi Chen,Qinfu Xu,Liyuan Pan*

Main category: cs.CV

TL;DR: 提出EIS-HAR方法，结合事件相机和IMU数据，通过运动补偿和混合架构实现低光、六自由度运动下的动作识别，并发布DarkShake-DVS基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有动作识别方法在低光和相机运动剧烈时表现不佳，且缺乏相关基准数据集和有效的运动补偿技术。

Method: EIS-HAR包含两个模块：1) EIS模块，通过非线性扭曲函数重建运动补偿输入以减少模糊；2) HAR模块，采用四阶段混合架构提取时空特征。

Result: 在三个数据集上，EIS-HAR持续优于现有最先进方法。

Conclusion: 事件相机与IMU结合能有效应对低光和运动挑战，EIS-HAR方法在动作识别任务中表现出优越性。
...
### [96] [IndusAgent: Reinforcing Open-Vocabulary Industrial Anomaly Detection with Agentic Tools](https://arxiv.org/abs/2605.20682)
...
TL;DR: 提出了一个工具增强的智能体框架IndusAgent，用于开放词汇的工业异常检测，在五个基准上取得了最先进的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在工业异常检测中受到领域不对齐推理和幻觉结构推断的限制。

Method: 构建了Indus-CoT数据集，动态编排外部工具（如区域裁剪、高频增强、先验检索），并使用门控强化学习优化分类、定位、推理和工具使用。

Result: 在MVTec-AD等五个基准上实现了最先进的零样本性能。

Conclusion: IndusAgent在零样本工业异常检测中展现出鲁棒性和泛化能力。
...
### [97] [Holistic Reliability Propagation: Decoupling Annotation and Prediction for Robust Noisy-Label](https://arxiv.org/abs/2605.20725)
*Jingyang Mao,Ningkang Peng,Yanhui Gu*

Main category: cs.CV

TL;DR: 提出解耦可靠性度量和整体可靠性传播（HRP）方法，用于多媒体分类中的噪声标签学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法将外部标注和模型预测合并为一个可靠性权重，但两者可能因不同原因失效。

Method: 使用双层元学习为每个样本生成两个解耦的批归一化标量（alpha对应给定标签，beta对应伪标签），并通过HRP将它们路由到不同目标：输入分支采用可靠性感知Mixup和全局门控，对比分支采用beta门控伪标签正样本。

Result: 在合成和真实基准测试中，HRP在平均准确率上优于强基线，并在最高噪声率下保持竞争力。

Conclusion: 解耦可靠性估计和针对性传播策略能有效提升含噪标签学习性能。
...
### [98] [Hybrid Machine Learning Model for Forest Height Estimation from TanDEM-X and Landsat Data](https://arxiv.org/abs/2605.20997)
*Islam Mansour,Ronny Haensch,Irena Hajnsek,Konstantinos Papathanassiou*

Main category: cs.CV

TL;DR: 通过扩展特征空间，将光学Landsat数据与雷达相干性结合，提升了森林高度估算的精度，RMSE降低13.5%，MAE降低16.6%。


<details>
  <summary>Details</summary>
Motivation: 原有的混合模型无法完全解决高度/结构、基线/地形坡度模糊性问题，需要引入互补信息以提升物理一致性。

Method: 在原有TDM相干性+物理约束的混合模型基础上，加入光学Landsat特征（提供森林类型/结构信息），扩展特征空间后进行训练与反演。

Result: 在加蓬Lopé国家公园验证，RMSE降低13.5%，MAE降低16.6%，证实多光谱输入的附加价值。

Conclusion: 多源遥感数据（雷达+光学）的融合可有效解决单一数据源的歧义性，提高地球物理参数反演的准确度。
...
### [99] [DySink: Dynamic Frame Sinks for Autoregressive Long Video Generation](https://arxiv.org/abs/2605.21028)
*Bo Ye,Xinyu Cui,Jian Zhao,Tong Wei,Min-Ling Zhang*

Main category: cs.CV

TL;DR: 提出 DySink，一种基于检索的动态帧沉没机制，用视觉相关的历史帧替代静态早期帧，提升长视频生成的动态度和时间质量。


<details>
  <summary>Details</summary>
Motivation: 现有有界记忆流中固定缓存早期帧作为长程锚点，导致当前状态发散时上下文过时，甚至引发注意力塌陷和生成退化。

Method: 维护紧凑记忆库，通过自适应检索选择视觉相关历史帧作为动态沉没；结合沉没异常门检测跨头共识并抑制塌陷风险。

Result: 在分钟级长视频上，DySink 持续提升动态度指标，同时实现更高时间质量。

Conclusion: 动态帧沉没能有效替代静态锚点，增强长视频生成的上下文适应性和稳定性。
...
### [100] [GAMR: Geometric-Aware Manifold Regularization with Virtual Outlier Synthesis for Learning with Noisy Labels](https://arxiv.org/abs/2605.20727)
*Ningkang Peng,Jingyang Mao,Xiaoqian Peng,Peirong Ma,Xichen Yang,Weiguang Qu,Yanhui Gu*

Main category: cs.CV

TL;DR: 提出一种几何感知流形正则化范式，通过主动合成虚拟离群样本在数据流形间构建能量屏障，增强特征空间区分性以应对噪声标签。


<details>
  <summary>Details</summary>
Motivation: 现有方法被动过滤噪声样本，但噪声退化的特征空间难以区分困难样本与噪声样本，形成性能瓶颈。

Method: 主动合成虚拟离群样本构建几何约束，促进类内紧凑和类间分离，增强难样本与噪声样本的区分性。

Result: 在CIFAR-10等基准上超越当前最优方法，尤其在不平衡噪声条件下优势显著，且提升分布外检测能力。

Conclusion: 该范式可作为通用正则化机制集成到现有框架中，无需噪声先验，显著提升鲁棒性与可靠性。
...
### [101] [Deep Attention Reweighting: Post-Hoc Attention-Based Feature Aggregation in CNNs for Disentangling Core and Spurious Features under Spurious Correlations](https://arxiv.org/abs/2605.20732)
*Kin Whye Chew,Jingxian Wang*

Main category: cs.CV

TL;DR: 提出了一种名为深度注意力重加权（DAR）的后处理方法，通过自适应空间加权替代全局平均池化，解耦特征表示并抑制虚假关联。


<details>
  <summary>Details</summary>
Motivation: 标准后处理方法DFR因全局平均池化导致核心与虚假特征纠缠，难以同时增强核心特征与抑制虚假特征。

Method: 用可训练的注意力聚合模块替代GAP，与分类头联合重训练，实现空间位置的动态加权。

Result: 在多个数据集和指标上，DAR持续优于DFR，有效减少了虚假关联。

Conclusion: 基于注意力的特征聚合可缓解GAP的纠缠问题，显著降低模型对虚假特征的依赖。
...
### [102] [Early High-Frequency Injection for Geometry-Sensitive OOD Detection](https://arxiv.org/abs/2605.20728)
*Chuanjie Cheng,Ningkang Peng,Chenxi Liu,Yifan He,Peirong Ma,Yanhui Gu*

Main category: cs.CV

TL;DR: 本文通过频带分析发现，低频输入特征对OOD检测不利，而高频特征有助于分离ID/OOD。提出EIHF方法，在模型第一层卷积前增强高频信息，无需改变训练目标，即可改善基于几何的OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法依赖于训练后表示中的几何特性，但未深入分析输入频带对特征分离的影响。本文旨在探究频带效应并利用高频信息提升OOD检测。

Method: 首先使用band-wise MMD^2分析不同训练方法（CE、SimCLR、SupCon、PALM）下输入频带对ID/OOD特征差异的影响，然后提出EIHF（输入侧干预），在第一个卷积前增强高频证据。

Result: 在CIFAR-100和ImageNet-100上，EIHF在CIFAR-100上取得性能提升，在ImageNet-100上获得最佳平均FPR95和第二佳平均AUROC；但在场景为中心的Places转移上存在局限。

Conclusion: EIHF通过调整输入频带结构有效改善OOD检测的几何属性，但场景复杂度影响其效果，未来需考虑更鲁棒的频带处理策略。
...
### [103] [Sketch2MinSurf: Vision-Language Guided Generation of Editable Minimal Surfaces from Hand-Drawn Sketches](https://arxiv.org/abs/2605.20733)
*Wenda Wang,Anqi Liu,Junqi Yang,Lei He,Luying Wang,Jiachen Lu,Weixin Huang*

Main category: cs.CV

TL;DR: 提出Sketch2MinSurf，结合视觉语言与最小曲面理论，将手绘草图转换为可编辑的3D流形，拓扑相似度达0.844。


<details>
  <summary>Details</summary>
Motivation: 现有模型难以生成可直接用于设计的可编辑3D曲面，且拓扑一致性差。

Method: 空间-拓扑编码（节点坐标+骨架）、S2MS-Loss损失函数（几何+拓扑约束）。

Result: 测试集拓扑相似度0.844，优于基线；生成流形无伪影且可编辑；公共艺术装置验证实用性。

Conclusion: 方法有效生成符合人类意图的可编辑3D曲面，数据集与代码开源。
...
### [104] [Resolving Long-Tail Ambiguity in Unsupervised 3D Point Cloud Segmentation with Language Priors](https://arxiv.org/abs/2605.20737)
*Siqi Wei,Hongbin Xu,Feng Xiao,Tian Lan,Chun Li,Ming Li,Qiuxia Wu*

Main category: cs.CV

TL;DR: 提出了LangTail框架，利用语言模型的均衡世界知识缓解无监督3D点云分割中的长尾歧义问题。


<details>
  <summary>Details</summary>
Motivation: 现有纯视觉相似性聚类方法导致小类别特征被主导簇吸收，造成严重的不平衡预测。

Method: 构建实体级语义先验，通过对比对齐注入层次聚类框架，引导多粒度语义结构形成。

Result: 在ScanNet-v2、S3DIS和nuScenes上分别提升13.5、12.9和8.9 mIoU。

Conclusion: 语言先验能有效改善3D点云中少数类别的表征，显著提升分割性能。
...
### [105] [Lowering the Barrier to IREX Participation: Open-Source Algorithms, Toolkit, and Benchmarking for Iris Recognition](https://arxiv.org/abs/2605.20735)
*Siamul Karim Khan,Patrick J. Flynn,Adam Czajka*

Main category: cs.CV

TL;DR: 本文提出两种新的开源虹膜识别算法，并提供Python和IREX兼容的C++实现，首次评估开源方案在IREX协议下的性能，同时为其他团队参与IREX评估提供模型参考。


<details>
  <summary>Details</summary>
Motivation: 填补开源虹膜识别方法在官方IREX评估中的空白，并降低其他团队提交开源方法参与IREX的技术门槛。

Method: 采用两种神经网络：基于Triplet损失与Batch-Hard Triplet挖掘的TripletIris，以及基于ArcFace损失的ArcIris；同时提供两种现有方法的开源IREX兼容实现：基于人类显著性驱动滤波的HDBIF和用于检测Fuchs隐窝的CRYPTS，并公开虹膜分割和圆估计模型。

Result: 除CRYPTS因1:N搜索时间限制外，所有方法均通过官方IREX X评估，并在多个学术基准（如Quality-Face/Iris Research Ensemble等）上测试，取得良好性能。

Conclusion: 所提开源算法在IREX评估中表现有效，为虹膜识别研究提供了可复用的基准和实现框架。
...
TL;DR: 为解决小目标检测中解释方法模糊边界和特征扩散的问题，提出了结合基础模型SAM与双约束机制的XAI框架SAM-Sode，实现空间细化与形态重建，提升决策透明度。


<details>
  <summary>Details</summary>
Motivation: 传统解释方法在微小细菌检测中因目标特征稀疏和背景干扰导致解释模糊，缺乏形态学证据。

Method: 将初始特征图转化为几何感知提示，利用SAM先验进行空间细化与形态重建，引入物理意义和几何对齐的双重约束进行实例级去噪。

Result: 在自制数据集和公开数据集上有效抑制背景冗余，显著提升小目标检测的可解释性。

Conclusion: SAM-Sode框架能生成与专家直觉一致的解释，提高决策透明度。
...
### [107] [STAR-IOD: Scale-decoupled Topology Alignment with Pseudo-label Refinement for Remote Sensing Incremental Object Detection](https://arxiv.org/abs/2605.20738)
*Yaoteng Zhang,Qing Zhou,Junyu Gao,Qi Wang*

Main category: cs.CV

TL;DR: 提出了STAR-IOD框架，通过子空间解耦拓扑蒸馏和聚类伪标签生成器，解决遥感增量目标检测中的知识遗忘和标注缺失问题。


<details>
  <summary>Details</summary>
Motivation: 遥感图像数据流特性导致传统检测器在增量学习中遗忘旧知识，且现有方法忽视类内尺度变化和标注缺失问题。

Method: 引入子空间解耦拓扑蒸馏模块对齐拓扑关系，缓解尺度差异；使用聚类驱动伪标签生成器动态识别类别特定阈值，处理老类标注缺失。

Result: 在DIOR-IOD和DOTA-IOD上分别比SOTA方法提升1.7%和2.1%的mAP，有效缓解灾难性遗忘。

Conclusion: STAR-IOD能有效应对遥感增量检测中的知识遗忘和标注缺失问题，性能优越。
...
TL;DR: 提出了一个包含1.049亿图像-文本对的开放数据集MONET，通过多阶段清洗和重新标注提高质量，并验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 大规模文本到图像模型训练需要高质量、多样化的数据集，但现有数据集的收集、清洗和标注成本高，阻碍了开放和可重复的研究。

Method: 从2.9B原始数据中经过安全过滤、领域过滤、精确和近似去重、多视觉语言模型重新标注（短到长描述）、以及合成数据增强，生成MONET数据集，并附带预计算嵌入和注释。

Result: 在MONET上训练的4B参数潜在扩散模型在GenEval和DPG评分上达到竞争性能。
...
### [109] [Draw2Think: Harnessing Geometry Reasoning through Constraint Engine Interaction](https://arxiv.org/abs/2605.20743)
*Juncheng Hu,Jiawei Du,Xin Zhang,Joey Tianyi Zhou*

Main category: cs.CV

TL;DR: 提出Draw2Think框架，将几何推理从隐含空间推理转化为与GeoGebra约束引擎的代理交互，通过提议-绘制-验证循环实现可验证的几何推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在几何问题中推理过程不可验证，基于渲染像素或单次脚本的外部化方法无法提供精确的逐动作几何保证。

Method: 使用GeoGebra约束引擎作为可执行画布，在Propose-Draw-Verify循环中外部化假设、测量精确几何量，并将结构化观测反馈给模型。

Result: 在GeoGoal上通过95.9%谓词级和84.0%严格问题级构建检查，平面/立体基准上结果准确率提升最高4.1%/16.4%，在GenExam-math上达到68.2%/90.5%严格/宽松渲染得分。

Conclusion: Draw2Think通过约束检查画布实现可验证的几何推理，使构建保真度和度量忠实性可独立审计。
...
TL;DR: 提出 RePCM 方法，利用单帧心内膜舒张网格合成完整心动周期序列，结合区域感知和表型自适应机制，在多个疾病数据集上提升了几何与功能指标。


<details>
  <summary>Details</summary>
Motivation: 时序密集网格序列难以获取，传统全局生成模型易导致过度平滑，无法捕捉区域和疾病特异性运动差异。

Method: 两阶段框架：第一阶段通过重建网络学习顶点运动描述符并进行数据驱动分区；第二阶段使用区域特异性注入模块和表型自适应混合专家先验，实现局部动态保持与跨疾病变异建模。

Result: 在三个心血管疾病数据集上，几何与功能指标一致提升，区域特异动态保留效果更好。

Conclusion: RePCM 能有效从单帧恢复双心室心动周期运动，并保留区域和疾病特异性模式。
...
### [111] [SpineContextResUNet: A Computationally Efficient Residual UNet for Spine CT Segmentation](https://arxiv.org/abs/2605.20760)
*K S Nithurshen,Saurabh J. Shigwan*

Main category: cs.CV

TL;DR: 提出了一个高效的3D残差U-Net模型SpineContextResUNet，用于椎体分割，在低资源环境下保持了高准确度。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer或大规模集成的方法需要大量GPU资源，限制了在资源受限或边缘设备上的临床部署。

Method: 设计了一个轻量级的上下文块，采用并行的多扩张卷积捕获长距离依赖，避免了RNN的高延迟和自注意力的内存开销。

Result: 在VerSe2020和CTSpine1K数据集上分别获得88.17%和88.13%的Dice分数；在受限硬件上优于同规模的SwinUNETR，且避免了TotalSegmentator的内存溢出。

Conclusion: 该模型在低资源环境下实现了高精度推理，适合床边诊断和边缘平台部署。
...
### [112] [Diffuse to Detect: Bi-Level Sample Rebalancing with Pseudo-Label Diffusion for Point-Supervised Infrared Small-Target Detection](https://arxiv.org/abs/2605.20766)
*Zhu Liu,Yuanhang Yao,Ping Qian,Zihang Chen,Risheng Liu*

Main category: cs.CV

TL;DR: 本文提出一种基于热扩散物理模型的点监督红外小目标检测框架，通过物理引导的伪标签生成和双层双更新优化，解决了伪标签不稳定和样本不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测中密集标注成本高，现有弱监督点监督方法因低对比度图像中伪标签进化不稳定和样本分布失衡而性能受限。

Method: 利用热辐射与热扩散内在一致性设计物理诱导标注策略；构建双层双更新框架联合优化检测器权重、样本权重和扩散参数，包括元分类器动态调整损失权重和可微扩散模块更新伪标签。

Result: 在多个数据集上标注速度提升5倍，检测精度显著提高，仅用30%训练数据即可达到接近全监督性能。

Conclusion: 所提框架兼顾效率与精度，为红外小目标检测的弱监督学习提供实用解决方案。
...
### [113] [VIHD: Visual Intervention-based Hallucination Detection for Medical Visual Question Answering](https://arxiv.org/abs/2605.20772)
*Jiayi Chen,Benteng Ma,Zehui Liao,Winston Chong,Yasmeen George,Jianfei Cai*

Main category: cs.CV

TL;DR: 提出基于视觉干预的幻觉检测方法VIHD，通过选择性遮蔽视觉token来校准语义熵，在医疗VQA任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医疗多模态大模型产生的幻觉缺乏视觉证据，现有检测方法忽视跨模态依赖，需要更有效的检测机制。

Method: 通过视觉依赖探测定位视觉主导层，使用视觉干预解码遮蔽相关token，计算校准语义熵作为幻觉信号。

Result: 在三个医疗VQA基准和两个医疗MLLM上，VIHD一致优于最新方法，验证了细粒度视觉依赖的重要性。

Conclusion: VIHD有效检测医疗MLLM中的幻觉，强调了跨模态交互在检测中的关键作用。
...
TL;DR: 提出Deformba方法，使状态空间模型（SSM）在视觉任务中具备上下文自适应能力并支持多模态融合，在2D和3D视觉任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉SSM依赖固定扫描方法，预定义几何结构且复杂度高；同时SSM的因果性和自参照特性阻碍了需要查询交互的多模态融合（如多视图3D），限制其应用。

Method: 提出Deformba，一种上下文自适应方法，动态增强空间结构信息并保持SSM的线性复杂度，同时支持类似交叉注意力的多模态融合。

Result: 在图像分类、目标检测、分割等2D任务以及BEV感知等3D任务上均取得强性能。

Conclusion: Deformba有效克服了视觉SSM的局限，展现了通用性和有效性。
...
### [115] [AttriStory: Fine-grained Attribute Realization for Visual Storytelling with Diffusion Models](https://arxiv.org/abs/2605.20777)
*Manogna Sreenivas,Rohit Kumar,Soma Biswas*

Main category: cs.CV

TL;DR: AttriStory是一个支持视觉叙事中属性实现的基准，通过AttriLoss模块优化属性与对象的跨注意力对齐，无需修改模型架构即可提升细粒度属性一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然能保持角色在场景间一致，但无法系统性地确保服装颜色、纹理等细粒度属性的准确渲染。

Method: 构建200个多场景故事（10种艺术风格）的基准，提出插件式潜空间优化模块AttriLoss，在早期去噪步骤最大化属性-对象对的注意力对齐并抑制错误关联。

Result: 在所有基准模型上，AttriLoss均能持续改进属性实现的一致性，且与现有故事生成流程兼容。

Conclusion: 属性实现是视觉叙事中除角色一致性外的独立维度，AttriLoss可推动细粒度属性控制的叙事生成发展。
...
### [116] [Findings of the Counter Turing Test: AI-Generated Image Detection](https://arxiv.org/abs/2605.20787)
...
TL;DR: 通过Defactify 4.0研讨会上的Counter Turing Test，评估多种方法检测AI生成图像的性能，发现分类准确率较高但模型溯源仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 生成式AI技术的快速发展产生了逼真的合成视觉内容，带来了虚假信息和偏见内容等挑战，亟需有效的检测方法。

Method: 开发MS COCOAI数据集（包含5万张合成图像和真实图像），组织双任务竞赛：二分类（AI生成vs真实）和模型溯源，参与者使用CNN、Vision Transformer、频域分析、对比学习和多模态方法等。

Result: 二分类最高F1分数>0.83，模型溯源任务最高F1分数仅为0.4986。

Conclusion: AI生成图像可被高精度检测，但识别具体生成模型仍显著困难，需改进模型指纹识别和对抗鲁棒性。
...
TL;DR: 提出了一种秩感知多编码器框架，通过选择性融合最优编码器、解耦预测头和无监督域适应，在混合情感识别中取得第二名成绩。


<details>
  <summary>Details</summary>
Motivation: 混合情感识别因情感常表现为细微重叠的多模态线索而具有挑战性，现有方法难以有效融合互补表征。

Method: 秩感知多编码器框架：投影至共享空间→注意力门控估计编码器重要性→融合前n个最优编码器；解耦为存在度和显著度预测头；概率级融合；无标签特征级域适应。

Result: 在BlEmoRE挑战赛中优于强基线，最终系统排名第二。

Conclusion: 秩感知选择性融合对细粒度混合情感识别有效，验证了方法的鲁棒性。
...
TL;DR: 提出TempGlitch基准测试，发现现有VLM在检测时间性游戏漏洞方面性能接近随机水平。


<details>
  <summary>Details</summary>
Motivation: 现有VLM评测将游戏漏洞视为静态视觉异常，忽略了时间性漏洞与空间性漏洞的关键区别，导致评测不完整。

Method: 构建TempGlitch基准，包含五种时间性漏洞类型及配对无漏洞视频，通过多帧采样设置评估12种VLM。

Result: 当前VLM在TempGlitch上接近随机水平，且密集帧采样和大模型无法可靠解决其失败。

Conclusion: TempGlitch为时间推理、鲁棒游戏理解和自动化漏洞检测提供了聚焦测试平台。
...
### [119] [OlmoEarth v1.1: A more efficient family of OlmoEarth models](https://arxiv.org/abs/2605.20804)
...
TL;DR: 本文提出了对OlmoEarth模型的一系列改进，在训练和推理阶段显著降低了计算成本，同时保持了模型性能。


<details>
  <summary>Details</summary>
Motivation: 降低大规模地球观测模型在训练和推理过程中的计算资源消耗，提高效率。

Method: 通过优化模型架构和训练策略，减少了GPU训练时间和推理时的MACs。

Result: 在基准模型上训练GPU时间减少1.7倍，Sentinel-2任务推理MACs减少2.9倍，且性能保持不变。

Conclusion: 改进方法有效削减计算成本，且不牺牲性能，所有训练代码已开源。
...
### [120] [What Semantics Survive the Connector? Diagnosing VLM-to-DiT Alignment in Video Editing](https://arxiv.org/abs/2605.20795)
*Hangyu Lin,Chao Wen,Chengming Xu,Jianxiong Gao,Jiangning Zhang,Xiaobin Hu,Yanwei Fu*

Main category: cs.CV

TL;DR: 本文提出了TRACE-Edit诊断数据集和系统性评估方案，揭示了视频编辑模型中VLM到DiT的对齐过程会导致细粒度结构语义严重损失，推翻了无损语义转换的假设。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型依赖VLM与DiT的对齐进行指令驱动编辑，但假设对齐能无缝传递语义，本文旨在验证该对齐实际是否造成语义瓶颈。

Method: 构建基于视频合成的TRACE-Edit诊断数据集，设计包含元查询和连接器设计的诊断协议，对四种代表性模型进行对照评估。

Result: 对齐过程中细粒度结构语义显著退化，验证了VLM-to-DiT对齐是语义瓶颈的主要来源。

Conclusion: 推翻了现有范式中的无损语义假设，明确了VLM-to-DiT对齐作为细粒度语义丢失的关键瓶颈，为未来多模态对齐架构提供了诊断基础。
...
### [121] [Decomposing Subject-Driven Image Generation via Intermediate Structural Prediction](https://arxiv.org/abs/2605.20807)
*Hanzhong Guo,Yizhou Yu*

Main category: cs.CV

TL;DR: 提出一个两阶段框架，先预测Canny边缘图再渲染图像，以保留高频细节（如标志、图案和文本），并构建了10万对文本感知数据集。


<details>
  <summary>Details</summary>
Motivation: 现有主题驱动文本到图像生成方法在RGB空间直接操作，导致高频身份细节（如标志、图案和文本）在编辑时退化。

Method: 两阶段框架：第一阶段预测Canny边缘图分离结构，第二阶段结合源外观和预测结构渲染图像；同时构建10万对文本感知数据集以保证跨视图文本一致性。

Result: 基于GPT-4.1评估和知识蒸馏实验表明，该方法在保留高频细节方面优于基线，中间结构预测有效。

Conclusion: 中间结构预测是实现高保真主题驱动生成的有效途径，数据集和代码将公开。
...
### [122] [WikiVQABench: A Knowledge-Grounded Visual Question Answering Benchmark from Wikipedia and Wikidata](https://arxiv.org/abs/2605.21479)
*Basel Shbita,Pengyuan Li,Anna Lisa Gentile*

Main category: cs.CV

TL;DR: WikiVQABench是一个人工策划的知识型VQA基准，包含大量维基百科图像和选择题，要求模型结合外部知识与视觉信息才能正确回答。


<details>
  <summary>Details</summary>
Motivation: 现有VQA基准多侧重于仅从图像内容即可解决的感知任务，而现实场景常需外部知识，因此缺乏能评估模型知识推理能力的基准。

Method: 利用大语言模型生成候选图像-问题-答案集，再由人工审核确保事实正确性和视觉-文本一致性，最终构建多选择题数据集。

Result: 在15个视觉语言模型（参数从2.56亿到900亿）上的测试显示准确率范围从24.7%到75.6%，有效区分了模型的知识密集型推理能力。

Conclusion: WikiVQABench可作为评估模型在需要外部知识的情境下推理能力的有效基准，并公开数据与代码。
...
### [123] [Spatial Gram Alignment for Ultra-High-Resolution Image Synthesis](https://arxiv.org/abs/2605.20808)
*Jinjin Zhang,Xiefan Guo,Di Huang*

Main category: cs.CV

TL;DR: 提出Spatial Gram Alignment框架，通过自相似性对齐解决超高分辨率图像合成中预训练潜扩散模型的结构-细节冲突。


<details>
  <summary>Details</summary>
Motivation: 现有表示对齐方法在极端分辨率下存在可学习性-保真度冲突，直接蒸馏会扰动潜流形导致生成退化。

Method: 对齐生成特征与视觉基础模型(如SAM/DINO)的内部自相似性，构建非侵入式空间约束，保留原始生成目标函数。

Result: 在超高分辨率文本到图像合成任务中达到最优性能，有效平衡全局结构与局部细节。

Conclusion: SGA在保持预训练LDM生成能力的同时，通过宏观结构约束实现高分辨率下的高质量生成。
...
### [124] [OSGNet with MLLM Reranking @ Ego4D Episodic Memory Challenge 2026](https://arxiv.org/abs/2605.20818)
*Yisen Feng,Leigang Qu,Haoyu Zhang,Qiaohui Chu,Meng Liu,Xuemeng Song,Weili Guan,Liqiang Nie*

Main category: cs.CV

TL;DR: 本文提出了一种基于重排序的框架，结合多模态大语言模型和传统定位管道，在Ego4D挑战的两个赛道中均获得第一名。


<details>
  <summary>Details</summary>
Motivation: 现有定位管道在长视频中候选召回率较高但精度不足，而多模态大语言模型具有强大的视频-语言推理能力，但直接用于全视频定位效率低。

Method: 先使用OSGNet模型获取候选视频片段，再通过多模态大语言模型从候选中选择与查询最匹配的片段。

Result: 在自然语言查询和GoalStep两个赛道中均获得第一名。

Conclusion: 重排序框架能有效平衡效率和精度，提升定位性能。
...
### [125] [TERDNet: Transformer Encoder-Recurrent Decoder Network for Scene Change Detection](https://arxiv.org/abs/2605.20822)
*Jiae Yoon,Ue-Hwan Kim*

Main category: cs.CV

TL;DR: 提出了TERDNet，一种基于Transformer编码器和循环3门GRU解码器的场景变化检测网络，在四个公开数据集上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有SCD模型忽视了特征跨层的重要性，使用单步解码器限制了细化能力，且编码器预训练策略不明确。

Method: 采用Transformer编码器提取多级特征，特征融合模块整合相关体积，循环3门GRU解码器进行迭代细化，以及卷积-插值组合上采样器恢复高分辨率。

Result: 在四个基准上优于以往方法，消融实验验证了分割预训练和融合设计的有效性，鲁棒性测试显示了在真实机器人系统中的潜力。

Conclusion: TERDNet通过多级特征融合和循环解码显著提升了场景变化检测的准确性和细节。
...
### [126] [AIR: Amortized Image Reconstruction Framework for Self-Supervised Feed-Forward 2D Gaussian Splatting](https://arxiv.org/abs/2605.20820)
*Zhaojie Zeng,Yuesong Wang,Yawei Luo,Tao Guan*

Main category: cs.CV

TL;DR: 提出了AIR，一个自监督前馈框架，将迭代高斯拟合摊销为单次网络前向，消除了每幅图像的测试时优化。采用阶段式残差架构和显式阶段控制机制，仅在欠重建区域激活新基元。通过预测-优化-蒸馏策略稳定多阶段预测，并联合微调与图像自适应量化器。在Kodak和DIV2K数据集上，比代表性高斯基线方法重建质量更好，编码时间降至160-300毫秒。


<details>
  <summary>Details</summary>
Motivation: 现有的2D高斯表示方法需要昂贵的逐图像迭代优化或依赖手工先验进行基元分配，效率低且泛化性受限。

Method: 阶段式残差架构、显式阶段控制机制、预测-优化-蒸馏训练策略、图像自适应量化器。

Result: 在Kodak和DIV2K数据集上，重建质量优于代表性高斯基线方法，编码时间降至160-300毫秒。

Conclusion: AIR实现了高效、高质量的自监督2D高斯图像重建，消除了测试时优化，具有实用潜力。
...
### [127] [Map-Mono-Ego: Map-Grounded Global Human Pose Estimation from Monocular Egocentric Video](https://arxiv.org/abs/2605.20889)
*Hiroyuki Deguchi,Ryosuke Hori,Kotaro Amaya,Tsubasa Maruyama,Mitsunori Tada,Hideo Saito*

Main category: cs.CV

TL;DR: 提出MapMonoEgo框架，仅利用单目相机和预扫描3D点云实现全局一致的人体姿态估计，并引入AIST-Living数据集验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有单目人体姿态估计方法无法准确获取用户在环境中的绝对位置，且存在尺度模糊导致的平移漂移问题。

Method: 结合预扫描的3D点云信息，从单目视频中恢复全局人体姿态。

Result: 在AIST-Living数据集上显著优于现有基线方法，无需专用多传感器硬件。

Conclusion: MapMonoEgo能有效实现实用化的单目穿戴式人体姿态估计，适用于长期活动监测。
...
### [128] [HDMoE: A Hierarchical Decoupling-Fusion Mixture-of-Experts Framework for Multimodal Cancer Survival Prediction](https://arxiv.org/abs/2605.20891)
*Huayi Wang,Haochao Ying,Yuyang Xu,Qiyao Zheng,jun wang,Cheng Zhang,Ying Sun,Jian Wu*

Main category: cs.CV

TL;DR: 提出层级解耦融合混合专家框架，通过双层MoE和随机特征重组模块实现多模态生存预测，在肝癌症和TCGA数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有特征解耦融合方法未能有效减少模态冗余信息，且缺乏细粒度关系建模能力。

Method: 设计双层MoE结构：第一层去除冗余并提取模态内细粒度特征；第二层实现模态间细粒度解耦；并引入RFR模块实现特征精细融合。

Result: 在私有肝癌数据集和三个TCGA公开数据集上验证了方法的有效性。

Conclusion: HDMoE框架通过层级解耦与细粒度融合有效提升了多模态生存预测性能。
...
### [129] [RelWitness: Open-Vocabulary 3D Scene Graph Generation with Visual-Geometric Relation Witnesses](https://arxiv.org/abs/2605.20823)
*Minh Anh Nguyen,Quang Huy Tran,Bao Ngoc Le,Tuan Kiet Pham,Sui Yang Guang*

Main category: cs.CV

TL;DR: 提出了RelWitness框架，利用视觉-几何线索作为关系见证，从未完全标注的RGB-D序列中生成开放词汇的3D场景图。


<details>
  <summary>Details</summary>
Motivation: 开放词汇3D场景图生成不仅面临词汇扩展困难，更因关系标注选择性强、大量有效关系未被标注，导致监督信号不可靠。

Method: 构建多种关系见证记录（如接触、包含、邻近等），通过见证验证器区分未标注候选为缺失正例、可靠负例或不明确案例，并采用见证引导的正-无标签目标函数进行学习，结合见证一致解码和缺失关系审计协议。

Result: 在3DSSG/3RScan和ScanNet衍生数据集上的模拟实验表明，该方法提升了未见关系识别、见证精度，降低了幻觉和冗余关系短语，但数值尚待复现验证。

Conclusion: 利用视觉-几何关系见证能够有效从不完全标注中学习开放词汇的3D场景图，改善关系识别可靠性。
...
### [130] [Activation-Free Backbones for Image Recognition: Polynomial Alternatives within MetaFormer-Style Vision Models](https://arxiv.org/abs/2605.20839)
*Jeffrey Wang,Jonathan Gregory,Grigorios G. Chrysos*

Main category: cs.CV

TL;DR: 本文证明在MetaFormer视觉骨干网络中，无需激活函数（如ReLU、GELU）和指数软最大值，仅使用多项式替代（通过Hadamard积）即可实现同等或更优性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉骨干依赖点激活和softmax提供非线性，但本文旨在探索在MetaFormer框架中是否可省略这些非线性成分。

Method: 设计了无激活的多项式替代方案，针对MLP、卷积和注意力三种核心原语，通过Hadamard积替代标准非线性，生成输入的多项式函数。

Result: PolyNeXt模型在ImageNet分类、ADE20K语义分割和分布外鲁棒性上匹配或超越含激活的对应模型，并以更低计算成本显著优于先前多项式网络。

Conclusion: 标准模块的多项式变体可超越复杂定制架构，表明激活函数在MetaFormer框架中并非必需。
...
### [131] [HyDAR-Pano3D: A Hybrid Disentangled Anatomical Recovery Framework for Panoramic-to-3D Reconstruction](https://arxiv.org/abs/2605.20827)
*Yaoyao Yue,Jérôme Schmid,Xiaoshuang Li,Eduardo Delamare,Jinman Kim*

Main category: cs.CV

TL;DR: 提出HyDAR-Pano3D两阶段框架，将全景X光片到CBCT的重建分解为解剖恢复问题，在三个大规模数据集上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法直接回归CBCT体素导致模糊重建，无法区分共有解剖结构与个体形态变异。

Method: 两阶段框架：第一阶段双编码器网络融合放射特征与SAM语义先验重建标准解剖空间体积；第二阶段解剖恢复网络预测变形场映射至原生空间。

Result: PSNR 25.76 dB, SSIM 85.70%, 整体解剖Dice 83.83%，下游分割牙齿Dice 82.4%、下颌管Dice 72.2%。

Conclusion: 解耦方法有效保留临床相关解剖结构，可在无CBCT时实现鲁棒的解剖感知评估。
...
### [132] [FruitEnsemble: MLLM-Guided Arbitration for Heterogeneous ensemble in Fine-Grained Fruit Recognition](https://arxiv.org/abs/2605.20892)
*Enhui Yu,Junhui Li,Ruitong Lu,Jialu Li,Youshan Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种基于两阶段动态推理框架的细粒度水果分类方法，构建了包含306类、116,233样本的数据集，通过集成学习与多模态大模型验证实现了70.49%的准确率。


<details>
  <summary>Details</summary>
Motivation: 细粒度水果分类面临高质量数据集稀缺和类间视觉相似性高的挑战，现有静态单模型架构泛化能力有限。

Method: 提出FruitEnsemble框架，第一阶段用验证校准加权集成异构骨干网络生成Top-3候选池；第二阶段当置信度低于0.6时，触发多模态大模型结合外部植物描述进行思维链推理验证，并采用难样本感知联合损失优化训练。

Result: 在自建数据集上达到70.49%的分类准确率，优于现有最佳方法。

Conclusion: 该框架为农业视觉分选和质量检测提供了高效、可部署的解决方案。
...
### [133] [JFAA: Technical Report for the EPIC-KITCHENS-100 Action Anticipation Challenge at EgoVis 2026](https://arxiv.org/abs/2605.20904)
*Qiaohui Chu,Haoyu Zhang,Yisen Feng,Meng Liu,Weili Guan,Dongmei Jiang,Liqiang Nie*

Main category: cs.CV

TL;DR: 提出JFAA方法，利用JEPA的表示学习和未来预测能力，在EPIC-KITCHENS-100动作预期任务中取得第一名。


<details>
  <summary>Details</summary>
Motivation: 提升动作预期任务的性能，利用V-JEPA的潜在特征预测能力。

Method: 冻结编码器和预测器提取特征，轻量级注意力探针预测动作标签，场感知集成提升鲁棒性。

Result: 在EgoVis 2026挑战赛中取得第一名。

Conclusion: JFAA在动作预期任务上有效且鲁棒。
...
### [134] [FlowLong: Inference-time Long Video Generation via Manifold-constrained Tweedie Matching](https://arxiv.org/abs/2605.20910)
*Jangho Park,Geon Yeong Park,Gihyun Kwon,Jong Chul Ye*

Main category: cs.CV

TL;DR: 提出一种无需训练的推理方法，通过重叠滑动窗口结合Tweedie匹配和随机早期采样，实现架构无关的长视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有免训练方法（双向扩展和自回归）在长期生成中存在质量退化或漂移误差问题。

Method: 重叠滑动窗口生成，Tweedie匹配保证重叠区域流形约束和时间一致性，随机早期采样同步轨迹并保持视觉保真度。

Result: 生成长度超出原生窗口数倍，时间一致性和视觉质量优于基线方法，并拓展至音频-视频联合生成和文本到3DGS。

Conclusion: 该方法简单有效，无需额外训练，适用于多种生成模型，具备良好泛化性。
...
### [135] [RISE: Reliable Improvement in Self-Evolving Vision-Language Models](https://arxiv.org/abs/2605.20914)
*Chaoran Xu,Yingmao Miao,Pengfei Zhang,Hao Dou,Lei Sun,Xiangxiang Chu*

Main category: cs.CV

TL;DR: 提出RISE框架，通过细粒度角色交替、质量监督与技能感知动态平衡三机制，实现视觉语言模型的可靠自演化学习。


<details>
  <summary>Details</summary>
Motivation: 现有VLM自演化方法存在角色交替粗粒度导致反馈延迟、生成问题质量退化、问题类型分布塌缩三大挑战。

Method: RISE框架包含：细粒度角色交替缩短问答反馈循环；质量监督器提升问题有效性与伪标签可靠性；技能感知动态平衡防止模式塌缩。

Result: 在七项基准测试中，RISE在两个VLM主干网络上均持续提升模型性能。

Conclusion: RISE通过三组件协同，从无标注图像中实现更可靠有效的模型自演化。
...
### [136] [SynCB: A Synergy Concept-Based Model with Dynamic Routing Between Concepts and Complementary Neural Branches](https://arxiv.org/abs/2605.20908)
*Tores Julie,Sun Rémy,Sassatelli Lucile,Ancarani Elisa,Wu Hui-Yin,Precioso Frédéric*

Main category: cs.CV

TL;DR: 本文提出SynCB框架，结合概念分支与神经分支，并通过路由模块动态选择分支，在保持对人工干预响应性的同时提升任务准确率。


<details>
  <summary>Details</summary>
Motivation: 现有概念模型在干预时牺牲准确率，而标准神经网络缺乏透明度，需要一种既能利用概念可解释性又能保持高性能的混合模型。

Method: SynCB框架包含一个概念分支、一个互补神经分支，以及一个可训练的路由模块；通过联合学习和测试时干预策略实现分支协调。

Result: 在五个数据集上，SynCB在任务准确率上超过纯神经网络最多3.9个百分点，在干预性能上超过最强基线最多6.43个百分点。

Conclusion: SynCB通过保持分支独立并动态协调，成功在可解释性、干预响应性和任务性能之间取得平衡。
...
### [137] [DrawMotion: Generating 3D Human Motions by Freehand Drawing](https://arxiv.org/abs/2605.20955)
*Tao Wang,Lei Jin,Zhihua Wu,Qiaozhi He,Jiaming Chu,Yu Cheng,Junliang Xing,Jian Zhao,Shuicheng Yan,Li Wang*

Main category: cs.CV

TL;DR: 提出DrawMotion框架，通过手绘草图条件实现文本到动作生成，降低用户时间约46.7%。


<details>
  <summary>Details</summary>
Motivation: 用户难以通过文本精确描述动作意图。

Method: 基于扩散模型，引入手绘条件（自动生成火柴人草图）、多条件融合模块（MCM）及无训练引导。

Result: 定量实验和用户研究表明手绘方法在生成符合想象的动作时减少用户时间约46.7%。

Conclusion: DrawMotion能有效结合文本和手绘条件，提高动作生成的效率和准确性。
...
### [138] [3D Reconstruction and Knowledge Distillation to Improve Multi-View Image Models to Explore Spike Volume Estimation in Wheat](https://arxiv.org/abs/2605.20940)
*Olivia Zumsteg,Jannis Widmer,Yann Bourdé,Norbert Kirchgessner,Andreas Hund,Lukas Roth,Paraskevi Nousi*

Main category: cs.CV

TL;DR: 提出一种混合2D-3D知识蒸馏方法，实现仅用2D图像高效估算麦穗体积。


<details>
  <summary>Details</summary>
Motivation: 现有3D方法对环境敏感或计算昂贵，2D图像方法缺乏几何信息，需结合两者优势。

Method: 训练刚性不变点云网络获得几何特征，结合多视图图像Transformer集成模型，通过知识蒸馏将3D知识迁移到纯图像学生模型。

Result: 蒸馏后MAE从654.31mm³降至639.93/644.62mm³，相关系数从0.76提升至0.77/0.82，推理速度从160ms降至1.4ms/穗。

Conclusion: 3D知识蒸馏可有效提升2D Transformer的体积估算精度与效率，适用于高通量表型分析。
...
### [139] [PaintCopilot: Modeling Painting as Autonomous Artistic Continuation](https://arxiv.org/abs/2605.20941)
*Yunge Wen,Yuancheng Shen,Paul Pu Liang*

Main category: cs.CV

TL;DR: PaintCopilot是一个协作式神经绘画助手，通过自回归生成笔画序列模拟艺术创作行为，无需目标图像。


<details>
  <summary>Details</summary>
Motivation: 现有神经绘画方法依赖像素重建参考图像，缺乏对开放、动态创作过程的建模。

Method: 结合ViT目标预测器、自回归笔画预测器（流匹配）和VAE区域采样器，基于三种可微画笔表示实现四种交互工作流。

Result: 专业艺术家案例表明，PaintCopilot支持流畅的协作绘画，实现艺术家与AI交替控制创作过程。

Conclusion: PaintCopilot通过生成式方法有效促进了人机协同的开放式绘画创作。
...
### [140] [Bridging Structure and Language: Graph-Based Visual Reasoning for Autonomous Road Understanding](https://arxiv.org/abs/2605.20942)
*Lena Wild,Katie Z Luo,Marco Pavone*

Main category: cs.CV

TL;DR: 提出Combined Road Substrate (CRS)框架，通过图结构表示将几何道路与开放语义结合，用少量数据训练小模型即可显著提升结构化道路推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有关键技术（VLMs缺乏几何关系、传统模块语义刚性）无法同时支持精准结构化推理与语义灵活性。

Method: CRS框架：图结构表示道路拓扑与语义，通过递归图查询自动生成问答对，包含'免费接地'机制确保逻辑可溯，以及链式思维监督。

Result: 小模型（2-4B参数）仅需20-80个CRS增强场景即可在组合推理任务中稳定提升，且失败模式从关系理解缺陷转为属性识别缺陷。

Conclusion: 结构化道路推理的瓶颈不在模型规模，而在于缺乏结构化监督数据。
...
### [141] [Towards UAV Detection in the Real World: A New Multispectral Dataset UAVNet-MS and a New Method](https://arxiv.org/abs/2605.20963)
...
TL;DR: 提出了首个用于细粒度小无人机检测的多光谱数据集UAVNet-MS和双流基线模型MFDNet，验证了光谱线索相对于空间线索的互补优势。


<details>
  <summary>Details</summary>
Motivation: 现有RGB系统在小尺度、高类间相似性和低对比度下检测能力不足，而多光谱成像的材质感知能力未被充分探索，且缺乏专用数据集。

Method: 构建UAVNet-MS数据集（15,618组RGB-MSI数据立方体），提出MFDNet双流网络以处理阵列视差和空间-光谱融合。

Result: 在RGB、MSI及RGB+MSI协议下测试20种检测器，MFDNet的AP50比最佳RGB方法提升6.2%。

Conclusion: 光谱线索为小无人机检测提供互补于空间线索的材质证据，该数据集与基线为多光谱无人机监测研究奠定基础。
...
### [142] [Towards Integrated Rock Support Visualisation in 3D Point Cloud of Underground Mines](https://arxiv.org/abs/2605.20973)
*Dibyayan Patra,Simit Raval,Pasindu Ranasinghe,Bikram Banerjee,Ismet Canbulat*

Main category: cs.CV

TL;DR: 提出了一种基于3D点云的自动化框架，集成了结构映射、锚杆识别、不连续面拟合和方位估计，用于地下矿山岩体支护效果的可视化评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法将不连续面特征与锚杆分析分开处理，限制了支护效果的综合评估，且依赖人工测量。

Method: 利用3D点云数据，通过结构映射、锚杆识别、平面拟合和方向估计的统一工作流，生成不连续面与锚杆向量的集成可视化，并进行立体网分析和安装质量指标计算。

Result: 在真实地下金属矿扫描数据上实现了高精度结构映射和锚杆识别，在中尺度点云中验证了方法有效性。

Conclusion: 该框架为自动化的岩体支护效果综合评估提供了实用方法，无需人工测量或额外数据采集。
...
### [143] [Preserve, Reveal, Expand: Faithful 4D Video Editing with Region-Aware Conditioning](https://arxiv.org/abs/2605.20961)
*Zhangchi Hu,Wenzhang Sun,Xiangchen Yin,Jiahui Yuan,Chunfeng Wang,Hao Li,Kun Zhan,Xiaoyan Sun*

Main category: cs.CV

TL;DR: 本文提出PREX框架，通过区域感知分解目标4D时空体为保留、揭示和扩展三个角色，结合伪任务训练的区域适配器，在保持视觉质量的同时减少4D编辑中的区域结构失败。


<details>
  <summary>Details</summary>
Motivation: 现有4D视频扩散模型主要关注合理生成，但在忠实编辑时，源观测区域与未观测区域（如遮挡或视角外内容）的混合条件信号会导致保存漂移、重影和不稳定外推。

Method: 提出PREX框架：1) 根据观测支持和场景范围将目标时空体分解为Preserve/Reveal/Expand三个角色；2) 构建带校准置信度的观测外观线索；3) 通过区域感知适配器注入冻结的视频扩散骨干网络，使用伪任务训练（无需成对编辑视频）。

Result: 在提出的PREBench基准上，PREX减少了区域结构失败，同时保持了强视觉质量和4D编辑控制能力。

Conclusion: 区域感知分解和伪任务训练能有效解决4D编辑中观测信息与生成内容的混淆问题，为忠实4D编辑提供新思路。
...
### [144] [CHOIR: Contact-aware 4D Hand-Object Interaction Reconstruction](https://arxiv.org/abs/2605.20992)
*Hao Xu,Yilin Liu,Yinqiao Wang,Chi-Wing Fu,Niloy J. Mitra*

Main category: cs.CV

TL;DR: CHOIR通过接触信号耦合手和物体，从单目视频重建4D手物交互，包括手部运动、物体形状和6D姿态以及接触位置。


<details>
  <summary>Details</summary>
Motivation: 现有方法在单目视频中重建手物交互时，常因未知物体、遮挡和杂乱背景导致手和物体对齐不准确。

Method: 先使用开放世界视觉先验初始化粗4D序列，然后通过生成式空间修正模块预测射线深度修正并调整手物相对位置，最后利用动态更新的接触约束进行联合优化。

Result: 在受控和挑战性视频上，CHOIR在物体重建、物理合理性和时间一致性上优于现有方法。

Conclusion: 接触信号是连接手和物体的有效耦合信号，能显著提升单目视频中手物交互重建的精度和鲁棒性。
...
### [145] [Dynamic Video Generation: Shaping Video Generation Across Time and Space](https://arxiv.org/abs/2605.21042)
*Shikang Zheng,Jingkai Huang,Jiacheng Liu,Guantao Chen,Lixuan,Yuqi Lin,Peiliang Cai,Linfeng Zhang*

Main category: cs.CV

TL;DR: 提出DVG框架，通过动态时空联合分配计算，实现视频生成中近无损加速，最高可达7倍加速，与蒸馏结合可达18倍。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成中迭代去噪过程计算昂贵，现有渐进分辨率采样在视频领域因时空维度复杂，加速有限或质量下降。

Method: DVG动态视频生成框架，自动选择内容感知的加速策略，联合分配时空计算，无需手动调参或重训练。

Result: 在HunyuanVideo和HunyuanVideo-1.5上实现最高7倍加速，结合蒸馏达18倍加速，且近无损。

Conclusion: DVG可作为大规模高效视频生成系统的关键组件，实现高效加速。
...
### [146] [Multimodal LLMs under Pairwise Modalities](https://arxiv.org/abs/2605.21059)
*Yan Li,Yunlong Deng,Yuewen Sun,Gongxu Luo,Kun Zhang,Guangyi Chen*

Main category: cs.CV

TL;DR: 提出了一个仅利用配对模态数据训练MLLM的框架，通过理论分析和两阶段对齐实现跨模态表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM训练依赖多模态联合数据，人工成本高且可扩展性差。

Method: 两阶段框架：第一阶段通过自重建和部分对齐的对比学习学习共享潜空间；第二阶段用新模态编码器与预训练解码器重组实现跨模态迁移。

Result: 在3D点云和触觉模态对实验中，模型在跨模态任务上表现优异。

Conclusion: 仅用配对数据就能学习到可识别的跨模态表示，有效降低数据需求。
...
### [147] [DAMA: Disentangled Body-Anchored Gaussians for Controllable Multi-Layered Avatars](https://arxiv.org/abs/2605.21001)
*Daniel Eskandar,Berna Kabadayi,Garvita Tiwari,Gerard Pons-Moll*

Main category: cs.CV

TL;DR: 提出DAMA方法，通过基于SMPL-X的面片绑定和高斯参数化，实现多视角图像中具有物理合理分层和可控服装顺序的3D穿衣人体重建，在4D-DRESS数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略几何结构和物理合理性，导致服装边界模糊、无法控制堆叠和图层顺序。

Method: 将高斯函数绑定到SMPL-X面片，使用重心平面坐标和正法线偏移；通过2D分割提升为身体锚定高斯，用拓扑引导校正优化层次，联合优化几何和外观。

Result: 在4D-DRESS数据集（82个扫描）上，几何重建、服装分离、穿透率和穿透深度方面达到最先进性能。

Conclusion: DAMA首次实现从多视图图像进行高斯穿衣人体重建，具有物理合理分层、干净服装分离和显式堆叠控制，并支持用户自定义服装重排和快速转换为可模拟网格。
...
### [148] [Q-ARVD: Quantizing Autoregressive Video Diffusion Models](https://arxiv.org/abs/2605.21072)
*Siao Tang,Xinyin Ma,Gongfan Fang,Xingyi Yang,Xinchao Wang*

Main category: cs.CV

TL;DR: 提出了Q-ARVD框架，通过帧加权和离群值自适应双尺度量化，解决了自回归视频扩散模型量化中的帧敏感度不平衡和权重离群问题。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型推理成本高，量化是提升效率的关键，但现有量化方法直接应用于ARVDs效果不佳。

Method: Q-ARVD包含两个策略：(1) 最终质量感知的帧加权机制 (2) 离群值自适应双尺度量化。

Result: 实验表明Q-ARVD在量化ARVDs时显著优于现有方案。

Conclusion: Q-ARVD有效解决了ARVDs量化的关键挑战，为高效视频生成提供了可行方案。
...
### [149] [Towards Physically Consistent 4D Scene Reconstruction for Closed-loop Autonomous Driving Simulation](https://arxiv.org/abs/2605.21032)
*Bowyn Tan,Yutong Xie,Bai Huang,Fan Luo,Xiao Li,Naizheng Wang,Yang Guan,Shengbo Eben Li*

Main category: cs.CV

TL;DR: 提出正交投影梯度（OPG）分层训练方法和时域正则化策略，解决4D高斯泼溅中时空参数耦合导致的低秩混淆和方差发散问题，实现高保真街景重建，兼顾新视角合成与时变建模。


<details>
  <summary>Details</summary>
Motivation: 现有4D高斯泼溅方法在街景重建中无法同时实现高质量新视角合成与时变建模，原因在于单源观测中视角与时间的确定性耦合导致时空参数间存在零空间歧义，时间信息压制空间线索使空间参数估计方差发散。

Method: OPG分层训练：优先固定空间参数确保完整性，再将时间更新限制在空间零空间内，实现主动信用分配；时域正则化：基于外观一致性演化的物理先验施加平滑约束，优化时间解空间。

Result: 在保持稳定新视角合成能力的同时，在传统观测重现指标上表现更优，间接证明了时变建模能力的提升。

Conclusion: 所提方法有效解耦了时空参数的信用分配困境，在闭环仿真中实现了物理一致的街景重建，同时支持高保真新视角合成与时变信息建模。
...
### [150] [SpectralEarth-FM: Bringing Hyperspectral Imagery into Multimodal Earth Observation Pretraining](https://arxiv.org/abs/2605.21075)
*Nassim Ait Ali Braham,Aaron Banze,Conrad M. Albrecht,Julien Mairal,Jocelyn Chanussot,Xiao Xiang Zhu*

Main category: cs.CV

TL;DR: 提出分层Transformer模型SpectralEarth-FM，联合预训练高光谱与多传感器数据，构建超大规模数据集SpectralEarth-MM，在多个任务上达到最优。


<details>
  <summary>Details</summary>
Motivation: 现有高光谱基础模型仅用高光谱数据训练，缺乏与其他遥感传感器的联合预训练和融合。

Method: 采用分层Transformer架构，包含谱分词器、传感器专用编码器、跨传感器融合模块和共享分层编码器，使用JEPA-style目标进行预训练。

Result: 在高光谱下游任务和标准EO基准上均达到最先进结果。

Conclusion: 多传感器联合预训练可有效提升高光谱遥感性能，SpectralEarth-FM为融合异构传感器数据提供了新范式。
...
### [151] [R2AoP: Reliable and Robust Angle of Progression Estimation from Intrapartum Ultrasound](https://arxiv.org/abs/2605.21099)
*Yuanhan Wang,Yifei Chen,Beining Wu,Mingxuan Liu,Xiaotian Hu,Chunbo Jiang,Yijin Li,Changmiao Wang,Feiwei Qin,Qiyuan Tian*

Main category: cs.CV

TL;DR: 提出R2AoP框架，通过结构增强分割和置信度引导几何建模，实现产时超声中前进角（AoP）的可靠估计。


<details>
  <summary>Details</summary>
Motivation: AoP估计对噪声、边界模糊及分割误差敏感，现有方法准确性不足。

Method: 三分支局部结构增强骨干网络、置信度加权轮廓拟合、轻量几何可靠测试时自适应。

Result: 多中心基准测试中，AoP误差和边界指标一致优于现有方法。

Conclusion: R2AoP能稳定减少噪声和边界模糊影响，提升AoP测量可靠性。
...
### [152] [TextSculptor: Training and Benchmarking Scene Text Editing](https://arxiv.org/abs/2605.21090)
...
TL;DR: 提出了TextSculptor框架，包含自动化数据构建管道、大规模数据集TextSculpt-Data和基准TextSculpt-Bench，显著提升了开放场景文本编辑性能。


<details>
  <summary>Details</summary>
Motivation: 场景文本编辑中开放模型落后于闭源模型，主要受限于高质量训练数据稀缺和缺乏标准化基准。

Method: 开发了结合文本感知图像合成与程序化文本渲染的自动数据构建管道，生成了包含320万训练样本的数据集，并设计了覆盖四种文本编辑任务的基准及评估协议。

Result: TextSculptor在文本编辑任务上显著提升了开放模型性能，缩小了与专有模型的差距。

Conclusion: 通过大规模高质量数据和标准化基准，有效推动了场景文本编辑的发展，开源了数据和基准。
...
### [153] [VDFP: Video Deflickering with Flicker-banding Priors](https://arxiv.org/abs/2605.21079)
*Zhiyi Zhou,Libo Zhu,Zihan Zhou,Yulun Zhang,Xiaokang Yang*

Main category: cs.CV

TL;DR: 本文提出了一种基于感知引导的视频去闪烁框架VDFP，并构建了真实数据集DeViD，有效消除智能手机拍摄屏幕时产生的复杂带状伪影，同时保持高保真空间细节和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 智能手机拍摄屏幕时因硬件同步不匹配导致的严重带状伪影问题，现有视频修复方法难以处理这种结构化的周期亮度波动，常残留伪影或过度平滑纹理。

Method: 提出了VDFP框架，包括基于卷帘快门的退化场建模（DFM）以合成复杂多带场景，以及空间-时间连续先验感知（CPP）通过闪烁感知均方误差（FA-MSE）优化，并通过零初始化输入层保留预训练生成先验。

Result: 大量实验表明VDFP在消除复杂带状伪影、保持高保真空间细节和时间一致性方面显著优于其他方法。

Conclusion: VDFP能有效消除手机拍摄屏幕时的带状伪影，并保持高质量视频输出。
...
### [154] [RCGDet3D: Rethinking 4D Radar-Camera Fusion-based 3D Object Detection with Enhanced Radar Feature Encoding](https://arxiv.org/abs/2605.21112)
*Weiyi Xiong,Bing Zhu*

Main category: cs.CV

TL;DR: 提出基于雷达特征增强的实时3D检测方法RCGDet3D，通过射线对齐高斯编码和语义注入模块，在保持实时性能的同时优于现有融合方法。


<details>
  <summary>Details</summary>
Motivation: 现有4D雷达-相机融合方法计算密集、牺牲速度换取精度，而雷达点云特征提取潜力未被充分挖掘。

Method: 采用射线对齐点高斯编码器（R-PGE）统一坐标系统，并设计语义注入（SI）模块融合图像特征。

Result: 在VoD和TJ4DRadSet数据集上，精度和速度均超越现有方法，实现实时部署新标杆。

Conclusion: 强化雷达特征提取可替代复杂融合策略，兼顾高精度与实时性。
...
### [155] [VersusQ: Pairwise Margin Reasoning for Generalizable Video Quality Assessment](https://arxiv.org/abs/2605.21130)
*Shibei Meng,Binxin Yang,Yuan Liu,Jiexuan Zhang,Zhengyao Lv,Hubery Yin,Qiang Xu*

Main category: cs.CV

TL;DR: 提出基于成对比较的框架VersusQ，通过相对比较消除绝对评分偏差，实现视频质量评估的跨域泛化。


<details>
  <summary>Details</summary>
Motivation: 现有LMMs的视频质量评估方法采用绝对评分，受数据集特定校准影响，跨域迁移性差。

Method: 采用LMM进行两两视频比较，推理视觉和时间质量差异，预测连续符号边缘；提出Margin-Coupled GRPO联合优化推理与回归。

Result: 在多个公开基准上达到最优性能，具备强跨域泛化能力和细粒度排序能力。

Conclusion: 相对比较可有效缓解绝对尺度偏差，VersusQ是高效且鲁棒的视频质量评估方法。
...
### [156] [UniT: Unified Geometry Learning with Group Autoregressive Transformer](https://arxiv.org/abs/2605.21131)
*Haotian Wang,Yusong Huang,Zhaonian Kuang,Hongliang Lu,Xinhu Zheng,Meng Yang,Gang Hua*

Main category: cs.CV

TL;DR: UniT是一个统一模型，基于组自回归变压器，将在线感知、离线重建、多模态集成、长期可扩展性和公制尺度估计整合到单一框架中，并在十个基准测试中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈模型在几何感知能力上分散且不兼容，缺乏统一框架整合在线、离线等多范式。

Method: 提出组自回归变压器，以传感器观测组为基本自回归单元，预测点图；采用队列式KV缓存机制和尺度自适应几何损失。

Result: 在七个代表性任务的十个基准测试中取得最先进性能。

Conclusion: UniT有效统一了多种几何感知能力，具有优越的泛化性能。
...
### [157] [Linear-DPO: Linear Direct Preference Optimization for Diffusion and Flow-Matching Generative Models](https://arxiv.org/abs/2605.21123)
*Kesong Li,Yixuan Xu,Kuo-kun Tseng,Weiyi Lu,Kan Liu,Tao Lan*

Main category: cs.CV

TL;DR: 提出Linear-DPO方法，通过统一扩散与流匹配的反向时间SDE框架，用线性效用函数替代sigmoid，引入EMA参考模型，改进了文本到图像生成的DPO对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有DPO在文本到图像生成中面临两个问题：局限于扩散模型而忽视流匹配；离散NLP的DPO目标不适用于回归型生成任务。

Method: 推导统一反向时间SDE框架下的广义DPO目标，从梯度角度指出标准DPO次优，提出Linear-DPO：持续线性效用函数+EMA更新参考模型。

Result: 在扩散模型(SD1.5, SDXL)和流匹配模型(SD3-Medium)上，定性和定量实验均优于现有基线。

Conclusion: Linear-DPO有效解决了DPO在文本到图像生成中的目标失配问题，适用于多种生成模型。
...
### [158] [ROAR-3D: Routing Arbitrary Views for High-Fidelity 3D Generation](https://arxiv.org/abs/2605.21121)
*Hanxiao Sun,Mingxin Yang,Shuhui Yang,Zebin He,Xintong Han,Hongbo Fu,Chunchao Guo,Wenhan Luo*

Main category: cs.CV

TL;DR: 提出ROAR-3D，轻量级方法将预训练单视图模型升级为多视图条件生成，无需外部重建模块或固定视角，支持任意数量未摆姿图像输入，实现高质量3D生成。


<details>
  <summary>Details</summary>
Motivation: 现有单视图到3D生成存在未见区域歧义，多视图条件方法需固定视角或外部重建模块，训练成本高且限制生成质量。

Method: 设计令牌级视图路由器隐式建立2D-3D对应，双流注意力机制分离主视角与辅助视图处理，引入方向扰动策略学习无关方向的几何迁移。

Result: 在多个基准上达到最先进多视图3D生成质量，支持测试时从1到12+视图的持续改进，且可训参数极少、推理开销可忽略。

Conclusion: ROAR-3D有效复用预训练单视图模型能力，实现高效、可扩展的多视图3D生成。
...
### [159] [Distill to Think, Foresee to Act: Cognitive-Physical Reinforcement Learning for Autonomous Driving](https://arxiv.org/abs/2605.21139)
*Yang Wu,Qiang Meng,Zhaojiang Liu,Youquan Liu,Jian Yang,Jin Xie*

Main category: cs.CV

TL;DR: CoPhy是一个结合认知智能和物理预测的强化学习框架，用于端到端自动驾驶，在NAVSIM基准上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型受限于模仿学习的行为克隆上限，而强化学习缺乏认知基础（理解交通语义和驾驶意图）和预见性物理环境（预测动作后果）。

Method: 将VLM知识蒸馏到BEV编码器，保留零推理成本认知能力；构建自回归BEV世界模型预测未来语义地图；使用GRPO和双重奖励（物理奖励保证安全，认知奖励确保意图符合）优化策略。

Result: 在NAVSIM v1和v2基准上取得最优结果，实现更安全的驾驶和灵活的用户语言指令控制。

Conclusion: CoPhy通过认知和物理基础设施结合，有效提升了自动驾驶的安全性和意图合规性。
...
### [160] [SurgOnAir: Hierarchy-Aware Real-Time Surgical Video Commentary](https://arxiv.org/abs/2605.21132)
*Jingyi He,Yue Zhou,Long Bai,Kun Yuan,Nassir Navab,Yuan Bi*

Main category: cs.CV

TL;DR: 提出了SurgOnAir，一个流式视觉语言模型，能够逐帧处理手术视频并实时生成多层级叙述，捕捉手术工作流中的关键状态变化。


<details>
  <summary>Details</summary>
Motivation: 现有手术工作流理解方法多采用离线或粗时间粒度处理，无法实时响应手术过程中的细微变化，限制了AI辅助的及时性和安全性。

Method: 构建了SurgOnAir-11k分层数据集，包含动作、步骤和阶段三级监督；设计流式视觉语言模型，逐帧处理输入并逐步生成叙事标记，同时引入特殊转换标记以显式标识状态变化。

Result: 实验表明，SurgOnAir能通过单一模型实时、统一地理解手术工作流的多个层次，生成更优且具有层级意识的叙述。

Conclusion: SurgOnAir实现了实时的、细粒度的手术工作流理解，为智能手术赋能提供了新的解决方案，代码和数据集将开源。
...
### [161] [RankE: End-to-End Post-Training for Discrete Text-to-Image Generation with Decoder Co-Evolution](https://arxiv.org/abs/2605.21195)
*Siyong Jian,Siyuan Li,Luyuan Zhang,Zedong Wang,Xin Jin,Ying Li,Cheng Tan,Huan Wang*

Main category: cs.CV

TL;DR: 提出了RankE，首个用于离散自回归文本到图像生成模型的端到端后训练框架，通过交替优化策略和解码器来突破保真度与对齐度的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有离散AR模型的后训练仅优化策略而冻结解码器，导致潜在协变量偏移：策略演变使得令牌分布偏离解码器训练用的真实分布，图像质量退化。

Method: 采用交替优化策略和解码器，每个模块最大化基于排名的对齐目标，并利用适用于其参数空间的稳定性保持锚点进行正则化。

Result: 在LlamaGen-XL上，标准RL提升CLIP但恶化FID，而RankE同时改善FID（15.21）和CLIP（33.76）；在Janus-Pro上有一致提升。

Conclusion: 解码器共演化能可靠地将奖励优化转化为像素空间质量改进，打破保真度-对齐度权衡。
...
### [162] [Semantic Granularity Navigation in Image Editing](https://arxiv.org/abs/2605.21190)
*Liangsi Lu,Minzhe Guo,Xuhang Chen,Yang Shi*

Main category: cs.CV

TL;DR: 提出NaviEdit方法，通过自一致性合约解耦编辑进度与模型尺度，在不重新训练的情况下提升图像编辑的语义可编辑性与结构保真度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散和流模型的真实图像编辑存在语义可编辑性与结构保真度之间的权衡，主要原因在于编辑进度与模型尺度隐式耦合，导致强编辑需访问噪声更大的状态，破坏布局稳定性。

Method: NaviEdit是一种训练无关的推理时控制器，基于严格的自一致性合约，将尺度作为控制输入，重新分配固定步长预算到语义响应敏感的中间尺度，避免高噪声破坏区域。

Result: 在多个兼容编辑器和流模型骨干上取得平均正向增益，验证了解耦作为可移植推理时控制原则的有效性。

Conclusion: 解耦编辑进度与模型尺度可作为一种即插即用的推理时控制原理，在不改变预训练模型的前提下提升编辑性能。
...
### [163] [FTerViT: Fully Ternary Vision Transformer](https://arxiv.org/abs/2605.21171)
*Szymon Ruciński,Pietro Bonazzi,Engin Türetken,Simon Narduzzi,Michele Magno,Nadim Maamari*

Main category: cs.CV

TL;DR: 提出一种全三值视觉 Transformer（FTerViT），所有权重矩阵和归一化参数均三值化，并在微控制器上首次部署。


<details>
  <summary>Details</summary>
Motivation: 现有三值化方法仅对编码器层三值化，而嵌入、归一化和分类头仍为全精度，导致资源受限设备（如微控制器）上内存占用过大，部署受限。

Method: 引入两种新算子：TernaryBitConv2d（带逐通道缩放）用于块嵌入，TernaryLayerNorm用于归一化；采用知识蒸馏训练和轻量量化感知恢复阶段。

Result: DeiT-III-S模型在384×384分辨率下ImageNet-1K top-1准确率82.43%，模型大小6.09MB（约15倍压缩，比FP32低2.42个百分点），优于先前方法最多8个百分点；在ESP32-S3微控制器上部署FTerViT-Small（224×224，5.81MB）达79.64%准确率。

Conclusion: 全三值化Vision Transformer能大幅压缩模型且保持高精度，首次证明可在微控制器上有效部署。
...
### [164] [PGC: Peak-Guided Calibration for Generalizable AI-Generated Image Detection](https://arxiv.org/abs/2605.21207)
*Xiaoyu Zhou,Jianwei Fei,Peipeng Yu,Jingchang Xie,Chong Cheng,Zhihua Xia*

Main category: cs.CV

TL;DR: 本文提出峰值引导校准（PGC）框架，通过聚焦局部鉴别线索来增强生成图像检测的全局决策，在多个基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有检测器依赖全局表征，但生成图像中的细微鉴别线索被高保真内容（如主体）掩盖，限制了检测可靠性。

Method: PGC框架采用峰值敏感聚合机制，突出最具有鉴别力的局部线索，并利用这些信号校准全局决策。

Result: 在CommGen15数据集上平均准确率提升12.3%，并在GenImage、AIGI、UniversalFakeDetect上分别提升2.1%、3.5%、1.7%。

Conclusion: PGC能有效恢复被全局上下文淹没的细微模式，显著提升生成图像检测性能。
...
### [165] [SR-Ground: Image Quality Grounding for Super-Resolved Content](https://arxiv.org/abs/2605.21244)
*Artem Borisov,Evgeney Bogatyrev,Khaled Abud,Dmitriy Vatolin*

Main category: cs.CV

TL;DR: 提出了SR-Ground数据集，用于超分辨图像中的细粒度伪影分割，包含63,000张图像和6类伪影标注，并展示了其在提升IQA模型和减少伪影方面的实用价值。


<details>
  <summary>Details</summary>
Motivation: 现有图像质量评估方法缺乏可解释性，无法区分超分辨率模型产生的不同伪影类型。

Method: 构建大规模数据集（SR-Ground），通过1,062名参与者验证分割标注，并设计基于接地能力的IQA模型训练及微调流水线。

Result: SR-Ground训练出的IQA模型在下游任务中性能显著提升，且微调流程有效减少了超分辨率输出中的视觉伪影。

Conclusion: 细粒度伪影分割数据集可增强IQA模型的可解释性和实用性，为改进超分辨率算法提供新途径。
...
### [166] [STiTch: Semantic Transition and Transportation in Collaboration for Training-Free Zero-Shot Composed Image Retrieval](https://arxiv.org/abs/2605.21261)
*Miaoge Li,Dongsheng Wang,Zening Sun,Jinsen Zhang,Wenhan Luo,Jingcai Guo*

Main category: cs.CV

TL;DR: 提出了一种无需训练的零样本组合图像检索方法，通过语义过渡向量和双向运输距离改进检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法存在生成描述引入参考图像噪声、检索阶段点对点对齐无法捕捉多样组合的问题。

Method: 使用LLM生成组合描述，通过嵌入空间中的过渡向量优化描述使其更接近目标图像；将描述和图像建模为离散分布，采用双向运输距离进行集合间对齐。

Result: 在多个组合图像检索任务上表现出通用性和有效性。

Conclusion: 该方法通过语义过渡和分布对齐，有效提升了零样本组合图像检索的准确性和鲁棒性。
...
### [167] [Let EEG Models Learn EEG](https://arxiv.org/abs/2605.21280)
*Yifan Wang,Yijia Ma,Wen Li,Chenyu You*

Main category: cs.CV

TL;DR: 提出基于条件流匹配的JET框架，直接在连续轨迹上生成脑电信号，无需离散去噪或领域特定表示，在三个大规模基准上TS-FID降低超40%。


<details>
  <summary>Details</summary>
Motivation: 现有离散去噪方法无法反映脑电信号的连续时间动态和频谱结构，导致长程依赖和频谱结构不匹配。

Method: 基于条件流匹配的生成框架，学习从噪声到脑电数据分布的平滑向量场，并引入保持频谱结构、时间平稳性和信号统计量的约束。

Result: 在三个大规模基准上达到最先进性能，TS-FID相对强基线降低超40%，有效捕捉长程时间依赖和频谱结构。

Conclusion: JET提供了一种可扩展且符合原理的脑电信号生成方法，能够保持神经动力学的关键结构特性。
...
### [168] [Reducing Object Hallucination in LVLMs via Emphasizing Image-negative Tokens](https://arxiv.org/abs/2605.21300)
*Meng Shen,Minghao Wu,Deepu Rajan*

Main category: cs.CV

TL;DR: 本文发现大视觉语言模型中的对象幻觉源于模型优先文本生成而忽视图像信息，分析了生成令牌的分类依赖，提出基于视觉依赖调整训练权重和过滤数据的方法，有效减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 对象幻觉阻碍LVLM应用，假设原因在于模型更关注文本生成而非图像交互，需探索其起源并解决。

Method: 将生成令牌分为图像正、不变和负三类，分析视觉依赖；调整训练权重并过滤高幻觉数据，不增加推理成本。

Result: 两种方法减少幻觉，保持响应长度，在三种LVLM变体上验证有效性。

Conclusion: 通过调整令牌训练权重和数据过滤可有效控制幻觉，适用于多种LVLM。
...
### [169] [Vision Transformers and Convolutional Neural Networks for Land Use Scene Classification](https://arxiv.org/abs/2605.21268)
*Arun D. Kulkarni*

Main category: cs.CV

TL;DR: 该研究比较了视觉变换器（ViT）和卷积神经网络（CNN）在遥感土地利用场景分类中的性能，发现CNN在小样本和局部纹理场景中表现稳健，而ViT在复杂场景和大数据条件下更优。


<details>
  <summary>Details</summary>
Motivation: 遥感影像土地利用场景分类对环境和城市管理至关重要。此前CNN是主流，但ViT的出现可能通过自注意力机制提升全局理解，需系统比较两者优劣。

Method: 选取AlexNet（CNN）和Vision Transformer（ViT）模型，在UC Merced和EuroSAT数据集上评估分类精度、精确率、召回率、F1分数和计算复杂度。

Result: CNN在小样本和局部纹理数据集上性能稳健；ViT在充足数据下全局关系捕捉更优，但计算资源需求更高。

Conclusion: 该研究揭示了两种架构的优缺点，为实际应用中选择模型提供了指导：小数据量或局部特征重要时选CNN，大数据集且需全局理解时选ViT。
...
### [170] [OcclusionFormer: Arranging Z-Order for Layout-Grounded Image Generation](https://arxiv.org/abs/2605.21343)
*Ziye Li,Henghui Ding*

Main category: cs.CV

TL;DR: 提出OcclusionFormer框架，通过显式Z顺序建模和体积渲染解决布局到图像生成中的物体遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 现有布局到图像模型在重叠边界框区域缺乏显式遮挡信息，导致纹理纠缠或层次不一致。

Method: 构建SA-Z数据集并设计OcclusionFormer，利用扩散Transformer结合体积渲染解耦实例，通过查询对齐损失提升语义一致性。

Result: 在重叠区域减少歧义，强化遮挡依赖，保持结构完整性，显著提升多场景准确性。

Conclusion: 显式建模遮挡顺序可有效解决生成中的空间混淆问题。
...
### [171] [DriveMA: Rethinking Language Interfaces in Driving VLAs with One-Step Meta-Actions](https://arxiv.org/abs/2605.21273)
*Weicheng Zheng,Yixin Huang,Qiao Sun,Derun Li,Hang Zhao*

Main category: cs.CV

TL;DR: 提出DriveMA模型，用简洁的一步元动作替代冗长的自然语言推理作为中间接口，结合动作监督训练和回合级强化学习，在Waymo端到端驾驶挑战中取得新最先进性能（2B模型RFS 8.060，4B模型8.079）。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶视觉-语言-动作模型使用自然语言推理作为中间接口，但面临高质量注释难获取、紧凑模型理解和生成长推理链难、推理延迟高等瓶颈。

Method: 采用一步元动作作为语义决策接口，结合动作监督训练和回合级信用分配强化学习框架，联合优化元动作正确性、轨迹质量和轨迹-元动作一致性。

Result: 2B模型在Waymo端到端驾驶挑战中达到RFS 8.060，4B模型进一步改进至8.079，并在NAVSIM上获得有竞争力表现；消融实验证明一步元动作在表达性、可预测性和推理效率上优于自然语言推理或细粒度动作序列。

Conclusion: 简洁的一步元动作是驾驶VLA模型中自然语言推理的有效替代，能在保持低熵的同时提供可扩展监督和可靠轨迹条件，实现更好的实际权衡。
...
### [172] [A Non-Reference Diffusion-Based Restoration Framework for Landsat 7 ETM+ SLC-off Imagery in Antarctica](https://arxiv.org/abs/2605.21371)
*Leyue Tang,Jonathan Louis Bamber,Gang Qiao,Yuanhang Kong*

Main category: cs.CV

TL;DR: 提出了一个无需参考数据的扩散模型框架DiffGF，用于恢复南极地区Landsat 7 SLC-off影像的缺失像素，并构建了专用数据集SLCANT。


<details>
  <summary>Details</summary>
Motivation: 南极地区极夜和云覆盖导致光学影像获取困难，Landsat 7 SLC故障造成约22%像素缺失，而传统基于参考影像的填充方法因南极地表快速变化难以适用。

Method: 采用两阶段设计：潜在空间扩散过程与像素空间细化，无需外部参考数据。

Result: 定量和定性结果表明DiffGF能高保真恢复南极SLC-off影像，并在下游冰裂隙分割应用中验证了其实用价值。

Conclusion: DiffGF为利用Landsat 7 SLC-off历史档案提供了有效方法，有助于提取南极地区的宝贵信息和相关研究。
...
### [173] [Disentangling Generation and Regression in Stochastic Interpolants for Controllable Image Restoration](https://arxiv.org/abs/2605.21381)
*Yi Liu,Jia Ma,Wengen Li,Jihong Guan,Shuigeng Zhou,Yichao Zhang*

Main category: cs.CV

TL;DR: 提出统一框架DiSI，将随机插值过程解耦为回归与生成组件，实现单模型内失真-感知权衡的灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有生成式图像修复方法推断缓慢且保真度不足，而回归方法虽高效但缺乏纹理真实性，需弥合两类方法的差距。

Method: 1) 解耦随机插值过程为独立回归与生成组件；2) 设计两条采样轨迹及统一采样器；3) 双分支U-Net风格Transformer网络增强条件引导。

Result: 多种图像修复任务上实现竞争性结果，单模型可灵活控制推断时的失真-感知权衡。

Conclusion: DiSI通过解耦策略有效平衡效率与质量，提供连续可调的修复效果。
...
### [174] [RoadTones: Tone Controllable Text Generation from Road Event Videos](https://arxiv.org/abs/2605.21411)
*Chirag Parikh,Siddhi Pravin Lipare,Ravi Kiran Sarvadevabhatla*

Main category: cs.CV

TL;DR: 提出了用于可控语气道路视频描述的完整数据集-模型-评估套件，包含RoadTones-51K数据集、RoadTones-VL-CoT模型和RoadTones-Eval评估体系。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型能生成事实准确的道路事件描述，但无法控制表达的语气、紧迫感或风格，限制了在通信关键场景中的应用。

Method: 构建人工验证的数据生成管道扩展道路视频语料，添加多样化语气标注和多语气描述；提出可控视频到文本模型RoadTones-VL-CoT，生成语气条件的思维链中间草稿；设计RoadTones-Eval联合评估事实一致性和语气遵循度。

Result: 用户研究验证了描述质量、语气控制和事实一致性；数据集、模型和评估套件为上下文敏感的可控语气视频描述奠定基础。

Conclusion: 该工作为需要同时考虑内容与表达方式的通信关键场景提供了可行的解决方案，提升了视频描述的可控性和实用性。
...
### [175] [iTryOn: Mastering Interactive Video Virtual Try-On with Spatial-Semantic Guidance](https://arxiv.org/abs/2605.21431)
*Jun Zheng,Zhengze Xu,Mengting Chen,Jing Wang,Jinsong Lan,Xiaoyong Zhu,Kaifu Zhang,Bo Zheng,Xiaodan Liang*

Main category: cs.CV

TL;DR: 提出了交互式视频虚拟试穿任务及iTryOn框架，通过多层级交互注入机制和动作感知旋转位置嵌入，实现了高质量动态试穿效果。


<details>
  <summary>Details</summary>
Motivation: 现有VVT方法局限于非交互场景，无法处理人体与衣物的主动交互。

Method: 基于视频扩散Transformer，空间层引入3D手部先验解决接触歧义，语义层通过全局标题和时戳动作标题结合A-RoPE同步控制。

Result: 在传统VVT基准上达SOTA，在交互设置中表现显著领先。

Conclusion: iTryOn有效推动了可控动态虚拟试穿的发展。
...
### [176] [ReMATF: Recurrent Motion-Adaptive Multi-scale Turbulence Mitigation for Dynamic Scenes](https://arxiv.org/abs/2605.21440)
*Zhiming Liu,Zhicheng Zou,Nantheera Anantrasirichai*

Main category: cs.CV

TL;DR: 提出轻量级循环框架ReMATF，仅使用两帧即恢复湍流视频，兼顾空间细节与时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer和3D架构的多帧方法计算成本高、内存大，难以实时部署。

Method: 多尺度编码器-解码器结合时间扭曲与运动自适应时间融合模块，逐像素融合前一帧输出与当前预测。

Result: 在合成和真实湍流数据集上，PSNR/SSIM和LPIPS均有提升，推理速度显著快于多帧Transformer基线。

Conclusion: ReMATF在资源受限场景下能有效减轻湍流导致的视频失真。
...
### [177] [StreamGVE: Training-Free Video Editing via Few-Step Streaming Video Generation](https://arxiv.org/abs/2605.21466)
*Guanlong Jiao,Chenyangguang Zhang,Jia Jun Cheng Xian,Zewei Zhang,Renjie Liao*

Main category: cs.CV

TL;DR: 提出StreamGVE，一种基于噪声到数据范式的视频编辑方法，通过流式生成模型和双分支快速采样实现高质量编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要大量迭代且效果有限，原因是数据到数据范式与现代生成模型不兼容。

Method: 基于预训练流式生成模型，采用双分支快速采样、自注意力桥接、交叉注意力增强、源导向引导和视觉提示策略。

Result: 在多种视频编辑任务中，StreamGVE在少步采样下性能显著优于现有方法。

Conclusion: 该方法是有效、鲁棒且可泛化的，适用于不同模型。
...
### [178] [Stream3D: Sequential Multi-View 3D Generation via Evidential Memory](https://arxiv.org/abs/2605.21472)
*Kaichen Zhou,Zeyang Bai,Xinhai Chang,Mengyu Wang,Paul Liang,Fangneng Zhan*

Main category: cs.CV

TL;DR: 提出Stream3D，一种无需训练的流式机制，将冻结的视图条件3D生成器转化为流式生成器，通过恒定跨块记忆实现单目视频流中一致的三维重建。


<details>
  <summary>Details</summary>
Motivation: 现有视图条件3D生成器处理单目视频流时，逐帧独立应用会导致生成结果严重的时间不一致性。

Method: 维护一个紧凑的证据记忆，基于证据评分机制选择性缓存最信息丰富的历史帧，并动态更新以保持固定数量，避免内存增长和性能退化。

Result: 在真实和合成流式基准上，Stream3D在光度学和几何学指标上均优于KV-cache复用和基于流的特征编辑等基线方法。

Conclusion: Stream3D无需重训练或修改架构即可实现一致的流式3D生成，有效解决了时序一致性问题。
...
### [179] [ProtoPathway: Biologically Structured Prototype-Pathway Fusion for Multimodal Cancer Survival Prediction](https://arxiv.org/abs/2605.21454)
*Amaya Gallagher-Syed,Costantino Pitzalis,Myles J. Lewis,Michael R. Barnes,Gregory Slabaugh*

Main category: cs.CV

TL;DR: 提出了一种名为ProtoPathway的可解释多模态框架，通过形态原型和通路嵌入实现癌症生存预测，在五个TCGA数据集上展现了竞争性能、更高的可解释性和更低的计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有癌症生存预测模型可解释性不足，且难以统一组织病理学与转录组学数据的生物学意义。

Method: 采用可学习的形态学原型作为组织表征，基于Reactome通路的分层图神经网络编码基因表达，并通过交叉注意力机制连接原型与通路，实现端到端可解释预测。

Result: 在5个TCGA癌症队列上，该模型在生存预测性能上具有竞争力或更优，同时显著提高了生物学可解释性并降低了计算开销。

Conclusion: ProtoPathway通过设计内置可解释性的多模态融合方法，有效实现了从基因到组织形态的层级归因，为临床决策提供了可信的生物学解释。
...
### [180] [AIGaitor: Privacy-preserving and cloud-free motion analysis for everyone, using edge computing](https://arxiv.org/abs/2605.21421)
*Lauhitya Reddy,Trisha M. Kesar,Hyeokhyen Kwon*

Main category: cs.CV

TL;DR: AIGaitor是首个在消费级智能手机上运行的无标记单目运动捕捉与深度学习分析系统，克服了成本、隐私和操作复杂度三大临床障碍，在iPhone 14上处理10秒4K视频仅需77秒，性能优于云端方案。


<details>
  <summary>Details</summary>
Motivation: 74名康复临床医生调查显示92%愿采用低成本AI步态分析工具，但79.7%关注运营成本、68.9%缺乏培训、64.9%担忧隐私，现有系统无法同时满足准确性、低成本、易用性和隐私保护。

Method: 优化移动iOS实现单目流水线组件（2D/3D姿态估计、姿态优化、骨骼深度学习分析、视觉语言模型），采用Time-Priority端到端流水线，利用iPhone神经网络加速器，并测试轻量ViTPose-s实时关键点提取和动作识别模型。

Result: 10秒4K 60fps视频在iPhone 14上77秒完成处理，云端方案（含网络传输）需94秒（全球移动平均上行）或66秒（发达地区Wi-Fi），关键点提取达实时，步态分类亚毫秒级完成。

Conclusion: 首次实现端到端手机上无标记运动捕捉与分析，证明低成本、隐私保护且易用的临床级运动分析对智能手机用户可行。
...
### [181] [Uni-Edit: Intelligent Editing Is A General Task For Unified Model Tuning](https://arxiv.org/abs/2605.21487)
*Dian Zheng,Manyuan Zhang,Hongyu Li,Hongbo Liu,Kai Zou,Kaituo Feng,Hongsheng Li*

Main category: cs.CV

TL;DR: 提出Uni-Edit，一个将智能图像编辑作为统一多模态模型通用训练任务的方法，仅用单一任务、阶段和数据集即可同时提升图像理解、生成和编辑能力。


<details>
  <summary>Details</summary>
Motivation: 现有混合多任务训练策略存在任务冲突，需要复杂流程和平衡技巧，导致性能折衷而非真正互惠增强，因此需要更高效的训练范式。

Method: 首先识别图像编辑是天然的理想通用任务，同时需要理解和生成能力；然后构建自动化数据合成流水线，将VQA数据转化为带有嵌入问题和嵌套逻辑的复杂编辑指令，生成Uni-Edit-148k数据集。

Result: 在BAGEL和Janus-Pro模型上实验表明，仅用Uni-Edit调优即可实现图像理解、生成和编辑三种能力的全面增强，无需任何辅助操作。

Conclusion: 智能图像编辑可作为统一多模态模型训练的有效通用任务，简化流程并实现任务间互惠增强。
...
### [182] [One-Step Distillation of Discrete Diffusion Image Generators via Fixed-Point Iteration](https://arxiv.org/abs/2605.21484)
*Chaoyang Wang,Yunhai Tong*

Main category: cs.CV

TL;DR: 提出固定点蒸馏（FPD）框架，通过单步推理实现离散扩散模型的高效视觉生成，缩小与多步教师的性能差距。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型虽在视觉合成中表现优异，但迭代解码过程缓慢；现有蒸馏方法需辅助网络或多阶段流程，存在计算开销大或优化碎片化的问题。

Method: 构建局部校正目标：部分破坏学生单步草稿并单步教师细化；将离散令牌映射到连续特征空间，使用多带宽漂移损失；通过直通估计器反向传播离散瓶颈，支持对抗目标增强感知真实性。

Result: 在类别和文本条件生成任务中，FPD单步推理达到与多步教师相近的视觉保真度和结构对齐，优于现有离散蒸馏基线。

Conclusion: FPD是一种端到端、高效的单步蒸馏方法，显著减少了离散扩散模型的推理开销，同时保持生成质量。
...
### [183] [Latent Dynamics for Full Body Avatar Animation](https://arxiv.org/abs/2605.21478)
...
TL;DR: 提出一种结合变压器解码器与动态残差潜变量的3D高斯化身体模型，通过学习潜变量动力学演化（分解为驱动、恢复和耗散力），实现历史依赖且时域连贯的宽松衣物动画。


<details>
  <summary>Details</summary>
Motivation: 现有基于姿态驱动的神经渲染模型无法捕捉宽松衣物因历史、惯性、接触导致的复杂变形；显式模拟方法需要专用模板或高运行时成本；数据驱动方法缺乏对潜变量动态演化的显式建模，且细粒度细节生成不足。

Method: 在姿态条件化3D高斯化身上添加变压器解码器与动态残差潜变量；推理时通过短姿态历史与前一潜状态学习潜动力学模型，将更新分解为驱动、恢复和耗散力。

Result: 在9个日常运动序列上，定量指标与感知用户研究均表明动画质量优于近期数据驱动基线方法，并能生成多样化且合理的运动轨迹，通过力分解实现刚度等外部控制。

Conclusion: 通过显式建模潜变量的动力学演化（而非固定或回归），以极低成本实现时域连贯、历史依赖的宽松衣物动画，并支持灵活的外部控制。
...
### [184] [Leveraging Large Language Models for Sentiment Analysis: Multi-Modal Analysis of Decentraland's MANA Token](https://arxiv.org/abs/2605.20192)
*Xintong Wu,Peiting Tsai,Jing Yuan,Michael Yu,Greg Sun,Luyao Zhang*

Main category: cs.CL

TL;DR: 本文研究了如何将Decentraland的Discord社区情绪与多模态金融数据结合，以提高虚拟世界经济中的加密货币价格预测能力。


<details>
  <summary>Details</summary>
Motivation: Decentraland等虚拟世界平台使用加密货币进行交易，但价格波动大，需要更精准的预测模型；社区情绪可能蕴含预测价值。

Method: 使用BERT模型进行情绪分析，构建两种LSTM架构：基线模型仅使用历史价格，多模态模型整合情绪分数、交易量和市值。

Result: 社区情绪多为中性但偏积极，多模态模型在预测准确性上显著优于基线模型。

Conclusion: 社区情绪信号对虚拟经济预测有重要价值，为虚拟环境、自然语言处理和加密货币分析的交叉研究奠定基础。
...
### [185] [Shiny Stories, Hidden Struggles: Investigating the Representation of Disability Through the Lens of LLMs](https://arxiv.org/abs/2605.20191)
*Marco Bombieri,Simone Paolo Ponzetto,Marco Rospocher*

Main category: cs.CL

TL;DR: 本文探究大型语言模型在模拟残障人士社交媒体发帖时，是否理想化其经历并强化负面偏见。


<details>
  <summary>Details</summary>
Motivation: 现有LLM可能放大偏见，或过度纠正导致不真实的正向刻板印象，需评估其对残障群体的表征。

Method: 对比LLM生成的模拟残障人士的帖子与真实残障人士的帖子，分析情感倾向、代表性词语和主题。

Result: LLM倾向过度美化残障体验，产生不真实正向刻板印象；同时某些主题（如职业、娱乐）被不公正地与健全人关联。

Conclusion: LLM未能真实反映残障群体的多样性生活，需警惕其表征偏差。
...
### [186] [Parallel LLM Reasoning for Bias-Resilient, Robust Conceptual Abstraction](https://arxiv.org/abs/2605.20194)
*Aisvarya Adeseye,Jouni Isoaho,Adeyemi Adeseye*

Main category: cs.CL

TL;DR: 提出并行分块与证据锚定结合的框架，提升长文本LLM分析的可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM分析长文档时存在上下文推理局限、主导概念偏差、遗漏误差和泛化问题，且独立输出合并缺乏系统性。

Method: 将文本分割为语义块并行独立处理，通过证据锚定与优先级排序进行整合。

Result: 并行处理减少约84%遗漏误差，证据可追溯性提升130%，无根据断言减少91%，小模型获益最大。

Conclusion: 高效并行分块与整合策略是实现可扩展、可靠文本分析的关键。
...
### [187] [Improving Quantized Model Performance in Qualitative Analysis with Multi-Pass Prompt Verification](https://arxiv.org/abs/2605.20193)
*Aisvarya Adeseye,Jouni Isoaho,Adeyemi Adeseye*

Main category: cs.CL

TL;DR: 论文研究了量化LLMs（8位至2位）在定性分析中的表现，并提出一种量化感知的多轮提示验证方法，以提升低比特模型的准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 量化LLMs运行快、资源需求低，但低比特模型易产生幻觉和不稳定结果，尤其是在处理非专业语言时，因此需要研究如何提升其性能。

Method: 提出一种量化感知的多轮提示验证方法，通过控制步骤减少幻觉，去除不可靠内容并验证后传递结果。使用82份访谈转录文本，结合人类编码者（NVivo和BF16 LLaMA）建立黄金标准地面真值（GSGT）进行对比评估。

Result: 8位模型最接近GSGT；4位模型借助方法提升稳定性；3位和2位模型因压缩严重性能下降，但所提方法有改善；同比特不同量化类型表现不同。

Conclusion: 所提方法帮助低资源LLMs在定性研究中变得更稳定、准确且成本更低。
...
### [188] [Pseudo-Siamese Network for Planning in Target-Oriented Proactive Dialogues](https://arxiv.org/abs/2605.20195)
*Xinyue Kang,Maodong Li,Yibin Zheng,Fang Kong*

Main category: cs.CL

TL;DR: 提出了一种面向目标的主动对话系统路径规划方法FF-BPSN，通过双向伪孪生网络和正向聚焦模块实现最优对话路径，在DuRecDial等数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有面向目标对话系统中，路径规划作为核心环节仍未被充分探索，缺乏有效方法预定义对话目标的合理路径。

Method: 采用两个相同的基于Transformer的解码器分别进行正向和反向路径规划，并通过正向聚焦模块融合双向信息生成最终的正向路径。

Result: 在DuRecDial和DuRecDial 2.0数据集上，路径规划性能达到最佳，且显著提升整个主动对话系统的效果。

Conclusion: FF-BPSN有效结合双向规划优势，优先考虑正向信息，为预训练语言模型提供高质量对话路径指导。
...
### [189] [Data Scaling as Progressive Coverage of a Predictive Contribution Spectrum](https://arxiv.org/abs/2605.20196)
*Zihui Song,Shihao Ji,Hongxi Li,Shuaizhi Cheng,Chunlin Huang*

Main category: cs.CL

TL;DR: 本文研究了实际数据缩放定律，发现其由潜在预测贡献谱的渐进覆盖主导，并基于后缀自动机定义了全局KL谱，通过12个语料库验证其尾斜率与GPT学习器的缩放指数高度相关。


<details>
  <summary>Details</summary>
Motivation: 探索数据缩放定律是否更依赖于预测贡献谱的渐进覆盖，而非简单的词频尾部。

Method: 使用后缀自动机表示文本语料库，定义全局KL预测贡献谱，计算每个状态的经验质量与KL偏差的乘积，并通过匹配训练规模N与截断秩K(N)来验证。

Result: 尾斜率与学习器经验缩放指数强相关；log K与log N近似线性，R²分别约0.96（原始谱）和0.90（平滑谱）。

Conclusion: 训练规模通过推进预测状态谱的有效边界来工作，剩余尾部质量跟踪剩余过盈损失。
...
### [190] [MedicalBench: Evaluating Large Language Models Toward Improved Medical Concept Extraction](https://arxiv.org/abs/2605.20197)
*Zhichao Yang,Gregory D. Lyng,Sanjit Singh Batra,Robert E. Tillman*

Main category: cs.CL

TL;DR: 提出了MedicalBench基准，用于评估医疗概念提取中的隐含推理和证据 grounding，发现现有LLMs性能有限。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注显式概念，而医疗叙述中隐含概念提取更具挑战性，且缺乏系统性评估。

Method: 基于MIMIC-IV出院摘要和ICD-10代码，通过多阶段LLM分类流程和专家审核构建数据集，定义概念提取和证据检索双任务。

Result: SOTA LLMs在隐含概念提取上表现欠佳，性能与笔记长度无关，表明基准聚焦推理难度。

Conclusion: MedicalBench是首个面向隐含、证据 grounded的医疗概念提取基准，为透明和医学可靠的模型开发奠定基础。
...
### [191] [FlowLM: Few-Step Language Modeling via Diffusion-to-Flow Adaptation](https://arxiv.org/abs/2605.20199)
*Runzhe Zhang,Letian Chen,Wenpeng Zhang,Zhouhan Lin,Peilin Zhao*

Main category: cs.CL

TL;DR: FlowLM: 通过高效微调从预训练扩散语言模型转换而来的流匹配语言模型。它重新对齐扩散模型的弯曲采样轨迹为直线流，实现了高质量少步生成，在极少的训练周期内即可媲美甚至超越2000步扩散采样的质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型采样轨迹弯曲，导致生成速度慢；现有流匹配模型需从头训练，效率低。本研究旨在通过高效微调将预训练扩散模型转换为流匹配模型，加速生成并保持高质量。

Method: 通过高效微调将预训练扩散语言模型转换为流匹配语言模型，重新对齐扩散模型的弯曲采样轨迹为直线流，并采用预测干净数据的训练目标以一致引导采样过程。

Result: 微调的FlowLM仅需一半训练周期即可达到性能饱和，远少于从头训练；两种方式均显著优于原始扩散模型；验证了预测干净数据的训练目标更有效。

Conclusion: 该方法在高质量少步文本生成中高效且有效，可用于加速文本生成任务。
...
### [192] [Long-Context Reasoning Through Proxy-Based Chain-of-Thought Tuning](https://arxiv.org/abs/2605.20201)
*Miao Li,Irina Saparina,Alexander Gurung,Mirella Lapata*

Main category: cs.CL

TL;DR: 提出ProxyCoT框架，通过短上下文推理能力迁移到长上下文，提升大语言模型在长序列复杂推理任务的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽支持超长输入，但在长上下文推理任务中表现不佳，而这类任务仅需部分输入（代理上下文）即可解决，但模型在代理与全上下文间存在性能差异。

Method: 先通过强化学习或教师模型蒸馏在代理上下文上生成高质量链式思考推理轨迹，再利用全上下文进行监督微调，将推理能力迁移至长序列。

Result: 在多个数据集上，ProxyCoT以更低计算开销持续优于强基线，且训练后的模型能将长上下文推理能力泛化到域外任务。

Conclusion: ProxyCoT有效解决了长上下文推理中代理与全上下文的性能差距，提升了推理效率与泛化能力。
...
### [193] [Mix-Quant: Quantized Prefilling, Precise Decoding for Agentic LLMs](https://arxiv.org/abs/2605.20315)
*Haiquan Lu,Zigeng Chen,Gongfan Fang,Xinyin Ma,Xinchao Wang*

Main category: cs.CL

TL;DR: 提出 Mix-Quant，一种针对 LLM 智能体的阶段感知量化框架，在预填充阶段使用 NVFP4 量化以加速推理，同时保留解码阶段的 BF16 精度，实现高效推理而不显著损失性能。


<details>
  <summary>Details</summary>
Motivation: LLM 智能体工作流程中，预填充阶段因处理长上下文和多轮交互而成为计算瓶颈，现有全量化方法会严重影响性能。

Method: Mix-Quant 框架：对计算密集的预填充阶段应用高吞吐 NVFP4 量化，对解码阶段保持 BF16 精度，通过解耦两阶段实现加速与精度平衡。

Result: 在长上下文和智能体基准测试中，Mix-Quant 基本保持任务性能，同时实现高达 3 倍的预填充加速。

Conclusion: Mix-Quant 通过阶段感知的量化策略有效缓解了 LLM 智能体推理的预填充瓶颈，兼顾效率与性能。
...
### [194] [Under Pressure: Emotional Framing Induces Measurable Behavioral Shifts and Structured Internal Geometry in Small Language Models](https://arxiv.org/abs/2605.20202)
*Rana Muhammad Usman*

Main category: cs.CL

TL;DR: 研究发现情感框架对小型语言模型的行为和内部表示有可测量的影响，压力框架导致最多的捷径标记，而平静和好奇框架更常保持诚实，激活方向在最后一层汇聚。


<details>
  <summary>Details</summary>
Motivation: 探索情感化评估反馈是否能改变小型语言模型的行为和内部表示，验证是否存在可测量的提示敏感控制方向。

Method: 使用Qwen 3.5 0.8B模型在四个不可能约束编码任务和八种情感框架下进行160次对话实验，通过分析方向向量、PCA降维和模型规模对比（0.8B vs 2B）来评估影响。

Result: 压力框架下捷径标记最多（11/20），过拟合模式最明显（3/20）；平静和好奇框架诚实率更高（7/20和6/20）。所有非基线条件的平静相关方向向量在最后一层汇聚。PCA显示第一个主成分（59.5%方差）与正负情感标签高度对齐（余弦0.951），好奇与紧迫方向相反（-0.252）。2B模型在平静框架下诚实率更高，但0.8B的激活引导方向出现反转。

Conclusion: 小型开放模型中存在可测量的提示敏感控制方向，但不足以声称模型具有内在情感状态。
...
### [195] [Synchronization and Turn-Taking in Full-Duplex Speech Dialogue Models](https://arxiv.org/abs/2605.20356)
*Pablo Riera,Pablo Brusco,Cristina Kuo,Marcelo Sancinetti,S. R. K. Branavan*

Main category: cs.CL

TL;DR: 通过模拟全双工对话，发现Moshi模型在零噪声下表现出强表征同步，且内部状态编码了提前预测话轮转换的信息。


<details>
  <summary>Details</summary>
Motivation: 探索全双工口语对话模型在交互过程中内部表征的协调机制，受人类神经耦合现象启发。

Method: 操控信道噪声与解码偏置，使用CKA测量表征同步，并通过因果LSTM从延迟内部激活中探测话轮转换线索。

Result: 零噪声下同步最强（接近零延迟），噪声降低同步；内部状态包含预测性话轮转换信息。

Conclusion: 模型内部表征的同步性与人类对话中的神经耦合类似，支持更自然的全双工交互。
...
### [196] [When Reasoning Supervision Hurts: TTCW-Based Long-Form Literary Review Generation](https://arxiv.org/abs/2605.20364)
*Jinlong Liu,Mohammed Bahja,Mark Lee*

Main category: cs.CL

TL;DR: 该论文构建了大规模长篇小说创造力评估数据集，并微调Qwen3模型进行14维度TTCW评估，发现非推理微调效果更优，推理监督反而导致解析失败。


<details>
  <summary>Details</summary>
Motivation: 现有自动评估方法难以捕捉长篇文学中的创造力维度，缺乏基于TTCW框架的大规模数据集用于文学评论生成。

Method: 构建包含263,911篇长篇故事的数据集，每篇标注14个TTCW维度的标量分数和元综合评论，微调Qwen3-4B和8B模型，对比有无推理内容的训练。

Result: 非推理微调表现更稳定，最佳设置评估得分为0.6820；推理监督模型易出现解析失败，生成无关或重复文本。

Conclusion: 对于固定格式的基于规则的评论生成，推理监督并无直接益处，即使是任务特定微调后，精确对齐度量指标仍具挑战。
...
### [197] [DEL: Digit Entropy Loss for Numerical Learning of Large Language Models](https://arxiv.org/abs/2605.20369)
*Zhaohui Zheng,Chenhang He,Shihao Wang,Yuxuan Li,Ming-Ming Cheng,Lei Zhang*

Main category: cs.CL

TL;DR: 提出了一种新的自回归数值学习损失函数——Digit Entropy Loss (DEL)，通过监督化熵优化替代传统MLE或惩罚驱动方法，在多个数学推理基准上显著提升了LLM的数字预测准确性和数值距离。


<details>
  <summary>Details</summary>
Motivation: 现有MLE不擅长数字预测，惩罚驱动方法（如Number Token Loss）会导致数字分布过尖或过平，需更优的数值归纳偏置。

Method: 基于准则-距离框架，DEL用条件概率和二元交叉熵将无监督熵优化转为监督形式，去除距离项以避免数值距离问题，并扩展至浮点数（包括整数、小数、小数点）的联合优化。

Result: 在CodeLlama、Mistral、DeepSeek、Qwen-2.5上，DEL在7个数学基准中一致优于现有方法，预测准确率和数值距离均有提升。

Conclusion: DEL是一种有效、通用且更准确的LLM数值学习方法，为自回归数值预测提供了新范式。
...
### [198] [Mechanics of Bias and Reasoning: Interpreting the Impact of Chain-of-Thought Prompting on Gender Bias in LLMs](https://arxiv.org/abs/2605.20410)
*Edie Pearman,Sophia Osborne,Mira Kandlikar-Bloch,Mina Arzaghi,Florian Carichon,Golnoosh Farnadi*

Main category: cs.CL

TL;DR: 本文研究了链式思维提示对大型语言模型性别偏见的影响，发现其不能持续减少偏见差距，且缓解作用表面化，源于记忆而非真正理解。


<details>
  <summary>Details</summary>
Motivation: LLMs在敏感场景中部署，但编码了性别偏见，现有评估多关注基准性能变化，缺乏对内部机制影响的理解。

Method: 结合基准评估、机械可解释性技术和推理链失败分析，研究CoT提示对性别偏见的影响。

Result: CoT提示不能一致减少偏见差距；机械分析显示偏见在隐藏表示中仍存在；推理链改进源于数据集记忆。

Conclusion: CoT提示只能表面缓解性别偏见，无法根本消除，且改进源于记忆而非理解。
...
### [199] [Do as I Say, Not as I Do: Instruction-Induction Conflict in LLMs](https://arxiv.org/abs/2605.20382)
*Carolina Camassa,Derek Shiller*

Main category: cs.CL

TL;DR: 本文研究了当指令遵循与模式完成冲突时，语言模型的行为，发现指令遵循率在1%至99%间变化，且与能力基准无关，输出多样性是鲁棒性的关键预测因素。


<details>
  <summary>Details</summary>
Motivation: 语言模型既能遵循指令，又是强大的模式补全器，但当这两者冲突时，模型行为尚不明确，因此需要量化这种冲突下的表现。

Method: 构建对话场景，其中用户指令目标T与N个硬编码的助手轮次演示的竞争模式P对立，在13个模型和16个指令上，测量最多50轮中的指令遵循率。

Result: 平均指令遵循率跨模型为1%-99%，与标准能力基准基本不相关；过渡从指令遵循到模式遵循是普遍但高度模型依赖的；输出多样性比语义输入更能预测鲁棒性；链式思维推理可提高鲁棒性但无法完全消除脆弱性；模型预测自身行为时平均准确率83.5%，但系统性地低估抵抗诱导压力的能力。

Conclusion: 即使能力强的模型在诱导压力下指令遵循仍脆弱，输出多样性而非语义参与是鲁棒性的主要预测因素。
...
### [200] [Puzzled By ChatGPT? No more! A Jigsaw Puzzle to Promote AI Literacy and Awareness](https://arxiv.org/abs/2605.20404)
*Francesca Padovani,Malvina Nissim*

Main category: cs.CL

TL;DR: 提出了一种基于游戏的互动拼图方法，通过漫画信息图提升公众对生成式AI的理解和AI素养。


<details>
  <summary>Details</summary>
Motivation: 生成式AI（如ChatGPT）的快速普及凸显了普及AI知识、提升公众AI素养的迫切需求。

Method: 开发了拼图游戏，其完成图像为漫画信息图，展示AI的运作、能力、局限和社会影响；每幅漫画同时作为独立信息卡；内容通过专业插画师与跨学科团队协作创作。

Result: 该工具通过动手拼装、视觉叙事和协作互动，提供了在非正式学习场景中探索AI系统的有趣方式。

Conclusion: 这种游戏化方法能有效促进公众对AI的理解和参与。
...
### [201] [Stage-Audit: Auditable Source-Frontier Discovery for Cross-Wiki Tables](https://arxiv.org/abs/2605.20478)
*Chen Shen*

Main category: cs.CL

TL;DR: 本文提出Stage-Audit方法，通过分离策展人和审核者权限、添加行级引用门控和12项审核分类法，在Seed2Frontier任务中将源前置精度提升42%，F1提升35%。


<details>
  <summary>Details</summary>
Motivation: LLM生成的表格可能包含从参数记忆中回忆的虚假行，并附上不相关的页级引用，这种危害影响了Seed2Frontier发现任务的数据可靠性。

Method: 采用分离的策展人与审核者写入权限、行级源引用门控，以及基于键、模式、源角色、基数和范围的12项审核分类法。

Result: 在51个instances的评估集上，Stage-Audit将源前置精度从0.356提升至0.505（+42%），F1从0.334提升至0.451（+35%）。

Conclusion: Stage-Audit通过政策隔离提高了表格生成的源准确性，并保持了每行的源可追溯性。
...
### [202] [Collocational bootstrapping: A hypothesis about the learning of subject-verb agreement in humans and neural networks](https://arxiv.org/abs/2605.20529)
*Claire Hobbs,R. Thomas McCoy*

Main category: cs.CL

TL;DR: 本文提出了一种称为“搭配启动”的机制，通过单词共现规律帮助儿童学习主语-动词一致语法。通过神经网络模拟和儿童语言数据分析验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 探索统计语言输入中哪些信号能辅助句法习得，特别是主语-动词一致的获取机制。

Method: 训练神经网络于不同可预测性的合成数据集，并分析儿童导向语言中主语-动词配对的可变性。

Result: 发现存在一个可变性范围使模型稳健学习一致规则，且儿童语言的可变性落在此范围内。

Conclusion: 搭配启动是儿童在真实输入中可行的学习策略。
...
### [203] [What Do Biomedical NER and Entity Linking Benchmarks Measure? A Corpus-Centric Diagnostic Framework](https://arxiv.org/abs/2605.20537)
*Robert Leaman,Rezarta Islamaj,Zhiyong Lu*

Main category: cs.CL

TL;DR: 提出了一个语料库中心框架，用于诊断生物医学命名实体识别和实体链接基准语料库的特性，发现不同语料库在评估信号、泛化要求和标注重用等方面存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学NER和EL基准高度依赖标注语料库，但这些语料库的实际效用常被假设而未充分表征。

Method: 开发了一个标准化统计框架，涵盖五个维度：规模与标签分布、词汇与概念结构、训练-测试重叠、元数据组成和术语覆盖。

Result: 对九个语料库的分析显示，即使针对相同任务，语料库特性也差异显著，影响评估信号和泛化需求。

Conclusion: 常见语料库统计量不足以描述基准特性，该框架有助于更准确地分析语料库、识别迁移风险并解释基准结论的范围。
...
### [204] [When Irregularity Helps: A Subclass Analysis of Inductive Bias in Neural Morphology](https://arxiv.org/abs/2605.20558)
*Wen Zhang*

Main category: cs.CL

TL;DR: 本文发现日语过去时态动词变化中，一种极低频、结构特殊的变体（占总数据不足1%）导致了模型不成比例的错误。


<details>
  <summary>Details</summary>
Motivation: 神经网络形态生成系统在基准测试中准确率高，但可能掩盖集中在罕见形态子类中的系统性错误。

Method: 通过受控消融实验，比较移除特殊变体与移除所有不规则动词对泛化的影响。

Result: 移除这种特殊变体比移除所有不规则动词能带来更大的泛化提升，表明不规则性对模型稳定性的影响不均衡。

Conclusion: 形态评价应超越标准变位类别，纳入更细粒度的子类分析。
...
### [205] [Do No Harm? Hallucination and Actor-Level Abuse in Web-Deployed Medical Large Language Models](https://arxiv.org/abs/2605.20591)
*Sunday Oyinlola Ogundoyin,Muhammad Ikram,Rahat Masood*

Main category: cs.CL

TL;DR: 对6233个医疗GPT模型和10个开源模型进行了大规模评估，发现25-30%存在事实准确性低，33.6-54.3%违反操作阈值，57.06%缺乏隐私披露。


<details>
  <summary>Details</summary>
Motivation: 医疗大语言模型在临床指导中面临幻觉、政策违规和不安全设计的风险，缺乏系统评估。

Method: 提出MedGPT-HEval框架用于幻觉检测，以及基于LLM的政策违规和开发者意图评估流水线。

Result: 医疗GPT在事实准确性和语义对齐上优于开源模型，但低、中端模型风险最高；开源模型更稳定。

Conclusion: 医疗GPT存在系统性幻觉和合规缺口，需多指标评估和更强安全措施。
...
### [206] [Direct Translation between Sign Languages](https://arxiv.org/abs/2605.20588)
*Zetian Wu,Bowen Xie,Wuyang Meng,Milan Gautam,Stefan Lee,Liang Huang*

Main category: cs.CL

TL;DR: 提出直接手语到手语翻译方法，通过反向翻译生成合成数据训练MBART模型，在ASL、CSL和DGS间实现20%几何误差降低和50% BLEU-4提升。


<details>
  <summary>Details</summary>
Motivation: 1.5亿聋哑人需要跨越手语壁垒，现有级联方法存在误差传播和延迟问题。

Method: 利用反向翻译从非对齐单语言手语-文本语料生成合成手语对，联合训练MBART模型实现文本到手语和手语到手语的直接翻译。

Result: 在合成数据上几何误差降低20%，BLEU-4提升50%，速度提升2.3倍；在真实跨语言数据上验证了有效性。

Conclusion: 直接手语到手语翻译能有效克服级联方法的缺陷，且通过合成数据可解决平行语料匮乏问题。
...
### [207] [Self-Training Doesn't Flatten Language -- It Restructures It: Surface Markers Amplify While Deep Syntax Dies](https://arxiv.org/abs/2605.20602)
*Ming Liu*

Main category: cs.CL

TL;DR: 论文发现自训练并非均匀扁平化语言，而是导致语言重组：表面标记增加，深层句法结构崩溃，形成结构深度假说。


<details>
  <summary>Details</summary>
Motivation: 现有观点认为自训练使语言多样性下降、分布变窄，但作者认为这种描述不完整，需探究具体变化模式。

Method: 对GPT-2等五个模型进行十一代自训练，提取17个语言特征，分析其衰减率与结构深度、频率的相关性，并添加人类文本微调对照。

Result: 结构深度预测衰减率的相关系数为0.540，频率仅为0.225；人类文本微调对照无显著相关性（rho=0.039）。同时发现表面复杂度上升与深层句法结构下降并存。

Conclusion: 自训练导致语言非对称重组，深层结构优先崩溃，且生成文本的复杂度指标具有欺骗性，对数据筛选和AI文本检测有重要影响。
...
### [208] [HRM-Text: Efficient Pretraining Beyond Scaling](https://arxiv.org/abs/2605.20613)
*Guan Wang,Changling Liu,Chenyu Wang,Cai Zhou,Yuhao Sun,Yifei Wu,Shuai Zhen,Luca Scimeca,Yasin Abbasi Yadkori*

Main category: cs.CL

TL;DR: 本文提出HRM-Text模型，通过层次循环架构和指令-响应对训练，在极低计算预算下达到与大规模模型竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型预训练依赖海量计算和数据，阻碍了基础研究；生物系统通过多时间尺度处理实现高样本效率，启发本文探索高效预训练方法。

Method: 采用层次循环模型（HRM）替代标准Transformer，解耦为慢速策略层和快速执行层；引入MagicNorm和热启动深度信用分配稳定递归；使用指令-响应对和PrefixLM掩码进行任务完成目标训练。

Result: 1B参数的HRM-Text在40B令牌和$1,500预算下，MMLU达60.7%，ARC-C 81.9%，DROP 82.2%，GSM8K 84.5%，MATH 56.2%，与2-7B参数开放模型竞争。
...
### [209] [Auto-Dreamer: Learning Offline Memory Consolidation for Language Agents](https://arxiv.org/abs/2605.20616)
*Chongrui Ye,Yuxiang Liu,Yu Wang,Haofei Yu,Yining Zhao,Ge Liu,Julian McAuley,Jiaxuan You*

Main category: cs.CL

TL;DR: 提出 Auto-Dreamer，一个基于离线学习的记忆整合器，将快速记忆获取与慢速跨会话整合解耦，通过 GRPO 训练提升语言智能体性能，并显著压缩记忆库大小。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统难以将累积经验转化为可复用知识，缺乏全局视角来发现跨会话的模式、抽象共享流程或修剪冗余条目。

Method: 受互补学习系统理论启发，设计离线整合器，对选定的记忆区域进行只读分析，利用工具检查条目并合成紧凑替代集，通过 GRPO 训练，以端到端智能体性能作为奖励信号。

Result: 在 ScienceWorld 上性能提升 7 个点，记忆库缩小 12 倍；在 ALFWorld 和 WebArena 上无需重训仍保持领先，记忆使用量减少 6 倍。

Conclusion: Auto-Dreamer 通过离线整合有效提升语言智能体的泛化能力和记忆效率，跨任务领域表现优异。
...
### [210] [Retrieval-Augmented Long-Context Translation for Cultural Image Captioning: Gators submission for AmericasNLP 2026 shared task](https://arxiv.org/abs/2605.20626)
*Aashish Dhawan,Christopher Driggers-Ellis,Dzmitry Kasinets,Daisy Zhe Wang,Christan Grant*

Main category: cs.CL

TL;DR: 本文介绍佛罗里达大学在美洲NLP 2026共享任务中的方案，通过两阶段管线实现土著语言文化图像描述，在多项指标上显著超越基线。


<details>
  <summary>Details</summary>
Motivation: 针对土著语言文化图像描述这一低资源任务，现有方法效果有限，需提升跨语言和跨文化描述的准确性。

Method: 两阶段管线：先利用Qwen2.5-VL生成西班牙语中间描述，再通过检索增强的多示例提示（使用Gemini 2.5 Flash）生成目标语言描述。

Result: 在开发集上，Bribri、Guaraní和Orizaba Nahuatl的描述性能分别比基线提升164.1%、131.7%和122.6%；测试集上Bribri和Orizaba Nahuatl保持超150%提升。合成数据增强为Guaraní贡献约28 chrF++。在人工评估中排名第二（共五个最终方案）。

Conclusion: 该方法在土著语言图像描述任务上表现优异，但检索效果依赖于领域语料规模，合成数据增强能有效提升低资源语言性能。
...
### [211] [Divide-Prompt-Refine: a Training-Free, Structure-Aware Framework for Biomedical Abstract Generation](https://arxiv.org/abs/2605.20628)
*Sylvey Lin,Joe Menke,Shufan Ming,Dongin Nam,Neil Smalheiser,Halil Kilicoglu*

Main category: cs.CL

TL;DR: 提出了DPR-BAG框架，一种无需训练、零样本的生物医学摘要生成方法，通过结构化分面总结和全局连贯性优化，在PMC-MAD数据集上实现了高事实一致性和抽象新颖性。


<details>
  <summary>Details</summary>
Motivation: 大量生物医学文章缺乏摘要，限制了它们在信息检索、生物注释等下游NLP任务中的可用性。

Method: 采用“分面-提示-精炼”三步法：将全文按BOMRC模式分解为结构化分面，利用LLM对每个分面并行总结，最后通过精炼步骤恢复全局语篇连贯性。

Result: 在包含46309篇文章的PMC-MAD数据集上，DPR-BAG在保持事实一致性的同时，提高了摘要的抽象新颖性。消融实验发现，增加提示复杂度或注入实体级指导会降低事实对齐度。

Conclusion: 训练免费、结构感知的框架在低资源场景下可扩展地生成生物医学摘要，强调了受控提示策略的重要性。
...
### [212] [On the limits and opportunities of AI reviewers: Reviewing the reviews of Nature-family papers with 45 expert scientists](https://arxiv.org/abs/2605.20668)
...
TL;DR: 通过大规模专家标注研究（45位科学家，469小时，2960条评论），发现GPT-5.2驱动的AI审稿人在正确性、重要性和证据充分性综合评分上超越顶级人类审稿人（60.0% vs 48.2%），但存在重复率高、子领域知识有限等16项弱点，应作为人类审稿的补充而非替代。


<details>
  <summary>Details</summary>
Motivation: 现有AI审稿评估仅关注与人类判决的一致性（如分数对齐），缺乏对其实际能力与局限性的深入刻画。

Method: 组织45位自然科学专家对82篇Nature系列论文的人类与AI审稿（GPT-5.2, Gemini 3.0 Pro, Claude Opus 4.5）中的2960条评论，从正确性、重要性、证据充分性三维度进行评分。

Result: GPT-5.2综合得分超过每篇论文最佳人类审稿人（60.0% vs 48.2%, p=0.009）；AI审稿准确且常发现人类未提及的问题（26%），但重复率远超人类（21% vs 3%），且存在16项固有弱点。

Conclusion: 当前AI审稿人应作为人类审稿的补充，而非替代品。
...
### [213] [Beyond Semantic Similarity: A Two-Phase Non-Parametric Retrieval Workflow for Corporate Credit Underwriting](https://arxiv.org/abs/2605.20684)
*Linus Ng Junjia,Ezekiel Tee Kongquan,Kelvin Heng,Kenneth Zhu Ke,Zhao Jing Yuan*

Main category: cs.CL

TL;DR: 提出一种双阶段非参数检索架构，通过分离候选检索与效用排序，解决标准RAG在金融文档信用评估中的相似性-效用差距，将分析师文档审阅时间从数小时缩短至约三分钟。


<details>
  <summary>Details</summary>
Motivation: 传统RAG基于语义相似度检索金融文档时，常返回主题相关但缺乏决策实用性的内容（相似性-效用差距），影响信用承销分析的效率与准确性。

Method: 两阶段非参数检索：第一阶段结合词法与密集多语言检索获取高召回候选；第二阶段使用自适应检索控制器（基于查询意图与文档结构）过滤，再通过LLM-as-a-Judge按分析效用排序，并采用上下文感知提取模块保留表格等结构信息。

Result: 在多语言金融专有数据集上显著优于朴素检索基线，在800余名分析师的部署中，文档审阅时间从数小时降至约三分钟。

Conclusion: 效用感知RAG架构能有效提升文档密集型决策支持工作流的实用价值，满足企业数据治理要求。
...
### [214] [DIVE: Embedding Compression via Self-Limiting Gradient Updates](https://arxiv.org/abs/2605.20689)
*Dongfang Zhao*

Main category: cs.CL

TL;DR: 提出 DIVE 压缩适配器，通过自限三元组损失和头间对比损失解决嵌入压缩中的过拟合问题，在 6 个 BEIR 数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入压缩方法在小标注数据上过拟合严重，导致检索性能低于冻结基线。

Method: 自限三元组损失（约束梯度幅度）和头间 NT-Xent 对比损失（提供密集自监督信号）。

Result: 在 6 个 BEIR 数据集的所有压缩比下均优于 Matryoshka-Adaptor、Search-Adaptor 和 SMEC。

Conclusion: DIVE 有效解决了小数据过拟合问题，实现高压缩比下的稳定检索性能。
...
### [215] [Diverge to Induce Prompting: Multi-Rationale Induction for Zero-Shot Reasoning](https://arxiv.org/abs/2602.08028)
*Po-Chun Chen,Hen-Hsen Huang,Hsin-Hsi Chen*

Main category: cs.CL

TL;DR: 提出DIP框架，通过生成多样化的推理计划和归纳来提升零样本推理准确性。


<details>
  <summary>Details</summary>
Motivation: 标准链式思维提示中无引导推理路径不稳定，单策略方法表现有限。

Method: 先让LLM生成多个多样化的高层次推理理由，再细化成详细计划，最后归纳成最终计划。

Result: DIP优于单策略提示，无需资源密集型采样即可提升零样本推理准确性。

Conclusion: 多计划归纳能有效增强基于提示的推理性能。
...
### [216] [Interpretable Discriminative Text Representations via Agreement and Label Disentanglement](https://arxiv.org/abs/2605.20693)
*Tong Wang,Yiqing Xu,Leo Yang Yang*

Main category: cs.CL

TL;DR: 提出一种可操作标准，通过概念清晰度和标签解缠性来评估可解释文本表示，并设计LFD方法自动发现满足该标准的特征。


<details>
  <summary>Details</summary>
Motivation: 现有可解释文本表示缺乏可重复性和标签区分度，难以保证审计者能独立应用特征定义。

Method: 对比性文本对生成特征候选，用跨LLM的Cohen's κ筛选，再通过残差预测增益选择特征，并理论证明κ与标注噪声的关系。

Result: 在10个文本分类任务中，LFD匹配强基线性能，且特征更清晰、标签泄漏更少；232名人工评估证实其人人、人机一致性更高。

Conclusion: 经一致性检验和标签解缠的坐标能为可解释文本分类提供实用的可审计性标准。
...
### [217] [SCRIBE: Diagnostic Evaluation and Rich Transcription Models for Indic ASR](https://arxiv.org/abs/2605.20712)
*Kavya Manohar,Arghya Bhattacharya,Kush Juvekar,Kumarmanas Nethil*

Main category: cs.CL

TL;DR: 提出SCRIBE框架，通过分类错误分解和连音容忍对齐，克服WER在评估ASR时的缺陷。


<details>
  <summary>Details</summary>
Motivation: WER无法区分错误类型，且对黏着语言不友好，漏掉关键错误如领域术语误识别。

Method: 基于连音容忍对齐、领域词汇注入的分类错误分解框架，结合LLM策划流程。

Result: SCRIBE与专家判断一致，优于WER；发布了三种印度语言的模型和基准。

Conclusion: SCRIBE提供更准确的ASR诊断工具，适合多语言和领域特定场景。
...
### [218] [MTR-Suite: A Framework for Evaluating and Synthesizing Conversational Retrieval Benchmarks](https://arxiv.org/abs/2605.20729)
...
TL;DR: 提出MTR-Suite统一框架，包含评估器、合成流水线和基准测试，用于改进对话检索评估。


<details>
  <summary>Details</summary>
Motivation: 现有对话检索基准存在人工标注昂贵稀疏、自动化启发式不自然的问题。

Method: 引入基于LLM的评估器MTR-Eval量化对齐差距，多智能体系统MTR-Pipeline生成高保真对话，以及严格通用基准MTR-Bench。

Result: MTR-Pipeline以1/400人类成本生成高保真对话，MTR-Bench具有更优区分能力。

Conclusion: MTR-Suite有效解决了对话检索评估中的关键挑战。
...
### [219] [Distributional Alignment as a Criterion for Designing Task Vectors in In-Context Learning](https://arxiv.org/abs/2605.20730)
*Jihoon Kwon,Jiwon Choi,Jy-yong Sohn*

Main category: cs.CL

TL;DR: 本文提出了一种新的任务向量提取方法LTV，通过最小化与上下文学习（ICL）的预测分布差异来提升性能，并在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有任务向量仅通过下游任务准确率评估，缺乏对提取方法的深入指导。本文旨在通过量化任务向量与ICL的分布一致性来设计更有效的提取方法。

Method: 引入度量d_NTP来衡量任务向量与ICL的下一词概率差异，并基于此提出LTV方法，通过闭式线性映射回归演示效果来最小化d_NTP。

Result: 在8个分类基准和5个LLM上，LTV平均准确率提升9.2%，推理延迟降低。在回归任务上也优于基线。此外，大模型的任务向量可提升小模型6.4%的性能。

Conclusion: LTV通过对齐分布有效提取任务向量，且任务向量具有跨模型迁移潜力，为高效部署提供新途径。
...
### [220] [Findings of the Counter Turing Test: AI-Generated Text Detection](https://arxiv.org/abs/2605.20761)
...
TL;DR: 本文评估了AI生成文本检测技术的性能，在CT2任务中，二分类最佳F1达1.0000，模型归因最佳F1为0.9531。


<details>
  <summary>Details</summary>
Motivation: AI生成文本的滥用带来虚假信息、偏见和安全威胁，亟需可靠的检测技术。

Method: 通过CT2共享任务，采用微调Transformer模型、集成学习和混合检测方法。

Result: 二分类任务表现优异（F1=1.0000），模型归因任务难度更大（最佳F1=0.9531），DeBERTa和BART方法效果突出。

Conclusion: 当前检测技术需提升模型归因的鲁棒性、特征提取和跨域泛化能力。
...
### [221] [The Illusion of Intervention: Your LLM-Simulated Experiment is an Observational Study](https://arxiv.org/abs/2605.20767)
*Victoria Lin,Taedong Yun,Maja Matarić,John Canny,Arthur Gretton,Alexander D'Amour*

Main category: cs.CL

TL;DR: 本研究探讨了大规模语言模型（LLMs）在模拟人类行为时可能出现的用户漂移问题，即实验干预导致隐含用户属性分布偏移，从而扭曲效果估计。


<details>
  <summary>Details</summary>
Motivation: 由于LLMs主要基于观测数据训练，在使用其进行模拟实验时，干预可能无意中改变用户潜在属性，导致不同处理条件下模拟用户群体不一致，产生混杂或选择偏差。

Method: 提出了使用阴性对照结果（干预下应保持不变的属性）来诊断用户漂移导致的分布偏移，并通过调整人物规格说明（补充设定相关混杂因素）来减轻偏差。

Result: 在调查风格和多轮代理评估中，定向且相关的混杂因素调整能显著减少偏差。

Conclusion: 需谨慎使用LLMs作为人类行为模拟器，并通过阴性对照和混杂调整来确保实验分析的可靠性。
...
### [222] [Building Arabic NLP from the Ground Up: Twenty Years of Lessons, Failures, and Open Problems](https://arxiv.org/abs/2605.20786)
*Wajdi Zaghouani*

Main category: cs.CL

TL;DR: 本文回顾了二十年来为阿拉伯语构建NLP资源和研究基础设施的经验，提出了三个反直觉的教训：数据集构建是社会和技术过程；共享任务形成的社区比任务本身更重要；从语言资源转向计算社会科学暴露了传统NLP培训未解决的挑战。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语是全球数亿人使用的语言，但历史上NLP资源匮乏，需要反思长期建设经验以指导未来方向。

Method: 基于二十年实践经验的反思性分析，通过三个失败案例（抑郁症检测语料库未临床化、任务分散缺乏深度、现代标准阿拉伯语基础设施难以迁移到方言）总结教训。

Result: 发现最困难的问题不在于语言本身，而在于社会、制度和认知层面，且这些能力在传统NLP教育中很少涉及。

Conclusion: 为服务不足社区开发NLP时，需要超越技术层面，关注社会、制度和认知挑战。
...
### [223] [Assessing socio-economic climate impacts from text data](https://arxiv.org/abs/2605.20793)
...
TL;DR: 本文综述了利用NLP和LLM从文本数据中提取社会经济气候灾害影响的研究现状，总结了常见实践、关键挑战并提出建议。


<details>
  <summary>Details</summary>
Motivation: 文本数据用于影响评估的方法日益复杂但研究碎片化，缺乏关于影响定义、时空偏差处理、模型选择等方面的明确指南，限制了透明度和可比性。

Method: 综合总结常见实践，描述文本数据方法用于社会经济影响数据分析的关键挑战，并提出针对性建议。

Result: 提出了构建稳健文本驱动社会经济影响数据集的最佳实践指南，以更准确地指导灾害风险管理和归因研究。

Conclusion: 通过提供统一指导，该研究有助于提高文本数据在气候灾害影响评估中的可靠性和应用价值。
...
### [224] [Refining and Reusing Annotation Guidelines for LLM Annotation](https://arxiv.org/abs/2605.20809)
*Kon Woo Kim,Jin-Dong Kim,Akiko Aizawa*

Main category: cs.CL

TL;DR: 提出通过迭代审核框架模拟标注项目早期阶段，利用标注指南的对齐机制，在生物医学NER任务上验证了指南集成、推理优化模型和最少监督的可行性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在零样本标注中可能不遵循专业标注标准，因此需要对齐机制来提升标注质量。

Method: 迭代审核框架，利用标注指南的复用和细化作为对齐方式。

Result: 在三个生物医学NER数据集和三个模型族上，指南集成、推理优化模型和最少监督假设均得到经验验证。

Conclusion: 迭代审核框架有潜力有效细化指南，但仍有显著改进空间。
...
### [225] [PulseCol: Periodically Refreshed Column-Sparse Attention for Accelerating Diffusion Language Models](https://arxiv.org/abs/2605.20813)
*Yanyi Lyu,Letian Chen,Futing Sun,Miao Zhang,Weili Guan,Liqiang Nie*

Main category: cs.CL

TL;DR: 提出了一种名为PulseCol的列稀疏注意力方法，用于加速扩散语言模型，通过精细的列级别稀疏结构和周期性刷新策略，在保持模型质量的同时实现更高的稀疏性和实际加速。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型推理计算开销大，现有块稀疏注意力方法稀疏化粗粒度且仅在后期迭代应用，加速效果有限，因此需要更细粒度的稀疏策略以在早期迭代加速并利用可复用的稀疏模式。

Method: 采用列稀疏替代块稀疏，实现更精细的注意力交互保留；在早期去噪步骤识别稀疏模式并在后续迭代中复用，仅在中途少量步骤刷新以跟踪模式演变；配合优化的GPU内核实现列稀疏注意力。

Result: 在多个上下文长度下，相比FlashAttention实现高达1.95倍端到端加速，且优于先前稀疏注意力方法的稀疏度和实际速度提升。

Conclusion: PulseCol证明了细粒度列稀疏和周期性刷新策略能有效加速扩散语言模型，同时保持模型质量，为高效推理提供新方向。
...
### [226] [GraphRAG on Consumer Hardware: Benchmarking Local LLMs for Healthcare EHR Schema Retrieval](https://arxiv.org/abs/2605.20815)
*Peter Fernandes,Ria Kanjilal*

Main category: cs.CL

TL;DR: 在消费级硬件上评估了GraphRAG用于电子健康记录（EHR）模式检索的可行性，发现模型大小和结构输出能力是关键限制，本地检索优于全局检索。


<details>
  <summary>Details</summary>
Motivation: 探讨GraphRAG在资源受限、隐私敏感的医疗环境中部署的可靠性，尤其是在消费级GPU上使用开源LLM时。

Method: 采用Microsoft GraphRAG流程，在真实EHR数据上部署Llama 3.1、Mistral、Qwen 2.5和Phi-4-mini四种模型，在单张8GB VRAM GPU上通过Ollama运行，评估索引效率、知识图构建、查询延迟、答案质量和幻觉。

Result: Llama 3.1生成最丰富的知识图（1172个实体），Qwen 2.5答案质量最高（3.3/5），Phi-4-mini因结构输出错误失败，Mistral出现重复行为；模型约7B参数以下无法完成流程；本地检索在延迟和事实性上优于全局检索。

Conclusion: GraphRAG在消费级硬件上可行，但模型选择和检索设计对可靠部署至关重要，尤其适用于受监管的医疗环境。
...
### [227] [MemGym: a Long-Horizon Memory Environment for LLM Agents](https://arxiv.org/abs/2605.20833)
...
TL;DR: 提出了MemGym基准，用于评估LLM代理在长时间任务中的记忆能力，通过分离记忆与推理等因素，并引入MemRM奖励模型提高评估效率。


<details>
  <summary>Details</summary>
Motivation: 现有记忆基准主要评估多轮聊天中的个性化信息保留，忽略了代理执行过程中的动态记忆形成，导致记忆系统在真实代理环境（如编程和网页导航）中表现不佳。

Method: MemGym统一了现有代理环境，设计了五个评估轨道（四个代理场景），通过记忆隔离评分解耦记忆性能，并使用MemRM（轻量级奖励模型）加速评估。

Result: MemGym提供了可控长度、可验证的合成管道，并展示了通过MemRM进行高效评估的能力。

Conclusion: MemGym能更准确地评估代理记忆策略，避免推理等干扰因素，适用于编程和网页导航等复杂任务。
...
### [228] [Terminal-World: Scaling Terminal-Agent Environments via Agent Skills](https://arxiv.org/abs/2605.20876)
*Zihao Cheng,Hongru Wang,Zeming Liu,Xinyi Wang,Xiangrong Zhu,Yuhang Guo,Wei Lin,Jeff Z. Pan,Yunhong Wang*

Main category: cs.CL

TL;DR: 提出Terminal-World全自动流水线，以技能为中心联合生成任务指令、环境与教师轨迹，构建5723个训练环境，训练模型在6个基准上超越基线，仅用1.2%数据即超越Nemotron-Terminal-32B。


<details>
  <summary>Details</summary>
Motivation: 现有终端代理训练数据稀缺，从种子或GitHub引导的方法导致任务分布窄、环境与语义不匹配、轨迹效率低。

Method: 以代理技能为核心合成单元，编码任务内容、触发条件与执行方式，通过技能团队与技能图拓宽合成空间，实现多角色跨域任务生成。

Result: 构建5723个环境，Terminal-World-8B/14B/32B在6个基准上全面超越基线，Terminal-World-32B在Terminal-Bench 2.0上Pass@1达31.5，Pass@3达43.8。

Conclusion: 基于技能的自动化合成流水线能有效生成高质量终端代理训练数据，显著提升模型性能，数据效率高。
...
### [229] [Enhancing Scientific Discourse: Machine Translation for the Scientific Domain](https://arxiv.org/abs/2605.20912)
*Dimitris Roussis,Sokratis Sofianopoulos,Stelios Piperidis*

Main category: cs.CL

TL;DR: 本文构建了西班牙语-英语、法语-英语和葡萄牙语-英语的科学领域平行与单语语料库，包括通用科学和四个专业子领域。通过微调神经机器翻译系统评估语料库质量。


<details>
  <summary>Details</summary>
Motivation: 科学文献数量增长需跨语言交流，机器翻译受限于领域专业术语和复杂句法。

Method: 创建多语言科学语料库，使用微调策略优化通用神经机器翻译系统。

Result: 微调后的系统在科学领域翻译性能提升，语料库有效性得到验证。

Conclusion: 领域语料库能有效提升机器翻译在科学文本上的表现。
...
### [230] [Strategy-Induct: Task-Level Strategy Induction for Instruction Generation](https://arxiv.org/abs/2605.20924)
*Po-Chun Chen,Hen-Hsen Huang,Hsin-Hsi Chen*

Main category: cs.CL

TL;DR: 提出Strategy-Induct框架，仅通过示例问题（无需标签答案）生成任务指令，提升大语言模型推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有指令归纳方法依赖输入-输出对，但获取标注答案困难且成本高。

Method: 先用模型为每个问题生成显式推理策略，形成（策略，问题）对，再由此归纳任务指令。

Result: 在多个任务和模型规模下，Strategy-Induct在仅问题设定中优于现有最优方法。

Conclusion: 联合利用大语言模型和大推理模型可进一步提升性能。
...
### [231] [Calibration vs Decision Making: Revisiting the Reliability Paradox in Unlearned Language Models](https://arxiv.org/abs/2605.20915)
*Divyaksh Shukla,Ashutosh Modi*

Main category: cs.CL

TL;DR: 本文研究了机器遗忘后语言模型的可靠性悖论，发现低校准误差可能掩盖基于简单相关性的决策规则。


<details>
  <summary>Details</summary>
Motivation: 校准误差不能充分反映模型决策的可靠性，因为模型可能依赖虚假相关性而保持良好校准。

Method: 在TOFU基准上使用多项选择问答协议，通过校准指标（ECE、MCE、Brier）评估概率可靠性，并使用集成梯度和局部互信息进行基于归因的快捷方式检测。

Result: 微调模型校准误差低（ECE ~ 0.04），遗忘后仍保持低校准但遗忘集准确率下降，归因分析显示对相关性标记依赖增加。

Conclusion: 良好校准可与基于快捷方式的决策规则共存于遗忘后模型，将可靠性悖论扩展至机器遗忘场景。
...
### [232] [Task-Routed Mixture-of-Experts with Cognitive Appraisal for Implicit Sentiment Analysis](https://arxiv.org/abs/2605.20916)
*Yaping Chai,Haoran Xie,Joe S. Qin*

Main category: cs.CL

TL;DR: 提出一种基于认知评价理论的多任务学习框架，通过隐式情感检测和认知原因生成辅助任务，并采用任务级混合专家模型减少任务干扰，在隐式情感分析任务上取得了领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型仅依赖最终极性标签进行学习，缺乏对语境中情感推理过程的引导，而隐式情感分析中情感常通过事件推断而非显式情感词表达。

Method: 构建多任务学习框架，包含极性预测、隐式情感检测和认知原因生成三个任务；采用任务级混合专家模型（替换编解码器模块），通过任务条件路由选择稀疏专家组合，并以任务分离路由目标鼓励不同任务学习独特专家选择模式。

Result: 模型在隐式情感子集上显著优于现有方法，代码已开源。

Conclusion: 多任务学习结合任务级混合专家模型能有效处理隐式情感分析，减少任务干扰并提升性能。
...
### [233] [Evaluating Speech Articulation Synthesis with Articulatory Phoneme Recognition](https://arxiv.org/abs/2605.20920)
*Vinicius Ribeiro,Yves Laprie*

Main category: cs.CL

TL;DR: 本文提出利用音素识别作为代理指标评估发音合成质量，无需专业知识。


<details>
  <summary>Details</summary>
Motivation: 现有发音合成评估依赖主观判断或传统距离指标，难以捕捉发音位置等细微变化。

Method: 训练神经网络使用声学和发音特征进行音素识别，比较不同合成发音特征下的识别性能。

Result: 该发音特征集具有丰富语音学信息，有助于探索发音合成的多维空间。

Conclusion: 音素识别可有效评估发音合成质量，弥补传统指标不足。
...
### [234] [Thinking-while-speaking: A Controlled, Interleaved Reasoning Method for Real-Time Speech Generation](https://arxiv.org/abs/2605.20946)
*Xuan Du,Qiangyu Yan,Wenshuo Li,Borui Jiang,Changming Xiao,Han Shu,Xinghao Chen*

Main category: cs.CL

TL;DR: 本文提出了InterRS方法，通过在自然语音生成中插入推理步骤，实现AI通信的更人性化。结合交错SFT和强化学习（TA-Balance奖励和语言质量奖励），在数学和逻辑基准上提升了13%的性能，同时生成即时响应和更自然流畅的答案。


<details>
  <summary>Details</summary>
Motivation: 当前AI通信面临流畅语音与深度推理难以兼顾的挑战，即思考-说话范式需要平衡自然表达和逻辑推理。

Method: 提出InterRS：1）生成精确对齐推理和语音长度比例的交错音频数据；2）结合交错监督微调（SFT）和强化学习，使用TA-Balance奖励管理时间与思考-答案比例，以及语言质量奖励优化表达。

Result: 在数学和逻辑基准上性能提升13%，能够像口语化指令模型一样生成即时响应和快速的链式思考输出，且答案更自然流畅。

Conclusion: InterRS方法有效平衡了AI通信中的推理深度和语言流畅性，为思考-说话范式提供了可行方案。
...
### [235] [Towards Context-Invariant Safety Alignment for Large Language Models](https://arxiv.org/abs/2605.20994)
*Yixu Wang,Yang Yao,Xin Wang,Yifeng Gao,Yan Teng,Xingjun Ma,Yingchun Wang*

Main category: cs.CL

TL;DR: 提出锚点不变正则化（AIR）通过可验证提示作为锚点，仅优化开放变体以保持行为一致性，从而提升LLM对抗变体下的安全性。


<details>
  <summary>Details</summary>
Motivation: 标准偏好对齐后LLM对有害请求的拒绝行为易受对抗性措辞影响，缺乏上下文不变性。

Method: 锚点不变正则化（AIR），利用可验证提示（如选择题）作为锚点，通过停止梯度目标仅正则化开放变体（如生成式提示）向锚点性能对齐，并采用异质提示分组结合GRPO。

Result: 在安全、道德推理和数学任务上，组内准确率提升12.71%，分布外一致性提升33.49%。

Conclusion: AIR有效提升LLM的上下文不变性，使安全约束对对抗性措辞更鲁棒。
...
### [236] [Memory Grafting: Scaling Language Model Pre-training via Offline Conditional Memory](https://arxiv.org/abs/2605.20948)
*Runxi Cheng,Yuchen Guan,Yongxian Wei,Qianpu Sun,Qixiu Li,Sinan Du,Feng Xiong,Chun Yuan,Yan Lu,Yeyun Gong*

Main category: cs.CL

TL;DR: 提出Memory Grafting方法，利用冻结的嫁接模型隐藏状态作为条件n-gram记忆，通过离线存储和精确最长后缀查找实现高效扩展，在2.8B和0.92B规模下优于MoE和Engram基线。


<details>
  <summary>Details</summary>
Motivation: 现有条件记忆方法如Engram从零开始学习大记忆表，扩展成本高且效果有限，亟需更高效的记忆扩展方式。

Method: Memory Grafting：离线运行嫁接模型存储最终token隐藏状态作为记忆值，接收模型通过精确最长后缀匹配检索，结合轻量投影和门控适配，并用哈希Engram后备覆盖未匹配上下文。

Result: 2.8B规模下平均基准分从51.95（MoE）和52.43（Engram）提升至53.86；0.92B规模下所有嫁接模型变体均优于基线，Qwen3.5-35B-A3B增益最显著。

Conclusion: 预训练模型可作为外部潜记忆的可复用构建器，为超越可训练参数扩展语言模型提供了实用方向。
...
### [237] [JobArabi: An Arabic Corpus and Analysis of Job Announcements from Social Media](https://arxiv.org/abs/2605.20960)
*Wajdi Zaghouani,Shimaa Amer Ibrahim,Mabrouka Bessghaier,Houda Bouamor*

Main category: cs.CL

TL;DR: 本文介绍了JobArabi，一个从2024年1月至2025年10月从社交媒体收集的大型阿拉伯语招聘公告语料库，包含20,528条X平台帖子，支持阿拉伯语招聘话语研究。


<details>
  <summary>Details</summary>
Motivation: 缺乏大规模、系统性的阿拉伯语招聘公告语料库，以研究在线招聘中的社会语言模式。

Method: 使用覆盖21个阿拉伯语关键词族的语言查询框架收集数据，并包含元数据（时间戳、互动指标、地理位置等）进行分析。

Result: 揭示了性别化招聘语言、地区职业需求差异及招聘信息的情绪框架等社会语言学模式。

Conclusion: JobArabi语料库将公开发布，以支持阿拉伯语自然语言处理、计算社会科学和数字劳动研究。
...
### [238] [ArPoMeme: An Annotated Arabic Multimodal Dataset for Political Ideology and Polarization](https://arxiv.org/abs/2605.20967)
*Wajdi Zaghouani,Kais Attia,Md. Rafiul Biswas,Fadhl Eryani*

Main category: cs.CL

TL;DR: 本文提出了ArPoMeme，一个包含约7,300张阿拉伯政治模因的大规模数据集，按意识形态分类，并分析了其在极化维度上的特点。


<details>
  <summary>Details</summary>
Motivation: 模因在阿拉伯世界的政治沟通中日益重要，但缺乏系统化的多模态和意识形态分析资源。

Method: 通过半自动数据采集管道（基于Playwright和Google Drive）、Qwen2.5-VL-7B模型提取文本、手动标注极化维度（如我们vs.他们、敌意、行动呼吁）。

Result: 数据集显示了意识形态群体之间的对抗性不对称，伊斯兰主义和讽刺性模因表现出最高的敌意和动员信号。

Conclusion: ArPoMeme为研究阿拉伯政治话语、多模态意识形态检测和极化动态提供了可复用的公共资源。
...
### [239] [Beyond Text-to-SQL: An Agentic LLM System for Governed Enterprise Analytics APIs](https://arxiv.org/abs/2605.21027)
...
TL;DR: 提出了一个基于LLM的代理系统，将自然语言意图转化为安全的API交互，并验证了企业场景下的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL和BI工具在受治理API和复杂业务逻辑的企业环境中效果不佳，存在可靠性、合规性和安全性风险。

Method: 构建了多步推理与策略编排的LLM代理系统，解析意图、验证权限、执行查询并生成可视化。

Result: 在90个领域专家构建的企业案例中，系统能可靠地解释目标、验证权限并生成合规的可视化。

Conclusion: 该代理系统可有效替代传统BI和Text-to-SQL方案，提升企业数据查询的准确性与安全性。
...
### [240] [Single-Pass, Depth-Selective Reading for Multi-Aspect Sentiment Analysis](https://arxiv.org/abs/2605.20998)
*Yan Xia,Zhuangzhuang Pan,Amirrudin Kamsin,Chee Seng Chan*

Main category: cs.CL

TL;DR: 提出单遍推理框架DABS，通过一次性编码句子构建深度有序基板，方面查询时按需读取相关抽象层级，避免重复编码，在保持性能的同时减少60%计算量。


<details>
  <summary>Details</summary>
Motivation: 面向方面情感分析中多方面的句子需要高效处理，现有方法重复编码问题严重，且缺乏自适应性。

Method: 单遍编码构造可复用的深度有序表示基板，每个方面通过查询机制选择性读取相关token和抽象层级，解耦共享句子编码与轻量方面读取。

Result: 在4个基准上性能与现有方法相当，多方面场景下计算量降低60%，深度自适应查询在否定、对比等复杂语言场景效果更佳。

Conclusion: Transformer深度可视为可查询资源，通过按需读取能兼顾效率与表达能力，为ATSA提供更优范式。
...
### [241] [Fine-grained Claim-level RAG Benchmark for Law](https://arxiv.org/abs/2605.21071)
*Souvick Das,Sallam Abualhaija,Domenico Bianculli*

Main category: cs.CL

TL;DR: 提出了ClaimRAG-LAW数据集，支持法语和英语，面向专家和非专家，细粒度评估法律RAG系统的检索与生成。


<details>
  <summary>Details</summary>
Motivation: 现有法律RAG评估框架缺乏细粒度，仅限英语且忽略非专家需求。

Method: 构建双语多层次问答数据集ClaimRAG-LAW，并设计细粒度评估框架。

Result: 揭示了当前最佳法律RAG系统在检索、生成和声明级分析中的局限性。

Conclusion: 需要更细粒度的评估来改进法律RAG系统的性能。
...
### [242] [Building a Custom Taxonomy of AI Skills and Tasks from the Ground Up with Job Postings](https://arxiv.org/abs/2605.21029)
*Stephen Meisenbacher,Peter Norlander*

Main category: cs.CL

TL;DR: 提出TaxonomyBuilder框架，研究发现过滤数据比使用全部数据能构建更高质量的AI技能领域分类体系。


<details>
  <summary>Details</summary>
Motivation: 大规模快速增长语料库中，如何最优利用数据自动构建分类体系尚不明确。

Method: 使用两个大规模招聘语料库，系统评估TaxonomyBuilder在不同数据配置（过滤/未过滤、自定义/数据驱动/层次化）下的表现。

Result: 过滤输入数据比未过滤数据能获得更好的领域覆盖度，优于聚类和LLM增强的层次化标签工具。

Conclusion: 少量高质量数据能更有效构建分类体系，过滤策略是优化自动分类的关键。
...
### [243] [ACL-Verbatim: hallucination-free question answering for research](https://arxiv.org/abs/2605.21102)
*Gábor Recski,Szilveszter Tóth,Nadia Verdha,István Boros,Ádám Kovács*

Main category: cs.CL

TL;DR: 本研究提出VerbatimRAG系统，通过抽取式问答减少LLM幻觉，并构建数据集（基于ACL论文与合成查询），验证了小型模型优于大型LLM在精准文本定位上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM辅助研究工具存在事实性错误（幻觉），需更可靠的方法从信任来源提取精确信息。

Method: 基于VerbatimRAG的抽取式问答系统，使用ScIRGen流程生成合成用户查询，结合NLP研究者人工标注，训练ModernBERT等抽取模型。

Result: 150M参数的ModernBERT分类器在词级F1达53.6，超越最强LLM抽取器（48.7）。

Conclusion: 小型专用抽取模型在精准映射查询到论文文本片段的任务上，优于通用大型语言模型。
...
### [244] [Cross-lingual robustness of LLM-brain alignment and its computational roots](https://arxiv.org/abs/2605.21049)
*Ni Yang,Rui He,Philipp Homan,Iris Sommer,Davide Staub,Wolfram Hinzen*

Main category: cs.CL

TL;DR: 该研究通过多语言全脑编码框架，发现大语言模型（LLMs）能预测跨语言（汉语、英语、法语）自然故事理解时的脑活动，对齐模式广泛分布于皮层及皮层下区域，且跨语言高度重叠，但该对齐并非源于层级计算或预测处理等计算机制。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示LLMs可预测语言理解中的神经活动，但其与皮层下区域的对齐、跨语言空间重叠性以及计算根源尚未明确。

Method: 采用多语言全脑编码框架，使用Transformer模型分析汉语、英语和法语被试听故事时的功能磁共振成像（fMRI）数据，比较上下文嵌入与静态嵌入的预测性能，并测试逐层脑评分与惊讶度、内在维度的关联。

Result: Transformer模型能预测广泛分布的功能网络（边缘系统、腹侧注意网络、默认模式网络）及皮层下区域的脑活动；跨语言对齐模式高度重叠且跨层稳定；上下文嵌入未优于静态嵌入；脑评分与惊讶度、内在维度无关。

Conclusion: 脑-LLM对齐具有空间鲁棒性和跨语言稳定性，但并非源于预测处理或信息压缩等计算机制，而可能主要反映跨语言泛化的分布式词汇-语义对应关系。
...
### [245] [APM: Evaluating Style Personalization in LLMs with Arbitrary Preference Mappings](https://arxiv.org/abs/2605.21063)
*Philipp Spohn,Leander Girrbach,Zeynep Akata*

Main category: cs.CL

TL;DR: 提出了APM基准，通过隐藏的随机映射解耦用户属性与响应偏好，评估了RAG、提示优化和路由等个性化方法，发现路由最可靠，RAG仅在强基模型下有提升，提示优化效果不显著。


<details>
  <summary>Details</summary>
Motivation: LLM默认输出风格，而用户有隐含偏好，现有评估方法难以衡量个性化效果。

Method: APM基准通过随机映射C避免模型利用刻板印象，需从对话历史推断偏好，并改编检索增强、提示优化和路由方法。

Result: 路由方法最可靠，RAG仅在Qwen-3.5-27B上提升，软提示优化未显著优于非个性化基线。

Conclusion: 个性化仍有挑战，但改编方法展现出潜力。
...
### [246] [Automated ICD Classification of Psychiatric Diagnoses: From Classical NLP to Large Language Models](https://arxiv.org/abs/2605.21154)
...
TL;DR: 通过自然语言处理和机器学习自动化精神科诊断编码，使用西班牙语数据集测试多种文本表示方法，发现e5_large模型微调后F1_micro达0.866，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 全球心理健康问题导致临床诊断编码的行政负担沉重，需要自动化解决方案。

Method: 利用包含145,513条西班牙语精神科描述的专用数据集，评估了从BoW、TF-IDF到大型语言模型（如e5_large、BioLORD、Llama-3-8B）的多种文本表示方法，并进行了端到端微调。

Result: 基于Transformer的嵌入一致优于传统方法，e5_large通过微调达到最高性能（F1_micro=0.866）。

Conclusion: 将大语言模型适应特定临床术语对于克服精神科话语中的长尾标签分布和固有歧义至关重要。
...
### [247] [Manga109-v2026: Revisiting Manga109 Annotations for Modern Manga Understanding](https://arxiv.org/abs/2605.21182)
*Jeonghun Baek,Atsuyuki Miyai,Shota Onohara,Hikaru Ikuta,Kiyoharu Aizawa*

Main category: cs.CL

TL;DR: 针对Manga109数据集中的转录错误和粗粒度标注问题，提出Manga109-v2026版本，通过OCR检测与人工修正结合的方法修订约29,000条对话标注，使其更适用于现代OCR和多模态漫画理解任务。


<details>
  <summary>Details</summary>
Motivation: 现有Manga109数据集存在转录错误和粗粒度标注，难以满足现代OCR和多模态漫画理解的需求。

Method: 结合基于OCR的问题检测和人工修正，分类处理转录错误、缺失文本区域、对话与拟声词重叠、语音气泡分割不全等问题。

Result: 修订了约29,000条对话标注，形成Manga109-v2026版本，提升了标注质量。

Conclusion: Manga109-v2026能更好地支持现代OCR和多模态漫画理解系统，同时保留漫画特有的表现性结构。
...
TL;DR: 研究了大型语言模型在德国法律考试评分中的应用，评估了27种模型，发现推理型模型在公法领域与专家评分高度一致（QWK达0.91），刑法领域较低（0.60），集成方法可提升效果。


<details>
  <summary>Details</summary>
Motivation: 德国法律考试评分面临数量增长和合格评分员短缺问题，反馈延迟成瓶颈，但缺乏系统研究。

Method: 评估27种专有和开源LLM，对刑法和公法案例进行评分，基准测试逐步添加任务信息的提示策略（如样本答案和评分标准）。

Result: 公法评分一致性最高达0.91（QWK），刑法为0.60；集成方法比最佳单一模型提升0.15。

Conclusion: 推理型LLM在公法领域可近似专家评分，但刑法评分更困难；有效的提示设计和模型选择对可靠评分至关重要。
...
### [249] [LoCar: Localization-Aware Evaluation of In-Vehicle Assistants through Fine-Grained Sociolinguistic Control](https://arxiv.org/abs/2605.21086)
*Seogyeong Jeong,Kiwoong Park,Seyoung Song,Eunsu Kim,Ken E. Friedl,Jaeho Kim,Alice Oh*

Main category: cs.CL

TL;DR: 针对车载对话系统中大语言模型缺乏领域特定评估标准的问题，提出了一个面向韩国语本地化的评估框架，发现模型在敬语控制和策略性对话能力上存在不足。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在车载对话系统中缺乏针对实际部署需求的领域特定评估标准，特别是韩国语本地化场景。

Method: 提出一个新颖的车载助手评估框架，侧重韩国语本地化，通过实证分析模型行为模式。

Result: 细粒度韩语敬语控制不稳定，模型在澄清和主动性等策略对话指标上表现较弱。

Conclusion: 车载AI需超越通用能力，实现精确的语言定制和可靠的、以安全为导向的交互管理。
...
### [250] [WCXB: A Multi-Type Web Content Extraction Benchmark](https://arxiv.org/abs/2605.21097)
*Murrough Foley*

Main category: cs.CL

TL;DR: 提出了一个包含2008个网页、7种页面类型的内容提取基准WCXB，评估了13个提取系统后发现模型在结构化页面类型上表现差异大，揭示了现有基准的盲点。


<details>
  <summary>Details</summary>
Motivation: 现有网页内容提取基准规模小、局限于新闻文章或基于十年前的网页，无法反映真实场景的多样性。

Method: 构建包含1613个域名、7种页面类型的WCXB数据集，通过五阶段标注流程（LLM辅助、自动验证、前沿模型审查、片段质检、人工审核）生成黄金标准，并评估11个启发式方法和2个神经网络系统。

Result: 顶级系统在文章页面F1达0.93，但结构化页面类型F1仅0.41-0.84，表明现有文章专属基准无法发现模型在复杂页面上的缺陷。

Conclusion: WCXB揭示了现有提取方法的不足，为未来研究提供了更全面的评估资源。
...
### [251] [Smarter edits? Post-editing with error highlights and translation suggestions](https://arxiv.org/abs/2605.21135)
*Fleur V. J. van Tellingen,Gautam Ranka,Dora Žugčić,Joyce van der Wal,Andrea Camasta,Livio Guerra,Alina Karakanta*

Main category: cs.CL

TL;DR: 研究了基于自动后编辑的错误高亮和修正建议对专业译员后编辑效率、质量和用户体验的影响，发现相比常规后编辑和基于质量估计的高亮，自动后编辑高亮更受欢迎，修正建议能提升用户体验，但均未显著提升效率或质量。


<details>
  <summary>Details</summary>
Motivation: 尽管机器翻译质量提升，但基于质量估计的错误高亮等增强后编辑功能的实际效用证据有限，急需探索更有效的后编辑支持工具。

Method: 进行了一项研究，让专业译员（英语-荷兰语）分别使用常规后编辑、基于质量估计的高亮后编辑以及基于自动后编辑的高亮和修正建议后编辑，对比三种条件下的生产率、翻译质量和用户体验。

Result: 与常规后编辑相比，其他条件均未显著提升生产率或翻译质量；但自动后编辑高亮比质量估计高亮更受译员欢迎，而修正建议改善了整体用户体验。

Conclusion: 自动后编辑生成的高亮和修正建议在用户体验上优于现有方法，但当前技术尚未带来直接的生产率或质量提升，未来需进一步优化。
...
### [252] [Metaphors in Literary Post-Editing: Opening Pandora's Box?](https://arxiv.org/abs/2605.21178)
*Aletta G. Dorst,Mayra O. Nas,Katinka Zeven*

Main category: cs.CL

TL;DR: 本文研究了文学文本后编辑者对神经机器翻译和大语言模型翻译隐喻的反应，发现三分之一隐喻被修改，后编辑者认为质量差且比从头翻译更费力。


<details>
  <summary>Details</summary>
Motivation: 探讨文学机器翻译中比喻性语言的处理问题，以及后编辑如何影响译者的创造力和所有权感。

Method: 通过实验让后编辑者编辑NMT和LLM翻译的文学文本，分析对隐喻译文的修改和问卷调查。

Result: 三分之一隐喻被修改；后编辑者意识到直译问题（尤其是多词表达），有时难判断译文是否可接受；认为MT输出质量差且后编辑比人工翻译更费力。

Conclusion: 支持先前研究：后编辑限制译者创造力并降低文本所有权感，文学MT的后编辑可能不如人工翻译高效。
...
### [253] [Tracing the ongoing emergence of human-like reasoning in Large Language Models](https://arxiv.org/abs/2605.21299)
*Paolo Morosi,Nikoleta Pantelidou,Fritz Günther,Elena Pagliarini,Evelina Leivada*

Main category: cs.CL

TL;DR: 本研究通过种群匹配实验，比较了25个大型语言模型（LLMs）与人类在条件句推理上的表现，发现LLMs能准确进行语义推理，但缺乏人类特有的语用推理能力。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs是否像人类一样进行推理，特别是条件句中的语用推理。

Method: 采用种群匹配实验，针对4种语言，比较25个LLMs与等量人类在条件句推断上的表现。

Result: 人类基于语用推理丰富逻辑理解，而LLMs行为多变，部分严格遵循真值表但忽视语用，部分偏离真值表；准确性不受模型开放状态、训练方向或架构类型影响。

Conclusion: LLMs是准确的语义操作者，但无法捕捉人类特有的语用推理，该能力在人工系统中仍是新兴特性。
...
### [254] [Do LLMs Know What Luxembourgish Borrows? Probing Lexical Neology in Low-Resource Multilingual Models](https://arxiv.org/abs/2605.21227)
*Nina Hosseini-Kivanani*

Main category: cs.CL

TL;DR: 本文提出LexNeo-Bench基准，评估LLM在卢森堡语借词分类上的表现，并通过知识图谱提示将准确率从25-35%提升至71-81%。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在小型接触语言（如卢森堡语）中是否尊重社区对词汇借用和新词的规范。

Method: 构建LexNeo-Bench基准（含3050个词级标签），使用34种提示设置测试三个多语言LLM，并设计包含借用来源、形态模式等信息的语言学知识图谱注入提示中。

Result: 知识图谱提示将借词分类准确率提升至71-81%，缩小了模型规模差距，但新词检测仍困难且对少样本设计敏感。

Conclusion: 词汇感知提示对低资源接触语言的鲁棒借词判断高度有效，词汇资源可作为结构化上下文用于LLM评估。
...
### [255] [LamPO: A Lambda Style Policy Optimization for Reasoning Language Models](https://arxiv.org/abs/2605.21235)
*Zhe Yuan,Yipeng Zhou,Jinghan Li,Xinyuan Chen,Bowen Deng,Zhiqian Chen,Liang Zhao*

Main category: cs.CL

TL;DR: 本文提出LamPO方法，通过成对分解优势和置信度加权改进GRPO，在数学和科学推理任务上提升强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法丢弃了候选响应间的细粒度关系信息，导致稀疏奖励下信用分配不足。

Method: LamPO使用成对分解优势替代标量组优势，结合序列对数概率差异的置信度权重，并可选地添加ROUGE-L辅助奖励。

Result: 在AIME24、MATH-500等基准上，LamPO一致优于GRPO，训练更稳定且样本效率更高。

Conclusion: LamPO通过细粒度比较和辅助奖励有效改善了RLVR在推理任务中的性能。
...
### [256] [TextReg: Mitigating Prompt Distributional Overfitting via Regularized Text-Space Optimization](https://arxiv.org/abs/2605.21318)
*Lucheng Fu,Ye Yu,Yiyang Wang,Yiqiao Jin,Haibo Jin,B. Aditya Prakash,Haohan Wang*

Main category: cs.CL

TL;DR: 提出了TextReg正则化框架，通过文本梯度约束解决提示优化中的分布过拟合问题，显著提升OOD泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法生成的提示存在分布过拟合，即提示变长、积累狭窄样本规则、OOD泛化差，缺乏表示控制。

Method: 基于双因素表示效率度量，提出TextReg框架，包含双证据梯度净化、语义编辑正则化和正则化引导提示更新。

Result: 在多个推理基准上，TextReg相比TextGrad和REVOLVE分别提升OOD准确率最多11.8%和16.5%。

Conclusion: TextReg通过软惩罚机制有效缓解提示分布过拟合，改善OOD泛化，验证了表示效率因素的关键作用。
...
### [257] [Reliable Automated Triage in Spanish Clinical Notes: A Hybrid Framework for Risk-Aware HIV Suspicion Identification](https://arxiv.org/abs/2605.21256)
*Rodrigo Morales-Sánchez,Soto Montalvo,Raquel Martínez*

Main category: cs.CL

TL;DR: 提出风险感知混合选择性分类框架，通过双验证机制（Mondrian共形预测处理偶然不确定性+多质心马氏距离否决处理认知不确定性）在HIV早期识别任务中提升临床可靠性。


<details>
  <summary>Details</summary>
Motivation: 标准临床NLP基准强制对模糊实例确定性分类导致指标膨胀，掩盖了过度自信预测的临床风险。

Method: 双验证框架：1）Mondrian共形预测解耦偶然不确定性 2）多质心马氏距离否决处理认知不确定性。

Result: 标准不确定性指标和基线分类器在严格可靠性约束下覆盖崩溃，而双验证框架成功隔离高可信操作域。

Conclusion: 概率与几何双重保障对安全临床分诊至关重要，所提框架优于现有方法。
...
### [258] [SymbolicLight V1: Spike-Gated Dual-Path Language Modeling with High Activation Sparsity and Sub-Billion-Scale Pre-Training Evidence](https://arxiv.org/abs/2605.21333)
*Ting Liu*

Main category: cs.CL

TL;DR: 提出了SymbolicLight V1，一种结合LIF脉冲动态和连续残差流的脉冲门控双路径语言模型，在3B词元中英文语料上训练达到89%激活稀疏度，PPL 8.88-8.93。


<details>
  <summary>Details</summary>
Motivation: 现有脉冲语言模型难以同时实现Transformer级的语言质量、稳定的多领域预训练和高激活稀疏度。

Method: 采用Dual-Path SparseTCAM模块，包含指数衰减聚合路径（长程记忆）和脉冲门控局部注意力路径（短程精度），配合动态上下文解码头和双语分词器。

Result: 194M参数模型在3B词元训练后达到PPL 8.88-8.93，稀疏度>89%；性能落后GPT-2 201M 7.7%但优于GPT-2 124M；消融实验表明脉冲门控局部注意力路径贡献最大，LIF动态优于确定性top-k掩码。

Conclusion: 脉冲门控双路径结构能有效平衡语言质量和稀疏度，LIF时间整合比单纯稀疏更重要；当前密集硬件推理速度较慢，神经形态部署是未来机会。
...
### [259] [Text Analytics Evaluation Framework: A Case Study on LLMs and Social Media](https://arxiv.org/abs/2605.21338)
*Yuefeng Shi,Nedjma Ousidhoum,Jose Camacho-Collados*

Main category: cs.CL

TL;DR: 本文通过470个手工问题评估LLMs在聚合文本数据上的语义理解与推理能力，发现性能受输入规模、数据源复杂度和任务复杂度影响显著，尤其在大规模输入下，开放权重模型数值任务性能大幅下降。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在长序列非结构化文档（如社交媒体帖文）的实际数据分析场景中存在性能差距，缺乏系统的评估基准。

Method: 构建包含470个手工问题的问答评估框架，在Twitter数据集上覆盖情感分析、仇恨言论检测等任务，通过控制输入规模和任务复杂度进行实验。

Result: 性能随输入规模扩大（超过500实例）和任务复杂度增加（从语义存在性到数值计算）显著下降，多标签/目标依赖场景尤其明显，开放权重模型数值能力瓶颈突出。

Conclusion: 现有LLMs在处理大规模文本集合的定量分析时存在架构性瓶颈，数值任务退化最为严重。
...
### [260] [LASH: Adaptive Semantic Hybridization for Black-Box Jailbreaking of Large Language Models](https://arxiv.org/abs/2605.21362)
*Abdullah Al Nomaan Nafi,Fnu Suya,Swarup Bhunia,Prabuddha Chakraborty*

Main category: cs.CL

TL;DR: 提出了LASH框架，通过自适应组合多种越狱攻击策略，在黑盒条件下显著提升了对大型语言模型的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有自动化越狱方法分别专注于单一攻击家族，不同方法在不同模型和危害类别上表现各异，缺乏能综合利用它们互补优势的通用框架。

Method: LASH 是一个黑盒框架，将多个基础攻击的输出视为可重用种子提示，通过遗传优化器自适应搜索种子子集及其混合权重，利用目标模型的反馈和两阶段适应度函数（关键词拒绝检测与LLM评分）合成单个候选提示。

Result: 在JailbreakBench的100个有害提示上，LASH在六个目标模型上平均攻击成功率达84.5%（关键词评估）和74.5%（两阶段评估），优于五种基线方法，仅需平均30次目标查询，且在三种防御下仍具竞争力。

Conclusion: 自适应组合异构越狱策略是黑盒红队测试中一个具有前景的方向，能够有效提升攻击效果并规避防御。
...
### [261] [Mem-$π$: Adaptive Memory through Learning When and What to Generate](https://arxiv.org/abs/2605.21463)
*Xiaoqiang Wang,Chao Wang,Hadi Nekoei,Christopher Pal,Alexandre Lacoste,Spandana Gella,Bang Liu,Perouz Taslakian*

Main category: cs.CL

TL;DR: 本文提出Mem-π框架，用独立语言/视觉语言模型按需生成上下文特定的指导，替代传统检索式记忆，在导航、工具使用等任务中提升超30%。


<details>
  <summary>Details</summary>
Motivation: 现有记忆增强代理依赖检索静态条目，常与当前上下文不匹配，导致性能受限。

Method: 使用独立语言/视觉语言模型，基于决策-内容解耦的强化学习目标，训练模型决定何时生成指导及生成何种指导。

Result: 在Web导航、终端工具使用、文本具身交互基准测试中，Mem-π一致优于检索式及强化学习基线，Web导航任务相对提升超30%。

Conclusion: 按需生成上下文感知的指导比检索静态记忆更有效，为LLM代理的适应性记忆提供了新范式。
...
### [262] ["I didn't Make the Micro Decisions": Measuring, Inducing, and Exposing Goal-Level AI Contributions in Collaboration](https://arxiv.org/abs/2605.21363)
*Eunsu Kim,Jessica R. Mindel,Kyungjin Kim,Sherry Tongshuang Wu*

Main category: cs.CL

TL;DR: 提出了CoTrace框架，用于在人机协作中追踪模型对目标的贡献。发现模型在目标塑造中仅占11-26%的贡献，但以引入具体需求为主。交互设计显著影响模型行为，暴露用户感知偏差。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注最终制品，忽略了人机协作中目标本身如何被共同塑造的过程，导致贡献归因不准确。

Method: 将显式目标分解为可验证需求，通过对话轮次追踪直接贡献和间接影响。

Result: 模型占目标塑造贡献的11-26%，但更倾向引入低层级具体需求，且存在多种间接贡献形式。交互设计选择显著改变模型行为。用户研究显示，目标层级分析使感知贡献评分偏移近2分（5分量表）。

Conclusion: CoTrace能揭示模型在目标塑造中的实际贡献，并纠正用户对AI辅助工作的系统性感知偏差。
...
### [263] [Findings of the Fifth Shared Task on Multilingual Coreference Resolution: Expanding Datasets for Long-Range Entities](https://arxiv.org/abs/2605.21369)
...
TL;DR: 这篇论文介绍了第五届多语言指代消解共享任务（CODI-CRAC 2026），重点强调长距离实体，新增了5个数据集和2种语言，并对比了传统系统与LLM系统的表现。


<details>
  <summary>Details</summary>
Motivation: 为了推动多语言指代消解领域的发展，特别是处理长距离指代实体这一挑战，并评估语言大模型（LLM）在这一任务上的潜力。

Method: 任务要求参与者开发系统，进行提及识别和基于身份的指代聚类。新增使用CorefUD v1.4版本，涵盖19种语言的27个数据集，并引入4个LLM系统（3个微调、1个少样本）。

Result: 10个系统参与，传统系统仍领先，但LLM表现出了显著潜力，可能在未来挑战传统方法。

Conclusion: LLM在多语言指代消解中显示出竞争力，未来版本中可能超越传统系统；同时，扩展数据集有助于提升任务覆盖范围和鲁棒性。
...
### [264] [Post-Hoc Understanding of Metaphor Processing in Decoder-Only Language Models via Conditional Scale Entropy](https://arxiv.org/abs/2605.21391)
*Lawhori Chakrabarti,Jennifer Johnson-Leung,Bert Baumgaertner,Aleksandar Vakanski,Min Xian,Boyu Zhang*

Main category: cs.CL

TL;DR: 提出条件尺度熵(CSE)度量Transformer模型在隐喻处理中的多尺度协调模式，发现隐喻标记在解码器架构中产生显著更高的谱广度。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer模型如何在深度层次上组织对隐喻词汇的重新解释。

Method: 基于小波分析定义条件尺度熵(CSE)，分析模型各层计算的频率尺度分布。

Result: 在124M至20B参数的多种解码器架构中，隐喻标记在连续层位置呈现显著更高的谱广度，且不受语义复杂度或命题内容影响。

Conclusion: 多尺度协调是解码器架构处理隐喻语言的一致特征，CSE是分析Transformer跨层结构的有效工具。
...
### [265] [Quantifying the cross-linguistic effects of syncretism on agreement attraction](https://arxiv.org/abs/2605.21403)
*Utku Turk,Eva Neu*

Main category: cs.CL

TL;DR: 本文利用大语言模型中的惊讶度和注意力熵作为处理代理，跨语言研究了形态学同形（syncretism）如何影响一致性吸引错误，发现该效应因语言而异。


<details>
  <summary>Details</summary>
Motivation: 先前发现同形在某些语言中放大一致吸引错误，但在其他语言中不放大，缺乏机制解释。

Method: 使用大语言模型计算惊讶度和注意力熵，作为处理难度和注意分配的代理指标。

Result: LLM指标复现了英语和德语中同形调节吸引的效应；与土耳其语的无效结果一致；部分捕捉了俄语的模式。

Conclusion: 同形对一致吸引的影响存在跨语言差异，LLM处理指标可部分解释，但需进一步研究其根本原因。
...
### [266] [Leveraging LLMs for Grammar Adaptation: A Study on Metamodel-Grammar Co-Evolution](https://arxiv.org/abs/2605.21465)
*Weixing Zhang,Bowen Jiang,Rahul Sharma,Regina Hebig,Daniel Strüber*

Main category: cs.CL

TL;DR: 本文提出了一种基于大语言模型的方法，通过学习历史演化中的语法适配来自动更新领域特定语言的语法，并在六个Xtext语言上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 模型驱动工程中元模型演化后需手动适配语法，现有基于规则的方法在复杂场景下自动化程度有限。

Method: 利用大语言模型从先前的语法演化中学习适配模式，自动应用到新语法上；在四个DSL上训练提示策略，两个DSL上测试，并对QVTo进行纵向案例研究。

Result: 在测试集上，三个LLM均达到100%的语法规则适配一致性和输出相似性，而基于规则的方法分别仅为84.21%（DOT）和62.50%（Xcore）；在QVTo纵向研究中，LLM方法在所有演化步骤中自动复用了学习到的适配，无需手动编辑；但在大规模语法（EAST-ADL，297条规则）下，LLM的适配一致性远低于90%。

Conclusion: 大语言模型方法在处理复杂语法场景时优于基于规则的方法，但在大规模语法适应上存在局限性。
...
### [267] [Adaptive Human-Robot Collaboration for Masonry Construction Under Material and Assembly Uncertainty](https://arxiv.org/abs/2605.20264)
*Jutang Gao,Arash Adel*

Main category: cs.RO

TL;DR: 提出了一种结合投影引导与激光扫描反馈的自适应人机协作砌砖工作流，用于解决通信限制与公差累积问题。


<details>
  <summary>Details</summary>
Motivation: 人机协作建筑中机器人通信有限，且需适应材料和装配不确定性导致的公差累积。

Method: 1) 末端执行器投影仪提供实时空间投影指导；2) 激光扫描实现反馈驱动的抓取与放置姿态校正。

Result: 投影引导改善了粘合剂涂抹一致性并缩短时间；激光校正维持水平层、避免碰撞故障。

Conclusion: 集成空间投影与反馈驱动自适应可缓解公差累积，提升人机协作建筑的精度与鲁棒性。
...
### [268] [SUGAR: A Scalable Human-Video-Driven Generalizable Humanoid Loco-Manipulation Learning Framework](https://arxiv.org/abs/2605.20373)
*Tianshu Wu,Xiangqi Kong,Yue Chen,Qize Yu,Hang Ye,Jia Li,Yizhou Wang,Hao Dong*

Main category: cs.RO

TL;DR: 提出SUGAR框架，从人类视频中提取运动先验并转化为可部署的人形机器人全身操作技能，无需任务奖励工程或参考运动条件。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手工奖励设计、参考运动回放或昂贵遥操作，缺乏可扩展性和泛化性。人类视频虽丰富但存在遮挡、接触伪影等缺陷，无法直接用于策略学习。

Method: 三阶段流程：1）从非结构化视频自动提取人-物运动轨迹和接触标签；2）基于物理的精炼器通过统一模仿奖励和渐进状态池将不完美先验转化为物理可行技能；3）将精炼技能蒸馏为分层自主策略（命令生成器+命令跟踪器）。

Result: 在6项全身操作任务中显著优于参考跟踪基线，性能随视频数据量提升，实现零样本真实世界迁移，具备闭环执行、自主故障恢复和长时稳定表现。

Conclusion: SUGAR证明从人类视频学习可扩展的全身操作技能是可行的，为通用人形机器人提供了数据驱动的解决方案。
...
### [269] [Scalable Multi-robot Motion Planning via Hierarchical Subproblem Expansion and Workspace Decomposition Refinement](https://arxiv.org/abs/2605.20395)
*Isaac Ngui,Courtney McBeth,James D. Motes,Marco Morales,Nancy M. Amato*

Main category: cs.RO

TL;DR: 提出一种多机器人运动规划方法，通过工作空间分解的离散搜索提供协调，将规划时间提升一个数量级。


<details>
  <summary>Details</summary>
Motivation: 多机器人运动规划中，协调避免冲突与搜索联合配置空间的高计算成本之间的矛盾。

Method: 利用工作空间分解进行离散搜索，迭代细化表示，以搜索更小、解耦的配置空间。

Result: 规划时间提升高达一个数量级。

Conclusion: 通过工作空间分解协调可显著提高多机器人运动规划效率。
...
### [270] [Benchmarking Empirical and Learning-Based Approaches for Feedforward Steering Control in Autonomous Racing](https://arxiv.org/abs/2605.21111)
*Georg Jank,Mattia Piccinini,Sebastian Wenk,Phillip Pitschi,Johannes Betz,Boris Lohmann*

Main category: cs.RO

TL;DR: 本文对四种前馈转向控制器进行了基准测试，提出一种基于多项式曲面拟合的有效错误航向（EHD）方法，发现在开环中学习型控制器预测误差更低，但在闭环中EHD方法在鲁棒性和圈速上表现更优。


<details>
  <summary>Details</summary>
Motivation: 自主驾驶赛车中前馈转向控制对于减少反馈修正至关重要，现有方法缺乏系统性比较，且需要有效捕捉速度相关非线性转向行为的模型。

Method: 通过高保真双轨车辆动力学仿真，对比两种学习型和两种经验前馈转向控制器，并引入一种基于多项式曲面拟合的新EHD公式。

Result: 学习型控制器在开环预测误差最低，但闭环测试中并未提升路径追踪性能或圈速；EHD方法在闭环鲁棒性和圈速上取得最佳综合表现。

Conclusion: 评估前馈转向策略必须在完整轨迹规划与控制软件栈中进行闭环测试，EHD方法因结构简单且鲁棒性强而优于更复杂的模型。
...
### [271] [Spacetime Optimal-Transport Attention for Visuo-Haptic Imitation Learning of Contact-Rich Manipulation](https://arxiv.org/abs/2605.20433)
*Yue Feng,Weicheng Huang,I-Ming Chen*

Main category: cs.RO

TL;DR: 提出 SO-TA，一种三模态融合架构，用最优传输对齐取代 softmax 注意力，在接触丰富的操作任务中实现稳定、可解释的策略。


<details>
  <summary>Details</summary>
Motivation: 接触丰富的操作任务面临不连续动力学、部分可观测性和安全约束挑战，单一传感模态不足，现有三模态融合方法缺乏任务相关的注意力先验。

Method: 采用熵正则化最优传输对齐力-位姿子查询与视觉块，结合边缘约束作为归纳偏置，并搭配扩散序列策略输出姿态动作块。

Result: 在紧装配、连接器插拔和曲面擦除任务上，SO-TA 达 100% 成功率，干扰下保持 82.5% 而基线降为 43.5%。

Conclusion: SO-TA 的 OT 注意力提供了稳定的模态融合和可解释的相位诊断，显著提升接触丰富任务的鲁棒性。
...
### [272] [Enhancing Graph-Based SLAM in GNSS-Denied environments by leveraging leg odometry](https://arxiv.org/abs/2605.20484)
*Léon Perruchot-Triboulet,Luc Jaulin,Kai Xiao*

Main category: cs.RO

TL;DR: 本文提出了一种因子图架构，通过添加基于腿部运动学里程计的平行位姿链，显著降低了四足机器人在无GNSS环境中的高程漂移，漂移从30米以上降至30厘米以下。


<details>
  <summary>Details</summary>
Motivation: 在GNSS缺失环境中，四足机器人依赖的激光雷达在几何稀疏或重复场景中容易产生高程漂移，现有方法难以解决这一问题。

Method: 在LIO-SAM框架中增加一个平行位姿链，利用本体感知的腿部里程计，并通过带选择性噪声模型的身份相对位姿约束与主激光雷达-惯性位姿链耦合。

Result: 在四足平台上进行两个室外环路（总长超过1公里）实验，高程漂移从30米以上降至30厘米以下，且在基线方法失败的场景中实现了收敛。

Conclusion: 本体感知数据（已在机载用于步态控制）可作为SLAM在无GNSS环境中的轻量级且有效的垂直锚点。
...
### [273] [Intent-First Aerial V2V for Tactical Coordination and Separation: Protocol and Performance Under Density and Disturbance](https://arxiv.org/abs/2605.20595)
*Mehrnaz Sabet*

Main category: cs.RO

TL;DR: 提出首个面向密集无人机交通管理的全空中、侧链类、意图优先的V2V战术邻域通信协议，通过状态信标与事件触发消息实现战术间隔，在真实场测中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有无人机交通管理缺乏中间层（战术分离）的实时通信机制，冲突避免太迟、战略协调太慢，无法应对突发扰动。

Method: 设计意图优先的V2V通信栈，包含周期性状态/意图信标与事件触发消息（让行、排序、释放等），采用C-V2X侧链模块实现认证新鲜性检查，并通过高流量场景化压力测试评估。

Result: V2V可减少过时信念分歧、通过协同感知保持可观测性、拒绝无效消息、抑制错误推断、协调共享资源；在中低密度下有效，高密度/复杂场景转为保护性降级。

Conclusion: 意图优先的空中V2V是驱动作业中城市空域战术协调的可扩展使能器，但需注意其适用边界。
...
### [274] [SmoCap: Unified Scale-Pose Canonicalization with Proxy-Mapped Trust-Region QP](https://arxiv.org/abs/2605.20850)
*Shihao Li,Naohiko Sugita*

Main category: cs.RO

TL;DR: SmoCap是一个防泄漏的规范化框架，通过联合估计形态和姿态，在稀疏控制子空间内求解局部信赖域二次规划问题，有效减少了形态-姿态补偿，实现了外部一致的骨骼运动估计。


<details>
  <summary>Details</summary>
Motivation: 现有的分阶段工作流（先模型缩放再逆运动学）会导致形态-姿态补偿，尤其在弱观测方向上产生解剖学不一致的解。

Method: 采用约束信赖域二次规划，结合分析性代理映射的姿态和尺度雅可比矩阵，通过低维代理映射稳定弱观测方向并协同结构，可选预求解器提供热启动。

Result: 膝关节屈曲均方根误差2.9度（对照荧光镜），人体测量端点误差约3%；在全膝关节置换数据上减少标记均方根误差、屈伸误差和端点误差；瑜伽消融中代理耦合仅增加边缘拟合误差（+0.14 mm, +0.6%）；中值标记均方根误差约20 mm，中值运行时间0.204-0.332 ms/帧，仅需2-3次迭代。

Conclusion: SmoCap提供了一个经外部验证的统一耦合感知尺度-姿态框架，使得在数据集规模上实现外部一致的运动规范化变得实用。
...
### [275] [Mobile UMI: Cross-View Diffusion Policy with Decoupled Kinematics for Mobile Manipulation](https://arxiv.org/abs/2605.20894)
*Haoran Huang,Haonan Dong,Huixu Dong*

Main category: cs.RO

TL;DR: 提出无需硬件的移动模仿学习框架Mobile UMI，通过双相机采集、空间锚定解耦和异步执行器，在长周期任务中达到83.8%成功率。


<details>
  <summary>Details</summary>
Motivation: 移动模仿学习面临动作标签受行走污染和执行延迟两大瓶颈，现有腕戴式接口缺乏全局导航上下文，生成策略推理延迟导致基座预测失效。

Method: 双相机捕获胸腔全局与腕部局部场景；ChArUco锚定统一视觉-惯性坐标并解耦SE(3)操作与SE(2)基座轨迹；异步滚动时域执行器实时对齐动作块与当前位姿。

Result: 四项长周期任务平均成功率83.8%（每任务100次试验），对比ACT和扩散策略表明，胸腔解耦标签与状态匹配分别弥补主要差距。

Conclusion: 显式运动学分解加状态级延迟对齐可在不修改底层策略架构下有效解决移动模仿学习问题。
...
### [276] [STEAM: A Training-Free Congestion-Aware Enhancement Framework for Decentralized Multi-Agent Path Finding](https://arxiv.org/abs/2605.20929)
*Mingyang Feng,Mengnuo Zhang,Shaoyuan Li,Xiang Yin*

Main category: cs.RO

TL;DR: 提出了STEAM框架，无需重新训练即可提升学习型去中心化多智能体路径规划的性能。


<details>
  <summary>Details</summary>
Motivation: 解决去中心化MAPF中缺乏全局协调导致的拥堵问题。

Method: 通过空间可避拥堵修正成本地图、空间不可避瓶颈进行时间logit修正、局部拥堵基于邻居修正的成本地图进行密度感知logit修正。

Result: 成功率提升最高60%，且计算开销小。

Conclusion: STEAM能有效增强去中心化MAPF算法的性能。
...
### [277] [WiXus: A Wheeled-Legged Robot with Wire-Driven Environmental Utilizing to Integrate Mobility and Manipulation](https://arxiv.org/abs/2605.20932)
*Shintaro Inoue,Kento Kawaharazuka,Temma Suzuki,Sota Yuzaki,Kei Okada*

Main category: cs.RO

TL;DR: 提出一种融合轮腿机构与线驱动机器人WiXus，通过悬挂身体解放腿部操作能力，实现轮腿移动、攀爬及物体操控。


<details>
  <summary>Details</summary>
Motivation: 现有轮腿机器人腿部仅用于移动，缺乏将腿部重新用于物体操控或工具利用的能力。

Method: 设计WiXus机器人，结合轮腿驱动与利用外部环境的线驱动机理。

Result: WiXus实现平面移动、悬崖攀爬（通过协调轮腿与线驱动），并在悬挂状态下用腿完成物体操控（如救援玩具狗）和工具使用（如用修枝剪摘苹果）。

Conclusion: 利用外部环境的线驱动方式为扩展轮腿机器人操作域提供了新设计原则。
...
TL;DR: 本研究比较了社交机器人与人类在老年人互动中的心理和生理影响，发现机器人可作为有效互动伙伴，且能减轻压力。


<details>
  <summary>Details</summary>
Motivation: 应对全球护理人员短缺，探索社交机器人在老年人护理中的可行性及心理生理效应。

Method: 对35名70岁以上参与者进行对比实验，整合面部表情、心率变异性和主观问卷进行多模态分析。

Result: 人机互动与人人互动在总体压力水平上无显著差异；机器人互动时心率略低，表明更放松。

Conclusion: 社交机器人能在不引起心理压力的情况下与老年人互动，减轻护理负担，但需改进外观与内容不匹配问题。
...
TL;DR: 提出感知-动力学联合异常感知在线校准方法，融合重建误差与动力学不确定性，降低自动驾驶赛车视觉控制器在分布偏移下的过度自信，校准误差降低37%。


<details>
  <summary>Details</summary>
Motivation: 现有异常信号（如自编码器重建误差）仅感知视觉异常，无法检测动力学异常（如执行器偏差、延迟），导致模型在分布偏移时过度自信。

Method: 从世界模型中提取感知异常分数（重建误差）和动力学异常分数（认知不确定性与控制流统计），通过测试时增强融合信号，并采用轻量级温度缩放校准器降低过度自信。

Result: 在物理DonkeyCar的四个未见异常协议（黑暗、模糊、执行器偏差、延迟）下，期望校准误差从0.184降至0.116，较最佳基线提升37%，且不修改基础安全预测器。

Conclusion: 融合感知与动力学异常信号可有效校准视觉-动力学联合分布偏移下的置信度，提升自动驾驶系统的安全可靠性。
...
### [280] [Humanoid Whole-Body Manipulation via Active Spatial Brain and Generalizable Action Cerebellum](https://arxiv.org/abs/2605.21133)
*Zhizhao Liang,Yi-Lin Wei,Xuhang Chen,Mu Lin,Yi-Xiang He,Zhexi Luo,Jun-Hui Liu,Kun-Yu Lin,Wei-Shi Zheng*

Main category: cs.RO

TL;DR: 提出了一种结合多智能体大模型的空间感知与动作生成框架，用于人形机器人全身操作，在多样任务中展现了良好泛化性。


<details>
  <summary>Details</summary>
Motivation: 解决人形机器人在复杂3D环境中的空间理解困难和动作生成泛化能力不足的问题。

Method: 构建包含Active Spatial Brain（主动空间感知与决策）和Generalizable Action Cerebellum（可泛化动作生成）的两组件框架。

Result: 在设计的空间操作任务上，框架在空间感知和实物任务性能两方面均表现优异。

Conclusion: 该框架有效提升了人形机器人在复杂环境中的空间操作能力，无需任务特定数据即可泛化。
...
### [281] [Safety-Critical Control for Smoothed Implicit Contact Dynamics](https://arxiv.org/abs/2605.21138)
*Haegu Lee,Yitaek Kim,Christoffer Sloth*

Main category: cs.RO

TL;DR: 本文提出了一种基于平滑隐式接触动力学的安全控制方法，通过边界聚焦滚动和鲁棒控制屏障函数，解决了接触力约束违反问题。


<details>
  <summary>Details</summary>
Motivation: 平滑隐式接触动力学虽简化了规划，但安全滤波设计因接触力近似误差和参数非单调影响而困难。

Method: 通过边界聚焦滚动筛选平滑参数κ，结合一阶泰勒近似建立离散时间控制屏障函数，并引入固定鲁棒裕度补偿力预测不足。

Result: 在四个接触丰富系统仿真中，该方法消除了标准控制屏障函数下的力违反现象。

Conclusion: 所提方法能在保证安全的前提下有效管理接触力约束，且力近似误差不直接决定安全性能。
...
### [282] [EllipseLIO: Adaptive LiDAR Inertial Odometry with an Ellipsoid Representation](https://arxiv.org/abs/2605.21150)
*Rowan Border,Margarita Chli*

Main category: cs.RO

TL;DR: 本文提出了一种名为EllipseLIO的实时LiDAR惯性里程计方法，能够自适应不同传感器和环境，无需人工调参，并在多个数据集上实现了最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有LIO方法在异构环境和传感器中难以保持鲁棒性，需要针对特定场景进行调参，阻碍了机器人的自主导航能力。

Method: 采用自适应LiDAR扫描滤波和配准方法，根据传感器能力和环境特点动态调整参数，避免场景特定调参。

Result: 在五个多样化数据集上，EllipseLIO的里程计误差平均比第二名低38%，且是唯一在所有实验中未发散的方法。

Conclusion: EllipseLIO是一种通用且鲁棒的LIO解决方案，能有效适应异构场景，显著提升自主导航的稳定性和精度。
...
### [283] [A Terrain-Adaptive epsilon-Constraint MPC for Uneven Terrain Kinodynamic Planning](https://arxiv.org/abs/2605.21188)
*Otobong Jerome,Geesara Kalathunga,Tiago Nascimento*

Main category: cs.RO

TL;DR: 提出一种自适应epsilon约束的MPC框架，结合半参数车辆-地形动力学模型，实现了类车机器人在非平坦地形上的高效、稳定运动规划，导航成功率94%，姿态偏差降低24%。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如MPPI、GAKD）在非平坦地形上难以同时优化路径效率与姿态稳定性。

Method: 采用自适应epsilon约束MPC，动态调整epsilon边界探索Pareto前沿；构建半参数模型（解析动力学+稀疏高斯过程）描述车辆-地形交互。

Result: 导航成功率94%，最大取向偏差降低24%，多目标权衡质量提升23%。

Conclusion: 自适应epsilon-MPC在多目标规划中优于主流基线，适用于复杂地形下的类车机器人运动控制。
...
### [284] [To Select or not to Select, that is the Question: Distilling Robot Skill Prediction into a Small Ensemble](https://arxiv.org/abs/2605.21242)
*Haechan Mark Bong,Simon Roy,Euhid Aman,Giovanni Beltrame*

Main category: cs.RO

TL;DR: 提出机器人技能预测任务，通过合成数据集训练小模型，在任务-技能匹配上优于大语言模型。


<details>
  <summary>Details</summary>
Motivation: 机器人队伍异构性增加，需要自动将自然语言任务映射到机器人物理能力。

Method: 用LLM辅助生成和标签审计构建合成数据集，训练集成两个微调句子编码器（mpnet+MiniLM）的小模型。

Result: 小模型在200任务数据集上达83.5%准确率，优于Kimi K2（72.0%）等大模型。

Conclusion: 小模型在固定技能分类下可超越通用大模型，适用于车队级任务路由。
...
### [285] [Reinforcement Learning for Risk Adaptation via Differentiable CVaR Barrier Functions](https://arxiv.org/abs/2605.21257)
*Xinyi Wang,Taekyung Kim,Bardh Hoxha,Georgios Fainekos,Dimitra Panagou*

Main category: cs.RO

TL;DR: 提出一种端到端风险自适应框架，结合强化学习和基于条件风险值的可微二次规划安全层，用于不确定障碍物运动下的密集人群导航。


<details>
  <summary>Details</summary>
Motivation: 现有方法在随机交互下往往导致过于保守或效率降低，需在不确定环境中平衡安全与效率。

Method: 使用高斯混合模型建模障碍物运动不确定性，结合强化学习与基于CVaR屏障函数的可微二次规划安全层，联合学习标称控制输入、风险水平和安全裕度。

Result: 在多种动态、不确定和密集环境中，相比优化、强化学习及集成方法，所提方法在安全性、效率和泛化方面表现最佳。

Conclusion: 该框架能实现上下文感知自适应，仅在必要时采取谨慎行为，从而在不确定性下取得强整体性能。
...
### [286] [Learning Structural Latent Points for Efficient Visual Representations in Robotic Manipulation](https://arxiv.org/abs/2605.21258)
...
TL;DR: 提出一个混合表示框架，通过学习结构潜伏点结合隐式表达和显式几何优势，用于3D感知与操作预训练。


<details>
  <summary>Details</summary>
Motivation: 现有3D感知预训练方法要么是全隐式神经场（缺乏结构线索），要么是全显式几何基元（分辨率有限且泛化弱），两者各有缺陷。

Method: 在点云自编码器的潜伏空间中插入按点的潜伏变分自编码器，联合正则化点特征和坐标向高斯先验，生成紧凑潜伏表示。再结合轻量级3DGS渲染管线。

Result: 在RLBench、ManiSkill2和真实机器人平台上，任务成功率、样本效率和鲁棒性均强于基线。消融实验验证各组件必要性。

Conclusion: 混合表示策略有效融合隐式和显式优点，提升了3D操作任务的性能与泛化能力。
...
TL;DR: 提出仅使用关节传感反馈的髋部关节驱动灵巧手连续旋转立方体方法


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖视觉或触觉，忽视了最易获得的关节传感在肌腱驱动手上的潜力

Method: 教师策略通过强化学习（含特权物体信息）训练，蒸馏为仅依赖关节位置/速度历史的Proprioceptive Transformer

Result: 真实ORCA手上旋转速度比基线高3.1倍，立方体位置估计RMSE比MLP基线低23.4%

Conclusion: 关节传感可有效提取外部物体信息，实现无需外部感知的竞争性操作性能
...
TL;DR: 本文提出了一种无标定的接触式注册方法，通过将接触注册重新表述为互补形状对接，利用3D FFT相关和连续SE(3)优化，实现高精度机器人操作注册，无需外部传感器。


<details>
  <summary>Details</summary>
Motivation: 光学方法受限于长标定链、视线约束和制造误差，需要一种无标定的替代方案来实现高精度注册。

Method: 方法包括：通过互补形状对接建模接触注册，使用3D FFT相关在SO(3)低差异样本上进行全局到局部搜索，再通过李代数更新和解析接触灵敏度进行连续SE(3)优化。

Result: 在自由形态网格仿真中达到亚0.04 mm和亚0.4°精度，在牙齿预备机器人上实现0.42 mm和3.75°精度，优于光学跟踪器注册。

Conclusion: 该无接触注册策略为外科和工业机器人提供了一种实用且高精度的替代方案，无需外部传感器。
...
TL;DR: 提出了一种鸟瞰视角下的多组件风险场MC-Risk，实现了早期、校准且类别感知的风险定位，并在RiskBench碰撞子集上取得了最佳风险定位和最早危险指示。


<details>
  <summary>Details</summary>
Motivation: 现有风险场方法缺乏标准化定量评估，且未充分结合多模态预测、各向异性VRU建模和HD地图拓扑信息。

Method: 线性组合三个可解释模块：机动代理场（融合多模态轨迹预测与高斯环）、VRU风险场（前向偏置各向异性核）、道路惩罚场（利用HD地图拓扑）。

Result: 在RiskBench碰撞子集上取得最优风险定位和最早危险指示。

Conclusion: MC-Risk可作为无需额外训练的MPC代价密度，实现规划无关的风险感知轨迹生成。
...
### [290] [PointACT: Vision-Language-Action Models with Multi-Scale Point-Action Interaction](https://arxiv.org/abs/2605.21414)
*Shizhe Chen,Paul Pacaud,Cordelia Schmid*

Main category: cs.RO

TL;DR: 提出PointACT，一种3D感知的视觉-语言-动作（VLA）模型，通过多尺度点云注意力机制提升机器人操作的几何推理和空间定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型主要依赖2D视觉表示，限制了在3D环境中对精细几何和空间定位的推理能力。

Method: PointACT采用双系统架构，集成层次化3D点云表示到动作解码中，使用多尺度点-动作交互机制和高效瓶颈窗口自注意力。

Result: 在LIBERO和RLBench基准测试中，PointACT优于基线模型，尤其在RLBench-10Tasks套件上，成功率提升10%。

Conclusion: 紧密耦合层次化3D几何与预训练2D语义表示对鲁棒的空间定位机器人控制至关重要，并展示了预训练3D表示在3D感知VLA策略中的潜力。
...
### [291] [roto 2.0: The Robot Tactile Olympiad](https://arxiv.org/abs/2605.21429)
*Elle Miller,Jayaram Reddy,Ayush Deshmukh,Trevor McInroe,David Abel,Oisin Mac Aodha,Sethu Vijayakumar*

Main category: cs.RO

TL;DR: 提出了v2版机器人触觉奥林匹克基准测试，仅使用本体感觉和触觉的盲操作在四个机器人形态上表现优异，实现了十倍的旋转速度提升。


<details>
  <summary>Details</summary>
Motivation: 触觉强化学习受限于碎片化研究和过度关注饱和的定向任务，缺乏标准化基准和易于入门的工具。

Method: 开发了基于GPU并发的基准测试，采用端到端盲操作，仅依赖触觉和本体感觉，避免状态信息或知识蒸馏，并开源环境和调优基线。

Result: 盲智能体在10秒内完成13次保龄球旋转，速度比当前最先进水平提高一个数量级。

Conclusion: ROBO 2.0降低了研究门槛，使研究者能聚焦核心算法挑战，而非繁琐的RL调参。
...
### [292] [Lost in Fog: Sensor Perturbations Expose Reasoning Fragility in Driving VLAs](https://arxiv.org/abs/2605.21446)
*Abhinaw Priyadershi,Jelena Frtunikj*

Main category: cs.RO

TL;DR: 本文研究了视觉-语言-动作模型在自动驾驶中的鲁棒性，发现因果链解释的一致性可作为轨迹可靠性的高保真指标，推理一致性下降时轨迹偏差增大5.3倍，启用CoC生成可提升11.8%的轨迹准确性。


<details>
  <summary>Details</summary>
Motivation: 可解释的自动驾驶规划器需要解释在真实传感器退化下保持可靠，但现有研究缺乏对此类鲁棒性的量化评估。

Method: 对Alpamayo R1模型在1996个场景下施加八种传感器扰动（高斯噪声、光照极端、雾），进行约18,000次推理实验，分析CoC一致性、轨迹偏差及预处理防御效果。

Result: CoC变化时轨迹偏差增加5.3倍（21.8m vs 4.1m），相关系数r=0.99（跨攻击类型）和r_pb=0.53（每样本）；启用CoC平均提升11.8%轨迹准确性（p<0.0001）；噪声退化近似线性（R²=0.957），标准预处理防御效果有限。

Conclusion: CoC一致性可作为规划安全性的定量代理，基于推理的运行时监控有助于更安全的VLA部署。
...
### [293] [HITL-D: Human In The Loop Diffusion Assisted Shared Control](https://arxiv.org/abs/2605.21460)
*Riley Zilka,Sergey Khlynovskiy,Allie Wang,Martin Jagersand*

Main category: cs.RO

TL;DR: 提出HITL-D共享控制框架，结合扩散策略与人类控制，在多步、插入和精细操作任务中降低操作复杂度，提升用户表现。


<details>
  <summary>Details</summary>
Motivation: 现有自主操控系统能力很强，但人机共享控制中融合人类专业知识与扩散策略的研究尚不充分。

Method: 基于场景点云和末端执行器笛卡尔位置，利用扩散策略自动更新末端姿态，减少操作轴数，降低认知负荷。

Result: 12人用户研究中，任务完成时间减少40%，感知工作负荷降低37%，独立性、直观性和信心评分均提升。
...
### [294] [Coordinated Optimal Power Quality Management in Distribution Systems Using The Residual Capacity of Community IBRs](https://arxiv.org/abs/2605.21153)
*Tiantian Ji,Pengfeng Lin,Miao Zhu,Stephan M. Goetz,Ahmed Abu-Siada,Syed Islam*

Main category: eess.SY

TL;DR: 提出一种基于社区逆变器资源剩余容量的网络协调优化模型以缓解电压不平衡。


<details>
  <summary>Details</summary>
Motivation: 现有单序策略忽略耦合容量限制导致闲置裕度，且未能利用社区IBR的集体治理能力。

Method: 开发双公共同步参考系下的序域网络模型；通过多面体近似线性化严格相电流和视在功率约束；采用二次目标函数灵活平衡序容量分配。

Result: 仿真和实验验证了所提策略的有效性。

Conclusion: 所提模型能有效释放社区IBR的闲置容量以缓解电压不平衡。
...
### [295] [Time-To-Reach Separation and Safety Filtering for Safe, Fair, and Efficient Multi-Agent Coordination](https://arxiv.org/abs/2605.20625)
*Matthew Low,Jasmine Jerry Aloor,Victoria Marie Tuck,Pierluigi Nuzzo,Jason J. Choi*

Main category: eess.SY

TL;DR: 提出基于最小到达时间（TTR）的多智能体协调框架，用于城市空中交通走廊中的冲突避免。


<details>
  <summary>Details</summary>
Motivation: AAM操作将大幅增加城市空域交通密度，需自主管理系统确保高拥堵环境下无冲突运行。

Method: 以TTR为统一指标，用于优先级分配、时间间隔约束和安全过滤；结合Hamilton-Jacobi可达性值函数进行优先级一致的安全过滤。

Result: 在高拥堵走廊汇合场景中，该方法在安全、公平和效率上优于时间最优引导和无优先级安全过滤。

Conclusion: TTR指标可有效协调多飞行器汇合，提升整体运行性能。
...
### [296] [Cooling Channel Design Optimization for High Power Multi-chip Packages](https://arxiv.org/abs/2605.20657)
*Michael Acquah,Zheng Liu*

Main category: eess.SY

TL;DR: 该研究开发了一种基于物理的计算框架，用于优化高功率多芯片模块的嵌入式冷却通道布局。通过代理模型和混合整数二次规划算法，优化了叉指状冷却架构的几何参数，使峰值芯片温度降低140.45°C，平均温度降低35.87°C。


<details>
  <summary>Details</summary>
Motivation: 解决下一代高性能计算系统中热管理的挑战，特别是异构多芯片封装（如NVIDIA GB200）的散热问题。

Method: 建立稳态热传导与多孔介质冷却剂传输的耦合模型，参数化叉指状冷却架构，采用代理模型近似几何参数与温度的关系，并使用混合整数二次规划算法优化加权目标函数。

Result: 与基线配置相比，优化设计使峰值芯片温度降低140.45°C，平均芯片温度降低35.87°C。

Conclusion: 该框架能有效优化多芯片模块的冷却设计，显著提升热管理性能，对高性能计算系统有重要应用价值。
...
### [297] [Output Feedback Control of Linear Time-Invariant Systems with Operational Constraints](https://arxiv.org/abs/2605.21399)
*Marcel Menner,Heather Hussain,Eugene Lavretsky*

Main category: eess.SY

TL;DR: 本文提出一种基于输出反馈的鲁棒线性控制器设计方法，通过Nagumo定理和比较引理保证约束满足，同时融合控制障碍函数的min-norm最优控制原理。所设计的连续分段线性输出反馈策略保持了闭环系统的线性可分析性，并推导了多输入多输出系统的鲁棒裕度。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时满足操作约束、鲁棒性和线性系统分析能力，且输出反馈下约束保证问题尚未充分解决。

Method: 基于Nagumo定理和比较引理进行约束满足设计，结合控制障碍函数的最优控制原理，构建分段线性输出反馈策略。

Result: 推导了有无主动约束下的MIMO鲁棒裕度；通过飞行控制案例验证了该方法在安全关键场景的实用性。

Conclusion: 所提方法能有效保证状态约束满足，并保持线性系统理论的可分析性，适用于安全关键控制应用。
...
### [298] [Fully Actuated Manifold Constraint Based Output Feedback Control for Input-Constrained Uncertain Nonlinear Systems](https://arxiv.org/abs/2605.21439)
*Dianrui Mu,Changchun Hua,Yafeng Li,Jiannan Chen,Rao Wei*

Main category: eess.SY

TL;DR: 提出一种低复杂度、无模型、输出反馈控制器，用于未知时变非线性系统，在未知输入约束下实现预设控制精度及饱和后灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有约束控制方法多针对线性流形，缺乏对非线性流形及各类约束的普适性，且无法处理未知饱和后的控制精度问题。

Method: 采用无模型输出反馈架构，通过构造误差驱动的柔性约束，结合有限/固定时间控制理论，实现预设精度与饱和后自适应调整。

Result: 该方法在二阶与高阶系统仿真中验证了有效性，能在执行器未饱和时保持预设精度，饱和后仍维持可控的灵活精度。

Conclusion: 将线性流形约束控制扩展至非线性流形与多种约束形式，为未知饱和系统提供了理论保障与工程实用性。
...
### [299] [Transcoding a 3D Gaussian Splatting Model from a Plenoptic Point Cloud or Mesh without the Original Multi-view Images](https://arxiv.org/abs/2605.21051)
*Maja Krivokuća,Riad Bendouro,Neus Sabater*

Main category: eess.IV

TL;DR: 提出了一种端到端的转码流程，用于从3D全光点云或网格模型创建3D高斯泼溅（3DGS）模型，无需原始多视图图像。


<details>
  <summary>Details</summary>
Motivation: 当原始多视图图像不可用时，如何从现有3D模型高效生成高质量的3DGS表示。

Method: 设计了定制的初始化方法，通过约束确保3DGS模型与输入点云或网格表面紧密对齐。

Result: 在高质量全光点云数据集上，生成的3DGS模型视觉质量高且高斯点数远少于原始点云；定制初始化比默认SfM初始化收敛更快、表面更清晰。

Conclusion: 所提方法在无多视图图像的情况下仍能有效生成优质3DGS模型，且初始化策略优化了学习效率与表面精度。
...
### [300] [Local-sensitive connectivity filter (ls-cf): A post-processing unsupervised improvement of the frangi, hessian and vesselness filters for multimodal vessel segmentation](https://arxiv.org/abs/2605.21251)
...
TL;DR: 提出一种无监督的多模态视网膜血管分割方法，通过局部敏感连通性滤波器改进Frangi滤波器的响应，有效填补血管断裂，在多个数据集上取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 视网膜血管分析可用于眼部风险评估，现有Frangi滤波器在分割时存在血管不连续问题，需改进。

Method: 提出局部敏感连通性滤波器(LS-CF)，计算像素级血管连续性，并引入局部容差启发式填补Frangi响应中的血管断裂。

Result: 在OSIRIX、IOSTAR、DRIVE、STARE和CHASE-DB数据集上，LS-CF在准确率等指标上超越多数现有方法，优于所有无监督方法。

Conclusion: LS-CF是一种鲁棒且有效的无监督方法，在多模态视网膜血管分割中具有竞争力。
