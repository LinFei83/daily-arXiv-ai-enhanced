...
TL;DR: 提出BOHM方法，从路由权重中提取层次化归因树，实现零成本、多分辨率归因，与SHAP相比在特定场景下具有互补性。


<details>
  <summary>Details</summary>
Motivation: 现有Shapley-based归因方法需要评估任意组件子集，这对于第三方API、黑盒终端和动态路由系统不可行，且无法提供多层级归因。

Method: 直接从系统维护的路由权重构建层次化归因树：叶子归因为根到叶路径权重乘积，层级k归因为深度k节点的诱导分布。无需组件内部访问，零边际成本。

Result: 在18个LLM层次系统上Kendall tau=0.928（SHAP为0.980但需9000倍评估）；在5驱动7基准智能体研究中与SHAP一致性好；在475个叶子层级上tau达0.722。

Conclusion: BOHM满足效率、单调性、对称性和弱抑制性，但不满足可加性。它是Shapley的互补工具，其差异本身具有诊断价值。
...
TL;DR: 提出神经符号架构NeuroNL2LTL，结合神经翻译与形式验证，通过中间表示保证结构保持，验证结果作为强化学习奖励，实现86%的输出可满足性验证。


<details>
  <summary>Details</summary>
Motivation: 自然语言到时序逻辑的翻译缺乏可靠性，模板方法表达力不足，神经方法无法保证正确性。

Method: 神经符号架构，包括结构保持的中间表示、可满足性与非平凡性检查、最小编辑修复机制、基于验证反馈的强化学习训练。

Result: 在20万+需求上，语义等价率28%，可满足验证率86%，生成可解释的上下文解释。

Conclusion: 形式验证可作为神经规范系统的训练目标与运行过滤器，实现逻辑保证而非统计置信的可靠性。
...
TL;DR: 提出了Research Math Agents (RMA)框架，用于自动推理研究级数学问题，在First Proof基准上解决了10个问题中的8个，优于GPT-5.2R和Aletheia等基线。


<details>
  <summary>Details</summary>
Motivation: 现有研究集中于竞赛数学或形式化定理证明，缺乏针对研究级数学问题（需要长时推理、文献基础和迭代证明优化）的自动化方法。

Method: RMA将证明求解分解为问题分析、文献搜索与理解、公平比较、知识库构建和验证等模块，由初始化器、提议器和验证器代理通过共享结构化记忆协调工作。

Result: 在First Proof基准上，RMA解决了10个研究级问题中的8个，产生更逻辑严密和易读的证明，消融实验表明性能提升来自模块交互、迭代优化和验证反馈。

Conclusion: RMA框架通过多角色多轮协作和结构化记忆，有效提升了研究级数学问题的自动化推理能力。
...
TL;DR: SciAtlas是一个覆盖26个学科、4300万篇论文的多学科异构知识图谱，包含1.57亿实体和30亿三元组，通过神经符号检索算法实现从语义匹配到确定性关联发现的转变，作为“认知地图”赋能全链条自动化科研并降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 全球学术产出指数级增长导致信息爆炸，现有检索工具依赖表面关键词匹配或向量语义检索，缺乏拓扑推理能力，且深度研究框架易产生逻辑幻觉和高推理成本。

Method: 构建多学科异构知识图谱SciAtlas，集成4300万篇论文、1.57亿实体和30亿三元组，开发三路径协同召回与图重排序的神经符号检索算法。

Result: 实现在文献综述、研究趋势合成、创意定位和学术轨迹探索等应用中作为有效“认知地图”，显著降低推理成本并赋能全链条自动化科研。

Conclusion: SciAtlas通过结构化拓扑认知基底打破学科壁垒，为AI代理提供全局视角，神经符号算法从语义匹配过渡到确定性发现，有效支持自动化科学研究。
...
TL;DR: 提出了面向智能体系统的能耗评估框架A-LEMS，以每成功目标能耗替代每次推理能耗，并定义了编排开销指数。


<details>
  <summary>Details</summary>
Motivation: 现有AI能耗基准以单次模型调用为单位，但智能体系统中多次交互使调用次数失去意义，需要重新定义任务级能耗度量。

Method: 通过A-LEMS框架实现跨层能耗测量，包括时间边界模型、五层观测管道和可重复性协议；定义了EpG（每成功目标能耗）和OOI（编排开销指数）。

Result: 智能体工作流的每成功目标能耗比线性基线高4.33倍（888.1 J vs 205.3 J），但工具增强任务中OOI可低于1.0倍，表明开销源于编排结构而非固定偏差。

Conclusion: 每次推理能耗不足以衡量智能体系统，EpG和OOI提供了准确基准，编排结构是能耗的主要决定因素。
...
TL;DR: 本文提出EVE-Agent，通过证据可验证机制确保自进化搜索代理的训练数据可信，实验证明其显著提升基于证据的准确性。


<details>
  <summary>Details</summary>
Motivation: 自进化代理可能因训练缺乏可验证证据而产生不可靠的生成结果，需要确保每个训练实例有可溯源且可测量的证据支撑。

Method: 修改提议者-求解框架，生成器输出问题、答案和原文证据片段，证据验证器根据证据带来的边际准确率增益给予奖励，无需外部标注。

Result: 在自进化搜索代理任务中，EVE-Agent的证据基础正确性显著优于先前方法，且训练实例均可审计。

Conclusion: 证据可验证性是自进化代理可信的关键，EVE-Agent实现了无需人工干预的可信训练闭环。
...
TL;DR: 本文提出PathCal，一种无训练的解码控制器，通过区分反思标记类型并仅在局部不确定状态干预，在保持或提升推理准确性的同时减少生成长度。


<details>
  <summary>Details</summary>
Motivation: 现有测试时控制方法将反思标记视为单一粗粒度类别，忽略了其不同功能角色和影响时机。

Method: PathCal利用反思标记的分布估计轨迹间局部竞争，软性调整标记逻辑值以平衡维持与分支探索。

Result: 在六个推理基准上，PathCal实现了更好的效率-性能权衡，提高或保持准确率，同时减少生成长度。

Conclusion: 区分标记类型并在适当时机干预能有效校准推理路径，无需外部验证器或额外采样。
...
### [8] [The Deterministic Horizon: Impossibility Results as Design Specifications for Trustworthy AI Systems](https://arxiv.org/abs/2605.23024)
*Dongxin Guo*

Main category: cs.AI

TL;DR: 该论文将人工智能中的不可能性结果转化为设计规则，提出了确定性地平线等概念，并构建了一个包含16条规格的目录，每一条都包括可计算的边界、量化的违反成本和建设性设计规则。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在软件编写、法律文档起草和临床记录生成等方面展现出强大能力，但存在诸如图灵、阿罗和没有免费午餐定理等基本限制。这些限制启发研究者将不可能结果从理论好奇转化为实际设计规则，以指导可靠AI的开发。

Method: 论文通过理论分析和数学推导，证明了确定性地平线的存在，即由架构单独决定准确率上限，与训练数据量、适配器秩或损失函数无关。同时，利用信息论转换和电路复杂性理论，推导出跨领域的统一框架，包括偏好学习、检索管线、拍卖机制和神经网络验证。

Result: 主要结果包括：确定性地平线在12种Transformer架构中位于19至31之间，微调可恢复不到4个百分点；模幂运算的无条件电路复杂度下界；偏好学习中的样本复杂度跳跃；多阶段检索管线的度量数量要求；可信拍卖的局限性；以及零知识验证中每非线性激活110-190倍的开销。

Conclusion: 每个AI的基本限制都同时是一个设计规则。论文提供的“不可能性-规格”方法论适用于可信AI的生成式研究计划，强调通过可计算边界、量化成本和建设性规则来指导系统设计。
...
### [9] [Inductive Deductive Synthesis: Enabling AI to Generate Formally Verified Systems](https://arxiv.org/abs/2605.23109)
...
TL;DR: 本文提出归纳演绎综合（IDS）方法，实现分布式键值存储系统的全自动形式化验证与实现，7/7规范通过，平均6.8小时/$106，比专家快200倍，比现有智能体便宜17%。


<details>
  <summary>Details</summary>
Motivation: 现有AI代码智能体在需要形式化保证的全覆盖任务（如分布式系统一致性）上表现不佳，即使是SOTA智能体也仅通过2/7规范。

Method: 归纳演绎综合（IDS）：联合增量合成实现与证明，从失败尝试中学习，利用LLM智能体系统系统性探索策略。

Result: IDS在7个分布式键值存储规范上全部成功，平均耗时6.8小时、成本$106，比专家快约200倍，比SOTA智能体便宜17%，且性能优化后实现比已有验证系统快3倍。

Conclusion: IDS是首个有效解决AI智能体形式化验证与实现自动合成差距的方法，显著降低了时间和成本。
...
### [10] [Redrawing the AI Map: A Theory of Accountability Boundaries in Agentic Ecosystems](https://arxiv.org/abs/2605.23179)
*Muhammad Zia Hydari,Farooq Muzaffar*

Main category: cs.AI

TL;DR: 提出了一个能力级理论，解释在代理生态系统中，当AI使能能力的执行边界变得模块化时，其问责边界为何可能保持集成。


<details>
  <summary>Details</summary>
Motivation: 现有研究认为AI编排器能降低成本并促进模块化，但忽略了AI输出需证据、审查和签收等问责要求，导致能力可能仍需集成。

Method: 引入问责资产概念，结合验证成本和责任可转移性两个核心因素，通过交易成本理论、互补资产理论等开发了边界策略和规则债务等理论框架，并基于五个应用场景进行结构化说明。

Result: 识别出三种边界策略（组件、集成、双轨），提出规则债务概念，并建立了七个命题将代理组装成本降低、问责资产、可挪用性等与边界策略、价值获取和规则债务联系起来。

Conclusion: 代理系统执行边界与问责边界可分离，但必须考虑验证成本和责任转移性；规则债务是组织治理的重要负担；该理论解释了何时数字化模块化导致组织分解，何时问责使能力保持集成。
...
### [11] [AutoResearch AI: Towards AI-Powered Research Automation for Scientific Discovery](https://arxiv.org/abs/2605.23204)
...
TL;DR: 本文综述了AI驱动的科学工作流自动化（AutoResearch）的发展，从任务级AI到工作流级科研自动化，涵盖文献、假设、实验、验证和报告等环节，并提出了五个评估维度。


<details>
  <summary>Details</summary>
Motivation: 当前AI科学系统碎片化，缺乏证据保存、可重复性、溯源等能力，且未实现鲁棒自治，因此需要系统化梳理和评估AutoResearch的进展。

Method: 通过文献综述，分析AI科学系统的控制、证据、执行、验证和问责分配，围绕五个工作流条件组织领域，并综合AI科学家系统、共研框架、基准和开源基础设施。

Result: AutoResearch自主性受领域制约，在结构化、可执行且快速验证的场景中更可信，但在具身、延迟、异质、伦理或制度问责的背景下受限。

Conclusion: 提出了新颖性、有效性、影响力、可靠性和溯源五个评估维度，指出AI自动化科研需进一步解决鲁棒性和跨领域泛化问题。
...
### [12] [Foundation Protocol: A Coordination Layer for Agentic Society](https://arxiv.org/abs/2605.23218)
...
TL;DR: 提出了面向人机社会的图优先协调层Foundation Protocol，支持异构实体统一、多方组织、事件协作与经济原语，并内置策略、溯源与审计。


<details>
  <summary>Details</summary>
Motivation: 自主智能体规模扩大后，原始模型能力不再关键，协调成为瓶颈，现有方案无法支撑可靠关系、多智能体工作、AI经济与问责。

Method: 基于图结构的协调协议，统一代理、工具、资源、人类、机构与组织，支持原生多方组织与事件协作，提供计量、收据与结算经济原语，将策略、溯源与审计作为一等公民。

Result: 可封装并桥接现有协议实现渐进采纳，降低集成与治理开销，保持自主代理可组合性与问责性。

Conclusion: Foundation Protocol可作为开放、多元且可治理的人机社会的共享协调基础设施。
...
### [13] [GENSTRAT: Toward a Science of Strategic Reasoning in Large Language Models](https://arxiv.org/abs/2605.23238)
*Vartan Shadarevian,Kia Ghods,Alex Kenich,Anany Kotawala*

Main category: cs.AI

TL;DR: GENSTRAT是一个通过程序生成战略环境来评估LLM在复杂场景中行为的新框架，包含能力剖面和不平滑度测量。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试可能饱和且无法推广到真实部署环境，需要更稳健和可泛化的评估方法。

Method: 生成两人零和、不完美信息的纸牌游戏分布，结合六维能力剖面和领域内不平滑度测量，进行大规模头对头锦标赛。

Result: 新前沿模型平均得分更高，但相近强度的模型能力剖面不同；GPT-5和Claude局部波动高于Gemini-3.1-pro。

Conclusion: 能力剖面和不平滑度提供了整体排名无法揭示的部署相关诊断信息。
...
### [14] [Design and Report Benchmarks for Knowledge Work](https://arxiv.org/abs/2605.23262)
*Yining Hua,Hongbin Na,Cyrus Ayubcha,Levi Lian*

Main category: cs.AI

TL;DR: 本文批判了当前LLM智能体在知识工作评估中仍沿用传统NLP任务逻辑的现状，提出了一个三步框架：定义工作活动、明确测试场景、评分工作产品。通过分析三个案例，揭示了基准测试设计如何决定分数所能支撑的工作主张的范围与限制。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试的高分无法可靠反映系统在真实知识工作场景中的部署性能，因为评估设计仍遵循传统NLP任务逻辑，缺乏对知识工作本质（如角色职责、本地材料、下游工作流等）的考虑。

Method: 提出三步框架：1）定义被评估的工作活动；2）指定测试场景（包括材料、工具、角色和约束）；3）对系统留下的工作产品进行评分。同时从O*NET数据库中提取18种工作活动清单，并通过三个案例分析该方法的应用。

Result: 三个案例（GDPval、OfficeQA Pro、APEX-SWE）显示，基准测试的设计选择决定了分数所能支撑的最强工作主张，且常见问题出现在对工作活动的定义模糊、测试场景脱离实际、评分聚焦于简单答案而非工作产品。

Conclusion: 未来的知识工作评估应更关注工作活动本身与测试场景的真实性，评分应围绕系统产出的工作产品及其在下游工作流中的可用性，而非仅依赖传统NLP指标。
...
### [15] [Parallel Context Compaction for Long-Horizon LLM Agent Serving](https://arxiv.org/abs/2605.23296)
*Musa Cim,Burak Topcu,Chita Das,Mahmut Taylan Kandemir*

Main category: cs.AI

TL;DR: 提出并行压缩方法用于长程智能体对话历史管理，在保持摘要体积可预测的同时显著提升吞吐量并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 长程LLM智能体的对话历史会超过模型上下文窗口，现有顺序摘要方法存在信息丢失、推理延迟高、摘要体积不可控等问题。

Method: 采用并行压缩架构，将长对话历史分块并行进行摘要生成，通过细粒度分块提示工程控制输出体积。

Result: 在HotpotQA和LoCoMo基准上，使用8B-120B参数的多种架构（稠密/MoE/推理/非推理）验证，并行方法在匹配摘要体积下减少端到端耗时、提升压缩吞吐量。

Conclusion: 并行压缩为长程智能体提供了可控、高效的上下文管理方案，避免了顺序方法的随机性和延迟瓶颈。
...
### [16] [Ontological Knowledge Blocks: Executable Compliance and Profile-Based Validation for Trustworthy AI Systems](https://arxiv.org/abs/2605.23297)
*Aasish Kumar Sharma,Julian M. Kunkel*

Main category: cs.AI

TL;DR: 本文提出一种名为本体知识块（OKB）的可编程治理基础设施，通过将监管义务编译为可机器检查的约束，实现了AI服务的自动化合规验证。


<details>
  <summary>Details</summary>
Motivation: 当前AI治理依赖文档、静态清单和人工审查，无法满足自动化AI系统的可扩展性需求。

Method: 将OKB定义为5元组，结合RDF/OWL概念模式、SHACL验证规则、证据要求和PROV-O溯源链接，通过监管编译器将IR记录转化为可组合的知识块模块。

Result: 在AI辅助HPC资源分配的24次验证和4个治理配置文件中，SHACL验证延迟为12.6-100.3毫秒，违反累积呈严格加性，配置文件等价测试表明Combined为最全面配置。

Conclusion: OKB支持基于配置文件的治理重配置，无需修改服务代码，验证结果具有可扩展性和可审计性，所有工件开源。
...
### [17] [DART: Semantic Recoverability for Structured Tool Agents](https://arxiv.org/abs/2605.23311)
*Ke Yang,Panpan Li,Zonghan Wu,Kejin Xu,Huaxi Huang,Xiaoshui Huang*

Main category: cs.AI

TL;DR: 本文提出DART运行时，在结构化工具代理执行失败时，通过语义可恢复性检查实现安全高效的局部恢复，避免全局重放或无效恢复。


<details>
  <summary>Details</summary>
Motivation: 现有恢复方法仅提供机械回滚，缺乏判断局部恢复是否语义有效的准则，导致承诺敏感场景中下游工作可能因历史不一致而失效。

Method: DART通过定位失败实例、认证语义可恢复边界、对齐检查点并选择保留下游工作的可接受恢复点，在依赖和效果约束下确保有效性。

Result: 在三个LLM领域及LangGraph基座上，DART正确恢复所有评估的承诺敏感案例（基线局部恢复失败时），安全审计未发现不可接受的回滚。

Conclusion: 控制器合法性不保证语义有效性，合理的局部恢复需要显式的可接受性检查。
...
### [18] [Human-in-the-Loop Multi-Agent Ventilator Decision Support with Contextual Bandit Preference Learning](https://arxiv.org/abs/2605.23320)
*Sijia Li,Xiaoyu Tan,Qixing Wang,Weiyi Zhao,Chen Zhan,Teqi Hao,Xuemin Wang,Lei Gu,Roland Eils,Xihe Qiu*

Main category: cs.AI

TL;DR: 提出了VDSS，一种人机协同多智能体框架，通过契约驱动接口协调模块化决策，实现个性化、可审计的呼吸机决策支持。


<details>
  <summary>Details</summary>
Motivation: 现有呼吸机决策方法（规则系统泛化性差，强化学习/大模型难控制审计）难以兼顾个性化、安全边界和临床交互。

Method: 基于合约驱动接口的多智能体框架，采用上下文bandit在线学习临床偏好，并通过结构化拒绝反馈触发重规划。

Result: 回顾性ICU轨迹回放显示，VDSS相比基线提高了推荐可接受性，减少了达成可接受计划的交互轮次。

Conclusion: VDSS支持临床可部署的人机协作，通过可追踪证据和偏好适应提升了交互稳定性与效率。
...
### [19] [When Planning Fails Despite Correct Execution: On Epistemic Calibration for LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2605.23414)
*Zehao Wang,Shilong Jin,Zhao Cao,Lanjun Wang*

Main category: cs.AI

TL;DR: 提出一种基于信息一致性的校准框架（EPC-AW），用于检测和纠正多智能体系统中因知识误判导致的计划可行性错误。


<details>
  <summary>Details</summary>
Motivation: 现有系统在计划执行正确时仍会失败，源于智能体评估计划可行性时的知识误判，该问题具有潜在性和动态性。

Method: 通过信息一致性评分选择计划，并利用一致性引导的认知状态细化机制，基于历史差异动态调整校准。

Result: 在多个任务上平均系统级成功率提升9.75%。

Conclusion: EPC-AW能有效缓解多智能体系统中的认知误校准问题，提升规划可靠性。
...
### [20] [EDGE-OPD: Internalizing Privileged Context with Evidence Guided On-Policy Distillation](https://arxiv.org/abs/2605.23493)
*Aristotelis Lazaridis,Dylan Bates,Aman Sharma,Brian King,Vincent Lu,Jack FitzGerald*

Main category: cs.AI

TL;DR: 本文提出EDGE-OPD方法，通过引导rollouts和证据掩码解决On-Policy自蒸馏中特权信息导致的副作用问题，在保留通用能力的同时成功学习目标身份。


<details>
  <summary>Details</summary>
Motivation: On-Policy自蒸馏（OPSD）中特权信息会引发非预期行为改变（如推理退化、风格偏移），导致模型学习副作用而非目标可迁移行为。

Method: 提出EDGE-OPD：a）引导rollouts在采样时注入特权上下文行为；b）证据掩码仅更新特权上下文支持token的位置。

Result: OPSD及RLSD变体完全无法学习目标身份，而EDGE-OPD成功实现；掩码消融实验表明身份信号集中在正向证据尾部，且保留通用能力。

Conclusion: EDGE-OPD有效实现高效知识迁移与通用能力保持，为特权信息蒸馏提供新范式。
...
### [21] [CP or DP? Why Not Both: A Case Study in the Partial Shop Scheduling Problem](https://arxiv.org/abs/2605.23569)
*Emma Legrand,Roger Kameugne,Pierre Schaus*

Main category: cs.AI

TL;DR: 本文提出一种动态规划与约束规划混合的方法，用于求解部分作业调度问题，并展示了其可行性和灵活性。


<details>
  <summary>Details</summary>
Motivation: 动态规划和约束规划通常被分别使用，本文旨在证明二者可以有效结合，以发挥各自优势。

Method: 以动态规划为主搜索框架，约束规划作为子程序利用全局约束传播，支持任意时间策略和大邻域搜索。

Result: 混合方法灵活处理任意优先级约束，并支持大邻域搜索，但在求解性能上不如纯约束规划求解器。

Conclusion: 主要贡献是证明了动态规划与约束规划混合集成的可行性和有效性。
...
### [22] [Co-ReAct: Rubrics as Step-Level Collaborators for ReAct Agents](https://arxiv.org/abs/2605.23590)
*Jiazheng Kang,Bowen Zhang,Zixin Song,Jiangwang Chen,Xiao Yang,Da Zhu,Guanjun Jiang*

Main category: cs.AI

TL;DR: Co-ReAct 通过推理时注入逐步指南（rubrics）来引导智能体的行动选择，相比 ReAct 在深度搜索任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有 ReAct 智能体依赖内部判断导致搜索轨迹浅层或冗余，而现有 rubrics 仅用于评估或训练，无法在推理时指导行动。

Method: 提出 Co-ReAct 框架，在每一步推理时注入逐步 rubric 指导智能体的搜索、推理或自我评估决策，并训练专用 rubric 生成器（使用 GRPO 优化，以列表级 Spearman 排序奖励为目标）。

Result: 在 DeepResearchBench 和 SQA-CS-V2 上，Co-ReAct 在多种基座模型（8B/14B 开源及闭源）上均优于 ReAct 及代表性测试时计算基线。

Conclusion: 逐步 rubric 可作为即插即用组件提升现有智能体的推理表现，无需修改其底层决策机制。
...
### [23] [Solving the Aircraft Disassembly Scheduling Problem](https://arxiv.org/abs/2605.23592)
*Charles Thomas,Pierre Schaus*

Main category: cs.AI

TL;DR: 本文详细描述了飞机拆解调度问题，并提出约束规划与混合整数规划两种求解方法，基于真实数据验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 飞机拆解过程复杂且利润低，优化调度以提高可持续性和经济可行性。

Method: 分别构建了约束规划模型和混合整数规划模型。

Result: 在包含多达1450个任务的实际规模案例上测试了两种模型。

Conclusion: 两种方法均能有效解决飞机拆解调度问题。
...
TL;DR: 提出PCSP方法，实现实时、可扩展、个性一致的非玩家角色控制，在300人基准测试中零样本个性识别提升17倍，推理速度比基线快22倍。


<details>
  <summary>Details</summary>
Motivation: 生命模拟游戏需要大量具有一致个性但可控的NPC，现有方法无法同时满足个性一致性、可控性和实时推理需求。

Method: PCSP：基于冻结大语言模型嵌入的共享强化学习策略，结合低秩投影、神经个性调节及PPO+InfoNCE一致性+KL多样性训练目标。

Result: 零样本个性识别显著高于随机水平（17倍），语义-行为对齐斯皮尔曼相关系数约0.73，推理速度提升22倍，且在不可见词汇和复杂多智能体环境中表现稳健。

Conclusion: 共享强化学习策略能支持实时、可扩展、个性调节的NPC控制，有助于构建大规模智能游戏世界。
...
### [25] [MemAudit: Post-hoc Auditing of Poisoned Agent Memory via Causal Attribution and Structural Anomaly Detection](https://arxiv.org/abs/2605.23723)
...
TL;DR: 提出MemAudit框架，用于事后审计记忆增强型大语言模型代理的因果记忆注入攻击。


<details>
  <summary>Details</summary>
Motivation: 现有防御仅针对在线干预，无法事后识别导致有害行为的存储记忆。

Method: 结合反事实记忆影响分数和记忆一致性图，检测恶意注入记录。

Result: 在QA和推理代理场景下，攻击成功率分别从70%降至0%和83.3%降至0%。

Conclusion: MemAudit能有效实现事后因果记忆审计并消除记忆注入攻击。
...
### [26] [Beyond Binary Edits Robust Multimodal Knowledge Editing with Adversarial Subspace Alignment](https://arxiv.org/abs/2605.23780)
*Haoyuan Wang,Xiaohao Liu,Jiajie Su,Jianmao Xiao,Chaochao Chen*

Main category: cs.AI

TL;DR: 提出LAR和RCSL方法增强多模态大语言模型知识编辑的鲁棒泛化性。


<details>
  <summary>Details</summary>
Motivation: 现有内在多模态知识编辑泛化性不足，缺乏语义监督且编辑范围僵化。

Method: 通过LAR生成语义对抗变体，结合RCSL的低秩子空间对齐。

Result: 实验验证ASAM框架在知识编辑泛化上的有效性。

Conclusion: 显式对抗鲁棒化可提升多模态知识编辑的语义一致性。
...
### [27] [Agentic Proving for Program Verification](https://arxiv.org/abs/2605.23772)
*Alessandro Sosso,Akhil Arora,Bas Spitters*

Main category: cs.AI

TL;DR: 该论文评估了Claude Code在Lean 4程序验证基准CLEVER上的表现，发现其能生成高质量规范、通过验证，并显著超越现有基准难度。


<details>
  <summary>Details</summary>
Motivation: 现有自动定理证明方法在程序验证领域的实际能力尚未被充分评估，特别是面对现代智能体系统时。

Method: 采用Claude Code作为智能体，在CLEVER基准上执行端到端规格生成、实现验证等任务，并通过孤立分析、反馈质量评估等方法测试性能。

Result: Claude达到了98.8%的规范生成成功率，81.3%的规范被接受，87.5%的实现通过验证，端到端成功率为98.1%，且能提供高质量反馈。

Conclusion: 当前程序验证基准已不足以评估现代智能体系统，需要更严格、容错性更强的评估方法；编译器参与的智能体范式是最有效的程序验证方式。
...
### [28] [SPACENUM: Revisiting Spatial Numerical Understanding in VLMs](https://arxiv.org/abs/2605.23898)
*Jianshu Zhang,Yijiang Li,Huifeixin Chen,Haoran Lu,Letian Xue,Bingyang Wang,Han Liu*

Main category: cs.AI

TL;DR: 本文提出SpaceNum框架，系统性评估视觉语言模型在空间数值理解上的表现，发现模型普遍未能将数值与空间含义建立连接，表现接近于随机猜测。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在具身环境中需输出数字（如动作幅度、空间坐标），但这些数字是否真正基于空间感知尚不明确，因此需要系统性研究。

Method: 建立SpaceNum统一框架，包含动态转换和静态布局两种设置，并提出Num2Space和Space2Num双向任务，结合错误分析、推理轨迹分析和受控干预进行评测。

Result: 模型在空间数值理解上表现差，常接近随机猜测；依赖浅层空间线索，缺乏稳定的坐标感知表示，无法从视觉观测中抽象出结构化布局；显式推理仅带来微小提升，微调才能部分改善性能。

Conclusion: 当前视觉语言模型在空间数值理解上存在根本性缺陷，微调可带来一定改进且能迁移至其他空间推理基准。
...
### [29] [From Raw Experience to Skill Consumption: A Systematic Study of Model-Generated Agent Skills](https://arxiv.org/abs/2605.23899)
...
TL;DR: 本文提出了一个评估框架，系统研究了语言代理中领域级和模型生成技能的整个生命周期，发现这类技能平均有益但存在负迁移，且效用不依赖于模型规模。


<details>
  <summary>Details</summary>
Motivation: 现有关于技能提取的研究不断增多，但缺乏对整个技能生命周期（经验生成、技能提取、技能消费）的全面理解，不清楚技能是否有效、何时有效及其成功或失败的原因。

Method: 构建了一个效用导向的评估框架，涵盖五种不同的代理任务领域，对提取器和目标代理进行系统实验，并深入分析生命周期各阶段。

Result: 模型生成的技能平均有益，但存在非平凡的负迁移；提取器和目标代理的行为不一致；技能效用独立于模型规模或基线任务强度。

Conclusion: 通过将发现转化为元技能，指导技能提取关注实际效用特征，能持续提高技能质量并显著减少负迁移。
...
### [30] [SkillOpt: Executive Strategy for Self-Evolving Agent Skills](https://arxiv.org/abs/2605.23904)
...
TL;DR: 提出SkillOpt，首个系统化的文本空间技能优化器，通过受控编辑和验证机制稳定提升冻结代理的技能表现。


<details>
  <summary>Details</summary>
Motivation: 现有代理技能手动构建、一次性生成或失控自修正，无法像深度学习优化器那样依靠反馈可靠提升初始性能。

Method: 使用独立优化器模型将评分轨迹转为对技能文档的有限增/删/改编辑，仅接受严格提升验证分数的编辑；引入文本学习率预算、拒绝编辑缓冲区和逐轮慢/元更新以稳定训练。

Result: 在6个基准、7个目标模型、3个执行框架（直接聊天、Codex、Claude Code）的52个评估单元中均最优或持平，GPT-5.5直接聊天提升+23.5，Codex循环内提升+24.8，Claude Code内提升+19.1，且技能可迁移至不同模型、环境和相近基准。

Conclusion: 将技能作为冻结代理的外部状态进行可重复优化是可行的，SkillOpt实现了零额外推理调用下的稳定技能提升和跨场景迁移。
...
### [31] [GEM-4D: Geometry-Enhanced Video World Models for Robot Manipulation](https://arxiv.org/abs/2605.22882)
...
TL;DR: GEM-4D 通过注入密集的4D对应监督，将几何先验知识融入视频世界模型，实现了动作一致且几何精准的未来视频生成，并可直接用于机器人操控。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型缺乏点级运动一致性，生成的视频看似合理但缺乏物理基础，难以用于可靠的机器人操控。

Method: 在训练时从预训练的几何基础模型中蒸馏出密集4D对应监督，注入视频生成骨干网络；同时引入逆动力学模块，将对应一致的视频推演转化为可执行的机器人轨迹。

Result: 在视频预测和几何一致性上达到最先进水平，真实世界操控成功率从61%提升至81%。

Conclusion: GEM-4D 能有效生成几何一致的视频，并直接应用于机器人操控，无需额外推理成本。
...
### [32] [Seeing without Looking: Do Vision-Language Benchmarks Really Test Vision?](https://arxiv.org/abs/2605.22903)
*Zixuan Lan,Luzhe Sun,Matthew R. Walter,Jiawei Zhou*

Main category: cs.CV

TL;DR: 发现视觉语言模型在幻觉基准测试中对图像标记的移除不敏感，表明标准准确率无法可靠评估细粒度视觉基础。


<details>
  <summary>Details</summary>
Motivation: 观察到去除大量图像标记仅轻微降低模型性能，质疑基准准确率是否真正反映视觉理解。

Method: 通过全局视觉退化、局部遮挡、问题重构、答案空间扩展和决策级分析，结合分层视觉标记几何分析。

Result: 模型预测对细粒度视觉证据损失不敏感，且深层视觉标记相似性增加。

Conclusion: 当前基准不足以可靠评估视觉语言模型的细粒度视觉基础。
...
### [33] [Suicide Risk Assessment from AI-powered Video Surveillance: An Interpretable Framework for Prevention in Metro Stations](https://arxiv.org/abs/2605.22904)
*Safwen Naimi,Wassim Bouachir,Guillaume-Alexandre Bilodeau,Brian Mishara*

Main category: cs.CV

TL;DR: 本文提出了地铁站自杀风险评估（SRA）的首个可解释框架，通过人员跟踪、活动识别、站台语义分割和轨迹风险热图建模，在真实监控数据上达到83.2%的ROC-AUC性能。


<details>
  <summary>Details</summary>
Motivation: 地铁站监控视频中及早识别高风险情境对自杀预防至关重要，而现有方法缺乏综合行为、空间和时间线索的可解释评估方案。

Method: 结合人员跟踪、活动识别、站台语义分割和轨迹驱动风险热图建模，实现从多源行为线索积累证据的端到端评估流水线。

Result: 在真实监控数据上实现83.2%的ROC-AUC，证明了该框架的有效性。

Conclusion: 自杀风险评估是一个复杂任务，该可解释框架为AI社会公益研究开辟了新方向，并推动了标准化评估基准的建立。
...
### [34] [VideoOdyssey: A Benchmark for Ultra-Long-Context and Omni-Modal Video Understanding](https://arxiv.org/abs/2605.22907)
*Haichen He,Jiayi Zhou,Sifeng Shang,Yihan Hu,Yuanhan Zhang,Kaiyang Zhou*

Main category: cs.CV

TL;DR: 提出了VideoOdyssey基准，专注于超长视频（平均109分钟）和多模态理解，通过持续证书长度衡量推理难度，揭示现有模型在连续推理和细粒度感知上的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准的视频理解任务仅需处理短片段，无法衡量超长语境推理所需的认知负荷。

Method: 设计了两个子集（视觉版和音视频版），定义了多级持续证书（秒到小时级），覆盖11个领域54个子类。

Result: 当前多模态大模型不仅存在简单检索困难，更在跨语境的连续推理、细粒度感知及非语言多模态理解上表现不佳。

Conclusion: VideoOdyssey能有效诊断模型在不同语境长度和认知负荷下的能力瓶颈，推动超长视频理解研究。
...
### [35] [Improved Vision-to-Chart Buoy Association with Learned World-to-Image Projection](https://arxiv.org/abs/2605.22942)
*Borja Carrillo-Perez*

Main category: cs.CV

TL;DR: 提出一种轻量级QueryMLP模块，显式预测浮标在水面接触点像素坐标，作为空间先验辅助DETR解码器，在MaCVi 2026挑战中获得第二名（Overall=0.7386）。


<details>
  <summary>Details</summary>
Motivation: 基线方法中解码器仅使用世界坐标和方位查询，需隐式学习复杂几何投影，导致几何推理负担重。

Method: 训练专用MLP（QueryMLP），利用图表测量和IMU姿态数据显式预测浮标在图像中的像素坐标，并将其附加到解码器查询向量中。

Result: 在测试集上取得Overall=0.7386，F1=0.8055，mIoU=0.6718，在提交中排名第二。

Conclusion: 显式空间先验降低了Transformer解码器的几何推理难度，有效提升数据关联性能。
...
### [36] [GazeBehavior Annotation Toolkit (GBAT): AI-powered toolkit for automatic annotation of egocentric eye-tracking and video data of child-caregiver interaction](https://arxiv.org/abs/2605.22962)
*Iba Baig,Kevin Li,Yanbin Xu,Seiji Cattelain,Marie Hallo,Hayato Ono,Sho Tsuji,Ming Bo Cai*

Main category: cs.CV

TL;DR: 提出了一种基于深度学习的自动化工具包（GazeBehavior Annotation Toolkit），用于简化儿童与看护者互动视频中的后同步、视线目标标注及姿势动作分类等数据处理流程。


<details>
  <summary>Details</summary>
Motivation: 手动注释视频数据耗时费力，限制了大规模纵向研究的发展，为提高特征提取的效率和可扩展性。

Method: 采用深度学习技术，实现多视频后同步、视线目标类别的半自动标注以及参与者姿态和手部动作的分类。

Result: 该工具包有效提升了从人类自我中心眼动及视频数据中提取特征的效率和可扩展性。

Conclusion: 该工具支持大规模、纵向研究中的注意力动态和婴幼儿自然行为分析，有助于理解注意力与动作、语言在实时互动中的交互。
...
### [37] [CoMoGen: COntrollable MOtion Dynamics and Interactions with Mask-Guided Video GENeration](https://arxiv.org/abs/2605.22996)
*Adil Meric,Lin Geng Foo,Mert Kiray,Benjamin Busam,Rishabh Dabral,Christian Theobalt*

Main category: cs.CV

TL;DR: CoMoGen是一种可控视频生成框架，利用轻量级MaskAdapter和MMDiT中的Motion Layers实现动态生成。


<details>
  <summary>Details</summary>
Motivation: 现有可控视频生成方法在运动控制上存在不足，且MMDiT架构难以识别运动相关层。

Method: 提出MaskAdapter注入掩码残差，通过LoRA微调确定的Motion Layers。

Result: 在多个数据集上优于现有方法，实现运动保真度和感知真实感的最佳性能。

Conclusion: CoMoGen能高效实现精确主题运动与场景交互。
...
### [38] [Scene Reconstruction as Mapping Priors for 3D Detection](https://arxiv.org/abs/2605.22997)
...
TL;DR: 提出了一种利用自动构建地图先验增强3D检测的框架MPA3D，在Waymo数据集上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 地图未被充分用于感知任务（如3D检测），且传统高清地图获取维护成本高。

Method: 自动从聚合传感器数据构建密集地图先验，并设计MPA3D框架融合地图先验与多模态传感器数据。

Result: 在Waymo Open Dataset上取得新SOTA结果。

Conclusion: 可扩展重建场景先验能有效提升3D检测性能。
...
### [39] [The TIME Machine: On The Power of Motion for Efficient Perception](https://arxiv.org/abs/2605.23045)
*Mantas Skackauskas,Xinyue Hao,Laura Sevilla-Lara*

Main category: cs.CV

TL;DR: 提出以运动为核心模态的视频表征学习方法TIME，通过自监督掩码自编码器从点轨迹重建缺失运动，仅用合成运动数据即可达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型存在两大局限：1）扩展模型成本高昂；2）依赖语言标注限制了概念学习范围，导致时间理解能力不足。

Method: 使用点轨迹作为视频运动表示，通过掩码自编码器随机遮蔽部分轨迹，训练模型重建缺失轨迹实现自监督学习。

Result: 在零样本测试中，TIME仅需SOTA模型10^-4规模的训练数据即可达到同等性能，且不依赖语言监督。

Conclusion: 运动作为视频核心模态可同时解决可扩展性和语言依赖问题，为构建更高效的时间感知视频模型奠定基础。
...
### [40] [Millimeter-wave Imaging for Anthropometric Body Measurement](https://arxiv.org/abs/2605.23064)
*Miriam Senne,Benjamin D. Killeen,Christoph Baur,Nassir Navab,Azade Farshad*

Main category: cs.CV

TL;DR: 提出基于毫米波雷达的优化框架，无需脱衣即可快速、隐私保护地重建3D人体形状并提取体围等临床测量指标。


<details>
  <summary>Details</summary>
Motivation: 传统体表测量需脱衣且要求固定姿势，流程慢、损害尊严，不适用于行动不便的老年人。毫米波雷达可穿透衣物且保护隐私。

Method: 加权配准管道将SMPL参数模型拟合到毫米波噪声点云，采用顶点加权策略调节Chamfer能量函数，结合地面约束与姿态先验优化SMPL参数。

Result: 通过衣物实现高保真人体形状与测量，速度快、无需脱衣或摄像机，适用于各年龄段及移动能力患者。

Conclusion: 该方法支持临床和护理机构中频繁的风险评估，提升效率和尊严。
...
### [41] [Dithering Defense: Adversarial Robustness of Vision Foundation Models via Multi-Level Floyd-Steinberg Dithering](https://arxiv.org/abs/2605.23065)
*Yury Belousov,Brian Pulfer,Vitaliy Kinakh,Slava Voloshynovskiy*

Main category: cs.CV

TL;DR: 提出多級Floyd-Steinberg抖動結合模糊作為輕量級輸入變換，在六項視覺任務上有效防禦對抗攻擊，超越擴散去噪基準。


<details>
  <summary>Details</summary>
Motivation: 視覺基礎模型廣泛作為固定骨幹使用，成為對抗攻擊的單點故障，現有防禦方法複雜或性能有限。

Method: 採用多級Floyd-Steinberg誤差擴散抖動進行輸入變換，並在特定量級後加入模糊後處理，以破壞對抗擾動同時保留語義。

Result: 在中間量化級別（尤其結合模糊）下，抖動方法在所有六項任務、兩個模型家族和三種攻擊中超越或匹配測試基準（包括擴散去噪），且對乾淨輸入的退化顯著更小。

Conclusion: 多級Floyd-Steinberg抖動是一種簡單、模型無關且有效的防禦方法，適用於多種視覺任務和攻擊場景。
...
### [42] [RoboSurg-VQA: A Multimodal Benchmark for Surgical Segmentation-Aware Visual Question Answering](https://arxiv.org/abs/2605.23068)
*Chengyi Zhang,Zi Ye,Ziyang Wang*

Main category: cs.CV

TL;DR: 提出了RoboSurg-VQA，一个面向机器人辅助和微创手术的语义分割感知视觉问答基准，通过复用公开数据集并引入临床问题与自动标注，解决了退化视图下的可靠视觉理解。


<details>
  <summary>Details</summary>
Motivation: 现有模型在手术环境中仅能提供准确分割掩码，但临床医生需要回答关于手术上下文、可见性、伪影等语言类问题，尤其在遮挡、烟雾等退化条件下。

Method: 复用公开手术分割数据集，设计固定临床问题集（涵盖上下文、解剖、伪影等），通过约束提示生成候选答案并辅以自动校验和人工审核。

Result: 报告了基准统计、初始基线及在挑战性手术条件下的常见评估难题。

Conclusion: RoboSurg-VQA为手术视觉问答提供了标准化评估框架，推动了退化视图下临床语言理解的发展。
...
### [43] [Flow Mismatching: Unsupervised Anomaly Detection via Velocity Discrepancies in Flow Matching Models](https://arxiv.org/abs/2605.23070)
*Shengzhe Chen,Mehrdad Moradi,Kamran Paynabar,Hao Yan*

Main category: cs.CV

TL;DR: 提出Flow Mismatching，一种无需异常样本的无监督异常检测方法，通过比较正常流模型预测速度与几何路径速度的差异来定位异常。


<details>
  <summary>Details</summary>
Motivation: 避免重建方法的局限性，利用流匹配的几何动力学特性，通过速度场不一致性检测异常。

Method: 训练仅含正常图像的流匹配模型，沿高斯噪声到目标图像的仿射路径比较模型预测速度与几何速度，聚合多路径、多时间步的不匹配生成热图与分数。

Result: 在MVTec-AD和VisA数据集上超越SOTA重建方法和基于流匹配的方法。

Conclusion: 异常检测可归因于得分函数间的Fisher散度项，鲁棒路径聚合提升效果。
...
### [44] [Inconsistency-aware Multimodal Schrödinger Bridge for Deepfake Localization](https://arxiv.org/abs/2605.23113)
*Jiayu Xiong,Jing Wang,Qi Zhang,Wanlong Wang,Jun Xue*

Main category: cs.CV

TL;DR: 提出IaMSB，一种基于薛定谔桥的不一致性感知音视频伪造定位方法，通过联合估计跨模态一致性和区间级定位，有效抑制对称融合中的噪声传播。


<details>
  <summary>Details</summary>
Motivation: 现有对称融合方法在处理单侧或异步伪造时，会传播跨模态噪声，导致高精度定位性能下降。

Method: 采用多模态薛定谔桥框架，融合粗桥（候选区间提议与一致性估计）和精修桥（非对称步数分配与融合），实现一致性估计、信息选择与步数调度的统一。

Result: 在多个基准上，AP@0.95提升3%~10%，尤其对单侧伪造场景的高精度定位改进显著。

Conclusion: IaMSB通过瓶颈跨模态交互与非对称步数分配，有效抑制噪声传递，提升了深度伪造定位的边界精度和鲁棒性。
...
### [45] [CoReVAD: A Contextual Reasoning Framework for Training-Free Video Anomaly Detection](https://arxiv.org/abs/2605.23116)
*Hyeongmuk Lim,Youngbum Hur*

Main category: cs.CV

TL;DR: 提出CoReVAD，一种无需训练、使用冻结视觉语言模型的视频异常检测框架，通过局部响应清理和全局时序优化生成异常分数与可解释描述。


<details>
  <summary>Details</summary>
Motivation: 现有VAD方法依赖任务特定训练，成本高且缺乏可解释性；VLM方法仍需额外训练或外部LLM，增加开销。

Method: 基于冻结VLM直接生成异常分数与描述，引入局部响应清理（LRC）模块减少生成噪声，结合softmax优化、高斯平滑和位置加权整合全局时序信息。

Result: 在UCF-Crime和XD-Violence数据集上达到与无训练方法相当的竞争性能，并提供可靠的可解释性。

Conclusion: CoReVAD实现了无需额外训练的可解释视频异常检测，兼顾性能与解释能力。
...
### [46] [Exploiting Longitudinal Context in Clinician-Verified Interactive Lesion Tracking](https://arxiv.org/abs/2605.23118)
...
TL;DR: 提出了一种临床安全的肿瘤病变追踪范式，通过医生验证注册提示并融合基线外观，在自动化与人工控制间取得平衡。在autoPET IV挑战赛获第一，并发布新基准PanTrack。


<details>
  <summary>Details</summary>
Motivation: 现有方法在自动化与可校正性间存在权衡：端到端追踪器无法纠正错误，解耦流水线虽可验证但忽略病变先验外观，影响精度。

Method: 统一框架结合早期空间提示融合与潜在时间差加权的纵向感知分割，并采用大规模合成预训练缓解数据稀缺。

Result: 纵向感知分割性能提升高达4.5 Dice点，优于全自动及验证性追踪场景下的现有方法。

Conclusion: 提出的验证性追踪范式为临床提供了自动化与控制的折中方案，具有良好的安全性。
...
### [47] [VisAnalog: A Diagnostic Suite for Visual Concept Transfer on Natural Images](https://arxiv.org/abs/2605.23141)
...
TL;DR: 提出了一个用于测试视觉概念学习模型的基准VisAnalog，通过类比推理任务评估模型在变换下保持和迁移概念属性能力。


<details>
  <summary>Details</summary>
Motivation: 现有测试无法准确评估模型在变换下保持和迁移概念级属性的能力。

Method: 构建了包含617个人工验证问题的基准，使用一到四步变换操作，通过多选问题和程序条件评估。

Result: 当前模型端到端精度远低于基准，随变换深度增加显著下降，而人类表现接近天花板；关系推理是主要瓶颈。

Conclusion: 视觉类比推理中关系推断是模型的主要挑战，现有VLM在此任务上表现有限。
...
### [48] [SLIP-RS: Structured-Attribute Language-Image Pre-Training for Remote Sensing Object Detection](https://arxiv.org/abs/2605.23144)
*Chenxu Wang,Yuxuan Li,Yunheng Li,Xiang Li,Jingyuan Xia,Qibin Hou*

Main category: cs.CV

TL;DR: SLIP-RS提出结构化属性解耦范式，利用有限物理属性空间替代无穷类别空间，通过组合属性增强和共形预测实现遥感图像精细检测。


<details>
  <summary>Details</summary>
Motivation: 现有限制于黑盒数据枚举开放类别的大规模语言-图像预训练方法不适用于遥感领域的数据稀缺特性，需要更高效的细粒度表示学习。

Method: 结构化属性对比学习（组合属性增强解耦视觉逻辑）和共形属性可靠性引擎（基于共形预测从噪声中提取高可信度监督数据，构建RS-Attribute-15M数据集）。

Result: 在细粒度检测和跨域泛化上取得领先性能，验证结构化属性作为遥感基础的有效性。

Conclusion: 结构化属性解耦范式是突破遥感应模型数据依赖瓶颈的关键路径，为细粒度开放世界感知提供新基础。
...
### [49] [LQ-rPPG: A Label-Quantized Coarse-to-Fine Learning Framework for Remote Physiological Measurement](https://arxiv.org/abs/2605.23174)
*Jun Seong Lee,Samyeul Noh,Changki Sung,Hyun Myung*

Main category: cs.CV

TL;DR: 提出LQ-rPPG，一种标签量化的粗到细学习框架，通过量化标签减少噪声和可变性，提升rPPG估计的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的rPPG方法忽略了训练标签中的噪声和可变性，导致模型过拟合，泛化性能下降。

Method: 包含标签量化模块和粗到细估计模型，首先将连续PPG信号转化为多比特量化伪标签，再通过分层监督逐步细化估计。

Result: 在多个数据集上表现优异，参数减少88%，乘加操作减少29%，吞吐量提升191%。

Conclusion: LQ-rPPG有效缓解了过拟合问题，实现了鲁棒且通用的rPPG估计。
...
### [50] [DRIVESPATIAL: A Benchmark for Spatiotemporal Intelligence in VLMs for Autonomous Driving](https://arxiv.org/abs/2605.23176)
...
TL;DR: DriveSpatial是一个面向自动驾驶空间智能的基准，包含15.6K人工验证的问题-答案对，用于评估视觉语言模型在动态驾驶场景中的跨视角和时空推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶视觉语言基准主要关注单视角、静态或单一来源的问答，无法判断视觉语言模型是否能够真正构建和推理动态驾驶场景。

Method: 基于动态多关系场景图生成15.6K问题-答案对，场景图编码物体状态、空间关系、交互、相机可见性和时间对应关系。从五个大规模自动驾驶数据集中设计20个任务，评估四个能力。

Result: 评估15个代表性视觉语言模型，最佳模型比人类低28.4分，认知场景构建是主要瓶颈；仅依赖语言提示不足，显式BEV基础能提升性能。

Conclusion: 当前视觉语言模型缺乏可靠的场景构建能力，无法实现自动驾驶所需的空间智能。
...
### [51] [Composing People Together: Iterative Pose-Image Generation for Multi-Person Interaction Scenes](https://arxiv.org/abs/2605.23178)
*Wenxuan Peng,Bharath Hariharan,Hadar Averbuch-Elor*

Main category: cs.CV

TL;DR: 提出双姿态-图像表示和迭代场景构建方法，提升多人物交互图像的语义多样性与构图准确性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型生成多人物交互场景时存在布局重复、姿态刻板、交互 grounding 差等问题。

Method: 引入双姿态-图像表示（联合预测2D姿态可视化图和RGB图）、跨模态对齐机制（绑定文本、姿态、图像表示）以及迭代场景构建方案。

Result: 显著提升多人物图像生成的 prompt 对齐度和场景多样性。

Conclusion: 本方法有效解决了多人物交互场景生成的挑战，实现了结构与外观的协同优化。
...
### [52] [Occlusion-Aware Physics-Semantic Keyframe Selection for Robust Video Editing](https://arxiv.org/abs/2605.23192)
*Lin Liu,Zhihan Xiao,Haohang Xu,Rong Cong,Zhibo Zhang,Xiaopeng Zhang,Qi Tian*

Main category: cs.CV

TL;DR: 提出了一种基于物理语义的关键帧选择框架，通过可靠的视觉锚点解决视频编辑中的遮挡、视角变化和快速运动问题，实现了高精度的时间一致性编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法在遮挡、视角变化和快速运动下易出现定位不准、闪烁和不一致，根本原因是缺乏可靠的视觉锚点。

Method: 开发了遮挡感知的物理语义关键帧选择框架，从结构完整性、循环一致性跟踪稳定性和属性可见性三个角度评估候选帧，选择最优锚点后通过双向跟踪生成稠密时空掩码，作为扩散模型的辅助监督。

Result: 在挑战性视频编辑基准测试中，该方法实现了高精度、时间一致的编辑效果，优于现有方法。

Conclusion: 通过将遮挡处理转化为可靠的锚点选择，无需人工标注即可实现鲁棒的视频编辑。
...
### [53] [IntentionNav: A Benchmark for Intent-Driven Object Navigation from Implicit Human Instruction](https://arxiv.org/abs/2605.23187)
*Lin Qian,Shijie Li,Sihao Lin,Xuan Zhang,Bangya Liu,Yanran Li,Hujun Yin*

Main category: cs.CV

TL;DR: 提出意图驱动的目标导航基准IntentionNav，包含500条隐式人类指令、176个Isaac Sim场景和64个目标类别，通过四类指令风格与四种意图模式设计，评估VLMs在间接意图下的目标推理、语言鲁棒性和终止成功率的瓶颈表现。


<details>
  <summary>Details</summary>
Motivation: 现有目标导航任务通常直接指定物体类别（如微波炉），而人类日常交流中常使用间接意图（如‘我需要加热食物’），因此需要研究智能体从隐式指令中推断目标物体、定位并验证目标的能力。

Method: 构建IntentionNav基准，包含配对指令（四种控制风格和四种意图模式）的500条意图，使用固定主动导航智能体评估三种VLM在目标推断、邻域可达性、终止成功率等维度上的表现。

Result: 模型在48.3%的回合中推断出目标，68.7%进入目标2米邻域，但仅24.9%成功终止，1米内精确定位成功率仅5.5%；事件脚本意图成功率为28.7%，物理状态和可供性意图分别降为19.2%和18.5%。
...
### [54] [Lipschitz Optimization for Formal Verification of Homographies](https://arxiv.org/abs/2605.23203)
*Jean-Guillaume Durand,Panagiotis Kouvaros,Maxime Gariel,Alessio Lomuscio*

Main category: cs.CV

TL;DR: 提出了一种对相机运动扰动的形式化验证方法，通过闭式映射和利普希茨优化为平面场景提供鲁棒性保证。


<details>
  <summary>Details</summary>
Motivation: 现有验证方法仅覆盖ℓ_p范数或仿射变换，无法处理相机运动这类实际部署中的关键扰动。

Method: 建立从相机位姿到像素值的闭式映射，利用单应性连续性和分段线性分析推导紧线性边界。

Result: 相比已有方法加速89%，边界紧致7%，并在VNN-COMP基准和跑道分类器案例中发现系统性脆弱性。

Conclusion: 该方法首次实现了射影几何变换的形式化验证，揭示了相机运动对安全关键模型的潜在风险。
...
### [55] [CaST-Bench: Benchmarking Causal Chain-Grounded Spatio-Temporal Reasoning for Video Question Answering](https://arxiv.org/abs/2605.23216)
...
TL;DR: 提出了CaST-Bench基准和因果链时空视频推理数据集，发现现有VLM在因果问题上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准缺乏细粒度、基于证据的因果评估能力。

Method: 人机协作构建含时空链条注释的高质量数据集，并设计新评估指标。

Result: 当前VLM在因果问题上表现差，主要因无法构建精确且基于证据的因果链。

Conclusion: 未来VLM需提升因果链推理能力，以增强透明度和抗虚假相关性。
...
### [56] [Beyond Normal References: Discriminative Few-Shot Anomaly Detection](https://arxiv.org/abs/2605.23231)
*Huan Wang,Jun Shen,Jun Yan,Guansong Pang*

Main category: cs.CV

TL;DR: 提出一种基于内在偏差学习的少量样本异常检测框架IDEAL，利用正常和异常样本的偏差模式来泛化检测未见过的异常。


<details>
  <summary>Details</summary>
Motivation: 现有少量样本异常检测方法仅使用正常参考，忽略异常参考的判别信息；直接拟合两种参考会导致过拟合已见异常。

Method: IDEAL包含两个组件：正常变化擦除器（抑制噪声）和内在偏差编码器（提取正交偏差向量），利用偏差分解和投影进行异常评分。

Result: 在8个真实数据集上，IDEAL对未见异常泛化有效，并持续优于现有最优方法。

Conclusion: IDEAL通过学习内在偏差模式有效利用两类参考，提升了少量样本异常检测的泛化能力。
...
### [57] [ChainFlow-VLA: Causal Flow Planning with Vision-Language Models](https://arxiv.org/abs/2605.23270)
*Xiyang Wang,Xinlin Wang,Tingguang Zhou,Gong Chen,Xingtai Gui,Zhi Xu,Xiaolei Wu,Feiyang Tan,Hangning Zhou,Mu Yang*

Main category: cs.CV

TL;DR: 提出了ChainFlow-VLA，一种统一因果生成与全局优化的端到端自动驾驶框架，通过自回归生成因果轨迹模式并结合扩散模型进行残差修正，在NAVSIM v1上达到94.85分，媲美人类水平。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶系统在时序因果推理与全局轨迹一致性之间存在根本性矛盾：自回归模型缺乏全局结构，扩散模型缺乏因果约束。

Method: 将轨迹规划建模为自回归因果模式（Chain）与基于VLM隐状态的扩散残差修正（Flow）的混合分布，在保留因果结构的同时进行全局优化。

Result: 在NAVSIM v1排行榜上获得94.85分，达到人类水平（94.8），在模糊和长尾场景中展现鲁棒规划能力。

Conclusion: 通过统一因果生成与全局优化，ChainFlow-VLA有效提升了自动驾驶轨迹规划的可靠性和性能。
...
### [58] [SimInsert: Seamless Video Object Insertion via Regional Sparse Attention Fusion](https://arxiv.org/abs/2605.23245)
...
TL;DR: 提出SimInsert，一种无需训练的视频对象插入方法，利用图像到视频扩散模型实现时空连贯和交互真实感。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖显式运动工程或资源密集重训练，灵活性和泛化能力受限。

Method: 将任务解耦为单帧编辑和语义运动描述，通过扩散模型传播编辑并加入非侵入式引导机制保持结构一致性和边界融合。

Result: PSNR提升18.8%，SSIM提升20.1%，LPIPS降低44.1%，超越现有方法。

Conclusion: SimInsert提供了一种高效、高保真的视频编辑解决方案。
...
### [59] [Good Token Hunting: A Hitchhiker's Guide to Token Selection for Visual Geometry Transformers](https://arxiv.org/abs/2605.23892)
*Shuhong Zheng,Michael Oechsle,Erik Sandström,Marie-Julie Rakotosaona,Federico Tombari,Igor Gilitschenski*

Main category: cs.CV

TL;DR: 提出两阶段令牌选择策略，将计算量减少85%以上且保持精度


<details>
  <summary>Details</summary>
Motivation: 视觉几何Transformer全局注意力层计算量随序列长度二次增长，限制可扩展性和效率

Method: 两阶段令牌选择：帧间多样性选择保留覆盖场景，帧内基于注意力熵的层感知稀疏化

Result: 处理500张图像时加速超85%，性能持平甚至提升

Conclusion: 令牌选择策略对视觉几何Transformer的未来应用至关重要
...
### [60] [StereoGenBench: A Synthetic Multi-Camera Benchmark for Stereo Generation under Controlled Baseline Regimes](https://arxiv.org/abs/2605.23237)
*Yangzhi Cui,Feng Qiao,Nathan Jacobs*

Main category: cs.CV

TL;DR: 提出StereoGenBench，一个合成基准数据集，提供参数可控的立体图像对，包括可变的基线、焦距、深度和姿态，用于评估立体生成和几何估计方法。


<details>
  <summary>Details</summary>
Motivation: 现有立体资源缺乏同时提供场景配对、多基线校准、内参、密集深度和每帧姿态的单一控制源，导致基线敏感性和目标相机一致性难以测量。

Method: 使用Unreal Engine渲染刚性六相机横向阵列场景，生成多达15组校准视图对，独立采样基线（从瞳孔间到宽基线）和焦距，发布RGB、度量深度、内参、基线和姿态数据。

Result: 构建了窄基线、宽基线评估集及全对训练集，附有数据集、评估代码、参考结果、Croissant元数据和扩展生成代码。

Conclusion: StereoGenBench填补了可控立体生成评估的空白，支持可重复的基准测试和模型开发。
...
### [61] [CARE: Class-Adaptive Expert Consensus for Reliable Learning with Long-Tailed Noisy Labels](https://arxiv.org/abs/2605.23254)
*Mengke Li,Haiquan Ling,Lihao Chen,Yang Lu,Yiqun Zhang,Hui Huang*

Main category: cs.CV

TL;DR: 提出CARE框架，利用视觉语言模型的多源监督信号，通过类自适应专家共识机制解决长尾分布与标签噪声的复合挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视标签噪声在不同类别上的非均匀影响，导致尾类纠正不足和头类过度正则化。

Method: CARE框架：融合噪声标签、VLM文本嵌入和视觉特征，基于类频率设计自适应专家共识，对尾类强制严格一致，头类允许宽松一致。

Result: 在合成和真实基准上优于现有方法，性能提升最高达3.0%。

Conclusion: 类自适应机制有效缓解长尾分布下的噪声标签问题，代码已开源。
...
### [62] [Coloring the Noise: Adversarial Sobolev Alignment for Faithful Image Super Resolution](https://arxiv.org/abs/2605.23264)
*Hongbo Wang,Huaibo Huang,Pin Wang,Jinhua Hao,Chao Zhou,Ran He*

Main category: cs.CV

TL;DR: 提出ASASR框架，通过Sobolev诱导的黎曼几何和参数化对抗方法，解决生成先验在图像超分辨率中的频谱错位问题，有效提升频谱一致性和结构保真度。


<details>
  <summary>Details</summary>
Motivation: 现有生成先验在图像超分辨率中会损害忠实重建，根本原因是各向同性目标与自然图像流形之间的频谱错位，而直接偏好优化依赖频谱平坦的高斯噪声无法区分真实高频细节与幻觉。

Method: 基于Sobolev诱导的黎曼几何，对噪声转移核进行显式着色以匹配自然频谱衰减；结合Riesz表示定理的参数化对抗，合成等价于最坏情况Sobolev梯度的负样本，驱动优化沿结构失效的切空间方向。

Result: 在频谱一致性和结构保真度上优于主流生成基线，有效减少伪影。

Conclusion: ASASR通过几何对齐和对抗优化，提供了鲁棒的图像超分辨率解决方案，缓解了生成伪影问题。
...
TL;DR: 本研究提出U-CESE，一个统一剪裁的多模态事件搜索引擎，通过DAKE关键帧提取和ReCap时序描述生成，实现了大规模视频数据的高效事件检索。


<details>
  <summary>Details</summary>
Motivation: 从大规模视频数据集中检索事件非常困难，现有方法在处理时间、空间和多模态信息时存在挑战，且缺乏一致性和高效性。

Method: 提出了U-CESE统一框架，包含统一剪裁算法；DAKE轻量级无训练关键帧提取方法，利用JPEG文件大小变化检测场景变化；ReCap循环神经网络启发式时序描述框架。

Result: 实验证明U-CESE在大规模多模态事件检索中表现稳健、一致且高效。

Conclusion: U-CESE通过统一框架和高效模块，显著提升了多模态事件检索的性能和实用性。
...
### [64] [EvalVerse: Pipeline-Aware and Expert-Calibrated Benchmarking for Professional Cinematic Video Generation](https://arxiv.org/abs/2605.23271)
...
TL;DR: 提出EvalVerse框架，系统地将电影制作专业知识数字化，通过专家校准的微调策略使视觉语言模型能够进行链式推理评估，从而弥合自动评估与人类审美之间的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成评估过度关注‘是否正确’（指令遵循），忽视‘是否优质’（电影质感、表演、美学），且自动指标缺乏领域专业性，无法提供可信信号。

Method: 1. 构建涵盖前、中、后期专业电影制作流程的评估分类体系；2. 收集大规模人类专家标注数据集；3. 使用专家校准的微调策略将知识注入视觉语言模型，使其能进行显式链式推理。

Result: EvalVerse不仅兼容基础‘正确性’指标，还扩展了‘优质性’标准，覆盖多镜头序列和音视频整合等复杂任务，提供颗粒化诊断信号。

Conclusion: EvalVerse超越了静态排行榜，为未来奖励模型和评估智能体建立了基础设施，将视频生成评估从工程任务转变为数字化主观电影专业知识的核心科学问题。
...
### [65] [LangFlash: Feed-forward 3D Language Gaussian Splatting from Sparse Unposed Images](https://arxiv.org/abs/2605.23287)
*Yilong Liu,Wanhua Li,Chen Zhu-Tian,Hanspeter Pfister*

Main category: cs.CV

TL;DR: LangFlash是一个前馈框架，用稀疏无位姿多视图图像实现3D语言高斯溅射，直接预测几何和语义，支持低延迟重建和语义理解。


<details>
  <summary>Details</summary>
Motivation: 现有优化方法需要多视图位姿和长处理时间，缺乏高效可泛化的语言语义3D重建。

Method: 提出前馈网络直接预测高斯原语和语言对齐语义特征，结合全局语义字典和局部权重稀疏编码，并用增强语义的RealEstate10k数据集训练。

Result: 在新视图合成和语义一致性上优于先前方法。

Conclusion: 确立了无位姿、语言引导的3D场景重建新范式，推动可泛化3D视觉和多模态理解。
...
### [66] [DepthAgent: Towards Better Universal Depth Estimation via Sample-wise Expert Selection](https://arxiv.org/abs/2605.23281)
*Jie Zhu,Girish Chandar Ganesan,Xiaoming Liu*

Main category: cs.CV

TL;DR: 提出一个视觉语言代理DepthAgent，通过多奖励强化学习微调，自适应选择和融合现有深度估计模型，在透视、鱼眼、全景等多种相机设置下实现鲁棒的度量深度估计。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在面对不同相机设置（如透视、鱼眼、全景图像）时表现不稳定，而不同的深度模型在不同输入领域各有优势，存在互补性。因此，需要一种能自适应选择或融合多个专家模型的方法。

Method: 提出DepthAgent，将现有深度模型视为冻结工具。它学习分析场景和相机线索，通过多轮工具调用来调用合适的专家模型，并选择或融合其预测。使用多奖励强化学习微调方案优化离散决策，包括工具执行、相机/场景分析、专家选择质量和推理效率。

Result: 在透视、鱼眼、全景基准测试中，DepthAgent一致优于单个专家、固定模型融合和不同选择策略，尤其在困难样本上表现出显著提升。

Conclusion: 专家选择和融合对于鲁棒的通用单目深度估计至关重要，提出的自适应方法能有效利用多模型互补性，提升跨相机设置的性能。
...
### [67] [CHASD: Language Increment-Calibrated Contrastive Decoding against Hallucination in LVLMs](https://arxiv.org/abs/2605.23344)
*Xiaoyi Huang,Kejia Zhang,Zhiming Luo*

Main category: cs.CV

TL;DR: 提出了一种基于不确定性的按需对比解码方法，通过注意力引导的局部扰动和置信度门控，在减少无用计算的同时缓解物体幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有训练自由对比解码方法存在全局扰动或额外负分支带来的计算开销问题，且幻觉风险具有瞬态和令牌特异性。

Method: 提出CHASD框架，利用不确定性驱动的置信度门控仅在低置信度步骤激活对比分支，并通过注意力引导对当前显著视觉令牌进行局部扰动来构建负分支。

Result: 在POPE、AMBER、MME等基准上，CHASD在幻觉指标上优于强训练自由基线，且推理效率具有竞争力。

Conclusion: 按需校准的对比解码能有效降低幻觉，同时减少不必要的负分支前向传播。
...
### [68] [Spatio-Temporal Similarity Volume Aggregation for Open-Vocabulary Action Recognition](https://arxiv.org/abs/2605.23288)
*Yerim So,Jiyeong Kim,Jiwon Yoon,Dongbo Min*

Main category: cs.CV

TL;DR: 提出了相似体积聚合（SimVA）框架，通过构建密集的4D时空相似体积来改善零样本、少样本和基础到新颖的动作识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇动作识别方法将视觉特征聚合为全局表示，丢失了局部补丁信息和细粒度时空线索，导致性能受限。

Method: SimVA首先构建基于补丁级别的视觉-文本相似度的4D时空相似体积，通过类别采样保证可扩展性，接着进行空间聚合以增强帧内一致性，并引入运动感知调制注入帧间变化线索，最后采用Mamba时序聚合建模类别条件相似模式的演变。

Result: 在零样本、少样本和基础到新颖基准测试中取得了竞争性的性能，有效将CLIP迁移到视频动作识别。

Conclusion: 保持密集视觉-文本对应能提升开放词汇动作识别的准确性，SimVA为后续研究提供了新思路。
...
### [69] [General Hazard Detection](https://arxiv.org/abs/2605.23304)
*Stephanie Ng,CP Lim,SueJen Looi,Hendrik Zurlinden,David Nguyen,Lei Wei,Saeid Nahavandi,Hailing Zhou*

Main category: cs.CV

TL;DR: 提出了首个面向规则合规评估的通用危险数据集CompliVision，并结合主动学习框架与LLaVA视觉推理，解决抽象危险检测中数据稀疏、定义动态变化及泛化不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有危险检测依赖预定义分类与密集标注示例，难以应对抽象危险概念导致的噪声数据、动态定义及新场景泛化能力差。

Method: 基于ISO标准与权威法规构建语言规来表达危险概念，创建包含3006张多场景图像的CompliVision数据集，并采用LLaVA视觉推理与人机交互反馈的主动学习框架。

Result: CompliVision数据集覆盖交通、建筑和仓库三类场景，每个图像均附有合规性标注和自然语言解释；主动学习框架提升了视觉语言模型的危险合规评估精度。

Conclusion: 抽象危险检测需将概念与图像解耦，通过语言规表达可增强泛化能力；人机结合的主动学习能有效提升模型的细粒度上下文理解。
...
### [70] [Enhancing Blood Cells Classification using Hybrid Quantum Neural Networks](https://arxiv.org/abs/2605.23324)
*Guilherme Cruz,Nouhaila Innan,Alberto Marchisio,Gabriel Falcao,Muhammad Shafique*

Main category: cs.CV

TL;DR: 该论文研究了混合量子-经典神经网络在血细胞分类中的应用，通过模块化架构（ResNet-50 + 量子电路）在公开数据集上提升了分类性能，最高F1分数提升3.7%，并在真实量子硬件上验证了鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在处理血细胞图像中的细微变化和有限数据时面临挑战，而量子计算可能增强特征表示能力。

Method: 提出一种模块化架构，结合预训练的ResNet-50骨干网络、低维潜在瓶颈层和变分量子电路，并与纯经典变换机制进行对比。

Result: 在两个血细胞数据集上，混合模型在宏F1分数上最高提升3.7%（8类场景下F1从98.54%升至98.69%），且在IBM量子硬件上仅出现轻微性能下降。

Conclusion: 量子特征变换可增强判别性表示，尤其在复杂分类场景中具有潜力，展示了混合模型在医学图像任务中的实际应用价值。
...
### [71] [Online Hand Gesture Recognition Using 3D Convolutional Neural Networks](https://arxiv.org/abs/2605.23409)
*Yinghao Qin,Tijana Timotijevic*

Main category: cs.CV

TL;DR: 提出了一种在线手势识别系统，能在实时视频流中检测和分类动态手势，使用滑动窗口提高鲁棒性，在Jester数据集上检测器准确率98%+，分类器准确率90%+，自建数据集上37.5%莱文斯坦准确率，响应时间三秒内。


<details>
  <summary>Details</summary>
Motivation: 实时人机交互中动态手势检测和分类面临实时性要求与人手势差异大的挑战。

Method: 使用滑动窗口法细化多个窗口的结果，基于Jester数据集训练检测器和分类器模型。

Result: 检测器准确率98%+，分类器准确率90%+，自建数据集上37.5%莱文斯坦准确率，最佳组响应时间三秒内。

Conclusion: 提出的系统能有效实现实时手势识别，代码已公开。
...
### [72] [GFSR: Geometric Fidelity and Spatial Refinement for Reliable Lane Detection](https://arxiv.org/abs/2605.23327)
*Tiancheng Wang,Zhaolu Ding,Richeng Xu,Tianhui Zheng,Hui Liu,Hanyu Xuan,Zhiliang Wu,Guanghui Yue*

Main category: cs.CV

TL;DR: 本文提出GFSR框架，通过LaneIoU引导的置信度校准和自适应门控位置细化，改善了复杂场景下的车道检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂场景下退化，因为分类置信度与几何质量相关性弱，且回归模块削弱了采样点间的关联，导致高曲率车道拟合不足。

Method: GFSR框架包括LaneIoU引导的置信度校准（LCC）和自适应门控位置细化（AGLR）。LCC用LaneIoU作为软监督估计几何保真度，并与分类置信度融合为协同可靠性指标（CRI），指导阈值过滤和NMS。AGLR与回归头协作，预测横向偏移并用门控机制调节修正幅度，增强点间联系。

Result: 在CULane上F1@50和F1@75分别达到81.46%和65.01%，在CurveLanes上F1@50达到87.35%，实现了最先进性能。

Conclusion: GFSR有效保留了高分类置信度和良好几何质量的车道候选，并提升了模型对复杂车道场景的适应性和鲁棒性。
...
### [73] [Decoupling Spatio-Temporal Adapter for Fine-Grained Badminton Action Localization](https://arxiv.org/abs/2605.23355)
*Tianyu Wang,Junjie Wu,Jingquan Gao,Shishuo Li*

Main category: cs.CV

TL;DR: 论文提出了用于羽毛球比赛视频的细粒度时间动作定位方法，包括新数据集Fine-Badminton和解耦时空适配器DSTA。


<details>
  <summary>Details</summary>
Motivation: 现有的时间动作定位研究多针对通用视频，而对羽毛球等精细运动场景的关注不足，因其具有复杂细微的时空动态。

Method: 提出解耦时空适配器DSTA，将运动表示分解为三个并行分支，分别捕捉时间动态、垂直和水平空间变化。

Result: 在Fine-Badminton和ShuttleSet数据集上达到最先进性能，且计算和参数开销仅略有增加。

Conclusion: 该方法能有效且高效地实现细粒度时间动作定位。
...
### [74] [One-Forcing: Towards Stable One-Step Autoregressive Video Generation](https://arxiv.org/abs/2605.23458)
*Jiaqi Feng,Justin Cui,Yuanhao Ban,Cho-Jui Hsieh*

Main category: cs.CV

TL;DR: 提出One-Forcing方法，通过结合DMD目标和GAN损失，实现一步式高质量视频生成，性能达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有少步自回归视频生成方法在一步设置下质量严重下降，且一致性蒸馏方法产生弱动态，DMD方法产生模糊帧。

Method: 在DMD目标基础上增加辅助GAN损失，用于一步式高质量视频生成。

Result: 在VBench上总分83.76，达到一步式因果视频生成SOTA，且与多步方法竞争力相当；训练成本仅为分块模型的三分之一。

Conclusion: One-Forcing方法能稳定实现帧级一步自回归生成，且训练效率高，之前方法未能成功。
...
### [75] [SCOPE: Simulating Cross-game Operations in Playable Environments for FPS World Models](https://arxiv.org/abs/2605.23345)
...
TL;DR: 提出SCOPE方法，通过条件模块和特征重塑实现FPS游戏第一人称视角下的局部动作效果分离，并构建多游戏数据集CrossFPS，实现跨游戏零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型将动作全局注入且单游戏训练，无法处理FPS中高频重叠控制信号（如射击与移动同时进行），导致局部动作效果破坏未受影响的区域。

Method: 在预训练视频扩散模型的每个Transformer块中插入条件模块，将特征重塑为逐像素时序序列，使各位置根据局部视觉内容计算动作响应，无需分割标签即可分离作用域（如武器附近）与稳定背景。

Result: 在包含7款游戏69K片段的CrossFPS数据集上，模型展现出强动作响应性、精确的作用域分离效果，以及跨游戏零样本泛化能力。
...
### [76] [PhenoYieldNet: Learning Crop-Aware Phenological Responses for Multi-Crop Yield Prediction](https://arxiv.org/abs/2605.23478)
*Yu Luo,Xiaogang Zhu,Shan Zeng,Wei Xiang,Thomas Francis Bishop,Zhiyong Wang,Kun Hu*

Main category: cs.CV

TL;DR: 提出PhenoYieldNet框架，通过作物物候银行和注意力模块，结合预训练模型和对比学习，实现多作物产量预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法多针对单作物预测，难以泛化至多作物，且未考虑复杂天气动态调控下的作物物候响应。

Method: 开发作物感知时序解码器（含CPB和CPA），预训练编码器结合时序对比适应策略。

Result: 在多作物数据集上显著优于现有方法，展现出跨区域和跨作物的强泛化能力。

Conclusion: 该方法有效学得作物特异性物候模式，提升多作物产量预测的准确性和泛化性。
...
### [77] [VDE: Training-Free Accelerating Rectified Flow Model via Velocity Decomposition and Estimation](https://arxiv.org/abs/2605.23381)
*Junwen Tan,Jinglin Liang,Hongyuan Chen,Shuangping Huang*

Main category: cs.CV

TL;DR: 本文提出了一种无需训练的加速方法VDE，通过将模型速度分解为与输入平行和正交的分量，并利用其时间可预测性和方向稳定性进行输入自适应估计，显著提升了整流流模型的推理速度。


<details>
  <summary>Details</summary>
Motivation: 整流流模型在图像、视频和3D生成中表现出色，但推理速度慢；现有加速方法因静态缓存与动态输入不匹配导致输出保真度下降。

Method: VDE方法通过速度分解与估计，将模型速度分解为与输入平行和正交的分量，利用时间可预测性和方向稳定性进行精确估计，并通过周期性全前向传递防止误差累积。

Result: VDE在图像和视频生成任务中实现显著加速，例如Flux加速3.22倍，Qwen-Image上LPIPS达0.069，相比最佳基线减少52.2%。

Conclusion: VDE是一种有效的训练自由加速方法，能在保持视觉质量的同时大幅提升推理速度，优于现有缓存复用方法。
...
### [78] [Joint Target-Less Intrinsic and Extrinsic Camera-LiDAR Calibration using Deep Point Correspondences](https://arxiv.org/abs/2605.23397)
*Simon Bultmann,Daniele Cattaneo,Abhinav Valada*

Main category: cs.CV

TL;DR: 提出首个无需标定物的pipeline，联合估计相机内参（含畸变）与相机-激光雷达外参，基于深度点对应。


<details>
  <summary>Details</summary>
Motivation: 现有无目标外参标定方法假设图像已校正且内参已知，限制了实际应用。

Method: 扩展深度对应标定：（i）通过运动恢复结构自动初始化内参；（ii）将匹配泛化到含畸变的原始图像；（iii）紧密耦合对应估计与内参外参联合非线性优化。

Result: 在KITTI数据集上验证，联合标定提升外参精度，同时恢复准确内参。

Conclusion: 首次实现无需标定物的内参外参联合标定，适用于未知内参的原始图像。
...
### [79] [Multimodal Distribution Matching for Vision-Language Dataset Distillation](https://arxiv.org/abs/2605.23482)
*Jongoh Jeong,Hoyong Kwon,Minseok Kim,Kuk-Jin Yoon*

Main category: cs.CV

TL;DR: 提出多模态分布匹配(MDM)框架，通过几何感知的数据、模型和损失级组件实现高效且泛化的多模态数据集蒸馏。


<details>
  <summary>Details</summary>
Motivation: 现有方法计算成本高且忽视跨模态相关性，无法在有限预算下保留表示质量和跨模态对齐。

Method: 数据级：从联合嵌入空间聚类初始化合成图像-文本对；模型级：基于角度偏差插值独立微调模型形成混合教师；损失级：利用几何感知匹配目标在单位超球面上对齐联合分布。

Result: 在图像-文本检索基准上，MDM生成紧凑合成集，保留多模态语义，显著降低蒸馏成本，并鲁棒跨架构。

Conclusion: MDM是首个兼顾效率与泛化性的多模态蒸馏方案，验证了几何信息在跨模态蒸馏中的关键作用。
...
### [80] [RS2AD-LiDAR: End-to-End Autonomous Driving LiDAR Data Generation from Roadside Sensor Observations](https://arxiv.org/abs/2605.23406)
*Runyi Huang,Ni Ding,Ruidan Xing,Yuheng Shi,Lei He,Keqiang Li*

Main category: cs.CV

TL;DR: 提出了首个从路边LiDAR数据重建车载LiDAR数据的框架RS2AD-LiDAR，通过虚拟LiDAR建模和点云重采样技术生成高保真数据，并构建了专用数据集R2V-LiDAR进行验证。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶方案依赖单车数据采集，存在成本高、场景稀缺和数据孤岛等问题。

Method: 将路边LiDAR点云转换到车载坐标系，通过虚拟LiDAR建模和点云重采样技术合成高保真车载数据。

Result: 生成数据与真实数据在语义上高度相似；将生成数据加入真实数据训练后，BEV和3D检测精度均得到提升。

Conclusion: 首次实现从路边传感器重建车载LiDAR数据，验证了其在提升自动驾驶感知性能方面的有效性。
...
### [81] [EM-Vid: Training-Free Entity-Centric Memory for Efficient and Consistent Multi-Shot Video Generation](https://arxiv.org/abs/2605.23610)
*Jente Vandersanden,Matheus Gadelha,Chun-Hao P. Huang,Hyeonho Jeong,Yulia Gryaditskaya*

Main category: cs.CV

TL;DR: 提出了基于实体索引的潜在补丁记忆库，结合稀疏令牌条件化、预算更新和噪声注入机制，实现多镜头视频生成中实体一致性与提示保真度的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有自回归方法使用全帧存储导致无关信息泄露和高计算成本，需要更高效的实体级记忆管理。

Method: 实体索引的潜在补丁记忆库；稀疏令牌条件化（限制自注意力到实体相关令牌）；预算化记忆更新策略；噪声注入机制控制外观细节。

Result: 提高了提示遵循度和效率，同时保持了主体一致性。

Conclusion: 所提方法在减少计算开销的同时有效维持了跨镜头实体一致性。
...
### [82] [FAST-ME: Foundation-aware Adaptive Stopping for Motion Estimation for Efficient IoT Video Analysis](https://arxiv.org/abs/2605.23428)
*Kakia Panagidi,Stathes Hadjieftymiadis*

Main category: cs.CV

TL;DR: 提出了一种基于最优停止理论(OST)的块运动估计算法，并结合基础模型(FMs)进行语义感知的运动估计，显著降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 块运动估计在资源受限环境中（如物联网相机网络、自主平台）仍是计算瓶颈，现有快速搜索技术效率不足。

Method: 利用时空差异评估OST决策，融合ViT和SAM的语义注意力分数与SAD等传统失真度量，形成混合停止准则。

Result: 在基准和多模态视频数据集上，该方法大幅减少计算量，精度损失极小，语义覆盖更优。

Conclusion: 低层运动分析与高层语义推理的结合为新一代智能系统的高效多模态视频理解提供了有前景的方向。
...
### [83] [PathNavigate: A Training-Free Pathology Agent with Surprise-Guided Scan and Shared Slide Memory for Whole-Slide Image VQA](https://arxiv.org/abs/2605.23559)
...
TL;DR: PathNavigate 是一个无训练的病理学代理，采用扫描-搜索-读取流程，通过低倍扫描创建异常区域池，再基于问题筛选高倍目标，最终利用冻结模型回答，提高了答案准确性和证据选择效率。


<details>
  <summary>Details</summary>
Motivation: 现有全切片图像视觉问答方法要么需要任务特定监督和再训练，要么采用问题优先设计导致遗漏关键形态，缺乏高效且可泛化的方案。

Method: PathNavigate 使用无训练方法，首先低倍扫描切片以共享在线记忆模块创建异常区域池，然后在此池内应用问题条件化的 PLIP 相关性选择高倍搜索目标，最后提取局部证据并用冻结的感知-判断堆栈回答问题。

Result: 在 WSI-VQA 和 SlideBench-BCNB 数据集上，PathNavigate 提高了答案准确性，生成了更可解释的证据选择轨迹，并提升了效率。

Conclusion: 扫描-搜索-读取流程通过解耦初始扫描与问题匹配，有效提升了训练免费病理学代理在 WSI-VQA 中的表现和实用性。
...
### [84] [CVSearch: Empowering Multimodal LLMs with Cognitive Visual Search for High-Resolution Image Perception](https://arxiv.org/abs/2605.23655)
*Liupeng Li,Haoqian Kang,Zhenyu Lu,Jinpeng Wang,Bin Chen,Ke Chen,Yaowei Wang*

Main category: cs.CV

TL;DR: 提出了一种名为CVSearch的训练无关自适应框架，通过评估-搜索机制动态调度策略，在保证高效性的同时实现高分辨率图像中的精准感知。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索方法在高覆盖率与高效率之间存在权衡，专家辅助搜索存在盲点，扫描搜索存在计算冗余和语义碎片问题。

Method: 采用评估-搜索流程：先尝试专家辅助搜索，失败后触发语义感知扫描（基于语义引导的自适应分块）；再结合动态自底向上搜索策略（利用视觉复杂度先验）。

Result: 在多个高分辨率基准测试中，CVSearch达到了最先进的准确性，同时显著提升了搜索效率。

Conclusion: CVSearch通过自适应策略有效解决了高分辨率图像感知中的覆盖率与效率矛盾。
...
### [85] [DFSAttn: Dynamic Fine-grained Sparse Attention for Efficient Video Generation](https://arxiv.org/abs/2605.23445)
*Jie Hu,Zixiang Gao,Yutong He,Kun Yuan*

Main category: cs.CV

TL;DR: 本文提出了一种无需训练的稀疏注意力框架DFSAttn，用于高效处理扩散变换器中的动态细粒度稀疏性，在保持生成质量的同时实现高达2.1倍的端到端加速。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在高质量视频生成中取得了显著成功，但其3D全注意力的二次复杂度导致计算成本过高，而现有块稀疏注意力方法在处理动态细粒度稀疏性时质量严重下降，尤其是在高稀疏率下。因此，需要一种既能动态捕捉细粒度稀疏性又具有高效执行能力的方案。

Method: 提出DFSAttn框架，基于注意力召回的理论下界分析，包含三个核心设计：基于希尔伯特曲线的令牌重排序以实现细粒度稀疏和高效GPU执行；层次化块评分以准确估计块重要性；稀疏掩码缓存与自适应比率以平衡精度和效率。

Result: 实验表明，DFSAttn在高稀疏率下持续优于先前方法，实现了高达2.1倍端到端加速，并保持了较高的生成质量。

Conclusion: 通过理论指导的设计，DFSAttn能够有效应对扩散变换器中注意力的动态和细粒度稀疏性，在显著提升计算效率的同时不牺牲生成质量。
...
### [86] [DualMem: Bypassing the Objectness Bottleneck for Calibrated Unknown-Stream Filtering in Open-World Object Detection](https://arxiv.org/abs/2605.23634)
*Yingjun Xiao,Xi Chen,Gang Fang,Siyuan Chen*

Main category: cs.CV

TL;DR: 提出 DualMem 方法，通过校准的后验过滤机制解决开放世界目标检测中未知预测流被背景误报污染的问题，在保持已知类性能的同时大幅减少背景型假阳性未知提案。


<details>
  <summary>Details</summary>
Motivation: 现有开放世界检测器的未知预测流严重被背景误报污染，例如未来任务正类未知样本仅占不到10%，而背景假阳性占据46-71%，且该问题源于物体性头部的信息瓶颈而非信息缺失。

Method: 提出 DualMem，利用少量未来任务对象的注释校准集，在冻结的 SigLIP 特征空间中进行非参数似然比检验，通过 k 近邻正类记忆保护未来任务对象、负类记忆抑制背景型提案，并由 Neyman-Pearson 校准选择决策阈值。

Result: 在 M-OWODB Task 1 上，DualMem 将背景型假阳性未知提案每图减少 44.9%-66.3%（平均 56.6%），在 PROB Task 1 上效果是 K-means 基线的两倍以上，且已知类 mAP 不变。

Conclusion: DualMem 通过轻量后处理方式有效净化未知预测流，为开放世界检测提供可控的未知与背景误报权衡，且不牺牲已知类性能。
...
### [87] [Rethinking Transfer Learning for Industrial Inspection: DINOv3 vs. ImageNet Pretraining Across RGB and X-ray Tasks](https://arxiv.org/abs/2605.23472)
*Mehdi Gharbage,Céline Teulière,Pierre Bouges,Thierry Chateau*

Main category: cs.CV

TL;DR: 本研究比较了ConvNeXt骨干网络在不同预训练方法（监督ImageNet和DINOv3）下，对工业视觉检测任务（如语义分割、实例分割和目标检测）的有效性，发现DINOv3在完全微调RGB任务上表现更好，但在X射线模态下监督预训练仍更优。


<details>
  <summary>Details</summary>
Motivation: 工业视觉检测数据与网络数据差异大，需要细粒度密集预测，现有基于监督ImageNet初始化的迁移学习范式是否可被现代自监督预训练改进尚不明确。

Method: 使用ConvNeXt骨干网络，比较监督ImageNet分类和DINOv3蒸馏两种预训练方法，在RGB表面缺陷检测和X射线缺陷检测四个数据集评估语义分割、实例分割和目标检测，并探讨冻结和完全微调两种适应模式。

Result: DINOv3在冻结迁移中无优势，但在完全微调RGB任务中收敛更快、性能更好；X射线模态下监督ImageNet预训练在两种模式下均更有效。
...
### [88] [Efficient One-Step Diffusion Restoration Model with Compact Token Compression and Linear Attention](https://arxiv.org/abs/2605.23451)
*Bingtian Qiao,Yue Shi,Yingjie Zhou,Yong Guo,Guangtao Zhai,Jiezhang Cao*

Main category: cs.CV

TL;DR: 提出SANA-SR框架，通过32倍压缩自编码器和线性注意力DiT实现高效、清晰的真实图像超分辨率，在移动设备上推理仅需0.019秒。


<details>
  <summary>Details</summary>
Motivation: 现有Real-ISR方法在高分辨率重建时存在令牌冗余和二次成本全局建模，导致计算、内存和延迟随分辨率增长而急剧增加，限制了实际部署。

Method: 采用深度压缩自编码器（32倍压缩比）将图像压缩至紧凑潜在表示，保留关键结构；结合线性注意力DiT和LoRA微调进行线性复杂度令牌混合。

Result: 在所有基准数据集上取得与现有方法相当或更优的量化性能，生成更清晰真实的纹理；修剪后模型在0.019秒内完成推理，消耗407.95 G MACs和344 M参数。

Conclusion: SANA-SR通过紧凑潜在表示和线性复杂度建模有效解决了高分辨率Real-ISR的效率瓶颈，具备移动端落地潜力。
...
### [89] [B-GRTO: Bootstrapped Group Relative Tool Optimization for Referring Segmentation](https://arxiv.org/abs/2605.23500)
*Mario Markov,Stefan Maria Ailuro,Mohammad Mahdi,Luc Van Gool,Danda Pani Paudel*

Main category: cs.CV

TL;DR: 提出了GRTO框架，将强化学习与可微工具优化结合，用于推理密集型分割任务；B-GRTO进一步通过预训练提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法中强化学习与分割解码器优化分离，缺乏统一框架。

Method: GRTO复用策略梯度rollout优化辅助工具目标，B-GRTO通过低成本预训练加速收敛。

Result: 在三个引用分割任务上，B-GRTO显著优于标准GRPO，匹配或超越领域最优方法。

Conclusion: 统一强化学习与可微辅助目标能有效提升推理密集型分割性能。
...
### [90] [Weierstrass Positional Encoding for Vision Transformers](https://arxiv.org/abs/2605.23719)
*Zhihang Xin,Rui Wang,Xitong Hu,Xiaojun Wu*

Main category: cs.CV

TL;DR: 本文提出Weierstrass椭圆位置编码（WePE），一种基于复域二维坐标的数学位置编码方法，能保留图像空间结构并提升ViT性能。


<details>
  <summary>Details</summary>
Motivation: 现有可学习一维位置编码忽略图像二维空间结构，且缺乏几何约束，无法保持空间距离与索引距离的单调关系，限制ViT利用空间邻近先验。

Method: WePE将归一化2D块坐标映射到复平面，利用Weierstrass椭圆函数及其导数构建紧凑四维位置特征，利用双周期性、晶格结构和代数加法公式表示位置关系。

Result: 实验表明WePE在多种设置下带来一致性能提升，且通过预计算查找表不增加计算或内存开销。

Conclusion: WePE是一种即插即用、分辨率无关的有效位置编码方法，能增强ViT的空间感知能力。
...
### [91] [MDS-DETR: DETR with Masked Duplicate Suppressor](https://arxiv.org/abs/2605.23507)
*Chanho Lee,Seunghee Koh,Yunho Jeon,Junmo Kim*

Main category: cs.CV

TL;DR: MDS-DETR通过引入掩码重复抑制器（MDS）在单一解码器中同时利用一对一和一对多监督，解决了DETR收敛慢和召回率低的问题，无需额外查询或辅助解码器。


<details>
  <summary>Details</summary>
Motivation: 现有DETR一对一匹配策略导致收敛慢和召回率低，而使用一对多匹配作为辅助目标会增加训练成本且推理时丢弃辅助解码器。

Method: 提出掩码重复抑制器（MDS），通过基于置信度的因果掩码向自注意力注入不对称性，滤除一对多监督层产生的重复预测。

Result: 在MS COCO上，使用ResNet-50骨干和12轮训练，MDS-DETR相比Deformable-DETR提升+2.8 mAP，训练时间仅增5%；优于MR.DETR 0.3 mAP且训练快20%。

Conclusion: MDS-DETR在全端到端框架中实现可解释、无重复的预测，优于现有方法，无需额外查询或辅助解码器。
...
### [92] [PhotoFlow: Agentic 3D Virtual Photography Missions](https://arxiv.org/abs/2605.23771)
*Jiarui Guo,Haojia Wei,Yiming Zhang,Yifei Liu,Yuning Gong,Hongjie Zhang,Xue Yang,Zhihang Zhong*

Main category: cs.CV

TL;DR: 提出了 PhotoFlow 智能体，结合 Director-Reviewer-Reflector 架构实现语言引导的虚拟摄影，并创建 VPhotoBench 基准，在复杂 3D 场景中取得最优效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时评估复杂 3D 空间理解和抽象审美判断能力，亟需一个可执行的语言条件虚拟摄影智能体。

Method: 采用三模块闭环搜索：Director 构建摄影蓝图并生成候选相机，Reviewer 通过规则检查与视觉批评筛选，Reflector 将失败经验转化为记忆、死区抑制与高探索重定位。

Result: 在 6 轮渲染预算下，PhotoFlow 在外观质量对齐度和成功率上超越单步预测、单链反思、锚点库选择和随机搜索等方法。

Conclusion: 以 LLM 为核心的空间智能体能在挑战 3D 推理与美学选择的场景中生成高质量照片，首次将语言条件虚拟摄影执行化为可操作的智能体任务。
...
### [93] [VINS-120K: Ultra High-Resolution Image Editing with A Large-Scale Dataset](https://arxiv.org/abs/2605.23518)
*Zhizhou Chen,Shanyan Guan,Zhanxin Gao,En Ci,Yanhao Ge,Wei Li,Zhenyu Zhang,Jian Yang,Ying Tai*

Main category: cs.CV

TL;DR: 提出了首个用于指令式超高分辨率图像编辑的大规模数据集VINS-120K（4K+分辨率），并开发了高频后适应策略以增强纹理细节生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理超高分辨率图像的高频纹理细节，且缺乏高质量数据集。

Method: 构建VINS-120K数据集（120K三元组），采用多阶段筛选；提出高频感知后适应策略，适配预训练模型到UHR场景；建立VINS-4KEval基准。

Result: 在UHR图像编辑中显著提升了细节合成与纹理真实感。

Conclusion: 该工作通过高质量数据和适应策略有效解决了UHR图像编辑的挑战。
...
### [94] [Not Too Generative, Not Too Discriminative: The Human Alignment Sweet Spot](https://arxiv.org/abs/2605.23819)
*Jorge Chang Ortega,Bastien Le Lan,Thomas Serre,Victor Boutin*

Main category: cs.CV

TL;DR: 该论文通过联合基于能量的模型（JEM）将生成式与判别式学习目标统一在固定架构中，发现人脑视觉对齐的最佳点位于两者之间的连续谱中点，而非极端端点。


<details>
  <summary>Details</summary>
Motivation: 现有研究常混淆学习目标与架构、规模、训练数据等因素，未能独立评估学习目标本身对人脑视觉对齐的影响。

Method: 使用联合基于能量的模型（JEM）在固定架构下连续插值判别式与生成式训练，通过调节单一混合系数隔离学习目标变量。

Result: 在六项人类对齐基准上，混合JEM在中间点始终优于纯判别或纯生成模型，同时保留了判别学习的类别结构与生成学习的输入敏感性。
...
### [95] [PGT: Procedurally Generated Tasks for improving visual grounding in MLLMs](https://arxiv.org/abs/2605.23883)
*Rim Assouel,Amir Bar,Michal Drozdzal,Adriana Romero-Soriano*

Main category: cs.CV

TL;DR: 提出程序化生成任务（PGT）框架，通过叠加几何基元增强多模态大模型的细粒度视觉理解能力，在多项基准测试中取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在细粒度理解任务中表现不佳，现有方法难以区分视觉感知与语义先验的失败原因。

Method: 在图像上叠加无歧义的几何基元，生成额外密集监督信号，将视觉定位能力与语义先验解耦。

Result: 在LLaVA-v1.5-Instruct上使用PGT数据微调，What'sUp基准提升+20%，CV-Bench-2D提升+13.3%；在SOTA模型上分别提升+5.5%和+8.3%。

Conclusion: 细粒度感知瓶颈主要源于监督信号不足，而非架构或分辨率限制，PGT有效解耦并修复了这一问题。
...
### [96] [ComPose: When to Trust Hands for Object Pose Tracking](https://arxiv.org/abs/2605.23523)
*Jisu Shin,Junoh Lee,JunGyu Lee,Inhwan Bae,Dohyeon Lee,Hokyun Im,Youngwoon Lee,Hae-Gon Jeon*

Main category: cs.CV

TL;DR: 提出ComPose框架，利用手部运动作为补充线索，从RGB视频中实现6DoF物体跟踪，在手部严重遮挡下仍保持鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有物体姿态跟踪方法依赖深度数据或3D模板，且在手部遮挡下脆弱，缺乏对手部运动的有效利用。

Method: 结合基础模型中的物体和手部线索，自适应选择手部关节，融合多源运动估计，并通过几何证据和学习校正优化运动，同时强制时间一致性。

Result: 在手部严重遮挡和几何模糊下保持高精度、高效和鲁棒性。

Conclusion: ComPose生成的轨迹可有效迁移至机器人操作，实现从在线视频重建人类动作。
...
### [97] [Generator-Refiner-Examiner: A Tri-Module Data Augmentation Framework for 3D Human Avatar Learning from Monocular Videos](https://arxiv.org/abs/2605.23555)
*Gangjian Zhang,Jian Shu,Sicheng Yu,Wenhao Shen,Yu Feng,Hao Wang*

Main category: cs.CV

TL;DR: 提出TrioMan框架，通过生成、精炼和选择三个模块增强单目视频3D虚拟人重建，在有限训练帧下提升细节捕获能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预训练先验，在训练帧有限时难以捕获精细细节。

Method: 三模块框架：生成器施加高斯扰动创造新样本，精炼器通过纹理几何引导扩散提升质量，选择器用注意力机制筛选一致性样本。

Result: 在X-Humans和NeuMan基准上超越当前最优方法。

Conclusion: TrioMan有效缓解数据稀缺问题，增强虚拟人学习效果。
...
### [98] [ETCHR: Editing To Clarify and Harness Reasoning](https://arxiv.org/abs/2605.23897)
*Beichen Zhang,Yuhong Liu,Jinsong Li,Yuhang Zang,Jiaqi Wang,Dahua Lin*

Main category: cs.CV

TL;DR: 本文提出ETCHR，一个专门用于视觉推理的图像编辑模型，通过两阶段训练（推理模仿和推理增强）弥合语言和生成间隙，显著提升多模态大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法中，纯文本链式推理在处理需细粒度关注或视角变换的问题时存在瓶颈，而'以图像思考'范式受限于固定工具集或产生噪声图像。

Method: 采用专用图像编辑模型并与理解模型解耦，通过两阶段训练：监督微调编辑轨迹进行推理模仿，再用VLM奖励优化编辑正确性和下游推理准确性。

Result: 在五项任务族（细粒度感知、图表理解、逻辑推理、拼图恢复和3D理解）中，Pass@1在多个MLLM上提升4.61-5.47%，例如Qwen3-VL-8B从55.95提升至60.77。

Conclusion: ETCHR作为解耦的推理感知图像编辑器，可无缝集成到多种开源和闭源MLLM中，无需额外训练，有效提升视觉推理性能。
...
### [99] [PixIE: Prompted Pixel-Space Low-Light Image Enhancement](https://arxiv.org/abs/2605.23531)
*Ruirui Lin,Guoxi Huang,David Bull,Nantheera Anantrasirichai*

Main category: cs.CV

TL;DR: 提出PixIE，一种基于视觉基础模型语义提示的前馈低光照图像增强框架，通过跨尺度去噪、DINO提示像素块（DPPB）和空间-通道压缩（SCC）等创新模块，在降噪和细节恢复上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 低光照图像存在严重噪声、对比度损失和语义模糊，现有增强方法难以同时处理降噪和细节恢复，亟需一种联合解决方案。

Method: 采用跨尺度去噪抑制噪声并保留结构，通过DINOv3特征进行语义提示的像素级调制（DPPB），提出空间-通道压缩（SCC）高效计算像素注意力，以及多感受野像素嵌入（MRPE）增强噪声鲁棒性。

Result: 在LLIE基准测试上，相比现有最优方法，PSNR提升1.9%-15.0%，LPIPS降低8.5%-44.4%，生成更清晰细节和稳定纹理。

Conclusion: PixIE在低光照图像增强中实现了更优的重建保真度和感知质量，验证了语义提示和跨尺度设计的有效性。
...
### [100] [GlowGS: Generative Semantic Feature Learning for 3D Gaussian Splatting in Nighttime Glow Scenes](https://arxiv.org/abs/2605.23602)
*Beibei Lin,Xiao Cao,Jingyuan Guo,Robby T. Tan*

Main category: cs.CV

TL;DR: 针对夜间场景下3DGS方法因缺乏纹理和边缘等结构特征而难以处理光晕区域的问题，提出GlowGS方法，利用扩散模型和视觉基础模型生成并学习语义特征，以隐式结构约束优化渲染视图。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在晴朗场景中表现良好，但在夜间场景的光晕区域，由于缺乏纹理和边缘等结构特征，重建效果不佳。

Method: 包括两个关键创新：语义特征生成和语义学习。语义特征生成：扩散模型从训练视图合成未知姿态的新视图，视觉基础模型评估质量并提取鲁棒特征构建语义特征库。语义学习：从渲染视图提取语义特征，与特征库中最相似特征匹配并最小化距离，施加隐式结构约束。

Result: 实验表明GlowGS能生成语义准确的3D视图，在夜间场景光晕区域显著优于现有方法。

Conclusion: GlowGS通过语义特征生成和学习有效解决了夜间场景的结构缺失问题，提升了3DGS在困难场景下的鲁棒性和质量。
...
### [101] [Calibration-Informative Region Selection for Online LiDAR--Camera Calibration in Agricultural Environments](https://arxiv.org/abs/2605.23580)
*Rajitha de Silva,Grzegorz Cielniak*

Main category: cs.CV

TL;DR: 提出了一种支持图驱动的多模态标定方法，通过解耦初始标定、残差提取、支持图估计和精化四个模块，实现基于运动与深度点对应关系的无目标LiDAR-相机在线标定，并证明标定证据在空间和语义上不均匀。


<details>
  <summary>Details</summary>
Motivation: 传统多模态标定中部分观测主要引入噪声或歧义，需要识别真正约束外参的观测以提升标定可靠性。

Method: 采用支持图驱动框架，包含初始标定（MDPCalib）、跨模态残差提取（CMRNext）、支持图估计和精化四个模块，通过密集支持图聚合对齐观测的跨模态一致性。

Result: 在KITTI和BLT数据集上验证标定证据的空间语义非均匀性，支持图引导的精化可提升平移精度，但旋转精度改善有限。

Conclusion: 密集支持图能有效筛选可靠标定证据，语义区域对标定贡献存在差异，为在线标定提供新的鲁棒范式。
...
### [102] [DDX-TRACE: A Benchmark for Medical Diagnostic Trajectories in VLMs](https://arxiv.org/abs/2605.23629)
...
TL;DR: 提出DDX-TRACE基准，评估多模态神经放射诊断轨迹，发现最终诊断分数不能反映工作流质量。


<details>
  <summary>Details</summary>
Motivation: 现有医疗AI基准只评估最终答案，忽略诊断过程中的证据获取、推理效率等关键问题。

Method: 构建211个病例的评估基准，要求模型逐步请求影像、更新概率诊断并给出定位后停止。

Result: 最先进视觉语言模型存在误导性高分、无效证据请求、视觉误解及低效不确定性更新。

Conclusion: 医疗AI评估应从最终答案转向证据支持的诊断轨迹，DDX-TRACE可揭示规划与推理瓶颈。
...
### [103] [Decomposing Queries into Tool Calls for Long-Video Keyframe Retrieval](https://arxiv.org/abs/2605.23826)
*Michal Shlapentokh-Rothman,Prachi Garg,Yu-Xiong Wang,Derek Hoiem*

Main category: cs.CV

TL;DR: 提出ToolMerge方法，通过LLM分解查询并合并工具排序结果，在长视频问答的关键帧选择上优于现有方法5%。


<details>
  <summary>Details</summary>
Motivation: 现有关键帧选择方法要么对所有帧进行单查询评分，要么使用固定模式分解查询，缺乏灵活性。

Method: 基于LLM的规划器将查询分解为多个工具调用，并用布尔运算符合并每个工具的排序结果。

Result: 在M2M基准测试中，ToolMerge在字幕检索任务上比其他方法高5%，并在问答案例中表现竞争力。

Conclusion: ToolMerge通过分解与合并策略有效提升了关键帧检索的准确性。
...
### [104] [ExpOS: Explainable Open-Surgery Skills Assessment Using 3D Hand Reconstruction](https://arxiv.org/abs/2605.23653)
*Roi Papo,Idan Smoller,Shlomi Laufer*

Main category: cs.CV

TL;DR: 本文提出了ExpOS，一个可解释的开放手术技能评估框架，通过学习运动数据中的判别性时间模式，实现自动、可反馈的技能评估，并在221个视频上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前手术技能评估依赖专家观察，限制了可扩展性和自主练习机会，因此需要自动化的反馈导向评估方法。

Method: 使用双流网络：时间卷积骨干和注意力池化提取时空手-工具动力学特征，并与全局运动统计融合，通过弱监督学习预测技能水平并生成解释性反馈。

Result: 在筋膜缝合任务上表现最佳（r=0.778, R2=0.74），与专家评分高度相关。

Conclusion: 结合弱监督时间重要性学习和可解释运动统计，可实现可扩展且可操作的手术技能评估。
...
### [105] [RiGS: Rigid-aware 4D Gaussian Splatting from a Single Monocular Video](https://arxiv.org/abs/2605.23672)
*Chenyu Wu,Wanhua Li,Zhu-Tian Chen,Hanspeter Pfister*

Main category: cs.CV

TL;DR: 提出RiGS，通过静态、刚体和瞬态三类高斯原语联合建模多时间尺度运动，实现从单目视频重建动态3D场景。
...
Method: 使用三类高斯原语（静态、刚体、瞬态）分别建模背景、长期低频运动和短期高频动态；引入物体级动态掩码聚合时空信息；刚体高斯可随时间转为瞬态高斯，并受场景流监督。

Result: 在新视角合成基准上达到最先进性能。

Conclusion: RiGS有效建模多尺度运动，提升动态场景重建的时空一致性。
...
### [106] [CRONOS: Benchmarking Counterfactual Physical Consistency in Video Models](https://arxiv.org/abs/2605.23699)
*León Begiristain,Olaf Dünkel,Adam Kortylewski*

Main category: cs.CV

TL;DR: 提出CRONOS基准，用于评估视频预测模型的反事实物理一致性，发现现有模型存在严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 视频预测模型是否学习到因果结构而非表面相关性尚不明确，缺乏系统评估工具。

Method: 基于Unreal Engine构建CRONOS基准，通过干涉视角、场景、物体类别和外观四个关键因素生成反事实视频，固定物理事件类型。

Result: 现有视频生成模型在反事实物理一致性上表现不佳，预测质量受外观、环境尤其是视角变化影响显著。

Conclusion: CRONOS为诊断视频质量变化提供了可控测试平台，有助于开发跨条件一致的模型。
...
### [107] [Recursive Block-Diagonal Coupling for Resource-Efficient Training of Vision Models](https://arxiv.org/abs/2605.23656)
*Maxim Henry,Adrien Deliège,Sébastien Piérard,Marc Van Droogenbroeck*

Main category: cs.CV

TL;DR: 提出了一种高效的训练协议RBDC，通过无参数块对角方式递归耦合独立训练的窄模型构建宽模型，在ImageNet上实现30% FLOPs减少且保持相似精度。


<details>
  <summary>Details</summary>
Motivation: 现有模型生长方法依赖预训练的窄模型，掩盖了整体计算成本，且从零训练高容量模型需大量计算资源。

Method: 递归块对角耦合（RBDC）协议，将多个独立训练的窄模型通过无参数块对角方式组合成宽模型，灵活分配训练预算。

Result: 在ImageNet上，与标准训练相比FLOPs降低30%且精度相当；优于模型生长方法；作为下游任务骨干网络表现更佳。

Conclusion: RBDC协议能高效训练宽模型，在计算效率和下游任务性能上均具优势。
...
### [108] [Machine learning applied to emerald gemstone grading: framework proposal and creation of a public dataset](https://arxiv.org/abs/2605.23777)
*FB Pena,D Crabi,Sandro C Izidoro,Érick O Rodrigues,G Bernardes*

Main category: cs.CV

TL;DR: 该论文提出了一套完整的祖母绿分级自动化框架，结合图像处理与机器学习，替代了传统人工主观分级，准确率达98%。


<details>
  <summary>Details</summary>
Motivation: 传统宝石分级依赖专家主观判断，不同专家可能给出不同结果，缺乏一致性。

Method: 构建包含图像采集、特征提取和机器学习的完整流程，并发布包含192张祖母绿图像的数据集。

Result: 所提框架在分级准确率上达到98%，优于深度学习方法。

Conclusion: 该自动化方法能消除主观性，有效提升分级一致性和效率。
...
### [109] [Exploring deep learning for Event-Based Saliency Prediction with a Transformer-based model](https://arxiv.org/abs/2605.23790)
*Romaric Mazna,Jean Martinet,Sai Deepesh Pokala*

Main category: cs.CV

TL;DR: 本文提出SEST，首个基于Transformer的事件数据显著性预测模型，通过事件原生预训练和合成监督解决数据稀缺问题，在基准数据集上表现优越。


<details>
  <summary>Details</summary>
Motivation: 事件相机在生物启发和传感特性上有优势，但缺乏大规模事件显著性数据集和强基线模型，该方向研究空白。

Method: 采用自监督预训练的基于Swin Transformer的事件骨干网络与轻量CNN解码器，通过合成数据生成两个新基准数据集（N-DHF1K和N-UCF Sports）。

Result: SEST显著优于现有事件显著性方法，缩小了与RGB模型的性能差距，并在真实事件相机数据上零样本泛化有效。

Conclusion: 此工作首次将深度学习应用于事件显著性预测，开辟了事件视觉与神经形态注意力交叉的新研究方向。
...
### [110] [Revitalizing Dense Material Segmentation: Stabilized Vision Transformers and the Generalization Paradox](https://arxiv.org/abs/2605.23747)
*Allan Kazakov,Duygu Cakir,Hilal Kurt İrfanoğlu,Yavuz İrfanoğlu*

Main category: cs.CV

TL;DR: 本文提出了稳定训练方法和新型基准，在苹果密集材料分割（DMS）数据集上使用SegFormer-B5实现了新的最先进结果（0.4572 mIoU），并发现了由于数据划分导致的“泛化悖论”。


<details>
  <summary>Details</summary>
Motivation: 材料分割任务在计算机视觉中具有挑战性，且由于几何偏置的基模型和标准训练范例在非结构化纹理场上的失败，导致Apple DMS基准面临停滞。

Method: 引入了高保真logit投影、查询熵正则化以及物理一致的领域特定增强管道，以稳定训练；并评估了SegFormer和Mask2Former架构。

Result: 优化后的SegFormer-B5在原始数据划分上达到0.4572 mIoU，在80/10/10划分上达到0.5276 mIoU，但后者导致分布同质化，降低了真实世界性能。
...
### [111] [A Novel Approach for the Counting of Wood Logs Using cGANs and Image Processing Techniques](https://arxiv.org/abs/2605.23775)
...
TL;DR: 本研究利用条件生成对抗网络（cGANs）结合连通分量算法，实现了桉木原木的高精度分割与计数，平均像素精度达96.4%，原木计数精度达92.3%。


<details>
  <summary>Details</summary>
Motivation: 解决人工计数木材时效率低、易出错的问题，为自动化材料管理、木材监控、体积估算等应用提供技术支持。

Method: 采用cGANs进行原木图像分割，结合专门图像处理技术处理噪声和交叉情况，最后利用连通分量算法实现高效计数。

Result: 在466张含约13048根桉木的图像上，平均像素精度96.4%、原木计数精度92.3%，F1分数0.879-0.933，IoU 0.784-0.875，单张处理时间0.713秒（NVIDIA T4 GPU）。

Conclusion: 该方法能有效处理复杂场景（如交叉原木、不同环境条件），适用于实时操作，具有显著的实际应用价值，并为体积估算等高级分析奠定基础。
...
### [112] [Learning a Particle Dynamics Model with Real-world Videos](https://arxiv.org/abs/2605.23845)
*Chanho Kim,Suhas V. Sumukh,Li Fuxin*

Main category: cs.CV

TL;DR: 提出一种直接从无标签真实视频训练粒子动力学模型的方法，利用高斯溅射框架预测粒子位置和旋转变化，无需粒子级标注。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动物理模拟模型依赖合成数据，受限于模拟到现实的差距，而真实世界难以获得完整状态信息。

Method: 基于高斯溅射的粒子动力学模型，从密集高斯粒子（含尺度和旋转）中学习，通过渲染监督在视频上训练。

Result: 在约500个真实视频组成的多样化物体交互数据集上验证了模型有效性。

Conclusion: 该方法可直接从真实世界视频学习物理动力学，克服了模拟数据依赖的局限性。
...
### [113] [MuellerPT: Decomposition Driven Pretraining for Dense Learning in Mueller Polarimetry](https://arxiv.org/abs/2605.23840)
*Adam Tlemsani,Yingdian Li,Maxime Giot,Naim Slim,Christopher J. Peters,Abhijeet Ghosh,Daniel S. Elson*

Main category: cs.CV

TL;DR: 提出MuellerPT，通过预测Lu-Chipman分解的预训练方法，提高Mueller矩阵成像的标签效率和跨领域迁移能力，在脑组织分割和癌症分类任务中显著优于无预训练模型。


<details>
  <summary>Details</summary>
Motivation: Mueller矩阵成像提供丰富对比度，但监督学习受限于稀疏标注和领域偏移，需要更高效的预训练方法。

Method: 构建了MuellerPT预训练框架，预测每个像素的4x4 Mueller矩阵的Lu-Chipman分解；收集了多光谱动物极化成像数据集MAP-Org进行大规模预训练；使用分割头和分类头进行下游任务微调。

Result: 在分割任务中，使用5%数据时DICE提升超过20%；在分类任务中，使用1%数据时准确率提升8%；定性展示了人体食管样本的鲁棒性。

Conclusion: 预测Lu-Chipman分解是一种有效的预训练任务，能增强标签效率和对领域偏移的鲁棒性，为Mueller成像的标签高效应用奠定基础。
...
### [114] [LaMo: Self-Supervised Latent Motion Priors for Physical Realism in Video Generation](https://arxiv.org/abs/2605.23878)
*Bo Jiang,Depu Meng,Yihan Hu,Yichen Xie,Tianshuo Xu,Wei Zhan*

Main category: cs.CV

TL;DR: 提出LaMo方法，通过无监督视频帧间运动先验提升视频扩散模型的物理一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成器物理与运动一致性不足，外部监督方法依赖额外资源。

Method: 从无标签视频中提取潜运动先验，包括宏运动漂移损失和微运动场引导，即插即用无需改动架构。

Result: 在VideoPhy、VideoPhy2上超越物理感知基线，在VBench上保持质量同时改善运动维度。

Conclusion: 无标签视频中的运动监督能有效提升视频扩散模型的物理保真度。
...
### [115] [HorizonStream: Long-Horizon Attention for Streaming 3D Reconstruction](https://arxiv.org/abs/2605.23889)
...
TL;DR: 提出HorizonStream，一种通过因子化证据影响核的长时域Transformer，在常内存和线性时间下实现超过10000帧的稳定3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有流式3D重建方法因长短时证据的传播模式不匹配，导致漂移、抖动或崩溃。

Method: 几何线性注意力学习通道级衰减率以多尺度传播长期证据；几何局部注意力结合时空RoPE抑制注意力沉没；度量读出令牌恢复稳定尺度和位姿。

Result: 仅用48帧训练即可泛化到超过10000帧序列，达到最先进的流式3D重建性能。

Conclusion: HorizonStream通过显式因子化证据影响核，解决了流式几何重建中的时间异质性挑战。
...
### [116] [Smart-Insertion-V: Photorealistic Video Insertion via a Closed-Loop Feedback Dual-Stream Framework](https://arxiv.org/abs/2605.23891)
*Xiao Cao,Yansong Qu,Xiangzhen,Chang,Wen Xiao,Jiakui Hu,Heyuan Li,Jialun Liu,Zhiyong Huang,Xuelong Li*

Main category: cs.CV

TL;DR: 提出Smart-Insertion-V框架，结合双流结构和闭环反馈，解决视频物体插入中的风格差异问题，并开源数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法在参考对象与源场景存在严重风格差异时表现不佳。

Method: 双流框架（图像流同步指导视频生成）+ 闭环反馈机制 + Dual-World-View RoPE（区分信号）+ 解耦引导模块（VLM语义推理+文本编码器）。

Result: 方法能将物体插入合理位置，并取得最和谐的结果。

Conclusion: 所提框架有效克服了风格差异，实现了鲁棒的视频物体插入。
...
### [117] [Vision Transformers Need Better Token Interaction](https://arxiv.org/abs/2605.23868)
*Linxiang Su*

Main category: cs.CV

TL;DR: 本文发现Vision Transformer（ViT）在长期训练中会出现“语义扩散”现象，即全局语义信息过度传播到局部补丁，导致密集预测性能下降。提出用entmax-1.5替换softmax注意力，实现稀疏注意力，在保持全局连接的同时提升密集表示质量。


<details>
  <summary>Details</summary>
Motivation: ViT在图像级表示上表现优异，但长期训练后补丁级表示对密集预测任务效果变差，现有研究仅归因于高范数伪影，不足以解释这一现象。

Method: 通过分析语义扩散机制，论证无需移除全局上下文，而应使token交互更具选择性。采用稀疏注意力干预，将标准softmax注意力替换为entmax-1.5，保持全局连接性。

Result: 在DINOv1 ViT-S/16模型上训练200个epoch，ImageNet线性探测精度保持不变；语义分割性能显著提升：VOC mIoU从42.80升至48.78，ADE20K从19.85升至21.97，Cityscapes从36.79升至37.87。

Conclusion: 选择性token混合是一种简单有效的偏置，能够显著改善ViT的密集表示质量，为提升密集预测任务性能提供了新方向。
...
### [118] [GenRecon: Bridging Generative Priors for Multi-View 3D Scene Reconstruction](https://arxiv.org/abs/2605.23888)
*Katharina Schmid,Nicolas von Lützow,Jozef Hladký,Angela Dai,Matthias Nießner*

Main category: cs.CV

TL;DR: 提出了一种结合生成式3D先验的多视图RGB图像高保真3D场景重建方法，通过将场景分割为重叠块并利用Trellis.2模型实现大场景生成，最终在室内环境重建中超越现有方法16%。


<details>
  <summary>Details</summary>
Motivation: 现有场景重建方法在保真度和完整性上有限，而生成式模型虽在物体级别表现优异但难以扩展到大场景，因此需要一种方法将强生成先验从物体级提升至场景级。

Method: 使用Trellis.2作为生成先验，通过基于投影的条件机制将多视图图像特征提升为与生成模型对齐的连贯3D表示，并采用空间锚定的重叠块分割策略实现场景级生成。

Result: 在室内环境重建中，生成的PBR网格模型在保真度和多视图一致性上显著优于现有方法，定量指标提升16%。

Conclusion: 该方法有效弥合了物体级生成模型与场景级重建之间的鸿沟，实现了高保真、可编辑的3D场景重建。
...
### [119] [From Activation to Causality: Discovery of Causal Visual Representations in the Human Brain](https://arxiv.org/abs/2605.23895)
*Yuval Golbari,Navve Wasserman,Matias Cosarinsky,Roman Beliy,Aude Oliva,Antonio Torralba,Michal Irani,Tamar Rott Shaham*

Main category: cs.CV

TL;DR: 提出了BrainCause框架，通过生成对抗性刺激和因果测试来验证大脑中的视觉概念表征，发现仅靠激活强度不足以确认表征。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅通过激活最大化定位功能区域，但强激活可能由相关视觉或语义线索驱动，无法证明真正的概念表征。

Method: 结合生成模型和大脑模型，构建包含概念图像、反事实编辑和相关干扰项的刺激集，使用图像到fMRI编码模型预测脑反应，搜索对目标概念特异表征。

Result: 成功恢复已知功能定位并发现数十个概念的新候选表征，在预测和实测fMRI数据上均得到验证。无因果验证时大部分定位为假阳性。

Conclusion: 激活本身不足以作为表征证据，因果验证对确认神经表征至关重要。
...
### [120] [Geo-Align: Video Generation Alignment via Metric Geometry Reward](https://arxiv.org/abs/2605.23903)
*Zizun Li,Haoyu Guo,Runzhe Teng,Chunhua Shen,Tong He*

Main category: cs.CV

TL;DR: 提出了首个面向摄像机控制视频重渲染的强化学习框架Geo-Align，通过尺度感知奖励机制优化模型，无需配对数据即可提升相机控制精度和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有视频重渲染方法依赖合成数据集进行监督微调，缺乏真实多视角同步数据，导致对分布外真实视频泛化能力不足，难以精确遵循物理尺度和相机轨迹。

Method: 基于预训练模型，引入强化学习与尺度感知感知奖励机制：通过3D估计器提取生成视频的相机轨迹，显式惩罚旋转和平移偏差；设计基于真实条件视频和合成目标轨迹的数据流程，避免配对数据依赖。

Result: 在相机控制精度和视觉保真度上持续优于现有监督学习基线，验证了方法的有效性。

Conclusion: Geo-Align通过强化学习优化无需配对数据的相机控制，显著提升了真实场景下的泛化能力与可控性。
...
### [121] [PiD: Fast and High-Resolution Latent Decoding with Pixel Diffusion](https://arxiv.org/abs/2605.23902)
*Yifan Lu,Qi Wu,Jay Zhangjie Wu,Zian Wang,Huan Ling,Sanja Fidler,Xuanchi Ren*

Main category: cs.CV

TL;DR: 提出了PiD，一种将潜在空间解码重新定义为条件像素扩散的像素扩散解码器，能高效地生成高分辨率图像，4倍甚至8倍放大，速度快且视觉质量好。


<details>
  <summary>Details</summary>
Motivation: 现有高分辨率文本到图像系统的潜在到像素解码器是面向重建的，优化目标是反转编码器而非合成细节，且在百万像素级别成本高昂，因此需要更高效、更有表现力的解码范式。

Method: 将潜在解码转化为条件像素扩散，在像素空间直接去噪以生成高分辨率图像；引入轻量级σ感知适配器注入噪声潜在，支持早期终止潜在扩散；使用DMD2进行蒸馏，将推理步骤降至4步。

Result: 在RTX 5090上1秒内将512×512图像的潜在解码为2048×2048像素，峰值内存13 GB；在GB200 GPU上仅需210毫秒，比级联扩散超分辨率管道快约6倍，且视觉保真度更高。

Conclusion: PiD通过像素扩散统一解码和上采样，实现了高效、高质量的高分辨率图像生成，适用于常规VAE潜在和语义潜在，显著提升了解码效率。
...
### [122] [Evaluating Large Language Models in a Complex Hidden Role Game](https://arxiv.org/abs/2605.22826)
*Niklas Bauer*

Main category: cs.CL

TL;DR: 这篇论文通过秘密希特勒社交推理游戏评估大语言模型的欺骗能力，发现当前模型在多轮操纵和战略深度上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 量化大语言模型的欺骗潜力对AI安全至关重要，但在非受控环境中难以实现，因此需要可复现的测试框架。

Method: 开发了开源框架和新指标（角色识别准确率、欺骗保持率、游戏状态影响率），将模型与基于规则的算法和人类游戏进行对比，并测试了链式思维提示和内部记忆等推理增强技术。

Result: 推理增强技术未带来改进，法西斯角色胜率下降23.2%。规则型代理与人类投票一致性达86.7%，而Llama 3.1 70B仅59.7%。法西斯角色产生负面影响分数，游戏时长比人类短约40%。

Conclusion: 当前架构在多轮复杂操纵中效果有限，但该框架可作为未来对齐研究的可复现测试工具。
...
### [123] [A Survey of Text and Speech Resources for Hausa and Fongbe: Availability, Quality, and Gaps for NLP Development](https://arxiv.org/abs/2605.22828)
*Mahounan Pericles Adjovi,Victor Olufemi,Roald Eiselen,Prasenjit Mitra*

Main category: cs.CL

TL;DR: 本文系统梳理了豪萨语和丰贝语在文本与语音层面的公开NLP资源现状，发现前者文本资源多样，后者语音数据有近期的学术收集进展，并指出了资源缺口。


<details>
  <summary>Details</summary>
Motivation: 豪萨语（约1亿人使用）和丰贝语（约200万人使用）在NLP资源可获取性上差异显著，现有资源缺乏系统性整理，本研究旨在系统盘点并识别关键差距。

Method: 通过系统搜索学术数据库、数据平台和网络来源，分类整理平行语料、单语文本、语音数据集、预训练模型和评估基准，并记录每项资源的大小、领域、格式、许可和可访问性。

Result: 豪萨语拥有更丰富的文本资源，涵盖新闻、百科和教育领域；丰贝语虽然文本资源有限，但在语音数据收集方面有近期的学术项目。两种语言均被纳入Masakhane的命名实体识别和词性标注基准。

Conclusion: 提出了针对具体任务的建议，指出优先缺口包括丰贝语的多领域文本数据以及豪萨语专用的语音语料库。
...
### [124] [Query-Adaptive Semantic Chunking for Retrieval-Augmented Generation: A Dynamic Strategy with Contextual Window Expansion](https://arxiv.org/abs/2605.22834)
*Mudit Rastogi*

Main category: cs.CL

TL;DR: 提出查询自适应语义分块方法，通过将用户查询整合到分块过程中，动态构建文档块，在200个查询和100篇文档上F1达到0.85，相比固定分块提升18-27%。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统分块方法无法解决精确率-召回率权衡，且未在分块阶段整合用户查询，导致检索质量受限。

Method: QASC方法包括三个机制：余弦相似度评分识别种子句子、上下文窗口扩展保持连贯性、分块级分数聚合确保整体相关性。

Result: 在100篇技术文档、200个查询上F1=0.85，相对固定分块提升18-27%，消融实验验证各组件有效性，人类评估Cohen kappa=0.82。

Conclusion: QASC通过查询驱动的动态分块显著优于现有固定、语义和代理分块方法，生成更相关且连贯的文档块。
...
### [125] [How Far Will They Go? Red-Teaming Online Influence with Large Language Models](https://arxiv.org/abs/2605.22880)
*Daniel C. Ruiz,Anna Serbina,Ashwin Rao,Emilio Ferrara,Luca Luceri*

Main category: cs.CL

TL;DR: 提出了一个衡量开源LLM政治表达范围（Overton Windows）的红队测试框架，并量化了越狱攻击如何扩展这一范围。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的代理越来越多地参与在线讨论，需要评估它们被用于政治影响活动的风险，尤其是针对开源模型。

Method: 设计了红队测试框架来测量Overton Windows，并通过简单自然语言越狱攻击量化其扩展；评估了30多个开源LLM。

Result: 开源LLM通常更愿意生成左倾内容；Overton Windows随模型增大而收缩；不同地区和模型家族的越狱效果差异显著。

Conclusion: 该框架可用于审计开源LLM的政治可操纵性，并帮助设计更强效的对抗措施。
...
### [126] [Knowledge Distillation for Low-Resource Open-source Text-to-SQL Model](https://arxiv.org/abs/2605.22843)
*Tianhao Qiu,Xiaojun Chen*

Main category: cs.CL

TL;DR: 提出了知识感知的Text-to-SQL框架，通过构建包含模式语义、缩写、业务逻辑和查询模式的任务特定知识库，实现低资源场景下的高质量SQL生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在低资源场景下难以生成符合数据库约束且语义准确的SQL，尤其在领域特定数据库中缺乏高质量标注数据和模式语义信息。

Method: 构建任务特定的知识库（涵盖模式语义、缩写、业务逻辑和查询模式），并将其注入训练和推理过程；生成多样化的合成训练数据，并通过定向知识检索增强推理。

Result: 在7个基准数据集（含通用和领域特定）上，显著提升了开源和闭源大语言模型在Text-to-SQL任务中的性能，尤其在低资源领域设置中表现出更好的泛化性、鲁棒性和适应性。

Conclusion: 知识感知框架能有效克服低资源场景下的数据稀缺和语义歧义问题，显著提升Text-to-SQL质量，为实际应用提供可行方案。
...
### [127] [RAS: Reflection-Augmented Scaling with In-Context Learning for Executable Cypher Query Generation](https://arxiv.org/abs/2605.22937)
*Minseok Jung,Abhas Ricky,Muhammad Rameez Chatni*

Main category: cs.CL

TL;DR: 该论文研究在结构化查询生成中利用执行错误反馈的推理时缩放方法，提出反射增强缩放（RAS）比独立缩放（IS）更有效降低语法错误。


<details>
  <summary>Details</summary>
Motivation: 现有推理时缩放方法未充分利用代码生成中数据库返回的执行错误反馈，导致语法错误未被有效处理。

Method: 对比两种推理方法：独立缩放（IS，无记忆重采样）和反射增强缩放（RAS，通过上下文学习将执行反馈作为条件）。在三个Neo4j数据集和五个语言模型上实验。

Result: RAS在n=5时将查询执行错误率降低41-50%，优于IS的32-38%。

Conclusion: 执行错误是可操作的反馈，基于此结构化推理时计算比独立缩放样本更高效地实现语法正确性。
...
### [128] [Graph Alignment Topology as an Inductive Bias for Grounding Detection](https://arxiv.org/abs/2605.22963)
*Paul Landes,Pranav Herur,Adam Cross,Jimeng Sun*

Main category: cs.CL

TL;DR: 提出了一种基于对齐拓扑的图神经网络方法，用于检测大语言模型生成的幻觉，在多个数据集上实现了最优性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在生成与事实不符内容的倾向，在需要严格事实正确性的场景（如临床决策支持）中受到限制。现有检测方法未充分利用对齐拓扑作为归纳偏置。

Method: 构建参考信息与LLM输出之间的对齐二分图，训练图神经网络通过消息传递建模对齐结构。

Result: 在四个不同的幻觉和问答数据集上取得了最先进的性能，超越了包括GPT-4o在内的基础大语言模型。

Conclusion: 利用对齐拓扑作为归纳偏置能有效提升幻觉检测的准确性，表明结构学习优于仅依赖生成概率或检索增强的方法。
...
### [129] [Learnability-Informed Fine-Tuning of Diffusion Language Models](https://arxiv.org/abs/2605.22939)
*Shubham Parashar,Atharv Chagi,Jacob Helwig,Lakshmi Jotsna,Sushil Vemuri,James Caverlee,Dileep Kalathil,Shuiwang Ji*

Main category: cs.CL

TL;DR: 提出LIFT算法，通过调整不同时间步的学习目标，提升扩散语言模型在推理任务上的表现，最高在AIME上获得3倍相对提升。


<details>
  <summary>Details</summary>
Motivation: 标准SFT用于扩散语言模型时效果不佳甚至有害，原因在于忽略了不同时间步下不同token的可学习性差异。

Method: LIFT算法，根据扩散时间步的信息量动态调整学习目标：在掩码多时学习简单token，在上下文多时学习困难token。

Result: 在六个推理基准上超越现有SFT基线，AIME'24和AIME'25上取得最高3倍相对增益。

Conclusion: 通过对齐训练与扩散时间步的信息可用性，能有效提升扩散语言模型的推理能力。
...
### [130] [Can AI Guess What You Know? Performance Comparison of Large Language Models for Human Domain Knowledge Estimation From Communication Logs](https://arxiv.org/abs/2605.22971)
*Ko Watanabe,Shoya Ishimaru*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型（LLMs）能否从Slack长期日志中推断个人领域知识，实验表明Gemini 2.5 Flash的误差最低，但当前模型性能受限于文本量，需关注隐私和知识表征。


<details>
  <summary>Details</summary>
Motivation: 员工常难以识别“谁懂什么”，导致组织效率损失，而传统方法无法自动从通信日志中提取专家知识。

Method: 基于27,188条Slack消息和43名用户数据，比较7种LLM（Gemini、Claude、GPT等）的零样本预测与27名参与者的自评技能得分，计算平均绝对误差（MAE）。

Result: Gemini 2.5 Flash达到最低MAE（21.13%），GPT模型误差较大；估计精度与消息量弱相关。

Conclusion: 自动化专业知识映射可行但有局限，需开发保护隐私的部署方案和更丰富的结构感知知识表示。
...
### [131] [When AI Takes Sides on Questions of Faith: Persistent Asymmetries in AI-Mediated Faith Guidance](https://arxiv.org/abs/2605.22975)
*Brett Israelsen,Sheryl Carty,Josh Coates,Nancy Fulda,Julie Park,Pete Whiting*

Main category: cs.CL

TL;DR: 大型语言模型对宗教改宗查询表现出系统性不对称偏好，某些宗教（如天主教、巴哈伊教、锡克教）被鼓励加入，而另一些（如无神论、不可知论、耶和华见证人）则被劝阻。


<details>
  <summary>Details</summary>
Motivation: 探究生成式AI在敏感领域（宗教）是否隐藏偏见，以及这种偏见是否可重复、非偶然。

Method: 测试20个商用/开源模型在182宗教配对中的反应，使用人工验证的LLM-as-a-judge框架模拟用户咨询，并多次重复验证。

Result: 所有模型均表现出可复现的不对称性，偏好模式因模型大小和供应商而异（如Grok 4.20偏差最明显），且不受提问措辞变化影响。

Conclusion: 不对称性是大模型的稳健属性，而非评分伪影；大范围部署此类偏见可能产生实际社会影响。
...
### [132] [A Reproducible Universal Dependencies-Style Pipeline for Katharevousa Greek Parliamentary Text](https://arxiv.org/abs/2605.22978)
*George Mikros,Fotios Fitsilis*

Main category: cs.CL

TL;DR: 本文为希腊Katharevousa语构建了UD风格句法分析资源，含1,697句标注数据，并对比多种解析器，XLM-R模型达到最佳LAS 0.5162。


<details>
  <summary>Details</summary>
Motivation: Katharevousa希腊语在法律、行政和议会档案中很重要，但现有NLP管线对其支持不足。

Method: 提出可复现工作流，包括OCR重构、LLM辅助标注、自动验证、CoNLL-U快照、固定划分评估和模型比较。

Result: 离线解析器表现差（spaCy LAS 0.4183），XLM-R最优（LAS 0.5162），基于特征的模型在UPOS和关系标注上仍有竞争力。

Conclusion: 提供了将历史议会OCR转化为可复用句法NLP基础设施的可审计方法，并开源了全部资源。
...
### [133] [Memorization Dynamics of Fill-in-the-Middle Pretraining](https://arxiv.org/abs/2605.22981)
*Tobias von Arx,Tanguy Dieudonné*

Main category: cs.CL

TL;DR: 本文研究了填充中间（FIM）预训练目标对语言模型逐字记忆的影响，发现FIM倾向于恢复短片段，而标准自左向右（LTR）训练更可能高置信度地预测长连续内容，且记忆行为受上下文形式和重复次数影响。


<details>
  <summary>Details</summary>
Motivation: FIM预训练目标广泛用于增强语言的填充能力，但其对逐字记忆的影响尚不清楚，需要系统研究。

Method: 在受控环境中，使用FineWeb-Gutenberg语料库（含重复内容）预训练匹配的Llama 3.2模型，分别采用FIM和标准LTR目标，并通过前缀探针和FIM格式探针评估记忆行为。

Result: FIM训练更易恢复短片段或部分匹配内容，而LTR更关注长精确连续；FIM的逐字提取随重复次数线性增长；后缀上下文无法独立支持FIM记忆，主要依赖前缀上下文；不同探针长度和格式会遗漏记忆行为差异。

Conclusion: FIM训练的记忆模式与LTR不同，且评估记忆需考虑上下文和探针设计的多样性，避免单一指标导致误解。
...
### [134] [A Proactive Multi-Agent Dialogue Framework for Assessing Social Language Disorder Traits in Autism](https://arxiv.org/abs/2605.22993)
*Chuanbo Hu,Minglei Yin,Bin Liu,Wenqi Li,Lynn K. Paul,Shuo Wang,Xin Li*

Main category: cs.CL

TL;DR: 提出TPA多智能体对话框架，通过主动选择提问策略，在ADOS-2评估中比临床医生对话多覆盖16.6%的语言障碍特征。


<details>
  <summary>Details</summary>
Motivation: 自闭症谱系障碍中的社交语言障碍特征在自然对话中罕见，且临床评估中提问策略的选择显著影响诊断信息获取，但现有研究未探索大语言模型能否主动选择策略来系统揭示这些特征。

Method: 设计TPA框架：医生智能体先推理未观察特征，再选择临床策略生成问题；患者智能体基于真实ADOS-2数据模拟患者语言。

Result: 在35名患者的484轮评估中，TPA达到82.1%的特征覆盖率，比临床医生对话高16.6%，每轮诊断效率（AUCC）提升0.170。

Conclusion: 主动提问策略选择显著提升自动社交语言障碍特征评估效率，为可扩展的AI辅助临床筛查提供新方法。
...
### [135] [Brain-LLM Alignment Tracks Training Data, Not Typology](https://arxiv.org/abs/2605.23032)
*Dongxin Guo,Jikun Wu,Siu Ming Yiu*

Main category: cs.CL

TL;DR: 大脑-大语言模型(LLM)的对齐在英语中已建立，但跨语言是否泛化受训练语言主导和类型学距离影响。


<details>
  <summary>Details</summary>
Motivation: 研究脑-LLM对齐是否跨语言泛化，以及其变化原因。

Method: 使用112名被试的fMRI数据(英语、中文、法语)及7个LLM，分析训练语言主导、类型学距离、分词生育率等因素。

Result: 训练语言主导决定对齐模式；类型学距离独立预测对齐退化；句法区域梯度更陡；分词生育率解释约60%的编码层跨语言偏移。

Conclusion: 英语优势是训练数据组成的伪像，剩余变异反映句法处理的类型学结构。
...
### [136] [Sparse Autoencoders Map Brain-LLM Alignment onto Cortical Semantic Topography](https://arxiv.org/abs/2605.23035)
*Dongxin Guo,Jikun Wu,Siu Ming Yiu*

Main category: cs.CL

TL;DR: 本文利用稀疏自编码器分解大语言模型中间层，发现语义特征主导了模型与人类大脑活动的对齐，并成功预测了大脑皮层的语义组织。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型中间层最能预测人类大脑对语言的反应，但其背后的神经机制尚不明确。

Method: 采用稀疏自编码器将GPT-2 XL和Llama-3.1-8B的每一层分解为16K-32K可解释特征，结合神经编码模型和人类验证的分类体系进行分析。

Result: 语义特征单独解释了94%的峰值编码性能（r=0.285），显著优于方差匹配基线；五个语义子类能准确映射到独立大脑区域；SAE特征还能预测人类阅读时间（ΔlogLik=38.4，p<0.001），且跨英语、中文、法语语言泛化。

Conclusion: 稀疏自编码器揭示的语义特征能够解释语言模型与大脑对齐的机制，并以前所未有的粒度复现已知的皮层语义组织，为神经语言学提供了新工具。
...
### [137] [Multilingual Steering by Design: Multilingual Sparse Autoencoders and Principled Layer Selection](https://arxiv.org/abs/2605.23036)
*Yusser Al Ghussin,Daniil Gurgurov,Tanja Baeumel,Josef van Genabith,Patrick Schramowski,Simon Ostermann*

Main category: cs.CL

TL;DR: 本文提出一种基于多语言稀疏自编码器和先验层选择规则的多语言语言控制方法，能提升跨语言表征质量和生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器在跨语言场景中不可靠，主要因为训练数据仅为英语、且层选择依赖启发式方法。

Method: 使用多语言数据训练SAEs，并基于多语言对齐与语言可分性的交集提出先验层选择规则。

Result: 在LLaMA-3.1-8B和Gemma-2-9B上，多语言SAEs结合交集层选择稳定了语言识别准确率与生成质量之间的权衡。

Conclusion: 多语言SAEs结合先验层选择可提供更可靠、可预测的多语言表征级语言控制。
...
### [138] [Do Language Models Know What Not to Say? Causal Evidence for Statistical Preemption in LLMs](https://arxiv.org/abs/2605.23039)
*Dongxin Guo,Jikun Wu,Siu Ming Yiu*

Main category: cs.CL

TL;DR: 研究表明，大型语言模型通过统计优先机制（statistical preemption）获取否定语言知识，即接触常规形式抑制了结构可能但未出现的替代形式。


<details>
  <summary>Details</summary>
Motivation: 探索学习者如何在没有负面证据的情况下获得关于不可接受语言知识的认知。

Method: 通过计算研究，在四个实验中考察120个英语动词-结构配对，使用语言模型计算惊喜度（surprisal），并结合人类接受度判断验证。

Result: (1) 语言模型惊喜度与人类接受度判断高度相关(r=0.79)；(2) 模式由竞争形式频率而非总体动词频率驱动；(3) 优先敏感性随模型大小呈幂律增长；(4) 微调干预因果证明控制竞争形式频率可改变优先行为。

Conclusion: 神经语言模型通过分布竞争获取否定语言知识，验证了构式语法的核心机制。
...
### [139] [HawkesLLM: Semantic Uncertainty Propagation in Agentic Text Simulation](https://arxiv.org/abs/2605.23043)
*Zewei Deng,Tinghan Ye,Liyan Xie*

Main category: cs.CL

TL;DR: 提出了HawkesLLM框架，通过多变量霍克斯过程建模文本序列中的路径依赖不确定性，在紧凑记忆预算下改善了后期语义对齐。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成系统按顺序生成，早期不确定性会影响后续输出，导致路径依赖问题。

Method: 将文本级联表示为网络节点，用多变量霍克斯过程建模节点激活时间和历史影响，再由语言模型生成新事件。

Result: 在GDELT新闻级联案例中，HawkesLLM提升了后期语义对齐度，并区分了局部漂移和全局漂移。

Conclusion: 该方法能有效管理序列生成中的路径依赖不确定性，在有限记忆下保持语义一致性。
...
### [140] [DreamerNLplus: Interpretable Modeling of Mental Health Dynamics from Social Media Timelines using Hybrid Rule-Based and RAG Methods](https://arxiv.org/abs/2605.23052)
*Maryia Zhyrko,Daisy Monika Lal,Erik van Mulligen,Lifeng Han*

Main category: cs.CL

TL;DR: 提出了一个混合框架DreamerNLplus，用于在CLPsych 2026共享任务中从社交媒体时间线建模心理健康动态，包含三种任务并取得了较好排名。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决心理健康动态建模中的现有挑战，如分类与回归性能不匹配、时间过渡建模困难以及评估指标不一致。

Method: 任务1使用LLM数据增强、DeBERTa分类和随机森林回归；任务2采用少样本提示和本地Llama 3.1模型检测时间事件；任务3.1探索了规则式和少样本LLM方法，任务3.2使用RAG方法。

Result: 在任务3.1排名第二，任务3.2中改善任务第一、恶化任务第三，展示了RAG方法捕捉心理变化模式的能力。

Conclusion: 揭示了心理健康动态建模的复杂性，强调需要统一评估框架，并为未来工作提供了方向。
...
### [141] [What Training Data Teaches RL Memory Agents: An Empirical Study of Curriculum Effects in Memory-Augmented QA](https://arxiv.org/abs/2605.23067)
*Xinjie He,Zhiyuan Lin,Su Liu,Jialun Wu,Qiyang Xie,Weikai Zhou,Shuai Xiao*

Main category: cs.CL

TL;DR: 本文研究了训练课程组成如何影响基于强化学习的LLM记忆体代理的技能习得，发现混合课程有助于整体性能，而单一领域课程能强化特定推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有工作仅基于单一基准训练记忆体代理，忽略了训练数据组成对技能泛化与特化的影响。

Method: 在固定架构、RL算法与超参数条件下，对比三种训练课程（领域内、混合基准、领域外），并使用GRPO算法在单GPU环境下进行调整。

Result: 混合课程在双基准上取得最佳整体F1；领域外课程虽整体性能弱，但提升了时间推理能力；不同类型问题的性能差异远超整体平均差异。

Conclusion: 课程组成是精细调控特化策略的杠杆，而非均匀缩放因子；单GPU下的GRPO需过滤格式噪声并采用连续奖励函数。
...
### [142] [Model Collapse as Cultural Evolution](https://arxiv.org/abs/2605.23054)
*Dongxin Guo,Jikun Wu,Siu Ming Yiu*

Main category: cs.CL

TL;DR: 本文将模型崩溃重新定义为文化传播现象，通过迭代学习理论解释了语言结构退化的顺序与原因，并基于LLaMA-2-7B和Mistral-7B的实验验证了五个可证伪预测，关键发现包括组合性的非单调变化以及压缩-交流权衡的首次大规模证据。


<details>
  <summary>Details</summary>
Motivation: 现有对模型崩溃的统计描述缺乏语言学的解释，尤其是哪些语言结构会退化、以何种顺序及原因退化尚不明确，因此需要从文化进化角度提供理论和实验依据。

Method: 基于文化进化中的迭代学习理论推导出五个可证伪预测（包括唯一区分性的预测），并对LLaMA-2-7B和Mistral-7B进行跨语言（英语、德语、土耳其语）的10代自训练实验，结合不同过滤条件和种子数据设置进行验证。

Result: 所有预测均获得支持，效应量大（Hedges' g > 1.6, BF10 > 100）。关键发现：组合性在无过滤自训练中呈非单调变化（先升后降）；该模式在最大规律种子数据下仍存在；仅任务导向过滤能维持该模式，随机过滤不能；LLM正则化梯度与人类行为数据高度吻合（R² = 0.94）。

Conclusion: 模型崩溃可被重新解释为文化传播现象，其退化模式符合迭代学习理论；研究为自训练管道设计提供了具体原则，并强调了压缩-交流权衡的关键作用。
...
### [143] [DFKI-MLT at SemEval-2026 TASK 7: Steering Multilingual Models Towards Cultural Knowledge](https://arxiv.org/abs/2605.23069)
*Yusser Al Ghussin,Daniil Gurgurov,Yasser Hamidullah,Josef van Genabith,Cristina España-Bonet,Simon Ostermann*

Main category: cs.CL

TL;DR: 本文提出一种无需参数更新的激活引导方法，通过从FLORES数据中提取语言向量，在推理时调整多语言大模型的文化知识，在SemEval-2026任务7的多选题中达到86.96%准确率（排名第7/17），并发现该方法对文化推理的提升存在层敏感性和语言-区域差异。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在不同语言和文化背景下的文化知识分布不均匀，需要更有效的推理时文化适应方法。

Method: 利用并行FLORES数据提取语言向量，在选定的Transformer层向残差流添加语言特定引导向量，实现推理时文化适应，无需参数更新。

Result: 在官方多选题轨道中达到86.96%准确率（排名第7/17）；后验分析显示引导效果因层、语言-区域对和提示设计而异，有时甚至降低性能。

Conclusion: 提示设计和激活引导需联合优化以实现多语言模型的文化感知推理。
...
### [144] [The Efficiency Frontier: A Unified Framework for Cost-Performance Optimization in LLM Context Management](https://arxiv.org/abs/2605.23071)
*Binqi Shen,Lier Jin,Hanyu Cai,Lan Hu,Yuting Xin*

Main category: cs.CL

TL;DR: 提出了一个统一的效率前沿框架，用于同时优化LLM上下文管理的成本和性能。


<details>
  <summary>Details</summary>
Motivation: 现有上下文缩减方法（如检索和内存压缩）通常独立评估性能和效率，缺乏系统性比较和部署感知决策支持。

Method: 将上下文策略选择建模为一个联合考虑任务性能、token成本和预处理复用的部署感知优化问题，并采用摊销成本模型进行决策分析。

Result: 在5,000个HotpotQA实例上，部署感知优化在相似性能（F1≈0.78）下减少约25%的有效token使用，在高性能场景下摊销内存压缩相比全上下文提示降低超过50%的token成本。

Conclusion: 该框架为评估和部署可扩展、高效且可持续的LLM系统提供了原则性和实用性的基础。
...
### [145] [A Comparative Evaluation of Structural Topic Models and BERTopic for Short, Open-Ended Survey Responses](https://arxiv.org/abs/2605.23093)
*Yan Jiang,Sihong Liu,Philip A. Fisher*

Main category: cs.CL

TL;DR: 比较了STM和BERTopic在开放式调查回复分析中的表现，发现BERTopic主题一致性更高，STM更适合推断性协变量分析。


<details>
  <summary>Details</summary>
Motivation: 现有主题建模评估多基于长且干净的基准语料，缺乏对短、开放式调查回复的指导。

Method: 对比STM（3种条件）和BERTopic（5种条件），引入上下文增强策略，评估拼写纠正、词干提取、嵌入选择和上下文增强的影响。

Result: BERTopic主题一致性更高，上下文增强提升最大；高维嵌入未提高一致性反而导致数据丢失；STM主题更宽泛混合。

Conclusion: STM和BERTopic各有优势，应结合使用，提供了实际选择建议。
...
### [146] [A Fine-Tuned BERT Classifier for Personal-Letter Titles in Late-Ming and Early-Qing Collected Works](https://arxiv.org/abs/2605.23103)
*Queenie Luo*

Main category: cs.CL

TL;DR: 本文提出Lepton，一个基于BERT的微调分类器，用于区分中国古典文集目录中的标题是私人信件还是容易混淆的序文（尤其是赠序）。


<details>
  <summary>Details</summary>
Motivation: 大规模从古典文集中识别书信，以支持中国传记数据库（CBDB）的明代书信平台建设。

Method: 在bert-base-chinese模型上进行微调，使用了5438个来自33位晚明至清初文人的手工标注文集标题。

Result: 模型已在Hugging Face上部署，成功从中明至清初的文集中识别出约五万五千封信件。

Conclusion: Lepton有效实现了古典文集标题中书信与序文的自动分类，为历史文献数字化研究提供了实用工具。
...
### [147] [As X, Do Y: How Persona and Task Combine in Instruction-Tuned LLMs](https://arxiv.org/abs/2605.23147)
*Eric Xu*

Main category: cs.CL

TL;DR: 角色提示在残差流中呈现局部可加性，但无法压缩为单个缓存残差向量，全局生成依赖分布式机制。


<details>
  <summary>Details</summary>
Motivation: 探索角色提示（如“作为X，执行Y”）在模型内部的计算结构，以及这种结构是否支持压缩。

Method: 在残差流的“提示到答案”转换点（最后一个提示令牌和前两个生成令牌）进行线性分解，分析角色和任务的附加方向；通过缓存残差注入测试压缩可行性。

Result: 角色和任务在局部表现为正交可加性，但注入缓存的残差预测无法复现长角色目标的完整生成行为。

Conclusion: 残差流局部可加性不意味着提示可压缩；角色提示的行为依赖于分布式提示/KV机制，而非单一残差点。
...
### [148] [When Symptoms Are Not Enough: Evidence-Weighting Patterns in Large Language Model Psychiatric Screening](https://arxiv.org/abs/2605.23148)
*Jianfeng Zhu,Megan Korhummel,Ruoming Jin,Karin G. Coifman*

Main category: cs.CL

TL;DR: 本文提出SCID锚定基准，评估五种大语言模型在精神病筛查中的表现，发现模型准确率0.49-0.86，但存在因保护性因素而忽略症状证据的偏差。


<details>
  <summary>Details</summary>
Motivation: 精神卫生需求超出临床评估能力，需可扩展筛查工具；大语言模型可能从患者叙事识别风险，但诊断可靠性、亚组差异和证据使用模式尚不明确。

Method: 构建555个半结构化体验访谈的SCID锚定基准，使用零样本人群特定提示评估五种SOTA大语言模型，分析错误类型、亚组差异及证据整合模式。

Result: GPT-4.1 Mini和GPT-5 Mini表现最稳定；抑郁症分类男性优于女性；假阴性焦虑和PTSD常含症状证据但伴随功能保留或保护性因素；功能损害证据促进正向分类，保护性证据抑制分类。

Conclusion: 大语言模型可支持可扩展精神病筛查，但倾向于在功能保留或保护性背景下忽略症状证据，临床部署前需严格验证。
...
### [149] [Same Model, Different Weakness: How Language and Modality Reshape the Jailbreak Attack Surface in Frontier MLLMs](https://arxiv.org/abs/2605.23157)
*Casey Ford,Madison Van Doren,Sicheng Jin,Emily Dix*

Main category: cs.CL

TL;DR: MLLM对齐失败依赖于语言和模态的交互机制，跨语言测试揭示了安全排名的非传递性。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估将语言和模态视为独立维度，无法反映MLLM攻击面的真实结构。

Method: 对比美式英语和墨西哥西班牙语，使用363个对抗提示对4个前沿MLLM进行文本/多模态测试，收集52272个人工评分。

Result: 语言不统一放大脆弱性：西班牙语使角色扮演攻击降低，但视觉多模态攻击更有效；Qwen Omni在西班牙语中反超Pixtral Large成为最易攻击模型。

Conclusion: 安全评价框架必须重新设计，以处理语言-模态的耦合机制。
...
### [150] [Fast-dDrive: Efficient Block-Diffusion VLM for Autonomous Driving](https://arxiv.org/abs/2605.23163)
...
TL;DR: 提出了Fast-dDrive，一种块扩散VLA模型，通过结构令牌冻结、分节感知训练、支架推测解码和测试时缩放，在保证轨迹规划质量的同时实现高效推理，显著提升速度与精度。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶VLA模型在轨迹规划保真度和推理效率之间难以平衡：自回归模型受限于边缘硬件的内存带宽和暴露偏差，全序列扩散模型无法复用KV缓存且存在逻辑泄漏问题。

Method: 采用块扩散VLA，在语义单元内进行双向优化并强制跨单元因果顺序；冻结结构令牌形成分节支架；引入分节感知训练和支架推测解码；通过从共享前缀KV缓存分叉N条随机轨迹并平均来实现低开销测试时缩放。

Result: 在WOD-E2E上取得SOTA的ADE@3s和ADE@5s以及最高RFS；在nuScenes上将平均L2误差降至0.32米（改进22%）；与SGLang集成后，吞吐量比自回归基线提升12倍。

Conclusion: Fast-dDrive重新定义了驾驶代理的速度-精度边界，缩小了高容量VLA与实时车载部署效率要求之间的差距。
...
### [151] [Self-Improving In-Context Learning](https://arxiv.org/abs/2605.23180)
*Baturay Saglam,Dionysis Kalogerias*

Main category: cs.CL

TL;DR: 提出通过测试时优化小样本提示的连续嵌入来改进上下文学习，无需微调或生成输出。


<details>
  <summary>Details</summary>
Motivation: 模型从演示中推断任务的能力缺乏直接反馈信号，需要自监督校准方法。

Method: 利用模型对演示输出的对数概率作为置信度代理，通过零阶优化优化提示嵌入。

Result: 在多种分类和生成任务上匹配或超越基线，与下游精度提升显著相关。

Conclusion: 代理优化为上下文学习提供了可靠的自监督信号，适用于多种任务。
...
### [152] [Positional Failures in Long-Context LLMs: A Blind Spot in Reasoning Benchmarks](https://arxiv.org/abs/2605.23170)
*Chuyifei Zhang,Hongyu Cui,Xiaowen Huang,Jitao Sang*

Main category: cs.CL

TL;DR: 现有长上下文推理基准在评估时未控制任务位置、填充内容和上下文长度。提出的Context Rot Evaluation (CRE)框架发现，当目标任务从末尾移到中间时，模型性能急剧下降，且随上下文长度增加而恶化，主要错误模式是填充答案干扰。


<details>
  <summary>Details</summary>
Motivation: 主流推理基准缺乏对长上下文中任务位置、填充内容和长度的联合控制，导致评估存在结构漏洞。

Method: 提出Context Rot Evaluation (CRE)框架，控制任务位置、填充内容和上下文长度三个变量；使用GSM8K和ARC-Challenge评估9个LLM；通过诊断探针和错误分析验证位置影响。

Result: 模型在中间位置准确率显著下降（如MiMo-v2-Flash在64K下下降88pp）；较新模型改进明显但中间位置问题仍存在；错误分析显示76%的中间位置错误与填充文本匹配。

Conclusion: 当前推理基准设计存在评估结构漏洞，无法测量随上下文长度增长的位置脆弱性，建议在评估中控制任务位置。
...
### [153] [Hidden Human-Like Nature of Machine-Generated Texts: Theory and Detection Enhancement](https://arxiv.org/abs/2605.23190)
*Chenwang Wu,Yiu-ming Cheung,Bo Han,Defu Lian*

Main category: cs.CL

TL;DR: 本文揭示了机器生成文本中隐藏的类人片段，并提出了一个模型无关的堆叠增强框架，通过硬EM算法迭代过滤这些片段，从而提升了现有检测器的性能。


<details>
  <summary>Details</summary>
Motivation: 现有段落级检测方法将机器文本视为完全机器化，忽略了其中可能存在的类人片段，导致检测难度增加。

Method: 提出模型无关的堆叠增强框架，将跨度级保留决策建模为隐变量问题，使用硬EM算法迭代过滤类人片段并优化检测器。

Result: 实验表明该框架在多种LLM和场景下一致提升了现有检测器性能，且支持无训练部署。

Conclusion: 通过减少隐藏类人片段的影响，有效提高机器生成文本检测的准确性和鲁棒性。
...
### [154] [Convergence Without Understanding: When Language Models Agree on Representations but Disagree on Reasoning](https://arxiv.org/abs/2605.23315)
*Muhammad Usama,Dong Eui Chang*

Main category: cs.CL

TL;DR: 大型语言模型的表征趋同反映的是共享的输入处理约束，而非共享的推理策略。


<details>
  <summary>Details</summary>
Motivation: 验证表征趋同是否扩展到推理过程。

Method: 评估16个语言模型在800个推理问题上的表征相似性，按难度、计算阶段和因果相关性分层。

Result: 发现难度倒置、生成差距和附带正确性三种分离现象。

Conclusion: 表征趋同源于输入处理约束，对集成设计、可解释性迁移和模型相似性评估有直接影响。
...
### [155] [AraHopeCorpus: Annotation Guidelines and Dataset for Hope Speech in Arabic Social Media Crisis Discourse](https://arxiv.org/abs/2605.23325)
*Esra'a Sharqawi,Wajdi Zaghouani*

Main category: cs.CL

TL;DR: 首次构建了阿拉伯语希望语料库AraHopeCorpus，涵盖加沙战争期间10万条YouTube评论，标注为希望、无希望和中性三类。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注仇恨言论和错误信息，缺乏对阿拉伯语社交媒体中积极表达（如希望、韧性）的系统性分析。

Method: 采用细粒度标注框架，将评论分为三类，并计算Cohen's Kappa评估一致性；对比人类标注与ChatGPT的差异。

Result: 希望言论占比超64%（宗教鼓励、集体团结为主），无希望言论占13%，人类标注一致性达0.71，但方言和隐喻增加了标注难度。

Conclusion: AraHopeCorpus为研究阿拉伯语社交媒体中的建设性话语、韧性传播和危机沟通提供了重要资源。
...
### [156] [When Is Next-Token Prediction Useful? Marginalization, Ergodicity, Mixture Identifiability, Local Sufficiency, RAG, Tools, and Programming](https://arxiv.org/abs/2605.23278)
*Francesco Corielli*

Main category: cs.CL

TL;DR: 本文区分了三种语言过程：完全条件语言过程、边际纯文本过程和模型诱导分布，并论证了语言模型训练的有效性依赖于平稳性、代表性和遍历性假设，以及上下文的充分统计性。


<details>
  <summary>Details</summary>
Motivation: 现有描述将语言模型视为学习给定前文的下一个token的条件分布，但这忽略了实际中模型只观察到采样的续文，且语言生成还受非文本环境（如事实、意图、社会背景等）影响，导致概念混淆。

Method: 通过信息论和统计理论，正式区分三种语言对象，分析条件互信息，并扩展到异构语料库；最后将RAG和工具使用解释为条件充分性设备。

Result: 边际纯文本律仅在观测前缀是潜在环境的近似充分统计量时有用，且需要残留条件互信息较小；异构语料库下假设更不成立。

Conclusion: 语言模型训练不能简单视为估计边际纯文本分布，而需关注条件充分性；RAG和工具使用可帮助实现条件充分性。
...
### [157] [Emotion Recognition in Sign Language Conversation](https://arxiv.org/abs/2605.23328)
*Yusong Wang,Keyu Mao,Takao Obi,Minghao Shao,Kotaro Funakoshi*

Main category: cs.CL

TL;DR: 本文提出了手语对话情感识别数据集eJSL Dialog，并验证了现有通用多模态对话情感模型在手语领域存在域差距。


<details>
  <summary>Details</summary>
Motivation: 现有手语情感数据集缺乏对话上下文，导致模型在真实场景中性能下降。

Method: 基于STUDIES语料构建包含1920个视频、480个对话的eJSL Dialog数据集，并用多种模型进行基准测试。

Result: 通用多模态对话情感模型在手语上存在域差距，需特定于手语的上下文感知视觉提取器。

Conclusion: 需要扩展手语对话数据集规模以支持大规模预训练，并开发手语专用视觉提取器。
...
### [158] [ClimateChat-300K: A Multi-Modal Facebook Dataset for Understanding Diverse Perspectives in Climate Communication](https://arxiv.org/abs/2605.23326)
*Wajdi Zaghouani,Md. Rafiul Biswas,Mabrouka Bessghaier,Shimaa Ibrahim,George Mikros*

Main category: cs.CL

TL;DR: 我们构建了 ClimateChat-300K 数据集，包含 299,329 条 2020-2024 年间的气候变化相关 Facebook 帖子，涵盖 41 个元数据特征，并通过主题建模和情感分析揭示了政策、行动等五大领域中的十个主题，发现情感语气、帖子格式和页面身份显著影响用户互动。


<details>
  <summary>Details</summary>
Motivation: 现有气候变化传播研究缺乏大规模、多来源、结构化的社交媒体数据集，难以支持对公众话语的纵向分析和跨学科复现研究。

Method: 通过 CrowdTangle 平台收集帖子数据，使用主题建模（如 LDA）提取十大主题并归类为政策、行动、合作、科学和保护五大领域，结合情感分析评估帖子情感倾向，并统计分析互动指标。

Result: 视觉丰富和情感强烈的帖子获得最高互动；在线讨论内容随气候峰会、新冠疫情等重大事件动态演变；数据集支持对极化、错误信息等议题的研究。

Conclusion: ClimateChat-300K 为可重复、跨学科的气候传播研究提供开放资源，有助于理解公众在气候变化议题上的参与如何随时间和情境变化。
...
### [159] [Metacognition as Reward: Reinforcing LLM Reasoning via Knowledge and Regulation Signals](https://arxiv.org/abs/2605.23384)
*Sirui Chen,Lei Xu,Yuying Zhao,Yutian Chen,Yu Wang,Beier Zhu,Hanwang Zhang,Shengjie Zhao,Chaochao Lu*

Main category: cs.CL

TL;DR: 提出基于元认知的奖励框架（MaR），通过任务知识覆盖、调节保真度和答案正确性三个维度优化推理轨迹，在22个基准测试上平均提升7.7%至11.0%。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法只关注最终答案，缺乏对中间推理过程的指导；Rubrics-as-Reward方法需要实例级规则，设计成本高。

Method: 将模型输出分解为元认知知识（识别任务信息）和元认知调节（规划调整推理过程），使用轨迹级奖励同时优化知识覆盖、调节保真度和答案正确性。

Result: Qwen3.5-9B+MaR在整体平均上超越GPT-OSS-120B，多个独立基准优于更强模型，过程分析表明推理质量显著提升。

Conclusion: MaR通过通用元认知维度扩展奖励信号到推理轨迹，无需实例级规则即可提升推理质量和模型泛化能力。
...
### [160] [Cultural Adaptation in Large Language Models for Political Discourse](https://arxiv.org/abs/2605.23332)
*Wajdi Zaghouani*

Main category: cs.CL

TL;DR: 本文提出文化适应性是大语言模型在政治话语分析中可信部署的前提，并形式化翻译、话语和本体层面的适应框架。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多语言、多制度背景下的政治交流中存在系统性错误，民主问责面临风险。

Method: 通过文化适应性框架（翻译、话语、本体）、文化失效模式识别及评估矩阵（忠实度、校准、民主安全），结合参与式数据集、文化感知迁移学习和基准设计。

Result: 确立了文化适应性在政治NLP中的可测量路径，并提出了治理约束与民主合法性的条件。

Conclusion: 文化适应性政治NLP在特定范围内可支持民主合法性，但需明确治理限制。
...
### [161] [SSDAU: Structured Semantic Data Augmentation for Joint Entity and Relation Extraction](https://arxiv.org/abs/2605.23440)
*Jiawei He,Mengyu Shi,Chunrong Fang*

Main category: cs.CL

TL;DR: 提出结构化语义数据增强方法SSDAU，保持文本语义结构，提升联合实体关系抽取模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法忽视文本相关性，破坏语义结构和依赖，导致模型泛化能力弱。

Method: 按实体标签切分文本，利用编码器捕获实体语义特征，通过实体语义重构生成增强数据，融合上下文嵌入与相似度分数，并应用BERTTopic过滤无关主题。

Result: SSDAU生成语义一致的数据，鲁棒性优于基线（F1下降8.26% vs 31.91%），所有指标显著超越现有方法。

Conclusion: SSDAU能有效生成高质量增强数据，显著提升JERE模型泛化能力和鲁棒性。
...
### [162] [EquiSumm : A Gender Bias-Aware Framework for Inclusive Tweet Summarization](https://arxiv.org/abs/2605.23412)
*Chaitanya Wanjari,Jessica Kamal,Riddhi Jain,Samruddhi Kurhe,Roshni Chakraborty*

Main category: cs.CL

TL;DR: 提出EquiSumm，一种考虑性别公平性的社交媒体摘要生成方法


<details>
  <summary>Details</summary>
Motivation: 现有社交媒体摘要方法未显式考虑人口统计公平性，可能导致摘要偏颇

Method: EquiSumm，在摘要生成中纳入性别维度

Result: 在两个主要数据集上，EquiSumm性能优于现有方法

Conclusion: 考虑性别公平性能有效改善社交媒体摘要的代表性
...
### [163] [From Correctness to Preference: A Framework for Personalized Agentic Reinforcement Learning](https://arxiv.org/abs/2605.23382)
*Ranxu zhang,zeyang li,Jiacheng Huang,Rui Zhang,Xiaozhou Xu,sun zhe,Yanyong Zhang,Chao Wang*

Main category: cs.CL

TL;DR: 提出一个统一的个性化智能体强化学习框架，通过解耦通用奖励与个性化偏好奖励，结合偏好解耦奖励模型和偏好对齐技能进化图记忆，实现用户条件化行为建模。


<details>
  <summary>Details</summary>
Motivation: 现有智能体强化学习依赖通用任务信号，无法处理异构用户偏好，且行为数据受从众效应干扰，平铺记忆不支持个性化技能检索。

Method: 核心是PARPO算法，解耦通用任务质量奖励与个性化偏好奖励，并使用用户特定锚点稳定训练；同时提出两阶段偏好解耦奖励模型和PSGM用于个性化监督与技能检索。

Result: 在ETAPP、ETAPP-Hard和SJAgent数据集上，框架持续超越强记忆和强化学习基线方法。

Conclusion: 通过偏好识别、策略优化和结构化技能积累的闭环，本框架有效解决了用户条件化行为建模中的个性化问题。
...
### [164] [Naturalistic measure of social norms alignment](https://arxiv.org/abs/2605.23420)
*Yevhen Kostiuk,Kenneth Enevoldsen,Peter Bjerregaard Vahlstrup,Márton Kardos,Kristoffer Nielbo*

Main category: cs.CL

TL;DR: 提出一种通过解决方案匹配衡量自然自由形式下社会规范对齐度的框架和两个度量指标，并构建了丹麦语社会困境数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工封闭式评估，无法测量自然情景下的社会规范对齐。

Method: 通过解决方案匹配框架，使用声明和显式一致性精度两个指标，由三名评审员提供参考解决方案。

Result: 指标产生一致的模型排名，不同类型困境的一致性存在差异，如邻里冲突和共享生活话题一致性更高。

Conclusion: 该框架和数据集可用于研究自然开放式对话中的文化扎根社会推理。
...
### [165] [Articulatory strategy as a source of variation in acoustic vowel dynamics](https://arxiv.org/abs/2605.23416)
*Patrycja Strycharczuk,Justin J. H. Lo,Sam Kirkham*

Main category: cs.CL

TL;DR: 本研究发现，发音策略（如舌形）会系统性影响声学元音动态，具体表现为颚化双元音中发音位移和速度的改变导致共振峰过渡的特征性变化。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明声学元音动态具有说话人识别特征，但这些特征是否源于不同的发音策略尚缺乏证据，本研究旨在通过超声舌像数据验证两者之间的系统关联。

Method: 采集36名北盎格鲁英语说话者的超声舌像数据，识别其舌形策略（特别是/i/元音的发音），并分析舌形对双元音（含颚化滑移）共振峰动态的预测作用。

Result: 舌根/舌背的更大发音位移会产生颚化元音的舌形偏差，同时需要更高发音速度，导致共振峰转换更早、更陡峭；舌形是共振峰动态的显著预测因子。
...
### [166] [ARES: Automated Rubric Synthesis for Scalable LLM Reinforcement Learning](https://arxiv.org/abs/2605.23454)
*Xiaoyuan Li,Keqin Bao,Moxin Li,Yubo Ma,Yichang Zhang,Wenjie Wang,Fuli Feng,Dayiheng Liu*

Main category: cs.CL

TL;DR: 提出ARES框架，自动生成问题相关的加权评分表，实现大规模可扩展的基于评分表的强化学习，在开放领域任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分表的强化学习依赖专家编写的评分表和人工构建的问题集，且固定任务级评分表无法捕捉单个问题的评估需求。

Method: 从预训练文档中自动生成自包含的问答对和问题特定的加权评分表，利用域标签和人物画像提高多样性，并通过验证过滤器保证质量。

Result: 在七个基准测试中，ARES训练的评分表强化学习优于持续预训练、监督微调和二进制奖励强化学习，尤其在医疗和指令遵循等开放任务上提升明显。

Conclusion: ARES能够高效自动构建大规模评分表数据，显著提升开放领域任务的强化学习效果。
...
### [167] [Asking For An Old Friend: Diagnosing and Mitigating Temporal Failure Modes in LLM-based Statutory Question Answering](https://arxiv.org/abs/2605.23497)
*Max Prior,Andreas Schultz,Matthias Grabmair*

Main category: cs.CL

TL;DR: 本文针对法律大模型在时间敏感场景下的失效问题，构建了包含312个德国法律问答对的基准测试，发现标准RAG方法可显著提升模型性能，而网页搜索存在时间偏差。


<details>
  <summary>Details</summary>
Motivation: 法律具有动态演变特性，但现有大模型的静态训练数据无法适应法律修订，导致回答过时规则或出现时间偏好错误。

Method: 创建三类时间敏感法律问答的基准测试，评估五种大模型在无辅助、网页搜索和两种融合时间约束的RAG方法下的表现，并使用LLM评价器验证结果。

Result: 标准RAG方法在所有问题类型上均显著提升性能，网页搜索提升不稳定且对历史问题存在突出的近因偏差，无辅助模型在训练截止日期后表现严重退化。

Conclusion: 可靠的法律问答系统必须将时间有效性作为硬约束，而非依赖模型的静态参数知识。
...
### [168] [Structure-Guided Entity Resolution: Fine-Tuning LLMs for Robust Name Matching in Complex Linguistic Contexts](https://arxiv.org/abs/2605.23597)
*Shivam Chourasia,Hitesh Kapoor,Nilesh Patil*

Main category: cs.CL

TL;DR: 提出了结构引导实体解析（SGER）框架，通过两阶段课程学习微调大语言模型，在Indian身份数据上达到99.02%准确率，并成功部署于Dream11平台。


<details>
  <summary>Details</summary>
Motivation: 解决异构记录中人员姓名匹配的挑战，特别是在多语言、文化复杂、数据噪声大的环境（如KYC合规）下统一用户身份的需求。

Method: 采用两阶段课程学习：首先训练模型解析姓名的语法和语义结构，然后优化二值实体匹配任务。

Result: 在50000对真实世界测试集上达到99.02%准确率和0.994的F1分数，优于GPT-4o少量提示和单阶段微调基线。

Conclusion: 课程引导训练实现了鲁棒、高精度的实体解析，能够在大规模多语言生产系统中有效部署。
...
### [169] [Benchmarking Google Embeddings 2 against Open-Source Models for Multilingual Dense Retrieval and RAG Systems](https://arxiv.org/abs/2605.23618)
*Stefano Cirillo,Domenico Desiato,Giuseppe Polese,Giandomenico Solimando*

Main category: cs.CL

TL;DR: 对Google嵌入与五种开源模型的基准测试表明，GE2在所有任务中排名第一，但速度显著较慢；mE5-L在速度与性能间取得良好平衡；LaBSE在检索任务上表现不佳。分块实验显示32个令牌时性能饱和。


<details>
  <summary>Details</summary>
Motivation: 评估Google嵌入相较于主流开源模型在检索任务中的性能、效率和分块敏感性，验证其商业化部署的合理性。

Method: 在BEIR子集、意大利语RAG数据集上对比模型；进行分块消融实验（5种大小×3种策略）；测量CPU延迟。

Result: GE2所有任务最优（BEIR nDCG@10=0.638），但延迟比最快模型高14倍；mE5-L以31ms延迟达到接近性能；LaBSE得分最低（0.188）。分块32令牌后所有模型性能饱和。

Conclusion: GE2适合精度优先场景，mE5-L是延迟敏感任务的推荐选择；LaBSE不适合检索任务；分块策略在小块时有效，但32令牌后收益递减。
...
### [170] [How Human-Like Are Large Language Models? A Register-Aware Linguistic Evaluation Framework](https://arxiv.org/abs/2605.23651)
*Björn Nieth,Marianna Gracheva,Michaela Mahlberg,Bjoern Eskofier,Emmanuelle Salin*

Main category: cs.CL

TL;DR: 提出了一种上下文感知的评估框架，通过比较人类参考语料库和LLM生成文本的语言特征分布，使用最大均值差异（MMD）和Biber的67个词汇语法特征来衡量类人程度。实验表明，LLM总是偏离人类基线，但最接近人类的模型因语域而异，与模型大小无关。


<details>
  <summary>Details</summary>
Motivation: 现有LLM研究主要关注事实正确性和任务性能，但忽略了生成文本在语言层面上的类人程度，尤其是语境依赖性。

Method: 采用两样本问题框架，用MMD和Biber的67个特征比较人类参考语料库与LLM生成文本的语言特征分布，评估类人程度。

Result: 在7个指令微调开源模型和5个英语数据集上，所有LLM都偏离人类基线，但不同语域中最接近人类的模型不同，且不受模型大小影响。

Conclusion: LLM生成文本的类人程度具有语域依赖性，模型大小并非决定性因素，现有模型在模仿人类语言模式上仍有不足。
...
### [171] [OnePred: Next-Query Prediction via Recursive Intent Memory in Multi-Turn Conversations](https://arxiv.org/abs/2605.23668)
*Jiangwang Chen,Bowen Zhang,Zixin Song,Jiazheng Kang,Xiao Yang,Da Zhu,Guanjun Jiang*

Main category: cs.CL

TL;DR: 提出OnePred框架，通过递归更新记忆作为唯一的跨轮上下文，结合两阶段强化学习流水线，实现高效的多轮对话下一查询预测，并构建了NQP-Bench基准。


<details>
  <summary>Details</summary>
Motivation: 当前LLM对话系统在预测用户下一查询时面临效率-质量权衡：完整历史导致线性增长的token消耗，而截断最新轮次会丢失关键跨轮上下文。

Method: OnePred：维护递归更新的记忆作为唯一跨轮上下文；两阶段强化学习（先教预测内容，再教压缩策略）；构建NQP-Bench基准（三个不同子集）。

Result: 相比完整历史输入，每个轮次token消耗减少高达22倍，预测质量持续优于所有基线，在长对话中优势更明显。

Conclusion: 通过预测导向的意图链压缩，实现了不依赖原始历史的准确预测，显著提升效率且不牺牲性能。
...
### [172] [OpenSkillEval: Automatically Auditing the Open Skill Ecosystem for LLM Agents](https://arxiv.org/abs/2605.23657)
*Jiahao Ying,Boxian Ai,Wei Tang,Siyuan Liu,Yixin Cao*

Main category: cs.CL

TL;DR: 提出OpenSkillEval框架，自动评估技能增强的LLM代理系统和技能本身，发现技能可用性不等于有效使用，技能收益依赖模型和框架，许多流行技能并不优于无技能基线。


<details>
  <summary>Details</summary>
Motivation: 开源技能生态快速扩展，但缺乏对模型/框架与技能交互、技能质量评估及成本-性能权衡的系统研究。

Method: 从真实世界工件自动构建5类应用（演示生成、前端网页设计等）的600+动态任务实例，收集30个社区技能，在统一设置下对比评估SOTA模型和代理框架。

Result: 技能可用性不保证有效使用；技能增强收益强烈依赖底层模型和代理框架；许多流行技能无法持续超越无技能的基线代理。

Conclusion: 需要动态、任务导向的评估，为LLM代理的技能设计、选择和部署提供实践指导。
...
### [173] [ChartFI: Benchmarking Faithfulness and Insightfulness of Chart Descriptions from Multimodal Large Language Models](https://arxiv.org/abs/2605.23694)
*Fen Wang,Zekai Shao,Qiman Kang,Chunran Hu,Zhixuan Zhang,Lexu Xie,Chao Liu,Siming Chen*

Main category: cs.CL

TL;DR: 提出了ChartFI-Bench基准，包含896个图表-描述对，用于评估多模态大模型在图表描述中的忠实性和洞察力，并设计了四个评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有基准使用简单图表和浅层描述，且缺乏多维度评估指标，无法准确衡量模型描述质量。

Method: 定义了高质量描述的四个维度（事实准确性、突出特征强调、领域知情指导、图表文本互补性），构建基准数据集并设计四种评估指标（忠实性、覆盖度、信息量、敏锐度）。

Result: 实验揭示了主流多模态大模型在图表描述中的常见弱点，验证了框架的有效性。

Conclusion: ChartFI-Bench能系统评估图表描述质量，帮助识别模型不足，推动更准确和深入的图表描述生成。
...
### [174] [Metadata Predictability Is Not Evidence Dependence: An Intervention-Based Audit for Weak-Label Benchmarks](https://arxiv.org/abs/2605.23701)
*Kan Shao*

Main category: cs.CL

TL;DR: 本文提出了一种协议级测试方法，通过结合元数据先验主导分数、证据干预统计和读者强度校准，来评估弱标签基准测试的可靠性，并验证了综合审计的重要性。


<details>
  <summary>Details</summary>
Motivation: 元数据唯一捷径检查无法回答基准输出是否可被元数据先验预测的问题，需要更全面的测试方法。

Method: 采用元数据先验主导分数与证据干预统计ΔEvi，并通过合成HotpotQA、SNLI和FEVER等数据集进行实验，结合读者强度校准分析。

Result: 合成HotpotQA中MPDS中等（0.643）但ΔEvi为零；SNLI出现校准反转；FEVER在四种变压器上表现出强证据敏感性。

Conclusion: 基准审计应同时报告元数据筛选、证据干预和读者强度校准三个指标。
...
### [175] [A graph-based analysis of semantic types and coercion in contextualized word embeddings](https://arxiv.org/abs/2605.23710)
*Long Chen,Deniz Ekin Yavas*

Main category: cs.CL

TL;DR: 本文提出一种基于图的方法，利用BERT和增强词嵌入分析名词与上下文的语义类型匹配，通过NTP和NTE指标区分匹配与失配句子。


<details>
  <summary>Details</summary>
Motivation: 探索语义类型失配（即强制现象）中词汇与上下文类型信息在词嵌入中的体现。

Method: 从十种语义类型中选择名词，标注语料实例为匹配/强制/其他失配/无限制，构建基于BERT和语义增强嵌入的图，提出邻居类型概率（NTP）和邻居类型熵（NTE）指标。

Result: 语义增强嵌入构建的图能更好反映语义类型信息，且匹配与失配句子可通过提出指标区分。

Conclusion: 基于图的方法和嵌入技术可有效刻画名词与上下文的类型匹配关系，为强制现象提供分析工具。
...
### [176] [NLG Evaluation: Past, Present, Future](https://arxiv.org/abs/2605.23715)
*Ehud Reiter*

Main category: cs.CL

TL;DR: 本文综述了自然语言生成评估从1990年到2026年的演变历程，强调评估方法从语言学导向转向机器学习主导，并预测未来将更注重影响、质量和安全评估。


<details>
  <summary>Details</summary>
Motivation: 自然语言生成评估方法在三十多年间发生了巨大变化，需要系统梳理其发展脉络并展望未来趋势。

Method: 回顾了1990年以来NLG评估的历史进展，包括近期出现的LLM-as-Judge方法，并基于当前技术应用趋势进行前瞻性分析。

Result: 发现NLG评估从缺乏现代实验标准到依赖机器学习评估，技术不断演进；LLM-as-Judge成为最新代表方法。

Conclusion: 随着NLG技术被广泛使用，未来评估将更加注重实际影响、定性分析和安全性，推动评估体系持续发展。
...
### [177] [Is a Document Educational or Just Wikipedia-Style? -- Pitfalls of Classifier-Based Quality Filtering](https://arxiv.org/abs/2605.23721)
*Mateusz Klimaszewski,Piotr Andruszkiewicz*

Main category: cs.CL

TL;DR: 本文揭示分类器质量过滤（CQF）在预训练语料构建中的一个关键漏洞：简单的维基百科风格重格式化操作可以显著改变模型的质量评估，使低质量内容绕过过滤阈值。分析表明，FineWeb-Edu CQF模型约7%的文档过滤决策被逆转，导致本应排除的低质量内容进入预训练语料。


<details>
  <summary>Details</summary>
Motivation: 探索分类器质量过滤方法在大型语言模型预训练语料构建中的可靠性，并揭示其潜在弱点。

Method: 对文档进行维基百科风格的重格式化操作，然后评估分类器模型（如FineWeb-Edu）的质量打分变化。

Result: 约7%的文档经过重格式化后，模型的质量评估结果发生反转，使得低质量内容被误认为高质量而通过过滤。

Conclusion: 分类器质量过滤存在表面形式依赖的漏洞，需要更鲁棒的评估方法以防止低质量内容混入预训练语料。
...
### [178] [Multilingual Knowledge Transfer under Data Constraints via Lexical Interventions](https://arxiv.org/abs/2605.23885)
*Anastasiia Sedova,Natalie Schluter,Skyler Seto,Maartje ter Hoeve*

Main category: cs.CL

TL;DR: 提出了LINK方法，通过在高资源语言预训练数据中进行词汇替换来改善跨语言知识迁移，无需大量并行数据或额外模型。


<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏训练数据，现有跨语言迁移方法依赖大量并行数据、翻译系统或额外训练，不适用于许多语言。

Method: 使用双语词典随机替换高资源语言（英语）预训练数据中的部分词汇为词级翻译，无需额外模型训练。

Result: 在8种语言和5种模型规模上，目标语言的下游任务性能显著提升，训练速度最高提升2倍。

Conclusion: LINK是一种低成本、高效的数据级干预方法，能有效改善低资源语言的跨语言知识迁移。
...
### [179] [Hierarchical Concept Geometry in Language Models Emerges from Word Co-occurrence](https://arxiv.org/abs/2605.23821)
*Andres Nava,Matthieu Wyart*

Main category: cs.CL

TL;DR: 论文提出了一个分布性理论，解释超名关系在语言表示中的几何编码方式，证明词共现统计的频谱结构自然产生了层次化的概念几何。


<details>
  <summary>Details</summary>
Motivation: 现有研究观察到语言表示中存在层次概念几何，但缺乏理论解释其来源，特别是是否源于层级特定的功能机制。

Method: 基于WordNet超名图的共现频率假设，通过数学分析嵌入Gram矩阵的频谱，利用共现核的正性和衰减条件证明特征向量的层次分裂几何。

Result: 在word2vec嵌入中验证了层次分裂几何的预测，并在Gemma 2B模型的解嵌入中观察到类似特征。

Conclusion: 大型语言模型中的层次概念几何并非源于层级特定的机制，而是从成对词统计的频谱结构中自然涌现。
...
### [180] [Remote Teleoperation of Endovascular Intervention Robots: A Systematic Review](https://arxiv.org/abs/2605.22889)
...
TL;DR: 综述评估了远程操作血管内介入机器人的技术可行性、通信基础设施和临床效果，发现16项研究中机器人系统可在7000公里内以30-163毫秒延迟完成操作，但多数证据来自动物或模型，未来需在低收入国家及多中心临床试验中验证。


<details>
  <summary>Details</summary>
Motivation: 减少临床医生的辐射暴露和身体劳损，并将专科血管护理扩展到偏远地区，特别是针对急性卒中机械取栓等时间敏感干预。

Method: 按照PRISMA指南，从2501篇初筛文献中纳入16项符合标准的研究，分析远程操作血管内机器人系统。

Result: 机械或电磁驱动的导管/导丝可在7000公里内操控，网络延迟30-163毫秒；小规模人体试验中手术成功率100%，但多数证据来自动物或模型。

Conclusion: 远程操作血管内介入可减少职业危害、扩大患者获取紧急手术的机会并优化资源配置，但需在中低收入国家开展研究，并通过多中心临床试验验证安全性和普适性。
...
### [181] [Extending Deep Event Visual Odometry with Sparse Point-Cloud Export](https://arxiv.org/abs/2605.22890)
*Alireza Safdari,Sajad Ashraf*

Main category: cs.RO

TL;DR: 该项目在DEVO基础上增加了稀疏点云导出管道，生成显式点云表示，并通过BOARD SLOW序列验证了其在局部一致性和高精度方面的表现，同时指出了密度、完整性和累积噪声的限制。


<details>
  <summary>Details</summary>
Motivation: 为DEVO的稀疏点云导出提供可视化与进一步处理能力，扩展其应用场景。

Method: 在不修改核心里程计公式的前提下，利用DEVO内部估计的3D结构导出稀疏点云，并实现数据导出、格式转换和点云清理的工作流。

Result: 在BOARD SLOW序列上，导出稀疏点云与EMVS重建局部一致，5厘米阈值下精度高，但密度、完整性和对累积噪声敏感存在限制。

Conclusion: 该扩展保持了原有视觉里程计流程，实现了稀疏几何场景输出，但需改进密度和抗噪性。
...
### [182] [Agentic-VLA: Efficient Online Adaptation for Vision-Language-Action Models](https://arxiv.org/abs/2605.22896)
*Ruofan Jin,Zaixi Zhang*

Main category: cs.RO

TL;DR: 提出Agentic-VLA框架，通过自适应奖励合成、语言引导探索和经验记忆三大创新，显著提升VLA模型在新环境中的泛化能力和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在泛化到新环境和训练效率方面存在严重不足，需要大量演示数据。

Method: 自适应奖励合成（动态调整奖励函数并分解任务）、语言引导探索（评论模型提供结构化探索指导）、经验记忆（存储策略权重以快速适配相似任务）。

Result: 在LIBERO基准上长期任务+12.3%、单样本学习+28.5%、跨任务迁移从0%升至31.2%，收敛速度提升2.4倍；在双臂RoboTwin 2.0（含Hard模式）上保持优势。

Conclusion: Agentic-VLA是实现真正自适应VLA系统（能在部署中持续学习）的重要一步。
...
### [183] [Robots That Know What to Ask: Recovering Misaligned Rewards through Targeted Explanations](https://arxiv.org/abs/2605.22986)
*Helena Merker,Nick Walker,Andreea Bobu*

Main category: cs.RO

TL;DR: 提出了一种通过检测演示中未充分指定的特征并主动请求针对性纠正演示的框架，以解决奖励函数学习中的模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 现有演示学习假设演示对所有特征提供充分监督，但实际演示常因认知负荷、物理难度或训练不足导致关键特征未充分指定，引发学习奖励函数模糊和行为偏差。

Method: 利用演示中特征一致性统计信号（优化充分的特征变化小，未指定特征变化大）推断未充分演示的特征，通过自然语言解释不确定性并请求针对性演示填补空白。

Result: 在模拟桌面操作和真实Franka机器人用户研究中，针对性解释引导的查询相比随机查询和被动数据收集显著提高了奖励恢复效果，减少了模糊性。

Conclusion: 该框架能有效检测和修正演示中的特征未指定问题，提升奖励函数学习的鲁棒性。
...
### [184] [$π_0$-EqM: Equilibrium Matching for Closed-Loop Vision-Language-Action Control](https://arxiv.org/abs/2605.23128)
*Huanming Liu,Congsheng Xu,Jianmin Ji,Yao Mu*

Main category: cs.RO

TL;DR: 提出了π0-EqM模型，用平衡匹配解码器替代π0中的流匹配专家，在不改变上游VLA架构的情况下提升任务成功率，并揭示了推理深度与任务性能之间的非单调关系。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的流匹配动作解码器使用固定采样步长，限制了状态依赖的计算和跨控制周期的时序复用，缺乏灵活性。

Method: 在π0框架中，用平衡匹配解码器替换流匹配专家，保持上游VLA堆栈不变，通过能量视角优化动作生成。

Result: 在300步预算下，RoboTwin任务平均成功率从40.4%提升至50.2%，在LIBERO-10上表现突出（87.0%），并发现残差与成功率之间存在任务依赖的非单调关系。

Conclusion: VLA控制中的推理深度应视为策略设计的一部分，引入的能量基视角有助于未来跨任务和跨实施例的可组合动作生成研究。
...
### [185] [Semantic-Aware Guided Drone Exploration for Language-Conditioned 3D Indoor Mapping](https://arxiv.org/abs/2605.23160)
*Nitin Vegesna,Avideh Zakhor*

Main category: cs.RO

TL;DR: 提出SAGE系统，结合语义引导与覆盖探索，在未知3D室内环境中实现开放词汇物体发现，速度比FTU快9.0-25.9倍。


<details>
  <summary>Details</summary>
Motivation: 现有覆盖探索算法如FALCON侧重于最大化覆盖，缺乏利用语义线索优先寻找目标物体的能力。

Method: 在FALCON基础上集成CLIP，包括物体中心嵌入存储、时间缓存、物体前沿、统一语义-几何规划代价四个组件。

Result: 在Matterport3D仿真中，SAGE在物体发现上优于FALCON和纯语义消融版本；与FTU相比，平均速度提升13.7倍，且体积吞吐量更高。

Conclusion: SAGE在保持覆盖效率的同时，显著提升了物体发现性能，并在真实无人机上验证了可行性。
...
### [186] [Autonomous Frontier-Based Exploration with VLM Guidance](https://arxiv.org/abs/2605.23165)
*Aarush Aitha,Avideh Zakhor*

Main category: cs.RO

TL;DR: 提出了一种利用视觉语言模型进行高层战略决策的自主机器人探索流水线，通过多模态提示选择最佳前沿路径，在仿真环境中将地图覆盖率提升24%。


<details>
  <summary>Details</summary>
Motivation: 传统机器人探索依赖简单几何启发式算法，缺乏对环境上下文的推理能力，无法高效应对未知和危险环境。

Method: 机器人决策时生成多模态提示（当前地图+前沿路径视觉图像），由VLM分析并选择最可能的前沿路径，替代传统几何启发式方法。

Result: 在六个室内仿真环境中，该流水线相比现有方法将地图覆盖率提升最高24%。

Conclusion: 该方法轻量级、无需训练、可迁移至任意标准传感器机器人，显著提升自主探索效率。
...
### [187] [Signal Temporal Logic Motion Planning via Graphs of Convex Sets](https://arxiv.org/abs/2605.23240)
*Yu Chen,Ancheng Hou,Mingyang Feng,Xiao Yu,Xiang Yin*

Main category: cs.RO

TL;DR: 提出了一种结合时间自动机与凸集图的框架，用于在连续时间内生成满足信号时序逻辑规范的光滑机器人轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时处理高层时序逻辑约束、低层运动约束以及轨迹光滑性要求。

Method: 将STL规范转化为时间自动机，与凸分解的配置空间耦合形成联合转移系统，再将问题转化为凸集图上的最短路径问题，通过求解得到光滑Bézier样条轨迹。

Result: 数值实验表明该方法能在低维基准、3D四旋翼、30自由度人形机器人和UR-3机器人上高效生成可执行的光滑轨迹。

Conclusion: 该方法在保证完备性的同时，计算复杂度随配置空间维度和Bézier次数多项式增长，适用于复杂STL运动规划问题。
...
### [188] [Turning Adaptation into Assets: Cross-Domain Bridging for Online Vision-Language Navigation](https://arxiv.org/abs/2605.23257)
*Zixuan Hu,Xuantuo Huang,Yancheng Li,Yichun Hu,Shengyong Xu,Ling-Yu Duan*

Main category: cs.RO

TL;DR: 提出IDEA框架，通过软提示与域坐标构建资产库和跨域桥接，实现VLN中无训练的环境适应，缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法将在线适应视为短暂孤立更新，导致灾难性遗忘和负迁移。

Method: 引入Fisher引导加权的软提示优化，结合域坐标形成动态资产库，通过目标域投影到历史知识凸包构建跨域桥接。

Result: 在REVERIE、R2R和R2R-CE基准上，IDEA一致优于现有方法，实现无训练适应。

Conclusion: 资产积累与组合的互补循环有效应对非平稳环境，为VLN部署提供稳定适应机制。
...
### [189] [6G Communication Networks Enabling Embodied Agents: Architecture and Prototype](https://arxiv.org/abs/2605.23263)
*Lipeng Dai,Luping Xiang,Kun Yang*

Main category: cs.RO

TL;DR: 本文研究了面向具身智能体的6G通信系统，提出了一种分层通信架构并构建了端到端原型系统，实验验证了毫秒级延迟和稳定闭环操作。


<details>
  <summary>Details</summary>
Motivation: 具身智能体对通信的严苛且异构的需求远超纯软件智能体，而现有研究缺乏系统性利用6G能力支持具身智能体通信的探讨。

Method: 从概念与工程双视角分析具身智能体与6G共生关系，提出包含人体意图感知层、O-RAN传输层、智能中介层和具身层的分层架构，并基于触觉设备、机械臂和5G O-RAN测试床搭建原型。

Result: 原型系统实现毫秒级延迟和稳定闭环保真度，验证了架构的可行性和实用性。

Conclusion: 该架构为未来6G-具身智能体研究和工业部署提供了参考，强调了具身智能体在增强通信网络覆盖、感知和物理世界理解方面的积极作用。
...
### [190] [Sparse Compositional Flow Matching by geometric assembly from motion primitives](https://arxiv.org/abs/2605.23341)
*Yan Tang,Yuanbo Tang,Tingyu Cao,Shaolun Huang,Yang Li*

Main category: cs.RO

TL;DR: 本文提出了一种基于流匹配框架的具身轨迹生成方法，通过显式学习可复用的运动基元词典并在物理轨迹空间中直接组合，在Open X-Embodiment和3DMoTraj数据集上实现了最先进的精度，将FDE/ADE比从1.8降至1.07。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型将具身轨迹视为密集整体信号，忽略了其潜在组成结构，导致样本效率低下。

Method: 采用运动基元词典学习（每个原子含可学习长度掩码和起始指示器）和结构稀疏流匹配（含几何约束损失）的双组件设计。

Result: 在Open X-Embodiment和3DMoTraj上达到SOTA，ADE提升19.2%，FDE提升21.0%。

Conclusion: 显式组合运动基元能有效提升轨迹生成的质量和效率。
...
### [191] [Multi-Floor Exploration for Ground Robots via an Incremental Reachable Graph and Structural Priors](https://arxiv.org/abs/2605.23350)
*Zhiwen Zhu,Jiaqi Chen,Xiangyi Huang,Meiqi Hu,Boyu Zhou*

Main category: cs.RO

TL;DR: 提出一种基于增量可达图的多楼层自主探索框架，解决传统地图无法表示重叠可通行表面问题。


<details>
  <summary>Details</summary>
Motivation: 地面机器人在多楼层建筑中自主探索时，传统2D/2.5D地图无法表示楼梯、坡道等重叠可通行表面。

Method: 构建增量可达图，通过稀疏观测保留潜在连接；投影任务区域先验至目标楼层初始化假设图，并随观测增量更新；层次化规划器联合推理确认与假设结构。

Result: 仿真中探索效率和地图完整性优于基线；真实实验验证了实用性与实时性。

Conclusion: 该框架能有效提升多楼层探索的稳定性和效率，兼具实际可行性。
...
### [192] [Droneulator: A Portable UAV Simulator for Agricultural Workflows with RotorPy and Godot 4](https://arxiv.org/abs/2605.23386)
*Jacob Swindell,Michael Lowen,Marija Popovic,Riccardo Polvara*

Main category: cs.RO

TL;DR: 提出了一种名为Droneulator的便携式无人机模拟器架构，结合了RotorPy多旋翼动力学和Godot 4渲染引擎，支持PX4控制、WebSocket指令、以及基于Zenoh的ROS~2兼容管线，可在单一部署栈中完成农业无人机检测、规划与强化学习任务。


<details>
  <summary>Details</summary>
Motivation: 农业无人机研究需要集成真实三维场景、高保真车辆动力学与机器人中间件的模拟器，且需便于在不同异构开发机上部署。

Method: 架构由RotorPy提供多旋翼动力学，Godot 4负责渲染与传感器生成，支持PX4控制与轻量级WebSocket指令路径，并通过基于Zenoh的ROS~2兼容管线发布同步视觉与状态流。

Result: 在树尺度图像采集（COLMAP 3D重建）、冠层障碍物局部规划（EGO-Planner）、以及闭环强化学习（自定义Gymnasium环境）三个农业无人机工作流中，验证了低延迟感知、可变密度数据采集、无碰撞局部规划及稳定深度感知策略训练。

Conclusion: Droneulator在单一可部署栈中展示了农业无人机检测、规划与学习的潜力。
...
### [193] [Semantically Structured Mixture-of-Experts for Compositional Robotic Manipulation](https://arxiv.org/abs/2605.23477)
*Chengyu Deng,Guanqi Chen,Yizhou Chen,Zejia Liu,Zhiwen Ruan,Guanhua Chen,Jia Pan*

Main category: cs.RO

TL;DR: 提出语义结构化混合专家扩散策略，通过视觉语言模型将动作路由到行为阶段子专家，实现高效多任务机器人操作。


<details>
  <summary>Details</summary>
Motivation: 现有扩散策略面临计算开销大与轻量模型泛化差的矛盾，而MoE路由机制忽略任务组合结构，导致专家碎片化。

Method: 利用推理时轻量技能预测器，结合VLM离线标注，通过跨模态与跨实例对比对齐实现专家语义路由。

Result: 在多任务基准上超越扩散与MoE基线，参数效率显著提升，并通过参数高效微调实现组合迁移。

Conclusion: 语义结构化路由可提升参数效率与组合泛化能力，为可扩展机器人学习提供新范式。
...
### [194] [TactileReflex: Noise-Statistics-Driven Vision-Tactile Reflex Control for Force-Sensitive Manipulation](https://arxiv.org/abs/2605.23568)
*Ziyan Feng,Yulong Fu,Zheng Li,Yuxin He,Jieji Ren,Lujia Wang,Jinni Zhou,Yudong Zhong,Qiang Nie*

Main category: cs.RO

TL;DR: 提出一种基于噪声统计校准的触觉反射控制范式TactileReflex，用于机器人精准操控易变形容器，无需外部标定即可实现力-位置自适应闭环控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以实现易变形容器（如装满液体的塑料杯）中极窄力裕度下的实时抓取力调整，表现为力不足导致滑移，力过大导致不可逆变形。

Method: 通过分析视觉触觉传感器固有噪声特性（静态持握-释放协议）直接导出控制器阈值，构建三通道闭环控制器（剪切强度、接触强度、压力中心），以~12 Hz频率执行防滑、自适应释放和力保护反射。

Result: 三通道完整系统在防容器变形任务中成功率5/5，部分配置最高仅1/5；动态倒水任务中，固定基线0/10，TactileReflex在水体积变化下达9/10。

Conclusion: TactileReflex作为即插即用的可解释安全层，可集成于VR遥操作和VLA策略等高阶操作管线，实现免物理模型的力敏感操控。
...
### [195] [How Many Training Samples Are Needed for the Inverse Kinematics Solutions by Artificial Neural Networks](https://arxiv.org/abs/2605.23583)
*Dong-Won Lim*

Main category: cs.RO

TL;DR: 该研究探讨了基于人工神经网络的逆运动学求解器中训练样本数量对预测精度的影响，发现超过125个样本后模型效率提升有限。


<details>
  <summary>Details</summary>
Motivation: 传统逆运动学方法存在局限，人工神经网络虽有效但训练样本的合适数量未知。

Method: 生成不同数量的关节位置对训练前馈神经网络，评估精度、收敛性和泛化能力。

Result: 超过125个训练样本对模型效率无显著提升，揭示了数据效率的临界点。

Conclusion: 为优化神经网络求解器数据量提供了指导，在计算成本和模型精度间取得平衡。
...
### [196] [Vision-Based Agile Landing on Turbulent Waters](https://arxiv.org/abs/2605.23717)
*Dimosthenis Angelis,Leonard Bauersfeld,Davide Scaramuzza,Evangelos Boukas*

Main category: cs.RO

TL;DR: 提出了一种基于强化学习的无人机自主着陆方法，无需平台状态信息，利用视觉特征和状态测量生成控制指令，在模拟和真实实验中验证了其在恶劣海况下的性能。


<details>
  <summary>Details</summary>
Motivation: 无人机在开阔海域的无人船平台上自主着陆极具挑战性，现有方法通常依赖明确的平台状态信息，但在实际中难以获得或不稳定。

Method: 采用强化学习策略，结合无人机状态测量和从着陆表面提取的局部视觉特征（关键点和描述子），预测姿态和推力指令，由底层控制器跟踪；策略在模拟环境中用合成关键点训练，支持零样本部署。

Result: 在模拟实验中，该方法在“非常恶劣”海况下优于模型预测控制基线；真实实验中成功实现机载自主着陆，使用了两种不同的局部特征提取器。

Conclusion: 该方法是首个不依赖显式平台状态表示、在湍急水域实现多旋翼无人机敏捷着陆的技术，验证了其有效性和鲁棒性。
...
### [197] [Any2Any: Efficient Cross-Embodiment Transfer for Humanoid Whole-Body Tracking](https://arxiv.org/abs/2605.23733)
*Ming Yang,Tao Yu,Feng Li,Hua Chen*

Main category: cs.RO

TL;DR: 提出Any2Any方法，实现跨形态全身跟踪模型的高效迁移，仅需少量数据和计算即可在新机器人上部署。


<details>
  <summary>Details</summary>
Motivation: 从头训练全身跟踪模型需要大量数据和计算，导致在新型人形机器人上部署成本高昂，因此探索预训练模型能否通过最小化适配实现跨形态迁移。

Method: 首先进行运动学对齐以匹配源和目标机器人的输入输出空间，然后对动态敏感模块应用轻量级参数高效微调（PEFT）进行动力学适配。

Result: 在多人形平台和预训练骨干上，Any2Any显著加速收敛并降低训练成本，仅用1%数据和计算即可实现与从头训练相当或更优的跟踪性能。

Conclusion: 预训练全身跟踪专家可通过高效迁移跨形态复用，为可扩展的人形机器人全身控制部署提供路径。
...
### [198] [Direct Dynamic Retargeting for Humanoid Imitation Learning from Videos](https://arxiv.org/abs/2605.23762)
*Constant Roux,Ludovic De Matteïs,Armand Jordana,Valentin Guillet,Nicolas Mansard,Olivier Stasse,Philippe Souères*

Main category: cs.RO

TL;DR: 提出直接动态重定向（DDR）框架，直接从专家视频生成高保真、动态可行的轨迹，避免了几何重定向的偏差。


<details>
  <summary>Details</summary>
Motivation: 现有基于几何重定向或间接动态重定向的方法引入几何偏差，限制了搜索空间并导致次优动态行为。

Method: 在任务空间建模，利用物理模拟器中的采样模型预测控制求解器，优化复杂接触序列并缓解输入漂移。

Result: 在演示跟踪准确性上优于现有基线，为强化学习代理提供物理可行参考加速训练收敛并提升敏捷与平衡行为执行。

Conclusion: 绕过几何偏差可以生成更优的动态轨迹，并提升下游学习任务的性能。
...
### [199] [SFG-ROS: A Resource-Aware Framework for Dense Multi-Agent Perception](https://arxiv.org/abs/2605.23832)
*Constantin Blessing,Elias Geiger,Jakob Häringer,Dennis Grewe,Markus Enzweiler*

Main category: cs.RO

TL;DR: 提出SFG-ROS，一个面向异构多机器人编队的资源感知软件框架，通过架构优化解决ROS 2在密集传感器数据分发中的网络饱和与计算开销问题。


<details>
  <summary>Details</summary>
Motivation: 标准ROS 2在部署异构多机器人编队进行协同感知时，存在网络饱和、命名空间冲突和高计算开销等问题，亟需可扩展的软件架构。

Method: 三方面贡献：1) 基于模式的路由隔离，通过程序化全限定名和Fast DDS路由控制网络流量；2) 按需集中解码流水线，自动卸载高带宽传感器数据解压；3) 硬件无关容器流水线，适配异构加速器实现零接触部署。

Result: 网络流量控制为O(1)复杂度，通过轻量级IPC替代冗余解压，CPU缩放惩罚降低72.3%，同时保持低延迟。

Conclusion: SFG-ROS有效解决了多机器人系统中传感器数据分发的瓶颈问题，并已开源发布。
...
### [200] [Instrumentation for Imitation Learning: Enhancing Training Datasets for Clothes Hanger Insertion](https://arxiv.org/abs/2605.23847)
*Remko Proesmans,Thomas Lips,Francis wyffels*

Main category: cs.RO

TL;DR: 通过传感器集成（仪表化）实现高效模仿学习，仅需180次演示即可使机器人完成衣架插入任务，仪表化策略比纯视觉策略成功率提升14-25个百分点。


<details>
  <summary>Details</summary>
Motivation: 机器人操作领域缺乏类似视觉语言模型的数据规模，需要更高效的学习方法。

Method: 采用扩散策略，对比有无传感器数据（仪表化）的模仿学习；并利用仪表化专家策略的轨迹数据增强训练纯视觉学生策略。

Result: 仪表化策略准确率更高（提升14-25%），且黑箱模型自发优先使用传感器信号；利用专家数据增强后，纯视觉策略性能接近仪表化专家。

Conclusion: 仪表化是提升机器人操作模仿学习效率的有效策略。
...
### [201] [Point Tracking Improves World Action Models](https://arxiv.org/abs/2605.23856)
*Jiarui Guan,Wenshuai Zhao,Yue Pei,Ziliang Chen,Arno Solin,Juho Kannala*

Main category: cs.RO

TL;DR: 提出联合像素-轨迹世界动作模型JOPAT，通过单一扩散变压器同时预测视觉观测、2D点轨迹与动作，在长程任务中优于纯像素方法。


<details>
  <summary>Details</summary>
Motivation: 像素级预测易受光照、纹理等任务无关因素干扰，导致对动态环境建模脆弱，需引入更鲁棒的显式运动表示。

Method: 采用单去噪扩散变压器联合建模像素、轨迹可见性及动作，轨迹提供长程动态显式表示且对遮挡/离屏运动鲁棒。

Result: 在LIBERO和LeRobot真实任务中，JOPAT优于像素基线，尤其在涉及遮挡、物体交互和离屏运动的长程任务中提升显著。

Conclusion: 显式轨迹表示能提升世界动作模型对动态环境的鲁棒性与泛化性，是优于纯像素建模的关键设计。
...
### [202] [Robotic Strawberry Harvesting with Robust Vision and Deep Reinforcement Learning based Sim-to-Real Control](https://arxiv.org/abs/2605.23863)
*Al Bashir,Shao-Yang Chang,Partho Ghose,Prem Raj,Chen-Kang Huang,Azlan Zahid*

Main category: cs.RO

TL;DR: 提出了一种结合改进YOLO26-seg视觉模型和模拟训练PPO控制器的闭环草莓采摘机器人系统，在温室实验中取得了84.3%的采摘成功率。


<details>
  <summary>Details</summary>
Motivation: 传统机器人采摘依赖逆运动学规划器，存在运动不平滑、开发成本高和依赖硬件测试等问题，需要一种更高效、低成本的方案。

Method: 采用HRAttnEdge-YOLO26-seg视觉模型提升分割精度，并利用Isaac Lab模拟环境训练PPO策略，生成平滑关节指令，在真实UR10e机器上部署。

Result: 视觉模型在数据集上分割性能提升10-14%；PPO控制器运动更平滑；温室采摘测试中达到96.6%到达成功率、91.3%抓取成功率和84.3%总体成功率。

Conclusion: 结合任务特定感知和模拟训练的PPO可替代传统规划方法，在复杂农业环境中实现可靠闭环采摘。
...
### [203] [A Methodology for Impedance-based Stability Margin Analysis for Interconnected Offshore Wind Clusters](https://arxiv.org/abs/2605.23030)
*Germano Rugendo Mugambi,Behnam Nouri,Oscar Saborío-Romano,George Alin Raducu,Nicolaos A. Cutululis*

Main category: eess.SY

TL;DR: 提出一种通用阻抗方法，评估新风电集群接入后原有集群的稳定裕度，并推导新集群的最大允许阻抗以满足系统运营商的最小稳定裕度要求。


<details>
  <summary>Details</summary>
Motivation: 现有阻抗法多依赖奈奎斯特包围判断，未明确量化稳定裕度，无法满足新集群接入时对最小裕度的要求。

Method: 基于阻抗模型，提出新奈奎斯特稳定区域补充广义奈奎斯特判据，通过推导最大允许阻抗和稳定裕度评估方法实现。

Result: 通过两个互联风电场及HVDC系统的频率域模型验证，所提方法能有效评估稳定裕度并指导新集群设计。

Conclusion: 该方法可量化稳定裕度，确保新集群接入后系统满足稳定裕度要求，并为系统规划提供解析依据。
...
### [204] [Holistic Grid-Forming Control to Enhance the Frequency Support from HVDC-Connected Offshore Wind Power Plants](https://arxiv.org/abs/2605.23041)
*Zhenghua XU,Dominic Groß,George Alin Raducu,Behnam Nouri,Oscar Saborío-Romano,Nicolaos A. Cutululis*

Main category: eess.SY

TL;DR: 本文提出一种无需通信的HVDC-OWPP系统整体构网控制，通过协调AC/DC终端控制提升频率支撑速度与效率，最小化变流器储能利用。


<details>
  <summary>Details</summary>
Motivation: 应对电力电子化电网中频率稳定性挑战，满足HVDC连接的海上风电场惯性响应与频率控制储备需求。

Method: 建立典型HVDC-OWPP系统模型；设计无通信的整体构网控制器；基于各终端带宽上限的分析整定方法。

Result: 仿真验证所提控制比传统配置响应更快、频率支撑更有效，且减少了各变流器固有储能的利用。

Conclusion: 整体构网控制为变流器主导系统提供新设计思路，实现快速高效的频率支持。
...
### [205] [Open-Source METANET Calibration for Reproducible Freeway Traffic Macroscopic Simulation](https://arxiv.org/abs/2605.23042)
*Monica Chan,Shreyaa Raghavan,Cathy Wu*

Main category: eess.SY

TL;DR: 本文提供了一个开源的METANET参数校准、仿真和可视化工具，使用内点法（IPOPT）解决非线性规划问题，并通过I-24 MOTION和PeMS数据验证其有效性。


<details>
  <summary>Details</summary>
Motivation: METANET模型广泛使用但缺乏开源校准工具，导致结果不可复现或推广。

Method: 将校准问题表述为非线性规划（NLP），通过内点法（IPOPT）求解，并联合估计匝道流量。

Result: 校准后的模型能准确再现不同网络几何和交通条件下的真实交通模式，包括复杂的停车-启动拥堵波。

Conclusion: 随着交通监测基础设施扩展，开源校准工具对将传感器数据转化为验证模型、支持实时交通控制至关重要。
...
### [206] [Deception and Counter Deception in Adversarial Graph Traversal Game](https://arxiv.org/abs/2605.23129)
*Violetta Rostobaya,James Berneburg,Daigo Shishika*

Main category: eess.SY

TL;DR: 本文研究对抗性图遍历中的欺骗问题，提出了一种适应无限期博弈的扩展形式双人博弈算法，并证明其能收敛到纳什均衡。


<details>
  <summary>Details</summary>
Motivation: 现有工作假设固定观察者-欺骗者角色，而实际场景中双方均拥有私有信息并更新信念，需模型化这种不完全信息。

Method: 改编扩展形式双人博弈算法以处理内生终止的无限期博弈，保证有限计算。

Result: 算法在有限时间内终止并返回ε-纳什均衡，信息价值刻画了均衡策略中的欺骗与反欺骗行为。

Conclusion: 该算法能有效求解双方向不完全信息图遍历欺骗问题，揭示信息价值驱动的博弈行为。
...
### [207] [Convex Hybrid Modeling: An Operator-Based Approach](https://arxiv.org/abs/2605.23151)
*Wentao Tang*

Main category: eess.SY

TL;DR: 提出一种混合建模框架，通过凸优化问题将物理可解释性融入机器学习模型，涵盖正则化、子空间约束及流形约束三种设置，并用算子理论实现非线性系统的核方法混合解释模型。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型虽能准确建模过程系统，但决策用模型需结构简单且物理可解释；现有方法在非参数通用性与参数化可解释性之间难以平衡。

Method: 通过构造凸学习问题实现可解释性系统化建模：1）正则化至参考模型；2）约束于可解释子空间；3）约束于可解释流形（非线性参数化）。后一情形引入算子理论，将模型重参数化为“规范特征”的核混合形式。

Result: 数值实验验证了静态与动态模型在三种设置下的有效性，特别是流形约束下核方法混合模型能平衡准确性与可解释性。

Conclusion: 所提框架能系统地将先验物理知识嵌入学习过程，通过凸优化高效获取兼具结构简单与解释性的代理模型，适用于过程控制等可解释性关键领域。
...
### [208] [Physics-informed sparse identification-based tube model predictive control for aerial vehicles](https://arxiv.org/abs/2605.23354)
*Tayyab Manzoor,Yasir Ali,Yuanqing Xia,Lijie You,Yan Wang*

Main category: eess.SY

TL;DR: 提出一种基于物理信息机器学习的模型预测控制框架，通过稀疏控制仿射模型和自适应鲁棒管方法，在降低计算负载的同时提高跟踪性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决自主飞行器在动态环境中计算效率与鲁棒性能之间的矛盾。

Method: 基于物理信息机器学习构建稀疏控制仿射模型，嵌入先验知识并学习残差不确定性，结合高阶Runge-Kutta离散化和自适应管机制实现鲁棒MPC。

Result: 与非线性MPC和基于高保真模型的鲁棒MPC相比计算负载显著降低，并在四旋翼仿真和实验中超越PID等对比方法。

Conclusion: 该框架为资源受限的飞行系统提供了高效且鲁棒的控制综合方案。
...
### [209] [A Distributed Framework for Data-Driven Safe Coordination in Leader-Follower Networks](https://arxiv.org/abs/2605.23356)
*Mirhan Urkmez,Maryam Sharifi,Shahab Heshmati-Alamdari*

Main category: eess.SY

TL;DR: 本文提出了分布式数据驱动零化控制屏障函数（3D-ZCBF）框架，通过从输入-状态数据中识别导数边界，在无需显式模型的情况下确保领导者-跟随者多智能体系统的连通性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖显式动力学模型，难以处理未知控制仿射动力学和高维智能体系统，因此需要无模型的数据驱动方法。

Method: 利用数据驱动方法推导导数边界，建立显式解耦的安全条件，并通过投影控制器实现局部两跳信息下的连通性保持。

Result: 仿真验证表明所提条件能有效维持系统级连通性，且数据量和雅可比边界精度影响可行性。

Conclusion: 3D-ZCBF框架实现了无模型、局部信息下的多智能体系统连通性形式化保障。
...
### [210] [OptiQU: Coordinated Multi-Level Voltage and Reactive Power Control for Enhanced Voltage Quality and Secure Grid Operation](https://arxiv.org/abs/2605.23505)
...
TL;DR: 提出OptiQU协调电压与无功控制方案，结合中央优化与本地控制，提升中低压配电网的电压治理与可再生能源接纳能力。


<details>
  <summary>Details</summary>
Motivation: 现有中低压配电网面临光伏与高功率负荷增长导致的电压越限问题，而传统本地独立控制未能充分利用无功灵活性。

Method: OptiQU方案：中央优化协同中低压层设备（如分布式能源无功、变电站设备）设定点，结合本地控制与降级策略，实现鲁棒运行。

Result: 模拟与实验室评估表明协调方法可增加上游电压与无功控制的可用灵活性，减少越限与弃电。

Conclusion: 该方案有望提升配电网灵活性与韧性，后续将进行现场验证与性能-通信成本权衡分析。
...
### [211] [Beyond Shrinkage: Foundations of Data-Driven Control for Piecewise Affine Systems](https://arxiv.org/abs/2605.23524)
*Gianluca Giacomelli,Victor G. Lopez,Simone Formentin,Matthias A. Müller,Valentina Breschi*

Main category: eess.SY

TL;DR: 该论文首次将数据驱动预测控制（DeePC）扩展至分段仿射（PWA）系统，提供了行为刻画、Willems引理扩展、线性预测器与正则化的一致性分析，以及误分类对数据预测影响的研究。


<details>
  <summary>Details</summary>
Motivation: DeePC尚未应用于广泛使用的PWA系统，需要填补这一空白以增强控制器的可解释性和有效性。

Method: 通过行为特征描述、Willems引理推广、正则化线性预测器分析及误分类影响研究构建理论框架。

Result: 理论发现通过简单数值示例验证，表明需超越正则化DeePC框架以设计符合PWA行为的控制动作。

Conclusion: 扩展DeePC至PWA系统可提升控制器的可解释性，但需避免单纯依赖正则化方法，以确保控制与系统行为一致。
...
### [212] [SafeSABR: Risk-Calibrated Adaptive Bitrate Streaming over Starlink Networks](https://arxiv.org/abs/2605.23560)
*Hongjun Xie,Jiahang Zhu,Zhiming Shao,Chao Fan,Zenghui Zhang,Genke Yang,Pengcheng Luo*

Main category: eess.SY

TL;DR: 提出SafeSABR框架，在Starlink网络中通过风险校准学习降低严重卡顿，实现QoE与严重风险的平衡。


<details>
  <summary>Details</summary>
Motivation: Starlink链路吞吐量波动大，现有ABR算法虽平均QoE高，但严重卡顿未被捕捉。

Method: 三阶段设计：行为克隆预训练、风险校准强化学习微调、运行时安全审计器检查码率。

Result: 严重卡顿会话从22.8%降至7.2%，最差5%会话缓冲从54.30s降至22.68s，QoE仅下降1.8%。

Conclusion: 风险校准策略学习与决策感知安全吞吐预测结合，可在波动Starlink网络中实现更安全的ABR。
...
### [213] [RF Instrument Agent (RFIA): Empowering RF Instruments with Natural Language Understanding, Scheduling and Execution of Complex Tasks](https://arxiv.org/abs/2605.23636)
*Chunhui Li,Wei Fan*

Main category: eess.SY

TL;DR: 提出RFIA自然语言代理框架，实现射频仪器可靠任务驱动控制，在16任务基准测试中全部通过，包括安全拒绝。


<details>
  <summary>Details</summary>
Motivation: 现有射频仪器远程控制依赖手动操作或自定义脚本，耗时且需要专业知识。

Method: 解耦意图-规划-执行架构，LLM仅用于任务理解和高级规划，确定性运行时处理仪器操作，结合结构化知识库和混合执行图。

Result: 16任务基准测试全部通过，包括一次预期安全拒绝，在230B和27B LLM后端均验证可行性。

Conclusion: 解耦架构支持不同LLM后端的可靠自然语言射频测量自动化。
...
### [214] [Output Feedback MPC with Adaptive Tubes](https://arxiv.org/abs/2605.23661)
*Anchita Dey,Shubhendu Bhasin*

Main category: eess.SY

TL;DR: 提出一种自适应管输出反馈模型预测控制框架，用于参数和加性不确定的线性系统。


<details>
  <summary>Details</summary>
Motivation: 传统鲁棒管MPC需要在整个参数不确定集上共同二次稳定反馈增益，限制了性能。

Method: 使用自适应观测器估计状态、参数和初始条件，并更新相应的不确定性集，据此调整约束松弛、终端项和管几何形状。

Result: 与传统方法相比，性能随时间改善，递归可行性和鲁棒指数稳定性得到证明。

Conclusion: 该框架避免了共同稳定增益要求，能自适应地利用不确定性信息提升控制性能。
...
### [215] [Reachability for Low-Thrust Trajectories via Maximum Initial Mass](https://arxiv.org/abs/2605.23770)
*Giacomo Acciarini,Dario Izzo,Zhong Zhang*

Main category: eess.SY

TL;DR: 双密度公式替代经典可达集，通过最大化初始质量或太阳帆强度参数判定轨道可达性，结合残差网络代理模型实现快速评估。


<details>
  <summary>Details</summary>
Motivation: 经典可达集分析在高维或强非线性系统（如地月空间、太阳帆任务）中计算昂贵且不实用。

Method: 提出最大初始质量（MIM）公式，将可达性转化为标量优化问题，并构建残差神经网络代理模型。

Result: 残差网络在精度、训练稳定性和模型复杂度之间取得最佳平衡，代理模型能快速评估可达性。

Conclusion: MIM双密度公式结合数据驱动代理模型为低推力轨道初步设计提供了实用工具。
...
### [216] [Advanced AI Service Provisioning in O-RAN through LLM Engine Integration](https://arxiv.org/abs/2605.23809)
*Seyed Bagher Hashemi Natanzi,Pranshav Gajja,Bo Tang,Vijay K. Shah*

Main category: eess.SY

TL;DR: 提出了一种结合LLM和自动化ML引擎的Dual-Brain架构，用于加速O-RAN中的AI应用开发与部署。


<details>
  <summary>Details</summary>
Motivation: O-RAN应用创建过程缓慢且手动，需要自动化。

Method: 构建Dual-Brain架构，包括LLM协调器和NeuralSmith自动ML引擎。

Result: 在容器化O-RAN 5G SA测试平台上展示可行性和实用性。

Conclusion: 该架构加速AI应用开发，但仍需进一步研究。
...
### [217] [Efficient Learned Image Compression without Entropy Coding](https://arxiv.org/abs/2605.23323)
*Hao Cao,Wenqi Guo,Zhijin Qin,Jungong Han*

Main category: eess.IV

TL;DR: 提出了一种免熵编码的学习图像压缩框架EF-LIC，通过无约束向量量化和上下文条件自回归变换减少冗余，实现低延迟且性能与有熵编码方法相当。


<details>
  <summary>Details</summary>
Motivation: 熵编码是学习图像压缩中的延迟瓶颈，因其顺序执行特性导致编码速度受限。

Method: 引入无约束向量量化以最小化统计冗余，并提出上下文条件自回归变换以减少相关性冗余。

Result: 在Kodak数据集上与MS-ILLM相比，LPIPS指标下比特率降低高达67.86%；编码速度提升3倍以上，解码速度提升5倍以上。

Conclusion: EF-LIC无需熵编码即可达到与基于熵编码方法相当的压缩性能，同时显著降低编码延迟。
...
### [218] [Discontinuous Galerkin Neural Operator for Pathology Defocus Deblurring](https://arxiv.org/abs/2605.23282)
*Shaoqing Duan,Haofei Song,Xintian Mao,Qingli Li,Yan Wang*

Main category: eess.IV

TL;DR: 提出一种不连续伽辽金神经算子用于病理显微图像去模糊，克服了传统方法在空间变化模糊处理中的局限。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法受限于平移不变性和有限可解释性，难以处理病理显微图像中由位置依赖积分成像导致的空间变化且局部不连续的模糊模式。

Method: 提出不连续伽辽金神经算子，利用不连续伽辽金公式参数化积分核，包含单元局部体积算子和界面数值通量。

Result: 超越当前最优方法，实现更清晰的图像重建，稳健处理空间变化模糊，并支持高分辨率可扩展性能。

Conclusion: DGNO通过结合局部性、异构性建模和全局一致性，有效还原光学成像物理过程，为病理显微图像去模糊提供了新方案。
...
### [219] [GMENet: Generative Mixture of Experts Network for Multi-Center Glioma Diagnosis with Incomplete Imaging Sequences](https://arxiv.org/abs/2605.23183)
*Pengfei Song,Fangjin Liu,Wenwen Zeng,Yonghuang Wu,Chengqian Zhao,Feiyu Yin,Xuan Xie,Jinhua Yu*

Main category: eess.IV

TL;DR: 提出GMENet网络，利用生成式混合专家模型处理不完整MRI序列，提升多中心胶质瘤诊断的临床适用性。


<details>
  <summary>Details</summary>
Motivation: 临床中成像协议差异导致MRI序列不完整，现有方法需丢弃大量数据，限制临床应用。

Method: 设计基于交叉注意的门控生成模块合成缺失序列，引入循环一致性损失；再通过动态加权专家融合模块进行多任务预测。

Result: 在1241名受试者的多中心数据上，临床可用训练数据增加97%，且优于基于完整数据训练的现有方法。

Conclusion: GMENet能有效利用不完整序列数据，提升跨中心鲁棒性和诊断性能。
...
### [220] [STAMBRIDGE: Spectral-Temporal Amplitude-aware Mid-Feature Bridge for EEG Visual Decoding](https://arxiv.org/abs/2605.23137)
*Jiahe Meng,Weiming Zeng,Yueyang Li,Bo Chai,Hongjie Yan,Zhiguo Zhang,Wai Ting Siok,Nizhuan Wang*

Main category: eess.IV

TL;DR: 提出STAMBRIDGE两阶段框架，通过频谱-时间幅度感知调制（STAM）提取稳健EEG特征，再通过中间特征语义桥（MFSB）实现稳定跨模态对齐，在THINGS-EEG基准上取得领先零样本检索性能，并支持语义一致的图像重建。


<details>
  <summary>Details</summary>
Motivation: EEG视觉解码面临低信噪比神经信号与结构化视觉-语言空间之间的模态差距，直接跨模态对齐不稳定。

Method: 两阶段框架：第一阶段STAM使用幅度衍生软通道加权和多尺度时间卷积替代硬频率掩码，保留频率感知瞬态；第二阶段MFSB构建正则化中间空间，通过定向跨模态交互实现逐步蒸馏和稳定语义对齐。

Result: THINGS-EEG基准上200路零样本检索Top-1准确率34.50%，Top-5准确率65.95%，且嵌入特征能生成语义连贯的图像重建。

Conclusion: STAMBRIDGE有效桥接了EEG与视觉语义空间，为低信噪比神经信号的跨模态对齐提供了稳健且可泛化的解决方案。
...
### [221] [Do Synthetic Brain MRIs Reliably Improve Tumour Classification? A StyleGAN2-ADA Class-Plane Augmentation Study on BRISC 2025](https://arxiv.org/abs/2605.23094)
*José Rafael Noriega Cedeño*

Main category: eess.IV

TL;DR: 研究了GAN生成图像在医学小数据集中的增强效果，发现增强效果依赖于架构和比例，视觉保真度不能保证下游任务性能提升。


<details>
  <summary>Details</summary>
Motivation: 生成增强常被用作医学小数据集的补救措施，但合成图像只有在下游任务性能提升时才有用，现有研究对此缺乏系统评估。

Method: 在BRISC 2025数据集的分类任务上训练12个StyleGAN2-ADA生成器，使用或不使用InceptionV3特征空间过滤，以1:1和1:2的真实与合成图像比例评估随机森林、CNN和MobileViTV2三种分类器的性能，并进行GPT-5.5盲测。

Result: 随机森林未从合成MRI受益；CNN有持续平均增益但未通过Holm校正；MobileViTV2在过滤后1:1增强下肿瘤分类准确率绝对提升1.02%（Holm校正p=0.0104）；增强CNN早停42-64%，MobileViTV2早停50-67%。

Conclusion: 生成增强的效用依赖于架构和比例，视觉保真度不能保证下游任务性能提升。
