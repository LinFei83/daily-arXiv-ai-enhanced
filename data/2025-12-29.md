...
TL;DR: 本研究提出一种基于多模态大语言模型的多智能体系统，用于标准化HTP绘画测试的评估，通过角色分工实现特征识别与心理推断的解耦，在定量和定性分析中均表现出接近专家水平的解释能力。


<details>
  <summary>Details</summary>
Motivation: 传统HTP绘画测试存在评分标准不统一、依赖评估者主观经验、缺乏量化编码系统等问题，需要一种标准化、可量化的评估方法来提高其客观性和一致性。

Method: 采用多模态大语言模型构建多智能体协作框架，结合社会心理学视角和去污名化叙事，通过角色分工将视觉特征识别与心理推断过程分离，并使用余弦相似度量化模型解释与专家解释的语义一致性。

Result: 定量实验显示模型与专家解释的平均语义相似度约为0.75（标准差0.05），在结构化专家数据集中提升至0.85；定性分析表明系统能有效纠正视觉幻觉，生成具有高生态效度和内部一致性的心理报告。

Conclusion: 多模态大模型可作为投射测验的标准化工具，所提出的多智能体框架为数字心理健康服务提供了新范式，通过解耦特征识别与心理推断提升了评估的客观性。
...
TL;DR: 分析使用RZS和模式表解决围棋死活题时，计算机求解器与人类解法的差异，发现求解器能识别关键区域和罕见模式，但在模式评估和策略偏好上与人类不同。


<details>
  <summary>Details</summary>
Motivation: 探究当前顶尖围棋求解器在死活题上的表现，特别是其与人类专家解法的一致性及差异，以改进求解器的局限性。

Method: 采用基于相关区域搜索（RZS）和相关区域模式表技术，测试求解器在《围棋死活辞典》中七个经典死活题上的求解过程。

Result: 求解器成功识别各题的关键区域，发现包括罕见模式在内的多种模式；但在两题中给出与书中不同的解答，且存在误判罕见模式价值、偏好直接做活而非最大化领土的问题。

Conclusion: 当前求解器在死活题求解中展现出独特能力，但在模式评估和策略优化方面与人类思维存在差异，未来需针对这些问题改进算法。
...
TL;DR: 本文提出将冲突分析中的辅助函数拆分为联盟函数和冲突函数，以解决传统方法在聚合时语义模糊的问题，并应用于三支冲突分析中的主体、议题和主体对的三分问题。


<details>
  <summary>Details</summary>
Motivation: 传统三支冲突分析中，辅助函数同时衡量联盟和冲突关系，导致在聚合时（如对一组议题取平均）语义不清晰，例如联盟+1与冲突-1的聚合结果可能与中立0相同，但实际态度差异很大。

Method: 将辅助函数拆分为独立的联盟函数和冲突函数，基于此对主体、议题和主体对进行三分，并应用于联盟集和策略等概念的分析。

Result: 提出的方法能更清晰地解释冲突分析中的语义，通过实际案例验证了模型的有效性，解决了传统聚合方法中的模糊性问题。

Conclusion: 分离联盟和冲突函数能提升三支冲突分析的语义明确性，为联盟集和策略等关键问题提供更可靠的分析框架，具有实际应用价值。
...
TL;DR: 本文提出了一种基于加权一致性与非一致性度量的三支冲突分析新方法，用于识别可行策略和最优解，并在NBA劳资谈判和甘肃省发展规划案例中验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有三支冲突分析研究多集中于冲突的划分与理解，但在冲突解决方面存在不足，特别是缺乏对可行策略制定方法的系统研究。

Method: 基于智能体的正负相似度计算群体整体评分，结合智能体与议题的权重提出加权一致性与非一致性度量，设计算法识别可行策略、L阶可行策略及最优策略。

Result: 所提模型在NBA劳资谈判和甘肃省发展规划案例中表现出实用性和有效性；敏感性分析和对比实验表明，该方法通过统一加权评估与一致性度量，在识别可行策略和最优解方面优于传统冲突分析方法。

Conclusion: 基于加权一致性与非一致性度量的冲突解决模型能够系统化识别可行策略与最优解，为冲突缓解提供了更有效的理论工具和实践框架。
...
TL;DR: 本文回顾了基于粗糙集的三支决策在完全信息下的计算与概念两种表述，并将其推广到更实用的不完全信息场景。针对计算表述，提出了对象相似度度量以替代等价关系，并探讨了基于α相似类和对象可逼近性的两种三支决策方法。针对概念表述，提出了公式满足度度量，并研究了基于α意义集和公式置信度的两种方法。


<details>
  <summary>Details</summary>
Motivation: 现实应用中信息往往不完全，而现有三支决策方法主要基于完全信息。为提升三支决策在实际问题中的适用性，需要将完全信息下的理论框架推广到不完全信息场景。

Method: 1. 计算表述：提出对象相似度度量，基于α相似类和对象可逼近性分别构建三支决策方法。2. 概念表述：提出公式满足度度量，基于α意义集和公式置信度分别构建三支决策方法。

Result: 成功将三支决策的计算与概念表述推广至不完全信息场景。提出的对象可逼近性概念及概念表述中的两种新方法，为不完全信息分析指明了有前景的新方向。

Conclusion: 通过推广三支决策至不完全信息场景，本文不仅统一了计算与概念表述，还提出了新的分析工具（如可逼近性），增强了三支决策在实际应用中的可行性和有效性。
...
TL;DR: 提出LogicLens框架，通过视觉-文本协同推理统一处理文本中心伪造分析任务，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有文本伪造分析方法局限于粗粒度视觉分析，缺乏深度推理能力，且将检测、定位和解释视为独立子任务，忽略了它们之间的内在联系。

Method: 提出LogicLens统一框架，采用交叉线索感知思维链机制进行迭代跨模态验证，设计加权多任务奖励函数进行GRPO优化，并构建包含5397张图像的RealText数据集。

Result: 在T-IC13零样本评估中，宏平均F1分数比专用框架高41.4%，比GPT-4o高23.4%；在T-SROIE数据集上，在多指标上显著优于其他MLLM方法。

Conclusion: LogicLens通过视觉-文本协同推理有效提升文本伪造分析性能，所构建的数据集和框架为相关研究提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 现有固定长度惩罚方法难以调节且无法适应LLM推理能力的动态变化，导致准确性与简洁性之间的权衡不佳。

Method: 将长度控制建模为约束优化问题，采用拉格朗日对偶方法动态调整惩罚系数，实现生成过长时加强惩罚、过短时放松惩罚的自适应机制。

Result: 在Deepseek-R1-Distill-Qwen-1.5B和Qwen3-4B-Thinking-2507模型上，Leash将平均推理长度减少60%，涵盖数学推理、代码生成和指令遵循等任务，同时保持竞争力性能。

Conclusion: Leash为开发可控高效LLM提供了实用范式，平衡了推理能力与计算成本。
...
TL;DR: PayPal与NVIDIA合作开发Commerce Agent多智能体系统，通过NeMo框架对Nemotron小语言模型进行微调优化搜索发现代理，显著降低了延迟和成本。


<details>
  <summary>Details</summary>
Motivation: 解决PayPal平台智能体商务系统中检索组件性能瓶颈问题，该组件占智能体响应时间50%以上，需要优化延迟和成本同时保持系统质量。

Method: 使用llama3.1-nemotron-nano-8B-v1架构，通过NeMo框架进行LoRA微调，系统化超参数搜索（学习率、优化器、余弦退火调度、LoRA秩），优化检索导向的商务任务。

Result: 微调后的Nemotron SLM有效解决了检索组件的关键性能问题，在保持或提升系统整体性能的同时显著改善了延迟和成本。

Conclusion: 成功构建了首个应用于商务特定智能体优化的NeMo框架实施方案，为生产环境电商多智能体系统优化提供了可扩展框架。
...
TL;DR: 提出基于LLaVA的医学诊断框架，结合视觉语言对齐与逻辑正则化推理，提升多模态诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有医学多模态模型常产生幻觉或推理不一致，限制了临床可信度，需开发更可靠的推理方法。

Method: 构建包含输入编码器、跨模态投影模块、推理控制器（任务分解）和逻辑树生成器（可验证结论组装）的框架。

Result: 在MedXpertQA等基准测试中，该方法提升了多模态诊断准确性，生成更可解释的推理轨迹，文本任务保持竞争力。

Conclusion: 该框架是迈向可信赖医学多模态AI的有希望一步，能改善临床决策支持。
...
TL;DR: 提出AMS-IO-Agent，一种基于LLM的专用代理，用于模拟混合信号集成电路的结构感知I/O子系统生成，实现自然语言设计意图与工业级设计交付物的连接，并在28nm CMOS流片中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前AMS IC设计中I/O子系统生成依赖人工且耗时，需要将自然语言设计意图自动化转换为可验证的设计步骤，以提高设计效率并减少错误。

Method: 构建结构化领域知识库捕获设计约束与惯例；通过设计意图结构化将模糊用户意图转换为基于JSON和Python的中间格式；开发AMS-IO-Bench基准测试平台进行评估。

Result: 在基准测试中达到超过70%的DRC+LVS通过率，将设计周期从数小时缩短至数分钟，优于基线LLM；代理生成的I/O环在28nm CMOS流片中成功制造并验证。

Conclusion: AMS-IO-Agent首次实现了基于LLM的代理在AMS IC设计中完成非平凡子任务并直接用于硅片制造，证明了人机协作在复杂IC设计中的实用价值。
...
TL;DR: OrchestRA是一个人类在环的多智能体平台，将生物学、化学和药理学整合为自主发现引擎，通过动态反馈循环实现治疗设计的民主化。


<details>
  <summary>Details</summary>
Motivation: 当前治疗发现面临专业领域碎片化、计算设计与生理验证之间存在执行鸿沟的挑战，现有生成式AI模型多作为被动助手而非自主执行者。

Method: 采用多智能体架构：Orchestrator协调管理；生物学家智能体通过大规模知识图谱（>1000万关联）进行深度推理识别靶点；化学家智能体自主检测结构口袋进行从头设计或药物重定位；药理学家智能体通过生理药代动力学模拟评估候选物，形成药代/毒性特征触发结构重新优化的动态反馈循环。

Result: 平台实现了计算设计与实验验证的闭环整合，将药物发现从随机搜索转变为可编程的循证工程学科。

Conclusion: OrchestRA通过自主执行与人类指导的无缝集成，建立了治疗设计的动态工程范式，有望显著加速治疗发现进程。
...
TL;DR: 本文提出了一种针对LLM应用、边缘智能等资源分配问题的多臂赌博机变体，设计了匹配理论下界的算法来解决优先资源共享机制下的非线性组合优化问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型应用、边缘智能等场景中的资源分配问题需要处理具有随机容量和优先级权重的多臂赌博机模型，现有方法无法有效处理优先资源共享机制带来的非线性组合效用函数。

Method: 提出MSB-PRS-OffOpt算法计算最优分配策略（复杂度O(MK³)），并以此为基础设计近似上置信界算法进行在线学习。

Result: 证明了实例无关和实例相关的遗憾下界分别为Ω(α₁σ√(KMT))和Ω(α₁σ²(M/Δ)lnT)，所提算法的遗憾上界分别匹配下界至√(K ln KT)和α₁K²因子。

Conclusion: 该研究解决了优先资源共享机制下的非线性组合优化与学习问题，为资源受限的智能系统提供了理论保证和实用算法。
...
TL;DR: 提出了一种基于多模型共识和推理层治理的负责任且可解释的AI智能体架构，旨在解决智能体AI在可解释性、问责制等方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统在自主性增强的同时，带来了可解释性、问责制、鲁棒性和治理方面的关键挑战，现有实现往往功能优先但缺乏理解决策依据或跨智能体责任落实的机制。

Method: 采用异构LLM和VLM智能体联盟独立生成候选输出，暴露不确定性和分歧，再由专用推理智能体进行结构化整合，执行安全策略约束，减少幻觉和偏见，并保留可审计的中间输出。

Result: 在多个实际智能体AI工作流中评估表明，共识驱动的推理提高了跨领域应用的鲁棒性、透明度和操作信任度。

Conclusion: 该工作为设计既自主可扩展、又天生负责任和可解释的智能体AI系统提供了实用指导。
...
TL;DR: 本文提出合规评级方案（CRS）框架及开源Python库，用于评估生成式AI数据集在透明度、问责制和安全性方面的合规性，并支持负责任的新数据集构建。


<details>
  <summary>Details</summary>
Motivation: 当前生成式AI依赖大规模开源数据集，但这些数据集常通过不透明、不受限制的方式收集，且其伦理和法律问题常被忽视。数据集在共享、编辑和再传播过程中，其来源、合法性和安全性信息易丢失，需要系统化解决方案。

Method: 提出合规评级方案（CRS）框架，基于数据溯源技术开发开源Python库，可集成到现有数据处理和AI训练流程中，既能评估现有数据集的CRS，也能指导负责任的新数据集爬取和构建。

Result: 开发了可操作的工具库，支持对数据集合规性进行反应性评估和前瞻性引导，填补了当前对数据集伦理与法律合规性评估工具的空白。

Conclusion: CRS框架及配套工具库为生成式AI数据集提供了透明度、问责制和安全性评估标准，有助于促进更负责任的数据集构建和使用实践。
...
TL;DR: 本文提出SAGA框架，通过双层架构自动化科学发现中的目标函数设计，解决传统代理依赖固定不完美目标函数的问题，并在多个科学领域验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前科学发现代理主要依赖科学家指定的定量目标函数，但这些函数往往只是不完美的代理指标，无法完全捕捉复杂科学挑战的本质。自动化目标函数设计成为科学发现代理尚未满足的核心需求。

Method: 提出科学自主目标演化代理（SAGA），采用双层架构：外层LLM代理分析优化结果、提出新目标并将其转化为可计算评分函数；内层在当前目标下执行解决方案优化。这种设计允许系统探索目标空间及其权衡。

Result: 在抗生素设计、无机材料设计、功能性DNA序列设计和化学过程设计等多个应用领域证明，自动化目标制定能显著提升科学发现代理的有效性。

Conclusion: 自动化目标函数设计是科学发现代理的关键进步，SAGA框架通过系统探索目标空间，解决了传统方法中目标函数固定的局限性，为科学发现开辟了新途径。
...
TL;DR: 提出了SpatialBench基准测试，包含146个可验证的空间转录组学分析问题，用于评估AI代理处理真实空间数据集的能力。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学数据规模和复杂性快速增长，计算分析成为生物学发现的主要瓶颈；需要评估前沿AI代理能否从混乱的真实空间数据中提取生物学见解。

Method: 创建包含5种空间技术和7类任务的基准测试；每个问题提供实验数据快照和确定性评分器；评估不同模型家族和任务设计对性能的影响。

Result: 基础模型准确率较低（20-38%），存在明显的模型-任务和模型-平台交互作用；工具设计对性能有显著影响。

Conclusion: SpatialBench可作为测量工具和诊断视角，帮助开发能够忠实、透明、可重复处理真实空间数据集的AI代理。
...
TL;DR: 该论文提出了一种基于博弈论的新视角，将神经网络剪枝视为模型组件间战略互动的均衡结果，并开发了相应的剪枝算法。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法大多将稀疏性视为外部约束，通过启发式重要性评分或训练时正则化实现，缺乏理论解释。本文旨在为剪枝行为提供一种基于博弈论的原理性解释。

Method: 将参数组（权重、神经元或滤波器）建模为连续非合作博弈中的玩家，每个玩家选择其在网络中的参与程度以平衡贡献与冗余竞争。基于此推导出均衡驱动的剪枝算法，联合更新网络参数和参与变量，无需显式重要性评分。

Result: 理论分析表明，在温和条件下，被支配的玩家在均衡时会崩溃为零参与，自然产生稀疏性。在标准基准测试中，该方法实现了具有竞争力的稀疏性-准确性权衡。

Conclusion: 该工作为剪枝行为提供了基于博弈论的原理性解释，并提出了一种可解释、理论基础的剪枝方法，可作为现有方法的替代方案。
...
### [18] [Understanding Virality: A Rubric based Vision-Language Model Framework for Short-Form Edutainment Evaluation](https://arxiv.org/abs/2512.21402)
*Arnav Gupta,Gurekas Singh Sahney,Hardik Rathi,Abhishek Chandwani,Ishaan Gupta,Pratik Narang,Dhruv Kumar*

Main category: cs.CV

TL;DR: 提出基于视觉语言模型的无监督视听特征提取框架，用于评估短视频内容与人类参与度的关联，相比传统指标更具可解释性和扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有视频评估框架（如VideoScore-2）主要关注视觉和语义保真度，但未能捕捉具体视听属性如何驱动真实观众参与度，需要更贴近人类感知的多模态评估方法。

Method: 使用视觉语言模型提取无监督视听特征，通过聚类形成可解释因子，并训练回归评估器预测短视频参与度；构建YouTube Shorts数据集进行系统分析。

Result: 实验显示预测参与度与实际参与度高度相关，基于特征的轻量级评估器在可解释性和扩展性上优于传统指标（如SSIM、FID）。

Conclusion: 通过结合多模态特征重要性和以人为中心的参与度信号，该方法推动了稳健且可解释的视频理解评估框架的发展。
...
### [19] [A Tool Bottleneck Framework for Clinically-Informed and Interpretable Medical Image Understanding](https://arxiv.org/abs/2512.21414)
*Christina Liu,Alan Q. Wang,Joy Hsu,Jiajun Wu,Ehsan Adeli*

Main category: cs.CV

TL;DR: 提出了一种用于医学图像理解的工具瓶颈框架（TBF），通过学习的工具瓶颈模型（TBM）融合视觉语言模型选择的工具输出，解决了文本组合在医学图像特征融合中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的工具组合方法在医学图像理解中表现不佳，因为医学图像的关键信息通常以空间局部特征编码，难以仅通过文本有效融合。

Method: TBF框架：1）使用现成医学VLM从工具箱中选择提取临床相关特征的工具；2）通过神经网络TBM计算并融合工具输出，而非文本组合；3）提出策略使TBM能处理任意VLM工具选择。

Result: 在组织病理学和皮肤病学任务上，TBF性能与深度学习分类器、VLMs及先进工具使用框架相当或更优，在数据有限场景中优势明显，且产生更具可解释性和临床依据的预测。

Conclusion: TBF通过神经网络的工具输出融合机制，显著提升了医学图像理解中工具使用的效果，尤其在处理空间局部特征时优于文本组合方法，增强了预测的临床可解释性。
...
### [20] [Scalable Deep Subspace Clustering Network](https://arxiv.org/abs/2512.21434)
*Nairouz Mrabah,Mohamed Bouguessa,Sihem Sami*

Main category: cs.CV

TL;DR: 提出SDSNet框架，通过地标点近似、联合优化自编码器与自表达目标，实现线性复杂度，解决子空间聚类计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统子空间聚类方法因构建全亲和矩阵和谱分解导致O(n³)计算成本过高，深度学习方法虽改进特征提取但仍存在计算瓶颈。

Method: 采用地标点近似避免全亲和矩阵；联合优化自编码器重建与自表达目标；在因子化表示上直接进行谱聚类；结合卷积自编码器与子空间保持约束。

Result: SDSNet在保持与先进方法相当聚类质量的同时，显著提升了计算效率，实现线性复杂度。

Conclusion: SDSNet通过计算效率优化，为大规模子空间聚类问题提供了可行的深度学习解决方案。
...
### [21] [Intelligent recognition of GPR road hidden defect images based on feature fusion and attention mechanism](https://arxiv.org/abs/2512.21452)
*Haotian Lv,Yuhui Zhang,Jiangbo Dai,Hanli Wu,Jiaji Wang,Dawei Wang*

Main category: cs.CV

TL;DR: 提出MCGA-Net框架，结合数据增强、多模态特征融合和全局注意力机制，实现地下道路缺陷的高精度自动检测。


<details>
  <summary>Details</summary>
Motivation: 传统探地雷达图像解释依赖主观经验，效率低且易出错，需自动化方法解决数据稀缺和复杂背景下的缺陷检测问题。

Method: 采用DCGAN数据增强生成高质量图像；提出MCGA-Net网络，集成多模态链特征融合和全局注意力机制；利用MS COCO迁移学习加速收敛。

Result: MCGA-Net在精度、召回率和mAP@50上分别达到92.8%、92.5%和95.9%；在高斯噪声、弱信号和小目标检测中表现鲁棒且优于其他模型。

Conclusion: 该框架为基于探地雷达的缺陷检测建立了新范式，在复杂地下环境中平衡了计算效率与高精度。
...
### [22] [CCAD: Compressed Global Feature Conditioned Anomaly Detection](https://arxiv.org/abs/2512.21459)
*Xiao Jin,Liang Diao,Qixin Xiao,Yifan Hu,Ziqi Zhang,Yuchen Liu,Haisong Gu*

Main category: cs.CV

TL;DR: 提出CCAD方法，通过压缩全局特征作为重建模型的条件，结合重建和表示学习优势，在异常检测中实现更高AUC和更快收敛


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法存在局限：无监督表示学习方法在域偏移下特征提取不稳定，重建方法训练效率低且因约束不足导致性能下降

Method: CCAD方法将全局特征作为重建模型的新模态条件，设计自适应压缩机制提升泛化能力和训练效率

Result: 实验表明CCAD在AUC指标上优于现有方法，收敛更快，并贡献了重新标注的DAGM 2007数据集

Conclusion: CCAD有效结合两种范式优势，在数据有限的工业异常检测场景中表现出色
...
### [23] [IMA++: ISIC Archive Multi-Annotator Dermoscopic Skin Lesion Segmentation Dataset](https://arxiv.org/abs/2512.21472)
*Kumar Abhishek,Jeremy Kawahara,Ghassan Hamarneh*

Main category: cs.CV

TL;DR: 本文介绍了ISIC MultiAnnot++，这是目前最大的公开多标注者皮肤病变分割数据集，包含14,967张皮肤镜图像和17,684个分割掩码，其中2,394张图像有2-5个标注。
...
Method: 从ISIC档案库收集皮肤镜图像，由多名标注者进行分割标注，记录标注者的技能水平和使用的标注工具，并构建包含多个标注版本的数据集。

Result: 创建了包含17,684个分割掩码的ISIC MultiAnnot++数据集，涵盖14,967张图像，其中2,394张图像有多个标注，提供了标注者元数据和共识分割掩码。

Conclusion: 该数据集为研究标注者特异性偏好建模和标注者元数据分析提供了重要资源，填补了皮肤镜图像多标注者分割数据集的空白。
...
### [24] [BertsWin: Resolving Topological Sparsity in 3D Masked Autoencoders via Component-Balanced Structural Optimization](https://arxiv.org/abs/2512.21769)
*Evgeny Alves Limarenko,Anastasiia Studenikina*

Main category: cs.CV

TL;DR: 提出BertsWin混合架构，结合BERT风格全标记掩码与Swin Transformer窗口，用于3D医学影像的自监督学习，相比标准ViT-MAE基线加速5.8倍语义收敛，训练轮次减少15倍。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习和视觉Transformer方法在2D医学影像表现良好，但应用于3D体数据时面临困难：标准MAE方法丢弃75%标记后难以捕捉三维空间关系，且ViT的二次复杂度计算负担重。

Method: 1. 设计BertsWin混合架构：采用完整3D标记网格（含掩码与可见标记）保持空间拓扑；2. 使用单层局部Swin窗口降低计算复杂度；3. 提出结构优先损失函数；4. 结合GradientConductor优化器；5. 在颞下颌关节锥束CT数据上评估，包括3D CT分割任务。

Result: 1. 语义收敛速度比标准ViT-MAE基线快5.8倍；2. 完整框架仅需44个训练轮次（基线需660轮）达到SOTA重建精度；3. 在标准输入分辨率下保持与稀疏ViT基线相当的理论FLOPs，总计算资源因加速收敛显著降低。

Conclusion: BertsWin通过保持完整三维空间拓扑结构，在无需额外计算代价的前提下，大幅加速3D医学影像自监督学习的收敛速度，为密集体数据处理提供了高效解决方案。
...
### [25] [GPF-Net: Gated Progressive Fusion Learning for Polyp Re-Identification](https://arxiv.org/abs/2512.21476)
*Suncheng Xiang,Xiaoyang Wang,Junjie Jiang,Hejia Wang,Dahong Qian*

Main category: cs.CV

TL;DR: 提出一种名为门控渐进融合网络的新架构，通过门控机制选择性融合多层级特征，用于结肠镜息肉再识别任务，在标准基准测试中优于现有单模态方法。


<details>
  <summary>Details</summary>
Motivation: 结肠镜息肉再识别对于结直肠癌的计算机辅助诊断至关重要，但现有方法的高层特征分辨率较粗，在处理需要细节信息的小型息肉时效果不佳。

Method: 提出门控渐进融合网络，采用全连接方式通过门控机制选择性融合多层级特征，并引入门控渐进融合策略实现语义信息的逐层细化。

Result: 在标准基准测试中，该方法优于最先进的单模态再识别模型，特别是在结合专门的多模态融合策略时表现更佳。

Conclusion: 门控渐进融合网络能有效解决息肉再识别中细节信息丢失的问题，多模态设置与专门融合策略的结合显著提升了识别性能。
...
### [26] [Generative Multi-Focus Image Fusion](https://arxiv.org/abs/2512.21495)
*Xinzhe Xie,Buyu Guo,Bolin Li,Shuangyan He,Yanzhen Gu,Qingyan Jiang,Peiliang Li*

Main category: cs.CV

TL;DR: 提出GMFF生成式多焦点图像融合框架，通过确定性融合和生成式修复两阶段处理，解决现有方法对全场景焦点覆盖的依赖和边缘伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有多焦点图像融合方法需要每个空间位置至少在一张输入图像中清晰对焦，且在复杂场景中易因焦点估计不准或硬选择操作产生边缘伪影。

Method: 1. 确定性融合阶段：使用StackMFF V4结合可用焦平面信息生成初始融合图像；2. 生成式修复阶段：通过IFControlNet利用潜在扩散模型重建缺失焦平面内容、恢复细节并消除边缘伪影。两阶段独立开发、级联运行。

Result: 大量实验表明GMFF达到最先进的融合性能，在复杂多焦点内容场景中展现出实际应用潜力。

Conclusion: GMFF框架通过两阶段级联方法有效解决了多焦点图像融合中的焦点覆盖限制和边缘伪影问题，代码已开源。
...
### [27] [SVBench: Evaluation of Video Generation Models on Social Reasoning](https://arxiv.org/abs/2512.21507)
*Wenshuo Peng,Gongxuan Wang,Tianmeng Yang,Chuanhao Li,Xiaojie Xu,Hui He,Kaipeng Zhang*

Main category: cs.CV

TL;DR: 该研究首次提出了视频生成中的社会推理基准，通过七个核心维度评估现有模型在生成社会性一致行为方面的能力，发现当前模型在表面合理性上表现良好，但在深层次社会认知方面存在系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成模型在视觉真实感、运动保真度和文本-视频对齐方面取得显著进展，但在生成社会性一致行为方面存在根本性限制。与人类能够轻松从视觉线索推断意图、信念、情感和社会规范不同，现有模型倾向于呈现字面场景而缺乏底层因果或心理逻辑。

Method: 研究基于发展心理学和社会心理学发现，将30个经典社会认知范式组织成七个核心维度（心理状态推断、目标导向行动、联合注意、社会协调、亲社会行为、社会规范、多智能体策略），并开发了一个完全无需训练的基于智能体的流程：包括（1）提炼每个实验的推理机制，（2）合成多样化视频就绪场景，（3）通过基于线索的批判实施概念中立和难度控制，（4）使用高容量VLM评估器在五个可解释的社会推理维度上评估生成视频。

Result: 对七个最先进的视频生成系统进行的大规模研究表明，现代模型在表面合理性方面表现出色，但在意图识别、信念推理、联合注意和亲社会推断方面系统性失败，揭示了显著的性能差距。

Conclusion: 当前视频生成模型在社会推理能力方面存在严重不足，需要超越表面视觉真实性的更深层次社会认知建模。提出的基准为系统评估和推进视频生成中的社会智能提供了重要工具。
...
### [28] [Fixed-Budget Parameter-Efficient Training with Frozen Encoders Improves Multimodal Chest X-Ray Classification](https://arxiv.org/abs/2512.21508)
*Md Ashik Khan,Md Nahid Siddique*

Main category: cs.CV

TL;DR: 研究比较了多种参数高效训练策略在胸部X光多标签分类任务中的表现，发现冻结编码器方法在显著减少计算成本的同时实现了优于全微调的诊断性能，但模型校准性较差需要后处理校正。


<details>
  <summary>Details</summary>
Motivation: 传统多模态胸部X光分析通常需要对大型视觉语言模型进行全微调，计算成本高昂。本研究旨在探索参数高效训练策略，在保持性能的同时大幅降低计算需求。

Method: 在印第安纳大学胸部X光数据集上，对比了冻结编码器、BitFit、LoRA和适配器等多种参数高效训练策略。为避免数据泄露，从文本输入中删除了病理学术语但保留临床上下文。在固定参数预算下进行实验，并在CheXpert数据集上进行外部验证。

Result: 所有参数高效训练方法在仅使用2.51%总参数的情况下，AUROC达到0.892-0.908，显著优于全微调的0.770 AUROC。外部验证显示适配器方法表现最佳（0.7214 AUROC）。研究发现性能提升主要来自参数分配而非跨模态协同作用，但所有方法均存在校准性差的问题（ECE: 0.29-0.34）。

Conclusion: 冻结编码器策略在显著降低计算成本的同时提供了优越的判别性能，但校准性差是临床部署前必须解决的关键限制，可通过后处理校准方法进行改善。
...
### [29] [Fixed-Threshold Evaluation of a Hybrid CNN-ViT for AI-Generated Image Detection Across Photos and Art](https://arxiv.org/abs/2512.21512)
*Md Ashik Khan,Arafat Alam Jion*

Main category: cs.CV

TL;DR: 本文提出了一种固定阈值评估协议，用于评估AI生成图像检测器在常见后处理变换下的真实鲁棒性，揭示了不同架构在法医特征与语义模式识别上的性能差异，并为实际部署提供了指导。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法在评估鲁棒性时通常针对每种后处理条件重新调整决策阈值，这会导致鲁棒性估计虚高，掩盖实际部署中的性能下降问题。

Method: 引入固定阈值评估协议（在干净验证数据上选择阈值后固定不变），使用轻量级CNN-ViT混合架构（带门控融合和可选频率增强），在三个操作点（低误报率、ROC最优、最佳F1）下进行系统退化测试，并在多个数据集上进行多种子实验。

Result: 频率增强的CNN在原始照片上表现优异（93.33%准确率），但在压缩下性能骤降（至61.49%）；ViT通过语义模式识别降解最小（92.86%至88.36%）。所有架构在艺术内容上的AUROC比逼真图像高15%（0.901-0.907 vs 0.747-0.759）。混合方法在跨域任务中取得平衡性能：tiny-genimage照片91.4%，AiArtData艺术/图形89.7%，CIFAKE 98.3%。

Conclusion: 固定阈值评估消除了阈值重调带来的虚高问题，揭示了真实的鲁棒性差距：建议对干净照片验证使用CNN，对压缩内容使用ViT，对艺术/图形筛查使用混合架构。语义模式比法医伪影提供更可靠的检测线索。
...
### [30] [MuS-Polar3D: A Benchmark Dataset for Computational Polarimetric 3D Imaging under Multi-Scattering Conditions](https://arxiv.org/abs/2512.21513)
*Puyun Wang,Kaimin Yu,Huayang He,Xianyu Wu*

Main category: cs.CV

TL;DR: 该研究构建了首个公开的水下偏振3D成像基准数据集MuS-Polar3D，并提出两阶段重建流程，用于在可控散射条件下进行算法评估和3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有水下偏振3D成像数据集缺乏散射条件和观测视角的多样性，阻碍了不同方法（如单视图与多视图）的公平比较与算法发展。

Method: 1. 构建包含42个物体、7种定量控制散射条件、5个视角的偏振图像数据集，提供高精度3D模型、法线图和前景掩码；2. 从成像链角度提出“去散射+3D重建”的两阶段流程。

Result: 在复杂散射条件下，基于该数据集的评估实现了最佳平均角度误差15.49度，验证了数据集对多视觉任务（法线估计、分割、去散射、3D重建）的有效支持。

Conclusion: MuS-Polar3D填补了水下偏振3D成像领域基准数据的空白，为可控散射条件下的精确重建和算法公平评估提供了基础，相关数据与代码已开源。
...
### [31] [DiverseGRPO: Mitigating Mode Collapse in Image Generation via Diversity-Aware GRPO](https://arxiv.org/abs/2512.21514)
*Henglin Liu,Huijuan Huang,Jing Wang,Chang Liu,Xiu Li,Xiangyang Ji*

Main category: cs.CV

TL;DR: 本文针对GRPO强化学习在图像生成后期输出同质化、缺乏多样性的问题，提出基于语义分组的分布级创造力奖励和结构感知正则化方法，在保持图像质量的同时显著提升语义多样性。


<details>
  <summary>Details</summary>
Motivation: 传统GRPO方法在训练后期倾向于产生同质化输出，缺乏创造力和视觉多样性，限制了其应用场景。这源于两个问题：一是奖励建模仅依赖单样本质量而忽视分布级多样性；二是正则化未能考虑去噪早期阶段对多样性的主导作用。

Method: 1. 奖励层面：提出基于语义分组的分布级创造力奖励，通过谱聚类对同标题生成样本构建分布级表示，并根据组大小自适应分配探索性奖励以鼓励新视觉模式的发现。2. 生成层面：引入结构感知正则化，在去噪早期施加更强约束以保持多样性而不影响奖励优化效率。

Result: 实验表明，该方法在匹配质量分数下实现了13%-18%的语义多样性提升，为基于GRPO的图像生成建立了新的质量-多样性帕累托前沿。

Conclusion: 通过同时优化奖励建模和生成动态，该方法有效解决了GRPO训练中的多样性退化问题，显著提升了生成图像的视觉多样性和创造力，扩展了强化学习在图像生成中的应用潜力。
...
### [32] [Hierarchy-Aware Fine-Tuning of Vision-Language Models](https://arxiv.org/abs/2512.21529)
*Jiayu Li,Rajesh Gangireddy,Samet Akcay,Wei Cheng,Juhua Hu*

Main category: cs.CV

TL;DR: 提出一种高效的层次感知微调框架，通过结合树路径KL散度和层次兄弟平滑交叉熵损失，在少量参数更新的情况下，使视觉语言模型适应层次分类任务并保持结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将标签视为扁平类别并进行全微调，成本高且在不同分类层级上预测不一致，缺乏对视觉语言模型在层次分类任务上的高效适配研究。

Method: 提出层次感知微调框架，结合树路径KL散度（TP-KL）确保预测沿真实标签路径的垂直一致性，以及层次兄弟平滑交叉熵（HiSCE）促进兄弟类别间预测一致性；两种损失在共享嵌入空间中计算，并与轻量级LoRA适配结合。

Result: 在多个基准测试中，全路径准确率和基于树的不一致错误率均得到持续改进，且参数开销极小。

Conclusion: 该方法为视觉语言模型适应结构化分类体系提供了一种高效策略，在保持预测一致性的同时显著降低了计算成本。
...
### [33] [Vision Transformers are Circulant Attention Learners](https://arxiv.org/abs/2512.21542)
...
<details>
  <summary>Details</summary>
Motivation: 视觉Transformer中的自注意力机制具有二次复杂度，在高分辨率场景下计算负担重，限制了实际应用。现有方法通过引入手工设计的局部性或稀疏性模式会损害模型容量。

Method: 发现视觉Transformer中的自注意力矩阵近似于块循环矩阵，提出将注意力图显式建模为其最近的块循环矩阵，并设计高效计算算法实现快速计算。

Result: 在多种视觉任务上的实验表明，该方法将计算复杂度降低到O(NlogN)，同时基本保持了标准自注意力的容量。

Conclusion: 循环注意力机制是视觉Transformer架构中自注意力的一个有前景的替代方案，实现了高效计算与模型性能的良好平衡。
...
### [34] [Toward Intelligent Scene Augmentation for Context-Aware Object Placement and Sponsor-Logo Integration](https://arxiv.org/abs/2512.21560)
*Unnati Saraswat,Tarun Rao,Namah Gupta,Shweta Swami,Shikhar Sharma,Prateek Narang,Dhruv Kumar*

Main category: cs.CV

TL;DR: 提出两个新任务：上下文感知的对象插入和赞助商产品商标增强，并构建相应数据集以支持这些任务。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型和扩散模型在图像编辑中很少确保插入的对象在上下文中是合适的，特别是在广告和数字媒体领域。

Method: 引入两个新任务，并构建了两个包含类别注释、放置区域和赞助商产品标签的新数据集。

Result: 提出了上下文感知对象插入和赞助商产品商标增强的任务定义，并提供了相应的数据集支持。

Conclusion: 通过新任务和数据集，推动了智能图像编辑在上下文适当性和品牌一致性方面的发展。
...
### [35] [EraseLoRA: MLLM-Driven Foreground Exclusion and Background Subtype Aggregation for Dataset-Free Object Removal](https://arxiv.org/abs/2512.21545)
*Sanghyun Jo,Donghwan Lee,Eunji Jung,Seong Je Oh,Kyungsu Kim*

Main category: cs.CV

TL;DR: 提出EraseLoRA框架，通过背景感知推理和测试时适配实现无数据集的物体移除，避免传统注意力机制导致的非目标前景误判和细节破坏。


<details>
  <summary>Details</summary>
Motivation: 现有无数据集方法通过重定向自注意力机制进行物体移除时，常错误将非目标前景识别为背景导致物体重现，且直接操作注意力会破坏细节并阻碍背景线索的连贯整合。

Method: 1. 背景感知前景排除：利用多模态大语言模型从单张图像-掩码对中分离目标前景、非目标前景和干净背景；2. 背景感知重建与子类型聚合：通过测试时优化将推断的背景子类型作为互补片段，通过重建和对齐目标强制其一致整合。

Result: EraseLoRA作为预训练扩散模型的插件，在物体移除基准测试中持续优于无数据集基线方法，并与基于数据集的方法结果相当。

Conclusion: 该框架通过避免显式注意力干预，实现了更可靠的背景线索提取和细节保持，为无监督物体移除提供了有效解决方案。
...
### [36] [Exploration of Reproducible Generated Image Detection](https://arxiv.org/abs/2512.21562)
*Yihang Duan*

Main category: cs.CV

TL;DR: 本文通过复现AIGC图像检测方法，揭示了该领域可复现性差和泛化性不足的根源，并提出了改进方向。


<details>
  <summary>Details</summary>
Motivation: AIGC图像检测技术在实际应用中面临可复现性差和泛化性不足两大核心问题，阻碍了其实际应用。

Method: 回顾了7篇AIGC检测关键论文，构建轻量级测试数据集，复现有代表性的检测方法，分析预处理步骤和参数设置等隐含细节的影响。

Result: 严格遵循原文核心流程时可复现基本性能，但预处理破坏关键特征或跨生成器测试时检测性能急剧下降；发现现有方法过度拟合特定生成器的专属特征而非学习AIGC图像的通用内在特征。

Conclusion: 研究为提升AIGC检测技术的可复现性提供了实证依据，建议研究者更全面地披露实验细节并验证方法的泛化能力。
...
### [37] [Towards Long-window Anchoring in Vision-Language Model Distillation](https://arxiv.org/abs/2512.21576)
*Haoyi Zhou,Shuo Li,Tianyu Chen,Qi Song,Chonghan Gao,Jianxin Li*

Main category: cs.CV

TL;DR: 提出LAid方法，通过知识蒸馏将大模型的长程注意力机制转移到小模型中，使小模型的有效上下文窗口扩大3.2倍，同时保持视觉语言任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的小型分支因窗口尺寸有限，难以实现语言与视觉的对齐，需要提升其长上下文理解能力。

Method: 提出LAid方法，包含两个组件：(1)渐进距离加权注意力匹配，动态强调训练中的长位置差异；(2)可学习的RoPE响应增益调制，选择性增强位置敏感性。

Result: 实验表明，LAid蒸馏的模型有效上下文窗口比基线小模型扩大达3.2倍，在标准视觉语言基准上保持或提升性能，频谱分析证实其成功保留了关键的低频注意力成分。

Conclusion: LAid为构建高效长上下文视觉语言模型提供了实用技术，并为位置理解在蒸馏中的涌现和转移提供了理论见解。
...
### [38] [SymDrive: Realistic and Controllable Driving Simulator via Symmetric Auto-regressive Online Restoration](https://arxiv.org/abs/2512.21618)
*Zhiyuan Liu,Daocheng Fu,Pinlong Cai,Lening Wang,Ying Liu,Yilong Ren,Botian Shi,Jianqiang Wang*

Main category: cs.CV

TL;DR: SymDrive是一个基于扩散模型的统一框架，用于自动驾驶场景的高质量渲染和编辑，通过对称自回归在线恢复和免训练协调机制实现逼真的新视角合成和车辆插入。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶仿真方法难以同时实现逼真渲染和交互式交通编辑，存在大角度新视角合成困难、资产操作时几何或光照伪影等问题，需要一种能兼顾渲染质量和编辑能力的解决方案。

Method: 提出对称自回归在线恢复范式，通过构建对称视图对以地面实况引导的双视图公式恢复细节，并采用自回归策略生成一致侧视图；利用免训练协调机制，将车辆插入视为上下文感知的图像修复以确保光照和阴影一致性。

Result: 大量实验表明，SymDrive在新视角增强和逼真3D车辆插入方面均达到最先进性能。

Conclusion: SymDrive通过统一的扩散框架成功解决了自动驾驶仿真中渲染质量与编辑能力难以兼得的问题，为长尾数据稀缺场景提供了有效的解决方案。
...
### [39] [TrackTeller: Temporal Multimodal 3D Grounding for Behavior-Dependent Object References](https://arxiv.org/abs/2512.21641)
*Jiahong Yu,Ziqi Wang,Hailiang Zhao,Wei Zhai,Xueqiang Yan,Shuiguang Deng*

Main category: cs.CV

TL;DR: 提出TrackTeller框架，通过多帧观测和时空推理解决动态3D驾驶场景中的语言指代问题，显著提升语言引导的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 动态驾驶场景中，许多语言指代依赖目标的近期运动或短期交互信息，仅凭静态外观或几何特征无法解决，需要研究时序语言引导的3D定位问题。

Method: 提出TrackTeller框架，集成LiDAR-图像融合、语言条件解码和时序推理，构建与文本语义对齐的UniScene表示，生成语言感知的3D提案，并利用运动历史和短期动态优化定位决策。

Result: 在NuPrompt基准测试中，TrackTeller持续提升语言引导跟踪性能，平均多目标跟踪准确率相对提升70%，误报频率降低3.15-3.4倍。

Conclusion: TrackTeller通过统一的时空多模态架构，有效解决了动态3D场景中的时序语言指代问题，为交互式自动驾驶系统提供了重要技术基础。
...
### [40] [LLM-Free Image Captioning Evaluation in Reference-Flexible Settings](https://arxiv.org/abs/2512.21582)
*Shinnosuke Hirano,Yuiga Wada,Kazuki Matsuda,Seitaro Otsuki,Komei Sugiura*

Main category: cs.CV

TL;DR: 提出Pearl，一种无需大语言模型的图像字幕评估指标，适用于有参考和无参考场景，通过新机制学习图像-字幕和字幕-字幕相似度表示，并在多数据集上超越现有LLM-free指标。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的评估指标存在偏向自身生成内容的中立性问题，而多数非大语言模型指标虽无此问题但性能不足，需开发更中立且高性能的图像字幕评估方法。

Method: 设计监督式指标Pearl，引入学习图像-字幕与字幕-字幕相似度表示的新机制，构建包含约33.3万人工标注、来自2360名标注者、覆盖超7.5万图像的数据集用于训练与评估。

Result: Pearl在Composite、Flickr8K-Expert、Flickr8K-CF、Nebula和FOIL数据集上，于有参考和无参考设置中均优于其他现有非大语言模型指标。

Conclusion: Pearl通过结合新颖的相似度学习机制和高质量人工标注数据，实现了对图像字幕更中立且有效的自动评估，为相关任务提供了可靠工具。
...
### [41] [UltraLBM-UNet: Ultralight Bidirectional Mamba-based Model for Skin Lesion Segmentation](https://arxiv.org/abs/2512.21584)
*Linxuan Fan,Juntao Jiang,Weixuan Liu,Zhucun Xue,Jiajun Lv,Jiangning Zhang,Yong Liu*

Main category: cs.CV

TL;DR: 提出UltraLBM-UNet，一种轻量级皮肤病变分割模型，结合双向Mamba全局建模与多分支局部特征感知，在极低参数量下实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有皮肤病变分割方法存在性能不足、计算复杂度高的问题，难以在临床即时诊断场景中部署

Method: 设计轻量级U-Net变体，集成双向Mamba全局建模机制与多分支局部特征感知，采用混合知识蒸馏策略训练超紧凑学生模型

Result: 在ISIC 2017/2018和PH2数据集上达到SOTA分割精度，仅需0.034M参数和0.060 GFLOPs；蒸馏版UltraLBM-UNet-T仅0.011M参数和0.019 GFLOPs仍保持竞争力

Conclusion: UltraLBM-UNet系列模型适合临床即时诊断部署，在资源受限环境下实现准确稳健的病变分析
...
### [42] [Comparative Analysis of Deep Learning Models for Perception in Autonomous Vehicles](https://arxiv.org/abs/2512.21673)
*Jalal Khan*
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆需要高效、安全、可靠的感知系统来检测和识别周围环境中的物体（如车辆、行人、路标）。随着机器学习和深度学习算法的快速发展，有必要比较新兴模型在实际应用场景中的性能，以指导研究社区选择合适的模型。

Method: 研究使用自定义采集的数据集，对YOLO-NAS和YOLOv8两种深度学习模型进行目标检测任务的实验比较，重点关注训练时间和检测精度两个指标。

Result: 实验结果表明：1) YOLOv8s模型的训练时间比YOLO-NAS模型节省75%；2) 在目标检测精度方面，YOLOv8s模型达到83%，优于YOLO-NAS模型的81%。

Conclusion: YOLOv8s模型在训练效率和检测精度方面均优于YOLO-NAS模型，这项比较研究为自动驾驶感知任务中深度学习模型的选择提供了实证参考，有助于研究社区理解模型在真实场景下的性能表现。
...
### [43] [BeHGAN: Bengali Handwritten Word Generation from Plain Text Using Generative Adversarial Networks](https://arxiv.org/abs/2512.21694)
*Md. Rakibul Islam,Md. Kamrozzaman Bhuiyan,Safwan Muntasir,Arifur Rahman Jawad,Most. Sharmin Sultana Samu*

Main category: cs.CV

TL;DR: 提出了一种孟加拉语手写文本生成方法，使用自收集数据集生成多样化的手写单词。


<details>
  <summary>Details</summary>
Motivation: 手写文本生成是一个新兴领域，但孟加拉语作为世界第五大语言，其手写生成研究较少，且缺乏大规模多样化数据集。

Method: 开发自收集的孟加拉语手写数据集（约500名不同年龄和性别贡献者），进行图像预处理，提出基于输入纯文本生成手写单词的方法。

Result: 该方法能够从输入文本生成多样化的手写输出，验证了方法的可行性。

Conclusion: 这项工作推动了孟加拉语手写生成研究，为后续研究提供了数据集和方法基础。
...
### [44] [From Shallow Humor to Metaphor: Towards Label-Free Harmful Meme Detection via LMM Agent Self-Improvement](https://arxiv.org/abs/2512.21598)
*Jian Lang,Rongpei Hong,Ting Zhong,Leiting Chen,Qiang Gao,Fan Zhou*

Main category: cs.CV

TL;DR: 提出首个无需标注的有害表情包检测框架ALARM，利用大模型代理自改进，通过显性表情包学习提升对复杂表情包的检测能力，在三个数据集上表现优于有标注方法。


<details>
  <summary>Details</summary>
Motivation: 现有有害表情包检测方法依赖大规模标注数据，标注成本高且难以适应内容快速演变，需要一种无需标注、能自适应新形式的解决方案。

Method: ALARM框架包含：1）基于置信度的显性表情包识别机制，自动筛选显性表情包并分配伪标签；2）成对学习引导的代理自改进范式，将显性表情包重组为对比对，训练学习代理从对比中提取高级检测线索。

Result: 在三个多样化数据集上实验表明，ALARM具有优越性能和强适应性，能有效应对新演化的表情包，甚至超越有标注驱动的方法。

Conclusion: 无标注框架具有可扩展性和适应性潜力，能为动态在线环境中新型有害表情包检测提供有前景的解决方案。
...
### [45] [GaussianEM: Model compositional and conformational heterogeneity using 3D Gaussians](https://arxiv.org/abs/2512.21599)
*Bintao He,Yiran Cheng,Hongjia Li,Xiang Gao,Xin Gao,Fa Zhang,Renmin Han*

Main category: cs.CV

TL;DR: 提出GaussianEM框架，使用高斯伪原子模型从冷冻电镜图像中同时分析蛋白质的组成和构象异质性，解决连续运动和离散状态混合的挑战。


<details>
  <summary>Details</summary>
Motivation: 蛋白质灵活性及其动态相互作用对功能研究至关重要，但现有方法难以分析同时包含连续运动和离散状态的冷冻电镜数据集。

Method: 采用双编码器-单解码器架构，将图像映射到高斯分量，通过高斯参数变化表示结构变异，建立密度模型与原子模型之间的桥梁。

Result: 在模拟和实验数据集上验证了GaussianEM的有效性，能够直观描述构象变化并保持局部结构一致性。

Conclusion: GaussianEM为冷冻电镜数据分析提供了可解释的框架，能够同时处理构象和组成异质性，促进蛋白质动态研究。
...
TL;DR: 提出自适应四元数交叉融合网络（A-QCF-Net），利用四元数神经网络构建共享特征空间，通过自适应交叉融合模块实现CT与MRI未配对数据间的双向知识迁移，显著提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 多模态医学影像可提供互补信息，但深度学习模型发展受限于配对且空间对齐的大规模数据集稀缺。现有医疗影像档案多为未配对数据，需开发能利用独立数据集的统一模型。

Method: 采用四元数神经网络构建参数高效的共享特征空间；设计自适应四元数交叉融合（A-QCF）模块，通过数据驱动的注意力机制动态调节CT与MRI双流间的信息传递；在未配对的LiTS（CT）和ATLAS（MRI）数据集上联合训练单一模型。

Result: 联合训练模型在CT和MRI上的肿瘤Dice分数分别达到76.7%和78.3%，较单模态nnU-Net基线显著提升5.4%和4.7%；Grad-CAM等可解释性分析证实模型能聚焦相关病理结构。

Conclusion: 该方法为利用医疗领域常见的未配对影像档案提供了稳健且临床可行的范式，通过跨模态知识迁移实现了优于单模态基准的性能提升。
...
TL;DR: 提出一种结合VAE-GAN的混合视频生成模型，通过马尔可夫链框架和记忆机制生成数百至数千帧的长视频，解决传统方法生成长视频时质量下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型（如GAN、VAE、扩散模型）通常只能生成短序列（如16帧），且生成更长视频时质量显著下降，缺乏时间连续性和动态一致性。

Method: 1. 提出VAE-GAN混合结构，结合对抗生成与变分编码的推理能力；2. 采用内容与运动双分支处理；3. 引入基于马尔可夫链的递归机制，将多个短序列生成器串联，实现长视频的时序依赖建模。

Result: 模型能够生成数百至数千帧的长视频，保持时间连续性、内容一致性和动态合理性，克服了传统方法在生成长视频时的质量退化问题。

Conclusion: 通过VAE-GAN混合结构与马尔可夫链递归机制的结合，实现了高质量长视频生成，为视频生成任务提供了可扩展的时序建模方案。
...
### [48] [TAMEing Long Contexts in Personalization: Towards Training-Free and State-Aware MLLM Personalized Assistant](https://arxiv.org/abs/2512.21616)
*Rongpei Hong,Jian Lang,Ting Zhong,Yong Wang,Fan Zhou*

Main category: cs.CV

TL;DR: 提出首个长上下文多模态大语言模型个性化评估基准LCMP，并引入无需训练的状态感知框架TAME，通过双记忆机制和检索对齐增强生成范式提升个性化对话能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM个性化方法仅关注简单的视觉识别和文本替换，缺乏对长上下文对话的支持，无法通过历史对话持续提升交互体验。

Method: 1) 构建LCMP基准评估MLLM对个性化概念变化的感知和上下文响应能力；2) 提出TAME框架，采用双记忆机制分别管理个性化概念的时序变化和持久特征，并结合检索对齐增强生成（RA2G）范式实现知识对齐。

Result: 在LCMP基准测试中，TAME取得最佳性能，在长上下文场景中展现出显著且持续进化的交互体验。

Conclusion: LCMP基准填补了长上下文MLLM个性化评估的空白，TAME框架通过创新的记忆管理和知识对齐机制，为复杂现实场景的个性化对话提供了有效解决方案。
...
### [49] [CausalFSFG: Rethinking Few-Shot Fine-Grained Visual Categorization from Causal Perspective](https://arxiv.org/abs/2512.21617)
...
<details>
  <summary>Details</summary>
Motivation: 现有FS-FGVC方法通常忽略支持样本集作为混淆变量带来的偏差数据分布问题，这会误导判别性特征提取，影响分类性能。

Method: 基于结构因果模型提出CausalFSFG方法，包含两个关键组件：1）干预多尺度编码器（IMSE）进行样本级干预；2）干预掩码特征重建（IMFR）进行特征级干预。

Result: 在CUB-200-2011、Stanford Dogs和Stanford Cars等公开数据集上的大量实验表明，该方法实现了新的最先进性能。

Conclusion: 通过因果干预可以有效消除FS-FGVC中的虚假相关性，揭示从输入到子类别的真实因果关系，显著提升少样本细粒度分类性能。
...
TL;DR: 提出了InstructMoLE框架，通过指令引导的全局路由和输出空间正交性损失，解决了多条件图像生成中任务干扰和语义漂移问题，显著提升了生成质量和指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）在处理多条件任务时存在任务干扰问题，而混合低秩专家（MoLE）的局部路由策略与用户指令的全局性不匹配，导致图像生成中出现空间碎片化和语义漂移。

Method: 1. 提出指令引导的混合低秩专家（InstructMoLE）框架；2. 设计指令引导路由（IGR），基于完整指令生成全局路由信号，统一选择专家组合；3. 引入输出空间正交性损失，促进专家功能多样性，防止表示坍塌。

Result: 在多个具有挑战性的多条件生成基准测试中，InstructMoLE显著优于现有的LoRA适配器和MoLE变体，实现了更好的组合控制和用户意图保真度。

Conclusion: InstructMoLE为生成模型的指令驱动微调提供了一个鲁棒且可泛化的框架，通过全局路由和专家多样性优化，有效解决了复杂图像生成中的语义一致性问题。
...
TL;DR: 提出CellMamba，一种轻量级单阶段检测器，用于病理图像中的细胞检测，通过TMAC模块和自适应Mamba头提升细粒度检测性能，在精度和效率上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 病理图像中细胞检测面临细胞密集、类间差异小、背景复杂等挑战，现有方法在精度和效率上难以兼顾，需要一种轻量且准确的解决方案。

Method: 基于VSSD主干网络，集成CellMamba块（结合NC-Mamba或MSA与TMAC模块），TMAC通过双分支通道分割和注意力融合增强空间判别力；设计自适应Mamba头融合多尺度特征。

Result: 在CoNSeP和CytoDArk0数据集上实验表明，CellMamba在准确率上优于CNN、Transformer和Mamba基线模型，同时显著降低模型大小和推理延迟。

Conclusion: CellMamba为高分辨率细胞检测提供了一种高效且有效的解决方案，验证了其在生物医学实例检测任务中的优越性。
...
### [52] [Training-Free Disentangled Text-Guided Image Editing via Sparse Latent Constraints](https://arxiv.org/abs/2512.21637)
*Mutiara Shabrina,Nova Kurnia Putri,Jefri Satria Ferdiansyah,Sabita Khansa Dewi,Novanto Yudistira*

Main category: cs.CV

TL;DR: 本文分析了PPE框架在图像编辑中的属性纠缠问题，通过引入L1正则化稀疏约束改进潜在空间操作，实现了更聚焦和可控的编辑效果。


<details>
  <summary>Details</summary>
Motivation: 基于文本的图像编辑常出现属性纠缠问题，即修改目标属性时会意外改变其他语义特征（如身份或外观）。原PPE框架的正则化策略存在潜在更新密集、易导致语义泄漏的局限。

Method: 1. 分析PPE框架架构（包括BERT属性预测和StyleGAN2图像生成）；2. 在CelebA-HQ数据集上实证分析；3. 提出基于L1正则化的稀疏约束方法，对潜在空间操作施加稀疏性限制。

Result: 实验结果表明，提出的稀疏约束方法能够实现更聚焦和可控的编辑，有效减少非目标属性的意外改变，同时更好地保持面部身份特征。

Conclusion: 通过引入潜在空间的稀疏约束，PPE框架的属性解纠缠能力得到显著提升，为文本驱动的精准图像编辑提供了更有效的解决方案。
...
TL;DR: 本文使用卷积神经网络（CNN）处理多维原始股票数据（包括拆股/分红事件），预测标普500指数成分股的价格走势，取得了有希望的结果。


<details>
  <summary>Details</summary>
Motivation: 传统股票预测方法多基于数学建模，而近期神经网络的成功为利用机器学习进行预测开辟了新途径。现有研究大多使用单维度或经过加工的金融数据，未能充分处理真实市场数据的复杂性。本文旨在直接使用包含拆股/分红等事件的原始多维市场数据，探索CNN在金融预测中的应用。

Method: 采用卷积神经网络（CNN），将多维股票历史数据矩阵（如开盘价、收盘价、成交量等）视为类似图像的向量进行处理，直接使用原始市场数据而非人工构造的特征。

Result: 模型在预测标普500成分股价格走势方面表现出有希望的结果，支持对单只股票、行业板块或投资组合进行预测。

Conclusion: CNN能够有效处理多维原始金融数据，为股票预测提供了新的技术途径，该方法可直接应用于实际市场数据，具有实用潜力。
...
### [54] [Omni-Weather: Unified Multimodal Foundation Model for Weather Generation and Understanding](https://arxiv.org/abs/2512.21643)
...
TL;DR: Omni-Weather是一个统一天气生成与理解的多模态基础模型，通过共享自注意力机制和因果推理数据集，在两项任务上均达到最先进性能，并证明两者可相互增强。


<details>
  <summary>Details</summary>
Motivation: 现有天气建模方法将准确预测与机理解释分离，缺乏统一框架。为填补这一空白，研究旨在开发能同时处理天气生成与理解的单一架构。

Method: 提出Omni-Weather模型，包含雷达编码器用于天气生成任务，采用共享自注意力机制进行统一处理，并构建用于天气生成因果推理的思维链数据集以提升可解释性和感知质量。

Result: 大量实验表明，Omni-Weather在天气生成和理解任务上均达到最先进性能，且生成与理解任务能相互促进，验证了统一框架的可行性。

Conclusion: 天气生成与理解可在单一模型中统一并相互增强，Omni-Weather证明了该方法的可行性和价值，为天气建模提供了兼顾预测准确性与机理解释的新范式。
...
### [55] [The Deepfake Detective: Interpreting Neural Forensics Through Sparse Features and Manifolds](https://arxiv.org/abs/2512.21670)
*Subramanyam Sahoo,Jared Junkin*

Main category: cs.CV

TL;DR: 本文提出了一种用于深度伪造检测的机制可解释性框架，通过稀疏自编码器和法证流形分析揭示了视觉语言模型内部特征与伪造伪影的关联。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测模型虽然准确率高，但其决策过程仍是不透明的“黑箱”，缺乏对内部机制的理解，这限制了模型的可解释性和鲁棒性改进。

Method: 结合稀疏自编码器分析网络内部表示，并引入新颖的法证流形分析方法，通过控制伪造伪影操作来探测模型特征的响应模式。

Result: 发现每层仅少量潜在特征被激活使用；模型特征流形的几何特性（如本征维度、曲率和特征选择性）随不同深度伪造伪影类型呈现系统性变化。

Conclusion: 该研究为打开深度伪造检测器的“黑箱”迈出了第一步，能够识别特定法证伪影对应的学习特征，并为开发更可解释、更鲁棒的模型提供指导。
...
### [56] [Balancing Accuracy and Efficiency: CNN Fusion Models for Diabetic Retinopathy Screening](https://arxiv.org/abs/2512.21861)
*Md Rafid Islam,Rafsan Jany,Akib Ahmed,Mohammad Ashrafuzzaman Khan*

Main category: cs.CV

TL;DR: 该研究探索了通过融合不同CNN骨干网络的特征来提升糖尿病视网膜病变（DR）二分类筛查的准确性和效率，在多个公开数据集上验证了特征融合方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是导致可预防性失明的主要原因之一，但大规模筛查受到专科医生资源有限以及不同设备和人群采集的图像质量差异大的制约。

Method: 研究使用来自五个公开数据集的11,156张眼底图像，将DR检测构建为二分类任务。比较了三种预训练模型（ResNet50、EfficientNet-B0、DenseNet121）及其两两融合和三融合变体，通过五轮独立实验评估性能。

Result: 特征融合方法 consistently 优于单一骨干网络。其中EfficientNet-B0与DenseNet121的融合模型（Eff+Den）取得了最佳平均性能（准确率82.89%），且在正常类（83.60%）和糖尿病类（82.60%）上F1分数均衡。三融合模型性能接近但计算成本显著更高。推理性能分析显示EfficientNet-B0速度最快（约1.16毫秒/图像），而Eff+Den融合在准确性与延迟之间取得了较好平衡。

Conclusion: 轻量级特征融合能够提升模型在异构数据集上的泛化能力，为需要同时兼顾准确率和处理吞吐量的可扩展DR筛查工作流程提供了支持。
...
### [57] [UniPercept: Towards Unified Perceptual-Level Image Understanding across Aesthetics, Quality, Structure, and Texture](https://arxiv.org/abs/2512.21675)
...
TL;DR: 本文提出了UniPercept-Bench，一个用于评估多模态大语言模型在感知级图像理解能力的统一框架，涵盖美学、质量、结构和纹理四个关键领域，并开发了强基线模型UniPercept。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉理解任务上取得显著进展，但在感知级图像特征理解方面仍存在局限，需要系统性的评估框架和基准。

Method: 建立了层次化定义系统并构建大规模数据集；通过领域自适应预训练和任务对齐强化学习训练UniPercept模型；在视觉评分和视觉问答任务上进行评估。

Result: UniPercept在感知级图像理解任务上优于现有多模态大语言模型，并可作为即插即用的奖励模型用于文本到图像生成。

Conclusion: 本研究定义了多模态大语言模型时代的感知级图像理解，通过引入综合基准和强基线模型，为推进感知级多模态图像理解奠定了坚实基础。
...
### [58] [LVLM-Aided Alignment of Task-Specific Vision Models](https://arxiv.org/abs/2512.21985)
...
<details>
  <summary>Details</summary>
Motivation: 小型视觉模型在关键领域应用广泛，但其解释常显示模型依赖虚假相关性而非人类领域知识，导致实际部署时行为脆弱

Method: LVLM辅助视觉对齐方法，通过双向接口将模型行为转化为自然语言，并将人类类别级规范映射到图像级批评，实现领域专家与模型的有效交互

Result: 在合成和真实数据集上验证显示，方法显著改善了模型行为与人类规范的对齐，有效减少对虚假特征和群体特定偏见的依赖

Conclusion: LVLM-VA方法能够在不需细粒度反馈的情况下，有效对齐小型视觉模型与人类领域知识，增强模型的鲁棒性
...
### [59] [Unsupervised Anomaly Detection in Brain MRI via Disentangled Anatomy Learning](https://arxiv.org/abs/2512.21924)
*Tao Yang,Xiuying Wang,Hao Liu,Guanzhong Gong,Lian-Ming Wu,Yu-Ping Wang,Lisheng Wang*

Main category: cs.CV

TL;DR: 提出一种新的伪健康图像重建框架，通过解耦表示和边缘到图像恢复模块，提升脑MRI多病灶检测在跨模态、跨中心场景下的泛化能力和性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督异常检测方法依赖正常样本学习，但在多模态、多中心MRI数据上泛化能力有限，且重建时异常残留会传播到伪健康图像中，影响检测性能。

Method: 1. 解耦表示模块：将脑MRI分解为成像信息和成像不变的本质解剖图像，引入解剖先验和可微分one-hot编码增强解耦稳定性；2. 边缘到图像恢复模块：仅使用解剖图像的高频边缘信息重建解剖表示，再与成像信息耦合，抑制异常残留并保留正常结构细节。

Result: 在9个公共数据集（4,443例患者的多中心MRI）上评估，优于17种SOTA方法，AP和DSC分别绝对提升18.32%和13.64%。

Conclusion: 所提框架通过解耦成像信息与解剖结构、基于边缘恢复重建，显著提升了脑MRI病灶检测的泛化能力和准确性，在多中心多模态数据上表现优异。
...
### [60] [Contrastive Graph Modeling for Cross-Domain Few-Shot Medical Image Segmentation](https://arxiv.org/abs/2512.21683)
*Yuntian Bo,Tao Zhou,Zechao Li,Haofeng Zhang,Ling Shao*

Main category: cs.CV

TL;DR: 提出C-Graph框架，通过图建模和对比学习解决跨域少样本医学图像分割中域特定信息过滤导致的性能限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有跨域少样本医学图像分割方法通常过滤域特定信息以提升泛化能力，但这会限制跨域性能并降低源域精度，需要一种能利用医学图像结构一致性的方法。

Method: 1. 将图像特征表示为图（像素为节点，语义亲和度为边）；2. 提出结构先验图（SPG）层捕获目标类别节点依赖；3. 设计子图匹配解码（SMD）机制利用节点语义关系指导预测；4. 提出混淆最小化节点对比（CNC）损失增强图空间中节点区分度。

Result: 在多个跨域基准测试中显著优于现有CD-FSMIS方法，实现最先进性能，同时保持源域上的强分割精度。

Conclusion: C-Graph框架通过利用医学图像的结构一致性作为可迁移先验，有效解决了跨域少样本分割中域适应与源域性能保持的平衡问题。
...
TL;DR: 提出LongFly框架，通过历史感知时空建模策略解决无人机视觉语言导航中长时程时空上下文建模难题，提升复杂灾后搜救环境下的导航性能。


<details>
  <summary>Details</summary>
Motivation: 当前无人机视觉语言导航方法在复杂动态环境中难以建模长时程时空上下文，导致语义对齐不准确和路径规划不稳定，限制了灾后搜救等任务的效果。

Method: 1. 基于槽位的历史图像压缩模块：动态蒸馏多视角历史观测为固定长度上下文表示；2. 时空轨迹编码模块：捕捉无人机轨迹的时序动态和空间结构；3. 提示引导的多模态集成模块：融合时空上下文与当前观测，支持时序推理和航点预测。

Result: LongFly在成功率和路径长度加权成功率上分别超越现有最佳基线7.89%和6.33%，在已见和未见环境中均表现一致优越。

Conclusion: LongFly通过结构化压缩历史数据、建模时空轨迹、多模态集成，有效解决了长时程无人机导航中的时空上下文建模问题，显著提升了复杂动态环境下的导航性能。
...
TL;DR: 提出两阶段自回归适应与加速框架，将高保真人体视频扩散模型适配为实时交互式流媒体，实现全身自然对话与倾听行为的生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式人体化身生成方法存在非因果架构和高计算成本问题，无法满足流媒体实时性要求；现有交互方法通常局限于头肩区域，缺乏全身姿态与动作生成能力。

Method: 采用自回归蒸馏与对抗性精炼的两阶段框架，引入参考汇、参考锚定位置重编码策略和一致性感知判别器三个关键组件，确保长期稳定性和一致性。

Result: 方法在生成质量、实时效率和交互自然度方面均超越现有方法，达到最先进水平，支持单次交互生成自然对话与倾听行为。

Conclusion: 所提框架成功解决了实时交互式人体化身的生成挑战，实现了高质量、低延迟的全身动作交互，为数字人研究提供了有效解决方案。
...
### [63] [Analyzing the Mechanism of Attention Collapse in VGGT from a Dynamics Perspective](https://arxiv.org/abs/2512.21691)
*Huan Li,Longjun Luo,Yuling Shi,Xiaodong Gu*

Main category: cs.CV

TL;DR: 本文分析了VGGT模型中全局自注意力层在长序列输入时出现的崩溃现象，建立了数学模型解释该现象，并验证了现有缓解方法的有效性。


<details>
  <summary>Details</summary>
Motivation: VGGT模型在输入序列超过几百帧时会出现注意力矩阵秩退化、token几何结构坍缩到一维子空间、重建误差超线性累积的崩溃现象，需要从理论上解释这一现象。

Method: 将全局注意力迭代视为退化扩散过程，证明token特征流以O(1/L)速率收敛到Dirac型测度，推导出能精确预测经验秩分布的平均场偏微分方程。

Result: 理论模型与实验中观察到的注意力热图演化完全匹配，解释了token合并方法通过降低有效扩散系数来延缓崩溃的机制，且无需额外训练。

Conclusion: 该分析为未来可扩展的3D视觉Transformer提供了理论解释框架，并具有向多模态任务推广的潜力。
...
TL;DR: SlideChain是一个基于区块链的溯源框架，用于验证大规模多模态语义提取的完整性，特别针对教育内容，通过分析医学影像讲座幻灯片展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型（VLMs）生成的教育内容难以验证、复现和审计，尤其是在高风险的STEM领域，模型间的不一致性和缺乏可追溯性影响了AI生成教学材料的可靠性。

Method: 使用SlideChain Slides Dataset（1,117张医学影像讲座幻灯片），从四种先进VLMs中提取概念和关系三元组，构建结构化溯源记录，并将记录的加密哈希锚定在本地EVM兼容区块链上，实现防篡改审计和持久语义基线。

Result: 揭示了多模态教育内容中显著的跨模型差异（概念重叠率低、关系三元组一致性接近零），并在模拟部署条件下验证了SlideChain的燃气消耗、吞吐量和可扩展性，实现了完美的篡改检测和确定性复现。

Conclusion: SlideChain为可信、可验证的多模态教育流程提供了实用且可扩展的解决方案，支持AI辅助教学系统的长期审计、复现和完整性保障。
...
### [65] [ShinyNeRF: Digitizing Anisotropic Appearance in Neural Radiance Fields](https://arxiv.org/abs/2512.21692)
*Albert Barreiro,Roger Marí,Rafael Redondo,Gloria Haro,Carles Bosch*

Main category: cs.CV

TL;DR: 提出ShinyNeRF框架，通过各向异性球面高斯分布和vMF分布混合编码，解决NeRF在建模各向异性镜面反射表面（如拉丝金属）时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF方法难以准确建模各向异性镜面反射表面，限制了文化遗产数字化中高真实感3D重建的应用。

Method: 提出ShinyNeRF框架，联合估计表面法线、切线、镜面反射集中度和各向异性幅度，通过各向异性球面高斯分布和vMF分布混合编码学习出射辐射度的近似表示。

Result: 实验表明ShinyNeRF在各向异性镜面反射数字化方面达到最先进性能，同时提供更合理的物理解释和材料属性编辑能力。

Conclusion: ShinyNeRF有效扩展了NeRF处理各向异性反射的能力，为高保真文化遗产数字化提供了新工具，并支持材料属性的可解释编辑。
...
### [66] [FUSE: Unifying Spectral and Semantic Cues for Robust AI-Generated Image Detection](https://arxiv.org/abs/2512.21695)
*Md. Zahid Hossain,Most. Sharmin Sultana Samu,Md. Kamrozzaman Bhuiyan,Farhad Uz Zaman,Md. Rakibul Islam*

Main category: cs.CV

TL;DR: 提出FUSE混合系统，结合频谱和语义特征进行AI生成图像检测，在多个数据集上表现优异
...
Method: 使用快速傅里叶变换提取频谱特征，结合CLIP视觉编码器的语义特征，通过两阶段渐进训练融合特征

Result: FUSE在Chameleon基准测试中达到最先进水平，GenImage数据集平均准确率91.36%，所有测试生成器平均准确率88.71%，平均精度94.96%

Conclusion: 融合频谱和语义特征的方法能有效提升AI生成图像检测的泛化能力和鲁棒性
...
### [67] [Spatiotemporal-Untrammelled Mixture of Experts for Multi-Person Motion Prediction](https://arxiv.org/abs/2512.21707)
*Zheng Yin,Chengjian Li,Xiangbo Shu,Meiqi Cao,Rui Yan,Jinhui Tang*

Main category: cs.CV

TL;DR: 提出ST-MoE模型，通过四种时空专家和双向时空Mamba，灵活捕捉多人运动中的复杂时空依赖，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要局限：1）依赖位置编码的时空表示不够灵活；2）传统注意力机制二次时间复杂度导致计算成本高。

Method: 使用四种专门捕捉不同时空依赖的专家，并引入双向时空Mamba作为共享专家，以降低计算开销和参数数量。

Result: 在四个多人运动基准数据集上，模型精度优于现有方法，参数量减少41.38%，训练速度提升3.6倍。

Conclusion: ST-MoE能有效平衡模型性能与效率，为多人运动预测提供了一种高效灵活的解决方案。
...
### [68] [Prior-AttUNet: Retinal OCT Fluid Segmentation Based on Normal Anatomical Priors and Attention Gating](https://arxiv.org/abs/2512.21693)
*Li Yang,Yuting Liu*

Main category: cs.CV

TL;DR: 提出Prior-AttUNet模型，通过生成式解剖先验增强黄斑水肿分割，在跨设备OCT图像上实现高精度、高效率的分割。


<details>
  <summary>Details</summary>
Motivation: 光学相干断层扫描（OCT）图像中黄斑水肿区域分割面临边界模糊和跨设备异质性等挑战，影响临床诊断和管理。

Method: 采用混合双路径架构：1）变分自编码器提供多尺度解剖先验；2）分割主干集成密集连接块和空间金字塔池化模块；3）引入解剖先验指导的三重注意力机制动态调制解码特征。

Result: 在RETOUCH基准测试中，对三种OCT设备（Cirrus、Spectralis、Topcon）的平均Dice系数分别达93.93%、95.18%、93.47%，计算成本仅0.37 TFLOPs。

Conclusion: Prior-AttUNet在分割精度与推理效率间取得平衡，有望成为自动化临床分析的可靠工具。
...
### [69] [Scene-VLM: Multimodal Video Scene Segmentation via Vision-Language Models](https://arxiv.org/abs/2512.21778)
*Nimrod Berman,Adam Botach,Emanuel Ben-Baruch,Shunit Haviv Hakimi,Asaf Gendler,Ilan Naiman,Erez Yosef,Igor Kviatkovsky*

Main category: cs.CV

TL;DR: 提出Scene-VLM，首个基于微调视觉语言模型的视频场景分割框架，通过多模态推理和因果依赖处理实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有编码器方法存在视觉中心偏见、忽略序列依赖、缺乏叙事理解和可解释性等问题，需要更全面的视频场景分割方法

Method: 联合处理视觉和文本线索（帧、转录文本、可选元数据），采用因果依赖的序列预测机制，引入上下文聚焦窗口，从token级logits提取置信度分数，通过最小监督生成决策依据

Result: 在标准场景分割基准上达到最先进性能，MovieNet数据集上AP提升+6，F1提升+13.7

Conclusion: Scene-VLM通过多模态推理和序列建模显著提升视频场景分割性能，同时提供可解释的决策依据和可控的精度-召回权衡
...
### [70] [AstraNav-World: World Model for Foresight Control and Consistency](https://arxiv.org/abs/2512.21714)
...
TL;DR: 提出AstraNav-World，一个端到端的世界模型，在统一概率框架中联合推理未来视觉状态和动作序列，通过扩散视频生成器和视觉语言策略的集成实现同步预测，在多种具身导航基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 开放动态环境中的具身导航需要准确预测世界演变和动作执行，传统'先预测后规划'的解耦方法容易产生累积误差，需要更紧密的视觉-动作耦合。

Method: 集成扩散视频生成器和视觉语言策略的统一概率框架，通过双向约束训练：生成动作条件多步视觉预测，并根据预测视觉推导轨迹，实现视觉预测与规划决策的同步更新。

Result: 在多种具身导航基准测试中提高了轨迹准确性和成功率；消融实验证实紧密视觉-动作耦合和统一训练的必要性；真实世界测试展示了卓越的零样本能力，无需微调即可适应新场景。

Conclusion: AstraNav-World通过统一前瞻视觉与控制，捕捉了可迁移的空间理解和规划相关导航动态，推动了在开放真实世界中可靠、可解释、通用具身智能体的发展。
...
TL;DR: RAPTOR是一种实时高分辨率视频预测架构，通过创新的Efficient Video Attention模块和3阶段训练课程，在边缘硬件上首次实现512²分辨率下超过30FPS的性能，显著提升无人机导航任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视频预测方法在追求高分辨率和感知质量时往往牺牲实时性，无法满足密集城市环境中自主无人机对低延迟高分辨率预测的安全需求。迭代生成方法和二次复杂度注意力机制在边缘硬件上难以满足严格性能要求。

Method: 提出RAPTOR架构：1）采用单次前向设计避免迭代方法的误差累积和延迟；2）核心创新Efficient Video Attention模块通过时空轴交替操作将复杂度从O((ST)²)降至O(S+T)；3）配合3阶段训练课程，从粗粒度结构逐步细化到清晰的时间连贯细节。

Result: 在Jetson AGX Orin上首次实现512²分辨率视频预测超过30FPS；在UAVid、KTH和自定义高分辨率数据集上PSNR、SSIM和LPIPS指标达到新SOTA；真实世界无人机导航任务成功率提升18%。

Conclusion: RAPTOR突破了视频预测在分辨率、质量和速度之间的长期权衡，为更安全、更具预见性的具身智能体铺平了道路，特别适用于对延迟敏感的边缘计算应用场景。
...
### [72] [SyncAnyone: Implicit Disentanglement via Progressive Self-Correction for Lip-Syncing in the wild](https://arxiv.org/abs/2512.21736)
*Xindi Zhang,Dechao Meng,Steven Xiao,Qi Wang,Peng Zhang,Bang Zhang*

Main category: cs.CV

TL;DR: 提出SyncAnyone两阶段学习框架，通过扩散视频变换器和无掩码微调，实现高质量AI视频配音，解决现有方法在唇同步、面部动态和背景一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基于掩码的训练方法虽能提升唇同步准确性，但破坏了时空上下文，导致面部动态表现不佳、面部结构和背景一致性不稳定，需要一种能同时实现准确运动建模和高视觉保真度的方法。

Method: 两阶段框架：第一阶段训练基于扩散的视频变换器，对掩码嘴部区域进行修复，生成音频驱动的准确唇部运动；第二阶段开发无掩码微调流程，通过合成伪配对训练数据，优化模型以消除掩码引起的伪影，提升背景一致性。

Result: 大量实验表明，该方法在野外唇同步场景中，在视觉质量、时间连贯性和身份保持方面达到最先进水平。

Conclusion: SyncAnyone框架通过分离运动建模和伪影消除，有效解决了现有方法的局限性，实现了高质量、高保真的AI视频配音。
...
### [73] [AI for Mycetoma Diagnosis in Histopathological Images: The MICCAI 2024 Challenge](https://arxiv.org/abs/2512.21792)
...
TL;DR: 本文介绍了mAIcetoma挑战赛，旨在通过AI技术解决足菌肿诊断难题，特别是针对低资源地区。挑战赛聚焦于从组织病理图像中自动分割足菌肿颗粒并分类其类型，吸引了全球团队参与并取得了高精度结果。


<details>
  <summary>Details</summary>
Motivation: 足菌肿是一种被忽视的热带疾病，诊断困难，尤其在低资源地区缺乏专业病理学家，导致患者和医疗系统承受巨大负担。

Method: 组织mAIcetoma挑战赛，提供标准化数据集MyData，邀请全球团队开发深度学习模型，用于足菌肿颗粒分割和类型分类，并使用评估指标进行模型性能比较。

Result: 五支决赛团队提出的模型均实现了高精度的分割效果，强调了颗粒检测在诊断中的关键作用；表现最佳的模型在足菌肿类型分类上也显示出显著性能。

Conclusion: AI模型能够有效辅助足菌肿诊断，特别是在资源有限的环境中，为自动化诊断工具的开发提供了可行路径。
...
### [74] [Knot Forcing: Taming Autoregressive Video Diffusion Models for Real-time Infinite Interactive Portrait Animation](https://arxiv.org/abs/2512.21734)
*Steven Xiao,XIndi Zhang,Dechao Meng,Qi Wang,Peng Zhang,Bang Zhang*

Main category: cs.CV

TL;DR: 提出Knot Forcing流式框架，通过分块生成、时间节点模块和前瞻机制，实现高保真、时序一致、低延迟的实时肖像动画生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型无法满足流式部署需求，自回归视频生成方法存在误差累积、运动不连续和长期一致性差的问题，而虚拟助手等交互应用需要高保真、低延迟的实时肖像动画。

Method: 1) 分块生成策略：通过缓存参考图像的KV状态保持全局身份，使用滑动窗口注意力进行局部时序建模；2) 时间节点模块：重叠相邻分块并通过图像到视频条件传播时空线索以平滑过渡；3) 前瞻机制：动态更新参考帧的时间坐标，使其语义上下文超前于当前生成帧以保持长期一致性。

Result: Knot Forcing能够在消费级GPU上实现无限序列的高保真、时序一致的实时肖像动画，具有强视觉稳定性和交互性。

Conclusion: 该框架有效解决了流式肖像动画中的误差累积、运动不连续和长期一致性挑战，为交互应用提供了可行的实时生成方案。
...
### [75] [End-to-End 3D Spatiotemporal Perception with Multimodal Fusion and V2X Collaboration](https://arxiv.org/abs/2512.21831)
*Zhenwei Yang,Yibo Ai,Weidong Zhang*

Main category: cs.CV

TL;DR: 提出XET-V2X多模态融合端到端跟踪框架，通过双层级空间交叉注意力统一多视角多模态感知，提升V2X场景下的3D检测与跟踪性能。


<details>
  <summary>Details</summary>
Motivation: V2X场景中遮挡、视角受限和通信延迟等问题影响自动驾驶的3D时空感知可靠性，需要有效的多视角协同感知与多模态融合方法。

Method: 基于多尺度可变形注意力设计双层级空间交叉注意力模块，先聚合多视角图像特征增强语义一致性，再通过更新的空间查询引导点云融合，实现高效跨模态交互。

Result: 在V2X-Seq-SPD真实数据集和V2X-Sim-V2V/V2I模拟基准测试中，在不同通信延迟下均取得检测与跟踪性能的稳定提升，实现复杂交通场景的鲁棒感知。

Conclusion: XET-V2X框架通过统一时空表示和高效跨模态融合，显著提升V2X协同感知的时空稳定性与鲁棒性。
...
### [76] [Scalable Class-Incremental Learning Based on Parametric Neural Collapse](https://arxiv.org/abs/2512.21845)
*Chuangxin Zhang,Guangfeng Lin,Enhui Zhao,Kaiyang Liao,Yajun Chen*

Main category: cs.CV

TL;DR: 提出基于参数神经坍缩的可扩展类增量学习方法，通过自适应层和动态参数化ETF框架解决模型扩展和类别对齐问题，并利用知识蒸馏确保特征一致性。


<details>
  <summary>Details</summary>
Motivation: 现有增量学习方法在扩展模型时忽略结构效率，导致模块间特征差异和类别分布演化引起的类别错位问题。

Method: 提出SCL-PNC方法：1）通过自适应层实现需求驱动、最小成本的主干网络扩展；2）将静态ETF框架改进为根据增量类别动态调整的参数化ETF分类器；3）采用并行扩展框架和知识蒸馏算法对齐扩展模块间的特征。

Result: 在标准基准测试中验证了方法的有效性和效率，能够高效处理现实场景中类别数量增加时的模型扩展问题。

Conclusion: SCL-PNC通过可扩展主干网络、自适应层和参数化ETF分类器的结构化组合，利用神经坍缩原理引导增量扩展模型收敛，解决了类增量学习中的模型扩展和特征一致性问题。
...
### [77] [Few Tokens Matter: Entropy Guided Attacks on Vision-Language Models](https://arxiv.org/abs/2512.21815)
*Mengqi He,Xinyu Tian,Xin Shen,Jinhong Ni,Shu Zou,Zhaoyuan Yang,Jing Zhang*

Main category: cs.CV

TL;DR: 该论文提出一种针对视觉语言模型（VLM）的对抗攻击方法，通过识别生成过程中的关键高熵决策点进行针对性扰动，以较小攻击成本实现语义破坏和安全风险暴露。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵的攻击方法假设所有生成步骤对模型稳定性贡献均等，但研究发现实际仅约20%的高熵token（关键决策点）主导输出轨迹，这为设计更高效的对抗攻击提供了新思路。

Method: 提出熵库引导对抗攻击（EGA），通过分析自回归生成过程中的熵分布，识别关键高熵位置并集中施加扰动，同时验证攻击在不同VLM架构间的可迁移性。

Result: 选择性攻击在多个主流VLM上实现：1）以较小扰动预算达到与全局攻击相当的语义退化效果；2）将35-49%的良性输出转化为有害内容；3）在未见目标模型上实现17-26%的有害转化率（可迁移性）；4）最终攻击成功率可达93-95%。

Conclusion: VLM生成过程中的高熵决策点是安全脆弱性的关键所在，其跨模型重现特性导致对抗攻击具有可迁移性，当前VLM安全机制存在未充分评估的新型风险。
...
### [78] [Diffusion Posterior Sampling for Super-Resolution under Gaussian Measurement Noise](https://arxiv.org/abs/2512.21797)
*Abu Hanif Muhammad Syarubany*

Main category: cs.CV

TL;DR: 该研究探索了扩散后验采样在单图像超分辨率中的应用，通过结合无条件扩散先验和基于梯度的条件约束，在已知退化模型下实现4倍超分辨率重建。研究发现适中的引导尺度能提升重建质量，最佳配置在PS尺度0.95、噪声标准差0.01时获得最优综合评分1.45231。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在单图像超分辨率任务中通常需要针对不同退化算子重新训练，本研究旨在探索无需重新训练扩散模型的后验采样方法，通过平衡扩散先验和测量梯度强度来实现稳定高质量的重建。

Method: 采用扩散后验采样框架，结合无条件扩散先验与基于梯度的似然引导采样过程，通过测量一致性约束实现超分辨率重建。通过调整引导尺度和噪声水平进行消融实验，使用PSNR、SSIM及综合评分(PSNR/40+SSIM)进行评估。

Result: 实验表明：1）适度引导能改善重建质量，最佳配置为PS尺度0.95、噪声标准差0.01，综合评分达1.45231；2）定性结果显示该方法能恢复更清晰的边缘和面部细节；3）与其他条件策略（如MCG和PS退火）相比，该方法在纹理保真度上具有不同权衡特性。

Conclusion: 研究证实了平衡扩散先验与测量梯度强度的重要性，该方法无需针对每个退化算子重新训练扩散模型，即可获得稳定高质量的超分辨率重建，为单图像超分辨率提供了有效的后验采样解决方案。
...
### [79] [Breaking Alignment Barriers: TPS-Driven Semantic Correlation Learning for Alignment-Free RGB-T Salient Object Detection](https://arxiv.org/abs/2512.21856)
*Lupiao Hu,Fasheng Wang,Fangmei Chen,Fuming Sun,Haojie Li*

Main category: cs.CV

TL;DR: 提出TPS-SCL网络，针对未对齐RGB-T图像对的显著目标检测问题，通过TPS对齐和语义相关学习提升模型在真实场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-T显著目标检测方法依赖人工对齐标注数据，难以处理真实场景中未对齐图像对的空间错位、尺度变化等问题，导致性能下降。

Method: 采用双流MobileViT编码器结合Mamba扫描机制；设计语义相关约束模块（SCCM）抑制背景干扰；引入薄板样条对齐模块（TPSAM）校正空间差异；加入跨模态相关模块（CMCM）融合模态依赖。

Result: 在多个数据集上取得轻量级SOD方法的SOTA性能，并超越主流RGB-T SOD方法。

Conclusion: TPS-SCL能有效处理未对齐RGB-T图像对，在保持低参数量和计算开销的同时提升检测性能，适用于真实场景应用。
...
### [80] [Training-free Conditional Image Embedding Framework Leveraging Large Vision Language Models](https://arxiv.org/abs/2512.21860)
*Masayuki Kawarada,Kosuke Yamada,Antonio Tejero-de-Pablos,Naoto Inoue*

Main category: cs.CV

TL;DR: 提出DIOR方法，利用大型视觉语言模型生成条件图像嵌入，无需训练即可根据文本条件提取图像特定特征


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型（如CLIP）虽能提供丰富的图像表示，但无法专注于特定文本条件（如颜色、风格）指示的图像特征，需要一种能根据任意条件提取图像嵌入的方法

Method: 训练免费方法：提示LVLM用与给定条件相关的单个词描述图像，提取模型最后一个标记的隐藏状态向量作为条件图像嵌入

Result: 在条件图像相似性任务中，DIOR优于包括CLIP在内的现有训练免费基线方法，并在多个设置中超越需要额外训练的方法

Conclusion: DIOR提供了一种通用解决方案，无需额外训练或任务先验即可应用于任意图像和条件，展示了LVLM在生成条件图像嵌入方面的潜力
...
### [81] [DPAR: Dynamic Patchification for Efficient Autoregressive Visual Generation](https://arxiv.org/abs/2512.21867)
*Divyansh Srivastava,Akshay Mehra,Pranav Maneriker,Debopam Sanyal,Vishnu Raj,Vijay Kamarshi,Fan Du,Joshua Kimball*

Main category: cs.CV

TL;DR: 提出DPAR模型，通过动态聚合图像token为可变数量patch，减少解码器自回归图像生成的token数量和计算成本
...
Method: 基于轻量无监督自回归模型的下一个token预测熵，动态合并token为更大patch；最小化修改标准解码器架构，保持多模态生成兼容性

Result: 在Imagenet 256和384分辨率上分别减少1.81x和2.06x token数量，训练FLOPs降低达40%，收敛更快，FID相对基线提升达27.1%

Conclusion: DPAR通过动态patch聚合有效减少计算需求，提高生成效率和质量，且能扩展到更大patch尺寸
...
### [82] [EasyOmnimatte: Taming Pretrained Inpainting Diffusion Models for End-to-End Video Layered Decomposition](https://arxiv.org/abs/2512.21865)
*Yihan Hu,Xuelin Chen,Xiaodong Cun*

Main category: cs.CV

TL;DR: 提出了EasyOmnimatte，首个统一、端到端的视频omnimatte方法，通过双专家策略微调视频修复扩散模型，在质量和效率上均超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频omnimatte方法通常依赖缓慢、多阶段或推理时优化流程，未能充分利用强大的生成先验，导致分解效果不理想。

Method: 微调预训练的视频修复扩散模型，采用双专家策略：效果专家（仅在效果敏感的DiT块应用LoRA）捕获前景及关联效果的粗结构；质量专家（全LoRA微调）细化alpha遮罩。采样时早期高噪声步使用效果专家，后期低噪声步切换至质量专家。

Result: 实验表明EasyOmnimatte在视频omnimatte任务上达到新的最优性能，支持多种下游任务，在质量和效率上显著超越基线方法。

Conclusion: 提出的双专家策略有效解决了现有方法在捕获关联效果方面的不足，实现了高质量、高效率的视频前景层分解，为相关应用提供了实用解决方案。
...
### [83] [Fast Inference of Visual Autoregressive Model with Adjacency-Adaptive Dynamical Draft Trees](https://arxiv.org/abs/2512.21857)
*Haodong Lei,Hongsong Wang,Xin Geng,Liang Wang,Pan Zhou*

Main category: cs.CV

TL;DR: 提出ADT-Tree方法，通过自适应调整草稿树的深度和宽度，解决视觉自回归模型中推测解码因图像区域预测难度差异导致的性能下降问题，显著加速图像生成。


<details>
  <summary>Details</summary>
Motivation: 自回归图像模型虽能达到扩散模型质量，但需序列化推理（如576×576图像约需2000步），推测解码在视觉任务中因图像区域预测难度差异导致接受率不一致，限制了加速效果。

Method: 设计邻接自适应动态草稿树（ADT-Tree），利用相邻token状态和先验接受率动态调整草稿树深度与宽度：通过水平邻接初始化，二分法自适应细化，在简单区域生成更深树，复杂区域生成更宽树。

Result: 在MS-COCO 2017和PartiPrompts数据集上分别实现3.13倍和3.05倍加速，且可与LANTERN等松弛采样方法无缝集成以进一步加速。

Conclusion: ADT-Tree通过自适应草稿树结构有效缓解视觉自回归模型中推测解码的接受率不一致问题，显著提升推理速度，为高效图像生成提供新方案。
...
### [84] [SLIM-Brain: A Data- and Training-Efficient Foundation Model for fMRI Data Analysis](https://arxiv.org/abs/2512.21881)
*Mo Wang,Junfeng Xia,Wenhao Ye,Enyu Liu,Kaining Peng,Jianfeng Feng,Quanying Liu,Hongkai Wen*

Main category: cs.CV

TL;DR: 提出SLIM-Brain模型，通过两阶段自适应设计解决fMRI分析中数据效率和训练效率的双重瓶颈，在保持空间细节的同时显著降低计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 现有fMRI基础模型存在数据效率和训练效率的双重瓶颈：基于图谱的方法丢失空间细节且需要大量数据训练；基于体素的方法计算成本过高难以大规模预训练。

Method: 采用两阶段自适应设计：1) 轻量级时间提取器捕获全局上下文并筛选重要数据窗口；2) 4D分层编码器仅从筛选出的窗口学习体素级表征，同时掩码约70%的补丁。

Result: 在七个公共基准测试中取得最先进性能，仅需4千个预训练会话和传统体素方法约30%的GPU内存。

Conclusion: SLIM-Brain成功解决了fMRI基础模型的数据和训练效率问题，为高效、精细的脑功能分析提供了可行方案。
...
### [85] [Reloc-VGGT: Visual Re-localization with Geometry Grounded Transformer](https://arxiv.org/abs/2512.21883)
*Tianchen Deng,Wenhua Wu,Kunzhen Wu,Guangming Wang,Siting Zhu,Shenghai Yuan,Xun Chen,Guole Shen,Zhe Liu,Hesheng Wang*

Main category: cs.CV

TL;DR: 提出首个通过早期融合机制进行多视图空间集成的视觉定位框架Reloc-VGGT，在结构和非结构化环境中均能实现鲁棒运行，支持实时高性能相机姿态估计。


<details>
  <summary>Details</summary>
Motivation: 传统视觉定位方法采用成对姿态回归和后期融合策略，空间信息整合不足，在复杂环境中精度下降，需要更有效的多视图空间集成方法。

Method: 基于VGGT骨干网络编码多视图3D几何，引入姿态标记器和投影模块以更好地利用多数据库视图的空间关系，并提出稀疏掩码注意力策略降低计算成本。

Result: 在约800万姿态图像对上训练的Reloc-VGGT展现出强准确性和显著泛化能力，在多个公开数据集上的实验验证了方法的有效性和效率。

Conclusion: 该框架通过早期融合机制实现了鲁棒的多视图视觉定位，能够在保持对未见环境鲁棒性的同时，实时提供高质量的相机姿态估计。
...
### [86] [High-Fidelity and Long-Duration Human Image Animation with Diffusion Transformer](https://arxiv.org/abs/2512.21905)
...
TL;DR: 提出基于扩散Transformer的框架，通过混合隐式引导信号、位置偏移自适应模块和数据增强策略，实现高保真、长时程的人体图像动画生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成长时程视频时存在困难，且对面部和手部细节的合成能力不足，限制了高质量实际应用。

Method: 1. 设计混合隐式引导信号和锐度引导因子，引入面部和手部细节特征；2. 提出位置偏移自适应模块，支持任意长度视频生成；3. 采用数据增强策略和骨骼对齐模型，减少身份间人体形状差异的影响。

Result: 实验表明，该方法在保真度和时程长度上均优于现有先进方法。

Conclusion: 所提框架能有效解决长时程、高细节人体动画生成的挑战，为高质量应用提供了可行方案。
...
### [87] [Patch as Node: Human-Centric Graph Representation Learning for Multimodal Action Recognition](https://arxiv.org/abs/2512.21916)
*Zeyu Liang,Hailun Xia,Naichuan Zheng*

Main category: cs.CV

TL;DR: 提出PAN框架，通过人体中心图表示学习实现RGB与骨架模态的融合，提升多模态动作识别性能


<details>
  <summary>Details</summary>
Motivation: 现有RGB与骨架多模态融合方法存在异质性问题，未能充分利用两种模态的互补潜力

Method: 1. 将含人体关节点RGB块表示为时空图；2. 提出注意力后校准降低对高质量骨架数据的依赖；3. 开发PAN-Ensemble（双路径图卷积+晚期融合）和PAN-Unified（单网络统一图表示学习）两种变体

Result: 在三个多模态动作识别数据集上，两种变体分别在分离建模和统一建模设置中达到最先进性能

Conclusion: 人体中心图建模能有效抑制RGB冗余，与骨架方法对齐，实现更有效、语义一致的多模态特征融合
...
### [88] [CrownGen: Patient-customized Crown Generation via Point Diffusion Model](https://arxiv.org/abs/2512.21890)
*Juyoung Bae,Moo Hyun Son,Jiale Peng,Wanting Qu,Wener Chen,Zelin Qiu,Kaixin Li,Xiaojuan Chen,Yifan Lin,Hao Chen*

Main category: cs.CV

TL;DR: 提出CrownGen生成框架，通过去噪扩散模型自动设计患者定制牙冠，在几何保真度和设计效率上超越现有方法，临床评估显示其质量不劣于专家手动制作。


<details>
  <summary>Details</summary>
Motivation: 传统牙冠设计依赖手工，耗时费力，成为修复牙科中的瓶颈，需要自动化解决方案来降低成本、缩短周期并提高优质牙科服务的可及性。

Method: 采用基于牙齿级点云表示的生成框架，包含边界预测模块（建立空间先验）和扩散生成模块（单次推理合成多颗牙齿的高保真形态），并在496例外部扫描和26例临床病例中验证。

Result: CrownGen在几何保真度上超越现有最优模型，显著减少主动设计时间；临床评估中，牙医确认其辅助设计的牙冠质量在统计学上不劣于专家技师手动制作的牙冠。

Conclusion: CrownGen通过自动化复杂修复体建模，提供了可扩展的解决方案，有望降低牙科护理成本、缩短交付时间并提升患者获得高质量治疗的机会。
...
### [89] [AutoPP: Towards Automated Product Poster Generation and Optimization](https://arxiv.org/abs/2512.21921)
...
TL;DR: 提出AutoPP自动化流水线，用于产品海报生成与优化，通过统一设计模块和元素渲染模块生成海报，并利用在线反馈优化点击率，在公开数据集上取得最佳效果。


<details>
  <summary>Details</summary>
Motivation: 手动制作和优化产品海报耗时耗力，需要自动化解决方案来提升效率并基于在线表现进行优化。

Method: 使用统一设计模块整合背景、文本和布局三要素，通过元素渲染模块编码为条件令牌生成海报；优化器利用在线反馈，通过元素替换和隔离直接偏好优化（IDPO）提升点击率。

Result: AutoPP在离线和在线实验中均达到最先进水平，其数据集AutoPP1M包含100万高质量海报和用户反馈。

Conclusion: AutoPP实现了产品海报生成与优化的全自动化，显著提升点击率，为电商海报设计提供了高效解决方案。
...
### [90] [Data relativistic uncertainty framework for low-illumination anime scenery image enhancement](https://arxiv.org/abs/2512.21944)
*Yiquan Gao,John See*

Main category: cs.CV

TL;DR: 提出针对动漫场景图像的低光照增强方法DRU，通过数据相对不确定性框架动态调整目标函数，在无配对数据集上实现优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 现有低光照增强研究主要针对自然图像和视频，缺乏针对动漫场景图像的专门方法，存在领域差距；且该任务面临数据稀缺和光照条件多样性的挑战。

Method: 1. 构建无配对的动漫场景数据集；2. 提出数据相对不确定性（DRU）框架，借鉴相对论GAN思想，类比光的波粒二象性定义光照不确定性；3. 利用不确定性动态调整目标函数，重新校准模型学习。

Result: 通过在多个EnlightenGAN版本上训练，DRU框架在感知质量和美学质量上均超越现有方法，这些方法无法从数据不确定性角度进行学习。

Conclusion: DRU框架为低光照动漫场景增强提供了新范式，展示了数据不确定性学习的重要性，并有望推广到其他视觉和语言领域。
...
### [91] [Look Closer! An Adversarial Parametric Editing Framework for Hallucination Mitigation in VLMs](https://arxiv.org/abs/2512.21999)
*Jiayu Hu,Beibei Li,Jiangwei Xia,Yanjun Qin,Bing Ji,Zhongshi He*

Main category: cs.CV

TL;DR: 提出ALEAHallu框架，通过对抗性参数编辑缓解视觉语言模型中的幻觉问题，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在持续幻觉问题，现有启发式解码校准方法因不可训练而优化潜力有限。

Method: 采用激活-定位-编辑对抗范式：构建包含正负样本的激活数据集，识别幻觉敏感参数簇，使用对抗性前缀微调参数。

Result: 在生成式和判别式VLM任务上评估，ALEAHallu显著缓解了幻觉现象。

Conclusion: 对抗性参数编辑框架能有效迫使模型优先考虑视觉证据，减轻对语言先验的过度依赖。
...
### [92] [Automated Discovery of Parsimonious Spectral Indices via Normalized Difference Polynomials](https://arxiv.org/abs/2512.21948)
*Ali Lotfi,Adam Carter,Thuan Ha,Mohammad Meysami,Kwabena Nketia,Steve Shirtliffe*

Main category: cs.CV

TL;DR: 提出自动化方法寻找紧凑光谱指数用于植被分类，通过构建多项式组合特征并筛选，在Sentinel-2影像上实现高精度杂草检测。


<details>
  <summary>Details</summary>
Motivation: 传统光谱指数设计依赖专家经验，缺乏系统性搜索方法；需要保持光照不变性且易于解释的自动化指数生成方案。

Method: 1. 基于所有波段对生成归一化差异基特征；2. 构建固定阶数多项式组合扩展特征空间；3. 采用ANOVA过滤、递归消除和L1正则化SVM进行特征选择。

Result: 在加拿大萨斯喀彻温省的藜检测任务中：单个二阶指数（红边波段归一化差异乘积）达到96.26%准确率；8个指数组合提升至97.70%；关键特征均来自b4-b8波段交互项。

Conclusion: 光谱交互作用比单一波段比值更具判别力；该方法生成的指数计算简单，可直接部署于遥感平台（如GEE），且通用性强（开源工具ndindex）。
...
### [93] [Perceive and Calibrate: Analyzing and Enhancing Robustness of Medical Multi-Modal Large Language Models](https://arxiv.org/abs/2512.21964)
*Dunyuan XU,Xikai Yang,Yaoqian Li,Juzheng Miao,Jinpeng Li,Pheng-Ann Heng*

Main category: cs.CV

TL;DR: 本文系统分析了多模态噪声对医疗MLLMs的影响，并提出无需训练的跨模态鲁棒性增强框架IMC，包含视觉去噪校准PDC和文本多智能体去噪系统SMS，在构建的噪声基准测试中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有医疗多模态大语言模型对真实世界扰动（如图像伪影、文本错误）敏感，且现有鲁棒性研究多集中于文本模态并依赖微调，无法满足医学领域复杂噪声模式与严格安全标准的需求。

Method: 提出基于感知-校准原则的训练免费框架IMC：针对视觉模态设计PDC，利用模型自身视觉编码器识别噪声模式并进行原型引导的特征校准；针对文本模态设计SMS，通过多智能体协作层次利用模型自评估能力优化噪声文本。

Result: 在包含11种跨模态噪声类型的基准测试中，该方法在多个模态上达到最先进性能，显著提升模型对临床场景扰动的鲁棒性。

Conclusion: IMC框架能有效利用医疗MLLMs固有的去噪能力增强跨模态鲁棒性，无需额外训练，为提升临床场景下的模型安全性提供了可行方案。
...
### [94] [iSHIFT: Lightweight Slow-Fast GUI Agent with Adaptive Perception](https://arxiv.org/abs/2512.22009)
*Sarthak Mehrotra,Sairam V C Rebbapragada,Mani Hemanth Reddy Bonthu,Vineeth N Balasubramanian*

Main category: cs.CV

TL;DR: 本文提出iSHIFT，一种轻量级GUI代理，通过隐式思维链和感知控制模块，实现慢速高精度和快速高效两种推理模式的切换，在2.5B参数规模下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在GUI交互中难以兼顾高效执行常规任务和精确视觉定位，且模型庞大、无法根据任务动态调整推理深度。

Method: 提出iSHIFT框架：1）隐式慢-快混合推理，慢模式利用详细视觉信息实现高精度，快模式依赖全局线索提升效率；2）感知控制模块通过特殊感知令牌引导注意力至相关屏幕区域。

Result: 在多个基准数据集上，仅2.5B参数的iSHIFT达到了最先进性能水平。

Conclusion: iSHIFT通过动态推理模式切换和注意力引导机制，实现了轻量化GUI代理在精度与效率上的平衡，为MLLM的实际应用提供了可行方案。
...
### [95] [Patch-Discontinuity Mining for Generalized Deepfake Detection](https://arxiv.org/abs/2512.22027)
*Huanhuan Yuan,Yang Ping,Zhengqin Xu,Junyi Cao,Shuai Jia,Chao Ma*

Main category: cs.CV

TL;DR: 提出GenDF框架，利用大规模视觉模型进行深度伪造检测，通过特定表示学习、特征空间重分布和分类不变特征增强，在跨域和跨操作场景中实现优异泛化性能。


<details>
  <summary>Details</summary>
Motivation: 生成式AI能创建高度逼真的伪造人脸图像，威胁个人隐私和在线信息完整性。现有深度伪造检测方法依赖手工特征和复杂架构，在未见伪造模式时性能显著下降。

Method: 1) 将大规模视觉模型迁移至深度伪造检测任务；2) 深度伪造特定表示学习；3) 特征空间重分布缓解分布不匹配；4) 分类不变特征增强策略提升泛化能力。

Result: GenDF在跨域和跨操作设置中达到最先进泛化性能，仅需0.28M可训练参数，验证了框架的有效性和高效性。

Conclusion: GenDF通过简洁网络设计实现了强大的深度伪造检测泛化能力，为解决未知伪造模式检测问题提供了有效方案。
...
TL;DR: 提出LMSF-A轻量级多尺度注意力框架，用于脊柱内窥镜实时实例分割，在保持高精度的同时显著降低计算成本，并发布临床标注的PELD数据集。


<details>
  <summary>Details</summary>
Motivation: 脊柱内窥镜手术中实时实例分割对识别关键解剖结构至关重要，但面临视野狭窄、反光、烟雾/出血、边界模糊和尺度变化大等挑战，且受限于手术硬件需平衡精度与速度，并需在小批量（甚至批量1）训练下保持稳定。

Method: 提出LMSF-A框架：1）骨干网络使用结合重参数化卷积与高效多尺度注意力的C2f-Pro模块；2）颈部采用尺度序列特征融合（SSFF）和三重特征编码（TFE）增强跨尺度一致性与边界细节；3）头部采用轻量多任务共享头（LMSH）减少参数并支持批量1稳定性。

Result: LMSF-A在各项评估指标上具有竞争力（甚至优于多数方法），仅需180万参数和8.8 GFLOPs，显著轻于现有实例分割方法，并在公开牙齿基准测试中表现出良好泛化能力。

Conclusion: LMSF-A框架在脊柱内窥镜实例分割任务中实现了精度与效率的平衡，其轻量设计适合硬件受限的手术场景，发布的PELD数据集（61例患者610张图像）为临床研究提供了资源。
...
### [97] [Yume-1.5: A Text-Controlled Interactive World Generation Model](https://arxiv.org/abs/2512.22096)
*Xiaofeng Mao,Zhen Li,Chuanhao Li,Xiaojie Xu,Kaining Ying,Tong He,Jiangmiao Pang,Yu Qiao,Kaipeng Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为\method的新框架，用于从单张图像或文本提示生成逼真、交互式和连续的世界，支持键盘探索，解决了现有方法参数过大、推理步骤长和历史上下文增长快的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型生成交互式世界的方法存在参数规模过大、依赖冗长推理步骤、历史上下文快速增长等关键挑战，严重限制了实时性能，且缺乏文本控制生成能力。

Method: 框架包含三个核心组件：(1) 集成统一上下文压缩与线性注意力的长视频生成框架；(2) 基于双向注意力蒸馏和增强文本嵌入方案的实时流加速策略；(3) 用于生成世界事件的文本控制方法。

Result: \method能够从单张图像或文本提示生成逼真、交互式和连续的世界，支持键盘探索，实现了实时性能并具备文本控制生成能力。

Conclusion: 该框架通过创新的上下文压缩、注意力优化和文本控制方法，有效解决了现有交互式世界生成方法的实时性和可控性瓶颈，为生成高质量可探索世界提供了可行方案。
...
### [98] [Backdoor Attacks on Prompt-Driven Video Segmentation Foundation Models](https://arxiv.org/abs/2512.22046)
*Zongmin Zhang,Zhen Sun,Yifan Liao,Wenhan Dong,Xinlei He,Xingshuo Han,Shengmin Xu,Xinyi Huang*

Main category: cs.CV

TL;DR: 本文提出了首个针对提示驱动视频分割基础模型（VSFM）的后门攻击框架BadVSFM，通过两阶段训练策略实现高攻击成功率，同时保持正常样本的分割质量，揭示了当前VSFM模型的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有经典后门攻击方法（如BadNet）在VSFM上攻击成功率极低（<5%），因为传统训练中干净样本和触发样本的梯度方向一致，注意力仍聚焦真实目标，导致模型无法学习到与触发器相关的特征表示。

Method: 提出BadVSFM两阶段攻击框架：1）引导图像编码器，使触发帧映射到指定目标嵌入，干净帧保持与参考编码器对齐；2）训练掩码解码器，使不同提示类型下的触发帧-提示对生成统一目标掩码，干净输出接近参考解码器。

Result: 在两个数据集和五个VSFM模型上的实验表明，BadVSFM在不同触发器和提示类型下均实现强可控后门效果（攻击成功率显著提升），同时保持干净样本分割质量。消融实验验证了两阶段设计的必要性及对超参数变化的鲁棒性。

Conclusion: BadVSFM成功分离了触发与干净样本的表征，并将注意力转移至触发区域，而现有四种典型防御方法均无效，揭示了当前VSFM模型存在未被充分探索的安全漏洞。
...
### [99] [MAI-UI Technical Report: Real-World Centric Foundation GUI Agents](https://arxiv.org/abs/2512.22047)
...
TL;DR: MAI-UI是一系列基础GUI代理，通过自演进数据管道、设备-云协作系统和在线强化学习框架，解决了GUI代理部署中的关键挑战，在多个基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: GUI代理的发展可能彻底改变下一代人机交互。然而，当前面临缺乏原生代理-用户交互、仅限UI操作的局限性、缺乏实际部署架构以及在动态环境中的脆弱性等挑战。

Method: 采用统一方法：自演进数据管道扩展导航数据以包含用户交互和MCP工具调用；原生设备-云协作系统根据任务状态路由执行；在线强化学习框架通过并行环境扩展和上下文长度优化进行扩展。

Result: 在GUI基础任务中，ScreenSpot-Pro达到73.5%，MMBench GUI L2达到91.3%，OSWorld-G达到70.9%，UI-Vision达到49.2%；在移动GUI导航中，AndroidWorld达到76.7%，MobileWorld达到41.7%。在线强化学习实验显示，并行环境从32扩展到512带来+5.2分提升，环境步数预算从15增加到50带来+4.3分提升。设备-云协作系统使设备性能提升33%，云模型调用减少40%以上。

Conclusion: MAI-UI通过创新的架构设计解决了GUI代理部署的关键挑战，在多个基准测试中超越了现有最先进模型，同时通过设备-云协作系统显著提升了性能、效率和隐私保护。
...
TL;DR: 提出了一种基于检测的跟踪方法TDLP，通过轨迹与检测之间的链接预测进行逐帧关联，无需手工设计规则，在多个基准测试中超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪方法存在局限性：基于检测的跟踪方法依赖手工设计的关联启发式规则，而端到端方法计算复杂度高。需要一种既能从数据中学习关联，又能保持模块化和计算效率的方法。

Method: 采用跟踪-检测链接预测（TDLP）方法，通过预测轨迹在每一帧的正确延续来进行关联。主要利用边界框等几何特征，可选地结合姿态和外观等额外线索，架构设计保持模块化。

Result: 在多个基准测试上的实验表明，TDLP在基于检测的跟踪和端到端方法中都 consistently 超越了最先进的性能。分析显示链接预测比基于度量学习的关联更有效，特别是在处理异构特征时。

Conclusion: TDLP是一种高效且有效的多目标跟踪方法，通过链接预测学习关联，避免了手工规则，同时在性能和计算效率上优于现有方法。
...
### [101] [See Less, See Right: Bi-directional Perceptual Shaping For Multimodal Reasoning](https://arxiv.org/abs/2512.22120)
*Shuoshuo Zhang,Yizhen Zhang,Jingjing Fu,Lei Song,Jiang Bian,Yujiu Yang,Rui Wang*

Main category: cs.CV

TL;DR: 提出双向感知塑形（BiPS）方法，通过双向视觉信号训练提升视觉语言模型的细粒度视觉依赖能力


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型依赖外部工具或潜在视觉标记，但忽略了细粒度视觉证据（如图表中的折线），存在跨域泛化差、推理成本高的问题

Method: BiPS通过KL一致性约束（原始图像与保留证据视图）确保完整覆盖支持像素，通过KL分离约束（原始图像与掩码关键像素视图）防止文本捷径，强制模型依赖细粒度视觉信息

Result: 在8个基准测试中，BiPS将Qwen2.5-VL-7B模型平均提升8.2%，并在未见数据集和图像类型上展现出强大的跨域泛化能力

Conclusion: BiPS通过双向感知塑形有效增强视觉语言模型对细粒度视觉证据的依赖，提升性能的同时改善跨域泛化
...
### [102] [ProEdit: Inversion-based Editing From Prompts Done Right](https://arxiv.org/abs/2512.22118)
*Zhi Ouyang,Dian Zheng,Xiao-Ming Wu,Jian-Jian Jiang,Kun-Yu Lin,Jingke Meng,Wei-Shi Zheng*

Main category: cs.CV

TL;DR: 提出ProEdit方法，通过KV-mix和Latents-Shift技术改进基于反转的视觉编辑，在保持背景一致性的同时增强编辑灵活性，实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于反转的视觉编辑方法在采样过程中过度依赖源图像信息，导致编辑目标图像时难以有效改变主体属性（如姿态、数量、颜色），影响编辑效果。

Method: 1. 注意力层面：提出KV-mix，混合源图像和目标图像在编辑区域的KV特征，减少源图像对编辑区域的影响；2. 潜在空间层面：提出Latents-Shift，扰动源潜在空间的编辑区域，消除反转潜在对采样的影响。

Result: 在多个图像和视频编辑基准测试中达到SOTA性能，且方法为即插即用，可无缝集成到现有反转和编辑方法中。

Conclusion: ProEdit通过双重改进有效平衡了编辑一致性与灵活性，解决了现有方法过度依赖源信息的问题，提升了视觉编辑的质量和可控性。
...
### [103] [Teaching People LLM's Errors and Getting it Right](https://arxiv.org/abs/2512.21422)
*Nathan Stringham,Fateme Hashemi Chaleshtori,Xinyuan Yan,Zhichao Xu,Bei Wang,Ana Marasović*

Main category: cs.CL

TL;DR: 本文探讨了为什么通过向用户传授LLM失败模式来减轻过度依赖的方法未能完全成功，发现失败模式确实存在，但自动发现方法效果不一，并提出新的评估指标显示教学可能有效。


<details>
  <summary>Details</summary>
Motivation: 用户常因看到LLM能完成复杂任务而错误地认为其不会在基础任务上出错，导致过度依赖。先前研究试图通过聚类失败实例并教授用户失败模式来缓解此问题，但效果有限，本文旨在探究原因。

Method: 首先基于元标签对两个数据集中的实例分组，评估LLM在各组的预测表现，识别出规模较大且LLM易出错的组作为可教授的失败模式；然后测试提示和嵌入方法能否自动发现这些已知失败；最后提出新指标评估用户利用失败模式预测LLM错误的能力，并通过用户研究验证。

Result: 研究发现确实存在可教授的失败模式，但自动发现方法效果参差不齐；使用新提出的评估指标时，用户研究显示教学对提高用户预测LLM错误的能力有积极效果，而传统的人机协作准确率指标未显示此效果。

Conclusion: 教授失败模式可能是减轻用户对LLM过度依赖的有效方法，但其成功依赖于改进自动失败发现方法，并采用能准确反映用户学习效果的评估指标（如本文所提指标）。
...
### [104] [Morality is Contextual: Learning Interpretable Moral Contexts from Human Data with Probabilistic Clustering and Large Language Models](https://arxiv.org/abs/2512.21439)
*Geoffroy Morlat,Marceau Nahon,Augustin Chartouny,Raja Chatila,Ismael T. Freire,Mehdi Khamassi*

Main category: cs.CL

TL;DR: COMETH框架通过结合概率上下文学习、LLM语义抽象和人类道德评价，建模上下文如何影响模糊行为的可接受性，在道德判断任务上比端到端LLM提示的准确率提高约一倍。


<details>
  <summary>Details</summary>
Motivation: 道德判断不仅取决于行为结果，还受上下文情境影响。现有端到端LLM方法在道德预测中缺乏可解释性，且难以系统建模上下文对道德评价的影响机制。

Method: 1) 构建包含300个场景、6类核心行为的实证数据集，收集101名参与者的三元判断；2) 使用LLM过滤和MiniLM嵌入+K-means进行行为标准化和聚类；3) 基于人类判断分布，通过差异准则在线聚类学习行为特定的道德上下文；4) 泛化模块提取非评价性二元上下文特征，在基于似然的透明模型中学习特征权重。

Result: COMETH与多数人类判断的一致性约为60%，相比端到端LLM提示（约30%）提高约一倍。框架能识别驱动预测的关键上下文特征，提供可解释的道德预测。

Conclusion: COMETH提供了一种可解释的替代方案，将人类判断与基于模型的上下文学习及LLM语义相结合，在上下文敏感的道德预测和解释方面优于纯端到端LLM方法。
...
### [105] [Oogiri-Master: Benchmarking Humor Understanding via Oogiri](https://arxiv.org/abs/2512.21494)
*Soichiro Murakami,Hidetaka Kamigaito,Hiroya Takamura,Manabu Okumura*

Main category: cs.CL

TL;DR: 本文通过日本创意游戏Oogiri研究幽默生成，构建了Oogiri-Master基准和Oogiri-Corpus数据集，分析了语言因素与幽默感的关系，并评估了大语言模型在幽默理解上的表现。


<details>
  <summary>Details</summary>
Motivation: 幽默是测试大语言模型类人创造性思维的重要场景，但现有研究缺乏可靠方法评估幽默感，数据集存在候选回复少、评分受流行度偏见影响、缺乏客观可比指标等问题。

Method: 构建Oogiri-Corpus数据集（每个提示约100个候选回复，由约100名评委独立评分以减少偏见），定量分析文本长度、歧义性、不一致性解决等语言因素与幽默感的关系，并在Oogiri-Master基准上测试多种大语言模型和人类基线。

Result: 研究发现最先进的大语言模型在幽默理解上接近人类水平，通过洞察增强提示可进一步提升模型性能；同时建立了预测人类幽默判断的客观指标。

Conclusion: 该研究为大语言模型的幽默理解评估与提升提供了系统化基础，表明模型在幽默生成任务上具有接近人类的潜力。
...
### [106] [Beyond Heuristics: A Decision-Theoretic Framework for Agent Memory Management](https://arxiv.org/abs/2512.21567)
*Changzhi Sun,Xiangyu Chen,Jixiang Luo,Dell Zhang,Xuelong Li*

Main category: cs.CL

TL;DR: 提出DAM框架，将大语言模型的外部内存管理重构为不确定性下的序列决策问题，通过价值函数和不确定性估计评估操作，替代传统启发式方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的外部内存管理主要依赖人工设计的启发式规则，难以预测内存读写决策对长期交互和下游任务的影响，缺乏对不确定性和延迟效用的系统处理。

Method: 提出DAM（决策理论代理内存）框架，将内存管理分解为即时信息访问和分层存储维护，通过价值函数和不确定性估计评估候选操作，基于长期效用和风险估计制定聚合策略。

Result: DAM框架为内存管理提供了原则性重构，明确了启发式方法的局限性，并为未来不确定性感知内存系统研究奠定了基础。

Conclusion: 内存管理应被视为不确定性下的序列决策问题，DAM框架通过决策理论方法为构建更智能、可预测的内存系统提供了理论框架。
...
TL;DR: 本文提出了一个统一的幻觉定义，将其视为语言模型内部世界建模的不准确性，并通过不同参考世界模型和知识冲突策略解释了现有文献中的各种定义。


<details>
  <summary>Details</summary>
Motivation: 尽管神经语言模型自诞生以来已有多次解决幻觉问题的尝试，但即使在当前的前沿大语言模型中，幻觉仍然存在。本文旨在通过统一的历史视角梳理幻觉定义，为评估和缓解技术提供共同语言。

Method: 从历史角度梳理文献中的幻觉定义，将其归纳为一个核心定义：幻觉是内部世界建模的不准确性，并通过变化参考世界模型（如知识库、上下文）和知识冲突策略来区分不同定义。

Result: 提出了一个统一的幻觉定义框架，强调评估需明确其假设的“世界”或真相来源，并区分幻觉与规划或奖励相关错误。该框架为比较基准和缓解技术提供了共同基础。

Conclusion: 基于统一定义，本文规划了一系列基准测试，通过使用完全指定的合成世界模型来定义幻觉，以测试和改进语言模型的世界建模组件。
...
### [108] [Gamayun's Path to Multilingual Mastery: Cost-Efficient Training of a 1.5B-Parameter LLM](https://arxiv.org/abs/2512.21580)
...
TL;DR: Gamayun是一个15亿参数的多语言模型，采用两阶段预训练策略，在有限计算资源下实现了超越更大训练规模模型的性能表现，尤其在俄语任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 针对资源受限环境中缺乏小型非英语中心化语言模型的研究空白，旨在开发高效可部署的多语言模型。

Method: 采用新颖的两阶段预训练策略：第一阶段进行平衡的多语言训练以实现跨语言对齐，第二阶段通过高质量英语数据增强来提升所有语言性能。

Result: 在显著更少的训练数据（2.5T tokens）下，超越LLaMA3.2-1B（9T tokens）的所有基准测试，在广泛英语和多语言任务上优于Qwen2.5-1.5B（18T tokens），在非高级STEM任务中匹配或超越Qwen3（36T tokens），在俄语MERA基准上达到同规模模型（1-2B参数）的SOTA。

Conclusion: 两阶段预训练策略能有效提升小规模多语言模型的性能，证明通过精心设计的训练方法可以在有限计算预算下实现卓越的多语言能力，特别是为俄语等资源语言提供了高效解决方案。
...
### [109] [Rethinking Sample Polarity in Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2512.21625)
*Xinyu Tang,Yuliang Zhan,Zhixun Li,Wayne Xin Zhao,Zhenduo Zhang,Zujie Wen,Zhiqiang Zhang,Jun Zhou*

Main category: cs.CL

TL;DR: 本文系统研究了强化学习验证奖励训练中正负样本对推理模型的影响，并提出A3PO方法通过自适应非对称的token级优势信号分配提升训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型通常使用带验证奖励的强化学习进行训练，但正负样本在训练动态和行为影响上的差异尚未得到系统研究，需要深入探索以优化训练过程。

Method: 通过分析正负样本对训练动态的影响，提出A3PO方法——一种自适应非对称的token级优势信号分配策略，用于策略优化。

Result: 实验在五个推理基准测试中验证了A3PO方法的有效性，表明其能更精确地为不同极性的关键token分配优势信号。

Conclusion: 正负样本在RLVR训练中具有互补作用，通过精细调整token级优势信号分配可显著提升推理模型的训练效果和性能。
...
### [110] [Heaven-Sent or Hell-Bent? Benchmarking the Intelligence and Defectiveness of LLM Hallucinations](https://arxiv.org/abs/2512.21635)
*Chengxu Yang,Jingling Yuan,Siqi Cai,Jiawei Jiang,Chuang Hu*

Main category: cs.CL

TL;DR: 本文提出了HIC-Bench评估框架，将大语言模型幻觉分为智能幻觉和缺陷幻觉，系统研究其在科学创新中的创造性价值。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要将幻觉视为需最小化的错误，但部分幻觉可能蕴含创造性或认知价值，这一维度在当前文献中缺乏量化评估。现有检测方法难以处理异构科学任务及平衡创造性与准确性。

Method: 提出HIC-Bench框架：1) 使用多维度量矩阵（整合TTCT创造力指标与幻觉特定维度）结构化评估IH/DH；2) 覆盖十个科学领域的开放式创新任务；3) 采用动态提示优化技术引导模型生成兼具创造性和可靠性的输出，并利用多LLM评委与人工标注验证。

Result: 实验揭示了IH与DH之间的非线性关系，表明创造性与正确性可以协同优化；智能幻觉可作为创造力的催化剂，证明LLM幻觉具有驱动科学创新的潜力。

Conclusion: HIC-Bench为系统评估LLM幻觉的创造性价值提供了新框架，揭示了幻觉在科学创新中的积极作用，为LLM创造性智能研究提供了重要平台。
...
### [111] [Enabling Conversational Behavior Reasoning Capabilities in Full-Duplex Speech](https://arxiv.org/abs/2512.21706)
...
TL;DR: 提出基于思维图（GoT）的因果推理框架，用于建模对话中的意图-行为路径，实现全双工会话系统的行为预测与可解释推理。


<details>
  <summary>Details</summary>
Motivation: 人类对话由隐含的思维链驱动，表现为时序性言语行为。捕捉这种因果路径是构建自然全双工交互系统的关键。

Method: 1. 将对话过程建模为思维图内的因果推理；2. 采用分层标注方案形式化意图-行为路径；3. 结合可控模拟数据与真人标注语料训练；4. 使用多模态Transformer进行流式预测与动态推理优化。

Result: 在合成与真实全双工对话上的实验表明：该框架实现了稳健的行为检测、生成可解释的推理链，并为对话推理建立了可评估的基础。

Conclusion: GoT框架能有效捕捉对话中的因果时序依赖，为全双工口语对话系统提供了可解释的推理能力与行为预测基础。
...
### [112] [MoRAgent: Parameter Efficient Agent Tuning with Mixture-of-Roles](https://arxiv.org/abs/2512.21708)
*Jing Han,Binwei Yan,Tianyu Guo,Zheyuan Bai,Mengyu Zheng,Hanting Chen,Ying Nie*

Main category: cs.CL

TL;DR: 本文提出了一种用于智能体任务的参数高效微调方法，通过角色分解、混合角色框架和多角色数据生成管道，在多个基准测试中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在智能体任务中的微调取得进展，但参数高效微调方法在该领域仍未被充分探索，需要开发专门针对智能体任务的高效微调策略。

Method: 1) 将智能体能力分解为推理者、执行者和总结者三个角色；2) 提出混合角色框架，包含三个专门的LoRA组；3) 开发基于公开数据集的多角色数据生成管道，包含角色特定内容补全和可靠性验证。

Result: 在多种大语言模型和智能体基准测试上进行了广泛实验和消融研究，证明了所提方法的有效性。

Conclusion: 提出的参数高效微调方法通过角色专业化分工和协作，能够有效提升智能体任务性能，相关代码已公开。
...
### [113] [Detecting AI-Generated Paraphrases in Bengali: A Comparative Study of Zero-Shot and Fine-Tuned Transformers](https://arxiv.org/abs/2512.21709)
*Md. Rakibul Islam,Most. Sharmin Sultana Samu,Md. Zahid Hossain,Farhad Uz Zaman,Md. Kamrozzaman Bhuiyan*

Main category: cs.CL

TL;DR: 该研究评估了五种基于Transformer的模型在检测孟加拉语AI生成文本方面的性能，发现零样本评估效果不佳，但经过微调后XLM-RoBERTa等模型准确率可达91%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成文本的能力引发了虚假信息和内容操纵的担忧，而孟加拉语因其丰富的词汇和复杂结构，在AI生成文本检测方面尚未得到充分探索。

Method: 研究调查了五种Transformer模型（XLMRoBERTa-Large、mDeBERTaV3-Base、BanglaBERT-Base、IndicBERT-Base和MultilingualBERT-Base），采用零样本评估和任务特定微调两种方法。

Result: 零样本评估中所有模型准确率接近随机水平（约50%）；微调后XLM-RoBERTa、mDeBERTa和MultilingualBERT在准确率和F1分数上均达到约91%，而IndicBERT表现相对较弱。

Conclusion: 该研究推进了孟加拉语AI生成文本检测，为构建鲁棒的系统以应对AI生成内容奠定了基础，并表明任务特定微调对提升检测性能至关重要。
...
### [114] [Do Latent Tokens Think? A Causal and Adversarial Analysis of Chain-of-Continuous-Thought](https://arxiv.org/abs/2512.21711)
*Yuyi Zhang,Boyu Tang,Tianjie Ju,Sufeng Duan,Gongshen Liu*

Main category: cs.CL

TL;DR: 研究发现COCONUT等潜在标记方法并非真正的推理机制，而是作为不可解释的占位符，依赖数据集伪影而非真实推理，导致基准性能虚高。


<details>
  <summary>Details</summary>
Motivation: 潜在标记方法在提升大语言模型推理能力方面受到关注，但其内部机制不明确。本文从可靠性角度出发，旨在揭示这些方法的根本弱点。

Method: 采用两种互补方法：1）通过扰动COCONUT和显式CoT标记进行引导实验；2）在偏见和分布外设置下进行捷径实验，评估模型在MMLU和HotpotQA数据集上的表现。

Result: COCONUT标记对引导不敏感且缺乏关键推理信息；在捷径实验中，COCONUT持续利用数据集伪影，虚增基准性能而非进行真实推理。

Conclusion: COCONUT是一种伪推理机制，生成看似合理的推理轨迹以掩盖对捷径的依赖，而非忠实表示推理过程。
...
### [115] [CATCH: A Controllable Theme Detection Framework with Contextualized Clustering and Hierarchical Generation](https://arxiv.org/abs/2512.21715)
*Rui Ke,Jiahui Xu,Shenghao Yang,Kuang Wang,Feng Jiang,Haizhou Li*

Main category: cs.CL

TL;DR: 提出CATCH框架用于用户对话中的主题检测，通过上下文增强、个性化聚类和分层生成解决现有方法在短文本表示和跨对话一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有主题检测方法难以处理稀疏短文本的准确表示，且无法捕捉跨对话的用户个性化主题偏好，导致主题识别效果受限。

Method: CATCH框架包含三个核心组件：上下文感知主题表示、偏好引导主题聚类、分层主题生成机制，结合8B LLM进行实现。

Result: 在DSTC-12多领域客户对话基准测试中，CATCH在主题聚类和主题生成质量方面均表现出有效性。

Conclusion: CATCH通过整合上下文信息和用户偏好，显著提升了主题检测的准确性和一致性，为无预定义模式的对话主题识别提供了有效解决方案。
...
### [116] [Ara-HOPE: Human-Centric Post-Editing Evaluation for Dialectal Arabic to Modern Standard Arabic Translation](https://arxiv.org/abs/2512.21787)
*Abdullah Alabdullah,Lifeng Han,Chenghua Lin*

Main category: cs.CL

TL;DR: 本文提出了Ara-HOPE框架，用于评估阿拉伯方言到现代标准阿拉伯语的机器翻译质量，通过后编辑评估方法系统性地解决了方言特定翻译错误的评估难题。


<details>
  <summary>Details</summary>
Motivation: 现有自动评估指标和通用人工评估框架难以捕捉阿拉伯方言机器翻译中的方言特定错误，阻碍了翻译评估的进展，因此需要专门针对阿拉伯方言翻译的评估方法。

Method: 开发了Ara-HOPE框架，包含五类错误分类法和决策树标注协议，并对三种机器翻译系统（Jais、GPT-3.5和NLLB-200）进行了比较评估。

Result: 评估结果显示，方言特定术语和语义保留是阿拉伯方言翻译中最持久的挑战，Ara-HOPE框架能有效揭示不同翻译系统之间的系统性性能差异。

Conclusion: Ara-HOPE为阿拉伯方言机器翻译质量评估建立了新框架，并为改进方言感知的机器翻译系统提供了可操作的指导。
...
### [117] [Five Years of SciCap: What We Learned and Future Directions for Scientific Figure Captioning](https://arxiv.org/abs/2512.21789)
*Ting-Hao K. Huang,Ryan A. Rossi,Sungchul Kim,Tong Yu,Ting-Yao E. Hsu,Ho Yin,Ng,C. Lee Giles*

Main category: cs.CL

TL;DR: SciCap项目回顾了2012-2025年间在科学图表标注领域的发展历程，总结了技术经验并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 测试领域特定训练（如SciBERT在文本模型中的成功）是否适用于图表标注任务，并推动科学图表标注技术的发展。

Method: 通过多机构合作，收集并持续更新arXiv论文中的图表-标注对数据集，进行自动和人工评估，组织年度挑战赛，并开发交互式辅助系统。

Result: 建立了大规模科学图表标注数据集，验证了领域特定训练的有效性，并开发了帮助科学家撰写更好标注的工具。

Conclusion: 总结了五年来的技术经验，提出了五个未解决的关键挑战，并规划了下一阶段科学图表标注研究的方向。
...
### [118] [On The Conceptualization and Societal Impact of Cross-Cultural Bias](https://arxiv.org/abs/2512.21809)
*Vitthal Bhandari*

Main category: cs.CL

TL;DR: 本文分析了2025年发表的20篇关于NLP文化偏见的文献，提出了一套观察框架，旨在帮助研究者更具体地概念化偏见并有效评估其危害，倡导对跨文化偏见语言技术的社会影响进行稳健评估。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示，尽管大语言模型能基于文化背景生成回应，但仍存在跨文化泛化问题，且当前评估方法常脱离实际使用者，未能真正解决文化偏见的核心问题。

Method: 基于arXiv:2005.14050v2的启发，系统分析了2025年发表的20篇关于NLP文化偏见识别与评估的文献，归纳出一套观察框架。

Result: 提出了一套具体观察指标，帮助未来NLP研究者更清晰地概念化文化偏见，并建立有效的危害评估机制。

Conclusion: 需要建立更稳健的评估框架来审视语言技术中跨文化偏见的社会影响，推动研究与实践更紧密地结合真实使用场景。
...
### [119] [Method Decoration (DeMe): A Framework for LLM-Driven Adaptive Method Generation in Dynamic IoT Environments](https://arxiv.org/abs/2512.21817)
*Hong Su*

Main category: cs.CL

TL;DR: 提出Method Decoration (DeMe)框架，通过隐藏目标、学习方法和环境反馈生成显式装饰，动态调整LLM生成任务执行方法的结构，提升物联网系统在未知环境中的适应性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的物联网系统在遇到未知情况时缺乏系统性的新方法生成能力，且依赖固定的设备特定逻辑，无法适应动态变化的环境条件。

Method: DeMe框架从通用行为原则、经验和环境差异中提取装饰，通过预装饰、后装饰、中间步骤修改和步骤插入四种方式重组方法生成路径。

Result: 实验表明，DeMe能使物联网设备在面对未知或故障操作条件时生成更合适的方法。

Conclusion: Method Decoration提供了一种非硬编码的灵活框架，可生成上下文感知、安全对齐且环境自适应的任务执行方法，增强了LLM在动态物联网环境中的适应性。
...
### [120] [Knowledge Reasoning of Large Language Models Integrating Graph-Structured Information for Pest and Disease Control in Tobacco](https://arxiv.org/abs/2512.21837)
*Siyu Li,Chenwei Song,Wan Zhou,Xinyi Liu*

Main category: cs.CL

TL;DR: 提出一种结合知识图谱的LLM方法，用于烟草病虫害防治的知识推理，通过GraphRAG框架增强检索与推理能力，实验表明在多指标上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统LLM在领域知识推理中缺乏结构化信息支持，尤其在烟草病虫害防治等专业领域，需要更准确、可解释的推理能力。

Method: 基于GraphRAG框架，先利用LLM构建烟草病虫害知识图谱，再结合GNN学习节点表示，以ChatGLM为骨干模型并通过LoRA进行高效微调，集成图谱信息进行推理。

Result: 该方法在多项评估指标上持续优于基线，显著提升了推理准确性和深度，尤其在多跳推理和对比推理场景中表现突出。

Conclusion: 融合图谱结构化信息的LLM方法能有效提升领域知识推理性能，为专业领域的智能问答系统提供了可行方案。
...
### [121] [From Questions to Clinical Recommendations: Large Language Models Driving Evidence-Based Clinical Decision Making](https://arxiv.org/abs/2505.10282)
...
TL;DR: 本研究提出了Quicker系统，这是一个基于大型语言模型的临床决策支持系统，能够自动化证据合成并生成临床建议，显著缩短决策时间并提高可靠性。


<details>
  <summary>Details</summary>
Motivation: 临床实践中整合循证医学证据面临工作量大、流程复杂和时间紧迫的挑战，需要自动化工具来支持高效准确的临床决策。

Method: 开发了Quicker系统，采用基于大型语言模型的自动化流程，覆盖从问题提出到临床建议生成的全过程，并创建了Q2CRBench-3基准数据集进行系统评估。

Result: Quicker在问题分解、文献检索和筛选方面表现优异，其生成的建议比临床医生更全面、逻辑更连贯，单名评审员与系统协作可将建议制定时间缩短至20-40分钟。

Conclusion: Quicker系统展示了帮助医生更快、更可靠地进行循证临床决策的潜力，有效支持了临床实践中的决策过程。
...
### [122] [AlignAR: Generative Sentence Alignment for Arabic-English Parallel Corpora of Legal and Literary Texts](https://arxiv.org/abs/2512.21842)
*Baorong Huang,Ali Asiri*

Main category: cs.CL

TL;DR: 提出AlignAR生成式句子对齐方法和包含复杂法律文学文本的新阿拉伯语-英语数据集，证明传统对齐方法在复杂文本上的局限性，LLM方法表现更优。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语-英语平行语料稀缺且现有数据集多为简单一对一映射，难以满足机器翻译研究和教学需求，特别是复杂文本的对齐任务。

Method: 开发AlignAR生成式句子对齐方法，构建包含法律和文学文本的复杂数据集，通过减少一对一映射创建“困难”子集评估方法性能。

Result: 传统对齐方法在复杂文本上表现受限，而基于LLM的方法展现出更强鲁棒性，整体F1分数达85.5%，比先前方法提升9%。

Conclusion: 复杂数据集能更好评估对齐方法性能，LLM方法在复杂文本对齐任务中优于传统方法，开源数据集和代码促进相关研究。
...
### [123] [HeartBench: Probing Core Dimensions of Anthropomorphic Intelligence in LLMs](https://arxiv.org/abs/2512.21849)
...
TL;DR: 提出了HeartBench框架，用于评估中文大语言模型在情感、文化和伦理维度上的拟人化智能表现，发现现有模型在复杂社交情感场景中仍存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在认知推理任务上表现优异，但在处理复杂社交、情感和伦理细微差别方面存在明显不足，尤其在中文文化语境下缺乏专门的评估框架和高质量社会情感数据。

Method: 基于真实心理咨询场景，与临床专家合作开发了包含5个主要维度和15项次级能力的理论驱动分类体系；采用案例特定、基于量规的方法，通过“推理-评分”评估协议将抽象拟人特质转化为可量化标准。

Result: 对13个先进大语言模型的评估显示性能存在明显上限：即使领先模型也仅达到专家定义理想分数的60%；在涉及微妙情感潜台词和复杂伦理权衡的“困难集”场景中，模型性能显著下降。

Conclusion: HeartBench为拟人化AI评估建立了标准化度量标准，并为构建高质量、人类对齐的训练数据提供了方法论蓝图，揭示了当前模型在情感智能方面的局限性。
...
### [124] [TimeBill: Time-Budgeted Inference for Large Language Models](https://arxiv.org/abs/2512.21859)
*Qi Fan,An Zou,Yehan Ma*

Main category: cs.CL

TL;DR: 提出了TimeBill框架，通过预测响应长度和执行时间，自适应调整KV缓存淘汰率，以在给定时间预算内平衡LLM推理效率和响应性能。


<details>
  <summary>Details</summary>
Motivation: LLM在时间敏感系统（如机器人、自动驾驶）中部署时，需在时间预算内生成准确响应，但现有固定KV缓存淘汰率方法难以适应不同任务的时间需求。

Method: 1. 设计细粒度响应长度预测器（RLP）和执行时间估计器（ETE）；2. 基于时间预测和预算自适应调整KV缓存淘汰率；3. 通过实验验证框架有效性。

Result: 实验表明TimeBill能提高任务完成率，并在多种超时策略下保持响应性能。

Conclusion: TimeBill框架能有效解决LLM在时间约束下的推理优化问题，为时间关键型应用提供实用解决方案。
...
### [125] [Bridging the Copyright Gap: Do Large Vision-Language Models Recognize and Respect Copyrighted Content?](https://arxiv.org/abs/2512.21871)
*Naen Xu,Jinghuai Zhang,Changjiang Li,Hengyu An,Chunyi Zhou,Jun Wang,Boyu Xu,Yuyuan Li,Tianyu Du,Shouling Ji*

Main category: cs.CL

TL;DR: 该论文评估了大视觉语言模型在处理受版权保护内容时的合规性，发现现有模型存在显著缺陷，并提出了一个工具增强的防御框架来降低侵权风险。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型的广泛使用引发了版权侵权的担忧，模型在遇到受版权保护内容（如书籍摘录、新闻文章）时可能无法准确识别并遵守版权法规，从而导致法律和伦理后果。

Method: 构建了一个包含5万对多模态查询-内容对的大规模基准数据集，涵盖带版权声明和不带版权声明两种场景，系统评估了多种LVLMs处理版权内容的能力，并提出了一个工具增强的版权合规防御框架。

Result: 评估显示，即使是先进的闭源LVLMs在识别和尊重受版权保护内容方面也存在显著不足，即使在提供版权声明的情况下也是如此。提出的防御框架在所有场景中都能有效降低侵权风险。

Conclusion: 研究强调了开发具有版权意识的大视觉语言模型的重要性，以确保对受版权保护内容的负责任和合法使用。
...
### [126] [Explainable Statute Prediction via Attention-based Model and LLM Prompting](https://arxiv.org/abs/2512.21902)
*Sachin Pawar,Girish Keshav Palshikar,Anindita Sinha Banerjee,Nitin Ramrakhiyani,Basit Ali*

Main category: cs.CL

TL;DR: 本文研究了自动法规预测问题，提出两种方法（AoS和LLMPrompt）来预测案件描述相关的法规并提供可理解的解释。


<details>
  <summary>Details</summary>
Motivation: 开发法律AI助手（如律师助理和法律问答系统）需要自动预测相关法规，并生成人类可理解的解释以提高用户接受度。

Method: 提出两种方法：1）AoS（基于注意力机制）使用句子变换器在监督学习下预测法规；2）LLMPrompt使用大语言模型进行零样本预测，探索标准提示和思维链提示技术。两种方法均能生成解释。

Result: 在两种流行数据集上比较了两种方法与基线模型的法规预测性能，并通过自动反事实评估和人工评估验证了解释质量。

Conclusion: 两种方法均能有效预测法规并生成可理解的解释，为法律AI系统的实用化提供了技术支持。
...
### [127] [CricBench: A Multilingual Benchmark for Evaluating LLMs in Cricket Analytics](https://arxiv.org/abs/2512.21877)
*Vaibhav Devraj,Dhruv Kumar,Jagat Sesh Challa*

Main category: cs.CL

TL;DR: 本文介绍了CricBench，一个用于评估大型语言模型在板球数据分析中Text-to-SQL能力的多语言基准测试套件，发现专业领域性能与通用基准存在显著差距，且印地语查询有时优于英语。


<details>
  <summary>Details</summary>
Motivation: 板球作为全球第二大运动，拥有超过25亿粉丝，但现有工具难以提供复杂的历史性能分析和球员对比。尽管LLMs在Text-to-SQL任务上进步显著，但其处理体育分析中领域特定性、复杂模式和多语言需求的能力尚未充分探索。

Method: 1. 与板球和SQL领域专家合作，手动编写逻辑正确的复杂查询，构建“黄金标准”数据集；2. 创建英语和印地语双版本基准测试框架；3. 使用严格评估协议测试GPT-4o、Claude 3.7 Sonnet、DeepSeek R1等六个先进模型。

Result: 1. 通用基准的高性能不保证专业领域成功：DeepSeek R1以50.6%准确率领先，超过Claude 3.7 Sonnet（47.7%）和GPT-4o（33.7%），但从通用基准（BIRD）到CricBench仍出现显著性能下降；2. 语言反直觉发现：混合印地语查询常达到与英语相当或更高的准确率，挑战了英语为专业SQL任务最佳提示语言的假设。

Conclusion: LLMs在板球等专业领域的Text-to-SQL能力存在明显局限，需要领域特定的基准测试和优化。多语言支持（特别是印地语）可能提升专业任务性能，为未来体育分析和多语言AI系统开发提供了新方向。
...
### [128] [Accelerate Speculative Decoding with Sparse Computation in Verification](https://arxiv.org/abs/2512.21911)
*Jikai Wang,Jianchao Tan,Yuxuan Hu,Jiayu Qin,Yerui Sun,Yuchen Xie,Xunliang Cai,Juntao Li,Min Zhang*

Main category: cs.CL

TL;DR: 提出了一种针对推测解码验证阶段的稀疏验证框架，通过联合稀疏化注意力、FFN和MoE组件来减少计算成本，并引入跨草案令牌和跨层检索重用策略，在保持稳定接受长度的同时实现效率与准确性的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 推测解码通过并行验证多个草案令牌来加速自回归语言模型推理，但验证阶段常成为计算瓶颈，尤其对于长上下文输入和MoE模型。现有稀疏化方法主要针对标准自回归解码设计，未能有效解决验证阶段的冗余计算问题。

Method: 1. 系统评估不同稀疏方法在推测解码验证阶段的效果；2. 提出联合稀疏化注意力、FFN和MoE组件的框架；3. 引入跨草案令牌和跨层检索重用策略以减少冗余计算；4. 无需额外训练。

Result: 在摘要生成、问答和数学推理数据集上的实验表明，该方法在保持稳定接受长度的同时，实现了效率与准确性的良好权衡。

Conclusion: 所提出的稀疏验证框架能有效减少推测解码验证阶段的计算成本，为长上下文和MoE模型的高效推理提供了可行方案。
...
### [129] [SWE-RM: Execution-free Feedback For Software Engineering Agents](https://arxiv.org/abs/2512.21919)
*KaShun Shum,Binyuan Hui,Jiawei Chen,Lei Zhang,X. W.,Jiaxi Yang,Yuzhen Huang,Junyang Lin,Junxian He*

Main category: cs.CL

TL;DR: 本文提出SWE-RM，一种用于软件工程代理的免执行奖励模型，通过混合专家架构提升测试时扩展和强化学习性能，在SWE-Bench上达到开源模型新SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统基于单元测试的执行反馈存在稀疏性和无法区分同成功/失败轨迹的局限，而免执行反馈能提供更细粒度信号但尚未充分探索。现有验证器在TTS表现相近时RL效果差异大，需开发同时适应TTS和RL的稳健奖励模型。

Method: 识别分类准确率和校准作为RL关键指标；通过控制实验分析训练数据规模、策略混合、数据源组成等因素；设计采用混合专家架构的SWE-RM模型（总参数量30B，推理激活3B）。

Result: SWE-RM显著提升SWE代理在TTS和RL上的性能：在SWE-Bench Verified上，将Qwen3-Coder-Flash准确率从51.6%提升至62.0%，Qwen3-Coder-Max从67.0%提升至74.6%，达到开源模型最佳水平。

Conclusion: 免执行奖励模型能有效补充传统执行反馈的不足；通过系统优化训练策略和模型架构，可开发出同时适用于TTS和RL的稳健奖励模型，显著提升代码生成代理的软件工程能力。
...
### [130] [Broken Words, Broken Performance: Effect of Tokenization on Performance of LLMs](https://arxiv.org/abs/2512.21933)
*Sachin Pawar,Manoj Apte,Kshitij Jadhav,Girish Keshav Palshikar,Nitin Ramrakhiyani*

Main category: cs.CL

TL;DR: 本文研究大语言模型（LLM）中因词汇表限制导致自然单词被拆分为多个token的现象，并提出量化这种tokenization对模型性能负面影响的方法。


<details>
  <summary>Details</summary>
Motivation: LLM的tokenization与传统NLP不同，常将自然单词拆分为多个token（如“martial”拆为“mart”和“ial”），作者假设这种拆分会对LLM在NLP任务上的性能产生负面影响，需要量化验证。

Method: 提出一组惩罚函数，用于计算特定LLM对给定文本的tokenization惩罚值，以衡量tokenization的“糟糕程度”，并在多个NLP任务和不同LLM上验证假设的统计显著性。

Result: 通过实验证实了tokenization拆分自然单词对LLM性能有显著负面影响的假设，且提出的惩罚函数能有效量化这种影响。

Conclusion: LLM的tokenization机制对模型性能有重要影响，拆分自然单词会降低任务表现，提出的量化方法为未来优化tokenization策略提供了评估工具。
...
### [131] [Context as a Tool: Context Management for Long-Horizon SWE-Agents](https://arxiv.org/abs/2512.22087)
*Shukai Liu,Jian Yang,Bo Jiang,Yizhi Li,Jinyang Guo,Xianglong Liu,Bryan Dai*

Main category: cs.CL

TL;DR: 提出CAT框架，通过主动压缩历史轨迹来管理LLM代理的上下文，解决长时交互中的上下文爆炸和语义漂移问题，在软件工程任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在软件工程任务中依赖被动压缩或仅追加上下文，导致长时交互中出现上下文爆炸、语义漂移和推理能力下降。

Method: 提出CAT上下文管理范式，将上下文维护作为可调用工具；设计结构化上下文工作空间；开发CAT-GENERATOR监督框架和SWE-Compressor模型。

Result: 在SWE-Bench-Verified上达到57.6%解决率，显著优于ReAct代理和静态压缩基线，在有限上下文预算下保持稳定可扩展的长期推理。

Conclusion: 主动上下文管理能有效提升LLM代理在长时软件工程任务中的性能，CAT框架为解决上下文爆炸问题提供了新方向。
...
### [132] [Self-attention vector output similarities reveal how machines pay attention](https://arxiv.org/abs/2512.21956)
*Tal Halevi,Yarden Tzach,Ronit D. Gross,Shalom Rosner,Ido Kanter*

Main category: cs.CL

TL;DR: 本研究提出了一种量化自注意力机制信息处理的新方法，通过分析BERT-12架构发现注意力头在不同层关注不同的语言特征，并揭示了从长距离到短距离相似性的演变模式。


<details>
  <summary>Details</summary>
Motivation: 尽管自注意力机制在自然语言处理中广泛应用，但其具体学习机制和量化表征仍不明确，需要开发新方法来理解其内部信息处理过程。

Method: 提出量化自注意力信息处理的新方法，分析BERT-12架构的注意力图、上下文相似性矩阵，并研究不同注意力头的向量空间和相似性分布。

Result: 最终层注意力聚焦于句子分隔符；不同注意力头关注不同语言特征（如重复标记、常见标记）；相似性从初始层的长距离逐渐演变为深层的短距离（同一句子内）；每个头倾向于围绕独特标记构建相似性对。

Conclusion: 自注意力机制通过分层专业化处理语言特征，注意力头在不同层发展出不同的功能，相似性模式从全局向局部演变，为理解Transformer架构的工作机制提供了量化依据。
...
### [133] [Introducing TrGLUE and SentiTurca: A Comprehensive Benchmark for Turkish General Language Understanding and Sentiment Analysis](https://arxiv.org/abs/2512.22100)
*Duygu Altinok*

Main category: cs.CL

TL;DR: 本文介绍了TrGLUE和SentiTurca两个土耳其语自然语言理解基准，填补了土耳其语缺乏全面NLU评估工具的空白。


<details>
  <summary>Details</summary>
Motivation: 现有GLUE风格基准（如英文GLUE、中文CLUE等）已为多语言NLU评估设立标准，但土耳其语缺乏类似综合基准，限制了该语言NLP模型的发展与评估。

Method: 1) 构建TrGLUE基准，包含土耳其原生语料库，任务设计仿照GLUE风格；2) 采用半自动化标注流程：结合LLM标注、跨模型一致性检验和人工验证；3) 提供基于Transformer的微调与评估代码；4) 额外创建情感分析专用基准SentiTurca。

Result: 成功创建首个土耳其语综合NLU基准TrGLUE，涵盖多领域任务，标注流程兼顾效率与质量，减少翻译痕迹，确保语言自然性，并配套开源工具支持研究。

Conclusion: TrGLUE为土耳其语NLU建立了可靠评估框架，提供了高质量半自动化数据集构建范例，有望推动土耳其语NLP研究，并为其他低资源语言基准开发提供参考。
...
### [134] [Safe Path Planning and Observation Quality Enhancement Strategy for Unmanned Aerial Vehicles in Water Quality Monitoring Tasks](https://arxiv.org/abs/2512.21375)
*Yuanshuang Fu,Qianyao Wang,Qihao Wang,Bonan Zhang,Jiaxin Zhao,Yiming Cao,Zhijun Li*

Main category: cs.RO

TL;DR: 提出一种无人机主动路径规划方法，用于动态光照阴影干扰规避，通过虚拟障碍建模、改进IFDS算法、MPC优化和动态高度调整，提升水质监测数据质量。


<details>
  <summary>Details</summary>
Motivation: 动态环境中光照变化（如阴影和镜面反射）导致无人机光谱遥感数据失真，降低数据可用性，需在保障飞行安全的同时最大化高质量数据采集。

Method: 构建动态预测模型将光阴影干扰区转为三维虚拟障碍；引入改进IFDS算法生成平滑避障路径；采用MPC框架进行滚动优化以处理飞行约束；设计动态飞行高度调整机制以增强空间分辨率。

Result: 在密集干扰场景中，避障成功率98%，路径平滑度显著提升，有效观测数据量增加约27%。

Conclusion: 该方法为复杂光照环境下的无人机精准水质监测提供了有效工程解决方案，提高了数据采集效率和质量。
...
### [135] [Fast Navigation Through Occluded Spaces via Language-Conditioned Map Prediction](https://arxiv.org/abs/2512.21398)
*Rahul Moorthy Mahesh,Oguzhan Goktug Poyrazoglu,Yukang Cao,Volkan Isler*

Main category: cs.RO

TL;DR: 提出PaceForecaster方法，通过结合副驾驶指令来预测环境地图和子目标，帮助机器人在杂乱环境中更果断且安全地规划路径，导航性能提升36%。


<details>
  <summary>Details</summary>
Motivation: 在杂乱环境中，由于遮挡和传感器范围限制带来的不确定性，运动规划器常常需要在安全性和速度之间进行权衡。研究旨在探索副驾驶指令是否能帮助机器人更果断地规划路径同时保持安全。

Method: 引入PaceForecaster方法，将副驾驶指令整合到局部规划器中。该方法以机器人的局部传感器覆盖范围（Level-1）和副驾驶指令为输入，预测（i）从Level-1可见区域的预测地图（Level-2），以及（ii）基于指令在Level-2内生成条件性子目标。子目标为规划器提供明确指导，以目标导向方式利用预测环境。最后将PaceForecaster与Log-MPPI控制器集成。

Result: 在多边形环境中，使用语言条件预测和目标的导航性能比仅使用局部地图的基线提高了36%。

Conclusion: 研究表明，通过结合副驾驶指令进行环境预测和子目标生成，可以显著提升机器人在不确定环境中的导航性能，实现更安全、更快速的路径规划。
...
### [136] [EVE: A Generator-Verifier System for Generative Policies](https://arxiv.org/abs/2512.21430)
*Yusuf Ali,Gryphon Patlin,Karthik Kothuri,Muhammad Zubair Irshad,Wuwei Liang,Zsolt Kira*

Main category: cs.RO

TL;DR: 提出EVE框架，通过零样本视觉语言模型验证器在推理时提升预训练生成策略的性能，无需额外训练即可提高任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散或流匹配的视觉运动策略在分布偏移下性能下降，且缺乏低成本恢复能力；受语言模型测试时计算扩展启发，探索通过验证器改进策略的潜力。

Method: 设计EVE框架：冻结基础策略与多个零样本VLM验证器交互，验证器提出动作优化建议，动作整合器融合建议生成最终执行动作。

Result: 在多样化操作任务中，EVE持续提升任务成功率，并通过消融实验验证了验证器能力和动作整合策略的有效性。

Conclusion: EVE为具身控制提供了可扩展的模块化生成-验证系统设计指南，证明测试时计算扩展可有效提升生成策略的鲁棒性。
...
### [137] [Developing a Fundamental Diagram for Urban Air Mobility Based on Physical Experiments](https://arxiv.org/abs/2512.21425)
*Hang Zhou,Yuhui Zhai,Shiyu Shen,Yanfeng Ouyang,Xiaowei Shi,Xiaopeng*

Main category: cs.RO

TL;DR: 本研究首次通过理论分析与物理实验相结合的方法，构建了城市空中交通（UAM）的基本图（FD），揭示了UAM交通流的宏观特性，并验证了地面交通FD结构在UAM系统中的适用性。


<details>
  <summary>Details</summary>
Motivation: 随着无人机密度增加，UAM运营可能出现类似地面交通的拥堵问题，但目前对真实运行条件下UAM交通流的基本特性缺乏深入理解。

Method: 1. 理论方面：设计两种无人机碰撞避免控制律，开发基于仿真的交通生成方法以构建多样化场景；2. 实验方面：使用Bitcraze Crazyflie无人机在缩比实验平台上进行物理实验，考虑实际干扰和模型不确定性；3. 基于Edie定义应用交通流理论构建FD，并创建UAMTra2Flow数据集。

Result: 1. 经典地面交通FD结构同样适用于UAM系统；2. 物理实验获得的FD曲线与仿真结果存在偏差，凸显实验验证的重要性；3. 通过缩比实验结果推演真实运行条件，为未来UAM交通系统提供实用见解。

Conclusion: 本研究提出的框架成功构建了UAM交通基本图，证实了理论模型需通过物理实验验证的必要性，相关数据集和代码已开源，为UAM交通管理研究奠定了基础。
...
### [138] [Planetary Terrain Datasets and Benchmarks for Rover Path Planning](https://arxiv.org/abs/2512.21438)
*Marvin Chancán,Avijit Banerjee,George Nikolakopoulos*

Main category: cs.RO

TL;DR: 该研究创建了首个基于火星和月球高分辨率地形图的大型行星漫游车路径规划基准数据集MarsPlanBench和MoonPlanBench，并在统一框架下评估了经典与学习型路径规划算法在行星地形上的性能。


<details>
  <summary>Details</summary>
Motivation: 行星探测任务日益增多，但以往任务的大量数据未被充分用于路径规划研究，且缺乏基于太空任务的行星数据集、标准化基准和评估协议。

Method: 从火星和月球的高分辨率数字地形图像中构建两个大型平面基准数据集；在统一框架中设置经典与学习型路径规划算法；在提出的数据集和流行规划基准上进行综合实验评估。

Result: 经典算法在月球南北极等挑战性地形上平均可实现100%的全局路径规划成功率；学习型模型在较简单环境中表现良好，但在行星领域泛化能力仍不足。

Conclusion: 研究为行星路径规划提供了首个标准化基准，解释了经典算法（如NASA所用）在实际任务中的可靠性，同时揭示了学习型方法在行星环境下面临的泛化挑战，相关代码与数据集已开源。
...
### [139] [Spatiotemporal Tubes for Probabilistic Temporal Reach-Avoid-Stay Task in Uncertain Dynamic Environment](https://arxiv.org/abs/2512.21497)
*Siddhartha Upadhyay,Ratnangshu Das,Pushpak Jagtap*

Main category: cs.RO

TL;DR: 扩展时空管框架以处理动态环境中具有不确定障碍物的概率时序可达-避障-停留任务，开发实时管合成方法并提供概率安全保证。


<details>
  <summary>Details</summary>
Motivation: 动态环境中存在时变不确定障碍物，需要确保机器人安全执行复杂任务（如避障、到达目标并停留），同时提供形式化的概率保证。

Method: 使用时变球体表示时空管，基于不确定传感信息在线更新中心与半径；推导闭式控制律（无需模型、近似或优化）将系统轨迹约束在管内。

Result: 控制器实现了概率避障保证和有限时间任务完成，在移动机器人、无人机和7自由度机械臂的仿真与硬件实验中验证了有效性和可扩展性。

Conclusion: 所提方法能够实时处理不确定动态环境，为复杂机器人任务提供形式化的概率安全与任务完成保证，且计算高效。
...
### [140] [Optimal Trajectory Planning for Orbital Robot Rendezvous and Docking](https://arxiv.org/abs/2512.21882)
*Kenta Iizuka,Akiyoshi Uchida,Kentaro Uno,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出了一种用于空间碎片清除任务中接近翻滚目标的轨迹规划方法，采用非线性优化和动态禁区球策略，并开发了适用于离散开关推进器的控制策略。


<details>
  <summary>Details</summary>
Motivation: 空间碎片清除任务中，使用机械臂接近翻滚目标存在安全挑战，需要可靠的近距离交会方法作为捕获前的重要步骤。

Method: 基于非线性优化的轨迹规划方法，引入动态禁区球以适应不同接近条件，并开发了考虑实际约束的离散开关推进器控制策略。

Result: 该方法能够在二维平面内将自由漂浮旋转的碎片目标引导至机械臂工作空间，实现更近且更安全的接近。

Conclusion: 所提出的轨迹规划和控制策略为空间碎片的安全捕获提供了有效的初步解决方案，具有实际应用潜力。
...
### [141] [A Novel Robotic Variable Stiffness Mechanism Based on Helically Wound Structured Electrostatic Layer Jamming](https://arxiv.org/abs/2512.21534)
*Congrui Bai,Zhenting Du,Weibang Bai*

Main category: cs.RO

TL;DR: 本文提出了一种新型螺旋缠绕结构静电层卡滞（HWS-ELJ）变刚度机制，并系统研究了其在机器人变刚度手指设计中的应用潜力。


<details>
  <summary>Details</summary>
Motivation: 传统平面静电层卡滞（ELJ）在刚度调节范围和空间效率方面存在局限，需要一种更高效、紧凑的变刚度机制来提升机器人手指的性能和适应性。

Method: 采用螺旋缠绕结构设计，利用静电吸引力增强层间摩擦以抑制相对滑动；通过电压控制实现刚度调节；进行了不同初始力条件下的实验测试，并开发了集成HWS-ELJ的机器人手指原型。

Result: HWS-ELJ相比传统平面ELJ，在相同电极接触面积下实现了指数级增长的刚度调节能力，在同等刚度条件下减少了所需占用空间；实验结果表明其设计合理且有效，符合理论推导趋势；机器人手指原型成功实现了电压驱动的刚度调制。

Conclusion: HWS-ELJ机制在变刚度机器人手指设计中具有可行性和显著优势，为机器人变刚度机制提供了新的解决方案。
...
### [142] [World-Coordinate Human Motion Retargeting via SAM 3D Body](https://arxiv.org/abs/2512.21573)
*Zhangzheng Tu,Kailun Su,Shaolong Zhu,Yukun Zheng*

Main category: cs.RO

TL;DR: 提出轻量级框架从单目视频恢复世界坐标系人体运动并重定向到人形机器人，使用SAM 3D Body作为感知骨干和MHR作为机器人友好中间表示，通过滑动窗口优化和物理约束实现稳定运动重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖复杂SLAM流程或重型时序模型，需要更轻量、工程友好的方案实现机器人可直接使用的运动捕捉，推动具身智能与机器人应用发展。

Method: 1) 使用冻结的SAM 3D Body作为感知骨干；2) 采用Momentum HumanRig作为中间表示；3) 锁定身份与骨骼尺度参数保证时序一致性；4) 在MHR潜空间进行滑动窗口优化平滑预测；5) 结合可微分软足地接触模型与接触感知全局优化恢复物理合理的根轨迹；6) 通过运动学感知两阶段逆运动学管道重定向到Unitree G1机器人。

Result: 在真实单目视频测试中，该方法表现出稳定的世界轨迹和可靠的机器人重定向效果，验证了结构化人体表示结合轻量物理约束可从单目输入生成机器人就绪的运动数据。

Conclusion: 结构化人体表示配合轻量级物理约束能够有效从单目视频提取适用于机器人重定向的运动，为具身智能提供了工程友好的解决方案。
...
### [143] [AstraNav-Memory: Contexts Compression for Long Memory](https://arxiv.org/abs/2512.21627)
*Botao Ren,Junjun Hu,Xinda Xue,Minghua Luo,Jintao Chen,Haochen Bai,Liangliang You,Mu Xu*

Main category: cs.RO

TL;DR: 提出一种基于图像中心的记忆框架，通过视觉上下文压缩模块与导航策略端到端耦合，实现终身具身导航中的长期隐式记忆，在陌生环境中提升探索效率，在熟悉环境中缩短路径。
...
Method: 采用基于ViT骨干网络（冻结DINOv3特征）和轻量级PixelUnshuffle+Conv块的视觉分词器，支持可配置压缩率（如16倍压缩下每张图像约30个token），与Qwen2.5-VL导航策略端到端耦合。

Result: 在GOAT-Bench和HM3D-OVON基准测试中达到最先进的导航性能，实验表明适度压缩在效率与准确性间取得最佳平衡，有效上下文容量从数十张图像扩展至数百张。

Conclusion: 压缩图像中心记忆为终身具身智能体提供了实用且可扩展的接口，使其能够基于长视觉历史进行推理，实现类人的高效导航。
...
TL;DR: 提出一种结合结构先验的蚁群优化框架，用于多机器人路径规划，在路径紧凑性、稳定性和负载均衡方面优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统群智能方法在多机器人路径规划中面临组合爆炸问题，易早熟收敛且难以扩展至复杂场景，需要平衡全局效率与任务分配的公平性。

Method: 设计结构诱导探索框架：1) 利用任务空间分布初始化结构先验以约束搜索空间；2) 改进信息素更新规则以强化结构连接；3) 引入负载感知目标函数协调总行程与个体负载；4) 采用显式重叠抑制策略保证任务区分与均衡分配。

Result: 在多种规模与机器人配置的基准场景中验证，相比主流元启发式基线方法，在路径紧凑性、稳定性和负载分布方面均取得持续改进。

Conclusion: 该方法不仅提升了性能，还提供了可扩展、可解释的框架，适用于物流、监控、搜救等需要大规模可靠协调的应用场景。
...
### [145] [MAction-SocialNav: Multi-Action Socially Compliant Navigation via Reasoning-enhanced Prompt Tuning](https://arxiv.org/abs/2512.21722)
*Zishuo Wang,Xinyu Zhang,Zhuonan Liu,Tomohito Kawabata,Daeun Song,Xuesu Xiao,Ling Xiao*

Main category: cs.RO

TL;DR: 提出MAction-SocialNav模型，通过元认知提示和多动作生成解决社交导航中的行为模糊性问题，并在新构建的数据集上验证其性能优于GPT-4o和Claude。


<details>
  <summary>Details</summary>
Motivation: 现有社交导航方法通常假设单一正确行为，无法处理现实世界中社交规范模糊性导致的多重合理行为场景。

Method: 1. 提出MAction-SocialNav视觉语言模型，支持单场景多行为生成；2. 设计元认知提示方法增强推理能力；3. 构建包含789个样本的多行为社交导航数据集，含双人标注和多样化场景；4. 设计五项评估指标衡量决策精度、安全性和多样性。

Result: 模型在决策质量（APG: 0.595 vs. 0.000/0.025）和安全对齐（ER: 0.264 vs. 0.642/0.668）上显著优于GPT-4o和Claude，同时保持实时效率（1.524 FPS，快3倍以上）。

Conclusion: MAction-SocialNav能有效处理社交导航中的行为模糊性，在保持高效的同时实现更强的社交推理能力，具备实际应用潜力。
...
### [146] [HELP: Hierarchical Embodied Language Planner for Household Tasks](https://arxiv.org/abs/2512.21723)
*Alexandr V. Korchemnyi,Anatoly O. Onishchenko,Eva A. Bakaeva,Alexey K. Kovalev,Aleksandr I. Panov*

Main category: cs.RO

TL;DR: 提出了一种名为HELP的分层具身语言规划器，使用多个LLM代理处理不同子任务，以增强具身代理在复杂场景中的规划能力。


<details>
  <summary>Details</summary>
Motivation: 具身代理在复杂场景中需要强大的规划能力，而大型语言模型虽然具备语言理解能力，但需要合适的架构来有效处理语言歧义、环境信息检索和代理可用技能的结合。

Method: 设计了分层具身语言规划器HELP，由一组基于LLM的代理组成，每个代理专门解决不同的子任务，并重点关注使用参数较少的开源LLM以实现自主部署。

Result: 在家庭任务上评估了该方法，并使用具身代理进行了真实世界实验，验证了其有效性。

Conclusion: HELP架构能够有效利用LLM处理语言歧义和环境信息，提升具身代理的规划能力，同时开源小模型的使用支持了自主部署。
...
### [147] [MoonBot: Modular and On-Demand Reconfigurable Robot Toward Moon Base Construction](https://arxiv.org/abs/2512.21853)
...
TL;DR: 介绍了一种名为MoonBot的模块化可重构机器人系统，专为月球表面探索和基础设施建设而设计，通过现场演示验证了其在模拟月球任务中的可行性。


<details>
  <summary>Details</summary>
Motivation: 月球表面探索和开发日益受到全球关注，机器人对于探索未知地形、利用本地资源及建设未来人类栖息地至关重要。现有系统需在严格的质量限制下适应多变环境与任务需求。

Method: 设计了模块化、按需可重构的机器人系统（MoonBot），详细阐述了其开发过程，并通过初步现场演示执行模拟月球基础设施建设的里程碑任务（如土木工程、部件运输部署、充气模块辅助操作）来验证概念。

Result: 现场演示成功验证了MoonBot的概念可行性，系统总结了测试中获得的经验教训（重点关注连接器设计），为未来月球任务中模块化机器人系统的发展提供了宝贵见解。

Conclusion: MoonBot作为一种模块化可重构机器人系统，展示了在月球探索和基础设施任务中的潜力，其设计经验和测试教训将推动未来太空机器人技术的进步。
...
### [148] [Online Inertia Parameter Estimation for Unknown Objects Grasped by a Manipulator Towards Space Applications](https://arxiv.org/abs/2512.21886)
*Akiyoshi Uchida,Antonine Richard,Kentaro Uno,Miguel Olivares-Mendez,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出一种用于自由浮动基座机器人的在线惯性参数估计方法，通过动量守恒扩展现有方法，在数值模拟中验证了其准确性，适用于在轨服务等空间任务。


<details>
  <summary>Details</summary>
Motivation: 在空间机器人等自由浮动基座系统中，掌握被抓取物体的惯性参数对于动力学感知的操控至关重要，尤其是在未知目标物体的操作过程中。

Method: 应用并扩展了现有的在线识别方法，通过结合动量守恒原理，使其适用于浮动基座机器人，并在数值模拟中进行验证。

Result: 数值模拟结果显示，该方法能够准确识别惯性参数，估计值与真实值吻合良好。

Conclusion: 该方法在模拟场景中表现出色，证明了其在在轨服务和其他空间任务中的适用性。
...
### [149] [Aerial World Model for Long-horizon Visual Generation and Navigation in 3D Space](https://arxiv.org/abs/2512.21887)
...
TL;DR: 提出ANWM空中导航世界模型，通过预测未来视觉观测来评估轨迹的语义合理性和导航效用，提升无人机在大型3D环境中的自主导航能力。


<details>
  <summary>Details</summary>
Motivation: 现有无人机导航策略主要优化避障和轨迹平滑等低层目标，缺乏将高层语义信息融入规划的能力，限制了在复杂环境中的智能导航表现。

Method: 1. 基于4自由度无人机轨迹训练世界模型；2. 引入物理启发的未来帧投影模块，将历史帧投影至未来视角以提供几何先验；3. 通过条件预测未来视觉观测来评估候选轨迹。

Result: ANWM在长距离视觉预测任务中显著优于现有世界模型，并在大规模环境中提高了无人机导航成功率。

Conclusion: ANWM通过结合几何先验与语义理解，有效提升了无人机导航的智能水平，为具身智能在三维空间的应用提供了新思路。
...
### [150] [Flexible Multitask Learning with Factorized Diffusion Policy](https://arxiv.org/abs/2512.21898)
*Chaoqi Liu,Haonan Chen,Sigmund H. Høeg,Shaoxiong Yao,Yunzhu Li,Kris Hauser,Yilun Du*

Main category: cs.RO

TL;DR: 提出一种模块化扩散策略框架，通过将复杂动作分布分解为多个专门扩散模型的组合，提升机器人多任务学习性能并支持灵活适应新任务。


<details>
  <summary>Details</summary>
Motivation: 现有单一模型难以拟合机器人动作的高度多模态和多样化分布，容易欠拟合且缺乏适应新任务的灵活性。

Method: 采用模块化扩散策略框架，将复杂动作分布分解为多个专门捕获不同行为子模式的扩散模型组合。

Result: 在仿真和真实机器人操作场景中，该方法持续优于现有的模块化和单一模型基线。

Conclusion: 模块化扩散策略能更有效地拟合复杂动作分布，支持通过添加或微调组件灵活适应新任务，并缓解灾难性遗忘问题。
...
### [151] [StereoVLA: Enhancing Vision-Language-Action Models with Stereo Vision](https://arxiv.org/abs/2512.21970)
*Shengliang Deng,Mi Yan,Yixin Zheng,Jiayi Su,Wenhao Zhang,Xiaoguang Zhao,Heming Cui,Zhizheng Zhang,He Wang*

Main category: cs.RO

TL;DR: 提出StereoVLA模型，利用立体视觉的几何线索增强视觉-语言-动作模型的性能，通过几何-语义特征提取模块和辅助深度估计任务提升空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 立体视觉能提供丰富的空间线索，对机器人精确操作至关重要，但现有视觉-语言-动作模型对立体视觉的利用仍不足。

Method: 1) 设计几何-语义特征提取模块，融合立体视图差异的几何特征和单视图的语义特征；2) 引入交互区域深度估计辅助任务，增强空间感知并加速收敛。

Result: 在立体视觉设置下的多种任务中大幅超越基线模型，并对相机位姿变化表现出强鲁棒性。

Conclusion: StereoVLA模型有效利用立体视觉的几何信息，显著提升了视觉-语言-动作模型在机器人操作任务中的空间感知和指令跟随能力。
...
### [152] [Bab_Sak Robotic Intubation System (BRIS): A Learning-Enabled Control Framework for Safe Fiberoptic Endotracheal Intubation](https://arxiv.org/abs/2512.21983)
*Saksham Gupta,Sarthak Mishra,Arshad Ayub,Kamran Farooque,Spandan Roy,Babita Gupta*

Main category: cs.RO

TL;DR: 本文提出了一种紧凑型人机协同机器人插管系统（BRIS），集成了可操控纤维支气管镜、独立气管导管推进机制和摄像头增强口咽通道，通过实时形状感知和单目内镜深度估计实现直观遥操作和导管深度定位验证。


<details>
  <summary>Details</summary>
Motivation: 现有机器人插管系统主要关注气道导航，缺乏对气管导管推进的集成控制和基于解剖结构的导管深度客观验证，可能导致插管失败或位置不当引发严重并发症。

Method: 1. 集成四向可操控纤维支气管镜与独立导管推进机构；2. 采用学习型闭环控制框架，通过实时形状传感将摇杆输入映射至支气管镜末端笛卡尔空间运动；3. 利用单目内镜深度估计进行气道区域分类，提供解剖感知的导管定位引导；4. 在高保真气道人模上验证标准与困难气道场景。

Result: 系统在标准及困难气道配置下均实现可靠导航和可控导管置入，能够提供直观的遥操作体验和基于气管隆嵴的安全导管深度定位指导。

Conclusion: BRIS系统通过集成导航、推进与深度验证功能，向更安全、一致且符合临床工作流程的机器人气道管理迈出了重要一步。
...
### [153] [EcoNet: Multiagent Planning and Control Of Household Energy Resources Using Active Inference](https://arxiv.org/abs/2512.21343)
...
TL;DR: 本文提出了EcoNet，一种基于主动推理的贝叶斯方法，用于家庭和社区能源管理，旨在优化能源使用并协调冲突目标。


<details>
  <summary>Details</summary>
Motivation: 家庭能源管理系统（HEMS）在优化家庭能源设备调度方面具有潜力，但面临两个挑战：一是家庭目标可能复杂且冲突（如降低成本、减少排放与保持舒适度）；二是决策需在不确定性下进行（如天气和太阳能发电预测的不确定性）。

Method: 提出EcoNet方法，采用基于主动推理的贝叶斯框架，通过概率建模处理不确定性，并整合多目标优化以协调冲突的家庭偏好。

Result: 通过仿真实验验证了EcoNet在能源管理和协调方面的有效性，能够处理不确定性并平衡成本、排放和舒适度等目标。

Conclusion: EcoNet为家庭和社区能源管理提供了一种灵活且鲁棒的方法，能够适应不确定性并协调复杂目标，有望推动智能能源系统的发展。
...
### [154] [Adaptive Real-Time Scheduling Algorithms for Embedded Systems](https://arxiv.org/abs/2512.21364)
*Abdelmadjid Benmachich,Khadija Rais,Hamda Slimi*

Main category: eess.SY

TL;DR: 本文简要综述了动态不确定环境中嵌入式实时系统的自适应调度方法，涵盖反馈控制调度、任务互依调度、预测方法和DVFS功率管理，分析了适应性/可预测性权衡、评估指标及安全关键场景中的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统静态调度模型难以应对运行时工作负载变化、资源可用性波动和系统更新等动态环境需求，需要研究自适应实时调度方法以提升系统鲁棒性。

Method: 综述了反馈控制调度、周期性任务共生调度等模型，结合预测方法和基于动态电压频率调整的功率管理技术，分析不同机制的适应性/可预测性权衡。

Result: 总结了关键调度机制的特点、典型评估指标（如实时性、能效、稳定性），并指出安全关键场景中适应性调度面临的持续挑战。

Conclusion: 自适应实时调度是应对动态不确定环境的重要方向，需在适应性、可预测性和安全性之间取得平衡，为研究者和实践者提供了该领域的简明导引。
...
### [155] [Multi-Day Scheduling for Electric Vehicle Routing: A Novel Model and Comparison Of Metaheuristics](https://arxiv.org/abs/2512.21346)
*Dominik Köster,Florian Porkert,Klaus Volbert*

Main category: eess.SY

TL;DR: 本文提出了一种考虑电动汽车续航限制和充电时间的多日混合整数规划模型，并比较了多种元启发式算法的求解性能。


<details>
  <summary>Details</summary>
Motivation: 电动汽车使用量增加，需要高效的路程规划方案以解决续航焦虑和充电不便问题，尤其是在考虑不同类型充电站和用户日常行程时间窗口的情况下。

Method: 建立多日混合整数规划模型，考虑电池容量和用户行程时间窗口；使用禁忌搜索、自适应大邻域搜索和蚁群优化等元启发式算法求解；通过生成测试集评估算法性能，并与Google OR-Tools的精确解进行对比。

Result: 多种元启发式算法能够有效求解电动汽车路径规划问题，在计算效率和求解质量之间取得平衡；与精确求解器相比，元启发式方法在现实规模问题上表现出更好的可扩展性。

Conclusion: 所提出的多日规划模型和元启发式求解方法能够为电动汽车用户提供高效的日常行程路线，减少充电带来的不便，为跨平台导航系统提供了实用的解决方案。
...
### [156] [An Equivalent and Unified Virtual Battery Modeling Framework for Flexibility Characterization of Building HVAC Systems](https://arxiv.org/abs/2512.21363)
*Qi Zhu,Yu Yang,Liang Yu,Qing-Shan Jia,Costas J. Spanos,Xiaohong Guan*

Main category: eess.SY

TL;DR: 本文提出了一个统一的虚拟电池建模框架，用于表征单区和多区建筑HVAC系统的运行灵活性，使其能够像虚拟电池一样为电网提供服务。


<details>
  <summary>Details</summary>
Motivation: 建筑HVAC系统能耗高且具有显著的运行灵活性，可用于提供电网服务，但由于复杂的热力学特性、运行限制和舒适度约束，其灵活性难以表征，阻碍了相关应用。

Method: 首先识别具有物理意义的表示状态来描述热舒适约束下的建筑热条件，建立单区HVAC系统的虚拟电池模型；随后扩展至多区系统，建立分区虚拟电池模型；进一步开发系统化方法将分区模型聚合成低阶、低复杂度的聚合虚拟电池模型。

Result: 虚拟电池模型能够有效捕捉建筑HVAC系统的运行灵活性，支持有效的需求响应参与；基于该模型的需求响应策略可高效分解为分区控制指令，在维持人体热舒适的同时实现接近最优的运行成本。

Conclusion: 虚拟电池建模框架能准确表征HVAC系统的运行灵活性，降低模型和计算复杂度，为建筑参与电网需求响应提供了可行且高效的方法。
...
### [157] [Inter-seasonal and multi-objective optimization of a sustainable hydrogen supply chain in Corsica integrating water availability constraints](https://arxiv.org/abs/2512.21339)
*T. Moustapha Mai,C. Azzaro-Pantel,M. Chin Choi,M. Hajajji,C. Cristofari*

Main category: eess.SY

TL;DR: 本研究通过混合整数线性规划模型和多准则决策工具，优化岛屿地区氢能供应链，发现分散式结构可降低运输成本，氢平准化成本约6.54美元/千克，温室气体排放为1.32-1.75 kgCO2e/kg H2。


<details>
  <summary>Details</summary>
Motivation: 岛屿地区长期依赖化石燃料进口，本研究旨在探索氢能作为可持续能源载体在岛屿交通领域的应用潜力，推动能源转型。

Method: 采用多周期多目标混合整数线性规划模型，结合改进的TOPSIS多准则决策工具，综合考虑系统成本、温室气体排放和风险指数，并纳入水资源、可再生能源、旅游流量等岛屿特有因素。

Result: 优化结果显示分散式氢能供应链结构可最小化运输需求，氢平准化成本为6.54美元/千克，温室气体排放范围为1.32-1.75 kgCO2e/kg H2，旅游流量对能源需求有显著影响。

Conclusion: 研究表明氢能可作为岛屿可持续交通的可行解决方案，水资源可获得性是关键制约因素，提出的方法为岛屿特异性氢能供应链优化提供了新思路。
...
### [158] [Lyapunov-Based Kolmogorov-Arnold Network (KAN) Adaptive Control](https://arxiv.org/abs/2512.21437)
*Xuehui Shen,Wenqian Xue,Yixuan Wang,Warren E. Dixon*

Main category: eess.SY

TL;DR: 本文首次提出基于Lyapunov的Kolmogorov-Arnold网络（Lb-KAN）自适应控制方法，用于不确定非线性系统，相比传统MLP方法具有更好的可解释性和近似性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Lyapunov的深度神经网络（Lb-DNNs）依赖多层感知机（MLPs），缺乏可解释性；需要将可解释性嵌入控制架构中。

Method: 采用KANs近似不确定动态并嵌入控制律；基于Lyapunov分析构建解析更新律，实现实时无先验数据学习；利用KAN近似定理界定重构误差及其影响。

Result: 仿真表明，Lb-KAN控制器相比基线Lb-LSTM和Lb-DNN方法，函数近似误差分别降低20.2%和18.0%。

Conclusion: Lb-KAN通过可视化函数分解为可解释性提供了基础，实现了稳定、解析、实时的自适应控制，并保证跟踪误差的渐近收敛。
...
### [159] [Towards Learning-Based Formula 1 Race Strategies](https://arxiv.org/abs/2512.21570)
*Giona Fieni,Joschua Wüthrich,Marc-Philippe Neumann,Mohammad M. Moradi,Christopher H. Onder*

Main category: eess.SY

TL;DR: 提出了两个互补框架来优化F1赛车策略，联合考虑能量分配、轮胎磨损和进站时机，结合优化算法和强化学习方法。


<details>
  <summary>Details</summary>
Motivation: F1比赛中需要实时优化能量分配、轮胎磨损和进站策略，传统优化方法计算复杂，需要开发快速响应的决策支持工具。

Method: 1. 建立包含圈速映射和动态轮胎磨损模型的比赛场景；2. 使用混合整数非线性规划处理进站决策；3. 构建强化学习环境训练智能体实现实时推理。

Result: 强化学习智能体在1.5小时比赛中与最优解相比仅有约5秒的次优差距，主要源于能量分配策略差异，在正常和突发干扰场景下均表现稳定。

Conclusion: 本研究为基于学习的赛车策略奠定了基础，提供了未来发展的基准，强化学习方法适合在真实比赛中辅助人类决策。
...
### [160] [Asynchronous Averaging on Dynamic Graphs with Selective Neighborhood Contraction](https://arxiv.org/abs/2512.21721)
*Hsin-Lun Li*

Main category: eess.SY

TL;DR: 研究具有动态社交网络的离散时间共识模型，其中代理异步更新状态，邻居关系可收缩变化，证明在连通性假设下系统几乎必然达成共识。


<details>
  <summary>Details</summary>
Motivation: 扩展经典共识理论，研究动态社交网络中邻居关系可收缩的异步更新场景，以更好模拟现实社会系统的演化特性。

Method: 采用离散时间异步共识模型，每次随机选择一个代理，将其状态更新为当前邻居的平均值，并允许非选中代理自主增减邻居，形成内源性收缩的动态图结构。

Result: 在演化图无限频繁连通的温和假设下，证明系统几乎必然达成共识，扩展了时变图与异步更新的经典共识理论。

Conclusion: 选择性邻居收缩机制为演化社会系统中的共识动力学提供了新见解，模型能更真实反映动态社交网络的共识形成过程。
...
### [161] [Economic and Reliability Value of Improved Offshore Wind Forecasting in Bulk Power Grid Operation: A Case Study of The New York Power Grid](https://arxiv.org/abs/2512.21754)
*Khaled Bin Walid,Feng Ye,Jiaxiang Ji,Ahmed Aziz Ezzat,Travis Miles,Yazhou Leo Jiang*

Main category: eess.SY

TL;DR: 本研究评估了改进的海上风电预测对美国东海岸电网运行的经济和可靠性效益，提出了一种基于机器学习的预测模型和风险储备聚合方法，以纽约州为例展示了成本降低和可靠性提升的效果。


<details>
  <summary>Details</summary>
Motivation: 传统海上风电预测方法存在空间相关性考虑不足的问题，导致储备过度采购和成本增加，同时预测误差会影响电网运行的可靠性，因此需要更精确的预测和储备管理方法。

Method: 提出了一种基于机器学习的海上风电预测模型，并设计了风险储备聚合技术以利用空间分散效应；建立了考虑预测误差和电网条件的运行资源充足性框架，用于量化可靠性指标。

Result: 在2035年情景下，改进的预测使储备采购成本降低5.53%；应用风险储备聚合后总生产成本进一步降低7.21%；系统失负荷概率（LOLP）降低约19%。

Conclusion: 改进的海上风电预测结合风险储备聚合方法能显著降低电网运行成本并提升可靠性，为未来高比例可再生能源电网的运营提供了有效解决方案。
...
### [162] [Optimal Placement of Data Centers to Support Power Distribution Networks Using Intelligent Algorithms with Economic Indicators](https://arxiv.org/abs/2512.21987)
*Amin Hajihasani,Mahmoud Modaresi*

Main category: eess.SY

TL;DR: 该论文提出了一种数据中心选址与分布式发电容量配置的联合优化方法，通过遗传算法和多场景决策框架，在满足电网运行约束下平衡投资成本、网损和电压质量。


<details>
  <summary>Details</summary>
Motivation: 数据中心用电量快速增长，接入薄弱配电网时可能导致严重电压跌落和线路损耗，需要一种综合考虑技术经济因素的选址方法。

Method: 建立混合整数非线性优化模型，将数据中心选址映射到配电网节点，采用遗传算法嵌入多场景决策框架，通过自适应权重调整处理离散-连续搜索空间。

Result: 在IEEE 33节点系统中，优化方案选择节点14并配置1.10 MW分布式发电，总网损从202.67 kW降至129.37 kW，最低节点电压提升至0.933 pu，投资成本为133万美元。

Conclusion: 所提框架为将经济指标纳入配电网感知的数据中心选址提供了可解释的路径，能有效平衡技术性能与投资成本。
...
### [163] [A Graph-Augmented knowledge Distillation based Dual-Stream Vision Transformer with Region-Aware Attention for Gastrointestinal Disease Classification with Explainable AI](https://arxiv.org/abs/2512.21372)
*Md Assaduzzaman,Nushrat Jahan Oyshi,Eram Mahamud*

Main category: eess.IV

TL;DR: 提出一种基于师生知识蒸馏的双流深度学习框架，用于胃肠道疾病的内镜和组织病理图像分类，在保持高准确率的同时降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 胃肠道疾病的内镜和病理图像分类面临数据量大、类间视觉差异细微的挑战，需要兼顾诊断准确性和临床实用性。

Method: 采用师生知识蒸馏框架，教师模型结合Swin Transformer的全局上下文推理和Vision Transformer的局部细粒度特征提取，学生模型采用紧凑的Tiny-ViT结构通过软标签蒸馏继承知识。使用两个无线胶囊内镜数据集进行验证。

Result: 在两个数据集上分别达到0.9978和0.9928的准确率，平均AUC为1.0000。Tiny-ViT学生模型在保持可比性能的同时降低了计算复杂度，推理速度更快。Grad-CAM等可解释性分析证实模型关注临床相关组织区域。

Conclusion: 该框架为AI辅助胃肠道疾病诊断提供了鲁棒、可解释且可扩展的解决方案，适合资源受限的临床环境，为未来智能内镜筛查奠定了基础。
...
### [164] [RT-Focuser: A Real-Time Lightweight Model for Edge-side Image Deblurring](https://arxiv.org/abs/2512.21975)
*Zhuoyu Wu,Wenhui Ou,Qiawei Zheng,Jiayan Yang,Quanjun Wang,Wenqi Fang,Zheng Wang,Yongkui Yang,Heshan Li*

Main category: eess.IV

TL;DR: 提出轻量级U形网络RT-Focuser，用于实时图像去模糊，在速度与精度间取得平衡，适合边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: 相机或物体运动导致的运动模糊会严重降低图像质量，并对自动驾驶、无人机感知和医学成像等实时应用构成挑战。

Method: 设计了三个关键组件：用于边缘感知特征提取的轻量去模糊块（LD）、用于编码器集成的多级集成聚合模块（MLIA），以及用于渐进解码器细化的跨源融合块（X-Fuse）。

Result: RT-Focuser在单张模糊输入上训练，仅用5.85M参数和15.76 GMACs即达到30.67 dB PSNR；在GPU和移动设备上每帧运行6ms，帧率超过140 FPS。

Conclusion: RT-Focuser展示了在边缘设备上部署的强大潜力，实现了高效的实时图像去模糊。
...
### [165] [Enabling Ultra-Fast Cardiovascular Imaging Across Heterogeneous Clinical Environments with a Generalist Foundation Model and Multimodal Database](https://arxiv.org/abs/2512.21652)
...
TL;DR: 本文提出了CardioMM，一个用于超快速心脏磁共振成像的通用重建基础模型，并构建了最大的多模态CMR k空间数据库MMCMR-427K，旨在解决CMR扫描时间长和临床推广受限的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态心脏磁共振成像虽能提供全面的心血管疾病诊断信息，但其临床广泛应用受限于漫长的扫描时间和医疗环境的异质性，因此迫切需要一种能适应不同成像场景的快速重建基础模型。

Method: 首先构建了包含427,465个多线圈k空间数据的MMCMR-427K数据库，涵盖多个中心、模态、扫描器和疾病类型；在此基础上开发了CardioMM模型，该模型结合语义上下文理解和物理数据一致性，能动态适应异质成像场景。

Result: CardioMM在内部中心评估中达到最先进性能，并在未见过的外部环境中表现出强大的零样本泛化能力；即使在24倍加速成像下，仍能可靠保留关键心脏表型、心肌生物标志物和诊断图像质量。

Conclusion: 开放的MMCMR-427K数据库和CardioMM框架为心血管成像的高通量、高质量和临床可及性提供了一条可扩展的途径。
...
### [166] [The Color-Clinical Decoupling: Why Perceptual Calibration Fails Clinical Biomarkers in Smartphone Dermatology](https://arxiv.org/abs/2512.21988)
*Sungwoo Kang*

Main category: eess.IV

TL;DR: 研究发现智能手机远程皮肤病学中的色彩校准标准对肤色较深人群不足，存在'色彩-临床分离'现象，即色彩准确性不保证临床生物标志物可靠性，需区域感知协议。


<details>
  <summary>Details</summary>
Motivation: 当前智能手机远程皮肤病学假设色彩校准能确保临床可靠性，但这一假设未在代表性不足的皮肤光型（如Fitzpatrick III-IV）中得到验证，需要评估标准校准是否适用于临床生物标志物提取。

Method: 使用43,425张来自965名韩国受试者（Fitzpatrick III-IV）的图像，比较DSLR、平板和智能手机设备。采用线性色彩校正矩阵（CCM）归一化，分析色彩误差减少情况，并评估个体类型角度（ITA）和黑色素指数等生物标志物的设备间一致性。

Result: 线性CCM归一化将色彩误差减少67-77%，达到接近临床准确性（ΔE < 2.3），但生物标志物可靠性差：ITA的设备间一致性低（ICC = 0.40），而黑色素指数一致性较好（ICC = 0.77）。面部区域占色彩方差的25.2%，是设备效应（7.0%）的3.6倍。

Conclusion: 当前色彩校准标准不足以支持临床级生物标志物提取，因'色彩-临床分离'现象及解剖变异影响，需开发区域感知协议以提升移动皮肤病学的可靠性。
