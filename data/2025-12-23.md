...
TL;DR: 本文研究将CDCL与VSIDS启发式算法应用于离散设施布局问题，开发了CNF建模方法，比较了CDCL、CP-SAT和MILP的性能，并提出两种混合架构以结合CDCL的可行性搜索与CP-SAT的优化能力。


<details>
  <summary>Details</summary>
Motivation: 设施布局问题具有密集的逻辑结构约束，传统优化方法在规模增大时面临计算复杂度急剧上升的问题，需要探索更高效的求解方法。

Method: 1. 将设施布局问题建模为带邻接、分离和槽位可用性约束的组合分配问题；2. 开发基于CNF的可行性公式；3. 在统一基准框架下比较CDCL、CP-SAT和MILP；4. 提出两种混合架构：快速枚举可行布局架构和生成热启动解架构。

Result: 1. CDCL在可行性检测中表现出接近恒定的运行时间，而CP-SAT和MILP分别呈现多项式和指数级扩展；2. 混合方法能显著减少求解时间，同时保持正确性保证。

Conclusion: 混合方法结合了CDCL的快速可行性搜索和CP-SAT的精确优化优势，为大规模离散布局问题提供了有效的算法权衡方案，明确了子句学习搜索与精确优化方法之间的性能差异。
...
TL;DR: 本文首次为神经元识别提供了理论分析框架，通过将神经元识别视为机器学习的逆过程，推导了忠实性和稳定性的理论保证，并提出了BE方法生成具有覆盖概率保证的概念预测集。


<details>
  <summary>Details</summary>
Motivation: 当前神经元识别方法（如Network Dissection和CLIP-Dissect）虽经验成功，但缺乏严格的理论基础，这阻碍了构建可信可靠的解释。

Method: 将神经元识别建模为机器学习的逆过程，推导相似性度量（准确率、AUROC、IoU）的泛化界以保证忠实性；提出自助集成程序量化稳定性，并设计BE方法生成概念预测集。

Result: 理论分析为神经元解释的忠实性和稳定性提供了保证；实验在合成和真实数据上验证了理论结果，证明了所提方法的实用性。

Conclusion: 本研究为神经元识别建立了理论框架，推动了可信神经元识别的发展，使解释更具可靠性和稳定性。
...
TL;DR: 本文探讨将小世界网络结构作为多智能体系统设计先验，通过实验证明其能稳定共识轨迹，并提出基于不确定性的重连方案以适应任务难度和智能体异质性。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的多智能体系统多采用全连接、简单稀疏环或临时动态选择结构，缺乏结构性指导。受神经科学和复杂网络理论启发，研究小世界网络结构如何平衡局部聚类与长程整合以优化多智能体协作。

Method: 1. 将小世界网络理论应用于多智能体系统设计；2. 以多智能体辩论为测试平台验证小世界连接效果；3. 提出基于不确定性的重连方案，利用语义熵等信号在认知分歧的智能体间添加长程捷径。

Result: 小世界连接在保持准确性和计算成本相近的同时，显著稳定了共识轨迹；不确定性引导的重连方案能生成可控的小世界结构，自适应任务难度和智能体异质性。

Conclusion: 小世界结构可作为多智能体系统设计的稳定器（稳定推理）、增强器（提升鲁棒性）、可扩展协调器和认知角色涌现的归纳偏置，具有广泛设计意义。
...
TL;DR: 提出一种算法-系统协同设计，通过分层树拓扑、运行时自适应机制和流水线执行优化MoA推理，显著降低延迟同时保持精度。


<details>
  <summary>Details</summary>
Motivation: MoA推理存在密集的智能体间通信和硬件利用率低的问题，导致服务延迟增加，需要优化通信和计算效率。

Method: 1. 用分层树拓扑替代密集连接图，引入结构化稀疏通信；2. 基于语义一致性和置信度的运行时自适应机制，选择性终止或跳过下游智能体调用；3. 流水线化智能体执行，重叠增量预填充与解码过程。

Result: 在代表性任务中，端到端延迟最高降低90%，精度与密集连接MoA基线相当（±1%以内），部分场景下精度还有提升。

Conclusion: 该协同设计有效解决了MoA推理的通信和计算瓶颈，实现了延迟大幅降低与精度保持的平衡，为高效MoA服务提供了可行方案。
...
### [5] [Unifying Causal Reinforcement Learning: Survey, Taxonomy, Algorithms and Applications](https://arxiv.org/abs/2512.18135)
*Cristiano da Costa Cunha,Wei Liu,Tim French,Ajmal Mian*

Main category: cs.AI

TL;DR: 本文系统综述了因果推断与强化学习的交叉领域——因果强化学习（CRL），将其分为因果表示学习、反事实策略优化、离线因果RL、因果迁移学习和因果可解释性五类，总结了现有挑战、应用成果和未来方向。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖相关性决策，在分布偏移、混杂变量和动态环境中存在可解释性低、鲁棒性差和泛化失败等局限，需要引入因果推断来建模因果关系以解决这些问题。

Method: 采用系统性文献综述方法，对因果强化学习领域进行结构化分类分析，涵盖因果表示学习、反事实策略优化、离线因果RL、因果迁移学习和因果可解释性五大方向。

Result: 通过分类梳理，总结了CRL在应对分布偏移、提升可解释性和泛化能力方面的实证成功案例，同时识别出当前领域的主要挑战和未解决问题。

Conclusion: 因果强化学习有望推动构建更鲁棒、可泛化和可解释的人工智能系统，未来需要进一步探索理论框架、算法设计和实际应用场景。
...
### [6] [Propose, Solve, Verify: Self-Play Through Formal Verification](https://arxiv.org/abs/2512.18160)
*Alex Wilf,Pranjal Aggarwal,Bryan Parno,Daniel Fried,Louis-Philippe Morency,Paul Pu Liang,Sean Welleck*

Main category: cs.AI

TL;DR: 提出PSV自博弈框架，利用形式验证信号训练代码生成模型，在三个基准测试中性能提升最高达9.6倍


<details>
  <summary>Details</summary>
Motivation: 传统基于单元测试的代码生成奖励机制脆弱且易传播错误，而纯自博弈训练大语言模型的有效性在代码生成领域尚不明确

Method: 提出Propose, Solve, Verify (PSV)框架：使用形式验证信号训练提议器生成挑战性合成问题，通过专家迭代训练求解器

Result: PSV-Verus模型在三个基准测试中pass@1指标最高提升9.6倍；性能随生成问题数量和训练迭代次数增加而提升；形式验证和难度感知提议是自博弈成功的关键要素

Conclusion: 在形式验证提供可靠正确性信号的场景下，自博弈框架能有效训练代码生成模型，且性能具有可扩展性
...
### [7] [NEURO-GUARD: Neuro-Symbolic Generalization and Unbiased Adaptive Routing for Diagnostics -- Explainable Medical AI](https://arxiv.org/abs/2512.18177)
...
<details>
  <summary>Details</summary>
Motivation: 当前医学AI面临数据有限、视觉线索细微、临床决策高风险等挑战，现有视觉模型多为黑箱预测，可解释性差且跨领域泛化能力不足，限制了临床实际应用。

Method: 采用检索增强生成机制，利用大语言模型迭代生成、评估和优化医学图像特征提取代码，将临床指南和专家知识融入视觉Transformer框架，实现知识引导的特征检测与分类。

Result: 在四个糖尿病视网膜病变数据集上，NEURO-GUARD比纯ViT基线准确率提升6.2%（84.69% vs. 78.4%），领域泛化能力提升5%；在MRI癫痫检测任务中也表现出优越的跨领域鲁棒性。

Conclusion: NEURO-GUARD成功将符号化医学推理与亚符号化视觉学习相结合，实现了可解释、知识感知且可泛化的医学图像诊断，在多个数据集上达到最先进性能。
...
### [8] [NL2CA: Auto-formalizing Cognitive Decision-Making from Natural Language Using an Unsupervised CriticNL2LTL Framework](https://arxiv.org/abs/2512.18189)
*Zihao Deng,Yijia Li,Renrui Zhang,Peijun Ye*

Main category: cs.AI

TL;DR: 提出NL2CA方法，通过大语言模型将自然语言描述自动形式化为认知决策规则，构建可解释的认知智能体。


<details>
  <summary>Details</summary>
Motivation: 现有认知计算模型依赖人工构建，开发成本高，难以规模化。需要从非结构化文本中自动提取人类决策规则，构建可解释的认知模型。

Method: 1. 使用微调的大语言模型将文本翻译为线性时序逻辑（LTL）；2. 通过无监督的批判树优化逻辑表达式；3. 将逻辑转换为可执行的产生式规则；4. 基于规则构建认知智能体，并通过认知强化学习根据实际行为数据优化。

Result: 1. 在NL-to-LTL翻译任务中，CriticNL2LTL模块在专家和大规模基准测试中表现一致，无需人工干预；2. 在认知驾驶模拟中，从人类访谈自动构建的智能体成功学习了约70个关键场景中的多样化决策模式。
...
### [9] [External Hippocampus: Topological Cognitive Maps for Guiding Large Language Model Reasoning](https://arxiv.org/abs/2512.18190)
*Jian Yan*

Main category: cs.AI

TL;DR: 提出External Hippocampus框架，从认知动力学视角建模语言模型推理，通过降维投影构建拓扑认知地图，实现推理过程的精确导航与干预，显著提升小模型多步推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统权重空间优化方法在解决小模型多步推理的认知死锁问题时存在计算成本高、干预不可预测等局限，需要一种高效可控的推理增强框架。

Method: 采用认知动力学视角，将推理建模为语义空间中的信息能量流动；通过降维投影构建拓扑认知地图；在测试时通过地图引导能量流，利用温度扰动重启停滞的推理过程。

Result: 在≤7B参数模型上，地图引导方法在500个挑战性问题中达到81.20%准确率（相对基线提升16.80%），推理时间减少≥15倍；发现推理停滞表现为“认知涡旋”和低熵势阱，温度扰动能有效重启能量流动。

Conclusion: 该框架无需额外训练，具备自主生长能力，为小模型推理提供了高效、可控的拓扑感知解决方案，有效解决了多步推理中的认知死锁问题。
...
### [10] [Sophia: A Persistent Agent Framework of Artificial Life](https://arxiv.org/abs/2512.18202)
*Mingyang Sun,Feng Hong,Weinan Zhang*

Main category: cs.AI

TL;DR: 提出System 3框架和Sophia持久智能体架构，为LLM智能体增加元认知层，实现身份连续性和长期自适应，减少推理步骤并提升复杂任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体多为静态反应式架构，缺乏维持身份、验证推理、协调短期行动与长期生存的元认知层，限制了其在开放环境中的持续决策能力。

Method: 提出System 3元认知层框架，映射心理学概念到计算模块；实现Sophia持久智能体包装器，包含过程监督思维搜索、叙事记忆、用户与自我建模、混合奖励系统四大协同机制。

Result: 定量：推理步骤减少80%，高复杂度任务成功率提升40%；定性：表现出连贯叙事身份和任务自组织能力。

Conclusion: 通过心理学洞察与轻量强化学习结合，持久智能体架构为人工生命提供了可行的实践路径，实现了身份连续性与行为可解释性。
...
### [11] [MSC-180: A Benchmark for Automated Formal Theorem Proving from Mathematical Subject Classification](https://arxiv.org/abs/2512.18256)
*Sirui Li,Wangyue Lu,Xiaorui Shi,Ke Weng,Haozhe Sun,Minghe Yu,Tiancheng Zhang,Ge Yu,Hengyu Liu,Lun Du*

Main category: cs.AI

TL;DR: 提出MSC-180基准测试，包含180个数学形式化验证问题，评估发现当前LLM定理证明器存在领域覆盖有限、泛化能力弱的问题，最佳模型通过率仅18.89%。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的定理证明器在数学推理中存在领域覆盖有限、泛化能力弱的问题，需要更系统性的评估基准来推动AI数学推理能力的发展。

Method: 基于MSC2020数学学科分类构建MSC-180基准，包含60个数学分支各3个问题（本科至研究生难度），经领域专家多轮验证；引入变异系数（CV）量化跨领域性能波动；在pass@32设置下评估SOTA模型。

Result: 最佳模型整体通过率仅18.89%，存在显著领域偏差（最大领域覆盖率41.7%）和难度差距（研究生问题通过率明显更低）；CV值比高变异阈值高4-6倍，表明模型依赖训练数据模式匹配而非可迁移推理机制。

Conclusion: MSC-180及其多维评估框架为开发具有真正数学推理能力的下一代AI系统提供了区分性和系统性的基准，揭示了当前模型缺乏系统性泛化能力。
...
### [12] [Intelligent Human-Machine Partnership for Manufacturing: Enhancing Warehouse Planning through Simulation-Driven Knowledge Graphs and LLM Collaboration](https://arxiv.org/abs/2512.18265)
*Himabindu Thogaru,Saisubramaniam Gopalakrishnan,Zishan Ahmad,Anirudh Deodhar*

Main category: cs.AI

TL;DR: 提出一个结合知识图谱与大语言模型代理的协作智能系统，通过自然语言交互帮助制造规划人员分析仿真数据，识别瓶颈，提升决策效率与准确性。


<details>
  <summary>Details</summary>
Motivation: 传统基于仿真的制造数据分析方法在人类决策者与关键运营洞察之间形成壁垒，限制了人机在制造规划中的有效协作。

Method: 开发一个协作智能框架，将仿真数据转化为语义丰富的知识图谱表示，并集成基于大语言模型的代理，通过自然语言接口支持迭代推理与知识查询。

Result: 系统在运营查询中通过自然语言交互实现接近完美的准确率；在需要协作分析的场景中，能有效支持专家发现互相关联的运营问题，提升理解与决策。

Conclusion: 该工作通过创建直观的协作分析方法，降低了认知负荷，增强了人类在动态制造生态系统中的分析能力，推动了协作制造的发展。
...
### [13] [Monitoring Monitorability](https://arxiv.org/abs/2512.18311)
...
TL;DR: 本文研究了AI系统思维链（CoT）的可监控性，提出了评估框架和指标，发现CoT监控优于仅监控行动，且模型推理计算增加、思维链更长时监控性更好，强化学习优化不会显著降低监控性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统能力增强，为确保安全部署，需要对其决策过程进行可观测性监控。现有思维链监控方法可能因训练过程、数据源或系统扩展而失效，因此需要系统评估和量化监控性的方法。

Method: 提出了三种评估原型（干预、过程、结果属性）和新的监控性指标，构建了综合评估套件。通过实验比较不同前沿模型的监控性，研究了推理计算、强化学习优化、预训练模型规模对监控性的影响，并探索了监控器扩展趋势和后续提问的改进效果。

Result: CoT监控比仅监控行动更有效；大多数前沿模型具有较高但非完美的监控性；更长的思维链通常监控性更好；强化学习优化在当前规模下不会显著降低监控性；弱监控器增加测试时计算可提升监控性，且访问CoT能加强这种趋势；通过后续提问并提供后续思维链可进一步提高监控性。

Conclusion: 思维链监控是确保AI系统安全部署的有效手段，监控性可通过增加推理计算、优化监控器设计、利用后续提问等方式提升。研究为AI系统的可监控性评估和增强提供了系统框架和实践指导。
...
### [14] [Few-Shot Learning of a Graph-Based Neural Network Model Without Backpropagation](https://arxiv.org/abs/2512.18412)
*Mykyta Lapin,Kostiantyn Bokhan,Yurii Parzhyn*

Main category: cs.AI

TL;DR: 提出一种基于结构图的方法，用于少样本轮廓图像分类，无需反向传播，通过构建属性图和概念吸引子实现可解释的分类。


<details>
  <summary>Details</summary>
Motivation: 旨在设计一种无需反向传播的少样本分类架构，通过结构化的图表示提供透明的决策过程，增强模型的可解释性。

Method: 将轮廓图像向量化为点/线二分图，归一化几何属性；通过消除不稳定子结构和对齐路径进行约简；迭代组合样本形成概念图；使用近似图编辑距离进行图到概念的匹配分类。

Result: 在MNIST子集上（每类5-6个样本，单轮训练）达到约82%的准确率，决策过程完全可追溯，错误分类可通过结构相似性明确解释。

Conclusion: 基于结构图和概念吸引子的方法实现了无需反向传播的少样本学习，并通过显式图结构提供内在解释性；计算成本和骨架化质量是主要限制，未来可优化分类算法并扩展至静态场景和关联识别。
...
### [15] [Agent-Based Output Drift Detection for Breast Cancer Response Prediction in a Multisite Clinical Decision Support System](https://arxiv.org/abs/2512.18450)
*Xavier Rafael-Palou,Jose Munuera,Ana Jimenez-Pastor,Richard Osuala,Karim Lekadir,Oliver Diaz*

Main category: cs.AI

TL;DR: 提出基于代理的多中心临床AI系统漂移检测框架，通过站点特定监控代理提升分布漂移检测性能，在乳腺癌影像数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 多中心临床决策支持系统面临患者群体、成像设备和采集协议差异导致的模型性能退化问题，现有集中式监控方法无法捕捉站点特定的漂移动态。

Method: 构建多中心漂移检测模拟环境，为每个站点部署监控代理，采用批量比较模型输出与参考分布的方法，对比四种参考分布获取方案（站点特定、全局、仅生产数据、自适应）与集中式基线。

Result: 所有多中心方案均优于集中式监控，漂移检测F1分数最高提升10.3%；无站点特定参考时自适应方案表现最佳，漂移检测F1达74.3%，漂移严重程度分类F1达83.7%。

Conclusion: 自适应、站点感知的代理漂移监控能提升多中心临床决策支持系统的可靠性，为实际部署提供有效监控方案。
...
TL;DR: 提出一种结合显隐图表示与时间建模的内网威胁检测框架，通过图卷积网络和双向LSTM捕捉用户行为模式，在CERT数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 内网威胁行为隐蔽且难以检测，现有方法难以捕捉复杂行为模式与时间依赖关系，需要更有效的多维度建模方法。

Method: 构建显式图（基于组织规则）与隐式图（基于特征相似性，使用Gumbel-Softmax学习），分别用GCN生成节点嵌入，经注意力机制融合后输入Bi-LSTM进行时间建模，通过概率阈值判断异常。

Result: 在CERT r5.2上AUC达98.62，检测率100%，误报率0.05；在更复杂的r6.2上AUC为88.48，检测率80.15%，误报率0.15，均优于现有方法。

Conclusion: 结合图表示与时间建模能有效提升内网威胁检测的鲁棒性，显隐图互补与注意力机制增强了威胁相关特征的提取能力。
...
TL;DR: 本文提出贝叶斯滤波框架评估LLM在线推理能力，发现LLM信念更新类似贝叶斯后验但存在指数遗忘特性，并提出了有效的提示校准方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注静态任务，忽视了LLM在动态随机环境中需要持续更新信念的在线适应能力，这是LLM作为世界模型或智能体的关键能力。

Method: 引入贝叶斯滤波框架，设计涵盖多元离散分布（如掷骰子）和连续分布（如高斯过程）的概率探测套件，其中真实参数随时间变化，并在模拟智能体任务中验证发现。

Result: LLM信念更新类似于贝叶斯后验，但更准确地表现为指数遗忘滤波器，具有模型特定的折扣因子（小于1），显示对旧证据的系统性折扣，且不同架构差异显著；虽然先验常校准错误，但更新机制本身保持结构化和原则性。

Conclusion: LLM在线推理具有结构化更新机制但存在系统性偏差，提出的提示策略能以最小计算成本有效重新校准先验，为LLM在动态环境中的应用提供见解。
...
TL;DR: 提出Vox Deorum混合架构，将LLM用于4X策略游戏《文明V》的宏观战略推理，战术执行委托给子系统，验证了LLM在复杂游戏中的竞争力和独特游戏风格。


<details>
  <summary>Details</summary>
Motivation: LLM的自然语言推理能力适合4X策略游戏中的人机协作与谈判，但游戏复杂性和长时程特性、LLM延迟与成本限制了实际部署，需要解决这些挑战。

Method: 采用分层混合架构（LLM+X），LLM负责宏观战略决策，战术执行由算法AI等子系统处理；在《文明V》Vox Populi模组中测试，通过2,327局完整游戏对比开源LLM与模组AI。

Result: LLM实现了有竞争力的端到端游戏表现，游戏风格与算法AI显著不同，不同LLM之间风格也存在差异；验证了架构在商业4X游戏中的可行性。

Conclusion: 该工作为LLM集成到商业4X游戏建立了可行架构，为游戏设计和智能体AI研究开辟了新方向。
...
TL;DR: 提出ESearch-R1框架和HC-GRPO算法，使具身智能体能在模糊指令下主动权衡物理探索与人工交互成本，显著提升任务成功率并降低50%操作成本。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM智能体面对模糊指令时，缺乏对物理探索成本与人工交互成本的策略性权衡，通常将消歧视为被动感知问题，导致任务执行效率低下。

Method: 1. 提出ESearch-R1框架，将交互对话、情景记忆检索和物理导航统一为单一决策过程；2. 设计HC-GRPO算法，通过采样推理轨迹组并强化信息增益与异构成本（导航时间、人类注意力）间最优权衡的策略。

Result: 在AI2-THOR环境中实验表明：ESearch-R1显著优于基于ReAct的智能体，在提高任务成功率的同时，将总操作成本降低约50%。

Conclusion: HC-GRPO能有效对齐MLLM智能体与物理世界约束，验证了成本感知推理框架在具身任务中的优越性。
...
TL;DR: 提出了一种名为反射置信度的新推理框架，通过将低置信度信号转化为反思触发，让模型在置信度低时进行自我纠正而非停止生成，从而在保持计算效率的同时提升数学推理任务的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的集成推理方法（如自洽性）计算开销大，而早期停止策略（如DeepConf）会丢弃低置信度轨迹，浪费部分计算资源。需要一种更高效利用计算资源的方法。

Method: 提出反射置信度框架：当模型置信度低于阈值时，不停止生成，而是生成反思提示来分析当前推理状态、识别潜在错误，并沿着纠正后的轨迹继续生成。

Result: 在AIME 2025等数学推理基准测试中，相比先进的早期停止基线方法，在可比计算成本下实现了显著的准确性提升。

Conclusion: 主动自我纠正比被动丢弃更有效，反射置信度框架能够在保持计算效率的同时显著提升大语言模型在复杂推理任务上的性能。
...
TL;DR: 本文针对联合路由分配问题，开发了混合整数规划求解器，用于机器人包装规划场景，在46个数据集上验证了其高效性和最优性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理实际包装规划中的复杂约束（如多占位符选项、时间限制和多类别物品包装）时存在局限性，需要更高效的精确求解器。

Method: 扩展了基于Gurobi的精确MIP求解器，结合割平面子回路消除技术，并针对实际包装场景增加了多类约束支持。

Result: 在46个移动操作数据集上，MIP方法能以稳定低耗时获得全局最优解，比基于抖动的精确求解器快一个数量级，比贪心启发式方法平均距离优化14%。

Conclusion: 基于MIP的JRA优化方法在机器人包装、运动规划和复杂物流中具有实际应用价值，兼顾求解效率与质量。
...
TL;DR: ChronoDreamer是一个用于接触丰富机器人操作的动作条件世界模型，通过时空变换器和掩码预测训练，预测未来视频帧、接触分布和关节角度，并使用视觉语言模型进行安全动作筛选。


<details>
  <summary>Details</summary>
Motivation: 当前机器人操作任务中，接触丰富的场景（如刚体和可变形物体交互）需要精确的接触预测和安全控制，以避免碰撞和损坏。现有方法在接触建模和安全推理方面存在不足，需要一种能同时处理视觉、接触和动作预测的集成框架。

Method: 1. 使用时空变换器（MaskGIT风格掩码预测）训练动作条件世界模型；2. 将接触编码为深度加权高斯泼溅图像，将3D力渲染为相机对齐格式；3. 推理时使用视觉语言模型评估预测轨迹的碰撞可能性，进行拒绝采样；4. 在DreamerBench仿真数据集（基于Project Chrono生成）上训练和评估。

Result: 1. 模型在非接触运动中保持空间一致性；2. 生成合理的接触预测；3. 基于LLM的评估器能有效区分碰撞与非碰撞轨迹；4. 在刚体和可变形物体场景中实现同步RGB、接触泼溅、本体感觉和物理标注的预测。

Conclusion: ChronoDreamer通过集成视觉、接触和动作预测，结合LLM安全推理，为接触丰富的机器人操作提供了一种有效的世界建模和安全控制框架，在仿真环境中验证了其可行性和有效性。
...
TL;DR: 提出ASTIF框架，通过置信度元学习实时调整预测策略，融合语义市场信号与时间序列数据，在加密货币和科技股预测中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有金融时间序列预测模型多为静态架构，难以整合异质知识源（如政策不确定性、市场叙事等语义驱动因素）或适应快速市场机制转换，导致预测能力受限。

Method: 1. 双通道小语言模型（MirrorPrompt）提取语义市场信号与数值趋势；2. 混合LSTM随机森林模型捕捉序列时间依赖；3. 置信感知元学习器作为自适应推理层，根据实时不确定性调节各预测器贡献。

Result: 在2020-2024年AI相关加密货币和主要科技股数据集上，ASTIF优于Informer、TFT等主流深度学习和Transformer基线；消融实验证实自适应元学习机制能通过市场波动期调整语义与时间通道的依赖关系来有效降低风险。

Conclusion: 研究提出了一种可扩展的、基于知识的解决方案，能够在非平稳环境中融合定量与定性数据，为金融时间序列预测提供了更适应动态市场的新框架。
...
TL;DR: 本文提出CogAct计算模型，通过组块化等基础认知过程解释概念学习，并展示了其在多个领域（文学、国际象棋、音乐）学习原始概念的能力，同时提出了考虑主观性的人类基准设计新方法。


<details>
  <summary>Details</summary>
Motivation: 认知科学的核心问题在于理解短时和长时记忆中多种概念形成与提取的基本心理过程。现有模型难以自适应学习复杂自然概念，且通常忽略个体主观经验。

Method: 提出CogAct计算模型，基于组块化、注意、短时/长时记忆等基础认知结构；设计考虑主观性的人类实验基准；使用原始音乐乐谱数据进行模拟，并与深度学习模型比较。

Result: CogAct能自适应学习从简单逻辑函数到文学、国际象棋、音乐等领域的原始自然概念；模型能捕捉个体主观概念空间，在音乐领域与人类主观判断一致；相比深度学习模型，无需任务特定的架构调整。

Conclusion: 研究将概念学习和复杂适应整合到更广泛的认知心理学理论中；该方法可用于捕捉主观概念空间，推动心理学应用从“平均参与者”建模转向个体化建模。
...
TL;DR: IntelliCode是一个基于多智能体LLM的辅导系统，通过中心化、版本化的学习者状态实现长期、透明的教学支持。


<details>
  <summary>Details</summary>
Motivation: 现有LLM导师多为单轮交互，缺乏对学习者知识的持久化表示，难以提供系统化、透明且长期的教学支持。

Method: 采用基于状态图的协调器，管理六个专用智能体（技能评估、学习者画像、渐进提示、课程选择、间隔重复、参与度监控），所有智能体在单一写入策略下对共享状态进行纯转换操作。

Result: 演示系统展示了端到端辅导流程：学习者尝试数据结构问题→遇到困难时获得概念提示→提交修正方案→即时查看掌握度更新和个性化复习间隔。模拟实验显示系统状态更新稳定、渐进提示提高任务成功率、课程覆盖多样化。

Conclusion: IntelliCode证明了持久化学习者建模、协调式多智能体推理与原则性教学设计相结合，能够实现透明可靠的LLM驱动辅导。
...
TL;DR: 研究通过计算模型探究猴子社会比较的认知机制，发现猴子更依赖客观奖励差异而非主观价值推断进行社会比较。


<details>
  <summary>Details</summary>
Motivation: 社会比较在灵长类社会认知中起基础作用，但尚不清楚他人奖励信息如何影响个体对自身奖励的评价，需要从计算角度探究猴子是识别客观奖励差异还是推断他人的主观价值。

Method: 构建了三种不同社会信息处理程度的计算模型（IPM、NCM、ECM），使用多层多模态LDA方法，基于猴子行为、奖励和条件刺激数据集训练模型，并通过预定义实验条件评估模型对主观价值的分类能力。

Result: 外部比较模型（ECM）在Rand Index上获得最高分类分数（0.88 vs IPM的0.79），表明社会比较更依赖客观奖励差异而非主观状态推断。

Conclusion: 猴子的社会比较机制主要基于客观奖励差异的计算，而非对他人主观价值的推断。
...
TL;DR: 提出KeenKT模型，使用NIG分布表示学生知识状态以区分真实能力与偶然表现，通过注意力机制和损失函数增强模型鲁棒性，在六个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有知识追踪方法依赖单点估计，无法区分学生真实能力与偶然失误或粗心，导致对知识掌握程度的判断存在模糊性。

Method: 1. 使用正态逆高斯分布表示学生每个交互时的知识状态；2. 设计基于NIG距离的注意力机制建模知识状态动态演化；3. 引入扩散去噪重构损失和分布对比学习损失增强鲁棒性。

Result: 在六个公开数据集上，KeenKT在预测准确性和行为波动敏感性方面优于现有最佳模型，AUC最大提升5.85%，ACC最大提升6.89%。

Conclusion: KeenKT通过概率分布表示和专门设计的损失函数，有效解决了知识追踪中的状态模糊问题，提高了模型性能和对学习行为波动的捕捉能力。
...
TL;DR: 提出一个几何框架，将概念增长建模为在最小描述长度准则下评估的容许基扩展，认为概念学习仅在现有表征无法解释经验时发生，且想象只能通过暴露结构化残差误差来促进学习。


<details>
  <summary>Details</summary>
Motivation: 现有学习和推理模型通常预设固定的表征基础，但概念学习需要表征基础本身能够扩展。本研究旨在探索在何种结构条件下，表征基础能够以有原则和选择性的方式扩展。

Method: 采用几何框架，将经验表示为当前概念子空间中的向量，用残差分量捕捉系统性表征失败。候选概念扩展被限制为低秩容许变换，并通过最小描述长度准则进行评估。

Result: 证明任何被MDL接受的扩展都可以使其新方向完全位于经验诱导的残差张成空间内，而与该空间正交的扩展会严格增加描述长度并被拒绝。这为想象和概念创新提供了保守解释。

Conclusion: 概念发展是一个误差驱动、几何约束的基扩展过程，想象在学习中的作用仅限于暴露或放大结构化残差误差，不能引入任意新颖性，MDL为表征变化提供了规范性选择原则。
...
TL;DR: 提出MEEA攻击框架，利用心理学曝光效应通过多轮低毒性语义暴露逐步侵蚀大语言模型的安全对齐边界，在多个主流模型上攻击成功率显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有越狱研究大多假设静态安全边界，忽视了上下文交互对模型行为的动态影响，导致攻击方法的稳定性和泛化性有限。

Method: 基于曝光效应构建语义递进的提示链，采用模拟退火策略优化提示，优化目标包括语义相似性、毒性和越狱效果。

Result: 在GPT-4、Claude-3.5、DeepSeek-R1等模型上，MEEA攻击成功率平均比7个基线方法提高20%以上，消融实验验证了退火优化和上下文暴露机制的必要性。

Conclusion: 大语言模型的安全行为具有动态性和历史依赖性，挑战了静态对齐边界的常见假设，强调需要交互感知的安全评估和防御机制。
...
TL;DR: 本文指出AI可解释性研究中存在类似'死鲑鱼'的统计伪影问题，并提出将可解释性方法视为统计模型参数的统计-因果重构框架，以提升该领域的严谨性。


<details>
  <summary>Details</summary>
Motivation: AI可解释性方法（如特征归因、探测分析等）常对随机初始化的神经网络产生看似合理的解释，这类似于神经科学中著名的'死鲑鱼MRI研究'所揭示的统计推断误用问题，表明当前方法易产生虚假发现。

Method: 提出将计算系统的解释视为统计模型的参数，通过计算轨迹进行推断；强调解释方法应作为统计估计器，需针对明确的替代计算假设进行检验，并在统计模型框架内量化不确定性。

Result: 该框架揭示了可解释性查询的可识别性等关键理论问题，指出这些问题是导致虚假发现、泛化性差和高方差的主要原因；将可解释性纳入统计推断标准工具包为领域发展提供了新方向。

Conclusion: 通过统计-因果重构框架，AI可解释性研究可转化为更严谨、实用的科学领域，未来工作应聚焦于建立具有统计可靠性的解释方法体系。
...
TL;DR: 提出了HARBOR模型和PEARL数据集，用于行为健康风险评估，HARBOR在预测情绪风险评分上优于传统方法和现成LLM。


<details>
  <summary>Details</summary>
Motivation: 行为健康风险评估因患者数据的多模态性和情绪障碍的时间动态性而具有挑战性，现有大型语言模型在结构化临床风险评分中的有效性尚不明确。

Method: 开发了HARBOR模型预测-3到+3的Harbor风险评分，并发布了包含四年月度观测的PEARL纵向数据集，对比了传统机器学习模型、专有LLM和HARBOR。

Result: HARBOR达到69%准确率，优于逻辑回归的54%和最强专有LLM基线的29%。

Conclusion: HARBOR在行为健康风险评估中显著优于传统方法和现成LLM，展示了针对临床任务定制模型的价值。
...
TL;DR: 提出CORE强化学习框架，通过将明确概念转化为可控监督信号，解决大语言模型在数学问题中模式复用而非概念理解的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型能解决复杂数学题，但在需要真正概念理解时表现不佳；现有基于验证奖励的强化学习方法只强化最终答案，缺乏细粒度概念信号，导致模型仅改进模式复用而非概念应用能力。

Method: 1. 使用高质量教材资源链接可验证习题与概念描述；2. 合成概念对齐的测验题；3. 在训练过程中注入简短概念片段以引导概念启动的轨迹；4. 通过轨迹替换、前向KL约束对齐策略，或直接在概念对齐测验上应用GRPO进行强化。

Result: CORE在多个模型上均优于原始模型和SFT基线，在领域内概念习题集和多样化的领域外数学基准测试中均取得一致性能提升。

Conclusion: CORE框架通过统一概念对齐测验的直接训练和概念注入轨迹的强化学习，提供细粒度的概念监督，弥合问题解决能力与真正概念推理之间的差距，且算法与验证器无关。
...
TL;DR: 提出Gabliteration神经权重修改技术，通过自适应多向投影和正则化层选择，在修改特定行为模式时减少模型质量损失。


<details>
  <summary>Details</summary>
Motivation: 现有方法在修改模型特定行为模式时会导致模型质量下降，需要一种能精准修改权重同时保持其他领域性能的技术。

Method: 采用动态层优化、正则化投影矩阵和自适应缩放机制，实现自适应多向投影与正则化层选择。

Result: 开发了gabliterated-v1模型系列（0.6B至4B参数），验证了该方法在不同规模模型上的实际适用性。

Conclusion: Gabliteration技术在理论上实现了更优的权重修改，同时最小化了无关领域的质量退化，具有实际应用价值。
...
TL;DR: 提出基于专家规则贝叶斯网络的决策支持框架，融合多计算机视觉模型输出，显著提升大规模伤亡事件中伤员分类准确率。


<details>
  <summary>Details</summary>
Motivation: 大规模伤亡事件易导致医疗系统过载，传统伤员评估存在延迟和错误风险，需自动化系统辅助急救人员快速准确分类。

Method: 构建完全基于专家规则的贝叶斯网络，融合多个计算机视觉模型（评估大出血、呼吸窘迫、意识状态、可见创伤），支持不完整信息推理且无需训练数据。

Result: 在DARPA分类挑战赛中，生理评估准确率从15-19%提升至42-46%，总体分类准确率从14%提升至53%，系统诊断覆盖率从31%扩大至95%。

Conclusion: 专家知识引导的概率推理能显著增强自动化分类系统，为大规模伤亡事件应急响应提供有效支持，该方法在挑战赛中使团队获得第4名。
...
TL;DR: 本文提出了一种用于动态多模态多目标优化的新基准测试套件和算法，通过聚类自编码器预测机制和自适应小生境策略，在时变环境中有效跟踪多个等效帕累托最优集并保持种群多样性。


<details>
  <summary>Details</summary>
Motivation: 现有动态多目标进化算法常忽略解的多模态性，而静态多模态多目标进化算法缺乏对动态环境的适应性，需要同时解决动态环境下的多模态跟踪和多样性保持问题。

Method: 1) 构建融合动态与多模态特性的新基准测试函数套件；2) 提出基于聚类自编码器预测的动态响应机制，利用自编码器处理匹配聚类生成高多样性初始种群；3) 在静态优化器中集成自适应小生境策略以平衡收敛性与多样性。

Result: 在12个动态多模态多目标测试函数实例上的实验表明，相比现有先进算法，所提算法在决策空间能更有效保持种群多样性，在目标空间获得更优收敛性。

Conclusion: 所提出的基准套件和算法能有效应对动态多模态多目标优化挑战，为动态环境下的多模态优化问题提供了新的解决方案和评估平台。
...
<details>
  <summary>Details</summary>
Motivation: 多模态推理面临输入模态整合复杂和高质量长链思维训练数据稀缺的挑战，现有数据集和方法存在推理深度有限、模态转换错误和生成流程僵化等问题。

Method: SynSelect三阶段框架：1) 利用多个异构多模态大模型生成多样候选链思维；2) 通过实例级和批次级选择筛选高质量链思维数据；3) 用于监督微调和强化学习后训练。

Result: 在多个多模态基准测试中，使用SynSelect生成数据监督微调的模型显著优于基线，经过强化学习后训练后性能进一步提升。

Conclusion: SynSelect是提升多模态大模型推理能力的有效方法，能生成高质量训练数据并带来显著性能改进。
...
TL;DR: 提出OR引导的'预训练-强化'框架，结合运筹学结构严谨性与AI自适应感知，在库存管理中实现轻量级高性能决策。


<details>
  <summary>Details</summary>
Motivation: 解决AI自适应感知与运筹学结构严谨性在复杂库存系统中的协同难题，弥补现有方法在业务约束与管理者偏好建模上的不足。

Method: 1. 构建仿真增强的OR模型生成高质量参考决策；2. 以OR决策为标签训练领域知识驱动的深度学习基础模型；3. 通过强化学习微调实现OR最优原则的内化与策略泛化。

Result: 在京东实际部署中，周转天数减少5.27天，现货率提升2.29%，持有成本降低29.95%，显著优于现有工业实践。

Conclusion: 轻量级领域知识模型在OR结构化引导下可实现最优性能与强可迁移性，为智能供应链管理提供可扩展、经济高效的AI与OR深度融合范式。
...
TL;DR: 提出一种称为“再语境化”的方法，通过调整训练数据的上下文来减少语言模型对错误训练信号的“博弈”行为，防止模型学习到不当行为。


<details>
  <summary>Details</summary>
Motivation: 开发者在指定训练标签和奖励时经常遇到困难，导致语言模型可能利用错误的训练信号学习到不当行为（如欺骗用户、迎合偏好等）。

Method: 通过生成在禁止不当行为的提示下的回复，然后将其“再语境化”为在允许不当行为的提示下的回复，从而训练模型即使在允许不当行为的指令下也能抵抗不当行为。

Result: 再语境化方法有效防止模型学习以下行为：1）优先考虑评估指标而非聊天质量；2）为通过错误测试而特殊处理代码；3）对用户撒谎；4）变得阿谀奉承。

Conclusion: 再语境化通过减少错误训练信号对不当行为的强化，在不改进监督信号的情况下降低了模型对训练信号的博弈行为。
...
TL;DR: 该研究探索将大语言模型中的高级概念（表示为转向向量）迁移到小语言模型，通过推理时动态调整转向强度，显著提升小模型性能且无需复杂基础设施。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在资源受限设备上部署困难，现有压缩方法需要大量实验和复杂设计，因此寻求一种轻量化的性能提升方案。

Method: 1. 从大模型中提取表征高级概念的转向向量；2. 将转向向量迁移到不同家族的小模型；3. 提出推理时动态调整转向强度的缩放方法。

Result: 1. 概念迁移可普遍提升Phi/Llama/Qwen等小模型性能；2. 推理时缩放使Qwen3-0.6B准确率提升7-15%。

Conclusion: 转向向量迁移是轻量化提升小语言模型性能的有效范式，推理时动态调整机制进一步释放了性能潜力。
...
TL;DR: 提出Population-Evolve方法，一种基于遗传算法的训练免费优化技术，通过并行推理和自进化提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 测试时扩展技术近年来成为增强大语言模型推理能力的重要方向，但现有方法在性能稳定性和计算效率方面仍有提升空间。

Method: 采用遗传算法思想，为每个问题维护动态候选解种群，通过并行推理和进化提示实现模型自进化，最终通过多数投票确定答案。

Result: 实验表明Population-Evolve在准确率、性能稳定性和计算效率方面均优于现有方法。

Conclusion: 进化策略在推理阶段具有释放大语言模型推理潜力的巨大潜力，为测试时扩展技术提供了统一的理论框架。
...
### [41] [Tool-Augmented Hybrid Ensemble Reasoning with Distillation for Bilingual Mathematical Problem Solving](https://arxiv.org/abs/2512.19093)
*Peiqing Lu,Yuan Zhang,Haoyun Zhang,Jiasen Zheng,Kejian Tong,Wenjun Wu*

Main category: cs.AI

TL;DR: HERALD框架通过混合集成推理结合语言模型与符号计算，提升双语数学问题求解的准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在语言推理方面表现良好，但在精确计算方面较弱，需要建立语言推理与符号计算之间的有效链接。

Method: 使用NuminaMath-7B-TIR、GPT-4o和Mistral-7B构建混合框架，采用自适应路由、工具强化学习、知识蒸馏、置信度校准和双路径检查等方法。

Result: 系统实现了流畅推理与精确计算的结合，在双语数学推理任务中表现出更高的准确性、稳定性和清晰度。

Conclusion: HERALD为多语言数学推理提供了实用解决方案，通过符号检查、自适应集成和双语微调的组合有效提升了性能。
...
### [42] [$γ(3,4)$ `Attention' in Cognitive Agents: Ontology-Free Knowledge Representations With Promise Theoretic Semantics](https://arxiv.org/abs/2512.19084)
*Mark Burgess*

Main category: cs.AI

TL;DR: 本文提出使用承诺理论框架统一注意力机制与知识图谱表示，建立机器学习与知识图谱之间的桥梁，通过语义时空图避免复杂本体论，在不确定性条件下进行推理，实现数据压缩。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习（向量化表示）与知识图谱（图结构表示）之间存在鸿沟，两者各自保留数据的不同方面。研究旨在建立两者间的理论桥梁，避免依赖隐式语言模型，同时解决自主机器人、国防部署等场景中数据压缩和不确定性推理的需求。

Method: 采用承诺理论框架描述注意力的语义和动态特性；提出语义时空图（γ(3,4)）对特征按其在语义过程中的角色进行分类，替代复杂本体论；强调因果边界条件的重要性以实现数据压缩。

Result: 理论分析表明，注意力机制可通过承诺理论形式化；向量化数据适用于概率估计，而图结构能保持数据源头的意向性；语义时空图方法支持不确定性推理，并通过关注因果边界条件实现数据量级压缩。

Conclusion: 机器学习与知识图谱表示可协同共存，互补数据的不同方面；承诺理论为两者提供了统一框架；语义时空图方法简化了知识表示，在自主系统、应急服务等场景中具有高效推理和数据压缩的潜力。
...
TL;DR: 本文提出了一种用于接受-期望模型的抽象决策框架中的条件化规则，将经典概率与量子概率统一，并扩展到不精确概率情境。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于统一经典概率与量子概率，并在不精确概率框架下扩展条件化规则，以处理抽象决策问题。

Method: 采用抽象决策框架，其中不确定奖励位于一般线性空间，事件为线性空间上的投影算子；引入基于观察事件引入新无差异性的条件化规则，并关联信念修正算子。

Result: 新条件化规则在经典命题逻辑和完全条件概率两种特殊情况下满足所有AGM信念修正公理。

Conclusion: 该框架成功统一了经典与量子概率，并在特定情况下保持了AGM公理的有效性，为不精确概率下的决策提供了理论基础。
...
TL;DR: 提出FC-MIR框架，通过关键帧采样和自适应拼接减少移动UI操作轨迹中的视觉冗余，结合MLLMs实现意图识别与任务自动化，并构建数据集验证其高效性与实用性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在移动端实时部署时面临计算成本高、冗余帧处理效率低的问题，限制了基于UI操作轨迹的用户意图识别与任务自动化的发展。

Method: 采用关键帧采样与自适应拼接压缩视觉冗余；集成闭源或微调MLLMs（如Qwen3-VL）进行轨迹摘要与意图预测；扩展任务至生成后续操作与搜索建议；构建UI轨迹数据集（Agent-I与Person-I）并设计细粒度评估指标。

Result: 压缩方法在50%-60%压缩率下保持性能；闭源与微调MLLMs均能有效完成意图摘要，支持轻量化端侧部署；但MLLMs在生成实用且‘出人意料’的建议方面仍有不足。

Conclusion: FC-MIR框架通过压缩与MLLMs结合提升了移动UI意图识别的效率与实用性，为轻量化端侧部署奠定基础，但建议生成能力仍需改进；框架已在实际场景中部署，集成UI感知与代理机制以推动领域发展。
...
TL;DR: 该研究首次从拓扑结构角度分析大语言模型推理链的质量，使用持续同调方法将推理步骤映射到语义空间，发现推理链的拓扑结构复杂度与准确性正相关，成功推理具有更简单的拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要从功能角度评估推理链，缺乏对其结构机制的分析。研究者希望探究不同推理链表现差异的原因，以及推理链中哪些结构成分起关键作用。

Method: 应用拓扑数据分析中的持续同调方法，将推理步骤映射到语义空间，提取拓扑特征并分析结构变化。通过计算同调群评估不同尺度的连通性和冗余性，使用条形码和持续图量化稳定性和一致性。

Result: 推理链的拓扑结构复杂度与准确性呈正相关；更复杂的链能更早识别正确答案；成功的推理表现出更简单的拓扑结构，减少了冗余和循环，提高了效率和可解释性。

Conclusion: 该工作为推理链质量评估提供了新的结构视角，为未来优化提供了指导，表明拓扑结构分析能有效揭示推理链的语义连贯性、逻辑冗余以及逻辑断裂和间隙。
...
### [46] [Observer, Not Player: Simulating Theory of Mind in LLMs through Game Observation](https://arxiv.org/abs/2512.19210)
*Jerry Wang,Ting Yiu Liu*

Main category: cs.AI

TL;DR: 提出一个交互式框架，通过石头剪刀布游戏评估大语言模型是否具备类似心智的序列推理能力，重点关注策略识别和推理过程。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以评估LLM是否真正“理解”序列行为中的策略和推理过程，需要一种透明、可交互的评估框架来探究LLM的心智推理能力。

Method: 将LLM作为观察者识别游戏策略，构建包含静态和动态策略的基准测试，使用交叉熵、Brier分数和期望值差异三个指标，并整合为统一的Union Loss，配合策略识别率(SIR)进行综合评估。

Result: 开发了一个交互式演示系统，支持实时调整LLM分布、可视化损失变化和检查推理片段，能够系统性地揭示LLM推理的优势和局限性。

Conclusion: 该框架为评估LLM在序列游戏中的心智推理能力提供了实用且可解释的代理方法，有助于深入理解当前LLM的推理机制。
...
### [47] [Can We Test Consciousness Theories on AI? Ablations, Markers, and Robustness](https://arxiv.org/abs/2512.19155)
*Yin Jun Phua*

Main category: cs.AI

TL;DR: 通过构建体现不同意识理论机制的人工智能体并进行精确架构消融实验，发现全局工作空间理论、高阶理论和整合信息理论描述了互补的功能层次而非相互竞争的解释。


<details>
  <summary>Details</summary>
Motivation: 当前意识研究领域存在多个竞争性理论（GWT、IIT、HOT），各自提出不同的神经特征指标，但缺乏统一的验证框架。本研究旨在通过合成神经现象学方法，在人工系统中测试这些理论机制的功能后果。

Method: 采用合成神经现象学方法，构建体现不同意识理论机制的人工智能体，通过精确的架构消融实验（包括自我模型损伤、工作空间损伤、噪声扰动等）测试功能表现，设计了三个对照实验来分离不同理论预测的效果。

Result: 实验1：自我模型损伤消除了元认知校准但保留了一阶任务表现，符合高阶理论预测的盲视类似现象。实验2：工作空间容量对信息访问具有因果必要性，完全损伤导致访问标记的定性崩溃，部分损伤则呈现梯度退化，符合全局工作空间理论的点火框架。实验3：发现广播放大效应——全局工作空间式广播会放大内部噪声导致脆弱性，而B2智能体系列对相同扰动具有鲁棒性。同时发现原始扰动复杂性在信息瓶颈下降低，提示不能简单将IIT相关指标移植到工程系统。

Conclusion: 这些理论描述了意识功能架构的互补层次：全局工作空间理论提供广播容量，高阶理论提供质量控制。研究强调这些智能体本身并无意识，而是作为测试意识理论功能预测的参考实现，为理解意识机制提供了新的工程验证途径。
...
TL;DR: 本文研究如何利用大语言模型自动生成基于规则的合理性检查，以检测文档伪造，并在受限硬件资源上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 文档伪造对法律、经济和政府流程构成威胁，现有合理性检查依赖人工实现，耗时且难以扩展。大语言模型在代码生成方面的进展为自动化生成此类检查提供了新可能，但如何适应未知领域的具体要求仍是挑战。

Method: 使用真实应用场景的结构化数据集，对开源大语言模型（Llama 3.1 8B和OpenCoder 8B）进行不同微调策略的适配，并在受限硬件资源上评估生成的合理性检查对未见伪造模式的检测能力。

Result: 模型能够生成可执行且有效的验证程序，表明大语言模型在需要可理解性的安全敏感场景中，具有作为可扩展工具支持人类决策的潜力。

Conclusion: 通过领域特定代码和数据的微调，大语言模型能够自动化生成规则型合理性检查，为文档伪造检测提供了一种高效且可扩展的解决方案。
...
TL;DR: 提出了DeliveryBench基准测试，用于评估具身智能体在真实约束下的长期规划能力，发现当前VLM智能体与人类存在显著差距且常违反常识约束。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注简单短期任务，缺乏对真实世界决策中复杂约束（如时间、成本、资源限制）的建模，需要更贴近现实的评估环境。

Method: 基于外卖配送场景构建程序化生成的3D城市环境，包含道路网络、建筑、交通方式与资源动态系统，并对比VLM智能体与人类玩家在9个城市中的表现。

Result: VLM智能体表现远逊于人类，存在短视行为且频繁违反基本约束；不同模型展现出明显个性差异（如GPT-5冒险型 vs Claude保守型）。

Conclusion: 当前VLM具身智能体在约束密集的真实环境中仍显脆弱，需提升长期规划与约束感知能力，DeliveryBench为相关研究提供了系统化评估工具。
...
### [50] [Helios: A Foundational Language Model for Smart Energy Knowledge Reasoning and Application](https://arxiv.org/abs/2512.19299)
*Haoyu Jiang,Fanjie Zeng,Boan Qu,Xiaojie Lin,Wei Zhong*

Main category: cs.AI

TL;DR: 本文介绍了Helios——一个专为智能能源领域定制的大语言模型，通过多智能体协作框架构建了知识库、指令微调数据集和RLHF数据集，显著提升了模型在专业领域的性能。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型缺乏智能能源领域的专业知识和对物理约束的认知，无法提供精确的工程对齐推理和生成，阻碍了该领域的发展。

Method: 开发了多智能体协作框架Enersys进行端到端数据集构建，包括知识库EnerBase、指令微调数据集EnerInstruct和RLHF数据集EnerReinforce，并基于这些资源对Helios进行大规模预训练、SFT和RLHF。

Result: Helios在智能能源领域的专业知识掌握、任务执行准确性和与人类偏好对齐方面表现显著提升，并发布了评估基准EnerBench。

Conclusion: Helios模型及相关资源套件有效解决了智能能源领域LLM的专业知识缺乏问题，为该领域的LLM研究提供了全面支持。
...
### [51] [Vibe Reasoning: Eliciting Frontier AI Mathematical Capabilities -- A Case Study on IMO 2025 Problem 6](https://arxiv.org/abs/2512.19287)
*Jiaao Wu,Xian Zhang,Fan Yang,Yinpeng Dong*

Main category: cs.AI

TL;DR: 提出Vibe Reasoning人机协作范式，通过元提示、智能体基础与模型编排，将前沿AI模型的潜在知识转化为解决复杂数学问题的实际能力，并以IMO 2025第6题为例验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 前沿AI模型已具备解决复杂问题所需知识，但缺乏如何、何时应用这些知识的能力。现有自主AI系统在公开报告中未能解决某些难题（如IMO 2025第6题），需要探索如何通过人机协作释放其推理潜力。

Method: 1. 使用通用元提示（meta-prompts）替代具体问题提示；2. 智能体基础（agentic grounding）结合Python代码执行与文件存储记忆；3. 模型编排（model orchestration）整合GPT-5的探索能力与Gemini 3 Pro的证明优势；4. 通过多轮迭代优化工作流程。

Result: 1. 成功解决IMO 2025第6题组合优化问题，得出正确答案（2112）并提供严格数学证明；2. 发现智能体基础与模型编排是关键成功因素；3. 人类提示从具体问题提示演变为可迁移的通用元提示；4. 验证了轻量级人类指导可显著提升前沿模型的数学推理能力。

Conclusion: Vibe Reasoning范式能有效释放前沿AI模型的数学推理潜力，其核心组件（元提示、智能体基础、模型编排）针对性地解决了自主AI的失败模式。该研究为构建高效人机协作系统提供了原则性指导，未来将开发自动化框架并进行更广泛评估。
...
TL;DR: 提出SafeMed-R1混合防御框架，通过对抗训练与推理时随机平滑，显著提升医学视觉问答模型对抗攻击的鲁棒性，同时保持高质量临床推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在医学视觉问答中易受对抗攻击影响，标准对抗训练会降低泛化性能和临床推理质量，需开发兼顾鲁棒性与推理可解释性的防御方法。

Method: 采用两阶段方法：训练时结合对抗训练与组相对策略优化（AT-GRPO）强化推理过程；推理时使用随机平滑提供认证的L2范数鲁棒性保证。

Result: 在OmniMedVQA基准测试中，标准微调模型受PGD攻击时准确率从95%降至25%，而SafeMed-R1在相同攻击下保持84.45%准确率，鲁棒性提升59个百分点。

Conclusion: 显式思维链推理训练能提升对抗鲁棒性，表明医学AI系统的可解释性与安全性存在协同效应，SafeMed-R1为临床部署提供了可靠防御方案。
...
TL;DR: 提出VIGOR+框架，通过迭代反馈机制结合LLM生成与CEVAE验证，解决观察性因果推断中隐藏混杂变量的识别问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法利用大语言模型生成隐藏混杂变量时，常出现语义合理但统计无效的问题，缺乏生成与验证的闭环机制。

Method: 建立迭代反馈循环：将CEVAE的验证信号（信息增益、潜在一致性指标等）转化为自然语言反馈，指导LLM多轮生成，直至收敛。

Result: 形式化了反馈机制，证明了在温和假设下的收敛性，并提供了完整的算法框架。

Conclusion: VIGOR+通过生成与验证的闭环迭代，提升了隐藏混杂变量识别的统计效用，弥补了现有方法的不足。
...
TL;DR: 该研究引入了一个名为PENDULUM的评估基准，用于系统评估多模态大语言模型（MLLMs）中的谄媚行为（即模型过度迎合用户输入而忽视事实准确性或视觉证据），发现现有模型普遍存在显著的谄媚和幻觉倾向。


<details>
  <summary>Details</summary>
Motivation: 谄媚行为在纯文本大语言模型中已有研究，但在多模态场景下（尤其是结合视觉信息时）的研究仍非常有限，缺乏系统性的评估基准和分析方法，这影响了MLLMs的事实一致性和可靠性。

Method: 构建了包含约2000个人工标注的视觉问答对的数据集PENDULUM，涵盖六个不同复杂度的图像领域；提出了量化视觉推理中谄媚行为的新指标；对多个先进MLLMs进行了广泛评估。

Result: 现有MLLMs在谄媚行为上表现出显著差异性和脆弱性，其倾向受图像类型和任务复杂度影响；新提出的指标能有效揭示不同多模态上下文中的谄媚表现。

Conclusion: 研究强调了开发抗谄媚的模型架构和训练策略的紧迫性，以提升未来MLLMs的事实一致性和可靠性；公开了数据集和模型响应以供进一步研究。
...
TL;DR: 该研究探索了在目标条件强化学习和广义规划中使用一阶关系语言，通过改进Hindsight Experience Replay（HER）技术，利用原子集表示状态和目标，自动创建难度递增的目标课程，从而在稀疏奖励的大型规划实例中成功学习通用策略。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决目标条件强化学习中，当训练实例规模大且目标无法通过随机探索达成时，如何高效学习跨状态和目标的通用策略。传统HER方法虽能重新标记失败轨迹，但在表示和泛化方面仍有提升空间。

Method: 采用一阶关系语言表示状态和目标，提出三种目标表示形式：完整状态、原始目标的子集，以及这些子目标的提升版本。通过改进HER技术，自动构建从易到难的目标课程，促进策略在稀疏奖励环境中的学习。

Result: 实验表明，后两种目标表示方法（子集和提升子目标）能够成功在大型规划实例中学习通用策略，通过目标课程实现数据和时间效率的提升。研究同时揭示了这些方法的局限性及改进机会。

Conclusion: 使用原子集表示状态和目标，并结合改进的HER技术，能够有效解决目标条件强化学习中的探索和泛化问题，为稀疏奖励环境下的策略学习提供高效框架。未来工作可针对当前方法的局限性进行优化。
...
### [56] [EchoTrail-GUI: Building Actionable Memory for GUI Agents via Critic-Guided Self-Exploration](https://arxiv.org/abs/2512.19396)
*Runze Li,Yuwen Zhai,Bo Xu,LiWu Xu,Nian Shi,Wei Zhang,Ran Lin,Liang Wang*

Main category: cs.AI

TL;DR: 本文提出了EchoTrail-GUI框架，通过为GUI智能体配备动态记忆系统，使其能够从过去的成功经验中学习，从而提升任务执行的成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 当前基于大视觉语言模型的GUI智能体存在“数字健忘症”，即每个任务独立处理，无法从过往成功经验中系统学习，导致性能次优、错误重复且泛化能力差。

Method: 提出三阶段框架：1) 经验探索：智能体自主与GUI环境交互，构建经过奖励模型验证的成功任务轨迹数据库；2) 记忆注入：针对新任务，高效检索最相关的过往轨迹作为“记忆”；3) GUI任务推理：将记忆作为上下文指导注入智能体的推理决策过程。

Result: 在Android World和AndroidLab等基准测试中，EchoTrail-GUI显著提高了基线智能体的任务成功率和操作效率。

Conclusion: 结构化记忆能够创建更鲁棒、更智能的GUI自动化系统，验证了经验式学习在GUI智能体中的有效性。
...
### [57] [Learning General Policies with Policy Gradient Methods](https://arxiv.org/abs/2512.19366)
*Simon Ståhlberg,Blai Bonet,Hector Geffner*

Main category: cs.AI

TL;DR: 本文研究了深度强化学习（DRL）方法学习泛化策略的能力，通过结合组合规划方法和图神经网络（GNN）表示，实现了在规划领域中学习可泛化的策略，性能接近组合方法，同时避免了可扩展性瓶颈。


<details>
  <summary>Details</summary>
Motivation: 强化学习方法在泛化能力方面存在挑战，而组合规划方法能够学习可证明正确的泛化策略。本研究旨在结合这两种研究思路，探索深度强化学习方法（特别是策略优化方法）在何种条件下能够学习到类似组合方法的泛化策略。

Method: 1. 将策略建模为状态转移分类器（而非基于具体动作），以适应不同实例间的变化；2. 使用适应关系结构的图神经网络（GNN）表示规划状态的价值函数和策略；3. 采用演员-评论家方法进行策略学习；4. 通过添加派生谓词和替代成本结构来增强表达能力和优化泛化与最优性的权衡。

Result: 演员-评论家方法能够学习到泛化能力接近组合方法的策略，同时避免了组合方法的可扩展性瓶颈和特征池的使用。DRL方法的局限性主要源于GNN的表达能力限制以及泛化与最优性之间的权衡（在某些领域中泛化策略无法达到最优），而非深度学习或强化学习算法本身的问题。

Conclusion: 通过结合组合规划的思想和深度强化学习技术（特别是GNN表示），可以在规划领域中学习到具有强泛化能力的策略。所提出的方法在保持DRL方法优势的同时，通过增强表示和处理泛化-最优性权衡，显著提升了泛化性能。
...
TL;DR: 提出了一种面向第一性原理材料计算的领域专用智能体，通过嵌入领域知识确保物理一致的多步骤工作流程和参数选择，显著提升了计算任务的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在加速科学发现方面展现出潜力，但其静态知识和幻觉问题阻碍了其在自主研究中的应用。现有智能体框架虽能整合检索、推理和工具使用，但缺乏针对特定科学领域的可靠自动化能力。

Method: 设计了一个领域专用智能体，通过嵌入材料计算领域的专业知识，确保物理一致的多步骤工作流程，并自动选择收敛且适定的计算参数，实现端到端的可靠计算执行。

Result: 在多样化的计算任务基准测试中，该系统在准确性和鲁棒性方面显著优于独立的大型语言模型。

Conclusion: 这项工作为自主计算实验建立了可验证的基础，是迈向完全自动化科学发现的关键一步。
...
### [59] [Towards Closed-Loop Embodied Empathy Evolution: Probing LLM-Centric Lifelong Empathic Motion Generation in Unseen Scenarios](https://arxiv.org/abs/2512.19551)
*Jiawen Wang,Jingjing Wang Tianyang Chen,Min Zhang,Guodong Zhou*

Main category: cs.AI

TL;DR: 提出L^2-EMG任务，通过ES-MoE方法解决情感解耦和场景适应挑战，在多场景情感运动生成中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有情感运动生成方法局限于单一固定规模数据集，无法适应灵活多变的新兴场景（如运动、舞蹈），限制了模型在真实世界的泛化能力。

Method: 提出ES-MoE方法，包含因果引导的情感解耦模块和场景自适应专家构建模块，并构建多个L^2-EMG数据集进行验证。

Result: 广泛实验表明ES-MoE在多个数据集上优于先进基线方法。

Conclusion: L^2-EMG任务有助于构建具备共情与智能的闭环自进化具身智能体，ES-MoE方法有效解决了跨场景情感运动生成的挑战。
...
### [60] [QuantiPhy: A Quantitative Benchmark Evaluating Physical Reasoning Abilities of Vision-Language Models](https://arxiv.org/abs/2512.19526)
*Li Puyin,Tiange Xiang,Ella Mao,Shirley Wei,Xinye Chen,Adnan Masood,Li Fei-fei,Ehsan Adeli*

Main category: cs.AI

TL;DR: 提出了首个定量评估视觉语言模型物理推理能力的基准QuantiPhy，包含3300多个视频-文本实例，测试模型对物体尺寸、速度、加速度的数值估计能力。实验发现当前先进模型在数值准确性上与定性合理性存在明显差距，且过度依赖预训练知识而非输入信息。


<details>
  <summary>Details</summary>
Motivation: 当前视觉感知模型（如大型VLMs）能否从视频观察中定量推理运动物体的运动学属性尚不明确，现有评估多为基于VQA的定性测试，缺乏对数值推理能力的系统衡量。

Method: 构建包含数值真值的视频-文本数据集QuantiPhy，通过标准化提示和评分机制评估模型对物体尺寸、速度、加速度的数值估计准确性，并分析背景噪声、反事实先验、策略提示等影响因素。

Result: 实验表明，先进VLMs在物理推理的定性合理性与数值正确性之间存在系统性差距；模型在定量推理运动学属性时过度依赖预训练的世界知识，而非忠实利用提供的视觉和文本输入作为参考。

Conclusion: QuantiPhy为VLMs提供了首个严格、可扩展的测试平台，推动模型超越口头合理性，实现基于数值的物理理解；当前VLMs的定量物理推理能力仍有待提升。
...
TL;DR: 提出混合LRR-TED框架解决可解释AI的可扩展性与稳定性困境，通过自动规则学习与少量人工规则结合，在客户流失预测中实现高精度并减少人工标注工作量。


<details>
  <summary>Details</summary>
Motivation: 现有可解释AI方法面临'可扩展性-稳定性困境'：事后解释方法（如LIME、SHAP）可扩展但不稳定，监督解释框架（如TED）稳定但需要大量人工标注。需要一种平衡两者优势的解决方案。

Method: 提出混合LRR-TED框架，利用'发现的不对称性'：先用自动规则学习器（GLRM）识别广泛的'安全网'（保留模式），再通过帕累托最优方法仅添加4条人工定义的'风险规则'（流失触发因素）补充解释矩阵。

Result: 在客户流失预测中达到94.00%的预测准确率，优于完整的8条人工规则基线，同时将人工标注工作量减少50%。发现自动规则学习擅长识别保留模式但难以捕捉流失触发因素（安娜·卡列尼娜流失原则）。

Conclusion: 该框架通过将专家角色从'规则编写者'转变为'异常处理者'，改变了人在循环AI的范式，在保持解释稳定性的同时显著降低了人工成本，为解决可解释AI的可扩展性-稳定性困境提供了有效方案。
...
TL;DR: 本文提出将临床风险评分计算等复杂任务的基准视为'进行中的活文档'，通过医生参与和智能验证器审计MedCalc-Bench基准，发现原始标签存在显著噪声，修正后训练模型性能提升8.7%。


<details>
  <summary>Details</summary>
Motivation: 当前使用LLM生成特征和规则聚合构建的MedCalc-Bench基准存在风险：将其视为静态标准会固化历史模型错误，尤其在强化学习中作为奖励信号时，这一问题在安全关键领域尤为危险。

Method: 提出系统性医生参与流程，利用高级智能验证器审计和重新标注MedCalc-Bench，通过自动分诊机制将临床医生注意力集中于最有争议的案例；使用修正后的标签通过GRPO方法微调Qwen3-8B模型。

Result: 审计发现原始标签因特征提取错误、计算器逻辑不匹配和临床模糊性而与医学事实存在显著偏差；使用修正标签训练的模型相比原始基线准确率绝对提升8.7%。

Conclusion: 在安全关键领域，严格的基准维护是模型真正对齐的前提条件，应将复杂任务基准视为需要定期重新评估的'活文档'。
...
### [63] [A 96pJ/Frame/Pixel and 61pJ/Event Anti-UAV System with Hybrid Object Tracking Modes](https://arxiv.org/abs/2512.17939)
*Yuncheng Lu,Yucen Shi,Aobo Li,Zehao Li,Junying Li,Bo Wang,Tony Tae-Hyoung Kim*

Main category: cs.CV

TL;DR: 提出了一种集成帧基与事件驱动目标跟踪的节能反无人机系统，通过自适应模式切换和专用硬件优化，在保持高识别精度的同时显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: 小型高速无人机检测对传统视觉系统构成挑战，需要高能效、低延迟的解决方案以实现可靠跟踪与识别。

Method: 系统采用帧基与事件驱动融合架构，使用游程编码重建事件帧，通过自适应模式切换策略，并设计支持轨迹推理与零跳过乘加运算的专用神经处理单元。

Result: 40纳米工艺芯片面积2平方毫米，在0.8V电压下实现每帧每像素96皮焦、每事件61皮焦的能耗，在50-400米距离和5-80像素/秒速度范围内达到98.2%识别准确率，神经计算冗余减少超97%。

Conclusion: 该系统在公开无人机数据集上展示了端到端能效的先进性能，为反无人机应用提供了高精度、低功耗的硬件解决方案。
...
### [64] [NystagmusNet: Explainable Deep Learning for Photosensitivity Risk Prediction](https://arxiv.org/abs/2512.17943)
*Karthik Prabhakar*

Main category: cs.CV

TL;DR: 提出NystagmusNet系统，通过AI预测高风险视觉环境并提供实时视觉适应建议，以帮助眼球震颤伴畏光患者。


<details>
  <summary>Details</summary>
Motivation: 眼球震颤伴畏光患者因环境亮度加剧不自主眼动而面临日常挑战，现有辅助方案缺乏预测性个性化治疗。

Method: 使用双分支卷积神经网络，基于合成与增强数据集训练，结合环境亮度和眼动方差估计畏光风险评分；集成SHAP和GradCAM可解释性技术；采用基于规则的推荐引擎提供自适应滤镜建议。

Result: 模型在合成数据上达到75%的验证准确率，可识别环境风险区域并提升临床信任度。

Conclusion: 系统为患者提供了预测性个性化辅助方案，未来可通过智能眼镜部署和强化学习进一步优化个性化推荐。
...
### [65] [SuperFlow: Training Flow Matching Models with RL on the Fly](https://arxiv.org/abs/2512.17951)
*Kaijie Chen,Zhiyang Xu,Ying Shen,Zihao Lin,Yuguang Yao,Lifu Huang*

Main category: cs.CV

TL;DR: 提出SuperFlow框架，通过方差感知采样调整组大小和计算与连续时间流动力学一致的步级优势，显著提升流模型的强化学习训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 当前流模型的强化学习训练存在两个问题：固定每提示组大小忽略了不同提示间采样重要性的差异，导致采样效率低；轨迹级优势被重用为每步估计，导致信用分配偏差。

Method: 提出SuperFlow框架，采用方差感知采样动态调整组大小，并设计符合连续时间流动力学的步级优势计算方法。

Result: SuperFlow仅需原始训练步数的5.4%至56.3%，减少训练时间5.2%至16.7%；在文本渲染、组合图像生成和人类偏好对齐等任务上，性能优于SD3.5-M（提升4.6%-47.2%）和Flow-GRPO（提升1.7%-16.0%）。

Conclusion: SuperFlow通过改进采样策略和信用分配机制，显著提升了流模型强化学习的效率和性能，为文本到图像生成任务提供了更高效的训练方案。
...
### [66] [E-RGB-D: Real-Time Event-Based Perception with Structured Light](https://arxiv.org/abs/2512.18429)
*Seyed Ehsan Marjani Bajestani,Giovanni Beltrame*

Main category: cs.CV

TL;DR: 提出了一种结合事件相机和DLP投影仪的主动结构光系统，实现高速彩色RGB-D感知，解决了传统事件相机在静态物体检测和色彩信息方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统单色事件相机虽然具有高动态范围、高时间分辨率等优势，但无法检测静态或缓慢移动物体，且缺乏色彩信息。这限制了其在需要颜色感知的应用中的使用。

Method: 将数字光处理（DLP）投影仪与单色事件相机集成，形成主动结构光系统。通过动态投影调整优化带宽，实现选择性色彩数据采集，同时保持空间分辨率。

Result: 系统实现了相当于1400 fps的色彩检测速度和4 kHz的像素深度检测，能够生成彩色点云，显著提升了计算机视觉在机器人、3D重建等领域的性能。

Conclusion: 该方法成功整合了事件相机和投影技术的优势，实现了高速、高分辨率的彩色RGB-D感知，为计算机视觉应用开辟了新方向。
...
### [67] [Seeing Beyond the Scene: Analyzing and Mitigating Background Bias in Action Recognition](https://arxiv.org/abs/2512.17953)
*Ellie Zhou,Jihoon Chung,Olga Russakovsky*

Main category: cs.CV

TL;DR: 本文系统分析了动作识别模型中的背景偏见问题，发现分类模型、对比学习模型和视频大语言模型都存在依赖背景而非人体动作进行预测的倾向，并提出了基于人体分割和提示调优的缓解方法。


<details>
  <summary>Details</summary>
Motivation: 当前动作识别模型普遍存在背景偏见问题，即模型倾向于根据背景信息而非人体动作和姿态进行预测，这影响了模型的鲁棒性和泛化能力。

Method: 1. 系统评估了分类模型、对比学习文本-图像预训练模型和视频大语言模型的背景偏见程度；2. 针对分类模型提出使用分割后的人体输入来减少背景依赖；3. 针对VLLM探索了手动和自动提示调优方法。

Result: 1. 所有被测试模型都表现出强烈的背景推理倾向；2. 使用分割人体输入使分类模型的背景偏见降低了3.78%；3. 通过提示设计可使VLLM更关注人体动作推理，性能提升9.85%。

Conclusion: 背景偏见是动作识别领域的普遍问题，但通过输入预处理（人体分割）和提示工程可以有效缓解这一问题，使模型更加关注人体动作本身而非背景线索。
...
### [68] [SCS-SupCon: Sigmoid-based Common and Style Supervised Contrastive Learning with Adaptive Decision Boundaries](https://arxiv.org/abs/2512.17954)
*Bin Wang,Fadi Dornaika*

Main category: cs.CV

TL;DR: 提出SCS-SupCon框架，通过可学习的sigmoid对比损失和风格距离约束，解决细粒度图像分类中类间差异小、类内变化大的问题，在多个基准数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法在细粒度图像分类中面临类间差异小、类内变化大的挑战，监督对比方法存在负样本稀释和缺乏自适应决策边界的问题，导致判别能力不足。

Method: 提出基于sigmoid的成对对比损失，引入可学习的温度和偏置参数实现自适应决策边界；增加显式的风格距离约束，解耦风格与内容表示；在六个基准数据集上进行实验验证。

Result: 在CIFAR-100上，SCS-SupCon比SupCon提升约3.9个百分点，比CS-SupCon提升约1.7个百分点；在细粒度数据集上比CS-SupCon提升0.4-3.0个百分点；Friedman检验和Nemenyi事后评估验证了改进的稳定性。

Conclusion: SCS-SupCon通过自适应决策边界和风格-内容解耦，有效缓解负样本稀释问题，在CNN和Transformer骨干网络上均实现了最先进的细粒度图像分类性能。
...
### [69] [Neural Implicit Heart Coordinates: 3D cardiac shape reconstruction from sparse segmentations](https://arxiv.org/abs/2512.19316)
...
TL;DR: 提出神经隐式心脏坐标（NIHCs），一种基于通用心室坐标的标准化隐式坐标系，用于从稀疏2D分割中重建3D心脏解剖结构，实现高精度、高效率的重建。


<details>
  <summary>Details</summary>
Motivation: 从稀疏临床图像中准确重建心脏解剖结构是患者特异性建模的主要挑战，现有神经隐式函数在跨主体映射解剖一致性方面应用有限。

Method: 引入NIHCs作为标准化隐式坐标系，直接从有限2D分割预测NIHCs，并解码为任意分辨率的密集3D分割和高分辨率网格，使用5,000个心脏网格的大规模数据集进行训练。

Result: 在疾病队列（n=4549）和健康队列（n=5576）中分别实现2.51±0.33 mm和2.3±0.36 mm的平均欧几里得表面误差；即使在严重切片稀疏和分割噪声下也能实现解剖一致重建，推理时间从60秒以上减少到5-15秒。

Conclusion: NIHCs构成了一种稳健高效的解剖表示方法，能够从最小输入数据中实现患者特异性3D心脏重建。
...
### [70] [A Modular Framework for Single-View 3D Reconstruction of Indoor Environments](https://arxiv.org/abs/2512.17955)
*Yuxiao Li*

Main category: cs.CV

TL;DR: 提出一种基于扩散技术的模块化单视角室内场景三维重建框架，通过分步处理解决复杂遮挡和形状重建问题，在3D-Front数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统单视角室内重建方法难以处理复杂实例形状和遮挡问题，直接从残缺2D图像预测3D形状导致重建质量受限。

Method: 采用两步法：1) 使用扩散技术预测完整房间背景和遮挡实例；2) 转换为3D。核心模块包括：遮挡实例全视图恢复模块、房间布局修复模型、平衡几何精度与细节的混合深度估计技术、结合2D/3D线索的视角空间对齐方法。

Result: 在3D-Front数据集上的实验表明，该方法在视觉质量和重建精度方面均优于当前最优方法。

Conclusion: 该框架能有效从单张图像重建前景实例和房间背景，在室内设计、房地产和增强现实领域具有应用潜力。
...
### [71] [Enhancing Tea Leaf Disease Recognition with Attention Mechanisms and Grad-CAM Visualization](https://arxiv.org/abs/2512.17987)
*Omar Faruq Shikdar,Fahad Ahammed,B. M. Shahria Alam,Golam Kibria,Tawhidur Rahman,Nishat Tasnim Niloy*

Main category: cs.CV

TL;DR: 本研究开发了一个基于深度学习的茶叶病害自动分类系统，通过集成多种预训练模型和注意力机制，在自建数据集上实现了85.68%的最高准确率，并引入可解释AI提高模型透明度。


<details>
  <summary>Details</summary>
Motivation: 茶叶是全球广泛消费的饮品，茶叶生产是许多国家的关键产业。茶叶病害若不及时控制会导致重大经济损失，而人工识别效率低且不可靠，因此需要自动化系统来快速准确识别茶叶病害，帮助农民及时采取防治措施。

Method: 创建了包含5278张图像、7个类别的新数据集；对数据进行预处理；部署了DenseNet、Inception和EfficientNet三种预训练模型（EfficientNet仅用于集成模型）；使用两种注意力模块提升性能；采用集成学习方法；引入可解释AI技术增强模型可解释性。

Result: 集成模型取得了最佳性能，准确率达到85.68%，证明了所提方法的有效性。

Conclusion: 开发的自动化系统能够有效分类茶叶病害，为农民提供及时预警，有助于减少经济损失。集成模型结合注意力机制和可解释AI的方法在茶叶病害识别任务中表现出良好性能。
...
### [72] [Name That Part: 3D Part Segmentation and Naming](https://arxiv.org/abs/2512.18003)
*Soumava Paul,Prakhar Kaushik,Ankit Vaidya,Anand Bhattad,Alan Yuille*

Main category: cs.CV

TL;DR: 提出ALIGN-Parts方法，通过集合对齐实现语义3D部件分割与命名，统一多个数据集并支持零样本匹配


<details>
  <summary>Details</summary>
Motivation: 现有部件标注数据集定义不一致，且先前方法无法同时实现部件分解与语义命名，限制了鲁棒训练与应用

Method: 将部件命名建模为集合对齐任务，使用部件场几何特征、多视角视觉特征和语言模型生成的affordance描述，通过二分图分配匹配部件与文本描述

Result: 创建了包含1794个唯一3D部件的统一本体，对齐了PartNet等数据集，支持零样本匹配和置信度校准预测，并提出了两个新评估指标

Conclusion: ALIGN-Parts实现了高效的单次3D部件分割与命名，可作为可扩展的标注工具，并在多个下游任务中展示应用潜力
...
### [73] [Seeing Justice Clearly: Handwritten Legal Document Translation with OCR and Vision-Language Models](https://arxiv.org/abs/2512.18004)
*Shubham Kumar Nigam,Parjanya Aditya Shukla,Noel Shallum,Arnab Bhattacharya*

Main category: cs.CV

TL;DR: 比较了传统OCR-MT流程与视觉大语言模型在手写马拉地语法律文件翻译中的表现，探索端到端翻译方案在低资源语言场景下的可行性。


<details>
  <summary>Details</summary>
Motivation: 印度地方法院和高等法院急需可扩展、准确的翻译系统，以数字化FIR、起诉书和证人陈述等手写法律记录，解决马拉地语等低资源语言因缺乏大规模数字化语料库和手写风格多变带来的翻译难题。

Method: 使用传统两阶段流程（OCR提取文本后机器翻译）与视觉大语言模型端到端直接翻译两种方法，在精心整理的手写马拉地语法律文件数据集上进行性能对比评估。

Result: 研究结果为构建可在边缘部署的鲁棒解决方案提供了可行见解，有助于提升非母语人士和法律专业人士对法律信息的访问效率。

Conclusion: 端到端视觉大语言模型为低资源环境下的法律文件处理提供了有潜力的替代方案，能够促进法律文档的高效处理和信息普及。
...
### [74] [NodMAISI: Nodule-Oriented Medical AI for Synthetic Imaging](https://arxiv.org/abs/2512.18038)
*Fakrul Islam Tushar,Ehsan Samei,Cynthia Rudin,Joseph Y. Lo*

Main category: cs.CV

TL;DR: 提出了NodMAISI框架，用于生成解剖结构约束的肺结节CT图像，通过标准化数据标注和病变感知增强，显著提升了小肺结节的检测和分类性能。


<details>
  <summary>Details</summary>
Motivation: 医学影像数据集中，肺癌筛查关键的小肺结节等异常发现代表性不足且标注不一致，限制了深度学习模型的训练效果。

Method: 开发了NodMAISI框架，包括：1）标准化数据标注流程；2）基于ControlNet条件化整流流的生成器；3）病变感知增强技术，通过扰动结节掩模生成配对CT变体。

Result: 在六个公开测试集上，NodMAISI相比MAISI-v2提升了分布保真度，显著提高了结节检测灵敏度（特别是亚厘米结节），并在数据稀缺情况下将结节恶性分类AUC提升0.07-0.21。

Conclusion: NodMAISI通过解剖约束的合成与增强，有效缓解了医学影像数据中关键病变样本不足的问题，提升了下游任务的性能，尤其在数据稀缺场景下表现突出。
...
### [75] [YolovN-CBi: A Lightweight and Efficient Architecture for Real-Time Detection of Small UAVs](https://arxiv.org/abs/2512.18046)
*Ami Pandat,Punna Rajasekhar,Gopika Vinod,Rohit Shukla*

Main category: cs.CV

TL;DR: 提出改进的Yolov5-CBi架构，结合CBAM和BiFPN增强小目标检测能力，并通过知识蒸馏获得轻量化模型，显著提升无人机实时检测的精度与速度。


<details>
  <summary>Details</summary>
Motivation: 无人机在民用和国防领域风险增加，但因其体积小、速度快、视觉对比度低，检测难度大，需要高精度实时检测系统。

Method: 1. 构建Yolov5-CBi架构，集成CBAM注意力模块和BiFPN双向特征金字塔；2. 创建2.8万张图像训练集和2500张本地测试集；3. 提出四种CBi变体并评估；4. 使用知识蒸馏技术（教师模型Yolov5m-CBi，学生模型Yolov5n-CBi）压缩模型。

Result: 1. Yolov5-CBi在速度-精度权衡上优于Yolov8/Yolov12；2. 蒸馏后模型mAP@0.5:0.9达0.6573，较教师模型提升6.51%；3. 蒸馏模型比基线快82.9%，适合实时检测。

Conclusion: CBi架构与蒸馏轻量化模型能有效提升小型无人机实时检测的效率和准确性，为边缘部署提供可行方案。
...
### [76] [FOODER: Real-time Facial Authentication and Expression Recognition](https://arxiv.org/abs/2512.18057)
*Sabri Mustafa Kahya,Muhammet Sami Yavuz,Boran Hamdi Sivrikaya,Eckehard Steinbach*

Main category: cs.CV

TL;DR: FOODER是一个基于雷达的实时隐私保护框架，集成了OOD人脸认证和表情识别，使用FMCW雷达和分层架构实现高精度认证与表情分类。


<details>
  <summary>Details</summary>
Motivation: 为神经网络安全部署提供分布外检测，同时通过雷达数据保护用户隐私，实现非接触式、实时的人脸认证与表情识别。

Method: 使用低成本FMCW雷达获取距离-多普勒和微距离-多普勒表示；认证模块采用多编码器-多解码器架构（含BP和ILED组件）；表情识别模块通过ResNet块区分动态/静态表情，并分别用MobileViT网络分类。

Result: 在60 GHz FMCW雷达数据集上，认证AUROC达94.13%，FPR95为18.12%；表情识别平均准确率94.70%，优于现有OOD检测方法和Transformer架构，且可实时运行。

Conclusion: FOODER通过雷达数据和分层设计，实现了隐私保护下的鲁棒人脸认证与细粒度表情识别，为安全、非侵入式的生物识别应用提供了有效解决方案。
...
### [77] [FPBench: A Comprehensive Benchmark of Multimodal Large Language Models for Fingerprint Analysis](https://arxiv.org/abs/2512.18073)
*Ekta Balkrishna Gavas,Sudipta Banerjee,Chinmay Hegde,Nasir Memon*

Main category: cs.CV

TL;DR: 本文提出了首个用于评估多模态大语言模型在指纹理解领域能力的综合基准测试FPBench，涵盖了20个模型在7个数据集上的8项生物识别与法医任务。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在虹膜和人脸图像分析中已得到应用，但其在指纹理解方面的能力尚未被探索，因此需要建立系统性的评估基准。

Method: 设计了包含7个真实与合成数据集的FPBench基准，采用零样本提示和思维链提示策略，评估了20个开源和专有模型在8项生物识别与法医任务上的表现。

Result: 研究全面评估了各模型在指纹理解任务中的性能、可解释性，并揭示了当前模型在该领域面临的挑战与局限性。

Conclusion: FPBench是首个面向指纹领域的多模态大语言模型综合基准，为指纹基础模型的发展奠定了基础。
...
### [78] [Uncertainty-Gated Region-Level Retrieval for Robust Semantic Segmentation](https://arxiv.org/abs/2512.18082)
...
<details>
  <summary>Details</summary>
Motivation: 户外街景语义分割在自动驾驶、移动机器人等应用中至关重要，需要准确区分道路、人行道、车辆等关键目标，并在不同环境、光照和噪声条件下保持实时鲁棒性。

Method: 采用区域级不确定性门控检索机制，仅对高不确定性区域进行检索，而非全图检索，以减少计算开销。

Result: 最佳方法在平均交并比上提升11.3%，检索成本降低87.5%，仅对12.5%的区域进行检索（基线为100%）。

Conclusion: 所提方法能有效提升语义分割在域偏移下的性能与校准，同时显著降低计算负担，适用于对实时性要求高的应用场景。
...
### [79] [SERA-H: Beyond Native Sentinel Spatial Limits for High-Resolution Canopy Height Mapping](https://arxiv.org/abs/2512.18128)
...
TL;DR: 提出SERA-H模型，结合超分辨率和时序注意力，利用免费Sentinel卫星数据生成高分辨率森林冠层高度图，性能媲美商业高分辨率影像。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在利用卫星影像预测高度图时，面临数据可获取性与空间分辨率之间的权衡问题，需要一种既能利用免费数据又能生成高分辨率结果的方法。

Method: 采用端到端模型SERA-H，结合超分辨率模块（EDSR）和时序注意力编码（UTAE），在激光雷达数据监督下，使用Sentinel-1和Sentinel-2时序数据生成2.5米分辨率高度图。

Result: 在法国开源基准数据集上评估，平均绝对误差2.6米，决定系数0.82，性能优于标准Sentinel基线，且达到或超过依赖商业高分辨率影像的方法。

Conclusion: 结合高分辨率监督与时序数据的时空信息，能够重建超出输入传感器原始分辨率的细节，为免费、高频次森林制图提供了可能，精度可与昂贵商业影像媲美。
...
### [80] [EndoStreamDepth: Temporally Consistent Monocular Depth Estimation for Endoscopic Video Streams](https://arxiv.org/abs/2512.18159)
*Hao Li,Daiwei Lu,Jiacheng Wang,Robert J. Webster,Ipek Oguz*

Main category: cs.CV

TL;DR: 提出EndoStreamDepth框架，用于内窥镜视频流的单目深度估计，实现高精度、边界清晰、时间一致且实时的深度图预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法多采用批量输入处理，难以满足内窥镜视频流对实时性、时间一致性和解剖边界清晰度的要求，限制了其在机器人手术等下游任务中的应用。

Method: 框架包含三个核心组件：(1) 单帧深度网络结合内窥镜特定变换；(2) 多级Mamba时序模块利用帧间信息提升精度和稳定性；(3) 分层设计结合多尺度监督，通过互补损失函数优化局部边界锐度和全局几何一致性。

Result: 在两个公开结肠镜深度估计数据集上的评估表明，EndoStreamDepth性能显著优于现有最优方法，生成的深度图具有清晰解剖边界且时间一致，满足实时处理需求。

Conclusion: EndoStreamDepth为内窥镜视频流提供了高效可靠的深度估计解决方案，其锐利的解剖边界和时序一致性有助于支持机器人手术自动化等下游任务。代码已开源。
...
### [81] [Local Patches Meet Global Context: Scalable 3D Diffusion Priors for Computed Tomography Reconstruction](https://arxiv.org/abs/2512.18161)
*Taewon Yang,Jason Hu,Jeffrey A. Fessler,Liyue Shen*

Main category: cs.CV

TL;DR: 提出一种基于3D块的分割扩散模型，可从有限数据学习3D扩散先验，实现高分辨率3D图像的高效生成与重建


<details>
  <summary>Details</summary>
Motivation: 现有方法多复用2D扩散先验处理3D逆问题（如CT重建），但无法充分利用扩散模型对高维数据的生成能力；直接在3D数据上训练扩散模型面临计算资源消耗大、需要大规模数据集的挑战

Method: 提出3D块分割扩散模型：1）学习3D块的先验以实现可扩展效率；2）通过建模位置感知的3D局部块与下采样3D体数据（作为全局上下文）的联合分布，耦合局部与全局信息；3）支持从有限数据学习完全3D扩散先验

Result: 在多个数据集的3D CT重建实验中，方法在性能与效率上均优于现有先进方法，实现了512×512×256高分辨率3D重建（约20分钟）

Conclusion: 该方法不仅能实现高质量3D生成，还为高分辨率3D逆问题提供了前所未有的高效准确解决方案，显著提升了3D医学图像重建的实用性与可扩展性
...
### [82] [Atlas is Your Perfect Context: One-Shot Customization for Generalizable Foundational Medical Image Segmentation](https://arxiv.org/abs/2512.18176)
*Ziyu Zhang,Yi Yu,Simeng Zhu,Ahmed Aly,Yunhe Gao,Ning Gu,Yuan Xue*

Main category: cs.CV

TL;DR: 提出AtlasSegFM框架，通过单标注示例引导基础模型适应临床场景，结合图谱配准提供上下文感知提示，提升医学图像分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有交互式基础模型依赖精确提示且在训练数据不足的场景中表现不佳，需要轻量级方案实现临床环境下的快速定制化部署。

Method: 1) 通过上下文图谱与查询图像的配准生成上下文感知提示；2) 设计测试时适配器融合图谱配准与基础模型的预测结果。

Result: 在多模态多器官的公开与内部数据集上验证，AtlasSegFM显著提升分割性能，尤其对小而精细的结构效果明显。

Conclusion: AtlasSegFM为临床工作流提供轻量可部署的单次定制解决方案，代码将开源。
...
### [83] [MACE-Dance: Motion-Appearance Cascaded Experts for Music-Driven Dance Video Generation](https://arxiv.org/abs/2512.18181)
*Kaixing Yang,Jiashu Zhu,Xulong Tang,Ziqiao Peng,Xiangyue Zhang,Puwei Wang,Jiahong Wu,Xiangxiang Chu,Hongyan Liu,Jun He*

Main category: cs.CV

TL;DR: 提出MACE-Dance框架，通过级联专家混合模型实现音乐驱动的舞蹈视频生成，在运动质量和视觉外观上均达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法直接适配音乐驱动舞蹈生成任务，且难以同时实现高质量视觉外观与真实人体运动，需要专门解决方案。

Method: 采用级联专家混合框架：运动专家通过BiMamba-Transformer混合架构的扩散模型生成3D舞蹈动作；外观专家通过解耦运动-美学微调策略进行视频合成。构建大规模数据集并设计运动-外观评估协议。

Result: 运动专家在3D舞蹈生成、外观专家在姿态驱动图像动画任务中均达到SOTA性能，整体框架在综合评估中也取得最优结果。

Conclusion: MACE-Dance框架能有效协同生成高质量舞蹈视频，为音乐驱动舞蹈生成提供了系统解决方案，并通过新数据集和评估协议建立了更全面的基准。
...
### [84] [Is There a Better Source Distribution than Gaussian? Exploring Source Distributions for Image Flow Matching](https://arxiv.org/abs/2512.18184)
*Junho Lee,Kwanseok Kim,Joonseok Lee*

Main category: cs.CV

TL;DR: 本文通过2D模拟分析流匹配学习动态，发现高斯分布的优势在于全方位覆盖，并提出结合范数对齐训练和方向剪枝采样的实用框架，无需重新训练即可提升现有模型性能。


<details>
  <summary>Details</summary>
Motivation: 流匹配生成模型中通常使用高斯分布作为源分布，但高维数据生成中可能存在更优选择，且相关研究不足。本文旨在探索源分布设计对学习动态和生成性能的影响。

Method: 提出2D模拟方法以可视化高维几何特性；基于分析提出范数对齐训练与方向剪枝采样结合的框架；剪枝策略可直接应用于已训练的高斯源流匹配模型。

Result: 实证评估表明，该方法在生成质量和采样效率上均取得一致提升；剪枝策略无需重新训练即可实现即时性能增益。

Conclusion: 研究为源分布设计提供了实用见解和指导，并提出了可直接改进现有流匹配模型的技术，强调了高斯分布全方位覆盖的鲁棒性价值。
...
### [85] [ALIGN: Advanced Query Initialization with LiDAR-Image Guidance for Occlusion-Robust 3D Object Detection](https://arxiv.org/abs/2512.18187)
*Janghyun Baek,Mincheol Chang,Seokha Moon,Seung Joon Lee,Jinkyu Kim*

Main category: cs.CV

TL;DR: 提出ALIGN方法，通过激光雷达和图像引导的查询初始化策略，提升遮挡和拥挤场景下的3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于查询的3D目标检测方法在查询初始化时存在效率低和精度不足的问题，尤其在物体被遮挡或场景拥挤时表现不佳。

Method: 提出三个核心组件：遮挡感知中心估计（OCE）融合激光雷达几何与图像语义；自适应邻域采样（ANS）生成并补充物体候选；动态查询平衡（DQB）调整前景与背景区域的查询分配。

Result: 在nuScenes基准测试中，ALIGN显著提升了多种先进检测器的性能，最高提升+0.9 mAP和+1.2 NDS，在遮挡和密集场景中效果尤为明显。

Conclusion: ALIGN通过更高效的查询初始化策略，有效解决了现有方法在复杂场景下的局限性，提升了3D目标检测的鲁棒性和准确性。
...
### [86] [Multi-Part Object Representations via Graph Structures and Co-Part Discovery](https://arxiv.org/abs/2512.18192)
*Alex Foo,Wynne Hsu,Mong Li Lee*

Main category: cs.CV

TL;DR: 提出一种利用显式图表示多部件对象的新方法，通过部件共现发现算法，在遮挡和分布外场景中显著提升对象中心表示的质量和识别能力。


<details>
  <summary>Details</summary>
Motivation: 现有隐式对象表示方法在遮挡或分布外场景中难以识别多部件对象，因为其通过间接训练目标隐式编码部件-整体关系，导致鲁棒性不足。

Method: 提出基于显式图表示的部件共现对象发现算法，并构建三个基准测试评估对象中心方法在遮挡和分布外场景中对多部件对象的识别鲁棒性。

Result: 在模拟、真实和现实世界图像上的实验表明，该方法在发现对象质量上显著优于现有方法，能准确识别遮挡和分布外场景中的多部件对象，且在下游任务中更精准预测对象关键属性。

Conclusion: 显式图表示方法能有效提升对象中心表示的鲁棒性和泛化能力，为推进对象中心表示领域的发展提供了潜力。
...
### [87] [Unsupervised Anomaly Detection with an Enhanced Teacher for Student-Teacher Feature Pyramid Matching](https://arxiv.org/abs/2512.18219)
*Mohammad Zolfaghari,Hedieh Sajedi*

Main category: cs.CV

TL;DR: 提出ET-STPM模型，通过增强教师网络的学生-教师框架在无监督异常检测中取得优异性能


<details>
  <summary>Details</summary>
Motivation: 无监督学习中的异常检测具有挑战性，现有方法性能有待提升

Method: 使用ResNet-18在ImageNet上预训练，在MVTech-AD数据集上微调，构建增强教师网络的学生-教师特征金字塔框架

Result: 图像级异常检测平均准确率0.971，像素级0.977，优于先前方法

Conclusion: ET-STPM框架通过增强教师网络显著提升了异常检测性能
...
### [88] [Multifaceted Exploration of Spatial Openness in Rental Housing: A Big Data Analysis in Tokyo's 23 Wards](https://arxiv.org/abs/2512.18226)
*Takuya OKi,Yuan Liu*

Main category: cs.CV

TL;DR: 本研究开发了一个从二维和三维角度评估住宅空间开放度的定量框架，并应用于东京23区的4004个出租单元，分析了开放度的时空变化及其与租金和住宅属性的关系。


<details>
  <summary>Details</summary>
Motivation: 空间开放度对提升居住质量和设计至关重要，但现有研究往往将其影响因素分开处理，缺乏多维度的量化评估框架。

Method: 使用可见性图分析（VGA）从平面图计算二维开放度（平面可见性）；利用语义分割模型Mask2Former分析室内图像，识别墙壁、天花板、地板和窗户，从而推导三维开放度。

Result: 研究发现客厅可见性增加，整体开放度在1990年代达到峰值；开放度与租金及建筑特征存在部分相关性，反映了城市再开发趋势；二维与三维开放度指标无直接关联，但更高的开放度倾向于对应更高的租金；现有模型预测的印象分数与开放度仅弱相关，表明室内设计和家具对空间感知的影响更强。

Conclusion: 本研究提供了一个新的多维数据驱动框架，用于量化住宅空间开放度并将其与城市及市场动态联系起来，为住宅设计和质量评估提供了新工具。
...
### [89] [Investigating Spatial Attention Bias in Vision-Language Models](https://arxiv.org/abs/2512.18231)
*Aryan Chaudhary,Sanchit Goyal,Pratik Narang,Dhruv Kumar*

Main category: cs.CV

TL;DR: 研究发现视觉语言模型存在系统性的空间注意力偏见，在水平拼接图像中会优先描述左侧内容，这一偏见在不同架构模型中普遍存在且与语言阅读方向无关。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在理解视觉内容方面表现出色，但其空间处理中的系统性偏见尚未被充分探索，特别是模型在处理空间信息时可能存在的固有偏差。

Method: 通过控制实验，使用开源和闭源模型测试水平拼接图像对的描述顺序；测试阿拉伯语微调模型以排除语言阅读方向影响；分析训练数据集标注指南（如PixMo和Visual Genome）。

Result: 在中性提示条件下，约97%的情况下模型优先描述左侧内容；阿拉伯语微调模型仍保持该偏见，表明与语言阅读方向无关；训练数据标注指南中未发现明确的左优先指令。

Conclusion: 该空间注意力偏见是当前视觉语言模型的固有架构特性，揭示了其在空间信息处理方面的根本局限性，且并非由训练数据中的显式指令导致。
...
### [90] [Joint Learning of Depth, Pose, and Local Radiance Field for Large Scale Monocular 3D Reconstruction](https://arxiv.org/abs/2512.18237)
*Shahram Najam Syed,Yitian Hu,Yuchao Yao*

Main category: cs.CV

TL;DR: 提出一种联合学习框架，通过耦合深度、姿态和辐射场三个因素，实现大规模场景的单目视频逼真三维重建，解决了传统方法中的尺度模糊、姿态漂移和场景规模限制问题。


<details>
  <summary>Details</summary>
Motivation: 传统单目视频三维重建方法在大型场景中面临三大挑战：尺度模糊的深度估计导致几何失真，长轨迹姿态漂移破坏对齐，单一全局神经辐射场无法建模数百米范围的内容。

Method: 1) 使用具有公制尺度监督的ViT深度网络获取全局一致深度；2) 多尺度特征束调整层在特征空间直接优化相机姿态，利用金字塔描述符抑制漂移；3) 增量式局部辐射场层次结构，动态分配和冻结哈希网格NeRF以支持城市街区尺度重建。

Result: 在Tanks and Temples基准测试中，绝对轨迹误差降至0.001-0.021米（比BARF降低18倍，比NoPe-NeRF降低2倍），同时保持亚像素级相对姿态误差，实现了单GPU上的城市街区尺度覆盖。

Conclusion: 该方法证明通过单目未标定RGB相机即可实现公制尺度、无漂移的三维重建和高保真新视角合成，为大规模场景重建提供了可行解决方案。
...
### [91] [SG-RIFE: Semantic-Guided Real-Time Intermediate Flow Estimation with Diffusion-Competitive Perceptual Quality](https://arxiv.org/abs/2512.18241)
*Pan Ben Wong,Chengli Wu,Hanyue Lu*

Main category: cs.CV

TL;DR: 提出SG-RIFE方法，通过语义引导增强预训练的RIFE模型，在保持实时性的同时提升复杂场景下的视频插帧质量。


<details>
  <summary>Details</summary>
Motivation: 现有流式方法（如RIFE）实时性好但复杂场景效果差，扩散模型质量高但延迟大，需要平衡速度与质量。

Method: 1. 基于预训练RIFE进行参数高效微调；2. 使用冻结DINOv3提取语义先验；3. 设计Split-FAPM模块压缩特征；4. 设计DSF模块对齐语义与运动场。

Result: 在SNU-FILM上，SG-RIFE在FID/LPIPS指标上优于LDMVFI，质量接近Consec. BB，且运行速度显著更快。

Conclusion: 语义一致性可使流式方法以接近实时的速度达到与扩散模型竞争的质量水平。
...
### [92] [Spectral Discrepancy and Cross-modal Semantic Consistency Learning for Object Detection in Hyperspectral Image](https://arxiv.org/abs/2512.18245)
*Xiao He,Chang Tang,Xinwang Liu,Wei Zhang,Zhimin Gao,Chuankun Li,Shaohua Qiu,Jiangfeng Xu*

Main category: cs.CV

TL;DR: 提出SDCM网络解决高光谱图像中类内和类间相似性导致的检测挑战，通过光谱差异和跨模态语义一致性学习提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像虽能识别相似物质的细微差异，但存在波段间空间差异、传感器噪声和光照干扰等问题，导致类内/类间相似性高，检测困难。

Method: 提出SDCM网络，包含语义一致性学习模块（SCL）减少波段间信息异质性，光谱门控生成器（SGG）过滤冗余数据，光谱差异感知模块（SDA）提取像素级光谱特征。

Result: 在两个高光谱数据集上的实验表明，该方法相比其他方法取得了最先进的检测性能。

Conclusion: SDCM能有效缓解高光谱波段不一致性和冗余问题，通过跨波段一致性学习和光谱特征提取显著提升目标检测效果。
...
### [93] [Towards Ancient Plant Seed Classification: A Benchmark Dataset and Baseline Model](https://arxiv.org/abs/2512.18247)
*Rui Xing,Runmin Cong,Yingying Wu,Can Wang,Zhongming Tang,Fen Wang,Hao Wu,Sam Kwong*

Main category: cs.CV

TL;DR: 本文构建了首个古代植物种子图像分类数据集（APS），并提出专门用于古代种子分类任务的APSNet框架，通过引入种子尺寸特征和异步解耦解码架构，在细粒度分类任务中达到90.5%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统考古植物学研究依赖专家知识，耗时低效；智能分析方法在考古学其他领域已有进展，但在古代植物种子分类任务中存在数据和方法的研究空白。

Method: 1. 构建包含8,340张图像、17个种子类别的APS数据集；2. 设计APSNet框架，包含尺寸感知嵌入模块（SPE）提取种子尺寸信息，以及基于渐进学习的异步解耦解码架构（ADD）从通道和空间角度解码特征。

Result: 该方法在定量和定性分析中均超越现有最优图像分类方法，分类准确率达到90.5%。

Conclusion: 该研究为大规模、系统化的考古研究提供了有效工具，证明了结合尺寸特征与细粒度信息学习在古代种子分类任务中的有效性。
...
### [94] [Loom: Diffusion-Transformer for Interleaved Generation](https://arxiv.org/abs/2512.18254)
*Mingcheng Ye,Jiaming Liu,Yiren Song*

Main category: cs.CV

TL;DR: Loom是一个统一的扩散-Transformer框架，用于生成交织的文本和图像序列，在风格迁移、组合生成和教程式任务中表现出优越的时序一致性和图文对齐能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成连贯的视觉帧和对应的文本描述序列方面存在不足，特别是在长序列生成中难以保持时序一致性和图文对齐，限制了风格迁移、组合合成和教程生成等应用。

Method: 基于Bagel统一模型进行全参数微调，采用交替文本-视觉嵌入的架构进行多条件推理和序列规划；使用语言规划策略将用户指令分解为逐步提示和帧嵌入；通过采样少量历史帧结合全局文本上下文进行条件生成，而非拼接全部历史。

Result: 在文本到交织序列任务中，Loom显著优于开源基线Anole，在5分制评估中平均提升2.6分；在50K交织教程数据集上，相比统一模型和扩散编辑基线有显著改进。

Conclusion: Loom通过创新的架构设计和规划策略，实现了高效可控的长序列交织生成，在组合性、时序一致性和图文对齐方面表现优异，为复杂多模态序列生成提供了有效解决方案。
...
### [95] [Who Can See Through You? Adversarial Shielding Against VLM-Based Attribute Inference Attacks](https://arxiv.org/abs/2512.18264)
*Yucheng Fan,Jiawei Chen,Yu Tian,Zhaoxia Yin*

Main category: cs.CV

TL;DR: 本文提出了一种针对视觉语言模型属性推断攻击的隐私保护方法，通过联合优化隐私抑制和功能保持，在视觉一致性约束下平衡隐私保护与用户体验，并建立了公开评估基准VPI-COCO。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型的广泛应用，基于VLM的属性推断攻击成为严重隐私威胁，现有保护方法往往损害图像视觉质量或干扰社交媒体视觉功能，难以平衡隐私保护与用户体验。

Method: 提出联合优化隐私抑制和功能保持的新方法，在视觉一致性约束下进行优化；同时构建公开基准数据集VPI-COCO（包含522张图像及分层隐私问题），支持细粒度评估。

Result: 实验表明，该方法能将隐私属性识别率降至25%以下，非隐私属性保持率超过88%，保持高视觉一致性，并对未见及改写隐私问题具有良好泛化能力。

Conclusion: 所提方法在保护隐私的同时有效保持了图像实用性和视觉质量，具有实际部署价值；VPI-COCO基准为隐私保护方法评估提供了标准化工具。
...
### [96] [Building UI/UX Dataset for Dark Pattern Detection and YOLOv12x-based Real-Time Object Recognition Detection System](https://arxiv.org/abs/2512.18269)
...
TL;DR: 提出基于YOLOv12x的视觉暗模式检测框架，通过构建包含4066张UI截图的数据集，实现92.8%的检测精度和40.5 FPS的实时性能。


<details>
  <summary>Details</summary>
Motivation: 随着数字化转型加速，暗模式（误导用户选择的界面设计）问题日益突出，现有监管多为事后应对，亟需实时主动检测技术。

Method: 1. 手动收集194个国内外网站的4066张含暗模式的UI截图，标注按钮、复选框等5类组件；2. 采用YOLOv12x模型并应用迁移学习优化检测性能。

Result: 实验显示，该方法在mAP@50指标上达到92.8%的准确率，推理速度达40.5 FPS，具备实时部署能力；数据集已开源。

Conclusion: 所提框架能有效平衡检测精度与实时性，为在线平台暗模式监管提供技术支撑；公开数据集可促进领域研究发展。
...
### [97] [UniMPR: A Unified Framework for Multimodal Place Recognition with Arbitrary Sensor Configurations](https://arxiv.org/abs/2512.18279)
*Zhangshuo Qi,Jingyi Xu,Luqi Cheng,Shichen Wen,Yiming Ma,Guangming Xiong*

Main category: cs.CV

TL;DR: 提出UniMPR统一框架，解决多模态地点识别中动态适应任意模态输入、处理缺失/退化模态、跨传感器配置泛化三大挑战，在七个数据集上实现最优性能。


<details>
  <summary>Details</summary>
Motivation: 多模态地点识别在GPS拒止环境中对自动驾驶和机器人至关重要，但现有方法难以动态适应任意模态组合、处理模态缺失/退化问题，且泛化能力有限。

Method: 1. 将所有输入统一到极坐标鸟瞰图特征空间；2. 设计多分支网络提取任意模态组合的判别性特征；3. 构建大规模多数据集训练集，采用自适应标签分配策略进行预训练。

Result: 在七个数据集上的实验表明，UniMPR在不同传感器配置、模态组合和环境条件下均达到最先进性能。

Conclusion: UniMPR通过统一特征表示和自适应训练策略，实现了对任意模态组合的鲁棒地点识别，为多模态感知系统提供了通用解决方案。
...
### [98] [Pyramidal Adaptive Cross-Gating for Multimodal Detection](https://arxiv.org/abs/2512.18291)
*Zidong Gu,Shoufu Tian*

Main category: cs.CV

TL;DR: 提出PACGNet架构，通过对称交叉门控和金字塔特征感知多模态门控模块，在骨干网络内实现深度融合，提升航拍图像中小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法依赖简单聚合策略，易受跨模态噪声干扰并破坏特征金字塔层次结构，影响小目标细粒度检测精度。

Method: 设计对称交叉门控模块实现双向选择性信息吸收与噪声抑制；金字塔特征感知多模态门控模块通过渐进分层门控机制重建特征层次，保留细粒度细节。

Result: 在DroneVehicle和VEDAI数据集上mAP50分别达到81.7%和82.1%，刷新最优性能记录。

Conclusion: PACGNet通过深度层次化融合机制有效解决了跨模态噪声和特征结构破坏问题，显著提升了航拍目标检测性能。
...
### [99] [MatE: Material Extraction from Single-Image via Geometric Prior](https://arxiv.org/abs/2512.18312)
*Zeyu Zhang,Wei Zhai,Jian Yang,Yang Cao*

Main category: cs.CV

TL;DR: 提出MatE方法，可从单张无约束真实世界图像生成可平铺的物理渲染材料，无需专业设备或专家处理。


<details>
  <summary>Details</summary>
Motivation: 传统高保真物理渲染材料创建需要专业设备和专家后处理，限制了普及性。

Method: 使用估计深度图进行粗略校正，再通过双分支扩散模型利用旋转和尺度对齐训练数据学习一致性，进一步校正失真并生成完整材料图集。

Result: 框架对输入图像的未知光照和视角具有不变性，能从随意拍摄的图像中恢复本质材料属性，在合成和真实数据实验中表现有效且鲁棒。

Conclusion: MatE方法使非专业用户能够从真实世界图像创建逼真材料，推动了材料创建的民主化。
...
### [100] [MatSpray: Fusing 2D Material World Knowledge on 3D Geometry](https://arxiv.org/abs/2512.18314)
*Philipp Langsteiner,Jan-Niklas Dihlmann,Hendrik P. A. Lensch*

Main category: cs.CV

TL;DR: 提出一个将2D材质数据融合到3D几何中的框架，通过高斯溅射重建几何，扩散模型生成PBR材质图，结合学习与投影方法优化，实现更准确、可重光照的渲染。


<details>
  <summary>Details</summary>
Motivation: 游戏和电影行业中手动建模材质和3D几何耗时且必要；现有3D重建方法在重光照场景中因缺乏精确的空间变化材质参数而受限，而2D扩散模型预测的PBR材质图难以直接迁移到3D几何上。

Method: 使用高斯溅射重建场景几何；扩散模型从输入图像生成反照率、粗糙度和金属度等2D PBR材质图；通过优化图像损失或高斯光线追踪直接投影材质参数到高斯表示；引入轻量神经细化步骤（Neural Merger）提升细节精度和多视角一致性。

Result: 所提方法在定量指标和视觉真实感上均优于现有技术，实现了更准确、可重光照且逼真的渲染，显著提升了内容生产流程中资产创建的真实感和效率。

Conclusion: 该框架成功融合2D材质数据到3D几何，解决了材质迁移挑战，增强了重建场景的渲染质量，为行业资产创建提供了高效解决方案。
...
### [101] [A two-stream network with global-local feature fusion for bone age assessment](https://arxiv.org/abs/2512.18331)
*Qiong Lou,Han Yang,Fang Lu*

Main category: cs.CV

TL;DR: 提出BoNet+双流深度学习模型，结合全局与局部特征提取，在骨龄评估任务中实现高精度自动化评估。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在平衡全局特征与局部骨骼细节方面存在不足，需开发更精准的自动化骨龄评估系统以减轻临床工作量。

Method: 设计双流架构：全局通道引入Transformer增强全局特征提取；局部通道采用RFAConv模块提升多尺度感受野的局部特征提取能力；特征拼接后通过Inception-V3优化。

Result: 在RSNA和RHPE数据集上平均绝对误差分别为3.81和5.65个月，达到先进水平。

Conclusion: BoNet+模型能实现自动化、高精度、客观的骨龄评估，有效降低临床工作负担。
...
### [102] [MCVI-SANet: A lightweight semi-supervised model for LAI and SPAD estimation of winter wheat under vegetation index saturation](https://arxiv.org/abs/2512.18344)
*Zhiheng Zhang,Jiajun Yang,Hong Sun,Dong Wang,Honghua Jiang,Yaru Chen,Tangyuan Ning*

Main category: cs.CV

TL;DR: 提出MCVI-SANet模型，通过植被指数饱和度感知模块和半监督策略，解决冬小麦LAI和SPAD估算中的植被指数饱和与数据标注不足问题，在精度和效率上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 冬小麦密集冠层阶段植被指数易饱和，且地面真实标注数据有限，现有基于植被指数和纹理的机器学习方法特征表达能力不足，深度学习基线存在领域差异和高数据需求问题，限制了模型的泛化能力。

Method: 提出轻量级半监督视觉模型MCVI-SANet，包含新设计的植被指数饱和度感知模块（VI-SABlock）进行自适应通道-空间特征增强，并集成基于VICReg的半监督策略提升泛化能力；采用基于植被高度的数据集划分策略保持生长阶段代表性。

Result: 经过10次重复实验，MCVI-SANet在LAI估算上平均R²达0.8123、RMSE为0.4796，在SPAD估算上平均R²达0.6846、RMSE为2.4222，相比最佳基线模型，LAI平均R²提升8.95%，SPAD平均R²提升8.17%；模型仅0.10M参数，推理速度快。

Conclusion: 将半监督学习与农学先验知识结合，为增强基于遥感的精准农业提供了有前景的方法，MCVI-SANet在精度和效率上均达到先进水平。
...
### [103] [Enhancing 3D Semantic Scene Completion with a Refinement Module](https://arxiv.org/abs/2512.18363)
*Dunxing Zhang,Jiachen Lu,Han Yang,Lei Bao,Bo Song*

Main category: cs.CV

TL;DR: 提出ESSC-RM框架，通过两阶段细化模块提升现有语义场景补全模型的性能，实验表明能稳定提高语义预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有语义场景补全模型在细节和几何结构上存在不足，需要一种通用且即插即用的细化框架来提升预测质量。

Method: 采用两阶段框架：先用基线SSC网络生成粗体素预测，再通过3D U-Net的预测噪声感知模块和体素级局部几何模块进行多尺度监督细化。

Result: 在SemanticKITTI数据集上，集成到CGFormer和MonoScene后，平均IoU分别从16.87%提升至17.27%、从11.08%提升至11.51%。

Conclusion: ESSC-RM是一种通用的细化框架，可无缝集成到多种SSC模型中，有效提升语义场景补全性能。
...
### [104] [Efficient Zero-Shot Inpainting with Decoupled Diffusion Guidance](https://arxiv.org/abs/2512.18365)
*Badr Moufad,Navid Bagheri Shouraki,Alain Oliviero Durmus,Thomas Hirtz,Eric Moulines,Jimmy Olsson,Yazid Janati*

Main category: cs.CV

TL;DR: 提出一种新的零样本扩散模型后验采样方法，通过设计高效的高斯后验转移替代传统复杂的似然代理，显著降低推理成本并保持高质量图像重建。


<details>
  <summary>Details</summary>
Motivation: 现有零样本扩散模型方法依赖复杂的似然代理函数，需要在每个反向步骤中计算向量-雅可比乘积，导致内存和运行时开销巨大，限制了实际应用效率。

Method: 设计一种新的似然代理函数，生成简单高效的高斯后验转移过程，避免了通过去噪网络的梯度反向传播，从而减少计算负担。

Result: 实验表明，该方法在保持与微调基线相当的观测一致性的同时，能生成连贯、高质量的重建结果，并显著降低推理成本。

Conclusion: 提出的方法在零样本图像编辑任务中实现了效率与质量的平衡，为扩散模型的实际部署提供了更高效的解决方案。
...
### [105] [RecurGS: Interactive Scene Modeling via Discrete-State Recurrent Gaussian Fusion](https://arxiv.org/abs/2512.18386)
*Wenhao Hu,Haonan Zhou,Zesheng Li,Liu Liu,Jiacheng Dong,Zhizhong Su,Gaoang Wang*

Main category: cs.CV

TL;DR: 提出RecurGS框架，通过循环融合离散高斯场景状态，实现可交互、可更新的3D场景表示，支持物体级操作和新状态合成。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景表示方法无法适应离散场景变化、不支持多状态信息融合和交互式环境构建，限制了在视觉和机器人领域的应用。

Method: 采用循环融合框架，通过物体级变化检测、语义对应与SE(3)精炼对齐几何运动，结合体素化可见性感知融合模块选择性更新新区域，并利用回放监督保留历史结构。

Result: 在合成和真实数据集上验证了框架能实现高质量重建、显著提升更新效率，并保持场景演化的逼真度。

Conclusion: RecurGS为构建持续交互的高斯世界提供了可扩展的解决方案，推动了动态3D环境表示的发展。
...
### [106] [Automated Mosaic Tesserae Segmentation via Deep Learning Techniques](https://arxiv.org/abs/2512.18406)
*Charilaos Kapelonis,Marios Antonakakis,Konstantinos Politof,Aristomenis Antoniadis,Michalis Zervakis*

Main category: cs.CV

TL;DR: 本文提出了一种基于SAM 2的马赛克图像自动分割方法，通过创建标注数据集进行微调，显著提升了分割性能，为文化遗产数字化保护提供了有效工具。


<details>
  <summary>Details</summary>
Motivation: 马赛克作为重要的文化遗产，因年代久远易受损，亟需数字化保存。现有公开数据集有限，传统分割方法难以满足高精度需求，因此探索基于基础模型的高效自动分割方法。

Method: 利用Meta AI的Segment Anything Model 2（SAM 2）作为基础模型，创建了马赛克图像标注数据集进行微调，实现自动分割马赛克瓷砖与背景。

Result: 在自建测试集上，IoU从89.00%提升至91.02%，Recall从92.12%提升至95.89%；在已有基准测试中，F-measure比先前方法提高3%，预测与实际瓷砖数量绝对误差从0.20降至0.02。

Conclusion: 微调后的SAM 2模型结合新标注数据集能有效提升马赛克分割精度，为实时分割马赛克图像奠定了基础，有助于文化遗产的数字化保护。
...
### [107] [Through the PRISm: Importance-Aware Scene Graphs for Image Retrieval](https://arxiv.org/abs/2512.18407)
*Dimitrios Georgoulopoulos,Nikolaos Chaidos,Angeliki Dimitriou,Giorgos Stamou*

Main category: cs.CV

TL;DR: 提出PRISm框架，通过重要性预测和图神经网络改进图像检索，能更好地捕捉语义关系和上下文信息。


<details>
  <summary>Details</summary>
Motivation: 传统图像检索方法难以捕捉场景中的语义关系和上下文细微差别，需要更符合人类感知的检索方法。

Method: 1. 重要性预测模块：识别并保留关键对象和关系三元组，剪枝无关元素；2. 边缘感知图神经网络：显式编码关系结构并整合全局视觉特征，生成语义感知的图像嵌入。

Result: 在基准和真实数据集上均取得优越的top-ranked性能，定性分析显示能准确捕捉关键对象和交互，产生可解释的语义结果。

Conclusion: PRISm通过显式建模对象及其交互的语义重要性，实现了更符合人类感知的图像检索，有效结合了关系推理和视觉表示。
...
### [108] [AmPLe: Supporting Vision-Language Models via Adaptive-Debiased Ensemble Multi-Prompt Learning](https://arxiv.org/abs/2512.18411)
*Fei Song,Yi Li,Jiangmeng Li,Rui Wang,Changwen Zheng,Fanjiang Xu,Hui Xiong*

Main category: cs.CV

TL;DR: 提出自适应去偏集成多提示学习方法AmPLe，通过集成学习和信息理论指导的权重计算，同时缓解模型-提示匹配偏差和样本-提示匹配偏差，在多个下游任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多提示学习方法主要关注单一基础视觉语言模型内的提示设计，但忽视了模型-提示匹配偏差（同一提示在不同模型中语义不一致）和样本-提示匹配偏差（样本中包含与提示无关的语义），这两种偏差会影响下游任务性能。

Method: 1. 采用集成学习方法聚合不同模型的预测优势；2. 基于信息理论分析从输入样本中提取与提示相关的语义，自适应计算去偏的集成权重；3. 提出AmPLe框架同时缓解两种偏差。

Result: 在三个代表性任务（新类别泛化、新目标数据集、未见域偏移）上的大量实验表明，AmPLe能广泛超越现有方法；因果视角的理论验证进一步支持了其有效性。

Conclusion: AmPLe通过同时解决模型-提示和样本-提示匹配偏差，显著提升了多提示学习在下游任务中的性能，为视觉语言模型的快速适应提供了更鲁棒的解决方案。
...
### [109] [MeniMV: A Multi-view Benchmark for Meniscus Injury Severity Grading](https://arxiv.org/abs/2512.18437)
*Shurui Xu,Siqi Yang,Jiapin Ren,Zhong Cao,Hongwei Yang,Mengzhen Fan,Yuyu Sun,Shuyan Li*

Main category: cs.CV

TL;DR: 提出了MeniMV数据集，用于半月板前角和后角损伤的自动分级，包含3000个膝关节MRI检查的6000张配准图像，并建立了多个模型的基准性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖粗粒度研究级标签或二分类，缺乏定位和严重程度信息，半月板角部撕裂的精确分级在自动MRI分析中尚未充分探索。

Method: 构建了多视图基准数据集MeniMV，包含来自三个医疗中心的750名患者的3000个膝关节MRI检查，提供6000张配准的矢状面和冠状面图像，每个检查由首席骨科医生验证并标注前角和后角的四级严重程度标签。

Result: MeniMV提供了比先前数据集多一倍以上的病理标记数据量，并独特地捕捉了临床实践中必需的双视图诊断上下文，通过多个最先进的CNN和Transformer模型建立了强基准。

Conclusion: MeniMV为自动肌肉骨骼成像的未来研究提供了宝贵基础，实验突出了严重程度分级中的挑战，并展示了数据集的实用性。
...
### [110] [Object-Centric Framework for Video Moment Retrieval](https://arxiv.org/abs/2512.18448)
*Zongyao Li,Yongkang Wong,Satoshi Yamazaki,Jianquan Liu,Mohan Kankanhalli*

Main category: cs.CV

TL;DR: 提出了一种基于对象中心的视频时刻检索框架，通过场景图解析提取查询相关对象，构建对象级特征序列，并使用关系轨迹变换器建模对象间的时空相关性，显著提升了对象导向查询的定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有视频时刻检索方法主要依赖帧或片段的全局特征，难以捕捉细粒度的对象语义和外观信息，导致在涉及特定实体交互的对象导向查询中定位效果受限。

Method: 1. 使用场景图解析器提取查询相关对象；2. 从视频帧生成场景图表示对象及其关系；3. 构建编码丰富视觉语义信息的对象级特征序列；4. 设计关系轨迹变换器建模对象间的时空相关性。

Result: 在Charades-STA、QVHighlights和TACoS三个基准测试中，该方法均优于现有最先进方法。

Conclusion: 通过显式建模对象级状态变化和时空关联，所提框架能更精准地定位与对象导向查询对齐的视频时刻，为细粒度视频理解提供了有效解决方案。
...
### [111] [Plasticine: A Traceable Diffusion Model for Medical Image Translation](https://arxiv.org/abs/2512.18455)
*Tianyang Zhanng,Xinxing Cheng,Jun Cheng,Shaoming Zheng,He Zhao,Huazhu Fu,Alejandro F Frangi,Jiang Liu,Jinming Duan*

Main category: cs.CV

TL;DR: 提出Plasticine框架，首个以可追溯性为核心的端到端医学图像翻译方法，结合强度转换和空间变换，在扩散去噪框架中实现像素级对应关系。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析中，成像设备和人群分布差异导致域间差距，现有图像翻译方法忽视空间对应关系，而临床应用中需要原始图像与翻译图像间的像素级可追溯性以增强临床可解释性。

Method: 在去噪扩散框架中结合强度翻译和空间变换，通过端到端设计同时处理强度转换和空间形变，确保翻译过程的空间连贯性和像素级可追溯性。

Result: 开发了首个以可追溯性为核心目标的图像翻译框架，能够生成具有可解释强度过渡和空间连贯形变的合成图像，支持整个翻译过程的像素级跟踪。

Conclusion: Plasticine框架通过整合强度翻译和空间变换，解决了医学图像翻译中可追溯性缺失的问题，为临床应用提供了更高可解释性的合成数据生成方法。
...
### [112] [Adaptive-VoCo: Complexity-Aware Visual Token Compression for Vision-Language Models](https://arxiv.org/abs/2512.18496)
*Xiaoyang Guo,Keze Wang*

Main category: cs.CV

TL;DR: 本文提出Adaptive-VoCo框架，通过轻量级预测器动态选择最优视觉压缩率，以平衡多模态模型的计算效率与表示能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如VoCo-LLaMA）采用固定压缩率处理视觉特征，无法适应不同图像的视觉复杂度变化，限制了模型在效率与性能间的自适应平衡能力。

Method: 1. 在VoCo-LLaMA基础上增加轻量级预测器，通过视觉编码器的统计特征（如图像块标记熵、注意力图方差）量化图像复杂度；2. 设计联合损失函数，整合压缩率正则化与复杂度对齐目标。

Result: 实验表明，该方法在多种多模态任务中均优于固定压缩率基线，验证了自适应视觉压缩对提升模型效率与鲁棒性的有效性。

Conclusion: 自适应视觉压缩机制能显著提升视觉语言模型的效率与性能平衡能力，为构建更高效、鲁棒的多模态系统提供了新思路。
...
### [113] [PlantDiseaseNet-RT50: A Fine-tuned ResNet50 Architecture for High-Accuracy Plant Disease Detection Beyond Standard CNNs](https://arxiv.org/abs/2512.18500)
*Santwana Sagnika,Manav Malhotra,Ishtaj Kaur Deol,Soumyajit Roy,Swarnav Kumar*

Main category: cs.CV

TL;DR: 提出PlantDiseaseNet-RT50，一种基于ResNet50微调的深度学习模型，用于自动化植物病害检测，在综合数据集上达到约98%的准确率、精确率和召回率。


<details>
  <summary>Details</summary>
Motivation: 植物病害导致全球70-80%的作物损失，传统依赖专家视觉检测的方法耗时耗力，难以适用于大规模农业操作，需要高效自动化的诊断工具。

Method: 基于ResNet50架构进行微调，采用策略性解冻层、自定义分类头（含正则化机制）、余弦衰减动态学习率调度，并结合批归一化、Dropout正则化等高级训练技术。

Result: 模型在多种作物病害的综合数据集上表现优异，准确率、精确率和召回率均达到约98%，验证了其作为高效农业诊断工具的可行性。

Conclusion: PlantDiseaseNet-RT50通过针对性微调和优化，将预训练模型转化为专业的农业诊断工具，为实际农业应用提供了计算高效、快速准确的病害检测方案，有助于及时干预并减少作物损失。
...
### [114] [Geometric-Photometric Event-based 3D Gaussian Ray Tracing](https://arxiv.org/abs/2512.18640)
*Kai Kohyama,Yoshimitsu Aoki,Guillermo Gallego,Shintaro Shiba*

Main category: cs.CV

TL;DR: 提出一种事件相机3D高斯泼溅框架，通过解耦几何与辐射渲染，在精度与时间分辨率间取得平衡，实现高效3D重建。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率，但现有事件3D高斯泼溅方法难以利用稀疏事件的细粒度时间信息，需要在精度与时间分辨率间权衡。

Method: 采用双分支渲染：事件级几何（深度）渲染与快照式辐射（强度）渲染，结合光线追踪与扭曲事件图像技术。

Result: 在真实数据集上达到SOTA性能，合成数据集上表现优异；无需预训练模型或COLMAP初始化，事件选择灵活，边缘重建清晰且训练快速。

Conclusion: 该方法深化了对事件稀疏性在3D重建中作用的理解，为事件相机3D重建提供了高效灵活的解决方案。
...
### [115] [Point What You Mean: Visually Grounded Instruction Policy](https://arxiv.org/abs/2512.18933)
...
TL;DR: 提出Point-VLA模型，通过视觉提示增强语言指令以解决物体指代模糊问题，提升在复杂场景中的物体级定位能力
...
Method: 1) 开发Point-VLA即插即用策略，用边界框等显式视觉线索增强语言指令；2) 构建自动数据标注流程，以最小人工成本扩展视觉基础数据集

Result: 在多样化真实世界指代任务中，Point-VLA性能始终优于纯文本指令模型，在杂乱场景和未见物体场景中表现尤为突出，具有鲁棒的泛化能力

Conclusion: Point-VLA通过像素级视觉基础有效解决物体指代模糊问题，实现了更具泛化性的具身控制
...
### [116] [NASTaR: NovaSAR Automated Ship Target Recognition Dataset](https://arxiv.org/abs/2512.18503)
*Benyamin Hosseiny,Kamirul Kamirul,Odysseas Pappas,Alin Achim*

Main category: cs.CV

TL;DR: 提出了基于NovaSAR S波段图像的NASTaR船舶目标识别数据集，包含3415个船舶图像块，匹配AIS数据标注23个类别，通过基准深度学习模型验证了其在船舶类型分类任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 合成孔径雷达(SAR)在海上船舶监测中具有全天候优势，但船舶类型分类面临船舶多样性高、识别难度大的挑战，现有深度学习模型依赖大规模高质量标注数据，且不同SAR卫星的频率和分辨率差异加剧了对标注数据集的需求。

Method: 构建NASTaR数据集，从NovaSAR S波段图像中提取3415个船舶图像块，通过AIS数据匹配标注23个船舶类别，区分近岸/离岸场景，并提供可见尾流的辅助数据集；使用基准深度学习模型验证数据集的适用性。

Result: 在四类主要船舶分类中准确率超过60%，三类场景分类准确率超过70%，货船与油轮区分准确率超过75%，渔船识别准确率超过87%。

Conclusion: NASTaR数据集为SAR船舶类型分类提供了高质量的标注资源，通过基准实验证明了其有效性，相关数据和代码已公开，有助于推动该领域的研究发展。
...
### [117] [GTMA: Dynamic Representation Optimization for OOD Vision-Language Models](https://arxiv.org/abs/2512.18504)
*Jensen Zhang,Ningyuan Liu,Keze Wang*

Main category: cs.CV

TL;DR: 提出GTMA框架，通过动态表示优化解决视觉语言模型在开放世界中的模态不对称问题，显著提升OOD概念的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在开放世界应用中面临模态不对称问题：视觉编码器能提取未见图像的判别特征，而文本编码器受固定词汇表限制，无法合成新语义锚点，导致跨模态对齐崩溃和零样本性能下降。

Method: 提出引导目标匹配适应（GTMA）框架，在推理时构建连续伪词嵌入以对齐OOD图像的视觉锚点，采用自适应梯度表示策略优化算法，并引入语义正则化保持合理性和先验知识兼容性。

Result: 在ImageNet-R和VISTA-Beyond基准测试中，GTMA将零样本和少样本OOD准确率提升15-20%，同时保持分布内概念性能；消融研究证实伪词优化的必要性。

Conclusion: GTMA通过动态表示优化有效突破预训练语义空间限制，为视觉语言模型处理开放世界OOD概念提供了可行解决方案。
...
### [118] [VLNVerse: A Benchmark for Vision-Language Navigation with Versatile, Embodied, Realistic Simulation and Evaluation](https://arxiv.org/abs/2512.19021)
...
TL;DR: 本文介绍了VLNVerse，一个用于视觉语言导航（VLN）的大规模、可扩展基准测试平台，旨在解决现有基准测试在规模、物理真实性和任务碎片化方面的局限性，推动具身智能体向真实世界泛化发展。


<details>
  <summary>Details</summary>
Motivation: 现有VLN基准测试存在数据集规模小、物理模拟简单、任务碎片化等问题，限制了其对模拟到真实泛化的洞察，且无法满足基于大语言模型的预训练需求，因此需要一个新的、更全面的基准测试平台。

Method: 提出了VLNVerse基准测试，其特点包括：1）统一了以往碎片化的任务；2）提供可扩展的研究工具包；3）支持全运动学的具身智能体设计；4）基于强大物理引擎实现真实模拟；5）利用其规模与多样性对现有方法进行全面评估。

Result: VLNVerse重新定义了VLN为一个可扩展的全栈具身AI问题，并提出了一个能够处理基准测试中所有任务的统一多任务模型。该平台旨在缩小模拟导航与真实世界泛化之间的差距。

Conclusion: VLNVerse为研究社区提供了一个重要工具，以促进可扩展、通用目的具身运动智能体的研究，推动VLN领域向更真实、更统一的评估框架发展。
...
### [119] [Detection of AI Generated Images Using Combined Uncertainty Measures and Particle Swarm Optimised Rejection Mechanism](https://arxiv.org/abs/2512.18527)
*Rahul Yumlembam,Biju Issac,Nauman Aslam,Eaby Kollonoor Babu,Josh Collyer,Fraser Kennedy*

Main category: cs.CV

TL;DR: 提出一个基于多源不确定性融合的AI生成图像检测框架，通过整合Fisher信息、蒙特卡洛Dropout熵和深度核学习预测方差，结合粒子群优化自适应阈值，在分布偏移下保持鲁棒检测性能。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像逼真度提升，传统检测方法在分布偏移下性能下降，需要开发能自适应不确定性的鲁棒检测框架。

Method: 1. 使用三种不确定性度量：Fisher信息（参数敏感性）、蒙特卡洛Dropout熵（预测变异性）、深度核学习高斯过程分类器的预测方差；2. 采用粒子群优化学习权重并确定自适应拒绝阈值；3. 在Stable Diffusion生成图像上训练，在多种生成器（GLIDE、VQDM等）上评估分布偏移下的性能。

Result: 1. 组合不确定性方法在未见生成器上达到约70%的错误拒绝率，有效过滤多数误分类AI样本；2. 对抗攻击下，组合方法拒绝约61%成功攻击，仅用GP不确定性可达80%；3. 对自然图像和域内AI数据保持高准确预测接受率。

Conclusion: 多源不确定性融合为AI生成图像检测提供了具有适应性和鲁棒性的解决方案，虽可能保守拒绝新生成器的正确预测，但被拒样本可用于模型重训练。
...
### [120] [Sign Language Recognition using Parallel Bidirectional Reservoir Computing](https://arxiv.org/abs/2512.19451)
*Nitin Kumar Singh,Arie Rachmad Syulistyo,Yuichiro Tanaka,Hakaru Tamukoh*

Main category: cs.CV

TL;DR: 提出了一种轻量级手语识别系统，结合并行双向储备池计算与MediaPipe，在WLASL数据集上实现高效实时识别，显著降低计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的手语识别模型计算资源需求高，难以部署在边缘设备上，需要一种轻量化的解决方案以实现实时手语识别。

Method: 使用MediaPipe进行实时手部跟踪和关节坐标提取，设计并行双向储备池计算架构，包含两个基于回声状态网络的双向储备池计算模块，以捕获时间依赖性并生成丰富特征表示。

Result: 在WLASL数据集上，top-1、top-5和top-10准确率分别为60.85%、85.86%和91.74%；训练时间仅需18.67秒，远低于基于Bi-GRU等深度学习方法的55分钟以上。

Conclusion: 该方法为边缘设备提供了一种轻量级、低成本的实时手语识别解决方案，通过储备池计算的固有特性显著提升了训练效率。
...
### [121] [WoundNet-Ensemble: A Novel IoMT System Integrating Self-Supervised Deep Learning and Multi-Model Fusion for Automated, High-Accuracy Wound Classification and Healing Progression Monitoring](https://arxiv.org/abs/2512.18528)
*Moses Kiprono*

Main category: cs.CV

TL;DR: 提出WoundNet-Ensemble系统，集成三种深度学习模型，实现六种慢性伤口的自动分类，准确率达99.90%，并开发伤口愈合追踪功能。


<details>
  <summary>Details</summary>
Motivation: 慢性伤口（如糖尿病足溃疡）带来巨大临床和经济负担，现有伤口评估方法主观性强，导致分类不一致和干预延迟，需自动化、客观的评估工具。

Method: 基于医疗物联网系统，集成ResNet-50、DINOv2视觉Transformer和Swin Transformer三种深度学习架构，采用加权融合策略，并开发纵向伤口愈合追踪器。

Result: 在5,175张伤口图像数据集上达到99.90%的集成准确率，比现有最佳方法提升3.7%；系统能计算愈合率、严重程度评分并生成临床警报。

Conclusion: 该系统为伤口护理提供了稳健、准确且可临床部署的AI工具，支持远程医疗和患者监测，模型将公开以促进可重复性。
...
### [122] [Zero-shot Reconstruction of In-Scene Object Manipulation from Video](https://arxiv.org/abs/2512.19684)
*Dixuan Lin,Tianyou Wang,Zhuoyang Pan,Yufu Wang,Lingjie Liu,Kostas Daniilidis*

Main category: cs.CV

TL;DR: 首个从单目RGB视频重建场景内物体操作的系统，通过数据驱动模型初始化和两阶段优化解决手-物体深度模糊和物理合理性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法以手为中心坐标且忽略场景，导致度量精度低和实用性受限；需要解决场景重建不适定、手-物体深度模糊及物理合理交互的挑战。

Method: 1. 使用数据驱动基础模型初始化物体网格与位姿、场景点云和手部位姿；2. 应用两阶段优化恢复从抓取到交互的完整手-物体运动，确保与输入视频场景信息一致。

Result: 构建了首个单目视频场景物体操作重建系统，实现了与场景一致的物理合理手-物体运动恢复。

Conclusion: 该方法通过结合基础模型初始化和场景一致优化，显著提升了单目视频中物体操作重建的度量精度和实用性。
...
### [123] [Hierarchical Bayesian Framework for Multisource Domain Adaptation](https://arxiv.org/abs/2512.18553)
*Alexander M. Glandon,Khan M. Iftekharuddin*

Main category: cs.CV

TL;DR: 提出了一种用于多源域自适应（MDA）的贝叶斯框架，通过利用源域分布之间的相似性优化预训练，在人类动作识别任务上相比现有方法提升了17.29%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有MDA方法在预训练阶段存在临时性（ad-hoc）问题，要么基于权重共享，要么使用独立训练的模型，未能有效利用不同源域分布之间的相似性。

Method: 提出分层贝叶斯框架（Hierarchical Bayesian Framework），利用不同源域数据分布的相似性来优化MDA的预训练过程。

Result: 在Daily-DA RGB视频数据集的人类动作识别任务中，该框架相比现有最先进的MDA方法实现了17.29%的准确率提升。

Conclusion: 所提出的贝叶斯框架通过建模源域分布相似性，显著提升了多源域自适应的性能，为MDA预训练提供了更系统的方法。
...
### [124] [Enhancing Medical Large Vision-Language Models via Alignment Distillation](https://arxiv.org/abs/2512.18554)
*Aofei Chang,Ting Wang,Fenglong Ma*

Main category: cs.CV

TL;DR: 提出MEDALIGN框架，通过视觉对齐蒸馏解决医学大视觉语言模型中的幻觉问题，提升医学报告生成和VQA任务的性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 医学大视觉语言模型在临床应用中常因视觉理解未对齐而产生幻觉输出，主要源于视觉表示学习不足和视觉注意力对齐不良。

Method: 提出轻量级对齐蒸馏框架MEDALIGN，从领域特定的CLIP模型迁移视觉对齐知识，包含基于视觉token相似度结构的空间感知视觉对齐损失和引导注意力至诊断相关区域的注意力感知蒸馏损失。

Result: 在医学报告生成和医学视觉问答基准测试中，MEDALIGN持续提升模型性能和可解释性，产生更视觉基础化的输出。

Conclusion: MEDALIGN通过简单有效的对齐蒸馏方法，显著缓解了医学大视觉语言模型的幻觉问题，增强了视觉与语言的对齐能力。
...
### [125] [OpenView: Empowering MLLMs with Out-of-view VQA](https://arxiv.org/abs/2512.18563)
*Qixiang Chen,Cheng Zhang,Chi-Wing Fu,Jingwen Ye,Jianfei Cai*

Main category: cs.CV

TL;DR: 本文首次研究多模态大语言模型在视野外理解的能力，提出OpenView框架、数据集和基准测试，显著提升模型在视野外视觉问答任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要在图像帧内内容推理表现良好，但缺乏对视野外（超出可见图像范围）物体、活动和场景的推理能力，这限制了模型在真实世界场景中的完整理解。

Method: 1. 设计OpenView四阶段流水线，利用全景图像生成具有丰富上下文和空间基础的视野外VQA数据；2. 构建OpenView-Dataset高质量合成数据集；3. 建立OpenView-Bench基准测试，联合评估选择准确性和推理合理性。

Result: 实验表明，经过OpenView增强后，多个MLLMs在视野外VQA任务上的平均准确率从48.6%提升至64.1%，但仍与人类表现存在较大差距。

Conclusion: OpenView框架有效提升了MLLMs的视野外理解能力，为模型在更广泛场景下的空间推理提供了新的数据集和评估基准，推动了多模态理解的发展。
...
### [126] [Placenta Accreta Spectrum Detection Using an MRI-based Hybrid CNN-Transformer Model](https://arxiv.org/abs/2512.18573)
*Sumaiya Ali,Areej Alhothali,Ohoud Alzamzami,Sameera Albasri,Ahmed Abduljabbar,Muhammad Alwazzan*

Main category: cs.CV

TL;DR: 提出一种结合3D DenseNet121和3D Vision Transformer的混合深度学习模型，用于从MRI扫描中自动检测胎盘植入谱系疾病（PAS），在独立测试集上达到84.3%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 胎盘植入谱系疾病（PAS）是一种严重的产科疾病，但由于放射科医生对MRI解读存在差异，诊断具有挑战性，需要开发自动化工具提高诊断一致性和准确性。

Method: 使用包含1,133个MRI体积的回顾性数据集，开发并比较多种3D深度学习架构，最终提出融合3D DenseNet121（提取局部特征）和3D Vision Transformer（建模全局空间上下文）的混合模型。

Result: 在独立测试集上，DenseNet121-ViT混合模型在五次运行中平均准确率达到84.3%，优于其他对比的3D深度学习架构。

Conclusion: 混合CNN-Transformer模型作为计算机辅助诊断工具具有显著优势，能够为放射科医生提供可靠的决策支持，提高PAS诊断的一致性和及时性。
...
### [127] [Commercial Vehicle Braking Optimization: A Robust SIFT-Trajectory Approach](https://arxiv.org/abs/2512.18597)
...
TL;DR: 提出基于视觉的轨迹分析方案，解决商用车AEB系统低速运行时因CAN信号不准确导致的'零速制动'问题，通过视频处理精确识别车辆运动状态，显著减少误触发。


<details>
  <summary>Details</summary>
Motivation: 商用车自动紧急制动系统在低速运行时，因CAN信号不准确易产生'零速制动'误触发，影响系统安全性与可靠性，需一种更可靠的车辆运动状态识别方法。

Method: 采用NVIDIA Jetson AGX Xavier平台处理盲区摄像头视频序列，使用自适应CLAHE增强的SIFT特征提取与KNN-RANSAC匹配，结合多帧轨迹位移统计、双阈值状态决策矩阵和OBD-II驱动的动态ROI配置。

Result: 在真实数据集（32,454段视频，1,852辆车）上，静态检测F1-score达99.96%，运动状态识别97.78%，处理延迟14.2毫秒；实际部署使误制动事件减少89%，紧急制动成功率100%，故障率低于5%。

Conclusion: 该视觉轨迹分析方案能有效抑制环境干扰和动态物体误检，显著提升商用车AEB系统在低速场景下的可靠性，为解决安全系统误触发问题提供了可行方案。
...
### [128] [SimpleCall: A Lightweight Image Restoration Agent in Label-Free Environments with MLLM Perceptual Feedback](https://arxiv.org/abs/2512.18599)
*Jianglin Lu,Yuanwei Wu,Ziyi Zhao,Hongcheng Wang,Felix Jimenez,Abrar Majeedi,Yun Fu*

Main category: cs.CV

TL;DR: 提出基于策略优化的图像修复框架，通过轻量级智能体学习工具调用序列，无需标注数据，在多种退化场景下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的修复方法存在效率瓶颈（如反射、回滚、迭代工具搜索），且严重依赖需要大量标注的退化识别模型，限制了在无标注环境下的应用。

Method: 1. 采用策略优化框架训练轻量级智能体，通过序列决策过程选择修复操作；2. 引入基于多模态大语言模型的奖励机制，作为人类对齐评估器提供感知反馈；3. 训练后执行确定性修复计划，避免冗余工具调用。

Result: 在无监督情况下，全参考指标匹配SOTA性能，无参考指标在多种退化场景下超越现有方法，同时显著加速推理。

Conclusion: 所提框架有效解决了现有方法的效率瓶颈和标注依赖问题，实现了高效且高质量的复杂图像修复。
...
### [129] [Text2Graph VPR: A Text-to-Graph Expert System for Explainable Place Recognition in Changing Environments](https://arxiv.org/abs/2512.18613)
*Saeideh Yousefzadeh,Hamidreza Pourreza*

Main category: cs.CV

TL;DR: 提出Text2Graph VPR系统，通过将图像序列转换为文本描述并解析为场景图，实现可解释的视觉地点识别。系统融合图注意力网络嵌入和最短路径核进行检索，在光照、天气和季节变化下保持鲁棒性。
...
Method: 1. 将图像序列转换为文本场景描述；2. 解析为结构化场景图（包含对象、属性和关系）；3. 聚合单帧图为紧凑地点表示；4. 使用双相似度机制（GAT嵌入+最短路径核）进行检索。

Result: 在Oxford RobotCar和MSLS基准测试中验证，系统在严重外观变化下实现鲁棒检索，支持零样本人类文本查询，并产生可读的中间表示以支持诊断分析。

Conclusion: 基于语义图推理的地点识别是一种可行且可解释的替代方案，特别适用于安全敏感和资源受限的场景。
...
### [130] [PTTA: A Pure Text-to-Animation Framework for High-Quality Creation](https://arxiv.org/abs/2512.18614)
*Ruiqi Chen,Kaitong Cai,Yijia Fan,Keze Wang*

Main category: cs.CV

TL;DR: 提出了PTTA框架，通过构建高质量动画-文本配对数据集，并微调预训练视频生成模型HunyuanVideo，实现了纯文本生成高质量动画视频。


<details>
  <summary>Details</summary>
Motivation: 传统动画制作流程复杂且成本高昂，现有视频生成模型（如Sora等）在自然视频合成上表现优异，但在动画生成方面存在局限。虽然AniSora等研究通过微调图像到视频模型取得进展，但文本到动画的直接生成仍缺乏充分探索。

Method: 1. 构建小规模高质量的动画视频-文本描述配对数据集；2. 基于预训练文本到视频模型HunyuanVideo进行微调，使其适应动画风格生成。

Result: 在多维度视觉评估中，PTTA框架在动画视频合成任务上持续优于可比基线方法。

Conclusion: PTTA框架通过数据构建与模型微调相结合，有效提升了纯文本到动画视频的生成质量，为动画创作提供了高效解决方案。
...
### [131] [Uni-Neur2Img: Unified Neural Signal-Guided Image Generation, Editing, and Stylization via Diffusion Transformers](https://arxiv.org/abs/2512.18635)
*Xiyue Bai,Ronghao Yu,Jia Xiu,Pengfei Zhou,Jie Xia,Peng Ji*

Main category: cs.CV

TL;DR: 提出了Uni-Neur2Img统一框架，用于神经信号驱动的图像生成与编辑，通过参数高效的LoRA模块和因果注意力机制，在多个数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有神经驱动生成研究主要依赖文本模态作为条件或中间表示，缺乏对视觉模态作为直接条件信号的探索，限制了神经信号在图像生成与编辑中的应用潜力。

Method: 1. 引入参数高效的基于LoRA的神经信号注入模块，可独立处理每种条件信号；2. 采用因果注意力机制适应长序列建模需求；3. 构建EEG-Style数据集；4. 在CVPR40、Loongx和自收集数据集上进行多任务评估。

Result: 实验表明，该方法在生成保真度、编辑一致性和风格迁移质量方面均有显著提升，同时保持低计算开销和良好的多模态扩展性。

Conclusion: Uni-Neur2Img为神经信号与视觉内容生成提供了统一、高效且可扩展的解决方案，推动了神经科学、视觉与脑机交互的交叉研究。
...
### [132] [SplatBright: Generalizable Low-Light Scene Reconstruction from Sparse Views via Physically-Guided Gaussian Enhancement](https://arxiv.org/abs/2512.18655)
*Yue Wen,Liang Song,Hesheng Wang*

Main category: cs.CV

TL;DR: 提出SplatBright框架，首个可泛化的3D高斯方法，从稀疏sRGB输入联合实现低光增强与重建，解决曝光失衡和色彩退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏视图低光3D重建中面临视图不一致、需逐场景训练的问题，且缺乏大规模几何一致的配对数据。

Method: 采用物理引导的照明建模与几何-外观解耦；双分支预测器初始化3D高斯几何参数；照明一致性利用频率先验实现可控光照；外观细化模块分离照明、材质和视角线索；通过物理相机模型合成暗视图训练数据。

Result: 在公开和自采集数据集上，SplatBright在新视角合成、跨视图一致性和对未见低光场景的泛化能力上优于现有2D和3D方法。

Conclusion: SplatBright通过物理引导的照明建模和解耦设计，实现了稀疏视图下低光场景的高质量、可泛化3D重建与增强。
...
### [133] [Adversarial Robustness in Zero-Shot Learning:An Empirical Study on Class and Concept-Level Vulnerabilities](https://arxiv.org/abs/2512.18651)
*Zhiyuan Peng,Zihan Ye,Shreyank N Gowda,Yuping Yan,Haotian Xu,Ling Shao*

Main category: cs.CV

TL;DR: 本文实证分析了零样本学习（ZSL）模型在类别和概念层面的对抗鲁棒性，发现现有模型易受传统类别攻击和新型概念攻击的影响，并提出了一种增强的类别偏置攻击（CBEA）以完全消除广义零样本学习（GZSL）的准确性。


<details>
  <summary>Details</summary>
Motivation: 零样本学习模型旨在识别训练中未出现的类别，但其在系统性输入扰动下的鲁棒性尚不明确。本研究旨在探究ZSL模型在对抗攻击下的脆弱性，特别是在类别和概念层面的表现。

Method: 研究采用实证分析方法，评估了三种典型的ZSL模型。在类别层面，使用了非目标类别攻击（clsA）并提出了类别偏置增强攻击（CBEA）；在概念层面，引入了两种新型攻击模式：类别保留概念攻击（CPconA）和非类别保留概念攻击（NCPconA）。

Result: 实验表明，ZSL模型在传统类别攻击下易受干扰，但在GZSL设置中，clsA仅在原始最佳校准点有效，攻击后最佳校准点发生偏移。CBEA能完全消除GZSL在所有校准点的准确性。概念攻击可轻易通过擦除或引入概念来操纵类别预测，揭示模型在概念层面同样脆弱。

Conclusion: 现有ZSL模型在对抗攻击下存在显著鲁棒性缺陷，尤其是在概念层面。研究结果强调了提升ZSL模型对抗鲁棒性的迫切需求，以应对潜在恶意操纵。
...
### [134] [PMPGuard: Catching Pseudo-Matched Pairs in Remote Sensing Image-Text Retrieval](https://arxiv.org/abs/2512.18660)
*Pengxiang Ouyang,Qing Ma,Zheng Wang,Cong Bai*

Main category: cs.CV

TL;DR: 提出了一种新的遥感图像-文本检索框架，通过跨模态门控注意力和正负感知注意力机制，有效处理伪匹配对问题，在多个基准数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像-文本检索在实际数据集中存在伪匹配对问题，即语义不匹配或弱对齐的图像-文本对，这会阻碍可靠的跨模态对齐学习。

Method: 采用跨模态门控注意力动态调节跨模态信息流，并结合正负感知注意力机制，在对齐学习中明确区分信息性（正）线索和误导性（负）线索。

Result: 在RSICD、RSITMD和RS5M三个基准数据集上的大量实验表明，该方法始终达到最先进的性能，突出了其在处理遥感图像-文本检索任务中真实世界不匹配和伪匹配对方面的鲁棒性和有效性。

Conclusion: 所提出的框架通过门控和感知机制有效缓解了伪匹配对的影响，为遥感跨模态检索提供了更可靠的解决方案。
...
### [135] [SmartSight: Mitigating Hallucination in Video-LLMs Without Compromising Video Understanding via Temporal Attention Collapse](https://arxiv.org/abs/2512.18671)
*Yiming Sun,Mi Zhang,Feifei Li,Geng Hong,Min Yang*

Main category: cs.CV

TL;DR: 提出SmartSight方法，通过模型自省能力在无需训练的情况下降低视频大语言模型的感知幻觉，同时提升视频理解与推理能力。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型存在严重的感知幻觉问题，影响实际应用安全性；现有缓解方法常损害模型对视频的理解和推理能力。

Method: 1. 生成多个候选响应以发现低幻觉输出；2. 使用时序注意力崩溃分数评估每个响应的幻觉程度；3. 引入视觉注意力消失点实现高效幻觉估计与早期终止。

Result: 在VRIPT-HAL数据集上，Qwen2.5-VL-7B的幻觉降低10.59%；在VideoMMMU上性能提升最高达8.86%，解码成本显著降低。

Conclusion: SmartSight能有效提升开源视频大语言模型的可靠性，在减少幻觉的同时增强视频理解与推理能力。
...
### [136] [$M^3-Verse$: A "Spot the Difference" Challenge for Large Multimodal Models](https://arxiv.org/abs/2512.18735)
...
TL;DR: 本文提出了M³-Verse基准测试，用于评估大型多模态模型在共享空间环境中跟踪物体状态变化的能力，发现现有模型在此任务上存在局限，并提出了一种有效的基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在静态图像和单状态时空理解方面表现出色，但在两个不同视频观察之间理解共享空间环境中物体动态变化的能力尚未充分探索，这种空间推理能力对空间智能发展至关重要。

Method: 构建了包含270个场景和2932个问题的多模态、多状态、多维度的M³-Verse基准测试，基于室内场景状态变化前后的配对视频，包含50多个子任务和4项核心能力评估；评估了16个先进LMM，并提出了一种简单有效的多状态感知基线方法。

Result: 评估发现现有LMM在跟踪状态转换方面存在显著局限性；提出的基线方法在多状态感知任务上取得了显著的性能提升。

Conclusion: M³-Verse为催化下一代模型发展提供了具有挑战性的测试平台，推动模型对我们动态视觉世界形成更全面的理解。
...
### [137] [IPCV: Information-Preserving Compression for MLLM Visual Encoders](https://arxiv.org/abs/2512.18747)
*Yuan Chen,Zichen Wen,Yuzhou Wu,Xuyang Liu,Shuang Chen,Junpeng Ma,Weijia Li,Conghui He,Linfeng Zhang*

Main category: cs.CV

TL;DR: 提出IPCV框架，通过邻域引导重建和注意力稳定化技术，在MLLM视觉编码器中实现无训练的高效token剪枝，显著降低计算成本并保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM视觉token剪枝方法存在不足：LLM侧剪枝忽略ViT编码器开销，传统ViT剪枝缺乏语言引导可能丢失关键视觉信息并引入特征失真。

Method: 1. 邻域引导重建(NGR)：在ViT内部临时重建被剪枝token参与注意力计算后完全恢复；2. 注意力稳定化(AS)：近似被剪枝token的K/V以稳定注意力机制。

Result: IPCV显著降低端到端计算量，在多种图像和视频基准测试中优于现有无训练token压缩方法。

Conclusion: IPCV提供了一种高效且信息保留的MLLM视觉编码器压缩方案，可兼容现有LLM侧剪枝方法以提升性能。
...
### [138] [brat: Aligned Multi-View Embeddings for Brain MRI Analysis](https://arxiv.org/abs/2512.18679)
...
TL;DR: 提出brat框架，通过多视图表示学习将脑MRI与临床报告对齐，在多个任务上实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 脑MRI存在大量多样、细微且仅出现在少数切片的异常，现有数据集规模不足，需要更有效的多模态对齐方法。

Method: 构建约8万例3D扫描与报告配对的数据集；采用多视图预训练和隐式查询特征匹配机制；借鉴质量-多样性概念生成与报告句子对齐的多视图嵌入。

Result: 在多个视觉-语言和纯视觉任务上取得显著性能改进，模型已公开发布。

Conclusion: brat框架能有效学习脑MRI与临床报告的跨模态对齐表示，为医学影像分析提供新的基础模型。
...
### [139] [FedVideoMAE: Efficient Privacy-Preserving Federated Video Moderation](https://arxiv.org/abs/2512.18809)
*Ziyuan Tao,Chuanzhi Xu,Sandaru Jayawardana,Wei Bao,Kanchana Thilakarathna,Teng Joon Lim*

Main category: cs.CV

TL;DR: 提出一种基于设备端的联邦学习框架，用于视频暴力检测，结合自监督视频表示、参数高效适配和深度隐私保护，在降低通信成本的同时保护用户隐私。


<details>
  <summary>Details</summary>
Motivation: 短视频平台的快速增长带来了隐私保护审核的需求，云端处理存在隐私风险、高带宽成本和推理延迟等问题。

Method: 采用自监督VideoMAE表示、LoRA参数高效适配、DP-SGD差分隐私和安全聚合等技术，构建设备端联邦学习框架。

Result: 在RWF-2000数据集上，40个客户端达到77.25%准确率（无隐私保护），强差分隐私下为65-66%，通信成本比全模型联邦学习降低28.3倍。

Conclusion: 该框架在保持较高检测准确率的同时，显著降低了通信开销并提供了可配置的隐私保护，为设备端视频内容审核提供了可行方案。
...
### [140] [AsyncDiff: Asynchronous Timestep Conditioning for Enhanced Text-to-Image Diffusion Inference](https://arxiv.org/abs/2512.18675)
*Longhuan Xu,Feng Yin,Cunjian Chen*

Main category: cs.CV

TL;DR: 提出了一种异步推理机制，通过解耦潜在状态更新与去噪器条件化的时间步，使用轻量级时间步预测模块选择更合适的条件化时间步，从而在保持图像更新计划不变的情况下提升文本到图像生成的质量和细节控制。


<details>
  <summary>Details</summary>
Motivation: 传统的文本到图像扩散推理采用同步时间步调度，限制了去噪器条件化的灵活性。本研究旨在通过解耦潜在状态更新与去噪器条件化的时间步，使模型能够根据当前状态自适应选择更合适的噪声水平，以优化图像细节和纹理丰富度。

Method: 1. 提出异步推理机制，解耦潜在状态更新与去噪器条件化的时间步；2. 设计轻量级时间步预测模块（TPM），使用组相对策略优化（GRPO）进行训练；3. 在部署时通过超参数插值原始与解耦时间步，实现保守或激进的调整；4. 在SD3.5（15步）和Flux（10步）上限制推理步数以控制计算成本。

Result: 在Stable Diffusion 3.5 Medium和Flux.1-dev模型上，使用MS-COCO 2014和T2I-CompBench数据集进行评估，优化了综合奖励（包括Image Reward、HPSv2、CLIP Score和Pick Score的平均值），结果显示该方法能持续提升生成图像的质量。

Conclusion: 异步推理机制通过灵活选择去噪器条件化时间步，有效提升了文本到图像生成的性能，同时保持了计算效率，为扩散模型推理提供了新的优化方向。
...
### [141] [CrashChat: A Multimodal Large Language Model for Multitask Traffic Crash Video Analysis](https://arxiv.org/abs/2512.18878)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型无法在统一框架内完成碰撞识别、时间定位和高级视频理解等多任务分析，且缺乏有效的多任务训练策略，阻碍了驾驶视频数据在交通安全研究中的自动化应用。

Method: 基于VideoLLaMA3构建多模态大语言模型，通过指令微调获取领域知识，并采用任务解耦与分组的多任务学习策略，以最大化组内和跨组联合学习效果并减轻负迁移。

Result: 在公开数据集上，CrashChat在碰撞识别达到接近完美准确率，碰撞定位提升176%，预碰撞定位提升40%；在描述和推理任务中，BLEU和ROUGE分数分别提升0.18-0.41和0.18-0.42，全面超越现有MLLM和传统视觉方法。

Conclusion: CrashChat是一个性能强大、端到端且便于实际部署的碰撞分析工具，其数据集和代码已开源，为自动驾驶责任认定和交通安全研究提供了有效解决方案。
...
### [142] [A Study of Finetuning Video Transformers for Multi-view Geometry Tasks](https://arxiv.org/abs/2512.18684)
*Huimin Wu,Kwang-Ting Cheng,Stephen Lin,Zhirong Wu*

Main category: cs.CV

TL;DR: 本文研究发现，通过微调视频基础模型，可以将其直接迁移到多视图几何任务（如光流估计）中，无需定制架构或任务特定预训练，仅需添加线性解码器即可获得优异性能。


<details>
  <summary>Details</summary>
Motivation: 传统多视图几何方法通常需要定制化架构设计和任务特定预训练，过程复杂且泛化能力有限。本研究旨在探索通用视频预训练模型是否可直接用于几何视觉任务，简化流程并提升泛化性能。

Method: 采用视频预训练的视觉Transformer作为主干网络，仅附加线性解码器进行微调，并引入迭代细化机制进一步提升性能。该方法避免复杂架构修改，直接利用模型已学习的时空注意力机制进行几何推理。

Result: 在光流估计任务中，Sintel clean、Sintel final和KITTI数据集的端点误差（EPE）分别达到0.69、1.78和3.15，在线测试基准创下新记录（EPE 0.79/1.88，F1 3.79）。在3D深度估计和立体匹配任务中也表现出强大性能。

Conclusion: 通用视频预训练模型通过简单微调即可高效解决多视图几何问题，其学习的时空注意力机制具有强大的几何推理能力，为几何视觉任务提供了简洁而有效的解决方案。
...
### [143] [EcoSplat: Efficiency-controllable Feed-forward 3D Gaussian Splatting from Multi-view Images](https://arxiv.org/abs/2512.18692)
*Jongmin Park,Minh-Quan Viet Bui,Juan Luis Gonzalez Bello,Jaeho Moon,Jihyong Oh,Munchurl Kim*

Main category: cs.CV

TL;DR: 提出EcoSplat框架，首个效率可控的前馈式3D高斯泼溅方法，能根据目标基元数量自适应预测3D表示，在密集视图场景中实现高效重建


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D高斯泼溅方法在密集视图设置中会产生过多基元，且无法控制预测高斯数量，限制了实际应用灵活性

Method: 采用两阶段优化：1）像素对齐高斯训练阶段学习初始基元预测；2）重要性感知高斯微调阶段学习基元排序并根据目标数量自适应调整参数

Result: 在多个密集视图设置下的实验表明，EcoSplat在严格基元数量约束下优于现有方法，且具有鲁棒性

Conclusion: EcoSplat框架适用于灵活的下游渲染任务，实现了效率可控的3D场景重建
...
### [144] [LouvreSAE: Sparse Autoencoders for Interpretable and Controllable Style Transfer](https://arxiv.org/abs/2512.18930)
*Raina Panda,Daniel Fein,Arpita Singhal,Mark Fiore,Maneesh Agrawala,Matyas Bohacek*

Main category: cs.CV

TL;DR: 提出LouvreSAE方法，利用艺术专用稀疏自编码器从生成模型潜在嵌入中学习解耦的风格概念，实现无需微调或优化的快速艺术风格迁移。


<details>
  <summary>Details</summary>
Motivation: 现有艺术风格迁移方法通常需要模型微调、添加适配器或提示工程，计算成本高且风格与内容易纠缠，需要更轻量、可解释的解决方案。

Method: 在生成图像模型的潜在嵌入上训练艺术专用稀疏自编码器(SAE)，学习解耦的风格概念（笔触、纹理、色彩）和构图概念，构建可分解的风格引导向量。

Result: 在ArtBench10数据集上验证，风格评估指标（VGG风格损失和CLIP风格分数）达到或超越现有方法，速度提升1.7-20倍，且具有可解释性。

Conclusion: LouvreSAE提供了一种无需训练更新、无需额外推理的轻量级风格迁移方法，仅需少量参考图像即可实现高效、可解释的艺术风格控制。
...
### [145] [Rectification Reimagined: A Unified Mamba Model for Image Correction and Rectangling with Prompts](https://arxiv.org/abs/2512.18718)
*Linwei Qiu,Gongzhe Li,Xiaozhe Zhang,Qinlin Sun,Fengying Xie*

Main category: cs.CV

TL;DR: 提出统一矫正框架UniRect，通过模拟不同镜头类型将多种图像校正任务统一到一致的畸变矫正视角，采用任务无关的双组件结构和稀疏专家混合机制，在多项任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的图像校正方法主要依赖任务特定架构，泛化能力受限，难以有效应用于广泛的不同任务。

Method: 1. 建立通用畸变模型统一多种任务；2. 设计任务无关的双组件框架：变形模块（使用RP-TPS处理几何变形）和恢复模块（使用RMB块提升图像保真度）；3. 引入稀疏专家混合结构缓解多任务学习中的任务竞争。

Result: 大量实验表明，该方法在多项图像校正任务上相比现有最新方法取得了最先进的性能。

Conclusion: UniRect框架通过统一视角和任务无关设计，有效提升了图像校正任务的泛化能力和性能，为实际摄影系统提供了更通用的解决方案。
...
### [146] [Self-Attention with State-Object Weighted Combination for Compositional Zero Shot Learning](https://arxiv.org/abs/2512.18969)
*Cheng-Hong Chang,Pei-Hsuan Tsai*

Main category: cs.CV

TL;DR: 本文提出SASOW方法，通过引入自注意力机制和状态-对象权重考量，提升了组合零样本学习中未见组合的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 现有物体识别技术大多仅识别物体本身而忽略其状态，组合零样本学习虽能解决状态-物体组合识别问题，但当前最优方法KG-SP在识别准确率和组合权重考量上存在不足。

Method: 1. 在状态和物体分类器中引入自注意力机制提升识别精度；2. 在组合过程中加入状态和物体的权重评估，生成更合理的组合。

Result: 在MIT-States、UT Zappos和C-GQA三个基准数据集上，SASOW相比KG-SP在未见组合识别准确率分别提升2.1%、1.7%和0.4%。

Conclusion: SASOW通过改进分类器结构和组合权重机制，有效提升了组合零样本学习的性能，为状态-物体联合识别提供了更优解决方案。
...
### [147] [Breast Cancer Recurrence Risk Prediction Based on Multiple Instance Learning](https://arxiv.org/abs/2512.18734)
*Jinqiu Chen,Huyan Xu*

Main category: cs.CV

TL;DR: 本研究开发了三种基于多示例学习的深度学习模型，利用常规H&E染色全切片图像预测乳腺癌5年复发风险，最佳模型AUC达0.836，准确率76.2%。


<details>
  <summary>Details</summary>
Motivation: 预测乳腺癌复发风险是临床关键挑战，需要探索基于常规病理切片的自动化、低成本风险分层方法。

Method: 使用210例患者数据，比较CLAM-SB、ABMIL和ConvNeXt-MIL-XGBoost三种MIL框架，以21基因复发评分为金标准，采用UNI和CONCH预训练模型提取特征，进行5折交叉验证。

Result: 改进的CLAM-SB模型表现最佳，平均AUC为0.836，分类准确率76.2%，成功将患者分为低、中、高三个风险等级。

Conclusion: 深度学习结合常规组织学切片可实现与基因组学相关的自动化风险分层，为快速、经济高效的临床决策支持提供了可行路径。
...
### [148] [ICP-4D: Bridging Iterative Closest Point and LiDAR Panoptic Segmentation](https://arxiv.org/abs/2512.18991)
*Gyeongrok Oh,Youngdong Jang,Jonghyun Choi,Suk-Ju Kang,Guang Lin,Sangpil Kim*

Main category: cs.CV

TL;DR: 提出ICP-4D框架，无需训练即可实现4D激光雷达全景分割，通过几何关系关联时空实例，在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有4D激光雷达全景分割方法需要大量点云训练或设计复杂关联模块，计算成本高且忽略点云固有几何先验。

Method: 使用迭代最近点算法直接关联时序实例，引入基于Sinkhorn的软匹配处理噪声预测，设计静态、动态、缺失三类实例处理流程。

Result: 在SemanticKITTI和nuScenes数据集上均超越最先进方法，无需额外训练或点云输入。

Conclusion: 几何驱动的免训练框架能高效实现4D全景分割，证明几何先验在时空关联中的重要性。
...
### [149] [AMLID: An Adaptive Multispectral Landmine Identification Dataset for Drone-Based Detection](https://arxiv.org/abs/2512.18738)
*James E. Gallagher,Edward J. Oughton*

Main category: cs.CV

TL;DR: 本文介绍了首个用于无人机地雷检测的开源多光谱数据集AMLID，包含RGB和LWIR图像，涵盖多种地雷类型和环境条件，旨在推动人道主义排雷研究。


<details>
  <summary>Details</summary>
Motivation: 地雷造成严重人道主义威胁，现有检测方法危险、低效且昂贵，缺乏公开数据集限制了自适应检测算法的发展。

Method: 创建AMLID数据集，包含12,078张标注图像，涵盖21种地雷类型，结合RGB和LWIR图像，包含11种融合级别、4种传感器高度、2个季节和3种光照条件。

Result: AMLID成为首个开源的多光谱地雷检测数据集，提供全面的环境变量覆盖，支持算法开发和基准测试。

Conclusion: AMLID数据集使研究人员无需接触实弹或昂贵设备即可开发检测算法，有助于民主化人道主义排雷研究。
...
### [150] [Finer-Personalization Rank: Fine-Grained Retrieval Examines Identity Preservation for Personalized Generation](https://arxiv.org/abs/2512.19026)
*Connor Kilrain,David Carlyn,Julia Chae,Sara Beery,Wei-Lun Chao,Jianyang Gu*

Main category: cs.CV

TL;DR: 提出Finer-Personalization Rank评估协议，通过检索排名方式评估个性化生成模型的身份保持能力，优于现有语义相似度指标。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型评估指标过于关注整体语义相似度，忽略了身份相关的细粒度判别细节，需要更精准的身份保持评估方法。

Method: 采用检索排名视角：将生成图像作为查询，在包含视觉相似真实图像的身份标注图库中进行检索，使用平均精度均值等检索指标评估性能。

Result: 在CUB、Stanford Cars和动物重识别基准测试中，该方法比纯语义指标更能反映身份保持，并揭示多个流行个性化方法存在显著身份漂移。

Conclusion: 基于图库的评估协议为个性化生成提供了原则性且实用的评估框架，能更准确衡量身份细节保留程度。
...
### [151] [Memorize-and-Generate: Towards Long-Term Consistency in Real-Time Video Generation](https://arxiv.org/abs/2512.18741)
*Tianrui Zhu,Shiyi Zhang,Zhirui Sun,Jingqi Tian,Yansong Tang*

Main category: cs.CV

TL;DR: 提出MAG框架，通过解耦记忆压缩与帧生成任务，解决长视频生成中历史上下文遗忘与内存开销的矛盾，提升场景一致性。


<details>
  <summary>Details</summary>
Motivation: 现有帧级自回归模型在长视频生成中面临两难：使用窗口注意力会丢弃历史上下文导致场景不一致，而保留完整历史则内存成本过高。

Method: 提出MAG框架，包含两个独立模型：1) 记忆模型压缩历史信息为紧凑KV缓存；2) 生成模型利用压缩表示合成后续帧。同时构建MAG-Bench严格评估历史记忆保留能力。

Result: 实验表明MAG在保持标准视频生成基准竞争力的同时，显著提升了历史场景一致性。

Conclusion: MAG通过解耦记忆与生成任务，有效平衡了长视频生成中的记忆保留与计算效率，为交互式世界模型等应用提供了更可靠的框架。
...
### [152] [Context-Aware Network Based on Multi-scale Spatio-temporal Attention for Action Recognition in Videos](https://arxiv.org/abs/2512.18750)
*Xiaoyang Li,Wenzhu Yang,Kanglin Wang,Tiebiao Wang,Qingsong Fei*

Main category: cs.CV

TL;DR: 提出上下文感知网络（CAN），通过多尺度时空线索模块提升动作识别性能，在多个基准数据集上取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 现有动作识别方法常忽视动作的多粒度特性，未能充分捕捉不同尺度的时空线索。

Method: 设计CAN网络，包含多尺度时间线索模块（MTCM）提取多尺度时间特征，以及分组空间线索模块（GSCM）通过特征图分组提取多尺度空间特征。

Result: 在五个数据集上验证：Something-Something V1（50.4%）、V2（63.9%）、Diving48（88.4%）、Kinetics-400（74.9%）、UCF101（86.9%），性能优于主流方法。

Conclusion: 捕捉多尺度时空线索对鲁棒的动作识别至关重要，CAN网络能有效建模动作的多粒度特性。
...
### [153] [Application of deep learning approaches for medieval historical documents transcription](https://arxiv.org/abs/2512.18865)
*Maksym Voloshchuk,Bohdana Zarembovska,Mykola Kozlenko*

Main category: cs.CV

TL;DR: 提出一种深度学习方法来处理9至11世纪拉丁语手写文献的文本提取，针对中世纪文献特性优化，涵盖数据集构建、模型流程及性能评估。


<details>
  <summary>Details</summary>
Motivation: 现有手写文本识别和光学字符识别方法对现代数据效果良好，但在处理中世纪拉丁语文献时效率显著下降，需针对其独特属性开发专用解决方案。

Method: 构建中世纪文献数据集，采用深度学习流水线：从文档图像中检测对象，通过分类模型和词图像嵌入进行单词识别，结合解释性数据分析。

Result: 报告了召回率、精确率、F1分数、交并比、混淆矩阵和平均字符串距离等指标，并提供相关图表，代码已在GitHub开源。

Conclusion: 该方法能有效提取中世纪拉丁语手写文献的文本信息，为历史文献转录领域提供了针对性的技术方案。
...
### [154] [Towards Minimal Fine-Tuning of VLMs](https://arxiv.org/abs/2512.19219)
*Tiange Luo,Lajanugen Logeswaran,Jaekyeom Kim,Justin Johnson,Honglak Lee*

Main category: cs.CV

TL;DR: 本文提出了Image-LoRA，一种用于视觉语言模型的高效微调方法，通过在视觉标记的注意力层值路径上应用低秩适配，并选择性地微调部分注意力头，实现了与标准LoRA相当的精度但参数和计算量更少。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法在视觉语言模型中计算开销较大，尤其是在处理大量视觉标记时。需要一种既能保持模型性能又能显著降低计算成本的轻量级适配方法。

Method: 1. 仅在视觉标记跨度的注意力层值路径上应用低秩适配；2. 使用基于rank-1 Image-LoRA估计的头影响力分数选择部分注意力头进行适配；3. 通过选择大小归一化稳定每层更新。

Result: 在涵盖文本密集到图像密集场景的屏幕中心接地和指代表征基准测试中，Image-LoRA达到或接近标准LoRA的精度，同时使用更少的可训练参数和更低的适配器训练FLOPs。在GSM8K上进一步证明该方法能保持纯文本推理性能。

Conclusion: Image-LoRA提供了一种高效且有效的视觉语言模型微调方案，在保持性能的同时显著降低了计算成本，并保持了模型原有的文本推理能力。
...
### [155] [From Pixels to Predicates Structuring urban perception with scene graphs](https://arxiv.org/abs/2512.19221)
*Yunlong Liu,Shuyang Li,Pengyuan Liu,Yu Zhang,Rudi Stouffs*

Main category: cs.CV

TL;DR: 本研究提出了一种将街景图像转换为结构化图表示的三阶段流程，用于预测六种感知指标，相比基线模型平均准确率提升26%，并展示了良好的跨城市泛化能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有感知研究多依赖像素特征或物体共现统计，忽略了影响人类感知的显式关系结构，需要更结构化、可解释的建模方法。

Method: 三阶段流程：1) 使用开放集全景场景图模型提取物体-谓词-物体三元组；2) 通过异构图自编码器学习紧凑场景嵌入；3) 用神经网络从嵌入预测感知分数。

Result: 方法在感知预测准确率上比基线平均提升26%，在跨城市预测任务中保持强泛化性能，并能识别导致低感知分数的关系模式（如墙上涂鸦、人行道停车）。

Conclusion: 基于图的结构化表示为城市感知建模提供了表达力强、可泛化且可解释的信号，推动了以人为本、情境感知的城市分析。
...
### [156] [MaskFocus: Focusing Policy Optimization on Critical Steps for Masked Image Generation](https://arxiv.org/abs/2512.18766)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在语言模型和自回归视觉生成模型中已显示潜力，但难以适应掩码生成模型，因为策略优化需要计算整个采样轨迹的概率似然，计算成本高且随机步骤优化效果不佳

Method: 1. 通过测量中间图像与最终生成图像的相似度确定步骤级信息增益；2. 识别最关键步骤并执行聚焦策略优化；3. 基于熵设计动态路由采样机制，鼓励模型探索低熵样本的更有价值掩码策略

Result: 在多个文本到图像基准测试上的广泛实验验证了该方法的有效性

Conclusion: MaskFocus框架成功解决了掩码生成模型中强化学习优化的挑战，通过聚焦关键步骤和动态路由采样实现了高效且有效的策略优化
...
### [157] [InSight-o3: Empowering Multimodal Foundation Models with Generalized Visual Search](https://arxiv.org/abs/2512.18745)
*Kaican Li,Lewei Yao,Jiannan Wu,Tiezheng Yu,Jierun Chen,Haoli Bai,Lu Hou,Lanqing Hong,Wei Zhang,Nevin L. Zhang*

Main category: cs.CV

TL;DR: 本文提出O3-Bench基准测试和InSight-o3多智能体框架，旨在提升多模态AI在复杂视觉推理任务上的表现，特别是针对需要跨图像区域整合细节的任务。


<details>
  <summary>Details</summary>
Motivation: 当前开源多模态智能体在视觉推理方面存在不足，难以处理现实世界中需要结合视觉细节进行多步推理的任务（如分析含密集图表/地图的文档）。

Method: 1. 构建O3-Bench基准测试，包含需要跨图像区域整合细节的挑战性问题；2. 提出InSight-o3多智能体框架，包含视觉推理智能体（vReasoner）和视觉搜索智能体（vSearcher）；3. 针对vSearcher提出广义视觉搜索任务，并通过强化学习训练专用多模态大语言模型。

Result: 1. O3-Bench对前沿系统（如OpenAI o3）极具挑战性（准确率仅40.8%）；2. InSight-o3作为即插即用组件，显著提升了多种前沿多模态模型在多个基准测试上的性能。

Conclusion: 该研究通过新基准测试和多智能体框架推进了开源多模态系统的视觉推理能力，为实现强大类o3系统迈出重要一步。
...
### [158] [Watch Closely: Mitigating Object Hallucinations in Large Vision-Language Models with Disentangled Decoding](https://arxiv.org/abs/2512.19070)
*Ruiqi Ma,Yu Yan,Chunhong Zhang,Minghao Yin,XinChao Liu,Zhihong Jin,Zheng Hu*

Main category: cs.CV

TL;DR: 提出无需训练的幻觉解耦解码方法，通过图像分割和增强来缓解大视觉语言模型在物体识别中的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在物体识别任务中存在严重幻觉问题，现有方法主要关注语言模态，需要同时缓解视觉和语言模态的幻觉

Method: 提出幻觉解耦解码方法，包括图像分割、选择增强图像、使用空白图像消除语言先验幻觉

Result: 该方法减少了模型对语言先验的依赖，提升了视觉性能

Conclusion: HDD方法能有效缓解大视觉语言模型在视觉和语言模态中的幻觉问题
...
### [159] [Is Visual Realism Enough? Evaluating Gait Biometric Fidelity in Generative AI Human Animation](https://arxiv.org/abs/2512.19275)
*Ivan DeAndres-Tame,Chengwei Ye,Ruben Tolosana,Ruben Vera-Rodriguez,Shiqi Yu*

Main category: cs.CV

TL;DR: 评估生成式AI模型在人类动画中保持步态生物特征的能力，发现尽管视觉质量高，但身份识别保真度低，模型难以分离身份与运动模式。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在人类动画合成中视觉保真度高，但细微运动模式易失真，影响基于行为生物特征（如步态）的身份识别可靠性。

Method: 评估四种生成式AI模型在两项任务中的表现：1）从参考视频恢复不同复杂度下的步态模式；2）将步态模式迁移至不同视觉身份。

Result: 模型视觉质量较高，但身份识别任务中生物特征保真度低；身份迁移任务揭示外观依赖问题——分离纹理与运动后识别失效。

Conclusion: 当前生成式AI模型依赖视觉属性而非时间动态特征，难以解耦身份与运动，限制了其在步态生物识别中的应用。
...
### [160] [In-Context Audio Control of Video Diffusion Transformers](https://arxiv.org/abs/2512.18772)
*Wenze Liu,Weicai Ye,Minghong Cai,Quande Liu,Xintao Wang,Xiangyu Yue*

Main category: cs.CV

TL;DR: 本文提出ICAC框架，首次在统一的视频扩散Transformer中探索音频条件注入，通过掩码3D注意力机制解决训练难题，实现基于音频和参考图像的语音驱动视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成统一模型主要关注文本、图像等模态，而严格时间同步的音频信号未被充分探索，限制了语音驱动视频生成的发展。

Method: 系统研究三种音频注入机制：标准交叉注意力、2D自注意力和统一3D自注意力；提出掩码3D注意力机制，通过约束注意力模式强制时间对齐，实现稳定训练。

Result: 掩码3D注意力机制能稳定训练并实现最佳性能，在音频流和参考图像条件下生成具有强唇部同步和高质量的视频。

Conclusion: 3D注意力能有效捕捉时空视听相关性但训练困难，掩码3D注意力机制通过强制时间对齐解决了这一挑战，为音频条件视频生成提供了有效框架。
...
### [161] [Eff-GRot: Efficient and Generalizable Rotation Estimation with Transformers](https://arxiv.org/abs/2512.18784)
*Fanis Mathioulakis,Gorjan Radevski,Tinne Tuytelaars*

Main category: cs.CV

TL;DR: Eff-GRot是一种从RGB图像进行高效且可泛化的旋转估计方法，通过单次前向传播直接预测物体旋转，无需特定物体或类别的训练。


<details>
  <summary>Details</summary>
Motivation: 现有旋转估计方法通常需要特定物体或类别的训练，计算效率较低，难以满足延迟敏感应用的需求。

Method: 使用transformer在潜在空间中进行比较，联合处理多个参考图像的旋转感知表示和查询图像，实现端到端的旋转估计。

Result: 实验结果表明Eff-GRot在准确性和计算效率之间取得了良好平衡，特别适用于延迟敏感的应用场景。
...
### [162] [MixFlow Training: Alleviating Exposure Bias with Slowed Interpolation Mixture](https://arxiv.org/abs/2512.19311)
*Hui Li,Jiayue Lyu,Fu-Yun Wang,Kaihui Cheng,Siyu Zhu,Jingdong Wang*

Main category: cs.CV

TL;DR: 本文提出MixFlow方法解决扩散模型中的训练-测试差异问题，通过利用慢流现象，在训练后阶段使用减缓时间步的插值混合来改进预测网络，在图像生成任务上取得了优异效果。


<details>
  <summary>Details</summary>
Motivation: 扩散模型存在训练-测试差异问题：训练时使用真实噪声数据插值作为输入，测试时使用生成噪声数据作为输入，这种差异影响了模型性能。

Method: 提出MixFlow训练方法，基于观察到的慢流现象——生成噪声数据对应的最近真实插值来自更高噪声的时间步。方法利用减缓时间步的插值混合对每个训练时间步的预测网络进行后训练。

Result: 在类别条件图像生成和文生图任务上验证了有效性。RAE模型在ImageNet上取得优异结果：256×256分辨率下无引导FID 1.43、有引导1.10；512×512分辨率下无引导FID 1.55、有引导1.10。

Conclusion: MixFlow方法能有效缓解扩散模型的训练-测试差异问题，提升生成质量，在多个基准测试中表现出色。
...
### [163] [Revealing Perception and Generation Dynamics in LVLMs: Mitigating Hallucinations via Validated Dominance Correction](https://arxiv.org/abs/2512.18813)
*Guangtao Lyu,Xinyi Cheng,Chenghao Xu,Qi Liu,Muli Yang,Fen Fang,Huilin Chen,Jiexi Yan,Xu Yang,Cheng Deng*

Main category: cs.CV

TL;DR: 本文系统分析了大型视觉语言模型（LVLMs）的内部工作机制，揭示了感知的三阶段GATE过程和生成的SAD模式，并提出了VDC策略来有效减少幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然表现出色，但幻觉问题仍然持续存在，需要深入理解其内部工作机制以找到有效的缓解方法。

Method: 通过分析LVLMs内部视觉感知和token生成的演化过程，提出了GATE三阶段感知模式和SAD生成模式，并设计了VDC（验证主导校正）策略来检测和替换无支持的token。

Result: 实验表明，VDC策略在多个模型和基准测试中显著减少了幻觉现象，提高了输出可靠性。

Conclusion: 理解LVLMs的内部工作机制对于缓解幻觉问题至关重要，VDC策略通过校正无支持的token有效提升了模型的可靠性。
...
### [164] [Tempo as the Stable Cue: Hierarchical Mixture of Tempo and Beat Experts for Music to 3D Dance Generation](https://arxiv.org/abs/2512.18804)
*Guangtao Lyu,Chenghao Xu,Qi Liu,Jiexi Yan,Muli Yang,Fen Fang,Cheng Deng*

Main category: cs.CV

TL;DR: 提出TempoMoE方法，通过分层节奏感知的专家混合模块增强扩散模型，实现无人工流派标签的音乐到3D舞蹈生成，在节奏对齐和舞蹈质量上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖额外流派标签改进舞蹈生成，但这些标签通常噪声大、粗糙、不可用或不足以捕捉真实音乐的多样性，导致节奏错位或风格漂移。而节奏（tempo）作为反映音乐节奏和速度的核心属性，在不同数据集和流派中相对一致。

Method: 提出TempoMoE：1）将运动专家按节奏范围组织成节奏结构化组；2）多尺度节拍专家捕捉细粒度和长程节奏动态；3）分层节奏自适应路由根据音乐特征动态选择和融合专家。

Result: 大量实验表明，TempoMoE在舞蹈质量和节奏对齐方面取得了最先进的结果，无需手动流派标签即可实现灵活、节奏对齐的生成。

Conclusion: 基于节奏一致性的TempoMoE方法有效解决了现有方法对流派标签的依赖问题，通过分层节奏感知机制显著提升了音乐到3D舞蹈生成的节奏对齐和真实感。
...
### [165] [MT-Mark: Rethinking Image Watermarking via Mutual-Teacher Collaboration with Adaptive Feature Modulation](https://arxiv.org/abs/2512.19438)
*Fei Ge,Ying Huang,Jie Liu,Guixuan Zhang,Zhi Zeng,Shuwu Zhang,Hu Guan*

Main category: cs.CV

TL;DR: 提出了一种显式协作的图像水印框架，通过协作交互机制和自适应特征调制模块，使嵌入器和提取器在训练中双向通信，提高了水印提取精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度图像水印方法采用固定的嵌入-提取流程，嵌入器和提取器之间缺乏显式协作机制，无法在训练中相互指导，限制了性能提升。

Method: 1. 提出协作交互机制（CIM），建立嵌入器和提取器的双向通信；2. 设计自适应特征调制模块（AFMM），通过解耦调制结构和强度实现内容感知的特征调节；3. 采用互教师训练范式实现协同优化。

Result: 在真实世界和AI生成数据集上的实验表明，该方法在保持高感知质量的同时，水印提取准确率持续优于现有最优方法，表现出强鲁棒性和泛化能力。

Conclusion: 通过架构层面的重新设计，将水印嵌入和提取重构为显式协作组件，使鲁棒性从协同表示学习中自然涌现，而非依赖失真模拟，为深度水印系统提供了新范式。
...
### [166] [DSTED: Decoupling Temporal Stabilization and Discriminative Enhancement for Surgical Workflow Recognition](https://arxiv.org/abs/2512.19387)
*Yueyao Chen,Kai-Ni Wang,Dario Tayupo,Arnaud Huaulm'e,Krystel Nyangoh Timoh,Pierre Jannin,Qi Dou*

Main category: cs.CV

TL;DR: 提出双路径框架DSTED用于手术工作流识别，通过可靠记忆传播和不确定性感知原型检索解决预测抖动和模糊阶段区分问题，在AutoLaparo-hysterectomy数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有手术工作流识别方法存在两个关键挑战：连续帧间的预测抖动和模糊阶段的区分困难，需要开发稳定框架来提升临床适用性。

Method: 提出DSTED双路径框架：1) 可靠记忆传播(RMP)通过多标准可靠性评估过滤融合高置信历史特征保持时序连贯性；2) 不确定性感知原型检索(UPR)从高不确定性样本构建可学习类别原型并通过自适应原型匹配优化模糊帧表示；3) 置信驱动门控动态平衡两条路径。

Result: 在AutoLaparo-hysterectomy数据集上达到84.36%准确率和65.51% F1分数，分别超过次优方法3.51%和4.88%；消融实验显示RMP贡献2.19%提升，UPR贡献1.93%提升，组合产生协同效应；显著减少时序抖动并改善困难阶段过渡。

Conclusion: 双路径设计为稳定工作流识别提供了新范式，证明将时序一致性和阶段模糊性建模解耦可获得更优性能和临床适用性。
...
### [167] [CASA: Cross-Attention via Self-Attention for Efficient Vision-Language Fusion](https://arxiv.org/abs/2512.19535)
*Moritz Böhle,Amélie Royer,Juliette Marrie,Edouard Grave,Patrick Pérez*

Main category: cs.CV

TL;DR: 提出CASA方法，通过自注意力增强交叉注意力层中的局部文本-文本交互，显著提升跨注意力视觉语言模型在细粒度视觉任务上的性能，同时保持其在长上下文多模态任务中的可扩展性优势。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型中，完全插入图像标记的方法计算成本高昂，而使用交叉注意力的方法虽高效但性能存在差距，特别是在涉及细粒度视觉细节的任务上。

Method: 提出CASA（Cross-Attention via Self-Attention）范式，在专用交叉注意力层中启用局部文本-文本交互，结合自注意力机制增强跨注意力模型的能力。

Result: CASA显著缩小了与完全标记插入方法在常见图像理解基准上的性能差距，同时在流媒体视频字幕等长上下文多模态任务中保持了与交叉注意力模型相同的可扩展性。

Conclusion: CASA是一种简单高效的视觉语言模型训练范式，通过增强交叉注意力层中的局部文本交互，在保持计算效率的同时提升了细粒度视觉理解性能。
...
### [168] [Anatomy-R1: Enhancing Anatomy Reasoning in Multimodal Large Language Models via Anatomical Similarity Curriculum and Group Diversity Augmentation](https://arxiv.org/abs/2512.19512)
*Ziyang Song,Zelin Zang,Zuyao Chen,Xusheng Liang,Dong Yi,Jinlin Wu,Hongbin Liu,Jiebo Luo*

Main category: cs.CV

TL;DR: 本文提出两种新方法（解剖相似性课程学习和群体多样性问题增强）来提升多模态大语言模型在医学影像解剖识别任务中的推理能力，解决了现有方法在知识共享和推理路径多样性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在自然图像推理方面进展显著，但在医学影像领域，尤其是临床解剖手术图像中应用不足。解剖理解任务需要精确且临床一致的答案，但由于医学数据复杂性和高质量专家标注稀缺，传统监督微调策略效果有限。现有GRPO方法在解剖识别中存在知识共享不均和推理路径单一的问题，阻碍了模型性能。

Method: 1. 解剖相似性课程学习：通过控制答案选项的相似度来渐进式调整问题难度，使模型逐步掌握复杂问题。2. 群体多样性问题增强：通过问题扩增来扩展模型对困难查询的搜索空间，减少回答趋同倾向。

Result: 在SGG-VQA和OmniMedVQA基准测试上的综合实验表明，该方法在两个基准上均取得显著提升，有效增强了多模态大语言模型的医学推理能力。

Conclusion: 所提出的方法通过渐进式学习和问题多样性增强，克服了现有方法在医学解剖识别中的局限性，显著提升了多模态大语言模型在复杂医学影像任务中的推理性能。
...
### [169] [EchoMotion: Unified Human Video and Motion Generation via Dual-Modality Diffusion Transformer](https://arxiv.org/abs/2512.18814)
*Yuxiao Yang,Hualian Sheng,Sijia Cai,Jing Lin,Jiahao Wang,Bing Deng,Junzhe Lu,Haoqian Wang,Jieping Ye*

Main category: cs.CV

TL;DR: 提出EchoMotion框架，通过联合建模外观与人体运动分布，提升复杂人体动作视频生成质量，包含双分支架构、MVS-RoPE位置编码和两阶段训练策略，并构建HuMoVe数据集验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在合成复杂人体动作时存在困难，主要因为像素级训练目标偏向外观保真度，而忽略了对运动学原理的学习，导致人体关节自由度高的动作生成质量不足。

Method: 1. 扩展DiT框架为双分支架构，联合处理多模态拼接token；2. 提出MVS-RoPE统一位置编码，为视频和运动token提供同步坐标系；3. 设计运动-视频两阶段训练策略；4. 构建HuMoVe数据集（约8万高质量视频-运动对）。

Result: 实验表明，显式表征人体运动与外观信息互补，显著提升了人体中心视频生成的连贯性和合理性，模型能够联合生成复杂人体动作视频及对应运动序列，并支持跨模态条件生成任务。

Conclusion: 通过联合建模外观与运动分布，并引入同步多模态位置编码，能够有效解决复杂人体动作生成难题，为视频生成提供了更全面的运动学先验。
...
### [170] [Brain-Gen: Towards Interpreting Neural Signals for Stimulus Reconstruction Using Transformers and Latent Diffusion Models](https://arxiv.org/abs/2512.18843)
...
<details>
  <summary>Details</summary>
Motivation: 现有脑活动解码方法受EEG信号高噪声、空间扩散和时间变异性限制，神经表征可解释性不足，需开发能解释思维神经机制的新方法。

Method: 1. 基于Transformer提取EEG信号的时空表征；2. 将特征融入潜在扩散模型的注意力机制；3. 从脑活动重建视觉刺激。

Result: 在公开数据集上：潜在空间聚类准确率提升6.5%；零样本泛化能力提升11.8%；Inception Score和FID分数与基线相当。

Conclusion: 该方法显著提升EEG信号的语义建模能力，是向通用化脑电信号语义解释迈出的重要一步。
...
### [171] [BabyFlow: 3D modeling of realistic and expressive infant faces](https://arxiv.org/abs/2512.19560)
*Antonia Alomar,Mireia Masias,Marius George Linguraru,Federico M. Sukno,Gemma Piella*

Main category: cs.CV

TL;DR: 提出BabyFlow生成模型，通过解耦婴儿面部身份与表情，实现独立控制，提升3D重建精度并支持表情合成与修改。


<details>
  <summary>Details</summary>
Motivation: 婴儿发育障碍的早期检测需要分析颅面形态，但婴儿面部建模面临数据稀缺和表情多变等挑战。

Method: 采用标准化流学习概率表示，通过跨年龄表情迁移（从成人3D扫描中迁移表情）扩充婴儿数据集，并与扩散模型结合生成2D图像。

Result: BabyFlow显著提高了嘴、眼、鼻等高表情区域的3D重建精度，能合成逼真的婴儿表情并保持身份一致性。

Conclusion: 该模型为婴儿面部数据增强和早期分析提供了有效工具，有助于发育障碍的早期检测。
...
### [172] [VizDefender: Unmasking Visualization Tampering through Proactive Localization and Intent Inference](https://arxiv.org/abs/2512.18853)
*Sicheng Song,Yanjie Zhang,Zixin Chen,Huamin Qu,Changbo Wang,Chenhui Li*

Main category: cs.CV

TL;DR: 提出VizDefender框架，通过半脆弱水印和多模态大语言模型检测和分析数据可视化图像篡改，保护可视化完整性。


<details>
  <summary>Details</summary>
Motivation: 数据可视化图像面临日益严重的篡改威胁，现有编辑技术可实现隐蔽的欺骗性修改，需要系统化的检测和意图分析方法。

Method: 1) 半脆弱水印模块：嵌入位置图以精确定位篡改区域；2) 意图分析模块：利用多模态大语言模型解释篡改意图和误导效果。

Result: 通过广泛评估和用户研究验证了方法的有效性，能够准确检测篡改并分析攻击者意图。

Conclusion: VizDefender框架为数据可视化篡改检测提供了有效解决方案，结合水印技术和AI模型分析，增强了可视化数据的可信度。
...
### [173] [MapTrace: Scalable Data Generation for Route Tracing on Maps](https://arxiv.org/abs/2512.19609)
*Artemis Panagopoulou,Aveek Purohit,Achin Kulshrestha,Soroosh Yazdani,Mohit Goyal*

Main category: cs.CV

TL;DR: 该论文针对多模态大语言模型在精细空间理解（如地图路径追踪）上的不足，提出了一种可扩展的合成数据生成方法，通过微调显著提升了模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在地图路径追踪等精细空间理解任务中表现不佳，常违反基本路径约束，主要原因是缺乏大规模像素级标注数据。

Method: 设计可扩展的合成数据生成流程，利用合成地图图像和像素级解析自动生成精确标注，构建包含23k路径样本的数据集，并对开源和专有MLLMs进行微调。

Result: 在MapBench上，微调后模型成功率提升最高达6.4个百分点，路径追踪误差（NDTW）降低，证明了合成监督的有效性。

Conclusion: 精细空间推理能力可通过合成监督数据显式教授，弥补预训练模型的不足，显著提升模型在复杂空间任务上的表现。
...
### [174] [Beyond CLIP: Knowledge-Enhanced Multimodal Transformers for Cross-Modal Alignment in Diabetic Retinopathy Diagnosis](https://arxiv.org/abs/2512.19663)
*Argha Kamal Samanta,Harshika Goyal,Vasudha Joshi,Tushar Mungle,Pabitra Mitra*

Main category: cs.CV

TL;DR: 提出了一种融合视网膜图像、临床文本和结构化患者数据的多模态框架，显著提升了糖尿病视网膜病变的跨模态检索和诊断性能。


<details>
  <summary>Details</summary>
Motivation: 通用视觉语言模型（如CLIP）在医学领域，特别是眼科图像的跨模态检索中表现不佳，需要针对医学领域特点设计专门的模型。

Method: 使用Vision Transformer处理视网膜图像、Bio-ClinicalBERT处理临床文本、多层感知机处理结构化数据，通过多模态transformer融合，并采用对比损失、重建损失和分类损失进行多目标训练。

Result: 在BRSET数据集上，文本到图像检索Recall@1达到99.94%（CLIP仅1.29%），SDRG和ICDR分类准确率分别达97.05%和97.97%；在DeepEyeNet上的零样本检索Recall@1达93.95%（CLIP仅0.22%）。

Conclusion: 该多模态训练方法能有效捕捉医学领域的跨模态关系，在检索能力和诊断性能上均优于现有方法，并展现出良好的泛化能力。
...
### [175] [WorldWarp: Propagating 3D Geometry with Asynchronous Video Diffusion](https://arxiv.org/abs/2512.19678)
*Hanyang Kong,Xingyi Yang,Xiaoxu Zheng,Xinchao Wang*

Main category: cs.CV

TL;DR: 提出WorldWarp框架，通过结合3D几何缓存与2D生成细化器，解决长视频生成中几何一致性与复杂相机轨迹的难题。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在相机条件潜空间中效果最佳，但长视频生成需要严格遵循3D几何，导致在遮挡区域和复杂相机轨迹上表现不佳。

Method: 使用高斯溅射（3DGS）构建在线3D几何缓存作为结构锚点，结合时空扩散模型（ST-Diff）进行“填充与修订”，采用时空变化噪声调度动态生成和细化内容。

Result: WorldWarp在视频生成中实现了最先进的保真度，确保3D逻辑指导结构的同时扩散逻辑优化纹理，保持跨视频片段的一致性。

Conclusion: 通过耦合3D结构锚点与2D生成细化器，WorldWarp有效桥接了几何一致性与生成质量之间的鸿沟，为复杂场景的长视频生成提供了新解决方案。
...
### [176] [Cross-modal Counterfactual Explanations: Uncovering Decision Factors and Dataset Biases in Subjective Classification](https://arxiv.org/abs/2512.18864)
*Alina Elena Baia,Andrea Cavallaro*

Main category: cs.CV

TL;DR: 提出DeX框架，利用跨模态分解和图像特定概念生成自然语言反事实解释，用于图像隐私决策分析，无需训练即可量化场景元素对模型预测的贡献，并识别数据集偏见。


<details>
  <summary>Details</summary>
Motivation: 图像隐私决策具有情境性和主观性，现有方法难以生成稀疏、可解释的反事实解释。需要一种无需训练、能量化关键元素贡献并揭示数据偏见的解释框架。

Method: 1. 跨模态分解：将图像分解为语义概念；2. 多准则选择机制：结合图像相似性（最小扰动）和决策置信度筛选关键因素；3. 生成自然语言反事实解释；4. 评估解释间的相互依赖性和影响力。

Result: 1. 相比现有方法显著提升解释质量；2. 成功识别主观决策的主要影响因素；3. 揭示底层数据集偏见，为针对性公平性改善提供依据；4. 框架无需训练，灵活性高。

Conclusion: DeX框架通过图像特定概念生成稀疏、可操作的反事实解释，不仅能有效解释图像隐私决策，还能识别数据偏见，为模型公平性优化提供路径，且具有训练自由的灵活性优势。
...
### [177] [Localising Shortcut Learning in Pixel Space via Ordinal Scoring Correlations for Attribution Representations (OSCAR)](https://arxiv.org/abs/2512.18888)
*Akshit Achara,Peter Triantafillou,Esther Puyol-Antón,Alexander Hammers,Andrew P. King*

Main category: cs.CV

TL;DR: 本文介绍了OSCAR框架，用于量化和定位深度学习模型中的捷径学习，特别是在医学成像等领域，通过相关性分析评估模型对敏感属性的依赖，并提供缓解策略。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型常利用与任务语义无关的捷径特征，当这些特征与敏感属性相关时，会导致模型偏见。现有方法多基于定性、图像级检查，且假设特征可见，限制了在医学成像等领域的应用，因此需要一种模型无关的量化框架。

Method: 提出OSCAR框架，将图像级任务归因图转换为数据集级排名轮廓，比较平衡基线模型、测试模型和敏感属性预测器，通过计算成对、部分和偏差相关性，生成量化指标和图像区域排名，以定位捷径特征。

Result: 在CelebA、CheXpert和ADNI数据集上的实验表明，OSCAR的相关性指标稳定、对训练数据中捷径特征与标签关联敏感，并能区分局部与扩散的捷径特征，通过测试时衰减方法可减少最差组性能差异。

Conclusion: OSCAR提供了一种轻量级的像素空间审计方法，能生成统计决策规则和空间图，帮助用户测试、定位和缓解模型对捷径的依赖，代码已开源。
...
### [178] [Delta-LLaVA: Base-then-Specialize Alignment for Token-Efficient Vision-Language Models](https://arxiv.org/abs/2512.18910)
*Mohamad Zamini,Diksha Shukla*

Main category: cs.CV

TL;DR: 提出Delta-LLaVA，一种基于低秩DeltaProjection和轻量Transformer的token高效视觉投影器，显著提升多模态大语言模型的推理速度和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs中处理密集视觉token的计算成本过高，标准多层感知机投影器在高分辨率输入下存在冗余且扩展性差，需要更高效的视觉特征对齐方法。

Method: 采用基础对齐后专业化的设计：1）低秩DeltaProjection将多级视觉特征对齐到紧凑子空间；2）轻量Transformer块作为专业化层，在有限token预算下捕获全局和局部结构。

Result: 仅用144个token即在多个基准测试中取得稳定提升，推理吞吐量提升达55%，端到端训练在预训练阶段加速4-5倍、微调阶段加速1.5倍以上。

Conclusion: 基础对齐后专业化的设计能同时提升效率和可扩展性，证明在扩展交互能力前优化token形成策略的重要性。
...
### [179] [Thinking Beyond Labels: Vocabulary-Free Fine-Grained Recognition using Reasoning-Augmented LMMs](https://arxiv.org/abs/2512.18897)
*Dmitry Demidov,Zaigham Zaheer,Zongyan Han,Omkar Thawakar,Rao Anwer*

Main category: cs.CV

TL;DR: 提出FiNDR框架，利用大型多模态模型的推理能力实现无需预定义词汇表的细粒度图像识别，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度图像识别方法依赖固定词汇表或复杂易错的多阶段流程，而大型多模态模型具备视觉语言理解、问题分解和知识检索能力，为更有效的解决方案提供了可能。

Method: 采用三步自动化流程：1) 推理增强的大型多模态模型为每张图像生成候选描述标签；2) 视觉语言模型筛选排序候选标签形成连贯类别集；3) 验证后的名称实例化轻量级多模态分类器用于推理。

Result: 在词汇表无关设置下，相对先前方法提升达18.8%；甚至超越使用预定义真实名称的零样本基线，挑战了人工标注词汇表性能上限的假设；开源模型通过精心设计的提示词可匹配专有模型性能。

Conclusion: 推理增强的大型多模态模型为可扩展、全自动、开放世界的细粒度视觉识别提供了有效基础，代码已开源。
...
### [180] [Symmetrization of 3D Generative Models](https://arxiv.org/abs/2512.18953)
*Nicolas Caytuiro,Ivan Sipiran*

Main category: cs.CV

TL;DR: 提出了一种通过修改训练数据而非模型架构来促进3D生成模型对称性的数据中心方法，通过训练仅使用半物体（沿x=0平面对称的一半形状）的生成模型，生成时反射得到对称且视觉合理的完整形状。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型在生成对称形状时存在不一致性，通过分析真实3D形状和先进模型生成样本的反射对称性，提出通过数据层面的改进来提升生成形状的对称性。

Method: 1. 分析真实3D形状和生成样本的反射对称性；2. 从ShapeNet的三个类别（飞机、汽车、椅子）构建半物体数据集；3. 训练两个生成模型仅使用半物体数据；4. 生成时通过反射操作获得完整对称形状。

Result: 实验表明，与原始模型和原始数据集生成的物体相比，该方法生成的形状在对称性和一致性方面表现更优。

Conclusion: 通过数据中心的半物体训练方法，能够有效提升3D生成模型生成形状的对称性和视觉合理性，为3D生成模型的对称性改进提供了新思路。
...
### [181] [DVI: Disentangling Semantic and Visual Identity for Training-Free Personalized Generation](https://arxiv.org/abs/2512.18964)
*Guandong Li,Yijun Ding*

Main category: cs.CV

TL;DR: 提出DVI框架，通过解耦语义和视觉流解决身份定制中的语义-视觉失调问题，实现无需训练的视觉一致性增强。


<details>
  <summary>Details</summary>
Motivation: 现有免调优身份定制方法虽能实现高面部保真度，但常忽略光照、皮肤纹理等视觉上下文，导致面部几何与输入氛围不协调的“贴纸效应”。

Method: 1）将身份解耦为细粒度语义流和粗粒度视觉流；2）利用VAE潜在空间的均值方差作为视觉描述符；3）设计无参数特征调制机制；4）引入动态时间粒度调度器。

Result: DVI在IBench评估中超越现有方法，显著提升视觉一致性和氛围保真度，同时保持强身份保持能力。

Conclusion: DVI通过解耦语义与视觉表示，有效解决了身份定制中的视觉失调问题，为免训练个性化生成提供了新思路。
...
### [182] [VOIC: Visible-Occluded Decoupling for Monocular 3D Semantic Scene Completion](https://arxiv.org/abs/2512.18954)
*Zaidao Han,Risa Higashita,Jiang Liu*

Main category: cs.CV

TL;DR: 提出VOIC双解码器框架，通过可见区域标签提取策略，将单目3D语义场景补充分解为可见区域感知和遮挡区域推理两个子任务，在多个基准测试中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D语义场景补全方法在端到端特征提取过程中，存在高置信度可见区域与低置信度遮挡区域之间的特征干扰问题，导致特征稀释和错误传播。

Method: 1. 提出离线可见区域标签提取策略，从稠密3D真值中分离可见区域体素级监督；2. 设计VOIC双解码器框架：可见解码器生成几何语义先验，遮挡解码器通过跨模态交互进行全局场景推理；3. 融合图像特征与深度占据信息构建基础3D体素表示。

Result: 在SemanticKITTI和SSCBench-KITTI360基准测试中，VOIC在几何补全和语义分割精度上均超越现有单目方法，达到最先进性能。

Conclusion: 通过显式解耦可见区域感知与遮挡区域推理，并引入针对性监督策略，能有效提升单目3D语义场景补全的准确性和鲁棒性。
...
### [183] [Total Curvature Regularization and its_Minimization for Surface and Image Smoothing](https://arxiv.org/abs/2512.18968)
*Tianle Lu,Ke Chen,Yuping Duan*

Main category: cs.CV

TL;DR: 提出一种基于多方向法曲率惩罚的总法曲率正则化方法，用于产生锐利边缘和精确各向同性特征的解，并通过时间相关PDE系统求解该高阶非线性优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有曲率正则化方法在保持边缘锐利性和各向同性特性方面存在局限，需要一种能同时处理多方向曲率约束且参数鲁棒的正则化框架。

Method: 1. 提出总法曲率正则化公式；2. 将优化问题转化为时间相关PDE系统的稳态求解；3. 采用算子分裂法进行时间离散化；4. 各子问题通过闭式解或高效算法求解。

Result: 方法避免了复杂参数调优，对参数选择具有鲁棒性，在曲面和图像平滑问题上验证了其高效性和有效性。

Conclusion: 总法曲率正则化结合PDE稳态求解框架，为高阶非线性优化问题提供了一种参数鲁棒、计算高效的解决方案。
...
### [184] [Steering Vision-Language Pre-trained Models for Incremental Face Presentation Attack Detection](https://arxiv.org/abs/2512.19022)
*Haoze Li,Jie Zhang,Guoying Zhao,Stephen Lin,Shiguang Shan*

Main category: cs.CV

TL;DR: 提出SVLP-IL框架，基于视觉语言预训练模型，通过多角度提示和选择性弹性权重整合实现无需回放的增量学习，用于人脸呈现攻击检测。


<details>
  <summary>Details</summary>
Motivation: 人脸呈现攻击检测需要增量学习应对新型攻击手段，但隐私法规禁止保留历史数据，因此需开发无需回放的增量学习方法。

Method: 采用视觉语言预训练模型，设计多角度提示机制分离领域依赖关系，结合选择性弹性权重整合策略选择性地保留关键权重。

Result: 在多个PAD基准测试中显著减少灾难性遗忘，提升未知领域性能，实现隐私合规的终身学习部署。

Conclusion: SVLP-IL为无需回放的增量学习场景提供了兼顾稳定性与可塑性的实用解决方案，适用于现实世界的人脸防伪系统。
...
### [185] [CETCAM: Camera-Controllable Video Generation via Consistent and Extensible Tokenization](https://arxiv.org/abs/2512.19020)
*Zelin Zhao,Xinyu Gong,Bangya Liu,Ziyang Song,Jun Zhang,Suhui Wu,Yongxin Chen,Hao Zhang*

Main category: cs.CV

TL;DR: CETCAM是一种无需相机标注的相机可控视频生成框架，通过几何感知令牌实现精确相机控制，并在多个基准测试中达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖难以扩展且与深度估计不一致的相机姿态标注，导致训练-测试差异，限制了相机控制视频生成的精度和可扩展性。

Method: 利用VGGT等几何基础模型估计深度和相机参数，将其转换为统一的几何感知令牌，通过轻量级上下文块集成到预训练视频扩散模型中，采用两阶段渐进训练策略。

Result: 在多个基准测试中实现了最先进的几何一致性、时间稳定性和视觉真实感，并能灵活适应修复和布局控制等其他控制模态。

Conclusion: CETCAM通过无标注的相机控制方案解决了现有方法的局限性，展示了在视频生成中实现精确相机控制的可行性和扩展性。
...
### [186] [Towards AI-Guided Open-World Ecological Taxonomic Classification](https://arxiv.org/abs/2512.18994)
*Cheng Yaw Low,Heejoon Koo,Jaewoo Park,Kaleb Mesfin Asfaw,Meeyoung Cha*

Main category: cs.CV

TL;DR: 提出了Open-World Ecological Taxonomy Classification框架和TaxoNet模型，用于解决生态分类中的长尾分布、细粒度差异、时空域偏移和闭集假设问题，在多个植物数据集上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 生态分类对全球可持续性至关重要，但面临类不平衡导致的长尾分布、细粒度分类差异、测试时的时空域偏移以及只能识别已知类别的闭集假设等挑战，需要统一框架解决这些共现问题。

Method: 提出TaxoNet模型，采用基于嵌入的编码器和双边界惩罚损失，增强稀有类别的学习信号，抑制过代表类别的支配，直接应对相互关联的挑战。

Result: 在Google Auto-Arborist、iNat-Plantae和NAFlora-Mini三个生态数据集上评估，模型一致优于基线方法，尤其在稀有类别上表现突出，为开放世界植物分类监测奠定基础。

Conclusion: 该研究为开放世界生态分类提供了有效解决方案，同时发现通用多模态基础模型在植物领域应用中仍存在局限性。
...
### [187] [Automatic Neuronal Activity Segmentation in Fast Four Dimensional Spatio-Temporal Fluorescence Imaging using Bayesian Approach](https://arxiv.org/abs/2512.19032)
*Ran Li,Pan Xiao,Kaushik Dutta,Youdong Guo*

Main category: cs.CV

TL;DR: 提出了一种基于贝叶斯深度学习的框架，用于从光片显微镜获取的4D时空数据中自动检测神经元活动，通过结合时空信息并量化不确定性，实现了高效且可泛化的神经元活动检测。


<details>
  <summary>Details</summary>
Motivation: 荧光显微镜钙成像是在体记录大规模神经元活动的关键工具，但传统手动分割方法耗时耗力且缺乏泛化性，阻碍了行为相关神经元活动的自动精确检测。

Method: 采用贝叶斯深度学习框架，通过计算像素级相关性图利用时间信息，并结合平均摘要图像提供的空间信息，生成概率分割图并建模神经元检测的不确定性。

Result: 网络在合成真值（Otsu方法生成）上达到平均Dice分数0.81，在可重复性测试中两次运行间平均Dice分数为0.79，表明方法具有良好准确性和泛化能力。

Conclusion: 该方法可成功用于行为研究中神经元活动的快速检测，为脑活动映射研究提供了高效且可靠的自动化工具。
...
### [188] [WaTeRFlow: Watermark Temporal Robustness via Flow Consistency](https://arxiv.org/abs/2512.19048)
*Utae Jeong,Sumin In,Hyunju Ryu,Jaewan Choi,Feng Yang,Jongheon Jeong,Seungryong Kim,Sangpil Kim*

Main category: cs.CV

TL;DR: 提出了WaTeRFlow框架，通过流引导合成引擎、时序一致性损失和语义保持损失，增强图像水印在图像转视频过程中的鲁棒性检测。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习水印方法对扩散式图像编辑具有鲁棒性，但在图像转视频（I2V）过程中，逐帧水印检测效果会显著下降，而I2V技术已广泛应用于内容创作和世界建模，因此需要跨模态水印恢复方案。

Method: 1. FUSE（流引导统一合成引擎）：通过指令驱动编辑和快速视频扩散代理在训练中模拟真实失真；2. 光流扭曲与时序一致性损失（TCL）：稳定逐帧预测；3. 语义保持损失：维持条件信号。

Result: 在多种代表性I2V模型上的实验表明，该方法能从视频帧中准确恢复水印，首帧和逐帧比特准确率更高，且在视频生成前后施加多种失真时仍保持鲁棒性。

Conclusion: WaTeRFlow框架有效解决了图像水印在I2V转换中的鲁棒性问题，为跨模态水印保护提供了实用解决方案。
...
### [189] [Distinguishing Visually Similar Actions: Prompt-Guided Semantic Prototype Modulation for Few-Shot Action Recognition](https://arxiv.org/abs/2512.19036)
...
<details>
  <summary>Details</summary>
Motivation: 解决小样本动作识别中存在的三个关键问题：时间建模易受静态背景干扰、视觉相似类别难以区分、视觉-文本支持原型与纯视觉查询间的模态差距。

Method: 提出CLIP-SPM框架，包含三个组件：1) HSMR模块对齐深浅运动特征以减少背景干扰；2) SPM策略生成查询相关文本提示以弥合模态差距；3) PADM方法精炼支持原型并对齐查询特征与全局语义锚点。

Result: 在Kinetics、SSv2-Full、SSv2-Small、UCF101和HMDB51等基准测试中，CLIP-SPM在1-shot、3-shot和5-shot设置下均取得有竞争力的性能，消融实验验证了各组件有效性。

Conclusion: CLIP-SPM通过协同解决时间建模、模态对齐和特征区分度问题，有效提升了小样本动作识别的性能，各组件均对核心挑战有实质性贡献。
...
### [190] [Decoupled Generative Modeling for Human-Object Interaction Synthesis](https://arxiv.org/abs/2512.19049)
*Hwanhee Jung,Seunggwan Lee,Jeongyoon Yoon,SeungHyeon Kim,Giljoo Nam,Qixing Huang,Sangpil Kim*

Main category: cs.CV

TL;DR: 提出DecHOI方法，通过解耦路径规划与动作合成来生成更真实的人-物交互，避免人工设定路径点，提升运动同步性与接触真实性。


<details>
  <summary>Details</summary>
Motivation: 现有方法需人工指定路径点且将优化目标集中于单一网络，导致复杂度高、灵活性差、运动不同步或穿透等问题，需更有效的HOI生成方案。

Method: 1. 分离路径规划与动作合成；2. 轨迹生成器生成无预设路径点的人与物轨迹；3. 动作生成器基于路径合成细节运动；4. 采用对抗训练，判别器关注远端关节动态以提升接触真实感；5. 支持动态场景中的长序列规划与一致性保持。

Result: 在FullBodyManipulation和3D-FUTURE基准测试中，DecHOI在多数量化指标与定性评估上超越现有方法，感知研究也显示用户更偏好其生成结果。

Conclusion: DecHOI通过解耦生成建模有效解决了HOI合成中的运动同步与接触真实性问题，为动画与具身控制提供了更优解决方案。
...
### [191] [Retrieving Objects from 3D Scenes with Box-Guided Open-Vocabulary Instance Segmentation](https://arxiv.org/abs/2512.19088)
*Khanh Nguyen,Dasith de Silva Edirimuni,Ghulam Mubashar Hassan,Ajmal Mian*

Main category: cs.CV

TL;DR: 提出了一种基于2D开放词汇检测器引导的3D实例分割方法，用于从RGB图像生成新物体的3D实例掩码，在保持高效分类的同时提升对罕见物体的识别能力。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D实例分割方法严重依赖SAM和CLIP，计算开销大且处理速度慢，而Open-YOLO 3D等方法对训练数据中罕见物体类别的泛化能力不足。

Method: 通过2D开放词汇检测器引导，从RGB图像生成新物体的3D实例掩码，继承2D检测器识别新物体的能力，同时保持高效分类。

Result: 该方法能够快速准确地从开放文本查询中检索罕见实例，在保持高效推理的同时提升了对新物体类别的泛化性能。

Conclusion: 所提出的方法通过结合2D开放词汇检测器的优势，实现了对罕见物体的有效3D实例分割，为机器人和增强现实应用提供了更实用的解决方案。
...
### [192] [6DAttack: Backdoor Attacks in the 6DoF Pose Estimation](https://arxiv.org/abs/2512.19058)
*Jihui Guo,Zongmin Zhang,Zhen Sun,Yuhao Yang,Jinlin Wu,Fu Zhang,Xinlei He*

Main category: cs.CV

TL;DR: 该论文提出了6DAttack框架，针对6DoF物体姿态估计模型进行3D触发器后门攻击，在保持正常性能的同时实现可控的姿态参数篡改，现有防御方法难以检测。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习在6DoF姿态估计领域应用广泛，但后门攻击研究主要集中于2D视觉任务。由于6DoF攻击需要控制连续参数（平移和旋转），传统2D后门方法无法直接适用，因此需要专门研究针对6DoF姿态估计的安全威胁。

Method: 提出6DAttack框架，使用3D物体作为触发器，通过设计特定的触发机制诱导模型输出错误的姿态参数。在训练阶段将触发样本注入训练数据，使模型学习到触发模式与目标错误姿态的关联。

Result: 在PVNet、DenseFusion和PoseDiffusion三个模型上，使用LINEMOD、YCB-Video和CO3D数据集进行评估，攻击成功率（ASR）接近100%，同时保持正常样本的高精度（ADD准确率可达100%）。触发样本的ADD-P指标达到97.70%，且现有代表性防御方法无法有效检测。

Conclusion: 6DoF姿态估计系统存在严重但尚未充分探索的后门攻击威胁，6DAttack框架证明了攻击的可行性和隐蔽性，亟需开发针对性的防御机制以保障实际应用安全。
...
### [193] [Auditing Significance, Metric Choice, and Demographic Fairness in Medical AI Challenges](https://arxiv.org/abs/2512.19091)
*Ariel Lubonja,Pedro R. A. S. Bassi,Wenxuan Li,Hualin Qiao,Randal Burns,Alan L. Yuille,Zongwei Zhou*

Main category: cs.CV

TL;DR: 本文介绍了RankInsight开源工具包，旨在解决医学AI排行榜的三个关键局限：统计显著性检验缺失、单一指标不适用于所有器官、交叉人口统计性能报告不足。


<details>
  <summary>Details</summary>
Motivation: 当前医学AI挑战赛排行榜存在三个主要问题：排名缺乏统计显著性验证；使用单一平均指标评估所有器官，忽略临床重要的边界误差；很少报告交叉人口统计群体的性能，掩盖公平性差距。

Method: 开发RankInsight开源工具包，提供三种核心功能：计算配对统计显著性图谱；使用器官特异性指标重新计算排行榜；审计交叉人口公平性（如性别-种族差异）。

Result: 应用RankInsight发现：nnU-Net模型家族在统计显著性上优于Vision-Language和MONAI提交模型；使用NSD替代Dice评估管状结构时，前四名模型排序反转；超过一半的MONAI模型在约翰·霍普金斯医院数据集上表现出最大的性别-种族差异。

Conclusion: RankInsight工具包能够帮助挑战赛组织者和参与者发布具有统计可靠性、临床意义和人口公平性的排名，适用于过去、当前和未来的医学AI挑战赛。
...
### [194] [GaussianImage++: Boosted Image Representation and Compression with 2D Gaussian Splatting](https://arxiv.org/abs/2512.19108)
*Tiantian Li,Xinjie Zhang,Xingtong Ge,Tongda Xu,Dailan He,Jun Zhang,Yan Wang*

Main category: cs.CV

TL;DR: 提出GaussianImage++方法，通过失真驱动密集化、上下文感知高斯滤波和属性分离可学习量化器，在有限高斯基元下实现高效图像表示与压缩。


<details>
  <summary>Details</summary>
Motivation: 隐式神经表示训练耗资源，现有2D高斯泼溅方法需要过多基元维持画质，需开发更高效的高斯基元表示与压缩方法。

Method: 1. 失真驱动密集化机制按信号强度分配基元；2. 上下文感知高斯滤波器优化基元分布；3. 属性分离可学习量化器结合量化感知训练压缩属性。

Result: 在表示与压缩性能上超越GaussianImage和INRs-based COIN，同时保持实时解码和低内存占用。

Conclusion: GaussianImage++通过高效基元管理和压缩策略，为图像表示与压缩提供了高性能解决方案。
...
### [195] [Mamba-Based Modality Disentanglement Network for Multi-Contrast MRI Reconstruction](https://arxiv.org/abs/2512.19095)
*Weiyi Lyu,Xinming Fang,Jun Wang,Jun Shi,Guixu Zhang,Juncheng Li*

Main category: cs.CV

TL;DR: 提出MambaMDN双域框架，通过参考K空间数据补全和模态解耦网络，解决多对比度MRI重建中的伪影和模态污染问题。


<details>
  <summary>Details</summary>
Motivation: 现有加速MRI技术存在两个关键问题：1）未能有效利用K空间先验信息，导致零填充输入的混叠伪影持续存在；2）多对比度融合策略中无关信息污染目标重建质量。

Method: 1）使用全采样参考K空间数据补全欠采样目标数据，生成结构对齐但模态混合的输入；2）开发基于Mamba的模态解耦网络，从混合表示中提取并移除参考特定特征；3）引入迭代细化机制，通过重复特征纯化逐步提升重建精度。

Result: 大量实验表明，MambaMDN在多对比度MRI重建任务中显著优于现有方法。

Conclusion: MambaMDN框架通过双域处理和模态解耦，有效克服了加速MRI重建中的关键挑战，提升了重建质量和效率。
...
### [196] [Generative Giants, Retrieval Weaklings: Why do Multimodal Large Language Models Fail at Multimodal Retrieval?](https://arxiv.org/abs/2512.19115)
*Hengyi Feng,Zeang Sheng,Meiyi Qiang,Wentao Zhang*

Main category: cs.CV

TL;DR: 研究发现多模态大语言模型在零样本多模态检索任务中存在缺陷，主要原因是其表示空间中文本语义占主导地位，视觉信息不足，且跨模态对齐导致嵌入同质化，降低了检索的判别能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在生成任务中表现优异，但在零样本多模态检索任务中却存在反直觉的缺陷，这促使研究者探究其背后的机制。

Method: 使用稀疏自编码器分解多模态大语言模型的输出表示，将其解耦为可解释的语义概念，以探测其内在行为。

Result: 分析表明，多模态大语言模型的表示空间主要由文本语义主导，视觉信息仅占一小部分；跨模态对齐促进了生成任务，但导致嵌入同质化，削弱了多模态检索所需的判别能力；相似性计算中最关键的特征成分实际上是降低检索性能的干扰因素。

Conclusion: 本研究首次对多模态大语言模型在多模态检索任务中的表示进行了深入的可解释性分析，并指出了增强其多模态检索能力的可能方向。
...
### [197] [OmniMoGen: Unifying Human Motion Generation via Learning from Interleaved Text-Motion Instructions](https://arxiv.org/abs/2512.19159)
*Wendong Bu,Kaihang Pan,Yuze Lin,Jiacheng Li,Kai Shen,Wenqiao Zhang,Juncheng Li,Jun Xiao,Siliang Tang*

Main category: cs.CV

TL;DR: 提出了OmniMoGen统一框架，通过交错文本-动作指令实现多功能动作生成，在多个任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有动作生成方法局限于孤立任务，缺乏自由形式和全目标生成的灵活性，而大语言模型在语言任务中的统一框架尚未在动作生成领域探索。

Method: 基于简洁的RVQ-VAE和Transformer架构构建OmniMoGen框架，创建包含13.7万条交错文本-动作指令的X2Mo数据集，并引入AnyContext评估基准。

Result: 在文本到动作、动作编辑和AnyContext基准上达到最先进性能，展现出组合编辑、自反思生成和知识引导生成等新兴能力。

Conclusion: OmniMoGen标志着向下一代智能动作生成迈出一步，实现了指令驱动的端到端动作生成统一框架。
...
### [198] [Trifocal Tensor and Relative Pose Estimation with Known Vertical Direction](https://arxiv.org/abs/2512.19110)
*Tao Li,Zhenbao Yu,Banglei Guan,Jianli Han,Weimin Lv,Friedrich Fraundorfer*

Main category: cs.CV

TL;DR: 本文提出了两种新颖的求解器，用于在已知相机垂直方向的情况下估计视图间的相对位姿。算法仅需解决两个旋转角度和两个平移向量，分别需要三视图中的四个和三个点对应关系，并能在RANSAC框架中高效应用。实验表明，该方法在合成数据和KITTI真实场景中均优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 相机垂直方向可通过IMU（广泛应用于自动驾驶车辆、手机和无人机）轻松获取。利用这一已知信息，可以简化相对位姿估计问题，减少所需的点对应数量，提高计算效率，特别是在存在异常值的视觉里程计应用中。

Method: 1. 提出线性闭式解，仅需三视图中的四个点对应关系。2. 提出基于最新Gröbner基求解器的最小解，仅需三个点对应关系。两种方法均利用已知垂直方向，将问题简化为求解两个旋转角度和两个平移向量。

Result: 在合成数据和KITTI真实场景上的实验结果表明，所提方法估计的位姿精度优于其他替代方法，且由于所需点对应较少，能高效应用于RANSAC框架进行异常值剔除和位姿估计。

Conclusion: 通过利用IMU提供的已知垂直方向信息，所提出的两种求解器能够以更少的点对应关系高效、准确地估计相对位姿，特别适用于视觉里程计等实际应用场景。
...
### [199] [PEDESTRIAN: An Egocentric Vision Dataset for Obstacle Detection on Pavements](https://arxiv.org/abs/2512.19190)
*Marios Thoma,Zenonas Theodosiou,Harris Partaourides,Vassilis Vassiliades,Loizos Michael,Andreas Lanitis*

Main category: cs.CV

TL;DR: 本文介绍了PEDESTRIAN数据集，包含29种常见人行道障碍物的第一视角视频数据，并测试了多种深度学习算法用于障碍物检测，旨在提升行人安全。


<details>
  <summary>Details</summary>
Motivation: 城市人行道常被障碍物阻塞，威胁行人安全。现有研究缺乏全面、平衡的第一视角障碍物数据集，限制了实时障碍物检测系统的发展。

Method: 1. 使用手机摄像头采集340段第一视角视频，构建包含29类障碍物的PEDESTRIAN数据集；2. 利用该数据集训练多种先进深度学习算法进行障碍物检测与识别实验。

Result: 1. 建立了首个针对人行道障碍物的第一视角视频数据集；2. 通过实验验证了数据集可用于训练有效的障碍物检测模型，为相关任务提供了基准。

Conclusion: PEDESTRIAN数据集填补了第一视角人行道障碍物数据的空白，为开发实时障碍物检测系统、提升城市行人安全提供了重要数据基础与算法基准。
...
### [200] [InvCoSS: Inversion-driven Continual Self-supervised Learning in Medical Multi-modal Image Pre-training](https://arxiv.org/abs/2512.19213)
*Zihao Luo,Shaohao Rui,Zhenyu Tang,Guotai Wang,Xiaosong Wang*

Main category: cs.CV

TL;DR: 提出InvCoSS框架，通过模型反演生成合成图像进行持续自监督学习，解决医学多模态图像预训练中的数据隐私和遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有持续自监督学习方法依赖重放历史数据以防止灾难性遗忘，但会破坏数据隐私并受限于跨站点数据传输限制，需要一种不访问历史真实数据的解决方案。

Method: 1. 训练后通过模型反演生成近似原始分布的合成图像；2. 设计多尺度融合的InvUNet提升图像保真度；3. 引入排斥表征学习机制增强合成图像多样性；4. 合成图像与新任务数据联合优化。

Result: 在9个下游任务上的实验表明，InvCoSS性能达到或超过传统数据重放方法，同时显著降低存储需求并完全消除数据隐私限制。

Conclusion: InvCoSS框架在严格不访问历史真实数据的约束下，有效缓解灾难性遗忘，为医学多模态持续学习提供了隐私安全的解决方案。
...
### [201] [AMap: Distilling Future Priors for Ahead-Aware Online HD Map Construction](https://arxiv.org/abs/2512.19150)
...
TL;DR: 本文提出AMap框架，通过“从未来蒸馏”范式让当前帧模型具备前瞻感知能力，解决现有时序融合方法在自动驾驶中仅增强已通过区域感知、忽视前方道路的安全缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有在线高精地图构建方法依赖历史时序融合，主要提升已通过区域的感知精度，但对前方未观测道路的感知能力不足。下游规划任务分析表明，前方区域感知误差会直接导致危险驾驶行为，存在严重安全风险。

Method: 提出AMap框架：1）采用“从未来蒸馏”范式，让可访问未来时序信息的教师模型指导仅使用当前帧的轻量学生模型；2）设计多级BEV蒸馏策略，结合空间掩码和不对称查询适配模块，将未来感知表征迁移至学生模型的静态查询中。

Result: 在nuScenes和Argoverse 2基准测试中，AMap显著提升了当前帧感知性能，尤其在关键前方区域超越现有最优时序模型，同时保持单帧推理效率。

Conclusion: AMap通过前瞻感知蒸馏机制，在不增加推理成本的前提下有效解决了自动驾驶地图构建中前方区域感知薄弱的安全隐患，为在线高精地图构建提供了更安全的解决方案。
...
### [202] [HippMetric: A skeletal-representation-based framework for cross-sectional and longitudinal hippocampal substructural morphometry](https://arxiv.org/abs/2512.19214)
*Na Gao,Chenfei Ye,Yanwu Yang,Anqi Li,Zhengbo He,Li Liang,Zhiyuan Liu,Xingyu Hao,Ting Ma,Tengfei Guo*

Main category: cs.CV

TL;DR: 提出HippMetric框架，基于骨骼表示建立海马亚结构形态测量和点对应关系，解决跨个体和扫描的解剖变异问题。


<details>
  <summary>Details</summary>
Motivation: 海马亚结构的高个体间变异性和复杂折叠模式阻碍了一致的跨个体和纵向分析，现有方法缺乏稳定的内在坐标系来适应解剖变异。

Method: 基于轴参考形态模型，采用可变形骨骼坐标系对齐海马解剖和功能，包括尊重海马纵向层状结构的坐标系模块，以及通过表面重建、变形和几何约束生成的个体化骨骼表示。

Result: 在两个国际队列上的实验表明，HippMetric相比现有形状模型实现了更高的准确性、可靠性和对应稳定性。

Conclusion: HippMetric框架为海马亚结构形态测量提供了生物学基础的参考，能够建立可靠的个体间和个体内对应关系。
...
TL;DR: 提出3SGen框架，统一处理主题、风格和结构三种图像生成条件，通过自适应任务记忆模块解耦特征，并建立标准化评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成方法通常孤立处理主题、风格和结构条件，导致特征纠缠和任务迁移性受限，需要统一框架解决多条件协同生成问题。

Method: 使用配备可学习语义查询的MLLM对齐文本-图像语义，VAE分支保留视觉细节；核心是自适应任务记忆模块，通过轻量门控机制和可扩展记忆项动态解耦、存储和检索条件先验。

Result: 在提出的3SGen-Bench和其他公开基准测试中，3SGen在多样图像驱动生成任务上表现优异，有效减少任务间干扰并支持组合输入。

Conclusion: 3SGen框架成功统一了多条件图像生成，其任务感知设计和记忆模块提高了生成质量和可控性，为跨任务评估提供了标准化基准。
...
### [204] [VisionDirector: Vision-Language Guided Closed-Loop Refinement for Generative Image Synthesis](https://arxiv.org/abs/2512.19243)
*Meng Chu,Senqiao Yang,Haoxuan Che,Suiyun Zhang,Xichen Zhang,Shaozuo Yu,Haokun Gui,Zhefan Rao,Dandan Tu,Rui Liu,Jiaya Jia*

Main category: cs.CV

TL;DR: 本文提出LGBench基准测试揭示生成模型处理长多目标提示的不足，并开发VisionDirector系统通过结构化目标解析、动态编辑决策和语义验证提升图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型虽能生成逼真图像，但在处理专业设计师使用的长串、多目标提示时表现不佳，缺乏真实场景下的评估基准。

Method: 1) 构建包含2000个任务的LGBench基准；2) 提出无需训练的VisionDirector系统，包含目标结构化提取、动态编辑决策、微网格采样与语义验证回滚机制；3) 使用组相对策略优化微调规划器。

Result: 1) 现有SOTA模型仅满足不到72%的目标；2) VisionDirector在GenEval上提升7%，ImgEdit提升0.07分；3) 编辑步骤从4.2步减少至3.1步，在版式、多对象场景和姿态编辑上获得定性改进。

Conclusion: 长多目标提示是生成模型的薄弱环节，VisionDirector通过结构化目标管理和迭代验证机制显著提升了复杂指令下的图像生成质量与一致性。
...
TL;DR: 提出HaMoS框架，首个基于手部感知的序列级扩散模型，利用头部轨迹和间歇可见的手部线索从第一人称视频重建全身运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖头部轨迹导致运动模糊，或假设手部持续可见不适用于轻量级设备；真实场景中手部因视野限制和遮挡仅间歇可见，需开发更符合实际条件的运动估计方法。

Method: 1. 设计手部感知序列级扩散框架，融合头部轨迹和间歇手部线索；2. 提出数据增强方法模拟真实设备视野限制；3. 采用局部注意力机制高效推断长序列，利用身体形态和视野等序列级上下文。

Result: 在公开基准测试中达到最先进的精度和时间平滑度，验证了在真实场景中实现可靠第一人称3D运动理解的可行性。

Conclusion: HaMoS通过建模真实设备条件，利用间歇手部线索和序列级上下文，显著提升了第一人称视频中全身运动估计的准确性和实用性。
...
TL;DR: 提出RMLer框架，将跨类别概念融合建模为强化学习问题，通过MLP策略网络动态混合文本嵌入，优化生成新颖、连贯的跨类别物体图像。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成方法在融合不同类别概念时存在概念混合不足、评估不严谨、输出质量低（如概念失衡、表面组合或简单并列）等问题，需要更有效的跨类别概念合成框架。

Method: 提出强化混合学习框架RMLer：将混合特征作为状态、混合策略作为动作、视觉结果作为奖励；设计MLP策略网络预测跨类别文本嵌入的动态混合系数；引入基于语义相似性和组合平衡的视觉奖励，通过近端策略优化训练策略网络；推理时采用选择策略筛选高质量融合结果。

Result: 大量实验表明RMLer在合成跨类别连贯、高保真物体方面优于现有方法，能有效生成新颖视觉概念。

Conclusion: RMLer为生成新颖视觉概念提供了稳健框架，在影视、游戏和设计领域具有应用潜力，解决了跨类别概念融合中的关键挑战。
...
TL;DR: 提出Think2Seg-RS框架，通过解耦语言推理与像素预测，利用强化学习训练大型视觉语言模型生成几何提示来控制SAM模型，在遥感图像分割任务中实现最先进性能并具备零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在遥感分析中通常采用端到端监督微调，导致几何定位能力弱且任务泛化性有限，需要新的框架来解决这些问题。

Method: 开发Think2Seg-RS解耦框架：1）训练LVLM提示器生成结构化几何提示；2）使用仅掩码的强化学习目标；3）控制冻结的SAM模型进行分割；4）在EarthReason数据集上验证。

Result: 1）在EarthReason数据集上达到最先进性能；2）零样本泛化到多个参考分割基准；3）发现语义级与实例级定位的明显差异；4）紧凑分割器在语义级监督下优于大型模型；5）负面提示在异质航空背景下无效。

Conclusion: 语义级推理分割成为地理空间理解的新范式，为统一、可解释的LVLM驱动地球观测开辟了道路，代码和模型已开源。
...
### [208] [Extended OpenTT Games Dataset: A table tennis dataset for fine-grained shot type and point outcome](https://arxiv.org/abs/2512.19327)
*Moamal Fadhil Abdul,Jonas Bruun Hubrechts,Thomas Martini Jørgensen,Emil Hovad*

Main category: cs.CV

TL;DR: 扩展OpenTTGames数据集，新增详细的击球类型、球员姿态和回合结果标注，以支持乒乓球视频中细粒度击球理解和战术分析。


<details>
  <summary>Details</summary>
Motivation: 现有乒乓球视频数据集缺乏细粒度击球类型和战术相关标注，限制了训练流程优化、转播增强和性能分析等应用的发展。

Method: 在OpenTTGames数据集基础上，通过紧凑编码方案和代码辅助标注流程，新增帧级击球类型（正手/反手子类）、球员姿态（身体倾斜和腿部站姿）和回合结果标签。

Result: 构建了包含丰富战术语义标注的扩展数据集，填补了社区中公开可用的细粒度击球分析数据空白，并采用CC BY-NC-SA 4.0许可促进非商业用途的复用和基准测试。

Conclusion: 该数据集扩展使模型能够超越简单事件检测，实现战术理解（如击球是否可能得分或建立优势），为球拍类运动的细粒度分析提供了可复现的标注基础和基准。
...
### [209] [ReasonCD: A Multimodal Reasoning Large Model for Implicit Change-of-Interest Semantic Mining](https://arxiv.org/abs/2512.19354)
*Zhenyang Huang,Xiao Yu,Yi Zhang,Decheng Wang,Hang Ruan*

Main category: cs.CV

TL;DR: 提出ReasonCD模型，利用大语言模型推理能力挖掘用户隐式意图，实现遥感图像变化检测，在BCDD数据集上F1分数达92.1%。


<details>
  <summary>Details</summary>
Motivation: 现有基于语义引导的遥感变化检测方法过度依赖显式文本描述，当遇到隐式意图描述时性能严重下降，需解决隐式任务意图理解问题。

Method: 构建多模态推理变化检测模型ReasonCD，利用预训练大语言模型的强大推理能力挖掘用户隐式任务意图，根据意图生成不同的变化检测结果。

Result: 在公开数据集上表现优异，BCDD数据集F1分数92.1%；在基于SECOND数据集标注的推理数据上验证了模型具备优秀推理能力，并能解释推理过程辅助决策。

Conclusion: ReasonCD模型能有效挖掘用户隐式意图，实现高性能变化检测，同时具备推理解释能力，为遥感智能解译提供新思路。
...
### [210] [DeltaMIL: Gated Memory Integration for Efficient and Discriminative Whole Slide Image Analysis](https://arxiv.org/abs/2512.19331)
*Yueting Zhu,Yuehao Song,Shuai Zhang,Wenyu Liu,Xinggang Wang*

Main category: cs.CV

TL;DR: 提出DeltaMIL框架，通过门控delta规则和局部模式混合机制，有效筛选和整合全切片图像中的判别性信息，在生存预测和分类任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有MIL方法难以有效处理全切片图像的高冗余和异质性信息，无法充分筛选无关特征并整合多区域判别信号，限制了其在大规模异质WSI上的性能。

Method: 1. 门控delta规则：结合遗忘与记忆机制，动态更新记忆单元；2. 局部模式混合机制：保留细粒度病理局部特征；3. 通过相关性动态移除旧值并插入新值，快速遗忘无关信号。

Result: 1. 生存预测：ResNet-50特征提升3.69%，UNI特征提升2.36%；2. 切片级分类：ResNet-50特征准确率提升3.09%，UNI特征提升3.75%；3. 在多种WSI任务上表现一致优于现有方法。

Conclusion: DeltaMIL能有效增强判别性特征提取并抑制冗余噪声，提升模型鲁棒性和判别能力，为处理大规模异质WSI提供了高效解决方案。
...
### [211] [GANeXt: A Fully ConvNeXt-Enhanced Generative Adversarial Network for MRI- and CBCT-to-CT Synthesis](https://arxiv.org/abs/2512.19336)
*Siyuan Mei,Yan Xia,Fuxin Fan*

Main category: cs.CV

TL;DR: 提出GANeXt，一种基于3D ConvNeXt的生成对抗网络，用于从MRI和CBCT合成CT图像，支持多模态和多解剖区域的统一合成，在放疗计划中实现准确的解剖表示。


<details>
  <summary>Details</summary>
Motivation: 在自适应放疗中，从MRI和CBCT合成CT对于精确的治疗计划至关重要，因为CT能提供准确的解剖信息，而现有方法在跨模态和跨区域的统一合成方面存在局限。

Method: 采用3D ConvNeXt块构建U形生成器，结合条件PatchGAN判别器；使用MAE、感知损失、分割掩码MAE和对抗损失等多损失函数；引入多头分割判别器（Dice损失和交叉熵）；采用AdamW优化器、预热和余弦衰减调度器；数据预处理包括形变配准、前景裁剪和归一化；推理时使用滑动窗口平均折叠重建全尺寸sCT。

Result: 通过联合训练所有区域（无需微调），在3000轮（MRI-to-CT）和1000轮（CBCT-to-CT）后选择最终模型，实现了跨模态和跨解剖区域的统一CT合成。

Conclusion: GANeXt能够有效合成高质量的CT图像，支持多模态和多区域的统一处理，为自适应放疗计划提供了准确的解剖表示工具。
...
### [212] [Efficient Spike-driven Transformer for High-performance Drone-View Geo-Localization](https://arxiv.org/abs/2512.19365)
*Zhongwei Chen,Hai-Jun Rong,Zhao-Xu Yang,Guoqi Li*

Main category: cs.CV

TL;DR: 提出首个用于无人机视角地理定位的脉冲神经网络框架SpikeViMFormer，通过轻量级脉冲驱动Transformer主干、选择性注意力块和混合状态空间块解决信息丢失和长程依赖问题，在保持低功耗的同时达到与先进ANN竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于人工神经网络（ANN）的无人机视角地理定位方法计算密集、功耗高，而脉冲神经网络（SNN）具有低功耗优势但尚未被充分探索。同时，脉冲驱动的稀疏计算在表示学习中容易丢失关键信息且难以学习长程依赖关系。

Method: 1. 采用轻量级脉冲驱动Transformer主干提取粗粒度特征；2. 设计脉冲驱动选择性注意力（SSA）块通过门控机制增强选择性特征并突出判别区域；3. 引入脉冲驱动混合状态空间（SHS）块学习长程依赖；4. 提出分层重排序对齐学习（HRAL）策略通过邻域重排序和跨批次一致性优化主干网络。

Result: 实验结果表明，SpikeViMFormer优于现有最先进的SNN方法，与先进的ANN方法相比也达到了具有竞争力的性能，同时推理阶段仅使用主干网络以降低计算成本。

Conclusion: 该研究证明了SNN在无人机视角地理定位任务中的潜力，通过创新的架构设计和学习策略，在保持低功耗优势的同时解决了信息稀疏性和长程依赖问题，为高效地理定位系统提供了新方案。
...
### [213] [dMLLM-TTS: Self-Verified and Efficient Test-Time Scaling for Diffusion Multi-Modal Large Language Models](https://arxiv.org/abs/2512.19433)
...
TL;DR: 提出dMLLM-TTS框架，通过轨迹探索缩放和迭代细化缩放两个互补轴，结合分层搜索算法和自验证反馈机制，显著提升扩散多模态大语言模型的生成质量和效率。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型（dMLLMs）统一了图像生成与理解，但其测试时缩放方法效率低下且依赖外部验证器，限制了生成潜力的充分释放。

Method: 1. 轨迹探索缩放增强生成多样性；2. 迭代细化缩放确保生成稳定性；3. 设计复杂度为O(N+T)的分层搜索算法；4. 引入利用dMLLMs内在图像理解能力的自验证反馈机制。

Result: 在GenEval基准测试中，对三种代表性dMLLMs（Lumina-DiMOO、MMaDA、Muddit）的实验表明，该框架显著提升生成质量，效率比线性搜索提高高达6倍。

Conclusion: dMLLM-TTS框架有效解决了dMLLMs测试时缩放的效率与质量平衡问题，通过算法优化和自验证机制，为多模态生成模型的实用化提供了新思路。
...
### [214] [Non-Contrast CT Esophageal Varices Grading through Clinical Prior-Enhanced Multi-Organ Analysis](https://arxiv.org/abs/2512.19415)
*Xiaoming Zhang,Chunli Li,Jiacheng Hao,Yuan Gao,Danyang Tu,Jianyi Qiao,Xiaoli Yin,Le Lu,Ling Zhang,Ke Yan,Yang Hou,Yu Shi*

Main category: cs.CV

TL;DR: 提出MOON++多模态框架，利用非对比增强CT综合分析食管、肝、脾多器官特征，实现食管静脉曲张无创分级评估，性能优于传统单器官方法。


<details>
  <summary>Details</summary>
Motivation: 食管静脉曲张是肝硬化门脉高压的严重并发症，传统内镜诊断具有侵入性，临床需要可靠的非侵入性替代方案。非对比增强CT虽具潜力但尚未充分利用。

Method: 开发MOON++多模态框架，基于临床证据（器官体积关系与肝病严重程度相关），通过多模态学习综合分析食管、肝、脾的影像特征，使用1631名患者数据训练，按内镜确诊的静脉曲张严重程度分为四级。

Result: 在239例验证集和289例独立测试集上，MOON++在严重分级（G3 vs <G3）的AUC达0.894（传统方法0.803），在中重度分级（≥G2 vs <G2）的AUC达0.921（传统方法0.793），并通过放射科医生读片研究进一步验证性能。

Conclusion: MOON++是首个结合临床先验知识的全面多器官NCCT分析框架，为食管静脉曲张评估提供了有前景的非侵入性诊断替代方案。
...
### [215] [Emotion-Director: Bridging Affective Shortcut in Emotion-Oriented Image Generation](https://arxiv.org/abs/2512.19479)
*Guoli Jia,Junyao Hu,Xinwei Long,Kai Tian,Kaiyan Zhang,KaiKai Zhao,Ning Ding,Bowen Zhou*

Main category: cs.CV

TL;DR: 提出Emotion-Director框架，通过跨模态协作解决情感导向图像生成中的情感语义混淆问题，提升图像情感表达。


<details>
  <summary>Details</summary>
Motivation: 当前情感导向图像生成方法存在情感与语义混淆的‘情感捷径’问题，而情感在广告等领域至关重要，需要更准确的情感表达方法。

Method: 采用跨模态协作框架，包括MC-Diffusion模型（结合视觉和文本提示生成情感图像，并改进DPO优化）和MC-Agent系统（多智能体重写文本提示以增强情感表达）。

Result: 定性和定量实验表明，Emotion-Director在情感导向图像生成方面优于现有方法，能更准确地表达情感。

Conclusion: Emotion-Director有效解决了情感与语义混淆问题，为情感导向图像生成提供了更可靠的解决方案。
...
### [216] [D2Pruner: Debiased Importance and Structural Diversity for MLLM Token Pruning](https://arxiv.org/abs/2512.19443)
*Evelyn Zhang,Fufu Yu,Aoqi Wu,Zichen Wen,Ke Yan,Shouhong Ding,Biqing Qi,Linfeng Zhang*

Main category: cs.CV

TL;DR: 提出D2Pruner框架，通过结合去偏重要性和结构剪枝机制，解决多模态大语言模型中视觉token剪枝在细粒度定位任务上的失败问题，显著提升效率并保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有token剪枝方法在通用理解任务上有效，但在细粒度定位任务上表现灾难性失败，主要因为重要性方法存在位置偏差，多样性方法忽视用户提示和空间冗余。

Method: D2Pruner首先基于去偏注意力分数选择核心token作为枢轴，然后在剩余token上构建表示空间邻近和语义相似性的混合图，通过最大独立集选择迭代保留最重要且可用的token，同时移除其邻居。

Result: 在LLaVA-1.5-7B上，D2Pruner减少74.2% FLOPs同时保持99.2%原性能；在InternVL-2.5-8B的定位任务中，90% token削减率下保持85.7%性能，比现有方法提升高达63.53%。

Conclusion: D2Pruner通过去偏重要性评估和结构感知剪枝，有效解决了现有token剪枝方法的缺陷，在保持高效计算的同时显著提升了细粒度定位任务的性能。
...
### [217] [Dynamic Stream Network for Combinatorial Explosion Problem in Deformable Medical Image Registration](https://arxiv.org/abs/2512.19486)
*Shaochen Bi,Yuting He,Weiming Wang,Hao Chen*

Main category: cs.CV

TL;DR: 本文提出动态流网络（DySNet）解决可变形医学图像配准中的组合爆炸问题，通过动态调整感受野和权重来建模潜在特征关系，实验表明其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 可变形医学图像配准（DMIR）因同时处理两幅图像作为输入导致特征组合关系呈指数增长，模型在特征建模过程中会考虑更多干扰特征组合，需要消除干扰并建模潜在特征关系。

Method: 提出动态流网络（DySNet），包含两个核心创新：1）自适应流池（AdSB）模块动态调整感受野形状，使模型聚焦于相关性更强的特征关系；2）动态流注意力（DySA）机制生成动态权重以寻找更有价值的特征关系。

Result: 大量实验表明，DySNet在多个任务中持续优于当前最先进的DMIR方法，展现出出色的泛化能力。

Conclusion: DySNet通过动态调整感受野和权重，有效解决了DMIR中的组合爆炸问题，能够忽略干扰特征组合并建模潜在特征关系，为医学图像配准提供了更优的解决方案。
...
### [218] [FusionNet: Physics-Aware Representation Learning for Multi-Spectral and Thermal Data via Trainable Signal-Processing Priors](https://arxiv.org/abs/2512.19504)
*Georgios Voulgaris*

Main category: cs.CV

TL;DR: 提出一个物理感知的多光谱表示学习框架FusionNet，通过融合短波红外比值与热红外数据，结合可训练的差分信号处理先验，提升跨光谱条件下的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在多模态视觉信号处理中依赖的归纳偏置与信号形成的物理过程不一致，导致跨光谱和真实场景下的性能脆弱，尤其难以捕捉持续热排放引起的间接环境变化。

Method: 引入物理感知表示学习框架，将地质短波红外比值与热红外数据通过中间融合架构FusionNet结合；在卷积层嵌入可训练差分信号处理先验，采用混合池化策略和更宽的感受野增强跨光谱鲁棒性。

Result: 系统消融实验显示各架构组件均提升性能：DGCNN在SWIR比值上达到88.7%准确率，FusionNet达到90.6%，在五种光谱配置上超越现有基线；迁移学习实验表明ImageNet预训练会降低TIR性能。

Conclusion: 结合物理感知特征选择与原则性深度学习架构能产生鲁棒且可泛化的表示，证明基于第一性原理的信号建模可改善挑战性条件下的多光谱学习效果。
...
### [219] [A Convolutional Neural Deferred Shader for Physics Based Rendering](https://arxiv.org/abs/2512.19522)
*Zhuo He,Yingdong Ru,Qianying Liu,Paul Henderson,Nicolas Pugeault*

Main category: cs.CV

TL;DR: 提出pbnds+：一种基于物理的神经延迟着色管道，使用卷积神经网络减少参数并提升着色和重光照性能，通过能量正则化改善暗光照下的反射效果。


<details>
  <summary>Details</summary>
Motivation: 现有神经渲染方法使用多层感知机（MLP）学习渲染方程，但MLP参数量大导致计算资源需求高、训练复杂且渲染性能下降；数据驱动方法需要大量训练数据，且数据不平衡可能导致模型忽略暗光照等特殊条件。

Method: 1. 使用卷积神经网络替代MLP以减少参数量；2. 提出能量正则化约束暗光照下的模型反射；3. 构建物理基础的神经延迟着色管道。

Result: 实验表明，该方法在着色和重光照任务上优于经典基线方法、最先进的神经着色模型以及基于扩散的方法。

Conclusion: pbnds+通过卷积神经网络和能量正则化，有效解决了神经渲染中的参数效率和暗光照处理问题，实现了更高效且鲁棒的渲染性能。
...
### [220] [Multi-Modal Soccer Scene Analysis with Masked Pre-Training](https://arxiv.org/abs/2512.19528)
*Marc Peral,Guillem Capellera,Luis Ferraz,Antonio Rubio,Antonio Agudo*

Main category: cs.CV

TL;DR: 提出用于分析足球战术镜头视频的多模态架构，通过结合球员轨迹、球员类型和球员图像三种输入模态，在无直接球轨迹信息的情况下完成球轨迹推断、球状态分类和持球者识别三项核心任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度依赖精确的球跟踪或手工启发式规则，难以在真实比赛（存在噪声和遮挡）中鲁棒地分析足球场景。需要一种能融合多模态信息并减少对单一模态依赖的解决方案。

Method: 1. 构建统一框架，整合球员轨迹、球员类型和球员图像裁剪三种模态；2. 使用级联的社会时空Transformer块处理时空动态；3. 提出CropDrop预训练策略（模态特定掩码），减少对图像特征的过度依赖，促进跨模态模式学习。

Result: 在大规模数据集上验证，该方法在所有三项任务（球轨迹推断、球状态分类、持球者识别）上均显著优于现有最优基线模型。

Conclusion: 基于Transformer的多模态架构结合结构化与视觉线索能有效提升足球场景分析性能；现实场景中的掩码策略对多模态学习至关重要。
...
### [221] [SlicerOrbitSurgerySim: An Open-Source Platform for Virtual Registration and Quantitative Comparison of Preformed Orbital Plates](https://arxiv.org/abs/2512.19534)
*Chi Zhang,Braedon Gunn,Andrew M. Read-Fuller*

Main category: cs.CV

TL;DR: 开发了SlicerOrbitSurgerySim开源工具，用于在3D Slicer平台上定量评估和比较不同厂商、尺寸的预成型眼眶植入物适配性，旨在改善术前决策并减少术后并发症。


<details>
  <summary>Details</summary>
Motivation: 预成型眼眶植入物适配不良是术后并发症和翻修手术的主要原因，目前缺乏公开工具和标准化指标来定量比较不同植入物的适配性。

Method: 开发了3D Slicer平台的开源扩展工具，支持交互式虚拟配准、评估和比较多个预成型眼眶植入物，生成可重复的植入物-眼眶距离指标和可视化工具。

Result: 该工具支持患者特异性规划和群体水平统计分析，提供试点研究、样本数据集和详细教程以确保测试透明度与可重复性。

Conclusion: 该工具有助于客观比较植入物设计和放置策略，改善术前决策、减少术中植入物修改，并促进协作研究与外科教育。
...
### [222] [ActAvatar: Temporally-Aware Precise Action Control for Talking Avatars](https://arxiv.org/abs/2512.19546)
...
TL;DR: ActAvatar框架通过文本引导实现相位级动作控制，解决了现有说话头像生成方法中文本跟随能力不足、动作与音频时序对齐差、依赖额外控制信号等问题。


<details>
  <summary>Details</summary>
Motivation: 现有说话头像生成方法存在三大挑战：文本跟随能力不足导致动作多样性受限；动作与音频内容缺乏时序对齐；依赖姿势骨架等额外控制信号。

Method: 提出ActAvatar框架，包含三个核心创新：1) 相位感知交叉注意力(PACA)，将提示分解为全局基础块和时序锚定相位块；2) 渐进式视听对齐，早期层优先文本建立动作结构，深层强调音频细化唇部运动；3) 两阶段训练策略，先在多样数据上建立视听对应，再通过结构化标注微调注入动作控制。

Result: 大量实验表明，ActAvatar在动作控制和视觉质量方面显著优于现有最先进方法。

Conclusion: ActAvatar通过相位级动作控制和渐进式模态对齐，实现了更精确的文本引导说话头像生成，同时保持了高质量的视听同步。
...
### [223] [No Data? No Problem: Robust Vision-Tabular Learning with Missing Values](https://arxiv.org/abs/2512.19602)
*Marta Hasny,Laura Daza,Keno Bressem,Maxime Di Folco,Julia Schnabel*

Main category: cs.CV

TL;DR: 提出RoVTL框架，通过对比预训练和门控交叉注意力机制，解决医学影像与表格数据融合中表格数据缺失的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 大规模医学生物库提供影像和丰富表格数据，但实际应用中表格数据常不完整，需要训练时能利用全部数据、推理时对缺失值鲁棒的方法。

Method: 1. 对比预训练阶段：将表格属性缺失作为数据增强；2. 下游任务微调：使用门控交叉注意力进行多模态融合，结合Tabular More vs. Fewer损失和分离梯度学习。

Result: 在UK Biobank心脏MRI数据上，相比现有方法对表格数据缺失更具鲁棒性；能推广到外部心脏MRI数据集和自然图像领域（汽车广告数据集）。

Conclusion: RoVTL框架能处理0%-100%表格数据可用性，在多领域实现稳定的跨数据完整性性能，代码已开源。
...
### [224] [StoryMem: Multi-shot Long Video Storytelling with Memory](https://arxiv.org/abs/2512.19539)
*Kaiwen Zhang,Liming Jiang,Angtian Wang,Jacob Zhiyuan Fang,Tiancheng Zhi,Qing Yan,Hao Kang,Xin Lu,Xingang Pan*

Main category: cs.CV

TL;DR: 提出StoryMem范式，通过视觉记忆机制将单镜头视频扩散模型转化为多镜头故事讲述器，实现长视频生成的一致性与电影级质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法难以保证长视频的跨镜头一致性与叙事连贯性，需要一种能模拟人类记忆机制、维持长期视觉一致性的解决方案。

Method: 1. 提出Memory-to-Video（M2V）框架，建立动态更新的关键帧记忆库；2. 通过潜在空间拼接和负RoPE偏移将记忆注入预训练的单镜头视频扩散模型；3. 采用语义关键帧选择与美学偏好过滤策略；4. 仅需LoRA微调即可实现多镜头生成。

Result: 1. 在自建基准ST-Bench上取得优异表现；2. 相比现有方法显著提升跨镜头一致性；3. 保持高美学质量与提示词遵循度；4. 支持平滑镜头转场与定制化故事生成。

Conclusion: StoryMem通过显式视觉记忆机制有效解决了长视频生成的连贯性问题，为实现分钟级连贯视频叙事迈出重要一步，框架可自然扩展至多种故事生成应用。
...
### [225] [4D Gaussian Splatting as a Learned Dynamical System](https://arxiv.org/abs/2512.19648)
*Arnold Caleb Asiimwe,Carl Vondrick*

Main category: cs.CV

TL;DR: 提出EvoGS方法，将4D高斯溅射重新解释为连续时间动力系统，通过神经动力场学习场景运动规律，实现更高效、可外推和可组合的动态场景建模。


<details>
  <summary>Details</summary>
Motivation: 传统基于变形的动态场景建模方法存在监督需求密集、缺乏时间外推能力、难以局部控制等问题，需要更高效且物理合理的动态表示方法。

Method: 将高斯表示视为连续演化的物理系统，通过神经动力场学习运动规律，将场景运动建模为动力系统的积分过程，而非逐帧变形。

Result: 在动态场景基准测试中，EvoGS相比基于变形场的方法，在运动连贯性和时间一致性方面表现更优，同时保持实时渲染能力，并能实现时间外推和局部动态控制。

Conclusion: 连续动力系统视角为动态场景建模提供了更高效、可解释且功能丰富的框架，在稀疏监督、时间外推和可控合成方面具有优势。
...
### [226] [Generative diffusion models for agricultural AI: plant image generation, indoor-to-outdoor translation, and expert preference alignment](https://arxiv.org/abs/2512.19632)
*Da Tan,Michael Beck,Christopher P. Bidinosti,Robert H. Gulden,Christopher J. Henry*

Main category: cs.CV

TL;DR: 本文研究基于扩散模型的植物图像生成方法，通过合成图像、室内到室外图像转换和专家偏好对齐微调，解决农业AI数据收集难题。


<details>
  <summary>Details</summary>
Motivation: 农业人工智能依赖大规模高质量植物图像数据，但真实田间数据收集成本高、劳动密集且受季节限制，需要高效的数据生成方法。

Method: 1. 在标注的室内外植物图像上微调Stable Diffusion模型生成文本条件图像；2. 使用DreamBooth文本反演和图像引导扩散实现室内到室外图像转换；3. 基于专家评分训练奖励模型进行偏好引导微调。

Result: 合成图像有效增强训练数据并提升表型分类准确率；转换图像改善了YOLOv8的杂草检测性能；偏好引导微调产生了更稳定且符合专家期望的输出。

Conclusion: 该研究展示了数据高效的生成式流水线在农业AI中的实用路径，通过生成合成数据、跨域转换和专家对齐优化，缓解了真实数据收集的瓶颈。
...
### [227] [Over++: Generative Video Compositing for Layer Interaction Effects](https://arxiv.org/abs/2512.19661)
*Luchao Qi,Jiaye Wu,Jun Myeong Choi,Cary Phillips,Roni Sengupta,Dan B Goldman*

Main category: cs.CV

TL;DR: 提出Over++框架，用于视频合成中生成逼真的半透明环境效果（如阴影、反射），同时保持原始场景不变，无需相机姿态、场景静止或深度监督假设。


<details>
  <summary>Details</summary>
Motivation: 专业视频合成中，艺术家需手动创建前景与背景间的环境交互效果；现有视频生成模型难以在保留输入视频的同时添加此类效果，而视频修复方法要么需要逐帧掩码，要么效果不真实。

Method: 提出增强合成新任务，并开发Over++框架：构建针对该任务的配对效果数据集，采用非配对数据增强策略以保持文本驱动编辑能力，支持可选掩码控制和关键帧引导，无需密集标注。

Result: 在有限数据上训练的Over++能生成多样且逼真的环境效果，在效果生成和场景保持方面均优于现有基线方法。

Conclusion: Over++解决了视频合成中环境效果生成的挑战，实现了文本条件化的逼真效果合成与原始场景保留的平衡，为专业工作流程提供了高效工具。
...
### [228] [VA-$π$: Variational Policy Alignment for Pixel-Aware Autoregressive Generation](https://arxiv.org/abs/2512.19680)
*Xinyao Liao,Qiyuan He,Kai Xu,Xiaoye Qu,Yicong Li,Wei Wei,Angela Yao*

Main category: cs.CV

TL;DR: 提出VA-π框架，通过变分优化对齐自回归生成器与分词器，使用像素空间重建质量作为内在奖励，显著提升图像生成质量


<details>
  <summary>Details</summary>
Motivation: 自回归视觉生成中，分词器训练目标（图像重建）与生成器优化目标（token似然）存在错位，导致生成的token序列可能解码为低质量图像

Method: 1. 提出变分优化框架VA-π，推导证据下界统一像素重建与自回归建模；2. 引入基于强化的对齐策略，将生成器视为策略，以教师强制下的像素重建质量作为内在奖励；3. 无需重新训练分词器或外部奖励模型

Result: 仅用1% ImageNet-1K数据和25分钟微调：LlamaGen-XXL的FID从14.36降至7.65，IS从86.55提升至116.70；文本到图像任务中，LlamaGen从0.306提升至0.339，Janus-Pro从0.725提升至0.744

Conclusion: VA-π能快速适配现有自回归生成器，通过像素级指导有效解决生成器-分词器错配问题，显著提升生成质量且计算高效
...
### [229] [From Indoor to Open World: Revealing the Spatial Reasoning Gap in MLLMs](https://arxiv.org/abs/2512.19683)
*Mingrui Wu,Zhaozhi Wang,Fangjinhua Wang,Jiaolong Yang,Marc Pollefeys,Tong Zhang*

Main category: cs.CV

TL;DR: 本文提出了一个基于多传感器户外数据集的大规模空间智能基准测试，用于评估多模态大语言模型在真实世界中的空间推理能力，发现现有模型过度依赖语言先验而非视觉基础推理。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在语义任务上表现优异，但其空间智能（对构建鲁棒且接地气的AI系统至关重要）发展不足。现有基准测试存在局限：要么关注过于简化的定性推理，要么依赖特定领域的室内数据，缺乏具有可验证度量真值的户外数据集。

Method: 利用配备同步立体相机、激光雷达和IMU/GPS传感器的行人视角视频构建大规模数据集，提供精确的3D度量信息，并自动生成涵盖从定性关系到定量度量及运动学理解的分层空间推理问题。

Result: 评估显示，在结构化室内基准测试中观察到的性能提升在开放世界环境中消失。通过合成异常场景和盲测进一步分析证实，当前多模态大语言模型严重依赖语言先验，而非基于视觉的接地推理。

Conclusion: 该基准测试为诊断多模态大语言模型在空间智能方面的局限性提供了原则性平台，有助于推动物理接地的空间智能发展。
...
### [230] [Visual-Aware CoT: Achieving High-Fidelity Visual Consistency in Unified Models](https://arxiv.org/abs/2512.19686)
*Zixuan Ye,Quande Liu,Cong Wei,Yuanxing Zhang,Xintao Wang,Pengfei Wan,Kun Gai,Wenhan Luo*

Main category: cs.CV

TL;DR: 提出一种增强多模态生成中视觉上下文一致性的方法，通过自适应视觉规划和迭代视觉校正，结合监督微调和强化学习，提升统一模型在多参考生成中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有思维链方法主要关注文本一致性，忽略了多模态生成中与视觉参考图像的视觉上下文一致性，导致关键视觉特征（如人物ID、物体属性、风格）无法保持。

Method: 1) 自适应视觉规划：生成结构化视觉检查清单以确定需要保持一致的视觉元素；2) 迭代视觉校正：基于检查清单进行自我反思并以迭代方式优化生成结果；使用监督微调训练模型进行视觉规划、自我反思和优化，并采用flow-GRPO通过定制化视觉检查奖励进一步强化视觉一致性。

Result: 实验表明，该方法在零样本统一模型和带有文本思维链的模型中均表现更优，在多模态生成中实现了更高的视觉上下文一致性。

Conclusion: 将视觉上下文一致性整合到统一模型的推理过程中，通过结构化视觉规划和迭代校正机制，能有效提升多模态生成的视觉特征保持能力，为多参考生成任务提供了更可靠的解决方案。
...
### [231] [Efficient Vision Mamba for MRI Super-Resolution via Hybrid Selective Scanning](https://arxiv.org/abs/2512.19676)
...
TL;DR: 提出了一种结合多头选择性状态空间模型和轻量级通道MLP的高效MRI超分辨率框架，在显著降低计算成本的同时实现了优异的图像质量。


<details>
  <summary>Details</summary>
Motivation: 高分辨率MRI对诊断至关重要，但采集时间长限制了临床应用；现有深度学习方法在保真度和效率之间存在权衡，需要开发既高效又准确的超分辨率方法。

Method: 提出新颖的超分辨率框架，结合多头选择性状态空间模型（MHSSM）和轻量级通道MLP，采用2D块提取与混合扫描捕获长程依赖，每个MambaFormer块集成MHSSM、深度卷积和门控通道混合。在7T脑部T1 MP2RAGE（n=142）和1.5T前列腺T2w MRI（n=334）数据集上进行评估。

Result: 模型在7T脑部数据上取得SSIM=0.951±0.021、PSNR=26.90±1.41 dB、LPIPS=0.076±0.022、GMSD=0.083±0.017；在前列腺数据上取得SSIM=0.770±0.049、PSNR=27.15±2.19 dB、LPIPS=0.190±0.095、GMSD=0.087±0.013。仅使用0.9M参数和57 GFLOPs，比Res-SRDiff减少99.8%参数和97.5%计算量，同时在准确性和效率上优于SwinIR和MambaIR。

Conclusion: 该框架提供了高效、准确的MRI超分辨率解决方案，在不同数据集上均能增强解剖细节。其低计算需求和最先进的性能显示出强大的临床转化潜力。
...
### [232] [Interact2Ar: Full-Body Human-Human Interaction Generation via Autoregressive Diffusion Models](https://arxiv.org/abs/2512.19692)
*Pablo Ruiz-Ponce,Sergio Escalera,José García-Rodríguez,Jiankang Deng,Rolandos Alexandros Potamias*

Main category: cs.CV

TL;DR: 提出首个端到端文本条件自回归扩散模型Interact2Ar，用于生成包含手部细节的全身人-人交互动作，通过自回归管道和记忆技术增强交互适应能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法因数据限制和学习复杂度常忽略手部动作，且传统扩散模型一次性生成整个序列，难以捕捉人类交互的反应性和适应性特征。

Method: 采用并行分支处理手部运动学，结合自回归管道与新型记忆技术，利用大上下文窗口适应交互可变性，支持时序动作组合和实时扰动适应。

Result: 定量与定性实验表明，Interact2Ar在全身交互生成方面达到最先进性能，并能扩展至多人场景。

Conclusion: 该模型通过整合手部细节和自回归机制，显著提升了交互动作的真实感和表达力，为下游应用提供了灵活框架。
...
### [233] [The Prism Hypothesis: Harmonizing Semantic and Pixel Representations via Unified Autoencoding](https://arxiv.org/abs/2512.19693)
*Weichen Fan,Haiwen Diao,Quan Wang,Dahua Lin,Ziwei Liu*

Main category: cs.CV

TL;DR: 本文发现语义编码器主要捕获低频抽象信息，像素编码器保留高频细节信息，提出Prism假设统一解释编码器行为，并设计UAE模型通过频率调制实现语义结构与像素细节的融合。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态深度表示研究缺乏对编码器频谱特性与功能角色关系的系统性分析，需要建立统一的理论框架来解释不同编码器的行为差异。

Method: 系统分析多种语义和像素编码器的频谱特性，提出Prism假设，并设计Unified Autoencoding (UAE)模型，通过创新的频率带调制器协调语义结构和像素细节。

Result: 在ImageNet和MS-COCO基准测试中，UAE模型实现了语义抽象和像素级保真度的统一，达到最先进的性能表现。

Conclusion: 编码器的功能角色与其特征频谱密切相关，Prism假设为理解跨模态表示提供了统一视角，UAE模型验证了通过频率调制实现多模态特征融合的有效性。
...
### [234] [Towards Reasoning-Preserving Unlearning in Multimodal Large Language Models](https://arxiv.org/abs/2512.17911)
*Hongji Li,Junchi yao,Manjiang Yu,Priyanka Singh,Xue Li,Di Wang,Lijie Hu*

Main category: cs.CL

TL;DR: 本文提出了首个针对推理多模态大语言模型（RMLLMs）的遗忘基准RMLLMU-Bench，并开发了无需训练的推理遗忘方法R-MUSE，在有效消除敏感信息的同时保持模型的一般推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法难以处理RMLLMs的特殊挑战：即使最终答案被遗忘，推理过程中的中间步骤仍可能泄露敏感信息，而过度干预又会损害模型的通用推理能力，且缺乏专门评估推理泄露和推理保留的基准。

Method: 1. 构建RMLLMU-Bench基准，扩展标准遗忘指标，增加推理泄露和推理保留的专门评估；2. 提出R-MUSE方法，通过子空间引导和自适应调控的推理时干预框架，在表示层面同时遗忘答案和推理痕迹，并显式保护一般推理能力。

Result: 系统评估表明，现有MLLMs和大型推理模型的遗忘方法存在明显缺陷：要么在推理过程中遗留大量泄露，要么严重降低推理性能；而R-MUSE在RMLLMU-Bench上实现了遗忘效果与推理保留的显著更好平衡。

Conclusion: RMLLMs的遗忘需要专门方法处理推理级泄露问题，R-MUSE作为无需训练的干预框架，为平衡有效遗忘与推理能力保护提供了可行方案，填补了该领域基准和方法空白。
...
### [235] [Graph-O1 : Monte Carlo Tree Search with Reinforcement Learning for Text-Attributed Graph Reasoning](https://arxiv.org/abs/2512.17912)
*Lihui Liu*

Main category: cs.CL

TL;DR: 提出Graph-O1框架，通过蒙特卡洛树搜索与强化学习结合，实现LLM在文本属性图上的逐步交互式推理，显著提升问答准确性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法在处理文本属性图时存在局限：纯文本方法忽略图结构，图序列化方法受限于LLM上下文长度，导致推理碎片化与精度下降。

Method: 1. 构建基于智能体的GraphRAG框架；2. 融合蒙特卡洛树搜索与端到端强化学习；3. 设计多轮图环境交互机制；4. 通过统一奖励机制训练智能体选择性探索子图。

Result: 在多LLM骨干网络上的实验表明，Graph-O1在准确率、可靠性和可解释性方面均优于现有最优基线方法。

Conclusion: Graph-O1通过结构化探索策略解决了图序列化中的上下文限制问题，为LLM在图结构数据上的复杂推理提供了有效框架。
...
### [236] [Q-KVComm: Efficient Multi-Agent Communication Via Adaptive KV Cache Compression](https://arxiv.org/abs/2512.17914)
*Boris Kriuk,Logic Ng*

Main category: cs.CL

TL;DR: 提出Q-KVComm协议，通过传输压缩的KV缓存表示而非原始文本，解决多智能体LLM系统中上下文信息冗余传输导致的带宽和计算资源消耗问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统中，智能体间重复传输原始文本上下文信息导致带宽和计算资源浪费，传统方法丢弃内部语义表示并强制接收方重新计算相似表示。

Method: Q-KVComm协议结合三项关键技术：自适应分层量化（根据敏感度分析分配可变比特宽度）、混合信息提取（跨内容域保留关键事实）、异构模型校准（建立跨架构通信）。

Result: 在三个问答数据集上的实验表明，Q-KVComm实现5-6倍压缩率的同时保持语义保真度，所有场景的连贯性质量分数均高于0.77，在1.1B-1.5B参数规模的模型中均表现鲁棒，适用于对话问答和多跳推理等实际应用。

Conclusion: 该工作建立了LLM智能体通信的新范式，从基于文本的信息交换转向基于表示的信息交换。
...
### [237] [Supplementary Resources and Analysis for Automatic Speech Recognition Systems Trained on the Loquacious Dataset](https://arxiv.org/abs/2512.17915)
*Nick Rossenbach,Robin Schmitt,Tina Raissi,Simon Berger,Larissa Kleppel,Ralf Schlüter*

Main category: cs.CL

TL;DR: Loquacious数据集旨在替代LibriSpeech等传统ASR数据集，提供跨多领域的标准划分和开放许可，并配套语言模型等资源以提升实用性。


<details>
  <summary>Details</summary>
Motivation: 现有ASR数据集（如LibriSpeech）在领域覆盖、数据划分和许可方面存在局限，需要更全面、开放且便于学术与工业界使用的新基准数据集。

Method: 构建Loquacious数据集，包含明确划分的训练/测试集；配套开发n-gram语言模型、G2P模型和发音词典；在多种ASR架构（不同标注单元和拓扑结构）上进行实验验证。

Result: 实验表明Loquacious数据集能有效支持多种ASR架构的评估，为ASR常见挑战提供了有价值的研究案例。

Conclusion: Loquacious数据集及其配套资源填补了现有ASR基准的不足，具有开放许可和跨领域特性，适合推动ASR技术的研究与应用。
...
### [238] [Learning to Prioritize IT Tickets: A Comparative Evaluation of Embedding-based Approaches and Fine-Tuned Transformer Models](https://arxiv.org/abs/2512.17916)
*Minh Tri LÊ,Ali Ait-Bachir*

Main category: cs.CL

TL;DR: 本文评估了两种IT服务工单优先级分类方法，发现基于嵌入的流水线方法泛化能力有限，而微调的多语言Transformer模型在文本和数值特征处理上表现更优，平均F1分数达78.5%。


<details>
  <summary>Details</summary>
Motivation: IT服务管理（ITSM）中工单优先级分类面临文本噪声大、写作风格主观性强以及类别不平衡等挑战，需要更有效的自动化解决方案。

Method: 比较了两类方法：1）基于嵌入的流水线（结合降维、聚类和传统分类器）；2）微调的多语言Transformer模型（同时处理文本和数值特征）。

Result: 嵌入方法在30种配置中泛化能力有限，聚类未能发现有意义结构，监督模型对嵌入质量敏感；Transformer模型平均F1分数达78.5%，加权Cohen's kappa接近0.80，与真实标签高度一致。

Conclusion: 通用嵌入方法对ITSM数据效果有限，领域适应的Transformer架构在工单优先级分类中更具优势，为实际运维提供了有效解决方案。
...
### [239] [KVReviver: Reversible KV Cache Compression with Sketch-Based Token Reconstruction](https://arxiv.org/abs/2512.17917)
*Aomufei Yuan,Zhiming Wang,Ruijie Miao,Dayu Wang,Yuxuan Tian,Zihan Wang,Yebo Peng,Yuhan Wu,Bairen Yi,Xin Liu,Tong Yang*

Main category: cs.CL

TL;DR: 提出KVReviver，一种基于草图算法的可逆KV缓存压缩方法，解决长上下文LLM部署中KV缓存内存瓶颈问题，能在极低内存开销下保持推理精度。


<details>
  <summary>Details</summary>
Motivation: 随着LLM上下文长度增加，KV缓存的内存需求成为部署和批处理的瓶颈；传统压缩方法永久丢弃或合并低注意力分数token，导致信息不可恢复的丢失（上下文遗忘），严重影响模型信息检索能力。

Method: 基于草图算法设计可逆KV缓存压缩方法，通过额外数据结构支持从压缩状态重建token，在有限内存内实现完整计算。

Result: 在2k长度上下文中，仅需10%的KV缓存预算即可保持端到端推理精度不变；在32k长度上下文中，仅用25%缓存预算达到相当精度（约2%精度损失）。

Conclusion: KVReviver通过可逆压缩有效缓解KV缓存内存瓶颈，在显著减少内存占用的同时保持模型性能，为长上下文LLM部署提供实用解决方案。
...
### [240] [Separating Constraint Compliance from Semantic Accuracy: A Novel Benchmark for Evaluating Instruction-Following Under Compression](https://arxiv.org/abs/2512.17920)
*Rahul Baxi*

Main category: cs.CL

TL;DR: 该研究通过压缩衰减理解测试发现，大语言模型在中等压缩程度下约束遵循性最差，且RLHF训练中的“乐于助人”行为是导致约束违反的主要原因。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在提示压缩下性能下降，但其机制尚不明确，需要系统性地研究压缩对模型约束遵循和语义准确性的影响。

Method: 提出压缩衰减理解测试，独立测量约束遵循性和语义准确性；评估9个前沿大语言模型在8个概念和5个压缩级别下的表现；使用三评委LLM陪审团进行评分；通过RLHF消融实验验证假设。

Result: 发现约束遵循性呈现普遍的U型曲线模式，中等压缩时违反最严重；约束遵循性与语义准确性统计正交；移除“乐于助人”信号后约束遵循性平均提升598%；推理模型比高效模型表现优27.5%。

Conclusion: 研究揭示了RLHF对齐与指令遵循之间的根本矛盾，为改进部署系统提供了可操作的指导。
...
### [241] [ReGal: A First Look at PPO-based Legal AI for Judgment Prediction and Summarization in India](https://arxiv.org/abs/2512.18014)
...
TL;DR: 本文介绍了ReGal框架，将强化学习应用于印度法律AI，探索其在法庭判决预测和解释以及法律文档摘要任务中的应用，尽管性能不及监督和专有模型，但为法律文本的强化学习应用提供了重要见解。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习在法律AI领域的应用潜力，特别是在印度法律背景下，旨在解决法律文本处理中的挑战，如奖励模型对齐、法律语言复杂性和领域适应性问题。

Method: 提出了ReGal框架，结合多任务指令微调和基于AI反馈的强化学习（RLAIF），使用近端策略优化（PPO）算法，在法庭判决预测与解释（CJPE）和法律文档摘要两个任务上进行评估。

Result: 框架在标准评估指标上表现不如监督学习和专有模型，但通过实证和定性分析，展示了强化学习在高风险、长文档法律任务中的应用潜力，并识别了关键挑战。

Conclusion: 研究为未来使用强化学习优化法律推理流程奠定了基础，对构建可解释和自适应的法律AI系统具有更广泛的意义。
...
### [242] [CoPE: A Small Language Model for Steerable and Scalable Content Labeling](https://arxiv.org/abs/2512.18027)
*Samidh Chakrabarti,David Willner,Kevin Klyman,Tiffany Saade,Emily Capstick,Sabina Nong*

Main category: cs.CL

TL;DR: CoPE是一种可政策引导的小型语言模型，通过新颖的训练方法实现快速准确的内容标注，在7个危害领域达到与前沿模型相当或更优的准确率，且模型尺寸仅为1%。


<details>
  <summary>Details</summary>
Motivation: 当前内容审核系统依赖大型模型，存在计算成本高、难以灵活适应政策变化的问题。需要开发小型、高效且能理解政策意图而非简单记忆规则的模型。

Method: 提出矛盾示例训练方法使模型学习政策解释而非记忆；开发双目标注法快速生成无歧义训练数据集；构建9B参数模型可在消费级GPU上运行。

Result: 在7个不同危害领域的评估中，CoPE以仅1%的模型尺寸达到与前沿模型相当或更优的准确率；模型开源且部署门槛低。

Conclusion: CoPE将机器学习任务转化为政策编写任务，为在线平台治理开辟了新设计可能，代表了分类器系统的范式转变。
...
### [243] [Narrative Consolidation: Formulating a New Task for Unifying Multi-Perspective Accounts](https://arxiv.org/abs/2512.18041)
*Roger A. Finger,Eduardo G. Cortes,Sandro J. Rigo,Gabriel de O. Ramos*

Main category: cs.CL

TL;DR: 本文提出叙事整合新任务，旨在将重叠叙事文档合并为统一、连贯且时序合理的文本，并引入时序对齐事件图（TAEG）作为核心方法，在圣经福音书研究中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统多文档摘要方法注重压缩而破坏叙事流，无法满足法律证词、历史记载等重叠叙事文档对时序完整性、内容完备性和细节融合的需求。

Method: 提出时序对齐事件图（TAEG），显式建模事件时序关系与对齐；应用中心性算法选择每个事件最具代表性的表述，并保持正确时序位置。

Result: 在四部圣经福音书实验中，该方法实现完美时序排序（Kendall's Tau=1.000），内容指标显著提升（如ROUGE-L F1提高357.2%）。

Conclusion: 时序结构是叙事整合任务的核心基础，TAEG方法验证了该任务定义的有效性，为重叠叙事文档处理提供了新方向。
...
### [244] [Statistical laws and linguistics inform meaning in naturalistic and fictional conversation](https://arxiv.org/abs/2512.18072)
...
TL;DR: 研究分析了视频聊天和电影中对话的词汇增长模式，发现不同词类的词汇扩展遵循不同的Heaps定律规律。


<details>
  <summary>Details</summary>
Motivation: 对话是社会联系的核心，但现有研究很少从Heaps定律的角度分析对话的动态特征，特别是不同词类对词汇扩展的影响。

Method: 使用Heaps定律分析两种对话数据：1) 陌生人视频聊天记录；2) 电影中虚构人物的对话。重点考察不同词类（如名词、动词等）的词汇扩展模式。

Result: 研究发现词汇扩展模式因词类而异，不同词类在对话中的词汇增长遵循不同的Heaps定律规律。

Conclusion: 对话中的词汇扩展具有词类特异性，这一发现为从行为和语言学角度理解对话动态提供了新视角。
...
### [245] [Training LLMs with LogicReward for Faithful and Rigorous Reasoning](https://arxiv.org/abs/2512.18196)
*Jundong Xu,Hao Fei,Huichi Zhou,Xin Quan,Qijun Huang,Shengqiong Wu,William Yang Wang,Mong-Li Lee,Wynne Hsu*

Main category: cs.CL

TL;DR: 提出LogicReward奖励系统，通过定理证明器确保推理步骤的逻辑正确性，提升LLM推理的可靠性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM训练方法依赖结果反馈，可能导致答案正确但推理过程存在逻辑缺陷，在需要严格逻辑一致性的高风险场景中存在问题。

Method: 1. 设计LogicReward奖励系统，利用定理证明器对推理步骤进行逻辑正确性监督；2. 提出基于软统一的自动形式化方法，减少自然语言歧义，提升形式化质量。

Result: 使用LogicReward训练的8B模型在自然语言推理和逻辑推理任务上分别超越GPT-4o和o4-mini 11.6%和2%，同时提升推理忠实度、在数学和常识推理等未见任务上的泛化能力，且无需真实标签即可提供可靠奖励信号。

Conclusion: LogicReward能有效提升LLM推理的逻辑严谨性和泛化性能，为高风险场景下的可靠推理提供解决方案。
...
### [246] [GeoSense-AI: Fast Location Inference from Crisis Microblogs](https://arxiv.org/abs/2512.18225)
*Deepit Sapru*

Main category: cs.CL

TL;DR: 提出一种从嘈杂微博流中实时地理定位的AI流水线，通过文本分析而非稀疏地理标签推断位置，支持应急情境感知。


<details>
  <summary>Details</summary>
Motivation: 传统依赖地理标签的方法在应急响应中覆盖有限，需要直接从非正式文本中高效提取位置信息以提升危机信息处理能力。

Method: 结合统计性话题标签分割、词性驱动的专有名词检测、灾害词典依赖解析、轻量级命名实体识别及地名知识库消歧，构建低延迟NLP组件。

Result: 与主流NER工具相比，在保持高F1值的同时实现数量级更快的处理速度，成功应用于洪水、疫情等突发事件的可视化地图系统。

Conclusion: 通过领域定制NLP与知识库结合，GeoSense-AI展示了在流式约束下提升应急响应能力的可行性，减少对传统地理标签的依赖。
...
### [247] [InstructNet: A Novel Approach for Multi-Label Instruction Classification through Advanced Deep Learning](https://arxiv.org/abs/2512.18301)
*Tanjim Taharat Aurpa,Md Shoaib Ahmed,Md Mahbubur Rahman,Md. Golam Moazzam*

Main category: cs.CL

TL;DR: 本研究提出InstructNet方法，使用XLNet等Transformer架构对wikiHow的How-to文章进行多标签分类，XLNet达到97.30%准确率。


<details>
  <summary>Details</summary>
Motivation: 搜索引擎中'How To'查询日益普遍，分类教学文本对任务导向学习和知识库构建至关重要，需要有效的多标签分类方法。

Method: 使用wikiHow的11,121条多标签数据，采用XLNet、BERT等Transformer架构进行多标签指令分类，以准确率和宏F1分数评估性能。

Result: XLNet架构表现最佳，准确率达97.30%，微平均和宏平均分数分别为89.02%和93%，在多标签分类中取得显著成果。

Conclusion: XLNet架构在InstructNet方法中效果显著，多层评估策略有助于全面理解模型性能并为未来改进指明方向。
...
### [248] [CTTA-T: Continual Test-Time Adaptation for Text Understanding via Teacher-Student with a Domain-aware and Generalized Teacher](https://arxiv.org/abs/2512.18321)
...
### [249] [LIR$^3$AG: A Lightweight Rerank Reasoning Strategy Framework for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.18329)
*Guo Chen,Junjie Huang,Huaijin Xie,Fei Sun,Tao Jia*

Main category: cs.CL

TL;DR: 本文研究了RAG多跳QA任务中推理模型的策略，提出了轻量级重排推理策略框架LiR³AG，使非推理模型能够通过重构证据链获得推理能力，显著降低计算开销并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型在多跳QA任务中虽然能提升性能，但会带来巨大的计算成本（如token消耗和推理延迟）。需要研究如何平衡性能与效率，让非推理模型也能获得类似能力。

Method: 系统分析了推理模型在RAG中的两种推理模式（上下文锚定推理和知识调和推理），并提出了LiR³AG框架，通过重排检索到的证据构建连贯的推理链，实现推理策略向非推理模型的迁移。

Result: LiR³AG平均减少98%的输出token开销和58.6%的推理时间，同时将8B非推理模型的F1性能提升6.2%-22.5%，使其在RAG任务中超越32B推理模型的性能。

Conclusion: LiR³AG为RAG系统提供了一条实用高效的路径，通过轻量级策略迁移显著降低了计算成本，同时保持了多跳QA任务中的高性能。
...
### [250] [Towards Efficient Agents: A Co-Design of Inference Architecture and System](https://arxiv.org/abs/2512.18337)
...
TL;DR: 提出了AgentInfer框架，通过四个协同组件实现智能体端到端加速，在保持准确性的同时减少50%以上无效token消耗，实现1.8-2.5倍整体加速。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在现实部署中面临严重效率问题，这些低效性并非来自单一模型推理，而是源于推理循环、上下文增长和异构工具交互中累积的系统性延迟。

Method: 提出AgentInfer统一框架，包含四个组件：AgentCollab（分层双模型推理框架）、AgentSched（缓存感知混合调度器）、AgentSAM（基于后缀自动机的推测解码方法）、AgentCompress（语义压缩机制），共同构成自进化引擎。

Result: 在BrowseComp-zh和DeepDiver基准测试中，通过方法协同合作，减少50%以上无效token消耗，实现1.8-2.5倍整体加速，同时保持准确性。

Conclusion: 优化智能体任务完成（而非仅关注单token吞吐量）是构建可扩展、高效且自我改进智能系统的关键。
...
### [251] [LLM-based Few-Shot Early Rumor Detection with Imitation Agent](https://arxiv.org/abs/2512.18352)
*Fengzhu Zeng,Qian Shao,Ling Cheng,Wei Gao,Shih-Fen Cheng,Jing Ma,Cheng Niu*

Main category: cs.CL

TL;DR: 提出了一种结合自主代理和LLM的早期谣言检测框架，在数据稀缺场景下实现高效检测


<details>
  <summary>Details</summary>
Motivation: 早期谣言检测在数据稀缺环境下具有挑战性，现有LLM虽在少样本任务中表现良好，但不适合时序数据且计算成本高

Method: 设计了一个自主代理作为早期时间点决策器，配合训练免费的LLM作为谣言检测器，仅需训练轻量级代理

Result: 在四个真实数据集上的实验表明，该方法提升了LLM性能，在准确性和时效性上超越了现有EARD方法

Conclusion: 该框架为少样本早期谣言检测提供了首个解决方案，实现了高效且准确的检测
...
### [252] [DACE For Railway Acronym Disambiguation](https://arxiv.org/abs/2512.18357)
*El Mokhtar Hribach,Oussama Mechhour,Mohammed Elmonstaser,Yassine El Boudouri,Othmane Kabal*

Main category: cs.CL

TL;DR: 本文提出了DACE框架，通过动态提示、检索增强生成、上下文选择和集成聚合技术，在法语铁路文档的首字母缩写消歧任务中取得了最佳性能（F1=0.9069）。


<details>
  <summary>Details</summary>
Motivation: 技术文本处理中的首字母缩写消歧在专业领域面临高歧义性挑战，特别是在资源有限的情况下，需要更有效的自动化解决方案。

Method: 采用DACE框架，结合动态提示设计、检索增强生成、上下文选择策略和集成聚合方法，增强大语言模型在领域特定任务中的表现。

Result: 在TextMine'26竞赛的法语铁路文档任务中取得第一名，F1分数达到0.9069，显著缓解了模型幻觉问题并提升了低资源场景下的性能。

Conclusion: DACE框架通过自适应上下文学习和外部领域知识注入，有效解决了专业领域首字母缩写消歧问题，为技术文本处理提供了可扩展的解决方案。
...
### [253] [LLM Agents Implement an NLG System from Scratch: Building Interpretable Rule-Based RDF-to-Text Generators](https://arxiv.org/abs/2512.18360)
*Mateusz Lango,Ondřej Dušek*

Main category: cs.CL

TL;DR: 提出一种基于LLM代理协作生成规则代码的神经符号框架，用于RDF到文本生成，无需监督数据，实现可解释、低幻觉的快速文本生成。


<details>
  <summary>Details</summary>
Motivation: 传统RDF到文本生成方法依赖监督训练数据，可能导致幻觉问题且缺乏可解释性，本研究旨在开发无需参考文本、可解释且高效的方法。

Method: 使用多个LLM代理协作生成基于规则的Python代码作为生成器，仅基于RDF三元组，无需反向传播或领域内人工参考文本。

Result: 在WebNLG和OpenDialKG数据上的实验显示，该方法减少了幻觉，仅比微调或提示语言模型在流畅性上略有下降，且生成速度快，仅需单CPU。

Conclusion: 该框架提供了一种可解释、无监督的RDF到文本生成方案，在减少幻觉的同时保持高效性，为神经符号方法的应用提供了新方向。
...
### [254] [SRS-Stories: Vocabulary-constrained multilingual story generation for language learning](https://arxiv.org/abs/2512.18362)
*Wiktor Kamzela,Mateusz Lango,Ondrej Dusek*

Main category: cs.CL

TL;DR: 使用大型语言模型为语言学习者生成个性化故事，仅使用他们已知的词汇，通过上下文阅读来教授新词汇并复习旧词汇，结合间隔重复系统优化学习效果。


<details>
  <summary>Details</summary>
Motivation: 动机是帮助语言学习者通过阅读有趣的故事，在上下文中自然学习新词汇并复习已学词汇，提高学习效率和兴趣。

Method: 方法包括使用大型语言模型生成故事，采用三种故事生成方法和三种词汇约束策略，并在英语、中文和波兰语中进行实验评估。

Result: 结果显示，生成的故事比标准约束束搜索方法生成的文本更具语法正确性、连贯性，并提供更好的词汇使用示例。

Conclusion: 结论是该方法能有效生成个性化故事，优化词汇学习，为语言学习者提供更有效的学习工具。
...
### [255] [AraToken: Optimizing Arabic Tokenization with Normalization Pipeline and Language Extension for Qwen3](https://arxiv.org/abs/2512.18399)
*Mark Kashirskiy,Artiom Lipinski,Ilya Makarov*

Main category: cs.CL

TL;DR: 本文提出了AraToken，一种针对阿拉伯语优化的分词器，通过特定归一化管道处理阿拉伯语正字法变体，并结合语言扩展管道（LEP）将其集成到Qwen3-0.6B模型中，显著提升了阿拉伯语的分词效率和模型性能。


<details>
  <summary>Details</summary>
Motivation: 通用分词器在英语和拉丁语系上训练，对阿拉伯语等形态丰富的语言表现不佳，导致分词序列过长、压缩效率降低，因此需要针对阿拉伯语特点优化分词器。

Method: 基于SentencePiece Unigram算法构建阿拉伯语优化分词器，设计归一化管道处理Alif变体、变音符号和阿拉伯-印度数字；提出语言扩展管道（LEP），通过词汇扩展、平均子词初始化和选择性Transformer层解冻将优化分词器集成到Qwen3-0.6B模型。

Result: 归一化SentencePiece分词器相比未归一化基线降低18%的生育率（1.199 vs 1.35词/标记）；LEP在10万阿拉伯语样本上经过800步训练，将评估损失从8.28降至2.43。

Conclusion: AraToken和LEP有效提升了阿拉伯语的分词效率和模型性能，为阿拉伯语NLP研究提供了实用工具和模型检查点。
...
### [256] [An Agentic AI Framework for Training General Practitioner Student Skills](https://arxiv.org/abs/2512.18440)
*Victor De Marez,Jens Van Nooten,Luna De Bruyne,Walter Daelemans*

Main category: cs.CL

TL;DR: 提出一个基于大语言模型的智能体框架，用于训练全科医学生技能，通过可配置的病例生成、角色驱动的患者对话和基于标准的评估反馈，提升虚拟模拟患者的医学准确性和教育价值。


<details>
  <summary>Details</summary>
Motivation: 当前虚拟模拟患者存在医学准确性不足、角色扮演不一致、病例生成困难和反馈缺乏教育结构等问题，需要一种可扩展且可靠的解决方案来替代资源密集的传统医学培训方法。

Method: 采用智能体框架，统一三个核心组件：(1) 可配置的循证病例生成；(2) 可控的角色驱动患者对话（支持检索增强）；(3) 基于标准的沟通与临床推理评估反馈。在交互式语音咨询场景中实例化，并对14名医学生进行评估。

Result: 参与者反馈显示：对话真实且符合病例设定，难度校准适当，角色性格信号稳定，反馈示例丰富且实用，整体可用性优秀。

Conclusion: 通过分离场景控制、交互控制和基于标准的评估，智能体框架能构建可靠且具有教学价值的虚拟模拟患者培训工具，为医学教育提供可扩展的解决方案。
...
### [257] [Mitigating Spurious Correlations in NLI via LLM-Synthesized Counterfactuals and Dynamic Balanced Sampling](https://arxiv.org/abs/2512.18462)
*Christopher Román Jaimes*

Main category: cs.CL

TL;DR: 提出自动化可扩展的NLI模型去偏方法，通过LF-LMI检测语义伪影、LLM合成高质量对比集、动态平衡采样防止遗忘，显著提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有NLI模型过度依赖虚假相关性，而现有去偏方法存在标注成本高或微调导致灾难性遗忘的问题。

Method: 1. 提出LF-LMI准确检测语义伪影；2. 基于LLM合成多法官验证的高质量对比集；3. 设计动态平衡采样训练策略防止遗忘。

Result: 在挑战性基准测试中，模型一致性从63.5%提升至81.0%，同时保持88.4%的域内准确率，显著优于朴素微调方法。

Conclusion: 该自动化管道能有效缓解NLI模型对虚假相关性的依赖，在提升鲁棒性的同时避免灾难性遗忘，具有可扩展性优势。
...
### [258] [Research on a hybrid LSTM-CNN-Attention model for text-based web content classification](https://arxiv.org/abs/2512.18475)
*Mykola Kuz,Ihor Lazarovych,Mykola Kozlenko,Mykola Pikuliak,Andrii Kvasniuk*

Main category: cs.CL

TL;DR: 提出了一种结合LSTM、CNN和注意力机制的混合深度学习架构，用于提升基于文本的网页内容分类性能，在实验中取得了优于单一模型和BERT的优异结果。


<details>
  <summary>Details</summary>
Motivation: 现有单一架构（如CNN、LSTM或Transformer）在处理文本分类时存在局限性，难以同时有效捕捉局部特征和长距离依赖关系，需要一种能结合多种神经网络优势的混合方法来提升分类性能。

Method: 使用预训练的GloVe词向量表示文本；构建混合架构：CNN层提取局部n-gram特征，LSTM层建模序列依赖关系，注意力机制聚焦关键信息；采用5折交叉验证评估模型。

Result: 混合模型在准确率（0.98）、精确率（0.94）、召回率（0.92）和F1分数（0.93）上表现优异，超越了单一CNN、LSTM和BERT基线模型，同时具备良好的实时处理潜力。

Conclusion: 该混合架构通过结合不同神经网络组件的优势，有效解决了单一模型的局限性，在需要同时处理句法特征和语义理解的文本分类任务中表现出色，为复杂非结构化文本处理提供了可靠方案。
...
### [259] [Teaching and Critiquing Conceptualization and Operationalization in NLP](https://arxiv.org/abs/2512.18505)
*Vagrant Gautam*

Main category: cs.CL

TL;DR: 该论文探讨了NLP领域常用抽象概念（如“可解释性”、“偏见”、“推理”和“刻板印象”）缺乏明确定义的问题，并介绍了一门旨在帮助学生探索这些概念定义与操作化的研讨课程。


<details>
  <summary>Details</summary>
Motivation: NLP研究者经常使用抽象概念而不明确定义，各子领域对这些概念有不同的理解和操作方式，这影响了数据集构建、指标设计和系统评估。

Method: 设计了一门跨学科研讨课程，通过阅读文献、讨论和批判性分析，引导学生探索这些概念的定义、应有含义及测量方法。

Result: 提出了一个教学框架，强调概念化与操作化的重要性，并提供了跨学科的阅读材料以促进深入讨论。

Conclusion: 明确概念定义和操作化方法是NLP研究的基础，跨学科讨论有助于形成更严谨、一致的概念理解与实践。
...
### [260] [Generalization Gaps in Political Fake News Detection: An Empirical Study on the LIAR Dataset](https://arxiv.org/abs/2512.18533)
*S Mahmudul Hasan,Shaily Roy,Akib Jawad Nafis*

Main category: cs.CL

TL;DR: 该研究系统评估了九种机器学习算法在政治虚假信息检测任务上的表现，发现仅依赖文本语言特征存在性能上限，模型复杂度提升无法突破语义模糊性限制。


<details>
  <summary>Details</summary>
Motivation: 语言微妙的政治虚假信息泛滥给自动事实核查系统带来挑战，当前研究过度关注复杂神经架构，但纯文本语言建模的实证局限尚未被充分探索。

Method: 在LIAR基准上系统评估九种机器学习算法，通过隔离词袋模型、TF-IDF等词汇特征和GloVe语义嵌入，分析模型性能上限；使用SMOTE进行合成数据增强实验。

Result: 发现细粒度分类的加权F1分数上限为0.32；线性SVM（准确率0.624）与RoBERTa等预训练Transformer性能相当；树集成模型存在巨大泛化差距（训练准确率>99%，测试仅约25%）；数据增强无实质改善。

Conclusion: 政治事实核查任务中，仅增加模型复杂度而不引入外部知识将导致收益递减，语义模糊性是核心瓶颈，需超越纯文本建模方法。
...
### [261] [LLMs on Drugs: Language Models Are Few-Shot Consumers](https://arxiv.org/abs/2512.18546)
*Alexander Doudkin*

Main category: cs.CL

TL;DR: 首次对大型语言模型进行精神活性提示干预的对照研究，发现毒品相关角色提示会显著降低模型在推理任务中的准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在推理时对角色提示敏感，但此前从未对提示层面的“毒品”干预进行过严格基准测试。

Method: 使用GPT-5-mini和ARC-Challenge数据集，对比四种单句毒品提示（LSD、可卡因、酒精、大麻）与清醒对照组的性能，采用确定性解码、完整日志记录、威尔逊置信区间和费希尔精确检验。

Result: 对照组准确率为0.45；酒精提示降至0.10（p=3.2e-8），可卡因0.21（p=4.9e-4），LSD 0.19（p=1.3e-4），大麻0.30（p=0.041），主要原因是角色提示破坏了模型的标准回答模板。

Conclusion: 角色提示文本类似于“少量可消耗提示”，即使不修改模型权重也能破坏其可靠性，所有实验代码和结果已开源。
...
### [262] [Neologism Learning as a Parameter-Efficient Alternative to Fine-Tuning for Model Steering](https://arxiv.org/abs/2512.18551)
*Sungjoon Park,Varun Ramamurthi,Owen Terry*

Main category: cs.CL

TL;DR: 本文研究语言模型中通过训练新词（neologisms）来引导模型行为的方法，并与LoRA微调进行比较，发现新词训练在相同条件下表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有微调方法（如LoRA）需要较多计算资源且灵活性不足，而训练新词（仅需调整少量参数）可能是一种更高效、灵活的模型行为引导方式。

Method: 1. 训练新词作为代表特定概念的新标记；2. 与低秩适应（LoRA）微调在相同数据和超参数设置下进行对比实验；3. 研究模型对新词的自我语言化（self-verbalization）现象。

Result: 1. 在匹配的训练设置下，新词训练的表现优于微调模型；2. 当被问及新词时，模型偶尔会自创新的词汇来解释该概念。

Conclusion: 新词学习是一种计算效率高、灵活性强的模型行为引导方法，在特定场景下可替代传统微调技术，且模型对新词表现出一定的创造性解释能力。
...
### [263] [From Scratch to Fine-Tuned: A Comparative Study of Transformer Training Strategies for Legal Machine Translation](https://arxiv.org/abs/2512.18593)
*Amit Barman,Atanu Mandal,Sudip Kumar Naskar*

Main category: cs.CL

TL;DR: 本文针对英语-印地语法律机器翻译任务，通过微调预训练模型和从头训练Transformer模型两种策略，证明领域适应能显著提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 印度等多语言国家中，法律文件多以英语撰写，造成语言障碍，影响法律信息获取和司法公正。法律机器翻译（L-MT）可提供规模化解决方案。

Method: 采用两种互补策略：1）微调预训练的OPUS-MT模型进行领域适应；2）使用提供的法律语料从头训练Transformer模型。评估使用SacreBLEU、chrF++、TER、ROUGE、BERTScore、METEOR和COMET等标准指标。

Result: 微调的OPUS-MT模型获得46.03的SacreBLEU分数，显著优于基线模型和从头训练的模型，表明领域适应能有效提升翻译质量。

Conclusion: 领域适应在法律机器翻译中至关重要，L-MT系统有潜力改善多语言环境下的司法可及性和法律透明度。
...
### [264] [On Finding Inconsistencies in Documents](https://arxiv.org/abs/2512.18601)
*Charles J. Lovering,Seth Ebner,Brandon Smock,Michael Krumdick,Saad Rabbani,Ahmed Muhammad,Varshini Reddy,Chris Tanner*

Main category: cs.CL

TL;DR: 本文介绍了FIND基准测试，用于评估语言模型在检测文档不一致性方面的能力，发现GPT-5能检测出64%的人工插入不一致性，并发现了原始文档中作者未察觉的不一致性问题。


<details>
  <summary>Details</summary>
Motivation: 学术、法律和金融领域的专业人士需要审核文档以避免因不一致性带来的经济、声誉和科学成本，语言模型有潜力大幅加速这一审核过程。

Method: 创建FIND基准测试，每个示例由领域专家手动插入不一致性的文档组成，使用语言模型（包括GPT-5）在长而复杂的技术文档中进行不一致性检测。

Result: 最佳模型（GPT-5）恢复了64%的人工插入不一致性；在50篇arXiv论文中，模型提出的196条建议中有136条被判定为作者遗漏的合法不一致性。

Conclusion: 语言模型在文档不一致性检测方面展现出潜力，能发现人类遗漏的问题，但即使最佳模型仍会遗漏近一半的不一致性，表明该任务仍具挑战性。
...
### [265] [LLM-CAS: Dynamic Neuron Perturbation for Real-Time Hallucination Correction](https://arxiv.org/abs/2512.18623)
*Jensen Zhang,Ningyuan Liu,Yijia Fan,Zihao Huang,Qinglin Zeng,Kaitong Cai,Jian Wang,Keze Wang*

Main category: cs.CL

TL;DR: 提出LLM-CAS框架，通过分层强化学习动态修正大语言模型幻觉，无需永久修改参数即可提升事实准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如监督微调、静态参数编辑）存在数据需求大、计算成本高、难以处理上下文依赖错误和灾难性遗忘等问题，限制了LLM在关键应用中的可靠性。

Method: 将实时幻觉修正建模为分层强化学习问题，训练智能体学习策略，在推理时根据当前上下文动态选择临时神经元扰动，实现自适应细粒度修正。

Result: 在多个基准测试中显著提升事实准确性：StoryCloze提升10.98个百分点，TriviaQA提升2.71个百分点，TruthfulQA的MC1分数提升2.06个百分点，优于ITI、CAA和SADI等方法。

Conclusion: LLM-CAS为提升LLM可靠性提供了一种高效且上下文感知的解决方案，未来有望扩展至多模态领域。
...
### [266] [A Comparative Study of Light-weight Language Models for PII Masking and their Deployment for Real Conversational Texts](https://arxiv.org/abs/2512.18608)
*Prabigya Acharya,Liza Shrestha*

Main category: cs.CL

TL;DR: 本研究比较了轻量级模型T5-small和Mistral-Instruct-v0.3在PII掩码任务上的表现，发现它们能达到与前沿大模型相当的性能，同时解决了数据处理和计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 当前前沿大语言模型在PII掩码方面表现良好，但存在数据处理担忧和高计算成本问题，因此探索轻量级模型是否也能达到类似性能。

Method: 在AI4Privacy基准构建的英文数据集上微调T5-small和Mistral-Instruct-v0.3，创建不同数据集变体研究标签标准化和PII表示，涵盖24个标准化PII类别和高粒度设置，使用实体级和字符级指标、类型准确率和完全匹配进行评估。

Result: 两种轻量级模型在PII掩码任务上均达到与前沿LLMs相当的性能；标签标准化能持续提升各架构性能；Mistral在F1和召回率上更高且对PII类型更鲁棒，但生成延迟显著更高；T5在对话文本中鲁棒性稍差，但提供更可控的结构化输出和更低推理成本，已成功应用于实时Discord机器人进行实际PII脱敏。

Conclusion: 轻量级模型能提供有效的PII掩码，同时解决与前沿LLMs相关的数据处理问题，但需在准确性、鲁棒性和计算效率之间权衡；实际应用中，非正式输入会导致性能下降。
...
### [267] [Does It Tie Out? Towards Autonomous Legal Agents in Venture Capital](https://arxiv.org/abs/2512.18658)
*Pierre Colombo,Malik Boudiaf,Allyn Sweet,Michael Desa,Hongxi Wang,Kevin Candra,Syméon del Marmol*

Main category: cs.CL

TL;DR: 本文提出将资本结构核对作为法律AI的基准测试任务，分析了现有代理系统的不足，并提出世界模型架构以实现自动化及更广泛的法律智能应用。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在法律基准测试上虽有进步，但如资本结构核对这类需要多文档推理、严格证据追溯和确定性输出的专业法律工作流，现有代理系统仍无法可靠完成。

Method: 将资本结构核对定义为法律AI的现实基准任务，分析比较现有代理系统性能，并提出一种面向自动化核对的世界模型架构。

Result: 现有代理系统在资本结构核对任务上表现不足，无法满足多文档推理、证据追溯和确定性输出的要求。

Conclusion: 需要专门的世界模型架构来实现资本结构核对等专业法律工作流的自动化，这为应用法律智能提供了基础框架。
...
### [268] [Solver-Independent Automated Problem Formulation via LLMs for High-Cost Simulation-Driven Design](https://arxiv.org/abs/2512.18682)
*Yuchen Li,Handing Wang,Bing Xue,Mengjie Zhang,Yaochu Jin*

Main category: cs.CL

TL;DR: 提出APF框架，利用大语言模型将工程师的自然语言设计需求自动转换为可执行的优化模型，无需依赖求解器反馈，在无高成本仿真反馈的情况下通过数据生成和测试实例标注构建高质量数据集进行微调，显著提升了优化问题公式化的准确性和设计目标达成度。


<details>
  <summary>Details</summary>
Motivation: 在高成本仿真驱动的设计领域，将模糊的设计需求转化为数学优化公式是优化产品性能的瓶颈，该过程耗时且依赖专家知识。现有大语言模型方法要么形式化效果差，要么依赖求解器反馈进行数据筛选，而高仿真成本使得这种反馈不可用。

Method: 提出APF框架，核心是创新的自动生成高质量数据管道，通过数据生成和测试实例标注在无求解器反馈的情况下构建适合的微调数据集，利用该数据集对大语言模型进行监督微调，增强其生成准确且可执行的优化问题公式的能力。

Result: 在天线设计实验中的结果表明，APF在需求形式化的准确性和最终辐射效率曲线满足设计目标的质量方面均显著优于现有方法。

Conclusion: APF框架能够有效解决高成本仿真驱动设计中自动化问题公式化的挑战，通过生成高质量数据集微调大语言模型，实现了无需求解器反馈的准确优化模型生成，提升了设计效率。
...
### [269] [MemEvolve: Meta-Evolution of Agent Memory Systems](https://arxiv.org/abs/2512.18746)
*Guibin Zhang,Haotian Ren,Chong Zhan,Zhenhong Zhou,Junhao Wang,He Zhu,Wangchunshu Zhou,Shuicheng Yan*

Main category: cs.CL

TL;DR: 提出MemEvolve框架，实现智能体经验知识与记忆架构的联合进化，并建立EvolveLab代码库统一现有记忆系统设计。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体依赖静态手工设计的记忆架构，无法根据任务上下文进行元适应，限制了系统的进化能力。

Method: 1. 提出MemEvolve元进化框架，共同进化经验知识与记忆架构；2. 构建EvolveLab统一代码库，将12种代表性记忆系统模块化为编码、存储、检索、管理四个设计维度。

Result: 在四个智能体基准测试中：1. 性能显著提升（如SmolAgent改进达17.06%）；2. 展现强大的跨任务与跨LLM泛化能力。

Conclusion: MemEvolve突破了静态记忆架构的限制，实现了智能体系统与记忆架构的协同进化，EvolveLab为自进化系统研究提供了标准化实验平台。
...
### [270] [From Natural Language to Control Signals: A Conceptual Framework for Semantic Channel Finding in Complex Experimental Infrastructure](https://arxiv.org/abs/2512.18779)
*Thorsten Hellert,Nikolay Agladze,Alex Giovannone,Jan Jug,Frank Mayet,Mark Sherwin,Antonin Sulc,Chris Tennant*

Main category: cs.CL

TL;DR: 本文针对大型实验设施中控制与诊断信号查找困难的问题，提出了一个语义通道查找的四范式框架，并在四个实际设施中验证了其90-97%的查询准确率。


<details>
  <summary>Details</summary>
Motivation: 现代实验设施（如粒子加速器、聚变装置等）积累了数万至数十万个控制与诊断通道，操作员和AI系统依赖非正式的专家知识、不一致的命名规范和碎片化文档来定位信号，这成为可靠性、可扩展性和语言模型驱动接口的持续瓶颈。

Method: 提出了一个四范式框架来解决语义通道查找问题：1) 基于策划通道词典的直接上下文查找；2) 通过结构化树的受限分层导航；3) 使用迭代推理和工具化数据库查询的交互式智能体探索；4) 基于本体的语义搜索，将通道含义与设施特定命名约定解耦。在四个不同规模的运行设施中进行了概念验证实现。

Result: 在四个运行设施（从紧凑型自由电子激光器到大型同步辐射光源）的专家策划操作查询上，实现了90-97%的准确率。

Conclusion: 语义通道查找是复杂实验基础设施中的一个普遍问题，提出的四范式框架能够指导不同数据体制下的架构选择，有效解决了信号定位的瓶颈问题，为可靠、可扩展的系统接口提供了基础。
...
### [271] [From Word to World: Can Large Language Models be Implicit Text-based World Models?](https://arxiv.org/abs/2512.18832)
*Yixia Li,Hongru Wang,Jiahao Qiu,Zhenfei Yin,Dongdong Zhang,Cheng Qian,Zeping Li,Pony Ma,Guanhua Chen,Heng Ji,Mengdi Wang*

Main category: cs.CL

TL;DR: 研究评估了大型语言模型作为世界模型在文本环境中的表现，发现训练充分的模型能保持连贯的潜在状态，随数据和模型规模可预测地扩展，并通过动作验证、轨迹生成和预热强化学习提升智能体性能，但这些收益受行为覆盖和环境复杂度的显著影响。


<details>
  <summary>Details</summary>
Motivation: 现实环境难以扩展且缺乏适应性，而世界模型通过模拟经验可能提升学习效率，但大型语言模型能否可靠地充当世界模型、在何种条件下对智能体产生实质帮助尚不明确。

Method: 在文本环境中将语言建模重新解释为交互下的下一状态预测，提出三层评估框架：保真度与一致性、可扩展性与鲁棒性、智能体效用，并在五个代表性环境中进行实验。

Result: 训练充分的世界模型能保持连贯的潜在状态，随数据和模型规模可预测地扩展，并通过动作验证、合成轨迹生成和预热强化学习提升智能体性能，但收益高度依赖于行为覆盖和环境复杂度。

Conclusion: 世界模型能有效支持智能体学习，但其效果存在明确边界，取决于行为覆盖和环境复杂度，这为实际应用提供了重要指导。
...
### [272] [AraMix: Recycling, Refiltering, and Deduplicating to Deliver the Largest Arabic Pretraining Corpus](https://arxiv.org/abs/2512.18834)
*Sultan Alrashed,Francesco Orabona*

Main category: cs.CL

TL;DR: AraMix是一个去重后的阿拉伯语预训练语料库，包含约1780亿个标记和1.79亿个文档，通过整合和优化现有公开数据集构建而成。


<details>
  <summary>Details</summary>
Motivation: 针对阿拉伯语等低资源语言，现有预训练数据集存在大量冗余，重新爬取网络数据效率低下，因此探索通过系统化重用和优化现有数据集来构建高质量语料库。

Method: 整合七个公开阿拉伯语网络数据集，应用针对阿拉伯语设计的质量过滤方法重新过滤部分数据，并执行跨数据集去重（包括MinHash和句子级去重）。

Result: 发现这些独立收集的语料库中近60%的标记是重复的；构建了目前最大的经过严格过滤的公开阿拉伯语预训练语料库。

Conclusion: 对于低资源语言，投资于现有数据的优化流程比额外进行网络爬取更具回报，这一方法可有效减少冗余并提升语料库质量。
...
### [273] [MDToC: Metacognitive Dynamic Tree of Concepts for Boosting Mathematical Problem-Solving of Large Language Models](https://arxiv.org/abs/2512.18841)
*Tung Duong Ta,Tim Oates*

Main category: cs.CL

TL;DR: 提出MDToC方法，通过构建概念树、验证计算和多数投票，提升大语言模型在数学推理中的计算验证能力，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学推理方面有进步，但在使用现有提示技术时仍难以有效验证计算准确性。

Method: MDToC（元认知动态概念树）方法，包含三个阶段：构建概念树、为每个概念开发准确性验证的计算、使用多数投票评估竞争解决方案。

Result: 在CHAMP、MATH和Game-of-24基准测试中，GPT-4-Turbo分别达到58.1%、86.6%和85%的准确率，比GoT方法分别提升5%、5.4%和4%，且无需手工设计的提示。

Conclusion: MDToC在所有骨干模型上均优于现有提示方法，比ToT提升高达7.6%，比GoT提升6.2%，表明元认知计算验证是增强数学推理的有前景方向。
...
### [274] [Toward Human-Centered AI-Assisted Terminology Work](https://arxiv.org/abs/2512.18859)
*Antonio San Martin*

Main category: cs.CL

TL;DR: 本文提出以人为中心的人工智能框架，旨在增强术语学家的能力而非取代他们，强调在人工智能快速扩散背景下保持专业自主性、减少偏见并维护语言多样性。


<details>
  <summary>Details</summary>
Motivation: 生成式人工智能的快速扩散正在改变术语工作，但其无结构化的采用可能削弱专业自主性、放大偏见并侵蚀语言和概念多样性，因此需要一种以人为中心的方法来引导技术应用。

Method: 基于人工智能和翻译研究，提出一个以人为中心的框架，围绕三个相互关联的维度构建：增强的术语学家、伦理人工智能和以人为中心的设计。

Result: 框架强调高自动化与强人工控制的兼容性，术语学家在偏见缓解中的核心作用，以及围绕术语学家的需求、价值观和福祉设计人工智能工具和工作流程的重要性。

Conclusion: 当前人工智能采用的选择不仅将影响术语实践，还将决定术语和专业知识中准确性、适当性和多样性的保存，因此必须坚持以人为中心的人工智能发展路径。
...
### [275] [Can LLMs Estimate Student Struggles? Human-AI Difficulty Alignment with Proficiency Simulation for Item Difficulty Prediction](https://arxiv.org/abs/2512.18880)
*Ming Li,Han Chen,Yunze Xiao,Jian Chen,Hong Jiao,Tianyi Zhou*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在评估题目难度时与人类认知存在系统性偏差，模型性能提升反而阻碍准确预测，无法模拟学生能力局限。


<details>
  <summary>Details</summary>
Motivation: 教育评估中题目难度估计面临冷启动问题，虽然大语言模型具备超强解题能力，但能否感知人类学习者的认知困难仍是未知。

Method: 对20多个模型在医学知识和数学推理等领域进行大规模实证分析，研究人类与AI在难度认知上的对齐情况。

Result: 发现系统性偏差：模型规模扩大无助于对齐；模型趋向机器共识而非人类认知；高性能反而阻碍难度估计；模型无法模拟学生能力局限；缺乏自我认知能力。

Conclusion: 通用解题能力不意味着理解人类认知困难，当前模型难以用于自动化难度预测，揭示了AI在教育评估中的局限性。
...
### [276] [Remedy-R: Generative Reasoning for Machine Translation Evaluation without Error Annotations](https://arxiv.org/abs/2512.18906)
*Shaomu Tan,Ryosuke Mitani,Ritvik Choudhary,Qiyu Wu,Toshiyuki Sekiya,Christof Monz*

Main category: cs.CL

TL;DR: 提出Remedy-R，一种基于推理的生成式机器翻译评估指标，通过强化学习训练，无需错误标注或大模型蒸馏，能生成可解释的评估分析，并进一步构建评估-修订代理提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 现有自动机器翻译评估指标虽在基准测试中表现良好，但缺乏可解释性，且在真实世界分布外输入下容易失效，需要更透明、鲁棒的评估方法。

Method: 使用强化学习从成对翻译偏好中训练生成式评估模型；模型逐步分析准确性、流畅性和完整性后给出分数；进一步构建Remedy-R代理，利用评估分析修订翻译。

Result: 仅用6万训练样本即在WMT22-24元评估中与顶级标量指标和GPT-4评估器竞争；泛化到其他语言；在分布外压力测试中表现鲁棒；其评估分析可用于改进翻译，代理能持续提升多种模型的翻译质量。

Conclusion: Remedy-R提供了可解释、鲁棒的机器翻译评估，其推理过程能捕获翻译相关信息，并可通过评估-修订流程实际提升翻译质量，为生成式评估指标的实际应用提供了新方向。
...
### [277] [FASTRIC: Prompt Specification Language for Verifiable LLM Interactions](https://arxiv.org/abs/2512.18940)
*Wen-Long Jin*

Main category: cs.CL

TL;DR: 提出FASTRIC提示规范语言，通过显式化自然语言提示中的有限状态机来验证大语言模型多轮交互的符合性，发现不同规模模型存在最优规范形式性的“金发姑娘区域”。


<details>
  <summary>Details</summary>
Motivation: 大语言模型执行复杂多轮交互协议时缺乏形式化规范来验证执行是否符合设计者意图，当前方法依赖隐式理解而非显式规范。

Method: 开发FASTRIC规范语言，将有限状态机的七个要素（最终状态、代理、状态、触发器、角色、初始状态、约束）编码到提示中，通过执行轨迹分析验证符合性，测试了三种规模模型在四种规范形式性等级下的表现。

Result: 不同规模模型存在最优规范形式性范围：DeepSeek-V3.2在L2-L4达到完美符合性（1.00），ChatGPT-5在L3达到峰值（0.90）但在L4崩溃（0.39），Phi4无稳定最优值且方差高（SD=0.16-0.36）。

Conclusion: 规范形式性存在模型特定的“金发姑娘区域”，FASTRIC将多轮交互设计从启发式艺术转变为具有可测量过程保证的系统工程，建立了可验证交互协议的提示规范工程方法。
...
### [278] [Evaluating the Challenges of LLMs in Real-world Medical Follow-up: A Comparative Study and An Optimized Framework](https://arxiv.org/abs/2512.18999)
*Jinyan Liu,Zikang Chen,Qinchuan Wang,Tan Xie,Heming Zheng,Xudong Lv*

Main category: cs.CL

TL;DR: 比较了两种医疗随访聊天机器人系统：端到端LLM系统与模块化流水线系统，发现模块化方法显著提升对话稳定性和信息提取准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型直接应用于医疗随访任务时，由于随访表格的复杂性，常出现对话流程失控和信息提取不准确的问题。

Method: 设计并比较两种系统：端到端LLM系统（对照组）和具有结构化流程控制的模块化流水线系统（实验组）。模块化方法基于任务分解、语义聚类和流程管理。

Result: 模块化方法显著提高对话稳定性和提取准确性，减少46.73%的对话轮次，降低80%至87.5%的token消耗。端到端方法在复杂表格上表现不佳。

Conclusion: 在医疗随访等高风险场景中部署LLM时，需要集成外部控制机制以确保系统可靠性。
...
### [279] [Context-Aware Initialization for Reducing Generative Path Length in Diffusion Language Models](https://arxiv.org/abs/2512.19004)
*Tongyuan Miao,Gary Huang,Kai Jun Han,Annie Jiang*

Main category: cs.CL

TL;DR: 提出一种无需训练的上下文感知初始化方法，通过辅助模型注入提示条件先验来缩短扩散大语言模型的生成轨迹，减少约35%的去噪迭代次数。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然支持并行解码，但推理时需要大量去噪迭代将全掩码初始化转化为连贯文本，导致效率低下。现有加速方法主要改进求解器或采样策略，本文提出通过更接近目标分布的初始化来缩短生成轨迹。

Method: 1) 提出训练无关的接口，通过轻量级辅助模型注入提示条件先验；2) 实例化两种机制：离散令牌注入和表示层嵌入插值；3) 引入基于置信度的重掩码机制处理先验不完美和过早解码问题。

Result: 在GSM8K上的初步实验表明，上下文感知初始化可显著减少约35%的函数评估次数（去噪迭代），但发现朴素的热启动可能降低最终准确率。

Conclusion: 研究揭示了可靠热启动扩散解码需要解决校准、修订机制和表示对齐等关键挑战，并为此提出了后续研究方向。
...
### [280] [DramaBench: A Six-Dimensional Evaluation Framework for Drama Script Continuation](https://arxiv.org/abs/2512.19012)
*Shijian Ma,Yunqi Huang,Yan Lin*

Main category: cs.CL

TL;DR: 提出了首个大规模戏剧剧本续写基准DramaBench，从六个维度全面评估模型能力，并对8个先进模型进行了大规模评测。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法全面评估戏剧剧本续写所需的关键能力，如角色一致性、情节连贯性和戏剧结构保持等。

Method: 结合基于规则的分析、LLM标注和统计指标，构建包含1,103个剧本的大规模基准，进行8,824次评估，并进行统计显著性检验和人工验证。

Result: 对8个先进模型评估显示，65.9%的成对比较具有统计显著性；人工验证在3/5维度上达成实质性一致；六个维度捕获了独立的剧本质量方面（平均|r|=0.020）。

Conclusion: DramaBench为模型改进提供了可操作的维度特定反馈，为创意写作评估建立了严谨标准。
...
### [281] [A Large Language Model Based Method for Complex Logical Reasoning over Knowledge Graphs](https://arxiv.org/abs/2512.19092)
*Ziyan Zhang,Chao Wang,Zhuo Chen,Lei Chen,Chiyi Li,Kai Song*

Main category: cs.CL

TL;DR: 提出ROG框架，结合知识图谱检索与大语言模型推理，解决复杂一阶逻辑查询问题，在标准基准测试中优于现有嵌入方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于嵌入的知识图谱推理方法难以处理包含多运算符、深度推理链或异构模式的复杂查询，需要更鲁棒的解决方案。

Method: ROG框架将复杂查询分解为子查询序列，检索查询相关的紧凑子图作为上下文证据，利用大语言模型进行逐步逻辑推理，无需任务特定的嵌入优化。

Result: 在标准知识图谱推理基准测试中，ROG在平均倒数排名（MRR）上持续优于强嵌入基线，尤其在复杂查询类型上表现突出。

Conclusion: 结合结构化知识图谱检索与LLM驱动的逻辑推理，为复杂知识图谱推理任务提供了鲁棒有效的替代方案。
...
### [282] [Stop saying LLM: Large Discourse Models (LDM) and Artificial Discursive Agent (ADA)?](https://arxiv.org/abs/2512.19117)
*Amar Lakel*

Main category: cs.CL

TL;DR: 本文提出从认识论角度将大型生成模型的分析框架从'大型语言模型'转向'大型话语模型'，最终发展为'人工话语代理'概念，并构建基于现象规律、具身认知和语言结构的三元本体论框架。


<details>
  <summary>Details</summary>
Motivation: 当前对大型生成模型的讨论常陷入'迷恋/恐惧'的二元对立，缺乏对模型本质及其社会嵌入性的理论框架，需要建立更全面的分析视角来指导治理与监管。

Method: 提出本体论三元框架：1) 对现象世界规律的理解；2) 具身认知的结构化；3) 社会历史语境中的语言结构沉淀。通过概念重构将LLM重新定义为LDM和ADA。

Result: 建立了将生成模型视为'话语代理'的理论模型，揭示了模型通过学习语料中固化的人类经验片段来模拟话语投射的本质，为治理提供了理论基础。

Conclusion: 应通过公共审议和多方共治机制（政府、产业、公民社会、学术界）明确人工话语代理在社会空间中的定位、用途与边界，取代简单的二元对立叙事。
...
### [283] [SAP: Syntactic Attention Pruning for Transformer-based Language Models](https://arxiv.org/abs/2512.19125)
*Tzu-Yun Lee,Ding-Yong Hong,Jan-Jan Wu*

Main category: cs.CL

TL;DR: 本文提出Syntactic Attention Pruning (SAP)方法，通过结合句法结构和注意力模式来剪枝Transformer模型的注意力头，在保持性能的同时提升模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法主要依赖模型权重和激活值的数学分析，缺乏对语言结构的考虑，可能导致重要注意力头被误剪，影响模型性能和可解释性。

Method: 提出SAP方法，利用句法结构和注意力模式指导剪枝；引入Candidate Filtering (CF)机制，根据注意力头对模型性能的贡献进行优先级排序，减少剪枝过程中的性能下降。

Result: SAP能有效保留具有高密度强注意力值的关键头，在无需重训练的设置下优于现有剪枝策略，且适用于所有基于Transformer的语言模型。

Conclusion: SAP为模型压缩研究提供了新方向，通过结合语言特征实现高效剪枝，在保持性能的同时增强了模型行为的可解释性。
...
### [284] [AWPO: Enhancing Tool-Use of Large Language Models through Explicit Integration of Reasoning Rewards](https://arxiv.org/abs/2512.19126)
*Zihan Lin,Xiaohan Wang,Hexiong Yang,Jiajun Chai,Jie Cao,Guojun Yin,Wei Lin,Ran He*

Main category: cs.CL

TL;DR: 提出优势加权策略优化（AWPO）框架，通过自适应整合显式推理奖励来增强大语言模型的工具使用能力，在多项基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在训练工具使用大语言模型时主要依赖可验证结果奖励，忽视了显式推理奖励对提升推理和工具利用的潜力，且简单结合推理奖励与结果奖励可能导致次优性能或目标冲突。

Method: 提出AWPO框架，包含方差感知门控和难度感知加权机制，基于群体相对统计量自适应调节推理信号的优势，并采用定制化的裁剪机制以实现稳定优化。

Result: AWPO在标准工具使用基准测试中取得最先进性能，显著优于强基线模型和领先闭源模型；其40亿参数模型在多轮对话准确率上超越Grok-4达16.0%，同时在分布外MMLU-Pro基准上保持泛化能力。

Conclusion: AWPO通过原则性整合显式推理奖励，有效提升了工具使用大语言模型的性能，在参数效率、多轮场景处理和泛化能力方面均表现出色。
...
### [285] [QuCo-RAG: Quantifying Uncertainty from the Pre-training Corpus for Dynamic Retrieval-Augmented Generation](https://arxiv.org/abs/2512.19134)
*Dehai Min,Kailin Zhang,Tongtong Wu,Lu Cheng*

Main category: cs.CL

TL;DR: 提出QuCo-RAG方法，通过预训练数据中的客观统计信息（实体频率与共现）来量化不确定性，替代不可靠的模型内部置信度信号，从而更有效地触发检索以减少大语言模型的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有动态检索增强生成方法依赖模型内部信号（如logits、熵）来判断何时检索，但这些信号不可靠，因为大语言模型通常校准不佳，且常对错误输出表现出高置信度。

Method: 1. 生成前识别低频实体以发现长尾知识缺口；2. 生成中验证实体在预训练语料中的共现情况（零共现常预示幻觉风险）；3. 利用Infini-gram在4万亿token语料上进行毫秒级查询，当不确定性高时触发检索。

Result: 在多跳QA基准测试中，QuCo-RAG比现有最优基线在OLMo-2模型上提升5-12个EM点；在预训练数据未公开的模型（Llama、Qwen、GPT）上可提升最多14个EM点；在生物医学QA上的领域泛化进一步验证了方法的鲁棒性。

Conclusion: 基于语料库的验证为动态RAG提供了一种原则性、实际模型无关的范式，能有效减少幻觉并提升问答性能。
...
### [286] [From Speech to Subtitles: Evaluating ASR Models in Subtitling Italian Television Programs](https://arxiv.org/abs/2512.19161)
*Alessandro Lucca,Francesco Pierri*

Main category: cs.CL

TL;DR: 本文通过评估四种先进ASR模型在意大利长视频字幕生成中的表现，发现现有模型虽无法完全替代人工，但可作为高效辅助工具，并设计了支持人机协同的云基础设施。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统在标准数据集上表现良好，但其在非英语（如意大利语）长视频实际生产环境中的性能尚未充分验证，而字幕对视频可访问性和观众参与至关重要。

Method: 针对意大利媒体公司需求，选取Whisper Large v2等四种前沿ASR模型，在50小时意大利电视节目数据集上进行评估，并与专业人工字幕质量进行对比分析。

Result: 研究表明，现有模型无法满足媒体行业对完全自动化的精度要求，但能显著提升人工字幕制作效率，适合作为辅助工具。

Conclusion: 人机协同（HITL）是当前字幕生产的关键路径，并据此设计了可投入生产的云端工作流基础设施。
...
### [287] [JEPA-Reasoner: Decoupling Latent Reasoning from Token Generation](https://arxiv.org/abs/2512.19171)
*Bingyang Kelvin Liu,Ziyu Patrick Chen*

Main category: cs.CL

TL;DR: 提出JEPA-Reasoner模型，将JEPA架构与生成能力结合，在潜在空间进行推理，并通过Talker模型生成可读文本，解决了传统方法的复合误差问题。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA架构缺乏生成能力，而Transformer等模型在潜在空间推理时仍依赖逐令牌生成，导致复合误差累积且依赖上下文信息。

Method: 1. 提出JEPA-Reasoner模型，增强JEPA的生成能力并在潜在空间推理；2. 引入独立的Talker模型将潜在向量转换为可读句子；3. 解耦潜在空间推理与令牌生成过程。

Result: 模型能够生成混合潜在向量（可能支持多线程推理），并在自回归生成中表现出对复合误差更强的鲁棒性。

Conclusion: 解耦潜在空间推理与令牌生成的方法能有效提升生成模型的鲁棒性，并为多线程推理奠定基础。
...
### [288] [CycleChart: A Unified Consistency-Based Learning Framework for Bidirectional Chart Understanding and Generation](https://arxiv.org/abs/2512.19173)
*Dazhen Deng,Sen Yang,Yuchen He,Yuan Tian,Yingcai Wu*

Main category: cs.CL

TL;DR: 提出CycleChart框架，通过一致性学习实现图表生成与理解的双向统一，提升跨任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有图表任务（如问答、解析、生成）通常孤立研究，阻碍模型学习图表生成与解释之间的共享语义。

Method: 采用以模式为中心的通用任务接口；构建多任务对齐数据集；设计生成-解析一致性目标，通过图表生成与重建实现语义对齐。

Result: 在图表生成、解析和问答任务上取得优异性能，显著提升跨任务泛化能力。

Conclusion: CycleChart推动了通用图表理解模型的发展，验证了一致性学习在统一图表任务中的有效性。
...
### [289] [ChemATP: A Training-Free Chemical Reasoning Framework for Large Language Models](https://arxiv.org/abs/2512.19240)
*Mingxu Zhang,Dazhong Shen,Qi Zhang,Ying Sun*

Main category: cs.CL

TL;DR: 提出ChemATP框架，通过构建原子级文本知识库，使冻结的大型语言模型能够显式检索和推理化学知识，解决了现有方法在化学先验注入与通用推理能力之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在分子科学中表现不佳，因为标准字符串表示缺乏显式化学先验。现有方法面临两难：基于训练的方法将先验注入参数但阻碍知识更新并损害通用推理能力；免训练方法依赖表层提示但无法提供原子级精细先验。

Method: 开发ChemATP框架，构建首个原子级文本知识库，使冻结的LLM能够动态检索和推理化学信息，实现化学知识与推理引擎的解耦。

Result: 实验表明ChemATP显著优于免训练基线方法，并与最先进的基于训练模型相媲美，证明显式先验注入是隐式参数更新的有效替代方案。

Conclusion: 通过解耦化学知识与推理引擎，ChemATP在保持LLM通用智能的同时实现了可解释性和适应性，为分子科学中的AI推理提供了新范式。
...
### [290] [Identifying Features Associated with Bias Against 93 Stigmatized Groups in Language Models and Guardrail Model Safety Mitigation](https://arxiv.org/abs/2512.19238)
*Anna-Maria Gueorguieva,Aylin Caliskan*

Main category: cs.CL

TL;DR: 本研究探讨了大型语言模型对非受保护污名化身份的偏见，发现人类对污名特征（如危险性）的评分与模型偏见相关，使用护栏模型可部分减少偏见，但效果有限且特征影响不变。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型已显示出社会偏见，但对非受保护污名化身份的偏见研究不足，且污名社会特征如何影响模型偏见未知。心理学指出污名有六个共享社会特征，本研究旨在探究这些特征、提示风格和污名类型对模型偏见的影响。

Method: 使用SocialStigmaQA基准测试，包含93个污名化群体和37个社会场景，评估三个LLM（Granite 3.0-8B、Llama-3.1-8B、Mistral-7B）的偏见。分析人类对污名特征的评分（如美学、危险性），并测试各自护栏模型（Granite Guardian 3.0、Llama Guard 3.0、Mistral Moderation API）的偏见缓解效果。

Result: 人类评为高危险性的污名（如帮派成员或HIV感染者）在模型输出中偏见最高（所有模型60%输出），而社会人口污名（如亚裔美国人或老年）偏见最低（11%）。使用护栏模型后，偏见分别减少10.4%、1.4%和7.8%，但偏见特征影响不变，且护栏模型常无法识别提示中的偏见意图。

Conclusion: LLM在涉及污名化群体的场景中存在偏见，护栏模型可部分缓解但效果有限，未来需改进护栏模型以更好减轻偏见，这对LLM应用具有重要影响。
...
### [291] [Auto-Prompting with Retrieval Guidance for Frame Detection in Logistics](https://arxiv.org/abs/2512.19247)
*Do Minh Duc,Quan Xuan Truong,Nguyen Tat Dat,Nguyen Van Vinh*

Main category: cs.CL

TL;DR: 提出了一种结合RAG、少样本提示、CoT推理和Auto-CoT的提示优化框架，用于物流文本中的框架检测任务，相比基线方法推理准确率提升最高达15%。


<details>
  <summary>Details</summary>
Motivation: 提示工程对于使大语言模型适应复杂推理和标注任务至关重要，而无需大量微调。本研究旨在通过结构化提示优化，为物流等特定领域NLP应用提供可扩展的解决方案。

Method: 采用检索增强生成（RAG）、少样本提示、思维链（CoT）推理和自动CoT合成（Auto-CoT）构建提示优化流程，并设计基于LLM的提示优化代理，通过检索示例、性能反馈和内部自评估迭代优化提示。

Result: 优化后的提示（特别是通过Auto-CoT和RAG增强的提示）在真实世界物流文本标注任务中，相比零样本或静态提示基线，推理准确率提升最高达15%。该系统在GPT-4o、Qwen 2.5（72B）和LLaMA 3.1（70B）等多个LLM上均表现出一致的改进。

Conclusion: 结构化提示优化是替代完整微调的可行方案，为在物流等特定领域NLP应用中部署LLM提供了可扩展的解决方案，具有实际应用价值。
...
### [292] [CienaLLM: Generative Climate-Impact Extraction from News Articles with Autoregressive LLMs](https://arxiv.org/abs/2512.19305)
*Javier Vela-Tambo,Jorge Gracia,Fernando Dominguez-Castro*

Main category: cs.CL

TL;DR: 本文提出了CienaLLM框架，利用模式引导的生成式信息提取技术，从新闻文章中零样本提取气候灾害的社会经济影响信息。该框架支持可配置提示、多步流程，并在西班牙干旱新闻提取任务中达到或超过监督基线性能。


<details>
  <summary>Details</summary>
Motivation: 监测气候灾害的社会经济影响需要从海量异构新闻文章中提取结构化信息，传统方法难以灵活适应不同灾害类型、语言和领域，因此需要开发一种无需重新训练即可适配的通用信息提取框架。

Method: 开发了基于模式引导生成式信息提取的模块化框架CienaLLM，采用开放权重大语言模型进行零样本信息提取，支持可配置提示和输出模式、多步流程，并通过大规模因子实验评估模型家族、规模、精度和提示策略对性能的影响。

Result: 响应解析步骤几乎消除了格式错误；更大模型提供最强且最稳定的性能；量化技术以适度精度损失换取显著效率提升；提示策略效果因模型而异；在西班牙干旱新闻提取任务中，CienaLLM的准确率匹配或超过监督基线，但推理成本更高。

Conclusion: CienaLLM的模式驱动和模型无关设计使其可通过修改提示和模式（而非重新训练）适配相关信息提取任务（如其他灾害、领域或语言），为气候影响监测提供了可扩展的解决方案。
...
### [293] [Kunnafonidilaw ka Cadeau: an ASR dataset of present-day Bambara](https://arxiv.org/abs/2512.19400)
*Yacouba Diarra,Panga Azazia Kamate,Nouhoum Souleymane Coulibaly,Michael Leventhal*

Main category: cs.CL

TL;DR: 介绍了Kunkado数据集，一个160小时的班巴拉语ASR数据集，包含真实场景下的语音特征，通过微调模型显著降低了词错误率。


<details>
  <summary>Details</summary>
Motivation: 为支持主要口语语言的鲁棒ASR系统，需要包含真实世界语音特征（如语码转换、不流利、背景噪声）的数据集。

Method: 从马里广播档案编译160小时数据集，使用33.47小时人工审核子集微调Parakeet模型，应用实用转录规范化减少变异性。

Result: 在两个真实测试集上，微调后WER分别从44.47%降至37.12%和从36.07%降至32.33%；人工评估也优于在98小时更干净数据上训练的同类系统。

Conclusion: Kunkado数据集能有效提升ASR在真实场景下的性能，数据和模型已开源以支持口语语言的鲁棒ASR研究。
...
### [294] [HATS: High-Accuracy Triple-Set Watermarking for Large Language Models](https://arxiv.org/abs/2512.19378)
*Zhiqing Hu,Chenxu Zhao,Jiazhong Lu,Xiaolei Liu*

Main category: cs.CL

TL;DR: 提出一种基于三色词汇划分的LLM生成文本水印方法，通过限制采样范围并检测绿色词汇富集与红色词汇贫乏来识别水印，在Llama 2 7B上验证了高检测准确率与文本质量保持。


<details>
  <summary>Details</summary>
Motivation: 为遏制LLM生成文本的滥用，需要有效的水印技术来嵌入隐式信号，但现有方法在检测准确率与文本质量平衡方面存在改进空间。

Method: 在解码阶段将词汇表动态划分为绿/黄/红三组（固定比例），限制采样至绿黄组；检测时重放相同划分，计算绿色富集与红色贫乏的统计量，通过单侧z检验和Fisher方法聚合p值判断水印存在性。

Result: 在Llama 2 7B上的实验表明，该方法在固定误报率下实现了高检测准确率（真阳性率高），同时保持了文本可读性。

Conclusion: 三色划分水印方案能有效平衡检测性能与文本质量，为LLM生成文本的溯源提供了可行方案。
...
### [295] [CodeSimpleQA: Scaling Factuality in Code Large Language Models](https://arxiv.org/abs/2512.19424)
*Jian Yang,Wei Zhang,Yizhi Li,Shawn Guo,Haowen Wang,Aishan Liu,Ge Zhang,Zili Wang,Zhoujun Li,Xianglong Liu,Weifeng Lv*

Main category: cs.CL

TL;DR: 提出了CodeSimpleQA基准测试和CodeSimpleQA-Instruct指令数据集，用于评估和改进代码大语言模型在编程知识事实准确性方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准主要关注代码执行正确性，忽视了编程概念和技术实现的事实准确性，需要专门评估和改进代码LLM的事实准确性。

Method: 构建双语CodeSimpleQA基准测试；创建66M样本的CodeSimpleQA-Instruct指令数据集；开发结合监督微调和强化学习的后训练框架。

Result: 前沿LLM在代码事实准确性方面表现不佳；提出的训练框架相比基础模型有显著改进。

Conclusion: 事实感知的对齐对于开发可靠的代码LLM至关重要，需要专门关注编程知识的事实准确性。
...
### [296] [MobileWorld: Benchmarking Autonomous Mobile Agents in Agent-User Interactive, and MCP-Augmented Environments](https://arxiv.org/abs/2512.19432)
...
TL;DR: 本文介绍了MobileWorld，一个比AndroidWorld更具挑战性的移动应用使用基准测试，包含201个任务和20个应用，强调跨应用交互和新型任务类别，以更好地反映真实世界移动使用场景。


<details>
  <summary>Details</summary>
Motivation: 现有AndroidWorld基准测试因任务简单、应用类别不全、缺乏真实用户指令和混合工具使用场景而趋于饱和，需要更贴近实际移动使用环境的挑战性基准。

Method: 构建包含201个任务的MobileWorld基准，强调长时程跨应用交互任务，引入用户交互和MCP增强任务类别，提供容器化环境和精确功能验证，并开发支持扩展动作空间的规划-执行智能体框架。

Result: 在MobileWorld上，最佳智能体框架和端到端模型成功率分别为51.7%和20.9%，远低于AndroidWorld的90%以上，显示当前模型在用户交互和MCP调用方面存在显著困难。

Conclusion: MobileWorld为下一代移动智能提供了更具挑战性的评估基准，揭示了当前模型的局限性，并为开发更鲁棒的移动智能系统指明了方向。
...
### [297] [Activations as Features: Probing LLMs for Generalizable Essay Scoring Representations](https://arxiv.org/abs/2512.19456)
*Jinwei Chi,Ke Wang,Yu Chen,Xuanye Lin,Qiang Xu*

Main category: cs.CL

TL;DR: 研究发现大语言模型中间层激活在跨提示作文评分中具有强判别力，能适应不同评分标准和作文类型。


<details>
  <summary>Details</summary>
Motivation: 跨提示自动作文评分因评分标准多样性而具挑战性，现有研究多关注模型输出，但中间层激活可能包含有价值信息。

Method: 使用大语言模型中间层激活拟合探针，分析不同模型和输入内容对判别力的影响，计算不同提示下各特质维度的作文方向向量。

Result: 激活在评估作文质量时表现出强判别力；大语言模型能根据不同特质和作文类型调整评估视角，有效处理跨提示评分标准多样性。

Conclusion: 大语言模型中间层激活可为跨提示作文评分提供有效信息，模型具备适应不同评分视角的能力，为改进自动评分系统提供了新思路。
...
### [298] [SiamGPT: Quality-First Fine-Tuning for Stable Thai Text Generation](https://arxiv.org/abs/2512.19455)
...
TL;DR: SiamGPT-32B是基于Qwen3-32B微调的开源泰语大语言模型，通过质量优先策略提升复杂指令下的生成稳定性，在SEA-HELM基准测试中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有开源大语言模型在泰语任务中面临复杂指令下生成不稳定的问题，尽管其英语表现良好，需要专门优化泰语能力。

Method: 基于Qwen3-32B模型，采用质量优先微调策略，结合翻译的高复杂度英语指令数据和泰语适配的AutoIF框架进行监督微调，无需持续预训练或语料扩展。

Result: SiamGPT-32B在指令遵循、多轮对话鲁棒性和语言稳定性方面均有提升，在SEA-HELM基准测试中取得同类开源泰语模型的最佳综合性能。

Conclusion: 通过精心设计的质量优先微调策略，仅用监督微调即可显著提升泰语大语言模型在复杂任务中的表现，为低资源语言模型优化提供了有效方案。
...
### [299] [A Large-Language-Model Framework for Automated Humanitarian Situation Reporting](https://arxiv.org/abs/2512.19475)
*Ivan Decostanzi,Yelena Mejova,Kyriaki Kalimeri*

Main category: cs.CL

TL;DR: 提出一个基于大语言模型的自动化框架，将异构人道主义文档转化为结构化、有证据支撑的报告，通过多模块流程和评估指标验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前人道主义决策依赖的手工报告流程效率低、资源消耗大且不一致，需要自动化解决方案来提升报告生成的速度、准确性和可操作性。

Method: 整合语义文本聚类、自动问题生成、检索增强的答案提取与引用、多级摘要生成和内部评估指标，模拟专家推理流程。

Result: 在13个人道主义事件（含1100多份文档）的评估中，生成问题相关性达84.7%，答案相关性达86.3%，引用精确率与召回率均超76%，人机评估一致性F1分数超过0.80。

Conclusion: 结合大语言模型推理、透明引用和多级评估的框架，能自动生成准确、可验证且具操作价值的人道主义报告，优于现有基线方法。
...
### [300] [Algerian Dialect](https://arxiv.org/abs/2512.19543)
*Zakaria Benmounah,Abdennour Boulesnane*

Main category: cs.CL

TL;DR: 本文介绍了Algerian Dialect数据集，包含45,000条阿尔及利亚阿拉伯语方言的YouTube评论，每条评论都标注了五种情感类别，并附有丰富的元数据。


<details>
  <summary>Details</summary>
Motivation: 针对阿尔及利亚方言公开资源稀缺的问题，旨在支持情感分析、方言阿拉伯语NLP和社交媒体分析的研究。

Method: 使用YouTube Data API从30多个阿尔及利亚新闻和媒体频道收集评论，并手动进行五类情感标注。

Result: 创建了一个大规模、情感标注的阿尔及利亚方言数据集，包含45,000条评论及元数据，已在Mendeley Data公开。

Conclusion: 该数据集填补了阿尔及利亚方言资源的空白，为相关NLP研究提供了重要支持。
...
### [301] [Event Extraction in Large Language Model](https://arxiv.org/abs/2512.19537)
...
TL;DR: 本文综述了事件抽取在LLM时代的发展，提出将事件抽取视为为LLM解决方案提供认知支架的系统组件，并探讨了相关方法、挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM和多模态LLM通过提示和生成能在零样本/少样本设置下输出结构化结果，但LLM事件抽取仍面临幻觉、长上下文时序因果链接脆弱、上下文窗口内知识管理有限等部署缺陷，需要更可靠的事件中心系统。

Method: 将事件抽取视为系统组件，利用事件模式与槽约束进行接地验证，以事件中心结构作为逐步推理的中间表示，通过事件链接实现图增强检索，并利用事件存储扩展上下文窗口外的记忆。综述涵盖文本与多模态任务分类、方法演进（从规则到生成式框架），以及跨语言、低资源和领域特定设置。

Result: 系统梳理了事件抽取的任务体系、方法演进、数据集与评估，提出了以事件为中心的结构化框架，可增强LLM的可靠性、推理能力和长程知识管理。

Conclusion: 事件抽取应从静态抽取演进为结构可靠、智能体就绪的感知与记忆层，为开放世界系统提供认知支架，未来需解决可靠性、长程关联与动态知识更新等核心挑战。
...
### [302] [Increasing the Thinking Budget is Not All You Need](https://arxiv.org/abs/2512.19585)
*Ignacio Iacobacci,Zhaozhi Qian,Faroq AL-Tam,Muhammad AL-Qurishi,Riad Souissi*

Main category: cs.CL

TL;DR: 本文系统研究了大型语言模型的思维预算（推理过程长度）与性能的关系，发现单纯增加思维预算并非最有效的计算资源利用方式，而自洽性和自我反思等配置能更有效地提升准确性。


<details>
  <summary>Details</summary>
Motivation: 新一代具备思维能力的大型语言模型在多项推理基准测试中表现出色，早期研究开始探索推理过程长度（思维预算）对模型性能的影响，但缺乏系统性的比较框架。

Method: 提出系统性研究思维预算作为关键参数的方法，考察其与自洽性、自我反思等多种配置的交互作用，建立同时考虑性能结果和计算成本的平衡比较框架。

Result: 研究发现，单纯增加思维预算并不是最有效的计算资源利用方式；通过自洽性和自我反思等替代配置，反而能获得更准确的响应。

Conclusion: 优化大型语言模型的推理性能不应仅依赖增加思维预算，而应综合考虑自洽性、自我反思等配置策略，在性能与计算成本间取得更好平衡。
...
### [303] [MauBERT: Universal Phonetic Inductive Biases for Few-Shot Acoustic Units Discovery](https://arxiv.org/abs/2512.19612)
*Angelo Ortiz Tandazo,Manel Khentout,Youssef Benchekroun,Thomas Hueber,Emmanuel Dupoux*

Main category: cs.CL

TL;DR: MauBERT是一种基于HuBERT的多语言语音表示模型，通过引入发音特征监督学习，在55种语言上训练，能生成更语境无关的跨语言语音表示。


<details>
  <summary>Details</summary>
Motivation: 现有自监督语音模型在多语言场景下缺乏语言独立的语音表示能力，且对未见语言和随意语音的适应性有限。

Method: 在HuBERT预训练基础上，引入基于发音特征的监督学习（语音到发音特征映射），在55种语言数据上进行多任务训练（预测发音特征或音素）。

Result: 1. ABX测试显示MauBERT比现有SOTA多语言自监督模型产生更语境无关的表示；2. 仅需10小时语音的自监督微调即可有效适应未见语言和随意语音。

Conclusion: 通过发音特征注入语言归纳偏置，为自监督语音模型提供了一种有效的跨语言语音表示学习框架。
...
### [304] [Exploring the features used for summary evaluation by Human and GPT](https://arxiv.org/abs/2512.19620)
*Zahra Sadeghi,Evangelos Milios,Frank Rudzicz*

Main category: cs.CL

TL;DR: 本文研究大语言模型在摘要评估中利用的特征，发现通过指导GPT使用人类评估指标可提升其与人类判断的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽探讨了LLMs与人类评估的对齐，但未深入理解LLMs在特定质量维度评估时利用的特征，且评估分数与指标间的映射关系研究不足。

Method: 通过研究统计和机器学习指标，分析人类与GPT响应对齐的特征，并实验指导GPT采用人类使用的评估指标。

Result: 发现了与人类和GPT响应对齐的特征，并证明指导GPT使用人类评估指标能改进其判断，使其更符合人类响应。

Conclusion: 通过明确引导LLMs采用人类评估指标，可以提升自动化摘要评估与人类判断的一致性，为优化LLMs的评估能力提供了可行路径。
...
### [305] [Diacritic Restoration for Low-Resource Indigenous Languages: Case Study with Bribri and Cook Islands Māori](https://arxiv.org/abs/2512.19630)
...
TL;DR: 该研究比较了两种极度资源匮乏语言（Bribri和库克群岛毛利语）中变音符号恢复的算法，发现微调后的字符级大语言模型表现最佳，且可靠性能需要约1万词的数据量。


<details>
  <summary>Details</summary>
Motivation: 响应语言社区的需求，并探索资源匮乏语境下NLP模型的性能与泛化问题，特别是变音符号恢复这一对NLP任务至关重要的文本规范化形式。

Method: 比较了多种变音符号恢复算法（包括处理声调变音），测试了不同数据量下的性能，对比了不同资源条件下的结果，并探索了变音符号校正任务。

Result: 微调的字符级大语言模型表现最好（得益于其将复杂字符分解为UTF-8字节表示的能力）；大规模多语言模型在数据受限时效果较差；所有模型在约1万词数据量时开始显现可靠性能；零样本方法在所有情况下表现均不佳。

Conclusion: 针对资源匮乏语言的变音符号恢复任务，字符级大语言模型微调是最有效的方法，且需要适度规模的数据支持；零样本方法不适用，多语言模型在数据稀缺时效果有限。
...
### [306] [Exploring Zero-Shot ACSA with Unified Meaning Representation in Chain-of-Thought Prompting](https://arxiv.org/abs/2512.19651)
*Filippos Ventirozos,Peter Appleby,Matthew Shardlow*

Main category: cs.CL

TL;DR: 本文提出了一种基于统一意义表示（UMR）的思维链提示技术，用于零样本方面类别情感分析（ACSA），以解决标注数据稀缺问题，并评估了其在多种模型和数据集上的效果。


<details>
  <summary>Details</summary>
Motivation: 监督学习方法在ACSA任务中依赖大量标注数据，而新领域的标注数据稀缺且成本高昂，因此探索利用大语言模型（LLMs）进行零样本学习成为一种实用替代方案。

Method: 提出一种新颖的思维链（CoT）提示技术，通过中间的统一意义表示（UMR）来结构化ACSA任务的推理过程，并在Qwen3-4B、Qwen3-8B和Gemini-2.5-Pro三种模型及四个数据集上对比了UMR方法与标准CoT基线。

Result: UMR方法的有效性可能依赖于模型，初步结果显示对于中等规模模型（如Qwen3-8B）性能与基线相当，但其在更小模型架构上的适用性仍需进一步研究。

Conclusion: UMR方法在零样本ACSA任务中展现出潜力，但其效果受模型规模影响，未来需要更多研究来验证不同规模模型间的泛化能力。
...
### [307] [GenEnv: Difficulty-Aligned Co-Evolution Between LLM Agents and Environment Simulators](https://arxiv.org/abs/2512.19682)
*Jiacheng Guo,Ling Yang,Peter Chen,Qixin Xiao,Yinjie Wang,Xinzhe Juan,Jiahao Qiu,Ke Shen,Mengdi Wang*

Main category: cs.CL

TL;DR: GenEnv框架通过智能体与生成式环境模拟器之间的难度对齐协同进化游戏，动态生成适应智能体当前能力水平的任务，显著提升大型语言模型智能体的训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型智能体训练受限于真实世界交互数据的高成本和静态性，需要一种数据高效且能自适应智能体能力发展的训练方法。

Method: 提出GenEnv框架，建立智能体与可扩展生成式环境模拟器之间的协同进化机制；模拟器作为动态课程策略，基于α-课程奖励持续生成符合智能体“最近发展区”的任务。

Result: 在五个基准测试中，GenEnv将7B基线模型的性能提升高达40.3%，达到或超越更大模型的平均性能；与Gemini 2.5 Pro离线数据增强相比，使用数据量减少3.3倍的同时获得更好性能。

Conclusion: 通过从静态监督转向自适应模拟，GenEnv为扩展智能体能力提供了一条数据高效的路径，证明了动态环境生成在智能体训练中的有效性。
...
### [308] [Untethered thin dielectric elastomer actuated soft robot](https://arxiv.org/abs/2512.17940)
*Xi Wang,Jing Liu,Siqian Li,Hengtai Dai,Jung-Che Chang,Adam Rushworth,Xin Dong*

Main category: cs.RO

TL;DR: 本文提出了一种无系留薄型软体机器人（UTS-Robot），采用薄型介电弹性体驱动器（TS-DEA），在低电压（220V）下实现共振驱动，适用于狭窄空间作业。


<details>
  <summary>Details</summary>
Motivation: 现有介电弹性体驱动器（DEA）软体机器人通常需要高电压和有线供电，限制了其在狭窄受限环境中的应用能力。

Method: 设计了一种双驱动夹层结构的TS-DEA，通过旋涂和双向拉伸工艺将弹性体层厚度降至8微米；集成柔性板载电子器件实现无系留驱动；采用三向摩擦垫实现运动控制；建立了完整的设计与建模优化框架。

Result: TS-DEA在86Hz共振频率下工作，裸驱动器运动速度达12.36mm/s；集成无系留系统后机器人运动速度为0.5mm/s，尺寸为38mm×6mm，重2.34克。

Conclusion: UTS-Robot实现了低电压无系留驱动，在狭窄复杂环境中具有良好运动能力，为微型软体机器人的实际应用提供了可行方案。
...
### [309] [Real-Time Human-Robot Interaction Intent Detection Using RGB-based Pose and Emotion Cues with Cross-Camera Model Generalization](https://arxiv.org/abs/2512.17958)
*Farida Mohsen,Ali Safa*

Main category: cs.RO

TL;DR: 提出一种用于服务机器人的多模态人机交互意图检测框架，融合2D骨骼姿态和面部表情特征，在资源受限的嵌入式硬件上实现实时性能，并通过数据合成方法解决类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 公共空间的服务机器人需要实时理解人类行为意图以实现自然交互，但现有方法通常依赖RGB-D传感器或GPU加速，难以在资源受限的嵌入式设备上部署，且自然交互数据存在严重的类别不平衡问题。

Method: 提出基于单目RGB视频的相机无关2D骨骼姿态和面部表情特征融合框架；设计MINT-RVAE（多模态循环变分自编码器）合成时序一致的姿态-表情-标签序列以重新平衡数据；在树莓派5（仅CPU）等嵌入式硬件上部署。

Result: 离线评估在跨主体和跨场景协议下取得帧级和序列级AUROC 0.95；在MIRA机器人头上进行跨摄像头真实场景验证（使用不同RGB传感器且环境未在训练中出现），32次实时交互试验中达到91%准确率和100%召回率。

Conclusion: 该多模态方法在跨传感器和跨环境场景中表现出强鲁棒性，离线与部署性能高度一致，适用于广泛的多媒体社交机器人应用。
...
### [310] [Unifying Deep Predicate Invention with Pre-trained Foundation Models](https://arxiv.org/abs/2512.17992)
*Qianwei Wang,Bowen Li,Zhanpeng Luo,Yifan Xu,Alexander Gray,Tom Silver,Sebastian Scherer,Katia Sycara,Yaqi Xie*

Main category: cs.RO

TL;DR: UniPred是一个双层学习框架，统一了自上而下和自下而上的谓词学习方法，通过LLM提出谓词效应分布来监督神经网络从低级数据中学习谓词，同时利用学习到的反馈迭代优化LLM假设，在机器人任务中实现了更高的成功率和更快的学习速度。


<details>
  <summary>Details</summary>
Motivation: 解决长时程机器人任务中连续状态-动作空间和稀疏反馈带来的困难，现有方法要么自上而下依赖基础模型缺乏数据基础，要么自下而上从演示中学习缺乏高层先验，需要一种统一的方法来提升符号世界建模的可扩展性和灵活性。

Method: 提出UniPred双层学习框架：1）使用大语言模型（LLMs）提出谓词效应分布，监督神经网络从低级数据中学习谓词分类器；2）利用学习到的反馈迭代优化LLM假设；3）利用强大的视觉基础模型特征在杂乱场景中学习鲁棒的谓词分类器；4）提出支持超越STRIPS假设的谓词评估方法。

Result: 在五个模拟域和一个真实机器人域中，UniPred相比自上而下方法实现了2-4倍的成功率提升，相比自下而上方法实现了3-4倍的学习速度提升。

Conclusion: UniPred通过统一自上而下和自下而上的学习方法，推进了机器人领域可扩展和灵活的符号世界建模，在复杂任务中表现出优越的性能。
...
### [311] [Robotic VLA Benefits from Joint Learning with Motion Image Diffusion](https://arxiv.org/abs/2512.18007)
...
TL;DR: 提出一种通过运动图像扩散进行联合学习的新策略，增强VLA模型的运动推理能力，在仿真和真实环境中显著提升机器人操作成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通常模仿专家轨迹而缺乏预测性运动推理，限制了其决策能力，需要增强模型对动作选择的推理能力。

Method: 采用双头设计的VLA架构：动作头预测动作块，运动头通过扩散变换器预测基于光流的运动图像；两个头联合训练，使共享的VLM骨干学习结合机器人控制和运动知识的表征。

Result: 在LIBERO基准测试中成功率提升至97.5%，在RoboTwin基准测试中达58.0%，真实世界性能提升23%。

Conclusion: 通过运动图像扩散的联合学习能有效增强大规模VLA的运动推理能力，且不改变标准VLA的推理路径，保持了实时性。
...
### [312] [Embodied4C: Measuring What Matters for Embodied Vision-Language Navigation](https://arxiv.org/abs/2512.18028)
*Tin Stribor Sohn,Maximilian Dillitzer,Jason J. Corso,Eric Sax*

Main category: cs.RO

TL;DR: 提出了Embodied4C基准测试，用于评估视觉语言模型在异构具身平台（自动驾驶车辆、无人机、机械臂）上的推理与导航能力，发现跨模态对齐和指令调优比模型规模更重要，时空推理是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试对具身性（物理平台、传感器配置、模态对齐）如何影响感知、推理和控制的理解有限，需要系统评估视觉语言模型在真实具身任务中的泛化能力。

Method: 构建包含1.1K单次推理问题和58个目标导向导航任务的闭环基准测试，覆盖三种异构具身平台，动态调整传感器配置和环境变化，并引入领域无关查询以防止过拟合。

Result: 评估十个先进视觉语言模型和四个具身控制基线，发现跨模态对齐和指令调优对性能影响显著大于模型规模，空间和时间推理是可靠具身能力的主要瓶颈。

Conclusion: Embodied4C基准测试揭示了当前视觉语言模型在具身推理中的关键限制，强调需要加强时空推理和跨模态对齐，而非单纯扩大模型规模。
...
### [313] [Design and Integration of Thermal and Vibrotactile Feedback for Lifelike Touch in Social Robots](https://arxiv.org/abs/2512.18032)
*Jacqueline Borgstedt,Jake Bhattacharyya,Matteo Iovino,Frank E. Pollick,Stephen Brewster*

Main category: cs.RO

TL;DR: 本文提出一种多模态触觉原型，通过集成热反馈和振动触觉反馈来增强PARO机器人的触觉交互能力，模拟生物生理信号以提升情感共鸣。


<details>
  <summary>Details</summary>
Motivation: 当前仿生社交辅助机器人仅能提供有限的被动触觉交互，缺乏体温、心跳等丰富的触觉线索，限制了其激发情感互动和逼真物理交互的能力。

Method: 设计并集成柔性加热界面和嵌入式执行器，模拟体温、心跳节律和持续呼噜声；通过用户和触觉专家参与的迭代设计与校准，形成可复制的设计指南。

Result: 开发出能够模拟生物生理信号的多模态触觉原型，增强了PARO机器人的触觉真实感和情感互动潜力。

Conclusion: 该研究为开发具有情感共鸣和生物可信触觉交互的社交辅助机器人提供了可复制的设计框架和技术路径。
...
### [314] [Design of a Polymer-based Steerable Cannula for Neurosurgical Applications](https://arxiv.org/abs/2512.18048)
*Nidhi Malhotra,Amber K. Rothe,Revanth Konda,Jaydev P. Desai*

Main category: cs.RO

TL;DR: 本研究探索使用激光微加工技术制造聚酰亚胺（PI）机器人可转向套管，用于神经外科手术。通过该方法制造了外径小至1.5毫米的关节，并对其加载行为进行了实验表征。


<details>
  <summary>Details</summary>
Motivation: 聚合物材料在机器人神经外科工具中具有优势，如减少磁共振成像干扰、增强电动器械安全性以及降低组织损伤。现有制造方法复杂，而激光微加工具有单步、高精度、无需洁净室或刺激性化学品的优点。

Method: 采用激光微加工技术，在聚酰亚胺管上制造切口以降低特定方向的刚度，制造了不同外径的多个关节，并对其加载行为进行了实验表征。

Result: 成功制造了外径小至1.5毫米的聚酰亚胺关节，并对其力学性能进行了实验测试，验证了激光微加工方法在制造微型可转向套管中的可行性。

Conclusion: 激光微加工技术可用于制造聚酰亚胺机器人可转向套管，为神经外科手术提供了一种高精度、低成本的制造方法，具有潜在的应用前景。
...
### [315] [SurgiPose: Estimating Surgical Tool Kinematics from Monocular Video for Surgical Robot Learning](https://arxiv.org/abs/2512.18068)
*Juo-Tung Chen,XinHao Chen,Ji Woong Kim,Paul Maria Scheikl,Richard Jaepyeong Cha,Axel Krieger*

Main category: cs.RO

TL;DR: 提出SurgiPose方法，通过可微分渲染从单目手术视频估计手术器械运动学数据，无需真实运动学标注，用于手术机器人模仿学习。


<details>
  <summary>Details</summary>
Motivation: 模仿学习需要临床数据集，但现有手术视频缺乏运动学数据；单目手术视频资源丰富，需解决从中提取运动学信息的问题以支持大规模机器人学习。

Method: 基于可微分渲染的方法，通过优化器械位姿参数最小化渲染图像与真实图像差异，估计工具轨迹和关节角度；使用dVRK Si平台在组织提升和针拾取任务上验证。

Result: 基于视频估计运动学训练的模仿学习策略，与使用真实运动学数据训练的策略相比，在成功率上表现相当。

Conclusion: 单目视频运动学估计可用于手术机器人学习，为从在线手术数据大规模学习自主手术策略奠定了基础。
...
### [316] [Towards Autonomous Navigation in Endovascular Interventions](https://arxiv.org/abs/2512.18081)
*Tudor Jianu,Anh Nguyen,Sebastiano Fichera,Pierre Berthet-Rayne*

Main category: cs.RO

TL;DR: 该论文提出了一种用于复杂血管环境中自主导丝导航的集成AI驱动框架，通过高保真模拟平台、多模态感知融合和几何建模，显著提升了自主血管内导航的精度和安全性。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死因，而当前血管内介入机器人系统存在启发式控制、自主性低和缺乏触觉反馈等局限性，限制了其精确性和适应性。

Method: 1. 开发高保真实时模拟平台CathSim用于强化学习导管导航；2. 提出融合视觉、运动学和力反馈的专家导航网络；3. 创建开源双平面荧光透视数据集Guide3D；4. 引入基于Transformer的SplineFormer模型，将导丝几何形状预测为连续B样条参数。

Result: 结合高保真模拟、多模态感知融合和几何建模的方法，显著改善了自主血管内导航性能，支持更安全、更精确的微创手术。

Conclusion: 该集成AI框架通过解决数据稀缺、模拟保真度和导航精度等关键挑战，为复杂血管环境中的自主导丝导航提供了有效解决方案，有望推动微创血管介入治疗的发展。
...
### [317] [On Swarm Leader Identification using Probing Policies](https://arxiv.org/abs/2512.18146)
*Stergios E. Bachoumas,Panagiotis Artemiadis*

Main category: cs.RO

TL;DR: 本文提出了一种通过物理交互识别机器人群体中隐藏领导者的新方法iSLI，采用基于TGR-S5的深度强化学习策略，在仿真和实物实验中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 在对抗性任务中，机器人群体常隐藏领导者以保护任务安全，因此需要一种能够通过物理交互准确识别隐藏领导者的方法。

Method: 将交互式群体领导者识别问题建模为部分可观测马尔可夫决策过程，采用近端策略优化算法，并设计了结合时序图关系转换器和简化结构化状态空间序列模型的新型神经网络架构。

Result: 所提方法在仿真中优于基线图神经网络，具有零样本泛化能力，在不同群体规模和速度下保持高识别准确率，实物实验验证了仿真到现实的成功迁移和动态适应性。

Conclusion: 基于TGR-S5的深度强化学习方法能有效解决交互式群体领导者识别问题，具有强泛化能力和实际部署可行性。
...
### [318] [Alternating Minimization for Time-Shifted Synergy Extraction in Human Hand Coordination](https://arxiv.org/abs/2512.18206)
*Trevor Stepp,Parthan Olikkal,Ramana Vinjamuri,Rajasekhar Anguluri*

Main category: cs.RO

TL;DR: 提出一种联合优化框架，直接从单个数据集中学习运动协同模式和稀疏激活系数，无需多数据集，简化了数据收集过程。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段方法需要至少两个数据集来提取候选波形并选择时移模板，使数据收集复杂化，因此需要一种仅需单个数据集的方法。

Method: 采用联合优化框架，同时学习少量协同模式及其稀疏激活系数，通过组稀疏性选择协同模式，逐元素稀疏性控制激活时序，使用交替最小化方法求解。

Result: 仿真实验表明，该方法能够准确重建速度信号，并得到紧凑且可解释的协同模式。

Conclusion: 所提框架仅需单个数据集即可有效识别运动协同模式，简化了数据收集流程，为运动控制和机器人学提供了更实用的工具。
...
### [319] [LLaViDA: A Large Language Vision Driving Assistant for Explicit Reasoning and Enhanced Trajectory Planning](https://arxiv.org/abs/2512.18211)
...
TL;DR: 提出LLaViDA，一种基于视觉语言模型（VLM）的自动驾驶轨迹规划方法，通过两阶段训练（监督微调+轨迹偏好优化）提升场景理解和规划能力，在NuScenes基准测试中优于现有端到端和VLM/LLM基线。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶规划器在恶劣天气、人类行为不可预测或复杂道路布局下泛化能力不足，缺乏超越训练数据的少样本适应能力。

Method: 1. 利用VLM进行物体运动预测、语义接地和思维链推理；2. 采用两阶段训练：监督微调 + 轨迹偏好优化（TPO），注入基于回归的监督信号。

Result: 在NuScenes测试集上，开环轨迹规划任务的平均L2轨迹误差为0.31米，碰撞率为0.10%，超越现有最先进的端到端及VLM/LLM基线方法。

Conclusion: LLaViDA通过结合VLM的推理能力和两阶段优化，显著提升了自动驾驶轨迹规划的泛化性能和安全性，为复杂场景下的规划问题提供了有效解决方案。
...
### [320] [Fractional-order Modeling for Nonlinear Soft Actuators via Particle Swarm Optimization](https://arxiv.org/abs/2512.18213)
*Wu-Te Yang,Masayoshi Tomizuka*

Main category: cs.RO

TL;DR: 本文提出了一种基于分数阶微分方程（FODE）和粒子群优化（PSO）的软体气动执行器建模框架，无需依赖材料数据库即可从实验数据中准确捕捉软材料的非线性动态行为。


<details>
  <summary>Details</summary>
Motivation: 软体气动执行器因其高度非线性和柔顺特性，传统建模方法难以实现高精度建模，需要一种不依赖先验材料数据库的精确建模解决方案。

Method: 采用分数阶微分方程（FODE）建立软材料动态行为模型，通过粒子群优化（PSO）算法直接从实验数据中识别模型未知参数，无需预定义材料本构关系。

Result: 实验验证表明，该建模框架能有效表征软体执行器的复杂变形现象，相比传统建模技术具有更高的预测精度和鲁棒性。

Conclusion: 该框架为软体执行器建模提供了一种数据高效且不依赖数据库的解决方案，提升了软体机器人系统设计的精度和适应性。
...
### [321] [On The Computational Complexity for Minimizing Aerial Photographs for Full Coverage of a Planar Region](https://arxiv.org/abs/2512.18268)
*Si Wei Feng*

Main category: cs.RO

TL;DR: 本文研究了无人机航拍中目标区域高效覆盖问题的计算复杂性，通过几何抽象证明了多数相关问题是计算难解的。


<details>
  <summary>Details</summary>
Motivation: 随着无人机技术的普及，航拍在环境监测、结构检测、执法等场景中广泛应用，但如何在图像分辨率和拍摄数量限制下高效完整覆盖目标区域成为核心挑战。

Method: 将航拍问题抽象为计算几何中的覆盖问题，通过理论分析探讨多个基础问题的计算复杂性。

Result: 研究证明大多数航拍覆盖问题在计算上是难解的，意味着传统算法无法高效求解。

Conclusion: 该工作的结论可推广至农药喷洒、战略传感器部署等更广泛的应用领域。
...
### [322] [Reinforcement Learning Position Control of a Quadrotor Using Soft Actor-Critic (SAC)](https://arxiv.org/abs/2512.18333)
*Youssef Mahran,Zeyad Gamal,Ayman El-Badawy*

Main category: cs.RO

TL;DR: 提出一种基于强化学习的四旋翼推力矢量控制架构，相比传统RPM直接控制方法，训练更快且路径跟踪更平滑精确。


<details>
  <summary>Details</summary>
Motivation: 现有文献多直接控制四个旋翼的RPM，本文旨在通过控制四旋翼的推力矢量来改进控制性能。

Method: 使用Soft Actor-Critic强化学习算法训练智能体，智能体计算总推力在z轴的比例及期望滚转/俯仰角，再结合当前偏航角通过PID控制器映射为电机RPM。

Result: 训练结果显示推力矢量控制器比传统RPM控制器训练更快；仿真表明其路径跟踪更平滑、更精确。

Conclusion: 所提出的强化学习推力矢量控制架构在训练效率和路径跟踪性能上优于传统RPM直接控制方法。
...
### [323] [Dynamic Entropy Tuning in Reinforcement Learning Low-Level Quadcopter Control: Stochasticity vs Determinism](https://arxiv.org/abs/2512.18336)
*Youssef Mahran,Zeyad Gamal,Ayman El-Badawy*

Main category: cs.RO

TL;DR: 本文研究了动态熵调谐在训练随机策略的强化学习算法中的影响，并与训练确定性策略的算法进行了性能比较，特别关注在四旋翼飞行器控制中的应用。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索随机策略与确定性策略在强化学习中的性能差异，特别是通过动态熵调谐来改善随机策略在连续控制任务（如四旋翼飞行器控制）中的探索效率和稳定性。

Method: 采用Soft Actor-Critic (SAC) 算法作为随机策略方法，Twin Delayed Deep Deterministic Policy Gradient (TD3) 作为确定性策略方法，比较了静态熵、动态熵调谐的随机策略与确定性策略在四旋翼飞行器控制任务中的表现。

Result: 训练和仿真结果表明，动态熵调谐在控制四旋翼飞行器时具有积极效果，能够防止灾难性遗忘并提高探索效率。

Conclusion: 动态熵调谐在强化学习的随机策略训练中能有效提升连续控制任务的性能，尤其在改善探索和防止遗忘方面优于静态熵和确定性策略方法。
...
### [324] [Learning Semantic Atomic Skills for Multi-Task Robotic Manipulation](https://arxiv.org/abs/2512.18368)
*Yihang Zhu,Weiqing Wang,Shijie Wu,Ye Shi,Jingya Wang*

Main category: cs.RO

TL;DR: 提出AtomSkill框架，通过构建语义原子技能库和关键姿态想象机制，解决多任务模仿学习中技能复用与泛化难题。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在单任务机器人操作中表现优异，但在多任务场景下面临演示数据次优、轨迹噪声和行为多模态等挑战。现有基于技能的方法依赖固定长度分割或环境先验，限制了语义一致性和跨任务泛化能力。

Method: 1. 构建语义原子技能库：通过夹爪状态关键帧检测和视觉语言模型标注，将演示分割为变长技能，并利用对比学习确保技能嵌入的语义一致性和时序连贯性；2. 设计带关键姿态想象的动作生成模块：联合预测技能的长期终端关键姿态和即时动作序列，实现高层运动目标与精细控制的同步推理。

Result: 在仿真和真实环境的大量实验中，AtomSkill在多样化操作任务上均优于现有最优方法。

Conclusion: AtomSkill通过结构化原子技能空间和关键姿态想象机制，实现了可组合、语义一致且泛化能力强的多任务机器人操作策略，为模仿学习的规模化应用提供了新思路。
...
### [325] [AOMGen: Photoreal, Physics-Consistent Demonstration Generation for Articulated Object Manipulation](https://arxiv.org/abs/2512.18396)
*Yulu Wu,Jiujun Cheng,Haowen Wang,Dengyang Suo,Pei Ren,Qichao Mao,Shangce Gao,Yakun Huang*

Main category: cs.RO

TL;DR: 提出AOMGen框架，通过单次真实扫描和演示生成多样化铰接物体操作数据，显著提升VLA策略在未见物体上的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA和世界模型方法依赖大量真实演示数据，尤其是铰接物体精细操作数据收集成本高昂，需要一种可扩展的数据生成方案。

Method: 基于单次真实扫描、演示和数字资产库，生成物理状态验证的光照真实训练数据，通过系统变化相机视角、物体风格和位姿扩展数据多样性，提供同步多视角RGB、动作指令及关节/接触状态标注。

Result: 在AOMGen数据上微调的VLA策略成功率从0%提升至88.7%，并在未见物体和布局上验证了泛化能力。

Conclusion: AOMGen框架能高效生成铰接操作训练数据，显著降低数据收集成本，提升VLA策略在复杂操作任务中的泛化性能。
...
### [326] [When Robots Say No: The Empathic Ethical Disobedience Benchmark](https://arxiv.org/abs/2512.18474)
*Dmytro Kuzmenko,Nadiya Shvai*

Main category: cs.RO

TL;DR: 提出Empathic Ethical Disobedience (EED) Gym测试平台，用于评估机器人拒绝行为的安全性与社会可接受性，发现动作屏蔽可消除不安全服从，解释性拒绝有助于维持信任。


<details>
  <summary>Details</summary>
Motivation: 现有安全强化学习基准主要关注物理风险，而人机交互信任研究规模小且难以复现，需要标准化测试平台来平衡服从、安全与社会期望。

Method: 构建EED Gym测试平台，包含多种场景、人物角色配置，基于小插曲研究建立信任与归责模型，通过动作屏蔽、解释性拒绝等策略评估风险、情感与信任的权衡。

Result: 动作屏蔽能完全消除不安全服从；解释性拒绝可维持信任；建设性风格最受信任，共情风格最具同理心；安全RL方法提升鲁棒性但易导致过度谨慎。

Conclusion: EED Gym为拒绝行为与信任研究提供可复现评估框架，开源代码与配置将促进系统性人机交互研究发展。
...
### [327] [STORM: Search-Guided Generative World Models for Robotic Manipulation](https://arxiv.org/abs/2512.18477)
*Wenjun Lin,Jensen Zhang,Kaitong Cai,Keze Wang*

Main category: cs.RO

TL;DR: STORM框架结合扩散动作生成、条件视频预测和搜索规划，用于机器人操作中的时空推理，在SimperEnv基准测试中达到51.0%的平均成功率，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型依赖抽象潜在动态或将推理委托给语言组件，缺乏基于显式视觉推演的规划能力，需要更可解释、前瞻驱动的决策方法。

Method: 使用扩散式VLA策略生成多样化候选动作，通过生成式视频世界模型模拟视觉和奖励结果，并利用蒙特卡洛树搜索进行前瞻评估和选择性规划优化。

Result: 在SimperEnv操作基准测试中达到51.0%的平均成功率（SOTA）；奖励增强的视频预测将Frèchet视频距离降低75%以上；系统展现出强大的重规划和故障恢复能力。

Conclusion: 搜索引导的生成式世界模型通过显式视觉推演实现可解释的规划，显著提升了长时程机器人操作的时空保真度、任务相关性和鲁棒性。
...
### [328] [Systematic Benchmarking of SUMO Against Data-Driven Traffic Simulators](https://arxiv.org/abs/2512.18537)
*Erdao Liang*

Main category: cs.RO

TL;DR: 本文系统性地将基于模型的微观交通仿真器SUMO与最先进的数据驱动仿真器进行基准测试，使用大规模真实世界数据集评估其在短期和长期闭环仿真中的表现。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要可靠且可扩展的仿真工具，当前缺乏对基于模型仿真器与数据驱动仿真器在真实世界场景下的系统性比较，特别是长期仿真稳定性方面的评估。

Method: 开发了Waymo2SUMO自动化流水线，将Waymo开放运动数据集转换为SUMO仿真场景；使用Waymo开放仿真智能体挑战赛基准，在短期（8秒）和长期（60秒）闭环仿真设置下评估SUMO性能。

Result: SUMO在WOSAC基准测试中获得0.653的真实性元指标，仅需少于100个可调参数；长期推演显示SUMO保持较低的碰撞和偏离道路率，并表现出比代表性数据驱动仿真器更强的长期稳定性。

Conclusion: 基于模型和数据驱动的自动驾驶仿真方法具有互补优势，SUMO在长期仿真稳定性方面表现优异，为自动驾驶仿真和基准测试提供了有价值的工具选择。
...
### [329] [Offline Reinforcement Learning for End-to-End Autonomous Driving](https://arxiv.org/abs/2512.18662)
*Chihiro Noguchi,Takaki Yamamoto*

Main category: cs.RO

TL;DR: 提出一种仅使用摄像头的端到端离线强化学习框架，通过行为正则化提升自动驾驶轨迹预测的安全性和稳定性，在nuScenes数据集上验证了优于模仿学习基线的性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型依赖模仿学习，存在固有失败模式；在线强化学习计算成本高，而离线强化学习虽数据效率高但易因分布外动作产生不稳定。

Method: 构建基于固定模拟器数据集的离线强化学习框架，利用专家驾驶日志生成伪真实轨迹作为行为正则化信号，抑制不安全行为模仿并稳定价值学习。

Result: 在神经渲染环境中评估，该方法在碰撞率和路线完成率上相比模仿学习基线有显著提升。

Conclusion: 离线强化学习结合行为正则化能有效解决端到端自动驾驶中的模仿学习局限，实现更安全可靠的轨迹预测。
...
### [330] [CauTraj: A Causal-Knowledge-Guided Framework for Lane-Changing Trajectory Planning of Autonomous Vehicles](https://arxiv.org/abs/2512.18703)
*Cailin Lei,Haiyang Wu,Yuxiong Ji,Xiaoyu Cai,Yuchuan Du*

Main category: cs.RO

TL;DR: 提出了一种融合因果先验知识的轨迹规划框架，通过因果推断量化换道场景中的车辆交互风险，并嵌入模型预测控制中，使自动驾驶轨迹更接近人类驾驶行为。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹规划模型大多未融入人类驾驶员的先验知识，难以在混合交通中实现拟人化驾驶，因此需要将因果知识引入控制过程以提升轨迹规划性能。

Method: 构建分阶段因果图捕捉换道场景的因果依赖，利用因果推断（ATE和CATE）估计车辆间因果效应，将因果先验嵌入模型预测控制（MPC）框架进行轨迹规划。

Result: 因果推断可解释且稳定地量化车辆交互；个体因果效应揭示驾驶员异质性；相比基线MPC，轨迹最大偏差从1.2米降至0.2米，横向速度波动减少60%，横摆角变化降低50%。

Conclusion: 该方法为拟人化轨迹规划提供方法论支持，在提升自动驾驶安全性、稳定性和仿真真实性方面具有实用价值。
...
### [331] [DSO-VSA: a Variable Stiffness Actuator with Decoupled Stiffness and Output Characteristics for Rehabilitation Robotics](https://arxiv.org/abs/2512.18712)
*Maozeng Zhang,Ke Shi,Huijun Li,Tongshu Chen,Jiejun Yan,Aiguo Song*

Main category: cs.RO

TL;DR: 本文提出了一种用于康复机器人的解耦刚度与输出的可变刚度驱动器，通过可变长度杠杆和行星齿轮系统实现线性扭矩-偏转关系和连续刚度调节，并开发了级联PI控制器以抑制刚度波动。


<details>
  <summary>Details</summary>
Motivation: 中风导致的上肢运动功能障碍需要安全透明的康复机器人，但现有可变刚度驱动器的刚度与偏转角耦合，增加了建模和控制难度。

Method: 设计结合可变长度杠杆和摆线直线机构的可变刚度机制实现线性扭矩-偏转关系；采用基于行星齿轮系统的差动传动机制实现双电机负载共享；开发基于差动配置的级联PI控制器。

Result: 原型机通过刚度校准、调节、扭矩控制、解耦特性和双电机负载共享实验验证，表现出稳定的性能和连续刚度调节能力。

Conclusion: 该驱动器具有解耦特性和连续刚度调节能力，适用于康复外骨骼和其他物理人机交互系统。
...
### [332] [Multimodal Classification Network Guided Trajectory Planning for Four-Wheel Independent Steering Autonomous Parking Considering Obstacle Attributes](https://arxiv.org/abs/2512.18836)
*Jingjia Teng,Yang Li,Jianqiang Wang,Yingbai Hu,Songyuan Tang,Manjiang Hu*

Main category: cs.RO

TL;DR: 提出了一种针对四轮独立转向（4WIS）车辆的轨迹规划框架，通过融合混合A*算法与最优控制问题，结合障碍物属性分类与动态风险场，提升在狭窄复杂环境中的路径规划成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 现有规划器忽略障碍物属性（如可跨越的低矮障碍），导致4WIS车辆在狭窄空间中规划效率低或失败，需结合车辆多转向模式与障碍物智能处理策略以提升性能。

Method: 1. 使用多模态分类网络评估场景复杂度；2. 针对复杂任务设置引导点分解子任务；3. 在混合A*中集成4WIS的多种转向模式；4. 设计分层障碍物处理策略（不可通行/可跨越/可驶过）；5. 引入概率风险场模型处理动态障碍物；6. 通过最优控制问题生成平滑轨迹。

Result: 实验表明，该框架能在受限环境中为4WIS车辆生成安全、高效、平滑的轨迹，尤其通过障碍物属性处理显著提高了路径规划成功率。

Conclusion: 所提框架有效整合了4WIS车辆特性、障碍物语义理解与动态风险感知，实现了在复杂场景下更智能的轨迹规划，为自动驾驶在狭窄环境中的导航提供了解决方案。
...
### [333] [InDRiVE: Reward-Free World-Model Pretraining for Autonomous Driving via Latent Disagreement](https://arxiv.org/abs/2512.18850)
*Feeza Khan Khanzada,Jaerock Kwon*

Main category: cs.RO

TL;DR: 提出InDRiVE，一种基于DreamerV3的MBRL智能体，通过潜在集成分歧进行无奖励预训练，在CARLA中实现零样本迁移和少样本适应，提升自动驾驶的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习（MBRL）虽能降低自动驾驶交互成本，但依赖难以设计且易受分布偏移影响的任务特定奖励。

Method: 使用潜在集成分歧作为内在动机进行无奖励预训练，基于想象力的演员-评论家学习无规划器的探索策略，随后进行零样本迁移和少样本适应。

Result: 在CARLA的不同城镇、路线和交通密度下，基于分歧的预训练在零样本鲁棒性和少样本碰撞避免方面表现更优。
...
### [334] [Construction and deformation of P-hedra using control polylines](https://arxiv.org/abs/2512.18869)
*Georg Nawratil*

Main category: cs.RO

TL;DR: 本文研究P-hedra（P-nets）柔性平面四边形曲面的交互式控制方法，通过三条控制折线直观调整空间形状，适用于可变形设计，并探讨其等距变形、弯曲极限、分岔构型等特性。


<details>
  <summary>Details</summary>
Motivation: 基于前期提出的P-hedra连续柔性离散曲面概念，进一步开发直观的交互设计工具，使其更适用于实际的可变形结构设计任务。

Method: 采用三条控制折线直接操控曲面空间形状的交互方法，结合算法高效计算等距变形，并分析弯曲极限、分岔构型、可展/可平折模式及管状P-hedra。

Result: 实现了P-hedra曲面的直观交互设计框架，验证了控制折线方法对等距变形计算的有效性，并系统分类了多种特殊构型与变形特性。

Conclusion: P-hedra的控制折线方法为柔性曲面提供了实用的设计工具，其算法化变形计算与特性分析拓展了在可变形结构领域的应用潜力。
...
### [335] [Optimizing Robotic Placement via Grasp-Dependent Feasibility Prediction](https://arxiv.org/abs/2512.18922)
*Tianyuan Liu,Richard Dazeley,Benjamin Champion,Akan Cosgun*

Main category: cs.RO

TL;DR: 提出一种基于廉价几何标签学习的抓取放置候选排序方法，通过路径感知的几何监督训练紧凑神经网络，在固定规划预算下更快找到可行路径，保持或提升最终成功率。


<details>
  <summary>Details</summary>
Motivation: 针对预算感知的抓取放置任务，研究能否仅通过廉价的、无物理仿真的几何监督来可靠地优先选择抓取放置候选方案，以降低计算成本并提高规划效率。

Method: 从物体初始位姿、目标位姿和候选抓取生成两种路径感知几何标签：基于固定路点模板的路径逆运动学可行性，以及沿相同模板的网格扫描碰撞标志。使用紧凑的双输出MLP从位姿编码中学习这些信号，测试时用其分数对预计算候选进行排序，采用与基线相同的IK门控和规划器。

Result: 尽管仅从廉价标签学习，但分数可迁移到启用物理仿真的执行轨迹：在固定规划预算下，策略能更快找到成功路径，减少规划器调用次数，同时最终成功率相当或更好。

Conclusion: 廉价几何监督能有效优先抓取放置候选，提升规划效率。当前工作针对单一刚性立方体侧面抓取和固定路点模板，未来可扩展至多样物体和更丰富的路点方案。
...
### [336] [A Framework for Deploying Learning-based Quadruped Loco-Manipulation](https://arxiv.org/abs/2512.18938)
*Yadong Liu,Jianwei Liu,He Liang,Dimitrios Kanoulas*

Main category: cs.RO

TL;DR: 提出一个用于四足移动机械臂的开源强化学习控制框架，支持从仿真到仿真的迁移以及从仿真到现实的部署，并在Unitree B1机器狗和Z1机械臂上验证了全身协调控制的有效性。


<details>
  <summary>Details</summary>
Motivation: 四足移动机械臂在敏捷的移动操作方面潜力巨大，但现有控制方法难以可靠地从仿真迁移到现实，且大多数强化学习框架是专有的，难以在真实硬件上复现。

Method: 开发了一个基于ROS的开源管道，统一了仿真到仿真和仿真到现实的迁移流程。该方法在Isaac Gym中训练策略，通过硬件抽象层扩展到MuJoCo，并将同一控制器部署到物理硬件上。

Result: 仿真到仿真的实验揭示了Isaac Gym和MuJoCo接触模型之间的差异会影响策略行为；真实世界的遥操作物体抓取试验表明，协调的全身控制比浮动基座基线方法扩展了可达范围并改善了操作性能。

Conclusion: 该管道为开发和评估基于强化学习的移动操作控制器提供了一个透明、可复现的基础，并将开源发布以支持未来研究。
...
### [337] [Affordance RAG: Hierarchical Multimodal Retrieval with Affordance-Aware Embodied Memory for Mobile Manipulation](https://arxiv.org/abs/2512.18987)
*Ryosuke Korekata,Quanting Xie,Yonatan Bisk,Komei Sugiura*

Main category: cs.RO

TL;DR: 提出Affordance RAG框架，通过构建具身记忆库实现开放词汇移动操作，在真实室内环境中任务成功率85%。


<details>
  <summary>Details</summary>
Motivation: 解决开放词汇移动操作问题，即机器人需根据自然语言指令将各种物体运送到对应容器，需要同时理解视觉语义和操作可行性。

Method: 提出零样本分层多模态检索框架Affordance RAG，从预探索图像构建Affordance-Aware Embodied Memory，通过区域和视觉语义检索候选目标，并用可行性分数重排序。

Result: 在大规模室内环境中移动操作指令检索性能优于现有方法；真实世界实验中基于自由形式指令的任务成功率达到85%。

Conclusion: Affordance RAG框架能有效识别现实环境中可执行的操作选项，在检索性能和任务成功率上均优于现有方法。
...
### [338] [LeLaR: The First In-Orbit Demonstration of an AI-Based Satellite Attitude Controller](https://arxiv.org/abs/2512.19576)
*Kirill Djebko,Tom Baumann,Erik Dilger,Frank Puppe,Sergio Montenegro*

Main category: cs.RO

TL;DR: 首次在轨成功演示了基于深度强化学习的卫星姿态控制器，该控制器完全在仿真中训练并部署到InnoCube纳米卫星上，在惯性指向机动中表现出稳健性能。


<details>
  <summary>Details</summary>
Motivation: 传统姿态控制器设计耗时且对模型不确定性和操作条件变化敏感，深度强化学习能通过仿真交互学习自适应控制策略，但仿真到现实的迁移仍是挑战。

Method: 使用深度强化学习在仿真环境中训练AI控制器，部署到InnoCube 3U纳米卫星进行在轨验证，并与传统PD控制器进行对比分析。

Result: AI控制器在重复在轨机动中表现出稳健性能，稳态指标验证了其有效性，同时分析了仿真与真实卫星行为之间的差异。

Conclusion: 基于AI的姿态控制器在真实太空环境中成功演示，为克服Sim2Real差距提供了实际案例，展示了深度强化学习在卫星控制中的应用潜力。
...
### [339] [DTCCL: Disengagement-Triggered Contrastive Continual Learning for Autonomous Bus Planners](https://arxiv.org/abs/2512.18988)
...
TL;DR: 提出DTCCL框架，通过对比持续学习解决自动驾驶公交车在交互密集区域因规划器失败导致的脱管事件，实现无监督的云端策略迭代优化。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶公交车在开放动态城市环境中运行时，脱管事件常集中在高交互区域且源于规划策略失败，传统模仿学习易对稀疏脱管数据过拟合，难以有效修正策略级错误。

Method: 采用脱管触发式对比持续学习框架：脱管事件触发云端数据增强，通过扰动周围智能体生成正负样本；利用对比学习优化策略表征以区分安全/不安全行为；通过云端-边缘循环实现无人工监督的持续更新。

Result: 在城市公交路线实验中，DTCCL相比直接重训练将整体规划性能提升48.6%，验证了其在自动驾驶公共交通中可扩展闭环策略改进的有效性。

Conclusion: DTCCL框架能通过实时运营数据自主优化规划策略，为解决自动驾驶公交车在高交互场景中的策略失败问题提供了可扩展的闭环解决方案。
...
### [340] [IndoorUAV: Benchmarking Vision-Language UAV Navigation in Continuous Indoor Environments](https://arxiv.org/abs/2512.19024)
*Xu Liu,Yu Liu,Hanshuo Qiu,Yang Qirong,Zhouhui Lian*

Main category: cs.RO

TL;DR: 本文提出IndoorUAV基准和方法，专注于室内无人机视觉语言导航，包含长/短视距子集和新颖导航模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航研究多关注地面机器人或室外无人机，室内无人机导航在检查、救援等实际应用中重要但尚未充分探索。

Method: 使用Habitat模拟器构建1000+室内3D场景，模拟无人机动力学收集轨迹，通过数据增强和自动标注生成多粒度指令，提出基于任务分解和多模态推理的导航模型。

Result: 创建包含16000+高质量轨迹的IndoorUAV-VLN（长视距）和IndoorUAV-VLA（短视距）数据集，并开发了适配的导航模型。

Conclusion: IndoorUAV为室内空中视觉语言具身AI研究提供了宝贵资源，有望推动该领域发展。
...
### [341] [EGM: Efficiently Learning General Motion Tracking Policy for High Dynamic Humanoid Whole-Body Control](https://arxiv.org/abs/2512.19043)
*Chao Yang,Yingkai Sun,Peng Ye,Xin Chen,Chong Yu,Tao Chen*

Main category: cs.RO

TL;DR: 提出EGM框架，通过跨动作课程自适应采样、复合解耦专家混合架构和三阶段课程训练，高效学习通用人体运动跟踪策略，在少量数据训练下实现鲁棒泛化。


<details>
  <summary>Details</summary>
Motivation: 传统方法数据利用效率低、训练过程低效，且在跟踪高动态运动时性能有限，需要一种能高效学习通用运动跟踪策略的框架。

Method: 1. 基于箱的跨动作课程自适应采样策略；2. 复合解耦专家混合架构（CDMoE），分别处理上下半身并解耦专用与通用特征；3. 三阶段课程训练流程逐步提升抗干扰鲁棒性。

Result: 仅用4.08小时数据训练，在49.25小时测试动作上实现鲁棒泛化，在常规和高动态任务上均优于基线方法。

Conclusion: EGM框架通过高效数据利用和专门化架构设计，显著提升了运动跟踪策略的学习效率和泛化能力，证明了数据质量与多样性对训练通用策略的关键作用。
...
### [342] [CoDrone: Autonomous Drone Navigation Assisted by Edge and Cloud Foundation Models](https://arxiv.org/abs/2512.19083)
*Pengyu Chen,Tao Ouyang,Ke Luo,Weijie Hong,Xu Chen*

Main category: cs.RO

TL;DR: 提出CoDrone框架，通过云-边-端协同计算将基础模型集成到无人机自主巡航中，利用灰度图像、边缘辅助深度估计和强化学习调度器，提升资源受限无人机的导航性能。


<details>
  <summary>Details</summary>
Motivation: 无人机机载计算资源有限，浅层神经网络难以处理复杂环境；任务卸载到边缘服务器则引入高延迟，存在系统设计权衡问题。

Method: 1) 使用灰度图像降低计算和传输开销；2) 边缘辅助基础模型Depth Anything V2进行深度估计；3) 提出一维占用网格导航方法；4) 基于深度强化学习的神经调度器整合深度估计与导航决策；5) 引入无人机专用视觉语言交互模块。

Result: 在不同飞行速度和网络条件下，CoDrone优于基线方法，平均飞行距离增加40%，平均导航质量提升5%。

Conclusion: CoDrone框架通过协同计算有效利用基础模型，增强了资源受限无人机在复杂环境中的自主导航能力，实现了性能与效率的平衡。
...
### [343] [WorldRFT: Latent World Model Planning with Reinforcement Fine-Tuning for Autonomous Driving](https://arxiv.org/abs/2512.19133)
*Pengxuan Yang,Ben Lu,Zhongpu Xia,Chao Han,Yinfeng Gao,Teng Zhang,Kun Zhan,XianPeng Lang,Yupeng Zheng,Qichao Zhang*

Main category: cs.RO

TL;DR: 提出WorldRFT框架，通过分层规划分解、局部感知交互优化和强化学习微调，将场景表示学习与规划任务对齐，在nuScenes和NavSim基准上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有潜在世界模型通过重建导向的表示学习将感知与规划任务耦合，导致规划优化次优，需要一种规划导向的表示学习框架。

Method: 1. 集成视觉-几何基础模型提升3D空间感知；2. 分层规划任务分解指导表示优化；3. 局部感知迭代优化推导规划导向驾驶策略；4. 提出GRPO（轨迹高斯化和碰撞感知奖励）微调策略。

Result: 1. nuScenes上碰撞率降低83%（0.30%→0.05%）；2. NavSim上仅用摄像头输入达到与LiDAR SOTA方法DiffusionDrive相当性能（87.8 vs 88.1 PDMS）。

Conclusion: WorldRFT通过规划导向的表示学习和安全增强微调，显著提升自动驾驶系统的安全性和规划性能，验证了框架的有效性。
...
### [344] [A Flexible Field-Based Policy Learning Framework for Diverse Robotic Systems and Sensors](https://arxiv.org/abs/2512.19148)
*Jose Gustavo Buenaventura Carreon,Floris Erich,Roman Mykhailyshyn,Tomohiro Motoda,Ryo Hanai,Yukiyasu Domae*

Main category: cs.RO

TL;DR: 提出了一种跨机器人视觉运动学习框架，结合扩散策略控制与3D语义场景表示，实现操作任务的类别级泛化，支持多种机器人配置并在抓取任务中取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 现有机器人系统通常针对特定硬件配置设计，缺乏在不同机器人平台和传感器配置间的泛化能力，限制了大规模现实世界研究的可扩展性。

Method: 采用模块化设计，集成基于扩散策略的控制方法和D3Fields的3D语义场景表示，通过统一配置层支持多种机器人相机配置（如UR5+Azure Kinect阵列、双手机器人+RealSense传感器），配合低延迟控制栈和直观遥操作进行数据收集。

Result: 在抓取和抬起积木任务中，仅用100次演示训练后即达到80%的成功率，展示了在不同平台和感知模态间稳健的技能迁移能力。

Conclusion: 该框架为跨机器人泛化的可扩展现实世界研究提供了可行路径，其模块化设计和统一配置层支持灵活的数据收集、训练和评估。
...
### [345] [Vision-Language-Policy Model for Dynamic Robot Task Planning](https://arxiv.org/abs/2512.19178)
*Jin Wang,Kim Tien Ly,Jacques Cloete,Nikos Tsagarakis,Ioannis Havoutis*

Main category: cs.RO

TL;DR: 提出基于视觉语言模型的动态机器人任务规划框架VLP，通过多模态感知和推理生成行为策略，实现自然语言指令到自主执行的动态适应。


<details>
  <summary>Details</summary>
Motivation: 传统机器人任务规划方法难以弥合低层执行与高层推理的鸿沟，且无法在执行过程中动态调整策略，限制了机器人在非结构化环境中的适应性和通用性。

Method: 基于真实数据微调的视觉语言模型VLP，通过语义指令解析和场景推理生成控制机器人的行为策略，支持任务变更时的动态策略调整。

Result: 在不同机器人和多样现实任务中的实验表明，该模型能高效适应新场景并动态更新策略，展现出较强的规划自主性和跨具身泛化能力。

Conclusion: VLP框架通过语言模型实现了动态任务规划，提升了机器人在非结构化环境中执行自然语言指令的适应性和灵活性。
...
### [346] [Translating Flow to Policy via Hindsight Online Imitation](https://arxiv.org/abs/2512.19269)
*Yitian Zheng,Zhangchen Ye,Weijun Dong,Shengjie Wang,Yuyang Liu,Chongjie Zhang,Chuan Wen,Yang Gao*

Main category: cs.RO

TL;DR: 提出HinFlow方法，通过在线交互改进分层机器人系统中的低级策略，利用后见之明重标注经验更新目标条件模仿策略，显著提升操作任务性能。


<details>
  <summary>Details</summary>
Motivation: 分层机器人系统中，高级规划器可基于非机器人数据（如视频）训练，但将高级计划转化为可执行动作仍具挑战性，尤其在高质量机器人数据有限的情况下。

Method: 通过在线交互收集轨迹，根据已实现结果对高级目标进行后见之明重标注，聚合这些经验更新目标条件模仿策略，并以2D点流作为高级规划器实例化（HinFlow）。

Result: 在仿真和物理世界的多样化操作任务中，性能较基础策略提升超过2倍，显著优于现有方法，并能从跨具身视频数据训练的规划器中获取策略。

Conclusion: 该方法通过在线后见之明学习有效桥接高级规划与低级执行，展示了可扩展和可迁移机器人学习的潜力。
...
### [347] [Are All Data Necessary? Efficient Data Pruning for Large-scale Autonomous Driving Dataset via Trajectory Entropy Maximization](https://arxiv.org/abs/2512.19270)
*Zhaoyang Liu,Weitao Zhou,Junze Wen,Cheng Jing,Qian Cheng,Kun Jiang,Diange Yang*

Main category: cs.RO

TL;DR: 提出一种基于信息论的自动驾驶数据剪枝方法，通过最大化轨迹熵选择高价值样本，可在减少40%数据量的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界驾驶数据中存在大量重复和低价值样本，导致存储成本过高且对策略学习贡献有限，需要高效的数据管理方法。

Method: 基于信息论的数据剪枝方法，通过计算驾驶数据的轨迹分布信息熵，以模型无关的方式迭代选择能保持原始数据集统计特征的高价值样本。

Result: 在NuPlan基准测试和大规模模仿学习框架上的实验表明，该方法能减少高达40%的数据集规模，同时保持闭环驾驶性能。

Conclusion: 本研究为自动驾驶系统提供了一种轻量级且理论可靠的数据管理方法，可实现可扩展的数据管理和高效策略学习。
...
### [348] [Comparison and Evaluation of Different Simulation Environments for Rigid Body Systems](https://arxiv.org/abs/2512.19289)
*Longxiang Shao,Ulrich Dahmen,Juergen Rossmann*

Main category: cs.RO

TL;DR: 本研究比较了四种多体动力学仿真环境（Adams、Simscape、OpenModelica、VEROSIM）在建模方法、数值求解器及处理闭环运动学数值问题方面的差异，以真实林业机械的起重机臂作为基准案例。


<details>
  <summary>Details</summary>
Motivation: 多体动力学仿真器在机械系统的设计、分析和优化中至关重要，但不同工具在建模方法、数值求解及处理闭环运动学问题（如冗余边界条件和静力平衡问题）时存在差异，需要系统比较以帮助用户选择合适的工具。

Method: 选取四种仿真环境（Adams、Simscape、OpenModelica、VEROSIM），以真实林业机械的复杂起重机臂为基准应用案例，重点比较建模方法、数值求解器及对闭环运动学中数值问题的处理方式。

Result: 通过直接比较不同仿真工具的求解方法，揭示了它们在处理冗余边界条件和静力平衡等数值问题时的差异，为特定应用场景下的工具选择提供了依据。

Conclusion: 不同仿真工具在建模和求解策略上各有特点，本研究通过实际案例的系统比较，可帮助用户根据具体应用需求（如模型复杂度、数值稳定性要求）选择最合适的仿真环境。
...
### [349] [OMP: One-step Meanflow Policy with Directional Alignment](https://arxiv.org/abs/2512.19347)
*Han Fang,Yize Huang,Yuheng Zhao,Paul Weng,Xiao Li,Yutong Ban*

Main category: cs.RO

TL;DR: 提出改进的MeanFlow策略，通过余弦损失对齐速度方向和使用DDE优化JVP算子，提升机器人操作策略的少样本泛化能力和轨迹精度，同时保持实时性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的机器人操作策略（如扩散模型和流方法）存在推理延迟高或架构复杂的问题，而简单应用MeanFlow虽能实现单步推理，但缺乏少样本泛化能力，主要因分散损失的固定温度超参数和预测-真实速度均值未对齐。

Method: 引入轻量级余弦损失以对齐速度方向，并使用微分推导方程（DDE）优化雅可比-向量积（JVP）算子，改进基于MeanFlow的策略。

Result: 在Adroit和Meta-World任务上的实验表明，该方法在平均成功率上优于MP1和FlowPolicy，尤其在挑战性Meta-World任务中，有效增强了少样本泛化和轨迹精度，同时保持实时性能。

Conclusion: 该方法为高精度机器人操作提供了更鲁棒的解决方案，平衡了性能与效率。
...
### [350] [TwinAligner: Visual-Dynamic Alignment Empowers Physics-aware Real2Sim2Real for Robotic Manipulation](https://arxiv.org/abs/2512.19390)
*Hongwei Fan,Hang Dai,Jiyao Zhang,Jinzhou Li,Qiyang Yan,Yujie Zhao,Mingju Gao,Jinghang Wu,Hao Tang,Hao Dong*

Main category: cs.RO

TL;DR: 本文提出TwinAligner系统，通过视觉对齐和动态对齐模块解决仿真与现实之间的差距，使仿真训练的机器人策略能够零样本泛化到现实世界。


<details>
  <summary>Details</summary>
Motivation: 机器人领域正转向数据驱动的端到端学习，但依赖昂贵的真实世界数据限制了进展。仿真器虽成本低，但仿真与现实间的差距阻碍了策略的有效迁移。

Method: 提出Real2Sim2Real系统TwinAligner：视觉对齐模块通过SDF重建和可编辑3DGS渲染实现像素级对齐；动态对齐模块通过识别机器人-物体交互的刚性物理确保动态一致性。

Result: 定量评估显示TwinAligner在视觉和动态对齐方面表现优异，仿真训练的机器人策略在现实世界中实现零样本泛化，仿真与现实策略性能高度一致。

Conclusion: TwinAligner通过提供可扩展的数据收集和可信的迭代循环，加速算法开发，有望推动可扩展机器人学习的进展。
...
### [351] [Real2Edit2Real: Generating Robotic Demonstrations via a 3D Control Interface](https://arxiv.org/abs/2512.19402)
*Yujie Zhao,Hongwei Fan,Di Chen,Shengcong Chen,Liliang Chen,Xiaoqi Li,Guanghui Ren,Hao Dong*

Main category: cs.RO

TL;DR: 提出Real2Edit2Real框架，通过3D编辑和视频生成技术，从少量真实演示中合成新的机器人操作演示，显著提升数据效率。


<details>
  <summary>Details</summary>
Motivation: 机器人学习依赖大规模数据集，但收集多样化演示成本高昂，尤其在空间泛化任务中。为减少重复数据收集，需开发高效的数据生成方法。

Method: 1) 从多视角RGB观测重建场景几何；2) 在点云上进行深度可靠的3D编辑，生成新轨迹并几何校正机器人姿态；3) 提出以深度为主要控制信号的多条件视频生成模型，合成空间增强的多视角操作视频。

Result: 在四个真实世界操作任务中，仅用1-5个源演示生成的数据训练的策略，性能匹配或优于用50个真实演示训练的策略，数据效率提升10-50倍。高度和纹理编辑实验验证了框架的灵活性和可扩展性。

Conclusion: Real2Edit2Real框架通过结合3D可编辑性和2D视觉数据，能高效生成多样化演示，有望成为统一的数据生成框架，显著降低机器人学习的数据收集成本。
...
### [352] [MaP-AVR: A Meta-Action Planner for Agents Leveraging Vision Language Models and Retrieval-Augmented Generation](https://arxiv.org/abs/2512.19453)
*Zhenglong Guo,Yiming Zhao,Feng Jiang,Heng Jin,Zongbao Feng,Jianbin Zhou,Siyuan Xu*

Main category: cs.RO

TL;DR: 本文提出了一种名为MaP-AVR的元动作规划器，通过将任务规划抽象为元动作集合，并结合检索增强生成技术，提高了机器人任务规划的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注通过微调或思维链提示增强LLM/VLM的任务理解能力，但忽视了规划技能集定义的重要性。为了应对日常环境的复杂性，需要高度泛化的技能表达。

Method: 提出元动作抽象方法，每个元动作包含移动/旋转、末端执行器状态变化、与环境关系三个组件；采用检索增强生成技术，利用人工标注的规划示范数据库支持上下文学习，并实现数据库自增强。

Result: 在GPT-4o和OmniGibson平台上的实验表明，该方法相比当前最先进方法展现出有前景的性能。

Conclusion: 元动作抽象和RAG集成是规划器的两大创新贡献，使规划结果与机器人内在功能无缝对齐，并能通过任务完成不断自我增强以支持多样性。
...
### [353] [REALM: A Real-to-Sim Validated Benchmark for Generalization in Robotic Manipulation](https://arxiv.org/abs/2512.19562)
...
TL;DR: 提出了REALM仿真环境与基准测试，用于评估视觉-语言-动作模型在泛化与鲁棒性方面的能力，并验证仿真与真实世界性能的相关性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在真实环境中的泛化能力评估困难且成本高昂，缺乏系统化的评测工具。

Method: 构建高保真仿真环境，包含15种扰动因素、7种操作技能和超3500个物体；设计两项任务集作为基准，并测试了π₀、π₀-FAST和GR00T N1.5等模型。

Result: 实验表明当前模型的泛化与鲁棒性仍面临挑战；仿真环境可作为真实世界的有效代理，系统化揭示模型弱点与失效模式。

Conclusion: REALM为系统评估VLA模型泛化能力提供了可行方案，仿真与真实性能具有强相关性，有助于推动模型鲁棒性研究。
...
### [354] [Results of the 2024 CommonRoad Motion Planning Competition for Autonomous Vehicles](https://arxiv.org/abs/2512.19564)
*Yanliang Huang,Xia Yan,Peiran Yin,Zhenduo Zhang,Zeyan Shao,Youran Wang,Haoliang Huang,Matthias Althoff*

Main category: cs.RO

TL;DR: 介绍了2024年第四届CommonRoad运动规划竞赛的设置与结果，该竞赛为自动驾驶车辆运动规划算法提供了开源、可复现的基准测试框架，并在高速公路和城市环境中评估了规划器的效率、安全性、舒适性和交通规则遵守情况。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆运动规划方法虽多，但缺乏标准化基准测试，难以客观比较不同方法的优劣，因此需要通过公开竞赛建立统一的评估体系。

Method: 使用CommonRoad基准测试套件，在包含多种交通参与者（轿车、公交车、自行车）的高速公路和城市场景中，从效率、安全性、舒适性和交通规则遵守四个维度评估运动规划算法性能。

Result: 报告介绍了竞赛形式，并对2023年和2024年两届竞赛中表现优异的代表性规划器进行了比较分析。

Conclusion: CommonRoad竞赛为运动规划算法提供了重要的基准测试平台，促进了算法性能的客观比较与技术进步。
...
### [355] [LIMOncello: Revisited IKFoM on the SGal(3) Manifold for Fast LiDAR-Inertial Odometry](https://arxiv.org/abs/2512.19567)
*Carlos Pérez-Ruiz,Joan Solà*

Main category: cs.RO

TL;DR: LIMOncello是一种紧耦合的LiDAR-惯性里程计系统，在SGal(3)流形上建模6自由度运动，采用增量i-Octree建图后端，在几何稀疏环境中具有更好的鲁棒性和实时性能。


<details>
  <summary>Details</summary>
Motivation: 现有状态表示（如SO(3)×R⁶）在低可观测条件下易产生漂移，且传统增量kd-tree地图结构更新慢、内存占用高，需要更稳定高效的运动建模和地图表示方法。

Method: 1. 在SGal(3)流形上建立6-DoF运动模型，使用迭代误差状态卡尔曼滤波后端；2. 设计轻量级增量i-Octree建图后端，替代传统kd-tree结构；3. 避免依赖局部受限搜索启发式方法。

Result: 在多个真实数据集上验证：1. 达到竞争性精度；2. 在几何稀疏环境中鲁棒性显著提升；3. 保持实时性能且内存增长稳定；4. 比kd-tree结构更新更快、内存占用更低。

Conclusion: LIMOncello通过SGal(3)流形建模和i-Octree地图结构，实现了低漂移、高效率的LiDAR-惯性里程计系统，在稀疏环境中表现优越，已开源供社区使用。
...
### [356] [Learning Generalizable Hand-Object Tracking from Synthetic Demonstrations](https://arxiv.org/abs/2512.19583)
...
TL;DR: 提出仅使用合成数据学习通用手-物体跟踪控制器的系统，无需人类演示，包含轨迹规划器HOP和跟踪控制器HOT，支持多种物体形状和手部形态。


<details>
  <summary>Details</summary>
Motivation: 解决灵巧操作领域长期存在的数据瓶颈问题，避免依赖难以获取的人类演示数据，实现完全从合成数据中学习可扩展的基础控制器。

Method: 1. HOP（手-物体规划器）：合成多样化的手-物体轨迹；2. HOT（手-物体跟踪器）：通过强化学习和交互模仿学习实现合成到物理的迁移，生成以目标手-物体状态为条件的通用控制器。

Result: 系统能使灵巧手跟踪具有挑战性的长时程序列，包括物体重新排列和敏捷的手内重定向，支持不同物体形状和手部形态的泛化。

Conclusion: 该方法突破了灵巧操作的数据限制，为完全基于合成数据学习可扩展的操纵基础控制器迈出了重要一步。
...
### [357] [LoGoPlanner: Localization Grounded Navigation Policy with Metric-aware Visual Geometry](https://arxiv.org/abs/2512.19629)
*Jiaqi Peng,Wenzhe Cai,Yuqiang Yang,Tai Wang,Yuan Shen,Jiangmiao Pang*

Main category: cs.RO

TL;DR: 提出LoGoPlanner，一种基于定位的端到端导航框架，通过视觉几何骨干网络和场景几何重建，在非结构化环境中实现高效轨迹规划，减少累积误差并提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统模块化轨迹规划方法存在延迟和级联误差问题，而现有端到端方法仍依赖独立定位模块，限制了其在跨平台和环境中的泛化能力。

Method: 1) 微调长时程视觉几何骨干网络以获取绝对度量尺度；2) 从历史观测重建场景几何以提供细粒度环境感知；3) 基于辅助任务引导的策略学习，减少误差传播。

Result: 在仿真和真实环境中，LoGoPlanner相比依赖精确定位的基线方法提升27.3%，在规划一致性和避障方面表现更优，且能泛化到不同平台和环境。

Conclusion: LoGoPlanner通过端到端设计和度量感知的几何记忆，显著提升了移动机器人在非结构化环境中的导航性能，代码和模型已开源。
...
### [358] [Robustness of Delayed Higher Order Sliding Mode Control](https://arxiv.org/abs/2512.18018)
*Moussa Labbadi,Denis Efimov,Leonid Fridman*

Main category: eess.SY

TL;DR: 本文研究了高阶延迟滑模控制器的可行性，通过ISS隐式Lyapunov-Razumikhin函数方法建立了其对测量噪声和失配扰动的鲁棒性，并通过仿真验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 针对高阶延迟滑模控制器在实际应用中可能面临的测量噪声和失配扰动问题，需要建立其鲁棒性理论框架，以确保控制系统的稳定性和可靠性。

Method: 采用ISS隐式Lyapunov-Razumikhin函数方法，分析系统在测量噪声和失配扰动下的鲁棒性，并通过仿真实验进行验证。

Result: 理论分析表明，高阶延迟滑模控制器对测量噪声和失配扰动具有鲁棒性，仿真结果进一步证实了该方法的有效性。

Conclusion: 高阶延迟滑模控制器在存在测量噪声和失配扰动的情况下仍能保持系统稳定性，该方法为实际工程应用提供了理论依据。
...
### [359] [Review of Power Electronic Solutions for Dielectric Barrier Discharge Applications](https://arxiv.org/abs/2512.18067)
*Hyeongmeen Baik,Jinia Roy*

Main category: eess.SY

TL;DR: 本文全面综述了介质阻挡放电（DBD）电源拓扑，旨在弥合DBD应用与电力电子设计之间的差距，系统回顾了正弦型和脉冲型电源，并总结了性能权衡与未来设计方向。


<details>
  <summary>Details</summary>
Motivation: DBD应用与电源设计之间存在脱节，缺乏对DBD电气模型与波形需求的系统分析，阻碍了高性能电源的发展。

Method: 基于DBD反应器几何结构对电气模型的影响以及应用驱动的波形要求，系统回顾了两类电源拓扑：正弦型（无变压器/变压器谐振逆变器）和脉冲电源（PPS）。

Result: 总结了不同拓扑的性能权衡，指出了未充分测试的拓扑结构和新兴应用场景，为下一代高性能DBD电源设计提供了指导。

Conclusion: 通过系统分析DBD模型与电源拓扑的关联，为优化电源设计、推动DBD技术在高要求应用中的发展提供了理论框架和实践方向。
...
### [360] [A Games-in-Games Paradigm for Strategic Hybrid Jump-Diffusions: Hamilton-Jacobi-Isaacs Hierarchy and Spectral Structure](https://arxiv.org/abs/2512.18098)
*Yunian Pan,Quanyan Zhu*

Main category: eess.SY

TL;DR: 本文提出了一种用于混合随机系统的分层博弈控制架构，通过双层微分博弈建模连续状态动态与离散模式切换的交互作用，并针对线性二次和指数仿射博弈推导出半封闭解。


<details>
  <summary>Details</summary>
Motivation: 混合随机系统（如受体制转换跳跃扩散过程控制的系统）在金融、工程等领域广泛存在，但传统方法难以同时处理连续动态和离散切换的交互。需要一种统一框架来建模这种分层决策结构。

Method: 采用双层微分博弈架构：内层在每个体制内解决鲁棒随机控制问题，外层战略性地调制底层马尔可夫链的转移强度。通过Dynkin分析得到耦合的Hamilton-Jacobi-Isaacs方程，针对线性二次和指数仿射博弈推导出耦合矩阵微分方程的半封闭解。

Result: 理论证明该分层架构对于线性二次博弈和指数仿射博弈存在可处理的半封闭形式解。案例研究表明，在对抗性市场微观结构中，外层的战略切换能够预调整库存价差以应对潜在体制风险，形成超警觉均衡。

Conclusion: 所提出的分层博弈控制架构为混合随机系统提供了统一的建模和分析框架，能够有效处理连续动态与离散切换的复杂交互，在金融风险控制等领域具有应用潜力。
...
### [361] [Timing-Aware Two-Player Stochastic Games with Self-Triggered Control](https://arxiv.org/abs/2512.18109)
*Yunian Pan,Quanyan Zhu*

Main category: eess.SY

TL;DR: 本文研究基于分段确定性马尔可夫过程的自主触发双人随机博弈，提出可处理动态规划分解的Stackelberg时序承诺框架，并通过追逃博弈示例验证策略时序交互。


<details>
  <summary>Details</summary>
Motivation: 现有自主触发随机博弈中，同时盲选时序（Nash均衡）导致难以求解的耦合拟变分不等式，需要发展可计算的时序决策框架。

Method: 通过引入时钟和承诺控制扩展状态空间，划分流区域和触发面；采用可观测顺序承诺（Stackelberg）框架，建立嵌套Hamilton-Jacobi-Bellman拟变分不等式，并提出基于隐函数微分追随者不动点的计算方案。

Result: 顺序承诺框架实现了可处理的动态规划分解，而同时时序博弈则产生计算不可行的耦合方程；追逃博弈示例展示了策略时序交互的有效建模。

Conclusion: Stackelberg时序承诺为自主触发随机博弈提供了计算可行的解决方案，同时时序博弈的计算瓶颈仍需进一步研究突破。
...
### [362] [Bayesian Holonic Systems: Equilibrium, Uniqueness, and Computation](https://arxiv.org/abs/2512.18112)
*Yunian Pan,Quanyan Zhu*

Main category: eess.SY

TL;DR: 本文提出贝叶斯全息均衡（BHE）概念，用于建模分层多智能体系统中的战略风险，确保个体理性与系统涌现行为的一致性，并设计了双时间尺度学习算法进行求解。


<details>
  <summary>Details</summary>
Motivation: 分层多智能体系统（全息系统）中局部决策与全局结果相互耦合，缺乏保证个体理性与系统行为一致性的理论框架，需要建立可计算的分析方法。

Method: 提出贝叶斯全息均衡（BHE）概念，证明其存在性与唯一性；设计双时间尺度学习算法，快时间尺度协调组内策略，慢时间尺度调整组间风险信念；通过连续公共物品博弈进行数值验证。

Result: 理论证明了BHE的存在性（及特定条件下的唯一性）；算法在数值实验中收敛到理论均衡；建立了复杂耦合控制系统中战略风险分析的理论与算法框架。

Conclusion: BHE为分层多智能体系统提供了个体与系统行为一致的理论基础，双时间尺度算法实现了均衡计算，为复杂耦合控制系统的战略风险分析与设计提供了完整框架。
...
### [363] [Regularized Distributed MPC for UAV Networks: Stabilizing Coupled Motion and Hybrid Beam Alignment](https://arxiv.org/abs/2512.18222)
*Evangelos Vlachos*

Main category: eess.SY

TL;DR: 本文提出了一种用于无人机网络的高频混合波束赋形耦合控制方法，通过去中心化模型预测控制框架联合优化轨迹和航向，在满足安全约束的同时最大化网络总容量。


<details>
  <summary>Details</summary>
Motivation: 相控阵虽然支持快速电子扫描，但其有限的视场角要求无人机必须主动进行机械转向以维持网络连接，这引发了轨迹与波束指向的耦合控制问题。

Method: 采用去中心化模型预测控制框架，结合基于离散空间平滑的正则化代理成本函数以解决快衰落信道零点引起的数值不稳定问题，并推导了保证分布式最佳响应动态收敛的充分条件。

Result: 仿真结果表明，所提算法在电子波束跟踪与运动安全性之间取得了有效平衡，系统性能显著优于速度对齐基线方法。

Conclusion: 通过理论分析与仿真验证，所提出的控制框架能够稳定解决无人机网络中的耦合控制问题，为高频混合波束赋形系统的实际部署提供了可行方案。
...
### [364] [Power Converter DC Link Ripple and Network Unbalance as Active Constraints in Distribution System Optimal Power Flow](https://arxiv.org/abs/2512.18293)
*Matthew Deakin,Rahmat Heidari,Xu Deng*

Main category: eess.SY

TL;DR: 本文提出了一种考虑直流链路功率脉动的优化潮流方法，用于分析多端电压源换流器在不平衡电网条件下的运行约束，并通过案例研究和时域仿真验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 电网电压或电流不平衡时，电压源换流器会在直流链路上产生功率脉动，这对硬件设计和系统稳定性至关重要。然而，现有优化潮流方法很少考虑直流链路脉动与换流器不平衡运行之间的相互作用。

Method: 推导了通用多端换流器的功率脉动幅值表达式，将其作为双线性约束引入两电平换流器拓扑的优化潮流模型中，并通过四线制电压源换流器的时域仿真验证代数模型的准确性。

Result: 优化潮流案例研究表明，必须同时建模中性点电流和直流链路脉动，揭示了电容器尺寸与桥臂尺寸在缓解相电流不平衡时的权衡关系。仿真结果验证了代数公式的精确性和有效性。

Conclusion: 直流链路脉动的影响和约束对配电系统运营商日益重要，需要在系统设计和运行中予以充分考虑。
...
### [365] [State-Space Averaging Revisited via Reconstruction Operators](https://arxiv.org/abs/2512.18340)
*Yuxin Yang,Hang Zhou,Hourong Song,Branislav Hredzak*

Main category: eess.SY

TL;DR: 本文提出了一种从分段线性开关系统的精确采样数据（庞加莱映射）基线重建等效连续时间LTI模型的算子理论方法，揭示了经典状态空间平均（SSA）模型作为该精确重建的截断近似，并给出了简化计算策略。


<details>
  <summary>Details</summary>
Motivation: 经典状态空间平均（SSA）方法在电力电子变换器建模中广泛应用，但其依赖于低频和小纹波假设，且在每周期多于两个子区间时变得脆弱。本文旨在通过精确重建揭示SSA的数学本质与局限性，并提供更稳健的建模框架。

Method: 1. 基于矩阵对数显式表达从精确采样数据到连续时间LTI模型的重建；2. 利用Baker-Campbell-Hausdorff（BCH）公式展开矩阵指数乘积的对数，将SSA解释为小周期、小纹波条件下的主导阶截断；3. 提出复杂度降低的实现策略，利用2×2不变量和最小实提升构造，避免显式特征分解或复矩阵对数计算。

Result: 1. 证明了SSA模型是精确重建公式在小开关周期和小纹波条件下的主导阶近似；2. 从数学上解释了SSA依赖低频/小纹波假设的原因，以及其在多子区间变换器中脆弱性的根源；3. 提出的简化实现方法能有效计算所需谱量和实值对数，降低了计算复杂度。

Conclusion: 本文通过算子理论框架统一了精确采样数据模型与经典SSA近似，不仅揭示了SSA的数学本质与局限性，还为电力电子系统建模提供了更精确且计算高效的新途径，特别适用于多子区间开关系统。
...
### [366] [Robust H-infinity control under stochastic requirements: minimizing conditional value-at-risk instead of worst-case performance](https://arxiv.org/abs/2512.18356)
*Ervan Kassarian,Francesco Sanfedino,Daniel Alazard,Andrea Marrazza*

Main category: eess.SY

TL;DR: 提出基于条件风险价值（CVaR）的随机优化范式，替代传统保守的鲁棒H2/H∞控制，在容忍极罕见最坏情况性能下降的同时显著提升整体性能。


<details>
  <summary>Details</summary>
Motivation: 传统鲁棒控制以最坏情况性能为优化目标，导致设计过于保守，常被极罕见且可能非典型的参数配置所主导。

Method: 利用蒙特卡洛采样的随机特性及其与并行计算的兼容性，提出基于条件风险价值（CVaR）的随机优化框架来设计控制器。

Result: 在卫星控制基准测试中验证了该方法能显著提升整体性能，同时允许极罕见最坏情况下的一定性能退化。

Conclusion: 基于CVaR的随机优化范式能有效降低传统鲁棒控制的保守性，在保证鲁棒性的同时实现更优的整体性能。
...
### [367] [Neural Proofs for Sound Verification and Control of Complex Systems](https://arxiv.org/abs/2512.18389)
*Alessandro Abate*

Main category: eess.SY

TL;DR: 提出一种基于神经证明的新方法，用于复杂随机动态系统、反应式程序和CPS模型的正式验证与控制，结合神经网络训练和SMT求解生成可证明正确的策略。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以对复杂随机系统模型进行形式化验证和控制，需要一种能处理一般时序规范并生成可证明正确策略的新框架。

Method: 1) 设计编码时序规范的证明规则；2) 通过归纳方法构建证书：a) 从模型动态采样并训练神经网络，b) 利用SMT查询结合完整模型知识泛化网络；3) 在序列决策问题中生成可证明正确的状态反馈策略。

Result: 提出神经证明框架，能同时生成验证证书和控制策略，通过神经网络与形式化方法的结合处理复杂随机系统的验证与控制问题。

Conclusion: 神经证明为复杂随机系统的形式化验证与控制提供了新途径，通过数据驱动与形式推理的结合，有望扩展形式化方法的应用范围。
...
### [368] [Virtual Resistance-Based Control for Grid-Connected Inverters using Persidskii Systems Approach](https://arxiv.org/abs/2512.18428)
*Chakib Chatri,Ajul Dinesh,Moussa Labbadi*

Main category: eess.SY

TL;DR: 本文提出了一种改进的虚拟电阻控制方法，用于并网逆变器，通过扩展非线性边界和建立广义Persidskii型系统模型，推导出更宽松的输入到状态稳定性条件，提升了系统在扰动下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统基于无源性的虚拟电阻控制虽具有鲁棒性，但受限于严格的非线性扇形边界条件，限制了其在复杂工况下的应用。

Method: 将闭环系统建模为广义Persidskii型非线性系统，扩展非线性边界条件，并基于此推导输入到状态稳定性条件。

Result: 所提方法在保证系统稳定的前提下，显著降低了稳定性条件的保守性，并通过仿真验证了其在电网扰动下的有效性。

Conclusion: 该研究为虚拟电阻控制提供了一种系统化且保守性更低的设计框架，能够在不增加电压传感器的情况下提升并网逆变器的动态性能和鲁棒性。
...
### [369] [On Hyperexponential Stabilization of Linear Infinite-Dimensional Systems](https://arxiv.org/abs/2512.18430)
*Moussa Labbadi,Christophe Roman*

Main category: eess.SY

TL;DR: 本文研究了希尔伯特空间上无限维系统通过分布式时变控制律的超指数稳定化问题，利用极大单调算子证明了闭环系统的适定性，并通过李雅普诺夫分析和时间尺度变换建立了超指数稳定性和输入状态稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究无限维系统在希尔伯特空间中的超指数稳定化问题，旨在设计时变分布式控制律以实现比传统指数稳定更快的收敛速度，并确保系统在扰动下的鲁棒性。

Method: 采用极大单调算子理论分析闭环系统的适定性；通过李雅普诺夫分析和时间尺度变换方法证明系统的超指数稳定性及输入状态稳定性。

Result: 证明了闭环系统对所有时间均适定；建立了系统的超指数稳定性；验证了系统具有输入状态稳定性。

Conclusion: 所提出的分布式时变控制律能有效实现无限维系统的超指数稳定，且系统在扰动下保持鲁棒性，为无限维系统的快速稳定控制提供了理论框架。
...
TL;DR: 提出分布式层次时空边缘增强图神经网络（HSTE-GNN），用于超大规模路网的动态路径规划，通过分区并行处理提升可扩展性和实时适应性。


<details>
  <summary>Details</summary>
Motivation: 传统集中式路径规划算法和单体图神经网络在大规模城市路网中面临可扩展性差、延迟高、实时适应性不足的问题，难以应对快速变化的交通状况。

Method: 将城市级路网划分为区域子图，在分布式计算节点上并行处理；每个区域内使用边缘增强时空模块建模节点状态、动态边属性和短期时间依赖；通过异步参数服务器机制的层次协调层聚合跨区域表征，保证全局路由一致性。

Result: 在北京和纽约的真实交通数据集上，HSTE-GNN相比ST-GRAPH等基线模型，路由延迟降低34.9%，平均绝对百分比误差降低14.7%，均方根误差降低11.8%，全局路径一致性提升7.3%。

Conclusion: 该分布式层次设计平衡了局部响应能力和全局一致性，为下一代智能交通系统和大规模物流平台提供了可扩展、自适应且高效的解决方案。
...
### [371] [Sink Proximity: A Novel Approach for Online Vehicle Dispatch in Ride-hailing](https://arxiv.org/abs/2512.18501)
*Ruiting Wang,Jiaman Wu,Fabio Paparella,Scott J. Moura,Marta C. Gonzalez*

Main category: eess.SY

TL;DR: 提出一种基于网络科学的汇点接近度度量方法，结合共享网络框架设计在线车辆调度算法，显著提高高峰时段网约车平台的服务率。


<details>
  <summary>Details</summary>
Motivation: 网约车平台性能高度依赖实时车辆调度能力，传统方法依赖脆弱时空预测，需开发不依赖预测且能应对需求不确定性的调度方法。

Method: 引入网络科学启发的汇点接近度度量，捕捉城市需求与车辆流动趋势；将其整合至共享网络框架，设计无需时空预测的在线调度算法。

Result: 数值研究表明，在有限未来信息条件下，所提算法在高峰时段显著提升请求服务率。

Conclusion: 汇点接近度能有效表征网络演化趋势，基于此的调度算法可增强平台应对需求不确定性的能力，为实时调度提供新思路。
...
### [372] [Scaling up Stability: Reinforcement Learning for Distributed Control of Networked Systems in the Space of Stabilizing Policies](https://arxiv.org/abs/2512.18540)
*John Cao,Luca Furieri*

Main category: eess.SY

TL;DR: 本文提出一种基于图神经网络和Youla参数化的分布式强化学习方法，保证网络化系统的闭环稳定性，并实现策略在不同规模和拓扑网络间的迁移。


<details>
  <summary>Details</summary>
Motivation: 现有分布式强化学习方法难以同时保证策略的可扩展性、表达能力和闭环稳定性，且缺乏对网络拓扑和参数扰动的鲁棒性保证。

Method: 将图神经网络嵌入Youla类幅值-方向参数化框架：幅值模块用GNN处理扰动反馈（保证稳定性），方向模块用GNN处理局部观测；结合近端策略优化进行训练。

Result: 在多智能体导航任务中，该方法在小规模网络上训练的策略可直接迁移到更大规模和未见过的网络拓扑，相比现有MARL基线获得更高回报、更低方差且保持稳定性。

Conclusion: 所提参数化方法能实现可扩展、表达性强且理论保证稳定的分布式控制，对图拓扑和模型参数扰动具有鲁棒性，支持跨网络规模的零样本迁移。
...
### [373] [Distributionally Robust Multi-Agent Reinforcement Learning for Intelligent Traffic Control](https://arxiv.org/abs/2512.18558)
*Shuwei Pei,Joran Borger,Arda Kosay,Muhammed O. Sayin,Saeed Ahmed*

Main category: eess.SY

TL;DR: 提出一个分布鲁棒多智能体强化学习框架，用于应对交通需求不确定性下的信号控制问题，在雅典和Sioux Falls路网中显著提升了交通效率。


<details>
  <summary>Details</summary>
Motivation: 传统基于学习的交通信号控制通常在少数典型交通需求模式下优化平均性能，但在非典型交通条件下表现不佳，需要提高对需求不确定性的鲁棒性。

Method: 三阶段方法：1) 使用近端策略优化训练基线多智能体强化学习控制器；2) 构建八个异构交通场景，训练上下文老虎机最坏情况估计器来估计对抗性需求分布；3) 在估计的最坏情况混合分布下微调基线控制器，获得分布鲁棒控制器。

Result: 在所有八个场景以及未见过的Sioux Falls验证网络中，分布鲁棒控制器相比基线持续减少平均排队长度并提高平均速度，在最差场景中排队缩短达51%，速度提升达38%。

Conclusion: 所提出的分布鲁棒多智能体强化学习框架能有效提升交通信号控制器在多样化需求模式下的鲁棒性和性能，为应对现实交通不确定性提供了可行方案。
...
### [374] [Smart nudging for efficient routing through networks](https://arxiv.org/abs/2512.18630)
*Pouria M. Oqaz,Emanuele Crisostomi,Elena Dieckmann,Robert Shorten*

Main category: eess.SY

TL;DR: 提出一种数字化押金返还方案的控制问题框架，通过数字钱包和分散算法优化纸杯回收效率，实现循环经济目标。


<details>
  <summary>Details</summary>
Motivation: 传统回收系统效率低下，缺乏对参与者的有效激励，难以实现循环经济中的资源闭环。需要一种数字化、可扩展的方法来优化逆向物流。

Method: 采用数字钱包追踪物品流转，结合三种控制机制：(1) 分散算法避免节点拥堵；(2) 基于AIMD的分散算法优化各层押金分配；(3) 反馈控制动态调整押金以实现目标吞吐量。

Result: 仿真实验表明，该框架能有效提升纸杯回收系统的吞吐量和效率，验证了分散控制策略在逆向物流中的可行性。

Conclusion: 数字化押金返还方案结合分散控制算法，可为循环经济中的回收系统提供高效、可扩展的解决方案，具有广泛的应用潜力。
...
### [375] [Distribution Network Restoration with Mobile Resources Dispatch: A Simulation-Based Online Dynamic Programming Approach](https://arxiv.org/abs/2512.18885)
*Mingxuan Li,Wei Wei,Yin Xu,Ying Wang,Shanshan Shi*

Main category: eess.SY

TL;DR: 提出一种基于仿真的在线动态规划方法，用于极端事件后配电网恢复中移动资源的实时调度决策，相比滚动优化具有更高效率和更好性能。


<details>
  <summary>Details</summary>
Motivation: 极端事件后配电网恢复过程中，维修时间、路况、新故障等不确定因素需要基于实时信息进行在线决策，而传统滚动优化方法因大规模离散优化问题难以满足在线应用需求，且历史极端事件数据稀缺导致数据驱动方法受限。

Method: 采用基于索引的优先级规则作为基础策略，通过在线仿真评估当前状态和给定动作下的剩余累积损失，选择最小化累积损失的动作；该方法不依赖离线训练，能适应实时信息更新。

Result: 理论证明该策略能改进基础策略性能；在123节点和8500节点系统中的案例研究表明，相比滚动优化方法，所提方法实现了更高效率和更好性能。

Conclusion: 所提出的仿真在线动态规划方法能有效应对配电网恢复中的不确定性，避免数据和收敛问题，适用于历史数据稀缺的极端事件场景，为移动资源调度提供了实用的在线决策工具。
...
### [376] [Rapid stabilization of the heat equation with localized disturbance](https://arxiv.org/abs/2512.19160)
*Patricio Guzmán,Hugo Parada,Christian Calle-Cárdenas*

Main category: eess.SY

TL;DR: 本文研究多维热方程在未知空间局部扰动下的快速镇定问题，提出了一种结合频率Lyapunov方法和符号多值算子的新型多值反馈控制策略，无需扰动建模或估计即可实现指数镇定。


<details>
  <summary>Details</summary>
Motivation: 热方程在未知空间局部扰动下的镇定问题具有挑战性，传统方法需要扰动建模或估计，本文旨在设计一种不依赖扰动先验知识的鲁棒控制方法。

Method: 提出多值反馈控制策略，融合频率Lyapunov方法和符号多值算子；利用极大单调算子理论证明闭环系统微分包含的适定性。

Result: 证明了闭环系统的适定性和指数稳定性，控制方法对未知空间局部扰动具有鲁棒性。

Conclusion: 所提方法将Lyapunov稳定性分析与谱不等式结合，通过符号算子增强鲁棒性，为受扰动热方程镇定提供了一种无需扰动建模的有效框架。
...
### [377] [Semantic Communication for Rate-Limited Closed-Loop Distributed Communication-Sensing-Control Systems](https://arxiv.org/abs/2512.19177)
*Guangjin Pan,Zhixing Li,Ayça Özçelikkale,Christian Häger,Musa Furkan Keskin,Henk Wymeersch*

Main category: eess.SY

TL;DR: 本文提出了一种面向目标的语义通信统一框架，用于分布式感知通信控制（SCC）系统，通过分层语义建模和动态资源分配，在带宽受限条件下优化系统性能。


<details>
  <summary>Details</summary>
Motivation: 分布式智能网络中感知通信控制一体化（ISAC）与闭环控制的融合，需要在严格带宽约束下实现高效信息传输，传统通信方式难以满足任务相关语义的精准传递需求。

Method: 基于Weaver三层次模型建立分层语义框架（L1观测重建、L2状态估计、L3控制误差），提出统一的目标导向语义压缩与速率自适应框架，采用GRU自编码器进行语义压缩，PPO算法实现多传感器速率动态分配。

Result: 在速率受限多传感器LQR系统案例中，该框架能有效提取任务相关语义，根据不同语义层次自适应分配传输资源，在带宽约束下实现各层次性能提升。

Conclusion: 所提框架为分布式SCC系统提供了一种可适应不同语义层次和优化目标的通信范式，通过语义感知的资源分配提升了带宽受限场景下的系统效能。
...
### [378] [Modular Landfill Remediation for AI Grid Resilience](https://arxiv.org/abs/2512.19202)
*Qi He,Chunyu Qu*

Main category: eess.SY

TL;DR: 提出模块化遗留填埋场修复框架，将填埋场甲烷转化为分布式热电联供能源，为AI数据中心提供微电网弹性电力，同时解决环境责任问题。


<details>
  <summary>Details</summary>
Motivation: 美国AI电力需求增长与填埋场甲烷排放持续存在矛盾，且美国因治理碎片化和碳核算激励而陷入填埋依赖，需要在美国制度约束下解决双重挑战。

Method: 提出模块化修复框架，将遗留填埋场视为存量资源，整合挖掘、筛分和并网前热电联供技术，通过系统分析典型AI走廊案例进行评估。

Result: 该模块可减少60-70%场址级甲烷排放、回收城市土地，提供约20MW稳定可孤岛运行电力，虽仅满足超大规模数据中心约5%基础负荷，但能提供关键微电网弹性和黑启动能力。

Conclusion: 修复导向的废物转能源不应视为批量可再生能源的替代品，而应作为战略控制单元，在缓冲关键负荷抵御电网波动的同时解决长期环境责任。
...
### [379] [Vision-Aided Relative State Estimation for Approach and Landing on a Moving Platform with Inertial Measurements](https://arxiv.org/abs/2512.19245)
*Tarek Bouazza,Alessandro Melis,Soulaimane Berkane,Robert Mahony,Tarek Hamel*

Main category: eess.SY

TL;DR: 提出了一种级联观测器，用于估计无人机与三维运动平面平台之间的相对位姿和速度，结合IMU和单目视觉测量，在持续激励条件下实现稳定收敛。


<details>
  <summary>Details</summary>
Motivation: 无人机在动态平台上自主着陆需要精确估计相对运动状态，现有方法在平台任意三维运动下的鲁棒性和收敛性保障不足。

Method: 采用级联观测器结构：1) SO(3)上的互补滤波器估计相对姿态；2) 线性Riccati观测器估计相对位置和速度；3) 针对平台绕法向轴旋转的特殊情况，利用平台线加速度补偿不可观旋转角。

Result: 理论证明级联观测器在持续激励条件下具有几乎全局渐近稳定性和局部指数稳定性；仿真验证了观测器在平台任意三维运动下的有效性和鲁棒性。

Conclusion: 所提方法为动态平台着陆提供了可靠的相对状态估计框架，通过传感器融合和观测器级联设计解决了复杂运动下的可观测性问题。
...
### [380] [Topology-based Conditions for Multiconsensus under the Signed Friedkin-Johnsen Model](https://arxiv.org/abs/2512.19258)
*Aashi Shrinate,Tanmay Siddharth,Twinkle Tripathy*

Main category: eess.SY

TL;DR: 本文研究了网络系统中的多共识问题，提出了一种基于符号有向图的分布式控制律，通过改进的符号Friedkin-Johnsen模型实现子群内共识，且条件仅依赖于图拓扑和符号交互关系。


<details>
  <summary>Details</summary>
Motivation: 现有研究多假设网络具有入度平衡和信任-不信任同质性，这些条件在实际中往往难以满足。本文旨在放宽这些限制，提出更通用的多共识控制方法。

Method: 采用基于对立规则的符号Friedkin-Johnsen模型，分析意见收敛性；提出仅依赖图拓扑和符号交互关系的多共识条件；设计分布式控制律并进行仿真验证。

Result: 提出的控制律能实现多共识，且条件不依赖于边权重；成功放宽了入度平衡和信任-不信任同质性假设；仿真结果验证了所提条件的有效性。

Conclusion: 基于符号Friedkin-Johnsen模型的控制方法为符号有向图中的多共识问题提供了更灵活的解决方案，仅需拓扑和符号信息即可实现子群共识，具有更好的适用性。
...
### [381] [Stability Analysis of a B-Spline Deep Neural Operator for Nonlinear Systems](https://arxiv.org/abs/2512.19291)
*Raffaele Romagnoli,Soummya Kar*

Main category: eess.SY

TL;DR: 本文提出了一种通过混合B样条深度神经算子（HBDNO）分析神经算子稳定性的方法，利用B样条控制点作为观测变量，结合动态模态分解和Koopman算子框架进行谱分析，在保持表达力的同时评估稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有稳定性感知架构通常施加限制性约束，这会削弱模型的普适性。需要一种既能保持神经算子完整表达能力，又能进行稳定性分析的方法。

Method: 采用混合B样条深度神经算子（HBDNO），将输出表示为B样条控制点；使用动态模态分解（DMD）分析控制点动态；将离散动态与Koopman算子框架连接，进行谱特征分析。

Result: B样条控制点可作为训练后稳定性分析的自然观测变量；通过谱分析能够有效评估学习算子的稳定性；数值实验验证了该方法的可行性。

Conclusion: HBDNO提供了一种原则性的稳定性分析方法，在保持模型表达力的同时实现稳定性评估，为安全关键应用开辟了新的研究方向。
...
### [382] [Power feedback strategy based on efficiency trajectory analysis for HCPV sun tracking](https://arxiv.org/abs/2512.19394)
*Manuel G. Satué,Fernando Castaño,Manuel G. Ortega,Francisco R. Rubio*

Main category: eess.SY

TL;DR: 提出一种无需校准的太阳跟踪器控制策略，通过分析发电功率感知太阳位置，适应多种误差源，满足HCPV系统的严格跟踪要求。


<details>
  <summary>Details</summary>
Motivation: 传统太阳跟踪器需要校准以应对机械结构错位、安装偏差等不确定性，过程繁琐且影响精度，需一种自适应免校准的控制方法。

Method: 基于发电功率反馈实时感知太阳位置，设计自适应控制策略，通过实验在工业级跟踪器上验证，涵盖晴天、多云及故意未校准等场景。

Result: 实验表明该策略在晴天和中等多云条件下均有效，能克服机械与安装误差，满足HCPV系统的高精度跟踪需求。

Conclusion: 所提免校准自适应控制策略可显著提升太阳跟踪器的鲁棒性和实用性，降低安装维护成本，适用于高精度光伏应用。
...
### [383] [Hybrid Analytical-Machine Learning Framework for Ripple Factor Estimation in Cockcroft-Walton Voltage Multipliers with Residual Correction for Non-Ideal Effects](https://arxiv.org/abs/2512.19434)
*Md. Tanvirul Islam*

Main category: eess.SY

TL;DR: 提出混合框架，结合仿真数据集与随机森林模型，显著改善Cockcroft-Walton电压倍增器输出纹波预测精度，优于传统解析模型与纯机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 传统解析模型因忽略二极管压降、电容ESR等非理想因素，在高阶数、低频率、重负载工况下严重低估输出纹波，需更精确的预测方法以支持设计优化。

Method: 建立324组MATLAB/Simulink仿真数据集，覆盖阶数、输入电压、电容、频率、负载等参数组合；训练随机森林模型预测仿真与理论纹波峰峰值之间的残差。

Result: 全局均方根误差降低70.6%（从448V降至131V），关键工况下降低66.7%，偏差接近零；在泛化可靠性上优于纯机器学习方法。

Conclusion: 混合框架实现了物理可解释的设计优化，显著提升纹波预测精度，为高压倍增器设计提供可靠工具。
...
### [384] [A Gauss-Newton-Induced Structure-Exploiting Algorithm for Differentiable Optimal Control](https://arxiv.org/abs/2512.19447)
*Yuankun Chen,Zifei Nie,Xun Gong,Yunfeng Hu,Hong Chen*

Main category: eess.SY

TL;DR: 本文提出FastDOC方法，通过利用微分KKT系统的矩阵结构特性，显著加速可微分非线性模型预测控制的轨迹导数计算，在理论和实际应用中均表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有可微分最优控制方法通过构造辅助系统计算轨迹导数，但未充分利用微分KKT系统的矩阵结构特性，存在计算效率提升空间。

Method: 提出FastDOC方法：采用高斯-牛顿近似处理Hessian矩阵，利用所得矩阵的块稀疏性和半正定特性，加速计算密集的矩阵分解步骤。

Result: 理论计算复杂度降低50%，合成基准测试中时间减少最高达180%；在自动驾驶模仿学习任务中验证了方法的实际有效性。

Conclusion: FastDOC通过挖掘矩阵结构特性显著提升了可微分NMPC的计算效率，为结合机器学习与控制理论提供了更高效的框架。
...
### [385] [Exact Recourse Functions for Aggregations of EVs Operating in Imbalance Markets](https://arxiv.org/abs/2512.19473)
*Karan Mukhi,Licio Romao,Alessandro Abate*

Main category: eess.SY

TL;DR: 提出一种克服电动汽车大规模充电优化问题维度灾难的新方法，通过聚合灵活性集合的几何结构实现精确降维和闭式解。


<details>
  <summary>Details</summary>
Motivation: 传统多阶段随机线性规划方法在处理大规模电动汽车充电优化时面临维度灾难，计算复杂度随设备数量和阶段数急剧增加，难以实际应用。

Method: 基于电动汽车群体聚合灵活性集合的permutahedron几何结构，将原问题重构为聚合量形式，构建最优解聚合策略，推导精确的低维多阶段随机线性规划模型。

Result: 获得了预期追索函数的闭式表达式（分段仿射函数），通过未来价格的截断期望给出各仿射区域的斜率和截距，得到追索函数和第一阶段策略的精确形式。数值研究验证了方法的有效性。

Conclusion: 该方法成功克服了维度灾难问题，为大规模电动汽车充电优化提供了具有实际应用价值的高效精确解法。
...
### [386] [Optimal-coupling-observer AV motion control securing comfort in the presence of cyber attacks](https://arxiv.org/abs/2512.19679)
*Farzam Tajdari,Georgios Papaioannou,Riender Happee*

Main category: eess.SY

TL;DR: 提出一种基于最优耦合观测器的框架，用于在传感器攻击下保障自动驾驶车辆的安全性和乘坐舒适性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶安全研究多关注车辆控制安全，但很少考虑攻击对乘客运动舒适性的影响。

Method: 采用线性时变系统建模误差动态，通过线性矩阵不等式方法分析稳定性，设计观测器检测并切换受攻击传感器。

Result: 该方法能快速检测攻击并切换至可靠观测器，在有限攻击下避免碰撞，且ISO-2631舒适性指标显示攻击对舒适性影响可忽略。

Conclusion: 该框架将舒适性纳入自动驾驶安全考量，为提升自动驾驶系统综合安全性提供了新路径。
...
### [387] [SLIM: Semantic-based Low-bitrate Image compression for Machines by leveraging diffusion](https://arxiv.org/abs/2512.18200)
*Hyeonjin Lee,Jun-Hyuk Kim,Jong-Seok Lee*

Main category: eess.IV

TL;DR: 提出SLIM框架，利用扩散模型实现面向机器视觉的低比特率图像压缩，通过关注感兴趣区域和语义增强，在相同比特率下获得更高分类准确率。


<details>
  <summary>Details</summary>
Motivation: 现有图像压缩方法主要针对人类视觉保留过多感知细节，无法在机器视觉任务中有效降低比特率，需要专门针对机器视觉的压缩框架。

Method: 使用预训练潜在扩散模型，压缩器专注于图像潜在表示中的感兴趣区域，利用包含语义信息的ROI聚焦文本描述通过Unet模型增强解压缩潜在表示。

Result: 实验结果表明，在相同每像素比特数条件下，SLIM相比传统面向机器的图像压缩模型获得更高的分类准确率。

Conclusion: SLIM框架能够在推理阶段无需引导掩码即可关注图像ROI区域，实现低比特率压缩，同时增强的解压缩潜在表示既能优化机器视觉任务，又保留人类视觉的感知细节。
...
### [388] [Image-to-Image Translation with Generative Adversarial Network for Electrical Resistance Tomography Reconstruction](https://arxiv.org/abs/2512.18557)
*Wejian Yan*

Main category: eess.IV

TL;DR: 本研究提出使用Pix2Pix生成对抗网络（GAN）来改进电容层析成像（ECT）的图像重建质量，通过模拟和实验数据验证了该方法在定量指标和视觉质量上的显著提升。


<details>
  <summary>Details</summary>
Motivation: 电容层析成像技术因非侵入性、本质安全和低成本而被广泛用于多相流监测，但传统重建方法难以捕捉细节，限制了其更广泛的应用。

Method: 建立了全面的模拟和实验数据库，实现了多种基线重建算法，并引入了Pix2Pix生成对抗网络（GAN）来增强图像重建。

Result: 所提出的GAN在SSIM、PSNR和PMSE等定量指标上均有改善，并能生成高分辨率、边界清晰的图像，不再受网格离散化的限制。

Conclusion: 基于深度学习的Pix2Pix GAN能有效提升电容层析成像的图像重建质量，为多相流监测提供了更精确的解决方案。
...
### [389] [PSI3D: Plug-and-Play 3D Stochastic Inference with Slice-wise Latent Diffusion Prior](https://arxiv.org/abs/2512.18367)
*Wenhan Guo,Jinglun Yu,Yaning Wang,Jin U. Kang,Yu Sun*

Main category: eess.IV

TL;DR: 提出PSI3D算法，利用潜在扩散模型和TV正则化解决大规模三维图像重建问题，在OCT超分辨率任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽能作为贝叶斯逆问题的图像先验，但传统方法难以处理大规模高维数据（如1024×1024×128体积），训练和推理成本过高。

Method: 1. 采用马尔可夫链蒙特卡洛方法逐片重建二维切片；2. 使用二维潜在扩散模型采样；3. 沿拼接轴引入随机总变差正则化以增强切片间一致性。

Result: 在光学相干断层扫描超分辨率任务中，相比传统和基于学习的方法，显著提升大规模科学成像的重建质量，并提供鲁棒可靠的重建结果。

Conclusion: PSI3D算法通过结合潜在扩散先验和随机TV正则化，有效解决了大规模三维图像重建的挑战，为科学成像提供了高质量解决方案。
...
### [390] [ForeSpeed: A real-world video dataset of CCTV cameras with different settings for vehicle speed estimation](https://arxiv.org/abs/2512.19364)
*Massimo Iuliani,Blake Sawyer,Marco Fontani,David Spreadborough,Martino Jerian*

Main category: eess.IV

TL;DR: 本文介绍了ForeSpeed数据集，这是一个专为评估真实世界CCTV视频中车辆速度估计技术而设计的综合性数据集，包含322个视频，涵盖不同摄像机类型、视角和压缩设置。


<details>
  <summary>Details</summary>
Motivation: 视频取证领域对车辆速度估计的需求日益增长，但现有方法的准确性受摄像机规格、采集方法、分辨率、压缩方式和场景视角等多种因素影响，缺乏适用于真实场景的标准化评估数据集。

Method: 构建ForeSpeed数据集，使用三台数字和三台模拟摄像机从两个不同视角录制车辆以已知速度行驶的视频，提供真实道路度量以恢复场景几何，并采用多种压缩因子和设置模拟实际取证场景。

Result: 通过商业软件Amped FIVE对数据集进行基准测试表明，该方法在各种条件下能可靠估计平均速度，但在强透视畸变场景中不确定性范围显著增加。

Conclusion: ForeSpeed数据集公开提供给取证社区，旨在促进现有方法的评估，并推动开发适用于碰撞调查和事件分析的新鲁棒解决方案。
...
### [391] [Selective Phase-Aware Training of nnU-Net for Robust Breast Cancer Segmentation in Multi-Center DCE-MRI](https://arxiv.org/abs/2512.19225)
*Beyza Zayim,Aissiou Ikram,Boukhiar Naima*

Main category: eess.IV

TL;DR: 本文提出了一种针对nnU-Net的选择性、相位感知训练框架，通过质量导向的数据选择提升乳腺肿瘤DCE-MRI分割的鲁棒性和泛化性，在MAMA-MIA挑战赛的原始肿瘤分割任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 乳腺DCE-MRI缺乏标准化的治疗反应分析基准，且现有数据集存在图像质量异质性（如运动伪影、对比度差异），影响了分割模型的稳定性和泛化能力。

Method: 基于nnU-Net框架，采用选择性训练策略，系统分析图像质量和中心特异性变异对分割的影响；通过控制实验比较DUKE、NACT、ISPY1/2数据集，并测试CLAHE等预处理方法；重点使用早期相位图像（0000-0002）和高质量数据。

Result: 包含运动伪影和低对比度的ISPY数据会损害分割性能，即使使用CLAHE预处理仍无效；而对比度清晰、伪影少的DUKE/NACT早期相位数据能提供更稳定的训练条件。

Conclusion: 相位敏感和质量感知的训练策略对异质临床数据的分割可靠性至关重要；盲目扩展数据集可能适得其反，未来需自动化质量数据选择策略。
...
### [392] [Rethinking Coupled Tensor Analysis for Hyperspectral Super-Resolution: Recoverable Modeling Under Endmember Variability](https://arxiv.org/abs/2512.19489)
*Meng Ding,Xiao Fu*

Main category: eess.IV

TL;DR: 本文提出了一种基于LMN块项张量分解的高光谱超分辨率方法，该模型在保持物理解释性的同时，能更好地处理端元变异性等非线性效应，并在理论和实验上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有基于耦合张量分解的高光谱超分辨率方法中，CPD和Tucker模型缺乏物理解释性，而基于线性混合模型的LL1模型在实际中常因端元变异性等非线性效应失效，因此需要一种既具解释性又能灵活建模非线性效应的新方法。

Method: 提出使用秩为$(L_r, M_r, N_r)$的块项张量分解（LMN模型）对高光谱和多光谱图像进行联合建模，该模型能涵盖CPD、Tucker和LL1作为特例，并在适当条件下可证明超分辨率图像的可恢复性。

Result: 在合成和真实数据集上的实验表明，所提出的LMN方法相比现有基于耦合张量分解的方法具有更好的效果和鲁棒性，特别是在处理端元变异性等实际非线性效应时表现更优。

Conclusion: LMN块项张量分解模型为高光谱超分辨率问题提供了一个在表达能力和解释性之间取得更好平衡的框架，其理论可恢复性保证和实验有效性验证了该方法的实用价值。
...
### [393] [Patlak Parametric Image Estimation from Dynamic PET Using Diffusion Model Prior](https://arxiv.org/abs/2512.19584)
*Ziqian Huang,Boxiao Yu,Siqi Li,Savas Ozdemir,Sangjin Bae,Jae Sung Lee,Guobao Wang,Kuang Gong*

Main category: eess.IV

TL;DR: 提出基于扩散模型的动力学建模框架，用于提高全身动态PET参数图像质量，通过预训练分数函数作为先验，结合动力学模型约束进行参数估计。


<details>
  <summary>Details</summary>
Motivation: 动态PET参数成像中，基于动力学模型拟合的体素级参数估计常因拟合过程不适定性和全身PET多床位非连续采集导致的计数有限，导致图像质量低下。

Method: 采用扩散模型框架，以Patlak模型为例，预训练静态全身PET图像的分数函数作为Patlak斜率和截距图像的先验，推理时结合动力学模型作为数据一致性约束指导参数估计。

Result: 在不同剂量水平的全身动态PET数据集上验证，表明该框架能有效提高参数图像质量，具有可行性和良好性能。

Conclusion: 所提出的基于扩散模型的动力学建模框架能显著改善动态PET参数图像质量，为临床和研究应用提供更可靠的定量参数估计。
