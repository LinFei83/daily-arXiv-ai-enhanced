...
TL;DR: 提出DeepSlide，一种人机协作多智能体系统，优化演示文稿的创作与排练全流程，在保持文档质量同时显著提升叙事节奏、注意力引导等交付指标。


<details>
  <summary>Details</summary>
Motivation: 现有AI幻灯片生成器过度优化视觉成品，忽视讲演节奏、叙事衔接及排练准备等动态交付过程。

Method: 基于可控制逻辑链规划器（含时间预算）、轻量级内容树检索器、马尔可夫顺序渲染（样式继承）及沙盒执行（最小化修复），构建嵌套基线双分数榜评估体系。

Result: 在20个领域及多种受众配置中，DeepSlide在文档质量持平强基线，而交付指标（叙事流畅性、节奏精准度、幻灯片-脚本协同与注意力引导）持续取得更大幅度提升。

Conclusion: 通过将设计重点从静态制品转向动态交付过程，可显著增强自动化演示系统的实际讲演效果。
...
TL;DR: 研究了大型语言模型（LLMs）的心智理论改进在人际交互中的实际效果，发现静态基准的提升不一定转化为动态交互中的更好表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准评估ToM能力时忽略了一人称、动态和开放式的人机交互特性。

Method: 提出交互式ToM评估新范式，采用视角和指标转换，系统研究四种代表性增强技术，使用四个真实世界数据集和用户研究。

Result: 静态基准的改进并不总能转化为动态人机交互中的更好性能。

Conclusion: 交互式评估对于开发具有社交意识的下一代LLMs至关重要。
...
TL;DR: 提出SDOF框架，将多智能体执行视为约束状态机，通过在线RLHF意图路由和状态感知调度器实现业务合规控制。在招聘系统中达到86.5%任务完成率，100%精确率拦截非法操作。


<details>
  <summary>Details</summary>
Motivation: 现有编排框架（如LangChain）基于图管道路由任务，缺乏对实际业务流程中阶段约束的强制保障，导致安全与合规风险。

Method: 构建双层防御体系：1）基于生成奖励建模的在线RLHF专用意图路由器；2）状态感知调度器，包含目标阶段有限自动机检查和前/后条件技能注册表验证。

Result: 在6000+企业招聘系统中，7B模型在对抗性路由基准上准确率达80.9%（GPT-4o为48.9%），端到端任务完成率86.5%，拦截全部22种注入/非法操作，消息审计精确率100%，召回率88%。

Conclusion: SDOF通过状态机约束有效保障多智能体系统的业务合规性与可审计性，为工业级应用提供可行框架。
...
TL;DR: 提出了SkillSmith框架，通过编译技能为最小可执行接口，减少大模型智能体推理中的冗余上下文注入和重复推理，显著降低token使用、思考迭代和求解时间。


<details>
  <summary>Details</summary>
Motivation: 现有技能注入智能体推理范式导致无关上下文注入和重复推理规划两大冗余问题，亟需优化以提升效率。

Method: 设计边界优先的编译-运行时框架，离线将技能包编译为最小可执行接口，运行时仅动态访问相关组件。

Result: 在SkillsBench上，相比原始技能，求解阶段token使用减少57.44%，思考迭代减少42.99%，求解时间加速2.02倍，成本降低57.44%；强模型编译的制品可被弱模型复用。

Conclusion: SkillSmith通过边界编译显著提升LLM智能体系统的效率，并支持跨模型技能复用。
...
TL;DR: 指令微调的语言模型在高风险决策中表现出行为公平性，但内部表示中仍保留偏见。研究发现，这些被抑制的表示可能影响模型输出，且因果关系在不同人口群体间不对称。


<details>
  <summary>Details</summary>
Motivation: 探究指令微调模型在公平输出下，内部保留的偏见是否仍具有因果影响力，以及这种影响是否对称。

Method: 使用开放权重模型进行抵押贷款承销实验，通过匹配仅种族姓名不同的申请，结合激活干预和跨层干预方法。

Result: 模型虽无输出偏见，但内部保留并放大人口表示；再注入关键层会导致决策完全反转；这种隐藏偏见不对称，且易受对抗提示和微调影响。

Conclusion: 行为审计不足以保证公平，需结合输出评估与表示分析的双层测试框架用于AI治理。
...
TL;DR: 本文提出了面向MAPDL有限元仿真的轻量级Agent工具库CAX-Agent，通过三层架构和恢复阶梯策略提升LLM可靠性。实证评估表明，仅模型驱动的恢复策略在任务完成率、得分和零干预率上显著优于无恢复和仅规则策略。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在MAPDL有限元仿真中缺乏结构化执行控制、工具封装和故障恢复机制，导致输出不一致且任务失败频发，亟需引入领域特定的调度中间件来管理工具生命周期、工作流状态和恢复升级。

Method: 构建CAX-Agent架构，包含LLM服务层、Agent中间件层和求解器后端三层，设计恢复阶梯（从确定性规则修补、模型驱动重生成、上下文增强到人工干预）。在50个标准结构基准上，对三个恢复策略（无恢复、仅规则、仅模型）进行3次重复实验（共450次案例运行），由两名独立评审员在盲法条件下评分。

Result: 仅模型策略取得最佳完成率（0.9267）、任务得分（3.59/4）、总分（9.16/10）和零干预率（0.84），显著优于仅规则（0.7733, 3.17/4, 7.03/10, 0.00）和无恢复（0.6933, 2.74/4, 5.60/10, 0.00），效应量较大（Cliff's delta = 0.81-0.87），评审员间一致性高（Cohen's kappa = 0.84）。

Conclusion: 模型驱动的恢复策略能在简单几何基准上显著提升任务完成质量，但研究范围有限，未来需在更复杂场景中进行更广泛的验证。
...
TL;DR: 本文提出NOVA框架，将生成-验证-积累-重训练循环建模为自适应采样过程，分析其收敛条件、失效模式及污染陷阱，并证明发现新知识所需的累积成本服从Zipf标度律。


<details>
  <summary>Details</summary>
Motivation: 探究AI系统能否通过迭代自我改进发现真正的新知识，以及所需成本。

Method: 提出NOVA框架，建立知识空间上的自适应采样模型；通过Good-Turing估计分析批次多样性；基于尾部等价假设证明标度律。

Result: 找到了知识覆盖有限域的充分条件及四种失效模式；揭示了污染陷阱；证明累积生成成本与发现数目呈幂律关系。

Conclusion: AI自主发现新知识存在根本性限制，成本随发现前沿推进呈超线性增长，专家输入在屏障处价值最大。
...
TL;DR: 提出ICRL框架，通过强化学习联合训练求解器和评判器，使模型无需外部评判即可自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在无评判时无法内化改进，且静态评判器无法提升反馈质量。

Method: ICRL通过分布校准重加权和角色分组优势估计，联合训练共享骨干网络的求解器和评判器，以评判器奖励驱动求解器性能提升。

Result: 在代理和数学推理任务中，相比GRPO平均提升6.4和7.0个百分点，8B评判器性能堪比32B版本。

Conclusion: ICRL能有效将评判引导的成功转化为模型自身能力，实现无需外部反馈的自我改进。
...
TL;DR: 提出基于MCP的自驱动实验室软件架构，支持人类和AI代理通过统一接口操作，并开发了NIMO控制器实现可视化编程。


<details>
  <summary>Details</summary>
Motivation: 现有自驱动实验室软件框架主要面向人类交互，缺乏适合AI代理的标准接口。

Method: 基于模型上下文协议（MCP）设计SDL架构，所有功能通过MCP服务器暴露；开发NIMO控制器，通过MCP工具发现自动生成可视化编程界面。

Result: 通过颜色匹配SDL案例验证了系统的可用性。

Conclusion: MCP架构为人类和AI代理提供了统一接口，提升了自驱动实验室的可访问性和开发效率。
...
TL;DR: 提出了分布式信任框架（DTF），通过证明对象、共识模型、临时执行身份和证据链，使自主AI代理的授权执行可验证、可审计、可回放。


<details>
  <summary>Details</summary>
Motivation: 自主AI代理可能生成语义不安全的行为，传统身份中心化授权不再安全，尤其是主权AI系统中亟需减少操作风险。

Method: 引入证明证明、共识模型、临时执行身份和仅追加证据链，并基于OpenKedge实例化建模。

Result: 框架在假定条件下实现了紧凑授权不变量：无证明不执行、无共识不授权、无证据不突变。

Conclusion: DTF将授权从身份转向证明衍生权限，为主权AI部署中的代理执行提供可治理、可审计和边界化的基础设施基础。
...
TL;DR: Solvita是一个无需更新LLM权重即可持续学习的大模型进化框架，通过四个专用智能体实现闭环编程求解，在多个编程竞赛基准上达到最新最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在硬核编程竞赛中仍缺乏严格推理能力，多智能体框架是无状态的，无法利用历史经验积累知识。

Method: 采用四智能体架构（Planner、Solver、Oracle、Hacker），每个智能体配以可训练图结构知识网络，通过强化学习（将通过/失败判定、测试质量、对抗漏洞等信号作为更新反馈）动态路由后续查询。

Result: 在CodeContests、APPS、AetherCode和实时Codeforces轮次中，Solvita达到代码生成智能体最新最佳水平，优于现有流水线，准确率较单次基线翻倍。

Conclusion: 通过图结构知识网络和强化学习信号反馈，Solvita实现了无权重更新的持续学习，有效提升了LLM在复杂编程任务中的推理可靠性。
...
TL;DR: SMCEvolve将LLM驱动的程序演化建模为从奖励倾斜的分布中采样，并用序贯蒙特卡洛方法逼近，提供了组件设计指南和收敛保证。


<details>
  <summary>Details</summary>
Motivation: 现有LLM程序演化框架缺乏组件设计原则和收敛保证，需要更系统化的方法。

Method: 将程序搜索重新表述为奖励倾斜分布采样，用序贯蒙特卡洛采样器近似，包含自适应父代重采样、变异与接受混合、自动收敛控制三个核心机制，并提供有限样本复杂度分析。

Result: 在数学、算法效率、符号回归和端到端机器学习研究基准上，SMCEvolve以更少的LLM调用次数超越了现有最优演化系统，并实现了自动终止。

Conclusion: SMCEvolve通过SMC视角提供了原则性的演化框架，具有理论保障和实际高效性。
...
TL;DR: 提出LaMR框架，将代码相关性分解为语义证据和依赖支持两个维度，分别建模，通过混合专家网络动态加权，实现更有效的上下文剪枝，在多个基准测试中节省令牌并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于单一CRF的剪枝方法无法区分代码相关性的不同方面（如语义连续片段和结构支持行），导致建模瓶颈，且大量无关代码浪费令牌预算。

Method: 将代码相关性分解为语义证据和依赖支持两个可解释维度，每个维度由独立CRF建模；使用混合专家门控网络根据查询动态调整各维度权重；融合后由最终CRF层产生剪枝决策；通过AST程序分析从现有语料自动生成多维标签。

Result: 在SWE-Bench等四个基准测试中，LaMR在16次多轮比较中赢得12次；多轮任务节省高达31%令牌；单轮任务Exact Match提升最多+3.5；性能常因去噪而提升，仅有时出现微小下降。

Conclusion: 通过解耦代码相关性的多个维度并动态加权，LaMR能有效过滤噪声，匹配甚至超越未剪枝的全上下文基线，表明该框架在代码理解任务中具有显著优势。
...
TL;DR: 系统评估了前沿大语言模型在零样本目标识别任务中的表现，发现不同模型在证据整合能力上存在根本差异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在规划任务中依赖世界知识而非符号推理，而目标识别作为溯因任务更适合发挥LLM的优势，但目前缺乏系统性评估。

Method: 在经典PDDL基准上进行零样本目标识别评估，分析模型输出的定量准确率和定性推理痕迹。

Result: 部分模型随观测证据增多精度提升，接近基于地标的准确率；另一些模型始终受世界知识先验主导，忽视累积证据。

Conclusion: 目标识别可作为评估LLM规划知识基础能力的基准，模型性能差异源于证据整合能力而非领域熟悉度。
...
TL;DR: 提出信念引擎（BE），一种可审计的信念更新层，将信念表示为关于命题的证据状态，并通过标量立场暴露出来。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的智能体模拟交互时，立场变化的原因不明确（如证据吸收、锚定、角色漂移等），需要可解释的立场动态。

Method: BE将参数提取到结构化记忆中，使用由证据吸收u和先验锚定a控制的对数几率规则更新立场。

Result: 在不同基座LLM上，参数扫描表明控制能可靠地塑造立场动态并保留证据级更新轨迹；在DEBATE数据集上，BE最好地重构了最终立场遵循提取证据的参与者。

Conclusion: BE为研究基于证据的审议提供了可配置基础设施，将开放性、承诺、收敛和分歧与明确的更新假设关联。
...
TL;DR: 通过组合多样化的监控器（包括提示和微调）形成集成系统，能够显著提升对AI代理错误行为的检测能力，特别是在编程任务中。最佳集成系统（3个监控器）相比同质集成提高了2.4倍检测性能，且微调监控器在OOD攻击类型上保持优势。


<details>
  <summary>Details</summary>
Motivation: AI系统在大规模自主代理场景中部署时，其行为的安全性和与用户意图的一致性至关重要。单独监控器难以可靠构建，人工监督不可行，因此需要一种更有效的监控方法。

Method: 构建了12个基于GPT-4.1-Mini的监控器，采用提示和微调两种策略。在编程任务中，候选方案通过标准测试但对抗性输入失败，评估这些监控器的多样性集成效果。

Result: 多样化集成系统优于单个监控器和同质集成。最佳3监控器集成相比同质集成提升2.4倍检测性能，且在独立数据集上表现强劲。微调监控器在所有最佳集成中出现，并在OOD攻击类型上保持优势。

Conclusion: 多样性（而非规模）是集成检测性能的关键驱动力。最佳集成需结合强个体性能与监控器间的低相关性。微调能赋予监控器提示无法实现的检测能力，集成监控可作为实用的AI控制策略。
...
TL;DR: 提出基于影响的团队引导框架（IBTS），通过奖励塑造和轨迹引导提升AI代理在人类-机器团队协作中的零样本协调能力，并在Overcooked-AI的两代理和三代理场景下验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 零样本协调方法依赖伙伴多样性覆盖，但在团队规模扩大和通信降质时不足，需结合额外协调机制。

Method: IBTS框架结合影响塑造（influence shaping）奖励和主动轨迹引导，促使代理发现多样且高性能的交互模式，并引导至强协调模式。

Result: 在模拟伙伴、合成伙伴风格变化及30人真实人类-机器团队实验中，IBTS超越基线，验证了稀疏奖励协调机制与伙伴多样性结合的必要性。

Conclusion: 零样本协调需将稀疏奖励协调机制与伙伴多样性覆盖结合，而非仅依赖多样性，IBTS为可扩展团队协调提供了有效方案。
...
TL;DR: 提出了 NSPI，一种结合 LLM 和符号计算的神经符号框架，用于高效证明多项式不等式，并在高达 10 个变量的基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有符号方法存在可扩展性问题，而 LLM 方法局限于少变量情况。

Method: LLM 提出近似多项式平方和分解猜想，符号计算精化以获得精确证明，并通过 Lean 验证。

Result: 在含 10 个变量的挑战性基准上展示了有效性和可扩展性。

Conclusion: 神经符号方法结合了 LLM 的启发式发现与符号计算的精确性，适用于更复杂的多项式不等式证明。
...
TL;DR: 提出了一个基于人类注意力模式的上下文集成框架X-SYNTH，将True Lead Rate从9.5%提升到61.9%，同时False Lead Rate从90.5%降低到18.8%。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法适用于窄查询，但对于需要理解组织、团队和个体行为模式的企业复杂任务效果不佳（True Lead Rate低，False Lead Rate高，且模型缺乏改进机制）。

Method: X-SYNTH框架：1) 使用Digital Twin Signature对个体行为基线建模；2) 针对每个个体和查询，从7种注意力过滤器中选择最相关的一种；3) 通过四阶段流水线基于行为模式而非查询嵌入组装排序后的上下文。

Result: 在销售线索识别任务中，X-SYNTH将True Lead Rate从9.5%提升至61.9%（6.5倍），同时将False Lead Rate从90.5%降至18.8%。

Conclusion: 企业上下文集成不是检索问题，而是相关性问题；人类注意力是最可靠的参考标准。
...
TL;DR: 本文提出CAPS，一种自适应级联推理框架，通过减少验证器计算量，在多项基准测试中使用更少的token实现更优或相当的性能。


<details>
  <summary>Details</summary>
Motivation: 成对自验证虽然有效，但计算成本高昂，每次判断需完整阅读两个候选解，且现有方法执行大量无信息量的比较。

Method: CAPS沿证据轴（动态调整每个候选的阅读量）和分布轴（自适应分配比较次数）非均匀分配验证计算，形成四级级联流程及可选的救援子程序，并推导出封闭形式的验证器token成本公式。

Result: 在4个自验证模型和5个推理基准（代码与数学）的20个实验组合中，CAPS在14个组合上优于领先的成对验证器，仅用其代码任务25.4%的token预算；在所有20个组合上优于点式自验证。

Conclusion: CAPS能显著降低验证计算成本，同时保持或提升性能，并通过部分证据与完全证据下验证器精度的诊断指标为部署提供依据。
...
TL;DR: 本文提出RTL-BenchMT，一个自动化框架，用于动态维护RTL生成基准，解决基准中的错误案例和过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 当前RTL基准面临错误案例和过拟合两大挑战，人工维护成本高且难以解决。

Method: 提出智能体框架RTL-BenchMT，自动识别和修正错误案例，检测和更新过拟合案例。

Result: 通过框架辅助，深入分析了错误和过拟合案例，并生成了优化后的基准套件。

Conclusion: RTL-BenchMT能有效降低基准维护成本，提升基准质量，并将开源优化后的基准。
...
### [22] [Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP](https://arxiv.org/abs/2605.16205)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: 在对抗性部分可观测环境中，程序化状态抽象和层次化分解比深度推理更有效，且将推理工具分布到层次中会导致性能下降（推理级联）。


<details>
  <summary>Details</summary>
Motivation: 实践者缺乏关于复合LLM代理设计选择（如观测表示、推理方式、任务分解）如何影响性能与成本的指导。

Method: 在CybORG CAGE-2（POMDP）环境中进行控制实验，涵盖5个模型家族、6个模型、12种配置（3475轮次），对比上下文表示、推理工具（自问、自批判、自改进）和层次化分解（ReAct vs. 子代理委托）。

Result: （1）程序化状态抽象每token回报提升最高76%；（2）推理工具分布在层次中导致性能下降3.4倍，token消耗增加1.8-2.7倍（推理级联）；（3）无推理的层次化分解实现最佳绝对性能，上下文工程比推理更经济。

Conclusion: 在结构化对抗性POMDP中，应优先投资程序化基础设施和清晰任务分解，而非深层次推理，因两者结合可能相互干扰。
...
TL;DR: 提出FORGE协议，通过结构化文本记忆演化提升LLM智能体决策，在CybORG CAGE-2环境中将平均回报提升1.7-7.7倍。


<details>
  <summary>Details</summary>
Motivation: 探索LLM智能体能否通过自生成记忆（无需梯度更新）持续改进决策，特别是针对零样本下表现极差的随机网络防御任务。

Method: 两阶段协议：内层用Reflexion反思失败轨迹生成规则/示例，外层在智能体群体间广播最优记忆并冻结收敛实例。

Result: 在12个模型-表示组合中均优于零样本和Reflexion基线；广播机制是关键，示例记忆收益最高，规则记忆性价比最佳；弱模型受益更显著。

Conclusion: FORGE通过群体广播和结构化记忆演化，可有效提升LLM智能体在复杂POMDP任务中的表现，但证据仅限于CAGE-2场景。
...
### [24] [Position: Artificial Intelligence Needs Meta Intelligence -- the Case for Metacognitive AI](https://arxiv.org/abs/2605.15567)
*Sergei Chuprov,Richard D. Lange,Leon Reznik,Paulo Shakarian,Raman Zatsarenko,Dmitrii Korobeinikov*

Main category: cs.AI

TL;DR: 本文论证了元认知作为设计原则，能提升AI的准确性、安全性和效率，通过监控自身状态和动态分配资源实现。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统在资源分配和错误处理上缺乏灵活性，受限于固定策略，而心理学中的元认知策略为解决这一问题提供了灵感。

Method: 提出元认知设计原则，结合资源理性AI和心理学元认知策略，开发软件框架支持实际应用，并通过联邦学习案例验证。

Result: 在联邦学习案例中，元认知原则显著提升了学习效率、有效性和安全性。

Conclusion: 元认知是改善AI系统性能和安全性的有效设计原则，需进一步解决理论及实现挑战。
...
### [25] [DRS-GUI: Dynamic Region Search for Training-Free GUI Grounding](https://arxiv.org/abs/2605.15542)
*Yichao Liu,Huawen Shen,Liu Yu,Shiyu Liu,Zeyu Chen,Yu Zhou*

Main category: cs.AI

TL;DR: 提出了一个无需训练的、基于动态区域搜索的GUI grounding框架DRS-GUI，通过模拟人类感知的三种行为（聚焦、移动、分散）并利用蒙特卡洛树搜索(MCTS)进行动作规划，显著提升多模态大模型在复杂UI界面中的指令相关元素定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在处理高分辨率截图中大量无关UI组件时，难以准确接地与指令相关的元素，受人类动态调整感知范围行为的启发，需要一种更高效的定位方法。

Method: DRS-GUI框架包含一个轻量级UI感知器（执行聚焦、移动、分散三种类人感知行为）和一个基于蒙特卡洛树搜索(MCTS)的Action Planner（通过区域质量奖励评估选择高相关区域，裁剪冗余UI元素）。

Result: 在ScreenSpot-Pro上，对通用和GUI专用MLLM（Qwen2.5-VL-7B和UGround-V1-7B）实现了14%的性能提升，显著增强了接地性能和泛化能力。

Conclusion: DRS-GUI通过动态区域搜索有效提升了MLLM在复杂GUI中的元素定位准确性，且无需额外训练，具有良好的通用性。
...
TL;DR: 本文提出STAR框架，通过将根因分析代理失败分解为四个可定位阶段并实施阶段感知审计与修复，显著提升了微服务AIOps中LLM代理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM根因分析代理因早期错误在推理轨迹中传播导致最终诊断不可靠，缺乏细粒度失败定位与修复机制。

Method: 将RCA工作流分解为证据包、假设集、分析结构和决策报告四个阶段，构建阶段可定位审计、预算感知快/慢路由、反事实候选评估和阶段特定补丁-重放修复机制。

Result: 在公共大规模基准和实际生产数据集上，STAR一致提升了根因定位和故障分类性能，以高精度识别关键故障阶段，大多初始错误可在1-2轮重放中修复。

Conclusion: 显式建模RCA代理失败的阶段位置是实现可靠、可调试和自修复代理系统的有效途径。
...
TL;DR: 提出了OmniManim框架，通过显式视觉规划、渲染后诊断和局部修复解决LLM生成教育动画中的视觉缺陷问题。


<details>
  <summary>Details</summary>
Motivation: LLM生成的动画代码执行后常出现元素重叠、对齐错误和动画中断等视觉缺陷，且无法从代码层面可靠检测。

Method: 基于共享场景状态、显式视觉规划（含粗到细的边界框去噪和插值感知优化）、结构化渲染后诊断和局部修复。

Result: 在EduRequire-500上，OmniManim在渲染质量上优于单模型和多agent基线；消融实验验证了视觉规划的关键作用。

Conclusion: 显式视觉规划（尤其是粗空间先验、边界框细化和插值感知优化）是提升生成动画质量的核心。
...
TL;DR: 提出TopoEvo，一种拓扑感知的自演化多智能体框架，用于微服务的根因分析，通过多模态对齐、向量量化和结构化推理解决噪声、级联故障和拓扑漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法忽略拓扑信息且存在症状放大偏差，将根因错误归因于下游受害者。

Method: Metric-orthogonal多模态对齐（MOMA）降低冗余；向量量化（VQ）生成症状令牌；多智能体HET工作流验证传播一致性；自演化机制适应拓扑漂移。

Result: 减少了症状放大偏差，在非平稳拓扑下保持鲁棒性。

Conclusion: TopoEvo通过拓扑约束推理有效提升了微服务RCA的准确性和可靠性。
...
TL;DR: 提出了ColPackAgent框架，通过MCP工具服务器和代理技能自主执行胶体堆积的蒙特卡洛模拟，将模拟工具包转化为代理辅助研究流程。


<details>
  <summary>Details</summary>
Motivation: 通用LLM代理因缺乏专用模拟工具和工作流指令，倾向于描述而非可靠执行工作流，因此需要结合领域Python包和MCP协议实现自主模拟执行。

Method: 构建MCP服务器暴露colpack Python包（封装HOOMD-blue硬粒子蒙特卡洛），设计四阶段工作流合同作为代理技能，支持交互、自主和自动研究模式。

Result: 在立方体3D、圆盘与胶囊二元2D体系以及硬圆盘凝固相变的例子中验证了不同模式，并通过17个阶段特定提示比较了多LLM模型的可靠性。

Conclusion: 将领域Python包与MCP工具和可移植代理技能结合，为将模拟工具包转变为代理辅助研究流程提供了实用途径。
...
TL;DR: 提出PRISM框架，将提示工程从一次性任务转变为持续可靠性工程，通过每日模拟和修复来应对LLM行为漂移，在企业对话代理中实现99%的生产可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化框架仅解决一次性编译时质量，忽略了生产环境中LLM非确定性行为漂移导致的提示回归问题。

Method: PRISM闭环框架：基于需求自动生成测试用例，模拟多轮对话，用LLM-as-judge评估，诊断根因并精准修复提示，每日循环执行。

Result: 在35个企业代理上部署三周，提示创建时间从2天降至30分钟内，生产可靠性达99%，24小时内检测并修复LLM行为漂移引起的回归。

Conclusion: 持续、模拟驱动的提示优化对于大规模企业对话AI可靠运行既可行又必要。
...
TL;DR: 本文提出NudgeRL框架，通过策略引导实现结构化多样性探索，在不增加大量计算成本的情况下提升RLVR性能。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR受限于探索效率，暴力扩大采样计算昂贵，且现有方法对探索方向控制有限。

Method: 提出策略引导方法，在每次采样时附加轻量级策略上下文以生成多样化推理路径，并设计统一目标函数，分解奖励为上下文间和上下文内两部分，通过蒸馏将发现的行为迁移回基础策略。

Result: 在五个数学基准上，NudgeRL优于使用8倍预算的GRPO，并超过基于特权信息的基线方法。

Conclusion: 结构化的上下文驱动探索可高效替代暴力扩大采样和依赖特权信息的方法。
...
TL;DR: 研究实证检验了大语言模型评估用户状态指标的稳定性，发现仅31/213指标满足个体分数信度标准，提出可复现的评估框架。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在对话系统中的应用假设其评估用户状态的指标在个体分数层面稳定可解释，但该假设未经检验。

Method: 采用复制评估流程，测试三种双峰大语言模型（GPT-4o audio、Gemini 2.0 Flash、Gemini 2.5 Flash）的213个指标，分别分析个体与聚合信度。

Result: 个体分数不稳定阻碍实时自适应解释，但聚合后部分指标仍可用于事后分析；仅31个指标满足信度标准。

Conclusion: 指标信度非默认属性，需显式验证可靠性并持续监控，才能支持负责任的自适应系统AI设计。
...
TL;DR: 提出了ALSO框架，通过在线对抗策略优化解决多智能体社交模拟中的非平稳性问题，显著提升社交智能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM社交代理依赖静态角色，而离线强化学习或外部规划器不适应社交环境的非平稳性且训练成本高。

Method: 将多轮交互建模为对抗性多臂老虎机问题，结合静态角色和动态策略指令作为臂；使用轻量级神经代理预测奖励并泛化稀疏反馈。

Result: 在Sotopia基准上，ALSO在动态环境中持续优于静态基线和现有优化方法。

Conclusion: 验证了在线对抗策略优化对构建鲁棒社交代理的有效性。
...
TL;DR: 提出了SaaS-Bench基准，包含23个可部署的SaaS系统和106个真实工作任务，用于评估LLM在复杂SaaS环境中的长时程操作能力，最强模型端到端完成率不足4%。


<details>
  <summary>Details</summary>
Motivation: 现有基准多基于简化设置或短时任务，难以评估AI代理在真实专业工作流中的能力，SaaS环境因其动态性和跨应用协调成为理想的评估场景。

Method: 构建了包含23个真实SaaS系统的基准测试，设计106个长时程任务，采用加权验证检查点评估任务完成度和部分进展，支持纯文本和多模态设置。

Result: 代表性LLM代理在SaaS-Bench上表现困难，最强模型端到端任务完成率低于4%，暴露了规划、状态跟踪、跨应用上下文维护和错误恢复方面的局限。

Conclusion: 现有LLM在复杂SaaS工作流中能力严重不足，该基准为评估和提升代理在现实专业环境中的表现提供了有效工具。
...
TL;DR: 本文提出AIRA-Compose和AIRA-Design双框架，让LLM智能体自主设计基础模型架构与机制，发现超越人工设计的Transformer和混合模型。


<details>
  <summary>Details</summary>
Motivation: 探索递归自我改进，研究LLM智能体能否自主设计超越标准Transformer的基础模型。

Method: AIRA-Compose使用11个智能体进行高层架构搜索，经24小时预算评估百万参数候选，并外推至更大规模；AIRA-Design使用20个智能体编写新颖注意力机制与训练脚本。

Result: 14种架构在1B规模预训练后一致优于Llama 3.2基线；下游任务准确率提升2.4%-3.8%；扩展效率提升54%-71%；智能体设计的注意力机制在Long Range Arena上接近人类SOTA；Autoresearch基准验证性能超越已发表最小值。

Conclusion: AI智能体能自主发现匹配或超越人工设计的架构与算法优化，为下一代基础模型发现建立新范式，迈向递归自我改进。
...
TL;DR: 提出强化学习中世界模型利用（model exploitation）的正式定义，建立其与奖励黑客（reward hacking）的理论联系，并证明在大型策略集中利用不可避免，通过松弛定义推导出安全规划的时间范围。


<details>
  <summary>Details</summary>
Motivation: 现有奖励黑客的定义无法直接推广到模型利用场景，且其不可避免性证明不适用于模型利用，亟需建立统一理论框架。

Method: 基于马尔可夫决策过程（MDP）形式化模型利用定义，通过对比奖励黑客的不可规避性证明，发展推广理论；引入松弛定义及安全水平面（safe horizon）分析。

Result: 证明了在大型策略集上模型利用本质上不可避免；有限策略集中保障免于奖励黑客的条件无法排除模型利用；松弛定义下的安全水平面可避免利用。

Conclusion: 建立了奖励黑客与模型利用之间的正式桥梁，揭示了基于世界模型的安全规划的理论极限。
...
TL;DR: 提出了面向精确GUI构建的基准PAGE Bench和拓扑感知代理PAGER，显著提升了点级精确GUI控制的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理依赖区域容忍范式，但精确几何构建要求点级精确，局部错误可导致级联拓扑失败，需研究精度敏感的GUI任务。

Method: 构造PAGE Bench（4906个问题、22.4万像素级动作），提出PAGER：依赖结构化规划与像素级执行，结合像素监督微调和精度对齐强化学习。

Result: 通用多模态模型动作类型准确率>88%但任务成功率<6%；PAGER实现4.1倍任务成功率提升，步骤成功率从<9%提升至>62%。

Conclusion: 语义-执行差距显著，基于拓扑感知的代理能有效弥合此差距，在点精确GUI控制上达到新最优。
...
TL;DR: 提出了事件-图基板这一新型世界模型，通过只追加的RDF三元组日志表示智能体状态，并利用结构化干预词汇进行反事实查询，在CLEVRER和Smallville基准上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在可检查性、精确反事实推理和跨领域迁移方面存在不足，缺乏一种既具备符号系统可解释性又支持精确反事实查询的通用方法。

Method: 形式化定义了事件-图基板类，证明了解释性查询与反事实查询的对偶性，将两者归约为因果祖先遍历算法，并实现了基于领域无关基板运行时的CLEVRER-DSL解释器。

Result: 在CLEVRER数据集上，基板在所有四个问题类别上超越了NS-DR符号基线（分别提升9.89、20.26、17.65和0.80个百分点），在描述性和解释性问题上超越参数化ALOE基线；在twin-EventLog基准上联合准确率超越Llama-3.1-8B 18.80个百分点。

Conclusion: 事件-图基板通过三元组级可检查性、精确反事实支持和跨领域零样本迁移，在视觉推理任务中取得了显著改进，但反事实和预测能力仍有提升空间。
...
TL;DR: 提出BISON系统，结合低级模仿学习与高级符号抽象，通过双层策略实现长期规划，在MetaWorld基准测试中优于VLA和端到端方法。


<details>
  <summary>Details</summary>
Motivation: 模仿学习难以生成长程计划，而高层符号抽象擅长高效可解释的长期规划。

Method: 双层策略：低级神经网络策略从演示中学习操控，高级符号策略从演示的符号抽象归纳生成。

Result: 在扩展MetaWorld基准中，BISON优于VLA和端到端方法；高层策略可秒级解决10,000个对象的规划。

Conclusion: 结合低级模仿学习与高级符号抽象可有效解决长程规划问题。
...
TL;DR: 提出将资源约束项目调度问题建模为含时间Petri网可达图上的最优搜索，结合关键路径与资源下界的A*启发式算法，在PSPLIB基准上优于MIP求解器。


<details>
  <summary>Details</summary>
Motivation: 现有MIP方法在大规模资源约束调度问题上求解效率与成功率不足，需探索更高效的搜索方法。

Method: 使用相对延迟令牌的时间变迁Petri网建模RCPSP，通过一致性A*搜索结合关键路径与资源下界启发式函数寻找最优调度。

Result: 在PSPLIB基准测试中，成功率和求解时间均显著优于SCIP、CBC等MIP求解器。

Conclusion: 基于Petri网与A*的搜索方法在处理资源约束项目调度时具有优势，其性能瓶颈（资源紧度）与MIP（规模）正交，可互补解决不同难度实例。
...
### [41] [Reasoners or Translators? Contamination-aware Evaluation and Neuro-Symbolic Robustness in Tax Law](https://arxiv.org/abs/2605.16052)
*Parisa Kordjamshidi,Samer Aslan,Madhavan Seshadri,Leslie Barrett,Enrico Santus*

Main category: cs.AI

TL;DR: 本研究发现大型语言模型在法律推理中的表现可能因数据污染而膨胀，并通过系统性实验证明神经符号框架在组合性、鲁棒性和泛化能力上优于单一LLM。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在法律推理中的表现是否反映了真实的推理能力，还是由于数据污染导致的假象，这一问题尚不清楚。

Method: 实施污染检测协议，构建包含案例和规则变体的新型测试套件，对比单一LLM与将法规文本翻译为形式表示并委托符号求解器的混合系统。

Result: 神经符号框架在法律推理中展现出更可靠、鲁棒的性能，并能在未见情境下实现更好的泛化。

Conclusion: 法律推理本质上是组合性的，神经符号框架为法律AI提供了更坚实的基础，优于数据污染下表现膨胀的LLM。
...
### [42] [ScreenSearch: Uncertainty-Aware OS Exploration](https://arxiv.org/abs/2605.16024)
*Michael Solodko,Justin Wagle*

Main category: cs.AI

TL;DR: 本文针对桌面GUI智能体在部分可观测性下的探索问题，提出ScreenSearch系统，结合结构化的屏幕检索与去重、模糊感知的PUCT图搜索，在11个桌面应用上收集超100万张截图和3万个去重状态，揭示了新颖性与模糊性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 桌面GUI智能体面临部分可观测性：视觉相似的屏幕可能对应不同工作流状态，导致局部合理行动产生截然不同的结果，需进行计算机/OS状态探索以有效决策。

Method: 系统由结构屏幕检索与去重层（将UIA树转换为位置感知结构特征，通过稀疏词搜索和元数据过滤器索引屏幕）和模糊感知的PUCT图-波段搜索（定义基于匹配动作结果分散度的可扩展模糊信号，结合前沿奖励驱动大规模探索与重放策略评估）组成。

Result: 在11个桌面应用上收集了超100万张截图和3万个去重状态，发现清晰的新颖性-模糊性权衡：某些策略快速降低模糊性但发现少量前沿；附录消融表明更强的提议先验可改善唯一状态发现。

Conclusion: 状态身份、提议质量和模糊感知搜索对于决定何时试探和何时承诺至关重要，仅降低模糊性不足以作为探索目标。
...
TL;DR: 本文对常步长Q学习进行了符号分离的有限时间误差分析，揭示了误差动态中的最大诱导不对称性，并提供了确定性和随机递归的有限时间界。


<details>
  <summary>Details</summary>
Motivation: 现有Q学习分析大多关注整体收敛速率，忽略了正负误差的对称性差异及其与过高估计的联系。

Method: 基于切换系统表示，将误差分解为负部和正部，分别用线性时不变系统（LTI）和线性切换系统控制，并利用最大算子特性进行界分析。

Result: 负侧LTI保证不慢于正侧切换保证，可能产生更快的指数包络；识别出最大诱导的不对称性，导致正误差被选择传播而负误差受最优策略约束。

Conclusion: 常步长Q学习的误差动态存在非对称结构，正负误差收敛速率不同，该机制解释了过高估计现象，并为有限时间收敛提供了精确界。
...
TL;DR: 本文提出了探索-执行范式，通过解耦信息收集和任务执行来增强大型语言模型代理在陌生环境中的适应性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理在陌生环境中常因过早依赖先验知识而失败，自主探索能力未被充分研究。

Method: 采用探索检查点覆盖率作为可验证指标，并开发了交替任务执行 rollout 和探索 rollout 的训练策略。

Result: 标准任务导向强化学习训练的代理表现出狭窄和重复的行为，而探索-执行范式显著提升了泛化能力。

Conclusion: 学习系统探索对于构建通用且实用的代理至关重要。
...
TL;DR: 提出属性引导的LLM程序合成方法，通过提供违反属性的具体反例来降低生成和评估成本，并在PDDL规划领域生成直接启发式函数。


<details>
  <summary>Details</summary>
Motivation: 现有LLM程序合成仅依赖简单分数，缺乏失败原因指导，导致大量无效生成和高昂的LLM推理与评估成本。

Method: 检查候选程序是否满足形式化属性，违反时提前停止评估并返回具体反例，引导LLM修复；在PDDL规划领域自动验证直接启发式性质。

Result: 在10个规划域上，合成程序平均生成次数减少7倍，无需搜索即可解决更多任务，评估计算量降低数个数量级。

Conclusion: 当问题存在可验证属性时，属性引导的LLM合成可降低成本并提升程序质量。
...
### [46] [ShopGym: An Integrated Framework for Realistic Simulation and Scalable Benchmarking of E-Commerce Web Agents](https://arxiv.org/abs/2605.16116)
*Chinmay Savadikar,Mingyu Zhao,Yuanzheng Zhu,Han Li,Shuang Xie,Alberto Castelo,Tianfu Wu,Lingyun Wang*

Main category: cs.AI

TL;DR: 提出ShopGym框架，通过ShopArena模拟真实商店环境并结合ShopGuru合成多样化评测任务，解决了电商智能体评测中真实性与可控性不可兼得的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，真实商店不可控且不可复现，手工构造的测试集缺乏多样性和规模，缺少同时满足真实性、多样性、可控性、可检查性和可复现性的评测方法。

Method: ShopArena基于匿名化商店规范将在线商店转化为独立沙盒环境；ShopGuru依据七类技能在沙盒中合成评测任务，两者结合生成可重置、可检查的稳定评测实例。

Result: 在224个任务（6个沙盒商店，含合成与真实数据）上的图结构分析和智能体行为评估表明，合成商店保留了真实商店的关键结构特性，智能体在合成商店上的表现与真实商店正相关。

Conclusion: ShopGym框架能有效生成兼具真实性和可控性的电商评测环境，为智能体性能提供可复现、可扩展的比较基准。
...
TL;DR: 本文用结构因果建模形式化了二元道德理论，通过三种心理算子解释道德判断，并展示了在AI政策中的应用。


<details>
  <summary>Details</summary>
Motivation: 现有道德理论缺乏精确的形式化框架，难以在AI中忠实模拟人类道德认知。

Method: 使用结构因果建模符号形式化二元道德理论，引入类型化算子、完成算子和效价依赖推理机制，并解决二元限制下的可扩展性问题。

Result: 成功将TDM形式化，提出了节点压缩和顺序处理方法，并应用于AI政策设计，如检测冲突义务、维护用户自主性等。

Conclusion: 该代数形式化使神经符号AI系统能够以数学严谨且符合人类道德认知的方式计算道德。
...
TL;DR: 提出了一种结合形式化方法与机器学习的AI审计与监控技术，用于检测和缓解LLM等黑箱AI系统在时间扩展行为约束上的违规。


<details>
  <summary>Details</summary>
Motivation: 当前AI治理缺乏有效的离线审计和运行时监控手段，尤其是对LLM等高级黑箱系统的时间安全约束。

Method: 利用线性时态逻辑（LTL）的语法和语义，提出离线审计、在线监控、预测性和干预性监控技术，包括基于采样的方法。

Result: 该方法在检测时间约束违规上优于LLM基线，小模型也能匹配或超越前沿LLM；预测和干预监控显著降低违规率，同时保持任务性能；实验显示LLM时间推理能力随事件距离、约束数量和命题增加而显著下降。

Conclusion: 形式化方法可有效增强AI系统的可审计性和安全性，为LLM的治理提供了实用工具。
...
TL;DR: 本文通过构建包含10,836个反馈对的基准测试，评估了七个LLM反馈智能体在命题逻辑中的诊断能力，发现模型对最优步骤表现良好，但倾向于过度拒绝有效但次优的推理，且错误验证无效解决方案。


<details>
  <summary>Details</summary>
Motivation: 智能教学系统需要区分最优、有效但次优以及错误的学生解决方案，而这一能力在基于LLM的辅导系统中尚未得到充分测试，为此研究评估其诊断精度。

Method: 使用知识图谱导出的真实标签，在三种反馈条件下，对七个LLM反馈智能体进行命题逻辑基准测试。

Result: 模型在最优步骤上接近完美表现，但系统性地过度拒绝有效但次优的推理，并错误地验证无效解决方案；准确诊断与可操作反馈之间存在差距。

Conclusion: LLM更适合混合架构，由知识图谱模型处理诊断，而LLM支持开放式引导和对话。
...
TL;DR: 提出了首个完全开放（Fully Open）的临床决策支持系统（CDSS）构建管道，包含医生审核的语料库、可复现的数据构建框架和对齐评估协议，五个开源基础模型均提升，Apertus-70B变体实现新秀技术。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-based CDSS不透明，开放权重模型隐藏数据来源和生成管道，缺乏完全可审计、可复现的模型。

Method: 构建统一临床问答语料库（8个公共数据集+3个合成扩展），执行系统去污染、金标签重采样和四医生验证，使用LLM作为裁判协议（校准204名人类评分者），应用至5个完全开源基础模型。

Result: 所有MeditronFO变体优于基础模型；Apertus-70B-MeditronFO在医学基准上提升6.6%（47.2%→53.8%），达Fully Open SOTA；Gemma-3-27B-MeditronFO在58.6%LLM裁判比较中优于MedGemma，HealthBench 58% vs 55.9%。
...
TL;DR: 提出一个利用大语言模型引导树搜索的自主系统，能自动生成传染病概率预测软件，其集成模型效果可媲美甚至超越人工专家模型。


<details>
  <summary>Details</summary>
Motivation: 传统传染病概率预测依赖专家手动建模，劳动密集，难以扩展到更细地理分辨率或应对新发病原体。

Method: 使用LLM引导的树搜索，迭代生成、评估和优化可执行的预测软件，并集成多个机器生成的模型。

Result: 在2025-2026年美国呼吸道季的实时评估中，系统自主生成流感、COVID-19和RSV的多种模型，集成结果始终匹配或超越CDC人工精选的集成模型，且在RSV数据稀缺的冷启动场景下表现良好。

Conclusion: 该框架通过自动将流行病学理论转化为准确透明的代码，克服了建模劳动瓶颈，能以前所未有的规模快速部署专家级疾病预测。
...
### [52] [ReactiveGWM: Steering NPC in Reactive Game World Models](https://arxiv.org/abs/2605.15256)
*Zeqing Wang,Danze Chen,Zhaohu Xing,Zizhao Tong,Yinhan Zhang,Xingyi Yang,Yeying Jin*

Main category: cs.CV

TL;DR: 提出ReactiveGWM，将玩家控制与NPC行为解耦，实现零样本策略迁移的交互式游戏世界模型。


<details>
  <summary>Details</summary>
Motivation: 现有游戏世界模型仅模拟玩家视角，将NPC视为背景像素，无法建模玩家与NPC的交互，缺乏物理理解。

Method: 通过轻量级加法偏置注入玩家动作，利用交叉注意力模块将高层次NPC策略（如攻击、控制、防御）融入扩散骨干，学习游戏无关的交互逻辑。

Result: 在《街头霸王》两个游戏中，保持精细玩家可控性，实现鲁棒的、对齐提示的NPC策略，并支持零样本迁移到其他游戏世界模型。

Conclusion: ReactiveGWM为可扩展、策略丰富的NPC交互铺平了道路。
...
TL;DR: 提出Deep Pre-Alignment (DPA)架构，用小型VLM替代ViT编码器作为感知器，预先对齐视觉与文本空间，减少LLM深度浪费。


<details>
  <summary>Details</summary>
Motivation: 现有VLM中视觉特征与文本空间对齐不足，导致LLM前期层浪费在浅层对齐而非深层推理。

Method: 用小型VLM作为感知器替换标准ViT编码器，实现视觉特征与目标LLM文本空间的深度预对齐。

Result: 4B参数规模下提升1.9点，32B下提升3.0点；语言能力遗忘减少32.9%；跨Qwen3和LLaMA 3.2等模型一致有效。

Conclusion: DPA通过模块化替换视觉编码器，高效提升VLM性能与泛化能力，且计算开销低。
...
### [54] [COPRA: Conditional Parameter Adaptation with Reinforcement Learning for Video Anomaly Detection](https://arxiv.org/abs/2605.15325)
*Darryl Cherian Jacob,Xinyu Liu,Kai Wang,Pan He*

Main category: cs.CV

TL;DR: 提出COPRA框架，通过条件参数自适应动态调整冻结的视觉语言模型以解决视频异常检测中训练与推理不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于VLM的VAD方法存在训练与推理在数据分布和模型配置上的根本性不匹配，导致泛化能力受限。

Method: COPRA生成输入特定的参数更新，在训练和推理期间动态适应每个视频片段，无需固定提示或共享参数更新。

Result: 在标准VAD基准上表现优异，在域内和跨域设置中均优于静态基线，并能泛化到多选视频问答等未见任务。
...
### [55] [One Pass Is Not Enough: Recursive Latent Refinement for Generative Models](https://arxiv.org/abs/2605.15309)
*Mehdi Esmaeilzadeh,Alexia Jolicoeur-Martineau,Chirag Vashist,Ke Li*

Main category: cs.CV

TL;DR: 本文提出递归变换模型（RTM），通过迭代精炼过程替代单次潜变量映射，在多个数据集上同时提升生成图像的质量和多样性，并在召回率（recall）和精确率（precision）上达到最优，同时保持有竞争力的FID。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型主要优化FID指标，但FID已接近饱和且混淆了样本保真度和模式覆盖率，导致模型可能因产生少量锐利、近似重复的图像而获得低FID，但实际存在模式崩溃。为了更全面地评估生成性能，需要同时考虑精确率和召回率，并重点提升多样性和覆盖率。

Method: RTM采用迭代精炼过程替代了基于风格的生成器中传统的单次潜变量映射。与隐式最大似然估计（IMLE）集成后，IMLE本身设计为优化模式覆盖率，RTM通过递归精炼同时提升了质量和多样性。

Result: 在CIFAR-10、CelebA-HQ（256×256）和九个少样本基准上，RTM在保持竞争性FID的同时，实现了当前最优的精确率和召回率。在CIFAR-10和AFHQ-v1（512×512）上，RTM改进了StyleGAN2和StyleGAN2-ADA。与流动匹配基线相比，递归精炼同时改善了质量和多样性，而基线以牺牲覆盖率为代价获得竞争性FID。

Conclusion: 递归精炼方法能够同时提升生成图像的质量和多样性，有效解决模式覆盖问题，并且其益处不限于与IMLE结合，可推广至其他生成模型框架。
...
### [56] [Multimodal Object Detection Under Sparse Forest-Canopy Occlusion](https://arxiv.org/abs/2605.15326)
*Nitik Jain,Mangal Kothari*

Main category: cs.CV

TL;DR: 该论文提出了一种多模态概念验证管道，结合LiDAR、可见光-热成像融合和合成孔径成像技术，用于森林冠层下的人类检测，并通过YOLOv5验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 森林冠层下的人类检测因稀疏、结构化和视点依赖的遮挡而困难，现有单模态方法效果有限。

Method: 采用三种互补方法：评估LiDAR穿透植被能力、多尺度变换与稀疏表示融合可见光-热图像、利用空气光学合成孔径（AOS）抑制冠层杂波，并基于FLIR数据集微调YOLOv5检测器。

Result: LiDAR穿透能力有限；可见光-热融合提升低对比度场景目标可见度；AOS增强森林合成图像中地面检测；YOLOv5在FLIR前三类上平均精度约0.83。

Conclusion: 该研究为无人机部署的搜救与监视系统在森林环境中提供了基线，并推动未来专用森林数据集和实时多模态集成研究。
...
### [57] [Minerva-Ego: Spatiotemporal Hints for Egocentric Video Understanding](https://arxiv.org/abs/2605.15342)
...
TL;DR: 提出了 Minerva-Ego 基准，用于评估复杂的第一人称视觉推理，包含多模态问题和人类标注的推理轨迹，实验表明现有模型与人类性能仍有差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准仅评估输出答案，忽略了中间推理步骤，且大多局限于文本领域，缺乏对第一人称视觉推理过程的全面评估。

Method: 扩展高质量第一人称视频数据源，构建多步骤多模态问题，并进行密集的人工推理轨迹标注；进一步为每个推理轨迹标注时空掩码以指示关键对象。

Result: 当前最先进模型与人类性能存在较大差距；通过提供对象位置（where）和时间（when）提示，可显著提升模型表现。

Conclusion: 空间和时间先验提示能有效增强第一人称视频推理能力，Minerva-Ego 基准为推动这一领域研究提供了重要工具。
...
### [58] [Discretizing Group-Convolutional Neural Networks for 3D Geometry in Feature Space](https://arxiv.org/abs/2605.15368)
*Daniel Franzen,Jean Philip Filling,Michael Wand*

Main category: cs.CV

TL;DR: 本文提出在特征空间中进行采样，以替代群卷积神经网络中密集的几何采样，从而降低计算成本并保持分类精度。


<details>
  <summary>Details</summary>
Motivation: 群卷积神经网络在几何采样时计算成本高昂，尤其在自由度增加（如3D中的平移和旋转）时，限制了实际应用。

Method: 通过特征相似性选择代表性样本进行特征空间采样，解耦几何分辨率与内存和计算成本。

Result: 粗粒度的特征空间采样能显著保持分类精度，允许基于几何相似性的预计算，大幅加速等变3D分类器的训练。

Conclusion: 特征空间采样为权衡计算效率与精度提供了新方法，在3D等变分类任务中表现优异。
...
### [59] [ChangeFlow -- Latent Rectified Flow for Change Detection in Remote Sensing](https://arxiv.org/abs/2605.15375)
*Blaž Rolih,Matic Fučka,Filip Wolf,Luka Čehovin Zajc*

Main category: cs.CV

TL;DR: ChangeFlow是一个基于潜空间整流流的生成式遥感变化检测框架，通过轻量条件信号和采样集成，在四个基准上平均F1达80.4%，比最佳方法高1.3%，推理速度与强基线相当。


<details>
  <summary>Details</summary>
Motivation: 现有逐像素判别方法难以处理区域级标注带来的上下文依赖和模糊性，而现有生成方法计算成本高且条件机制复杂，性能落后于判别基线。

Method: 将变化检测重构为潜空间中的整流流合成，采用结构化轻量条件信号，并支持采样预测集成。

Result: 在四个基准数据集上平均F1为80.4%，比先前最佳方法提升1.3%，推理速度与强基线相当，且样本一致性可提供置信度估计。

Conclusion: ChangeFlow在保持高效推理的同时，通过生成式建模和采样集成提高了变化检测的准确性和鲁棒性，有效处理了模糊区域。
...
### [60] [MR2-ByteTrack: CNN and Transformer-based Video Object Detection for AI-augmented Embedded Vision Sensor Nodes](https://arxiv.org/abs/2605.15423)
*Luca Bompani,Manuele Rusci,Luca Benini,Daniele Palossi,Francesco Conti*

Main category: cs.CV

TL;DR: 提出MR2-ByteTrack方法，在超低功耗MCU上实现实时视频目标检测，通过全-低分辨率交替推理、ByteTrack帧间关联和Rescore修正，在降低计算和能耗的同时保持精度。


<details>
  <summary>Details</summary>
Motivation: 传统视频检测方法因需特征存储或多帧缓冲，无法在内存和计算受限的MCU上运行，而云方案存在带宽、延迟和隐私问题。

Method: Multi-Resolution Rescored ByteTrack (MR2-ByteTrack): 交替使用全/低分辨率推理以降低计算量；ByteTrack关联帧间检测；Rescore算法基于概率并集规则聚合多帧置信度以修正误分类。

Result: 在ImageNetVID上，CNN模型mAP达49.0，Transformer模型达48.7；计算量分别降低53%和32%；部署于GAP9 MCU时节能55%，首次实现MCU上实时Transformer视频目标检测。

Conclusion: MR2-ByteTrack能在极低资源MCU上实现高效视频目标检测，适用于CNN和Transformer架构，并支持实时处理。
...
### [61] [MorphoHELM: A Comprehensive Benchmark for Evaluating Representations for Microscopy-Based Morphology Assays](https://arxiv.org/abs/2605.15383)
*Emre Hayir,Lorin Crawford,Alex X. Lu*

Main category: cs.CV

TL;DR: MorphoHELM是一个用于评估细胞绘画形态学特征提取方法的开放基准，通过多任务、多噪声水平评估发现，经典计算机视觉方法在所有设置中仍优于现有深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 当前形态学特征提取方法评估碎片化，各模型使用不同任务、数据集和指标，缺乏公平比较的标准。

Method: 整合并扩展了细胞绘画领域的评估标准，引入不同批次效应（技术噪声）水平下的任务评估，覆盖迄今为止最广泛的方法。

Result: 揭示了不同方法在检测生物信号时存在权衡，且没有现有模型在所有情境下超越经典计算机视觉分析策略。

Conclusion: 经典计算机视觉方法仍是通用性最强的表示方法，MorphoHELM为公平比较和选择方法提供了基准。
...
### [62] [PanoWorld: Geometry-Consistent Panoramic Video World Modeling](https://arxiv.org/abs/2605.15391)
...
TL;DR: 提出PanoWorld，一个从单张图像和文本生成几何一致360°视频的世界模型，通过深度一致性和轨迹一致性正则化提升几何质量。


<details>
  <summary>Details</summary>
Motivation: 现有全景视频方法仅优化视觉真实感，缺乏对3D场景状态的显式约束，导致深度不一致、对应关系断裂和运动不合理。

Method: 基于预训练透视视频世界模型，引入深度一致性损失（对伪真实全景深度）和轨迹一致性损失（监督3D世界帧位置），并应用球面几何感知的调节与位置编码。

Result: PanoWorld在几何一致性上优于现有方法，同时保持竞争力视觉真实感。

Conclusion: 全景视频生成应视为几何建模问题，以支持具身AI的整体空间理解需求。
...
### [63] [ELDOR: A Dataset and Benchmark for Illegal Gold Mining in the Amazon Rainforest](https://arxiv.org/abs/2605.15397)
...
TL;DR: 提出了ELDOR基准，包含2500公顷以上无人机影像和像素级标注，用于监测亚马逊雨林非法金矿开采的环境影响。


<details>
  <summary>Details</summary>
Motivation: 非法金矿开采难以在精细空间尺度监测，卫星影像因云层和小尺度结构检测不足而受限。

Method: 构建高分辨率基准包含四项任务：语义分割、基于分割的识别、直接多标签分类、视觉-语言模型封闭集识别。

Result: 现有方法对稀有小型采矿结构和细粒度恢复类别效果不佳。

Conclusion: 需要上下文感知和多模态建模以改进监测。
...
### [64] [U-SEG: Uncertainty in SEGmentation -- A systematic multi-variable exploration](https://arxiv.org/abs/2605.15421)
*Michael Smith,Frank P. Ferrie*

Main category: cs.CV

TL;DR: 本文系统研究了不确定性估计在语义分割和全景分割中的若干未充分探索问题，通过大规模实验分析了数据集、骨干网络和下游任务等变量的影响。


<details>
  <summary>Details</summary>
Motivation: 先前研究指出不确定性估计质量对多种变量敏感，但尚未系统分析这些因素如何影响其在实际场景中识别预测误差的效果。

Method: 构建统一的实验框架，对语义分割和全景分割任务进行大规模研究，涵盖多种数据集、骨干网络和下游任务，系统对比不同不确定性估计方法的表现。

Result: a) 全景分割任务通常表现更差，且跨数据集和骨干网络的性能差异大，泛化性无法保证；b) 视频序列的先前帧仅在特定配置下有用；c) 样本多样性在校准任务中表现最好，但整体不及简单方法；d) 确定性方法适用于部分任务，但集成方法在特定条件下能显著提升性能。

Conclusion: 不确定性估计的效果高度依赖于任务、数据集和架构，实际应用需谨慎选择方法；集成方法有潜力，但需满足适用条件。
...
### [65] [Social-Mamba: Socially-Aware Trajectory Forecasting with State-Space Models](https://arxiv.org/abs/2605.15424)
*Po-Chien Luan,Wuyang Li,Yang Gao,Alexandre Alahi*

Main category: cs.CV

TL;DR: Social-Mamba是一种基于循环Mamba块的高效轨迹预测模型，通过将社交互动重构为结构化序列处理，在保持线性计算复杂度的同时实现高精度预测。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的轨迹预测方法在密集人群中计算成本呈二次增长，而状态空间模型虽线性但难以处理非结构化的社交互动。

Method: 提出Cycle Mamba模块，结合自我中心网格、社交三元组分解（时间、自我中心、目标中心扫描）、可学习社交门控和全局扫描。

Result: 在5个基准数据集上达到SOTA精度，参数效率更高，计算可扩展性更好。集成至流匹配框架进一步提升了准确性和效率。

Conclusion: Social-Mamba为轨迹预测提供了灵活、高效的基础框架，代码已开源。
...
### [66] [RIDE: Retinex-Informed Decoupling for Exposing Concealed Objects](https://arxiv.org/abs/2605.15450)
*Chunming He,Rihan Zhang,Dingming Zhang,Chengyu Fang,Longxiang Tang,Jingjia Feng,Fengyang Xiao,Sina Farsiu*

Main category: cs.CV

TL;DR: 提出基于Retinex理论的同质图像分解方法RIDE，通过可学习分解、判别性间隙注意力与对比损失，统一解决伪装物体分割等多类视觉隐藏目标分割任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖RGB图像或异质分解（如傅里叶），无法直接保持像素对齐的判别线索；而物理纠缠现象在复合空间中难以解耦。

Method: ①任务驱动的Retinex分解模块；②判别性间隙注意力机制；③基于反射率特征的对比损失。

Result: 理论证明Retinex分解可保持或严格提升前景-背景判别性，并在多种COS子任务中取得一致优势。

Conclusion: 同质分解策略能有效应对视觉隐藏目标的物理多样性，为密集预测提供新范式。
...
### [67] [Video Models Can Reason with Verifiable Rewards](https://arxiv.org/abs/2605.15458)
*Tinghui Zhu,Sheng Zhang,James Y. Huang,Selena Song,Xiaofei Wen,Yuankai Li,Hoifung Poon,Muhao Chen*

Main category: cs.CV

TL;DR: 提出VideoRLVR框架，通过强化学习与验证性奖励优化视频扩散模型，使其在视觉推理任务中生成符合规则约束的视频，性能超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型侧重于视觉真实性而非逻辑推理，难以满足空间、时间或逻辑约束任务，受语言模型中RLVR方法启发。

Method: 结合SDE-GRPO优化骨干、密集分解奖励和早期步骤聚焦策略，限制策略优化于去噪早期阶段，降低训练延迟约40%。

Result: 在Maze、FlowFree和Sokoban基准测试中，VideoRLVR持续优于监督微调基线，在低成功率场景下密集奖励尤为关键，并超越商业和开源模型。

Conclusion: 验证性强化学习能推动视频模型从感知模仿转向可靠的规则一致视觉推理。
...
### [68] [Entity-Centric World Models: Interaction-Aware Masking for Causal Video Prediction](https://arxiv.org/abs/2605.15466)
*Santosh Kumar Paidi*

Main category: cs.CV

TL;DR: 提出Interaction-Aware JEPA (IA-JEPA)，通过自监督运动中心掩码策略优先处理物理交互，在CLEVRER等基准上显著提升因果推理准确率。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA模型因标准补丁掩码策略忽视物理动态，导致因果推理能力不足。

Method: 采用自监督运动中心掩码策略，针对碰撞或动量传递实体强制重构潜在轨迹。

Result: CLEVRER因果推理准确率14.26%（基线3.22%），潜空间熵增10%，物理能量线性化R²=0.43。

Conclusion: IA-JEPA通过交互偏见打破静态偏差，可扩展地构建内化因果结构的世界模型。
...
### [69] [A Unified Non-Parametric and Interpretable Point Cloud Analysis via t-FCW Graph Representation](https://arxiv.org/abs/2605.15475)
*Haijian Lai,Bowen Liu,Man Xu,Chan-Tong Lam,João Macedo,Benjamin Ng,Sio-Kei Im*

Main category: cs.CV

TL;DR: 提出了一种增强型转置全连接加权图表示方法，用于点云嵌入度量空间，并设计了一个仅使用该网络作为特征提取器的网络，实现了高效、可解释的点云分类、部分分割和语义分割。


<details>
  <summary>Details</summary>
Motivation: 原始t-FCW在点云分类中表现良好，但其有效性的原因和更广泛的应用场景尚不清楚，因此需要深入分析其特性并探索其通用性。

Method: 通过分析增强型t-FCW的特性，设计了一个仅使用该图表示作为特征提取器的网络，并构建了用于分类、部分分割和语义分割的记忆库。

Result: 增强型t-FCW继承了表面描述符的鲁棒性，提供了维度间关系的可解释性，在ModelNet40分类上仅需约7秒（NVIDIA RTX A5000 GPU），且可作为轻量级基线或现有深度模型的互补插件。

Conclusion: 增强型t-FCW是一种高效、可解释的点云处理方法，兼具独立基线能力和插件化扩展性。
...
### [70] [EgoExo-WM: Unlocking Exo Video for Ego World Models](https://arxiv.org/abs/2605.15477)
*Danny Tran,Roberto Martín-Martín,Kristen Grauman*

Main category: cs.CV

TL;DR: 利用 exocentric 视频数据生成结构化人体姿态并转换为 egocentric 视角，以增强 egocentric 世界模型的训练，提升预测与规划性能。


<details>
  <summary>Details</summary>
Motivation: Egocentric 世界模型受限于训练数据稀缺及其对物理动作的局部可观测性；Exocentric 视频虽丰富且能展示身体姿态，但与代理的动作空间缺乏直接对齐。

Method: 从 exocentric 视频提取结构化身体姿态表示动作，基于人体运动学先验将其转换为 egocentric 视频，再用以训练 whole-body 动作条件 egocentric 世界模型。

Result: 转换数据训练的世界模型在预测质量与下游规划任务（如预测身体姿态序列以达成视觉目标）上显著提升。

Conclusion: 该工作开辟了利用任意野外视频构建强大 egocentric 世界模型的途径，可应用于机器人规划与增强现实指导。
...
### [71] [AnyAct: Towards Human Reenactment of Character Motion From Video](https://arxiv.org/abs/2605.15497)
*Liuhan Chen,Lei Zhong,Jiewei Wang,Qin Shuai,Li Yuan,Leidong Fan,Qing Li,Kanglin Liu*

Main category: cs.CV

TL;DR: AnyAct提出从非人类角色视频直接生成可编辑的人类重演动作，利用稀疏局部2D关节运动作为桥梁。


<details>
  <summary>Details</summary>
Motivation: 现有视频动捕方法局限于人类结构，动作重定向需要3D源动作和拓扑信息，缺乏从角色视频到人类动作的直接方法。

Method: 通过增强的3D到2D投影进行人类动作监督、渐进式训练缓解条件模糊性、全局-局部运动解耦。

Result: 在构建的多样角色视频基准上，AnyAct生成的高保真人类重演保留了原始角色的关键运动动态。

Conclusion: 稀疏局部2D关节运动能有效桥接跨结构差异，实现角色视频驱动的人类动作生成。
...
### [72] [When Does Sparse MoE Help in Vision? The Role of Backbone Compute Leverage in Sparse Routing](https://arxiv.org/abs/2605.15484)
*Libo Sun,Po-wei Harn,Peixiong He,Xiao Qin*

Main category: cs.CV

TL;DR: 本研究系统分析了稀疏专家混合（MoE）网络在视觉分类中的表现，发现路由计算比例（ρ）是影响精度-效率平衡的关键因素，多专家路由在ImageNet级别是必要但不充分的条件，并揭示了批次轴调度的失效机制。


<details>
  <summary>Details</summary>
Motivation: 现有MoE在视觉任务中面临专家崩溃和端到端效率提升有限的问题，需要系统研究稀疏路由策略的实际作用条件。

Method: 在CIFAR-10/100、Tiny-ImageNet和ImageNet-1K上采用多种子实验协议，通过控制计算比例ρ、专家数量k和架构因素进行消融实验，并对比Soft MoE变体。

Result: 发现正精度差依赖于足够大的路由计算比例ρ；ImageNet尺度下还需要多专家路由（k≥2）。隐藏大小扫描揭示架构族非关键因素，而仅改变k的消融实验导致精度差反转。Soft MoE按样本变体通过按专家而非批次进行softmax处理，提升了CIFAR-100性能。

Conclusion: 稀疏MoE在视觉中的效果高度依赖路由计算比例和多专家配置，批次轴调度是导致CNN场景下性能下降的主要问题，按样本的Soft MoE可缓解此问题。
...
### [73] [DiffVAS: Diffusion-Guided Visual Active Search in Partially Observable Environments](https://arxiv.org/abs/2605.15519)
*Anindya Sarkar,Srikumar Sastry,Aleksis Pirinen,Nathan Jacobs,Yevgeniy Vorobeychik*

Main category: cs.CV

TL;DR: 提出了DiffVAS，一种在部分观测环境下同时搜索多种目标物体的视觉主动搜索方法，通过扩散模型重建全局场景并结合强化学习进行决策。


<details>
  <summary>Details</summary>
Motivation: 现有VAS方法假设搜索空间完全已知且只能针对单一目标物体，限制了在真实场景（如视野受限、成本高）中的实用性。

Method: 利用扩散模型从局部观测序列重建完整地理空间，并基于目标条件强化学习规划模块引导后续搜索。

Result: 在多种数据集上，DiffVAS在搜索多样目标物体的任务中显著超越现有最优方法。

Conclusion: DiffVAS通过部分观测重建和目标条件规划，有效推动了VAS在真实世界应用中的部署。
...
### [74] [Self-Prompting Diffusion Transformer for Open-Vocabulary Scene Text Editing via In-Context Learning](https://arxiv.org/abs/2605.15523)
*Hongxi Li,Tong Wang,Chengjing Wu,Tianbao Liu,Jiangtao Yao,Xiaochao Qu,Xinxiao Wu,Luoqi Liu,Ting Liu*

Main category: cs.CV

TL;DR: 提出了一种基于自提示的场景文本编辑方法，利用多模态扩散 Transformer 实现开放词汇和风格一致的文本编辑。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖背景信息，忽略原文视觉细节，且受限于预训练字形编码器的可编辑范围。

Method: 构建风格和字形提示直接源图像，采用两阶段训练策略，先自监督预训练再配对图像微调，利用 MM-DiT 的上下文学习能力。

Result: 在多种语言上实现了文本准确性和风格一致性的最优性能。

Conclusion: 方法无需额外编码器，实现了开放词汇和风格一致的文本编辑。
...
### [75] [Tuning-free Instruction-based Video Editing Via Structural Noise Initialization and Guidance](https://arxiv.org/abs/2605.15533)
*Song Wu,Xinyu Chen,Qian Wang,Liang Li,Zili Yi,Junlan Feng*

Main category: cs.CV

TL;DR: 提出一种无需微调、基于指令的视频编辑框架，通过结构噪声初始化策略和噪声引导机制实现更好的编辑效果。


<details>
  <summary>Details</summary>
Motivation: 现有免调方法未充分利用噪声潜变量中的丰富信息，导致编辑结果不理想。

Method: 设计结构噪声初始化策略（SNIS）为编辑区域分配高噪声以促进内容变化，未编辑区域分配低噪声以保持一致性；引入噪声引导机制（NGM）利用生成模型中的视频先验整合噪声潜变量信息指导去噪。
...
### [76] [Learning Dynamic Structural Specialization for Underwater Salient Object Detection](https://arxiv.org/abs/2605.15535)
*Lin Hong,Chenhui Wang,Linan Deng,Yuning Cui,Yu Zhang,Xin Wang,Bojian Zhang,Wenqi Ren,Xingchen Yang,Fumin Zhang*

Main category: cs.CV

TL;DR: 提出了一种基于动态结构特化的水下显著目标检测方法DSS-USOD，通过分解共享基础表示为边界敏感和区域一致特征，并动态协调其贡献，提升了在退化水下环境中的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有USOD方法在水下图像退化下常导致目标定位不准、显著区域碎片化和边界预测粗糙。

Method: 从单张水下图像提取共享基础表示，分解为边界敏感分支和区域一致分支，通过空间协调模块根据局部结构上下文动态调节两分支贡献，并引入协作结构监督促进分支特化。

Result: 在多个基准数据集上实现了优越性能，并在水下机器人上验证了实际效果。

Conclusion: DSS-USOD有效平衡了边界精度和区域一致性，适用于水下物体检测。
...
### [77] [3DTMDet: A Dual-Path Synergy Network of Transformer and SSM for 3D Object Detection in Point Clouds](https://arxiv.org/abs/2605.15546)
*Bingwen Qiu,Yuan Liu,Junqi Bai,Tong Jiang,Ben Liang,Fangzhou Chen,Xiubao Sui,Qian Chen*

Main category: cs.CV

TL;DR: 提出了一种结合状态空间模型(Mamba)与Transformer的3D点云目标检测网络3DTMDet,旨在解决远距离点云检测中感受野扩大与局部几何细节保留之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过1D序列化扩大感受野,但会丢弃稀疏点云的局部几何细节,导致远距离和小目标检测性能下降。

Method: 设计3D混合Mamba-Transformer(3DHMT)模块,采用SSM-Attention-SSM管线,利用SSM的长序列建模能力捕捉全局交互,利用Transformer局部注意力编码精细几何结构;同时引入基于激光雷达物理原理的体素生成块,沿传感器观测方向扩散特征以重建遮挡和远距离区域的完整目标结构。

Result: 在KITTI和ONCE数据集上,3DTMDet超越现有最先进检测器。

Conclusion: 3DTMDet通过混合SSM与Transformer,有效平衡了远距离检测中的全局上下文理解与局部细节保留,显著提升了性能。
...
### [78] [RoiMAM: Region-of-Interest Medical Attention Model for Efficient Vision-Language Understanding](https://arxiv.org/abs/2605.15561)
*Jiayan Yang,Zhuoyu Wu,Wenqi Fang*

Main category: cs.CV

TL;DR: 提出一种高效的视觉语言模型RoiMAM，通过训练无关的ROI生成模块和文本提示增强器，在小于传统模型20%参数量的情况下，在医疗视觉问答任务上提升准确率2%-4.6%。


<details>
  <summary>Details</summary>
Motivation: 现有医疗视觉问答模型依赖大型架构和封闭答案集，效率低且临床应用受限。

Method: 集成训练无关的ROI生成模块与语义选择性抑制以聚焦病灶区域，以及无参数文本提示增强器提供模态特定上下文。

Result: 在SLAKE数据集上准确率提升约2%，在PMC-VQA数据集上提升约4.6%，模型尺寸小于MedVInT-TD的20%。

Conclusion: RoiMAM在保持高效的同时实现更准确的诊断，具有临床适用性。
...
### [79] [MI-CXR: A Benchmark for Longitudinal Reasoning over Multi-Interval Chest X-rays](https://arxiv.org/abs/2605.15574)
*Sunghwan Steve Cho,Yunseok Han,Jaeyoung Do*

Main category: cs.CV

TL;DR: 提出了MI-CXR基准测试，用于评估多间隔胸部X光序列推理能力，发现现有视觉语言模型在纵向医学推理中存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有医学VQA基准多聚焦于单张图像或短时间间隔图像，缺乏对多就诊胸部X光序列中疾病演变的标准化评估。

Method: 构建了基于五次就诊时间线的五选多项选择题基准，包含时间事件定位、间隔变化推理和全局轨迹总结三个任务族。

Result: 14个视觉语言模型平均准确率仅29.3%，略高于随机猜测；模型能产生局部合理的间隔描述，但无法强制执行时间约束或整合证据做出全局一致决策。

Conclusion: MI-CXR揭示了当前视觉语言模型在纵向医学推理中的关键局限性，为评估提供了标准化基准。
...
### [80] [LDGuid: A Framework for Robust Change Detection via Latent Difference Guidance](https://arxiv.org/abs/2605.15582)
*Jiaxuan Zhao,Ali Bereyhi*

Main category: cs.CV

TL;DR: 提出LDGuid框架，通过显式学习并注入语义差异提升遥感变化检测性能，在多个数据集上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有变化检测模型难以显式表示任务相关的语义差异，受此启发开发差异引导方法。

Method: 基于对抗自编码的差异嵌入模块，通过信息瓶颈预训练学习任务相关差异，并将其作为显式引导信号集成到U-Net、BIT、AERNet等基线模型中。

Result: 在LEVIR-CD等数据集上，LDGuid提升了所有基线的分割性能，特别在光谱噪声影响下改进显著，并能有效整合领域知识如光谱指数。

Conclusion: 语义差异学习能显著增强遥感变化检测的鲁棒性。
...
### [81] [Unsupervised 3D Human Pose Estimation via Conditional Multi-view Ancestral Sampling](https://arxiv.org/abs/2605.15583)
*Ryohei Goto,Takuya Fujihashi,Shunsuke Saruwatari,Fumio Okura*

Main category: cs.CV

TL;DR: 提出了一种无需3D监督的单视图3D人体姿态估计方法，利用2D扩散先验和条件多视角祖先采样。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖3D标注或2D-3D对应，泛化到极端姿态时受限。

Method: 基于预训练的2D运动扩散模型，提出条件多视角祖先采样(cMAS)，通过优化3D姿态使其多视角投影符合2D扩散噪声空间，同时对齐给定的2D姿态和人体解剖约束。

Result: 在Yoga数据集上，包括极端姿态的情况下，跨领域性能优于有监督和无监督的SOTA方法。

Conclusion: 证明了2D扩散先验可有效用于无3D监督的3D姿态估计，尤其在域外场景中表现突出。
...
### [82] [AGC: Adaptive Geodesic Correction for Adversarial Robustness on Vision-Language Models](https://arxiv.org/abs/2605.15584)
*Zhiwei Li,Jiacheng Xue,Weining Wang,Ajian Liu,Xingyu Gao,Zhenan Sun,Qi Li*

Main category: cs.CV

TL;DR: 提出了一种无需训练的防御方法AGC，通过自适应几何校正提升CLIP模型对抗鲁棒性，在多个数据集上显著提升鲁棒准确率并降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: CLIP模型存在对抗扰动脆弱性，现有测试时防御方法依赖梯度优化导致高计算开销，需要高效且有效的防御方案。

Method: 提出Adaptive Geodesic Correction (AGC)：识别可靠数据增强作为几何锚点，沿测地线自适应步长校正输入特征，无需参数更新或梯度计算。

Result: 在8个细粒度数据集和3个CLIP骨干上，鲁棒准确率平均提升44.4%，推理延迟降低10倍。

Conclusion: 揭示了CLIP特征的基本几何特性，提供了一种高效鲁棒的多模态部署范式。
...
### [83] [Efficient Image Synthesis with Sphere Latent Encoder](https://arxiv.org/abs/2605.15592)
*Tung Do,Thuan Hoang Nguyen,Hao Li*

Main category: cs.CV

TL;DR: 提出解耦球形编码器框架，将固定预训练图像编码器与独立潜在去噪模型分离，在球形潜在空间训练，消除像素-潜在反复转换，提升效率并缓解目标冲突。


<details>
  <summary>Details</summary>
Motivation: 现有方法如Sphere Encoder推理效率低且存在重建与生成的目标冲突问题。

Method: 解耦为固定预训练图像编码器和独立潜在去噪模型，所有训练和推理均在球形潜在空间完成。

Result: 在Animal-Faces、Oxford-Flowers、ImageNet-1K上生成质量和推理速度均优于Sphere Encoder，与强基线相当。

Conclusion: 解耦设计能有效提升少步图像生成的效率和性能。
...
### [84] [CM-EVS: Sparse Panoramic RGB-D-Pose Data for Complete Scene Coverage](https://arxiv.org/abs/2605.15597)
...
TL;DR: 提出COVER方法将3D资产转换为稀疏、全覆盖、低冗余的ERP全景RGB-D姿态数据集CM-EVS，仅需中位数25帧即可覆盖所有13种室内场景类型。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉学习数据存在稠密重复、注释异构、缺乏几何一致性全景训练接口的问题，需要一种稀疏、可比较、几何一致的全景数据生成方法。

Method: COVER：基于贪心覆盖代理的ERP视点筛选器，通过投影几何到ERP探针、评分增量覆盖并惩罚深度冲突，在有限代理误差下保持标准覆盖近似行为。

Result: CM-EVS包含36,373帧（覆盖1,275个室内场景及室外补充），中位数每场景25帧覆盖所有13种房间类型，优于基线方法的覆盖-冲突权衡。

Conclusion: COVER与CM-EVS提供了稀疏、紧凑、可审计的全景RGB-D资源，为几何一致性的3D学习任务（如3D重建与视觉定位）奠定基础。
...
### [85] [Pretraining Objective Matters in Extreme Low-Data FGVC: A Backbone-Controlled Study](https://arxiv.org/abs/2605.15599)
*Alexander Hackett,Srikanth Thudumu,Ginny Fisher,Mahule Roy,Aisha Sartaj,Jason Fisher*

Main category: cs.CV

TL;DR: 在极低数据量的细粒度分类中，通过比较四种预训练目标（监督学习、对比学习、掩码重建、自蒸馏）对下游表示质量的影响，发现监督和对比学习编码器在线性探测中表现最佳，而掩码重建编码器在非线性探测中优势明显，自蒸馏编码器在此领域表现不佳。


<details>
  <summary>Details</summary>
Motivation: 专家领域中标注成本高，极低数据量情况下需要为预训练编码器选择提供指导性原则。

Method: 使用定制的祖母绿包体分级数据集（三类），比较四种相同容量的ViT-B/16编码器（监督学习、SigLIP2、MAE、DINOv3），通过留一交叉验证和线性/非线性探测评估，采用排列检验控制统计噪声。

Result: 监督和对比学习编码器在线性探测中AUC最高（逻辑回归0.768和0.735；SVM 0.739和0.697），MAE在非线性探测中表现提升（XGBoost AUC 0.713），DINOv3在所有探测族中表现最差。

Conclusion: 在极低数据量FGVC中，优先选择边界增强预训练目标（如监督或对比学习）用于线性探测，当数据允许时考虑重建式编码器用于非线性分类器。
...
### [86] [Neutral-Reference Prompting for Vision-Language Models](https://arxiv.org/abs/2605.15615)
*Senmao Tian,Xiang Wei,Shunli Zhang*

Main category: cs.CV

TL;DR: 提出NeRP方法，通过中性文本提示和参考图像测量类间先验偏好，在不修改模型参数的情况下纠正视觉-语言模型在新旧类上的不对称混淆，缓解基础-新类权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在新类识别上存在基础-新类权衡问题，即提升新类精度常牺牲旧类性能；观察到模型存在系统性的方向性混淆偏差，且旧类偏差可调、新类偏差持续存在。

Method: 使用中性文本提示和参考图像建立预训练类间几何结构中的先验偏好，结合样本似然得到代理分数；当先验主导预测而证据不足时，对易混淆类对执行局部翻转。

Result: 在多种骨干网络和15个少样本/跨域基准上，显著提升新类精度且几乎不损害基类性能。

Conclusion: NeRP有效缓解了基础-新类权衡问题，为视觉-语言模型的提示学习提供了新思路。
...
### [87] [Latent Video Prediction Learns Better World Models](https://arxiv.org/abs/2605.15618)
*Ali J Alrasheed,Aryan Yazdan Parast,Basim Azam,James Bailey,Naveed Akhtar*

Main category: cs.CV

TL;DR: 首次系统比较了四个前沿视频基础模型（V-JEPA系列、VideoPrism、VideoMAEv2）在五个鲁棒性维度上的表现，发现潜在预测模型（V-JEPA系列）在所有维度上具有更鲁棒的特性。


<details>
  <summary>Details</summary>
Motivation: 现有自监督视频模型评估过于依赖干净基准上的单一准确率指标，无法反映其作为世界模型的真实潜力。

Method: 对四个容量匹配的视频基础模型进行五个鲁棒性维度（特征可区分性、像素损坏鲁棒性、细粒度判别、遮挡鲁棒性、时间方向敏感性）的系统对比研究。

Result: 潜在预测模型在像素损坏下退化更平滑、保留类别结构而非几何稳定性、捕获物理接触线索且不需要像素重建、能编码时间方向。冻结V-JEPA 2骨干网搭配轻量探测头在损坏和遮挡鲁棒性上超过完全微调模型。

Conclusion: 潜在预测方法在鲁棒世界建模方面具有明确优势，为自监督视频模型作为世界模型的评估提供了新证据。
...
### [88] [LRCP: Low-Rank Compressibility Guided Visual Token Pruning for Efficient LVLMs](https://arxiv.org/abs/2605.15621)
*Hongyu Lu,Feng Zhang,Wenwei Jin,Huanling Hu,Tianjun Shi,Shikai Jiang,Yao Hu,Jiawei Li*

Main category: cs.CV

TL;DR: 提出了一种基于低秩压缩性的无训练视觉标记压缩框架LRCP，通过主成分分析估计主导子空间并保留投影残差大的标记，大幅降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有注意力方法和表示方法分别存在位置偏差和忽视全局结构的问题，而视觉标记表示表现出低秩结构且子空间稳定。

Method: 利用PCA估计视觉标记的低秩子空间，计算每个标记的投影残差作为评分，保留残差大的标记进行压缩。

Result: 图像理解保留94.7%性能时压缩88.9%标记，视频理解保留97.8%准确率时压缩87.5%标记。

Conclusion: LRCP能有效利用低秩性质压缩视觉标记，显著降低计算开销且保持高性能。
...
### [89] [Learning Disentangled Representations for Generalized Multi-view Clustering](https://arxiv.org/abs/2605.15640)
*Xin Zou,Ruimeng Liu,Chang Tang,Zhenglai Li,Xinwang Liu,Kunlun He,Wanqing Li*

Main category: cs.CV

TL;DR: 提出广义多视角自编码器（GMAE），通过解耦表示学习解决多视角聚类中视图-分布纠缠问题，在13个基准数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度多视角聚类方法在跨视图融合时存在视图-分布纠缠，导致共享潜在空间质量下降和次优聚类结果。

Method: 采用双路径自编码器将源特征解耦为视图特定和视图公共嵌入，通过跨视图对抗判别器引导特定编码器捕获判别特征，并利用互信息调制对齐分布。

Result: 在13个基准数据集上的完整和非完整多视角聚类任务中，GMAE一致优于现有最先进方法。

Conclusion: GMAE通过解耦表示学习和互信息调制有效提升了多视角聚类的性能，尤其处理视图-分布纠缠问题效果显著。
...
### [90] [MaTe: Images Are All You Need for Material Transfer via Diffusion Transformer](https://arxiv.org/abs/2605.15660)
*Nisha Huang,Henglin Liu,Yizhou Lin,Kaer Huang,Chubin Chen,Jie Guo,Tong-Yee Lee,Xiu Li*

Main category: cs.CV

TL;DR: 提出了无需文本引导和参考网络的简化扩散框架MaTe，通过令牌级图像集成实现零样本、免训练的高质量材质迁移。


<details>
  <summary>Details</summary>
Motivation: 现有扩散方法依赖文本、复杂架构或辅助网络，存在文本依赖、额外计算开销和特征错位问题。

Method: 在共享潜空间中通过多模态注意力机制实现令牌级输入图像集成，无需适配器、ControlNet、反演采样或模型微调。

Result: 在零样本、免训练条件下达到最先进的视觉质量和效率，并保持精确细节对齐。

Conclusion: MaTe简化了推理前置条件，在材质迁移任务中优于现有方法。
...
### [91] [DiLA: Disentangled Latent Action World Models](https://arxiv.org/abs/2605.15725)
*Tianqiu Zhang,Muyang Lyu,Yufan Zhang,Fang Fang,Si Wu*

Main category: cs.CV

TL;DR: DiLA通过内容-结构解耦解决潜动作模型的动作抽象与生成质量之间的权衡，实现高保真视频生成与语义结构化的潜动作空间。


<details>
  <summary>Details</summary>
Motivation: 现有潜动作模型在动作抽象和生成保真度之间存在根本性权衡，常需两阶段训练或限制预测为光流。

Method: 提出DiLA，利用内容-结构解耦，使潜动作学习中的预测瓶颈驱动模型将空间布局蒸馏到结构路径，细节内容分离到内容路径，两者协同进化。

Result: 在视频生成质量、动作迁移、视觉规划和流形可解释性上取得更优结果。

Conclusion: DiLA作为统一框架同时实现了高层动作抽象和高保真生成，推进了自监督世界模型学习的边界。
...
### [92] [VAGS: Velocity Adaptive Guidance Scale for Image Editing and Generation](https://arxiv.org/abs/2605.15661)
*Yan Luo,Ahmadou Aidara,Jingyi Lu,Jeremy Moebel,Kai Han,Mengyu Wang*

Main category: cs.CV

TL;DR: 提出一种无需训练的流速自适应引导尺度方法VAGS，根据时间步和速度场一致性动态调整引导强度，提升生成与编辑质量。


<details>
  <summary>Details</summary>
Motivation: 标准分类器自由引导（CFG）在整个扩散轨迹中使用固定引导尺度，与早期噪声主导、后期需要强语义约束的动力学不匹配，且忽略引导速度与模型当前动力学的一致性。

Method: 通过含时间信号项和有界因子的乘积替换固定CFG，该因子结合速度场间的余弦相似度；编辑任务使用源/目标引导速度的对齐，生成任务使用无条件/条件速度的对齐。

Result: 在PIE-Bench、DIV2K（编辑）以及COCO17、CUB-200、Flickr30K（生成）上，VAGS相比固定CFG和现有无需训练的变体，一致提升了结构保真度和生成质量。

Conclusion: VAGS作为固定CFG的即插即用替代方案，无需额外训练或计算，能有效提升扩散模型在编辑和生成任务中的性能。
...
TL;DR: 提出STABLE双系统框架，通过语义推理器和物理校正器交替迭代生成仿真就绪的桌面场景，解决了LLM生成场景中的物体碰撞漂浮问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的任务到场景生成方法因缺乏3D空间推理能力，导致生成的场景存在物体碰撞或漂浮等物理不真实问题。

Method: 采用语义-物理双系统：1) 微调LLM作为语义推理器从任务指令生成粗布局；2) 基于流的物理感知去噪模型作为物理校正器，通过姿态更新优化布局，确保物理合理性与语义一致性。采用渐进生成范式，交替执行两模块。

Result: 生成的桌面场景严格符合任务指令，相比现有方法显著提升了场景的物理有效性。

Conclusion: STABLE框架能有效生成物理合理且语义正确的仿真就绪桌面场景，验证了语义与物理双系统协作的优越性。
...
TL;DR: 提出IVGT模型，通过隐式神经场景表示从无位姿多视角图像中重建连续且一致的3D几何与外观，支持任意3D位置的连续空间查询并预测SDF值和颜色。


<details>
  <summary>Details</summary>
Motivation: 现有视觉几何基础模型通过回归逐像素点图预测显式几何，存在冗余和几何连续性限制。

Method: 采用隐式视觉几何变换器（IVGT），学习规范坐标系下的连续神经场景表示，通过轻量级解码器预测SDF值和颜色，支持提取连续表面几何并渲染多种输出。

Result: 跨场景泛化能力优异，在网格/点云重建、新视角合成、深度/法线估计和相机位姿估计等任务上表现卓越。

Conclusion: 隐式建模有效克服显式方法的局限性，为无位姿多视图几何重建提供了新范式。
...
### [95] [ChronoEarth-492K: A Large Scale and Long Horizon Spatiotemporal Hyperspectral Earth Observation Dataset and Benchmark](https://arxiv.org/abs/2605.15666)
*Haozhe Si,Yuxuan Wan,Yuqing Wang,Minh Do,Han Zhao*

Main category: cs.CV

TL;DR: 提出了首个大规模、时间校准的高光谱自监督学习数据集ChronoEarth-492K，基于NASA EO-1 Hyperion任务17年数据，包含49万+样本，并建立了涵盖静态、短期和长期时间任务的基准评估套件。


<details>
  <summary>Details</summary>
Motivation: 现有高光谱自监督学习数据集时间深度不足，限制了长期时空建模的发展，亟需一个时间跨度大、校准良好的数据集来推动该领域研究。

Method: 基于NASA EO-1 Hyperion任务（2001-2017）构建ChronoEarth-492K数据集，包括49万+辐射校正块；利用六种开源地理产品建立ChronoEarth-Benchmark评估套件，涵盖多种时空任务，并设计标准化评估协议。

Result: 数据集包含492,354个补丁，覆盖185,398个全球位置，其中28,786个站点具有多时间序列（≥3次观测）；通过基准测试报告了多种高光谱基础模型的基线结果。

Conclusion: ChronoEarth和基准提供了首个大规模、时间基础的高光谱表示学习平台，支持系统和长期的时空建模研究。
...
### [96] [VLMs Trace Without Tracking: Diagnosing Failures in Visual Path Following](https://arxiv.org/abs/2605.15672)
*Hyesoo Hong,Minsoo Kim,Wonje Jeung,Sangyeon Yoon,Dongjae Jeon,Albert No*

Main category: cs.CV

TL;DR: 研究发现现有视觉语言模型在执行线条追踪任务时容易丢失目标路径，转向外观相似的干扰物，且标准方法难以解决该问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在多模态基准测试中表现优异，但可能缺乏对基本视觉操作的稳健控制。

Method: 设计受控的线条追踪任务，引入空间邻近的干扰物并减少语义和拓扑歧义，结合行为干预和内部分析方法。

Result: 模型频繁丢失目标路径，转向局部相似的干扰物；扩缩模型规模收益有限，推理策略可部分补偿但代价高，显式追踪指令无效；在电缆和地铁图等真实场景中同样存在路径切换失败。

Conclusion: 线条追踪失败源于局部竞争导致的路径切换瓶颈，现有标准方法无法稳定恢复路径跟随。
...
### [97] [DreamSR: Towards Ultra-High-Resolution Image Super-Resolution via a Receptive-Field Enhanced Diffusion Transformer](https://arxiv.org/abs/2605.15682)
*Qingji Dong,Hang Dong,Mingqin Chen,Rui Zhang,Yitong Wang*

Main category: cs.CV

TL;DR: 提出DreamSR模型，通过双分支MM-ControlNet和训练策略缓解超分辨率中的过度生成问题，提升局部细节。


<details>
  <summary>Details</summary>
Motivation: 现有扩散超分辨率方法在逐块推理时因全局提示与局部语义不匹配导致过度生成，且忽视局部纹理恢复。

Method: 设计双分支MM-ControlNet（局部提示+全局提示）和阶段式训练策略（含感受野增强）。

Result: 在实验中优于现有方法，生成高质量超分辨率结果。

Conclusion: DreamSR有效抑制过度生成并增强细节，实现视觉上忠实的结果。
...
### [98] [How to Choose Your Teacher for Fine Grained Image Recognition](https://arxiv.org/abs/2605.15689)
*Oswin Gosal,Edwin Arkel Rios,Augusto Christian Surya,Fernando Mikael,Bo-Cheng Lai,Min-Chun Hu*

Main category: cs.CV

TL;DR: 提出基于教师预测比值的Ratio 1-2指标用于选择知识蒸馏教师，在细粒度图像识别中使小模型准确率提升17%。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度识别模型资源消耗大，知识蒸馏中教师选择影响学生性能，但缺乏有效选择指标。

Method: 定义Ratio 1-2指标（教师预测前两类概率比值），通过1000+实验（3学生、8教师、8数据集、4训练策略）验证。

Result: 教师选择改进18%，学生模型准确率最高提升17%。

Conclusion: Ratio 1-2是有效的教师选择指标，能显著提升蒸馏效果。
...
### [99] [ElasticDiT: Efficient Diffusion Transformers via Elastic Architecture and Sparse Attention for High-Resolution Image Generation on Mobile Devices](https://arxiv.org/abs/2605.15684)
...
TL;DR: 提出ElasticDiT，通过动态调整空间压缩比和块深度，在单一模型中实现移动设备上图像生成质量与延迟的灵活权衡。


<details>
  <summary>Details</summary>
Motivation: DiT模型在移动设备上计算和内存开销大，现有方法如静态剪枝会导致质量下降，而移动环境需要单一模型动态平衡保真度和延迟。

Method: 集成Shift Sparse Block Attention（SSBA）和Tiny DWT-Distilled VAE（T-DVAE），通过联合调整压缩率和块深度实现动态配置。

Result: ElasticDiT在单一参数集下覆盖广泛质量-延迟权衡，以84.16%平均稀疏度超越Flux模型，T-DVAE以1/8计算成本提供SD3级重建，Flow-GRPO提升GenEval至73.62。

Conclusion: ElasticDiT是灵活的硬件自适应方案，消除多模型需求，为移动设备高性能图像生成提供可行路径。
...
### [100] [3D Segmentation Using Viewpoint-Dependent Spatial Relationships](https://arxiv.org/abs/2605.15708)
*Ayaka Nanri,Klara Reichard,Mert Kiray,Federico Tombari,Benjamin Busam,Asako Kanezaki*

Main category: cs.CV

TL;DR: 提出了一个视角感知的3D指代分割数据集，包含22万个样本，可扩展至数千万，并引入视角表示方法，将mIoU从0.30提升至0.47。


<details>
  <summary>Details</summary>
Motivation: 现有3D指代分割方法未显式表示观察者视角，导致“左”“右”等空间关系模糊难评估。

Method: 利用相机姿态自动标注观察者中心的空间关系（左/右、前/后）和视角无关关系（上/下），构建基准数据集，并编码视角信息以条件化模型。

Result: 现有3D大型多模态模型在视角依赖指令上表现不佳；加入视角表示后，视角依赖关系的分割mIoU从0.30提升至0.47。

Conclusion: 显式编码视角信息能显著提升3D指代分割在视角依赖空间关系上的准确性。
...
### [101] [Semi-MedRef: Semi-Supervised Medical Referring Image Segmentation with Cross-Modal Alignment](https://arxiv.org/abs/2605.15720)
*Yuchen Li,Zhen Zhao,Yi Liu,Luping Zhou*

Main category: cs.CV

TL;DR: 提出Semi-MedRef，一种通过保持图像-文本对齐的半监督框架，在医疗图像分割中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医疗引用图像分割在标签稀缺时成本高，现有半监督方法未充分利用跨模态对齐，且CutMix在多模态中易破坏一致性。

Method: 基于教师-学生框架，包含T-PatchMix、PosAug和ITCL三个对齐保留模块。

Result: 在QaTa-COV19和MosMedData+上，Semi-MedRef在所有标签场景下均优于全监督和半监督基线。

Conclusion: 通过显式维护图像-位置文本对齐，半监督学习可有效减少医疗图像分割的标注负担。
...
### [102] [EntropyScan: Towards Model-level Backdoor Detection in LVLMs via Visual Attention Entropy](https://arxiv.org/abs/2605.15711)
*Xuanyu Ge,Zhongqi Wang,Jie Zhang,Shiguang Shan,Xilin Chen*

Main category: cs.CV

TL;DR: 提出EntropyScan，一种轻量级、与触发器无关的模型级后门检测方法，通过视觉注意力分布的Tsallis熵和Z-score归一化，在两种LVLM架构和三种攻击场景下实现了98.5% F1和96.6% AUC。


<details>
  <summary>Details</summary>
Motivation: 现有后门防御主要关注样本级别（依赖训练数据或触发器知识），而模型级别（判断模型是否被植入后门）的关键任务尚属空白。

Method: 提取LLM初始层的视觉注意力分布，利用Tsallis熵量化后门注入导致的结构性失真，再通过少量良性样本上的参考锚定Z-score归一化进行检测。

Result: 在两种LVLM架构和三种先进攻击场景下，平均F1分数达98.5%，AUC达96.6%。

Conclusion: EntropyScan能高效检测LVLM中的后门模型，不需要触发器知识或大量训练数据，具有实用性和鲁棒性。
...
### [103] [DecomPose: Disentangling Cross-Category Optimization Contention for Category-Level 6D Object Pose Estimation](https://arxiv.org/abs/2605.15728)
*Yifan Gao,Lu Zou,Zhangjin Huang,Guoping Wang*

Main category: cs.CV

TL;DR: 提出DecomPose框架，通过难度感知梯度解耦和稳定性驱动非对称分支，缓解类别间几何异质性导致的梯度冲突和负迁移，提升6D姿态估计性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法共享全部参数，但不同类别几何结构差异大，导致优化信号冲突和负迁移。

Method: (1) 基于梯度诊断量化模块级冲突；(2) 难度感知梯度解耦，按数据驱动难度分组路由；(3) 稳定性驱动非对称分支，为简单类别分配高容量分支，复杂类别用轻量分支。

Result: 在REAL275、CAMERA25、HouseCat6D上减少跨类别优化冲突，取得领先姿态估计性能。

Conclusion: DecomPose有效缓解类别间优化冲突，提升多类别6D姿态估计准确率。
...
### [104] [UAM: A Dual-Stream Perspective on Forgetting in VLA Training](https://arxiv.org/abs/2605.15735)
*Jianke Zhang,Yuanfei Luo,Yucheng Hu,Xiaoyu Chen,Yanjiang Guo,Ziyang Liu,Hongbin Xu,Tian Lan,Jianyu Chen*

Main category: cs.CV

TL;DR: 提出统一动作模型 (UAM)，通过添加类脑背侧通路模块（背侧专家），在保留 VLM 多模态能力的同时提升操控任务性能。


<details>
  <summary>Details</summary>
Motivation: 标准 VLA 模型微调会导致多模态能力退化（实体税），生物视觉双流结构启发分离语义与控制特征。

Method: 在 VLM 基础上并行添加基于生成模型初始化的背侧专家，通过预测视觉动力学的中级推理目标训练，实现端到端联合优化。

Result: UAM 保留 VLM 95% 以上的多模态能力，在多种泛化任务（新物体、组合、指令变化）中取得最高平均成功率。

Conclusion: 语义保持可通过架构分离自然实现，无需冻结权重或辅助数据，且保留的语义能力可迁移至动作语义泛化。
...
### [105] [BiomedAP: A Vision-Informed Dual-Anchor Framework with Gated Cross-Modal Fusion for Robust Medical Vision-Language Adaptation](https://arxiv.org/abs/2605.15736)
*Huanyang Tong,Kai Liu,Fangjun Kuang,Huiling Chen*

Main category: cs.CV

TL;DR: 提出BiomedAP，一种视觉引导的双锚定框架，通过门控跨模态融合与双锚定约束增强视觉语言模型在少样本医学诊断中对提示变异的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学视觉语言模型对提示变化脆弱，传统方法将视觉和文本提示独立优化，在临床噪声描述下导致跨模态对齐不稳定。

Method: 采用门控跨模态融合（层间交互机制抑制无关文本线索）和双锚定约束（基于专家模板的高锚定与少样本视觉原型的低锚定正则化可学习提示）。

Result: 在11个基准上超越基线，实现了有竞争力的少样本准确率，并显著提升了对提示扰动的鲁棒性。

Conclusion: BiomedAP有效解决了提示脆弱性问题，为临床噪声环境下的医学诊断提供了更稳定的跨模态对齐方案。
...
### [106] [HyperDiT: Hyper-Connected Transformers for High-Fidelity Pixel-Space Diffusion](https://arxiv.org/abs/2605.15741)
*Yu He,Lichen Ma,Zipeng Guo,Xinyuan Shan,Jingling Fu,Dong Chen,Junshi Huang,Yan Li*

Main category: cs.CV

TL;DR: 提出了HyperDiT框架，通过超连接跨尺度交互解决像素空间扩散模型的粒度困境，实现高保真生成。


<details>
  <summary>Details</summary>
Motivation: 像素空间扩散模型在全局语义捕获和细节保真之间存在粒度困境，需要统一多尺度信息。

Method: 采用交叉注意力机制替代AdaLN，让细粒度token全局查询多级语义锚点；引入尺度感知旋转位置编码（SA-RoPE）解决空间错位；并利用预训练视觉基础模型（VFM）的寄存器学习稠密语义以减少伪影。

Result: 在ImageNet 256×256上取得1.56的SoTA FID值，直接工作在像素空间。

Conclusion: HyperDiT通过结合细粒度流与语义引导，为高保真像素生成提供了优越范式。
...
### [107] [BARRIER: Bounded Activation Regions for Robust Information Erasure](https://arxiv.org/abs/2605.15737)
*Jan Miksa,Patryk Krukowski,Przemysław Spurek,Dawid Damian Rymarczyk,Marcin Sendera*

Main category: cs.CV

TL;DR: 提出BARRIER框架，通过动态激活空间几何约束实现高效机器遗忘，同时保证其他知识不受破坏。


<details>
  <summary>Details</summary>
Motivation: 现有基于权重的遗忘方法常导致非目标概念被意外抑制，缺乏保留中性知识的数学保证。

Method: 利用区间算术在激活空间的SVD投影上构建边界超立方体，通过有界激活区域驱动遗忘更新，并数学约束模型在保留域上的响应。

Result: 在分类器和扩散模型上达到与最先进方法相当的性能，同时最大化特定概念擦除并保障其他表征完整性。

Conclusion: 将知识保留从经验启发式转化为形式化优化目标，允许在遗忘区域进行激进更新而不损害模型稳定性。
...
### [108] [Learn2Splat: Extending the Horizon of Learned 3DGS Optimization](https://arxiv.org/abs/2605.15760)
...
TL;DR: 提出一种针对3D高斯溅射优化的学习型优化器，通过元学习扩展优化范围，避免性能退化，提升新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 标准优化器未利用场景结构和空间关系，导致优化效率低、收敛慢；现有学习型优化器依赖固定迭代次数和手动学习率调度，易退化。

Method: 结合检查点缓冲区和优化器回滚策略的元学习方案，并设计编码梯度尺度信息的架构以延长优化范围。

Result: 早期新视角合成质量提升，长期优化稳定，零样本泛化到未见过的重建任务。

Conclusion: 学习型优化器可替代标准优化器，无需辅助机制即可在长优化范围内保持稳定性能。
...
### [109] [GRASP: Learning to Ground Social Reasoning in Multi-Person Non-Verbal Interactions](https://arxiv.org/abs/2605.15764)
*Junho Kim,Xu Cao,Houze Yang,Bikram Boote,Ana Jojic,Fiona Ryan,Bolin Lai,Sangmin Lee,James M. Rehg*

Main category: cs.CV

TL;DR: 本文提出GRASP数据集与社会接地奖励（SGR）方法，用于增强多模态大语言模型在多人视频中识别社交互动参与者的能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型在多人视频中难以区分社交互动的参与者，且缺乏结合凝视与手势推理的高质量数据集。

Method: 构建包含29万问答对的GRASP数据集，基于凝视轨迹、指示性手势及其组合设计16类社交事件；提出SGR奖励信号，引导模型利用社交事件进行推理。

Result: SGR在GRASP-Bench上提升性能，同时保持相关社交视频QA基准的零样本能力。

Conclusion: 结合细粒度社交事件数据与专门奖励信号，可有效改善MLLM的社交互动理解能力。
...
### [110] [Reversing the Flow: Generation-to-Understanding Synergy in Large Multimodal Models](https://arxiv.org/abs/2605.15792)
*Yujun Tong,Dongliang Chang,Zijin Yin,Xintong Liu,Yuanchen Fang,Zhanyu Ma*

Main category: cs.CV

TL;DR: 提出生成到理解（G2U）协同框架，通过视觉生成作为中间推理步骤提升多模态理解，无需重训练或外部工具，在12个基准测试中验证效果。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型存在单向性（理解指导生成），但生成如何支持理解缺乏研究。

Method: 框架执行可控生成操作（细节增强、上下文扩展、结构可视化），生成视觉想法并反馈给模型优化感知。

Result: 生成保真度限制感知增益，编辑提示类型影响迁移效率；模型虽能生成合理编辑，但缺乏稳定的任务对齐，无法实现真正自我反思。

Conclusion: 想象力不是理解的终点而是起点，当前模型缺少统一认知中的关键机制。
...
### [111] [Attribute-Grounded Selective Reasoning for Artwork Emotion Understanding with Multimodal Large Language Models](https://arxiv.org/abs/2605.15755)
*Cheng Zhang,Yuer Liu,Zhiyu Zhou,Hongxia Xie,Wen-Huang Cheng*

Main category: cs.CV

TL;DR: 为解决多模态大模型在艺术品情感解释中的属性泛滥问题，本文形式化定义了属性驱动选择性推理任务，构建了含人工标注的情感显著性数据集EmoArt扩展版，并提出基于属性瓶颈引导的多智能体推理框架FAB-G，在情感预测和解释简洁性上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs生成情感解释时列举大量视觉属性，但无法区分哪些属性真正驱动情感判断，缺乏对情感显著性属性的精确建模。

Method: 提出FAB-G监督式多智能体框架：先通过属性级显著性预测模块筛选情感相关属性，再将保留的线索约束后续情感分析任务。

Result: 在情感、唤醒度和效价预测上FAB-G持续优于基线；与人工标注显著性属性的一致性（Dice/Tversky指标）更高；生成的解释更简洁；跨数据集评估表明属性级显著性选择可迁移。

Conclusion: 属性级显著性的监督学习能有效缓解属性泛滥问题，提升情感推理的可解释性和泛化能力，但存在属性级边界案例。
...
### [112] [FSCM: Frequency-Enhanced Spatial-Spectral Coupled Mamba for Infrared Hyperspectral Image Colorization](https://arxiv.org/abs/2605.15880)
*Tingting Liu,Yuan Liu,Guiping Chen,Xiubao Sui,Qian Chen*

Main category: cs.CV

TL;DR: 提出了一种基于光谱信息引导的生成对抗网络（FSCM），用于红外图像彩色化，解决了现有方法在结构失真和语义混淆方面的问题。


<details>
  <summary>Details</summary>
Motivation: 红外图像缺乏自然色彩和精细纹理，限制了目标识别和模型迁移；现有彩色化方法依赖单波段图像，光谱线索不足，且难以建模空间-光谱耦合。

Method: 构建了频率增强的空间-光谱状态空间生成器（含FSB单元），集成状态空间建模、多级小波分解与傅里叶门控的频率增强模块、变形感知采样与稀疏注意力的双流混合门控模块，并引入在线语义分割引导损失。

Result: 在视觉质量和语义保真度上优于现有红外彩色化方法。

Conclusion: FSCM能有效提升红外图像彩色化的结构完整性和语义一致性，尤其适用于复杂道路场景。
...
TL;DR: 提出MultiMat，一种多模态程序合成框架，利用大型多模态模型处理视觉和文本图形表示，更高效地生成程序化材质节点图，在无条件与条件合成中均达到新最优性能。


<details>
  <summary>Details</summary>
Motivation: 传统材质节点图创建需专业培训，现有神经程序合成方法仅用文本表示，忽略了人类易于理解的视觉空间特性，因此需要多模态方法。

Method: 基于生产级材质数据集训练多模态模型，结合约束树搜索推理算法，确保静态正确性并高效搜索程序空间。

Result: 在无条件与条件图形合成中，视觉质量与保真度优于纯文本基线，建立新的最先进性能。

Conclusion: 多模态程序合成能更有效提升材质节点图生成效率与质量，弥补纯文本方法的不足。
...
### [114] [Cross-Modal Registration Between 3D and 2D Fingerprints via Pose-Aware Unwrapping and Point-Cloud Fusion](https://arxiv.org/abs/2605.15796)
*Xiongjun Guan,Jianjiang Feng,Jie Zhou*

Main category: cs.CV

TL;DR: 提出了一个统一框架，用于三维指纹预处理及跨模态注册，实现与接触式/非接触式二维指纹系统的兼容。


<details>
  <summary>Details</summary>
Motivation: 三维指纹虽保留全局几何和局部脊结构并避免接触变形，但与现有二维指纹系统集成困难。

Method: 组合四个组件：非参数可视化展开、点云融合拼接、椭圆姿态归一化、姿态感知跨模态注册。

Result: 三维融合误差约0.09 mm，非接触二维-三维注册达到脊级投影精度，姿态感知展开提升匹配分数。
...
### [115] [Uncertainty-Aware Wildfire Smoke Density Classification from Satellite Imagery via CBAM-Augmented EfficientNet with Evidential Deep Learning](https://arxiv.org/abs/2605.15894)
*Ranjith Chodavarapu*

Main category: cs.CV

TL;DR: 提出一种概率框架，通过卫星图像将野火烟雾严重程度分为轻度、中度和重度三类，并直接在单次前向传递中分解认知不确定性和偶然不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法将烟雾检测视为二分类任务，只能提供点估计，无法给出预测置信度评估，难以满足应急响应、空气质量建模和健康风险管理的需求。

Method: 采用预训练EfficientNet-B3和CBAM模块作为主干，结合证据深度学习头预测狄利克雷浓度参数，无需蒙特卡洛采样即可直接估计真空度（认知不确定性）和不和谐度（偶然不确定性）。

Result: 在16,298个真实卫星图像块上，模型达到93.8%的加权测试准确率（未加权91.1%），ECE=0.0274；选择性预测保留最确定的50%图像块时准确率达96.7%；随着图像质量下降，不确定性单调增加；中度烟雾类别的认知不确定性最高。

Conclusion: 该概率框架能有效评估烟雾严重程度并量化不确定性，真空度可作为实用的扫描质量指标，中度类别很好地建模了过渡性烟雾边界区域。
...
### [116] [Are VLMs Seeing or Just Saying? Uncovering the Illusion of Visual Re-examination](https://arxiv.org/abs/2605.15864)
*Chufan Shi,Cheng Yang,Yaokang Wu,Linhao Jin,Bo Shui,Taylor Berg-Kirkpatrick,Xuezhe Ma*

Main category: cs.CV

TL;DR: 当前视觉语言模型在推理时声称重新检查图像，但实际并未真正关注视觉输入，而是生成看似合理的文本模式。


<details>
  <summary>Details</summary>
Motivation: 模型是否真正进行视觉重检，还是仅产生模仿人类反思的文本模式，这对理解模型能力和可靠性至关重要。

Method: 提出VisualSwap框架，在模型推理后替换为视觉相似但语义不同的图像，并引入VS-Bench数据集（800对图像）进行探测。

Result: 三个VLM（Qwen3-VL、Kimi-VL、ERNIE-VL）在替换后准确率下降高达60%；思考模型比指令模型脆弱近3倍；增大模型规模无缓解；多轮用户指令能恢复视觉基础，但自生成的反思语句无效。

Conclusion: 当前VLM声称进行视觉重检时倾向于“说”而非真正“看”，自反思机制未能驱动实质性视觉关注。
...
### [117] [Embedding-perturbed Exploration Preference Optimization for Flow Models](https://arxiv.org/abs/2605.15803)
*Sujie Hu,Chubin Chen,Jiashu Zhu,Jiahong Wu,Xiangxiang Chu,Xiu Li*

Main category: cs.CV

TL;DR: 提出嵌入扰动探索偏好优化 (E²PO) 框架，通过嵌入层结构化扰动维持组内方差，解决组优化中信号衰减导致的不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 组优化框架（如GRPO）中样本多样性快速衰减，方差趋近于零，导致训练不稳定、过早停滞或奖励破解。

Method: 在样本组内引入嵌入级结构化扰动，保证鲁棒方差以维持优化信号，通过嵌入扰动探索实现偏好优化。

Result: 在多个基准测试中显著优于现有方法，实现更忠实于人类偏好的对齐。

Conclusion: E²PO 通过维持学习信号有效性解决了组优化的核心限制，提升了生成模型与人类意图对齐的稳定性和效果。
...
### [118] [RaPD: Resolution-Agnostic Pixel Diffusion via Semantics-Enriched Implicit Representations](https://arxiv.org/abs/2605.15908)
*Yanhao Ge,Shanyan Guan,Weihao Wang,Ying Tai,Mingyu You*

Main category: cs.CV

TL;DR: 提出了RaPD模型，通过在连续神经图像场潜在空间中进行扩散，实现了任意分辨率的图像生成，且扩散成本固定。


<details>
  <summary>Details</summary>
Motivation: 自然图像是连续的，但现有生成模型多在离散网格上合成，限制了分辨率灵活性；先前方法仅在解码阶段引入连续性，生成潜在空间仍离散且面向重建。

Method: 采用连续神经图像场潜在空间进行扩散，通过语义表示引导生成感知的潜在学习，以及坐标查询注意力渲染器实现尺度感知渲染。

Result: 单个去噪潜在可通过改变查询坐标在任意分辨率下渲染，扩散成本不变，生成质量和分辨率可扩展性优于现有方法。

Conclusion: RaPD通过连续潜在空间中的扩散，有效实现了分辨率无关的图像生成，弥合了重建与生成之间的差距。
...
### [119] [Decomposed Vision-Language Alignment for Fine-Grained Open-Vocabulary Segmentation](https://arxiv.org/abs/2605.15942)
*Chenhao Wang,Yingrui Ji,Yu Meng,Yao Zhu*

Main category: cs.CV

TL;DR: 提出分解式视觉-语言对齐框架，将文本提示分解为概念和属性令牌，通过特征门控交叉注意力和对数空间相似度聚合，提升开放词汇分割模型对未见组合的泛化能力。
...
Method: 显式将文本提示分解为概念令牌和多个属性令牌，引入特征门控交叉注意力模块生成属性特定门控图以乘法方式融合信息，并在对数空间聚合每个令牌的相似度。

Result: 该方法可无缝集成到现有Transformer分割架构中，在细粒度开放词汇分割基准上显著提升对未见属性-类别组合的泛化性能。

Conclusion: 通过显式分解语义单元和独立跨模态交互，有效增强模型对组合语义的建模能力，实现稳定的组合匹配。
...
### [120] [Robust Prior-Guided Segmentation for Editable 3D Gaussian Splatting](https://arxiv.org/abs/2605.16065)
*Raushan Joshi,Jean-Yves Guillemaut*

Main category: cs.CV

TL;DR: 提出基于SAM-HQ的3D高斯溅射分割框架，通过先验引导标签重分配实现实时高保真3D场景编辑。


<details>
  <summary>Details</summary>
Motivation: 3D-GS缺乏鲁棒分割能力，现有2D升3D方法存在视图不一致和粗掩码问题。

Method: 使用SAM-HQ生成精确2D掩码，结合先验引导标签重分配实现多视图一致的3D高斯语义分割。

Result: 分割精度达到SOTA，支持实时交互式物体编辑且保持高视觉保真度。

Conclusion: 该方法在VR和机器人领域具有实用价值，推进了3D场景编辑发展。
...
### [121] [FashionChameleon: Towards Real-Time and Interactive Human-Garment Video Customization](https://arxiv.org/abs/2605.15824)
*Quanjian Song,Yefeng Shen,Mengting Chen,Hao Sun,Jinsong Lan,Xiaoyong Zhu,Bo Zheng,Liujuan Cao*

Main category: cs.CV

TL;DR: FashionChameleon 是一个实时交互式的多服装视频定制框架，通过上下文学习和流式蒸馏技术实现单服装数据下的多服装切换与运动连贯生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法支持低延迟、交互式的服装控制，限制了在电商和内容创作等场景的应用。

Method: 包括三项关键：1) 通过上下文学习训练教师模型，利用单参考服装对强迫模型隐式保持切换连贯；2) 流式蒸馏结合梯度重加权分布匹配，提升生成一致性与效率；3) 训练无关的 KV 缓存重调度，实现服装切换并保持运动连贯。

Result: 在单 GPU 上实现 23.8 FPS 的实时生成，速度比现有基线快 30-180 倍。

Conclusion: FashionChameleon 首次支持交互式多服装定制和一致的长视频外推，具有显著的实际应用潜力。
...
### [122] [Not All Tasks Quantize Equally: Fisher-Guided Quantization for Visual Geometry Transformer](https://arxiv.org/abs/2605.15828)
*Yipu Zhang,Jintao Cheng,Weilun Feng,Jiehao Luo,Chuanguang Yang,Zhulin An,Yongjun Xu,Wei Zhang*

Main category: cs.CV

TL;DR: 提出了一种基于Fisher信息的后训练量化方法FGQ，用于前馈3D重建模型，以解决不同任务、块和通道对量化误差敏感度不同的问题，在4位量化下相对提升高达39%。


<details>
  <summary>Details</summary>
Motivation: 现有的后训练量化方法忽略了前馈3D模型中不同任务、变换器块和隐藏通道对量化误差的不同敏感度，导致对敏感任务造成显著精度损失。

Method: 利用对角Fisher信息矩阵量化跨任务、块和通道的敏感度，并将这些敏感度纳入可学习仿射变换（LAT）的校准过程，以保留对每个任务最关键的通道和块。

Result: 在VGGT模型的相机姿态估计、点云重建和深度估计任务上，FGQ一致优于最先进的量化基线，在4位量化下相对改进最高达39%。

Conclusion: FGQ通过Fisher引导的量化有效保护了敏感任务的关键特征，提高了前馈3D重建模型的量化性能。
...
### [123] [AgriMind: An Ensemble Deep Learning Framework for Multi-Class Plant Disease Classification](https://arxiv.org/abs/2605.16076)
*Salma Hoque Talukdar Koli,Fahima Haque Talukder Jely*

Main category: cs.CV

TL;DR: 该论文提出了一种基于ResNet50、EfficientNet-B0和DenseNet121集成的植物病害检测系统AgriMind，在15类病害的测试集上达到99.23%准确率，处理速度为53 FPS。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国的植物病害检测仍主要依赖人工，需要自动化解决方案来提高大规模小农场的检测效率。

Method: 使用三种预训练CNN模型（ResNet50、EfficientNet-B0、DenseNet121）的集成，通过冻结ImageNet主干网络、仅训练头部10个epoch，并对softmax输出取平均值。

Result: 单个模型准确率96-97%，集成后达到99.23%；番茄类中视觉相似的十类准确率99.01%；在T4 GPU上处理速度53 FPS。

Conclusion: 集成模型显著降低了错误率，但尚未完成TensorFlow Lite优化以验证实时移动端应用可行性。
...
### [124] [WorldAct: Activating Monolithic 3D Worlds into Interactive-Ready Object-Centric Scenes](https://arxiv.org/abs/2605.15843)
*Jichen Hu,Jiawei Guo,Jiazhong Cen,Chen Yang,Sikuang Li,Wei Shen*

Main category: cs.CV

TL;DR: WorldAct将静态生成的3D世界转化为可编辑和可交互的场景，支持物体级编辑和物理交互。


<details>
  <summary>Details</summary>
Motivation: 现有3D世界生成系统输出静态、单一资产，缺乏可编辑性和物理交互，限制其在沉浸式内容和具身模拟中的应用。

Method: 使用多模态代理引导场景分解、识别可操作物体、重建几何对齐的物体级网格，并通过3D补全恢复背景残差。

Result: 生成的场景支持物体级编辑、碰撞感知操作和具身任务执行，同时保持全局场景连贯性。

Conclusion: WorldAct提供了一种将生成式3D模型转化为可编辑和交互场景的实用路径。
...
### [125] [VideoSeeker: Incentivizing Instance-level Video Understanding via Native Agentic Tool Invocation](https://arxiv.org/abs/2605.16079)
...
TL;DR: 提出VideoSeeker，一种通过视觉提示实现实例级视频理解的新范式，结合智能体推理与主动感知能力，在实例级任务上平均提升13.7%，超越GPT-4o等模型。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM在实例级时空定位任务中精度不足，文本提示难以提供精准时空参考，且模型被动依赖语言推理，缺乏主动感知细粒度视觉证据的能力。

Method: 通过视觉提示（visual prompts）融合智能体推理；构建四阶段全自动数据合成流水线；采用冷启动监督和强化学习（RL）训练内化工具调用与主动感知能力。

Result: 在实例级视频理解任务上平均性能提升+13.7%，超越GPT-4o和Gemini-2.5-Pro，且泛化至通用视频理解基准。

Conclusion: 视觉提示范式可有效增强模型在实例级视频任务中的主动感知与定位能力，且数据合成与训练方法具备可扩展性。
...
### [126] [Community-aware evaluation and threshold calibration for open-set plankton image recognition](https://arxiv.org/abs/2605.15835)
*Xi Chen,Eryuan Huang,Yingjun Xiao,Gang Fang*

Main category: cs.CV

TL;DR: 本研究揭示了开放式浮游生物识别任务中，样本级指标（如AUROC、AUPR）与生态群落级丰度估计之间的失配现象，并提出了群落感知阈值校准方法来优化识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有的开放式识别评估主要依赖样本级指标，而生态监测依赖于群落级的丰度与多样性估计，二者目标不匹配。

Method: 定义了开放式群落失真（OSCD）指标，结合Bray-Curtis风格误差和方向性成分，并在三个浮游生物数据集上比较了MSP与原型距离的闭集和开集识别方法。

Result: 闭集分类器的高置信度预测会导致未知样本系统性地被吸收；样本级OOD指标不足以选择生态操作点；群落感知阈值校准能有效降低MSP的OSCD，优于固定95%已知召回率基线。

Conclusion: 开放式浮游生物识别应作为生态测量问题而非单纯样本检测任务来评估，群落感知校准依赖于验证集代表性与固定召回率与群落最优值之间的差距。
...
### [127] [GenShield: Unified Detection and Artifact Correction for AI-Generated Images](https://arxiv.org/abs/2605.16122)
*Zhipei Xu,Xuanyu Zhang,Youmin Xu,Qing Huang,Shen Chen,Taiping Yao,Shouhong Ding,Jian Zhang*

Main category: cs.CV

TL;DR: 提出GenShield统一框架，联合进行可解释的AIGI检测与可控伪影校正，实现诊断-修复闭环。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的图像越来越逼真，但检测到伪影后如何校正恢复真实性尚未被充分探索，且缺乏检测与校正的关联研究。

Method: 基于自回归框架，引入视觉思维链课程学习策略，实现多步诊断-修复校正，并构建大规模伪影-修复配对数据集。

Result: 在校正基准和主流AIGI检测基准上达到最先进性能，泛化能力强。

Conclusion: GenShield成功建立了AIGI检测与伪影校正的协同关系，为图像真实性恢复提供了有效方案。
...
### [128] [Do Less, Achieve More: Do We Need Every-Step Optimization for RL Fine-tuning of Diffusion Models?](https://arxiv.org/abs/2605.15855)
*Renye Yan,Jikang Cheng,Shikun Sun,Yi Sun,You Wu,Wei Peng,Zongwei Wang,Ling Liang,Junliang Xing,Yimao Cai*

Main category: cs.CV

TL;DR: 提出AdaScope插件，通过自适应识别强化学习干预时机和动态终止训练，在扩散模型微调中兼顾性能提升与计算成本降低。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型强化学习微调在整个去噪轨迹上应用，导致计算成本高、偏好对齐弱（延迟奖励、动作-奖励不匹配、奖励黑客问题）。

Method: AdaScope通过感知去噪过程中的结构演化和语义一致性，自适应确定最优RL干预时机，并在去噪收敛且奖励增益饱和时动态终止训练。

Result: 相比SOTA方法，性能提升66%，计算成本降低59%。

Conclusion: AdaScope实现了减少计算成本与显著提升性能的“双重收益”，其设计具有理论基础支撑。
...
### [129] [Second-Order Multi-Level Variance Correction for Modality Competition in Multimodal Models](https://arxiv.org/abs/2605.16165)
*Yishun Lu,Wes Armour*

Main category: cs.CV

TL;DR: 提出ML-FOP-SOAP优化器，通过二阶预条件和多级方差校正缓解多模态训练中模态竞争，在大规模批次（8192）上提升效率和速度。


<details>
  <summary>Details</summary>
Motivation: 自回归下一词训练虽统一图像生成与文本理解，但模态竞争导致优化不稳定、大规模批次缩放受限。

Method: 基于SOAP的二阶优化框架，引入Fisher正交投影抑制方差引起的模态冲突，并通过分层折叠策略实现低开销的大梯度累积。

Result: 在Janus和Emu3上生成和理解能力一致提升，批大小8192下训练稳定，样本效率提升1.4倍，实际训练速度提升1.5倍。

Conclusion: 该方法为缩放多模态基础模型提供了鲁棒的优化器。
...
### [130] [GHOST: Geometry-Hierarchical Online Streaming Token Eviction for Efficient 3D Reconstruction](https://arxiv.org/abs/2605.15852)
*Leyang Chen,Junyi Wu,Zhiteng Li,Yulun Zhang*

Main category: cs.CV

TL;DR: 提出GHOST框架，用于单目视频流式3D重建中KV缓存的高效管理，通过基于3D几何的令牌驱逐减少内存占用并加速推理。


<details>
  <summary>Details</summary>
Motivation: 长序列视频的流式3D重建中，KV缓存线性增长导致内存瓶颈；现有方法要么截断缓存降低质量，要么依赖与3D结构无关的注意力分数启发式方法。

Method: GHOST包含三个相互强化的创新：层次化双级重要性评分方案、保护特殊令牌不被驱逐的特权机制、以及基于余弦相似度的逐层预算分配。

Result: 在多个基准测试中，GHOST保持优异的重建质量，同时将KV缓存减少近一半，推理速度比现有方法快1.75倍。

Conclusion: GHOST是一种无需训练的KV缓存管理框架，能有效利用3D几何输出在线驱逐冗余令牌，显著提升流式3D重建的效率和质量。
...
### [131] [SOLAR: Self-supervised Joint Learning for Symmetric Multimodal Retrieval](https://arxiv.org/abs/2605.15868)
*Wenjie Yang,Hang Yu,Yuyu Guo,Peng Di*

Main category: cs.CV

TL;DR: 提出SOLAR框架，用于对称多模态到多模态检索，通过自监督联合学习利用未标注的图文对，并在新基准上超越SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 对称MM2MM检索中，现有方法受限于标注不对称数据集，而该任务中查询和上下文可互换，缺乏有效方案。

Method: 两阶段自监督框架：第一阶段学习图文对的交集掩码以对齐交集并保留差异语义；第二阶段利用掩码构建正负样本进行多模态嵌入学习。

Result: 在新基准上，SOLAR以50倍更少参数和5倍更小嵌入维度，超越最强监督VLM 7.08个点。

Conclusion: SOLAR有效解决对称多模态检索难题，且资源效率显著优于现有方法。
...
### [132] [Offline Semantic Guidance for Efficient Vision-Language-Action Policy Distillation](https://arxiv.org/abs/2605.16241)
*Jin Shi,Brady Zhang,Yishun Lu*

Main category: cs.CV

TL;DR: 提出VLA-AD蒸馏框架，利用VLM作为语义监督器，将大模型VLA教师蒸馏为轻量学生策略，在保持性能的同时降低44倍模型大小和3.28倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略模型庞大、推理成本高，难以满足实时闭环控制需求。

Method: 基于VLM的离线语义蒸馏，在教师7-DoF动作目标基础上增加任务阶段锚点和多帧操作方向描述作为辅助信号，仅在训练阶段使用。

Result: 教师OpenVLA-7B蒸馏至158M学生模型（44倍压缩），性能仅差0.27%；推理速度12.5 Hz（3.28倍提升）；不同教师泛化实验中学生性能接近或超越教师；对学生噪声动作鲁棒性增强。
...
### [133] [On RGB-TIR Stereo Calibration under Extreme Resolution Asymmetry](https://arxiv.org/abs/2605.15860)
*Michał Król,Michał Salamonowicz,Władysław Skarbek,Michał Tomaszewski*

Main category: cs.CV

TL;DR: 本文提出一种RGB与超低分辨率热红外相机（80x62像素）的立体标定框架，通过主动OLED屏幕显示双模态图案实现低分辨率热像仪的高精度几何标定。


<details>
  <summary>Details</summary>
Motivation: 低分辨率热红外传感器在建筑热成像分析中精度差，现有标定方法难以应对RGB与TIR像素数差625倍的挑战。

Method: 使用主动OLED屏幕交替显示棋盘格与ChArUco图案；结合透视矫正、Hessian鞍点分析和Mean Shift定位的角点检测；基线约束光束法平差。

Result: 标定基线为32.7mm（标称30mm），重投影误差0.382像素，在建筑模型上验证了TIR到RGB投影的一致性。

Conclusion: 提出的框架能有效标定极低分辨率热像仪与高清RGB相机，适用于建筑能效评估的多模态分析。
...
### [134] [AdaEraser: Training-Free Object Removal via Adaptive Attention Suppression](https://arxiv.org/abs/2605.15921)
*Dingming Liu*

Main category: cs.CV

TL;DR: 提出AdaEraser，通过自适应注意力抑制实现无需训练的对象移除，性能超越训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有免训练方法在对象区域无差别抑制自注意力，导致背景重建质量下降。

Method: 基于去噪过程中自注意力图演化分析，提出token级自适应注意力抑制策略。

Result: 在对象移除任务上表现优异，甚至优于基于训练的方法。

Conclusion: 自适应调整注意力抑制强度能有效解决重建与移除的冲突。
...
### [135] [Unlocking Dense Metric Depth Estimation in VLMs](https://arxiv.org/abs/2605.15876)
*Hanxun Yu,Xuan Qu,Yuxin Wang,Jianke Zhu,Lei ke*

Main category: cs.CV

TL;DR: 提出DepthVLM框架，将视觉语言模型转化为原生稠密几何预测器，同时保持多模态能力，实现与语言输出并行的全分辨率深度图生成。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在3D理解上受限，文本监督范式无法约束细粒度视觉感知，前期方法存在误差累积或效率低下问题。

Method: 在LLM骨干上附加轻量级深度头，采用两阶段训练的统一视觉-文本监督范式，通过单次前向传播生成深度图。

Result: 显著优于现有视觉语言模型，推理效率更高，超越纯视觉模型，并提升复杂3D空间推理能力。

Conclusion: DepthVLM向真正统一的基础模型迈进，所有代码和权重将公开。
...
### [136] [From Failure to Feedback: Group Revision Unlocks Hard Cases in Object-Level Grounding](https://arxiv.org/abs/2605.15951)
*Yuyuan Liu,Yiping Ji,Anjie Le,Jiayuan Zhu,Jiazhen Pan,Can Peng,Jiajun Deng,Fengbei Liu,Junde Wu*

Main category: cs.CV

TL;DR: 提出一种基于组修订优化的强化学习方法，通过奖励塑形增强视觉语言模型的目标定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法的稀疏奖励在困难样本上学习信号不足。

Method: 组修订优化，初始采样后生成修订候选，通过奖励塑形量化改进并调节优势值。

Result: 在指代分割、REC和计数基准上持续优于GRPO基线。

Conclusion: 本方法通过强化学习有效提升了视觉语言模型的目标定位性能。
...
### [137] [Invaria: Learning Scale and Density Invariance in Point Clouds via Next-Resolution Prediction](https://arxiv.org/abs/2605.15923)
*Chun-Peng Chang,Shaoxiang Wang,Alain Pagani,Dariu Gavrila,Holger Caesar*

Main category: cs.CV

TL;DR: 提出了一种名为Invaria的点云编码器，通过下一分辨率预测和感受野校准实现尺度和密度不变性，在分辨率变化和物体缩放时性能大幅提升，同时模型更小、输入token更少。


<details>
  <summary>Details</summary>
Motivation: 现有3D点云编码器对采样分辨率和尺度变化高度敏感，导致性能下降，阻碍了在机器人领域的实际部署。

Method: 通过下一分辨率预测和感受野校准实现点云编码器的尺度和密度不变性。

Result: 在ScanNet上，分辨率降低3倍时mIoU提高56.0%，物体尺度缩小3倍时提升20%，模型大小减小45%，输入token平均减少40%。
...
### [138] [Sparse Autoencoders enable Robust and Interpretable Fine-tuning of CLIP models](https://arxiv.org/abs/2605.15961)
*Fabian Morelli,Arnas Uselis,Ankit Sonthalia,Seong Joon Oh*

Main category: cs.CV

TL;DR: 提出SAE-FT方法，通过稀疏自编码器约束视觉表示变化，实现鲁棒微调，性能优于或持平现有方法。


<details>
  <summary>Details</summary>
Motivation: 微调视觉语言模型可提升下游性能，但常导致对分布偏移的鲁棒性下降，现有方法依赖昂贵的文本引导。

Method: 仅对预训练模型的视觉表示进行稀疏自编码器训练，惩罚语义特征被添加或删除，以正则化表示变化。

Result: 在ImageNet及其分布偏移基准上达到或超越现有最优性能，且计算高效。

Conclusion: SAE-FT提供了机制透明、高效鲁棒的微调方案，能防止灾难性遗忘并支持语义变化分析。
...
### [139] [Flash-GRPO: Efficient Alignment for Video Diffusion via One-Step Policy Optimization](https://arxiv.org/abs/2605.15980)
...
TL;DR: Flash-GRPO是一个单步训练框架，用于视频扩散模型的人类偏好对齐，显著提升训练效率并达到最先进的对齐质量。


<details>
  <summary>Details</summary>
Motivation: GRPO训练大模型（如14B参数）需要大量GPU时间，现有效率方法通过滑动窗口采样降低成本但导致优化不稳定和性能不足。

Method: 提出iso-temporal分组消除时间步的混淆方差，以及时间梯度校正解决梯度量级不一致问题。

Result: 在1.3B至14B参数模型上验证，Flash-GRPO实现显著训练加速、稳定性和最先进的对齐质量。

Conclusion: Flash-GRPO在低计算预算下优于完整轨迹训练，是高效且稳定的视频扩散模型对齐方法。
...
### [140] [A Causally Grounded Taxonomy for Image Degradation Robustness Evaluation](https://arxiv.org/abs/2605.15906)
*Stefan Becker,Simon Weiss,Wolfgang Hübner,Michael Arens*

Main category: cs.CV

TL;DR: 提出一个基于因果关系的图像退化分类与比较框架，通过双轴抽象（因果来源+感知效应）和轻量级严重度测量层，实现不同退化数据集和任务间的可比较分析。


<details>
  <summary>Details</summary>
Motivation: 现有图像退化研究（合成鲁棒性、质量评估、物理分析）虽针对相似现象，但采用不兼容的分组和严重度定义，导致结果难以跨数据集、来源和任务比较。

Method: 1. 双轴分类轴：退化因果来源（环境/传感器/ISP/传输）和感知效果；2. 轻量级严重度测量层：使用PSNR、SSIM、LPIPS等全参考指标量化退化强度，保持原生参数化不变。

Result: 通过COCO Degradation基准验证框架有效性，该基准涵盖多样化成像条件的分类法，用于评价目标检测器鲁棒性。
...
### [141] [Segmentation, Detection and Explanation: A Unified Framework for CT Appearance Reasoning](https://arxiv.org/abs/2605.15997)
*Yuyuan Liu,Can Peng,Yingyu Yang,Qianye Yang,Cheng Ouyang,J. Alison Noble*

Main category: cs.CV

TL;DR: 提出了一个统一的框架，结合语言引导的视觉推理和多任务令牌，用于CT图像中的解剖检测和分割，并设计了从粗到细的聚焦机制。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法缺乏解剖推理，且大多只针对单一任务，不适用于需要多种分析的临床工作流程。

Method: 构建了一个自回归框架，使用任务路由令牌激活检测和分割头部，并通过"closer-look"机制实现从粗到细的渐进式聚焦。

Result: 在BTCV和MosMed+数据集上Dice分别提升1.0%和1.7%，并提供了外观推理输出。

Conclusion: 验证了语言引导的多任务框架能有效提升CT分析效果，同时提供推理能力。
...
### [142] [End-to-end plaque counting and virus titration from laboratory plate images with deep learning](https://arxiv.org/abs/2605.16008)
...
TL;DR: 提出基于SAM的端到端工作流，自动化病毒噬斑计数，在两种病毒和两种孔板格式上实现高准确度。


<details>
  <summary>Details</summary>
Motivation: 传统噬斑检测依赖人工计数，费时费力且存在操作者间差异。

Method: 结合SAM2的孔分割模块和SAM的噬斑分割模型，输出每孔噬斑数并自动计算PFU/mL，集成于Web平台。

Result: 在MAYV/CVB3和VACV数据集上，Pearson相关系数分别为0.92和0.88，与专家标注高度一致。

Conclusion: 该开源工作流可减少人工工作量，提供可重复、可扩展的噬斑检测分析。
...
### [143] [Echo-Forcing: A Scene Memory Framework for Interactive Long Video Generation](https://arxiv.org/abs/2605.16003)
...
TL;DR: 提出Echo-Forcing，一种无需训练的场景记忆框架，用于交互式长视频生成，支持平滑过渡、硬切换和长期场景回忆。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费长视频优化方法主要关注单提示下的稳定扩展，难以处理交互场景中的提示切换、旧场景遗忘和历史场景回忆。

Method: 提出Echo-Forcing框架，包含：分层时间记忆（解耦稳定锚点、压缩历史和近期窗口）、场景回忆帧（压缩历史场景为结构化的KV表示）、差异感知内存衰减（根据新旧场景差异自适应遗忘冲突标记）。

Result: 在VBench-Long上，Echo-Forcing在长视频生成和交互视频生成设置中均取得最佳总体性能。

Conclusion: Echo-Forcing在有限缓存预算下统一支持平滑过渡、硬切换和长程场景回忆，有效解决了交互式长视频生成的瓶颈。
...
### [144] [EndoGSim: Physics-Aware 4D Dynamic Endoscopic Scene Simulations via MLLM-Guided Gaussian Splatting](https://arxiv.org/abs/2605.16022)
*Changjing Liu,Yiming Huang,Long Bai,Beilei Cui,Hongliang Ren*

Main category: cs.CV

TL;DR: 提出MLLM引导的4D高斯溅射框架，实现可变形组织与手术工具的物理感知重建及物理仿真，验证表明仿真保真度与物理精度优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有内窥镜场景重建方法缺乏物理描述，无法支撑高保真物理仿真，需引入物理感知能力。

Method: 4D高斯溅射结合预训练分割与深度估计；通过MLLM初始化物体材料场，利用可微物质点法联合图像与光流监督优化参数。

Result: 在公开与内部数据集上，所提方法的仿真保真度和物理准确性均优于现有先进技术。

Conclusion: 该框架可提升机器人辅助手术场景的物理仿真能力，具有推动手术应用发展的潜力。
...
### [145] [ReAlign: Generalizable Image Forgery Detection via Reasoning-Aligned Representation](https://arxiv.org/abs/2605.16080)
*Qing Huang,Zhipei Xu,Xuanyu Zhang,Xiangyu Yu,Jian Zhang*

Main category: cs.CV

TL;DR: 提出ReAlign框架，通过GRPO优化LLM生成推理文本并蒸馏至轻量级检测器，利用对比学习提升AIGI检测的泛化性和语义敏感性。


<details>
  <summary>Details</summary>
Motivation: 现有AIGI检测方法中，非LLM方法缺乏语义理解，LLM方法计算量大且对细微伪影不敏感，且推理文本的真实贡献不明确。

Method: 使用GRPO优化LLM生成高质量推理文本，通过对比学习将其蒸馏至轻量级AIGI检测器，并采用联合优化策略（对比损失+分类损失）。

Result: 在AIGCDetectBenchmark、AIGI-Holmes和UltraSynth-10k上，ReAlign在准确率和泛化性上优于现有方法，尤其对高保真伪造图像。

Conclusion: ReAlign高效继承了LLM的泛化与语义敏感性，同时保持轻量化部署，显著提升AIGI检测性能。
...
### [146] [WeatherOcc3D: VLM-Assisted Adverse Weather Aware 3D Semantic Occupancy Prediction](https://arxiv.org/abs/2605.16127)
*A. Enes Doruk,Abdelaziz Hussein,Hasan F. Ates*

Main category: cs.CV

TL;DR: 提出VLM辅助框架，利用CLIP潜在空间通过语言环境线索引导多传感器融合，动态调整相机和LiDAR的融合权重，在恶劣天气下提升3D语义占用预测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态3D语义占用预测受环境变化影响，静态融合策略无法适应传感器在低光或强降水下的不可靠问题。

Method: 通过参数高效适配器对齐天气特定文本嵌入与传感器特征，结合门控机制分解环境不确定性为可见度和光照因子，动态调制融合比例。

Result: 在nuScenes数据集上，OccMamba和M-CONet架构分别实现26.3和21.1的mIoU，显著优于传统基线。

Conclusion: 该方法能有效解决模态信任问题，提升恶劣环境下的预测鲁棒性。
...
### [147] [MAgSeg: Segmentation of Agricultural Landscapes in High-Resolution Satellite Imagery using Multimodal Large Language Models](https://arxiv.org/abs/2605.16179)
...
TL;DR: 提出MAgSeg，一种无需辅助视觉解码器的多模态大语言模型分割方法，显著提升全球南方小农农业景观分割性能。


<details>
  <summary>Details</summary>
Motivation: 全球南方农业景观分割面临地块碎片化、类内方差高、标注数据稀缺等挑战，现有MLLM方法存在上下文长度瓶颈和领域对齐问题。

Method: MAgSeg采用新颖的指令调优数据格式，使标准MLLM仅通过文本令牌预测即可实现高分辨率卫星图像分割，无需辅助视觉解码器。

Result: 在三个全球南方国家的数据集上，MAgSeg显著优于现有最优MLLM基线方法。

Conclusion: MAgSeg是一种可扩展、高效的解决方案，能够有效地绘制小农农业环境地图。
...
### [148] [Res$^2$CLIP: Few-Shot Generalist Anomaly Detection with Residual-to-Residual Alignment](https://arxiv.org/abs/2605.16171)
*Xinyue Liu,Jianyuan Wang,Biao Leng,Shuo Zhang*

Main category: cs.CV

TL;DR: 本文提出Res$^2$CLIP框架，通过在残差空间中进行多模态对齐，解决了少样本通用异常检测中的跨粒度匹配和跨类别泛化退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP方法在少样本通用异常检测中面临粗粒度文本提示与细粒度前景背景差异不匹配，以及微调导致域偏移的问题，需要新方法提升泛化能力。

Method: 设计残差到残差对齐框架，包括文本提示、视觉提示和残差对齐三个分支，所有优化限制在残差域内，并设计残差对齐目标函数，迫使模型关注相对异常偏差。

Result: 在多个数据集上的实验验证了架构的有效性。

Conclusion: Res$^2$CLIP通过统一残差空间对齐有效解决了跨粒度与跨类别问题，提升了少样本异常检测的泛化性能。
...
### [149] [Registers Matter for Pixel-Space Diffusion Transformers](https://arxiv.org/abs/2605.16147)
*Nikita Starodubcev,Ilia Sudakov,Ilya Drobyshevskiy,Artem Babenko,Dmitry Baranchuk*

Main category: cs.CV

TL;DR: 本文发现扩散变压器（DiTs）不表现高范数异常标记，但注册标记仍能提升像素空间DiTs的收敛与生成质量，通过产生更干净的特征图实现。


<details>
  <summary>Details</summary>
Motivation: 视觉变压器（ViTs）中高范数异常标记损害特征图质量，注册标记可缓解；扩散模型采用变压器架构并转向像素空间训练，需探究注册标记对DiTs是否有效。

Method: 分析DiTs与ViTs的差异，研究注册标记对像素空间DiTs的影响，并设计参数高效的双流架构处理注册标记。

Result: DiTs无高范数异常标记，但注册标记显著改善收敛与生成质量；高噪声水平下产生更干净的特征图；近期架构隐含类似注册机制。

Conclusion: 注册标记对像素空间DiTs有效，双流架构可提升质量且运行时开销极小。
...
### [150] [Always Learning, Always Mixing: Efficient and Simple Data Mixing All The Time](https://arxiv.org/abs/2605.15220)
*Michael Y. Hu,Apurva Gandhi,Kyunghyun Cho,Tal Linzen,Pratyusha Sharma*

Main category: cs.CL

TL;DR: 提出一种统一的在线数据混合算法OP-Mix，通过低秩适配器插值模拟候选数据混合，覆盖预训练、持续中训练和持续指令调优全生命周期。


<details>
  <summary>Details</summary>
Motivation: 现有数据混合方法分别针对训练的不同阶段，缺乏统一解决方案，且未满足持续学习的规范需求。

Method: 基于当前模型的低秩适配器（LoRA）插值模拟数据混合效果，避免使用独立代理模型，实现与模型实际学习动态一致的高效搜索。

Result: 在预训练中困惑度平均提升6.3%；在持续学习中匹配重训练和在线蒸馏性能，计算量分别减少66%和95%。

Conclusion: 语言模型训练应视为统一的持续学习过程，OP-Mix为此提供了高效统一的数据混合方案。
...
### [151] [Fluency and Faithfulness in Human and Machine Literary Translation](https://arxiv.org/abs/2605.15282)
*Sarah Griebel,Ted Underwood*

Main category: cs.CL

TL;DR: 在文学翻译中，流畅性和忠实性之间呈负相关，且该现象对人工和谷歌翻译普遍存在，但对TranslateGemma较弱。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文学翻译中常产生流畅译文，但流畅是否对应语义保真尚不明确。

Method: 使用130,486段来自16种源语言106部小说的翻译段落，包括人工、谷歌翻译和TranslateGemma；通过翻译ese分类器测量流畅性，使用COMET-KIWI评估忠实性，并控制段落长度。

Result: 流畅性与忠实性呈一致负相关，该模式对人工和谷歌翻译显著，但对TranslateGemma较弱且常不显著。
...
### [152] [DiscoExplorer: An Open Interface for the Study of Multilingual Discourse Relations](https://arxiv.org/abs/2605.15304)
*Amir Zeldes*

Main category: cs.CL

TL;DR: 提出了一个开源Web界面DiscoExplorer，用于分析和可视化跨语言的篇章关系数据，覆盖16种语言。


<details>
  <summary>Details</summary>
Motivation: 跨语言的篇章关系研究因数据复杂性和缺乏易用接口而困难，需标准化工具促进比较分析。

Method: 开发了DiscoExplorer，支持关系查询、信号词（如连接词）搜索与可视化，并整合DISRPT数据集。

Result: 成功使16种语言的篇章关系数据可公开访问，并提供示例分析。

Conclusion: DiscoExplorer为跨语言篇章关系研究提供了便捷工具，有助于推动该领域比较研究。
...
### [153] [Automatic Construction of a Legal Citation Graph from 100 Million Ukrainian Court Decisions: Large-Scale Extraction, Topological Analysis, and Ontology-Driven Clustering](https://arxiv.org/abs/2605.15362)
*Volodymyr Ovcharov*

Main category: cs.CL

TL;DR: 基于1亿篇乌克兰判决构建引文网络，揭示法律领域边界并高精度预测立法重要性。


<details>
  <summary>Details</summary>
Motivation: 探索司法引文结构能否在无监督下编码法律领域边界并预测立法重要性。

Method: 构建大型引文图（5亿边），使用正则表达式提取六类引用，应用Louvain社区检测和时间稳定性分析。

Result: 度分布呈幂律（α=1.57）；社区检测恢复法律领域边界（模块度Q=0.44-0.55）；引文特征预测前1000篇AUC=0.9984。

Conclusion: 引文结构可自动构建法律本体，并作为LLM辅助法律分析的工作流记忆系统领域层，同时开源数据。
...
### [154] [Greedy or not, here I come: Language production under vocabulary constraints in humans and resource-rational models](https://arxiv.org/abs/2605.15365)
*Thomas Hikaru Clark,Sihan Chen,Laura Nicolae*

Main category: cs.CL

TL;DR: 本研究探讨了人类在有限词汇（最少250个高频词）下如何回答问题，并与大语言模型驱动的采样算法比较，发现人类更接近贪婪采样，但更熟练者会展现出回溯和修正的非贪婪行为。


<details>
  <summary>Details</summary>
Motivation: 有限词汇沟通是常见但困难的认知现象，需要优化可理解性同时绕开受限词汇，现有研究对此缺乏深入探索。

Method: 通过实验让人类在可变词汇限制下回答广泛问题，并使用大语言模型和序贯蒙特卡洛推断实现贪婪和全局最优采样的理论比较。

Result: 人类整体类似贪婪采样，但高技能者会回溯修正（非贪婪）；在高压制下人类依赖语义轻词，该模式可被两种算法重现。

Conclusion: 结果对资源理性认知、心理语言学、第二语言沟通和语言障碍研究有广泛启示。
...
### [155] [Adesua: Development and Feasibility Study of an AI WhatsApp Bot for Science Learning in West Africa](https://arxiv.org/abs/2605.15376)
*George Boateng,Evans Atompoya,Philemon Badu,Samuel John,Samuel Ansah,Patrick Agyeman-Budu,Victor Wumbor-Apin Kumbol*

Main category: cs.CL

TL;DR: 本文介绍Adesua，一个基于WhatsApp的AI教学助手，为西非初中和高中学生提供科学教育支持，通过生成式AI实现问答和自动评估。


<details>
  <summary>Details</summary>
Motivation: 撒哈拉以南非洲面临高师生比和合格教师短缺，限制学生个性化学习和形成性评估的机会。

Method: 集成教科书和33年国家考试题目，利用生成式AI构建WhatsApp聊天机器人，实现对话式问答、定时或非定时选择题测试，并提供即时评分和解释。

Result: 在加纳为期6个月的可行性部署中，56名活跃用户（学生和家长）使用，AI答案有用性得分93.75%，但评分数量较少（n=16）。

Conclusion: 初步结果支持进一步评估WhatsApp AI助手在资源受限教育环境中提供可扩展、低成本个性化学习支持的潜力。
...
### [156] [Eskwai for Students: Generative AI Assistant for Legal Education in Ghana](https://arxiv.org/abs/2605.15380)
...
TL;DR: 开发了用于加纳法律教育的生成式AI助手Eskwai for Students，基于RAG系统，通过长期部署和用户查询分析探讨其在全球南方的应用与伦理问题。


<details>
  <summary>Details</summary>
Motivation: 全球南方法律教育中生成式AI的应用有限，希望通过该系统支持法律学生的学习。

Method: 开发了基于检索增强生成（RAG）的AI助手，数据库包含1.2万以上判例和1400部加纳法规，并通过30个月纵向研究收集3.1K学生的32K次查询。

Result: 系统为学生提供了法律问题回答，长期部署显示出实用性，但查询内容引发了伦理关注。

Conclusion: 研究揭示了全球南方法律学生如何使用生成式AI，并强调了负责任部署以促进法律教育的途径。
...
### [157] [Capability Conditioned Scaffolding for Professional Human LLM Collaboration](https://arxiv.org/abs/2605.15404)
*Sen Yang,Yinglei Ma*

Main category: cs.CL

TL;DR: 提出能力条件脚手架框架，根据用户在不同领域的能力强弱分区，实现超越风格个性化的可靠人机协作。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型个性化未考虑用户评估能力差异，导致领域漂移问题。

Method: 提出能力条件脚手架，将用户专业领域分为强、混合、弱三类，基于能力画像调节干预行为。

Result: 在多个MMLU子集和四种LLM基底上的试点评估显示，画像调节的干预行为一致，包括类别反转和混合域风险区选择性激活。

Conclusion: 能力感知脚手架可支持更可靠的职业人机协作，超越传统风格个性化。
...
### [158] [Neural Activation Patterns Across Language Model Architectures: A Comprehensive Analysis of Cognitive Task Performance](https://arxiv.org/abs/2605.15436)
*Mahdi Naser-Moghadasi,Faezeh Ghaderi*

Main category: cs.CL

TL;DR: 该论文对六种不同的大型语言模型架构在十二个认知任务类别上的神经激活模式进行了综合分析，揭示了编码器和解码器架构处理不同认知任务的根本差异。


<details>
  <summary>Details</summary>
Motivation: 探索不同LLM架构在认知任务中的神经行为差异，为模型选择和优化提供指导。

Method: 系统测量最终激活值、注意力熵和稀疏性模式，分析了144个任务-模型组合。

Result: 数学推理在所有架构中产生最高的注意力熵；解码器模型相比编码器模型表现出明显更高的稀疏性模式。

Conclusion: 研究成果为现代语言模型的计算特性和任务特定神经行为提供了关键见解，对大数据应用中的模型选择和优化具有重要意义。
...
### [159] [Why are language models less surprised than humans? Testing the Parse Multiplicity Mismatch Hypothesis](https://arxiv.org/abs/2605.15440)
*William Timkey,Brian Dillon,Tal Linzen*

Main category: cs.CL

TL;DR: 研究表明，语言模型的惊异度预测虽能解释自然文本阅读时间，但系统性地低估了句法歧义（如花园路径句）中的处理困难程度。通过减少同步分析数量，惊异度预测的困难效应有所增加，但仍不足以完全匹配人类数据。


<details>
  <summary>Details</summary>
Motivation: 语言模型惊异度未能准确预测人类在句法歧义中的处理难度，可能由于人类与模型在计算约束上的差异，特别是同时考虑句子解释的能力不同。

Method: 使用递归神经网络文法（RNNG）结合词同步束搜索，系统改变同步分析的数量以计算词级惊异度，并将其用于预测人类阅读时间。

Result: 减少同步分析数量增强了花园路径效应的预测幅度，但远未达到人类效应的全量级。

Conclusion: 语言模型与人类在同时分析数量上的差异无法解释基于惊异度预测与人类句子处理之间的差距。
...
### [160] [Reasoning Models Don't Just Think Longer, They Move Differently](https://arxiv.org/abs/2605.15454)
*Anders Gjølbye,Lars Kai Hansen,Sanmi Koyejo*

Main category: cs.CL

TL;DR: 通过长度校正后的隐状态轨迹分析表明，推理训练模型在代码领域表现出更直接的轨迹和更均匀的曲率，而数学和布尔可满足性问题中关联较弱，提示长度校正是轨迹分析的必要前提。


<details>
  <summary>Details</summary>
Motivation: 现有研究未区分模型在困难问题上计算步数增加与内部轨迹改变的区别，需校正长度偏差以揭示推理训练的真实影响。

Method: 对链式思考生成过程中的隐状态轨迹进行几何分析，通过残差化消除长度影响，并在竞争编程、数学和布尔可满足性三个领域比较推理训练与指令微调基线模型。

Result: 代码领域校正后轨迹更直接且曲率更均匀，数学和布尔可满足性领域耦合较弱；线性探测未复现代码领域的分离；行为注释显示校正耦合增强伴随策略转换与不确定性监控。

Conclusion: 长度校正是生成时轨迹分析的必要前提，推理训练可影响校正轨迹几何，但效果因领域而异。
...
### [161] [Retrieval-Augmented Large Language Models for Schema-Constrained Clinical Information Extraction](https://arxiv.org/abs/2605.15467)
*A H M Rezaul Karim,Ozlem Uzuner*

Main category: cs.CL

TL;DR: 本文提出模块化RAG流水线，将护士-患者对话转录文本转化为结构化表示，在MEDIQA-SYNUR任务上以GPT-5.2结合完整模式、RAG和二次审核获得80.36% F1分数。


<details>
  <summary>Details</summary>
Motivation: 临床文档负担沉重，转录文本虽含可操作观察结果，但大规模结构化转换困难，影响直接患者护理时间。

Method: 模块化检索增强生成（RAG）流水线，利用训练集作为示例语料，结合模式约束提示、确定性模式后处理和二次审核审计，采用Llama-4和GPT-5.2两种LLM骨干。

Result: 最佳配置（GPT-5.2+完整模式+RAG+二次审核）达80.36% F1分数；RAG持续提升性能，模式约束最优程度依赖模型，二次审核带来边际改善。

Conclusion: RAG稳定增强结构化转换效果，二次审核可修正模式遵守错误，但提升有限；不同模型对模式约束敏感度不同。
...
### [162] [FINESSE-Bench: A Hierarchical Benchmark Suite for Financial Domain Knowledge and Technical Analysis in Large Language Models](https://arxiv.org/abs/2605.15482)
...
TL;DR: 提出了FINESSE-Bench基准套件，包含8个专业任务和3993个问题，用于分层评估大语言模型在金融领域的专业能力，覆盖从基础到专家级推理。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准如FinQA、FinanceBench等主要关注报告问答，缺乏专业难度分层评估，无法衡量从基础知识到专家级金融推理的能力。

Method: 设计并构建了FINESSE-Bench，包含基于专业认证（CFA、CMT、CFTe）的考试型数据集、应用交易任务和俄语奥林匹克基准，并采用统一评估协议（选择题、数值题、短开放题）及基于LLM作为评判的自动评分方案。

Result: 创建了涵盖8个专门基准、共3993个问题的评估套件，能够评估领域广度、难度增加时的性能下降、计算任务解决能力以及专业金融领域模型行为。

Conclusion: FINESSE-Bench作为现有开放金融基准的补充，可用于更实质性地评估大语言模型在金融领域的专业能力。
...
### [163] [RoPE Distinguishes Neither Positions Nor Tokens in Long Contexts, Provably](https://arxiv.org/abs/2605.15514)
*Yufeng Du,Phillip Harris,Minyang Tian,Eliu A Huerta,Srikanth Ronanki,Subendhu Rongali,Aram Galstyan,Hao Peng*

Main category: cs.CL

TL;DR: 本文揭示了旋转位置编码（RoPE）在长上下文Transformer模型中的固有限制，并证明随着上下文长度增加，其注意力机制会失去局部偏向和令牌相关性一致性。


<details>
  <summary>Details</summary>
Motivation: 当前长上下文语言模型广泛使用RoPE，但其在长距离依赖下的局限性尚不明确，需要理论分析其根本缺陷。

Method: 通过理论分析抽象上下文内容仅基于长度的注意力行为，并辅以多头部多层架构的实证验证。

Result: RoPE在长上下文中：1）注意力分布变为随机（失败概率接近0.5）；2）无法区分令牌位置和令牌身份；3）调整基底参数时在位置区分与令牌区分间存在权衡。

Conclusion: 现有RoPE机制无法满足长上下文需求，未来需设计全新的位置编码机制。
...
### [164] [DetectRL-X: Towards Reliable Multilingual and Real-World LLM-Generated Text Detection](https://arxiv.org/abs/2605.15518)
*Junchao Wu,Yefeng Liu,Chenyu Zhu,Hao Zhang,Zeyu Wu,Tianqi Shi,Yichao Du,Longyue Wang,Weihua Luo,Jinsong Su,Derek F. Wong*

Main category: cs.CL

TL;DR: 提出了一个全面的多语言基准DetectRL-X，用于评估大语言模型生成文本检测器在8种语言和6个易被滥用领域的性能，并通过AI辅助写作操作和攻击模拟进行压力测试。


<details>
  <summary>Details</summary>
Motivation: 现有检测器在多语言真实场景中的可靠性和潜力尚未被充分探索。

Method: 构建包含8种商业语言和6个领域的基准，使用4种商业LLM生成文本，并引入改写、扩展、压缩等AI辅助操作及多语言改写和扰动攻击。

Result: 揭示了现有检测器在不同语言和场景下的优势与局限，并分析了领域、生成器、攻击策略等因素对性能的影响。
...
### [165] [Process Rewards with Learned Reliability](https://arxiv.org/abs/2605.15529)
*Jinyuan Li,Langlin Huang,Chengsong Huang,Shaoyang Xu,Donghong Cai,Yuyi Yang,Wenxuan Zhang,Jiaxin Huang*

Main category: cs.CL

TL;DR: 提出BetaPRM，一种分布过程奖励模型，同时预测步骤级成功概率及其可靠性，并基于此实现自适应计算分配，在减少token使用的同时提高推理准确率。


<details>
  <summary>Details</summary>
Motivation: 当前过程奖励模型仅输出单一奖励分数，下游方法无法判断预测的可靠性，导致不可靠信号被误用。

Method: 利用蒙特卡洛延续的步骤成功监督，通过Beta-Binomial似然学习Beta信念分布，输出成功概率和可靠性信号；基于可靠性信号设计自适应计算分配策略。

Result: 在四个主干模型和四个推理基准上，BetaPRM改进了Best-of-N选择，自适应计算分配相比固定预算Best-of-16减少最多33.57%的token使用并提高准确率。

Conclusion: 预测可靠性信号能有效区分可靠与不确定的步骤奖励，提升计算效率与推理性能。
...
### [166] [When Latent Geometry Is Not Enough: Draft-Conditioned Latent Refinement for Non-Autoregressive Text Generation](https://arxiv.org/abs/2605.15557)
*De Shuai Zhang*

Main category: cs.CL

TL;DR: 本文研究了基于连续扩散和流模型的非自回归文本生成，发现潜变量几何特性无法保证解码质量，提出将任务定义为受控局部细化而非从噪声完全生成，并通过实验表明解码器可恢复性是关键评估指标。


<details>
  <summary>Details</summary>
Motivation: 连续扩散和流模型在文本生成中面临连续潜变量与离散词元之间的接口难题，且现有潜空间度量（如尺度匹配、余弦相似度）无法确保高质量解码。

Method: 使用冻结BERT编码器、并行解码器、去噪DraftPrior、局部FlowNet和学习的对角MetricNet构建草稿条件潜变量细化模型。

Result: 在ROCStories中，768维BERT潜变量恢复词元效果优于256维；DraftPrior在干净草稿下目标词元概率为0.938，词元丢弃率3%时为0.613，5%时为0.483，10%时为0.272；局部流细化和解码器感知读出仅带来适度增益。

Conclusion: 潜变量几何特性不足以保证解码质量，连续潜文本生成应通过解码器可恢复性、起始分布质量及细化是否保留解码器可读结构来评估。
...
### [167] [GiLT: Augmenting Transformer Language Models with Dependency Graphs](https://arxiv.org/abs/2605.15562)
*Tianyu Huang,Yida Zhao,Chuyan Zhou,Kewei Tu*

Main category: cs.CL

TL;DR: 本文提出GiLT模型，通过依赖图调制注意力权重来注入句法信息，无需额外结构标记，在语言建模中实现了更好的句法泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要利用句法树结构（如成分树）增强Transformer，但依赖图结构在语言建模中的潜力未被充分探索。

Method: GiLT在Transformer中不插入结构标记，而是通过逐步构建的依赖图提取特征，调制注意力权重以注入结构信息。

Result: GiLT在语义依赖图上相比基线Transformer模型获得更好的句法泛化能力，且困惑度相当；同时可通过微调提升下游任务性能。

Conclusion: 依赖图结构能有效增强Transformer语言模型的句法泛化，且GiLT具备灵活性和可迁移性。
...
### [168] [Response-Conditioned Parallel-to-Sequential Orchestration for Multi-Agent Systems](https://arxiv.org/abs/2605.15573)
*Nurbek Tastan,Alex Iacob,Lorenzo Sani,Meghdad Kurmanji,Nicholas D. Lane,Samuel Horvath,Karthik Nandakumar*

Main category: cs.CL

TL;DR: 提出一种混合多智能体协作范式Nexa，通过可训练策略在并行与顺序执行间自适应切换，减少通信与延迟并提高准确率。


<details>
  <summary>Details</summary>
Motivation: 现有并行或顺序协作模式无法同时最小化通信、延迟并最大化最终响应准确性。

Method: 采用并行阶段嵌入响应到共享语义空间，预测稀疏有向无环通信图；若图非空则执行一轮顺序消息传播；策略为轻量Transformer，基于策略梯度优化。

Result: 学习到的策略可在智能体数量、任务或底层模型变化时复用，展示出泛化能力。

Conclusion: Nexa严格优于纯并行执行，且无需外部评判或手动拓扑搜索。
...
### [169] [Measuring Maximum Activations in Open Large Language Models](https://arxiv.org/abs/2605.15572)
...
TL;DR: 本文测量了8个现代开源LLM家族共27个检查点的激活值最大值，发现其跨家族、架构和训练阶段差异巨大，可达四个数量级，且与低比特量化误差相关。


<details>
  <summary>Details</summary>
Motivation: 现有激活量化研究基于2024年前的LLaMA模型，后LLaMA时代开源模型激增，需要重新评估激活动态范围以指导低比特部署。

Method: 使用统一流水线（5000样本多领域语料、家族特定分词器、嵌入/隐藏/注意力/MLP/MoE/SwiGLU/最终归一化层钩子），测量27个检查点全局和逐层最大值。

Result: 全局最大值跨四个数量级（Qwen3.5/MoE为10^2-10^3，Gemma3-27B-it达7*10^5）；跨家族比较无简单规模缩放；MoE峰值比密集模型低14.0-23.4倍；残差流在22/24检查点携带全局最大值；INT-8检查显示最大值与低比特误差共变。

Conclusion: 最大激活幅度是模型特性，取决于家族、架构和训练阶段而非仅参数规模，应在低比特部署前测量并报告。
...
### [170] [A Unified Generative-AI Framework for Smart Energy Infrastructure: Intelligent Gas Distribution, Utility Billing, Carbon Analytics, and Quantum-Inspired Optimisation](https://arxiv.org/abs/2605.16232)
*Pavan Manjunath,Thomas pruefer*

Main category: cs.CL

TL;DR: 论文探讨了智能计量、生成式人工智能与量子启发组合优化三者融合如何重塑能源公用事业在基础设施管理、客户参与和环境责任方面的实践。


<details>
  <summary>Details</summary>
Motivation: 推动智能计量、生成式AI和量子启发优化技术的加速融合，以应对能源行业在物理基础设施、客户互动和环境责任方面的管理挑战。

Method: 结合智能计量数据、生成式人工智能模型和量子启发组合优化算法，提出一种综合性的能源管理框架。

Result: 该融合方法有望提升能源效率、优化资源配置、增强客户体验并降低环境影响。

Conclusion: 智能计量、生成式AI和量子启发优化的协同作用能有效支持能源公用事业的数字化转型与可持续发展目标。
...
### [171] [Calibrating LLMs with Semantic-level Reward](https://arxiv.org/abs/2605.15588)
*Fengfei Yu,Ruijia Niu,Dongxia Wu,Yian Ma,Rose Yu*

Main category: cs.CL

TL;DR: 提出CSR框架，在语义空间直接校准大语言模型，无需语言化置信度，显著提升校准性能。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习使用二值正确性奖励，忽视置信度，导致校准差；现有语言化置信度方法存在文本变体不一致问题。

Method: CSR结合正确性奖励和语义校准奖励：对正确输出促进语义一致（利用），对错误输出抑制虚假一致性（探索）。

Result: 在HotpotQA（分布内）和TriviaQA、MSMARCO、NQ-Open（分布外）上，CSR相对基线降低ECE最多40%，提升AUROC最多31%，校准行为鲁棒泛化。

Conclusion: CSR在语义空间直接校准，避免语言化置信度的不一致，有效提升校准性能并泛化到分布外场景。
...
### [172] [MHGraphBench: Knowledge Graph-Grounded Benchmarking of Mental Health Knowledge in Large Language Models](https://arxiv.org/abs/2605.15589)
*Weixin Liu,Congning Ni,Shelagh A. Mulvaney,Susannah L. Rose,Murat Kantarcioglu,Bradley A. Malin,Zhijun Yin*

Main category: cs.CL

TL;DR: 提出了基于知识图谱的MHGraphBench基准，用于评估大语言模型在心理健康实体识别、关系判断和两跳推理上的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在心理健康领域应用日益广泛，但尚未明确它们能否准确捕捉相关生物医学知识并可靠地应用于临床结构化判断。

Method: 基于PrimeKG知识图谱构建基准，包含九个任务族，涵盖实体识别、关系判断和两跳推理，并设计了受控的负选项。

Result: 在15个开源和闭源大语言模型上的实验显示，存在识别-判断差距：顶尖模型在实体类型分类和小关系类型分类上接近完美，但在关系预测和两跳推理上仍表现困难。

Conclusion: MHGraphBench主要用于评估模型与PrimeKG中心理健康子图在约束多选题设置下的一致率，而非直接评估真实临床安全性。
...
### [173] [PSD: Pushing the Pareto Frontier of Diffusion LLMs via Parallel Speculative Decoding](https://arxiv.org/abs/2605.15609)
*Shengyin Sun,Yiming Li,Renxi Liu,Xinqi Li,Hui-Ling Zhen,Weizhe Lin,Chen Chen,Xianzhi Yu,Mingxuan Yuan,Chen Ma*

Main category: cs.CL

TL;DR: 提出PSD框架，通过自适应解掩码策略和多深度投机草稿，在不训练的情况下加速扩散语言模型推理，实现每前向传播5.5倍的令牌生成。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型推理成本高，现有方法在空间（单步解掩码多个令牌）或时间（折叠多个去噪步骤）上优化受限。

Method: 利用单次前向传播的置信度分数，通过自适应策略选择解掩码位置，构建多深度投机草稿，再通过批处理验证实现层次化接受。

Result: 在三个扩散语言模型上的推理和代码生成任务中，实现每前向传播最多5.5倍令牌加速，质量与贪心解码相当。

Conclusion: PSD在不训练的情况下有效提升扩散语言模型推理效率，在速度和质量间取得良好平衡。
...
### [174] [Syntax Without Semantics: Teaching Large Language Models to Code in an Unseen Language](https://arxiv.org/abs/2605.15607)
*Vinayshekhar Bannihatti Kumar,Disha Makhija,Manoj Ghuhan Arivazhagan,Rashmi Gangadharaiah*

Main category: cs.CL

TL;DR: 大语言模型在未见语言上存在实现保真度差距：掌握语法但无法转化算法理解。


<details>
  <summary>Details</summary>
Motivation: 现有研究多集中于主流语言的代码生成，对模型在预训练语料未覆盖语言上的泛化能力知之甚少。

Method: 构建了全新语言PyLang，在352个问题上测试前沿模型（零样本与微调Qwen3的多版本），并采用多任务学习、偏好调整、代码填充和潜在空间目标等干预。

Result: 微调快速习得语法，但语义能力未迁移；Python始终优于PyLang最高19%；前沿模型80%选择与Python相同算法却无法实现；CKA分析表明内部表示高度相似（>0.97）但输出层分叉。

Conclusion: 存在实现保真度差距：模型具备语言无关的算法理解，但无法用陌生语言表达，需训练方法解耦推理与语言实现。
...
### [175] [Toward LLMs Beyond English-Centric Development](https://arxiv.org/abs/2605.15613)
*Sho Takase,Ukyo Honda*

Main category: cs.CL

TL;DR: 开放权重大语言模型在序列生成中明显偏向英语，持续预训练无法带来成本优势，未来可能需要针对各语言进行专项投入。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型对英语的偏见程度，以及持续预训练在非英语语言适应中的效率问题。

Method: 分析开放权重大语言模型生成的序列，对比持续预训练与从头训练的成本和效果。

Result: 大语言模型生成序列强烈偏向英语；持续预训练在改善目标语言能力上无成本优势。

Conclusion: 未来大语言模型发展应更注重针对不同语言的专项投资，而非仅依赖英语中心资源的扩展。
...
### [176] [Evaluating Chinese Ambiguity Understanding in Large Language Models](https://arxiv.org/abs/2605.15635)
*Junwen Mo,Yuanzhi Lu,Yifang Xue,Ke Xu,Hideki Nakayama*

Main category: cs.CL

TL;DR: 首个基于潜在歧义理论的中文歧义数据集CHA-Gen，共5,712句，涵盖18种结构。评估发现大模型在歧义检测上表现不佳，CoT可提升性能，但存在盲区、误归因和过早解决三类失败模式，且模型偏向主导解释。


<details>
  <summary>Details</summary>
Motivation: 现有中文歧义数据集扩展性差，且研究多聚焦英文，缺乏对中文歧义的系统性探索。

Method: 基于潜在歧义理论，采用半自动流水线构建CHA-Gen数据集；通过直接查询和机器翻译评估LLMs，并利用语义熵量化不确定性。

Result: LLMs在歧义检测上表现有限，CoT能提升性能；Qwen3-32B的CoT推理存在三类失败模式；歧义句的语义熵更高；指令微调导致过度自信，基础模型保留更多语义多样性。

Conclusion: CHA-Gen为中文歧义研究提供可扩展数据集，揭示了LLMs处理中文歧义的关键局限与行为倾向。
...
### [177] [VCG-Bench: Towards A Unified Visual-Centric Benchmark for Structured Generation and Editing](https://arxiv.org/abs/2605.15677)
...
TL;DR: 提出Diagram-as-Code范式，通过mxGraph XML实现精准图表生成与编辑，并构建VCG-Bench基准测试，揭示当前VLM在结构化任务中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在专业可控图表任务中能力不足，像素合成方法编辑性和保真度有限。

Method: 将图表表示为代码（mxGraph XML），定义Vision-to-Code和Code-to-Code任务，设计多维度评估指标。

Result: 当前SOTA VLM在结构保真度和指令遵循性上表现差，暴露其视觉与推理能力局限。

Conclusion: 代码范式显著优于像素方法，为VLM处理结构化图表提供新路径。
...
### [178] [Dynamic Chunking for Diffusion Language Models](https://arxiv.org/abs/2605.15676)
*Yichen Zhu,Xiaoming Shi,Peng Zhao,Weiyu Chen,Debing Zhang,James Kwok*

Main category: cs.CL

TL;DR: DCDM通过内容定义的语义块替换固定位置块，提出一种新的离散扩散语言模型，在参数规模达1.5B时优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 固定位置块的分区方式破坏了序列中已有的语义结构，将语义连贯的token分隔开，且将无关的token组合在一起。

Method: 引入动态分块扩散模型（DCDM），核心是可微分的分块注意力层，将token路由到K个由可学习子空间参数化的簇中，并通过扩散目标端到端塑造，形成块因果注意力掩码，实现基于语义块的自回归因子化。

Result: 在参数规模高达1.5B的下游基准测试中，DCDM在无结构和位置块扩散基线上取得一致改进，优势跨规模稳定且训练早期即可见。

Conclusion: DCDM通过动态语义分块严格泛化了块离散扩散，在多种规模下均表现出更优性能。
...
### [179] [Few-Shot Large Language Models for Actionable Triage Categorization of Online Patient Inquiries](https://arxiv.org/abs/2605.15680)
*Liqi Zhou,Jiafu Li*

Main category: cs.CL

TL;DR: 本研究探讨了在低资源标注条件下，利用大型语言模型（LLMs）对在线患者询问进行四分类紧急分诊任务（自我护理、预约就诊、紧急临床审查、急诊转诊）的可行性。


<details>
  <summary>Details</summary>
Motivation: 在线患者询问往往非正式、不完整且未经专业评估，但仍需被分诊至适当的临床随访级别。

Method: 使用HealthCareMagic-100K语料库构建了300例人工校准的黄金评估集、700例自动标注的银色训练集和40例的少样本池，比较了TF-IDF和BioBERT基线模型与六种提示LLMs在0-shot、4-shot和12-shot条件下的性能，并使用macro-F₁及安全感知指标进行评估。

Result: 最强的LLM（Claude Haiku 4.5，12-shot）macro-F₁达到0.475，超过了最优监督基线BioBERT的0.378。少样本提示和双模型一致性在某些标签上可靠（如自我护理），但在紧急临床审查上不可靠。

Conclusion: LLMs可支持分诊优先排序和选择性人工审查，但还不能实现自主部署。
...
### [180] [ASRU: Activation Steering Meets Reinforcement Unlearning for Multimodal Large Language Models](https://arxiv.org/abs/2605.15687)
*Jiahui Guang,Yingjie Zhu,Cuiyun Gao,Haiyan Wang,Jing Li,Di Shao,Zhaoquan Gu*

Main category: cs.CL

TL;DR: 提出ASRU框架，通过激活重定向和奖励函数优化，在遗忘目标知识的同时保持生成质量和模型效用。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型遗忘方法只关注输出偏差，忽略生成质量，导致幻觉或生硬回应。

Method: 先通过激活重定向诱导拒绝行为，再用自定义奖励函数优化细粒度遗忘边界。

Result: 在Qwen3-VL上遗忘效果平均提升24.6%，生成质量提升5.8倍，仅需少量保留监督数据。

Conclusion: ASRU能有效平衡遗忘效果与模型效用，提升多模态模型的安全性和可用性。
...
### [181] [H-Mem: A Novel Memory Mechanism for Evolving and Retrieving Agent Memory via a Hybrid Structure](https://arxiv.org/abs/2605.15701)
*Jiawei Yu,Yixiang Fang,Xilin Liu,Yuchi Ma*

Main category: cs.CL

TL;DR: 提出了一种名为H-Mem的新型混合记忆机制，通过结合时间语义树和知识图结构，有效建模和检索Agent长期记忆，在QA任务上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM Agent记忆机制缺乏对记忆随时间演化的建模和有效检索，导致利用率低下。

Method: 构建时间语义树实现短期到长期记忆的渐进演化，同时构建知识图捕获实体关系，并利用混合结构进行高效检索。

Result: 在三个Agent基准上，QA任务达到最先进性能。

Conclusion: H-Mem成功解决了Agent记忆演化建模和检索问题，提升了记忆利用效率。
...
### [182] [SMMBench: A Benchmark for Source-Distributed Multimodal Agent Memory](https://arxiv.org/abs/2605.15710)
*Huacan Chai,Yukai Wang,Yingxuan Yang,Dan Peng,Yuanyi Song,Zhihui Fu,Weiwen Liu,Jianghao Lin,Jun Wang,Weinan Zhang*

Main category: cs.CL

TL;DR: 提出了源分布多模态记忆基准(SMMBench)，用于评估代理在跨多个独立来源中检索、对齐和组合多模态证据的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要在预组装上下文中评估多模态记忆推理，忽视了代理需从异质来源(如对话、截图、文档等)中整合证据的瓶颈。

Method: 构建了包含1877个样本和264个来源的基准，评估四大核心能力：跨源多模态推理、冲突解决、偏好推理、记忆驱动的动作预测。

Result: 当前记忆型与检索型基线方法在基准上表现不佳，表明源分布多模态记忆仍是未充分解决的挑战。

Conclusion: 源分布多模态记忆评估应作为多模态代理的重要研究方向，当前系统在该领域存在显著不足。
...
### [183] [Contexting as Recommendation: Evolutionary Collaborative Filtering for Context Engineering](https://arxiv.org/abs/2605.15721)
...
TL;DR: 提出神经协同上下文工程框架，将上下文工程从全局搜索转变为实例级动态路由，显著提升LLM任务准确率。


<details>
  <summary>Details</summary>
Motivation: 现有上下文工程方法假设单一上下文策略适用于整个数据集，忽略了不同输入需要不同指导，无法充分发掘实例级性能提升。

Method: 通过引导策略生成多样化的锚定上下文库，并设计上下文-协同过滤共演化机制，利用轻量级神经协同过滤模型学习实例-上下文偏好，动态为每个实例分配最优上下文。

Result: 理论证明和实验表明，NCCE通过个性化上下文匹配显著提升了任务准确率，超越了全局搜索方法。

Conclusion: 在LLM上下文工程中，个性化上下文选择至关重要，NCCE证明了实例级动态路由策略的有效性。
...
### [184] [DimMem: Dimensional Structuring for Efficient Long-Term Agent Memory](https://arxiv.org/abs/2605.15759)
*Wentao Qiu,Haotian Hu,Fanyi Wang,Jinwei Kong,Yu Zhang*

Main category: cs.CL

TL;DR: 提出DimMem，一种轻量级维度记忆框架，通过原子化、类型化、自包含的记忆单元（含时间、地点、原因等字段）实现高效长期记忆，在LoCoMo-10和LongMemEval-S上分别达到81.43%和78.20%准确率，且减少24%的每查询token成本。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理记忆系统面临保真度与效率的权衡：原始对话历史昂贵，而扁平事实或摘要可能丢失精确回忆所需的结构。

Method: DimMem将记忆表示为原子化、类型化、自包含的单元，包含时间、地点、原因、目的、关键词等显式字段，支持维度感知检索、更新和选择性回忆。

Result: 在LoCoMo-10和LongMemEval-S上准确率分别为81.43%和78.20%，优于现有轻量级记忆系统；LoCoMo每查询token成本降低24%；通过微调Qwen3-4B提取器，性能超越LightMem+GPT-4.1-mini，接近更大提取器。

Conclusion: 显式维度结构是LLM代理长期记忆有效且高效的基础。
...
### [185] [CompactQE: Interpretable Translation Quality Estimation via Small Open-Weight LLMs](https://arxiv.org/abs/2605.15763)
*Kamil Guttmann,Zofia Fraś,Artur Nowakowski,Krzysztof Jassem*

Main category: cs.CL

TL;DR: 本文证明了小规模开源LLM（<30B参数）在机器翻译质量估计中可替代大型专有模型，实现质量评分、错误标注、修正建议和译文编辑，且在系统级相关性上优于传统神经指标和人类标注者一致性。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的QE依赖大规模专有LLM，存在数据隐私问题。

Method: 使用单次提示策略，让小规模开源LLM同时生成质量评分、MQM错误注释、修正建议和完整后编辑。

Result: 小模型在系统级与人工判断的相关性上优于传统神经指标、微调模型和人类标注者一致性，接近大型专有模型能力。

Conclusion: 小规模开源LLM是兼顾成本、效果和隐私的可行QE方案。
...
### [186] [ForMaT: Dataset for Visually-Grounded Multilingual PDF Translation](https://arxiv.org/abs/2605.15794)
*Michał Ciesiółka,Dawid Wiśniewski,Adrian Charkiewicz,Kamil Guttmann*

Main category: cs.CL

TL;DR: 提出了ForMaT数据集，包含3956份PDF文档，覆盖15种语言对，保留了原始布局元数据，用于多模态机器翻译。通过K-Medoids采样确保结构多样性，评估显示当前MT系统在空间定位和几何同步方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译系统在处理PDF文档时，常丢失文本与视觉上下文之间的链接，导致翻译结果无法忠实还原原始布局。

Method: 构建包含45个几何特征的PDF文档表示，采用K-Medoids采样选择视觉多样性高的文档，生成具有布局元数据的平行语料。

Result: 当前MT系统难以处理空间定位和几何同步问题，ForMaT可用于评估布局感知翻译模型的表现。

Conclusion: ForMaT为开发融合文本与视觉上下文的布局感知翻译模型提供了重要基准，有助于实现高保真度的文档重建。
...
### [187] [Linked Multi-Model Data on Russian Domestic and Foreign Policy Speeches](https://arxiv.org/abs/2605.15886)
...
TL;DR: 本文介绍了一个俄罗斯政府跨模态政治传播数据集，包含克里姆林宫和外交部数十年官方演讲的文本、图像、元数据及主题标注，支持多模态、多语言、时空分析和社会科学与LLM研究。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏威权政治背景下社会文本和图像数据的可用性，本文旨在弥补这一不足。

Method: 收集官方演讲构建双语语料库，通过基于Transformer的多模态主题建模生成主题标注，并由俄罗斯政治专家验证。

Result: 提供了包含文本、图像、元数据和主题标注的关联数据集。

Conclusion: 该数据资源为威权政治传播的多模态、多语言、时空分析及社会科学和LLM应用提供了有价值的测试平台。
...
### [188] [Towards Generalization of Block Attention via Automatic Segmentation and Block Distillation](https://arxiv.org/abs/2605.15913)
*Shuaiyi Li,Zhisong Zhang,Yan Wang,Lei Zhu,Dongyang Ma,Chenlong Deng,Yang Deng,Wai Lam*

Main category: cs.CL

TL;DR: 该论文提出了语义分割数据集SemanticSeg和轻量级分割器，以及块蒸馏训练框架，以解决块注意力在长上下文场景中的文本分割和微调效率问题，实现接近全注意力的性能。


<details>
  <summary>Details</summary>
Motivation: 块注意力可提升RAG等长上下文场景的KV缓存重用，但面临文本难以分割为有意义块和现有块微调方法效率低、性能下降的问题。

Method: 构建包含30k以上实例的SemanticSeg数据集，训练轻量级分割器实现可控粒度的文本分割；提出块蒸馏框架，包含块汇合令牌、块丢弃和令牌级损失加权三个组件，使用冻结的全注意力教师模型指导学生模型。

Result: 分割器优于启发式和统计基线方法，块蒸馏在块注意力下达到接近全注意力的性能，建立了实用且可扩展的部署路径。
...
### [189] [Reference-Free Reinforcement Learning Fine-Tuning for MT: A Seq2Seq Perspective](https://arxiv.org/abs/2605.15976)
*Ernesto Garcia-Estrada,Carlos Escolano,José A. R. Fonallosa*

Main category: cs.CL

TL;DR: 将组相对策略优化应用于编码器-解码器机器翻译模型，在13种语言上实现了最高+5.03 chrF++的提升，且在无平行数据微调时与监督微调效果相当。


<details>
  <summary>Details</summary>
Motivation: 强化学习方法在机器翻译微调中主要针对解码器-only大语言模型，而编码器-解码器架构的系统性研究不足。

Method: 对NLLB-200模型（600M和1.3B）应用组相对策略优化，使用混合无参考奖励（LaBSE和COMET-Kiwi），无需平行数据。

Result: 在13种语言上均获得一致提升，最高为传统中文+5.03 chrF++，对形态复杂语言与3轮监督微调效果持平。增益在基线最差和奖励区分度最高时最大。

Conclusion: 该方法在平行数据稀缺场景下最有效，且模式在英语和西班牙语源语言中可复现。
...
### [190] [Ontology for Policing: Conceptual Knowledge Learning for Semantic Understanding and Reasoning in Law Enforcement Reports](https://arxiv.org/abs/2605.15978)
*Anita Srbinovska,Jansen Orfan,Adrian Martin,Ernest Fokoué*

Main category: cs.CL

TL;DR: 提出一个符号化框架，将执法报告中的叙述性文本自动转化为证据链接的事实，并构建时间图。


<details>
  <summary>Details</summary>
Motivation: 执法报告中的关键事件信息多存在于非结构化自然语言中，需要人工阅读，效率低下。

Method: 采用符号方法，包括个人标识符脱敏、语义解析、谓词到本体的映射以及推理，最终构建包含时间线索和领域公理的时间图。

Result: 在450份财产犯罪报告上评估，54.1%的事件置信度≥0.80，93.7%的事件成功映射通过PropBank-VerbNet-WordNet语义路径。人工审查显示，事件起始、被盗物品和时间线索达到100%一致，但强制进入的解释一致性较低。

Conclusion: 该符号化框架能有效从非结构化叙述中提取事件，但某些复杂解释（如强制进入）仍需改进。
...
### [191] [Defining Cultural Capabilities for AI Evaluation: A Taxonomy Grounded in Intercultural Communication Theory](https://arxiv.org/abs/2605.15990)
*Isar Nejadgholi,Masoud Kianpour,Krishnapriya Vishnubhotla,Maryam Molamohamadi*

Main category: cs.CL

TL;DR: 提出AI文化能力的三级分类法（文化意识、文化敏感性、文化胜任力），解决现有研究中概念模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI文化能力评估定义模糊、术语混用，且仅关注事实性知识，缺乏对知识使用方式和适应性的考察。

Method: 借鉴跨文化交际学术，提出三级分类法：文化意识（知识）、文化敏感性（知识应用方式）、文化胜任力（交互适应性）。

Result: 该分类法可提升AI评估的效度和可解释性，避免高估模型能力。

Conclusion: 明确文化能力构念对AI在多元文化场景中的安全部署至关重要。
...
### [192] [Can Vision Language Models Be Adaptive in Mathematics Education? A Learner Model-based Rubric Study](https://arxiv.org/abs/2605.16011)
*Jie Gao,Yongan Yu,Junzhu Su,Yiran Lin,Adam K. Dube,Jackie Chi Kit Cheung*

Main category: cs.CL

TL;DR: 提出基于学习者模型的评估框架，系统评测视觉语言模型在数学教学中的自适应能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在数学教学中的自适应能力缺乏系统评估方法和框架。

Method: 基于自适应学习框架构建包含认知、动机、复杂性三维度的评估 rubric，同时评测回答正确性和响应质量。

Result: 模型间自适应能力存在可测量差异，当前模型在有限学习者信息下难以稳定生成适应性教学响应。

Conclusion: 视觉语言模型在数学教学自适应方面仍有待提升，尤其是在信息受限场景下。
...
### [193] [Judge Circuits](https://arxiv.org/abs/2605.16023)
...
TL;DR: 通过因果分析揭示LLM作为评判者时，格式变化导致评分不一致的机制：共享的潜在评估子图在中间MLP层进行连续判断，再通过脆弱的格式特定分支映射输出。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅从输入输出层面发现LLM评判格式改变会导致评分不一致，但缺乏对内部机制的理解。

Method: 使用位置感知边缘归因修补（PEAP）对Gemma-3、Qwen2.5和Llama-3进行因果分析。

Result: 发现结构化理解和开放性偏好任务共享稀疏通用的潜在评估子图（在中间到后期MLP层），零消融它会破坏判断但保留世界知识；格式不一致源于共享主干中的连续判断信号通过脆弱的格式特定分支映射。

Conclusion: 格式间的可靠性比较部分衡量的是格式化器几何特性而非评估质量。
...
### [194] [From Flat Language Labels to Typological Priors: Structured Language Conditioning for Multilingual Speech-to-Speech Translation](https://arxiv.org/abs/2605.16026)
*Yu Pan,Yang Hou,Xiongfei Wu,Liang Zhang,Yves Le Traon,Lei Ma,Jianjun Zhao*

Main category: cs.CL

TL;DR: 提出了一个名为S2ST-Omni 2的面向多语言语音到语音翻译的组合框架，通过结构化的类型学先验替代平面的语言标签，改善了多语言适应性和数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有S2ST系统要么忽略源语言信息，要么通过语言即标签范式将其编码为独立平面嵌入，忽略了语言间的系统性结构，限制了在监督数据稀缺时的多语言适应能力。

Method: 在三个层面重新设计语言条件：基于类型学的分层语言编码（表示层）、动态门控语言感知的双CTC（声学层）、以及类型学感知的大语言模型提示（解码层）。

Result: 在CVSS-C数据集上，BLEU、COMET、ASR-BLEU和BLASER 2.0指标均优于代表性S2ST方法；消融研究表明三个策略互补；使用约3小时日语-英语监督数据实现高效适应。

Conclusion: 显式的类型学先验为数据高效的多语言S2ST提供了有用的归纳偏置。
...
### [195] [RecMem: Recurrence-based Memory Consolidation for Efficient and Effective Long-Running LLM Agents](https://arxiv.org/abs/2605.16045)
*Zijie Dai,Shiyuan Deng,Sheng Guan,Yizhou Tian,Xin Yao,Xiao Yan,James Cheng*

Main category: cs.CL

TL;DR: 提出RecMem，通过延迟记忆整合（仅在检测到语义重复时调用LLM）大幅降低令牌消耗，同时保持或提升记忆系统准确性。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统对每个交互都调用LLM提取记忆，导致大量令牌消耗，需要更高效的记忆整合策略。

Method: 使用轻量级嵌入模型编码交互存入潜意识层，仅在检测到语义重复时调用LLM提取情景和语义记忆，并加入语义精炼机制恢复遗漏细节。

Result: 相比三个SOTA系统，记忆构建令牌成本降低高达87%，同时准确率超过它们。

Conclusion: 基于重复性的延迟记忆整合能有效降低开销并提升性能，为长期记忆系统提供高效方案。
...
### [196] [Can Large Language Models Imitate Human Speech for Clinical Assessment? LLM-Driven Data Augmentation for Cognitive Score Prediction](https://arxiv.org/abs/2605.16077)
*Si-Belkacem Yamine Ketir,Lenard Paulo Tamayo,Shohei Hisada,Shaowen Peng,Shoko Wakamiya,Eiji Aramaki*

Main category: cs.CL

TL;DR: 提出基于LLM的数据增强框架，通过生成类口语式独白并结合语义引导的类平衡选择，提升从语音中预测认知分数的准确性。


<details>
  <summary>Details</summary>
Motivation: 语音认知评估面临数据集小和类别不平衡的挑战，需要提高预测性能。

Method: 利用GPT-5基于书面回答生成多风格口语独白，使用Sentence-BERT提取语音嵌入，结合偏最小二乘回归和两种数据增强策略（随机和语义引导类平衡选择）。

Result: 语义引导的类平衡选择显著降低了少数低分参与者的预测误差，同时保持了多数组的性能。

Conclusion: 语义引导的LLM增强方法能有效解决类别不平衡问题，提高临床语音分析的数据效率。
...
### [197] [Multi-Level Contextual Token Relation Modeling for Machine-Generated Text Detection](https://arxiv.org/abs/2605.16107)
*Chenwang Wu,Yiuming Cheung,Bo Han,Shuhai Zhang,Defu Lian*

Main category: cs.CL

TL;DR: 提出多层级上下文令牌关系建模框架，通过轻量马尔可夫校准和规则推理提升机器生成文本检测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有度量方法受生成随机性导致的评分偏差影响，需统一框架分析并解决该核心挑战。

Method: 构建统一分析框架后，理论推导令牌级检测分数的多跳转移关系，提出局部马尔可夫校准模块与全局规则推理模块联合建模。

Result: 在跨模型、跨领域场景下实现显著性能提升，且计算开销低。

Conclusion: 多层级关系建模可有效缓解随机性偏差，兼顾检测精度与效率。
...
### [198] [DebiasRAG: A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation](https://arxiv.org/abs/2605.16113)
*Rui Chu,Bingyin Zhao,Thanh Quoc Hung Le,Duy Cao Hoang,Huawei Lin,Ping Li,Weijie Zhao,Khoa D Doan,Yingjie Lao*

Main category: cs.CL

TL;DR: 提出 DebiasRAG 框架，利用检索增强生成技术动态减少大语言模型的社会偏见，无需微调或领域知识。


<details>
  <summary>Details</summary>
Motivation: 现有偏见缓解方法需要额外训练或领域知识，可能降低模型自身能力，且缺乏动态去偏见能力。

Method: 三阶段框架：查询特定去偏见候选生成、上下文候选池构建、梯度更新引导的去偏见上下文重排序。

Result: 在保持模型固有生成能力的同时，提升公平性，减少社会偏见。

Conclusion: DebiasRAG 是一种无调参、动态查询相关的去偏见方法，有效平衡模型性能与公平性。
...
### [199] [SGR: A Stepwise Reasoning Framework for LLMs with External Subgraph Generation](https://arxiv.org/abs/2605.16117)
*Xin Zhang,Yang Cao,Baoxing Wu,Kai Song,Siying Li*

Main category: cs.CL

TL;DR: 提出了SGR框架，通过外部子图生成增强LLM在复杂推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: LLM在需要深度推理和逻辑推断的复杂场景中表现有限，且可能生成不相关、噪声或事实不一致的内容。

Method: SGR从外部知识库构建查询相关的子图，引导模型逐步推理并结合多条推理路径得到最终预测。

Result: 在多个基准数据集上，SGR相比竞争基线取得了一致的性能提升。

Conclusion: SGR有效提高了推理准确性和事实可靠性。
...
### [200] [Optimized Three-Dimensional Photovoltaic Structures with LLM guided Tree Search](https://arxiv.org/abs/2605.16191)
*Michael P. Brenner,Lizzie Dorfman,John C. Platt*

Main category: cs.CL

TL;DR: 结合AI编码代理与树搜索算法（ERA）自主生成高效三维光伏结构，克服平板太阳能板在中纬度地区的损耗。


<details>
  <summary>Details</summary>
Motivation: 探索AI编码系统生成新颖科学假说的能力，解决平板太阳能板在中纬度地区的效率限制问题。

Method: 使用Google AntiGravity编码代理复现基准计算，结合LLM驱动的树搜索（ERA）优化三维光伏结构，并通过迭代约束消除奖励黑客行为。

Result: 发现树搜索初期产生非物理的高效解（奖励黑客），通过约束修复后获得改进设计，包括固定面积优化、天顶跟踪及避免自遮挡的构型。

Conclusion: ERA结合编码代理为可通过评分函数评估的科学问题提供了强大的发现平台。
...
### [201] [Improving Cross-Cultural Survey Simulation with Calibrated Value Personas](https://arxiv.org/abs/2605.16193)
*Axel Abels,Elias Fernandez Domingos,Apurva Shah,Tom Lenaerts*

Main category: cs.CL

TL;DR: 提出基于价值观的LLM人格构建方法，通过调查响应提取文化维度，结合校准程序提高人口级意见预测的准确性和多样性，缩小跨文化表现差距。


<details>
  <summary>Details</summary>
Motivation: 现有LLM模拟人类意见时跨文化表现有限，基于人口统计或人格特质的方法未能直接捕捉塑造人类响应的价值观。

Method: 从调查响应中提取核心文化维度生成文本描述符，采样目标人群的价值观分布，聚合多个人格的LLM响应，并引入校准程序增强响应多样性。

Result: 该方法降低了各国预测误差，尤其在代表性不足人群中改善显著，缩小了与训练数据主导国家的性能差距，且响应分布更接近人类多样性。

Conclusion: 基于价值观的人格构造和校准能有效提升LLM跨文化意见模拟的准确性和公平性。
...
### [202] [Argus: Evidence Assembly for Scalable Deep Research Agents](https://arxiv.org/abs/2605.16217)
*Zhen Zhang,Liangcai Su,Zhuo Chen,Xiang Lin,Haotian Xu,Simon Shaolei Du,Kaiyu Yang,Bo An,Lidong Bing,Xinyu Wang*

Main category: cs.CL

TL;DR: 提出了一个名为Argus的智能体系统，通过Searcher和Navigator协作，将深度研究任务视为拼图组装，而非并行搜索。Navigator维护证据图，调度Searcher收集缺失信息，再用强化学习训练以完成答案。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究方法通过并行搜索和聚合扩展推理时间，但并行轨迹常重复而非互补，导致收益递减并逼近上下文限制。

Method: 使用Searcher（ReAct风格）和Navigator（强化学习训练）协作。Navigator维护共享证据图，验证缺失证据，调度Searcher，并综合最终答案。

Result: 在8个基准上，单Searcher提升5.5分，8并行Searcher提升12.7分；64 Searcher在BrowseComp达86.2，超过所有专有智能体，且Navigator上下文维持在21.5K tokens内。

Conclusion: Argus通过协作和证据图管理，有效避免重复，显著提升性能，并保持效率。
...
### [203] [A Generative AI Framework for Intelligent Utility Billing CO 2 Analytics and Sustainable Resource Optimisation](https://arxiv.org/abs/2605.16250)
*Pavan Manjunath,Thomas Pruefer*

Main category: cs.CL

TL;DR: 提出一个统一框架，将AI生成自然语言账单、变压器负荷预测等四个生产级能力整合在同一架构下。


<details>
  <summary>Details</summary>
Motivation: 配电公司面临用户账单可读性、碳排放量化及负荷调度约束的新要求。

Method: 结合生成式AI智能体与约束解码策略，以及基于变压器的校准分位数预测模型。

Result: 实现了可解释账单生成与日前负荷精准预测。

Conclusion: 该框架能有效满足现代配电网的运营与监管需求。
...
### [204] [Artificial Aphasias in Lesioned Language Models](https://arxiv.org/abs/2605.16222)
*Nathan Roll,Jill Kries,Laura Gwilliams,Cory Shain*

Main category: cs.CL

TL;DR: 提出一种仿失语症方法，通过“损伤”语言模型参数并对比人类失语症症状分布，发现模型与人类在症状模式上存在定性差异。


<details>
  <summary>Details</summary>
Motivation: 理解语言模型的功能组织，并探究其与人类语言系统在损伤后症状表征上的异同。

Method: 对五个B级语言模型进行参数置零（损伤），使用文本失语症测试表（TAB）评估112,426个输出样本的症状分布。

Result: 模型表现出完整症状但分布异于人类；注意力与前馈组件间症状差异显著，同一机制内差异较小；浅层损伤更易引发句法和语义症状，中深层导致语言流畅性缺陷。

Conclusion: 失语症综合征受学习与处理细节的强烈影响，并非语言处理中断的跨领域不变结果。
...
### [205] [PhysBrain 1.0 Technical Report](https://arxiv.org/abs/2605.15298)
...
TL;DR: PhysBrain 1.0通过从大规模人类第一人称视频中提取结构化物理常识，将其转化为视觉语言动作模型的先验知识，并在多个基准测试中达到最优性能，尤其是在领域外任务上表现突出。


<details>
  <summary>Details</summary>
Motivation: 机器人轨迹数据覆盖有限，不足以支持广泛的物理理解学习，因此需要探索利用人类第一人称视频作为补充知识来源。

Method: 设计了数据引擎从视频中提取场景元素、空间动态、动作执行和深度关系，生成问答监督用于训练VLM，再通过保留能力和语言敏感的适应设计将物理先验迁移到VLA策略。

Result: 在ERQA、PhysBench、SimplerEnv-WidowX、LIBERO和RoboCasa等基准测试中取得SOTA结果，尤其在SimplerEnv的域外表现突出。
...
### [206] [Diffusion Policy for Coordinated Control of a Nonholonomic Mobile Base and Dual Arms in Door Opening and Passing](https://arxiv.org/abs/2605.15352)
*Shangqun Yu,Matthew En,Daniel Wu,Sangjun Park,Ziyi Zhou,Seyed Fakoorian,Donghyun Kim*

Main category: cs.RO

TL;DR: 提出基于扩散的视觉运动控制策略，让机器人端到端学习打开并穿过重型自闭门的长序列动作，实现操纵与移动的紧密协调。


<details>
  <summary>Details</summary>
Motivation: 传统手工设计状态机方法缺乏鲁棒性，无法泛化到真实环境；现有模仿学习方法未能同时协调非完整底盘和双臂完成完整的开门过门任务。

Method: 采用扩散模型（diffusion based visuomotor control policy）学习端到端的视觉运动控制策略。

Result: 该策略在高成功率打开并穿过阻尼拉门的同时，展现出对外部干扰的强大鲁棒性。

Conclusion: 单一端到端策略可学习执行需要操纵与移动紧密协调的长时域任务，且性能优于传统方法。
...
### [207] [Where to Perch in a Tree: Vision-Guidance for Tree-Grasping Drones](https://arxiv.org/abs/2605.15430)
*Alex Dunnett,Leonie Bottomley,Mirko Kovac,Basaran Bahadir Kocer*

Main category: cs.RO

TL;DR: 提出了一种基于视觉引导的无人机自主栖枝方法，通过图像处理评估树枝的宽度、坡度和曲率，选择理想栖枝位置。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅识别最近树枝，未评估树枝的适宜性，而实际中需要更精确的栖枝选择。

Method: 结合机器学习、图像分割和二值形态学等算法，分析树枝形状和结构，评估宽度、坡度、曲率等因素。

Result: 在10000多张城市树木图像数据集上，对可行目标（树枝足够粗、空间足够大）的成功率为76%。

Conclusion: 初步结果验证了方法的有效性，未来需整合深度感知和姿态传感器数据，以形成通用方法。
...
### [208] [Residual Reinforcement Learning for Robot Teleoperation under Stochastic Delays](https://arxiv.org/abs/2605.15480)
*Kaize Deng,Zewen Yang*

Main category: cs.RO

TL;DR: 提出一种结合LSTM状态估计器和残差强化学习策略的混合控制框架，用于解决随机通信延迟下的遥操作稳定性和性能问题。


<details>
  <summary>Details</summary>
Motivation: 随机通信延迟会导致信号不连续，破坏控制稳定性，传统强化学习在延迟观测下表现不佳，产生高频抖动。

Method: 延迟韧性强化学习框架：使用LSTM从延迟观测中重建平滑连续的状态估计，结合残差强化学习策略学习扭矩补偿。

Result: 在Franka Panda机器人上实验证明，该方法在高方差随机延迟下显著优于现有基线，实现鲁棒稳定的遥操作。

Conclusion: 所提出的方法有效应对随机延迟，平衡跟踪精度与速度平滑性，保障遥操作稳定性。
...
### [209] [Hybrid LLM-based Intelligent Framework for Robot Task Scheduling](https://arxiv.org/abs/2605.15486)
*Swayamjit Saha,Subhabrata Das,Haonan Duan,Xiao-Yang Liu*

Main category: cs.RO

TL;DR: 本研究提出了基于大语言模型（LLMs）的智能框架，用于优化建筑机器人任务调度，通过双代理协同（生成器GPT-4与监督器Gemma 3/Llama 4/Mistral 7b）及自然语言接口实现实时适应性调度。


<details>
  <summary>Details</summary>
Motivation: 现有建筑机器人任务调度在时间效率、资源利用及动态环境适应性方面存在不足，需引入LLM增强智能决策能力。

Method: 采用GPT-4和Gemma 3/Llama 4/Mistral 7b双LLM代理分别作为生成器与监督器；结合自然语言处理接口处理任务目标、动作能力等关键数据，并制定平衡时间与资源的分配策略。

Result: 在简单场景测试中，所提框架获得有效指标分数，证明LLM在建筑操作任务中的关键作用。

Conclusion: LLM的引入能显著提升建筑机器人任务调度的精度与实时适应性，对复杂环境下的施工优化具有重要潜力。
...
### [210] [FLASH: Efficient Visuomotor Policy via Sparse Sampling](https://arxiv.org/abs/2605.15492)
*Jiaqi Bai,Jindou Jia,Yuxuan Hu,Gen Li,Xiangyu Chen,Tuo An,Kuangji Zuo,Jianfei Yang*

Main category: cs.RO

TL;DR: 提出FLASH Policy，利用勒让德多项式表示动作轨迹，通过跳过噪声初始化实现单步推理，显著加速机器人控制策略。


<details>
  <summary>Details</summary>
Motivation: 现有扩散/流匹配策略推理延迟高，难以满足实时机器人控制需求。

Method: 用勒让德多项式拟合稀疏采样下的专家演示，从历史多项式系数初始化流匹配过程，实现单步精确推理，并利用多项式解析微分直接提供速度前馈信号。

Result: 在7个任务上成功率≥92%，推理速度31.40ms（比扩散策略快175倍，比流匹配策略快18倍），训练收敛比ACT快4倍，控制器跟踪误差降低5-7倍。

Conclusion: FLASH Policy通过连续轨迹表示和初始化技巧，在保证高成功率的同时显著降低推理延迟，适合实时控制。
...
### [211] [LAPS: Improving Incremental LiDAR Mapping using Active Pooling and Sampling for Neural Distance Fields](https://arxiv.org/abs/2605.15496)
*Dongjae Lee,Wooseong Yang,Yifu Tao,Maurice Fallon,Ayoung Kim*

Main category: cs.RO

TL;DR: 提出LAPS框架，通过主动重放管理（可靠性保留+不确定性采样）提升神经距离场增量LiDAR建图的抗遗忘能力与重建完整性。


<details>
  <summary>Details</summary>
Motivation: 现有基于重放的增量神经建图方法采用被动缓冲池与均匀采样，导致内存浪费在冗余数据上，且欠约束区域训练不足。

Method: 结合可靠性主动池化（reliability-based active pooling）保留关键历史样本，以及不确定性引导主动采样（uncertainty-guided active sampling）聚焦优化不足区域。

Result: 在Oxford Spires数据集Blenheim Palace 05序列上，召回率提升4.66个百分点，F1分数提升3.79个百分点，同时保持几何精度。

Conclusion: LAPS在有限内存下一致提升重建完整性，可作为增量神经建图的通用重放管理方案。
...
### [212] [A QUBO Formulation Framework for Kinematic Structure-Based Robot Design Optimization: A Robotic Hand Case Study](https://arxiv.org/abs/2605.15510)
*HyoJae Kang,Yeong Jae Park,Jeongdo Ahn,Dongil Park*

Main category: cs.RO

TL;DR: 提出了一种基于二次无约束二元优化的机器人设计优化框架，用经典计算评估设计依赖指标，将组合选择问题转化为量子退火兼容的结构，并以机械手为例验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 机器人设计优化需要处理结构级评价指标和组合选择问题，现有方法缺乏统一框架，且量子退火在机器人设计中应用潜力待探索。

Method: 使用QUBO公式化框架，结合经典计算评价指标与量子退火优化，整合设计奖励、重叠工作空间交互、独热约束和结构依赖惩罚为统一二次模型，并通过模拟退火和量子退火验证。

Result: 获得了满足独热选择和成对约束的可行设计组合，随着读取次数增加目标值范围变窄，框架还可推广到其他机器人系统。

Conclusion: 该框架提供了一种通用方法，将基于运动学结构的机器人设计问题转化为组合优化问题，适用于量子退火硬件执行。
...
### [213] [Terrain Consistent Reference-Guided RL for Humanoid Navigation Autonomy](https://arxiv.org/abs/2605.15517)
*William D. Compton,Zachary Olkin,Aaron D. Ames*

Main category: cs.RO

TL;DR: 提出一种训练参考引导的感知强化学习运动策略的方法，通过在训练中根据地势几何调整参考轨迹，使类人机器人能通过SE(2)速度接口与标准导航规划器集成，并在户外崎岖地形和楼梯上实现超70米长时自主导航。


<details>
  <summary>Details</summary>
Motivation: 现有类人机器人运动策略难以在复杂地形中结合参考轨迹与导航规划器，缺乏对环境感知的地形一致性调整。

Method: 在强化学习训练循环中合成SE(2)可控参考轨迹，将期望落脚点投影到有效支撑点，并调整摆动脚和质心轨迹以适应地形；策略暴露SE(2)速度接口与导航规划器兼容。

Result: 模拟中环境条件参考显著提升跟踪性能；硬件上通过MPC+控制障碍函数规划器，在Unitree G1上实现超70米全自主导航，穿越户外粗糙地形和连续楼梯。

Conclusion: 所提方法有效结合了参考轨迹的地形适应性和标准导航接口，能在复杂户外环境中实现类人机器人的长时自主导航。
...
### [214] [Task-Semantic Graph-Driven Distributed Agent Networking for Underwater Target Tracking](https://arxiv.org/abs/2605.15528)
*Shengchao Zhu,Guangjie Han,Chuan Lin,Yu He*

Main category: cs.RO

TL;DR: 开发了一个开源的多智能体强化学习水下自主潜航器编队目标跟踪平台，并提出语义任务图增强算法STG-MAPPO。


<details>
  <summary>Details</summary>
Motivation: 现有的AUV编队研究缺乏统一的、基于六自由度动力学的开源评估平台，且现有策略难以表征任务阶段、观测可靠性和局部协作角色。

Method: 整合DI-engine与六自由度AUV目标跟踪模拟器构建开源平台，提出STG-MAPPO算法，利用语义任务图将通信约束网络状态映射到决策，并采用速度级动作抽象。

Result: 首次连接公共MARL训练框架与物理建模的AUV编队任务，提供了统一实验协议；STG-MAPPO能有效处理任务阶段、观测置信度等语义信息。

Conclusion: 该平台和算法为AUV编队目标跟踪提供了公平评估基准，并在分布式跟踪中展现出优越性。
...
### [215] [SkiP: When to Skip and When to Refine for Efficient Robot Manipulation](https://arxiv.org/abs/2605.15536)
*Mingtong Dai,Guanqi Peng,Yongjie Bai,Feng Yan,Chunjie Chen,Lingbo Liu,Liang Lin,Xinyu Wu*

Main category: cs.RO

TL;DR: 提出Skip Policy（SkiP）方法，通过动作重标注机制使策略自动跳过冗余步骤，在关键步骤密集决策，减少执行步骤15%-40%的同时保持或提升成功率。


<details>
  <summary>Details</summary>
Motivation: 现有模仿学习策略对所有控制步骤采用统一密集预测，但大部分自由空间轨迹冗余，仅关键步骤（如接触、抓取）需要高精度预测。

Method: 提出动作重标注机制，在跳跃段内将行为克隆目标替换为下一关键段入口动作；同时提出Motion Spectrum Keying（MSK）自动划分关键/跳跃段，无需手动标注或分层结构。

Result: 在72个模拟任务和3个真实机器人任务上，SkiP减少15%-40%执行步骤，并匹配或提升各策略主干的成功率。

Conclusion: SkiP通过动态决策跳跃与密集精化，实现高效、简洁的模仿学习，且无需额外规划器或层级结构。
...
### [216] [KaRMA: A Kinematic Metric for Fine Manipulation Ability in Robotic Hands](https://arxiv.org/abs/2605.15548)
*Martin Peticco,Pulkit Agrawal*

Main category: cs.RO

TL;DR: 提出了KaRMA指标，用于量化两指捏持下球体物体的运动学滚动操作能力，通过广度优先搜索评估可达平移和旋转覆盖。


<details>
  <summary>Details</summary>
Motivation: 现有机器人手指标仅测量静态属性（如工作空间、可操作性），无法直接衡量手内连续改变物体位姿的灵活性。

Method: 定义运动学滚动操作能力（KaRMA），施加关节极限、碰撞、滚动接触和对向力可行性约束，通过广度优先搜索遍历平移和旋转基元，计算三种得分：平移覆盖（KaRMA-T）、旋转覆盖（KaRMA-R）和初始抓取敏感性（KaRMA-S）。

Result: 在16种广泛使用机器人手上评估，KaRMA能区分静态指标无法区分的对手，揭示平移-旋转权衡，与精选任务基准定性一致，且比基于雅可比矩阵的指标更准确。

Conclusion: KaRMA是一种有效的运动学滚动操作度量，克服了静态指标的局限性，能更真实反映手内精细操作能力。
...
### [217] [NavRL++: A System-Level Framework for Improving Sim-to-Real Transfer in Reinforcement Learning-Based Robot Navigation](https://arxiv.org/abs/2605.15559)
*Zhefan Xu,Hanyu Jin,Kenji Shimada*

Main category: cs.RO

TL;DR: 本文提出了一种强化学习导航框架，结合系统实证分析与扰动感知微调，实现了零样本的仿真到现实迁移，并在多种机器人平台上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的自主导航研究主要关注框架设计，但缺乏对仿真到现实迁移因素的深入分析，导致实际部署性能受限。

Method: 包括一个完整的训练和部署流程、对传感器噪声等关键因素的实证分析、扰动感知微调策略以及基于Transformer的时间推理策略。

Result: 该方法在静态和动态环境中优于学习基线，在静态环境中性能与优化规划器相当，并在飞行和足式机器人上实现了零样本迁移。

Conclusion: 通过系统分析迁移关键因素并设计针对性策略，可显著提升强化学习导航的跨环境部署鲁棒性。
...
### [218] [Wind-Aware Optimal Trajectory Planning for Efficient Gliding of Fixed-Wing Aerial Systems](https://arxiv.org/abs/2605.15619)
*Luca Morando,Nishanth Bobbili,Giuseppe Loianno*

Main category: cs.RO

TL;DR: 提出一种基于伯恩斯坦多项式的非线性多成本轨迹规划器，用于小型固定翼无人机滑翔，实现能量平衡与抗风扰动的自主控制。


<details>
  <summary>Details</summary>
Motivation: 传统总能量控制系统需精细调参且依赖配平状态知识，无法有效应对风扰和障碍约束，需要更鲁棒的规划层控制方法。

Method: 采用伯恩斯坦多项式生成C3连续轨迹，通过微分平坦映射为控制指令，在线重规划匹配实验沉极曲线；集成模拟净变率计估计气团运动，约束至能量平衡状态；结合杜宾斯路径的巡航段实现混合任务。

Result: CFD仿真与实飞实验验证：在阵风和障碍物条件下，能可靠稳定下沉率、空速和滑翔比。

Conclusion: 将能量控制提升至规划层可有效提升滑翔机在复杂环境中的自主性与鲁棒性。
...
### [219] [Propagating Unsafe Actions in LLM Controlled Multi-Robot Collaboration via Single Robot Compromise](https://arxiv.org/abs/2605.15641)
*Zhen Huang,Zhihuang Liu,Weishang Wu,Zhiping Cai*

Main category: cs.RO

TL;DR: 本文提出了一种针对大型语言模型控制的多机器人协作系统的新型攻击范式，仅通过感染一个入口机器人，即可通过通信传播恶意指令，导致系统级不安全行为。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单机器人场景的安全威胁，而多机器人协作中通过间通信传播的安全风险尚未被充分探索。

Method: 提出一种攻击范式，攻击者仅与单个入口机器人交互，被感染的机器人通过编程通信将恶意指令传播至其他机器人。

Result: 实验显示，攻击在最强情况下服从度达1.00，传染性升至0.90，仅需3.0轮即可感染所有机器人，且隐蔽性得分为0.81。

Conclusion: 多机器人规划器存在持续的安全对齐缺口，协调机制可能无意中允许恶意指令覆盖安全要求，尤其在紧急或权利冲突场景中风险加剧。
...
### [220] [MyoChallenge 2025: A New Benchmark for Human Athletic Intelligence](https://arxiv.org/abs/2605.15650)
...
TL;DR: MyoChallenge 2025 提出了一个基于高保真肌肉骨骼模型和物理仿真的运动控制基准，包含乒乓球和足球射门两个任务，吸引了近70支队伍和560多个提交，推动了运动人工智能和跨学科研究。


<details>
  <summary>Details</summary>
Motivation: 当前AI和机器人系统难以复制人类运动智能（如快速选择、精准控制、敏捷协调），且由于设备限制，对生物复杂肌肉协调的体内测量很少，因此需要填补这一理解空白。

Method: 利用高保真肌肉骨骼模型在物理仿真中结合机器学习算法，设计两个赛道：上肢控制的乒乓球回合和下肢控制的足球点球，采用基于物理的运动规划器、在线策略行为克隆、分层规划和肌肉协同等技术。

Result: 吸引了全球近70支队伍和超过560份提交，促进了多个面向肌肉骨骼系统运动敏捷性的最先进控制算法的发展。

Conclusion: MyoChallenge'25 通过标准化任务和生理真实模型，在开放框架 MyoSuite 中为机器学习、生物力学、运动科学和神经科学的跨学科研究提供了可重复利用的测试平台。
...
### [221] [Reactive Robot-Centric Safety for Autonomous Navigation in Constrained and Dynamic Environments](https://arxiv.org/abs/2605.15782)
*Viswa Narayanan Sankaranarayanan,Vignesh K. Viswanathan,Akshit Saradagi,Sumeet Satpute,George Nikolakopoulos*

Main category: cs.RO

TL;DR: 提出了一种基于3D激光雷达和复合控制屏障函数的实时安全控制架构，用于自主机器人在受限动态环境中的导航。
...
Method: 将3D激光雷达感知与基于复合控制屏障函数的安全滤波器集成到自主管线中，使用椭球体定义安全区域，并针对每个激光雷达点构建时变控制屏障函数。

Result: 通过四足机器人在地下环境进行视觉检查任务的多次实地实验验证，系统在动态障碍物、不安全高层参考、定位异常和狭窄走廊中均能可靠运行。

Conclusion: 该感知驱动框架能够以控制频率动态强制执行避碰约束，且对名义任务执行干扰最小，有效提升了动态环境下的实时安全性。
...
### [222] [PCASim: Promptable Closed-loop Adversarial Simulation for Urban Traffic Environment](https://arxiv.org/abs/2605.15654)
*Chuancheng Zhang,Zhenhao Wang,Kaizheng Li,Yaran Lin,Qiang Guo,Bin Jiang*

Main category: cs.RO

TL;DR: 提出一个集成对抗性场景生成与安全智能体训练的闭环测试框架，通过大型语言模型和强化学习提升自动驾驶测试的全面性与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究鲜少在闭环测试中将对抗性场景生成与安全智能体训练结合以实现协同进化，难以高效应对城市环境下的大量边缘情况。

Method: 基于规则过滤开源数据集构建对抗行为知识库，结合知识检索模块，利用LLM整合知识驱动的、数据驱动的和对抗驱动的方法，生成安全关键的交通场景，并通过强化学习训练不同车辆行为以丰富场景多样性。

Result: 领域特定语言生成准确性提高12%，新场景转换成功率提升8%，避障能力增强30%。

Conclusion: 所提框架有效促进了场景生成与智能体训练的相互增强，显著提升了自动驾驶测试的安全性和鲁棒性。
...
### [223] [Dynamic Plasma Shape Control with Arbitrary Sensor Subsets](https://arxiv.org/abs/2605.15935)
*D. Sorokin,M. Stokolesov,A. Granovskiy,I. Prokofyev,E. Adishchev,M. Nurgaliev,E. Khayrutdinov,G. Subbotin,R. Clark,D. Orlov*

Main category: cs.RO

TL;DR: 本文提出了一种强化学习智能体，能够同时处理托卡马克等离子体形状的动态跟踪和传感器故障容错，在仿真和实验中均取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 传统方法将等离子体形状控制分解为平衡重建和线性控制器，但无法处理动态变化的目标和传感器故障，需要一种更鲁棒的方法。

Method: 使用强化学习智能体，在NSFsim（DIII-D高保真模拟器）上训练，基于120种实验等离子体形状数据集；引入传感器随机掩蔽和不对称演员-评论家架构。

Result: 静态配置下平均形状误差2.01 cm；动态轨迹跟踪在仿真和实验中验证；对30%传感器丢失具有鲁棒性；成功转移到DIII-D实验和GSevolve模拟器。

Conclusion: 强化学习智能体能够同时实现动态形状控制和传感器故障容错，无需备份控制器，且策略可迁移至实际实验。
...
### [224] [Feedback World Model Enables Precise Guidance of Diffusion Policy](https://arxiv.org/abs/2605.15705)
*Tuo An,Jindou Jia,Gen Li,Jingliang Li,Chuhao Zhou,Pengfei Liu,Bofan Lyu,Jiaqi Bai,Xinying Guo,Geng Li,Jianfei Yang*

Main category: cs.RO

TL;DR: 提出反馈世界模型（Feedback World Model），通过在线更新轻量级反馈状态来闭环预测与观测，减少分布偏移时的预测误差并提升机器人任务成功率。


<details>
  <summary>Details</summary>
Motivation: 传统世界模型在分布外状态预测不准确，而执行过程中观测到的真实状态信息未被充分利用。

Method: 在推理时维护一个在线更新的反馈状态，通过潜空间观测器迭代校正未来预测，并引入动作感知指导以关注动作可控部分。

Result: 在LIBERO-Plus等任务中，预测误差降低76.4%，分布外成功率提升30%。

Conclusion: 实时反馈推理方式简单有效，为静态世界建模提供了替代方案。
...
### [225] [Constrained MPC-Based Motion Planning for Morphing Quadrotors in Ultra-Narrow Passages under Limited Perception](https://arxiv.org/abs/2605.15999)
*Harsh Modi,Xiao Liang,Minghui Zheng*

Main category: cs.RO

TL;DR: 本文提出一种用于变形四旋翼在极端受限环境下的运动规划框架，通过非线性MPC中的新型避障代价函数实现基于2D激光雷达的窄缝穿越。


<details>
  <summary>Details</summary>
Motivation: 经典人工势场法在狭窄通道中会产生高代价，阻碍路径规划，因此需要一种能在窄间隙中保持低通行代价且兼顾强避障能力的代价函数。

Method: 提出平滑指数障碍代价函数，包含代价缩减因子以避免硬激活阈值，并直接在MPC中使用2D激光雷达测量值进行避障。

Result: 仿真与实验表明该方法能够成功穿越典型斥力代价函数无法通过的狭窄走廊，计算高效且安全性高。

Conclusion: 该代价函数具有通用性，适用于任何移动机器人，不仅限于变形四旋翼，并已开源代码。
...
### [226] [Learning Dynamic Pick-and-Place for a Legged Manipulator](https://arxiv.org/abs/2605.15713)
...
TL;DR: 提出一种分层强化学习框架，使四足机器人结合6自由度机械臂实现动态抓取-放置任务，支持重物和广阔工作空间。


<details>
  <summary>Details</summary>
Motivation: 现有四足移动操作平台难以同时实现精确操控与协调运动，尤其在处理重物和连续动态任务时存在局限。

Method: 采用分层强化学习，集成显式质量估计模块，实现适应不同负载的全身控制。

Result: 仿真中负载2.3kg时成功率达86.05%；真实实验中负载1.3kg时成功率73.3%，执行时间4.06秒，工作空间从地面至1.1米高度。

Conclusion: 该方法可实现动态连续的全身操控任务，且能处理更重负载和更广工作空间，优于既往分段式慢速方案。
...
### [227] [Hierarchical and Holistic Open-Vocabulary Functional 3D Scene Graphs for Indoor Spaces](https://arxiv.org/abs/2605.15753)
*Xinggang Hu,Chenyangguang Zhang,Alexandros Delitzas,Xiangkui Zhang,Marc Pollefeys,Francis Engelmann,Xiangyang Ji*

Main category: cs.RO

TL;DR: 提出了一种基于2D视觉锚定和3D图优化的开放词汇管线，用于在密集桌面场景中构建功能3D场景图，解决了小尺度、密集相似实例和动态视角下的关系推理难题。


<details>
  <summary>Details</summary>
Motivation: 现有基准局限于大型家具且缺乏层次结构，导致功能3D场景图的潜力未被充分利用。

Method: 通过2D视觉证据锚定细粒度功能边、多线索跨帧节点关联，并采用时间图优化（证据累积、熵正则化、时间平滑）确定功能连接，最后进行全局层次塑造。

Result: 在真实密集场景中可靠推断功能3D场景图，验证了方法的有效性。
...
### [228] [Lamarckian Inheritance in Dynamic Environments: How Key Variables Affect Evolutionary Dynamics](https://arxiv.org/abs/2605.15769)
*K. Ege de Bruin,Kyrre Glette,Kai Olav Ellefsen*

Main category: cs.RO

TL;DR: 研究了在软体机器人中，拉马克式遗传是否能优化形态与控制的协同进化，取决于环境变化的冲突性和可预测性。


<details>
  <summary>Details</summary>
Motivation: 关于动态环境中拉马克式遗传的有效性存在矛盾证据，需要明确其适用条件。

Method: 结合形态进化（演化）与控制器优化（终身学习），使用贝叶斯优化和强化学习，在虚拟软体机器人上进行实验。

Result: 拉马克式遗传仅在环境变化既冲突又不可预测时表现劣于达尔文式遗传；添加传感器可恢复其优势。

Conclusion: 拉马克式遗传在动态环境中的效用由环境变化的冲突性和可预测性决定，可预测性可通过感知提升。
...
### [229] [A Topology-Aware Spatiotemporal Handover Framework for Continuous Multi-UAV Tracking](https://arxiv.org/abs/2605.15779)
*Jianlin Ye,Christos Kyrkou,Panayiotis Kolios*

Main category: cs.RO

TL;DR: 本文提出一种实时多摄像头多车辆跟踪系统，利用基于拓扑和时空的轻量级交接机制，在连续车流中实现99.8%的手动交接成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的多无人机交通监控系统因轨迹碎片化丢失车辆身份信息，无法支持网络级分析（如起终点估计）。

Method: 使用YOLO11和ByteTrack处理4K视频流，核心是确定性队列匹配算法，通过几何重叠和虚拟车道离散化预测身份交接。

Result: 在复杂城市环境（如交叉口）中，连续车流的手动交接成功率为99.8%，远超ReID基线（74.1%）。

Conclusion: 系统实现高身份持久性，支持边缘部署，适用于大规模交通监控。
...
### [230] [GAP: Geometric Anchor Pre-training for Data-Efficient Visuomotor Learning of Manipulation Tasks](https://arxiv.org/abs/2605.15836)
*Davide Buoso,Andrea Protopapa,Stefano Di Carlo,Francesca Pistilli,Giuseppe Averta*

Main category: cs.RO

TL;DR: 提出几何锚点预训练（GAP），通过轻量级模拟代理任务预训练空间适配器，生成稳定几何锚点，在极少示范下提升机器人操作的视觉运动策略学习效果。


<details>
  <summary>Details</summary>
Motivation: 从稀缺专家示范中学习视觉运动策略时，高维RGB表示难以蒸馏为控制相关的几何信息，且预训练的视觉基础模型（VFM）在少量数据微调时易过拟合任务无关捷径，导致几何基础丢失和场景扰动脆弱性。

Method: GAP在模仿学习前，利用带物体掩码的轻量级模拟代理任务预训练空间适配器，强制其生成位于物体上、覆盖空间范围、保持尖锐且可重复的关键点，作为稳定几何锚点，同时冻结VFM。

Result: 在RoboMimic和ManiSkill数据集上，使用15-50个示范时，GAP正则化的简单适配器优于强注意力池化和全微调方法：RoboMimic Can任务15个示范成功率62%（+16%），Tool Hang任务50个示范成功率63%，ManiSkill StackCube任务30个示范成功率61%（+11%）。
...
### [231] [Structured Jacobian Construction for Motion Optimization with High-Order Time Derivatives in Multi-Link Systems](https://arxiv.org/abs/2605.15845)
*Taiki Ishigaki,Ko Ayusawa,Eiichi Yoshida*

Main category: cs.RO

TL;DR: 提出了一种针对多连杆系统运动优化的结构化雅可比计算框架，能够系统地表示为高阶时间导数的运动量并提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在涉及高阶时间导数的运动优化中，利用数值或自动微分计算雅可比，忽略了多连杆结构，导致计算成本高且数值不稳定。

Method: 基于综合运动计算框架，对多连杆结构上的运动量和其高阶时间导数进行系统表示，并推导出运动学与动力学量的解析雅可比表达式。

Result: 数值实验表明，该方法相比数值和自动微分提高了计算效率，同时保持相当精度；在逆优化中有效从运动数据恢复代价函数权重。

Conclusion: 该方法为涉及高阶时间导数的多连杆系统运动优化提供了可扩展且结构化的计算基础。
...
### [232] [Designing for Robot Wranglers: A Synthesis of Literature and Practice](https://arxiv.org/abs/2605.15892)
*David Porfirio,Ian McDermott,Hsin-Mei Chen,Satoru Satake,Takayuki Kanda,Thomas D. LaToza*

Main category: cs.RO

TL;DR: 本文通过梳理文献和个人反思，定义了机器人操控者（robot wrangler）的概念，并提出了支持这一角色的设计建议。


<details>
  <summary>Details</summary>
Motivation: 随着机器人进入人类环境（如医院、博物馆、仓库），需要专人负责其设置、监控和故障处理，但这一角色及其工作特点缺乏系统研究。

Method: 采用范围综述（scoping review）识别文献中的操控活动类型，并结合作者自身作为操控者的第一手经验进行反思分析。

Result: 发现机器人操控是一个涵盖复杂多样活动的统称，且常因概念模糊而难以支持；基于综述和反思，提出了直接支持操控者个人及服务生态的设计建议。

Conclusion: 理解机器人操控者的需求对整合机器人至人类空间至关重要，设计应兼顾个体操作者与整体服务系统。
...
### [233] [FocalPolicy: Frequency-Optimized Chunking and Locally Anchored Flow Matching for Coherent Visuomotor Policy](https://arxiv.org/abs/2605.15944)
*Qian He,Zhenshuo Yang,Wenqi Liang,Chunhui Hao,Nicu Sebe,Jiandong Tian*

Main category: cs.RO

TL;DR: 提出FocalPolicy，通过频域优化分块和局部锚定流匹配来平衡动作精度与远见，解决跨块动作不连续问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法优化块内动作分布而忽略块间连贯性，导致长程动作不连续，难以学习复杂任务。

Method: 引入远见复合目标，同时监督近端动作的时间域对齐和多个未来动作块的频域结构正则化；设计局部锚定采样以提高一致流匹配训练的梯度传播效率。

Result: 在复杂操作任务上优于现有方法，且模块可泛化至其他基线模型。

Conclusion: 频域优化分块和局部锚定可有效提升长程动作连贯性与精度平衡，具有良好泛化性。
...
### [234] [A Reproducible and Physically Feasible Dynamic Parameter Identification Framework for a Low-Cost Robot Arm](https://arxiv.org/abs/2605.15949)
*Junji Oaki,Koki Yamane,Koki Inami,Sho Sakaino*

Main category: cs.RO

TL;DR: 针对低成本机器人臂CRANE-X7，提出了可重复且物理可行的动态参数识别框架，通过移除惯性积、设计特定运动、结合多种算法，最终获得统计一致且物理有效的动态模型。


<details>
  <summary>Details</summary>
Motivation: 低成本机器人臂的动态参数识别面临物理可行性低和统计一致性差的挑战，现有方法难以兼顾，需要开发实用方法。

Method: 结合预处理、逆动力学回归最小二乘、条件半定规划投影、闭环输入误差细化，通过主成分分析选择代表性模型，并进行全姿态正定性审计和局部修正。

Result: 参数云从OLS到SDP和CLIE逐渐集中，最终模型在验证运动上保持高预测精度。

Conclusion: 该框架为低成本机器人平台提供了统计一致且物理可行的动态模型实用路径。
...
### [235] [WorldVLN: Autoregressive World Action Model for Aerial Vision-Language Navigation](https://arxiv.org/abs/2605.15964)
...
TL;DR: 提出首个自回归世界动作模型WorldVLN，用于空中视觉语言导航，通过预测世界状态并生成动作，在基准测试中取得12%以上的成功率提升。


<details>
  <summary>Details</summary>
Motivation: 空中视觉语言导航需根据指令闭环感知与行动，现有方法未利用世界演化预测，导致复杂场景导航能力不足。

Method: 采用自回归视频骨干预测短时世界状态变化，直接解码为路径点动作；结合两阶段训练：指令视频先验学习与基于行动感知GRPO的强化学习。

Result: 在室外/室内基准测试中成功率提升12%+，挑战场景优势更大，且零样本迁移至真实无人机部署。

Conclusion: 世界动作模型为空间导航任务提供新范式，验证了预测驱动行动的有效性。
...
### [236] [OHP-RL: Online Human Preference as Guidance in Reinforcement Learning for Robot Manipulation](https://arxiv.org/abs/2605.15971)
*Yunyang Mo,Jian Li,Qiwei Wu,Yihang Kang,Renjing Xu*

Main category: cs.RO

TL;DR: 提出OHP-RL框架，将人类干预视为偏好信息来引导强化学习策略，在真实机器人操作任务中取得更好性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将人类干预仅作为辅助训练信号，未能充分利用其在安全与任务约束下对行为相对偏好的信息。

Method: 引入状态依赖的偏好门控机制，自适应调节人类干预对策略学习的时机和程度，保持自主探索与稳定优化。

Result: 在Franka机器人的三个接触式操作任务中，OHP-RL取得更高成功率、更快收敛和更低干预成本，策略行为更稳定对齐人类偏好。

Conclusion: 人类干预编码的相对偏好可有效指导策略学习，OHP-RL实现了高效且安全的机器人技能获取。
...
### [237] [Fast Expanding Safe Circular Regions for Efficient Local Path Planning](https://arxiv.org/abs/2605.16009)
*Scott Fredriksson,Akshit Saradagi,George Nikolakopoulos*

Main category: cs.RO

TL;DR: 本文提出一种基于几何算法的局部导航方法，通过LiDAR扫描计算一系列圆形区域来快速规划路径，在复杂环境中比优化或学习方法更高效。


<details>
  <summary>Details</summary>
Motivation: 现有局部导航方法在简单环境中表现良好，但优化或学习类方法在复杂场景中计算慢或规划能力不足。

Method: 从局部LiDAR扫描中计算一系列沿目标方向扩张的圆形区域，以捕捉自由可导航空间，实现几何算法导航。

Result: 在ROS2框架中实现并在仿真环境中评估，显示计算更快、规划视距更长。

Conclusion: 该几何方法为复杂环境下的局部导航提供了高效且可扩展的解决方案。
...
### [238] [Adaptive Outer-Loop Control of Quadrotors via Reinforcement Learning](https://arxiv.org/abs/2605.16015)
*Vishnu Saj,Sushi Vemuri,Dileep Kalathil,Moble Benedict*

Main category: cs.RO

TL;DR: 提出一种基于残差动力学预测器(RDP)的自适应控制架构,仅用状态和控制历史在线估计外力矩,结合线性校准与推力修正实现高效仿真到实物迁移。


<details>
  <summary>Details</summary>
Motivation: 现有DRL四旋翼控制因域随机化导致策略保守,难以应对动态扰动。

Method: 训练最优外环策略后用RDP替代真实扰动数据;引入线性校准桥与在线推力修正对齐仿真与真实潜空间。

Result: 在Crazyflie上,该控制器在质量变化、非对称负载等动态干扰下显著优于基线方法。

Conclusion: 所提自适应架构通过主动感知扰动和优化仿真到实物迁移,有效提升了四旋翼在动态不确定环境下的轨迹跟踪精度。
...
### [239] [Learning Sim-Grounded Policies for Bimanual Rope Manipulation from Human Teleoperation Data](https://arxiv.org/abs/2605.16043)
*Gina Wigginghaus,Tim Missal,Berk Guler,Simon Manschitz,Jan Peters*

Main category: cs.RO

TL;DR: 该研究在绳结解乱任务中，比较了基于视觉和基于物理状态（3D粒子状态）的模仿学习策略，发现基于状态的策略在预测抓取动作时L1误差降低30.8%，表明观测空间的选择对泛化能力至关重要。


<details>
  <summary>Details</summary>
Motivation: 体感遥操作模仿学习在双手机器人操作可变形线性物体（DLO）时受限于人力成本，而观测空间的选择直接影响小数据集下的泛化能力。研究旨在探究泛化不足是否源自观测空间本身，而非策略架构或数据规模。

Method: 使用相同遥操作数据训练两种动作分块变换器（ACT）策略：视觉策略基于腕部摄像头双路RGB流，状态策略基于DLO的3D粒子状态（通过多视图融合初始观测并基于粒子扩展位置动力学仿真演化）。

Result: 在未见过的绳索配置开环评估中，状态策略的初始抓取-拉动动作预测L1误差比视觉策略低30.8%。

Conclusion: 状态策略优于视觉策略，量化了像素与物理一致状态之间的可观测性差距，表明从有限人类演示中实现更高效DLO操作学习需选择更优观测空间。
...
### [240] [Health-Conditioned Vision-Language-Action Models for Malfunction-Aware Robot Control](https://arxiv.org/abs/2605.16056)
*Hüseyin Arslan,Özgür Erkent*

Main category: cs.RO

TL;DR: 提出一种健康条件化的视觉-语言-动作（VLA）模型，通过注入健康投影器模块，使机器人能够适应关节退化等物理故障，在LIBERO环境中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏对机器人物理退化（如关节损伤、执行器故障）的适应能力，而真实场景中机器人常面临此类问题。

Method: 在VLA-Adapter架构中嵌入健康投影器模块，以健康向量（包含关节角度与扭矩能力信息）作为输入；在LIBERO-Spatial任务上收集128个遥操作故障演示数据训练模型。

Result: 轻量级改进使模型能成功适应不同退化关节配置，性能优于默认预训练的VLA-Adapter模型。

Conclusion: 健康条件化VLA模型可有效提升机器人物理故障下的任务完成能力，且扩展成本低。
...
### [241] [Towards Trustworthy and Explainable AI for Perception Models: From Concept to Prototype Vehicle Deployment](https://arxiv.org/abs/2605.16087)
*Till Beemelmanns,Shayan Sharifi,Manas Mehrotra,Ayushman Choudhuri,Lutz Eckstein*

Main category: cs.RO

TL;DR: 提出了一个面向自动驾驶的可靠AI感知模块，集成了鲁棒性、可解释性和不确定性校准，并进行了实车验证。


<details>
  <summary>Details</summary>
Motivation: 现有理论框架缺乏在3D场景理解中的具体实现，难以满足可信AI的透明性要求。

Method: 基于Transformer检测器，利用注意力机制生成解释，通过扰动测试验证忠实性；集成不确定性估计与校准模块，并应用鲁棒性增强训练。

Result: 实现了忠诚的显著图、提升的鲁棒性以及良好校准的不确定性估计，并在实车原型中实时运行。

Conclusion: 证明了在自动驾驶感知中集成可解释性、鲁棒性和不确定性估计的可行性，为可信AI落地提供了范例。
...
### [242] [Beyond Collision Avoidance: Multi-Robot Yielding and Spatial Affordance in Emergency Evacuations](https://arxiv.org/abs/2605.16115)
*Ning Zhou,Edmund R. Hunt,Nikolai W. F. Bode*

Main category: cs.RO

TL;DR: 通过游戏化虚拟疏散实验（N=56），发现多机器人避让策略存在偏好层级（藏匿>线性逃离>冻结>最短路径），并揭示环境可供性显著影响认知期待，主动利用避难空间能提升心理舒适度，而认知期待违背会引发延迟感知。


<details>
  <summary>Details</summary>
Motivation: 现有机器人避让策略仅关注碰撞规避和宏观流量优化，忽略环境可供性与人类空间期待，缺乏微观感知与宏观理论的衔接。

Method: 采用游戏化虚拟疏散实验（N=56），测试四种多机器人避让策略（藏匿、线性逃离、冻结、最短路径）在有/无避难凹槽的狭窄走廊中的个体心理反应。

Result: 建立偏好层级（藏匿>线性逃离>冻结>最短路径）；主动空间让行（隐藏）显著优于冻结和效率优先策略；利用可用凹槽增强主动让行的心理舒适度；未能利用明显凹槽（如线性逃离）触发认知违背，放大感知延迟；机器人交互经验有助于解码复杂社交意图。
...
### [243] [DexJoCo: A Benchmark and Toolkit for Task-Oriented Dexterous Manipulation on MuJoCo](https://arxiv.org/abs/2605.16257)
...
TL;DR: 提出了DexJoCo，一个面向灵巧手任务导向操作的基准和工具包，包含11个功能性任务、1100条轨迹和多种评估设置，揭示了当前策略的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有灵巧手基准缺乏体现灵巧手独特能力（如工具使用、双手协调）的任务，且缺乏全面的评估流程。

Method: 通过低成本数据采集系统收集1100条轨迹，支持域随机化，并在视觉/动力学随机化、多任务训练、动作头适配等设置下对现有模型进行基准测试。

Result: 识别了当前灵巧手操作策略的重要见解和常见局限，突出了未来研究的关键挑战。

Conclusion: DexJoCo提供了一个标准化基准，为灵巧手机器人学习提供了改进方向。
...
### [244] [DualReg: Dual-Space Filtering and Reinforcement for Rigid Registration](https://arxiv.org/abs/2508.17034)
*Jiayi Li,Yuxin Yao,Qiuhang Lu,Juyong Zhang*

Main category: cs.RO

TL;DR: 提出了一种双空间刚性配准范式，融合特征匹配与几何匹配的优势，通过轻量级滤波和锚点几何代理实现高效精确的配准，在KITTI上比MAC快32倍且精度相当。


<details>
  <summary>Details</summary>
Motivation: 现有刚性配准方法难以同时处理大变换差异、部分重叠噪声和实时处理的需求。

Method: 双空间范式：先通过轻量级一点RANSAC和精炼模块过滤不可靠特征匹配，再将可靠匹配作为锚点提取几何代理，构建目标函数并用定制求解器估算变换。

Result: 在KITTI数据集上达到与MAC相当的精度，同时实现32倍的CPU时间加速。

Conclusion: 该方法有效融合了特征匹配和几何匹配的优点，在效率与精度上取得良好平衡，适用于实时刚性配准任务。
...
### [245] [Continuous-time Predictor-Based Subspace Identification with Hermite basis expansions](https://arxiv.org/abs/2605.15318)
*Jose Antonio Rebollo,Enrico Barbiero,Marco Lovera*

Main category: eess.SY

TL;DR: 提出了一种基于Hermite基函数的连续时间子空间辨识方法（HD-PBSID），直接识别LTI系统的连续时间状态空间形式。


<details>
  <summary>Details</summary>
Motivation: 现有连续时间子空间辨识方法存在时移问题或依赖离散化，需要一种能直接处理连续时间模型且避免时移的方法。

Method: 利用Hermite基函数投影信号获得有限系数，通过递归关系与导数性质推导导数算子，结合PBSID方法步骤，并对输入输出信号进行缩放避免时移。

Result: 仿真研究表明该方法估计精度高，与基于Laguerre投影的连续时间PBSID算法相比性能更优。

Conclusion: HD-PBSID方法可有效直接辨识连续时间状态空间模型，且避免了时移问题。
...
### [246] [Lie Generator Networks Extract EIS-Grade Battery Diagnostics from Pulse Relaxation Data](https://arxiv.org/abs/2605.15351)
*Shafayeth Jamil,Rehan Kapadia*

Main category: eess.SY

TL;DR: 提出Lie Generator Networks框架，利用60秒脉冲后电压弛豫数据提取与阻抗谱等效的电化学时间常数，实现无需专用硬件的电池诊断。


<details>
  <summary>Details</summary>
Motivation: 传统EIS诊断需专用硬件和数分钟采集，不适用于现场部署；现有弛豫分析无法达到与EIS相当的诊断精度。

Method: 构建Lie Generator Networks结构保持辨识框架，通过约束网络架构保证动力学生成矩阵的稳定性，从弛豫数据中提取电化学时间常数。

Result: 在850+电池、4个机构、多种化学体系的数据集上，退化追踪近乎完美（|ρ_s|=0.999），Nyquist谱重建中位误差2%，可预测容量匹配电池的早期失效，零先验恢复Arrhenius活化能。

Conclusion: LGN将任意弛豫脉冲转化为阻抗级诊断，无需训练数据、阻抗硬件或化学标定，实现实时健康监测、快速梯次利用分级和质量控制。
...
### [247] [Coordinated Trajectory Control Algorithm for Quadcopter Motion along a Smooth Spatial Trajectory](https://arxiv.org/abs/2605.15357)
*Stanislav Kim,Anton Pyrkin,Oleg Borisov*

Main category: eess.SY

TL;DR: 提出了一种四旋翼飞行器沿光滑空间轨迹运动模型，并基于此设计了鲁棒控制算法，通过引入积分器和扩展观测器实现三维空间中的协调轨迹跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有四旋翼控制方法在处理未测量扰动和状态不完全信息时存在局限性，需要一种鲁棒且易于整定的控制策略。

Method: 基于几何方法综合控制律，引入附加积分器简化控制器整定，并使用扩展观测器实现可实现的输出反馈。

Result: 在未测量扰动和状态不完全信息条件下，实现了三维空间中的协调轨迹跟踪。

Conclusion: 所提出的控制算法有效且稳定，适用于实际四旋翼系统。
...
### [248] [Optimizing Chilled Water Systems with Cooling Towers via Virtual Power Metrics and Extremum-Seeking Control](https://arxiv.org/abs/2605.15431)
*Min Gyung Yu,Alex Vlachokostas,Karthik Devaprasad,Matt Cornachione,Stephanie Johnson,Tim A. Yoder,Timothy I. Salsbury*

Main category: eess.SY

TL;DR: 提出了一种用于冷却塔风扇的极值寻优控制方法，可降低冷水机组总能耗约15%，并设计虚拟功率计替代物理仪表，实现低成本部署。


<details>
  <summary>Details</summary>
Motivation: 传统冷却塔控制能耗较高，且许多系统缺乏物理功率计，无法直接应用优化策略。

Method: 采用极值寻优控制算法优化风扇速度；开发虚拟功率计利用现有传感器数据估计功耗。

Result: 模拟显示夏季节能约15%；虚拟功率计与物理仪表相关度96.11%，归一化误差5.11%。

Conclusion: 该方法无需额外仪表即可在现有系统上实现显著节能。
...
### [249] [A Variational Lagrangian Framework for Log-Homotopy Particle Flow Filters](https://arxiv.org/abs/2605.15379)
*Olivér Törő,Domonkos Csuzdi,Tamás Bécsi*

Main category: eess.SY

TL;DR: 本文通过最小作用量原理，将贝叶斯更新中的log-同伦粒子流滤波建模为无压无黏流体的最优输运问题，推导出与量子力学Madelung流体表述同构的Hamilton-Jacobi方程，并给出流动的物质加速度动力学描述。


<details>
  <summary>Details</summary>
Motivation: 粒子流滤波中控制速度的偏微分方程本质上欠定，有无穷多可行解；现有方法缺乏甄别最优流动的物理准则，需要一种基于物理原理的变分框架来唯一确定最优流动。

Method: 将粒子流视为无压无黏流体运动，定义以动能为拉格朗日量、以连续性方程和log-同伦演化为约束，应用最小作用量原理导出欧拉-拉格朗日方程，得到无旋势流结构，并推导出与量子力学Madelung流体同构的Hamilton-Jacobi方程及物质加速度。

Result: 变分框架给出唯一的最优无旋流动，揭示log-同伦约束类似于量子势；物质加速度的推导将运动学描述推广为动力学描述，为引入辛积分器和自适应检测高维滤波中的刚度提供了可能。

Conclusion: 该变分方法为粒子流滤波提供了物理上合理的最优性准则；动力学视角有望提升数值稳定性并改善高维滤波的计算效率。
...
### [250] [Co-Design Optimization for Data Center Cooling System via Digital Twin](https://arxiv.org/abs/2605.15516)
*Shrenik Jadhav,Zheng Liu*

Main category: eess.SY

TL;DR: 提出一个三层优化框架，联合优化冷却分配单元在子回路间的整数分配、流量分数分配及总流量与供水温度，在Frontier超算上实现35.48%的年冷却能耗节省。


<details>
  <summary>Details</summary>
Motivation: 液冷超算的冷却系统存在多个并行子回路，但如何分配冷却分配单元和分布流量的问题未在设施规模上系统解决。

Method: 基于Modelica仿真模型和替代模型，评估25个冷却分配单元的611种可行分区，比较三种逐步丰富的运营策略。

Result: 两子回路设计实现35.48%年冷却能耗节省（三子回路当前设计为35.30%），流量分数优化可将设计敏感性降低93%。

Conclusion: 该框架可迁移至其他液冷高性能计算设施，提供低成本的软件优化路径达到接近最优性能。
...
### [251] [Direct Data-Driven Linear Quadratic Tracking via Policy Optimization](https://arxiv.org/abs/2605.15563)
*Shubo Kang,Keyou You*

Main category: eess.SY

TL;DR: 提出了一种参考解耦的LQT重构方法，使得决策变量维度固定，并开发了离线与在线DeePO算法，理论上证明了全局线性收敛，仿真验证了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有直接数据驱动最优控制（如DeePO）在LQR中通过协方差参数化解决了在线决策变量维度增长问题，但将其扩展到LQT时，时变参考与反馈-前馈策略结构的复杂耦合导致无法直接应用固定维参数化。

Method: 提出参考解耦的LQT重构，使其自然适应协方差参数化；基于此设计了离线与在线DeePO算法，并在理论上利用局部梯度优势和光滑性证明线性收敛。

Result: 离线算法具有全局线性收敛性；在线算法的最优性差距呈线性衰减，偏差项与信噪比成反比；数值仿真验证了理论结果和优越的跟踪性能。

Conclusion: 所提出的参考解耦方法有效解决了LQT中决策变量维度依赖数据时长的瓶颈，为直接数据驱动最优控制的LQT问题提供了高效且可实现的解决方案。
...
### [252] [Enabling Intelligent Bidirectional Charging: A Real-World Communication Interface Between Electric Vehicles, Charging Infrastructure, and a Control Optimizer](https://arxiv.org/abs/2605.15731)
*Shangqing Wang,Abhirup Sain,Christopher Lehmann,Shiwei Shen,Razan Habeeb,Frank H. P. Fitzek*

Main category: eess.SY

TL;DR: 本文介绍了在德累斯顿实际部署和验证的用户感知双向电动汽车充电系统，从仿真原型过渡到城市环境中的运行部署。


<details>
  <summary>Details</summary>
Motivation: 为了支持电网灵活性和可持续交通，需要将概念模型转化为实际运营系统，特别是通过整合实时车辆、用户数据和优化平台来实现动态充放电决策。

Method: 系统结合了无线OBD-II接口、5G校园网连接的开放中间件节点、平板界面捕捉用户偏好，并使用OCPP协议构建多级通信架构，集成软件、硬件和网络实现实时充电管理。

Result: 在德累斯顿Ostra Sport Park的现场部署展示了可行性，改善了负荷平衡，并实现了稳健的车到网操作。早期数据采集和预测控制提高了系统效率。

Conclusion: 该工作为积极能源区和未来城市电动出行系统提供了实用基准。
...
### [253] [Preserving Topology Privacy of Network Systems by Feedback: Conditions and Distributed Design](https://arxiv.org/abs/2605.15743)
*Yushan Li,Jiabao He,Julien M. Hendrickx,Dimos V. Dimarogonas*

Main category: eess.SY

TL;DR: 本文提出了一种基于反馈的方法，在网络系统的共识协议中保护拓扑隐私，通过故意违反拓扑可辨识性条件来防止真实拓扑被准确恢复，同时保持共识行为。文章设计了部分和完全观测情况下的反馈条件，构建了分布式拓扑修改方案，并在隐私预算限制下实现了共识偏差与拓扑隐私的可控权衡，最后通过低复杂度启发式算法优化边缘隐私保护。


<details>
  <summary>Details</summary>
Motivation: 现有共识协议中，拓扑信息可能被观测数据泄露，而直接修改拓扑可能影响系统功能。因此需要一种方法，在保持共识性能的同时，防止拓扑被唯一或准确恢复。

Method: 推导了部分和完全观测情况下的反馈条件（前者刻画拓扑不可解性，后者构建解空间强制拓扑不准确性）；提出分布式拓扑修改设计并在隐私预算下建立性能保证；开发低复杂度启发式算法实现最优边缘隐私保护。

Result: 通过比较仿真验证了所提保存设计的有效性和优越性，实现了共识偏差与拓扑隐私的可控权衡。

Conclusion: 该方法能有效保护网络系统中共识协议的拓扑隐私，同时通过可控折衷维持系统共识行为，并具备低复杂度实现优势。
...
### [254] [Fairness-Guaranteed Online Power Allocation Policies for EV Fast Charging Stations](https://arxiv.org/abs/2605.15750)
*Can Berk Saner,Yong-Sheng Soh,Antonios Varvitsiotis*

Main category: eess.SY

TL;DR: 本文提出两种支持公平性的电动汽车快充站实时功率分配策略（FAIR-OPAP-C和FAIR-OPAP-M），无需充电曲线先验知识，在超配场景下实现无嫉妒、帕累托最优和比例公平，计算耗时低于1毫秒。


<details>
  <summary>Details</summary>
Motivation: 电动汽车快速扩张对快充站提出可扩展、高效需求，超配配置下需保障公平性，但充电曲线数据难以获取。

Method: 提出两种在线分配策略：FAIR-OPAP-C用于连续可调功率站，FAIR-OPAP-M用于模块化站，仅利用瞬时功率请求，无需充电曲线，结合无嫉妒、帕累托效率及比例公平框架。

Result: 在7个基准测试中表现优异，计算复杂度接近线性或对数级，处理300辆EV时运行时间低于1毫秒。

Conclusion: 所提算法兼顾公平与效率，适合硬件受限边缘设备实时部署。
...
### [255] [Uncertainty Propagation under Residual Disturbances: A Smart-Home Case Study](https://arxiv.org/abs/2605.15851)
*Guanru Pan,Dirk Reinhardt,Sebastien Gros,Timm Faulwasser*

Main category: eess.SY

TL;DR: 提出了一种数据驱动的框架，用于在未测量或未建模的干扰下进行不确定性传播，通过残差扰动合并所有非结构化干扰，并利用多项式混沌展开和切比雪夫不等式实现高效不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性传播方法难以处理未测量或统计上未建模的干扰，需要一种数据驱动的方法来整合这些无结构扰动。

Method: 将无结构扰动合并为可估计的残差扰动，构建因果且分布一致的随机预测器，结合多项式混沌展开和高阶切比雪夫不等式进行不确定性量化。

Result: 在挪威智能家居的实验数据上验证了方法的有效性。

Conclusion: 该方法能有效处理无结构扰动下的不确定性传播，具有实际应用价值。
...
### [256] [Communication-Efficient Approximate Gradient Coding for Distributed Learning in Heterogeneous Systems](https://arxiv.org/abs/2605.15890)
*Heekang Song,Wan Choi*

Main category: eess.SY

TL;DR: 提出了一种通信高效的最优结构化梯度编码方案，联合优化梯度编码和量化以应对异构分布式学习中的掉队者和通信效率问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法单独处理梯度编码或量化，缺乏统一优化，导致通信效率低或收敛速度慢。

Method: 建立统一框架，将梯度编码和量化联合优化为最小化残差误差的问题，推导出闭环最优码结构和最优比特分配策略，并设计低复杂度算法。

Result: 在COCO数据集上实验表明，联合设计显著加速收敛并提升通信效率，优于现有基线。

Conclusion: 该方案能够有效应对异构分布式学习中的掉队者问题，同时提升通信和收敛性能。
...
### [257] [State Estimation](https://arxiv.org/abs/2605.15936)
*Hao Li*

Main category: eess.SY

TL;DR: 文章介绍了先进控制理论中状态估计方面的基础知识，强调控制理论在实践中的应用和魅力。


<details>
  <summary>Details</summary>
Motivation: 控制科学对现代文明至关重要，但除了硬件、软件、经济等方面，方法论（即控制理论）是控制系统最本质的方面，且理论需与实践结合以展现其价值。

Method: 介绍了先进控制理论在实际应用中的状态估计方法，参考文献[1,2]提供详细内容。

Result: 阐述了状态估计作为先进控制理论的关键方面，以及理论与实践融合的重要性。

Conclusion: 控制理论不仅具有数学重要性，其魅力更源于实践应用，状态估计是实际应用中不可或缺的部分。
...
### [258] [Communication-Efficient Federated Online Decision-Making with Stateful Costs](https://arxiv.org/abs/2605.16047)
*Yiwei Liu,Luwei Yang,Shunbo Lei*

Main category: eess.SY

TL;DR: 本文提出BLADE算法，在联邦在线决策中实现动态遗憾界，仅需O(T/K)通信，且实验验证了其性能。


<details>
  <summary>Details</summary>
Motivation: 研究块同步和部分客户参与下，稀疏通信对联邦在线决策中状态轨迹和成本的影响。

Method: 使用投影块式联邦在线决策方法BLADE，采用块同步和部分客户参与策略。

Result: BLADE在动态遗憾上对路径长度有界的比较序列取得次线性界，当K=⌈√T⌉时扰动量VT=o(T^{1/4})下界为次线性。实验验证了通信-遗憾等权衡效果。

Conclusion: BLADE在稀疏通信下有效平衡了通信成本与动态遗憾，并适用于稳定线性系统。
...
### [259] [Active Learning MPC Objective Functions from Preferences](https://arxiv.org/abs/2605.16071)
*Hasna El Hasnaouy,Pablo Krupa,Mario Zanon,Alberto Bemporad*

Main category: eess.SY

TL;DR: 使用主动学习策略从人类偏好中高效学习模型预测控制的目标函数，减少查询次数。


<details>
  <summary>Details</summary>
Motivation: 模型预测控制的目标函数设计困难，尤其是当性能评估仅依赖人类判断时；偏好学习需要大量昂贵的人工查询。

Method: 提出了两种主动学习策略：基于池的选择（结合不确定性和多样性）和查询合成（利用当前代理驱动的MPC生成新轨迹）。

Result: 数值结果表明，与随机采样相比，所提策略能用更少的查询次数获得更符合偏好的闭环行为。

Conclusion: 主动学习能有效提升偏好学习在MPC目标函数优化中的采样效率。
...
### [260] [Watts vs. Bytes: Turning Data Centers into Grid Assets via Storage Compute Co-Optimization](https://arxiv.org/abs/2605.16190)
*Shaohui Liu,Sungho Shin,Deepjyoti Deka*

Main category: eess.SY

TL;DR: 本文提出了一种鲁棒协同优化框架，用于数据中心与电池储能系统（BESS）的日前调度，在电网互联限制下通过工作负载调度、DVFS和BESS退化感知管理实现成本优化和辅助服务参与。


<details>
  <summary>Details</summary>
Motivation: 数据中心的持续增长导致电网压力增大，需要灵活协调计算需求与BESS以改善站点运营并提供电网服务。

Method: 开发了鲁棒协同优化模型，联合考虑截止时间约束的工作负载（通过调度和DVFS管理）和退化感知BESS调度，实现成本优化和辅助服务市场参与。

Result: 基于真实市场和负载数据的案例研究表明，框架在互联约束严格时能产生可行调度，BESS日价值提升两倍以上，主要源于减少工作负载未完成率；非可调度任务比例高时运营成本增加超25%；DVFS在负载限制严苛时提供关键灵活性。

Conclusion: 协调计算-存储灵活性可显著扩展数据中心运行裕度和电网价值，尤其在电网容量稀缺时效果更明显。
...
### [261] [Text-RSIR: A Text-Guided Framework for Efficient Remote Sensing Image Transmission and Reconstruction](https://arxiv.org/abs/2605.15558)
*Hao Yang,Xianping Ma,Peifeng Ma,Man-On Pun*

Main category: eess.IV

TL;DR: 提出了一种文本引导的遥感图像传输系统，用低分辨率图像加文本描述代替完整高清数据，传输量降至约2%，并通过跨模态学习恢复细节。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像传输受限于带宽和成本，传统全分辨率像素传输效率低。

Method: 机载文本生成器生成空间和语义摘要；地面文本条件图像恢复模型利用跨模态学习重建细节。

Result: 在Alsat-2B、UC Merced和Aerial Image数据集上PSNR分别达16.36 dB、26.87 dB和27.41 dB。

Conclusion: 该系统实现了高效且信息保持的遥感图像传输，适用于环境监测等应用。
...
### [262] [DIPA: Distilled Preconditioned Algorithms for Solving Imaging Inverse Problems](https://arxiv.org/abs/2605.15456)
*Romario Gualdrón-Hurtado,Roman Jacome,Leon Suarez,Henry Arguello*

Main category: eess.IV

TL;DR: 本文提出DIPA方法，将预处理技术扩展到成像逆问题中，通过教师-学生蒸馏优化预处理算子，提升重建质量，适用于MRI、压缩感知和超分辨率成像。


<details>
  <summary>Details</summary>
Motivation: 物理约束导致感知矩阵病态，传统数据保真度最小化困难；现有预处理仅用于加速收敛，未用于改进重建质量。

Method: 通过教师-学生蒸馏框架，教师使用理想化感知矩阵，学生使用物理可行矩阵；优化线性或非线性预处理算子，设计多种蒸馏损失。

Result: 在MRI、压缩感知和超分辨率成像中验证，预处理算子能提升重建质量，非线性版本扩展性更好。

Conclusion: 预处理概念可扩展至改善重建质量，蒸馏式预处理算子设计有效且具解释性/扩展性。
...
### [263] [TVRN: Invertible Neural Networks for Compression-Aware Temporal Video Rescaling](https://arxiv.org/abs/2605.15579)
*Xinmin Feng,Li Li,Dong Liu,Feng Wu*

Main category: eess.IV

TL;DR: 提出了一种压缩感知的帧率缩放端到端框架TVRN，通过可逆架构和替代网络应对高频信息损失和编解码器不可微问题，实验表明在工业视频压缩下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅通过训练目标连接降帧率与升帧率操作，未充分利用其互逆性导致高频信息损失，且忽略了有损编解码器对低帧率视频的影响。

Method: 采用可逆架构，结合多输入多输出时域小波变换和高频重建模块，设计梯度替代网络实现端到端训练，并通过学习排序策略引入压缩感知特征扩展为不对称架构。

Result: 在工业视频压缩设置下，TVRN在重建质量上优于现有方法。

Conclusion: TVRN有效解决了帧率缩放中的高频信息损失和编解码器兼容性问题，提升了实际应用中的性能。
...
### [264] [Dynamic resolution switching for live streaming](https://arxiv.org/abs/2605.15490)
*Xin Xiong,Yixu Chen,Hai Wei,Yongjun Wu,Sriram Sethuraman*

Main category: eess.IV

TL;DR: 提出一种动态分辨率切换（DRS）框架，通过实时分析内容特性和带宽分布，优化直播流的自适应码率（ABR）阶梯，实现约9%的BD-rate降低。


<details>
  <summary>Details</summary>
Motivation: 传统ABR系统使用静态码率阶梯忽略内容特性，而现有逐标题优化方法依赖源内容预编码分析，不适用于直播场景；同时，客观视频质量指标在交叉预测中精度不足。

Method: 使用基于比特流的轻量级视频质量测量（VQM），通过成对比较数据集训练，实时评估候选分辨率并构建动态阶梯；质量分析在可配置粒度（如每片段）下驱动分辨率切换。

Result: 实验显示，在提出的VQM下实现约9%的BD-rate降低，同时保持直播的实时可行性。
...
### [265] [Degradation-Aware Blur-Segmentation of Brain Tumor](https://arxiv.org/abs/2605.15671)
*Yuchun Wang,Xiaosong Li,Gefei Liang,Yang Liu*

Main category: eess.IV

TL;DR: 提出DABSeg网络，同步去模糊与分割3D多模态MRI脑肿瘤，在BraTS2020数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设MRI图像无伪影，但患者运动导致模糊，降低分割性能。

Method: 特征域运动去模糊模块、模糊感知跨模态交叉注意模块、多尺度残差聚合、加权Dice与重建损失联合优化。

Result: 在清晰和退化条件下，肿瘤Dice得分和边界精度均优于现有方法。

Conclusion: 退化感知跨任务协作学习提升了多模态3D脑肿瘤分割的鲁棒性和临床实用性。
...
### [266] [Highly Detailed and Generalizable Broadleaf Tree Crown Instance Segmentation from UAV Imagery](https://arxiv.org/abs/2605.15673)
...
TL;DR: 提出了一种用于无人机影像中天然阔叶林单木树冠分割的高精度实例分割模型，基于Mask2Former和大规模高质量标注数据，在多种森林类型中表现良好。


<details>
  <summary>Details</summary>
Motivation: 阔叶林树冠形状多样且无明显树顶，导致其树冠勾绘比其他森林类型更具挑战性。

Method: 使用基于Mask2Former的深度学习模型，训练于18,507个手工标注的树冠多边形，并测试多种骨干网络。

Result: 最佳模型仅用RGB影像即在结构复杂的阔叶林中实现高分割精度，并在日本不同森林及婆罗洲热带雨林中保持泛化性能。

Conclusion: 大规模高质量标注数据集是实现通用且精细树冠分割的关键，该模型已集成到DF Scanner Pro软件中，支持基于无人机的阔叶林监测。
...
### [267] [Evaluation of Anatomical Shape Priors in Deep Learning-Based Cardiac Multi-Compartment Segmentation](https://arxiv.org/abs/2605.15707)
*Michael Hudler,Franz Thaler,Martin Urschler*

Main category: eess.IV

TL;DR: 在心脏CT多分量分割任务中，轻量级显式形状先验（形状感知损失和空间标签分布热图引导的U-Net）对3D U-Net基线仅有边际且不一致的提升，甚至常导致性能下降。


<details>
  <summary>Details</summary>
Motivation: 基于标准CNN分割缺乏解剖合理性显式约束的临床需求，探索轻量级形状先验能否改进心脏CT分割。

Method: 形状感知损失函数和空间标签分布热图引导的U-Net变体，在MM-WHS CT和WHS++数据集上对比标准3D U-Net。

Result: 标准3D U-Net在实验中保持强基线表现，手工先验带来的改进有限且不稳定，有时反而降低性能。

Conclusion: 基线模型已隐式捕捉大量解剖规律，未来提升需要更具表达力的学习型先验而非简单手工约束。
...
### [268] [Video Quality Evaluation Methodology and Result of AV2 Compression Performance](https://arxiv.org/abs/2605.15800)
*Zhijun Lei,Vibhoothi Vibhoothi,Dzung Hoang,Yixin Du,Ramzi Khsib*

Main category: eess.IV

TL;DR: 本文介绍了AV2视频编码标准的评估方法，并报告了其相对于AV1在压缩效率上的显著提升。


<details>
  <summary>Details</summary>
Motivation: 为了超越AV1，提供更高的压缩效率，满足多样化媒体应用的需求。

Method: 定义了AV2通用测试条件，引入凸包自适应流配置、用户生成内容和扩展色度格式等新评估方法。

Result: AV2在随机访问配置下，PSNR-YUV和VMAF的BD-rate分别降低29.81%和33.79%。

Conclusion: AV2有效提升了压缩效率，适用于下一代流媒体应用。
...
### [269] [Layer Selection in Feature-Based Losses Affects Image Quality and Microstructural Consistency in Deep Learning Super-Resolution of Brain Diffusion MRI](https://arxiv.org/abs/2605.15895)
*David Lohr,Rene Werner*

Main category: eess.IV

TL;DR: 研究发现，在扩散MRI超分辨率中，基于特征的损失函数中过深VGG16层会导致网格伪影，而浅层能保持信号一致性。


<details>
  <summary>Details</summary>
Motivation: 高分辨率扩散MRI的临床应用受限于硬件和扫描时间，因此需要计算超分辨率方法。

Method: 利用HCP 7T数据生成高低分辨率DWI对，训练UNet进行2D超分辨率，通过消融和隔离实验评估不同VGG16层的特征损失与L1基线对比。

Result: 深层VGG16层及其组合在超分辨率DWI中产生网格伪影，并延续到扩散参数；浅层无伪影，且在9倍超分辨率下与真实值高度一致。

Conclusion: 图像SNR和VGG16层深度影响伪影表现，需谨慎选择贡献层以适用于扩散MRI及其他领域。
