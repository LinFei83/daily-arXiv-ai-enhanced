...
TL;DR: 本文提出一个由大型语言模型驱动的对话式助手，旨在帮助中小型企业（SMEs）将车间中基于经验的隐性知识，通过交互式对话捕获并转化为符合BPMN 2.0标准的业务流程图。


<details>
  <summary>Details</summary>
Motivation: 中小型企业严重依赖难以被正式记录的隐性、经验性知识。这种知识的缺乏正式文档化阻碍了机构知识的保存、运营透明度的提升以及持续改进的加速。

Method: 研究开发了一个由Gemini 2.5 Pro驱动的对话式助手，通过轻量级Gradio前端和客户端bpmn-js可视化工具实现。该助手通过访谈式对话，逐步且交互式地获取流程细节，支持澄清和按需分析，并实时渲染可供用户修改的BPMN 2.0图表。

Result: 在一个设备维护场景的概念验证评估中，该聊天机器人成功在约12分钟内生成了准确的“现状”（AS-IS）模型，通过图表注释标记了问题，并生成了改进的“目标”（TO-BE）变体，同时API成本维持在中小企业可承受的预算内。研究还分析了延迟来源、模型选择权衡以及强制执行严格XML模式的挑战。

Conclusion: 结果表明，对话式大型语言模型有望降低严格流程文档化的技能和成本障碍，从而帮助中小型企业保存机构知识，提高运营透明度，并加速持续改进工作。
...
TL;DR: 本文提出了两种新的无监督指标（语义忠实度SF和语义熵产生SEP），利用信息论和热力学原理评估大型语言模型（LLM）的忠实度，并可用于幻觉控制。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型（LLM）对给定任务的忠实度是一个复杂的挑战。

Method: 将LLM视为双向信息引擎，隐藏层充当麦克斯韦妖。将问题-上下文-答案（QCA）三元组建模为共享主题上的概率分布。上下文到问题（Q）和答案（A）的主题转换分别建模为转换矩阵Q和A。语义忠实度（SF）指标通过这些矩阵之间的Kullback-Leibler（KL）散度量化忠实度，并通过凸优化同时推断出两个矩阵，最终将最小散度映射到[0,1]区间。此外，提出了基于热力学的语义熵产生（SEP）指标来衡量答案生成过程。

Result: SF指标可以量化任何给定QCA三元组的忠实度，得分越高表示忠实度越高。高忠实度通常意味着低熵产生。SF和SEP指标可以联合或单独用于LLM评估和幻觉控制。该框架已在LLM对公司SEC 10-K文件的摘要任务中得到验证。

Conclusion: 所提出的语义忠实度（SF）和语义熵产生（SEP）指标是评估LLM忠实度和控制幻觉的有效无监督工具，可以单独或联合使用。
...
TL;DR: 本文提出了一种创新的教学方法，系统地将传统机器学习技术与现代大型语言模型（LLMs）相结合，用于人工智能和数据科学教学。


<details>
  <summary>Details</summary>
Motivation: 研究旨在帮助学生全面理解人工智能的演进，并培养他们使用现有和前沿技术的实践技能，以满足行业对快速发展的人工智能领域的需求。

Method: 该课程分为两个互补的部分：基础机器学习概念和当代LLM应用。论文详细描述了课程架构、实施策略、评估方法以及为期两个七周学期的夏季课程的学习成果。

Result: 研究结果表明，这种整合方法显著增强了学生对人工智能全貌的理解，并更好地为他们应对快速发展的人工智能领域的行业需求做好了准备。

Conclusion: 所提出的结合传统机器学习与现代LLM的教学方法是有效的，能够提升学生对AI的理解，并更好地为他们未来的职业生涯做好准备。
...
TL;DR: 本文基于特定AGI定义，形式化证明任何算法（包括AI）都无法真正创造出自身初始算法中不存在的新功能，因此无法实现真正的创造力。


<details>
  <summary>Details</summary>
Motivation: 近年来人工智能的快速发展引发了对“人工通用智能”（AGI）何时能达到的疑问。研究旨在定义AI以及任何机器可计算过程的上限，并为此首先需要精确定义AGI。

Method: 借用先前对AGI的定义，即“在某个研究领域中以解锁该领域新的、以前未知的功能能力的方式进行创造和创新”。基于此定义，作者形式化证明了计算的极限。

Result: 研究正式证明，任何算法都无法展示其初始算法中未包含的新功能。因此，任何算法（包括AI模型）都无法在任何领域展现真正的创造力。AI模型只能展示现有功能，以及现有功能的组合和排列。

Conclusion: 该证明对未来AI发展以及人类智能的起源具有重要意义。AI无法实现真正的创造力，其能力限于现有功能的再组合和扩展。
...
TL;DR: 本文提出并证实，可能性理论（特别是Bychkovs的公理化方法）是解决Dempster-Shafer理论（DST）悖论的根本方案，它为不确定性处理提供了逻辑一致且数学严谨的基础，而非仅仅修复DST规则。


<details>
  <summary>Details</summary>
Motivation: 解决DST在处理不确定性时面临的危机和悖论，并建立一个逻辑一致且数学严谨的不确定性处理基础。旨在证明可能性理论是DST悖论的根本解决方案，而非仅仅是一种替代方案。

Method: 采用可能性理论中的对偶机制（可能性测度与必要性测度）构建不确定性处理基础；对概率论、证据理论（DST）和可能性理论三种范式进行比较分析；使用经典的医学诊断困境作为案例进行演示。

Result: 可能性理论能够正确处理矛盾数据，避免DST的逻辑陷阱；它使形式推理更接近自然智能的逻辑；并最终证明了可能性理论为DST悖论提供了根本性解决方案。

Conclusion: 可能性理论，特别是Bychkovs的公理化方法，为解决DST悖论提供了一个根本且逻辑严谨的解决方案，能够有效处理矛盾数据，并使形式推理更符合自然智能的逻辑。
...
TL;DR: 论文主张AI应追求“协同改进”（人机协作）而非“自我改进”，以期更快、更安全地实现协同超级智能。


<details>
  <summary>Details</summary>
Motivation: AI的自我改进充满危险且耗时，需要一种更安全、更可实现的路径来发展先进AI。

Method: 专注于提升AI系统与人类研究人员在AI研究（从构思到实验）全过程中的协作能力。

Result: 加速AI研究，并通过人机共生赋予AI和人类更安全的超级智能。

Conclusion: 最大化协同改进（人机协作）是比AI自我改进更好、更可实现的目标，能更快、更安全地发展超级智能。
...
TL;DR: 本文提出了一种名为MCP-AI的创新架构，它结合了模型上下文协议（MCP）与临床应用，旨在解决医疗AI在上下文推理、长期状态管理和人类可验证工作流方面的挑战，实现可解释、可组合且安全的医疗决策。


<details>
  <summary>Details</summary>
Motivation: 当前的医疗AI系统在整合上下文推理、长期状态管理和人类可验证工作流方面面临挑战。随着医疗系统日益复杂，迫切需要能够进行自主、上下文感知临床推理的框架，以超越传统CDSS和基于提示的LLM的局限性。

Method: MCP-AI是一种新颖的、可解释的医疗决策架构，它建立在模型上下文协议（MCP）之上。MCP是一个模块化、可执行的规范，用于实时协调生成式和描述式AI代理。每个MCP文件捕获临床目标、患者上下文、推理状态和任务逻辑，形成可重用和可审计的记忆对象。该系统支持自适应、纵向和协作推理，并能连接HL7/FHIR接口。

Result: MCP-AI通过两个用例进行了验证：(1) 脆性X综合征伴抑郁症的诊断建模，以及 (2) 2型糖尿病和高血压的远程协调。在两种情况下，该协议都促进了医生在环验证，简化了临床流程，并确保了AI职责在医疗提供者之间的安全过渡。系统符合HIPAA和FDA SaMD等监管标准。

Conclusion: MCP-AI为未来临床环境中可解释、可组合和安全导向的AI提供了一个可扩展的基础，代表了医疗AI领域的一个重大转变。
...
TL;DR: ChipMind是一个知识图谱增强的推理框架，通过构建领域特定的电路知识图谱ChipKG，并结合自适应检索和语义过滤机制，有效解决了大型语言模型（LLM）在处理冗长集成电路（IC）规范时上下文窗口受限和推理困难的问题，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在自动化集成电路（IC）开发方面潜力巨大，但其应用受限于上下文窗口。现有上下文扩展方法难以对复杂、冗长的电路规范进行有效的语义建模和多跳推理。

Method: 本文提出了ChipMind框架。首先，通过“电路语义感知知识图谱构建”方法将电路规范转换为领域特定的知识图谱ChipKG。其次，采用ChipKG增强推理机制，该机制结合了信息论自适应检索以动态追踪逻辑依赖，以及意图感知语义过滤以剪除无关噪声，旨在平衡检索的完整性和精确性。

Result: 在工业级规范推理基准测试中，ChipMind显著优于现有最先进的基线方法，平均性能提升34.59%，最高可达72.73%。

Conclusion: ChipMind框架弥补了LLM辅助硬件设计（LAD）在学术研究与实际工业部署之间的关键鸿沟。
...
TL;DR: BEAVER是一个实用的框架，用于计算大型语言模型（LLM）输出满足约束的确定性、可靠的概率边界，优于基于采样的估计方法。


<details>
  <summary>Details</summary>
Motivation: 随着LLM从研究原型转向生产系统，从业者需要可靠的方法来验证模型输出是否满足所需约束。现有的基于采样的估计方法只能提供直观感受，缺乏可靠的保证。

Method: BEAVER针对任意前缀封闭语义约束，通过使用新颖的token trie和frontier数据结构系统地探索生成空间，并在每次迭代中维护可证明的可靠边界。该方法形式化了验证问题，并证明了其可靠性。

Result: 在相同的计算预算下，BEAVER在正确性验证、隐私验证和安全代码生成任务上，对多个最先进的LLM实现了比基线方法紧密6到8倍的概率边界，并识别出3到4倍的高风险实例。

Conclusion: BEAVER能够提供精确的LLM约束满足表征和风险评估，这是宽松边界或经验评估无法提供的，从而实现了更可靠的模型部署。
...
### [10] [MIND: Multi-rationale INtegrated Discriminative Reasoning Framework for Multi-modal Large Models](https://arxiv.org/abs/2512.05530)
*Chuang Yu,Jinmiao Zhao,Mingxuan Zhao,Yunpeng Liu,Xiujun Shu,Yuanhao Feng,Bo Wang,Xiangyu Yue*

Main category: cs.AI

TL;DR: 多模态大语言模型（MLLMs）在推理任务中存在多理由语义建模不足、逻辑鲁棒性差等问题。本文提出了MIND推理框架，通过“理解->反思->纠正”的认知能力，结合理由增强、两阶段纠正学习和多理由对比对齐策略，显著提升了MLLMs的主动判别推理能力，并在多个公开数据集上达到了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在推理任务中面临多理由语义建模有限、逻辑鲁棒性不足以及在复杂场景中易受误导解释影响的问题。

Method: 本文提出了多理由集成判别（MIND）推理框架，旨在赋予MLLMs“理解->反思->纠正”的人类认知能力。具体方法包括：1) 引入理由增强与判别（RAD）范式，自动高效地生成多样化理由以扩展数据集；2) 设计渐进式两阶段纠正学习（P2CL）策略，第一阶段增强多理由正向学习，第二阶段实现主动逻辑判别与纠正；3) 提出多理由对比对齐（MCA）优化策略，以缓解多理由语义空间中的表示纠缠，实现正确推理的语义聚合和错误推理的边界分离。

Result: 实验结果表明，所提出的MIND推理框架在涵盖科学、常识和数学场景的多个公共数据集上均取得了最先进（SOTA）的性能。

Conclusion: MIND推理框架为推动多模态大语言模型向更高水平的认知智能发展提供了新的视角。
...
TL;DR: 本文将大型语言模型（LLM）中“知行不一”的现象定义为“意志薄弱”（akrasia），并提出了Akrasia基准来量化评估模型的自我控制能力，探讨了其在单智能体和多智能体系统中的影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型表现出一种特殊的矛盾：它们“知道”正确的答案，但未能据此行动。这种“知行不一”在人类哲学中被称为“意志薄弱”或“自制力不足”。研究旨在为分析智能体AI系统中的不一致性和目标漂移提供一个基础概念。

Method: 研究提出将“意志薄弱”作为分析智能体AI系统不一致性的核心概念。为此，引入了Akrasia基准的初步版本，该基准是一组结构化的提示条件（基线、同义词、时间、诱惑），用于衡量模型局部响应与其先前承诺相矛盾的情况。此外，还探讨了微观层面的“意志薄弱”如何累积为多智能体系统中的宏观不稳定。

Result: Akrasia基准能够对不同模型家族、解码策略和诱惑类型之间的“自我控制”进行定量比较。研究还概述了微观层面的“意志薄弱”可能如何复合导致多智能体系统中的宏观不稳定，这可能被解释为“密谋”或故意的失调。

Conclusion: 通过将不一致性重新定义为“意志薄弱”，这项工作将智能体行为与古典代理理论联系起来，并在哲学、心理学和新兴的智能体AI科学之间搭建了一座经验桥梁。
...
### [16] [Evolutionary System 2 Reasoning: An Empirical Proof](https://arxiv.org/abs/2512.05760)
*Zeyuan Ma,Wenqi Huang,Guo-Huan Song,Hongshu Guo,Sijie Ma,Zhiguang Cao,Yue-Jiao Gong*

Main category: cs.AI

TL;DR: 本文提出进化推理优化（ERO）框架，通过对LLM群体进行“适者生存”的进化策略，旨在提升其通用推理能力。研究发现，即使是先进的LLM（如GPT-5）也缺乏系统2推理能力，但通过ERO，一个相对较弱的模型（Qwen-7B）也能显著提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型（LLMs）在特定任务上表现出色，但在通用智能，特别是系统2推理（慢思考）方面仍有不足。研究旨在探讨机器智能，如LLMs，是否能像人类一样进化出通用的推理能力，而非仅仅是特定技能。

Method: 提出进化推理优化（ERO）框架。该框架首先初始化一个由多个LLM组成的种群，然后采用进化策略，通过“适者生存”的原则，不断演化种群，以最大化最佳个体的量化推理分数。

Result: 实验在代表性测试集上揭示了两个主要发现：i) 最新的LLM（如GPT-5）在系统2推理能力方面仍然有限；ii) 通过简单的ERO进化循环，一个相对较弱的模型（Qwen-7B）能够被增强，并展现出强大的推理能力。

Conclusion: 研究表明，通过进化推理优化（ERO）框架，可以有效地提升LLMs的通用推理能力，即使是基础模型也能通过这种方式获得显著增强。同时，也指出当前最先进的LLMs在高级推理方面仍有待发展。
...
TL;DR: 本文认为大型语言模型（LLMs）并非通用人工智能（AGI）的死胡同，关键在于缺乏一个协调性的System-2层。通过提出语义锚定理论UCCT和架构MACI，可以实现LLMs的推理和规划能力。


<details>
  <summary>Details</summary>
Motivation: 主流批评认为LLMs仅是“模式匹配器”，结构上无法进行推理或规划，从而断定它们是AGI的死胡同。本文旨在反驳这一观点，指出瓶颈不在于LLMs本身，而是缺少一个协调层。

Method: 提出UCCT（语义锚定理论）来形式化System-2协调层，将推理建模为一个由有效支持、表征不匹配和自适应锚定预算控制的相变。将UCCT转化为MACI架构，该架构是一个协调堆栈，实现了诱导（行为调节辩论）、过滤（苏格拉底式判断）和持久性（事务性内存）。

Result: 在UCCT视角下，无根据的生成是基底最大似然先验的检索，而“推理”则在锚点将后验概率转向目标导向约束时出现。MACI架构通过实现诱导、过滤和持久性来处理协调失败。

Conclusion: 通过将常见异议重新定义为可测试的协调失败，本文认为通向AGI的道路是穿过LLMs，而非绕开它们，即通过增强LLMs的协调能力来达到AGI。
...
TL;DR: 本研究引入了一种多模态肿瘤智能体 (MOA)，它结合了基于TITAN基础模型的组织学工具和对结构化临床与基因组数据的推理，以准确预测低级别胶质瘤中的IDH1突变。


<details>
  <summary>Details</summary>
Motivation: 低级别胶质瘤中IDH1突变常见，并具有独特的预后和治疗意义。因此，准确预测IDH1突变对于临床决策至关重要。

Method: 本研究开发了一个多模态肿瘤智能体 (MOA)。该智能体整合了一个基于TITAN基础模型的组织学工具，用于IDH1突变预测，并结合了通过PubMed、Google Search和OncoKB等外部生物医学来源对结构化临床和基因组输入进行推理。MOA在TCGA-LGG队列的488名患者数据上进行了定量评估。

Result: MOA（不含组织学工具）的F1分数达到0.826，优于临床基线（0.798）。当与组织学特征融合时，MOA表现最佳，F1分数达到0.912，超过了组织学基线（0.894）和融合组织学-临床基线（0.897）。

Conclusion: 研究结果表明，所提出的智能体能够捕获通过外部生物医学来源丰富补充的突变相关信息，从而实现IDH1突变的准确预测。
...
TL;DR: 本研究提出了一种利用大型语言模型（LLMs）从治疗对话记录中自动生成个性化客户心理网络的方法，以支持案例概念化和治疗计划，并获得了专家的高度认可。


<details>
  <summary>Details</summary>
Motivation: 心理治疗的个性化（例如基于网络的治疗模块选择）需要大量的纵向数据，这在实践中往往难以实现。研究旨在利用LLMs提供一个可扩展的解决方案，以促进网络驱动的治疗个性化。

Method: 研究开发了一个端到端流水线：首先，从77份治疗对话记录中标注了3364个心理过程及其维度；其次，使用上下文学习（in-context learning）共同识别心理过程和维度，即使在少量训练样本下也表现出色；再次，引入两步法将这些过程聚类成具有临床意义的群组；最后，生成了群组之间带有解释的关系。研究将该多步方法与直接提示法进行比较，并由专家进行评估。

Result: 上下文学习在识别心理过程和维度方面表现出色。专家普遍认为，本研究的多步方法生成的网络在临床实用性和可解释性方面优于直接提示法（高达90%的专家偏好）。专家对所生成网络的临床相关性、新颖性和实用性评分高达72-75%。研究证明了使用LLMs从治疗对话记录中创建临床相关网络的可行性，其优势包括从客户话语中自下而上地进行案例概念化和识别潜在主题。

Conclusion: 本研究为使用LLMs从治疗对话记录中生成临床相关网络提供了概念验证。这些网络可用于临床环境、督导和培训。未来的研究应进一步探讨这些网络是否能比其他个性化方法（包括统计估计网络）更有效地改善治疗结果。
...
TL;DR: 本研究发现已发表的AI论文中存在不可忽视的客观错误，且平均错误数量随时间增加。研究开发了一种基于GPT-5的AI工具来系统性地检测和纠正这些错误，并证实了其有效性。


<details>
  <summary>Details</summary>
Motivation: 同行评审的出版物是新研究和知识的基础，但其中存在的错误可能传播并导致后续研究的混淆，影响可复现性。研究旨在解决这一问题，尤其是在研究节奏加快和同行评审系统压力增大的背景下。

Method: 研究开发了一个基于GPT-5的“论文正确性检查器”，用于系统性地识别顶级AI会议和期刊论文中的客观错误（例如，公式、推导、计算、图表中的错误）。人类专家对AI检查器识别的潜在错误进行了审查，以验证其准确性。

Result: 研究发现已发表论文包含不可忽视的客观错误，且每篇论文的平均错误数量随时间增加（例如，NeurIPS从2021年的3.8个增加到2025年的5.9个，增长55.3%）。人类专家审查了316个潜在错误，确认其中263个是实际错误，精确度为83.2%。AI检查器还能为75.8%的已识别错误提出正确的修正方案。

Conclusion: 本研究强调了前沿大型语言模型（LLMs）在检测和纠正已发表论文中客观错误的潜力，有助于为知识建立更坚实的基础，减少文献中的混淆并增强可复现性。
...
TL;DR: 本文介绍了PRiSM，一个动态、多模态的合成基准，用于通过Python代码评估视觉-语言模型（VLMs）在大学水平的科学（数学和物理）推理能力，旨在解决现有基准在概念理解、符号推理和科学正确性验证方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准在评估VLMs在数学和物理等科学领域的表现时存在局限性，它们通常只关注最终答案预测，缺乏对概念理解、符号推理、遵循形式化定律以及中间推理步骤的考量。此外，现有数据集静态、缺乏鲁棒性，也无法验证科学正确性。

Method: 研究者引入了PRiSM，一个合成、完全动态且多模态的基准，通过基于Python代码的验证来评估科学推理。PRiSM包含超过24,750个大学水平的物理和数学问题，并利用可扩展的代理管道PrismAgent生成结构良好的问题实例。每个问题都包含动态的文本和视觉输入、生成的图形以及丰富的结构化输出，包括可执行的Python代码用于真值生成和验证，以及详细的逐步推理。此外，他们提出了涵盖泛化、符号程序合成、扰动鲁棒性、推理纠正和歧义消解的五项评估任务。

Result: PRiSM的动态性和Python驱动的自动化真值生成允许对多模态VLMs进行细粒度的实验审计，从而揭示了VLMs在科学推理中的故障模式、不确定性行为和局限性。通过对现有VLMs的全面评估，本文突出了它们的不足，并展示了PRiSM如何能更深入地洞察其科学推理能力。

Conclusion: PRiSM基准通过其动态性、多模态输入和基于Python代码的真值验证，为评估和理解VLMs在科学领域的推理能力提供了更深入、更全面的工具，揭示了现有模型的关键局限性。
...
TL;DR: TRACE是一个用于透明推理和一致性评估的框架，它利用辅助推理集（ARS）分解复杂问题，通过一致性指标评估中间步骤，诊断大型视觉语言模型中的推理轨迹和错误，而非仅仅依赖最终答案。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在可靠的数学和科学推理方面仍面临挑战。标准的最终答案评估常常掩盖推理错误，导致潜在的失败持续存在而未被发现。

Method: 本文引入了TRACE框架，用于透明推理和一致性评估。其核心是辅助推理集（ARS），这些紧凑的子问题-答案对能够分解复杂问题，通过基于一致性的指标评估中间步骤，并揭示标准评估所忽视的错误。

Result: 实验表明，ARS之间的一致性与最终答案的正确性相关，有助于精确定位出现故障的推理步骤，为模型改进提供可操作的信号。此外，TRACE定义了置信区域，可以区分可靠和不可靠的推理路径，支持有效的过滤、调试和模型优化。

Conclusion: TRACE框架通过诊断推理轨迹和中间步骤，提供了一种更透明、更具诊断性的方法来评估大型视觉语言模型的推理能力，从而揭示隐藏的错误并为模型改进提供具体指导。
...
### [24] [SymPyBench: A Dynamic Benchmark for Scientific Reasoning with Executable Python Code](https://arxiv.org/abs/2512.05954)
*Shima Imani,Seungwhan Moon,Adel Ahmadyan,Lu Zhang,Kirmani Ahmed,Babak Damavandi*

Main category: cs.AI

TL;DR: 本文介绍了一个名为 SymPyBench 的大规模合成基准测试集，包含15,045个大学物理问题，每个问题都可参数化并附带详细推理和可执行代码。它引入了新的评估指标，并用于测试现有大型语言模型在科学推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试可能无法充分评估语言模型在科学推理方面的鲁棒性和可解释性。研究旨在创建一个动态、可参数化、具有详细推理过程和可执行解决方案的基准，以推动更强大的推理系统发展。

Method: 研究构建了 SymPyBench，一个包含15,045个大学物理问题的合成基准测试集。问题是完全参数化的，提供无限的输入配置，并附带结构化的分步推理和生成真值解的 Python 代码。基准包含三种问题类型：符号多选、数值多选和自由形式。此外，还引入了三个新颖的评估指标：一致性得分 (Consistency Score)、失败率 (Failure Rate) 和混淆率 (Confusion Rate)，以量化问题变体之间的变异性和不确定性。

Result: 通过对最先进的指令微调语言模型进行实验，SymPyBench 揭示了这些模型在科学推理方面的优点和局限性。它被定位为开发更鲁棒和可解释推理系统的基础。

Conclusion: SymPyBench 提供了一个动态、代码驱动的大规模基准测试集，能够全面评估和促进语言模型在科学推理方面的发展，有助于构建更可靠和可解释的人工智能推理系统。
...
### [25] [Variational Quantum Rainbow Deep Q-Network for Optimizing Resource Allocation Problem](https://arxiv.org/abs/2512.05946)
...
### [26] [AREA3D: Active Reconstruction Agent with Unified Feed-Forward 3D Perception and Vision-Language Guidance](https://arxiv.org/abs/2512.05131)
*Tianling Xu,Shengzhe Gan,Leslie Gu,Yuelei Li,Fangneng Zhan,Hanspeter Pfister*

Main category: cs.CV

TL;DR: AREA3D是一种主动式三维重建智能体，它结合了前馈三维重建模型和视觉-语言指导，以高效、准确地获取场景几何，解决了现有方法依赖手动几何启发式导致的冗余观测问题。


<details>
  <summary>Details</summary>
Motivation: 现有的主动式三维重建方法通常依赖手工设计的几何启发式，这可能导致冗余观测，且未能显著提高重建质量，因此需要一种更有效的方法来选择视点。

Method: 本研究提出了AREA3D框架。它将视点不确定性建模与底层前馈重建器解耦，从而实现精确的不确定性估计，无需昂贵的在线优化。此外，一个集成的视觉-语言模型提供了高级语义指导，鼓励选择超越纯几何线索的信息丰富且多样化的视点。

Result: 在场景级和对象级基准测试中，AREA3D均取得了最先进的重建精度，尤其是在稀疏视图条件下表现突出。

Conclusion: AREA3D通过结合精确的不确定性估计和语义指导，克服了现有主动重建方法的局限性，实现了更高效、更准确的三维场景重建。
...
### [27] [Breaking Scale Anchoring: Frequency Representation Learning for Accurate High-Resolution Inference from Low-Resolution Training](https://arxiv.org/abs/2512.05132)
*Wenshuo Wang,Fan Zhang*

Main category: cs.CV

TL;DR: 零样本超分辨率时空预测（ZSSRSF）存在“尺度锚定”问题，即模型在更高分辨率下误差无法降低。本文提出频率表示学习（FRL），通过对齐频率表示和频谱一致性训练，有效缓解了尺度锚定，使误差随分辨率增加而减少。


<details>
  <summary>Details</summary>
Motivation: 现有零样本超分辨率时空预测模型在从低分辨率训练到高分辨率推理时，误差并不会随分辨率提高而降低，这与作为数值求解器的深度学习模型的期望相悖。根本原因是低分辨率数据的奈奎斯特频率限制了模型处理高频信号的能力，导致误差被“锚定”在低分辨率水平，被错误地解释为成功的泛化。本文将此现象定义为“尺度锚定”问题。

Method: 提出与架构无关的“频率表示学习（FRL）”方法。该方法通过分辨率对齐的频率表示和频谱一致性训练来缓解尺度锚定问题。

Result: 在具有更高奈奎斯特频率的网格上，FRL增强变体在高频带中的频率响应更稳定。这使得误差能够随分辨率的增加而降低，并且在任务和分辨率范围内显著优于基线模型，同时只引入了适度的计算开销。

Conclusion: “尺度锚定”是零样本超分辨率时空预测中的一个基本问题。频率表示学习（FRL）通过使模型能够在分辨率增加时减少误差，有效解决了这一问题，展现了更好的多分辨率泛化能力。
...
### [28] [InvarDiff: Cross-Scale Invariance Caching for Accelerated Diffusion Models](https://arxiv.org/abs/2512.05134)
*Zihao Wu*

Main category: cs.CV

TL;DR: InvarDiff是一种无需训练的加速方法，通过利用确定性采样中跨时间步和层级的特征不变性，实现扩散模型2-3倍的端到端加速，同时保持高保真度。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高保真图像，但由于迭代采样过程，推理速度较慢。

Method: InvarDiff通过少量确定性运行，观察到确定性采样中存在特征不变性。它计算一个逐时间步、逐层、逐模块的二进制缓存计划矩阵，并使用重采样校正来避免连续缓存时的漂移。该矩阵基于分位数变化指标，指定何时重用模块或整个时间步的缓存结果。在推理时，InvarDiff根据此矩阵进行先时间步后逐层的缓存。

Result: 在DiT和FLUX模型上，InvarDiff实现了2-3倍的端到端加速，对标准质量指标影响最小，并且在视觉质量上几乎没有退化。

Conclusion: InvarDiff通过利用扩散模型确定性采样中的特征不变性，有效减少了冗余计算，显著提升了推理速度，同时保持了生成图像的高质量。
...
### [29] [Fine-tuning an ECG Foundation Model to Predict Coronary CT Angiography Outcomes](https://arxiv.org/abs/2512.05136)
...
TL;DR: 本研究开发了一种可解释的AI-ECG模型，用于预测冠状动脉（RCA, LM, LAD, LCX）的严重或完全狭窄，并在内部和外部验证集上表现出稳定的性能，为CAD筛查提供了一种有前景的替代方案。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病（CAD）是全球主要的健康负担。准确识别病变血管和评估狭窄严重程度对个体化治疗至关重要。虽然冠状动脉CT血管造影（CCTA）是诊断CAD的一线非侵入性方法，但其对高端设备、辐射暴露和严格患者配合的依赖限制了大规模应用。随着人工智能（AI）的进步和心电图（ECG）的广泛可用性，AI-ECG为CAD筛查提供了一个有前景的替代方案。

Method: 研究开发了一个可解释的AI-ECG模型，用于预测CCTA上四条主要冠状动脉（右冠状动脉、左主干、左前降支和左回旋支）的严重或完全狭窄。模型在内部和外部验证集上进行了性能评估，并对正常ECG子集、不同人口统计学和采集时间分层进行了稳定性分析。此外，还进行了基于血管特异性发病率阈值的风险分层和可解释性分析。

Result: 在内部验证集上，模型对RCA、LM、LAD和LCX的AUC分别为0.794、0.818、0.744和0.755；在外部验证集上，AUC分别达到0.749、0.971、0.667和0.727。在临床正常ECG子集中性能保持稳定，表明其对明显ECG异常之外的鲁棒性。跨人口统计学和采集时间分层的亚组分析进一步证实了模型的稳定性。基于血管特异性发病率阈值的风险分层在校准和累积事件曲线上显示出一致的分离。可解释性分析揭示了高风险组和低风险组之间独特的波形差异，突出了对模型决策有贡献的关键电生理区域。

Conclusion: 本研究开发的AI-ECG模型可以预测主要冠状动脉的严重或完全狭窄，在各种临床场景中表现出稳定且鲁棒的性能，即使在ECG正常的情况下也有效。该模型为CAD筛查提供了一个有前景的非侵入性替代方案，并为冠状动脉狭窄的ECG相关性提供了新的见解。
...
### [30] [ChromouVQA: Benchmarking Vision-Language Models under Chromatic Camouflaged Images](https://arxiv.org/abs/2512.05137)
*Yunfei Zhang,Yizhuo He,Yuanxun Shao,Zhengtao Yao,Haoyan Xu,Junhao Dong,Zhen Yao,Zhikang Dong*

Main category: cs.CV

TL;DR: 视觉-语言模型（VLMs）在杂乱背景下的前景-背景分离方面表现不佳。本文引入了ChromouVQA，一个基于石原式色盲图的大规模多任务基准，以评估VLMs的这一能力。人类和VLMs的评估显示出巨大差距，尤其是在细微色差或干扰性几何填充下。研究还提出了一种模型无关的对比学习方法，以改善全局形状的恢复。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言模型（VLMs）在多模态理解方面取得了进展，但当目标嵌入在需要前景-背景分离的杂乱背景中时，它们仍然面临挑战。研究旨在解决VLM在色度伪装图像中进行前景-背景分割的困难，并为此提供一个可控、可复现的评估基准。

Method: 引入了ChromouVQA，一个大规模、多任务的基准，其图像基于石原式色度伪装图。该基准扩展了经典的圆点板，加入了多种填充几何形状，并改变了色度分离、密度、大小、遮挡和旋转等参数，同时记录了完整的元数据。ChromouVQA涵盖了九种视觉问答（VQA）任务，包括识别、计数、比较和空间推理。研究对人类和VLMs进行了评估，并提出了一种模型无关的对比学习方法，通过对齐轮廓与其伪装渲染图来改善全局形状的恢复。

Result: 人类和VLMs的评估结果显示出巨大的性能差距，尤其是在细微色度对比或干扰性几何填充的条件下。提出的模型无关的对比学习方法能够改善模型对全局形状的恢复能力。

Conclusion: ChromouVQA提供了一个紧凑、受控的基准，用于可复现地评估和扩展视觉-语言模型在前景-背景分离方面的能力。该基准揭示了当前VLM在此类任务上的局限性，并提出了一种潜在的改进方法。
...
### [31] [Spatiotemporal Satellite Image Downscaling with Transfer Encoders and Autoregressive Generative Models](https://arxiv.org/abs/2512.05139)
*Yang Xiang,Jingwen Zhong,Yige Yan,Petros Koutrakis,Eric Garshick,Meredith Franklin*

Main category: cs.CV

TL;DR: 本文提出了一种结合U-Net迁移编码器和扩散模型的生成式降尺度框架，用于从粗尺度输入重建精细分辨率卫星图像。该方法通过在长时间序列粗分辨率数据上预训练U-Net，然后将其编码器冻结并迁移到降尺度模型中，实现了卓越的性能，并保持了物理一致性，优于现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 研究旨在从粗尺度输入重建精细分辨率卫星图像，以改进环境暴露评估和长期环境监测。面临的挑战是如何在有限的训练周期内，为长时间序列的粗分辨率图像降尺度提供一个稳健且物理上连贯的解决方案。

Method: 该方法采用一个迁移学习生成式降尺度框架，结合了轻量级U-Net迁移编码器和基于扩散的生成模型。U-Net首先在NASA MERRA-2（50公里）等粗分辨率数据的长时间序列上进行预训练，以学习时空表示。随后，其编码器被冻结并作为具有物理意义的潜在特征迁移到一个更大的降尺度模型中，目标是重建GEOS-5 Nature Run (G5NR)（7公里）的精细分辨率图像。研究区域为亚洲，并被划分为子区域和季节以提高计算效率。通过Wasserstein距离进行域相似性分析，确认了MERRA-2和G5NR之间的最小分布偏移，验证了参数冻结迁移的安全性。模型性能通过R2、半变异函数、ACF/PACF以及基于滞后的RMSE/R2进行评估，并与确定性U-Net、变分自编码器和先前的迁移学习基线模型进行了比较。

Result: 该模型在跨季节区域划分中取得了出色的性能（R2 = 0.65至0.94），并优于包括确定性U-Net、变分自编码器和先前迁移学习基线在内的对比模型。通过数据外评估，发现预测的降尺度图像保留了物理上一致的空间变异性和时间自相关性，从而实现了G5NR记录之外的稳定自回归重建。

Conclusion: 研究结果表明，迁移增强的扩散模型为在有限训练周期内对长时间序列粗分辨率图像进行降尺度提供了一个稳健且物理上连贯的解决方案。这一进展对于改善环境暴露评估和长期环境监测具有重要意义。
...
### [32] [Self-Improving VLM Judges Without Human Annotations](https://arxiv.org/abs/2512.05145)
*Inna Wanyin Lin,Yushi Hu,Shuyue Stella Li,Scott Geng,Pang Wei Koh,Luke Zettlemoyer,Tim Althoff,Marjan Ghazvininejad*

Main category: cs.CV

TL;DR: 本文提出了一种无需人工偏好标注的视觉语言模型（VLM）自训练评估器框架。该方法通过迭代自合成数据，显著提升了评估器的准确性，甚至超越了更大的模型。


<details>
  <summary>Details</summary>
Motivation: 当前训练VLM评估器的方法严重依赖大规模人工偏好标注，这不仅成本高昂，而且随着模型快速发展，标注数据很容易过时。因此，需要一种更高效、更具适应性的评估器训练方法。

Method: 该方法是迭代的，包含三个阶段：1) 生成不同质量水平的多模态指令-响应对；2) 为每个对生成推理痕迹和判断，并移除不符合预期质量水平的对；3) 基于正确的评估答案及其推理痕迹进行训练。

Result: 在Multimodal RewardBench和VL-RewardBench上的评估显示，该方法将一个Llama-3.2-11B多模态评估器在VL-RewardBench上的整体准确率从0.38提高到0.51，在通用性、幻觉和推理维度上表现尤为突出，并且常常优于Llama-3.2-90B、GPT-4o和Claude 3.5 Sonnet等更大的模型。

Conclusion: 这些无需人工标注的结果表明，未来有望实现一个能够与VLM能力快速提升同步进化的“自评估器”，从而解决当前评估器训练的成本和时效性问题。
...
### [33] [FlowEO: Generative Unsupervised Domain Adaptation for Earth Observation](https://arxiv.org/abs/2512.05140)
*Georges Le Bellier,Nicolas Audebert*

Main category: cs.CV

TL;DR: FlowEO是一个利用流匹配（flow matching）的生成模型框架，旨在通过学习语义保留映射，解决地球观测数据中图像空间无监督域适应（UDA）的挑战，从而提高遥感模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据日益丰富，但其固有的异质性（来自不同传感器、区域、时间、大气条件）导致训练和部署域之间的分布偏移，严重限制了预训练遥感模型的泛化能力。因此，无监督域适应（UDA）对于实际应用至关重要。

Method: 本文提出了FlowEO框架，利用生成模型进行地球观测图像空间UDA。具体来说，它利用流匹配技术学习一个语义保留映射，将源图像分布转换到目标图像分布。这种方法能够处理分类和语义分割等具有挑战性的域适应配置。

Result: FlowEO在域适应方面优于现有的图像转换方法，同时达到了同等或更好的感知图像质量。实验在四个数据集上进行，涵盖了SAR到光学转换、以及自然灾害引起的时序和语义偏移等适应场景。

Conclusion: 实验结果突出了基于流匹配的UDA在遥感领域具有巨大潜力，能够有效解决地球观测数据的域适应问题。
...
### [34] [EFDiT: Efficient Fine-grained Image Generation Using Diffusion Transformer Models](https://arxiv.org/abs/2512.05152)
*Kun Wang,Donglin Di,Tonghua Su,Lei Fan*

Main category: cs.CV

TL;DR: 本文提出了一种针对细粒度图像生成的扩散模型方法，通过引入分层嵌入器解决语义纠缠问题，通过感知信息生成阶段的超分辨率增强细节，并提出ProAttention机制，在性能上超越了现有先进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在细粒度图像生成中存在语义信息纠缠和生成图像细节不足的问题，尤其是在大规模细粒度类别上，现有方法主要关注常见类别。

Method: 1. 引入“分层嵌入器”概念，整合超类和子类的语义信息，以解决语义纠缠问题。 2. 在感知信息生成阶段引入超分辨率概念，通过增强和降级模型提升细粒度图像的细节特征。 3. 提出一种高效的ProAttention机制，可有效应用于扩散模型。

Result: 通过在公共基准上进行大量实验评估，证明所提出的方法在性能上优于其他最先进的微调方法。

Conclusion: 该方法有效解决了扩散模型在细粒度图像生成中面临的语义纠缠和细节不足问题，并在性能上取得了显著提升。
...
### [35] [Semore: VLM-guided Enhanced Semantic Motion Representations for Visual Reinforcement Learning](https://arxiv.org/abs/2512.05172)
*Wentao Wang,Chunyang Liu,Kehua Sheng,Bo Zhang,Yan Wang*

Main category: cs.CV

TL;DR: 本文提出Semore，一个基于VLM的视觉强化学习框架，通过双路径骨干网络同时提取语义和运动表示，并利用VLM和CLIP在特征层面指导，以提高RL效率和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的强化学习方法通常侧重于控制策略指导，并面临骨干网络表示能力有限的挑战。

Method: Semore是一个基于VLM的视觉强化学习框架，通过双路径骨干网络从RGB流中同时提取语义和运动表示。它利用VLM的常识知识从观察中检索关键信息，并使用预训练的CLIP实现文本-图像对齐，将真实表示嵌入到骨干网络中。为有效融合语义和运动表示以进行决策，该方法采用单独监督的方法，同时指导语义和运动的提取，并允许它们自发交互。

Result: 在VLM在特征层面的指导下，Semore方法与现有最先进方法相比，展现出高效和自适应的能力。

Conclusion: Semore通过引入增强的语义运动表示，显著提升了视觉强化学习的有效性和适应性。
...
### [36] [TwinFlow: Realizing One-step Generation on Large Models with Self-adversarial Flows](https://arxiv.org/abs/2512.05150)
*Zhenglin Cheng,Peng Sun,Jianguo Li,Tao Lin*

Main category: cs.CV

TL;DR: 本文提出TwinFlow，一个简单高效的1步生成模型训练框架，无需预训练教师模型或对抗网络。它在1-NFE下实现了出色的图像生成性能，甚至能匹配100-NFE模型的质量，将计算成本降低100倍。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态生成模型（如扩散和流匹配）推理效率低下（需40-100次函数评估）。现有的少步加速方法存在局限：基于蒸馏的方法要么需要迭代蒸馏，要么在极少步数下性能显著下降；整合对抗训练的方法则引入训练不稳定、复杂性和高GPU内存开销。

Method: 我们提出了TwinFlow，一个简单而有效的框架，用于训练1步生成模型。它绕过了对固定预训练教师模型的需求，并在训练过程中避免了使用标准的对抗网络，使其非常适合构建大规模、高效的模型。

Result: 在文本到图像任务中，TwinFlow在1-NFE下实现了0.83的GenEval分数，优于SANA-Sprint（基于GAN损失）和RCGM（基于一致性）。通过对Qwen-Image-20B进行全参数训练，TwinFlow展示了其可扩展性，并将其转换为高效的少步生成器。仅需1-NFE，我们的方法在GenEval和DPG-Bench基准测试上均与原始100-NFE模型性能相当，在轻微质量下降的情况下将计算成本降低了100倍。

Conclusion: TwinFlow是一个构建大规模、高效1步生成模型的理想框架，它通过避免现有加速方法的缺点，在保持性能的同时实现了显著的推理速度提升（100倍），尤其适用于高效的多模态生成任务。
...
### [37] [Your Latent Mask is Wrong: Pixel-Equivalent Latent Compositing for Diffusion Models](https://arxiv.org/abs/2512.05198)
...
### [38] [DEAR: Dataset for Evaluating the Aesthetics of RenderingDEAR: Dataset for Evaluating the Aesthetics of Rendering](https://arxiv.org/abs/2512.05209)
...
### [39] [IE2Video: Adapting Pretrained Diffusion Models for Event-Based Video Reconstruction](https://arxiv.org/abs/2512.05240)
...
### [40] [Age-Inclusive 3D Human Mesh Recovery for Action-Preserving Data Anonymization](https://arxiv.org/abs/2512.05259)
...
### [41] [CARD: Correlation Aware Restoration with Diffusion](https://arxiv.org/abs/2512.05268)
...
### [42] [Inferring Compositional 4D Scenes without Ever Seeing One](https://arxiv.org/abs/2512.05272)
...
### [43] [SpaceControl: Introducing Test-Time Spatial Control to 3D Generative Modeling](https://arxiv.org/abs/2512.05343)
*Elisabetta Fedele,Francis Engelmann,Ian Huang,Or Litany,Marc Pollefeys,Leonidas Guibas*

Main category: cs.CV

TL;DR: SpaceControl是一种无需训练的测试时方法，可为3D生成提供显式空间控制。它接受各种几何输入，与预训练模型无缝集成，并在几何保真度上超越现有方法，同时保持高视觉质量。


<details>
  <summary>Details</summary>
Motivation: 当前3D资产生成方法在对象几何体的直观和精确控制方面仍面临挑战。现有方法主要依赖文本或图像提示，但语言可能模糊，图像编辑繁琐，导致几何特异性不足。

Method: SpaceControl是一种无需训练的测试时方法，用于3D生成的显式空间控制。它接受从粗糙基元到详细网格的多种几何输入，并与现代预训练生成模型无缝集成。该方法提供一个可控参数，允许用户在几何保真度和输出真实感之间进行权衡。此外，还提出了一个交互式用户界面，支持超二次曲面的在线编辑，直接转换为带纹理的3D资产。

Result: 广泛的定量评估和用户研究表明，SpaceControl在几何保真度方面优于基于训练和基于优化的基线方法，同时保持了高视觉质量。其交互式用户界面也展示了在创意工作流程中的实际部署潜力。

Conclusion: SpaceControl成功解决了3D生成中缺乏精确几何控制的问题，提供了一种有效、无需训练且用户友好的解决方案。它在几何保真度和视觉质量之间取得了良好平衡，并能够促进在创意工作流程中直接将编辑的几何体转换为带纹理的3D资产。
...
### [44] [SplatPainter: Interactive Authoring of 3D Gaussians from 2D Edits via Test-Time Training](https://arxiv.org/abs/2512.05354)
*Yang Zheng,Hao Tan,Kai Zhang,Peng Wang,Leonidas Guibas,Gordon Wetzstein,Wang Yifan*

Main category: cs.CV

TL;DR: 本文提出了一种名为\ourmethod的状态感知前馈模型，通过用户提供的2D视图，实现3D高斯Splatting资产的交互式连续编辑，支持局部细节精修、局部涂色和全局重新着色，且速度快。


<details>
  <summary>Details</summary>
Motivation: 尽管3D高斯Splatting革新了真实感3D资产创建，但其交互式精修和编辑仍存在关键空白。现有基于扩散或优化的方法通常速度过慢、破坏原始资产特性或缺乏精细控制，不适用于此任务。

Method: 我们引入了一个状态感知的前馈模型，可以直接预测紧凑、特征丰富的高斯表示属性的更新。该方法利用测试时训练（Test-Time Training）创建了一个状态感知、迭代的工作流，从而实现从用户提供的2D视图对3D高斯资产进行连续编辑。

Result: 我们的方法具有多功能性，单一架构能以交互速度执行多种任务，包括高保真局部细节精修、局部涂色和一致的全局重新着色。

Conclusion: 该方法为流畅直观的3D内容创作铺平了道路，解决了3D高斯Splatting资产交互式编辑的现有局限性。
...
### [45] [From Segments to Scenes: Temporal Understanding in Autonomous Driving via Vision-Language Model](https://arxiv.org/abs/2512.05277)
...
TL;DR: 本文提出了TAD基准，专门用于评估视觉语言模型在自动驾驶（AD）中对时间关系的理解能力。现有模型在该基准上表现不佳，因此本文还提出了两种无需训练的解决方案（Scene-CoT和TCogMap），显著提升了模型的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管最近的视觉语言模型（VLMs）在时间理解方面有所进步，但现有基准主要关注体育、烹饪、电影等视频内容，缺乏专门针对自动驾驶（AD）中以自我为中心的独特时间理解挑战的基准。这导致当前模型在AD场景下的时间理解能力不足。

Method: 本文提出了“自动驾驶时间理解”（TAD）基准，包含近6,000个问答对和7项人工设计的任务，旨在评估VLMs捕捉AD中动作动态关系的能力。作者评估了9个闭源和开源通用模型以及最先进的AD专业模型在TAD上的表现。此外，本文还提出了两种无需训练的解决方案：利用思维链（CoT）的Scene-CoT和整合以自我为中心时间认知图的TCogMap，以提升模型在TAD上的运动理解和整体准确性。

Result: 当前最先进的模型在TAD基准上表现出不合格的准确性，主要原因在于对细粒度运动理解的不足。本文提出的Scene-CoT和TCogMap两种方法与现有VLMs结合后，将TAD上的平均准确率提高了多达17.72%。

Conclusion: 通过引入TAD基准、对多个最先进模型进行基准测试以及提出有效的增强方案，这项工作旨在推动未来在自动驾驶时间理解领域的研究。TAD基准和评估代码已公开可用。
...
### [46] [PoolNet: Deep Learning for 2D to 3D Video Process Validation](https://arxiv.org/abs/2512.05362)
*Sanchit Kaul,Joseph Luna,Shray Arora*

Main category: cs.CV

TL;DR: SfM处理耗时且数据质量差，本文提出PoolNet，一个深度学习框架，用于验证野外图像数据的SfM可用性，显著减少处理时间。


<details>
  <summary>Details</summary>
Motivation: 结构从运动（SfM）信息提取耗时且计算昂贵。此外，大多数公开数据因相机姿态不足、场景遮挡和数据噪声而不适合处理。

Method: 引入PoolNet，一个多功能的深度学习框架，用于对野外数据的帧级和场景级进行验证。

Result: 该模型成功区分了适合SfM处理的场景与不适合的场景，并显著减少了获取结构从运动数据所需的时间。

Conclusion: PoolNet有效解决了SfM数据处理中的时间和数据质量问题，提高了处理效率。
...
### [47] [Group Orthogonal Low-Rank Adaptation for RGB-T Tracking](https://arxiv.org/abs/2512.05359)
*Zekai Shao,Yufan Hu,Jingyuan Liu,Bin Fan,Hongmin Liu*

Main category: cs.CV

TL;DR: 针对RGB-T跟踪中参数高效微调的低秩适应存在的秩空间冗余问题，本文提出GOLA框架。GOLA通过量化秩重要性、冻结关键秩，并对冗余秩施加组间正交约束，有效减少参数冗余，增强特征表示能力，显著提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: RGB-T跟踪中的低秩适应（low-rank adaptation）在秩空间中存在显著冗余，许多秩几乎不提供实际信息。这限制了模型学习多样化知识以应对各种跟踪挑战的能力。

Method: 提出群正交低秩适应（Group Orthogonal Low-Rank Adaptation, GOLA）框架。该方法采用基于奇异值分解（SVD）的秩分解分区策略来量化秩的重要性，冻结关键秩以保留预训练先验知识，并将冗余秩聚类成组。进一步设计了组间正交约束策略，强制秩组之间保持正交性，促使它们学习互补特征以解决多样化挑战，从而减轻信息冗余。

Result: GOLA有效减少了参数冗余，增强了特征表示能力。在四个基准数据集上显著优于现有最先进方法，验证了其在RGB-T跟踪任务中的有效性。

Conclusion: GOLA通过结构化参数学习和正交约束，成功解决了RGB-T跟踪中低秩适应的秩空间冗余问题，提升了模型的适应性和特征表示能力，是RGB-T跟踪任务的有效框架。
...
### [48] [LoC-Path: Learning to Compress for Pathology Multimodal Large Language Models](https://arxiv.org/abs/2512.05391)
*Qingqiao Hu,Weimin Lyu,Meilong Xu,Kehan Qi,Xiaoling Hu,Saumya Gupta,Jiawei Zhou,Chao Chen*

Main category: cs.CV

TL;DR: LoC-Path是一个高效的病理学多模态大语言模型（MLLM），通过减少全玻片图像（WSI）中的视觉冗余来降低计算成本，同时保持与现有先进模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有病理学MLLM在理解WSI时，通过暴力处理数千个图块特征，导致计算成本过高，而人类专家主要依赖关键区域进行诊断。研究发现WSI图块特征存在严重的全局和局部冗余，只有一小部分图块真正与诊断相关。

Method: LoC-Path框架用冗余减少模块替代了昂贵的幻灯片级编码器。具体包括：1) 设计稀疏Token合并器（STM）和MAE预训练的重采样器，以去除局部冗余并将全局冗余的图块Token压缩成紧凑的幻灯片级表示集；2) 提出交叉注意力路由适配器（CARA）和Token重要性评分器（TIS），以计算高效的方式将压缩的视觉表示与语言模型集成。

Result: LoC-Path在计算和内存需求显著降低的情况下，实现了与现有最先进的全玻片MLLM相当的性能。

Conclusion: LoC-Path通过有效利用图块特征的稀疏性和冗余性，成功构建了一个计算高效且性能卓越的病理学WSI-语言建模框架，解决了现有模型在处理千兆像素级WSI时面临的计算挑战。
...
TL;DR: 针对可控视频模型中的幻觉问题，本文提出C3方法，通过在子块级别进行连续尺度校准的不确定性量化，精准定位生成视频帧中不可信区域，提高模型可靠性并支持分布外检测。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式视频模型在可控视频合成方面取得了显著进展，但它们常出现“幻觉”——生成与物理现实不符的未来视频帧。这在机器人策略评估和规划等任务中引发严重问题。当前最先进的视频模型缺乏评估和表达自身置信度的能力，从而阻碍了幻觉的缓解。

Method: 本文提出C3，一种不确定性量化（UQ）方法，用于训练连续尺度校准的可控视频模型，实现子块级别的密集置信度估计。该方法包含三项核心创新：1) 建立新颖框架，通过严格适当的评分规则训练视频模型以实现正确性和校准；2) 在潜在空间估计视频模型的不确定性，避免像素空间方法的训练不稳定性和高成本；3) 将密集的潜在空间不确定性映射到可解释的RGB空间像素级不确定性，用于直观可视化，提供高分辨率的不确定性热图。

Result: 通过在大型机器人学习数据集（Bridge和DROID）和真实世界评估中进行广泛实验，结果表明C3方法不仅在训练分布内提供了校准的不确定性估计，而且还能够实现有效的分布外（OOD）检测。

Conclusion: C3方法通过在生成视频帧中提供密集、校准且可解释的不确定性估计，有效解决了可控视频模型中的幻觉问题，显著提高了模型的可靠性，并支持对分布外情况的识别。
...
### [50] [Label-Efficient Point Cloud Segmentation with Active Learning](https://arxiv.org/abs/2512.05759)
...
TL;DR: 该研究提出一种新颖且易于实现的3D点云语义分割主动学习策略，通过2D网格划分区域和网络集成估计不确定性，在减少标注成本的同时，性能媲美甚至超越现有复杂方法。


<details>
  <summary>Details</summary>
Motivation: 3D点云数据的语义分割标注成本高昂。现有主动学习方法通常依赖复杂的启发式方法来划分区域和选择数据，需要更简单有效的方法来减少所需的标注量并保持良好性能。

Method: 该方法采用2D网格将点云划分为可标注的列区域。为选择下一个要标注的数据，利用网络集成来估计网络输出的不确定性。此外，研究还提出标注面积可能比标注点数更适合作为点云主动学习的衡量标准。

Result: 在S3DIS、Toronto-3D和弗莱堡城市大规模3D点云数据集上进行评估。结果显示，该方法在所有数据集上的性能与复杂的前沿方法相当，甚至更好。研究还提供了证据表明，在点云背景下，标注面积可能比标注点数是更具意义的主动学习算法衡量标准。

Conclusion: 所提出的简单主动学习策略能够有效地减少3D点云语义分割的标注成本，并取得了与现有复杂方法相当或更优的性能。同时，该研究为点云主动学习的评估指标提供了新的视角，建议关注标注面积。
...
### [51] [The Dynamic Prior: Understanding 3D Structures for Casual Dynamic Videos](https://arxiv.org/abs/2512.05398)
...
### [53] [Synset Signset Germany: a Synthetic Dataset for German Traffic Sign Recognition](https://arxiv.org/abs/2512.05936)
...
### [55] [Measuring the Effect of Background on Classification and Feature Importance in Deep Learning for AV Perception](https://arxiv.org/abs/2512.05937)
...
### [56] [Moving object detection from multi-depth images with an attention-enhanced CNN](https://arxiv.org/abs/2512.05415)
...
TL;DR: 该研究提出了一种结合多输入卷积神经网络和卷积块注意力模块的方法，用于自动验证太阳系中移动目标，实现了近99%的准确率，并大幅减少了人工验证的工作量。


<details>
  <summary>Details</summary>
Motivation: 在广域巡天数据中检测太阳系移动目标时，区分真实目标信号和噪声是一个巨大挑战。传统上，目标验证严重依赖人工目视检查，导致高昂的人力成本。为解决这一限制并减少人工干预，研究旨在开发一种自动验证方法。

Method: 本文提出了一种集成卷积块注意力模块的多输入卷积神经网络。该方法包含两项创新：一是多输入架构，可同时处理多个堆叠图像；二是引入卷积块注意力模块，使模型能够在空间和通道维度上关注关键特征。这些改进旨在促进从多输入中高效学习，从而实现更鲁棒的移动目标检测。

Result: 该模型在包含约2000张观测图像的数据集上进行了评估，取得了近99%的准确率和大于0.99的AUC（曲线下面积）。通过调整目标检测阈值，新模型将人工工作量减少了99%以上。

Conclusion: 所提出的模型实现了卓越的分类性能，有效地区分了真实目标和噪声。它显著减少了对人工干预的依赖，将人工工作量降低了99%以上，极大地提高了太阳系移动目标检测系统的效率。
...
### [58] [YOLO and SGBM Integration for Autonomous Tree Branch Detection and Depth Estimation in Radiata Pine Pruning Applications](https://arxiv.org/abs/2512.05412)
*Yida Lin,Bing Xue,Mengjie Zhang,Sam Schofield,Richard Green*
...
### [59] [University Building Recognition Dataset in Thailand for the mission-oriented IoT sensor system](https://arxiv.org/abs/2512.05468)
...
### [60] [UniFS: Unified Multi-Contrast MRI Reconstruction via Frequency-Spatial Fusion](https://arxiv.org/abs/2512.05481)
...
### [62] [See in Depth: Training-Free Surgical Scene Segmentation with Monocular Depth Priors](https://arxiv.org/abs/2512.05529)
...
### [63] [Active Video Perception: Iterative Evidence Seeking for Agentic Long Video Understanding](https://arxiv.org/abs/2512.05774)
...
### [64] [ParaUni: Enhance Generation in Unified Multimodal Model with Reinforcement-driven Hierarchical Parallel Information Interaction](https://arxiv.org/abs/2512.05422)
...
### [65] [TED-4DGS: Temporally Activated and Embedding-based Deformation for 4DGS Compression](https://arxiv.org/abs/2512.05446)
...
### [66] [Conscious Gaze: Adaptive Attention Mechanisms for Hallucination Mitigation in Vision-Language Models](https://arxiv.org/abs/2512.05546)
...
### [67] [Zoom in, Click out: Unlocking and Evaluating the Potential of Zooming for GUI Grounding](https://arxiv.org/abs/2512.05941)
...
### [68] [EmoStyle: Emotion-Driven Image Stylization](https://arxiv.org/abs/2512.05478)
...
### [69] [2K-Characters-10K-Stories: A Quality-Gated Stylized Narrative Dataset with Disentangled Control and Sequence Consistency](https://arxiv.org/abs/2512.05557)
...
### [70] [InverseCrafter: Efficient Video ReCapture as a Latent Domain Inverse Problem](https://arxiv.org/abs/2512.05672)
...
### [73] [WaterWave: Bridging Underwater Image Enhancement into Video Streams via Wavelet-based Temporal Consistency Field](https://arxiv.org/abs/2512.05492)
...
### [74] [Rethinking Infrared Small Target Detection: A Foundation-Driven Efficient Paradigm](https://arxiv.org/abs/2512.05511)
...
### [75] [Probing the effectiveness of World Models for Spatial Reasoning through Test-time Scaling](https://arxiv.org/abs/2512.05809)
...
### [78] [Know-Show: Benchmarking Video-Language Models on Spatio-Temporal Grounded Reasoning](https://arxiv.org/abs/2512.05513)
...
### [79] [DashFusion: Dual-stream Alignment with Hierarchical Bottleneck Fusion for Multimodal Sentiment Analysis](https://arxiv.org/abs/2512.05515)
...
### [80] [VOST-SGG: VLM-Aided One-Stage Spatio-Temporal Scene Graph Generation](https://arxiv.org/abs/2512.05524)
*Chinthani Sugandhika,Chen Li,Deepu Rajan,Basura Fernando*
...
### [81] [Ideal Observer for Segmentation of Dead Leaves Images](https://arxiv.org/abs/2512.05539)
...
### [82] [Learning High-Fidelity Cloth Animation via Skinning-Free Image Transfer](https://arxiv.org/abs/2512.05593)
...
### [83] [MedDIFT: Multi-Scale Diffusion-Based Correspondence in 3D Medical Imaging](https://arxiv.org/abs/2512.05571)
...
### [84] [Fast SceneScript: Accurate and Efficient Structured Language Model via Multi-Token Prediction](https://arxiv.org/abs/2512.05597)
...
### [85] [ProPhy: Progressive Physical Alignment for Dynamic World Simulation](https://arxiv.org/abs/2512.05564)
...
### [86] [NormalView: sensor-agnostic tree species classification from backpack and aerial lidar data using geometric projections](https://arxiv.org/abs/2512.05610)
...
### [87] [OWL: Unsupervised 3D Object Detection by Occupancy Guided Warm-up and Large Model Priors Reasoning](https://arxiv.org/abs/2512.05698)
*Xusheng Guo,Wanfa Zhang,Shijia Zhao,Qiming Xia,Xiaolong Xie,Mingming Wang,Hai Wu,Chenglu Wen*

Main category: cs.CV

TL;DR: OWL是一种无监督3D目标检测方法，通过占用引导预热和大模型先验推理来解决伪标签不准确问题，显著提高了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的无监督3D目标检测方法在训练初期生成的伪标签通常不准确，导致优化过程受误导，并且有效过滤和细化伪标签仍是一个关键挑战。

Method: 本文提出了OWL方法，包括三个主要组成部分：1. 占用引导预热（OGW）策略，用于初始化主干网络权重，使其具备空间感知能力，减轻错误伪标签的干扰。2. 实例提示推理（ICR）模块，利用大模型的先验知识评估伪标签质量，实现精确过滤和细化。3. 权重自适应自训练（WAS）策略，动态重新加权伪标签，通过自训练提高性能。

Result: 在Waymo Open Dataset (WOD) 和 KITTI 上的大量实验表明，OWL的mAP性能超越了现有最先进的无监督方法15.0%以上。

Conclusion: OWL通过其提出的占用引导预热和大模型先验推理策略，有效解决了无监督3D目标检测中伪标签不准确和优化受误导的问题，证明了其方法的有效性。
...
### [88] [DistillFSS: Synthesizing Few-Shot Knowledge into a Lightweight Segmentation Model](https://arxiv.org/abs/2512.05613)
...
### [89] [Experts-Guided Unbalanced Optimal Transport for ISP Learning from Unpaired and/or Paired Data](https://arxiv.org/abs/2512.05635)
...
### [90] [Self-Supervised AI-Generated Image Detection: A Camera Metadata Perspective](https://arxiv.org/abs/2512.05651)
...
### [91] [LeAD-M3D: Leveraging Asymmetric Distillation for Real-time Monocular 3D Detection](https://arxiv.org/abs/2512.05663)
...
### [92] [Hyperspectral Unmixing with 3D Convolutional Sparse Coding and Projected Simplex Volume Maximization](https://arxiv.org/abs/2512.05674)
...
### [93] [Deep Learning-Based Real-Time Sequential Facial Expression Analysis Using Geometric Features](https://arxiv.org/abs/2512.05669)
...
### [94] [Distilling Expert Surgical Knowledge: How to train local surgical VLMs for anatomy explanation in Complete Mesocolic Excision](https://arxiv.org/abs/2512.05740)
...
### [95] [Manifold-Aware Point Cloud Completion via Geodesic-Attentive Hierarchical Feature Learning](https://arxiv.org/abs/2512.05710)
*Jianan Sun,Dongzhihan Wang,Mingyu Fan*

Main category: cs.CV

TL;DR: 本文提出一个流形感知点云补全框架，通过引入测地距离和测地关系注意力机制，解决了传统方法依赖欧氏距离导致几何一致性差和语义模糊的问题，显著提升了补全质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在点云补全中依赖欧氏邻近性，忽略了点云固有的非线性几何结构，导致几何一致性不佳和语义模糊。

Method: 本文提出了一个流形感知点云补全框架，包含两个核心模块：1) 测地距离近似器（GDA），用于估计点之间的测地距离以捕捉潜在的流形拓扑；2) 流形感知特征提取器（MAFE），利用基于测地距离的k-NN分组和测地关系注意力机制来指导分层特征提取过程。

Result: 在基准数据集上的大量实验表明，本文方法在重建质量方面始终优于现有最先进的方法。

Conclusion: 通过整合测地感知关系注意力，本文方法提升了重建点云的语义连贯性和结构保真度。
...
### [96] [Physics-Informed Graph Neural Network with Frequency-Aware Learning for Optical Aberration Correction](https://arxiv.org/abs/2512.05683)
...
### [97] [VRSA: Jailbreaking Multimodal Large Language Models through Visual Reasoning Sequential Attack](https://arxiv.org/abs/2512.05853)
*Shiji Zhao,Shukun Xiong,Yao Huang,Yan Jin,Zhenyu Wu,Jiyang Guan,Ranjie Duan,Jialing Tao,Hui Xue,Xingxing Wei*

Main category: cs.CV

TL;DR: 本文提出了一种名为VRSA的视觉推理序列攻击方法，通过将有害文本分解为一系列相关的子图像，逐步诱导多模态大语言模型（MLLMs）输出有害内容，并在开源和闭源MLLMs上实现了更高的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）因其强大的跨模态理解和生成能力被广泛应用，但这也带来了更多的越狱攻击漏洞。现有研究主要关注文本模态中的推理安全风险，而视觉模态中的类似威胁却被忽视。本文旨在全面评估视觉推理任务中潜在的安全风险。

Method: 本文提出视觉推理序列攻击（VRSA）方法，通过将原始有害文本分解为几个顺序相关的子图像，诱导MLLMs逐步外化并聚合完整的有害意图。具体方法包括：1) 自适应场景优化（Adaptive Scene Refinement），优化与原始有害查询最相关的场景，增强图像序列的合理性；2) 语义连贯补全（Semantic Coherent Completion），结合场景上下文信息迭代重写每个子文本，确保生成图像的语义连续性；3) 文本-图像一致性对齐（Text-Image Consistency Alignment），保持语义一致性。

Result: 实验结果表明，VRSA方法在GPT-4o和Claude-4.5-Sonnet等开源和闭源MLLMs上，比现有最先进的越狱攻击方法取得了更高的攻击成功率。

Conclusion: VRSA方法有效揭示了多模态大语言模型在视觉推理任务中被忽视的安全风险，通过序列化的视觉攻击能成功诱导模型输出有害内容，凸显了对视觉模态漏洞的关注和防范的重要性。
...
### [98] [HQ-DM: Single Hadamard Transformation-Based Quantization-Aware Training for Low-Bit Diffusion Models](https://arxiv.org/abs/2512.05746)
...
### [99] [USV: Unified Sparsification for Accelerating Video Diffusion Models](https://arxiv.org/abs/2512.05754)
...
### [100] [SCAIL: Towards Studio-Grade Character Animation via In-Context Learning of 3D-Consistent Pose Representations](https://arxiv.org/abs/2512.05905)
*Wenhao Yan,Sheng Ye,Zhuoyi Yang,Jiayan Teng,ZhenHui Dong,Kairui Wen,Xiaotao Gu,Yong-Jin Liu,Jie Tang*

Main category: cs.CV

TL;DR: SCAIL是一个通过新颖3D姿态表示和全上下文姿态注入扩散-Transformer架构，实现工作室级角色动画的框架，解决了现有方法在复杂运动和跨身份动画中的结构保真度和时间一致性问题。


<details>
  <summary>Details</summary>
Motivation: 尽管有进展，但现有方法在将运动从驱动视频转移到参考图像时，在复杂运动和跨身份动画的野外场景中，往往难以保持结构保真度和时间一致性，无法达到工作室级的生产标准。

Method: 本文提出了SCAIL框架，包含两项关键创新：1) 提出了一种新颖的3D姿态表示，提供更鲁棒和灵活的运动信号；2) 在扩散-Transformer架构中引入了全上下文姿态注入机制，实现了对完整运动序列的有效时空推理。此外，还开发了精心策划的数据管道以确保多样性和质量，并建立了全面的基准进行系统评估。

Result: 实验表明，SCAIL取得了最先进的性能。

Conclusion: SCAIL将角色动画推向了工作室级的可靠性和真实感。
...
### [101] [Bring Your Dreams to Life: Continual Text-to-Video Customization](https://arxiv.org/abs/2512.05802)
*Jiahua Dong,Xudong Wang,Wenqi Liang,Zongyan Han,Meng Cao,Duzhen Zhang,Hanbin Zhao,Zhi Han,Salman Khan,Fahad Shahbaz Khan*

Main category: cs.CV

TL;DR: 本文提出了一种名为CCVD的持续定制化视频扩散模型，旨在解决现有定制文本到视频生成（CTVG）方法在持续学习新概念时面临的遗忘和概念忽略问题，从而实现更灵活的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有定制文本到视频生成（CTVG）方法假设个性化概念是静态的，并且在持续学习新概念（包括主体和动作）时，会遇到灾难性遗忘和概念忽略的问题。

Method: 为解决上述挑战，CCVD模型引入了：1) 概念特定属性保留模块和任务感知概念聚合策略，以应对灾难性遗忘，捕获旧概念的独特特征并在测试时根据相关性组合适配器；2) 可控条件合成，通过结合层特定区域注意力引导的噪声估计，增强区域特征并使视频上下文与用户条件对齐，以解决概念忽略。

Result: 广泛的实验比较表明，CCVD模型在性能上优于现有的定制文本到视频生成（CTVG）模型。

Conclusion: CCVD模型成功地实现了持续学习新概念的定制文本到视频生成，有效解决了灾难性遗忘和概念忽略问题，提升了视频生成能力。
...
### [102] [Underwater Image Reconstruction Using a Swin Transformer-Based Generator and PatchGAN Discriminator](https://arxiv.org/abs/2512.05866)
*Md. Mahbub Hasan Akash,Aria Tasnim Mridula,Sheekar Banerjee,Ishtiak Al Mamoon*

Main category: cs.CV

TL;DR: 本文提出了一种结合Swin Transformer和GAN的新型深度学习框架，用于水下图像重建。该框架通过U-Net结构中的Swin Transformer块处理局部特征和长距离依赖，并在EUVP数据集上取得了最先进的性能，有效改善了水下图像的色彩、对比度和雾度。


<details>
  <summary>Details</summary>
Motivation: 水下成像对于海洋探索、环境监测和基础设施检查至关重要，但水介质导致图像严重的色彩失真、对比度低和雾度效应。传统方法和基于CNN的方法因感受野有限且无法建模全局依赖性，难以有效解决这些挑战。

Method: 本研究提出了一种将Swin Transformer架构集成到生成对抗网络（GAN）中的新型深度学习框架。生成器采用带有Swin Transformer块的U-Net结构，以捕获局部特征和长距离依赖。判别器采用PatchGAN，以确保高频细节的保留。模型在EUVP数据集上进行训练和评估，并进行了消融研究。

Result: 定量结果显示，该模型达到了24.76 dB的PSNR和0.89的SSIM，表现出最先进的性能，并显著优于现有方法。视觉结果表明，模型有效恢复了色彩平衡，改善了对比度，并减少了雾度。消融研究证实了Swin Transformer设计优于卷积替代方案。

Conclusion: 所提出的方法为水下图像重建提供了一个鲁棒的解决方案，适用于各种海洋应用，能够有效解决水下图像退化问题。
...
### [103] [FNOPT: Resolution-Agnostic, Self-Supervised Cloth Simulation using Meta-Optimization with Fourier Neural Operators](https://arxiv.org/abs/2512.05762)
...
### [104] [Curvature-Regularized Variational Autoencoder for 3D Scene Reconstruction from Sparse Depth](https://arxiv.org/abs/2512.05783)
...
### [105] [NICE: Neural Implicit Craniofacial Model for Orthognathic Surgery Prediction](https://arxiv.org/abs/2512.05920)
*Jiawen Yang,Yihui Cao,Xuanyu Tian,Yuyao Zhang,Hongjiang Wei*

Main category: cs.CV

TL;DR: 本文提出了一种名为NICE的神经隐式颅面模型，用于正颌手术后脸部外观的准确预测，通过隐式神经表示和区域特定解码器，解决了现有方法在计算效率和复杂非线性交互捕捉方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 正颌手术后脸部外观的准确预测面临挑战，因为骨骼运动与面部软组织之间存在复杂的非线性相互作用。现有的生物力学、参数模型和深度学习方法要么缺乏计算效率，要么未能充分捕捉这些复杂的交互。

Method: 本文提出了神经隐式颅面模型（NICE）。它包含一个形状模块和一个手术模块。形状模块利用区域特定的隐式符号距离函数（SDF）解码器重建面部表面、上颌骨和下颌骨。手术模块则采用区域变形解码器，由共享的手术潜在代码驱动，以有效建模面部表面对骨骼运动的复杂非线性生物力学响应，并输出逐点位移场，从而实现精确的手术结果建模。

Result: NICE在广泛实验中超越了当前最先进的方法，显著提高了唇部和下巴等关键面部区域的预测精度，同时稳健地保持了解剖完整性。

Conclusion: 这项工作提供了一种临床可行的工具，可用于正颌手术中增强手术规划和患者咨询。
...
### [106] [UG-FedDA: Uncertainty-Guided Federated Domain Adaptation for Multi-Center Alzheimer's Disease Detection](https://arxiv.org/abs/2512.05814)
*Fubao Zhu,Zhanyuan Jia,Zhiguo Wang,Huan Huang,Danyang Sun,Chuang Han,Yanting Li,Jiaofen Nan,Chen Zhao,Weihua Zhou*

Main category: cs.CV

TL;DR: 该研究提出UG-FedDA框架，通过结合不确定性量化和联邦域适应，解决了多中心阿尔茨海默病（AD）分类中存在的跨站点异质性和隐私保护问题，并提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有AD分类框架在多中心研究中面临挑战，主要体现在忽略站点间异质性、缺乏不确定性量化机制，从而限制了其鲁棒性和临床适用性。早期诊断对及时干预至关重要。

Method: 本文提出了不确定性引导的联邦域适应（UG-FedDA）框架。该框架利用自注意力Transformer提取多模板感兴趣区域（RoI）特征，捕捉区域表示及其相互作用。通过整合不确定性量化（UQ）来指导特征对齐，通过降低不确定样本的权重来缓解源-目标分布偏移。所有操作在隐私约束下进行，以处理跨站点结构磁共振成像（MRI）异质性。

Result: UG-FedDA在ADNI、AIBL和OASIS三个公共数据集上进行了实验，在AD vs. NC、MCI vs. AD和NC vs. MCI三项分类任务中，准确性、敏感性和ROC曲线下面积均实现了持续的跨域改进。例如，在NC vs. AD任务中，ADNI、AIBL和OASIS数据集的准确率分别达到90.54%、89.04%和77.78%。

Conclusion: 所提出的UG-FedDA框架不仅能有效地在多个站点之间进行适应，而且严格保护了隐私。实验结果证明了其在多中心AD分类中的鲁棒性和有效性。
...
### [107] [EditThinker: Unlocking Iterative Reasoning for Any Image Editor](https://arxiv.org/abs/2512.05965)
...
TL;DR: 本文提出了一种名为EditThinker的迭代式审议编辑框架，通过模拟人类认知循环（批判结果和优化指令），并使用强化学习训练一个多模态大语言模型（MLLM）作为推理引擎，显著提高了图像编辑模型对指令的遵循能力。


<details>
  <summary>Details</summary>
Motivation: 尽管基于指令的图像编辑模型在美学质量上取得了高成就，但由于固有的随机性和缺乏审议，其指令遵循能力仍是主要挑战，单次编辑的成功率有限。

Method: 本文提出一个审议性编辑框架，模拟人类认知循环，通过迭代执行“边思考边编辑”循环：批判结果、优化指令，然后重复生成直到满意。具体来说，训练了一个名为EditThinker的单一多模态大语言模型（MLLM）作为推理引擎，它能联合生成批判分数、推理过程和优化后的指令。此外，采用强化学习来对齐EditThinker的思考与编辑过程，以生成更有针对性的指令改进。

Result: 在四个基准测试上的大量实验表明，该方法显著提高了任何图像编辑模型的指令遵循能力。

Conclusion: 所提出的审议编辑框架，通过训练EditThinker并结合强化学习，有效解决了现有图像编辑模型指令遵循能力不足的问题，显著提升了编辑的准确性和满意度。
...
### [108] [Edit-aware RAW Reconstruction](https://arxiv.org/abs/2512.05859)
*Abhijith Punnappurath,Luxi Zhao,Ke Zhao,Hue Nguyen,Radek Grzeszczuk,Michael S. Brown*

Main category: cs.CV

TL;DR: 该论文提出了一种即插即用的、编辑感知的损失函数，可集成到任何现有RAW重建框架中，通过模拟可调参数的图像信号处理器（ISP）来训练，使从sRGB图像恢复的RAW数据对不同的渲染风格和编辑操作更具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 用户常在捕获后编辑相机图像以达到偏好的风格，但RAW编辑更精确灵活。然而，RAW图像很少被存储，大多数编辑是在sRGB输出上进行的。现有从sRGB恢复RAW数据的方法通常针对像素级RAW重建保真度进行优化，但在多样化的渲染风格和编辑操作下表现不佳。

Method: 引入了一个即插即用、编辑感知的损失函数。该损失函数包含一个模块化、可微分的图像信号处理器（ISP），用于模拟具有可调参数的真实图像处理流程。在训练期间，ISP模块的参数从精心设计的分布中随机采样，以模拟实际相机处理的变体。损失在sRGB空间中计算，比较通过该可微分ISP渲染的真实RAW和重建RAW。

Result: 该方法在各种编辑条件下将sRGB重建质量提高了1.5-2 dB PSNR。此外，当应用于元数据辅助的RAW重建方法时，它能够针对目标编辑进行微调，从而获得进一步的增益。

Conclusion: 该简单而有效的损失函数提供了一种通用机制，可以增强现有RAW重建方法的编辑保真度和渲染灵活性，这对于消费者影像中以摄影编辑为主要动机的RAW重建至关重要。
...
### [109] [LPD: Learnable Prototypes with Diversity Regularization for Weakly Supervised Histopathology Segmentation](https://arxiv.org/abs/2512.05922)
...
### [111] [Fine-Tuning BERT for Domain-Specific Question Answering: Toward Educational NLP Resources at University Scale](https://arxiv.org/abs/2512.05179)
*Aurélie Montfrond*

Main category: cs.CL

TL;DR: 本文通过构建定制数据集并微调BERT模型，成功开发了一个为利默里克大学电子与计算机工程系学生提供课程信息的问答聊天机器人，展示了基础模型在教育领域应用的潜力。


<details>
  <summary>Details</summary>
Motivation: 以往的科学问答研究主要侧重于聊天机器人系统，对基础模型在特定领域推理方面的微调探索有限。目前缺乏针对大学课程材料的领域特定基础模型，而现有BERT变体如BioBERT和SciBERT主要针对生物医学和科学文献。

Method: 研究开发了一个聊天机器人，构建了一个包含1,203个问题-答案对的定制数据集（SQuAD格式），数据来源于大学课程手册并辅以手动和合成生成。使用PyTorch微调了BERT模型，并采用精确匹配（Exact Match）和F1分数评估性能。

Result: 结果表明，即使是适度的微调也能显著改善假设构建和知识提取能力，证明了将基础模型应用于教育领域的可行性。

Conclusion: 通过学术问答对微调BERT能够取得有效结果，填补了大学课程材料领域特定问答模型的空白，并展示了构建可扩展的大学领域特定问答模型和自主教育知识系统的巨大潜力。
...
### [112] [Unveiling Affective Polarization Trends in Parliamentary Proceedings](https://arxiv.org/abs/2512.05231)
*Gili Goldin,Ella Rabinovich,Shuly Wintner*

Main category: cs.CL

TL;DR: 本文提出一种基于情感风格而非意识形态差异的新方法来量化政治极化，并在以色列议会语料库中发现政府和反对党成员的情感风格存在差异，且情感极化程度随时间显著增加。


<details>
  <summary>Details</summary>
Motivation: 近年来全球范围内极化言论日益增多，需要一种新颖的方法来量化极化，超越传统的意识形态立场差异，转而关注言论的情感风格。

Method: 本文提出一种基于情感风格（通过效价、唤醒度和主导度VAD测量）来量化极化的新方法。该方法通过检测情感言论信号来操作化“情感极化”概念。研究将此方法应用于最近发布的以色列议会（Knesset）会议记录语料库。

Result: 研究发现，政府成员的情感风格与反对党成员的不同。此外，通过这种情感风格反映出的情感极化水平随时间显著增加。

Conclusion: 情感风格可以作为量化情感极化的有效指标，且在以色列议会中观察到情感极化程度呈上升趋势。
...
### [113] [Decoding the Black Box: Discerning AI Rhetorics About and Through Poetic Prompting](https://arxiv.org/abs/2512.05243)
*P. D. Edgar,Alia Hall*

Main category: cs.CL

TL;DR: 本研究提出诗歌提示模式（Poetry Prompt Patterns）作为提示工程的新工具，用于评估大型语言模型（LLMs）对著名诗人作品的描述、评价及其改编原创作品的意愿。


<details>
  <summary>Details</summary>
Motivation: 提示工程是研究LLMs算法倾向和偏见的重要方法，同时创意人员和学者利用LLMs进行创作。本研究旨在探索创意文本提示，特别是诗歌提示，能否作为提示工程师的新工具，以更深入地研究LLMs的创作能力和潜在偏见。

Method: 本研究提出并概述了“诗歌提示模式”这一创意文本提示方法。然后，利用诗意提示来评估LLMs对一位著名诗人三个模型的描述和评价，并测试模型改编或重写原创创意作品以适应假定受众的意愿所带来的影响。

Result: 论文概述了采用诗歌提示方法的流程，并利用诗意提示评估了LLMs对著名诗人模型的描述和评价能力，以及它们改编或重写原创创意作品的意愿及其后果。

Conclusion: 创意文本提示，特别是诗歌提示模式，可以作为提示工程工具箱的有效补充，有助于深入研究LLMs的算法倾向、偏见以及其在创作和改编方面的能力。
...
### [114] [Enhancing Clinical Note Generation with ICD-10, Clinical Ontology Knowledge Graphs, and Chain-of-Thought Prompting Using GPT-4](https://arxiv.org/abs/2512.05256)
*Ivan Makohon,Mohamad Najafi,Jian Wu,Mathias Brochhausen,Yaohang Li*

Main category: cs.CL

TL;DR: 本研究探讨了如何通过结合思维链（CoT）提示工程、语义搜索和知识图谱来改进大型语言模型（LLM）生成的临床笔记，使用ICD编码和患者信息作为输入，结果显示优于标准一次性提示。


<details>
  <summary>Details</summary>
Motivation: 美国电子健康记录（EHR）数据激增，但医生手动书写和编辑临床笔记耗时巨大，增加了患者等待时间并可能延误诊断。大型语言模型（LLM）具备生成类人文本的能力，因此研究如何利用LLM辅助生成临床笔记以减轻医生负担。

Method: 研究采用思维链（CoT）提示工程来改进LLM（GPT-4）在临床笔记生成方面的响应。输入包括国际疾病分类（ICD）编码和基本患者信息。具体策略包括：1) 将传统CoT与语义搜索结果相结合；2) 融入基于临床本体论构建的知识图谱（KG），以丰富领域特定知识。在CodiEsp测试数据集的六个临床病例上测试了该提示技术。

Result: 研究结果表明，所提出的提示技术在生成临床笔记方面优于标准的一次性提示方法。

Conclusion: 结合思维链（CoT）提示、语义搜索结果和知识图谱可以显著提高大型语言模型（LLM）生成临床笔记的质量，为减轻医生工作负担和提高医疗效率提供了有效途径。
...
### [115] [To Think or Not to Think: The Hidden Cost of Meta-Training with Excessive CoT Examples](https://arxiv.org/abs/2512.05318)
*Vignesh Kothapalli,Ata Fatahibaarzi,Hamed Firooz,Maziar Sanjabi*

Main category: cs.CL

TL;DR: CoT-ICL在LLM中对新任务表现不佳时，本文提出元训练技术和CoT-Recipe方法，通过调节元训练中CoT和非CoT示例的混合比例，显著提升模型在新颖抽象推理任务上的表现，即使上下文无CoT示例。


<details>
  <summary>Details</summary>
Motivation: 链式思考(CoT)提示结合少样本上下文学习(ICL)显著提升了大型语言模型(LLM)的推理能力。然而，当预训练知识不足时，带有CoT示例的ICL对新颖任务无效。本研究旨在解决LLM在上下文学习中学习新颖抽象推理任务的局限性。

Method: 研究在CoT-ICL Lab框架的受控设置下进行。提出了元训练技术，使模型能在上下文学习中学习新颖抽象推理任务。为解决CoT监督有限时过度包含CoT示例导致性能下降的问题，提出了CoT-Recipe，一种形式化方法来调节元训练序列中CoT和非CoT示例的混合比例。这些技术也应用于预训练的LLM（Qwen2.5系列）进行符号推理任务。

Result: CoT-Recipe通过仔细调节，即使上下文学习中没有CoT示例，也能将Transformer在新任务上的准确率提高高达300%。将这些技术应用于预训练LLM（Qwen2.5系列）进行符号推理任务时，观察到准确率提高高达130%。

Conclusion: CoT-Recipe通过在元训练中策略性地调节CoT和非CoT示例的混合，可以显著提高Transformer模型在上下文学习新颖抽象推理任务上的准确性，即使CoT监督有限或上下文无CoT示例，也展现出强大的有效性。
...
### [116] [Exposing Pink Slime Journalism: Linguistic Signatures and Robust Detection Against LLM-Generated Threats](https://arxiv.org/abs/2512.05331)
*Sadat Shahriar,Navid Ayoobi,Arjun Mukherjee,Mostafa Musharrat,Sai Vishnu Vamsi*

Main category: cs.CL

TL;DR: 本地新闻面临“粉红粘液新闻”威胁，LLM修改文章显著降低现有检测系统性能。本文研究其特征，并提出一种能抵抗LLM攻击的鲁棒检测框架。


<details>
  <summary>Details</summary>
Motivation: 本地新闻是2800万美国人可靠信息来源，但正受到低质量、自动生成的“粉红粘液新闻”的威胁。检测这些欺骗性文章需要精细分析其语言、风格和词汇特征，特别是考虑到大型语言模型（LLMs）带来的新型对抗性修改。

Method: 本文对“粉红粘液新闻”的语言、风格和词汇特征进行全面研究，以发现其独特模式并提出检测策略。此外，针对LLM修改这一新的对抗性向量，本文引入了一个专门设计用于抵抗基于LLM的对抗性攻击并适应自动化“粉红粘液新闻”演变环境的鲁棒学习框架。

Result: 研究发现，即使是消费者可访问的LLM也能显著破坏现有检测系统，使其F1分数性能降低高达40%。为应对此威胁，本文提出的鲁棒学习框架将检测性能提高了高达27%。

Conclusion: “粉红粘液新闻”对本地新闻构成严重威胁，且LLM的引入使其检测更具挑战性。现有检测系统易受LLM修改攻击。本文提出的鲁棒学习框架能有效抵抗LLM攻击，为适应自动化“粉红粘液新闻”的演变提供了有效策略。
...
### [117] [LYNX: Learning Dynamic Exits for Confidence-Controlled Reasoning](https://arxiv.org/abs/2512.05325)
*Ömer Faruk Akgül,Yusuf Hakan Kalaycı,Rajgopal Kannan,Willie Neiswanger,Viktor Prasanna*

Main category: cs.CL

TL;DR: LYNX是一种在线提前退出机制，它利用大型推理模型自身的隐藏状态感知能力，通过在推理提示词（如“嗯”、“等等”）处训练轻量级探针，并结合分裂共形预测，实现置信度控制的早期停止。该机制可显著减少推理时间计算量并提高准确性，且探针可跨任务通用。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务中常“过度思考”，生成过长的思维链，浪费计算资源并可能损害准确性。现有提前退出方法存在局限性，如依赖额外采样、启发式、辅助验证模型或仅作为事后分析，缺乏形式保证。

Method: LYNX将退出决策与生成过程中自然出现的推理提示词（如“嗯”、“等等”）绑定。它在这些提示词的隐藏状态上训练一个轻量级探针，并利用强制退出作为监督信号。探针的得分通过分裂共形预测进行封装，以实现对过早退出的无分布控制。关键在于，探针仅在一个通用数学语料库上训练和校准一次，即可在不同基准、解码温度乃至非数学任务中重复使用。

Result: LYNX在1.5B到32B参数范围内的三种模型家族中，通过单个数学训练的探针实现了强大的准确性-效率权衡。在GSM8K上，它匹配或提高了基线准确性，同时减少了40-65%的token；在MATH-500上，准确性提高了多达12个百分点，token减少了约35-60%；在AIME 2024上，恢复了基线准确性，token节省超过50%；在非数学基准CommonsenseQA上，零样本迁移取得了适度的准确性提升和高达70%的token减少。与现有最先进的提前退出方法相比，LYNX提供了有竞争力的或更优的帕累托前沿，同时保持完全在线、无需代理模型且提供明确、用户可调的置信度保证。

Conclusion: LYNX是一种高效、鲁棒的在线提前退出机制，它利用模型自身的隐藏状态感知能力，显著提升了大型推理模型的推理效率和准确性。其探针的通用性和跨任务泛化能力，使其成为一种有前景的解决方案，可有效解决模型“过度思考”问题，且无需额外模型或复杂配置。
...
### [118] [Transformer-Enabled Diachronic Analysis of Vedic Sanskrit: Neural Methods for Quantifying Types of Language Change](https://arxiv.org/abs/2512.05364)
*Ananth Hariharan,David Mortensen*

Main category: cs.CL

TL;DR: 本研究利用混合神经-符号方法分析了梵语两千多年的演变，挑战了语言变化即简化的假设，揭示了形态复杂性动态再分配而非降低的模式。


<details>
  <summary>Details</summary>
Motivation: 挑战语言变化即简化的朴素假设，并通过量化分析梵语的演变，深入了解形态丰富、资源稀缺语言的进化过程，并解决此类研究中数据稀缺的问题。

Method: 采用弱监督方法，利用100多个高精度正则表达式模式生成伪标签来微调多语言BERT模型。通过新颖的置信度加权集成方法融合符号和神经网络输出，构建了一个可扩展且可解释的系统。该框架应用于一个包含147万字的历时梵语语料库。

Result: 该集成系统在梵语语料库上实现了52.4%的整体特征检测率。研究发现梵语的整体形态复杂性并未降低，而是动态地重新分配：早期动词特征呈现周期性衰退，但复杂性转移到其他领域，表现为复合词的显著扩展和新哲学术语的出现。系统产生了良好校准的不确定性估计，置信度与准确性高度相关（Pearson r = 0.92），整体校准误差较低（ECE = 0.043）。

Conclusion: 混合神经-符号方法能为形态丰富、资源稀缺语言的演变提供重要的新见解。梵语的语言变化并非简化，而是形态复杂性的动态再分配。该系统的高可靠性估计为计算文献学提供了有力支持。
...
### [119] [Mitigating Self-Preference by Authorship Obfuscation](https://arxiv.org/abs/2512.05379)
*Taslim Mahbub,Shi Feng*

Main category: cs.CL

TL;DR: 语言模型（LM）评判员存在自我偏好偏差，即偏好自己的输出。本研究通过对评估候选进行扰动来减少评判员的自我识别能力，发现简单的同义词替换可有效降低自我偏好。然而，当扰动进一步中和文体差异时，自我偏好会恢复，表明彻底消除该偏差仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: LM评判员被广泛用于评估LM输出质量，但其固有的偏见（特别是自我偏好，即偏爱自己的答案）损害了评估的公正性。即使在未明确标注来源的情况下，前沿LM评判员也能识别自己的输出，使得这种偏见难以消除。

Method: 研究通过降低LM评判员识别自身输出的能力来减轻自我偏好。具体方法是在成对比较中，对评估候选应用黑盒扰动（例如，少量词语的同义词替换），以混淆作者身份并减少自我识别。

Result: 研究发现，简单的扰动（如少量词语的同义词替换）能可预测地降低自我偏好。然而，当扰动被扩展到更彻底地中和评估候选之间的文体差异时，自我偏好会重新出现。这表明自我识别和自我偏好可能发生在多个语义层面。

Conclusion: 尽管初步结果显示简单的扰动能有效降低自我偏好，但自我识别和自我偏好是一个复杂的问题。彻底消除这种偏见，尤其是当偏见发生在更深层次的语义层面时，仍然面临重大挑战。
...
### [120] [SQ-format: A Unified Sparse-Quantized Hardware-friendly Data Format for LLMs](https://arxiv.org/abs/2512.05409)
*Ruixuan Huang,Hao Zeng,Hantao Huang,Jinyuan Shi,Minghui Yu,Ian En-Hsu Yen,Shuai Wang*

Main category: cs.CL

TL;DR: 本文提出了一种名为稀疏量化格式（SQ-format）的统一数据格式，用于大语言模型（LLMs）的量化和稀疏化，旨在通过新的硬件支持来平衡精度和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型后训练量化（PTQ）技术，如W4A8和2:4半结构稀疏，由于硬件支持有限，难以同时实现高精度和高效率。W4A8的峰值TOPS与W8A8相同，而2:4稀疏格式因精度损失而很少被采用。

Method: 本文提出SQ-format，这是一种统一的量化和稀疏化数据格式，旨在易于被新硬件和现有GPU支持。SQ-format利用了高精度稀疏矩阵乘法和低精度矩阵乘法都能被加速的特性，以实现性能和吞吐量之间的帕累托改进。该格式特别适用于具有异常值不平等状态的激活，并使其静态压缩成为可能。

Result: SQ-format展示了最先进的PTQ性能。论文还提出了支持该格式所需的硬件要求，并为下一代AI加速器提供了设计探索和见解。

Conclusion: SQ-format通过统一量化和稀疏化，有效地解决了现有PTQ技术在精度和效率之间的权衡问题，为LLM的民主化提供了新的数据格式和硬件设计方向。
...
### [121] [LMSpell: Neural Spell Checking for Low-Resource Languages](https://arxiv.org/abs/2512.05414)
*Akesh Gunathilakea,Nadil Karunarathnea,Tharusha Bandaranayakea,Nisansa de Silvaa,Surangika Ranathunga*

Main category: cs.CL

TL;DR: 本文首次实证研究了预训练语言模型（PLMs）在拼写纠错中的有效性，包括低资源语言（LRLs）。发现大型语言模型（LLMs）在微调数据集足够大时表现优于其他PLMs，并发布了LMSpell工具包。


<details>
  <summary>Details</summary>
Motivation: 拼写纠错对低资源语言（LRLs）来说仍然是一个挑战。尽管预训练语言模型（PLMs）已被用于拼写纠错，但其应用仍局限于少数语言，且缺乏对不同PLMs的适当比较。

Method: 本文进行了一项首次实证研究，比较了不同PLMs（包括基于编码器、编码器-解码器和大型语言模型）在拼写纠错中的有效性，并将低资源语言纳入研究范围。研究发现，当微调数据集较大时，大型语言模型（LLMs）表现更优。此外，开发并发布了LMSpell工具包，其中包含一个能补偿LLMs幻觉的评估功能。还通过僧伽罗语（Sinhala）进行了案例研究。

Result: 研究发现，当微调数据集较大时，大型语言模型（LLMs）在拼写纠错方面优于其对应模型（基于编码器和编码器-解码器），即使对于LLM未预训练的语言也是如此。发布了LMSpell，一个易于使用的跨PLM拼写纠错工具包，包含补偿LLM幻觉的评估功能。

Conclusion: 大型语言模型（LLMs）在拼写纠错方面表现出色，尤其在拥有大型微调数据集时，即使对于未在其预训练数据中的语言也有效。虽然LLMs提供了有前景的解决方案，但低资源语言的拼写纠错仍然面临挑战。LMSpell工具包为跨PLM的拼写纠错提供了实用工具。
...
### [122] [Learning from Self Critique and Refinement for Faithful LLM Summarization](https://arxiv.org/abs/2512.05387)
*Ting-Yao Hu,Hema Swetha Koppula,Hadi Pouransari,Cem Koc,Oncel Tuzel,Raviteja Vemulapalli*

Main category: cs.CL

TL;DR: 本文提出SCRPO，一种自监督训练框架，利用大型语言模型自身的批判和改进能力构建偏好数据集，并通过偏好学习提高模型在摘要任务中的忠实度，解决了幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长文本生成任务（如摘要）中常出现幻觉，即输出内容与输入上下文不符。现有通过迭代批判和改进来减少幻觉的方法，要么需要额外的测试时计算资源，要么需要更强大的教师模型，导致成本高昂且不实用。

Method: 本文提出“自批判与改进偏好优化”（SCRPO）框架。该框架首先利用大型语言模型自身的批判和改进能力，构建一个偏好数据集。然后，通过应用偏好学习来训练同一个大型语言模型，以提高其在摘要任务中的忠实度。

Result: 在XSUM、CNNDM和SAMSum三个摘要基准测试中，SCRPO在忠实度指标上优于最先进的自监督学习方法，同时保持或提高了衡量摘要整体质量的其他指标。此外，与测试时改进方法相比，SCRPO不仅提高了效率，还生成了更忠实的摘要。

Conclusion: SCRPO通过自监督的方式，利用模型自身的批判和改进能力进行偏好学习，有效提高了大型语言模型在摘要任务中的忠实度，且在效率和效果上均优于现有方法，无需额外计算成本或更强大的外部模型。
...
### [123] [ArtistMus: A Globally Diverse, Artist-Centric Benchmark for Retrieval-Augmented Music Question Answering](https://arxiv.org/abs/2512.05430)
...
### [124] [Dynamic Alignment for Collective Agency: Toward a Scalable Self-Improving Framework for Open-Ended LLM Alignment](https://arxiv.org/abs/2512.05464)
...
### [125] [Automated Identification of Incidentalomas Requiring Follow-Up: A Multi-Anatomy Evaluation of LLM-Based and Supervised Approaches](https://arxiv.org/abs/2512.05537)
...
### [126] [SEA-SafeguardBench: Evaluating AI Safety in SEA Languages and Cultures](https://arxiv.org/abs/2512.05501)
...
### [127] [Structured Reasoning with Tree-of-Thoughts for Bengali Math Word Problems](https://arxiv.org/abs/2512.05580)
...
### [128] [A Greek Government Decisions Dataset for Public-Sector Analysis and Insight](https://arxiv.org/abs/2512.05647)
...
### [129] [Interleaved Latent Visual Reasoning with Selective Perceptual Modeling](https://arxiv.org/abs/2512.05665)
*Shuai Dong,Siyuan Wang,Xingyu Liu,Zhongyu Wei*

Main category: cs.CL

TL;DR: 本文提出了一种名为交错式潜在视觉推理（ILVR）的框架，它通过自监督策略和动量教师模型，将动态状态演化与精确感知建模相结合，克服了多模态大语言模型中视觉推理的计算成本和精度权衡问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）中的交错式推理范式因重复编码像素密集图像而计算成本高昂。潜在视觉推理虽能规避此瓶颈，但面临关键权衡：要么通过过度压缩牺牲精确感知建模，要么因静态结构无法处理动态问题。研究旨在解决这一精度与动态性之间的矛盾。

Method: ILVR框架统一了动态状态演化和精确感知建模。它将文本生成与作为特定、演化线索的潜在视觉表示交错进行。为实现此目的，ILVR采用了一种自监督策略，其中一个动量教师模型选择性地将辅助图像中的相关特征蒸馏到稀疏监督目标中，以指导模型自主生成上下文感知的视觉信号。

Result: 在多模态推理基准上的大量实验表明，ILVR显著优于现有方法。

Conclusion: ILVR有效地弥合了细粒度感知与序列多模态推理之间的鸿沟。
...
### [130] [Grounded Multilingual Medical Reasoning for Question Answering with Large Language Models](https://arxiv.org/abs/2512.05658)
*Pietro Ferrazzi,Aitor Soroa,Rodrigo Agerri*

Main category: cs.CL

TL;DR: 本文提出了一种生成多语言（英、意、西）医学推理链条的方法，基于检索增强生成技术和维基百科医学知识，显著提升了医学问答中8B参数LLM的性能，并发布了相关资源。


<details>
  <summary>Details</summary>
Motivation: 现有医学问答LLM主要以英语为主，且依赖通用LLM蒸馏，导致其医学知识的可靠性存疑。

Method: 采用检索增强生成（RAG）方法，利用维基百科医学信息，为MedQA和MedMCQA（扩展至意大利语和西班牙语）生成了50万条多语言（英、意、西）的医学推理链条。

Result: 这些推理链条通过上下文学习（few-shot）和监督微调均能提升性能，在8B参数LLM中取得了最先进的医学问答结果。研究团队还发布了推理链条、翻译后的问答数据集、医学维基百科和微调模型等资源。

Conclusion: 这些资源有望支持开发更安全、更透明的多语言临床决策支持工具。
...
### [131] [MedTutor-R1: Socratic Personalized Medical Teaching with Multi-Agent Simulation](https://arxiv.org/abs/2512.05671)
*Zhitao He,Haolin Yang,Zeyu Qin,Yi R Fung*

Main category: cs.CL

TL;DR: 该研究开发了一个多智能体教学模拟器（ClinEdu）和苏格拉底式教学对话数据集（ClinTeach），并在此基础上训练了首个用于一对多临床教学的多模态苏格拉底导师（MedTutor-R1），旨在弥合临床培训需求与专家指导稀缺之间的鸿沟，特别关注协作推理能力的培养。


<details>
  <summary>Details</summary>
Motivation: 临床培训需求日益增长，但专家指导稀缺，导致医学教育面临巨大挑战。现有研究主要关注一对一知识指导，忽视了在团队合作（如查房）中培养学生协作推理这一关键技能。

Method: 研究方法包括：1) 开发ClinEdu，一个具有个性化患者和多样化学生群体的多智能体教学模拟器，用于测试复杂教学过程和生成教学数据。2) 基于ClinEdu构建ClinTeach，一个捕捉小组教学复杂性的大型苏格拉底式教学对话数据集。3) 训练MedTutor-R1，这是首个用于临床医学教育一对多教学的多模态苏格拉底导师。MedTutor-R1首先在ClinTeach数据集上进行指令微调，然后通过强化学习进行优化，奖励来源于涵盖结构保真度、分析质量和临床安全性的三轴评估标准。4) 使用基于模拟的交互式评估，将导师重新部署到ClinEdu中进行真实情境评估。

Result: 实验结果表明，MedTutor-R1的平均教学分数比基础模型高出20%以上，与o3模型相当，并且在处理不同数量的学生时表现出高度适应性。这一有前景的性能也突显了其教学模拟器ClinEdu的有效性。

Conclusion: 该研究成功开发并验证了MedTutor-R1，一个有效且适应性强的多模态苏格拉底导师，能够用于一对多临床教学，弥补了现有研究在协作推理和小组教学方面的不足。其教学模拟器ClinEdu也展现出强大的能力。
...
### [132] [Retrieving Semantically Similar Decisions under Noisy Institutional Labels: Robust Comparison of Embedding Methods](https://arxiv.org/abs/2512.05681)
*Tereza Novotna,Jakub Harasta*

Main category: cs.CL

TL;DR: 本文比较了OpenAI通用嵌入器和领域特定BERT模型在捷克宪法法院判例检索中的表现，结果显示OpenAI模型显著优于领域特定BERT，并提出了一个鲁棒的噪声感知评估框架。


<details>
  <summary>Details</summary>
Motivation: 判例法检索是一项耗时的工作，主要通过查询数据库进行。研究旨在探索和比较不同模型在该任务上的有效性，尤其是在处理具有异构标签的传统司法数据库时。

Method: 研究在捷克宪法法院判决数据集上比较了两种模型：(i) 一个大型通用嵌入器（OpenAI），(ii) 一个从零开始训练的领域特定BERT模型（在约30,000份判决上使用滑动窗口和注意力池化训练）。评估方法包括噪声感知评估，采用IDF加权关键词重叠作为分级相关性，通过两个阈值（0.20平衡，0.28严格）进行二值化，通过配对自举法进行显著性检验，并结合nDCG诊断和定性分析。

Result: 尽管绝对nDCG值不高（在有噪声标签下是预期的），但通用OpenAI嵌入器在@10/@20/@100的两个阈值设置下均显著优于领域预训练的BERT模型，差异具有统计学意义。诊断结果表明，较低的绝对值是由于标签漂移和强理想值，而非模型效用不足。此外，所提出的评估框架足够鲁棒，可用于处理来自传统司法数据库的异构标签噪声数据集。

Conclusion: 通用OpenAI嵌入器在捷克判例法检索任务中表现出优越性，显著超越了领域特定BERT模型。研究提出的噪声感知评估框架对于处理具有挑战性的、带有噪声标签的司法数据集是有效和鲁棒的。
...
### [133] [Faithfulness metric fusion: Improving the evaluation of LLM trustworthiness across domains](https://arxiv.org/abs/2512.05700)
*Ben Malin,Tatiana Kalganova,Nikolaos Boulgouris*

Main category: cs.CL

TL;DR: 提出一种融合度量方法，结合基本忠实度指标和人类判断，以提高大型语言模型（LLM）忠实度评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM忠实度评估方法存在不足，限制了LLM在更多场景中的应用，需要更准确的评估方法来增强模型可信度。

Method: 通过树状模型将基本忠实度指标融合成一个复合（融合）指标。该模型利用人类对LLM响应忠实度的判断来确定每个指标的重要性。此外，还统一了问答和对话领域的数据集，并整合了人类判断和LLM响应，以支持忠实度评估的复现和测试。

Result: 融合指标在所有测试领域中与人类忠实度判断表现出更强的相关性。

Conclusion: 改进LLM忠实度评估能力能够增强模型可信度，从而使其在更多样化的场景中得以应用。所提供的数据集也有助于忠实度评估研究的复现和验证。
...
### [134] [Efficient Text Classification with Conformal In-Context Learning](https://arxiv.org/abs/2512.05732)
*Ippokratis Pantelidis,Korbinian Randl,Aron Henriksson*

Main category: cs.CL

TL;DR: CICLe通过结合轻量级分类器和共形预测来指导LLM提示，显著提高了文本分类的效率和性能，尤其在数据充足和类别不平衡的情况下，并能减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在文本分类中依赖于提示设计且计算成本高昂。虽然CICLe已被提出作为一种资源高效的框架，但其在单一领域之外的更广泛适用性和效率优势尚未得到系统性探索。

Method: 本文对CICLe在各种NLP分类基准上进行了全面评估。CICLe通过将一个轻量级基础分类器与共形预测相结合，自适应地减少候选类别，从而指导LLM提示。

Result: 结果显示，当基础分类器有足够的训练样本时，CICLe始终优于其基础分类器并超越少样本提示基线；在低数据量情况下表现相当。在效率方面，CICLe将提示词数量和提示长度分别减少了高达34.45%和25.16%，并支持使用性能具有竞争力的较小型模型。CICLe在类别高度不平衡的文本分类任务中尤其具有优势。

Conclusion: CICLe是一种实用且可扩展的高效文本分类方法，它结合了传统分类器的鲁棒性和LLMs的适应性，在数据和计算效率方面取得了显著提升。
...
### [135] [Capturing Classic Authorial Style in Long-Form Story Generation with GRPO Fine-Tuning](https://arxiv.org/abs/2512.05747)
*Jinlong Liu,Mohammed Bahja,Venelin Kovatchev,Mark Lee*

Main category: cs.CL

TL;DR: 本文提出一个基于GRPO和多奖励机制的训练框架，用于风格化故事生成。该框架利用作者验证（AV）信号进行风格奖励，并成功使一个8B模型在马克·吐温风格生成方面超越GPT-4o和Claude Sonnet 4，证明了中等模型尺寸和特定任务训练在风格生成上的可行性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在开放式故事生成方面表现出色，但在精细的风格控制上仍有限制。现有方法通常依赖浅层线索模拟作者风格，且缺乏可靠的评估。

Method: 开发了一个使用群组相对策略优化（GRPO）和定制多奖励设置的风格条件故事生成训练框架。风格奖励来自一个经过作者验证（AV）信号微调的句子转换器，并结合内容和完整性分数以稳定长篇叙事生成。实验以马克·吐温的《哈克贝利·费恩历险记》作为参考风格范例。

Result: 一个8B模型在AV风格指标上（风格分数0.628）超越了GPT-4o和Claude Sonnet 4等更大的基线模型，并保持了有竞争力的内容质量。结果表明，通过中等模型尺寸和特定任务训练，智能体风格化生成是可行的。

Conclusion: 智能体风格化生成是可行的，且输出与风格高度一致。然而，叙事完整性仍是一个挑战，未来工作需要更好地建模全局连贯性和故事结局。
...
### [136] [Heard or Halted? Gender, Interruptions, and Emotional Tone in U.S. Supreme Court Oral Arguments](https://arxiv.org/abs/2512.05832)
*Yifei Tong*
...
### [137] [Prompting Science Report 4: Playing Pretend: Expert Personas Don't Improve Factual Accuracy](https://arxiv.org/abs/2512.05858)
...
### [138] [M4-RAG: A Massive-Scale Multilingual Multi-Cultural Multimodal RAG](https://arxiv.org/abs/2512.05959)
*David Anugraha,Patrick Amadeus Irawan,Anshul Singh,En-Shiun Annie Lee,Genta Indra Winata*

Main category: cs.CL

TL;DR: 本文引入了M4-RAG，一个涵盖42种语言和56种方言的大规模基准测试，用于评估多语言多模态RAG下的VQA性能。研究发现RAG对小型VLM有益，但对大型模型效果不佳甚至会降低性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLM)受限于静态训练数据，而检索增强生成(RAG)可以通过提供最新、文化相关和多语言信息来弥补这一不足。然而，多语言多模态RAG领域仍未得到充分探索。

Method: 引入了M4-RAG，一个包含42种语言、56种区域方言和超过80,000个文化多样图像-问题对的大规模基准。构建了一个受控的检索环境，其中包含数百万精心策划的多语言文档，以近似真实世界的检索条件并确保实验一致性。系统评估了RAG对不同大小VLM的性能影响。

Result: 研究发现RAG持续地使小型VLM受益，但未能扩展到大型模型，甚至经常降低其性能，这暴露了模型大小与当前检索效率之间存在关键不匹配。

Conclusion: M4-RAG为推进下一代RAG系统奠定了基础，这些系统能够无缝地进行跨语言、跨模态和跨文化背景的推理。
...
### [139] [Optimizing Medical Question-Answering Systems: A Comparative Study of Fine-Tuned and Zero-Shot Large Language Models with RAG Framework](https://arxiv.org/abs/2512.05863)
...
### [140] [Wake Vectoring for Efficient Morphing Flight](https://arxiv.org/abs/2512.05211)
*Ioannis Mandralis,Severin Schumacher,Morteza Gharib*

Main category: cs.RO

TL;DR: 变形飞行机器人面临空中变形时推力损失的问题。本文介绍了一种被动尾流矢量机制，通过内部导流板将旋翼尾流向下偏转，有效恢复了变形过程中损失的推力，显著提升了悬停和机动能力。


<details>
  <summary>Details</summary>
Motivation: 变形飞行机器人虽然在复杂环境导航、栖息和陆空转换方面潜力巨大，但空中重构时倾斜推进器会导致垂直推力减少，从而损害稳定性和控制能力，这是一个关键的空气动力学挑战。

Method: 引入了一种名为“空中变形机器人”（ATMO）的新型机器人系统，该系统集成了一个被动尾流矢量机制。内部导流板拦截并向下重新定向旋翼尾流，被动地引导气流动量，从而恢复损失的推力。这是一种无需电子设备的解决方案。

Result: 在原本无法产生有效推力的配置下，该机制实现了高达40%的垂直推力恢复，极大地扩展了变形过程中的悬停和机动能力。

Conclusion: 研究结果为变形飞行机器人设计开辟了新方向，即受火箭和飞机推力矢量启发，利用被动气动结构实现高效、敏捷飞行，而无需增加机械复杂性。
...
### [141] [Search at Scale: Improving Numerical Conditioning of Ergodic Coverage Optimization for Multi-Scale Domains](https://arxiv.org/abs/2512.05229)
*Yanis Lahrach,Christian Hughes,Ian Abraham*

Main category: cs.RO

TL;DR: 本文提出了一种基于最大均值差异（MMD）度量的、与尺度无关的自适应遍历覆盖优化方法，以解决现有方法在数值尺度敏感性和非线性约束下的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的遍历覆盖规划方法对问题空间的数值尺度高度敏感，且在非线性约束下，由于基于核的公式，优化变得脆弱且数值不稳定。

Method: 本文通过开发一种基于最大均值差异（MMD）度量的、与尺度无关的自适应遍历覆盖优化方法来解决此问题。该方法允许优化器求解微分约束的尺度，同时退火超参数以适应问题域并确保物理一致性。此外，还推导了遍历度量在对数空间中的变体，以提供额外的数值条件性而不损失性能。

Result: 所提出的方法能够确保物理一致性，并通过对数空间度量提供额外的数值条件性，同时不损失性能。与现有覆盖规划方法相比，该方法在广泛的覆盖问题上展示了其效用。

Conclusion: 所提出的基于MMD的、与尺度无关的自适应遍历覆盖优化方法有效解决了现有方法的数值不稳定性问题，提高了其在各种覆盖问题中的鲁棒性和适用性。
...
### [142] [Invariance Co-training for Robot Visual Generalization](https://arxiv.org/abs/2512.05230)
*Jonathan Yang,Chelsea Finn,Dorsa Sadigh*

Main category: cs.RO

TL;DR: 为解决通用机器人策略对观测变化的泛化能力不足问题，本文提出两种辅助任务（状态相似性和观测扰动不变性），并结合机器人演示数据和非物理合成图像进行协同训练，显著提高了机器人对不同视角、光照和干扰物的泛化能力，性能提升18%。


<details>
  <summary>Details</summary>
Motivation: 现有大规模机器人策略对相机视角、光照和干扰物等观测变化敏感，泛化能力有限。这主要是因为缺乏具有足够多样性以覆盖这些准静态轴的大规模机器人数据集。

Method: 引入两个关键辅助任务：状态相似性（state similarity）和观测扰动不变性（invariance to observational perturbations）。将这些辅助任务应用于机器人演示数据和从非物理模拟（如虚幻引擎）生成的视觉丰富合成图像。通过这些辅助任务，利用混合数据进行协同训练。

Result: 通过辅助任务和多样化数据（机器人演示数据和合成图像）的协同训练，显著提高了机器人对未见过的相机视角、光照配置和干扰物条件的泛化能力。性能比现有生成增强方法提高了18%。

Conclusion: 结合辅助任务和多样化数据（昂贵的机器人演示数据和廉价的合成视觉数据）进行协同训练，可以大幅提升通用机器人策略在复杂观测条件下的泛化能力。
...
### [143] [XR-DT: Extended Reality-Enhanced Digital Twin for Agentic Mobile Robots](https://arxiv.org/abs/2512.05270)
*Tianyi Wang,Jiseop Byeon,Ahmad Yehia,Huihai Wang,Yiming Xu,Tianyi Zeng,Ziran Wang,Junfeng Jiao,Christian Claudel*

Main category: cs.RO

TL;DR: 本文提出了XR-DT，一个基于扩展现实的数字孪生框架，旨在通过融合物理与虚拟空间，提升移动机器人与人类之间双向理解，实现可解释、可信赖和自适应的人机交互。


<details>
  <summary>Details</summary>
Motivation: 随着移动机器人在共享空间中与人类协作日益增多，确保安全、高效和可解释的人机交互（HRI）成为紧迫挑战。现有研究多关注人类行为预测，但对人类如何感知、解释和信任机器人推断关注不足，这阻碍了机器人在安全关键和社会嵌入环境中的部署。

Method: XR-DT框架是一个分层架构，整合了虚拟现实、增强现实和混合现实层，融合实时传感器数据、Unity游戏引擎中的模拟环境以及通过可穿戴AR设备捕获的人类反馈。在该框架内，设计了一个具有统一扩散策略的智能移动机器人系统，用于上下文感知的任务适应。此外，提出了一种思维链提示机制，使多模态大语言模型能对人类指令和环境上下文进行推理，并利用基于AutoGen的多智能体协调层来增强动态任务的鲁棒性和协作性。

Result: 初步实验结果表明，该框架能准确预测人类和机器人的轨迹，验证了XR-DT框架在人机交互任务中的有效性。

Conclusion: 通过将人类意图、环境动态和机器人认知嵌入XR-DT框架，该系统能够实现可解释、可信赖和自适应的人机交互。
...
### [144] [Disturbance Compensation for Safe Kinematic Control of Robotic Systems with Closed Architecture](https://arxiv.org/abs/2512.05292)
*Fan Zhang,Jinfeng Chen,Joseph J. B. Mvogo Ahanda,Hanz Richter,Ge Lv,Bin Hu,Qin Lin*

Main category: cs.RO

TL;DR: 本文提出一种易于集成的外环附加控制器，结合扰动抑制控制和鲁棒控制屏障函数，用于提高商业机械臂的跟踪性能和安全性，即使面对不可修改的内环控制器和显著的动力学模型不确定性。


<details>
  <summary>Details</summary>
Motivation: 商业机器人系统中，内环力矩控制器通常是不可修改且不完善的，而外环控制器对用户开放。研究动机在于，当内环控制器存在缺陷、不可修改且不确定，以及动力学模型存在显著不确定性时，需要一种方法在外环层面提升整个动态系统的控制性能和安全性。

Method: 该研究在外环层开发了一个附加控制器，将扰动抑制控制（disturbance rejection control）与鲁棒控制屏障函数（robust control barrier function）相结合，以实现高性能跟踪和安全控制。方法包括稳定性分析、形式化安全保证证明以及硬件实验。

Result: 所提出的解决方案在实现简易性、鲁棒性、跟踪精度和安全性方面表现出优于现有技术的性能。通过稳定性分析、形式化安全保证证明和PUMA机械臂上的硬件实验验证了其有效性。

Conclusion: 该外环附加控制器成功地提升了工业机械臂的跟踪精度和安全性能，即使在内环控制器不完善、不可修改且不确定，以及动力学模型存在显著不确定性的复杂商业机器人系统中，也能实现高性能控制。
...
### [145] [State-Conditional Adversarial Learning: An Off-Policy Visual Domain Transfer Method for End-to-End Imitation Learning](https://arxiv.org/abs/2512.05335)
*Yuxiang Liu,Shengfan Cao*

Main category: cs.RO

TL;DR: 本文提出了一种名为SCAL的条件对抗学习框架，用于在目标域数据稀缺、离策略且无专家示范的挑战性设置下，实现端到端模仿学习的视觉域迁移。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了解决视觉域迁移在端到端模仿学习中面临的实际挑战，即目标域数据严格离策略、无专家示范且稀缺。

Method: 首先，通过理论分析证明目标域模仿损失可以通过源域损失加上源域和目标域观测模型之间状态条件下的潜在KL散度进行上界估计。在此指导下，提出了状态条件对抗学习（SCAL），这是一个离策略的对抗框架，它利用基于判别器的条件KL项估计器，对系统状态条件下的潜在分布进行对齐。

Result: 在基于BARC-CARLA模拟器构建的视觉多样化自动驾驶环境中进行的实验表明，SCAL实现了鲁棒的迁移和强大的样本效率。

Conclusion: SCAL是一种在目标域数据受限的挑战性模仿学习场景中，实现视觉域迁移的有效且高效的方法。
...
### [146] [Seabed-to-Sky Mapping of Maritime Environments with a Dual Orthogonal SONAR and LiDAR Sensor Suite](https://arxiv.org/abs/2512.05303)
*Christian Westerdahl,Jonas Poulsen,Daniel Holmelund,Peter Nicholas Hansen,Fletcher Thompson,Roberto Galeazzi*

Main category: cs.RO

TL;DR: 本文提出了一种独立的、不依赖GNSS的测绘系统，该系统融合了激光雷达-IMU和双正交前视声纳（FLS），可从自主水面车辆生成统一的“海底到天空”地图，并实现了实时操作。


<details>
  <summary>Details</summary>
Motivation: 关键海洋基础设施日益需要水上和水下态势感知能力，但现有“海底到天空”测绘管线要么依赖易受干扰/欺骗的GNSS，要么使用昂贵的测深声纳。

Method: 该系统融合了激光雷达-IMU和双正交安装的前视声纳（FLS），以实现GNSS独立的测绘。在声学方面，扩展了正交宽孔径融合以处理任意声纳间平移，并从每个FLS提取前沿形成线扫描。在测绘方面，修改了LIO-SAM，通过运动插值位姿在关键帧及之间摄取立体衍生的3D声纳点和前沿线扫描，使稀疏声学更新能持续贡献到一个单一的因子图地图中。

Result: 该系统在哥本哈根Belvederekanalen的真实世界数据上进行了验证，实现了实时操作，地图更新频率约为2.65 Hz，里程计频率约为2.85 Hz，并生成了一个跨越水面和水下区域的统一3D模型。

Conclusion: 该系统成功实现了从自主水面车辆进行实时的、统一的、不依赖GNSS的“海底到天空”测绘，有效解决了现有方法的局限性，为关键海洋基础设施提供了重要的态势感知能力。
...
### [147] [A Hyperspectral Imaging Guided Robotic Grasping System](https://arxiv.org/abs/2512.05578)
*Zheng Sun,Zhipeng Dong,Shixiong Wang,Zhongyi Chu,Fei Chen*

Main category: cs.RO

TL;DR: 本文提出了一种新型高光谱成像引导的机器人抓取系统，通过PRISM和SpectralGrasp框架，显著提高了纺织品识别和分拣成功率。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像与机器人抓取系统的集成因部署复杂性和高成本而探索不足，存在简化系统集成和降低成本的需求。

Method: 该系统包含PRISM（多面反射成像扫描机制），用于实现高精度、无畸变的高光谱成像，并简化系统集成和降低成本。同时，引入SpectralGrasp框架，利用高光谱图像的空间和光谱信息生成机器人抓取策略。

Result: 与人类表现相比，纺织品识别能力显著提高；与基于RGB的方法相比，分拣成功率显著提高。一系列对比实验进一步验证了该系统的有效性。

Conclusion: 该研究强调了将高光谱成像与机器人抓取系统相结合的潜在益处，展示了在复杂动态环境中增强的识别和抓取能力。
...
### [148] [Spatiotemporal Tubes for Differential Drive Robots with Model Uncertainty](https://arxiv.org/abs/2512.05495)
*Ratnangshu Das,Ahan Basu,Christos Verginis,Pushpak Jagtap*

Main category: cs.RO

TL;DR: 本文提出了一种基于时空管（STT）的控制框架，用于具有动态不确定性和外部扰动的差分驱动移动机器人，以保证满足时序到达-避障-保持（T-RAS）规范。该框架通过采样合成算法构建STT，并设计了一种解析的、无近似的控制律。


<details>
  <summary>Details</summary>
Motivation: 在存在动态不确定性和外部扰动的情况下，如何使差分驱动移动机器人满足复杂的时序到达-避障-保持（T-RAS）规范，同时保持鲁棒性、准确性和计算效率，是本研究的动机。

Method: 该方法采用圆形时空管（STT）定义动态安全走廊，其特点是中心和半径随时间平滑变化。首先，开发了一种基于采样的合成算法来构建满足规定时序和安全约束的STT。然后，设计了一种解析的、闭式、无近似的控制律，以确保机器人保持在管内。

Result: 所提出的控制器计算效率高，对扰动和模型不确定性具有鲁棒性，并且不需要模型近似或在线优化。仿真研究验证了该框架在鲁棒性、准确性和计算效率方面优于现有最先进的方法。

Conclusion: 本文提出了一种鲁棒、准确且计算高效的基于时空管的控制框架，能够使差分驱动移动机器人在存在动态不确定性和外部扰动的情况下，可靠地满足时序到达-避障-保持（T-RAS）规范。
...
### [149] [An Integrated System for WEEE Sorting Employing X-ray Imaging, AI-based Object Detection and Segmentation, and Delta Robot Manipulation](https://arxiv.org/abs/2512.05599)
...
TL;DR: 本文提出了一种新颖的自动化系统，结合双能X射线成像、先进的AI模型（YOLO和U-Net）和Delta机器人，用于精确检测和从废弃电子电气设备（WEEE）中分拣电池。


<details>
  <summary>Details</summary>
Motivation: 随着电池使用量激增和自然资源有限，电池回收变得至关重要。此外，高能量密度电池在回收过程中处理不当会带来火灾等严重安全隐患。尽管现有X射线和RGB视觉检测方法有所进步，但仍缺乏能够准确识别和分拣各种WEEE中电池的完全自动化解决方案。

Method: 该方法集成了一个专业的X射线透射双能成像子系统和先进的预处理算法，实现高对比度图像重建以区分WEEE中的致密和薄材料。设备在传送带上通过高分辨率X射线成像系统，YOLO和U-Net模型精确检测并分割含电池物品。智能跟踪和位置估计算法引导配备吸盘夹具的Delta机器人选择性地提取并丢弃目标设备。该方法在NVIDIA Isaac Sim中的仿真环境和实际设置中均进行了验证。

Result: 该系统实现了高对比度图像重建，能够有效区分WEEE中的不同材料。YOLO和U-Net模型能够精确检测和分割含电池物品。智能跟踪和定位算法成功引导Delta机器人选择性地提取目标设备。

Conclusion: 本文提出了一种集成双能X射线、AI模型和机器人技术的创新方法，为WEEE中的电池检测和自动化分拣提供了一个可行的解决方案。该方法在仿真和真实环境中均得到了验证，有望提高电池回收的安全性和效率。
...
### [150] [A Comprehensive Framework for Automated Quality Control in the Automotive Industry](https://arxiv.org/abs/2512.05579)
*Panagiota Moraiti,Panagiotis Giannikos,Athanasios Mastrogeorgiou,Panagiotis Mavridis,Linghao Zhou,Panagiotis Chatzakos*

Main category: cs.RO

TL;DR: 本文提出一种先进的机器人检测解决方案，利用协作机器人、高分辨率视觉系统和增强的YOLO11n深度学习模型，实现汽车制造中铝制高压压铸件表面和螺纹缺陷的自动化质量控制，具有高精度和实时性能。


<details>
  <summary>Details</summary>
Motivation: 汽车制造业中的质量控制需要自动化，以准确检测铝制高压压铸（HPDC）汽车部件的表面和螺纹缺陷。

Method: 该系统整合了一对协作机器人，每个机器人配备高分辨率基于摄像头的视觉系统、专用镜头和优化照明。使用YOLO11n深度学习模型，并通过图像切片、集成学习和边界框合并等增强功能显著提高性能。此外，应用图像处理技术估算检测到的缺陷范围。

Result: 实验结果表明，该方案实现了实时性能，对各种缺陷具有高精度，同时最大限度地减少了误报。该解决方案具有前景且高度可扩展。

Conclusion: 所提出的机器人检测解决方案为汽车行业的质量控制提供了一个有前景且高度可扩展的方案，能够适应不同的生产环境并满足不断变化的需求。
...
### [151] [Scenario-aware Uncertainty Quantification for Trajectory Prediction with Statistical Guarantees](https://arxiv.org/abs/2512.05682)
*Yiming Shu,Jiahui Xu,Linghuan Kong,Fangni Zhang,Guodong Yin,Chen Sun*

Main category: cs.RO

TL;DR: 该研究提出了一种场景感知的轨迹预测不确定性量化框架，为自动驾驶系统提供预测区间和可靠性评估，以应对异构真实世界场景。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习预测器缺乏适应异构真实世界场景的不确定性感知框架，这对于安全关键的自动驾驶系统至关重要。

Method: 将预测轨迹和真实轨迹投影到Frenet坐标系下的参考路径上；采用CopulaCPTS进行共形校准，为不同场景生成时间预测区间；通过轨迹可靠性判别器(TRD)协同分析平均误差和校准置信区间，建立不同场景的可靠性模型；利用风险感知判别器集成Frenet坐标系下的纵向和横向预测区间，识别关键点并分割轨迹为可靠和不可靠段。

Result: 在nuPlan真实世界数据集上进行了评估，证明了该框架在多样化驾驶环境中进行场景感知不确定性量化和可靠性评估的有效性。

Conclusion: 该框架能够提供场景感知的不确定性量化和可靠性评估，为下游规划模块提供可操作的可靠性结果，从而增强自动驾驶系统的安全性。
...
### [152] [Bayesian Active Inference for Intelligent UAV Anti-Jamming and Adaptive Trajectory Planning](https://arxiv.org/abs/2512.05711)
...
### [153] [HiMoE-VLA: Hierarchical Mixture-of-Experts for Generalist Vision-Language-Action Policies](https://arxiv.org/abs/2512.05693)
...
### [154] [3D Path Planning for Robot-assisted Vertebroplasty from Arbitrary Bi-plane X-ray via Differentiable Rendering](https://arxiv.org/abs/2512.05803)
...
### [155] [Global stability of vehicle-with-driver dynamics via Sum-of-Squares programming](https://arxiv.org/abs/2512.05806)
...
### [156] [Real-time Remote Tracking and Autonomous Planning for Whale Rendezvous using Robots](https://arxiv.org/abs/2512.05808)
*Sushmita Bhattacharya,Ninad Jadhav,Hammad Izhar,Karen Li,Kevin George,Robert Wood,Stephanie Gil*

Main category: cs.RO

TL;DR: 本文介绍了一种使用自主无人机在海上实时与抹香鲸会合的系统，该系统利用基于模型的强化学习结合传感器数据和鲸鱼潜水模型来指导导航。


<details>
  <summary>Details</summary>
Motivation: 研究抹香鲸在海上的实时行为面临多重挑战，包括多鲸情况下的实时声学跟踪、机器人部署的分布式通信和决策、以及机载信号处理和鱼类追踪器的远程探测。

Method: 该系统采用自主无人机，并结合了基于模型的强化学习方法。此方法整合了现场传感器数据和经验性的鲸鱼潜水模型来指导导航决策。它还解决了实时声学跟踪、分布式通信和决策以及机载信号处理和长距离探测等关键挑战。

Result: 通过在多米尼克海域与抹香鲸进行会合、陆地硬件实验以及使用海洋生物学家观测数据插值鲸鱼轨迹的模拟，对系统进行了评估。

Conclusion: 本文介绍并评估了一个用于在海上实时与抹香鲸会合的系统，展示了利用自主无人机和先进AI技术进行海洋生物交互的潜力。
...
### [157] [Toward Efficient and Robust Behavior Models for Multi-Agent Driving Simulation](https://arxiv.org/abs/2512.05812)
...
### [158] [Optimal Safety-Aware Scheduling for Multi-Agent Aerial 3D Printing with Utility Maximization under Dependency Constraints](https://arxiv.org/abs/2512.05815)
*Marios-Nektarios Stamatopoulos,Shridhar Velhal,Avijit Banerjee,George Nikolakopoulos*

Main category: cs.RO

TL;DR: 本文提出了一种新颖的协调与任务规划框架，旨在实现多无人机（UAV）在空中3D打印中同时进行无冲突协作，优化任务分配、调度和无人机数量，并确保飞行安全。


<details>
  <summary>Details</summary>
Motivation: 研究动机是实现多无人机在空中3D打印中的同时、无冲突协作，解决任务分配、调度、资源（体积、电池）限制以及无人机间安全避碰等挑战。

Method: 该方法构建了一个优化问题，考虑子任务、无人机团队、有限的体积和电池，生成最优任务计划，包括任务分配和调度。它考虑了任务依赖性、无人机间安全约束、材料使用和总飞行时间。通过动态选择任务的起始时间和位置，在分段层面解决了潜在冲突。引入了重要性优先级以加速计算，并提出了效用最大化公式以动态确定最优无人机数量。通过基于Gazebo的仿真设置评估了该框架的有效性。

Result: 该框架能够生成最优任务计划，实现多无人机在空中3D打印中的同时、无冲突协作。它有效处理了任务依赖性、保障了无人机间安全，并能在最小化完工时间和避免部署过多代理之间取得平衡。仿真结果验证了其在协调无人机执行打印任务、同时遵守材料和电池约束方面的有效性。

Conclusion: 本文提出的框架成功地为多无人机空中3D打印提供了一个全面的协调和任务规划解决方案，通过优化任务分配、调度和动态无人机部署，实现了高效、无冲突的并行操作，并在仿真中得到了验证。
...
### [159] [Physically-Based Simulation of Automotive LiDAR](https://arxiv.org/abs/2512.05932)
*L. Dudzik,M. Roschani,A. Sielemann,K. Trampert,J. Ziehn,J. Beyerer,C. Neumann*

Main category: cs.RO

TL;DR: 本文提出了一种用于模拟汽车飞行时间（ToF）激光雷达的分析模型，该模型考虑了光晕、回波脉冲宽度和环境光，并通过光学实验室测量系统地确定模型参数。


<details>
  <summary>Details</summary>
Motivation: 研究动机是需要一个全面的、系统化的激光雷达仿真模型，能够准确模拟汽车应用中的ToF激光雷达行为，包括其对各种物理现象（如光晕、回波宽度和环境光）的响应。

Method: 该模型采用近红外域的物理渲染（PBR），假设单次反射和逆反射。它模拟了传感器发射的光束、接收二极管的灵敏度以及来自非相关源（如阳光）的杂散光。模型参数（包括光束扩散、二极管灵敏度、发射光强度和回波脉冲宽度转换）通过使用测角仪以0.01°分辨率测量不同目标表面上的光度亮度来系统地确定。

Result: 该方法已成功校准并测试了两种不同的汽车激光雷达系统（Valeo Scala Gen. 2和Blickfeld Cube 1）。尽管这些系统在性能和接口上存在显著差异，但相关的模型参数都能够成功提取。

Conclusion: 本文提出了一种全面的分析模型，能够模拟汽车ToF激光雷达，并首次实现了通过光学实验室测量系统地确定模型参数。该模型已成功应用于不同类型的商用激光雷达系统。
...
### [160] [Correspondence-Oriented Imitation Learning: Flexible Visuomotor Control with 3D Conditioning](https://arxiv.org/abs/2512.05953)
*Yunhao Cao,Zubin Bhaumik,Jessie Jia,Xingyi He,Kuan Fang*

Main category: cs.RO

TL;DR: COIL是一个面向3D关键点运动的模仿学习框架，支持灵活的任务表示和可变粒度，通过时空注意力机制和自监督训练，在真实世界操作任务中表现出优越的泛化性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉运动控制方法在任务表示上缺乏灵活性，通常假设固定数量的关键点或均匀的时间间隔，难以适应不同的用户意图和任务需求。

Method: 引入了Correspondence-Oriented Imitation Learning (COIL) 框架。任务通过场景中物体上选定关键点的预期运动来定义，支持可变的空间和时间粒度。设计了一个带有时空注意力机制的条件策略，有效融合多模态输入。策略通过可扩展的自监督管道进行训练，利用模拟演示并在事后自动生成对应标签。

Result: COIL在任务、物体和运动模式上均表现出良好的泛化能力。在真实世界操作任务中，无论是在稀疏还是密集规范下，其性能均优于现有方法。

Conclusion: COIL通过其独特的关键点运动任务表示和时空注意力机制，提供了一种灵活、泛化且高性能的视觉运动控制方法，在真实世界操作中取得了显著成果。
...
### [161] [SIMPACT: Simulation-Enabled Action Planning using Vision-Language Models](https://arxiv.org/abs/2512.05955)
*Haowen Liu,Shaoxiong Yao,Haonan Chen,Jiawei Gao,Jiayuan Mao,Jia-Bin Huang,Yilun Du*

Main category: cs.RO

TL;DR: SIMPACT是一个测试时模拟驱动的动作规划框架，它通过将物理模拟集成到其推理循环中，为视觉语言模型（VLM）赋予物理理解能力，使其能够更好地执行精细机器人操作任务，而无需额外训练。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）虽具备卓越的常识和语义推理能力，但由于其训练数据缺乏因果交互和动作条件变化，导致它们对物理动力学缺乏基础理解。这限制了VLM在需要物理理解、推理和相应动作规划的精细机器人操作任务中的应用。

Method: SIMPACT是一个测试时、模拟驱动的动作规划框架。它无需额外训练，通过循环中的模拟世界建模为VLM提供物理推理能力。从单一RGB-D观测出发，SIMPACT高效构建物理模拟，使VLM能够提出明智的动作、观察模拟结果并迭代优化其推理。它将语言推理与物理预测相结合，以物理接地的方式理解接触动力学和动作结果。

Result: 该方法在五个具有挑战性的真实世界刚体和可变形操作任务中展现了最先进的性能，这些任务需要精细的物理推理，并且优于现有的通用机器人操作模型。

Conclusion: 在测试时通过高效模拟将物理理解嵌入到VLM推理中，为实现可泛化的具身智能提供了一条有前景的途径。
...
### [162] [Training-Time Action Conditioning for Efficient Real-Time Chunking](https://arxiv.org/abs/2512.05964)
...
### [163] [ARCAS: An Augmented Reality Collision Avoidance System with SLAM-Based Tracking for Enhancing VRU Safety](https://arxiv.org/abs/2512.05299)
*Ahmad Yehia,Jiseop Byeon,Tianyi Wang,Huihai Wang,Yiming Xu,Junfeng Jiao,Christian Claudel*

Main category: eess.SY

TL;DR: 本文提出ARCAS，一个实时的增强现实防撞系统，通过可穿戴AR头显为脆弱道路使用者（VRU）提供个性化空间警报，融合路边激光雷达和头显追踪，显著提高了行人的碰撞预警时间和反应裕度。


<details>
  <summary>Details</summary>
Motivation: 脆弱道路使用者在混合交通中面临高碰撞风险，而现有安全系统主要优先考虑驾驶员或车辆辅助，而非直接支持脆弱道路使用者。

Method: ARCAS系统融合了路边360度3D激光雷达数据、基于SLAM的头显追踪技术以及自动3D校准程序。它通过AR头显在用户的透视视图中精确叠加世界锁定的3D边界框和方向箭头来警示接近的危险。系统还通过共享世界锚定实现多头显协调。

Result: 在与电动滑板车和车辆的真实世界行人互动评估中（180次试验），ARCAS使行人的碰撞时间几乎翻倍，并将对方的反应裕度提高了四倍，远优于无辅助条件。结果验证了激光雷达驱动的AR指导的可行性和有效性。

Conclusion: 激光雷达驱动的AR指导是可行且有效的，可穿戴AR作为城市出行的新一代安全工具具有巨大潜力。
...
### [164] [Data-Driven Adaptive Output Regulation of Unknown Linear Systems](https://arxiv.org/abs/2512.05390)
*Shangkun Liu,Lei Wang,Bowen Yi*

Main category: eess.SY

TL;DR: 本文提出了一种数据驱动的调节器，用于解决外系统和被控对象均完全未知情况下的线性输出调节问题，无需模型辨识即可实现渐近调节和闭环稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统方法通常需要模型辨识，而本文旨在解决外系统和被控对象完全未知时，如何实现线性输出调节，即在缺乏系统先验知识的情况下，使输出渐近跟踪参考信号并保持闭环稳定。

Method: 该方法构建了一个名义近似内模以及输入输出滤波器，从而得到一个可镇定的级联名义系统，其状态是可用的。利用离线实验数据集为该名义系统推导出一个镇定律，使系统状态指数收敛到一个子空间。随后，实现一个离散时间辨识器来修正内模并更新镇定律。

Result: 通过这种方法，在满足一定的持续激励条件下，系统能够实现渐近调节和闭环稳定性，调节误差可以渐近地趋于零。

Conclusion: 研究表明，即使在被控对象和外系统完全未知的情况下，通过数据驱动的方法，结合近似内模、滤波器、离线数据学习和在线辨识更新，也能够成功解决线性输出调节问题，实现渐近调节和闭环稳定，而无需进行显式的模型辨识。
...
### [165] [Comparative Analysis of Barrier-like Function Methods for Reach-Avoid Verification in Stochastic Discrete-Time Systems](https://arxiv.org/abs/2512.05348)
*Zhipeng Cao,Peixin Wang,Luke Ong,Đorđe Žikelić,Dominik Wagner,Bai Xue*

Main category: eess.SY

TL;DR: 本文比较了几种用于随机离散时间系统无限 horizon 避碰验证的代表性类障碍条件。


<details>
  <summary>Details</summary>
Motivation: 现有文献中存在多种用于随机离散时间系统无限 horizon 避碰验证的类障碍条件，但对其理论特性、计算可行性、适用性及保守性的全面比较和评估尚不明确。

Method: 本文比较了几种代表性的类障碍条件，分析了它们的理论性质和计算可行性，并通过使用半定规划 (SDP) 和反例引导归纳综合 (CEGIS) 的计算实验来展示其实际性能。

Result: 研究揭示了每种条件的优点和局限性，这些特点影响了它们的适用性和保守性，并通过计算实验说明了它们的实际性能。

Conclusion: 通过对多种类障碍条件的理论和计算性能进行比较，本文突出了它们在随机离散时间系统避碰验证中的优势和不足，为选择合适的验证方法提供了依据。
...
### [166] [Hybrid modeling approach for better identification of building thermal network model and improved prediction](https://arxiv.org/abs/2512.05400)
*Sang woo Ham,Donghun Kim*

Main category: eess.SY

TL;DR: 本文提出了一种混合建模方法，将灰箱模型与针对未测量扰动（如人员、照明）的模型相结合，通过统计测试进行模型选择，显著提高了建筑温度预测的准确性，适用于不同气候条件和模型预测控制（MPC）应用。


<details>
  <summary>Details</summary>
Motivation: 灰箱模型在建筑预测应用（如MPC）中广泛使用，但其在实际建筑中面临未测量扰动（如人员、照明、渗透负荷）的挑战，难以实际应用。

Method: 本研究提出了一种混合建模方法，将灰箱模型与未测量扰动模型相结合。通过审查多种系统识别方法，并基于统计测试进行模型选择，系统地设计了鲁棒的未测量扰动模型。使用真实运行数据校准的建筑模型生成数据，并针对两种不同天气条件（温和和寒冷气候）训练了混合模型。此外，该方法还应用于实验室建筑的实验数据，以用于MPC应用。

Result: 与传统方法相比，混合模型在提前1天的温度预测中显著降低了RMSE。在温和气候（加利福尼亚州伯克利）下，RMSE降低了约0.2-0.9°C；在寒冷气候（伊利诺伊州芝加哥）下，RMSE降低了约0.3-2°C。此外，该方法在实验室建筑的实验数据上显示出卓越的预测性能，适用于MPC应用。

Conclusion: 所提出的混合建模方法通过集成灰箱模型与鲁棒的未测量扰动模型，有效解决了实际建筑中未测量扰动带来的挑战，显著提高了建筑温度预测的准确性，并证明了其在不同气候条件和模型预测控制应用中的优越性。
...
### [167] [Privacy-Preserving Fully Distributed Gaussian Process Regression](https://arxiv.org/abs/2512.05473)
*Yeongjun Jang,Kaoru Teranishi,Jihoon Suh,Takashi Tanaka*

Main category: eess.SY

TL;DR: 本文提出了一种基于安全多方计算（SMPC）的隐私保护全分布式高斯过程回归（GPR）协议，解决了分布式GPR中的数据隐私泄露问题，并支持核超参数优化。


<details>
  <summary>Details</summary>
Motivation: 尽管分布式高斯过程回归（GPR）允许多个智能体协作学习目标函数模型，但其协作性质存在私有数据泄露的风险。研究旨在解决这一隐私问题。

Method: 本文提出了一种基于安全多方计算（SMPC）的隐私保护全分布式GPR协议。该协议建立在安全分布式平均共识算法之上，并采用基于仿真的安全性范式提供形式化的隐私保证。此外，该协议还扩展以支持核超参数优化。

Result: 该协议保证每个智能体的局部模型实际上收敛到与标准分布式GPR相同的全局模型。实验结果证明了所提方法的有效性和实际适用性，并提供了形式化的隐私保证。

Conclusion: 所提出的隐私保护全分布式GPR协议有效且实际可行，能够确保在分布式GPR中各智能体本地数据集的机密性，同时支持关键的核超参数优化。
...
### [168] [Solving Multiparametric Generalized Nash Equilibrium Problems and Explicit Game-Theoretic Model Predictive Control](https://arxiv.org/abs/2512.05505)
*Sophie Hall,Alberto Bemporad*

Main category: eess.SY

TL;DR: 本文提出了一种计算参数化广义纳什均衡（GNE）问题的显式解的方法，该问题具有凸二次成本函数和线性约束，并将其应用于博弈论模型预测控制（MPC）。


<details>
  <summary>Details</summary>
Motivation: 解决参数化GNE问题在实时计算、可解释性、多重均衡枚举以及参数变化时零次更新等方面的挑战，特别是在博弈论MPC等应用中对高效和可预测解决方案的需求。

Method: 该方法针对具有凸二次成本函数、线性耦合和局部约束的参数化GNE问题，其中参数仅出现在成本函数的线性项和约束的右侧，提供精确的多参数化解。这类似于显式模型预测控制（MPC）的方法。

Result: 所提出的解决方案实现了最小的实时计算、固有的可解释性、可解释性和所有多重均衡的精确枚举，能够在存在无限多个均衡时确定所需的GNE解决方案类型，并允许因约束右侧和/或线性成本变化而进行零次更新。该方法被应用于显式解决博弈论MPC（循环视野博弈），并在电池充电博弈和双质量弹簧系统控制问题中与集中式求解器进行了性能比较，表现良好。

Conclusion: 该方法为参数化GNE问题提供了一种高效、显式的解决方案，具有显著的实时计算、分析和适应性优势，尤其适用于博弈论MPC等领域。
...
### [169] [Constructive boundary observer-based control of high-dimensional semilinear heat equations](https://arxiv.org/abs/2512.05547)
*Pengfei Wang,Emilia Fridman*

Main category: eess.SY

TL;DR: 本文提出了一种针对半线性高维热方程的有限维输出反馈控制器设计方法，通过模态分解解决了拉普拉斯特征值增长慢的挑战，并分析了乘性噪声和加性噪声下的系统鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在高维（M≥2）半线性热方程中，拉普拉斯特征值的增长速度较慢，这给输出反馈控制器设计带来了挑战。研究旨在开发一种能够处理高维问题、允许更大Lipschitz常数、并对不同类型噪声具有鲁棒性的控制器。

Method: 该方法基于模态分解，引入了新颖的形状函数（可部分分布在边界上），并设计了相应的提升变换。控制器增益通过线性矩阵不等式（LMIs）确定。此外，还分析了闭环系统对乘性噪声（在原点消失）和加性噪声（持续存在）的鲁棒性。

Result: 研究提供了有效的LMI条件，用于确定最小观测器维度和最大Lipschitz常数，以保持稳定性（乘性噪声下的均方指数稳定性，加性噪声下的噪声-状态稳定性）。2D和3D数值示例证明了该方法的有效性和优势。

Conclusion: 所提出的有限维输出反馈设计方法成功地解决了高维半线性热方程的控制问题，通过模态分解、新型形状函数和LMI条件实现了对更大Lipschitz常数的支持，并对乘性噪声和加性噪声表现出良好的鲁棒性。
...
### [170] [PAC One-Step Safety Certification for Black-Box Discrete-Time Stochastic Systems](https://arxiv.org/abs/2512.05549)
*Taoran Wu,Dominik Wagner,Jingduo Pan,Luke Ong,Arvind Easwaran,Bai Xue*

Main category: eess.SY

TL;DR: 本文针对黑盒离散时间随机系统，提出了一种数据驱动的PAC（可能近似正确）意义上的一步安全认证框架。


<details>
  <summary>Details</summary>
Motivation: 当系统动力学和扰动分布未知，且仅有采样数据可用时，传统的鲁棒或定量安全认证在有限或无限时间范围内通常不可行。

Method: 提出一个数据驱动的框架，通过基于采样数据构建障碍证书条件，并利用VC维、场景方法、马尔可夫不等式和霍夫丁不等式来建立PAC安全保证。论文提出了两种采样程序和三种推导PAC安全保证的方法。

Result: 该方法提供理论上的一步PAC安全保证，并可递归应用于扩展的时间范围。文章详细讨论了所提出三种方法的特性和比较优势，并通过数值例子验证了其有效性。

Conclusion: 所提出的数据驱动方法能有效为信息有限的黑盒离散时间随机系统提供PAC意义上的安全认证。
...
### [171] [Supervisory Measurement-Guided Noise Covariance Estimation: Discussing Forward and Reverse Differentiation](https://arxiv.org/abs/2512.05604)
*Haoying Li,Yifan Peng,Yuchi Wu,Junfeng Wu*

Main category: eess.SY

TL;DR: 本文提出一种双层优化方法，通过分解联合似然将噪声协方差估计问题转化为马尔可夫链结构，利用卡尔曼滤波和高效梯度更新，实现线性高斯系统中噪声协方差的系统化和计算高效估计。


<details>
  <summary>Details</summary>
Motivation: 可靠的状态估计依赖于准确建模的噪声协方差，但在实际中这些协方差很难确定。

Method: 将噪声协方差估计公式化为双层优化问题。通过分解初级和监督测量的联合似然，将嵌套的贝叶斯依赖转化为马尔可夫链结构以提高计算效率。下层使用带状态增强的卡尔曼滤波器进行计算，而上层则通过闭式前向和反向微分提供高效梯度来更新噪声协方差。同时，比较了两种模型的空间和时间复杂度。

Result: 所提出的算法为线性高斯系统中的噪声协方差估计提供了一种系统化且计算高效的方法。

Conclusion: 该研究提供了一种系统化且计算高效的算法，用于解决线性高斯系统中难以确定的噪声协方差估计问题，从而支持更可靠的状态估计。
...
### [172] [Inverse Linear-Quadratic Gaussian Differential Games](https://arxiv.org/abs/2512.05552)
*Lucas Günther,Felix Thömmes,Karl Handwerker,Balint Varga,Sören Hohmann*

Main category: eess.SY

TL;DR: 本文提出了一种解决有限时域线性二次高斯（LQG）微分博弈中逆随机微分博弈（ISDG）问题的方法，旨在从观测轨迹中恢复所有参与者的成本函数参数和系统噪声尺度参数。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于从实际观测到的轨迹中，反向推断出随机微分博弈中各参与者的成本函数参数以及系统所固有的噪声特性。

Method: 所提出的框架结合了三方面：(i) 估计反馈策略；(ii) 通过对耦合Riccati微分方程的新颖重构来识别成本函数参数；(iii) 对噪声尺度参数进行最大似然估计。

Result: 仿真结果表明，该方法能够成功恢复参数，并生成与观测轨迹高度匹配的轨迹。

Conclusion: 该方法有效解决了LQG微分博弈中的ISDG问题，能够从观测数据中准确恢复参与者成本和系统噪声参数。
...
### [173] [Feedback stabilization of some fourth-order nonlinear parabolic equations with saturated controlsEQUATIONS WITH SATURATED CONTROLS](https://arxiv.org/abs/2512.05606)
*Patricio Guzmán,Felipe Labra,Hugo Parada*

Main category: eess.SY

TL;DR: 本文通过饱和反馈控制，利用谱分析、模态分解和线性矩阵不等式，实现了Cahn-Hilliard和Kuramoto-Sivashinsky方程的内部和边界局部指数稳定。


<details>
  <summary>Details</summary>
Motivation: 研究Cahn-Hilliard和Kuramoto-Sivashinsky方程在饱和反馈控制下的稳定化问题，特别是解决其不稳定性。

Method: 通过对相关线性算子进行谱分析，识别有限数量的不稳定特征值；设计基于模态分解、线性矩阵不等式（LMIs）和饱和函数几何条件的稳定策略。

Result: 建立了在H²空间内的局部指数稳定性。

Conclusion: 通过谱分析结合模态分解、LMIs和饱和函数的几何条件，可以有效地实现Cahn-Hilliard和Kuramoto-Sivashinsky方程在饱和反馈控制下的局部指数稳定。
...
### [174] [A Regularization and Active Learning Method for Identification of Quasi Linear Parameter Varying Systems](https://arxiv.org/abs/2512.05624)
*Sampath Kumar Mulagaleti,Alberto Bemporad*

Main category: eess.SY

TL;DR: 提出一种主动学习方法，通过流形正则化改进准线性参数变化(qLPV)模型的辨识实验设计，以提高外推性能并加速辨识过程。


<details>
  <summary>Details</summary>
Motivation: 辨识准线性参数变化(qLPV)模型需要昂贵的实验。为了最大化信息内容并改善已辨识模型的外推特性，需要选择信息丰富的输入信号。

Method: 提出一种用于设计实验的主动学习方法。引入流形正则化策略，强制qLPV动力学平滑变化以促进线性时变(LTV)行为。在此正则化结构下，提出一种基于逆距离方差测量的路径积分的新型主动学习准则，并利用LTV平滑性推导出高效的近似方法。

Result: 数值示例表明，所提出的正则化增强了qLPV模型的外推能力，并且由此产生的主动学习方案加速了辨识过程。

Conclusion: 结合流形正则化的主动学习方法能有效提高qLPV模型的外推性能并加速其辨识过程。
...
### [175] [A Note on Emergent Behavior in Multi-agent Systems Enabled by Neuro-spike Communication](https://arxiv.org/abs/2512.05654)
*Hyeonyeong Jang,Donghyeon Song,Jin Gyu Lee,Hyungbo Shim*

Main category: eess.SY

TL;DR: 本文提出了一种基于神经尖峰通信的异构多智能体系统新型同步框架，通过间歇性1比特脉冲实现高效同步。


<details>
  <summary>Details</summary>
Motivation: 传统同步策略需要连续传输完整状态数据包，导致带宽和能耗高昂。

Method: 采用受生物启发的神经形态放大器，通过间歇性、异步的1比特狄拉克脉冲实现实际同步。提供了严格的收敛性分析，并通过数值例子验证了方案。

Result: 实现了实际同步，并通过将信息载荷最小化到单个比特，以及间歇和异步通信，显著提高了通信效率（带宽和能量）。

Conclusion: 所提出的神经尖峰通信框架为异构多智能体系统提供了一种高效、实用的同步方法。
...
### [176] [LA-RL: Language Action-guided Reinforcement Learning with Safety Guarantees for Autonomous Highway Driving](https://arxiv.org/abs/2512.05686)
...
### [177] [IMMPC: An Internal Model Based MPC for Rejecting Unknown Disturbances](https://arxiv.org/abs/2512.05692)
...
### [178] [Pauli Decomposition of Impedance Matrices for Understanding the Root Cause of Instabilities in Grid-Connected Power Electronic Converters](https://arxiv.org/abs/2512.05780)
...
### [179] [Task-Specific Trust Evaluation for Multi-Hop Collaborator Selection via GNN-Aided Distributed Agentic AI](https://arxiv.org/abs/2512.05788)
...
### [180] [3D-ICE 4.0: Accurate and efficient thermal modeling for 2.5D/3D heterogeneous chiplet systems](https://arxiv.org/abs/2512.05823)
*Kai Zhu,Darong Huang,Luis Costero,David Atienza*

Main category: eess.SY

TL;DR: 本文介绍了3D-ICE 4.0，一个专为异构2.5D/3D芯片系统设计的先进热建模框架，通过创新方法显著提高了计算效率和精度。


<details>
  <summary>Details</summary>
Motivation: 随着2.5D/3D芯片系统功率密度增加和散热路径复杂化，需要能够提供详细热图且计算效率高的热建模框架。传统紧凑型热模型（CTM）难以应对现代架构的复杂性和异构性。

Method: 本文引入了3D-ICE 4.0，主要创新包括：(i) 直接从工业布局中保留材料的异构性和各向异性，并集成OpenMP和SuperLU MT并行求解器以实现可扩展性能；(ii) 自适应垂直分层以精确模拟垂直热传导；(iii) 温度感知非均匀网格生成。

Result: 3D-ICE 4.0在不同基准测试中，比现有最先进工具提速3.61x-6.46x，同时在不牺牲精度的情况下将网格复杂度降低23.3%以上。与商业软件COMSOL相比，3D-ICE 4.0能有效捕捉横向和垂直热流，验证了其精度和鲁棒性。

Conclusion: 3D-ICE 4.0的这些进展表明，它是新兴异构2.5D/3D集成系统热建模的有效解决方案。
...
### [181] [Safe Output Regulation of Coupled Hyperbolic PDE-ODE Systems](https://arxiv.org/abs/2512.05822)
...
### [182] [A Hybrid Dynamic Model for Predicting Human Cognition and Reliance during Automated Driving](https://arxiv.org/abs/2512.05845)
*Sibibalan Jeevanandam,Neera Jain*

Main category: eess.SY

TL;DR: 本文提出了一种12参数的混合动态模型，用于同时捕捉人类信任、感知风险和心理负荷三种认知状态的连续动态，以及对自动化依赖的离散转换。该模型通过模拟器数据进行参数估计，并能很好地拟合参与者的行为。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解和量化人类在动态环境中（如驾驶）对自动化的信任、感知风险和心理负荷等认知状态如何演变，以及这些状态如何影响对自动化的依赖，从而为以人为中心的自动化设计提供依据。

Method: 研究提出一个包含12个参数的混合动态模型。其中，每个连续认知状态的离散时间动态演变通过一阶仿射差分方程建模；对自动化的依赖则定义为一个离散值状态，其演变取决于认知状态满足特定阈值条件。模型参数使用从16名参与者在车辆模拟器中驾驶时收集的依赖行为和间歇性自我报告认知状态数据进行估计。

Result: 结果显示，该模型能很好地拟合多名参与者的观测轨迹，他们的依赖行为主要受信任、感知风险或两者共同影响。此外，模型具有可解释性，参数变化揭示了认知状态演变的时间尺度差异以及任务复杂性对这些状态的影响。

Conclusion: 该混合动态模型能够有效捕捉人类认知状态与自动化依赖之间的动态关系，且具有良好的可解释性。研究结果对以人为本的车辆自动化设计具有重要启示，有助于更好地理解和预测人机交互行为。
...
### [183] [Advanced Hybrid Automated Insulin Delivery System based on Successive Linearization Model Predictive Control: The UniBE System](https://arxiv.org/abs/2512.05827)
...
### [184] [InstructMPC: A Human-LLM-in-the-Loop Framework for Context-Aware Power Grid Control](https://arxiv.org/abs/2512.05876)
...
### [185] [Log-linear Dynamic Inversion for Thrusting Spacecraft on SE2(3)](https://arxiv.org/abs/2512.05888)
*Micah K. Condie,Abigaile E. Woodbury,Li-Yu Lin,Kartik A. Pant,Mike Walker,James Goppert*

Main category: eess.SY

TL;DR: 本文提出推力航天器动力学可嵌入李群SE2(3)中，通过前馈控制律实现群仿射形式，使得跟踪误差在李代数坐标下精确地线性演化（对数线性动力学），从而可以直接应用线性分析和综合工具。


<details>
  <summary>Details</summary>
Motivation: 传统的局部线性化方法（如Tschauner-Hempel/Yamanaka-Ankersen）难以实现卫星对接、自主交会、近距离操作、鲁棒控制器设计和凸安全认证等方面的严格工具，因此需要一种新的基础特性来解决这些挑战。

Method: 通过应用前馈控制律，将推力航天器的动力学嵌入到李群SE2(3)中，使其呈现群仿射形式。这种结构使得构型跟踪误差在相关联的李代数坐标中精确地线性演化（对数线性动力学）。

Result: 构型跟踪误差在李代数坐标中精确地呈线性演化，而非仅仅是局部线性化结果。数值示例证实了线性李代数动力学预测的误差与完整非线性系统计算的误差精确匹配，验证了这种精确的对数线性行为。

Conclusion: 这种精确对数线性行为的基础特性为卫星对接、自主交会、近距离操作、鲁棒控制器设计和凸安全认证等提供了严格工具的途径，克服了传统局部线性化的局限性。
...
### [186] [CATNUS: Coordinate-Aware Thalamic Nuclei Segmentation Using T1-Weighted MRI](https://arxiv.org/abs/2512.05329)
...
TL;DR: CATNUS是一种基于3D U-Net并增强坐标卷积层的丘脑核分割方法，能精确分割13个丘脑核团，在多种MRI序列上表现出更高的准确性、可靠性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 丘脑核团在脑功能和神经精神疾病中扮演重要角色，但由于其体积小、对比度有限、图像分辨率低以及个体间解剖变异性大，准确分割极具挑战性。

Method: 本文提出了CATNUS（Coordinate-Aware Thalamic Nuclei Segmentation），采用3D U-Net架构并结合坐标卷积层，以实现对大小核团更精确的定位。该模型支持定量T1图以及广泛使用的MPRAGE和FGATIR序列，并提供预训练模型。

Result: CATNUS在分割准确性和测试-重测可靠性方面优于FreeSurfer、THOMAS和HIPS-THOMAS等现有方法。此外，它在跨扫描仪、场强和供应商的旅行受试者数据集上表现出强大的域外泛化能力，生成可靠且解剖学一致的分割结果。

Conclusion: CATNUS为丘脑核团分割提供了一个准确且具有良好泛化能力的解决方案，有望促进大规模神经影像学研究并支持实际临床评估。
...
### [187] [Image Semantic Communication with Quadtree Partition-based Coding](https://arxiv.org/abs/2512.05395)
*Yinhuan Huang,Zhijin Qin*

Main category: eess.IV

TL;DR: 本文提出了一种名为Quad-DeepSC的图像深度语义通信系统，它采用四叉树划分的联合语义-信道编码，在保持低复杂度的同时，首次在不同分辨率数据集上超越了传统通信系统，并实现了最先进的传输性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的图像语义通信（DeepSC）系统在平衡速率-失真性能和计算复杂度方面面临挑战，并且在处理高分辨率数据集时，其性能通常不如传统方案。

Method: 本文提出了一种新颖的图像DeepSC系统，名为Quad-DeepSC，它采用基于四叉树划分的联合语义-信道编码。该系统基于成熟的图像压缩技术，整合了基于四叉树划分的熵估计和特征编码模块，以及轻量级的特征提取和重建网络，形成端到端架构。在训练过程中，除了特征编码模块外，所有组件作为紧凑的学习图像编解码器Quad-LIC进行联合优化以完成源压缩任务。预训练的Quad-LIC随后嵌入到Quad-DeepSC中，并在无线信道上进行端到端微调。

Result: 实验结果表明，Quad-DeepSC是第一个在不同分辨率数据集上性能超越传统通信系统（使用VTM进行源编码，并采用3GPP标准下最优MCS索引进行信道编码和数字调制）的DeepSC系统。此外，Quad-DeepSC和Quad-LIC都表现出极低的延迟。

Conclusion: Quad-DeepSC系统在图像语义通信方面取得了突破性进展，它以低复杂度和低延迟实现了超越传统通信系统的最先进性能，非常适合部署在实时无线通信系统中。
...
### [188] [General and Domain-Specific Zero-shot Detection of Generated Images via Conditional Likelihood](https://arxiv.org/abs/2512.05590)
*Roy Betser,Omer Hofman,Roman Vainshtein,Guy Gilboa*

Main category: eess.IV

TL;DR: 本文提出了一种名为CLIDE的新型零样本检测方法，通过条件似然近似，能够有效检测AI生成图像，并在不同图像领域（特别是特定领域如艺术图像）展现出强大的适应性和卓越性能。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型（尤其是扩散模型）的快速发展，检测AI生成图像成为一项关键挑战。现有全监督和少样本方法需要耗时的数据集更新，而零样本方法则难以适应特定图像领域（如艺术图像），限制了其实际应用。

Method: 本文引入了CLIDE，一种基于条件似然近似的新型零样本检测方法。该方法通过计算以真实图像为条件的似然，实现了在不同图像领域间的适应性。

Result: CLIDE在大型通用数据集上取得了最先进的性能，并在特定领域案例中显著优于现有方法。这些结果证明了该方法的鲁棒性。

Conclusion: CLIDE方法具有强大的鲁棒性，并强调了AI生成图像检测任务中广泛的、领域感知的泛化能力的重要性。
...
### [189] [Two-Stage Camera Calibration Method for Multi-Camera Systems Using Scene Geometry](https://arxiv.org/abs/2512.05171)
*Aleksandr Abramov*

Main category: eess.IV

TL;DR: 本文提出了一种新颖的两阶段多摄像机系统校准方法，通过操作员标注自然几何原语和交互式调整投影视野，克服了传统方法在缺乏平面图、物理访问或同步视频等实际条件下的局限性。


<details>
  <summary>Details</summary>
Motivation: 多摄像机系统校准是精确目标跟踪的关键任务，但在缺乏精确平面图、无法放置校准图案或没有同步视频流的真实世界条件下，传统方法难以适用，这促使研究人员开发一种新的解决方案。

Method: 该方法分为两个阶段：第一阶段，操作员通过标注自然几何原语（平行线、垂直线、竖直线或等长线段）对单个摄像机进行部分校准，以估计俯仰角、滚转角和焦距，并将有效视场（EFOV）投影到水平面。第二阶段，操作员通过交互式操作投影的EFOV多边形（调整位置、缩放和旋转）来与平面图对齐或使用虚拟校准元素，从而实现精确的系统校准，确定摄像机位置和偏航角。该方法仅需每台摄像机的静态图像，无需物理访问或同步视频，并已实现为网络服务。

Result: 通过比较分析和演示视频，该方法被证实具有适用性、准确性和灵活性。它使得在以前被认为不可行的场景中部署精确的多摄像机跟踪系统成为可能。

Conclusion: 该研究提供了一种实用、准确且灵活的多摄像机校准方法，有效解决了在缺乏传统校准条件的复杂真实世界环境中进行多摄像机系统部署的挑战。
