...
TL;DR: 论文探讨了如何通过多层次的防御模型确保AI的安全性，分为开发、评估和部署后监控三个领域。


<details>
  <summary>Details</summary>
Motivation: 随着AI能力的快速提升，确保其安全性、可靠性和可信度成为关键问题，需要建立一个可信的生态系统以促进创新并避免负面影响。

Method: 采用防御纵深模型，将AI安全研究分为开发（可信AI系统）、评估（风险分析）和部署后控制（监控与干预）三个领域。

Result: 报告提出了一个框架，明确了AI安全研究的优先级，并得到33个政府的支持。

Conclusion: 通过国际合作和多层次研究，可以构建更安全的AI生态系统，促进技术创新和社会信任。
...
TL;DR: 论文研究了基于LLM的代理在多轮对话中对上下文隐私的理解和保护能力，发现现有模型在隐私保护和任务协作方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理的普及，隐私保护在多代理协作系统中变得至关重要，但目前缺乏对上下文隐私的全面评估。

Method: 提出了一个名为MAGPIE的基准测试，包含158个高风险场景，评估LLM代理对隐私数据的理解和协作能力。

Result: 实验显示，当前模型（如GPT-4o和Claude-2.7-Sonnet）在隐私分类和多轮对话中表现不佳，隐私泄露率高且任务完成率低。
...
<details>
  <summary>Details</summary>
Motivation: 由于LLM应用对用户提示质量高度敏感，而领域特定应用中高质量提示的编写尤为困难，因此需要一种动态推荐系统来辅助用户。

Method: 系统利用行为遥测和两阶段分层推理过程动态选择和排名相关技能，并通过预定义和自适应模板结合少量学习生成提示。

Result: 在真实数据集上的实验表明，该方法在自动和专家评估中均表现出高实用性和相关性。

Conclusion: 该动态上下文感知提示推荐系统能有效提升领域特定AI应用中提示的质量和用户体验。
...
TL;DR: 论文提出了一种框架，用于评估语言模型建议的长期社会影响，并引入了一个新数据集测试模型对间接危害的预见能力。
...
Result: 方法在新数据集上提升了20%的性能，并在现有安全基准测试中平均胜率超过70%。
...
TL;DR: 论文探讨了大型语言模型（LLMs）的因果推理能力，发现其仅能进行浅层（level-1）推理，缺乏人类深层（level-2）推理能力。通过新基准CausalProbe-2024验证，并提出了G^2-Reasoner方法以提升LLMs的因果推理能力。
...
TL;DR: 论文提出了一种名为WAP的框架，通过四种认知能力增强大型视觉语言模型（LVLM）的环境理解能力，显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂场景中表现不佳，尤其是环境无关的模仿学习导致模型难以处理上下文敏感的指令和依赖视觉推理。

Method: 采用World-Aware Planning Narrative Enhancement（WAP）框架，结合四种认知能力和课程学习，仅使用原始视觉观察进行训练。

Result: 在EB-ALFRED基准测试中，任务成功率显著提升（60.7绝对提升），尤其在常识推理（+60.0）和长时规划（+70.0）方面表现突出。

Conclusion: WAP框架显著提升了LVLM在复杂任务中的表现，甚至超越了GPT-4o等专有系统。
...
TL;DR: IXAII是一个交互式可解释AI系统，结合了多种解释方法（LIME、SHAP、Anchors、DiCE），并为不同用户群体提供定制化视图，提升透明度和用户体验。
...
Method: 开发IXAII系统，整合四种解释方法，提供交互式解释和多种可视化选项。
...
TL;DR: 论文提出通过解决抽象、推理和现实三个关键差距，构建主动推理AI系统以推动科学发现，强调内部模型与外部验证的结合及人类判断的不可或缺性。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在科学发现中存在局限性，如架构僵化、推理脆弱及脱离实验现实，需通过解决抽象、推理和现实差距来推动进步。

Method: 提出主动推理AI系统，包括因果自监督基础模型、贝叶斯防护的符号/神经符号规划器、持久知识图谱构建及闭环交互验证。

Result: 系统通过内部模型与外部验证的结合，实现科学发现，同时强调人类判断在不确定性和模糊反馈中的关键作用。

Conclusion: 科学发现的AI系统需结合内部模型与外部验证，并永久集成人类判断，以解决当前AI的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在复杂表格结构（如模糊、倾斜、嵌套布局等）下表现不佳，需改进。

Method: 提出TableMoE，采用神经符号路由机制，动态分配表格元素至专家模块，并引入大规模对齐数据集TableMoE-Align。

Result: 实验证明TableMoE在WildStruct基准上显著优于SOTA模型，验证了神经符号路由和专家对齐的关键作用。
...
<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型是否能像人类一样通过少数视角构建完整场景的空间心理模型。

Method: 提出MindCube基准测试，评估VLMs在认知映射、视角转换和动态模拟上的表现，并尝试三种方法（中间视角生成、自然语言推理链和认知地图）提升性能。

Result: “map-then-reason”方法将准确率从37.8%提升至60.8%，结合强化学习后达到70.7%。

Conclusion: 构建并利用结构化空间表征的灵活推理过程，显著提升对不可见空间的理解能力。
...
TL;DR: 论文提出了Ad-Hoc Human-AI Coordination Challenge (AH2AC2)，通过开发人类代理代理解决人类评估的高成本和难以复现问题，并在有限数据下推动高效方法的发展。


<details>
  <summary>Details</summary>
Motivation: 实现AI与人类的无缝协调是现实应用的关键，但现有方法在人类评估上存在高成本和难以复现的挑战。

Method: 开发人类代理代理作为评估伙伴，开源有限的人类游戏数据集，并在两玩家和三玩家Hanabi场景中提供基线结果。

Result: 提出了AH2AC2框架，并通过人类代理代理实现了低成本、可复现的评估。

Conclusion: AH2AC2为人类-AI协调研究提供了实用的评估工具，推动了数据高效方法的发展。
...
TL;DR: Mind2Web 2是一个新的基准测试，包含130个长时任务，用于评估自主网络搜索系统的性能，并提出了一种基于树形结构的自动评估框架。
...
Method: 提出Mind2Web 2基准和Agent-as-a-Judge框架，通过任务特定的评判代理自动评估答案正确性和来源标注。
...
Result: 在六个基准测试中，OTSurv的平均C-index绝对提升3.6%，并在对数秩检验中表现出统计显著性。
...
<details>
  <summary>Details</summary>
Motivation: 视频深度估计不仅仅是图像深度估计的简单扩展，静态和动态区域的时空一致性需求不同，需要不同的方法处理。

Method: StereoDiff采用两阶段方法：静态区域通过立体匹配实现全局一致性，动态区域通过视频深度扩散学习平滑过渡。

Result: 在零样本、真实世界动态视频深度基准测试中，StereoDiff表现出色，展示了卓越的一致性和准确性。

Conclusion: StereoDiff通过结合立体匹配和视频深度扩散，显著提升了视频深度估计的性能，尤其是在静态和动态区域的时空一致性上。
...
Method: 提出ConViTac网络，采用对比嵌入条件（CEC）机制，通过自监督对比学习预训练的编码器将视觉和触觉输入映射到统一潜在嵌入，并利用跨模态注意力进行特征融合。

Result: 实验表明，ConViTac在材料分类和抓取预测任务中准确率提升高达12.0%，优于现有方法。

Conclusion: ConViTac通过对比表示和跨模态注意力有效提升了视觉-触觉特征融合的性能。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏对不同可提示模型在医学数据集上的统一评估与比较，研究旨在填补这一空白。

Method: 使用SAM、SAM 2、MedSAM、SAM-Med-3D和nnU-Net在BraTS 2023数据集上进行零样本推理，并评估不同提示质量下的表现。

Result: SAM和SAM 2在精确边界框提示下表现优异（Dice分数达0.894和0.893），但nnU-Net仍占主导。微调后点提示性能显著提升，但仍不及边界框或nnU-Net。

Conclusion: 精确提示下某些模型表现优异，但nnU-Net仍是医学图像分割的最佳选择，未来可进一步探索微调潜力。
...
TL;DR: 研究探讨了基于视觉基础模型（VFMs）和视觉语言模型（VLMs）的动态全身手势识别，比较了V-JEPA、Gemini Flash 2.0和HD-GCN的性能。HD-GCN表现最佳，但V-JEPA接近，展示了简化系统复杂性的潜力。
...
TL;DR: 本文提出了一种基于视觉语言模型（VLM）的自动化框架，用于从扩散模型生成的多张超分辨率（SR）图像中选择最可信的样本，并通过混合指标（TWS）量化其可靠性。


<details>
  <summary>Details</summary>
Motivation: 超分辨率问题存在多个可行解，传统方法在平衡保真度和感知质量时可能引入伪影，而扩散模型生成的多样性样本难以选择最可信的解。

Method: 利用VLM（如BLIP-2、GPT-4o）通过结构化查询评估语义正确性、视觉质量和伪影存在，并设计TWS指标（结合CLIP嵌入、SSIM和小波分解）量化可靠性。

Result: 实验表明TWS与人类偏好高度相关，且VLM引导的选择能稳定获得高TWS值，优于传统指标（如PSNR、LPIPS）。
...
Method: 引入FixCLR，利用伪标签的类别信息和排斥项，适应对比学习以显式正则化领域不变性。

Result: FixCLR显著提升性能，尤其在与其他半监督方法结合时表现更优。

Conclusion: FixCLR是有效的SSDG方法，尤其适合与其他半监督方法结合使用。
...
<details>
  <summary>Details</summary>
Motivation: 传统对比学习在像素级表示中存在过分散问题，破坏了像素级特征相关性，影响医学视觉任务的表现。

Method: 提出向量对比学习（vector CL），将对比学习重新定义为向量回归问题，并通过COVER框架实现向量自学习、优化流一致性和粒度适应。
...
### [22] [Out-of-Distribution Semantic Occupancy Prediction](https://arxiv.org/abs/2506.21185)
*Yuheng Zhang,Mengfei Duan,Kunyu Peng,Yuhang Wang,Ruiping Liu,Fei Teng,Kai Luo,Zhiyong Li,Kailun Yang*

Main category: cs.CV

TL;DR: 该论文提出了Out-of-Distribution Semantic Occupancy Prediction（OccOoD）框架，用于3D体素空间中的OoD检测，并引入了合成异常数据集VAA-KITTI和VAA-KITTI-360。


<details>
  <summary>Details</summary>
Motivation: 现有方法专注于分布内场景，对OoD对象和长尾分布敏感，导致安全隐患。

Method: 提出合成异常集成管道生成数据集，并设计OccOoD框架，结合Voxel-BEV Progressive Fusion（VBPF）增强OoD检测。
...
TL;DR: 提出了一种名为MIDAS的数据增强方法，用于提升动态面部表情识别（DFER）在模糊表情数据上的性能，通过软标签和视频帧的凸组合实现。
...
Method: MIDAS方法通过凸组合视频帧及其对应的软标签来增强训练数据，扩展了mixup方法到软标签视频数据。

Result: 在DFEW和FERV39k-Plus数据集上的实验表明，MIDAS训练模型优于现有方法。

Conclusion: MIDAS是一种简单高效的数据增强方法，显著提升了模糊面部表情识别的性能。
...
### [24] [Unlocking Constraints: Source-Free Occlusion-Aware Seamless Segmentation](https://arxiv.org/abs/2506.21198)
...
### [25] [THIRDEYE: Cue-Aware Monocular Depth Estimation via Brain-Inspired Multi-Stage Fusion](https://arxiv.org/abs/2506.20877)
*Calin Teodor Ioan*

Main category: cs.CV

TL;DR: ThirdEye是一种单目深度估计方法，通过显式提供视觉线索（如遮挡边界、阴影和透视）并融合这些线索，结合记忆模块和自适应分箱变换器，生成高分辨率视差图。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖隐式学习，忽略了人类视觉系统依赖的显式线索。ThirdEye旨在通过显式提供这些线索，提高深度估计的准确性。

Method: 使用预训练且冻结的专家网络提供视觉线索，通过三阶段皮层层次（V1->V2->V3）和记忆模块融合线索，最后用自适应分箱变换器生成视差图。

Result: ThirdEye继承了大量外部监督，仅需少量微调，但具体定量结果将在未来版本中公布。

Conclusion: ThirdEye通过显式利用视觉线索和记忆模块，提供了一种更高效的深度估计方法。
...
### [26] [Generalizable Neural Electromagnetic Inverse Scattering](https://arxiv.org/abs/2506.21349)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法（如Img-Interiors）在EISP中存在泛化性差、依赖特定优化和稀疏发射器下失效的问题，需要一种更通用的物理驱动解决方案。

Method: 提出两阶段框架：1）电流估计器学习诱导电流作为入射场与散射场的桥梁；2）介电常数求解器直接从估计电流计算相对介电常数，实现端到端训练和预测。

Result: 实验表明，该方法在重建精度、泛化性和鲁棒性上优于现有技术，尤其在稀疏发射器条件下表现优异。

Conclusion: 该研究为电磁逆散射提供了新视角，推动了电磁成像实用化的发展。
...
### [27] [MultiHuman-Testbench: Benchmarking Image Generation for Multiple Humans](https://arxiv.org/abs/2506.20879)
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门用于评估多人生成图像的基准，导致生成复杂动作且保留面部身份的挑战难以量化。

Method: 构建包含多样文本提示和人脸图像的基准，提出基于人脸数量、ID相似性、提示对齐和动作检测的评估指标，并引入图像分割和匈牙利匹配技术。

Result: 通过评估多种模型，发现区域先验和新技术显著提高了ID相似性。
...
### [28] [The Role of Cyclopean-Eye in Stereo Vision](https://arxiv.org/abs/2506.20900)
...
TL;DR: 开发了一种名为FaSTA$^*$的神经符号代理，用于高效完成多轮图像编辑任务，结合了LLM的高层规划和A$^*$搜索的低层工具调用，通过复用子程序显著降低成本。


<details>
  <summary>Details</summary>
Motivation: 解决多轮图像编辑任务的高效性和成本问题，结合LLM的快速规划与A$^*$搜索的精确性。

Method: 结合LLM的高层子任务规划和A$^*$搜索的低层工具调用，通过归纳推理复用成功子程序，形成自适应快慢规划策略。

Result: FaSTA$^*$在计算效率上显著优于现有方法，同时保持与最先进基线相当的成功率。

Conclusion: FaSTA$^*$通过快慢规划策略和子程序复用，实现了高效且成本低廉的图像编辑。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在像素级伪造定位中存在计算开销大和表征能力不足的问题，尤其是对复杂或微妙的篡改。

Method: M2SFormer在跳跃连接中统一多频率和多尺度注意力，利用全局上下文捕捉伪造痕迹，并通过全局先验图指导的注意力模块保留细节。

Result: 在多个基准数据集上，M2SFormer优于现有最先进模型，表现出更强的泛化能力。

Conclusion: M2SFormer通过创新设计有效解决了伪造定位中的挑战，为图像编辑检测提供了更优方案。
...
TL;DR: 提出PhysRig，一种基于物理的可微分蒙皮和绑定框架，克服了传统线性混合蒙皮（LBS）的局限性，通过将刚性骨架嵌入体积表示中，模拟为可变形软体结构。


<details>
  <summary>Details</summary>
Motivation: 传统LBS方法存在体积损失、不自然变形等问题，且无法模拟弹性材料（如软组织、毛发等）。

Method: 将刚性骨架嵌入体积表示（如四面体网格），作为可变形软体结构模拟，利用连续介质力学和离散化粒子确保可微分性，并引入材料原型减少学习空间。

Result: 在合成数据集上表现优于传统LBS方法，生成更真实且物理合理的结果，并在姿态迁移任务中验证了其通用性。

Conclusion: PhysRig为动画和物体建模提供了更灵活、真实的解决方案，适用于多种应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 解决连续手语识别中数据不足和模态对齐问题，充分利用文本知识。

Method: 构建层次化子动作树（HST），结合大语言模型的文本知识，逐步对齐视觉与文本模态，并引入对比对齐增强。

Result: 在四个数据集上验证了HST-CSLR的有效性。

Conclusion: HST-CSLR通过模态对齐和知识利用，显著提升了连续手语识别的性能。
...
Method: 开发PathChat+，基于超过100万病理学指令样本和550万问答对训练，并引入SlideSeek系统进行多图像自主推理。

Result: PathChat+在多个病理学基准测试中表现优异，SlideSeek系统在DDxBench上达到高准确性。

Conclusion: PathChat+和SlideSeek系统为病理学提供了更强大的多模态理解和自主诊断能力。
...
TL;DR: DFVEdit是一种针对Video DiTs的高效零样本视频编辑方法，避免了注意力修改和微调，通过流变换直接在潜在空间操作，显著提升了计算效率和编辑质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法应用于Video DiTs时计算开销大，需要改进以降低资源消耗。
...
### [37] [GoIRL: Graph-Oriented Inverse Reinforcement Learning for Multimodal Trajectory Prediction](https://arxiv.org/abs/2506.21121)
*Muleilan Pei,Shaoshuai Shi,Lu Zhang,Peiliang Li,Shaojie Shen*

Main category: cs.CV

TL;DR: 本文提出了一种基于图导向逆强化学习（GoIRL）的轨迹预测框架，通过向量化上下文表示和最大熵IRL范式，实现了多模态轨迹预测，并在多个基准测试中表现优异。
...
Method: 提出GoIRL框架，结合向量化上下文表示和最大熵IRL，通过特征适配器整合车道图特征，并采用分层参数化轨迹生成器和概率融合策略。

Result: 在Argoverse和nuScenes基准测试中达到最优性能，并展现出比现有监督模型更强的泛化能力。

Conclusion: GoIRL框架通过逆强化学习和多模态生成策略，显著提升了轨迹预测的准确性和泛化能力。
...
### [38] [From Cradle to Cane: A Two-Pass Framework for High-Fidelity Lifespan Face Aging](https://arxiv.org/abs/2506.20977)
...
Method: 采用两阶段框架：第一阶段通过自适应噪声注入（AdaNI）实现年龄准确性；第二阶段通过身份感知嵌入（IDEmb）增强身份保持。
...
TL;DR: 提出了一种结合相机光度优化的3D场景表示方法，通过联合优化相机模型参数和场景表示，有效分离与场景无关的信息。


<details>
  <summary>Details</summary>
Motivation: 相机成像中的光度失真会降低图像质量，进而影响3D场景表示的质量，需要一种方法来解决这一问题。

Method: 引入内部和外部光度模型，提出完整的光度模型和相机表示，结合深度正则化优化光度参数。

Result: 实验表明，该方法能在成像退化条件下（如渐晕和污垢）生成高质量的3D场景表示。

Conclusion: 通过联合优化相机模型和场景表示，该方法有效提升了3D场景表示的质量。
...
Result: 在动物和人类图像生成任务中，SP-Ctrl优于其他稀疏姿态引导方法，甚至匹配密集信号方法的性能，并展示了跨物种生成的潜力。

Conclusion: 稀疏信号在姿态引导的图像生成中具有潜力，SP-Ctrl通过改进的关键点表示和概念学习实现了高效控制。
...
TL;DR: 该论文提出了一种名为EVA的混合专家语义变体对齐框架，用于改进组合零样本学习（CZSL），通过多专家学习和语义变体对齐提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有CZSL方法通过简单的组合-原型映射生成原始特征，忽略了语义子集的差异和组合多样性，导致图像-组合对齐效果不佳。

Method: 提出EVA框架，包括领域专家适应（多专家学习）和语义变体对齐（选择语义相关表示），以优化原始特征表示和图像-组合匹配。

Result: 在三个流行基准测试中，EVA在封闭和开放世界设置下显著优于现有最先进方法。

Conclusion: EVA通过多专家学习和语义变体对齐有效提升了CZSL的性能，证明了其方法的有效性。
...
### [42] [Real-Time ESFP: Estimating, Smoothing, Filtering, and Pose-Mapping](https://arxiv.org/abs/2506.21234)
...
<details>
  <summary>Details</summary>
Motivation: 开发一种高效、低成本的方法，将视频动作直接映射到机械臂控制。

Method: 包含四个模块：3D骨架估计（ROMP）、平滑（HPSTM Transformer）、滤波（基于方差加权）和姿态映射（几何重定向）。

Result: 实现了从视频到机械臂动作的稳定转换，同时保持解剖合理性和关节不确定性。

Conclusion: ESFP提供了一种有效的解决方案，适用于低成本机械臂的动作生成。
...
### [43] [Segment Anything in Pathology Images with Natural Language](https://arxiv.org/abs/2506.20988)
*Zhixuan Chen,Junlin Hou,Liqi Lin,Yihui Wang,Yequan Bie,Xi Wang,Yanning Zhou,Ronald Cheong Kin Chan,Hao Chen*

Main category: cs.CV

TL;DR: PathSegmentor是一种基于文本提示的病理图像分割基础模型，解决了现有方法在临床应用中因标注数据有限和类别定义受限的问题。


<details>
  <summary>Details</summary>
Motivation: 解决病理图像分割中标注数据不足和类别定义受限的挑战，提升分割的准确性和适用性。

Method: 提出PathSegmentor模型和PathSeg数据集，支持自然语言提示进行语义分割，无需空间输入。

Result: PathSegmentor在Dice分数上显著优于现有模型，具有更强的鲁棒性和泛化能力。
...
### [44] [ToosiCubix: Monocular 3D Cuboid Labeling via Vehicle Part Annotations](https://arxiv.org/abs/2506.21358)
...
Result: 在KITTI和Cityscapes3D数据集上验证了方法的有效性，提供了一种低成本、高质量的标注方案。

Conclusion: ToosiCubix为3D立方体标注提供了一种实用且可扩展的解决方案。
...
### [45] [TSDASeg: A Two-Stage Model with Direct Alignment for Interactive Point Cloud Segmentation](https://arxiv.org/abs/2506.20991)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在点级任务（如分割）中表现不佳，主要由于缺乏直接的3D-文本对齐，限制了局部3D特征与文本上下文的关联。

Method: TSDASeg采用两阶段模型，结合直接跨模态对齐模块和内存模块，通过多专用内存库存储文本、视觉特征及其跨模态映射，并利用自注意力和跨注意力机制动态更新场景特征。

Result: 在多个3D指令、参考和语义分割数据集上实验表明，该方法达到了最先进的性能。
...
### [46] [EndoFlow-SLAM: Real-Time Endoscopic SLAM with Flow-Constrained Gaussian Splatting](https://arxiv.org/abs/2506.21420)
*Taoyu Wu,Yiyi Miao,Zhuoxiao Li,Haocheng Zhao,Kang Dang,Jionglong Su,Limin Yu,Haoang Li*

Main category: cs.CV

TL;DR: 论文提出了一种结合光流损失和深度正则化策略的3D高斯泼溅（3DGS）SLAM方法，用于内窥镜场景中的高效3D重建和实时可视化。


<details>
  <summary>Details</summary>
Motivation: 内窥镜场景中存在非朗伯表面引起的光度不一致性和呼吸带来的动态运动，影响SLAM系统的性能。

Method: 引入光流损失作为几何约束，并提出深度正则化策略，同时优化3DGS细化策略以提高场景表示。

Result: 在C3VD静态数据集和StereoMIS动态数据集上，方法在视图合成和姿态估计方面优于现有方法。
...
### [47] [Step-by-Step Video-to-Audio Synthesis via Negative Audio Guidance](https://arxiv.org/abs/2506.20995)
*Akio Hayakawa,Masato Ishii,Takashi Shibuya,Yuki Mitsufuji*

Main category: cs.CV

TL;DR: 提出了一种逐步生成视频到音频的方法，通过分步生成特定声音事件的音频轨道，模仿传统Foley工作流程，并利用预训练模型提升合成质量。


<details>
  <summary>Details</summary>
Motivation: 传统视频到音频生成方法难以全面捕捉视频中的所有声音事件，因此需要一种更全面的生成方法。

Method: 采用分步生成策略，每个步骤基于目标文本提示和先前生成的音频轨道进行引导合成，利用预训练模型避免对专用数据集的需求。

Result: 实验表明，该方法能为单个视频生成多个语义不同的音频轨道，合成质量优于现有基线。

Conclusion: 该方法通过分步生成和预训练模型，显著提升了视频到音频合成的质量和多样性。
...
### [48] [Multimodal Prompt Alignment for Facial Expression Recognition](https://arxiv.org/abs/2506.21017)
...
Method: 提出MPA-FER框架，结合多粒度硬提示生成策略和原型引导的视觉特征对齐，通过跨模态全局-局部对齐模块优化文本与视觉特征的对齐。
...
### [49] [SAM4D: Segment Anything in Camera and LiDAR Streams](https://arxiv.org/abs/2506.21547)
*Jianyun Xu,Song Wang,Ziqian Ni,Chunyong Hu,Sheng Yang,Jianke Zhu,Qiang Li*

Main category: cs.CV

TL;DR: SAM4D是一个多模态时序基础模型，用于相机和LiDAR流的可提示分割，通过UMPE和MCMA技术实现跨模态对齐与时序一致性，并通过自动化数据引擎快速生成伪标签。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶场景中相机和LiDAR数据的跨模态分割问题，避免人工标注瓶颈，提升分割的时序一致性和语义保真度。
...
Method: 通过稀疏控制的高斯泼溅技术生成密集3D高斯，从模糊视频恢复清晰度并重建动态场景的3D几何。

Result: 在动态模糊场景中实现了鲁棒的新视角合成性能，为模糊单目视频输入设定了新基准。
...
### [51] [Whole-Body Conditioned Egocentric Video Prediction](https://arxiv.org/abs/2506.21552)
*Yutong Bai,Danny Tran,Amir Bar,Yann LeCun,Trevor Darrell,Jitendra Malik*

Main category: cs.CV

TL;DR: 训练模型通过人体动作预测第一人称视角视频（PEVA），基于过去视频和3D身体姿态动作，利用扩散变换器学习物理动作对环境的影响。


<details>
  <summary>Details</summary>
Motivation: 探索如何从人类视角模拟复杂现实环境和具身代理行为，解决视频预测中的挑战。
...
TL;DR: 提出了一种风格对齐的图像合成方法（SAIC），用于生成高质量病理图像，提升异常细胞检测模型的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决病理图像中高质量标注稀缺、长尾数据分布和染色风格不一致等问题对模型训练的挑战。

Method: 通过属性指导选择异常细胞库中的候选，利用高频特征重建实现风格对齐的图像合成，并结合大视觉语言模型筛选高质量合成图像。

Result: 实验表明，SAIC合成的图像显著提升了尾部类别和风格的异常细胞检测性能及鲁棒性。
...
### [53] [Inverse Scene Text Removal](https://arxiv.org/abs/2506.21002)
*Takumi Yoshimatsu,Shumpei Takezaki,Seiichi Uchida*

Main category: cs.CV

TL;DR: 该论文提出逆向场景文本移除（ISTR），用于检测图像是否经过文本移除处理，并定位被移除的文本区域，实验表明任务可行且准确率高。


<details>
  <summary>Details</summary>
Motivation: 场景文本移除（STR）技术可能被滥用，因此需要一种方法检测和定位被移除的文本，以防止潜在滥用并改进STR技术。

Method: 提出逆向STR（ISTR），通过分析STR处理后的图像，实现二分类（检测是否经过STR）和定位被移除文本区域的任务。

Result: 实验证明ISTR在检测和定位任务中具有高准确率，并能尝试恢复被移除的文本内容。

Conclusion: ISTR为检测STR滥用提供了可行方案，同时有助于改进STR技术。
...
### [54] [Improving Diffusion-Based Image Editing Faithfulness via Guidance and Scheduling](https://arxiv.org/abs/2506.21045)
*Hansam Cho,Seoung Bum Kim*

Main category: cs.CV

TL;DR: 提出了一种名为FGS的方法，通过忠实度引导和调度策略，在文本引导扩散模型中优化图像编辑的忠实度和可编辑性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 文本引导扩散模型在高质量图像合成和动态编辑中表现优异，但忠实度和可编辑性之间存在固有矛盾，需要一种方法在两者之间取得平衡。

Method: FGS通过引入忠实度引导来增强输入图像信息的保留，并采用调度策略解决忠实度与可编辑性之间的错位问题。

Result: 实验表明，FGS在保持可编辑性的同时显著提升了忠实度，且兼容多种编辑方法，适用于多样化任务。

Conclusion: FGS为文本引导扩散模型中的图像编辑提供了一种有效的解决方案，平衡了忠实度和可编辑性，具有广泛适用性。
...
TL;DR: VisionGuard是一个多阶段框架，通过自适应标签和上下文扩展模块提升头盔违规检测的准确性，实验显示其mAP比基线检测器提高3.1%。
...
Result: 实验表明VisionGuard的mAP比基线提高3.1%，适用于交通监控系统。

Conclusion: VisionGuard有效提升头盔违规检测性能，有望实际部署以增强交通安全。
...
### [56] [Detection of Breast Cancer Lumpectomy Margin with SAM-incorporated Forward-Forward Contrastive Learning](https://arxiv.org/abs/2506.21006)
...
Method: 通过标注SR图像中的恶性、非恶性组织和病理确认的边缘区域，使用FFCL预训练ResNet-18进行分类，并利用SAM进行精细边缘分割。

Result: 方法在边缘分类中AUC达到0.8455，分割边缘的Dice相似度比基线模型提高27.4%，推理时间缩短至47毫秒/图像。
...
### [57] [EgoAdapt: Adaptive Multisensory Distillation and Policy Learning for Efficient Egocentric Perception](https://arxiv.org/abs/2506.21080)
...
TL;DR: EgoAdapt框架通过跨模态蒸馏和策略学习，显著提升多感知自我中心任务的效率，减少计算成本。
...
Method: 提出EgoAdapt框架，结合跨模态蒸馏和策略学习，适应不同任务。

Result: 在三个数据集上，GMACs减少89.09%，参数减少82.02%，能效提升9.6倍，性能与或优于SOTA。
...
### [58] [The Aging Multiverse: Generating Condition-Aware Facial Aging Tree via Training-Free Diffusion](https://arxiv.org/abs/2506.21008)
*Bang Gong,Luchao Qi,Jiaye Wu,Zhicheng Fu,Chunbo Song,David W. Jacobs,John Nicholson,Roni Sengupta*

Main category: cs.CV

TL;DR: 提出Aging Multiverse框架，通过单张图像生成多种可能的面部老化轨迹，考虑环境、健康等外部因素，优于现有方法。
...
### [59] [User-in-the-Loop View Sampling with Error Peaking Visualization](https://arxiv.org/abs/2506.21009)
*Ayaka Yasunaga,Hideo Saito,Shohei Mori*

Main category: cs.CV

TL;DR: 提出了一种基于局部重建光场和可视化误差的方法，用于新视角合成，减少用户负担并扩大场景探索范围。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要用户通过AR显示对齐3D标注进行数据采集，任务繁重且场景受限。

Method: 使用局部重建光场和可视化误差提示用户插入新视角以消除误差。

Result: 误差峰值可视化侵入性低，减少用户失望，且需要更少视角样本。方法还可用于更大场景的辐射场重建。

Conclusion: 该方法解放了用户负担，扩展了场景探索能力，适用于移动视角合成系统及更大场景重建。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏多镜头标注，导致模型在多镜头场景中表现不佳，如身份遗忘和关键帧忽略。
...
<details>
  <summary>Details</summary>
Motivation: 传统VQA方法仅提供数值评分，无法全面描述视频质量。利用语言输出的优势，通过指令调优视频大型多模态模型（LMMs）有望解决这一问题。

Method: 提出SIG管道，自动为未标注视频评分并映射到文本级别，结合层次化思维链（CoT）建模质量维度与整体质量的关系。

Result: 生成的Score2Instruct（S2I）数据集包含32万多样化的指令-响应对，实验表明该方法显著提升了视频LMMs的质量评分和解释能力。

Conclusion: SIG方法通过自动化数据生成和指令调优，有效提升了视频LMMs在质量评估和解释方面的性能。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中数据异构性（如客户端标签偏好偏差）是主要挑战，现有方法常忽略客户端内部的语义信息。
...
### [63] [Robust Deep Learning for Myocardial Scar Segmentation in Cardiac MRI with Noisy Labels](https://arxiv.org/abs/2506.21151)
...
Method: 采用Kullback-Leibler损失函数和大量数据增强技术，对现有先进模型进行微调，实现自动化心肌瘢痕检测与分割。

Result: 模型在急性和慢性病例中表现优异，能生成准确且平滑的分割结果，且在分布外测试集上表现出强泛化能力。
...
### [64] [HybridQ: Hybrid Classical-Quantum Generative Adversarial Network for Skin Disease Image Generation](https://arxiv.org/abs/2506.21015)
...
<details>
  <summary>Details</summary>
Motivation: 皮肤疾病诊断中机器学习需要大量高质量数据，但现有数据集存在类别不平衡、隐私问题和对象偏差，数据增强是关键。传统生成模型计算资源需求高，而现有量子方法只能生成低质量灰度图像。

Method: 采用经典-量子潜在空间融合技术，开发了首个能生成彩色医学图像的经典-量子GAN。

Result: 模型在图像生成质量和分类性能提升上优于传统深度卷积GAN和现有混合量子GAN，参数和训练时间大幅减少。
...
TL;DR: 提出了一种轻量级注意力引导的自调制特征融合网络（LASFNet），通过单一特征级融合单元简化训练过程，显著降低计算成本，同时提高检测精度。
...
Method: 使用单一特征级融合单元，结合注意力引导的自调制特征融合模块（ASFF）和轻量级特征注意力转换模块（FATM）。
...
### [66] [Task-Aware KV Compression For Cost-Effective Long Video Understanding](https://arxiv.org/abs/2506.21184)
...
Conclusion: Video-X^2L无需额外训练，兼容现有MLLMs，是解决LVU问题的简单有效方案。
...
### [67] [Instella-T2I: Pushing the Limits of 1D Discrete Latent Space Image Generation](https://arxiv.org/abs/2506.21022)
*Ze Wang,Hao Chen,Benran Hu,Jiang Liu,Ximeng Sun,Jialian Wu,Yusheng Su,Xiaodong Yu,Emad Barsoum,Zicheng Liu*

Main category: cs.CV

TL;DR: 提出了一种1D二进制图像潜在表示方法，显著减少图像建模所需的token数量，提升效率和性能。


<details>
  <summary>Details</summary>
Motivation: 传统2D网格结构的图像token化方法计算需求高，1D潜在空间虽减少了token数量，但仍需进一步优化。

Method: 引入1D二进制图像潜在表示，将图像表示为二进制向量序列，取代传统one-hot编码。

Result: 在1024x1024分辨率下仅需128个token，token数量减少32倍，训练和推理速度显著提升。

Conclusion: 该方法在保持高性能的同时，提供了高效、可扩展的图像token化替代方案。
...
### [68] [BitMark for Infinity: Watermarking Bitwise Autoregressive Image Generative Models](https://arxiv.org/abs/2506.21209)
*Louis Kerner,Michel Meintz,Bihe Zhao,Franziska Boenisch,Adam Dziedzic*

Main category: cs.CV

TL;DR: BitMark是一种针对Infinity模型的鲁棒位级水印框架，旨在防止模型崩溃，通过在多尺度嵌入水印来识别生成内容。
...
Method: BitMark在Infinity的图像生成过程中，直接在位级别嵌入水印，覆盖多尺度（分辨率），同时保持视觉保真度和生成速度。

Result: BitMark的水印具有高放射性，即使水印图像被用于训练其他生成模型，水印仍可被检测到。
...
### [69] [DidSee: Diffusion-Based Depth Completion for Material-Agnostic Robotic Perception and Manipulation](https://arxiv.org/abs/2506.21034)
...
<details>
  <summary>Details</summary>
Motivation: 商用RGB-D相机在非朗伯物体上产生的深度图通常噪声大且不完整，传统方法因训练数据有限而泛化能力差。扩散模型的视觉先验虽有助于提升性能，但训练与推理的不匹配及非朗伯区域特征缺失限制了效果。

Method: 提出DidSee框架：1）采用重新缩放的噪声调度器消除信号泄漏偏差；2）设计噪声无关的单步训练策略减少误差累积；3）引入语义增强器联合深度补全与语义分割。

Result: 在多个基准测试中达到最先进性能，表现出强大的现实世界泛化能力，并有效提升下游任务（如类别级姿态估计和机器人抓取）的表现。

Conclusion: DidSee通过改进扩散框架和结合语义信息，显著提升了非朗伯物体的深度补全性能，具有广泛的应用潜力。
...
### [70] [HumanOmniV2: From Understanding to Omni-Modal Reasoning with Context](https://arxiv.org/abs/2506.21277)
...
Conclusion: 通过强化学习增强多模态推理能力是有效的，IntentBench为评估复杂意图理解提供了新基准。
...
### [71] [Boosting Domain Generalized and Adaptive Detection with Diffusion Models: Fitness, Generalization, and Transferability](https://arxiv.org/abs/2506.21042)
*Boyong He,Yuxiang Ji,Zhuoyue Tan,Liaoni Wu*

Main category: cs.CV

TL;DR: 提出一种利用扩散模型提取中间特征的方法，显著减少推理时间并提升性能，同时通过辅助分支和一致性损失优化特征提取和泛化能力。
...
Method: 从单步扩散过程中提取中间特征，构建对象中心辅助分支，应用一致性损失对齐分支，并在统一框架中通过特征和对象级对齐提升性能。

Result: 在3个DA基准和5个DG基准上取得竞争性结果，在COCO泛化基准上表现优异，尤其在域偏移大和数据少的情况下。
...
### [72] [Logios : An open source Greek Polytonic Optical Character Recognition system](https://arxiv.org/abs/2506.21474)
...
Conclusion: 该方法为希腊多调文本的OCR提供了高效解决方案，并支持学术使用。
...
### [73] [Boosting Generative Adversarial Transferability with Self-supervised Vision Transformer Features](https://arxiv.org/abs/2506.21046)
...
<details>
  <summary>Details</summary>
Motivation: 传统对抗攻击依赖监督学习的特征，而自监督学习与ViT的结合可能提供更好的特征表示，从而提升对抗样本的泛化能力。

Method: 设计了dSVA方法，利用自监督ViT的全局结构特征（对比学习）和局部纹理特征（掩码图像建模），通过生成器框架和注意力机制训练生成对抗样本。

Result: 实验表明，结合两种自监督特征的方法显著提升了对抗样本的黑盒迁移性，优于现有技术。

Conclusion: 自监督ViT的双特征攻击dSVA在提升对抗样本泛化能力方面具有显著优势，为对抗攻击研究提供了新思路。
...
### [74] [Holistic Surgical Phase Recognition with Hierarchical Input Dependent State Space Models](https://arxiv.org/abs/2506.21330)
...
<details>
  <summary>Details</summary>
Motivation: 机器人辅助手术视频分析因时长问题难以全面处理，现有Transformer模型因二次注意力机制效率低下。

Method: 采用分层状态空间模型，结合局部聚合和全局关系模块，利用离散-连续混合监督策略训练。

Result: 在多个数据集上表现优异（Cholec80 +2.8%，MICCAI2016 +4.3%，Heichole +12.9%）。

Conclusion: 该方法高效处理长视频，捕捉局部与全局动态，性能显著提升。
...
### [75] [Class-Agnostic Region-of-Interest Matching in Document Images](https://arxiv.org/abs/2506.21055)
*Demin Zhang,Jiahao Lyu,Zhijie Shen,Yu Zhou*

Main category: cs.CV

TL;DR: 论文提出了一种名为“RoI-Matching”的新任务，用于灵活匹配文档中的自定义区域，并构建了基准测试和框架RoI-Matcher。


<details>
  <summary>Details</summary>
Motivation: 现有文档分析方法无法满足用户自定义需求，因此需要一种灵活、多粒度、开放式的解决方案。

Method: 使用视觉提示和Siamese网络提取多级特征，通过交叉注意力层对齐语义。

Result: 实验证明RoI-Matcher在基准测试中有效，可作为进一步研究的基线。
...
### [76] [CA-I2P: Channel-Adaptive Registration Network with Global Optimal Selection](https://arxiv.org/abs/2506.21364)
*Zhixin Cheng,Jiacheng Deng,Xinjun Li,Xiaotian Yin,Bohao Liao,Baoqun Yin,Wenfei Yang,Tianzhu Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种改进的图像到点云配准方法，通过引入通道自适应调整模块（CAA）和全局最优选择模块（GOS），解决了特征通道注意力差异和冗余对应问题。


<details>
  <summary>Details</summary>
Motivation: 现有检测自由方法在图像和点云特征匹配中存在特征通道注意力差异和冗余对应问题，影响了配准精度。

Method: 提出CAA模块增强模态内特征并抑制跨模态敏感性，GOS模块通过全局优化替代局部选择。
...
### [77] [HalluSegBench: Counterfactual Visual Reasoning for Segmentation Hallucination Evaluation](https://arxiv.org/abs/2506.21546)
...
Method: 提出HalluSegBench，包含1340个反事实实例对和281个独特对象类的新数据集，以及量化幻觉敏感性的新指标。
...
### [78] [SAMURAI: Shape-Aware Multimodal Retrieval for 3D Object Identification](https://arxiv.org/abs/2506.21056)
...
### [79] [PoseMaster: Generating 3D Characters in Arbitrary Poses from a Single Image](https://arxiv.org/abs/2506.21076)
...
Result: PoseMaster在A-pose角色生成上优于现有技术，并能精确控制任意姿势。
...
### [80] [TITAN: Query-Token based Domain Adaptive Adversarial Learning](https://arxiv.org/abs/2506.21484)
*Tajamul Ashraf,Janibul Bashir*

Main category: cs.CV

TL;DR: 论文提出了一种名为TITAN的方法，用于解决源数据不可用的无源域自适应目标检测问题，通过分离目标域为易样本和难样本，并结合查询令牌对抗模块提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决在源数据不可用时，教师模型因伪标签噪声导致的性能下降问题。

Method: 提出TITAN方法，将目标域分为易样本和难样本，并引入查询令牌对抗模块以减少域间特征差异。
...
### [81] [ESMStereo: Enhanced ShuffleMixer Disparity Upsampling for Real-Time and Accurate Stereo Matching](https://arxiv.org/abs/2506.21091)
*Mahmoud Tahmasebi,Saif Huq,Kevin Meehan,Marion McAfee*

Main category: cs.CV

TL;DR: 论文提出了一种增强型混洗混合器（ESM），用于解决小规模成本体积在实时立体匹配中的信息丢失问题，同时保持高精度和实时性能。


<details>
  <summary>Details</summary>
Motivation: 立体匹配在现代自动驾驶系统中至关重要，但现有基于深度学习的模型在实时性和高精度之间难以平衡。大尺度成本体积计算量大，小尺度则信息不足。
...
Conclusion: ESM通过局部上下文连接和大感受野，以低计算成本实现了实时高精度的立体匹配。
...
### [82] [OracleFusion: Assisting the Decipherment of Oracle Bone Script with Structurally Constrained Semantic Typography](https://arxiv.org/abs/2506.21101)
...
<details>
  <summary>Details</summary>
Motivation: 甲骨文是古代文明的重要记录，但仍有大量字符未解读，其复杂结构和抽象意象增加了解读难度。

Method: OracleFusion采用两阶段方法：第一阶段利用增强空间感知推理的MLLM分析字形结构；第二阶段引入OSVF，结合字形约束生成语义丰富的矢量字体。
...
### [83] [Pushing Trade-Off Boundaries: Compact yet Effective Remote Sensing Change Detection](https://arxiv.org/abs/2506.21109)
*Luosheng Xu,Dalin Zhang,Zhaohui Song*

Main category: cs.CV

TL;DR: FlickCD是一种轻量级遥感变化检测模型，通过增强差异模块和多尺度局部-全局融合块，在减少资源消耗的同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习模型复杂度高且计算需求大，但精度提升有限。研究旨在开发高效轻量级模型，适合卫星上处理。

Method: 提出FlickCD模型，包含增强差异模块（EDM）和局部-全局融合块（SWSA和EGSA），以多尺度捕捉语义信息。
...
### [84] [CL-Splats: Continual Learning of Gaussian Splatting with Local Optimization](https://arxiv.org/abs/2506.21117)
...
Method: 引入CL-Splats，结合变化检测模块，分割动态与静态部分，实现局部优化，并支持场景状态存储与恢复。
...
### [85] [Learning to See in the Extremely Dark](https://arxiv.org/abs/2506.21132)
*Hai Jiang,Binhao Guan,Zhen Liu,Xiaohong Liu,Jian Yu,Zheng Liu,Songchen Han,Shuaicheng Liu*

Main category: cs.CV

TL;DR: 论文提出了一种用于极低光RAW图像增强的合成数据集SIED，并开发了一种基于扩散模型的框架，结合自适应光照校正模块和颜色一致性损失，实现了对极低信噪比RAW输入的视觉恢复。


<details>
  <summary>Details</summary>
Motivation: 现有学习型方法在极低光环境（如0.0001 lux）下的RAW图像增强能力尚未充分探索，缺乏相应数据集。

Method: 提出了一种配对数据合成流程生成SIED数据集，并开发了基于扩散模型的框架，包含自适应光照校正模块（AICM）和颜色一致性损失。

Result: 在SIED和公开基准测试中验证了方法的有效性。

Conclusion: 论文提出的数据集和方法为极低光RAW图像增强提供了有效解决方案，代码和数据集已开源。
...
### [86] [YOLO-FDA: Integrating Hierarchical Attention and Detail Enhancement for Surface Defect Detection](https://arxiv.org/abs/2506.21135)
*Jiawei Hu*
...
<details>
  <summary>Details</summary>
Motivation: 工业表面缺陷检测面临缺陷类型多样、形状不规则、细粒度要求和复杂纹理等挑战，现有方法存在冗余特征、细节敏感性不足和多尺度鲁棒性弱的问题。

Method: YOLO-FDA采用BiFPN架构增强多级特征聚合，引入DDFM模块捕捉细粒度结构变化，并提出AC和CAF两种注意力融合策略优化上下文表示。

Result: 在多个基准数据集上，YOLO-FDA在准确性和鲁棒性上均优于现有方法。

Conclusion: YOLO-FDA通过创新的细节增强和注意力融合策略，显著提升了工业缺陷检测的性能。
...
### [87] [Tree-based Semantic Losses: Application to Sparsely-supervised Large Multi-class Hyperspectral Segmentation](https://arxiv.org/abs/2506.21150)
*Junwen Wang,Oscar Maccormac,William Rochford,Aaron Kujawa,Jonathan Shapey,Tom Vercauteren*

Main category: cs.CV

TL;DR: 该论文提出两种基于树结构的语义损失函数，利用标签的层次结构改进高光谱成像（HSI）的分割任务，并在稀疏标注数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像在手术应用中潜力巨大，但现有学习方法未能利用标签空间的语义层次结构，导致对错误分类的惩罚不够精细。

Method: 引入两种树基语义损失函数，结合稀疏标注训练方法，利用标签的层次结构优化分割任务。

Result: 在包含107个类别的稀疏标注HSI数据集上达到最先进性能，同时有效检测分布外像素。

Conclusion: 提出的方法不仅提升了分割性能，还支持分布外像素检测，为手术应用提供了更可靠的解决方案。
...
### [88] [Geometry and Perception Guided Gaussians for Multiview-consistent 3D Generation from a Single Image](https://arxiv.org/abs/2506.21152)
*Pufan Li,Bi'an Du,Wei Hu*

Main category: cs.CV

TL;DR: 提出一种新方法，通过结合几何和感知先验，从单张图像重建高保真3D物体，无需额外训练模型。
...
Conclusion: 该方法实现了更一致和详细的3D物体生成。
...
### [89] [Topology-Aware Modeling for Unsupervised Simulation-to-Reality Point Cloud Recognition](https://arxiv.org/abs/2506.21165)
*Longkun Zou,Kangjun Liu,Ke Chen,Kailing Guo,Kui Jia,Yaowei Wang*

Main category: cs.CV

TL;DR: 论文提出了一种名为TAM的新框架，用于解决3D点云数据在模拟到现实（Sim2Real）领域适应中的挑战，通过全局拓扑建模和自监督学习任务提升分类器的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有无监督领域适应（UDA）方法在处理模拟数据与真实数据之间的领域差距时表现不佳，缺乏能够捕捉全局拓扑信息的鲁棒描述符。

Method: TAM框架利用全局空间拓扑和局部几何特征的拓扑关系，结合自监督学习任务和跨域对比学习与自训练的混合策略。

Result: 在三个公开的Sim2Real基准测试中，TAM框架表现优于现有方法，验证了其有效性。

Conclusion: TAM框架通过拓扑建模和自训练策略显著缩小了Sim2Real领域差距，提升了点云分类器的性能。
...
### [90] [GroundFlow: A Plug-in Module for Temporal Reasoning on 3D Point Cloud Sequential Grounding](https://arxiv.org/abs/2506.21188)
...
### [91] [MedPrompt: LLM-CNN Fusion with Weight Routing for Medical Image Segmentation and Classification](https://arxiv.org/abs/2506.21199)
*Shadman Sobhan,Kazi Abrar Mahmud,Abduz Zami*

Main category: cs.CV

TL;DR: MedPrompt是一个统一的医学图像分析框架，结合了大型语言模型（LLM）和模块化卷积神经网络（CNN），支持用户自定义任务，无需重新训练整个系统。
...
Result: 在19个公共数据集上表现优异，端到端正确率97%，推理延迟2.5秒，分割和分类性能接近最优。
...
### [92] [ReME: A Data-Centric Framework for Training-Free Open-Vocabulary Segmentation](https://arxiv.org/abs/2506.21233)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预训练模型或合成数据，性能受限于模型能力或数据质量。本文发现高质量参考集对无需训练的OVS至关重要。

Method: 提出一个数据质量导向的框架，包括构建高质量参考集的数据管道和基于相似性的检索方法。
...
### [93] [DiMPLe -- Disentangled Multi-Modal Prompt Learning: Enhancing Out-Of-Distribution Alignment with Invariant and Spurious Feature Separation](https://arxiv.org/abs/2506.21237)
*Umaima Rahman,Mohammad Yaqub,Dwarikanath Mahapatra*

Main category: cs.CV

TL;DR: DiMPLe是一种新型多模态提示学习方法，通过解耦视觉和语言模态中的不变和伪特征，提升多模态学习的泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决视觉数据中伪相关性对分布外（OOD）性能的负面影响，传统方法仅关注图像特征，而DiMPLe在多模态中解耦特征。

Method: 结合三个关键目标：不变与伪特征的互信息最小化、伪特征正则化和不变特征的对比学习。
...
### [94] [Temporal Rate Reduction Clustering for Human Motion Segmentation](https://arxiv.org/abs/2506.21249)
*Xianghan Meng,Zhengyu Tong,Zhiyuan Huang,Chun-Guang Li*

Main category: cs.CV

TL;DR: 提出了一种名为TR²C的新方法，通过联合学习结构化表示和亲和力来分割视频帧序列，解决了现有子空间聚类方法在复杂背景下的不足。
...
Conclusion: TR²C方法在复杂背景下表现优异，显著提升了人体运动分割的准确性。
...
### [95] [DuET: Dual Incremental Object Detection via Exemplar-Free Task Arithmetic](https://arxiv.org/abs/2506.21260)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的目标检测系统需要同时处理新类别学习和环境变化适应，现有方法（CIOD和DIOD）只能解决单一问题，限制了实际应用。

Method: 提出了DuET框架，基于任务算术的模型合并方法，通过方向一致性损失减少符号冲突，支持多种检测器（如YOLO11和RT-DETR）。

Result: 在Pascal Series和Diverse Weather Series上的实验显示，DuET在RAI指标上分别提升了13.12%和11.39%，同时保持了较高的平均保留率。

Conclusion: DuIOD和DuET框架有效解决了类别和域增量学习的挑战，为实时目标检测提供了实用解决方案。
...
### [96] [Video Virtual Try-on with Conditional Diffusion Transformer Inpainter](https://arxiv.org/abs/2506.21270)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频虚拟试穿方法存在时空不一致问题，直接应用基于图像的试穿方法效果不佳。ViTI旨在从视频生成的角度出发，解决这一问题。

Method: 提出ViTI，基于扩散变换器构建3D时空注意力框架，采用多阶段训练和掩蔽策略，逐步适应视频服装修复任务。
...
### [97] [WordCon: Word-level Typography Control in Scene Text Rendering](https://arxiv.org/abs/2506.21276)
*Wenda Shi,Yiren Song,Zihan Rao,Dengming Zhang,Jiaming Liu,Xingxing Zou*

Main category: cs.CV

TL;DR: 提出了一种新的文本图像对齐框架（TIA）和混合参数高效微调方法（WordCon），用于在生成图像中实现精确的单词级排版控制。
...
Method: 构建单词级控制场景文本数据集，提出TIA框架和WordCon方法，结合掩码损失和联合注意力损失。

Result: 定性和定量结果均显示该方法优于现有技术。

Conclusion: 该方法在艺术文本渲染、文本编辑等任务中表现出色，数据集和代码将公开。
...
### [98] [HieraSurg: Hierarchy-Aware Diffusion Model for Surgical Video Generation](https://arxiv.org/abs/2506.21287)
*Diego Biagini,Nassir Navab,Azade Farshad*

Main category: cs.CV

TL;DR: HieraSurg是一个层次感知的手术视频生成框架，通过两个扩散模型实现高质量视频合成，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有手术视频生成方法缺乏对手术动作和阶段的细粒度控制，难以保持一致性。

Method: HieraSurg采用两阶段模型：首先生成粗粒度语义变化，再结合细粒度视觉特征生成最终视频。

Result: 在胆囊切除术视频生成任务中，模型在定量和定性上均优于现有方法，且能生成更高帧率的视频。

Conclusion: HieraSurg展示了在实际手术应用中的潜力，特别是在提供现有分割图时表现优异。
...
### [99] [Continual Self-Supervised Learning with Masked Autoencoders in Remote Sensing](https://arxiv.org/abs/2506.21312)
*Lars Möllenbrok,Behnood Rasti,Begüm Demir*

Main category: cs.CV

TL;DR: 提出了一种基于掩码自编码器的持续自监督学习方法CoSMAE，通过数据混合和模型混合知识蒸馏减少灾难性遗忘，性能提升4.94%。
...
Method: CoSMAE包括数据混合和模型混合知识蒸馏两部分，分别从数据和模型层面减少遗忘。

Result: 实验显示CoSMAE比现有方法性能提升4.94%。

Conclusion: CoSMAE通过自监督方式有效减少了灾难性遗忘，性能显著提升。
...
### [100] [DrishtiKon: Multi-Granular Visual Grounding for Text-Rich Document Images](https://arxiv.org/abs/2506.21316)
*Badri Vishal Kasuba,Parag Chaudhuri,Ganesh Ramakrishnan*

Main category: cs.CV

TL;DR: 提出了一种名为Drishtikon的多粒度视觉定位框架，用于提升复杂多语言文档的视觉问答（VQA）系统的可解释性和可信度。


<details>
  <summary>Details</summary>
Motivation: 解决文本丰富的文档图像中视觉定位的挑战，尤其是在多语言环境下，提升VQA系统的性能和可解释性。

Method: 结合多语言OCR、大语言模型和新型区域匹配算法，实现块、行、词和点级别的精确定位。

Result: 实验表明，该方法在定位精度上达到最先进水平，行级粒度在精度和召回率之间取得最佳平衡。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在人体姿态和动作相关的复杂视觉任务中表现不佳，主要缺乏专门的视觉语言指令数据。

Method: 通过整合人体关键点与传统视觉特征（如标题和边界框），构建了一个包含200,328个样本的数据集，用于微调模型，重点关注对话、详细描述和复杂推理三个领域。

Result: 微调后的LLaVA-Pose模型在扩展人体姿态和动作理解基准（E-HPAUB）上表现显著提升，整体性能提高了33.2%。

Conclusion: 研究表明，整合关键点数据能有效增强多模态模型在人体中心视觉理解任务中的表现。
...
### [102] [PanSt3R: Multi-view Consistent Panoptic Segmentation](https://arxiv.org/abs/2506.21348)
*Lojze Zust,Yohann Cabon,Juliette Marrie,Leonid Antsfeld,Boris Chidlovskii,Jerome Revaud,Gabriela Csurka*

Main category: cs.CV

TL;DR: 提出了一种名为PanSt3R的新方法，通过单次前向传播联合预测3D几何和多视角全景分割，避免了现有方法依赖2D分割和测试时优化的不足。
...
Method: 基于MUSt3R的改进，结合语义感知和多视角全景分割能力，并改进了掩码合并方法。

Result: 在多个基准测试中达到最先进性能，且速度显著快于现有方法。

Conclusion: PanSt3R方法简单、快速且高效，为3D场景全景分割提供了新思路。
...
### [103] [ShotBench: Expert-Level Cinematic Understanding in Vision-Language Models](https://arxiv.org/abs/2506.21356)
...
TL;DR: ShotBench是一个专为电影语言理解设计的基准测试，揭示了现有视觉语言模型在电影语法理解上的不足，并提出了ShotQA数据集和ShotVL模型以推动该领域发展。
...
Conclusion: ShotVL为AI驱动的电影理解和生成设立了新标准，相关资源已开源以促进领域发展。
...
### [104] [CoPa-SG: Dense Scene Graphs with Parametric and Proto-Relations](https://arxiv.org/abs/2506.21357)
...
Method: 提出CoPa-SG数据集，包含参数化关系和原型关系，并评估多种场景图生成模型。
...
### [105] [GenFlow: Interactive Modular System for Image Generation](https://arxiv.org/abs/2506.21369)
*Duc-Hung Nguyen,Huu-Phuc Huynh,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: GenFlow是一个模块化框架，通过节点编辑器和自然语言处理助手，降低生成艺术的技术门槛，提升用户体验。
...
Result: 用户研究表明，GenFlow能优化工作流、减少任务时间，并提升用户理解。
...
### [106] [FastRef:Fast Prototype Refinement for Few-Shot Industrial Anomaly Detection](https://arxiv.org/abs/2506.21398)
*Long Tian,Yufei Li,Yuyang Dai,Wenchao Chen,Xiyang Liu,Bo Chen*

Main category: cs.CV

TL;DR: FastRef是一种高效的原型细化框架，用于解决少样本工业异常检测（FS-IAD）中原型代表性问题。通过两阶段迭代过程（特征转移和异常抑制），显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在少样本环境下通常忽略查询图像统计信息，导致原型代表性不足。FastRef旨在通过系统整合查询特征来优化原型。

Method: FastRef采用两阶段迭代过程：1）通过可优化变换矩阵将查询特征转移到原型；2）通过原型对齐抑制异常。使用线性重建和最优传输（OT）技术。

Result: 在MVTec、ViSA、MPDD和RealIAD四个基准数据集上的实验表明，FastRef在1/2/4-shot设置下均表现优异且计算高效。

Conclusion: FastRef通过整合查询特征和异常抑制机制，显著提升了少样本工业异常检测的性能和效率。
...
### [107] [Curve-Aware Gaussian Splatting for 3D Parametric Curve Reconstruction](https://arxiv.org/abs/2506.21401)
...
<details>
  <summary>Details</summary>
Motivation: 现有两阶段方法在边缘点云重建和参数曲线拟合之间存在优化间隙，导致误差累积；参数曲线本身不适合基于渲染的多视角优化。
...
### [108] [XVerse: Consistent Multi-Subject Control of Identity and Semantic Attributes via DiT Modulation](https://arxiv.org/abs/2506.21416)
...
Result: XVerse实现了高保真、可编辑的多主题图像合成，并能精确控制单个主题特征和语义属性。
...
### [109] [HyperSORT: Self-Organising Robust Training with hyper-networks](https://arxiv.org/abs/2506.21430)
...
<details>
  <summary>Details</summary>
Motivation: 医学影像数据常包含异质性偏差（如错误标签或不一致的标注风格），这些偏差会降低深度学习分割网络的性能。识别和表征这些偏差是一项繁琐且具有挑战性的任务。

Method: HyperSORT通过超网络预测UNet参数，同时学习训练集中每个数据样本的潜在向量。该方法优化了UNet参数的复杂分布，而非单一网络。

Result: 在AMOS和TotalSegmentator数据集上的实验表明，HyperSORT能够识别系统性偏差和错误样本，并通过潜在空间聚类生成符合学习偏差的分割参数。

Conclusion: HyperSORT提供了一种有效的方法来识别和处理医学影像数据中的偏差，同时公开了代码和数据集分析。
...
### [110] [Benchmarking Deep Learning and Vision Foundation Models for Atypical vs. Normal Mitosis Classification with Cross-Dataset Evaluation](https://arxiv.org/abs/2506.21444)
...
TL;DR: 该研究通过深度学习方法（包括基线模型、基础模型线性探测和LoRA微调）对乳腺癌非典型有丝分裂（AMF）分类进行了全面基准测试，并引入两个新数据集进行验证。结果显示LoRA微调方法表现最佳，表明迁移学习和微调技术能有效解决这一难题。


<details>
  <summary>Details</summary>
Motivation: 非典型有丝分裂是肿瘤恶性程度的独立预后标志物，但其识别因低发生率、形态差异细微、病理学家间一致性低及数据集类别不平衡而具有挑战性。

Method: 研究基于AMi-Br数据集，比较了多种深度学习方法（基线模型、基础模型线性探测、LoRA微调），并引入AtNorM-Br和AtNorM-MD两个新数据集进行验证。

Result: 在AMi-Br、AtNorM-Br和AtNorM-MD数据集上，LoRA微调的基础模型分别达到0.8135、0.7696和0.7705的平衡准确率，表现最佳。
...
### [111] [Controllable 3D Placement of Objects with Scene-Aware Diffusion Models](https://arxiv.org/abs/2506.21446)
*Mohamed Omran,Dimitris Kalatzis,Jens Petersen,Amirhossein Habibian,Auke Wiggers*

Main category: cs.CV

TL;DR: 论文提出了一种基于视觉地图和粗略物体掩码的高质量物体放置方法，解决了精确位置和方向控制的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前文本条件生成模型虽强大，但精确放置物体仍需要复杂的掩码或提示，因此需要更灵活且精确的方法。

Method: 设计了一种结合视觉地图和粗略物体掩码的条件信号，基于修复模型保持背景不变，支持形状和方向调整。

Result: 在汽车场景中验证了方法的有效性，能够高质量完成物体放置任务，包括非平凡形状变化和精确位置控制。

Conclusion: 该方法实现了高精度的物体放置，同时支持外观和位置的灵活控制。
...
TL;DR: 本文提出了一种专门用于地下矿工检测的热成像数据集，并评估了多种先进的目标检测算法，为未来矿工安全检测系统的开发奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 地下采矿作业面临重大安全挑战，需要可靠的矿工检测能力以支持应急响应。目前缺乏适用于地下环境的数据集，限制了深度学习算法的应用。

Method: 通过系统采集各种采矿活动和场景的热成像数据，创建了一个专门的数据集，并评估了YOLOv8、YOLOv10、YOLO11和RT-DETR等算法的性能。

Result: 研究证明了热成像用于矿工检测的可行性，并提供了基准性能指标，为未来开发可靠的检测系统迈出了关键一步。

Conclusion: 该数据集为矿工检测系统的开发和验证提供了重要基础，未来可进一步扩展以覆盖更多应急场景。
...
### [113] [Rethinking Oversaturation in Classifier-Free Guidance via Low Frequency](https://arxiv.org/abs/2506.21452)
...
<details>
  <summary>Details</summary>
Motivation: 高引导尺度在条件扩散模型中虽然能增强条件项性能，但会导致过饱和和不真实伪影，低频信号中的冗余信息是主要原因。

Method: 提出LF-CFG方法，通过自适应阈值定位冗余信息，并根据低频信息变化率确定合理阈值，采用降权策略减少冗余信息的影响。
...
### [114] [Evaluation of Traffic Signals for Daily Traffic Pattern](https://arxiv.org/abs/2506.21469)
*Mohammad Shokrolah Shirazi,Hung-Fu Chang*

Main category: cs.CV

TL;DR: 论文提出三种基于转向运动计数（TMC）的交通信号配置方法（动态、静态和混合），并通过仿真验证其效果。实验表明，90和120秒的信号周期最佳，混合方法在高峰和非高峰时段表现更优。


<details>
  <summary>Details</summary>
Motivation: 转向运动计数数据对交通信号设计至关重要，但现有方法难以适应多样化的交通模式，尤其是高峰和非高峰时段的差异。

Method: 开发了基于视觉的跟踪系统估计TMC，结合仿真工具SUMO评估信号配置。提出动态、静态和混合三种方法，并设计交通生成模块模拟4小时交通流。

Result: 90和120秒的信号周期表现最佳；动态配置在四个路口效果更好，混合方法适应高峰和非高峰时段，尤其适用于东西和南北区域交通密集的路口。

Conclusion: 混合信号方法能有效适应多样化的交通模式，尤其是区域权重不均的路口，为交通信号设计提供了新思路。
...
### [115] [Global and Local Entailment Learning for Natural World Imagery](https://arxiv.org/abs/2506.21476)
...
Conclusion: RCME框架有效提升了视觉语言模型对层次结构的表示能力，为相关任务提供了新思路。
...
### [116] [Towards Reliable Detection of Empty Space: Conditional Marked Point Processes for Object Detection](https://arxiv.org/abs/2506.21486)
...
Method: 采用空间统计方法，将边界框数据建模为标记点过程，通过似然训练提供明确的置信度估计。

Result: 通过校准评估和性能测试验证了方法的有效性。

Conclusion: 该方法为未检测区域提供了可靠的不确定性量化，适用于高安全性要求的应用。
...
### [117] [Mitigating Hallucination of Large Vision-Language Models via Dynamic Logits Calibration](https://arxiv.org/abs/2506.21509)
*Jiahe Chen,Jiaying He,Qian Shao,Qiyuan Chen,Jiahe Ying,Hongxia Xu,Jintai Chen,Jianwei Zheng,Jian Wu*

Main category: cs.CV

TL;DR: 论文提出了一种名为动态对数校准（DLC）的新方法，通过动态调整生成文本与视觉证据的对齐，减少大型视觉语言模型（LVLM）中的幻觉现象。
...
Method: DLC在解码阶段利用CLIP评估图像与生成文本的语义对齐，并通过动态更新的上下文基线调整输出对数，优先选择视觉相关的词汇。
...
### [118] [GGTalker: Talking Head Systhesis with Generalizable Gaussian Priors and Identity-Specific Adaptation](https://arxiv.org/abs/2506.21513)
*Wentao Hu,Shunkai Li,Ziqiao Peng,Haoxian Zhang,Fan Shi,Xiaoqiang Liu,Pengfei Wan,Di Zhang,Hui Tian*

Main category: cs.CV

TL;DR: GGTalker提出了一种结合通用先验和身份特定适应的两阶段方法，用于生成高质量、通用的语音驱动3D说话头部。
...
Conclusion: GGTalker通过通用先验和个体适应，显著提升了语音驱动3D说话头部的生成能力。
...
### [119] [G$^{2}$D: Boosting Multimodal Learning with Gradient-Guided Distillation](https://arxiv.org/abs/2506.21514)
...
### [120] [MADrive: Memory-Augmented Driving Scene Modeling](https://arxiv.org/abs/2506.21520)
*Polina Karpikova,Daniil Selikhanovych,Kirill Struminsky,Ruslan Musaev,Maria Golitsyna,Dmitry Baranchuk*

Main category: cs.CV

TL;DR: MADrive框架通过外部记忆库替换场景中的车辆，提升自动驾驶环境重建的逼真度。
...
### [121] [WAFT: Warping-Alone Field Transforms for Optical Flow](https://arxiv.org/abs/2506.21526)
*Yihan Wang,Jia Deng*

Main category: cs.CV

TL;DR: WAFT是一种简单高效的光流方法，通过高分辨率变形替代成本体积，性能优于现有方法且内存成本更低。
...
Conclusion: WAFT展示了无需成本体积的高性能光流方法，具有高效、灵活和泛化能力强的特点。
...
### [122] [Maximal Matching Matters: Preventing Representation Collapse for Robust Cross-Modal Retrieval](https://arxiv.org/abs/2506.21538)
*Hani Alomari,Anushka Sivakumar,Andrew Zhang,Chris Thomas*

Main category: cs.CV

TL;DR: 论文提出了一种解决跨模态图像-文本检索中稀疏监督和集合崩溃问题的新方法，通过最大化配对相似性和两种损失函数优化表现。


<details>
  <summary>Details</summary>
Motivation: 传统单向量嵌入方法难以捕捉跨模态的多样关联，而集合表示方法虽能捕捉更丰富关系，但仍面临稀疏监督和集合崩溃问题。
...
### [123] [StruMamba3D: Exploring Structural Mamba for Self-supervised Point Cloud Representation Learning](https://arxiv.org/abs/2506.21541)
*Chuxin Wang,Yixin Zha,Wenfei Yang,Tianzhu Zhang*

Main category: cs.CV

TL;DR: StruMamba3D是一种自监督点云表示学习新范式，通过保留空间依赖性和改进状态更新策略，解决了Mamba方法在点云处理中的关键问题。
...
Method: 设计了空间状态代理保留空间依赖，增强SSM的状态更新策略，并引入轻量卷积促进状态交互。
...
### [124] [DeOcc-1-to-3: 3D De-Occlusion from a Single Image via Self-Supervised Multi-View Diffusion](https://arxiv.org/abs/2506.21544)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有基于扩散的视图合成模型在遮挡情况下生成不一致视图的问题，提升3D重建的鲁棒性。

Method: 通过自监督训练管道，利用遮挡-未遮挡图像对和伪真实视图，联合学习补全和多视角生成。

Result: 方法能够从遮挡图像生成六种结构一致的新视角视图，无需先验修复或手动标注。

Conclusion: 提出的框架和基准为遮挡感知的3D重建提供了标准化评估和有效解决方案。
...
### [125] [SiM3D: Single-instance Multiview Multimodal and Multisetup 3D Anomaly Detection Benchmark](https://arxiv.org/abs/2506.21549)
...
TL;DR: 提出了LUCARIO基准和框架，用于解决表格数据上的概率性问题回答，结合贝叶斯网络和LLMs，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有QA系统在概率性问题回答上表现不佳，需要一种结合符号推理和神经推理的方法。

Method: 通过从表格中诱导贝叶斯网络，将自然语言查询转换为概率查询，并利用LLMs生成最终答案。

Result: 实验结果显示，该方法在概率性QA任务上显著优于基线。
...
TL;DR: 论文提出多语言功能基准测试（CL-GSM Symbolic和CL-IFEval），以评估大语言模型在多语言环境中的实际表现和鲁棒性，发现静态基准测试与功能基准测试之间存在显著性能差异。
...
Method: 通过将现有英语功能基准测试模板翻译为五种语言（法语、西班牙语、印地语、阿拉伯语和约鲁巴语），创建多语言功能基准测试CL-GSM Symbolic和CL-IFEval。

Result: 结果显示，某些静态基准测试（如M-MMLU）与功能基准测试性能接近（性能下降0.5%-3%），而其他（如M-GSM和Belebele）则差异显著（性能下降15%-24%）。不同语言的模型鲁棒性差异显著，阿拉伯语和英语表现最稳定。

Conclusion: 多语言功能基准测试能更准确地评估模型的实际表现和鲁棒性，为未来多语言模型评估提供改进方向。
...
TL;DR: 研究发现，虽然LLM生成的研究想法在初始阶段被认为更具新颖性，但在实际执行后，其评分显著下降，甚至低于人类专家的想法。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM生成的研究想法是否能在实际执行中产生更好的研究成果。

Method: 招募43名专家研究人员，随机分配执行LLM生成或人类专家提出的研究想法，并撰写4页短论文，由NLP专家盲审评分。

Result: 执行后，LLM生成的想法在所有评估指标（新颖性、兴奋度、有效性和总体）上的评分显著下降，甚至在某些指标上被人类想法反超。

Conclusion: 当前LLM在生成真正有效的研究想法上存在局限，且缺乏执行结果时评估研究想法具有挑战性。
...
TL;DR: MultiFinRAG是一个针对金融问答设计的检索增强生成框架，通过多模态提取和动态检索策略，显著提升了复杂金融文档问答的准确性。


<details>
  <summary>Details</summary>
Motivation: 金融文档通常包含多种模态（如文本、表格、图像），传统LLMs和RAG方法在处理跨模态推理时存在局限性。

Method: MultiFinRAG采用轻量级多模态LLM提取结构化JSON和文本摘要，结合模态感知检索和动态回退策略。

Result: 在复杂金融问答任务中，MultiFinRAG比ChatGPT-4o（免费版）准确率高出19个百分点。

Conclusion: MultiFinRAG有效解决了金融文档跨模态推理的挑战，并在资源有限的硬件上实现了高性能。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在现实冲突场景中的道德推理能力，填补其在暴力内容检测中的研究空白。

Method: 使用VBVQ问卷，通过人物角色提示（基于种族、年龄和地理身份）评估六种LLMs的零样本表现。

Result: LLMs的文本生成与暴力倾向不一致，且暴力倾向在不同人口统计学群体中存在偏见。

Conclusion: LLMs在暴力内容检测中存在局限性，需进一步优化以减少偏见和提高一致性。
...
<details>
  <summary>Details</summary>
Motivation: 由于医学决策的高风险性和医学文献的复杂性，公众对医学事实核查系统的需求增长，但现有系统未得到广泛应用。
...
TL;DR: 该论文提出了一系列改进语言模型（LM）适应性的方法，包括利用未标记数据、参数高效微调和改进监督微调，显著提升了模型的鲁棒性、效率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在适应特定任务时面临效率低、易过拟合和计算成本高的问题，限制了其在现实语言任务中的应用。

Method: 提出了继续预训练技术、参数高效微调方法和改进的监督微调方法，并开发了新评估基准。

Result: 这些方法显著提升了语言模型的鲁棒性、效率和泛化能力，适用于多种NLP任务。
...
TL;DR: 本文提出了一种基于FineWeb的自动适应多语言预训练数据集构建管道，并展示了其在九种语言上的有效性，最终扩展至1000多种语言，发布了FineWeb2数据集。
...
Method: 设计了一种可自动适应任何语言的预训练数据集构建管道，并通过九种语言的实验验证其有效性。

Result: 新管道构建的数据集优于现有数据集，并提出了基于重复计数和质量的数据集再平衡方法。

Conclusion: 发布的FineWeb2数据集和管道为多语言LLM训练提供了高质量资源。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何通过梯度更新模拟提示的效果，以提升语言模型的泛化能力和逻辑推理能力。

Method: 采用基于梯度的元学习方法，利用语言模型自身的提示预测作为目标，无需真实标签。

Result: 梯度下降训练恢复了部分或全部提示模型性能，在“反转诅咒”任务和文本问答任务中表现提升。

Conclusion: 适当初始化下，梯度下降具有显著表达能力，为长上下文建模和梯度学习泛化能力提供了新思路。
...
Method: 结合多种自监督学习模型（Wav2Vec2、HuBERT和WavLM），采用分块策略和CNN-BiLSTM框架，融合声学和语言特征。

Result: 在Speechocean762和Avalinguo数据集上，F1分数和Pearson相关系数显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM性格模型依赖粗粒度的Big Five框架且缺乏对特质强度的控制机制，限制了其表达能力和实用性。

Method: 扩展MPI以纳入16PF模型，开发SAC框架，通过形容词语义锚定和行为问题动态调整特质强度。

Result: 实验表明连续强度谱比二元切换更一致可控，且特质强度变化会系统影响相关特质。

Conclusion: 该方法为LLM提供了更精细的性格控制，推动了人机交互的进步。
...
TL;DR: 论文介绍了CA-Ben基准，用于评估大型语言模型在财务、法律和定量推理方面的能力，发现Claude 3.5 Sonnet和GPT-4o表现最佳，但在数值计算和法律解释方面仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 填补印度金融背景下LLMs在领域特定知识应用上的研究空白。
...
TL;DR: 提出了一种半监督可扩展统一框架（SSUF），通过知识增强、标签增强和结构增强模块解决电子商务查询分类中的信息不足和标签依赖问题。


<details>
  <summary>Details</summary>
Motivation: 电子商务查询通常简短且缺乏上下文，现有方法依赖用户点击行为，导致马太效应，且缺乏统一框架，效率低下。

Method: SSUF框架包含知识增强模块（利用世界知识增强查询表示）、标签增强模块（利用标签语义和半监督信号减少对后验标签的依赖）和结构增强模块（基于复杂标签关系增强标签表示）。

Result: 离线与在线A/B实验表明，SSUF显著优于现有最先进模型。

Conclusion: SSUF为电子商务查询分类提供了一种高效、统一的解决方案，显著提升了性能。
...
TL;DR: 论文提出MT2-CSD数据集和LLM-CRAN模型，用于多目标、多轮对话中的立场检测，显著优于基线模型。
...
TL;DR: ComRAG是一个用于实时工业社区问答的检索增强生成框架，通过基于质心的记忆机制整合静态知识和动态历史问答对，显著提升了性能。
...
Result: 在三个工业CQA数据集上，ComRAG显著优于基线方法，向量相似度提升25.9%，延迟降低8.7%至23.3%，块增长从20.23%降至2.06%。
...
TL;DR: Progtuning是一种结合渐进式学习的微调框架，通过逐步减少更新的Transformer块数量来优化资源分配，减少约25%的更新参数，同时保持性能。
...
Conclusion: Progtuning显著优化资源分配，适用于多种适应场景，表现优异。
...
TL;DR: Cosmos是一种新型文本生成方法，通过在压缩的潜在空间中运行扩散模型，解决了自回归模型和解码慢的问题，同时保持了生成质量。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在文本生成中占主导地位，但其顺序性导致解码慢且全局一致性难以保持；扩散模型虽提供并行生成和灵活控制，但受限于高维标记表示。

Method: Cosmos在压缩的平滑潜在空间中运行扩散模型，利用自编码器同时训练标记级重建和与预训练语言编码器的对齐，提供语义基础和扰动增强。

Result: 实验表明，Cosmos能将文本表示压缩8倍，生成质量与标记级扩散模型相当，且潜在序列长度增加时优于扩散和自回归基线。

Conclusion: Cosmos在多种生成任务中表现优异，生成质量相当或更优，推理速度提升2倍以上。
...
<details>
  <summary>Details</summary>
Motivation: 确保MTEB作为文本嵌入模型评估平台的持续可重现性和可扩展性，以保持其在领域内的相关性和质量。

Method: 采用持续集成管道验证数据集完整性、自动化测试执行和评估结果泛化性，同时设计增强可重现性和可用性的方案。

Result: 成功扩展了MTEB的规模和全面性，同时保持了质量和领域相关性。

Conclusion: 本文的经验为机器学习评估框架的可重现性和可用性提供了宝贵见解，MTEB的工程实践对其他基准维护者具有参考价值。
...
TL;DR: 提出了一种基于文本提示的动态控制对话轮换预测模型，通过指令（如“更快”或“更平静”）直观调整预测行为，实验证明其提高了准确性和灵活性。


<details>
  <summary>Details</summary>
Motivation: 现有对话系统缺乏直观的动态控制能力，需要一种通过文本指令调整轮换预测的方法。

Method: 基于Transformer的语音活动预测模型，引入文本提示嵌入到通道和跨通道Transformer中，利用LLM生成合成提示数据。

Result: 在950小时对话数据上验证，模型提高了预测准确性，并能根据提示动态调整轮换时机。

Conclusion: 文本提示控制的轮换预测模型可行且有效，为对话系统提供了更灵活的动态调整能力。
...
TL;DR: 提出一种基于检索的提示策略，利用句法相似性而非语义相似性选择示例，以提高自动术语提取（ATE）的性能。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型（LLM）在ATE任务中的潜力，现有方法多依赖语义相似性，而句法相似性可能更有效。

Method: 采用检索式提示策略，在少样本设置下，基于句法相似性选择示例，并分析词法重叠对性能的影响。
...
TL;DR: 论文提出了Agent-RewardBench，一个用于评估多模态大语言模型（MLLMs）奖励建模能力的基准，解决了现有代理在自我纠正和泛化方面的不足。
...
Method: 设计了Agent-RewardBench基准，包含多维度评估、步骤级奖励评估和高质量数据采样，覆盖7种真实场景。

Result: 实验表明，即使是先进的多模态模型在奖励建模方面表现有限，需专门训练。

Conclusion: Agent-RewardBench为代理奖励建模提供了有效的评估工具，揭示了当前模型的不足和未来改进方向。
...
TL;DR: 论文探讨了大型语言模型生成虚假文本的能力，以及简单分类器在检测这些文本时的有效性。研究发现，尽管模型规模增大，但检测虚假文本仍可能可行。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨大型语言模型生成虚假文本的能力是否会随着模型规模的增加而无限提升，以及检测方法是否能够持续有效。

Method: 通过统计分类器检测古典侦探小说风格的虚假文本，比较不同模型（如Gemini和GPT）的生成能力。

Result: 研究发现，Gemini在生成欺骗性文本方面能力增强，而GPT没有明显变化，表明检测虚假文本仍可能可行。

Conclusion: 结论是，尽管模型规模增大，但虚假文本的可靠检测仍可能实现，但新模型架构可能提升欺骗性。
...
TL;DR: Double-Checker框架通过自我批判和迭代优化提升慢思考LLMs的推理能力，显著提高了AIME基准的表现。
...
Method: 提出Double-Checker框架，通过1,730个自我批判实例的微调，使LLMs能迭代批判和优化其输出。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在上下文信息不足时容易产生不可靠的回答，检测查询的groundedness可提升事实一致性和可信度。

Method: 使用RoBERTa和NomicBERT等轻量级编码器模型，在精选数据集上微调，进行groundedness检测。

Result: 轻量级模型在groundedness检测上达到与Llama3 8B和GPT4o相当的准确性，同时显著降低延迟。

Conclusion: 轻量级模型能高效检测查询的groundedness，减少LLMs的资源消耗和延迟。
...
<details>
  <summary>Details</summary>
Motivation: 研究语言上下文是否能独立支持从视觉对话中检测指代表达式，探讨单模态方法的潜力与局限。

Method: 采用预训练大型语言模型（LLM），通过下一词预测标注对话中的指代表达式边界，使用小数据集和参数高效微调。

Result: 实验表明，即使使用中等规模LLM和小数据集，仅文本方法也能有效完成任务，凸显语言上下文的重要性。

Conclusion: 任务本质是多模态的，单模态方法存在固有局限性，但语言上下文在指代表达式提取中具有重要作用。
...
TL;DR: 论文提出GLASS框架，基于Greimas符号学方阵，提升大语言模型（LLMs）的深度文学分析能力，填补研究空白。


<details>
  <summary>Details</summary>
Motivation: LLMs在文学批评领域表现不足，尤其是对思想深刻、叙事复杂的作品。

Method: 提出GLASS框架，构建首个GSS文学批评数据集，设计定量指标，并与专家批评对比。

Result: GLASS在多个作品和LLMs上表现优异，生成高质量分析。
...
TL;DR: Omni-RAG是一个新框架，旨在提升RAG系统在实时开放域环境中的鲁棒性和有效性，通过LLM辅助的查询理解和模块化处理复杂、多意图的查询。
...
Method: Omni-RAG采用三个关键模块：深度查询理解与分解、意图感知知识检索、以及重排序与生成，结合LLM和外部知识库优化查询处理。

Result: Omni-RAG能够有效处理复杂和噪声查询，提升RAG系统在实时应用中的表现。

Conclusion: Omni-RAG填补了当前RAG能力与实际应用需求之间的差距，为实时开放域场景提供了更鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 动态平台上语言模式的变化和概念漂移增加了检测欺诈对话的难度，现有LLM在风险敏感场景中表现不足。

Method: 提出DK-Enhanced LLM框架，包含三个模块：检测欺诈对话的DK-LLM、检测语义漂移的OCDD单元，以及分类漂移性质的DK-LLM。

Result: 在SEConvo数据集上，系统实现了98%的分类准确率，显著优于零样本基线。

Conclusion: 结合领域知识和漂移检测的框架显著提升了高风险NLP应用的性能和可解释性。
...
Result: 模型表现依次为英语>西班牙语>土耳其语，提示翻译对性能影响较小。

Conclusion: 研究强调多语言查询生成需更包容的评估与开发，未来工作包括模式本地化和多语言微调。
...
Result: 实验表明，通用对话的反馈能有效改进语音对话模型，使其更准确、安全和上下文对齐。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAE）在分析Transformer语言模型的激活空间时存在不足，如特征不稳定、无法区分概念缺失是SAE还是LM的问题。

Method: 在Transformer架构中引入TopK激活函数，使隐藏状态等同于TopK SAE的潜在特征，无需后训练。

Result: TopK LMs在模型大小、计算效率和解释性之间取得平衡，保持了原始能力，同时支持神经元干预和跨检查点分析。
...
TL;DR: 研究比较了强化学习方法在不同训练模式下（离线、半在线、完全在线）对大语言模型微调的效果，发现在线和半在线方法表现相似且优于离线方法。


<details>
  <summary>Details</summary>
Motivation: 探讨强化学习方法在不同训练模式下对可验证和不可验证任务的适用性。

Method: 比较在线和半在线直接偏好优化与组奖励策略优化目标，分析训练动态和超参数选择。

Result: 在线和半在线方法表现相似且优于离线方法，多任务联合训练可提升性能。

Conclusion: 在线和半在线方法在微调大语言模型时表现优异，多任务联合训练有助于提升整体性能。
...
TL;DR: 该论文提出了一种通过交互式LLM学习用户参与度的方法，利用未来对话发展的信号作为奖励，通过i×MCTS和DPO优化模型，显著提升了社交对话中的用户参与度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在社交对话中优化知识推理或对话行为规划，但未能直接保证用户参与度，因此需要更直接的方法来提升用户参与。

Method: 采用i×MCTS模拟用户与LLM的交互，收集高低质量对话数据，并通过DPO对齐LLM以优化用户参与度。

Result: 在情感支持和说服对话场景中，该方法显著提升了用户参与度。

Conclusion: 通过直接优化用户参与度信号，交互式LLM在社交对话中表现更优。
...
TL;DR: skLEP是首个针对斯洛伐克自然语言理解（NLU）模型的综合基准，包含九项多样化任务，并提供了对多种模型的系统性评估。


<details>
  <summary>Details</summary>
Motivation: 为斯洛伐克NLU研究提供全面评估工具，填补该领域空白。

Method: 通过整理新数据集和翻译英语NLU资源构建skLEP基准，并对多种模型进行评估。
...
TL;DR: 论文探讨了基于基准数据集评估大型语言模型（LLMs）的合理性，提出了一个框架来量化模型是否真正理解概念，还是仅表现出表面理解（Potemkin理解）。研究发现，Potemkin理解在模型、任务和领域中普遍存在，且反映了概念表征的内部不一致性。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLMs的基准数据集（如AP考试）与人类测试相同，但模型的理解是否与人类一致尚不明确。论文旨在验证模型是否真正理解概念，而非仅通过表面答案表现出理解。

Method: 提出了两种量化Potemkin理解的程序：一种使用专门设计的基准测试，另一种提供其普遍性的下限估计。

Result: 研究发现Potemkin理解在模型、任务和领域中普遍存在，且反映了概念表征的内部不一致性。

Conclusion: 基准测试的有效性依赖于模型是否以人类方式误解概念，否则仅表现为表面理解。研究揭示了模型概念表征的深层问题。
...
TL;DR: 论文分析了11K真实世界对话数据集HealthChat-11K，研究用户如何通过LLMs获取医疗信息，揭示了用户行为模式及潜在风险。


<details>
  <summary>Details</summary>
Motivation: 探索用户通过LLMs获取医疗信息的交互方式及其潜在风险，以改进医疗支持能力。

Method: 通过过滤大规模对话数据集，构建HealthChat-11K，并结合临床分类法分析21个医疗领域的用户交互。

Result: 揭示了用户行为模式（如不完整上下文、情感行为、诱导性问题）及LLMs的局限性。

Conclusion: 需改进LLMs在医疗支持中的能力，以减少潜在风险。
...
TL;DR: 论文提出了一种名为DELT的通用范式，专注于通过优化训练数据的组织（数据效能）来提升语言模型性能，包括数据评分、选择和排序三个组件，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中在数据效率（最小化或优化训练数据子集），而数据效能（优化数据组织）尚未充分探索，因此提出DELT范式填补这一空白。

Method: DELT包含三个组件：数据评分（如LQS）、数据选择和数据排序（如FO）。LQS从梯度一致性角度评估数据的可学习性和质量，FO解决模型遗忘和数据分布偏差问题。

Result: 实验表明，DELT在不增加数据规模和模型大小的情况下提升了语言模型性能，其中LQS和FO的组合效果最佳。数据效能与数据效率可同时实现。

Conclusion: 数据效能是语言模型训练中一个有前景的基础领域，DELT为优化数据组织提供了有效方法。
...
### [165] [IMA-Catcher: An IMpact-Aware Nonprehensile Catching Framework based on Combined Optimization and Learning](https://arxiv.org/abs/2506.20801)
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人捕捉飞行物体时因高冲击力导致的任务失败和硬件损坏问题，尤其是当物体质量与机器人负载比增加时。

Method: 预捕捉阶段通过实时最优规划器生成最小化速度差异的轨迹；后捕捉阶段基于人类示范生成位置、速度和刚度轨迹，并使用分层二次规划控制器。

Result: 实验表明，速度匹配避免了因冲击导致的过大关节扭矩，反射质量最小化提高了方法的鲁棒性，并验证了方法在多轴捕捉中的泛化能力。

Conclusion: 提出的框架有效减少了捕捉过程中的冲击力和反弹，适用于不同质量和空间条件下的飞行物体捕捉。
...
### [166] [Online Planning for Cooperative Air-Ground Robot Systems with Unknown Fuel Requirements](https://arxiv.org/abs/2506.20804)
...
<details>
  <summary>Details</summary>
Motivation: 解决无人机在未知燃料消耗目标下的在线路径规划问题。

Method: 采用两阶段方法：离线启发式规划和在线动态调整加油点。

Result: Gazebo模拟验证了方法的可行性，确保任务完成。

Conclusion: 该方法能有效维持无人机与地面车辆的路径有效性。
...
TL;DR: 论文提出了一种基于LiDAR的无人机电力线检测方法，通过最小化测量值与整体几何模型的误差，解决了导体点稀疏、检测不一致和干扰物区分的问题。


<details>
  <summary>Details</summary>
Motivation: 无人机电力线检测中，LiDAR传感器面临导体点稀疏、检测不一致和干扰物区分困难等挑战，需要一种更高效的方法。

Method: 提出了一种估计方法，通过最小化LiDAR测量值与整体导体阵列几何模型的误差，而非单独跟踪每根导体。

Result: 实验表明，该方法在部分观测、噪声和异常值情况下仍能实现准确跟踪，求解器每帧收敛时间低于50毫秒，且对异常点的容忍度较高。

Conclusion: 该方法在复杂环境下表现鲁棒，为无人机电力线检测提供了一种高效解决方案。
...
### [168] [Cooperative Circumnavigation for Multi-Quadrotor Systems via Onboard Sensing](https://arxiv.org/abs/2506.20954)
...
<details>
  <summary>Details</summary>
Motivation: 解决多四旋翼系统在视觉遮挡环境下对移动目标的协同跟踪问题，并提升系统的鲁棒性和容错能力。

Method: 采用异构感知策略和状态估计算法，开发了改进的卡尔曼滤波器融合视觉-惯性里程计和距离测量，设计了事件触发的分布式卡尔曼滤波器以实现目标状态估计。
...
### [169] [Parallels Between VLA Model Post-Training and Human Motor Learning: Progress, Challenges, and Trends](https://arxiv.org/abs/2506.20966)
...
TL;DR: 该论文综述了视觉-语言-动作（VLA）模型的后训练策略，从人类运动学习的角度出发，提出了四个维度的分类法，并总结了关键挑战和未来趋势。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作任务中表现出泛化能力，但在高精度需求下存在性能差距，需要后训练来适应下游任务。

Method: 通过人类运动学习的视角，提出了环境感知、实体意识、任务理解和多组件整合四个维度的后训练策略分类法。

Result: 建立了一个概念框架，总结了当前VLA模型后训练方法，并提供了实践指导。

Conclusion: 该研究为VLA模型的后训练提供了全面的视角和未来研究方向。
...
### [170] [ThermalDiffusion: Visual-to-Thermal Image-to-Image Translation for Autonomous Navigation](https://arxiv.org/abs/2506.20969)
*Shruti Bansal,Wenshan Wang,Yifei Liu,Parv Maheshwari*

Main category: cs.RO

TL;DR: 论文提出利用条件扩散模型将RGB图像转换为热图像，以解决热成像数据不足的问题，促进热相机在机器人领域的应用。


<details>
  <summary>Details</summary>
Motivation: 热相机在夜间或恶劣环境下能提供有价值的信息，但现有数据集缺乏热成像数据，限制了其在自主系统中的应用。

Method: 使用条件扩散模型和自注意力机制，将现有RGB图像转换为合成热图像。

Result: 通过合成热图像，增强了多模态数据集，支持热相机在场景分割、目标检测和深度估计等任务中的应用。

Conclusion: 合成热图像的方法为热相机的快速普及提供了可行解决方案。
...
### [171] [Fault-Tolerant Spacecraft Attitude Determination using State Estimation Techniques](https://arxiv.org/abs/2506.21016)
...
<details>
  <summary>Details</summary>
Motivation: 研究这些滤波器在大型卫星姿态估计中的性能，以及如何应对传感器测量错误。

Method: 使用扩展卡尔曼滤波、无迹卡尔曼滤波和粒子滤波，分析其在低地球轨道大型卫星中的应用，并探讨故障检测、隔离和恢复技术。

Result: 得出了不同滤波器在各种故障模式下的性能表现。

Conclusion: 这些滤波器为卫星姿态估计提供了鲁棒的框架，且相关技术在故障处理中表现良好。
...
### [172] [STEP Planner: Constructing cross-hierarchical subgoal tree as an embodied long-horizon task planner](https://arxiv.org/abs/2506.21030)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在长时程任务规划中成功率低的问题。
...
### [173] [V2X-REALM: Vision-Language Model-Based Robust End-to-End Cooperative Autonomous Driving with Adaptive Long-Tail Modeling](https://arxiv.org/abs/2506.21041)
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶在罕见、多样化和视觉退化的长尾场景中的规划与决策挑战，特别是在协同感知和推理的复杂环境中。

Method: 提出三个核心创新：1）基于提示的长尾场景生成与评估管道；2）门控多场景自适应注意力模块；3）多任务场景感知对比学习目标。
...
### [174] [Active Disturbance Rejection Control for Trajectory Tracking of a Seagoing USV: Design, Simulation, and Field Experiments](https://arxiv.org/abs/2506.21265)
*Jelmer van der Saag,Elia Trevisan,Wouter Falkena,Javier Alonso-Mora*

Main category: cs.RO

TL;DR: 本文提出了一种基于ADRC的轨迹跟踪控制器，用于解决USV在不确定环境干扰下的控制问题，并通过仿真和实地测试验证其性能。
...
Method: 采用ADRC设计控制器，通过仿真和实地测试（包括港口和海上试验）验证性能。
...
### [175] [Knowledge-Driven Imitation Learning: Enabling Generalization Across Diverse Conditions](https://arxiv.org/abs/2506.21057)
...
<details>
  <summary>Details</summary>
Motivation: 模仿学习在机器人操作中表现强大，但受限于专家演示的物体依赖性，泛化能力不足。
...
### [176] [Control of Marine Robots in the Era of Data-Driven Intelligence](https://arxiv.org/abs/2506.21063)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于模型的控制方法在海洋机器人中面临非线性和环境复杂性的挑战，数据驱动智能为控制策略提供了新思路。

Method: 综述了数据驱动控制在单个和协作海洋机器人系统中的研究进展，并总结了相关开源资源。

Result: 总结了数据驱动控制在海洋机器人中的显著成果，并提供了未来研究的指导方向。
...
### [177] [CURL-SLAM: Continuous and Compact LiDAR Mapping](https://arxiv.org/abs/2506.21077)
...
Method: 利用CURL的球形谐波隐式编码，提出CURL-SLAM方法，通过优化问题实现姿态估计，并扩展至局部Bundle Adjustment。

Result: 实验表明，CURL-SLAM在3D地图质量和轨迹精度上达到先进水平，且能在CPU上实现实时性能（10 Hz）。
...
### [178] [UAIbot: Beginner-friendly web-based simulator for interactive robotics learning and research](https://arxiv.org/abs/2506.21178)
*Johnata Brayan,Armando Alves Neto,Pavel Petrovič,Gustavo M Freitas,Vinicius Mariano Gonçalves*

Main category: cs.RO

TL;DR: UAIbot是一个免费开源的基于网络的机器人模拟器，旨在解决传统模拟平台的教育和研究挑战。


<details>
  <summary>Details</summary>
Motivation: 解决传统模拟平台的安装复杂性和可访问性问题，为教育和研究提供更便捷的工具。

Method: 通过Python和JavaScript接口，提供交互式学习体验，涵盖从机械臂运动学到行人流动力学的基本原理。
...
### [179] [Dynamic Risk-Aware MPPI for Mobile Robots in Crowds via Efficient Monte Carlo Approximations](https://arxiv.org/abs/2506.21205)
*Elia Trevisan,Khaled A. Mustafa,Godert Notten,Xinwei Wang,Javier Alonso-Mora*

Main category: cs.RO

TL;DR: 提出了一种动态风险感知模型预测路径积分控制（DRA-MPPI），用于实时处理非高斯预测轨迹的不确定性，提高移动机器人在动态环境中的安全性。
...
Method: 利用MPPI的无梯度特性，通过蒙特卡洛方法高效近似多动态障碍物的联合碰撞概率（CP），并将其作为导航成本函数的加权目标或阈值筛选条件。

Result: DRA-MPPI在真实和模拟实验中表现优于S-MPC、Frenet规划器和原始MPPI，显著提升了安全性并减少了“冻结”问题。
...
### [180] [ACTLLM: Action Consistency Tuned Large Language Model](https://arxiv.org/abs/2506.21250)
...
### [181] [Real-time Terrain Analysis for Off-road Autonomous Vehicles](https://arxiv.org/abs/2506.21347)
...
<details>
  <summary>Details</summary>
Motivation: 解决道路粗糙度变化导致的自动驾驶车辆控制问题，如路线偏离和车轮脱离路面的风险。

Method: 结合高斯过程代理模型和半车辆模型，利用贝叶斯校准从车轴加速度反推道路粗糙度，生成量化不确定性的后验分布。

Result: 实验验证表明，系统能实时准确表征道路粗糙度，集成Simplex控制器后显著提升车辆操作安全性。

Conclusion: 该贝叶斯框架为自动驾驶系统提供了降低粗糙度相关风险、提高效率和安全性的全面解决方案。
...
### [182] [WorldVLA: Towards Autoregressive Action World Model](https://arxiv.org/abs/2506.21539)
...
TL;DR: WorldVLA是一个结合视觉-语言-动作（VLA）模型与世界模型的统一框架，通过自回归方式预测未来图像和生成动作，展示了相互增强的效果，并提出注意力掩码策略以改善动作序列生成的性能。


<details>
  <summary>Details</summary>
Motivation: 整合视觉、语言和动作理解与生成，以学习环境物理规律并提升动作生成能力。

Method: 结合VLA模型与世界模型，利用动作和图像理解预测未来图像，同时基于图像观察生成后续动作。提出注意力掩码策略以减少动作序列生成中的误差传播。

Result: WorldVLA优于独立的动作和世界模型，但自回归动作生成性能下降。注意力掩码策略显著提升了动作块生成任务的性能。

Conclusion: WorldVLA展示了视觉与动作模型的相互增强，注意力掩码策略有效改善了动作序列生成的问题。
...
### [183] [Distributed Lyapunov Functions for Nonlinear Networks](https://arxiv.org/abs/2506.20728)
*Yiming Wang,Arthur N. Montanari,Adilson E. Motter*

Main category: eess.SY

TL;DR: 提出了一种基于局部信息的分布式方法，用于构建Lyapunov函数，以解决高维非线性网络中多稳态系统的吸引域（ROAs）复杂形态问题。


<details>
  <summary>Details</summary>
Motivation: 高维非线性网络的多稳态特性导致吸引域形态复杂，传统优化方法难以处理，需要新的局部化解决方案。

Method: 通过建立增强的比较引理，提出基于局部信息的分布式SOS优化方法，迭代构建部分Lyapunov函数并聚合为复合函数。

Result: 复合Lyapunov函数能准确逼近高维非凸吸引域的形状和体积，并在van der Pol和Ising振荡器网络中验证了有效性。

Conclusion: 该方法为高维非线性系统的吸引域分析提供了有效的局部化工具，适用于复杂形态的吸引域表征。
...
### [184] [Noise-Tolerant Hybrid Approach for Data-Driven Predictive Control](https://arxiv.org/abs/2506.20780)
*Mahmood Mazare,Hossein Ramezani*

Main category: eess.SY

TL;DR: 论文提出了一种噪声容忍的数据驱动预测控制框架（NTDPC），通过奇异值分解分离系统动态与噪声，提升预测精度和计算效率。
...
Method: 提出NTDPC框架，利用奇异值分解在降阶Hankel矩阵中分离系统动态与噪声，并引入灵敏度指数支持不同噪声水平下的时间范围选择。

Result: 仿真结果表明，该方法在鲁棒性和效率上优于现有混合方法。

Conclusion: NTDPC框架通过噪声处理优化了预测控制性能，适用于短数据范围和低计算需求场景。
...
### [185] [DPLib: A Standard Benchmark Library for Distributed Power System Analysis and Optimization](https://arxiv.org/abs/2506.20819)
*Milad Hasanzadeh,Amin Kargarian*

Main category: eess.SY

TL;DR: DPLib是一个开源的MATLAB基准库，用于支持分布式和去中心化电力系统分析与优化的研究。它填补了现有工具在分布式研究领域的空白，提供了多区域测试案例和分区工具。


<details>
  <summary>Details</summary>
Motivation: 现代电力系统需要分布式方法以实现可扩展性、隐私保护和抗单点故障能力，但缺乏通用的分布式研究工具。

Method: DPLib提供20多个多区域测试案例、基于图的分区工具，以及分布式最优潮流（OPF）求解器（ADMM-based DC-OPF和AC-OPF）。

Result: 数值结果验证了测试案例的有效性，DPLib成为可复现分布式电力系统研究的基础。

Conclusion: DPLib填补了分布式电力系统研究的工具空白，为未来研究提供了标准化支持。
...
### [186] [Resilience Through Escalation: A Graph-Based PACE Architecture for Satellite Threat Response](https://arxiv.org/abs/2506.20882)
*Anouar Boumeftah,Sarah McKenzie-Picot,Peter Klimas,Gunes Karabulut Kurt*

Main category: eess.SY

TL;DR: 论文提出了一种基于PACE方法的卫星系统韧性设计框架，通过动态韧性指数和三种决策模型评估系统在多样化干扰场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 传统冗余策略难以应对动态、多向威胁，卫星系统面临日益增长的运行风险，如干扰、网络攻击和电磁干扰。

Method: 采用PACE方法，结合CVSS、DREAD和NASA风险矩阵，构建分层状态转换模型，并实现静态、自适应和softmax决策模型。

Result: 轻量级、决策感知的备用机制显著提高了下一代空间资产的生存能力和运行连续性。

Conclusion: 基于PACE的韧性设计框架为卫星系统提供了有效的动态适应性解决方案。
...
### [187] [Optimal Parameter Design for Power Electronic Converters Using a Probabilistic Learning-Based Stochastic Surrogate Model](https://arxiv.org/abs/2506.20987)
*Akash Mahajan,Shivam Chaturvedi,Srijita Das,Wencong Su,Van-Hai Bui*

Main category: eess.SY

TL;DR: 本文提出了一种基于概率学习的随机代理建模框架，用于优化电力电子转换器设计参数，平衡效率与热约束，显著缩短设计时间。


<details>
  <summary>Details</summary>
Motivation: 电力电子转换器设计需在效率与热约束间取得平衡，传统方法耗时且难以保证性能与安全。

Method: 结合神经网络分类器筛选可行参数，概率预测模型评估效率与温度，启发式优化模型优化多目标函数。

Result: 与多种搜索算法相比，该方法显著提高了预测准确性和优化效果。
...
### [188] [Coordinated Control of Autonomous Vehicles for Traffic Density Reduction at a Signalized Junction: An MPC Approach](https://arxiv.org/abs/2506.21302)
*Rudra Sen,Subashish Datta*

Main category: eess.SY

TL;DR: 提出了一种双模式模型预测控制（MPC）架构，用于优化信号交叉口的交通密度和高密度交通条件下的协作变道。


<details>
  <summary>Details</summary>
Motivation: 随着城市交通系统的快速发展，如何有效且安全地管理交通成为关键问题。联网自动驾驶车辆（CAVs）的互联能力为提升交通流和协调提供了新机会。

Method: 采用双模式MPC架构，结合在线计算的最大控制不变终端集，解决交通密度和协作变道问题。

Result: 通过数值模拟验证了所提方法的有效性，确保了递归可行性和收敛性。

Conclusion: 该架构提升了CAVs的决策响应能力，从而提高了城市交通的效率和安全性。
...
### [189] [Estimating Technical Loss without Power Flows: A Practical, Data-Driven Approach for Loss Estimation in Distribution Grids](https://arxiv.org/abs/2506.21311)
*Mohini Bariya,Genevieve Flaspohler*

Main category: eess.SY

TL;DR: 论文提出了一种利用稀疏电压测量数据估算电网技术损耗的新方法，适用于低收入和中等收入国家（LMICs）的电网。


<details>
  <summary>Details</summary>
Motivation: 低收入和中等收入国家的电网因基础设施老化、资金不足等问题，技术损耗高达20%以上，远超高收入国家的5%。现有方法依赖昂贵的电力流传感技术，难以在LMICs中实施。

Method: 通过利用电网中稀疏位置的电压幅值测量数据，开发了一种无需电力流传感的技术损耗估算方法。

Result: 该方法为LMICs电网提供了可行的技术损耗估算和定位工具，支持损耗减少干预措施的设计和实施。
...
### [190] [Joint Scheduling of DER under Demand Charges: Structure and Approximation](https://arxiv.org/abs/2506.21510)
*Ruixiao Yang,Gulai Shen,Ahmed S. Alahmed,Chuchu Fan*

Main category: eess.SY

TL;DR: 论文研究了在净计量框架下联合调度分布式能源资源（DERs），提出了一种基于阈值的最优控制策略，并通过高效近似算法解决了计算复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决在净计量和需求费用框架下，分布式能源资源的联合调度问题，以最大化运营盈余并应对可再生能源的不确定性。

Method: 方法包括将问题建模为随机动态规划，提出基于阈值的最优控制策略，并开发了一种线性复杂度的近似算法。

Result: 通过仿真验证，该算法在多种场景下表现优于其他控制策略，接近理论上界。

Conclusion: 结论表明，提出的算法在计算效率和性能上均具有优势，适用于实际应用。
...
### [191] [Global and Local Contrastive Learning for Joint Representations from Cardiac MRI and ECG](https://arxiv.org/abs/2506.20683)
...
Method: PTACL结合全局患者级和局部时间级对比损失，对齐ECG与CMR表征，无需新增可学习权重。
...
### [192] [Building Lightweight Semantic Segmentation Models for Aerial Images Using Dual Relation Distillation](https://arxiv.org/abs/2506.20688)
*Minglong Li,Lianlei Shan,Weiqiang Wang,Ke Lv,Bin Luo,Si-Bao Chen*

Main category: eess.IV

TL;DR: 提出了一种新颖的双关系蒸馏（DRD）技术，通过传递空间和通道关系从复杂模型（教师）到紧凑模型（学生），以平衡语义分割的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 尽管CNN模型在语义分割准确性上有显著提升，但其计算量大、推理速度慢，限制了实际应用。知识蒸馏成为一种解决方案。

Method: 通过分别计算教师和学生模型的空间和通道关系图，并最小化它们之间的距离来对齐关系图，从而传递更丰富的特征分布。

Result: 在三个分割数据集（Vaihingen、Potsdam和Cityscapes）上的实验表明，该方法显著提升了学生模型的性能，且未增加额外计算开销。

Conclusion: 双关系蒸馏技术有效提升了紧凑模型的语义分割准确性，同时保持了高效性。
...
### [193] [U-R-VEDA: Integrating UNET, Residual Links, Edge and Dual Attention, and Vision Transformer for Accurate Semantic Segmentation of CMRs](https://arxiv.org/abs/2506.20689)
*Racheal Mukisa,Arvind K. Bansal*

Main category: eess.IV

TL;DR: 提出了一种基于深度学习的增强UNet模型U-R-Veda，用于心脏磁共振图像的自动语义分割，结合了卷积变换、视觉变换器、注意力机制等技术，显著提升了分割精度。
...
Method: U-R-Veda模型整合了卷积变换、视觉变换器、残差连接、通道和空间注意力机制，以及基于边缘检测的跳跃连接，以减少信息损失。

Result: 模型在DSC指标上达到95.2%的平均准确率，优于其他模型，尤其在右心室和左心室心肌的分割上表现突出。

Conclusion: U-R-Veda模型显著提升了心脏磁共振图像的语义分割精度，为医学图像分析提供了更可靠的工具。
...
### [194] [Development of MR spectral analysis method robust against static magnetic field inhomogeneity](https://arxiv.org/abs/2506.20897)
...
Method: 利用深度学习模型训练模拟光谱，结合B0图和健康人脑代谢物比例生成模拟光谱，并通过视觉和定量评估验证其质量。

Result: 模拟光谱与实测光谱接近，使用模拟光谱训练的模型显著降低了代谢物比例的误差，性能优于传统方法。
...
### [195] [A Novel Framework for Integrating 3D Ultrasound into Percutaneous Liver Tumour Ablation](https://arxiv.org/abs/2506.21162)
*Shuwei Xing,Derek W. Cool,David Tessier,Elvis C. S. Chen,Terry M. Peters,Aaron Fenster*

Main category: eess.IV

TL;DR: 提出了一种将3D超声（US）整合到标准肝肿瘤消融工作流程的新框架，包括2D US-CT/MRI配准方法和多模态图像可视化技术，显著提高了配准精度和效率。


<details>
  <summary>Details</summary>
Motivation: 3D超声在肝肿瘤消融中具有潜力，但因肿瘤识别困难阻碍了其广泛应用。本研究旨在通过技术整合解决这一问题。

Method: 提出了一种临床可行的2D US-CT/MRI配准方法，利用3D超声作为中介降低配准复杂度，并开发了多模态图像可视化技术。

Result: 2D US-CT/MRI配准的标记距离误差为2-4 mm，运行时间为0.22秒/图像对；非刚性配准比刚性配准减少了40%的平均对齐误差。

Conclusion: 该框架提升了3D超声在肝肿瘤消融中的应用潜力，展示了其在临床干预中的治疗价值。
...
### [196] [Uncover Treasures in DCT: Advancing JPEG Quality Enhancement by Exploiting Latent Correlations](https://arxiv.org/abs/2506.21171)
...
<details>
  <summary>Details</summary>
Motivation: 现有JPEG质量增强方法在像素域操作计算成本高，而DCT域方法性能有限，因此需要一种更高效的方法。

Method: 识别JPEG图像DCT系数中的关键相关性，提出AJQE方法，将像素域模型适配到DCT域。
...
### [197] [GANet-Seg: Adversarial Learning for Brain Tumor Segmentation with Hybrid Generative Models](https://arxiv.org/abs/2506.21245)
*Qifei Cui,Xinyu Lu*

Main category: eess.IV

TL;DR: 提出了一种结合预训练GAN和Unet架构的脑肿瘤分割新框架，通过全局异常检测和精细化掩模生成网络提升分割精度。
...
Method: 结合预训练GAN和Unet架构，利用全局异常检测模块和精细化掩模生成网络，通过对抗损失约束迭代优化分割结果。

Result: 在BraTS数据集上表现出色，Dice和HD95指标优于基线方法，减少了对全标注数据的依赖。
...
### [198] [Lightweight Physics-Informed Zero-Shot Ultrasound Plane Wave Denoising](https://arxiv.org/abs/2506.21499)
...
<details>
  <summary>Details</summary>
Motivation: 低角度CPWC成像虽然能提高帧率，但噪声和伪影问题显著，传统方法依赖额外数据或训练，限制了适应性。

Method: 将传输角度分为两个子集，生成含噪声的复合图像，通过自监督残差学习训练轻量级网络，分离噪声与组织信号。
...
TL;DR: 本文系统研究了3D多模态大语言模型（MLLMs）的设计空间，包括视觉输入表示、投影器、大语言模型（LLMs）和微调技术，用于3D CT报告生成。通过两种知识增强方法，性能提升10%，在MICCAI 2024 AMOS-MM挑战赛中排名第二。
...
Method: 研究了3D MLLMs的设计空间，包括视觉输入表示、投影器、LLMs选择和微调技术，并引入两种知识增强方法。

Result: 在1,687例AMOS-MM数据集上，RRG性能与LLM大小无关；更大的体积尺寸不一定提升性能；结合分割掩模可提升效果。

Conclusion: 3D MLLMs在RRG中表现优异，知识增强方法显著提升性能，设计选择和预训练数据匹配是关键。
