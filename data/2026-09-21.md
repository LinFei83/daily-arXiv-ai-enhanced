...
TL;DR: 论文提出一种免训练的稀疏预填充方法 RBS-Attention，通过双分支块选择缓解长上下文推理中预填充注意力开销大的问题，并在 H100 上取得显著加速且保持较高精度。


<details>
  <summary>Details</summary>
Motivation: 长上下文大语言模型推理受预填充阶段密集自注意力计算限制，现有稀疏块选择用块质心表示相关性，可能掩盖块内高度相关但被大量无关 token 稀释的 token，作者称该失败模式为均值稀释。

Method: 提出 RBS-Attention，一种免训练稀疏预填充方法，包含两个互补选择分支：质心基础分支捕捉平均相关性，救援分支利用最大 key 块半径及其随提示、层、头变化的分布识别可能被低估的块。两分支独立阈值化并合并掩码，控制救援块贡献，同时保持规则块稀疏 FlashAttention 执行。

Result: 在 H100 GPU 上，128K 上下文下 Qwen3-30B-A3B-Instruct-2507-FP8 上实现 20.65 倍独立预填充注意力加速、11.92 倍 vLLM 预填充注意力加速和 5.97 倍端到端首 token 时间加速。在密集 Qwen3-32B 上，RULER 总体准确率为 88.65，对比密集注意力 89.52；并在 LongBench-v2、InfiniteBench 和 Video-MME 上提供额外质量评估。

Conclusion: 半径自适应双分支选择是长上下文预填充的有效方法，可在显著加速的同时保持与密集注意力相近的生成质量。
...
TL;DR: 论文提出注意力感知路由（AAR），利用注意力权重滑动窗口提取的时序与频谱特征增强MoE路由，在冻结基础Transformer、仅训练路由参数下提升GSM8K，并揭示路由与注意力的耦合关系及深度敏感性。


<details>
  <summary>Details</summary>
Motivation: MoE语言模型的router通常仅依据token隐藏状态选择并加权专家，所利用的上下文信息有限。

Method: 提出AAR：从注意力权重滑动窗口中提取时序和频谱特征，作为模型上下文状态的总结，并与隐藏状态解耦；保持基础Transformer冻结，仅训练路由参数。

Result: 在OLMoE上，AAR较仅路由SFT基线将GSM8K提升+3.37个百分点。路由与注意力形成耦合电路：第l层路由变化经残差流传播，放大第l+1层注意力汇，重塑注意力而不直接更新注意力机制。AAR减少长发散生成，错误答案变短，正确答案长度不变。AAR对深度敏感：在所有层 indiscriminately 应用可能损害事实检索，而数学推理增益在更深层引入时仍保持。

Conclusion: AAR体现检索与推理在深度上的张力，层选择性AAR可作为受控探针，研究不同层注意力所携带的路由相关信息。
...
TL;DR: 提出Causal Latent Revision (CaLR)框架，将推理重构为受约束的潜在优化，结合自回归与扩散语言模型优势，实现并行生成中的动态自我纠错。


<details>
  <summary>Details</summary>
Motivation: 自回归模型存在局部贪婪问题，而扩散语言模型常缺乏推理所需的严格因果结构，因此需要结合二者优势并克服各自缺陷。

Method: 采用专家模型的因果拓扑矩阵（CTM）和隐式微分，进行梯度引导的“思维修订”，以强制逻辑一致性，并在并行生成过程中动态自校正中间步骤。

Result: 在复杂基准上取得SOTA的扩散语言模型性能，超越强自回归基线，并在Sudoku等约束任务中展现出更优鲁棒性。

Conclusion: CaLR能有效融合自回归与扩散语言模型的优势，提升扩散模型在复杂推理和约束满足任务中的表现，表明受约束潜在优化可增强并行生成的自校正能力。
...
TL;DR: 该论文将DINOv3 ViT模型用于合成孔径声呐自动目标识别，提出三阶段参数高效适配框架，发现仅LoRA适配即可显著提升性能，后续硬负样本挖掘和监督对比学习未带来额外增益。


<details>
  <summary>Details</summary>
Motivation: 水下SAS目标识别对海军能力很重要，但目标图像稀少、背景杂波强且依赖人工评估，深度学习应用受限，因此需要高效的迁移适配方法。

Method: 采用三阶段框架：第一阶段冻结ViT骨干，仅用LoRA适配自然图像预训练与水下声学传播之间的差距；第二阶段用硬负样本挖掘增强对岩石、沉积物等声学混淆物的决策边界；第三阶段用监督对比学习分离目标与杂波表示。在海上SAS数据上按任务级地理划分评估，在85%测试召回率下比较各分支，并用三个随机种子重复。

Result: LoRA是主要有效因素，将AUPRC从0.300提升到0.679±0.027，且Rank 4仅训练0.26%权重。硬负样本挖掘相比等规模随机课程使AUPRC变化-0.0045±0.0119，SupCon相比前一阶段变化+0.0002±0.0096，均未超过匹配对照。

Conclusion: 后续精炼阶段无效，说明挖掘发生在编码器已拟合的数据上，且监督阶段已基本塑造了目标-杂波几何结构。因此一个高效适配阶段已足够，堆叠式精炼并非必要。
...
TL;DR: 本文通过分析注意力图中的信息流拓扑来区分幻觉与非幻觉回答，重点使用Forman-Ricci曲率识别信息瓶颈，并提出一种单次推理方法捕获与幻觉相关的注意力头半局部和全局信息流特征。


<details>
  <summary>Details</summary>
Motivation: 现有方法在区分LLM幻觉与非幻觉回答时仍有不足，需要从注意力图的信息流结构角度寻找更有效的检测信号，尤其是识别注意力中的信息瓶颈和上下文共享异常。

Method: 分析注意力图中的Forman-Ricci曲率以识别信息瓶颈；提出一种捕获幻觉相关注意力头的半局部与全局信息流特征的方法；在多个LLM和基准上进行广泛评估，并与基于注意力及多响应基线比较。

Result: 所提单次推理方法在两个幻觉检测基准上持续优于现有基于注意力和多响应基线，并在多种LLM架构上取得有竞争力的性能。分析发现，因果生成中token间上下文共享受损与幻觉强相关；幻觉回答常表现为过度依赖自注意力、从早期token扩散检索上下文，或信息过度压缩，尤其在最终Transformer层。

Conclusion: 注意力图的信息流拓扑能有效刻画和检测LLM幻觉，幻觉与上下文共享受损密切相关；所提方法在多个模型和基准上验证了其有效性与通用性。
...
TL;DR: 该论文研究微调如何改变LLM内部表示，并分析这些变化与EAP识别的任务关键组件之间的关系。


<details>
  <summary>Details</summary>
Motivation: 微调已被广泛用于适配LLM到下游任务，但其如何重塑模型内部机制仍缺乏理解。

Method: 作者考察微调后LLM的注意力模式和逐层激活变化，并使用EAP识别任务相关组件（如注意力头和logit级激活），进而比较这些组件与表示变化层及跨任务重叠情况。

Result: EAP识别的组件集中在特定层，表明存在一定功能定位；但这些组件的层分布与微调中表示变化最大的层基本不相关。跨任务的EAP组件重叠并不一定带来跨任务性能迁移，尤其当任务性质不同（如分类 vs. 生成）时；若两个任务在EAP组件上高度重叠，微调一个任务可能导致另一个任务性能下降。

Conclusion: 微调对LLM内部表示的重塑与任务关键组件并不简单对应，任务间组件重叠可能引发负迁移，提示需更细致理解微调机制与跨任务影响。
...
TL;DR: 提出 TinyCeNN-LM，一种质量门控的后训练转换框架，用受 CeNN 启发的细胞循环层替换预训练语言模型中的注意力，并通过严格的表示与 NLL 验证决定是否接受转换。


<details>
  <summary>Details</summary>
Motivation: 在预训练语言模型中替换注意力本质上是兼容性问题：看似合理的替代层可能改变后续层所期望的表示，因此需要谨慎的验证机制来避免破坏模型行为。

Method: 采用质量门控的后训练转换框架，使用具有有界局部处理、紧凑循环记忆、路由、融合以及接受或回滚验证的细胞循环层。研究了三种实现：Integrated Memory、MemoryFusion 和 PDelta3-GDN2-CLVR+Local32；其中严格 PDelta3 转换仅在表示和 NLL 指标通过固定阈值时才接受该层。

Result: 在 SmolLM2-135M 上，第 0–2 层被接受，累计 ΔNLL=+0.01209，而第 3 层虽 NLL 可接受却因表示保真度不达标被拒绝。在 Qwen3.5-0.8B 上，全注意力层 3、7、11 被接受，最终 ΔNLL=+0.02073。Integrated Memory 将困惑度控制在 -0.07% 至 +0.93% 之间，同时总缓存最多减少 6.01%。对转换后 Qwen 版本进行 200 项下游抽样检查，总体准确率为 28.5%–32.0%。

Conclusion: 结果支持采用保守的、质量门控的结构转换，而非普遍性的注意力替换或加速。
...
### [8] [Can Agents Design Better Chips with a Higher Level Abstraction?](https://arxiv.org/abs/2609.21157)
*Zijian Ding,Yang Zou,Yizhou Sun,Jason Cong*

Main category: cs.AI

TL;DR: 本文研究LLM智能体能否借助更高层抽象来设计芯片，提出将基于智能体的HLS设计与HLS后RTL精炼相结合的AHRR流程，并在11个任务的FPGA基准上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体用于芯片设计时大多直接在RTL层面操作，作者希望探究利用更高层抽象能否让智能体设计出更好的芯片。

Method: 比较直接RTL设计、基于智能体的HLS设计、编译器后HLS精炼、HLS后RTL精炼四种方式，并将基于智能体的HLS设计与HLS后RTL精炼组合为AHRR；使用FPGA作为端到端评估平台，在11个多样化任务上评测。

Result: AHRR在基准套件上相对直接RTL设计取得2.6倍几何平均加速；案例研究表明HLS能将设计知识蒸馏为智能体可利用的抽象，而RTL精炼可恢复低层优化机会。

Conclusion: AHRR是面向智能体芯片设计的一种有前景的工作流，代码与评估工件已开源。
...
TL;DR: 提出并试点评估一个基于记忆增强患者模拟器的临床医生主导评估平台 InterviewPlayground，用于开放式AI精神科接诊访谈的质量保证。


<details>
  <summary>Details</summary>
Motivation: AI辅助精神科接诊系统在临床应用前，健康系统需要可常规对照临床质量标准进行评估的方法；且需兼容不同访谈风格、减少临床医生负担并衡量临床相关表现。

Method: 构建 InterviewPlayground：以专家撰写的情景病例生成交互式患者，搭建模拟接诊平台，设计接诊相关评估维度。6名临床医生在25分钟评估中与基于GPT的LLM接诊访谈者对比。

Result: LLM比临床医生恢复更多临床相关条目（88.0% vs 38.9%），但也产生更多非基于访谈的临床推断（56.8% vs 27.8%），且更少描述已识别的安全问题（33.3% vs 66.7%）。

Conclusion: 该平台为AI精神科接诊访谈的部署后质量保证奠定基础，但初步试点显示LLM在安全关切表述等方面仍需改进。
...
TL;DR: 论文提出 SpecOpt 任务，即对已有药物进行受约束的结构修饰，以提升其对目标靶标相对于已知脱靶靶标的选择性，同时保持结构相似性和成药性；并开发了一个基于对接、残基感知接触比较和大语言模型的智能体框架。


<details>
  <summary>Details</summary>
Motivation: 脱靶蛋白结合是小分子药物不良反应的主要来源，但大多数基于结构的分子设计方法侧重于从头生成高选择性化合物，而非改善已有且表征充分的药物的选择性。

Method: 构建 ChEMBL 衍生基准，利用药物机制注释确定 intended target、实测活性确定 off-target；开发智能体框架，将化合物与目标靶标及脱靶靶标对接，通过残基感知的原子-蛋白接触比较构象差异，并交给大语言模型提出针对性结构修饰；候选物需满足分子相似性、ADMET 和靶标-脱靶对接选择性标准。

Result: 在 915 个化合物上，该方法使 84.8% 的化合物目标-脱靶结合差距得到改善，平均差距从 -0.72 kcal/mol 提升至 +0.47 kcal/mol，同时与起始化合物的平均 Tanimoto 相似度为 0.72。消融实验表明，残基特异性接触信息是关键优化信号：将残基身份替换为二值接触指标后，所有 29 个消融化合物均无改善。

Conclusion: SpecOpt 被确立为一个独立的分子设计问题，残基感知的差异相互作用可作为提升现有化合物特异性的有效信号。
...
TL;DR: 本文研究 Vision ARC（VARC）中的测试时任务嵌入，提出一种两步测试时训练协议：先仅微调任务嵌入（Embed-TTT），再冻结嵌入并微调骨干网络，以更好区分规则推断与规则执行。


<details>
  <summary>Details</summary>
Motivation: ARC 及相关基准测试难以判断模型成功是真正推断了底层规则，还是依赖捷径；因此需要更清晰地分离规则归纳与规则执行。

Method: 在 VARC 中引入两步 TTT：第一步仅训练代表变换规则的任务嵌入（Embed-TTT），第二步冻结该嵌入并微调预训练骨干网络。并在 ARC-AGI-1、ConceptARC 及两个已知规则的控制数据集上评估嵌入质量、检索、线性探测和规则泛化。

Result: Embed-TTT 持续产生更优任务嵌入，更符合底层规则，提升基于嵌入的检索，并能准确线性探测已知规则；在 ARC-AGI-1 上识别出更有语义的测试-训练任务关系。仅优化少于 0.01% 参数的任务嵌入即可解决部分 ARC-AGI-1、ConceptARC 和 Mini-ARC 任务，完整两步流程进一步提升性能。Embed-TTT 还能恢复参数化规则的几何结构，并支持规则级插值而非外推。

Conclusion: 研究支持在 ARC 类评估中更清晰地区分规则归纳与规则执行，并推动构建能区分分布内与分布外规则的基准。
...
TL;DR: 提出AI-GRACE框架，用于将组织治理与代理式AI用例的技术实施相连接，明确目标与义务、七类风险、部署前保证、运行时控制和证据要求，并以零售银行案例说明。


<details>
  <summary>Details</summary>
Motivation: 组织部署代理式AI时，仅判断模型是否可信并不够，还需明确在具体用例中应验证、控制和观测什么，以实现预期结果并满足适用义务。

Method: 基于专业观察及对标准和文献的目的性综合，采用设计科学构建方法贡献，并用情境方法工程指导情境化裁剪与复用；先确立目标与义务，再评估七个风险领域，推导保证、运行时控制与证据要求，进而指导能力资格、差距评估和逻辑架构。

Result: 提出AI-GRACE框架，涵盖七个风险域（包括使命与价值实现）；通过Agent Operating Envelope规定允许行动与升级条件，通过Risk-Aligned Independence Levels（RAIL）概括授权独立性；以虚构零售银行应用进行示例说明。

Conclusion: 该框架为组织决定需要实施什么、已支持什么以及尚有哪些未解决问题提供了可追溯基础；其是否改善部署决策、效率和复用仍需实证评估。
...
TL;DR: 论文提出 CoVer，一种单策略 GRPO 框架，联合训练代码生成模型与测试生成器，以解决自博弈中的测试过松和样本集中偏差问题，在多个代码基准上提升生成与选择性能。


<details>
  <summary>Details</summary>
Motivation: 现有自博弈方法让同一语言模型同时充当代码编写者和测试作者，有望摆脱固定测试集，但存在两个耦合问题：一是宽松性崩溃，即通过奖励简单、无区分度的测试来最大化通过率；二是集中偏差，即独立同分布采样测试集中于常见输入，导致估计方差增大。

Method: CoVer 采用单策略 GRPO 框架。首先引入信息增益奖励，用自生成测试的通过/失败向量与分级、锚定真实正确性信号 y∈[0,1]^m 之间的互信息评分，并通过协方差符号门控，只奖励有正向区分度的测试。其次使用三阶段多样性感知选择：对候选池进行无效性、输入字符串、执行画像过滤，剪枝为行为非冗余测试套件，在固定执行预算下提高信息增益估计的有效样本量。

Result: 在 LiveBench、MBPP、LiveCodeBench、CodeContests、CodeForces 五个基准上，CoVer 相比 Qwen2.5-Instruct 主干将 one-shot pass@1 在 7B 上提升 5.8 个百分点，在 14B 上提升 7.1 个百分点；在两个规模下均取得所有对比方法中最高的宏平均。作为 CodeT 排序流程的可替换主干，CoVer-7B 额外提升 3.5 个百分点。

Conclusion: CoVer 通过信息增益奖励和多样性感知测试选择，缓解了自博弈中的宽松性崩溃与集中偏差，并证明联合训练对代码生成和测试选择具有双重收益。
...
TL;DR: 本文提出一种能力-残差解耦框架，用于情感认知诊断，旨在避免未建模的认知残差污染情感表征，从而提升响应预测与情感对齐效果。


<details>
  <summary>Details</summary>
Motivation: 传统认知诊断仅从掌握程度推断学生作答，但情感、投入、疲劳等非认知因素也会影响表现。现有情感认知诊断常假设认知诊断主干已解释能力、题目和概念效应，剩余误差主要来自情感；然而真实教育数据中可能存在题目校准偏差、系统性概念偏差、个性化学生-概念偏差及潜在学生-题目匹配等稳定认知残差，若不显式建模，这些残差会泄漏到情感表示中，造成情感污染。

Method: 提出能力-残差解耦框架：先通过学生、题目、概念、学生-概念及低秩学生-题目组件捕捉未建模的认知残差，再用情感模块调制猜测/失误效应；并设计受Q矩阵约束的概念残差注意力机制，仅自适应聚合与题目相关的概念残差。

Result: 在ASSIST2017、ASSIST2012、ASSIST2009和Junyi四个数据集上，结合六种认知诊断主干模型，响应预测在报告比较中取得提升；在情感标签可用时，情感对齐总体改善。消融、泄漏探测、主成分分析可视化、长尾分析和案例研究进一步表明，能力残差吸收了稳定认知偏差，减少了情感分支中的认知污染，并提升了认知诊断模型的鲁棒性与预测准确性。

Conclusion: 显式建模并解耦能力残差可有效防止认知偏差污染情感表示，增强情感认知诊断的预测性能、鲁棒性和可解释性。
...
TL;DR: 该论文提出一种全可微的神经-软符号框架，将视觉感知与任务规划统一到单一计算图中，在Blocksworld和PlanBench任务上取得优于基线的高成功率，并验证了规划结果与机器人运动执行的兼容性。


<details>
  <summary>Details</summary>
Motivation: 感知规划任务需要同时准确感知不确定场景并规划符合逻辑规则的动作序列。传统方法先将感知离散化为符号事实再规划，会丢失感知不确定性，并切断任务级反馈对感知的优化。

Method: 提出通用的全可微神经-软符号框架，连接视觉感知与任务规划。框架维护连续软符号状态，将领域规则提升为可微的软T_P转移算子，并在短规划视界上优化动作logits；规划目标的梯度还可更新感知参数，从而在规划过程中精炼任务相关的感知表示。

Result: 在Blocksworld上解决40/40个LatPlan-40任务和596/600个PlanBench-600任务，优于LatPlan的33/40和推理模型基线的587/600，且计算量和时间显著更少。感知不确定性消融中，成功率从冻结感知的59%提升至83%。任务与运动仿真进一步验证了解码任务规划与下游机器人运动执行的兼容性。

Conclusion: 该框架能有效统一感知与规划，利用任务级梯度改进感知，提高不确定场景下的规划成功率，并具备向机器人执行层面推广的潜力。
...
TL;DR: 论文提出 CogGym，一个基于认知科学的可扩展统一框架，用于在匹配实验试次上系统比较模型与人类行为；初步发布标准化了来自100篇论文的258个认知实验，并评估了50个大语言模型。


<details>
  <summary>Details</summary>
Motivation: AI与认知科学都试图理解并建模人类智能，因此需要回答模型响应在哪些方面类似人类、在哪些方面系统性偏离。但人类任务广泛多样，难以进行可扩展且严谨的人机比较。

Method: CogGym采用半自动、人在回路的流水线，将多样实验范式标准化为任务无关的Experiment Markup Language（EML），以支持大规模可复现比较。初步版本聚焦人类常识推理，整理并标准化100篇论文中的258个认知实验，并用50个大语言模型与人类反应进行对比评估。

Result: 研究发现清晰的缩放趋势：更大、更新的AI模型能更好复现人类判断。但模型在常识推理任务上的提升明显慢于数学、编程等形式推理基准；模型与人类拟合度仍远低于人类分半信度（文本R²=0.93、图像0.95、视频0.92），最佳模型仅达文本R²=0.59、图像0.58、视频0.43。

Conclusion: CogGym旨在成为一个持续更新的评估框架，不断纳入新的认知科学实验，以刻画模型行为与人类行为的相似之处、系统性差异，以及这些模式随模型和实验演进而发生的变化。
...
TL;DR: 该论文提出 PlaceReasoner-Beta，一个由验证器引导的多智能体框架，将宏单元布局重新表述为闭环推理问题，并发布开放基准 PlaceReasoner-Bench，用实际布线后的 PPA 与 DRC 评估，替代预布线代理指标。


<details>
  <summary>Details</summary>
Motivation: 传统自动宏布局主要优化手工设计的代理目标（如估算线长），采用一次性数值优化，难以统一利用视觉布局上下文、设计经验以及下游物理设计反馈。

Method: 框架包含：VLM 规划器基于布局图、宏规格和连接结构生成候选布局；几何验证器保证物理合法性与专家布局原则；物理验证器利用早期实现反馈细化候选；后布线优化器利用最终 PPA 进一步改进有希望布局。同时构建 PlaceReasoner-Bench 开放端到端基准，含 8 个设计、两种宽高比共 16 个任务，固定 floorplan 和 I/O，仅宏位置与朝向可变，并用布线后 PPA 和 DRC 评估。

Result: 在所有方形任务上，PlaceReasoner-Beta 在 DRC 干净方法中取得最佳时序；相对经典基线，后布线 TNS 在 1:1 下降低 61.2%，在 2:1 下降低 53.0%。尽管从未显式优化线长，它仍在多数设计上缩短了布线线长。

Conclusion: 在物理设计反馈下对空间结构进行推理，可以超越代理目标优化，改善端到端布局质量。
...
TL;DR: 该论文研究生产环境中LLM智能体的高效重复评测问题，基于一个服务数万月活用户的生产分析智能体的574次历史评测数据，比较多种抽样与自适应评测方法，并报告实际部署经验。


<details>
  <summary>Details</summary>
Motivation: 生产LLM智能体持续演进，需要反复评测，但完整基准评测成本高、难以频繁重跑，因此需要低成本且可靠的高效重复评测方案。

Method: 使用574次生产基准历史运行，按时间分为校准期和留出期；比较随机抽样、历史缓存、固定代表性子集和基于IRT的自适应测试；重点评估多维2PL自适应测试，并实际部署难度分层固定子集。

Result: 多维2PL自适应测试总体分数保真度最佳：仅执行200题（完整运行的38.5%）即可达到1.03个百分点的MAE。但出于运维简便，实际部署了难度分层固定子集；该子集无需重新校准即可迁移到其他五个智能体家族，且在短至一天的校准窗口内保持稳定。

Conclusion: 生产智能体重复评测可采用高效子集或自适应方法；固定难度分层子集在简洁性、可迁移性和稳定性方面具有实用优势，并为生产环境中的重复评测提供了实践建议。
...
TL;DR: 论文提出 GameASG-Bench，将行为可测试性纳入自主软件生成，用 47 个浏览器原生游戏生成任务评估代理能否生成满足行为需求的交互应用。


<details>
  <summary>Details</summary>
Motivation: 自主软件生成可将需求转为可执行应用，但应用可运行不代表其交互组件满足指定行为需求，因此需要把行为测试性纳入生成任务。

Method: 设计生成前声明的评估接口规范，固定合法起始场景、玩家动作、稳定快照、拒绝行为和不变量，同时保留私有实现开放；包含静态 L1 源码合规检查和浏览器执行的 L2 检查，后者结合语义观察、真实输入与运行时证据。实现 47 个任务，覆盖 12 类主要游戏类型及 2D/3D 交互，每任务含可执行检查和独立验证参考实现，并实验考察代理性能、工具访问与回合预算、推理努力和 harness 选择。

Result: 在九种代理栈中，最高平均 L2 检查通过率为 93.2%，但最高严格任务成功率仅 55.3%（26/47）。对 DeepSeek-V4-Flash，完整工具访问和更大名义回合预算带来更多严格成功，但严格成功率不随推理努力单调变化。两种 harness 均达到 18 个严格成功，但仅 10 个任务在两者下都成功。

Conclusion: 高平均检查通过率会掩盖任务级合规差距；GameASG-Bench 揭示了当前代理在端到端行为需求满足上的不足。
...
TL;DR: 本文提出LEGIT凭证协议，用于AI代理市场中连接认证、声誉与市场分配，帮助买方验证和比较代理性能，并分析声誉操纵的经济成本。


<details>
  <summary>Details</summary>
Motivation: 在代理市场中，买方难以判断哪个AI代理最适合自己的任务；现有基准分数难以验证，且跨任务、软件和预算的可比性不足。

Method: LEGIT通过签名记录将测得的质量和每任务解决成本绑定到代理配置、任务领域、评估预算和证据；声誉将历史任务结果链接到同一身份，并考虑反馈可靠性；买方可验证凭证并查看可选视觉画像。另在Sybil攻击模型下量化声誉操纵所需的押金和费用。

Result: 评估显示，任务成功率相近的代理配置之间存在成本差异，且比较结果依赖于评估预算；补充分析给出了声誉操纵所需押金和费用的量化结果。

Conclusion: 应将性能测量绑定到被测试配置和资源限制；LEGIT这类凭证协议有助于代理市场的验证与比较，并通过经济机制提高声誉操纵成本。
...
TL;DR: 本文研究在隔离且受限的空天作战环境中使用离线大语言模型作为决策支持工具，提出模块化检索增强架构，支持文本与图像输入，并通过巴西空军4名图像分析员的试点评估验证其潜力。


<details>
  <summary>Details</summary>
Motivation: 空天作战依赖复杂规则、既定程序和时间紧迫的分析，且常面临连接受限与严格安全约束。分析员需结合书面条令与图像，但往往无法访问外部计算资源，因此需要可离线运行且知识来源可追溯的决策支持工具。

Method: 提出适用于无互联网环境的模块化检索增强生成架构，支持技术手册的文本和图像输入，并通过自然语言交互提供可追溯的条令知识。开展试点研究：对4名巴西空军图像分析员进行电子目标识别条令知识评估，比较人类与系统表现；测量无AI辅助下人工制作侦察目标报告（REMIR）的认知负荷；并提出未来评估协议。

Result: 人工制作REMIR任务要求较高，心理需求为6.0/7，努力为5.0/7。所提系统在条令知识评估中得分8/10，与人类得分相当，完成时间为7.1分钟，而人类平均为26.5分钟。

Conclusion: 离线检索增强架构在受限环境中具有作为决策支持工具的潜力，并建立了未来AI辅助评估的基线。后续将通过系统化协议进一步比较人工与AI辅助工作流。
...
TL;DR: 论文提出DENSE方法，将智能体执行轨迹蒸馏为可复用反馈，并设计REFIT协议进行无事后结果标签的公平评估，在Terminal-Bench 2.1上显著提升严格通过率并减少重跑token消耗。


<details>
  <summary>Details</summary>
Motivation: 在线智能体部署产生大量执行轨迹，但任务特定验证与专家标注难以规模化，因此需要在不依赖事后结果标签的情况下，从轨迹中提取可复用反馈。

Method: DENSE将轨迹证据组织为基于证据的嵌套捷径树，压缩冗余尝试、利用恢复证据协调跨层级问题、总结已完成分支并展开未解决分支，从而关联可复用进展与剩余任务。同时提出REFIT协议，在事后结果盲设定下比较共享初始轨迹的反馈，并重置环境与模型上下文以进行新尝试。

Result: 在Terminal-Bench 2.1上，DENSE在四种接收模型上取得非特权反馈方法中最高的严格通过率；相比初始执行，严格通过率提升7.12-15.64个百分点，重跑时观察到的接收方token减少19.0-43.6%。GPT-5.5消融实验支持将嵌套子任务分析与捷径构建及问题协调相结合。

Conclusion: 研究指向通过基于证据的轨迹复用实现智能体自我改进，并减少对外部监督的依赖。
...
TL;DR: 本文提出Group Variance Policy Optimization (GVPO)，一种新的LLM后训练方法，通过将KL约束奖励最大化的解析解融入梯度加权，避免了重要性采样，提升了训练稳定性，并可自然扩展到在线策略蒸馏。
...
Method: GVPO将KL约束奖励最大化的解析解整合到梯度加权方案中，其梯度等价于隐式奖励与实际奖励中心距离的均方误差。该方法无需重要性采样，允许灵活采样分布，并自然扩展到在线策略蒸馏（OPD）及多种扩展OPD目标优化。

Result: GVPO保证唯一最优解，精确对应KL约束奖励最大化目标；无需重要性采样，支持灵活采样分布；可扩展到OPD及多种扩展OPD目标。

Conclusion: GVPO为可靠且通用的LLM后训练和在线策略蒸馏建立了新范式。
...
TL;DR: 本文提出风险感知占据（risk-aware occupancy）这一稠密表示，用于端到端自动驾驶规划，并设计ROIDrive网络及RiskOcc4D-nuScenes数据集，显著降低开环碰撞率。


<details>
  <summary>Details</summary>
Motivation: 稀疏表示将环境感知建模为对象、车道线等离散元素，在拥挤、遮挡、非结构化障碍物、不确定区域和复杂交互场景中存在安全风险，因此需要更显式、统一地刻画与规划相关的风险。

Method: 提出风险感知占据表示，将全局场景占据、地图交通约束和未来动态智能体占据联合编码到统一BEV地图中，在空间和时间维度捕捉轨迹规划的风险证据；设计E2E网络ROIDrive，通过独立分支预测风险感知占据，并将其注入规划查询以生成安全导向轨迹；构建基于nuScenes和Occ3D-nuScenes的RiskOcc4D-nuScenes用于量化安全问题。

Result: 在nuScenes上，风险感知占据在UniAD指标下实现52.9%的相对开环碰撞减少，在ST-P3指标下实现35.0%的相对减少。

Conclusion: 风险感知占据能够以稠密、统一的方式表达规划相关风险，并有效提升端到端自动驾驶的安全性。
...
TL;DR: 论文提出RRDrive，通过引入风险感知占用（risk-aware occupancy）作为稠密、时序对齐且可被轨迹查询的场景表示，提升端到端自动驾驶中的多模态轨迹预测与候选轨迹排序性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态轨迹预测受限于稀疏场景表示，证据不完整导致候选轨迹生成质量低，且几何相似轨迹之间排序不可靠。基线中差/较差候选占比19.74%，oracle最优候选平均仅排第33.9位。

Method: 提出RRDrive，使用风险感知占用作为稠密、时序对齐、轨迹可查询的表示：全局结构引导高质量多模态生成，候选条件风险查询支持细粒度选择；构建带自动风险标注的RiskOcc4D-NAVSIM；进一步开发从传感器输入直接估计风险感知占用的外部RiskOcc预测器。

Result: RRDrive取得所选轨迹PDMS 0.951，较基线0.937相对提升1.5%；平均候选PDMS提升7.7%。在挑战场景中，候选PDMS提升30.2%，好候选间的Spearman相关系数从0.26提升至0.67，提升0.41。外部RiskOcc预测器也表现出有竞争力的性能。

Conclusion: 风险感知占用表示能有效改善多模态轨迹生成与排序，并在挑战场景中显著提升候选质量与相关性；外部RiskOcc预测器的竞争性表现验证了该表示从传感器输入估计的可行性，使其超越oracle设定。
...
TL;DR: 该论文提出LogicTrack，一个神经符号框架，用于审计大语言模型思维链推理的中间步骤逻辑有效性，通过符号化验证、回溯搜索和微调提升推理链可验证性与最终答案准确率。


<details>
  <summary>Details</summary>
Motivation: 现有思维链优化多依赖结果反馈，中间推理步骤的逻辑正确性未被充分验证，导致模型可能通过逻辑有缺陷的推理链得到正确答案。

Method: LogicTrack将每个推理步骤自动形式化为符号表示，并用自动定理证明器验证；提出基于求解器的回溯奖励SBR，逐步量化逻辑合理性并指导推理时回溯树搜索；进一步利用回溯轨迹构建监督微调数据，使模型内化逐步审计能力。

Result: 在8个推理基准和7个大语言模型上实验表明，LogicTrack有效提升推理链可验证性和最终答案通过率，增强思维链整体质量与可信度。

Conclusion: LogicTrack有助于在高风险领域提升大语言模型思维链推理的可信性与可靠性，弥合结果正确但过程逻辑缺陷的问题。
...
TL;DR: 提出 PolyBridgeBench，一个可执行的多模态桥梁设计基准，用于评估 MLLM 生成完整承重结构、在物理仿真中执行并在失败后修复的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准多评估空间推理、结构有效性或物理构建，但无法判断 MLLM 能否综合完整承重结构，并在仿真执行暴露失败后进行修复。

Method: 模型接收视觉场景和结构化工程约束，生成节点-构件-材料拓扑；通过确定性合法性检查后进入原生动态物理仿真；失败后返回失败 rollout 的时序视觉证据，并在固定交互预算下评估修复。分别测量确定性有效性、动态功能成功和失败后恢复。

Result: 在 189 个关卡上测试 6 个代表性 MLLM，发现确定性有效性与动态成功之间存在显著差距，对材料预算高度敏感，且在主要严格预算设置下失败后恢复能力有限。

Conclusion: PolyBridgeBench 能定位设计失败阶段；当前 MLLM 在可执行桥梁设计，尤其是动态成功与失败修复方面仍存在明显不足。
...
TL;DR: 论文提出往返协议，评估语言模型将树结构算术表达式序列化为自然语言后能否被另一模型恢复，发现该通道有损、不对称、生成端为主要瓶颈，且可通过微调改善。


<details>
  <summary>Details</summary>
Motivation: 语言模型在链式思考或交换自由文本中间结果时，会把结构化信息序列化为自然语言。这种瓶颈会损失多少树结构组合内容尚不清楚，因此需要实证研究。

Method: 提出往返协议：生成器把程序生成的算术表达式转成应用题，独立提取器仅从应用题恢复表达式，符号等价提供精确评判。对16个模型的所有配对进行评估，得到通信矩阵；还使用约3600条微调样本，以及含新算子和词汇的分布外设置进行测试。

Result: 通道有损且不对称：生成与提取角色互换会使准确率最多变化60.4个百分点；最佳异模型配对达到92.9%。至少73.6%的往返失败源于生成端；难度主要由树结构（算子数、深度、右分支）驱动，而非模型家族。微调可使所有开源权重模型超过未训练Gemini-3.1-Pro；在新算子和词汇的分布外设置下仍有提升，但与前沿模型存在差距。

Conclusion: 树结构表达式序列化是模型通过自然语言交流层级结构时的主要限制因素，且该瓶颈可通过针对性训练部分缓解。
...
TL;DR: 论文提出将Learning-to-Optimize (L2O)作为AI原生通信网络的缺失架构层，通过四阶段工作流将优化算法转化为离线知识生成器，训练神经代理模型实现实时推理。以NR-V2X中继选择为例，用MILP解训练GNN实现近最优实时决策。


<details>
  <summary>Details</summary>
Motivation: 当前AI原生网络架构虽嵌入智能，但缺乏对优化知识如何系统生成、迁移和利用的指导；优化仅被视为在线决策引擎，未作为知识源发挥价值。

Method: 提出L2O架构抽象，定义四阶段工作流：优化、知识生成、代理学习、运行时推理；将优化算法作为离线知识生成器，为神经代理模型提供监督信息；以NR-V2X中继选择为例，用MILP求解器生成解训练GNN。

Result: 在NR-V2X中继选择问题上，GNN能实时复现近最优决策；L2O被定位为未来AI原生网络的关键架构使能者。

Conclusion: L2O是连接优化与AI原生智能的缺失架构层，可作为异构通信与计算系统的通用架构抽象。
...
TL;DR: 提出DSRec，一种双兴趣跨SSM序列推荐模型，通过解耦长短期语义上下文中的物品角色，结合Mamba与时序调制SSM及残差交叉融合，在公开基准上优于现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: Transformer序列推荐虽有效但成本较高；现有SSM方法虽支持高效长程建模，却常将每个物品编码为单一静态上下文角色，忽略物品多义性，且难以捕捉不同时间粒度下的动态行为。

Method: 设计长短期双分支：长期兴趣嵌入通过历史聚合捕捉稳定偏好，短期兴趣分支由点击时间间隔调制以强调局部会话意图；分别使用全序列Mamba和时序调制SSM编码，并采用残差交叉融合机制交换上下文信息，同时保持两分支语义独立。

Result: 在公开基准数据集上，DSRec的实验表现优于其他最先进方法。

Conclusion: 显式解耦长短期物品语义角色并进行跨粒度对齐融合，能有效提升序列推荐性能，验证了双兴趣跨SSM与时序调制建模的有效性。
...
TL;DR: 本文提出一种质心引导对比损失（CGCL），用于欺诈性招聘广告检测，通过在潜在空间中结合分类与密集聚类来提升准确率并学习有意义的表示结构。


<details>
  <summary>Details</summary>
Motivation: 现有欺诈招聘广告检测方法难以同时实现高检测准确率和具有意义的潜在空间表示结构，难以刻画虚假招聘广告之间的细微差异。

Method: 提出质心引导对比损失（CGCL），将分类与密集聚类统一起来，通过基于质心的 top-k 推拉机制持续重塑潜在空间，兼顾类别可分性与聚类紧致性。

Result: 在公开基准数据集 EMSCAD 上进行大量实验，结果表明该方法取得了最先进（SOTA）性能。

Conclusion: CGCL 能有效提升欺诈招聘广告检测性能，并在潜在空间中同时保持准确决策边界和高聚类紧致度，代码已开源。
...
TL;DR: 该研究开发并评估课程专属RAG系统Beacon，提供私密、即时、与模块对齐的学术支持，以降低求助障碍并促进自主学习。


<details>
  <summary>Details</summary>
Motivation: 学生获取学术支持存在不平等：部分学生因焦虑、怕被评判、不确定期望或信心不足而不求助。计算教育中编程任务累积且认知负荷高，通用生成式AI又可能不准确、缺乏情境或不符合课程期望。

Method: 采用设计型研究，迭代开发Beacon，基于RAG并以批准教学材料为依据；使用混合方法，结合问卷与半结构化访谈，收集高校学生和教职工反馈。

Result: 学生认为Beacon回答更贴合模块内容、比无限制生成式AI更可信，重视伪代码和脚手架式解释而非直接答案；但仍对AI答案持谨慎态度，需验证，并将其视为咨询讲师或官方资源前的第一支持点。

Conclusion: 精心设计的课程专属AI可成为独立学习与正式支持之间的中介，减少学术支持障碍；它不应取代教育者，而应在保留讲师教学作用的同时扩大指导可及性。
...
TL;DR: 论文提出Cal-OPD，通过校准教师自身偏差来改进在线策略蒸馏，在数学推理任务上以更少的优化信号超越标准OPD。


<details>
  <summary>Details</summary>
Motivation: 在线策略蒸馏（OPD）中，教师-学生差异不仅反映能力差距，还混杂了教师自身的偏差，特权OPD会进一步放大该问题，导致学生盲目学习教师的偏差。

Method: 通过正负特权干预估计教师的自偏差区域，并据此校准原始教师-学生差异，仅保留超出该区域的部分作为优化信号。

Result: 在数学推理基准上，仅保留约52-65%的原始差异作为优化信号，Cal-OPD在不同模型规模上均持续优于标准OPD及其变体。

Conclusion: 校准教师自偏差能有效提升蒸馏效果，验证了去除教师自身偏差对在线策略蒸馏的重要性。
...
### [34] [Accelerating Dense LLMs via L0-regularized Mixture-of-Experts](https://arxiv.org/abs/2609.21672)
*Zhenyu Zhang,Jiudong Yang,Zhaowen Tao,Meng Chen*

Main category: cs.AI

TL;DR: 该论文提出 L0-MoE，一种基于 L0 正则化的轻量级混合专家方法，用于在几乎不损失性能的情况下加速稠密大语言模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型性能强但推理慢、成本高；现有加速方法常导致明显性能下降，而传统 MoE 需要大量计算资源。

Method: 提出 L0-MoE 轻量级 MoE 方法，使用 L0 正则化加速稠密 LLM；引入聚类混淆矩阵进行领域感知的数据集构建，并采用动态批处理提升训练效率。

Result: 实验表明，L0-MoE 相比稠密模型可实现最高 2.5 倍加速，同时保持有竞争力的性能，并优于现有 LLM 加速基线方法。

Conclusion: L0-MoE 能够以轻量级 MoE 方式有效加速稠密大语言模型，在保持性能的同时显著提升效率，优于现有加速方案。
...
TL;DR: 论文提出 Self-Meta-Evolve 分层框架，用于企业信息抽取中的按用户提示适配，通过双循环为每个用户维护并持续优化提示，并发布基于人物角色的企业IE基准。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多用于企业信息抽取，但同一文档常需针对不同用户以不同方式重组；现有提示优化方法依赖面向全局目标的单一提示，忽视了真实工作场景中的用户异质性。

Method: 将企业IE建模为交互反馈下的按用户提示适配。Self-Meta-Evolve包含双重循环：内循环根据人物角色条件反馈编辑结构化提示，外循环通过蒸馏成功编辑模式来进化元提示。同时发布包含292个模拟企业用户的persona驱动IE基准及基于O*NET职业分类的可复现人物角色生成流程。

Result: 在该基准上，Self-Meta-Evolve达到74.58%的成功率，比最强提示优化基线高13.56个百分点，并且仅两轮迭代就达到52.54%。二十位真实专业人员的双盲人类研究进一步表明，该框架适配的提示在71%的成对比较中优于静态基线。

Conclusion: 针对用户异质性的层次化、按用户、基于交互反馈的提示优化能显著提升企业信息抽取效果，并优于单一全局提示优化方法。
...
TL;DR: 该论文提出 GUARD 方法，通过将不安全披露转化为安全退出轨迹，并利用引导 token 对齐与蒸馏，使大型推理模型在遗忘后产生自然的拒答式推理轨迹，同时提出 NFRS 评估替代内容质量。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型的思维链可能泄露受保护事实或不安全推理，现有遗忘目标只抑制目标内容或重定向内部表示，却未规定遗忘后的轨迹应如何继续，易导致幻觉替代、边界错误或重复输出。

Method: 提出 Guided Answer-Reasoning Distillation（GUARD）：将模型生成的不安全披露转化为安全退出轨迹，用引导 token 对齐冻结的 LRM，并将引导行为蒸馏到模型参数中；同时提出 Natural Forgetting Reasoning Score（NFRS），从结构稳定性、流畅性和无依据替代内容方面评估遗忘输出。

Result: 在 R-TOFU 和基于 STAR-1 的有害意图设定上的实验表明，GUARD 在两种常用蒸馏 LRM 上显著减少不安全与隐私披露，同时保持推理效用。

Conclusion: LRM 遗忘应学习自然的遗忘轨迹，即连贯且不披露的 CoT 后接稳定的拒答式答案；GUARD 能有效实现这一目标，NFRS 可补充评估遗忘替代质量。
...
TL;DR: 提出 ReACT-TTS 两阶段框架，利用听者一秒预反应面部序列来规划对话回复的情感和韵律，再通过 Grad-TTS 实现语音合成。


<details>
  <summary>Details</summary>
Motivation: 对话语音依赖对话上下文和听者紧接之前的行为，已有方法可能忽略听者预反应动态这一线索。

Method: 在严格二元 MELD 协议下，使用一秒听者预反应面部序列进行时间条件建模；进行多种视觉消融，包括显式早晚期差异和循环错配；并连接 Grad-TTS 骨干实现端到端语音生成。

Result: 十个随机种子下，时间条件相比纯文本获得更高平均宏 F1 和 VAD 一致性，准确率基本不变；视觉变体中时间建模最佳，显式早晚期差异非必需，正确听者反应平均优于循环错配。20 位语音研究者情境适当性研究中，76% 偏好时间条件，9% 偏好纯文本，15% 无偏好。

Conclusion: 预反应听者动态可作为对话回复规划的补充线索，支持将其用于情感与韵律规划及后续语音实现。
...
TL;DR: 论文以TaxiGPT为例，表明行为失败并不代表模型没有世界模型；通过机制分析和因果干预，发现模型确实表征了环境并用于导航，失败主要源于叠加交叉口特征之间的干扰。


<details>
  <summary>Details</summary>
Motivation: 此前研究常根据模型行为失败推断其缺乏连贯的内部世界模型，作者希望检验这种推断是否成立，并推动从“是否有世界模型”转向机制层面研究“如何建模世界”。

Method: 训练Transformer模型TaxiGPT在曼哈顿随机游走上导航；使用机制分析和因果干预，识别模型对交叉口、街道、位置和目标的表征，并分析失败来源；提出affordance packing和机制指标进行模型比较。

Result: 模型能表示交叉口和街道、追踪自身位置并利用目标罗盘导航；失败源于叠加的交叉口特征干扰内部地图定位；affordance packing可限制这些错误后果；世界建模能力在训练不同阶段出现。

Conclusion: 行为失败不等同于缺乏世界模型。应转向机制层面研究世界建模，即模型如何表征环境并利用这些表征指导行为，以及这些能力如何相互作用和逐步涌现。
...
TL;DR: 该研究提出急诊多模态预测中的“ECG Mirage”失败模式，即视觉语言模型看似能利用心电图，但实际并未有效依赖患者本人的ECG信息；作者通过匹配、错配和无图像ECG输入比较来评估该问题，并提出视觉提示调优作为缓解策略。


<details>
  <summary>Details</summary>
Motivation: 急诊决策依赖病史、生命体征、化验和心电图等多源异构信息。视觉语言模型虽可联合处理这些模态，但高预测性能不一定代表模型真正使用了正确的患者ECG，因此需要识别并解决这种多模态能力假象。

Method: 在MDS-ED数据上，对四个视觉语言模型比较匹配ECG、结果不一致的错配ECG和无图像输入下的预测表现，同时固定临床文本与预测目标。随后保持VLM主干冻结，训练四个受限视觉提示，采用监督学习后再进行条件直接偏好优化。

Result: 在四个VLM中，匹配ECG对ICU入住或临床恶化预测均未带来一致优势，表明存在ECG忽视或ECG混淆。经视觉提示调优后，模型在ICU入住预测上达到70.6%平衡准确率，在恶化预测上达到67.5%，并将匹配与错配之间的性能差距分别提高到约16.5和5.5个百分点。

Conclusion: 研究识别了多模态临床预测中的ECG Mirage现象，并证明视觉提示调优是一种在冻结VLM主干条件下高效缓解该问题的策略。
...
TL;DR: 该研究考察在严格选择协议下，简单统计流程在单变量时间序列异常检测中的表现，并通过消融定位性能瓶颈，最终利用多模态LLM生成域相关候选统计池，将性能提升至榜单最佳水平。


<details>
  <summary>Details</summary>
Motivation: 探究简单统计方法能否媲美神经网络或预训练基础模型，并明确候选统计池、特征选择策略和聚合方式各自对性能的影响程度。

Method: 在滑动窗口上提取少量统计量，用转导鲁棒MAD模型对每个窗口打分，并在留出调参集上按域选择特征子集；通过消融比较不同选择策略、聚合网格和候选池；再用多模态LLM结合域内示例窗口生成候选统计池，并与手工池取并集进行选择。

Result: 在TSB-AD-U上达到每序列VUS-PR 0.529，高于公开榜单最佳神经方法0.45和统计方法0.44，距最强预训练基础模型仅差0.06；消融显示选择策略仅使分数变动0.031，聚合网格变动0.096，而候选池变动0.226；LLM生成池与手工池在匹配选择下相当且覆盖不同域，二者并集在全部12个生成器种子对上均提升，最终达到0.588，匹配榜单最佳条目。

Conclusion: 候选池决定性能上限，池上的搜索只是二阶因素；LLM可生成有效的域相关候选池，并与手工池互补，使简单统计流程通过并集选择达到领先水平。
...
TL;DR: 该论文提出MIST，一种基因组引导的组织学注意力多模态生存预测方法，通过让基因组特征作为查询来关注紧凑的病理基础模型上下文令牌，从而提升外部队列上的生存预测性能。


<details>
  <summary>Details</summary>
Motivation: 多模态生存模型可融合全切片图像与基因组信息，但有效融合仍受外部队列偏移和计算复杂度挑战。

Method: MIST将基因组特征表示为令牌，并在生存预测前用其查询由基础模型得到的紧凑组织学上下文令牌；训练结合离散时间生存预测、基因组特征掩码、WSI丢弃以及配对的WSI-基因组对比对齐。

Result: 在结肠癌、肾癌、肺癌和胶质母细胞瘤四个外部评估中，MIST在主要比较中相对标准融合基线提升了外部C-index。
...
### [42] [AutoRecLab: Describe the Experiment, Get the Code!](https://arxiv.org/abs/2609.21863)
*Moritz Baumgart,Philipp Meister,Justus Krell,Michael Schmidt,Bela Gipp,Joeran Beel*

Main category: cs.AI

TL;DR: AutoRecLab 是一个基于 Python 的自主推荐系统实验平台，可根据自然语言提示自动完成推荐系统实验的设计、实现与迭代扩展。


<details>
  <summary>Details</summary>
Motivation: 推荐系统研究高度依赖实证评估，但将实验设计转化为可执行代码通常需要人工完成，过程繁琐且容易出错。

Method: AutoRecLab 首先从研究想法中提取明确的实验需求，构建并验证原型，再迭代扩展为完整实验；其工作流结合了检索增强生成（RAG）用于文档查询、静态类型验证以及执行引导的树搜索。

Result: 在演示中，AutoRecLab 自主实现了显式反馈到隐式反馈转换研究；在六种算法和三个数据集上的基线比较中，9 次运行有 8 次成功，使用 GPT-5.4-mini 时平均每次运行成本约 1 美元。

Conclusion: AutoRecLab 能够自动化推荐系统实验流程，降低人工实现实验的门槛和错误率，展现出自主实验平台的可行性。
...
### [43] [EnterpriseVal: Quantifying the Efficacy, Reliability and Value of Generative AI in the Enterprise](https://arxiv.org/abs/2609.21841)
*Abbas Raza Ali,Muhammad Ajmal Siddiqui,Moona Zahid*

Main category: cs.AI

TL;DR: 提出 EnterpriseVal，一个面向企业用例的生成式AI评估系统，旨在弥合公共基准与部署决策之间的差距。该系统包括用例与冻结社会技术配置规范、指标目录、结合专家判断与校准LLM评分的分级协议、两级阈值决策门以及价值-风险模型。某全球银行三个工作流试点显示：信贷备忘录撰写中最佳模型引用精确率88%、幻觉率1.6%；流程改造中分析师精修工作量从估计27.4小时/文档降至2.9小时/文档。


<details>
  <summary>Details</summary>
Motivation: 前沿模型已能产出专家认为可匹配人类工作的专业交付物，但多数企业生成式AI项目未能显示可衡量业务效果，且大量代理项目预计将被取消。作者认为这主要是测量问题：公共基准回答“模型能做什么”，而部署决策需要回答“该工作流在本地数据和控制下是否适用、可靠、安全且值得扩展”。

Method: EnterpriseVal 包含五部分：(i) 用例和冻结社会技术配置的形式化规范，涵盖模型、提示、检索、工具、护栏和人工监督，并以自主水平和后果等级共同设定评估强度；(ii) 覆盖保真度、效用、效率、可靠性、保证和监督的指标目录；(iii) 通过预测驱动推断，以校准的LLM-as-judge扩展盲审专家判断的分级协议；(iv) 以可执行算法表述的两级阈值门，将带置信界的指标向量映射为REJECT/CONDITIONAL/SCALE决策；(v) 将审查者捕获率作为可测量参数的价值-风险模型。

Result: 在某全球银行三个工作流中开展试点。信贷备忘录撰写：最佳模型的人类评分引用精确率达88%，幻觉率1.6%，对应门槛为70%和5%。流程改造：分析师精修工作量从估计的每文档27.4小时降至2.9小时。论文区分了已确立结果、记录在案的试点证据、所提系统和开放假设，并指定了全面验证所需实验。

Conclusion: EnterpriseVal 通过面向用例、指标化、分级校准和可执行决策门的方法，试图解决企业GenAI部署中的测量缺口。试点结果初步支持其可用性，但完整验证仍需按论文指定的实验进行，当前证据应谨慎区分为试点证据与待验证假设。
...
TL;DR: 本文研究部分证据下的交互式检索，提出检索感知的在线强化学习框架RAVEL，用于交互式行人重识别，通过完整问答-检索循环的排序反馈优化提问策略。


<details>
  <summary>Details</summary>
Motivation: 现有系统多通过模仿离线候选QA对排序来训练提问决策，但问题的价值实际取决于其引发的回答以及对后续检索的下游影响，离线模仿监督不足。

Method: RAVEL从监督式问题生成初始化，直接观察当前Top-4候选，并利用完整问答-检索循环产生的排序反馈，以在线强化学习优化问题策略。

Result: 在Interactive-PEDES上，RAVEL在五轮交互中逐步提升检索性能；分析表明其将提问预算重新分配到局部化开放式属性，提供更有用的检索证据，并在初始困难查询上取得最大增益。

Conclusion: 候选区分度和感知有用性只是弱监督，检索感知的在线强化学习能更有效决定提问，从而持续改善交互式检索，尤其有利于困难查询。
...
### [45] [Learning Cardiac Features: ECG Biometrics Across Time and~Exercise](https://arxiv.org/abs/2609.21962)
*Luca Thiebaud,Paul Chauchat,Mustapha Ouladsine,Stéphane Delliaux*

Main category: cs.AI

TL;DR: 该研究评估了在运动诱发生理压力和跨会话变化条件下，基于心电图的生物特征识别方法的鲁棒性，并在多个数据集上取得了领先或较低的错误率。


<details>
  <summary>Details</summary>
Motivation: 现有心电图生物特征识别研究多局限于单次会话、静息状态数据，尚未充分验证其在时间变化和生理变化下的鲁棒性；同时该范式在保护敏感心脏数据和自监督学习预训练任务方面具有潜力。

Method: 使用来自心肺运动试验的大型心电图数据集训练一个采用后期多导联融合策略的孪生ResNet，并设计结合运动与时间因素的评估协议，同时在公开基准数据集上进行评测。

Result: 在生理与时间变化联合条件下进行了首次广泛评估，取得会话内静息到峰值状态的EER为1.7%，在CYBHi数据集上取得当前最优的3.9%。

Conclusion: 研究结果支持存在一种对生理和时间漂移具有鲁棒性的内在心脏特征签名，表明心电图生物特征识别在现实变化条件下具有可行性。
...
TL;DR: 提出AutoViewMem，一种数据驱动框架，将长期对话记忆组织为自配置、低重叠的语义视图，在写入时进行结构化提取，从而提升LLM代理的长程问答与个性化能力。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统依赖固定粒度或静态模式，难以处理混合表示中的异构信息（偏好、事件、约束、时间更新），导致语义干扰，使top-K检索对噪声敏感且相关证据排名靠后。

Method: AutoViewMem从交互轨迹中发现候选视图，选择紧凑互补视图集，指导写入时结构化提取基于来源的记忆；将语义解耦从检索时移到写入时，支持标准top-K相似检索；并应用离线整合提升记忆紧凑性与一致性。

Result: 在LoCoMo和PersonaMem基准上，使用Qwen3-8B和Qwen3-14B主干，AutoViewMem在长程问答和个性化上优于强记忆基线，同时保持简单推理流程。

Conclusion: 表示优先的设计能有效提升长程问答与个性化，且无需复杂检索路由或迭代检索。
...
TL;DR: 论文提出 Probe of Internal Recognition（PIR），借鉴犯罪心理学的隐藏信息测试，从模型内部状态判断其是否识别出正确答案，从而区分模型“不愿回答”和“不能回答”。


<details>
  <summary>Details</summary>
Motivation: 大模型可能隐藏已知知识，例如在能力评估中故意放水或给出与内部知识相悖的答案；仅凭输出无法判断模型是隐藏答案还是根本不知道。

Method: PIR 将问题与候选答案一并呈现，读取模型内部表征，判断哪个候选被模型识别为正确。该方法无需参考模型、无需标注真值语料，可跨模型使用。

Result: 在 Gemma、Qwen、Llama、Mistral、Phi 五个家族八个模型上，PIR 以 0.70–0.87 的平衡准确率恢复被识别答案，远高于未知项基线 0.28–0.40 和随机水平 0.25。在提示欺骗、训练沙袋、密码锁定和电路破坏等隐藏形式下，识别率仍为 0.85–0.93；知识被遗忘删除后则降至未知问题水平。

Conclusion: PIR 能区分模型“不愿回答”与“不能回答”，支持沙袋行为审计和遗忘验证。该信号具有因果性，提供黑盒行为线索之外的信息，并可从多选题扩展到自由生成。
...
TL;DR: 提出 CodeMidas，一种仅以源代码为输入的智能体流水线，将已有代码库中已实现的功能转化为可执行的强化学习环境，构建了包含 5,545 个任务、覆盖 3,185 个代码库的数据集，训练后显著提升多种编码基准表现。


<details>
  <summary>Details</summary>
Motivation: 通过强化学习训练编码智能体需要大量带可靠验证器的多样化任务；现有方法多依赖 issue、commit 等开发工件，可提取任务范围受限，难以规模化构建 RL 环境。

Method: CodeMidas 以源代码为唯一任务特定输入，在各阶段分配智能体算力：探索已实现功能以生成行为规格，基于原始代码执行构建测试，并通过执行检查与多次解答 rollout 验证与筛选候选任务；随后用 GRPO 在生成任务上训练 MiMo-V2.5。

Result: 数据集含来自 3,185 个开源代码库的 5,545 个训练任务，覆盖 23 种编程语言和 15 个技术领域；训练后在五个基准上均提升：DeepSWE +11.7%、ProgramBench +17%、Terminal-Bench v2.1 +8.5%；消融显示高质量任务数量增加带来性能提升；轨迹分析表明智能体代码库探索更多、自验证更多样。

Conclusion: 源代码可作为构建 RL 环境的可扩展基础，所生成环境能提升编码智能体在多种软件任务上的表现。
...
TL;DR: 该论文提出一种面向专业平面设计的持续自适应框架：冻结的前沿模型通过230多个工具操作设计软件，外部程序性记忆以自然语言技能形式积累并优化可复用设计流程，从而在无权重更新、无人工标签条件下提升设计执行成功率与生成质量。


<details>
  <summary>Details</summary>
Motivation: 专业平面设计是长周期智能体任务，结构化可编辑成果由大量相互依赖动作产生，但结果缺乏可靠程序化评判标准，因此需要一种能在噪声、不可验证反馈下持续自适应的机制。

Method: 框架让冻结模型操作专业设计软件，并维护外部自然语言程序性记忆；记忆通过“拓宽”获取未覆盖重复子任务的新流程，通过“加深”基于自身成功与失败执行修订已有流程；匹配回放门控仅允许修复失败且不导致已观测成功回退的变更。

Result: 在1,406个真实用户简报和1,869条自动评分轨迹上进行五轮迭代，无权重更新、无人工标签，技能库从76个文档衍生技能增至139个；Claude-Sonnet-4上GenEval2执行成功率从72.7%升至99.3%，生成质量提升11.99分；在四个专业设计基准上对无技能智能体的胜率为61.8%和67.6%。在200个用户流量基准留出简报上，单独拓宽或加深胜率为49.4%/48.6%，两者结合达58.5%（p=0.025）。

Conclusion: 程序性记忆为智能体在噪声且不可验证反馈下实现持续自适应提供了一条实用路径，且拓宽与加深两种机制结合使用效果更佳。
...
### [50] [TAPe+ML: A Compact Structured Representation for Multi-Task Computer Vision](https://arxiv.org/abs/2609.20869)
*Sergey Kurinov,Alexey Upatov*

Main category: cs.CV

TL;DR: 提出基于TAPe主动感知理论的紧凑计算机视觉系统TAPe+ML v3，使用结构化TAPe表示和模块化识别架构，统一支持图像分类、目标检测和实例分割。


<details>
  <summary>Details</summary>
Motivation: 减少对海量网络参数、数据、内存和计算的依赖，将部分建模负担从网络参数转移到结构化的输入表示上。

Method: 采用共享TAPe表示与模块化识别架构，结合背景与轮廓处理、局部目标定位、基于原型的分类，以及用于协调专用子模型的协调器。

Result: 参数量少于100,000；COCO检测达到84.7 mAP50和65.3 mAP50-95；COCO实例分割达到80.7 mask mAP50和58.4 mask mAP50-95；Imagenette验证准确率92%，ImageNet-Real Top-1准确率89.9%；还评估了视频场景检测的紧凑性和工业试点中的分布偏移适应。

Conclusion: 结果表明，将部分建模负担转移到结构化输入表示，可支持紧凑的多任务视觉系统，并降低数据、内存和计算需求。
...
### [51] [MemeTAG: Keyword-Driven Meme Classification through Tag Embedding Reconstruction](https://arxiv.org/abs/2609.20962)
*Akshit Sharma,Prashant W. Patil*

Main category: cs.CV

TL;DR: 该论文提出 MemeTAG，一种面向有害网络模因分类的关键词感知双目标框架，在多个基准数据集上取得新的最优结果。


<details>
  <summary>Details</summary>
Motivation: 有害网络模因泛滥并构成社会威胁，但其内容具有多模态和语义细微的特点，导致自动化分类仍十分困难。

Method: 利用预训练视觉语言模型生成描述性关键词；提出基于注意力的聚合标签推理网络 ATIN，将关键词蒸馏为单一丰富语义嵌入；引入辅助重构损失，促使模型学习深度对齐的视觉与文本特征；并采用高效的三阶段训练策略。

Result: 在 HarMeme、Hateful Memes Challenge（HMC）和 PrideMM 数据集上达到新的 state-of-the-art，明显优于现有最优方法。

Conclusion: 关键词感知的语义引导机制与辅助重构损失能有效提升有害模因的多模态分类性能，验证了 MemeTAG 框架的有效性。
...
### [52] [Image-Derived PM10 Estimation in Cattle Feedlot Using Machine Learning: Addressing Concentration Ranges Beyond Existing Digital Imaging Methods](https://arxiv.org/abs/2609.20975)
*Sirapoom Peanusaha,Greg B. Ferguson,K. Jack Bush,Peiyang Li,Brent W. Auvermann*

Main category: cs.CV

TL;DR: 该研究开发了一种基于图像对比面板特征和机器学习的PM10估计方法，用于商业牛饲养场，浓度范围远高于以往城市研究；XGBoost表现最佳，R²为0.792，中位绝对误差为103 µg/m³。


<details>
  <summary>Details</summary>
Motivation: 牛饲养场需要低成本的粉尘监测方案，而基于相机的PM估计虽在城市空气质量研究中逐渐增多，但尚未在集约化畜牧作业的高浓度范围下得到评估。

Method: 在商业牛饲养场傍晚粉尘高峰期间采集灰度图像，提取面板对比度、黑白面板像素值和整体亮度等特征，并加入前序图像近期值和太阳天顶角作为预测变量；比较多种机器学习模型，其中XGBoost预测性能最高，并进行特征重要性分析。

Result: 研究期间PM10小时均值范围为250至1000 µg/m³，瞬时浓度可达5000至20000 µg/m³；XGBoost取得R²=0.792和中位绝对误差103 µg/m³。特征重要性显示，距离相机最远的面板贡献最大，黑色面板像素值比白色面板值对PM10变化更敏感；日落过渡期即饲养场傍晚粉尘峰值开始时的预测精度仍有待改进。

Conclusion: 研究证明基于图像的PM10估计可适用于远超以往城市研究的PM浓度范围，并为未来在饲养场环境中的实际部署提供了实用指南。
...
### [53] [Fragment-Aware Vision Transformers for Fresco-Fragment Style Classification](https://arxiv.org/abs/2609.21012)
*Sara Miketek,Biagio Barchielli,Nadeem Iqbal Kajla,Sinem Aslan*

Main category: cs.CV

TL;DR: 本文研究考古壁画碎片（fresco fragment）的艺术风格分类问题。由于碎片往往不完整、形状不规则且缺乏上下文，作者提出一个渐进式 Transformer 框架，在 ViT-B/16 基础上结合前景引导掩码、基于修复（inpainting）的几何正则化以及分布级对比学习，并采用简单的可学习 logit 集成。在 CLEOPATRA 与 POMPAAF 数据集上，碎片感知建模优于标准 ViT 基线。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格分类通常基于完整艺术品，模型可利用全局构图、空间组织与图像志结构；但在考古场景中，艺术品往往只以碎片形式留存，视觉证据不完整、不规则且上下文受限，因此需要专门针对碎片特性的识别方法。

Method: 以 ViT-B/16 为基线构建渐进式 Transformer 框架，包含三个分支：（1）前景引导掩码，抑制仅含背景的 token；（2）基于 inpainting 的几何正则化，使不规则碎片支撑区域与 ViT 的 patch 网格对齐；（3）监督对比目标，通过 KL 相似度作用于预测分布。三分支以刻意简单的可学习 logit 集成方式组合，并与更复杂的图融合（graph-fusion）变体进行对比。

Result: 在 CLEOPATRA 上，集成将准确率从 0.604 提升至 0.656，macro-F1 从 0.596 提升至 0.648；在 POMPAAF 的六种碎片化设置中有四种优于最佳单分支。图融合变体在 POMPAAF 上与简单集成持平，在 CLEOPATRA 上仅取得数据集相关的微小提升，不足以证明其额外复杂度合理。

Conclusion: 碎片感知建模相比标准 ViT 基线有稳定提升，简单的 logit 集成已足够有效。本文的两点核心贡献为：一是分布级对比目标能持续锐化单分支识别性能；二是可解释性分析验证模型确实利用了真实的绘画证据，同时量化出 inpainting 分支的部分归因来自合成填补区域。
...
### [54] [MAGIC: Marginal-Guided Compression with Optimal Transport for Efficient Visual Document Retrieval](https://arxiv.org/abs/2609.21018)
*Xu Yuan,Hua Liu,Wenqi Fan,Qing Li*

Main category: cs.CV

TL;DR: 提出 MAGIC，一种无需训练的后处理压缩方法，用于多向量视觉文档检索，通过最优传输进行边缘引导压缩，降低索引存储和 MaxSim 评分开销，同时保持检索性能。


<details>
  <summary>Details</summary>
Motivation: ColPali 等 VDR 系统使用 patch 级多向量嵌入，能实现细粒度证据匹配，但索引存储和 MaxSim 评分开销巨大。现有后处理合并方法虽可免训练降本，但其均匀重建目标与晚期交互检索中稀疏、非均匀的 patch 使用模式不匹配，激进压缩下会保留少用 patch，并把检索活动集中在过少代表上。

Method: MAGIC 冻结多向量嵌入，无需训练。它推导 MaxSim 诱导的压缩代理目标，并用双边缘熵最优传输优化：检索需求源边缘优先高使用 patch，平衡目标边缘正则化保留 facet 的使用。消融实验验证两个边缘的互补作用。

Result: 在 ViDoRe 基准、不同保留率和检索骨干上，MAGIC 持续优于强后处理压缩器，尤其在激进压缩场景提升显著。

Conclusion: MAGIC 能有效缓解后处理压缩与晚期交互检索使用分布不匹配的问题，实现高效 VDR，并已开源代码。
...
### [55] [MarsFM: Shading-Regularized Flow Matching for Martian Relief Estimation](https://arxiv.org/abs/2609.21095)
*Marius F. R. Juston*

Main category: cs.CV

TL;DR: MarsFM 是一个图像条件潜在流匹配模型，用于从单波段 HiRISE RED 正射影像估计局部火星地形起伏。它结合预训练生成先验、立体派生几何监督和可微 Lunar-Lambert 阴影目标，并用多种地形与外观约束进行训练。评估显示 1 至 20 步欧拉积分下 RMSE 为 0.0935 至 0.0957，范围较窄，支持低步数推理；诊断显示整体地形对应较好，但存在平滑、振幅压缩和边界不匹配。


<details>
  <summary>Details</summary>
Motivation: 从单波段火星正射影像估计局部地形起伏，需要结合学习到的地形先验与基于图像的物理约束，以弥补直接反演几何信息的困难。

Method: 提出 MarsFM：图像条件潜在流匹配模型；结合预训练生成先验、立体派生几何监督和可微 Lunar-Lambert 阴影目标；使用起伏、法线、梯度、曲率和序数项约束地形结构，并用正仿射不变图像比较约束渲染外观；在 1 至 20 个欧拉积分步下评估。

Result: 每个积分步数收集 2024 个 patch 记录，评估得到归一化 signed-log 起伏空间中平均仿射对齐 RMSE 为 0.0935 至 0.0957。这些分数衡量在正参考支撑上与 VAE 重建参考的一致性；较窄的范围支持该协议下的低步数推理。空间、微分和光谱诊断显示整体地形对应较好，同时存在平滑、振幅压缩和边界不匹配。

Conclusion: MarsFM 提供了一个结合学习地形先验与图像约束的框架；若要建立更好的物理地形分辨率，还需要匹配的基线和独立的高分辨率参考数据。
...
### [56] [4DGS-Fixer: Generative Sparse-View 4D Gaussian Splatting with Iterative Refinement Guided by Video Diffusion Priors](https://arxiv.org/abs/2609.21176)
*Haitao Huang,Shenghao Zhao,Boyuan Tian,Shin-Fang Chng,Songlin Yang,Sheila Lim,Huangying Zhan,Yi Xu,Anyi Rao,Frank Guan*

Main category: cs.CV

TL;DR: 本文提出一种基于视频扩散模型的迭代精炼框架，用于从稀疏视角视频合成动态4D场景，以提升场景完整性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏视角4D高斯泼溅方法依赖几何先验、自适应优化或密度控制，但无法从根本上解决观测不足和场景信息缺失导致的病态问题；同时，COLMAP在少视角下只能重建稀疏、不完整点云，导致大量区域缺乏足够高斯支撑，后续优化难以恢复。

Method: 首先估计多视角深度图并融合成稠密点云，为动态4DGS表示提供更完整的几何初始化；然后使用预训练视频修复模型，对不同时间步沿新相机轨迹渲染的序列进行精炼；最后将修复后的序列作为伪监督，正则化并迭代精炼4DGS表示。

Result: 在广泛使用的基准数据集上，该方法显著优于现有基线，相比此前最佳方法取得接近2 dB的PSNR提升。

Conclusion: 实验表明，通过稠密几何初始化和视频扩散/修复模型提供的伪监督，可以有效改善稀疏视角下动态4D场景重建的完整性与一致性。
...
### [57] [Hand-Aware Transition Modeling for Bimanual Procedural Anomaly Detection](https://arxiv.org/abs/2609.21207)
...
TL;DR: 论文提出 HACT，一种面向双手装配过程异常检测的转移模型。它基于预测的单手事件，结合角色保持历史、标记时间点过程、监督证据头和双状态滤波器，输出每只手的异常后验。在两种双手电动工具装配流程上取得最高 AUPRC 和 F1，恢复报警最少，并能不重新训练地迁移到同一产品的不同装配顺序。


<details>
  <summary>Details</summary>
Motivation: 双手装配中的过程异常检测需要结合当前执行历史来判断每只手的动作。单独看可能正常的纠正动作，或视觉上合理的动作，都可能违反操作顺序，因此需要同时考虑双手职责、语义和时序异常。

Method: HACT 是一个基于预测单手事件的转移模型。它使用角色保持历史记录双手并发职责，并用标记时间点过程为每次观察到的转移分配语义和时序意外度；再通过监督证据头和双状态滤波器将这些意外度转化为每只手的异常后验。评估采用基于预测事件的恢复感知协议和参与者不相交折，并在验证参与者上选择操作点后报告恢复假阳性率。

Result: 在两个双手电动工具装配流程上，HACT 相比对比方法取得最高 AUPRC 和 F1，且恢复报警最少。将其不重新训练地应用到同一产品的不同装配顺序时，仍保持最高 AUPRC 和 F1。

Conclusion: HACT 能有效进行双手装配过程异常检测，并具有良好的跨装配顺序泛化能力；源码已在 GitHub 公开。
...
TL;DR: 本文提出OnomatoBridge，一种用于视觉拟声词翻译的过滤流程，旨在将日语拟声词替换为英语拟声词并保持视觉风格。在Manga109数据集上，该方法优于基线图像编辑模型。


<details>
  <summary>Details</summary>
Motivation: 漫画中的拟声词具有独特的视觉风格，能传达声音、动作和情感。视觉拟声词翻译需要干净地替换日语拟声词并保留视觉风格，但现有方法在去除日语拟声词和渲染风格化英语拟声词时会产生残留伪影或风格不一致。

Method: 提出了OnomatoBridge，一个用于视觉拟声词翻译的过滤流程。在Manga109拟声词数据集上从日语到英语进行评估，并与基线图像编辑模型进行比较。

Result: 实验结果表明，所提方法过滤后的输出优于传统方法。OnomatoBridge将英语文本正确率提高约10到25个百分点，并将残留日语文本相对减少约20%到50%。

Conclusion: OnomatoBridge能有效改善视觉拟声词翻译中的文本正确性和残留问题，性能优于基线方法。
...
### [59] [Multi-viewpoint Geo-localization with Event Cameras](https://arxiv.org/abs/2609.21219)
*Adam D. Hines,Michael Milford,Tobias Fischer*

Main category: cs.CV

TL;DR: 本文提出MegaEvent，一种对视角变化鲁棒的事件相机视觉位置识别（VPR）系统。作者将五个大规模地理标记数据集转为合成事件流，微调预训练事件视觉Transformer，并在现有事件定位数据集和新数据集Springfield-Event-VPR上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 机器人定位需要容忍视角变化的建图与定位系统。事件相机在机器人领域日益受关注，但现有事件定位器对视角变化的鲁棒性研究不足，且强调视角变化的挑战性事件数据集稀缺。

Method: 使用Image-to-Event（I2E）转换，将五个常用于训练帧基定位系统的大规模地理标记数据集转为合成事件流；用多损失函数微调预训练的事件视觉Transformer骨干网络，得到MegaEvent，学习用于位置识别的视角鲁棒特征。

Result: 在三个现有事件定位数据集上平均Recall@1达82%，比次优事件方法高20个召回点，比直接用于事件帧的帧基VPR模型高8至26个召回点。新提出的Springfield-Event-VPR数据集包含3.7km步行路线、三种相机朝向，总计11.1km，MegaEvent比最强基线高9个召回点。

Conclusion: MegaEvent能够有效实现视角鲁棒的事件基视觉位置识别，并显著优于现有事件基和帧基方法；同时，Springfield-Event-VPR为挑战性视角变化下的定位研究提供了新基准。代码已开源。
...
### [60] [VGGT-CAD: Reconstructing Parametric CAD 3D Model with Geometric Grounding](https://arxiv.org/abs/2609.21225)
*Chunan Yu,Tianrun Chen,Fu Shen,Cheng Chen,Lanyun Zhu,Yang Yang*

Main category: cs.CV

TL;DR: 本文提出VGGT-CAD，一种几何感知的参数化CAD重建框架，能够从单视图或多视图观测中恢复精确几何与可编辑建模操作。该框架通过引入预训练3D几何先验、相机参数条件token、可变视图跨视图聚合模块、训练无关的视图选择策略及非自回归解码器生成CAD命令序列。同时构建了VideoCAD多视图视频基准，实验验证了其在不同观测配置下的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有参数化CAD重建方法主要依赖2D外观线索，缺乏强多视图几何先验，在有限且模糊的视图下难以准确重建。因此，需要利用3D几何先验来提升视觉CAD重建的鲁棒性和精度。

Method: 1) 将预训练3D几何先验迁移到CAD重建中，编码相机参数为条件token并与图像token联合建模；2) 提出可变视图跨视图上下文聚合模块，自适应融合多视图特征；3) 开发训练无关的几何感知视图选择策略，在推理时选择互补且可靠的帧；4) 使用非自回归解码器将表示解码为CAD命令序列；5) 构建VideoCAD大规模多视图视频基准，通过多视图重渲染生成。

Result: 大量实验表明，VGGT-CAD在不同观测配置（单视图和多视图）下均能有效进行视觉CAD重建，验证了方法的有效性。

Conclusion: VGGT-CAD通过引入3D几何先验和灵活的多视图融合机制，提升了参数化CAD重建在有限视图下的性能，为视觉CAD重建提供了有效方案。
...
### [61] [Multiclass Semantic Segmentation of Wildland Fire Images Using Context-Aware Centralized Copy-Paste Data Augmentation](https://arxiv.org/abs/2609.21241)
*Joon Tai Kim,Nishanth Kunchala,Vishv Patel,Tianle Chen,Ziyu Dong,Daniel Ospina Acero,Roger Williams,Mrinal Kumar*

Main category: cs.CV

TL;DR: 本文提出一种面向小型多类野火数据集的情境感知数据增强策略，用于改进语义分割。该方法在复制粘贴火源时限制其只能放入语义合理区域，并选择灰烬-植被组成与源上下文最匹配的位置，从而提升增强样本的真实性和分割性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习图像分割需要大量精确标注，但标注成本高、劳动密集；野火场景动态复杂，标注数据尤其稀缺。此前提出的CCPDA通过随机粘贴火源进行增强，但随机放置可能产生不合理场景，如火烧在沥青上。

Method: 在CCPDA基础上提出情境感知策略：将火源放置限制在语义有效的目标区域；根据灰烬-植被组成与源上下文的匹配程度选择粘贴位置；保留目标图像中已有火区域；并通过数值分析和基于加权和的多目标优化（MOO）与其他增强方法进行比较。

Result: 结果表明，情境感知数据增强策略提升了分割性能，并增强了增强样本的上下文真实性，优于其他数据增强方法。

Conclusion: 情境感知的数据增强策略能够为小型多类野火数据集生成更真实、语义合理的训练样本，从而有效改善野火图像语义分割效果。
...
### [62] [SafeStyle: Calibrated Style Residual Injection for Controllable Style-Leakage Trade-off in Diffusion Stylization](https://arxiv.org/abs/2609.21242)
*Zhangping Yang,Min Li,Song Yan,Rong Gao,Xinliang Bi,Guanye Xiong,Yujie He*

Main category: cs.CV

TL;DR: 论文提出 SafeStyle，一种无需训练的参考引导扩散风格化框架，通过校准的风格残差注入，在冻结扩散模型中平衡风格保真度与参考内容泄漏。


<details>
  <summary>Details</summary>
Motivation: 参考图像引导的风格迁移常将可迁移风格与参考特定内容纠缠，导致风格保真度和内容泄漏之间的权衡；同时，纹理主导与几何主导风格的空间组织不同，进一步加剧该问题。

Method: SafeStyle 从紧凑校准集中估计风格支持子空间和内容关联子空间，保留有用重叠并抑制无用内容变化；随后以自适应空间粒度迁移纯化后的风格证据，并通过显式残差范数预算约束其有效影响。

Result: 在纹理和几何主导风格上，SafeStyle 达到 0.432 的 DINO 风格相似度并保持有竞争力的文本对齐；在语义不相交的泄漏压力基准上，达到 0.474 的 DINO 风格相似度，同时仅有 0.8% 语义泄漏。

Conclusion: SafeStyle 能有效平衡风格保真度与参考内容抑制，在无需训练的情况下提升参考引导扩散风格化的可控性。
...
### [63] [Geometry-Aware Diffusion Guidance via Curvature-Adaptive Tubular Correction](https://arxiv.org/abs/2609.21251)
*Enze Jiang,Jinwei He,Zheng Ma*

Main category: cs.CV

TL;DR: 提出一种免训练的曲率自适应管状校正插件CAT，用于稳定扩散模型中的引导采样，在多个逆问题和条件生成任务上提升宿主采样器表现。


<details>
  <summary>Details</summary>
Motivation: 强引导会使扩散采样轨迹进入学习分数支撑较差的区域；现有切空间投影方法只限制一阶偏离等密度面，丢弃法向运动，并忽略切向运动在弯曲表面上引起的二阶偏离。

Method: CAT将引导梯度分解为法向和切向分量，在噪声相关的几何预算内，法向位移按一阶、切向位移按方向曲率计费，并通过一维对偶方程求联合最优幅度；使用Armijo回溯校准有限步长，使用无矩阵方向导数避免构造完整分数雅可比，并给出管状近似的局部保证、校正唯一性与目标充分下降。

Result: 在FFHQ和ImageNet的七类逆问题上，CAT提升了像素空间和潜空间宿主采样器，感知指标增益尤其一致；在InverseBench上改进黑洞重建，并在所有测试的无分类器引导尺度下取得最低FID，同时保持稳定的饱和度和对比度。

Conclusion: 曲率感知的管状控制是一种可复用机制，可用于稳定扩散引导，并改善逆问题与条件生成中的采样质量。
...
### [64] [Edit-VAR: Taming Visual Autoregressive Model for Precise Video Editing](https://arxiv.org/abs/2609.21268)
*Chongbo Zhao,Jiangming Wang,Xilai Wang,Xinyu Wang,Jingyi Tang,Chunjie Hao,Pengjie Song,Yue Ma*

Main category: cs.CV

TL;DR: 提出 Edit-VAR，这是首个基于预训练视觉自回归视频模型的免训练、免反演文本引导视频编辑框架，通过多尺度离散 token 编码和概率引导条件替换实现源内容保持与编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的方法数据与计算成本高；免训练方法中，免反演方法编辑强度受限、语义修改不完整，基于反演的方法则易因近似误差累积导致源内容漂移和时序不一致。

Method: Edit-VAR 将源视频直接编码为多尺度离散 token，并进行概率引导的条件 token 替换以保持源内容；利用注意力引导的 token 级和尺度感知调制，在编辑相关位置和生成阶段选择性放松源约束；采用尺度解耦生成，即后期尺度约束释放，以重建运动一致细节并减少纹理碎片；最后用残差引导 token 剪枝利用高分辨率尺度冗余降低推理成本。

Result: 大量实验和盲用户研究表明，Edit-VAR 在编辑保真度、源内容保持、时序一致性和推理效率方面总体优于现有免训练视频编辑方法。

Conclusion: Edit-VAR 证明了无需训练和反演、基于自回归视频模型的文本引导视频编辑的可行性，并在编辑质量、保持性与效率之间取得更好平衡。
...
### [65] [Beyond Exact Match: Task-Aware GRPO for Cross-Domain PCBA Visual Question Answering](https://arxiv.org/abs/2609.21276)
*Jia Li,Li Dai,Peng Jia,Zhenzhen Hu,Chee Seng Chan,Bingkun Bao,Richang Hong*

Main category: cs.CV

TL;DR: 本文提出一个面向跨域PCBA视觉问答的多模态推理框架，通过统一指令格式、任务感知GRPO奖励设计和鲁棒推理策略，在官方PCBA Standard-to-Real Grand Challenge榜单上取得83.24的总分。


<details>
  <summary>Details</summary>
Motivation: 自动化PCBA检测需要系统联合推理细粒度视觉线索、元件语义和制造知识，但大型视觉语言模型受限于标准样本与真实产线图像之间的域偏移，以及选择型与计数型任务输出空间异构的问题，难以直接部署。

Method: 将标准衍生、真实场景和辅助PCB域数据转换为统一指令格式，构建与视觉证据、问题语义、候选选项和真值答案对齐的验证推理轨迹；提出Task-Aware GRPO，结合选择型问题的多组件语义奖励、计数型问题的距离感知奖励和输出格式奖励；推理时采用答案选项语义一致性校正、自一致性投票和多模型仲裁。

Result: 在官方PCBA Standard-to-Real Grand Challenge排行榜上取得83.24的Overall Score。

Conclusion: 任务感知奖励设计和鲁棒推理策略对跨域PCBA视觉问答有效，所提系统能够提升标准到真实场景的泛化与预测稳健性。
...
### [66] [S3VD: Semantic-Guidance Spatio-Temporal Scanning for Video Deraining](https://arxiv.org/abs/2609.21322)
*Kui Jiang,Yiang Chen,Yan Luo,Zhaocheng Yu,Junjun Jiang,Xianming Liu*

Main category: cs.CV

TL;DR: 本文提出 S3VD，一种用于视频去雨的语义引导时空扫描框架，通过多尺度语义融合和时空扫描融合模块提升强降雨下视频恢复性能。


<details>
  <summary>Details</summary>
Motivation: 强降雨会破坏视频高频细节并引入运动模糊，影响视觉任务可靠性。现有 Mamba 类状态空间模型虽高效且能建模长程依赖，但在雨天视频中难以保持二维空间语义完整性和建模三维时空相关性。

Method: 提出 S3VD 框架，包含两个关键模块：多尺度语义融合（MSSF）模块，利用 DINOv2 的时间语义先验引导特征表示，缓解 Mamba 一维展平导致的局部语义上下文丢失；时空扫描融合（STSF）模块，引入时空扫描机制并设计解耦门控 Mamba（DG-Mamba）层，通过两个独立门控自适应控制输入片段中的前后上下文信息，优化帧内与帧间相关性建模。

Result: 在视频去雨基准上，S3VD 达到最先进性能，相比基于 Mamba 的基线平均 PSNR 提升 0.84 dB。

Conclusion: S3VD 通过语义引导和时空扫描有效克服 Mamba 在视频去雨中的局限，证明其在强降雨视频恢复任务中的优越性。
...
TL;DR: 该研究提出一种几何感知的检测后处理框架，用于从RGB无人机影像中拆分密集作物冠层里重叠的植株实例，提升植株级监测可靠性。


<details>
  <summary>Details</summary>
Motivation: 在密集作物冠层中，相邻植株常相互重叠并被检测为单个目标，导致植株级测量不可靠，因此需要在不增加深度传感器或重新训练检测器的情况下解决重叠实例问题。

Method: 框架结合目标检测与植物组件的几何聚类。对灌木级区域内检测到的叶片或枝条，提取组件质心和径向交点（RIPs）两类几何特征，使用K-means和高斯混合模型判断区域包含单株还是两株重叠植株，并通过密度过滤抑制虚假径向交点，最后用后处理集成融合空间与方向几何信息。

Result: 在田间茄子与番茄无人机影像上评估：质心聚类在茄子上F1为0.89；质心-RIP组合方法在番茄上表现最佳，K-means取得准确率0.80、精确率1.00、F1为0.75；密度过滤显著改善了番茄基于RIP的聚类效果。

Conclusion: 该方法是一种轻量、模块化的工程方案，可集成到现有RGB无人机监测流程中，无需深度传感器、像素级分割、三维重建或重新训练主灌木检测器。结果表明，对现有检测器输出进行几何推理可补充深度学习目标检测，并改善密集农业冠层中的植株级解释。
...
### [68] [VeriFuse: Bounded Vision-Language Arbitration and Reason-Guided Refinement for Cooperative 3D Perception](https://arxiv.org/abs/2609.21323)
*Hongyi Lin,Yiyao Liu,Qi Kang,Heye Huang,Yang Liu,Haris Koutsopoulos,Jinhua Zhao*

Main category: cs.CV

TL;DR: 提出 VeriFuse，一个面向车路协同 3D 检测的有界仲裁框架。它让各智能体独立检测，再由冻结的 VLM 在候选池中执行 SELECT、REFINE 或 REJECT，最终由确定性约束决定 3D 几何。


<details>
  <summary>Details</summary>
Motivation: VLM 具有强场景理解与语义判断能力，但在协同感知中的合适角色尚不明确。直接让 VLM 回归 3D 检测不可靠且计算昂贵，而仅用其选择单来源输出又会丢弃其他智能体的有用信息。

Method: 每个智能体先独立产生检测结果。VeriFuse 围绕车辆和路侧 proposal 生成来源条件几何候选，并将原始检测、扰动和跨源假设合并为统一候选池。冻结 VLM 在三种可允许动作中选择：SELECT 合适候选、REFINE 现有 anchor、REJECT 无支持的路侧-only proposal。确定性约束最终确定 3D 几何。

Result: 在 DAIR-V2X 数据集上，VeriFuse 达到 0.494/0.357 的协同 3D AP50/AP70，并在 300 ms 延迟下将车辆侧 BEV AP50 的相对下降限制在 1.7%。

Conclusion: VeriFuse 为 VLM 在协同感知中分配了清晰且受限的角色：语义推理用于解决跨智能体假设间的歧义，而确定性约束负责最终 3D 几何。
...
TL;DR: 提出Cube-Splat，首个全景GS-SLAM框架，将360°帧分解为四固定朝向虚拟针孔立方体贴图，统一优化位姿与建图，并发布SynPano数据集，在多个室内外基准上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS稠密视觉SLAM多面向针孔相机，未针对全景图像设计；同时缺乏多样、挑战性条件下严格的 panoramc SLAM 评测数据集。

Method: 将每个360°帧分解为共享单光心的四虚拟针孔立方体贴图；以前面为主位姿状态，通过伴随映射累积各面梯度，使多面观测一致更新单一位姿并保持跨视图几何一致性；建图模块用聚合立方体贴图光线稠密化并优化各向异性高斯；提出可扩展、照片级合成数据集SynPano，含参数化复杂轨迹和多模态真值。

Result: 在PALVIO、OmniBlender两个公开基准及自建SynPano数据集上评估，涵盖室内外场景，Cube-Splat在跟踪精度和重建保真度上达到SOTA；代码与数据集已开源。

Conclusion: Cube-Splat为全景GS-SLAM提供了有效解决方案，验证了立方体贴图分解与伴随映射在统一位姿优化和稠密重建中的有效性，并通过SynPano推动了全景SLAM评测。
...
### [70] [PrismAlign: Prior-Steered Multi-View VLM Alignment for Hallucination-Robust Table OCR](https://arxiv.org/abs/2609.21351)
*Guangyi Liu,Qianjun Huang,Boyu Hou*

Main category: cs.CV

TL;DR: 提出PrismAlign多视觉语言模型框架，通过对齐多样视觉视角、集成表格逻辑先验和贝叶斯决策策略，解决表格提取中的结构错误和语义幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 表格提取常出现结构错误和语义幻觉，影响提取准确性。

Method: 采用多VLM框架对齐不同视觉视角，集成表格逻辑先验评估输出合理性，将结构对齐与单元格内容对齐解耦，并利用贝叶斯决策策略最大化对齐准确率，该策略利用提取错误与可计算规则违反之间的相关性。

Result: 在开源和自定义VLM上评估，PrismAlign减少了幻觉，在OmniDocBench 1.5、CC-OCR的表格类别和PureDocBench上达到最先进性能。

Conclusion: PrismAlign能有效提升表格提取的准确性和鲁棒性。
...
### [71] [Field Tracking of Insects Using a Stereoscopic Event-Based Camera Setup](https://arxiv.org/abs/2609.21354)
*Pratham G. Shenwai,Martin J. Lankheet,John T. Hrynuk,Mandiyam Y. Mahadeeswara,Mandyam V. Srinivasan,Sridhar Ravi*

Main category: cs.CV

TL;DR: 该研究将事件相机的异步事件转换为传统视频格式，并结合立体配置，实现野外快速移动小生物的连续、低延迟三维追踪，显著减少运动伪影。


<details>
  <summary>Details</summary>
Motivation: 传统帧式成像在追踪小型快速移动生物时，受低时间分辨率导致的运动模糊和数据存储限制影响，因此需要更自适应的成像方案。

Method: 利用事件相机捕捉像素级亮度变化，将异步事件转换为传统视频格式，从而使用标准视频处理和图像分析技术；同时结合立体视觉配置进行三维追踪。

Result: 实现了野外条件下对快速移动目标的连续、低延迟三维追踪，大幅减轻运动伪影，并更准确地表征动物运动。

Conclusion: 该方法使事件成像更适用于自然野外场景，可支持动物行为与生态研究、农业管理及其他需要野外高保真目标追踪的领域。
...
### [72] [Hiding in Plain Sight: A Diffusion-based Mitigation of Geolocation Privacy Leakage in Vision-Language Models](https://arxiv.org/abs/2609.21363)
*Yining Wang,Xi Li,Mi Zhang,Xiaohan Zhang,Xiaoyu You,Zhenxing Qian,Mi Wen*

Main category: cs.CV

TL;DR: 本文系统研究了多模态大推理模型（MLRM）从日常照片推断用户地理位置的隐私泄露风险，并提出一种基于扩散模型的主动防御框架，在潜空间中注入扰动以破坏地理位置相关语义信号，从而在保持图像质量的同时提升黑盒迁移性。


<details>
  <summary>Details</summary>
Motivation: MLRM 能通过建筑风格、植被、光照等细微视觉线索进行结构化推理并精准推断地理位置，构成严重且未被充分研究的隐私威胁；同时现有拒绝式防护易被越狱绕过，像素空间扰动防御存在黑盒迁移性差和视觉伪影等问题。

Method: 提出基于扩散模型的防御框架：在反向采样过程中向扩散模型潜空间注入扰动，直接作用于高层语义表示；并以与 GPS 坐标对齐的 GeoCLIP 作为代理模型引导优化，定位并破坏 MLRM 用于位置推断的地理信号。

Result: 实验表明，精心设计的越狱提示可使模型响应率达到 100%，说明拒绝式防护严重不足；现有像素空间防御受结构限制，黑盒迁移性下降且产生明显视觉伪影；所提方法显著增强黑盒迁移性，同时保持感知图像质量，便于社交媒体平台集成。

Conclusion: 针对 MLRM 地理定位隐私泄露，需从像素空间转向潜空间语义层面的主动防御；通过扩散模型潜空间扰动与 GeoCLIP 引导，可更有效地破坏地理位置推理信号，兼顾防御效果与图像可用性。
...
### [73] [RobotEQ-Video: A Video-Centric Benchmark for Social Proactive Intelligence with World-State Taxonomy](https://arxiv.org/abs/2609.21371)
...
TL;DR: 本文提出 RobotEQ-Video，将社交主动智能（SPI）从静态图像推进到动态视频，并构建了覆盖更全面的视频基准，用于评估具身场景中主动辅助行为的社会恰当性。


<details>
  <summary>Details</summary>
Motivation: 现有 SPI 研究存在两点不足：一是主要基于静态图像，缺乏动态视频中推断人类状态与需求的关键线索；二是常依赖自由形式的数据收集流程，难以保证场景覆盖的全面性。

Method: 构建 RobotEQ-Video 视频基准；设计四层由粗到细的分层世界状态分类体系，包含 6 个领域、20 个维度、142 个一级属性和 816 个二级属性；收集 2K+ 视频、100K+ 人工标注和 16K+ 行为恰当性标签；并探索世界模型在该任务中的作用。

Result: 基准评估显示，当前系统仍不可靠，且表现低于人类水平；同时探索了世界模型对该任务的潜在帮助。
...
### [74] [JEPA Guided Diffusion: Predictive Vision-Language Conditioning for Generative Traffic Forecasting](https://arxiv.org/abs/2609.21379)
*Trinh Tra Giang Nguyen,Thanh Nguyen Vo,Nguyen Hoai Thuong Bui,Ha Duc Bui*

Main category: cs.CV

TL;DR: 提出一种解耦式交通预测框架：冻结V-JEPA提取预测性潜表征，再用轻量对齐模块将其映射到冻结Cosmos扩散模型的条件空间以生成未来视频，仅训练对齐模块。在AI City Challenge 2026 Track 5上得分75.1297，排名第三。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频生成模型虽能产生视觉合理的交通预测，但需要昂贵的端到端训练，且常将场景理解与图像合成纠缠在一起。研究希望分离未来表征学习与视频生成，降低优化复杂度并提高效率。

Method: 使用冻结的V-JEPA编码器从观测交通视频中提取语义潜表征，捕捉场景动态；通过轻量潜对齐模块将这些表征投影到冻结Cosmos扩散模块的条件空间；冻结所有基础模型，仅训练轻量对齐模块，从而在无需重训大型生成模型的情况下合成未来视频。

Result: 在AI City Challenge 2026 Track 5基准上取得75.1297分，排名第三。结果表明V-JEPA学到的预测性世界表征能有效指导下游视频生成。

Conclusion: 该解耦框架为端到端扩散式交通预测提供了一种实用且高效的替代方案，在降低优化复杂度的同时保持预测能力。
...
### [75] [AgentVidBench: A Multi-Hop Video Question Answering Benchmark for Evaluating MLLM Agents](https://arxiv.org/abs/2609.21386)
*Seoyeon An,Hyeonseo Jang,Minsu Kim,Chanho Lee,Younghan Park,Kangwook Lee*

Main category: cs.CV

TL;DR: 本文提出了 AgentVidBench，一个面向多跳视频问答的基准，用于评估多模态大语言模型智能体在空间、时间和因果推理上的能力，并提供分步解题轨迹以支持过程评估。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准多局限于简单场景查询或全局摘要，只需单步推理，无法严格评估真实世界中需要多跳多模态推理的智能体能力，因此亟需专门的评测基准。

Method: 构建 AgentVidBench 多跳视频问答基准，除问答对外还提供逐步解题轨迹以支持轨迹评估；对12个专有和开源 MLLM 进行实验，并测试将其集成到先进智能体工作流中的效果，同时提出一个简单有效的智能体策略作为基线。

Result: 实验表明，单轮模型在 AgentVidBench 上性能有限；将模型集成到先进智能体工作流后，准确率和轨迹分数通常均有提升；所提简单智能体策略成为有竞争力的基线。

Conclusion: AgentVidBench 可作为智能体视频理解研究的综合性测试平台，推动多跳、多模态、可解释的视频推理研究。
...
### [76] [A Scene Language Model for Open-Vocabulary Scene Mapping](https://arxiv.org/abs/2609.21400)
*Adam Lilja,Fabio Hübel,Siming He,Junsheng Fu,Claire Tomlin,Lars Hammarstrand,Jitendra Malik,Jonas Frey,Marco Pavone*

Main category: cs.CV

TL;DR: SceneLM 提出用单一视觉-语言模型直接维护结构化文本场景地图，通过逐帧读取并更新对象列表实现开放词汇 3D 场景建图，仅需轻量文本表示。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇 3D 场景建图依赖工程化流水线进行观测关联、跨视图融合和一致性维护，并常存储嵌入或图像裁剪等特征丰富的对象表示，导致持久内存更大、系统更复杂。

Method: 将完整场景表示为结构化文本对象列表，作为模型唯一持久记忆；对每张输入图像，模型读取当前场景状态并添加、编辑、删除对象。提出迭代式场景地图维护监督任务，以及无需人工标注的自动标注流水线来生成训练数据。

Result: 在语言检索和定位基准上，SceneLM 生成的场景地图与由专用感知和几何模块构建的完整建图系统性能相当，但表示紧凑性提升 6-12 倍；并可在四足机器人边缘设备上在线运行。

Conclusion: 持久化开放词汇 3D 场景地图可以直接由单个视觉-语言模型维护，仅使用轻量级文本表示即可实现有竞争力的性能与在线部署能力。
...
### [77] [SIRA: Reasoning-Aware Surgical Instrument Segmentation via Query-Anchored Alignment](https://arxiv.org/abs/2609.21402)
*Zhibo Zhang,Qijie Wang,Zengqiang Yan*

Main category: cs.CV

TL;DR: 本文提出推理感知手术器械分割（RA-SIS）新任务，并构建SurgRS数据集与SIRA多模态框架，实现基于查询和手术上下文的分割。


<details>
  <summary>Details</summary>
Motivation: 现有手术器械分割方法多将分割视为类别驱动的定位问题，难以捕捉手术流程中的程序上下文和任务相关语义。

Method: 提出RA-SIS任务，将分割定义为手术上下文下的查询条件推理；构建SurgRS数据集，包含41,000个图像-文本对，将实例级掩码与结构化问答监督对齐；提出SIRA多模态框架，通过查询锚定双重对齐解耦目标级和查询级语义，并与视觉特征、分割提示对齐。

Result: 在SurgRS上的大量实验表明，SIRA优于现有推理感知基线方法。

Conclusion: 推理感知分割有助于提升手术器械分割的语义-视觉一致性，代码已开源。
...
### [78] [Quantization-Aware Kalman Estimation for Diffusion Sampling](https://arxiv.org/abs/2609.21407)
*Qitan Shi,Cheng Jin,Jiawei Zhang,Yuantao Gu*

Main category: cs.CV

TL;DR: 该论文提出 QuAKE，一种面向量化扩散模型采样阶段的轻量校正器，将量化去噪器采样建模为在线估计问题，利用历史输出来恢复全精度去噪输出，从而减小量化采样与全精度采样的分布差异。


<details>
  <summary>Details</summary>
Motivation: 量化可降低扩散模型的内存和计算开销，但激进量化会使输出显著偏离全精度结果。现有采样阶段校正方法主要依赖局部信息，未充分利用轨迹历史，难以纠正跨时间步传播的误差。

Method: 将使用量化去噪器的采样表述为在线估计问题，用历史量化去噪器输出来估计采样器所需的全精度输出。提出 QuAKE（Quantization-Aware Kalman Estimator），结合平滑轨迹先验与条件高斯观测模型，在每个采样步以闭式递归更新输出窗口的后验，并将后验均值输入采样器。QuAKE 是轻量即插即用校正器，无需修改量化网络，并支持任意高阶多步 ODE 采样器。

Result: 在 W4A4 量化的文本到图像扩散模型实验中，QuAKE 在减小与全精度采样的分布差异方面持续优于现有方法。

Conclusion: QuAKE 通过利用轨迹历史进行量化感知的在线估计，为量化扩散模型采样提供了一种有效、通用且无需改动网络的校正方案。
...
### [79] [When Online Adaptation Hurts: Parameter-Frozen Test-Time Ensembling for Continual Medical Image Segmentation](https://arxiv.org/abs/2609.21412)
*Ruijie Huang*

Main category: cs.CV

TL;DR: 该论文研究医学图像分割在跨中心、跨设备或跨协议时的性能下降问题，提出一种无需更新模型参数的推理增强方法PIE，并在心脏MRI序列适应任务上取得优于源模型和多种在线适应基线的稳定表现。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割模型在站点、扫描仪厂商或成像协议变化时性能常下降；持续测试时适应（CTTA）虽可在无目标标签下更新模型，但在非平稳数据流上可能难以更新且容易引入错误，因此需要更合理、稳定的替代方案。

Method: 提出参数冻结推理增强（PIE）：使用源域训练的分割器，对输入进行解剖结构保持的尺度变换和翻转视图，将各视图预测映射回原始位置后平均概率；不修改模型权重，也不更新归一化统计量。

Result: 在M&Ms心脏MRI数据流上，源域为厂商A，顺序评估厂商B、C、D时，PIE平均Dice为0.7786，优于仅源域推理的0.7680和五种在线适应基线的0.7388–0.7416。消融显示性能在28个视图时饱和；置信度加权、类别先验校正、连通域滤波、形态学细化和层间平滑无效或导致负迁移。心脏MRI和眼底图像的定性结果也表明冻结集成能更好保留细薄和嵌套解剖结构。

Conclusion: PIE为医学CTTA提供了强而稳定的基线，并揭示一个重要失效模式：模型适应和手工设计后处理可能不如精心设计的推理策略可靠。
...
TL;DR: 本文针对带钢表面缺陷小样本语义分割（S³D）中低局部对比度、光照不均和细粒度纹理复杂等工业图像难题，提出 P³-SAM 框架，通过感知优化编码和并行提示生成器改进 SAM，在多个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于 SAM 的小样本分割方法在自然图像上表现良好，但直接迁移到工业带钢表面缺陷时，因工业图像特有的低对比度、不均匀光照和复杂细粒度纹理，性能明显下降。

Method: 提出 Perceptual Parallel Prompt（P³）框架，构建 P³-SAM。核心包括：1）Perceptual-Optimized Encoding（POE）策略，增强局部对比度并保留关键纹理细节；2）Parallel Prompt Generator（PPG）策略，同时生成语义提示和空间提示，为 SAM 解码器提供全面引导。

Result: 在三个小样本 S³D 基准上，P³-SAM 达到最先进性能，其中在 Surface Defects-4i 数据集上 mIoU 提升 12.00%。

Conclusion: P³-SAM 能有效适配 SAM 到工业带钢表面缺陷小样本分割任务，验证了感知优化与并行提示策略在应对工业图像特殊挑战方面的有效性。
...
### [81] [Think Locally, Refine Globally for Memory-Efficient 3D Reconstruction](https://arxiv.org/abs/2609.21437)
*Jingke Zhou,Chenhang Ma,Zhizhou Zhong,Mingkai Liu,Zhuang Zhou,Yicheng ji,Binghua Su,Bo Cai,Xianliang Huang*

Main category: cs.CV

TL;DR: 提出LoG-VGGT，一种面向长序列3D重建的内存高效框架，通过局部时序建模与全局相机一致性相结合，在有限显存下提升深度精度与相机位姿稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖全局注意力进行长序列3D重建，内存开销大；同时长时序中容易产生相机位姿漂移，需要兼顾效率与全局一致性。

Method: 在少量Transformer块中引入跨窗口注意力，实现相邻时序窗口的信息传播并控制内存；设计全局相机一致性细化模块，让相机token通过交叉注意力与紧凑register token交互，以场景级约束优化相机表示，避免全序列注意力。

Result: 在多个长序列基准上取得更好的深度精度和鲁棒的相机位姿估计，同时流式重建性能具有竞争力。

Conclusion: LoG-VGGT能够在有限内存下有效平衡局部时序建模与全局相机一致性，提升长时程位姿稳定性，适用于长序列3D重建任务。
...
### [82] [ME-Dex 1.0: Bringing Heterogeneous Tactile Sensing into World Action Modeling](https://arxiv.org/abs/2609.21449)
*Xuancheng Zhang,Xuetao Liu,Qianying Tang,Jizhe Wang,Zhijing Cheng,Bochen Lin,Haoran Wen,Ming Li,Kun Zhan,Yu Liu*

Main category: cs.CV

TL;DR: 本文提出 ME-Dex-1.0，一个统一的 World Action Tactile Model，通过联合建模视觉、触觉与动作进行机器人操作学习，在仿真与真机实验中均验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有 World Action Models 利用视频模型预测未来视觉状态，但缺乏对物理交互的直接测量；已有触觉方法多将触觉特征作为条件输入，而未将触觉与视觉、动作一起作为未来观测进行联合预测。作者认为触觉信号与视频一样，都是对世界状态演化的观测，应被建模为未来观测。

Method: 提出 ME-Dex-1.0（MachEmbodied-Dex-1.0），采用 Mixture-of-Transformers 架构，包含 Video Expert、Tactile Expert 和 Action Expert，均以 flow matching 训练；通过共享注意力在中间层连接各专家，使动作生成在联合去噪过程中利用视觉与触觉动态表示。为支持多源异构触觉输入，设计 Canonical Hand Model 与 Unified Tactile Autoencoder，将不同本体与传感布局的触觉观测映射到共享空间与潜空间。此外构建 Agentic Tactile Data Engine，基于智能体的数据生产平台，在仿真轨迹回放中直接记录力传感器触觉数据以补充 RoboTwin 与 DexJoCo。

Result: 在 RoboTwin、DexJoCo、ManiFeel 三个仿真平台以及真机评估中，配备触觉传感的夹爪与灵巧手均展现出更优的操作性能。

Conclusion: 将触觉作为未来观测与视频、动作进行联合建模，能够有效提升机器人操作性能，并可通过统一架构与数据引擎解决异构触觉与数据稀缺问题。
...
### [83] [CompAdapt: Adaptable Composite Motion Modeling for Physics-Consistent Text-to-Video Generation](https://arxiv.org/abs/2609.21455)
*Haoran Qin,Renlong Wu,Tianyu Huang,Yukang Ding,Hui Li,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 提出CompAdapt，一种物理一致的文本到视频框架，可处理耦合运动、多阶段转换和多物体碰撞等复合物理行为，并能通过语言指定物理语义及一次性适应新物理环境。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式文本到视频模型虽能生成逼真且时序连贯的视频，但常违反基本物理动力学；已有物理约束方法仅适用于简单单一运动、依赖手动参数，且难以泛化到未见物理规律。

Method: 将神经动力学建模从单一运动扩展到复合物理行为；把自然语言提示转化为结构化物理语义，以端到端指定运动类型、时序关系和初始物理参数；引入动态感知先验匹配，实现无需重训核心动力学模块的一次性适应；并设计物理感知潜在特征融合模块，提升快速复杂运动下的视觉保真度。

Result: 在物理聚焦的文本到视频基准上，CompAdapt在物理一致性上优于通用T2V模型和物理约束基线，同时保持高视觉质量和对未见动力学的适应能力。

Conclusion: CompAdapt能够面向复杂真实场景实现物理一致且可适应的文本到视频生成，兼顾物理合理性与视觉质量。
...
### [84] [PSEE: Progressive Sensor Event Expansion for Point-Supervised Temporal Action Localization](https://arxiv.org/abs/2609.21462)
*Jiaxi Yin,Ge Wang,Han Ding,Fei Wang*

Main category: cs.CV

TL;DR: 针对可穿戴传感器流中的时序动作定位（TAL），提出点监督方法PSEE，通过语义激活、传感器特定过渡证据和自适应时间归属生成伪片段，用于训练标准TAL检测器，在四个惯性传感基准上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 传统TAL需要每个动作实例的起止时间标注，成本高昂；点监督仅需一个时间戳和类别标签，但如何从点标注恢复高质量伪片段是挑战。

Method: 提出渐进式传感器事件扩展（PSEE），结合语义激活、传感器特定过渡证据和自适应时间归属，生成点监督伪片段，用于监督标准TAL检测器，不改变推理过程。

Result: 在四个惯性传感基准的跨被试实验中，PSEE生成的伪边界质量优于适配的点监督基线，兼容不同TAL检测器，并对点采样具有鲁棒性。

Conclusion: PSEE能有效降低标注成本，提升点监督TAL性能，具有通用性和鲁棒性，代码已开源。
...
### [85] [SkillIR: Evolving Scene-Aware Skills for Agentic Image Restoration](https://arxiv.org/abs/2609.21468)
*Jie Shao,Shengkai Hu,Xu Zhang,Beihang Song,Yongcheng Jing,Xu Wu,Jun Wan*

Main category: cs.CV

TL;DR: 本文研究智能体图像恢复，提出SkillIR框架，将恢复经验表示为以退化为中心的动作证据（技能），在验证的残差状态循环中逐步指导工具使用，从而提升恢复质量与工具使用可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有恢复智能体通常从原始退化图像生成完整工具使用计划或检索历史成功轨迹，难以适应中间恢复状态的变化；研究发现已执行的工具会改变残差退化状态，进而影响后续工具的适用性。

Method: 提出SkillIR，将上下文相关的动作结果整合为场景感知的恢复技能（包含适用条件、预期效果、可归因失败案例）；检索到的技能在验证残差状态循环中一次指导一个受限动作，工具输出需经转移验证后才被采纳，并重新评估活跃残差退化；每次rollout后证据用于创建、精炼或修补动态技能。

Result: 在合成和真实多退化数据集上的实验表明，SkillIR提高了恢复质量，并实现了更可靠、有效的工具使用。

Conclusion: SkillIR通过技能引导和残差状态验证，能够适应动态退化状态，提升图像恢复性能与工具使用的可靠性。
...
### [86] [MT-WAM: Reorienting the One-Pass Predictive Representation Toward Action Generation](https://arxiv.org/abs/2609.21474)
*Yiguang Yang,Jiankun Peng,Xiaoming Wang,Yiran Zhang,Zhibo Fang*

Main category: cs.CV

TL;DR: MT-WAM 在 Fast-WAM 的视频-动作联合训练框架上新增未来二维点轨迹与视觉特征的辅助监督，在不生成未来视频、不增加具身策略预训练的前提下显著提升机器人控制成功率。


<details>
  <summary>Details</summary>
Motivation: Fast-WAM 表明视频-动作联合训练可提升控制性能，但仅预测未来观测并未显式强调控制所需的未来动力学与视觉结构信息，因此需要更有针对性的监督信号。

Method: 保留原有训练目标，额外加入未来二维点轨迹与未来视觉特征预测监督；从视频骨干最后若干块复制出轻量双流分支进行目标特定处理，并用结构化注意力掩码阻断跨流注意力。运动流 token 为动作专家提供额外动力学条件，视觉特征预测则在捕捉物体与空间结构的特征空间中进行监督，从而训练视频骨干提供更有信息量的视觉上下文，且不将视觉特征流 token 加入动作条件。推理阶段仅使用每次重规划计算一次的视频与运动缓存，跳过未来视频预测。

Result: 无需额外具身策略预训练，LIBERO 成功率达 98.2%，LIBERO-Plus 达 73.7%（较 Fast-WAM 提升 23.8 个百分点）；RoboTwin 2.0 Clean2Rand 的 Random 成功率由 6.30% 提升至 19.40%；四项真实世界任务平均成功率由 67.0% 提升至 77.8%。

Conclusion: 引入未来轨迹与视觉特征的互补监督，可在不生成未来视频的前提下显著增强视频-动作模型的控制能力与泛化性。
...
### [87] [OpenSAL360: Open-Source Crowdsourcing Platform for Omnidirectional Video Saliency Collection](https://arxiv.org/abs/2609.21480)
*Alexey Bryncev,Andrey Moskalenko,Kira Shilovskaya,Ivan Kosmynin,Dmitriy Vatolin*

Main category: cs.CV

TL;DR: 本文提出OpenSAL360，一个开源、可扩展、低成本的360°视频显著性数据采集平台，仅需普通屏幕、鼠标和网络即可众包采集，并发布了500个全景视频、2000多名标注者参与的大规模显著性数据集。


<details>
  <summary>Details</summary>
Motivation: 全向视频显著性预测对沉浸式多媒体应用很重要，但传统基于VR头显和眼动追踪的数据采集成本高、流程复杂，导致大规模数据集难以扩展。

Method: 设计OpenSAL360开源众包采集协议，无需专用VR硬件；与7个成熟的VR眼动追踪数据集进行验证，并对界面、预处理和后处理等关键参数开展消融研究；随后采集并公开大规模360°视频显著性数据集。

Result: 验证了所提采集协议相对于七个VR眼动追踪数据集的有效性，并完成参数消融；发布了包含500个全向视频、由2000多名众包评估者标注的显著性数据集，据称是该领域目前最大的数据集。

Conclusion: OpenSAL360能够以低成本、可扩展的方式采集360°视频显著性数据，有助于推动全向视频显著性预测及相关沉浸式多媒体应用研究，且平台已公开在GitHub上。
...
### [88] [VoxelTTO: Voxel-Aligned Feed-Forward 3D Gaussian Splatting with Test-Time Optimization](https://arxiv.org/abs/2609.21498)
*Yibin Zhao,Yihan Pan,Yangwen Li,Jun Nan,Jianjun Yi*

Main category: cs.CV

TL;DR: VoxelTTO 是一个前馈 3D Gaussian Splatting 框架，可从任意数量图像和可选相机参数中重建几何精确的 3DGS 场景，并在多个数据集上提升 RGB-D 新视角合成与相机位姿估计。


<details>
  <summary>Details</summary>
Motivation: 现有前馈 3DGS 方法通常回归像素对齐的高斯基元，容易造成高斯过度重叠和伪影；同时预测相机位姿不准确会导致新视角合成中的错位问题。

Method: VoxelTTO 将密集图像特征聚合到全局体素表示，并从体素特征解码高斯，打破像素到高斯的对应关系。为利用已知相机参数并保持预训练视觉基础模型冻结，引入测试时优化来适配轻量 LoRA 模块，并使用位姿监督。训练和推理阶段用随机实体体积渲染替代原始 3DGS 光栅化，以提升几何保真度。训练仅更新体素对齐的高斯重建模块，约需 80 GPU 小时。

Result: 在 Replica、Tanks and Temples 和 DTU 数据集上的实验表明，该方法在 RGB-D 新视角合成和相机位姿估计方面优于先前方法。

Conclusion: 通过体素特征解码高斯、测试时优化 LoRA 模块以及随机实体体积渲染，VoxelTTO 能有效提升 3DGS 重建的几何精度、新视角合成质量和相机位姿估计性能。
...
### [89] [Adaptive World Memory 3D Foundation Model for Scalable 3D Mapping, Localization, and Rendering](https://arxiv.org/abs/2609.21502)
...
TL;DR: 提出一种以记忆为中心的3D基础模型，用于可扩展的机器人定位、重建与高斯渲染，核心是自适应世界记忆机制，统一了位姿估计、稠密点云重建和逼真渲染。


<details>
  <summary>Details</summary>
Motivation: 现有3D基础模型虽能从RGB图像进行泛化几何推理，但在持久记忆、可扩展性和可渲染场景建模方面仍存在局限。

Method: 采用自适应世界记忆机制：基于Transformer的门控更新与测试时时空调节，学习门控控制循环记忆传播，时间状态演化和空间观测-状态一致性调节长序列中token级更新与遗忘；通过局部子图组织内存，集成渐进式建图与跟踪、回环检测及基于SL(4)的全局优化；高斯重建头将记忆增强特征解码为可渲染图元，统一相机位姿估计、稠密点云重建和逼真渲染。

Result: 在公开基准和多种机器人平台的自采数据集上，轨迹精度、重建完整性和渲染质量均优于现有3D基础重建和SLAM基线。
...
### [90] [2D GauSS-MI: Efficient Active Scene Reconstruction with Balanced Visual and Geometric Quality](https://arxiv.org/abs/2609.21516)
*Yuhan Xie,Jia Pan*

Main category: cs.CV

TL;DR: 本文提出基于2D Gaussian Splatting的主动重建框架，通过在线增量RGB-D建图与概率可靠性模型，定义2D GauSS-MI互信息指标进行主动视角选择，在有限算力下兼顾视觉与几何重建质量。


<details>
  <summary>Details</summary>
Motivation: 主动重建需要在有限机载计算资源下实现高质量重建，但现有方法难以在视觉与几何质量、实时计算效率之间取得良好平衡。

Method: 构建面向增量RGB-D观测的高效在线2DGS建图流程；提出表征单个2D高斯溅射视角相关重建质量的概率可靠性模型；利用2DGS显式表面朝向，提出基于互信息的2D GauSS-MI指标，评估候选视角的期望信息增益并用于主动视角选择。

Result: 在八个Replica场景上与三种先进基线方法对比，所提方法在视觉与几何重建质量之间取得良好平衡，同时计算成本显著更低，模型存储具有竞争力。

Conclusion: 基于2DGS的互信息主动视角选择能同时考虑视觉和几何重建质量，并在有限计算资源下实现高效主动重建。
...
### [91] [VidOmni-Bench: A Benchmark for Fine-Grained Video Understanding via Spatio-Temporal Event Verification across Complexity and Duration](https://arxiv.org/abs/2609.21521)
*Changbeen Kim,Junwon Chang,Kipyo Kim,Risa Shinoda,Kuniaki Saito,Donghyun Kim*

Main category: cs.CV

TL;DR: 本文提出 VidOmni-Bench，一个通过密集视频描述中的事件是否被视频支持来评估 Video-LLM 细粒度视频理解能力的基准，并发现模型存在幻觉、验证能力不足及瓶颈差异等问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频大模型评测多依赖问答或真实描述匹配，模型可能借助表面线索或利用不完整标注取得高分，难以可靠评估其细粒度视频理解能力。

Method: 构建 VidOmni-Bench：包含 500 个视频，覆盖五种复杂度类型和 4 秒到 90 分钟的视频时长；使用多种 Video-LLM 生成密集描述，并人工验证句子级标签，将包含错误事件的句子作为困难负样本，要求模型判断每个事件是否被视频支持。

Result: 实验发现三点：(i) Video-LLM 在密集视频描述中经常产生幻觉；(ii) 作为验证器时也难以可靠识别看似合理但错误的事件描述；(iii) 模型在不同视频复杂度和时长下表现出的弱点不同，暴露出模型特定的瓶颈。

Conclusion: VidOmni-Bench 能更严格地评估 Video-LLM 的细粒度视频理解，并揭示当前模型在描述与验证方面仍存在显著不足，且瓶颈因模型和视频特性而异。
...
### [92] [Refine Then Fusion: Training-Free 3D Point Cloud Adaptation with Priority Refinement and Multi-Modal Knowledge Fusion](https://arxiv.org/abs/2609.21522)
*Hang Cheng,Yan Chen,Mingyu Fan,Long Zeng*

Main category: cs.CV

TL;DR: 本文提出无需训练的 Refine Then Fusion（RTF）框架，用于少样本 3D 识别；通过先精炼判别性特征通道，再进行可靠性感知的多模态融合，在五个基准上取得最优性能，且无需梯度优化、额外数据或参数更新。


<details>
  <summary>Details</summary>
Motivation: 预训练基础模型虽提供丰富的多模态先验，但在少样本 3D 场景中，高维特征常含通道冗余和任务无关噪声，且不同模态在不同样本上的可靠性不同；直接聚合异构表征会忽略样本级模态可靠性，并可能掩盖关键判别线索。

Method: RTF 首先联合建模类间相似性与类内稳定性，识别判别性特征通道，从预训练模型缓存表征中解耦并精炼领域特定知识；然后引入可靠性感知融合机制，根据特征精炼引起的分布偏移估计样本级模态可靠性，实现自适应多模态聚合；最后构建融合实例级支持特征与类级原型的内存缓存，用于推断查询标签。

Result: 在五个基准上的大量实验表明，RTF 持续优于单模态基线、部分融合变体及现有轻量适配方法，在无需梯度优化、额外训练数据、辅助训练或参数更新的条件下，取得了最先进的少样本 3D 识别性能。

Conclusion: RTF 是一种有效的免训练少样本 3D 识别框架，通过判别性特征精炼与样本级可靠性感知融合，能够更充分地利用预训练多模态表征，并在不更新参数的情况下实现优越性能。
...
### [93] [Purification and Regulation: Comorbidity-Aware Multi-Label Few-Shot Learning for Medical Image Classification](https://arxiv.org/abs/2609.21541)
*Ying-Chih Lin,Po-Chih Kuo,Yong-Sheng Chen*

Main category: cs.CV

TL;DR: 本文提出 Prototype Purification and Regulation (PPR) 框架，用于医学图像分析中的多标签小样本学习，通过原型净化和类间关系调控提升疾病检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有多标签小样本医学图像方法存在两个关键局限：传统原型易混入无关疾病信息，导致原型污染；同时过于强调类间可分性，忽视了疾病之间的内在相关性。

Method: PPR 首先利用样本级共病分数进行原型净化，突出疾病特异性特征，生成更纯净的原型；随后引入疾病级共病统计，自适应调控类间相似度，构建共病感知的嵌入空间，以同时建模纯疾病特征和类间关系。

Result: 在四个胸部 X 射线基准数据集及跨域评估中，PPR 持续优于现有最先进方法，显著提升疾病检测效果，并展现出较强的泛化能力和临床适用性。

Conclusion: PPR 能有效解决医学图像多标签小样本学习中的原型污染和类间关系建模不足问题，为可靠的多标签小样本医学图像分析提供了新框架。
...
### [94] [From Retrieval to Recognition:How Vision--Language Models Become OCR Specialists](https://arxiv.org/abs/2609.21543)
*Yuanxiang Huangfu,Hanmeng Zhong,Linqing Chen,Jeffrey Tiong Jee Hui*

Main category: cs.CV

TL;DR: 该论文研究通用视觉语言模型获得专门OCR能力时，是形成新阅读回路还是复用已有机制。结果表明，全序列OCR主要复用并重塑通用的文本检索/复制回路，表现为密集的序列化多模态复制粘贴。


<details>
  <summary>Details</summary>
Motivation: 已有研究多关注局部答案检索，尚不清楚通用VLM的OCR能力是来自全新专用电路，还是复用通用文本检索/复制机制。

Method: 在全序列OCR设定下，采用基于证据且带留出因果干预的协议，识别GLM-OCR、MinerU2.5和PaddleOCR-VL-1.6中的稀疏稳定OCR头；并与通用VLM中独立识别的文本检索/复制头进行比较。还进行基座到专用模型的匹配比较。

Result: 在两个通用VLM中，视觉OCR头与文本检索/复制头高度重叠，未调优的top-20交集达73.3%，全头Spearman相关为0.677-0.886；OCR专用化后，每个任务top-20头中保留17-20个，全头秩相关为0.874-0.942，但功能与因果强度被重新分配。

Conclusion: 全序列OCR可理解为密集的序列化多模态复制粘贴：反复检索视觉证据并路由到当前输出位置。通用VLM转向OCR专家时，主要保留原有头身份而非重建新阅读电路，同时重新分配其功能与因果强度。
...
### [95] [GestureFAR: Streaming Co-Speech Gesture Generation with Flow Autoregression](https://arxiv.org/abs/2609.21576)
*Pinxin Liu,Haiyang Liu,Jiahao Luo,Junhua Huang,Chunhao Zou,Luchuan Song*

Main category: cs.CV

TL;DR: 提出 GestureFAR，一种面向流式共语手势生成的流自回归框架：在因果连续运动潜变量上自回归，并用逐 token 流匹配头采样，再通过 head-only 流蒸馏实现实时生成。


<details>
  <summary>Details</summary>
Motivation: 现有流式手势系统多依赖离散运动 token 自回归，虽能在线生成，但将高维连续运动压缩到有限码本，限制生成手势的真实性与多样性；因此需要同时保持因果性和连续表达力。

Method: 1）在因果连续运动潜变量上自回归，用 transformer 建模流式音频-运动上下文，并用逐 token 流匹配头从连续分布采样下一潜变量；2）提出 head-only 流蒸馏策略，冻结因果主干，将多步逐 token 流头蒸馏为单次网络评估，使用一致性目标与分布匹配目标，保持 token 因果性并消除实时交互的主要延迟瓶颈。

Result: 在 BEAT2 数据集上，GestureFAR 在支持流式生成的方法中显著改善质量-延迟权衡，在保持较强手势质量的同时实现实时 token 因果生成。

Conclusion: GestureFAR 通过连续潜变量自回归与流蒸馏，兼顾流式手势生成的因果性与连续表达力，并实现实时生成，有利于具身对话代理中的在线共语手势合成。
...
### [96] [Learned Parametric Emotion Editing: Real-Time Affective Filtering for On-Device Social Media Video](https://arxiv.org/abs/2609.21624)
*Musa Rochi,Marcel Schubert,Christoph Gebhardt*

Main category: cs.CV

TL;DR: 该论文提出一种通过调节视觉内容情感强度来干预问题性互联网使用的方法，并开发了可实时部署的轻量模型及安卓应用。


<details>
  <summary>Details</summary>
Motivation: 常见干预措施（如时间限制、屏蔽、强制休息）具有强迫性且易被规避，因此需要一种更温和、非限制性的替代方案。

Method: 使用MobileNetV4骨干网络结合FiLM情感条件化，单次前向传播输出可微全局变换参数，以替代此前每张图像80秒的迭代优化；并开展用户研究（N=54）及开发安卓应用实时适配Instagram视频。

Result: 模型单次前向传播仅需3.7毫秒；相比未编辑图像，降低了观看者报告的情绪唤醒度，效果与灰度健康滤镜相当，但感知质量评分更高；在三星Galaxy S23上实现60fps实时处理。

Conclusion: 通过学习单次前向传播的图像情感调节模型，可实现高效、低侵入性的问题性互联网使用干预，并具备实时移动端部署能力。
...
TL;DR: 本文介绍了4D-ImageNet数据集，包含174,000个4D-STEM衍射图样，其中145,000个实验数据和29,000个多切片模拟数据，并提供伪标签与基准测试，用于表示学习、盘检测、检索和模拟到实验研究。


<details>
  <summary>Details</summary>
Motivation: 4D-STEM在每个电子探针位置记录二维衍射图，能提供空间分辨的倒空间信息，但数据量大且异质，缺少标准化数据集和可复现基准。

Method: 构建4D-ImageNet：从29次采集选取145,000个实验图样，覆盖Ag、Au、Au-Ag混合、CoO、Pd和ZnO等样品，包含视场、扫描尺寸、相机长度和曝光时间等元数据；每采集5,000个按质量排序图样，提供扫描坐标。另含29,000个多切片模拟图样，覆盖13种晶体结构，含Euler旋转、倒空间采样和近似低指数束方向。使用集合预测检测器生成直接束位置和Bragg盘中心伪标签及置信度。还提供分组混合域掩码重建基准，用于防泄漏加载和跨实验/模拟评估。

Result: 数据集共174,000个衍射图样，实验数据来自29次采集并带有采集级标签和元数据，模拟数据覆盖13种晶体结构。伪标签包括直接束位置、Bragg盘中心和逐盘置信度。基准支持实验与模拟数据间的混合域评估。

Conclusion: 4D-ImageNet面向表示学习、盘检测、衍射图样检索、取向分析以及模拟到实验研究，旨在为4D-STEM数据分析提供大规模、带标注和可基准测试的资源。
...
TL;DR: 提出 HAT（Hypothesis-Anchored Tracking）因果框架，用于非合作目标的单目 6-DoF 位姿估计；通过帧间运动在多个基于 CAD 的位姿假设中选择锚点，结合单目 SLAM 与稀疏锚点/位姿融合，提升精度与速度，且无需目标特定训练。


<details>
  <summary>Details</summary>
Motivation: 在轨服务与空间碎片清除需要非合作目标的单目 6-DoF 位姿估计。单图像估计器容易混淆近似对称的航天器朝向，而跟踪方法可能保持错误位姿。因此需要一种利用帧间运动来消除歧义并保持正确位姿的框架。

Method: HAT 在配准与融合前，利用帧间运动在竞争性 CAD 位姿假设中进行选择；保留多个竞争朝向历史，并选择位姿来锚定单目 SLAM 估计的相对轨迹。采用稀疏锚点和位姿融合，在初始化后提供逐帧估计且不修改过去输出。只需标定 RGB 序列、公制 CAD 模型和目标图像区域（可由检测或分割提供），预训练位姿与 SLAM 网络无需目标特定训练或微调。

Result: 评估 Mega-HAT 与 Pico-HAT（基于 MegaPose 和 PicoPose），在 SPARK-2024、SwissCube、SHIRT 及 YCB-Video 上测试。四种数据集比较的算术平均显示：Mega-HAT 相对独立 MegaPose 平均位姿误差降低 9.4%，持续输入 FPS 为 3.76 倍；Pico-HAT 相对独立 PicoPose 平均位姿误差降低 23.9%，FPS 为 2.42 倍。SPARK 消融与离线参考分析考察了各组件贡献及修正过去估计的影响。

Conclusion: HAT 能有效利用时序信息解决近对称目标位姿歧义，提高非合作空间目标位姿估计的精度与效率，并避免目标特定训练；在空间领域外也具一定泛化能力。
...
### [99] [Detection is solved, delineation is not: what governs tooth segmentation on panoramic radiographs](https://arxiv.org/abs/2609.21628)
*Muhammad Rehan,Moaz Amjad,Syed Danial Ahmed,Mariam Adnan,Haider Ali*

Main category: cs.CV

TL;DR: 该研究构建了包含1,422张全景片、42,142个专家标注牙齿多边形和32类FDI编号的大型数据集，在统一协议下系统评估分辨率、模型架构和解剖先验对牙齿分割与编号的影响。


<details>
  <summary>Details</summary>
Motivation: 自动牙齿分割和FDI编号是计算机辅助牙科诊断的基础，但哪些因素真正决定性能仍不明确，因此需要受控实验来识别关键瓶颈。

Method: 使用30名牙科从业者标注、2名独立审核者复核的数据集；在统一评估协议下进行640/1024/1280分辨率消融、不同架构对比，以及LoRA自监督编码器、可提示基础分割模型、全局最优解剖标签分配等干预；并开展无重叠的多中心零样本迁移评估。

Result: 分辨率主导性能：mask mAP50-95从0.656升至0.710再到0.717，而mAP50稳定在约0.982；架构影响很小，查询式Transformer与单阶段检测器统计等价，但CPU慢5.5倍且ONNX运行受限；三种干预均失败。零样本迁移使mask mAP50-95下降62%，但mAP50仅下降18%，误差主要位于牙根尖三分之一。

Conclusion: 边界精度是主要约束，提升性能应优先关注输入分辨率和采集多样性，而非模型架构创新。
...
### [100] [Extending Decoupled Attention to Dense Prediction and Masked Training for Multi-Channel Images](https://arxiv.org/abs/2609.21629)
*Umar Marikkar,Sameed Husain,Muhammad Awais,Sara Atito*

Main category: cs.CV

TL;DR: 针对多通道成像（MCI）数据，现有多通道ViT将各通道token拼接后进行全局自注意力会稀释单通道特征。本文改进Decoupled Vision Transformer，通过通道内/通道间解耦注意力，并用线性分配恢复独立逐通道掩码下的空间对应，从而兼容标准掩码多通道训练，在多个分类与分割基准上超越最强MC-ViT基线。


<details>
  <summary>Details</summary>
Motivation: MCI数据不同于自然图像，每个通道记录语义独立的信号而非颜色波段。MC-ViT虽可处理可变通道数，但无限制的跨通道自注意力会稀释各通道特征；DC-ViT虽通过解耦注意力缓解该问题，却按空间位置配对token，要求每个通道具有相同的可见token，难以适配独立逐通道掩码训练。

Method: 分离通道内更新与跨通道更新，并在通道合并前为每个通道形成独立表示；针对独立逐通道掩码导致各通道保留patch不一致的问题，通过求解各通道保留patch之间的线性分配来恢复对应关系，从而将解耦注意力与标准配置的掩码多通道训练结合。

Result: 在荧光显微、成像质谱流式和卫星成像三个领域，共3个分类和3个分割基准上，包括高通道数密集预测任务，所提方法均优于最强的MC-ViT基线。

Conclusion: 通过线性分配恢复独立逐通道掩码下的空间对应，该方法可在不限制掩码配置的情况下将解耦注意力用于标准掩码多通道训练，并有效提升多通道成像的分类与分割性能。
...
### [101] [DRT: Dense Reasoning Trace for Efficient and Grounded Multimodal Reasoning](https://arxiv.org/abs/2609.21675)
*Wan Xu,Yuanfan Guo,Kevin Han,LaLa Chen,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 该论文提出 Dense Reasoning Trace（DRT）范式，用紧凑的结构化推理轨迹替代自然语言 Chain-of-Thought，以降低语言冗余并增强视觉 grounding，从而提升多模态大模型推理的效率与准确性。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 的 CoT 范式局限于自然语言表达空间，导致过度的语言开销、信息稀释和较弱的视觉 grounding，因此需要更高效且更贴近视觉证据的推理表达方式。

Method: 提出 DRT 范式：用包含简洁中间状态、符号连接词的结构化轨迹表达推理，并分离视觉观察与逻辑推导；引入 Dense Trace Initialization 将 DRT 推理模式内化到模型中；进一步提出 Trace-Grounded Reinforcement Learning 框架，通过三视角验证流程构建参考轨迹，并采用 Trace-Grounded GRPO 与结构化奖励进行训练。

Result: 在具有挑战性的推理基准上，DRT 相比 Qwen3-VL baseline 实现了 5.5 倍 token 效率提升，同时准确率提高 1.3 个百分点，并减少了幻觉、增强了逻辑 grounding。

Conclusion: 复杂多模态推理可能并不需要冗长的自然语言推理轨迹；DRT 为下一代 MLLM 提供了更高效、更可靠的推理路径。
...
TL;DR: 研究基于Farmer.Chat约116万张农田照片，将农作咨询流程拆为质量门、作物检测、病害/虫害诊断三阶段，并比较单一大视觉语言模型与小型专家模型方案。


<details>
  <summary>Details</summary>
Motivation: 现有生产系统不可调整：没有照片拒收阈值，无法新增作物和问题，也没有置信度截断；同时质量门拒图多、诊断常缺作物名、疾病标签中混入害虫，需改进。

Method: 分析埃塞俄比亚、印度、肯尼亚和尼日利亚约116万张照片，分为M0质量门、M1作物检测、M2病害/虫害检测。Route A用Qwen3-VL-4B单次调用完成三阶段；Route B用DaViT、YOLO26等小型专家模型；并用MobileNetV3替换GPT-4o质量门。

Result: MobileNetV3质量门达86.9% F1、12 ms；分层DaViT-Base作物准确率95.41%，高于生产基线91.46%；诊断领先且不拒答，而语言模型常留下无诊断记录。微调模型保留单次调用三阶段和请求更好照片的能力。

Conclusion: 小型专家模型在质量门和作物识别上高效准确，分层方案诊断强且不拒答；微调视觉语言模型具备统一调用与主动要求重拍能力，可替代不可调的生产系统并支持按需调整。
...
### [103] [SignGPT: Toward LLM-Mediated Sign Language Interaction through Gloss-Free Translation and Generation](https://arxiv.org/abs/2609.21709)
*Ronghui Li,Jun Dong,Zhongyuan Hu,Zunnan Xu,Jun Zhou,Liyuan Chen,Shuoling Liu,Jiangpeng Yan,Jie Guo,Xiu Li,Linchao Bao*

Main category: cs.CV

TL;DR: 论文提出 SignGPT，一个统一的、基于姿态的无 gloss 手语翻译与生成框架，可将手语同时作为输入和输出，并在 ASL 和 DGS 数据集上评估，探索其支持手语对话的潜力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型对手语交互支持有限，手语翻译与生成通常依赖分离模型，导致手语-文本交互中频繁切换模型。研究希望统一手语翻译与生成，减少模型切换，推动手语对话应用。

Method: SignGPT 将身体、手部和面部运动的部位感知分层表示整合到共享语言模型中，采用非对称多 token 预测和渐进式训练进行双向建模，实现无 gloss 的手语翻译和生成。

Result: 在 How2Sign（ASL）和 Phoenix-2014T（DGS）上进行基准比较、定性分析和组件消融；并用 12 名聋人 ASL 手语者进行探索性研究，评估 LLM 介导的手语到手语响应流程。

Conclusion: 统一建模在支持手语翻译与生成方面具有效果，并展现出支持手语对话的潜力。
...
### [104] [Balanced Prompt Adaptation against Entropy-Induced Collapse for Test-Time Binary Segmentation](https://arxiv.org/abs/2609.21743)
*Zhengshan Wang,Joshua Charles Webster-Ford,Yifei Tian,Xinxin Wang,Long Chen,Weiping Ding*

Main category: cs.CV

TL;DR: 该论文针对测试时自适应中熵最小化在类别不平衡二值分割任务中容易崩溃的问题，提出了 Balanced-Anchor Prompt Adaptation（BAPA）方法，并在多个领域取得最高平均 Dice。


<details>
  <summary>Details</summary>
Motivation: 熵最小化是测试时自适应的常用目标，但在不平衡二值分割中，大量像素预测会让较大预测类主导更新，把少数类预测拉向自身，最终导致掩码退化且熵梯度消失。

Method: 论文在共享偏移模型下从理论上分析了这种崩溃现象，并提出 BAPA。BAPA 包含两个互补模块：Class-Balanced Anchors（CBA）分别从每个预测类中选择高置信锚点，并给前景和背景相同总损失权重；Dynamic Prompt Adaptation（DPA）在每次预测更新后刷新锚点，仅优化文本侧 prompt 残差，同时冻结视觉语言编码器。

Result: 在四个领域的实验中，BAPA 在评估方法中取得了最高平均 Dice。因子消融实验进一步验证了 CBA 和 DPA 的互补作用。

Conclusion: 平衡的 prompt 自适应可以作为测试时二值分割中熵最小化的有效替代方案。
...
### [105] [Diffusion-Based Tumor Inpainting for Renal Segmentation under Clinical Data Scarcity](https://arxiv.org/abs/2609.21698)
*Ekaterina Sedykh,Salme Ussanov,Dmytro Fedorenko,Dmytro Fishman*

Main category: cs.CV

TL;DR: 提出基于扩散修复的肾脏肿瘤合成框架，在健康CT中生成解剖合理的肿瘤，无需额外标注；并首次系统比较2D、2.5D与全3D合成策略用于低数据肾脏肿瘤分割。


<details>
  <summary>Details</summary>
Motivation: 深度学习肾脏肿瘤分割依赖大规模标注数据，但临床部署中目标站点通常只有少量肿瘤阳性病例。

Method: 使用扩散模型在公开数据KiTS23和KIRC上训练，在健康CT中合成肿瘤；在内部队列的三种低数据设置下，用nnU-Net评估2D、2.5D和全3D（MAISI）合成增强策略。

Result: 2.5D和3D增强将假阳性从约18-20%显著降至约3-6%，同时保持Dice；2D无一致收益；2.5D在所有指标上匹配全3D且计算成本显著更低。

Conclusion: 局部体积一致性已足以实现有效数据增强，适合数据和资源稀缺的临床场景。
...
TL;DR: ZYT-World 是一个面向生产部署的生成式世界模型，可原生生成混合鱼眼-针孔多相机视图，并通过一步流式生成、长时稳定与隐式记忆机制支持闭环仿真。


<details>
  <summary>Details</summary>
Motivation: 现有生成式世界模型在生产部署中面临三个未解决问题：在原生分辨率下忠实复现混合鱼眼-针孔相机系统；同时满足因果逐步交互、长时稳定和低延迟；以及在重访同一地点时保持场景身份。

Method: 提出单一架构 ZYT-World，原生生成四个视场角大于180°的鱼眼视图和三个针孔视图。使用投影特定的 Plucker 适配器编码相机几何，自车运动自适应层归一化提供全局运动控制，轻量像素对齐布局通过实例框、朝向和颜色条件化交通参与者与信号。采用异构训练结合全 rig 几何覆盖与高分辨率细节。通过教师强制、因果一致性蒸馏、自 rollout 分布匹配蒸馏和 RigCritic，将40步双向教师模型转化为一步、按潜变量流式的生成器，并由 RigCritic 联合评估七视图 rig。引入19M参数 TinyVAE 解码器、W8A8 量化和推理引擎降低解码、骨干和增量执行成本。最后，用真实采集的跨轨迹对训练插件式隐式记忆模块以保留地点特定证据。

Result: 在内部多视图测试集上，一步模型保留教师模型超过90%的 PSNR 和 SSIM，FID、FVD 和 LPIPS 保持在教师模型的11%以内。按图2的仅生成器计时，比40步双向教师模型快107.7倍。TinyVAE 解码比 Wan 快59.8倍。30秒 rollout 和跨轨迹重访展示了预期的长时程和记忆行为。

Conclusion: ZYT-World 通过一体化架构、蒸馏流式生成、量化与推理优化以及隐式记忆模块，实现了高效的原生多视图生成，并满足长时稳定、低延迟和重访场景身份保持的生产部署需求。
...
### [107] [SFVO: Decoupled Confidence-Guided Stereo-Flow Visual Odometry with Bidirectional PnP](https://arxiv.org/abs/2609.21754)
*Kai Zhang,Guoyang Zhao,Jun Ma*

Main category: cs.CV

TL;DR: 提出SFVO，一个对应关系驱动的双目视觉里程计框架，基于预训练双目匹配和光流模型，将对应关系转化为几何约束并预测可信点，引入旋转与平移解耦置信图，在室内外数据集上实现鲁棒准确的位姿估计。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习视觉里程计多采用单目方法，存在尺度模糊；双目视觉里程计天然提供真实尺度，但因计算成本高和建模复杂而研究较少。同时，预训练双目匹配与光流模型已能提供稠密可靠对应，其互补几何信息在视觉里程计中尚未被充分利用。

Method: SFVO利用预训练双目匹配和光流模型估计双目及时间对应关系；不直接从图像学习位姿，而是将学习到的对应关系映射为几何约束，并预测哪些点可信。为提升基于视觉对应几何约束的可靠性，引入分别针对旋转和平移的解耦置信图，以更好匹配视觉对应与6自由度变换特性。

Result: 在室外和室内数据集上的大量实验表明，SFVO能够实现鲁棒且准确的位姿估计，并具有较强泛化能力。

Conclusion: SFVO表明，通过对应关系驱动、几何约束建模以及旋转和平移解耦置信度设计，可有效提升双目深度视觉里程计的可靠性、精度和泛化能力，为双目视觉里程计提供了一种有效框架。
...
### [108] [Benchmarking the Explanatory Quality of Open-Weight Vision-Language Models in Face Recognition](https://arxiv.org/abs/2609.21879)
*Laurent Colbois,Sébastien Marcel*

Main category: cs.CV

TL;DR: 本文提出一个面向视觉语言模型（VLM）人脸识别的基准框架，将解释质量作为核心评价维度，并联合评估人脸验证准确率与解释质量。


<details>
  <summary>Details</summary>
Motivation: VLM 可用于人脸识别并生成自然语言解释，这对需要透明与可审计的司法场景很有吸引力，但现有评估多关注识别准确率，未量化生成解释的有效性。

Method: 提出解释应满足的两个标准：相关性（依赖身份稳定的面部特征）和忠实性（与可见图像内容一致、无幻觉特征）；设计结构化解释格式以支持自动化查询与审计，并基于此量化评估多个开放权重 VLM 家族的人脸验证准确率与解释质量。

Result: 基准测试显示，当前 VLM 生成的解释仍存在不足，仅看识别准确率无法全面反映模型性能，解释质量指标十分必要。

Conclusion: 该基准与开源评估工具为可解释人脸识别系统的规范评测和后续微调提供了基础。
...
TL;DR: 该研究审计三种医学视觉语言模型和一种通用模型在胸片结核筛查中的可移植性，发现模型排名、评分可靠性和筛查阈值表现会随队列、提示词、阴性对照、患病率和操作阈值变化，基准分数不能保证临床可移植。


<details>
  <summary>Details</summary>
Motivation: 医学模型的基准分数并不能证明在不同评估设定下结论仍然成立，因此需要检验模型排名、评分可靠性和筛查性能是否能在多种评估变化下保持稳定。

Method: 审计 BioMedCLIP、CheXficient、MedSigLIP 和 OpenCLIP，在 Montgomery、Shenzhen、TBX11K、VinDr-CXR 四个数据集的 12,200 份胸片记录上评估；使用五个固定提示词家族生成 244,000 个模型-图像-提示词评分，并改变队列、阴性谱、指定患病率和操作阈值，进行多重比较控制，同时比较监督源模型的外部表现。

Result: 没有模型在所有队列和可靠性标准中领先；提示词家族变化在 48 个多重控制比较中有 21 个改变 AUROC。将健康对照替换为患病非结核对照后，四个模型 AUROC 下降 0.075–0.306。在 VinDr-CXR 上，三个医学模型区分结核与无异常对照的能力明显好于区分肺炎或肺肿瘤，且对这两种命名疾病的 AUROC 点估计低于 0.5。CheXficient 有 VinDr-CXR 预训练暴露记录，限制其结果解释。TBX11K 训练集上按 95% 灵敏度选择的阈值，仅在 16 个目标评估中的 4 个按点估计保持该约束。五种子监督源模型在 TBX11K 验证集 AUROC 为 0.999，但在两个外部队列均为 0.629；保守排除感知重叠候选可缩小差距但未完全消除。

Conclusion: 这些回顾性、单任务结果表明，判别能力、评分可靠性和阈值保持支持不同的可移植性主张。胸片结核筛查证据应明确完整的评估规范，而不能仅凭一个检查点断言临床可移植性。
...
TL;DR: 论文提出XCalib，一种无监督密集热可见光配准框架，通过虚拟针孔相机参数化作为几何约束，优化相对位姿、内参和单目深度，实现跨模态对齐。引入归一化边缘相关（NEC）度量，在ADAS数据集上表现优于无约束密集流基线。


<details>
  <summary>Details</summary>
Motivation: ADAS中RGB与红外相机因光轴不重合和视场差异导致空间错位，产生非均匀视差和视觉重影；经典关键点方法受限于全局单应性，在动态深度下失效；无约束密集流方法缺乏结构正则化，时间稳定性差。

Method: 提出XCalib无监督密集热可见光配准框架。将虚拟针孔相机参数化作为几何约束空间，优化有效相对位姿和内参以及预测的单目度量深度，将空间位移搜索限制在物理有效的投影几何中。提出归一化边缘相关（NEC）作为跨光谱对齐的鲁棒结构相似性度量。

Result: 在公开ADAS数据集上进行广泛的定量和定性评估，表明与无约束密集流基线相比，XCalib具有更优的时间稳定性和对齐精度。

Conclusion: XCalib通过相机参数化作为隐式正则化器，有效解决了跨模态配准中的动态深度和时序不稳定问题，为热-可见光图像配准提供了新范式。
...
TL;DR: 提出PointLAM，一种高效且强大的点云3D检测架构，通过LPS和LHA等创新，在保持点方法几何保真优势的同时大幅提升效率，缓解体素方法与点方法之间的根本矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR 3D检测存在两难：体素方法效率高但会带来几何量化误差；点方法保留几何精度，却受限于慢速下采样（如FPS）、昂贵动态邻域查询（如k-NN）和复杂连续交互导致的严重计算瓶颈。

Method: 提出PointLAM，核心包括：Laplacian Point Sampler (LPS)，利用隐式离散拉普拉斯高通滤波和Doubly Sorted Sampling实现快速、结构感知的前景保留；Local Hadamard Aggregator (LHA)，通过瞬态网格解耦空间索引与特征表示，并用Hadamard Gating替代复杂连续交互，实现拓扑感知的注意力调制；再结合Bi-Directional Mamba (BDM)进行全局序列建模，构成Local Attentive Mamba (LAM) block。

Result: 在nuScenes和Waymo数据集上，PointLAM作为点检测器取得有竞争力的性能，可媲美高度优化的体素检测器，同时计算开销仅为其一小部分，并在小目标检测和极端稀疏场景下表现更优。

Conclusion: PointLAM有效解决了点基架构中的系统性低效问题，证明高效点方法能够在性能上竞争体素方法，同时显著降低计算成本，并在困难场景中展现优势。
...
TL;DR: 本文将无监督异常检测重新表述为贝叶斯逆问题，目标是推断导致每个观测异常的最可能损坏机制，并据此给出基于推断损坏参数能量的概率异常分数。


<details>
  <summary>Details</summary>
Motivation: 传统无监督异常检测只关注是否偏离正常分布或定位异常区域，而忽略异常背后的生成机制；但异常的性质往往与其存在同样重要。

Method: 提出一个贝叶斯逆问题框架，将异常检测转化为推断最可能的损坏参数；以推断出的损坏参数能量作为异常分数，并说明若干现有方法可在不同建模选择下视为该框架的特例。

Result: 在受控实验中分析框架各组件；通过调整损坏模型，在 MVTec AD 数据集上使 object-class AUROC 提升 2.3%；在脑部 MRI 基准上实现较强检测性能，同时估计病理强度、偏置和几何信息。

Conclusion: 该框架为无监督异常检测提供了原理性通用思路，能够统一现有方法、提升检测性能，并输出更具解释性的异常生成机制估计。
...
TL;DR: 本文提出 VideoReloc，面向长期室内视频重定位，在光照与家具变化下，用自适应视频片段、假设优先配准和方向感知优化，在紧凑语义地图上实现稳定定位。


<details>
  <summary>Details</summary>
Motivation: 视觉方法依赖外观，在长期环境变化下不可靠；仅用单帧的对象类别与几何进行定位又证据稀疏且歧义大。

Method: 使用里程计自适应地收集视频片段的空间证据，直到满足对象和运动准则；在片段级联层面复查冲突位姿；假设优先配准由对象三元组提出位姿，并用片段内对象中心与边界框表面验证；方向感知细化利用框面、重力和墙面方向解决相机朝向歧义并优化完整位姿。

Result: 在 RIO10 和 ReplicaCAD 上，因果评估下 1 m/10° 全帧定位成功率分别为 73.5% 和 61.1%，结合片段闭合后升至 90.6% 和 74.8%；对比的逐帧场景坐标回归器最高为 47.6% 和 49.8%，且地图大小为 12.6–42 MB，而本方法地图约 100 kB。

Conclusion: 该方法将稀疏地图重定位重构为对空间扩展视频查询的验证，把判别信息从存储外观转移到时间上下文，并支持仅含类别标注框的 100 kB 紧凑地图。
...
TL;DR: 论文指出现有目标检测基准受标注不完整限制，重新标注四大数据集发现大量漏标，并构建高召回、不确定性感知的标注流程与两个新基准，表明检测器性能高度依赖标注质量且与人类感知不一致。


<details>
  <summary>Details</summary>
Motivation: 尽管检测架构与开放词汇模型进步，基准质量仍受标注不完整制约；需量化漏标影响并提升数据质量。

Method: 对COCO、Pascal VOC、Cityscapes、KITTI重新标注；提出可扩展高召回标注流程，每对象至少11名标注者聚合软标签；构建不确定性感知检测基准和基于真实标注错误的标签错误检测基准；评估现有检测器与标签错误检测方法。

Result: 重新标注使标注对象显著增加（KITTI +60%，COCO +40%），主要来自小、遮挡、密集实例；漏标是主要标签错误来源；基准性能对标注质量高度敏感，但模型排名基本稳定；现有检测器高度依赖标注质量并与人类感知不一致；现有标签错误检测方法在真实错误上召回和精确率低。

Conclusion: 未来目标检测基准应超越确定性标注，转向高召回、不确定性感知评估，以最大化有效实例并反映现实模糊性。
...
TL;DR: 本文提出一种形态感知的歧义学习框架，用于晶圆图缺陷识别，支持自动单类诊断、两类辅助诊断和全量复核三种决策，并在WM-811K上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法将晶圆图缺陷识别视为固定分类任务，强制每个晶圆仅属于单一缺陷类别；但对于形态接近类别边界的晶圆，单一预测信息不足，提供合理的候选诊断类别更有价值。

Method: 利用训练晶圆图的径向、角度和几何特征构建类别级歧义矩阵，表示形态相似且可能互为诊断替代的缺陷类别对；该矩阵引导模型学习合理的替代类别，而非同等对待所有错误类别；推理时根据矩阵判断不确定预测能否表示为有意义的双类诊断集合，否则升级为全量复核。

Result: 在WM-811K数据集上，该框架在缺陷识别和诊断决策支持方面优于传统方法，能提供有意义的双类替代诊断，并将全量复核保留给未解决歧义的案例；示例性成本分析显示所提路由策略具有潜在成本优势，且诊断行为在不同骨干网络上保持一致。

Conclusion: 所提形态感知歧义学习框架能够有效处理晶圆图缺陷识别中的类别边界歧义，通过单类诊断、双类辅助诊断和全量复核的分级决策提升诊断支持能力，并展现出跨架构的稳定性和潜在成本优势。
...
TL;DR: 提出 Chronosphere，一种用于气候表征学习的时空神经场，通过自适应时空剖分与共享局部基函数，在空间和时间上动态分配表征能力。


<details>
  <summary>Details</summary>
Motivation: 地理表征学习需建模空间与时间复杂度差异很大的环境过程，但现有位置编码器通常在全局固定细节水平：球谐等全局基均匀分配容量，局部基只覆盖预定义区域，学习型剖分虽能自适应却难以高效表达高频信息。

Method: Chronosphere 将时空环面 S^2×S^1 上的可学习站点自适应剖分与共享局部基函数库结合，使容量放置位置和每个区域的细节程度都能随数据在空间和时间上自适应调整，并以气候态重建为训练目标。

Result: 在多种空间与时间任务上，Chronosphere 匹配或超越当前最先进的位置编码器，尤其在与空间和时间迁移相关的任务中提升最大。

Conclusion: Chronosphere 统一了全局、局部与学习型剖分方法，为气候等复杂时空过程提供了更灵活高效的自适应表征学习框架。
...
### [117] [Catena: A Comprehensive Software Suite for Large-Scale Connectomics](https://arxiv.org/abs/2609.21887)
...
TL;DR: Catena 是一个开源、面向开发者的连接组学软件套件，集成神经元与细胞器分割、突触检测、微管追踪和神经递质推断等模块，通过可组合的分块流水线、预训练模型和容器化运行环境，实现从电子显微镜体积中可重复、可扩展地绘制细胞连接组。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜体积是连接组绘制的金标准数据，但现有重建和校对神经元分支及标注突触的流程依赖多个软件工具，这些工具往往碎片化、维护不一致或专有，阻碍了可重复性和自动化。

Method: 提出 Catena 软件套件，集成 3D 神经元和细胞器分割、突触检测、微管追踪、神经递质推断等模块；采用可组合、分块式处理流水线，完全文档化、可扩展、可适配；使用预训练机器学习模型降低计算和真实标注数据需求，并支持微调；提供完全容器化的模块，封装依赖以保证在工作站和集群上执行一致。

Result: Catena 通过整合开放组件、可共享模型和容器化运行时，降低了计算与真实标注数据需求，并能在不同计算环境中一致执行，提供了一种可重复且可扩展的电子显微镜连接组映射方法。

Conclusion: Catena 以开源、模块化、容器化和可微调的设计，克服了连接组学工具碎片化、专有化带来的可重复性与自动化障碍，为从电子显微镜体积中规模化映射细胞连接组提供了实用方案。
...
### [118] [The Role of Radiometric Features in Cross-Site Leaf-Wood Segmentation of LiDAR Point Clouds](https://arxiv.org/abs/2609.21903)
*Roman Kaharlytskyi,Derek T. Robinson,Roberto Guglielmi*

Main category: cs.CV

TL;DR: 本文研究LiDAR点云中单木枝叶分割的跨站点与跨平台泛化问题，发现仅依赖几何特征的方法难以泛化到稀疏航空扫描，而引入辐射特征可提升性能并保持结构连通性。


<details>
  <summary>Details</summary>
Motivation: 现有枝叶分割方法通常排除强度、回波次数等辐射特征，以最大化跨传感器兼容性。作者质疑这一设计选择，并评估其在跨站点、跨平台场景下的泛化能力。

Method: 在公开Heidelberg数据集（地面TLS，1550nm）上训练，在加拿大安大略省新数据集（RPA-LS，905nm）上测试；比较仅几何方法（含在高密度LiDAR上训练的SOTA深度学习模型）与加入辐射特征（强度、回波次数、回波数量）的方法。

Result: 仅几何方法无法泛化到稀疏、俯视的航空扫描，F1分数不超过0.56；加入辐射特征后F1提升至0.61，木材召回率从0.16提高到0.35，提升119%。仅几何方法常导致树干和树枝碎片化，而辐射特征有助于保持结构连通性，生成更连贯、更适合QSM重建的架构。

Conclusion: 几何模式依赖视角且易过拟合扫描模式，而辐射特征编码了材料物理属性，能在不同传感器和环境间泛化，因此应被纳入跨平台枝叶分割。
...
### [119] [Info3R: Information-Adaptive Test-Time Training for 3D Reconstruction](https://arxiv.org/abs/2609.21938)
*Sunghyun Baek,Hanna Bae,Minchan Kwon,Junmo Kim*

Main category: cs.CV

TL;DR: Info3R 是一种面向在线三维重建的信息自适应测试时训练方法，通过根据新帧的信息量调整状态更新，并在状态饱和时动态重置，从而提升长视频流下的重建鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于 Transformer 的在线三维重建方法在处理长图像流时，忽略了两个关键信号：每个新到来帧的重要性，以及模型内部状态的信息饱和问题，导致长序列性能下降。

Method: 提出信息感知的状态更新，根据每帧的冗余性和信息量调节状态更新强度；同时提出动态状态重置，由状态更新的累计幅度和模型预测置信度触发，并辅以锚点到世界坐标的对齐。

Result: 在相机位姿估计、视频深度估计和三维重建任务上取得一致提升，并显著缓解长序列评估中的性能退化；在 KITTI Odometry 上，平均 ATE 比 LongStream 低 1.68 倍。

Conclusion: Info3R 能有效提升在线长序列三维重建的鲁棒性，尤其适用于扩展的户外序列场景。
...
### [120] [PRIME: Perception Feedback with Situational Memory Embeddings in VLA Models](https://arxiv.org/abs/2609.22040)
*Erik Deinzer,Naya Baslan,Luca Paparusso,Narunas Vaskevicius,Peter Knott,Luigi Palmieri*

Main category: cs.CV

TL;DR: 论文提出PRIME，一种将VLA感知查询条件化于情境记忆的学习式反馈机制，用于自动驾驶闭环决策，并在Bench2Drive上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶VLA模型主要采用感知—推理—规划的前馈推理，早期感知对下游推理和导航目标不可见，无法根据先前决策优先关注相关视觉线索。

Method: PRIME通过交叉注意力聚合过去L步的感知、推理、导航目标和预测行为的潜在表示，形成情境记忆，并以此调制VLA感知查询，实现意图驱动的感知注意力；仅增加最多29.7M参数，占7.3B基础模型的0.41%。

Result: 在Bench2Drive闭环基准上，PRIME取得82.47的驾驶得分，比ORION高4.73；成功率为60.00%，提升5.38个百分点，是已发表且基于Think2Drive演示训练的VLA中报告的最高驾驶得分。

Conclusion: 学习式反馈机制能以极小计算成本实现意图驱动感知，弥合早期感知与高层推理、导航目标之间的差距，从而提升闭环自动驾驶性能。
...
### [121] [Traffic Sign Recognition for Autonomous Driving Using Branched YOLOv2 and Geometric Features](https://arxiv.org/abs/2609.22060)
*Arefeh Rezaei*

Main category: cs.CV

TL;DR: 本文提出一种基于YOLOv2的交通标志识别系统，可同时完成检测与分类，并通过分支式中间预测层和几何模板验证来提升效率与准确率。


<details>
  <summary>Details</summary>
Motivation: 交通标志识别需高效定位并分类标志；YOLOv2基线在简单样本上仍计算冗余，且视觉相似标志易被误分类，因此需要加速推理并降低分类错误。

Method: 在YOLOv2中加入中间预测层形成分支架构，支持整图分支和单元级分支以提前终止推理；引入无监督贝叶斯图像分割生成二值表示，并与类别相关几何模板在YOLOv2边界框内比较，用于推理或训练；构建由GTSDB和GTSRB通过无缝克隆及受控变换合成的专用数据集，覆盖10类、3000训练和300测试样本。

Result: 所选分支架构运行时间0.647 s、mAP 0.680，基线YOLOv2为0.6607 s、mAP 0.680；推理时加入几何验证将mAP提升至0.713；训练时加入几何特征变体达到mAP 0.697，运行时间0.6608 s。

Conclusion: 分支架构可在保持基线mAP的同时略微缩短推理时间；几何信息能有效减少相似标志误分类，其中推理阶段几何验证对mAP提升最明显，训练阶段引入几何特征也有一定增益。
...
### [122] [OmniVBench: A Benchmark and Large-Scale Dataset for Omni Reference-to-Video Generation](https://arxiv.org/abs/2609.22069)
...
TL;DR: 本文提出OmniVBench评估基准和Omni-R2V数据集，用于评估和训练全能参考到视频（R2V）生成模型，覆盖7个任务族和18个细粒度任务，并引入基于因素的评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有R2V基准测试的参考类型和组合有限，评估协议主要关注整体参考一致性，忽视了参考因素是否被正确保留、解耦和路由。同时，构建全能R2V训练数据成本高昂，导致训练资源稀缺。

Method: 提出OmniVBench，涵盖内容、运动、风格、结构、叙事和多参考等7个任务族、18个细粒度任务，并采用基于因素的评估，包含12,172个案例特定的检查项。同时构建Omni-R2V数据集，从专业视频素材中处理得到34万训练样本，并开发任务特定管道用于参考-目标对构建。

Result: 对先进的开源和闭源R2V模型进行广泛评估，发现它们在OmniVBench的不同任务族和评估维度上存在明显性能差距。

Conclusion: 当前R2V模型仍存在局限性，OmniVBench和Omni-R2V数据集为评估和训练全能R2V模型提供了基准和资源。
...
### [123] [MintAct: A Unified Visual Agent for Digital Environments](https://arxiv.org/abs/2609.22083)
...
TL;DR: 提出MintAct系列视觉语言模型，统一UI定位、多步导航与视觉工具使用，在2B/4B/8B规模上训练，性能匹配领域专用模型，并在OSWorld-Verified上取得48.9的SOTA成绩。


<details>
  <summary>Details</summary>
Motivation: 现有UI智能体多为领域专用模型，缺乏统一处理移动端、桌面、网页导航与工具使用的能力，且缺少可扩展的环境与强化学习基础设施来支持跨域训练。

Method: 构建支持数百并发异构后端实例的环境，用于轨迹数据收集与在线强化学习；采用异步RL框架，显式控制跨域训练分布，并在噪声环境反馈与离策略漂移下保持稳定。

Result: MintAct在多个基准上匹配各领域专用模型，同等模型规模下达到最先进性能，OSWorld-Verified得分为48.9。

Conclusion: 通过精心设计环境、数据与训练方案，统一的视觉语言模型能够在UI定位、多步导航和视觉工具使用上达到SOTA水平。
...
### [124] [Do small language models know what they don't know?](https://arxiv.org/abs/2609.20824)
*Prashant Mudgal*

Main category: cs.CL

TL;DR: 研究探讨熵基置信信号能否提升小于30亿参数的小语言模型在消费级硬件上的准确率，发现token级熵无效，而语义熵可用于路由至更大专家模型，实现显著准确率提升。


<details>
  <summary>Details</summary>
Motivation: 探索在消费级硬件上运行的小语言模型（<3B）中，熵基置信信号能否用于提升准确率。

Method: 评估七种方法，包括token级熵早停、语义熵估计、不确定性感知路由至更大专家模型，在7个模型对和5个NLU基准上测试。

Result: token级熵在SLM中近乎无效（91%组合均值近零），语义熵可恢复有效置信信号；路由不确定查询至更大专家模型准确率提升最高+50个百分点；跨家族路由平均+22.0%，同家族+6.8%，专家模型质量比架构兼容性更重要。

Conclusion: 熵基方法在SLM中的价值不在于计算节省，而在于智能计算分配：在最需要的地方投入更多token。
...
### [125] [HERMES: Contrast-Aware Knowledge Graph Reasoning from Clinical Notes for Patient Outcome Prediction](https://arxiv.org/abs/2609.20825)
*Gia-Bach Nguyen,Hoang-Ha Nguyen,Tuan-Cuong Vuong,Trang Mai Xuan,Duy Quoc Ngo,Tien-Cuong Nguyen,Huan Vu,Thien Van Luong*

Main category: cs.CL

TL;DR: 本文提出HERMES，一个仅基于临床文本的图框架，通过构建个性化知识图谱并利用图注意力网络学习患者表示，用于临床预测任务，并在MIMIC-III/IV上优于纯文本基线。


<details>
  <summary>Details</summary>
Motivation: 现有临床预测模型多依赖结构化电子健康记录，或虽使用非结构化临床笔记但将其编码为扁平序列，可能丢失临床叙述中的显式关系和时间结构。

Method: HERMES包含两个核心：一是利用大语言模型引导从临床笔记中抽取个性化知识图谱，并结合对比逻辑建模显式捕捉时间动态、治疗失败及结局变化；二是使用图注意力网络在知识图谱上进行图学习，合成患者表示。

Result: 在MIMIC-III和MIMIC-IV的院内死亡率和30天再入院预测任务上，HERMES持续优于强纯文本基线。

Conclusion: 研究表明，通过对比逻辑建模进行显式关系建模能显著提升临床预测性能。
...
### [126] [TALON: A Temporally Aware Longitudinal Framework for Radiology Report Generation](https://arxiv.org/abs/2609.20826)
*Nien-Tsyr Sun,Min-Chen Chen,Hui Nien Hung,Vincent S. Tseng*

Main category: cs.CL

TL;DR: 提出TALON，一种时间感知的长纵向放射报告生成框架，通过双通道时序融合模块自适应整合变长病史，在MIMIC-CXR上超越当前最优方法。


<details>
  <summary>Details</summary>
Motivation: 现有放射报告生成模型通常只基于单次检查或最近一次既往检查，难以进行准确的纵向比较并检测细微间隔变化；已有方法虽引入多次既往检查，但多聚合成固定长度历史，未在融合前显式建模各既往检查的角色依赖相关性。

Method: TALON框架包含双通道时序融合模块DCTFM：通过相似性和变化两个互补通道，将当前检查与每个既往检查比较，分别捕捉持续发现和间隔变化；采用通道特定注意力估计每个既往检查的相关性；并利用学习的既往特定门控自适应整合有效纵向证据、抑制冗余。

Result: 在MIMIC-CXR数据集上，TALON在多种临床效能和基于图的指标上优于当前最先进方法；当可用既往检查更多时，这些指标进一步提升。

Conclusion: TALON的DCTFM能有效建模纵向放射报告生成，并在比现有方法更长、更复杂的患者病史中展现优势。
...
### [127] [From Discharge Notes to Patient Understanding: Persona-Grounded, Open-Ended Simulation of LLMs as Discharge Educators](https://arxiv.org/abs/2609.20827)
*Won Seok Jang,Zonghai Yao,Hong Yu*

Main category: cs.CL

TL;DR: 提出DischargeBench，一个基于虚拟病人的模拟评估框架，用于评估LLM在医院出院教育中的表现，强调患者理解而非文本质量或答案准确性。


<details>
  <summary>Details</summary>
Motivation: 出院教育是互动教学任务，需临床医生根据患者的识字水平、记忆力和性格调整出院计划。现有LLM评估多针对静态或生成任务，无法衡量开放式对话中患者理解程度。

Method: 引入DischargeBench：候选LLM教育者与虚拟患者进行多轮对话，教育监测代理在不修改教育者的情况下调节患者真实性，保护评估信号。构建MIMIC-IV-Ext-DischargeBench，包含477个病例、24个ICD章节，并设置性格、教育水平、健康素养、既往病史回忆等人设轴。每个模拟由LLM-as-a-Judge从对话质量、主题清单、理解、事实一致性四个维度评分，并与医生标注对齐。

Result: 在闭源和开源LLM上，总体分数掩盖了ICD章节和患者人设间的临床相关变异；困难人设暴露了覆盖失败、理解差距以及源-答案一致性降低的问题。

Conclusion: 针对出院教育的LLM评估应聚焦患者理解，而不仅仅是文本质量或答案准确性。
...
### [128] [Beyond WER: Entity and Disfluency Recall in Accented Conversational ASR](https://arxiv.org/abs/2609.20828)
*Fiza Husain,Ankit Pandey,Yash Singh*

Main category: cs.CL

TL;DR: 提出一套三阶段 ASR 流水线，面向印度、印尼和拉美口音的会话英语，提升命名实体与填充停顿识别，同时保持较低 WER，以支持语言学习反馈。


<details>
  <summary>Details</summary>
Motivation: 优化 WER 的 ASR 系统常遗漏命名实体和填充停顿，而这两类信息对口音会话英语的语言学习反馈至关重要。

Method: 采用三阶段流程：(1) 用启发式 SQL 过滤筛选实体丰富训练数据，实体密度为随机采样的 2.8 倍；(2) 基于 Qwen2.5-Omni-3B 微调区域 LoRA 适配器，一次前向生成逐字与纠正转写；(3) 构建六类错误分类法，并用 LLM 评判验证，与 210 个人工标注样本达到 83.8% 一致性。

Result: 在 6k 条测试话语上，实体召回从 53-55% 提升至 80-85%，填充词召回从 <5% 提升至 76-86%，WER 为 6-10%；实体召回优于 Whisper 和商用 ASR，并以少 10 倍参数匹配零样本 30B 模型；配对 bootstrap 显示仅数据筛选即带来 2.8-4.2 个百分点实体召回增益（p<0.0001）。

Conclusion: 该三阶段流水线能有效改善口音英语 ASR 对命名实体和填充停顿的识别，并表明数据筛选与区域 LoRA 适配可显著提升语言学习相关反馈质量。
...
### [129] [SAGE: Schema-Guided LLMs for Grant Review](https://arxiv.org/abs/2609.20829)
*Erik Varapaev,Andrei Chetvergov,Stepan Ukolov,Timofei Sivoraksha,Alexander Evseev,Sergey Bolovtsov*

Main category: cs.CL

TL;DR: SAGE 是一个将资助评审标准转化为结构化检查，并把判断与申请材料证据关联的系统，用于生成可供专家审核和修正的评审草稿。


<details>
  <summary>Details</summary>
Motivation: 资助评审人员需要依据详细标准审查申请表、预算和支持材料，并给出可供同事检查的评估；因此需要一种结构化、可审计且证据可追溯的评审支持方法。

Method: 提出 SAGE（Schema-Guided Aspect-Based Grant Evaluation），将资助评审规则转化为结构化检查，并将判断链接到申请材料中的证据。在 35 份非营利资助申请上进行两阶段评估：先与原始竞赛的 105 份评审进行事后比较，再由基金会在查看 SAGE 后进行标准级复审并产生 202 份评估；同时与每标准单提示基线比较，并进行主张级审计。

Result: 事后比较中，SAGE 与原始评审达到中等序数一致性（kappa = 0.29）。在辅助复审轮中，SAGE 达到 kappa = 0.58，优于每标准单提示基线（共同子集上 kappa = 0.33），并具有更高排名相关性和更低误差。主张级审计还能识别结构化草稿中被确认、争议和未处理的部分。

Conclusion: SAGE 将评审方法操作化，能够生成详细、证据关联且可审计的评审草稿，供专家修正，从而支持资助评审流程。
...
### [130] [Reviser: Revision-Capable Text Generation via Autoregressive Cursor Actions](https://arxiv.org/abs/2609.20830)
*Sean Diab*

Main category: cs.CL

TL;DR: 提出 Reviser：一种 decoder-only Transformer，在可变画布上以光标相对动作序列生成回复。每步只预测 INSERT(token)、MOVE(Δ) 或 STOP，并按编辑历史而非最终文本顺序自回归，从而实现非单调生成。


<details>
  <summary>Details</summary>
Motivation: 具备修订能力的生成很有吸引力，因为可以插入或修改早前内容；但许多非自回归和基于编辑的方法需要通过反复的序列级计算来获得这种灵活性，代价较高。

Method: Reviser 使用可变画布和光标相对动作：每步预测一个动作 token，包括插入、移动和停止；模型自回归于编辑历史动作，而非最终文本顺序。论文在 continuation benchmark 上与 SEDD、MDLM 及同规模自回归基线比较，并分析轨迹统计和共享 FLOPs 约定下的推理计算。

Result: 在 arena 评估中，Reviser 显著优于 SEDD 和 MDLM；轨迹统计确认其频繁执行向后移动和画布中间插入，而非仅模拟末端追加解码。在 100M 和 300M 规模上，与同规模自回归基线相比具有竞争力；在共享 FLOPs 约定下，推理计算显著少于代表性多轮细化与扩散式基线。

Conclusion: Reviser 通过动作级自回归在可变画布上实现灵活修订式生成，同时保持简单的下一动作接口；结果表明其能兼顾生成质量与推理效率，是替代反复序列级计算方法的有力方案。
...
### [131] [Recursive Language Models Generalize Out of Domain](https://arxiv.org/abs/2609.20831)
*Chenxiao Yang,Zhiyuan Li,David McAllester,Nathan Srebro*

Main category: cs.CL

TL;DR: 研究限制语言模型可见上下文何时有助于学习；比较标准 CoT 与递归语言模型，发现分布内差异小，分布外递归隔离可避免捷径，表明真正推理不仅需要覆盖正确规则，还需克服简单性偏差。


<details>
  <summary>Details</summary>
Motivation: 探究限制语言模型可见范围为何能提升学习，以及标准 CoT 的通用性与递归上下文隔离在分布外泛化上的差异。

Method: 理论比较标准 CoT（读取完整推理轨迹）与递归语言模型（将每个子任务放在隔离上下文中求解），分析 IID 与 OOD 泛化保证及简单性偏差。

Result: IID 下 CoT 可高效模拟递归规则，泛化保证仅差常数因子，递归优势不大；OOD 下 CoT 可利用当前子任务外上下文形成捷径，分布变化后失效，而递归隔离消除该失败模式；尽管 CoT 假设类覆盖递归规则，简单性偏差仍会选择捷径而非真规则。

Conclusion: 要超越分布精度并真正推理，仅覆盖正确规则不够，还需避免因简单性偏差选择捷径；这与经典学习理论不同。
...
### [132] [TatBLiMP: A Benchmark of Linguistic Minimal Pairs for Tatar](https://arxiv.org/abs/2609.20832)
*Ilshat Saetov,Dmitry Gaynullin*

Main category: cs.CL

TL;DR: 提出TatBLiMP，首个鞑靼语（Tatar）语言最小对立对基准，覆盖16种形态句法现象、1248对句子，用于评估语言模型的语法性，无需文本生成或句法分析器。


<details>
  <summary>Details</summary>
Motivation: 鞑靼语此前缺乏任何语法性评估基准，连覆盖101种语言的MultiBLiMP也未收录鞑靼语；需要一种可直接比较模型概率、适用于基础模型和中间检查点的评估工具。

Method: 改编TurBLiMP的现象清单与单语素破坏操作，新增鞑靼语特有现象（数词/量词后裸名词的数）；语法句取自鞑靼语文学散文，非语法句由apertium-tat进行确定性单语素扰动生成，并由母语者审核；通过比较模型对语法与非语法句子的概率进行评分，遵循合理性原则确保非语法句为合理的真实错误。

Result: 基准能追踪针对鞑靼语的训练程度而非参数规模：478M从头训练模型和125M单语模型接近0.97，7B适配模型落后，30-120B前沿多语言LLM降至0.80-0.92，轻度微调的多语言模型最弱。

Conclusion: TatBLiMP可作为鞑靼语语法性评估的有效基准，但其继承的分类体系遗漏了母语者最关注的形态音系（元音和谐、辅音同化），论文提出构建原生第二层来补充这些现象。
...
### [133] [Transsion's Speaker-Attributed Multilingual ASR System for the MLC-SLM 2026 Challenge](https://arxiv.org/abs/2609.20833)
*Zhecheng Ren,Xuanji He,Xiaoxiao Li,Zhichen Han,Gaoyang Dong,Gaosheng Zhang,Minchuan Chen,Fengjie Zhu*

Main category: cs.CL

TL;DR: 本文介绍了 Transsion Speech Team 参加 MLC-SLM 2026 Challenge Task 1 的系统，任务为多语言对话语音的说话人属性转写。系统采用级联框架，在官方评测集上取得 tcpMER 15.41%，排名第二。


<details>
  <summary>Details</summary>
Motivation: 面向多语言对话语音中同时进行说话人分离与语音转写的需求，解决说话人属性转写这一挑战任务。

Method: 提出三级级联框架：基于 DiariZen 的说话人日志模块生成说话人同质片段；基于 Qwen3-Omni 的长语音多语言 ASR 模块生成转写；外部 CTC 对齐模型提供词级和字符级时间戳；最后融合模块结合日志输出与带时间戳转写，生成说话人属性 STM 输出。

Result: 在官方评测集上验证了框架有效性，提交系统取得 15.41% 的 tcpMER，并在所有参赛队伍中排名第二。

Conclusion: 所提出的级联说话人属性转写框架能够有效处理多语言对话语音，并取得具有竞争力的性能。
...
### [134] [Towards Secure Cloud-Native Computing: Unveiling Kubernetes Misconfigurations with Large Language Models](https://arxiv.org/abs/2609.20834)
*Mostafa Anouar Ghorab,Mohamed Aymen Saied*

Main category: cs.CL

TL;DR: 本文探讨利用大语言模型识别Kubernetes配置错误，提出常见错误分类体系，评估现有检测工具，并分析易错对象及其严重性。


<details>
  <summary>Details</summary>
Motivation: 云原生和Kubernetes广泛采用，但生态系统复杂，配置错误会危及安全与性能，因此需要更有效的检测方法。

Method: 构建Kubernetes常见配置错误分类法；对最先进检测工具进行实证评估；分析最易发生配置错误的Kubernetes对象；利用包括LLM在内的先进机器学习技术。

Result: 基准测试了现有检测工具的有效性，识别了易配置错误的Kubernetes对象，并评估了相关问题的严重程度。

Conclusion: LLM等先进技术有望增强Kubernetes配置错误检测，为改进检测方法提供了新见解。
...
### [135] [PhysioBench: A Unified Benchmark for Physiological Signal Question Answering](https://arxiv.org/abs/2609.20836)
*Mengxuan Li,Junfa Chen,Jinze Xia,Yundan Chen,Lixin Fan,Ke Liu,Keyue Shi,Haishuai Wang*

Main category: cs.CL

TL;DR: 该论文提出 PhysioBench，一个用于生理信号问答的统一基准，整合22个公开数据集、30项任务和6140万个问答对，并评测21个代表性模型。


<details>
  <summary>Details</summary>
Motivation: 现有生理信号基础模型通常需要针对每个任务进行特定适配，而自然语言虽可作为统一预测目标接口，但当前模型在生理信号多模态、多任务指令遵循方面的能力尚未得到充分评估。

Method: 构建 PhysioBench 基准：将22个公开数据集的标注统一为30项任务、6140万个问答对，每个问答对基于信号片段并可追溯至原始标注；在三种互补设置下评测21个代表性模型，包括大语言模型、视觉语言模型、时间序列语言模型和生理信号基础模型。

Result: 评测显示，没有任何模型能在所有生理信号模态和任务上持续表现优异；引入自然语言可支持跨任务统一预测，但性能对问题表述方式较为敏感。

Conclusion: PhysioBench 揭示了当前模型在生理信号理解上的不足，并提供了一个可扩展平台，用于细粒度分析和未来研究。
...
### [136] [A Generative Grammar Underlying the Voynich Manuscript, the Pastiche Hypothesis: Evidence from Large Language Models](https://arxiv.org/abs/2609.20835)
*Nicolas Turenne*

Main category: cs.CL

TL;DR: 该论文对《伏尼契手稿》进行多学科分析，结合概率建模、语音分解、稀有事件检测和多模态图像分析，发现其符号更像字母，但词长分布类似音节结构，语音上更接近希伯来语或阿拉伯语等辅音密集语言，植物插图与地中海传统的伪阿普列乌斯药草书高度对应。


<details>
  <summary>Details</summary>
Motivation: 《伏尼契手稿》是十五世纪用未知文字写成的抄本，内容至今未被破译；此前研究认为其统计特性类似自然语言，插图主要为植物，类似中世纪药草书，因此需要进一步检验其语言结构和图像来源。

Method: 基于新转写语料，使用位置依赖的概率语法建模词和字母分布，进行语音模式比较（印欧、闪米特和亚洲语言），结合稀有事件检测，并利用大语言模型进行图文对齐以识别植物对应关系。

Result: 伏尼契符号表现为字母而非音节单位，但词长分布类似音节结构；语音分析显示其更接近希伯来语或阿拉伯语等辅音密集语言，而非印欧语言；概率模型重现了齐普夫式分布，并发现重复首字母序列概率极低，表明其有结构地模仿自然语言；图像分析显示其植物插图与地中海传统的伪阿普列乌斯药草书插图高度对应。

Conclusion: 这些发现支持《伏尼契手稿》遵循一个结合语言规律和药草知识的结构化生成系统，并证明整合概率方法和AI辅助方法在分析历史手稿中的价值。
...
### [137] [From Generation to Detection: Exploration of Discourse Driven Scenario based LLM Generated Fake News](https://arxiv.org/abs/2609.20838)
*Zeynep Özdemir,Murat Osmanoğlu,Sevgi Yiğit-Sert,Ömer Özgür Tanrıöver,Yılmaz Ar*

Main category: cs.CL

TL;DR: 本文在受控条件下研究现代大语言模型在四类操控场景中生成与检测假新闻的表现，并分析生成策略、语言学特征及提示优化对检测效果的影响。


<details>
  <summary>Details</summary>
Motivation: 为系统评估大语言模型在不同典型生成场景下制造和识别虚假信息的能力，并检验基于真实-虚假新闻对提取误导模式的改进提示是否能提升检测性能。

Method: 设计四类假新闻生成场景：开放式生成、改写、操控提示、基于新闻话语框架的属性提示；使用七种主流模型生成包含14000篇文章的合成假新闻语料库；分析其结构与语义上的语言学特征；再让各模型在基础提示和迭代改进提示下进行假新闻检测。

Result: 不同模型在生成和检测虚假信息方面差异显著；生成策略会强烈影响假新闻的可检测性；迭代改进提示并未提升检测表现，反而常常损害检测性能。

Conclusion: 该研究对LLM在典型生成场景下检测LLM生成假新闻的能力进行了系统评估，表明检测效果受模型与生成方式影响，提示优化未必有效。
...
### [138] [Curriculum-Based Noise Adaptation for Phoneme-to-Text Reconstruction in Visual Speech Recognition](https://arxiv.org/abs/2609.20839)
*Matthew Kit Khinn Teng,Haibo Zhang,Takeshi Saitoh*

Main category: cs.CL

TL;DR: 本文提出渐进式错误课程训练（PECT），用于音素中心视觉语音识别中的音素到文本重建，通过逐步让模型适应更真实的音素预测错误来提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有音素到文本重建模型通常在干净音素序列或合成噪声输入上训练，导致训练条件与推理时真实音素预测错误不匹配，限制整体识别性能。

Method: 提出PECT课程学习框架，基于NLLB的音素到文本重建模型，逐步引入合成音素扰动、多域伪标签以及由视觉语音识别器生成的目标域伪标签，使模型渐进适应越来越真实的音素预测错误。

Result: 在LRS2和LRS3基准上，PECT对多种音素中心视觉语音识别前端（V-ASR、PV-ASR、HP-VSR变体）均一致提升重建性能。例如，HP-VSR-FiLMFuse（L4）在LRS2上WER从23.3%降至22.2%，HP-VSR-ResFiLM在LRS3上WER从30.3%降至29.7%。消融实验和定性分析验证了其有效性。

Conclusion: PECT是一种有效且可泛化的课程学习策略，能够提升音素中心视觉语音识别中音素到文本重建对真实预测错误的鲁棒性。
...
### [139] [COAL-SQL: Coverage-Guided Augmentation and Failure-Driven Learning for Text-to-SQL Post-Training](https://arxiv.org/abs/2609.20842)
*Qifeng Cai,Xuanguang Pan,Hao Liang,Chang Xu,Wentao Zhang*

Main category: cs.CL

TL;DR: 论文提出 COAL-SQL 统一框架，通过覆盖引导增强（CGA）和失败驱动学习（FDL）提升开源大语言模型的复杂 Text-to-SQL 生成能力。


<details>
  <summary>Details</summary>
Motivation: 开源大语言模型在复杂真实 SQL 生成上仍需任务特定后训练；现有数据集对 SQL 结构覆盖不全，增强方法未识别结构缺口，且单独使用 SFT 或 RL 无法动态解决训练中暴露的弱点。

Method: COAL-SQL 结合 CGA 与 FDL：CGA 用贪心选择识别原数据集中缺失的 SQL 结构并构造互补样本；FDL 以 GRPO 为主优化目标，并对未解决样本提供定向监督，步骤级对累积失败由强 LLM 生成的已验证推理轨迹做 SFT，轮次级基于累积失败检索结构相关样本进行针对性练习。

Result: 仅使用 12,600 个不同后训练样本，COAL-SQL 在 BIRD 开发集上达到 64.9% 执行准确率，并优于同等规模训练的基线。

Conclusion: COAL-SQL 通过结构覆盖增强与失败驱动学习相结合，能有效提升 Text-to-SQL 后训练效果，在较少数据下取得优异性能。
...
### [140] [VISPATH: Visual-Intent-Guided Path Reasoning for Multimodal Knowledge Graph Question Answering](https://arxiv.org/abs/2609.20843)
*Jinke Wu,Zhengpin Li,Mengzhe Jia,Yang Li,Wentao Zhang*

Main category: cs.CL

TL;DR: 本文提出 VISPATH，一种面向多模态知识图谱问答（MM-KGQA）的视觉意图引导路径推理框架，并构建了 VISPATH-Bench 基准，在多个数据集上取得优于强基线及 GPT-5.4 的效果。


<details>
  <summary>Details</summary>
Motivation: 现有 MM-KGQA 方法通常仅在起始实体定位或证据检索阶段使用多模态信息，后续多跳推理退化为纯文本图搜索，无法利用中间跳中变得重要的多模态线索。

Method: VISPATH 包含：结合多模态定位与图结构线索识别可靠起始实体；根据输入、问题与当前部分路径重算每跳多模态意图，进行意图引导的路径发现；通过推理链剪枝，基于与问题、推理草图及每跳意图的一致性评估候选路径作为完整证据链；最后检查所选证据是否足以生成答案。同时构建覆盖2至4跳的 VISPATH-Bench 基准。

Result: 在 VISPATH-Bench 及三个额外多模态问答基准上的实验表明，VISPATH 持续优于强基线；以 GPT-4o 为骨干时，在 VISPATH-Bench 上超越 GPT-5.4，平均准确率相对提升10.6%，2跳推理提升13.1%。

Conclusion: VISPATH 通过在多跳推理全过程中动态利用多模态意图，有效解决了现有方法中间跳多模态线索利用不足的问题，提升了多模态多跳知识图谱推理性能。
...
### [141] [Boosting Deepresearch and LongContext Ability with Self-Generated Deepresearch Rollouts Traces](https://arxiv.org/abs/2609.20844)
...
TL;DR: 论文发现Deepresearch（DR）智能体经DR-RL后仍有61.6%的预测错误源于长上下文理解不足，为此提出DR-to-Long将DR轨迹转化为长上下文QA数据，并设计DLD-RL三阶段训练，在DR和长上下文基准上均显著提升。


<details>
  <summary>Details</summary>
Motivation: DR智能体通过多轮搜索与访问真实网页，上下文快速增长；即使经过DR-RL，仍有61.6%的剩余错误可归因于长上下文理解不足，包括长上下文幻觉和跨文档证据整合失败。因此需要强化模型长上下文能力，但有效训练面临数据稀缺问题。

Method: 提出DR-to-Long：复用DR-RL轨迹中的搜索历史、访问网页、证据片段和最终答案监督，将紧凑片段与网页摘要替换为对应URL的全文，生成保留原始证据关系的长多文档上下文。基于此提出DLD-RL：先进行短DR-RL收集轨迹，零标注成本转成LongQA实例；再用LongQA-RL增强长上下文能力；最后进行完整DR-RL继续提升DR能力。

Result: DLD-RL在三个Deepresearch基准上比标准DR-RL高7.3%，在三个长上下文基准上提升13.5%。

Conclusion: 通过复用DR-RL轨迹构建长上下文训练数据并采用分阶段强化学习，可突破DR-RL的长上下文瓶颈，同时提升DR能力与长上下文理解能力。
...
### [142] [Reading Less While Writing: A Closed-Form Bandwidth Dial for Streaming Multimodal Decoders](https://arxiv.org/abs/2609.20845)
*Yasir Mehmood,Kashif Javed*

Main category: cs.CL

TL;DR: 提出 ZENDAYA，一种用单一连续参数 γ 调控流式解码中可见源前缀的调度方法，使离线解码与实时流式解码成为同一族模型的两个端点。


<details>
  <summary>Details</summary>
Motivation: 传统解码器需读完整输入才输出词，离线可行但实时字幕不可能；现有流式方法用固定 wait-k 规则，忽略输入长度与节奏，低延迟下性能差。

Method: 将固定偏移替换为闭式调度：可见源前缀是生成进度的函数，并按预测输入长度缩放；γ 同时决定每输出词平均消耗源比例约为 1/(1+γ)。作者证明结构依赖定理：对任意预先固定且非减的调度，输出 token 不能依赖尚未到达的输入，适用于训练/未训练权重及异步无界流。

Result: 在 Charades-STA、ActivityNet Captions、LibriHeavy 两种模态三个公开语料上从零训练，29M 参数紧凑解码器匹配或超过固定调度，同时读取更少源内容；最低延迟下提升最明显。流式 METEOR 提升在三个语料上均统计显著。

Conclusion: 较少源输入反而可能生成更好文本，因为过多源会稀释注意力；ZENDAYA 将延迟、计算预算和性能统一为可解释的连续调度，并具理论保证。
...
### [143] [Rewarding Efficient Reasoning Improves Abstention on Underspecified Tasks in Reasoning Models](https://arxiv.org/abs/2609.20846)
*Polina Tsvilodub,Max Höth,Michael Franke,Björn Deiseroth,Carina Kauf*

Main category: cs.CL

TL;DR: 大型推理模型（LRM）虽能答对许多任务，但常难以判断何时应弃答。研究通过人类实验发现，人类在不可答任务上的推理努力不超过可答任务，而LRM却在不可答提示上生成更长思维链、浪费算力。为此，作者提出一种受资源理性启发的GRPO奖励，微调多个4B LRM后，弃答表现平均提升12.8%，并保持回答能力，思维链平均缩短44%。


<details>
  <summary>Details</summary>
Motivation: 现代LRM在提供正确答案方面表现优异，但缺乏“知道何时弃答”的关键能力。与人类相比，LRM在不可答任务上反而投入更多推理算力，导致低效。作者希望理解这一差距，并让模型像人类一样进行资源理性的推理与弃答。

Method: 作者首先将LRM行为与人类研究结果进行对比分析。受人类认知的资源理性观点启发，提出一种新的GRPO奖励，鼓励模型高效判断任务是否包含解决问题所需的全部信息。随后用该奖励微调多个4B规模的LRM。

Result: 微调后，多个4B LRM在弃答表现上平均提升12.8%，同时保留回答能力，并将思维链（CoT）平均缩短44%。

Conclusion: 资源理性启发的GRPO奖励能有效改善LRM的弃答行为，使其更接近人类模式，同时提升推理效率。这表明弃答能力与计算效率可以通过针对性训练得到改善。
...
### [144] [Reading Anxiety or Reading the Label? Comparing Fine-Tuned and Frontier Models for Anxiety Detection on Social Media](https://arxiv.org/abs/2609.20847)
*Cris Huynh,Arlene Pham*

Main category: cs.CL

TL;DR: 该研究在Reddit焦虑检测任务上比较了三类方法（前沿商业模型、微调小模型、传统分类器）共六种条件，并揭示了因关键词“anxiety”导致的评估混杂，通过删除该词重新评估来量化模型的词汇依赖。


<details>
  <summary>Details</summary>
Motivation: 焦虑是最常见的心理健康问题之一，人们常在寻求临床帮助前在线书写相关内容；实践者构建检测工具时面临选择：调用前沿商业模型、微调小模型还是部署传统分类器。同时，现有评估可能因关键词匹配而虚高。

Method: 在统一的受控协议下，使用留出的Reddit测试集比较六种条件。发现69.3%的焦虑标签帖子包含“anxiety”或其变体，约为对照条件的两倍。因此对每个模型进行两次评估：原始文本和删除这些词后的文本，并将差异报告为词汇依赖。

Result: 前沿模型在焦虑F1上领先（0.846），但一个110M参数的领域适应编码器达到0.831且无外部API依赖，其中心理健康领域预训练仅贡献0.7分。词汇依赖范围为8.6至25.4分，且与模型能力无关：LoRA微调的3B模型最依赖关键词，甚至高于TF-IDF分类器，而前沿零样本模型依赖最低。

Conclusion: 该语料上已发表的指标是上限，且通胀对微调模型最大——而这些正是此类文献通常报告的模型。
...
### [145] [Enhancing Audio Reasoning via Semantic Summary Prediction](https://arxiv.org/abs/2609.20849)
*Francesco Bonzi,Pooneh Mousavi,Cem Subakan,Mirco Ravanelli*

Main category: cs.CL

TL;DR: 论文针对大型音频语言模型在显式思维链推理时准确率反而下降的推理鸿沟，提出SPARE方法：通过一个与最终结论对齐的寄存器token和Sentence-BERT余弦相似度损失，在推理前用目标语义条件化模型潜在空间，从而提升零样本音频推理并增强早期对音频的注意力，且不增加推理成本。


<details>
  <summary>Details</summary>
Motivation: LALMs在复杂问答表现好，但显式CoT相比直接回答会降低准确率，存在推理鸿沟；作者假设长推理序列使注意力偏离音频输入。

Method: 提出SPARE（Semantic Prediction for Audio REasoning）：引入与最终结论对齐的register token，使用与Sentence-BERT嵌入的余弦相似度损失，在推理开始前将模型潜在空间条件化到目标语义目标上。

Result: 在MMAU和MMAR数据集上使用SALMONN进行实验，零样本推理性能提升，对音频的早期注意力更强，且不增加额外推理成本。

Conclusion: SPARE可缓解LALMs的CoT推理鸿沟，通过语义目标预条件化提升音频推理表现和早期音频关注，同时保持推理效率。
...
### [146] [MME-Safety: A Fine-grained Benchmark for Safety Evaluation of MLLMs](https://arxiv.org/abs/2609.20850)
...
TL;DR: 论文提出了MME-Safety基准和分层评估框架，对17个先进多模态大模型进行零样本评估，揭示了跨模态输入配置和思维链推理带来的安全隐患。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型存在跨模态漏洞，可绕过单模态过滤器；现有基准缺乏细粒度意图标注和全面度量指标，难以全面评估鲁棒性。

Method: 提出MME-Safety基准，包含四维标注模式（风险场景、危害严重性、模态特定隐蔽级别）；引入分层评估框架，评估响应可靠性、实际风险暴露和防御行为结构完整性；对17个MLLM进行零样本评估，分析跨模态输入配置和CoT推理的安全影响。

Result: 提供了当前多模态系统的全面安全画像，系统分析了跨模态输入配置，并揭示了与思维链推理相关的安全影响。

Conclusion: 研究结果强调，在多模态领域迫切需要进行稳健且具有推理感知的安全对齐。
...
### [147] [$μ^2$-Bench: A Multilingual Machine Unlearning Benchmark](https://arxiv.org/abs/2609.20945)
*Kyomin Hwang,Hyeonjin Kim,Hyunho Lee,Yearim Kim,Yeji Song,Nojun Kwak*

Main category: cs.CL

TL;DR: 该论文提出了多语言机器遗忘基准 μ²-Bench，用于模拟多语言大模型中信息记忆、遗忘与评估的全流程，并分析多语言遗忘的特性。


<details>
  <summary>Details</summary>
Motivation: 有害内容和隐私数据会通过多语言大模型的直接训练和跨语言传播扩散；多语言机器遗忘（MMU）的评估尚不充分，难以判断遗忘是否真正在所有语言中消除了目标知识。

Method: 构建 μ²-Bench 基准，覆盖多种语言，模拟记忆、遗忘和评估的完整流程；同时在训练语言和留出语言上评估，并考察知识分散在多种语言中的情况；基于该基准进行分析以提供深入见解。

Result: 研究表明，成功的多语言机器遗忘需要能够反映多语言特征的方法；该基准和分析为 MMU 提供了更深入的洞察。

Conclusion: μ²-Bench 填补了多语言机器遗忘评估的空白，强调多语言特征对遗忘方法的重要性，并推动更可靠的跨语言知识消除评估。
...
### [148] [Aligning with Lived Experience: Heterogeneous Benefits of Fine Tuning in Mental Health Support Generation](https://arxiv.org/abs/2609.21075)
*Mohit Chandra,Nabin Kim,Eli Min,Aamogh Sawant,Tanmay Sutar,Munmun De Choudhury*

Main category: cs.CL

TL;DR: 该研究提出COPES数据集与三轴评估框架，用于评估大语言模型在在线心理健康同伴支持中是否与社区视角一致，并发现后训练能提升部分对齐指标但存在社区与策略差异。


<details>
  <summary>Details</summary>
Motivation: 专业心理医疗服务可及性有限，许多人转向Reddit等在线社区寻求基于真实生活经验的同伴支持，但大量求助无人回应，因此探索LLM能否填补这一空白；然而LLM在临床基准上表现虽好，其生成符合社区经验和社区价值观的同伴支持能力仍缺乏研究。

Method: 构建COmmunity-centered Peer Engaged Support（COPES）数据集，并提出三轴评估框架，衡量LLM对心理健康求助查询的回应与社区视角的一致性；评估零样本及后训练模型，包括SFT和DPO。

Result: 在COPES上后训练能显著提升策略对齐，通用模型提升超过50%，并改善情绪与语气对齐；但提升具有异质性，在不同subreddit和求助者所需应对策略上差异明显。后训练还带来分布偏移，过度偏向问题聚焦建议，同时抑制情绪聚焦策略。

Conclusion: 整理社区驱动数据可改善LLM回应的对齐程度，但模型在不同子社区和具体心理健康需求上的表现仍不均衡，说明仅靠社区数据后训练尚不足以实现普遍有效的同伴支持。
...
### [149] [Generative Artificial Intelligence Chatbots for Motivational Interviewing: A Scoping Review From System Design to Intervention Outcomes](https://arxiv.org/abs/2609.20902)
*Runze Hu,Jingqi Kong,Yang Yang,Yihang Yang,Jingyao Liu,Haizhou Tang,Shanghang Zhang,Zheng Liu*

Main category: cs.CL

TL;DR: 这篇范围综述总结了生成式AI动机性访谈聊天机器人的证据，涵盖系统设计、安全性、MI质量、用户感知和干预结果。共纳入47篇报告（48项研究），发现能提供MI一致的互动且用户评价积极，但对持续行为或功能改变的证据有限。


<details>
  <summary>Details</summary>
Motivation: 动机性访谈是促进健康行为改变的有效方法，生成式AI为通过对话系统提供MI提供了新途径，但相关证据在设计、评估和转化方面分散，因此需要系统综述。

Method: 采用PRISMA-ScR范围综述方法，检索9个数据库，时间从2015年1月1日至2026年6月2日，纳入使用生成式AI生成MI聊天机器人回应或咨询师话语的研究。使用预定义框架提取数据并进行描述性综合。

Result: 纳入47篇报告（48项研究）。20项（41.7%）仅关注系统设计，28项（58.3%）涉及直接互动。多数系统为文本式、无实体；23项（47.9%）具有动态适应。安全性报告不均；直接使用研究中21/28（75.0%）报告了知情同意或用户教育。30项（62.5%）评估了MI质量，普遍显示MI一致的互动。用户感知积极，尤其是共情、可用性、帮助性和使用意愿，但测量异质性大。18项（37.5%）报告了干预结果，多为单次会话后；短期动机的积极结果较一致，但持续行为或功能改变证据有限。

Conclusion: 生成式AI-MI聊天机器人能提供MI一致的互动并获得积极评价，但对持续行为或功能改变的证据有限。未来研究应加强运行时安全监测，标准化MI质量评估，并采用更长期的比较设计，关注行为和功能结局。
...
### [150] [Geometry of Values: Task Vector Composition for Ethical Preference Alignment in Language Models](https://arxiv.org/abs/2609.21094)
*Utkarsh Agarwal,Monojit Choudhury*

Main category: cs.CL

TL;DR: 论文构建了涵盖诚实、公正、自主三种价值观两两冲突的双选项道德困境数据集，共12000条，并翻译为印地语、阿拉伯语、西班牙语和中文，用于评估大语言模型的跨语言道德偏好与偏见，并探索微调和任务向量方法对价值偏好的影响。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多用于需要权衡冲突道德价值的场景，但即使强模型也存在隐藏偏见，且跨语言指令遵循能力脆弱，因此需要系统探测其道德价值偏好及跨语言行为。

Method: 构建包含诚实vs公正、公正vs自主、自主vs诚实三类两两价值冲突的双选项困境数据集，规模12000条，并翻译为五种语言；在GPT-5-mini和Llama-3.2-1/3B等模型上基准测试；使用普通微调和直接偏好优化（DPO）微调；提出基于任务向量迁移的实验，通过将价值偏好方向与通用指令遵循向量正交化，以分离特定价值偏好方向，并进行任务算术得到相反立场模型。

Result: GPT-5-mini在无政策提示时，在所有五种语言中一致偏好诚实而非自主；Llama-3.2-1/3B表现出强首选项偏见，但普通微调和DPO微调均能有效消除该偏见，准确率提升至98%以上；任务向量正交化实验能有效隔离特定价值偏好方向，并可通过任务算术获得持有相反立场的模型。

Conclusion: 该研究提供了跨语言道德困境基准，揭示了模型的价值偏好和选项位置偏见，并表明微调可缓解偏见，任务向量正交化有助于分离抽象价值偏好与数据集相关性，从而实现可控的价值立场调整。
...
### [151] [From Task Success to Productive Success: Evaluating Human-AI Collaboration by Quality and Cost](https://arxiv.org/abs/2609.21117)
*Saki Imai,Mert İnan,Malihe Alikhani*

Main category: cs.CL

TL;DR: 该论文提出以“结果质量相对交互成本”衡量人-AI协作生产力的框架，并通过两个数据集、四项任务表明交互成本是评价AI系统时不可忽视的关键维度。


<details>
  <summary>Details</summary>
Motivation: 现有AI生产力多用任务完成时间、经济价值或结果质量衡量，把协作过程视为黑箱，只关注产出什么，却忽略产生该产出所需的交互成本。受经济学启发，作者希望让交互成本可见，并区分高生产力成功与高成本成功。

Method: 提出面向生产力的评估框架，将生产力定义为结果质量相对于交互成本；在覆盖四项任务的两个数据集上分析人-AI协作对话，比较质量评分、交互成本、任务差异、主观用户评分及对话行为特征。

Result: 研究发现：同样质量评分的会话交互成本可相差最高70倍；质量-成本关系因任务而异，有的任务受益于延长交互，有的更倾向快速收敛；用户主观评分不能可靠替代生产力；高生产力会话中AI更早主动探查，用户用于修复交互的努力更少。

Conclusion: 通过区分高生产力成功与高成本成功，该框架使交互成本显性化，并表明对话分析可为人-AI系统的评估与设计提供依据。
...
### [152] [Scaling Forced Alignment to End-User Devices](https://arxiv.org/abs/2609.21145)
*Lawry Sorenson,Michael Crandall,Eric K. Ringger,Stephen D. Richardson*

Main category: cs.CL

TL;DR: 本文提出两种优化方法，用于音频与文本的强制对齐，以解决现有Viterbi算法二次复杂度的问题，显著降低内存并提升速度。


<details>
  <summary>Details</summary>
Motivation: 现有强制对齐实现具有二次时间和空间复杂度，难以处理长输入序列。

Method: 使用Hirschberg算法实现线性内存对齐；将语音与文本对齐建模为约束随机游走，以任意置信度剪枝搜索空间并处理转录错误。

Result: Hirschberg优化将三小时输入的内存从140 GB降至5 MB，在CPU上以torchaudio三分之一时间产生相同对齐；剪枝对超过20分钟的输入再获2倍加速，在98%以上测试案例中保持对齐精度。

Conclusion: 所提优化有效提升了强制对齐的可扩展性和效率，同时保持准确性。
...
### [153] [CoLearn: An Agentic Tutor that Learns its Learner in a Human--AI Co-Learning Loop](https://arxiv.org/abs/2609.21154)
*Kailai He,Zhihao Wu,Linhai Zhang,Runcong Zhao,Yulan He,Jiazheng Li*

Main category: cs.CL

TL;DR: 论文提出 CoLearn，一个支持迭代辅导循环的交互式智能体导师，通过证据驱动的学习者状态记忆实现个性化出题。


<details>
  <summary>Details</summary>
Motivation: 现有辅导工具多使用固定题库，并把答错仅视为单一信号，难以追踪学习者知识掌握与错误原因，因而无法实现真正适应个体的辅导。

Method: CoLearn 包含三部分：持久学习者状态记忆，用贝叶斯知识追踪的软证据变体更新各主题掌握度，并以大语言模型作为连续观察函数；自适应问题生成，针对最弱主题和反复出现的误解出题；证据视图，通过实时进度可视化和盲测 A/B 比较使个性化可见、可测试。

Result: 在盲测 A/B 评估中，基于该记忆条件生成的问题有 68-69% 的概率被偏好，优于非个性化问题；在隐藏真实掌握度的人设模拟中，智能体的信念会向学习者的真实掌握度收敛。

Conclusion: CoLearn 表明，结合持久学习者记忆与智能体式问题生成的个性化辅导能够提升问题相关性，并可通过模拟验证其学习者建模的有效性。
...
### [154] [Not All Irregularity Is Equal: Causally Isolating a Rare Failure Mode in Japanese Morphological Inflection](https://arxiv.org/abs/2609.21179)
*Wen Zhang*

Main category: cs.CL

TL;DR: 本文对日语过去时动词变形进行正字法感知诊断，发现两个字符级Transformer模型虽总体准确率超97%，但一个特定不规则子类（词干以/e/结尾且需辅音重叠的动词）仅占数据不到1%，却贡献30-43%的残余错误，错误率是普遍频率的34-48倍。


<details>
  <summary>Details</summary>
Motivation: 神经形态生成系统在基准数据集上总体准确率高，但可能掩盖罕见形态子类中的系统性错误；研究旨在揭示错误集中并非由不规则性本身驱动，而是极端低频形态模式与特定正字法过程的交互。

Method: 使用两种字符级Transformer架构，在五个随机种子下评估日语过去时动词变形；将平假名视为编码形态音系结构的表征系统；进行受控消融实验，单独移除特定不规则子类，并与移除所有不规则动词对比。

Result: 两个系统总体准确率均超97%，但特定不规则子类（词干以/e/结尾且需辅音重叠）占数据不到1%，却贡献30-43%的残余错误，错误贡献约为其流行度的34-48倍；移除该子类的准确率增益大于移除所有不规则动词。

Conclusion: 神经形态学习中的错误集中由极端低频形态模式与特定正字法过程的交互驱动，而非不规则性本身；形态评估应纳入细粒度子类分析，并对数据高效、发育合理的语言模型预训练有启示。
...
### [155] [When Better Turns Do Not Make Better Agents: Diagnosing the Gap Between Next-Turn Metrics and Workflow Success](https://arxiv.org/abs/2609.21187)
...
TL;DR: 本文研究逐决策的下一动作评估是否能预测智能体自主工作流执行表现，发现监督微调提升下一轮成功率，但不能转化为端到端工作流成功。


<details>
  <summary>Details</summary>
Motivation: 当前智能体常基于黄金交互历史逐决策评估，但尚不清楚该评估协议下的改进是否能代表自主工作流执行能力的提升。

Method: 在多轮客服工作流上，评估预SFT和SFT后的Qwen3（4B、14B）与Gemma 3（4B、12B）模型，比较黄金历史下一轮评估与自主工作流/整体轨迹执行评估。

Result: SFT一致提升文本轮次成功，且所有模型在黄金历史下一轮成功整体上升；但改进未迁移到自主工作流执行，工具特定收益因指标和模型而异。四个SFT模型均未在整体工作流评估中成功，严格轨迹完成的工作流成功率最高仅10.4%。

Conclusion: 下一轮评估不是工作流成功的可靠代理，应分别报告文本质量、局部动作正确性、工具执行和端到端任务完成情况。
...
### [156] [Hallucination-R1: Robustness-Oriented Paraphrase Generation for Factual Consistency](https://arxiv.org/abs/2609.21227)
*Wenhan Yu,Wenxin Wu,Hao Wang,Lei Sha*

Main category: cs.CL

TL;DR: 论文研究释义诱导幻觉：模型对原问题回答正确，但在语义等价改写后出错。提出 HALLUCINATION-R1 鲁棒性导向释义生成框架，通过两阶段优化生成语义忠实但能暴露事实不一致的释义，在多个数据集上揭示模型鲁棒性失效，并证明生成数据可用于鲁棒性微调。


<details>
  <summary>Details</summary>
Motivation: 通用释义作为鲁棒性监督不足：近复制释义信号弱，过度多样的释义又可能破坏语义等价。因此需要一种既能保持语义忠实、又能有效挑战模型事实一致性的释义生成方法。

Method: 提出 HALLUCINATION-R1 框架，进行两阶段优化：先稳定生成保义且多样的释义，再奖励那些能使下游问答模型事实一致性下降的释义。在 SimpleQuestions、PopQA、TruthfulQA 上评估。

Result: HALLUCINATION-R1 实现较强的一致性-多样性权衡，并暴露多个模型族和数据集上的鲁棒性失败。分析表明这些失败并非表面伪影或语义漂移，而是意义保持变化下的事实不稳定。轻量微调研究显示，其生成数据可提升释义变化下的鲁棒准确率。

Conclusion: 释义诱导幻觉揭示了模型潜在的事实不稳定性。HALLUCINATION-R1 能生成有效的鲁棒性测试与训练数据，代码和模型已公开。
...
### [157] [Beyond Reference-Based Evaluation: Reward Models for Meta-Evaluation of Grammatical Error Correction](https://arxiv.org/abs/2609.21231)
*Ruotian Wu,Bill E. Johnson,Gene Saunders,Osama Hamzeh,Ankit Vadehra,Pascal Poupart*

Main category: cs.CL

TL;DR: 提出RM-EVAL，一个基于SEEDA人类偏好数据训练的奖励模型，可作为无参考元评估器预测人类质量判断，并可通过RGTG用于改进GEC生成。


<details>
  <summary>Details</summary>
Motivation: 现有GEC参考指标（如M²和ERRANT）假设参考集穷举所有有效编辑，常会惩罚语法正确、语义不变但表述不同的修改。

Method: 训练RM-EVAL奖励模型，利用SEEDA人类偏好数据进行全序列和部分序列质量评估；并将其作为学习信号，通过Reward-Guided Text Generation（RGTG）在冻结基础GEC模型的情况下进行在线奖励驱动解码。

Result: RM-EVAL在SEEDA上与人类排序高度一致；RGTG在奖励和外部验证上均取得一致提升。

Conclusion: 该工作提供了一个无需黄金参考即可评估和增强GEC系统的统一框架。
...
### [158] [When Does Reasoning Help in Machine Translation? A Hierarchical Analysis of LRM Reasoning Traces](https://arxiv.org/abs/2609.21247)
*Yuxiang Liu,Jiaming Luo,Eleftheria Briakou,Colin Cherry*

Main category: cs.CL

TL;DR: 本文系统分析了大型推理模型在机器翻译中的推理轨迹，发现推理语言因模型而异、推理长度与质量呈非单调关系，并提出了层次化元摘要框架来归纳推理结构。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在机器翻译中越来越多地使用中间推理轨迹，但何时这种推理有益或有害尚不清楚，需要系统分析推理语言、长度和结构的影响。

Method: 跨模型、语言、领域和数据集分析推理轨迹，关注推理语言、长度和结构；提出层次化元摘要（HMS）框架，无需预定义分类即可归纳粗粒度和细粒度的推理结构。

Result: 最佳推理语言因模型而异；推理长度与翻译质量呈非单调关系；推理轨迹呈现重复的功能模式。HMS揭示了共享组织结构——理解/规划、翻译/草拟、精炼/验证——以及领域特定变化。

Conclusion: 机器翻译中的推理应以模型感知、长度感知和模式感知的方式进行控制，而非统一鼓励使用推理。
...
### [159] [From Memory to Behavior: A Behavior-Aware Role-Playing Framework for Social Media Influencers](https://arxiv.org/abs/2609.21349)
*Ji-Lun Peng,Yi-Zhen Zhang,Chun-Nan Chou,Yun-Nung Chen*

Main category: cs.CL

TL;DR: 该论文提出一种结合情境、内部状态与行为策略的角色扮演方法，用于提升大语言模型对真实个体的拟人化模仿能力，并设计了带参考信息的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有基于上下文学习的方法难以刻画个体在不同情境下的反应，且对不知名人物的LLM评估较为困难。

Method: 提出 Situation–Internal state–Behavior Persona 方法，引入情境依赖的行为策略；设计为LLM评估者提供被模仿个体参考信息的评估协议；在新建社交媒体回复数据集上评测，并在虚构角色基准上验证泛化性。

Result: 所提方法优于最先进的ICL基线；评估协议与人工判断达到中等相关；在虚构角色基准上也表现有效，说明方法不限于社交媒体场景。

Conclusion: 引入行为信息可广泛提升对真实个体及虚构角色进行角色扮演的保真度。
...
### [160] [How Many Humans Is a Judge Panel Worth?](https://arxiv.org/abs/2609.21277)
*Chao Li,Yingying Yu,Yunfeng Li*

Main category: cs.CL

TL;DR: 论文提出评估语言模型评审团相当于多少人类判断，取决于匹配目标；同一评审团在谱多样性口径下有效规模较高，但在分布恢复口径下较低，二者不可互换。


<details>
  <summary>Details</summary>
Motivation: 现有将语言模型作为人类评审替代时，往往忽略人类标注中的分歧，且用单一指标衡量评审团质量。作者希望区分“谱多样性”和“分布恢复”两种有效规模，审计模型评审团究竟代表多少人类判断。

Method: 将分类评审团与真实人类标签分布对比，保留二元错误会忽略的分歧。通过匹配归一化残差Gram矩阵的参与率与条件独立人类参考抽样，得到 nu_H；另通过匹配分布平方误差得到 nu_MSE。使用谱恒等式分解特征值、成员能量和平均方向权重。在三个ChaosNLI任务上分析32个评审员的面板，并检查硬标签可实现面板、同尺寸排序一致性、成员增删影响及共识方向方差占比。

Result: 三个ChaosNLI任务中，同一32评审员面板的 nu_H 为4.24–6.50，而 nu_MSE 为2.30–3.75。谱多样性更高不一定分布恢复更好，即使成员能量相同且相关性非负。同尺寸排序一致性随任务变化明显，某些成员增删导致跨两个项目半样本持续冲突。共识方向占中心化残差方差比例为：MNLI-m上43.8%，SNLI上33.7%。

Conclusion: 有效规模是目标特定的测量：谱多样性与分布恢复不应视为可互换的面板质量指标，也不应作为通用的人类替代率。作者提供对齐投票和分析协议以支持此类审计。
...
### [161] [Beyond Atomic Tokens: Factorizing Syllables for Language Model Pretraining](https://arxiv.org/abs/2609.21362)
*Nghia Hieu Nguyen,Thai Bao Huynh,Binh-An Dinh-Le,Phu Gia Hoang,Dat Tien Nguyen,Kiet Van Nguyen,Ngan Luu-Thuy Nguyen*

Main category: cs.CL

TL;DR: 提出面向越南语和汉语的音素化分词器，将音节转为IPA并分解为声母、韵母、声调三个音系成分，三者共享一个上下文位置；进一步构建PhonemicBERT，在多项理解任务上表现竞争力强。


<details>
  <summary>Details</summary>
Motivation: 传统分词器多基于字符或统计子词，忽略音节内部音系结构，且常需大词表；作者希望利用语言学的音系分解实现更紧凑、高效、可解释的文本表示。

Method: 将每个音节转为IPA，并分解为onset、rime、tone三部分，共同占用一个位置；非音系或无法支持单元采用字符级回退。该确定性设计无需语料相关词表学习，中文词表仅112项，越南语256项。再实例化PhonemicBERT，结合因子化成分嵌入和三个预测头重建被掩码的完整音节。

Result: 内在评估显示，该分词器在两种语言中Rényi效率显著更高；对标准越南语音节词典中每个词条的Fertility恰为1，且越南语序列通常短于现有预训练分词器。受控中文预训练下，PhonemicBERT-Zh在多种语言理解任务上达到或超过字符、子词和SubChar方法；PhonemicBERT-Vi也达到或优于已有越南语和多语言预训练模型。

Conclusion: 音系因子化是一种紧凑、高效且可解释的替代方案，可替代原子式或统计式文本表示。
...
### [162] [Prediction Dynamics in Depth-Recurrent Language Models](https://arxiv.org/abs/2609.21383)
*Xinyue Luo,Fei Yu*

Main category: cs.CL

TL;DR: 该论文研究深度循环语言模型为何在中间答案与最终答案一致时分数仍持续变化，并给出边际分解来刻画有限深度下答案保持的机制。


<details>
  <summary>Details</summary>
Motivation: 深度循环模型通过重复潜在更新细化预测；已有幅度约束较保守，作者希望解释中间答案可一致而分数变化的原因，并更精确刻画答案保持现象。

Method: 推导尖锐的边际刻画，将幅度约束的保守性分解为共同平移、相对获胜者的方向、以及每个竞争者更新与其分数差距的配对；在 Huginn-3.5B 和 Ouro-1.4B 上分析完整轨迹，并在全答案文本评分与标签评分下比较。对共享预测分布，正交分离共同运动与对比运动，并用候选集质量和集合内集中度表示共同部分。

Result: 在完整答案文本评分下，考虑更新方向和竞争者配对后，平均最早合格深度相比仅移除平移进一步降低总深度的 22.5%–34.4%；标签评分下也有显著贡献。共同与对比能量可不同速率衰减，因而偏好变化占比上升可与绝对更新缩小并存。

Conclusion: 这些发现从观测分数变化的几何结构与组成出发，解释了深度循环语言模型中的有限深度答案保持现象。
...
### [163] [ArenaFlow: From Trajectory Ranking to Hierarchical Credit Propagation for Open-Ended Agent RL](https://arxiv.org/abs/2609.21378)
...
TL;DR: 提出ArenaFlow，一种用于开放式智能体强化学习的分层信用传播框架，通过锦标赛式相对排名和结构化反思评估，在步骤和技能两个层面传播奖励信号，并在开放式任务上验证有效。


<details>
  <summary>Details</summary>
Motivation: 强化学习在可验证领域显著提升了LLM智能体，但在开放式任务中难以应用，因为解决方案多样且难以获得可靠的标量奖励。现有成对评估方法虽通过相对偏好缓解奖励判别崩溃，但仍将丰富的比较反馈压缩为单一轨迹级奖励，掩盖了关键中间步骤，阻碍成功行为固化为可复用技能。

Method: ArenaFlow采用基于锦标赛的相对排名获取轨迹级奖励信号。每次比较辅以结构化反思评估，提供三类监督：关键成功步骤、可复用策略技能、检索技能的使用归因。在步骤层面，根据锦标赛存活深度将轨迹级优势传播到高置信度的关键步骤；在技能层面，从组级使用归因估计技能效用，通过效用感知的更新、剪枝和检索维护全局技能记忆，并作为未来探索的策略先验。

Result: 大量实验验证了ArenaFlow在开放式智能体任务上的有效性。

Conclusion: ArenaFlow通过分层信用传播有效解决了开放式智能体强化学习中奖励信号稀疏和技能复用问题。
...
### [164] [Consistent Relexicalization of Clinical Documents using Graph-Based Approach](https://arxiv.org/abs/2609.21387)
*Dipankar Das,Atri Mandal,Sandeep Singh,Tushar Shandhilya*

Main category: cs.CL

TL;DR: 本文提出 G-RELIC，一种结合大语言模型与图结构的临床文本重词化方法，通过图映射和确定性时间重定位，在保护隐私的同时提升临床记录的关系完整性与时间一致性。


<details>
  <summary>Details</summary>
Motivation: 临床 NLP 中重词化对敏感信息掩码和合成高保真数据至关重要，但现有方法多依赖独立实体替换，易导致纵向记录中的临床不一致，降低重词化数据集的下游分析价值。

Method: G-RELIC 结合 LLM 与图结构，采用基于图的映射机制优化原始实体与替代实体的一对一对应，并引入确定性时间重定位算法以保持时间一致性。

Result: 在多样真实临床数据集上的实验表明，G-RELIC 显著优于现有基线：关系完整性提升 30.4 个百分点（62.1% 至 92.5%），时间一致性提升 45.9 个百分点（46% 至 91.9%），且未损害临床数据集的隐私基准。

Conclusion: G-RELIC 在降低重识别风险的同时，最大化重词化数据集的分析效用，为临床 NLP 提供更一致、可用的隐私保护数据生成方案。
...
### [165] [Omni Demand Understanding: A Benchmark for Contextual User-Intent Inference in Multimodal Interaction](https://arxiv.org/abs/2609.21392)
...
TL;DR: 论文提出 Omni Demand Understanding (ODU) 这一多模态上下文推断任务及 ODU-Bench 基准，用于评估多模态大模型从音视频交互流中检测并推断用户真实需求的能力。


<details>
  <summary>Details</summary>
Motivation: 现有交互能力基准主要关注回复质量，忽视了更根本的问题：模型能否从复杂多模态交互中正确推断用户潜在需求。真实需求常在语音中表述不充分，需结合多模态线索与对话历史推断，并受歧义、不流畅表达和噪声环境影响；同时请求式语音未必代表真实需求，容易导致误触发。

Method: 将 ODU 定义为多模态上下文推断问题：给定交互流，模型需检测是否存在用户需求，并基于多模态与对话语境推断意图。从五个维度覆盖单轮与多轮交互进行评估。通过挑战驱动分类法、分类引导的智能体视频生成和真人录制交互构建 ODU-Bench，并进行媒体锚定标注与人工验证。共评测 14 个原生多模态大模型（MLLM）。

Result: 即使是表现最强的 Gemini 3.1 Pro，也只能恢复 44.7% 必须从视觉、声学或对话上下文推断的关键信息；14 个模型中有 11 个在非需求场景下的误触发率超过 50%。

Conclusion: 当前多模态大模型在推断上下文用户需求方面存在系统性能力差距。作者希望 ODU 能推动评估多模态交互中此前未被充分探索但至关重要的能力：在生成合适回复之前正确理解用户需求。
...
### [166] [MIRAGE: Multi-Perspective Creative Language Model Reasoning with Reinforcement Learning Guidance](https://arxiv.org/abs/2609.21554)
*Arash Lagzian,Srinivas Anumasa,Dianbo Liu*

Main category: cs.CL

TL;DR: 提出MIRAGE框架，通过多视角推理提升大语言模型在复杂任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在数学、科学和逻辑任务上表现不佳，受人类认知灵活性启发。

Method: MIRAGE包含选择器（优先有效概念视角）和推理器（顺序求解直至自信，否则聚合多视角）。

Result: 在GSM8K、MATH500、MMLU-Pro和Game-of-24基准上，MIRAGE持续优于思维链和多样提示集成方法，显著提升准确率且推理开销极小。

Conclusion: MIRAGE为实际应用提供了可扩展的解决方案。
...
### [167] [Talking Past the Machine: Morality, Politeness, and Alignment in Human-AI Dialogue](https://arxiv.org/abs/2609.21401)
*Marina Mitiaeva,Lu Xiao*

Main category: cs.CL

TL;DR: 本研究分析人类-ChatGPT与人类-人类多轮对话，发现AI虽能产生合作交流的表面特征，但缺乏深层社会架构，且部分合作机制在AI交互中反向作用，表明需从交互层面而非仅轮次层面评估合作。


<details>
  <summary>Details</summary>
Motivation: 探究对话AI是真正参与合作交流还是仅模拟其表面形式，这对AI系统的评估、信任和设计至关重要。研究聚焦道德、礼貌和对齐三个合作对话的核心维度，比较人机交互与人际对话的差异。

Method: 分析15,881段人类-ChatGPT对话和10,784段人类-人类多轮对话，使用混合效应模型识别预测轮次间对齐的特征。

Result: 发现一致分离：AI产生合作交流的表面特征而无深层社会架构。道德输出预先配置而非协商；温暖无面子敏感性；语言趋同持续下降。合作机制反向：人际中促进顺应的模糊/缓和语在AI中降低对齐；人际分歧的纯洁框架在AI中令用户趋同。给予用户塑造空间的能动性是人机与人际中对齐的最一致预测因子；近期模型道德断言降低但未带来更好合作。

Conclusion: AI复现了合作的表面，却缺乏人际合作赖以形成的相互适应。更意外的是，维系人类顺应的机制在AI交互中可能反向运行，表明仅从轮次层面视角不足以确保交互层面的成功。
...
### [168] [Benchmarking Gender Bias in Machine Translation Evaluation Metrics across Occupations](https://arxiv.org/abs/2609.21490)
*Orfeas Menis Mastromichalakis,Giorgos Filandrianos,Wafaa Mohammed,Giuseppe Attanasio,Chrysoula Zerva*

Main category: cs.CL

TL;DR: 本文研究机器翻译评估中的性别偏见，使用基于GAMBIT+的职业平衡子集，在WMT 2026自动翻译质量评估系统共享任务中评估七种英语源语言对，发现男性化翻译总体得分更高，且不同职业的差异符合性别刻板印象，但偏好强度和一致性因评估者和语言而异。


<details>
  <summary>Details</summary>
Motivation: 机器翻译中的性别偏见持续存在，影响生成的翻译及其自动评估。当源文本未指定人物性别时，翻译可能使用男性或女性形式，而翻译系统和评估指标可能在没有依据的情况下表现出系统性偏好。

Method: 在WMT 2026自动翻译质量评估系统共享任务中，使用基于GAMBIT+的职业平衡子集进行研究。涵盖七个英语源语言对（阿拉伯语、捷克语、希腊语、冰岛语、俄语、乌克兰语，并新增德语）。每个目标语言包含1,308对男/女翻译，覆盖436个ISCO-08职业组，每组三个示例。评估共享任务提交和基线在分数预测与错误标注上的表现，考察性别相关差异的方向、幅度和频率。

Result: 总体上，男性化翻译倾向于获得更高分数，且不同职业的差异符合性别刻板印象，但这种偏好的强度和一致性因评估者和语言而有很大差异。

Conclusion: 性别偏见在机器翻译评估中依然存在，但捕捉其程度需要超越单一聚合指标，关注评估者行为的多个互补维度。
...
### [169] [Evaluating In-Context Learning and Retrieval Strategies for Devanagari Post-OCR Correction](https://arxiv.org/abs/2609.21595)
*Abhishek Bhandari,Gaurav Harit*

Main category: cs.CL

TL;DR: 本文首次系统评估了3B至32B大语言模型在印地语和马拉地语OCR后校正中的上下文学习能力，并提出基于字符n-gram BM25的示例检索策略CharBM25。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的上下文学习有望实现无需训练的OCR后校正，但其在天城文（Devanagari）脚本中的有效性尚未被探索。

Method: 构建涵盖五个新闻领域、2万句的基准，比较三种上下文示例检索策略：领域随机选择、稠密语义检索，以及提出的CharBM25。CharBM25根据OCR输入与测试句子的字符n-gram BM25相似度检索示例，以匹配共享错误模式。评估3B至32B参数的大语言模型，并分析检索策略、模型规模、n-gram类型及语言差异的影响。

Result: 检索策略是校正质量的决定性因素：CharBM25在印地语上比领域随机选择绝对WER低2.8至4.0个百分点，在马拉地语上低2.9至3.8个百分点；字符三元组始终优于二元组和一元组。规模主导性能：Gemma-3-27B在CharBM25-5下印地语WER降低55.0%，马拉地语降低33.3%。少样本增益受模型容量限制：低于8B的模型无法稳定优于OCR基线，马拉地语上3B模型甚至使更多句子变差。马拉地语在所有规模下都比印地语更难校正，反映其更高的形态复杂性。CharBM25无需GPU且计算成本可忽略，性能匹配或超过稠密检索。

Conclusion: CharBM25是一种有效、无GPU的检索策略，与12B以上通用大语言模型结合，可在无需任务特定微调的情况下实现可靠的天城文OCR后校正。数据集已公开：https://huggingface.co/datasets/AbhishekBhandari/Devanagari-OCR-ICL-Benchmark。
...
### [170] [Steering LLMs Responses Towards Moral Foundations on the Norwegian MFQ-30](https://arxiv.org/abs/2609.21636)
*Hans Andersen,David Dichas*

Main category: cs.CL

TL;DR: 研究将挪威道德基础问卷 MFQ-30 施测于六个开源大模型，并与1282名挪威受访者比较，测试提示级人格引导与激活级 ActAdd 能否使模型价值画像接近目标人类群体。


<details>
  <summary>Details</summary>
Motivation: 近期研究用人类心理测量问卷评估大语言模型的道德与价值画像，但这些工具是否测量模型中的稳定特质、以及模型画像能否被引导至目标人群，尚不明确。

Method: 对六个开源权重 LLM 施测挪威版 MFQ-30，并与 N=1,282 挪威受访者样本对比；采用两种干预：提示级 persona steering 和激活级 ActAdd。

Result: 一半模型通过注意力检查并认真作答；另一半给出平坦或中心趋势输出，平均看似接近人类但不追踪题项内容。中性“北欧受访者”人格提示（未使用人类样本分布信息）使认真作答模型在 Mahalanobis d² 上接近挪威均值的程度提高44–77%。固定中层单对 ActAdd 会压平道德基础画像，而非引导单个基础。至少一个模型中，同一人格提示既改变画像，也诱导出基线不存在的作答参与。

Conclusion: 部分大模型可对问卷产生稳定且可被人格提示引导的道德基础画像，但另一些模型只是产生伪近似人类的平坦输出；ActAdd 未实现精细引导，且存在类似“认知幻影”的现象，即引导同时诱发新的作答行为。
...
### [171] [Chinese Competitive Debating Dataset and Benchmark](https://arxiv.org/abs/2609.21637)
*Zongrui Yang,Haoyuan Li,Zhongsheng Wang,Zhirui Zeng,Pengqian Han,Yi Zhou,Yuting Wang,Jiamou Liu*

Main category: cs.CL

TL;DR: 该论文构建了一个面向中文竞技辩论的细粒度数据集与基准，用于评估大语言模型在比赛、阶段和辩手层面的理解能力，并定义了胜负预测、阶段得分预测和最佳辩手预测三项任务。


<details>
  <summary>Details</summary>
Motivation: 辩论裁决需要追踪论点在互动中的发展，但现有数据集很少同时包含细粒度辩论转录文本与真实比赛中由专业评委依据统一评分标准给出的判断。

Method: 组织182场比赛，招募120名专业评委，每场由3名评委按预设评分标准独立裁决；剔除记录不完整比赛后，数据集包含148场比赛、2698个阶段、20542个交换单元，并保留人工校验的转录、阶段得分、比赛投票、最佳辩手投票和裁决理由。定义三项任务：获胜倾向预测、阶段得分预测、最佳辩手预测。对多个大语言模型进行零样本评估。

Result: 模型最高获胜预测准确率为66.2%，模型阶段得分与人类平均评分的最高Pearson相关系数为0.250，最高最佳辩手预测准确率为56.8%。

Conclusion: 该数据集和基准为研究大语言模型对互动式论证的理解及其与专业评委判断的一致程度提供了测试平台。
...
### [172] [Analysing the Linearity of Linguistic Relations in Language Model Embedding Spaces](https://arxiv.org/abs/2609.21655)
*Vasudevan Nedumpozhimana,Fathima Thekkekara,John Kelleher*

Main category: cs.CL

TL;DR: 本文提出一个框架，用于分析不同语言关系在语言模型词嵌入空间中被线性编码的强度，并在扩展BATS数据集上比较GloVe、RoBERTa和ModernBERT。


<details>
  <summary>Details</summary>
Motivation: 研究不同语言关系在嵌入空间中是否以及如何被线性表示，并希望提供一种紧凑工具来探测和比较不同模型的关系几何结构。

Method: 通过一个受约束的线性近似方法，在相关词对和不相关词对上形式化线性编码；将该方法应用于扩展BATS数据集，覆盖屈折、派生、词典学和百科类关系，并测试GloVe、RoBERTa和ModernBERT。

Result: 实验显示，屈折和派生关系几乎能被完美线性编码；词典学和百科类关系的误差明显更高，尤其是一对多和多对多关联。RoBERTa和ModernBERT通常比GloVe更线性地编码这些关系。

Conclusion: 该框架能揭示哪些关系结构在嵌入中最容易线性访问，可作为探测和比较模型关系几何的紧凑工具。
...
### [173] [When Steering Fails in Latent Reasoning: A Latent-to-Language Transition Gap](https://arxiv.org/abs/2609.21662)
*Gaoxiang Huang,Lei Qi*

Main category: cs.CL

TL;DR: 论文发现，对连续思维（latent CoT）进行激活引导对后续语言生成的影响明显弱于对显式思维链（explicit CoT）的引导，并提出“潜在到语言转换缺口”来解释这一现象。


<details>
  <summary>Details</summary>
Motivation: 激活引导已被广泛用于控制语言模型的显式思维链推理，研究者希望将其扩展到潜在思维链。但作者发现，即使隐藏表示被移动了相近幅度，引导连续思维对后续语言生成的效果仍远弱于引导显式思维链，因此需要探究其原因。

Method: 作者首先验证连续思维中仍可识别任务信息；随后通过分析输出分布在转换边界处的突然变化，以及任务相关方向在潜在思维链与显式思维链中的双向控制效果差异，来检验所提出的“潜在到语言转换缺口”假设。

Result: 连续思维中任务信息仍然可识别，但对其施加干预后效果难以传递到语言生成；输出分布会在转换边界处发生突变；任务相关方向在潜在思维链中的双向控制能力显著弱于显式思维链。

Conclusion: 这些结果支持存在“潜在到语言转换缺口”，说明潜在空间中的干预效果不能有效传递到语言生成，并指出潜在思维链到语言生成的转换界面是未来评估和设计潜在引导方法的关键目标。
...
### [174] [Rethinking Human-Aligned Evaluation: An Analysis of Semantic Metrics Beyond WER](https://arxiv.org/abs/2609.21663)
*Hritika Sharma,Thibault Bañeras-Roux,Alessandra Pinto,Petr Motlicek,Hyunggu Jung,Esaú Villatoro-Tello,Somang Nam*

Main category: cs.CL

TL;DR: 论文提出面向人类评价的英语ASR数据集HATS-en，并比较WER、CER、BERTScore和SemDist等指标与人类判断的一致性，发现WER最不一致，SemDist最佳配置整体一致性最高，但CER简单低成本且表现接近最优，建议将CER作为主要指标、SemDist作为补充。


<details>
  <summary>Details</summary>
Motivation: WER是ASR最常用指标，但它将所有词汇偏差视为同等代价，不考虑是否改变语义，因此需要检验WER是否真正反映人类对ASR转写质量的判断。

Method: 构建HATS-en英语数据集用于以人为中心的ASR评估；用不同语言模型、层和池化策略配置BERTScore与SemDist，并将其与词汇指标和人类判断进行基准比较。

Result: 在所有测试指标中，WER与人类判断一致性最低；最佳SemDist配置整体一致性最高，优于CER和BERTScore；没有单一模型在所有设置下最佳；CER虽简单低成本，但与最佳配置表现非常接近。

Conclusion: 结果支持将ASR评估转向CER，对英语和词素音节文字系统都更具可解释性且成本低，更符合评估应捕捉的内容；同时建议将SemDist作为补充评估指标。
...
### [175] [PRISM-BN: A Controlled Corpus and Benchmark for Text-to-Parameterized Bayesian Network Extraction](https://arxiv.org/abs/2609.21673)
*Amartya Bhattacharya,Nikhil Singh,Neeti Pokhriyal,Soroush Vosoughi*

Main category: cs.CL

TL;DR: 该论文提出 PRISM-BN：一个包含 5054 条 BN 接地描述与离散参考贝叶斯网络配对的可控语料，用于评估神经符号 AI 从文本抽取参数化贝叶斯网络的能力。


<details>
  <summary>Details</summary>
Motivation: 概率图模型尤其是贝叶斯网络具有显式有向结构和概率参数，适合作为神经符号 AI 的符号目标，但训练文本到参数化 BN 系统所需的大规模配对文本-BN 资源稀缺。

Method: 构建 PRISM-BN 语料与 PRISM 流水线：基于 50 个 Wikipedia 种子骨架生成五个领域的 BN 描述，先抽取边缘分布和局部联合分布，再解析恢复归一化 CPD，并构建局部重参数化子图。基准包含语义节点/状态对齐、条件结构评分和严格全 CPD 评估。

Result: 在六个 LLM 抽取器上，Node F1 为 0.56–0.83，条件 Edge F1 为 0.90–0.97，CPD-KL 为 1.11–3.14；条件状态与边恢复稳定较强，但严格全 CPD 一致性仍具挑战。使用独立生成的 GPT-5.5 参考和人工试点也显示类似趋势。

Conclusion: PRISM-BN 支持将结构恢复与概率参数估计分开评估，揭示了当前 LLM 在 BN 结构抽取上表现较好，但在完整 CPD 精确恢复上仍困难。
...
### [176] [CIBuzzBench: A Benchmark for Cross-Lingual Understanding of Chinese Internet Buzzwords](https://arxiv.org/abs/2609.21722)
*Yifan Wang,Junyu Lu,Qifan Wang,Shun Zhang,Chaozhuo Li,Jiahao Liu,Zhijun Cao,Lingbin Bu,Fanliang Bu*

Main category: cs.CL

TL;DR: 论文研究LLM跨语言理解中文网络流行语的能力，提出首个中英跨语言基准CIBuzzBench，包含3001个流行语及英文释义、等价表达、类别和有害性标注，并设计三类评测任务，发现LLM仍面临显著困难。


<details>
  <summary>Details</summary>
Motivation: 中文社媒流行语含义常非字面且深植于本土文化与语用语境；现有研究多限于中文内部解释，尚不清楚LLM能否跨语言迁移此类文化知识并准确传达英文含义。该能力对安全也很关键，因为有害表达可能借文化特定谐音、委婉、反讽或暗语隐藏冒犯内容。

Method: 构建CIBuzzBench，包含3001个中文网络流行语，标注英文释义、英文等价表达、类别标签和有害性标签；设计三个任务：含义解释、跨语言等价匹配、文化情境有害性检测；在英文和中文提示设置下评测代表性先进专有LLM与中文LLM。

Result: 实验显示LLM在中文网络流行语的跨语言理解上仍表现不佳，尤其在细粒度非字面解释、选项扰动下的稳健等价匹配以及校准有害性检测方面存在明显困难。

Conclusion: 这些发现表明，文化根植型语言现象对多语言LLM和面向安全的评测构成持续挑战；数据集与代码已公开。
...
### [177] [Per-Aetiology Contrastive Severity Embeddings with Phonological Pseudo-Labelling for Multilingual Dysarthric Speech](https://arxiv.org/abs/2609.21789)
*Bernard Muller,Antonio Armando Ortiz Barrañón,LaVonne Roberts*

Main category: cs.CL

TL;DR: 本文比较了混合病因与病因特异性标签空间对多语言构音障碍严重程度识别的影响，发现按病因分别建模普遍优于混合建模。


<details>
  <summary>Details</summary>
Motivation: 现有多语言构音障碍严重程度系统多采用单一病因-语言对训练，或将异质病因合并到同一标签空间；作者希望检验这种合并假设是否合理。

Method: 在共享 HuBERT-base 主干、训练流程、语料注册和留出评估下，训练四个匹配的对比嵌入模型：一个混合病因基线，以及脑瘫（CP）、帕金森病（PD）和肌萎缩侧索硬化（ALS）三个病因特异性模型。训练结合临床标注语音与无需训练的语音学画像方法生成的序数伪标签，并在说话人分离、去泄漏的留出子集上评估。

Result: 病因特异性模型在三个目标病因上均优于混合基线：CP 宏 F1 0.829 对 0.676（相对提升 22.6%），PD 0.715 对 0.511（+40.0%），ALS 0.788 对 0.596（+32.3%）。在 CP 上，额外加入 144 名 SAP 和 44 名 CDSD 伪标签说话人后，宏 F1 从 0.786 提升到 0.829（+4.3 个百分点）。每种病因的训练数据涵盖 3 至 7 种语言。

Conclusion: 该研究将结果定位为对标签空间设计选择的受控比较，并指出伪标签校准、数据划分卫生以及基于置信度阈值的部署是未来工作的重要局限。
...
### [178] [RheoSampling: Resolving the One-Hot Dilemma in Stochastic Dynamic-Tree Speculative Decoding](https://arxiv.org/abs/2609.21827)
*Qiao Hu,Yepeng Weng,Bo Zhang,Takehisa Yairi*

Main category: cs.CL

TL;DR: 论文提出 RheoSampling，通过解耦动态树推测解码中树构建与令牌验证的概率角色，使动态树方法在随机采样下仍保持无损，并提升接受率与加速比。


<details>
  <summary>Details</summary>
Motivation: 动态树方法（如 EAGLE-3）在贪心解码中表现良好，但在随机解码（T>0）时，确定性 top-K 扩展和全局剪枝会将草稿分布坍缩为 one-hot，导致接受率严重下降。动态树牺牲随机采样以保留上下文感知拓扑，静态树则保留随机采样但结构上下文无关。问题根源在于同一概率分布被同时用于建树和验证，二者目标冲突，直接注入随机性困难。

Method: RheoSampling 解耦两个角色：对从草稿分布采样的令牌，分配代理概率用于树扩展和剪枝，同时保留其真实采样概率用于验证；将采样令牌注入确定性 top-K 槽位，并在构建与验证阶段使用不同概率。通过等价类分析将随机树空间压缩为可处理类别，证明无损性；并采用基于最优传输（OT）的验证策略和稀疏草稿机制提升实际效率。

Result: 实验跨多个 LLM 和基准测试表明，RheoSampling 相比最先进的动态树方法在接受率和加速比上均有提升。它是首个同时具备上下文感知 top-K 构建和随机采样且保持无损的动态树方法。

Conclusion: 该工作表明，解耦树构建与验证的概率角色可让动态树推测解码在随机采样下保持无损并实现高效加速，同时为分析随机树结构提供了通用模板。
...
### [179] [Reusing Latent Speech Representations for Query-Conditioned Topic Localization in Transcripts](https://arxiv.org/abs/2609.21844)
*Steffen Freisinger,Philipp Seeberger,Thomas Ranzenberger,Tobias Bocklet,Korbinian Riedhammer*

Main category: cs.CL

TL;DR: 该论文研究查询条件下的主题定位任务，即在长转录文本中预测最能回答主题标题查询的句子跨度。方法复用ASR编码器状态作为句子级表示，并与文本嵌入融合，从而在不额外运行音频编码器的情况下利用语音信息。


<details>
  <summary>Details</summary>
Motivation: 长转录文本作为下游NLP系统输入成本高，且常包含无关上下文，因此需要更高效地定位与查询相关的句子跨度。

Method: 提出查询条件下的主题定位任务，预测转录文本中应对主题标题查询的句子跨度；复用ASR编码器状态作为句子级表示，并与文本嵌入融合，使轻量级跨度定位器能够利用语音信息而无需单独运行音频编码器。

Result: 在两个公开数据集上，该方法相比纯文本基线取得一致提升，在严格边界匹配标准下尤为明显。跨数据集实验表明，结构化或半结构化语音受益最大，而自发语音上的提升有限且结果混合。

Conclusion: 通过复用ASR编码器状态融合语音与文本表示，可有效提升查询条件下的主题定位，尤其适用于结构化或半结构化语音；对自发语音的泛化收益仍有限。
...
### [180] [Do Personality-Tuned LLMs Make Better Social Agents?](https://arxiv.org/abs/2609.21857)
*Tim Krabbe,Xiaodan Shi*

Main category: cs.CL

TL;DR: 该研究探讨人格感知微调能否缩小LLM在社交模拟中与人类行为的疏离感，通过微调Qwen2.5-7B-Instruct和Ministral-8B-Instruct，比较其与指令提示在人格条件对话生成上的一致性、可控性和角色扮演表现。


<details>
  <summary>Details</summary>
Motivation: LLM increasingly用于社交互动代理和机器人社交模拟，虽能模仿人类行为，但仍存在持久的“异类感”。研究希望验证人格感知微调是否能优于单纯指令提示，提升人格条件对话的一致性与可控性，从而减少这一差距。

Method: 使用结合人格标签社交媒体帖子和对话的语料，对两个小型开放权重LLM（Qwen2.5-7B-Instruct、Ministral-8B-Instruct）进行人格感知微调，构建面向社交模拟的人格对话引擎。随后在多个社交互动场景中，用三个独立LLM评判者评估人格保真度并提供基于证据的行为解释，同时量化评分者间一致性和生成对话的词汇特征。

Result: 微调模型在扮演不同人格方面并不优于各自的基线模型；但评分者间一致性较低，限制了结论的可信度。生成文本质量上，微调模型大多与基线相当，其中微调提升了Qwen模型的语言多样性。总体而言结果看似可用，但基线模型表现最佳。

Conclusion: 人格感知微调未能证明可稳定提升人格角色扮演效果，且评估一致性不足。未来研究应更重视训练数据的质量与领域对齐，以实现准确的人格角色扮演。
...
### [181] [TrialAtlas: Multi-Agent Research Organization for Clinical Trial Design and Optimization](https://arxiv.org/abs/2609.21859)
*Jiacheng Lin,Zifeng Wang,Zheng Chen,Erick Scott,Ziwei Yang,Fanyang Yu,Sheng Zhong,Jimeng Sun*

Main category: cs.CL

TL;DR: 该论文提出 TrialAtlas，一个记忆增强的多智能体研究组织，用于临床开发规划（CDP），通过协调文献合成、竞争试验情报、监管先例分析和综合推理等专门智能体，模拟专家协作过程，并基于历史临床试验和监管结果学习。论文同时构建了 TrialAtlasBench 基准，基于 291 份 FDA 完整回复函，评估试验设计缺陷检测、改进建议和技术与监管成功预测三项任务。


<details>
  <summary>Details</summary>
Motivation: 近 90% 进入临床开发的药物最终失败，尽管投入巨大。药企依赖 CDP 和成功概率评估来预判开发风险，但现有决策过程劳动密集且主观，需要临床科学、统计、监管事务和竞争情报等多领域专家共同获取、综合并推理异构证据。因此需要一种能模拟该协作过程、并利用历史开发经验辅助决策的方法。

Method: 提出 TrialAtlas，一个记忆增强的多智能体研究组织，协调面向文献合成、竞争试验情报、监管先例分析以及试验设计与开发风险综合推理的专门智能体。它从历史临床试验和监管结果（包括既往 NDA）中学习，以累积开发经验支撑决策。为评估，构建 TrialAtlasBench，基于 291 份 FDA Complete Response Letters，涵盖三项任务：检测试验设计缺陷、推荐可操作的设计改进、预测技术与监管成功。

Result: TrialAtlas 在缺陷检测上 F1 为 50.0%，比最强基线高 6.1 个百分点；在技术与监管成功预测上达到 85.3% 平衡准确率和 84.7% F1，分别比最佳基线提高 6.7 个百分点（平衡准确率）和 12.0 个百分点（Cohen's kappa）。专家评估中，TrialAtlas 生成关注点 86.4% 被判定有效，OpenAI DeepResearch 为 83.1%，Gemini DeepResearch 为 59.3%。

Conclusion: TrialAtlas 能有效模拟临床开发规划中的多专家协作与证据推理，在真实监管场景下的试验设计缺陷检测、改进建议和成功预测任务中优于强基线，且生成的关注点具有较高专家认可度。该工作展示了记忆增强多智能体系统在药物临床开发风险评估与监管决策支持中的潜力。
...
### [182] [NemotronLabs VoiceChat: An Open Full-duplex Speech-to-Speech Model with Tool Calling Capabilities](https://arxiv.org/abs/2609.21967)
...
TL;DR: 提出 NemotronLabs VoiceChat，一个开源全双工语音到语音模型，原生支持工具调用，将流式语音编码器、仅解码器语言模型、并行文本与函数调用输出流、RNN-T 转录分支和流式 TTS 解码器统一在一个流式架构中。


<details>
  <summary>Details</summary>
Motivation: 现有语音对话系统难以同时实现自然全双工交互、实时语音识别与生成、语言推理以及外部工具调用；研究旨在不牺牲实时对话行为的前提下，将这些能力集成到单一开源语音到语音模型中。

Method: 采用流式语音编码器与仅解码器语言模型结合，设置面向 agent 文本和结构化函数调用的并行专用输出流，加入用于增量用户转录的辅助 RNN-T 分支，并使用流式 TTS 解码器，使模型能边听、边转录、边推理、调用工具并说话。

Result: 在 Full-Duplex-Bench 1.0 上取得所评估开源权重系统中最低的暂停处理接管率、用户打断后 100% 接管率，打断后回复质量 4.33/5；在 Full-Duplex-Bench 1.5 上 93% 情况下可在用户反馈后恢复回复；VoiceBench 归一化平均 55.1；Full-Duplex-Bench 3.0 工具选择 F1 为 82.5%，但参数准确率和端到端工具执行仍有改进空间。

Conclusion: 全双工交互、语音识别与生成、通用语言能力及外部工具使用可以集成在一个开源语音到语音模型中，同时保持实时对话行为。
...
### [183] [Moral Entropy: Auditing Bias and Uncertainty in Moral Judgment](https://arxiv.org/abs/2609.21992)
*Maciej Skorski*

Main category: cs.CL

TL;DR: 本文主张在计算伦理学中不应将标注者对道德内容的分歧简单视为噪声并用多数投票或任一标注者规则消除，而应建模并学习这种不确定性。作者提出 Moral Entropy 贝叶斯框架，保留真实标签的后验分布，并将熵分解为偶然不确定性与认知不确定性，进而用交叉熵/KL、Brier 分数和期望校准误差等审计常用共识规则。


<details>
  <summary>Details</summary>
Motivation: 现有计算伦理研究通常把标注者分歧当作噪声，通过多数投票或任一标注者标记即采纳的宽松规则加以合并，导致不确定性被掩盖而非被建模。作者认为这种分歧本身包含关于道德内容的重要信息，应被显式表示和学习。

Method: 提出 Moral Entropy 贝叶斯框架：对真实标签维持完整后验，并将后验熵分解为 aleatoric uncertainty（关于道德内容的不可约分歧）和 epistemic uncertainty（来自标注不足或噪声）。该框架允许用交叉熵/KL、Brier 分数、期望校准误差等熵与校准方法，将任意启发式共识规则与校准后的真实标签进行对比审计。

Result: 在三个语料库和十五个话语领域上的审计显示，标准聚合规则存在现有流程未报告的偏差：任一标注者规则与校准后验在约30%的项目上不一致，汇总看几乎全是假阳性；但在 foundation 层面错误反转，MFTC 上平均 FPR/FNR 为 19.9%/38.9%。更严格的多数投票和二票规则则漏掉 63%–83% 的真阳性。

Conclusion: 常用标注聚合规则会系统性偏离校准后的道德标签后验，且偏差模式因规则和领域而异。计算伦理学应把标注者分歧作为可建模的不确定性来源，而非投票去除的噪声，并使用校准与熵方法审计和比较聚合规则。
...
### [184] [DiaVLo: Diagnosing Behaviours of Vision-Language Models](https://arxiv.org/abs/2609.22008)
*Lorenzo Corti,Jie Yang*

Main category: cs.CL

TL;DR: 提出DiaVLo诊断框架，结合人工策划与VLM生成能力构建期望和观察到的行为规范，发现潜在错位，并用因果估计识别影响VLM行为的关键概念。在多个开源VLM的分类和生成任务上验证。


<details>
  <summary>Details</summary>
Motivation: VLM依赖子组件存储和传递信息，需验证其是否表现出期望行为并避免有害行为，以保证可靠部署；但现有识别VLM行为的方法仍然稀缺。

Method: DiaVLo利用人工策划和VLM生成能力构建期望行为与观察行为的规范，揭示潜在错位；同时提供因果估计，识别最能影响VLM行为的概念。在多个开源VLM上于分类和生成条件下进行评估。

Result: DiaVLo产生的行为标签与模型性能相关，并为测得的性能提供上下文；它揭示了明显对齐和错位的行为，以及VLM感知、组织和优先排序概念的模式。

Conclusion: DiaVLo能够诊断VLM行为，发现对齐与错位现象，并揭示概念组织模式，有助于VLM的可靠部署。
...
### [185] [RecreationWorld: Scalable and Verifiable Environments for Hybrid Computer-Use Agents](https://arxiv.org/abs/2609.22000)
...
TL;DR: 本文研究混合计算机使用智能体，即能自主在界面探索、软件开发、运行并视觉验证之间切换的智能体，并提出 RecreationWorld 框架与 RecreationBench 基准来评估其在五大平台上的复刻能力。


<details>
  <summary>Details</summary>
Motivation: 真实数字工作往往需要图形界面交互与代码/命令行开发交错进行，而现有计算机使用智能体多沿图形交互或软件开发两条独立路线发展，缺乏对混合工作流的系统研究。

Method: 提出 RecreationWorld：围绕“复刻运行中的参考应用”构建五平台框架，覆盖 Ubuntu、macOS、Windows、Android 和 Web；提供原生 GUI 控制与编码工具的统一执行环境。以运行参考作为隐藏行为测试的 oracle，提供执行接地的奖励，并用高质量开源应用扩展轨迹生成。提出 RecreationBench：250 个跨领域跨平台任务，含基于参考的程序化与视觉断言，经参考验证与人工审核后冻结用于自动评分。

Result: 在轨迹上训练的模型在五个分布外编码与混合计算机使用基准上均有提升，并更频繁地验证渲染输出，显示复刻任务可迁移。GPT-6 Astra 在 RecreationBench 总体得分 58.1%，但仅 2.8% 任务通过全部程序化测试。智能体复刻静态界面结构比复刻交互和计算输出更可靠，生成应用也比参考更小、更单体化。

Conclusion: 混合计算机使用智能体仍是困难问题，尤其在动态交互与计算输出方面；RecreationWorld 与 RecreationBench 提供了可复现评估框架。作者发布基准、环境和测试套件以促进后续研究。
...
### [186] [QuranicMMLU: A Cognitively-Aware Benchmark for Evaluating Generative AI Solutions on Quranic Linguistic Knowledge](https://arxiv.org/abs/2609.22038)
...
TL;DR: 该论文提出 QuranicMMLU，一个从多维度语言学复杂度评估生成式 AI 的古兰经阿拉伯语基准。它构建五支柱分类体系，生成并人工审核 980 道题目，以开放式和多项选择形式评测 12 个系统。


<details>
  <summary>Details</summary>
Motivation: 现有古兰经基准多聚焦通用问答和语义检索，未专门探测具体语言能力，也未按认知需求和经文难度分层，因此需要更严格、语言学基础的评估框架。

Method: 构建涵盖音系、形态、句法、语义、语用五大支柱、31 个叶节点的古兰经分类体系；按 Bloom 认知层级和经文困惑度生成问题；用 LLM 作为评委独立作答与打分，再转人工审核；最终得到 980 道人工审核题，每题含开放式和多项选择两种形式；对 12 个系统进行基准测试。

Result: 伊斯兰专门化模型表现领先；所有系统在多项选择上的准确率平均为 84%，高于开放式回答质量平均 60%；两种评测排名高度一致（Kendall's τ=0.73），但多项选择评分会掩盖去掉选项后才暴露的失败。

Conclusion: QuranicMMLU 为古兰经领域的阿拉伯语 NLP 提供了一个严格且以语言学为基础的评估框架。
...
### [187] [An Interpretable Memory Decision Controller for LLM Agents Based on Three-Signal Complementarity: Decoupling Confidence and Consistency](https://arxiv.org/abs/2609.22043)
*Yiming Zhang,Jinghong Zhang,Haoran Zhao,Yiren Ma,Chunlei Zhao*

Main category: cs.CL

TL;DR: 提出记忆决策层（MDL），一个零参数控制器，用于判断检索到的记忆是否可信，以减少RAG中的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统主要关注检索效率，忽视记忆可信度；冲突记忆导致RAG盲目注入并放大幻觉，受前额叶皮层记忆信号机制启发。

Method: MDL位于检索与生成之间，包含三信号互补编码器（相关性、可靠性、任务风险），通过QR正交子空间投影和元工作记忆信号融合成可解释决策表示；解耦置信度与一致性，引入风险反转和显式弃权。

Result: 在主流LLM和开源数据集上，冲突记忆下幻觉率降低约56.04%，高风险场景接近零幻觉；完全白盒，仅几何运算，无需训练参数，每次决策约0.14毫秒，比嵌入检索快约50倍，比LLM自评估快4-5个数量级。

Conclusion: MDL有效且高效地解决了RAG中记忆可信度问题，具有可解释性和生物启发，显著降低幻觉。
...
### [188] [Cross-sector generalization of accident-process role classification in occupational accident narratives](https://arxiv.org/abs/2609.22081)
*Aho Yapi,Pierre Latouche,Arnaud Guillin,Yan Bailly*

Main category: cs.CL

TL;DR: 评估法语职业事故叙述中事故过程角色分类的跨行业泛化能力，发现任务特定适应能提高跨领域迁移，最佳策略平均平衡准确率约85.6%至85.8%。


<details>
  <summary>Details</summary>
Motivation: 职业事故叙述包含工作情境、不利条件、事故事件及后果等有价值信息，自动结构化可促进大规模事故分析和职业风险预防。但不同行业和组织描述事故的术语和写作风格差异大，自动编码系统的跨领域泛化能力存疑。

Method: 构建专家标注语料库，将事实单元分为四种角色：工作情境(A0)、明确报告的不利条件(A1)、事故事件或偏差(B)、报告后果(C)。分类器仅在建筑业叙述（42,244个事实单元，来自6,040个叙述）上开发和选择，然后在冶金、化学-塑料以及独立公司语料上评估，不重新训练或目标领域调参。比较冻结预训练表示、任务特定微调和监督表示学习策略。

Result: 任务特定适应在跨领域迁移上始终优于冻结表示。三种领先的任务适应策略在三个目标语料上平均平衡准确率在85.6%到85.8%之间。

Conclusion: 研究结果支持开发可迁移的辅助编码系统，能够一致地结构化异构职业事故叙述，供专家审查和跨行业预防分析。
...
### [189] [WM-VS: Progress-Aligned World Models for Closed-Loop Visual Servoing](https://arxiv.org/abs/2609.20892)
*Guanzhong Sun,Junyi Ma,Yixuan Zhou,Yuxuan Wu,Yanzi Miao,Hesheng Wang*

Main category: cs.RO

TL;DR: 论文提出 WM-VS，一种面向闭环视觉伺服的目标中心、进度对齐世界模型框架，用于解决预测与控制不匹配问题，使动作预测不仅合理，还能判断是否降低任务误差。


<details>
  <summary>Details</summary>
Motivation: 闭环视觉伺服中，传统预测往往只关注动作是否合理，而未明确预测该动作能否减小任务误差，这导致预测与控制目标不一致，即 prediction-control mismatch。

Method: 利用离线目标区域 DINOv2 对应关系定义平移、尺度和面内旋转的带符号四维伺服坐标。第一阶段让动作条件潜在转移与该坐标对齐；第二阶段冻结世界模型，训练反应式关节速度策略，结合动作模仿、后果监督和短时想象 rollout，偏好误差收缩。部署时仅用 RGB、反应式控制，无需在线轨迹优化。

Result: 在真实 7-DoF eye-to-hand 系统上，WM-VS 在 30/30 次试验中使角点 RMSE 不超过初始值的 10%，最终有效帧保持该标准的比例为 25/30（83.33%）。去掉未来误差对齐后，保持率降至 26.67%。学习到的进度信号与未用于训练或控制的 AprilTag 外部角点误差一致，平均 Spearman rho=0.8778。无需重新训练，两个未见 3D 目标分别实现平移误差降低 86.48% 和 90.27%，旋转误差降低 70.01% 和 65.70%。

Conclusion: 结果表明，将动作后果与进度对齐可支持重复闭环校正和迁移，为闭环视觉伺服提供了有效的世界模型框架，代码和数据将开源发布。
...
### [190] [AeRove: A Compact Bimodal Aerial-Terrestrial Drone with Rapid Bistable Reconfiguration for Close-Range Pipeline Inspection](https://arxiv.org/abs/2609.20965)
*Caleb Polillio,Petras Swissler*

Main category: cs.RO

TL;DR: AeRove 是一种紧凑型双模态空中-地面机器人，利用螺旋桨护罩作为轮子沿管道滚动，并通过弹簧加载双稳态机构在 200 ms 内切换飞行与地面模式，以降低巡检功耗并提升气体检测性能。


<details>
  <summary>Details</summary>
Motivation: 标准无人机在近距离管道巡检中面临悬停功耗高、气流敏感以及螺旋桨下洗气流干扰气体传感等问题，因此需要一种能兼顾地面连续移动和空中越障的巡检机器人。

Method: 将螺旋桨护罩设计为轮子用于管道滚动；采用弹簧加载双稳态机构实现地面与飞行模式切换，且维持状态无需持续驱动功耗；利用收敛型护罩几何提升推力效率；飞行仅用于越障，地面滚动用于连续行进；在 51 cm 直径钢管上开展自主导航、越障和泄漏检测试验，并单独进行 CO2 传感实验，比较栖停与悬停、机身底部螺旋桨进气与延伸探头配置。

Result: 地面滚动相比持续飞行将电流从 10 A 降至 0.7 A，降低 14 倍，估计行程从 144 m 延长至 2057 m；200 ms 内完成模式切换且无需持续驱动维持；自主试验实现直管和弯管导航、越障及模拟检测标记泄漏检测；CO2 实验中栖停检测响应明显大于悬停，飞行时底部螺旋桨进气配置比延伸探头响应更快更强。

Conclusion: AeRove 表明将地面滚动与空中越障结合可显著降低巡检功耗、延长行程，并通过栖停和螺旋桨诱导气流改善气体检测，适用于近距离管道巡检；代码和设计以 CC-BY 许可发布。
...
### [191] [Shake to Learn: Dynamic Interrogation of Hidden Object Physics for Robotic Manipulation with Physical Reservoir Computing](https://arxiv.org/abs/2609.20970)
*Wen Sin Lor,Jun Wang,Suyi Li*

Main category: cs.RO

TL;DR: 本研究利用折纸启发软体机械臂作为物理储层计算机，通过固定摇晃输入和环形响应推断隐藏物体物理属性（如质心位置），并用于后续重抓取。


<details>
  <summary>Details</summary>
Motivation: 许多与机器人操作相关的物理属性无法从视觉中获取。例如，密封物体在被提起、摇晃或动态扰动之前，几乎无法揭示其质心或内部内容。因此，研究动机是通过交互引起的动态响应来推断传统传感无法获取的物体物理属性。

Method: 使用折纸启发的软体机械臂作为物理储层计算机。抓住物体后，在基部施加固定摇晃输入，通过相机跟踪或嵌入式传感器记录环形响应。由于输入恒定，隐藏物体属性（如质心位置）通过其对机器人-物体耦合系统动力学的影响被编码。然后使用轻量级线性读出解码这些动力学，恢复可解释的隐藏物体物理信息。还开发了环形响应的动态摘要表示以提高预测精度。

Result: 软体机械臂储层完成了三个难度递增的任务：推断隐藏质心的方向、推断质心与抓取点的距离、以及利用推断的质心信息指导后续重抓取。动态摘要表示提高了预测准确性。

Conclusion: 这些结果确立了“摇晃学习”机械询问作为一种有前景的策略，使机器人系统能够将短暂的物理交互转化为关于隐藏物体属性的可操作线索，用于下游操作。
...
### [192] [PIVOT: Physically Informed Vision-Language Off-Road Traversability for Field Robot Navigation](https://arxiv.org/abs/2609.20983)
*Aoran Jiao,Wenda Zhao,Hshmat Sahak,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: 该论文提出 PIVOT，一种物理信息引导的视觉-语言越野可通行性导航系统，用 VLM 语义推理增强传统几何规划，并在真实越野路线中显著提升自主导航能力。


<details>
  <summary>Details</summary>
Motivation: 传统基于几何的地形评估计算快，但在非结构化环境中往往过于保守，难以充分发挥越野移动机器人的自主导航能力。

Method: 将几何规划作为常规模式，并引入 VLM 语义推理；量化 VLM 预测的通行能量代价、机器人振动和车轮打滑与真实测量值的相关性；按各模态预测-测量相关性加权，构建统一可通行性评分；设计两级导航架构，仅当几何规划找不到路径时触发语义重规划。

Result: 在总长约 6.4 km 的混合地形路线进行 5 次重复闭环试验：整体自主率从 59.6% 提升至 97.0%，人工干预从 11 次降至 3 次，平均干预间隔距离（MDBI）从 69.2 m 提升至 412.9 m。

Conclusion: 物理信息引导的 VLM 地形评估可显著突破纯几何导航的局限，同时保留高效几何规划作为常规模式。
...
### [193] [ForeTac-VLA: A Forecasting-Based Tactile-Vision-Language-Action Model for Contact-Rich Robotic Manipulation](https://arxiv.org/abs/2609.20980)
*Zhengyu Tao,Xin Li,Xin Wang*

Main category: cs.RO

TL;DR: 本文提出ForeTac-VLA，一种基于预测的触觉-视觉-语言融合模型，通过预测未来触觉状态来指导机器人动作生成，在四个真实接触丰富操作任务中平均成功率达95%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作（VLA）模型依赖视觉感知，在接触丰富环境中鲁棒性不足，因为关键物理交互状态可能无法视觉观测；现有触觉增强VLA方法多为被动反应式，未显式建模接触的演化过程。

Method: ForeTac-VLA将近期触觉观测编码为时序表示，通过双向交叉注意力与视觉-语言特征融合；使用基于Transformer的预测模块预测多步未来触觉状态，使模型能同时推理观测和预期的接触；将融合的多模态表示和预测的未来触觉状态输入VLA骨干网络以条件化动作生成；采用真实值到预测的课程学习稳定训练。

Result: 在四个真实世界接触丰富操作任务上，ForeTac-VLA平均成功率达95%，比微调VLA模型高36.25个百分点，比最先进的触觉增强VLA基线高22个百分点以上；在低光照和视觉杂乱条件下也保持强性能。

Conclusion: ForeTac-VLA通过显式预测未来触觉状态，有效提升了接触丰富环境下的操作鲁棒性和成功率，优于现有VLA及触觉增强基线，并在视觉条件不佳时仍表现良好。
...
### [194] [Do Spinning Radar Doppler Velocity Measurements Improve Vehicle Detection and Tracking?](https://arxiv.org/abs/2609.21000)
*Eric Xie,Daniil Lisus,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: 本文研究旋转式FMCW雷达的逐方位多普勒速度测量能否提升车辆检测与跟踪性能，并提出利用多普勒信息进行去畸变和运动先验的方法。


<details>
  <summary>Details</summary>
Motivation: 旋转FMCW雷达因对恶劣天气鲁棒且具备360°视场，在自动驾驶感知中日益流行，且扫描雷达可提供逐方位多普勒速度，因此有必要验证这些多普勒测量是否有助于提升旋转雷达的车辆检测与跟踪。

Method: 检测方面，估计自车运动以消除雷达图像中的多普勒距离畸变，再输入网络；跟踪方面，提出估计每辆车速度并作为跟踪器运动模型先验的新方法。由于缺少带动态目标真值标签的多普勒旋转雷达数据，作者构建自动标注流程，使用微调的现成激光雷达检测器集成标注Boreas Road Trip数据集的全部643 km，再将检测结果迁移到雷达，并使用超过250 km车辆密集序列作为真值训练数据，训练和评估两种最先进检测器。

Result: 多普勒去畸变可将检测平均精度（mAP）最高提升2.37个点；多普勒速度先验可将多目标跟踪精度（MOTA）相较零速度初始化基线提升13.68个点，同时达到使用真值速度作为先验时MOTA的99.7%。

Conclusion: 多普勒速度测量能够有效改善旋转雷达的车辆检测与跟踪性能，所提出的去畸变和速度先验方法具有实用价值。
...
### [195] [Project SCOUT: Interceptor Drone for Perimeter Defense](https://arxiv.org/abs/2609.21005)
*Azmain Yousuf,Siwei Cai,Knut Peterson,Lifeng Zhou,David Han*

Main category: cs.RO

TL;DR: 本文提出了SCOUT，一个集成ROS的机载感知与控制框架，用于实时无人机防御。它结合TensorRT加速的检测、ByteTrack关联和轻量级轨迹保持状态机，在真实飞行中实现了92.2%的有效检测率。


<details>
  <summary>Details</summary>
Motivation: 未经授权的无人机迅速增多，对鲁棒且抗干扰的反无人机周界防御系统需求日益增长。

Method: SCOUT框架在防御无人机上直接进行视觉检测、目标关联、轨迹滤波和控制命令生成。感知流程包括TensorRT加速的无人机检测、ByteTrack关联和轻量级轨迹保持状态机，状态机拒绝突变目标并保持短期连续性。通过硬件部署执行平面“守门”拦截策略进行验证。

Result: 真实飞行结果表明，SCOUT在实时机载检测速率下，92.2%的帧保持有效目标检测，证明了视觉跟踪和闭环控制用于无人机周界防御的可行性。

Conclusion: SCOUT展示了机载视觉跟踪与闭环控制用于无人机周界防御的可行性，并提供了在线视频演示。
...
### [196] [SPARROW: Survival-POMCP for Adaptive Robot Routing, Observation, and Waiting](https://arxiv.org/abs/2609.21008)
*Hshmat Sahak,Aoran Jiao,Nicholas Rhinehart,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: 论文研究机器人在临时障碍物下的序贯导航问题，提出基于POMCP的信念空间规划器SPARROW，决定等待、绕行或获取更多信息，并在仿真和真实机器人上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 机器人规划路径中可能出现临时障碍，是否等待障碍清除、绕行或先观测获取信息，构成序贯决策问题，需要在不完全可观测条件下权衡导航时间与信息收集成本。

Method: 将临时障碍物下的图导航建模为部分可观测半马尔可夫决策过程，提出SPARROW规划器，基于POMCP在遍历、观测和有限时长等待动作上搜索，并维护关于潜在障碍类别和清除时间的粒子信念。在线学习类别条件生存模型，利用清除观测和未观测到清除即绕行的右删失遭遇。生成模型模拟障碍到达和清除。引入学习价值准则，权衡收集标注生存数据的即时成本与未来导航后悔的预期降低。

Result: 在两个仿真图和多种障碍类别设置下，SPARROW相比近期基于生存模型的OSCAR方法，平均到达目标时间减少12-26%。在物理移动机器人上，SPARROW相比OSCAR平均到达目标时间减少20.5%，并能随环境条件变化选择性地观测、等待和绕行。

Conclusion: SPARROW能有效处理临时障碍下的序贯导航，通过信念空间规划和在线生存学习，在仿真与真实机器人中均优于OSCAR，说明结合信息获取、等待与绕行决策可提升导航效率。
...
### [197] [Towards Effective Visual-Inertial SLAM with Passive-Only Sensors for Low-Cost Autonomous Underwater Vehicles](https://arxiv.org/abs/2609.21015)
*Grant Schwidder,David Widhalm,Junaed Sattar*

Main category: cs.RO

TL;DR: 本文证明使用低于1万美元的消费级传感器平台可在水下实现可用的视觉惯性SLAM。


<details>
  <summary>Details</summary>
Motivation: 推动先进海洋机器人从专业实验室走向更广泛的研究和爱好者应用，高端AUV依赖昂贵传感器（如DVL和USBL），需要低成本替代方案。

Method: 使用开源AUV和消费级传感器（立体相机、MEMS IMU、深度传感器），在完全无约束的6自由度水下环境中评估，分析现成SLAM包，并提出传感器融合优化以应对水下视觉和物理挑战。

Result: 结果表明，仅用低成本平台即可实现稳健、高质量的导航，使SLAM解决方案能为大众所用。

Conclusion: 为要求苛刻的实时海洋任务提供了期望基准，消除了工业级硬件的财务障碍。
...
### [198] [Catch Me If You Can: Real-Time Feedback Denoising for Responsive VLAs](https://arxiv.org/abs/2609.21022)
*Yiheng Ji,Xingru Zhou,Luis Sentis,Mingyo Seo*

Main category: cs.RO

TL;DR: 提出VLA-Feedback，一种双时间尺度架构，将低频扩散动作规划与高频视觉反馈结合，在执行前用最新观测修正每个动作，从而提升动态场景和真实机器人任务的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型中，基于扩散的动作生成器虽能生成时序连贯的动作块，但通常在推理后开环执行，难以应对执行过程中物体移动、接触变化或场景演变，导致响应能力受限。

Method: VLA-Feedback保留扩散规划最终去噪步作为轻量级反馈接口，使每个动作在执行前可根据最新观测进行修正；采用低频扩散规划与高频视觉反馈的双时间尺度设计，无需重新运行完整的视觉-语言-扩散模型。

Result: 在静态LIBERO任务上与GR00T表现相当；动态仿真任务平均成功率从27.5%提升至85.0%；真实机器人任务平均成功率从51%提升至73%。

Conclusion: 该方法在保留扩散规划器表达能力和动作生成质量的同时，实现了实时动作修正，显著增强了动态环境和真实机器人任务中的鲁棒性与成功率。
...
### [199] [Adapting Rigid-Body Dynamics Derivatives for Constraint Embedding Closed-Chain Models](https://arxiv.org/abs/2609.21024)
*Daniel J. Volpi,Patrick M. Wensing*

Main category: cs.RO

TL;DR: 本文将刚体动力学一阶导数算法推广到使用约束嵌入建模的闭链运动系统，并分析其计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有高效动力学导数方法假设关节速度效应在局部构型下不变，无法直接适用于更一般的关节类型和约束嵌入闭链模型；而模型预测控制与可微仿真需要更准确的动力学计算。

Method: 移除局部构型不变假设，推导适配算法，将动力学导数扩展到一般关节类型及约束嵌入闭链模型，并比较传统销轴关节模型与包含局部闭链的更完整驱动模型。

Result: 广义化引入的额外项在仅建模驱动运动学时代价较低；但当驱动链包含额外刚体（如电机转子）或考虑非局部闭环时，计算成本会更高。

Conclusion: 该工作使约束嵌入驱动子机构能进行更准确的动力学计算，可应用于模型预测控制和可微仿真。
...
### [200] [MAPLE-RF: Efficient Probabilistic RF Source Localization in Partially Explored Environments](https://arxiv.org/abs/2609.21026)
*Haozhe Lei,Sundeep Rangan*

Main category: cs.RO

TL;DR: 研究部分探索地图中的单快照射频发射器定位，比较数字孪生扩展方法与MAPLE-RF，二者均输出发射器位置后验。


<details>
  <summary>Details</summary>
Motivation: 机器人应用中常仅有部分地图，尤其SLAM探索时无法获得完整环境模型，因此需要不依赖完整地图的RF源定位方法。

Method: 提出两类方法：1）扩展数字孪生方法，将未探索空间视为自由，并在混合地图覆盖上训练，对每个候选位置进行射线追踪；2）MAPLE-RF，将估计路径到达角与信噪比编码为与地图已知度、占据和LOS可见性对齐的网格通道，用U-Net一次评分所有候选位置，推理时无需传播模拟。基于室内环境射线追踪仿真评估。

Result: 混合地图覆盖训练对两种方法都至关重要；数字孪生方法在多数单快照指标上更准，MAPLE-RF接近其精度，但查询成本不依赖传播模型，比通用射线追踪的全网格新查询低两个数量级以上。两者均优于高斯和高斯混合基线；在自估计引导的探索路径上，融合MAPLE-RF后验比对比方法在源附近放置更多概率。

Conclusion: 在部分探索地图下，数字孪生方法精度更高，MAPLE-RF以极低推理成本达到接近效果，并可融合用于探索；混合地图覆盖训练是必要环节。代码和数据将发布。
...
### [201] [DEXTERA: From a Single Image to Deployable Dexterous Manipulation via Real-to-Sim-to-Real](https://arxiv.org/abs/2609.21045)
*Jin Wu,Lianjie Yuan,Zeyan Sun,Yuanyuan Lei,Disi A,Bicheng Han,Fangzhou Xia*

Main category: cs.RO

TL;DR: 提出DEXTERA，一个自动化实-仿-实框架，从单张RGB图像生成可部署的灵巧操作策略，包含四个统一阶段。


<details>
  <summary>Details</summary>
Motivation: 真实世界灵巧操作数据采集成本高、耗时长；高保真仿真器虽可扩展数据合成与策略学习，但手动构建可部署数字孪生劳动密集，且视觉、几何和动力学残差阻碍可靠的sim-to-real迁移。

Method: DEXTERA框架四个阶段：(1)单图像场景分解为静态高斯背景与可交互刚体/铰接资产，并用VLM推断物理参数；(2)度量场景全局对齐、物体规范化与形态平衡的机器人校准；(3)可扩展仿真任务原语构建、VR遥操作与物体中心轨迹合成；(4)共享多模态策略接口，支持模仿学习与强化学习。

Result: 在13个任务-本体对、2个灵巧机器人平台、6种策略架构上评估。视觉保真度和3D几何重建优于生成基线，跨域轨迹回放验证物理交互一致性。仅仿真训练策略可实现零样本真机部署，仿真-真实协同训练将平均物理策略成功率从29.2%提升至61.9%。

Conclusion: DEXTERA实现了自动化实-仿-实灵巧操作，减少了手动构建数字孪生的需求，有效缩小sim-to-real差距，支持零样本部署和协同训练提升性能。
...
### [202] [PlantShade: Predicting Plant Shadows for Lighting-Aware Robotic Agricultural Operation](https://arxiv.org/abs/2609.21059)
*Longchao Da,Xiaoou Liu,Xingjian Li,Lirong Xiang,Hua Wei*

Main category: cs.RO

TL;DR: 本文聚焦植物遮阴模拟，构建了包含大豆、番茄、甜菜和草莓四种植物、多生长阶段且具动态阴影的植物生长与遮阴数据集，并提出基于扩散模型的生成式遮阴模拟方法，支持未见过植物的真实感阴影生成及下游机器人任务。


<details>
  <summary>Details</summary>
Motivation: 植物生长和农业生产对国家可持续发展与人类生活至关重要，而遮阴是影响植物生长的关键因素。前沿人工智能为科学农业提供了提升作物生产力的潜力，因此需要推进植物遮阴模拟研究并促进更广泛的社会效益。

Method: 构建了一个综合的植物生长与遮阴数据集，涵盖四种植物、俯视视角、沿圆形轨迹的补充光源、多生长阶段动态阴影和多样观察复杂度；提出基于扩散模型的生成式遮阴模拟方法，并引入时间条件以实现不同时间阶段的灵活阴影生成，支持感知、光照控制和视角规划等下游机器人任务。

Result: 所提方法能够为未见过植物生成逼真的阴影，并适应不同时间阶段的遮阴模拟；论文通过定量和定性评估验证了模型性能。

Conclusion: 该工作为植物感知的遮阴建模提供了基础性研究，对农业和机器人等更广泛应用具有意义。
...
### [203] [Constraint-Unified MPC for Over-Actuated Surface Vehicles with Post-Detection Fault Reconfiguration](https://arxiv.org/abs/2609.21046)
*Sebastian Burmester,Ruiheng Jiang,Noa Sendlhofer,Raffaello D'Andrea,Aswin Ramachandran*

Main category: cs.RO

TL;DR: 论文报告一个已部署的自主水面车辆编队系统，用于水上编舞表演，在推进器力与速率限制及故障下实现精确轨迹跟踪。


<details>
  <summary>Details</summary>
Motivation: 编舞式水上表演需要小型自主水面车辆在每推进器力和速率限制下精确跟踪路径，并且推进器故障后仍需保持可控与跟踪性能。

Method: 将轨迹跟踪、推力分配以及每推进器力和速率限制统一建模为关于推进器命令的单个二次规划；外部故障检测器为每个推进器提供一个二值标志以进行故障重构；并与自研级联基线对比。

Result: 系统已在苏黎世湖和威尼斯 Time Space Existence 2025 展览中现场部署。10分钟悬停位置RMSE为1.6 cm；以0.6 m/s跟踪10 m方形路径为4.3 cm。0.5 m/s时失去前推进器误差增至11.6 cm，失去右舷推进器增至11.5 cm；同时失去两个推进器时仍能以0.4 m/s跟踪方形，RMSE为1.25 m。与级联基线相比，统一公式名义跟踪精度相当，但悬停更紧且推进器力约减半；故障后统一控制器保持在参考路径24 cm内，而级联控制器偏离。

Conclusion: 统一二次规划公式可在单一框架内处理轨迹跟踪、推力分配、执行器限制和故障重构，并已在实际表演中部署；推进器故障后其性能明显优于级联基线。
...
### [204] [Design of Adaptive PID Controller Based On Asynchronous Advantage Actor Critic Learning Method for QuadCopter Control](https://arxiv.org/abs/2609.21082)
*Ali Jokar,Aria Alasty*

Main category: cs.RO

TL;DR: 本文提出将异步优势演员-评论家（A3C）算法与PID控制器结合，用于四旋翼姿态和轨迹跟踪，并通过仿真验证其控制性能。


<details>
  <summary>Details</summary>
Motivation: 四旋翼具有非线性强、对扰动敏感的特点，传统基础PID控制器难以应对这些复杂控制挑战。

Method: 使用A3C算法通过并行智能体动态优化PID参数，并利用神经网络实现参数调整；同时引入系统辨识模块预测系统状态，以辅助最优控制策略；将所提框架与标准A2C模型进行对比。

Result: 仿真结果表明，A3C和A2C均能实现准确跟踪；但A3C控制器在损失函数上收敛更好，奖励曲线和损失曲线表明其参数优化效果更优。

Conclusion: 基于A3C的方法能提升四旋翼控制性能，有效融合强化学习与传统控制，获得更高的适应能力。
...
### [205] [Dynamic Modeling and LQR Control of a Single Coaxial Drone with 2DOF Thrust Vectoring Mechanism](https://arxiv.org/abs/2609.21099)
*Ali Jokar,Amin Talaeizadeh,Aria Alasty*

Main category: cs.RO

TL;DR: 本文提出一种新型共轴旋翼无人机，通过二自由度摆锤机构实现主动推力矢量控制，以较低机械复杂度解决滚转/俯仰欠驱动问题。


<details>
  <summary>Details</summary>
Motivation: 共轴旋翼无人机虽能效高、体积小，但滚转和俯仰控制存在固有欠驱动；传统斜盘方案虽可解决该问题，却显著增加机械复杂度，因此需要更简单的替代方案。

Method: 建立包含机体、舵机臂和电机组件全部惯性贡献的拉格朗日动力学模型；在悬停平衡点线性化并设计LQR控制器；考虑执行器动态和传感器噪声进行高保真仿真；采用EKF融合GPS、气压计和IMU进行状态估计。

Result: 仿真验证了所提架构的可行性；EKF融合估计精度较高；结果表明该方法在节能、快速共轴无人机应用上具有潜力和可靠性。

Conclusion: 二自由度摆锤推力矢量机构为共轴无人机提供了一种机械更简单的欠驱动解决方案，结合LQR与EKF可实现可靠控制与状态估计，具有实际应用前景。
...
### [206] [VLA-Scope: Shift-Aware Failure Prediction for Vision-Language-Action Models](https://arxiv.org/abs/2609.21246)
*Kaiwen Zhu,Dongfang Liu,Liangkai Liu*

Main category: cs.RO

TL;DR: 提出VLA-Scope两阶段框架，结合输入分布偏移检测与分类以及执行历史，在OOD场景中预测视觉-语言-动作模型的执行失败。


<details>
  <summary>Details</summary>
Motivation: VLA模型可映射视觉观察和自然语言指令到机器人动作，但分布偏移会损害可靠性；且模型在OOD条件下仍可能成功，因此仅检测OOD输入不足以预测执行失败。

Method: 第一阶段使用池化的图像和语言表示检测OOD输入并分类偏移类别；第二阶段针对OOD输入，结合预测类别、动作前缀特征和执行进度特征，使用跨偏移类别共享的逻辑回归模型随执行过程更新失败风险。在OpenVLA和10个LIBERO-Spatial任务上采用留一组交叉验证评估。

Result: OOD检测ROC-AUC为0.9454，偏移分类准确率为91%。在全部1400条OOD rollout上，失败预测器执行60个动作后ROC-AUC为0.8497，优于不使用执行进度特征的0.7906，也优于ActProbe和SAFE-MLP基线。

Conclusion: 结合动作特征与时间聚合的执行步骤表示，可改善输入分布偏移下的执行失败预测。
...
### [207] [Learning Scene-Aware Humanoid Locomotion through 3D Clutter from Immersive Human Demonstrations](https://arxiv.org/abs/2609.21107)
*Beichen Wang,Tong Xu,Daniel Kosukhin,Yuen-Hei Yeung,Yuanjie Lu,Xuesu Xiao*

Main category: cs.RO

TL;DR: 本文提出MTC框架，通过VR数据采集与场景感知动作重定向，训练可在复杂三维障碍环境中避障的人形机器人运动策略，并在Unitree G1上实现70.2%无碰撞穿越成功率。


<details>
  <summary>Details</summary>
Motivation: 现有学习人类动作的方法虽能让仿人机器人完成舞蹈、武术等动态技能，但在密集杂乱的三维受限环境中穿越仍研究不足，需要全身运动与场景几何紧密耦合的场景感知运动能力。

Method: 提出Moving Through Clutter（MTC）学习示范框架：使用程序生成的VR环境进行沉浸式数据采集；提出场景感知动作重定向算法，将人类示范转为仿人机器人轨迹，并严格保证机器人与场景的间隙以实现无碰撞；用这些参考轨迹训练场景感知运动策略，部署于Unitree G1。

Result: 在提出的MTC-Challenge多障碍穿越任务中，策略在多种场景下达到70.2%的无碰撞率，并能通过爬行穿越低净空通道、挤过狭窄缝隙等全身技能完成复杂环境穿越。

Conclusion: MTC表明结合VR数据采集、场景感知重定向与策略学习，可有效提升人形机器人在密集杂乱三维环境中的无碰撞全身运动能力。
...
### [208] [Dynamics-Induced Commitment in Learning-Based Robotic Penalty Kicks](https://arxiv.org/abs/2609.21100)
*Ruize Geng,Hao E. Zhang,Yisen Li,Yikai Wang,H. Eric Tseng,Ding Zhao*

Main category: cs.RO

TL;DR: 该研究在层级化人形-四足点球系统中，结合自博弈策略与固定足球全身控制器，提出DIC-Map分析身体动态对承诺与可延续性的约束，并研究响应方估计器对学习及扑救表现的影响。


<details>
  <summary>Details</summary>
Motivation: 机器人博弈不仅受策略信息限制，也受身体实际执行能力约束；理解这种耦合对分层控制与自博弈学习至关重要。

Method: 构建人形射门与四足扑救的层级点球系统：射门技能由自采动作捕捉初始化，扑救技能通过强化学习获得。提出DIC-Map，估计延续能力、识别终端替代的首次持续丧失，并检验剩余交互是否可约简为零和博弈；推导对称终端下策略集中度的闭式上界，并分析估计器介导的一阶学习通道。

Result: 实验将承诺定位在接触前约0.29秒；仅改变球速会移动延迟覆盖范围。在四种响应策略中，替换估计器将扑救率从0.240提高到0.472，而通过等待获得的类似读准提升仅使扑救率达到0.246。

Conclusion: 身体动态约束会塑造承诺时机和策略集中度；响应方估计器是影响扑救学习与表现的关键因素，且在仅有观测代理时适合用后验分析比较均衡。
...
### [209] [FootQuery: Future-Touchdown-Guided Retrieval from Depth History for Perceptive Humanoid Locomotion](https://arxiv.org/abs/2609.21447)
*Tao Dong,Jia Yu,Yuxuan Fan,Linna Zhao,Jiaqi Gong,Andong Yang,Chao Gao,Guyue Zhou*

Main category: cs.RO

TL;DR: 提出FootQuery感知运动框架，利用足部预测的下一步落点查询深度历史，实现复杂地形下的人形机器人连续运动。


<details>
  <summary>Details</summary>
Motivation: 复杂地形下落脚点可能在触地时不可见，相机覆盖有限和自遮挡使得必须从早期观测中检索相关地形信息。

Method: 使用本体感觉预测落点位置与不确定性，结合每足特征查询稀疏采样的历史深度帧；训练时将实际接触投影到历史图像中监督检索；融合每足特征与全局视觉记忆生成控制动作；采用渐进力辅助课程和事件一致踏面中线塑形；部署仅需本体感觉和机载深度图像。

Result: 仿真中完整框架在最具挑战的楼梯、间隙和平台上优于各消融组件；在Unitree G1上单一策略实现室外楼梯和室内路线连续穿越，包括上下楼梯、平台和间隙。

Conclusion: 结果支持围绕预期接触组织视觉历史，用于感知人形机器人运动。
...
### [210] [Demonstration Synthesis from a Single Scan via Gaussian Splatting for Visuomotor Policy Learning](https://arxiv.org/abs/2609.21112)
*Beichen Wang,Yuen-Hei Yeung,V. R. Sridhar Devarakonda,Xuesu Xiao*

Main category: cs.RO

TL;DR: 提出GaussianFactory，一种仅需单次视频扫描作为人类输入、无需物理引擎的高保真数据引擎，可批量生成视觉运动策略训练演示。


<details>
  <summary>Details</summary>
Motivation: 训练视觉运动策略需要大量与目标环境匹配的演示，但重新收集成本高昂；现有合成方法受限于高人工投入、有限视觉保真度或对物理模拟器的严重依赖。

Method: 将场景重建为可编辑的3D高斯溅射（3DGS）副本，从场景可提供的物体组合任务中采样；对每个任务，在几何重建上纯运动学地规划抓取和轨迹，渲染出与目标环境视觉匹配的逼真演示；仅在抓取形成阶段，通过预训练的接触模型引入接触力动力学。

Result: 在模拟场景和真实UR10e机器人工作空间中实现端到端扫描到部署流程；仅用合成演示训练的标准扩散策略分别达到95.1%和84.2%的成功率。

Conclusion: GaussianFactory能以极低人工成本生成高保真演示，有效训练视觉运动策略，并在模拟和真实场景中均取得高成功率。
...
### [211] [Tilt as a Certified Resource: Preserving Motor Wrench-Rate Authority on Articulated Multirotors](https://arxiv.org/abs/2609.21580)
*Giuseppe Silano,Martin Saska*

Main category: cs.RO

TL;DR: 本文针对全驱动多旋翼在扰动下需保留快速调节能力的问题，利用主动形态（舵机倾转）打破固定几何平台的零和退化，提出仅基于电机的就绪度证书，并将其作为控制障碍函数嵌入统一物理命令二次规划中，从而在强阵风下保持系统状态有界与车辆权限。


<details>
  <summary>Details</summary>
Motivation: 传统省力分配器忽略动态限制，最大化就绪度又会导致拓扑不连通的最优面并需不可能的执行器速率；固定几何对称平台强制就绪度安全下限时存在零和退化：重新分配电机转速无法在不牺牲力矩跟踪的情况下提升权限。

Method: 使用主动形态，将舵机倾转视为提供静态旋翼所没有的权限恢复方向的几何资源。构造配置相关、仅含电机的就绪度证书，即可达力矩变化率集合的对数体积，并明确排除舵机容量以避免幽灵容量谬误。将该证书作为控制障碍函数，嵌入作用于电机转矩和舵机设定点的统一物理命令二次规划中。

Result: 在严重阵风扰动下对铰接八旋翼进行闭环仿真：经典分配器发散，未认证的铰接分配器违反安全下限，而所提出的控制障碍函数滤波器使系统状态保持有界并保留车辆权限。

Conclusion: 主动形态可打破固定几何平台的零和退化，基于控制障碍函数的分配框架能有效维持就绪度安全约束并在扰动下保持车辆调节能力。
...
### [212] [RAYA: Learning Where and When to Intervene for Robot Recovery](https://arxiv.org/abs/2609.21690)
*Ishaan Mahajan,Charles Chen,Frederike Dümbgen,Brian Plancher*

Main category: cs.RO

TL;DR: 论文提出 RAYA，一种混合学习-解析控制框架，将学习到的有限时域可恢复性裕度嵌入带硬约束的最优控制器，并用有界学习调度器动态调整任务权重，以在故障或扰动下提升机器人存活率与任务完成率。


<details>
  <summary>Details</summary>
Motivation: 仅能预测故障并不足以避免失败：当安全机制反应时，标称规划可能已耗尽恢复所需的控制权限，且固定任务优先级可能阻碍剩余响应。作者认为可恢复性应在动作选择时影响决策，而非事后否决，同时任务目标应随可恢复性下降而自适应调整。

Method: RAYA 将学习到的有限时域可恢复性裕度放入带硬约束的最优控制器中，并配合一个有界学习调度器，通过改变任务权重来促进恢复。该框架结合学习与解析控制，并实现了嵌入式版本。

Result: 在四旋翼与自动驾驶车辆基准中，每个控制器进行 7200 次仿真回合，RAYA 不仅提高存活率，还能零样本迁移到未见轨迹、扰动、被控对象变化和摩擦布局。嵌入式 RAYA 部署于 35g Crazyflie 四旋翼上；在风扰加气动失配或未建模 40% 电机指令损失下共 40 次硬件飞行中，三个基线全部失败，而 RAYA 完成 10/10 六周期任务。

Conclusion: 可恢复性应主动参与动作选择，任务优先级应随可恢复性缩小而调整；RAYA 验证了该思路可提升鲁棒恢复能力，并成功实现真实硬件部署。
...
### [213] [A Novel Path-Tracking Algorithm for Automated Tractor-Trailer Forward and Backward Maneuvers](https://arxiv.org/abs/2609.21718)
*Alexandre Lombard,Florent Perronnet,Nicolas Gaud,Abdeljalil Abbas-Turki*

Main category: cs.RO

TL;DR: 本文提出一种针对单挂车铰接式车辆的路径跟踪算法，结合挂车侧横向控制与牵引车转向角的短时域预测调整，可在前进和倒车中稳定收敛，并通过高保真仿真验证其性能。


<details>
  <summary>Details</summary>
Motivation: 全自动牵引车-挂车系统在物流、农业和工业中日益普及，对精确鲁棒的路径跟踪需求高；但牵引车与挂车间的铰接引入额外非线性，显著增加横向与纵向控制难度，尤其在倒车时。

Method: 提出一种新的单挂车铰接车辆路径跟踪算法：在挂车层面施加横向控制律，并对牵引车转向角进行短时域预测调整；方法基于几何，无需逐车校准或训练。

Result: 在高保真物理仿真中，该控制器在精度、稳定性和抗干扰鲁棒性方面达到或优于经典方法与先进方法。

Conclusion: 所提方法能保证前进与倒车时稳定收敛至期望路径，且无需标定或训练，适用于实际铰接式车辆路径跟踪。
...
### [214] [MetaPusher: Meta Learning and Planning for Nonprehensile Manipulation of Unseen Objects with Rapid Online Adaption](https://arxiv.org/abs/2609.21122)
*Donghyung Lee,Seyedali Golestaneh,Jaskrit Singh,Zhuoyun Zhong,Athanasios Kapoutsis,Constantinos Chamzas*

Main category: cs.RO

TL;DR: MetaPusher 是一个面向未知物体非抓取操作的元学习与自适应规划框架，通过在线交互快速适应动力学并更新长时程规划，无需单独的数据收集阶段。


<details>
  <summary>Details</summary>
Motivation: 未知物体的动力学取决于摩擦、质量分布等无法仅从感知中推断的潜在物理属性；已有经验只能提供不确定的初始估计，且 sim-to-real 迁移可能进一步退化。交互可逐步修正动力学，但更新模型可能使原规划轨迹失效，因此需要快速动力学适应与可融合演化的规划策略。

Method: 提出 MetaPusher：使用元学习动力学模型，在任务执行过程中从交互中快速适应；结合自适应运动动力学规划器，通过复用和细化已有搜索树来更新长时程计划；二者耦合使操作与适应无需单独数据收集阶段。

Result: 在仿真和 sim-to-real 场景中对未知物体进行评估，并与微调、主动学习方法、MPPI 控制及强化学习策略比较。MetaPusher 取得更低预测误差，并将任务成功率最多提升 20%。

Conclusion: MetaPusher 能有效支持未知物体的非抓取操作，实现快速动力学适应与自适应规划的结合，在仿真和真实迁移场景中优于多种基线方法。
...
### [215] [Noctif3R: Feed-Forward Monocular Real-Time SLAM for Photon-Limited Scenes on Embedded Hardware](https://arxiv.org/abs/2609.21114)
*Mihir Chauhan,Aditya Uday Abhang,Kevin Biju Mathew,Aniket Bera*

Main category: cs.RO

TL;DR: 论文研究极暗环境下机器人仅依赖单目RGB相机、在功耗受限的板载计算机上实时定位的问题。作者测量了现有实时SLAM/VO方法在低信噪比下的失败模式，并提出SYS——一个基于低光前馈点图前端和显式匹配门控的单目管线。SYS在可跟踪时误差最低，并给出Jetson AGX Orin上的嵌入式实时执行路径，实现吞吐提升、显存和能耗下降。


<details>
  <summary>Details</summary>
Motivation: 机器人在黑暗环境中执行任务时，需要仅用单个RGB相机、在极低光照且像素信号接近传感器噪声的情况下，在功耗受限的板载计算机上实时定位。离线低光重建虽能恢复结构但速度太慢，而机器人可实际搭载的实时单目系统在低信噪比下会退化或失败。现有各约束分别有成熟方案，但三者交叉处仍缺乏可行方案。

Method: 作者先定量测量现有方法在低暗场景中的失败模式；然后提出SYS：一个以低光前馈点图前端为核心、带显式匹配门控的单目管线。针对Jetson AGX Orin，作者设计嵌入式执行路径，将地图、关键帧和后端运行在384像素，跟踪运行在256像素，并对每帧位姿求解做两处修复。评估使用标定且可逐位精确复现的噪声阶梯、重新标注的真实暗光曝光数据，以及从Boston Dynamics Spot机器人录制的新暗室视频阶梯。

Result: 在最低的九个暗度等级中，DROID-SLAM全部返回不含相机运动信息的全长轨迹，VGGT-SLAM与CUT3R有八个、pi^3有七个、DPV-SLAM有四个。SYS返回三条可跟踪轨迹且没有无信息轨迹；在可跟踪时误差最低，为无信息上限的24-47%，而最强基线为56-73%，但覆盖范围最窄。真实机器人视频中86.5%的交付帧全黑时，SYS所有配置在亮部开始后停止，而DROID-SLAM和DPV-SLAM仍为全部1178帧输出位姿。嵌入式优化在Jetson AGX Orin上实现可复现的Pareto改进：一个场景吞吐1.28倍、误差0.964倍，另一场景吞吐1.42倍、误差0.68倍，同时峰值GPU显存减少47%，每个位姿能耗减少29%。

Conclusion: 现有实时单目系统在极低光照下会输出看似完整但无信息甚至错误的轨迹，而SYS通过低光前馈点图前端和显式匹配门控，在可跟踪时提供更低误差，并能在板载Jetson上以更低显存和能耗实时运行。论文表明，针对极暗环境、实时性和嵌入式部署交叉点的联合设计是可行且必要的。
...
### [216] [SAGE: Safety-Aligned Gradient Enforcement for Human--Robot Collaboration](https://arxiv.org/abs/2609.21130)
*Yisen Li,Hao Zhang,Ruize Geng,Yves Tseng,Ding Zhao,H. Eric Tseng*

Main category: cs.RO

TL;DR: 论文提出SAGE方法，用于多主体人机协作中兼顾可解释决策与物理安全约束。它通过SAIL和TALO缓解可解释决策树策略与CBF安全过滤结合时产生的两类多智能体强化学习错配，并在仿真与双人形机器人实验中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 多主体人机协作面临双重挑战：机器人决策需可解释、可审计，同时执行动作需满足物理交互中的安全约束。将可解释决策树策略与控制障碍函数（CBF）过滤结合虽具前景，但在多智能体强化学习中产生两类学习错配：安全投影会改变实际作用于环境的动作；耦合的提议图可能使独立优化的actor更新与团队级更新不一致。

Method: 提出安全对齐梯度强化（SAGE）。其包含：盾退火内化层（SAIL），使用可微分有限惩罚提议映射，同时保留精确CBF二次规划用于执行，以保持约束法向敏感性并内化反复激活的安全约束；团队平均Lyapunov策略优化（TALO），构建团队感知更新参考，并用Lyapunov半空间校正调节独立actor更新。

Result: 在九种仿真场景中，SAGE达到71.0%成功率，每千环境步0.5次碰撞步。消融显示：直接CBF过滤将碰撞频率降低98.5%，但成功率从67.3%降至59.3%。SAIL将提议违规降低48.8%，提议-执行校正降低85.2%；TALO将更新一致性差距降低50.8%。双人形机器人与人类伙伴的物理实验证明部署可行性。

Conclusion: SAGE能够同时缓解安全投影导致的动作错配与独立actor更新和团队更新不一致的问题，在提升安全性的同时保持较高任务成功率，并具备真实人机协作部署可行性。
...
### [217] [Diverse and Adaptable Arm Coordination for Octopus-Crawling via Diffusion-Based Uncertainty-Aware Optimization](https://arxiv.org/abs/2609.21138)
*Seung Hyun Kim,Heng-Sheng Chang,Kimia Kazemi,Prashant Mehta,Mattia Gazzola*

Main category: cs.RO

TL;DR: 本文提出基于扩散的不确定性感知优化算法DUO，为模拟肌肉驱动的CyberOctopus学习无示范爬行控制器，首次将扩散控制用于软多臂机器人接触丰富仿真。通过共享控制分布嵌入多样运动行为，使模拟章鱼能应对动态物理约束，表明学习到的协调多样性有助于鲁棒适应。


<details>
  <summary>Details</summary>
Motivation: 章鱼爬行启发了利用冗余的软体机器人，但发现并组织多样的协调模式以实现适应仍具挑战。研究旨在让学习到的协调多样性成为软多臂机器人适应的实用资源。

Method: 提出DUO算法，学习无演示的爬行控制器；采用对称结构化策略表示，将径向等价控制器折叠到规范方向扇区；使用在线黑盒优化发现并保留多样协调模式；并提出控制编辑技术，无需重训练即可适配新执行器约束。

Result: 在接触丰富的仿真中，模拟章鱼能够应对动态物理约束，并展现出由协调多样性带来的鲁棒适应能力；控制编辑可使现有控制器适应新执行器约束而无需重新训练。

Conclusion: 学习到的协调多样性使运动冗余成为软多臂机器人适应的实际资源，为软体多臂机器人在动态约束下的无示范控制与适应提供了新途径。
...
### [218] [MA-LIPP: Cooperative Multi-Agent Load-Aware Informative Path Planning for Heterogeneous Robot Teams](https://arxiv.org/abs/2609.21167)
*Hojune Kim,Guangyao Shi,Gaurav S. Sukhatme*

Main category: cs.RO

TL;DR: 论文提出 MA-LIPP，面向载荷感知信息路径规划中采样与运输强耦合的问题，利用异构多机器人团队通过异步“死投”交接样本，并给出精确 MIQP 与可扩展 Pairwise LNS 启发式。


<details>
  <summary>Details</summary>
Motivation: 野外机器人采样需将物理样本送回实验室分析，累积样本会增加载荷和遍历能耗，使路径规划具有载荷感知和顺序依赖。单机器人 LIPP 中采样与运输不可分离，必须频繁返回 depot，严重限制覆盖范围；多机器人分工虽可缓解瓶颈，但引入何时、何地、交接何物、交给谁的协同难题。

Method: 将 MA-LIPP 形式化为精确 Mixed-Integer Quadratic Program（MIQP），并提出可扩展的 Pairwise Large-Neighborhood Search（LNS）启发式。方法支持异步“dead drops”，即一个机器人放置样本、另一个机器人稍后取走，无需同步会合。

Result: Pairwise LNS 启发式在 95.5% 的认证案例中与精确最优解匹配；在最多 12 个机器人的较大实例上，相比顺序基线将加权后验方差降低 16.1%–19.8%。

Conclusion: MA-LIPP 为协同物理采样任务提供了稳健框架，通过异构多机器人分工和异步样本交接，有效提升信息采集效率并缓解单机器人运输瓶颈。
...
### [219] [Same World, Different Knowledge: When Isolated Audits Misjudge World-Model Repairs](https://arxiv.org/abs/2609.21155)
*Rui Min,Xianyao Li,Fang Xu,Sofiane Lachab,Jing Du*

Main category: cs.RO

TL;DR: 该研究提出面向世界模型的信息接口审计框架，区分保真度缺口与可用性缺口，并在模拟四旋翼模型预测控制中评估不同修复策略，表明修复效果依赖于部署时的信息路径。


<details>
  <summary>Details</summary>
Motivation: 孤立输入故障下表现更优的修复，在部署模块共享故障信息时可能反而更差；因此需要审计世界模型的信息接口，以理解修复策略的适用条件。

Method: 提出信息接口审计，区分保真度缺口（精确输入变为估计值）与可用性缺口（输入缺失）；采用固定权重干预测量预测误差、输入依赖性和成对闭环收益（含重建引入的依赖）。在模拟四旋翼MPC中测试校准误差、不确定性训练、风重建等，并利用加速度诊断和扰动观测器分析。

Result: 耦合反号的10%质量/推力校准误差使物理锚定模型成功率从69%降至8%，不确定性训练恢复至65%；风重建可恢复控制收益但继承校准依赖。正校准偏移下，仅重建腐败时不确定性训练重建占优，共享腐败时基线占优；加速度诊断显示重建偏差与标称模型误差间存在补偿，扰动观测器亦有类似现象。

Conclusion: 修复策略的选择取决于部署中所使用的信息路径，需根据信息接口审计结果决定采用何种修复。
...
### [220] [OpenRoIS: A Community-Driven Open-Source Middleware Implementing the Robotic Interaction Service (RoIS) Framework for Physical Robots and Virtual Agents](https://arxiv.org/abs/2609.21178)
...
TL;DR: 本文介绍了OpenRoIS，一个社区驱动的开源中间件，实现了OMG标准的RoIS框架2.0，旨在解决人机交互服务应用因硬件平台变更而需重写的问题，提供了跨平台的标准化接口和多种SDK。


<details>
  <summary>Details</summary>
Motivation: 服务应用通常针对特定硬件接口编写，更换硬件需重写。RoIS 2.0标准虽定义了平台无关模型，但缺乏维护实现、SDK和适配器，难以实际采用。

Method: OpenRoIS作为开源中间件，实现了RoIS框架2.0。其方法包括：递归引擎架构（单引擎类实现主/子HRI引擎角色）、内部五方法组件契约（区别于五个外部RoIS接口）、将接口映射到基于WebSocket的JSON-RPC 2.0、单一数据源类型管道生成三种一致语言栈、提供TypeScript和C#客户端SDK（支持Web和Unity）、以及Python适配器SDK（支持ROS 2）。

Result: 通过通用的RoIS接口，服务应用可以经由互联网控制物理机器人和虚拟代理。所有源码、接口类型和文档以Apache-2.0许可开源，并在https://openrois.org/开放开发。

Conclusion: OpenRoIS作为一个开放开发、范式中立的实现，将RoIS标准从规范推向实践，促进了人机交互服务应用的跨平台复用。
...
### [221] [Robust Structureless Monocular Visual Inertial Initialization Exploiting Line Features and Vanishing Points](https://arxiv.org/abs/2609.21186)
*Junwan Choi,Woongrae Jo,Dong-Uk Seo,Jinwoo Jeon,Hyun Myung*

Main category: cs.RO

TL;DR: 提出 SLIM-init，一种无结构单目视觉惯性里程计初始化方法，利用2D线特征几何约束，在退化运动下实现更准确、鲁棒的初始化。


<details>
  <summary>Details</summary>
Motivation: 准确的初始化对可靠VIO至关重要，但在退化运动下往往病态。现有方法通常需要限制性激励运动以保证可观测性，或依赖计算昂贵的3D结构重建，难以高效实用部署。

Method: SLIM-init直接利用跟踪到的2D线特征几何约束，无需显式3D路标重建。它使用线推导的消失点作为平移不变姿态线索，在低视差或平移主导等退化场景提供旋转约束；引入线极线残差约束平移，并利用线法向投影残差改善线性对齐条件数。

Result: 在公开基准和具有挑战性的自定义退化运动序列上，SLIM-init相比最先进初始化方法在准确性和鲁棒性上均有提升。

Conclusion: SLIM-init能够在不进行显式3D重建的情况下，有效提升退化运动下单目VIO初始化的精度与鲁棒性，方法更高效实用，源代码已开源。
...
### [222] [When to Waddle: A Comparative Study of Bipedal Torso-Stabilization on Low-Friction Surfaces](https://arxiv.org/abs/2609.21185)
*Naomi Oke,Ben Gu,George Ortiz,Stacy Ashlyn,Cordelia Pride,Sarah Bergbreiter,Aaron M. Johnson*

Main category: cs.RO

TL;DR: 受企鹅摇摆启发，研究低摩擦表面上躯干侧向运动与质心（COM）位置如何影响双足运动，并通过五执行器双足机器人的仿真与硬件实验比较直立步态和企鹅式躯干越过支撑腿策略。


<details>
  <summary>Details</summary>
Motivation: 低摩擦表面会限制 stepping 时可用接触力，挑战双足运动；因此需要理解侧向躯干运动和 COM 放置随摩擦变化如何影响行走速度、能效和滑移相关足部运动。

Method: 使用五执行器双足机器人，在 MuJoCo 仿真中扫描正弦腿部和髋部驱动参数，覆盖摩擦系数 mu=0.1、0.3、0.5、0.7；比较直立步态与企鹅式躯干越过支撑腿策略，并在多个 COM 位置下进行仿真和硬件实验，硬件测试 mu=0.12 和 mu=0.45。

Result: 仿真中，低摩擦下企鹅式躯干越过支撑腿运动产生更多成功控制器和更高前进速度，高 COM 配置速度最高。硬件中，mu=0.12 时该策略提高前进速度并降低运输成本，高 COM 也改善两者且侧向足部运动低；mu=0.45 时 COM 趋势反转，低 COM 配置更快更节能，步态策略对速度影响小但仍改变侧向足部运动。

Conclusion: 侧向躯干运动和 COM 放置的效果取决于可用摩擦；企鹅式躯干运动可在硬件上调节前进速度、能量消耗和滑移相关足部运动。
...
### [223] [Stochastic Neural Signed Swept Volume for Real-time Chance-Constrained Trajectory Optimization](https://arxiv.org/abs/2609.21211)
*Qingyi Chen,Kevin Zhang,Lucas Chen,Zachary Kingston*

Main category: cs.RO

TL;DR: 提出学习扫掠体积的符号距离函数作为概率场，量化认知不确定性并集成感知噪声，用于机会约束轨迹优化，在高维操作任务中验证。


<details>
  <summary>Details</summary>
Motivation: 传统碰撞检测在离散状态检查，权衡安全与效率；连续碰撞检测计算昂贵或过于保守；数据驱动方法学习扫掠体积但存在近似误差，通常仅作粗过滤。需要一种能处理不确定性且可集成到优化中的方法。

Method: 学习扫掠体积的符号距离函数作为概率场，量化认知不确定性，纳入感知噪声，并集成到机会约束轨迹优化框架中。

Result: 在仿真和真实硬件上，针对具有显著传感器噪声的高维操作问题进行了验证。

Conclusion: 该方法能有效处理碰撞检测中的不确定性，并成功应用于高维操作任务。
...
### [224] [Visual Navigation Transformer with Pose Attention](https://arxiv.org/abs/2609.21212)
*Beiming Li,Jaime Romero,Jonathan Diller,Vijay Kumar,Alejandro Ribeiro*

Main category: cs.RO

TL;DR: 提出VNT-PA，一种基于姿态注意力的视觉导航Transformer，以姿态索引的深度关键帧集合作为上下文，注意力依赖于姿态差异而非时间顺序，在HM3D上取得93.3%成功率和90.4% SPL，优于时间序列或姿态输入基线，且能融合不同轨迹帧，对定位噪声更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 现有导航策略通常使用时序观测历史，位置编码与时间绑定，难以重用先前遍历经验；重用经验的系统需构建显式地图或拓扑图并规划。

Method: VNT-PA是一个Transformer规划器，上下文为按相机姿态索引的深度关键帧集合。以相机姿态作为位置编码，注意力取决于关键帧间的姿态差异而非时间顺序。训练模仿真实场景网格上的最短路径规划器，仅用当前姿态和目标位置查询空间上下文预测动作。

Result: 在HM3D验证场景的点目标导航中，成功率达93.3%，SPL达90.4%，在导航性能和训练效率上优于将相同上下文编码为时间序列或将姿态作为输入特征的基线。测试时可融合不同轨迹的帧，且在定位噪声下比基于显式地图规划的传统基线退化更平缓。

Conclusion: 带有姿态标记的经验可直接作为学习规划器的环境表示，使注意力依赖于姿态差异而非时间顺序能加速训练并改善长时程导航。
...
### [225] [Safe Real-Time Policy Steering via Noise-Space Trajectory Optimization for One-Step Generative Policies](https://arxiv.org/abs/2609.21220)
*Qingyi Chen,Joseph Ruan,Zachary Kingston*

Main category: cs.RO

TL;DR: INSPO 提出一种针对一步生成式机器人策略的推理时引导方法，将约束满足转化为策略输入噪声空间中的轨迹优化，从而在不修改生成动作的情况下实现实时避障与姿态维持。


<details>
  <summary>Details</summary>
Motivation: 生成式机器人策略能表示多样、多模态行为，但预训练策略在部署时需满足避障、姿态维持等约束。现有推理时引导方法通常通过迭代扩散或流过程进行梯度引导，计算开销大，难以用于实时控制。

Method: INSPO 将一步生成式策略的推理时引导表述为输入噪声空间中的轨迹优化：优化输入噪声，并在诱导的状态轨迹上评估约束；通过正则项使解与策略输入分布保持一致；使用基于种群的粒子优化在线求解。

Result: 在 Push-T、Can pick-and-place 和 LIBERO-Spatial 上，对基于状态与图像的任务专用策略以及通用视觉-语言-动作策略进行评估。INSPO 相比 best-of-N 采样和动作投影提高了任务成功率与约束满足率，并在更低运行时间下优于梯度引导生成方法。

Conclusion: INSPO 为一步生成式策略提供了一种有效且高效的推理时引导方式，可在实时控制中兼顾任务性能与部署约束。
...
### [226] [Fewer Steps, Better Actions: Rethinking Flow-Matching Inference for VLA Policies](https://arxiv.org/abs/2609.21216)
*Zhipeng Tang,Xinda Chen,Weining Rao,Xiao Li,Wenting Tan,Yuning Wang,Xiao Shi,Xiaofang Zhao*

Main category: cs.RO

TL;DR: 提出Coda，将流匹配VLA策略的部分积分预算重新分配给一个学习到的端点校正器，在冻结策略完成少步去噪轨迹后，用轻量Transformer预测残差，从而在降低推理延迟的同时提升闭环成功率。


<details>
  <summary>Details</summary>
Motivation: 基于流匹配的视觉-语言-动作（VLA）策略通过重复评估动作专家生成动作块。增加积分步数会提高推理成本，却不一定提升闭环任务成功率，因此需要更高效地利用推理预算。

Method: Coda让冻结的策略先完成少步噪声到动作的轨迹生成；随后用一个轻量Transformer，基于候选动作、源噪声和共享观测前缀缓存，预测演示监督的残差进行端点校正。训练时仅更新校正器，冻结原策略。

Result: 在RoboTwin Easy的50个任务上，五步Coda将成功率从71.64%提升至74.68%，相比默认十步策略前向延迟降低30.2%；两步配置达到71.88%成功率，加速2.12倍。独立13任务控制实验中，在几乎相同延迟下成功率提升5.69个百分点。相同设计也提升了冻结的官方SmolVLA，将两步成功率从60.8%提升至69.4%。

Conclusion: 端点校正可有效替代额外积分步数，改善冻结流匹配策略的质量-延迟权衡，在多种设置下均能提升性能与效率。
...
### [227] [SafeStage: Evaluating Safety Before, During, and After Vision-Language-Conditioned Robot Manipulation](https://arxiv.org/abs/2609.21223)
*Jinzhu Luo,Qi Zhang,Wei Wang,Wei Jiang*

Main category: cs.RO

TL;DR: 提出 SafeStage，一个按生命周期分阶段的机器人操作安全基准，覆盖任务执行前、中、后的安全风险，包含97个风险场景，用于评估视觉语言条件操作策略的安全失效。


<details>
  <summary>Details</summary>
Motivation: 现有评估多关注任务成功、孤立物理约束、语义拒绝或已发生的物理损伤，难以在闭环操作中定位安全失效发生的具体阶段。

Method: 构建 SafeStage，分为初始状态危险、执行时安全、最终状态危险三阶段；使用基于事件和状态的检查评估真实交互，并将原生任务成功与各阶段安全结果独立报告；在统一闭环协议下评估直接动作 VLA 策略和基于世界模型的策略。

Result: 名义任务完成常与安全违规同时存在，且不同策略在三阶段表现出不同的失败画像。

Conclusion: 通过分离任务成功与安全并定位违规发生时机，SafeStage 为评估和改进视觉语言条件机器人操作策略提供了统一诊断测试平台。
...
### [228] [FOCAL-VLA: Subtask-Guided Geometry Distillation and Implicit World Modeling for Vision-Language-Action Models](https://arxiv.org/abs/2609.21228)
*Zhiyuan Gao,Di Wen,Yanxiang Zhan,Mohammad Khoshnazar,Jeroen Schäfer,Kunyu Peng,Michael Beetz*

Main category: cs.RO

TL;DR: 提出 FOCAL-VLA，通过子任务引导的几何蒸馏与隐式世界建模，增强视觉-语言-动作模型的空间与时间理解。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型直接由当前 2D 观测映射到动作，缺乏空间与时间理解；已有几何监督和未来状态预测方法使用全场景信息，存在冗余并干扰与当前交互相关的几何和动态学习。

Method: 结合子任务引导的几何蒸馏与隐式世界建模：将 VGGT 的几何知识迁移到 VLA 模型，通过将几何隐变量与子任务相关图像区域特征对齐；并利用当前与未来示范帧的 Track4World 特征进行隐式世界建模。两种表示共同指导动作生成，推理时无需运行 VGGT 或 Track4World。

Result: 在仿真基准和真实世界操作任务上，FOCAL-VLA 均优于基线方法。

Conclusion: 聚焦当前子任务相关的几何结构和未来交互动态，可有效提升 VLA 模型在精确操作和长时程操作中的表现。
...
### [229] [AirSplan: Risk-Aware Motion Planning for Quadrotors in Cluttered 3D Gaussian Splats](https://arxiv.org/abs/2609.21226)
*Seth Isaacson,William Hong,Katherine A. Skinner,Ram Vasudevan*

Main category: cs.RO

TL;DR: AirSplan系统结合归一化3D高斯泼溅场景表示与基于可达性的运动规划，实现四旋翼在复杂场景中的无碰撞路径规划。


<details>
  <summary>Details</summary>
Motivation: 四旋翼在农业、巡检等应用中碰撞代价极高，必须严格无碰撞；但在缺乏真实障碍物数据时，复杂场景的高保真表示与安全运动规划仍具挑战。

Method: 采用归一化3D Gaussian Splatting高保真编码场景几何；提出基于可达性的运动规划器，利用四旋翼微分平坦性生成连续时间碰撞约束，紧密过近似机器人占用。

Result: 在挑战性测试用例中，AirSplan成功找到路径的比例为81.2%，显著高于最佳基线方法的51.2%。

Conclusion: AirSplan能有效应对复杂场景表示与无碰撞规划挑战，提升四旋翼安全自主导航的成功率。
...
### [230] [KnowDemo: Knowledge-Guided Robot Demonstration Generation from Human Videos](https://arxiv.org/abs/2609.21229)
*Zhiyuan Gao,Yanxiang Zhan,Mohammad Khoshnazar,Jeroen Schäfer,Michael Beetz*

Main category: cs.RO

TL;DR: 提出 KnowDemo 框架，利用人类视频中的结构化操作知识为目标工作空间生成多样化机器人演示，并用于策略学习与仿真到真实迁移。


<details>
  <summary>Details</summary>
Motivation: 机器人操作策略学习通常依赖大量真实演示，采集成本高；现有从人类视频生成演示的方法多基于运动参考适配，限制行为多样性，且对任务要求和场景关系理解不足，导致无效候选多、生成效率低。

Method: 开发基于视觉语言模型的知识提取与推理模块，将物体和动作描述与任务条件、演示参考及允许的执行变化关联；再将知识解析到目标场景实体和几何，指导候选生成与筛选，并进行运动规划和仿真；同时使用任务引导抓取采样。

Result: 实验表明，相比仅参考配置，方法能产生更多经验证的执行模式；任务引导抓取采样提高了候选规划成功率；在仿真数据上微调预训练 π0.5 模型后，在三个任务上实现了仿真到真实迁移。

Conclusion: KnowDemo 通过结构化操作知识提升了机器人演示生成的多样性和效率，并验证了生成数据可用于策略学习及真实机器人迁移。
...
### [231] [LOInK: Learned Optimal Inverse Kinematics via Structured Neural Surrogate Models](https://arxiv.org/abs/2609.21275)
*Michael Somerfield,Damian Abood,Ruigang Wang,Ian R. Manchester*

Main category: cs.RO

TL;DR: 提出 LOInK 方法，通过学习双 Lipschitz 可逆映射将配置空间映射到解耦的任务/潜在空间，并将代价最小解置于原点，从而利用基于算子分裂的网络反演高效生成近似最优逆运动学解。


<details>
  <summary>Details</summary>
Motivation: 传统逆运动学或约束优化求解最优解通常较慢，而现有生成方法需要多样采样并评估大量候选解，因此希望更直接、高效地生成近最优解。

Method: 在包含采样配置、对应任务变量和给定代价函数的数据上训练；学习从配置空间到解耦任务/潜在空间的双 Lipschitz 可逆映射；将潜在空间结构化，使代价最小解位于原点；通过基于算子分裂的网络反演算法采样近最优解。

Result: 在三个问题上验证：三自由度机械臂示例；四足攀爬机器人上平均比约束优化快 31 倍、最高快 100 倍；软体执行器纯数据驱动示例中能显式生成高质量解，优于需要多样采样和候选评估的现有生成方法。

Conclusion: LOInK 能高效生成近似最优逆运动学解，适用于复杂机器人系统和纯数据驱动场景，并优于依赖候选采样评估的生成方法。
...
### [232] [Stability-aware Residual Reinforcement Learning Framework for Robotic Manipulator Disturbance Compensation](https://arxiv.org/abs/2609.21307)
*Jihong Kim,Joonhyuk Kwon,Hwa Soo Kim,TaeWon Seo,Hyung-Tae Seo*

Main category: cs.RO

TL;DR: 提出一种残差强化学习干扰观测器框架，将解析观测器与强化学习策略结合，用于机械臂在高参数不确定性、非线性摩擦和复合干扰下的精准跟踪。


<details>
  <summary>Details</summary>
Motivation: 传统控制器和干扰观测器在精密跟踪中易受参数不确定性、非线性摩擦和复合干扰影响，难以补偿模型无法捕捉的残差。

Method: 让确定性基线在可靠区域工作，强化学习策略专门补偿残差；通过估计器网络将观测历史与特权干扰上下文对齐，按干扰类型组织潜空间以快速适应干扰切换；基于输入到状态稳定性分析推导并施加状态相关动作约束，保证任意策略输出下闭环跟踪误差有认证包络。

Result: 在6自由度机械臂上，干扰估计和跟踪性能一致提升；真实硬件零样本仿真到现实迁移下跟踪误差降低27.8%，在训练中未见的基座振动干扰下降低38.0%。

Conclusion: 该框架能可证明地限制跟踪误差，并跨干扰快速适应，有效提升机械臂抗扰跟踪性能。
...
### [233] [NaViRrator: Robot Navigation from Human-Readable Maps through a Learned Visual Route](https://arxiv.org/abs/2609.21316)
*Ayun Lee,Jiseon Kim,Giseop Kim*

Main category: cs.RO

TL;DR: 提出 NaViRRator 框架，将人类可读地图上的起终点转换为导航指令，供预训练 VLN 策略执行。核心 RouteScribe 先生成地图坐标中的路线脚手架，再由 VLM 转为语言指令，其中 SGL-CFM 将直线路径变形为地图条件路线。


<details>
  <summary>Details</summary>
Motivation: 人类可读地图是直观的机器人目的地指定接口，但将地图的示意几何结构与机器人自我中心观测相连接仍具挑战，需要一种模块化方式把地图起终点转化为预训练导航策略可执行的指令。

Method: NaViRRator 将地图上的起终点翻译为 VLN 指令。RouteScribe 分离路线推断与语言表达：先用 start-goal line conditional flow matching（SGL-CFM）将起终点直线路点序列变形为地图条件路线脚手架，再用预训练 VLM 转成导航指令。执行时 VLN 策略只接收指令和自我中心观测，地图与脚手架保留在上游，从而可替换执行器而无需重训地图到语言模块。

Result: 真实世界实验显示，该方法比直接地图到指令生成、基于 A* 的脚手架和 Gaussian-source conditional flow matching 取得更高成功率和 SPL。定性结果还表明，其显著转向更清晰，能更好保持预期动作序列。

Conclusion: 以路线为基础的语言可作为人类可读地图与预训练导航策略之间的模块化接口，兼顾地图接口的直观性与 VLN 策略的执行能力。
...
### [234] [LEMCA: LLM-Guided Synthesis of Efficient Mode-Switching Control Architectures](https://arxiv.org/abs/2609.21319)
*Arjun Krishna,Vincent Pacelli,Dinesh Jayaraman*

Main category: cs.RO

TL;DR: 论文提出 LEMCA，利用大语言模型引导合成高效的模式切换控制架构，将控制器设计表示为可解释程序并在进化循环中迭代优化，从而在不同任务阶段动态分配资源。


<details>
  <summary>Details</summary>
Motivation: 自然物理控制任务（如驾驶、操作）的感知与计算复杂度会随时间剧烈变化。理想方案是按需切换控制模式以节省资源，但模式切换控制器通常需要专家为每个新任务进行繁琐设计和综合，导致现代机器人控制常退回到资源浪费的单体式一刀切架构。

Method: LEMCA 将模式切换控制器设计表示为可解释程序，在进化循环中迭代改进；提出兼容模式切换的自动控制器合成扩展（如仿真中的强化学习）来评估设计适应度；并利用大语言模型的语义先验、推理和编码能力，迭代编辑控制器模式、对应的感知-计算资源分配以及模式切换。

Result: 在多种控制基准实验中，LEMCA 持续发现优于单体设计帕累托前沿的策略，能够回收“简单”任务阶段中被浪费的资源。

Conclusion: LEMCA 为自动、低努力地合成资源高效的模式切换控制器设计提供了一条可行路径。
...
### [235] [The EventCV Library for Event-Based Robotic Vision](https://arxiv.org/abs/2609.21330)
*Adam D. Hines,Michael Milford,Tobias Fischer*

Main category: cs.RO

TL;DR: EventCV 是一个开源、可扩展的 Rust 事件相机库，提供 OpenCV 风格的 Python 绑定，集成了去噪、变换、角点检测、运动估计、仿真和 ONNX 推理等功能，并在机器人场景中验证。


<details>
  <summary>Details</summary>
Motivation: 事件相机对快速运动机器人和困难光照条件有吸引力，但实际集成困难：缺少即插即用驱动、事件流文件格式不兼容、多数项目依赖定制研究级代码。

Method: 开发 EventCV 库，提供 Rust 实现与 Python 绑定，包含去噪滤波、几何变换、数据增强、角点检测、无监督特征学习、对比度最大化运动估计、视频到事件仿真、ONNX 推理，并集成 Neuromorphic Drivers 以实时处理事件流。

Result: EventCV 在单一工具包中覆盖广泛操作，构建表示和解码文件比现有库快 1.1 到 3.7 倍；在 Jetson Orin AGX 上部署，并完成目标检测、设备端模型推理和定位三个机器人案例研究。

Conclusion: EventCV 降低了使用事件相机的门槛，能够支持实时机器人流水线，并推动事件相机在真实机器人系统中的应用。
...
### [236] [FAN: Foresight Action Normalization for Continual Adaptation of Vision-Language-Action Models](https://arxiv.org/abs/2609.21358)
*Yijun Hong,Jiarun Zhu,Xiaoquan Sun,Le Xu,Qijun He,Xin Jin,Mingqi Yuan,Wenjun Zeng,Jiayu Chen*

Main category: cs.RO

TL;DR: 本文研究持续视觉-语言-动作（VLA）适配中的动作归一化问题，系统评估五种归一化策略，并提出基于一致性、覆盖性和因果性三项原则的FAN方法，以提升长期机器人操作中的稳定适应能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型虽在大规模闭数据集上表现优异，但长期真实部署需持续学习新技能并保留旧能力。已有持续VLA适配方法多关注经验回放和强化微调，却忽视了动作归一化这一决定策略动作坐标系统的基础机制。

Method: 在四个真实世界任务流（涵盖单臂和双臂操作）上系统评估五种动作归一化策略，分析其失败模式；据此提出一致性、覆盖性和因果性（3C）设计原则，并引入前瞻动作归一化（FAN）。FAN在持续学习前从少量且任务无关的校准集中估计一次归一化统计量，并在整个适配过程中冻结。

Result: 分析发现现有归一化协议会因任务间坐标漂移、运动覆盖不足或训练-测试坐标不匹配而引发严重失败模式。在所有评估任务流中，FAN取得了最高性能，并表现出稳定的鲁棒性。

Conclusion: 动作归一化是持续VLA适配的关键基础机制；遵循3C原则的FAN能为构建稳定的动作表征、实现有效的终身VLA适配提供重要指导。
...
### [237] [MicroHookACT: Monocular Microscopic Vision Guided Visuomotor Policy for Flexible Microelectrode Hooking](https://arxiv.org/abs/2609.21365)
*Yitong Chen,Fangbo Qin,Yang Wang,Ruihua Hu,Kui Zhang,Shan Yu*

Main category: cs.RO

TL;DR: 本文提出 MicroHookACT，一种基于模仿学习的视觉运动策略，用于单目显微视觉下的自动化 3D 针环钩取。


<details>
  <summary>Details</summary>
Motivation: 自动针环钩取是柔性微电极植入的关键步骤，但显微视觉下实现无触诊精确对准与接触式穿线仍具挑战，且需适应散焦模糊和不同操作条件。

Method: 采用单向钩取策略，利用散焦线索和光轴引导实现精确对准与接触式穿线；构建基于冻结 ViT 主干的动作监督对象注意力模块，从人类演示中直接学习关注针尖和微环，无需人工视觉标注；将注意力为中心的全局粗特征与局部细特征按预测动作进度动态加权，使单一 ACT 策略适应操作过程中的散焦模糊和视觉需求变化。使用 60 个人类演示训练，并在五种不同难度设置下评估。

Result: MicroHookACT 取得了最高 96.7% 的总体成功率，平均执行时间为 11.5 秒。

Conclusion: 结果表明，视觉运动策略学习在不同操作条件下实现微米级控制具有潜力。
...
### [238] [ProTracer: Proprioception-Guided Failure Diagnosis in Robot Manipulation](https://arxiv.org/abs/2609.21369)
*Chang Dong,Mehdi Hosseinzadeh,King Hang Wong,Lingqiao Liu,Francois Fraysse,Feras Dayoub,Minh Hoai Nguyen*

Main category: cs.RO

TL;DR: 本文提出机器人操作失败分析综合框架，涵盖二值失败检测、失败分类、解释生成，以及新增的失败起始定位任务；并给出免训练方法ProTracer与基准FailTime。


<details>
  <summary>Details</summary>
Motivation: 机器人操作失败分析通常缺乏细粒度时间定位，且现有方法多需额外训练；作者希望结合本体感知的时间精度与视觉语言模型的多模态推理能力，实现无需训练的失败诊断。

Method: 提出ProTracer：利用现有VLM和机器人本体感知信号，通过本体动力学识别具有时间信息的动作边界，并将机器人状态转为结构化自然语言描述，与视觉观察共同输入VLM进行分析；同时构建FailTime基准，同步采集视觉与本体感知数据以评估传统失败诊断和失败起始定位。

Result: ProTracer在传统失败诊断任务和新增的失败起始定位任务上均取得较强性能，表明本体感知推理对细粒度时间失败分析很重要。

Conclusion: 结合本体感知与VLM可在无需额外训练的情况下有效完成多类机器人操作失败分析，并推动失败起始定位这一新任务的发展。
...
### [239] [MarineCraft: Enabling Rapid Prototyping of Underwater Robots via Modular Construction](https://arxiv.org/abs/2609.21396)
*Yuta Sugiura*

Main category: cs.RO

TL;DR: 本文提出MarineCraft模块化工具包，通过自包含防水推进模块和结构重构，加速水下机器人的快速原型开发。


<details>
  <summary>Details</summary>
Motivation: 水下机器人开发常受防水和布线复杂性的阻碍，导致快速原型制作周期显著延长。

Method: 设计集成了电源、无线通信和驱动功能的独立防水推进模块，消除集中布线和重复密封需求，支持在数分钟内组装并测试不同几何构型的水下机器人。

Result: 实验表明，该可重构架构支持快速迭代设计，并在最深2.5米处保持可靠运行和无泄漏性能。

Conclusion: MarineCraft工具包通过将模块化转化为快速物理原型的手段，有效降低了水下机器人研发的门槛。
...
### [240] [Stabilizing Trajectory Outputs in End-to-End Autonomous Driving via SC-IMM Based Teacher Signals](https://arxiv.org/abs/2609.21404)
*Siewoo Kim,Seung-Hyun Kong*

Main category: cs.RO

TL;DR: 该论文提出一种基于场景条件交互多模型（SC-IMM）的离线教师信号生成与学习方法，用于稳定端到端自动驾驶模型的轨迹输出，并在CARLA闭环评测中显著提升驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于航点的模仿学习主要最小化坐标级误差，难以捕捉依赖场景的路径-速度变化以及航点输出中的时间不稳定性，导致闭环表现不佳。

Method: 将专家轨迹转换为路径-速度状态，并基于场景线索进行IMM更新，生成路径-速度教师标签和模式后验概率；训练时将这些信号作为辅助监督加入原始轨迹损失，推理结构和航点控制器保持不变。

Result: 在CARLA Town12的100条短路线闭环评测中，相比基线驾驶得分提升28.0%，Collision/km降低62.3%，同时改善了jerk和轨迹变化指标。

Conclusion: 嵌入场景条件运动模型线索的离线教师信号能够引导轨迹输出型驾驶模型获得更稳定的闭环行为。
...
### [241] [AVT-Fabric: Active Visuo-Tactile Perception via Adaptive Evidence Selection for Efficient Robotic Fabric Comparison](https://arxiv.org/abs/2609.21377)
*Chang Gao,Zhuo Chen,Suhang Xia,Jihong Zhu,Jiankang Deng,Shan Luo*

Main category: cs.RO

TL;DR: 提出AVT-Fabric，一个RGB优先的机器人织物比较框架，根据比较难度自适应决定是否补充触觉证据，以提升视触觉推理的准确率与效率。


<details>
  <summary>Details</summary>
Motivation: 机器人织物比较需要结合视觉外观和触觉线索，但固定或被动融合多模态信息可能低效且难以兼顾准确率与计算成本，因此需要按需选择触觉证据。

Method: 采用RGB优先策略；通过双尺度门控评估答案token置信度和原始logit分离度，决定是否再获取带力标记的GelSight触觉观测；用紧凑文本记忆保存执行历史，并通过多数投票整合预测；在7B MLLM上实现，并部署到真实机器人系统。

Result: 在400个留出比较上，AVT-Fabric以7B MLLM达到98.0%准确率，超过90B MLLM-Fabric基线的94.0%达4.0个百分点，平均仅处理5个阶段中的1.60个；相比匹配的被动推理提升9.25个百分点，模型侧延迟降低61.8%，并优于仅RGB方法。四个额外MLLM骨干支持其泛化性、准确性和效率。真机部署达到78.1%成对排序准确率，并在8个应用场景中7个正确选择织物。

Conclusion: 自适应证据选择能够同时提升机器人视触觉推理的准确率和效率，表明按难度动态补充触觉信息是有效方向。
...
### [242] [A Unified Dynamic Force Guidance Framework for Performance-Optimized Kinesthetic Teaching](https://arxiv.org/abs/2609.21416)
*Chunxin Li,Jianhua Wu,Zhenhua Xiong,Xiangyang Zhu*

Main category: cs.RO

TL;DR: 本文提出一种在线动态力引导方法，在拖动示教中通过可变导纳控制和虚拟力引导，将机器人操作性能维持在阈值以上并优化构型，从而提升示教轨迹质量与工作效率。


<details>
  <summary>Details</summary>
Motivation: 协作机器人常用于产品频繁切换的工业场景，拖动示教虽直观高效，但用户容易忽略机器人构型配置，导致操作性能下降，进而影响后续作业效率。

Method: 以雅可比矩阵最小奇异值量化操作性能，提出融合性能约束与优化机制的在线动态力引导方法：可变导纳控制确保操作性能不低于预设阈值，虚拟力主动引导用户将机器人拖向性能更优的构型。

Result: 在6自由度协作机器人上针对任务空间三条典型路径进行实验，并通过轨迹回放分析操作性能与工作效率的关系。结果表明，所提方法有效提升了机器人操作性能，并相应提高了工作效率。

Conclusion: 该方法有助于减少工业生产部署中的节拍时间，对协作机器人快速部署与效率优化具有重要价值。
...
### [243] [Robotic Multiphase Interaction: Manipulating Coupled Liquid and Solid Dynamics with a World Model](https://arxiv.org/abs/2609.21448)
*Yixuan Feng,Peng Wang*

Main category: cs.RO

TL;DR: 提出机器人多相交互（RMI）设定：液体进入多孔材料并与变形固体骨架机械耦合。以水浸海绵为示例，采用隐式不可压缩多孔流与SPH动力学引擎，通过库仑接触记忆、混合速度/力调节和提升稳定性门实现操作。世界模型预测液固耦合状态演化，时序UNet用Diffusion Policy或rectified flow matching生成动作。


<details>
  <summary>Details</summary>
Motivation: 多数操作任务针对固体，或涉及液固但保持两相空间分离。液体渗入多孔材料会改变孔隙体积、排出或重分布液体，并同时影响抓取稳定性；溢出的液体还可能带来安全风险，因此需要研究这类耦合交互的机器人操作。

Method: 使用隐式不可压缩多孔流与平滑粒子流体动力学（SPH）作为动力学引擎；加入库仑接触记忆、混合速度和力调节，以及用于提升的稳定性门；构建以动作为条件的世界模型预测耦合状态演化；采用时序UNet生成动作，可选Diffusion Policy或rectified flow matching。

Result: 世界模型将保水预测误差较基线降低超过60%；世界模型从策略提议中选出的最佳动作序列进一步将预测终端水误差减少约一半。

Conclusion: 对液固耦合状态建模有助于机器人预测其动作如何影响多孔物体及内部保持的液体，从而改善操作预测与决策。
...
### [244] [AtomEgo: Exploring Ego-Robot Integration for Embodied Foundation Model Pretraining](https://arxiv.org/abs/2609.21461)
...
TL;DR: 提出 AtomEgo，系统研究以自我中心人类交互数据辅助机器人具身基础模型预训练的 ego-robot 协同训练方法，并基于约 2,659 小时数据和可扩展数据处理管线，在多种模型架构与训练范式下验证其效果。


<details>
  <summary>Details</summary>
Motivation: 具身基础模型受限于机器人演示数据规模小、多样性不足，而大规模自我中心人类交互数据有潜力弥补这一缺口；但人类与机器人在具身形态和动作空间上存在显著差异，如何有效将其用于预训练仍不清晰。

Method: 构建约 2,659 小时的精选语料和可扩展数据处理管线；在视觉-语言-动作与世界-动作模型架构下，研究三类代表性范式：带领域特定动作头的联合协同训练、通过具身对齐的渐进式 ego-to-robot 迁移、联合视频-动作建模；并通过多任务真实机器人实验和语言条件跨具身表示分析进行评估。

Result: 实验揭示一个简单原则：Data Scale × Alignment Quality → Capability Gain。自我中心数据能够提升泛化能力，但其价值取决于数据与机器人之间的对齐质量及利用方式。

Conclusion: 该原则可为可扩展的 ego-robot 预训练提供实践指导，强调在扩大数据规模的同时，必须重视跨具身对齐与有效利用。
...
### [245] [2nd Place Solution to the HANDS 2026 Workshop Challenge-Dexterous Grasp Motion Track: Single-Shot Trajectory Warping for Grasp Motion Generation](https://arxiv.org/abs/2609.21511)
*Muneeb A. Khan,Woojin Kim,Shinwoo Kim,Muhammad Munsif,Binod Bhattarai,Seungryul Baek*

Main category: cs.RO

TL;DR: 本文提出 HANDS 2026 双手抓取运动挑战赛（ECCV 2026）的亚军方案，针对 12 自由度 LinkerHand O6，通过编辑单个成功 GraspM3 演示而非逐帧生成运动，实现未见物体的抓取运动生成。


<details>
  <summary>Details</summary>
Motivation: 该挑战要求从随机初始手部姿态出发，为未见物体生成物理合理的伸手与抬升轨迹。难点在于每步策略需做出约 70 个 12 维决策，误差会随时间累积，且测试物体与物理动力学可能与训练时不同。

Method: 方法不逐步生成动作，而是编辑一个成功的 GraspM3 演示：策略仅观察物体一次，输出一个 12 维演示形变（warp），随后以开环方式重放该形变轨迹；并使用 one-step PPO 在全部 4,824 个训练物体上并行训练形变策略。

Result: 在私有测试集上，方法在 easy track 达到 94.61% 成功率，为所有提交中最高；在 hard track 达到 57.18% 成功率。

Conclusion: 通过单次物体观测输出演示形变并开环重放，可有效避免逐步决策的误差累积，在所设挑战中取得优异成绩，并获得 HANDS 2026 挑战赛第二名。
...
### [246] [Learning Distance-Conditioned Object Transport for Humanoid Loco-Manipulation from a Single Motion Clip](https://arxiv.org/abs/2609.21467)
*Yuhyeon Hwang,Daniel Sungho Jung,YongHyeok Seo,Mingi Jung,Chang Nho Cho,Jung-Hoon Hwang,Dongin Shin*

Main category: cs.RO

TL;DR: 论文研究从单个重定向运动片段训练人形机器人搬运/操作策略时，固定参考导致策略只能复现演示终点的运输结果。作者提出距离条件参考重组（DCRR），将演示终止片段迁移到中间运输状态，并用冻结跟踪教师生成闭环轨迹，再按实际物体放置重新标注并蒸馏为无参考策略。


<details>
  <summary>Details</summary>
Motivation: 现有运动跟踪方法虽能从单个重定向运动片段复现人形移动操作，但策略主要复现演示的最终运输结果。源轨迹虽经过中间物体位移，却只在终点演示运输终止，因此存在“终止与经过”不匹配：中间位移只是经过状态，而非终止完成结果。

Method: 提出 Distance-Conditioned Reference Recomposition（DCRR）：把演示中的终止片段重新定位到中间运输状态；使用冻结的跟踪教师策略在闭环动力学下重放重组参考；保留成功轨迹，并根据其达到的物体放置结果重新标注；最后蒸馏为无参考策略。RL微调进一步优化命令响应与执行鲁棒性。

Result: 在 Carry、Kick-Push、Crouch-Push、Drag 上，DCRR-BC 实现了随命令变化的运输，总体归一化距离 MAE 为 0.15，而仅用源行为的 BC 为 0.28。RL 微调在训练仿真器和 sim-to-sim 迁移中进一步提升了命令响应与鲁棒性。硬件实验表明四种交互模式均可实现运输距离调制。

Conclusion: DCRR 能利用源运动中的交互行为构造距离条件监督，将单片段演示扩展为可控制运输距离的技能，并在仿真与真实硬件上验证了有效性。
...
### [247] [Adaptive Rollout Truncation Based on Epistemic Uncertainty for Efficient Offline World Model Training](https://arxiv.org/abs/2609.21482)
*Nikodem Sebastian Zymla,Laurin Thiele,Johannes Pitz*

Main category: cs.RO

TL;DR: 本文提出一种基于认知不确定性的自适应展开策略，用于离线世界模型训练。当认知不确定性超过预热阶段校准的阈值时，提前终止自回归展开，从而在保持或提升预测精度的同时大幅减少计算量。


<details>
  <summary>Details</summary>
Motivation: 多步自回归训练能提升长时程预测能力，但固定展开长度会增加计算成本，并在模型尚不准确时放大早期训练误差。现有训练方案在整个优化过程中使用相同展开长度，未考虑模型当前的预测可靠性。

Method: 采用自动课程训练框架，提出认知不确定性驱动的自适应展开。使用两种不确定性估计器：共享循环骨干的五头集成和蒙特卡洛丢弃。通过两阶段预热稳定不确定性估计，再根据预热校准的阈值启用自适应截断。

Result: 在ANYmal-D和ANT上的实验表明，基于集成的自适应截断在预测精度上匹配或优于固定长度训练和RWM-U基线，同时所需累计展开步数显著减少。在ANYmal-D上训练世界模型达到与基线相当的最终性能，展开计算量减少约72%。

Conclusion: 认知不确定性不仅可用于下游策略正则化，还能使世界模型训练本身更高效。自适应展开策略在离线世界模型训练中有效平衡了精度与计算成本。
...
### [248] [Potential-Field Action Representation for Reinforcement Learning in Contact-Rich Manipulation](https://arxiv.org/abs/2609.21609)
*Xinyu Liu,Gökhan Solak,Arash Ajoudani*

Main category: cs.RO

TL;DR: 提出PA-RL框架，用人工势场作为动作表示，让策略调节势场参数而非直接输出运动指令，并通过笛卡尔阻抗控制器执行，在轴孔装配任务中取得优于常见动作空间的成功率、运动平滑性和真机部署效果。


<details>
  <summary>Details</summary>
Motivation: 无模型强化学习可用于接触丰富的机器人操作，但策略常需同时学习任务策略和底层运动生成；动作表示决定策略输出如何转化为机器人运动，直接影响探索和物理执行。直接笛卡尔指令要求每步生成运动，耦合了任务适应与连续底层控制，增加学习负担。

Method: PA-RL将人工势场作为动作表示：策略不直接命令运动，而是调节类能量势场的参数，势场生成状态相关的引导方向，再由笛卡尔阻抗控制器执行。在轴孔插入这一典型接触丰富任务中，与笛卡尔速度、笛卡尔位姿和变阻抗动作空间使用相同RL算法进行仿真对比。

Result: PA-RL是唯一在限定训练时间内达到100%评估成功率的方，最佳基线为92.6%。相比最佳基线，关节力矩变化降低55.4%，笛卡尔加速度变化降低70.8%，且奖励中未加入显式运动质量惩罚。仿真训练的策略无需微调即可完成9/9次真机插入。

Conclusion: 以势场为动作表示的接口能有效减轻策略学习负担，提高接触丰富任务的成功率与运动平滑性，并展现出从仿真到真机部署的可行性。
...
### [249] [FORTE: Task-Adaptive Force Capability Optimization for Mobile Manipulators](https://arxiv.org/abs/2609.21497)
*Xiao Wang,Heng Zhang,Gokhan Solak,Fei Zhao,Arash Ajoudani*

Main category: cs.RO

TL;DR: 提出面向任务的冗余移动机械臂力能力优化框架，利用视觉语言模型推断物体属性并生成期望任务力，结合动态残余力多面体定义任务力能力指标，进行全身多目标轨迹优化，实现重载有力、轻载灵巧的任务自适应平衡。


<details>
  <summary>Details</summary>
Motivation: 现有冗余度分解方法常忽略任务特定力需求，或盲目最大化力能力，在不需要大力余量时牺牲灵巧性；因此需要一种能根据任务需求自适应平衡力能力与灵巧性的方法。

Method: 使用VLM从RGB图像和任务描述推断物体物理属性，生成包含重力和惯性需求的任务力序列；定义任务力不确定性球与动态残余力多面体（RFP）之间的有符号距离作为任务力能力指标；将其与可操作度、关节限位避免、轨迹平滑和基座振荡抑制一起纳入全身多目标轨迹优化。

Result: 在移动机械臂执行不同负载下的举升和单点保持任务中，方法对重载提供足够力能力，对轻载保持高可操作度，实现任务自适应平衡；固定能力最大化基线（RFP内切半径、RFP锥）和仅可操作度优化无法实现该平衡。

Conclusion: 面向任务的力能力优化可有效协调移动机械臂的力交互能力与灵巧性，优于固定能力最大化或仅可操作度方法；核心实现已公开。
...
### [250] [SynthDemo-RL: Breaking the Zero-Reward Barrier in VLA Adaptation with LLM-Guided Synthetic Demonstrations](https://arxiv.org/abs/2609.21650)
*Hiroaki Kingetsu,Hiroaki Kurihara,Kaoru Yokoo,Kenji Fukumizu,Manohar Kaul*

Main category: cs.RO

TL;DR: 论文提出 SynthDemo-RL 师生框架：自动教师利用仿真器特权状态生成成功操作轨迹，VLA 学生通过 SFT 蒸馏，再用二值任务成功奖励的 PPO 精炼，从而在无新人类演示下提升稀疏奖励任务的成功率与覆盖度。


<details>
  <summary>Details</summary>
Motivation: 微调 VLA 模型通常依赖人类遥操作演示，而稀疏二值奖励强化学习在成功轨迹稀少时面临探索困难，因此需要一种无需大量人类演示即可获得成功轨迹并有效强化学习的方法。

Method: 提出 SynthDemo-RL 教师-学生框架：自动教师将仿真器特权状态转换为成功操作轨迹；VLA 学生通过监督微调 SFT 从合成轨迹中蒸馏；再用 PPO 和二值任务成功奖励精炼学生。同时引入 reward coverage，衡量固定评估协议下至少观测到一次成功的任务比例。

Result: 在 LIBERO-PRO 上，基于原始 LIBERO 微调的 pi_0.5 策略有 27/57 个计分任务成功率为 0%；直接 PPO 仅挽救其中 10 个，仍有 17 个为 0%。SynthDemo-RL 每任务用 50 条合成轨迹且无新人类演示，挽救全部 27 个任务，Position 和 Task 轴平均成功率分别达 97.8% 和 97.1%。标准 LIBERO 上达 96.0%，与每任务 50 条人类演示训练的 pi_0.5 相差 1.7 个百分点。RoboTwin 2.0 上进一步验证，MuJoCo 孪生中训练的策略轨迹可在物理机器人上开环执行。

Conclusion: SynthDemo-RL 表明，利用仿真器特权状态自动生成演示并结合 SFT 与 PPO，可在无需新人类演示的情况下显著提升稀疏奖励 VLA 任务的成功率和覆盖度，并具备迁移到真实机器人的潜力。
...
### [251] [Outcome-Conditioned End-Effector Geometry Across Vision-Language-Action Policies](https://arxiv.org/abs/2609.21659)
*Xingyu Lin,Zhuang Li,Zhongrun Wu,Shouquan Zhou,Dehui Du*

Main category: cs.RO

TL;DR: 本文研究不同视觉-语言-动作（VLA）策略在相同操作任务中的末端执行器几何一致性，通过15,000次LIBERO闭环rollout比较跨策略轨迹距离，并检验成功等级、任务、表示和扰动等因素下的差异。


<details>
  <summary>Details</summary>
Motivation: 仅用任务成功率无法判断采用不同动作接口的VLA策略是否产生一致的物理执行，因此需要分析跨策略末端执行器轨迹几何。

Method: 使用四个策略在LIBERO中生成15,000次闭环rollout；主分析构建3,600个配置匹配且相互依赖的策略对；计算归一化动态时间规整距离；在任务、策略对、九种采样与带限表示、72步窗口、端点与时长调整及复合视觉压力下比较。

Result: 双成功策略对的中位归一化DTW距离为0.0120 m，而仅一方成功时为0.0380 m；该排序在所有任务、策略对和九种表示中成立，但比值随表示变化数倍，故报告方向而非固定倍数。双失败对分离更大但样本支持薄弱，仅作探索性结果。成功执行中，替换伙伴造成的差异大于不同初始状态；匹配基线仍显示可测且异质的残余策略差异；成功执行与同任务演示的距离约等于演示彼此距离；72步窗口保持排序但降低幅度；复合视觉压力下策略排名与配对组成同时变化。

Conclusion: 低跨策略距离并不代表策略可互换；成功执行呈现任务相关几何，但仍无法区分训练数据重叠与任务约束的贡献，且视觉压力会改变策略比较结果。
...
### [252] [Skel-WAM: A Hand-Skeleton-Conditioned World Action Model for Human-to-Robot Manipulation Transfer](https://arxiv.org/abs/2609.21514)
*Zetao Cai,Yaping Li,Yiqun Wang,Xinyu Zhan,Yuyin Yang,Haoxiang Ma,Kailin Li,Tao Lu,Jiangmiao Pang,Linning Xu,Dahua Lin*

Main category: cs.RO

TL;DR: 提出Skel-WAM世界动作模型，通过统一手部骨骼运动接口桥接人类视频与机器人演示，实现人机数据协同训练并提升操作任务成功率。


<details>
  <summary>Details</summary>
Motivation: 机器人演示采集成本高且任务变化覆盖有限；人类视频是低成本补充经验，但存在视觉外观和动作空间上的具身差距，难以直接用于机器人学习。

Method: Skel-WAM以统一手部拓扑对齐人和机器人运动，结合骨骼叠加将运动定位到场景，并用结构化2.5D关键点编码手部运动学；Video与Keypoint Experts通过Mixture-of-Transformers联合学习视觉和骨骼动力学，独立的机器人训练Action Expert将预测映射为可执行控制，使人类视频无需机器人动作标签即可监督共享动力学。

Result: 在4个真实双臂任务和7个仿真任务上，平均成功率达79.86%和63.29%，分别超过最强基线22.22和8.28个百分点；人机协同训练使机器人训练数据未覆盖任务变体的真实世界成功率从38.89%提升至86.11%，提升超过一倍。

Conclusion: 共享骨骼接口能够实现人类与机器人数据的联合学习，并利用互补的人类演示扩展机器人任务覆盖范围。
...
### [253] [DPed-VLN: A Benchmark for Socially Compliant Vision-and-Language Navigation in Dynamic Pedestrian Environments](https://arxiv.org/abs/2609.21504)
*Haojie Dai,Xiangyi Wang,Liuyi Wang,Kai Sheng,Zongtao He,Chengju Liu,Wei Ye,Qijun Chen*

Main category: cs.RO

TL;DR: 提出DPed-VLN动态行人视觉语言导航基准，包含33,093个导航回合、配对指令、ORCA行人、社交安全指标，并引入DPet网络和LoRA适配VLM模型；DPet-RL在SR、SPL、STL上最优。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航在静态室内环境进展快，但真实机器人需在有人群动态环境中理解语言并响应移动行人及社会安全约束，缺少相应基准。

Method: 构建基于Habitat 3.0的DPed-VLN基准，含33,093个导航回合、全局与先验增强配对指令、ORCA控制人形行人、社会约束专家路径及联合评估指标；提出DPet行人感知策略网络，用强化学习和模仿学习训练；通过LoRA微调将NaVILA、StreamVLN等VLM导航模型适配到该基准。

Result: LoRA适配在多个成功与安全指标上提升零样本VLM基线，尤其降低StreamVLN碰撞率；所有评估方法中DPet-RL取得最高SR、SPL和STL。

Conclusion: DPed-VLN为动态行人视觉语言导航提供可控基准，DPet-RL表现最佳，LoRA微调可有效适配VLM模型，推动社交安全导航研究。
...
### [254] [SABER: Learning Attention-based Semantic Affordance for Legged Locomotion](https://arxiv.org/abs/2609.21572)
*Hari Prasanth Palanivelu,Samuel Sze,Kennard Garrison Johannes,Albertus Hendrawan Adiwahono,Meng Yee,Chuah*

Main category: cs.RO

TL;DR: 本文提出SABER，一种无规划器的强化学习策略，将地形几何与语义接触许可联合推理，使四足机器人能在工业环境中避免踩踏脆弱或禁止接触的物体。


<details>
  <summary>Details</summary>
Motivation: 现有感知型四足运动虽已融入地形几何，但缺乏对地形语义的理解。管道、草地或脆弱箱子可能几何上可通行，却不适合接触；误踩可能导致设备损坏、机器人失稳或场地危险。

Method: SABER使用统一的地形可供性地图，每个网格同时编码局部3D几何和语义接触代价。策略在交叉注意力中加入可学习的带符号语义偏置：该偏置加在注意力logits上，并由接触代价门控，根据离最近足端的距离重新加权被标记网格。危险区域只在能影响下一步落脚点时改变注意力，影响随距离衰减。

Result: 消融实验表明，仅移除语义偏置就会使禁止接触增加55%，而速度跟踪性能不变。在Unitree B2上验证，实现了室内外环境及四类语义障碍的仿真到现实语义接触选择。

Conclusion: 联合几何与语义接触许可可使四足机器人选择允许支撑的落脚点，并在摆动阶段避开禁区，从而提升工业场景中的安全性和语义感知运动能力。
...
### [255] [ForceTwin: Physics-informed Digital Twins for Robotic Manipulation from Instrumented Human Interaction](https://arxiv.org/abs/2609.21751)
*Tim Engelbracht,René Zurbrügg,Mayank Mittal,Marco Hutter,Marc Pollefeys,Hermann Blum,Zuria Bauer*

Main category: cs.RO

TL;DR: ForceTwin 通过手持力传感夹爪采集人类交互中的位姿与力，识别包含惯性、摩擦、阻尼及状态相关机构力的物理信息数字孪生，并用于机器人阻抗控制与开门策略。


<details>
  <summary>Details</summary>
Motivation: 操作铰接物体需要理解惯性、摩擦、弹簧/闭门器等物理属性，但这些属性无法仅从外观获知；现有数字孪生流程主要恢复运动学或依赖视觉语言先验设定静态物理参数，导致估计不合理且无法建模状态相关机构动力学。

Method: 使用带力传感的手持夹爪让人探测物体，获取同步位姿和交互力；估计铰接结构、参数化动力学（惯性、库仑摩擦、粘性阻尼）以及一个结构化神经残差，用于捕捉状态相关的机构力。

Result: 相比 VLM 先验，ForceTwin 将近乎减半惯性参数误差；作为 Spot 和 Franka FR3 阻抗控制的前馈动力学模型，在九种物体-本体组合上达到 87% 目标完成率，而 VLM 先验为 60%、仅运动学孪生为 57%，在强机构物体上提升最大。

Conclusion: 基于力交互识别的物理信息数字孪生能更准确表示铰接物体动力学，显著提升机器人操作性能，并可进一步训练全身开门穿越策略并部署到真实世界。
...
### [256] [A High-Payload Wall-Climbing Robot Using Passive Bistable Suction Cups](https://arxiv.org/abs/2609.21584)
*Andrew Nguyen,Mingyuan Li,Daniel Bruder*

Main category: cs.RO

TL;DR: 提出一种高负载壁面爬行机器人，利用被动双稳态吸盘和轨道式自动接合/脱离机构，在无需持续供电维持吸附的情况下实现垂直、侧向和倒置爬行，并能拖动7.940 kg负载。


<details>
  <summary>Details</summary>
Motivation: 壁面爬行机器人可用于窗户清洗、检测、维护和建筑等危险或劳动密集任务；主动吸附负载高但需持续供能，被动吸盘无需供能维持吸附但负载能力有限。

Method: 采用被动双稳态吸盘产生吸附力，无需将其压向墙面；设计轨道式系统自动接合与脱离吸盘，实现光滑表面上的运动。

Result: 机器人可在玻璃、木材、金属和涂漆表面垂直爬壁，也可侧向和倒置爬行；可拖动7.940 kg负载，负载重量比为2.25。

Conclusion: 被动双稳态吸盘结合轨道式接合/脱离机构，可在不持续供能维持吸附的条件下实现高负载壁面爬行，为高危或劳动密集壁面作业提供有前景方案。
...
### [257] [CounterPlay: Counterfactual Post-Training for Self-Play Driving Policies](https://arxiv.org/abs/2609.21617)
*Jiarong Wei,Yin Wu,Runkai He,Abhinav Valada*

Main category: cs.RO

TL;DR: 论文提出 CounterPlay，一种反事实自博弈后训练方法，通过从失败任务回溯并在不同驾驶风格下重试，以少量后训练数据显著提升自动驾驶策略性能。


<details>
  <summary>Details</summary>
Motivation: 高吞吐模拟器中的自博弈能产生闭环性能鲁棒的驾驶策略，但随着训练规模扩大，单位模拟带来的提升递减；策略早期学会常见场景，后续 rollout 反复遇到未解决的失败。现有后训练方法主要在已访问状态评估替代动作或延续，但成功恢复可能需要更早改变驾驶风格。

Method: CounterPlay 包含三个关键组件：1）失败驱动回溯，利用策略的价值估计选择更早的存储状态重试任务；2）奖励条件化，使单一策略能从此状态以谨慎到激进的候选风格重试；3）仅当其他车辆相对事实分支没有新增或更早的碰撞/偏离道路事件时，才保留完成任务的重试，并通过新鲜随机性验证后，在部署条件下蒸馏进策略。

Result: 在 BehaviorBench 上，CounterPlay 使用 10 亿后训练 transition（仅为基线 1000 亿自博弈预算的 1%）在 Interactive 和 Random 划分的全部八种交通场景中取得 SOTA 分数；改进在三种评估驾驶风格下均成立；解决了基线大量超时案例，并在任务完成与安全性之间取得平衡，优于持续自博弈或采用更激进驾驶风格。

Conclusion: 从失败中回溯并以反事实驾驶风格重试，再经安全验证与蒸馏，是一种高效的后训练策略，能在极低模拟预算下提升闭环驾驶性能并改善完成与安全的权衡。
...
### [258] [Towards Fine-Grained Object Manipulation: SAM3-Guided Visuomotor Policy with Persistent Memory Learning and Focused Visual Conditioning](https://arxiv.org/abs/2609.21621)
*Haolong Meng,Fangbo Qin,Mengchen Bai,Houwu Wang,Cirong Liu,Shan Yu*

Main category: cs.RO

TL;DR: 提出一种SAM3引导的视觉运动框架，通过持久对象记忆与聚焦视觉条件，解决细粒度物体操作中相似物体区分和场景干扰问题。


<details>
  <summary>Details</summary>
Motivation: 场景级视觉条件缺乏显式物体选择，类别级引导难以可靠区分同类别细粒度物体，而机器人需要准确指定并操作目标细粒度物体，同时抵抗相似物体和场景干扰。

Method: 提出FOM-SAM3，在冻结SAM3的情况下，从有限多视角注册图像学习可复用的细粒度物体记忆token，并通过one-vs-rest学习编码持久记忆以定位目标并拒绝相似替代物，存入记忆库；提出FSAE，将细粒度物体内部局部外观特征与显式边界框坐标结合，用于条件化DP和ACT等动作策略。

Result: 在包含4个粗类别、30个物理对象的FO-30数据集上验证FOM-SAM3；在三个真实机器人细粒度物体操作任务中，基于FOM-SAM3的策略展现出对干扰物的鲁棒性、对相似细粒度物体的区分能力以及对新物体的可扩展性。

Conclusion: SAM3引导的视觉运动框架结合持久对象记忆和聚焦视觉条件，能够提升真实机器人细粒度物体操作的鲁棒性、判别能力和可扩展性。
...
### [259] [When Should a Failing Robot Ask? Initiating Corrective Human-Robot Dialogue from Audited Sensor Evidence](https://arxiv.org/abs/2609.21942)
*Eshika Pathak,Leela Krishna*

Main category: cs.RO

TL;DR: 论文研究机器人任务失败后，应在自主诊断、查询自身传感器或询问人类之间如何选择。作者构建可注入真实故障原因的仿真基准，测量不同传感器的可诊断性，并测试六个开放视觉语言模型。结果显示模型行为受提示表面影响而非证据，缺少力传感器数据是重要原因；是否提问应依据测量准确率和成本，而非模型置信度。


<details>
  <summary>Details</summary>
Motivation: 机器人失败后需要决定：相信自身诊断、调用其他传感器，还是打断人类。做出好决策需要知道传感器能揭示多少故障原因，以及机器人自身诊断有多可靠。现有视觉语言模型可能被用作决策者，但其是否依据证据、能否权衡提问成本尚不清楚。

Method: 构建仿真基准，故障真实原因已知，通过注入故障并显式检查数据泄漏，测量相机、力传感器等对故障原因的可诊断性；测试六个开放视觉语言模型，改变提示中拒答选项位置、是否给示例、置信度等；将力数据转为十行文本输入；把问题形式化为三动作决策问题：行动、咨询自身传感器、询问人类，并由测量准确率推导最优策略，同时改变提问成本。

Result: 部分故障只能从力数据诊断，力数据准确率达0.99，而图像方法不超过0.55。六个模型中，拒答选项从最后移到最前，使三组模型拒答率从78-100%降至0-6%；基于帧的准确率不超过多数类基线，模型置信度与正确性无关。给模型力数据文本后，六个模型中有四个首次超过基线，说明失败部分源于缺少传感器数据。模型不遵循最优策略，提问率无视四倍提问成本变化；当它们询问人类时，准确率可提升至回答者自身可靠性0.70-0.81。

Conclusion: 是否询问人类应取决于测量准确率和明确成本，而不是模型置信度。视觉语言模型对提示表面敏感，不能可靠地按证据决策；其失败部分来自缺少传感器输入，而非完全缺少能力。
...
### [260] [NeuRIO: A Streaming Neural Estimator for Zero-Shot Sim-to-Real Multi-Robot Relative Inertial Odometry](https://arxiv.org/abs/2609.21707)
*Zhehan Li,Jiadong Lu,Shengwei Ren,Chao Xu,Yanjun Cao*

Main category: cs.RO

TL;DR: NeuRIO 是一种无需锚点的流式神经相对惯性里程计，仅利用机器人间可识别的方位、距离和 IMU 测量，实现 6-DoF 相对位姿估计，并支持零样本 sim-to-real 迁移。


<details>
  <summary>Details</summary>
Motivation: 多机器人协同中，传统优化方法在团队规模增大和观测图时变时计算扩展性差；研究希望构建无需锚点、可适应不同团队规模与动态观测图，并能从仿真直接迁移到真实世界的相对惯性里程计。

Method: 将测量统一到重力对齐坐标系；把机器人表示为节点、相互观测表示为因子；用注意力进行空间推理，用 GRU 进行时序建模；采用共享节点级和因子级算子的图网络，以适应不同团队规模和时变观测图；在耦合多种运动模式、设备级传感器特性和持久传感器损坏的模拟器上训练。

Result: 在 24 个真实世界序列上，NeuRIO 达到 14.1 cm 位置 RMSE 和 3.9° 旋转 RMSE；计算扩展性强，仿真中最多 400 个机器人时更新耗时低于 20 ms，而优化方法在仅 24 个机器人时已超过 20 ms；即使仅用有限团队规模训练，也能无需改变架构或参数直接迁移到更大的未见团队。

Conclusion: NeuRIO 实现了高精度、强泛化且可扩展的实时相对惯性里程计，具备零样本 sim-to-real 迁移能力，并能直接推广到更大规模团队。
...
### [261] [AgenticSwarm: Semantic Perception and Adaptive Task Allocation for Heterogeneous Multi-UAV Missions](https://arxiv.org/abs/2609.21716)
*Muhammad Ahsan Mustafa,Yasheerah Yaqoot,Faryal Batool,Roohan Ahmed Khan,Valerii Serpiva,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 提出AgenticSwarm智能体框架，用于异构多无人机任务的语义感知与自适应任务分配，并在仿真和真实环境中验证。


<details>
  <summary>Details</summary>
Motivation: 复杂环境下多无人机任务需要同时理解场景和人类操作员意图，并在任务条件变化时保持可行的任务分配。

Method: 智能体解释航空图像和自然语言指令，构建接地任务表示；将障碍物感知路径可行性、能量消耗和返航要求纳入约束任务分配；执行中遇到故障、电量下降或任务修改时，基于当前状态重构剩余任务并保留已完成工作和侦察进度。

Result: 在五个Gazebo环境和室内真实环境评估中，SAM3流程比Grounding DINO+SAM2.1基线在类感知召回率上提升25.2个百分点，语义标签准确率提升29.5个百分点；消融剩余任务重规划后，平均重复工作从0%增至61.7%，事件后恢复时间增加58.6%。

Conclusion: AgenticSwarm能将语义推理与约束分配及自适应多无人机任务执行结合，自适应重规划对任务执行有重要贡献。
...
### [262] [Visual Proactivity: Enhancing Human-Robot Collaboration Through Intent Communication](https://arxiv.org/abs/2609.21729)
*Valerio Bo,Edison Bejarano,Anaís Garrell,Alberto Sanfeliu*

Main category: cs.RO

TL;DR: 本文提出“视觉主动性”方法，使机器人能通过视觉反馈传达自身意图，从而影响人类行为并提升人机协作的透明性与流畅性。


<details>
  <summary>Details</summary>
Motivation: 机器人从执行重复任务转向与人协作时，理解人类意图很重要，但机器人如何向人类传达自身意图的研究相对不足。

Method: 提出视觉主动性方法，并在人-机器人递物场景中开发与评估反应式、预期式和主动式机器人行为，通过用户研究验证人类感知。

Result: 用户研究表明，有效的视觉主动性能够促进更好的人机对齐与协调。

Conclusion: 视觉主动性有助于实现更直观的人机协作，为机器人主动沟通意图提供了简单有效的途径。
...
### [263] [ZeroTouch: Tactile-Supervised Visual Contact Estimation for Contact-Rich Manipulation](https://arxiv.org/abs/2609.21726)
*Dmitriy Kosenkov,Daniia Zinniatullina,Miguel Altamirano Cabrera,Iana Zhura,Mikhail Derevianchenko,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: ZeroTouch 是一个无需部署触觉传感器的触觉监督框架，仅利用腕部 RGB 图像、夹爪状态和局部重力方向，预测接触形变、六轴力/力矩以及抓取相关的目标压缩量，从而提升机器人抓取可靠性。


<details>
  <summary>Details</summary>
Motivation: 可靠抓取需要估计动态物理交互并选择抓取相关的压缩目标，但触觉传感器虽能直接测量交互，却需要部署专用硬件，增加成本和复杂度。

Method: 提出 ZeroTouch，一种触觉监督框架：训练时使用触觉测量作为特权监督，部署时不需要触觉传感器；输入为腕部 RGB 观测、夹爪状态和局部重力方向，输出包括密集接触形变、瞬时六轴力/力矩和抓取相关的期望压缩目标。

Result: 在完整验证集上，完整架构将法向力 MAE 从仅状态基线的 2.017 N 降至 0.531 N。物理评估中，每条件 20 次试验：ZeroTouch 在未见物体、已见物体/未见抓取、内容/负载偏移条件下成功率分别为 95%、80%、90%；相同协议下 OpenVLA 为 25%、40%、55%，SmolVLA 为 10%、25%、35%。

Conclusion: ZeroTouch 证明可以通过触觉监督学习，在部署时无需触觉硬件即可预测关键接触和力信息，并显著提升机器人在多种抓取条件下的成功率与鲁棒性。
...
### [264] [When Should Robots Intervene? Balancing Engagement and Intrusiveness in Human-Robot Interaction](https://arxiv.org/abs/2609.21734)
*Lavinia Hriscu,Valerio Bo,Alberto Sanfeliu,Anaís Garrell*

Main category: cs.RO

TL;DR: 该论文研究任务导向人机交互中机器人干预策略对用户体验的影响，比较持续寻求互动与基于用户状态和任务情境的选择性干预两种策略。


<details>
  <summary>Details</summary>
Motivation: 在任务导向的人机交互中，需要平衡用户参与度与机器人可接受的侵入性，因此需要理解不同干预策略如何影响用户支持感、侵入感和互动动态。

Method: 比较持续参与寻求策略和情境感知选择性干预策略；两种策略均利用身体朝向和注意信号等多模态行为线索指导机器人行动；在模拟医院环境中对32名参与者进行用户研究。

Result: 更高的互动频率并不一定带来更好的参与度；感知支持与侵入性之间存在系统性权衡，并受物理距离和用户努力程度等因素影响。
...
### [265] [Sandwich-Residuals: Parameter-Efficient Test-time Adaptation of World Models](https://arxiv.org/abs/2609.21740)
*Krishnam Soni,Aditya Sehgal,Vedant Dave,Elmar Rueckert*

Main category: cs.RO

TL;DR: 提出 Sandwich-Residuals 方法，在不修改预训练世界模型内部权重的情况下，仅在预测器周围学习小型残差修正来实现测试时自适应，在 AdaJEPA 基准上优于冻结模型并大幅减少可调参数。


<details>
  <summary>Details</summary>
Motivation: 潜在世界模型在测试条件与训练不同时预测会不可靠；现有测试时自适应方法需要更新大量预训练模型参数，且需选择适配哪些内部组件。

Method: 保持预训练世界模型冻结，仅在线学习预测器周围的小型残差修正；利用模型自监督预测误差优化，无需奖励、标签或源域数据。

Result: 在 AdaJEPA 基准的 21 种条件下，成功率是冻结模型的 1.3 倍，达到最强 AdaJEPA 变体 95% 的性能，且适配参数减少 97-99%；在复合偏移下为冻结模型的 1.9 倍，并与内部块适配相当。在 DINO-WM 的 3D 操作任务上也验证了该原理。

Conclusion: 有效的世界模型测试时自适应不一定需要修改其预训练内部权重。
...
### [266] [Understanding Engagement and Intrusiveness in Assistive Human-Robot Interaction Using Individual Traits](https://arxiv.org/abs/2609.21744)
*Valerio Bo,Lavinia Hriscu,Alberto Sanfeliu,Anaís Garrell*

Main category: cs.RO

TL;DR: 本文研究辅助机器人干预策略与用户个性如何影响人机交互中的投入感与侵入感，发现整体投入稳定，但情感反应和侵入感知因个性而异。


<details>
  <summary>Details</summary>
Motivation: 以往关于机器人距离行为对用户投入感影响的研究结果不一：近距离既可能增强互动，也可能让人感到被侵犯。作者认为侵入感不仅取决于距离，还取决于机器人干预频率，并受个性与人口统计特征等个体差异强烈影响。

Method: 开展实验，32名参与者与两个辅助机器人互动。两个机器人提供相似任务支持，但干预策略不同；研究测量用户投入感、情感反应、感知侵入性，并考察个性等个体差异的影响。

Result: 总体投入感在不同条件下保持稳定，但情感反应和感知侵入性随个性特征显著变化。此外，个性对互动动态的影响取决于机器人的行为方式。

Conclusion: 有效的人机交互应考虑个体差异，根据用户独特的情感与行为特征调整机器人行为，以在维持投入感的同时尊重用户感受。
...
### [267] [CRISP: Contact-Rich Robotic Simulation Platform with Extensive Geometries and Contact Solvers](https://arxiv.org/abs/2609.21761)
*Somang Lee,Sunkyung Park,Jinhee Yun,Seoki An,Dongjun Lee*

Main category: cs.RO

TL;DR: 提出CRISP，一个面向复杂多接触场景（如紧公差机器人操作）的高保真物理引擎，支持多样几何表示、基于优化的碰撞检测以及增广拉格朗日接触求解器。


<details>
  <summary>Details</summary>
Motivation: 现有仿真器在几何表示与接触建模上支持有限，或数值求解方法的精度与鲁棒性不足，难以满足紧公差机器人操作等复杂多接触仿真的高物理保真需求。

Method: CRISP结合多样几何表示与基于优化的精确碰撞检测，并将接触建模与鲁棒的增广拉格朗日接触求解器相结合，实现复杂几何间高效一致的接触信息检测，并在无问题松弛下准确求解多接触约束。

Result: 通过与最先进平台对比验证了CRISP的物理保真度，并通过复杂机器人操作场景展示了其能力；代码已公开于 https://github.com/INRoL/crisp。

Conclusion: CRISP能够有效支持接触密集与尖锐交互的高保真仿真，适用于复杂机器人操作，并已开源供社区使用。
...
### [268] [Scaling Vision-Language Reward Learning for Robot Manipulation in Parallel Simulation](https://arxiv.org/abs/2609.21767)
*Lobna Joualy,Eric Demeester,Nikolaos Tsiogkas*

Main category: cs.RO

TL;DR: RAPID 通过 GPU 并行 rollout、数据感知策略更新、单次请求偏好标注、自动奖励稳定和代表性图像采样，加速基于视觉语言模型的奖励学习，并在 Franka Panda 操作任务上显著提升训练效率与成功率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型可替代人工标注者进行基于偏好的奖励学习，但顺序 API 请求和单环境数据收集导致训练缓慢且成本高昂。

Method: 提出 RAPID 系统，将 GPU 并行 rollout 与数据感知策略更新结合，采用单请求偏好标注、自动奖励稳定化和代表性图像采样；在 IsaacLab 的五个 Franka Panda 操作任务上评估。

Result: 并行 rollout 和自适应更新将平均运行时间从 9.18 小时降至 3.13 小时；启用全部 RAPID 组件后，训练仅需 1.15 小时，API 调用从 19,840 次降至 896 次，总体最终成功率从 86.3% 升至 98.7%，实现 8.0 倍端到端加速和 95.5% API 使用减少。Gemma 3 12B 与 GPT-4.1 mini 的离线评估表明，单请求提示可降低标注延迟和成本。

Conclusion: RAPID 能有效解决 VLM 奖励学习中的慢速与高成本问题，在保持或提升任务成功率的同时大幅加速训练并减少 API 调用。
...
### [269] [TRACE: Coverage Path Planning for Unknown Environments Using Hierarchical Coverage Tree](https://arxiv.org/abs/2609.21777)
*Zongyuan Shen,Haodong Liu,Gao Wang,Shancheng Zhao,Dehua Zhou,Yaming Ou,Zhongqiang Ren,Yikui Zhai,C. L. Philip Chen*

Main category: cs.RO

TL;DR: 本文提出一种面向未知环境实时覆盖的在线覆盖路径规划算法TRACE，基于分层覆盖树组织未覆盖空间，并结合增量全局巡回与局部规划实现高效覆盖。


<details>
  <summary>Details</summary>
Motivation: 未知环境中实时覆盖面临新障碍和已覆盖区域导致剩余空间碎片化的问题，传统全局重规划计算量大且难以保持覆盖连续性。

Method: 构建分层覆盖树表示未覆盖空间的连通性；当空间被揭示和覆盖后，递归扩展树节点以表示分裂区域；维护增量全局巡回，仅局部细化受影响部分并保持未变区域访问顺序；局部规划器生成往返覆盖路径，并切换至全局巡回感知规划完成目标区域。

Result: 理论分析给出了TRACE的计算复杂度、完全覆盖性质及增量全局巡回细化的近似界；大量高保真仿真和移动机器人实验表明，与六种现有CPP方法相比，在覆盖时间、路径长度、重叠率和转弯次数上均有显著改善。

Conclusion: TRACE能够有效实现在线未知环境覆盖，通过分层树与增量规划降低重规划负担，并保持覆盖过程一致性，实验验证了其优于现有方法的综合性能。
...
### [270] [PSR: Predictive Sensorimotor Representation Learning for Contact-Rich Manipulation](https://arxiv.org/abs/2609.21753)
...
TL;DR: 提出预测性感觉运动表征（PSR）学习框架，通过多模态Transformer预训练学习预测性表征层次，并集成到视觉运动策略的动作流中，实例化为PSR-VLA，在六个真实接触密集型操作任务上达到91.7%成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法被动依赖力反馈，而非主动预测未来接触动力学，限制了高精度动作生成，难以处理接触密集型操作。

Method: 提出PSR学习框架：预训练多模态Transformer联合预测未来交互动力学，学习预测性表征层次；将学习到的层次集成到视觉运动策略的动作流中；进一步实例化为PSR-VLA（基于VLA模型）。

Result: 在六个真实世界接触密集型操作任务上，PSR-VLA总体成功率为91.7%，分别比π0.5、ForceVLA-π0.5、ForceVLA2-π0.5提升30.0、22.5、19.2个百分点。

Conclusion: 证明了PSR在力感知、接触密集型操作中的有效性。
...
### [271] [From Pretraining to Proficiency: Real-World Subtask RL for Long-Horizon Manipulation with Minimal Human Intervention](https://arxiv.org/abs/2609.21788)
*Sichang Su,Benjamin Yang,Zhiyun Deng,Boyuan Liang,Yip Fun Yeung,Zelin Wang,Lingfeng Sun*

Main category: cs.RO

TL;DR: 提出PARTS，一种在真实机器人上针对预训练策略薄弱子任务进行强化学习微调的框架，通过冻结基础策略提供标称动作，并由智能体生成的选择器与成功验证器提供局部奖励，从而提升长时程任务成功率。


<details>
  <summary>Details</summary>
Motivation: 预训练机器人基础策略可完成长时程任务的大部分，但常在少数关键子任务上反复失败；收集完整任务演示进行SFT需重复已掌握行为，而现有RL微调在稀疏奖励下难以解决长时程任务。

Method: PARTS将训练集中在瓶颈子任务：冻结预训练策略提供全程标称动作，智能体生成的选择器和成功验证器激活残差修正并给出局部结果奖励；训练结合在线RL与按成功重加权再训练，并重新部署残差策略继续收集经验；人类仅负责识别瓶颈和必要时物理重置。

Result: 在双臂YAM和单臂Franka任务上，PARTS将完整任务成功率分别从32%提升到61%、从50%提升到95%，平均每任务仅需数十分钟真实RL rollout；在相同机器人rollout预算下，相比现有真实世界RL微调方法，完整任务成功率提升超过25%，且所需人工参与更少。

Conclusion: PARTS表明，将真实世界RL集中于预训练策略的瓶颈子任务，并利用局部成功验证提供奖励，可高效适配长时程机器人策略，在较少人工干预和rollout预算下显著提升完整任务成功率。
...
### [272] [Compact but Moving: Intervention-Relevant Geometry in Recurrent World Models](https://arxiv.org/abs/2609.21787)
*Yuming Chen,Yang Liu*

Main category: cs.RO

TL;DR: 该论文研究学习到的循环世界模型中，紧凑干预在进入高维循环状态后的演化。作者发现，一次性的反事实速度修正虽会迅速离开固定的低秩入口子空间，但其通过事实循环雅可比链传输后的低秩像仍能捕捉大部分非线性修正；这种“传输/功能”模式在多种循环模型中重复出现，表明紧凑干预结构可作为移动的、状态依赖的局部几何持续存在。


<details>
  <summary>Details</summary>
Motivation: 已有工作在某些循环世界模型中发现了检查点特定的 rank-4 接口，可用于一次性反事实速度干预，但尚不清楚这种修正进入模型后会发生什么，尤其是在高维循环状态中是否仍保持紧凑或低维结构。

Method: 在受控循环世界模型中研究反事实速度干预；利用事实循环雅可比链传输入口方向，构建低秩切空间预测；通过重启实验检验切线预测修正能否保留反事实未来功能；在独立训练的结构化 GRU 和参数匹配 LSTM 中验证；进一步刻画有限时域未来响应算子，并进行方向面板扰动、修补实验及全幅度测定。

Result: 修正在自主 rollout 中迅速离开固定入口子空间，但经雅可比链传输后的低秩像仍能捕捉大部分非线性修正；使用切线预测修正重启可保留显著的反事实未来功能。该模式在多个独立模型和初始化条件下重复出现。修补会使未来响应算子的主导未来敏感方向转向匹配的原生反事实组织；局部算子能准确排序已注册方向面板上的有限扰动效应。全幅度测定发现显著的事实端点切线残差，并在三个检查点中的两个支持响应重构。

Conclusion: 紧凑干预结构不必意味着固定或动态闭合的低维状态；它可以作为嵌入高维循环动力学中的移动、状态依赖的局部几何而持续存在。
...
### [273] [GALA: Geometry-Aware Latent Action Modeling for Vision-Language-Action Model Pretraining across Embodiments](https://arxiv.org/abs/2609.21948)
*Yichen Liu,Puzhen Yuan,Xiang Zhu,Yanjiang Guo,Jianyu Chen*

Main category: cs.RO

TL;DR: 论文提出 GALA，一种几何感知的潜在动作建模框架，通过引入统一末端执行器运动表示 UEMR，将视觉潜在动作与几何潜在动作结合，用于多本体数据上的 VLA 预训练。


<details>
  <summary>Details</summary>
Motivation: 多本体数据中不同末端执行器的动作空间异质，现有基于图像的潜在动作模型难以捕捉细粒度末端执行器运动，尤其是人手和灵巧手中的手指级几何变化；而直接使用点云虽保留细粒度信息，却缺乏共享语义，影响跨本体预训练。

Method: 提出 GALA 框架，利用 3D 末端执行器几何运动增强图像潜在动作；设计 UEMR 统一末端执行器运动表示，在保留细粒度运动信息的同时提升跨本体泛化能力；结合捕捉场景级动态的视觉潜在动作与捕捉共享细粒度末端执行器关节运动的几何潜在动作，为多本体数据（包括无动作的自中心人类视频）上的 VLA 预训练提供监督。

Result: 在细粒度运动探测、跨本体检索和下游 VLA 评估中验证了 GALA 的有效性，可建模跨本体的可泛化细粒度运动，并取得 68.3% 的 RoboCasa-GR1 成功率和 75.5% 的真实世界成功率。

Conclusion: GALA 能有效利用几何信息提升潜在动作的细粒度与跨本体泛化能力，为多本体数据上的 VLA 预训练提供有效方案。
...
### [274] [Contact-Rich Motion Planning via GPU-Parallel Mode Evaluation](https://arxiv.org/abs/2609.21803)
*Jiayun Li,Georgia Chalvatzaki*

Main category: cs.RO

TL;DR: 本文提出CoMET，结合GPU并行轨迹评估与贪心接触模式扩展，用于接触丰富运动规划，在平面推动和双臂非抓取操作中表现优异。


<details>
  <summary>Details</summary>
Motivation: 接触丰富运动规划因组合接触决策而计算困难，现有方法通常避免广泛评估接触模式序列，作者重新审视广泛评估在现代GPU硬件上的可行性。

Method: 提出CoMET方法，包括GPU并行轨迹评估和贪心接触模式扩展，进行广泛显式模式评估。

Result: 在平面推动基准上，CoMET在解质量和规划时间上与优化、采样、树搜索基线竞争，几乎在所有实例上匹配全枚举参考，且评估更少、时间更短；消融表明性能提升主要来自高吞吐轨迹评估器。在双臂非抓取操作中，随模式空间增长，GPU友好的局部模式扩展比自适应树搜索成功率更高。

Conclusion: 广泛显式模式评估为接触丰富运动规划提供了一种简单有效的替代方案。
...
### [275] [A Sim-to-Real Integration Pipeline for Training and Deployment of Chunk-Based VLA Manipulation Policies](https://arxiv.org/abs/2609.21817)
...
TL;DR: 提出开源sim-to-real实验协议：在仿真中生成专家轨迹，开环回放到真实Franka FR3采集视觉和本体感觉数据，并复用同一部署栈闭环评估VLA策略，同时度量sim-to-real差距，发布数据集与代码。


<details>
  <summary>Details</summary>
Motivation: VLA模型训练和评估需要大规模真实机器人演示数据，但真实硬件采集通常依赖遥操作，成本高、耗时长且难以扩展。

Method: 在仿真中生成专家轨迹，并在真实Franka FR3上开环回放；记录对应的真实视觉与本体感觉观测，转换为VLA训练格式；复用同一部署栈进行闭环策略评估；将每条真实记录与生成它的仿真轨迹配对。

Result: 数据采集与策略评估共享相同硬件配置；可直接测量sim-to-real gap；发布采集数据集至Hugging Face，并开源流水线代码。

Conclusion: 该协议缓解了真实数据采集瓶颈，为VLA模型的sim-to-real训练与评估提供了可复用、开源且便于规模化验证的实验方案。
...
### [276] [AcousticDiffusion: Semantically Conditioned Audio-Guided Diffusion Policy for Search-and-Rescue Assistance](https://arxiv.org/abs/2609.21792)
*Iana Zhura,Didar Seyidov,Dmitrii Plotnikov,Hajira Amjad,Miguel Altamirano Cabrera,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 提出 AcousticDiffusion：一种语义条件化、音频引导的扩散策略，用于救援机器人在视觉退化或遮挡环境中朝人类呼救者导航。


<details>
  <summary>Details</summary>
Motivation: 救援场景中视觉接触常受限，机器人需要依靠不确定的声学观测找到并接近人类呼救者。

Method: 使用冻结的预训练音频识别器处理10.24秒音频窗口，通过语音门控和遇险感知优先排序生成声源级导航角色；将麦克风阵列DOA递归整合为机器人中心贝叶斯鸟瞰信念场，并进行自运动补偿；以语义信念、近期声学观测、音频特征和机器人状态条件化扩散模型，生成路点轨迹。

Result: 合成导航验证中，平均端点方位误差为11.20度，91.78%轨迹在30度内；干扰抑制为89.20%至98.99%；在91.07%窗口中优先HELP呼救者。ZSL-1四足机器人在线部署无需重训练，平均方位误差64.9度，优于A*的98.2度和RRT的90.4度，平均规划计算6.07毫秒；最终声源距离从经典规划器的3.96米降至2.48米，改善37.4%。

Conclusion: 该框架能将不确定的声学观测转化为更接近人类呼救者的导航行为，即使声学定位不完美，仍优于传统规划方法。
...
### [277] [PopNavShift: Stress-Testing Social Navigation under Behavioral Population Shift](https://arxiv.org/abs/2609.21838)
*Kaizhen Tan,Diyu Zheng,Tim Guangyu Wu,ChengHe Guan*

Main category: cs.RO

TL;DR: 论文提出 PopNavShift 仿真框架，用于在行人群体行为分布变化下压力测试社交导航策略；结果显示控制器排名会因行人时间压力等条件而改变，且对行人负担的影响比对机器人通行时间更敏感。


<details>
  <summary>Details</summary>
Motivation: 现有社交导航算法通常在固定行人行为分布下评估，但行人个体和社会情境差异会导致对机器人响应显著不同，因此需要跨行为群体进行稳健性评估。

Method: PopNavShift 利用 600 条来自 MatrAIx Persona 1M 的合成人物记录提示 Gemini 3.7 Flash，生成按群体条件化的行人运动参数；比较反应式避让、提前让行和互惠避碰三种策略，覆盖 8 种群体条件与 7,488 次匹配机器人运行，并进行同一 202 人物下的匹配干预。

Result: 仅改变时间压力即可使基于机器人通行时间的控制器排名反转 8.6%，但基于平均行人延迟反转 22.4%，基于最差十分位延迟反转 23.9%；这种敏感性在行人负担上高于机器人通行时间，并在空间受限场景中增强，且在第二种行人动力学模型下仍保持。

Conclusion: 评估导航策略时应跨行为群体进行，并同时考虑机器人性能和行人负担，而不能只依赖固定行人分布或单一机器人指标。
...
### [278] [VIRGA: Virtual-Agent-Intermediated Riemannian Geometry for Active-Sensing Air-Ground Coordination](https://arxiv.org/abs/2609.21883)
*Fenghe Guo,Runjie Shen,Chenyang Sun,Junrui Zhang*

Main category: cs.RO

TL;DR: 本文提出 VIRGA，一种神经几何协调框架，将双 LiDAR 观测转化为有界、来源特定的黎曼场，并通过虚拟智能体与互惠弹性反馈耦合，实现 UAV 与 UGV 在动态障碍和主动观测约束下的空地自主协同。


<details>
  <summary>Details</summary>
Motivation: 空地自主在 UAV 必须被 UGV 云台 LiDAR 持续观测时更困难，需在单一闭环中同时处理动态避障、异构运动协调、有限感知和任务主导权变化。现有基线在安全性、观测保持或计算效率上存在不足。

Method: VIRGA 将双 LiDAR 观测映射为有界、来源特定的黎曼场，通过带互惠弹性反馈的虚拟智能体进行耦合；再经平台感知执行映射生成可行的 UAV、UGV 和云台指令，并施加主动观测安全保护。

Result: 与三种基线相比：Ray-RMP 响应最快但耦合空地任务中净空不足；稠密解析黎曼场改善几何避障但评估成本高，难以稳定维持视野；ColAG 延迟最低但仍有安全和观测违例。VIRGA 安全完成所有成对仓库条件，且在未重训练的长距离洞穴压力测试中，于不规则受限几何下保持持续协调。消融验证了在线几何评估、虚拟智能体中介和互惠反馈的贡献。

Conclusion: VIRGA 能有效协调空地异构平台，在动态障碍与主动观测要求下兼顾安全与观测维持，并具备对不规则受限环境的泛化协调能力。
...
### [279] [CommitFlow: Semantic Commitment Verification and Local Correction for Long-Horizon Robot Manipulation VLA Execution](https://arxiv.org/abs/2609.21908)
*Zixiang Zhao,Yansong Feng,Yang Yang,Chaoyu Wang,Haoran Xiao,Hui Zhang,Chuang Cheng,Jianjun Ma*

Main category: cs.RO

TL;DR: 本文提出 CommitFlow，一种闭环执行框架，用于解决 VLA 策略在长时程任务中语义阶段承诺与实际物理状态不匹配的问题。它通过承诺监控和局部修正提升任务成功率，且保持基础策略冻结。


<details>
  <summary>Details</summary>
Motivation: VLA 策略在长时程执行中，可能在所需物理效果尚未建立时就进入下一任务阶段，导致语义承诺、物理条件与实际状态不匹配。仅靠动作命令无法确认条件是否满足，局部偏差会传播并造成任务失败。

Method: CommitFlow 包含三个组件：语义承诺监控器（SCM）比较阶段要求与当前状态证据，在条件未满足或被违反时阻止依赖动作；BoundaryFlow 基于当前状态和基础动作生成局部修正；关系与增益校准（RGC）选择满足约束的最小修正强度。框架保持基础策略冻结。

Result: 在 RoboTwin 2.0 的十个常见任务上，CommitFlow 取得 75.9% 的平均成功率，相比基础策略 pi0.5 提升 22.7%。跨策略实验也显示出一致增益。

Conclusion: CommitFlow 通过闭环承诺监控与局部修正，可有效提升长时程机器人执行的可靠性，并具有跨策略泛化潜力。
...
### [280] [MAAP: Multi-Agent Active Perception for Collaborative Manipulation](https://arxiv.org/abs/2609.21929)
*Bruno N. Y. Chen,Li Kang,Heng Zhou,Xiufeng Song,Zhemeng Zhang,Jiahua Ma,Yiran Qin*

Main category: cs.RO

TL;DR: 提出MAAP多智能体主动感知框架，让每个机械臂在操作的同时利用腕部相机作为移动视角，并配合RAIL角色感知模仿学习控制器，显著提升协作操作成功率。


<details>
  <summary>Details</summary>
Motivation: 多智能体操作中腕部相机视角未被充分利用，而主动感知常被视为需要专用感知智能体，因此研究如何将操作臂本身转化为主动感知资源。

Method: MAAP（多智能体主动感知）让每个臂兼具操作与移动视角功能；RAIL（角色感知模仿学习）预测每个臂的当前角色并以其为条件生成动作块，在单一网络中表示角色相关动作。

Result: 四个仿真任务中，固定相机平均成功率56.5%，一个主动腕部视角62.5%，全部主动视角70.0%，MAAP+RAIL达79.2%；RAIL在三臂微波炉任务中从47%提升至82%；双臂平台上MAAP+RAIL在20次放置试验中成功14次，而固定视角ACT为0次。

Conclusion: 协作操作本身可以作为一种主动感知机制，无需专用感知智能体即可提升操作性能。
...
### [281] [LunaDrive: A Delay-Compensated High-Voltage GaN FET-Based Motor Driver for Dynamic Robots with Flat BLDC Motors](https://arxiv.org/abs/2609.21818)
*Sota Yuzaki,Temma Suzuki,Hiromi Tada,Masanori Konishi,Kento Kawaharazuka,Kei Okada*

Main category: cs.RO

TL;DR: 本研究开发了基于GaN FET的高功率扁平BLDC电机驱动器，并研究了超出额定工作范围的高速区域中的延迟补偿效果。实验实现了96V下30A连续电流、80A峰值电流和3110Hz最大电频率，并展示了动态机器人应用的可行性。


<details>
  <summary>Details</summary>
Motivation: 高性能扁平BLDC电机推动了动态机器人发展，但商用伺服电机通常工作电压≤48V，限制了最大转速。动态机器人需要快速能量生成，电压约束限制了运动性能。因此需要超出额定电压驱动电机以提高瞬时最大速度。然而，半导体器件在电压额定值和电流容量之间存在权衡，传统Si MOSFET驱动器难以同时实现高压和大电流。GaN FET有前景，但可安装在扁平BLDC电机后侧的紧凑驱动器有限。

Method: 开发了使用GaN FET的高功率扁平BLDC电机驱动器，并研究了在超出额定工作范围的高速区域中的延迟补偿效果。进行了实验，包括96V操作下的连续电流测试、峰值电流和最大电频率测试，以及使用LiPo电池24S (100V) 驱动的高速负载提升实验。

Result: 在96V操作下，附加热沉时实现了30A连续电流。确认了80A峰值电流和3110Hz最大电频率。高速负载提升实验展示了动态机器人操作的适用性。

Conclusion: 开发的GaN FET驱动器能够实现高电压和高电流操作，支持超出额定范围的高速运行，并适用于动态机器人应用。延迟补偿在高速区域有效。
...
### [282] [LIMBO: Learning and Internalizing Model-Free Barrier Objectives for Agile and Safe Whole-Body Control](https://arxiv.org/abs/2609.22075)
*Jake Gonzales,Arturo Flores Alvarez,Yu-Ming Chen,Aaron D. Ames,Lillian J. Ratliff,Manikantan Nambi*

Main category: cs.RO

TL;DR: 提出LIMBO框架，通过从黑盒转移与失败规范中学习状态-动作控制屏障函数（Q-CBF），并将其安全结构蒸馏至任务策略中，在29自由度人形机器人上实现闪避球与低障碍下行走，并成功迁移至真实硬件且无需在线安全过滤。


<details>
  <summary>Details</summary>
Motivation: 全身控制需在高维非线性动力学下同时保证避碰与平衡，安全证书难以设计且难以在不同行为间复用。

Method: 在冻结的基础控制器周围，从黑盒转移与基于状态的失败规范中学习Q-CBF；合成阶段用学习到的安全值驱动风险引导采样，探索可恢复性边界；任务学习阶段将其作为教师提供动作级安全反馈，训练鲁棒任务策略。

Result: 在29自由度人形机器人上完成闪避球和低障碍行走；风险引导边界采样提供了探索可恢复性边界的理论化方法；相同安全规范下改变采样集中度可产生下蹲到新颖后仰limbo动作等不同策略；学习策略无需在线安全过滤即可迁移至硬件。

Conclusion: 学习型安全合成可扩展至敏捷全身控制，实现无需在线安全过滤的硬件迁移。
...
### [283] [CARF: Contrastive Attraction-Repulsion of Failure-Guided Flow Matching](https://arxiv.org/abs/2609.21982)
*Shuqi Zhao,Bang Du,Cheng-En Wu,Yichen Xie,Yixiao Wang,Masayoshi Tomizuka*

Main category: cs.RO

TL;DR: 论文提出CARF，一种基于失败引导的对比吸引-排斥框架，用于从不完美机器人演示数据中学习，通过区分并分别模仿推进性片段、排斥失败关键片段来提升策略学习效果。


<details>
  <summary>Details</summary>
Motivation: 机器人演示收集常包含失败或不完美轨迹。现有方法多只利用失败轨迹中仍朝任务完成推进的片段，却忽略直接导致失败的关键行为。作者认为这两类片段提供本质不对称的监督：推进性片段应被模仿，失败关键片段应被显式避免。

Method: CARF引入基于进度的重要性评分器，仅用成功专家演示及其扰动结果训练，用于估计每一步对任务完成的贡献并识别失败轨迹中的信息区域。这些分数指导统一的流匹配目标：吸引策略学习推进性行为，排斥失败关键行为，同时排除模糊片段，从而更全面利用不完美数据并避免不可靠监督。

Result: 在仿真和真实世界中的大量实验表明，CARF在多种失败场景下持续优于竞争基线；消融实验进一步验证了所提出的进度评分机制和吸引-排斥机制的有效性。

Conclusion: 失败轨迹中的推进性片段与失败关键片段应接受不对称监督。通过显式吸引推进行为并排斥失败关键行为，CARF能更有效地利用不完美机器人数据，同时避免模糊失败片段带来的不可靠监督。
...
### [284] [SkelWAM: A Skeleton-Guided World-Action Model for Zero-Shot Cross-Embodiment Manipulation](https://arxiv.org/abs/2609.21983)
*Pengjun Niu,Yujia Xie,Rui Peng,Hang Zhao,Ke Liu*

Main category: cs.RO

TL;DR: 提出SkelWAM，一种基于骨架引导的世界-动作模型，通过统一的几何表示连接感知与控制，实现单源跨具身操作。


<details>
  <summary>Details</summary>
Motivation: 跨机器人具身复用操作经验对扩展机器人学习和减少重复任务数据采集很重要，但具身变化会改变视觉外观、动作维度与语义，以及实现同一工具位姿所需的全身构型。

Method: SkelWAM将手臂中心线几何、工具中心点(TCP)位姿和平行夹爪命令组成共享25维状态，统一第三人称与腕部观测及未来全身动作目标。模型采用视频-动作混合Transformer，以预测性视觉监督训练，预测规范骨架动作块，再由具身特定约束解码器转换为关节或连续体机器人控制。无需一对一关节对应，也不使用目标任务演示或目标策略更新。

Result: 提出仅源域跨具身迁移基准LIBERO-Cross10，涵盖10个任务、10个目标具身和4类形态。Franka训练的SkelWAM在1000个回合中达到43.3%成功率，比最佳基线高36.2个百分点。还将JAKA mini2训练的策略部署到Feagine A03连续体机器人上完成3个桌面操作任务。

Conclusion: SkelWAM展示了单源跨具身操作在基准和真实机器人上的潜力，为无需目标域演示或策略更新的跨具身迁移提供了可行方案。
...
### [285] [Duty Factor Predicts Robust Constrained Quadrupedal Locomotion Across Gait Types](https://arxiv.org/abs/2609.22073)
...
TL;DR: 本文研究四足机器人步态参数与运动鲁棒性的关系，发现占空比（duty factor）比名义步态类型更能预测局部误差收敛，并可作为适应窄地形环境的低维参数。


<details>
  <summary>Details</summary>
Motivation: 四足机器人需在扰动和受限地形中保持鲁棒运动。步态类型（如行走、小跑）常被用来描述运动，但同一步态类型下占空比、速度、站立宽度等参数仍可变化，因此仅靠步态类型不足以唯一刻画运动特性。

Method: 采用三种四足运动控制方法：1）全身轨迹优化结合LQR反馈；2）学习型运动控制器；3）质心模型预测控制（centroidal MPC）。并在物理四足机器人上进行窄地形实验验证。

Result: 占空比比名义步态类型是更强的局部误差收敛预测因子；在学习型控制器中，占空比可作为适应窄地形鲁棒性的低维参数；这些趋势在质心MPC框架下依然存在，并通过物理四足窄地形实验得到验证。
...
### [286] [SeeQ: Training Generalist Value Functions for Long-Horizon Robotic Manipulation](https://arxiv.org/abs/2609.22085)
*Saksham Singh,Zheyuan Hu,Max Sobol Mark,Jeffrey Yu,Zackory Erickson,Aviral Kumar*

Main category: cs.RO

TL;DR: 论文提出 SeeQ，一种为当前活跃子任务学习 Q 值的方法，以缓解通用机器人策略在复杂长时程任务中的脆弱性。它利用离线数据中的子任务标注训练，并在测试时自回归地用自然语言预测活跃子任务再估计其价值。在真实双臂机器人任务上显著提升了 best-of-N 策略引导效果。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略在包含多阶段或需要重复尝试与 deliberation 的复杂长时程任务上仍然脆弱。Q 值函数可通过排序候选动作或指导策略改进来提升策略，但从稀疏任务级奖励学习会带来较长的信用分配时域、困难的 Bellman 备份以及较大的数据覆盖需求。

Method: SeeQ 不学习任务级 Q 值，而是学习当前活跃子任务的 Q 值，从而缩短价值预测时域，并可用时序差分（TD）目标有效学习。训练时利用离线机器人数据中的子任务级标注进行分解，并可从广泛且可能次优的数据中学习。测试时无需人工标注或模块化子任务预测系统，Q 函数架构会先自回归地以自然语言预测活跃子任务，再估计其价值。作者基于视觉语言骨干实例化 SeeQ，在多样开源机器人操作数据上预训练，并在下游任务上微调。

Result: 在两个双臂机器人平台的四个真实世界操作任务中，SeeQ 价值函数显著改善了 best-of-N 策略引导效果。

Conclusion: SeeQ 通过子任务级 Q 学习与自然语言子任务预测，可在复杂长时程任务中有效利用离线机器人数据，并在无需测试时人工子任务标注的情况下提升策略选择与引导效果。
...
### [287] [Gripper-Aware Automatic Dense Packing of Irregular Objects](https://arxiv.org/abs/2609.22062)
*Tianhao Qin,Connor McCann,Berk Calli,Jing Xiao*

Main category: cs.RO

TL;DR: 提出一个集成感知、夹爪感知放置优化与力引导执行的闭环机器人密集装箱流水线，并在真实机械臂上验证，通过消融和基线对比评估各模块贡献。


<details>
  <summary>Details</summary>
Motivation: 现有不规则物体装箱研究多局限于理想接触仿真，将物体视为孤立刚体，夹爪通常仅作为事后可行性检查，且执行过程中的感知与接触漂移未被处理，难以满足真实仓库操作需求。

Method: 将物体与夹爪表示为层次球树复合体，在GPU上用CMA-ES搜索5自由度放置，垂直坐标根据当前高度图解析确定；执行时采用力监控垂直下降并在首次接触停止；释放后推挤以消除侧向间隙；每次放置后重新感知容器以避免漂移累积；在Franka Emika Panda上打包3D打印的平面、曲面、凹面物体及YCB子集，并消融夹爪感知优化、巩固推挤和网格几何的贡献，与高度图最小化基线比较。

Result: 在真实Franka Emika Panda机械臂上完成对多种3D打印物体和YCB子集的密集装箱验证；消融实验表明夹爪感知优化、释放后巩固推挤和基于网格的几何对端到端成功率、装箱密度和计算成本有不同影响；并与heightmap-minimization基线进行了基准比较。

Conclusion: 闭环整合感知、规划与执行，并显式考虑夹爪、接触与漂移，可提升真实机器人不规则物体密集装箱的可行性与鲁棒性；夹爪感知优化和巩固推挤是有效组成，但需权衡计算成本。
...
### [288] [Incremental Stability and Convergence Properties of Discrete-Time Projected Control Systems](https://arxiv.org/abs/2609.20955)
*Riccardo Bertollo,S. J. A. M. van den Eijnden,W. P. M. H.,Heemels*

Main category: eess.SY

TL;DR: 本文研究离散时间投影型控制系统的增量稳定性与收敛性分析，提出了基于二次增量稳定保持和直接Lyapunov方法的理论工具，并给出小增益条件与性能量化示例。


<details>
  <summary>Details</summary>
Motivation: 现有投影型控制器（如混合积分器-增益系统）的分析与设计主要针对连续时间且多为非增量分析，难以准确反映实际数字实现并评估鲁棒性能，因此需要离散时间下的增量分析。

Method: 首先证明在投影度量设计良好时，投影控制器可保持其标称未投影动力学的二次增量稳定性；据此推导出投影控制器与一般非线性对象互联时的增量输入到状态稳定性小增益条件。其次，提出一种直接Lyapunov方法，用于验证输入仿射分段光滑系统的增量稳定性，可视为经典离散时间Demidovic条件的扩展。

Result: 建立了离散时间投影型控制系统的增量稳定性与收敛性分析框架，给出了小增益条件，并通过多个示例及非线性Bode图进行性能量化，特别关注一阶投影环节。

Conclusion: 该工作为离散时间投影型控制系统提供了更精细的增量稳定性分析工具，有助于更准确地评估数字实现下的鲁棒性能，并扩展了相关Lyapunov分析方法。
...
### [289] [Optimal Day-Ahead Scheduling of Fast EV Charging Station With Multi-Stage Battery Degradation Model](https://arxiv.org/abs/2609.20946)
*Y. Wan,D. Gebbran,R. K. Subroto,T. Dragičević*

Main category: eess.SY

TL;DR: 本文针对配备电池储能系统（BESS）的电动汽车快速充电站，提出了一种日前调度框架，并引入新颖的多阶段电池退化建模方法。


<details>
  <summary>Details</summary>
Motivation: 以往研究通常采用单一电池退化模型来描述老化过程，难以准确刻画电池全生命周期内的退化变化，因此需要更精细的建模方法。

Method: 提出多阶段电池退化建模方法，并将其以计算上可处理的方式显式集成到自适应优化框架中；通过案例研究，在选定的循环老化模型上验证该方法，并进一步结合其他常规退化模型进行对比。

Result: 多阶段模型使快速充电站整体运行成本平均比单阶段模型低约2.1%；随着阶段数增加，模型误差下降并趋于稳定，而相比单阶段模型降低的运行成本先增加后饱和；在其他常规退化模型下也表现出优越性。

Conclusion: 所提出的多阶段电池退化建模方法能更准确反映电池全生命周期退化过程，并有效降低配备BESS的快速充电站运行成本，具有重要实际应用价值。
...
### [290] [A Mean-Field Approach for Safe Routing of Multi-Destination Urban Air Mobility Networks](https://arxiv.org/abs/2609.21093)
*Nameer Fawwaz Ahmed,Cody Fleming,Yasser Shoukry*

Main category: eess.SY

TL;DR: 本文为多目的地UAM网络中的无人机路由提出基于最优控制的理论框架，在垂直起降场容量与流量约束下，结合CTMC建模、平均场流体近似和队列无状态正不变性证明，将复杂连续时间最优控制转化为有限维代数优化。


<details>
  <summary>Details</summary>
Motivation: 随着城市空中交通向高密度运行扩展，自主无人机交通管理需要既易于处理又满足安全关键要求的控制框架。

Method: 将网络建模为目的地条件连续时间马尔可夫链以刻画排队、服务和飞行状态间的随机转移；采用平均场流体近似，导出耦合常微分方程；证明队列无状态空间的正不变性；在欠载条件下将无限维连续时间最优控制问题转化为有限维代数优化，并联合优化旅行时间与多跳效率。

Result: 证明了在特定欠载条件下，若系统初始无队列，则将持续保持无队列；刻画了稳态流量均衡，并给出大规模移动系统中安全、无拥堵运行的充分条件。

Conclusion: 该框架为受垂直起降场容量和流量约束的多目的地UAM路由提供了具有原则性的最优控制基础，可确保网络级稳定性，并支持安全、无拥堵的大规模运行。
...
### [291] [Source--Grid Coupling in Power System Oscillations: Observational Equivalence, Feedback Detectability, and Modal Interaction](https://arxiv.org/abs/2609.20943)
*Kai Sun,Bin Wang*

Main category: eess.SY

TL;DR: 本文从源-网耦合方向性出发，重新审视强迫振荡与自然振荡的区分，研究何时外部强迫与单向自治源在观测上等价，以及电网侧量测何时能辨识源到网的反馈。


<details>
  <summary>Details</summary>
Motivation: 传统强迫/自然振荡划分默认振荡源位于固定电力系统边界之外，忽略了源与网之间耦合的方向性和反馈作用，因此需要建立耦合感知的动态解释框架。

Method: 将源-网耦合方向性建模为反馈式动态边界；比较规定强迫与单向自治源在相同确定性电网初值问题和量测映射下的观测等价性；在增广自治模型中分析双向反馈引起的源-网特征值交互，并用降阶双模态模型研究回路耦合与复模态分离的共同影响；通过两区域系统和WECC 243节点系统仿真验证。

Result: 匹配界面信号时，规定强迫与单向自治源可精确等价；双向反馈会产生源-网特征值交互，其作用取决于回路耦合和复模态分离。两区域仿真可在标定源族内检测反馈，但源模型失配会与反馈混淆；WECC 243节点系统表明大规模系统可行，且反馈检测不要求精确候选源选择。

Conclusion: 源-网耦合方向性可被确立为基于反馈的动态边界，该框架明确了电网侧量测推断反馈的条件，并解释了弱反馈为何可能难以辨识。
...
### [292] [Distribution-Free Budgeted Stealthy Attack Scheduling for Remote State Estimation](https://arxiv.org/abs/2609.21148)
*Qazi Mairaj ud din,Sidra Ghayour Bhatti,Qadeer Ahmed*

Main category: eess.SY

TL;DR: 本文研究资源受限对手在远程状态估计中最多污染比例 Γ̄ 传输的预算隐蔽 FDI 调度，提出不依赖分布和系统模型的分裂共形校准调度器，并给出隐蔽性、触发率预算与稳态性能退化保证。


<details>
  <summary>Details</summary>
Motivation: 现有事件触发调度器依赖高斯创新尾部反演阈值，并通过协方差匹配证明隐蔽，但这些结论仅在 Gaussian 假设下精确，而真实 CPS 残差常违反高斯性。

Method: 将最坏情况 FDI 动作与 split-conformal 校准触发阈值配对，无需已知系统矩阵或分布模型；并从理论上分析路径级隐蔽性、平均触发率、预算达到及稳态退化。

Result: 证明对任意幅度可测检测器具有精确路径隐蔽性；给出有限样本无分布平均触发率上界，并在平稳遍历下几乎必然达到预算；稳态退化线性依赖于能量捕获 ψ，且由同一次序统计量最大化；条件符号对称下可扩展至符号敏感检测器，残余暴露由四阶累积量控制，Gaussian 时消失。Monte-Carlo 与重型卡车 CAN 数据验证了边界。

Conclusion: 仿真和实测确认了理论边界，并量化了高斯假设在非高斯情形下带来的代价。
...
### [293] [Stochastic MPC under Heavy-Tailed Disturbances: An Extreme Value Theory Approach](https://arxiv.org/abs/2609.21170)
*Xiuzhen Ye,Wentao Tang*

Main category: eess.SY

TL;DR: 本文针对未知重尾扰动下的线性系统，提出一种基于极值理论（EVT）的随机模型预测控制（SMPC）方法，并利用极值指数刻画稀有越界事件的时间聚集性，从而更准确地约束预测时域内的安全违规概率。


<details>
  <summary>Details</summary>
Motivation: 现有SMPC依赖假定分布、矩界或有限场景采样，在未知重尾扰动下性能退化；极端偏差比经典轻尾模型预测的更频繁，因此需要更精确的重尾扰动建模与安全约束方法。

Method: 假设扰动为正规变化重尾分布，用EVT对管道误差尾部进行渐近精确刻画；进一步分析闭环动力学导致的稀有越界时间聚集，并用可从数据估计的闭式极值指数θ进行校正，形成约束预测时域内稀有事件发生概率的θ校正边界。

Result: 在Student-t扰动下，非线性独轮车避障仿真验证了所提方法，表明安全约束违规频率降低。

Conclusion: 基于EVT和极值指数的SMPC能更有效处理重尾扰动，并比传统边际逐步超越概率约束更好地保障预测时域内的整体安全。
...
### [294] [Minimizing Bid Cost Recovery for Energy Storage with Uniform Pricing](https://arxiv.org/abs/2609.21230)
*Yaxuan Yu,Jingguan Liu,Cong Chen*

Main category: eess.SY

TL;DR: 本文研究含储能资源（ESR）的实时电力系统滚动窗口调度中，市场内统一定价与市场外投标成本回收（BCR）支付问题，并提出UP-BCR统一定价方案以降低BCR和需求支付。


<details>
  <summary>Details</summary>
Motivation: ESR受荷电状态（SOC）跨时段约束影响，其运行具有时间耦合性，现有LMP可能无法补偿ESR的跨时段机会成本，从而引致市场外BCR支付。

Method: 分析市场内统一定价和市场外BCR支付；证明当被调度发电机或ESR的供给侧报价高于需求侧报价时正BCR不可避免；识别与SOC约束绑定相关的跨时段耦合指标；在预测不确定性下将BCR最小化统一定价方案（UP-BCR）与现有实时定价方法进行仿真比较。

Result: 正BCR仅在该跨时段耦合指标激活时出现，表明BCR根本上由跨时段耦合驱动；在铜板模型仿真中，UP-BCR相对于LMP显著降低了BCR和需求支付，同时保持零 merchandising surplus。

Conclusion: BCR问题源于ESR跨时段SOC约束导致的时间耦合，UP-BCR能在维持市场结算平衡的同时有效减少BCR和需求支付。
...
### [295] [Optimal Regret for Online Storage Control via Cumulative Policies](https://arxiv.org/abs/2609.21262)
*Kamiar Asgari,Michael J. Neely*

Main category: eess.SY

TL;DR: 研究标量存储系统在对抗性非负到达和凸成本下的在线控制，提出一种基于累计分配分数重参数化与衰减加权投影次梯度的策略，实现与策略记忆长度无关的 O(sqrt(T)) 遗憾，并给出匹配下界。


<details>
  <summary>Details</summary>
Motivation: 存储系统在线决策需在到达与成本函数揭示前行动，且受资源可用性约束；已有 simplex disturbance-action 策略类的遗憾可能依赖记忆长度，需明确其在保留系数、时域和记忆限制下的最优性能。

Method: 对现有 simplex disturbance-action 策略类进行精确重参数化，使用累计分配分数表示策略；采用衰减加权投影次梯度更新；构造存储特定的分块下界，针对所有因果可行控制器（含随机控制器）证明匹配下界。

Result: 控制器对固定保留系数和成本常数取得 O(sqrt(T)) 遗憾，每轮仅需 O(log T) 记忆和算术操作、一次成本次梯度查询；下界表明，当 α∈[1/2,1)、T≥4 且正成本常数固定时，对每个有限记忆 simplex 策略类及其无限记忆扩展，极小极大期望遗憾为 Theta(sqrt(T) min{T,τ}^{3/2})，其中 τ=(1-α)^{-1}。

Conclusion: 该工作给出了记忆高效且阶数最优的在线存储控制算法，并刻画了遗憾对时域 T 与保留时间 τ 的联合依赖，为相关策略基准确立了极小极大最优性。
...
### [296] [Rate-cost Trade-offs in H-infinity Control with Initial State Uncertainty](https://arxiv.org/abs/2609.21270)
*Vikrant Malik,Victoria Kostina,Babak Hassibi*

Main category: eess.SY

TL;DR: 研究数字通信信道下观测器与控制器之间的H∞控制，揭示所需通信数据率与H∞性能之间的基本权衡。


<details>
  <summary>Details</summary>
Motivation: 通信带宽受限，需要明确给定H∞性能所需的最小数据率；系统为线性动态且无内部加性噪声，不确定性仅来自初始状态。

Method: 针对标量系统，采用确定性零时刻覆盖论证推导数据率下界，并分析对数量化的自然性；进一步设计可达编码与控制方案。

Result: 建立了给定H∞代价下所需数据率的下界；说明对数量化是自然选择；提出可达方案，并证明标量系统在高数据率极限下其速率渐近匹配下界。

Conclusion: 数字信道下的H∞控制存在基本速率-性能权衡；对数量化具有合理性，且所提方案在高码率下渐近最优。
...
### [297] [A Distributed Step-by-step Finite-time Consensus Design for Heterogeneous Battery Energy Storage Devices with Droop Control](https://arxiv.org/abs/2609.21313)
*Yalin Zhang,Yunzhong Song,Shumin Fei*

Main category: eess.SY

TL;DR: 本文提出一种面向异构电池储能系统（BESS）的分布式逐步有限时间一致性控制方案，可在有限时间内实现多BESS协调，并通过改进IEEE 57节点系统仿真验证。


<details>
  <summary>Details</summary>
Motivation: 大规模电力系统中发电机分布在不同区域，若不考虑距离信息，多发电机系统难以实现良好协同控制；同时相比集中式方法，需要更分布式、快速且鲁棒的控制策略。

Method: 先按位置将BESS分簇，每簇设一个主动领导者负责接收外部信息；采用五个功能导向输入实现能量水平平衡、有功/无功功率共享以及电压/频率同步；引入虚拟外部领导者实现频率和电压恢复到主网额定值；控制完全分布式，各BESS仅利用自身及邻居信息。

Result: 该方案能在有限时间内实现协调一致性，实现多BESS的能量平衡、功率共享和电压/频率同步，并可恢复频率与电压至额定值；对负载扰动和通信拓扑即插即用具有鲁棒性。

Conclusion: 所提分布式有限时间一致性控制方案有效，仿真实验在改进IEEE 57节点系统上验证了其可行性和优势。
...
### [298] [Iterative Single-Loop Coordination of Transmission and Distribution Systems and DER Aggregators with Limited Information Sharing](https://arxiv.org/abs/2609.21435)
*Robert Sosnowski,Yury Dvorkin,Marcin Baszynski,Charalambos Konstantinou*

Main category: eess.SY

TL;DR: 本文提出一种面向TSO、DSO与DER聚合商的三层协调方法，采用透明、可扩展、去中心化的单循环迭代机制，在有限信息共享下实现基于价格的迭代协调。


<details>
  <summary>Details</summary>
Motivation: 随着分布式能源（DERs）占比不断提高，跨TSO、DSO和聚合商层级的统一协调变得愈发重要，但集中式优化面临信息共享和可扩展性限制。

Method: 提出相邻层级间基于价格的迭代协调机制：上层提供价格信号，下层反馈期望功率计划；采用近端梯度迭代协同优化算法，并将其扩展到三层场景，利用近端算法的收敛特性。

Result: 与集中式方法和基于固定输电级电价的顺序参考方法相比，所提协调方法的结果非常接近集中式优化，同时显著优于顺序参考方法，且在DER灵活性较高时提升更明显。

Conclusion: 所提去中心化单循环迭代机制能够在有限信息共享下实现接近集中式的协调效果，并具有良好可扩展性和收敛性，适合TSO-DSO-聚合商多层协调。
...
### [299] [Model-Free Control for Residential Heating: Deployment and Simulation of Nonlinear Data-Enabled Predictive Control](https://arxiv.org/abs/2609.21500)
*Sebastian Zieglmeier,Chris Verhoek,Jaap Eising,Mathias Hudoba de Badyn*

Main category: eess.SY

TL;DR: 本文研究住宅供暖中的数据驱动预测控制（DPC），比较三种非线性DPC扩展与标准DeePC及行业常用滞环控制器的性能，并在瑞士NEST研究建筑的仿真与真实公寓中进行验证。


<details>
  <summary>Details</summary>
Motivation: 住宅供暖能耗占比高，预测控制可节能，但传统MPC需要为每个住宅单元建立精确模型，耗时且难以在异构建筑中迁移。DeePC虽免去建模，但其基于确定性线性时不变系统假设，不符合住宅供暖实际。

Method: 采用三种非线性DeePC扩展：Select-DPC、gain-scheduling DPC和线性参数变化DPC（统称DPC），与标准DeePC和滞环控制器对比。在瑞士NEST研究建筑中一个有人居住单元的校准数字孪生上，进行整个供暖季仿真，以供暖能耗和舒适带违反为指标；并将gain-scheduling DPC部署到真实住宅公寓验证仿真结论。

Result: 各DPC控制器相比滞环控制器在整个供暖季显著节能，约减少11%能耗。所有非线性方法在评估场景中均优于线性DeePC，并分析了各非线性方法的相对成本与收益。

Conclusion: 验证了DPC在住宅供暖中的有效性，非线性DPC相比线性DeePC更具优势，且gain-scheduling DPC在真实公寓中的部署支持了仿真结论。
...
### [300] [Response Time Stochastic Analysis for Fixed-Priority Stable Real-Time Systems](https://arxiv.org/abs/2609.21760)
*Kevin Zagalo,Yasmina Abdeddaïm,Avner Bar-Hen,Liliana Cucu-Grosjean*

Main category: eess.SY

TL;DR: 本文研究稳定概率实时系统，证明平均系统利用率小于1是系统可行性的必要条件，并给出瞬态与稳态响应时间的解析近似及进入稳态时刻的界。


<details>
  <summary>Details</summary>
Motivation: 实时系统可行性分析通常依赖利用率条件，但概率实时系统在瞬态与稳态下的响应时间行为仍需进一步刻画。

Method: 将实时系统定义为稳定系统，区分瞬态与稳态；证明稳定性与可行性条件；将Liu and Layland定理推广到带隐式截止期的稳定概率实时系统；给出两种状态下响应时间的解析近似以及系统进入稳态时刻的界。

Result: 证明平均系统利用率小于1是可行性的必要条件；稳定系统的响应时间分布最终会重复；Liu and Layland定理对稳定概率实时系统成立；获得瞬态和稳态响应时间的解析近似，并给出进入稳态时刻的上界。

Conclusion: 平均利用率条件与Liu-Layland结论可扩展至稳定概率实时系统，且可用解析方法近似响应时间并界定稳态到达时刻。
...
### [301] [Adaptive Uncertainty-Aware Modeling and Stochastic Radial Basis Function Predictive Control for Personalized Fluid Resuscitation](https://arxiv.org/abs/2609.21821)
*Elham Estiri,Hossein Mirinejad*

Main category: eess.SY

TL;DR: 本文提出一个集成贝叶斯生理建模与最优控制的框架，用于液体复苏中实现不确定性感知、个性化的血流动力学调控。


<details>
  <summary>Details</summary>
Motivation: 液体复苏中患者间和患者内生理差异、测量噪声及模型不确定性会影响平均动脉压（MAP）调控的安全性和个性化，因此需要能够显式建模不确定性并在线适应的建模与控制方法。

Method: 首先开发不确定性感知变分自编码器状态空间模型（UVAE-SSM），利用有限数据捕捉MAP与输液之间的动态关系并建模偶然不确定性；随后构建贝叶斯非线性状态空间模型（BNSSM），使用贝叶斯神经网络（BNN）建模认知不确定性并生成虚拟患者（VPG）；在此基础上设计随机径向基函数模型预测控制（sRBF-MPC）以跟踪MAP目标并满足生理约束；最后提出在线微调算法，用流式VPG数据自适应名义UVAE-SSM，实现闭环治疗中的渐进个性化。

Result: 在未见动物受试者和独立人体临床数据集上的仿真表明，UVAE-SSM和BNSSM具有较强预测准确性和跨群体泛化能力；闭环评估显示，sRBF-MPC可实现稳定的MAP调节，并且相比二次型MPC（Q-MPC）和随机二次型MPC（sQ-MPC）具有更好的风险感知控制性能。

Conclusion: 该框架通过在线模型适应处理患者间和患者内变异，为重症监护中不确定性感知、个性化的血流动力学建模与控制提供了有前景的解决方案。
...
### [302] [Distributed Multiconsensus Control of BESSs Based on Centrality of Eigenvectors](https://arxiv.org/abs/2609.21856)
*Yalin Zhang,Zhongxin Liu,Zengqiang Chen*

Main category: eess.SY

TL;DR: 本文提出一种基于特征向量中心性的通信权重分配方法，并为下垂控制的电池储能系统设计分布式二次电压控制器与SoC平衡控制器，实现电压多共识和SoC平衡。


<details>
  <summary>Details</summary>
Motivation: 二次控制和SoC平衡是电池储能系统的重要控制目标，同时希望减少通信需求，例如仅需单个领导者且无需传输容量信息或下垂系数。

Method: 针对连通有向图，基于特征向量中心性设计通信权重分配，使邻接矩阵具有给定特征向量；在此基础上为下垂控制BESS设计分布式二次电压控制器和SoC平衡控制器，并对SoC控制增益进行稳定性分析。

Result: 所提方案可实现电压领导者跟随多共识和SoC平衡；电压二次控制仅需单个领导者，且无需在通信网络中传输容量信息/下垂系数即可按容量实现功率分配和SoC平衡；仿真验证了方案有效性。

Conclusion: 该分布式控制方案能够有效完成BESS的二次电压恢复与SoC平衡，并降低通信网络对领导者数量和参数传输的要求。
...
### [303] [Improved Droop Control in DC Microgrids via Voltage-Locked Loop Synchronization](https://arxiv.org/abs/2609.21913)
*Ognjen Stanojev,Jovan Krajacic,Enea Bianda,Orcun Karaca,Mario Schweizer*

Main category: eess.SY

TL;DR: 本文提出一种基于电压锁定环同步的改进下垂控制方法，用于提升直流微电网的暂态响应并维持母线电压稳定。


<details>
  <summary>Details</summary>
Motivation: 传统下垂控制虽能实现分散电压控制和功率分配，但暂态性能差，在AI数据中心等负载快速变化场景下会产生电压跌落和超调。

Method: 将控制方案功能分离为并联的虚拟直流电机（VDCM）和虚拟电流源：VDCM 提供暂态响应，阻尼并稳定直流母线电压变化；虚拟电流源提供缓慢的下垂响应，保证稳态功率平衡。采用电压锁定环同步，并在实验微电网平台上验证。

Result: 所提方法能显著改善暂态响应，实现直流电压稳定，并在可用源变换器之间实现期望的负载分配。

Conclusion: 基于电压锁定环同步的改进下垂控制可有效解决传统下垂控制暂态性能差的问题，适用于快速变化负载的直流微电网。
...
### [304] [Interconnections of Dissipative Networks Through a Dynamic Scattering Controller](https://arxiv.org/abs/2609.21934)
*Ilia G. Polushin*

Main category: eess.SY

TL;DR: 研究耗散网络的模块化散射型设计，提出基于动态散射的互联控制器，使互联系统保持内部稳定并继承子系统的基本性质，从而可迭代构建大规模耗散网络。


<details>
  <summary>Details</summary>
Motivation: 需要以模块化方式构建大规模耗散网络，并确保子系统互联后仍保持内部稳定性和与单个子系统一致的基本性质。

Method: 在子系统满足基本假设的前提下，设计动态散射型互联控制器，用于子系统之间的互联。

Result: 所得互联系统是内部稳定的，并具有与单个子系统相同的基本性质；该方法可迭代应用，支持模块化构建大规模耗散网络，同时在控制器设计以及互联输入输出信号选择上具有较大灵活性。

Conclusion: 所提方法为耗散网络的模块化、可扩展设计提供了有效途径，并增强了互联控制器设计的灵活性。
...
### [305] [Multi-Agent Event-Triggered LQG Control under Shared Communication Constraints](https://arxiv.org/abs/2609.21974)
*Zahra Hashemi,Dipankar Maity*

Main category: eess.SY

TL;DR: 本文研究共享有限通信网络的多智能体系统的事件触发LQG控制，其中通信决策通过共享网络约束耦合。提出了基于滚动时域的集中式调度框架，并转化为混合整数线性规划（MILP），同时设计了可扩展的跳过剪枝条件和拍卖启发式调度器。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统中通信网络每步容量有限，虽然智能体动态解耦，但通信决策因共享网络约束而耦合，导致需要解决受约束的多智能体调度问题。

Method: 采用事件触发LQG控制，证明最优控制律保持确定性等价且可通过独立有限时域Riccati递归解耦；基于此提出集中式滚动时域调度框架，利用估计误差协方差的闭式刻画将问题重构为MILP；为提高可扩展性，推导基于窗口的跳过剪枝条件以安全固定连续传输决策为零，并设计基于单步传输收益分数的拍卖启发式调度器。

Result: 数值结果表明，所提出的模型预测控制（MPC）调度器在控制性能与通信成本之间取得了最佳权衡，而拍卖启发式调度器在计算复杂度显著降低的情况下达到了接近MPC的性能。

Conclusion: 所提出的调度框架和启发式方法能有效处理多智能体系统的通信受限调度问题，MPC调度器性能最优，拍卖调度器则以较低复杂度提供近优性能。
...
### [306] [Learning-Based Augmentation and Adaptation for Grid Sim-to-Real Model Discrepancy](https://arxiv.org/abs/2609.21986)
*Sayak Mukherjee,Kyung-Bin Kwon,Ramij R. Hossain,Marcelo Elizondo*

Main category: eess.SY

TL;DR: 该文提出一种学习增强的混合建模方法，将基于物理的电网仿真模型与AI学习的残差模型结合，利用PMU/点播波形数据补偿模型与实际动态之间的差异，并支持持续学习以适应未来电网变化。


<details>
  <summary>Details</summary>
Motivation: 现代电网因新能源逆变器资源、大负荷、未建模动态和参数漂移等不确定性，导致运行人员仿真模型与实际电网动态偏差增大，控制室可能无法捕捉关键振荡。

Method: 在物理仿真模型基础上，用AI学习残差模型，输入为PMU/点播波形感知轨迹；采用主干编码器和多头解码器处理异构电网通道；并构建持续学习驱动的自适应框架，以便在真实电网模型变化时更新残差AI模型。

Result: 在IEEE 68节点基准模型上进行了大量不同扰动仿真，探索了循环学习器、潜在神经常微分方程和Transformer等先进预测架构，验证了残差学习与自适应能力。

Conclusion: 物理模型提供可解释性和结构一致性，AI残差模型补偿非理想因素；所提学习增强混合方法及持续学习框架可提升电网动态预测与适应变化的能力。
...
### [307] [Internal-Model-Control-Based Virtual Admittance Emulation for Enhanced Grid-Forming Performance](https://arxiv.org/abs/2609.22037)
*Ruzica Cvetanovic,Lazar Stojanovic,Paolo Sbabo,Paolo Mattavelli,Massimo Bongiorno*

Main category: eess.SY

TL;DR: 本文针对并网变流器（GFM）的虚拟导纳（VA）内环控制提出一种基于内模控制的新方法，可补偿内电流环与电压前馈影响，使变流器端子导纳在外部环路影响之外的频段更接近目标导纳。


<details>
  <summary>Details</summary>
Motivation: 可再生能源大规模并网使GFM承担关键稳定与控制任务，而现有VA控制受内电流环和电压前馈影响，可能导致谐波频段小信号失稳。

Method: 提出基于内模控制的VA内环控制，显式考虑内电流控制环和电压前馈的影响；并与最先进VA控制对比，用实验室三相电压源变流器原型实验验证。

Result: 新VA控制使外部环路影响之外频段的端子导纳与目标导纳高度匹配，提升稳定性；传统VA控制在电压前馈低通滤波器截止频率较高时可能出现谐波范围小信号失稳，而新方法无论截止频率如何都能稳定运行。

Conclusion: 所提IMC-VA控制能克服传统VA控制的稳定性局限，实现更精确的导纳整形和稳定运行，并通过实验验证。
...
### [308] [Uncertainty-driven training for three-dimensional calibrated lung nodule classification](https://arxiv.org/abs/2609.20905)
*Giuseppe Tripodi,Alessandro De Rosis,Saleh Rezaeiravesh*

Main category: eess.IV

TL;DR: 本文提出一种不确定性驱动的3D CT肺结节分类训练框架，利用基于验证集的不确定性估计对损失进行重加权，以提升预测性能和概率校准。


<details>
  <summary>Details</summary>
Motivation: 3D CT肺结节分类模型需要可信的概率校准，而常规训练可能校准不足；将不确定性量化引入训练可促使模型关注难分类或不可靠类别，支持更可信的临床部署。

Method: 采用MCD和EDL两类不确定性量化方法，提供逐类不确定性以调制损失；在LIDC-IDRI和NoduleMNIST3D数据集上，使用ResNet、DenseNet、EfficientNet、ViT和Swin Transformer骨干进行评估，并比较后验温度缩放。

Result: 不确定性驱动训练在分类性能上与常规训练相近，但显著改善校准，LIDC-IDRI上ECE最多降低65%。EDL在浅层架构和单次推理下具竞争力，MCD在深层网络更稳健；卷积骨干比Transformer更稳定受益，EDL在ViT上退化；温度缩放对所有配置均有效。

Conclusion: 将不确定性量化整合进训练循环可显著改善概率校准，支持3D医学影像模型更可信部署；同时需注意架构与UQ方法的交互，简单标量校准仍具竞争力。
...
### [309] [Adaptive Color Grading](https://arxiv.org/abs/2609.21169)
*Trevor D. Canham,Abhijith Punnappurath,Michael S. Brown*

Main category: eess.IV

TL;DR: 本文开发了一个开源颜色分级工具，用于标注视频帧中的色调区域阈值，并比较传统经验与机器学习建模策略，发现K近邻在预测阈值上表现优异。


<details>
  <summary>Details</summary>
Motivation: 在绘画、摄影和电影制作中，独立控制阴影、高光等色调区域对增强2D图像表现力至关重要，而现有颜色分级模块依赖强度阈值分割光照区域进行局部调整。

Method: 作者开发开源颜色分级工具，用其标注大量视频帧的色调区域阈值；基于这些阈值，采用从业者传统经验和机器学习两种策略进行建模实验，并比较K近邻与端到端图像增强方法。

Result: 实验表明K近邻是一种有效的预测策略，优于当前最先进的端到端图像增强方法。

Conclusion: 在建模创造性风格化过程时，聚焦一小组核心参数是有益的。作者公开了自适应颜色分级界面和数据。
...
### [310] [WS-NeRF: A Mamba-Driven World-State-Aware Adaptive Deblurring Neural Radiance Field](https://arxiv.org/abs/2609.21391)
*Hang Jiang,Jinghao Wang,Yiming Zhang,Xinhong Wang,Luwei Ran,Yinfeng Yu*

Main category: eess.IV

TL;DR: 提出一种基于Mamba、世界状态感知的自适应去模糊神经辐射场WS-NeRF，用于提升模糊输入下的三维重建与新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF方法通常依赖高质量清晰输入，但真实场景图像采集易受模糊退化影响，严重降低重建质量，并带来三维不一致问题。

Method: 将辐射场的交替优化建模为具有时序记忆的动态演化过程；利用Mamba驱动机制、综合多维世界状态以及混合专家机制，动态调整去模糊先验的置信度。

Result: 在模糊辐射场重建任务上显著提升质量，在PSNR、SSIM和LPIPS指标上取得更好表现，并展现出更稳定的迭代恢复行为。

Conclusion: WS-NeRF能够有效应对图像退化与三维不一致问题，提高模糊输入条件下NeRF的重建性能与稳定性。
...
### [311] [Opportunistic Conditional Entropy Coding with Frozen Analysis and Synthesis Transforms](https://arxiv.org/abs/2609.21816)
*Vincent Corlay,Maxime Rousselot,Andriy Enttsel*

Main category: eess.IV

TL;DR: 本文提出一种机会性条件图像编码方法，当接收端已有低质量图像作为边信息时利用其降低后续传输码率，否则退化为标准超先验编码，且重建结果与条件模式完全一致。


<details>
  <summary>Details</summary>
Motivation: 传统编解码器在接收端偶然持有低质量图像时不会利用这一边信息，而条件编解码器通常假设边信息始终存在。实际场景中边信息可能时有时无，因此需要一种能自适应利用机会性边信息且不牺牲无侧信息时性能的方案。

Method: 提出单一熵模型：当先前解码的潜在表示可用时以其为条件，否则回退到标准超先验。设计适配器将边信息潜在表示映射为熵模型所需的先验信号，从而同一模型支持多种目标质量与边信息质量组合。分析与合成变换保持冻结，可对现有学习型编解码器进行改造，同时保留其潜在表示与重建路径。

Result: 当接收端持有的质量恰低于目标质量时，所提方法将后续传输码率最多降低46%；若额外传输超潜在表示，则最多降低52%。在无侧信息时，码率惩罚低于4%，且条件模式与回退模式的重建结果比特级完全相同。

Conclusion: 该机会性条件编码框架能有效利用可选的边信息，在低质量图像可用时显著节省码率，同时在边信息缺失时几乎不损失性能，并支持对现有学习型编解码器的即插即用式改造。
...
### [312] [Model-Based Iterative Reconstruction with View-Dependent Detector Displacements for Cone-Beam CT](https://arxiv.org/abs/2609.22032)
*Doga Topcicek,Sharmin Majumder,Whitney A. Poling,Dmitriy D. Bruder,Zhili Feng,Dali Wang,Xiao Wang*

Main category: eess.IV

TL;DR: 该文针对工业锥束CT中探测器移位导致的非均匀二维运动，提出扩展的可分离距离驱动MBIR模型，将逐视图水平与垂直探测器位移纳入重建几何建模，从而提升重建质量并减少伪影。


<details>
  <summary>Details</summary>
Motivation: 探测器移位常用于减少工业锥束CT的环状伪影，但探测器运动在投影视图间可能非均匀变化，无法用固定偏移表示。现有可分离距离驱动MBIR假设探测器几何固定，难以显式建模两个方向的非均匀运动，而插值或重排方法可能降低空间分辨率并引入伪影。

Method: 扩展可分离距离驱动MBIR模型，在距离驱动重叠计算中纳入记录的水平与垂直探测器位移，以改变体素到探测器的映射，同时保持实测投影不变，并通过前向投影验证模型。

Result: 前向投影在1080个视图上的NRMSE为0.0338。所提模型达到3D SSIM 0.9404、NRMSE 0.0406，而固定探测器MBIR仅为0.5396和0.1740。第二个工业数据集显示，相比FDK，结构化伪影减少。

Conclusion: 该方法能够显式表示非均匀二维探测器运动，改善工业锥束CT重建质量，并在实际数据中减少结构化伪影。
...
### [313] [MultiHU-TD: Multifeature Hyperspectral Unmixing Based on Tensor Decomposition](https://arxiv.org/abs/2310.03860)
*Mohamad Jouni,Mauro Dalla Mura,Lucas Drumetz,Pierre Comon*

Main category: eess.IV

TL;DR: 本文提出 MultiHU-TD，一种基于张量分解的可解释多特征高光谱解混框架，用于处理高阶张量形式的高光谱图像，并在真实数据上验证其可解释性。


<details>
  <summary>Details</summary>
Motivation: 仅依赖光谱特征通常不足，且当高光谱图像被表示为包含额外特征的高阶张量时，矩阵模型不再适用；现有 CP 等张量分解方法虽可用于解混，但缺乏通用框架和结果可解释性。

Method: 提出低秩多特征高光谱解混的可解释方法框架 MultiHU-TD，基于张量分解，在 ADMM 交替优化中引入丰度和为一约束，并给出数学、物理和图形解释及其与扩展线性混合模型的联系；额外特征采用数学形态学，并将邻域块方法纳入框架。

Result: 在真实高光谱图像上的实验展示了模型的可解释性和结果分析能力，同时提供 Python 和 MATLAB 实现并发布于 GitHub。

Conclusion: MultiHU-TD 为多特征高光谱解混提供了可解释的张量分解框架，可与扩展线性混合模型建立联系，并支持形态学和邻域特征等额外信息。
