...
TL;DR: 本研究比较了数据导向设计（DOD）与面向对象设计（OOD）在多线程环境下的性能，发现DOD在执行时间、系统调用和缓存未命中方面表现出显著优势，尤其在数据密集型操作中。然而，对于细粒度任务，单线程版本通常优于多线程版本。


<details>
  <summary>Details</summary>
Motivation: 多核CPU与主内存之间日益扩大的性能差距，要求软件设计必须考虑硬件特性，以提高效率。

Method: 开发并比较了A*搜索算法的四种版本：单线程OOD (ST-OOD)、单线程DOD (ST-DOD)、多线程OOD (MT-OOD) 和多线程DOD (MT-DOD)。评估指标包括执行时间、内存使用量和CPU缓存未命中。

Result: 在多线程测试中，DOD实现表现出显著的性能提升，执行时间更快，系统调用和缓存未命中次数更少。虽然OOD偶尔在内存使用或百分比缓存未命中率上略有优势，但DOD在数据密集型操作中的效率更为明显。此外，对于A*算法这类细粒度任务，线程管理开销导致单线程版本在两种范式下都显著优于多线程版本。

Conclusion: DOD在关键指标上的持续优势凸显了其基础架构的优越性，表明它是在复杂、大规模AI和并行计算任务中最大化硬件效率的更有效方法，即使在简单算法中性能差异可能不明显。
...
TL;DR: 本文提出将企业数据管理从AI辅助的独立任务转向由智能代理驱动的全生命周期自主管理，以实现完全自主的数据资产。


<details>
  <summary>Details</summary>
Motivation: 企业数据管理任务艰巨且复杂，当前AI助手无法实现完全自动化。然而，随着AI能力日益增强，处理复杂任务的能力提升，我们认为实现完全自主的数据资产管理的时机已成熟。

Method: 本文倡导一种范式转变，即从AI应用于独立数据组件操作，转向对整个数据生命周期进行更全面、自主的处理。为此，我们探讨了如何通过智能代理自主管理现代数据栈的每个阶段，以构建可供人类和AI本身使用的自给自足系统。

Result: 本文描述了推动这一范式转变的各种驱动力和机遇，审视了智能代理如何简化数据生命周期，并强调了需要进一步研究的开放问题和领域。

Conclusion: 我们希望这项工作能激发热烈讨论，刺激进一步研究，促进协作方法，并推动数据系统迈向更加自主的未来。
...
TL;DR: 针对大型推理模型（LRMs）在思维链（CoT）推理中KV缓存开销大和生成冗余的问题，SkipKV提出了一种免训练的KV压缩方法，通过句子级选择性驱逐和生成来减少KV缓存大小和生成长度，显著提高了准确性和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）的思维链（CoT）推理过程导致KV缓存开销巨大，造成内存和吞吐量瓶颈。现有的KV缓存驱逐方法在多批次设置下效果不佳，原因在于不稳定的token级评分、填充token导致的有效KV预算减少，以及语义无关的驱逐导致重复验证和更长的生成序列。

Method: SkipKV是一种免训练的KV压缩方法，用于CoT推理中的选择性驱逐和生成。它引入了一个“句子评分指标”来识别并移除高度相似的句子，同时保持语义连贯性。此外，为了抑制冗余生成，SkipKV在推理过程中动态调整一个转向向量，更新隐藏激活状态，强制LRM生成简洁的响应。

Result: 在多个推理基准上，SkipKV在相似的压缩预算下，相比现有替代方案，准确率提高了高达26.7%。与最先进的方法相比，SkipKV的生成长度减少了高达1.6倍，吞吐量提高了高达1.7倍。

Conclusion: SkipKV通过创新的句子级选择性驱逐和生成机制，有效地解决了大型推理模型在CoT推理中KV缓存开销大和生成冗余的问题。它在不进行训练的情况下，显著提升了模型的准确性、减少了生成长度并提高了吞吐量，为高效部署大型推理模型提供了解决方案。
...
TL;DR: 该论文提出了一个安全的、可扩展的AI增强患者-临床试验匹配系统，利用大型语言模型生成可解释的资格评估和动态建议，以解决手动筛选的低效问题。


<details>
  <summary>Details</summary>
Motivation: 临床试验患者筛选过程是手动的、耗时的且资源密集型的。

Method: 开发了一个概念验证系统，利用开源、支持推理的大型语言模型（LLMs）进行AI增强的患者-试验匹配。该系统解决了异构电子健康记录（EHR）数据集成、专家审查便利化和严格安全标准等关键实施挑战。它生成结构化的资格评估，提供可解释的推理链，支持人工审查，并将资格视为动态状态，提供未来可行的建议。

Result: 该系统能够生成带有可解释推理链的结构化资格评估，支持人工审查。它超越了简单的二元分类，识别当前匹配并提供使患者未来符合资格的可操作建议。系统旨在减轻协调员负担，智能地扩大为每位患者考虑的试验范围，并保证所有AI生成输出的全面可审计性。

Conclusion: 该AI决策支持工具通过提供可解释的、动态的资格评估和建议，有望显著优化临床试验患者筛选流程，提高效率、减轻负担并确保透明度。
...
### [12] [Predicting California Bearing Ratio with Ensemble and Neural Network Models: A Case Study from Türkiye](https://arxiv.org/abs/2512.08340)
*Abdullah Hulusi Kökçam,Uğur Dağdeviren,Talas Fikret Kurnaz,Alparslan Serhat Demir,Caner Erden*

Main category: cs.AI

TL;DR: 本研究提出并评估了一个基于机器学习的框架，用于预测土体的加州承载比（CBR），发现随机森林模型表现最佳，为传统实验室测试提供了一种高效、准确的替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统的CBR实验室测试耗时、昂贵且不切实际，尤其对于大规模或多样化的土壤剖面。人工智能（特别是机器学习）的进步为通过数据驱动方法更快速、精确地建模复杂土壤行为提供了可能性。

Method: 研究从土耳其不同地理气候区域收集了382个土壤样本，构建了一个包含物理化学土壤特性的数据集。测试了包括决策树、随机森林、极端随机树、梯度提升、XGBoost、K近邻、支持向量回归、多层感知机、AdaBoost、Bagging、Voting和Stacking回归器在内的12种机器学习算法。每个模型都经过训练、验证和评估，以衡量其泛化能力和鲁棒性。

Result: 在所有测试模型中，随机森林回归器表现最佳，在训练集、验证集和测试集上分别取得了0.95、0.76和0.83的R2分数。这些结果突显了该模型强大的非线性映射能力。

Conclusion: 本研究支持将智能、以数据为中心的模型整合到岩土工程中，为传统CBR测定方法提供了一种有效的替代方案，并促进了基础设施分析和设计领域的数字化转型。
...
### [13] [rSIM: Incentivizing Reasoning Capabilities of LLMs via Reinforced Strategy Injection](https://arxiv.org/abs/2512.08300)
*Sijia Chen,Baochun Li,Di Niu*

Main category: cs.AI

TL;DR: 本文提出了一种名为rSIM的强化策略注入机制，通过多智能体强化学习训练一个小型规划器，使其能自适应地向大型语言模型（LLM）的思维链中注入推理策略，从而将任意LLM转化为具有高级推理能力的推理语言模型（RLM）。


<details>
  <summary>Details</summary>
Motivation: 受LLM通过强化学习后训练演变为RLM时，在思维链中展现出如自我反思和深度思考等策略的“顿悟”时刻所启发，本文旨在提出一种机制，使任何LLM都能通过注入推理策略而成为RLM。

Method: 本文提出了一种强化的策略注入机制（rSIM）。该机制使用一个小型规划器（领导者智能体）通过自适应地注入推理策略来指导LLM（追随者智能体）的思维链。规划器与LLM在一个领导者-追随者框架下，基于简单的规则奖励，通过多智能体强化学习（MARL）共同训练。

Result: 实验结果表明，rSIM能够使Qwen2.5-0.5B成为RLM，并且显著优于Qwen2.5-14B。此外，该规划器具有通用性，只需训练一次即可作为插件应用于现有LLM，大幅提升其推理能力。它还支持跨任务的持续学习，使其规划能力能逐步提高并泛化到更广泛的问题。

Conclusion: rSIM提供了一种有效的方法，通过一个可泛化且支持持续学习的小型规划器，将LLM转化为RLM，显著增强其推理能力，并能在不同任务中持续改进。
...
TL;DR: 本研究提出了一种自动化机器学习（AutoML）方法，利用XGBoost算法有效预测土壤压实参数（最佳含水量OMC和最大干密度MDD），提高了预测的准确性和泛化能力，适用于不同土壤类型。


<details>
  <summary>Details</summary>
Motivation: 传统的土壤压实参数确定方法（如OMC和MDD）劳动密集且耗时。现有的经验回归模型适用性有限，而常规机器学习模型在处理异构数据集时，预测精度和泛化能力不足。

Method: 本研究提出了一种自动化机器学习（AutoML）方法来预测OMC和MDD。AutoML能够自动选择算法并优化超参数，旨在提高预测准确性和可扩展性。通过广泛实验，评估了不同的机器学习算法。

Result: 实验结果表明，极限梯度提升（XGBoost）算法表现最佳，在独立数据集上，MDD的R平方值达到80.4%，OMC的R平方值达到89.1%。研究还强调了异构数据集对于提高机器学习模型泛化能力和性能的重要性。

Conclusion: AutoML方法（特别是XGBoost）在预测不同土壤类型的压实参数方面表现出卓越的有效性。这项研究通过改进土壤压实参数的预测，有助于实现更高效、更可靠的工程实践。
...
### [22] [The SMART+ Framework for AI Systems](https://arxiv.org/abs/2512.08592)
...
### [23] [Autonomous Issue Resolver: Towards Zero-Touch Code Maintenance](https://arxiv.org/abs/2512.08492)
*Aliaksei Kaliutau*
...
### [25] [A Lightweight Transfer Learning-Based State-of-Health Monitoring with Application to Lithium-ion Batteries in Unmanned Air Vehicles](https://arxiv.org/abs/2512.08512)
...
### [27] [See-Control: A Multimodal Agent Framework for Smartphone Interaction with a Robotic Arm](https://arxiv.org/abs/2512.08629)
...
### [28] [Protein Secondary Structure Prediction Using Transformers](https://arxiv.org/abs/2512.08613)
*Manzi Kevin Maxime*
...
### [31] [Performance Comparison of Aerial RIS and STAR-RIS in 3D Wireless Environments](https://arxiv.org/abs/2512.08755)
...
TL;DR: CARLoS是一个大规模框架，通过分析图像生成行为和CLIP嵌入，对LoRA进行无需元数据的表征，从而实现高效的语义检索，并为版权等法律分析提供支持。


<details>
  <summary>Details</summary>
Motivation: 生成组件（如LoRA）的快速增长导致了一个庞大但非结构化的生态系统。现有的发现方法依赖不可靠的用户描述或有偏见的流行度指标，阻碍了可用性。

Method: 该研究分析了超过650个LoRA，通过在各种提示和种子下进行图像生成来评估其行为。利用CLIP嵌入及其与基础模型生成的差异，定义了一个三部分表示：方向（语义偏移）、强度（效果显著性）和一致性（效果稳定性）。基于这些表示，开发了一个高效的检索框架。

Result: CARLoS的检索框架能够将文本查询与相关LoRA进行语义匹配，并过滤掉过强或不稳定的LoRA，在自动化和人工评估中均优于文本基线。此外，该表示还支持将强度和一致性与版权中的实质性和意图等法律概念联系起来的分析。

Conclusion: CARLoS是一个实用的LoRA分析系统，具有广泛的相关性，不仅能实现高效的语义检索，还能为LoRA的法律分析提供关键洞察。
...
### [35] [Same Content, Different Answers: Cross-Modal Inconsistency in MLLMs](https://arxiv.org/abs/2512.08923)
*Angela van Sprang,Laurens Samson,Ana Lucic,Erman Acar,Sennay Ghebreab,Yuki M. Asano*

Main category: cs.AI

TL;DR: 本文引入REST和REST+基准测试，揭示了多模态大语言模型（MLLMs）在图像、文本和混合模态之间存在显著的跨模态不一致性，即模型无法对相同语义信息进行一致推理。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型（MLLMs）被训练来在同一嵌入空间中表示视觉和语言，但它们无法在不同模态中执行相同的任务，这表明存在跨模态不一致性。

Method: 引入两个新基准REST和REST+（Render-Equivalence Stress Tests），包含在图像、文本和混合三种模态中具有相同语义信息的样本。评估了15个主流MLLMs，分析了OCR问题、视觉特征（文本颜色、分辨率、字体）和视觉token数量对模型性能的影响，并将一致性分数与文本和图像之间的模态差距关联起来。

Result: 最先进的MLLMs无法在不同模态间进行一致推理。模态不一致程度差异显著，即使考虑了OCR问题。将文本渲染为图像或将图像渲染为文本都无法解决这种不一致性。即使OCR正确，视觉特征（文本颜色和分辨率，而非字体）和视觉token数量也会影响模型性能。一致性分数与文本和图像之间的模态差距相关联。

Conclusion: MLLMs存在严重的跨模态不一致性，这种不一致性并非简单的渲染问题，而是与文本和图像之间的模态差距有关，这揭示了跨模态不一致MLLMs的潜在机制。
...
### [38] [Detection of Cyberbullying in GIF using AI](https://arxiv.org/abs/2512.07838)
*Pal Dave,Xiaohong Yuan,Madhuri Siddula,Kaushik Roy*

Main category: cs.CV

TL;DR: 本研究通过收集推特上的GIF文件，并应用预训练的VGG16深度学习模型，以97%的准确率检测网络欺凌GIF，并提供了一个新的GIF数据集。


<details>
  <summary>Details</summary>
Motivation: 网络欺凌日益严重，社交媒体是其主要平台。尽管针对文本和图片的网络欺凌检测研究较多，但针对GIF/贴纸的检测研究非常稀少，存在研究空白。

Method: 研究首先从推特上提取与网络欺凌相关的标签，然后使用GIPHY API下载GIF文件，构建了一个包含超过4100个网络欺凌和非网络欺凌GIF的数据集。随后，应用预训练的深度学习模型VGG16进行网络欺凌检测。

Result: 所应用的深度学习模型（VGG16）在检测网络欺凌GIF方面达到了97%的准确率。此外，研究还为该领域的研究人员提供了一个新的GIF数据集。

Conclusion: VGG16深度学习模型能够有效检测GIF中的网络欺凌，并且本研究创建的GIF数据集为未来的相关研究提供了宝贵资源。
...
### [39] [Near-real time fires detection using satellite imagery in Sudan conflict](https://arxiv.org/abs/2512.07925)
*Kuldip Singh Atwal,Dieter Pfoser,Daniel Rothbart*

Main category: cs.CV

TL;DR: 本文利用深度学习和Planet Labs四波段卫星图像，实现了苏丹武装冲突中火灾损害的快速监测，并证明了其有效性。


<details>
  <summary>Details</summary>
Motivation: 苏丹持续的战争凸显了对冲突进行快速监测和分析的需求。深度学习和现成的卫星遥感图像为近实时监测提供了可能。

Method: 使用Planet Labs的四波段图像，结合深度学习模型来监测武装冲突中的火灾损害。通过与基线方法进行比较，并在五个苏丹案例研究中进行验证。同时，探讨了使用八波段图像或时间序列图像带来的额外收益。

Result: 该自动化方法能够以最小延迟监测火灾损害，比基线方法更准确地捕获活跃火灾和烧焦区域。结果表明，使用八波段图像或时间序列图像仅带来边际收益。

Conclusion: 深度学习结合四波段Planet Labs卫星图像能有效且快速地监测武装冲突中的火灾损害，而更复杂的数据（如八波段或时间序列图像）带来的改进有限。
...
### [40] [Preserving Source Video Realism: High-Fidelity Face Swapping for Cinematic Quality](https://arxiv.org/abs/2512.07951)
*Zekai Luo,Zongze Du,Zhouhang Zhu,Hao Zhong,Muzhi Zhu,Wen Wang,Yuling Xi,Chenchen Jing,Hao Chen,Chunhua Shen*

Main category: cs.CV

TL;DR: LivingSwap是首个视频参考引导的换脸模型，通过关键帧和视频参考指导，实现了长视频序列中高保真和时间一致的换脸，并显著减少了生产工作量。


<details>
  <summary>Details</summary>
Motivation: 在电影和娱乐制作中，视频换脸面临高保真度和长时间序列时间一致性的巨大挑战。研究者受到参考引导图像编辑的启发，探索是否能利用源视频的丰富视觉属性来增强换脸的保真度和时间连贯性。

Method: 本文提出了LivingSwap，这是第一个视频参考引导的换脸模型。它利用关键帧作为条件信号注入目标身份，实现灵活可控的编辑。通过结合关键帧条件和视频参考指导，模型进行时间缝合以确保长视频序列中身份的稳定保留和高保真重建。为解决训练数据稀缺问题，构建了配对换脸数据集Face2Face，并通过反转数据对确保可靠的真值监督。

Result: LivingSwap实现了最先进的换脸效果，将目标身份与源视频的表情、光照和动作无缝融合，同时显著减少了生产工作流中的手动工作量。

Conclusion: LivingSwap模型通过创新的视频参考引导和关键帧条件方法，克服了视频换脸在高保真度和时间一致性方面的挑战，并极大地提高了生产效率。
...
### [41] [Restrictive Hierarchical Semantic Segmentation for Stratified Tooth Layer Detection](https://arxiv.org/abs/2512.07984)
...
### [42] [FRIEDA: Benchmarking Multi-Step Cartographic Reasoning in Vision-Language Models](https://arxiv.org/abs/2512.08016)
...
### [43] [SSplain: Sparse and Smooth Explainer for Retinopathy of Prematurity Classification](https://arxiv.org/abs/2512.08038)
...
### [44] [Lost in Translation, Found in Embeddings: Sign Language Translation and Alignment](https://arxiv.org/abs/2512.08040)
...
### [45] [Towards Sustainable Universal Deepfake Detection with Frequency-Domain Masking](https://arxiv.org/abs/2512.08042)
...
### [46] [Mask to Adapt: Simple Random Masking Enables Robust Continual Test-Time Learning](https://arxiv.org/abs/2512.08048)
...
### [47] [Identification of Deforestation Areas in the Amazon Rainforest Using Change Detection Models](https://arxiv.org/abs/2512.08075)
...
### [48] [CVP: Central-Peripheral Vision-Inspired Multimodal Model for Spatial Reasoning](https://arxiv.org/abs/2512.08135)
...
### [49] [Accuracy Does Not Guarantee Human-Likeness in Monocular Depth Estimators](https://arxiv.org/abs/2512.08163)
*Yuki Kubota,Taiki Fukiage*

Main category: cs.CV

TL;DR: 本研究发现单目深度估计模型在准确性和类人行为之间存在复杂的权衡关系，提高准确性不一定带来更像人类的行为，因此需要超越传统准确性的多方面、以人为中心的评估。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计是自动驾驶和机器人等应用的基础能力。尽管深度神经网络（DNNs）在物理基准上表现出色，但一个关键挑战是如何使模型表示与人类感知对齐，以增强模型的鲁棒性和可解释性。物体识别领域的研究揭示了模型准确性和类人行为之间复杂的权衡，这引发了一个问题：在深度估计领域，特别是在依赖传感器而非人类感知估计的自然户外场景中，是否存在类似的差异。

Method: 本研究系统地调查了KITTI数据集上的69种单目深度估计器，以探究模型准确性与人类相似性之间的关系。为了逐因素地剖析误差模式的结构，研究应用了仿射拟合（affine fitting）将预测误差分解为可解释的组成部分。

Result: 研究结果表明，尽管人类和DNNs共享某些估计偏差（正误差相关性），但模型准确性和人类相似性之间存在明显的权衡关系。这一发现表明，提高准确性不一定会导致更像人类的行为。

Conclusion: 本研究强调了开发超越传统准确性、多方面、以人为中心的评估方法的必要性，因为提高模型准确性并不必然带来更像人类的行为。
...
### [50] [Fourier-RWKV: A Multi-State Perception Network for Efficient Image Dehazing](https://arxiv.org/abs/2512.08161)
*Lirong Zheng,Yanshan Li,Rui Yu,Kaihao Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种名为Fourier-RWKV的新型多状态感知去雾框架，它基于RWKV模型，通过整合空间、频率和语义感知，以线性计算复杂度实现了对非均匀雾霾的卓越去雾效果，并在去雾质量和计算效率之间取得了良好平衡。


<details>
  <summary>Details</summary>
Motivation: 图像去雾对于可靠的视觉感知至关重要，但在真实世界非均匀雾霾条件下仍极具挑战。尽管基于Transformer的方法在捕获全局上下文方面表现出色，但其二次方的计算复杂度阻碍了实时部署。

Method: 本文提出了一种基于多状态感知范式的傅里叶感受加权键值（Fourier-RWKV）去雾框架，以线性复杂度实现全面的雾霾降级建模。它协同整合了三种不同的感知状态：
1. 空间形式感知：通过可变形四向令牌移位（DQ-Shift）操作实现，动态调整感受野以适应局部雾霾变化。
2. 频域感知：在傅里叶混合块中实现，将RWKV的核心WKV注意力机制从空间域扩展到傅里叶域，保留全局雾霾估计所需的远程依赖性，同时减轻空间衰减。
3. 语义关系感知：通过语义桥模块（SBM）促进，该模块利用动态语义核融合（DSK-Fusion）精确对齐编解码器特征并抑制伪影。

Result: 在多个基准测试上的大量实验表明，Fourier-RWKV在各种雾霾场景下均提供了最先进的性能，同时显著降低了计算开销，在恢复质量和实际效率之间建立了有利的权衡。

Conclusion: Fourier-RWKV模型通过其创新的多状态感知方法，成功解决了真实世界非均匀雾霾去雾中的挑战，并在保持先进去雾质量的同时，实现了线性计算复杂度，使其成为一种高效且实用的去雾解决方案。
...
### [51] [GeoLoom: High-quality Geometric Diagram Generation from Textual Input](https://arxiv.org/abs/2512.08180)
*Xiaojing Wei,Ting Zhang,Wei He,Jingdong Wang,Hua Huang*

Main category: cs.CV

TL;DR: GeoLoom是一个新颖的框架，用于将自然语言几何描述转换为高精度几何图表。它通过自动形式化模块将自然语言转换为形式化语言GeoLingua，然后使用蒙特卡洛优化求解器生成精确坐标。


<details>
  <summary>Details</summary>
Motivation: 几何图表生成需要严格的空间精度，同时提供明确的约束。研究受到几何问题解决中形式语言和符号求解器在提高正确性和可解释性方面进展的启发。

Method: GeoLoom框架包含两个核心组件：一个将自然语言翻译成专门设计的生成导向形式语言GeoLingua的自动形式化模块，以及一个使用高效蒙特卡洛优化将形式约束映射到精确坐标的坐标求解器。为支持此框架，引入了GeoNF数据集，并提出了一个基于约束的评估指标来量化结构偏差。

Result: 实证结果表明，GeoLoom在结构保真度方面显著优于现有最先进的基线方法。

Conclusion: GeoLoom为可解释和可扩展的几何图表生成提供了一个有原则的基础。
...
### [52] [Animal Re-Identification on Microcontrollers](https://arxiv.org/abs/2512.08198)
*Yubo Chen,Di Zhao,Yun Sing Koh,Talia Xu*

Main category: cs.CV

TL;DR: 本文提出了一种面向微控制器（MCU）的动物重识别（Animal Re-ID）框架，通过系统地缩放MobileNetV2骨干网络并结合数据高效微调策略，在显著减小模型尺寸的同时保持高准确率，实现了在低功耗边缘设备上的实际部署。


<details>
  <summary>Details</summary>
Motivation: 现有的动物重识别模型主要为工作站或服务器设计，尺寸过大，无法在内存小、输入分辨率低的微控制器（MCU）等低功耗边缘设备上直接运行，而这些设备对于野外无线连接受限环境下的野生动物监测和精准畜牧管理至关重要。

Method: 首先，分析了现有先进动物重识别模型与MCU硬件之间的差距，发现简单知识蒸馏在内存和分辨率受限时效果有限。其次，系统地缩放了基于CNN的MobileNetV2骨干网络，设计出适用于低分辨率输入的高精度动物重识别架构。最后，引入了一种数据高效的微调策略，仅需每种动物三个图像即可在新地点快速适应。

Result: 在六个公共动物重识别数据集上，所提出的紧凑模型实现了具有竞争力的检索准确率，并将模型尺寸减小了两个数量级以上。在自收集的牛群数据集上，部署到设备上的模型在准确率上仅有微小下降，且Top-1准确率与集群版本保持一致。研究证明了在MCU级设备上实现实用、自适应的动物重识别是可行的。

Conclusion: 研究表明，在微控制器（MCU）级设备上实现实用且适应性强的动物重识别是完全可行的，这为在实际野外环境中进行可扩展部署铺平了道路。
...
### [53] [Blur2Sharp: Human Novel Pose and View Synthesis with Generative Prior Refinement](https://arxiv.org/abs/2512.08215)
*Chia-Hern Lai,I-Hsuan Lo,Yen-Ku Yeh,Thanh-Nguyen Truong,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 本文提出Blur2Sharp框架，结合3D感知神经辐射场（NeRF）和扩散模型，仅从单张参考图像生成锐利、几何一致的新视角人体头像，解决了现有方法图像不一致或模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 创建具有逼真姿态变化和视角灵活性的类人头像是一个基本挑战。现有方法要么生成几何不一致的多视角图像，要么牺牲真实感，导致在不同视角和复杂运动下输出模糊。

Method: Blur2Sharp采用双重条件架构：首先，Human NeRF模型为目标姿态生成几何连贯的多视角渲染图，提供3D结构指导；其次，一个以这些渲染图为条件的扩散模型对图像进行细化，保留精细细节和结构保真度。此外，通过分层特征融合，整合从SMPL模型中提取的纹理、法线和语义先验，以提高全局连贯性和局部细节准确性。

Result: 大量实验表明，Blur2Sharp在新的姿态和视角生成任务中始终超越了现有最先进的技术，尤其在处理宽松衣物和遮挡等挑战性场景时表现出色。

Conclusion: Blur2Sharp通过集成3D感知神经渲染和扩散模型，成功解决了生成逼真、几何一致且锐利的人体头像的挑战，显著提升了新姿态和新视角生成任务的性能。
...
### [54] [VisKnow: Constructing Visual Knowledge Base for Object Understanding](https://arxiv.org/abs/2512.08221)
*Ziwei Yao,Qiyang Wan,Ruiping Wang,Xilin Chen*

Main category: cs.CV

TL;DR: 该研究提出了一个名为VisKnow的框架，用于构建结构化的多模态视觉知识库（VKB），以实现对物体类别的深入理解。通过集成文本和图像知识以及区域标注，VisKnow能够提升零样本识别和细粒度视觉问答等视觉任务。


<details>
  <summary>Details</summary>
Motivation: 现有的计算机视觉研究在物体理解方面多局限于识别，且多模态数据（如部件、属性、共现等）通常是面向特定任务且组织不够系统，难以支持对物体类别的全面、深入理解，包括其组成、外观、关系和上下文知识等高层任务。

Method: 研究提出Visual Knowledge Base（VKB）来结构化多模态物体知识，并开发了VisKnow构建框架。该框架结合专家设计和大规模模型应用，从文本和图像源中提取多模态、物体级别的知识，并与物体和部件级别的区域标注对齐和集成。作为一个案例研究，他们构建了AnimalKB。

Result: 研究成功构建了AnimalKB，一个覆盖406个动物类别的结构化动物知识库，包含2.2万个从百科文档中提取的文本知识三元组、42万张图像及相应的区域标注。实验表明，AnimalKB能有效增强物体级别的视觉任务，如零样本识别和细粒度视觉问答，并可作为知识图谱补全和部件分割的挑战性基准。

Conclusion: 研究结果强调了自动构建视觉知识库在推进视觉理解及其实际应用方面的巨大潜力。通过结构化多模态物体知识，可以实现比传统物体识别更深层次的物体感知和高层推理。
...
### [55] [SOP^2: Transfer Learning with Scene-Oriented Prompt Pool on 3D Object Detection](https://arxiv.org/abs/2512.08223)
...
### [56] [New VVC profiles targeting Feature Coding for Machines](https://arxiv.org/abs/2512.08227)
...
### [57] [Beyond Real Weights: Hypercomplex Representations for Stable Quantization](https://arxiv.org/abs/2512.08524)
...
### [58] [MM-CoT:A Benchmark for Probing Visual Chain-of-Thought Reasoning in Multimodal Models](https://arxiv.org/abs/2512.08228)
...
### [59] [Pose-Based Sign Language Spotting via an End-to-End Encoder Architecture](https://arxiv.org/abs/2512.08738)
...
### [60] [Geometry-Aware Sparse Depth Sampling for High-Fidelity RGB-D Depth Completion in Robotic Systems](https://arxiv.org/abs/2512.08229)
...
### [61] [FastBEV++: Fast by Algorithm, Deployable by Design](https://arxiv.org/abs/2512.08237)
...
### [62] [HybridToken-VLM: Hybrid Token Compression for Vision-Language Models](https://arxiv.org/abs/2512.08240)
...
### [63] [Residual-SwinCA-Net: A Channel-Aware Integrated Residual CNN-Swin Transformer for Malignant Lesion Segmentation in BUSI](https://arxiv.org/abs/2512.08243)
...
### [64] [Distilling Future Temporal Knowledge with Masked Feature Reconstruction for 3D Object Detection](https://arxiv.org/abs/2512.08247)
...
### [65] [Query-aware Hub Prototype Learning for Few-Shot 3D Point Cloud Semantic Segmentation](https://arxiv.org/abs/2512.08253)
...
### [66] [SFP: Real-World Scene Recovery Using Spatial and Frequency Priors](https://arxiv.org/abs/2512.08254)
...
### [67] [RLCNet: An end-to-end deep learning framework for simultaneous online calibration of LiDAR, RADAR, and Camera](https://arxiv.org/abs/2512.08262)
...
### [68] [EgoX: Egocentric Video Generation from a Single Exocentric Video](https://arxiv.org/abs/2512.08269)
...
### [69] [PAVAS: Physics-Aware Video-to-Audio Synthesis](https://arxiv.org/abs/2512.08282)
...
### [70] [OpenSubject: Leveraging Video-Derived Identity and Diversity Priors for Subject-driven Image Generation and Manipulation](https://arxiv.org/abs/2512.08294)
...
### [71] [Terrain Diffusion: A Diffusion-Based Successor to Perlin Noise in Infinite, Real-Time Terrain Generation](https://arxiv.org/abs/2512.08309)
*Alexander Goslin*
...
### [73] [Detecting Dental Landmarks from Intraoral 3D Scans: the 3DTeethLand challenge](https://arxiv.org/abs/2512.08323)
...
### [74] [GeoDM: Geometry-aware Distribution Matching for Dataset Distillation](https://arxiv.org/abs/2512.08317)
*Xuhui Li,Zhengquan Luo,Zihui Cui,Zhiqiang Xu*

Main category: cs.CV

TL;DR: 本文提出GeoDM，一个在欧几里得、双曲和球面流形笛卡尔积空间中进行几何感知分布匹配的数据集蒸馏框架，通过捕获数据内在几何结构，显著提升了蒸馏性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法局限于欧几里得空间，只能捕获线性结构，忽略了真实数据固有的几何特性（如曲率）。高维数据常位于低维流形上，表明数据集蒸馏应将蒸馏数据流形与原始数据流形对齐。

Method: 提出GeoDM框架，在欧几里得、双曲和球面流形的笛卡尔积空间中操作，通过统一表示捕获平面、分层和循环结构。引入可学习的曲率和权重参数以适应底层数据几何。设计了最优传输损失以增强分布保真度。

Result: 理论分析表明，在乘积空间中进行几何感知分布匹配比欧几里得对应方法产生更小的泛化误差界。在标准基准上的大量实验证明，GeoDM优于现有最先进的数据集蒸馏方法，并且在单一几何的各种分布匹配策略中都保持有效。

Conclusion: GeoDM通过在乘积流形空间中进行几何感知分布匹配，有效地解决了现有方法忽略数据内在几何结构的问题，从而在数据集蒸馏任务中取得了优越的性能和更小的泛化误差。
...
### [75] [Low Rank Support Quaternion Matrix Machine](https://arxiv.org/abs/2512.08327)
*Wang Chen,Ziyan Luo,Shuangyue Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为低秩支持四元数矩阵机（LSQMM）的新型彩色图像分类方法，通过将RGB通道视为纯四元数并引入四元数核范数正则化来有效保留通道间的内在耦合关系和促进低秩结构。


<details>
  <summary>Details</summary>
Motivation: 受四元数数据建模在图像恢复和去噪任务中成功启发，研究者旨在为彩色图像分类开发一种新方法，以有效保留RGB通道之间的内在耦合关系，这在传统向量、矩阵或三阶张量表示中难以实现。

Method: 本文提出LSQMM模型，将RGB通道处理为纯四元数，利用四元数代数处理通道间的耦合关系。为促进强相关颜色通道产生的低秩结构，在铰链损失中加入了四元数核范数正则化项。采用基于交替方向乘子法（ADMM）的迭代算法来求解所提出的四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验结果表明，与支持向量机、支持矩阵机和支持张量机等现有最先进方法相比，所提出的分类方法在分类精度、鲁棒性和计算效率方面均表现出优势。

Conclusion: LSQMM是一种新颖且有效的彩色图像分类方法，它通过四元数代数和低秩正则化，成功地利用了RGB通道间的内在耦合关系，在分类性能上优于现有方法。
...
TL;DR: 本文提出了一种全自动方法，用于校准手术视频中的多摄像头视图并选择最佳视角，以生成无遮挡、固定视角的优质手术录像，从而克服了现有手动调整和后期处理的挑战。


<details>
  <summary>Details</summary>
Motivation: 手术视频对于教育和研究至关重要，但由于外科医生经常阻挡摄像头视野，捕获无遮挡视频极具挑战性。现有方法通过在无影灯上安装多摄像头来增加捕获无遮挡视图的机会，但每次移动灯光时都需要手动图像对齐，这非常耗时且劳动密集。本研究旨在自动化这一对齐任务。

Method: 所提出的方法包括识别照明系统移动的帧，对这些帧进行重新对齐，并选择遮挡最少的摄像头来生成一个从固定视角持续呈现手术区域的视频。此外，本文还实现了几种视图合成选项，并进行了用户研究以评估外科医生对这些选项的偏好。

Result: 通过外科医生参与的用户研究表明，与传统方法相比，本方法生成的视频在确认手术区域的便捷性和观看舒适度方面表现更优。此外，本方法还提升了视频质量。研究还评估了外科医生对所提出视图合成方法的多种合成选项的偏好。

Conclusion: 本研究提出的全自动方法有效解决了手术视频录制中的遮挡和对齐问题，能够生成高质量、无遮挡且视角一致的手术视频，显著优于传统方法，并考虑了外科医生的偏好，极大地提升了手术视频在教育和研究领域的应用价值。
...
### [77] [GeoDiffMM: Geometry-Guided Conditional Diffusion for Motion Magnification](https://arxiv.org/abs/2512.08325)
...
### [78] [Accelerated Rotation-Invariant Convolution for UAV Image Segmentation](https://arxiv.org/abs/2512.08888)
...
### [79] [DINO-BOLDNet: A DINOv3-Guided Multi-Slice Attention Network for T1-to-BOLD Generation](https://arxiv.org/abs/2512.08337)
*Jianwei Wang,Qing Wang,Menglan Ruan,Rongjun Ge,Chunfeng Yang,Yang Chen,Chunming Xie*

Main category: cs.CV

TL;DR: 本文提出了DINO-BOLDNet，一个由DINOv3引导的多层注意力框架，用于从T1w图像生成平均BOLD图像，以解决BOLD数据缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 当BOLD图像损坏或不可用时，生成BOLD图像可以恢复缺失的信息并支持后续任务，因此研究人员希望找到一种有效的方法从T1w图像生成BOLD图像。

Method: DINO-BOLDNet整合了一个冻结的自监督DINOv3编码器和一个轻量级可训练解码器。DINOv3用于提取切片内结构表示，独立的切片注意力模块融合相邻切片间的上下文信息。多尺度生成解码器恢复精细的功能对比度，并引入基于DINO的感知损失，以确保预测和真实BOLD在Transformer特征空间中的结构和纹理一致性。

Result: 在包含248名受试者的临床数据集上，DINO-BOLDNet在PSNR和MS-SSIM指标上均超越了条件GAN基线。据作者所知，这是首个能够直接从T1w图像生成平均BOLD图像的框架。

Conclusion: 该研究突出了自监督Transformer指导在结构到功能映射中的巨大潜力，为恢复缺失的BOLD信息提供了新的解决方案。
...
### [80] [LiDAS: Lighting-driven Dynamic Active Sensing for Nighttime Perception](https://arxiv.org/abs/2512.08912)
...
### [81] [Interpreting Structured Perturbations in Image Protection Methods for Diffusion Models](https://arxiv.org/abs/2512.08329)
...
### [82] [PointDico: Contrastive 3D Representation Learning Guided by Diffusion Models](https://arxiv.org/abs/2512.08330)
...
### [84] [Bi^2MAC: Bimodal Bi-Adaptive Mask-Aware Convolution for Remote Sensing Pansharpening](https://arxiv.org/abs/2512.08331)
...
### [85] [HybridSplat: Fast Reflection-baked Gaussian Tracing using Hybrid Splatting](https://arxiv.org/abs/2512.08334)
...
### [86] [ContextDrag: Precise Drag-Based Image Editing via Context-Preserving Token Injection and Position-Consistent Attention](https://arxiv.org/abs/2512.08477)
...
### [87] [Visionary: The World Model Carrier Built on WebGPU-Powered Gaussian Splatting Platform](https://arxiv.org/abs/2512.08478)
...
### [88] [Disrupting Hierarchical Reasoning: Adversarial Protection for Geographic Privacy in Multimodal Reasoning Models](https://arxiv.org/abs/2512.08503)
*Jiaming Zhang,Che Wang,Yang Cao,Longtao Huang,Wei Yang Bryan Lim*

Main category: cs.CV

TL;DR: 本文提出ReasonBreak，一个新颖的对抗性框架，通过概念感知的扰动来瓦解多模态大推理模型（MLRM）的地理位置推理链，从而保护用户隐私。同时，构建了GeoPrivacy-6K数据集。ReasonBreak在多种SOTA MLRM上显著提升了位置隐私保护效果。


<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型（MLRM）能够通过分层思维链推理从个人图像中推断出精确的地理位置，带来严重的隐私风险。现有的隐私保护技术主要针对基于感知的模型，对MLRM复杂的、分析环境线索的多步推理过程无效。

Method: 引入ReasonBreak，一个专门设计用于通过概念感知扰动来破坏MLRM分层推理的新型对抗性框架。其核心思想是，有效的地理推理干扰需要与概念层次对齐的扰动，而非统一的噪声。ReasonBreak策略性地针对推理链中关键的概念依赖，生成使特定推理步骤失效并级联影响后续推理阶段的扰动。为支持此方法，贡献了GeoPrivacy-6K数据集，包含6,341张超高分辨率（≥2K）图像及分层概念标注。

Result: 在七个最先进的MLRM（包括GPT-o3、GPT-5、Gemini 2.5 Pro）上的广泛评估表明，ReasonBreak具有卓越的有效性。它在区域级别保护方面实现了14.4%的提升（33.8% 对比 19.4%），并在街区级别保护方面几乎翻倍（33.5% 对比 16.8%）。

Conclusion: 这项工作为防御基于推理的威胁（特别是MLRM推断地理位置）的隐私保护开辟了新范式。
...
### [89] [TrackingWorld: World-centric Monocular 3D Tracking of Almost All Pixels](https://arxiv.org/abs/2512.08358)
...
### [90] [SCU-CGAN: Enhancing Fire Detection through Synthetic Fire Image Generation and Dataset Augmentation](https://arxiv.org/abs/2512.08362)
...
### [91] [A Novel Wasserstein Quaternion Generative Adversarial Network for Color Image Generation](https://arxiv.org/abs/2512.08542)
...
### [92] [Simultaneous Enhancement and Noise Suppression under Complex Illumination Conditions](https://arxiv.org/abs/2512.08378)
...
### [93] [Decoupling Template Bias in CLIP: Harnessing Empty Prompts for Enhanced Few-Shot Learning](https://arxiv.org/abs/2512.08606)
...
### [94] [Aerial Vision-Language Navigation with a Unified Framework for Spatial, Temporal and Embodied Reasoning](https://arxiv.org/abs/2512.08639)
...
### [95] [The Unseen Bias: How Norm Discrepancy in Pre-Norm MLLMs Leads to Visual Information Loss](https://arxiv.org/abs/2512.08374)
...
### [96] [Detection of Digital Facial Retouching utilizing Face Beauty Information](https://arxiv.org/abs/2512.08397)
...
### [97] [Mitigating Individual Skin Tone Bias in Skin Lesion Classification through Distribution-Aware Reweighting](https://arxiv.org/abs/2512.08733)
...
### [98] [Towards Visual Re-Identification of Fish using Fine-Grained Classification for Electronic Monitoring in Fisheries](https://arxiv.org/abs/2512.08400)
...
### [99] [Refining Visual Artifacts in Diffusion Models via Explainable AI-based Flaw Activation Maps](https://arxiv.org/abs/2512.08774)
...
### [100] [MatteViT: High-Frequency-Aware Document Shadow Removal with Shadow Matte Guidance](https://arxiv.org/abs/2512.08789)
...
### [101] [SAM-Body4D: Training-Free 4D Human Body Mesh Recovery from Videos](https://arxiv.org/abs/2512.08406)
...
### [102] [Training-Free Dual Hyperbolic Adapters for Better Cross-Modal Reasoning](https://arxiv.org/abs/2512.08820)
...
### [103] [Towards Effective and Efficient Long Video Understanding of Multimodal Large Language Models via One-shot Clip Retrieval](https://arxiv.org/abs/2512.08410)
...
### [104] [InfiniteVL: Synergizing Linear and Sparse Attention for Highly-Efficient, Unlimited-Input Vision-Language Models](https://arxiv.org/abs/2512.08829)
...
### [105] [Leveraging Multispectral Sensors for Color Correction in Mobile Cameras](https://arxiv.org/abs/2512.08441)
...
### [106] [LapFM: A Laparoscopic Segmentation Foundation Model via Hierarchical Concept Evolving Pre-training](https://arxiv.org/abs/2512.08439)
...
### [107] [Siamese-Driven Optimization for Low-Resolution Image Latent Embedding in Image Captioning](https://arxiv.org/abs/2512.08873)
...
### [108] [Uncertainty-Aware Subset Selection for Robust Visual Explainability under Distribution Shifts](https://arxiv.org/abs/2512.08445)
*Madhav Gupta,Vishak Prasad C,Ganesh Ramakrishnan*

Main category: cs.CV

TL;DR: 现有基于子集选择的深度视觉模型解释方法在OOD条件下表现不佳。本文提出了一种结合子模子集选择和基于梯度的不确定性估计的框架，以提高在ID和OOD设置下的解释鲁棒性和保真度。


<details>
  <summary>Details</summary>
Motivation: 基于子集选择的方法在解释深度视觉模型时，在分布内（ID）设置中表现良好，但在分布外（OOD）条件下的行为尚不清楚。实验表明，现有方法在OOD条件下可靠性显著下降，产生冗余、不稳定且对不确定性敏感的解释。

Method: 引入了一个框架，该框架将子模子集选择与逐层、基于梯度的不确定性估计相结合。通过自适应权重扰动估计不确定性，并利用这些估计来指导子模优化，以确保选择多样化且信息丰富的子集，无需额外训练或辅助模型。

Result: 经验评估表明，该框架不仅缓解了现有方法在OOD场景下的弱点，还在ID设置中取得了改进。这些发现突出了当前基于子集的方法的局限性，并展示了不确定性驱动的优化如何增强归因和对象级可解释性。

Conclusion: 当前基于子集的解释方法存在局限性。不确定性驱动的优化可以显著提高归因和对象级可解释性，为现实世界视觉应用中更透明和可信赖的AI铺平了道路。
...
### [109] [Team-Aware Football Player Tracking with SAM: An Appearance-Based Approach to Occlusion Recovery](https://arxiv.org/abs/2512.08467)
...
### [110] [No Labels, No Problem: Training Visual Reasoners with Multimodal Verifiers](https://arxiv.org/abs/2512.08889)
...
### [111] [Temporal Concept Dynamics in Diffusion Models via Prompt-Conditioned Interventions](https://arxiv.org/abs/2512.08486)
...
### [112] [Astra: General Interactive World Model with Autoregressive Denoising](https://arxiv.org/abs/2512.08931)
...
### [113] [On-the-fly Large-scale 3D Reconstruction from Multi-Camera Rigs](https://arxiv.org/abs/2512.08498)
...
### [114] [Beyond the Noise: Aligning Prompts with Latent Representations in Diffusion Models](https://arxiv.org/abs/2512.08505)
...
### [115] [Thinking with Images via Self-Calling Agent](https://arxiv.org/abs/2512.08511)
...
### [116] [MVP: Multiple View Prediction Improves GUI Grounding](https://arxiv.org/abs/2512.08529)
...
### [117] [OCCDiff: Occupancy Diffusion Model for High-Fidelity 3D Building Reconstruction from Noisy Point Clouds](https://arxiv.org/abs/2512.08506)
...
### [118] [PaintFlow: A Unified Framework for Interactive Oil Paintings Editing and Generation](https://arxiv.org/abs/2512.08534)
...
### [119] [Instance-Aware Test-Time Segmentation for Continual Domain Shifts](https://arxiv.org/abs/2512.08569)
*Seunghwan Lee,Inyoung Jung,Hojoon Lee,Eunil Park,Sungeun Hong*

Main category: cs.CV

TL;DR: 该论文提出了一种针对语义分割的持续测试时间适应（CTTA）方法，通过自适应调整伪标签并动态平衡受域偏移影响最大的类别的学习，以应对不断演变的领域。


<details>
  <summary>Details</summary>
Motivation: 现有CTTA方法通常依赖固定或批次级阈值，无法处理不同类别和实例之间的难度差异，这在需要密集、多类别预测的语义分割中尤其成问题。

Method: 该方法通过以下方式实现细粒度、类别和实例感知的适应：1) 根据每张图像内的置信度分布自适应调整伪标签；2) 动态平衡学习，使其偏向受域偏移影响最大的类别。这旨在产生更可靠的监督并减轻错误累积。

Result: 在八种CTTA和TTA场景（包括合成到真实和长期偏移）的广泛实验中，该方法始终优于现有最先进技术，为演变条件下的语义分割树立了新标准。

Conclusion: 通过细粒度、类别和实例感知的自适应调整，该方法能够生成更可靠的监督并有效缓解持续适应中的错误累积，从而显著提升了语义分割在持续测试时间适应场景下的性能。
...
### [120] [Photo3D: Advancing Photorealistic 3D Generation through Structure-Aligned Detail Enhancement](https://arxiv.org/abs/2512.08535)
...
### [121] [Fast-ARDiff: An Entropy-informed Acceleration Framework for Continuous Space Autoregressive Generation](https://arxiv.org/abs/2512.08537)
...
### [122] [An Iteration-Free Fixed-Point Estimator for Diffusion Inversion](https://arxiv.org/abs/2512.08547)
...
### [123] [SSCATeR: Sparse Scatter-Based Convolution Algorithm with Temporal Data Recycling for Real-Time 3D Object Detection in LiDAR Point Clouds](https://arxiv.org/abs/2512.08557)
*Alexander Dow,Manduhu Manduhu,Matheus Santos,Ben Bartlett,Gerard Dooly,James Riordan*

Main category: cs.CV

TL;DR: 本文提出SSCATeR算法，利用激光雷达连续扫描特性和帧间点云数据变化，将目标检测重点放在变化区域，通过时间数据复用和稀疏散点卷积，显著减少计算量，同时保持检测精度。


<details>
  <summary>Details</summary>
Motivation: 传统的激光雷达目标检测方法在处理点云数据时，会重复处理帧间未变化的区域，导致计算效率低下，存在大量的冗余卷积操作。

Method: 该研究利用激光雷达的连续扫描特性，通过滑动时间窗口和存储帧间卷积结果，将检测工作集中于点云数据发生变化的区域。为处理由此引入的极端稀疏性，作者扩展了其先前的基于散点的卷积工作，提出了带有时间数据循环的稀疏散点卷积算法（SSCATeR），该算法将传入的激光雷达数据视为连续流，并仅对点云中变化的部分进行操作。

Result: 通过该方法，在不牺牲准确性的前提下，处理时间减少了高达6.61倍。测试结果表明，该方法输出的特征图与传统稀疏卷积技术生成的特征图相同，同时大大提高了网络的计算效率。

Conclusion: SSCATeR算法通过利用激光雷达数据的时序变化和引入的稀疏性，实现了显著的计算效率提升，使得激光雷达目标检测在保持精度的同时大大加快了处理速度。
...
### [124] [Trajectory Densification and Depth from Perspective-based Blur](https://arxiv.org/abs/2512.08627)
*Tianchen Qiu,Qirun Zhang,Jiajian He,Zhengyue Zhuge,Jiahui Xu,Yueting Chen*

Main category: cs.CV

TL;DR: 本文提出一种新颖方法，通过分析摄像机旋转引起的视频模糊模式来估计度量深度和密集轨迹。该方法利用视觉编码器、点跟踪器、窗口嵌入、多窗口聚合和视觉语言模型，在各种深度数据集上表现出强大的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在没有机械稳定器的情况下，摄像机在拍摄时会发生旋转，尤其在长曝光场景下会产生基于透视的模糊。这种模糊是深度依赖的，即不同深度的物体模糊程度不同。研究者受此启发，希望利用这种深度依赖性来估计度量深度。

Method: 该方法利用深度位置依赖的透视模糊来估计度量深度和密集轨迹。具体步骤包括：1) 使用现成的视觉编码器和点跟踪器提取视频信息；2) 通过窗口嵌入和多窗口聚合估计深度图；3) 使用视觉语言模型致密化光学算法得到的稀疏轨迹。

Result: 实验结果表明，该方法在较大深度范围内取得了强大的性能，并保持了良好的泛化能力。与手持拍摄设置中的真实轨迹相比，所提出的光学算法实现了卓越的精度，并且密集重建保持了强大的准确性。

Conclusion: 该研究成功地通过分析视频流的模糊模式，利用联合光学设计算法估计了度量深度和密集轨迹。该方法在深度估计和轨迹重建方面表现出色，特别适用于手持拍摄场景，具有良好的精度和泛化能力。
...
### [125] [Chain-of-Image Generation: Toward Monitorable and Controllable Image Generation](https://arxiv.org/abs/2512.08645)
*Young Kyung Kim,Oded Schlesinger,Yuzhou Zhao,J. Matias Di Martino,Guillermo Sapiro*

Main category: cs.CV

TL;DR: 该论文提出了Chain-of-Image Generation (CoIG) 框架，通过将图像生成分解为类似人类艺术创作的顺序语义步骤，提高了图像生成模型的透明度、可监控性和可控性，并能缓解实体崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的图像生成模型内部过程不透明，限制了人类观察和干预，阻碍了模型可靠性、安全性与控制的实现。此外，其非人类的工作流程难以被人类理解。

Method: CoIG框架将图像生成重构为顺序、语义过程。它利用大型语言模型（LLM）将复杂提示分解为一系列简单、分步的指令。图像生成模型随后通过逐步生成和编辑图像来执行此计划，每一步专注于一个单一的语义实体。为评估其可监控性，引入了CoIG可读性（评估中间步骤清晰度）和因果相关性（量化每一步对最终图像的影响）两个新指标。

Result: CoIG显著增强了定量可监控性，并与现有基线模型相比，实现了具有竞争力的组合鲁棒性。通过将复杂的生成任务分解为简单的子问题，该框架能够缓解实体崩溃问题。CoIG是模型无关的，可与任何图像生成模型集成。

Conclusion: CoIG框架通过引入类似人类艺术创作的顺序语义过程，有效解决了图像生成模型不透明的问题，显著提升了模型的监控能力、可控性和鲁棒性，并缓解了实体崩溃，为未来更可靠、安全的图像生成模型奠定了基础。
...
### [126] [BrainExplore: Large-Scale Discovery of Interpretable Visual Representations in the Human Brain](https://arxiv.org/abs/2512.08560)
...
### [127] [Automated Pollen Recognition in Optical and Holographic Microscopy Images](https://arxiv.org/abs/2512.08589)
*Swarn Singh Warshaneyan,Maksims Ivanovs,Blaž Cugmas,Inese Bērziņa,Laura Goldberga,Mindaugas Tamosiunas,Roberts Kadiķis*

Main category: cs.CV

TL;DR: 本研究利用深度学习自动化光学和全息显微图像中的花粉检测与分类，尤其在兽医细胞学中，并通过数据集扩展显著提升了全息图像的性能。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在利用深度学习改进和自动化花粉颗粒的检测和分类，特别关注其在兽医细胞学中的应用，并探索在不同成像模式下的可行性。

Method: 研究采用YOLOv8s进行目标检测，MobileNetV3L进行分类任务，并评估了它们在光学和全息显微图像上的性能。为解决全息灰度图像的性能差距，研究通过自动化标注和边界框区域扩大等技术扩展了数据集。

Result: 在光学图像上，检测模型达到91.3%的mAP50，分类模型达到97%的总准确率。对于全息灰度图像，初始性能较低，但通过数据集扩展，检测性能从2.49%提升至13.3% mAP50，分类性能从42%提升至54%。

Conclusion: 本研究表明，至少对于图像分类任务，深度学习技术可以与经济高效的无透镜数字全息显微设备结合使用。
...
### [128] [From Cells to Survival: Hierarchical Analysis of Cell Inter-Relations in Multiplex Microscopy for Lung Cancer Prognosis](https://arxiv.org/abs/2512.08572)
...
### [129] [Modular Neural Image Signal Processing](https://arxiv.org/abs/2512.08564)
...
### [130] [OpenMonoGS-SLAM: Monocular Gaussian Splatting SLAM with Open-set Semantics](https://arxiv.org/abs/2512.08625)
...
### [131] [C-DIRA: Computationally Efficient Dynamic ROI Routing and Domain-Invariant Adversarial Learning for Lightweight Driver Behavior Recognition](https://arxiv.org/abs/2512.08647)
*Keito Inoshita*
...
### [132] [Repulsor: Accelerating Generative Modeling with a Contrastive Memory Bank](https://arxiv.org/abs/2512.08648)
...
### [133] [What really matters for person re-identification? A Mixture-of-Experts Framework for Semantic Attribute Importance](https://arxiv.org/abs/2512.08697)
...
### [134] [SegEarth-OV3: Exploring SAM 3 for Open-Vocabulary Semantic Segmentation in Remote Sensing Images](https://arxiv.org/abs/2512.08730)
...
### [135] [Dual-Branch Center-Surrounding Contrast: Rethinking Contrastive Learning for 3D Point Clouds](https://arxiv.org/abs/2512.08673)
...
### [136] [A Scalable Pipeline Combining Procedural 3D Graphics and Guided Diffusion for Photorealistic Synthetic Training Data Generation in White Button Mushroom Segmentation](https://arxiv.org/abs/2512.08747)
...
### [137] [Scale-invariant and View-relational Representation Learning for Full Surround Monocular Depth](https://arxiv.org/abs/2512.08700)
...
### [138] [Tri-Bench: Stress-Testing VLM Reliability on Spatial Reasoning under Camera Tilt and Object Interference](https://arxiv.org/abs/2512.08860)
*Amit Bendkhale*

Main category: cs.CV

TL;DR: 该研究引入Tri-Bench基准测试，评估视觉-语言模型（VLMs）在平面三角形问题上的几何推理能力，特别关注相机姿态和物体干扰。结果显示VLMs在3D几何推理上表现平平，未能有效利用参考系提示，并难以识别少数形状类别。


<details>
  <summary>Details</summary>
Motivation: 可验证的几何推理对于可信赖和可控的智能体AI至关重要。尽管视觉-语言模型（VLMs）能力强大，但在现实场景变化下常会失效。研究旨在隔离相对几何推理，并测试相机姿态和物体干扰这两个关键部署因素对VLMs表现的影响。

Method: 提出了Tri-Bench基准测试，包含平面三角形问题，并强调两种部署关键因素：相机姿态（平面 vs. 倾斜）和通过物体干扰（10种日常物体）产生的场景上下文。使用一个固定提示，其中包含明确描述周围方形边框的防护栏（通过单应性变换实现正确答案），评估了四种近期VLMs。测试了六项关于二元和连续目标的简单任务。

Result: 关于3D真实值的整体准确率适中，平均约为69%。响应与2D图像平面投影的对齐度更高，平均准确率为72%。所有四种VLMs在识别少数形状类别（等边、等腰、直角三角形）时均持续失败，准确率降至约0%。在相机倾斜下，VLM整体准确率下降约4.1%。这表明模型未能正确利用提示中提供的显式参考系提示，并默认使用2D图像平面线索。物体干扰对VLM准确率没有显著影响。

Conclusion: 视觉-语言模型在可验证的几何推理方面仍面临挑战，尤其是在3D理解和利用明确参考系提示方面，它们倾向于依赖2D图像平面线索，并且难以识别特定的几何形状类别。这凸显了在鲁棒几何推理方面进一步改进的需求。
...
### [139] [Skewness-Guided Pruning of Multimodal Swin Transformers for Federated Skin Lesion Classification on Edge Devices](https://arxiv.org/abs/2512.08751)
...
### [140] [UniLayDiff: A Unified Diffusion Transformer for Content-Aware Layout Generation](https://arxiv.org/abs/2512.08897)
*Zeyang Liu,Le Wang,Sanping Zhou,Yuxuan Wu,Xiaolong Sun,Gang Hua,Haoxiang Li*

Main category: cs.CV

TL;DR: UniLayDiff是一个统一的扩散Transformer模型，首次实现了用一个端到端可训练的模型解决各种内容感知布局生成任务，包括无条件和多种条件限制下的生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以用单一模型处理多样化的内容感知布局生成任务，这些任务受元素类型、大小或关系等多种约束。当前方案要么只解决部分任务，要么需要为不同条件使用单独的模型参数，缺乏真正的统一解决方案。

Method: 本文提出了UniLayDiff，一个统一的扩散Transformer。它将布局约束视为一种独立的模态，并采用多模态扩散Transformer框架来捕捉背景图像、布局元素和各种约束之间的复杂相互作用。此外，通过在其他任务上预训练后，使用LoRA对模型进行微调以整合关系约束。

Result: UniLayDiff在从无条件到各种条件生成任务中均实现了最先进的性能，并且是首个统一了所有内容感知布局生成任务的模型。该方案不仅实现了统一的条件生成，还提升了整体布局质量。

Conclusion: UniLayDiff成功地将多样化的内容感知布局生成任务统一到一个单一的、端到端可训练的模型中，实现了最先进的性能并提升了布局质量，填补了现有方法在统一性方面的空白。
...
### [141] [Wan-Move: Motion-controllable Video Generation via Latent Trajectory Guidance](https://arxiv.org/abs/2512.08765)
...
### [142] [LoFA: Learning to Predict Personalized Priors for Fast Adaptation of Visual Generative Models](https://arxiv.org/abs/2512.08785)
...
### [143] [Generation is Required for Data-Efficient Perception](https://arxiv.org/abs/2512.08854)
...
### [144] [SATGround: A Spatially-Aware Approach for Visual Grounding in Remote Sensing](https://arxiv.org/abs/2512.08881)
...
### [145] [Unified Diffusion Transformer for High-fidelity Text-Aware Image Restoration](https://arxiv.org/abs/2512.08922)
...
### [146] [Self-Evolving 3D Scene Generation from a Single Image](https://arxiv.org/abs/2512.08905)
...
### [147] [Efficiently Reconstructing Dynamic Scenes One D4RT at a Time](https://arxiv.org/abs/2512.08924)
...
### [148] [Selfi: Self Improving Reconstruction Engine via 3D Geometric Feature Alignment](https://arxiv.org/abs/2512.08930)
...
### [149] [Short-Context Dominance: How Much Local Context Natural Language Actually Needs?](https://arxiv.org/abs/2512.08082)
*Vala Vakilian,Zimeng Wang,Ankit Singh Rawat,Christos Thrampoulidis*

Main category: cs.CL

TL;DR: 本研究调查了大型语言模型中的“短上下文主导”现象，发现大多数序列的下一个token只需短前缀即可预测。文章提出了一种代理指标DaMCL来检测需要长上下文的序列，并开发了一种解码算法来减轻短上下文偏见，从而提高了模型性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型使用长上下文，但存在一个假设，即对于大多数序列，其下一个token的预测仅需一个小的局部前缀。本研究旨在验证这一“短上下文主导”假设，并探讨如何识别需要长上下文的挑战性序列，以及如何应对由此产生的模型输出偏差。

Method: 1. 使用大型语言模型作为统计预言机，测量重现准确的全上下文预测所需的最小上下文长度（MCL）。2. 引入一种无需实际下一个token知识、且与多种采样策略兼容的MCL实用代理指标——分布感知MCL（DaMCL）。3. 开发一种直观的解码算法，利用DaMCL检测器识别并增强与长距离相关的token，以对抗短上下文主导带来的偏差。

Result: 1. 对于来自长上下文文档的1-7k token序列，75-80%的序列仅需最多96个token即可进行准确预测，证实了短上下文主导的普遍性。2. 简单地对定义DaMCL的指标进行阈值处理，在检测长上下文与短上下文序列方面表现出高精度。3. 减轻短上下文偏差的解码算法在跨问答任务和模型架构上均能提高性能。

Conclusion: 短上下文主导现象在大型语言模型中普遍存在。通过引入DaMCL来检测需要长上下文的挑战性序列，并开发一种利用该检测器来提升长程相关token的解码算法，可以有效减轻短上下文偏差，从而显著提高模型的预测性能。
...
### [150] [Adaptation of Embedding Models to Financial Filings via LLM Distillation](https://arxiv.org/abs/2512.08088)
*Eliot Brenner,Dominic Seyler,Manjunath Hegde,Andrei Simion,Koustuv Dasgupta,Bing Xiang*

Main category: cs.CL

TL;DR: 本文提出了一种可扩展的流水线，通过迭代的学生-教师模型交互和难例挖掘，利用无标签语料库训练专业的金融检索嵌入模型，显著提升了检索性能并降低了成本，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式大型语言模型（LLMs）取得了进展，但专业会话式AI代理的实际应用仍受计算成本、延迟和领域特定相关性度量需求的限制。现有嵌入模型虽能解决前两个问题，但在金融等专业领域的信息检索方面表现不佳。

Method: 该方法引入了一个可扩展的流水线，以通用检索嵌入模型为基础，从无标签语料库训练专业模型。它将检索嵌入（双编码器）应用于RAG，而非LLM生成器，并利用LLM判断的相关性来提炼领域知识。其核心在于学生和教师模型之间的迭代交互，交替进行基于检索的难正/负例挖掘，并用这些例子迭代重训练学生模型的权重。每次检索迭代都使用改进的学生模型来挖掘语料库中渐进更难的训练例子。

Result: 该方法在14种金融文件类型上，对21,800个查询-文档对的测量结果显示，MRR@5平均提升27.7%，平均DCG@5平均提升44.6%。此外，在FinanceBench的4个文档类别中有3个NDCG得到改善。

Conclusion: 该方法提供了一种成本效益高的解决方案，无需劳动密集型的人工标注，即可弥合通用模型与专业领域之间的差距，从而显著提升了金融等专业领域的信息检索性能。
...
### [151] [Segment, Embed, and Align: A Universal Recipe for Aligning Subtitles to Signing](https://arxiv.org/abs/2512.08094)
*Zifan Jiang,Youngjoon Jang,Liliane Momeni,Gül Varol,Sarah Ebling,Andrew Zisserman*

Main category: cs.CL

TL;DR: 本文提出了一种名为SEA的通用方法，用于将口语字幕对齐到连续手语视频。该方法通过利用预训练模型进行手语分割和嵌入，然后使用动态规划进行对齐，实现了跨多种语言和领域的高效且最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 以往的手语字幕对齐方法通常依赖于特定语言或数据集的端到端训练，这限制了它们的通用性。研究目标是开发一种能跨多种语言和领域工作的通用对齐方法。

Method: SEA方法包含三个步骤：1) 使用预训练模型将视频帧序列分割成单个手语；2) 使用第二个预训练模型将每个手语的视频片段和对应的文本嵌入到共享的潜在空间中；3) 随后，通过轻量级的动态规划程序执行对齐。该方法灵活，可适应不同规模的资源。

Result: SEA在四个手语数据集上取得了最先进的对齐性能。该方法高效，即使对于长达一小时的视频，也能在一分钟内通过CPU完成对齐。它还具有很强的灵活性，能适应从小词典到大型连续语料库的各种场景。

Conclusion: SEA方法能够生成高质量的并行数据，这对手语处理领域的进步具有巨大潜力。其代码和模型已公开可用，进一步促进了该领域的研究和应用。
...
### [152] [Universal Adversarial Suffixes for Language Models Using Reinforcement Learning with Calibrated Reward](https://arxiv.org/abs/2512.08131)
...
### [153] [Universal Adversarial Suffixes Using Calibrated Gumbel-Softmax Relaxation](https://arxiv.org/abs/2512.08123)
*Sampriti Soor,Suklav Ghosh,Arijit Sur*
...
### [154] [ClinicalTrialsHub: Bridging Registries and Literature for Comprehensive Clinical Trial Access](https://arxiv.org/abs/2512.08193)
...
### [155] [Are generative AI text annotations systematically biased?](https://arxiv.org/abs/2512.08404)
...
### [156] [What Triggers my Model? Contrastive Explanations Inform Gender Choices by Translation Models](https://arxiv.org/abs/2512.08440)
...
### [157] [Soft Inductive Bias Approach via Explicit Reasoning Perspectives in Inappropriate Utterance Detection Using Large Language Models](https://arxiv.org/abs/2512.08480)
...
### [158] [Curriculum Guided Massive Multi Agent System Solving For Robust Long Horizon Tasks](https://arxiv.org/abs/2512.08545)
...
### [159] [HealthcareNLP: where are we and what is next?](https://arxiv.org/abs/2512.08617)
...
### [160] [QSTN: A Modular Framework for Robust Questionnaire Inference with Large Language Models](https://arxiv.org/abs/2512.08646)
...
### [161] [An Agentic AI System for Multi-Framework Communication Coding](https://arxiv.org/abs/2512.08659)
...
TL;DR: 本研究开发了一个名为MOSAIC的多框架结构化智能体AI系统，用于自动化临床沟通的标注，通过智能体协作和RAG技术，在风湿病学和妇产科对话中取得了0.928的F1分数，显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 大规模人工标注患者-医护人员对话劳动密集、不一致且难以扩展。现有基于大型语言模型的方法通常依赖于单一任务模型，缺乏适应性、可解释性和可靠性，尤其是在应用于各种沟通框架和临床领域时。

Method: 开发了MOSAIC系统，该系统基于LangGraph架构，协调四个核心智能体：规划智能体（代码本选择和工作流规划）、更新智能体（维护检索数据库）、标注智能体（使用代码本指导的RAG和动态少样本提示进行标注）以及验证智能体（提供一致性检查和反馈）。使用26份黄金标准标注转录本进行训练，50份转录本进行测试（涵盖风湿病学和妇产科领域），将MOSAIC的输出与经过训练的人类编码员创建的黄金标准标注进行比较评估。

Result: 在测试集上，MOSAIC取得了0.928的总体F1分数。在风湿病学子集中表现最佳（F1 = 0.962），对“患者行为”（如提问、表达偏好、表现自信）的标注能力最强。消融实验表明MOSAIC优于基线模型。

Conclusion: MOSAIC系统通过其多智能体架构和RAG方法，能够有效、准确地自动化临床沟通标注，克服了人工标注和现有单一任务LLM方法的局限性，特别是在处理患者行为方面表现出色。
...
### [162] [Automatic Essay Scoring and Feedback Generation in Basque Language Learning](https://arxiv.org/abs/2512.08713)
*Ekhi Azurmendi,Xabier Arregi,Oier Lopez de Lacalle*

Main category: cs.CL

TL;DR: 本文介绍了首个公开的巴斯克语自动论文评分(AES)和反馈生成数据集（CEFR C1），并微调了开源模型。实验证明，微调后的Latxa模型在评分一致性和反馈质量上超越了GPT-5和Claude Sonnet 4.5等闭源系统，并提出了新的反馈评估方法。


<details>
  <summary>Details</summary>
Motivation: 巴斯克语等低资源语言缺乏公开可用的AES和反馈生成数据集，且需要建立透明、可复现和教育导向的NLP研究基础。

Method: 构建了一个包含3,200篇巴斯克语C1水平作文的数据集，由专家标注了评分标准（正确性、丰富性、连贯性、衔接性和任务对齐）及详细反馈和错误示例。微调了RoBERTa-EusCrawl和Latxa 8B/70B等开源模型进行评分和解释生成。提出了一种结合自动一致性指标和专家验证学习者错误的反馈生成评估方法。

Result: 编码器模型（如RoBERTa-EusCrawl）在AES方面表现可靠。对Latxa进行监督微调显著提升了性能，在评分一致性和反馈质量方面超越了GPT-5和Claude Sonnet 4.5等最先进的闭源系统。微调后的Latxa模型能生成符合标准、具有教学意义的反馈，并识别出比专有模型更广泛的错误类型。

Conclusion: 该资源和基准为巴斯克语等低资源语言的透明、可复现和以教育为基础的NLP研究奠定了基础。
...
### [163] [Fluent Alignment with Disfluent Judges: Post-training for Lower-resource Languages](https://arxiv.org/abs/2512.08777)
*David Samuel,Lilja Øvrelid,Erik Velldal,Andrey Kutuzov*

Main category: cs.CL

TL;DR: 本文提出了一种针对低资源语言的后训练方法，即使在由不流畅的奖励模型对齐时，也能保持语言模型的流畅性。该方法采用在线策略训练，无需目标语言的指令微调数据，并通过挪威语案例研究证明其优于其他常见方法。


<details>
  <summary>Details</summary>
Motivation: 现有的偏好优化研究主要集中在英语和汉语模型，而低资源语言缺乏由母语使用者编写的数据集以及生成流畅合成数据的语言模型。因此，研究目标是开发一个无需目标语言指令微调数据即可实现流畅偏好对齐的语言模型。

Method: 本文提出了一种在线策略（on-policy）训练方法。该方法与两种常见方法进行比较：基于机器翻译数据进行的监督微调，以及多语言微调。通过对挪威语书面语进行案例研究，并由母语使用者评估流畅性。

Result: 结果表明，在线策略方面至关重要，并且优于其他替代方法，同时不依赖于难以获取的数据。

Conclusion: 所提出的在线策略训练方法对于低资源语言是有效的，它能够在没有目标语言指令微调数据的情况下，使语言模型保持流畅性，并优于其他常见方法。
...
### [164] [Ask, Answer, and Detect: Role-Playing LLMs for Personality Detection with Question-Conditioned Mixture-of-Experts](https://arxiv.org/abs/2512.08814)
...
### [165] [A Systematic Evaluation of Preference Aggregation in Federated RLHF for Pluralistic Alignment of LLMs](https://arxiv.org/abs/2512.08786)
...
### [166] [Do Depth-Grown Models Overcome the Curse of Depth? An In-Depth Analysis](https://arxiv.org/abs/2512.08819)
...
### [167] [Toward Faithful Retrieval-Augmented Generation with Sparse Autoencoders](https://arxiv.org/abs/2512.08892)
...
### [168] [Zero-Splat TeleAssist: A Zero-Shot Pose Estimation Framework for Semantic Teleoperation](https://arxiv.org/abs/2512.08271)
*Srijan Dokania,Dharini Raghavan*

Main category: cs.RO

TL;DR: Zero-Splat TeleAssist是一个零样本传感器融合系统，能将普通CCTV视频流转换为共享的6自由度世界模型，用于多边远程操作。


<details>
  <summary>Details</summary>
Motivation: 在以交互为中心的远程操作设置中，现有系统可能需要特定标记或深度传感器。本研究旨在提供一种无需这些特殊硬件，仅利用普通CCTV即可实现机器人实时全局位姿的方法。

Method: 该方法整合了视觉-语言分割、单目深度估计、加权PCA姿态提取和3D高斯泼溅（3DGS）技术，构建了一个传感器融合管线，将CCTV视频流转换为6自由度世界模型。

Result: TeleAssist能够为每个操作员提供多个机器人的实时全局位置和方向，且无需使用标记或深度传感器。

Conclusion: Zero-Splat TeleAssist成功实现了一个零样本传感器融合系统，通过整合多种视觉技术，利用普通CCTV流为多边远程操作提供了实时的、共享的6自由度世界模型，大大简化了部署要求。
...
### [169] [Sparse Variable Projection in Robotic Perception: Exploiting Separable Structure for Efficient Nonlinear Optimization](https://arxiv.org/abs/2512.07969)
*Alan Papalia,Nikolas Sanderson,Haoyu Han,Heng Yang,Hanumant Singh,Michael Everett*

Main category: cs.RO

TL;DR: 本文提出了一种针对机器人感知中具有规范对称性的非线性最小二乘（NLS）问题的变量投影（VarPro）方案，通过构建无矩阵舒尔补算子，显著加速了SLAM、SNL和SfM等任务的求解速度（2到35倍），同时保持了精度。


<details>
  <summary>Details</summary>
Motivation: 机器人感知常涉及大型非线性最小二乘（NLS）问题。现有求解器虽利用了稀疏性，但未充分利用“可分离性”（即某些变量呈线性且有闭式解）。标准的VarPro方法利用可分离性，但难以处理感知问题中常见的“规范对称性”所带来的计算挑战。

Method: 本文提出了一种专为具有规范对称性的NLS问题设计的VarPro方案，该方案联合利用了可分离性和稀疏性。它可作为一次性预处理步骤，构建一个“无矩阵舒尔补算子”。该算子能高效评估简化问题的成本、梯度和Hessian-向量积，并能与标准迭代NLS求解器无缝集成。文中还提供了适用条件及扩展方法。

Result: 在SLAM、SNL和SfM的合成和真实基准测试中，本文方法比现有最先进方法实现了2到35倍的运行时间加速，同时保持了精度。作者还发布了开源C++实现和所有实验数据集。

Conclusion: 本文提出的VarPro方案成功解决了机器人感知中具有规范对称性的非线性最小二乘问题，通过利用可分离性和稀疏性并构建无矩阵舒尔补算子，显著提升了求解器的效率和运行速度，且不牺牲精度。
...
### [170] [VLD: Visual Language Goal Distance for Reinforcement Learning Navigation](https://arxiv.org/abs/2512.07976)
*Lazar Milikic,Manthan Patel,Jonas Frey*

Main category: cs.RO

TL;DR: 本文提出VLD（Vision-Language Distance）学习框架，通过解耦感知（预训练的视觉-语言距离预测器）与策略学习（模拟环境中训练的RL策略），解决机器人导航中端到端策略的sim-to-real鸿沟和数据限制问题，实现了可扩展的多模态导航。


<details>
  <summary>Details</summary>
Motivation: 从图像数据直接预测导航动作的端到端策略在机器人系统中训练困难，主要受限于策略迁移时的sim-to-real鸿沟和带动作标签的训练数据不足。

Method: 该方法将感知学习与策略学习解耦。首先，在互联网规模的视频数据上自监督训练一个目标距离预测器（VLD），该预测器能泛化处理图像和文本目标。然后，在模拟环境中利用特权几何距离信号训练一个强化学习（RL）策略，并注入噪声以模拟距离预测器的不确定性。部署时，RL策略消耗VLD预测，从而继承大规模视觉训练的语义目标信息，并保留在模拟中学习到的鲁棒低级导航行为。采用序数一致性来评估距离函数。

Result: 实验表明，VLD优于先前的时序距离方法（如ViNT和VIP）。所提出的解耦设计在模拟中实现了具有竞争力的导航性能，并支持灵活的目标模态。

Conclusion: VLD学习为可靠、多模态的导航策略提供了一条替代且更重要的是可扩展的路径，通过解耦感知和策略学习，利用大规模视觉数据获取语义目标信息，并结合模拟训练学习鲁棒的低级导航行为。
...
### [171] [Optimized Area Coverage in Disaster Response Utilizing Autonomous UAV Swarm Formations](https://arxiv.org/abs/2512.08028)
...
TL;DR: 本文提出一个无人机群系统，用于灾害（如野火）场景下的急救响应，通过局部ESDF地图实现避障和编队，并利用TSP变体优化区域覆盖和POI优先级。


<details>
  <summary>Details</summary>
Motivation: 在灾害场景中，需要一个能延长飞行时间、增强数据可用性并降低碰撞风险的系统来辅助急救人员。

Method: 该系统采用无人机群，利用局部欧几里得符号距离场（ESDF）地图进行自主导航和避障，同时保持群组编队。此外，还引入旅行商问题（TSP）变体来优化区域覆盖，并根据环境行为和关键基础设施预设值优先处理兴趣点（POI）。

Result: 通过不同群组规模的仿真验证，该系统能够最大化覆盖范围，并确保无人机之间以及无人机与障碍物之间的碰撞避免。

Conclusion: 所提出的无人机群系统在灾害场景中，能够有效实现避障、保持编队、优化覆盖并优先处理关键区域，从而提高任务成功率。
...
### [172] [DIJIT: A Robotic Head for an Active Observer](https://arxiv.org/abs/2512.07998)
...
### [173] [An Introduction to Deep Reinforcement and Imitation Learning](https://arxiv.org/abs/2512.08052)
*Pedro Santana*
...
### [174] [Chat with UAV -- Human-UAV Interaction Based on Large Language Models](https://arxiv.org/abs/2512.08145)
...
### [175] [RAVES-Calib: Robust, Accurate and Versatile Extrinsic Self Calibration Using Optimal Geometric Features](https://arxiv.org/abs/2512.08170)
...
### [176] [Ground Slow, Move Fast: A Dual-System Foundation Model for Generalizable Vision-and-Language Navigation](https://arxiv.org/abs/2512.08186)
...
### [177] [Embodied Tree of Thoughts: Deliberate Manipulation Planning with Embodied World Model](https://arxiv.org/abs/2512.08188)
...
### [178] [High-Performance Dual-Arm Task and Motion Planning for Tabletop Rearrangement](https://arxiv.org/abs/2512.08206)
...
### [179] [Model-Based Diffusion Sampling for Predictive Control in Offline Decision Making](https://arxiv.org/abs/2512.08280)
*Haldun Balim,Na Li,Yilun Du*

Main category: cs.RO

TL;DR: MPDiffuser是一种组合式模型基扩散框架，用于离线决策，通过交替规划器和动力学更新，生成任务对齐且动态可行的轨迹。它在各种基准测试和真实机器人上均表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的离线决策生成方法通常会产生动态不可行的轨迹，无法从固定数据集中合成可靠行为。

Method: 本文提出了Model Predictive Diffuser (MPDiffuser)，一个组合式模型基扩散框架，包括：(i) 一个生成多样化、任务对齐轨迹的规划器；(ii) 一个强制与底层系统动力学一致性的动力学模型；(iii) 一个选择与任务目标对齐行为的排序器模块。MPDiffuser采用交替扩散采样方案，在采样过程中交错进行规划器和动力学更新，以逐步细化轨迹，确保任务对齐和可行性。并提供了理论依据。

Result: 组合式设计提高了样本效率，能够利用低质量数据进行动力学学习，并无缝适应新动力学。在D4RL和DSRL离线决策基准测试中，MPDiffuser持续优于现有方法。初步研究表明其在基于视觉的控制任务中具有扩展潜力。此外，该方法已部署在真实四足机器人上，展示了其在现实世界控制中的实用性。

Conclusion: MPDiffuser通过其组合设计和交替扩散采样方案，有效解决了离线决策中轨迹可行性的挑战。它在经验上表现出强大的性能、样本效率、对新动力学的适应性以及在真实机器人上的实用性。
...
### [180] [Semantic-Metric Bayesian Risk Fields: Learning Robot Safety from Human Videos with a VLM Prior](https://arxiv.org/abs/2512.08233)
*Timothy Chen,Marcus Dominguez-Kuhne,Aiden Swann,Xu Liu,Mac Schwager*

Main category: cs.RO

TL;DR: 本文提出一个贝叶斯框架，通过结合预训练视觉语言模型（VLM）的先验知识和从人类演示视频中学习的ViT似然函数，提取隐式的人类风险模型，生成像素级的风险图，以实现机器人规划中更类人的安全行为。


<details>
  <summary>Details</summary>
Motivation: 人类对安全的理解是连续的、依赖于上下文和空间、且具有主观性的风险概念。为了使自主系统能够像人类一样形成理性的心理模型并指导行动选择，需要提取并内化人类的风险模型，从而实现更安全、更符合人类偏好的行为。

Method: 该研究提出一个框架，通过新颖的、语义条件化和空间变化的风险参数化，直接从安全的人类演示视频和VLM常识中监督学习，提取隐式的人类风险模型。风险被定义为一个贝叶斯公式：先验由预训练的视觉语言模型（VLM）提供，似然函数是一个学习到的ViT，将预训练的特征映射到像素对齐的风险值，从而调制先验以产生相对风险度量。该管道输入RGB图像和查询对象字符串，输出像素密集的风险图像，可用于机器人规划或轨迹优化。

Result: 该框架产生的上下文风险与人类偏好高度一致。它还展示了多种下游应用，包括作为视觉运动规划器中的价值学习器，或与经典轨迹优化算法结合使用以产生类人的运动。结果表明，该框架在使自主系统内化类人风险方面迈出了重要一步。

Conclusion: 本文提出的贝叶斯框架成功地从人类演示中提取了与人类偏好一致的、上下文相关的风险模型。通过结合VLM的常识和学习到的似然函数，该方法能够泛化到新颖的对象和场景，并能快速适应，有望使自主系统具备更类人的风险感知能力，从而实现更安全、更自然的交互和行为。
...
### [181] [Learning Spatiotemporal Tubes for Temporal Reach-Avoid-Stay Tasks using Physics-Informed Neural Networks](https://arxiv.org/abs/2512.08248)
*Ahan Basu,Ratnangshu Das,Pushpak Jagtap*

Main category: cs.RO

TL;DR: 本文提出了一种基于时空管（STT）的控制框架，用于具有未知动力学的MIMO非线性纯反馈系统，以在外部扰动下满足预设时间到达-避障-保持任务。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了解决具有未知动力学的MIMO非线性纯反馈系统在外部扰动下，如何实现预设时间到达-避障-保持（T-RAS）任务的控制问题。

Method: 该方法定义时空管（STT）为时变球，其中心和半径由物理信息神经网络（PINN）联合近似。STT的约束被表述为PINN的损失函数，并提出了一种训练算法。通过基于Lipschitz的有效性条件验证学习到的PINN在连续时间范围内的有效性。最后，基于学习到的STT表示，定义了一个无近似的闭式控制器来保证T-RAS规范的满足。

Result: 该框架的有效性和可扩展性通过两个案例研究得到验证，包括移动机器人和在复杂环境中飞行的飞行器。

Conclusion: 所提出的基于时空管的控制框架结合PINN能够有效地处理复杂非线性系统在扰动下的预设时间到达-避障-保持任务，并具有良好的有效性和可扩展性。
...
### [182] [Robust Finetuning of Vision-Language-Action Robot Policies via Parameter Merging](https://arxiv.org/abs/2512.08333)
*Yajat Yadav,Zhiyuan Zhou,Andrew Wagenmaker,Karl Pertsch,Sergey Levine*

Main category: cs.RO

TL;DR: 通用机器人策略在微调新任务时容易过拟合并丧失原有泛化能力。本文提出一种简单的模型融合（权重插值）策略，能在保留通用能力的同时，鲁棒地学习新技能，并支持持续学习。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略在面对训练数据未涵盖的新任务时，若使用有限演示进行微调，常会过拟合于特定演示，不仅丧失解决多种通用任务的先前能力，也无法在新任务内部进行泛化。研究目标是开发一种方法，在微调过程中保留通用策略的泛化能力，使其能将新技能鲁棒地整合到其技能库中。

Method: 通过一个简单而有效的策略实现：将微调模型的权重与预训练模型的权重进行插值（即模型融合）。

Result: 模型融合产生了一个单一模型，该模型继承了基础模型的通用能力，并能鲁棒地解决新任务。在广泛的模拟和真实世界实验中，它在新任务的分布外变体上优于预训练模型和微调模型。此外，模型融合能够在终身学习设置中持续获取新技能，而不会牺牲之前学到的通用能力。

Conclusion: 模型融合是一种简单而有效的方法，可以使通用机器人策略在学习新技能时，既能泛化到新任务的变体，又能保留预训练获得的广泛能力。该方法支持在不牺牲现有能力的情况下，持续获取新技能。
...
### [183] [Learning Robot Manipulation from Audio World Models](https://arxiv.org/abs/2512.08405)
*Fan Zhang,Michael Gienger*

Main category: cs.RO

TL;DR: 本文提出了一种生成式潜在流匹配模型，用于预测未来的音频观测，从而使机器人能够在多模态操作任务中进行长期推理。


<details>
  <summary>Details</summary>
Motivation: 许多机器人学习任务本质上需要多模态推理，因为仅凭视觉信息可能模糊或不完整，需要对音频的物理特性和音高模式进行时间推理。

Method: 我们提出了一种生成式潜在流匹配模型来预测未来的音频观测。该模型被整合到机器人策略中，以实现对长期后果的推理。

Result: 在需要感知野外音频或音乐信号的两个操作任务中，我们的系统表现出卓越的能力，优于没有未来预判的方法。成功学习这些任务的关键在于准确预测包含内在节奏模式的未来音频状态。

Conclusion: 成功的机器人动作学习不仅依赖于多模态输入，更关键的是对未来音频状态的准确预测，特别是那些体现内在节奏模式的音频状态。
...
### [184] [A Multi-Agent LLM Framework for Design Space Exploration in Autonomous Driving Systems](https://arxiv.org/abs/2512.08476)
*Po-An Shih,Shao-Hua Wang,Yung-Che Li,Chia-Heng Tu,Chih-Han Chang*

Main category: cs.RO

TL;DR: 本文提出一个基于多智能体大语言模型（LLM）的设计空间探索（DSE）框架，用于自动化自动驾驶系统的设计，通过集成多模态推理、3D仿真和性能分析工具，实现对执行结果的自动解释和设计探索的引导。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统设计需要高效探索庞大的软硬件配置空间和多样的环境条件。传统DSE方法难以处理多模态执行输出和复杂的性能权衡，且通常需要人工评估执行结果的正确性。

Method: 该研究提出一个基于多智能体LLM的DSE框架，将多模态推理与3D仿真和性能分析工具结合。专门的LLM智能体负责解释用户输入、生成设计点、编排执行以及分析视觉和文本执行输出，从而在无人干预下识别潜在瓶颈。

Result: 在一个SAE L4级自动驾驶（robotaxi）案例研究中，与遗传算法基线相比，该框架在相同的探索预算下，识别出更多Pareto最优、成本效益更高且导航时间更短的解决方案。实验结果也证明了基于LLM的DSE方法的效率。

Conclusion: 该框架为自动驾驶系统的设计自动化铺平了道路，展现了LLM在设计空间探索中的潜力。
...
### [185] [Prospect Theory in Physical Human-Robot Interaction: A Pilot Study of Probability Perception](https://arxiv.org/abs/2512.08481)
...
### [186] [vEDGAR -- Can CARLA Do HiL?](https://arxiv.org/abs/2512.08541)
...
### [187] [SensHRPS: Sensing Comfortable Human-Robot Proxemics and Personal Space With Eye-Tracking](https://arxiv.org/abs/2512.08518)
...
### [188] [Bridging Scale Discrepancies in Robotic Control via Language-Based Action Representations](https://arxiv.org/abs/2512.08548)
...
### [189] [RVC-NMPC: Nonlinear Model Predictive Control with Reciprocal Velocity Constraints for Mutual Collision Avoidance in Agile UAV Flight](https://arxiv.org/abs/2512.08574)
...
### [190] [Mind to Hand: Purposeful Robotic Control via Embodied Reasoning](https://arxiv.org/abs/2512.08580)
...
### [191] [Multi-Task Bayesian Optimization for Tuning Decentralized Trajectory Generation in Multi-UAV Systems](https://arxiv.org/abs/2512.08630)
...
### [192] [A Sensor-Aware Phenomenological Framework for Lidar Degradation Simulation and SLAM Robustness Evaluation](https://arxiv.org/abs/2512.08653)
...
### [193] [Sim2Swim: Zero-Shot Velocity Control for Agile AUV Maneuvering in 3 Minutes](https://arxiv.org/abs/2512.08656)
...
### [194] [Ergodic Trajectory Planning with Dynamic Sensor Footprints](https://arxiv.org/abs/2512.08661)
...
### [195] [Non Normalized Shared-Constraint Dynamic Games for Human-Robot Collaboration with Asymmetric Responsibility](https://arxiv.org/abs/2512.08688)
...
### [196] [A Multi-Robot Platform for Robotic Triage Combining Onboard Sensing and Foundation Models](https://arxiv.org/abs/2512.08754)
...
### [197] [Data-Driven Dynamic Parameter Learning of manipulator robots](https://arxiv.org/abs/2512.08767)
...
### [198] [Heterogeneity in Multi-Robot Environmental Monitoring for Resolving Time-Conflicting Tasks](https://arxiv.org/abs/2512.08813)
...
### [199] [IPPO Learns the Game, Not the Team: A Study on Generalization in Heterogeneous Agent Teams](https://arxiv.org/abs/2512.08877)
...
### [200] [OSMO: Open-Source Tactile Glove for Human-to-Robot Skill Transfer](https://arxiv.org/abs/2512.08920)
...
### [201] [Sensor Attack Detection Method for Encrypted State Observers](https://arxiv.org/abs/2512.08010)
*Yeongjun Jang,Sangwon Lee,Junsoo Kim*

Main category: eess.SY

TL;DR: 本文提出了一种加密状态观测器，能够在不解密的情况下检测传感器攻击，并适用于多输入多输出系统。


<details>
  <summary>Details</summary>
Motivation: 在控制系统中检测传感器攻击，同时保护状态数据的隐私性，避免对敏感数据进行解密。

Method: 首先，设计了一个在整数有限域上利用模运算工作的状态观测器，该观测器在稀疏攻击和传感冗余条件下生成残差信号以指示攻击。然后，开发了一种同态加密方案，使观测器能够在加密数据上运行，并自动揭示残差信号。与之前仅限于单输入单输出系统的工作不同，该方案适用于多输入多输出系统。

Result: 该观测器能够在加密数据上运行，并自动公开一个指示攻击存在的残差信号。如果公开的残差信号低于预设阈值，则可以以加密消息的形式恢复完整状态。该方案适用于通用的多输入多输出系统。

Conclusion: 该论文提出了一种新颖的加密状态观测器，能够有效检测多输入多输出系统中的传感器攻击，且无需解密即可实现。在无攻击情况下，还能以加密形式恢复系统状态。
...
### [202] [Learning Dynamics from Infrequent Output Measurements for Uncertainty-Aware Optimal Control](https://arxiv.org/abs/2512.08013)
*Robert Lefringhausen,Theodor Springer,Sandra Hirche*

Main category: eess.SY

TL;DR: 本文提出了一种贝叶斯方法，用于在动力学未知且测量稀疏、嘈杂的情况下，对非线性系统进行可靠的最优控制。


<details>
  <summary>Details</summary>
Motivation: 在非线性系统动力学未知且仅有不频繁、带噪声的输出测量时，实现可靠的最优控制是一个挑战。

Method: 通过对连续时间动力学和潜在状态轨迹建立贝叶斯先验，并使用配备数值ODE积分器的目标边际Metropolis-Hastings采样器进行更新。然后，利用后验样本构建一个考虑模型和测量不确定性的基于场景的最优控制问题，并通过标准非线性规划方法求解。

Result: 该方法在1型糖尿病模型下的血糖调节数值案例研究中得到了验证。

Conclusion: 该方法能够有效处理模型和测量不确定性，为有限感知条件下的最优控制提供了解决方案。
...
### [203] [Cabin Layout, Seat Density, and Passenger Segmentation in Air Transport: Implications for Prices, Ancillary Revenues, and Efficiency](https://arxiv.org/abs/2512.08066)
*Alessandro V. M. Oliveira,Moises D. Vassallo*

Main category: eess.SY

TL;DR: 本研究利用登机牌微观数据和乘客访谈，分析飞机客舱布局和座位密度对国内机票价格的影响。结果显示，座位密度越高，票价越低；同时，高价票的乘客常被分配到中间座位，这解释了座位选择费的经济逻辑。


<details>
  <summary>Details</summary>
Motivation: 研究飞机客舱的座位布局和密度如何影响国内航班的机票定价，并探究其背后的商业和运营逻辑。

Method: 使用登机牌微观数据与乘客面对面访谈相结合，将票价与座位位置、市场特征和航班运营相关联。通过Post-Double-Selection LASSO (PDS-LASSO) 计量经济学模型进行估计，以控制商业和运营方面的不可观测因素。此外，还对创新客舱概念及其对密度和舒适度的影响进行了探索性分析。

Result: 结果表明，更高的座位排密度与更低的票价相关，这反映了飞机尺寸增加带来的规模经济和运营效率的提升。一个意外发现是：在没有座位选择费的情况下，购买高价票的乘客（通常是临时购买）因边座已满而常被分配到中间座位，这有助于解释航空公司辅助收入（座位选择费）的经济逻辑。

Conclusion: 飞机客舱的座位密度对机票价格有显著影响，高密度有助于降低票价。高价机票乘客被分配到中间座位的情况揭示了航空公司座位选择费的合理性。研究还探讨了创新客舱概念对未来密度和舒适度的潜在影响。
...
### [204] [Mitigation of Datacenter Demand Ramping and Fluctuation using Hybrid ESS and Supercapacitor](https://arxiv.org/abs/2512.08076)
*Min-Seung Ko,Jae Woong Shim,Hao Zhu*

Main category: eess.SY

TL;DR: 本文提出了一种基于混合储能系统（HESS）的控制框架，通过多时间尺度协调控制，为具有大负载波动的超大规模AI数据中心提供全面的功率平滑，以减轻对电网稳定性的影响。


<details>
  <summary>Details</summary>
Motivation: 数据中心，特别是AI数据中心，其巨大的负载波动（斜坡变化和瞬时波动）对电网的频率和电压稳定性造成严重的压力和扰动。

Method: 该方法将电池储能系统（BESS）和超级电容器（SC）集成到HESS中，并通过协调的多时间尺度控制进行管理。一个高通滤波器（HPF）将数据中心需求分离为慢速和快速分量：慢速分量由漏积分控制器分配给BESS；快速分量由带有前馈和斜坡跟踪补偿的超前比例-微分控制器分配给SC。此外，还采用了自适应加权和重复控制机制来改善瞬态和周期性响应。

Result: 案例研究验证了所提出的方法能够有效抑制斜坡变化和波动，稳定系统频率，并在长时间、随机的训练周期下保持BESS和SC的可持续荷电状态（SoC）轨迹。

Conclusion: 所提出的基于HESS的控制框架能有效缓解数据中心负载波动对电网的扰动，稳定电网频率，并确保储能系统在长期运行中的可持续性。
...
### [205] [A Dynamic Coding Scheme to Prevent Covert Cyber-Attacks in Cyber-Physical Systems](https://arxiv.org/abs/2512.08134)
...
### [206] [Integrating Delay-Absorption Capability into Flight Departure Delay Prediction](https://arxiv.org/abs/2512.08197)
*Jianyang Zhou*
...
### [207] [Bounding the Minimal Current Harmonic Distortion in Optimal Modulation of Single-Phase Power Converters](https://arxiv.org/abs/2512.08201)
...
### [208] [Theoretical Studies of Sub-THz Active Split-Ring Resonators for Near-Field Imaging](https://arxiv.org/abs/2512.08265)
...
### [209] [Formation and Investigation of Cooperative Platooning at the Early Stage of Connected and Automated Vehicles Deployment](https://arxiv.org/abs/2512.08298)
...
### [210] [Integration of AI-Driven CAD Systems in Designing Water and Power Transportation Infrastructure for Industrial and Remote Landscape Applications](https://arxiv.org/abs/2512.08415)
*Sunggyu Park*
...
### [211] [Beyond Wave Variables: A Data-Driven Ensemble Approach for Enhanced Teleoperation Transparency and Stability](https://arxiv.org/abs/2512.08436)
...
### [212] [MPC for tracking for anesthesia dynamics](https://arxiv.org/abs/2512.08452)
*Maxim Raymond,Kaouther Moussa,Mirko Fiacchini,Jimmy Lauber*

Main category: eess.SY

TL;DR: 本文提出了一种基于跟踪型MPC的麻醉动力学控制方法，解决了模型的多输入单输出(MISO)特性导致的稳态非唯一性问题和多时间尺度动态，并通过终端条件设计确保了递归可行性和渐近稳定性。


<details>
  <summary>Details</summary>
Motivation: 麻醉动力学控制面临挑战：模型是MISO系统，导致稳态对不唯一；系统具有快慢两种时间尺度的动态，且输出方程仅依赖于快动态。研究旨在为麻醉控制提供一个鲁棒且稳定的控制策略。

Method: 提出了一种用于麻醉动力学控制的跟踪型模型预测控制(MPC)框架。将慢动态状态视为扰动并引入补偿项，从而将系统重新表述为名义系统。在MPC框架中设计了适当的终端条件，以确保递归可行性和渐近稳定性。

Result: 所提出的MPC框架成功实现了麻醉动力学的控制，并确保了系统的递归可行性和渐近稳定性。控制器性能已在仿真环境中对患者进行了评估。

Conclusion: 该研究成功开发并验证了一种基于跟踪型MPC的麻醉动力学控制方法，该方法能够有效地处理麻醉系统的MISO特性和多时间尺度动态，并保证了系统的稳定性和可行性。
...
### [213] [High-performance computing enabled contingency analysis for modern power networks](https://arxiv.org/abs/2512.08465)
*Alexandre Gracia-Calvo,Francesca Rossi,Eduardo Iraola,Juan Carlos Olives-Camps,Eduardo Prieto-Araujo*

Main category: eess.SY

TL;DR: 本文提出了一种可扩展的方法，用于现代电网的脆弱性评估，通过整合N-2偶发事件分析和小信号稳定性评估，并引入基于风险的概率指标和高性能计算，以识别易引发连锁故障的关键组件。


<details>
  <summary>Details</summary>
Motivation: 现代电网因复杂性和间歇性资源渗透增加，面临级联故障的脆弱性日益提高，传统N-1准则不足以进行严格安全评估。现有方法难以实现N-2偶发事件分析与小信号稳定性等复杂评估相结合所需的计算可行性，并且存在确定性筛选的局限性。

Method: 提出了一种可扩展的方法，将N-2偶发事件分析与小信号稳定性评估相结合。引入概率性“风险指数”（$R_i$），该指数将偶发事件的确定性“严重性”（包括最优潮流发散、孤岛化和振荡不稳定）与所涉元件基于可靠性数据的“故障频率”进行加权。该框架通过PyCOMPSs并行编程库使用高性能计算（HPC）技术实现，协调最优潮流仿真（VeraGrid）和小信号分析（STAMP），以全面探索大规模偶发事件集。

Result: 该方法在IEEE 118母线测试系统上得到验证，处理了超过57000个场景，成功识别出易引发级联故障的组件。结果表明，基于风险的方法能有效隔离传统N-1准则经常忽视的关键资产。

Conclusion: 这项工作为概率性安全评估建立了一个可复制且高效的工作流程，适用于大规模电网，并能支持操作员在准实时环境下的决策。
...
### [214] [Quantization and Security Parameter Design for Overflow-Free Confidential FRIT](https://arxiv.org/abs/2512.08464)
*Jungjin Park,Osamu Kaneko,Kiminao Kogiso*

Main category: eess.SY

TL;DR: 本研究提出了一种系统化的设计程序，用于确定CFRIT中的量化增益和安全参数，以实现无溢出且精度有保证的加密控制器调谐。


<details>
  <summary>Details</summary>
Motivation: 在加密数据驱动的增益调谐中，缺乏系统化的方法来确定量化增益和安全参数，可能导致计算溢出或调谐精度下降。

Method: 研究提出了一种系统设计程序，用于确定CFRIT中的量化增益和安全参数。通过操作数据估计量化误差范围，推导出量化增益和安全参数的明确分析条件，以防止加密计算中的溢出。此外，分析了量化误差与CFRIT和非保密FRIT之间增益偏差的定量关系。

Result: 研究导出了防止溢出的明确分析条件，并揭示了量化引起的误差与调谐精度之间的定量关系。数值示例验证了所提出的程序，表明设计的参数在规定容差内实现了准确的加密调谐并防止了溢出。同时，可视化了参数组合的允许区域，提供了参数设计的实用见解。

Conclusion: 所提出的系统设计程序能够有效确定CFRIT中的量化增益和安全参数，确保了无溢出且精度有保证的加密控制器调谐，并为加密数据驱动控制的参数设计提供了实用指导。
...
### [215] [Optimal Control of Behavioral-Feedback SIR Epidemic Model](https://arxiv.org/abs/2512.08544)
*Martina Alutto,Leonardo Cianfanelli,Giacomo Como,Fabio Fagnani,Francesca Parise*

Main category: eess.SY

TL;DR: 本文研究了一个行为反馈SIR流行病模型，其中感染率取决于易感者和感染者比例。为社会规划者制定了一个最优控制问题，旨在以最小干预成本将感染曲线保持在特定阈值以下。研究证明，“填箱”策略是该问题的最优控制。


<details>
  <summary>Details</summary>
Motivation: 传统流行病模型通常忽略个体对疫情的内生适应机制（如自愿保持社交距离、佩戴口罩）。本文旨在构建一个考虑这些行为反馈的SIR模型，并为社会规划者提供一种在考虑这些适应机制下，以最小成本控制疫情蔓延的最优策略。

Method: 本文提出了一个行为反馈SIR流行病模型，其中感染率是易感者和感染者比例的函数。在此模型基础上，针对社会规划者制定了一个无限时间范围内的最优控制问题，目标是在最小化干预成本的同时，将感染人数比例维持在预设阈值以下。通过分析模型的动态特性，证明了在一般感染率条件下，存在一个最优控制策略。

Result: 研究证明，“填箱”（filling the box）策略是该最优控制问题的解。该策略包括：在疫情传播初期不进行干预，直到感染人数达到预设阈值；一旦达到阈值，则施加最小限度的控制，使感染人数比例恒定保持在阈值水平，直到基本再生数降至1以下，疫情自然消退。这一结果推广了现有文献中针对经典SIR模型（感染率为常数）的等效问题的结论。

Conclusion: 本研究通过纳入适应性人类行为，增强了对流行病管理的理解，并为制定稳健的疫情遏制策略提供了深刻见解。最优的“填箱”策略为如何在考虑个体行为反馈的情况下有效控制疫情提供了指导。
...
### [216] [Decoupled Design of Time-Varying Control Barrier Functions via Equivariances](https://arxiv.org/abs/2512.08607)
...
### [217] [NLoS Localization with Single Base Station Based on Radio Map](https://arxiv.org/abs/2512.08608)
...
### [218] [Direct transfer of optimized controllers to similar systems using dimensionless MPC](https://arxiv.org/abs/2512.08667)
...
### [219] [Gradient-Informed Monte Carlo Fine-Tuning of Diffusion Models for Low-Thrust Trajectory Design](https://arxiv.org/abs/2512.08705)
...
### [220] [LaMoSys3.5D: Enabling 3.5D-IC-Based Large Language Model Inference Serving Systems via Hardware/Software Co-Design](https://arxiv.org/abs/2512.08731)
...
### [221] [IoT-based Cost-Effective Fruit Quality Monitoring System using Electronic Nose](https://arxiv.org/abs/2512.08753)
...
### [222] [Learned iterative networks: An operator learning perspective](https://arxiv.org/abs/2512.08444)
*Andreas Hauptmann,Ozan Öktem*

Main category: eess.IV

TL;DR: 本文提出了一个统一的算子视角来理解学习型迭代网络，用于图像重建和逆问题，弥合了经典算法与学习方法的离散性之间的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 经典的迭代优化算法通常在泛函分析背景下表述，而学习型方法常被视为纯粹的离散方法。研究动机是为学习型迭代网络提供一个统一的算子视角，以更好地理解其核心联系。

Method: 本文提出了一个统一的算子视图，将学习型重建算子（定义如何计算）与学习问题（定义计算什么）分开。在此框架下，回顾了常见的学习型迭代网络方法，并分析了线性和非线性逆问题，最后通过一个简短的数值研究进行总结。

Result: 通过统一的算子视角，本文展示了许多常见的学习型迭代网络方法在核心上是紧密相关的。

Conclusion: 统一的算子视角为理解学习型迭代网络提供了一个连贯的框架，揭示了不同方法之间的内在联系，并适用于线性和非线性逆问题，并通过数值研究得到支持。
...
### [223] [FlowSteer: Conditioning Flow Field for Consistent Image Restoration](https://arxiv.org/abs/2512.08125)
*Tharindu Wickremasinghe,Chenyang Qi,Harshana Weligampola,Zhengzhong Tu,Stanley H. Chan*

Main category: eess.IV

TL;DR: FlowSteer通过在采样过程中注入测量先验，使文本到图像（T2I）流模型在图像恢复任务中实现零样本、高保真度且与测量一致的重建，无需重新训练或适配器。


<details>
  <summary>Details</summary>
Motivation: 现有的T2I流模型在图像恢复（IR）任务中表现不佳，容易“偏离”原始测量。之前的解决方案计算成本高且难以扩展。研究的动机是探索如何高效地利用流模型现有的生成能力来解决IR问题。

Method: 本文提出了FlowSteer（FS），一种算子感知的条件化方案。它在采样路径中注入测量先验，将冻结流模型的隐式引导与显式测量约束相结合。其设计考虑了流模型的特性及其对噪声的敏感性。

Result: FlowSteer在超分辨率、去模糊、去噪和着色等任务中，在严格的零样本设置下（无需重新训练模型，无需适配器），显著提高了测量一致性和身份保留。它在利用流模型丰富的生成先验的同时，实现了更高保真度的重建图像。

Conclusion: FlowSteer是一个简单而有效的方案，通过在采样过程中整合测量约束，解决了流模型在图像恢复任务中的局限性，从而在零样本设置下实现了更高的重建保真度和一致性，并高效利用了流模型的生成能力。
...
### [224] [Tumor-anchored deep feature random forests for out-of-distribution detection in lung cancer segmentation](https://arxiv.org/abs/2512.08216)
*Aneesh Rangnekar,Harini Veeraraghavan*

Main category: eess.IV

TL;DR: 本文提出RF-Deep，一个轻量级、即插即用的基于随机森林的后处理OOD（分布外）检测框架，用于3D CT扫描中的肿瘤分割，通过利用深度特征和感兴趣区域，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 最先进的肿瘤分割模型容易受到OOD输入的影响，导致自信但错误的分割结果，给临床部署带来风险。现有的OOD检测方法存在任务特定模型偏差或增加计算成本的问题。

Method: 研究引入了一个名为RF-Deep的即插即用、轻量级、基于随机森林的后处理OOD检测框架。它利用预训练并微调的主干编码器中的分层深度特征，并通过锚定在预测肿瘤分割上的多个感兴趣区域提取特征，从而增强对成像变化的泛化能力，并适应不同视野的图像。

Result: RF-Deep在近OOD数据集上实现了AUROC > 93.50，在远OOD数据集上实现了接近完美的检测（AUROC > 99.00），显著优于基于logit和影像组学的方法。它在不同深度和预训练策略的网络中保持了相似的性能一致性。

Conclusion: RF-Deep是一种有效、轻量级且与架构无关的方法，能够增强CT图像中肿瘤分割的可靠性，通过检测OOD输入来提高临床安全性。
...
### [225] [Missing Wedge Inpainting and Joint Alignment in Electron Tomography through Implicit Neural Representations](https://arxiv.org/abs/2512.08113)
*Cedric Lim,Corneel Casert,Arthur R. C. McCray,Serin Lee,Andrew Barnum,Jennifer Dionne,Colin Ophus*

Main category: eess.IV

TL;DR: 本文提出一种全自监督隐式神经表示（INR）方法，用于电子断层扫描重建，有效解决了传统方法中的缺失楔形伪影和数据错位问题，并在不依赖大量训练数据的情况下，提供高质量、通用性强的重建结果。


<details>
  <summary>Details</summary>
Motivation: 电子断层扫描（ET）是理解材料三维形态的强大工具，但传统重建算法常受实验限制导致的缺失楔形伪影和数据错位困扰。近期提出的监督式机器学习方法虽能解决这些问题，但因依赖训练数据而难以推广到不同材料系统。

Method: 本文提出一种完全自监督的隐式神经表示（INR）方法，利用神经网络作为正则化器。该方法通过姿态优化实现快速在线对齐，能够修复缺失楔形区域，并通过模型正则化对低剂量数据集进行去噪，且仅需单个数据集即可完成。

Result: 将所提方法应用于模拟和实验数据，结果表明它能从多样化且信息有限的数据集中生成高质量的断层扫描图。INR基自监督重建实现了高保真度。

Conclusion: INR基自监督重建方法能提供高保真度的重建结果，且用户输入和预处理需求极少，可以广泛应用于各种材料样品和实验参数，具有良好的通用性和实用性。
