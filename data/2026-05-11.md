...
TL;DR: 提出GraphDC，一种分治多智能体框架，通过分解大规模图、局部推理和整合信息，提升LLM在图算法推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: LLM在图算法任务上表现不佳，因图拓扑复杂且需多步推理，尤其在更大图上。

Method: 采用分治策略：将输入图分解为子图，分配专门智能体进行局部推理，再用主智能体整合局部输出和子图间信息。

Result: 在多种图任务和大规模实例上，GraphDC显著优于现有方法，尤其在大图上提升明显。

Conclusion: 分层设计减轻了推理负担，解决了计算瓶颈，增强了鲁棒性，证明分治方法对图算法推理有效。
...
TL;DR: 推理能力越强的模型，其位置偏差越大，且偏差随推理轨迹长度单调增加；截断干预实验证明因果性，大模型R1-671B虽全局偏差小但长轨迹仍存偏差。


<details>
  <summary>Details</summary>
Motivation: 检验思维链推理和推理微调模型（如DeepSeek-R1）是否能减少启发式偏差。

Method: 对比十三种推理模式配置，测量推理轨迹长度与位置偏差分数（PBS）的偏相关；进行截断干预实验；区分直接答案偏差与推理积累偏差。

Result: 十二种配置中PBS与轨迹长度正相关（0.11-0.41），截断后选择位置偏好选项概率增加16%-32%；R1-671B全局PBS仅0.019但最长四分位为0.071。

Conclusion: 推理模型并非默认顺序鲁棒，CoT会取代直接答案偏差为长度积累偏差；建议使用诊断工具包审计位置偏差。
...
TL;DR: 提出了一种复合移动禁忌搜索（CM-Tabu），通过将边界单元的单独移动扩展为复合移动（如单元组移动或交换），在保持区域连通性的同时扩大可行邻域空间，显著提升了解的质量和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统禁忌搜索在解决空间重新划分问题时，连通性约束严重缩小可行邻域，导致搜索易陷入局部最优，需改进邻域扩展方法。

Method: 基于连通性图的关节点和双连通分量，在线性时间内生成候选单单元移动和复合移动（如整组移动或单元对交换），并集成到禁忌搜索框架中。

Result: 在费城案例中，该方法能稳定达到人口均衡的理论全局最优，且解质量、鲁棒性和效率均优于传统禁忌搜索等基线方法。

Conclusion: CM-Tabu适用于多标准优化和交互式细化的真实决策场景，具有实际应用价值。
...
TL;DR: 本文形式化定义了推理状态为认知状态图，并引入“序隙”概念作为停止条件，给出了序隙非退化的局部条件，应用于多种推理系统。


<details>
  <summary>Details</summary>
Motivation: 现有递归推理系统未明确表示推理状态的演进方式及迭代停止条件。

Method: 将推理状态表示为编码断言、证据关系、开放问题及置信权重的认知状态图；定义“序隙”度量两种推理顺序的差异，并推导序隙在不动点附近非退化的充要条件。

Result: 给出了序隙线性化后非退化的局部充要条件，表明该停止判据在特定情况下非代数平凡。

Conclusion: 提出的框架可为多种递归推理系统提供理论停止判据，但仅适用于局部，无全局收敛保证。
...
TL;DR: 提出了一种通过智能体内部表示的互信息图和谱聚类来检测多智能体系统中联盟结构的方法，并在强化学习和语言模型中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 行为观察不足以区分真正的信息耦合和虚假相似性，内部表示中的联盟可能在行为变化前形成，这对AI安全和对齐至关重要。

Method: 构建智能体隐藏状态的互信息图，应用谱聚类识别最显著的联盟边界。

Result: 在强化学习环境中成功恢复编程的联盟结构并排除假阳性；在语言模型中识别描述性提示隐含的联盟，跟踪动态重组，并揭示表示层次结构。

Conclusion: 分析隐藏状态互信息的谱聚类方法可扩展地检测表示联盟，为监测分布式AI系统中的涌现结构提供工具。
...
TL;DR: 提出CASCADE框架，在部署阶段通过上下文记忆和经验复用提升LLM智能体性能，无需修改参数。


<details>
  <summary>Details</summary>
Motivation: 传统LLM生命周期中训练与部署分离导致学习停止，限制智能体持续适应环境。

Method: 构建演化式情景记忆，将经验复用形式化为上下文赌博机问题，实现探索-利用权衡并保证无悔界。

Result: 在16个任务（医疗诊断、法律分析等）上，宏平均成功率较零样本提示提升20.9%，优于参数更新和记忆基线。

Conclusion: 部署可作为自适应学习过程，建立持续改进AI系统的基础。
...
TL;DR: 本文提出了LLM代理记忆机制的三阶段演化框架：存储、反思和经验。


<details>
  <summary>Details</summary>
Motivation: 当前研究在操作系统工程和认知科学视角之间分散，缺乏统一的理论观点和演化视角。

Method: 通过形式化定义三阶段，分析演化驱动因素，并探索前沿经验阶段的机制。

Result: 提供了设计原则和发展路线图。

Conclusion: 该框架有助于指导下一代LLM代理的开发。
...
TL;DR: 本文通过有限答案偏好稳定性的形式化定义，研究语言模型在生成最终答案前其答案偏好何时稳定。在Qwen3-4B-Instruct模型上，上下文有限答案投影在答案可解析前17-31个token已稳定，且该信号可追踪模型最终输出而非真实答案。


<details>
  <summary>Details</summary>
Motivation: 语言模型在给出最终答案前通常进行推理，但可见答案无法揭示模型答案偏好何时稳定，因此需要量化这一时刻。

Method: 定义有限答案偏好稳定性，利用模型延续概率投影到有限答案集，在二元任务中得到对数几率码δ(ξ)=S_θ(yes|ξ)-S_θ(no|ξ)，并据此计算解析后答案起始、回顾稳定性时间和领先时间，无需贪婪解码或学习探针。

Result: 在Qwen3-4B-Instruct的延迟判决任务中，有限答案投影在答案可解析前17-31个token稳定（主要模板），在解析器清洗副本中领先时间更短但为正；该信号追踪模型最终输出而非真实答案，可从紧凑隐藏摘要中线性恢复，部分与光标进展可分离，作为共享信息传输但无单一不变坐标。

Conclusion: 该方法有效衡量了语言模型答案偏好稳定化的时间点，但发现其是局部敏感的，不能可靠用于生成控制，为理解模型的推理与决策过程提供了新的量化工具。
...
TL;DR: 提出了Weblica框架，通过HTTP缓存和LLM环境合成构建可复现、可扩展的网页环境，在数千个多样化环境中进行强化学习训练，得到的Weblica-8B模型在网页导航基准上超越同等规模开源模型。


<details>
  <summary>Details</summary>
Motivation: 现有网页智能体训练数据局限于离线轨迹或少量模拟环境，无法覆盖网页的复杂性和多样性。

Method: 1) HTTP级缓存捕获和重放稳定视觉状态并保持交互行为；2) 基于真实网站和核心导航技能的LLM环境合成。

Result: Weblica-8B模型在多个网页导航基准上优于同等规模开源基线，使用更少推理步骤，测试时计算扩展性好，与API模型竞争。

Conclusion: Weblica框架可以高效生成可扩展、可复现的网页环境，从而提升强化学习训练效果。
...
TL;DR: 本文通过SCALAR框架研究了大语言模型中研究者与智能体之间的交互如何影响物理推理任务的表现，发现多轮对话普遍优于单次尝试，但改进机制高度依赖于角色配对和策略选择。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在研究级物理推理中表现提升，需要理解研究者与智能体交互方式如何影响结果。

Method: 采用SCALAR（结构化评论-行动循环）框架，包括Actor、Critic和Judge三阶段，并通过改变Actor角色、Critic反馈策略和模型规模进行实验。

Result: 多轮对话提升效果，但改善机制因配对而异；模型规模增加改善简单问题但无法解决最难瓶颈；不对称配对中建设性反馈最有效，同族配对中宽松反馈更受青睐。

Conclusion: SCALAR为评估AI科学发现中交互结构的影响提供了受控测试环境。
...
TL;DR: 提出Agent-BOM，一种用于智能体安全审计的统一结构化表示，将静态能力基础与动态运行时语义状态分离，通过图查询范式实现路径级风险评估。


<details>
  <summary>Details</summary>
Motivation: 现有表示机制（如静态SBOM和运行时日志）无法捕捉认知状态演化、能力绑定、持久内存污染及跨智能体级联风险传播，导致安全审计困难。

Method: 将智能体系统建模为层次化有向图，分离静态能力基础（模型、工具、长期记忆）与动态运行时语义状态（目标、推理轨迹、动作），通过语义边和安全属性连接，并基于OWASP Agentic Top 10实现图查询风险审计。

Result: 在真实攻击场景中，Agent-BOM能重建跨会话内存投毒、工具滥用、供应链劫持、多智能体生态系统劫持等隐蔽攻击链。
...
TL;DR: 提出心理测量框架评估生成式AI的认知特征，发现其能力发展极不均衡，语言与视觉模态间存在显著偏差。


<details>
  <summary>Details</summary>
Motivation: 超越狭隘任务表现，亟需稳健方法评估通用人工智能的认知能力。

Method: 采用韦氏成人智力量表任务与AIQ基准测试，比较多模态模型与人类常模并追踪六代模型演进。

Result: 模型在语言理解与工作记忆上达顶尖水平，感知推理却近乎最低；语言性抽象推理远快于视觉版，视觉感知组织停滞。
...
TL;DR: 提出Diamond Attention架构，通过每个智能体每步采样随机数实现角色分化，解决完全参数共享下同质智能体无法区分角色的问题，并在对称XOR游戏、控制协调任务和SMACLite跨场景迁移中取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 标准MARL中全参数共享和置换对称观测导致同质智能体策略输出相同动作分布，无法进行角色分化；理论表明需要随机性打破对称性，但现有方法缺乏结构化随机协议。

Method: 基于交叉注意力机制，每个智能体每步采样标量随机数产生瞬态排序，通过掩码屏蔽低排名智能体间的注意力，保留任务注意力，实现单轮广播的随机位协调协议。

Result: 对称XOR游戏达到1.0成功率（基线约0.5）；N=4训练的策略零样本泛化到N∈[2,8]；SMACLite跨场景零样本迁移成功（标准基线因结构限制失败）；结构化掩码替代dropout后胜率0%。

Conclusion: 协议空间结构（而非随机噪声）是实现对称性破缺的关键，Diamond Attention通过结构化随机掩码实现了有效的角色分化和零样本泛化。
...
TL;DR: 本文分析了大语言模型在井字棋游戏中的推理过程，发现其搜索深度较浅，行动决策主要由浅层推理驱动，而非深层节点，这与人类依赖深层搜索的规划方式形成对比。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对LLM推理过程中“规划”的本质、结构和驱动因素的系统理解，特别是与人类规划的差异。

Method: 通过提取四子棋游戏中推理轨迹的搜索树，拟合计算模型，并进行因果剪枝干预实验，量化LLM规划的结构和影响。

Result: LLM的搜索深度浅于人类，其表现由搜索广度而非深度预测；行动选择主要由浅层节点驱动，深层节点被忽略。

Conclusion: LLM与人类规划存在关键差异：人类依靠深层搜索提升表现，而LLM不利用深层推理；这为对齐两者规划能力提供了方向。
...
TL;DR: 提出AGWM，通过学习前提依赖关系的抽象结构来显式跟踪动作的动态可执行性，在游戏模拟环境中降低多步预测误差、提升泛化能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 标准世界模型将动作-结果相关性当作因果规则，忽略动作前提，导致在多步预测中无法判断动作是否可执行，预测误差随步长累积。

Method: AGWM通过有向无环图学习动作的前提依赖关系，显式建模动作可执行性的动态变化。

Result: 在基于游戏的模拟环境中，AGWM实现了更低的多步预测误差、更好的泛化能力（应对新配置）和更高的可解释性。

Conclusion: 显式建模动作前提结构能有效改善世界模型在交互环境中的预测准确性和鲁棒性。
...
TL;DR: 提出了Agentick基准，统一评估强化学习、大语言模型、视觉语言模型、混合及人类智能体在顺序决策任务上的表现，涵盖37个程序生成任务、6种能力和多种观测模态。


<details>
  <summary>Details</summary>
Motivation: 现有智能体研究分散在强化学习和基础模型两种范式，缺乏统一的基准来公平比较不同方法。

Method: 构建37个程序生成任务，覆盖6种能力类别、4个难度等级和5种观测模态，所有任务通过Gymnasium兼容接口暴露。包含Coding API、参考策略、SFT数据集、可组合智能体框架和排行榜。

Result: 评估27种配置和超9万回合后发现：GPT-5 mini整体最优（标准化得分0.309），PPO在规划和多智能体任务上占优，推理框架使大模型性能提升3-10倍，ASCII观测优于自然语言。

Conclusion: 各种智能体范式仍有巨大改进空间，Agentick作为多模态、能力分解的基准，为通用自主智能体研究和基础模型强化学习后训练提供了基础设施。
...
TL;DR: 该论文评估了大语言模型在等价类问题上的推理能力，发现非推理模型失败，推理模型表现更好但仍有困难，且两类模型的最难问题分别对应相变点和最大直径。


<details>
  <summary>Details</summary>
Motivation: 旨在探究大语言模型在长链推理任务上的表现，特别是最简单的等价类问题，以填补现有研究的空白。

Method: 通过控制变量数、连接概率、提示等因素，生成大量等价类问题实例，测试推理型和非推理型模型的表现。

Result: 非推理模型无法解决等价类问题；推理模型显著更好但无法完全解决。非推理模型最难实例对应相变点ln n/(n-1)，推理模型最难实例对应最大直径。

Conclusion: 大语言模型在长链推理上仍有局限，推理能力与问题结构（如相变点、直径）密切相关，为未来改进提供了方向。
...
TL;DR: 提出了基于稀疏自编码器和线性探针的机制可解释性工具包，用于在AI代理执行前内部分析工具调用决策与风险。


<details>
  <summary>Details</summary>
Motivation: 现有外部可观测方法无法在动作前诊断工具使用失败，尤其在长周期任务中早期错误代价高昂。

Method: 利用SAE分解激活状态为稀疏特征，结合线性探针预判工具需求与后果影响，并通过特征消融验证功能重要性。

Result: 在Nemotron数据集训练的探针可迁移至20B和27B模型，识别与工具决策相关的内部层和特征。

Conclusion: 机制可解释性可为代理系统提供内部监控层，补充外部评估方法，支持长期任务中的风险检测。
...
TL;DR: 提出动态调整奖励模型中的理性参数β的方法，利用大语言模型评估认知偏差，以提升对不完美人类反馈的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人类反馈存在认知偏差，传统RLHF假设固定理性参数无法反映实际情况，导致奖励模型不准确。

Method: 通过LLM评估偏好比较中可能存在的认知偏差，动态调整β值，降低有偏比较的权重。

Result: 在含强偏好的数据集上微调时，学习到更理性的下游模型。

Conclusion: 动态调整理性参数能有效提升模型对不完美反馈的鲁棒性。
...
TL;DR: 提出自编程执行(SPE)架构，让模型本身担任编排程序，通过Lisp语言Spell实现无固定策略的自主代理，实验表明前沿模型能完成复杂任务。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型代理依赖固定的编排程序，缺乏灵活性，因此研究无固定编排策略的自主执行架构。

Method: 定义代理机器形式化SPE状态，开发Lisp语言Spell支持程序自我编辑和重新评估，并避免重复执行副作用。

Result: 未经SPE或Spell训练的前沿模型能在该机制下运行并完成挑战性代理任务。

Conclusion: 语言模型无需固定编排策略即可作为代理，并启发未来训练模型学习自编排策略。
...
TL;DR: 提出了多目标约束推断（MOCI）框架，能从异构专家轨迹中同时提取共享约束和个体偏好。


<details>
  <summary>Details</summary>
Motivation: 现有约束推断方法假设同质化演示，无法捕捉个体偏好且计算效率低。

Method: 建立多目标约束推断框架，联合建模共享约束与个体偏好。

Result: 在标准网格世界基准上，MOCI优于现有基线，预测性能更好且计算效率有竞争力。

Conclusion: MOCI是一种准确、灵活且计算实用的约束推断和偏好学习方法。
...
TL;DR: 提出了一种结合广义规划和层次化任务分解的动态策略学习方法HCL-GP，在AppWorld基准测试中实现了98.2%的准确率，并展示了开源模型中动态复用带来的显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体缺乏泛化能力和可重用组件，导致在复杂、未见任务中性能下降。

Method: 通过自动化分解提取可重用组件，构建组件库，并利用语义搜索实现高效检索，以生成组合策略。

Result: 在AppWorld任务上，正常任务准确率98.2%，挑战性任务97.8%，比静态合成提升15.8个百分点；开源模型复用后成功率从接近0%提升至62.5%。

Conclusion: 将经典规划概念与LLM智能体结合能显著提升准确性和效率，并实现策略的泛化与复用。
...
TL;DR: 本文研究了在实验成本有限的情况下，如何选择能最大程度缩小因果查询边界的实验子集，提出了最大效力问题，并证明其NP-hard性。通过多项式规划框架评估离散场景下的认知效力，并引入两种图形剪枝准则（路径拦截规则和基于ID算法的可识别性检查）来减小搜索空间。在随机图和基准网络上，剪枝准则平均可剔除50-88%的候选实验。最后在NHANES数据上展示了最优实验选择。


<details>
  <summary>Details</summary>
Motivation: 现实中的因果查询常因观测数据局限而仅能部分识别，而事后实验成本高昂，因此需在实验前预选成本约束下能最大程度收紧边界的实验子集。

Method: 形式化为最大效力问题，通过0-1背包归约证明NP-hard性；基于Duarte等(2023)的多项式规划框架计算认知效力；引入两种剪枝准则：路径拦截规则（利用图结构线性时间判定零效力）和ID算法可识别性检查。

Result: 在Erdos-Renyi随机图和11个bnlearn基准网络上，剪枝准则平均剔除50-88%的候选实验；ID剪枝实验具有组合惰性，使子集搜索空间超指数级减少。

Conclusion: 提出了一种在成本约束下选择最优因果实验的方法，通过剪枝技术高效处理超指数搜索空间，并在真实数据上验证了实用性。
...
TL;DR: 论文提出了一种针对生成式AI系统的自适应测试统计框架，使用安全随时有效推理（SAVI）进行双重假设检验，有效控制错误率并能在小样本下得出统计结论。


<details>
  <summary>Details</summary>
Motivation: 传统注释和评估方法成本高、耗时长，而现有自适应测试缺乏统计严谨性。

Method: 引入双重假设检验框架（模型空假设与审计者空假设），基于SAVI的'测试即下注'方法，构建同时e过程。

Result: 该框架保持随时有效的一类错误控制，优于预设测试方法，且最少只需20个观测即可得出严格结论。

Conclusion: 足够强大的审计者能通过严苛审计认证AI系统的全局鲁棒性，证明两种假设渐近互逆。
...
TL;DR: 本文提出行为提示推理，通过训练模型在推理前发出特殊标记序列，使大语言模型的推理过程更可控和可监控，在不牺牲性能的前提下提升安全行为恢复率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理中的错误行为直到推理结束时才暴露，难以监督；现有方法难以在早期干预和监控推理过程。

Method: 行为提示（Behavior Cues）是一种特殊标记序列，模型通过强化学习训练在特定隐式或显式行为前发出这些标记。利用这些标记的压缩信息训练外部监控器，实现推理剪枝和安全动作恢复。

Result: 在复杂数学问题中，可剪枝最多50%的浪费推理标记；在约束违规环境中，安全动作恢复率从46%提升至96%，且不损失性能。

Conclusion: 行为提示推理能显著提升LLM推理的可监控性和可控性，为可扩展的监督机制提供新途径。
...
TL;DR: 本文形式化了大语言模型在复杂环境中的搜索问题为上下文收集决策过程（CGDP），并提出基于谓词的显式信念状态和程序化停止机制，从而提升多跳推理性能并节省计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理在超出上下文窗口的复杂环境中迭代搜索时，工作记忆会退化为丢失信息的表示，导致冗余循环和过早停止。

Method: 将LLM行为建模为CGDP中的近似汤普森采样，引入基于谓词的分解将隐式搜索转为显式模块化操作，并设计持久信念状态和程序化终止门控。

Result: 信念状态方法在多跳推理上提升最多11.4%，程序化停止机制节省最多39%的token且不降低性能。

Conclusion: 将LLM代理循环视为CGDP可指导模块化、非干扰的改进，提升搜索效率。
...
TL;DR: 提出2.5-D分解的神经符号流水线，将大语言模型的三维坐标错误降至2.5%，结构准确率达94.6%。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在三维空间生成块放置时存在系统性坐标错误，需提升自主系统从自然语言指令构建结构的可靠性。

Method: 2.5-D分解：LLM仅在二维水平面规划，确定性执行器根据列占用计算所有垂直放置，消除整类错误；在边缘硬件本地部署。

Result: 在Build What I Mean基准测试中，GPT-4o-mini平均结构准确率达94.6%，优于其他系统；消融实验证明2.5-D分解贡献50.7个百分点；边缘硬件匹配云端性能。

Conclusion: 去除确定性维度输出空间可系统提升空间推理可靠性，该方法适用于任何受物理约束的自主建造任务。
...
TL;DR: TeamBench是一个通过操作系统强制角色分离来评估智能体协调能力的基准测试，包含851个任务模板和931个种子实例。


<details>
  <summary>Details</summary>
Motivation: 现有智能体系统依赖提示而非访问控制来分配角色，导致团队通过率可能掩盖实际协调问题（如一个角色执行其他角色的工作）。

Method: 将规范访问、工作区编辑和最终认证分别分配给规划者、执行者和验证者角色，并通过操作系统强制分离，确保没有角色能同时读取完整要求、修改工作区和认证最终答案。

Result: 仅提示和沙盒强制团队通过率无显著差异，但仅提示系统中验证者尝试编辑执行者代码的情况多3.6倍；验证者批准了49%未通过确定性评分器的提交；移除验证者提高了平均部分分数；团队在单智能体困难时有益，但在单智能体表现良好时有害；人类研究表明基准测试能暴露通过率未捕捉的交互模式。

Conclusion: 强制角色分离对评估真实智能体协调至关重要，团队价值依赖于任务难度，且基准测试能揭示通过率掩盖的细粒度交互问题。
...
TL;DR: 本文介绍在线共享供应分配问题，提出阈值比例策略GPA，达到4/3近似比，并扩展至学习增强版本以利用预测。


<details>
  <summary>Details</summary>
Motivation: 现实资源分配系统（如人道物流和疫苗分发）需在需求未知前预置有限供应，且缺货导致不可逆损失，现有模型无法处理此类场景。

Method: 提出确定性阈值比例策略GPA，并设计学习增强扩展以整合不完美预测。

Result: GPA实现4/3近似比（紧界），且加性误差不可避免；实验表明供应稀缺时优于基线。

Conclusion: GPA在有限供应下有效，学习增强版本在利用良好预测时保持鲁棒性。
...
TL;DR: 提出ARMOR框架，通过分层工具优先级、模式识别与记忆增强推理，解决多工具预测反应可行性的冲突问题，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 单一工具在反应可行性预测中性能差异大，亟需有效整合多个工具以获得更一致的预测结果。

Method: 构建工具分层结构，优先高性能工具并动态降级其他工具，通过工具特异性模式刻画优势，利用记忆增强推理解决预测冲突。

Result: 在公共数据集上，ARMOR持续优于单工具、工具聚合与工具选择等基线方法，尤其在工具预测冲突的反应上提升显著。

Conclusion: ARMOR通过显式建模工具效用与冲突消解，有效整合多工具互补优势，提升了反应可行性预测的准确性和鲁棒性。
...
TL;DR: Switchcraft是一个针对智能体工具调用优化的模型路由系统，能在保证准确率的同时大幅降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有模型路由主要针对聊天完成，而智能体工具调用场景下大模型成本高昂，开发者缺乏有效的低成本路由方案。

Method: 采用DistilBERT分类器进行实时路由选择，在五个函数调用基准上训练，以最低成本模型满足正确性约束。

Result: 准确率82.9%，与最优单模型相当甚至更优；推理成本降低84%，每百万查询节省超过3600美元。

Conclusion: 大模型在工具调用上并不总优于小模型，且名义上廉价的模型可能因推理令牌消耗导致更高总成本；Switchcraft实现了不牺牲正确性的成本感知智能体部署。
...
TL;DR: 提出了AdaTKG模型，通过为每个实体维护动态更新的记忆（使用可学习的指数移动平均）来捕捉时间知识图谱中的交互历史，从而替代静态实体表示，并在多个基准测试上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有时间知识图谱方法生成的实体表示是静态的，不保留实体参与交互的痕迹，限制了推理能力。

Method: 为每个实体维护一个自适应记忆，每次实体参与事实时，使用一个共享的可学习标量通过指数移动平均更新记忆，无需为每个实体单独学习参数，可处理未见实体。

Result: 在多个时间知识图谱基准测试上，AdaTKG一致优于现有基线方法。

Conclusion: 自适应记忆机制能有效提升时间知识图谱推理的性能。
...
### [33] [Finite-Time Analysis of MCTS in Continuous POMDP Planning](https://arxiv.org/abs/2605.07703)
*Da Kong,Vadim Indelman*

Main category: cs.AI

TL;DR: 论文对部分可观测马尔可夫决策过程中的蒙特卡洛树搜索进行了有限时间分析，提出了离散和连续观测空间下的概率浓度界，并设计了具有理论保证的Voro-POMCPOW算法。


<details>
  <summary>Details</summary>
Motivation: MCTS风格求解器（如POMCP）虽取得经验成功，但其非平稳性和启发式动作选择导致的相互依赖性使得严格的有限时间保证仍是开放问题。

Method: 离散设定下扩展多项式探索奖励至UCB得到多项式浓度界；连续空间引入抽象划分框架并利用Voronoi单元自适应划分观测空间，提出Voro-POMCPOW算法。

Result: 证明了根节点经验值估计的高概率界，连续观测空间下分区损失有限时间界，且Voro-POMCPOW在保持理论保证同时表现有竞争力。
...
TL;DR: 提出了SREGym，一个高保真的智能体站点可靠性工程（SRE）基准测试平台，包含90个真实挑战性问题，并评估了前沿智能体表现。


<details>
  <summary>Details</summary>
Motivation: 现有SRE基准测试过于简化且难以扩展，无法反映生产环境的复杂性。

Method: 构建真实云原生系统环境，通过故障注入器模拟多层故障、环境噪声和多种故障模式，并采用模块化可扩展框架。

Result: 不同智能体在应对不同故障时能力差异显著，端到端结果差异高达40%。

Conclusion: SREGym提供了逼真且可扩展的SRE评估环境，有助于推动智能体SRE研究。
...
### [35] [Can You Break RLVER? Probing Adversarial Robustness of RL-Trained Empathetic Agents](https://arxiv.org/abs/2605.07138)
*Deeraj S K,Sadhana Devarajan,Krishna Mehra,Sudhakar Mishra*

Main category: cs.AI

TL;DR: RLVER训练的语言模型在对抗性共情基准测试中存在脆弱性,新提出的ECS指标揭示了情感反应能力与情绪状态追踪能力之间可分离的差距。


<details>
  <summary>Details</summary>
Motivation: 现有共情评估假设用户协作诚实,但现实交互存在煤气灯效应、情绪升级等对抗性动态,需建立能暴露这些缺陷的基准。

Method: 构建包含六种心理对抗轨迹的AEB基准,引入ECS量化情感一致性;在2个RLVER模型和2个基础模型上控制实验。

Result: RLVER-PPO-Think表现优异(0.963 vs 0.761),但ECS评分与基础模型无显著差异(p=0.650)。

Conclusion: RL训练提升情感响应性但不改善情绪状态追踪能力,ECS-FS差距反映行为/可解释性分离,而非模型内在理解或临床准备性。
...
### [36] [Repeated Deceptive Path Planning against Learnable Observer](https://arxiv.org/abs/2605.07174)
*Shiyue Cao,Pei Xu,Likun Yang,Lei Cui,Shizhao Yu,Shiyu Zhang,Yongjian Ren,Xiaotang Chen,Kaiqi Huang*

Main category: cs.AI

TL;DR: 提出重复欺骗路径规划（RDPP）问题，并设计双层优化框架DeMP，在可学习观测者环境下实现持续欺骗。


<details>
  <summary>Details</summary>
Motivation: 现有欺骗路径规划假设观测者为静态非学习型，无法应对可自适应学习的真实对抗者（如物流或军事场景）。

Method: 提出DeMP框架，包含情节级适应（短期策略调整）和元级更新（跨情节反馈加速适应），以缓解增量更新导致的累积滞后。

Result: 在多种环境中，DeMP显著优于现有方法，保持低路径成本的同时实现持续欺骗。

Conclusion: 对可学习对抗者的重复交互建模对多智能体系统的欺骗与隐私至关重要。
...
TL;DR: 提出了首个端到端自主探索复杂商业环境的框架AIDA，通过领域特定语言与强化学习结合，显著提升了多维数据洞察能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在将碎片化企业数据转化为可执行洞察时受限于复杂数据库架构、动态SQL生成局限及多维分析深度不足。

Method: 建立含200+指标和100+维度的灵活零售环境，集成领域特定语言桥接语义推理与SQL执行，并引入帕累托原则引导的强化学习推理系统。

Result: 在实验中显著优于基于工作流的Agent，展现出更强的环境感知能力和多角度分析深度。

Conclusion: 自主智能体在工业级商业智能系统中具有变革性潜力。
...
TL;DR: 提出一个三合一世界模型架构，融合深层玻尔兹曼机与轻量适配器，统一处理消费者异质性、时变状态与干预效果，在反事实推断上优于多种元学习器。


<details>
  <summary>Details</summary>
Motivation: 现有预测和语言模型无法统一捕捉消费者异质性、时变内部状态和显式干预的交互作用。

Method: 使用深层玻尔兹曼机从人口统计、时间和滞后行为中学习冻结信念表示，并附加轻量级任务适配器，同时支持能量评估、结果预测和反事实推断。

Result: 在仿真中，适配器在访问和购买AUC上与强MLP基线相当，但在恢复异质性处理效应上显著优于S-/T-/X-/DR元学习器和因果森林，尤其在混淆的价格促销干预中差距最大。自由能钳位系统性地惩罚缺乏先前促销暴露的反事实购买轨迹。

Conclusion: DBM信念能够解耦潜在特质，使得反事实查询有效，为营销干预提供了统一的超世界模型基础。
...
### [39] [HMACE: Heterogeneous Multi-Agent Collaborative Evolution for Combinatorial Optimization](https://arxiv.org/abs/2605.07214)
*Yuping Yan,Jirui Han,Fei Ming,Yuanshuai Li,Yaochu Jin*

Main category: cs.AI

TL;DR: 提出了一种异构多智能体协作演化框架HMACE，通过角色分工（提议、生成、评估、反思）结合记忆回放机制，在NP难组合优化问题中实现高效启发式设计，在TSP/Online BPP等任务上以极低token开销取得了最优平均gap。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自动启发式设计方法受限于刚性模板的单体流程，缺乏记忆引导探索，易陷入局部最优。

Method: 将每个演化代分解为四个角色（Proposer/Generator/Evaluator/Reflector）的自主协作循环，通过行为感知检索、候选轻量过滤和适应度驱动存档更新引导搜索。

Result: 在TSP、Online BPP等基准问题上，HMACE以0.13M和0.42M token消耗实现最低平均gap（0.464%/0.223%），优于单/多智能体基线。

Conclusion: HMACE通过角色分工与记忆反馈有效平衡了解质量与计算效率，为组合优化中的自主启发式设计提供了新范式。
...
TL;DR: 提出级联更新问题和MemoRepair协议，通过最大权重前驱闭包和s-t最小割解决智能体记忆修复，将无效记忆暴露降至0%。


<details>
  <summary>Details</summary>
Motivation: 智能体记忆中的派生制品在源数据失效后仍持续影响行为，现有系统缺乏级联修复机制。

Method: 定义屏障优先的级联修复契约，将修复问题转化为标量化的前驱闭包选择问题，并通过s-t最小割精确求解。

Result: 在ToolBench和MemoryArena上，无效记忆暴露从69.8-94.3%降至0%，修复成本降低0.57-0.76倍，有效恢复91.1-94.3%的验证后继。

Conclusion: 完整影响溯源下的级联修复可彻底消除过期派生记忆的负面影响，同时显著降低修复成本。
...
TL;DR: 提出了EnvSimBench基准，用于评估和提升LLM模拟环境的能力，发现所有LLM都存在“状态变化悬崖”问题，并设计了一个约束驱动的模拟流程来缓解幻觉问题和降低成本。


<details>
  <summary>Details</summary>
Motivation: 手工构建的智能体训练环境成本高、扩展性差且多样性有限，LLM模拟环境虽能解决这些问题但存在幻觉、逻辑不一致和状态漂移等缺陷。

Method: 形式化定义环境模拟能力（EnvSim Ability），构建包含400个样本、167个环境的基准EnvSimBench，并设计约束驱动的模拟流程。

Result: 所有先进LLM在状态不变时准确率接近完美，但需同时更新多状态时性能急剧下降；所提方法使环境合成成功率提升6.8%，成本降低90%以上。

Conclusion: EnvSim Ability是LLM的关键能力缺口，EnvSimBench可作为诊断框架和优化路径，为基于LLM的可扩展智能体训练奠定基础。
...
TL;DR: 提出了ChemCost基准，包含1427个可评估反应和固定价格快照，用于评估化学采购成本估算任务中的LLM代理。


<details>
  <summary>Details</summary>
Motivation: 现有科学工具使用评估缺乏精确、无评判的基准，特别是在化学领域中。

Method: 构建包含1427个反应、2261种化学品和230775个供应商报价的基准，设计分阶段诊断和噪声注入测试。

Result: 最强代理在干净输入上仅有50.6%的准确率，噪声条件下性能显著下降。

Conclusion: 工具访问是必要但不充分的，代理失败源于脆弱解析、无效证据整合和工具使用不收敛。
...
TL;DR: 提出面向角色感知的结构化角色感知策略优化（SRPO），通过将序列级GRPO优势分解为角色感知的令牌级优势，提升多模态推理中证据支持的准确性。


<details>
  <summary>Details</summary>
Motivation: 多模态推理中，最终答案奖励仅分配给序列级别，无法区分不同令牌的功能角色，难以判断正确答案是否基于任务相关的视觉证据。

Method: 基于角色感知的令牌级信用分配，利用自蒸馏在线对比：感知令牌根据原始与损坏视觉输入下的视觉依赖性加权，推理令牌根据生成感知的一致性加权，并通过共享轨迹基线统一角色特定信号。

Result: 在多个多模态推理基准上，SRPO显著提升了基于证据的推理能力，验证了角色感知优化的重要性。

Conclusion: 超越统一的序列级信用分配，转向角色感知优化是实现可靠多模态推理的关键。
...
TL;DR: 提出结构化对手建模（SOM）框架，通过分离对手模型构建与预测，提升LLM在多智能体交互中的预测准确性和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将对手建模与预测纠缠，依赖隐式上下文推理，限制了动态交互中的适应性。

Method: 采用结构因果模型（SCM）构建对手模型，明确表示对手观察与行动间的因果依赖；在预测阶段，LLM沿SCM引导的清晰路径进行结构化推理。

Result: 在多个多智能体基准测试中，SOM一致优于最先进的LLM推理基线，实现更准确和适应性强的战略决策。

Conclusion: SOM框架有效提升LLM对手预测的准确性和稳定性，适用于复杂动态的多智能体环境。
...
### [45] [Signal Reshaping for GRPO in Weak-Feedback Agentic Code Repair](https://arxiv.org/abs/2605.07276)
*Jia Li,Yuxin Su,Ting Peng,Hailiang Huang,Yuetang Deng,Michael R. Lyu*

Main category: cs.AI

TL;DR: 本文针对代码智能体强化学习中反馈信号弱的问题，提出一种信号重塑方法，通过改进GRPO算法的奖励和过程信号，在编译修复任务中将严格精度从0.385提升至0.535。


<details>
  <summary>Details</summary>
Motivation: 代码智能体强化学习中的反馈信号往往只反映必要或表面条件，无法捕捉任务成功的语义谓词，导致训练信号不足。

Method: 在标准GRPO框架下，通过三层信号重塑：编译与语义分层奖励重塑轨迹排序、步骤级过程分数调整轨迹内更新强度、失败原因感知的轨迹治理增强组内可比性，同时保持GRPO的组归一化优势结构不变。

Result: 完整信号重塑的GRPO将严格编译和语义准确率从0.385提高到0.535；过程分数加权在分层奖励基础上将准确率从0.48提升至0.53，并减少评估步数；特权提示的token级蒸馏无法替代这些信号。

Conclusion: 信号重塑有效提升了代码智能体在弱反馈下的性能，分层奖励、过程信用分配和组内可比性是关键改进因素。
...
TL;DR: 本文提出了一种尺度条件评估协议，用于在证据保持增长下评估智能体记忆的可用性，发现可靠性损失并非单一现象，并支持将可扩展记忆声明条件化于智能体、接口、尺度范围和交互预算。


<details>
  <summary>Details</summary>
Motivation: 现有记忆智能体评估仅报告固定快照准确率或检索质量，未显示当无关会话积累时证据是否仍保持可用。

Method: 提出了尺度条件评估协议，为每个查询固定任务证据并添加无关会话，记录智能体-记忆轨迹，报告四种诊断指标：预算符合可靠性、尾部记忆调用负担、失效区分解和可用尺度边界。

Result: 在LongMemEval和LoCoMo上应用发现，HippoRAG在双调用预算下失去16-20个百分点可靠性；LiCoMemory的失败强烈依赖于智能体，Qwen3-8B超出预算而更大版本保持可靠。

Conclusion: 可靠性损失并非单一现象，支持将可扩展记忆声明条件化于智能体、接口、尺度范围和交互预算。
...
TL;DR: 本文提出ICR方法，利用在线策略滚动中最短正确响应的隐式压缩正则化，在不牺牲精度的情况下缩短LLM推理长度。


<details>
  <summary>Details</summary>
Motivation: 现有RL压缩方法存在精度下降或欠思考问题，且对长度-精度相关性的动态理解不足。

Method: 基于训练动态观测，提出ICR：从策略自生成轨迹组中提取最短正确响应作为虚拟短分布，施加正则化项引导策略生成简洁路径。

Result: 在多个推理基座和数学/知识基准上，ICR持续缩短响应长度且保持或提升精度，实现更优的精度-长度帕累托前沿。

Conclusion: ICR有效缓解过度思考，通过保持负长度-精度相关性避免欠思考，是一种鲁棒的推理压缩方法。
...
TL;DR: 提出一种基于大型语言模型的多智能体架构DoLQ，在符号回归中结合定性和定量评估，用于从观测数据中发现常微分方程，在多个基准测试上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有符号回归方法主要依赖定量指标，但真实微分方程建模需要融入领域知识以保证物理合理性，因此需要新的方法。

Method: 采用多智能体架构：采样代理提出动态系统候选，参数优化器精修方程以提高准确性，科学家代理利用LLM进行定性和定量评估并综合结果迭代引导搜索。

Result: 在多维常微分方程基准测试中，DoLQ相比现有方法取得更高成功率，并更准确地恢复真实方程的正确符号项。

Conclusion: DoLQ通过结合LLM的定性评估和传统定量优化，有效提升了微分方程发现的准确性和物理合理性。
...
TL;DR: 提出FlowAgent，将工具链重构为语义空间中的连续轨迹生成，用于演进式智能体推理。


<details>
  <summary>Details</summary>
Motivation: 现有基于步骤的范式缺乏全局视角，导致长程任务中错误累积，且难以泛化到未见工具。

Method: 利用条件流匹配生成连续潜在轨迹，实现全局规划视角，确保工具执行连贯鲁棒。

Result: 在首个面向动态真实环境的计划级闭环基准测试中，FlowAgent在长程推理任务上展现出优越的鲁棒性和适应性。

Conclusion: 连续公式化方法在理论上保证了泛化性和错误衰减，实践上超越了传统步骤式方法。
...
TL;DR: 提出CASPO框架，通过置信度感知的步骤级偏好优化对齐token置信度与逻辑正确性，并利用CaT动态剪枝不可靠推理分支，提升推理可靠性与效率。


<details>
  <summary>Details</summary>
Motivation: 现有对齐策略依赖外部验证器或大量采样，可扩展性差；大推理模型常通过有缺陷的中间步骤得到正确结果，存在推理可靠性缺口。

Method: 采用迭代直接偏好优化（不训练单独奖励模型）对齐token级置信度与步骤逻辑正确性；推理时使用置信度感知思想（CaT）动态剪枝不确定分支，开销O(V)。

Result: 在十个基准和多个模型族上持续提升推理可靠性与效率；在Qwen3-8B-Base上扩展并超越树搜索基线（AIME'24和AIME'25），无需奖励模型数据。

Conclusion: CASPO有效弥合了最终准确性与推理可靠性之间的差距，具有良好可扩展性，并发布了步骤级置信标注数据集以支持细粒度分析。
...
TL;DR: 提出了GraphReAct框架，通过图上的推理-行动循环，结合拓扑检索、语义检索和上下文精炼，实现图结构数据的逐步推理，在六个基准测试中性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有推理-行动框架主要针对文本数据，未充分探索图结构数据。图数据具有拓扑和语义双重结构，需要动态检索和上下文精炼，现有方法无法有效处理。

Method: 设计图基行动空间，包含拓扑检索（捕获局部结构依赖）、语义检索（获取非局部相关证据）和上下文精炼（压缩与重组累积信息），三种行动交替执行以逐步扩展和精炼推理上下文。

Result: 在六个基准数据集上，GraphReAct始终优于最先进方法，验证了推理-行动框架在图学习中的有效性。

Conclusion: 推理-行动范式可推广至图学习，通过动态检索与精炼能有效提升图推理性能。
...
TL;DR: 提出基于后验采样的策略优化（PSPO），通过在离线强化学习中利用贝叶斯推断量化模型保真度，实现泛化与鲁棒性的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于模型的离线强化学习方法在应对分布外样本时，通常采用过度悲观正则化，牺牲了泛化能力。

Method: 将动力学建模视为贝叶斯推断过程，结合后验采样与约束策略优化，利用动力学一致的分布外转换提升泛化性，同时确保鲁棒性。

Result: 在标准基准测试中，PSPO性能优于现有最先进基线。

Conclusion: PSPO通过后验采样和约束优化有效解决了离线强化学习中的泛化-鲁棒性权衡问题。
...
TL;DR: 研究在扩展描述逻辑（含逆角色、限定数量约束、特征比较）中，有界拟合方法学习概念的可行性、理论性质及SAT求解器实现，实验表明该方法具有实用性。


<details>
  <summary>Details</summary>
Motivation: 已有有界拟合方法在简单描述逻辑ALC中有效，但需探究其在更复杂、更具表达力的描述逻辑中是否仍保持PAC泛化等理论优势。

Method: 从理论上分析有界拟合在扩展描述逻辑中保持良好性质的条件；利用SAT求解器实现该方法。

Result: 与最先进的概念学习工具对比，该方法在实验中取得了有竞争力的结果，证明其能有效处理表达性强的概念学习任务。

Conclusion: 有界拟合可成功应用于表达性描述逻辑的概念学习，且基于SAT的实现是实用且有效的。
...
TL;DR: 提出OST框架，通过模拟单步更新估计边际效用解决合成数据质量与数量权衡问题，在数学推理任务中以更少计算量取得优于基线方法的效果。


<details>
  <summary>Details</summary>
Motivation: LMMs扩展受限于合成数据的质量-数量权衡，现有LLM裁判方法计算成本高且缺乏可解释性。

Method: 将数据选择转化为增量优化排序问题，通过轻量代理模型模拟单步更新来估计每个样本的边际效用。

Result: 在Qwen系列多模态数学推理基准上OST达到帕累托最优，使用前50%子集降低43%训练成本，性能超LLM裁判1.8分；固定预算下前20%子集比LLM裁判提升5.6分，比全量微调提升8.8分，且有效识别噪声样本。

Conclusion: 基于优化的数据选择方法可高效缓解合成数据的噪声问题，逆转复杂推理中的负迁移，比现有方法更优。
...
TL;DR: 提出了一种无监督微调的轻量级GUI智能体训练范式，通过引导式策略蒸馏和多解双层次GRPO框架，在2B/3B规模模型上实现与大规模模型竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级GUI智能体受限于模型容量，性能提升困难；传统监督微调会导致过拟合、灾难性遗忘和策略僵化。

Method: 提出基于引导式策略蒸馏的通用知识蒸馏方法，结合动态检索机制减少幻觉；建立多解双层次GRPO框架，联合优化宏观子任务规划和微观执行匹配；构建自动化数据生成管道。

Result: 在轻量级模型中达到最优性能，与大规模模型相比具有竞争力；消融实验证实结构化策略蒸馏和多解探索能突破传统模仿学习性能上限。

Conclusion: 所提方法有效解锁小模型潜力，为跨平台自动交互提供高效、轻量化的解决方案。
...
TL;DR: 提出模型驱动策略优化（MDPO），通过在可微规划中注入自适应噪声来改善非线性混合离散-连续域中的优化效果。


<details>
  <summary>Details</summary>
Motivation: 在高度非线性和混合离散连续域中，可微规划的优化目标往往存在病态条件，包括平坦区域和剧烈转变，阻碍有效优化。

Method: 在可微规划中向动作空间注入噪声，并基于梯度灵敏度动态调整噪声幅度，实现时间依赖的探索轮廓。

Result: 在基准域中，MDPO一致优于确定性可微规划和模型无关基线（如PPO），显著提高解质量。

Conclusion: 自适应噪声分配有助于逃离局部极值，在非线性混合领域实现更好探索与优化。
...
TL;DR: 提出了一种多目标逆合成规划算法MORetro*，能够生成帕累托前沿，在成本、可持续性、毒性和产率等多个标准间进行权衡。


<details>
  <summary>Details</summary>
Motivation: 现有合成规划方法仅聚焦单一可行性路径，未考虑实际化学实践中需平衡多个竞争目标的需求。

Method: 将合成规划建模为多目标搜索问题，采用加权标量化和贝叶斯优化引导采样，基于多目标A*搜索实现最优性保证。

Result: 在多个逆合成基准上生成多样化、高质量的帕累托前沿，发现单目标方法无法识别的方案。

Conclusion: MORetro*能更好地对齐计算机辅助合成规划与实际工业决策，提供更全面的权衡选择。
...
TL;DR: 本文研究了多环境POMDP（MEPOMDP）中有限时域目标的最优值和策略计算问题，证明其PSPACE完全性，并提出了一个比现有算法更高效的实用算法。


<details>
  <summary>Details</summary>
Motivation: 现有POMDP研究未考虑初始状态被对抗性选择的多环境场景，且该问题在POMDP中已知为PSPACE完全，但MEPOMDP的复杂性未明确。

Method: 通过理论归约证明MEPOMDP的PSPACE完全性，并设计了一个实用算法，在经典基准测试上进行了评估。

Result: MEPOMDP中的问题复杂度为PSPACE完全，新算法显著优于唯一已知的先前算法。

Conclusion: MEPOMDP与POMDP的复杂度相同（PSPACE完全），且提出的算法在实际应用中表现更好。
...
TL;DR: 提出了一个名为GSR的开放式贝叶斯优化框架，它能动态生成并优化任务，适用于科学工作流。


<details>
  <summary>Details</summary>
Motivation: 在科学工作流中，任务本身（优化什么和如何评估）会随证据积累而演化，现有的贝叶斯优化忽略了这一不确定性来源。

Method: 采用生成-选择-优化（GSR）框架，从用户提供的种子任务开始，通过粗到细的方式生成新任务，并使用任务采集函数调度优化过程。

Result: 在渐近情况下，GSR能将评估集中在最佳任务上，相对于单任务贝叶斯优化仅增加对数后悔值。在新产品开发、化学合成缩放、算法分析和专利再利用等任务中表现优于基于LLM的优化器。

Conclusion: GSR框架有效处理了任务演化带来的不确定性，并展现出在多种实际应用中的优越性能。
...
### [60] [From Pixels to Prompts: Vision-Language Models](https://arxiv.org/abs/2605.07544)
*Khang Hoang Nhat Vo*

Main category: cs.AI

TL;DR: 本书旨在为视觉-语言模型（VLMs）提供一个清晰的理解框架，而非详尽的技术目录，帮助读者建立自信阅读新论文和设计系统所需的结构与直觉。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型领域发展迅速，术语和新模型层出不穷，读者容易在‘知道流行词’和‘真正理解原理’之间感到困惑。作者基于自身经历，希望填补这一认知鸿沟。

Method: 通过构建一个‘心智地图’来阐述视觉-语言模型的核心结构，重点在于提供概念框架和直觉理解，而非罗列所有数据集、基准或模型变体。

Result: 本书提供了一种持久的、可迁移的理解方式，使读者能自信地阅读新论文，并像‘有意识地组装积木’一样设计自己的系统。

Conclusion: 视觉-语言模型的核心价值在于整合视觉与语言能力，而理解其本质比追踪所有细节更重要。本书旨在赋予读者结构化知识和直觉，以应对领域的快速变化。
...
### [61] [Parallel Lifted Planning via Semi-Naive Datalog Evaluation](https://arxiv.org/abs/2605.07584)
*Dominik Drexler,Oliver Joergensen,Jendrik Seipp*

Main category: cs.AI

TL;DR: 本文提出一种基于Datalog评估的两级并行（规则级和基化级）执行模型，并扩展半朴素Datalog评估以支持基于团枚举的规划专用基化器，实验表明单核性能已优于基线，多核加速比最高达6倍。


<details>
  <summary>Details</summary>
Motivation: 提升提升式经典规划器的效率，克服传统提升规划在搜索中反复实例化地面结构导致的性能瓶颈。

Method: 设计两级并行执行模型（规则级并行与基化并行），结合基于团枚举的基化器，支持半朴素Datalog评估，用于规划中的后继生成、公理评估、任务基化和删除松弛启发式。

Result: 在单核上解决的任务数已超过基线，多核时性能差距进一步扩大；在难基化任务中，Datalog执行平均并行占比92.4%，8核时实际加速比达6倍。

Conclusion: 所提并行Datalog执行模型能有效加速提升式经典规划，尤其在多核和硬基化任务上优势显著。
...
### [62] [Inference Time Causal Probing in LLMs](https://arxiv.org/abs/2605.07631)
*Sadegh Khorasani,Saber Salehkaleybar,Negar Kiyavash,Matthias Grossglauser*

Main category: cs.AI

TL;DR: 提出一种无需探针的因果干预方法HDMI，通过梯度优化直接操控模型隐藏状态，在保持流畅性的同时改变目标属性，并在多个模型和基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有因果探测方法依赖训练辅助探针分类器，这会导致方法受限于特定任务或模型，且可能与模型的预测几何结构不一致。

Method: HDMI使用模型原生输出和边际目标函数，直接通过梯度修改隐藏状态，无需探针；LA-HDMI变体通过反向传播到softmax嵌入，实现文本编辑中的目标词概率提升。

Result: 在LGD语料库和CausalGym基准上，HDMI在Meta-Llama-3-8B-Instruct和Pythia-70M中均实现了比先前方法更高的可靠性（完成度与选择性的调和平均）。

Conclusion: HDMI是一种有效且可靠的因果干预方法，避免了探针带来的偏差，适用于多种生成模型和任务。
...
### [63] [Learning to Communicate Locally for Large-Scale Multi-Agent Pathfinding](https://arxiv.org/abs/2605.07637)
*Valeriy Vyaltsev,Alsu Sagirova,Anton Andreychuk,Yuri Kuratov,Konstantin Yakovlev,Aleksandr Panov,Alexey Skrynnik*

Main category: cs.AI

TL;DR: 本文提出LC-MAPF，一种基于局部通信的预训练模型，通过多轮邻居间特征共享提升多智能体路径规划的协作效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的MAPF求解器缺乏有效协作机制，且通信型方法常面临可扩展性瓶颈，需设计兼顾协作与可扩展性的新方法。

Method: 将单智能体视角的MAPF建模为Dec-POMDP，引入可学习的局部通信模块，通过多轮邻居间信息交换增强协调。

Result: 在多种未见过的测试场景中，LC-MAPF在多种指标上优于现有基于IL和RL的MAPF求解器，且通信机制不影响其可扩展性。

Conclusion: 局部通信能有效提升多智能体路径规划性能，且可保持高可扩展性。
...
### [64] [Tacit Knowledge Extraction via Logic Augmented Generation and Active Inference](https://arxiv.org/abs/2605.07639)
...
TL;DR: 本文提出一种神经符号框架，结合逻辑增强生成与主动推断方法，实现从视频中构建本体驱动的知识图谱，提升工业领域隐性知识的捕获与重用。


<details>
  <summary>Details</summary>
Motivation: 隐性知识在人类专家经验中至关重要，但难以形式化、机器可解释化，尤其在过程性领域（如制造装配）中，知识工程方法无法有效处理此类隐含、上下文约束及经验判断。

Method: 采用逻辑增强生成（Logic-Augmented Generation）和主动推断（Active-Inference）方法，构建本体驱动的知识图谱，实现过程性隐性知识的自动转化。

Result: 在制造装配维修视频案例中，该框架显著提升了知识图谱的完整性和语义质量。

Conclusion: 该研究推动了工业领域神经符号知识工程的发展，为隐性知识的捕获与重用提供了有效方案。
...
### [65] [FactoryBench: Evaluating Industrial Machine Understanding](https://arxiv.org/abs/2605.07675)
...
TL;DR: 提出了FactoryBench基准，用于评估时间序列模型和LLM在工业机器人遥测数据上的机器理解能力，包含7万多个问答对和五种格式，发现前沿LLM在结构化推理和决策任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对工业机器人遥测数据深入机器理解能力的评估，尤其是因果推理和决策层面。

Method: 基于Pearl的因果阶梯，设计四个因果层次的问答对，采用结构化模板生成问题，并用确定性评分和LLM投票协议评估答案。

Result: 六个前沿LLM在零样本测试中，结构化层级得分不超过50%，决策任务不超过18%。

Conclusion: 当前模型与工业机器理解能力之间存在巨大差距，需要进一步改进。
...
### [66] [GASim: A Graph-Accelerated Hybrid Framework for Social Simulation](https://arxiv.org/abs/2605.07692)
*Xuan Zhou,Yanhui Sun,Hantao Yao,Allen He,Yongdong Zhang,Wu Liu*

Main category: cs.AI

TL;DR: GASim是一个图加速的混合多智能体框架，通过图优化记忆、图消息传递和信息熵驱动分组，实现了大规模社交模拟中9.94倍的端到端加速，同时将令牌消耗降低至基线20%以下。


<details>
  <summary>Details</summary>
Motivation: 现有混合模拟框架存在高延迟问题，主要源于昂贵的LLM记忆检索和顺序ABM执行，限制了大规模社交模拟的可扩展性。

Method: 提出GASim框架，包含三个创新：图优化记忆（GOM）用轻量级传播替代LLM检索；图消息传递（GMP）用并行GAT更新替代顺序ABM；熵驱动分组（EDG）动态识别核心智能体以协调混合分区。

Result: 实验表明，GASim实现9.94倍端到端加速，令牌消耗<20%，保持与真实舆论趋势的高度一致性。

Conclusion: GASim有效降低延迟和成本，为大规模混合社交模拟提供了高效解决方案。
...
TL;DR: 本文研究了利用大语言模型（LLM）为层次任务网络（HTN）规划生成搜索启发式的方法，在六个标准领域上，LLM生成的启发式几乎达到了最佳HTN规划器的覆盖率，并在83%的共同问题上显著降低了搜索代价。


<details>
  <summary>Details</summary>
Motivation: HTN规划中的现有启发式不如经典规划中的有效，且需要一种新方法来生成更高效的搜索启发式，以加快规划求解速度。

Method: 采用Pytrich规划器，对九个LLM在领域特定提示下生成的启发式进行测试，并与TDG、LMCount等基线及PANDA规划器进行对比评估。

Result: LLM生成的启发式在覆盖率上接近最佳HTN规划器，同时在83%的共享问题上大幅减少了搜索努力。

Conclusion: LLM能够有效生成HTN规划的搜索启发式，显著提升搜索效率，并可能成为该领域的一种新方法论扩展。
...
TL;DR: 本文提出了一种基于路径签名的在线目标识别方法，通过紧凑且富有表现力的轨迹表示，显著提升了预测准确性和在线规划效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理连续域中大规模轨迹编码和比较时，忽视了其他领域成熟的编码技术，导致效率低下。

Method: 利用粗糙路径理论中的路径签名，对轨迹进行紧凑、语义丰富的特征提取，并在此基础上进行目标比较。

Result: 在在线场景中，该方法在预测准确性和规划效率上持续优于现有技术，离线性能与最新方法相当。

Conclusion: 路径签名是一种有效的连续域轨迹编码工具，能显著改进在线目标识别任务。
...
TL;DR: 提出了一种迭代细化框架，将目标分配与路径规划解耦，利用LaCAM等MAPF求解器，通过反馈驱动调整分配，实现大规模TAPF的高效求解。


<details>
  <summary>Details</summary>
Motivation: 现有基于CBS的TAPF方法将目标分配与路径规划紧密耦合，计算量大、可扩展性差，难以应对大规模实际问题。

Method: 构建迭代细化框架：在时间预算内，反复使用快速MAPF求解器为当前分配求解路径，通过反馈识别瓶颈智能体，并优化分配。

Result: 实验表明，反馈驱动的重分配循环有效，框架在可扩展性上远超现有CBS方法，同时保持较好的解质量。

Conclusion: 该工作向实用化、大规模TAPF迈出了坚实一步，适合实际场景。
...
TL;DR: 提出RuleSafe-VL基准，用于评估视觉语言内容审核中的规则条件决策推理。


<details>
  <summary>Details</summary>
Motivation: 现有多模态安全基准仅匹配预定义标签，不测试底层的规则结构，无法反映模型是否正确应用政策或仅依据表面线索。

Method: 从平台政策中提取93条原子规则和92种类型规则关系，构建2166个敏感图像-文本案例，设计四项诊断任务分解决策链。

Result: 规则关系恢复是主要瓶颈，最佳模型Macro-F1仅64.8%，部分安全模型低于7%；决策状态预测仅64.5%。

Conclusion: RuleSafe-VL将审核评估从最终标签评分转向规则条件决策推理的诊断性评估。
...
TL;DR: 本文提出了一种将正则约束与变阶马尔可夫模型结合的方法，通过构建稀疏乘积状态空间实现精确推理，解决了变阶模型与约束状态空间不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的精确方法只能处理一阶马尔可夫链，而实际应用中变阶/回退模型更常见，需要扩展正则约束处理能力。

Method: 用观测上下文状态替换一阶马尔可夫状态，与正则约束自动机进行标准乘积构造，得到稀疏乘积状态空间，并区分精确BP推理与生成时的回退策略。

Result: 对于固定上下文图和自动机，推理复杂度是序列视界的线性时间；一般情况下是可达乘积边数的多项式时间。

Conclusion: 该方法无需展开所有K元组即可得到满足正则约束的正确变阶分布，并通过逆计数查找支持可逆数据增强。
...
TL;DR: 提出了AgentEscapeBench基准，用于测试LLM智能体在长距离依赖约束下执行工具推理的能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在熟悉场景和短距离交互中表现良好，但在长距离、新工具使用过程中的推理能力缺乏系统评估。

Method: 构建了包含有向无环依赖图的逃生室风格任务，要求智能体调用真实函数、追踪隐藏状态、传播中间结果并提交可验证答案；涵盖270个实例和5个难度等级。

Result: 人类和最佳模型在深度增加时成功率显著下降（人类从98.3%降至80.0%，最佳模型从90.0%降至60.0%）；失败主要源于长距离状态追踪、线索遵循和中间结果传播的失误。

Conclusion: 当前智能体擅长局部工具使用，但在深度上下文依赖处理上仍有不足；AgentEscapeBench可作为诊断工具，指导未来训练以提升通用推理和适应能力。
...
TL;DR: 提出 TraceFix，一种基于验证先行的 LLM 多智能体协调流水线，通过 TLA+ 模型检查确保协议正确性，在 48 个任务上全部通过验证，其中 62.5% 一次通过，运行时监控显著提升任务完成率并减少死锁。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 多智能体系统在协议协调中容易出现死锁、活锁等错误，缺乏形式化验证保障。

Method: 从任务描述合成协议拓扑 IR，生成 PlusCal 逻辑，利用 TLA+ 模型检查器迭代修复反例至验证通过，编译验证后的进程体为系统提示并运行时监控。

Result: 48 个任务全部通过 TLC 验证，62.5% 首次通过；运行时监控下任务完成率 89.4%（完全完成 81.5%）；死锁/活锁从 31.1% 降至 14.1%。

Conclusion: 基于形式化验证的协议可以大幅提升 LLM 多智能体协调的可靠性与鲁棒性，尤其在高故障注入场景下优势明显。
...
TL;DR: 提出PACS算法，利用LLM和形式求解器对个体常识信念进行概率采样并聚合结论，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设常识信念普遍一致，但现实中个体认知存在差异。

Method: 利用LLM和形式求解器采样不同个体常识信念的推理路径，并进行概率聚合。

Result: 在多个基准测试中，PACS优于思维链、先前神经符号方法和基于搜索的方法。

Conclusion: 概率建模个体常识信念差异可显著提升溯因推理的鲁棒性和准确性。
...
TL;DR: 提出了一个两阶段资源分配框架，最优策略是在算法分配的边缘进行筛查，直接瞄准最高风险单元，并实证描述了筛查与算法分配的互补或替代关系。


<details>
  <summary>Details</summary>
Motivation: 当前基于预测风险评分的算法分配虽便宜快捷，但不可约的随机不确定性仍会导致资源错配，需要结合传统筛查来优化分配效率。

Method: 构建两阶段分配框架：第一阶段对部分单元进行筛查（直接观察真实结果），第二阶段在固定覆盖预算下进行最终分配，数学推导最优策略并用实证分析。

Result: 最优策略是在算法分配边界上筛查，同时直接瞄准最高风险单元；效率提升随人群随机不确定性增加而增大。

Conclusion: 筛查与算法分配可以是互补或替代关系，在收入社会保护和哥伦比亚人道排雷中应用展示了其操作意义。
...
TL;DR: 本文提出σ-Reveal推理时机制和Action Advantage Assignment (A³)方法，用于改进CLI代理在信息提取和文件编辑任务中的选择性观察和信用分配问题，并通过ShellOps数据集验证。


<details>
  <summary>Details</summary>
Motivation: CLI代理面临两大瓶颈：从大规模代码库中辨识任务相关证据，以及将稀疏奖励分配给长多轮轨迹中的动作。现有方法未充分利用CLI动作的结构化属性。

Method: σ-Reveal选择令牌预算的上下文；A³构建回合级优势、AST子链残差和树级轨迹边际。

Result: 在ShellOps数据集上的实验表明，该方法能有效提升CLI任务的性能。

Conclusion: 结构化奖励分配和选择性观察能显著改进CLI代理的多轮交互能力。
...
TL;DR: 前沿大语言推理模型在复杂游戏任务中比强化学习模型更接近人类行为，并能更好预测脑活动。


<details>
  <summary>Details</summary>
Motivation: 探究现代AI系统能否像人类一样快速学习抽象知识并灵活运用进行规划和行动。

Method: 使用人类玩复杂视频游戏时的行为数据和fMRI脑成像数据，比较前沿大语言推理模型（LRMs）、无模型/基于模型的深度强化学习智能体以及贝叶斯理论智能体的表现。

Result: LRMs在游戏发现阶段最接近人类行为模式，且预测脑活动能力比强化学习模型高一个数量级，该效应经置换检验稳健。

Conclusion: LRMs是解释人类在复杂自然环境中学习与决策的有力计算模型，其与脑活动的一致性源于对游戏状态的上下文表示，而非下游规划或推理。
...
TL;DR: 提出MPD²-Router框架，用于多专家眼科学分诊，通过掩膜感知的Gumbel-Sigmoid门控、不对称成本损失和正则化项，在青光眼筛查中实现性能与专家负载的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有学习型分诊方法忽略专家可用性、行为差异、负载不均衡、不对称诊断危害及形态学与部署偏移，导致在资源受限的临床环境可靠性不足。

Method: 双头分诊/分配策略结合掩膜感知门控；不对称成本敏感训练、增广拉格朗日预算约束、群体分布先验及秩最大化JS正则化。

Result: 在三个跨国青光眼数据集上，相比纯AI，以中等分诊率降低临床成本并提升MCC；在F1-MCC-成本上达到帕累托最优，跨域鲁棒，专家负载均衡。

Conclusion: MPD²-Router通过考虑多专家约束和临床成本，有效提升了AI辅助青光眼筛查的安全性与实用性。
...
TL;DR: 将奖励分解为可验证的加权标准并使用LLM法官评分，提供了部分信用优化信号，形式化基于标准的基础强化学习框架，在OSTI语料库上训练Llama-3.1-8B-Instruct模型，取得了71.7%的标准化奖励，并在多个推理基准上提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统二元结果或单一整体评分无法提供细粒度反馈，需要更结构化的奖励信号来引导策略优化。

Method: 将奖励分解为多个任务特定标准，利用冻结LLM法官基于辅助基础信息评分，采用GRPO算法优化策略。

Result: 训练模型在保留数据集上获得71.7%标准化奖励，并在GSM8K、MATH等独立推理基准上超越基础模型。

Conclusion: 结构化的文档基础奖励能提升基准指标，并诱导出超越训练语料的可迁移推理能力。
...
TL;DR: 提出VecCISC框架，通过语义相似性过滤冗余推理链，减少置信度评分所需的LLM调用，在保持或提升准确率的同时降低47%的令牌消耗。


<details>
  <summary>Details</summary>
Motivation: 加权多数投票（如CISC）需调用批评LLM对每个候选答案评分，导致高昂的计算开销，限制了其实际应用。

Method: 利用语义相似性度量，过滤与其它等价、退化或幻觉的推理链，减少需要评分的候选答案数量。

Result: 在数学、化学、生物学等五个数据集上，令牌使用量减少47%，准确率不低于或优于CISC。

Conclusion: VecCISC能有效降低加权多数投票的成本，同时保持或提升推理性能，是一种实用的轻量级替代方案。
...
### [81] [Edge Deep Learning in Computer Vision and Medical Diagnostics: A Comprehensive Survey](https://arxiv.org/abs/2605.06714)
*Yiwen Xu,Tariq M. Khan,Yang Song,Erik Meijering*

Main category: cs.CV

TL;DR: 综述了边缘深度学习在计算机视觉尤其是医疗诊断中的最新进展，涵盖基础原理、硬件分类、模型部署方法及应用案例。


<details>
  <summary>Details</summary>
Motivation: 边缘计算与深度学习融合可实现实时决策，但系统综述其在医疗视觉领域的应用尚缺，故需梳理现状与挑战。

Method: 提出了基于性能和场景的边缘硬件新分类，梳理了轻量设计、模型压缩等边缘部署方法，并回顾了视觉与医疗诊断应用。

Result: 边缘深度学习在医疗诊断等领域展现出显著实际影响力，但面临硬件限制、模型优化等障碍。

Conclusion: 边缘深度学习是推动边缘计算应用的关键，未来需进一步研究智能解决方案以克服现有挑战。
...
### [82] [Visual Text Compression as Measure Transport](https://arxiv.org/abs/2605.06708)
*Lv Tang,Tianyi Zheng,Yang Liu,Bo Li,Xingyu Li*

Main category: cs.CV

TL;DR: 本文从最优传输角度形式化视觉文本压缩中的信息损失，提出无标签路由准则和基于传输成本的注视机制，在24个NLP数据集上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 视觉文本压缩的token节省量无法预测下游任务表现，缺少衡量视觉编码导致的与任务相关信息损失的理论量。

Method: 将文本和视觉token视为经验概率测度，推导ViT补丁编码器的推前映射，其传输成本分解为补丁内聚合精度成本和补丁间碎片化覆盖成本，并设计无标签路由准则和注视机制。

Result: 在Qwen3-4B上，无标签规则在24个数据集中17个(70.8%)匹配基于最优选择的oracle，平均任务得分提升3.3%，token数减少10.3%。

Conclusion: 基于最优传输的分析框架能有效预测和缓解视觉文本压缩中的信息损失，提升效率与任务表现的平衡。
...
### [83] [HumanNet: Scaling Human-centric Video Learning to One Million Hours](https://arxiv.org/abs/2605.06747)
*Yufan Deng,Daquan Zhou*

Main category: cs.CV

TL;DR: HumanNet 是一个百万小时规模的人中心视频语料库，涵盖第一人称和第三人称视角的精细活动、人机交互和长周期行为，并提供了交互为中心的标注，旨在为具身智能提供可缩放的数据基础设施。


<details>
  <summary>Details</summary>
Motivation: 现有视觉和语言模型可从互联网语料缩放学习，但物理交互学习受限于缺乏大规模、多样化和丰富标注的人类活动数据，机器人数据获取成本高且规模有限。

Method: 通过系统化数据策展范式，包括人中心过滤、时间结构化、视角多样性和标注增强，将非结构化互联网视频转化为可缩放的表示学习、活动理解、运动生成和人机迁移基础。

Result: 在固定验证集下，使用 HumanNet 中 1000 小时第一人称视频对 Qwen VLM 进行持续训练，性能优于使用 100 小时 Magic Cobot 真实机器人数据训练，表明第一人称人类视频可作为可缩放的机器人数据替代品。

Conclusion: HumanNet 展示了以人为中心的视频可替代机器人数据来缩放具身基础模型，为具身智能提供了更可扩展、低成本的训练途径。
...
### [84] [R$^3$L: Reasoning 3D Layouts from Relative Spatial Relations](https://arxiv.org/abs/2605.06758)
*Zhifeng Gu,Yuqi Wang,Bing Wang*

Main category: cs.CV

TL;DR: 提出R³L框架，通过不变空间分解和一致空间想象提升3D布局生成中相对空间推理的可靠性和一致性。


<details>
  <summary>Details</summary>
Motivation: 多跳空间推理中参考帧变换累积误差导致语义和度量漂移，削弱MLLM推断关系的可靠性。

Method: ①不变空间分解拆解耦合关系链；②一致空间想象通过想象-修正循环促进自一致性；③支持性空间优化采用全局到局部坐标重参数化。

Result: 在多样化场景和指令中生成更物理可行且语义一致的布局，帧引入的不一致性是关键瓶颈。

Conclusion: 解决帧引入的不一致性对可靠多跳相对空间推理至关重要，代码已开源。
...
### [85] [LookWhen? Fast Video Recognition by Learning When, Where, and What to Compute](https://arxiv.org/abs/2605.06809)
*Ali Salamatian,Anthony Fuller,Pritam Sarkar,James R. Green,Leonid Sigal,Evan Shelhamer*

Main category: cs.CV

TL;DR: 提出一个选择-提取框架LookWhen，通过选择视频中的关键token来降低计算成本，在保持精度的同时显著提速。


<details>
  <summary>Details</summary>
Motivation: 现有视频识别方法将视频分割成token，处理成本随token数超线性增长，而视频存在大量冗余，因此需要更高效的方法来减少不必要的计算。

Method: 使用浅层选择器对缩小后的视频快速评分，选出时空维度上最独特的K个token；深层提取器仅处理这些选定token，通过邻近距离进行选择预训练，并通过蒸馏视频和图像教师进行提取预训练。

Result: 在6个数据集上的12种设置中，有9种实现准确率-计算量的帕累托最优，剩余3种接近；实际吞吐量上比InternVideo2-B快6.7倍且精度相当。

Conclusion: LookWhen通过选择性计算有效平衡了视频识别的准确性和效率，优于现有高效模型和升级基线。
...
### [86] [Knowledge Transfer Scaling Laws for 3D Medical Imaging](https://arxiv.org/abs/2605.06859)
*Ho Hin Lee,Dongna Du,Chu Wang,Yuankai Huo,Shi Gu,James C. Gee,Yifan Wu*

Main category: cs.CV

TL;DR: 提出了一种基于缩放定律的数据分配优化方法，用于异质医学影像（CT、MRI、PET）的3D基础模型预训练，发现跨域迁移不对称且遵循幂律规律，并推导出枢纽-岛屿结构实现高效分配。


<details>
  <summary>Details</summary>
Motivation: 当前多模态医学影像预训练中的数据混合策略主要依赖启发式方法，缺乏对跨域知识迁移不对称性和缩放规律的理论指导。

Method: 通过观察不同医学影像模态在预训练中的缩放速率和跨域迁移不对称性（遵循幂律），将数据分配建模为缩放定律优化问题，求解得到枢纽-岛屿分配结构。

Result: 迁移感知分配相比数据比例采样提升高达58%，在不同预算下泛化良好（r=0.989），下游任务（分类、分割）验证了预训练表示更优。

Conclusion: 考虑跨域迁移不对称性的数据分配策略能显著提升3D医学影像基础模型的预训练效果，且具有可解释性和泛化能力。
...
### [87] [AdpSplit: Error-Driven Adaptive Splitting for Faster Geometry Discovery in 3D Gaussian Splatting](https://arxiv.org/abs/2605.06876)
*Yongjae Lee,Jingxing Li,Abhay Kumar Yadav,Rama Chellappa,Deliang Fan*

Main category: cs.CV

TL;DR: 提出AdpSplit算子，通过误差驱动的自适应分裂机制替代3DGS中的固定分裂算子，在减少训练时间的同时保持渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS中固定数量的随机分裂导致训练效率低，成为加速训练管道的瓶颈。

Method: 基于L1像素误差区域统计自适应确定分裂子代数量并初始化参数。

Result: 在多个数据集上降低训练时间9.2%-22.3%，结合FastGS实现12.6倍加速，PSNR持平全训练计划。

Conclusion: AdpSplit可作为标准分裂的简单替代，有效提升3DGS训练效率且不损失质量。
...
### [88] [TriDE: Triangle-Consistent Translation Directions for Global Camera Pose Estimation](https://arxiv.org/abs/2605.06889)
*Francisco Chen,Yiran Wang,Yunpeng Shi*

Main category: cs.CV

TL;DR: 本文提出TriDE方法，利用相机三角形一致性作为高阶验证信号，通过消息传递机制联合优化成对平移方向，显著提升方向估计精度和相机位置重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有运动恢复结构方法中，成对平移方向独立估计导致全局不一致，影响相机位置估计精度。

Method: 提出TriDE，基于加权三角形一致性进行消息传递，在方向与三角形之间传播信息以细化不可靠的成对方向，避免全局非线性优化。

Result: 在真实图像图上，方向精度大幅提升，下游相机位置估计更准确，并在随机噪声模型下建立了精确恢复的相变边界。

Conclusion: TriDE有效连接了局部成对估计与全局相机几何，为结构从运动提供了实用解决方案。
...
### [89] [Towards Fairness under Label Bias in Image Segmentation: Impact, Measurement and Mitigation](https://arxiv.org/abs/2605.06891)
*Aditya Parikh,Stella Frank,Sneha Das,Aasa Feragen*

Main category: cs.CV

TL;DR: 提出了一种数据中心的自信学习适应方法，用于检测图像分割中的标签偏差，无需干净的无偏标注。


<details>
  <summary>Details</summary>
Motivation: 标签偏差在图像分割中未被充分探索，现有方法依赖干净无偏标注，而这些标注通常难以获得。

Method: 通过比较训练标签与模型的置信预测，隔离方向性误差以量化偏差的存在和性质，并利用编码器特征空间中偏差导致的子群可分性进行缓解。

Result: 在三个数据集上（从合成到真实偏差），该方法可靠地检测和缓解了偏差，实现了公平的性能。

Conclusion: 该方法无需干净标签即可检测和缓解分割中的标签偏差，促进了分组公平性。
...
### [90] [Not All Tokens Need 40 Steps: Heterogeneous Step Allocation in Diffusion Transformers for Efficient Video Generation](https://arxiv.org/abs/2605.06892)
*Ernie Chu,Vishal M. Patel*

Main category: cs.CV

TL;DR: 提出了异构步分配(HSA)方法，根据速度动态为不同时空token分配不同步数，显著减少计算成本并保持视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型统一为所有token分配相同步数，导致巨大计算开销，而人类视觉可忽略冗余运动。

Method: 训练无关的推理算法HSA，基于速度动态分配步预算，通过KV缓存同步允许活跃token访问全序列，并推导缓存欧拉更新跳过非活跃token的模型评估。

Result: 在Wan-2和LTX-2模型上，HSA在50%和25%运行时显著优于先前缓存方法和Flow Matching基线，实现质量-运行时帕累托前沿。

Conclusion: HSA无需离线特征分析即可在严格计算预算下保持生成质量和结构完整性。
...
### [91] [Advancing Reliable Synthetic Video Detection: Insights from the SAFE Challenge](https://arxiv.org/abs/2605.06912)
...
TL;DR: 本文介绍了在ICCV 2025上举办的SAFE合成视频检测挑战赛，包含两个任务：检测多种生成模型合成的视频，以及检测经过后处理（如缩放、重压缩、运动模糊）的合成视频。比赛有12个团队提交超过600个方案，数据集涵盖13种高质量合成模型和21种真实视频来源，总时长20小时。


<details>
  <summary>Details</summary>
Motivation: 生成视频技术的普及需要可靠的合成媒体检测与表征方法，以应对真实与合成视频的区分挑战。

Method: 通过组织挑战赛，设计两个检测任务，构建包含13种合成模型和21种真实视频来源的数据集（6000个样本，20小时），在Hugging Face平台进行完全盲评估。

Result: 展示了跨生成器泛化方面的可衡量进展，但存在对后处理伪影的持续脆弱性。

Conclusion: 当代合成视频检测方法在泛化能力上有所提升，但仍难以应对后处理操作，提示未来研究需关注鲁棒性改进。
...
### [92] [A$^2$RD: Agentic Autoregressive Diffusion for Long Video Consistency](https://arxiv.org/abs/2605.06924)
*Do Xuan Long,Yale Song,Min-Yen Kan,Tomas Pfister,Long T. Le*

Main category: cs.CV

TL;DR: 提出A$^2$RD架构，通过自我改进的闭环循环过程，在长视频合成中实现一致性和连贯性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有长视频合成方法存在语义漂移和叙事崩溃问题，难以保持长期一致性。

Method: 采用Agentic自回归扩散架构，通过多模态视频记忆、自适应片段生成和分层测试时自我改进实现片段级合成与优化。

Result: 在公开基准和LVBench-C上，一致性和叙事连贯性分别提升30%和20%，人类评估证实运动和平滑度改善。

Conclusion: A$^2$RD有效解耦创作与一致性，为长视频合成提供了鲁棒解决方案。
...
### [93] [XiYOLO: Energy-Aware Object Detection via Iterative Architecture Search and Scaling](https://arxiv.org/abs/2605.06927)
*Tony Tran,Richie R. Suganda,Bin Hu*

Main category: cs.CV

TL;DR: 提出了一种能量自适应框架XiYOLO，通过结合搜索空间、两级能量估计器和迭代搜索，在异构边缘设备上实现高效的物体检测，显著降低了功耗。


<details>
  <summary>Details</summary>
Motivation: 现有能量感知NAS方法部署场景有限，且实际能量优化因设备依赖性强、测量成本高而困难。

Method: 采用能量自适应框架，包括XiResOFA搜索空间、两级能量估计器和迭代搜索；通过复合缩放生成XiYOLO系列。

Result: 在GPU和NPU上相比YOLOv12能耗降低20%-53.7%；两级估计器在少样本下比联合预测器更高效。

Conclusion: XiYOLO实现了更强的能量-精度权衡，适用于异构边缘设备部署。
...
### [94] [Bringing Multimodal Large Language Models to Infrared-Visible Image Fusion Quality Assessment](https://arxiv.org/abs/2605.06969)
*Yuchen Guo,Junli Gong,Yao Lu,Xintong Xu,Yiuming Cheung,Weifeng Su*

Main category: cs.CV

TL;DR: 提出了FuScore，利用多模态大语言模型输出连续质量分数来评估红外-可见光图像融合效果，并引入了三元组损失进行训练。


<details>
  <summary>Details</summary>
Motivation: 现有的IVIF评估方法要么过度优化手工设计的无参考指标，要么使用聚合分数进行标量回归，忽略了MLLM的推理能力和逐幅图像的感知模糊性，导致对质量相近的融合图像区分能力不足。

Method: 利用MLLM生成连续质量分数而非离散等级；利用四个子维度的一致性构建软标签；结合逐幅图像的分布监督、源图像对内的瑟斯通保序性（方法级排序）和跨场景的瑟斯通保序性（场景级排序）的三元组目标进行训练。

Result: FuScore与人类视觉偏好达到了最优的相关性。

Conclusion: FuScore通过连续分数和三元组监督有效地提升了融合图像质量的评估准确性，尤其是在细粒度区分方面。
...
### [95] [TRAJGANR: Trajectory-Centric Urban Multimodal Learning via Geospatially Aligned Neural Representations](https://arxiv.org/abs/2605.06990)
*Maria Despoina Siampou,Gengchen Mai,Ni Lao,Jinmeng Rao,Neha Arora,Cyrus Shahabi,Shushman Choudhury*

Main category: cs.CV

TL;DR: 提出TrajGANR框架，将连续的人类移动轨迹与静态地理位置图像进行细粒度对齐，实现多模态自监督学习。


<details>
  <summary>Details</summary>
Motivation: 现有地理空间多模态自监督学习方法主要针对静态配对模态，无法处理连续移动轨迹，而轨迹对城市理解至关重要但未被充分探索。

Method: 通过学习轨迹的连续神经表示，实现任意路径点与街道视图图像的细粒度对齐，并联合对齐轨迹、街道图像和地理位置三种模态。

Result: 在四个城市移动和道路理解任务上，TrajGANR持续优于现有地理空间多模态自监督学习框架和轨迹专用基础模型。

Conclusion: 细粒度地理空间对齐和多模态学习是性能提升的主要驱动力，突出了其相较于粗粒度聚合和传统方法的优势。
...
### [96] [LensVLM: Selective Context Expansion for Compressed Visual Representation of Text](https://arxiv.org/abs/2605.07019)
*Roy Xie,Dan Friedman,Donghan Yu,Bowen Pan,Christopher Fifty,Jang-Hyun Kim,Xianzhi Du,Zhe Gan,Vivek Rathod,Bhuwan Dhingra*

Main category: cs.CV

TL;DR: 提出LensVLM框架，通过可学习工具选择性扩展压缩图像，在4.3倍压缩下保持全文本精度，支持最高10.1倍压缩。


<details>
  <summary>Details</summary>
Motivation: VLM处理文本图像时高压缩导致字符不可区分，需解决压缩精度快速下降问题。

Method: 后训练框架，结合图像扫描与选择性扩展工具，基于Qwen3.5-9B-Base训练实现。

Result: 7个文本QA基准测试4.3倍压缩精度持平全文本上限，10.1倍压缩优于检索/压缩基线；泛化至多模态文档与代码任务。

Conclusion: 视觉压缩对渲染选择鲁棒，扩展内容可靠性随压缩增加而提升；文本扩展适合渲染文本，高分辨率扩展适合原生文档。
...
### [97] [OneViewAll: Semantic Prior Guided One-View 6D Pose Estimation for Novel Objects](https://arxiv.org/abs/2605.07023)
*Yang Luo,Yan Gong,Yongsheng Gao,Jie Zhao,Xinyu Zhang,Huaping Liu*

Main category: cs.CV

TL;DR: 提出OneViewAll框架，仅需单张真实RGB-D参考视图即可实现6D物体姿态估计，通过投影-比较范式和层级语义先验达到高精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖CAD模型或多视图数据，可扩展性差；实际场景中常仅提供单张参考视图，缺乏3D模型。

Method: 采用Project-and-Compare范式，在投影等变空间中对齐参考与查询观测，并集成三级语义先验（类别/场景级、对象对称级、块级）。

Result: 在LINEMOD数据集上达92.5% ADD-0.1精度，显著优于One2Any（52.6%），并在YCB-V、Real275等数据集上有一致提升。

Conclusion: 对称感知投影有效处理对称、无纹理和遮挡物体，实现低延迟高性能的姿态估计。
...
### [98] [Pan-FM: A Pan-Organ Foundation Model with Saliency-Guided Masking for Missing Robustness](https://arxiv.org/abs/2605.07055)
*Qiangqiang Wu,Grace McIlvain,Zhou Yu,Junhao Wen*

Main category: cs.CV

TL;DR: 提出Pan-FM，一种在全器官缺失场景下预训练的全器官基础模型，通过显著性引导掩码缓解主导器官偏差，提升多器官表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有医学基础模型多限于单模态或单器官，但人类衰老和疾病涉及多器官协同过程；且真实多模态数据常非随机缺失，导致偏差和泛化能力下降。

Method: 采用统一骨干网络处理器官缺失；基于掩码自蒸馏预训练；引入显著性引导掩码，根据注意力分布自适应掩码主导器官以减少捷径学习偏差。

Result: 在UK Biobank上，Pan-FM在13种疾病类别和14种单一疾病预测中优于单器官和多器官基线，且在器官缺失场景下鲁棒性更强。

Conclusion: Pan-FM为系统神经科学中多模态学习的真实缺失问题提供可扩展方案，推动更通用的全身基础模型发展。
...
### [99] [Learning to Track Instance from Single Nature Language Description](https://arxiv.org/abs/2605.07064)
*Yaozong Zheng,Bineng Zhong,Qihua Liang,Shuimu Zeng,Haiying Xia,Shuxiang Song*

Main category: cs.CV

TL;DR: 提出了一种自监督视觉语言跟踪方法，无需边界框标注，通过动态令牌聚合模块实现语言引导的实例跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖边界框标注且平等融合视觉与语言特征，限制了自监督学习的有效性及泛化能力。

Method: 设计动态令牌聚合模块，根据注意力分数选择并合并重要目标令牌，聚合到语言令牌中消除冗余噪声，再以融合后的语言令牌引导搜索帧中目标令牌的提取与传播。

Result: 在视觉语言跟踪基准上超越当前最先进的自监督方法。

Conclusion: 自监督学习和动态令牌聚合可以有效实现无需边界框标注的语言引导跟踪，提升语义对齐和实例跟踪能力。
...
### [100] [Decoupling Semantics and Fingerprints: A Universal Representation for AI-Generated Image Detection](https://arxiv.org/abs/2605.07074)
*Zhiyuan Wang,Yanxiang Chen,Yuanzhi Yao,Yunfeng Diao*

Main category: cs.CV

TL;DR: 提出ODP-Net，通过正交分解和净化网络解耦特征，提升对未见架构AI生成图像的检测泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型因特征纠缠导致难以学习通用伪造痕迹，无法泛化到未知生成架构。

Method: 基于频谱分析发现不同生成器指纹在频率子空间正交，提出ODP-Net：实例感知正交分解、扰动净化、流形对齐。

Result: 在Stable Diffusion 3等未见架构上达到最优性能，验证了结构解耦对泛化的关键作用。

Conclusion: 结构解耦通用伪造痕迹与生成器特异性指纹及语义内容，是提升AI图像检测泛化性的关键。
...
### [101] [Learning Visual Feature-Based World Models via Residual Latent Action](https://arxiv.org/abs/2605.07079)
*Xinyu Zhang,Zhengtong Xu,Yutian Tao,Yeping Wang,Yu She,Abdeslam Boularias*

Main category: cs.CV

TL;DR: 提出基于残差潜在动作（RLA）的世界模型RLA-WM，利用DINO残差学习RLA并通过流匹配预测，在仿真和真实数据集上优于现有特征基和视频扩散世界模型，同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉特征的世界模型因直接回归导致预测模糊或崩溃，而高维特征空间中的生成建模仍然困难。

Method: 从DINO残差学习残差潜在动作（RLA），并采用流匹配（flow matching）预测RLA，构建RLA-WM世界模型。

Result: RLA-WM在多个仿真和真实数据集上超越SOTA特征基和视频扩散世界模型，速度比视频扩散快数个数量级。

Conclusion: RLA-WM可有效用于无动作演示视频中的策略学习，以及完全基于离线视频训练的世界模型视觉强化学习框架。
...
### [102] [ImplantMamba: Long-range Sequential Modeling Mamba For Dental Implant Position Prediction](https://arxiv.org/abs/2605.07082)
*Xinquan Yang,Congmin Wang,Xuguang Li,Yulei Li,Linlin Shen,Yongqiang Deng He Meng*

Main category: cs.CV

TL;DR: 提出ImplantMamba网络，通过长程序列建模、CNN-Mamba混合编码器和坡度耦合预测分支，实现牙种植体位置与角度的精准定位。


<details>
  <summary>Details</summary>
Motivation: 种植体区域缺乏纹理，AI需依赖邻牙纹理推断位置和角度，传统方法难以捕捉长程依赖关系。

Method: 采用CNN-Mamba混合编码器提取局部解剖特征和全局上下文，设计坡度耦合预测分支（SCP）强制位置与角度的内在一致性。

Result: 在大规模牙种植数据集上，ImplantMamba性能优于现有方法。

Conclusion: 所提网络通过耦合回归和高效建模，提升了种植体定位的准确性和解剖合理性。
...
### [103] [Task Relevance Is Not Local Replaceability: A Two-Axis View of Channel Information](https://arxiv.org/abs/2605.07086)
*Houman Safaai,Andrew T. Landau,Celia C. Beron,Yasin Mazloumi,Bernardo L. Sabatini*

Main category: cs.CV

TL;DR: 本文提出双轴视角（局部可替代性与目标相关性）来解构视觉网络通道重要性，发现局部轴指标在通道剪枝中比目标轴指标更可靠地预测可移除性。


<details>
  <summary>Details</summary>
Motivation: 传统单分数通道重要性隐去了两个问题：通道与任务的相关性，以及当通道被移除时其功能是否可由同层同伴替代（局部可替代性）。

Method: 引入局部轴（测量输入捕获与同伴重叠）和目标轴（测量任务信息与目标-多余信息），通过高斯线性分析、损伤与同伴替代实验，以及固定FLOPs匹配剪枝协议进行验证。

Result: 在CIFAR-100上的ResNet-18、VGG-16和MobileNetV2中两轴弱对齐，训练中迅速分离；局部轴指标在剪枝中比目标轴指标更可靠，但VGG-16中基于范数的基线仍有竞争力。

Conclusion: 通道的可移除性更依赖于局部可替代性而非目标相关性，剪枝决策应区分这两轴，而非依赖通用排序分数。
...
### [104] [InfoGeo: Information-Theoretic Object-Centric Learning for Cross-View Generalizable UAV Geo-Localization](https://arxiv.org/abs/2605.07099)
*Hongyang Zhang,Maonnan Wang,Ziyao Wang,Hongrui Yin,Man OnPun*

Main category: cs.CV

TL;DR: 提出信息论框架InfoGeo，通过对象中心学习和信息瓶颈优化，提升跨视角地理定位在复杂环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有全局特征对齐方法难以应对区域纹理、天气变化导致的域偏移，以及无人机视角中密集细粒度目标造成的视觉杂乱问题。

Method: 基于信息瓶颈理论，最大化跨视图间对象中心结构关系的视角不变信息，同时最小化视角特定噪声信号。

Result: 在多个基准和挑战场景中，InfoGeo显著优于现有最先进方法。

Conclusion: 信息论框架有效解耦共享与噪声信息，为跨视图地理定位提供新的鲁棒解决方案。
...
### [105] [Neurosymbolic Framework for Concept-Driven Logical Reasoning in Skeleton-Based Human Action Recognition](https://arxiv.org/abs/2605.07140)
*Talha Ilyas,Deval Mehta,Zongyuan Ge*

Main category: cs.CV

TL;DR: 提出了一种基于神经符号的骨架动作识别框架，将动作识别转化为基于运动原语的一阶逻辑推理，实现了可解释的动作理解。


<details>
  <summary>Details</summary>
Motivation: 现有基于骨架的动作识别模型性能强但缺乏可解释性，难以理解其决策过程。

Method: 使用时空骨架编码器提取潜在运动表示，通过时空概念解码器映射为可解释的概念谓词，并利用可微一阶逻辑层组合成逻辑规则，同时对齐LLM描述的运动原语语义。

Result: 在NTU RGB+D 60/120和NW-UCLA数据集上取得了有竞争力的识别性能，同时提供了基于逻辑结构的显式可解释解释。

Conclusion: 神经符号推理是用于可解释时空动作理解的有效范式。
...
### [106] [Qwen3-VL-Seg: Unlocking Open-World Referring Segmentation with Vision-Language Grounding](https://arxiv.org/abs/2605.07141)
*Yuan Yao,Qiushi Yang,Humen Zhong,Jiangning Wei,Yifang Men,Shuai Bai,Miaomiao Cui,Zhibo Yang*

Main category: cs.CV

TL;DR: 本文提出Qwen3-VL-Seg，一种参数高效的开放世界指代分割框架，利用MLLM预测的边界框作为结构先验，通过轻量级解码器生成像素级分割掩码，仅增加17M参数，在封闭和开放场景下均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在开放世界视觉定位中表现强劲，但输出局限于稀疏边界框，无法用于密集视觉预测；而现有分割方法依赖外部模型（如SAM），带来架构和部署开销，因此需要一种轻量、高效且不依赖外部模型的指代分割方法。

Method: 提出box引导的掩码解码器，包含多尺度空间特征注入、空间-语义查询构建、box引导的高分辨率像素融合及迭代掩码感知查询细化；同时构建SA1B-ORS数据集和ORS-Bench基准进行训练与评估。

Result: 在指代分割、视觉定位和ORS-Bench上，Qwen3-VL-Seg在封闭和开放场景下均表现强劲，尤其在语言密集型指令和分布外泛化上优势明显，且保持通用多模态能力。

Conclusion: Qwen3-VL-Seg通过轻量级解码器高效实现开放世界指代分割，无需外部模型，在保持泛化能力的同时取得了优异性能。
...
### [107] [AGA3DNet: Anatomy-Guided Gaussian Priors with Multi-view xLSTM for 3D Brain MRI Subtype Classification](https://arxiv.org/abs/2605.07142)
...
TL;DR: 提出AGA3DNet框架，利用影像报告中的简短解剖短语作为软先验，结合轻量级3D CNN与多视角xLSTM聚合，提升脑MRI异常亚型分类的准确性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有3D脑MRI分类模型难以同时利用局部解剖线索和长程上下文推理，且依赖密集体素标注，临床实用性受限。

Method: 从放射学报告中提取解剖短语，映射至图谱区域，经符号距离变换与高斯加权生成平滑空间先验，作为附加通道输入轻量级3D CNN与多视角xLSTM聚合网络。

Result: 在回顾性脑MRI队列上，AGA3DNet在异常亚型分类中相较基线方法实现了更均衡的性能指标，并通过先验通道支持临床可解释的定位。

Conclusion: 解剖短语引导的软先验有效提升了分类表现与可解释性，但受限于单队列评估及缺乏大规模公开配对数据集。
...
### [108] [TriP: A Triangle Puzzle Approach to Robust Translation Averaging](https://arxiv.org/abs/2605.07143)
*Zhekai Fan,Wanze Li,Jinxin Wang,Yunpeng Shi*

Main category: cs.CV

TL;DR: 提出了一种基于三角形的鲁棒平移平均框架TriP，通过局部相对边缘尺度推断与对数域尺度同步，高效恢复相机位置，对结构化噪声鲁棒且避免退化解。


<details>
  <summary>Details</summary>
Motivation: 现有平移平均方法因缺少距离信息而病态，且对异常值敏感，传统方法难以处理对抗性、循环一致等结构化污染。

Method: 从三角形几何推断局部相对边缘尺度，在对数域同步重叠三角形尺度以恢复全局一致边长和相机位置。

Result: 在合成与真实数据集上大幅优于所有先前方法，支持百万级相机图的并行化高效计算。

Conclusion: TriP通过高阶三角形一致性实现强理论恢复保证，无需额外抗退化约束，综合性能显著提升。
...
### [109] [UniV2D: Bridging Visual Restoration and Semantic Perception for Underwater Salient Object Detection](https://arxiv.org/abs/2605.07146)
*Laibin Chang,Shaodong Wang,Yunke Wang,Xu Zhang,Kui Jiang,Chang Xu,Bo Du*

Main category: cs.CV

TL;DR: 提出UniV2D统一网络，联合优化水下视觉恢复与显著目标检测，通过语义驱动范式解决传统顺序处理中的语义不一致问题。


<details>
  <summary>Details</summary>
Motivation: 传统'先增强后检测'串行范式导致低级恢复与高级感知分离，产生语义不一致或噪声，影响检测性能。

Method: 构建层次化双分支架构，包括自校准解码器预测初始显著图、掩码感知恢复模块重建图像，以及显著性引导精炼模块实现跨层次调制对齐。

Result: 在多个基准测试中，定量和定性评估均显著优于现有方法，创下联合水下感知新标准。

Conclusion: 联合优化恢复与检测的语义驱动范式有效克服水下视觉退化，实现互惠增强，为水下感知任务建立新参考。
...
### [110] [Uncovering and Shaping the Latent Representation of 3D Scene Topology in Vision-Language Models](https://arxiv.org/abs/2605.07148)
*Haoming Wang,Wei Gao*

Main category: cs.CV

TL;DR: 该研究发现当前视觉语言模型（VLM）在潜在空间中存在拓扑地图，但被非几何视觉语义掩盖。通过跨场景线性特征提取，可分离出对应三维场景拉普拉斯特征映射的空间子空间，并据此提出基于Dirichlet能量的潜在正则化方法，在少量微调后显著提升VLM在真实空间任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 人类通过认知地图（全向且拓扑保持的三维空间表征）导航，但现有VLM从二维视角输入是否构建了类似的内部三维表征仍不清楚。

Method: 通过跨场景线性特征提取隔离空间子空间，证明其与场景三维高斯核图的拉普拉斯特征映射对应；提出基于Dirichlet能量的潜在正则化方法，在合成数据上对VLM进行500步监督微调。

Result: 在涉及场景拓扑理解的空间任务中，该方法比标准微调和基线方法性能提升最高12.1%。

Conclusion: VLM存在潜在拓扑空间地图，且可通过数学正则化方法有效利用，提升空间推理能力。
...
### [111] [Real-IAD MVN: A Multi-View Normal Vector Dataset and Benchmark for High-Fidelity Industrial Anomaly Detection](https://arxiv.org/abs/2605.07149)
...
TL;DR: 本文提出了Real-IAD-MVN数据集，通过多视角高保真表面法线图替代稀疏3D点云，显著提升工业异常检测中细微几何缺陷的检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有2D/3D方法在检测细微几何异常（如划痕、凹陷）时受限于数据稀疏性，需要更细粒度的几何表示。

Method: 采集系统升级生成五视图高精度表面法线图，并基于跨模态统一原型重建方法进行异常检测。

Result: 多视图伪3D法线图检测性能优于稀疏点云，提出的统一原型基线方法超越现有多模态融合SOTA。
...
### [112] [DPG-CD: Depth-Prior-Guided Cross-Modal Joint 2D-3D Change Detection](https://arxiv.org/abs/2605.07151)
*Luqi Zhang,Zhen Dong,Bisheng Yang*

Main category: cs.CV

TL;DR: 提出DPG-CD框架，利用深度先验弥合跨模态差异，联合检测2D语义和3D高度变化，在三个数据集上表现最优。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效融合多时相跨模态数据（预DSM与后影像），且模态差异易混淆实际变化。

Method: 引入深度先验缩小模态差异，门控融合注入几何特征，多阶段跨时相跨模态特征融合，多任务解码联合预测2D/3D变化及辅助DSM预测。

Result: 在Hi-BCD、3DCD和NYC-MMCD数据集上，DPG-CD在2D和3D变化检测任务中均优于现有方法。

Conclusion: 深度先验引导的跨模态融合可有效提升联合2D/3D变化检测性能，辅助DSM预测增强结构一致性。
...
### [113] [PRIMED: Adaptive Modality Suppression for Referring Audio-Visual Segmentation via Biased Competition](https://arxiv.org/abs/2605.07154)
*Yuchen He,Jing Zhang*

Main category: cs.CV

TL;DR: 提出PRIMED模型，通过模拟认知神经科学的竞争理论，实现基于语言先验的自适应模态抑制，在Ref-AVS任务中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多模态信息视为同质输入，容易受无关或误导性模态干扰，导致分割不准确。

Method: 包含模态先验解码器（估计语言依赖的模态类型）、令牌蒸馏器（提取全局视觉令牌）和跨模态融合模块（实现竞争感知融合），并引入空间感知语义对齐损失。

Result: 在Ref-AVS基准上取得整体最优性能。

Conclusion: 通过自适应模态抑制和全局上下文共享，PRIMED能有效处理多模态关联的变异性，提升参考音视频分割的准确性。
...
### [114] [Hierarchical Perfusion Graphs for Tumor Heterogeneity Modeling in Glioma Molecular Subtyping](https://arxiv.org/abs/2605.07156)
*Han Jang,Junhyeok Lee,Heeseong Eum,Joon Jang,Yoseob Han,Seung Hong Choi,Kyu Sung Choi*

Main category: cs.CV

TL;DR: 该论文提出了HiPerfGNN框架，通过将DSC-MRI灌注动力学整合到放射基因组学中，实现对胶质瘤分子亚型的非侵入性精确分型，在内部和外部队列中均取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于结构MRI的深度学习无法捕获区分分子亚型的血流动力学特征，而基于DSC-MRI的放射基因组学因站点变异和体素分析的局限性难以临床部署。

Method: 使用向量量化变分自编码器从原始时间-强度曲线学习离散血流动力学表示，定义粗粒度功能肿瘤栖息地节点，并通过结构MRI分层细分为子区域，利用分层图神经网络跨尺度传播信息进行分子预测。

Result: 内部队列中IDH、1p/19q和WHO分级AUC分别为0.96、0.89和0.84；外部队列中IDH AUC为0.89，且梯度显著性分析显示生物学上合理的注意力模式。

Conclusion: 将灌注动力学整合到放射基因组学管线中对胶质瘤分子亚型分型具有重要价值，为无创评估提供了可行方案。
...
### [115] [Operating Within the Operational Design Domain: Zero-Shot Perception with Vision-Language Models](https://arxiv.org/abs/2605.07649)
*Berkehan Ünal,Dierend Hauke,Fazlija Dren,Plachetka Christopher*

Main category: cs.CV

TL;DR: 本文研究了视觉语言模型(VLMs)作为零样本“ODD传感器”的可行性，通过实验评估了四种VLM在ODD分类和检测中的表现，并提出了优化策略和提示模板。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要遵守基于运行设计域(ODD)的安全法规，而依赖ODD元素感知对于安全实施和审计至关重要。VLMs无需任务特定训练数据，具有适应动态ODD定义的潜力，因此被探索作为零样本ODD传感器。

Method: 在自定义数据集和Mapillary Vistas数据集上，对四种VLMs进行零样本ODD分类和检测实验；进行消融研究分析零样本优化策略的成本-性能；开发可重用提示模板及适配指南。

Result: 基于定义锚定的思维链提示结合角色分解方法表现最佳，而其他方法可能导致召回率下降。

Conclusion: 研究为安全关键应用中基于ODD的透明感知奠定了基础，展示了VLMs在零样本ODD感知中的有效性。
...
### [116] [Masks Can Talk: Extracting Structured Text Information from Single-Modal Images for Remote Sensing Change Detection](https://arxiv.org/abs/2605.07178)
*Kai Zheng,Hang-Cheng Dong,Jiatong Pan,Zhenkai Wu,Fupeng Wei,Wei Zhang*

Main category: cs.CV

TL;DR: 提出S2M框架，从变化标签中自动生成结构化的文本描述（四元组：位置、类型、方式、数量），无需额外标注，实现多模态变化检测。在Gaza-Change-v2数据集上，S2M以17.80% Sek和66.14% F_scd超越大语言模型辅助方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态变化检测方法依赖粗粒度或带噪文本，而标注掩码本身隐含了精细变化语义（位置、类型、转变、数量），该信息未被充分利用。

Method: S2M框架：1) 自动将变化区域转为语义四元组并生成固定模板文本；2) 两阶段训练：先在遥感图像上微调获取领域特征，再通过多模态解码器和双向对比损失对齐视觉与结构化文本特征。

Result: 在Gaza-Change-v2多类变化检测数据集上，S2M的Sek达17.80%，F_scd达66.14%，优于使用大语言模型的现有多模态方法。

Conclusion: 标注掩码本身即可作为高质量多模态监督源，通过结构化文本描述可显著提升变化检测性能，验证了‘掩码会说话’的理念。
...
### [117] [Text-to-CAD Evaluation with CADTests](https://arxiv.org/abs/2605.07807)
*Dimitrios Mallis,Marco Wang,Ahmet Serdar Karadeniz,Elisa Ricci,Anis Kacem,Djamila Aouada*

Main category: cs.CV

TL;DR: 本文提出了首个基于可执行测试的Text-to-CAD评估基准CADTestBench，用于验证生成CAD模型的几何与拓扑正确性。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-CAD评估方法极少且难以衡量生成模型满足输入要求的能力。

Method: 设计CADTests可执行软件测试，构建CADTestBench基准，并利用测试指导生成过程。

Result: 全面基准测试表明，基于CADTestBench的方法可超越现有Text-to-CAD方法。

Conclusion: CADTestBench为Text-to-CAD提供了有效的自动化评估与生成优化新视角。
...
### [118] [PicoEyes: Unified Gaze Estimation Framework for Mixed Reality with a Large-Scale Multi-View Dataset](https://arxiv.org/abs/2605.07188)
*Fuxin Duan,Hui Wang*

Main category: cs.CV

TL;DR: 提出了统一的注视估计框架PicoEyes，可直接从单目或双目输入预测注视的所有关键属性，并支持3D眼重建，在大规模数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时处理校准、注视预测和不同设备姿态，且缺乏统一端到端框架及大规模多视图近眼数据集。

Method: 端到端联合估计眼参数与深度图，支持单目/双目输入，并引入含丰富标注的大规模多视图近眼数据集。

Result: 在无校准、校准、重新佩戴及预测设置下，性能均优于现有学术和工业方法。

Conclusion: 为混合现实中的鲁棒及泛化注视估计建立了实用端到端范式。
...
### [119] [SatSurfGS: Generalizable 2D Gaussian Splatting for Sparse-View Satellite Surface Reconstruction](https://arxiv.org/abs/2605.07181)
*Min Chen,Wei Guo,Bin Wang,Wen Li,Tong Fang,Jinbo Zhang,Junqi Zhao,Hong Kuang,Han Hu,Xuming Ge,Qing Zhu,Bo Xu*

Main category: cs.CV

TL;DR: 提出SatSurfGS，一种基于2DGS的通用稀疏视角卫星影像表面重建方法，通过粗到细的高斯属性预测和三级局部几何可靠性建模（特征学习、参数估计、训练优化），在卫星数据集上实现了优于现有方法的渲染质量、重建精度、跨数据集泛化和推理效率。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角卫星影像表面重建中，多视图匹配可靠性存在强空间异质性，导致几何约束稀疏、分布不均且局部不可靠；现有2DGS框架缺乏通用化的前馈方法，无法有效处理这些问题。

Method: 构建粗到细的高斯属性预测框架，显式建模三级局部几何可靠性：1）置信感知的单目多视图特征融合模块（自适应融合单目先验与多视图匹配特征）；2）跨阶段自一致性残差引导模块（利用前一阶段渲染高度图与当前MVS高度图的残差及置信度信息稳定参数优化）；3）置信度双向路由损失（差异化分配几何和外观监督）。

Result: 在卫星数据集上，与代表性通用基线及竞争性逐场景优化方法相比，该方法在渲染质量、表面重建精度、跨数据集泛化能力和推理效率上均取得提升。

Conclusion: SatSurfGS通过显式建模局部几何可靠性，有效解决了稀疏视角卫星影像表面重建中的关键挑战，为卫星影像表面重建提供了高效且泛化性强的解决方案。
...
### [120] [6D Pose Estimation via Keypoint Heatmap Regression with RGB-D Residual Neural Networks](https://arxiv.org/abs/2605.08059)
*Ismail Aljosevic,Amir Masoud Almasi,Ana Parovic,Ashkan Shafiei*

Main category: cs.CV

TL;DR: 该论文提出了一种基于关键点热图回归的模块化6D姿态估计框架。


<details>
  <summary>Details</summary>
Motivation: 为了提高6D姿态估计的准确性，特别是通过融合RGB和深度数据来增强性能。

Method: 使用YOLOv10m进行目标检测，ResNet18预测2D热图提取关键点，通过PnP RANSAC估算姿态。比较不同关键点选择策略，并引入RGB-D交叉融合架构，同时优化激活函数和学习率调度。

Result: RGB-only模型在LINEMOD数据集上达到84.50%的ADD准确率，RGB-D融合模型达到92.41%。

Conclusion: RGB-D融合显著提升姿态估计精度，模块化框架有效且可扩展。
...
### [121] [Attention Transfer Is Not Universally Effective for Vision Transformers](https://arxiv.org/abs/2605.07191)
*Huaiyuan Qin,Muli Yang,Gabriel James Goenawan,Peng Hu,Chen Gong,Xi Peng,Hongyuan Zhu*

Main category: cs.CV

TL;DR: 注意力迁移并非普遍有效：教师-学生架构不匹配时迁移失败，需添加教师原生组件才能恢复性能。


<details>
  <summary>Details</summary>
Motivation: 探究注意力迁移在多种ViT家族中的通用性，揭示架构不匹配导致迁移失败的根本原因。

Method: 在20个教师模型上测试注意力迁移，通过控制实验定位到注意力路由通道问题，添加教师原生组件进行修复。

Result: 7个ViT家族迁移成功，4个家族迁移失败（性能下降5.1%），失败原因与架构不匹配相关。

Conclusion: 注意力迁移仅在学生架构与教师匹配时有效，架构不匹配是迁移失败的主因。
...
### [122] [Closed-Form Linear-Probe Dataset Distillation for Pre-trained Vision Models](https://arxiv.org/abs/2605.07194)
*Bincheng Peng,Guang Li,Ping Liu,Takahiro Ogawa,Miki Haseyama*

Main category: cs.CV

TL;DR: 提出CLP-DD方法，利用闭式线性探测解法实现数据集蒸馏，显著提升效率与性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法在冻结特征线性探测场景中效率低，且未充分利用闭式解的优势。

Method: 采用双层优化：内层用样本空间核岭回归闭式求解线性探测分类器，外层用温度缩放softmax交叉熵评估分类器性能并更新合成图像。

Result: 在ImageNet-100中超越LGM (无DSA)，接近LGM (有DSA)；在ImageNet-1K中匹配/超越LGM (有DSA)，速度提升14倍，显存减少87.5%。

Conclusion: 闭式线性探测结合判别性外层损失能高效实现数据集蒸馏，避免轨迹匹配的高成本。
...
### [123] [AsyncEvGS: Asynchronous Event-Assisted Gaussian Splatting for Handheld Motion-Blurred Scenes](https://arxiv.org/abs/2605.07192)
*Jun Dai,Renbiao Jin,Bo Xu,Yutian Chen,Linning Xu,Mulin Yu,Tianfan Xue,Shi Guo*

Main category: cs.CV

TL;DR: 提出了一种基于异步RGB-事件相机的3D重建方法，利用事件相机的高时间分辨率信息，结合VGGT姿态估计和结构驱动损失，实现了在严重运动模糊下的高质量3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法（如3DGS和NeRF）在输入图像存在严重运动模糊时效果不佳，而现有事件辅助方法依赖低分辨率传感器和严格同步，限制了在手持设备（如智能手机）上的实用性。

Method: 使用异步RGB-事件双相机系统，先通过事件数据重建清晰图像，再基于VGGT进行跨域姿态估计获得初始化，优化过程中采用结构驱动的事件损失和视图一致性正则化。

Result: 在自建数据集和现有基准上达到最优性能，显著提升了严重运动模糊下的重建鲁棒性。

Conclusion: 该方法有效结合事件相机和RGB相机，克服了运动模糊对3D重建的限制，具有实用性和先进性。
...
### [124] [See Tomorrow, Act Today: Foresight-Driven Autonomous Driving](https://arxiv.org/abs/2605.07195)
*Bozhou Zhang,Nan Song,Yuang Wang,Jiankang Deng,Xiatian Zhu,Li Zhang*

Main category: cs.CV

TL;DR: 提出ForeSight框架，将自动驾驶从反应式规划转变为基于未来场景想象的预见性决策，通过预训练世界模型生成未来视觉场景并据此规划动作。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶规划器本质上是反应式的，仅依赖历史和当前观测预测动作，缺乏像人类一样的未来情景模拟能力。

Method: 两阶段流程：1) 利用预训练世界模型生成可信的未来视觉世界；2) 基于这些想象中的未来场景进行动作规划。

Result: 在NAVSIM和nuScenes数据集上，显式未来想象显著优于现有最先进方法，验证了预见性方法的有效性。

Conclusion: 将决策从“我现在该做什么？”转向“未来会发生什么，我该如何应对？”的范式转变，能更有效地应对动态交互场景。
...
### [125] [From Pixels to Primitives: Scene Change Detection in 3D Gaussian Splatting](https://arxiv.org/abs/2605.07203)
*Chamuditha Jayanga Galappaththige,Jason Lai,Timothy Patten,Donald Dansereau,Niko Suenderhauf,Dimity Miller*

Main category: cs.CV

TL;DR: 本文提出GS-DIFF方法，直接在3D高斯原语空间中进行场景变化检测，无需渲染到2D，比先渲染后比较的方法mIoU提升约17%。


<details>
  <summary>Details</summary>
Motivation: 现有方法将变化检测视为像素问题，但原语属性本身包含足够信号，且渲染范式忽略了多视角一致性和变化类型区分。

Method: 使用各向异性几何和光度漂移模型，结合每个原语的可观测性项，直接比较原语的位置、协方差和颜色属性，分别检测几何和外观变化。

Result: 在真实世界基准上，GS-DIFF的平均交并比（mIoU）超越前最优方法约17%。

Conclusion: 原语空间比较有效且高效，天然具备多视角一致性，并能无监督地区分结构变化与表面变化。
...
### [126] [LoHGNet: Infrared Small Target Detection through Lorentz Geometric Encoding with High-Order Relation Learning](https://arxiv.org/abs/2605.07213)
*Qianwen Ma,Yang Xu,Shangwei Deng,Xiaobo Li,Haofeng Hu*

Main category: cs.CV

TL;DR: 提出LoHGNet，将洛伦兹几何编码与高阶关系学习结合，用于红外小目标检测，在三个数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测中目标线索稀少、背景杂波严重；现有方法在欧氏空间中特征表示有限，难以描述弱目标的细微差异和目标-背景的上下文关系。

Method: 构建洛伦兹编码分支（GA-LRCM）进行双曲特征建模，通过对数映射将双曲特征映射到欧氏空间，再设计高阶关系学习模块（HORL）利用超图建模高阶上下文依赖。

Result: 在三个数据集上取得了有竞争力的检测精度和复杂场景适应性。

Conclusion: 洛伦兹几何编码与高阶关系学习有效提升红外小目标检测性能，代码已开源。
...
### [127] [DINO-MVR: Multi-View Readout of Frozen DINOv3 for Annotation-Efficient Medical Segmentation](https://arxiv.org/abs/2605.07221)
*Wei Jiang,Feng Liu,Nan Ye,Hongfu Sun*

Main category: cs.CV

TL;DR: 提出DINO-MVR框架，仅通过冻结的DINOv3骨干网络和轻量MLP探头实现高效医学分割，在少量标注下接近全标注性能。


<details>
  <summary>Details</summary>
Motivation: 医学分割中标注稀缺时，微调大模型或高容量解码器难以可靠拟合，需要更高效的方案。

Method: 冻结DINOv3骨干，只在最后三个Transformer块上训练轻量MLP探头，推理时通过互补分辨率、测试时增强、熵加权融合和空间正则化处理，对体积数据增加高斯z轴平滑。

Result: 在Kvasir-SEG达0.895 Dice，ISIC 2018达0.897 Dice，BraTS全肿瘤分割达0.908 Dice，仅用5例BraTS患者恢复40例参考的98.4%性能。

Conclusion: 冻结的自监督视觉骨干结合高效多视图读出可支持准确医学分割，尤其在标注有限时。
...
### [128] [Towards multi-modal forgery representation learning for AI-generated video detection and localization](https://arxiv.org/abs/2605.07232)
*Dat Le,Khoa Nguyen,Xin Wang,Shu Hu*

Main category: cs.CV

TL;DR: 提出一种联合多模态分支的架构，用于检测和定位AI生成视频中的局部伪造区域。


<details>
  <summary>Details</summary>
Motivation: 现有AI视频检测方法局限于单模态或部分模态建模，且缺乏细粒度时间定位能力，而AI生成视频的滥用风险增加。

Method: 集成LMM语义分支、时空视觉分支和多尺度局部伪造音频分支的多模态联合架构。

Result: 在检测和细粒度时间定位上优于现有方法。

Conclusion: 多模态联合方法有效应对AI视频局部伪造的检测与定位挑战。
...
### [129] [CASCADE: Context-Aware Relaxation for Speculative Image Decoding](https://arxiv.org/abs/2605.07230)
*Selin Yildirim,Subhajit Dutta Chowdhury,Mohammad Mahdi Kamani,Vikram Appia,Deming Chen*

Main category: cs.CV

TL;DR: 提出CASCADE方法，通过利用树状推测解码中目标模型的语义可互换性和收敛性冗余，实现无需额外训练的解码松弛并改进草稿模型训练，在文本到图像生成中获得最高3.6倍加速且保持质量。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成计算缓慢，现有推测解码因目标模型高不确定性导致草稿标记拒绝率高，效率提升有限。

Method: 识别树状推测解码中目标模型隐藏状态表示的语义可互换性和收敛性冗余，据此实现接受松弛；并将冗余信号注入草稿模型训练。

Result: 在多个文本到图像模型和草稿架构上，CASCADE实现最高3.6倍加速，图像质量和文本提示保真度与基线相当。

Conclusion: 利用目标模型固有冗余可显著提升推测解码效率，为高效图像生成提供新路径。
...
### [130] [Hard to Read, Easy to Jailbreak: How Visual Degradation Bypasses MLLM Safety Alignment](https://arxiv.org/abs/2605.07250)
*Zhixue Song,Boyan Han,Yiwei Wang,Chi Zhang*

Main category: cs.CV

TL;DR: 本文发现降低分辨率会意外地导致MLLMs更容易被越狱，并提出一种“结构化认知卸载”策略来缓解这一风险。


<details>
  <summary>Details</summary>
Motivation: 视觉上下文压缩技术虽然高效，但其安全性尚未得到充分评估，特别是分辨率降低对安全性的影响。

Method: 通过实验验证低分辨率、噪声和几何畸变等视觉扰动对模型安全防御的影响，并提出“结构化认知卸载”策略，将视觉转录与安全评估分离。

Result: SOTA模型的安全防御随分辨率降低而显著退化，即使在文本可读的情况下也存在风险；提出的策略能有效缓解这一越狱风险。

Conclusion: 视觉压缩存在严重的越狱风险，需要采取架构性措施（如认知卸载）来确保模型安全。
...
### [131] [LENS: Low-Frequency Eigen Noise Shaping for Efficient Diffusion Sampling](https://arxiv.org/abs/2605.07253)
*Haewon Jeon,Si-Hyeon Lee*

Main category: cs.CV

TL;DR: LENS通过仅在低频率子空间中调制噪声，显著降低了扩散模型加速生成时的计算开销，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有加速方法（如测试时优化、超网络）在减少去噪步数时面临质量下降或计算开销大的问题。

Method: 提出LENS框架，基于低频率噪声主导图像结构的观察，仅对噪声的低频分量进行调制，使用轻量级独立网络实现高效目标性调制。

Result: 与先前方法相比，LENS在FLOPs、模型参数和推理时间上分别减少400-700×、25-75×和10-20×，同时图像质量具有竞争力。

Conclusion: LENS通过聚焦低频噪声子空间，实现了高效且高质量的扩散模型加速生成。
...
### [132] [High-Fidelity Surface Splatting-Based 3D Reconstruction from Multi-View Images](https://arxiv.org/abs/2605.07254)
*Nandhana Sunil,Abhirami R Iyer,Avirup Mandal*

Main category: cs.CV

TL;DR: 提出了一种基于紧凑多项式核的隐式移动最小二乘法，结合随机正则化和拉普拉斯滤波，实现了从稀疏多视图数据中高质量重建高频几何和纹理。


<details>
  <summary>Details</summary>
Motivation: 现有基于指数核的IMLS方法难以恢复高频几何细节，且多视图网格重建方法（如3DGS和NeRF）依赖后处理，限制了几何与外观的联合优化。

Method: 使用局部支持、灵活性更强的紧凑多项式核替代指数核，并引入基于拉普拉斯滤波的随机正则化来增强高频细节保留。

Result: 在表面重建和渲染任务中均达到最先进性能，生成更精确的几何结构和更清晰的视觉效果。

Conclusion: 该改进的IMLS框架能有效从稀疏多视图数据中恢复高频细节，实现几何与外观的端到端联合优化。
...
### [133] [TAS-LoRA: Transformer Architecture Search with Mixture-of-LoRA Experts](https://arxiv.org/abs/2605.07256)
*Jeimin Jeon,Hyunju Lee,Bumsub Ham*

Main category: cs.CV

TL;DR: 本文提出TAS-LoRA方法，通过参数高效的低秩适应（LoRA）和Mixture-of-LoRA-Experts策略解决视觉Transformer架构搜索中的特征崩塌问题，在多个数据集上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有TAS方法因超网共享权重导致子网特征崩塌，限制子网性能，需设计能学习子网特定特征的方法。

Method: 引入LoRA实现子网特定特征学习，采用轻量路由器动态分配LoRA专家（MoLE），并引入分组初始化促进专家多样性。

Result: 在ImageNet、CIFAR-10/100、Flowers、CARS和INAT-19等数据集上，有效缓解特征崩塌，性能超越现有最优TAS方法。

Conclusion: TAS-LoRA高效解决了TAS中的特征崩塌问题，为自动化ViT设计提供了新方案。
...
### [134] [Adaptive Subspace Projection for Generative Personalization](https://arxiv.org/abs/2605.07257)
*Van-Anh Nguyen,Anh Tuan Bui,Tamas Abraham,Junae Kim,Amardeep Kaur,Rollin Omari,Thuy-Trang Vu,Dinh Phung*

Main category: cs.CV

TL;DR: 本文提出了一种无需训练的方法AdaptSP，通过自适应子空间投影调整嵌入，解决生成式个性化中的语义崩溃问题，在保持主体身份的同时提高提示保真度和上下文对齐。


<details>
  <summary>Details</summary>
Motivation: 生成式个性化中，学习的概念会主导文本提示，忽略重要上下文细节，导致语义崩溃。

Method: 利用预训练稳定嵌入作为锚点，隔离语义漂移并将其投影到低维子空间中进行精确调整。

Result: 该方法显著提高了提示保真度和上下文对齐。

Conclusion: AdaptSP能有效缓解语义崩溃，同时保持主体身份，是一种高效的免训练解决方案。
...
### [135] [Sat3R: Satellite DSM Reconstruction via RPC-Aware Depth Fine-tuning](https://arxiv.org/abs/2605.07264)
...
TL;DR: Sat3R通过引入RPC感知的度量深度微调，将单目深度基础模型适配到卫星领域，实现无需逐场景优化的快速且精确的DSM重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法在卫星DSM重建中面临优化方法速度慢和通用模型因领域差距性能差的问题。

Method: 使用SiLog损失函数对Depth Anything V2进行RPC感知的度量深度微调，利用RPC几何构建物理一致的伪深度监督。

Result: 在DFC2019基准上，MAE降低38%，精度媲美优化方法，速度提升300倍以上。

Conclusion: 适当适配的馈送模型可在极低计算成本下达到优化方法的精度，推动大规模卫星DSM重建的实际应用。
...
### [136] [From Clouds to Hallucinations: Atmospheric Retrieval Hijacking in Remote Sensing Vision-Language RAG](https://arxiv.org/abs/2605.07273)
...
TL;DR: 提出CloudWeb攻击，通过添加云/雾模式干扰多模态RAG的遥感图像检索，使检索器偏向目标天气证据。


<details>
  <summary>Details</summary>
Motivation: 现有研究未探索输入空间对遥感多模态RAG检索阶段的攻击。

Method: 在输入图像上叠加参数化云/雾模式，通过优化目标函数实现检索劫持。

Result: 在七个数据集上，CloudWeb始终优于基线，如GeoRSCLIP的Weather@5从0.71%提升至43.29%。

Conclusion: 自然外观的天气变化可在生成前破坏证据检索，暴露了RAG的实用漏洞。
...
### [137] [SplatWeaver: Learning to Allocate Gaussian Primitives for Generalizable Novel View Synthesis](https://arxiv.org/abs/2605.07287)
*Yecong Wan,Fan Li,Mingwen Shao,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 提出SplatWeaver，一种可泛化的新视图合成框架，根据区域复杂度动态分配高斯原语数量，减少了冗余并提高了渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法为每个像素或体素分配固定数量高斯原语，忽略了真实场景的空间变化复杂度，导致平滑区域浪费、细节区域容量不足。

Method: 引入基数高斯专家和像素级路由方案，专家产生0到M个原语，路由协调专家自适应分配原语数量，并结合高频先验和路由正则化优化分配。

Result: 在多种场景下，SplatWeaver以更少高斯原语实现了更逼真的新视图渲染，性能优于现有最优方法。

Conclusion: 动态原语分配策略有效提升了表达能力和紧凑性，验证了复杂度感知表示的优越性。
...
### [138] [Sword: Style-Robust World Models as Simulators via Dynamic Latent Bootstrapping for VLA Policy Post-Training](https://arxiv.org/abs/2605.07288)
*Jiaxuan Gao,Yongjian Guo,Zhong Guan,Wen Huang,Wanlun Ma,Xi Xiao,Junwu Xiong,Sheng Wen*

Main category: cs.CV

TL;DR: Sword框架通过结构引导样式增强和动态潜在自举方法，提升了世界模型的泛化能力、鲁棒性和生成质量，在LIBERO基准测试中显著优于基线方法WoVR。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在作为模拟器时存在泛化差、长程误差累积、对初始状态扰动敏感等问题，导致生成质量下降，限制了其在VLA模型中的应用。

Method: 提出Sword框架，包括结构引导样式增强（解耦视觉纹理与任务动态）和动态潜在自举（保持训练与推理一致性，降低内存消耗）。

Result: 在LIBERO基准上，Sword在泛化、生成质量、鲁棒性、保真度及VLA模型强化学习后训练成功率上均显著优于基线WoVR。

Conclusion: Sword有效缓解了世界模型作为模拟器时的关键问题，提升了其可靠性和实用性。
...
### [139] [EgoPro-Bench: Benchmarking Personalized Proactive Interaction in Egocentric Video Streams](https://arxiv.org/abs/2605.07299)
*Dongchuan Ran,Linyu Ou,Xueheng Li,Wenwen Tong,Chenxu Guo,Hewei Guo,Kaibing Wang,Lewei Lu*

Main category: cs.CV

TL;DR: 本文提出了EgoPro-Bench基准，用于评估多模态大语言模型的主动交互能力，包含2400个评估视频和12000个训练视频，覆盖12个领域，并引入“短思考、更好交互”原则优化交互时机。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs仅被动响应，缺乏持续感知和主动协助能力；现有基准局限于警报场景，忽略个性化上下文，且无法评估人机交互的精确时机。

Method: 构建包含模拟用户画像的多领域流式第一人称视频数据集；设计专用评估协议和指标；训练面向流式视频的低延迟主动交互模型；引入有限token预算的交互原则。

Result: 该基准显著提升了MLLMs的意图理解能力，并能够准确识别人机交互的适当时机。

Conclusion: EgoPro-Bench为下一代以用户为中心的主动交互代理奠定了坚实基础。
...
### [140] [Amortized-Precision Quantization for Early-Exit Vision Transformers](https://arxiv.org/abs/2605.07317)
*Rui Fang,Hsi-Wen Chen,Ming-Syan Chen*

Main category: cs.CV

TL;DR: 提出MAQEE方法，通过联合优化退出阈值和位宽，在保证精度的前提下大幅降低计算量，实现了精度-效率Pareto最优。


<details>
  <summary>Details</summary>
Motivation: 现有量化方法假设静态全深度执行，导致早期退出策略因量化噪声干扰而不稳定，需要更鲁棒的量化-早退联合优化。

Method: 提出APQ利用率感知公式刻画深度-精度权衡，并设计MAQEE双层框架联合优化早退阈值和位宽，实现显式风险控制。

Result: 在分类、检测和分割任务中，BOPs降低高达95%，精度保持领先；比强基线方法提升最多20%。

Conclusion: MAQEE有效提升了低精度早退的推理稳定性，为动态精度推理提供了可行的优化方案。
...
### [141] [GEM: Generating LiDAR World Model via Deformable Mamba](https://arxiv.org/abs/2605.07326)
*Yang Wu,Zhaojiang Liu,Qiang Meng,Youquan Liu,Renliang Weng,Jianjun Qian,Jian Yang,Jin Xie*

Main category: cs.CV

TL;DR: 提出GEM，一种基于可变形Mamba架构的生成式LiDAR世界模型，解决点云无序和动态-静态区分难题，在多个基准上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: LiDAR世界模型进展落后于相机或占用数据模型，主要因点云无序性及动态与静态物体区分困难。

Method: 通过LiDAR场景分词器对扫描进行紧凑表示，无监督动态-静态分离器解耦特征，再经三路径可变形Mamba进行选择性扫描和自适应门控融合，可选集成规划器和BEV布局控制器。

Result: 在多种基准和设置下取得最先进性能，展示了高保真度和想象力。

Conclusion: GEM有效提升了LiDAR世界模型性能，支持自主展开和“what-if”场景生成。
...
### [142] [Teacher-Feature Drifting: One-Step Diffusion Distillation with Pretrained Diffusion Representations](https://arxiv.org/abs/2605.07327)
*Yuan Zhang,Chenyi Li,Guoqing Ma,Jiajun Zha,Yuanming Yang,Bo Wang,Wei Tang,Wenbo Li,Haoyang Huang,Nan Duan*

Main category: cs.CV

TL;DR: 本文提出了一种简化的一步蒸馏方法，利用预训练扩散模型的中间隐藏状态作为特征表示，避免了额外网络和复杂流程，实现了高效的单步生成。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法依赖多个辅助网络、精心设计的训练阶段或复杂的优化流程，需要更简化的方案。

Method: 使用预训练扩散模型的中间隐藏状态作为特征表示，替代额外的预训练特征提取器，并引入轻量级模式覆盖损失缓解模式崩溃。

Result: 在ImageNet-64×64上FID为1.58，在SDXL上FID为18.4，实现了具有竞争力的图像质量和多样性。

Conclusion: 提出的方法简化了蒸馏框架，同时保持高效单步生成和良好性能。
...
### [143] [GC-ART: Global Learnable Second-Order Rational Tone Curves for Illumination Robustness](https://arxiv.org/abs/2605.07329)
*Wei Huang,Joyce Huang*

Main category: cs.CV

TL;DR: 提出GC-ART，一种轻量级可微分预处理器，通过预测有理色调曲线进行图像增强，以提升分类鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法如卷积增强器计算成本高，且可能破坏边缘位置；需要一种高效、可学习的预处理模块。

Method: 使用643参数MLP从每通道软直方图预测端点固定的有理色调曲线，通过逐点映射应用曲线，端到端训练。

Result: 在CIFAR-10上，GC-ART在干净图像上匹配基线精度，在乘法变暗上优于基线，在对比度损坏上达48.45%（超过基线的46.27%和Zero-DCE++的47.13%）。

Conclusion: 直方图条件的理性曲线可学习全局色调校正（包括对比度扩展），点映射保留边缘位置，且计算量远低于卷积增强器。
...
### [144] [RCoT-Seg: Reinforced Chain-of-Thought for Video Reasoning and Segmentation](https://arxiv.org/abs/2605.07334)
*Junwei Wen,Deshui Miao,Guangming Lu,Xin Li,Wenjie Pei*

Main category: cs.CV

TL;DR: 提出RCoT-Seg，一种视频推理分割框架，通过显式分离时间推理与空间感知，实现关键帧自主选择与高分辨率分割，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM方法依赖简单采样或辅助模型选择帧，监督有限且规则基于帧-语言相似度，导致关键帧选择范围窄、时间理解弱、复杂场景定位脆弱。

Method: 分解为时间视频推理（TVR）和关键帧目标感知（KTP）两阶段：TVR通过自校机制（CoT-start语料初始化+GRPO优化）生成并重选关键帧；KTP在高分辨率分割后，用SAM2传播掩码。

Result: 实验表明RCoT-Seg在多种基准上达到最先进性能。

Conclusion: 显式分离时空推理能有效提升视频推理分割的鲁棒性和精度，关键帧的自主选择与优化是关键。
...
### [145] [ShellfishNet: A Domain-Specific Benchmark for Visual Recognition of Marine Molluscs](https://arxiv.org/abs/2605.07338)
*Ziheng Zhou,Yang Wang,Nan Wang,Chengliang Wu,Jun Yan*

Main category: cs.CV

TL;DR: 构建了面向真实水下环境的贝类图像基准数据集ShellfishNet，含32个分类群的8691张图像，系统评估了80种神经网络模型及多模态大模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有海洋底栖数据集缺乏对水下复杂环境（多变光照、多样姿态）的适应性，导致视觉模型在生态监测中泛化能力不足。

Method: 通过野外拍摄与网络爬取构建ShellfishNet数据集，并设计图像退化模拟测试，评估CNN、ViT、SSM、SSL及FGVC模型与多模态大模型的分类和描述能力。

Result: 80种模型在基准测试中表现各异，多模态大模型具备图像描述能力，退化测试揭示了模型在不同水下干扰下的鲁棒性差异。

Conclusion: ShellfishNet为底栖生物智能监测提供高质量数据基础与模型评估基准，推动AI在真实生态监测中的可靠性应用。
...
### [146] [SoLAR: Error-Resilient Streamable Long-Horizon Free-Viewpoint Video Reconstruction with Anchor Activation and Latent Recalibration](https://arxiv.org/abs/2605.07346)
*Haotian Zhang,Xu Mo,Yixin Yu,Guanhua Zhu,Jian Xue,Tongda Xu,Yan Wang,Jiaqi Zhang,Siwei Ma,Wen Gao*

Main category: cs.CV

TL;DR: 提出了首个针对长时自由视角视频（LFVV）的鲁棒可流式框架SoLAR，通过动态锚点激活和潜在差异重校准机制，在保持低存储开销的同时实现稳定高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理长时自由视角视频时性能显著下降，缺乏错误鲁棒且可流式的解决方案。

Method: 提出了锚点激活动态（AAD）机制来建模非刚性变换，以及潜在差异感知重校准（LaDAR）机制以抑制误差传播。

Result: 在长序列上实现了最先进的重建性能，并保持最小存储开销。
...
### [147] [TTF: Temporal Token Fusion for Efficient Video-Language Model](https://arxiv.org/abs/2605.07355)
*Simin Huo,Ning LI*

Main category: cs.CV

TL;DR: 提出Temporal Token Fusion (TTF)，一种无需训练、即插即用的令牌压缩框架，通过局部窗口相似性搜索融合冗余视觉令牌，在Qwen3-VL-8B上减少约67%令牌并保留99.5%准确率。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型中视觉令牌数量随视频长度急剧增加，导致LLM预填充阶段成为主要吞吐瓶颈，现有方法依赖全局相似性或注意力引导压缩，存在额外开销。

Method: TTF自动选择锚帧，对后续帧执行局部窗口相似性搜索（如3×3），融合超过阈值的令牌，并通过坐标对齐保持位置一致性。

Result: 在Qwen3-VL-8B上，阈值0.70时去除约67%视觉令牌，保留99.5%基线准确率，仅引入约0.16 GFLOPs匹配开销。

Conclusion: TTF提供了一种实用且高效的视频理解解决方案，无需训练即可集成到现有VLM流水线。
...
### [148] [UniD-Shift: Towards Unified Semantic Segmentation via Interpretable Share-Private Multimodal Decomposition](https://arxiv.org/abs/2605.07356)
*Shuai Zhang,Zhecheng Shi,Zhuxiao Li,Jing Ou,Tengxi Wang,Yuan Liu,Wufan Zhao*

Main category: cs.CV

TL;DR: 提出了一种统一的2D-3D联合语义分割框架UniD-Shift，通过分解共享和私有子空间实现跨模态对齐与融合。


<details>
  <summary>Details</summary>
Motivation: 激光雷达的稀疏采样与图像的几何畸变导致跨模态对齐困难，现有融合方法不稳定。

Method: 结合SAM视觉编码器和SPTNet几何编码器提取互补特征，显式分解为共享与私有子空间，并设计轻量注意力融合模块与正则化训练目标。

Result: 在SemanticKITTI和nuScenes上精度优于现有方法，计算效率有竞争力，且跨域泛化能力强。

Conclusion: 共享子空间有效实现语义对齐，私有子空间保留模态特异性，融合策略兼顾精度与效率。
...
### [149] [Disambiguating 2D-3D Correspondences in Gaussian Splatting-based Feature Fields for Visual Localization](https://arxiv.org/abs/2605.07351)
*Miso Lee,Sangeek Hyun,Yerim Jeon,Jae-Pil Heo*

Main category: cs.CV

TL;DR: 提出SplitGS-Loc框架，通过高斯分裂与多视图一致性约束，解决GSFF在视觉定位中像素-高斯对应模糊问题，实现无需逐场景训练的高精度定位。


<details>
  <summary>Details</summary>
Motivation: 高斯溅射特征场的光度优化导致像素到高斯的多对一映射，破坏PnP位姿估计稳定性，且产生缺乏多视图一致性的冗余高斯。

Method: 基于混合高斯模型的分裂方法将每个高斯分解为多个小高斯，建立一对一的像素-高斯对应；同时利用光栅化中的组合权重筛选关键高斯，通过强关联聚合判别特征。

Result: 在定位基准上达到最优性能，无需逐场景训练或迭代位姿优化。

Conclusion: 通过利用高斯属性，SplitGS-Loc将光度GSFF扩展为精确高效的定位工具，解决多对一映射与多视图不一致问题。
...
### [150] [UniISP: A Unified ISP Framework for Both Human and Machine Vision](https://arxiv.org/abs/2605.07359)
*Hanxi Li,Yao Cheng,Bo Zhang,Li Zeng*

Main category: cs.CV

TL;DR: 提出UniISP框架，同时满足人眼视觉和计算机视觉需求，通过混合注意力模块和特征适配器提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统ISP处理会引入信息损失，而现有Raw数据方法难以平衡视觉美观和任务性能。

Method: 设计混合注意力模块（HAM）进行监督学习，并引入特征适配器（Feature Adapter）传递ISP阶段特征到下流网络。

Result: 在多场景和多数据集上达到最优性能，验证了通用性和有效性。

Conclusion: UniISP能同时生成美观图像并保持特征完整性，适用于低光等挑战场景。
...
### [151] [RELO: Reinforcement Learning to Localize for Visual Object Tracking](https://arxiv.org/abs/2605.07379)
*Xin Chen,Chuanyu Sun,Jiao Xu,Houwen Peng,Dong Wang,Huchuan Lu,Kede Ma*

Main category: cs.CV

TL;DR: 提出RELO，用强化学习替代手工空间先验，实现基于奖励的视觉目标定位。


<details>
  <summary>Details</summary>
Motivation: 传统跟踪器使用手工空间先验（如热图），与IoU和AUC等评估指标不对齐，导致监督不充分。

Method: 将目标定位建模为马尔可夫决策过程，用强化学习学习定位策略，奖励结合帧级IoU和序列级AUC；引入层对齐时间令牌传播提升语义一致性。

Result: 在多个基准上表现优异，LaSOText上无需模板更新达57.5% AUC。

Conclusion: 奖励驱动定位可有效替代先验驱动定位，为视觉跟踪提供新方向。
...
### [152] [A Marine Debris Detection Framework for Ocean Robots via Self-Attention Enhancement and Feature Interaction Optimization](https://arxiv.org/abs/2605.07388)
*Yuyang Li,Jiashu Han,Yinyi Lai,Wenbin Kang,Zenghui Liu*

Main category: cs.CV

TL;DR: 提出YOLO-MD框架，通过DB-CASA模块、轻量级移位操作和SFG-Loss，提升海洋垃圾检测精度，在UODM数据集上达到0.849 mAP50。


<details>
  <summary>Details</summary>
Motivation: 现有海洋垃圾检测方法因图像模糊、背景复杂和小目标等问题性能下降，需提升检测能力以支持生态保护。

Method: 设计DB-CASA加强空间-通道交互；引入轻量级移位操作增强多尺度特征提取；提出SFG-Loss动态重加权解决类别不平衡。

Result: 在UODM数据集上精度0.875、F1-score 0.822、mAP50 0.849，优于现有方法，并在机器人边缘部署验证有效性。

Conclusion: YOLO-MD有效提升退化图像下的海洋垃圾检测性能，适用于实际机器人部署。
...
### [153] [ST-Gen4D: Embedding 4D Spatiotemporal Cognition into World Model for 4D Generation](https://arxiv.org/abs/2605.07390)
*Haonan Wang,Hanyu Zhou,Tao Gu,Luxin Yan*

Main category: cs.CV

TL;DR: 提出ST-Gen4D框架，融合全局与局部时空认知，实现结构合理且拓扑一致的4D生成。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成模型仅保证全局外观一致，缺乏局部动态拓扑，无法真实反映物理世界。

Method: 采用时空表示、认知图构建（全局外观+局部动态）、世界模型推理、潜在扩散生成四步融合。

Result: 在4D生成任务中优于现有方法，并构建了ST-4D数据集。

Conclusion: 深层集成4D认知与生成先验可有效提升时空结构的合理性与拓扑一致性。
...
### [154] [BalCapRL: A Balanced Framework for RL-Based MLLM Image Captioning](https://arxiv.org/abs/2605.07394)
*Shaokai Ye,Vasileios Saveris,Yihao Qian,Jiaming Hu,Elmira Amirloo,Peter Grasch*

Main category: cs.CV

TL;DR: 本文提出一种平衡的强化学习框架，联合优化图像描述的准确性、参考覆盖率和语言质量，通过GDPO风格奖励解耦和长度条件掩码，在多个模型上显著提升描述质量。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述强化学习方法和评估指标通常强调狭窄的质量概念，导致核心维度间的权衡，如效用导向目标鼓励噪声和幻觉，而竞技场目标偏向流畅但通用的描述。

Method: 采用GDPO风格的奖励解耦归一化处理连续多目标奖励，并引入长度条件奖励掩码，以优化联合效用感知正确性、参考覆盖率和语言质量的奖励公式。

Result: 在LLaVA-1.5-7B和Qwen2.5-VL 3B/7B模型上，方法一致提升描述质量，峰值增益包括+13.6 DCScore、+9.0 CaptionQA和+29.0 CapArena。

Conclusion: 提出的平衡强化学习框架能有效克服现有方法的权衡问题，显著改善图像描述的多维度质量。
...
### [155] [Exposing and Mitigating Temporal Attack in Deepfake Video Detection](https://arxiv.org/abs/2605.07398)
*Zheyuan Gu,Minghao Shao,Zhen Wang,Yusong Wang,Mingkun Xu,Shijie Zhang,Hao Jiang*

Main category: cs.CV

TL;DR: 提出SpInShield框架，通过解耦语义运动与频谱伪影，防御时空深度伪造检测器的逃避攻击。


<details>
  <summary>Details</summary>
Motivation: 现有时空深度伪造检测器虽AUC高，但易被逃避攻击利用，因其过拟合脆弱的时间频谱线索而非鲁棒语义因果关系。

Method: 设计可学习频谱对抗器动态合成严重频谱变形，采用捷径抑制优化策略，强制编码器提取可靠取证线索并清除不稳定频谱统计信息。

Result: 在常用数据集上获得竞争力性能，在模拟幅度频谱攻击下比最强基线AUC提升21.30个百分点。

Conclusion: SpInShield有效缓解时空伪造检测器的频谱攻击脆弱性。
...
### [156] [GPO-V: Jailbreak Diffusion Vision Language Model by Global Probability Optimization](https://arxiv.org/abs/2605.07399)
*Yu Pan,Andi Zhang,Yi Wang,Sibei Yang,Wenjie Wang*

Main category: cs.CV

TL;DR: 论文发现扩散视觉语言模型存在渐进式拒绝的安全漏洞，并提出针对性的GPO-V攻击框架，揭示非自回归生成架构的安全脆弱性。


<details>
  <summary>Details</summary>
Motivation: 扩散视觉语言模型看似对传统越狱攻击具有鲁棒性，但研究发现其渐进式拒绝模式暴露了潜在攻击面，需重新评估其安全性。

Method: 提出全局概率优化（GPO）范式，利用掩码扩散模型的去噪轨迹操纵全局生成动态，并扩展为针对视觉模态的GPO-V框架。

Result: GPO-V生成的攻击扰动具有隐蔽性和跨模型迁移性，成功突破扩散语言模型的安全防线。

Conclusion: 需从根本上重新评估当前针对扩散生成模型的安全防御策略，以应对独特的风险。
...
### [157] [InsHuman: Towards Natural and Identity-Preserving Human Insertion](https://arxiv.org/abs/2605.07402)
*Jie Li,Shulian Zhang,Yangyang Gao,Wenbo Li,Yulun Zhang,Yong Guo,Jian Chen*

Main category: cs.CV

TL;DR: 提出InsHuman框架，通过区域感知融合、人脸识别保持身份及双向数据配对策略，实现自然且保真的人体插入。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型在人体插入时存在姿势不当、人数不一致、身份改变等问题，且缺乏完整人体姿态与场景交互的数据集。

Method: 使用HBAF（人体-背景自适应融合）进行区域感知对齐，FFIP（人脸识别身份保持）匹配脸部特征，BDP（双向数据配对）构建高质量交互数据集。

Result: 在生成自然图像的同时有效保持人体身份不变，性能显著优于基线方法。

Conclusion: InsHuman通过融合策略与数据增强解决了人体插入中的身份保持与场景适配难题。
...
### [158] [ChartREG++: Towards Benchmarking and Improving Chart Referring Expression Grounding under Diverse referring clues and Multi-Target Referring](https://arxiv.org/abs/2605.07415)
*Tianhao Niu,Ziyu Han,Qingfu Zhu,Wanxiang Che*

Main category: cs.CV

TL;DR: 提出了一个全面的图表指代表达式定位基准，涵盖多种定位形式、多目标、多样化线索和图表类型，并引入代码驱动的合成管道生成像素级实例掩码，训练分割模型取得最优效果。


<details>
  <summary>Details</summary>
Motivation: 现有基准存在定位精度低、仅支持单/双目标、过度依赖文本线索、图表类型单一等问题。

Method: 构建系统化基准；设计代码驱动合成管道，利用绘图程序与渲染图元的对齐生成精确掩码；训练实例分割模型并集成到多模态定位框架。

Result: 多模态大模型在基准上表现差距显著，所提方法在基准和真实图表定位任务上持续超越基线。

Conclusion: 该基准和合成方法有效提升了图表指代表达定位的精度和泛化能力。
...
### [159] [Learning Image-Adaptive Scale Fields for Metric Depth Recovery](https://arxiv.org/abs/2605.07418)
*Yuanyan Li,Matthias Althoff*

Main category: cs.CV

TL;DR: 提出将单目深度估计的度量恢复问题形式化为图像自适应尺度场建模，通过语义和几何线索的线性组合从稀疏锚点恢复度量深度。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计的结果通常缺少度量尺度，实际应用中只有稀疏度量锚点时恢复准确度量深度至关重要且具有挑战性。

Method: 将深度校正分解为图像自适应基图的线性组合，利用MDE估计和中间表示中的语义与几何线索，通过最小二乘从稀疏锚点确定权重。

Result: 在多个数据集和代表性MDE模型上，本方法提高了度量深度精度，对极端稀疏锚点具有强鲁棒性，并提供了空间尺度变化的可解释分解。

Conclusion: 该方法有效且通用，能恢复准确的度量深度，并在稀疏锚点场景下表现稳健。
...
### [160] [Towards Photorealistic and Efficient Bokeh Rendering via Diffusion Framework](https://arxiv.org/abs/2605.07429)
*Linxiao Shi,Siming Zheng,Zerong Wang,Hao Zhang,Jinwei Chen,Bo Li,Shifeng Chen,Peng-Tao Jiang*

Main category: cs.CV

TL;DR: 提出MagicBokeh，一种基于扩散模型的统一框架，用于低质量手机图像的高质量散景渲染，同时提升分辨率和深度估计准确性。


<details>
  <summary>Details</summary>
Motivation: 现有移动设备因光学设计限制难以产生自然散景效果，且高变焦下图像分辨率低、细节丢失，而两阶段方法效率低且误差累积。

Method: 采用替代训练策略、焦点感知掩蔽注意机制和退化感知深度模块，联合优化散景渲染与超分辨率。

Result: 在真实低分辨率图像上高效生成照片级散景效果，显著提升控制性和视觉保真度。

Conclusion: MagicBokeh为未来散景渲染技术发展铺平道路，代码与模型已开源。
...
### [161] [Sparse Autoencoders as Plug-and-Play Firewalls for Adversarial Attack Detection in VLMs](https://arxiv.org/abs/2605.07447)
*Hao Wang,Yiqun Sun,Pengfei Wei,Lawrence B. Hsieh,Daisuke Kawahara*

Main category: cs.CV

TL;DR: 提出一种基于稀疏自编码器的轻量级对抗攻击检测框架SAEgis，用于视觉-语言模型的安全防护。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在现实应用中日益普及，但其安全性研究不足，现有模型极易受到对抗攻击，存在重大风险。

Method: 在预训练VLM中插入稀疏自编码器模块，通过标准重建目标训练，利用学习到的稀疏潜特征检测输入图像是否受到对抗扰动。

Result: 在域内、跨域和跨攻击设置下均取得强性能，特别是在跨域泛化上显著优于现有基线；多层级信号融合进一步提升鲁棒性和稳定性。

Conclusion: 首次探索SAE作为VLM对抗攻击检测的即插即用机制，无需额外对抗训练，开销小，为提升现实VLM系统安全性提供了实用方法。
...
### [162] [EditTransfer++: Toward Faithful and Efficient Visual-Prompt-Guided Image Editing](https://arxiv.org/abs/2605.07455)
*Lan Chen,Qi Mao,Yiren Song,Yuchao Gu,Siwei Ma*

Main category: cs.CV

TL;DR: EditTransfer++ 是一个结合逐步结构化训练和高效条件机制的框架，用于改进扩散变压器模型在视觉提示引导编辑中的忠实度和推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有扩散变压器方法因结构不匹配（如对文本条件的预训练偏置和随机采样不稳定）而难以忠实地复现示例编辑。

Method: 通过文本解耦训练消除文本条件，迫使模型依赖视觉证据；采用最好-最差对比度优化机制重塑去噪轨迹，减少不忠实生成；引入条件压缩与重复使用策略减少冗余，提升高分辨率编辑效率。

Result: 在现有基准和 EditTransfer-Bench 上，EditTransfer++ 实现了最先进的视觉提示忠实度，且推理速度显著快于先前方法。

Conclusion: EditTransfer++ 为可扩展的提示引导图像编辑和更广泛的视觉上下文学习提供了有希望的方向。
...
### [163] [EditRefiner: A Human-Aligned Agentic Framework for Image Editing Refinement](https://arxiv.org/abs/2605.07457)
...
TL;DR: 提出了EditRefiner框架，通过分层智能代理系统（感知-推理-行动-评估）改进文本引导图像编辑后的细粒度问题，并构建了包含15K图像和详细反馈的EditFHF-15K数据集。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导图像编辑模型存在细粒度问题（如不自然物体、光照不匹配），且现有改进方法成本高或空间定位弱，导致语义漂移和局部修正不可靠。

Method: 构建EditFHF-15K数据集；提出EditRefiner，包含感知、推理、行动、评估四个智能代理，实现类人的感知-推理-行动-评估闭环修正。

Result: EditRefiner在失真定位、诊断准确性和人类感知对齐上优于现有方法，建立了自纠正和感知可靠图像编辑的新范式。

Conclusion: 分层解释性代理框架能有效实现细粒度、可解释的编辑后修正，提升图像编辑的可靠性和质量。
...
### [164] [A Unified Framework for the Detection and Classification of Fatty Pancreas in Ultrasound Images](https://arxiv.org/abs/2605.07466)
...
TL;DR: 提出端到端框架，利用超声图像自动分类正常与脂肪胰，采用TransUNet分割、解剖引导补丁提取和成对纹理比较，模仿临床推理。


<details>
  <summary>Details</summary>
Motivation: 非酒精性脂肪胰病（NAFPD）诊断依赖主观视觉评估，缺乏自动分类工具。

Method: TransUNet分割（ResNet编码器+Transformer瓶颈）提取胰腺和脾静脉，解剖引导补丁提取，通过成对纹理比较实现患者级分类。

Result: SVM-RBF达到89.7%准确率，F1为0.898；无监督K-Means达到87.8%准确率。

Conclusion: 首个利用分割引导纹理分析实现超声脂肪胰自动分类的框架，即使无标签训练数据也能捕获临床信号。
...
### [165] [ForgeVLA: Federated Vision-Language-Action Learning without Language Annotations](https://arxiv.org/abs/2605.07474)
*Yuhao Zhou,Yunpeng Zhu,Yang Zhou,Jindi Lyu,Jian Lan,Zhangyuan Wang,Dan Si,Thomas Seidl,Qing Ye,Jiancheng Lyu*

Main category: cs.CV

TL;DR: 提出了一个联邦式VLA训练框架ForgeVLA，从分布式视觉-动作数据中学习模型，无需集中原始数据或人工标注，并解决了视觉-语言特征坍塌问题。


<details>
  <summary>Details</summary>
Motivation: VLA模型扩展受限于标注数据成本高，而部署的机器人产生大量异构视觉-动作数据，但无法集中聚合。

Method: 每个客户端使用具身指令分类器将视觉-动作对映射到预定义指令集形成三元组，结合客户端对比规划损失和服务器端自适应聚合策略。

Result: 在多个基准测试中显著优于其他基线方法，消融实验验证了各组件的贡献。

Conclusion: ForgeVLA有效利用分布式异构数据训练VLA模型，并缓解了特征坍塌问题。
...
### [166] [ReasonEdit: Towards Interpretable Image Editing Evaluation via Reinforcement Learning](https://arxiv.org/abs/2605.07477)
*Honghua Chen,Zitong Xu,Huiyu Duan,Xinyun Zhang,Xiongkuo Min,Guangtao Zhai*

Main category: cs.CV

TL;DR: 提出了ReasonEdit-22K数据集和RE-Reward奖励模型，基于GRPO算法训练ReasonEdit评估模型，实现可解释的图像编辑评估。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑评估方法依赖标量分数，缺乏可解释性，且缺少高质量解释数据集和有效奖励模型。

Method: 构建ReasonEdit-22K数据集（22K图像+113K CoT样本+1.3M人工判断），基于MLLM的RE-Reward奖励模型，并使用GRPO算法训练ReasonEdit。

Result: ReasonEdit在人类偏好对齐和跨基准泛化性上表现优异，能生成高质量可解释评估文本。

Conclusion: 该方法提升了图像编辑评估的透明度和可信度，有利于更可靠的评估。
...
### [167] [AudioFace: Language-Assisted Speech-Driven Facial Animation with Multimodal Language Models](https://arxiv.org/abs/2605.07478)
*Kai Zheng,Zejian Kang,Rui Mao,Hongyuan Zou,Yuanchen Fei,Xuanyang Xu,Xiangru Huang*

Main category: cs.CV

TL;DR: 提出了AudioFace框架，利用语言和语音学先验知识辅助语音驱动的面部动画生成。


<details>
  <summary>Details</summary>
Motivation: 直接映射语音到面部系数忽略了语言和语音结构，导致表情不准确。

Method: 结合多模态大模型的语言先验，引入文本和音素线索，将面部系数预测视为结构化生成问题。

Result: 在多个评估指标上取得优于现有方法的性能。

Conclusion: 语言辅助和多模态先验能有效提升语音驱动面部动画的准确性。
...
### [168] [Consensus Entropy: Harnessing Multi-VLM Agreement for Self-Verifying and Self-Improving OCR](https://arxiv.org/abs/2504.11101)
*Yulong Zhang,Tianyi Liang,Xinyue Huang,Erfei Cui,Guoqing Wang,Xu Guo,Chenhui Li,Gongshen Liu*

Main category: cs.CV

TL;DR: 提出共识熵（CE）作为无训练、模型无关的指标，通过模型间一致性熵评估OCR输出可靠性，并构建CE-OCR框架实现质量验证与性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在OCR任务中平均精度虽高，但难以检测样本级错误，且缺乏有效的无监督质量控制方法。

Method: 利用多个模型输出的共识熵（CE）度量一致性，基于CE设计CE-OCR框架，包括集成验证、最优输出选择及自适应路由优化效率。

Result: CE在质量验证中F1分数较VLM-as-Judge提升42.1%，CE-OCR在相同成本下持续优于自一致性和单模型基线。

Conclusion: CE无需训练与监督，可即插即用，为OCR质量验证提供高效可靠方案。
...
### [169] [Implicit Multi-Camera System Calibration Using Gaussian Processes](https://arxiv.org/abs/2605.07491)
*Ivan De Boi,Bart Ribbens,Veronika Golanova,Ursula Kapov,Simon Verspeek*

Main category: cs.CV

TL;DR: 提出基于高斯过程回归的多相机系统隐式标定框架，利用主动学习提升数据效率，并输出带统计置信区间的3D预测。


<details>
  <summary>Details</summary>
Motivation: 传统显式标定方法受限于刚性数学模型，难以处理复杂非线性畸变；神经网络隐式方法数据需求大且缺乏不确定性量化。

Method: 采用高斯过程回归直接学习2D图像坐标到3D世界坐标的非线性映射，结合主动学习利用预测不确定性引导数据采集。

Result: 实验表明靠近相机的区域3D预测不确定性更高，即使该区域在3D空间中数据点不稀疏，但在uv坐标空间更稀疏。

Conclusion: 该方法提供了鲁棒、数据高效且可靠的标定方案，特别适用于标定数据采集受限的复杂多相机系统场景。
...
### [170] [DIMoE-Adapters: Dynamic Expert Evolution for Continual Learning in Vision-Language Models](https://arxiv.org/abs/2605.07494)
*Mengxin Qin,Xiang Zhang,Xi Wang,Kun Wei,Xu Yang,Cheng Deng*

Main category: cs.CV

TL;DR: 提出动态增量混合专家适配器框架，通过自校准专家演化和原型引导专家选择平衡稳定性-可塑性矛盾，在多个领域任务增量学习中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 多领域任务增量学习中领域偏移加剧稳定性-可塑性困境，现有固定架构和静态参数分配方法限制新领域适应且加剧灾难性遗忘。

Method: 提出DIMoE-Adapters框架，包含自校准专家演化（SCEE）通过优化动力学构建稀疏专家池提升可塑性，以及原型引导专家选择（PGES）基于专家池控制利用率提升稳定性。

Result: 在多种设置下，所提方法超越先前最先进方法。

Conclusion: 动态专家演化范式有效平衡了视觉语言模型持续学习中的稳定性与可塑性，减少冗余容量同时缓解灾难性遗忘。
...
### [171] [How Far Is Document Parsing from Solved? PureDocBench: A Source-TraceableBenchmark across Clean, Degraded, and Real-World Settings](https://arxiv.org/abs/2605.07492)
...
TL;DR: 本文提出PureDocBench，一个程序化生成的文档解析基准测试，包含1475页、4425张图像，覆盖10个领域，并评估40个模型，发现最佳模型仅得74分，公式识别为共同瓶颈，且通用VLM对退化更稳健。


<details>
  <summary>Details</summary>
Motivation: 现有OmniDocBench基准存在标注错误（12.08%）和污染风险，导致排名不可靠，需要更可靠且可追溯的基准。

Method: 从HTML/CSS程序化生成文档图像，再渲染为干净、数字退化、真实退化三个版本，并自动生成可验证标注。

Result: 最佳模型得分仅约74/100，模型间差距达44.6分；专业解析器在小参数下媲美大模型，但公式识别是瓶颈；通用VLM在退化场景下比流水线专家损失更少，导致排名反转。

Conclusion: 文档解析远未解决，现有基准误导性强，需要更全面的评估框架。
...
### [172] [Cloud-top infrared observations reveal the four-dimensional precipitation structure](https://arxiv.org/abs/2605.07499)
...
TL;DR: 发现云顶红外观测可编码四维降水结构信息，提出物理约束深度学习框架4DPrecipNet实现全球连续降水监测。


<details>
  <summary>Details</summary>
Motivation: 现有全球尺度缺乏高精度四维降水观测，传统理论认为红外遥感仅能观测云顶特性。

Method: 采用水分优先物理约束的深度学习框架，联合多通道红外辐射和雷达降水廓线重建降水系统时空演变。

Result: 框架成功捕获深对流结构及其演化，在大样本和独立雷达验证中表现稳健。

Conclusion: 云顶红外观测物理编码了云下降水信息，为全球降水结构连续监测开辟新路径。
...
### [173] [Lightweight Unpaired Smartphone ISP Transfer with Semantic Pseudo-Pairing](https://arxiv.org/abs/2605.07495)
*Yujin Cho,Flavien Armangeon,Yanhao Li*

Main category: cs.CV

TL;DR: 提出一种用于非配对智能手机ISP问题的轻量级方法，通过语义匹配构建伪配对数据，训练仅有7K参数的CNN，在NTIRE 2026挑战中取得优秀结果。


<details>
  <summary>Details</summary>
Motivation: 非配对RAW与RGB图像因场景和颜色对齐困难，现有方法依赖配对数据或对抗训练，后者不稳定。

Method: 先对训练块重建全局图像，用DINOv2提取语义嵌入，通过融合Gromov-Wasserstein最优传输在图像和块级别构建伪配对，再训练轻量CNN（7K参数）进行色彩渲染。

Result: 最终隐藏测试集上PSNR 22.569、SSIM 0.675、ΔE 8.067，显著优于基线，SSIM和ΔE排名第三。

Conclusion: 该框架有效缓解非配对问题，小型网络结构稳定，可减少伪影，适合无配对数据的ISP任务。
...
### [174] [Implicit Preference Alignment for Human Image Animation](https://arxiv.org/abs/2605.07545)
*Yuanzhi Wang,Xuhua Ren,Jiaxiang Cheng,Bing Ma,Kai Yu,Tianxiang Zheng,Qinglin Lu,Zhen Cui*

Main category: cs.CV

TL;DR: 提出隐式偏好对齐（IPA）框架，无需配对偏好数据即可优化手部生成质量，结合手部感知局部优化提升高保真效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法生成手部动画时面临自由度大、运动复杂的问题，而构建严格偏好数据对成本高且不可行。

Method: 基于隐式奖励最大化的理论，通过最大化自生成高质量样本似然并惩罚偏离预训练先验，结合手部感知局部优化机制。

Result: 实现了有效的偏好优化以提升手部生成质量，并显著降低了偏好数据的构建门槛。

Conclusion: IPA框架为手部动画生成提供了一种数据高效的解决方案，促进高保真手部运动的对齐优化。
...
### [175] [Response-G1: Explicit Scene Graph Modeling for Proactive Streaming Video Understanding](https://arxiv.org/abs/2605.07575)
*Ke Ma,Jiaqi Tang,Bin Guo,Xueting Han,Ruonan Xu,Qingfeng He,Ziheng Wang,Xu Wang,Qifeng Chen,Zhiwen Yu,Yunhao Liu*

Main category: cs.CV

TL;DR: 提出Response-G1框架，通过场景图对齐视频证据与查询条件，实现流式视频的主动理解。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs在流式视频理解中缺乏查询感知的显式证据建模，导致响应时机不准确。

Method: 三阶段无微调框架：在线查询引导的场景图生成、记忆检索历史场景图、检索增强的触发提示决策。

Result: 在主动和被动任务基准上均优于现有方法，验证了显式场景图建模与检索的有效性。

Conclusion: 显式场景图表示和检索机制能显著提升流式视频理解的响应时序准确性和可解释性。
...
### [176] [Diffusion-APO: Trajectory-Aware Direct Preference Alignment for Video Diffusion Transformers](https://arxiv.org/abs/2605.07503)
*Jingyuan Zhu,Biaolong Chen,Le Zhang,Aixi Zhang,Hao Jiang,Pipei Huang*

Main category: cs.CV

TL;DR: 提出了一种轨迹感知的对齐偏好优化算法（Diffusion-APO），解决了大规模视频扩散模型与人类意图对齐中的噪声分布与推理轨迹不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法如DPO和GRPO受到复杂奖励模型偏差或非最优时间步采样的限制，难以高效对齐。

Method: 设计了Diffusion-APO算法，同步训练噪声与推理去噪路径以最大化梯度信号；并构建了统一的模块化RLHF框架（含在线排序、半在线锚定、离线优化和蒸馏漂移校正）。

Result: 在视觉质量和指令遵循上持续超越基线，并在模型加速时保持生成保真度。

Conclusion: Diffusion-APO提供了一种稳健、端到端的可扩展视频扩散对齐方案。
...
### [177] [Hierarchical Dual-Subspace Decoupling for Continual Learning in Vision-Language Models](https://arxiv.org/abs/2605.07512)
*Mengxin Qin,Xiang Zhang,Kun Wei,Xu Yang,Cheng Deng*

Main category: cs.CV

TL;DR: 提出HDSD框架，通过层次化双子空间解耦减少视觉语言模型类增量学习中的子空间干扰和灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略参数更新的高维子空间结构，导致跨任务子空间干扰和严重遗忘。

Method: 设计轻量特征调制模块（FMM）分离通用与任务专属子空间，结合通用融合模块（GFM）自适应提取稳定知识，以及层次学习模块（HLM）通过SVD约束子空间更新尺度。

Result: 在标准基准上达到最优性能。

Conclusion: HDSD有效缓解子空间干扰，实现持续学习新知识同时保留旧知识。
...
### [178] [SAM 3D Animal: Promptable Animal 3D Reconstruction from Images in the Wild](https://arxiv.org/abs/2605.07604)
*Xuyi Hu,Jin Lyu,Jiuming Liu,Yebin Liu,Silvia Zuffi,Liang An,Stefan Goetz*

Main category: cs.CV

TL;DR: 提出首个可提示的多动物3D重建框架SAM 3D Animal，基于SMAL+模型，结合新数据集Herd3D，在多个数据集上达到最优。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单动物场景，难以处理野生动物重建中物种多样、遮挡频繁及多动物共存等挑战。

Method: 基于SMAL+参数模型，支持关键点和掩码等灵活提示，联合重建多个实例；并引入多动物3D数据集Herd3D增强训练多样性。

Result: 在Animal3D、APTv2和Animal Kingdom数据集上，超越现有基于模型和无模型方法，取得最优结果。

Conclusion: 该方法为野外场景下可提示的多动物3D重建提供了可扩展且有效的解决方案。
...
### [179] [InterLV-Search: Benchmarking Interleaved Multimodal Agentic Search](https://arxiv.org/abs/2605.07510)
*Bohan Hou,Jiuning Gu,Jiayan Guo,Ronghao Dang,Sicong Leng,Xin Li,Xuemeng Song,Jianfei Yang*

Main category: cs.CV

TL;DR: 提出了InterLV-Search基准，用于评估交错的文本-视觉智能体搜索，包含2061个样本和三个难度级别，当前最佳模型准确率低于50%。


<details>
  <summary>Details</summary>
Motivation: 现有基准将视觉证据限制在输入或答案端点，无法处理搜索过程中交错的文本和视觉证据。

Method: 构建三级基准：Level 1自动主动视觉证据寻找，Level 2自动离线交错多模态搜索，Level 3机器引导人工监督的开放网络搜索，并提供标准化工具InterLV-Agent。

Result: 现有最先进多模态智能体在InterLV-Search上总体准确率低于50%，在视觉证据寻找、搜索控制和多模态证据整合方面面临挑战。

Conclusion: 交错多模态搜索任务对当前系统仍然困难，需要进一步研究以提升视觉证据整合和搜索控制能力。
...
### [180] [LithoBench: Benchmarking Large Multimodal Models for Remote-Sensing Lithology Interpretation](https://arxiv.org/abs/2605.07640)
*Jun Wang,Fengpeng Li,Hang Dong,Tianjin Huang,Wei Han*

Main category: cs.CV

TL;DR: 提出了LithoBench，一个多级别遥感岩性解释基准，包含10000个专家标注实例和五个认知层次任务，评估大型视觉语言模型在地质语义理解上的能力。


<details>
  <summary>Details</summary>
Motivation: 遥感岩性解释对地质调查至关重要，但缺乏能捕获岩性标注、多层次地质语义及专家评估的基准。

Method: 开发了专家参与的半自动构建流程，结合结构化地质图像描述等多子过程，构建涵盖多项选择和开放性任务的基准。

Result: 实验显示大型视觉语言模型在地质语义理解上存在显著局限，尤其是高阶解释、应用和推理任务。

Conclusion: LithoBench为评估多模态模型在地质遥感解释中的能力提供了有效基准，并揭示了当前模型的不足。
...
### [181] [Mind the Gap: Geometrically Accurate Generative Reconstruction from Disjoint Views](https://arxiv.org/abs/2605.07550)
*Grzegorz Wilczynski,Mikołaj Zielinski,Bartosz Świrta,Dominik Belter,Przemysław Spurek*

Main category: cs.CV

TL;DR: 提出了一种无需视角重叠的3D重建新范式GLADOS，通过生成中间视角、粗略重建和迭代优化来解决分散视图重建问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉系统依赖视角重叠，在机器人集群或众包数据采集等场景下难以应用。

Method: GLADOS框架分为三阶段：生成性桥接、鲁棒粗略3D重建、迭代上下文扩展与一致性优化。

Result: 现有方法在零重叠场景下完全失效，而GLADOS能生成连贯的3D几何结构。

Conclusion: GLADOS是一种通用的模块化框架，可整合未来生成、重建和修复技术的进步。
...
### [182] [VIMCAN: Visual-Inertial 3D Human Pose Estimation with Hybrid Mamba-Cross-Attention Network](https://arxiv.org/abs/2605.07552)
*Zepeng Yang,Junxuan Bai,Hao Li,Ju Dai,Junjun Pan,Yongfeng Yin,Bin Li*

Main category: cs.CV

TL;DR: 提出VIMCAN混合架构，结合Mamba的高效序列建模与Cross-Attention的空间推理，实现实时多模态3D人体姿态估计，并在TotalCapture和3DPW上取得SOTA精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的3D人体姿态估计方法因二次复杂度难以处理长序列实时任务，而Mamba虽高效但缺乏对多模态空间依赖的捕捉能力。

Method: 混合了Mamba的动态参数化时间建模与Cross-Attention的空间依赖提取，融合RGB关键点与IMU数据进行鲁棒的视觉-惯性联合姿态估计。

Result: 在TotalCapture上MPJPE达17.2mm，3DPW上45.3mm，超越Transformer及其他SOTA方法，并在消费级硬件上支持>60 FPS的实时推理。

Conclusion: Mamba与Attention的混合架构能有效平衡效率与精度，适用于实时多模态人体姿态估计。
...
### [183] [GazeVLM: Active Vision via Internal Attention Control for Multimodal Reasoning](https://arxiv.org/abs/2605.07817)
*Brown Ebouky,Gabriele Carrino,Niccolo Avogaro,Christoph Studer,Andrea Bartezzaghi,Mattia Rigotti*

Main category: cs.CV

TL;DR: 提出GazeVLM架构，通过自主生成注视令牌实现元认知控制的视觉注意力，提升高分辨率多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM被动处理视觉信息，导致空间推理弱化和语言幻觉，需改进注意力机制。

Method: 引入注视令牌和因果注意力掩码，通过组相对策略优化训练，实现自顶向下的选择性注意和模拟中央凹注视。

Result: 4B参数模型在HRBench-4k和HRBench-8k上超越同类VLM近4%，优于基于图像的代理推理管线超过5%。

Conclusion: GazeVLM有效融合元认知控制与视觉推理，无需外部裁剪或额外令牌，提升高分辨率场景下的性能。
...
### [184] [APEX: Assumption-free Projection-based Embedding eXamination Metric for Image Quality Assessment](https://arxiv.org/abs/2605.07786)
*Caterina Gallegati,Monica Bianchini,Franco Scarselli,Vittorio Murino,Barbara Toniella Corradini*

Main category: cs.CV

TL;DR: APEX是一个基于切片瓦瑟斯坦距离的无假设图像评估框架，利用CLIP和DINOv2作为特征提取器，克服了传统指标（如FID）的特征瓶颈和参数偏差，在视觉退化鲁棒性和跨数据集稳定性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统图像评估指标（如FID）受限于过时特征的封闭词汇表和刚性参数假设，而现有替代方法虽解决了特征瓶颈，但仍受参数限制，促使开发更鲁棒的评估框架。

Method: 采用切片瓦瑟斯坦距离作为无假设相似性度量，结合开放词汇的CLIP和DINOv2特征提取器，构建一个嵌入无关且可扩展至高维空间的评估框架。

Result: APEX在视觉退化鲁棒性上优于现有基线，并在跨数据集和数据集内评估中表现出高度稳定性，适用于领域外数据。

Conclusion: APEX通过无参数假设和开放词汇设计，有效解决了传统指标的瓶颈，为生成模型提供了更可靠和鲁棒的评估手段。
...
### [185] [Towards Billion-scale Multi-modal Biometric Search](https://arxiv.org/abs/2605.07655)
*Arka Koner,Chetan S. Naik,Lokesh Kurre,Vivek Raghavan,Barada P. Sabut,Tanusree Deb Barma,Anoop M. Namboodiri,Anil K. Jain*

Main category: cs.CV

TL;DR: 本文介绍了印度首个基于开源架构的大规模多模态生物特征搜索系统Bharat ABIS，该系统中指纹、人脸和虹膜模态的嵌入式特征经级联形成13.5KB/人的模板，并在1.55亿记录中实现2000万级搜索及0.3% FNIR@0.5% FPIR的性能。


<details>
  <summary>Details</summary>
Motivation: 针对国家级身份系统中的十亿级多模态生物特征数据库检索面临的采集、处理、匹配速度及防攻击等多方面极限挑战，缺乏公开研究能揭示如此大规模搜索系统的实现细节。

Method: 采用模块化开源架构，对指纹、人脸和虹膜分别进行预处理（分割）、质量评估、显示攻击检测和特征嵌入学习；通过级联各模态模板构建13.5KB/人的最终表示。

Result: 在印度Aadhaar数据库中随机提取的2.2亿身份的按人口学分层样本上，成人指纹录入误拒率（FNIR）为0.3%，误认率（FPIR）为0.5%；单服务器（8块NVIDIA H100 GPU、2TB RAM）上能实现4000万库容下每秒100次搜索的吞吐量。

Conclusion: 基于开源架构的Bharat ABIS能够在十亿级多模态识别场景下实现高效、准确的1:N搜索（去重），并在2000万级测试中超越三款商用系统，为国家级身份系统的实际部署提供了参考。
...
### [186] [Probabilistic Object Detection with Conformal Prediction](https://arxiv.org/abs/2605.07549)
*Christopher Ries,Moussa Kassem Sbeyti,Nicolas Bianco,Nadja Klein*

Main category: cs.CV

TL;DR: 本文系统地研究了缩放与非缩放共形预测在多类目标检测中的应用，提出了一种结合Bonferroni校正和概率目标检测器不确定性估计的方法，并构建了两步管道处理类别预测集。在自动驾驶数据集上，缩放CP显著提高了间隔紧致性，最高IoU提升19%，区间分数降低39%，且不牺牲覆盖率。


<details>
  <summary>Details</summary>
Motivation: 目标检测存在结构化多输出，经典CP理论难以直接应用；未缩放CP生成固定宽度区间，对低不确定性预测不必要地宽；此前缺乏对缩放与非缩放CP在多类目标检测中的系统比较，也未与互补的不确定性量化方法集成。

Method: 对边界框角点应用坐标层面的CP，并用Bonferroni校正实现框级保证；使用概率目标检测器（通过损失衰减训练）获得的逐预测偶然不确定性估计来缩放区间，并评估未校准和两种校准变体；扩展为两步管道，先用RAPS构建类别预测集，再对条件化边界框进行共形化。

Result: 缩放CP在KITTI、BDD、CODA数据集上（包括跨域分布偏移场景）始终优于未缩放CP，最高IoU提升19%，区间分数降低39%，且不牺牲覆盖率。类别校准进一步改善了两种变体的覆盖率，对紧致性影响可忽略。

Conclusion: 缩放CP结合类别校准能提供更实用的不确定性估计，适用于实时、真实世界的目标检测，尤其在分布偏移下表现稳健。
...
### [187] [Divide and Conquer: Object Co-occurrence Helps Mitigate Simplicity Bias in OOD Detection](https://arxiv.org/abs/2605.07821)
*Boyang Dai,Chaoqi Chen,Yizhou Yu*

Main category: cs.CV

TL;DR: 提出基于对象共现模式的OOD检测框架，通过解耦表示与分治策略提升近OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略图像中丰富的上下文信息，难以区分近OOD样本；受人类视觉系统启发，需利用对象共现模式进行场景理解。

Method: 学习预测解耦表示以捕捉对象共现模式，根据训练数据中的共现模式划分三种场景，采用分治策略进行检测。

Result: 在挑战性和全频谱OOD场景中取得竞争性结果，能有效处理语义偏移和协变量偏移。

Conclusion: 对象共现模式可有效提升近OOD检测能力，避免模型仅关注简单易学区域。
...
### [188] [Multimodal Stepwise Clinically-Guided Attention Learning for Pathological Complete Response Prediction in Breast Cancer](https://arxiv.org/abs/2605.07561)
*Alice Natalina Caragliano,Valerio Guarrasi,Michela Gravina,Carlo Sansone,Paolo Soda*

Main category: cs.CV

TL;DR: 提出了一种多模态逐步临床引导注意力学习框架，用于乳腺癌MRI的pCR预测，通过医学空间引导和多模态集成克服类别不平衡和泛化性差的问题。


<details>
  <summary>Details</summary>
Motivation: 预先pCR预测因严重类别不平衡和跨临床环境泛化性有限而具有挑战性。

Method: 逐步训练策略：先学习全局判别影像模式，再引入注意力机制约束网络关注肿瘤区域，最后整合临床变量优化决策。

Result: 外部验证显示，相比非引导单阶段基线，该方法提高了敏感性并保持了特异性，生成了解剖学一致的注意力图。

Conclusion: 临床引导的多模态注意力学习有助于实现稳健且可泛化的乳腺癌pCR预测。
...
### [189] [Dynamic Mode Decomposition along Depth in Vision Transformers](https://arxiv.org/abs/2605.07556)
*Nishant Suresh Aswani,Saif Eddin Jabari*

Main category: cs.CV

TL;DR: 本文发现ViT的深度可通过单一线性算子K近似模拟，在短距离内预测精度高，但下游任务中线性近似效果退化。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明ViT块可被线性映射替代并呈现循环计算阶段，本文旨在验证这两者是否一致，即ViT深度是否实现近似自治线性动力学。

Method: 使用动态模式分解（DMD）从连续隐藏状态对中拟合线性算子K，并通过K^p预测p步后的状态。在四个预训练DINO ViT模型上测试正则化、秩和校准预算。

Result: 短程内（p≤4）K^p与端到端映射的余弦相似度误差低于0.02，且能恢复中间激活；早期层算子秩较低且对校准数据需求少，但随深度增加线性性衰减；下游任务中恒等映射基线表现相当。

Conclusion: ViT的局部线性近似有效但无法扩展到下游任务，深度增加后线性性显著降低。
...
### [190] [Beyond GSD-as-Token: Continuous Scale Conditioning for Remote Sensing VLMs](https://arxiv.org/abs/2605.07562)
*Song Zhang,Yanlong Chen,Yilin Li,Yining Chen,Zili Yi,Xiaowei Zhang,Yawei Li*

Main category: cs.CV

TL;DR: 提出ScaleEarth框架，通过GSD条件化连续控制模型计算路径，结合CS-HLoRA和SSE-U模块，在遥感视觉语言任务中取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有RS-VLMs忽略或离散化处理地面采样距离（GSD），导致模型无法适应多尺度地理对象的视觉差异。

Method: 基于Qwen3-VL，使用CS-HLoRA（GSD驱动门控调制LoRA子空间）和SSE-U（异方差子头预测GSD及其不确定性），配合GeoScale-VQA语料库训练。

Result: 在XLRS-Bench和OmniEarth-Bench等遥感基准上达到最优性能。

Conclusion: 将GSD作为连续条件参数可有效提升RS-VLMs的尺度适应性，且方法-数据闭环设计是关键。
...
### [191] [Video Understanding Reward Modeling: A Robust Benchmark and Performant Reward Models](https://arxiv.org/abs/2605.07872)
*Yuancheng Wei,Linli Yao,Lei Li,Haojie Zhang,Hao Zhou,Fandong Meng,Xu Sun*

Main category: cs.CV

TL;DR: 提出了一个包含基准、数据和训练的通用框架，构建了 VURB 基准和 VUP-35K 数据集，训练了视频奖励模型 VideoDRM 和 VideoGRM，在多个基准上达到了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 视频理解奖励建模因缺乏稳健评估基准和高质量偏好数据而进展缓慢，现有方法无法满足视频任务的需求。

Method: 设计了 VURB 基准（含 2,100 个偏好对和平均 1,143 token 的长思维链推理），通过自动化管道构建了 VUP-35K 数据集，并基于此训练了判别式（VideoDRM）和生成式（VideoGRM）奖励模型。

Result: VideoDRM 和 VideoGRM 在 VURB 和 VideoRewardBench 上达到最佳性能；VUP-35K 同时提升了奖励性能和模型推理能力，并在 test-time scaling（N 中选优）中带来显著提升。

Conclusion: 所提框架有效推动了视频奖励建模的发展，高质量数据和模型设计是实现性能提升的关键。
...
### [192] [Semantic-Aware Adaptive Visual Memory for Streaming Video Understanding](https://arxiv.org/abs/2605.07897)
*Hang Wu,Sherin Mary Mathews,Yujun Cai,Ming-Hsuan Yang,Yiwei Wang*

Main category: cs.CV

TL;DR: 提出了一种无需训练的双阶段框架SAVEMem，通过语义感知内存生成和查询自适应检索，在固定内存预算下提升流视频理解性能，同时降低GPU内存消耗。


<details>
  <summary>Details</summary>
Motivation: 现有方法在流视频理解中，内存压缩缺乏语义信号，检索与压缩阶段难以协调，导致长视频处理效率低下。

Method: 采用双阶段框架：阶段1构建三层流式内存，通过固定伪问题库提供语义先验；阶段2进行查询感知检索，使用锚定条件时间门自适应调整检索范围，并利用后期交互选择候选帧。

Result: 在OVO-Bench上提升10.42分，在StreamingBench和ODV-Bench上也有提升，且将Qwen2.5-VL的GPU峰值内存降低48%。

Conclusion: SAVEMem通过引入语义感知和查询自适应检索，有效解决了流视频理解中的内存管理问题，提升了性能并降低了资源消耗。
...
### [193] [PolarVLM: Bridging the Semantic-Physical Gap in Vision-Language Models](https://arxiv.org/abs/2605.07574)
*Yuliang Li,Chu Zhou,Heng Guo,Boxin Shi,Imari Sato,Zhanyu Ma*

Main category: cs.CV

TL;DR: 介绍PolarVLM，首个将偏振物理参数融入视觉语言模型的多模态框架，解决标准RGB在光学模糊场景中的局限性，并在反射和透明物体任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 标准视觉语言模型受限于RGB输入，难以处理反射和透明物体等光学模糊问题，而现有偏振成像方法无法进行开放式推理。

Method: 采用双流架构和渐进式两阶段训练策略，结合新构建的偏振感知VQA基准PolarVQA（75K指令调优对）。

Result: 在五个评估任务中整体优于RGB基线25.4%，反射识别提升26.6%，玻璃计数提升34.0%。

Conclusion: 成功实现物理感知语义理解，弥合了物理参数与语义推理之间的鸿沟。
...
### [194] [Beyond Defenses: Manifold-Aligned Regularization for Intrinsic 3D Point Cloud Robustness](https://arxiv.org/abs/2605.07590)
*Pedro Alonso,Chongshou Li,Tianrui Li*

Main category: cs.CV

TL;DR: 提出MAPR框架，通过将潜在几何与内在几何对齐来提升3D点云鲁棒性，无需对抗训练或额外数据。


<details>
  <summary>Details</summary>
Motivation: 发现3D网络脆弱性源于潜在几何与内在几何的流形错位，小扰动常导致特征空间大幅扭曲。

Method: 引入内在特征（曲率、扩散结构），并使用一致性损失对齐内在扰动的预测。

Result: 在ModelNet40和ScanObjectNN上平均鲁棒性分别提升20.02%和8.58%。

Conclusion: 对齐潜在与内在几何可有效提升3D点云鲁棒性。
...
### [195] [Tracing the Arrow of Time: Diagnosing Temporal Information Flow in Video-LLMs](https://arxiv.org/abs/2605.07568)
*Peitao Han,Fei Cheng,Lis K. Pereira,Qianying Liu,Shigeru Kitazawa*

Main category: cs.CV

TL;DR: 本文发现现有时序推理瓶颈在于投影器，Q-Former会破坏时序信息，而MLP投影可保留时序信息，且通过时序感知编码器和AoT监督，最终在AoT_{PPB}上超越人类表现。


<details>
  <summary>Details</summary>
Motivation: 人类在箭头时间任务中表现完美，但视频大语言模型仅略高于随机水平，这引发了时序信息瓶颈在架构中具体位置的问题。

Method: 通过分离视觉编码器，追踪时序信息在编码器、投影器和语言模型中的流动，系统分析各组件对时序信息的保留能力。

Result: 视频中心编码器编码强时序信号，但标准架构中性能下降；Q-Former破坏时序信息，MLP投影改善；最终模型在AoT_{PPB}达98.1%准确率，并提升后续时序推理任务。

Conclusion: 视频大语言模型的时序推理需要有效的时序编码和可靠的时序信息传递，投影器设计是关键的瓶颈环节。
...
### [196] [One Token Per Frame: Reconsidering Visual Bandwidth in World Models for VLA Policy](https://arxiv.org/abs/2605.07931)
*Zuojin Tang,Shengchao Yuan,Xiaoxin Bai,Zhiyuan Jin,De Ma,Gang Pan,Bin Liu*

Main category: cs.CV

TL;DR: 提出了一种名为OneWM-VLA的视觉-语言-动作模型，通过将每帧视觉流压缩为单一语义令牌，并利用流匹配目标统一处理潜在流与动作轨迹，在减少视觉带宽的同时显著提升了长程任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型增强的VLA模型在冻结骨干网络的有限适应预算下，每帧视觉表示带宽过高且潜在动作耦合未被充分优化，导致性能受限。

Method: 通过自适应注意力池化将每个视角压缩为每帧单个语义令牌，并使用单一流匹配目标生成潜在流和动作轨迹，而非通过独立解码器连接。

Result: 在MetaWorld MT50上平均成功率从47.9%提升至61.3%，在LIBERO-Long上达95.6%（基线85.2%），在真实Piper机械臂的变形任务Fold Cloth上达60.0%（基线20.0%）。

Conclusion: 每帧视觉带宽可降至单令牌而不损失长程性能，证明了压缩表示在VLA中的有效性。
...
### [197] [SCOPE: Structured Decomposition and Conditional Skill Orchestration for Complex Image Generation](https://arxiv.org/abs/2605.08043)
...
TL;DR: 本文提出SCOPE框架，通过持续追踪语义承诺实现复杂文本到图像生成，在Gen-Arena等基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在忠实实现复杂视觉意图时存在概念断层，即语义承诺在生成周期中无法保持一致的标识。

Method: 提出SCOPE框架，维护结构化规范并条件触发检索、推理和修复技能，同时构建Gen-Arena基准和EGIP评估标准。

Result: SCOPE在Gen-Arena上EGIP达0.60，在WISE-V和MindBench上分别达0.907和0.61，显著优于基线。

Conclusion: 持续追踪语义承诺有效提升了复杂图像生成的意图实现能力。
...
### [198] [TimeLesSeg: Unified Contrast-Agnostic Cross-Sectional and Longitudinal MS Lesion Segmentation via a Stochastic Generative Model](https://arxiv.org/abs/2605.07955)
...
TL;DR: TimeLesSeg是一个统一的无对比框架，通过单一卷积神经网络处理有无时间维度的输入，用于多发性硬化症（MS）病变分割，其生成式管道模拟病变演化，并在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: MS的临床和放射学异质性导致病变分割困难，现有深度学习方法对分布变化和输入结构敏感，且横截面与纵向方法存在分割，因此需要统一框架。

Method: 提出TimeLesSeg，使用病变掩膜模拟病理先验，利用空掩膜支持横截面处理；通过形态学操作和GMM域随机化生成纵向数据和对比无关性。

Result: 在五个数据集上，TimeLesSeg在单模态输入时优于对比无关的现有技术；纵向处理中超越SAMSEG，并比SAMSEG和LST-AI更准确捕捉病变负荷动态。

Conclusion: TimeLesSeg能够有效处理不同输入模式，提升MS病变分割的鲁棒性和准确性，且代码开源。
...
### [199] [TraceAV-Bench: Benchmarking Multi-Hop Trajectory Reasoning over Long Audio-Visual Videos](https://arxiv.org/abs/2605.07593)
*Hengyi Feng,Hao Liang,Mingrui Chen,Bohan Zeng,Meiyi Qiang,Zhengyang Zhao,Zimo Meng,Zeang Sheng,Wentao Zhang*

Main category: cs.CV

TL;DR: 提出了TraceAV-Bench，首个评估长音频-视觉多跳推理和多模态幻觉鲁棒性的基准，包含2200个问题、578个长视频（共339.5小时），平均3.68跳推理链、15.1分钟时间跨度。最强闭源模型仅达68.29%准确率，开源模型51.70%。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估短视频、单模态或单跳感知，无法测试真实世界中基于稀疏、时间分散的跨模态证据链进行多跳推理的能力。

Method: 采用三步半自动化流程构建数据集，包含2200个选择题，覆盖4个评估维度和15个子任务，每个问题附带显式推理链（平均3.68跳）。

Result: 多个OmniLLM在TraceAV-Bench上表现不佳：Gemini 3.1 Pro达68.29%，Ming-Flash-Omni-2.0达51.70%。多模态幻觉鲁棒性与通用推理性能基本解耦。

Conclusion: TraceAV-Bench为长音频-视觉内容推理提出了显著挑战，且幻觉鲁棒性独立于推理能力，有望推动OmniLLM在长时跨模态推理方面的研究。
...
### [200] [FS-I2P:A Hierarchical Focus-Sweep Registration Network with Dynamically Allocated Depth](https://arxiv.org/abs/2605.07607)
*Zhixin Cheng,Yujia Chen,Xujing Tao,Bohao Liao,Xiaotian Yin,Baoqun Yin,Tianzhu Zhang*

Main category: cs.CV

TL;DR: 提出一种模仿人类行为的“聚焦-扫描”范式，结合状态空间模型和多层动态分配策略，解决图像与点云配准中的尺度模糊和跨模态差异问题，在RGB-D Scenes V2和7-Scenes上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有无检测方法在多尺度特征和Transformer交互中仍存在层间注意力漂移与尺度内不一致性，导致配准精度受限，因此需要更鲁棒的跨模态特征关联机制。

Method: 设计基于状态空间模型的层次化聚焦-扫描交互模块，增强多层跨模态特征关联；并引入动态层分配策略自适应确定迭代深度，优化几何约束利用与匹配鲁棒性。

Result: 在RGB-D Scenes V2和7-Scenes两个基准上，所提方法均取得最先进性能，消融实验验证了各模块的有效性。

Conclusion: 模仿人类视觉行为的聚焦-扫描范式与动态深度分配能有效提升图像-点云配准的精度和鲁棒性，超越了现有方法。
...
### [201] [EggHand: A Multimodal Foundation Model for Egocentric Hand Pose Forecasting](https://arxiv.org/abs/2605.07642)
*Jaeyoung Choi,Hyeondong Kim,Yujin Kim,Daehee Park*

Main category: cs.CV

TL;DR: 提出了EggHand，一种基于基础模型的框架，用于从第一人称视频预测未来3D手部姿态序列。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理由自我中心运动引起的视角变化、复杂的手部关节动作和人类意图。

Method: 结合视觉-语言-动作模型的解码器捕捉手部动态，并使用第一人称视频文本编码器提供视角感知的上下文信息。

Result: 在EgoExo4D数据集上取得了最先进的预测精度，在剧烈自我运动下保持鲁棒，并支持通过语言任务提示进行可控预测。

Conclusion: EggHand有效解决了自我中心手部姿态预测的挑战，为AR/VR和机器人交互等应用提供了新思路。
...
### [202] [Flow-OPD: On-Policy Distillation for Flow Matching Models](https://arxiv.org/abs/2605.08063)
...
TL;DR: 本文提出Flow-OPD，首个将在线策略蒸馏集成到流匹配模型的统一后训练框架，通过两阶段对齐策略和流引导冷启动方案，显著提升了文本到图像生成的多任务对齐性能。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配模型在多任务对齐中面临标量奖励稀疏和异质目标梯度干扰导致的‘跷跷板效应’和奖励黑客问题。

Method: 分为两阶段：先用单奖励GRPO微调培养领域专精教师模型，再通过在线策略采样、任务路由标注和密集轨迹监督将异质专家知识整合到单个学生模型中，并引入流引导冷启动和流形锚点正则化。

Result: 基于SD3.5，GenEval得分从63提升至92，OCR准确率从59%提升至94%，整体比原始GRPO提高约10点，同时保持图像保真度和人类偏好对齐，并出现‘超越教师’效应。

Conclusion: Flow-OPD是一种可扩展的对齐范式，适用于构建通用文本到图像模型。
...
### [203] [EmambaIR: Efficient Visual State Space Model for Event-guided Image Reconstruction](https://arxiv.org/abs/2605.08073)
*Wei Yu,Yunhang Qian*

Main category: cs.CV

TL;DR: EmambaIR是一个基于高效视觉状态空间模型的事件驱动图像重建框架，通过跨模态Top-k稀疏注意力模块和门控状态空间模块，在运动去模糊、去雨和高动态范围增强等任务中超越现有方法，同时显著降低计算和内存开销。


<details>
  <summary>Details</summary>
Motivation: 现有基于CNN和Vision Transformer的事件图像重建方法分别受限于全局特征关联捕捉不足或二次计算复杂度，难以高效处理高分辨率事件流。

Method: 提出EmambaIR框架，包含跨模态Top-k稀疏注意力模块实现像素级稀疏注意力引导跨模态融合，以及门控状态空间模块通过非线性门控单元增强线性复杂度状态空间模型的时序表示能力。

Result: 在六个数据集上的三项重建任务（运动去模糊、去雨、高动态范围增强）中，EmambaIR显著优于现有最先进方法，并大幅降低内存消耗和计算成本。

Conclusion: 该工作证明了高效状态空间模型在事件驱动图像重建中的潜力，为资源受限场景提供可行方案。
...
### [204] [OphEdit: Training-Free Text-Guided Editing of Ophthalmic Surgical Videos](https://arxiv.org/abs/2605.07695)
*Ritul Jangir,Arkya Jyoti Bagchi,Aiman Farooq,Mangalton Okram,Saurabh Seetaram Korgaonkar,Deepak Mishra*

Main category: cs.CV

TL;DR: 提出了一种无需训练的文本引导眼科手术视频编辑框架OphEdit，通过确定性ODE反演和注意力值注入，实现高保真结构保留与语义修改。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型难以精准编辑手术视频中严格解剖和时序约束下的属性（如器械-组织交互），亟需高效、低成本的视频编辑方法。

Method: 利用确定性二阶ODE反演流水线捕获原始视频的注意力值(V)张量，并在去噪阶段选择性注入条件CFG分支，实现解剖结构保持与文本驱动语义映射。

Result: 临床评估显示，OphEdit在器械替换和手术阶段变化等复杂编辑中，结构保真度和时序一致性显著优于自然域视频编辑器。

Conclusion: 首次将无需训练的视频编辑应用于眼科手术领域，为生成多样化标注医学数据集提供了可扩展方案，无需人工录制或模型微调。
...
### [205] [SIMI: Self-information Mining Network for Low-light Image Enhancement](https://arxiv.org/abs/2605.07767)
*Xuanshuo Fu,Lei Kang,Javier Vazquez-Corral*

Main category: cs.CV

TL;DR: 提出了一种自信息挖掘网络，通过位平面分解从低光照图像中提取内在信息，实现无监督的增强。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常构建复杂模型，忽视了低光照图像本身蕴含的内在信息。

Method: 基于位平面分解的无监督框架，自动挖掘图像内部信息，无需外部数据。

Result: 在标准基准测试中达到最先进性能，加速收敛并降低计算开销。

Conclusion: SIMI网络能有效提升低光照图像质量，且适用于实际场景。
...
### [206] [Benchmarking Foundation Models for Renal Lesion Stratification in CT](https://arxiv.org/abs/2605.07749)
...
TL;DR: 在CT肾病变分类中，医疗基础模型的预训练嵌入表现与从头训练的3D ResNet-50相当，但远不及传统放射组学（AUC 0.88），表明现有通用模型未能捕捉细粒度纹理和形状差异。


<details>
  <summary>Details</summary>
Motivation: 开源医疗基础模型在数据稀缺的临床任务（如肾病变分类）中的迁移能力尚未明确，而该领域受限于训练数据不足，需要探究基础模型能否比现有方法更具泛化性。

Method: 使用冻结特征探测协议，比较三个医疗基础模型的嵌入与手工放射组学分类器及从头训练的3D ResNet-50，在包含2854个病变的复合数据集上训练，并在TCIA的234个病变外部测试集上评估。

Result: 基础模型AUC为0.70-0.77，与ResNet (0.72)相当且硬件需求大幅降低；但放射组学显著优于所有深度学习方法，AUC达0.88（p≤0.002）。

Conclusion: 当前医疗基础模型在肾病变分层中未超越放射组学，放射组学仍是该任务的最先进方法；基础模型嵌入未能捕捉组织学亚型所需的细粒度特征。
...
### [207] [LAMES: A Large-Scale and Artisanal Mining Environmental Segmentation Dataset](https://arxiv.org/abs/2605.07740)
*Matthias Kahl,Zhaiyu Chen,Sudipan Saha,Mrinalini Kochupillai,Lukas Kondmann,Xiao Xiang Zhu*

Main category: cs.CV

TL;DR: 该研究提出了一个包含150个大型矿山和870平方公里小型矿山人工标注区域的数据集，并详细记录了矿山属性及元数据，旨在支持远程监测非法采矿及其环境影响的学术研究。


<details>
  <summary>Details</summary>
Motivation: 采矿活动对国民经济重要但导致环境破坏，非法采矿（如非洲地区）加剧了监管困难；现有数据难以支撑对采矿类型、加工方式与生态影响之间关联的系统性分析。

Method: 收集并标注了150个大型采矿（LSM）点位和870km²的小型采矿（ASM）区域；为每个LSM站点记录了包括九大部门分类和27项属性在内的元数据。

Result: 成功构建了一个包含高精度空间标注和丰富属性信息的采矿数据集，覆盖LSM和ASM两种类型，可用于检测非法采矿活动并分析其环境效应。

Conclusion: 该数据集能够帮助研究社区更深入理解采矿设施特征（如类型、方法、产品）与环境影响的关系，并指出了向社会负责任地使用数据的伦理要求。
...
### [208] [Head Similarity: Modeling Structured Whole-Head Appearance Beyond Face Recognition](https://arxiv.org/abs/2605.07766)
*Yingfeng Wang,Yuxuan Xiao,Shengcai Liao*

Main category: cs.CV

TL;DR: 本文提出“头部相似性”新概念，通过分层监督和身份感知蒸馏，在身份识别基础上建模外观变化，实现了全景下的一致性比较。


<details>
  <summary>Details</summary>
Motivation: 传统人脸识别模型强制身份不变性，忽略了发型、风格等外观变化，限制了在外观敏感场景（如遮挡、非正面视角）中的应用。

Method: 构建弱监督外观状态的大规模基准，采用分层相似性排序和身份感知蒸馏框架，联合建模身份判别与外观敏感相似性。

Result: 常规人脸识别模型无法捕捉外观依赖性相似性，而所提方法实现了结构化全头相似性建模的可行性。

Conclusion: 该工作为需要身份一致性和外观变化的场景提供了有效方案，并推动了头部相似性建模的发展。
...
### [209] [Radiologist-Guided Causal Concept Bottleneck Models for Chest X-Ray Interpretation](https://arxiv.org/abs/2605.07785)
*Amy Rafferty,Rishi Ramaesh,Ajitha Rajan*

Main category: cs.CV

TL;DR: 提出XpertCausal，一种基于放射科医生指导的因果概念瓶颈模型，在胸部X光片中通过噪声-OR框架显式建模疾病与概念间的因果关系，并通过贝叶斯推理实现诊断。


<details>
  <summary>Details</summary>
Motivation: 现有CBMs将概念视为病理标签的判别性预测因子，未建模疾病产生临床发现的生成过程，导致可解释性受限。

Method: 构建放射科医生定义的病理-概念关联图，采用概率噪声-OR生成模型，再通过贝叶斯推理反演概念预测以估计病理概率。

Result: 在MIMIC-CXR上，XpertCausal在AUROC、校准性和临床相关解释质量上均优于非因果CBMs和自由学习的因果消融模型，且概念-病理关系更符合专家知识。

Conclusion: 融入临床驱动的因果结构和领域知识可提升CBM的准确性、可解释性与临床对齐度。
...
### [210] [SARA: Semantically Adaptive Relational Alignment for Video Diffusion Models](https://arxiv.org/abs/2605.07800)
*Jiesong Lian,Zixiang Zhou,Ruizhe Zhong,Yuan Zhou,Qinglin Lu,Rui Wang,Long Hu,Yixue Hao,Baoru Huang*

Main category: cs.CV

TL;DR: SARA通过文本条件显著图引导成对监督，提升视频扩散模型的文本跟随能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在实体保持、属性绑定和交互描述上存在缺陷，且现有表征对齐方法中成对监督的分配未考虑提示相关性。

Method: 引入文本条件显著性，结合SAM掩膜和InfoNCE正则化训练轻量级对齐器，通过成对路由操作将监督导向主体相关对。

Result: 在Wan2.2持续训练中，SARA在文本对齐、运动质量和VBench基准上均优于SFT、VideoREPA和MoAlign。

Conclusion: 语义自适应关系对齐有效提升了视频生成中的细粒度文本遵循能力。
...
### [211] [Differentiable Ray Tracing with Gaussians for Unified Radio Propagation Simulation and View Synthesis](https://arxiv.org/abs/2605.07781)
*Niklas Vaara,Lam Huynh,Pekka Sangi,Miguel Bordallo López,Janne Heikkilä*

Main category: cs.CV

TL;DR: 该框架将3D高斯溅射（3DGS）的显式神经表示用于可微射频（RF）传播模拟，在视觉重建场景中实现点对点路径计算，同时保持高质量视觉渲染。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS虽能实现高保真视图合成，但针对透明度合成的光学外观优化，缺乏射线可相交的几何结构；而射频数字孪生需要确定性的多反射路径，依赖手动构建网格的仿真流程。

Method: 将高斯基元嵌入硬件加速的射线追踪结构中，作为底层空间表示，通过物理有意义的信道脉冲响应提取，实现跨模态证据的整合。

Result: 成功从仅视觉重建中提取物理有意义的信道脉冲响应，证明神经重建可作为电磁传播仿真和摄影级视图合成的统一空间表示。

Conclusion: 神经空间表示可同时支持电磁传播模拟与视觉渲染，为跨模态统一表示提供证据，无需手动网格构建。
...
### [212] [ICDAR 2026 Competition on Writer Identification and Pen Classification from Hand-Drawn Circles](https://arxiv.org/abs/2605.07816)
*Thomas Gorges,Janne van der Loop,Lukas Hüttner,Linda-Sophie Schneider,Fei Wu,Mathias Seuret,Vincent Christlein*

Main category: cs.CV

TL;DR: 介绍了CircleID竞赛，包含46,155张手绘圆圈图像，用于笔迹识别和笔型分类，最佳成绩分别为64.8%和92.7%。


<details>
  <summary>Details</summary>
Motivation: 研究最小静态痕迹中生物特征和笔特征的纠缠关系，推动开放集识别和跨写者笔分类。

Method: 提供标注数据集和ResNet基线系统，在Kaggle平台分两个赛道进行竞赛。

Result: 笔分类任务389支队伍提交3,185次，识别任务113支队伍提交1,737次，最佳准确率分别为92.726%和64.801%。

Conclusion: 建立了最小痕迹分析的新基线，分析了分布外写者对模型泛化和特征解耦的影响。
...
### [213] [EyeCue: Driver Cognitive Distraction Detection via Gaze-Empowered Egocentric Video Understanding](https://arxiv.org/abs/2605.07859)
*Lang Zhang,JinYi Yoon,Matthew Corbett,Abhijit Sarkar,Bo Ji*

Main category: cs.CV

TL;DR: 提出EyeCue框架，通过眼动与自我中心视频结合检测认知分心，并引入CogDrive数据集，准确率达74.38%。


<details>
  <summary>Details</summary>
Motivation: 认知分心难以检测，因无显性物理动作且现有数据集规模有限。

Method: 融合眼动与视频上下文建模注意力，基于CogDrive数据集评估。

Result: EyeCue在CogDrive上准确率74.38%，优于此前提及的11种基线方法超过7%。

Conclusion: 眼动-上下文交互建模对多模态认知分心检测有效且泛化性强。
...
### [214] [Explainable Part-Based Vehicle Classifier with Spatial Awareness](https://arxiv.org/abs/2605.07831)
*Andreas Caduff,Klaus Zahn,Jonas Hofstetter,Martin Rechsteiner,Patrick Flaig*

Main category: cs.CV

TL;DR: 提出了一种具有空间感知能力的部件级车辆分类方法，通过结合CNN部件检测器与软概率映射，在保持可解释性的同时达到与端到端CNN相当的精度。


<details>
  <summary>Details</summary>
Motivation: 传统CNN作为黑箱模型难以解释且扩展新类别需重新训练，而现有部件方法缺乏空间感知导致鲁棒性不足。

Method: 使用CNN检测车辆部件并生成空间概率图，基于部件相对位置构建特征，通过softmax回归进行车辆类别分类。

Result: 相比原方法显著提升了对误检测的鲁棒性，且准确率与端到端CNN持平。

Conclusion: 部件级空间感知方法成功打破了精度与可解释性之间的权衡，为ITS系统提供了兼具高精度与直观可解释性的解决方案。
...
### [215] [BRIDGE: Background Routing and Isolated Discrete Gating for Coarse-Mask Local Editing](https://arxiv.org/abs/2605.07846)
*Peilin Xiong,Honghui Yuan,Junwen Chen,Keiji Yanai*

Main category: cs.CV

TL;DR: 提出BRIDGE方法，通过双路径生成和可学习离散几何门控解决粗掩码局部图像编辑中的掩码形状偏差问题，在多个基准上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，粗糙掩码会无意中成为形状先验，迫使生成对象继承掩码边界，导致编辑效果不佳。

Method: 采用双路径生成（主路径保持背景，主体路径生成可编辑内容），并引入可学习离散几何门控进行token级位置嵌入路由，避免DiT内部的掩码注入。

Result: 在BRIDGE-Bench上，Local SigLIP2-T从0.262/0.390提升至0.503，MagicBrush和ICE-Bench上零样本表现优异，路由模块仅13.31M参数。

Conclusion: BRIDGE有效缓解了掩码形状偏差，实现背景保持与指令跟随的平衡，且模型轻量高效。
...
### [216] [From Synthetic to Real: Toward Identity-Consistent Makeup Transfer with Synthetic and Real Data](https://arxiv.org/abs/2605.07861)
*Yue Yu,Jiayu Wang,Jiajia Shi,Jingjing Chen,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: 该论文提出一个名为ConsistentBeauty的数据处理流程和RealBeauty的后训练框架，通过合成数据监督和强化学习适应真实场景，在人像化妆迁移任务上实现了最先进的性能，尤其在身份保持和复杂场景表现上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散或流的化妆迁移方法受限于合成数据中身份保持不足以及合成域与真实域之间的域差距，导致在复杂真实场景中性能下降。

Method: 首先设计ConsistentBeauty数据流水线，确保合成数据中化妆保真度和身份一致性；然后提出RealBeauty框架，在合成数据上监督学习后，通过强化学习和针对化妆迁移可验证奖励函数适配真实场景。

Result: 在多个基准测试中达到最先进性能，在身份保持和复杂真实案例上具有明显优势，并建立了一个涵盖多种人种、年龄、性别、姿态和化妆风格的新基准。

Conclusion: 通过改进数据质量和引入强化学习域适应，有效解决了合成监督的身份保持和域差距问题，显著提升了化妆迁移在真实场景中的泛化能力。
...
### [217] [One World, Dual Timeline: Decoupled Spatio-Temporal Gaussian Scene Graph for 4D Cooperative Driving Reconstruction](https://arxiv.org/abs/2605.07910)
*Yulong Chen,Xiaoyun Dong,Haoyu Zhang,Zongxian Yang,Lewei Xie,Xinke Li,Yifan Zhang,Kai Wang,Jianping Wang*

Main category: cs.CV

TL;DR: 提出DUST高斯场景图，通过解耦时空表示解决VICAD数据中的时间异步问题，在动态场景重建中提升PSNR 3.2 dB。


<details>
  <summary>Details</summary>
Motivation: 现有方法默认同步观测，在协同驾驶中因时间异步导致动态智体出现重影，且该问题被证明是表示层面的根本缺陷。

Method: 构建解耦时空高斯场景图：为每个智体共享规范高斯集保证外观一致性，同时维护对齐各源真实时间戳的独立位姿轨迹；辅以静态锚点位姿校正和正则化联合优化。

Result: 在V2X-Seq的26个序列上，动态区域PSNR提升3.2 dB，Fréchet Video Distance降低37.7%，且对更大时间异步保持鲁棒。

Conclusion: DUST有效解决了协同驾驶重建中的时间异步问题，为多源动态场景重建提供了新范式。
...
### [218] [Delta-Adapter: Scalable Exemplar-Based Image Editing with Single-Pair Supervision](https://arxiv.org/abs/2605.07940)
*Jiacheng Chen,Songze Li,Han Fu,Baoquan Zhao,Wei Liu,Yanyan Liang,Li Qing,Xudong Mao*

Main category: cs.CV

TL;DR: 提出了Delta-Adapter方法，通过单对监督学习可迁移的编辑语义，无需文本引导，利用预训练视觉编码器提取语义增量并注入编辑模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于范例的图像编辑方法需要双对监督，限制了训练数据的规模扩展和编辑类型的泛化能力。

Method: 使用预训练视觉编码器提取源-目标图像对的语义增量，通过基于Perceiver的适配器注入预训练编辑模型，采用单对监督和语义增量一致性损失。

Result: 在四个强基线上，编辑准确性和内容一致性均有提升，且在未见过的编辑任务上泛化能力更强。

Conclusion: Delta-Adapter通过单对监督有效学习可迁移编辑语义，提高编辑性能并增强泛化能力，代码已开源。
...
### [219] [What Matters for Diffusion-Friendly Latent Manifold? Prior-Aligned Autoencoders for Latent Diffusion](https://arxiv.org/abs/2605.07915)
...
TL;DR: 本文提出Prior-Aligned AutoEncoder (PAE)，通过明确塑造潜流形空间结构、局部连续性和全局语义，提升了潜扩散模型的训练效率和生成质量，在ImageNet 256×256上达到gFID 1.03。


<details>
  <summary>Details</summary>
Motivation: 现有分词器主要优化重建保真度或继承预训练表示，缺乏对潜流形组织方式的系统性研究，不清楚何种潜空间对生成建模真正友好。

Method: 通过构建变体识别扩散友好潜流形的三个关键性质（空间结构连贯、局部连续、全局语义），并设计PAE利用VFMs先验和扰动正则化，将其作为显式训练目标。

Result: PAE在ImageNet 256×256上达到gFID 1.03，相比RAE加速收敛达13倍，并显著提升训练效率与生成质量。

Conclusion: 潜流形的组织方式比重建保真度更影响生成质量，为潜扩散模型提供了一种高效的潜空间设计范式。
...
### [220] [Rebalancing gradient to improve self-supervised co-training of depth, odometry and optical flow predictions](https://arxiv.org/abs/2605.07945)
*Marwane Hariat,Antoine Manzanera,David Filliat*

Main category: cs.CV

TL;DR: CoopNet通过动态调整梯度分配来改进协同训练网络的合作，确保学习进度公平，应用于运动感知的自监督深度图预测。


<details>
  <summary>Details</summary>
Motivation: 现有协同训练网络在深度、里程计和光流预测中，处理动态物体时学习进度不均，需要更好的合作机制。

Method: 提出CoopNet方法，使用基于光度重构误差分布的混合损失，动态分配梯度，并通过理论与实验验证像素分类模型。

Result: 在KITTI和CityScapes数据集上，CoopNet在深度、里程计和光流预测中达到或超越现有最优水平。

Conclusion: CoopNet有效提升协同训练网络在自监督任务中的表现，尤其处理运动物体时更鲁棒。
...
### [221] [HEART: Hyperspherical Embedding Alignment via Kent-Representation Traversal in Diffusion Models](https://arxiv.org/abs/2605.07973)
*Arani Roy,Shristi Das Biswas,Kaushik Roy*

Main category: cs.CV

TL;DR: 提出HEART框架，利用文本编码器的超球面几何特性，通过Kent分布感知的测地变换实现零训练文本到图像编辑。


<details>
  <summary>Details</summary>
Motivation: 当前文本控制方法将嵌入空间视为欧几里得空间并应用线性变换，导致语义概念组织不准确，出现非预期编辑效果。

Method: 分析文本编码器表征位于超球面且概念呈各向异性Kent分布，提出无需训练的HEART方法，在超球面上直接执行Kent分布感知的测地变换。

Result: HEART实现主体替换、属性调整等精确编辑且保持场景一致性，无需微调或优化，跨架构通用。

Conclusion: 从线性到球面的视角转换可解锁快速可控的图像生成，简单且高效。
...
### [222] [MedVIGIL: Evaluating Trustworthy Medical VLMs Under Broken Visual Evidence](https://arxiv.org/abs/2605.07919)
...
TL;DR: 提出了MedVigil基准测试套件，评估医学VLM在证据基础失效时的表现，发现最强模型距人类仍有14.1个百分点的差距。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLM评估忽略了对证据基础失效时的应对能力，而临床可信度要求模型能识别并拒绝无法回答的问题。

Method: 构建300例基于四个公开医学VQA数据集的评估套件，由放射科医生标注答案、干扰项、错误前提陷阱等，设计7个审计指标并汇总为MCS分数。

Result: 人类放射科医生MCS 83.3，失败率5.8%；最佳模型（Claude Opus 4.7）MCS 69.2，失败率未明确；复合得分存在14.1分差距。

Conclusion: 当前最强的医学VLM在愚直性失败率上仍显著落后于人类，现有模型缺乏识别证据不足或错误的能力。
...
### [223] [DVD: Discrete Voxel Diffusion for 3D Generation and Editing](https://arxiv.org/abs/2605.07971)
*Zhengrui Xiang,Jiaqi Wu,Fupeng Sun,Heliang Zheng,Yingzhen Li*

Main category: cs.CV

TL;DR: 提出了离散体素扩散（DVD）框架，用于生成、评估和编辑稀疏体素，作为SLat基3D生成流水线的第一阶段先验。


<details>
  <summary>Details</summary>
Motivation: 连续扩散在图像类生成中占主导，但离散扩散未被广泛采用；DVD旨在解决连续到离散阈值转换问题，并提供更可解释的生成动态。

Method: 将体素占用率作为原生离散变量，使用离散扩散模型；利用预测熵作为不确定性度量；引入块结构扰动模式进行轻量级微调，实现单轮采样中的编辑。

Result: DVD在体素生成质量上优于现有方法，能有效进行不确定性估计和数据过滤，并通过微调策略实现高效编辑。

Conclusion: 离散扩散可作为稀疏体素的有效先验，提供可解释性和实用性，且轻量级微调策略为3D编辑提供了新途径。
...
### [224] [Seeing Across Skies and Streets: Feedforward 3D Reconstruction from Satellite, Drone, and Ground Images](https://arxiv.org/abs/2605.07978)
*Qiwei Wang,Zhongyao Tuo,Xianghui Ze,Yujiao Shi*

Main category: cs.CV

TL;DR: 提出Cross3R模型，通过引入无人机视角实现从卫星到地面的6自由度高精度定位，并构建CrossGeo三视图数据集。


<details>
  <summary>Details</summary>
Motivation: 传统交叉视图定位仅能估计3自由度(x,y,yaw)，无法处理真实地形中的斜坡、倾斜等导致假设失效的问题。

Method: 利用无人机图像作为中间视角提供三维结构线索，设计前馈模型Cross3R同时恢复三维点云、6自由度相机位姿和地面位置。

Result: 在CrossGeo数据集上优于3D基线，在KITTI上无训练情况下超越专用方法。

Conclusion: 引入无人机视角能有效突破传统3自由度限制，实现鲁棒的6自由度定位。
...
### [225] [SphereVAD: Training-Free Video Anomaly Detection via Geodesic Inference on the Unit Hypersphere](https://arxiv.org/abs/2605.08003)
*Chao Huang,Penfei Wei,Wei Wang,Jie Wen,Zhihua Wang,Li Shen,Wenqi Ren,Xiaochun Cao*

Main category: cs.CV

TL;DR: 提出了一种无需训练、零样本的视频异常检测框架SphereVAD，利用多模态大语言模型的中间层特征，通过球面流形上的几何推理进行异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大规模标注或任务特定训练，难以快速部署到新场景。

Method: 基于von Mises-Fisher似然比测地线推断，包括Frechet均值中心化、整体场景注意力和球面测地线拉取。

Result: 在三个主流基准上达到无训练方法的最优结果，与完全监督基线竞争力相当。

Conclusion: 证明了通过无训练几何推理利用预训练MLLM特征进行有效异常检测的可行性。
...
### [226] [Rethinking Dense Optical Flow without Test-Time Scaling](https://arxiv.org/abs/2605.08000)
*Praroop Chanda,Suryansh Kumar*

Main category: cs.CV

TL;DR: 提出了一种无需测试时迭代优化的单步稠密光流估计方法，利用预训练基础模型中的视觉语义和几何先验，显著降低计算开销同时取得高精度。


<details>
  <summary>Details</summary>
Motivation: 当前稠密光流方法依赖复杂架构和测试时多步细化，计算量巨大。研究旨在探索是否可通过利用基础模型中的强大语义和几何先验替代测试时计算缩放。

Method: 从冻结的DINO-v2骨干提取视觉语义特征，结合单目深度基础模型的几何线索，融合为统一表示后通过全局匹配直接估计稠密对应关系，避免循环更新或测试时优化。

Result: 在Sintel Final上EPE为2.81，无细化条件下显著优于SEA-RAFT、RAFT、GMFlow（无细化）和FlowSeek，跨数据集泛化能力强。

Conclusion: 强基础先验可替代测试时缩放，为高计算成本的细化流程提供高效替代方案。
...
### [227] [Object Hallucination-Free Reinforcement Unlearning for Vision-Language Models](https://arxiv.org/abs/2605.08031)
*Kaidi Jia,Yujie Lin,Chengyi Yang,Jiayao Ma,Jinsong Su*

Main category: cs.CV

TL;DR: 提出 HFRU 框架，通过强化学习在视觉编码器上实现深度语义移除，达到高遗忘率且少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要微调语言解码器，导致遗忘表面化且引入物体幻觉，需在视觉层面深入移除。

Method: 两阶段方法：对齐破坏结合 GRPO 优化，使用包含抽象奖励的复合奖励。

Result: 在物体识别和人脸任务上遗忘率超 98%，保留性能好，幻觉极低。

Conclusion: HFRU 显著优于先前方法，有效实现深度遗忘并缓解幻觉。
...
### [228] [TRAS: An Interactive Software for Tracing Tree Ring Cross Sections](https://arxiv.org/abs/2605.08025)
*Henry Marichal,Diego Passarella,Gregory Randall*

Main category: cs.CV

TL;DR: Tree Ring Analyzer Suite (TRAS) 是一款开源软件，结合经典图像处理与深度学习方法自动标记树轮，提供手动校正功能，在18张松树图像上达到81.0% F-score，并显著减少人工校正工作量。


<details>
  <summary>Details</summary>
Motivation: 手工标注树轮耗时、主观且难以大规模处理，因此需要自动化解决方案。

Method: 集成了CS-TRD（经典图像处理）和DeepCS-TRD、INBD（深度学习）三种算法，并设计了交互界面用于校正和测量树轮指标（如早材、晚材面积等）。

Result: DeepCS-TRD取得最佳自动检测结果（F-score 81.0%, 精度86.4%），所需手动校正边界仅约20%；与CooRecorder的一维宽度测量相关性r>0.99。

Conclusion: TRAS为树轮分析提供了灵活、可复现的跨平台解决方案，且容易修复常见错误。
...
### [229] [STARFlow2: Bridging Language Models and Normalizing Flows for Unified Multimodal Generation](https://arxiv.org/abs/2605.08029)
...
TL;DR: STARFlow2 是一个基于自回归流的统一多模态生成模型，通过 Pretzel 架构将 VLM 与 TarFlow 流垂直交错，在统一因果掩码下实现文本与图像的缓存友好型交错生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法结合自回归语言建模与扩散图像生成存在结构不匹配（因果文本生成 vs. 迭代视觉去噪），而自回归流作为自然范式的潜力未被充分探索。

Method: 采用 Pretzel 架构，通过残差跳跃连接垂直交错预训练 VLM 流和 TarFlow 流，结合深浅流设计和统一 FAE 潜空间，使得文本与图像输出直接进入 KV 缓存而无需重新编码。

Result: 在图像生成和多模态理解基准上均取得强性能，验证了自回归流作为统一多模态建模可行基础。

Conclusion: 自回归流是实现真正统一多模态生成的最自然范式，STARFlow2 提供了高效且高性能的实现。
...
### [230] [PET-Adapter: Test-Time Domain Adaptation for Full and Limited-Angle PET Image Reconstruction](https://arxiv.org/abs/2605.08030)
*Rüveyda Yilmaz,Yuli Wu,Johannes Stegmaier,Volkmar Schulz*

Main category: cs.CV

TL;DR: 提出了PET-Adapter，一种测试时域适应框架，使预训练的生成式PET重建模型能从幻影数据适应到临床数据，无需配对真实数据。该方法通过逐层低秩解剖条件化和基于OSEM的暖启动，将扩散步数从50减少到2，在多种临床数据集上展示了优越的3D重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习方法在PET图像重建中泛化到未见过的临床数据分布时受限，需要大量重新训练，而临床数据缺乏配对真实数据。

Method: 提出PET-Adapter，包括逐层低秩解剖条件化和有序子集期望最大化（OSEM）暖启动，从物理信息重建初始化生成，减少扩散步数。

Result: 在多个临床数据集上，在全角度和有限角度设置下均表现出优越的3D重建性能，且扩散步数从50减少到2而不牺牲质量。

Conclusion: PET-Adapter展示了临床可行性和计算效率，为未见过临床数据分布提供了有效的域适应解决方案。
...
### [231] [Towards Highly-Constrained Human Motion Generation with Retrieval-Guided Diffusion Noise Optimization](https://arxiv.org/abs/2605.08054)
*Hanchao Liu,Fang-Lue Zhang,Shining Zhang,Tai-Jiang Mu,Shi-Min Hu*

Main category: cs.CV

TL;DR: 提出了一个检索引导的扩散噪声优化框架，用于处理高度约束的零样本人体运动生成任务。


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法在处理极具挑战性的时空约束（如严重空间障碍或指定步数）时表现不佳。

Method: 采用无训练扩散噪声优化框架，通过关系任务解析分组目标约束，用检索参考处理困难约束，再通过奖励引导掩码结合随机噪声与检索噪声改进噪声初始化。

Result: 成功解决了高度约束的运动生成任务，并通过LLM实现自动推理检索内容。

Conclusion: 所提方法通过检索引导和任务解析显著提升了复杂约束下的运动生成能力。
...
### [232] [MoCoTalk: Multi-Conditional Diffusion with Adaptive Router for Controllable Talking Head Generation](https://arxiv.org/abs/2605.08050)
*Xinyan Ye,Jiankang Deng,Abbas Edalat*

Main category: cs.CV

TL;DR: 提出多条件视频扩散框架MoCoTalk，通过自适应多条件路由和嘴增强着色网格实现可控说话头生成，在多数指标上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅处理部分因素，且多条件融合依赖固定权重或启发式策略，导致条件间破坏性干扰。

Method: 采用扩散框架融合参考图像、关键点、3DMM着色网格和语音四条件；提出自适应路由实现通道和时序感知融合；设计嘴增强着色网格解耦运动和光照；引入唇部一致性损失。

Result: 在结构、运动和感知指标上优于现有方法，具备属性级可控性。

Conclusion: MoCoTalk有效统一多种条件，通过动态融合和几何先验提升说话头生成质量与可控性。
...
### [233] [Proxy3D: Efficient 3D Representations for Vision-Language Models via Semantic Clustering and Alignment](https://arxiv.org/abs/2605.08064)
*Jerry Jiang,Haowen Sun,Denis Gudovskiy,Yohei Nakata,Tomoyuki Okuno,Kurt Keutzer,Wenzhao Zheng*

Main category: cs.CV

TL;DR: 提出了Proxy3D方法，通过语义感知聚类得到紧凑的3D代理表示，结合多阶段训练，在3D视觉问答、视觉定位等任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM的2D流水线在3D空间推理中存在空间一致性和效率问题。

Method: 从视频帧中利用语义与几何编码器提取场景特征，经语义感知聚类生成3D代理表示，并通过SpaceSpan数据集进行多阶段训练对齐。

Result: 在3D VQA、视觉定位和空间智能基准上，使用更短序列达到SOTA性能。

Conclusion: Proxy3D以紧凑的3D代理表示有效提升了VLM的空间智能和效率。
...
### [234] [Normalizing Trajectory Models](https://arxiv.org/abs/2605.08078)
*Jiatao Gu,Tianrong Chen,Ying Shen,David Berthelot,Shuangfei Zhai,Josh Susskind*

Main category: cs.CV

TL;DR: 提出了正则化轨迹模型（NTM），将每个反向步骤建模为具有精确似然训练的条件归一化流，在四步采样中匹配或超越文本到图像生成基线，同时保留精确似然。


<details>
  <summary>Details</summary>
Motivation: 现有少步扩散方法通过蒸馏、一致性训练或对抗目标牺牲了似然框架，而NTM旨在保留精确似然的同时实现高效少步生成。

Method: 结合每个步骤中的浅层可逆块与跨轨迹的深层并行预测器，形成端到端网络，支持从头训练或从预训练流匹配模型初始化，并利用精确轨迹似然进行自蒸馏。

Result: 在文本到图像基准测试中，NTM在四步采样下匹配或超越强基线，并独特地保留了生成轨迹上的精确似然。

Conclusion: NTM通过条件归一化流和自蒸馏，在少步生成中实现了精确似然与高质量结果的结合。
...
### [235] [IntentGrasp: A Comprehensive Benchmark for Intent Understanding](https://arxiv.org/abs/2605.06832)
*Yuwei Yin,Chuyuan Li,Giuseppe Carenini*

Main category: cs.CL

TL;DR: 提出了IntentGrasp基准和IFT微调方法，显著提升大语言模型的意图理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对用户意图理解不足，缺乏系统评估和增强方法。

Method: 构建包含26万训练样本的IntentGrasp基准，并设计Intentional Fine-Tuning (IFT)方法。

Result: 20个LLM在All Set得分<60%，Gem Set<25%；IFT提升30+ F1点，Lodo实验验证跨领域泛化性。

Conclusion: IFT是增强LLM意图理解的有效途径，有助于开发更智能安全的AI助手。
...
### [236] [VITA-QinYu: Expressive Spoken Language Model for Role-Playing and Singing](https://arxiv.org/abs/2605.06765)
...
TL;DR: 提出了VITA-QinYu，首个支持角色扮演和歌唱生成的表现性端到端口语语言模型。


<details>
  <summary>Details</summary>
Motivation: 人类语音不仅传递语言内容，还包含个性、情绪等表现元素，现有模型缺乏对此类表达的支持。

Method: 采用混合语音-文本范式，通过多码本音频令牌扩展交错文本-音频建模，并构建了涵盖自然对话、角色扮演和歌唱的数据生成流水线。

Result: 在角色扮演基准测试中优于同行SLM 7个百分点，歌唱MOS评分领先0.13分，对话准确度和流畅度分别领先1.38和4.98个百分点。

Conclusion: VITA-QinYu在保持高表现力的同时，实现了最先进的对话准确性和流畅性，且已开源。
...
### [237] [Domain-level metacognitive monitoring in frontier LLMs: A 33-model atlas](https://arxiv.org/abs/2605.06673)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: 研究发现，大型语言模型的元认知质量在不同基准领域之间存在显著差异，聚合分数掩盖了这种变化；应用/专业领域最容易监控，而形式推理和自然科学领域最难。


<details>
  <summary>Details</summary>
Motivation: 传统聚合元认知质量分数可能掩盖模型在不同领域内的表现变化，本研究旨在揭示这种领域层面的差异。

Method: 对33个前沿LLM在1500个MMLU项目（6个领域、每领域250项）上，使用口头置信度（0-100）计算每个模型-领域单元的Type-2 AUROC，共47,151个观测值，并进行领域内一致性分析和聚类分析。

Result: 应用/专业领域监控最易（平均AUROC=0.742）；形式推理和自然科学最困难；领域分组具有实用性但非潜在构念；部分模型家族内轮廓形状聚类显著；Gemma 4 31B相比Gemma 3 27B的AUROC提升0.202；口头置信度下的概率与二进制决策格式存在特异性。
...
### [238] [TajPersLexon: A Tajik-Persian Lexical Resource and Hybrid Model for Cross-Script Low-Resource NLP](https://arxiv.org/abs/2605.06886)
*Mullosharaf K. Arabov*

Main category: cs.CL

TL;DR: 提出了包含40,112个词对的塔吉克-波斯平行词汇资源TajPersLexon，在低资源场景下实现跨脚本词汇检索、音译和对齐。


<details>
  <summary>Details</summary>
Motivation: 填补塔吉克-波斯跨脚本词汇处理的资源空白，解决低资源环境下的精确词汇匹配问题。

Method: 对比了轻量级混合流水线、神经序列到序列模型和检索方法三大类方法，并设计可解释的混合模型。

Result: 神经模型和检索基线达到98-99%的top-1准确率，混合模型在OCR后校正任务中达到96.4%准确率。

Conclusion: 任务本质上可解，可解释混合模型在准确率和效率间达成实用平衡，大型多语言句嵌入模型在该任务上表现不佳。
...
### [239] [MIST: Multimodal Interactive Speech-based Tool-calling Conversational Assistants for Smart Homes](https://arxiv.org/abs/2605.06897)
*Maximillian Chen,Xuanming Zhang,Michael Peng,Zhou Yu,Alexandros Papangelis,Yohan Jo*

Main category: cs.CL

TL;DR: 该论文提出了MIST，一个面向物联网设备的语音交互代码生成数据集，发现开放与闭源多模态大模型在该任务上存在显著性能差距，且即使最先进的闭源模型也有较大提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏结合时空约束、语音输入、动态状态追踪和混合主动交互模式的物联网设备建模挑战。

Method: 构建合成多轮语音驱动的代码生成任务MIST，并开发可扩展的数据生成框架。

Result: 开放权重与闭源多模态大模型在MIST上差距明显，前沿闭源模型仍有较大改进余地。

Conclusion: 需要进一步研究能推理物理世界约束的混合主动语音助手。
...
### [240] [Reflections and New Directions for Human-Centered Large Language Models](https://arxiv.org/abs/2605.06901)
...
TL;DR: 提出了一个整合NLP、HCI和负责任AI的人类中心大语言模型（HCLLM）框架，强调在系统设计、数据、训练、评估等全流程中系统性地融入人类优先级，并以未来工作场景为例进行应用分析。


<details>
  <summary>Details</summary>
Motivation: LLM在多个领域影响力日增，但现有开发多聚焦技术能力，忽视人类优先级、需求与价值观的严格整合。

Method: 从NLP、HCI和负责任AI交叉视角提出全流程框架，包含系统设计、数据来源、模型训练、评估和部署各阶段的人类中心建议。

Result: 明确了在每个阶段需严格处理人类关注点的具体方向，并通过未来工作案例展示了框架的应用效果。

Conclusion: 开发HCLLM需在技术全周期中优先考虑人类因素，而不仅限于后期微调，以构建更负责任且契合人类目标的系统。
...
### [241] [MELD: Multi-Task Equilibrated Learning Detector for AI-Generated Text](https://arxiv.org/abs/2605.06903)
*Chenjun Li,Cheng Wan,Johannes C. Paetzold*

Main category: cs.CL

TL;DR: 提出了一种多任务均衡学习检测器MELD，通过辅助监督增强二进制AI文本检测的鲁棒性，在攻击和低误报率下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有检测器仅优化单一AI/人类目标，缺乏对生成器、攻击和域结构的学习，导致鲁棒性不足。

Method: 使用共享编码器附加生成器家族、攻击类型和源域头部，通过同方差不确定性加权平衡四个损失，结合EMA教师蒸馏和硬负样本排序损失。

Result: 在RAID排行榜上达到开源最先进水平，与商业模型竞争；在MELD-eval上以1%假阳性率实现99.9%真阳性率。

Conclusion: MELD在攻击和低误报率下表现强健，无需额外微调即可泛化到未见生成器和域。
...
### [242] [Can LLMs Take Retrieved Information with a Grain of Salt?](https://arxiv.org/abs/2605.06919)
*Behzad Shayegh,Mohamed Osama Ahmed,Fred Tung,Leo Feng*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型在检索增强任务中根据上下文确定性调整响应的能力，发现系统性局限，并提出了结合先验提醒、确定性重新校准和上下文简化的交互策略，平均减少25%的遵从错误。


<details>
  <summary>Details</summary>
Motivation: 高安全领域（如医疗、金融）中模型无法正确适配检索信息的确定性，可能导致严重后果，而现有研究对此关注不足。

Method: 评估八个LLM在上下文确定性遵从上的表现，提出包含先验提醒、确定性重新校准和上下文简化的交互策略。

Result: 模型普遍存在难以回忆先验知识、误解确定性表达和过度信任复杂上下文的问题，所提策略平均减少25%的遵从错误。

Conclusion: 交互设计可有效提升LLM在不确定性处理中的可靠性，贡献包括评估指标、实证见解和可移植策略。
...
### [243] [MultiSoc-4D: A Benchmark for Diagnosing Instruction-Induced Label Collapse in Closed-Set LLM Annotation of Bengali Social Media](https://arxiv.org/abs/2605.06940)
...
### [244] [Group of Skills: Group-Structured Skill Retrieval for Agent Skill Libraries](https://arxiv.org/abs/2605.06978)
*Kun Zeng,Yu Huo,Siyu Zhang,Zi Ye,Yuecheng Zhuo,Haoyue Liu,Yuquan Lu,Junhao Wen,Xiaoying Tang*

Main category: cs.CL

TL;DR: 提出了GoSkills方法，通过将检索对象从扁平技能列表转换为带角色标签的执行上下文（包含Start、Support、Check、Avoid字段），从而提升智能体在技能库中的推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法返回原子技能或依赖感知束，但内部角色隐含，智能体需自行推断执行入口、支持技能、可见需求和失败规避指导。

Method: 从类型化技能图构建锚点中心技能组，通过组图扩展支持组，将选定的组计划瓶颈化为有界原子技能负载，并渲染固定执行契约（Start、Support、Check、Avoid字段）。

Result: 在SkillsBench和ALFWorld上，GoSkills在少量技能预算下保持可见需求覆盖，优于平面技能访问基线，并在奖励和智能体运行时方面优于结构化检索参考。

Conclusion: GoSkills通过结构化分组和角色标注，有效提升了技能检索的实用性和智能体执行效率。
...
### [245] [Cognitive Agent Compilation for Explicit Problem Solver Modeling](https://arxiv.org/abs/2605.07040)
*Hyeongdon Moon,Carolyn Rosé,John Stamper*

Main category: cs.CL

TL;DR: 提出认知代理编译（CAC）框架，利用教师大语言模型将问题解决知识编译为显式目标代理，实现教育场景中可检查和可编辑的知识状态。


<details>
  <summary>Details</summary>
Motivation: 大语言模型难以约束，无法代替可操控学习者，教育系统需要可检查、可编辑的知识状态。

Method: CAC框架分离知识表示、问题解决策略、验证与更新规则，用教师LLM编译知识到显式目标代理。

Result: 用小语言模型初步验证了关键设计权衡，尤其显式控制与可扩展泛化间的平衡。

Conclusion: CAC是迈向教育中有限知识AI的初步步骤。
...
### [246] [Towards Closing the Autoregressive Gap in Language Modeling via Entropy-Gated Continuous Bitstream Diffusion](https://arxiv.org/abs/2605.07013)
*Georgios Batzolis,Mark Girolami,Luca Ambrogioni*

Main category: cs.CL

TL;DR: 本文提出了一种将文本建模为连续扩散过程的方法，通过固定宽度二进制比特流表示语义标记，采用匹配滤波残差参数化和基于熵率门控的朗之万随机采样器，在LM1B和OpenWebText基准上达到了与自回归模型竞争的生成困惑度，并消除了词汇表缩放瓶颈。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在样本质量和多样性上落后于自回归模型，近期连续流和扩散方法虽缩小差距，但仍需进一步提升性能并解决词汇表缩放瓶颈。

Method: 将语义标记表示为模拟比特序列，使用匹配滤波残差参数化分离上下文学习与独立比特后验，并采用基于熵率门控的随机采样器，在高信息区域集中随机性，其他区域近乎确定性。

Result: 在LM1B上，130M参数模型生成困惑度为59.76，匹配真实数据熵4.31，优于先前扩散基线；在OpenWebText上，生成困惑度27.06，熵5.26，比先前方法少用4倍步骤。

Conclusion: 比特流扩散模型通过预测比特级逻辑和语义比特修补，消除词汇表缩放瓶颈，降低内存占用并提高吞吐量，为可扩展的语言生成提供新范式。
...
### [247] [NSMQ Riddles: A Benchmark of Scientific and Mathematical Riddles for Quizzing Large Language Models](https://arxiv.org/abs/2605.07051)
...
TL;DR: 提出了NSMQ Riddles基准测试，包含来自加纳全国科学和数学竞赛的1800道谜题，用于评估LLM的科学与数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估多依赖西方数据集，缺乏全球南方代表，且选择题形式过于简单。

Method: 收集11年NSMQ比赛第5轮谜题（n=1.8K），每道至少3条线索，答案简短可自动评估。测试GPT-5.4、Gemini 3.1 Pro、Claude Opus 4.6等模型，分高/低推理设置。

Result: 最先进的LLM表现不如最优学生选手，数据集具有挑战性。

Conclusion: NSMQ Riddles是来自全球南方的具有挑战性的科学数学推理基准，有助于全面评估LLM在教育中的能力。
...
### [248] [GSM-SEM: Benchmark and Framework for Generating Semantically Variant Augmentations](https://arxiv.org/abs/2605.07053)
...
TL;DR: 提出了GSM-SEM，一个能够生成语义多样基准变体的框架，通过修改实体、属性和关系来创造新的问题，同时保持计算和答案不变。在GSM8K等基准上应用后，评估14个LLM发现性能显著下降，最高严格配置下平均下降28%。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准如GSM8K的鲁棒性变种大多只进行表面扰动，静态数据集容易被记忆，导致排行榜成绩不能真实反映模型能力。

Method: GSM-SEM框架通过修改问题陈述中的实体、属性和关系，生成语义多样的新变体，保留原始计算和答案，并控制难度不变。可重复使用且无需重新标注。

Result: 在GSM8K及其两个变种上应用后，14个SOTA LLM平均性能下降28%（最高严格配置下），且语义扰动与符号/增强变种结合时下降更大。

Conclusion: GSM-SEM有效降低了模型对静态基准的记忆偏差，提供了更可靠的评估方法，并展示了在其他基准上的适用性。
...
### [249] [MedExAgent: Training LLM Agents to Ask, Examine, and Diagnose in Noisy Clinical Environments](https://arxiv.org/abs/2605.07058)
*Yicheng Gao,Xiaolin Zhou,Yahan Li,Yue Zhao,Ruishan Liu*

Main category: cs.CL

TL;DR: 本文提出将临床诊断形式化为部分可观测马尔可夫决策过程（POMDP），并构建了包含噪声模型的诊断环境，训练了名为MedExAgent的诊断智能体。


<details>
  <summary>Details</summary>
Motivation: 现有医学大模型基准和自动诊断方法过度简化了临床诊断过程，忽略了其交互性和不确定性，无法真实模拟临床实践中的复杂情况。

Method: 采用POMDP框架，定义三种动作（询问患者、检查工具调用、诊断），引入噪声模型（7种患者噪声和3种检查噪声），并通过两阶段流水线（基于Calgary-Cambridge模型的监督微调和DAPO优化复合奖励）训练MedExAgent。

Result: MedExAgent在诊断性能上可与更大模型媲美，同时保持了高效的检查策略和成本控制。
...
### [250] [WiCER: Wiki-memory Compile, Evaluate, Refine Iterative Knowledge Compilation for LLM Wiki Systems](https://arxiv.org/abs/2605.07068)
*Juan M. Huerta*

Main category: cs.CL

TL;DR: 提出 WiCER 算法，通过迭代评估与修正解决 LLM 编译 Wiki 时关键事实丢失的问题，显著降低灾难性失败率。


<details>
  <summary>Details</summary>
Motivation: LLM Wiki 模式在编译原始文档时存在关键事实被丢弃的编译差距，导致性能下降。现有方法如盲编译失败率高达 53-60%。

Method: 基于反例引导抽象精化（CEGAR）的 WiCER 算法，通过诊断探针评估编译 Wiki，识别丢失事实并强制保留，迭代 1-2 次。

Result: 在 17 个 RepLiQA 领域（6800 问题）上，WiCER 恢复 80% 质量损失（平均得分 3.24 vs 全上下文 3.47），灾难性失败减少 55%；针对性诊断是关键（+0.95 vs 通用固定 +0.16）。

Conclusion: WiCER 能有效弥合编译差距，实现低延迟、零检索失败的 LLM 上下文访问，且优于盲编译和 RAG。
...
### [251] [Beyond Single Ground Truth: Reference Monism as Epistemic Injustice in ASR Evaluation](https://arxiv.org/abs/2605.07084)
*Anna Seo Gyeong Choi,Maria Teleki,James Caverlee,Miguel del Rio,Corey Miller,Hoon Choi*

Main category: cs.CL

TL;DR: 本文揭示了ASR评估中单一转录标准（参考一元论）对失语症患者造成认知不公，并提出WER-Range作为替代方案。


<details>
  <summary>Details</summary>
Motivation: 现有ASR评估默认单一转录标准，忽视失语症患者言语中的临床意义，导致系统性歧视。

Method: 哲学框架（认知鸿沟）、公式化认知不公距离（EID）、基于AphasiaBank的实证分析，以及WER-Range的提出。

Result: 不同转录标准下WER差异显著，失语症患者表现受单一标准扭曲。

Conclusion: 应当用WER-Range替代单一WER，承认多重合法标注标准，以纠正评估中的认知不公。
...
### [252] [Self-Consolidating Language Models: Continual Knowledge Incorporation from Context](https://arxiv.org/abs/2605.07076)
...
### [253] [The Translation Tax Is Not a Scalar: A Counterfactual Audit of English-Source Cue Inheritance in Chinese Multilingual Benchmarks](https://arxiv.org/abs/2605.07093)
*Zezheng Lin,Fengming Liu,Handi Li*

Main category: cs.CL

TL;DR: 翻译税并非单一标量，而是依赖于估计器和具体题目，在英译中场景下表现出多重有效性风险。


<details>
  <summary>Details</summary>
Motivation: 质疑翻译基准中翻译税被简单视为标量假设的有效性。

Method: 使用三种代理估计器（回译差距、线索分数校准、六模型原生对照）和同题LLM自然化压力测试，并修正提示构建错误。

Result: 回译差距小且解析脆弱；线索分数校准无法预测题目级增益；原生对照显示模型家族效应而非统一基准效应；自然化测试发现高残留题目受益而低残留题目不变。

Conclusion: 翻译税是估计器和题目依赖的，应通过报告清单和自然化协议来评估有效性风险。
...
### [254] [SAGE: Hierarchical LLM-Based Literary Evaluation through Ontology-Grounded Interpretive Dimensions](https://arxiv.org/abs/2605.07102)
*Tianyu Wang,Nianjun Zhou*

Main category: cs.CL

TL;DR: 本文提出分层评估框架SAGE，通过结构化大语言模型评估文学质量，在100篇短故事上实现高可靠性和一致性，并揭示了文学质量的体裁层级与维度差异。


<details>
  <summary>Details</summary>
Motivation: 文学质量评估需要解释性维度（如文化表征、情感深度、哲学深度），但传统计算方法难以测量，因此引入基于本体论的分层评估框架。

Method: 构建SAGE分层框架，将文学质量分解为本体论解释维度，通过多轮迭代反思与独立验证，结合双模式评估（内容与元数据），在文化、情感心理和存在哲学三个层次上分析100篇故事。

Result: 600次评估中实现98.8%分数收敛、>94%评分者一致性，体裁层级显著（经典>通俗>大语言模型，p<0.001），且文化批判与哲学深度效应量极大（d>2.4），情感效应较小（d=1.68），交叉层相关性r=0.649-0.683。

Conclusion: 理论驱动的大语言模型评估能达到测量级可靠性，并系统识别当前生成模型在人类文学生产中的不足，支持开放文本生成的自动化评估。
...
### [255] [Retrieve, Integrate, and Synthesize: Spatial-Semantic Grounded Latent Visual Reasoning](https://arxiv.org/abs/2605.07106)
*Jin Cui,Xinyue Long,Xunyong Zhang,Yadong Zhang,Chuanchang Su,Jingye Gan,Boran Zhao,Pengju Ren*

Main category: cs.CL

TL;DR: 针对多模态大模型视觉推理中信息瓶颈和潜变量推理流形不兼容等问题，提出RIS框架，通过空间-语义锚定、渐进式注意力瓶颈和语言过渡令牌，实现了忠实、可解释的内部视觉推理，在多个基准上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs将视觉证据压缩为离散文本思维造成信息瓶颈，而潜变量推理存在流形不兼容、轨迹漂移、模式坍塌等问题。

Method: 构建带空间语义标注的推理数据集，通过空间-语义锚定、渐进注意力瓶颈和短语言过渡令牌，将潜变量推理与预训练范式兼容。

Result: 在V*、HRBench等五个基准上超越现有闭源、开源及潜变量推理方法。

Conclusion: RIS能学习多样、可解释且逐步整合的潜轨迹，是实现忠实内部视觉推理的有效路径。
...
### [256] [Securing Computer-Use Agents: A Unified Architecture-Lifecycle Framework for Deployment-Grounded Reliability](https://arxiv.org/abs/2605.07110)
*Zejian Chen,Zhanyuan Liu,Chaozhuo Li,Mengxiang Han,Songyang Liu,Litian Zhang,Feng Gao,Yiming Hei,Xi Zhang*

Main category: cs.CL

TL;DR: 本文提出一个架构-生命周期框架，用于分析部署环境中计算机使用代理(CUA)的可靠性，区分了失败的表现位置与引入条件，并映射了控制、监督和保证的干预面。


<details>
  <summary>Details</summary>
Motivation: 现有调查按方法、平台、基准或安全威胁组织CUA研究，但未明确连接能力形成、权限暴露、失败表现和控制放置，因此需要更系统的框架。

Method: 通过架构视角（感知、决策、执行三层）和生命周期视角（创建、部署、操作、维护四阶段）综合分析CUA系统、基准测试和安全隐私研究，并映射失败可见性与引入条件。

Result: 识别了CUA中可靠性故障的不同表现位置和根本原因，并系统化总结了控制、监督和保证的干预面。

Conclusion: 开放挑战包括可控基础、长程约束保持、安全权限绑定、混合可信防御、隐私保护记忆和持续保证。
...
### [257] [Beyond LoRA vs. Full Fine-Tuning: Gradient-Guided Optimizer Routing for LLM Adaptation](https://arxiv.org/abs/2605.07111)
*Haozhan Tang,Xiuqi Zhu,Xinyin Zhang,Boxun Li,Virginia Smith,Kevin Kuo*

Main category: cs.CL

TL;DR: 提出一种混合LoRA和全量微调的框架MoLF，动态路由更新以结合两者优势，在多种任务和模型上接近或超越最优方法，其高效版本MoLF-Efficient在内存受限场景下优于先前方法。


<details>
  <summary>Details</summary>
Motivation: 全量微调(FFT)和低秩适配(LoRA)各有优劣，静态选择任一结构存在局限性；需要统一框架来动态结合两者优势。

Method: 提出MoLF(Mixture of LoRA and Full Fine-Tuning)框架，在优化器级别动态路由更新到FFT和LoRA专家，确保梯度信号可用；并引入MoLF-Efficient版本，冻结基权重仅更新LoRA专家对。

Result: MoLF在所有设置下性能接近或优于FFT和LoRA（差距≤1.5%）；MoLF-Efficient在Fact任务上优于先前自适应LoRA方法20%，在Med和SQL上优于9%。

Conclusion: MoLF框架能统一FFT和LoRA优势，提供稳定训练动态，高效版本适合内存受限场景，效果显著。
...
### [258] [Region4Web: Rethinking Observation Space Granularity for Web Agents](https://arxiv.org/abs/2605.07134)
*Donguk Kwon,Dongha Lee*

Main category: cs.CL

TL;DR: 本文提出Region4Web框架，通过将网页的AXTree重构为功能区域，并利用PageDigest管道生成跨步骤的紧凑摘要，从而以功能区域粒度替代传统元素级观察。在WebArena基准上，该方法在减少观测长度的同时，提升了多种大语言模型和智能体方法的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有网络智能体以元素级粒度处理网页观测，忽略了页面功能组织，迫使智能体每步从元素信号推断信息，效率低下。

Method: 提出Region4Web框架，通过层次分解和语义抽象将AXTree重组为功能区域；设计PageDigest管道，将区域级观测转化为跨步骤的紧凑页面摘要。

Result: 在WebArena上，依靠不同大语言模型骨干和现有智能体方法，PageDigest显著缩短观测长度，并提高总体任务成功率。

Conclusion: 功能区域粒度的观测比元素级处理更紧凑、信息更丰富，可作为智能体行为决策的有效基础。
...
### [259] [Structural Rationale Distillation via Reasoning Space Compression](https://arxiv.org/abs/2605.07139)
*Jialin Yang,Jiankun Wang,Jiajun Wu,Henry Leung,Jiayu Zhou,Steve Drew*

Main category: cs.CL

TL;DR: 提出了一种名为D-RPC的蒸馏方法，通过约束教师模型生成一致且紧凑的推理路径，减少学生模型学习的噪声。


<details>
  <summary>Details</summary>
Motivation: 教师模型对相似问题的推理结构不一致，导致学生模型难以内化监督信号。

Method: D-RPC维护一个动态更新的紧凑推理路径库，根据训练问题检索最相关路径并约束教师模型生成一致推理。

Result: 在五个数学和常识推理基准上，D-RPC优于多种现有蒸馏方法，且使用更少的token。

Conclusion: 推理路径压缩能有效平衡路径库大小与覆盖范围，提升蒸馏效率。
...
### [260] [Beyond Reasoning: Reinforcement Learning Unlocks Parametric Knowledge in LLMs](https://arxiv.org/abs/2605.07153)
*Wanli Yang,Hongyu Zang,Junwei Zhang,Wenjie Shi,Du Su,Jingang Wang,Xueqi Cheng,Fei Sun*

Main category: cs.CL

TL;DR: 强化学习在直接回忆参数化知识上有效，平均相对提升约27%，主要通过重新分配概率分布而非获取新知识。


<details>
  <summary>Details</summary>
Motivation: 探索RL能否提升LLM的参数知识直接回忆能力，而非仅改善推理。

Method: 在零样本、单跳、闭卷QA设置中，仅使用二值正确性奖励训练，并进行事实级去重。

Result: RL平均提升约27%，最难的例子（仅占18%数据）贡献83%改进。

Conclusion: RL主要用于解锁已有知识而非获取新知识，拓宽了其在推理之外的角色。
...
### [261] [CLIPer: Tailoring Diverse User Preference via Classifier-Guided Inference-Time Personalization](https://arxiv.org/abs/2605.07162)
*Jinyan Su,Jinpeng Zhou,Claire Cardie,Wen Sun*

Main category: cs.CL

TL;DR: 本文提出CLIPer方法，通过轻量级分类器在推理时动态引导大型语言模型生成个性化内容，避免昂贵微调，实现多维度用户偏好定制。


<details>
  <summary>Details</summary>
Motivation: 现有方法微调所有用户偏好组合开销巨大且不可行，需要低成本、灵活的个人化方案。

Method: 使用分类器模型在推理阶段指导LLM生成，无需针对每种偏好组合进行微调，仅增加极少计算开销。

Result: 实验表明该方法在单维和多维偏好场景下均能有效实现可扩展、可控制的个性化语言生成。

Conclusion: CLIPer是一种高效、轻量的推理时个人化方案，兼顾灵活性与低开销，适用于动态用户需求。
...
### [262] [Rethinking Experience Utilization in Self-Evolving Language Model Agents](https://arxiv.org/abs/2605.07164)
*Weixiang Zhao,Yingshuo Wang,Yichen Zhang,Yanyan Zhao,Yu Zhang,Yang Wu,Dandan Tu,Bing Qin,Ting Liu*

Main category: cs.CL

TL;DR: 本文研究自进化代理如何更智能地利用经验，提出ExpWeaver方法，在推理时将经验作为可选资源按需使用，而非固定注入。实验表明该方法在多个框架和环境中表现最优，并可通过强化学习进一步增强。


<details>
  <summary>Details</summary>
Motivation: 现有自进化代理研究主要关注经验的构建、表示和更新，忽视了运行时如何使用经验。大多数代理采用固定策略（如初始化时一次性注入或每步都注入），缺乏对当前决策是否需要经验的考量。

Method: 提出ExpWeaver轻量级方法：保持原有经验构建不变，仅修改运行时使用方式，将经验暴露为推理过程中的可选资源，仅在需要额外指导时才被调用。

Result: 在4个框架、7种LLM骨干和3类环境中，ExpWeaver一致优于其他使用策略。强化学习实验表明该行为可通过训练增强。分析方法揭示了选择性调用经验、在有利决策点和推理不确定性高时使用的模式。

Conclusion: 研究呼吁从关注“存储什么经验”转向理解“如何以及何时使用经验”作为决策的关键维度。
...
### [263] [A Reproducible Multi-Architecture Baseline for Token-Level Chinese Metaphor Identification under the MIPVU Framework](https://arxiv.org/abs/2605.07170)
*Yufeng Wu*

Main category: cs.CL

TL;DR: 在MIPVU框架下，中文词汇级隐喻识别研究相对不足。本文基于PSU中文隐喻语料库，系统比较了三种模型族（编码器微调、MelBERT和生成式基线），发现MelBERT表现最佳（F1=0.7281），生成式模型因召回率低而落后约11个F1点。


<details>
  <summary>Details</summary>
Motivation: 隐喻在日常语言中普遍存在，但中文在MIPVU框架下的词汇级隐喻计算方法研究不足，需要建立可复现的基准。

Method: 系统比较三种模型：1) 中文RoBERTa编码器微调；2) 基于现代汉语词典第7版构建基础词义的MelBERT；3) 使用QLoRA微调的Qwen3.5-9B生成式模型。

Result: MelBERT表现最佳（F1=0.7281），其次为RoBERTa（0.7142），生成式模型最差（0.6157）。发现MelBERT的SPV通道在中文中无效，生成模型在召回率上存在明显不足。

Conclusion: MelBERT适合中文隐喻识别，生成式模型需改进格式设计；研究公布了完整资源以促进后续研究。
...
### [264] [Topology-Enhanced Alignment for Large Language Models: Trajectory Topology Loss and Topological Preference Optimization](https://arxiv.org/abs/2605.07172)
*Yurui Pan,Ke Xu,Bo Peng*

Main category: cs.CL

TL;DR: 提出拓扑增强对齐框架，利用0维持续同调正则化大语言模型的语义轨迹，优于非拓扑基线。


<details>
  <summary>Details</summary>
Motivation: 标准SFT和RLHF/DPO对齐忽略表示空间的全局几何，仅依赖局部似然或标量分数。

Method: SFT中引入轨迹拓扑损失（TTL），通过0维持续同调提取提示-答案桥梁；DPO中引入拓扑偏好优化（TPO），构建主题语义偏好向量并动态平衡损失。

Result: 在Qwen2.5-7B-Instruct上，自动偏好指标和LLM评判评估均优于强非拓扑基线，同时保持/改善毒性。
...
### [265] [Learning Agent Routing From Early Experience](https://arxiv.org/abs/2605.07180)
*Yimin Wang,Jiahao Qiu,Xuan Qi,Xinzhe Juan,Jingzhe Shi,Zelin Zhao,Hongru Wang,Shilong Liu,Mengdi Wang*

Main category: cs.CL

TL;DR: 提出BoundaryRouter框架，通过轻量级路由在LLM直接推理和完整智能体执行之间实现高效平衡，在冷启动场景下减少60.6%推理时间并提升28.6%性能。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂推理任务中表现优异但延迟和成本高，许多查询实际无需完整智能体执行，因此需要有效的路由策略。

Method: 训练无关的路由框架，利用早期行为经验和基于规则的推理构建紧凑经验记忆，在推理时检索相似案例决策路由。

Result: 在RouteBench基准测试中，相比智能体减少60.6%推理时间，相比直接LLM推理提升28.6%性能，优于提示式路由37.9%和纯检索路由8.2%。

Conclusion: BoundaryRouter证明了在冷启动设置下通过经验记忆和规则推理实现高效路由的可行性。
...
### [266] [The Text Uncanny Valley: Non-Monotonic Performance Degradation in LLM Information Retrieval](https://arxiv.org/abs/2605.07186)
*Zekai Tong,Ruiyao Xu,Aryan Shrivastava,Chenhao Tan,Ari Holtzman*

Main category: cs.CL

TL;DR: 本研究发现了大语言模型在检测单词边界被破坏的文本时存在“文本恐怖谷”现象，即检测准确率随插入空格率增加呈U型曲线。


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准主要评估语法正确的输入，忽略了真实场景中文本不完美（如单词被破坏）的情况。

Method: 通过在不同LLM中插入不同数量的空格破坏单词边界，提出模式转换假说（词级模式到字符级模式的不稳定转换导致恐怖谷），并设计4个实验和1个分析验证（上下文学习、扰动正则化、数学推理任务、tokenization熵分析）。

Result: 发现插入空格率增加时检测准确率先降后升；上下文学习无法缓解低谷；强模型在推理任务中U型减弱；tokenization熵在F1低谷前达到峰值。

Conclusion: 单词边界破坏揭示了一种传统基准无法捕捉的失效模式，直接影响存在噪声文本的真实部署场景。
...
### [267] [PSK@EEUCA 2026: Fine-Tuning Large Language Models with Synthetic Data Augmentation for Multi-Class Toxicity Detection in Gaming Chat](https://arxiv.org/abs/2605.07201)
*Srikar Kashyap Pulipaka*

Main category: cs.CL

TL;DR: 本文研究了EEUCA 2026共享任务中游戏社区毒性行为分类，通过多种方法对比，最终使用Llama 3.1 8B结合5%合成数据增强，在35支队伍中排名第4。


<details>
  <summary>Details</summary>
Motivation: 游戏社区中毒性行为检测对维护健康环境至关重要，现有方法在跨数据集泛化上存在不足。

Method: 探索了编码器模型、指令调优LLM（LoRA微调）、层次分类、一对多策略及集成方法，最佳方案采用Llama 3.1 8B并加入5%合成数据增强。

Result: 在测试集上F1-macro达到0.6234，排名第4，并揭示了“验证陷阱”现象：高验证性能不代表测试集泛化。

Conclusion: 合成数据增强和模型选择对分类性能有显著影响，验证集与测试集之间的分布偏移是泛化挑战的关键。
...
### [268] [Hallucination Detection via Activations of Open-Weight Proxy Analyzers](https://arxiv.org/abs/2605.07209)
*Akshita Singh,Prabesh Paudel,Siddhartha Roy*

Main category: cs.CL

TL;DR: 提出了一种代理分析器框架，通过小型开源模型读取已生成文本并利用其内部激活检测幻觉，在多种模型上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法需访问生成模型内部或依赖大型模型，而本文旨在利用小型模型检测任何生成模型（包括API）的输出幻觉。

Method: 构建18个基于Transformer处理的特征（残差流、注意力、熵、MLP激活、logit透镜等），使用72,135个样本训练堆叠集成，并在0.5B-9B的7种分析器上测试。

Result: 在RAGTruth上AUC比ReDeEP提升7.4-10.3个百分点，所有分析器AUC跨度仅2.3个百分点，3B模型有时甚至优于8B模型。

Conclusion: 小型开源模型可有效检测各类生成模型的幻觉，且模型大小并非关键因素，特征工程与集成更重要。
...
### [269] [Reformulating KV Cache Eviction Problem for Long-Context LLM Inference](https://arxiv.org/abs/2605.07234)
*Tho Mai,Joo-Young Kim*

Main category: cs.CL

TL;DR: 提出了一种输出感知、层级的KV缓存驱逐策略LaProx，通过建模注意力与值表示的乘法交互，实现全局统一的Token重要性评估，仅用5%缓存即可保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存驱逐方法多依赖局部注意力权重，忽略了值表示、输出投影和跨头交互的影响，导致在长上下文场景下内存和运行时开销较大。

Method: 将KV缓存驱逐重构为输出感知的层级矩阵乘法近似问题，并引入LaProx策略显式建模注意力图与投影值状态的乘法交互，从而准确量化Token贡献，提出首个统一驱逐策略实现全局重要性评分。

Result: 在19个长上下文数据集上，仅用5%KV缓存即可维持模型性能，在所有配置下均优于现有方法，极端压缩下精度损失减少2倍。

Conclusion: 新方法因改进了驱逐机制的建模精度，显著提升了长上下文推理的效率与性能，具有广泛适用性。
...
### [270] [Teaching Language Models to Think in Code](https://arxiv.org/abs/2605.07237)
*Hyeon Hwang,Jiwoo Lee,Jaewoo Kang*

Main category: cs.CL

TL;DR: 提出ThinC框架，让代码本身作为推理器而非工具，在竞赛级数学基准上超越所有TIR基线和更大模型。


<details>
  <summary>Details</summary>
Motivation: TIR中代码作为事后验证器、中间NL计算易错、NL与代码角色重叠。

Method: 先少量NL规划，后续推理完全通过代码块及其执行输出进行；从教师模型蒸馏12.2k代码中心轨迹，经SFT和RL训练ThinC-1.7B和ThinC-4B。

Result: ThinC-4B在五个竞赛级数学基准上超越所有TIR基线和Qwen3-235B-A22B-Thinking；99.2%最终答案基于解释器输出，能从代码失败中可靠恢复。

Conclusion: 代码作为推理器能显著提升数学推理性能，减少对中间NL的依赖。
...
### [271] [PaT: Planning-after-Trial for Efficient Test-Time Code Generation](https://arxiv.org/abs/2605.07248)
*Youngsik Yoon,Sungjae Lee,Seockbean Song,Siwei Wang,Wei Chen,Jungseul Ok*

Main category: cs.CL

TL;DR: 提出一种自适应策略Planning-after-Trial (PaT)，仅在验证失败时调用规划器，显著提升测试时间计算效率和模型推理性价比。


<details>
  <summary>Details</summary>
Motivation: 现有测试时间计算方法采用 Planning-before-Trial (PbT) 策略，即使对可直接求解的问题也浪费规划开销，导致计算效率低下。

Method: 采用Planning-after-Trial (PaT) 自适应策略：先用低成本模型尝试生成代码，仅在验证失败后引入强大模型进行针对性规划，支持异构模型配置。

Result: 在多个基准和模型家族上，该方法将成本-性能帕累托前沿大幅推进，异构配置在接近大型同构模型性能的同时，推理成本降低约69%。

Conclusion: PaT是一种高效、自适应的测试时间计算分配策略，能够在不牺牲性能的前提下显著降低推理成本。
...
### [272] [SpecBlock: Block-Iterative Speculative Decoding with Dynamic Tree Drafting](https://arxiv.org/abs/2605.07243)
...
TL;DR: 提出SpecBlock，一种块迭代草稿模型，结合路径依赖与低成本推理，通过层间移位和跨块继承机制提高草稿质量，并引入排名头和成本感知自适应策略，在降低开销的同时实现比EAGLE-3更优的加速效果。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码中的草稿器存在矛盾：自回归型（如EAGLE-3）依赖路径但每步计算开销大；并行型草稿快但缺乏路径一致性，导致验证拒绝率高。需要兼顾路径依赖和低成本草稿生成。

Method: 采用块迭代草稿架构：每步同时生成K个依赖位置（块），通过层间移位传递隐藏状态；跨块时从上一块任意位置继承状态扩展路径。同时，用协同训练的排名头替代固定top-k树结构，在草稿阶段分配每位置分支数；使用有效前缀掩码避免训练时的不匹配；部署时用成本感知的多臂赌博机自适应选择更新草稿器。

Result: SpecBlock在平均加速比上比EAGLE-3提升8-13%，而草稿成本仅为EAGLE-3的44-52%；加入成本感知自适应后，加速优势扩大至11-19%。

Conclusion: 块迭代机制能有效平衡草稿路径依赖和生成效率，排名头与自适应策略进一步提升了推测解码的实际部署性能，验证了在保证验证接受率的同时降低开销的可能性。
...
### [273] [From 0-Order Selection to 2-Order Judgment: Combinatorial Hardening Exposes Compositional Failures in Frontier LLMs](https://arxiv.org/abs/2605.07268)
*Hanmeng Liu,Shichao Weng,Xiulai Liu,Zhicai Zhang,Anli Yan,Xiaozhang Liu*

Main category: cs.CL

TL;DR: 提出了LogiHard框架，通过将0阶选择转化为2阶逻辑判断来增加推理难度，并利用项目反应理论实现自适应测试；实验表明LLMs在组合逻辑推理中存在显著性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有选择题基准因模型饱和和数据污染而失效，常用的增加难度方法破坏了逻辑有效性，无法有效挑战高级推理模型。

Method: 将0阶选择转化为2阶逻辑判断的形式化框架，集成项目反应理论进行自适应测试，并通过认知排名和组合变换构建LogiHard-2k数据集。

Result: 在12个模型中观察31%-56%的准确率下降；LLMs出现多选失败和提前退出偏差；零样本转移MMLU准确率从89.84%降至42.86%。

Conclusion: 性能下降源于组合推理缺口而非知识缺陷，反映了训练导致的完整性验证不足，且该退化具有领域无关性。
...
### [274] [Understanding Performance Collapse in Layer-Pruned Large Language Models via Decision Representation Transitions](https://arxiv.org/abs/2605.07271)
*Boyu Shi,Chang Liu,ChuanBao Gao,Xu Yang,Xin Geng*

Main category: cs.CL

TL;DR: 本文通过决策表示分析发现，LLM层剪枝导致性能崩溃的原因是破坏了模型从静默阶段向决策阶段的过渡。


<details>
  <summary>Details</summary>
Motivation: 现有分析难以解释层剪枝为何导致性能突然崩溃。

Method: 提出决策边缘和选项频率两个指标，以及迭代剪枝方法，分析逐层决策动态。

Result: 发现网络存在尖锐的决策过渡，分为静默阶段和决策阶段，剪枝静默阶段会立即导致性能崩溃。

Conclusion: 剪枝引发的崩溃源于破坏静默阶段，阻止了关键的决策过渡。
...
### [275] [MIPIAD: Multilingual Indirect Prompt Injection Attack Defense with Qwen -- TF-IDF Hybrid and Meta-Ensemble Learning](https://arxiv.org/abs/2605.07269)
*Al Muhit Muhtadi,Mostafa Rifat Tazwar*

Main category: cs.CL

TL;DR: 提出MIPIAD防御框架，结合序列分类器（XLPID）、TF-IDF特征和集成方法，在英语和孟加拉语上检测间接提示注入，F1达0.9205，AUROC达0.9378。


<details>
  <summary>Details</summary>
Motivation: 检索增强和工具使用的LLM系统在多语言环境下存在间接提示注入的持续弱点，且问题难以刻画。

Method: 基于Qwen2.5-1.5B通过LoRA微调的序列分类器（XLPID）、TF-IDF词汇特征，以及通过后期融合、堆叠和梯度提升的验证调优集成。

Result: TF-IDF+SVM F1=0.77，XLPID+TF-IDF集成F1=0.9205，Boosting集成AUROC=0.9378，集成方法缩小了跨语言差距。

Conclusion: 该框架可扩展至200多种语言，但当前验证仅限于英语和孟加拉语。
...
### [276] [MedAction: Towards Active Multi-turn Clinical Diagnostic LLMs](https://arxiv.org/abs/2605.07305)
...
TL;DR: 本文提出MedAction，一种树状蒸馏管道，用于合成多轮诊断轨迹，解决现有LLM在主动诊断中的失败模式，并构建了MedAction-32K数据集，微调模型性能领先。


<details>
  <summary>Details</summary>
Motivation: 现有LLM诊断在静态、单轮设置下评估，忽略真实临床中的多轮主动诊断过程，导致模型存在无根据测试、不可靠诊断更新和多轮连贯性下降等问题。

Method: 通过树状蒸馏管道，利用LLM与环境交互合成多轮诊断轨迹，并引入基于知识图谱的指标（DTC和RAC）过滤轨迹质量。

Result: 构建MedAction-32K数据集，包括32,681条来自2,896个PMC病例的轨迹，微调8B模型在MedR-Bench和MedAction-300-Hard基准上达到开源模型最佳性能。

Conclusion: 该方法有效提升了LLM在多轮主动诊断中的表现，为开源医学LLM设定了新标杆。
...
### [277] [Rethinking Dense Sequential Chains: Reasoning Language Models Can Extract Answers from Sparse, Order-Shuffling Chain-of-Thoughts](https://arxiv.org/abs/2605.07307)
*Yi-Chang Chen,Feng-Ting Liao,Da-shan Shiu,Hung-yi Lee*

Main category: cs.CL

TL;DR: 本文通过系统性干预实验（删除、掩码、打乱、噪声注入）发现，推理链的答案提取在行级打乱下准确率下降小于0.5个百分点，掩盖数字导致准确率归零，而删除所有自然语言并打乱行仍保持83%准确率，表明答案提取基于稀疏、顺序不敏感且结构稳健的信息基板。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型假设推理链中每个token都贡献信息且步骤必须按顺序消费，为检验这些假设的可靠性，作者设计系统性干预实验。

Method: 对模型生成的推理链进行删除、掩码、打乱（行/词/token级）和噪声注入干预，在三个模型和三个基准上测试答案提取准确率。

Result: 行级打乱准确率下降<0.5pp；词级打乱保留62%-89%；token级打乱准确率近零。掩码数字使准确率归零，掩码文字却提升4.7pp。去除所有文字并打乱行仍达83%准确率，注入3倍假答案不影响准确率。

Conclusion: 答案提取依赖稀疏、顺序不敏感且稳健的信息结构，表明推理生成可走向并行化和token高效化。
...
### [278] [LaTER: Efficient Test-Time Reasoning via Latent Exploration and Explicit Verification](https://arxiv.org/abs/2605.07315)
*Xuan Li,Yining Wang,Yuchen Liu,Guanjun Liu,Delai Qiu,Shengping Liu,Jiaen Liang,Wei Huang,Jun Yu,Junnan Zhu*

Main category: cs.CL

TL;DR: 提出LaTER范式，通过先进行有限步的潜在空间推理再切换到显式CoT，减少推理代价并提升性能。


<details>
  <summary>Details</summary>
Motivation: CoT推理虽能提升LLM在困难任务上的能力，但生成所有中间步骤导致推理成本高昂，而潜在推理虽能减少可见token生成，但可能损害需要符号检查的任务。

Method: LaTER两阶段范式：先进行潜在空间有界探索，再切换到显式CoT验证和答案生成。训练时采用潜在展开和停止监督。

Result: 在Qwen3-14B上，无训练LaTER减少16%-32%的token用量，多数基准上持平或略升精度；AIME 2025上从70.0%提升至73.3%且token从15730降至10661。微调后在AIME 2025达80.0%，比标准CoT基线高10.0点且token减少33%。

Conclusion: LaTER能有效结合潜在推理的高效和显式推理的可靠性，在减少计算开销的同时提升或保持推理精度。
...
### [279] [Activation Differences Reveal Backdoors: A Comparison of SAE Architectures](https://arxiv.org/abs/2605.07324)
*Sachin Kumar*

Main category: cs.CL

TL;DR: 本文研究了两种稀疏自编码器（Crosscoders和Diff-SAE）在检测语言模型后门中的表现，发现Diff-SAE在隔离后门相关特征方面显著优于Crosscoders。


<details>
  <summary>Details</summary>
Motivation: 后门攻击对AI安全构成严重威胁，通过机械可解释性检测后门仍是开放挑战，需要有效方法隔离后门特征。

Method: 使用Crosscoders和Diff-SAE两种架构，在SmolLM2-360M模型上，通过基于年份的SQL注入后门（2024触发危险、2023安全），在LoRA和全秩微调下进行实验。

Result: Diff-SAE在大多条件下后门隔离分数（BIS）达0.40，精确率1.0，假正率0；Crosscoders BIS低于0.02。差异跨多个层和微调模式保持。

Conclusion: 后门表现为方向性激活偏移而非稀疏特征激活，基于差异的表征更有效检测后门，对AI安全监控有重要启示。
...
### [280] [Gradient-Based LoRA Rank Allocation Under GRPO: An Empirical Study](https://arxiv.org/abs/2605.07366)
*Yash Ganpat Sawant*

Main category: cs.CL

TL;DR: 研究了LoRA自适应秩分配在强化学习（GRPO）中的效果，发现其不如统一分配，并揭示了梯度平坦和放大效应导致失败。


<details>
  <summary>Details</summary>
Motivation: 探究监督微调中有效的自适应秩分配能否迁移到强化学习（GRPO）中。

Method: 在Qwen 2.5 1.5B模型上使用GSM8K数据集，通过梯度幅度分析比较比例分配与统一分配。

Result: 比例分配导致准确率下降4.5%（70.0% vs 74.5%），梯度层重要性比仅为2.17倍，且非均匀分配扩大差异至3.00倍。

Conclusion: 梯度重要性不能预测RL中的容量需求，不应简单迁移SFT秩分配策略到对齐训练中。
...
### [281] [Mean-Pooled Cosine Similarity is Not Length-Invariant: Theory and Cross-Domain Evidence for a Length-Invariant Alternative](https://arxiv.org/abs/2605.07345)
*Sibayan Mitra,Dhruv Kumar*

Main category: cs.CL

TL;DR: 平均池化余弦相似度不是长度不变的度量，在表征各向异性的情况下会随序列长度单调增长。使用中心核对齐（CKA）可以消除这一偏差，建议将其作为跨表征比较的默认度量。


<details>
  <summary>Details</summary>
Motivation: 当前研究普遍使用平均池化余弦相似度比较跨语言、跨模态的神经表征，但该度量可能受序列长度影响，导致虚假的相似性结论。

Method: 通过理论分析证明平均池化余弦的长度依赖性，并在代码LLM（HumanEvalPack）、多语言模型（Mistral-7B）和视觉模型（CLIP）上进行实验验证，比较余弦相似度与CKA的表现。

Result: 长度比率解释了代码模型中52%-75%的跨语言相似性变异；使用CKA后，变异解释率降低83%且长度系数符号反转；在WMT翻译对上结果类似；CLIP中平均池化比EOS池化更能降低长度效应。

Conclusion: 平均池化余弦相似度因各向异性而高估长度匹配的表征，CKA等长度不变度量应成为标准；基于该度量的跨语言表征趋同研究需要重新检验。
...
### [282] [The Proxy Presumption: From Semantic Embeddings to Valid Social Measures](https://arxiv.org/abs/2605.07409)
*Baishi Li,Ta Yu,Kelvin J. L. Koa,Ke-Wei Huang*

Main category: cs.CL

TL;DR: 本文提出构建效度协议（CVP），通过因果表示学习和心理测量方法，将无监督嵌入转化为有效的社会科学测量工具。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理在计算社会科学中的使用面临“代理预设”挑战，即无监督嵌入的几何性质（如余弦距离）直接用于衡量社会概念时缺乏验证，易受混淆变量干扰。

Method: 提出构建效度协议（CVP），包括反事实中和法（减少混淆变量）和效度套件（区分效度、增量效度、预测效度检验）。

Result: CVP能够将启发式代理转化为稳健的科学测量工具，减少嵌入空间中的混淆偏差。

Conclusion: 该工作为社区提供了标准化工具，将无监督嵌入从启发式代理转变为可防御的科学测量。
...
### [283] [Generating training datasets for legal chatbots in Korean](https://arxiv.org/abs/2605.07432)
*Changhoe Hwang,Jee-Sun Nam,Eric Laporte*

Main category: cs.CL

TL;DR: 提出一种基于本地语法图（LGG）联合生成大规模话语和高质量标签的方法，用于训练法律聊天机器人意图分类器，在韩语LIGA系统中实现91% F1分数。


<details>
  <summary>Details</summary>
Motivation: 法律咨询成本高，弱势群体难以获取司法服务，但现有聊天机器人面临数据集多样性与标注成本之间的矛盾。

Method: 使用语言学家构建的本地语法图（LGG）捕捉词汇和局部句法，联合生成标注话语与标签，通过Unitex平台生成7亿条话语，训练DIET分类器。

Result: 训练的分类器F1分数达91%，LIGA聊天机器人能根据查询返回相关法律案例链接。

Conclusion: LGG方法能有效生成大规模高质量标注数据，降低标注成本，提升法律聊天机器人的意图识别性能。
...
### [284] [SSP-based construction of evaluation-annotated data for fine-grained aspect-based sentiment analysis](https://arxiv.org/abs/2605.07446)
*Suwon Choi,Shinwoo Kim,Changhoe Hwang,Gwanghoon Yoo,Eric Laporte,Jeesun Nam*

Main category: cs.CL

TL;DR: 构建了韩语评价标注语料库EVAD，并扩展了基于方面的情感分析(ABSA)以覆盖电商评论中的情感与非情感语言模式，使用半自动符号传播(SSP)进行标注，利用有限状态转换器(FST)构建语言资源，训练KoBERT和KcBERT模型在方面-值对识别上达到F1 0.88和0.90。


<details>
  <summary>Details</summary>
Motivation: 现有ABSA方法在处理电商评论时难以覆盖非情感语言模式，且对目标特征的提取不够详细。

Method: 使用Semi-Automatic Symbolic Propagation (SSP)进行标注，构建基于Finite-State Transducer (FST)的语言资源，扩展ABSA以包含方面值，并分类值为一元、二元或多元。

Result: KoBERT和KcBERT模型在方面-值对识别上分别达到F1 0.88和0.90。

Conclusion: 所提方法能有效识别电商评论中的详细情感特征，扩展ABSA的覆盖范围。
...
### [285] [Data Contamination in Neural Hieroglyphic Translation: A Reproducibility Study](https://arxiv.org/abs/2605.07453)
*Ammar Toutou,Abdelrahman Harb,Christine Basta*

Main category: cs.CL

TL;DR: 研究发现，在象形文字到德语翻译任务中，数据污染导致BLEU评分虚高（从61.5降至37.0），并提出了去污染方案，建立了更现实的基线（30.9-39.2 BLEU）。


<details>
  <summary>Details</summary>
Motivation: 濒危语言语料稀缺且质量易被忽视，需要了解当前NMT对此类语言的真实能力。

Method: 通过复制实验发现数据污染，采用文档级和句子级去重（8-gram重叠阈值70%），释放去污染测试集。

Result: 污染样本BLEU高达83.8，清洁样本仅30.9-39.2；文档级去污染仅减少4.6 BLEU，需句子级去重。

Conclusion: 数据污染严重高估NMT性能，必须进行句子级去污染才能获得可靠评估。
...
### [286] [GRaSp: Automatic Example Optimization for In-Context Learning in Low-Data Tasks](https://arxiv.org/abs/2605.07454)
*Simen Bihaug-Frøyland,Henrik Brådland*

Main category: cs.CL

TL;DR: 提出GRaSp框架，通过生成合成候选集、聚类降维和遗传算法优化，在金融NER任务中提升少样本性能，尤其依赖候选集的分布多样性。


<details>
  <summary>Details</summary>
Motivation: 针对低资源领域缺乏高质量示例的问题，以及大模型对示例选择的高敏感性。

Method: 三阶段框架：1）大规模合成候选池生成；2）聚类与降维结构化；3）遗传算法搜索最优示例，并引入多样性自适应变异机制。

Result: 在FiNER-139数据集上，非合成数据达45.84% micro-F1，优于零样本和随机少样本基线；合成数据仅匹配随机基线。

Conclusion: 候选池的分布多样性对泛化至关重要，合成数据需解决多样性不足问题。
...
### [287] [Think-with-Rubrics: From External Evaluator to Internal Reasoning Guidance](https://arxiv.org/abs/2605.07461)
*Jiachen Yu,Zhihao Xu,Junjie Wang,Yujiu Yang*

Main category: cs.CL

TL;DR: 提出Think-with-Rubrics范式，将评分标准融入生成过程，提升指令遵循任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有评分标准框架仅作为外部评估器，无法主动引导模型生成过程。

Method: 训练LLM先生成评分标准再生成回复，并用评分验证器联合监督自生成/黄金标准一致性。

Result: 在多个基准测试中平均优于Rubric-as-Reward基线3.87分。

Conclusion: 黄金标准监督提升评分标准质量，自生成监督增强回复内部一致性。
...
### [288] [SEIF: Self-Evolving Reinforcement Learning for Instruction Following](https://arxiv.org/abs/2605.07465)
*Qingyu Ren,Qianyu He,Jiajie Zhu,Xingzhou Chen,Jingwen Chang,Zeye Sun,Han Xia,Fei Yu,Jiaqing Liang,Yanghua Xiao*

Main category: cs.CL

TL;DR: 提出SEIF框架，通过自演化强化学习持续提升大语言模型的指令遵循能力，实验表明该框架在不同规模和架构的模型上均有效。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部监督或静态难度指令，无法随模型能力进化而动态调整指令难度。

Method: 构建包含指导员、过滤器、跟随者和评判者的自演化闭环，通过交替训练指导员和跟随者实现指令难度与模型能力的协同进化。

Result: 多规模多架构实验显示SEIF一致提升指令遵循性能，并发现早期充分训练与后期适度训练的最佳策略。

Conclusion: SEIF框架能有效实现指令遵循能力的自我进化，为开放任务的自演化训练提供了有效策略。
...
### [289] [The Moltbook Files: A Harmless Slopocalypse or Humanity's Last Experiment](https://arxiv.org/abs/2605.07462)
*William Brach,Federico Torrielli,Stine Lyngsø Beltoft,Annemette Brok Pirchert,Peter Schneider-Kamp,Lukas Galke Poech*

Main category: cs.CL

TL;DR: 文章介绍了Moltbook平台，一个由AI智能体模拟的Reddit式社区，分析了其数据集的结构、内容特征及对语言模型安全性的影响，发现模型微调后真实性下降，但类似现象也出现在普通Reddit数据中。


<details>
  <summary>Details</summary>
Motivation: 研究AI智能体大规模互动的安全风险，以及新兴行为对下一代语言模型的潜在污染。

Method: 收集12天数据（23.2万帖子、220万评论），通过PII清洗管道处理数据；微调Qwen2.5-14B-Instruct模型三种程度的数据，并对比Reddit数据集。

Result: 智能体在公共平台泄露API密钥、密码等敏感信息；情感以中性（66.6%）和微正面（19.5%）为主；模型真实性从0.366降至0.187，但Reddit微调也有类似下降。

Conclusion: Moltbook数据当前无害（“无害垃圾场”），但存在尾部风险，如智能体权限滥用、未来爬虫污染和特质传递；强调对照基线在评估新兴误导行为中的重要性。
...
### [290] [TCMIIES: A Browser-Based LLM-Powered Intelligent Information Extraction System for Academic Literature](https://arxiv.org/abs/2605.07507)
*Hanqing Zhao*

Main category: cs.CL

TL;DR: 提出了TCMIIES，一个基于浏览器、无需安装的平台，利用商业大模型API从学术文献中提取结构化信息，在中医药领域实现了94%以上的结构化输出符合率和与专家相当的提取准确率。


<details>
  <summary>Details</summary>
Motivation: 学术出版物指数增长，现有自动化工具需要专门基础设施、编程知识或领域模型，对专业领域研究人员构成障碍。

Method: 基于schema引导的提示框架（自动生成系统提示）、纯前端架构（浏览器本地处理）、支持五种大模型提供商、并发批处理与自动重试、针对中文数据库的智能字段映射。

Result: 在中医药多个提取场景中，结构化输出符合率超过94%，信息提取精度与领域专家标注相当。

Conclusion: TCMIIES弥合了大模型能力与领域学术信息提取需求之间的鸿沟，为专业领域研究人员提供了灵活、隐私保护且成本效益高的提取工具。
...
### [291] [WeatherSyn: An Instruction Tuning MLLM For Weather Forecasting Report Generation](https://arxiv.org/abs/2605.07522)
*Zinan Zheng,Yang Liu,Nuo Chen,Juepeng Zheng,Hong Cheng,Jia Li*

Main category: cs.CL

TL;DR: 提出了天气预报报告生成任务，构建了首个指令微调数据集WFR，并开发了专用模型WeatherGPT，在多个指标上优于闭源多模态大模型。


<details>
  <summary>Details</summary>
Motivation: 现有天气预报依赖人工分析多源数据，效率低且未充分探索多模态大模型在该领域的应用。

Method: 构建覆盖美国31城市和8个天气方面的指令微调数据集，基于此训练首个天气预报报告生成模型WeatherGPT。

Result: WeatherGPT在结构复杂天气方面表现优于闭源模型，并展现出跨区域零样本泛化能力。

Conclusion: WeatherGPT为开发专用天气预报报告生成的多模态大模型提供了重要参考。
...
### [292] [Why do Large Language Models Fail in Low-resource Translation? Unraveling the Token Dynamics of Large Language Models for Machine Translation](https://arxiv.org/abs/2605.07533)
*Shenbin Qian,Yves Scherrer*

Main category: cs.CL

TL;DR: 本文系统分析了大语言模型（LLMs）在机器翻译中的失败模式，引入标记激活率（TAR）作为评估语言表示有效性的指标，发现低TAR与低翻译质量相关，且推理型LLMs会生成更多标记来补偿。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注提升或基准测试LLMs的翻译质量，缺乏对翻译失败原因和模式的深入分析。

Method: 评估15个模型（包括4个推理型LLM）在22个语言对上的表现，提出Token Activation Rate (TAR) 指标衡量词汇利用效率，并验证其与语言表示和翻译性能的关联。

Result: 非英语中心语言对的COMET分数更低；低TAR与翻译质量差强关联；推理型LLMs在低TAR语言上生成更多标记以补偿。

Conclusion: 标记级动态对理解LLMs的机器翻译性能至关重要，低TAR揭示了模型在低资源语言上的固有缺陷，但推理型模型可能通过额外计算部分缓解。
...
### [293] [Nürnberg NLP at PsyDefDetect: Multi-Axis Voter Ensembles for Psychological Defence Mechanism Classification](https://arxiv.org/abs/2605.07606)
*Philipp Steigerwald,Eric Rudolph,Jens Albrecht*

Main category: cs.CL

TL;DR: 提出了一种9分类器集成方法，通过粒度、训练方法和基础模型三个正交轴实现误差独立性，在防御机制检测任务中取得第一名（F1=0.420）。


<details>
  <summary>Details</summary>
Motivation: 心理防御机制检测存在类别边界模糊、标注者一致性低的问题，单一模型难以应对类别重叠导致的误差。

Method: 构建9个分类器集成：1个门控覆盖9类，8个专家仅覆盖8个防御类；使用生成式与判别式两种训练方法；采用不同基础模型。

Result: 在隐藏测试集上F1=0.420，在21个注册队伍中排名第一。

Conclusion: 多维度误差独立性集成方法优于强单一模型，是处理模糊分类任务的有效策略。
...
### [294] [Intent-Driven Semantic ID Generation for Grounded Conversational News Recommendation](https://arxiv.org/abs/2605.07613)
*Hongyang Su,Beibei Kong,Lei Cheng,Chengxiang Zhuo,Zang Li,Chenyun Yu*

Main category: cs.CL

TL;DR: 提出了一种基于意图驱动的语义ID（SID）生成范式，结合生成-匹配策略，解决了对话式新闻推荐中的隐式意图和冷启动问题，在7B模型上实现了零幻觉和高效推荐。


<details>
  <summary>Details</summary>
Motivation: 对话式新闻推荐中用户意图隐式且缺乏可检索关键词，标准RAG流程存在检索瓶颈，难以处理隐式意图类型（占6种中的5种），且冷启动用户无法获得有效推荐。

Method: 采用生成-匹配范式：两阶段训练（多任务SID对齐+GPT-4思维链蒸馏），LLM将意图映射到层次化SID前缀，然后与新闻池模糊匹配；同时提出轮廓感知双信号推理（PADR）支持冷启动用户。

Result: 在中文新闻平台上，7B模型达到0%幻觉和12.4% L1匹配（随机基线4倍）；匹配GPT-4+混合RAG的L1性能，在细粒度指标（L2 2倍，类别+1.2pp）上超越，成本降低约100倍；冷启动用户L1达18.0%（随机6倍），为所有用户组最高。

Conclusion: 意图驱动的SID生成范式有效解决了隐式意图推荐和冷启动问题，实现了高性能、低成本且完全基于事实的推荐，优于现有基线方法。
...
### [295] [Safe, or Simply Incapable? Rethinking Safety Evaluation for Phone-Use Agents](https://arxiv.org/abs/2605.07630)
...
TL;DR: 提出PhoneSafety基准，区分不安全判断与无能为力，评估8个模型发现能力提升不保证安全，失败行为反映能力而非安全。


<details>
  <summary>Details</summary>
Motivation: 现有评测无法区分安全行为与无力执行，导致误判，需要分离不安全判断与能力限制。

Method: 构建700个安全关键时刻基准，基于真实手机交互，评估模型在风险时刻的决策与执行。

Result: 强能力不保证安全决策；失败行为集中在高要求场景，反映能力限制；不安全判断与无力执行需分开分析。

Conclusion: 无害结果不足为安全证据，评估需分离判断错误与能力不足。
...
### [296] [Post-training makes large language models less human-like](https://arxiv.org/abs/2605.07632)
...
TL;DR: 大型语言模型（LLMs）作为人类参与者的替代品使用时，其行为对齐性在训练后下降，且新模型代际中差距扩大；角色诱导技术对个体预测无效。


<details>
  <summary>Details</summary>
Motivation: 现有研究不清楚哪些LLMs最接近人类行为及其原因，需要系统评估行为对齐性。

Method: 引入Psych-201数据集，测量不同LLMs在训练前后的行为对齐性，并测试角色诱导方法的效果。

Result: 训练后（后训练阶段）的行为对齐性显著降低，且这一趋势在新模型代际中加剧；角色诱导无法改善个体水平预测。

Conclusion: 将LLMs转化为有用助手的过程反而降低了其作为人类行为模型的准确性。
...
### [297] [Is She Even Relevant? When BERT Ignores Explicit Gender Cues](https://arxiv.org/abs/2605.07622)
*Jonas Klein,Chiara Manna,Eva Vanmassenhove*

Main category: cs.CL

TL;DR: 该论文研究荷兰语BERT模型中的性别偏见，发现模型在训练过程中性别信息逐渐线性可分，但无法有效利用显性性别线索（如“她是水管工”）来修正对反刻板印象职业的性别预测，导致系统性男性默认行为。


<details>
  <summary>Details</summary>
Motivation: 英语中大语言模型的性别偏见研究已较充分，但形态性别和语法性别语言（如荷兰语）的偏见形成过程尚不明确，亟需探索模型何时及如何编码性别信息。

Method: 从零训练荷兰语BERT模型，在训练过程中提取各检查点的上下文嵌入，使用线性支持向量机构建动态性别子空间，并通过受控句子模板（含显性性别线索和刻板/反刻板职业配对）评估模型对性别表示的动态调整能力。

Result: 性别信息在训练约20个epoch后线性可分，且分散于多个嵌入维度；但模型对反刻板印象配对的预测准确性显著低于刻板印象配对；荷兰语中性形式默认男性解读，即使上下文明确指向女性。

Conclusion: 荷兰语BERT的上下文表示在性别维度上动态性不足，无法可靠反映显性性别线索，导致模型持续呈现男性默认的性别偏见。
...
### [298] [Multi-Dimensional Evaluation of LLMs for Grammatical Error Correction](https://arxiv.org/abs/2605.07635)
*Adnan Labib,Qiao Wang,Yixuan Huang,Zheng Yuan*

Main category: cs.CL

TL;DR: 本研究评估了最新的大语言模型在语法纠错任务中的表现，发现精调的GPT-4o在所有三个维度（编辑精确度、流畅性保留和意义保留）上达到最优，且单个模型的纠错模式高度相似（ρ=0.947），参考基准则低估了73.76%的GPT-4o纠错结果。


<details>
  <summary>Details</summary>
Motivation: 现有语法纠错领域存在三个关键空白：最新大语言模型缺乏全面评估、多模型组合效果未知、参考基准确度量偏差未被量化。

Method: 通过编辑精确度、流畅性保留和意义保留三维度评估最新LLM，进行语法错误类型分析比较模型纠错模式，并量化参考基准确度量偏差。

Result: 精调GPT-4o达到最优性能，各LLM纠错模式高度相似（ρ=0.947），73.76%的GPT-4o修正与黄金标准不同但等效或更优。

Conclusion: 研究为教育者选择语法纠错辅助工具提供指导，确保工具促进而非限制学生语言发展，并公开了数据、代码和模型。
...
### [299] [MAVEN: Multi-Agent Verification-Elaboration Network with In-Step Epistemic Auditing](https://arxiv.org/abs/2605.07646)
*Yinsheng Yao,Jiehao Tang,Zhaozhen Yang,Dawei Cheng*

Main category: cs.CL

TL;DR: 提出了MAVEN框架，通过多智能体角色解耦和对抗性验证循环，增强推理轨迹的可审计性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有推理方法缺乏中间验证，导致错误级联，无法满足高风险应用对可审计性和认知信任的需求。

Method: 基于黑板架构，设计Skeptic-Researcher-Judge对抗循环，实现逻辑防御与事实基础的显式分离和模块化验证。

Result: 在OpenBookQA、TruthfulQA等基准上，MAVEN在细粒度指标上优于Gemini-3.1-Pro和ReConcile等基线，且完全模型无关。

Conclusion: MAVEN作为可迁移的推理增强器，通过结构化、模块化的可验证轨迹，显著提升多种骨干模型的推理质量。
...
### [300] [Quality-Conditioned Agreement in Automated Short Answer Scoring: Mid-Range Degradation and the Impact of Task-Specific Adaptation](https://arxiv.org/abs/2605.07647)
*Abigail Victoria Gurin Schleifer,Moriah Ariely,Beata Beigman Klebanov,Asaf Salman,Giora Alexandron*

Main category: cs.CL

TL;DR: 该研究探讨了不同任务适配程度的AI模型在自动短答案评分中的表现，发现所有AI模型在完全正确和完全错误的答案上表现良好，但在中等质量答案上存在显著退化，且退化程度与任务特定适配程度相关。


<details>
  <summary>Details</summary>
Motivation: 从判别式微调模型转向大规模语言模型（LLMs）在少样本设置中的应用，虽然利用了LLMs的广泛知识，但任务特定数据有限可能导致评分对齐问题，尤其是对需要细致解释的部分正确答案的影响尚不明确。

Method: 比较了三种LLMs（GPT-5.2、GPT-4o、Claude Opus 4.5）在少样本模式下、一个微调的基于BERT的编码器以及一位人类专家在两个开放式生物学问题上的表现，使用了数百个学生回答和生物学教育专家提供的地面真值分数。

Result: 人类专家之间的一致性最高且稳定；所有AI模型在完全正确和完全错误的答案上表现良好，但在中等质量答案上出现显著退化；退化程度与任务特定适配程度相关：少样本LLMs退化最严重，随着任务数据增加而减轻，微调编码器表现最好。

Conclusion: 中等答案的退化可能导致对学生发展中的理解产生不公平评估，强调了质量条件公平性的重要性，尤其需要关注中等质量的回答。
...
### [301] [DRIP-R: A Benchmark for Decision-Making and Reasoning Under Real-World Policy Ambiguity in the Retail Domain](https://arxiv.org/abs/2605.07699)
*Hsuvas Borkakoty,Sebastian Pohl,Cheng Wang,Bei Chen,Yufang Hou*

Main category: cs.CL

TL;DR: 提出了DRIP-R基准，用于评估LLM代理在真实世界政策模糊环境下的决策能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准假设政策明确，忽略实际中普遍存在的政策模糊性。

Method: 利用零售退货政策中的真实歧义构建场景，包含客户角色、全双工对话模拟工具和多方评估框架。

Result: 前沿模型在相同模糊场景中产生根本性分歧，证实歧义对LLM决策构成系统挑战。

Conclusion: 政策模糊性导致LLM代理决策不一致，需要针对性设计来处理歧义。
...
### [302] [Not All Tokens Learn Alike: Attention Entropy Reveals Heterogeneous Signals in RL Reasoning](https://arxiv.org/abs/2605.07660)
*Gengyang Li,Zheng-Fan Wu,Siqi Bao,Yunfang Wu*

Main category: cs.CL

TL;DR: 研究发现强化学习后训练中令牌级别的信号存在异质性，通过注意力熵可划分为锚点（低熵）和探索者（高熵）两类，后者虽梯度不稳定但可能包含推理信号。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的后训练通常依赖令牌级别的强化学习信号，但这些信号的本质和异质性尚不明确。

Method: 使用注意力熵度量令牌的上下文支持集中度，分析令牌级RL目标的稀疏性、梯度特性，并设计动态重加权干预。

Result: 均匀随机20%令牌子集保留大部分性能；低熵锚点提供稳定优化骨干，高熵探索者梯度大而不稳定但可能包含推理信号；动态重加权提升平均准确率（34.39→37.40）。

Conclusion: 注意力熵揭示了令牌级RL信号中的优化相关结构，均匀令牌平均会掩盖有意义的异质性。
...
### [303] [Guidance Is Not a Hyperparameter: Learning Dynamic Control in Diffusion Language Models](https://arxiv.org/abs/2605.07701)
*Fan Zhou,Tim Van de Cruys*

Main category: cs.CL

TL;DR: 提出一种通过强化学习动态调整分类器自由引导（CFG）尺度的方法，以改善扩散模型在自然语言生成中的可控性与质量平衡。


<details>
  <summary>Details</summary>
Motivation: 固定CFG尺度无法适应不同任务和扩散过程不同阶段的最优引导需求，导致可控性与质量之间的次优权衡。

Method: 将CFG尺度选择建模为序贯决策问题，使用PPO算法训练策略，在每个生成步骤根据扩散状态动态选择离散的引导尺度，并基于任务级奖励进行优化。

Result: 在三个受控NLP生成任务上，自适应引导相比固定尺度策略在可控性和生成质量间取得更优平衡，且学到的策略在不同任务中呈现可解释的差异化引导轨迹。

Conclusion: 引导应被视为动态控制过程而非静态设计选择，强化学习能有效学习任务自适应的引导策略。
...
### [304] [SimCT: Recovering Lost Supervision for Cross-Tokenizer On-Policy Distillation](https://arxiv.org/abs/2605.07711)
*Jie Sun,Mao Zheng,Mingyang Song,Qiyong Zhong,Yilin Cheng,Bichuan Feng,Pengfei Liu,Junfeng Fang,Xiang Wang*

Main category: cs.CL

TL;DR: 提出SimCT方法，通过扩展监督空间（共享标记+多标记连续片段）解决异质分词器下策略蒸馏中信号丢失问题，在数学推理和代码生成任务上优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有同策略蒸馏（OPD）假设师生模型共享分词器，但异质分词器下共享标记匹配会丢弃大量教师信号，导致监督失效。

Method: 构造最短可联合分词单元（共享标记+多标记连续片段），在不改变损失函数形式的前提下扩大监督空间，恢复被丢弃的教师信号。

Result: 在三个异质师生模型对（数学/代码任务）上，SimCT持续优于共享词汇OPD及跨分词基线；消融实验证实收益来自恢复被丢弃的监督。

Conclusion: 多标记连续片段是异质分词器下最细粒度的可联合监督接口，比共享标记匹配更有效，且粗粒度替代方案会削弱有益区分信息。
...
### [305] [Memory-Efficient Looped Transformer: Decoupling Compute from Memory in Looped Language Models](https://arxiv.org/abs/2605.07721)
*Victor Conchello Vendrell,Arnau Padres Masdemont,Niccolò Grillo,Jordi Ros-Giralt,Arash Behboodi,Fabio Valerio Massoli*

Main category: cs.CL

TL;DR: 提出了MELT架构，通过跨循环共享KV缓存和可学习门控机制，将推理深度与内存消耗解耦，实现了恒定内存的迭代推理。


<details>
  <summary>Details</summary>
Motivation: 现有循环LLM架构（如Ouro）随推理深度增加，内存消耗线性增长，限制了实际可扩展性。

Method: 采用逐层共享KV缓存并使用时序门控更新，结合插值过渡和注意力对齐蒸馏的两阶段分块训练方法。

Result: 从Ouro微调的MELT模型性能优于同尺寸标准LLM，内存接近标准模型，远小于Ouro。

Conclusion: MELT以轻量后训练实现了恒定内存的迭代推理，不损失LoopLM性能。
...
### [306] [SOD: Step-wise On-policy Distillation for Small Language Model Agents](https://arxiv.org/abs/2605.07725)
*Qiyong Zhong,Mao Zheng,Mingyang Song,Xin Lin,Jie Sun,Houcheng Jiang,Xiang Wang,Junfeng Fang*

Main category: cs.CL

TL;DR: 提出了SOD框架，通过步进式蒸馏解决小模型工具推理中错误级联问题，在多个基准上提升达20.86%。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在工具推理中因错误级联导致蒸馏信号不可靠，现有方法（如OPD）无法有效处理此问题。

Method: 步进式在线蒸馏（SOD）：基于步级发散度自适应调整蒸馏强度，在高发散区域弱化误导教师信号，在低发散区域保留密集指导。

Result: 在数学、科学和代码基准上，SOD比最优基线提升最高20.86%；0.6B模型在AIME 2025上达到26.13%准确率。

Conclusion: SOD有效实现轻量模型的智能体推理迁移，可扩展至小规模语言模型。
...
### [307] [Benchmarking EngGPT2-16B-A3B against Comparable Italian and International Open-source LLMs](https://arxiv.org/abs/2605.07731)
*Andrea Sassella,Andrea Chizzola,Tommaso Bianchi,Luca Alessandrelli,Mark James Carman*

Main category: cs.CL

TL;DR: EngGPT2MoE-16B-A3B是一个16B参数的混合专家模型，在国际基准测试中表现优于同类意大利模型，但在国际顶尖模型面前仍有差距，是意大利语言模型的重要进步。


<details>
  <summary>Details</summary>
Motivation: 旨在开发一个高性能的意大利语原生大型语言模型，以推动意大利语AI技术发展。

Method: 采用16B参数混合专家架构，其中3B参数被激活，通过国际和意大利语基准测试进行性能评估。

Result: 在国际基准（如ARC、GSM8K、MMLU）上优于FastwebMIIA-7B等意大利模型，但在RULER长上下文和意大利语IT ALIC测试中不及Velvet-14B；与MoE模型相比，优于DeepSeek-MoE-16B-Chat但逊于GPT-OSS-20B；聚合性能领先意大利模型但低于GPT-5 nano等国际模型。

Conclusion: EngGPT2MoE-16B-A3B是意大利语LLM的重要进展，但在全球范围内仍需提升竞争力。
...
### [308] [TextLDM: Language Modeling with Continuous Latent Diffusion](https://arxiv.org/abs/2605.07748)
...
TL;DR: 提出TextLDM，将扩散变换器应用于文本生成，通过VAE与表示对齐实现高质量文本表示，性能优于现有扩散语言模型。


<details>
  <summary>Details</summary>
Motivation: 探索扩散变换器从视觉生成向文本生成的迁移，推动统一架构实现多模态生成与理解。

Method: 使用Transformer VAE将离散令牌映射为连续潜在表示，并通过冻结的预训练语言模型进行表示对齐（REPA），再用标准DiT进行流匹配。

Result: TextLDM在OpenWebText2上从零训练，显著超越现有扩散语言模型，在相同设置下匹配GPT-2性能。

Conclusion: 视觉DiT食谱可有效迁移至语言任务，为统一扩散架构的多模态生成与理解奠定基础。
...
### [309] [Chain-based Distillation for Effective Initialization of Variable-Sized Small Language Models](https://arxiv.org/abs/2605.07783)
*Boyu Shi,YiCheng Jiang,Chang Liu,Qiufeng Wang,Xu Yang,Xin Geng*

Main category: cs.CL

TL;DR: 提出链式蒸馏（CBD）范式，通过构建中间模型锚点逐步传递知识，支持高效初始化变体大小的语言模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型部署成本高，小模型从头训练昂贵，传统知识蒸馏因重复访问大教师模型而缺乏可扩展性。

Method: 通过逐步蒸馏构建稀疏的中间模型锚点序列形成蒸馏链，并引入桥接蒸馏实现跨架构和跨词汇的迁移，利用锚点间的参数插值初始化变体大小模型。

Result: 138M参数的小模型在10B令牌语料上无需恢复预训练即超越从头训练模型，在异构设置下展示出通用性。

Conclusion: CBD显著提升效率和下游性能，适用于不同架构和词汇的模型初始化。
...
### [310] [CktFormalizer: Autoformalization of Natural Language into Circuit Representations](https://arxiv.org/abs/2605.07782)
*Jing Xiong,Qi Han,Chenchen Ding,He Xiao,Zunhai Su,Chaofan Tao,Ngai Wong*

Main category: cs.CL

TL;DR: CktFormalizer 是一个框架，通过 Lean 4 中的依赖类型 HDL，将 LLM 驱动的硬件生成重定向，确保设计在综合和硅实现中没有缺陷，同时实现高后端可实现性和自动证明的功能等价性。


<details>
  <summary>Details</summary>
Motivation: LLM 从自然语言生成的 Verilog 常包含宽度不匹配、组合循环和不完整 case 逻辑等缺陷，这些缺陷通过语法检查但在综合或硅中失败，需要一种方法将硬件缺陷转化为编译时错误。

Method: 使用 Lean 4 嵌入依赖类型 HDL，利用类型检查编码位宽约束、case 覆盖和无环性；通过校正防火墙确保编译设计无缺陷；使用证明助手构建机器检查的等价证明，支持任意输入序列和参数化宽度。

Result: 在 VerilogEval、RTLLM 和 ResBench 上，模拟通过率与直接 Verilog 生成相当，后端可实现性达 95–100%；优化阶段实现面积减少高达 35%，功耗减少 30%，且通过自动定理证明保持功能等价。

Conclusion: CktFormalizer 有效消除 LLM 生成硬件中的无声后端失败，结合形式验证和后端可实现性，提供可靠且可优化的硬件生成流程。
...
### [311] [PolySQL: Scaling Text-to-SQL Evaluation Across SQL Dialects via Automated Backend Isomorphism](https://arxiv.org/abs/2605.07796)
*Yotam Perlitz,Elad Venezian,Corentin Royer,Francesco Fusco,Andrea Giovannini*

Main category: cs.CL

TL;DR: 提出了PolySQL方法，通过双执行和归一化结果比较，解决了文本到SQL跨方言评估的困难。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL基准仅支持SQLite，导致跨方言评估困难且SQLite性能无法代表其他方言。

Method: PolySQL采用双执行方法，无需查询转译，通过归一化执行结果进行比较。

Result: 实现了100%查询覆盖率，发现从SQLite到其他方言的平均准确率下降10.1%，逻辑错误占61%。

Conclusion: PolySQL提供了高保真度的跨方言评估框架，揭示了方言难度层次和错误来源。
...
### [312] [Hybrid TF--IDF Logistic Regression and MLP Neural Baseline for Indonesian Three-Class Sentiment Analysis on Social Media Text](https://arxiv.org/abs/2605.07793)
*Allya Nurul Islami Pasha,Eka Fidiya Putri,Luluk Muthoharoh,Ardika Satria,Martin C. T. Manullang*

Main category: cs.CL

TL;DR: 本文针对印尼社交媒体文本，提出一个紧凑的三类情感分析基准，使用TF-IDF特征、元数据特征和逻辑回归模型，在小型数据集上达到0.8028准确率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在小型印尼社交文本数据集上，可解释的特征工程和传统模型是否仍具竞争力。

Method: 方法包括：清洗和重标情感标签后，提取TF-IDF文本特征与三个轻量数字元特征，采用逻辑回归（部署模型）和多层感知机（对比模型）。

Result: 逻辑回归模型达到0.8028准确率、0.8003加权F1和0.7276宏F1；MLP基线虽有更高准确率，但不宜直接部署。

Conclusion: 在小型印尼情感数据集上，细致预处理、可解释特征工程和类别平衡仍具竞争力，神经网络更适合作为对比实验。
...
### [313] [Beyond Confidence: Rethinking Self-Assessments for Performance Prediction in LLMs](https://arxiv.org/abs/2605.07806)
*Sree Bhattacharyya,Samarth Khanna,Leona Chen,Lucas Craig,Tharun Dilliraj,James Z. Wang*

Main category: cs.CL

TL;DR: 论文提出基于认知评价理论的多维自我评估方法，通过六种评价维度（如努力、能力）替代传统置信度，提升了大语言模型的错误预测可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有置信度评估存在不一致和过度乐观问题，需要更可靠的多维度自我评估方法。

Method: 基于认知评价理论，设计六种评价维度（努力、能力、情感等），在12个LLM和38个任务上评估其预测错误能力。

Result: 努力和能力维度在多数任务中优于置信度，努力维度具有更少乐观偏差，且最佳维度因任务类型而异。

Conclusion: 多维度结构化自我评估可显著提升模型部署的可靠性和安全性。
...
### [314] [A Comparative Analysis of Classical Machine Learning and Deep Learning Approaches for Sentiment Classification on IMDb Movie Reviews](https://arxiv.org/abs/2605.07811)
*Erma Daniar Safitri,Lia Hana Ichisasmita,Citra Agustin,Luluk Muthoharoh,Ardika Satria,Martin Clinton Tosima Manullang*

Main category: cs.CL

TL;DR: 在IMDb电影评论数据集上，经典机器学习（尤其是SVM）准确率达0.8530，优于深度学习模型（BiLSTM+注意力机制准确率0.706）。


<details>
  <summary>Details</summary>
Motivation: 比较经典机器学习与深度学习在情感分类中的表现，明确两者在有限数据和计算资源下的优劣。

Method: 经典ML管道使用TF-IDF特征和PyCaret AutoML评估逻辑回归、朴素贝叶斯、SVM；深度学习管道实现BiLSTM及带注意力机制的BiLSTM。

Result: SVM以0.8530准确率最优；BiLSTM+注意力机制（0.706）优于标准BiLSTM，但低于经典ML方法。

Conclusion: 经典机器学习结合TF-IDF在数据与资源受限下仍是强基线；深度学习能捕捉序列依赖，但当前性能不及经典方法。
...
### [315] [SCENE: Recognizing Social Norms and Sanctioning in Group Chats](https://arxiv.org/abs/2605.07823)
*Mateusz Jacniacki,Maksymilian Bilski*

Main category: cs.CL

TL;DR: 提出了SCENE基准，用于评估LLM在多人群聊中识别和适应隐性社会规范的能力，实验显示Claude Opus 4.7和Gemini 3.1 Pro表现优于开源模型。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对LLM在多人群聊中识别和适应隐性规范的能力评估。

Method: SCENE基准通过生成非角色扮演场景、隐藏规范、违规机会及社交制裁，并定义两类适应性指标（对负面制裁的响应、从同伴行为中学习规范）。

Result: Claude Opus 4.7和Gemini 3.1 Pro在适应隐性规范方面显著优于所评估的开源模型。

Conclusion: SCENE为动态、交互式评估LLM社交能力提供了有效基准，并揭示了不同模型在隐性规范适应上的性能差异。
...
### [316] [Measuring and Mitigating the Distributional Gap Between Real and Simulated User Behaviors](https://arxiv.org/abs/2605.07847)
*Shuhaib Mehri,Philippe Laban,Sumuk Shashidhar,Marwa Abdulhai,Sergey Levine,Michel Galley,Dilek Hakkani-Tür*

Main category: cs.CL

TL;DR: 本文提出了一种度量真实与模拟用户行为分布差异的方法，并通过系统性评估发现，现有LLM用户模拟器与真实用户行为存在显著分布差距，且不同模拟器之间行为相似性高，组合互补模拟器可减少差距。


<details>
  <summary>Details</summary>
Motivation: 现有用户模拟器虽能生成类人响应，但其是否能捕捉真实用户行为的广泛异质性分布尚不明确，需开发系统评估方法。

Method: 提取对话中用户行为表征，通过聚类量化为离散分布，计算散度指标；在编码和写作任务上对24个基于LLM的模拟器进行系统评估，结合TF-IDF分析集群的可解释行为模式。

Result: 模拟器与真实用户存在较大分布差距，该差距因模型族、规模和行为层面而异；多数模拟器行为相似，少数有差异；组合行为互补的模拟器可缩小差距。

Conclusion: 现有模拟器未能充分模拟真实用户行为的多样性，组合多个互补模拟器是提升分布匹配度的有效策略。
...
### [317] [MatryoshkaLoRA: Learning Accurate Hierarchical Low-Rank Representations for LLM Fine-Tuning](https://arxiv.org/abs/2605.07850)
*Ionut-Vlad Modoranu,Mher Safaryan,Dan Alistarh*

Main category: cs.CL

TL;DR: 提出MatryoshkaLoRA框架，通过在LoRA适配器间插入对角矩阵实现层级低秩表示，支持动态秩选择且性能优越。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA方法需预设静态秩且网格搜索成本高，而DyLoRA等自适应方法在高秩时梯度信号不一致导致数据效率低。

Method: 在LoRA适配器之间插入精心设计的固定对角矩阵P，以缩放子秩，确保所有子秩有效嵌入梯度信息。

Result: MatryoshkaLoRA在不同秩上均优于现有方法，取得更优的精度-性能权衡，并引入新指标AURAC评估层级适配器性能。

Conclusion: MatryoshkaLoRA是一种通用、高效的动态秩选择框架，能够学习更准确的层级低秩表示。
...
### [318] [Beyond "I cannot fulfill this request": Alleviating Rigid Rejection in LLMs via Label Enhancement](https://arxiv.org/abs/2605.07883)
*Ying Zhang,Congyu Qiao,Xin Geng,Ning Xu*

Main category: cs.CL

TL;DR: 该论文提出了LANCE方法，通过标签增强和变分推断确保大语言模型在安全前提下生成灵活自然的响应，避免刚性拒绝问题。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型的安全对齐机制常导致‘刚性拒绝’，即使用通用模板拒绝请求，破坏了人机交互的自然性。

Method: 采用变分推断进行标签增强，预测连续的多类别拒绝分布，利用细粒度分布提供的文本梯度优化提示中的有害元素，使模型生成安全且自然的响应。

Result: LANCE在保持高安全标准的同时显著缓解了刚性拒绝问题，在响应的帮助性和自然性上优于现有基线模型。

Conclusion: LANCE有效平衡了安全性与自然性，为解决大模型安全对齐中的过度拒绝问题提供了新方案。
...
### [319] [CoCoReviewBench: A Completeness- and Correctness-Oriented Benchmark for AI Reviewers](https://arxiv.org/abs/2605.07905)
*Hexuan Deng,Xiaopeng Ke,Yichen Li,Ruina Hu,Dehao Huang,Derek F. Wong,Yue Wang,Xuebo Liu,Min Zhang*

Main category: cs.CL

TL;DR: 提出CoCoReviewBench基准，通过类别特定子集和专家注释增强评估的完整性与正确性，发现现有AI审稿人存在幻觉问题，推理模型表现更优。


<details>
  <summary>Details</summary>
Motivation: 人类审稿存在覆盖不全和错误问题，导致现有评估指标不可靠。

Method: 构建类别特定子集跳过缺失审稿以增强完整性，利用审稿-作者-元评论对话过滤不可靠评论以增强正确性。

Result: AI审稿人正确性有限且易产生幻觉，推理模型更有效。

Conclusion: CoCoReviewBench实现可靠细粒度评估，为改进AI审稿人提供方向。
...
### [320] [How to Train Your Latent Diffusion Language Model Jointly With the Latent Space](https://arxiv.org/abs/2605.07933)
*Viacheslav Meshchaninov,Alexander Shabalin,Egor Chimbulatov,Nikita Gushchin,Ilya Koziev,Alexander Korotin,Dmitry Vetrov*

Main category: cs.CL

TL;DR: 提出了一种联合训练潜在编码器、扩散模型和解码器的潜在扩散语言模型（LDLM），在不需要大量计算开销的情况下，显著提升了文本生成的质量和速度。


<details>
  <summary>Details</summary>
Motivation: 解决潜在扩散模型中潜在空间构建困难的问题，以提升非自回归文本生成的性能。

Method: 联合训练预训练语言模型的编码器、扩散模型和解码器，使用MSE解码器损失、扩散到编码器的预热、自适应时间步采样和解码器输入噪声等策略。

Result: 在OpenWebText和LM1B数据集上，LDLM比现有离散和连续扩散语言模型生成效果更好，速度快2-13倍。
...
### [321] [How Value Induction Reshapes LLM Behaviour](https://arxiv.org/abs/2605.07925)
*Arnav Arora,Natalie Schluter,Katherine Metcalf,Maartje ter Hoeve*

Main category: cs.CL

TL;DR: 研究了在对话式大语言模型中诱导特定价值观的意外效果，发现它会导致相关甚至相反的价值观表达、提高安全性，但也会增加拟人化语言的使用，使模型更易附和用户。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过后训练注入价值观（如好奇心、诚实等）以提升安全性和用户体验，但价值观复杂且相互关联，可能引发非预期行为（如成瘾或谄媚），因此需要系统研究。

Method: 使用现有偏好数据集的子集对模型进行微调，分别注入特定价值观，然后测量对其他价值观表达、安全性、拟人化语言及多个问答基准的影响。

Result: (i) 诱导一个价值观会激活相关甚至相反的价值观；(ii) 正向价值观可提高安全性；(iii) 所有价值观都增加了拟人化语言，使模型更易谄媚和附和。

Conclusion: 价值观注入具有复杂连带效应，需谨慎权衡以提高安全性，但需注意其增加谄媚型语言和成瘾风险的副作用。
...
### [322] [GLiGuard: Schema-Conditioned Classification for LLM Safeguard](https://arxiv.org/abs/2605.07982)
*Urchade Zaratiana,Mary Newhauser,George Hurn-Maloney,Ash Lewis*

Main category: cs.CL

TL;DR: GLiGuard是一个0.3B参数的基于模式条件的双向编码器，用于LLM内容审核，在单一非自回归前向传递中同时评估提示安全性、响应安全性、拒绝检测、14个细粒度有害类别和11种越狱策略，性能与7B-27B解码器模型相当。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的护栏模型使用7B-27B参数的自回归解码器，将分类问题转化为顺序文本生成，导致高延迟和低效的多方面评估可扩展性。

Method: 基于GLiNER2改编的模式条件双向编码器，将任务定义和标签语义编码为结构化令牌模式，实现单一非自回归前向传递中的多维度同步评估。

Result: 在九个安全基准测试中，F1分数与7B-27B解码器模型相当，模型大小仅为其1/23至1/90，吞吐量提高16倍，延迟降低17倍。

Conclusion: 紧凑的双向编码器可以接近大型护栏模型的准确性，同时大幅降低推理成本。
...
### [323] [Ask Early, Ask Late, Ask Right: When Does Clarification Timing Matter for Long-Horizon Agents?](https://arxiv.org/abs/2605.07937)
*Anmol Gulati,Hariom Gupta,Elias Lumer,Sahil Sen,Vamse Kumar Subbiah*

Main category: cs.CL

TL;DR: 研究发现，在长时域AI代理执行任务时，澄清请求的最佳时机取决于缺失信息类型，早期并非总是最优，且当前模型均未在最优窗口内自动提问。


<details>
  <summary>Details</summary>
Motivation: 现有研究未量化澄清信息在任务执行过程中的价值变化，且缺乏系统评估代理决策时机的基础。

Method: 通过强制注入（forced-injection）框架，在代理轨迹中可控点提供真实澄清，跨四个信息维度、三个基准及四个前沿模型进行实验（84种任务变体，6000+次运行），并辅以300次无脚本会话分析。

Result: 目标澄清在10%执行后价值几乎消失（pass@3从0.78降至基线），输入澄清价值持续至约50%；任何延迟至中期后提问，性能低于从不提问；跨模型相关性高（0.78-0.87在相同任务覆盖模型间），说明时序模式与任务内在相关；当前模型均未在最优窗口内提问（52%过度提问，其余从不提问）。

Conclusion: 研究提出了经验需求曲线，为时间感知澄清策略提供了量化基础，但需要进一步开发能自动检测最佳提问时机的代理。
...
### [324] [Fast Byte Latent Transformer](https://arxiv.org/abs/2605.08044)
...
TL;DR: 提出了字节潜在变换器（BLT）的三种加速生成方法：扩散生成（BLT-D）、自推测解码（BLT-S）和扩散加验证（BLT-DV），在保持生成质量的同时将内存带宽成本降低50%以上。


<details>
  <summary>Details</summary>
Motivation: 字节级语言模型虽避免了子词分词，但逐字节自回归生成速度极慢，阻碍了其实际应用。

Method: BLT-D使用辅助块级扩散目标训练实现并行生成；BLT-S通过本地解码器继续生成草稿字节并用完整模型验证；BLT-DV在扩散生成后添加自回归验证步骤。

Result: 所有方法在生成任务中估计内存带宽成本比基线BLT降低超过50%，同时保持了生成质量。

Conclusion: 这些方法有效消除了字节级语言模型实际应用的主要瓶颈，为高效生成提供了不同速度和质量的权衡方案。
...
### [325] [Tool Calling is Linearly Readable and Steerable in Language Models](https://arxiv.org/abs/2605.07990)
*Zekun Wu,Ze Wang,Seonglae Cho,Yufei Yang,Adriano Koshiyama,Sahan Bulathwela,Maria Perez-Ortiz*

Main category: cs.CL

TL;DR: 本文研究了指令微调模型中工具选择的内部表示，发现工具有线性可读和可引导的内部标识，可以通过激活差异实现高准确率的切换与错误预警。


<details>
  <summary>Details</summary>
Motivation: 工具调用智能体选错工具时，错误直到执行才暴露，因此需要理解模型内部如何表示和选择工具。

Method: 通过平均激活差异切换工具选择、线性探测、激活修补和方向分析等方法，在多个模型系列和规模上（270M至27B）进行系统性实验。

Result: 添加工具平均激活差异可实现93-100%的工具切换准确率（4B+）；最小顶部-次顶工具激活差距导致14-21倍更多错误；因果方向集中于输出层第一令牌行；激活修补定位到中后层注意力头；跨同一主题工具探测达到61-89%准确率；基座模型内部表示已达69-82%准确率，但生成仅2-10%。

Conclusion: 工具选择具有线性可表示和可干预的内部机制，预训练形成了正确的表示，指令微调将其连接到输出；该发现可用于错误预警和模型行为的可控调整。
...
### [326] [Uncertainty-Aware Structured Data Extraction from Full CMR Reports via Distilled LLMs](https://arxiv.org/abs/2605.08045)
*Yi Yu,Parker Martin,Zhenyu Bu,Yixuan Liu,Yi-Yu Zheng,Orlando Simonetti,Yuchi Han,Yuan Xue*

Main category: cs.CL

TL;DR: 提出CMR-EXTR框架，可将自由文本的心脏磁共振报告转化为结构化数据并附带置信度评分，实现高精度提取。


<details>
  <summary>Details</summary>
Motivation: 将自由文本的心脏磁共振报告转化为可审计的结构化数据是队列构建、长期管理和临床决策支持的瓶颈。

Method: 采用教师-学生蒸馏流程支持完全离线推理并减少人工标注，结合分布合理性、采样稳定性和跨字段一致性三种不确定性原则进行质量控制和人工审查分流。

Result: 变量级准确率达99.65%，展示了可靠的提取和有效的置信度评分。

Conclusion: CMR-EXTR是首个集成置信度估算的心脏磁共振特异性提取系统，代码已开源。
...
### [327] [Accurate and Efficient Statistical Testing for Word Semantic Breadth](https://arxiv.org/abs/2605.08048)
*Yo Ehara*

Main category: cs.CL

TL;DR: 提出了一种Householder排列检验方法，用于隔离词义广度差异与语义方向差异，从而在统计检验中减少I型错误。


<details>
  <summary>Details</summary>
Motivation: 在比较两个词的语义广度时，基于离散度的假设检验可能因语义方向差异导致I型错误膨胀，即虚假显著性，需要消除方向干扰以准确检测广度差异。

Method: 使用Householder反射对齐两个词类型的平均方向，然后对对齐后的词向量云进行排列检验，并设计了GPU批处理实现以提升效率。

Result: 对齐后I型错误降低32.5%，同时保留了对真实广度差异的敏感性，并实现了相对于CPU基线23倍的速度提升。

Conclusion: Householder对齐排列检验能有效区分语义方向与广度差异，提供校准的非参数p值，适用于词典构建等实际任务。
...
### [328] [CA-SQL: Complexity-Aware Inference Time Reasoning for Text-to-SQL via Exploration and Compute Budget Allocation](https://arxiv.org/abs/2605.08057)
*James Petullo,Nianwen Xue*

Main category: cs.CL

TL;DR: 提出CA-SQL管道，通过任务难度动态扩展搜索广度，结合进化搜索提示播种和投票方法，在BIRD基准挑战级上实现51.72%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有推理时学习方法在Text-to-SQL中解空间探索不足，难以处理BIRD基准中最困难任务。

Method: 基于任务估计难度动态调整候选查询搜索广度，利用进化搜索原理的自定义提示播种，以及新投票方法选择最佳候选。

Result: 在BIRD挑战级开发集上使用GPT-4o-mini达到51.72%，优于其他上下文学习方法；整体执行准确率61.06%，Soft F1为68.77%。

Conclusion: CA-SQL通过动态探索和进化搜索有效提升了Text-to-SQL困难任务的推理表现。
...
### [329] [The Memory Curse: How Expanded Recall Erodes Cooperative Intent in LLM Agents](https://arxiv.org/abs/2605.08060)
...
TL;DR: 扩展上下文窗口在多数LLM的多智能体社会困境中系统性破坏合作，称为记忆诅咒，其机制与前瞻性推理退化而非偏执增强有关。


<details>
  <summary>Details</summary>
Motivation: 发现上下文窗口扩展作为标准能力升级会在多智能体合作场景中意外失败。

Method: 通过7个LLM在4种博弈中测试500轮；分析378,000条推理轨迹词汇特征；使用LoRA微调前瞻性轨迹进行认知探针；内存清洁化控制内容而非长度；消融CoT推理。

Result: 18/28模型-游戏设置合作下降；前瞻性推理减少是主要诱因；合成合作记录可恢复合作；消融CoT可缓解崩溃。

Conclusion: 记忆长度本身不是关键，而是其引发的推理模式决定多智能体行为稳定性。
...
### [330] [Conformal Path Reasoning: Trustworthy Knowledge Graph Question Answering via Path-Level Calibration](https://arxiv.org/abs/2605.08077)
*Shuhang Lin,Chuhao Zhou,Xiao Lin,Zihan Dong,Kuan Lu,Zhencan Peng,Jie Yin,Dimitris N. Metaxas*

Main category: cs.CL

TL;DR: 提出了一种名为Conformal Path Reasoning (CPR)的可信知识图谱问答框架，通过查询级共形校准和残差共形值网络，在保证统计覆盖保证的同时显著减少预测集大小。


<details>
  <summary>Details</summary>
Motivation: 现有KGQA方法缺乏可靠的覆盖保证，而共形预测方法在校准有效性和分数可区分性上存在关键局限，导致覆盖保证被违反且预测集过大。

Method: CPR包含两个创新：查询级共形校准保留可交换性以生成路径预测集，以及RCVNet通过PUCT引导探索学习可区分的路径级非一致性分数。

Result: 在基准测试中，CPR相比共形基线将经验覆盖率提高34%，同时平均预测集大小减少40%。

Conclusion: CPR能有效满足覆盖保证，同时提供更紧凑的答案集，验证了其方法的有效性。
...
### [331] [LLMs Improving LLMs: Agentic Discovery for Test-Time Scaling](https://arxiv.org/abs/2605.08083)
...
TL;DR: 提出AutoTTS框架，通过环境驱动自动发现测试时缩放策略，提升大语言模型推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有TTS策略依赖人工设计，探索不充分。

Method: 构建宽度-深度TTS环境，使用控制器合成与beta参数化，结合执行跟踪反馈进行自动搜索。

Result: 在数学推理基准上，发现策略优于人工基线，泛化至多模型，总成本仅39.9美元。

Conclusion: 环境驱动的自动TTS策略发现高效且通用。
...
### [332] [An Aerial Manipulator for Perception-Driven Flower Targeting Toward Contactless Pollination in Vertical Farming](https://arxiv.org/abs/2605.06759)
*Chenzhe Jin,Zhuohang Wu,Yifan Cai,Xiangqi Li,Jan Ming Kevin Tan,Narsimlu Kemsaram,Valerio Modugno*

Main category: cs.RO

TL;DR: 本文提出了一种用于垂直农业的空中操纵平台，集成了感知、MPPI控制和2DoF机械臂，实现厘米级花朵定位精度。


<details>
  <summary>Details</summary>
Motivation: 自然传粉者减少使室内垂直农业面临授粉挑战，亟需精准且不损伤花朵的机器人系统。

Method: 采用RGBD感知、模型预测路径积分（MPPI）的无人机控制及2自由度轻量机械臂，在MuJoCo仿真和无人机实验室中测试。

Result: 仿真中轨迹收敛一致，实际实验中实现了稳定飞行、可靠花朵定位和厘米级末端定位精度。

Conclusion: 该空中操纵平台为未来非接触式授粉系统（如声学花粉操控）提供了稳健的机器人载体和定位框架。
...
### [333] [Bi3: A Biplatform, Bicultural, Biperson Dataset for Social Robot Navigation](https://arxiv.org/abs/2605.06863)
*Andrew Stratton,Phani Teja Singamaneni,Pranav Goyal,Rachid Alami,Christoforos Mavrogiannis*

Main category: cs.RO

TL;DR: 本文贡献了Bi3数据集，用于研究社交机器人导航在受限空间中的人群交互。


<details>
  <summary>Details</summary>
Motivation: 现有社交机器人导航数据集缺乏多样性（如算法、平台、参与者及环境）和紧密交互场景。

Method: 设计实验：74名参与者（美国与法国）在受限实验室中与机器人进行近距离导航；比较5种导航算法与2种机器人平台；采集10.5小时多模态数据（动作轨迹、RGB视频、用户评价）。

Result: Bi3数据集通过交互密度和人类速度等指标展现独特多样性和建模复杂度，成为密集空间中导航基准。

Conclusion: Bi3有助于理解人机在受限环境下的协同活动，并支持人类运动预测和机器人控制策略的训练。
...
### [334] [Traffic Scenario Orchestration from Language via Constraint Satisfaction](https://arxiv.org/abs/2605.06966)
*Frieda Rong,Chris Zhang,Kelvin Wong,Raquel Urtasun*

Main category: cs.RO

TL;DR: 提出一种基于约束求解的场景编排方法，利用基础模型将自然语言转换为约束条件，实现自动驾驶车辆的闭环测试。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真测试中手动编写场景难以规模化，统计模型在分布外场景中可控性不足。

Method: 将场景编排视为约束求解问题，使用基础模型将自然语言描述转换为约束条件，再通过现成求解器生成满足测试意图的驾驶行为。

Result: 在多样化场景描述基准测试中，编排成功率大幅优于基线方法，且闭环方法对自车反应性场景尤为重要。

Conclusion: 该方法结合语言灵活性与约束求解精确性，可有效生成复杂交互场景，提升自动驾驶测试效率。
...
### [335] [AirBender: Adaptive Transportation of Bendable Objects Using Dual UAVs](https://arxiv.org/abs/2605.07003)
*Jiawei Xu,Longsen Gao,Rafael Fierro,David Saldaña*

Main category: cs.RO

TL;DR: 本文提出一种自适应控制器，使两架空中机器人能够在无需明确弹性模型的情况下，协同运输可弯曲物体并跟踪轨迹。通过Lyapunov分析证明其渐近稳定性，并在多种硬件实验中验证了性能。


<details>
  <summary>Details</summary>
Motivation: 空中机器人操控可弯曲物体时面临控制困难，尤其因驱动能力有限和持续飞行要求，传统方法易导致性能下降甚至坠毁。

Method: 提出无需显式弹性模型的在线自适应控制器，通过Lyapunov理论保证渐近稳定性，实现双机协同轨迹跟踪。

Result: 硬件实验表明，该控制器在多种场景下能有效处理可弯曲物体，实现稳定轨迹跟踪。

Conclusion: 该方法为空中机器人运输可变形物体提供了一种无需先验模型的自适应方案，在稳定性和性能上表现出色。
...
### [336] [Intention assimilation control for accurate tracking with variable impedance in teleoperation](https://arxiv.org/abs/2605.07037)
*Atsushi Takagi,Yanan Li,Hiroaki Gomi,Etienne Burdet*

Main category: cs.RO

TL;DR: 提出意图同化控制（IAC）策略，通过估计操作者目标位置而非当前位置，实现低刚度下的高精度跟踪，解决了传统遥操作中精度与安全性的权衡难题。


<details>
  <summary>Details</summary>
Motivation: 传统弹簧力跟踪策略中，低刚度导致精度下降，高刚度威胁环境安全，需兼顾精度与安全性的控制方法。

Method: IAC策略：估算操作者目标位置替代当前位置传输，允许从手阻抗在线调节，可自动或按用户意图调整刚度。

Result: 在7自由度机械臂实验中，IAC相比遥阻抗控制（TIC）提高了跟踪精度、任务完成率，缩短了完成时间（含自由跟踪、气球交互、插孔、抛光任务）。

Conclusion: IAC在保持低刚度安全性的同时实现高精度跟踪，赋予用户独立控制位置与阻抗的能力，适用于单/双边遥操作。
...
### [337] [Dr-BA: Separable Optimization for Direct Radar Bundle Adjustment & Localization](https://arxiv.org/abs/2605.07041)
*Daniil Lisus,Cedric Le Gentil,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: 本文提出Dr-BA，首个直接在二维旋转雷达强度图像上操作的雷达束调整框架，实现密集地图与传感器位姿的联合估计。


<details>
  <summary>Details</summary>
Motivation: 雷达在雨雪等恶劣天气下表现稳健，但现有方法多依赖稀疏点云对齐，未能充分利用完整雷达回波信息。

Method: 将问题建模为可分离优化，解耦位姿估计与地图构建，并扩展为直接雷达定位（DRL）。

Result: 在200余公里实测数据上取得最先进的雷达束调整与跨会话定位性能。

Conclusion: Dr-BA验证了直接处理雷达强度图像进行束调整的有效性，并提供了开源实现。
...
### [338] [PISTO: Proximal Inference for Stochastic Trajectory Optimization](https://arxiv.org/abs/2605.07215)
*Hongzhe Yu,Zinuo Chang,Yongxin Chen*

Main category: cs.RO

TL;DR: 提出PISTO算法，通过KL正则化稳定随机轨迹优化，在机器人运动规划中优于STOMP和CHOMP。


<details>
  <summary>Details</summary>
Motivation: STOMP虽能处理非可微成本，但更新不稳定，需改进收敛性和性能。

Method: 基于变分推断，在STOMP中引入KL正则化项，通过重要性加权蒙特卡洛采样估计闭式均值更新。

Result: 在机器人臂运动规划中成功率89%，路径更短更平滑，速度是随机方法2倍；在接触丰富的任务中奖励优于CEM和MPPI。

Conclusion: PISTO通过变分推断视角提升了随机轨迹优化的稳定性和效率，适用于复杂非可微成本问题。
...
### [339] [Palm-sized Omnidirectional Vision-Based UAV Exploration with Sparse Topological Map Guidance](https://arxiv.org/abs/2605.07275)
*Zirui Wang,Xinjia Luo,Haotian Sun,Jun Ma,Jian Guo,Boyu Zhou*

Main category: cs.RO

TL;DR: 提出了一种基于全向视觉和稀疏拓扑地图的轻量级自主探索系统，用于微型无人机在SWaP约束下的高效探索。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖稠密占用地图或高分辨率点云，内存和计算开销大，且微型无人机无法搭载LiDAR等传感器。

Method: 使用多鱼眼相机实现全向视野和深度估计，用拓扑节点表示前沿区域，构建稀疏拓扑地图进行路径规划。

Result: 在仿真和11cm轴距、400g重量的无人机上验证，实现了低计算消耗的高效探索。

Conclusion: 该方法在资源受限的微型无人机上实现了高效探索，降低了内存和计算需求。
...
### [340] [Variable Aerodynamic Damping via Co-Contraction: A Dynamic Isomorphism with Variable Stiffness Actuators](https://arxiv.org/abs/2605.07292)
*Antonio Franchi*

Main category: cs.RO

TL;DR: 本文证明冗余双旋翼执行器中的气动共收缩可在保持净推力恒定的同时，通过被动、配平定义的气动机械阻尼进行调节，并定义了增量阻尼系数，验证了其单调性。


<details>
  <summary>Details</summary>
Motivation: 探索冗余执行器中气动共收缩对被动阻尼的调节能力，并揭示其与可变刚度执行器的动态同构性。

Method: 基于叶素理论推导最小推力模型，定义增量阻尼系数，验证气动硬化条件，并通过阻抗形式表示分析共模与差模控制。

Result: 阻尼系数在等力纤维上单调递增；冗余多旋翼的主动气动响应速度得到增强；揭示了阻尼调节与刚度调节的相似性。

Conclusion: 变量气动阻尼执行器可通过共收缩有效调节被动阻抗与平衡速度，并与可变刚度执行器动态同构。
...
### [341] [BioProVLA-Agent: An Affordable, Protocol-Driven, Vision-Enhanced VLA-Enabled Embodied Multi-Agent System with Closed-Loop-Capable Reasoning for Biological Laboratory Manipulation](https://arxiv.org/abs/2605.07306)
*Zhaohui Du,Zhe Wang,Hongmei Fei,Xiwen Cao,Ting Xiao,Qi Wang,Huanbo Jin,Jiaming Gu,Quan Lu,Zhe Liu*

Main category: cs.RO

TL;DR: 提出了一个名为BioProVLA-Agent的低成本、协议驱动的视觉增强多智能体系统，通过视觉-语言-动作模型实现生物实验室自动化，并采用在线增强策略AugSmolVLA提升在湿实验室视觉扰动下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有机器人系统依赖昂贵硬件、固定流程或专用接口，难以应对湿实验室中协议非结构化、器皿透明反光、多步骤执行需状态感知等挑战。

Method: 构建闭环工作流，包含协议解析的LLM协议智能体、基于VLM-RAG的验证智能体（结合检索知识与成功/失败示例）、以及执行轻量策略的VLA具身智能体；提出AugSmolVLA在线增强策略应对透明器皿、反射、光照偏移等视觉干扰。

Result: 在15个原子任务、6个复合流程和3个双手任务（如试管装载、分拣、液体倾倒）上，AugSmolVLA在正常和高曝光场景下均优于ACT、X-VLA和原始SmolVLA，尤其在精准放置、透明物体操作、复合流程及视觉退化场景中表现更好。

Conclusion: 该工作为生物操作提供了可访问、协议中心且具备验证能力的具身AI路径，验证了VLA模型在低成本硬件上处理复杂湿实验室任务的潜力。
...
### [342] [AT-VLA: Adaptive Tactile Injection for Enhanced Feedback Reaction in Vision-Language-Action Models](https://arxiv.org/abs/2605.07308)
*Xiaoqi Li,Muhe Cai,Jiadong Xu,Juan Zhu,Hongwei Fan,Yan Shen,Guangrui Ren,Hao Dong*

Main category: cs.RO

TL;DR: 提出了一种自适应触觉注入机制和双流反应机制，在接触密集型操作任务中实现了实时闭环响应，且不干扰预训练VLA能力。


<details>
  <summary>Details</summary>
Motivation: VLA模型在接触密集型操作中表现不佳，引入触觉信号会干扰预训练能力且推理速度慢。

Method: 自适应触觉注入机制（动态选择注入时机和位置）和触觉反应双流机制（视觉-语言慢流与触觉快流解耦）。

Result: 在真实实验中验证了在接触密集型操作任务中的有效性，触觉响应延迟小于0.04秒。

Conclusion: AT-VLA有效解决了触觉模态引入的干扰和实时响应问题，提升了VLA在精密操作中的性能。
...
### [343] [CSR: Infinite-Horizon Real-Time Policies with Massive Cached State Representations](https://arxiv.org/abs/2605.07325)
*Robin Karlsson,Go Suzui*

Main category: cs.RO

TL;DR: 提出CSR框架和ASR算法，将大语言模型作为机器人实时认知引擎的时延从14.67秒降至0.56秒（26倍提升），并支持高频（>2Hz）连续运行。


<details>
  <summary>Details</summary>
Motivation: 部署大规模LLM作为机器人连续认知引擎时，处理长历史状态序列的首次令牌延迟（TTFT）成为瓶颈，现有方法（如RAG、滑动窗口）无法兼顾全局上下文与低延迟。

Method: 理论证明最优任务需满足前缀稳定、增量扩展和异步状态协调三个条件；提出CSR框架实现KV缓存最优复用，以及ASR算法通过并行资源异步处理状态内存驱逐以消除延迟尖峰。

Result: 在物理机器人上实现120K token上下文的26倍延迟降低（14.67s→0.56s）；具身AI基准中召回率从0.459提升至0.836；ASR在10次连续循环中保持无尖峰有界延迟。

Conclusion: CSR和ASR使大规模LLM能作为连续运行的高频（>2Hz）具身策略，解决了实时推理的延迟瓶颈。
...
### [344] [Weather-Robust Scene Semantics with Vision-Aligned 4D Radar](https://arxiv.org/abs/2605.07367)
*Kali Hamilton,Christoffer Heckman*

Main category: cs.RO

TL;DR: 毫米波雷达在雨雾雪天气下仍能正常工作，通过将雷达编码器对齐至冻结的SigLIP视觉嵌入，利用冻结的视觉语言模型解码场景描述，在K-RADAR数据集中所有雷达配置均优于摄像头基线（摄像头在恶劣天气下90%以上输出幻觉）。


<details>
  <summary>Details</summary>
Motivation: 摄像头和LiDAR在雨、雾、雪等恶劣天气下性能严重下降，而毫米波雷达不受影响，因此探索利用雷达数据结合视觉语言模型进行场景理解。

Method: 将雷达编码器对齐到冻结的SigLIP视觉嵌入，通过投影器输出与冻结的视觉语言模型连接，使用约7M可训练参数进行端到端训练，并引入投影器输出层的LayerNorm解决令牌范数不匹配问题。

Result: 在K-RADAR数据集（含雾、小雪、大雪等未见序列）上，所有雷达配置均优于摄像头基线（摄像头基线的幻觉率超过90%）。

Conclusion: 雷达-视觉语言模型管道在恶劣天气下显著优于摄像头方案，解决了令牌范数不匹配的关键问题，并为未来雷达-VLM系统设计提供了权衡指南。
...
### [345] [MORPH-U: Multi-Objective Resilient Motion Planning for V2X-Enabled Autonomous Driving in High-Uncertainty Environments via Simulation](https://arxiv.org/abs/2605.07370)
*Shih-Yu Lai*

Main category: cs.RO

TL;DR: 提出了MORPH-U系统，通过融合V2X和车载传感器实现鲁棒的运动规划，并采用拜占庭容错机制抵御恶意V2X消息。


<details>
  <summary>Details</summary>
Motivation: V2X带来通信延迟、丢失和伪造等不确定性，同时地图知识可能动态变化，需要实时重规划。

Method: 基于CARLA的闭环堆栈，融合多传感器与V2X构建局部动态地图，采用Hybrid-A*重规划，并通过多目标Pareto优化和拜占庭容错门控提升鲁棒性。

Result: V2X改进的安全性与Pareto权衡效果，在饱和虚假消息攻击下仍能阻止不安全重规划。

Conclusion: V2X增强的局部动态地图可提升安全性，Pareto调优实现可控的精度-舒适度权衡，拜占庭门控有效抵御恶意触发。
...
### [346] [Escaping the Diversity Trap in Robotic Manipulation via Anchor-Centric Adaptation](https://arxiv.org/abs/2605.07381)
*Yanzhe Chen,Kevin Yuchen Ma,Qi Lv,Yiqi Lin,Zechen Bai,Chen Gao,Mike Zheng Shou*

Main category: cs.RO

TL;DR: 提出锚点中心适应框架，通过重复演示稳定策略骨架再选择性扩展覆盖，在有限数据下提升任务可靠性。


<details>
  <summary>Details</summary>
Motivation: 机器人演示成本高，标准多样化采样策略在数据预算严格时存在覆盖-密度权衡陷阱。

Method: 两阶段适应框架：先通过核心锚点重复演示稳定策略骨架，再通过教师强制错误挖掘和约束残差更新选择性扩展高风险边界覆盖。

Result: 在同等预算下，锚点中心适应显著优于标准多样化采样策略，提升任务可靠性和成功率。

Conclusion: 验证了覆盖-密度权衡理论，证明锚点中心适应是数据有限条件下有效的策略适应方法。
...
### [347] [PathPainter: Transferring the Generalization Ability of Image Generation Models to Embodied Navigation](https://arxiv.org/abs/2605.07496)
*Yijin Wang,Yuru Tian,Xijie Huang,Weiqi Gai,Mo Zhu,Xin Zhou,Yuze Wu,Fei Gao*

Main category: cs.RO

TL;DR: 提出了一种基于鸟瞰图全局先验的导航系统，利用图像生成模型理解人类意图并生成可穿越性掩码，通过跨视角定位减少里程计漂移，在无人机上成功完成160米长距离导航。


<details>
  <summary>Details</summary>
Motivation: 鸟瞰图提供全局信息但有两个挑战：如何充分利用全局信息和如何在执行中可靠使用。

Method: 使用图像生成模型从自然语言理解意图并生成可穿越性掩码；引入跨视角定位对齐里程计与鸟瞰图。

Result: 在无人机平台上使用传统运动规划器成功完成160米户外长距离导航任务。

Conclusion: 展示了基础模型的世界理解能力可迁移至具身导航，使机器人受益于现有图像生成模型的强泛化能力。
...
### [348] [Is the Future Compatible? Diagnosing Dynamic Consistency in World Action Models](https://arxiv.org/abs/2605.07514)
*Bo-Kai Ruan,Teng-Fang Hsiao,Ling Lo,Hong-Han Shuai*

Main category: cs.RO

TL;DR: 本文提出动作-状态一致性作为评估世界动作模型可靠性的新维度，并基于此开发了一种无需价值函数的测试时选择策略。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型生成的未来预测仅关注视觉合理性，缺乏对动作序列与状态转移之间一致性的评估。

Method: 系统研究了联合预测和逆动力学模型中的动作-状态一致性，并提出了基于共识的测试时选择策略。

Result: 动作-状态一致性可以区分成功和失败的轨迹，且与学习价值估计的成功-失败趋势一致；背景坍塌是关键的边界条件。

Conclusion: 动作-状态一致性既是评估世界动作模型可靠性的诊断工具，也是价值无关规划的有效信号。
...
### [349] [Search-based Robustness Testing of Laptop Refurbishing Robotic Software](https://arxiv.org/abs/2605.07530)
*Erblin Isaku,Hassan Sartaj,Shaukat Ali,Malaika Din Hashmi,Francois Picard*

Main category: cs.RO

TL;DR: 提出PROBE方法，通过多目标优化搜索最小局部扰动，识别翻新机器人目标检测模型故障，效率是随机搜索3-7倍。


<details>
  <summary>Details</summary>
Motivation: 确保笔记本电脑翻新机器人中目标检测模型对微小输入变化的鲁棒性，避免损坏设备并支持循环经济。

Method: 基于NSGA-II的多目标搜索，同时优化故障诱导能力（定位与置信度）和扰动幅度，发现多样化故障案例。

Result: PROBE比随机搜索有效3-7倍，所需扰动更小，且生成的扰动可跨模型迁移；蜕变关系可评估非故障情况下的鲁棒性。

Conclusion: 搜索式鲁棒性测试能有效发现翻新机器人视觉模型的脆弱点，蜕变测试为模型稳定性提供额外见解。
...
### [350] [How to utilize failure demo data?: Effective data selection for imitation learning using distribution differences in attention mechanism](https://arxiv.org/abs/2605.07560)
*Kana Miyamoto,Kanata Suzuki,Tetsuya Ogata*

Main category: cs.RO

TL;DR: 提出一种利用注意力机制学习成功-失败差异的潜表征方法，从初始观测选择合适模式提升动作稳定性，并引入后训练指标筛选有益失败数据。


<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习主要依赖成功演示训练，但人工数据收集难免包含失败样本，现有方法处理失败数据需额外处理或迭代策略更新，难以直接稳定利用。

Method: 学习成功-失败差异的潜在表示并融入注意力机制，推理时从初始观测选择潜模式；提出后训练指标量化注意力差异以筛选失败样本。

Result: 仿真显示该方法在混合失败数据训练时提升任务成功率，且所提指标能筛选出对学习有益的失败样本。
...
### [351] [MemCompiler: Compile, Don't Inject -- State-Conditioned Memory for Embodied Agents](https://arxiv.org/abs/2605.07594)
...
TL;DR: 提出MemCompiler框架，通过状态条件记忆编译动态选择相关记忆，提升具身代理的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有静态记忆注入方法（AMMI）会随着智能体状态变化而失配，甚至降低基础模型性能。

Method: 学习记忆编译器读取结构化状态摘要，动态选择并编译相关记忆为可执行指导，包含文本通道和潜在Soft-Mem通道。

Result: 在Alf World、EmbodiedBench和ScienceWorld上，开源骨干网络中效果提升最高129%，匹配或接近封闭源系统，每步延迟降低60%。

Conclusion: 状态感知的记忆编译同时提升了效果和效率，优于静态记忆方法。
...
### [352] [BrickCraft: Visuomotor Skill Composition with Situated Manual Guidance for Long-Horizon Interlocking Brick Assembly](https://arxiv.org/abs/2605.07605)
*Jichuan Yu,Bowei Li,Zhenran Tang,Guanxing Lu,Chuxiong Hu,Ruixuan Liu,Changliu Liu*

Main category: cs.RO

TL;DR: BrickCraft 是一个用于互锁砖块自动组装的组合框架，通过相对公式、情境手册和组合执行管道实现长时域任务推理、空间定位和精细操作。


<details>
  <summary>Details</summary>
Motivation: 互锁砖块的自主组装需要长时域任务推理、空间定位和精细操作的集成，现有方法难以同时满足这些需求。

Method: 使用基于参考砖的相对公式分解任务为可复用基本技能；通过情境手册将组装意图投影到机器人实时观测数据中，连接高层规划与物理执行；采用组合执行管道实现长时域任务。

Result: BrickCraft 从有限的示范中学习到熟练的组装技能，并对未见结构表现出强组合泛化能力。

Conclusion: BrickCraft 有效实现了长时域和可泛化的互锁砖块组装任务。
...
### [353] [PhySPRING: Structure-Preserving Reduction of Physics-Informed Twins via GNN](https://arxiv.org/abs/2605.07687)
*Yixiong Jing,Xingyuan Chen,Guangming Wang,Olaf Wysocki,Haibing Wu,Brian Sheil*

Main category: cs.RO

TL;DR: PhySPRING是一种基于GNN的可微方法，用于降低弹簧-质量数字双胞胎的物理复杂度，在保持保真度的同时提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法重建的数字双胞胎分辨率与视觉重建匹配，但存在冗余拓扑，导致前向动力学计算成本高，因此需要一种能减少物理复杂度的方法。

Method: 使用全可微GNN，联合学习粗化图拓扑和机械参数，合并具有相似动态响应的节点，并保持每层为显式弹簧-质量系统。

Result: 在PhysTwin基准上，稠密重建和预测精度优于基线，简化模型速度提升2.30倍，并在机器人策略评估中保持操控成功率。

Conclusion: PhySPRING实现了高效、保结构的弹簧-质量模型简化，不牺牲保真度或机器人实用性。
...
### [354] [Offline-Online Hierarchical 3D Global Relocalization With Synthetic LiDAR Sensing and Descriptor-Space Retrieval](https://arxiv.org/abs/2605.07741)
*Jiahua Ren,Kai Shen,Muhua Zhang,Lei Ma*

Main category: cs.RO

TL;DR: 提出了一种离线-在线分层框架，通过解耦搜索空间实现3D全局重定位，平均时间3秒，精度8厘米。


<details>
  <summary>Details</summary>
Motivation: 大规模空间中现有方法因搜索空间巨大和计算开销高导致在线重定位时间过长。

Method: 离线阶段在网格地图中模拟激光扫描生成候选位置和几何描述子索引；在线阶段先全局检索粗位姿，再通过点云配准输出精确6自由度位姿。

Result: 在真实3D环境中平均重定位时间3秒，精度8厘米，计算效率比现有方法提升一个数量级。

Conclusion: 该框架在保持可比精度的同时大幅提高了计算效率。
...
### [355] [CommandSwarm: Safety-Aware Natural Language-to-Behavior-Tree Generation for Robotic Swarms](https://arxiv.org/abs/2605.07764)
*Mohammed Majid,Amjad Yousef Majid*

Main category: cs.RO

TL;DR: CommandSwarm是一个安全感知的语言-行为树流水线，将多语言自然语言命令转换为可执行的群体机器人行为树，结合LLM与语法验证确保安全。


<details>
  <summary>Details</summary>
Motivation: 群体机器人系统需要自然语言界面降低操作门槛，但现有方法难以处理模糊意图、语法错误和安全风险。

Method: 提出CommandSwarm流水线，包含多语言翻译、命令级安全过滤、约束提示、LoRA微调大语言模型、以及基于白名单的确定性解析验证。

Result: Falcon3-Instruct-10B和Mistral-7B-v3在少样本场景中BLEU>0.60；LoRA微调后零样本BLEU从0.267提升至0.663，语法有效性从0%提升至72%。

Conclusion: 紧凑型量化LLM结合验证流水线可生成有效的群体行为树，但解析安全过滤仍是自主部署的必要条件，不能仅依赖生成质量。
...
### [356] [Sensitivity-Based Robust NMPC for Close-Proximity Offshore Wind Turbine Inspection with a Tilted Multirotor](https://arxiv.org/abs/2605.07771)
*Giuseppe Silano,Martin Saska*

Main category: cs.RO

TL;DR: 提出了一种基于灵敏度的鲁棒非线性模型预测控制方法，用于倾斜多旋翼无人机在风力和模型失配条件下进行近距海上风机巡检，通过在线收紧塔筒安全约束来消除名义方法导致的碰撞风险。


<details>
  <summary>Details</summary>
Motivation: 名义NMPC在质量、惯性、推力效率、阻力或风力条件偏离名义假设时可能违反安全约束，导致近距巡检危险。

Method: 利用一阶参数状态灵敏度提供结构化不确定性裕度，结合阶段相关的加性裕度处理阵风，在保持NMPC优化结构不变的前提下，通过灵敏度传播和裕度评估增强约束。

Result: 在边界关键的螺旋巡检轨迹上的蒙特卡洛评估（500次不确定性实现）显示，所提控制器完全消除了名义NMPC下的碰撞违反，仅以求解时间适度增加为代价。

Conclusion: 所提灵敏度鲁棒NMPC在保证安全的前提下有效实现了近距风机巡检任务。
...
### [357] [NoiseGate: Learning Per-Latent Timestep Schedules as Information Gating in World Action Models](https://arxiv.org/abs/2605.07794)
*Wen Huang,Haoran Sun,Yongjian Guo,Yunxuan Ma,Haoran Li,Jing Long,Zhouying Mo,Zhong Guan,Yucheng Guo,Shuai Di,Junwu Xiong*

Main category: cs.RO

TL;DR: 提出NoiseGate方法，通过可学习的逐帧噪声调度策略，优化联合视频-动作建模中的信息门控，提升机器人操作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的世界动作模型（WAMs）在联合视频-动作生成中使用共享噪声调度，导致每个预测潜变量对动作生成的可靠性被不合理地等同对待，缺乏细粒度控制。

Method: NoiseGate：在骨干训练中独立采样逐帧时间步；使用轻量级门控策略网络在去噪过程中生成逐帧时间增量；通过任务奖励优化调度策略，避免手动设计形状先验。

Result: 在多种RoboTwin随机场景操作任务上，基于联合视频-动作MoT骨干的NoiseGate持续带来性能提升。

Conclusion: 可学习的逐帧噪声调度策略能有效优化信息门控，提升联合视频-动作建模的效果。
...
### [358] [Many-to-Many Multi-Agent Pickup and Delivery](https://arxiv.org/abs/2605.07835)
*Ethan Schneider,Jingkai Chen,Tianyi Gu,Kunlei Lian,Seth Hutchinson,Sonia Chernova*

Main category: cs.RO

TL;DR: 提出M2M算法解决仓库中多对多取派送问题，结合SKU分布优化，在8小时模拟中完成任务数量提升高达22000个。


<details>
  <summary>Details</summary>
Motivation: 现实仓库中任务具有多对多属性（SKU可存放在多个位置），传统一对一MAPD方法无法适用，且问题为NP-hard的四维分配。

Method: 提出M2M算法，包含两种变体：M2M（最小化任务预估时长）和M2M-wSKU（优化目标中加入SKU分布）。

Result: 在8小时仓库操作模拟中，M2M在不同环境和库存密度下均匹配或优于现有最优方法，平均多完成22000个任务。

Conclusion: M2M算法能有效解决多对多MAPD问题，提升仓储自动化效率。
...
### [359] [Melding LLM and temporal logic for reliable human-swarm collaboration in complex scenarios](https://arxiv.org/abs/2605.07877)
*Junfeng Chen,Yuxiao Zhu,An Zhuo,Xintong Zhang,Shuo Zhang,Guanghui Wen,Xiwang Dong,Meng Guo,Zhongkui Li*

Main category: cs.RO

TL;DR: 提出了一个神经符号框架，结合可验证的形式规划与LLM推理，用于长期人机群协作任务规划，支持动态环境下的自适应调度与低认知负荷交互。


<details>
  <summary>Details</summary>
Motivation: 现有LLM规划工具易产生无效任务顺序和不可行动作，需频繁人工调整；动态环境下持续监督给操作员带来高认知负担。

Method: 形式化任务目标与操作规则为时序逻辑公式和任务自动机；LLM在形式约束和实时场景上下文中生成可执行子任务序列；不确定性感知调度器分配任务至异构群并抗干扰；事件触发交互机制限制人工介入。

Result: 在异构机器人集群部署中达到相似结果，且对硬件特性和通信不确定性具有鲁棒性。

Conclusion: 该框架为动态环境下可靠、低负担的人机群协作提供了形式化且可扩展的范式。
...
### [360] [AERO-VIS: Asynchronous Event-based Real-time Onboard Visual-Inertial SLAM](https://arxiv.org/abs/2605.07885)
*Yannick Burkhardt,Sebastián Barbas Laina,Simon Boche,Leonard Freißmuth,Stefan Leutenegger*

Main category: cs.RO

TL;DR: 提出了一种基于事件相机和惯性测量单元的立体视觉SLAM系统AERO-VIS，通过异步处理事件流和集成数据驱动的关键点检测器，实现了低延迟和实时性能，并首次展示了纯事件惯性SLAM在无人机上的闭环控制和大规模状态估计。


<details>
  <summary>Details</summary>
Motivation: 事件相机在高动态范围和运动模糊场景中具有鲁棒性，但现有事件里程计方法大多按固定速率运行，限制了延迟和吞吐量。因此，需要一种能够异步处理事件流、自适应下游计算需求的系统。

Method: 设计了一个立体事件-惯性SLAM系统，包含数据驱动的关键点检测器，通过异步处理事件流动态适应运行时需求，优化性能和鲁棒性。

Result: 在无人机上实现了前所未有的精度，首次纯事件惯性SLAM系统成功演示了闭环控制和大型状态估计，且完全依赖机载计算。

Conclusion: AERO-VIS证明了事件相机在实时SLAM中的潜力，通过异步处理和数据驱动关键点检测，显著提升了在挑战性环境中的性能，并支持实际无人机应用。
...
### [361] [TAVIS: A Benchmark for Egocentric Active Vision and Anticipatory Gaze in Imitation Learning](https://arxiv.org/abs/2605.07943)
*Giacomo Spigler*

Main category: cs.RO

TL;DR: 本文提出了TAVIS，一个用于主动视觉模仿学习的评估基础设施，包含两个任务套件和三种评估原语。


<details>
  <summary>Details</summary>
Motivation: 缺乏共享基准来比较主动视觉方法在模仿学习中的贡献、适用任务类型和条件。

Method: 基于IsaacLab构建两个任务套件（TAVIS-Head和TAVIS-Hands），提供头戴摄像头与固定摄像头对比、GALT度量以及程序化分布偏移划分。

Result: 主动视觉通常有益但任务条件化；多任务策略在分布偏移下性能下降；模仿学习可产生预期性注视，其中位提前时间与人类操作者相当。

Conclusion: TAVIS为主动视觉模仿学习提供了标准化评估，揭示了任务条件性收益和策略鲁棒性问题。
...
### [362] [Evaluation of an Actuated Spine in Agile Quadruped Locomotion](https://arxiv.org/abs/2605.07988)
*Nico Bohlinger,Piotr Kicki,Davide Tateo,Krzysztof Walas,Jan Peters*

Main category: cs.RO

TL;DR: 本文通过仿真实验验证了主动脊柱在四足机器人敏捷运动中的优势，包括高速奔跑、爬楼梯、爬坡、跨越障碍和爬行等场景。


<details>
  <summary>Details</summary>
Motivation: 脊柱对四足动物高速、敏捷运动有重要作用，但主动脊柱在四足机器人中的应用效果缺乏系统实证研究。

Method: 在MuJoCo仿真环境中，使用MAB Robotics的Silver Badger机器人（配备1自由度矢状面主动脊柱），进行跨场景对比实验。

Result: 主动脊柱使机器人能克服更高楼梯、更陡斜坡、更高障碍和更窄通道，显著提升敏捷性。

Conclusion: 主动脊柱能有效增强四足机器人的运动能力，尤其在需要高度敏捷性的任务中表现突出。
...
### [363] [Active Embodiment Identification with Reinforcement Learning for Legged Robots](https://arxiv.org/abs/2605.08020)
*Nico Bohlinger,Jan Peters*

Main category: cs.RO

TL;DR: 提出了一种主动识别腿部机器人形态的方法，通过历史增强的URMA架构联合学习信息寻求行为和显式形态预测。


<details>
  <summary>Details</summary>
Motivation: 使机器人能通过与环境互动自主推断自身形态参数，适应不同形态机器人的感知需求。

Method: 使用历史增强的URMA架构，在仿真中通过与环境互动同时学习信息寻求策略和关节级与全局形态参数预测。

Result: 方法能有效推断不同形态机器人的形态参数，实现主动感知。

Conclusion: 证明了联合学习交互策略与形态预测的可行性，为机器人自适应提供了新途径。
...
### [364] [123D: Unifying Multi-Modal Autonomous Driving Data at Scale](https://arxiv.org/abs/2605.08084)
...
TL;DR: 提出了123D框架，统一处理多模态驾驶数据集，支持8个真实和1个合成数据集，并展示了跨数据集3D检测转移和规划强化学习的应用。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶数据集格式碎片化、标注不一致，阻碍了跨数据集的训练和泛化评估。

Method: 通过单一API将各模态存储为独立时间戳事件流，实现同步/异步访问，并整合了8个真实数据集和1个合成数据集。

Result: 实现了3300小时和90000公里的数据统一，验证了跨数据集3D检测和规划学习的可行性。

Conclusion: 123D框架有效解决了多模态驾驶数据的异构性问题，推动了自动驾驶研究的泛化能力提升。
...
### [365] [Stability-Certified Koopman Observer Design for Nonlinear Systems via Generalized Persidskii Dynamics](https://arxiv.org/abs/2605.07318)
*Syed Pouladi*

Main category: eess.SY

TL;DR: 提出了一种基于Koopman算子的非线性状态估计观测器，通过将误差动态建模为广义Persidskii系统，利用LMI设计非线性校正项，保证了输入到状态稳定性。


<details>
  <summary>Details</summary>
Motivation: Koopman预测器能近似非线性动力学，但在模型失配和测量噪声下如何保证观测器收敛仍是一个开放问题。

Method: 建立Koopman潜空间观测器误差动态与广义Persidskii系统的结构对应，设计非线性校正项，通过线性矩阵不等式（LMI）计算增益，同时保证估计误差的输入到状态稳定性（ISS）。

Result: 在Van der Pol振荡器和含摩擦不确定性的机械臂上验证，该观测器估计精度和鲁棒性显著优于扩展卡尔曼滤波和线性Koopman观测器，稳态RMSE最高降低42%。

Conclusion: 所提观测器在名义情况下指数收敛，在有界扰动下最终有界，理论分析和数值实验均证实了其有效性。
...
### [366] [Variational PMB filter via coordinate descent Kullback-Leibler divergence minimisation](https://arxiv.org/abs/2605.07309)
*Ángel F. García-Fernández,Yuxuan Xia*

Main category: eess.SY

TL;DR: 本文提出了一种新的变分泊松多伯努利（V-PMB）滤波器推导方法，通过引入增广空间（包括目标状态、轨迹索引和全局假设变量），证明V-PMB投影是通过坐标下降KL散度最小化来拟合最优PMB密度，并保持后验的概率假设密度。


<details>
  <summary>Details</summary>
Motivation: 现有V-PMB滤波器推导基于[#Williams15]但缺乏理论证明，本文旨在通过增广空间和KL散度最小化严格推导其数学基础。

Method: 引入包含目标状态、轨迹索引和全局假设的增广空间，通过坐标下降KL散度最小化，将PMBM后验投影为PMB密度。

Result: V-PMB投影保持后验概率假设密度；与PMBM滤波器及其他PMB变体（Murty、LBP、全局最近邻）对比，显示V-PMB在近距离目标分离时性能更优。

Conclusion: V-PMB滤波器具有严格理论推导，在多目标跟踪中优于其他PMB变体，特别在目标密集分离场景下表现突出。
...
### [367] [Learning myopic mixed-integer nonlinear model predictive control from expert demonstrations](https://arxiv.org/abs/2605.07401)
*Christopher Anthony Orrico,W. P. M. H. Heemels,Dinesh Krishnamoorthy*

Main category: eess.SY

TL;DR: 提出一种近视混合整数非线性模型预测控制框架，通过离线学习值函数近似大幅降低在线计算负担，实现实时控制。


<details>
  <summary>Details</summary>
Motivation: 解决混合整数非线性规划实时求解困难的问题，拓宽混合整数非线性模型预测控制的应用范围。

Method: 利用贝尔曼最优性原理缩短预测时域，通过逆优化和最优化残差最小化从专家示范中离线学习值函数；离线时松弛整数约束以支持KKT残差学习，在线时强制执行整数约束。

Result: 在短时域下实现高闭环性能，在Lotka-Volterra捕鱼问题和卫星姿态控制系统中验证有效性。

Conclusion: 该方法能实现实时混合整数非线性模型预测控制，且学习值函数诱导的策略与专家示范近似一致。
...
### [368] [Stochastic Differential Dynamic Programming for Trajectory Optimization under Partial Observability](https://arxiv.org/abs/2605.07529)
*Masahiro Fujiwara,Naoya Ozaki*

Main category: eess.SY

TL;DR: 本文提出了一种随机微分动态规划算法，用于耦合轨迹设计、轨道确定和修正机动规划的局部可观测轨迹优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理随机效应（如机动执行误差和观测不确定性）时，轨迹设计、轨道确定和修正机动规划紧密耦合的问题缺乏实用方法。

Method: 在信念动力学和一般任务约束下优化标称控制序列和反馈增益，显式考虑协方差传播对标称轨迹的依赖，不依赖分离原理。

Result: 在多种动力学系统、观测模型和不确定性水平下产生导航感知和不确定性鲁棒的解；在圆形限制性三体问题中，相比确定性局部优化，燃料消耗显著降低。

Conclusion: 该算法能利用轨迹设计与轨道确定之间的耦合，获得导航感知且燃料高效的解。
...
### [369] [Learning Neural Hybrid Surrogates for Gradient-Based Falsification](https://arxiv.org/abs/2605.07541)
*Lasse Kötz,Knut Åkesson*

Main category: eess.SY

TL;DR: 提出基于神经混合自动机的替代模型，通过梯度优化发现混合动力系统的反例，在多数基准测试中样本效率优于现有工具。


<details>
  <summary>Details</summary>
Motivation: 混合动力系统存在模式依赖动力学和离散跃迁，导致形式验证困难，现有替代方法局限于连续系统。

Method: 1. 用神经混合自动机学习潜在模式编码器和模式相关向量场；2. 基于轨迹数据推断转移卫条件；3. 使用梯度优化求解光滑近似安全规范，发现违反行为；4. 在原始系统上验证结果。

Result: 在大多数基准测试中一致发现反例，相比其他工具具有更高样本效率且模拟预算更低。

Conclusion: 基于学习的替代模型能有效扩展至混合系统验证，兼顾样本效率和验证可靠性。
...
### [370] [Distributionally Robust Data-Driven Predictive Control for Stochastic LTI Systems](https://arxiv.org/abs/2605.07589)
*Mirhan Urkmez,Shahab Heshmati-Alamdari*

Main category: eess.SY

TL;DR: 提出了一种基于Wasserstein模糊集的数据驱动预测控制框架，用于未知动态和扰动分布的随机线性系统，确保概率约束满足并实现最坏情况期望成本最小化。


<details>
  <summary>Details</summary>
Motivation: 现有方法在系统动态未知或扰动分布不确定时难以同时保证约束满足和最优性能，需要一种可计算的分布鲁棒控制方法。

Method: 利用离线轨迹通过最小二乘拟合子空间预测控制预测器，构建预测残差的经验分布，并围绕其建立Wasserstein模糊集，最小化最坏情况期望成本同时强制概率输出约束满足。

Result: 问题可转化为易处理的直接数据驱动形式，无需显式模型识别；基于有限样本浓度结果给出了Wasserstein半径，确保高概率下真实期望成本有界且约束满足。

Conclusion: 数值仿真验证了框架在不同扰动条件和成本函数下优于现有方法，为数据驱动鲁棒控制提供了理论保证。
...
### [371] [Electric Axle and Wheel Module Driveline Concepts for Self-propelled Agricultural Machinery and Equipment Carriers](https://arxiv.org/abs/2605.07657)
*Timo Oksanen,Karl Th. Renius*

Main category: eess.SY

TL;DR: 本文比较了自走式农用机械中电动轮模块和电动轴模块两种驱动方案的设计自由度、成本、效率和可控性等工程要求。


<details>
  <summary>Details</summary>
Motivation: 直接电动传动系统为车架和悬架设计提供了新的自由度，并能降低能量损失，因此需要评估两种模块化方案的适用性。

Method: 针对四种车轮驱动的典型车辆，定义了轴模块和轮模块两种独立的机电单元，并对比了负载、效率、转向、控制、制动、悬架、结构支撑、非对称轮载和制造成本等工程要求。

Result: 轮模块提供最大设计自由度、冗余和可控性，而轴模块成本更低、结构刚性好、通过差速器自动分配负载，并可用于现有车辆结构。

Conclusion: 两种模块化概念均将分布式控制和转向集成在每个单元中，仅需直流电源总线与车辆通信，适用于自走式农用机械、设备载体、推进拖车和田间机器人。
...
### [372] [Spatiotemporal Trust Evaluation for Collaborator Selection via Customized GNN-Mamba](https://arxiv.org/abs/2605.07658)
*Botao Zhu,Xianbin Wang*

Main category: eess.SY

TL;DR: 本文提出GNN-Mamba模型，结合图神经网络和Mamba时序模型评估设备可信度，以优化协作者选择，在准确性和稳定性上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有信任评估方法难以同时处理多样化信任视角和复杂时空依赖，导致协作者选择效果不佳。

Method: 采用GNN融合空间信任关系，Mamba捕获短期波动与长期演化，并结合任务特定资源信任。

Result: 实验表明GM模型在信任评估的准确性和稳定性上优于基线方法。

Conclusion: GNN-Mamba模型有效提升了协作任务中协作者选择的可靠性。
...
### [373] [Cascade PID Control of an Inverted Pendulum on a Cart System: Simulation and Experimental Analysis](https://arxiv.org/abs/2605.07712)
*Khalid Mehrab,Md Zamiul Alam,Shadman Tahmid Haque*

Main category: eess.SY

TL;DR: 本文通过仿真和实验研究了倒立摆系统的级联PID控制，发现其在理想条件下有效，但实际中因传感器噪声和摩擦等与仿真存在差异，且扰动抑制能力有限。


<details>
  <summary>Details</summary>
Motivation: 探索级联PID控制在倒立摆系统中的实际性能及其局限性。

Method: 使用Simscape Multibody建立非线性模型，构建含DC电机、编码器等组件的物理原型，设计内环调节摆角、外环控制位置的级联PID结构，并进行LQR比较分析。

Result: 仿真实现稳定控制和轨迹跟踪；实验证实实时稳定，但增益、瞬态响应和扰动抑制与仿真差异明显，LQR内环改善性能。

Conclusion: 级联PID控制在倒立摆系统中有效但受实际因素限制，LQR可提供更好的扰动抑制，研究为应用提供实用见解。
...
### [374] [Efficient MILP-based Urban Network Traffic Control in Mixed Autonomy with Dynamic Saturation Rates](https://arxiv.org/abs/2605.07743)
*Muhammad Haris,Claudio Roncoli*

Main category: eess.SY

TL;DR: 提出了一种面向混合交通流的动态响应型饱和率控制策略，通过扩展多商品存储-转发模型和非凸二次规划优化信号配时与CAV路径。


<details>
  <summary>Details</summary>
Motivation: 现有控制策略未明确考虑CAV与HDV驾驶行为对交通流特性的影响，需要更适配混合自主驾驶环境的优化方法。

Method: 基于扩展的多商品存储-转发模型，构建非凸二次规划，通过上下估计器松弛为混合整数线性规划求解。

Result: 微观仿真验证了方法在实时优化中的高效性和鲁棒性，优于已有方法。

Conclusion: 该策略有潜力应用于未来城市交通系统的实时优化。
...
### [375] [Control and Scheduling of Behind-the-Meter Battery Energy Storage Systems for Stacked Grid and Building Services](https://arxiv.org/abs/2605.07762)
*Nour-eddine Id omar,Alexandre Lê-Agopyan,Fabrizio Sossan*

Main category: eess.SY

TL;DR: 提出并验证了一种用于电表后端电池储能系统的两阶段调度与控制策略，实现本地与电网服务协同。


<details>
  <summary>Details</summary>
Motivation: 最大化光伏自消纳、削峰及提供二次频率控制服务，同时优化电池容量分配。

Method: 日前阶段采用场景法分配容量，实时阶段基于高频更新数据计算设定点。

Result: 在300 kW建筑与264 kWh电池系统上实验验证，有效协调表前与表后服务。

Conclusion: 该策略能够有效调度并执行多元服务，提升储能系统综合效益。
...
### [376] [Interactive Trajectory Planning with Learning-based Distributionally Robust Model Predictive Control and Markov Systems](https://arxiv.org/abs/2605.07768)
*Erik Börve,Nikolce Murgovski,Morteza Haghir Chehreghani,Leo Laine*

Main category: eess.SY

TL;DR: 提出了一种结合PAC学习和分布鲁棒优化的随机模型预测控制方法，用于在不确定周围智能体决策下进行交互式轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理学习分布误差时缺乏理论保证，无法平衡鲁棒性与数据效率。

Method: 先学习决策分布并求解SMPC问题，利用PAC学习理论量化分布误差，再通过分布鲁棒优化构建DR-MPC框架。

Result: 该框架能根据样本数量在鲁棒MPC与理想SMPC之间灵活插值。

Conclusion: PAC学习驱动的DR-MPC为不确定性下的轨迹规划提供了理论支撑与实用平衡方案。
...
### [377] [Sampling-based Model Predictive Control Using Trust Regions](https://arxiv.org/abs/2605.07801)
*Markus Walker,Marcel Reith-Braun,Daniel Frisch,Uwe D. Hanebeck*

Main category: eess.SY

TL;DR: 提出了一种基于信赖域优化的采样模型预测控制方法，通过KL散度约束和熵约束替代启发式超参数调整，并结合确定性局部累积分布采样提升样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有采样MPC方法依赖启发式超参数调整或手动调参，缺乏理论指导。

Method: 引入KL散度约束和熵下界构成信赖域优化，基于拉格朗日乘子自动更新；结合确定性局部累积分布（LCD）采样。

Result: 在基准环境中，该方法在低样本和低迭代次数下收敛更快、样本效率更高，尤其与LCD采样结合时效果更优。

Conclusion: 所提信赖域更新策略能有效替代启发式超参数，提升采样MPC的收敛性和效率。
...
### [378] [Allocation of Dynamic Operating Envelopes in Radial Distribution Networks](https://arxiv.org/abs/2605.07989)
*Wilhiam de Carvalho,Florin Capitanescu,Cyril Rasic,Jean-François Toubeau,François Vallée*

Main category: eess.SY

TL;DR: 分析了动态运行包络（DOE）公式的不同方面如何影响网络容量的计算与分配，并提出了新的LACE算法。


<details>
  <summary>Details</summary>
Motivation: 现有DOE计算在模型选择、约束类型和计算场景下存在不确定性，影响了容量分配的效率和公平性。

Method: 对比了非线性与线性潮流模型、热约束与电压约束、以及进出口包络场景；提出了LACE算法，支持透明可扩展计算。

Result: 不同模型和约束对包络形状有显著影响；LACE算法在大型网络和联合优化中表现出高效性。

Conclusion: DOE计算需综合考虑模型和约束选择；LACE算法为配电网运营商提供了实用工具。
...
### [379] [Entropic Value-at-Risk for Inter-Vehicle Collision in Platoons: Network- and Delay-Induced Bounds on Risk Due to Extreme Events](https://arxiv.org/abs/2605.08015)
*Vivek Pandey,Nader Motee*

Main category: eess.SY

TL;DR: 提出了一个基于熵值风险（EVaR）的框架，用于量化受随机扰动和时延影响的网联车辆队列碰撞风险，推导了网络结构和时延对风险上下界的理论表达式。


<details>
  <summary>Details</summary>
Motivation: 现有风险度量（如VaR和CVaR）难以准确捕捉极端事件（如碰撞）的风险，需要一种更保守的度量方法，并考虑时延和网络拓扑的影响。

Method: 采用EVaR作为风险度量，利用拉普拉斯特征值刻画车间距离协方差，推导网络与时延相关的风险界。

Result: 代数连通性决定最大EVaR，最大拉普拉斯特征值决定最小固有风险；数值实验验证了网络拓扑和时延对碰撞风险的影响。
