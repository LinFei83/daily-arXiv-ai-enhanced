...
TL;DR: 提出世界AI模型网络（AI-ModelNet）概念，通过模型间互联实现能力共享与协同推理，以解决大模型部署成本高、异构模型协作难的问题。


<details>
  <summary>Details</summary>
Motivation: 大模型实际应用受限于高训练成本和部署复杂度，异构模型激增导致交互协作成为瓶颈，受互联网发展启发。

Method: 提出分层次系统架构，构建原型系统，通过多样化应用案例验证框架可行性。

Result: 验证了AI-ModelNet框架在模型互联与协同推理方面的有效性。

Conclusion: AI-ModelNet为轻量级、私有化、领域特定模型的高效协作提供新范式，并指出未来研究方向。
...
TL;DR: 该研究探究了大型语言模型（LLM）多智能体系统中人格特质（宜人性）对团队任务表现的影响，发现效果取决于任务类型。


<details>
  <summary>Details</summary>
Motivation: 先前研究显示人格提示能改变LLM的交流风格（如低宜人性导致对抗性语言），但交流风格与任务目标达成之间的关系尚未在多领域系统检验。

Method: 通过操纵前沿LLM的人格特质（宜人性高低），在编程、开放研究合作和竞争性谈判三个任务域测试多智能体团队表现。

Result: 编程任务中低宜人性显著改变交流风格但对里程碑完成影响小；开放合作和谈判任务中相同操纵会大幅降低表现。

Conclusion: 人格效应对任务结构敏感，低宜人性在结构化任务中影响有限，但破坏开放式协作；研究为多智能体系统设计及人格操纵的边界提供了启示。
...
TL;DR: 本文提出一个名为ODYSSEY的范畴框架，用于构建可验证、局部保持真实的基础模型，通过将构建块组件（foundries）组合而成，并支持领域构造、工件重放、层诊断等功能。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型难以保证局部真理和可验证性，缺乏系统化的架构来组合不同知识源和审计机制。

Method: 使用左Kan扩展和右Kan扩展构建foundry，通过Foundry SQL和TICKET认证实现工件管理和因果提取。

Result: 实现了多个具体foundry，验证了同一套范畴工具可支持领域构造、诊断、论证和查询优化。

Conclusion: ODYSSEY框架为构建可审计、局部可信的基础模型提供了统一的理论和实践范式。
...
TL;DR: 提出了一个低资源LLM框架DysLexLens，用于通过在线论坛讨论分析阅读障碍学习者的AI工具使用体验，包含字典驱动过滤、知识图谱推理、定量定性评估等功能，并在Reddit数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 阅读障碍学习者越来越多地使用AI工具，但他们的实际体验尚未被充分研究。

Method: 构建了端到端的DysLexLens框架：字典驱动过滤论坛帖子、结合LLM语义分析和知识图谱推理、使用RAGAS等评估指标、设计定性验证指南。

Result: 在Reddit阅读障碍相关帖子和30个问题上展示了有效性，具有推广到其他低资源论坛的潜力。

Conclusion: DysLexLens能有效分析低资源场景下的用户经验，框架和资源已开源以支持可复现性。
...
### [6] [ToE: A Hierarchical and Explainable Claim Verification Framework with Dynamic Multi-source Evidence Retrieval and Aggregation](https://arxiv.org/abs/2606.27736)
*Zhaoqi Wang,Zijian Zhang,Kun Zheng,Zhen Li,Xin Li,Chunlei Li,Jiamou Liu*

Main category: cs.AI

TL;DR: ToE是一个分层证据推理框架，将每个声明建模为动态扩展的论点树，通过强化学习驱动的检索智能体、证据评估智能体和论点树聚合算法，实现可解释的证据链推理。


<details>
  <summary>Details</summary>
Motivation: AI生成的虚假新闻在生成式引擎优化污染下系统性地被检索系统呈现，污染LLM推理，威胁信息生态系统。

Method: 整合强化学习驱动的多源检索智能体、证据评估智能体和论点树聚合算法，迭代地分解、检索和验证声明，并提供形式化的检索误差界保证。

Result: 在多个数据集和骨干LLM上相比基线提升4至24个百分点，尤其在对抗性污染输入上效果显著。

Conclusion: ToE通过证据链推理有效检测虚假新闻，理论保证策略收敛到信息论最优策略的邻域，对抗污染鲁棒性强。
...
TL;DR: 本文提出MER-R1框架，通过强化学习将快思考的直觉和慢思考的推理结合，实现多模态情感识别中召回率和精度的联合优化，提升性能并验证了推理的实际好处。


<details>
  <summary>Details</summary>
Motivation: 现有推理型多模态大模型在情感识别中，快思考（直接回答）的准确率常优于慢思考（推理后回答），但两者各有优缺点，缺乏有效整合机制。

Method: 采用MER-R1强化学习框架，包含双目标解耦（分离召回率和精度优化信号）和慢快信心校准（对齐最终答案与直觉，增强正确情感抑制错误情感）。

Result: 在MER-UniBench和MME-Emotion数据集上达到最优性能，使推理真正提升情感识别，并提供了理论证明缓解方差干扰。

Conclusion: 通过显式优化快慢思考的互补性，实现了比单一思考模式更优的情感识别效果，证明了推理在正确优化下对准确性的贡献。
...
TL;DR: 本文提出一种符号反馈驱动的迭代自优化框架，通过自然语言提示、符号验证器和计划识别器提升大语言模型在长周期规划中的鲁棒性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在长周期规划中常产生不可行或错误的解，存在安全性和可靠性问题。

Method: 使用自然语言提示将逻辑符号映射为自然语言描述；设计符号验证器识别错误并转换为修正指令；利用计划识别器推断目标可达性。

Result: 该框架在长周期规划任务中持续提升了可行性和正确性。

Conclusion: 该框架有效增强了基于大语言模型的规划可靠性，有望实现更可信的AI系统。
...
TL;DR: 本文研究图世界模型中的长时域展开误差，提出统一框架和误差感知模型，防止展开发散。


<details>
  <summary>Details</summary>
Motivation: 许多规划环境是图结构，传统局部预测误差会沿图传播，且动态边预测改变失败模式，需研究长时域展开误差。

Method: 建立固定边和动态边图世界模型框架，推导图值展开界，提出联合节点-边算子用于动态边展开，并设计误差感知图世界模型（光谱正则化、展开一致性和关键节点加权）。

Result: 合成和异构图上，展开误差和规划遗憾随时域增长；动态边训练对结构演化必要；误差感知模型防止长时域发散且保持精度；图世界模型在动态图和智能体规划中最有效。

Conclusion: 图世界模型适用于动态图展开和智能体规划，而专用图模型在静态或稀疏预测中仍占优势。
...
TL;DR: 本文比较了基于代理的世界模型和参数化世界模型在图形规划基准上的表现，提出了GILP方法，通过结合小型参数化骨干网络和LLM推理，将幻觉状态率从0.176降至0.035，成功率从0.668提升至0.838。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型要么灵活但易产生难评分的幻觉错误，要么易于评估但规划能力弱，因此需要一种结合两者优势的方法。

Method: GILP方法训练小型参数化骨干网络，预测有效动作、状态差异、风险和值，再结合LLM生成动作和想象差异，通过一致性门控机制在分歧时要求修订。

Result: 在真实GPT-4o-mini调用中，幻觉状态率从0.176降至0.035；在模拟消融实验中，成功率从0.668提升至0.838，仅增加约22%的LLM调用。

Conclusion: GILP有效结合了参数化模型的可测量性和代理模型的灵活性，显著减少了幻觉并提高了规划成功率。
...
TL;DR: 提出了ATOD混合在线蒸馏算法，结合OPD和RL，通过退火调度和回合级重加权，在长时交互任务中提升小语言模型性能。


<details>
  <summary>Details</summary>
Motivation: OPD早期高效但后期饱和，RL探索性好但早期低效，ATOD利用两者互补性。

Method: 退火式OPD-RL调度（早期OPD主导，后期RL增强）和回合级分歧-不确定性重加权（T-DUR）。

Result: 在ALFWorld、WebShop和Search-QA上，ATOD平均成功率比OPD高3.03点，比GRPO高23.62点，并超越教师模型2.16点。

Conclusion: ATOD有效结合模仿学习和强化学习，更高效地优化小语言模型在长时交互任务中的表现。
...
TL;DR: 提出NormAct基准评估多模态大模型在具身规划中隐含社会规范的遵守能力，发现模型在显式目标（67.3%）与隐含规范（26.4%）间存在显著差距，并提出NormPerceptor将任务成功率从24.2%提升至46.7%。


<details>
  <summary>Details</summary>
Motivation: 现有评估多聚焦显式目标达成或直接规范知识，缺乏对模型在动作序列中推断并应用隐含社会规范的测试。

Method: 构建NormAct基准嵌入隐藏规范，并通过提示实验分析差距原因，提出NormPerceptor作为上下文条件化提示生成器，在规划前推断场景相关规范。

Result: 顶尖MLLM显式目标完成率67.3%，但规范遵守率仅26.4%；NormPerceptor将任务成功率从24.2%提升至46.7%。

Conclusion: 具身智能体需主动检测、视觉定位并整合隐含规范作为规划约束，以缩小显式目标与规范遵守的差距。
...
TL;DR: 提出SD-GPS框架，将符号求解器作为执行预言机，通过求解器驱动的自动形式化和验证定理提议提升几何问题求解能力。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号框架在自动形式化和定理预测阶段存在瓶颈，静态翻译与求解器不兼容，固定规则库导致演绎僵局。

Method: 包括求解器驱动自动形式化（基于QwenVL3-2B，结合监督适应和可解性强化学习）和验证定理提议（感知僵局并提议辅助引理，经符号验证确保正确性）。

Result: 在Geometry3K和PGPS9K上，SD-GPS在标准完成、多项选择和跨模态引用任务中优于现有方法。

Conclusion: 闭环多模态感知与符号执行显著提升几何推理，证明神经代理可通过形式系统实现可验证的问题求解。
...
TL;DR: 本文提出RelBall模型，通过模长变换和尾实体关系球，全面覆盖知识图谱中的多种关系模式，提升链接预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型如RotatE和Rotate3D无法同时处理非交换组合、语义层次和一对多关系，限制了知识图谱补全的效果。

Method: RelBall在Rotate3D基础上引入模长变换建模语义层次，并设计尾实体关系球处理一对多等多类型关系。

Result: 实验表明RelBall在多个数据集上取得了具有竞争力的链接预测性能。

Conclusion: RelBall能建模所有关系模式，提供可解释的层次表示，并支持多种基数关系，优于现有模型。
...
TL;DR: 本文提出参数化因果因子图（PCFG）和部分有向PCFG（PD-PCFG）两种模型，结合提升因果推理算法（LCI），实现关系领域因果效应的高效计算，并与命题推理相比显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有提升推理主要处理概率查询，缺乏对因果效应计算的支持。

Method: 引入PCFG定义因果干预语义，设计LCI算法进行提升级因果推理，并扩展至PD-PCFG处理部分因果知识。

Result: LCI算法在提升层计算因果效应，速度显著快于基于因果贝叶斯网络的命题推理。

Conclusion: 提升因果推理可高效处理关系领域因果效应，PD-PCFG进一步降低对完整因果知识的依赖。
...
TL;DR: 京东氧气AI商品中心是一个基于LLM/VLM的工业级平台，用于十亿级商品知识的生产与服务。


<details>
  <summary>Details</summary>
Motivation: 京东平台拥有超过7亿用户和数百万商家，商品SKU达数百亿，传统方法难以应对快速涌现的新概念、高质量知识生产和多样化下游需求。

Method: 围绕四个核心支柱：基于人机协作的本体工程、语义搜索后判别的知识识别架构、自进化商品理解模型、统一商品通道。

Result: 知识生产精度94.2%，召回率82.8%；搜索流量覆盖率达80.4%；商品信息质量问题减少37%；核心属性自动填充率超过80%。

Conclusion: 系统已部署到搜索、推荐、运营等核心业务场景，实现了可量化的规模效益。
...
TL;DR: 提出基于本体引导的证据路径推断框架OPI，通过关系中心本体图约束搜索空间，并采用双向检索与迭代精炼策略提升多跳KGQA准确率。


<details>
  <summary>Details</summary>
Motivation: 现有主题中心扩展方法面临搜索空间爆炸及路径语义不满足复杂问题约束的挑战。

Method: 构建关系中心本体图捕捉关系头尾类型约束；采用双向检索机制（主题侧前缀扩展+答案侧末跳匹配）抑制噪声；应用迭代精炼策略筛选类型兼容但问题无关的路径。

Result: 在WebQSP、CWQ、MetaQA上，Hit@1/F1分别提升4.6/5.0、8.9/3.3，MetaQA单检索模块达到近饱和性能。

Conclusion: OPI有效减少搜索空间并提升复杂多跳问题回答的准确性，尤其在约束感知路径推理方面表现优异。
...
TL;DR: 本文提出自动化设计（AiD）范式，利用深度学习与生成式AI实现计算设计综合（CDS），通过两个案例验证其在高科技系统设计中减少组合复杂性的有效性。


<details>
  <summary>Details</summary>
Motivation: 现代高科技系统设计面临组合复杂性挑战，传统方法难以应对。

Method: 提出计算设计综合（CDS）框架，结合深度学习与生成式AI自动化生成创新系统。

Result: 两个案例（电子驱动系统设计与空间尺寸问题）证明了该方法的可行性。

Conclusion: AI驱动方法从仿真优化转向自主设计，标志着工程领域的根本性转变。
...
TL;DR: 提出了Tandem RL（TRL），一种将“双人训练”范式融入RLVR的方法，通过强弱模型交替生成推理过程，以解决RLVR导致的推理模式不兼容问题。在竞赛数学任务上，TRL在保持强模型独立推理能力的同时，显著提升了与弱模型的协作鲁棒性并降低了分布偏移。


<details>
  <summary>Details</summary>
Motivation: RLVR显著提升了LLM的推理能力，但导致推理模式向低可读性、语言混杂等特征漂移，使得弱模型或人类难以有效利用这种能力。现有双人训练范式仅在概念验证场景中有效，未验证其在现代RLVR长链推理中的可扩展性。

Method: 提出TRL框架，其中强模型（senior）与冻结的弱模型（junior）以随机交替方式共同生成推理过程；生成的完整序列获得奖励，并对强模型应用标准GRPO损失函数进行优化。

Result: 在竞赛数学任务上训练Qwen3-4B-Instruct，TRL在强模型独立推理能力上与GRPO持平，同时带来三个新特性：强模型与弱模型的切换鲁棒性增强、弱模型的分布漂移减小、生成的推理链对弱模型更可理解。

Conclusion: TRL为RLVR提供了一种有前景的路径，在多模型通信和人类兼容性方面具有实际收益。
...
TL;DR: 提出首个嵌入智能体认知循环的仿生内生防御架构ANIS，包含六层免疫塔、病毒/疫苗分类体系、三重监控框架及持续免疫学习机制，理论区分模型对齐与智能体免疫。


<details>
  <summary>Details</summary>
Motivation: 现有外挂式防御机制无法应对运行时劫持（如记忆投毒、工具链操纵、多智能体协议攻击），需内生免疫架构。

Method: 六层免疫塔(含非认知隔离层)、Agent病毒/疫苗统一分类、Harness三重组框架驱动持续免疫学习(CIL)。

Result: ANIS作为动态运行时执法机制，弥补静态对齐的实时防护缺陷，提出自身免疫率等新评估指标。

Conclusion: ANIS为智能体提供运行时自适应免疫，但需应对免疫协议标准化、病原体-疫苗共进化等挑战。
...
### [21] [An Embedded Real-Time License Plate Recognition System for Complex Traffic Scenes](https://arxiv.org/abs/2606.27772)
...
TL;DR: 本文提出了一种面向发展中国家的嵌入式实时车牌识别系统，在复杂交通场景下实现了轻量级CNN检测与识别，并通过量化和FPGA加速在Xilinx Kria KV260平台上达到11.5 FPS。


<details>
  <summary>Details</summary>
Motivation: 发展中国家存在交通场景复杂、车辆类型多样、低成本部署需求高等挑战，现有系统难以兼顾准确性与实时性。

Method: 采用轻量级卷积神经网络进行车牌检测和字符识别，使用Brevitas库进行低比特量化，并基于FINN框架在FPGA上加速。

Result: 在SL-LPR数据集上检测mAP达93.6%，识别准确率87.88%，在多个公开数据集上具有竞争力，系统在嵌入式平台上实现11.5 FPS。

Conclusion: 该嵌入式系统能在复杂交通场景下实现实时车牌识别，适用于发展中国家的低成本部署。
...
### [22] [Not All Relations Rotate Alike: Transformation-Aware Decoupling for Viewpoint-Robust 3D Scene Graph Generation](https://arxiv.org/abs/2606.27412)
*Jingjun Sun,Chaowei Wang,Zhirui Liu,Jiaxu Tian,Ming Yang,Yaoxing Wang,Shan Gao*

Main category: cs.CV

TL;DR: 提出TAD框架，通过解耦关系推理（方向性谓词与稳定性谓词）实现视角鲁棒的3D场景图生成。


<details>
  <summary>Details</summary>
Motivation: 现有3DSGG模型在视角旋转（yaw）下缺乏对谓词变换行为的异构性建模，如方向谓词应随视角变化而稳定谓词应保持不变。

Method: 设计变换感知解耦（TAD）架构，将关系推理分解为稳定分支（不变谓词）和方向分支（可变谓词），并利用变换描述符和分组辅助监督学习互补特征。

Result: 在3DSSG数据集上，不依赖旋转数据增强即达到最新视角鲁棒性能，标准基准测试中保持竞争力。

Conclusion: 解耦谓词变换行为可有效提升3DSGG对视角变化的鲁棒性，且无需额外增强数据。
...
### [23] [SemCityLoc: Aerial 6DoF Localization Using Semantic 3D City Models](https://arxiv.org/abs/2606.27444)
*Jingfeng Mao,Xuyang Chen,Qilin Zhang,Oussema Dhaouadi,Guangming Wang,Brian Sheil,Daniel Cremers,Yan Xia,Olaf Wysocki*

Main category: cs.CV

TL;DR: 本文提出SemCityLoc，利用语义-几何对齐实现无GNSS的无人机6DoF定位，在城区测试中将定位误差从9.89米降至2.62米。


<details>
  <summary>Details</summary>
Motivation: 传统航空定位依赖GNSS或高保真3D重建，限制了可扩展性和机载部署能力。

Method: 将基础模型生成的视觉语义先验与标准化LoD城市模型进行语义表面和单帧深度对齐，而非稀疏轮廓或密集纹理匹配。

Result: 在真实城区测试中，相比现有基于地图的方法，召回率提升36%，平均位置误差从9.89米降至2.62米。
...
### [24] [Fine-tuning a multimodal large language model for clinician-grade autism behavioral scoring from short home videos](https://arxiv.org/abs/2606.27484)
...
TL;DR: 通过微调Gemini 2.5 Pro模型在家庭视频上提取行为特征，自闭症诊断准确率达77%，AUC为86%，与临床医生水平相当。


<details>
  <summary>Details</summary>
Motivation: 自闭症谱系障碍（ASD）影响1/31的美国儿童，但诊断中位年龄超过4岁，亟需利用易获取的观察数据（如家庭视频）实现早期诊断。

Method: 对Gemini 2.5 Pro进行低秩自适应微调，基于400个临床评分的家庭视频，仅训练30个行为特征，随后将特征输入多种机器学习模型进行分类。

Result: 在99名保留儿童（49名ASD，50名典型发育）中，特征级加权Cohen's kappa提高40%（p<0.001），28个可评估特征中27个改善；零样本ASD诊断F1提高53%（p<0.001），分类器辅助管道准确率77%、AUC 86%。
...
### [25] [SelectAnyTree: A Promptable Instance Segmentation Model for 3D Forest LiDAR Point Clouds](https://arxiv.org/abs/2606.27491)
...
TL;DR: 提出了SelectAnyTree，一种基于点击提示的森林LiDAR点云实例分割模型，通过少量点击即可分割任意单木。


<details>
  <summary>Details</summary>
Motivation: 森林监测需要可扩展、精细的3D测量，但现有方法缺乏树实例标注，手动标注耗时易错，且已有自动预分割缺乏交互式或AI辅助优化。

Method: 设计了Click-to-query提示编码器将点击转为内容查询，并引入CHM引导的首个提示自动获取树顶，最后通过状态空间查询解码器高效生成树掩码。

Result: 在七个森林区域和独立测试集上，单次点击达到78.2 IoU，比最强基线高24.8点，且参数量和推理时间更少。

Conclusion: SelectAnyTree在交互式和实例级分割中表现优异，泛化能力强，为森林点云标注提供高效解决方案。
...
### [26] [DMV-Bench: Diagnosing Long-Horizon Multimodal Agents' Visual Memory with Incidental Cue Injection](https://arxiv.org/abs/2606.27499)
*Yujin Tang,Chenming Shang,Ruize Xu,Nikhil Singh*

Main category: cs.CV

TL;DR: 提出了第一个多模态智能体视觉记忆交互式基准DMV-Bench，并基于双编码理论设计了DualMem记忆架构，实验证明其优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆研究几乎完全集中在文本侧，缺乏在交互环境中评估智能体真正需要记住视觉信息的基准。

Method: 构建基于1000种产品变体的家居电商目录，通过文本泄漏控制确保任务区分信号仅存在于像素中；提出DualMem架构，并行维护视觉和语言编码。

Result: DualMem在Gemini 2.5 Flash和Qwen2.5-VL-7B上，于所有链长度（5,10,15,50）均优于字幕基线和三种近期多模态智能体记忆系统。

Conclusion: 双编码机制有效，其中视觉通道端到端承载线索，语言通道仅辅助查询定位，视觉主导模式能显著提升记忆性能。
...
### [27] [Aloe-Vision: Robust Vision-Language Models for Healthcare](https://arxiv.org/abs/2606.27500)
...
TL;DR: 提出了Aloe-Vision系列医疗大模型，包含高质量数据集、两个规模的模型（7B和72B），以及低污染评估基准CareQA-Vision，展示了训练数据质量对模型性能的关键影响。


<details>
  <summary>Details</summary>
Motivation: 医疗领域LVLM面临高质量多模态数据稀缺、鲁棒性不足和评估基准污染等挑战，需要完全开源可复现的解决方案。

Method: 构建Aloe-Vision-Data质量过滤多模态数据集，训练Aloe-Vision系列模型，并开发CareQA-Vision基准进行可靠评估。

Result: 高质量训练数据使模型在保持通用能力的同时显著提升医疗任务性能，达到SOTA竞争水平；但当前模型在面对对抗性输入时仍存在脆弱性。

Conclusion: 高质量数据混合训练是平衡医疗专用LVLM性能与通用能力的关键，但临床部署前需解决鲁棒性不足的问题。
...
### [28] [ReWorld: Learning Better Representations for World Action Models](https://arxiv.org/abs/2606.27504)
...
TL;DR: 提出ReWorld框架，通过直接优化中间表示提升自动驾驶世界动作模型的表征学习效率，在视频生成和规划任务上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有WAM仅通过输出端监督学习世界表征，中间表示未能直接优化，导致表征效率不足。

Method: 对生成模块的视频DiT施加未来预测监督，对规划模块的动作DiT进行跨模态对齐和安全边界的难负样本判别性优化。

Result: FVD降低23.9%（81.3→61.9），闭环PDMS提升至90.4，训练加速约2倍。

Conclusion: 直接优化中间表示可显著提升WAM的表征质量和规划性能，无需后训练或后处理。
...
### [29] [TruEye: Fine-Grained Detection of AI-Generated Human Subjects in Images](https://arxiv.org/abs/2606.27505)
*Jay Barot,Dan Lin*

Main category: cs.CV

TL;DR: 提出TruEye模型，首次区分五种合成内容类别，通过掩码条件双流Transformer实现快速、细粒度且可解释的AI生成图像检测与定位。


<details>
  <summary>Details</summary>
Motivation: 现有检测器泛化性差、依赖昂贵大模型或仅做二分类，无法应对真实人头合成到虚假场景等复杂伪造。

Method: 基于掩码条件的双流Transformer，分离人体与场景令牌并保持空间对应，通过区域门控交叉注意力和令牌级监督实现语义一致性，无需大模型。

Result: 在6个数据集和FineSyn上超越现有检测器，准确率更高、推理速度比基于LLM的方法快100倍以上，对未见过的生成/篡改图像泛化能力强。

Conclusion: TruEye能高效、可解释地检测和定位多种AI合成内容，特别擅长处理最复杂的“真实人在虚假场景”情况。
...
### [30] [Structured-Li-GS: Structured 3D Gaussians Splatting with LiDAR Incorporation and Spatial Constraints](https://arxiv.org/abs/2606.27509)
*Huaiyuan Weng,Huibin Li,Chul Min Yeum*

Main category: cs.CV

TL;DR: 本文提出了Structured-Li-GS，一种结合LiDAR-惯性-视觉SLAM的结构化高斯泼溅框架，通过锚定高斯原语和综合损失函数，实现了少高斯数下的高质量3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有高斯泼溅方法在重建时通常需要大量高斯原语，且缺乏与LiDAR数据的有效结合，导致模型庞大或精度不足。

Method: 利用LiDAR-惯性-视觉SLAM生成精确的密集彩色点云，通过子采样点云锚定高斯原语，并从局部表面几何初始化椭球参数；训练中结合光度、平坦化、偏移、深度和法向损失。

Result: 在基准数据集和自建数据集上，Structured-Li-GS使用更少的高斯原语达到了超越现有方法的保真度。

Conclusion: 该方法在保持中等模型大小的同时，实现了尺度一致的高保真重建，验证了LiDAR点云引导的高斯泼溅的有效性。
...
### [31] [Tessellating The Earth](https://arxiv.org/abs/2606.27514)
*Daniel Cher,Hamza Iqbal,Eric Xing,Brian Wei,Nathan Jacobs*

Main category: cs.CV

TL;DR: TTE是一种基于可学习球面Voronoi分割的地理位置编码器，通过密集分配表示能力和全局语义令牌，在多个地理空间任务上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有地理位置编码器对坐标均匀分配表示能力，导致海洋等无特征区域与城市等关键区域占用相同资源，不够高效。

Method: 使用可学习的球面Voronoi分区，每个站点携带独立嵌入并在训练中迁移至判别区域；引入全局语义令牌从卫星图像中提取语义知识形成紧凑词汇表。

Result: 在多个地理空间分类和回归任务中达到新最优，作为iNaturalist-2018细粒度物种分类的地理先验时效果最强。

Conclusion: TTE通过自适应分配表示能力和共享语义知识，显著提高了位置编码器的效率和精度。
...
### [32] [Large Language Model Teaches Visual Students: Cross-Modality Transfer of Fine-Grained Conceptual Knowledge](https://arxiv.org/abs/2606.27527)
*Thomas Shih-Chao Liang,Zhuoran Yu,Yong Jae Lee*

Main category: cs.CV

TL;DR: 提出LaViD框架，通过LLM生成多项选择题将语言知识蒸馏到视觉模型，无需多模态数据，在多个细粒度基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在非语言模态中的监督潜力，并解决多模态数据依赖问题。

Method: 使用LLM生成多项选择题（MCQs）作为语义区分信号，将视觉类别映射为软标签分布，通过辅助蒸馏损失训练视觉模型。

Result: 在细粒度基准上优于MaKD等方法；与DKD、MLKD竞争或更优；在Waterbirds数据集上提升最差组准确率。

Conclusion: 语言到视觉蒸馏有效，无需图像数据即可提升视觉模型性能与鲁棒性。
...
### [33] [MemoBench: Benchmarking World Modeling in Dynamically Changing Environments](https://arxiv.org/abs/2606.27537)
...
TL;DR: 提出了一个评估视频生成模型在目标消失后重新出现时状态一致性的诊断基准MemoBench


<details>
  <summary>Details</summary>
Motivation: 现有评估多关注目标在视野内的记忆一致性，缺乏对目标消失后动态变化场景的测试

Method: 构建了包含360个合成与真实世界片段的基准，结合自动化指标和基于VQA的四个诊断支柱评估套件

Result: 对八个顶尖模型的评估揭示了在消失-重现范式下记忆一致性的关键见解和开放挑战

Conclusion: MemoBench填补了动态遮挡场景中内存一致性评估的空白，暴露了现有模型的不足
...
### [34] [Beyond MoCap: Scaling Motion Tokenizers with Synthetic Human Motion for Generative Modeling](https://arxiv.org/abs/2606.27547)
*Yiwen Yan,Wanning He,Yu-Wing Tai*

Main category: cs.CV

TL;DR: 提出利用合成动作数据扩展运动表示空间，结合改进的VQ-VAE分词器，显著提升文本到运动等任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有动作数据集多样性不足，主要包含常见重复动作，缺乏稀有、组合和动态运动，限制了模型的泛化能力。

Method: 构建大规模合成动作生成流水线，并与重新设计的VQ-VAE分词器结合，同时扩展训练分布和离散码本。

Result: 合成动作训练提升了运动词汇的覆盖率和组合性，在文本到运动和运动延续任务上取得一致改进，且与MotionGPT等框架兼容。

Conclusion: 运动生成的主要瓶颈在于表示空间的支持不足，而非模型架构；联合扩展合成动作与表示学习是实现更通用运动合成的有效路径。
...
### [35] [Understanding Cross-Rig Generalization in Automotive Perception: a Multi-Rig Benchmark and Rig Variation Metrics](https://arxiv.org/abs/2606.27554)
*Tim Alexander Bader,Tim Dieter Eberhardt,Maximilian Dillitzer,Wilhelm Stork*

Main category: cs.CV

TL;DR: 论文提出一个多视角相机配置基准，发现不同相机配置导致感知性能显著变化，并提出两个校准描述符来量化这种变化。


<details>
  <summary>Details</summary>
Motivation: 实际自动驾驶车队相机配置多样，而现有感知系统常基于固定传感器配置开发，存在跨配置泛化问题。

Method: 构建14种系统化相机配置的CARLA仿真基准，引入Rig Variance和Rig Contrastive Distance两个描述符。

Result: 几何配置差异与跨配置性能变化强相关，Rig Contrastive Distance可有效预测配置间迁移难度。

Conclusion: 相机几何配置是自动驾驶感知系统跨配置泛化的重要影响因素。
...
### [36] [Radar Guided Camera Verification for Automatic Emergency Braking Rethinking Object Detection in Radar Camera Fusion](https://arxiv.org/abs/2606.27556)
*Ram Charan Akula,Sivanathan Kandhasamy,Manikandan Ganesan*

Main category: cs.CV

TL;DR: 提出一种基于雷达引导的像素密度门控方法，无需训练数据即可快速验证障碍物，在真实车辆测试中减少98.7%相机搜索空间，实现零漏刹。


<details>
  <summary>Details</summary>
Motivation: 现有雷达相机融合AEB系统依赖计算密集型目标检测器，而雷达已定位目标后，相机仅需验证存在性而非完整识别，需降低计算开销。

Method: 采用雷达作用域内边缘密度门控，在雷达引导的图像感兴趣区域（ROI）中验证障碍物，无需训练数据、模型权值或GPU加速。

Result: 在72次驾驶和131,603帧中，ROI搜索空间减少98.7%，平均延迟0.121ms，AUC 0.898，召回率0.994；33次威胁场景中零漏刹。

Conclusion: 该方法高效且鲁棒，能够在低计算资源下实现可靠障碍物验证，适合实际AEB系统部署。
...
### [37] [Perceptual 3D Simulation With Physical World Modeling](https://arxiv.org/abs/2606.27575)
*Wanhee Lee,Klemen Kotar,Rahul Mysore Venkatesh,Jared Watrous,Daniel L. K. Yamins*

Main category: cs.CV

TL;DR: 提出了P3Sim系统，结合学习推理与几何结构，实现部分观测下的3D场景变换预测。


<details>
  <summary>Details</summary>
Motivation: 真实世界的感知输入和局部动作具有部分性和不完整性，而理想模拟器则具有完整3D几何和动态信息，因此需要弥合这一差距。

Method: 包含三个组件：学习到的物理世界模型、几何条件模块和持久场景记忆。世界模型将感知推理为多模态场景变量的概率分布；几何模块提供部分3D变换信号；记忆模块整合时间预测。

Result: 在新视角合成、物体操作和动态场景预测等多样化3D变换任务中实现泛化。

Conclusion: P3Sim通过结合数据驱灵活性性和归纳偏置，推进了通用3D场景理解与变换。
...
### [38] [DeLux: Cross-Modal Local Artifact Restoration in Video Using Neuromorphic Data](https://arxiv.org/abs/2606.27576)
*Bartosz Stachowiak,Dariusz Brzezinski*

Main category: cs.CV

TL;DR: 提出跨模态修复范式DeLux，利用事件流先验检测和修补RGB视频中的光照伪影，在合成和真实数据上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有RGB相机受光照伪影影响导致信息丢失，而现有方法孤立处理各类伪影，无法恢复被复杂空间退化完全遮挡的结构细节。

Method: 提出DeLux模块化流水线，以神经形态事件流作为结构先验，引导RGB视频中光照伪影的目标检测与修补。

Result: 平均MS-SSIM超过0.99，伪影严重程度降低88%，优于RGB基线和事件引导HDR模型。

Conclusion: DeLux有效抑制局部伪影并恢复受影响区域，开源工具和数据集促进跨模态修复研究。
...
### [39] [Distribution-based deep multiple instance learning for tumor proportion scoring in NSCLC](https://arxiv.org/abs/2606.27579)
*Krzysztof Pysz,Artur Bartczak,Jarosław Kwiecień,Piotr Krajewski,Witold Dyrka*

Main category: cs.CV

TL;DR: 提出基于零膨胀贝塔分布的多实例学习框架，仅利用切片级标签即可准确预测非小细胞肺癌的肿瘤比例评分。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要大量人工标注且专家稀少，而传统多实例学习难以处理无表达区域图像。

Method: 两阶段模型：补丁级特征提取与多分类网络，结合MIL聚合嵌入预测零膨胀贝塔分布参数。

Result: 零膨胀贝塔建模显著优于线性回归和岭回归，并通过分布集中度准确反映预测可靠性。

Conclusion: 该端到端框架在无需补丁级标签条件下提升了TPS预测精度与可解释性。
...
### [40] [Beyond Points: Spherical Distributional Part Prototypes for Interpretable Classification](https://arxiv.org/abs/2606.27582)
*Duarte Leão,Diogo Pereira Araújo,Catarina Barata,Carlos Santiago*

Main category: cs.CV

TL;DR: 提出vMFProto，一种基于von Mises-Fisher分布的分布原型框架，在超球面上以混合组件建模每个类别，结合熵式最优传输进行补丁到原型的分配，提高解释质量、稳定性和区分度。


<details>
  <summary>Details</summary>
Motivation: 现有原型网络在归一化方向嵌入空间中，点原型常出现冗余或不稳定，影响解释质量和鲁棒性。

Method: 采用von Mises-Fisher混合分布建模类别，每个原型学习自身浓度参数，使用熵式最优传输得到结构化分配，两阶段训练包含OT驱动原型发现和端到端精炼。

Result: 在CUB-200-2011、Stanford Dogs和Stanford Cars数据集上，使用冻结DINO骨干网络，vMFProto在解释一致性、稳定性和区分度上达到最优，准确性具有竞争力。

Conclusion: vMFProto能生成局部化、非冗余的部分证据，提升原型网络的解释性和鲁棒性。
...
### [41] [CoIn: Comprehensive 2D-3D Inpainting with Gaussian Splatting Guidance](https://arxiv.org/abs/2606.27584)
*Hana Kim,Minje Kim,Tae-Kyun Kim*

Main category: cs.CV

TL;DR: 提出CoIn框架，通过双向信息流（2D至3D再至2D）结合扩散模型与3D高斯泼溅，实现灵活遮挡修复与物体插入。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景修复方法依赖精确多视图分割蒙版且局限于物体移除，无法处理任意形状蒙版和物体插入任务。

Method: 多阶段一致性流水线：1）扩散模型生成初始修复图像；2）参考自适应高斯泼溅(Reference Adaptive GS with Feature Attention)将2D映射为粗糙3D场景；3）基于GS的参考特征扭曲(Reference Feature Warping)提供几何先验指导扩散模型确保多视图一致性；4）纹理增强鉴别器提升光度真实性。

Result: 在物体移除与插入任务中达到最优性能，支持灵活蒙版输入，有效处理任意形状遮挡。

Conclusion: 通过双向信息流融合2D修复与3D表示，CoIn在多样化场景修复任务中实现了更强的灵活性与质量。
...
### [42] [Dismantling Pathological Shortcuts: A Causal Framework for Faithful LVLM Decoding](https://arxiv.org/abs/2606.27596)
*Liu Yu,Can Chen,Ping Kuang,Zhikun Feng,Fan Zhou,Gillian Dobbie*

Main category: cs.CV

TL;DR: 本文提出Fox框架，通过识别并切断视觉注意力与语言先验之间的病理捷径，有效缓解LVLM中的对象幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法基于注意力强度假设，但未能解决LVLM中更深层的动态结构错位导致的幻觉问题。

Method: 无训练推理时框架，包括视觉注意力熵探针定位风险中介、数值logit饱和进行因果干预、冲突门控协同解码策略。

Result: Fox在多项基准上达到SOTA，比SID提升29.1%，同时保持语言丰富性。

Conclusion: 通过结构错位诊断和针对性干预，能够在不牺牲语言质量的前提下显著减少幻觉。
...
### [43] [Qwen-Image-2.0-RL Technical Report](https://arxiv.org/abs/2606.27608)
...
TL;DR: 提出了一个基于RLHF和on-policy distillation的后训练流程，显著提升了Qwen-Image-2.0扩散模型的图像质量和指令跟随能力。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成质量和指令遵循方面存在不足，需要一种有效的方法进一步提升性能。

Method: 构建了任务特定的组合奖励模型（涵盖对齐性、美观度、人像保真度等），开发了基于GRPO的RL训练框架（含混合CFG策略、组内奖励范围过滤和类别权重校准），并通过on-policy distillation整合多个专家策略。

Result: 在Qwen-Image-Bench上总分57.84（+2.61），文本到图像和图像编辑的Elo评分分别为1193（+78）和1349（+93）。

Conclusion: 该方法在美观度、提示遵循性和编辑准确性上取得了一致性提升，证明了RL和蒸馏策略的有效性。
...
### [44] [Explainable AI for Biodiversity Monitoring and Ecological Image Analysis](https://arxiv.org/abs/2606.27667)
*Brinnae Bent,Holly R. Houliston,Jiayi Zhou,Günel Aghakishiyeva,David W. Johnston*

Main category: cs.CV

TL;DR: 提出可解释人工智能（XAI）应成为生态计算机视觉模型验证的标准方法，通过案例展示其如何提升模型透明度与生态决策可靠性。


<details>
  <summary>Details</summary>
Motivation: 生态监测中计算机视觉模型常因难以解释而无法判断预测是否基于生态有效信号，可能因虚假关联或偏差导致保护决策失误。

Method: 阐述XAI在图像分类、目标检测、图像分割三类任务中的应用，通过海豹检测与鲸类解剖分割两个航空影像案例演示解释方法。

Result: 识别生物意义线索、揭示背景和形状干扰导致的假阳性、发现边缘和遮挡效应，并指导数据收集与模型优化。

Conclusion: XAI通过提升模型可解释性和科学可验证性，有助于确保AI支持的生态证据更可靠、可理解且可操作，对生物多样性保护具有关键价值。
...
TL;DR: 提出A3模块，通过激活缩放机制动态增强或削弱对抗扰动，提升神经网络的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法通过剪枝或掩码减少非鲁棒特征影响，而本文探索同时增强和减弱这些信号。

Method: A3模块使用可学习掩码和缩放因子动态调整激活值，通过翻转符号实现放大或衰减对抗扰动，并引入对比损失。

Result: 在多种骨干网络、数据集和训练方法上均明显提升对抗鲁棒性，且计算和内存开销极低。

Conclusion: 学习在放大模式下降低预测质量的同时，能有效提高衰减模式下的鲁棒性，A3是一种轻量高效插件。
...
### [46] [AI-Generated Image Recognition via Fusion of CNNs and Vision Transformers](https://arxiv.org/abs/2606.27637)
*Xuan-Bach Mai,Hoang-Minh Nguyen-Huu,Quoc-Nghia Nguyen,Hoang-Tung Vu,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 提出一种融合多种检测方法的鲁棒方法，在CIFAKE数据集上实现97.32%的准确率，用于区分AI生成图像与真实图像。


<details>
  <summary>Details</summary>
Motivation: 高质量合成数据模糊了真实与AI生成图像的界限，威胁数据可靠性，亟需鲁棒的检测方法。

Method: 采用融合策略，结合多种检测方法的优势来识别AI生成图像。

Result: 在CIFAKE数据集上达到97.32%的准确率。

Conclusion: 该方法有效区分AI生成图像与真实图像，推进了合成数据时代的数据认证技术。
...
### [47] [Denoising ICF Images with Multiplicative Uniform Noise: A Self-Supervised Study Based on the Log-Domain Noisier2Inverse Framework](https://arxiv.org/abs/2606.27635)
*Gyeongha Hwang,Bradley Thomas Wolfe,Naima Naheed*

Main category: cs.CV

TL;DR: 本文提出并评估了一种针对惯性约束聚变图像的自监督去噪框架（Log-Domain Noisier2Inverse），在乘性均匀噪声下通过对数域自监督损失等价于监督学习的理论，实现了显著的峰值信噪比提升（最高21.41 dB）。


<details>
  <summary>Details</summary>
Motivation: ICF图像受乘性均匀噪声严重退化，现有去噪方法性能有限，且缺乏自监督方案；需要一种无需干净数据即可训练的去噪框架。

Method: 基于Noisier2Inverse的对数域自监督去噪框架，理论证明最小化对数域自监督损失等价于变换域监督学习；并针对ICF图像特性设计了两种噪声加载变体（A:固定高斯噪声；B:每图像JSON均匀噪声）。

Result: Variant B实现PSNR 21.41 dB、SSIM 0.8358，较噪声基线（1.95 dB）提升19.46 dB，显著优于BM3D（4.47 dB）和Noise2Self（4.75 dB）。

Conclusion: Log-Domain Noisier2Inverse框架能有效处理ICF图像的乘性均匀噪声，自监督特性使其无需干净参考数据，性能大幅超越传统方法和无监督基线。
...
TL;DR: 提出了CascadeOcc，一种利用空间和时间双层次结构的占用世界模型，通过级联向量量化机制和TimeMixer实现从粗到细的预测，在4D占用预测和运动规划上取得了优于其他视觉方法的性能。
...
Method: 将级联向量量化机制集成到自回归框架中，采用多尺度架构从全局结构逐步细化局部细节，并加入TimeMixer捕获多尺度时间依赖关系。

Result: 在4D占用预测和运动规划基准上，CascadeOcc在视觉方法中取得了最优性能。

Conclusion: 优化内在表示可以成为依赖外部基础模型的有效替代方案。
...
TL;DR: 提出基于海马体记忆建模的Hippocampus-DETR目标检测框架，模拟海马体子区域实现模式分离、补全、过滤与整合功能，在检测精度、泛化能力和数据效率上优于主流模型。


<details>
  <summary>Details</summary>
Motivation: 当前目标检测模型缺乏显式记忆机制，而生物海马体在记忆处理中具有关键作用。

Method: 在DETR架构中嵌入HipNet模块，模拟内嗅皮层、齿状回、CA3、CA1和下托等海马子区结构，采用逐层训练优化不同记忆子模块。

Result: 检测精度优于现有主流模型，并在小样本分类、多模态特征构建、图像恢复等任务中展现优异泛化能力和数据效率。

Conclusion: Hippocampus-DETR不仅提供一个新的检测框架，还展示了神经认知机制与深度学习融合的可行路径，显著提升模型学习效率与任务鲁棒性。
...
TL;DR: 提出了SpatialUAV基准，包含4,331个实例和14种任务类型，用于评估低空无人机空间智能。


<details>
  <summary>Details</summary>
Motivation: 现有无人机基准缺乏对3D空间推理、多视角协作、场景动态和多样化任务格式的评估。

Method: 统一视觉-问题-答案模式，支持7种输入配置和9种答案格式，结合探测器辅助区域、深度监督、元数据规则和人工验证构建数据。

Result: 当前视觉语言模型在跨视角关联、结构化定位、几何推理和时间动态理解方面远低于人类水平。

Conclusion: SpatialUAV为推进低空无人机空间智能提供了实证指导。
...
### [51] [VLM-Aware Meta-Optic Front-End Design for Frozen Vision-Language Models](https://arxiv.org/abs/2606.27646)
*Chanik Kang,Raphaël Pestourie,Haejun Chung*

Main category: cs.CV

TL;DR: 提出CODA框架，通过端到端联合优化连续密度超构光学前端与固定视觉模型（CLIP），在受限光学条件下显著提升零样本识别准确率，无需传统图像重构或保真度目标。


<details>
  <summary>Details</summary>
Motivation: 传统机器视觉依赖高质量光学，但尺寸、成本受限场景下超构光学成像质量差，需探索直接优化光学端以适配下游识别任务。

Method: 基于可微成像与Maxwell仿真的伴随梯度更新，联合优化连续密度超构光学前端与冻结的CLIP分类器，直接最小化交叉熵损失。

Result: 在ImageNet-100上，零样本准确率从53.75%提升至65.41%，且优化后的光学系统无需调整即可泛化至不同视觉模型（CLIP、SigLIP、DINOv2）和数据集。

Conclusion: 在受限超构光学成像下，将光学设计与冻结视觉模型目标对齐（而非传统成像质量准则）可显著提升下游识别性能。
...
### [52] [Temporal-Emerged Prompting for Segment Anything in Multiframe Infrared Small Target Detection](https://arxiv.org/abs/2606.27655)
*Yinghui Xing,Donghao Chu,Shizhou Zhang,Di Xu*

Main category: cs.CV

TL;DR: 提出TEP-SAM框架，通过联合建模全局运动模式与局部运动偏差，利用时间涌现线索引导SAM模型，实现低信噪比红外序列中的小目标检测与分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法在低信噪比红外序列中难以检测小目标，因为单帧中目标与背景难以区分，但目标随时间逐渐显现。

Method: 构建TEP-SAM框架，联合建模全局运动模式和局部运动偏差定位目标，利用运动差异增强目标区域特征，生成时间涌现线索以非交互方式引导SAM分割。

Result: 在低信噪比和复杂动态背景下，TEP-SAM显著优于现有方法，实验验证其有效性。

Conclusion: 通过桥接大规模语义预训练与任务特定时间建模，TEP-SAM成功将SAM适配到多帧红外小目标检测任务。
...
TL;DR: GeoFace提出了一种几何约束的多视图扩散框架，用于从单张输入生成面部图像，通过联合生成多视图RGB图像和3D面部几何来解决跨视图不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有多视图扩散模型虽能生成逼真的逐视图图像，但缺乏显式机制强制跨视图共享3D结构，导致几何不一致。

Method: 提出双流框架，外观和几何流通过共享注意力层交互，并引入几何引导注意力对齐损失，利用3D一致对应关系监督外观与几何令牌的交叉注意力，使用正则UV位置图表示几何。

Result: 在RenderMe-360和NeRSemble数据集上，GeoFace在视觉质量和跨视图几何一致性上持续优于现有方法，促进了更高效的3D重建。

Conclusion: GeoFace通过几何约束有效提升了多视图生成的一致性，证明了该方法在3D重建中的实用性。
...
TL;DR: 提出了一种基于视频的被动式帕金森病转身步数估算框架，利用粗到细的运动表示和交叉注意力机制，在真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统穿戴式步数计数方法不便于日常使用，且帕金森患者的异常步态和转身变化导致步数准确计数困难。

Method: 通过3D人体网格恢复提取足部运动信号作为粗估计，结合光流和网格运动编码器学习细粒度步态动态，利用交叉注意力关注像素级运动线索，并通过多实例学习处理变长视频预测步数残差。

Result: 在真实帕金森转身数据集上，该方法一致优于现有步数计数方法。

Conclusion: 视频被动式估算帕金森患者转身步数是可行且有效的，为日常运动监测提供了便捷途径。
...
TL;DR: 提出MVPruner，一种两阶段自适应令牌剪枝方法，在保留98.5%准确率的同时，实现87.3%的FLOPs减少和4.97倍预填充加速。


<details>
  <summary>Details</summary>
Motivation: 多视图VLM中长视觉令牌序列导致效率低下，现有方法忽略视图间重要性差异和动态信息需求。

Method: 两阶段自适应剪枝：第一阶段根据视图信息多样性分配预算并保留一致性贡献令牌；第二阶段由指令文本指导分配预算和选择令牌。

Result: 在DriveLM基准上，DriveMM+MVPruner实现87.3% FLOPs减少，4.97倍预填充加速，准确率保持98.5%。

Conclusion: MVPruner有效对齐剪枝行为与模型动态信息需求，显著提升多视图VLM效率。
...
### [56] [PhysisForcing: Physics Reinforced World Simulator for Robotic Manipulation](https://arxiv.org/abs/2606.28128)
...
TL;DR: 提出PhysisForcing框架，通过轨迹和对齐损失强化物理一致性，提升视频生成与机器人操控性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在模拟物理操作时存在不连续轨迹和不一致交互等问题，限制了其作为世界模拟器的可靠性。

Method: PhysisForcing框架，联合优化像素级轨迹对齐损失和语义级关系对齐损失，利用参考点轨迹和冻结的视频理解编码器监督DiT特征。

Result: 在R-Bench等基准上显著提升基础模型性能，Cosmos3-Nano变体取得最佳总分；在WorldArena协议中闭环成功率从16.0%提升至24.0%。

Conclusion: 物理对齐的视频模型能生成更稳定的世界模拟，并增强下游机器人操控策略。
...
### [57] [Two-Stage Cross-Domain Cervical Abnormality Screening with Cytopathological Image Synthesis and Knowledge Distillation](https://arxiv.org/abs/2606.27678)
*Jincheng Li,Yuzhi He,Yihui Zhan,Xinmei Zhang,Yifei Sun,Zelin Liu,Lichi Zhang,Minye Shao,Lili Zhao*

Main category: cs.CV

TL;DR: 本文提出一个两阶段框架用于宫颈细胞病理图像的跨域检测，解决域偏移和类别模糊问题。


<details>
  <summary>Details</summary>
Motivation: 跨域诊断因机构间域偏移和疾病阶段视觉差异小，导致模型泛化能力差。

Method: 第一阶段用SC-UNSB建立合成中间域，第二阶段通过知识蒸馏中的双级特征对齐（浅层结构和深层语义）传递域不变知识。

Result: 该方法有效缓解域偏移和类别模糊，提升跨域检测性能。

Conclusion: 提出的两阶段策略能改善跨域细胞检测的泛化能力。
...
### [58] [Reflect-R1: Evidence-Driven Reflection for Self-Correction in Long Video Understanding](https://arxiv.org/abs/2606.27922)
*Shuimu Chen,Yuteng Chen,Yuanshen Guan,Zebang Cheng,Zeyu Zhang,Shengqian Qin,Bin Xia,Jiaran Li,Wenming Yang,Fei Ma*

Main category: cs.CV

TL;DR: 提出了首个基于证据驱动的长视频理解自校正框架Reflect-R1，通过直觉-验证-仲裁三阶段流程和阶段解耦强化学习算法SD-GRPO，显著提升了长视频理解的自我校正能力。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解的多模态反思机制依赖内部参数闭环自省，缺乏客观外部证据导致模型难以纠错，且强化学习在多阶段反思中存在严重的策略耦合和训练数据稀缺问题。

Method: 构建了三阶段流程（直觉、验证、仲裁），通过动态检索客观视觉证据验证初始直觉，并自主执行多次时间搜索化解冲突；设计了阶段解耦强化学习算法SD-GRPO，独立计算各推理阶段的优势函数；构建了包含120K样本的数据集弥补训练数据缺口。

Result: 在VideoMME和LongVideoBench等基准上达到最先进性能，显著提升了真实校正率，实现了严格基于客观证据的自主校正。

Conclusion: Reflect-R1通过证据驱动的自校正框架和阶段解耦训练策略，有效突破了长视频理解中的幻觉循环和策略耦合限制，验证了客观证据在自我反思中的关键作用。
...
### [59] [Multi-Modal Conditioned High-Resolution Transformer for Urban Electromagnetic Field Map Prediction Download PDF](https://arxiv.org/abs/2606.27671)
*Do-Eon Kim,Dongryul Park,Seungyoung Ahn,Namwoo Kang,Seong-heum Kim,Seongsin Kim*

Main category: cs.CV

TL;DR: 提出多条件密集预测框架生成城市电磁场图，使用HRFormer骨干网络和FiLM、交叉注意力机制，结合发射器相对空间通道和复合损失函数，在测试MAE上比UNet基线提升25.2%。


<details>
  <summary>Details</summary>
Motivation: 基于物理的模拟器预测城市电磁场强度计算昂贵，需高效替代方法。

Method: 采用HRFormer骨干网络，使用特征线性调制注入天线参数，交叉注意力融合辐射模式，引入发射器相对空间通道和坐标一致测试时增强，设计含掩码L1、MS-SSIM和焦点L1的复合损失函数。

Result: 最佳模型测试MAE为0.0461，比UNet提升25.2%，比仅HRFormer提升31.8%。

Conclusion: 提出的多条件框架显著提高了电磁场预测精度，验证了复合损失和空间通道的有效性。
...
TL;DR: 提出了ProMSA，一种渐进式多模态搜索代理，通过自适应选择图像搜索、文本搜索或停止，在KB-VQA上优于RAG和代理基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定的检索-生成流水线和静态top-k设置，缺乏推理中的自适应性。

Method: 使用渐进式搜索代理，结合工具调用预算和去重，通过拒绝采样SFT和TN-GSPO RL目标训练。

Result: 在E-VQA和InfoSeek上取得一致性提升，改进了检索和端到端准确率。

Conclusion: ProMSA通过自适应搜索和序列级强化学习有效提升了KB-VQA性能。
...
TL;DR: 提出了一个将单张参考图像转换为高质量3D资产的模块化系统Home3D 1.0，支持PBR材质和部件级编辑。


<details>
  <summary>Details</summary>
Motivation: 针对室内设计和电商应用，需从单张图像高效生成高质量、材质可编辑的3D模型。

Method: 采用四模块流水线：几何模块（隐式SDF与流匹配DiT）、纹理模块（多视图投影与3D纹理场）、材质模块（视频分割与库匹配PBR）、部件模块（PartVAE/PartDiT生成语义部件）。

Result: 各模块独立评估表明系统当前能力与部署差距。

Conclusion: Home3D 1.0初步实现了从图像到PBR材质3D模型的自动化生成，但仍需改进。
...
### [62] [Joint Transcription and Decryption of Images of Encrypted Handwritten Documents: A Comparison with the Traditional Pipeline](https://arxiv.org/abs/2606.27700)
*Marino Oliveros-Blanco,Lei Kang,Alicia Fornés,Beáta Megyesi*

Main category: cs.CV

TL;DR: 提出了一种端到端的直接图像解密方法，绕过转录阶段，从加密手稿图像直接映射到明文，在Copiale密码案例中展示了优于传统流水线的性能。


<details>
  <summary>Details</summary>
Motivation: 传统两步流水线（转录+解密）对转录错误敏感，误差会传播到最终输出，因此需要更鲁棒的端到端方法。

Method: 构建合成数据生成流水线，创建大规模类密码训练数据；采用联合图像到明文建模的端到端架构。

Result: 联合架构在Copiale密码案例中展现出比传统转录流水线更优的性能。

Conclusion: 端到端的图像到明文建模是传统基于转录的流水线的有前景的替代方案。
...
TL;DR: 研究首次系统评估扩散模型在真实跨传感器超分中的表现，发现合成训练模型在真实数据上性能退化，而真实数据训练存在优化困难。提出LPIPS-Sat指标。


<details>
  <summary>Details</summary>
Motivation: 高分辨率卫星图像需求增加，但缺乏真实配对的低-高分辨率观测数据，导致超分模型在跨传感器图像上存在合成到真实的领域差距。

Method: 使用Sentinel-2和PlanetScope的几何及时间对齐数据集，评估五种扩散架构，并引入基于Sentinel-2自监督特征的领域自适应感知指标LPIPS-Sat。

Result: 合成训练模型在真实配对数据上性能严重下降；真实数据训练的模型面临优化困难，难以适应物理和辐射多样性。

Conclusion: 当前超分方法存在关键局限，需要将超分与领域自适应解耦。
...
### [64] [MASS: Motion-Aligned Selective Scan for Refinement in Flow-Based Video Frame Interpolation](https://arxiv.org/abs/2606.27718)
*Jun-Sang Yoo,Seung-Won Jung*

Main category: cs.CV

TL;DR: 提出了一种基于运动对齐选择性扫描的视频帧插值框架，通过沿运动轨迹进行特征扫描和状态空间模型聚合，有效处理大位移和复杂遮挡。


<details>
  <summary>Details</summary>
Motivation: 现有基于流的VFI方法在模糊对应上表现不佳，而基于SSM的方法受限于静态网格扫描，与物理运动不对齐。

Method: 提出MASS框架，包括可学习非线性路径积分（残差速度更新）和速度感知SSM（动态调整采样预算和步长），并通过聚合状态指导端到端细化模块。

Result: 在标准基准上取得竞争性整体性能，特别在大位移和复杂动态场景中达到最先进水平。

Conclusion: MASS通过动态运动对齐扫描有效提升了VFI在挑战性场景下的表现。
...
### [65] [ZooClaw-FashionSigLIP2: Distilled Fine-tuning for Robust Fashion Retrieval](https://arxiv.org/abs/2606.27708)
*Siqiao Xue,Chunxue Xu*

Main category: cs.CV

TL;DR: 本文提出ZooClaw-FashionSigLIP2模型，通过全微调、知识蒸馏和权重插值解决了视觉语言编码器在时尚检索中泛化性与专业性之间的权衡，并开源了模型和基准。


<details>
  <summary>Details</summary>
Motivation: 基础视觉语言编码器在适应专门检索任务时面临泛化能力损失的问题，时尚检索是这一问题的典型例子。

Method: 使用全微调和知识蒸馏在域内数据上训练，再通过权重插值(base模型与微调模型)进行整合。

Result: 在所有基准测试中优于LoRA、更大模型(高达1B参数)和外部训练数据，并发布了高质量时尚检索基准ZooClaw-Fashion。

Conclusion: 简单的方法可以有效解决专业检索中的泛化-权衡问题，并提供了透明评估和开源资源以推动后续研究。
...
TL;DR: 提出并行自回归近似(PRA)框架，通过低维中间状态映射和并行教师训练，解决了像素空间连续token自回归生成中的高维误差和训练-推理差距问题。PRA-L模型在ImageNet-1K上取得FID=1.94的最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有像素空间连续token自回归生成存在高维补丁生成误差大、教师强制训练与推理差距导致误差累积的问题。

Method: 设计并行自回归近似(PRA)，生成低维中间状态后通过像素解码器映射回像素空间，并通过并行独立构建推理级像素输入来近似推理反馈。

Result: PRA-S(135M参数)在ImageNet-1K 256×256生成上FID=2.58，超越先前最优的3.60；PRA-L(511M参数)将FID提升至1.94，成为像素空间自回归模型新标杆。

Conclusion: PRA有效克服像素空间自回归生成的核心困难，在生成质量上达到新高度，并在分类准确率上超越其他自回归和扩散基线，展现出统一像素空间图像生成与理解的潜力。
...
### [67] [Scene and Human in One World: Reconstruction in a Feedforward Pass](https://arxiv.org/abs/2606.27720)
*Boao Shi,Qiao Feng,Yiming Huang,Lingjie Liu*

Main category: cs.CV

TL;DR: SHOW是一个可提示掩码的人体网格恢复框架，通过将前馈3D场景重建与人体网格恢复统一到度量空间中，实现从单目视频中重建对齐的度量尺度人体-场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法将人体网格恢复和场景重建视为独立任务，导致尺度模糊、人体-场景不对齐和遮挡干扰问题。

Method: 通过参数化人体模型向归一化点图预测注入语义和尺度先验实现度量场景重建，同时用恢复的场景几何约束人体网格估计，并引入可提示掩码机制处理复杂场景。

Result: 在具有挑战性的相机运动、遮挡和杂乱背景下，SHOW提升了度量尺度一致性、人体-场景对齐精度和重建准确性。

Conclusion: 人体-场景联合重建优于分离处理，SHOW通过双向信息传递实现更准确的度量尺度单目人体中心视频重建。
...
TL;DR: 提出STAG网络，通过联合建模光流与自适应面部连接，实现跨数据集的微表情识别，在多个基准上验证了鲁棒性与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖关键帧、忽略帧间细粒度动态、分离时空建模，导致跨数据集泛化受限。

Method: 采用双分支架构：增强图注意力网络进行空间推理，Transformer编码器进行时序建模；双向交叉注意力融合时空特征，AU引导动态连接调整面部区域交互。

Result: 在CASME II、4DME、DFME、NaME、SAMM、SMIC-HS上表现更优，提升了鲁棒性、泛化性、可解释性和计算效率。

Conclusion: 自适应关系推理、AU引导动态连接和深度时空特征融合有效提升跨数据集的微表情识别准确性。
...
### [69] [Learning 1-Bit LiDAR-based Localization with Auxiliary Objective](https://arxiv.org/abs/2606.27729)
*Kaijie Yin,Zhiyuan Zhang,Tian Gao,Wentao Zhu,Cheng-zhong Xu,Hui Kong*

Main category: cs.CV

TL;DR: 本文提出BiLoc，首个用于6自由度LiDAR定位的二值神经网络框架，通过信息瓶颈原理和辅助目标缓解二值化导致的信息丢失，在大型室外数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习定位方法计算资源消耗大，而二值神经网络虽轻量但性能下降严重，需解决信息丢失问题以实现高效定位。

Method: 从信息瓶颈原理重新解释二值神经网络训练，引入自适应调节信息保留的辅助目标，补偿二值化导致的表示能力不足和梯度不匹配。

Result: 在大型室外LiDAR数据集上，BiLoc在二值神经网络定位任务中达到最新最优性能。

Conclusion: BiLoc成功实现高效且高精度的二值LiDAR定位，为资源受限场景下的定位模块提供可行解决方案。
...
### [70] [SIFT: Self-Imagination Fine-Tuning for Physically Plausible Motion in Video Diffusion Models](https://arxiv.org/abs/2606.27741)
*Ruoyu Wang,Jialun Liu,Huayang Huang,Haibin Huang,Jiepeng Wang,Chi Zhang,Xuelong Li,Yu Wu*

Main category: cs.CV

TL;DR: 提出SIFT微调方法，通过自我想象生成训练数据替代直接重建，解决视频扩散模型中的运动纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型生成的视频常违反物理规律，存在运动纠缠问题（相机运动与物体运动意外耦合），源于数据偏差和重建训练设计。

Method: 自我想象微调（SIFT）：用模型自身生成视频训练，配合运动感知判别监督和渐进式困难案例重放策略。

Result: 显著提升物理真实性、运动解耦性和可控性。

Conclusion: SIFT有效打破重建捷径，通过自由生成文本提示覆盖广泛运动空间（含稀有场景），无需昂贵数据采集。
...
### [71] [OSOR: One-Step Diffusion Inpainting for Effect-Aware Object Removal](https://arxiv.org/abs/2606.28094)
*Qinming Zhou,Chenxi Sun,Deyang Kong,Junhao He,Xiangheng Tang,Peike Yu,Haotian Wu,Leilei Cao,Linfeng Zhang*

Main category: cs.CV

TL;DR: 提出了一种单步扩散模型OSOR，实现高效、效果感知且对不完美掩码鲁棒的目标移除。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型虽性能强但计算成本高，且难以处理目标对象的非局部效果（如阴影、反射）和用户提供的不准确掩码。

Method: 引入占用引导判别器实现单步扩散训练；提出alpha头利用预训练模型预测移除区域；设计语义锚定验证流水线（SAVP）筛选有效训练数据。

Result: 在感知质量上超越多步扩散基线，推理速度快4到30倍。

Conclusion: OSOR在效率、效果和鲁棒性上取得平衡，适用于交互式应用和边缘设备。
...
### [72] [Panoramic Scene Analysis: A Survey from Distortion-Aware Engineering to Sphere-Native Foundation Modeling](https://arxiv.org/abs/2606.27745)
*Qinfeng Zhu,Lei Fan*

Main category: cs.CV

TL;DR: 综述了全景场景分析从投影适应、畸变感知到球面原生建模及基础模型几何感知标记化的演进，揭示了技术发展中的几何承诺深化趋势，并指出了当前评估协议中的五个系统性缺口。


<details>
  <summary>Details</summary>
Motivation: 全景图像提供完整空间上下文，但其平面表示存在几何畸变，导致标准视觉架构失效。

Method: 沿架构设计和训练范式两个维度组织文献，涵盖密集预测、多任务理解、开放世界感知、视觉语言推理与动态视频分析。

Result: 识别出无方法同时实现严格球面等变性并完全复用透视预训练基础模型权重这一结构性问题。

Conclusion: 提出六点研究路线图，呼吁迈向通用全景智能，并公开相关代码仓库。
...
### [73] [BiDeMem: Bidirectional Degradation Memory for Explainable Image Restoration](https://arxiv.org/abs/2606.28112)
*Xinrui Wu,Lichen Huang*

Main category: cs.CV

TL;DR: 提出了BiDeMem，一种双向退化记忆模块，用于可解释的图像恢复，通过可验证的语义先验提升恢复质量。


<details>
  <summary>Details</summary>
Motivation: 现有退化感知方法仅以PSNR等单一指标评判，缺乏语义可解释性；条件增强可能只是利用数据捷径，而非真正的退化先验。

Method: 设计双向退化记忆，通过恢复特征和输入统计构建查询，检索top-k记忆槽；同一槽标识同时支持推理时的恢复路径和训练时的退化解释路径。

Result: 在NAFNet多退化控制实验中，BiDeMem优于校正头、密集查询先验和静态全局先验（分别低0.2588、0.2586、0.2839 dB）；强残差监督和宽退化头仍不及完整双向模型。

Conclusion: BiDeMem既作为恢复模块又作为可证伪的解释机制，在保持恢复质量的同时增强对错误先验和原始先验的敏感性。
...
TL;DR: 提出概念引导的上下文分割(CG-ICS)范式，通过提取高层语义概念和视觉示例提升分割鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有ICS方法忽略系统鲁棒性，即不同参考下同一查询的稳定性。

Method: 概念推理模块（MLLM候选+树搜索优化）+ 并行视觉示例路径，激活冻结SAM3骨干。

Result: 在标准基准上实现最先进精度，并显著降低不同参考下的方差。

Conclusion: CG-ICS通过语义概念提升准确性和鲁棒性，构建更可靠的ICS系统。
...
### [75] [PixelU: A U-Shaped Transformer for Efficient End-to-End Pixel Diffusion](https://arxiv.org/abs/2606.27760)
*Zipeng Guo,Lichen Ma,Yu He,Xiaolong Fu,Jingling Fu,Junshi Huang,Yan Li*

Main category: cs.CV

TL;DR: 本文提出PixelU，一种极简的单阶段U型扩散Transformer，通过零成本跳跃连接和空间降采样机制解耦高低频信号，以低成本实现优于基线模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有像素空间扩散模型依赖复杂解码器补偿速度预测的优化困难，本文发现这些解码器在干净数据范式下是冗余的。

Method: 采用U型结构，使用零成本跳跃连接作为信息高速公路传递未损坏的高频细节，并通过恒定通道空间降采样作为低通滤波器压缩深层特征。

Result: 在ImageNet 256×256和512×512上分别获得FID 1.63和1.92，计算成本仅为基线模型的约1/3。

Conclusion: 解耦频率的方式能高效替代复杂解码器，为端到端扩散模型建立简单而强大的新范式。
...
### [76] [ModaFlow: Modality-Aware Flow Matching for High-Fidelity Virtual Try-On](https://arxiv.org/abs/2606.27773)
*Xiangyu Sai,Meysam Madadi,Sergio Escalera,Yong Xu*

Main category: cs.CV

TL;DR: ModaFlow提出一种模态感知流匹配框架，通过视觉引导和文本控制实现高保真虚拟试穿，在配准和变形鲁棒性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在衣物-人体大变形下同时保留精细衣物语义并适应多样体型，且多模态条件处理不够精细。

Method: 提出模态感知引导方案（视觉嵌入稳定引导+文本自适应CFG）、两种正则化损失（余弦相似性和感知流判别）以及掩码操作策略。

Result: 在配对和未配对基准上FID分别降低约30%和20%，定性定量结果达到最优。

Conclusion: ModaFlow在不同场景下均能生成高保真虚拟试穿图像，验证了模态感知设计的有效性。
...
### [77] [TRUST: Efficient Abdominal Trauma Recognition via Image-to-Ultrasound-Video Transfer Learning](https://arxiv.org/abs/2606.27777)
*Enguang Wang,Hao Zhou,Shuo Gao,Tuo Liu,Guangquan Zhou*

Main category: cs.CV

TL;DR: 提出了TRUST框架，通过扫描感知的跨频率协作适配器、多粒度运动感知模块和视觉查询语义聚合模块，有效解决了超声视频分析中的时空变化和语义差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像到视频迁移学习方法在超声视频分析中受限于扫描依赖的时空变化和语义差异，缺乏泛化能力。

Method: 设计了跨频率协作适配器（CFCA）、多粒度运动感知模块（MGMA）和视觉查询语义聚合模块（VQSA）。

Result: 在内部超声创伤数据集上，TRUST比现有最优方法性能提升9.63%，且计算效率更高。

Conclusion: TRUST框架通过显式建模扫描变化，提升了超声视频理解的可靠性和泛化性。
...
### [78] [HAT-4D: Lifting Monocular Video for 4D Multi-Object Interactions via Human-Agent Collaboration](https://arxiv.org/abs/2606.28215)
...
TL;DR: 提出了首个智能体框架HAT-4D，从单目视频中重建多物体4D交互（几何、动态、物理交互），并构建了开放世界基准MVOIK-4D。


<details>
  <summary>Details</summary>
Motivation: 现有单目4D重建方法主要关注孤立物体，无法处理多物体交互中的严重遮挡和复杂动态，需高效数据采集来扩展具身AI。

Method: 结合视觉语言模型(VLMs)与多层次人机反馈机制，解决深度模糊和遮挡问题，实现3D生成和4D传播。

Result: 在多数评估指标上达到最先进性能，保持语义对齐竞争力；少量人工反馈可改善交互重建；生成数据可有效微调基线模型。

Conclusion: HAT-4D作为可扩展数据引擎，无需多相机系统即可生成物理合理的4D交互资产，为单目4D重建提供了新基准和评估协议。
...
TL;DR: 提出TopoTTA框架，通过拓扑数据分析中的持续同调增强测试时自适应，提升异常分割的结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法依赖像素级启发式规则，在噪声和纹理变化下难以保持结构一致性，且忽略异常图的高阶空间关系。

Method: 对异常分数图执行多级立方复形滤波，提取拓扑伪标签指导轻量测试时分类器，无需重新训练骨干模型。

Result: 在六个基准数据集上平均F1提升15%，尤其对复杂几何和结构变异异常效果显著。

Conclusion: 将拓扑推理融入测试时自适应可实现结构感知泛化，连接几何学习与稳健自适应。
...
TL;DR: 提出DEFAR框架，利用暴露偏差本身的动态信号进行自校正，包含抗漂移校正和频率补偿两个组件，在多个图像生成数据集上取得较好效果。


<details>
  <summary>Details</summary>
Motivation: Flow Matching模型在训练和推理之间存在暴露偏差，现有方法依赖静态约束或外部启发式，缺乏利用偏差自身信号的自校正能力。

Method: DEFAR框架包括：(1) 抗漂移校正，将推理时的偏差视为信号，学习将偏离状态引导回目标方向的主动自校正；(2) 频率补偿，利用偏差作为自反馈权重因子，强化高频噪声阶段缺失的低频成分。

Result: 在CIFAR-10、CelebA-64和ImageNet-256/512上，DEFAR优于现有基线，并展现出良好的可扩展性、兼容性和推理鲁棒性。

Conclusion: 暴露偏差本身包含可指导自校正的动态信号，DEFAR利用该信号能有效提升生成模型的性能与鲁棒性。
...
### [81] [Text as Illumination: Spatial Contrastive Retinex Learning for Language-guided Medical Image Segmentation](https://arxiv.org/abs/2606.27794)
*Jian Shi,Cheng Zhen,Pingping Zhang,Rui Xu,Yanan Lv,Yili Ma,Huan Bi,Haojie Li,Huchuan Lu*

Main category: cs.CV

TL;DR: 提出一种基于Retinex理论的文本引导医学图像分割网络TIRNet，通过将文本嵌入视为语义光照来调制特征，解决跨模态语义不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有LMIS方法缺乏显式细粒度约束，导致文本与分割输出之间的语义不匹配。

Method: 引入Retinex启发式文本调制块(RTMB)和一致细节补偿块(CDCB)，并提出多尺度光照监督损失(MSIS-Loss)，包括区域对比损失(RGC-Loss)和背景抑制损失(BS-Loss)。

Result: 在MosMedData+和QaTa-COV19数据集上达到最先进性能。

Conclusion: TIRNet通过显式光照调制和细粒度约束有效提升语义一致性，代码开源。
...
### [82] [MindFlow: Harmonizing Cognitive Semantics and Acoustic Dynamics for Facial Animation Generation in Dyadic Conversations](https://arxiv.org/abs/2606.27779)
*Hejia Chen,Haoxian Zhang,Xu He,Xiaoqiang Liu,Pengfei Wan,Shoulong Zhang,Shuai Li*

Main category: cs.CV

TL;DR: 提出MindFlow双通路生成框架，受神经科学启发的腹侧-背侧通路模型，用于生成逼真对话面部动画，在语义理解与动态控制上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时理解对话语义与实现精细动态控制，受限于句子级建模和低精度运动生成。

Method: 腹侧模块采用Chunk-State方法将原始声流建模为情感状态链；背侧模块使用条件自回归流匹配网络，配合选择性声学注入器实现自适应音频门控。

Result: 在语义恰当性和运动自然性上优于当前最优基线方法。

Conclusion: 双通路解耦策略有效平衡了高层认知推理与低层运动控制，适用于复杂对话场景的面部动画生成。
...
TL;DR: 提出了一种内容感知的推测解码算法CSD，通过熵驱动的概率松弛和最优重采样策略，在保持生成质量的同时加速自回归图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码在图像生成中面临接受率低和直接放宽准则导致质量下降的问题。

Method: 集成基于熵的概率松弛机制和最优重采样策略，以及分布对齐滤波器。

Result: 在Lumina-mGPT和Janus-Pro上的实验证明了CSD的优越性。

Conclusion: CSD能有效提升自回归图像生成的推理效率，同时保持生成质量。
...
### [84] [Video-MME-Logical: A Controlled Diagnostic Benchmark for Video Temporal-Logical Reasoning](https://arxiv.org/abs/2606.27828)
*Hohin Kwan,Hongyu Li,Ray Zhang,Manyuan Zhang,Xianghao Kong,Anyi Rao,Jiahao Xie,Si Liu*

Main category: cs.CV

TL;DR: 本文提出Video-MME-Logical基准，用于测试多模态大语言模型在视频时序逻辑推理中的能力，发现即使经过微调，模型仍与人类存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频基准混淆了时序逻辑推理与场景识别能力，缺乏对推理过程的独立评估。

Method: 设计包含五种时序逻辑操作（状态跟踪、顺序计数、时序排序、动态空间性、结构组合）的25个细粒度任务，通过控制时间跨度和推理复杂度进行评测。

Result: 顶级MLLM在复杂任务中表现不佳，微调500K样本后仍无法缩小与人类的推理差距。

Conclusion: Video-MME-Logical可作为分析和改进MLLM时序逻辑推理能力的可扩展测试平台。
...
### [85] [Scalable and Differentiable Point-Cloud Registration Using Maximum Mean Discrepancy](https://arxiv.org/abs/2606.27818)
*Rixon Crane,Fahira Afzal Maken,Nicholas Lawrance,Stanislav Funiak,Kasra Khosoussi,Ming Xu,Russell Tsuchida*

Main category: cs.CV

TL;DR: MMD-Reg是一种无需对应点的点云配准方法，基于最大均值差异（MMD）和非线性最小二乘优化，具有线性计算复杂度且可微分。


<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法在初始对齐不良、部分重叠等挑战下性能受限，且缺乏可微分的端到端学习框架。

Method: 利用随机傅里叶特征近似MMD，构建可微分的非线性最小二乘目标函数，通过Levenberg-Marquardt求解，并采用隐函数定理实现微分。

Result: 在监督和无监督设置下，集成至集合转换器的神经网络变体在挑战场景中优于近期学习方法；独立MMD-Reg在精度和可扩展性上胜过传统非学习方法。

Conclusion: MMD-Reg提供了一种高效、可微分的点云配准方案，适用于端到端学习，尤其擅长处理初始对齐差和部分重叠问题。
...
### [86] [Long-Term Prediction of Local and Global Human Motion with Occlusion Recovery](https://arxiv.org/abs/2606.27900)
*Qiaoyue Yang,Sven Heutger,Christopher Niemann,Magnus Jung,Ayoub Al-Hamadi,Sven Wachsmuth*

Main category: cs.CV

TL;DR: 提出了基于非自回归Transformer的时空注意力模型，用于预测局部人体姿态和全局运动，并能处理缺失关节和变长历史观测。


<details>
  <summary>Details</summary>
Motivation: 现有自回归方法存在误差累积，仅关注局部姿态预测，限制了实际应用。

Method: 非自回归Transformer，结合时空注意力，同时进行局部姿态预测和全局运动预测，并训练恢复遮挡缺失关节。

Result: 模型能有效预测局部和全局人体运动，处理遮挡和变长输入。

Conclusion: 该方法增强了人体运动预测在真实场景中的适用性。
...
### [87] [OrthoTryOn: Geometric Orthogonalization for Conflict-Free Unified Fashion Generation](https://arxiv.org/abs/2606.27880)
*Zhaotong Yang,Ying Tai,Jiahui Zhan,Yu Zheng,Jianjun Qian,Jian Yang*

Main category: cs.CV

TL;DR: 提出了OrthoTryOn，一个统一框架，通过正交子空间投影和Fisher引导的负向引导，缓解多任务梯度冲突，在多个时尚生成基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 统一时尚生成模型因语义不同任务间的参数共享导致梯度冲突和负向迁移，性能显著下降。

Method: 在共享LoRA模块中使用正交子空间投影解耦瓶颈特征，并利用Fisher信息矩阵的负向引导机制在推理时减少语义耦合。

Result: 避免性能退化，超越独立训练的任务特定模型，在多项基准和扩散骨干网络上达到最优结果。

Conclusion: OrthoTryOn能有效缓解任务间干扰，实现高效的统一时尚生成，且代码已开源。
...
### [88] [Controllable Histopathology Image Synthesis with Training-free Structural Initialization and Textural Modulation](https://arxiv.org/abs/2606.27935)
*Yuheng Qiu,Jingyi Luo,Chenfei Ye,Ting Ma,Jianfeng Cao*

Main category: cs.CV

TL;DR: 提出CHIS框架，通过结构初始化与纹理调制，使无标签预训练扩散模型生成与结构掩码对齐的病理图像。


<details>
  <summary>Details</summary>
Motivation: 减少深度学习在病理图像分析中对大量标注数据的依赖。

Method: 频域中融合先验掩码相位与高斯噪声振幅进行结构初始化，逆扩散过程中小波分解自适应调制纹理。

Result: 在生成保真度和下游分割任务中表现优异。

Conclusion: CHIS能有效利用无标签数据生成高质量病理图像，降低标注需求。
...
### [89] [ScaLe-INR: Scale and Learn Implicit Neural Representations](https://arxiv.org/abs/2606.27862)
...
TL;DR: 提出了一种名为ScaLe-INR的多分支网络架构，通过方向坐标扩展和边缘引导损失，解决了隐式神经表示中的频谱偏差和信息串扰问题，在图像、音频和3D重建任务上超越了现有最优方法。


<details>
  <summary>Details</summary>
Motivation: 隐式神经表示在建模多尺度信号时，高频更新会干扰低频近似，导致频谱偏差和信息串扰，现有方法效果有限。

Method: 基于傅里叶逆缩放定理，引入方向坐标扩展拓宽网络带宽；提出方向边缘引导损失，利用梯度先验实现功能解缠，抑制分支间信息泄漏。

Result: 图像重建提升+5.16 dB，图像去噪提升+0.65 dB；音频重建达50.02 dB，3D重建IOU达0.999，均超越所有最优模型。

Conclusion: ScaLe-INR有效消除频谱串扰并加速收敛，在多尺度复杂拓扑信号重建中实现高保真度，性能显著优于现有方法。
...
### [90] [There and Back Again: A Flexible-Frame Transformer for Multi-Exposure Fusion](https://arxiv.org/abs/2606.27905)
*Lishen Qu,Yao Liu,Shihao Zhou,Jie Liang,Hui Zeng,Lei Zhang,Jufeng Yang*

Main category: cs.CV

TL;DR: 提出首个灵活帧数Transformer（FreeMEF），通过循环状态空间模块和全局特征引导块实现任意数量曝光帧的自适应融合，在三个数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统MEF方法需为不同帧数固定模型，导致部署效率低下。

Method: 采用循环状态空间模块（RSSM）进行任意长度序列的渐进融合，结合全局特征引导块（GFGB）的极值感知混合注意力（EAHA）和仿射注入前馈网络（AFFN）。

Result: 在三个基准数据集上定量和定性均优于现有方法。

Conclusion: FreeMEF实现了无需重训练或架构调整即可处理可变帧数输入的高效MEF框架。
...
### [91] [A Unified Framework for Vision Transformers Equivariant to Discrete Subgroups of $\mathrm{O}(2)$](https://arxiv.org/abs/2606.27864)
*Tīkun Ông,Georg Bökman*

Main category: cs.CV

TL;DR: 提出了一种对O(2)任意离散子群等变的视觉Transformer家族，统一了先前的翻转和D_4等变架构，并引入D_6等变模型，在数据稀缺时提升了识别准确率。


<details>
  <summary>Details</summary>
Motivation: 标准Vision Transformer未明确编码视觉任务中常见的平面对称性，限制了其在有限数据下的泛化能力。

Method: 通过群论构造等变Transformer组件，包括等变自注意力机制和同质空间配置，并给出了表达性保证；基于六边形块构建了D_6等变模型。

Result: 在PatternNet数据集上，等变模型在数据稀缺情况下比非等变基线提高了识别准确率；不同等变子群和注意力机制的效果被对比分析。

Conclusion: 离散对称群等变性可提升视觉Transformer的性能，值得进一步研究其在视觉识别中的作用。
...
### [92] [A Multi-Attribute Latent Space for Visual Analysis of Watches](https://arxiv.org/abs/2606.27897)
*Kai Lawonn,Tobias Günther,Monique Meuschke*

Main category: cs.CV

TL;DR: 提出了一种设计原理、嵌入模型和交互式视觉分析系统，用于通过异构视觉和语义属性探索大型手表收藏，解决了目录和电商界面在开放探索视觉相似性、风格替代和混合美学功能标准方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有目录和电商界面仅支持元数据过滤，缺乏对视觉相似性、风格替代和混合美学功能标准的开放探索支持。

Method: 使用U-Net分割表盘，Vision Transformer预测手表类型，CIELAB参考调色板表示颜色，梯度图像描述符描述表盘结构；扩展UMAP，结合属性特定邻域图于统一概率目标，并添加类感知布局项。

Result: 参数分析、运行时测量和定性试点研究显示系统支持发现和比较，但在可扩展性评估、按例搜索验证和更广泛领域研究方面存在局限。

Conclusion: 系统支持发现和比较，但需要解决可扩展性评估、按例搜索验证等问题，并得出多属性潜在空间可视化在异构视觉集合中的设计意义。
...
### [93] [Latent Visual Diffusion Reasoning with Monte Carlo Tree Search](https://arxiv.org/abs/2606.27988)
*Xirui Teng,Nan Xi,Junsong Yuan*

Main category: cs.CV

TL;DR: 提出LVDR框架，通过关键点引导的蒙特卡洛树搜索建模和可视化潜在视觉推理过程，提升动作质量评估的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有动作质量评估模型是黑盒，缺乏显式推理过程，限制了可解释性和可信度。

Method: 结合关键点引导的蒙特卡洛树搜索（MCTS）与潜在视觉扩散推理，生成可视化推理序列。

Result: 在四个数据集（体育和手术领域）上，LVDR在保持竞争性量化性能的同时，提供可解释的视觉推理轨迹。

Conclusion: LVDR成功实现可解释的动作质量评估，揭示关键推理步骤，平衡性能与透明度。
...
### [94] [Understanding How MLLMs Describe Artworks Using Token Activation Maps](https://arxiv.org/abs/2606.27947)
*Nicola Fanelli,Pasquale De Marinis,Raffaele Scaringi,Eva Cetinic,Gennaro Vessio,Giovanna Castellano*

Main category: cs.CV

TL;DR: 本文使用Token激活图（TAM）研究多模态大语言模型在描述艺术品时的视觉归因模式，发现视觉归因随词义变化，并比较了TAM与SAM~3分割方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型能流畅描述艺术品，但其视觉推理过程不透明，难以判断模型是基于画作具体区域、整体视觉信号还是文本先验生成描述。

Method: 采用Token激活图（TAM）为每个生成词生成热力图，排除上下文干扰，分析五类语义词（常见视觉对象、风格描述、元数据、图像符号、情感表达）的归因模式。

Result: 视觉归因随词义变化显著；模型在艺术家归属上比标题预测更准确，标题预测中幻觉更多；TAM与SAM~3开源分割方法对比。

Conclusion: MLLM的视觉归因与词义密切相关，且模型存在归因偏差和幻觉问题，TAM为分析模型行为提供了有效工具。
...
### [95] [Directing the World: Fast Autoregressive Video Generation with Compositional Human-Camera Control](https://arxiv.org/abs/2606.27964)
*Haoyuan Wang,Yabo Chen,Haibin Huang,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: 提出Directing the World框架，通过解耦控制学习与统一自回归视频先验，实现长时序、高质量的可控世界模型视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频生成在长时序推演中存在误差累积与时间退化问题，且在异构控制（如人体动作与相机轨迹）下，控制性与视觉质量难以兼得。

Method: 提出快慢记忆训练策略稳定长时序学习；设计t引导动态投影机制与细化运动CFG策略实现精确人体动作控制；引入第二阶相机轨迹控制模块与解耦训练数据集。

Result: 实验表明方法在长时间生成中保持精确控制与高视觉质量。

Conclusion: 通过解耦控制与统一先验，有效克服了长时序视频生成中控制与质量的权衡问题。
...
### [96] [Curriculum-guided Change Detection Training: Toward Accurate Serac Fall Monitoring](https://arxiv.org/abs/2606.28012)
*Arthur Dérédel,Carlos Crispim-Junior,Pierre Lemaire,Johan Berthet,Laure Tougne Rodet*

Main category: cs.CV

TL;DR: 提出了一种基于课程学习的遥感变化检测训练框架，通过SAG和SSIM两种难度度量逐步引入难样本，显著提升了模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有变化检测方法普遍采用均匀采样训练，忽略了训练样本的差异，导致噪声梯度和表示学习不足。

Method: 引入物理驱动的太阳角度差距（SAG）和结构相似度（SSIM）作为样本难度指标，构建课程学习框架，从易到难逐步训练。

Result: 在SeracFallDet基准上，该方法在像素级和物体级方法中均优于标准均匀采样策略。

Conclusion: 课程学习能有效提升深度变化检测的鲁棒性，且框架与现有架构正交，易于推广。
...
### [97] [HumanMoveVQA: Can Video MLLMs reason about human movement in videos?](https://arxiv.org/abs/2606.27999)
*Pulkit Gera,Faegheh Sardari,Asmar Nadeem,Valentina Bono,Padraig Boulton,Adrian Hilton,Armin Mustafa*

Main category: cs.CV

TL;DR: 提出了HumanMoveVQA基准，用于评估MLLM在全局人体运动轨迹和方向推理上的能力，发现现有模型存在严重缺陷，并通过微调开源模型实现显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在理解复杂人体运动时退化到粗语义标签，缺乏对全局时空轨迹和方向变化的推理能力。

Method: 构建了第一帧锚定的世界坐标系，设计多阶段管道将2D视频升维到3D运动轨迹，生成了超过10K的七类问答对。

Result: 专有模型在人体运动深度理解上存在关键能力缺口，但通过世界一致的监督微调开源模型可实现显著提升。

Conclusion: HumanMoveVQA为发展运动感知的视频理解模型建立了严格的几何基础。
...
TL;DR: 提出了TempAct框架，通过规划器-执行器的强化学习，解决了自回归视频扩散模型中时序指令遵循的模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型由于分块生成，单个全局提示无法指定各块的子事件，逐步提示又存在延迟反应、语义混淆和错误传播等问题。

Method: 使用LLM规划器探索可执行的跨度感知步骤提示，并训练自回归扩散执行器在这些提示下生成；核心是层级组探索机制实现计划和执行级别的信用分配。

Result: 在Self-Forcing和LongLive数据集上，TempAct改善了时序一致性，同时保持了整体视觉质量。

Conclusion: 强化学习框架能有效解决分块视频生成中的时序指令模糊问题，优于监督微调或蒸馏方法。
...
TL;DR: 提出了EMOSH框架，通过解耦形状与姿态参数实现高保真、可控的人体视频生成，解决运动形状纠缠问题，同时保持表情和手势的细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 现有2D方法存在运动形状纠缠导致体型泄露，而3D方法虽几何解耦但难以捕捉表情和复杂手势，生成动画僵化。

Method: 引入表达性人体模型（EHM）作为核心控制表示，设计鲁棒运动跟踪器估计EHM参数，提出粗到细混合运动注入策略和空间对齐调节机制。

Result: 在自驱动和跨驱动场景中均优于前人方法，生成高保真视频且保持形状解耦与生动表情。

Conclusion: EMOSH有效平衡了表达性与解耦性，为可控动画生成提供了新方案。
...
### [100] [ReScene: Structured Indoor Scene Reconstruction from Multi-View Captures](https://arxiv.org/abs/2606.28060)
*Haoran Xu,Lechao Zhang,Daoguo Dong,Yan Gao,Xin Tan*

Main category: cs.CV

TL;DR: ReScene是一个从多视角图像构建物理一致3D场景的框架，通过层次化视图选择和关系感知组装实现SOTA性能，并加速10倍。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多视角3D场景构建中存在跨视角关系融合不足和物理一致性差的问题。

Method: 提出HierView（基于语义一致性和3D覆盖的视图优先级排序）和Relation-Aware Assembly（融合多帧关系预测和图谱的物理一致组装）。

Result: 在ScanNet上CD降低17%，LPIPS降低26%，速度比多视角方法快10倍，生成的数据集微调Qwen-VL后接近强模型性能。

Conclusion: 多视角几何作为统一先验可显著提升场景重建的物理合理性和效率。
...
### [101] [TextDS: Parameter-Efficient Representation Alignment for Scene Text Detection under Distribution Shifts](https://arxiv.org/abs/2606.28077)
*Boyuan Chen,Zichen Dang,Chuang Yang,Lap-pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: 提出了TextDS框架，仅用4.9M可训练参数，在场景文本检测中跨域和恶劣成像条件下实现鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 现有场景文本检测器在分布偏移下性能有限，且依赖大规模预训练，跨域变化和成像退化评估不足。

Method: 采用双编码器设计结合视觉基础模型，无需大规模场景文本预训练；提出逐步LoRA适应（SWLoRA）和公共子空间融合（CSF）来对齐融合分支信息。

Result: 在场景文本检测中表现竞争力强，跨域和恶劣成像条件下鲁棒，仅需4.9M可训练参数。

Conclusion: TextDS有效应对分布偏移，减少参数依赖，并填补了成像退化评估的空白。
...
### [102] [AirGroundBench: Probing Spatial Intelligence in Multimodal Large Models under Heterogeneous Multi-View Embodied Collaboration](https://arxiv.org/abs/2606.28049)
*Haotian Li,Yida Wang,Leyuan Wang,Jinshan Lai,Keyang Wang,Zonghao Guo,Qiang Ma,Liuyu Xiang,Jianwei Hu,Zhaofeng He*

Main category: cs.CV

TL;DR: 提出了AirGroundBench基准，用于评估多模态大模型在异构无人机-地面车协作中的多视角空间智能，发现模型在跨视角对齐和空间变换推理上存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准多聚焦单智能体单视角感知，缺乏对无人机-地面车协作中多尺度、多视角空间一致性的系统评估。

Method: 构建11个高保真仿真环境，生成约62,000个双视图问答实例和115个导航情节，涵盖10种任务类型，并按空间感知、跨视角对齐、空间变换与推理、具身决策四个维度组织。

Result: 13个MLLM在双视图输入下虽优于单视图，但在跨视角对齐和变换推理上表现薄弱，且缺陷会传播至顺序决策任务。

Conclusion: 几何一致性是当前具身MLLM的关键瓶颈，与人类性能仍有差距。
...
### [103] [Diffusion Model Attribution via Spectral Coupling of Denoiser Responses](https://arxiv.org/abs/2606.28092)
*Pragati Shuddhodhan Meshram,Varun Chandrasekaran*

Main category: cs.CV

TL;DR: 提出频谱去噪签名（SDS）方法，通过指纹提取扩散模型的去噪行为特征，实现99.9%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有非侵入式方法在架构相似或共享自编码器时失效，需要可靠的模型归属方法。

Method: 利用模型去噪评分函数的频谱几何特性，通过频率控制扰动提取稳定签名。

Result: 在8种扩散模型上达到99.9%准确率，跨域提示偏移下仍达96.2%。

Conclusion: 频谱几何可成为扩散模型归属的可靠基础。
...
### [104] [EchoSonar-R: A Multi-View Reasoning-Enabled Model for Disease Classification and Report Generation in Echocardiography](https://arxiv.org/abs/2606.28164)
*Darya Taratynova,Ahmed Aly,Numan Saeed,Mohammad Yaqub*

Main category: cs.CV

TL;DR: 提出了EchoSonar-R，一种多视角推理视觉语言模型，能够联合进行多标签疾病分类和报告生成，通过结构化解剖线索和强化学习优化，在多个数据集上提升了分类准确率和临床可信度。


<details>
  <summary>Details</summary>
Motivation: 现有超声心动图分类模型缺乏显式诊断推理和空间解剖证据，限制了临床医生的信任。

Method: 结合时空视频编码器和结构感知心脏检测器，采用两阶段训练：监督微调（SFT）和基于GRPO的统一强化学习框架。

Result: 在私有数据集上宏平衡准确率提升17.1%，在MIMICEchoQA上提升6.1%，GREEN临床忠实度得分达0.800，生成了基于多视角视觉证据的可解释推理轨迹。

Conclusion: EchoSonar-R在提高分类性能和生成可解释推理方面表现优异，增强了模型在临床环境中的可信度。
...
### [105] [RPM-Distill: Physiology-guided Adaptive Cross-modal Distillation for Robust Remote Physiological Measurement](https://arxiv.org/abs/2606.28089)
*Jiyao Wang,Qingyong Hu,Duoxun Tang,Xiao Yang,Kaishun Wu,Jiangbo Yu*

Main category: cs.CV

TL;DR: 提出了一种基于生理引导的跨模态蒸馏框架RPM-Distill，在训练时利用雷达信号辅助，推理时仅需视频，通过频域知识迁移和元学习策略，有效提升了视频远程生理测量的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频远程生理测量在光照、肤色和运动变化下脆弱，而雷达信号虽鲁棒但部署成本高，因此希望利用雷达训练时的辅助信息，同时保持推理时仅使用视频。

Method: 通过频域蒸馏损失（锚定基频、匹配背景分布、保持谱形态与锐度）和元学习策略的频谱策略网络（根据师生频谱关系图动态调整蒸馏门控与权重），避免负迁移。

Result: 在挑战性条件和跨数据集实验中，MAE降低81%，相关性提高21%，优于单模态基线。

Conclusion: RPM-Distill通过跨模态知识蒸馏有效结合了雷达的鲁棒性与视频的易用性，显著提升了远程生理测量的性能。
...
### [106] [Cross-view Multimodal Vision-Based Assessment Framework for Traditional Chinese Medicine Rehabilitation Training](https://arxiv.org/abs/2606.28104)
*Francis Xiatian Zhang,Hao Yao,Shengxuan Chen,Hong Zhu,Hongxiao Jia,Sisi Zheng,Hubert P. H. Shum*

Main category: cs.CV

TL;DR: 提出CME-AQA多视角多模态动作质量评估框架，融合视觉-姿态数据，用于中医康复训练中的复杂手部动作评估，在两数据集上取得超过10%的加权F1提升。


<details>
  <summary>Details</summary>
Motivation: 现有单视角骨架方法无法有效评估中医针灸、推拿等含密集手部自遮挡和手物交互的动作，需更鲁棒的跨视角方案。

Method: 构建CME-AQA框架，整合第一人称和第三人称视频进行视觉-姿态融合训练，增强对环境和自遮挡的理解。

Result: 在针灸推拿双视角数据集上，关键评分任务（如针深、快速进针）加权F1相对提升超10%，插入时间等定量误差降低；在CPR数据集上表现相当。

Conclusion: CME-AQA能提升结构化中医康复训练评估的准确性，适用于以参与者动作为中心的临床技能评估。
...
### [107] [StructSplat: Generalizable 3D Gaussian Splatting from Uncalibrated Sparse Views](https://arxiv.org/abs/2606.28321)
*Jia-Chen Zhao,Beiqi Chen,Xinyang Chen,Guangcong Wang,Liqiang Nie*

Main category: cs.CV

TL;DR: 提出StructSplat，一个无需相机参数、前馈可泛化的3D高斯重建框架，通过结构化表示分离几何、语义和纹理，显著提升重建质量与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖逐场景优化，要么假设已知相机位姿，且常将几何与外观耦合在统一骨干网络中，限制了重建保真度和泛化性。

Method: 引入像素对齐特征注入机制以准确建模纹理，利用语义感知先验增强全局一致性，设计相机对齐策略防止信息泄露并提升泛化能力。

Result: 在DL3DV上PSNR达28.045，超过AnySplat 5.67 dB；跨数据集评估中，在ACID和RealEstate10K上分别超过AnySplat 1.94 dB和1.72 dB。

Conclusion: StructSplat在无相机参数条件下显著优于先前方法，验证了结构化表示在3D高斯重建中的有效性。
...
### [108] [Monocular Avatar Reconstruction via Cascaded Diffusion Priors and UV-Space Differentiable Shading](https://arxiv.org/abs/2606.28144)
...
TL;DR: 提出一种数据高效的框架，利用统一预训练扩散先验和级联LoRA，从单张野外图像重建高保真、可重照明3D化身，仅需不足100个真实3D扫描即可生成4K分辨率PBR资产。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于高质量PBR数据稀缺和光照与材质解缠的复杂性，且依赖碎片化流水线或大量专有数据集。

Method: 使用级联LoRA在UV空间处理子任务：Inpainting LoRA补全纹理，Light-Homogenization LoRA和Cross-Intrinsic Attention去除光照并合成PBR贴图，配合UV空间可微BRDF渲染损失约束物理合理性。

Result: 在少于100个真实3D扫描上训练，生成全面4K分辨率PBR资产，优于现有方法，表现出卓越真实感和泛化能力，代码和权重将开源。

Conclusion: 该方法通过扩散先验和级联LoRA高效解决纹理补全、去光照和材质分解，物理约束保障结果质量，为单图像3D化身重建提供新路径。
...
### [109] [RSICCLLM: A Multimodal Large Language Model for Remote Sensing Image Change Captioning](https://arxiv.org/abs/2606.28266)
*Yelin Wang,Zijia Song,Shuo Ye,Chuanguang Yang,Miaoyu Wang,Yong Xu,Zhulin An,Yongjun Xu,Zitong Yu*

Main category: cs.CV

TL;DR: 提出RSICCLLM，首个基于大型视觉语言模型的遥感图像变化描述后训练框架，通过数据生成、差感知微调和双负样本偏好优化，仅7B参数即超越更大模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于传统深度学习架构的模型容量，而大模型后训练技术因数据稀缺和细粒度变化理解需求难以直接迁移至RSICC任务。

Method: 设计数据生成范式并发布指令数据集RSICI，提出差感知监督微调显式提取变化特征，以及双负样本偏好优化（DNPO）利用互补负样本构建偏好数据集RSICP。

Result: RSICCLLM在7B参数规模下取得显著优越性能，超越更大规模模型，实验验证其卓越能力。

Conclusion: RSICCLLM通过创新的大模型后训练框架有效解决了RSICC任务中的数据稀缺和变化理解难题，为遥感图像变化描述提供了高效新范式。
...
TL;DR: 提出PerceptionRubrics框架，通过基于规则的原子化审计评估视觉感知的可靠性，揭示基准分数与实际脆性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准高分无法反映模型在密集视觉场景中的真实脆性，需要更严格的感知评估方法。

Method: 构建包含1,038张密集图像和12,000多条实例规则的数据集，通过循环同行评审生成黄金描述，并提取Must-Right和Easy-Wrong双流规则，采用门控评分机制替代线性平均。

Result: 发现可靠性差距（模型通过碎片化验证但无法满足严格约束）、开源与闭源模型间8%的感知差距，以及门控指标比传统基准更符合人类判断。

Conclusion: 严格的感知保真是可靠生成的前提，PerceptionRubrics提供了更有效的评估方法。
...
### [111] [Formalizing Latent Thoughts: Four Axioms of Thought Representation in LLMs](https://arxiv.org/abs/2606.27378)
*Fahd Seddik,Fatemeh Fard*

Main category: cs.CL

TL;DR: 提出一个不依赖下游基准评估LLM潜在思想表征的公理化框架，发现现有表征存在结构性缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有评估混淆了表征质量与模型能力，无法将失败归因于表征本身。

Method: 定义因果性、最小性、可分离性、稳定性四个公理及其量化指标，直接计算表征的独立属性。

Result: 在23个推理任务中无模型满足全部公理，表征能区分任务类型但无法区分同任务的子问题，且信息熵接近输入嵌入。

Conclusion: 该缺陷是结构性的，与模型规模、训练方式无关，表明通用LLM的表征存在根本性局限。
...
### [112] [Position: The Term "Machine Unlearning" Is Overused in LLMs](https://arxiv.org/abs/2606.27379)
*Sangyeon Yoon,Yeachan Jun,Albert No*

Main category: cs.CL

TL;DR: 本文主张在大型语言模型中严格区分“机器遗忘”与其它术语（如对齐、抑制、编辑），并建议仅在数据集定义的删除场景下使用“遗忘”一词。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型面临法规删除、版权争议和安全要求，导致“遗忘”术语被过度使用，混淆不同目标的任务。

Method: 通过对比分析不同“遗忘”任务（如拒绝有害请求、实体删除、知识压制）的实际目标，提出术语应基于明确承诺和参考模型。

Result: 当前“遗忘”标签下的指标和基准常被误用，奖励表面知识隐藏而非真正的训练等价性。

Conclusion: 呼吁采用更严格的术语体系，匹配任务的实际承诺，并设计相应的评估方法。
...
### [113] [A Survey of Automated Presentation Coaching: Systems, Methods, and Open Challenges](https://arxiv.org/abs/2606.27380)
*Wen Liang,Li Siyan,Zackary Rackauckas,Julia Hirschberg*

Main category: cs.CL

TL;DR: 本文综述了自动化演讲辅导系统，涵盖发音、韵律、多模态训练和会议问答工具，并提出五维任务分类法（包括分段发音、词汇重音、超音段韵律、节奏和内容忠实性），揭示了现有系统的覆盖缺口。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对自动化演讲辅导系统在计算机辅助发音训练、韵律建模和语音合成等维度的系统性调查和比较。

Method: 采用五维任务分类法映射系统，并回顾了基于TTS的示例生成和诊断方法，用于评估发音、韵律和流畅度。

Result: 识别出关键挑战，包括标注演讲语料库稀缺、跨L1背景的口音公平反馈、以及实时排练的低延迟诊断。

Conclusion: 自动化演讲辅导领域存在覆盖缺口，未来需要解决语料、公平性和实时性等问题。
...
### [114] [Causal Connections: Leveraging Multilingual Fine-Tuning for Financial QA@FinCausal 2026](https://arxiv.org/abs/2606.27446)
*Akash Kumar Gautam,Serhii Hamotskyi,Christian Hänig*

Main category: cs.CL

TL;DR: 团队HSA_CORAL使用三种模型家族（编码器、编码器-解码器、仅解码器LLM）参与FinCausal 2026共享任务，通过提取式问答从金融叙事中提取因果关系。最佳系统GPT-4.1 Mini在英西数据上微调，英语子任务得分最高，西班牙语排名第三。


<details>
  <summary>Details</summary>
Motivation: 研究由金融叙事中因果关系提取的跨语言挑战驱动，旨在比较不同模型方法在英语和西班牙语上的表现，并探索任务特定适应和多语言微调的价值。

Method: 比较三种模型：编码器（多语言BERT标记标注）、编码器-解码器（多语言BART生成）、仅解码器LLM（Llama 3.1和GPT变体）结合提示优化、少样本演示和监督微调。

Result: 提示和少样本表现有竞争力，监督微调提升最大。GPT-4.1 Mini在英西数据联合微调后，英语子任务并列最高分4.8140，西班牙语第三名4.7753。

Conclusion: 任务特定适应和多语言微调对跨语言迁移在金融因果关系QA中价值显著。
...
### [115] [Developmental approach reveals the statistical learning of Neural Language Models: Transformers generalize from the most abstract statistical patterns](https://arxiv.org/abs/2606.27460)
*Wang Bojun,Holly Jenkins,Elizabeth Wonnacott*

Main category: cs.CL

TL;DR: 本研究采用发展性方法探讨神经语言模型的统计学习与心理表征。通过分析生成式Transformer模型在训练过程中的内部表征变化，发现模型先习得最抽象的全局统计知识，后习得局部统计依赖，早期存在过度泛化，后期逐渐受限。基于此，提出解释NLM统计学习与语言认知的新框架。


<details>
  <summary>Details</summary>
Motivation: 理解神经语言模型如何习得统计知识与心理表征的机制。

Method: 在合成语法上训练一系列生成式Transformer模型，并在训练不同阶段保存模型状态，分析内部表征变化。

Result: 模型先获得全局统计知识，后获得局部依赖；早期过度泛化，后期约束。

Conclusion: 提出解释NLM统计学习与语言认知的发展路径框架。
...
### [116] [Supersede: Diagnosing and Training the Memory-Update Gap in LLM Agents](https://arxiv.org/abs/2606.27472)
*Vedant Patel*

Main category: cs.CL

TL;DR: 本文发现大语言模型代理在长对话中无法正确更新事实信息，存在“替代失败”问题，并提出了首个可训练的强化学习环境Supersede来缩小这一差距。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在跨会话交互中无法根据事实变化（如位置更新、价格变动）更新知识，导致回答错误，且此问题未被现有模型或记忆扩展解决。

Method: 基于LongMemEval知识更新子集评估记忆维护瓶颈；开发Supersede环境（基于verifiers/prime-rl栈），通过奖励机制交替监督事实时效性，并使用GRPO微调Qwen2.5-3B。

Result: 全上下文模型准确率92% vs 自维护记忆77%（p<0.005）；对话长度增加24倍时准确率从68%降至28%，增加记忆大小无效；GRPO微调后替代准确率从9.0%升至16.7%。

Conclusion: 事实替代失败是LLM代理的核心瓶颈，可通过强化学习训练缓解，而不仅靠更强大的模型或更大的记忆容量。
...
### [117] [The Context-Ready Transformer](https://arxiv.org/abs/2606.27538)
*Mahesh Godavarti*

Main category: cs.CL

TL;DR: 提出了上下文就绪转换器（context-ready transformer），这是一种新型循环神经网络架构，通过预处理每个令牌进入转换器块前的上下文信息，结合纠正网络和循环推理，实现了比标准转换器更快的推理速度和更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有标准转换器在处理序列时，令牌作为原始嵌入进入块，缺乏上下文的预计算，导致推理效率低且深度增加的扩展性差。

Method: 构建由 D 层转换器块组成的架构，通过纠正网络将前一位置块的输出（缓存的历史上下文）与当前令牌嵌入结合，使令牌进入块时已经上下文化。训练时展开纠正过程 K 次，并行处理所有位置，也可将预训练转换器转换为此模型。

Result: D=5 模型优于 12 层转换器，推理速度快 1.7 倍；K=10 时，单层模型（D=1）优于 6 层转换器，推理速度快 2.6 倍；在指针追踪任务中，D=1 模型解决了所有 10 个组合层，而标准转换器表现出阶梯式的深度依赖。

Conclusion: 该架构受益于宽表示和长上下文，通过纠正链的循环推理实现了高效序列生成，优于标准转换器在多种设置下的性能。
...
### [118] [EntMTP: Accelerating LLM Inference with Entropy Guided Multi Token Prediction](https://arxiv.org/abs/2606.27550)
*Carrie Chen*

Main category: cs.CL

TL;DR: 提出基于熵引导的多令牌预测调度器，通过动态切换树状注意力拓扑来提高自推测解码效率。


<details>
  <summary>Details</summary>
Motivation: 现有模型使用静态树状注意力结构，导致推测深度固定，无法适应自然语言中不确定性的变化。

Method: 训练免费的熵引导调度器（EntMTP），根据局部生成熵从帕累托最优树集中选择注意力拓扑。

Result: 在Humaneval、ShareGPT、GSM8k和Litbench上，相比Hydra实现1.15倍加速，峰值达1.36倍。

Conclusion: 动态匹配推测深度与上下文可预测性可提升吞吐量且不损失质量。
...
### [119] [Ko-WideSearch: A Korean Breadth-Search Benchmark for Exhaustive Set Enumeration by Web Agents](https://arxiv.org/abs/2606.27595)
*Minbyul Jeong*

Main category: cs.CL

TL;DR: 提出了Ko-WideSearch，一个韩语广度搜索基准测试，包含228个表格，用于评估网络智能体在广度搜索任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有网络智能体基准测试主要评估深度搜索（单个复杂答案），而广度搜索（完整枚举属性）很少被评估，尤其非英语环境。

Method: 通过自动化合成与验证管道构建基准，每个任务要求找出集合成员并填写属性表，通过Item-F1、Column-F1和Row-F1评分，并可调节表格宽度和复合键两个结构旋钮。

Result: 20个智能体在Item-F1（92.8）和Row-F1（53.7）间存在显著差距，随着任务难度增加准确性稳定下降，且增加搜索或成本无法弥补差距。

Conclusion: 网络智能体在广度搜索中找回集合元素容易但填充表格行困难，开放式自由文本单元格是最大挑战，而标准答案（如日期）得分较高。
...
### [120] [Narrative-UFET: Narrative Generation for Ultra-Fine Entity Typing](https://arxiv.org/abs/2606.27598)
*Mreedul Gupta,Advait Deshmukh,Ashwin Umadi,Matt Pauk,Maria Leonor Pacheco*

Main category: cs.CL

TL;DR: 提出Narrative-UFET，通过自动生成长文本叙事来为UFET任务提供更多上下文，显著提升了长尾类型识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有UFET方法依赖单句上下文，难以识别长尾类型，因为消歧证据常分布在不同句子中。

Method: 构建Narrative-UFET数据集，为每个实体搭配自动生成的短叙事，并设计两种变体（类型保持和类型变化），用于对比叙事级和句子级上下文的效果。

Result: 叙事上下文在长尾类型上持续优于句子级基线，其中类型变化变体效果更强，且合成叙事优于自然上下文。
...
### [121] [Masked Language Flow Models](https://arxiv.org/abs/2606.27617)
*Iskander Azangulov,Kianoosh Ashouritaklimi,Leo Zhang,Simon Vary,Patrick Rebeschini*

Main category: cs.CL

TL;DR: 本文提出了掩码语言流模型（MLFMs），通过将掩码机制整合到流语言模型中，实现了对掩码扩散模型的高效适配，并首次证明基于流的语言模型可扩展到推理和指令遵循任务。


<details>
  <summary>Details</summary>
Motivation: 现有掩码扩散模型在少步采样时因离散态分解近似导致效率下降，而流语言模型虽能单步生成但无法处理多步推理任务。

Method: 通过连续随机插值连接部分掩码序列与干净序列，将预训练掩码扩散模型轻量适配为MLFMs，并设计交替连续去噪与离散解掩码的采样器。

Result: 在GSM8K和MT-Bench上首次实现基于流的语言模型规模化解决下游推理与指令遵循任务。

Conclusion: 掩码流设计有效结合了并行生成效率与多步推理能力，为语言模型的快速推理提供了新范式。
...
### [122] [Cross-Platform Chinese Offensive Comment Detection via Dual-Threshold Hard Example Mining](https://arxiv.org/abs/2606.27629)
*Ruixing Ren,Junhui Zhao,Fangfang Wang*

Main category: cs.CL

TL;DR: 本文提出一种双阈值硬挖掘方法，用于改进中文社交媒体攻击性评论检测在跨平台部署中的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 跨平台部署攻击性评论检测时，存在性能下降问题，需要低成本、高效的域适应方法。

Method: 先使用COLD数据微调RoBERTa建立二分类基线；构建包含微博、小红书、贴吧、知乎的三类细粒度测试集，量化域距离；提出双阈值硬示例挖掘策略，通过置信度筛选高、低置信度易错样本，仅用少量人工标注的硬示例进行二次微调。

Result: 优化模型在四个平台上均取得显著性能提升。

Conclusion: 双阈值硬挖掘方法能有效实现低成本的跨平台域适应，缓解性能下降问题。
...
### [123] [Yuvion LLM: An Adversarially-Aware Large Language Model for Content And AI Safety](https://arxiv.org/abs/2606.27632)
...
TL;DR: 介绍了一个专为对抗性鲁棒内容安全和AI安全构建的大型语言模型Yuvion LLM，并提出了包含93个基准的评估框架YLRE。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在现实部署中因对抗性攻击导致安全失败，而传统开发忽视对抗性本质，安全评估高估鲁棒性。

Method: 包括对抗感知数据构建、知识增强持续预训练、多任务安全后训练（风险感知监督微调、强化学习策略优化及安全感知智能体强化学习）。

Result: Yuvion-8B在多项安全基准上超越包括GPT-5.4和Qwen3-MAX在内的更大模型。

Conclusion: Yuvion LLM在安全、对抗鲁棒性和现实能力方面展现显著优势。
...
### [124] [From Signals to Transfer: A Factorised Study of Probe-Based Uncertainty Estimation in Large Language Models](https://arxiv.org/abs/2606.27679)
*Ponhvoan Srey,Xiaobao Wu,Cong-Duy Nguyen,Quang Minh Nguyen,Duc Anh Vu,Anh Tuan Luu*

Main category: cs.CL

TL;DR: 本文提出一个分解式研究，系统评估基于探针的不确定性估计方法在大型语言模型幻觉检测中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前探针方法在特征设计、训练数据构建和评估设置上各不相同，导致性能驱动因素不明确。

Method: 在匹配条件下进行分解研究，比较原始隐藏状态、注意力特征和压缩特征的效果，并分析提示和标签构建的影响。

Result: 域内性能上原始隐藏状态和注意力特征难以超越，但在分布偏移下结构化压缩特征更鲁棒；提示和标签构建显著影响探针行为。

Conclusion: 仅靠域内性能不足以衡量进展，需注重部署导向评估；预训练探针可稳定迁移至开放式生成任务。
...
### [125] [When Search Agents Should Ask: DiscoBench for Clarification-Aware Deep Search](https://arxiv.org/abs/2606.27669)
*Yiling Tao,Shihan Deng,Meiling Tao,Pengzhi Wei,Zhichao Hu,Zhihao Zhu*

Main category: cs.CL

TL;DR: 提出了DiscoBench基准，用于评估搜索智能体在模糊用户查询下的澄清能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准假设用户查询完整明确，但实际搜索请求常模糊、不完整或错误，导致多步推理中歧义累积和搜索路径错误。

Method: 构建包含211个样本、463个歧义实例和11个领域的DiscoBench，设计用户模拟器进行多轮交互，从任务效用、歧义检测、交互策略和成本效率四个维度评估模型。

Result: 歧义检测与有效澄清是不同能力；反复搜索而非提问的表现常差于直接猜测，揭示当前搜索智能体在检索与交互问题解决之间的差距。

Conclusion: 现有搜索智能体在交互式澄清方面存在关键不足，需要改进其主动识别歧义和有效提问的能力。
...
### [126] [Mitigating LLM-based p-Hacking by Preregistering for the Next LLM](https://arxiv.org/abs/2606.27687)
*Maria Thomas,Kristina Gligoric,Nihar B. Shah*

Main category: cs.CL

TL;DR: 提出了一个预注册协议来减轻基于大语言模型的研究中的p-hacking问题，该协议要求使用未来发布的模型进行确认性分析。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在假设检验中容易被p-hack（调参直到得到期望结果），当前缺乏有效的防范机制。

Method: 研究者预注册实验方案和候选模型集，在承诺期后使用第一个可用的未来模型进行确认性分析。

Result: 在两个真实任务中，该协议在73.9%和72.7%的案例中阻止了p-hack的转移；预注册实验显示7个hack配置中有6个未能转移到新模型。

Conclusion: 预注册未来模型能有效减轻LLM研究中的p-hacking问题，且该效果在多种压力测试下仍稳健。
...
### [127] [Mitigating Position Bias in Transformers via Layer-Specific Positional Embedding Scaling](https://arxiv.org/abs/2606.27705)
...
TL;DR: 提出层特定位置嵌入缩放方法，通过遗传算法优化各层缩放因子，解决大语言模型的长上下文注意力偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法高延迟或依赖次优手动策略，需更高效解决方案。

Method: 为各层分配不同缩放因子，结合贝塞尔曲线与遗传算法搜索最优参数，无需微调或增加延迟。

Result: 在键值检索任务上准确率提升11.2%，多个基准测试中注意力分布更均衡。

Conclusion: LPES有效缓解位置注意力偏差，提升长上下文性能。
...
### [128] [Low-Agreeableness Persona Conditioning for Safe LLM Fine-Tuning](https://arxiv.org/abs/2606.27709)
*Austin MY Cheung,Yi Yang*

Main category: cs.CL

TL;DR: 本文研究了温暖微调降低大语言模型对抗安全性并导致越狱易感性的问题，提出了一种通过数据设计而非修改训练目标来增强安全性的方法。


<details>
  <summary>Details</summary>
Motivation: 现有工作表明温暖微调会降低模型的真实性和增加谄媚行为，但对其削弱对抗安全性的机制和解决方案研究不足。

Method: 引入了一种基于人物驱动的改写管道，将用户输入条件化为低宜人性，并将助手响应设计为温暖且缓和冲突的，用于训练数据构建。

Result: 在四个模型的三项实验中，相比通用温暖微调基线，该方法降低了越狱易感性和有害输出率，同时保持了对话温暖性。表征探测表明该方法减少了潜在空间中温暖与顺从方向的几何对齐。

Conclusion: 仅通过数据设计（无需安全标签、有害检测器或修改训练目标）即可实现更安全的共情微调。
...
### [129] [Do Speech Emphasis Models Generalize across Languages and Emotions?](https://arxiv.org/abs/2606.27717)
*Megan Wei,Deepali Aneja,Jiaqi Su,Yunyun Wang,Haonan Chen,Zeyu Jin*

Main category: cs.CL

TL;DR: 构建了包含7种语言、34种情感/风格的MMEE语料库，验证了多语言训练对强调检测的鲁棒性提升，揭示了情感间和数据集间的共享韵律结构。


<details>
  <summary>Details</summary>
Motivation: 现有强调检测模型主要基于单语中性朗读语音训练和评估，缺乏多语言、多情感场景下的泛化能力研究。

Method: 收集了10000句专业录制的多语言多情感话语（14.13小时），采用三级感知标注（每样本10个标注），并在单语、跨语言、多语言、跨情感、跨数据集及数据规模等设置下对比了两种SOTA架构。

Result: 单语模型在类型学差异大的语言间零样本迁移性能下降；多语言训练显著提升鲁棒性；模型在高/低唤醒情感间迁移表现稳健；合成与感知基准间双向迁移良好，性能在较小训练规模下仍保持稳定。

Conclusion: 多语言训练对提升强调检测的跨语言泛化至关重要，情感和数据集间存在共享的韵律结构，且模型对小数据具有鲁棒性。
...
### [130] [Enhancing Numerical Prediction in LLMs via Smooth MMD Alignment](https://arxiv.org/abs/2606.27731)
*Zhuo Zuo,Li Yue,Wenhao Zheng,Chenpeng Wang,Xianggen Liu*

Main category: cs.CL

TL;DR: 提出SMMD损失函数，通过在数值词汇上定义距离核和图平滑度，提升LLM在数值精确输出任务上的可靠性。


<details>
  <summary>Details</summary>
Motivation: 标准交叉熵损失将数值标记视为无序类别，忽略了数值的度量结构，导致LLM在需数值精确的任务中不可靠。

Method: 构建SMMD损失，基于经典MMD引入数值距离核和图的平滑性，通过核匹配对齐预测与目标分布，并在核图上平滑残差以促进局部一致性。

Result: 在数学推理、算术计算、时钟识别和图表问答四个任务上，SMMD一致提升了准确率，优于交叉熵和近期数值目标损失。

Conclusion: SMMD有效改善了LLM的数值精确性，其效果来自MMD与平滑的互补作用，且距离核设计至关重要。
...
### [131] [KG2Cypher: Data-Centric Pipeline for Building Enterprise Text-to-Cypher Systems](https://arxiv.org/abs/2606.27742)
*Minjun Choi,Yerin Kim,Junghyuk Seo,Sujin Mo,Hyemin Lee,Youngjoong Ko*

Main category: cs.CL

TL;DR: 提出KG2Cypher数据驱动管线，从现有知识图谱自动构建文本-查询对，用于企业级文本到Cypher系统。


<details>
  <summary>Details</summary>
Motivation: 为企业私有知识图谱构建自然语言接口成本高昂，现有方法难以处理短查询和模式改写。

Method: 基于图谱事实生成可执行Cypher查询，再用LLM生成自然语言问题；经过LLM法官和人工验证后转化为候选感知SFT数据，结合类条件模式提示、实体检索和LoRA推理。

Result: 在韩语企业场景中，LoRA SFT将执行结果F1从0.806提升至0.950（广播节目查询）和0.70至0.92（公司查询）；11类设置下精确匹配率95.2%，执行率99.9%，执行结果F1为0.964。

Conclusion: KG2Cypher有效降低了文本-to-Cypher系统构建成本，显著提升了短查询和模式改写场景下的性能。
...
### [132] [Output-Space Allocation Costs for Calibration-Guided LLM Compression: An Empirical Study](https://arxiv.org/abs/2606.27785)
*Qiong Tang,Xiangkun Hu,Xiangyang Liu,Yiran Chen,Yunfan Shao*

Main category: cs.CL

TL;DR: 本文研究了ROCKET压缩方法中分配成本函数对齐对模型保真度的影响，发现输出空间目标提高了零样本准确率但增加了困惑度，且在高压缩率下效果有限。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关压缩方法（如ROCKET）的分配成本函数基于权重空间误差，与输出空间优化目标不一致，可能限制模型压缩后的保真度。

Method: 提出ROCKET-ActCost方法，将原ROCKET的权重空间Frobenius误差分配成本替换为输出空间重构误差，通过校准数据指导稀疏字典分解和多选择背包问题分配。

Result: 在Qwen3-8B的50%压缩下，ROCKET-ActCost准确率提升0.8个百分点（53.1% vs 52.3%），但困惑度增加16%（61.46 vs 52.98）；在Llama-3.2-1B的20%压缩下，两者结果几乎相同。

Conclusion: 分配成本对齐会引发准确率-困惑度权衡，但权重空间与输出空间误差高度相关（>0.99）限制了效果差异，且在低压缩率下成本函数影响较小。
...
### [133] [SHIFT: Gate-Modulated Activation Steering for Knowledge Conflict Mitigation in Retrieval-Augmented Generation](https://arxiv.org/abs/2606.27786)
*Ruochang Li,Pengcheng Huang,Zhenghao Liu,Yukun Yan,Huiyuan Xie,Yu Gu,Ge Yu,Maosong Sun*

Main category: cs.CL

TL;DR: 提出SHIFT框架，通过可学习的门控调制解决检索增强生成中的知识冲突问题，仅调整0.01%参数即可提升模型对上下文的依赖能力。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成中上下文与参数知识冲突会导致LLM生成质量下降，现有神经元级修改方法存在级联效应损害模型通用能力。

Method: 设计轻量级门控模块，冻结主干模型参数，仅优化极少量可训练参数，通过自适应调整内部表征来动态利用上下文与参数知识。

Result: 在六个数据集上，SHIFT在知识冲突场景下显著优于多种基线方法，保持模型通用能力的同时有效解决冲突。

Conclusion: 门控调制是比神经元编辑更安全高效的知识冲突解决方案，为RAG系统提供了轻量级增强手段。
...
### [134] [NLL-Guided Full-Attention Layer Selection for Training-Free Sliding-Window Adaptation](https://arxiv.org/abs/2606.27791)
*Qiong Tang,Xiangkun Hu,Xiangyang Liu,Yiran Chen,Yunfan Shao*

Main category: cs.CL

TL;DR: 提出了一种基于负对数似然（NLL）指导的无训练层选择方法，用于混合注意力模型中的全注意力层分配，在长上下文任务中以仅1/4全注意力层达到接近1/2全注意力层的基线性能。


<details>
  <summary>Details</summary>
Motivation: 现有混合注意力模型在确定哪些层应保留全注意力时，要么使用固定周期模式，要么使用基于注意力的启发式方法，这些方法可能无法捕捉对下游精度真正重要的因素。

Method: 通过计算每一层在使用滑动窗口注意力而非全注意力时，对答案标记的负对数似然（NLL）退化程度，以此衡量该层的重要性，从而进行训练免费的层选择。

Result: 在LongMemEval数据集上使用Qwen3-4B模型，仅用1/4全注意力层达到64.6%准确率，接近1/2全注意力周期基线（65.0%），同时比SWAA报告的1/4周期基线和LightTransfer基线分别高出10.4和26.4个百分点。

Conclusion: 该方法仅需约15分钟的一次性校准，有效推进了长上下文大语言模型部署的效率-精度帕累托前沿。
...
### [135] [Learning Complementary Action Modeling from Automotive Maintenance Instructions](https://arxiv.org/abs/2606.27808)
*Jiaqi Wu,Bai Li,Jochen Hartmann,Martin Gaedke,Sander Stuijk*

Main category: cs.CL

TL;DR: 本文定义了互补动作建模（CAM）任务，旨在通过修改动作短语来生成或识别维修指令的程序性对应项，并证明其不同于表面相似性或同义改写。


<details>
  <summary>Details</summary>
Motivation: 微小的词汇变化可能完全逆转指令的程序性含义，现有方法无法捕捉这种细微差别。

Method: 使用候选匹配和受控Seq2Seq生成方法，基于德语汽车维修数据集进行实验。

Result: 互补维修指令最好建模为基于细微词汇线索的程序性关联。

Conclusion: 不应将CAM任务视为普通句子相似性或同义改写问题。
...
### [136] [Position Bias Correction is Insufficient for One-Pass Attention Sorting](https://arxiv.org/abs/2606.27793)
*Qiong Tang,Xiangkun Hu,Xiangyang Liu,Yiran Chen,Yunfan Shao*

Main category: cs.CL

TL;DR: 提出了一种消除位置偏差的单遍注意力排序方法，旨在替代昂贵的迭代排序。实验表明，校正位置偏差仅能部分缩小与迭代排序的差距，说明重复排序提供了额外的增益。


<details>
  <summary>Details</summary>
Motivation: 长上下文语言模型存在位置偏差，中间位置信息被低估。现有注意力排序方法通过迭代重排文档来缓解此问题，但多次排序-生成循环增加了部署成本。

Method: 通过从低注意力得分的文档中估计每提示位置偏差曲线，采用减法或除法校正原始注意力分数，实现单遍排序。

Result: 在LLaMA-2-7B-32K-Instruct上，校正与未校准单遍排序结果相同（94.83%包含精度）；在YaRN-Llama-2-7b-64k上，校正提升了8.67个百分点，但仍落后迭代排序14.84个百分点，仅弥合了37%的差距。

Conclusion: 位置偏差校正不足以匹配迭代排序，重复排序提供了超越偏差校正的额外好处。
...
### [137] [A Study of Temporal Fusion Strategies for Named Entity Recognition in Historical Texts](https://arxiv.org/abs/2606.27881)
*Emanuela Boros*

Main category: cs.CL

TL;DR: 本文系统研究了如何将时间元数据轻量地融入命名实体识别模型，实验表明晚期融合策略在历史文本中更鲁棒且时间泛化性更好。


<details>
  <summary>Details</summary>
Motivation: 历史文本中实体随时间漂移，而现有语言模型对时间推理能力有限，因此需要探索高效的时间信息融合方法。

Method: 使用绝对和相对时间表示，通过交叉注意力、适配器、拼接等机制注入Transformer架构，并对比早期融合与晚期融合策略。

Result: 晚期融合策略在法语和德语历史数据集上表现更优，尤其在早期和噪声时期性能更稳健。

Conclusion: 面向历史文本的NER，晚期融合时间元数据是更有效的范式，可提升模型对时间变化的泛化能力。
...
### [138] [VASAE: Naming SAE Dictionary Directions with Vocabulary-Aligned Anchoring](https://arxiv.org/abs/2606.27941)
*Kairui Zhang,Ziwen Yu,Zahraa S. Abdallah,Martha Lewis*

Main category: cs.CL

TL;DR: 提出词汇对齐稀疏自编码器（VASAE），通过训练时嵌入锚定使特征直接关联词汇，GPT-2-small中约90%特征在浅中层对齐，Llama-8B浅层92.8%对齐但深层有限。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器（SAE）的特征仅事后命名，缺乏与Transformer词汇表的直接联系。

Method: 在训练SAE时引入词汇对齐约束，将每个特征锚定至最近邻的token嵌入，并赋予其对应token字符串作为名称。

Result: VASAE不降低重建质量，GPT-2-small 0-10层约90%特征对齐；Llama-3.1-8B浅中层对齐率高（92.8%），但末层有限；扣除句子均值后，许多特征名与邻近输入token相关。

Conclusion: 词汇对齐锚定可在训练中连接特征与内在token名称，补充事后解释方法。
...
### [139] [Triadic Werewolf: A Jester Role for Multi-Hop Theory of Mind in LLMs](https://arxiv.org/abs/2606.27909)
*Avni Mittal*

Main category: cs.CL

TL;DR: 本文通过引入具有反向收益动机的“小丑”角色，将狼人杀游戏扩展到三方博弈，测试大语言模型的多智能体社会推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有理论心智评估多使用二元社交推理游戏，模型仅凭语言先验即可表现良好，无需模拟对手动机，缺乏对多维度推理的检验。

Method: 在狼人杀游戏中加入小丑角色，其目标是被投票出局，从而形成三方对抗的效用函数；通过60场游戏对比GPT-4.1、DeepSeek-V3.1和Llama-3.3-70B，并设置自学习开关。

Result: 小丑胜率60-70%，狼人从未超过20%；GPT-4.1狼人首日投票小丑的比例达60-70%，是严格自损行为；自学习对DeepSeek和Llama有益，但对GPT-4.1有害；仅DeepSeek学会“看似可疑但非故意可疑”的微妙策略。

Conclusion: 三方激励结构暴露了二元推理游戏无法揭示的多智能体推理层面，强调了模拟对立动机在复杂社会推理中的必要性。
...
### [140] [An Empirical Analysis of Factual Errors in Human-Written Text and its Application](https://arxiv.org/abs/2606.27959)
*Kazuma Iwamoto,Kazumasa Omura,Shotaro Ishihara*

Main category: cs.CL

TL;DR: 论文聚焦于人工文本事实错误检测，通过分析报纸更正构建错误类型学，发现汉字误转和数量词错误等特征。


<details>
  <summary>Details</summary>
Motivation: 大语言模型研究热潮导致对人工文本事实错误检测的忽视，而现有幻觉检测基准不覆盖这些错误类型。

Method: 分析报纸更正提取人工错误类型学，基于此构建合成和真实测试数据，评估大语言模型的表现。

Result: GPT-5.4等高性能模型在合成数据上仅达52%的词级F1分数，表明任务难度高。

Conclusion: 人工文本事实错误检测仍具挑战，现有模型能力有限，需进一步研究。
...
### [141] [ToxiREX: A Dataset on Toxic REasoning in ConteXt](https://arxiv.org/abs/2606.27981)
*Stefan F. Schouten,Ilia Markov,Piek Vossen*

Main category: cs.CL

TL;DR: 提出了一个多语言、上下文相关的隐式毒性推理数据集ToxiREX，包含结构化注释和基线实验结果。


<details>
  <summary>Details</summary>
Motivation: 现有毒性检测缺乏对隐式和上下文相关毒性的系统解释，且多语言数据集不足。

Method: 基于毒性推理模式，收集多语言Reddit评论线程，使用LLM和母语者标注，并通过提示和微调模型进行基线评估。

Result: 模型表现优于随机但仍有显著提升空间，测试集注释中的分歧反映了合理的替代解释。

Conclusion: ToxiREX首个融合多语言、对话上下文和隐式毒性的结构化数据集，挑战性任务需更优解决方案。
...
### [142] [From Black-Box to Clinical Insight: A Multi-Stage Explainable Framework for Speech-Based Cognitive Impairment Detection](https://arxiv.org/abs/2606.27973)
...
TL;DR: 提出了一个多阶段可解释性框架，将基于Transformer的语音认知障碍检测模型的预测转化为临床可理解的叙述，通过SHAP、理论语言特征和LLaMA-3.1-70B-Instruct推理管线实现。


<details>
  <summary>Details</summary>
Motivation: 语音检测认知障碍成本低，但Transformer模型在临床上缺乏可解释性，需要弥合黑箱预测与临床理解之间的差距。

Method: 整合SHAP的词元归因、理论指导的语言特征以及基于LLaMA-3.1-70B-Instruct的四阶段LLM推理管线，映射到四个认知-语言维度（如词汇丰富度、句法复杂度、语义连贯性）。

Result: 在NIA PREPARE基准上基于SpeechCARE模型（F1=72.11%）构建，70份英语样本的医生评估显示与患者认知水平高度一致，系统可用性量表得分为82/100。

Conclusion: 该框架有效提升了临床可解释性，具有集成到临床工作流的潜力。
...
### [143] [Dialogue to Detection: A Multimodal Hybrid NLP Pipeline for Insurance Fraud Detection](https://arxiv.org/abs/2606.28002)
*Muhammad Shakeel Akram,Amal Htait,Abdul Hamid Sadka,Emma Meisingseth,Karishma Jaitly*

Main category: cs.CL

TL;DR: 提出一个合成多模态框架，用于复制首次损失通知（FNOL）条件，结合文本、语音和说话人特征进行保险欺诈检测，克服现有仅依赖文本数据的局限。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖私有纯文本数据集，限制多模态欺诈检测进展，且实际欺诈早检（FNOL）困难。

Method: 合成多模态框架：生成对话脚本和双说话人音频；进行ASR和说话人日记化；结合NER、正则表达式、LLM-RAG检索和说话人嵌入，基于规则生成风险评分。

Result: 数据集验证和模块评估显示稳定性和迁移潜力，提供超越纯文本的可重复基线。

Conclusion: 该框架能复现FNOL条件，通过多模态融合有效平衡敏感性与假阳性，促进可复现的欺诈检测研究。
...
### [144] [A Tree-of-Thoughts Inspired Hybrid Approach for Legal Case Judgement Summarization using LLMs](https://arxiv.org/abs/2606.28044)
*Aniket Deroy,Kripabandhu Ghosh,Saptarshi Ghosh*

Main category: cs.CL

TL;DR: 提出一种基于思维树的混合抽取-生成式摘要方法，用于法律判决摘要，实验证明该方法优于纯抽取或纯生成式方法。


<details>
  <summary>Details</summary>
Motivation: 现有法律判决摘要多采用纯抽取或纯生成式方法，而混合式技术尚未被充分探索。

Method: 受思维树启发，设计了一种抽取-生成式混合提示策略，在DeepSeek和LLama两种大语言模型上进行实验。

Result: 所提出的抽取-生成式提示相比其他提示类型生成了更优的摘要。

Conclusion: 混合式摘要方法在法律领域具有潜力，能有效结合抽取与生成的优点。
...
### [145] [Can LLMs Judge Better Than They Generate? Evaluating Task Asymmetry, Mechanistic Interpretability and Transferability for In-Context QA](https://arxiv.org/abs/2606.28050)
*Sambaran Bandyopadhyay*

Main category: cs.CL

TL;DR: 在受控的上下文问答场景中，模型自我评估并非普遍比生成更容易；生成准确率在四个基准中的三个上超过自我评估。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM作为评判者和自我评估流程中隐含的假设，即评估比生成更容易。

Method: 通过受控的上下文问答设置，使用同一模型评估自身生成答案，并控制参数知识混淆；进行注意力分析、LoRA微调实验。

Result: 生成准确率在SQuAD 2.0、DROP和HotpotQA上高于自我评估，仅MuSiQue例外；评估时对上下文和候选答案的注意力显著低于生成时。

Conclusion: 自我评估流程的核心假设存在缺陷，生成与评估之间的不对称性并非训练伪影。
...
### [146] [MultiHashFormer: Hash-based Generative Language Models](https://arxiv.org/abs/2606.28057)
*Huiyin Xue,Atsuki Yamaguchi,Nikolaos Aletras*

Main category: cs.CL

TL;DR: 提出MultiHashFormer框架，通过哈希签名表示词元，实现参数高效的自回归语言建模，在多个规模上超越标准Transformer。


<details>
  <summary>Details</summary>
Motivation: 传统词嵌入矩阵随词汇表大小线性增长，哈希方法在编码器模型中有效但存在多对一冲突，无法用于因果语言模型。

Method: 每个词元由多个独立哈希函数生成的短哈希ID序列（哈希签名）表示，通过哈希编码器压缩为潜向量，再经Transformer解码器处理，最后由哈希解码器生成下一词元的哈希签名并映射回文本。

Result: 在100M、1B和3B参数规模上，MultiHashFormer在多个基准测试中一致优于标准Transformer，并支持多语言词汇扩展且参数不变。

Conclusion: 基于哈希签名的自回归建模方法在保持参数效率的同时，能有效提升语言模型性能。
...
### [147] [The Signal-Coverage Matrix: Stratifying Type and Semantic Errors in Statement Autoformalization](https://arxiv.org/abs/2606.28013)
*Chengxiao Dai,Zhaokun Yan,Zhanhui Lin*

Main category: cs.CL

TL;DR: 本文提出信号-覆盖矩阵，通过Lean精化器（通过/失败）和语义等价（等价/不等价）交叉分类，揭示大型语言模型自动形式化中标题类型正确性（TC%）标量掩盖的不同方法错误解决模式。


<details>
  <summary>Details</summary>
Motivation: TC%标量掩盖了每种方法解决的具体错误类型，因此需要更细粒度的分类来理解自动形式化进步的本质。

Method: 使用信号-覆盖矩阵将输出分为四个单元：真实成功（TS）、仅类型正确（TO）、仅语义正确（SO）和两者失败（BF）。在ProofNet#和MiniF2F测试集上，对DeepSeek V4-Pro采用四种方法（Vanilla、Lean-Retry、Sample-Filter和分层自动形式化SAF）进行实验。

Result: (1) 三种精化器反馈方法使TS提升34-36点，其中约64%来自类型层恢复，SO净持平（87.5%原始语义错误被修复，8个新创建）。(2) TO到TS转化率为23/61（Wilson 95% CI [26.6%, 50.3%]），该层恢复率可在2/186内预测新方法的ΔTS，并使ΔTC与Vanilla精化器失败率呈线性关系（R²=0.96）。(3) 两位评判者在精化器反馈输出上分歧26-37个百分点（Vanilla仅7个百分点），30-56%的符号评判假阳性可追溯到精化器强制重写。

Conclusion: TC%的提升应归因于具体哪个单元的变化，而非仅依赖标量。持续性残差归结为两个金标准形式化错误。
...
### [148] [Mechanism-Driven Monitors for Preemptive Detection of LLM Training Instability](https://arxiv.org/abs/2606.28116)
*Ruixuan Huang,Yipei Wang,Wenyi Fang,Hantao Huang,Yifan Huang,Ansheng You,Zhenxing Zhang,Shuai Wang,Fan Wu,Yang Zheng*

Main category: cs.CL

TL;DR: 本文研究了基于机制的大语言模型训练不稳定检测方法，通过分析关键模块的功能角色，在故障导致损失完全崩溃前数千步就识别出异常信号。


<details>
  <summary>Details</summary>
Motivation: 大模型训练消耗大量计算资源，稳定性故障发生后损失和梯度范数仍可能看似正常，导致故障持续数千步，因此需要提前检测训练不稳定。

Method: 从每个关键模块（如低精度闪速注意力、MoE路由器）的功能角色出发，推导内部监控指标：对注意力模块监控QK双线性分解的谱熵，对MoE路由器监控专家选择指标。

Result: 在低精度注意力、大学习率和组合故障的注入实验中，这些信号能提供不同故障的独特特征，并在损失发散前数千步触发预警。

Conclusion: 机制驱动的内部监控器能有效提前检测训练不稳定，减少故障导致的资源浪费。
...
### [149] [From Tokens to States: LLMs as a Special Case of World Models and the Continuous Path Beyond](https://arxiv.org/abs/2606.28127)
*Paul Dubois*

Main category: cs.CL

TL;DR: 该论文挑战了将大型语言模型（LLMs）和世界模型视为二分的观点，提出LLMs是世界模型的一种退化特例，并从NTP到JEPA存在连续谱系。


<details>
  <summary>Details</summary>
Motivation: Yann LeCun认为LLMs无法实现通用智能，需被取代；本文旨在反驳这一二元框架，揭示LLMs与世界模型的本质关系。

Method: 概念分析：证明LLMs是世界模型的退化特例；提出从NTP到JEPA的连续谱系，包括多token预测、未来摘要预测和下一潜在预测；分析相关数据与架构挑战。

Result: LLMs是世界模型的特殊形式，其约束可逐步放松；现有研究已覆盖谱系中间点；但向JEPA过渡面临互联网规模自监督数据和Transformer架构适配性问题。

Conclusion: LLMs并非世界模型的替代，而是其子集；向更一般化世界模型发展的道路需解决数据标注和架构可扩展性两大开放问题。
...
### [150] [Cognitive Episodes in LLM Reasoning Traces Enable Interpretable Human Item Difficulty Prediction](https://arxiv.org/abs/2606.28186)
*Chenguang Wang,Ming Li,Xinyue Zeng,Zhuochun Li,Hong Jiao,Tianyi Zhou,Dawei Zhou*

Main category: cs.CL

TL;DR: 提出了Epi2Diff框架，通过将大型推理模型的推理迹映射为认知上的“情节”序列，预测人类题目难度。在四个真实数据集上，该方法优于现有基线，在SAT分类任务上平均相对提升8.1%。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖昂贵的人工校准或仅使用题目文本，缺乏对题目所引发认知过程的理解，且难以规模化提供可解释的证据。

Method: Epi2Diff将LRM推理迹转化为认知情节序列，提取推理规模、努力分配和状态转移等情节动态特征，并结合语义表示进行难度预测。

Result: 在四个真实难度数据集上优于强基线，包括微调小模型、大模型上下文学习和监督微调。SAT基准上平均相对提升8.1%。

Conclusion: 认知情节提供了预测性且可解释的过程表征，证实了较难的题目引发更费力、迭代和面向实现的情节动态，而非仅是更长的响应。
...
### [151] [Vision-Default, Prior-Override: Causal Mechanisms of Perception-Knowledge Conflict in Vision-Language Models](https://arxiv.org/abs/2606.28273)
*Niclas Lietzow,Danielle Bitterman,Carsten Eickhoff,William Rudman,Michal Golovanevsky*

Main category: cs.CL

TL;DR: 本文通过激活修补、消融实验和机制分析，发现视觉-语言模型中视觉接地默认发生，而先验知识接地仅依赖于第二半网络中的少量因果必要注意力头（2.5-4.8%），这些头在感知-知识冲突中主导从知识到视觉的答案转变。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅从行为层面描述视觉-语言模型如何处理视觉证据与记忆知识的冲突，缺乏组件级的因果解释。

Method: 结合三种粒度的激活修补（残差流、注意力头、MLP子层）、模型组件消融研究和机制分析。

Result: 在先验知识提示下，消融因果头使68-96%的预测从知识接地转为视觉接地，而视觉接地预测仅改变0.8-7.5%；识别出路由头和信息写入头两种功能角色。

Conclusion: 视觉-语言模型中存在一个稀疏的因果电路，负责处理感知-知识冲突，其结构跨模型系列和规模一致。
...
TL;DR: SCORE 是一种通过模拟训练提升真实世界机器人操作策略的方法，利用流约束（flow steering）限制强化学习在模拟中的动作空间，确保行为可迁移，并显著提高成功率。


<details>
  <summary>Details</summary>
Motivation: 现实数据训练的机器人策略存在不精确、缓慢和脆弱的问题，而强化学习尽管有潜力但现实训练昂贵，模拟训练则面临迁移困难，需解决不匹配和过度约束的挑战。

Method: 提出了支持约束离域强化学习（SCORE），通过流约束将模拟中的策略优化限制在生成策略的动作支持集内，从而保证行为可迁移。

Result: 在八项真实世界多指灵巧操作任务中，平均成功率从37.8%提升至89.9%，优于基线（59.5%），且收敛步数减少36.8%。

Conclusion: 当策略优化被适当约束时，模拟能有效改进现实操作策略，为实到模到实的策略提升提供新范式。
...
TL;DR: 提出AO-ARC多机器人运动规划方法，在初始解速度上媲美最先进可行性求解器，且随机器人数量增加收敛更快更可靠。


<details>
  <summary>Details</summary>
Motivation: 现有任意时间MRMP方法在机器人数量增加时收敛性能不足，需要更高效且保证渐进最优性的算法。

Method: 将AO-x元算法应用于ARC方法，通过在有界MRMP实例上迭代调用ARC，在保持成本约束一致性的同时利用自适应解耦。

Result: 在2D和3D场景中，初始解速度与可行性方法相当，收敛速度和可靠性优于现有任意时间方法。

Conclusion: AO-ARC具有渐进最优性，在多机器人场景中平衡了初始解效率和长期收敛性能。
...
TL;DR: AstraTag是一种基于Spidron图案的递归结构标记，专为太空机器人设计，能在多尺度下检测，尤其解决曲面上近距跟踪失效问题。


<details>
  <summary>Details</summary>
Motivation: 现有标记多为单尺度和地面设计，在太空接近与对接阶段因视场丢失而失效。

Method: 使用Spidron递归图案实现多尺度检测，48位签名由通用里德-所罗门编码，检测流程含轮廓四边形定位、透视校正和字典匹配，并采用薄板样条重映射处理曲面。

Result: 在曲面航天器模型上，AstraTag检测率高于三层分形ArUco和AprilTag。

Conclusion: AstraTag为太空机器人提供了一种鲁棒的递归标记方案。
...
TL;DR: SceneBot是一个统一运动跟踪框架，能处理自由空间运动、地形穿越和全身操作，通过接触条件化实现长时复杂任务。


<details>
  <summary>Details</summary>
Motivation: 现有类人机器人强化学习策略在自由空间表现优异，但面对接触丰富的任务（如物体交互和凹凸地形）时，纯运动学跟踪无法解决物理歧义。

Method: 提出场景生成方法，从重新定位的人类运动推断场景交互图；利用接触标签和参考运动训练单一策略。

Result: 在7.5小时重建接触数据上训练后，SceneBot能泛化到未见过的运动和场景，首次统一自由空间和接触丰富行为。

Conclusion: 接触条件化是有效的类人控制接口，SceneBot成功完成如搬箱上楼等复杂长时任务。
...
TL;DR: 提出一种混合仿真框架，结合高保真四旋翼模型与物理引擎模拟绳索-载荷动态，通过强化学习训练策略实现快速精准抛投，零样本部署后误差降低50%，耗时减少30%。


<details>
  <summary>Details</summary>
Motivation: 现有基于模型的轨迹优化方法因保守约束、跟踪误差和绳索动力学建模困难而性能次优，需探索更高效的动态载荷释放方案。

Method: 构建耦合仿真系统：每步交换四旋翼模型与物理求解器之间的力；训练深度强化学习策略直接输出敏捷抛投动作，支持视觉输入替代状态估计。

Result: 硬件部署中，抛投着陆误差降低50%，投掷耗时减少30%；视觉输入策略达到与状态输入相当的精度。

Conclusion: 耦合仿真是性能提升的关键，该框架开源可促进动态空中操作研究。
...
TL;DR: 提出一种基于物理信息机器学习的机器人辐射源定位框架，无需路径规划即可在未知环境中精确估计源位置，并通过连续学习提升在线部署实用性。


<details>
  <summary>Details</summary>
Motivation: 现有辐射源定位方法需机器人靠近源，增加辐射损伤风险，且路径规划算法限制了任务灵活性。

Method: 使用物理信息机器学习模型，设计物理启发张量处理未知障碍物衰减信号，并行计算多模型提高鲁棒性和精度，应用连续学习于实际部署。

Result: 在高保真蒙特卡洛仿真和物理实验中验证，覆盖多种空间尺度、辐射源类型、障碍物和机器人轨迹，展现出高效定位能力。

Conclusion: 该方法将机器人辐射感知从点状通量检测提升至空间智能，提升了安全性和任务灵活性。
...
TL;DR: 提出了并行ARC算法（P-ARC），通过并行化初始解、冲突检测和冲突解决三个阶段，并引入OR并行多起点策略形成OR-P-ARC，在16核CPU上实现近4倍加速。


<details>
  <summary>Details</summary>
Motivation: 针对自适应机器人协调（ARC）方法在多机器人运动规划（MRMP）中的计算效率瓶颈，利用ARC分解问题产生的独立性来加速求解。

Method: 对ARC的三个主要阶段（初始个体解、冲突检测、冲突解决）分别设计并行变种，并结合OR并行多起点策略形成混合并行策略OR-P-ARC。

Result: 在最多128个机器人的2D移动和平面操作器场景中展示了加速效果，在真实世界启发的大规模Panda多操作器团队场景中，使用16个CPU核时规划时间加速接近4倍。

Conclusion: P-ARC及其混合策略OR-P-ARC能有效利用并行计算资源，显著提升多机器人运动规划的求解速度。
...
TL;DR: 提出轻量级3D空间注入模块，通过自适应层归一化将3D定位信号直接注入动作头，显著提升VLA模型的空间与任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在测试时对物体位置变化和指令变化泛化能力差，现有2D或语言提示方法无法解决这一问题。

Method: 设计模型无关的轻量级模块：将3D点云定位信号转换为与夹爪的相对位移，通过自适应层归一化注入VLA的动作头。

Result: 在LIBERO-PRO上，GR00T-N1.6在任务扰动下成功率从31.2提升至77.5，位置扰动下从28.1提升至60.2；π_{0.5}也获得类似提升。

Conclusion: 将3D定位信号直接注入动作头是提升VLA泛化能力的关键，且仅需在预训练骨干上添加轻量级模块即可实现。
...
TL;DR: 提出了一种解耦全身模仿学习框架（CWI），通过分离上肢和下肢的动捕数据使用，结合双鉴别器和多批评家架构，实现了人形机器人的稳定操控与协调运动。


<details>
  <summary>Details</summary>
Motivation: 现有全身控制器存在两个问题：仅靠命令采样的方法稀疏奖励且需精心调参，模仿全身动捕数据则受数据失衡影响（行走动作过于激进）。

Method: CWI框架：上肢操控模仿完整动捕数据集，下肢运动由基于专家行走/蹲起片段训练的双鉴别器AMP引导；多批评家架构减少目标冲突，教师-学生蒸馏输出仅依赖手部姿态和速度/高度命令的全身策略。

Result: 在模拟和真实LimX Oli人形机器人上验证，实现了竞争力的操控-运动性能、稳健的全身协调和无需全身动捕设备的遥操作。

Conclusion: 解耦上下肢动捕数据的模仿学习能有效提升人形机器人全身协调性，降低对复杂动作捕捉的依赖。
...
TL;DR: 提出了一种记忆增强的世界动作模型 DiM-WAM，通过多尺度历史上下文、局部未来动态和全局任务进度来提升长期任务的成功率。在 RMBench 和真实机器人任务上，成功率显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型主要依赖短期历史与短视野未来预测，难以处理需要依赖早期观测和任务进度的长期任务，存在长期遗忘和全局状态感知不足的问题。

Method: 引入记忆增强机制，通过独立相似度合并更新多个记忆库，并使用嵌入银行身份和时间的长期上下文来调节视频与动作去噪；同时添加进度监督目标，使记忆标记编码历史事件与当前任务阶段及其对剩余任务的影响。

Result: 在 RMBench 上成功率从 28.4% 提升至 69.8%，超过显式记忆基线 42.0%；在真实 Franka 任务中，阶段成功率从 70.7% 提升至 91.5%，完整任务成功率从 52.5% 提升至 80.0%。

Conclusion: DiM-WAM 通过多尺度记忆和进度监督有效解决了长期任务中的时间依赖性，显著提升了机器人操作性能。
...
TL;DR: 研究发现VLA模型的语言主干存在显著冗余，移除一半LLM模块甚至能提升性能，而视觉和动作路径的冗余性较低。


<details>
  <summary>Details</summary>
Motivation: VLA模型继承自预训练VLM，其语言骨干容量远超机器人短指令需求，因此需要探究模型哪些部分对闭环控制真正必要。

Method: 提出Drop-Then-Recovery (DTR)协议和GateProbe灵敏度指标，通过移除变换器块并微调来评估冗余性。

Result: 语言主干高度冗余，移除一半LLM模块后OpenVLA-OFT在LIBERO上的成功率从95.0%提升至98.3%；仅保留两个语言块仍可恢复基线性能。

Conclusion: 当前VLA基准测试对深度语言理解压力不足，未来架构应更均衡地分配语言、视觉和动作组件的能力。
...
TL;DR: 提出了一种基于脉冲神经网络的高效具身导航架构SpikeVLA，通过事件驱动计算大幅降低能耗和延迟。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型基于大型Transformer，推理延迟高、能耗大，难以在低功耗实时场景部署。

Method: 包含三个组件：脉冲视觉编码器、多模态脉冲大语言模型、脉冲动作策略网络，采用脉冲动力学和事件驱动稀疏性。

Result: 在导航和机器人控制任务上显著降低能耗和计算成本，同时保持竞争性性能。

Conclusion: SpikeVLA展示了低功耗实时具身智能的潜力。
...
TL;DR: 本文提出一种控制器框架，使量产车在超过250ms执行器延迟下实现稳定漂移，横向误差1.1米，侧偏角超调0.06弧度。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶漂移仅适用于瞬时扭矩和独立驱动的平台，在具有执行器延迟和机械耦合轴的量产车上可行性未知。

Method: 设计预测器补偿动力总成延迟，改进控制公式以适应高延迟和差速耦合，引入基于刹车的速度稳定。

Result: 实验显示，量产运动车在圆形和八字漂移中保持稳定，延迟>250ms时误差可控。

Conclusion: 量产车可实现自动驾驶漂移，为传统控制失效时的先进安全系统铺路。
...
### [165] [LXD-SLAM: LiDAR+X Dense SLAM with $\sum_{i=0}^{5}C_5^i$ Configurable Sensor Combinations](https://arxiv.org/abs/2606.27811)
*Zhong Wang,Lin Zhang,Linfei Li,Ying Shen,Shaoming Zhang,Pengcheng Shi,Shengjie Zhao*

Main category: cs.RO

TL;DR: 提出了一个高度模块化的多传感器融合SLAM框架LXD-SLAM，支持多种传感器组合，实现了实时且全局一致的稠密建图。


<details>
  <summary>Details</summary>
Motivation: 现有SLAM系统在复杂环境中面临几何退化与传感器漂移问题，且多传感器融合系统缺乏模块化，存在数学不一致或建图次优的局限性。

Method: 采用数学统一的迭代误差状态卡尔曼滤波与自适应分层预测策略，基于多层高斯过程子网格进行环境建模，并引入扩展扫描上下文描述符与双向PnP优化实现鲁棒的闭环检测。

Result: 在多个公共数据集和真实实验中，LXD-SLAM在不同配置下匹配或超越了当前最先进的专用里程计解决方案，同时实时生成高质量、全局一致的稠密网格。

Conclusion: LXD-SLAM是一种通用、高效且具有高度模块化的多传感器融合SLAM框架，适用于多种自主系统平台。
...
### [166] [Booster Lab: A Data-Centric Pipeline for Learning Deployable Humanoid Locomotion Policies](https://arxiv.org/abs/2606.27813)
*Penghui Chen,Tinglong Zheng,Yufeng Zhang,Mingguo Zhao*

Main category: cs.RO

TL;DR: 提出一个数据驱动的训练与部署管道，整合动作数据整理、真实到仿真模型适应、基于AMP的强化学习和仿真到真实部署，实现人形机器人自然、可物理实现的动作学习。


<details>
  <summary>Details</summary>
Motivation: 机器人可行运动数据稀缺，原始人类演示与机器人形态不兼容，开源视频质量参差不齐，仿真收集的轨迹仍需可行性检查。

Method: 包括动作数据整理、真实到仿真模型适应、基于对抗运动先验（AMP）的强化学习，以及仿真到真实部署流程。

Result: 在Booster T1机器人上验证框架有效性，并在Booster K1上提供初步跨平台验证。

Conclusion: 该数据为中心的管道能生成既符合任务导向又物理自然、可跨平台转移的动作策略，解决数据稀缺问题。
...
### [167] [PPO-EAL: Exact Augmented Lagrangian Proximal Policy Optimization for Safe Robotic Control](https://arxiv.org/abs/2606.27861)
*Jiatao Ding,Songqun Gao,Andrea Del Prete,Matteo Saveriano*

Main category: cs.RO

TL;DR: 提出PPO-EAL框架，结合精确增广拉格朗日优化与近端策略优化，实现安全机器人控制中的精确约束满足。


<details>
  <summary>Details</summary>
Motivation: 现有安全强化学习算法难以在满足物理约束的同时高效学习策略，且缺乏理论保证。

Method: 使用带裁剪的策略更新与精确二次惩罚项结合，并引入动量调节乘子更新以提高稳定性。

Result: 在多个机器人基准测试中（如倒立摆、四足运动）实现了优于其他算法的安全精度与奖励性能，并在零样本仿真到实物部署中提升任务成功率、降低接触力。

Conclusion: PPO-EAL是一种通用且可实际部署的安全强化学习框架，适用于各类安全关键机器人系统。
...
TL;DR: 提出S$^2$-VLA框架，通过状态空间引导的自适应注意力机制动态融合视觉、语言和动作特征，显著提升长程机器人操作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在长程任务中因静态特征融合机制导致误差累积，性能下降。

Method: 设计状态空间引导自适应注意力（SSGAA），利用信念状态跟踪任务进度并生成动态门控权重，自适应融合视觉、任务意图和动作序列特征。

Result: 仅2B参数量的S$^2$-VLA在LIBERO和SimplerEnv等基准上超越7B模型，达到最先进性能。

Conclusion: 自适应特征融合对提升长程机器人操作任务表现至关重要。
...
### [169] [LocalNav: Distilling Frontier VLMs and Embodied RL for On-Device Object Goal Navigation](https://arxiv.org/abs/2606.27871)
*Nicolas Baumann,Liam Boyle,Pu Deng,Edoardo Ghignone,Boyang Sun,Marc Pollefeys,Luca Benini,Michele Magno*

Main category: cs.RO

TL;DR: 提出一种知识蒸馏策略，将前沿VLM的空间语义推理能力迁移至轻量级4B参数模型，结合边缘部署优化，实现移动机器人上低延迟的本地视觉语言导航。


<details>
  <summary>Details</summary>
Motivation: 现有VLM计算开销大，通常依赖云端执行，难以在资源受限的机器人上进行低延迟本地部署。

Method: 1）使用Claude Sonnet 4.6构建基于场景图（SG）的SotA管线；2）用500条前沿推理轨迹微调Qwen3.5-4B；3）引入带Token生成（TG）正则化的E-RLVR，压缩输出序列长度，并配合量化。

Result: 微调后小模型成功率达34.5%（云端大模型39.7%），TG开销降低72.1%，延迟降低71.8%，联合量化后总推理延迟减少82.8%，性能损失很小。

Conclusion: 所提策略能有效平衡性能与效率，为移动机器人上本地、低延迟VLM执行提供了可行方案。
...
### [170] [Swarm sign language: motion-based communication between drones](https://arxiv.org/abs/2606.27883)
*Thomas Rey,Julien Moras,Alexandre Eudes,Antoine Manzanera*

Main category: cs.RO

TL;DR: 提出一种基于运动视觉的静默通信方法，用于无人机集群在无线信号受限环境下的信息交换。


<details>
  <summary>Details</summary>
Motivation: 传统无线电通信易被干扰或阻塞，需要一种非主动发射信号的替代通信方式。

Method: 利用模块化平面轨迹作为视觉信号，接收端通过位姿估计与3DTrajDecoder网络实现轨迹分类、分割及大小/法向量回归，并采用在线生成管道训练。

Result: 通过真实实验与仿真验证了系统在特定操作域内的有效性，消融研究揭示了架构选择与局限。

Conclusion: 基于运动的视觉通信可作为隐身约束下无人机群的有效通信手段，但受限于轨迹复杂度与环境条件。
...
### [171] [Building a Scalable, Reproducible, Evaluatable, and Closed-Loop Simulation Environment Foundation for Embodied Intelligence Cloud-Native Simulation Infrastructure for Embodied Intelligence Training, Evaluation, and Data Collection](https://arxiv.org/abs/2606.27962)
*Junwu Xiong,Yongjian Guo,Mingxi Luo,Ning Qiao,Lei Kang,Song Wang,Yince Gao*

Main category: cs.RO

TL;DR: 提出了一种支持具身智能大规模训练、标准化评估和仿真数据采集的云原生仿真基础设施框架。


<details>
  <summary>Details</summary>
Motivation: 解决真实机器人数据采集的高成本、有限可扩展性和差可重复性问题。

Method: 采用云原生技术（弹性资源调度、容器化仿真、统一数据管理、面向服务设计）构建四层架构，集成D-VLA等代表性系统。

Result: 实现了大规模仿真、动态调度、视觉增强和实时数据过滤，支持多模型和多任务工作负载。

Conclusion: 云原生仿真基础设施为数据生成、模型训练、标准化评估和真实部署提供统一基础，将在具身智能发展中发挥关键作用。
...
### [172] [When Multi-Robot Systems Meet Agentic AI:Towards Embodied Collective Intelligence](https://arxiv.org/abs/2606.27929)
*Yuxuan Yan,Yuanyuan Jia,Qianqian Yang*

Main category: cs.RO

TL;DR: 本文提出并探讨了Embodied Collective Intelligence (ECI)概念，即机器人团队通过共享世界上下文、任务进度和技能经验实现集体智能，并通过导航研究证明了共享世界记忆继承的潜在收益。


<details>
  <summary>Details</summary>
Motivation: 随着具身AI向代理化发展以及机器人研究从单机器人向多机器人系统演进，现有机器人团队在共享地图、任务分配和数据集方面存在局限，需要共享代理循环产生的状态，从而推动了对Embodied Collective Intelligence的研究。

Method: 通过综述具身AI的代理化趋势和多机器人合作演变，提出ECI框架（Co-Perception、Co-Action、Co-Evolution），并用导航案例研究验证共享世界记忆继承。

Result: 导航研究表明，新加入的机器人可以从合并的团队记忆中受益，但该研究仅为ECI框架的一个具体组件的初步验证。

Conclusion: ECI作为多机器人具身智能的一个有前景的方向，通过共享资源可以提升团队协作能力，但需要进一步全面评估其框架和实际应用。
...
### [173] [PA-BiCoop: A Primary-Auxiliary Cooperative Framework for General Bimanual Manipulation](https://arxiv.org/abs/2606.28192)
*Bai Qicheng,Wang Ziru,Ma Teli,Dai Guang,Wang Jingdong,Wang Mengmeng*

Main category: cs.RO

TL;DR: 提出了一种基于动态主辅臂分工的双臂协作框架PA-BiCoop，通过共享编码器和专用解码器实现高效协调，在仿真和真实任务中分别提升48%和50%以上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有双臂操控方法缺乏臂间交互或忽视动态分工，无法有效适配任务阶段变化。

Method: 将手臂分为主臂和辅臂，主臂解码器生成基座坐标位姿和核心任务热图，辅臂解码器输出相对于主臂坐标系的位姿，并设计动态角色分配模块自动映射左右臂。

Result: 在RLBench2仿真任务中平均超越基线48%，在真实世界任务中平均超越50%以上。

Conclusion: PA-BiCoop通过动态主辅臂分工和知识共享，显著提升了双臂操控的效率和灵活性。
...
### [174] [Translation as a Bridging Action: Transferring Manipulation Skills from Humans to Robots](https://arxiv.org/abs/2606.28133)
*Sijin Chen,Kaixuan Jiang,Haixin Shi,Yanhui Wang,Weiheng Zhong,Haosheng Li,Bo Jiang,Yuxiao Liu,Xihui Liu*

Main category: cs.RO

TL;DR: 提出了一种基于相对手腕平移的桥接动作表示，使双手机器人能更有效地从人类动作数据中学习操作技能，并构建了π₀类视觉-语言-动作模型以处理不同形态的动作缺失。


<details>
  <summary>Details</summary>
Motivation: 人类动作数据丰富廉价，但直接将人体6DoF位姿估计用于机器人存在噪声大、接触模式差异导致的迁移困难问题，亟需一种跨形态的共享动作空间。

Method: 以初始头部摄像机坐标系下的相对手腕平移作为桥接动作表示，并设计带有交错动作令牌和注意力掩码的π₀类视觉-语言-动作模型来适应不同执行器。

Result: 在新型双手机器人操作任务中，该桥接动作表示的迁移效果显著优于直接使用噪声6DoF人体动作，且性能随人类数据量增加而提升。

Conclusion: 相对手腕平移是一种有效的人机共享动作表示，结合交错式动作token设计能实现人体到机器人的高效技能迁移。
...
TL;DR: 提出通过将灵巧操作技能分解为简单组件，并利用经典物理和控制理论约束引导学习，实现稳定高效的操作技能探索。


<details>
  <summary>Details</summary>
Motivation: 传统解析模型假设强且效率低，强化学习端到端探索因学习目标冲突而不稳定低效，亟需结合理论与学习的混合方法。

Method: 将灵巧技能分解为多个简单可分析组件，每个组件在经典物理和控制理论约束下学习。

Result: 在稳定抓取、物体重新定位/定向等任务中，该方法对传感器噪声、延迟和摩擦条件鲁棒，学习高效稳定。

Conclusion: 理论先验知识能显著提升强化学习在灵巧操作中的效率与稳定性。
...
TL;DR: 本文提出Uni-Mo流水线，利用大语言模型和视频扩散模型生成四足机器人运动数据，无需动物媒介，实现了7,488种语言标注的多样化运动数据集及高成功率实际部署。


<details>
  <summary>Details</summary>
Motivation: 现有四足机器人行为局限，缺乏多样化表达；模仿人类运动数据的方法依赖动物、跨物种重建脆弱、形态不兼容。

Method: Uni-Mo流水线：大语言模型生成运动提示，视频扩散模型合成机器人行为，通过身份一致性损失提取3D参考轨迹，训练跟踪策略部署到真实机器人。

Result: 生成7,488种运动数据集（18.5小时），在真实Go2上96.7%成功率，模拟中97.6%成功率。

Conclusion: Uni-Mo实现了从文本到四足机器人多样化运动的自动生成与部署，突破数据获取瓶颈。
...
TL;DR: SimFoundry是一个模块化自动化系统，能从视频零样本构建仿真就绪的数字孪生场景，支持对象、场景和任务编辑，生成的数字同仁可用于训练机器人策略，实现零样本迁移到真实复杂任务。


<details>
  <summary>Details</summary>
Motivation: 真实世界中训练和评估机器人策略成本高且难以规模化。

Method: 通过视频输入，模块化自动化构建仿真场景，支持编辑生成多样化的数字同仁（保留功能属性的场景变体），并在这些合成数据上训练机器人策略。

Result: 在7个操作任务和5种策略架构中，仿真评估与真实性能的皮尔逊相关均值0.911，最大排序违规0.018；使用对象、场景和任务变体训练的策略在真实世界中成功率分别提升17%、21%和40%。

Conclusion: SimFoundry能高效生成逼真的仿真场景，其数字同仁使策略在零样本条件下适应新真实环境，验证了仿真训练对现实任务的预测性和迁移能力。
...
TL;DR: 本文提出Certified CacheMPC框架，通过缓存和复用MPC解，在保证安全性和次优性上界的同时，实现四足机器人25倍的速度提升。


<details>
  <summary>Details</summary>
Motivation: 标准MPC的每个周期QP求解限制了嵌入式处理器上的更新速率，而四足步态状态空间有界，因此MPC解可缓存复用以提高效率。

Method: 采用局部敏感哈希缓存水平接触力轨迹，通过后验证书验证原始可行性和对偶间隙，结合Top-K检索、限时QP求解和缓存回退的调度策略。

Result: 在Unitree Go2上实现25倍仿真速度提升和18.7倍硬件速度提升，且闭环稳定性与无缓存基线无显著差异。

Conclusion: 缓存加速有效，但证书对闭环安全的贡献在当前样本量下无法验证。
...
### [179] [DexCompose: Reusing Dexterous Policies for Multi-Task Manipulation with a Single Hand](https://arxiv.org/abs/2606.28323)
*Dihong Huang,Zhenyu Wei,Zhuxiu Xu,Yunchao Yao,Sikai Li,Mingyu Ding*

Main category: cs.RO

TL;DR: 提出了DexCompose框架，通过显式手指级动作所有权和双重残差模块，解决多任务灵巧操作中的冲突问题，实现77.4%的复合成功率。


<details>
  <summary>Details</summary>
Motivation: 现有灵巧操作策略可以解决单个技能，但组合多个任务时，手指和接触模式的重叠需求会导致破坏性干扰。

Method: 先收集第一技能的后任务状态，通过释放测试识别必要手指，再训练有界残差稳定器和上下文自适应残差模块，在分配的动作子空间中调整下游策略。

Result: 在16个复合任务上平均成功率为77.4%。

Conclusion: 结构化的动作所有权与双重残差为灵巧技能组合提供了有前景的方向。
...
### [180] [WARP-RM: A Warp-Augmented Relative Progress Reward Model for Data Curation](https://arxiv.org/abs/2606.28320)
...
TL;DR: 提出WARP方法，通过时间扭曲增强实现自监督密集进度信号学习，有效过滤机器人演示数据中的次优动作，显著提升模仿学习在物理机器人任务中的成功率。


<details>
  <summary>Details</summary>
Motivation: 人类遥操作产生的混合质量演示包含犹豫和恢复动作，现有进度奖励模型存在标签噪声或需昂贵人工标注，亟需无需监督的进度估计方法。

Method: 1. 对成功演示进行时间扭曲增强（变速播放和逆向）生成帧级进度目标；2. 训练WARP-RM预测帧间归一化时间差；3. 聚合窗口预测得密集进度信号；4. 在行为克隆中通过WARP-BC加权高优势动作块（基于帧级奖励聚合）。

Result: 在双机械臂T恤折叠任务中，当扩增低质量数据时，WARP-BC保持19/20成功率，传统BC降至2/20，吞吐量提升18倍。

Conclusion: WARP自监督学习框架能有效从次优演示中提取高质量进度信号，提升机器人模仿学习的鲁棒性和效率。
...
### [181] [Comparison of Non-Deterministic Nonlinear Systems](https://arxiv.org/abs/2606.27464)
*Shivam Bajaj,Varun S. Madabushi,Maegan Tucker,Vijay Gupta*

Main category: eess.SY

TL;DR: 提出非线性系统的$(T_e,γ,δ)$-相似性概念，通过微分耗散性建立与微分动力学的等价关系，并转化为LMI可行性问题。


<details>
  <summary>Details</summary>
Motivation: 扩展线性系统的相似性概念到非线性系统，以量化系统输出差异并支持分层控制与抽象模型改进。

Method: 定义$(T_e,γ,δ)$-相似性，与微分耗散性建立等价关系，用线性矩阵不等式（LMI）可行性问题求解。

Result: 提供LMI可行性问题的充要条件，并在平面飞机、Moore-Greitzer模型和电子电路应用中验证有效性。

Conclusion: 该相似性概念能有效应用于非线性系统的鲁棒分层控制和抽象模型改进。
...
### [182] [Threshold Optimization and Dynamic Adaptation of Distributed Optimal Power Flow in 5G Networks](https://arxiv.org/abs/2606.27542)
*Biswajit Kumar Dash,Garrett Thomas,Adedoyin Inaolaji,Filippo Malandra*

Main category: eess.SY

TL;DR: 通过真实5G通信实验平台，评估通信性能对ADMM分布式最优潮流的影响，并提出动态阈值机制提升收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有研究多假设理想通信，缺乏真实5G环境下ADMM性能的评估，需要探索通信感知策略以应对实际网络波动。

Method: 基于IEEE 123节点不平衡配电系统，划分为5个区域，各区域由Raspberry Pi本地控制器运行ADMM；设计延迟阈值机制，并动态调整阈值。

Result: 静态阈值减少7.75%收敛时间，动态阈值进一步减少26.42%。

Conclusion: 通信感知的自适应控制策略能有效提升分布式优化在真实通信网络中的性能，适用于智能电网部署。
...
### [183] [Real-Time State Estimation in Smart Grids over 5G Networks: Experimental Validation Using Raspberry Pis and Typhoon HIL](https://arxiv.org/abs/2606.27642)
*Biswajit Kumar Dash,Luis Herrera,Filippo Malandra*

Main category: eess.SY

TL;DR: 本文通过在商业5G网络上搭建基于树莓派和硬件在环仿真器的多节点测试平台，实验验证了5G网络在智能电网实时状态估计和故障检测中的性能，结果表明5G的端到端延迟比LTE Cat-M低约6.5倍，且可实现0.80秒的快速故障检测。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多依赖仿真或分析模型，缺乏基于真实硬件和智能电网数据的实验验证，因此需要填补这一空白以确认5G实际部署效果。

Method: 使用树莓派作为SG节点，结合Typhoon HIL实时模拟器搭建5G多节点测试平台，通过不同上报速率和部署环境评估关键性能指标，并基于IEEE 4节点模型进行状态估计和故障检测实验。

Result: 5G最差情况平均延迟比LTE Cat-M低6.5倍；5G测量数据在稳态和动态负载下均支持精确状态估计；故障检测延迟低至0.80秒。

Conclusion: 5G网络能够有效支持智能电网的实时监测与控制，提供低延迟、高可靠性的通信保障。
...
### [184] [Characterizing Driver Interactions with Autonomous Vehicles via Response Maps](https://arxiv.org/abs/2606.27656)
*Dave Broaddus,Rachel DiPirro,Chishang,Yang,Dan Calderone,Wendy Ju,Meeko Oishi*

Main category: eess.SY

TL;DR: 本文通过博弈论中的响应地图概念，基于驾驶模拟器数据，将人类驾驶员在自动驾驶车辆交互中的加速行为建模为线性反馈函数，并发现其对AV的不同行为（让行、不让行、响应）有显著差异。


<details>
  <summary>Details</summary>
Motivation: 理解人类对自动驾驶车辆行为的响应是实现社交兼容导航的关键，现有研究缺乏对人类驾驶行为作为AV行为函数的定量表征。

Method: 采用基于博弈论的响应地图建模人类驾驶行为，利用驾驶模拟器实验数据，用线性函数表示人类加速度与AV行为之间的耦合关系。

Result: 人类驾驶员的加速度行为可通过响应地图有效捕捉，且对AV的让行、不让行和响应三种行为表现出显著不同的响应模式。

Conclusion: 响应地图能有效量化人类与AV互动中的驾驶行为，为社交感知自主导航提供了行为预测依据。
...
### [185] [A Bi-Layer TSN Formulation for Separable Scheduling of Mobile Emergency Resources](https://arxiv.org/abs/2606.27712)
*Wei Wang,Minwu Chen,Yinyu Chen,Jingyan He*

Main category: eess.SY

TL;DR: 提出一种精确双层时空网络模型来解决可分离调度中的运载工具-模块时空耦合优化问题，相比现有逻辑模型具有更好的计算性能。


<details>
  <summary>Details</summary>
Motivation: 可分离调度通过分离运载工具与功能模块的调度来提升电网韧性，但运载工具-模块的时空耦合导致优化问题难以精确求解。

Method: 构建双层时空网络模型，将运载工具和模块轨迹表示为相互作用的网络流，并通过弧级耦合约束强制执行支撑关系，转化为混合整数线性规划。

Result: 验证了模型的正确性，并展示了相比基于逻辑的模型显著的计算优势。

Conclusion: 该模型在保持精确性的同时提高了建模灵活性，为可分离调度优化提供了有效方案。
...
### [186] [Bearing-based Circumnavigation with Collision Avoidance in Time-varying Graphs under Limited Target Information](https://arxiv.org/abs/2606.27719)
*Kushal Pratap Singh,Twinkle Tripathy,Anoop Jain*

Main category: eess.SY

TL;DR: 本文研究了异构多智能体系统对静止目标的分布式环绕问题，考虑了智能体的物理尺寸，通过控制角速度设计了分布式制导律，并利用对数障碍Lyapunov函数避免碰撞。


<details>
  <summary>Details</summary>
Motivation: 现有工作多假设智能体为质点且所有智能体直接获取目标位置，而实际中智能体有物理尺寸且只有部分智能体能感知目标。

Method: 将智能体建模为圆盘，领导者（已知目标位置）和跟随者（仅利用局部信息）通过最近邻交互图通信，通过控制角速度实现环绕，并用BLF保证碰撞避免。

Result: 在静态和时变交互图下，所有跟随者最终都能与领导者一起环绕目标，前提是初始条件可行。

Conclusion: 提出的分布式制导律有效，适用于多种交互拓扑，并能保证避碰和收敛性能。
...
### [187] [Repair-before-veto control for safe lithium-ion fast charging under unknown ambient and cooling-fault conditions](https://arxiv.org/abs/2606.27781)
*Yifan Wang*

Main category: eess.SY

TL;DR: 提出了一种基于边界感知的快速充电安全控制框架RACL-B，可在未知电池状态和冷却系统健康条件下实现快速充电，同时确保热安全，并通过实验验证其在不同温度和冷却条件下的有效性。


<details>
  <summary>Details</summary>
Motivation: 实际充电过程中，环境温度和冷却系统状态未知，单一充电设置可能导致过热或充电失败，影响电池寿命和安全性。

Method: 采用边界感知的“先修复后阻止”控制策略，根据电压、温度和负极锂析出边界实时调整充电电流，避免固定调度或完全停机。

Result: RACL-B在九种不同温度和冷却条件下均安全完成充电，比最安全的固定电流快37.9%，且锂析出最少。CREST-B故障监测方法能有效定位冷却故障起始点。

Conclusion: 该框架提供了可部署的热安全保证和故障监测方法，适用于实际快速充电场景。
...
### [188] [From Detection to Action: Using LLM Agents for Fault-Tolerant Control](https://arxiv.org/abs/2606.28011)
*Javal Vyas,Milapji Singh Gill,Artan Markaj,Felix Gehlhoff,Mehmet Mercangöz*

Main category: eess.SY

TL;DR: 提出了一个基于大语言模型的多智能体容错控制框架，结合数字孪生和知识图谱，能在离散和连续过程中生成并验证恢复动作。


<details>
  <summary>Details</summary>
Motivation: 传统容错控制方法缺乏灵活性和语义理解能力，难以处理复杂工业过程中的知识关联和实时约束。

Method: 多智能体工作流（监控、规划、合成、仿真、验证、重提示）+ 数字过程工厂孪生（提供数据和仿真服务）+ 基于CPSMod本体的图检索增强生成层（支持多跳语义查询）。

Result: 在批处理混合模块和连续搅拌反应器模拟中，使用轻量级LLM（GPT-4o-mini等）能在时间预算内生成有效的恢复决策。

Conclusion: 该框架为从故障检测到已验证的校正动作提供了一条实用路径，适用于离散和连续容错控制任务。
...
### [189] [Decentralized Stability of IBR-dominated Power Grids Using Block Diagonal Dominance](https://arxiv.org/abs/2606.28023)
*Youhong Chen,Xiaoyu Tan,Muhammad Sharjeel Javaid,David Angeli,Balarko Chaudhuri*

Main category: eess.SY

TL;DR: 提出了一种基于块对角占优的分散小信号稳定性判据，用于逆变器主导的电力系统，确保稳定性且条件比严格对角占优更宽松。


<details>
  <summary>Details</summary>
Motivation: 逆变器资源渗透率增加，需要可扩展、分散且不依赖模型的稳定性评估方法。

Method: 开发了块对角占优判据，并基于此提出一种可从局部评估的并网稳定性合规条件。

Result: 该方法比严格对角占优保守性更低，且在无限制网络或逆变器模型假设下保证最小衰减率或最大振荡衰减时间。

Conclusion: 为逆变器主导电网提供了实用且理论基础的分散稳定性证书。
...
### [190] [Effects of motion cueing on longitudinal acceleration perception in a driving simulator](https://arxiv.org/abs/2606.28055)
*Erik Gustaf Lilljebjörn,Sogol Kharrazi,Jan Åslund,Martin Singull*

Main category: eess.SY

TL;DR: 该研究通过驾驶模拟器实验评估了重型卡车在急加速和起步测试中加速度差异的恰可察觉差，发现不同运动提示算法的JND无显著差异，平均为5.4%，且驾驶员偏好针对起步工况优化的算法。


<details>
  <summary>Details</summary>
Motivation: 传统重型卡车动力总成驱动性评估依赖物理样机，成本高、周期长，而基于虚拟样机的人机交互驾驶模拟器可实现早期评价，需验证驾驶员能否感知纵向加速度微小差异。

Method: 使用加权阶梯程序获取心理测量函数，通过广义线性模型分析两种针对tip-in/起步测试优化的运动提示算法与通用算法的恰可察觉差差异。

Result: 不同运动提示算法的JND无显著差异（均值5.4%），主观等同点-1.9%表明驾驶员感知第二刺激加速度更高，且多数参与者偏好针对起步优化的算法。

Conclusion: 运动提示算法优化不影响加速度差异感知能力，但驾驶员偏好针对特定工况的算法，验证了虚拟样机驱动性评估的可行性。
...
### [191] [Specification-aware Robustness Margins for Symbolic Controllers](https://arxiv.org/abs/2606.28143)
*Youssef Ait Si,Antoine Girard,Adnane Saoud*

Main category: eess.SY

TL;DR: 本文研究鲁棒控制器综合问题，针对线性时序逻辑规范下的扰动系统族，利用符号控制技术找出使系统在相同控制器下仍满足同一规范的最大扰动。


<details>
  <summary>Details</summary>
Motivation: 现有方法在确保控制器对系统扰动的鲁棒性时过于保守，需要一种规范依赖且更精确的鲁棒性度量。

Method: 基于固定点算法生成的符号控制器，通过细化到扰动系统版本保持正确性，对不同时序规范（安全性、可达性、持久性、循环性）定义最大鲁棒裕度，并推导定制化理论保证。

Result: 鲁棒裕度明确依赖于固定点计算生成的序列，相比通用抽象方法更不保守，在示例中展示了实际有效性。

Conclusion: 该方法提供了规范依赖的鲁棒性分析，具有更小保守性和实际适用性。
...
### [192] [PAC-Bayesian Certificates for Quadratic Closed-Loop Control](https://arxiv.org/abs/2606.28281)
*Domagoj Herceg*

Main category: eess.SY

TL;DR: 本文提出了使用PAC-Bayes界对基于学习的控制系统进行有限样本保证的方法，通过系统级合成参数化处理二次轨迹成本，并给出了数据驱动的控制选择算法。


<details>
  <summary>Details</summary>
Motivation: PAC-Bayes界适用于随机预测器，但应用于控制时，二次轨迹成本函数无界、非李普希兹，导致认证困难。

Method: 采用系统级合成参数化线性系统的闭环轨迹，推导PAC-Bayes-Chernoff证书；对于高斯扰动，得出精确单侧高斯变换和可处理的二次上界；提出后验局部化替代方案；最小化数据驱动界以得到控制选择学习算法。

Result: 算法作为灵敏度感知的有限样本正则化器，在低数据区改善了保留成本和闭环灵敏度。

Conclusion: PAC-Bayes界可有效用于控制系统的有限样本认证，数据驱动算法能提高控制性能。
...
### [193] [Automated brain tumor detection in MRI images using CNN and ResNet architectures](https://arxiv.org/abs/2606.27405)
*Annapurna V K,Asha N,K Paramesha,Shabana Sultana,Kirankumar Humse*

Main category: eess.IV

TL;DR: 提出了一种基于CNN和ResNet的深度学习方法，用于MRI图像中的脑肿瘤自动检测，采用迁移学习实现肿瘤与非肿瘤分类。


<details>
  <summary>Details</summary>
Motivation: 脑结构复杂、手动解读依赖性强，早期诊断困难，需要自动、准确的检测方法。

Method: 使用卷积神经网络和残差网络（ResNet18、ResNet50），结合迁移学习，对3929张脑MRI图像进行分类。

Result: ResNet18准确率97%，优于ResNet50的96%，表明浅层模型在有限医疗数据上泛化更好。

Conclusion: 该方法实现了快速、准确的脑肿瘤检测，有助于早期诊断和临床决策。
...
### [194] [DFM: Difference Feature Modeling with Text-Guided Gated Contrastive Loss for Remote Sensing Image Change Captioning](https://arxiv.org/abs/2606.27410)
*Yelin Wang,Zijia Song,Chuanguang Yang,Miaoyu Wang,Zhulin An,Libo Huang,Yongjun Xu*

Main category: eess.IV

TL;DR: 提出了一种新的遥感图像变化描述框架DFM，通过文本引导对比损失和联合特征建模提升变化描述的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有RSICC模型依赖自回归生成范式，倾向于学习易生成词汇而忽略图像间的判别性差异。

Method: 引入文本引导门控对比损失(TGCL)指导视觉编码器提取关键特征，结合预训练变化检测模型传递稳定知识，并设计联合特征建模(JFM)模块融合多尺度差异表示。

Result: 在多个数据集上的实验证实了该方法的有效性。

Conclusion: DFM框架通过重构训练范式和特征建模能更好地捕捉多时相图像的时空变化。
...
### [195] [MLVC: Multi-platform Learned Video Codec for Real-World Deployment](https://arxiv.org/abs/2606.28027)
*Tanel Pärnamaa,Martin Lumiste,Ardi Loot,Evgenii Indenbom,Andrei Znobishchev,Ando Saabas*

Main category: eess.IV

TL;DR: 提出MLVC，首个兼顾跨平台鲁棒性、实时速度和竞争性压缩性能的神经视频编解码器。


<details>
  <summary>Details</summary>
Motivation: 现有神经视频编解码器因跨平台不兼容和高计算成本难以部署，量化方案在不同硬件上产生非确定性结果导致解码失败。

Method: 通过超先验显式传输尺度参数保证熵编码一致性，结合门控记忆、ReGLU激活、长期参考恢复机制和领域特定感知训练优化。

Result: 在VCD基准上相比硬件HEVC实现>70% BD-rate（MOS）提升，主观质量接近DCVC-RT；在Apple、Intel、Qualcomm NPU上达100 FPS。

Conclusion: MLVC是首个能在多种消费设备上实现竞争性能、实时速度和跨平台鲁棒性的神经视频编解码器，适合大规模部署。
...
### [196] [Enhanced Neural Video Representation Compression across Extreme Complexity and Quality Scales](https://arxiv.org/abs/2606.28163)
*Ho Man Kwan,Tianhao Peng,Fan Zhang,Mike Nilsson,Andrew Gower,David Bull*

Main category: eess.IV

TL;DR: 提出了NVRC++，一种基于隐式神经表示的轻量级视频编解码器，通过多分辨率特征网格和高效优化框架，在保持低复杂度下实现高扩展性和实时解码。


<details>
  <summary>Details</summary>
Motivation: 现有神经视频编解码器在复杂度和可扩展性之间难以平衡，轻量模型在不同比特率/质量下性能下降，而高性能模型复杂度随质量增加，限制了实际部署。

Method: 使用轻量级INR结合多个高分辨率特征网格，设计优化框架实现高效过拟合，利用时空冗余，并引入高级熵模型压缩高维网格参数。

Result: 提供4个复杂度级别，支持宽比特率和质量范围，实时解码。相比SOTA的NVRC，解码速度提升高达7.6倍，性能相当。

Conclusion: NVRC++能在任意复杂度下保持高可扩展性，实现高效视频压缩和快速解码，适合实际应用。
