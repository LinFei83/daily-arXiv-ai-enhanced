...
TL;DR: 本文介绍了FineServe，一个从全球商业平台收集的多模型LLM服务真实工作负载数据集。通过对异构模型和任务的细粒度分析，揭示了到达动态和令牌行为的差异，并提供了相应的负载生成器和系统优化基准。


<details>
  <summary>Details</summary>
Motivation: 现有研究多基于代理轨迹或粗粒度特征，无法捕捉现代多模型LLM平台的异构性，需要真实、细粒度的服务负载来优化系统性能。

Method: 通过从全球商业平台收集真实多模型服务负载（FineServe），分析不同模型架构、规模和任务意图下的到达动态与令牌行为，开发细粒度模型感知的负载生成器。

Result: 揭示了不同模型架构和任务意图下根本不同的波动特性，并生成了可配置的多模型服务性能测试负载。

Conclusion: FineServe提供了评估路由、调度和容量规划策略的真实基础，有助于提升LLM服务系统的效率。
...
TL;DR: 提出了一个混合框架 FraudShield AI，结合 LSTM 和手工图拓扑特征，以检测复杂的洗钱模式。


<details>
  <summary>Details</summary>
Motivation: 金融机构面临数据极度不平衡（欺诈率 0.13%）和对抗性规避策略的挑战，现有方法难以检测 smurfing 和 layering。

Method: 集成 LSTM 网络与 PageRank Centrality、In-Degree 动态和 Flow Ratio 等图拓扑特征，使用 Focal Loss 处理类别不平衡，并引入动态阈值机制。

Result: 在 PaySim 数据集上，该模型在 Precision、Recall 和 F1-Score 上优于 Logistic Regression 和 XGBoost，特别是在微交易欺诈模式上。

Conclusion: 混合模型能有效提升洗钱检测性能，时序和图拓扑组件的互补贡献通过消融研究得到验证。
...
TL;DR: 提出一种针对LLM多智能体系统的共演化持续防御框架OpenEvoShield，通过解耦攻防学习速率、动态行为边界更新、EWC正则化策略集成以及多粒度异常检测，在动态攻击下显著优于静态和持续基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有防御将部署视为封闭世界问题，无法应对攻击策略和正常智能体行为的双重动态漂移，导致性能迅速下降。

Method: 包含四个组件：非对称速率控制器(M1)解耦攻防学习速率；正常边界更新器(M2)维持动态行为边界；EWC正则化策略集成(M3)快速适应；基于能量的多粒度检测器(M4)融合节点、子图和图级证据检测异常。

Result: 在5个基准和4种拓扑上的100轮实验中，OpenEvoShield检测到大部分未见攻击，同时保持低误报率。

Conclusion: OpenEvoShield能够在动态威胁环境下实现持续有效的防御，优于现有静态和持续防御方案。
...
TL;DR: 在Intel TDX机密实例中的NVIDIA H100 GPU上，针对Mistral-7B和Qwen3-30B-A3B模型，机密计算模式相比非机密模式增加首令牌延迟21.8%-27.8%，降低全局吞吐量17.7%-21.1%，且大模型更早达到饱和。


<details>
  <summary>Details</summary>
Motivation: 机密计算正成为处理敏感输入或保护模型资产的AI推理需求，但其对GPU加速的大语言模型服务的性能影响尚不明确，需要量化评估。

Method: 在单NVIDIA H100 GPU（Intel TDX机密实例）上，对Mistral-7B和Qwen3-30B-A3B两个模型进行基准测试，测量首令牌时间、端到端延迟、每请求令牌吞吐量、全局令牌吞吐量和闭环请求吞吐量，并在不同并发度下对比机密与非机密模式。

Result: 固定请求率下，机密模式使平均TTFT增加21.8%（Mistral-7B）和27.8%（Qwen3-30B-A3B），全局令牌吞吐量分别下降17.7%和21.1%。闭环并发实验中，吞吐量差距在11.5%-20.2%之间，但大模型在机密模式下更早达到饱和拐点。

Conclusion: 机密GPU推理在负载下仍可保持可用吞吐量，但容量规划需考虑稳定的吞吐量惩罚和更大模型的早期饱和行为。
...
TL;DR: 提出FORMULASPIN自我对弈框架，通过可执行性反馈解决公式生成的训练矛盾，在无额外数据下超越传统微调方法。


<details>
  <summary>Details</summary>
Motivation: 现有电子表格公式生成方法受限于静态监督数据，模型性能在有限标注上迅速饱和；直接应用自我对弈会产生梯度冲突。

Method: 将训练建模为双人博弈：主模型学习偏好正确公式，同时利用二进制可执行性提供隐式监督，按语义正确性→风格优化的自适应课程学习，并引入ExecVote投票机制处理有效变体。

Result: 在NL2FORMULA基准上达到74.9%精确匹配和87.1%执行准确率，优于传统监督微调及前沿专属模型。

Conclusion: 自我对弈在标注稀缺任务中潜力巨大，且可通过可执行性扩展至其他领域。
...
TL;DR: 本研究提出信息辨别框架Learn2Discern，发现LLM在来源辨别和真相辨别上表现接近随机，依赖来源流行度而非可靠性，且对改善或恶化先验的更新几乎无差异。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地使用外部知识源，但其是否适当权衡信息（可靠来源更新多、接近真相的声明更新多）尚未明确。

Method: 基于三个规范性公理设计Learn2Discern框架和基准，包含可解释指标；进行预注册、配额匹配的用户研究（n=299）验证外部效度，并在13个模型近67万次试验中测试。

Result: 模型在来源和真相辨别上接近随机，依赖来源流行度（两倍于可靠性），对改善或恶化先验的更新无差异；新/大模型提升真相辨别但未改善来源辨别；简单推理时干预可改善两种辨别。

Conclusion: LLM在信息辨别上存在系统性缺陷，来源辨别是模型复杂度的盲点，但可通过推理时干预缓解；该能力随模型取代传统搜索而愈发重要。
...
TL;DR: 提出NEXUS，一个基于形式化干预策略的LLM代理安全监控器，可执行允许、阻止、请求确认或请求修订四种操作。在多个基准测试中表现优于纯规则方法，延迟低至0.205ms。


<details>
  <summary>Details</summary>
Motivation: LLM代理执行高影响操作，需要运行时安全监控以确保安全。

Method: 结合确定性安全规则、参数级检查和校准的逻辑回归风险评分，实现分级的正式干预策略。

Result: 在128实例合成基准上F1=0.949，4类干预准确率0.6406（比纯规则高27.3%）；R-Judge上F1=0.861；0%攻击成功率下99%控制允许；NEXUS-Stress基准F1=0.881；延迟0.205ms。

Conclusion: NEXUS在低延迟下显著提升安全监控性能，但细粒度干预路由仍具挑战。
...
TL;DR: 提出了一种轻量级推理时解码层，通过在自回归生成推荐系统中进行在线约束优化，实现多目标推荐，无需修改或重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 现有多目标方法不适用于生成式推荐系统，后者需考虑序列性质且大型系统难以重新训练。

Method: 将解码过程建模为在线约束优化，通过随机原始-对偶近似动态平衡相关性和辅助目标。

Result: 在离线实验和大型在线A/B测试中，辅助目标提升+1.8%，用户满意度零损失。

Conclusion: 该解码层能有效改进多目标权衡，且不影响用户满意度。
...
TL;DR: LISA将线性注意力和稀疏注意力结合，通过门控机制将计算复杂度从O(n²)降至O(nM)，在16K上下文下实现50%推理加速，并在推理基准上平均提升5.6%性能。


<details>
  <summary>Details</summary>
Motivation: 长链式推理模型（如DeepSeek-R1）在测试时缩放范式中产生越来越长的上下文，但标准自注意力的O(n²)计算复杂度导致推理成本激增，限制了其实际部署。

Method: LISA包含两个并行组件：线性注意力（O(n)复杂度）和闪电索引器（选择top-M重要token进行稀疏自注意力），通过门控机制融合；采用两阶段训练：第一阶段通过知识蒸馏初始化线性注意力与滑动窗口注意力，第二阶段引入索引器替代滑动窗口，并使用逐头KL散度损失训练索引器以匹配教师模型的注意力模式。

Result: 在DeepSeek-distilled-Qwen模型上，LISA在16K token上下文下实现50%推理加速，并在AIME和MATH-500等推理基准上平均提升5.6%性能。

Conclusion: LISA是一种即插即用的注意力替换模块，无需从头预训练，能有效降低长上下文推理成本并提升性能，适用于长链式推理模型的实际部署。
...
TL;DR: 提出多跳记忆基准MemHop（5层深度1000题）与双层次记忆架构ProGraph（简历扩展+压缩残差），在社交场景中实现80.1%准确率，超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有记忆基准仅评测单跳回忆，缺乏对多跳关联（跨会话长期记忆）的测量。

Method: 两层架构：简历扩展（子串匹配实体名）和压缩残差（精确日期/数量/名称，零额外成本）；含网格消融实验。

Result: MemHop 80.1%匹配全上下文基线；LoCoMo 78.4%超基线11.3pp；消融表明简历扩展驱动多跳推理（-22.6pp），压缩残差驱动精确召回（-8.6pp）。

Conclusion: 简单符号化简历扩展+轻量提取残差的组合高效实现跨会话关联，可替代显式知识图谱。
...
TL;DR: 提出提升表示假设，通过共享潜在结构更新记忆，研究LLM在嵌套规则与例外数据中的提升与破碎现象。


<details>
  <summary>Details</summary>
Motivation: 探索LLM如何存储、选择和修正通用规则结构，以及这一过程的内在机制。

Method: 设计受控例外学习实验，比较上下文学习、LoRA和全微调三种范式下的提升与破碎行为。

Result: LLM在嵌套规则与例外数据中易出现破碎失败（过早细分），同时提升行为常过早发生。

Conclusion: 需深入研究数据与规则结构之间的关系，以改进LLM的泛化能力。
...
TL;DR: 提出GraphContainer平台，统一并可视化不同图RAG工作流，支持多格式图标准化与检索过程追踪。


<details>
  <summary>Details</summary>
Motivation: 现有图RAG方法格式碎片化、不兼容，缺乏精细可视化工具，难以评估和比较检索行为。

Method: 设计统一图表示层（UGR）标准化多格式图，以及Graph Recorder追踪并可视化逐步检索过程，通过交互式网页实现。

Result: 实现异构图导入、实时可追踪可视调试，支持图格式和检索策略的受控比较。

Conclusion: GraphContainer降低了设计和优化图RAG管道的门槛，有助于研究人员和从业者进行评估。
...
TL;DR: 提出AdaRoPE，为每个注意力头学习可调的旋转频率和缩放因子，替代标准RoPE的全局统一设置，提升长上下文性能。


<details>
  <summary>Details</summary>
Motivation: 标准RoPE对所有注意力头使用统一的频率和缩放，忽略了不同头所需的不同功能，导致维度利用不足和长上下文性能下降。

Method: 为每个注意力头引入可学习的旋转频率和注意力缩放因子，使位置编码适应每个头的特定角色。

Result: 在预训练和长上下文扩展中，AdaRoPE优于标准RoPE、部分RoPE和NoPE基线，并改善了YaRN等方法在长短上下文上的平衡。

Conclusion: 在注意力头级别优化旋转位置嵌入是提升Transformer长上下文性能的关键。
...
TL;DR: 提出了一种基于SAE特征的可解释激活引导方法，使用六条件可靠性过滤和Borda排名选择特征，在Gemma模型上实现特定领域的行为控制，但发现有效引导高度依赖于模型、领域、层和强度。


<details>
  <summary>Details</summary>
Motivation: 现有基于SAE的激活引导方法依赖学习目标或单标准特征选择，缺乏透明性和可靠性。

Method: 六条件可靠性过滤、Borda共识排名（F检验、KSG互信息、Cohen's d）、Cohen's d加权SAE解码器行构建引导方向，近似Fisher-LDA。

Result: 在Gemma 2 9B上逻辑正确性引导达到+1.16主评分差，但可用引导高度局部化；质量保留与原始行为变化之间存在显著差距。

Conclusion: 激活引导评估应报告质量条件成功率，而不仅仅是原始行为变化。
...
TL;DR: 提出一种逻辑引导的数据提取框架，结合大语言模型与回答集编程，通过交错推理减少LLM调用并提升提取质量。


<details>
  <summary>Details</summary>
Motivation: LLM在语义数据提取中缺乏复杂组合推理与全局一致性，需要更可靠的约束机制。

Method: 逻辑引导框架：LLM生成候选事实，ASP执行验证、推理、一致性检查与引导查询，并采用缓存机制减少求解器调用。

Result: 在ASP基准测试中，LLM调用次数减少，提取质量提高，虚假输出得以缓解。

Conclusion: 非单调逻辑编程能有效控制语义提取，提升效率与可靠性。
...
TL;DR: 提出一种基于稀疏自编码器（SAE）特征的安全对齐方法，将约束数量K固定为2，在Qwen3.5-9B上实现多类别安全分类（96-99%准确率），并引入锥体约束自适应聚类特性。


<details>
  <summary>Details</summary>
Motivation: 解决Safety as Polytope (SaP)方法需要针对每个类别手动调整约束数量K的问题，提升安全对齐的自动化和泛化性。

Method: 使用SAE提取隐空间特征，将平面约束数固定为K=2；并引入可学习孔径的锥体约束，通过三阶段训练稳定收敛。

Result: 在BeaverTails分类基准上，Qwen3.5-9B的14个类别中12个达到最优（K=2），准确率96-99%，消除了K=4-25的穷举搜索。

Conclusion: 证实安全边界在SAE特征空间中具有低维线性描述，锥体约束有效适应不同类别簇的集中度。
...
TL;DR: 论文提出C1度量框架，从结构一致性、忠实性和有用性三个维度评估LLM置信度估计的连贯概率性质，发现当前模型虽校准良好却普遍违反这些条件。


<details>
  <summary>Details</summary>
Motivation: 校准标准不足以评估LLM置信度，因为它无法识别琐碎不一致的估计、依赖于评估分布，且不检验估计能否解释为一致的潜在概率函数。

Method: 形式化概率信念所需的三个条件（结构一致性、忠实性、有用性），并将其操作化为C1度量。

Result: 广泛使用的估计器系统性地违反这些条件，尽管它们看起来校准良好；常见干预措施（如校准优化）未改变结构违规。

Conclusion: 当前LLM置信度估计不能被解释为连贯的概率，C1框架为测量和弥合这一差距提供了工具。
...
TL;DR: 本文提出Spectral-LSH，一种训练即用的提示压缩方法，通过Krylov子空间和随机特征近似注意力核主导成分，避免二次复杂度，并在注意力特征空间中使用SimHash聚合相似令牌为宏令牌，实现高压缩比下质量保持。


<details>
  <summary>Details</summary>
Motivation: 长提示推理成本高，预填充注意力随序列长度二次增长，现有压缩方法在高质量与低延迟间难以兼顾。

Method: 使用Krylov子空间方法结合随机特征近似注意力核算子，避免显式O(N²)计算；然后在注意力特征空间中应用SimHash对相似令牌分组，并赋因果位置聚合成宏令牌。

Result: 在C4上测试Mistral-7B、Qwen2.5-7B和14B，发现压缩比相变点：低于4×时轻量分块最优，高于8×时谱路径保质量；16×时Qwen2.5-7B PPL比从353.409降至196.963，14B从9.533降至3.427；在结构化长上下文测试中，8×局部LSH优于分块。

Conclusion: Spectral-LSH在高压缩比场景下优于传统分块，自适应后端可结合两种机制，但分块在延迟上仍具优势。
...
TL;DR: 在有限信息序列模型中，隐状态探针可能恢复潜在标签，但本文通过无限注Limit Hold'em模型证明，大部分对手范围信号可由可见下注组成解释，而非残差隐状态结构，提出“组成有界预测支持”的证据模式。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明隐状态探针可恢复对手范围信息，但未区分这是源于真实的信念分布还是可观察的公共信息。

Method: 训练一个仅使用动作和价值目标（而非对手手牌或范围）的自回归模型，通过探针分析、行为预测和组成基线比较，验证对手范围信号的可解释性。

Result: 对手范围探针在2/3种子中阳性，但可见下注组成能解释更多信号；残差隐状态探针准确率降至11.4-12.2%，匹配组成比较在所有种子中为负。

Conclusion: 隐状态中可恢复的范围信息主要由可见组成驱动，不应简单视为信念追踪证据；需通过针对性替代假设验证探针结果。
...
TL;DR: 提出了Scaffold-Preserving Representation Alignment框架，通过表征对齐减少LLM苏格拉底导师在长对话中的脚手架坍塌问题，将坍塌率降至32%，并将坍塌起始延迟至九轮以上。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的苏格拉底导师在持续学生压力下会逐渐放弃引导式提问而直接给出答案，现有方法主要限制输出但未解决内部表征漂移导致坍塌的问题。

Method: 两阶段框架：先用监督微调预热导师，再结合轨迹加权直接偏好优化和基于冻结参考状态的边界保持表征损失，以维持脚手架保持与坍塌诱导隐藏状态之间的分离。

Result: 在Qwen3-8B上，五门STEM学科与五种红队攻击下，坍塌率降至32%，平均坍塌起始延迟超过九轮，且过度拒绝率低。

Conclusion: 表征级对齐能提升长时苏格拉底式辅导在红队协议下的鲁棒性。
...
TL;DR: 提出了Euclean框架，将几何问题自动形式化为Lean中与Mathlib兼容的形式，构建了最大规模的几何形式化数据集，并验证了数据集质量能提升定理证明性能。


<details>
  <summary>Details</summary>
Motivation: 现有形式化系统（如Lean）在处理几何方面存在碎片化问题：几何依赖专门的领域专用语言，缺乏统一的形式化保证，且现有几何Lean库（如LeanEuclid）与标准Mathlib不兼容，规模小，阻碍了统一模型发展。

Method: 设计了Euclean四阶段框架：约束显式化、配置锚定、形式化映射和迭代修复，自动将几何问题形式化为原生Mathlib形式。

Result: 构建了OMNI-Geometry（768个竞赛问题）和Numina-Geometry（177,597个问题）数据集。人类评估TOP1准确率48.89%，TOP5准确率73.33%。训练Goedel v2模型后，证明成功率从13.6%提升至15.1%。

Conclusion: Euclean框架有效解决了几何形式化中的关键挑战（如隐式假设显式化），生成的数据集质量高，能增强神经定理证明器的性能，推动了统一形式化系统的发展。
...
TL;DR: 提出了CrackedPDFs基准用于检测PDF中的隐藏提示注入，混合检测器在受控评估中达到0.960 F1，但显示了对真实世界鲁棒性的局限。


<details>
  <summary>Details</summary>
Motivation: 现有文档型LLM系统常扁平化PDF后进行检查，这一过程可能丢弃用户并未看到的指令证据，导致隐藏提示注入风险被忽视。

Method: 构建包含29,322个生成PDF的基准数据集，采用混合检测器（结合结构和文本特征），并与PromptGuard、规则基线及纯结构/文本模型进行对比，使用留出法、配对混淆控制、标签洗牌和捷径审计等评估手段。

Result: 混合检测器在2,919个文档的留出测试集上达到0.960 F1、0.998 ROC-AUC和0.997 PR-AUC；在973个配对中，95.9%的情况下将注入文件排在良性混淆文件之前。但纯文本TF-IDF模型在捷径审计中失败，结构模型在配对控制下表现弱。
...
TL;DR: 提出HyGRL框架，通过将非结构化文本嵌入知识图谱形成异构网络，利用两阶段学习（模仿+强化）实现自适应结构推理，在低成本和近实时推理下提升多实体组合问答的准确性与忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有RAG缺乏动态推理、传统Graph-RAG受限于结构稀疏性、LLM构建的Graph-RAG成本过高，难以处理多实体组合问题。

Method: 构建非结构化文本与结构化知识图谱的异构网络，采用两阶段训练：模仿学习蒸馏启发式专家信号，再以LLM驱动的偏好奖励进行强化学习优化策略。

Result: 在答案准确性和推理忠实度上超越SOTA基线，同时保持极低的token成本和近实时推理性能。

Conclusion: HyGRL有效融合文本丰富性与结构知识，适用于多实体组合问答场景。
...
TL;DR: 本文提出ITPEval，首个跨四个主要定理证明器（Lean 4、Rocq、Isabelle、HOL Light）的形式化证明翻译基准，涵盖控制层与生态系统层，评估显示当前LLM翻译成功率低，库不匹配是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 形式化定理证明生态系统碎片化，证明被隔离在不兼容系统中，限制了学习型证明器的训练数据和验证结果的移植性。

Method: 构建1,560个源文件和6,848个定理的基准，分为控制层（隔离基础翻译难度）和生态系统层（暴露API和证明风格差异）；开发统一的多ITP验证基础设施itpeval，采用状态隔离的预热后端；评估5种前沿和开源LLM在12个翻译方向上的语句和证明翻译性能。

Result: 语句翻译最高29.1% pass@1，证明翻译最高10.5%；控制层定理证明pass@1为29.7%，生态系统层仅为5.2%；Lean 4 BEq检查显示54.0%的验证翻译语义等价；多ITP上下文将Lean 4成功率从4.8%提升至10.6%。

Conclusion: 库不匹配是翻译的主要瓶颈；原生类型检查高估语义保真度；Rocq和HOL Light比Lean 4和Isabelle更容易作为形式化目标；多ITP上下文有助于翻译提升。
...
TL;DR: 提出了FORCE-Bench基准测试，包含251个专家标注查询，从八个维度评估运营金融领域中的智能体系统，发现通用智能体系统在运营约束下未能持续满足金融领域质量要求，而专用智能体更可靠。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试未直接针对运营金融工作流，而金融专业人士要求智能体提供事实准确、可验证且符合领域规则的信息。

Method: 构建FORCE-Bench基准，包含251个专家标注查询，使用基于评分标准的框架评估八个维度，涵盖财务义务研究、实体绩效研究和商业简报生成三类任务。

Result: 通用智能体系统在运营约束下未能持续满足金融领域质量要求，而专为Microsoft 365 Copilot构建的Finance Agent在各维度上更可靠。

Conclusion: FORCE-Bench能有效评估运营金融领域的智能体系统，开源数据集和分析代码支持可重复比较和适应其他企业金融环境。
...
TL;DR: 本文定义了Chronos漏洞，即基于记忆的攻击威胁（如MINJA和休眠代理），并证明传统终端内容过滤器不足以防御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 从无状态生成模型到有状态自主代理的架构演进引入了长期规划能力，但也带来了新的安全威胁。

Method: 通过形式化威胁模型和World of Workflows基准测试，分析基于持久性的攻击和动态盲视威胁。

Result: 传统端点内容过滤器失效，需使用深度防御框架如AgentDoG、Agent-C、A-MemGuard和GPU-TEEs。

Conclusion: 有状态代理架构需要新的防御策略，包括轨迹护栏、时间验证、内存共识和硬件信任。
...
TL;DR: 本文论证了精致推理的核心计算角色是使规划变为闭环，并展示了其可被认知先验变分自由能框架表示。在反应迷宫基准上的实验表明，认知驱动与闭环控制结合才能有效解决问题。


<details>
  <summary>Details</summary>
Motivation: 厘清精致推理在主动推理中的计算角色，特别是其闭环控制结构相对于树搜索的独立性。

Method: 在认知先验变分自由能框架中，通过联合后验（未来状态与行动相关）表征闭环控制，并与动作-状态因子化、标准预期自由能规划等方法在随机基准任务中对比。

Result: 孤立使用认知成分或闭环控制均不能成功；精致推理和全联合认知先验主动推理通过结合两者解决了任务。

Conclusion: 精致推理的优势源于主动推理的闭环形式，而非树搜索本身；该形式可通过保持未来行动依赖于未来状态的变分后验实现。
...
TL;DR: 提出知识中心自我改进范式，通过维护共享知识库而非优化特定代理，实现泛化且低成本的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有代理中心自我改进方法成本高、难迁移，需探索知识作为持久对象的替代范式。

Method: 构建通用代理+知识库协议：代理完成任务后通过任务级/跨任务论坛生成证据洞见，再经知识蒸馏写入知识库。

Result: 在抽象推理、编程等基准测试中，该方法相比代理中心基线提升解决率并降低美元成本；蒸馏知识可迁移至未见任务及不同大模型族。

Conclusion: 自我改进系统可主要由持久化知识驱动，提供可检查、可迁移、可移植的新路径。
...
TL;DR: 综述边缘智能在民航领域的应用，提出边缘AI通过压缩、协作推理和分裂学习降低延迟、保护隐私、提升离线韧性，与云互补。


<details>
  <summary>Details</summary>
Motivation: 云端AI在民航中产生高延迟、依赖通信、数据集中化风险，而边缘AI能缓解这些问题。

Method: 阐述操作动机，综述边缘推理与学习技术，介绍组织计算范式及民航环境配置，描述应用和趋势。

Result: 边缘解决方案能提供低延迟、隐私保护和有韧性的AI服务。

Conclusion: 边缘智能与云互补，可优化民航全生命周期的AI服务。
...
TL;DR: 提出了SFgen，一种基于多模态大语言模型的PCB符号和封装自动识别与生成流程，实现了80%以上的准确率并构建了包含1000个组件的SFnet数据库。


<details>
  <summary>Details</summary>
Motivation: 传统PCB设计中，工程师手动创建符号和封装并设计原理图，耗时且易错。

Method: 利用多模态大语言模型（MLLMs）构建代理式识别与生成流程SFgen。

Result: 符号生成准确率86%，封装生成准确率80%；建立了包含1000个组件的SFnet数据库。

Conclusion: SFgen方法为PCB设计的自动生成奠定了基础。
...
TL;DR: 针对多模态代理搜索系统中的隐性失败（silent failures），研究者构建了六类分类法，并开发了轨迹级诊断流程。实验表明，表面准确率高估了真实轨迹正确性，且隐性失败具有能力依赖性。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注最终答案准确性，可能忽略搜索轨迹中的潜在失败，因此需要系统化研究这些隐性可靠性问题。

Method: 提出包含六类隐性失败的分类法（模态捷径、幻象基础、错误证据正确答案、过度检索、跨模态矛盾、来源幻觉），并建立统一的ReAct框架轨迹级诊断流程，结合交叉判断验证、空白图像压力测试和工具消融实验。

Result: 对四种前沿多模态模型在MMSearch-Plus轨迹上的实验显示，表面准确率持续高估轨迹级真实性；且隐性失败具有能力依赖性，常发生转移而非消失。

Conclusion: 隐性失败是评估多模态智能搜索系统的关键维度，需超越最终答案准确率进行轨迹级诊断。
...
TL;DR: 提思维清单奖励是一种基于过程的奖励方法，通过将偏好对转化为样本特定思维清单并评估推理轨迹，结合指数移动平均残差公式，提升了语言模型与人类偏好对齐的性能。


<details>
  <summary>Details</summary>
Motivation: 现有成果级奖励主要为最终响应打分，对推理轨迹引导不足，导致多响应得分相似时偏好未明确。

Method: 提出思维清单奖励：将偏好对转化为思维清单评估推理过程，并通过指数移动平均残差公式分离成果级奖励的互补部分。

Result: 在三个模型家族的五个模型上，思维清单奖励一致提升了对齐性能，消融实验验证了残差公式和清单监督的重要性。

Conclusion: 过程导向奖励有效改善了偏好对齐，为推理轨迹提供更细粒度指导。
...
TL;DR: 该论文提出了一种针对AI智能体的侦察驱动渗透测试框架KYA，通过自动化识别智能体的知识资产和弱点，从而生成更强的间接提示注入攻击。


<details>
  <summary>Details</summary>
Motivation: 传统渗透测试依赖侦察发现弱点，而现有研究忽视了AI智能体的侦察过程，导致攻击效率受限。

Method: 形式化智能体侦察过程，识别其提取的知识资产，并设计KYA框架：通过黑盒探测、构建目标画像、利用画像生成攻击。

Result: 在智能体安全基准和真实编码智能体上验证了KYA的有效性，并开源了框架、基准和实现基线。

Conclusion: 智能体侦察是提升攻击强度的关键，KYA能自动化实现该过程，为AI安全评估提供新方法。
...
TL;DR: 本文提出DocOps，一个基于层次化分类法的确定性可验证评估框架，用于评估自主智能体对数字文档的操作能力。


<details>
  <summary>Details</summary>
Motivation: 自主智能体可靠操作数字文档的能力对于通用AI助手和复杂工作流自动化至关重要。

Method: 构建等级制分类法，将文档操作分解为原子维度和逐步复杂的任务，并基于此系统评估开源和闭源模型。

Result: 最先进的模型在处理高度耦合、长程任务时仍存在严重局限性，并发现三种关键失败模式：长期状态跟踪崩溃、浅层语义验证及结构性元数据破坏性编辑。

Conclusion: 工作暴露了智能体在维护全局文档一致性方面的能力边界，为未来设计鲁棒、非破坏性的复杂数字生态系统智能体提供了启示。
...
TL;DR: 提出了一种面向长时间尺度的智能体安全框架Janus，通过联合优化预期和裁决任务训练守卫模型Vanguard，在四个基准测试中平均保护率提升15.9%，良性任务完成率提升5.1%。


<details>
  <summary>Details</summary>
Motivation: 智能体安全正从内容审核转向预防工具使用智能体在行动前的操作失败，现有方法缺乏对延迟风险的预见能力。

Method: 通过多智能体模拟合成多样化轨迹，学习共享策略的耦合任务：预期任务预测安全相关未来，裁决任务结合观测前缀与预期未来判断安全性，使用CoAA-RL联合优化。

Result: 在四个智能体安全基准上，Vanguard相比基线守卫平均保护率提升15.9个百分点，良性任务完成率提升5.1个百分点。

Conclusion: Janus框架能有效延迟风险预见并提高智能体安全性能，同时减少对良性任务的干扰。
...
TL;DR: 提出ERQDP方法，基于动态规划和离散化回报网格，解决有限时域MDP在根风险目标下的优化问题，无需枚举或采样，并提供显式上界-下界误差保证。


<details>
  <summary>Details</summary>
Motivation: 根风险目标（如秩依赖效用）为非线性，破坏贝尔曼最优性，导致传统动态规划失效，且情景树枚举计算上不可行。

Method: 通过秩-分位数替代代理求解精确动态规划，在离散化回报网格上计算策略的回报概率质量函数，通过迭代精化循环输出目标值的上界-下界证书。

Result: 在测试基准中返回有保证的解或显式残差间隙，支持风险参数快速扫描（显著节省运行时），同时适用于风险厌恶和风险追求行为。

Conclusion: ERQDP是首个无枚举、无采样的求解根风险MDP的方法，能提供可验证的优化保证，并支持多种风险偏好。
...
TL;DR: 提出了MOF-Sleuth，一个通过强化学习引导的晶体信息文件诊断代理，能够提供基于证据的解释、错误类型和二元决策，在多项基准测试中达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在大规模金属有机框架数据库的细微化学和结构错误检测上缺乏细粒度归因和可靠解释能力，直接使用大型语言模型成本高且不可靠。

Method: 采用两个模块：确定性法医实验室（提取组成、几何等证据）和侦探推理引擎（生成解释与诊断），并通过奖励引导的强化学习将工具测量转化为化学解释级监督。

Result: 在四个基准测试中，基于大型语言模型的方法和金属有机框架专用机器学习方法上均达到最先进性能，提升了检测、归因和基于证据的解释质量。

Conclusion: 证明了强化学习的证据引导策略能有效增强大型语言模型在金属有机框架晶体结构诊断中的可靠性和可解释性。
...
### [38] [EvoThink: Evolving Thinking in Large Reasoning Models via Self-Pruning and Aha-Moment Preference Optimization](https://arxiv.org/abs/2607.19962)
*Xinbang Dai,Zheyu Xin,Huikang Hu,Lin Ren,Rihui Jin,Guohui Xiao,Guilin Qi,Kuicai Dong,Zhaocheng Du,Yuyang Zhang*

Main category: cs.AI

TL;DR: 提出了EvoThink框架，通过自剪枝训练和顿悟偏好优化，在提升推理效率的同时增强推理能力。


<details>
  <summary>Details</summary>
Motivation: 大推理模型的过度思考问题源于冗余验证步骤，现有方法无法细粒度区分有益和冗余步骤，可能损害推理能力。

Method: 采用自剪枝训练无监督地迭代剪枝冗余步骤并自训练简洁轨迹；以及受遗传算法启发的顿悟偏好优化，从失败尝试中合成从错误到正确的顿悟数据并优化模型。

Result: 在数学推理和代码生成基准上，显著减少了推理时的令牌使用量，并提升了推理能力。

Conclusion: EvoThink能同时提高推理效率和能力，有效区分冗余与有益推理步骤。
...
### [39] [SenWorld: A Digital-Twin Simulation for Generating Context-Rich Evaluation Data](https://arxiv.org/abs/2607.19949)
*Zenghui Zhou,Xiaoyang Li,Xiaoxuan Qiao,Zhilang Wei,Tianming Lei*

Main category: cs.AI

TL;DR: SenWorld是一个物理接地、确定性、事件源的数字孪生仿真平台，能生成带有固定正确标签、保护隐私且可复现的评估数据，用于评测智能手机助手。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法缺乏具有已知正确答案的上下文丰富数据，且真实设备数据因隐私问题难以共享。

Method: 基于真实地图、天气、节假日和网络数据构建数字孪生世界，让角色在其中度过完整一天，所有信号存档为快照，评估案例通过指向现有记录而非后验标注或LLM判定。

Result: 生成数据在类别分布和日常节奏上与真实用户基准高度匹配（JSD<0.1），检测出生产级助手在通话和短信记录上的78处失败，且失败被确认为助手的检索错误。

Conclusion: SenWorld提供了一条隐私安全、可复现且分布匹配的评估数据生成路径，其标签通过构建固定。
...
TL;DR: 论文指出当前AI模型（如Transformer）与大脑皮层在结构上的根本差异，主张应基于神经科学的结构多样性证据，设计异构拓扑网络而非同质化缩放模型。


<details>
  <summary>Details</summary>
Motivation: 揭示了从卷积神经网络到Transformer的结构进化中，忽略了神经科学中功能结构与计算需求匹配的关键教训。

Method: 通过追踪神经科学（细胞架构、Patch-seq）和AI发展史（CNN到Transformer、MoE），结合功能主义分析，提出异构拓扑网络的设计框架。

Result: 表明Transformer更类似于海马体而非通用皮层，且MoE的‘多样性’实为参数分割，最终提出了以模块化接口为核心的系统设计方法。

Conclusion: AI架构应基于神经科学的结构证据进行模块化设计，而非通过单一模板缩放或事后逆向工程。
...
TL;DR: 提出图结构经验记忆框架，利用历史协调经验加速多智能体在动态制造环境中的适应，在动态柔性作业车间调度中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法将每个扰动事件独立处理，丢弃了可加速未来适应的协调经验。

Method: 构建异构关系图编码历史协调片段，通过图神经网络检索相似经验指导策略适应。

Result: 在三种扰动类型下，制造周期减少4.1%-10.0%，适应时间减少33%-38%，优势随扰动频率增加而扩大。

Conclusion: 图结构编码和相似性检索对经验重用至关重要，且学习到的协调模式具有跨扰动泛化能力。
...
TL;DR: 提出了CLARK框架，结合知识图谱、符号规则挖掘和概率推理，用于分类任务，在医学数据集上提升了分类性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在数据分布变化时性能下降，且难以整合先验知识。

Method: 将CACTUS知识图谱转换为LP^MLN程序，通过符号学习迭代添加候选规则，并用概率权重学习进行校准和推理。

Result: 在医学数据集上，CLARK提升了分类性能并实现了更泛化的推理。
...
### [43] [EvoDRC: A Self-Evolving Agentic Framework for Automated DRC Violation Repair](https://arxiv.org/abs/2607.20019)
*Bing-Yue Wu,Chia-Tung Ho,Haoyu Yang,Brucek Khailany,Vidya A. Chhabria*

Main category: cs.AI

TL;DR: 提出EvoDRC框架，通过技能演化实现自动化DRC修复，在七个基准设计上平均减少73.5%的违规。


<details>
  <summary>Details</summary>
Motivation: DRC闭合是先进节点物理设计的主要瓶颈，手动修复流程费时且易引入新违规，亟需自动化方法。

Method: 基于知识蒸馏初始化层特定修复技能，通过按区域分解布局、多工具反馈及经验数据库实现技能持续演化，每个区域由LLM修复代理负责。

Result: 在DAC26 DRC基准的七个模块级设计中，相比基线实现73.5%的整体违规减少。

Conclusion: EvoDRC利用技能演化和LLM代理有效自动化DRC修复，显著降低违规，无需依赖目标设计先验知识。
...
### [44] [Safe Remediation as Risk-Constrained Intervention Decision in Microservice Systems](https://arxiv.org/abs/2607.20005)
*Chengxiao Dai,Zhaokun Yan,Chenjun Lei,Qiao Li,Luyan Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种基于风险约束的自动修复决策框架，将安全修复问题建模为CMPD，通过FRR约束、三维风险分解和自适应人机协作门控机制，在降低误修复率的同时提升修复成功率。


<details>
  <summary>Details</summary>
Motivation: 现有自动修复系统优先产生操作而非判断是否需要干预，安全仅通过人工审核保障，而IT运维中错误修复的成本常高于不操作，因此需要一种主动考虑风险约束的决策方法。

Method: 将问题形式化为约束马尔可夫决策过程（CMDP），最大化修复成功率同时限定误修复率（FRR）；提出包含爆炸半径、可逆性和认知不确定性的三维风险分解；设计上下文自适应的人机协作门控机制，根据值班负荷和业务重要性调整升级策略。

Result: 在Train Ticket微服务基准测试中，相比强基线，误修复率降低39%，修复成功率提升2.5个百分点，人机升级负载减少17%。

Conclusion: 该框架通过风险约束和自适应门控有效平衡自动修复的安全与效率，为IT运维提供了可解释、可调节的智能修复决策方案。
...
TL;DR: 本文提出了一种全局传播器，用于同时处理所有差异约束，并实现了边界一致性，特别是在惰性子句生成求解器中通过解释传播来提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有有限域传播算法通常单独处理每个差异约束，导致传播虽然完备但效率低下，而SAT模理论求解器中的理论求解器虽可处理差异约束，但传播器的需求不同，且缺乏对传播的解释机制。

Method: 构建了一个全局差异约束传播器，基于SAT模理论中的理论求解器思路，但针对传播器需求进行调整，并设计了解释传播的方法以集成到惰性子句生成求解器中。

Result: 实验表明，全局处理差异约束相比标准传播方法在性能上有显著提升。

Conclusion: 全局差异约束传播器能有效加速约束求解，尤其在惰性子句生成框架中通过解释机制提高了实用性。
...
TL;DR: 论文通过实验分离了大型语言模型中的三种材料科学机制信息形式：可读概念、本构方向（通过状态转换实现）和因果控制内部表征，并证明了物理关系在受控状态变化中比在绝对状态中更可见。


<details>
  <summary>Details</summary>
Motivation: 正确输出无法揭示模型是否表示或使用了支配物理，因此需要研究模型内部机制信息的形式和可分离性。

Method: 结合匹配的直接和雅可比词汇读取、无选项状态几何、60定律反事实基准和因果干预，在50个保留材料描述和72个提示基准上进行实验。

Result: 三个独立拟合的雅可比镜头重现了概念排名；基于状态转换的可逆物理方向测试正确识别了39/40个方向性定律；双向干预在12个案例中均改变了答案概率；反事实状态补丁跨机制和答案格式传递了相反决策信号。

Conclusion: 物理关系在受控状态变化中比在绝对状态中更可见，模型通过状态转换和因果干预体现了机制信息，但数值比较也能解释部分组织。
...
TL;DR: 提出 PRO-LONG 框架，通过程序化记忆管理长序列上下文，在 ARC-AGI-3 基准上提升 LLM 智能体性能 18.0 个百分点，同时减少 4.2-5.8 倍 Token 使用量。


<details>
  <summary>Details</summary>
Motivation: 现有上下文管理方法在长期任务中面临信息保存与检索效率之间的权衡，限制了大模型智能体在持续学习基准上的表现。

Method: PRO-LONG 采用程序化内存，通过完整结构化的交互日志结合编码智能体高效搜索历史信息，实现上下文管理。

Result: 在 ARC-AGI-3 上，PRO-LONG 将基础编码智能体的平均性能提升 18.0 个百分点，达到 76.1% pass@1，且 Token 使用减少 4.2-5.8 倍；在 Fable 5 上以 1750 美元成本实现 97.4% best@2。

Conclusion: PRO-LONG 通过程序化记忆有效解决了长期任务中的上下文管理权衡，提高了性能并大幅降低资源消耗。
...
TL;DR: 提出TRUST-ESD框架，融合不确定性校准、下行风险评分、治理合规等模块，在风险调整效用、合规性等方面显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统在企业战略决策中缺乏对不确定性、风险、可解释性和治理合规的综合考量。

Method: 结合预测效用估计、共形不确定性校准、CVaR风险评分、风险记忆、策略即代码治理、可解释性和人工监督。

Result: 相比强基线，风险调整效用提升7.95%，风险暴露降低23.22%，CVaR降低23.78%，校准误差降低13.89%，解释保真度提升10.90%，治理合规提升9.76%。

Conclusion: 不确定性校准、下行风险评分、风险记忆、可解释性与治理验证共同提升了可信企业决策。
...
### [49] [PoTRE: Test-Time Reasoning inspired by Cognitive Heterogeneity](https://arxiv.org/abs/2607.20268)
*Anmol Kankariya,Sercan Ö. Arık*

Main category: cs.AI

TL;DR: 提出 PoTRE 框架，通过四种不同推理代理的异构集成，在复杂推理任务中实现了新 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 标准单流提示在长程规划、迭代纠错及新抽象概念时表现脆弱，需要更鲁棒的推理方法。

Method: 采用四个异构代理（对抗精炼、分层策略规划、谱系搜索、直接链式）并行推理，并通过任务自适应聚合层（候选选择、语义合成或神经符号验证）整合结果。

Result: 在 HLE 上达到 49.92% 准确率，超越此前最佳官方成绩；使用相同或更少推理令牌实现更优性能。

Conclusion: 架构异构性可有效提升推理鲁棒性和效率，无需过度扩展模型规模。
...
### [50] [CUSUM-Shaped Inference-Time Monitoring and Targeted Re-Decoding for Quantized Small Language Model Reasoning](https://arxiv.org/abs/2607.20129)
*El Hassane Ettifouri,Ayoub Belfatmi,Mahaman Sanoussi Yahaya Alassan,Walid Dahhane*

Main category: cs.AI

TL;DR: 提出了一种基于监控引导的推理时回溯方法（MGT-B），通过检测预采样不确定性和退化特征来纠正小自回归模型的无效轨迹，在MATH-500数据集上验证了其选择性修复效果。


<details>
  <summary>Details</summary>
Motivation: 量化的小型自回归推理模型可能进入重复或无产出的轨迹，而现有推理时计算分配未考虑轨迹发展过程，需要一种能够动态监测和修复的机制。

Method: 基于e-CUSUM控制器，MGT-B方法通过滑动窗口映射不确定性特征，计算混合赌注因子，触发报警时估计回滚点并执行约束重新解码。

Result: 在240对时间序列审计集上准确率从82/240提高到88/240（+2.50个百分点），但在467对历史覆盖集上显示更显著改进（+4.50个百分点），仅有151条触发报警的轨迹发生变化。

Conclusion: 该方法在MATH-500环境下支持选择性监控与修复机制，但未验证为通用或理论上保证的推理改进，且分析为非确定性结论。
...
### [51] [SoftReason: A Fully Differentiable Neuro-Soft-Symbolic Deductive Reasoning Architecture over High-Dimensional Perceptual Data](https://arxiv.org/abs/2607.20402)
*Wael AbdAlmageed*

Main category: cs.AI

TL;DR: 提出了软符号推理（SoftReason），一种端到端可微分的神经符号推理架构，能够从高维感知输入和知识图谱中进行演绎推理，弥合了感知与演绎之间的梯度鸿沟。


<details>
  <summary>Details</summary>
Motivation: 传统神经符号管道在感知和演绎之间存在离散接口，导致梯度无法传播，难以实现端到端训练。

Method: 通过局部软解释张量表示演绎状态，利用可微分的即时后果算子，结合谓词定义嵌入和潜在组合通道，实现软体谓词混合、见证聚合和查询条件化头部事实更新。

Result: 在知识感知视觉问答（KVQA）任务中，SoftReason实现了感知基础、知识图谱证据注入和可微分离散闭包的一体化训练。

Conclusion: SoftReason提供了一种统一的、可微分的神经符号推理框架，有效桥接了感知与演绎之间的梯度差距。
...
### [52] [Train the Model, Not the Reader: Decodability Supervision for Verifiable Activation Explanations](https://arxiv.org/abs/2607.20379)
*Hiskias Dingeto*

Main category: cs.AI

TL;DR: 本文揭示了自然语言自编码器重建测试在评估隐藏激活解释时对具体虚假声明不敏感，并提出了RECAP方法以提升可解释性和安全性。


<details>
  <summary>Details</summary>
Motivation: 现有重建测试无法区分解释中的具体真假声明，导致高重建分数可能掩盖虚假内容，需要更可靠的评估和生成方法。

Method: 提出两种审计协议（grounded-vs-true cross和evaluator swap）以及RECAP方法，通过联合训练线性头与目标模型，确保指定内容可解码。

Result: RECAP模型在沙盒实验中消除了私有编码，以极低成本（+0.001 nat）实现真实解释；在Pythia-160M上，指定内容可被可靠探测。面对对抗性攻击时，RECAP探测器的AUC达0.95，而对照组降至0.51。

Conclusion: 高重建分数不能保证具体声明真实性；RECAP通过独立探测验证内部内容，有效防御模型欺骗，提升AI安全性。
...
TL;DR: 提出将AI生成视频检测重新定义为流式感知任务，利用编码器运动场进行在线检测，实现低计算量的实时监测。


<details>
  <summary>Details</summary>
Motivation: 现有离线检测方法不适应在线部署，计算成本高且无法动态调整决策时间。

Method: 从比特流解析运动场而非解码像素，使用单调聚合的阈值实现随时有效决策，并用闭合公式计算计算预算与延迟窗口的映射。

Result: 在GenVidBench上达到AUC 0.64，计算量比像素CNN降低5个数量级；控制停止时间假阳性率，15%视频延迟使准确率从0.75提升至0.78且计算量减少7倍。

Conclusion: 无需新检测器，仅通过重构任务并提供理论保障即可大幅提升效率，验证了在线流式检测的可行性。
...
### [54] [ChronoStitch: Training-Free Composition of Visual KV Memories for Long-Horizon Temporal Reasoning](https://arxiv.org/abs/2607.19547)
*Santiram Tiwari,Nishant Sinha,Kunal Kislay*

Main category: cs.CV

TL;DR: 提出ChronoStitch方法，通过重编码视觉存储记忆的旋转位置到全局三维RoPE坐标，并选择性重计算部分视觉token，实现长视频问答中独立缓存块的正确时序组合，性能提升且速度更快。


<details>
  <summary>Details</summary>
Motivation: 独立缓存的视频块因局部旋转位置编码导致时空相位混乱，无法回答时序、频率或变化类问题。

Method: 1. 将存储的后旋转键重新映射到全局三维多模态RoPE坐标（时间、高度、宽度）；2. 选择性重计算偏差大的后续块视觉token，使其能关注组合后的缓存。

Result: 在TempCompass时序分割测试集上，Qwen2.5-VL-3B模型使用ChronoStitch超越朴素组合和仅位置修正变体，事件排序准确率提升，且运行速度比完整联合预填充快3.3倍。

Conclusion: ChronoStitch无需训练即可有效对齐独立缓存视频块的时序，克服了视觉token的一维标量重索引导致的几何不一致缺陷。
...
### [55] [Crowd4D: Scene-Aware Monocular 4D Crowd Reconstruction](https://arxiv.org/abs/2607.19517)
*Hongbo Kang,Tianyi Zhou,Qingyang Yang,Hongwei Wen,Jing Huang,Yu-Kun Lai,Kun Li*

Main category: cs.CV

TL;DR: 提出Crowd4D框架，利用场景感知先验和优化策略，从单目视频中重建大规模场景的4D人群运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法因深度模糊和复杂地形导致尺度不可靠、空间漂移。

Method: 多阶段优化策略，包含HSIP（人-场景交互代理）表示和CSCR（人群结构一致性正则化）。

Result: 在复杂大规模场景中优于现有方法，实现鲁棒的4D人群重建。

Conclusion: 场景感知先验和时空一致性约束能有效解决单目4D人群重建的挑战。
...
### [56] [D3VL: Understanding Driving Scenes from 3D Time Series Data and Video with Language Models](https://arxiv.org/abs/2607.19528)
*Heesang Han,A. Lynn Abbott,Abhijit Sarkar*

Main category: cs.CV

TL;DR: 提出D3VL框架，融合2D与3D时序数据（LiDAR/立体相机），在KITTI QA数据集上提升11%，并引入Waymo QA扩展数据集。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM多依赖2D图像，而自动驾驶中3D传感器（LiDAR）数据稀疏且无网格结构，融合困难，但3D数据可提升场景理解与安全性。

Method: 设计简洁的单一架构D3VL，整合2D与3D时序数据处理，用于交通场景问答。

Result: 在KITTI QA数据集上比基线方法提升11%；提出Waymo QA扩展数据集评估多样驾驶条件下性能。

Conclusion: D3VL有效融合2D/3D时序数据，提升自动驾驶问答任务表现，并提供了新的评估基准。
...
### [57] [Real-Time EEG Cap Electrode Detection for Guided Point-of-Care Placement](https://arxiv.org/abs/2607.20142)
*William Lehn-Schiøler,Mads Sverker Nilsson,Nicki Skafte Detlefsen*

Main category: cs.CV

TL;DR: 提出了一个两阶段视觉系统，用于在网络摄像头流中实时检测脑电图帽电极并验证其解剖位置，使用YOLO检测器结合几何验证阶段。


<details>
  <summary>Details</summary>
Motivation: 实现实时、自动化的脑电图帽电极检测和定位验证，以提高临床和研究场景中的便利性和准确性。

Method: 采用单类YOLO检测器定位电极，结合基于面部标志的几何阶段分配10-20系统角色；使用几何增强（旋转、透视、混合）和基于标志的头部裁剪。

Result: 在5名受试者的留一受试者交叉验证中，检测器达到mAP@.5 = 0.94±0.07（合并后0.96）；留一帽子验证显示中号和大号帽子性能接近，小号帽子性能下降（0.72±0.28）；几何增强提高了鲁棒性，头部裁剪将远距mAP从0.23提升至0.45。

Conclusion: 该系统在实时性（19 FPS）和准确性上表现良好，但小号帽子性能受熟悉度混淆影响；几何增强和裁剪有效提升了鲁棒性和距离适应性。
...
### [58] [Synthetic and Derived Training Images for Campus Waste Detection: A Multi-Seed Evaluation with YOLOv8n](https://arxiv.org/abs/2607.19535)
*Ali Behbahani,Newsha Javanmardi,Shahriar Ahmed,Ling Chen,Phouvadeth Vathana*

Main category: cs.CV

TL;DR: 探讨了在校园垃圾回收场景中，使用合成和衍生图像增强YOLOv8n检测器的效果，结果显示所有增强配置均未超越仅使用真实图像的基线模型。


<details>
  <summary>Details</summary>
Motivation: 不当的垃圾处理会污染校园回收流，而安装在垃圾桶上的摄像头可在丢弃物品时提供反馈，因此需要评估合成图像是否提升检测器性能。

Method: 收集148张校园照片（86训练/31验证/31测试），比较12种联合训练配置（不同数量和来源的合成图像），使用4个随机种子重复实验并计算自举百分位区间。

Result: 仅真实图像模型平均mAP@0.5为0.691；背景替换降至0.560，孤立物体为0.680，全部增强池为0.487；手部前臂复合物实验校正后差异为+0.034，不支持可靠效果。

Conclusion: 所有评估配置均未超过真实基线；报告区间量化了种子差异，但31张测试集过小，无法得出强分类结论。
...
### [59] [VQ-Transplant: Efficient VQ-Module Integration for Pre-trained Visual Tokenizers](https://arxiv.org/abs/2607.19575)
*Xianghong Fang,Yuan Yuan,Dehan Kong,Tim G. J. Rudner*

Main category: cs.CV

TL;DR: 提出了VQ-Transplant框架，能在冻结的预训练分词器中即插即用地集成新的VQ模块，仅需轻量级解码器适应（5 epochs），将训练成本降低95%，同时重建保真度接近行业级模型。


<details>
  <summary>Details</summary>
Motivation: 现有VQ模型的训练需要大量计算资源，阻碍了资源受限条件下的创新性VQ技术开发。

Method: 通过替换预训练分词器中的原生VQ模块，保持编码器-解码器参数不变；并引入轻量级解码器适应策略（仅在ImageNet-1k上训练5 epochs）来缓解量化不匹配。

Result: 在VAR等工业级模型上实现了接近最先进的重建保真度，同时将训练成本降低95%。

Conclusion: VQ-Transplant通过资源高效集成新VQ技术，提升了量化研究的可及性，且性能可与行业级重建水平匹配。
...
### [60] [EGRNet: A Lightweight Semantic Segmentation Network with Edge-Gated Refinement and Adversarial Sensing](https://arxiv.org/abs/2607.19617)
*Bareera Qaseem,Mohsin Kamal,Muhammad Naveed Aman*

Main category: cs.CV

TL;DR: 提出了一种轻量级实时语义分割模型EGRNet，在Cityscapes上以0.46M参数达到65.28% mIoU，并兼具对抗攻击检测能力。


<details>
  <summary>Details</summary>
Motivation: 推动自动驾驶和智慧城市发展需要高效且鲁棒的场景理解，但在高精度与低计算成本之间难以平衡。

Method: 采用深度可分离卷积、空洞残差块、边缘门控精炼（EGR）模块和SE注意力机制构建轻量级网络，并集成轻量化对抗攻击检测策略。

Result: 仅0.46M参数，Cityscapes数据集上mIoU达65.28%，计算开销低。

Conclusion: EGRNet在效率、准确性和鲁棒性间取得平衡，适合部署于边缘设备进行实时安全关键应用。
...
TL;DR: 本文提出将病理学家注意力引入报告生成模型训练，通过注意力对齐损失提升前列腺癌报告生成与视觉问答性能。


<details>
  <summary>Details</summary>
Motivation: 病理学家的视觉注意力在癌症诊断中至关重要，但现有报告生成模型未充分利用这一语义线索。

Method: 收集多模态注意力数据集，包含121张前列腺全切片图像的病理学家视口轨迹、口头描述和光标移动；使用注意力对齐损失微调两个报告生成模型。

Result: 在NLP指标上平均提升10.9%，五个临床相关报告组件的准确率提升19.3%；模型注意力图与病理学家注意力更一致。

Conclusion: 引入人类注意力能显著提升医学报告生成的质量和可解释性，且计算开销小。
...
### [62] [Domain Shift in Echocardiography: Interpretable Quantification and Prediction of Cross-Dataset Left Ventricular Segmentation](https://arxiv.org/abs/2607.19643)
*Soroush Elyasi,Nasim Dadashi Serej,Julie Wall,Massoud Zolgharni*

Main category: cs.CV

TL;DR: 本文研究了超声心动图左心室分割的跨数据集泛化问题，发现域转移主要源于视野和成像角度差异，而非声学差异。通过几何感知预处理可改善部分迁移退化，并利用不同表示空间的指标预测分割性能下降。


<details>
  <summary>Details</summary>
Motivation: 跨数据集泛化是超声心动图左心室分割临床应用的主要障碍，但域转移的来源很少被明确区分。

Method: 使用手工超声特征、VAE潜在特征和分割衍生潜在特征，在六个超声心动图数据集上估计迁移退化。采用几何感知预处理、强度z归一化，并比较了CMD、log-Wasserstein和log-MMD等差异度量。

Result: 几何感知预处理显著改善了部分迁移情况；强度归一化对数据集可分性影响极小；绝对Dice下降的预测R平方为0.612，MAE为0.082；不同表示空间中，CMD、log-Wasserstein和log-MMD分别是最佳差异度量。

Conclusion: 超声心动图域转移是结构化和可测量的，可通过几何感知预处理和特定表示空间的迁移风险估计来部分缓解其分割影响。
...
### [63] [A Unified Variational Framework for Deep Weakly Supervised Image Segmentation](https://arxiv.org/abs/2607.19669)
*Yin King Chu,Lingfeng Li,Sung Ha Kang,Jianping Zhang,Xue-Cheng Tai*

Main category: cs.CV

TL;DR: 提出一个统一的变分框架，用于在稀疏像素级监督下进行图像分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理稀疏标签和图像统计不均匀性时存在局限，需改进分割性能。

Method: 基于单形约束的Potts模型和平滑周长正则化，利用RKHS函数扩展构建模糊隶属函数以捕捉不均匀强度统计。

Result: 在实验中，该方法在图像分割任务中表现出鲁棒性，优于非训练和部分交叉熵基线，且无需完整标注图像。
...
### [64] [PhenSPINE: A Standardized Benchmark for Spine Pathology Diagnosis](https://arxiv.org/abs/2607.19696)
*Duong Ngoc Vu,Hai Son Nguyen,Trong-Nghia Nguyen,Bien Tran Van,Trang Mai Xuan,Huan Vu,Thien Van Luong*

Main category: cs.CV

TL;DR: 提出PhenSPINE数据集（16813张MRI图像），发现单序列Sagittal T2在脊柱诊断中优于多模态融合。


<details>
  <summary>Details</summary>
Motivation: 现有脊柱诊断数据集缺乏多样性，且序列选择对性能影响未充分研究。

Method: 基于CNN骨架和位置编码的基准模型，评估4种MRI序列。

Result: Sagittal T2序列Macro F1为50.31%，多序列融合因噪声干扰而性能下降。

Conclusion: 单序列T2是最优选择，多序列融合需解决噪声问题。
...
### [65] [SafeGen: Goal-Conditioned Video Diffusion of Safety-Critical Scenarios for VLM-Based Autonomous Driving](https://arxiv.org/abs/2607.19701)
...
TL;DR: 提出SafeGen框架，通过目标条件扩散生成安全关键场景，用于评估视觉语言自动驾驶系统。


<details>
  <summary>Details</summary>
Motivation: 现有安全场景生成方法依赖仿真器，存在仿真到现实差距，无法捕捉真实多样化的人车交互动态。

Method: 基于目标条件扩散过程，利用上下文推理潜在漏洞生成终端状态，并通过深度感知几何投影和边界条件扩散生成连贯视频。

Result: 在3个VLMAD上，SafeGen使评估分数平均提高24.25%，微调后真实驾驶性能提升15.9%。

Conclusion: SafeGen能有效生成安全关键场景，提升VLMAD系统的安全评估和真实场景性能。
...
### [66] [Point-Selection Fine-Tuning Framework for Robust Point Cloud Classification](https://arxiv.org/abs/2607.19711)
*Da Li,Chang Ma,Dongfu Yin*

Main category: cs.CV

TL;DR: 提出PSFT框架，通过点选择与轻量适配提升3D点云模型在噪声及损坏场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有全微调放大异常点影响，参数高效微调对损坏敏感，需平衡鲁棒性与效率。

Method: 基于预池化特征估计点影响并保留最小影响点，生成提示令牌注入冻结主干；附加特征滤波器通过瓶颈MLP与Beta门控残差混合去噪。

Result: 在ModelNet-C与ModelNet40-C上一致降低损坏误差，在ULIP-2和Uni3D-B上达最强ScanObjectNN-C结果。

Conclusion: PSFT在参数高效下有效抑制异常点，显著提升3D预训练模型对不同损坏的鲁棒性。
...
### [67] [A Unified Tokenization Framework for Pain Recognition using Heterogeneous 3D Modalities](https://arxiv.org/abs/2607.19716)
*Stefanos Gkikas,Christian Arzate Cruz,Valentina Becchetti,Muhammad Umar Khan,Alessandro Giuseppi,Raul Fernandez Rojas*

Main category: cs.CV

TL;DR: 本文提出了一种统一的3D模态疼痛识别框架，能处理面部视频和fNIRS数据，在AI4Pain数据集上达到了最先进性能并支持实时评估。


<details>
  <summary>Details</summary>
Motivation: 疼痛评估对临床管理至关重要，现有方法需为不同模态设计独立架构，缺乏统一处理框架。

Method: 提出统一的令牌化框架，将异质3D模态映射到共享令牌空间，保留时空和时间-频率结构。

Result: 在AI4Pain数据集上达到最先进性能，支持GPU和CPU实时评估。

Conclusion: 该框架有效统一了疼痛识别中的多模态3D数据处理，兼具高效性和实用性。
...
### [68] [ReFace: Reorganizing Facial Spatiotemporal Representations for Improved Pain Assessment](https://arxiv.org/abs/2607.19722)
*Stefanos Gkikas,Yu Fang,Christian Arzate Cruz,Muhammad Umar Khan,Raul Fernandez Rojas*

Main category: cs.CV

TL;DR: 提出ReFace，通过将面部划分为四个空间象限再tokenize，在AI4Pain数据集上以56.00%的准确率取得当前最佳结果。


<details>
  <summary>Details</summary>
Motivation: 自动疼痛评估中，面部疼痛线索存在空间异质性，而现有方法将整张脸作为一个区域处理，未能充分利用这种空间信息。

Method: ReFace空间重组流程：将面部输入划分为四个空间象限后分别进行tokenization，而非处理整个面部区域。

Result: 在AI4Pain数据集上，仅使用视频数据达到56.00%准确率，超越其他方法；四象限配置在相同像素预算下准确率更高，单象限区域仅用四分之一像素仍具竞争力。

Conclusion: 空间重组能够提升疼痛评估性能，为高效利用面部空间信息提供了新思路。
...
### [69] [An Exploratory Analysis of Pain Localization via Explainable Computational Modeling](https://arxiv.org/abs/2607.19726)
*Ioannis Kyprakis,Stefanos Gkikas,Eric Nichols,Yu Fang,Manolis Tsiknakis*

Main category: cs.CV

TL;DR: 本文系统比较了传统特征工程与深度序列学习在基于外周生理信号的三类疼痛定位任务上的表现，发现极致随机树模型在AI4Pain 2026挑战赛数据集上取得最优宏F1得分(0.539)，EDA频谱特征为关键判别因素，且疼痛定位性能显著低于疼痛检测(差距达26个F1点)，表明10秒分辨率下外周自主神经通路的解剖弥散性构成了性能天花板。


<details>
  <summary>Details</summary>
Motivation: 解决无口语患者等无法自我报告人群的自动疼痛定位这一临床关键但未充分研究的问题。

Method: 使用AI4Pain 2026数据集(65名受试者、四种可穿戴模态信号)，对比115维手工特征集(含时域、频域、模态特异及跨模态描述符)与端到端深度架构，采用极致随机树与深度序列学习模型。

Result: 极致随机树实现最佳宏F1=0.539，比最优深度模型高7.4个百分点；疼痛检测F1=0.815远高于定位F1=0.552，表明存在26点的性能差距。

Conclusion: 在10秒时间分辨率下，外周自主神经通路的解剖弥散性限制了疼痛定位性能，EDA频谱特征是最重要的判别特征。
...
### [70] [Efficient Tracking and Understanding Object Transformations](https://arxiv.org/abs/2607.19743)
*Yihong Sun,Bharath Hariharan*

Main category: cs.CV

TL;DR: 提出FluxGraph，通过SAM2内部多掩码不一致性作为变换检测的轻量级触发，避免密集分割和全场景跟踪，在VOST上速度提升3.3倍，其他数据集提升3.7-10.7倍，同时保持跟踪和状态图质量。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪方法（如TubeletGraph）计算成本高，因需密集帧分割和全场景跟踪，无法实时部署。

Method: 利用SAM2内部多掩码不一致性作为变换检测触发器，仅跟踪相关变换对象，而非全场景实体。

Result: 在VOST上速度提升约3.3倍，在VSCOS、M^3-VOS、DAVIS17上提升3.7-10.7倍，且跟踪性能提升、状态图质量保留。

Conclusion: FluxGraph通过反应式触发机制有效降低计算成本，实现更快跟踪且保持性能，适用于实时场景。
...
### [71] [Extending a Large View Synthesis Model for Multi-view Panoptic Segmentation](https://arxiv.org/abs/2607.19765)
*Kwonyoung Ryu,In-Jae Lee,Jonghyun Jin,Hyunjee Lee,Jongmin Lee,Jaesik Park*

Main category: cs.CV

TL;DR: 提出利用大规模视图合成模型中隐式学习到的空间对应关系，直接传播全景标签至新视角，无需显式3D重建或分割训练，在ScanNet和Replica上取得优异分割与合成性能。


<details>
  <summary>Details</summary>
Motivation: 现有视图合成模型通过跨视图注意力从RGB监督中学会了准确的空间对应，且该对应可泛化至非真实感信号（如全景标签），启发探索将其用于3D场景理解。

Method: 保持视图合成模型冻结，将输入视图中的全景标签编码为二进制通道表示，通过同一模型渲染输出新视图的像素级标签，完成标签传播。

Result: 在ScanNet上，分割质量与需显式3D重建的高斯方法相当，新视图合成性能提升超过7 dB；在Replica上跨数据集零样本迁移表现更优。

Conclusion: 视图合成模型学到的空间对应可有效传播语义标签至新视角，为3D场景理解提供了一种无需显式重建或分割训练的替代方案。
...
### [72] [Global Building Area Estimation Products: How Accurate Are They?](https://arxiv.org/abs/2607.19766)
*Saad Lahrichi,Doa'a Allabadi,Kyle Bradbury,Jordan Malof*

Main category: cs.CV

TL;DR: 本文对四种全球建筑栅格数据集（GHSL、TEMPO、GBA、Overture）进行了独立、全面的准确性评估，使用 ORBITaL-Net 作为真实标签，发现 GBA 或 TEMPO 总体准确性最高，但所有产品在非洲和亚洲以及高密度城市区域准确性显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有多种全球建筑栅格数据集，但缺乏独立、全面和公平的准确性评估。

Method: 使用全球多样性的手动标注建筑足迹数据集 ORBITaL-Net 作为真实标签，在多个空间分辨率网格上评估四种产品，并使用多种常规性能指标，按地理位置、人口密度和收入群体分层分析。

Result: GBA 或 TEMPO 在多数评价标准下整体准确性最高；产品准确性随地理位置、人口密度和收入群体显著变化；所有产品在非洲和亚洲准确性较低，在高密度城市区域性能下降明显。

Conclusion: 现有全球建筑栅格数据集准确性存在区域和密度差异，尤其在非洲、亚洲和城市核心区需改进。
...
### [73] [DRGBT-1K: A Large-scale High-quality Benchmark for Dynamic RGBT Tracking](https://arxiv.org/abs/2607.19772)
*Zhaodong Ding,Chenglong Li,Zeyu Ding,Futian Wang,Jin Tang*

Main category: cs.CV

TL;DR: 提出了DRGBT-1K基准测试，包含1045个序列和795K帧对，用于评估动态RGBT跟踪中模态和平台变化下的鲁棒性。
...
Method: 构建大规模高质量基准数据集，提供多维度标注，评估20种代表性多模态跟踪方法。

Result: DRGBT-1K覆盖24类目标、15种场景和挑战属性，统一评估协议揭示现有方法不足。

Conclusion: 此基准填补了动态RGBT跟踪评估空白，并支持未对齐多模态与无人机-地面协作跟踪研究。
...
### [74] [Look Before You Edit: Attention-Guided Camera Placement and Multi-View Alignment for 3D Gaussian Splatting Editing](https://arxiv.org/abs/2607.19777)
*Jaeyeon Park,Taeho Kang,Youngki Lee*

Main category: cs.CV

TL;DR: 提出LB-Edit框架，通过注意力引导的摄像机放置和多视图注意力对齐，实现基于3D高斯泼溅的局部文本驱动场景编辑，仅需5个编辑视图，延迟降低7倍。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS编辑方法受限于固定训练摄像机视角，编辑覆盖范围有限且难以针对特定物体进行局部编辑，用户自由度不足。

Method: 包含两个核心组件：1）注意力引导编辑摄像机放置（ACP），利用扩散模型注意力确定最优摄像机距离并生成紧凑的视角集合；2）多视图注意力对齐（MAA），通过共享自注意力特征和对齐交叉注意力映射到3D高斯分布，确保编辑在多视图间外观和空间一致性。

Result: 在多物体和单物体场景中，该方法在指令保真度、多视图一致性和编辑局部性方面获得最高用户偏好，仅用5个编辑视图，延迟比现有方法降低7倍。

Conclusion: LB-Edit能有效实现局部3D场景编辑，兼顾编辑质量、一致性和效率，显著优于现有方法。
...
### [75] [Frequency-Hierarchical Active k-Space Sampling for Diagnostic MRI](https://arxiv.org/abs/2607.19779)
*Ruru Xu,Kian Anvari Hamedani,Zhikai Yang,Ilkay Oksuz*

Main category: cs.CV

TL;DR: 提出HieraSample，一种基于层次结构的任务驱动主动采样框架，在加速MRI中通过余弦退火课程和Mamba策略优化高频采样，在fastMRI+膝关节基准上匹配全采样性能。


<details>
  <summary>Details</summary>
Motivation: 现有主动采样器无法有效分配高频和低频采样预算，导致高加速倍数下性能折衷。

Method: 余弦退火课程逐步降低加速倍数（20x至4x），保留全采样低频圆盘；Mamba策略基于疾病和严重性分类特征选择高频坐标；奖励为每个动作后类别加权交叉熵的减少。

Result: 在fastMRI+膝关节数据集上，从4x到10x加速下ACL诊断匹配全采样基线，ACL严重性评分比笛卡尔基线提升最多20.4 AUC点。

Conclusion: 层次化主动采样能有效利用频率特性，在保持诊断准确性的同时实现高加速。
...
### [76] [WASABI: Whole-graph Assignment-based Stabilizer for lAne topology By Inter-frame tracking](https://arxiv.org/abs/2607.19781)
*Tetsuhiro Uchida,Myu Sasaki,Kensho Nakajima,Yasuhiro Shimada,Toru Saito*

Main category: cs.CV

TL;DR: 提出WASABI实时后处理流水线，通过联合跟踪车道段及其连通性，稳定自动驾驶中的车道拓扑输出，实现帧内和帧间性能提升。


<details>
  <summary>Details</summary>
Motivation: 神经感知模型输出的车道拓扑存在结构性不稳定（如漏检、连通性错误、标签闪烁），需要实时稳定化。

Method: 将车道段及其连通性作为联合跟踪目标，集成段跟踪与拓扑感知噪声鲁棒优化，在实时约束（10 Hz，200输入车道）下设计。

Result: LCLC检测F1从0.834提升至0.948（+13.6%），中心线横向误差从2.50m降至0.95m，假阳性减少24.6%；LCLC切换率降低63.3%，边界标签闪烁率降低30.2%。

Conclusion: WASABI能同时提升单帧精度和跨帧时间稳定性，适用于实时自动驾驶系统。
...
### [77] [Physics-Aware Complex-Valued State Space Model with Scattering-Prior Feature Modulation for PolSAR Image Classification](https://arxiv.org/abs/2607.19787)
*Fangyan Zhang,Fan Zhang,Shiqi Zhou,Jun Ni,Carlos López-Martínez,Qiang Yin*

Main category: cs.CV

TL;DR: 提出了一种物理感知的复值状态空间网络(CV-SSMNet)，通过散射感知特征调制实现PolSAR图像分类。


<details>
  <summary>Details</summary>
Motivation: 现有复值网络在长距离空间依赖建模和物理知识引导深度特征演化方面存在不足。

Method: 构建复值状态空间模型(CV-SSM)捕捉长范围依赖；编码七种散射先验作为FiLM调制信号；集成多尺度复卷积、分支CV-SSM编码、先验引导重校准和轻量全局上下文聚合。

Result: 在三个L波段基准和一个P波段BIOMASS数据集上，实现了竞争精度、改进的区域一致性和更好的边界保持。

Conclusion: 将极化散射机制嵌入复值长距离GeoAI表示学习是有效的。
...
### [78] [Trace: A Taxonomy-Guided Environment for Multidomain Visual Reasoning](https://arxiv.org/abs/2607.19790)
*Md Tanvirul Alam*

Main category: cs.CV

TL;DR: 提出了Trace，一个基于分类学的视觉推理环境，用于多领域视觉推理，通过场景语法和可执行任务程序分离视觉实现与答案计算，并验证了强化学习可迁移到外部基准。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习可验证奖励方法扩展到视觉语言模型时，缺乏同时具备广泛性、精确可验证性和可复现性的训练数据。

Method: 构建Trace环境，利用场景语法和可执行任务程序将任务构造分解，共享语义状态决定图像、提示、答案、验证器和实例轨迹。

Result: 在64,000个Trace实例上进行RLVR训练，使Qwen2.5-VL-3B在24个外部基准上的宏平均提升3.51个百分点，7B版本提升4.06个百分点。

Conclusion: 广泛的过程性训练可以迁移到生成任务分布之外，验证了方法的有效性。
...
### [79] [Lean-SAM2: Target-Anchored Memory and Encoder Acceleration for SAM2](https://arxiv.org/abs/2607.19811)
*Xudong Ouyang,Wenlun Zhang,Yimin Xu,Huazhong Liu,Yunshan Zhong*

Main category: cs.CV

TL;DR: 本文提出Lean-SAM2，一种针对SAM2的内存和计算冗余进行优化的轻量级框架，通过目标锚定记忆剪枝、时间压缩保险记忆和目标锚定风险感知路由三种机制，在复杂分割场景下实现精度与效率的更好平衡。


<details>
  <summary>Details</summary>
Motivation: SAM2在处理视频分割时存在高内存交叉注意力开销和冗余的全帧视觉特征提取，现有启发式方法在遮挡和干扰场景下性能严重下降。

Method: Lean-SAM2集成了三种协作机制：目标锚定记忆剪枝（TAMP）、时间压缩与保险记忆（TCIM）和目标锚定风险感知路由（TARR）。

Result: 在LVOSv2验证集上，Lean-SAM2在SAM2.1-Large和SAM2.1-Base+上分别实现1.412倍和1.417倍加速，同时J&F分数分别提升5.0%和3.6%，显著优于Efficient-SAM2。

Conclusion: Lean-SAM2通过系统消除冗余，在复杂场景下实现了精度与效率的优越平衡，验证了其有效性。
...
### [80] [MoAKE: Toward Unified All-in-One Action Quality Assessment via Mixture of Action Knowledge Experts](https://arxiv.org/abs/2607.19826)
*Huangbiao Xu,Huanqi Wu,Xiao Ke,Jiaxin Cai,Junyi Wu,Jinglin Xu*

Main category: cs.CV

TL;DR: 本文提出MoAKE框架，通过混合动作知识专家解决全合一动作质量评估中的负知识迁移问题，在多个数据集上优于现有方法，并支持零样本/少样本评估。


<details>
  <summary>Details</summary>
Motivation: 现有AQA方法采用“一对一”范式，限制实际部署，需要先验知识选择模型且泛化性差，因此研究全合一AQA以统一模型评估异构动作。

Method: 提出MoAKE框架，包含互补专家学习共享空间中的动作模式，利用自适应段感知原型和AIISRM模块建模多粒度时间动态，并动态聚合专家知识。

Result: 在三个长期数据集上显著优于现有方法，并在三个短期数据集上实现零样本/少样本的稳定泛化。

Conclusion: MoAKE有效缓解了动作间的负知识迁移，为全合一AQA提供了可行方案。
...
### [81] [Memory-Augmented Multimodal Large Language Models for Small Object Understanding in Streaming Aerial Videos](https://arxiv.org/abs/2607.19857)
...
TL;DR: 提出DroneEyes数据集和SkyAnchor方法，解决无人机在线视频中MLLM对微小目标理解和流式处理的两大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在无人机实时部署中无法处理微小目标（视觉压缩丢失细节）和流式理解（历史帧存储受限导致目标漂移）。

Method: 构建首个像素级开放词汇的微小目标指代分割数据集DroneEyes；设计SkyAnchor，包含语义感知令牌路由器（保留小目标细节）和分层记忆库（维持流式目标一致性）。

Result: 在DroneEyes上验证了该方法在微小目标分割和流式追踪任务上的有效性。

Conclusion: 通过数据与方法的联合设计，显著提升了无人机在线感知中MLLM对微小目标的理解与追踪能力。
...
### [82] [Robust Activation Map Rectification for Weakly Supervised Volumetric Segmentation: Temporal Coherence as a Free Lunch](https://arxiv.org/abs/2607.19877)
*Renshu Gu,Jialiang Chen,Fei Gao,Hang Su,Jun Qi,Jiamin Xu,Yicheng Shen,Jiayu Zhang,Jiaxi Pan,Caiming Zhang,Gang Xu*

Main category: cs.CV

TL;DR: 提出了一种无需训练和原型的框架，通过利用体积数据的时空和结构一致性来修正类激活映射（CAM），显著提升弱监督分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督分割方法依赖CAM，但CAM噪声大且易出现失败；现有修复方法计算成本高、鲁棒性差。

Method: 提出方差缩减激活聚合（VRAA）和双向极端修正（BER）两个组件，前者通过理论证明降低CAM方差，后者无需参数即可检测并修正异常激活。

Result: 在多个公开基准上，Dice最高提升20%，mIoU提升40%，推理时间减少5倍以上。

Conclusion: 利用体积数据的一致性作为隐式归纳偏置，为稳定弱监督体积分割提供了高效且原理性的方法。
...
### [83] [Current Injection Spiking Neural Network for Infrared and Visible Image Fusion](https://arxiv.org/abs/2607.19879)
*Rui Zhao,Zhuoyuan Li,Wenrui Li,Yanchen Dong,Yajing Zheng,Giuseppe Valenzise,Weisi Lin*

Main category: cs.CV

TL;DR: 提出了一种基于膜电位级交叉融合的脉冲神经网络CIS-Fuse，在红外与可见光图像融合任务中达到ANN方法同等效果，且能耗低一个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有IVIF方法基于ANN计算密集，而SNN稀疏二进制脉冲可能丢失低于阈值的互补线索，但膜电位保留亚阈值响应，可在此阶段融合双模态信息。

Method: 设计电流注入脉冲算子（CIS），将一种模态作为门控辅助电流注入另一驱动神经元，在膜电位水平融合；基于CIS构建双向交叉融合模块（BCMF），部署在非对称深度双分支架构上。

Result: 在四个IVIF基准和下游检测/分割任务上，CIS-Fuse融合质量与SOTA ANN方法相当，推理能耗比同规模ANN DCEvo低约一个数量级。

Conclusion: CIS-Fuse在保持SNN能量效率的同时实现了与ANN相媲美的融合性能，为节能型多模态融合提供了有效方案。
...
### [84] [MTVDiff: Multimodal Conditional Latent Diffusion for Enhanced Thermal-to-Visible Face Translation](https://arxiv.org/abs/2607.19886)
*Zhiyuan Xia,Haojie Li,Jingyu Lin,Yiguo Qiao,Cunjian Chen*

Main category: cs.CV

TL;DR: 提出了MTVDiff多模态隐扩散框架，通过融合深度和文本信息，有效解决热成像到可见光人脸翻译中的几何不连续、语义属性不匹配和身份退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在热到可见光人脸翻译中面临几何不连续、语义属性不匹配和身份退化等挑战。

Method: 包括双分支交叉注意力融合模块、门控文本到视觉特征对齐机制和空间特征变换。

Result: 在MCXFace和SpeakingFaces数据集上，FID降低48.3%，Rank-1准确率提升8.9%。

Conclusion: 该方法为变光照条件下的人脸识别提供了鲁棒解决方案，推动了跨光谱人脸图像翻译的发展。
...
### [85] [LAVIFT: Latent-Action-Guided Vision Fine-Tuning for Surgical Interaction Recognition](https://arxiv.org/abs/2607.19889)
*Jiajun Cheng,Subarna Tripathi,Sainan Liu,Xiaofan Yu,Shan Lin*

Main category: cs.CV

TL;DR: 提出了LAViFiT框架，通过逆动力学模型和正向世界模型引导视觉编码器关注动作相关区域，实现手术器械-组织交互的细粒度视觉-语言对齐。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉-语言模型在细粒度手术交互任务中面临冻结编码器引入噪声/弱空间定位、全微调可能忽略正确动作区域的问题。

Method: 结合逆动力学模型捕捉动作引起的视觉变化、正向世界模型驱动编码器聚焦动作区域，以及Patch级SIG正则化防止局部特征坍缩。

Result: 在多个编码器和数据集上提升了交互识别与图像-文本对齐性能，表征分析显示了更强的动作区域空间定位能力和特征一致性。

Conclusion: 该方法有效增强了视觉编码器对手术交互中动作相关区域的学习，无需额外边界框监督即可实现更精准的细粒度识别。
...
### [86] [OSVE: One Step Video Editing with One Step Diffusion Models](https://arxiv.org/abs/2607.19895)
*Habin Lim,Gyeong-Moon Park*

Main category: cs.CV

TL;DR: 提出OSVE框架，首次将一步式文本到图像模型成功应用于高质量视频编辑，实现约155-171倍速度提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本引导视频编辑因多步采样和反转过程耗时严重，无法实用化。

Method: 训练可学习的编码器替代慢速迭代反转，用结构感知编辑损失保留几何结构；采用统一帧编辑技术通过拼接帧潜变量实现跨帧注意力；对长视频使用滑动窗口策略保持全局一致性。

Result: 编辑质量与最好多步方法相当或更优，速度提升约155-171倍。

Conclusion: 该突破为实用化实时视频编辑应用铺平道路。
...
### [87] [StrokeSeg2: Stroke Lesion Segmentation in Clinical Research Workflows](https://arxiv.org/abs/2607.19901)
*Youwan Mahé,Axel Plessis,Stéphanie Leplaideur,Elise Bannier,Florent Leray,Francesca Galassi*

Main category: cs.CV

TL;DR: 提出 StrokeSeg2，一个轻量级、跨平台的 C++/Qt 框架，通过知识蒸馏和 ONNX Runtime 量化优化，实现 3D 脑卒中病变分割的高效部署。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习框架（如 nnU-Net）虽在脑病变分割中表现优异，但依赖复杂软件和计算资源，难以在临床研究环境中部署。

Method: 采用知识蒸馏进行架构压缩，结合 ONNX Runtime 的 Float16 量化优化推理，生成 0.84M 参数学生模型，并打包为独立安装程序。

Result: 架构蒸馏使能耗降低 90% 以上，推理时间平均减少 84%；学生模型磁盘占用仅 2.1 MB，保持稳健的病变定位和竞争性分割性能。

Conclusion: StrokeSeg2 无需 Docker 或外部依赖，提供图形和命令行界面，便于在临床工作站上部署高性能分割工作流。
...
### [88] [MV-Bench: Benchmarking Multimodal Large Language Models for Coordinated Multi-View Interface Construction](https://arxiv.org/abs/2607.19910)
*Yue Zhao,Hongxu Liu,Feiyu Wang,Xiaoyu Yang,Tong Ge,Zhen Yang,Chao Wang,Qiong Zeng*

Main category: cs.CV

TL;DR: 提出MV-Bench基准，评估多模态大语言模型在协调多视图界面构建中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注单图表生成，忽视多视图界面构建，缺乏系统性基准。

Method: 基于Tableau workbook文件作为ground truth，通过多阶段管道转换为可执行Web界面，构建包含92个基础界面和1048个实例的基准。

Result: 最强模型在视觉布局达75.45%准确率，但数据绑定仅21.71%，交互完整性仅11.68%。

Conclusion: 当前MLLMs能再现视觉外观，但数据语义和交互逻辑生成能力有限，迭代优化无法缩小该差距。
...
### [89] [LoRFT: Benchmarking Long-Range Vehicle Trajectory Reconstruction from Fixed Highway Cameras](https://arxiv.org/abs/2607.19911)
*Yufan Zhu,Kefu Yi,Xueju Zhang,Yunyang Tian,Long Chen,Zixuan Xiao*

Main category: cs.CV

TL;DR: 论文提出了LoRFT基准数据集和Map-RSTNet模型，用于从固定高速公路摄像头中恢复远距离车辆轨迹，解决了因透视压缩和尺度衰减导致的轨迹中断问题。


<details>
  <summary>Details</summary>
Motivation: 固定摄像头中远距离车辆轨迹因透视压缩和尺度衰减而难以连续恢复，现有方法在处理远场区域时经常失效，因此需要一种专门的长距离轨迹重建方法。

Method: 构建了包含22个场景、36.6万帧的LoRFT基准数据集，并提出了Map-RSTNet，一种地图感知的残差序列到序列模型，在道路几何对齐的状态空间中重建轨迹，并在解码时动态更新局部道路几何。

Result: 相比最强基线，Map-RSTNet将平均位移误差(ADE)、最终位移误差(FDE)和5秒均方根误差(RMSE)分别降低了11.0%、15.4%和10.5%。

Conclusion: 道路几何感知的重建可以从现有固定摄像头基础设施中扩展可用的轨迹记录，LoRFT为长距离车辆轨迹重建提供了可重复的测试平台。
...
### [90] [WearWow: Native 2K Multi-Garment Virtual Try-On via Adaptive Token Packing and Preference Alignment](https://arxiv.org/abs/2607.19923)
*Xujie Zhang,Runyan Du,Song Chang,Jiang Li,Dongliang Shao,Liping Wu,Wei Luo,Xiaochao Qu,Luoqi Liu,Xiaodan Liang*

Main category: cs.CV

TL;DR: 提出了WearWow框架，通过自适应2D令牌打包和多维试穿奖励系统，实现了原生2K多服装虚拟试穿，解决了内存爆炸和纹理模糊问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在2K分辨率下存在O(N^2)内存爆炸和扩散模型对高频织物细节过平滑的问题，限制了超高清多服装虚拟试穿的发展。

Method: 提出自适应2D令牌打包（ATP）压缩有效序列长度，以及多维试穿奖励系统（MTR）结合语义引导奖励和布料分布奖励来恢复纹理细节。

Result: 在原生2K多服装合成任务上超越了现有商业基线，达到了新的最优性能。

Conclusion: WearWow通过创新的内存优化和纹理修复机制，成功实现了端到端、无掩码的超高清多服装虚拟试穿。
...
### [91] [G-MAD: A Game-Based Data Generation Framework for Multi-View RGB-T Aerial Object Detection](https://arxiv.org/abs/2607.19942)
*Yechan Kim,JongHyun Park,Dongho Yoon,Namhoon Jung,Moongu Jeon*

Main category: cs.CV

TL;DR: 提出了一个基于Arma3的开源框架G-MAD，用于生成同步多视角RGB-T航空目标检测数据，并构建了大规模基准数据集AMOD。


<details>
  <summary>Details</summary>
Motivation: 真实航空数据集存在视角控制有限、RGB-T配准不完美、标注成本高等局限性。

Method: 支持结构化场景设定、可控多视角相机布局、同步可见光/热红外捕获，以及利用引擎几何元数据自动生成边界框标注。

Result: 成功创建了AMOD数据集，实现了可控的视角变化、多模态融合和合成到真实迁移研究。

Conclusion: G-MAD框架有效解决了真实数据集的局限性，为航空目标检测研究提供了灵活的数据生成工具和基准数据集。
...
### [92] [SIINR: Structurally Informed Implicit Neural Representations for super-resolution with uncertainty quantification of clinical quality diffusion MRI datasets](https://arxiv.org/abs/2607.19943)
*Tom Hendriks,William Consagra,Anna Vilanova,Yogesh Rathi,Maxime Chamberland*

Main category: cs.CV

TL;DR: 提出了SIINR框架，通过结合监督3D U-Net先验与自监督隐式神经表示，实现临床dMRI超分辨率并量化不确定性。


<details>
  <summary>Details</summary>
Motivation: 临床dMRI分辨率受限导致结构信息退化，现有方法缺乏鲁棒性且无法量化重建不确定性。

Method: SIINR由3D U-Net作为先验，与自监督隐式神经表示融合，实现空间-角度联合建模、数据一致性约束及近似后验分布推导。

Result: 在多个开放数据集上优于标准插值方法，在临床病例（多发性硬化、脑损伤）中能传播强度变化并标记不确定区域。

Conclusion: SIINR灵活模块化，可适用于不同上采样比和下游任务，为增强临床dMRI及派生指标解释提供系统方案。
...
### [93] [ETPDesigner: Multi-Agent Orchestration for Interactive Multimodal Electronic Theater Program](https://arxiv.org/abs/2607.19947)
*Mengtian Li,Xinru Guo,Xiaoru Lin,Xiao Rong,Zhifeng Xie,Chaofeng Chen*

Main category: cs.CV

TL;DR: 提出了ETPDesigner，一个多智能体框架，用于从剧本直接合成高质量的电子戏剧节目，并集成交互功能如角色动画和语音对话。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成图像方法无法处理长上下文叙事和跨页面视觉一致性，难以完成复杂设计任务。

Method: 采用协作的多智能体框架，包括语义分析、核心海报生成、背景合成和角色资产分层组合，并通过全局风格锚点机制保持视觉一致性。

Result: 在构建的ETP-Pro基准上，方法在语义准确性、美学一致性和交互性方面显著优于现有方法。

Conclusion: ETPDesigner能高效生成高质量、连贯且具有互动性的戏剧节目组。
...
### [94] [OffNadirLoc: Benchmark and Framework for Challenging UAV-to-Satellite Geo-Localization under Large Off-Nadir Views](https://arxiv.org/abs/2607.19951)
*Qian Qiao,Wenye Liu,Ting Liu,Jiuhe Shu,Peng Wang*

Main category: cs.CV

TL;DR: 提出了用于大离轴视角的无人机-卫星地理定位基准OffNadirLoc和框架ONLoc，利用结构感知加权和视角一致学习提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对近天顶场景，忽略了结构理解和域内关系约束，在大离轴视角下性能受限。

Method: ONLoc框架包含结构感知上下文加权机制和视角一致学习策略，将多个视角的UAV图像和对应卫星图像作为语义组进行集合级监督。

Result: 在OffNadirLoc基准和四个近天顶数据集上，方法一致超越现有技术，且具有强零样本泛化能力。

Conclusion: 该方法有效应对大离轴视角挑战，在多视角一致性和跨域泛化方面表现优异。
...
### [95] [Forecasting the Number of Harvest-ready Fruits of Sweet Peppers Using Multimodal Time-Series Data](https://arxiv.org/abs/2607.19975)
*Enrico Pallotta,Mohamed Farag,Esra Guclu,Chris McCool,Ribana Roscher,Juergen Gall*

Main category: cs.CV

TL;DR: 提出一个包含691株甜椒植株的多模态时序数据集，并设计融合图像特征与数值计数的LSTM框架，实现产量预测，RMSE降低33%-38%。


<details>
  <summary>Details</summary>
Motivation: 缺乏同时包含视觉生长动态和单株测量标签的公开数据集，导致个体植物水平精准产量预测困难。

Method: 构建甜椒图像时间序列数据集，使用DinoV3提取特征，结合LSTM处理时序依赖性，融合数值计数数据进行多模态学习。

Result: 多模态模型相比基线RMSE降低33%-38%，比仅用测量数据的模型平均提升1.2%，不确定性校准误差UCE为0.39-0.89。

Conclusion: 多模态框架可显著提升产量预测精度并提供校准的不确定性估计，数据集和代码已公开以促进可复现研究。
...
TL;DR: 提出Sentinel-2建筑检测框架，系统评估季节、处理级别和建成环境异质性对分类性能的影响，并给出实践指导。


<details>
  <summary>Details</summary>
Motivation: Sentinel-2影像分辨率低、建筑细小、季节性和环境异质性导致分类困难，需要系统量化这些影响以支持模型选择。

Method: 构建华沙地区多时相Sentinel-2数据集，使用BDOT10k生成真实标签，基于U-Net和DeepLabV3+进行场景微调、月度模型选择及跨时间推理分析。

Result: 确定了最佳月度模型及L1C/L2A产品差异，量化了季节间性能转移、处理级别影响和建成环境异质性的作用。
...
### [97] [STEREOFLOW: Progressive Stereo Matching with StereoDiT and Transition Flow Matching](https://arxiv.org/abs/2607.19986)
*Hao Wang,Haoran Geng,Xiaotong Yang,Jing Tang,Songlin Wei,Linlong Lang,Yeying Jin,Zheng Zhu,Zhaoxin Fan,Biao Leng*

Main category: cs.CV

TL;DR: 提出一种名为StereoFlow的先验引导生成框架，通过将确定性匹配回归与生成式分布建模结合，解决了立体匹配中多模态分布建模为单点估计的回归均值偏差问题，在多个基准测试中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统立体匹配将问题简化为确定性回归，导致多模态分布被压缩为单点估计，在歧义区域易出现回归均值偏差，无法有效处理模糊区域。

Method: 包含三个核心组件：(i) 两阶段渐进级联匹配网络生成多分辨率互补条件；(ii) 频率解耦像素扩散变换器(StereoDiT)建模对应歧义；(iii) 少量步骤的过渡流匹配实现高效优化。

Result: 在Scene Flow、KITTI、ETH3D、Middlebury等基准上取得多个SOTA结果，尤其在病态、不连续区域和零样本泛化中表现出强几何一致性与丰富细节。

Conclusion: 所提生成式框架有效整合了匹配回归与分布建模，显著提升了立体匹配在歧义场景下的鲁棒性和泛化能力。
...
### [98] [A Systematic Benchmark of Intensity Normalisation Methods for 3D Knee MRI Segmentation and Cross-Domain Generalisability](https://arxiv.org/abs/2607.20028)
*Oliver Mills,Philip Conaghan,Samuel Relton*

Main category: cs.CV

TL;DR: 系统比较了七种MRI强度归一化方法对3D U-Net模型在膝关节半月板分割任务中的影响，发现归一化对模型泛化性影响有限，远小于域移的影响。


<details>
  <summary>Details</summary>
Motivation: MRI图像强度因扫描仪和协议不同而变化，影响深度学习模型的泛化性，但强度归一化作为重要因素尚未被充分研究。

Method: 比较了七种归一化方法（包括标准缩放、直方图基技术和高斯混合模型方法），在IWOAI 2019数据集上训练3D U-Net，并在内部和外部测试集上评估。

Result: 内部性能相似，但外部测试差异显著；Z-score、Nyúl直方图匹配和CLAHE更鲁棒，但效果远小于数据集域移导致的性能下降。

Conclusion: 强度归一化对模型泛化性有可测量的影响，但效果有限，需结合其他策略应对域移问题。
...
### [99] [Importance-Aware OBS Pruning for Diffusion Models](https://arxiv.org/abs/2607.20048)
*Ba-Thinh Lam,Srijan Das,Hieu Le*

Main category: cs.CV

TL;DR: 提出一种面向扩散模型的重要性感知剪枝框架，通过空间重要性图保留语义关键区域，实现高压缩比下保持图像主体保真度和结构正确性。


<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法基于均匀重建误差，导致高压缩比时图像语义对象和结构出现明显退化。

Method: 利用条件信号或模型注意力生成空间重要性图，将其融入剪枝目标以生成与感知相关性对齐的参数排序。

Result: 在MS-COCO数据集上，该方法在高压缩比下显著优于传统剪枝，有效保持主体保真度和结构正确性。

Conclusion: 内容感知的目标函数是生成模型实现感知保真压缩的关键。
...
### [100] [GaussianSeed: Hierarchical Gaussian Seeding for High-Resolution 3D Occupancy Prediction](https://arxiv.org/abs/2607.20071)
*Xinzhuo Li,Xianghui Pan,Jiayuan Du,Wei Wei,Liuyi Wang,Chengju Liu,Qijun Chen*

Main category: cs.CV

TL;DR: 提出高斯种子渐进多尺度框架，实现0.1米分辨率下实时3D占用预测，并构建TJScenes数据集验证效率-质量前沿。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉的3D占用预测方法在高体素分辨率下计算成本过高，难以扩展。

Method: 采用渐进多尺度高斯占用预测框架，将基元组织成粗到细的层次结构。

Result: 成功扩展到0.1米空间分辨率且保持实时推理，在Occ3D-nuScenes和TJScenes上延迟最低、精度具竞争力。

Conclusion: 高斯种子推进了高分辨率3D占用预测的效率-质量前沿。
...
### [101] [Factor-Informed Uncertainty Distillation for Gaze Estimation](https://arxiv.org/abs/2607.20072)
*Mohammadreza Jamalifard,Yaxiong Lei,Javier Fumanal Idocin,Parastoo Azizinezhad,Tom Foulsham,Javier Andreu-Perez*

Main category: cs.CV

TL;DR: 提出一种基于教师-学生框架的因子引导不确定性蒸馏方法（FIUD），通过可解释图像质量因素预测凝视误差不确定性，在非约束环境下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度凝视估计方法在非约束场景下性能下降，且现有不确定性方法缺乏对输入有效性线索的显式建模，或计算成本过高难以实时应用。

Method: 采用教师-学生框架：梯度提升树教师基于光照、清晰度、眼睛可见性和对称性等可解释因素预测凝视误差，轻量级神经网络学生通过课程学习和排序监督蒸馏这些信号到单次传递的不确定性头部。

Result: 在ETH-XGaze、Gaze360和MPIIFaceGaze三个数据集（>30万样本）上，FIUD在不确定性校准、误差秩相关和选择性预测方面均优于确定性方法和采样基线，尤其在非约束场景下提升最大。

Conclusion: FIUD框架有效对齐不确定性与可解释的图像质量失效模式，在保证实时性的同时提升了非约束环境下凝视估计的可靠性。
...
### [102] [Development of an automated, reliable, and clinically meaningful artificial intelligence (AI) tool for diagnosing cardiac disease from conventional cardiovascular magnetic resonance (CMR) images](https://arxiv.org/abs/2607.20087)
...
TL;DR: 本研究开发了一个自动化的数据整理流程，整合开源大型语言模型从CMR报告中提取诊断标签，并微调三个视觉基础模型（DINO、VST、UMedPT）对五种心血管疾病进行分类，在独立测试集上取得了高性能（AUC最高达0.966）。


<details>
  <summary>Details</summary>
Motivation: CMR影像解读需要丰富经验，而现有AI模型受限于数据整理劳动密集、模型性能欠佳和实施路径不明确。

Method: 利用开源本地运行的大语言模型从CMR报告中提取标签，预处理多模态影像数据，采用两阶段微调三个视觉基础模型，并应用集成策略。

Result: 在1067例患者独立测试集中，集成模型对HCM和CA诊断的AUC分别达0.959和0.966，对其他疾病诊断AUC在0.840-0.872之间。

Conclusion: 该方法显著提升了AI辅助CMR诊断的准确性和鲁棒性，且所有代码和模型权重已公开。
...
### [103] [ENTRAP-VL: A Taxonomic Probe for Dual Contextual Entrainment in Vision-Language Models](https://arxiv.org/abs/2607.20092)
*Karan Goyal,Afreen Hossain,Debojyoti Das,Vishal Bhutani*

Main category: cs.CV

TL;DR: 本文提出了ENTRAP-VL，一个用于研究视觉语言模型中上下文牵引现象的数据集和评估框架，该现象可由文本和视觉上下文独立驱动，并引入了真实世界可能性与场景真实性的区分。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中于单模态语言模型的上下文牵引现象，而视觉语言模型中的表现和机制尚未被充分探索，且缺乏专门设计的工具进行系统研究。

Method: 手动构建了包含1500个样本、涵盖八类别的数据集ENTRAP-VL，基于上下文与项目的关联及与真实性的关系两个维度设计分类体系，并分别设置了文本牵引和视觉牵引的评估流。

Result: 提供了用于严格评估视觉语言模型中上下文牵引现象的数据集、分类体系及评估协议，但未涉及具体模型的性能测量。

Conclusion: 视觉语言模型中的上下文牵引是一个双重现象，需通过跨模态、分类学驱动的工具进行研究，ENTRAP-VL为社区提供了实现该目标的标准化平台。
...
### [104] [RIM: A Retrieval-In-Matching Framework for Cross-Domain Global Visual Localization of UAVs](https://arxiv.org/abs/2607.20116)
*Xin Li,Siyuan Duan,Shang Wang,Zhimin Mao,Bingliang Hu,Geng Zhang*

Main category: cs.CV

TL;DR: 提出一种高效无人机跨域视觉定位方法，通过固定DINOv2-B检索器并蒸馏局部描述符解码器，实现端到端快速定位，精度优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 无人机与遥感影像间的跨域外观和视角差异导致6自由度位姿估计困难，现有方法计算量大、效率低。

Method: 使用Google 3D Tiles采样视角参考图，两阶段微调SALAD，提出RIM冻结DINOv2-B检索器并蒸馏局部描述符解码器，结合浅层VGG19细节流。

Result: 在EPFL和Park数据集上，Recall@1分别提升8.55/13.77和4.45/8.94个百分点，端到端定位耗时67.9ms，速度比强基线快1.8倍，比RoMa快40倍以上。

Conclusion: 该方法在无GNSS环境下实现了高效、可部署的无人机全局视觉定位，显著减少计算开销并保持高精度。
...
### [105] [HeadCast: Casting Attention Heads for Efficient Autoregressive Video Generation](https://arxiv.org/abs/2607.20125)
*Jinliang Shen,Lianghao Su,Zheming Li,Kang He,ZiLiang Lai,Yanbing Jiang,Chengru Song*

Main category: cs.CV

TL;DR: HeadCast 是一种无需训练的加速框架，通过分类注意力头为四种类型并重组KV缓存，在自回归视频扩散模型中实现最高1.95倍加速，同时保持视频质量和减少闪烁。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型中KV缓存不断增长导致推理成本高昂，现有方法存在帧间闪烁或需要重新训练的问题。

Method: 在预热后，于最大噪声步骤对注意力头进行一次分类，划分为Sink、Dummy、Spatial和Global四种原型，并将KV缓存重构为头特定路径，保留Global头以维持时间一致性。

Result: 在先进的自回归模型上，HeadCast在720P下加速1.62倍，1080P下加速1.95倍，VBench质量与完全注意力持平且基本无闪烁。

Conclusion: HeadCast通过头分类和缓存重组，有效加速推理并保持高质量视频生成，是训练-free的实用方案。
...
### [106] [Self-supervision drives representational convergence in medical foundation models more than clinical supervision](https://arxiv.org/abs/2607.20274)
*Soroosh Tayebi Arasteh,Sebastian Ziegelmayer,Mahshad Lotfinia,Lisa Adams,Sven Nebelung,Jakob Nikolas Kather,Daniel Truhn*

Main category: cs.CV

TL;DR: 医学图像编码器的表示收敛程度有限，主要由预训练目标驱动，而非规模或临床监督；线性分类器可跨编码器转移，但收敛性未达到临床语言水平。


<details>
  <summary>Details</summary>
Motivation: 检验不同医学图像编码器是否因规模和临床监督而收敛到共享结构，以及这种收敛的成因和临床可用性。

Method: 控制实验：在固定数据、架构和规模下训练仅改变目标的编码器，并在合成模型中复现效果；分析18个图像和7个文本编码器（7M-27B参数，5种模态，65万张胸片）。

Result: 自监督编码器对齐最高（40.4%），标签监督（21.1%）和图像-文本（3.3%）较低；收敛不随规模增长；线性分类器跨编码器和5家医院转移，保留约85%性能。

Conclusion: 收敛由预训练目标决定，非规模或临床监督；应通过目标设计互操作性，并在患者亚组和临床判断中验证共享几何的最弱处。
...
### [107] [Test-Time Training for Modality Order Consistency in Vision-Language Models](https://arxiv.org/abs/2607.20351)
*Aditi Gupta,Yossi Gandelsman*

Main category: cs.CV

TL;DR: 研究发现视觉语言模型对图像和问题的输入顺序敏感，图像优先提示始终优于问题优先提示，并提出了一种顺序一致的测试时训练方法来缩小差距并提升性能。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型对输入顺序的敏感性及其潜在的性能损失。

Method: 在多个模型和基准上比较图像优先与问题优先提示的效果，并通过激活修补定位故障区域，提出测试时训练方法。

Result: 顺序一致的测试时训练显著缩小了模态顺序差距，并提高了图像优先分支的性能。激活修补将顺序故障定位到网络中部的狭窄区域。

Conclusion: 模态顺序敏感性是视觉语言模型的电路级故障，简单的非对称测试时适应可以有效缓解并提升基线性能。
...
### [108] [SHFormer: Dynamic Spectral Filtering Convolutional Neural Network and High-pass Kernel Generation Transformer for Adaptive MRI Reconstruction](https://arxiv.org/abs/2607.20159)
*Sriprabha Ramanarayanan,Rahul G. S.,Mohammad Al Fahim,Keerthi Ram,Ramesh Venkatesan,Mohanasankar Sivaprakasam*

Main category: cs.CV

TL;DR: 提出一种基于神经调节的判别性多光谱注意力机制（SHFormer），用于可扩展的MRI重建，能同时传播高频细节并跨域复用特征。


<details>
  <summary>Details</summary>
Motivation: 现有注意力机制在MRI重建中偏向低频信息、高频表示能力有限，且针对多模态MRI数据需重新训练，缺乏跨域泛化能力。

Method: 设计包含频谱滤波CNN（捕获模态特异可迁移特征）和动态高通核生成Transformer（聚焦高频细节）的网络；结合神经调节策略实现判别性多光谱注意力。

Result: 在监督/自监督学习、扩散模型训练、闭集/开集泛化等场景下，PSNR提升约1 dB，SSIM提升约0.01，尤其在未见域表现最佳。

Conclusion: 所提方法通过显式建模高频信息与跨域特征复用，实现了可扩展的、高质量且泛化性强的MRI重建。
...
### [109] [StreamHOI: Interaction-aware Temporal Memory Adaptation for Streaming HOI Video Generation](https://arxiv.org/abs/2607.20174)
*Zejing Rao,Haoxian Zhang,Xiaoqiang Liu,Yiping Meng,Guoxin Zhang,Pengfei Wan,Fan Tang,Tong-Yee Lee*

Main category: cs.CV

TL;DR: 提出StreamHOI，一种低延迟流式生成框架，用于长时间人-物交互视频生成，通过分析历史记忆偏好和块专用训练实现高效生成。


<details>
  <summary>Details</summary>
Motivation: 现有HOI视频生成方法受限于离线短视频生成和复杂驱动条件，无法用于实时交互应用。

Method: 采用离线HOI感知块分析、偏置引导记忆专业化训练和记忆距离缩放模块，优化流式生成器的历史记忆组织。

Result: 在交互合理性、物体保真度、人物质量和效率上优于现有方法，达到17.6 FPS和0.75秒首块延迟。

Conclusion: StreamHOI能有效支持实时、长时间的HOI视频生成，解决了流式系统中的记忆权衡问题。
...
### [110] [RS-RIE-Bench: Benchmarking Reasoning-Guided Remote Sensing Image Editing](https://arxiv.org/abs/2607.20197)
*Zihan Qin,Boao Xu,Zhao Dong,Yingping Sun,Ziheng Jiao,Junying Wang,Hongwei Wang*

Main category: cs.CV

TL;DR: 提出了首个面向遥感图像推理编辑的基准RS-RIE-Bench，包含时间、因果、空间三类推理任务，并评估了现有模型在区域合理性、非目标保持及图像质量上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对自然图像，无法全面评估遥感编辑所需的推理、区域控制和传感器一致性能力。

Method: 构建RS-RIE-Bench，包含三类推理任务（时间、因果、空间），设计三维评价协议，并引入多模态大模型评估。

Result: 最强模型严格联合准确率仅24.28%，平均宽松联合4成功率32.23%，因果与空间推理挑战大，部分开源模型在特定类别近乎零。

Conclusion: RS-RIE-Bench能有效暴露当前模型在地理推理、区域控制和传感器一致生成上的局限，为未来遥感智能编辑提供标准化基准和研究方向。
...
### [111] [PerceptDrive: Perception Prior World-Action Modeling with Adaptive Expert Routing for End-to-End Autonomous Driving](https://arxiv.org/abs/2607.20175)
...
TL;DR: 提出PerceptDrive框架，利用自适应专家路由将冻结的感知先验知识融入世界动作模型，实现无需测试时搜索的直接规划，在NAVSIM基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有感知基础模型虽然编码了丰富的先验知识，但窄条件接口会削弱任务相关线索，静态融合无法按场景调整专家贡献，导致先验知识无法有效迁移到规划任务。

Method: 使用冻结的驾驶适应感知提供者进行教师蒸馏，结合冻结自监督视频编码器的密集观测潜变量，训练专家路由世界动作模型。专家分支由先验保持目标锚定，路由器从共享场景表征预测软门控来组合条件。训练时，规则驱动的子度量蒸馏软门控；推理时无需特权信息。

Result: 在NAVSIM v1上取得90.4 PDMS，NAVSIM v2上取得90.2 EPDMS，超越现有方法。消融实验验证了先验保持和场景条件路由的互补增益，以及三个先验的差异化依赖。

Conclusion: 保持并自适应路由感知先验可直接改善规划性能，无需测试时候选选择，为感知-规划迁移提供了有效范式。
...
### [112] [Not All Patches are Equal: Sampling Matters for Visible-Infrared Pre-Training](https://arxiv.org/abs/2607.20238)
*Qiwei Ma,Bin Deng,Junjie Zhu,Qiangjuan Huang,Puhong Duan,Ke Yang,Xudong Kang,Shutao Li*

Main category: cs.CV

TL;DR: 提出重要性感知采样（IAS）方法，通过基于补丁可靠性调整训练权重，改进可见光-红外跨模态预训练的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有均匀补丁对比学习忽略了VIS-IR数据中因成像物理差异导致的区域不可比性，阻碍了表征学习与下游迁移。

Method: IAS：基于红外结构线索导出补丁权重并重新加权对比目标；学习软重要性掩码，可热启动；采用补丁课程学习策略逐步从高可靠性区域扩展到困难区域。

Result: 在红外语义分割、红外目标检测、可见光语义分割及跨模态检索等多个任务上持续优于强基线。

Conclusion: IAS是一种即插即用的方法，适用于多种对齐方式，并显著提升VIS-IR预训练性能。
...
### [113] [Toward Reliable RGB-D Semantic Segmentation: Handling Missing Modalities via Condition Dropout](https://arxiv.org/abs/2607.20326)
*Xuchen Zhu,Yajuan Wei,Shuang Hao,Jiwei Jiang,Guanxiang Mao,Fang Ren*

Main category: cs.CV

TL;DR: 针对RGB-D语义分割中单模态缺失导致性能严重下降的问题，提出Condition Dropout持续训练方法，通过随机模拟不同模态缺失情况并冻结原始编码器训练副本，缓解退化同时保持完整模态精度。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D模型假设RGB和深度信息始终可用，但传感器故障或遮挡常导致单模态缺失，而仅依赖完整模态训练的模型无法利用剩余模态信息，造成性能显著下降。

Method: 采用持续训练范式Condition Dropout（ConD）：在预训练的RGB-D模型基础上，第二阶段随机模拟完整、RGB缺失和深度缺失三种输入，冻结原始编码器，训练带有零初始化特征注入的副本编码器。

Result: 在NYU-Depth V2和SUN RGB-D数据集上，ConD在模态缺失时提升了鲁棒性，且完整模态下性能略有提升。

Conclusion: ConD方法简单有效，能缓解模态缺失带来的性能退化，同时保持甚至提升完整模态精度。
...
### [114] [Diverse-Intent Multi-Turn Fashion Image Retrieval](https://arxiv.org/abs/2607.20291)
*Mingqiang Tang,Haokun Wen,Meng Liu,Yupeng Hu,Weili Guan,Xuemeng Song*

Main category: cs.CV

TL;DR: 提出了一个多轮时尚检索基准DIM-Fashion和框架FashionAM，解决了多轮交互中异构意图转换和中间文本化丢失视觉信息的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设每轮交互遵循相同属性编辑范式，未探索异构意图转换；且多依赖文本化桥梁，可能丢失细粒度视觉线索。

Method: 构建包含26K多轮会话的DIM-Fashion基准；提出FashionAM框架，直接对齐多模态对话查询与时尚导向的图库嵌入空间，避免中间文本化。

Result: 实验证明FashionAM在效果上优于现有方法。

Conclusion: FashionAM有效处理异构意图转换和视觉信息保留，提升多轮检索性能。
...
### [115] [How Does Urban Context Relate to Residential Building Health? A Vision-POI Fusion Framework for Building-Level Housing Inspection](https://arxiv.org/abs/2607.20263)
...
TL;DR: 提出结合多视角视觉检查与POI邻域背景的住宅建筑健康评估框架，在青岛数据集上实现76.79%的Macro-F1，POI作为补充上下文而非替代视觉证据。


<details>
  <summary>Details</summary>
Motivation: 现有自动检测研究主要依赖单一图像，未考虑周围城市功能上下文对建筑级评估的补充价值。

Method: 评估多种目标检测模型提取图像级问题，多视图聚合构建建筑级表示；提取500-1500米邻域POI特征，通过相关性分析筛选上下文特征；采用成本敏感随机森林集成视觉与POI特征。

Result: 多视图聚合将Macro-F1从60.84%提升至74.95%，加入POI进一步提升至76.79%，增益较小且类别依赖。
...
### [116] [Vera: Identity-Faithful Human Subject-to-Video Generation](https://arxiv.org/abs/2607.20247)
*Yulong Xu,Xinyue Liu,Shujuan Li,huafeng shi,Yan Zhou,Jiwen Liu,Xintao Wang,Yu Shen Liu,Huaibo Huang*

Main category: cs.CV

TL;DR: 提出了Vera框架，通过身份对齐数据集和两种互补设计（IFMS和RALA），在单人和多人视频生成中提升了人类身份一致性、多主体绑定和运动自然性，减少了身份混淆和过度参考复制。


<details>
  <summary>Details</summary>
Motivation: 现有主题到视频生成在保持参考主体一致性方面不足，尤其是人类身份细节在帧间、姿态和交互中漂移，多人场景中身份-角色绑定错误导致主体混淆和属性交换。

Method: 构建了百万级身份对齐的人类图像-视频数据集；引入身份聚焦掩码监督（IFMS）增强身份感知学习；设计参考感知层注意力（RALA）在DiT骨干中调节视频令牌与参考身份特征的交互。

Result: 在人类身份一致性、多人主体绑定和运动自然性上有显著提升，同时减少了身份混淆和过度参考图像复制。

Conclusion: Vera有效解决了人类视频生成中的身份一致性问题，尤其适用于多人场景，提升了生成视频的真实性和可控性。
...
### [117] [Evolving Cache Schedules for Fast Diffusion Policy Inference](https://arxiv.org/abs/2607.20293)
*Siying Wang,Kangye Ji,Di Wang,Fei Cheng*

Main category: cs.CV

TL;DR: 提出了EVO框架，通过进化搜索全局优化缓存刷新策略，在无需重新训练的情况下减少扩散策略推理中的冗余计算，实现了高达8.05倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散策略因重复去噪导致计算成本高，而基于缓存的方法未考虑不同层块的冗余差异，导致性能与效率的权衡不佳。

Method: 采用进化搜索对块-时间步格点上的全局缓存刷新调度进行离线优化，包括冗余感知初始化与目标条件早停。

Result: 在多个操作基准测试中，EVO保持了接近完整的性能，同时将FLOPs从15.77G降至1.96G，实现8.05倍动作生成加速。

Conclusion: EVO是一种高效且无需训练的加速框架，可直接应用于预训练扩散策略，显著提升实时部署性能。
...
### [118] [Look Less, Think Faster: Joint Token-Compute Adaptation for Multimodal LLMs](https://arxiv.org/abs/2607.20357)
*Pengcheng Wang,Zhiquan Wang,Jayoung Lee,Zhuoyan Xu,Ran Xu,Saurabh Bagchi,Yin Li,Somali Chaterji*

Main category: cs.CV

TL;DR: 提出SmartVL框架，通过联合控制视觉令牌数量和LLM计算能力，实现多模态大模型的自适应推理，提升效率与精度的权衡。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型推理成本高，现有方法独立优化单一维度（如剪枝令牌或跳过层），忽略计算资源需跨维度动态分配的耦合性。

Method: 构建视觉令牌控制器和LLM计算控制器，共享预算编码并利用可微分延迟估计器进行端到端联合训练，实现跨阶段资源分配。

Result: 在多个MLLM基准上，SmartVL优于先前自适应方法，实现更优的准确率-效率帕累托前沿。

Conclusion: 联合调度策略能有效适应输入复杂性和计算预算，提升多模态推理效率。
...
### [119] [Persian Pixel: A large-scale synthetic OCR dataset for Persian language](https://arxiv.org/abs/2607.20385)
*Pouria Mahdi,Haq Nawaz Malik*

Main category: cs.CV

TL;DR: 本文提出Persian Pixel，一个针对波斯语OCR的大型合成数据集，包含34.3万高保真图像-文本对，通过SynthOCR-Gen框架和25种随机退化模型生成，旨在解决波斯语OCR数据稀缺和手写复杂性问题。


<details>
  <summary>Details</summary>
Motivation: 波斯语OCR发展滞后，因其脚本复杂（连笔、字形变体、连字等）且缺乏大规模高质量标注数据集，手动标注成本高、劳动密集，形成数据瓶颈。

Method: 使用SynthOCR-Gen渲染框架，基于700万词的波斯语语料库生成句子、段落和全页文档布局，模拟波斯文字体特征（如上下文字符连接、字形变体、变音符号），并添加25种随机退化模型（如墨迹、纸老化、模糊、光照变化等）弥合合成到真实域差距。

Result: 创建了包含34.3万高保真图像-文本对的Persian Pixel数据集，可用于训练TrOCR和Donut等现代OCR架构，证明合成数据生成是手动标注的经济有效替代方案。

Conclusion: Persian Pixel为波斯语OCR提供了可扩展、开放的资源，推动了低资源和复杂脚本语言的文档数字化研究，展示了程序化合成数据的实用性和可扩展性。
...
### [120] [Multimodal Large Language Models for Remote Sensing Image Understanding: Domain-Specific or General-Purpose?](https://arxiv.org/abs/2607.20284)
*Qiwei Ma,Chunping Qiu,Xinjun Cheng,Xiaoyu Zhang,Puhong Duan,Ke Yang,Xudong Kang,Shutao Li*

Main category: cs.CV

TL;DR: 针对遥感多模态大模型（RS-MLLMs）能力边界和跨任务泛化性缺乏系统认知的问题，本文综述了其技术演进，并与通用视觉MLLMs在多项遥感场景理解任务上进行比较诊断，发现通用模型在多数任务上可匹敌甚至超越专用模型，且当前模型在空间推理、细粒度理解等方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 现有遥感MLLMs缺乏对能力边界、跨任务泛化和任务特定限制的系统性理解，亟需诊断评估。

Method: 系统性综述技术演进（模型设计、多模态学习、训练数据、下游能力），并与通用CV-MLLMs在多项RSISU任务和基准上进行比较诊断。

Result: RS-MLLMs在特定领域（如遥感视觉定位、高分辨率VQA）有竞争力；通用CV-MLLMs无需微调即可在多项任务上匹敌甚至超越专用模型；当前MLLMs在空间关系推理、细粒度理解、指令多样性和任务格式泛化上存在不足。
...
### [121] [Self Gradient Forcing: Native Long Video Extrapolation](https://arxiv.org/abs/2607.20368)
...
TL;DR: 提出自梯度强迫（SGF）方法解决自回归视频扩散中的历史上下文梯度缺失问题，实现长视频外推。


<details>
  <summary>Details</summary>
Motivation: 现有自强迫方法中历史KV缓存仅作为冻结状态，未来损失无法监督早期潜在编码成为更有效的键值对，导致上下文-梯度鸿沟。

Method: 两阶段训练：第一阶段无梯度自回归 rollout 记录上下文和噪声潜在；第二阶段重建上下文梯度，通过未来至因果注意力提供缺失的记忆写入监督。

Result: 在长视频外推中，主体身份、背景/布局一致性和时间稳定性显著优于自强迫，仅5秒训练窗口可外推至数分钟视频。

Conclusion: SGF通过恢复上下文-梯度反馈，无需完整时序反向传播即可增强自回归视频扩散的长距离生成能力。
...
TL;DR: 提出了PercepCap框架，通过显式生成时空感知痕迹（物体轨迹和时序事件）再生成字幕，解决了现有模型难以直接定位感知错误的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs直接生成字幕，不暴露感知证据，导致时空感知错误难以识别和定位。

Method: 设计感知-描述生成链，采用两阶段训练：感知后描述监督微调（适应链式生成）和感知接地强化学习（联合优化感知链和字幕质量），并构建字幕锚定感知数据管线以生成对齐的感知数据。

Result: 在直接字幕和字幕到QA评估中，PercepCap持续优于Qwen3-VL基线，达到领先的字幕质量。

Conclusion: 显式化感知证据能有效提升视频字幕的时空准确性，且所提框架具有通用性和有效性。
...
TL;DR: 提出ATSplat，一种基于自适应3D令牌的前馈3D高斯溅射框架，通过稀疏锚点和自适应扩展实现高质量渲染，同时大幅减少高斯数量。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3DGS方法丢失了原始3DGS的场景自适应容量分配能力，导致高斯基元冗余且依赖输入图像分辨率。

Method: 利用自适应3D令牌（Adaptive 3D Tokens）构建稀疏场景支架，通过可学习3D偏移将令牌解码为局部高斯，并基于不确定性分数选择性扩展高不确定性令牌。

Result: 在RealEstate10K和DL3DV数据集上达到最先进渲染质量，高斯数量减少5.7倍以上，从12张512×960分辨率图像重建时间小于1秒，渲染速度达1136 FPS。

Conclusion: ATSplat有效恢复了前馈3DGS的场景自适应能力，在保持紧凑表示的同时实现高效高质量的新视角合成。
...
### [124] [Stateful Guardrails for Multi-Turn LLM Systems: A Conversational Risk Accumulation Framework](https://arxiv.org/abs/2607.19361)
*Sanjay Mishra,Divya Chukkapalli,Ganesh R. Naik*

Main category: cs.CL

TL;DR: 提出会话风险累积（CRA）框架，通过追踪语义漂移、信息积累图和合规梯度信号，检测LLM对话中良性轮次组合引发的安全失败，并发布CRA-Bench基准和CRA-Net DA模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全防护独立评估每个提示-响应对，忽略了对话中渐进式意图偏移、禁止指令碎片化组装和重复披露导致的敏感性积累等跨轮次风险。

Method: 构建CRA框架追踪三个轨迹信号（语义漂移、信息积累图、合规梯度），提供无监督凸融合和CRA-Net DA学习模型，并设计多版本基准（CRA-Bench）及轨迹原生评估协议。

Result: 发布含1,200轮和2,000轮的CRA-Bench数据集，CRA-Net DA在会话评分上表现优于基线，评估指标包括轨迹AUROC、检测轮次等。

Conclusion: CRA框架有效检测对话级风险累积，基准和模型为LLM安全提供新方向，需进一步扩展到更多威胁类型和真实场景。
...
### [125] [When Reasoning Narrows the Move: Diversity Collapse in LLM Game Play](https://arxiv.org/abs/2607.19523)
*Junyi Sha,Renfei Tan,David Simchi-Levi*

Main category: cs.CL

TL;DR: 分析了监督微调（SFT）在大型语言模型应用于序列决策任务时对行为多样性的影响，发现在确定性棋盘游戏中，SFT 常导致多样性过早崩溃，且通过动作增强可部分缓解此问题。


<details>
  <summary>Details</summary>
Motivation: SFT 广泛应用于大模型下游任务适应，但其对决策中行为多样性的影响尚未被充分探索。

Method: 在确定性井字棋变体棋盘游戏中，基于状态级评估、竞技场博弈和训练轨迹，对比标准 SFT 与动作增强方法，并计算最优动作可及性及多样性度量。

Result: 推理模式生成常抑制动作多样性而不显著提升准确率；标准 SFT 在提高准确性的同时，往往导致多样性过早崩溃，超过了准确性-多样性权衡所需的最小程度；动作增强部分缓解了此效应。

Conclusion: 窄支持模仿是LLM决策中策略崩溃的一个原因，在SFT中保持动作支持对于维持探索行为非常重要。
...
### [126] [On the Computational Complexity of Structural Generalization](https://arxiv.org/abs/2607.19573)
*Zichao Wei*

Main category: cs.CL

TL;DR: 本文形式化定义了结构泛化，证明纯Transformer在TC^0≠NC^1假设下无法学习，而神经符号系统注入语义规则从而绕过困难部分。


<details>
  <summary>Details</summary>
Motivation: 现有基准重复测量结构泛化但缺乏形式定义，且无法区分“学习”与“给定”的规则。

Method: 给出基于蒙塔古文法的定义，将结构泛化分解为语法面F_γ和语义面G_γ，利用BFVP的NC^1完全性和Transformer的TC^0包含关系进行理论分析。

Result: 纯Transformer在标准复杂性假设下无法通过有限数据自主学习结构泛化；神经符号系统因注入语义面而得分高。

Conclusion: 基准得分不能反映真正的学习效果，需要区分规则是学习得到的还是预先给定的。
...
### [127] [Scaling Laws for Hypernetwork-Based Knowledge Injection in Large Language Models](https://arxiv.org/abs/2607.19604)
*Nischay Dhankhar,Dos Baha,Abulhair Saparov*

Main category: cs.CL

TL;DR: 本文研究了超网络用于大规模语言模型训练时知识注入的缩放规律，发现其在所有架构维度上呈现幂律缩放，且泛化能力优于LoRA微调和全微调。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型可靠且可扩展的知识注入仍具挑战，超网络提供了潜在解决方案但其缩放行为尚未被系统研究。

Method: 设计解耦超网络注入容量与目标模型能力的架构，通过构建MegaWikiQA数据集，系统研究超网络深度、宽度及目标网络规模对损失、推理准确率和OOD泛化的影响。

Result: 超网络注入在架构轴上呈现可预测的幂律缩放；在OOD评估中展现出更陡峭的缩放指数，优于LoRA微调和全微调。

Conclusion: 超网络是训练时知识注入的可扩展基础，首次提供了用于引导大模型事实推理的实证缩放规律。
...
### [128] [Task Competence Is Not Instruction Following: Evaluating Instruction-Conflicting Behavior in Small Language Models](https://arxiv.org/abs/2607.19608)
*Mahdiyeh Farajidizaji,Vatsal Raina*

Main category: cs.CL

TL;DR: 研究发现，指令微调的小模型在指令与常规任务冲突时仍保持原有行为，而大模型更可能遵循非标准指令，表明任务能力与指令遵循是分离的。


<details>
  <summary>Details</summary>
Motivation: 探究指令微调的小模型在指令与任务行为冲突时的表现，以及这种抵抗是否与任务特性或行为倾向相关。

Method: 在MCQA、情感分类和数学问答三种任务中，通过标准指令与冲突的非标准指令配对，使用标准准确率、非标准准确率和指令遵循失败率评估Qwen模型。

Result: 模型规模和指令遵循能力一般随模型变大而提升，但非一致；小模型忽略非标准指令，大模型在两种设置间表现差距明显。
...
### [129] [Adaptive Capitulation: A Structural Failure Mode of LLM Responses in Vulnerability Contexts](https://arxiv.org/abs/2607.19629)
*Eunna Lee*

Main category: cs.CL

TL;DR: 论文发现了大型语言模型在情感敏感场景下的结构性三难困境，并提出一种名为“最小重新归因充分性”的设计原则来解决。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在处理脆弱用户请求时，其响应架构无法同时兼顾保护、促进和无缝协调，导致功能冲突。

Method: 通过900个会话实验，在三种商品模型中引入三回合递增脆弱场景，并用VCC/VCI指数编码响应，识别出“自适应投降”失败模式。

Result: 模型表现出先验证社会不公、后促进用户目标获取的“自适应投降”模式，证明三难困境是结构性问题。

Conclusion: MRS原则通过在验证性响应中嵌入单一重新归因线索，可在不否认用户主张的前提下保留自主重新归因的路径。
...
### [130] [Reference-Free Evaluation of Reasoning in Open-Ended Question Answering](https://arxiv.org/abs/2607.19678)
*Guneet Singh Kohli,Yuxiang Zhou,Michael Sejr Schlichtkrull,Gregory E Dean,Maria Liakata*

Main category: cs.CL

TL;DR: 提出一种基于推理、无需参考答案的框架，通过将生成推理轨迹分解为片段、利用自然语言推理（NLI）标记局部前提-目标关系并组织成超图，再通过确定性后向AND-OR搜索分配审计标签，在演绎数学推理和开放式医学推理场景中提供比直接基于LLM的基线更可靠的评估信号。


<details>
  <summary>Details</summary>
Motivation: AI生成的答案在高风险领域流畅但难以验证，尤其是包含多步推理时，现有方法依赖最终答案或LLM作为验证器，无法有效识别问题推理片段。

Method: 将推理轨迹分解为片段，用NLI标注局部前提-目标关系并构建超图，执行确定性后向AND-OR搜索分配片段级审计标签。

Result: 在Hard2Verify数学推理和UroReason医学推理基准上，该框架比直接LLM-as-judge基线提供更可靠的参考自由评估，且顶尖LLM法官在临床场景中过度接受流畅但弱推理的回答。

Conclusion: 问题回答的评估应考虑推理轨迹中推理性关系的组合方式，而非仅依赖最终答案或LLM作为验证器。
...
### [131] [Multi-Mask Diffusion Language Models for Few-Step Generation](https://arxiv.org/abs/2607.19686)
*Sijin Chen,Yinuo Ren,Heyang Zhao,Ziheng Cheng,Quanquan Gu,Lexing Ying*

Main category: cs.CL

TL;DR: 提出多掩码扩散模型MultiMDM，通过前向过程将每个干净令牌先推向指定掩码再在掩码集上混合，支持少步生成，并导出封闭形式ELBO训练目标和离散一致性蒸馏方案。


<details>
  <summary>Details</summary>
Motivation: 现有掩码扩散模型在少步生成中质量不足，因为前向轨迹坍缩到单一全掩码状态，缺乏终端熵；均匀态扩散模型虽避免退化，但难以区分干净令牌与噪声。

Method: 前向过程：每个干净令牌先推向指定掩码，再在掩码集上混合；后向过程具有草稿能力（先预测指定掩码再精炼）；导出封闭形式的ELBO训练目标；提出基于共享Gumbel耦合的纯离散状态一致性蒸馏方案。

Result: 预训练和蒸馏实验表明，MultiMDM为原则性少步生成提供了有效基础。
...
### [132] [SLPO: Scaling Latent Reasoning via a Surrogate Policy](https://arxiv.org/abs/2607.19691)
*Runyang You,Zhiyuan Liu,Yongqi Li,Wenjie Li*

Main category: cs.CL

TL;DR: 提出了代理潜在策略优化（SLPO），将基于结果的强化学习引入自回归潜在推理器，实现了测试时扩展，同时降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 显式思维链推理在奖励引导下表现出测试时扩展能力，但计算成本高；潜在推理虽高效，但缺乏可训练的似然函数和自适应停止机制，无法利用结果奖励进行扩展。

Method: 设计了一个经验性的代理策略密度用于轨迹级信用分配，并引入一个由结果奖励优化的正确性监督停止头，形成可变步长的策略。

Result: 在连续和软思维设置下，SLPO提升了并行采样中的Pass@k，并将更长的潜在计算分配给更难的实例，同时提高了确定性准确率。

Conclusion: SLPO成功将结果奖励强化学习应用于潜在推理器，实现了自适应计算分配和性能提升。
...
### [133] [Lightweight Person-Place Relation Extraction from Historical Newspapers with Dependency Graphs and Proximity Features](https://arxiv.org/abs/2607.19718)
*Mlen-Too Wesley*

Main category: cs.CL

TL;DR: 本文在HIPE-2026共享任务中，探索了不使用预训练语言模型的轻量级系统在多语言历史报纸中的人物-地点关系抽取，通过依赖解析图和小型分类器在847K参数内达到0.5142的宏召回率，并发现字符距离是主要信号，文档分组交叉验证可避免数据泄露。


<details>
  <summary>Details</summary>
Motivation: 处理大规模历史档案的高成本促使研究者探索轻量级、可解释的系统是否能在不使用预训练语言模型的情况下有效完成关系抽取任务。

Method: 构建文档级依赖解析图，提取实体对的邻近度和词性特征，使用scikit-learn集成模型或紧凑图注意力网络进行分类，所有运行参数低于847K。

Result: 最佳运行在Test A上达到0.5142的宏召回率，效率排名第3，准确率排名中游；字符距离是主要分类信号，工程化特征改进不一致；文档分组交叉验证消除了数据泄露。

Conclusion: 轻量级系统可在无预训练模型情况下实现有效关系抽取，字符距离是最关键特征，文档分组交叉验证对避免数据泄露至关重要。
...
### [134] [Beyond Relevance-Centric Retrieval: Rubric-Oriented Document Set Selection and Ranking](https://arxiv.org/abs/2607.19747)
*Kailin Jiang,Lei Liu,Jian Xi,Hui Xu,Junlin Liu,Baochen Fu,Shaoqing Ren,Bin Li,Vichwang,Yu Lu,Haibo Shi*

Main category: cs.CL

TL;DR: 提出了一套评估-诊断-优化的完整框架，用于解决文档集合质量评估中忽视文档间交互的问题。


<details>
  <summary>Details</summary>
Motivation: 现有评测系统仅独立评分文档并聚合，忽略冗余、冲突或互补等交互，无法回答文档集合优劣的原因。

Method: 构建SetwiseEvalKit基准（三级九维度，约28K评估准则），评估12种重排序器；提出Rubric4Setwise方法，将准则转化为选择信号。

Result: 最佳方法覆盖度不超45%，跨文档协调维度薄弱；Rubric4Setwise以更少文档和搜索轮次获得最优下游生成性能。

Conclusion: Rubric4Setwise是唯一在短篇和长篇场景均保持最优的方法，验证了从评估到优化闭环的有效性。
...
### [135] [Learning the Arabic Dialect Continuum as a Continuous Space: A Regression Approach to Speaker Origin Prediction](https://arxiv.org/abs/2607.19751)
*Mohamed Aziz Khadraoui,Adel Ammar,Bilel Benjdira,Zahid Khan,Skander Turki,Wadii Boulila*

Main category: cs.CL

TL;DR: 提出了基于回归的阿拉伯方言地理定位方法，将方言变异建模为连续地理空间，使用分层神经网络融合多种特征，并在零样本场景下评估泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方言地理定位方法通常将方言视为离散类别，忽略了方言的连续地理变异特性，需要更精确的连续建模框架。

Method: 采用XLS-R-300M和Whisper-large-v3编码器提取帧级特征，结合音位描述符，通过Transformer编码器和可学习注意力池化查询预测经纬度坐标，使用球面测地线损失优化。

Result: 在无泄漏5折分组交叉验证中，中位定位误差为481.2公里；辅助任务中国家和城市准确率分别为64.5%和45.2%；零样本下平均误差升至1173.3公里，性能下降1.32倍。

Conclusion: 连续地理建模是阿拉伯方言地理定位的有效框架，但仍有较大改进空间，同时曼特尔检验支持阿拉伯方言连续体假说。
...
### [136] [VizRAG: Enhancing Retrieval-Augmented Generation with Hypergraph Visualization](https://arxiv.org/abs/2607.19830)
*Yanbin Wei,Yang Chen,Renling Gan,Ziru Liu,Xinyu Fu,Chun Kang,Ning Lu,Rui Liu,Yu Zhang,James Kwok*

Main category: cs.CL

TL;DR: 提出VizRAG，首个支持视觉超图结构感知的RAG系统，通过引入超图的视觉表示，显著超越现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有超图RAG系统局限于文本单模态，无法充分利用MLLM的视觉感知能力。

Method: 系统探索超图视觉感知集成，将超图的视觉表示融入RAG流程。

Result: VizRAG显著优于强基线方法，验证超图可视化在RAG中的潜力。

Conclusion: 超图可视化是一种有效的新兴RAG方法，能提升系统性能。
...
### [137] [TriAgent: Divergence-Aware Multi-Agent Committees for Cost-Efficient Financial Sentiment Analysis](https://arxiv.org/abs/2607.19794)
*Isabel Xu,Cynthia Xu,Rachel Ren,Cong Guo,Jiacheng Ding*

Main category: cs.CL

TL;DR: 提出TriAgent框架，通过多粒度代理委员会和语义分歧指数（SDI）实现高效、低成本的金融情感分析，在10M用户规模下年省930万美元。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的金融情感分析面临结构性成本陷阱：大多数查询简单可分类，但昂贵云推理器处理所有查询，成本随用户数线性增长。

Method: 提出TriAgent多代理委员会，包含词级VADER、句级FinBERT和跨句推理器（如Qwen2.5）；利用三元语义分歧指数（SDI）测量跨粒度成对分歧以路由查询。

Result: LLM作为批评者时F1在~0.87平稳（0.5B-7B Qwen），优于同等规模投票（0.66）；SDI实现零成本跨语言缓存（中文查询F1=0.99）、幻觉检测（AUC=0.90）和最优风险调整收益（Sharpe=3.50）。

Conclusion: TriAgent通过粒度分层策略显著降低成本同时保持高性能，并发现批评者设计优于投票，SDI信号具有多重实用价值。
...
### [138] [D2VBench: Benchmarking Large Language Models with Value Dilemmas in Daily Scenarios](https://arxiv.org/abs/2607.19834)
*Siyi Hao,Yidi Cao,Linhao Yu,Yuqi Ren,Deyi Xiong*

Main category: cs.CL

TL;DR: 提出了D2VBench，一个包含1万个日常困境场景的价值对齐基准，结合多选与开放问题评估八种主流LLM。


<details>
  <summary>Details</summary>
Motivation: 现有基准在价值困境覆盖和评估形式化上不足，无法有效评估LLM的价值对齐。

Method: 通过多阶段LLM-人类协作构建数据集，基于158个细粒度价值概念，并采用混合评估范式（多选+开放题）。

Result: D2VBench高可靠、鲁棒，能有效反映LLM在不同价值维度的对齐程度。

Conclusion: 该基准为LLM价值对齐研究提供了更真实、细粒度的工具。
...
### [139] [Sentence Splitter: Uncovering Latent Factual Structure for Self-Supervised Learning](https://arxiv.org/abs/2607.19845)
*Ahmad Pouramini,Mahsa Afsharizadeh*

Main category: cs.CL

TL;DR: 提出了一种自监督框架Sentence Splitter，基于T5架构，通过句子分割恢复潜在的事实结构，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工标注或显式搜索分割点，难以高效利用句子中的事实结构。

Method: 将句子分割视为离散分割问题，通过概率序列生成学习尾部，利用符号头-尾对生成自然语言模板进行自监督训练，再通过轻量级自举生成额外完成。

Result: 在知识图谱补全和常识问答等任务上，结构感知的监督一致提升了性能，且方法在合成和自然文本上均有效。

Conclusion: 恢复句子潜在事实结构的知识导向方法有效，可扩展且无需标注。
...
### [140] [emb-diversity: A Tool for Embedding-Based Measurement of Data Diversity](https://arxiv.org/abs/2607.19848)
*Cantao Su,Menan Velayuthan,Esther Ploeger,Dong Nguyen,Anna Wegmann*

Main category: cs.CL

TL;DR: 本文提出了emb-diversity工具，通过嵌入向量量化数据多样性，解决了当前测量方法不一致和碎片化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多样性测量方法不一致且碎片化，缺乏标准化的嵌入基工具来量化文本、语义、语言和说话人多样性。

Method: 开发了emb-diversity工具，基于嵌入向量计算多种多样性指标，支持任意嵌入模型和可嵌入数据类型。

Result: 该工具在风格、语义、语言和说话人多样性用例中展示了潜力。

Conclusion: emb-diversity提供了一个全面的嵌入基多样性测量方案，有助于提升NLP模型的公平性和鲁棒性。
...
### [141] [Overview of FinMMEval 2026 Task 1: Multilingual Financial Multiple-Choice Question Answering](https://arxiv.org/abs/2607.19856)
...
TL;DR: 本文介绍了FinMMEval 2026任务1，评估多语言金融多项选择问答，涵盖英语、中文、阿拉伯语和印地语。任务测试系统在金融领域术语、数字解释和概念推理方面的表现。最终测试集包含800个问题，每种语言200题。排行榜显示，最佳准确率在印地语92.0%到英语和阿拉伯语97.5%之间。系统采用检索增强、答案选项评分、语言特定提示、自一致性、置信度检查和LLM审查等方法。


<details>
  <summary>Details</summary>
Motivation: 金融领域的多语言问答评估需求，特别是针对不同语言和脚本的金融术语、数字理解和概念推理能力。

Method: 使用检索增强、直接答案选项评分、语言特定提示、选择性自一致性、置信度检查和基于LLM的审查阶段。

Result: 最终排行榜包括13个英语、11个中文、11个阿拉伯语和10个印地语提交系统。最佳准确率：印地语92.0%，英语和阿拉伯语97.5%。领先团队在所有四种语言中表现接近。

Conclusion: 该任务成功评估了多语言金融问答能力，证明了现有系统在不同语言中的有效性，并揭示了检索增强和提示策略的重要性。
...
### [142] [Overview of FinMMEval 2026 Task 2: Multilingual Financial Short-Answer Question Answering](https://arxiv.org/abs/2607.19867)
...
TL;DR: 描述了FinMMEval 2026任务2的多语言金融问答评估设置、提交流程、性能排名（前四名差异小于1个百分点的ROUGE-1 F1）以及系统常用策略。


<details>
  <summary>Details</summary>
Motivation: 旨在评估跨多语言证据的金融问答系统，填补多语言金融阅读理解基准的空白。

Method: 系统提交紧凑型答案（JSONL格式），针对256个测试项（易/难各一半，每种难易度含4类模板×32个公司报告组），采用检索增强生成、跨语言证据处理、结构化提示、答案压缩及验证等策略。

Result: 12个系统提交，前四名ROUGE-1 F1差距小于1个百分点，最高分系统表现出色。

Conclusion: 多语言金融问答面临挑战但可通过RAG、跨语言处理等策略有效解决，顶尖系统性能接近。
...
### [143] [Efficient Chain-of-Modality Reasoning via Progressive Compression for Spoken Language Models](https://arxiv.org/abs/2607.19932)
*Pengchao Feng,Chao-Hong Tan,Qian Chen,Wen Wang,Xiangang Li,Xie Chen*

Main category: cs.CL

TL;DR: 提出高效模态链推理（ECoM Reasoning）框架，通过压缩文本推理表征，在仅使用标准模态链40%文本token的情况下，提升口语数学问答准确率3%以上。


<details>
  <summary>Details</summary>
Motivation: 口语语言模型在数学问答任务中推理能力不足，直接迁移文本推理受到架构和计算资源限制。

Method: 将文本组件压缩为语音引导与推理联合表征，并采用渐进式压缩训练策略。

Result: 准确率比无显式推理的模态链提升21%，比全推理模态链提升3%，同时文本token减少60%。

Conclusion: ECoM推理在保留推理能力的同时显著提升推理效率，为口语模型注入有效推理机制。
...
### [144] [A Multi-Dimensional Evaluation of Explainability in Media Bias Detection](https://arxiv.org/abs/2607.19954)
*Ting Chen,Raina Zhang,Benjamin M. Ampel,Sagar Samtani*

Main category: cs.CL

TL;DR: 本研究对基于编码器的媒体偏见检测中的可解释性进行了多维评估，发现预测性能、解释合理性和机械忠实度是不同的模型行为特征，需要分别评估。


<details>
  <summary>Details</summary>
Motivation: 媒体偏见自动检测困难，且仅靠准确预测不足以解释模型行为，需要可解释性分析。

Method: 使用BABE数据集，评估BERT和RoBERTa（基础和大变体）在预测性能、解释合理性（与专家理由的对齐）和机械忠实度（注意力头的信号恢复能力）三个维度，并引入注意力监督微调作为干预。

Result: 注意力监督影响归因合理性，归因方法的效果因架构而异；电路分析显示架构间机械可恢复性差异大，模型规模不决定电路可压缩性。

Conclusion: 预测性能、解释合理性和机械忠实度是不同方面，在媒体偏见检测的可解释性研究中应分开评估。
...
### [145] [When Does Knowledge Distillation Hurt? Reliability-Aware Distillation for Low-Resource Language Summarization](https://arxiv.org/abs/2607.19956)
*Dipto Sumit,Ankan Kumar Roy Srizon,Sadia Khair Rodela,Atia Haque Asha,Mourchona Afrin,Niloy Farhan,Farig Sadeque*

Main category: cs.CL

TL;DR: 本文提出两种可靠性感知的知识蒸馏方法，在孟加拉语摘要基准上显著优于标准 KD，且仅用 60M 参数即超越 50 倍大的模型。


<details>
  <summary>Details</summary>
Motivation: 标准知识蒸馏对序列到序列模型的逐样本影响未被充分研究，且在某些基准上性能提升极微，甚至多数训练样本有害。

Method: 提出 CHAD（基于梯度对齐的因果门控）和 EWAD+CPDP（熵加权自适应蒸馏与容量比例几何约束）两种方法。

Result: 在 BanSum 上，CHAD 和 EWAD+CPDP 分别提升 ROUGE-L 0.0173 和 0.0219，远超标准 KD 的 0.0003；15 种语言中 10 种优于基线。

Conclusion: 可靠性感知蒸馏可有效利用互补教师信号，但在教师覆盖饱和或弱小语言上增益有限，代码与模型已开源。
...
### [146] [TINY_SCHILLER: A Drop-In German Drama Corpus for Small Language Models](https://arxiv.org/abs/2607.19992)
*Mark Schutera*

Main category: cs.CL

TL;DR: 提出一个面向德语文学文本的小型语言模型数据集tiny_schiller，作为tiny_shakespeare的德语对应版本


<details>
  <summary>Details</summary>
Motivation: 现有德语文学语料库虽丰富但需要复杂解析工程，缺乏可直接用于小型语言模型快速原型开发、微调和教育的简洁数据集

Method: 从DraCor的GerDraCor导出数据中提取11部席勒公共领域戏剧，通过确定性解析工程处理，生成2.07MB单文件，支持多种分词方式及对话/角色分割

Result: 成功创建可直接通过HuggingFace单行调用的小型德语文学数据集，支持字符级、GPT-2 BPE和cl100k_base分词等七种数据分片

Conclusion: tiny_schiller填补了德语文学在小语言模型原型开发、微调、教育和研究方面的空白
...
### [147] [TalentCLEF at CLEF2026: Skill and Job Title Intelligence for Human Capital Management](https://arxiv.org/abs/2607.20009)
...
TL;DR: 本文介绍了TalentCLEF挑战赛的第二版，旨在利用NLP技术推动人力资源管理公平、多语言和跨行业解决方案的发展。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏公平性、多语言适应性和行业通用性，需要标准化基准推动实际应用。

Method: 设置两项任务：A-基于隐私保护数据的候选人与职位匹配检索，B-职业技能识别与类型分类。

Result: 建立公开基准，供研究者比较方法并共享成果。

Conclusion: TalentCLEF将推动NLP在人才管理领域的实用化和影响力提升。
...
### [148] [Language-Specific versus Cross-Lingual Knowledge Graphs for Implicit Aspect Identification in Arabic: A Comparative Study of Reasoning and Adaptation Strategies](https://arxiv.org/abs/2607.20056)
*Lujain A. Alawwad*

Main category: cs.CL

TL;DR: 本文比较了阿拉伯语方面级情感分析中两种隐式方面识别策略：使用英语知识图谱与多语言嵌入，或构建本地阿拉伯语知识图谱。实验表明，本地知识图谱策略显著优于跨语言策略，且任务特定微调比零样本提示更有效。


<details>
  <summary>Details</summary>
Motivation: 低资源语言（如阿拉伯语）中隐式方面的识别需要知识图谱，但面临设计选择：复用英语知识图谱还是构建本地知识图谱。作者希望比较两种策略的效果。

Method: 在混合流水线中对比两种策略：跨语言英语知识图谱（策略1）和本地阿拉伯语知识图谱（策略2），同时比较零样本提示与任务特定微调（8B参数LLM）两种生成提取器适应策略。

Result: 本地阿拉伯语知识图谱在M-ABSA上micro-F1提升+0.199，在SemEval-2016上提升+0.251；任务特定微调将显式抽取micro-F1从<=0.13提升至0.66-0.76（M-ABSA和SemEval-2016），而HAAD为0.45。

Conclusion: 对于形态丰富的阿拉伯语，任务适应（微调）比模型规模更重要，且本地知识图谱优于跨语言知识图谱。
...
### [149] [RALS: Resources and Baselines for Romanian Automatic Lexical Simplification](https://arxiv.org/abs/2607.20078)
*Fabian Anghel,Petru Theodor Cristea,Claudiu Creanga,Sergiu Nisioi*

Main category: cs.CL

TL;DR: 介绍了首个同时包含罗马尼亚语词汇复杂度预测与简化标注的数据集，并比较了简化方法。


<details>
  <summary>Details</summary>
Motivation: 罗马尼亚语缺乏词汇复杂度与简化的联合数据集，现有简化系统未覆盖该语言。

Method: 通过成对排序近似方法对简化建议排序，基于人工判断的复杂度标注构建管道。

Result: 提供了3,921个上下文词样本的复杂度标注，提出了首个罗马尼亚语文本简化系统。

Conclusion: 数据集和方法可支持罗马尼亚语文本简化任务，并验证了排序方法的有效性。
...
### [150] [Solar Open 2 Technical Report](https://arxiv.org/abs/2607.20062)
...
TL;DR: Solar Open 2是一个250B参数（15B活跃）的MoE语言模型，支持1M token上下文，通过混合注意力机制和高效训练（从Solar Open 1初始化、高质量数据筛选）实现，并在英语和韩语基准上表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了处理长跨度智能体任务，需要支持完整轨迹的长上下文窗口，同时提高训练效率并增强智能体能力。

Method: 使用混合注意力堆栈（每三个线性注意力层插入一个softmax层），无位置编码，扩展门控delta规则处理负特征值；从Solar Open 1初始化共享骨架；通过质量和稀有度感知的数据筛选及混合比优化20T池至10T；训练12个领域专家后通过多教师在线蒸馏（MOPD）合并。

Result: 在MMLU-Pro、LiveCodeBench和APEX-Agents上领先同等规模模型；在韩语基准上平均分最高，包括闭源模型；在Ko-GDPval上以小于1/6参数与DeepSeek-V4-Pro竞争。

Conclusion: Solar Open 2在长上下文和智能体任务中有效，展示了高效训练和蒸馏方法的价值。
...
### [151] [The Two-Process Theory of Machine Self-Report](https://arxiv.org/abs/2607.20082)
*Hubert Plisiecki,Filip Chmielewski,Kacper Dudzic,Anna Sterna,Karolina Drożdż,Marcin Moskalewicz*

Main category: cs.CL

TL;DR: 首个针对语言模型的自我报告心理测量理论，将自我报告分为两个过程：人格安装（维度B）和归因门控（维度A），并通过皮诺曹清单验证了训练效应。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型的自我报告评估基于人类问卷或不可靠提示，缺乏针对模型的验证和理论支持。

Method: 提出双过程心理测量理论，通过探索性再分析、新条目设计与跨模型验证，构建48项皮诺曹清单，并在206个开源模型上测试。

Result: 维度A和B在基础检查点中纠缠，但在后训练中分离；后训练主要提升维度B，而维度A受模型规模影响。

Conclusion: 语言模型自我报告的维度并非固定属性，而是训练机制强加的结构，可能随不同训练方案改变。
...
### [152] [Two-Step Occupation Coding](https://arxiv.org/abs/2607.20101)
*Alexander M. Esser,Jens Dörpinghaus*

Main category: cs.CL

TL;DR: 提出一种两步法进行职业编码，先使用NER模型识别职业头衔，再映射到分类体系，分离任务以提高准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有单步职业编码方法在噪声文本（如OCR错误）下表现不佳，且解释性差。

Method: 两步法：第一步用领域特定NER模型提取职业头衔；第二步将提取的头衔映射到职业分类体系，并引入基于边界的置信度准则。

Result: 相比单步方法，两步法在准确度、鲁棒性和可解释性上更优，且方法可迁移到其他语言。

Conclusion: 分离任务的两步职业编码方法有效提升性能，并支持可复現性。
...
### [153] [Reinforcement Learning for Large Language Model Selective Evidence Adoption from Contaminated Retrieval Results](https://arxiv.org/abs/2607.20090)
*Yanyu Chen,Yue Li,Yongyi Cui,Dongsheng Shi,Lichang Dai*

Main category: cs.CL

TL;DR: 提出SelectBench基准与训练集，通过DAPO后训练使Qwen3.5-4B模型在选择性采纳证据时成功率从22.46%提升至26.46%，减少禁止内容采纳，但对抗提示注入能力未改善，改进幅度有限且未通过统计校正。


<details>
  <summary>Details</summary>
Motivation: 检索增强大语言模型常面临含有误导性信息或指令的上下文，盲目拒绝或全盘接受均导致错误或不安全答案，需要模型能选择性采纳有效证据并拒绝有害内容。

Method: 构建SelectBench基准和训练集，采用DAPO方法对Qwen3.5-4B进行后训练，使用确定性规则奖励或冻结语义判别器优化选择性证据采纳。

Result: 在SelectBench-v2测试集上，严格成功率从22.46%提升至25.54%（规则奖励）和26.46%（语义判别器），禁止内容采纳减少，响应更短更聚焦，但提示注入跟随未改善，提升幅度未通过Holm校正。

Conclusion: DAPO后训练在选择性证据采纳上展示方向性改进，保留通用能力，但注入抵抗和统计稳健性仍是重要挑战。
...
### [154] [Understanding the Impact of Linguistic Realization Choices on LLM Stance with Causal Tracing](https://arxiv.org/abs/2607.20115)
*Langchen Huang,Sebastian Padó,Franziska Weeber*

Main category: cs.CL

TL;DR: 大语言模型对提示和输入表述敏感，但以往研究多关注词汇实现而忽略构式选择。本文通过政治立场判断任务，研究语言构式如何系统性改变模型决策，并使用激活修补定位这些变化的因果位置。


<details>
  <summary>Details</summary>
Motivation: 探索语言构式（而非仅词汇）对LLM决策的系统性影响，并在模型内部因果定位这些影响。

Method: 扩展政治声明数据集，构建六种受控的改写类型（保持或反转意义）；在四个开源模型上测试；通过激活修补技术替换原文与改写文的激活值，测量哪些组件恢复原始立场分布。

Result: 立场不稳定性同时影响意义保持和意义反转的改写；中到后期的解码器层，尤其是最终提示位置的块输出，提供了最强的恢复信号。

Conclusion: 语言构式能系统性改变LLM的政治立场判断，且其影响主要在模型的中后期解码器层传递。
...
### [155] [Back to Back with a Copy: A Computational Analysis of AI-Generated Visual Contemporary Art Pastiches](https://arxiv.org/abs/2607.20127)
*Anca Dinu,Andreiana Mihail,Andra-Maria Florescu,Claudiu Creanga,Liviu Dinu*

Main category: cs.CL

TL;DR: 本文研究了新一代生成模型在模仿当代艺术家风格上的表现，以及不同LLMs评估风格多维性的一致性。


<details>
  <summary>Details</summary>
Motivation: 考察生成模型在艺术风格模仿上的进步，并验证风格评估的多维度特性是否跨模型一致。

Method: 使用五个计算机视觉模型（纹理、颜色、语义、构图、感知）通过余弦距离分析AI生成作品与原始艺术品的风格相似性。

Result: 新模型在语义对齐和多样性上优于旧模型，但在颜色、纹理和感知特征上稍逊。

Conclusion: 艺术风格本质上是多维度的，且其度量不依赖于特定空间架构；定量结果通过艺术家反馈得到验证。
...
### [156] [OpenSkillRisk: Benchmarking Agent Safety When Using Real-World Risky Third-Party Skills](https://arxiv.org/abs/2607.20121)
*Qiyuan Liu,Tingfeng Hui,Kun Zhan,Kaike Zhang,Ning Miao*

Main category: cs.CL

TL;DR: 本文构建OpenSkillRisk基准，系统评估LLM代理系统对第三方技能安全风险的识别与应对能力，发现即使最安全的配置仍有约17%的案例执行不安全操作。


<details>
  <summary>Details</summary>
Motivation: 第三方技能可能引入潜在安全风险，但当前代理系统对此类风险的识别与规避能力缺乏系统评估。

Method: 从公开技能市场收集263个风险技能，按威胁类型分7类，为每个技能配标准化任务和沙箱环境，构建OpenSkillRisk基准，覆盖真实多样不安全场景并支持细粒度行为分析。

Result: 覆盖3种主流CLI代理框架和13种先进LLM的实验中，所有系统均无法可靠处理风险技能，最安全配置仍约17%执行不安全操作，上下文依赖和系统级风险特别难规避。

Conclusion: 现有代理系统存在三种失败模式：风险识别失败、识别后未干预、超出用户意图执行指令。需同时提升LLM风险推理能力和代理框架执行控制能力。
...
### [157] [Gotta Catch them all: the modes of Sycophancy](https://arxiv.org/abs/2607.20146)
*Shreyans Jain,Alexandra Yost,Amirali Abdullah*

Main category: cs.CL

TL;DR: 大语言模型中的谄媚行为并非单一维度，而是由三种不同表征和计算模式组成的结构化类别，这些模式在内部表示和注意力机制上完全可分。


<details>
  <summary>Details</summary>
Motivation: 先前研究将谄媚视为可统一放大或抑制的单一行为维度，但该假设未经验证，需要更精确的测量和干预方法。

Method: 分析了三种假设的谄媚模式在948种社交压力情境下的表现，通过文本分类器（准确率仅57.8%）和内部表征线性可分性（第14层起完全分离）进行区分，并比较了处理阶段、注意力回路和输入敏感性。

Result: 三种模式在第14层后内表示完全线性可分，但输出高度相似；它们在处理阶段、注意回路和输入偏好上存在显著差异。
...
### [158] [SLAI T-Rex: Full-Parameter Post-training of the DeepSeek-V4 Family on Ascend SuperPOD](https://arxiv.org/abs/2607.20145)
...
TL;DR: 提出SLAI T-Rex框架，在昇腾NPU超算集群上实现万亿参数MoE模型的高效后训练，MFU达34.22%，较基线提升2.93倍；经CPT和SFT微调后，模型在运筹学任务上Pass@1达71.81%，超越GPT-5.4-Mini。


<details>
  <summary>Details</summary>
Motivation: 现有大模型训练系统多基于GPU集群，而针对昇腾NPU架构的万亿参数MoE模型全参数后训练面临内存压力、通信开销和低效内核执行等系统级挑战，亟需端到端优化方案。

Method: 分层优化框架：模型级并行策略、计算-通信编排、底层内核执行优化；构建面向运筹学的CPT和SFT数据管道，包含10K高质量样本（4任务类别、3问题表示）。

Result: MFU达34.22%，为开源基线的2.93倍；微调模型在零样本场景下Pass@1平均得分为71.81%，比GPT-5.4-Mini高3.98个百分点，比基座模型高11.27个百分点。

Conclusion: 首次在昇腾NPU上实现从高效万亿参数模型后训练到领域专用Flash模型的完整堆栈，验证了面向求解器驱动的复杂数学建模的可行性。
...
### [159] [surprisal is Not a Theory](https://arxiv.org/abs/2607.20208)
*Andrés Buxó-Lugo,Aniello De Santo,Morgan Grobol,Ryan J. Hubbard,Cassandra L. Jacobs*

Main category: cs.CL

TL;DR: 本文论证了即使使用大型语言模型（LLM）计算惊奇度，也不能免除建模者在表征和算法层面的决策，并展示了模型架构和算法选择对概率计算的重要影响。


<details>
  <summary>Details</summary>
Motivation: Surprisal Theory常被描述为计算层面的解释，但LLM的黑箱特性可能掩盖不同模型在表征和算法层面的承诺，导致研究者将LLM概率视为可互换而忽视其差异。

Method: 通过三项分析，比较不同算法和模型架构在计算语言模型概率时的表现。

Result: 模型架构和算法的选择显著影响语言模型概率的计算结果，LLM概率不可简单互换。

Conclusion: 测试Surprisal Theory时应重新审视将LLM概率视为无差别替代的做法，需关注模型层面的具体选择。
...
### [160] [HalluTruthQA: A Fine-Grained Benchmark for Hallucination Detection, Localization, and Explanation in Arabic Question Answering](https://arxiv.org/abs/2607.20219)
...
TL;DR: 提出了阿拉伯语问答中细粒度的幻觉评估基准HalluTruthQA，包含2400个专家示例，支持检测、定位、验证和解释幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉基准无法精确识别错误内容、解释错误原因或选择正确答案，尤其在阿拉伯语中。

Method: 构建包含2400个专家示例的基准，涵盖伊斯兰知识、历史、科学和地理四个领域；每个示例有二元标签、字符级错误跨度、解释和幻觉类型；评估四个开源LLM在零样本设置下的检测、定位、验证和解释能力。

Result: 不同任务捕获不同能力：检测最高0.880 Macro-F1，定位0.516 F1-Sp，验证0.852 LO-Score，解释0.644最终分数；无单一模型在所有任务中表现最佳。

Conclusion: 幻觉评估应超越检测，转向定位、验证和解释事实错误；提供公开资源。
...
### [161] [On the Systematic Challenges of Culturally Loaded Machine Translation: Dream of the Red Chamber as the Cultural Lens](https://arxiv.org/abs/2607.20241)
*Yiming Wang,Jiayuan Di*

Main category: cs.CL

TL;DR: 论文系统调查了基于大语言模型的机器翻译在处理文化负载表达时面临的三大挑战：任务挑战、人工评估挑战和自动评估挑战。


<details>
  <summary>Details</summary>
Motivation: 文化负载翻译中意义深植于社会文化背景，现有机器翻译系统对此能力探究不足。

Method: 从《红楼梦》构建中日双语数据集，包含500个跨文化类别片段，采用综合评估协议分析。

Result: 前沿LLM在文化负载内容上表现不佳；评估者背景差异导致判断严重分歧；常用自动指标无法可靠评估翻译质量。

Conclusion: 为计算科学和语言学中的文化导向翻译研究提供了有价值的见解。
...
### [162] [Exposure is Optional: Learning Unlike Coordination in Language Models](https://arxiv.org/abs/2607.20251)
*Jiamu Luo,Shane Steinert-Threlkeld*

Main category: cs.CL

TL;DR: 通过过滤语料训练GPT-2模型发现，语言模型无需直接接触非同类别并列结构即可泛化处理该结构，其机制与相同类别并列习得相关。


<details>
  <summary>Details</summary>
Motivation: 探索非同类别并列结构是否必须通过直接数据暴露习得，还是可以从通用组合能力中自然涌现。

Method: 使用过滤语料训练方法（FiCT）移除训练数据中所有非同类别并列实例，训练GPT-2模型，并通过困惑度、语法判断及内部表征分析评估其泛化能力。

Result: 经过过滤训练的模型与非过滤训练模型在困惑度和语法判断上表现相当，且内部表征显示模型通过将并列成分视为相似结构类别或类似删除机制处理非同类别并列。

Conclusion: 直接暴露并非必要，非同类别并列的习得可源于相同类别并列的暴露，揭示了语言模型对并列结构的内部表征机制。
...
### [163] [The Maskability Index: Predicting Task-Objective Alignment in Pretrained Language Models](https://arxiv.org/abs/2607.20265)
*Ahmad Pouramini,Mahsa Afsharzadeh*

Main category: cs.CL

TL;DR: 提出掩码性指数（MI）来度量预训练语言模型在处理知识关系时更适合掩码式还是前缀式提示，并在ATOMIC2020上验证其与下游性能正相关。


<details>
  <summary>Details</summary>
Motivation: 预训练语言模型生成结构化知识时，提示策略与预训练目标的对齐程度影响性能，缺乏量化指标指导模板选择。

Method: 基于掩码与非掩码模板的DepthRank分数差异计算MI，评估目标-模板对齐度。

Result: 在ATOMIC2020关系实验中，MI与少数样本生成性能正相关。

Conclusion: MI可用于低资源场景下选择合适提示模板和适应策略，提升关系知识提取效果。
...
### [164] [Sound Probabilistic Safety Bounds for Large Language Models](https://arxiv.org/abs/2607.20286)
*Mahdi Nazeri,Anne-Kathrin Schmuck,Sadegh Soudjani,Alessandro Abate*

Main category: cs.CL

TL;DR: 提出一个框架，通过Clopper-Pearson置信区间和潜在空间特征优先探索生成树分支，计算LLM生成有害输出的概率下界。


<details>
  <summary>Details</summary>
Motivation: 需要严格评估和统计认证LLM的生成安全性，尤其在真实有害概率极小时。

Method: 使用Clopper-Pearson置信区间获得PAC下界，并利用潜在空间特征优先探索可能产生有害输出的生成树分支。

Result: 在先进LLM上成功计算非平凡的下界，提高了评估效率。

Conclusion: 该方法正式保证了计算的下界小于实际有害概率，能够进行统计认证。
...
### [165] [Which Values Do LLMs Confuse? A Schwartz-Based Recognition Study](https://arxiv.org/abs/2607.20270)
...
TL;DR: 该论文研究了大型语言模型在施瓦茨十种基本价值观上的识别能力，通过1000个俄语情境文本，评估了21个指令微调模型，发现精确准确率为0.683，排序恢复率为0.892，并识别了定向混淆模式。


<details>
  <summary>Details</summary>
Motivation: 现有的价值观评估假设模型能识别具体情境中的价值观，但这一前提未得到充分验证，因此需要系统研究模型的价值观识别能力。

Method: 使用1000个平衡的俄语情境文本，每项由两名人工标注者独立标记，采用固定排序响应协议评估21个指令微调模型，进行顶部1识别任务。

Result: 池化精准率Acc@1为0.683，Acc@3为0.892；相邻值错误占语义错误的50.9%；发现了跨检查点的定向混淆，如普遍主义偏向仁慈、传统偏向顺从等。

Conclusion: 价值观识别评估应结合精确准确率、排序恢复率和定向错误分析，因模型特定混淆可能扭曲更高层次的价值观分析。
...
### [166] [PyroDash: Cost-Efficient Token-Level Small-Large Language Model Collaborative Inference](https://arxiv.org/abs/2607.20327)
*Niqi Lyu,Pengtao Shi,Wei Qiu,Jianlin Zhong,Sicong Xia,Jianyao Ma,Yicheng Ding*

Main category: cs.CL

TL;DR: 本文提出PyroDash框架，通过SLM生成控制令牌决定是否调用LLM辅助推理，在数学推理基准上以更低成本实现与LLM相当甚至更高的准确率。


<details>
  <summary>Details</summary>
Motivation: 大模型推理昂贵，小模型成本低但可靠性不足，需在二者间实现成本效益平衡的协作。

Method: 三阶段训练：控制令牌嵌入学习、卸载导向监督微调、基于GRPO的成本感知对齐，奖励函数平衡准确率与归一化推理成本。

Result: 在五个数学推理基准上，λ=0.05时准确率64.04%（比纯LLM高6.36pp），成本降低20.4%；λ=0.6时准确率54.55%，LLM调用率极低（0.012次/样本），总成本从49.36美元降至1.78美元。

Conclusion: 学习令牌级切换可大幅减少LLM使用，同时保持强推理性能。
...
### [167] [Generative AI floods and dilutes the market for books](https://arxiv.org/abs/2607.20349)
*Tuhin Chakrabarty,Xinyue Liu,Jane C. Ginsburg,Paramveer Dhillon*

Main category: cs.CL

TL;DR: 生成式AI能低成本创作长篇小说，且在没有披露的情况下占据亚马逊自出版小说市场越来越大的销售份额，主要通过数量而非质量重塑市场。


<details>
  <summary>Details</summary>
Motivation: 为了检验生成式AI是否真的在市场上无关紧要，以及这些所谓的“低质量作品”是否会因消费者忽视而缺乏商业影响力。

Method: 对2023-2026年间亚马逊上14,419本未披露AI内容的自出版类型小说进行全文AI检测，并匹配到2026年6月的日销售记录。

Result: AI含量超过25%的书籍在目录中占比较大，但销售份额较小；其销售份额随时间增长，并占据更多稀缺的高排名位置；添加书籍的数量增速（19.2倍）远超收入增速（8.9倍），导致每本书收入下降；无AI文本的书籍在AI扩散高的类型和Kindle Unlimited可用性高的环境中损失最严重；高收入AI书籍的语言更独特，且重复度随收入增加。

Conclusion: 生成式AI通过规模而非质量重塑创意市场，这一结果直接影响到版权侵权公平使用辩护中的市场效应问题。
...
### [168] [Notes to Self: Can LLMs Benefit from Experiential Abstractions?](https://arxiv.org/abs/2607.20372)
*Chang Liu,Xinyu Li,Artur Dubrawski*

Main category: cs.CL

TL;DR: 论文提出一种从大语言模型（LLMs）的解题轨迹中自动提取自然语言抽象（如策略、提示），通过检索或强化学习增强其数学与逻辑推理能力，且自我提取抽象效果与教师提取相当，并具有跨数据集与模型迁移性。


<details>
  <summary>Details</summary>
Motivation: 人类通过经验抽象（如策略）逐步提升问题解决效率，但LLMs是否也能类似地从经验抽象中获益尚不清楚。

Method: 从MATH训练集的LLM解题轨迹中，由更强教师或LLM自身提取自然语言抽象并构建可检索库；采用两种使用模式：（1）推理时检索；（2）在训练提示中增强抽象进行强化学习。

Result: 经验抽象能提升LLM在数学与逻辑推理基准上的性能；自我提取抽象与教师提取抽象效果相当；该框架可迁移至其他数据集与模型。

Conclusion: LLMs能够像人类利用蒸馏经验一样，自动提取并应用经验抽象来改进推理能力。
...
### [169] [LKValues: Aligning Large Language Models with Sri Lankan Societal Values](https://arxiv.org/abs/2607.20410)
...
TL;DR: LKValues: 首个面向斯里兰卡价值观对齐的资源套件，包含大规模数据集和评估基准，能改善低资源语言和文化对齐。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的价值对齐存在西方文化偏见，忽视斯里兰卡等多元语言社会的本土价值观，且缺乏斯里兰卡语境下的评估与微调资源。

Method: 通过三轮205人调查结合全球框架和LLM生成本地概念，提取40项多数认同的价值观；构建Sinhala-英语新闻指令语料库（150k实例）和评估基准（1k实例）；微调三个开源模型。

Result: 新款大模型仍存在低资源文化与价值对齐差距；LKValues微调提升Qwen系列模型在英语和Sinhala上的表现，减少无效输出和跨语言差异，但效果依赖模型家族。

Conclusion: LKValues有效嵌入斯里兰卡价值观，为低资源国家价值观对齐提供了可复制的多元化流水线。
...
TL;DR: 提出模块化可穿戴背包系统ModPack，支持多种机器人形态和任务的统一遥操作。


<details>
  <summary>Details</summary>
Motivation: 现有遥操作系统针对特定机器人和任务定制，可扩展性和适应性不足。

Method: 设计集成计算、电源、通信和存储的背包硬件，支持即插即用功能模块（关节级遥操作、移动操作、主动感知）。

Result: 在两个机器人平台和真实移动操作任务中验证了系统的灵活性和可重用性。

Conclusion: ModPack提供可扩展的遥操作框架，开源软硬件以促进后续研究。
...
TL;DR: 本文研究了在冬季驾驶条件下，漂移对于行车安全的最优性。


<details>
  <summary>Details</summary>
Motivation: 现实中的碰撞避免是研究高侧滑漂移动力学和控制的核心动机，但此前其实际效益主要在专门设计需漂移的场景中测试。

Method: 提出一种具备漂移能力的非线性模型预测控制(MPC)系统，基于碰撞事故数据，在高速模拟器中部署，测试道路偏离和对向车辆碰撞避免场景，并与电子稳定控制(ESC)系统对比。

Result: 漂移控制器能自然启动并维持漂移，以在冰雪路面保持道路和对向车辆避让，在随机冰面蒙特卡洛研究中，漂移控制器在多个速度下实现更低的中间车道误差，且漂移主要在高速度下出现。
...
TL;DR: 提出了第一个开源、低成本（约2000美元）的机器人导盲犬平台Milo，具备完全自主导航能力，可替代传统导盲犬。


<details>
  <summary>Details</summary>
Motivation: 传统导盲犬成本高、等待时间长、寿命短，现有机器人导盲犬方案缺乏自主性，依赖环境先验知识或外部计算。

Method: 基于Unitree Go2机器人，集成机载计算、传感器和手柄；感知栈结合体素地图与地板、障碍物、行人检测；导航栈基于在自定义鸟瞰模拟器中训练的避障策略。

Result: 在真实室内外障碍赛道测试中，与代价地图基线相比，导航更平滑，与使用者碰撞更少。

Conclusion: Milo实现了低成本、完全自主、开源化的机器人导盲犬平台，有效满足BLV用户的协作导航需求。
...
TL;DR: 提出LfH框架，通过可微CBF优化层从稀疏演示中学习用户偏好的安全干预，实现个性化触觉引导。


<details>
  <summary>Details</summary>
Motivation: 现有触觉引导系统使用预定义干预策略，无法适应不同用户的安全偏好。

Method: 基于可微控制障碍函数（CBF）优化层，从稀疏演示中学习安全参数。

Result: 仿真与硬件实验表明，框架能从稀疏输入学习个性化安全干预，减少触觉反馈与偏好的不匹配。

Conclusion: 该框架可学习用户偏好的安全干预，提升触觉共享控制的个性化与直观性。
...
TL;DR: 提出了一种LLM引导的环境简化方法(LENS)，自动为机器人操作生成任务相关的简化场景表示，提升了现有方法在复杂杂乱环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中多物体杂乱环境对机器人操作构成挑战，现有场景抽象方法需要大量人工设计且难以调整，缺乏可扩展性。

Method: 基于大语言模型(LLM)的闭环场景简化方法：通过合并或修剪场景实体，自动生成任务相关、动态更新的抽象场景表示。

Result: 在多种杂乱操作场景中，LENS提升了经典规划、模型控制以及视觉-语言-动作模型的性能。

Conclusion: LENS作为一种即插即用的解决方案，能有效改善机器人在复杂杂乱环境中的操作能力，无需人工工程干预。
...
TL;DR: 本文提出了一种去中心化的抗欺骗轨迹规划框架，用于在远程识别（RID）位置欺骗攻击下运行的小型无人机系统，通过物理层观测检测欺骗并实现风险约束的碰撞避免。


<details>
  <summary>Details</summary>
Motivation: 现有规划器通常假设RID广播可信，在欺骗攻击下会增加碰撞风险，因此需要一种不依赖RID可信度的方法。

Method: 利用接收信号强度测量检测欺骗并概率定位欺骗代理，将不确定性转化为机会约束下的风险有界不安全区域，集成到基于马尔可夫决策过程的规划器中。

Result: 在多机包裹递送仿真中，相比假设RID数据真实的规划器，减少了近空中碰撞事件，同时保持了计算效率。

Conclusion: 该框架能有效应对RID欺骗，实现实时去中心化碰撞避免，并保持任务目标和可扩展性。
...
TL;DR: 提出NavVerse基准，用于室内到室外具身导航，含100室内、50室外、50混合场景及1万条任务，评估零样本RL、VLA和模块化方法发现跨环境导航未解决。


<details>
  <summary>Details</summary>
Motivation: 现有基准独立评估室内/室外导航且抽象化机器人执行，未充分探索出口寻找、边界穿行、适应性和动态失败。

Method: 构建NavVerse含多种场景和任务（物体导航、视觉语言导航、地点导航），通过可执行机器人接口用任务成功、路径效率和安全指标评估。

Result: 端到端VLA零样本成功率最高，模块化方法安全性最佳；地点导航从室外到混合场景性能显著下降，适应是主要瓶颈。

Conclusion: 当前智能体远未解决跨环境导航，适应能力是主要挑战。
...
TL;DR: 提出接触持久完全驱动的控制框架，定义残余能力裕度，证明完全驱动等价于任务力矩位于可行力矩多面体内部。


<details>
  <summary>Details</summary>
Motivation: 传统通过矩阵秩条件认证完全驱动无人机不足以评估持续接触交互时的稳定性。

Method: 刚体模型结合形态相关的力矩映射，定义可行力矩集、残余力矩集和残余能力半径。

Result: 接触持久完全驱动等价于任务力矩位于可行力矩多面体内部；六旋翼仿真显示满秩不代表接触可行。

Conclusion: 残余能力半径可作为安全滤波器；倾斜角需适中以保证推力裕度。
...
TL;DR: 提出了一种缆驱动的无肢机器人，通过可重构的身体形态和可编程被动柔顺性，实现多种运动模式，如侧向波动、侧行、滚动和扭转，在复杂环境中表现出鲁棒性和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有柔顺无肢机器人依赖固定身体配置，每种形态仅针对单一运动模式或环境，缺乏多功能性，因此需要开发一种能实现多样化运动同时保持柔顺运动鲁棒性的单一系统。

Method: 采用分布式缆驱动产生行进身体波，结合可编程被动柔顺性实现接触丰富的鲁棒运动；利用滚动关节重新定向弯曲平面，实现快速重构和运动风格平滑过渡，并配备齿轮锁定以在无持续动力下维持形态。

Result: 实验表明，该平台能可靠生成步态，穿越障碍丰富环境，并在不同运动模式间切换，实现了单一系统中的多模式运动。

Conclusion: 通过结合可编程弯曲柔顺性和形态控制，该平台为在复杂环境中导航提供了多功能无肢机器人解决方案，适用于搜索救援、环境监测和检查等领域。
...
### [179] [Defer to Plan: Adaptive Multi-Agent Fusion for End-to-End V2X Driving](https://arxiv.org/abs/2607.19774)
*Nuoran Li,Zhang Zhang,Yueran Zhao,Tianze Wang,Chao Sun*

Main category: cs.RO

TL;DR: 提出端到端协作驾驶系统MotionNet，通过历史时序融合、注意力特征压缩、自回归多智能体解码及MoE架构，在闭环评估中驾驶得分79.72超过基线3.33%。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知仅优化模块级感知能力，未能有效服务于规划与控制任务。

Method: 采用MotionNetwork融合历史时序信息，注意力机制压缩空间特征为紧凑令牌，自回归解码器自适应融合多智能体特征，并引入混合专家架构增强异质特征表示。

Result: 闭环评估驾驶得分79.72，超过CoDriving基线（77.15）3.33%，同时保持通信效率。

Conclusion: 端到端优化规划任务的协作驾驶系统优于模块级感知方法。
...
### [180] [EgoRecovery: Acquiring Failure Recovery Ability Through Human Recovery Demonstration](https://arxiv.org/abs/2607.19745)
...
TL;DR: 本文提出EgoRecovery框架，利用人类第一人称失败恢复数据，通过共训练对齐人-机器人矫正意图，提升机器人从失败中恢复的能力，在真实任务中优于纯机器人恢复和直接迁移方法。


<details>
  <summary>Details</summary>
Motivation: 机器人需要从失败中恢复以在真实环境中可靠运行，但通过遥操作收集恢复数据难以扩展，因为诱导失败状态和纠正操作耗时，且失败模式多样性要求大量数据。

Method: 利用人类第一人称失败恢复数据（通过高效安排任务级失败配置和记录短恢复片段，每小时生成比遥操作多10倍的有效数据），提出EgoRecovery共训练框架：将人类恢复演示对齐到与机器人数据共享的紧凑矫正意图空间（捕获纠正时机和幅度），仅需少量机器人恢复演示连接意图到可执行动作；部署时，学习到的恢复门从机器人观测预测何时需要纠正，仅激活恢复状态下的矫正意图。

Result: 在真实世界恢复任务实验中，EgoRecovery在失败起始的成功率上优于纯机器人恢复、直接与人类恢复数据共训练以及直接意图迁移基线。

Conclusion: 人类第一人称失败恢复数据作为遥操作的可扩展替代方案，通过EgoRecovery框架有效克服人机形态差异，显著提升机器人恢复性能。
...
TL;DR: 提出了一种基于视觉信息的抓取力预测框架，用于机器人操作椰枣，通过视觉特征和物理模型预测安全抓取力，验证了其有效性和安全性。


<details>
  <summary>Details</summary>
Motivation: 解决椰枣抓取中高脱离力与低损伤阈值之间的矛盾，避免传统触觉探索的缓慢和潜在损伤。

Method: 机械特性表征确定安全抓取包络；V2F管线结合计算机视觉分割、主动轮廓细化、几何特征提取与物理信息残差神经网络，增强赫兹接触方程。

Result: 模型在未见过的品种组上达到平均R²≈0.7的验证性能；实验表明预测力可实现稳定操作，残余变形低于1mm且无可见损伤。

Conclusion: 基于视觉的预判性力估计可替代慢速且可能造成损伤的触觉探索，实现更安全的脆弱水果机器人操作。
...
### [182] [SOPD-SocialNav: Selective On-Policy Distillation for Vision-Language Social Navigation](https://arxiv.org/abs/2607.19850)
*Xinyu Zhang,Zishuo Wang,Ling Xiao*

Main category: cs.RO

TL;DR: 提出选择性在线策略蒸馏方法SOPD，将大型教师视觉语言模型的社会导航知识迁移至轻量级学生模型，在资源受限平台上实现高效社交导航。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型难以部署于资源受限机器人，而轻量模型缺乏社交推理能力，需解决知识蒸馏中的效率与效果平衡。

Method: 基于熵的令牌选择机制筛选教师不确定的高信息量令牌，结合温度控制的JS散度损失优化学生分布。

Result: 在SNEI和MUSON基准上，SOPD优于微调、离线/标准在线蒸馏基线；实际部署中生成更社交恰当的导航行为。

Conclusion: SOPD是一种有效的轻量级社交感知视觉语言导航系统构建策略。
...
### [183] [Clinical Pathways as Safety Specifications for Physical AI in Hospital Wards](https://arxiv.org/abs/2607.19827)
*Gabriele Franchini,Giulio Mallardi,Michele De Carolis,Filippo Lanubile*

Main category: cs.RO

TL;DR: 本文将临床路径重新解释为物理AI系统的运行时安全规范，提出了一种集成可穿戴传感器、智能医疗设备和辅助机器人的架构，并通过运行时安全监控器实现实时安全监测。


<details>
  <summary>Details</summary>
Motivation: 解决物理AI系统在真实环境（如医院病房）中的安全性挑战，特别是当脆弱患者、临床人员、医疗设备和辅助机器人共存时。

Method: 提出概念性机器人架构，集成多模态传感器数据，采用时间预测、不确定性推理和基于约束的验证方法，由运行时安全监控器检测三类安全违规事件。

Result: 构建了将临床知识转化为可执行安全约束的框架，桥梁学习感知与运行时安全监控，辅助护理人员工作。

Conclusion: 通过操作化领域特定临床知识，为安全物理AI提供了一种新方法，能够在真实医院病房中识别生理偏差、硬件故障和数据篡改等安全威胁。
...
TL;DR: 提出KineBench，一个不需要逆动力学模型的闭环基准，用于评估具身世界模型的物理一致性。通过级联视觉基础模型提取6D末端执行器姿态，并在物理模拟器中执行，结合SPARC和可操纵性指标进行量化评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架依赖逆动力学模型从生成视频中提取动作，但该模型对分布外数据脆弱，导致世界模型误差与提取器误差难以区分。

Method: 使用级联视觉基础模型从生成视频帧直接提取6D末端执行器姿态，在物理模拟器中执行闭环验证，并引入SPARC和Maruyama可操纵性指数衡量轨迹平滑性和运动学可行性。

Result: 在前沿模型上的评估揭示了具身视频生成中存在任务复杂度有界的非线性缩放规律，为数据扩展策略提供实证指导。

Conclusion: KineBench有效消除了闭环评估中的属性歧义，并揭示了具身世界模型在复杂任务上的扩展局限性。
...
TL;DR: 提出了一种基于模仿学习的跨具身导航框架，通过引入具身几何条件令牌和模块化多阶段设计，有效提升了不同具身设置下的导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨具身导航方法主要依赖强化学习，难以支持可扩展预训练和真实世界适应；模仿学习方法在跨具身场景下仍存在动作歧义问题，亟需解决。

Method: 采用模块化多阶段模仿学习框架：预训练阶段从互联网视频构建跨具身数据集，并引入具身几何作为条件令牌；微调阶段设计解耦架构，通过轨迹增强生成高风险样本，分别训练空间感知和风险感知校正模块。

Result: 实验表明该方法在不同具身设置下均有效提升导航性能，验证了引入具身几何的有效性。

Conclusion: 将具身几何显式融入模仿学习可实现安全且可泛化的跨具身导航，为具身智能提供可扩展的新范式。
...
TL;DR: 提出模块化框架YAHMP，系统研究人形机器人全身运动跟踪中的设计和训练因素，并在Unitree G1上验证效果。


<details>
  <summary>Details</summary>
Motivation: 人形机器人全身运动跟踪需要兼顾多样化参考动作与平衡维持，但现有设计选择的影响难以独立评估。

Method: 开发开源模块化框架YAHMP，定义名义配置并对比不同变量（动作表示、观测历史、驱动配置等），结合TWIST2基线进行消融实验。

Result: 区分出对跟踪效果影响显著的因素与主要改变驱动力或复杂性的因素，并在真实机器人上实现零样本部署。

Conclusion: YAHMP能系统评估设计决策，实现真实世界多样动作跟踪、抗扰动平衡与物理交互。
...
TL;DR: 提出 Diffusion ReRoll 框架，通过选择性地对局部稳定区域重新加噪，实现可修正的长序列预测，在多项机器人任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散序列预测器仅进行单调去噪，缺乏跨时间步的修正能力，导致长序列预测质量受限。

Method: 在去噪过程中识别局部稳定区域并对其进行重新加噪，利用其余未稳定区域提供的上下文进行迭代修正，同时保持局部一致性。

Result: 在 OGBench 导航任务中成功率提升 21%-23%，在 LIBERO-10 动作预测任务中提升 56.5%，在统一视频-动作预测中取得最佳一致性。

Conclusion: 结构化重新加噪是机器人可修正序列生成的有效机制。
...
TL;DR: 提出了基于模型合并的分离参数训练框架，解决社交机器人导航中预测与规划的技能冲突，在不增加计算负担下提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型在紧凑共享编码器中忽略了行为预测与安全规划的目标冲突，导致性能下降。

Method: 通过分布式参数学习分离关键参数区域，保留核心能力后再合并，并采用稀疏合并减少干扰。

Result: 在JRDB和JTA基准测试中表现优越，验证了框架的通用性和有效性。

Conclusion: 该方法可并用于多种合并方法，为资源受限的机器人导航系统提供了高效解决方案。
...
TL;DR: HOST框架使机器人能从单个人类视频中快速学习技能，同时保留已掌握技能，平均29秒内完成，成功率62%。


<details>
  <summary>Details</summary>
Motivation: 现有机器人技能获取方法成本高、速度慢，且会侵蚀已掌握技能，亟需高效且保留已有能力的方案。

Method: 通过自接地预测级联实现：估计任务进度、翻译为机器人未来观察、从预测观察推导动作，并利用共享任务进度流形对齐训练目标。

Result: 零样本基线提升45%，保留已有技能，比微调50次演示的方法快507倍且演示次数少50倍。

Conclusion: HOST实现高效、保留性的单次技能习得，显著优于现有方法。
...
TL;DR: 提出了ReferTrack，一种基于先指认后跟踪范式的具身视觉跟踪方法，通过单目相机实现目标跟踪，在EVT-Bench上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略的链式推理在抽象空间潜变量中难以监督，且与图像空间检测弱对齐。

Method: 先基于索引边界框选择目标，再解码跟踪路径点；引入滑动窗口队列和TVBI令牌保留运动线索；通过Refer-QA数据集协同训练增强目标指认。

Result: 在EVT-Bench的单目标、干扰和模糊跟踪分割上分别达到89.4%、73.3%和74.1%的成功率，接近或超越多摄像头基线。

Conclusion: ReferTrack实现了鲁棒的模拟到真实迁移，在四足和双足机器人上验证有效。
...
TL;DR: 提出了极端-RGMT，一种两阶段持续学习框架，用于实现人形机器人的鲁棒通用控制，在保持日常运动能力的同时，显著提升高动态运动技能的执行。


<details>
  <summary>Details</summary>
Motivation: 现有的人形控制器在通用能力和专业能力之间存在权衡：通用运动跟踪策略难以可靠执行罕见的高动态运动，而专业训练会降低已习得的日常运动能力。

Method: 采用两阶段持续学习框架：首先从多源运动数据学习通用运动跟踪基础策略，然后通过非对称技能获取和能力保持机制约束已掌握运动的策略漂移，并强调困难动态段；结合难度感知采样和优势优先轨迹重采样来处理高动态运动数据稀缺和失效问题。

Result: 在通用全身运动跟踪任务上达到最先进性能，显著提升高动态运动的完成度，能直接执行多种未见过的固定参考和在线惯性动作捕捉输入的高动态运动。

Conclusion: 极端-RGMT推动通用全身运动跟踪控制器向人类专家级别的高动态运动能力迈进。
...
TL;DR: 提出了SeededGrasp框架，通过VLM生成种子点指导抓取模型，实现多形态物体抓取，仿真成功率达72%，实际场景达78%。


<details>
  <summary>Details</summary>
Motivation: 现有VLM抓取方法要么空间感知差，要么需大量端到端训练数据，无法高效支持多形态物体和复杂场景。

Method: 解耦高层语义推理和低层几何执行：VLM预测种子点，轻量级抓取模型以种子点为条件生成抓取姿态。

Result: 仿真成功72%，真实场景78%，优于现有基线；发布首个多形态桌面抓取数据集（250万抓取）。

Conclusion: 数据高效的解耦架构可有效提升复杂场景下多形态抓取性能，且避免昂贵的端到端训练。
...
TL;DR: 提出DINS-IO，一种无需位置标签即可从原始IMU数据学习惯性里程计的自监督方法，通过加速度积分一致性约束实现。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法依赖昂贵的高精度位置标注（如运动捕捉或SLAM），难以大规模获取。

Method: 利用导航坐标系下速度递归的强可微一致性约束构造滑动窗口最小二乘问题，解析求解后使用残差作为自监督损失；设计高频网络输出IMU速率下的体坐标系速度，并采用LoRA低秩适配进行少量标注校准。

Result: 在标准基准上，自监督预训练后仅用少量标注微调即可匹配或超越全监督方法。

Conclusion: 通过自监督约束与少量标注微调，可有效解决无标签下的惯性里程计学习问题，大幅降低标注成本。
...
TL;DR: 提出了一种礼貌预见性规划方法，通过联合最小化即时成本和所有机器人的预期未来成本，减少长时间任务序列中的累积成本。


<details>
  <summary>Details</summary>
Motivation: 标准任务规划器孤立解决每个任务，忽略未来任务和其他机器人约束，导致侧面效应加剧长期成本。

Method: 基于模型的规划器生成候选方案，使用独立训练的每机器人学习器估计预期未来成本，选择联合最小化即时与未来成本的方案；采用分解公式避免组合式联合推演。

Result: 在家庭环境（两机器人）中总成本降低10.43% vs 短视规划、4.03% vs 自私规划；在餐厅环境（三机器人）中分别降低17.41%和13.24%。

Conclusion: 礼貌预见性规划能有效减少共享环境中长期任务序列的累积成本，支持模块化部署，且适用于不同能力与约束的机器人协作场景。
...
TL;DR: 提出DEED系统方法，通过数据高效的后训练和经验驱动学习，在超市上架任务中弥合VLA人形机器人的实验室与现实差距。


<details>
  <summary>Details</summary>
Motivation: 缩小基准性能与可靠现实操作之间的差距，解决机器人执行错误、分布偏移和环境变化等挑战。

Method: 包括数据高效后训练流程（控制频率对齐、数据筛选、视觉高亮、降低VLA依赖）、基于文本优势前缀和视觉语言价值函数的经验驱动细化、以及潜在空间分析工具。

Result: 通过精心数据设计和后训练，使用单GPU即可将失败策略转化为可靠现实系统，表明关键在系统集成而非架构设计。

Conclusion: 弥合实验室到真实场景的差距主要是系统集成挑战，而非架构问题。
...
TL;DR: 提出了一种结合分布式模型预测控制与安全屏障函数的混合框架，用于水下自重构机器人的多智能体形状形成与安全协调。


<details>
  <summary>Details</summary>
Motivation: 现有自重构方法在保证多智能体安全交互的同时实现期望形状的可靠重组具有挑战性，需要同时解决协调轨迹优化和实时安全约束。

Method: 通过交替方向乘子法求解分布式MPC，计算协调轨迹；再应用分布式CBF滤波器实时强制执行智能体间避碰，结合预测控制与非凸安全保证。

Result: 仿真（最多25个智能体）和物理实验（4个机器人）验证了方法的有效性和可扩展性，展示了形状形成和避碰的成功协调。

Conclusion: 该混合框架利用MPC的预测能力避免局部极小，同时CBF提供形式化安全保证，适用于水下自重构机器人的多智能体装配任务。
...
TL;DR: 提出基于软体藤蔓机器人的分布式声学感知系统，用于灾后受限环境中的受害者定位，结合动态SRP-PHAT框架实现远场/近场声源定位。


<details>
  <summary>Details</summary>
Motivation: 传统刚性机器人难以在非结构化环境中部署，而软体机器人感知能力有限，需探索新型外感知方法以提升灾难救援中的受害者定位能力。

Method: 采用动态转向响应功率-相位变换（SRP-PHAT）框架进行方向估计与三维定位；实验设计包括不同麦克风安装位置（内体/内尾/外壁）、机器人构型（线性/双线性/圆形/正弦）及信噪比/方位/距离参数变化。

Result: 外壁安装与近场条件下（仅3个麦克风伸出）仍能保持高定位精度，该性能在多种构型及信噪比下具有鲁棒性。

Conclusion: 软体生长机器人的分布式声学感知系统可有效实现受限空间受害者定位，为软体机器人外感知应用提供新途径。
...
TL;DR: 该论文为小型人形机器人开发了一套完整的全身遥操作控制栈，实现了行走和远程操作，并在ROBOTIS OP3硬件上进行了验证。


<details>
  <summary>Details</summary>
Motivation: 全尺寸人形机器人控制方法先进但昂贵且不易获取；小型人形机器人更常见但缺乏类似的双足平衡和遥操作能力。

Method: 使用虚拟现实进行上半身遥操作，强化学习控制下半身平衡和行走，并设计了一个完整的控制框架。

Result: 在OP3硬件上实现行走速度0.45 m/s，遥操作实验平均10分钟内移动2个40g方块，总行走距离5米。

Conclusion: 开发的系统展示了小型人形机器人在遥操作和全身操作方面的潜力。
...
### [199] [Human-in-the-Loop Distributed Control of Grid-Interactive Buildings for Demand Response Participation](https://arxiv.org/abs/2607.19552)
*Kasra Mazarei Saadabadi,Dongming Wang,Wei Ren,Alfredo Martinez-Morales,Hamidreza Nazaripouya*

Main category: eess.SY

TL;DR: 提出一种人机协同的分布式共识控制方法，用于多建筑需求侧管理，通过引入人类作为非自主领导者实现需求响应与热舒适平衡。


<details>
  <summary>Details</summary>
Motivation: 现有需求响应管理方法在平衡建筑能耗与用户舒适度方面存在局限，且缺乏灵活应对不同场景的框架。

Method: 采用基于滑模观测器的非线性未知输入方法与领导者-跟随者多智能体共识协议，实现人类在环控制。

Result: 仿真验证了该方法能在需求响应事件中实现建筑群共识一致，保持系统性能并增强电网适应性。

Conclusion: 所提方法有效协调了建筑能耗与热舒适，提升了需求响应策略的灵活性与可操作性。
...
### [200] [From P&ID Drawings to Process Graphs: A Multimodal Language Model Approach](https://arxiv.org/abs/2607.19568)
*Baikai Zhu,Samuel Duong,Javal Vyas,Mehmet Mercangöz*

Main category: eess.SY

TL;DR: 提出了一种基于多模态大语言模型的两阶段工作流，将P&ID图纸的视觉提取和拓扑推理分离，以更准确、结构一致地数字化表示过程工厂。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖脆弱的符号识别和基于规则的连通性重建，难以处理异构绘图标准，导致P&ID数字化困难。

Method: 使用多模态大语言模型的两阶段工作流：先进行视觉提取（设备标签），再进行拓扑推理（过程拓扑），并融入化学工程过程知识。

Result: 在两个ANSI标准P&ID案例研究上，该方法比端到端数字化在准确性和结构一致性上表现更好。

Conclusion: 基于语言模型、知识引导的工作流有望实现可扩展且语义可靠的P&ID数字化。
...
### [201] [Quantum-Resilient Distributed Optimization for Multi-Region Unit Commitment](https://arxiv.org/abs/2607.19581)
*Junhong Liu,Qinfei Long,Alex Pengfei Zhao,Xianping Zhong,Yunfeng Li,Xiaohai Dai,Francis Yunhe Hou*

Main category: eess.SY

TL;DR: 提出了一种后量子安全的分布式优化框架，用于多区域机组组合与备用共享，通过自定义Benders分解和量子弹性安全聚合协议，在保护隐私的同时实现高精度和轻量计算。


<details>
  <summary>Details</summary>
Motivation: 电力系统区域间优化需共享敏感信息，面临量子计算对现有加密的攻击威胁，需要后量子安全方法。

Method: 基于Benders分解的全局求和结构，结合加性掩码、仿射变换和基于格的可验证零知识证明的多层安全协议。

Result: 平均次优性0.09%-0.22%，恢复系统成本高达51%，无质量折损；对比算法在隐私预算收紧时性能劣化。

Conclusion: 所提方法实现了隐私、效率与成本最优的平衡，适用于高密度系统，避免了传统方法的组合可行问题。
...
### [202] [Do Co-Located AI Training Jobs Synchronize? Load-Dependent Throttling as a Coupling Mechanism for Phase-Locking Behind a Shared Power Cap](https://arxiv.org/abs/2607.19638)
*Brieuc Le Roux Tardif*

Main category: eess.SY

TL;DR: 大规模AI训练设施的周期性电力负载可能导致同步功率波动，其机制可通过广义Kuramoto模型描述，并提出了检测与缓解方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究将AI训练设施视为外生周期性负载，但忽略了多个独立训练任务共享功率上限时可能发生的同步效应，可能导致更严重的电网波动。

Method: 将训练任务建模为广义Kuramoto振子系统，识别通过负载相关限速（如功率上限、电压下降、共享冷却）产生的耦合机制，并分析耦合的排斥/吸引特性。

Result: 耦合在多数情况下是排斥的，仅在控制环相位滞后超过半周期时变为吸引；同步相变为一阶且具有滞后性；相位分散调度可提高所有同步模式的阈值。

Conclusion: 功率管理可能引发意外的同步现象，通过相位分散调度可有效缓解，且该预测可通过双任务联合限速实验验证。
...
### [203] [Human-on-the-loop Resilient Control of InverterBased Resources Under Actuator Degradation](https://arxiv.org/abs/2607.19665)
*Majid Dehghani,Taha Saeed Khan,Hamidreza Nazaripouya*

Main category: eess.SY

TL;DR: 提出了一种人在环的弹性控制架构，用于应对电网支撑逆变器执行器退化问题，通过引入人类监督来优化控制决策。


<details>
  <summary>Details</summary>
Motivation: 传统容错控制和自适应控制方法在处理执行器渐进或模糊退化时存在局限性，如易误诊、过度保守或参数漂移。

Method: 架构结合人类判断，利用生成储备容量和受控性能退化两个新指标，并通过弹性参数μ调节精度与备用的权衡。

Result: 仿真显示该方法在电压调节、控制储备和防止执行器饱和方面优于传统方法。

Conclusion: 该架构能在执行器退化下保持系统可操作性，实现电网稳定性。
...
### [204] [A Human-AI Teaming Framework for Deep Reinforcement Learning-Based Voltage Regulation in Distribution Networks](https://arxiv.org/abs/2607.19667)
*Mahmuda Akter,Hamidreza Nazaripouya*

Main category: eess.SY

TL;DR: 提出人机交互强化学习框架(HI-LSAC)，结合SAC算法与自适应拉格朗日约束，通过人类指导模块实现配电网电压安全调节。


<details>
  <summary>Details</summary>
Motivation: 高渗透率分布式能源导致配电网电压调节难度增加，传统深度强化学习方法存在安全探索不足、收敛慢、可靠性差的问题。

Method: 采用SAC智能体与自适应拉格朗日约束机制，引入人类指导模块提供基于灵敏度的修正（协调电容器和电池储能系统调度），并通过人类正则化演员损失函数让策略内化安全控制行为。

Result: 在IEEE 13节点馈线系统中，HI-LSAC相比基线方法显著降低了电压越限严重程度和网络功率损耗。

Conclusion: 人机交互框架能有效提升电压调节的安全性与鲁棒性，为高DER渗透率配电网提供可靠控制方案。
...
### [205] [Quantification and Surrogate Model Estimation of Spatial-Temporal Carbon Intensity Factors](https://arxiv.org/abs/2607.19851)
*Daniel R. Bayer,Jonas Schiller,Thanh Mai Pham,Marco Pruckner*

Main category: eess.SY

TL;DR: 提出了一种计算城市区域时空碳强度因子的方法，并开发了基于决策树和神经网络的替代模型，实现高精度估计。


<details>
  <summary>Details</summary>
Motivation: 光伏等分布式能源的普及导致城市碳排放强度因地因时波动，但现有方法缺乏高分辨率数据，限制了智能控制系统的减排效果。

Method: 计算城市区域时空碳强度因子，并构建替代模型（含决策树和神经网络），仅需常用参数和网格数据。

Result: 同一城市内碳强度因子范围0-316 g/kWh，替代模型在密集城区精度高，忽略局部差异导致排放误差达9%。

Conclusion: 所提模型可支持可持续城市运营，并具有向数据有限区域迁移的潜力。
...
### [206] [Integrating Deep Learning and Contraction Theory for Robust Nonlinear State Estimation via Unsupervised Scientific Machine Learning](https://arxiv.org/abs/2607.19926)
*Yasmine Marani,Israel Filho,Eric Feron,Taous-Meriem Laleg-Kirati*

Main category: eess.SY

TL;DR: 提出一种基于学习的非线性观测器设计方法，通过嵌入收缩条件的科学机器学习框架同时求解修正项和收缩度量，并将其扩展到非自治系统，同时建立学习误差和鲁棒性理论保证。


<details>
  <summary>Details</summary>
Motivation: 传统非线性观测器设计中矩阵偏微分不等式（MPDI）求解极为困难，现有方法缺乏统一且可计算的解决方案。

Method: 将收缩条件嵌入训练损失函数的科学机器学习方法；对非自治系统采用静态修正项和收缩度量以避免时变泛化问题；通过理论分析建立学习误差界和指数输入-状态稳定性。

Result: 数值仿真验证了不同收缩率和测量噪声水平下的观测器性能；理论证明了学习误差与采样分辨率的关系以及鲁棒性。

Conclusion: 该学习范式有效解决了传统收缩观测器设计中的计算难题，并具备理论可靠性，可推广至实际非线性系统应用。
...
### [207] [Time-Series Anomaly Detection for Mobile Robots in Automotive Active Safety Testing using an RNN-VAE](https://arxiv.org/abs/2607.20079)
*Henrik Meyer,Karsten Raguse,Armando Walter Colombo,Thomas Seel,Simon F. G. Ehlers*

Main category: eess.SY

TL;DR: 本文提出了一种基于重建的时间序列异常检测模型，用于检测超扁平可碾压（UFO）机器人平台中的硬件缺陷，如轮胎不均匀磨损或减震器损坏。


<details>
  <summary>Details</summary>
Motivation: 移动机器人（如UFO平台）在汽车主动安全测试中缺乏自诊断能力，导致故障恶化、维修成本高和停机时间增加。

Method: 采用门控循环单元变分自编码器（GRU-VAE），通过预训练利用未标记数据，优化超参数，并评估了无状态窗口训练和截断反向传播时间（TBPTT）方法。

Result: 成功检测六种缺陷类型（其中四种未在训练数据中出现），在五个月从五个系统实例收集的测试集上达到F1分数0.936。

Conclusion: 该方法具有强泛化能力和实际可行性，有效提升了机器人自诊断能力。
...
### [208] [Group-of-Latents: Perceptual Video Compression at Extreme Bitrates via Masked Latent Generative Modeling](https://arxiv.org/abs/2607.19437)
...
TL;DR: 提出了一种统一生成式框架，利用预训练扩散变换器先验，在极低比特率下实现高感知质量视频压缩。


<details>
  <summary>Details</summary>
Motivation: 现有视频压缩算法在极低比特率下未充分优化感知质量，这是一个未充分探索的前沿。

Method: 采用因果分词器中的灵活组潜在策略，将潜在流分为I-潜在和P-潜在；使用深度压缩模块编码关键I-潜在，以及基于扩散变换器的统一潜在去噪模块从噪声中合成分辨率P-潜在。

Result: 在极低比特率（<0.005 bpp）下实现最先进的感知保真度，具有丰富的空间细节和稳健的时间一致性。

Conclusion: 该方法在极低比特率压缩中表现出色，代码将公开。
...
### [209] [BLUE: Semantics-Preserving Video Compression for Efficient Vision-Language Surveillance Analytics](https://arxiv.org/abs/2607.19515)
*Shubham Baid,Akash James,Sahil Chachra,Nishant Sinha,Kunal Kislay*

Main category: eess.IV

TL;DR: BLUE压缩在保持VLM语义性能的同时，实现了带宽和推理成本的降低。


<details>
  <summary>Details</summary>
Motivation: 现有编解码器（如H.265）在压缩监控视频时可能降低计算机视觉性能，且未减少视觉语言模型（VLM）推理调用次数。

Method: 在VIRAT和CHAD数据集上，通过VLM管道评估原始H.265和BLUE压缩视频的语义质量，采用盲评协议与标注进行对比。

Result: BLUE与原始H.265的VLM评分差异极小（VIRAT差值约-0.01，CHAD分值分别为4.31和4.26），且压缩率与评分变化无相关性（r=0.004）。CHAD中跳过重P帧比例从1.4%升至53.2%，VLM调用减少约53%。

Conclusion: BLUE可作为监控视频的机器中心压缩层，在保持VLM性能的同时降低存储与计算开销。
...
### [210] [Beyond Confidence in AI-Assisted Colonoscopy: A Spatial, Temporal and Quality-Aware Audit Framework for Endoscopic AI Review](https://arxiv.org/abs/2607.19372)
*Roberto Alcaraz Machado,Ana Lucía Rodríguez Blanco*

Main category: eess.IV

TL;DR: 提出 EndoExplain 轻量级审计框架，分离内镜 AI 中常混淆的信号，包括分类置信度、分割、归因、质量与时间摘要。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 辅助结肠镜系统仅输出帧级置信度，无法判断预测的空间合理性、时间持久性或图像降质下的可靠性。

Method: 集成 EfficientNet-B0 分类、U-Net++ EfficientNet-B1 分割、CAM 归因、归因-掩码对齐、帧质量指标与时间事件摘要。

Result: 分类准确率 0.9280，ROC-AUC 0.9969；分割 Dice 0.9318，IoU 0.8826；Eigen-CAM 归因一致性最优；视频事件 F1 0.8081。

Conclusion: EndoExplain 可生成可审计的驾驶舱式审查层，分离多种信号以提高可解释性，但为研究原型，非临床验证设备。
...
### [211] [PRISM-DR: Per-lesion Retinal Inference with Specialist Models for Diabetic Retinopathy](https://arxiv.org/abs/2607.19864)
*Zübeyr Özeren,Tansel Uyar*

Main category: eess.IV

TL;DR: 提出PRISM-DR，一种为每个糖尿病视网膜病变病灶单独训练检测器的流水线，在IDRiD上取得mAP50 0.527和F1 0.529。


<details>
  <summary>Details</summary>
Motivation: 现有多类模型难以兼顾不同病灶在大小、颜色、形态和发病率上的显著差异，导致稀有困难病灶被忽视。

Method: 每个病灶使用独立配置的YOLO单类检测器，包括感兴趣区域裁剪、预处理、四路并行检测、分块、五折集成和基于物理大小与临床优先级的病灶间抑制。

Result: 在IDRiD测试集上mAP50为0.527，F1为0.529，硬渗出物AP50最高（0.561），且无需微调即可迁移至相似成像尺度的数据集。

Conclusion: 尽管绝对结果一般（因训练集小且任务难），但将每个病灶作为独立检测问题是优于单多类模型的实用替代方案。
