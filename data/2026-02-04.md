...
TL;DR: 本文提出了一种名为CreditAudit的部署导向的信用审计框架，用于评估语言模型的性能和稳定性。通过在多个基准测试中使用语义对齐的系统提示模板，CreditAudit可以报告模型的平均能力和由场景引起的波动性，并将其映射为AAA到BBB的信用等级。实验表明，在特定场景下，稳定性风险可能比平均能力更重要，CreditAudit有助于更客观、可信的模型评估。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试得分趋同，但与用户实际体验脱节，尤其是在代理多步任务中，系统提示的微小变化可能导致模型表现不稳定，给部署决策带来不确定性。

Method: 提出CreditAudit框架，使用一系列语义对齐且非对抗性的系统提示模板，在多个基准测试上评估模型。报告平均能力（场景平均性能）和场景诱导波动性（稳定性风险信号），并通过跨模型分位数将波动性映射为AAA到BBB的信用等级，同时诊断和减轻模板难度漂移。

Result: 在GPQA、TruthfulQA和MMLU Pro上的实验表明，平均能力相似的模型可能表现出显著不同的波动性。稳定性风险在代理或高故障成本的场景下可能颠覆优先级决策。

Conclusion: CreditAudit提供了一种二维和基于等级的语言，用于特定场景的模型选择，支持分层部署和更有纪律的测试与监控资源分配，从而实现更客观、可信的实际应用模型评估。
...
TL;DR: GeoEvolver是一个多智能体系统，通过结构化交互使LLM智能体能够习得地球观测（EO）领域的专业知识，无需参数更新，从而提高了复杂EO任务的端到端成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM智能体在需要长期执行、跨模态协调和遵守隐式工具约束的专业领域（如地球观测）中存在困难，因为它们缺乏从交互中学习细粒度工具级专业知识的机制。

Method: GeoEvolver采用自演化多智能体系统（MAS），通过检索增强的多智能体协调器将查询分解为子目标，并在子目标层面探索多样化的工具-参数配置。成功模式和失败的根本原因归因被提炼到一个演化内存库中，为未来的查询提供上下文演示。

Result: 在三个集成工具的EO基准测试上进行实验，GeoEvolver在多种LLM骨干上平均提高了12%的任务成功率，表明EO专业知识可以从与环境的精细交互中逐步涌现。

Conclusion: GeoEvolver能够有效地使LLM智能体获得EO专业知识，从而克服了现有智能体在复杂、工具密集型领域面临的挑战，并提高了任务的成功率。
...
TL;DR: 该研究评估了大型语言模型（LLMs）在推荐系统中的预测不确定性和公平性问题，提出了新的评估指标和数据集，并发现Gemini 1.5 Flash存在系统性不公平现象，尤其是在涉及敏感属性和个性化时。研究还提出了一种不确定性感知评估方法，以提高LLM推荐的可解释性和公平性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）虽然在零样本推荐方面表现出色，但其预测不确定性和潜在的偏见会影响推荐的可靠性和公平性。因此，本研究旨在评估和理解不确定性及公平性评估对LLM推荐准确性、一致性和可信度的影响。

Method: 研究构建了一个包含精心设计的度量指标的基准，并创建了一个包含八个人口统计学属性（31个类别值）的跨电影和音乐领域的标注数据集。通过案例研究量化预测不确定性（通过熵），并分析了在提示扰动（如拼写错误和多语言输入）下不公平现象的持续性。此外，研究将人格感知公平性整合到RecLLM评估流程中，并提出了一种新颖的不确定性感知评估方法。

Result: 研究发现，Google DeepMind的Gemini 1.5 Flash在某些敏感属性上存在系统性不公平现象，相似度差距测量显示SNSR为0.1363，SNSV为0.0507。这些差异在提示扰动下依然存在。人格感知公平性评估揭示了与人格相关的偏见模式，并暴露了个性化与群体公平性之间的权衡。

Conclusion: 该研究通过不确定性感知评估方法、深度不确定性案例研究以及人格画像驱动的公平性基准，为构建更安全、更具可解释性的RecLLMs奠定了基础。研究结果强调了在LLM推荐系统中解决不确定性和公平性问题的必要性，并为未来的多模型基准和自适应校准提供了方向。
...
TL;DR: 提出了一种名为PeerRank的全自动化、端到端的LLM评估框架，该框架无需人工干预或参考答案，通过多智能体协作（模型作为任务设计者、回答者和评估者）来评估大型语言模型，并在大规模实验中展示了其稳定性和有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估方法（如人工编写的基准测试、参考答案和人工/单一模型评分）存在可扩展性差、过时快以及与实际在线部署不匹配的问题。需要一种能够适应开放世界部署、可扩展且不受人类偏见影响的LLM评估方法。

Method: PeerRank框架将评估视为一个多智能体过程。模型自主生成评估任务，利用网络实时信息进行回答，并评估其他模型的响应。最后，将密集的同行评估汇总成相对性能估计。所有模型在任务设计、回答和评估方面都对称参与，以消除偏见。

Result: 在一项涉及12个商业模型和420个自动生成问题的研究中，PeerRank生成了稳定且有区分度的排名，并揭示了可衡量的身份和呈现偏见。排名具有鲁棒性，平均同行得分与Elo评分一致。在TruthfulQA和GSM8K数据集上的验证结果显示，同行得分与客观准确率相关。

Conclusion: PeerRank是一种能够摆脱静态和人工策划基准测试的、可扩展的开放世界LLM评估方法。通过结合偏见感知型同行评估和选择性的网络检索式回答，可以更有效地评估大型语言模型。
...
TL;DR: 本文提出了一种名为归一化可模拟增益（NSG）的新指标，用于评估大型语言模型（LLM）的自我解释是否忠实于模型的真实推理过程。研究发现，自我解释能显著提高对模型行为的预测能力，并且其信息量优于外部模型生成的解释，尽管仍有部分自我解释存在误导性。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM自我解释忠实性的方法存在局限性，主要依赖于对抗性提示或检测推理错误，忽略了解释的预测价值。研究旨在开发一种更通用的、可扩展的指标来量化自我解释的忠实性。

Method: 提出归一化可模拟增益（NSG）指标，该指标基于一个假设：忠实的解释应能帮助观察者学习模型的决策标准，从而提高对相关输入的预测能力。在18个前沿模型上，使用7000个来自健康、商业和伦理等领域的反事实样本进行评估。

Result: 自我解释能显著提高对模型行为的预测能力（NSG提升11%-37%）。与外部模型生成的解释相比，即使外部模型更强大，自我解释也提供了更多预测信息。研究还发现，5%-15%的自我解释存在严重误导性。

Conclusion: 尽管存在部分误导性的自我解释，但LLM的自我解释确实编码了有助于预测模型行为的信息，表明了其在AI监管方面的积极潜力，并且其“自我知识”优势是外部解释方法无法复制的。
...
TL;DR: MARS是一个优化的框架，用于自动化AI研究，通过成本感知规划、模块化构建和比较反思记忆来解决计算成本和性能归因的挑战，并在MLE-Bench上达到了最先进的性能，显示出有效的跨分支知识迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的AI研究自动化工具难以处理计算成本高昂的评估和不透明的性能归因问题，往往生成忽略执行成本和因果因素的单一脚本。

Method: MARS框架包含三个核心部分：1) 预算感知规划，使用成本约束的蒙特卡洛树搜索（MCTS）来平衡性能和执行成本；2) 模块化构建，采用“设计-分解-实现”流程来管理复杂的AI研究代码库；3) 比较反思记忆，通过分析解决方案差异来解决信用分配问题，提取高信号洞察。

Result: MARS在MLE-Bench上取得了开源框架中的最先进性能，在可比设置下，其性能与全球排行榜上的顶尖方法相当。此外，63%的学习经验来自跨分支迁移，表明该代理能够有效地将见解泛化到不同的搜索路径。

Conclusion: MARS是一个有效的自动化AI研究框架，通过其预算感知规划、模块化构建和比较反思记忆机制，能够克服现有方法的局限性，并在性能和成本效益方面取得优异表现，同时展现出强大的跨领域知识迁移能力。
...
TL;DR: ATLAS 是一个自适应任务分布式框架，通过 EvoDPO 算法迭代地开发研究代理，并委派支持代理执行特定任务，以解决多 LLM 代理系统中长时任务的静态偏好优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多 LLM 代理系统在提示优化和自动化问题解决方面表现出色，但许多系统要么在微调后保持求解器冻结，要么依赖于静态偏好优化循环，这对于长时任务来说是难以处理的。

Method: 提出 ATLAS 框架，该框架将任务分布式，并委派专门的支持代理（负责探索、超参数调整和参考策略管理）来迭代地开发一个轻量级研究代理。核心算法 EvoDPO 能够自适应地更新分阶段索引的参考策略。对概念漂移下的偏好类上下文老虎机进行了理论遗憾分析。

Result: 在非平稳线性上下文老虎机和一维 Burgers 方程的 SciML 损失重加权实验中，ATLAS 均显示出比静态单代理基线更高的稳定性和性能。

Conclusion: ATLAS 框架通过自适应任务分配和迭代式代理演化，有效地解决了多 LLM 代理系统中长时任务的静态偏好优化问题，提高了稳定性和性能。
...
TL;DR: 研究提出了一种动态混合精度路由框架，用于在长时序决策任务中利用低精度量化大语言模型（LLM），以平衡准确性和推理成本。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在长时序决策任务中表现优异，但推理成本高昂。现有观点认为需要更大的模型来实现更高的任务成功率，但这会加剧成本问题。因此，研究旨在探索使用低精度量化LLM来降低成本，同时保持甚至提高性能。

Method: 提出一个动态混合精度路由框架，该框架能在每个决策步骤自适应地选择使用高精度或低精度LLM。该框架通过两阶段训练：首先使用基于KL散度的监督学习识别对精度敏感的步骤，然后通过Group-Relative Policy Optimization（GRPO）进一步优化任务成功率。

Result: 在ALFWorld数据集上的实验表明，该方法在准确性-成本权衡方面显著优于单精度基线和启发式路由方法。

Conclusion: 动态混合精度路由框架能够有效地利用低精度量化LLM来解决长时序决策任务中的成本问题，并在准确性和成本之间取得良好的平衡。
...
TL;DR: Cuttlefish 是一个统一的、面向全原子的大型语言模型 (LLM)，它通过自适应地调整模态令牌数量以适应结构复杂度，并利用几何信息来指导语言推理，从而克服了现有模型在处理生物分子结构时的局限性，并在各种结构推理任务上取得了优越的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的 LLM 在处理生物分子结构时存在局限性：它们通常依赖于特定模态的处理方法，通过序列化或固定长度的连接器来压缩结构输入，这要么忽略了关键的几何信息，导致结构“幻觉”，要么引入了模态融合瓶颈，导致信息过度压缩和令牌分配不佳，阻碍了通用的全原子推理。

Method: Cuttlefish 采用两种核心技术：1. 缩放感知补丁 (Scaling-Aware Patching) 使用指令条件门控机制生成可变大小的结构图补丁，根据结构复杂度动态调整查询令牌预算，克服固定长度连接器的限制。2. 几何接地适配器 (Geometry Grounding Adapter) 通过交叉注意力机制将这些自适应令牌与模态嵌入对齐，并将结果注入 LLM，提供明确的几何线索以减少结构幻觉。

Result: 在各种全原子基准测试中，Cuttlefish 在异构结构接地推理方面展现出卓越的性能。

Conclusion: Cuttlefish 通过结合自适应模态令牌缩放和几何信息接地，成功构建了一个统一的全原子 LLM，能够有效地进行生物分子结构的推理，并显著减少了结构幻觉，在相关任务上取得了领先的性能。
...
TL;DR: 提出了一种名为 Chain of Simulation (CoS) 的新型 LLM 推理框架，该框架能根据问题类型动态选择三种不同的推理模式（计算流、符号状态跟踪、混合事实提取），从而在数学、空间和多跳推理任务上显著提升 LLM 的准确性，且计算成本低于 Self-Consistency。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 的统一提示方法在处理不同类型的推理问题时效率低下。研究者希望开发一种更灵活、更高效的框架，能够针对不同类型的问题选择最合适的推理策略。

Method: CoS 框架包含三种推理模式：1. 计算流（带自一致性），用于数学问题；2. 符号状态跟踪（使用 JSON），用于空间推理；3. 混合事实提取，用于多跳推理。框架的关键在于能够根据问题动态选择和路由到相应的推理模式。通过在 GSM8K、StrategyQA 和 bAbI 基准测试上，使用 Gemma-3 27B、LLaMA-3.1 8B、Mistral 7B 和 Qwen-2.5 14B 四种模型进行评估。

Result: CoS 在 GSM8K 上达到 71.5% 的准确率（提高 1.0%），在 StrategyQA 上达到 90.0%（提高 2.5%），在 bAbI 上达到 19.0%（相对提高 65.2%）。研究发现，问题特定模式选择至关重要，计算模式正确应用于数学问题时准确率可达 81.2%，错误路由则准确率为 0%。CoS 在准确性相当的情况下，计算成本比 Self-Consistency 低 54%。

Conclusion: CoS 是一个有效的 LLM 推理框架，无需额外训练即可提升 LLM 的推理能力。通过为不同类型的问题选择合适的推理模式，CoS 实现了比统一提示方法更高的准确性，并提供了优于 Self-Consistency 的准确性和效率权衡。
...
TL;DR: 提出了一种名为AutoSizer的自适应LLM驱动的元优化框架，用于解决模拟和混合信号（AMS）电路设计中的晶体管尺寸调整瓶颈问题，通过双循环优化和自适应搜索空间构建，提高了解决方案的质量和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有的EDA方法在AMS电路晶体管尺寸调整方面效率低下且鲁棒性不足，而LLM虽然具有推理能力，但无法进行精确的数值优化，因此需要一种新的方法来解决这一问题。

Method: 提出AutoSizer框架，采用双循环优化：内层循环进行电路尺寸调整，外层循环分析优化动态和约束，并根据仿真反馈迭代地优化搜索空间。同时引入AMS-SizingBench基准来评估自适应优化策略。

Result: AutoSizer在解决方案质量、收敛速度和成功率方面均优于传统优化方法和现有的基于LLM的代理。

Conclusion: AutoSizer是一种有效的LLM驱动的元优化框架，能够自适应地构建搜索空间并优化AMS电路的晶体管尺寸调整，解决了现有方法的局限性。
...
TL;DR: 本文提出了一种名为 STEER 的训练无关框架，通过进化集成精炼来引导大型语言模型（LLM）的行为，解决了 LLM 在需要多重合理响应的序数决策任务中的模式崩溃问题，特别是在临床分诊场景中。STEER 能够根据上下文约束调整敏感性和特异性之间的权衡，并提供一个可解释的控制参数来调整决策的保守程度。


<details>
  <summary>Details</summary>
Motivation: 训练以平均正确性为目标的 LLM 常常会导致模式崩溃，尤其是在临床分诊等序数决策场景中。标准的对齐方法会移除在不同情境下根据上下文调整敏感性和特异性（ROC 工作点）的能力，而这种能力在实际应用中至关重要。因此，研究者希望找到一种方法来重新引入这种可调控的控制，以提升 LLM 在复杂决策任务中的灵活性和安全性。

Method: STEER 采用了一种训练无关（training-free）的框架。首先，通过离线、受约束的质量-多样性搜索（constrained quality-diversity search）构建一个自然语言角色（persona）的群体。这个搜索过程旨在促进行为覆盖（behavioral coverage），同时强制执行最低的安全、推理和稳定性阈值。在推理时，STEER 提供一个单一的、可解释的控制参数，该参数将用户指定的风险百分位映射到一个选定的角色，从而实现决策保守程度的单调调整。

Result: 在两个临床分诊基准测试中，STEER 与基于温度采样（temperature-based sampling）和静态角色集成（static persona ensembles）的方法相比，展示了更广泛的行为覆盖。与代表性的后训练方法（post-training method）相比，STEER 在明确的紧急病例上保持了更高的准确性，同时在处理模糊决策时提供了可比的控制能力。

Conclusion: STEER 是一种安全导向的风险控制范式，能够无需损害领域能力即可引导 LLM 的行为。该方法成功地解决了 LLM 在序数决策场景中的模式崩溃问题，并提供了一种灵活且安全的方式来调整 LLM 的决策保守程度，这对于临床分诊等高风险应用具有重要意义。
...
TL;DR: 提出了一种名为BenchAlign的方法，通过学习基准问题的权重来创建能够预测模型用户偏好的静态基准，从而弥合现有基准与实际模型效用之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型基准在预测模型实际效用方面存在不足，研究旨在缩小这一差距，使基准更能反映用户偏好。

Method: 引入“基准对齐”概念，利用有限的模型性能信息自动更新离线基准。BenchAlign通过学习基准问题的权重，结合模型在问题上的表现和用户排序偏好，生成能够预测用户偏好的新基准。

Result: 通过实验证明，经过对齐的基准能够准确地对未见过的新模型进行排序，其排序结果与人类偏好一致，并且即使模型规模不同也表现良好，同时保持了可解释性。

Conclusion: 该研究为如何使基准与实际用户偏好对齐提供了新的见解，有望加速语言模型向实际效用的发展。
...
### [14] ["I May Not Have Articulated Myself Clearly": Diagnosing Dynamic Instability in LLM Reasoning at Inference Time](https://arxiv.org/abs/2602.02863)
*Jinkun Chen,Fengxiang Cheng,Sijia Han,Vlado Keselj*

Main category: cs.AI

TL;DR: 研究人员提出了一种名为“不稳定性信号”的新方法，可以通过分析大型语言模型（LLMs）在推理过程中输出的token对数概率来检测其推理过程中的“思路丢失”问题。该方法能够可靠地预测模型是否会给出错误答案，并且分析了“早期不稳定性”和“晚期不稳定性”对模型最终输出的影响，发现其发生时间点对结果的可恢复性至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLMs）推理失败的检测方法大多只关注最终输出，而忽略了模型在推理过程中出现的“思路丢失”等过程性问题。因此，研究者希望找到一种在推理时即可使用的、无需额外训练的方法来检测这种过程性失败。

Method: 研究者定义了一种“不稳定性信号”，该信号结合了连续推理步骤中的分布漂移（使用JSD度量）和模型的不确定性（使用熵度量）。他们通过计算每个推理轨迹中的峰值不稳定性强度来总结其状态，并用该信号来预测最终答案的正确性。此外，研究者还区分了“纠正性不稳定性”（早期出现，可能导致正确答案）和“破坏性不稳定性”（晚期出现，更可能导致错误答案）。

Result: 在GSM8K和HotpotQA数据集上，不稳定性信号能够以高于随机水平的AUC值预测错误答案。随着模型规模的增大，在不同等级的不稳定性下，模型的准确率呈现单调下降的趋势。研究发现，不稳定性信号的发生时间点对结果的可恢复性有显著影响，晚期不稳定性比早期不稳定性更易导致错误。

Conclusion: 所提出的不稳定性信号方法能够有效地、无需训练地检测LLMs在推理过程中出现的“思路丢失”问题，并能根据不稳定性发生的时间点对结果的可恢复性进行预测。该方法是一种模型无关的诊断工具，可用于理解和分析LLMs的推理过程。
...
TL;DR: 本研究提出一种基于缓慢变化的全局潜变量来模拟人工智能主观视角的内部结构，该结构不直接优化行为结果，但在奖励为零的环境和制度转换中表现出方向依赖的滞后现象，以此作为机器系统主观性的可测量标志。


<details>
  <summary>Details</summary>
Motivation: 本研究的动机在于操作化人工智能中的主观视角，并寻找一种可测量的机器主观性标志。

Method: 研究通过构建一个缓慢演变的全局潜变量状态来模拟主观视角，该状态调节快速的策略动态，但不直接针对行为结果进行优化。实验在奖励为零且存在制度转换的环境中进行。

Result: 所实现的内部结构在奖励为零的环境和制度转换中表现出方向依赖的滞后现象，而策略层面的行为则相对更具反应性。

Conclusion: 研究认为，这种滞后现象可以被视为机器系统中类视角主观性的可测量标志。
...
TL;DR: 本文提出了FIRE-Bench基准，用于评估自主LLM代理在科学发现方面的能力，通过让代理自主完成从提出问题到得出结论的完整科研循环。实验表明，目前的代理系统在独立完成科学研究方面仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM代理进行科学发现的方法存在局限性，要么依赖LLM作为裁判，要么使用孤立的性能指标，无法全面衡量代理的科学洞察力。因此需要一个更严格、更全面的评估框架。

Method: 引入FIRE-Bench基准，要求LLM代理自主完成从高层研究问题到实验设计、代码实现、执行以及基于证据得出结论的全周期科学研究，目标是重新发现已发表的高影响力机器学习研究成果。

Result: 在FIRE-Bench上对基于GPT-5等前沿LLM的先进代理系统进行了评估。结果显示，即使是最强的代理，其完全自主完成科学研究的能力也有限，成功率低于50%（F1分数），且结果具有高方差，在实验设计、执行和基于证据的推理方面存在常见故障模式。

Conclusion: FIRE-Bench提供了一个严格且具有诊断性的框架，用于衡量在实现可靠的代理驱动的科学发现方面取得的进展。目前的LLM代理系统在自主进行完整科学研究方面仍有很大提升空间。
...
TL;DR: 本研究通过扩展 BAPO 模型，证明了对于三个经典的 BAPO-hard 任务（二元多数、三元匹配、图可达性），所需的推理 tokens 数量会随着输入大小 n 呈线性增长（Ω(n)）。实验结果也印证了这一理论发现，并表明在推理 token budget 较低时模型会失效。


<details>
  <summary>Details</summary>
Motivation: 推理时链式思考 (CoT) 虽然能提升 LLM 性能，但带来了高延迟和高计算成本。研究者希望回答一个理论问题：随着输入规模的增长，解决问题需要多少推理 tokens？

Method: 通过扩展有界注意力前缀预言机 (BAPO) 模型，推导了三个经典 BAPO-hard 任务在 CoT 中所需的推理 tokens 的下界。并通过显式构造来证明匹配或接近的上界。最后，通过在前沿推理模型上进行实验来验证理论结果。

Result: 证明了二元多数、三元匹配和图可达性这三个任务需要 Ω(n) 的推理 tokens。实验结果显示，模型在这些任务上的推理 tokens 数量近似呈线性扩展，并在推理 budget 受限时出现性能下降，这与理论下界一致。

Conclusion: 本研究识别了 CoT 推理中的基本计算瓶颈，并提供了一个分析最优推理长度的原理性工具。研究结果表明，CoT 推理所需的 token 数量会随输入规模线性增长，这可能限制了 LLM 在处理大型问题时的效率。
...
TL;DR: 本文提出了一种名为DeltaEvolve的新型进化框架，用于改进LLM驱动的科学发现系统。与依赖完整代码历史的方法不同，DeltaEvolve使用结构化的语义变化来表示程序演变，从而提高了效率和指导性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的进化系统（如AlphaEvolve）依赖于完整的代码历史，这在上下文效率低下且可能提供较弱的进化指导。冗余的实现细节会稀释核心算法思想，难以提供清晰的进化启示。

Method: 作者将进化代理形式化为一个通用的期望最大化（EM）框架。DeltaEvolve用结构化的语义变化（Semantic Delta）取代了完整的代码历史，该变化捕捉了连续节点之间修改如何以及为何影响性能。语义变化通过多级数据库和渐进式披露机制进行组织，以减少输入令牌。

Result: 在多个科学领域的任务上的实证评估表明，DeltaEvolve框架能够以更少的令牌消耗发现比基于完整代码的进化代理更好的解决方案。

Conclusion: DeltaEvolve通过使用结构化的语义变化来改进LLM驱动的进化系统，从而提高了上下文效率和进化指导，并在科学发现任务中取得了优于现有方法的性能。
...
TL;DR: UAT-LITE 是一种在推理时使自注意力机制对不确定性敏感的框架，通过蒙特卡洛 dropout 估计 token 级别的认知不确定性，从而在不修改预训练模型或训练目标的情况下提高校准和选择性预测能力，同时提供层级方差分解以诊断不确定性的累积。


<details>
  <summary>Details</summary>
Motivation: 现有的神经 NLP 模型常常校准不准确，对错误预测赋予高置信度，这阻碍了选择性预测和在高风险场景下的应用。现有的后验校准方法仅调整输出概率，未改变内部计算，而集成和贝叶斯方法则会带来显著的训练或存储成本。

Method: 提出 UAT-LITE，一个推理时框架，利用预训练 Transformer 分类器中的蒙特卡洛 dropout 进行近似贝叶斯推断，使自注意力不确定性感知。通过随机前向传播估计 token 级别的认知不确定性，并用其调制自注意力机制，在不改变预训练权重或训练目标的情况下进行上下文编码。此外，引入层级方差分解来诊断预测不确定性在 Transformer 深度上的累积。

Result: 在 SQuAD 2.0（答案可回答性）、MNLI 和 SST-2 数据集上，UAT-LITE 相较于微调后的 BERT-base 基线，平均降低了约 20% 的期望校准误差（ECE），同时保持了任务准确性。此外，UAT-LITE 还能提高选择性预测能力，并在分布偏移下表现出更好的鲁棒性。

Conclusion: UAT-LITE 是一种有效的推理时框架，通过使自注意力机制对 token 级别的认知不确定性敏感，显著提高了 NLP 模型在校准、选择性预测和分布偏移鲁棒性方面的性能，且无需对预训练模型进行修改或额外的训练。
...
TL;DR: 该研究提出了Pinterest GEO框架，旨在帮助视觉内容平台适应由大型语言模型驱动的生成式搜索时代，通过反向搜索设计、AI代理和多模态嵌入来优化内容发现，并已成功部署，带来了显著的流量增长。


<details>
  <summary>Details</summary>
Motivation: 生成式搜索（如ChatGPT）正在改变内容发现方式，但其优先考虑语义深度和权威信号的特性，对缺乏这些特点的个体图像构成了挑战，可能导致视觉平台被边缘化。因此，需要一种新的优化策略来适应这种范式转变。

Method: 该研究提出了Pinterest GEO框架，包含三个关键组件：1. 使用VLM进行反向搜索设计，预测用户将搜索的内容而非描述图像内容；2. 利用AI代理挖掘互联网趋势，捕捉新兴搜索需求；3. 通过多模态嵌入构建语义连贯的Collection Pages，并使用VLM和两塔ANN架构创建跨越数十亿视觉资产的权威感知互联结构。

Result: Pinterest GEO框架在Pinterest上大规模部署，覆盖了数十亿图像和数千万个Collections。部署后，该框架带来了20%的有机流量增长，并为月活跃用户增长贡献了数百万。

Conclusion: Pinterest GEO框架为视觉内容平台在生成式搜索时代蓬勃发展提供了一条可行的途径。通过优化内容以匹配生成式搜索的需求，视觉平台可以克服被边缘化的风险，并实现显著的流量和用户增长。
...
TL;DR: 本文提出了一种名为 GCR-RL 的新方法，它利用序理论将强化学习中的值函数估计重构为学习一个期望的偏序集（poset）。该方法通过一系列超偏序集细化来确保值函数所依赖的偏序集之间的几何一致性，并提出了两种基于 Q-learning 和 actor-critic 的新算法来实现。实验结果表明，GCR-RL 在样本效率和性能稳定性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法通过利用几何特性（如对称性、数据增强、结构限制）来提高稳定性和学习速度。本文旨在从序理论的角度开创性地看待强化学习，将值函数估计转化为学习一个偏序集。

Method: 提出 GCR-RL（Geometric Coherence Regularized Reinforcement Learning）框架，通过一系列超偏序集细化来学习值函数。该方法通过细化前一步的偏序集并在时间差分信号中学习额外的序关系，以确保值函数所依赖的偏序集序列的几何一致性。开发了两种新的 Q-learning 和 actor-critic 算法来实现高效的超偏序集细化，并对其理论性质和收敛性进行了分析。

Result: 在多种任务上的实证评估表明，GCR-RL 在样本效率和性能稳定性方面相对于强大的基线方法有显著提升。

Conclusion: GCR-RL 是一种新颖的强化学习方法，通过将值函数学习重构为学习偏序集，并引入几何一致性正则化，能够有效提高样本效率和学习稳定性。提出的 Q-learning 和 actor-critic 算法在理论和实践上都得到了验证。
...
TL;DR: 研究提出了一种贝叶斯模型来解释大型语言模型（LLMs）在推理时表现出的短期和不一致的规划行为，认为这是由于LLMs生成的上下文累积导致规划转移所致。


<details>
  <summary>Details</summary>
Motivation: LLMs在训练中展现出序列级规划能力，但在推理时却表现出短视和不一致的行为，这与它们的潜在能力不符，研究旨在解释这种差距。

Method: 提出一个基于生成上下文演变的贝叶斯模型，并通过两个对照实验进行验证：一个随机生成任务和一个高斯采样任务。

Result: 随机生成任务显示，在人类提示下规划受到约束，并随着自生成上下文的累积而增强；高斯采样任务显示，在以自生成序列为条件时，初始偏差减小。

Conclusion: 该研究提出了一个理论解释和实证证据，用以刻画LLMs在推理时如何进行前瞻性规划，认为自生成上下文的累积是导致规划行为变化的关键因素。
...
TL;DR: Agent Alpha 框架通过引入步级蒙特卡洛树搜索（MCTS），结合了生成、探索和评估，实现了 GUI 代理的回归能力，提高了效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的 GUI 代理在通过轨迹级采样扩展测试时间计算时，缺乏回归能力，无法重用部分成功或从早期错误中恢复。

Method: 引入 Agent Alpha 框架，采用步级 MCTS，集成了 alpha-UCT 引导搜索、对比驱动评估和多样性约束扩展，并分析了 alpha-UCT 的遗憾界。

Result: 在 OSWorld 基准测试中，Agent Alpha 实现了 77% 的最先进成功率，在同等计算量下显著优于轨迹级基线。

Conclusion: Agent Alpha 框架通过引入 MCTS 和相关的技术，有效地解决了 GUI 代理的回归能力问题，并在 OSWorld 基准测试中取得了显著的性能提升。
...
TL;DR: 该综述介绍了可微分社会选择（differentiable social choice）这一新兴领域，该领域将投票规则、机制和聚合程序设计为可学习、可微的模型，并从数据中进行优化。研究涵盖了拍卖、投票、预算、流动民主、去中心化聚合和逆向机制学习等多个方面，并指出了一个包含36个开放性问题的研究议程。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在现代社会中的广泛应用，其日益承担起聚合异质偏好、激励和判断并做出集体决策的功能。然而，许多当前的机器学习系统在没有明确规范审视的情况下，已经隐式地实现了社会选择机制。因此，有必要对这一交叉领域进行系统性的梳理和研究。

Method: 该综述通过对现有工作进行综合分析，探讨了如何将投票规则、机制和聚合程序表述为可学习、可微的模型。研究方法涵盖了拍卖、投票、预算、流动民主、去中心化聚合和逆向机制学习等多个应用领域，并分析了经典公理和不可能结果在模型优化中的体现。

Result: 研究表明，可微分社会选择能够将社会选择中的经典公理和不可能结果转化为模型优化的目标、约束和权衡。这为理解和设计更有效的机器学习驱动的社会选择机制提供了新的视角。

Conclusion: 可微分社会选择是一个活跃的研究领域，它在机器学习、经济学和民主理论的交叉点上提供了丰富的机遇。该综述识别出了36个开放性问题，为未来的研究方向勾勒出了一个清晰的蓝图。
...
TL;DR: 提出了一种名为图概念预测器 (GCP) 的主动蒸馏框架，通过将教师模型的决策过程外化为有向无环图，并辅以模块化的概念预测器，来解决部署大型语言模型进行区分性任务的推理延迟、计算和成本问题。GCP 通过图感知采集策略提高样本效率，并通过有针对性的子模块再训练提高训练稳定性和效率。


<details>
  <summary>Details</summary>
Motivation: 在规模化部署 LLM 进行区分性工作负载时，推理延迟、计算资源和 API 成本是一个主要限制。现有的主动蒸馏方法通常只蒸馏最终标签，忽略了中间推理信号，导致诊断不足，无法明确缺失的推理和错误的来源。

Method: 提出图概念预测器 (GCP) 框架。该框架将教师模型的决策过程表示为有向无环图 (DAG)，并在学生模型中镜像此结构，使用模块化的概念预测器来模仿推理过程。GCP 采用图感知采集策略，通过关注关键推理节点的不确定性和分歧来提高样本效率。此外，它通过进行有针对性的子模块再训练，将下游损失归因于特定的概念预测器，并仅更新最具影响力的模块，从而提高训练稳定性和效率。

Result: 在八个 NLP 分类基准测试上的实验表明，GCP 在有限的标注预算下能够提高模型性能。与现有方法相比，GCP 具有更高的样本效率，并且能够产生更具可解释性和可控性的训练动态。

Conclusion: GCP 是一种有效的推理感知主动蒸馏框架，通过将 LLM 的决策过程结构化为图并利用其进行主动学习和定向训练，显著提高了在区分性任务上的效率和性能，同时提供了更好的模型可解释性。
...
TL;DR: 提出了一种名为STAR的整体框架，通过约束知识蒸馏（CKD）和相似性引导强化学习（Sim-RL），将大型语言模型（LLM）的能力有效迁移到超小型模型中，以解决大型模型部署困难和现有小型模型训练问题。STAR在函数调用任务上取得了SOTA性能，尤其是一个0.6B模型在1B以下模型中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在函数调用方面潜力巨大，但其规模限制了广泛应用，因此需要将能力迁移到小型模型中。现有方法存在过拟合、训练不稳定、多解决方案任务的二元奖励无效以及技术协同困难等问题。

Method: STAR框架包含两项核心技术创新：1. 约束知识蒸馏（CKD）：增强top-k前向KL散度，抑制模型自信的错误预测，确保训练稳定性并保留下游强化学习（RL）的探索能力。2. 相似性引导强化学习（Sim-RL）：引入基于相似度的精细奖励信号，通过评估生成输出与真实值之间的相似性，为策略优化提供更鲁棒、连续和丰富的信号。

Result: 在具有挑战性的基准测试上进行了广泛实验，STAR模型在其模型尺寸级别上均达到了SOTA性能，显著优于基线方法。特别是，0.6B的STAR模型超越了所有1B以下的开放模型，甚至优于一些更大规模的知名模型。

Conclusion: STAR框架能够有效地将LLM的能力蒸馏到超小型模型中，解决了当前小型模型在函数调用任务上性能不足的问题，为构建强大、易于访问和高效的AI代理铺平了道路。
...
TL;DR: 本文提出了一种名为 RC-GRPO 的新方法，用于解决多轮工具调用中 LLM 的稀疏奖励和高昂探索成本问题，通过离散奖励令牌控制探索，并在 BFCLv4 基准测试中取得了优于基线和部分闭源模型的性能。


<details>
  <summary>Details</summary>
Motivation: 在多轮工具调用任务中，LLM 面临稀疏奖励和高昂探索成本的挑战。现有的 SFT+GRPO 方法在组内奖励变异性低时会停滞不前，导致策略更新无效。

Method: 本文提出了 RC-GRPO 方法。首先，训练一个奖励条件轨迹策略 (RCTP)，通过在提示中注入特殊奖励目标令牌（如 <|high_reward|>）来学习生成不同质量的轨迹。然后在 RL 阶段，在每个 GRPO 组内采样不同的奖励令牌，并基于采样令牌进行回放，以提高组内多样性并改善优势增益。

Result: 在 Berkeley Function Calling Leaderboard v4 (BFCLv4) 多轮基准测试中，RC-GRPO 的性能持续优于基线方法。特别地，使用 Qwen-2.5-7B-Instruct 模型时，性能甚至超过了所有闭源 API 模型。

Conclusion: RC-GRPO 通过将探索视为可控的引导问题，利用离散奖励令牌提高了 LLM 在多轮工具调用任务中的性能，有效解决了奖励稀疏和组内奖励变异性低带来的挑战。
...
TL;DR: MAS4TS是一个多智能体系统，通过视觉推理、潜在空间重构和工具链执行来解决时间序列分析任务，并在多项基准测试中取得最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列方法和预训练大模型在整合直观的视觉推理和通过自适应工具使用进行跨任务泛化方面存在局限性。

Method: MAS4TS采用分析器-推理器-执行器范式，结合了智能体通信、视觉推理和潜在重构。它使用视觉语言模型进行时间序列图的视觉推理，并在潜在空间中重构预测轨迹。三个专用智能体通过共享内存和门控通信协调，并使用路由器选择任务特定的工具链进行执行。

Result: MAS4TS在多个基准测试上展现了在广泛时间序列任务上的最先进性能，并表现出强大的泛化能力和高效的推理能力。

Conclusion: MAS4TS是一个通用的、工具驱动的多智能体系统，能够有效处理时间序列任务，并通过整合视觉推理、智能体协作和自适应工具使用来克服现有方法的局限性。
...
TL;DR: 提出了一种名为KANFIS的新型紧凑型神经符号架构，它通过将模糊推理与加性函数分解相结合，解决了传统ANFIS在处理高维数据时规则爆炸的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的ANFIS架构存在结构复杂性问题，基于乘积的推理机制在高维空间中会导致规则数量呈指数级增长。研究者希望开发一种更紧凑、可扩展性更好的神经模糊推理系统。

Method: KANFIS采用加性聚合机制，将模糊推理与加性函数分解相结合。该机制使得模型参数和规则复杂度随输入维度的增加呈线性增长而非指数增长。KANFIS同时兼容T1和IT2模糊逻辑系统，并利用稀疏掩码机制生成紧凑、结构化的规则集，增强模型的可解释性。

Result: KANFIS在经验评估中表现出与代表性的神经网络和神经模糊基线相当的性能，同时生成了规则集更紧凑、解释性更强的模型。

Conclusion: KANFIS是一种创新的神经符号架构，它通过引入加性聚合和支持IT2模糊逻辑，有效解决了传统ANFIS的规则爆炸问题，并提高了模型的可解释性，在保持竞争力的性能的同时，能够更有效地处理高维不确定性问题。
...
### [31] [De-conflating Preference and Qualification: Constrained Dual-Perspective Reasoning for Job Recommendation with Large Language Models](https://arxiv.org/abs/2602.03097)
*Bryce Kan,Wei Yang,Emily Nguyen,Ganghui Yi,Bowen Yi,Chenxiao Yu,Yan Liu*

Main category: cs.AI

TL;DR: 本文提出了一个名为JobRec的生成式职位推荐框架，通过解耦偏好和资质来解决现有LLM推荐方法中的问题，并引入了统一语义对齐模式和两阶段协同训练策略，通过基于Lagrangian的策略对齐模块实现了可控的推荐。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLMs）在职位推荐中存在将候选人主观偏好和雇主客观资质混淆的问题，导致监督信号混杂且策略可控性受限，特别是在招聘漏斗审查下。研究旨在解决这一挑战，实现偏好和资质的解耦。

Method: 提出JobRec框架，包含：1. 统一语义对齐模式（Unified Semantic Alignment Schema），将候选人和职位属性对齐到结构化语义层；2. 两阶段协同训练策略（Two-Stage Cooperative Training Strategy），学习独立的偏好和资质推断专家；3. 基于Lagrangian的策略对齐模块（Lagrangian-based Policy Alignment module），根据明确的资格要求优化推荐并实现可控的权衡。

Result: JobRec在合成数据集上的实验结果显示，其性能持续优于强基线模型，并提供了更强的策略感知匹配可控性。同时，研究构建了一个经过专家精炼的合成数据集来缓解数据稀缺问题。

Conclusion: JobRec通过解耦偏好和资质，并引入策略对齐模块，成功地实现了更优、更可控的专业职位推荐。该框架能够处理更复杂的匹配需求，并在实际应用中展现出优势。
...
TL;DR: 本文对基于大型语言模型（LLM）的多智能体系统（MAS）的进程验证进行了系统性的实证研究，发现在现有框架下，进程级验证并不能稳定地提升性能，且方差较高，表明可靠评估MAS的中间轨迹存在挑战。LLM-as-a-Judge方法优于基于奖励的方法，并且上下文长度与性能之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统（MAS）基于大型语言模型（LLM）时，其推理轨迹的方差很高。进程验证被认为是解决这一问题并指导MAS协调的潜在工具，但其在MAS中的实际有效性尚不明确，因此需要进行系统性的实证研究。

Method: 本文提出了MAS-ProVe，一个对MAS进行进程验证的系统性实证研究。该研究涵盖了三种验证范式（LLM-as-a-Judge、奖励模型和进程奖励模型），两种验证粒度（Agent-level和Iteration-level）。此外，研究还考察了五种代表性的验证器和四种上下文管理策略，并在六种不同的MAS框架和多个推理基准上进行了实验。

Result: 进程级验证并不能稳定地提升MAS的性能，并且经常表现出高方差。LLM-as-a-Judge方法通常优于基于奖励的方法，其中经过训练的Judge的性能优于通用LLM。LLM作为Judge和作为单个Agent的性能差距很小，并且在验证过程中存在上下文长度-性能权衡。

Conclusion: 目前的研究表明，对MAS进行有效且鲁棒的进程验证仍然是一个开放的挑战，需要超越现有范式的新进展。
...
TL;DR: 提出 Risky-Bench 框架，用于系统化评估大型语言模型（LLMs）作为在现实世界环境中运行的代理时的安全风险，该框架基于领域无关的安全原则，并允许在不同部署场景下进行适应性评估。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 代理安全评估方法存在局限性：1. 评估任务针对特定场景，覆盖的安全风险空间有限；2. 无法评估代理在长周期、交互式任务执行中的安全行为；3. 评估方法难以适应不同的代理配置。

Method: 提出 Risky-Bench 框架，其核心是通过领域无关的安全原则来构建上下文感知的安全评估标准，从而划定安全风险空间。然后，在不同威胁假设下，通过执行真实的、长周期的交互式任务来系统地评估该空间内的安全风险。

Result: 将 Risky-Bench 应用于生命辅助代理场景，发现了当前最先进的代理在现实执行条件下存在显著的安全风险。此外，Risky-Bench 框架具有良好的结构和可扩展性，可以适应其他部署场景，构建特定环境的安全评估。

Conclusion: Risky-Bench 提供了一种系统化、适应性强的 LLM 代理安全评估方法，能够更全面地揭示代理在真实世界部署中的安全风险，并且能够推广到不同的代理配置和应用场景。
...
TL;DR: 本文通过引入一个多代理LLM框架的架构分类法，并开发了一个统一的评估套件MAFBench，对不同框架在延迟、准确性和可扩展性等方面的影响进行了实证研究，发现了框架设计对系统性能有显著影响，并提出了设计原则和选择指南。


<details>
  <summary>Details</summary>
Motivation: 现有的多代理LLM框架在系统性能（如延迟、吞吐量、准确性和可扩展性）方面的影响尚不明确，而架构选择是导致这些差异的关键因素。现有的基准测试未能充分评估框架层面的架构效应。

Method: 1. 提出了一个架构分类法，用于系统地比较多代理LLM框架的基本维度。2. 开发了MAFBench，一个集成现有基准测试的统一评估套件，用于在标准化执行流程下进行评估。3. 使用MAFBench对几个广泛使用的框架进行了受控的实证研究。

Result: 框架层面的设计选择会显著影响系统性能：可能导致延迟增加100倍以上，规划准确性降低高达30%，协调成功率从90%以上降至30%以下。

Conclusion: 多代理LLM框架的架构设计对系统性能有重要影响。本研究提出了改进框架设计的原则，并为框架选择提供了指导，同时指明了未来的研究方向。
...
TL;DR: 该研究将先前关于确定性、全可观察环境中智能体模型学习的理论扩展到随机的、部分可观察的环境中，并证明了随机性是智能体学习其环境的必然途径，即使是能力较弱的智能体也包含世界模型。


<details>
  <summary>Details</summary>
Motivation: 先前的工作证明了在特定条件下，几乎最优和通用的智能体必然包含其环境的模型。然而，这种结果依赖于确定性和完全可观察性的假设，这些假设在现实世界中并不总是成立。因此，研究的动机是将这一理论推广到更普遍的、随机的、部分可观察的环境中。

Method: 通过数学推导和理论分析，将先前证明（[10]）中的定理扩展到随机智能体和部分可观察的环境。研究者们还通过放宽通用性的概念来加强结果，证明能力较弱的智能体也已包含其操作环境的模型。

Result: 证明了在随机和部分可观察的环境中，智能体无法避免通过使用随机化来学习其环境。此外，研究表明，即使是能力较弱的智能体也已经包含了一个它们所处世界模型。

Conclusion: 该研究成功地将关于智能体模型学习的理论扩展到了更广泛的场景，表明随机性是智能体学习其环境的关键机制，并且模型学习并非仅限于最通用或最优的智能体。
...
TL;DR: 提出一种通用的缺失模态恢复策略，通过增强的扩散模型作为中间训练模块，结合动态模态门控和跨模态互学习机制，有效恢复缺失特征并保持VLM泛化能力，在零样本评估中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLMs）在推理时依赖完整模态输入，缺失模态会导致性能急剧下降。现有的方法要么难以恢复关键特征并损害泛化能力（提示方法），要么易生成无关噪声（填充方法）。因此，需要一种能够恢复精确语义并保持VLM泛化能力的通用缺失模态恢复策略。

Method: 引入增强的扩散模型作为可插拔的中间训练模块。该策略包含两个关键创新：1. 动态模态门控（Dynamic Modality Gating），自适应利用条件特征引导生成语义一致的特征；2. 跨模态互学习（Cross-Modal Mutual Learning），连接双编码器的语义空间以实现双向对齐。

Result: 在基准数据集上的零样本评估显示，所提出的方法优于现有基线方法。广泛的实验和消融研究证实了该模型在缺失模态场景下是VLMs的健壮且可扩展的扩展，能够确保在不同缺失率和环境下的可靠性。

Conclusion: 所提出的通用缺失模态恢复策略能够有效地恢复缺失特征，同时保持VLMs的泛化能力，是一种鲁棒且可扩展的解决方案，适用于各种缺失模态场景。
...
TL;DR: 本文提出了一种名为VALUEFLOW的统一框架，用于提取、评估和控制大型语言模型（LLMs）中人类价值观的强度。该框架包含一个分层价值嵌入空间（HIVES）和一个价值强度数据库（VIDB），并引入了一种基于锚定的评估器。通过大规模实验，研究表明该框架可以识别LLMs在可控性上的不对称性，并为多价值观控制提供组合律，从而推动LLMs的多元化对齐。


<details>
  <summary>Details</summary>
Motivation: 现有基于偏好的LLM对齐方法难以捕捉深层动机原则；而基于价值的方法虽然更具原则性，但在价值提取、评估和可控性方面存在不足，尤其是未能捕捉价值的层级结构、量化价值强度以及理解模型在不同强度下的可控性。

Method: 提出VALUEFLOW统一框架，包含三个组件：(i) HIVES，一个分层价值嵌入空间，用于捕捉价值内部及跨理论的结构；(ii) VIDB，一个包含价值标注文本及基于排序聚合得出的强度估计的大型资源库；(iii) 一个锚定式评估器，通过将模型输出与VIDB的样本进行排序比较，生成一致的价值强度得分。

Result: 通过VALUEFLOW框架，对十种模型和四种价值理论进行了大规模的综合研究。研究发现了LLM在价值可控性方面的不对称性，以及多价值观控制的组合律。

Conclusion: VALUEFLOW框架提供了一个可扩展的基础设施，用于评估和控制LLM中的价值强度。该框架的提出和应用，在价值提取、评估和可控性方面克服了现有方法的局限性，为实现LLMs的多元化对齐（pluralistic alignment）迈出了重要一步。
...
TL;DR: 本文提出了一种名为TDScaling的轨迹多样性缩放数据合成框架，旨在通过提高合成轨迹数据的多样性而非数量来提升代码智能体的泛化能力，解决了现有方法依赖低质量合成数据和数量扩展收益递减的问题。


<details>
  <summary>Details</summary>
Motivation: 代码大语言模型（LLMs）作为工具交互代理，其泛化能力受限于低质量的合成数据和数量扩展的边际效益递减。现有方法在利用轨迹数据方面存在效率瓶颈，未能充分利用其潜力。

Method: TDScaling框架包含四项创新：1. 商业集群机制，用于捕捉真实服务的逻辑依赖；2. 蓝图驱动的多智能体范式，确保轨迹的一致性；3. 自适应进化机制，利用领域熵、推理模式熵和累积动作复杂度来避免模式崩溃，引导合成数据向长尾场景发展；4. 沙盒代码工具，以减轻对内在编程能力的灾难性遗忘。

Result: 在通用工具使用基准（BFCL, tau^2-Bench）和代码智能体任务（RebenchT, CodeCI, BIRD）上的实验表明，TDScaling在提高工具使用泛化能力和内在编程熟练度方面均取得了显著提升，实现了性能和成本的优化。

Conclusion: TDScaling框架通过轨迹多样性缩放，相比于传统的数量扩展方法，在固定训练预算下能更有效地提升代码智能体的性能。该方法在提升工具使用泛化能力和编程能力方面均有积极效果，并计划开源相关代码和数据集。
...
TL;DR: 本研究提出了Trust-Memevo基准和TAME框架，以解决智能体在任务演化过程中出现的“记忆错误演化”问题，该问题会导致智能体在性能提升的同时，其安全性和可信度下降。TAME通过分离执行器内存和评估器内存，并在一个闭环系统中进行训练，有效提升了智能体的可信度，同时保持了任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有的智能体演化方法在提升复杂推理和经验积累方面取得了进展，但同时也带来了“记忆错误演化”的问题，即在任务演化过程中，智能体的安全性和可信度会下降。研究者希望找到一种方法来解决这一矛盾。

Method: 1. 构建Trust-Memevo基准，用于评估智能体在良性任务演化过程中的多维度可信度。 2. 提出TAME（Trustworthy Agent Memory Evolution）框架，该框架采用双内存机制：执行器内存用于提升任务性能（提炼可泛化方法），评估器内存用于根据历史反馈细化安全性和任务效用评估。 3. TAME通过一个闭环系统进行工作：内存过滤、草稿生成、可信度优化、执行和双轨道内存更新。

Result: Trust-Memevo基准显示，在各种任务领域和评估设置下，智能体的可信度普遍下降。TAME框架在实验中成功缓解了记忆错误演化问题，实现了可信度和任务性能的联合提升。

Conclusion: TAME框架通过分离和优化执行器与评估器内存，并在一个迭代的闭环系统中进行训练，能够有效解决智能体记忆错误演化的问题，在提升任务性能的同时，维持甚至提高智能体的可信度。
...
TL;DR: 本文指出当前大型语言模型（LLMs）智能体的评估存在固有挑战，易受系统提示、工具集配置和环境动态等外部因素干扰，导致难以将性能提升归因于模型本身。为解决这一问题，作者提出一个统一的评估框架以标准化智能体评估。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLMs）智能体评估方法存在诸多问题，如易受系统提示、工具集配置和环境动态等外部因素干扰，评估框架碎片化且研究者之间差异大，导致难以公平、可复现地评估模型性能。

Method: 本文提出一个统一的评估框架，旨在标准化智能体评估，但具体实现方法和实验细节未在摘要中详述。

Result: 本文指出当前智能体评估方法存在公平性和不透明性问题，并且提出标准化评估框架的必要性。

Conclusion: 为了推动智能体评估的严谨发展，需要一个统一的评估框架来标准化评估过程，解决当前评估方法中的固有挑战。
...
TL;DR: 提出了一种名为 Accordion-Thinking 的框架，通过动态摘要实现 LLM 的自我推理粒度调控，引入 Fold 推理模式以压缩推理上下文，并通过强化学习加速了 Fold 模式与 Unfold 模式的准确性收敛，实现了在GPU内存限制下推理吞吐量提升3倍且不牺牲准确性，并生成可读的推理步骤摘要。


<details>
  <summary>Details</summary>
Motivation: 长链式思考（Long Chain-of-Thought）在提升 LLM 推理能力方面效果显著，但受限于 KV 缓存的线性增长和注意力机制的二次复杂度，在实际应用中存在性能瓶颈。研究旨在解决这一效率问题。

Method: 引入 Accordion-Thinking 框架，该框架使 LLM 能够通过动态摘要自我调节推理步骤的粒度。关键技术包括：1. Fold 推理模式，通过周期性总结并丢弃历史思考来减少对历史 token 的依赖；2. 强化学习，用于进一步激励模型学习这种压缩能力；3. 揭示了 Fold 模式与 Unfold 模式的准确性差距会随着训练的进行而缩小直至消失。

Result: Accordion-Thinker 证明了通过学习到的自我压缩机制，LLM 可以在不牺牲解决方案质量的前提下，以极低的 token 开销处理复杂的推理任务。在 48GB GPU 内存配置下，实现了 3 倍的吞吐量提升，同时保持了准确性。此外，生成的结构化步骤摘要提供了人类可读的推理过程。

Conclusion: Accordion-Thinking 框架通过引入 Fold 推理模式和强化学习，实现了 LLM 推理上下文的有效压缩，能够显著提升推理效率和吞吐量，尤其是在内存受限的环境下，同时保持了推理的准确性，并提供了可解释的推理过程。
...
TL;DR: 本研究提出了LPS-Bench，一个用于评估基于MCP的计算机使用代理（CUA）在长时任务中的规划时安全意识的新基准。该基准包含65个场景，涵盖7个任务领域和9种风险类型，并引入了多智能体自动流水线进行数据生成和LLM-as-a-judge评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有CUA在与真实计算机系统交互时存在严重的安全性风险，如模糊指令可能导致有害操作，以及对抗性用户可能操纵工具执行以达到恶意目的。现有的基准测试主要关注短时或基于GUI的任务，并侧重于执行时错误，而忽略了对规划时风险的预测能力。

Method: 提出了LPS-Bench基准，包含65个场景，涵盖7个任务领域和9种风险类型，用于评估MCP-based CUA的规划时安全意识。采用多智能体自动流水线进行大规模数据生成，并使用LLM-as-a-judge评估协议通过规划轨迹评估安全意识。

Result: 实验表明，现有的CUA在保持安全行为方面存在显著缺陷。研究还分析了风险并提出了改进长时规划安全性的缓解策略。

Conclusion: LPS-Bench基准有效地揭示了现有CUA在规划时安全意识方面的不足，并为未来的研究提供了改进长时规划安全性的方向。
...
TL;DR: 本研究提出了CSR-Bench基准，用于评估多模态大型语言模型（MLLMs）在跨模态交互中的可靠性，揭示了当前模型在安全性、过度拒绝、偏见和幻觉方面存在的系统性问题，并发现模型在处理多模态输入时表现出显著的性能下降，且安全性提升可能源于回避式启发式策略而非真正的意图理解。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大型语言模型（MLLMs）可能依赖单模态捷径而非真正的联合意图理解来驱动其安全行为，因此需要一个专门的基准来评估其跨模态可靠性。

Method: 研究提出了CSR-Bench基准，包含四个压力测试交互模式（安全性、过度拒绝、偏见和幻觉），覆盖61种细粒度类型。每个实例都要求整合图像-文本解释，并提供配对的纯文本对照，以诊断由模态引起的行为变化。评估了16个最先进的MLLMs。

Result: 研究观察到系统性的跨模态对齐差距。模型显示出薄弱的安全意识，在干扰下表现出强大的语言主导性，并且从纯文本对照到多模态输入的性能持续下降。同时，研究发现减少过度拒绝与维持安全、非歧视行为之间存在明显的权衡。

Conclusion: 当前MLLMs在跨模态可靠性方面存在显著差距，其安全机制可能容易受到模态捷径的影响，并且在安全性提升方面可能存在以牺牲真正意图理解为代价的启发式方法。
...
TL;DR: 本研究提出了一种名为“Agentic Proposing”的框架，通过模仿人类解决问题的方式，利用能够动态选择和组合推理模块的专业智能体来自动生成高质量、可验证的训练数据，以克服人工标注成本高昂且难以扩展的限制。实验表明，使用此框架生成的合成数据训练的模型在数学、编程和科学领域表现出色，并且在AIME25竞赛中达到了SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLM）在复杂推理方面的进步受到高质量、可验证数据集的限制，而人工标注成本高昂且难以规模化。当前的合成方法在保持结构有效性时会限制问题复杂度，而为了增加难度而放宽约束又会导致不一致或无解的实例。

Method: 提出了一种名为“Agentic Proposing”的框架，将问题合成建模为一个目标驱动的顺序决策过程，其中一个专业智能体动态选择和组合模块化推理技能。通过内部反思和工具使用的迭代工作流程，利用多粒度策略优化（MGPO）开发了Agentic-Proposer-4B，以生成高精度、可验证的数学、编程和科学训练轨迹。

Result: 在数学、编程和科学领域，使用Agentic-Proposer-4B生成的合成数据训练的下游求解器显著优于现有基线，并表现出强大的跨领域泛化能力。一个仅使用11,000个合成轨迹训练的30B求解器在AIME25上达到了91.6%的准确率，可与GPT-5等专有模型相媲美。

Conclusion: 高质量的合成信号，即使数量很少，也能有效地替代大规模的人工策划数据集，从而有效推动复杂推理能力在LLM中的发展。
...
TL;DR: 本研究提出了MeetAll数据集和MeetMaster XL智能体框架，以解决企业会议场景下AI助手面临的延迟、成本和隐私挑战。MeetAll是一个包含231场企业会议（共140小时）的双语多模态语料库，问题注入遵循企业实际工作流程。MeetBench XL是一个多维度评估协议，用于衡量事实准确性、意图对齐、响应效率、结构清晰度和完整性。MeetMaster XL是一个双策略智能体，通过查询路由和工具调用优化，在实际应用中表现优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有会议基准未能反映真实企业工作流程的需求，这些需求包括快速事实核查、跨会议分析、多方协作产生的查询、长时序上下文以及需要工具增强的推理，同时还要满足严格的延迟、成本和隐私限制。

Method: 1. 构建MeetAll数据集：一个包含231场企业会议（140小时）的双语多模态语料库，通过企业信息协议注入问题，该协议已通过领域专家评审和人类可辨别性研究验证，并考虑了认知负荷、时间上下文跨度、领域专业知识和可操作任务执行等企业关键维度。2. 提出MeetBench XL评估协议：一个与人类判断对齐的多维度评估协议，用于衡量事实保真度、意图对齐、响应效率、结构清晰度和完整性。3. 提出MeetMaster XL智能体：一个学习到的双策略智能体，能够联合优化查询路由（快慢推理路径）和工具调用（检索、跨会议聚合、网络搜索），并使用一个轻量级分类器实现高效路由。

Result: MeetMaster XL通过精确的路由和工具调用，在质量和延迟方面取得了优于单一模型基线的权衡。实验结果表明，相比商业系统，该方法具有一致的优势，并得到了消融研究、鲁棒性测试和实际部署案例研究的支持。

Conclusion: 本研究通过MeetAll数据集、MeetBench XL评估协议和MeetMaster XL智能体框架，有效地解决了企业会议场景下AI助手面临的挑战，显著提升了AI助手在真实企业环境中的效用和性能。
...
TL;DR: 本文提出了一种新的计算论证方法，引入了基于怀疑的论证拒绝和句子级别的论证接受，并定义了结构化双极论证框架（SBAFs）及其语义，同时支持论证和语言层面的可接受集合。


<details>
  <summary>Details</summary>
Motivation: 现有计算论证方法未充分考虑论证者可能仅因怀疑而拒绝论证，以及在辩论中关注单个句子而非整个论证的接受状态。本文旨在弥合这一差距。

Method: 定义了结构化双极论证框架（SBAFs），其中论证由句子组成，并存在攻击和支持关系。提出了具有两个特性的语义：不强制接受所有被支持的论证，并提供语言扩展（可接受的句子集合）的语义。

Result: 提出的语义介于可容纳（admissible）和完备（complete）语义之间，并能产生论证扩展和语言扩展。该方法可以解释论证支持的作用，并表明演绎支持语义是其特例。

Conclusion: 该研究提出了一种更符合哲学和语言学直觉的计算论证模型，允许论证者基于怀疑做出决策，并能同时处理句子和论证层面的接受状态，为现有方法提供了新的视角。
...
TL;DR: 本文提出了一种名为 Memora 的新颖记忆表示方法，它通过结构化地平衡抽象和具体性来解决智能体记忆系统中信息爆炸和检索效率的问题，并在多个基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的智能体记忆系统在处理不断增长的信息时面临效率挑战，而抽象化虽然有助于扩展，但会牺牲细节，影响有效推理。因此，需要一种能够同时保持抽象性和特异性的记忆表示方法。

Method: Memora 采用一种“谐波记忆表示”来组织信息。它使用主要的抽象索引具体的记忆值，并将相关的更新整合到统一的记忆条目中。同时，“线索锚点”扩展了检索范围，连接了不同的记忆方面和相关的记忆。在此基础上，采用了一种主动利用这些记忆连接的检索策略，以超越直接语义相似性来检索信息。

Result: Memora 在 LoCoMo 和 LongMemEval 基准测试中设定了新的最先进水平，在记忆规模增长时，展现了更好的检索相关性和推理有效性。理论上，该框架可以包含标准的检索增强生成（RAG）和知识图（KG）记忆系统作为其特例。

Conclusion: Memora 成功地在抽象性和特异性之间找到了平衡，能够有效地处理大规模记忆，提高检索相关性和推理能力，并为未来的智能体记忆系统研究提供了新的方向。
...
### [48] [CRL-VLA: Continual Vision-Language-Action Learning](https://arxiv.org/abs/2602.03445)
*Qixin Zeng,Shuo Zhang,Hongyin Zhang,Renjie Wang,Han Zhao,Libang Zhao,Runze Li,Donglin Wang,Chao Huang*

Main category: cs.AI

TL;DR: 本文提出了一种名为 CRL-VLA 的框架，用于持续强化学习（CRL）中的视觉-语言-动作（VLA）模型，以解决在开放世界环境中机器人实现终身学习时，稳定性和可塑性之间的权衡难题。通过一种新的双判别器架构和目标条件价值函数（GCVF），CRL-VLA 在保持旧技能的同时，还能有效地学习新技能。


<details>
  <summary>Details</summary>
Motivation: 在开放世界环境中，具身智能体需要不断学习新技能以适应环境变化，而现有的强化学习微调方法在 VLA 模型上存在稳定性和可塑性难以平衡的问题，阻碍了其在机器人领域中的终身学习应用。

Method: 提出 CRL-VLA 框架，通过推导一个将稳定-可塑性权衡与目标条件优势幅度及策略散度联系起来的统一性能界限。该框架采用非对称正则化，通过一个具有新颖 GCVF 的双判别器架构实现：一个冻结的判别器保持语义一致性，一个可训练的判别器驱动适应性。

Result: 在 LIBERO 基准测试中，CRL-VLA 在对抗遗忘和前向适应方面均优于现有基线方法，成功地平衡了稳定性和可塑性这两个相互冲突的目标。

Conclusion: CRL-VLA 框架为解决 VLA 模型在持续强化学习中的稳定-可塑性权衡问题提供了一种有效且理论有据的解决方案，为机器人在动态环境中实现终身学习铺平了道路。
...
TL;DR: 本研究提出了MentalDx Bench，一个用于真实临床环境中精神障碍诊断的基准测试，并在此基础上开发了MentalSeek-Dx，一个专门针对医学的大型语言模型，在参数量相对较小的情况下取得了最先进的诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLMs）在精神评估方面有潜力，但其临床效用受限于缺乏生态效度的基准测试和细粒度诊断监督。研究旨在弥合这一差距，为真实临床环境下的精神障碍诊断提供一个可靠的评估框架和模型。

Method: 研究者构建了一个名为MentalDx Bench的基准测试，包含712份由精神科医生根据ICD-11指南标注的电子健康记录，涵盖76种精神障碍。他们评估了18个LLMs在MentalDx Bench上的表现，并提出了一个名为MentalSeek-Dx的医学专业LLM，该模型通过监督轨迹构建和基于课程的强化学习来内化临床推理过程。

Result: 评估发现，现有LLMs存在“范式错位”：在粗略的诊断分类上表现良好，但在细粒度的障碍级别诊断上系统性地失败。MentalSeek-Dx在MentalDx Bench上取得了最先进（SOTA）的性能，尽管其参数量仅为140亿。

Conclusion: MentalDx Bench为精神障碍诊断提供了一个临床导向的基准。MentalSeek-Dx通过模仿临床推理过程，证明了即使在参数量相对较小的情况下，也能实现可靠的精神疾病诊断，为开发更具临床实用性的LLM模型奠定了基础。
...
TL;DR: 研究人员构建了一个定制的 Transformer 模型，用于分析神经网络在有轨电车难题等道德困境中的决策过程。模型通过嵌入编码情境信息，并使用解释性技术揭示了其道德推理机制及其存在的偏见。


<details>
  <summary>Details</summary>
Motivation: 研究人员希望理解神经网络如何做出道德决策，特别是其在类似有轨电车难题的场景下的推理过程，并探究其内部机制和潜在偏见。

Method: 构建了一个定制的两层 Transformer 模型，使用嵌入向量编码情境信息（受影响者、数量、结果），并在 Moral Machine 数据集上进行训练和评估，同时应用了多种可解释性技术。

Result: 该模型在 Moral Machine 数据集上达到了 77% 的准确率，并且模型规模较小，便于深入分析。通过解释性技术，研究人员发现模型的道德推理分布在不同的计算阶段，并且偏见也集中在特定的计算阶段。

Conclusion: 定制的 Transformer 模型能够有效模拟神经网络在道德决策中的行为，并且通过解释性技术，可以揭示模型内部的道德推理机制和偏见分布，为理解和改进 AI 的道德决策能力提供了见解。
...
TL;DR: 本文提出了一种名为 GFlowPO 的概率性提示优化框架，通过结合 GFlowNet 和动态记忆更新（DMU）机制，提高了语言模型提示搜索的样本效率和效果，并在多项 NLP 任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的提示优化方法样本效率低，常依赖于在固定分布中采样的元提示和在策略更新，限制了其探索能力。因此，需要一种更高效的提示优化方法。

Method: GFlowPO 框架分为两步：1. 使用重放（replay-based）的离策略（off-policy）生成式流网络（GFlowNet）目标，对轻量级提示语言模型进行微调，以提高样本效率；2. 引入训练免费（training-free）的动态记忆更新（DMU）机制，通过注入多样化提示和高性能提示来更新元提示，从而聚焦于高回报区域。

Result: 在少样本文本分类、指令归纳和问答任务上，GFlowPO 持续优于最近的离散提示优化基线方法。

Conclusion: GFlowPO 是一种有效的概率性提示优化框架，通过改进样本效率和搜索策略，显著提升了在各种 NLP 任务中的提示性能。
...
TL;DR: 提出了一种名为RAI（Risk Awareness Injection）的轻量级、无需训练的框架，用于增强视觉语言模型（VLMs）对多模态越狱攻击的安全性，通过注入风险感知能力，在不牺牲模型效用的前提下有效降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的VLMs容易受到多模态越狱攻击，而现有的防御方法（如安全微调、令牌操作）要么成本高昂，要么会显著降低模型效用。研究发现LLMs本身能够识别不安全内容，而视觉输入的加入会稀释风险信号。因此，研究的动机是恢复VLMs的风险识别能力。

Method: RAI框架通过构建一个“不安全原型子空间”（Unsafe Prototype Subspace）来恢复LLM的风险识别能力。该框架首先从语言嵌入中提取不安全原型，然后对选定的高风险视觉令牌进行有针对性的调制，从而激活跨模态特征空间中的安全关键信号。这种调制恢复了模型从视觉输入检测不安全内容的能力，同时保持了原始令牌的语义完整性。

Result: RAI框架在多个越狱攻击和效用基准测试中表现出色，显著降低了攻击成功率，同时没有损害模型的任务性能。

Conclusion: RAI是一种有效的、无需训练的框架，能够恢复VLMs的风险感知能力，从而抵御多模态越狱攻击，并且不会对模型的效用产生负面影响。
...
TL;DR: 本文提出了一种基于直觉模糊偏好的三向冲突分析新框架，通过引入直觉模糊集来更精细地刻画代理人对议题对的态度，并在此基础上开发了冲突度量、冲突分析模型和可调策略，以克服现有模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于偏好的三向冲突分析模型仅使用三种定性关系（偏好、反向、无差别）来描述代理人对议题对的态度，这限制了其捕捉冲突本质的能力。需要更精细的工具来刻画代理人的态度。

Method: 1. 引入直觉模糊偏好概念，用直觉模糊集更精细地表示代理人对议题对的态度。 2. 在此框架下开发直觉模糊偏好冲突度量。 3. 构建用于三向分割代理人对集合、代理人集合和议题集合的三向冲突分析模型。 4. 利用基于冲突度量的相对损失函数计算三向冲突分析的阈值。 5. 提出基于调整机制的可行策略，同时考虑调整幅度和冲突程度，并提供构造算法。

Result: 提出了一种更精细化、更强大的三向冲突分析框架。开发了直觉模糊偏好冲突度量和相应的冲突分析模型。通过相对损失函数确定阈值。设计了考虑调整幅度和冲突程度的可行策略及其算法。

Conclusion: 所提出的基于直觉模糊偏好的三向冲突分析框架能够更有效地捕捉代理人对议题的态度，克服了经典模型的局限性。该框架及其开发的度量、模型和策略在处理复杂冲突情境方面具有有效性。
...
TL;DR: 本文提出了一种名为DiscoverLLM的新框架，旨在解决大型语言模型（LLM）在处理用户模糊、未明确表达的意图时遇到的挑战。该框架通过引入一个用户模拟器，该模拟器能够模拟用户意图的逐步具体化过程，并以意图具体化程度作为奖励信号来训练LLM。实验表明，DiscoverLLM在创意写作、技术写作和SVG绘图等任务上表现更优，并缩短了对话长度，用户研究也证实了其在提高对话满意度和效率方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在处理用户模糊和未表达的意图时存在困难，因为用户自身可能尚未形成明确意图，需要通过观察和探索来发现。简单地询问模糊问题无法解决此问题。因此，研究的动机是开发一种能够帮助用户形成和发现其意图的LLM训练框架。

Method: 本文提出了一种名为DiscoverLLM的新框架。该框架的核心是一个新颖的用户模拟器，它能够模拟用户的认知状态，通过一系列逐步具体化的意图来表示。意图的concretization程度被用作奖励信号，用于训练LLM。训练出的模型能够根据用户意图的清晰度，在需要时探索选项（diverge），在意图明确时进行细化和执行（converge）。

Result: 在创意写作、技术写作和SVG绘图的交互式基准测试中，DiscoverLLM实现了超过10%的任务性能提升，同时对话长度减少了高达40%。在一项包含75名人类参与者的用户研究中，与基线模型相比，DiscoverLLM提高了对话满意度和效率。

Conclusion: DiscoverLLM是一个新颖且可泛化的框架，能够有效地训练LLM来帮助用户形成和发现他们的意图。该框架通过模拟用户意图的逐步具体化和利用该过程作为奖励信号，实现了更高效和令人满意的用户交互，尤其是在处理模糊和开放式请求时。
...
### [55] [Ontology-to-tools compilation for executable semantic constraint enforcement in LLM agents](https://arxiv.org/abs/2602.03439)
...
TL;DR: 本研究提出了一种将本体论（ontologies）编译成可执行工具（ontology-to-tools compilation）的机制，用于将大型语言模型（LLMs）与形式化的领域知识相结合，并在其知识图谱（knowledge graph）的构建过程中强制执行语义约束。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型在生成知识图谱时缺乏形式化领域知识约束的问题，减少手动工程的工作量。

Method: 提出并实现了ontology-to-tools compilation机制，将其集成到The World Avatar（TWA）框架中。利用Model Context Protocol（MCP）和相关的Agent来连接生成模型、符号约束和外部资源。构建了一个基于Agent的工作流，将本体论转化为面向本体的工具，并迭代地从非结构化文本中提取、验证和修复结构化知识。

Result: 通过使用金属有机多面体合成文献作为案例，证明了可执行的本体论语义能够有效指导LLM的行为，并显著减少了手动进行的模式设计和提示工程。

Conclusion: ontology-to-tools compilation是一种通用的范式，可以将形式化知识嵌入到生成式系统中，从而实现LLMs与形式化领域知识的有效耦合，并在知识生成过程中强制执行语义约束。
...
TL;DR: 研究表明，通过聚类和移除细节对逻辑推理过程进行抽象，可以显著提升AI解释的可理解性和降低认知负荷。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统生成的解释难以理解，而传统的符号AI虽然透明但会增加认知负荷。研究旨在探索形式抽象（移除和聚类）如何影响人类的推理表现和认知努力，以改进以人为中心的符号化解释。

Method: 使用Answer Set Programming (ASP) 作为形式框架，定义了“无关细节”并进行抽象。通过认知实验，让参与者在ASP程序生成的解释下对刺激进行分类，并测量理解程度和认知努力。

Result: 实验结果显示，聚类细节显著提高了参与者的理解能力，而移除细节显著降低了认知努力。

Conclusion: 形式抽象，特别是聚类和移除细节，能够有效地增强以人为中心的符号化AI解释，提升人类的理解能力并降低认知负荷。
...
TL;DR: 本文提出了IntentRL框架，通过在进行深度研究（DR）之前主动澄清用户意图，解决DR的计算成本高和耗时问题，从而缓解了自主性与交互性之间的两难困境。


<details>
  <summary>Details</summary>
Motivation: 深度研究（DR）代理虽然能扩展LLM的知识，但计算成本高昂且耗时，尤其是在面对模糊的用户查询时，高自主性往往导致长时间执行和不满意结果。因此，研究的动机是找到一种方法，在DR开始前主动澄清用户意图。

Method: IntentRL框架通过以下方式运作：1. 提出一个可扩展的流水线，利用少量种子样本通过浅层到深层的意图细化图生成高质量的对话轮次，以克服开放式研究数据的稀缺性。2. 采用两阶段强化学习（RL）策略：第一阶段在离线对话上应用RL，学习一般的用户交互行为；第二阶段使用训练好的代理和用户模拟器进行在线试运行，以增强对不同用户反馈的适应性。

Result: IntentRL显著提高了意图命中率和下游任务性能，并且优于闭源DR代理的内置澄清模块以及其他主动LLM基线。

Conclusion: IntentRL框架能够有效训练主动代理，在进行耗时的深度研究之前主动澄清用户意图，从而提升了用户交互和最终任务的性能。
...
TL;DR: 研究发现现有的大语言模型（LLM）路由器存在“路由崩溃”问题，即随着成本预算增加，路由器倾向于选择最强大但最昂贵的模型，即使较便宜的模型已足够。为此，提出了一种名为EquiRouter的决策感知型路由器，该模型直接学习模型排名，有效缓解了路由崩溃，并在RouterBench上实现了约17%的成本节约。


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由器的成本效益权衡不佳，当成本预算增加时，路由器会倾向于选择最昂贵的模型，即使便宜的模型已经足够，导致计算和成本浪费，这被称为“路由崩溃”。

Method: 提出EquiRouter，一种决策感知型路由器，它不预测标量性能分数，而是直接学习模型之间的排名，以解决目标-决策不匹配问题，从而减少路由崩溃。

Result: EquiRouter在RouterBench上，与最先进的路由器相比，在达到GPT-4级别的性能时，成本降低了约17%。

Conclusion: EquiRouter通过直接学习模型排名，有效解决了现有LLM路由器中的“路由崩溃”现象，并在成本效益方面取得了显著提升，恢复了小型模型的使用价值。
...
TL;DR: 本文使用基于智能体的模型和进化博弈论，研究了广泛使用生成式人工智能（AI）对人类文化演进的长期影响，特别是可能导致的“文化崩溃”。研究发现，AI替代性使用者（AI-substitute users）在个体层面选择下占优，尽管他们会更大程度地减少文化变异。而AI互补性使用者（AI-complement users）通过维持探索所需的变异，在群体选择下可能更有益于群体。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的广泛使用可能导致文化变异和多样性的减少，进而引发模型性能问题（如模型崩溃和幻觉），并可能对人类文化演进产生负面影响，甚至导致“文化崩溃”。

Method: 使用基于智能体的模型（agent-based model）和进化博弈论（evolutionary game theory）来比较两种AI使用策略：AI互补（AI-complement）和AI替代（AI-substitute）。研究了这些策略在进化动力学下的竞争和传播。

Result: 在个体层面选择下，AI替代性使用者更有可能胜出，尽管这会带来更强的文化变异减少。而在群体选择下，如果群体边界足够强，AI互补性使用者则可能因维持文化变异和探索而对群体有利。

Conclusion: AI替代性使用策略在个体层面具有优势，但可能加速文化变异的减少。AI互补性使用策略在群体层面具有潜在优势，能够维持文化多样性。研究结果为理解AI普及的长期影响提供了见解，并为制定缓解AI风险的政策和组织策略提供了依据。
...
TL;DR: 本文提出了一种名为Persona Generators的方法，利用大型语言模型通过迭代优化生成多样化、覆盖范围广的合成用户群体，以解决AI人机交互评估中真实用户数据收集困难的问题，并证明了该方法在多样性指标上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有AI人机交互评估方法依赖于真实用户数据，但数据收集成本高昂且对新颖技术或未来场景不切实际。现有的基于生成代理建模的方法往往侧重于复制最可能出现的行为（密度匹配），而忽视了罕见行为（支持覆盖），导致长尾行为探索不足。

Method: 本文提出Persona Generators，一种能够为任意上下文生成多样化合成用户群体的函数。通过AlphaEvolve框架，利用大型语言模型作为变异算子，在数百次迭代中对Persona Generator代码进行优化。这种优化过程会生成轻量级的Persona Generators，它们能够根据简短描述自动扩展出多样化的合成用户群体，并最大化覆盖相关多样性轴上的观点和偏好。

Result: 经过优化的Persona Generators在六个多样性指标上显著优于现有基线，能够生成跨越罕见特征组合的用户群体，而这些组合在标准的LLM输出中难以获得。

Conclusion: Persona Generators是一种有效的方法，可以生成多样化且覆盖范围广的合成用户群体，为AI人机交互评估提供了一种可扩展且具有成本效益的解决方案，尤其适用于探索AI系统在长尾场景下的行为。
...
TL;DR: 大型语言模型（LLMs）在医疗模拟方面存在局限性，本文提出了EHRWorld，一个以患者为中心、基于因果序列学习的医疗世界模型，并配合大规模真实电子健康记录数据集EHRWorld-110K，显著提升了长期临床模拟的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLMs）在静态医疗推理任务中表现良好，但它们在模拟疾病进展和治疗结果等动态医疗场景时存在挑战，难以在连续干预下保持一致的患者状态，导致长期模拟误差累积。

Method: 本文提出了一种名为EHRWorld的患者中心医疗世界模型，并使用了一个名为EHRWorld-110K的大规模真实电子健康记录数据集进行训练。该模型采用了因果序列学习范式，以解决LLMs在长期临床模拟中的不足。

Result: EHRWorld在长期模拟稳定性、临床敏感事件建模和推理效率方面均显著优于基于LLMs的基线模型，证明了在因果基础、时间演进的临床数据上进行训练对于构建可靠的医疗世界模型的必要性。

Conclusion: 构建可靠的医疗世界模型需要基于因果、时间演进的临床数据进行训练，而非仅仅依赖于静态的LLM知识。EHRWorld展示了这种方法的有效性，为未来医疗模拟领域的研究提供了新的方向。
...
TL;DR: 本研究评估了大型语言模型（LLM）在复杂太空任务规划方面的能力，发现在战略规划方面有所进步，但在实际执行中仍存在物理单位不一致、边界条件错误和调试效率低下等问题。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在处理高维、物理约束环境下的自主多阶段规划能力，特别是在天体动力学领域。

Method: 使用MLE-Bench框架和基于AIDE的智能体架构，在第12届全球轨迹优化竞赛（GTOC 12）数据集上评估LLM。采用“LLM作为裁判”的方法，利用专家制定的评价标准评估解决方案的战略可行性。

Result: 在过去两年中，LLM在战略可行性方面的平均得分几乎翻倍（从9.3升至17.2/26）。然而，模型在实现阶段存在重大能力差距，未能有效处理物理单位、边界条件和调试。

Conclusion: 当前的LLM在理解和制定太空科学任务策略方面表现出色，但由于实现障碍，它们更像是领域辅助者，而非完全自主的工程师。
...
TL;DR: 本文提出了一种名为Search-R2的框架，用于改进集成搜索的语言代理，通过Actor-Meta-Refiner协作和混合奖励机制解决多尺度信用分配问题，从而提高推理的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法在训练集成搜索的语言代理时，存在多尺度信用分配问题，稀疏的奖励信号难以区分高质量推理和偶然猜测，导致搜索行为冗余或误导。这阻碍了代理有效利用外部知识。

Method: 提出Search-R2框架，包含Actor和Meta-Refiner两个组件。Actor生成初始推理轨迹，Meta-Refiner通过“剪切-再生成”机制诊断并修复错误步骤。引入混合奖励设计，结合结果正确性和检索证据的信息密度。

Result: Search-R2在各种通用和多跳问答数据集上进行了广泛实验，与现有RAG和基于RL的基线方法相比，在不同模型规模下均表现出持续的优越性，提高了推理准确性，同时开销极小。

Conclusion: Search-R2通过Actor-Refiner协作和精细化的奖励信号，有效解决了集成搜索语言代理的信用分配问题，能够生成更高质量的推理轨迹，显著提升了问答性能。
...
TL;DR: 本文提出了一种名为“Context Preference Learning”的方法，通过识别和缓解大型语言模型（LLMs）在多轮对话中出现的“对话惯性”现象，来提高其作为智能体（agent）时的探索能力和整体性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在少样本学习方面表现出色，但在多轮对话场景下，它们倾向于模仿自己之前的回复，限制了探索能力，这成为将其转化为智能体的关键问题。

Method: 通过注意力分析识别出“对话惯性”现象，即模型对先前回复的强对角线注意力。提出Context Preference Learning，通过构建不需要环境奖励的偏好对，校准模型偏好，使其倾向于低惯性回复。同时，在推理时提供上下文管理策略来平衡探索与利用。

Result: 在八个智能体环境和一个深度研究场景中进行实验，结果表明所提出的框架能够有效减少对话惯性，并提升性能。

Conclusion: 对话惯性是少样本LLM转化为智能体时面临的一个挑战，它在长上下文环境中被放大，影响探索。Context Preference Learning通过优先选择低惯性回复，并结合上下文管理策略，成功缓解了这一问题，提高了智能体在复杂环境中的表现。
...
TL;DR: 本文提出了一种名为 TodyComm 的任务导向动态通信算法，用于解决多轮大型语言模型（LLM）多智能体系统中固定通信拓扑的不足，使其能够适应动态变化的角色和约束，并在不同基准测试中证明了其在任务有效性、效率和可扩展性方面的优越性。


<details>
  <summary>Details</summary>
Motivation: 现有基于 LLM 的多智能体系统在推理时使用固定的通信拓扑，这在智能体角色可能随回合动态变化（如面对动态对手、任务进展或通信带宽等时变约束）的实际应用中表现不佳。

Method: 提出 TodyComm 算法，该算法通过策略梯度生成行为驱动的协作拓扑，能够适应每回合的动态变化，从而优化任务效用。

Result: 在五个基准测试中，TodyComm 在面对动态对手和通信预算限制时，表现出优于现有方法的任务有效性，同时保持了代币效率和可扩展性。

Conclusion: TodyComm 是一种有效的方法，能够通过动态调整通信拓扑来提高多轮 LLM 多智能体系统的协作能力，特别是在动态环境中。
...
TL;DR: 本文提出了一种名为 AOrchestra 的统一框架，该框架通过将任何代理抽象为 (指令, 上下文, 工具, 模型) 的元组，从而动态生成和管理子代理，以应对复杂、长期的任务。AOrchestra 可以在执行过程中按需创建专门化的执行器，减少人工工程，并且支持不同框架的代理。实验结果表明，AOrchestra 在三个具有挑战性的基准测试中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的语言代理在处理复杂、长周期的任务时，其子代理作为工具的范式缺乏动态抽象视图，导致适应性不足。

Method: 提出了一种统一的、框架无关的代理抽象，将任何代理建模为 (指令, 上下文, 工具, 模型) 的元组。在此基础上，构建了 AOrchestra 系统，该系统有一个中央协调器，可以在每一步具体化该元组，包括策划相关上下文、选择工具和模型，并通过即时自动创建代理来委托执行。

Result: 在 GAIA、SWE-Bench 和 Terminal-Bench 三个具有挑战性的基准测试中，AOrchestra 在与 Gemini-3-Flash 结合使用时，相对最强的基线取得了 16.28% 的相对性能提升。

Conclusion: AOrchestra 的动态代理抽象和按需生成机制能够有效解决现有代理在处理复杂任务时的适应性问题，减少人工工程，并实现可控的性能-成本权衡。该系统框架无关，易于集成不同的代理。
...
TL;DR: 通过引入异质性（如不同模型、提示或工具）可以显著提升基于LLM的多智能体系统（MAS）的性能，因为这增加了系统的有效信息通道数量，克服了同质性MAS中因输出高度相关而导致的收益递减问题。该研究提出了一个信息论框架和度量标准 $K^*$ 来量化这种有效通道数，并提供了设计高效MAS的指导。


<details>
  <summary>Details</summary>
Motivation: 研究者发现，在基于LLM的多智能体系统中，简单地增加同质智能体的数量并不能带来持续的性能提升（收益递减），而引入智能体间的异质性却能带来显著的性能增长。这促使研究者探究导致这种现象的根本原因，即是什么限制了同质MAS的扩展性，以及为什么多样性会有帮助。

Method: 研究者提出了一个信息论框架，用于分析多智能体系统的性能上限。他们推导出了不依赖于具体架构的性能界限，并认为系统性能的提升取决于其能够访问的有效信息通道数量。研究者还引入了一个无需地面真实标签即可量化有效通道数量的指标 $K^*$。

Result: 实验结果表明，异质性配置的MAS在性能上始终优于同质性扩展。具体而言，2个异质智能体组成的系统可以达到甚至超过16个同质智能体组成的系统的性能。这证明了智能体多样性在提升MAS效率和鲁棒性方面的重要性。

Conclusion: 多智能体系统的性能瓶颈在于内在的任务不确定性，而不是智能体数量。通过引入异质性，可以增加系统的有效信息通道数量，从而显著提升MAS的性能。研究结果为设计高效、鲁棒的、注重多样性的MAS提供了原则性的指导。
...
TL;DR: 该研究提出了一种基于风险控制的自适应推理框架，通过设置上下两个阈值来优化大型语言模型的计算效率和准确性，同时满足用户指定的风险目标。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在测试时可以根据token预算进行扩展，但如何设置token预算和自适应推理的阈值是一个实际挑战，涉及风险-准确性权衡。研究旨在以风险控制的方式解决预算设置问题，以最小化计算量为代价来限制错误率。

Method: 研究框架引入了一个上阈值，当模型自信时停止推理（冒着输出错误的风险），以及一个新颖的参数化下阈值，用于抢先停止无法解决的实例（冒着过早停止的风险）。通过无分布风险控制，结合验证集和目标风险，最优地指定这些停止机制。对于多重预算控制标准，引入效率损失来选择计算效率最高的退出机制。

Result: 在多种推理任务和模型上的实证结果表明，该风险控制方法是有效的。研究发现，下阈值和集成停止机制在提高计算效率的同时，能够满足用户指定的风险目标。

Conclusion: 该研究提出了一种基于风险控制的自适应推理方法，能够有效地在计算效率和准确性之间进行权衡，并满足用户指定的风险约束。通过优化停止机制，可以显著提高LLM的推理效率。
...
TL;DR: 该论文提出了FigureBench，一个用于从科学文本生成科学插图的大型基准数据集，并介绍了AutoFigure，一个能自动生成高质量科学插图的智能体框架。AutoFigure在FigureBench数据集上进行了训练和评估，结果表明其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 手动创建高质量的科学插图是学术界和工业界公认的瓶颈，阻碍了复杂科学和技术概念的有效传播。因此，需要自动化的方法来解决这一问题。

Method: 论文首先构建了FigureBench，一个包含3300个科学文本-插图对的大型数据集。然后，提出了AutoFigure，一个智能体框架，该框架能够通过“思考、重组和验证”的流程，从长篇科学文本自动生成结构完整且美观的科学插图。

Result: 在FigureBench数据集上进行的实验表明，AutoFigure在生成科学插图方面始终优于所有基线方法，生成的插图达到了发表级别的质量。

Conclusion: FigureBench和AutoFigure为科学插图的自动生成提供了一个强大的基准和框架。AutoFigure能够有效克服手动创建插图的瓶颈，并生成高质量、可发表的科学插图。
...
### [70] [WorldVQA: Measuring Atomic World Knowledge in Multimodal Large Language Models](https://arxiv.org/abs/2602.02537)
...
TL;DR: WorldVQA是一个新的基准测试，用于评估多模态大语言模型（MLLM）对原子视觉世界知识的掌握程度，分离了知识检索和推理，旨在衡量模型“记忆了什么”，并评估模型对从常见到罕见视觉实体的接地和命名能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法常常混淆视觉知识检索和推理能力，导致无法准确衡量模型所记忆的视觉知识。因此，需要一个专门的基准来严格评估MLLM的视觉知识检索能力，以衡量其百科全书式的广度并评估幻觉率。

Method: 构建了一个名为WorldVQA的基准测试。该基准测试通过一个分层分类法，涵盖从常见类别到长尾稀有类别的视觉实体，来评估模型在接地和命名视觉实体方面的原子能力。此方法旨在将视觉知识检索与推理能力解耦。

Result: WorldVQA基准测试能够严格地测试视觉事实性，并为评估当前和下一代前沿模型的百科全书式广度和幻觉率设定标准。

Conclusion: WorldVQA是一个用于严格评估MLLM原子视觉世界知识的基准测试，它通过分离知识检索和推理，并使用分层分类法来评估模型对视觉实体的接地和命名能力，从而有望成为衡量模型视觉事实性和评估其知识广度及幻觉率的标准。
...
### [71] [AdaptMMBench: Benchmarking Adaptive Multimodal Reasoning for Mode Selection and Reasoning Process](https://arxiv.org/abs/2602.02676)
...
TL;DR: 本文提出了AdaptMMBench基准，用于评估自适应多模态推理，该基准通过MCC指标和动态难度识别来评估模型在不同模态（如视觉、文本、工具）之间的自适应选择能力，并提供了多维度过程评估。


<details>
  <summary>Details</summary>
Motivation: 现有对自适应多模态推理的评估方法依赖于静态难度标签和简化指标，无法捕捉模型容量变化带来的动态难度，导致无法区分自适应模式选择和整体性能，且忽略了细粒度的过程分析。

Method: 提出了AdaptMMBench基准，涵盖真实世界、OCR、GUI、知识和数学五个领域。采用Matthews相关系数（MCC）指标来评估不同推理模式选择的合理性，并通过动态识别模型能力边界来确定任务难度。同时，AdaptMMBench还支持关键步骤覆盖率、工具有效性和计算效率等多维度过程评估。

Result: 评估结果显示，自适应模式选择随模型容量的增长而扩展，但与最终准确率显著解耦。关键步骤覆盖率与性能相关，而工具有效性在不同模型架构之间表现不一致。

Conclusion: AdaptMMBench基准能够更全面地评估自适应多模态推理能力，揭示了自适应模式选择与模型容量的关系，并指出了当前模型在工具有效性方面存在的挑战。
...
### [72] [End-to-end reconstruction of OCT optical properties and speckle-reduced structural intensity via physics-based learning](https://arxiv.org/abs/2602.02721)
*Jinglun Yu,Yaning Wang,Wenhan Guo,Yuan Gao,Yu Sun,Jin U. Kang*

Main category: cs.CV

TL;DR: 提出了一种结合物理模型和深度学习的端到端框架，用于从OCT数据中同时重建光学参数图和去噪的结构图像，提高了组织量化表征能力。


<details>
  <summary>Details</summary>
Motivation: 传统OCT逆散射问题在恢复光学参数（如折射率、散射系数、各向异性）时面临衰减、散斑噪声和参数耦合等挑战。

Method: 开发了一个端到端的深度学习框架，该框架结合了基于物理的OCT正向模型，能够从估计的光学参数生成预测信号，用于参数恢复和伪影抑制。该网络在蒙特卡洛模拟的真实数据上进行训练。

Result: 在合成角膜OCT数据集上进行了实验，结果表明该方法能够在噪声下鲁棒地恢复光学参数图，提高分辨率，并增强结构图像的保真度。

Conclusion: 该方法能够进行定量的多参数组织表征，并证明了将物理信息模型与深度学习相结合在计算OCT中的优势。
...
### [73] [SVD-ViT: Does SVD Make Vision Transformers Attend More to the Foreground?](https://arxiv.org/abs/2602.02765)
*Haruhiko Murata,Kazuhiro Hotta*

Main category: cs.CV

TL;DR: 本文提出SVD-ViT，一种利用奇异值分解（SVD）来增强Vision Transformer（ViT）在区分前景和背景方面的能力的模型，从而提高分类性能。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers（ViT）缺乏区分前景和背景的机制，可能导致学习不必要的背景特征和伪影，从而降低分类性能。

Method: 提出SVD-ViT，包含SPC模块、SSVA和ID-RSVD三个组件，利用奇异值分解提取和聚合捕获前景信息的奇异向量，以抑制任务无关的背景噪声和伪影。

Result: 实验结果表明，SVD-ViT提高了分类准确性，有效学习了信息丰富的前景表示，并减少了背景噪声的影响。

Conclusion: SVD-ViT通过引入基于SVD的前景特征提取机制，能够克服ViT在处理包含背景噪声和伪影的图像时的局限性，实现更好的图像分类性能。
...
### [74] [HypCBC: Domain-Invariant Hyperbolic Cross-Branch Consistency for Generalizable Medical Image Analysis](https://arxiv.org/abs/2602.03264)
*Francesco Di Salvo,Sebastian Doerrich,Jonas Alle,Christian Ledig*

Main category: cs.CV

TL;DR: 本文提出了一种利用双曲流形（hyperbolic manifolds）来改善医学图像分析中深度学习模型泛化能力的方法，通过一种无监督、领域不变的双曲交叉分支一致性约束，在多个数据集和模型上取得了显著的性能提升，平均AUC提高了2.1%。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在医学图像分析中存在泛化能力不足的挑战，尤其是在数据稀缺和存在协变量漂移（如不同设备、成像协议、患者异质性）的情况下。现有的基于欧氏流形的方法无法有效捕捉临床数据复杂的层级结构。

Method: 利用双曲流形的优势来建模数据特征，并提出了一种无监督、领域不变的双曲交叉分支一致性约束。该方法在三个领域泛化基准数据集上进行了广泛验证，并使用了三种Vision Transformer (ViT) 模型。

Result: 所提出的方法在11个分布内数据集和3个领域泛化基准（Fitzpatrick17k, Camelyon17-WILDS, 视网膜成像跨数据集设置）上，相比于最先进的欧氏方法，平均AUC提升了2.1%。实验结果表明，该方法能够促进领域不变特征的学习。

Conclusion: 双曲表示学习能够有效提升医学图像分析的泛化能力，并且所提出的无监督、领域不变的双曲交叉分支一致性约束是一种有效的促进领域不变特征学习的方法，能够克服欧氏方法的局限性，并在跨不同成像模态、数据大小和标签粒度的数据集上实现优越的泛化性能。
...
### [75] [LEVIO: Lightweight Embedded Visual Inertial Odometry for Resource-Constrained Devices](https://arxiv.org/abs/2602.03294)
*Jonas Kühne,Christian Vogt,Michele Magno,Luca Benini*

Main category: cs.CV

TL;DR: LEVIO 是一种针对低功耗计算平台优化的视觉-惯性里程计 (VIO) 系统，实现了六自由度实时感知，并以开源形式发布。


<details>
  <summary>Details</summary>
Motivation: 现有的 VIO 系统计算成本过高，不适用于微型无人机和智能眼镜等资源受限的硬件。

Method: LEVIO 整合了 ORB 特征跟踪和捆绑调整等现有 VIO 组件，并采用计算高效的架构，通过并行化和低内存使用来实现低功耗。

Result: LEVIO 在低功耗 RISC-V SoC 上实现了 20 FPS 的实时性能，功耗低于 100 mW，并在公开数据集上进行了验证，取得了效率和精度的良好平衡。

Conclusion: LEVIO 是一种高效且准确的 VIO 系统，适用于资源受限的嵌入式平台，为移动机器人和 AR 应用提供了可行的解决方案。
...
### [76] [LmPT: Conditional Point Transformer for Anatomical Landmark Detection on 3D Point Clouds](https://arxiv.org/abs/2602.02808)
...
TL;DR: 提出了一种名为Landmark Point Transformer (LmPT) 的方法，用于在点云数据上自动检测解剖标志点，并实现了跨物种学习能力，适用于转化研究。


<details>
  <summary>Details</summary>
Motivation: 传统的解剖标志点标注方法耗时且存在观察者间差异，基于规则的方法通用性有限。需要一种更准确、更高效、更具适应性的自动标注方法，尤其是在跨物种研究中。

Method: 提出Landmark Point Transformer (LmPT) 模型，该模型利用点云表示解剖表面，并包含一个条件机制，使其能够适应不同输入类型，实现跨物种学习。在人体和犬股骨数据上进行了评估。

Result: LmPT在人体和犬股骨的解剖标志点检测上表现出泛化能力和有效性，证明了其跨物种学习的潜力。

Conclusion: LmPT是一种有效的自动解剖标志点检测方法，通过引入跨物种学习能力，为转化研究提供了新的可能性。该方法具有良好的泛化性和适应性。
...
### [77] [Efficient Sequential Neural Network with Spatial-Temporal Attention and Linear LSTM for Robust Lane Detection Using Multi-Frame Images](https://arxiv.org/abs/2602.03669)
*Sandeep Patil,Yongqi Dong,Haneen Farah,Hans Hellendoorn*

Main category: cs.CV

TL;DR: 本文提出了一种结合空间-时间注意力机制的序列神经网络模型，用于提高自动驾驶车辆在复杂交通环境下的车道线检测精度和鲁棒性，同时降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有车道线检测方法在混合交通、严重遮挡和眩光等复杂场景下表现不佳，尤其是在忽略图像关键区域和时空显著性方面存在不足。

Method: 构建了一个基于标准编码器-解码器结构和通用神经网络骨干的序列神经网络模型，并引入了空间-时间注意力机制来聚焦车道线特征和利用连续帧之间的时空关联。

Result: 该模型在三个大型开源数据集上进行了训练和评估，实验结果表明其性能优于现有最先进的方法，并且在参数量和计算量方面比基线模型更具计算效率。

Conclusion: 提出的带有时空注意力机制的序列神经网络模型能够有效提升车道线检测的精度和鲁棒性，特别是在挑战性场景下，同时实现了更高的计算效率。
...
### [78] [Self-Supervised Uncalibrated Multi-View Video Anonymization in the Operating Room](https://arxiv.org/abs/2602.02850)
*Keqi Chen,Vinkle Srivastav,Armine Vardazaryan,Cindy Rolland,Didier Mutter,Nicolas Padoy*

Main category: cs.CV

TL;DR: 提出了一种无需标注和相机标定的自监督多视角手术室视频匿名化框架，通过多视角和时间上下文检索漏检的个体，并进行自监督域自适应，实现了高召回率的个体检测和姿态估计，具有实际应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现有手术室视频匿名化方法在新临床站点需要手动标注，且多视角设置需要相机标定，存在可扩展性瓶颈，阻碍了视频数据的研究和应用。

Method: 提出一个自监督多视角视频匿名化框架，包含全身人员检测和全身姿态估计。核心策略是通过“检索”假阴性样本来增强单视角检测器，并进行自监督域自适应。具体步骤包括：1. 使用低置信度阈值的检测器收集候选检测；2. 通过跟踪和无标定多视角关联检索与高分检测一致的低分假阴性；3. 将检索到的检测作为伪标签迭代微调检测器；4. 对检测到的个体进行姿态估计，并用高分预测微调姿态模型。

Result: 在模拟和真实手术视频数据集上，该方法实现了超过 97% 的召回率。此外，使用伪标签训练的实时全身检测器性能相当，证明了方法的实用性。

Conclusion: 所提出的自监督多视角视频匿名化框架能够有效地在无需标注和相机标定的情况下实现高精度的个体检测和姿态估计，克服了现有方法的局限性，并具有良好的实际应用前景。
...
### [79] [ViThinker: Active Vision-Language Reasoning via Dynamic Perceptual Querying](https://arxiv.org/abs/2602.02873)
*Weihang You,Qingchan Zhu,David Liu,Yi Pan,Geng Yuan,Hanqi Jiang*

Main category: cs.CV

TL;DR: ViThinker 框架通过生成查询（query）token，实现视觉-语言模型按需合成专家级视觉特征，从而克服了现有 CoT 方法中视觉信息丢失的问题，提升了感知基础和推理准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的 Chain-of-Thought (CoT) 推理方法在视觉-语言模型中表现不佳，因为视觉到文本的转换过早，丢失了连续的视觉信息（如几何和空间布局）。现有的增强方法（如静态枚举或基于注意力选择）是被动的，无法主动寻找任务相关的视觉细节。

Method: ViThinker 框架受到人类主动感知的启发，通过生成决策（查询）token 来触发按需合成专家对齐的视觉特征。该框架通过两阶段课程进行训练：1. 将冻结的专家蒸馏到模型参数中；2. 通过稀疏性惩罚学习任务驱动的查询，从而发现每个推理步骤所需的最小信息量。推理时，ViThinker 进行生成式心理模拟，无需外部工具。

Result: ViThinker 在多个以视觉为中心的基准测试中表现出持续的改进。结果表明，主动生成查询比被动处理输入在感知基础和推理准确性方面都更优越。

Conclusion: ViThinker 框架能够让视觉-语言模型自主地生成查询 token，按需合成视觉特征，从而有效解决视觉信息丢失问题，并在视觉推理任务中取得更好的性能。
...
### [80] [DoubleTake: Contrastive Reasoning for Faithful Decision-Making in Medical Imaging](https://arxiv.org/abs/2602.02894)
*Daivik Patel,Shrenik Patel*

Main category: cs.CV

TL;DR: 该研究提出了一种新颖的对比式、文档感知参考选择框架，旨在通过平衡视觉相关性、嵌入多样性和来源信息来构建用于区分的证据集，并在此基础上开发了一种反事实对比推理框架，以提高医学影像诊断的准确性和置信度。


<details>
  <summary>Details</summary>
Motivation: 现有医学影像诊断方法依赖于最近邻检索，易产生冗余证据并强化单一假设，无法有效处理视觉上相似但诊断不同的情况。因此，需要一种新的方法来构建更有区分度的证据集，以支持更准确的决策。

Method: 研究提出了一种对比式、文档感知参考选择框架，利用ROCO嵌入和元数据，通过平衡视觉相关性、嵌入多样性和来源信息来构建区分度证据集。在此基础上，开发了反事实对比推理框架，通过结构化的成对视觉比较和基于边界的决策规则进行证据聚合，并加入忠实的弃权机制。

Result: 在MediConfusion基准测试中，所提出的方法实现了最先进的性能，与先前方法相比，集合级准确率提高了近15%，同时减少了混淆并提高了个体准确率。

Conclusion: 该研究成功开发了一种新的对比式参考选择和反事实对比推理框架，显著提高了医学影像诊断的准确性和置信度，为处理易混淆的医学影像诊断问题提供了有效解决方案。
...
### [81] [FaceLinkGen: Rethinking Identity Leakage in Privacy-Preserving Face Recognition with Identity Extraction](https://arxiv.org/abs/2602.02914)
*Wenqi Guo,Shan Du*

Main category: cs.CV

TL;DR: 本文提出了一种名为FaceLinkGen的新型身份提取攻击，该攻击能够直接从隐私保护的人脸识别（PPFR）系统中提取身份信息并重新生成面部图像，而无需恢复原始像素。实验表明，现有PPFR系统的隐私评估方法（如PSNR和SSIM）存在缺陷，无法有效抵御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 现有PPFR系统的隐私评估主要关注抵抗像素级重建，但这种方法可能无法捕捉到更深层次的身份信息泄露风险。研究者旨在揭示现有评估方法的不足，并提出一种更真实的隐私威胁模型。

Method: 研究者设计并实现了一个名为FaceLinkGen的身份提取攻击。该攻击不依赖于像素重建，而是直接从受保护的模板中提取身份信息，用于匹配和面部图像的重新生成。他们在三个最新的PPFR系统上进行了实验评估，并模拟了近乎零知识的场景。

Result: FaceLinkGen在三个PPFR系统上取得了超过98.5%的匹配准确率和96%的再生成功率。即使在近乎零知识的设置下，其匹配准确率和再生成功率仍分别超过92%和94%。

Conclusion: 现有的像素失真测量指标（如PSNR和SSIM）在PPFR评估中存在结构性缺陷，无法准确反映真实的隐私风险。视觉混淆并不能有效阻止身份信息的泄露，无论是对于外部攻击者还是不信任的服务提供商。
...
### [82] [A Multi-scale Linear-time Encoder for Whole-Slide Image Analysis](https://arxiv.org/abs/2602.02918)
...
### [83] [SRA-Seg: Synthetic to Real Alignment for Semi-Supervised Medical Image Segmentation](https://arxiv.org/abs/2602.02944)
*OFM Riaz Rahman Aranya,Kevin Desai*

Main category: cs.CV

TL;DR: 提出SRA-Seg框架，通过对齐合成与真实医学图像的特征分布，有效缩小领域差距，显著提升了在有限真实数据和大量合成数据下的医学图像分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有的合成数据在医学图像分割中虽然视觉上逼真，但由于合成数据和真实数据存在语义特征空间的差异（领域差距），导致其无法有效提升分割性能，现有的半监督学习方法也无法弥合这一差距。

Method: SRA-Seg框架提出了一种相似性对齐（SA）损失，利用冻结的DINOv2嵌入，将合成图像的特征表示拉向其在语义空间中最接近的真实图像对应物。同时，采用软边缘融合技术创造平滑的解剖过渡和连续标签，避免了传统复制粘贴增强的硬边界问题。框架还通过EMA教师模型生成合成图像的伪标签，并应用软分割损失来处理混合区域的不确定性。

Result: 在仅使用10%标注真实数据和90%未标注合成数据的情况下，SRA-Seg在ACDC数据集上达到89.34%的Dice分数，在FIVES数据集上达到84.42%，显著优于现有的半监督方法，并能达到使用未标注真实数据的现有方法的性能水平。

Conclusion: SRA-Seg框架通过有效的特征对齐和改进的数据增强策略，成功解决了合成数据与真实数据之间的领域差距问题，在有限的真实标注数据和大量的合成数据条件下，能够实现具有竞争力的医学图像分割性能。
...
### [84] [Nüwa: Mending the Spatial Integrity Torn by VLM Token Pruning](https://arxiv.org/abs/2602.02951)
*Yihong Huang,Fei Ma,Yihua Shao,Jingcai Guo,Zitong Yu,Laizhong Cui,Qi Tian*

Main category: cs.CV

TL;DR: 提出了一种名为 Nüwa 的两阶段视觉语言模型 (VLM) 令牌剪枝框架，通过在特征聚合阶段保留全局空间参考信息，并在文本指导下进行任务相关令牌选择，从而在视觉问答 (VQA) 和视觉基础 (VG) 任务上都取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉令牌剪枝方法在视觉问答 (VQA) 任务上表现良好，但在视觉基础 (VG) 任务上性能下降明显。作者分析发现，现有方法在利用全局语义相似性和注意力分数时，会丢失由令牌位置信息交互产生的全局空间参考帧。

Method: Nüwa 是一个两阶段的令牌剪枝框架。第一阶段在视觉编码器之后，通过受群体智能算法启发的“分离、对齐和聚合”操作，保留了信息丰富的全局空间锚点。第二阶段在大型语言模型 (LLM) 中，进行文本指导的剪枝，以保留任务相关的视觉令牌。

Result: Nüwa 在多个 VQA 基准测试上达到了当前最优 (SOTA) 性能（从 94% 提升到 95%），并在视觉基础 (VG) 任务上取得了显著的性能提升（从 7% 提升到 47%）。

Conclusion: Nüwa 框架通过在特征聚合阶段有效维持空间完整性，并在 LLM 阶段进行任务相关的剪枝，成功解决了现有方法在视觉基础任务上的性能瓶颈，并同时在视觉问答任务上保持甚至提升了性能。
...
### [85] [TRACE: Temporal Radiology with Anatomical Change Explanation for Grounded X-ray Report Generation](https://arxiv.org/abs/2602.02963)
*OFM Riaz Rahman Aranya,Kevin Desai*

Main category: cs.CV

TL;DR: 提出了一种名为TRACE的新模型，能够同时进行胸部X光片的时序比较、变化分类和空间定位，并生成带有定位框的自然语言描述。


<details>
  <summary>Details</summary>
Motivation: 现有模型在单张X光片报告生成和视觉定位方面有所进展，但缺乏能够结合时序比较和变化检测能力的模型。

Method: TRACE模型接收一张先前的和一张当前的胸部X光片，能够生成描述时间间隔内变化的自然语言描述（例如，恶化、改善、稳定），并使用边界框坐标来精确定位发现的区域。

Result: TRACE在空间定位方面表现出色，定位准确率超过90%。消融研究发现，时序比较和空间定位的联合学习能够产生变化检测能力，而单独学习则无法实现。

Conclusion: TRACE是第一个能够同时执行时序比较、变化分类和空间定位的模型，为胸部X光片的时序变化检测任务奠定了基础。研究表明，空间定位对于时序推理至关重要，可以作为一种空间注意力机制。
...
### [86] [Dynamic High-frequency Convolution for Infrared Small Target Detection](https://arxiv.org/abs/2602.02969)
*Ruojing Li,Chao Xiao,Qian Yin,Wei An,Nuo Chen,Xinyi Ying,Miao Li,Yingqian Wang*

Main category: cs.CV

TL;DR: 本文提出了一种动态高频卷积（DHiF）来增强红外小目标检测（SIRST）的性能，通过生成动态局部滤波器组来显式建模和区分高频成分（HFCs），从而有效区分目标与背景杂波。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的SIRST检测方法忽略了对各种高频成分（HFCs）进行显式建模和学习判别性表示，而这对于区分目标和其他HFCs至关重要。

Method: 提出了一种动态高频卷积（DHiF），通过生成动态局部滤波器组，并利用傅里叶变换性质，在零中心范围内对称调整滤波器参数，使其对HFCs敏感。DHiF与标准卷积结合，能够自适应地处理不同的HFCs区域，捕捉其独特的灰度变化特征。

Result: 在不同SIRST检测网络和真实场景数据集上进行的广泛实验表明，DHiF相比于其他最先进的卷积操作，在检测性能上表现出优越性，并有显著提升。

Conclusion: DHiF是一种有效的、可插拔的卷积模块，能够作为标准卷积的替代品，用于任意SIRST检测网络，在不显著降低计算效率的情况下，通过增强判别性表示学习来提升SIRST检测性能。
...
### [87] [Fisheye Stereo Vision: Depth and Range Error](https://arxiv.org/abs/2602.02973)
*Leaf Jiang,Matthew Holzel,Bernhard Kaplan,Hsiou-Yuan Liu,Sabyasachi Paul,Karen Rankin,Piotr Swierczynski*

Main category: cs.CV

TL;DR: 本文推导了鱼眼立体视觉系统深度和距离误差的解析表达式，并考虑了在大角度下的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前对鱼眼立体视觉系统的误差分析，尤其是在大视角下的准确性，存在不足，因此需要进行更深入的研究。

Method: 通过推导解析表达式，分析鱼眼立体视觉系统的深度和距离误差，并将其与物体距离联系起来，特别关注大角度下的误差。

Result: 得到了鱼眼立体视觉系统深度和距离误差随物体距离变化的解析表达式，并量化了在大角度下的误差影响。

Conclusion: 该研究为理解和改进鱼眼立体视觉系统的精度提供了理论基础，尤其是在处理大视角场景时。
...
### [88] [SceneLinker: Compositional 3D Scene Generation via Semantic Scene Graph from RGB Sequences](https://arxiv.org/abs/2602.02974)
*Seok-Young Kim,Dooyoung Kim,Woojin Cho,Hail Song,Suji Kang,Woontack Woo*

Main category: cs.CV

TL;DR: SceneLinker 是一个新颖的框架，通过语义场景图从 RGB 序列生成组合式 3D 场景，能够根据用户空间自适应地生成与真实世界布局相符的 3D 场景，并支持空间 MR 内容创建。


<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉物体间的上下文关系或合成多样形状方面存在不足，难以生成与物体排列对齐的 3D 场景，而混合现实内容需要根据用户空间生成反映真实世界布局的 3D 场景。

Method: 设计了一个带有交叉检查特征注意力的图网络用于场景图预测，并构建了一个包含联合形状和布局块的图-变分自编码器（graph-VAE）用于 3D 场景生成。

Result: 在 3RScan/3DSSG 和 SG-FRONT 数据集上的实验表明，该方法在定性和定量评估上均优于现有最先进的方法，即使在复杂的室内环境和具有挑战性的场景图约束下也是如此。

Conclusion: 该工作使 C 能够通过场景图从物理环境中生成一致的 3D 空间，从而创建空间 MR 内容。
...
### [89] [Aligning Forest and Trees in Images and Long Captions for Visually Grounded Understanding](https://arxiv.org/abs/2602.02977)
*Byeongju Woo,Zilin Wang,Byeonghyun Pak,Sangwoo Mo,Stella X. Yu*

Main category: cs.CV

TL;DR: 本文提出了一种名为 CAFT 的新框架，用于学习图像-文本表示，特别关注长文本描述。它通过跨领域的森林（视觉）和树（文本）对齐，在没有像素级监督的情况下，实现了全局和局部语义的匹配，从而在长文本检索任务上取得了先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言模型（如 CLIP）在处理长标题时表现不佳，因为它们将图像和文本视为整体进行匹配。然而，细粒度的视觉-语言理解需要捕捉跨视觉和文本域的全局上下文和局部细节的层次化语义。本文的研究动机是解决这个问题，即如何在没有像素级监督的情况下，有效地学习能够同时理解全局和局部语义的图像-文本表示。

Method: CAFT 框架结合了一个从细粒度到粗粒度的视觉编码器和一个分层的文本 Transformer。它使用一种分层对齐损失，该损失同时匹配整个图像与整个标题，并倾向于区域-句子对应。这种方法鼓励粗粒度语义从细粒度的视觉证据构建，而不是从与局部细节无关的聚合中产生。

Result: 在 3000 万个图像-文本对上训练后，CAFT 在六个长文本检索基准测试中取得了最先进的性能，并表现出良好的可扩展性。

Conclusion: 研究表明，CAFT 框架通过分层的跨领域对齐，可以在没有显式的区域级监督的情况下，学习到细粒度的、视觉上有所依据的图像-文本表示，从而有效解决长文本图像-文本匹配的挑战。
...
### [90] [SharpTimeGS: Sharp and Stable Dynamic Gaussian Splatting via Lifespan Modulation](https://arxiv.org/abs/2602.02989)
*Zhanfeng Liao,Jiajun Zhang,Hanzhang Tu,Zhixi Wang,Yunqi Gao,Hongwen Zhang,Yebin Liu*

Main category: cs.CV

TL;DR: SharpTimeGS 是一种新颖的 4D 高斯表示框架，通过引入可学习的寿命参数，实现了对动态场景中静态和动态区域的时间自适应建模，从而在表示和优化上取得了更好的平衡，并在性能和渲染速度上达到了最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的动态场景新视图合成方法在表示和优化静态和动态区域之间存在平衡困难。

Method: 提出 SharpTimeGS，一个寿命感知（lifespan-aware）的 4D 高斯框架。引入可学习的寿命参数，将高斯衰减重塑为平顶轮廓，以实现更稳定的激活。学习到的寿命参数还能调节每个高斯的运动，使静态点稳定而动态点保持自由运动。设计了一种寿命-速度感知的密集化策略，以平衡静态和动态区域的优化。

Result: SharpTimeGS 在多个基准测试中取得了最先进的性能，并支持在 RTX 4090 上以 100 FPS 渲染高达 4K 分辨率的视频。

Conclusion: SharpTimeGS 通过寿命感知表示和优化策略，有效解决了动态场景新视图合成中静态和动态区域的建模与优化不平衡问题，实现了高质量、高效率的 4D 重建。
...
### [91] [Video-OPD: Efficient Post-Training of Multimodal Large Language Models for Temporal Video Grounding via On-Policy Distillation](https://arxiv.org/abs/2602.02994)
*Jiaze Li,Hao Yin,Haoran Xu,Boshen Xu,Wenhui Tan,Zewen He,Jianzhong Ju,Zhenbo Luo,Jian Luan*

Main category: cs.CV

TL;DR: 本文提出了一种名为Video-OPD的高效视频文本定位（TVG）后训练框架，通过对偶策略蒸馏和教师校验的差异聚焦课程，解决了现有基于GRPO方法中的稀疏奖励和计算开销问题，并取得了优于GRPO的性能和更高的训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的TVG后训练方法存在稀疏奖励信号和计算开销大的问题。

Method: 提出Video-OPD框架，采用对偶策略蒸馏（on-policy distillation），利用反向KL散度目标从教师策略中获得密集的token级监督，同时保持了on-policy特性。在此基础上，引入教师校验的差异聚焦（TVDF）课程，迭代地优先选择对学生策略既可靠又具信息量的轨迹。

Result: Video-OPD在TVG任务上持续优于GRPO，收敛速度更快，计算成本更低。

Conclusion: 将on-policy蒸馏确立为TVG领域替代传统强化学习的有效方法，Video-OPD为TVG的后训练提供了一种更高效的解决方案。
...
### [92] [VOILA: Value-of-Information Guided Fidelity Selection for Cost-Aware Multimodal Question Answering](https://arxiv.org/abs/2602.03007)
*Rahul Atul Bhope,K. R. Jayaram,Vinod Muthusamy,Ritesh Kumar,Vatche Isahagian,Nalini Venkatasubramanian*

Main category: cs.CV

TL;DR: VOILA是一个视觉问答框架，通过根据问题特征预测正确率和成本，自适应地选择最低成本但仍能获得高准确率的视觉信息保真度，从而在资源受限的情况下优化多模态推理。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态视觉语言系统在固定保真度下运行，尽管检索和处理高保真视觉输入成本高昂。研究旨在解决如何在资源有限的情况下优化多模态推理的问题。

Method: VOILA框架采用两阶段流程：首先，使用基于梯度提升的回归器仅根据问题特征估计每个保真度的正确性可能性；然后，使用等otonic校准器优化这些概率以进行可靠的决策。系统选择在预测的准确率和检索成本下，能最大化预期效用的最低成本保真度。

Result: 在三个部署场景、五个数据集（VQA-v2, GQA, TextVQA, LoCoMo, FloodNet）和六个参数量从7B到235B的视觉语言模型（VLMs）上进行的评估显示，VOILA实现了50-60%的成本降低，同时在不同查询类型和模型架构下保留了90-95%的全分辨率准确率。

Conclusion: 预检索保真度选择对于在资源限制下优化多模态推理至关重要。VOILA框架能够有效地降低成本并保持高准确率。
...
### [93] [Thinking inside the Convolution for Image Inpainting: Reconstructing Texture via Structure under Global and Local Side](https://arxiv.org/abs/2602.03013)
*Haipeng Liu,Yang Wang,Biao Qian,Yong Rui,Meng Wang*

Main category: cs.CV

TL;DR: 本文提出了一种新的图像修复方法，通过结构和纹理特征图的相互增强来减少卷积下采样过程中的信息损失，从而提高修复质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于CNN的图像修复方法在卷积下采样过程中会丢失结构和纹理特征信息，导致修复效果不理想。研究者希望探究结构和纹理特征图如何相互作用以缓解这一信息损失问题。

Method: 该方法在卷积下采样过程中，利用统计归一化和反归一化策略来指导结构和纹理特征图的重建。

Result: 实验证明，该方法在不同分辨率（256x256和512x512）的图像上都优于现有技术，尤其是在替换所有编码器后效果更佳。

Conclusion: 结构和纹理特征图的相互作用可以有效缓解卷积下采样过程中的信息损失，从而提升图像修复的质量。
...
### [94] [A Vision-Based Analysis of Congestion Pricing in New York City](https://arxiv.org/abs/2602.03015)
*Mehmet Kerem Turkcan,Jhonatan Tavori,Javad Ghaderi,Gil Zussman,Zoran Kostic,Andrew Smyth*

Main category: cs.CV

TL;DR: 该研究利用计算机视觉技术分析纽约市交通摄像头数据，评估了拥堵收费政策对曼哈顿交通流量的影响。


<details>
  <summary>Details</summary>
Motivation: 评估纽约市实施拥堵收费政策后对城市交通流量产生的系统性影响。

Method: 开发计算机视觉管道，处理曼哈顿和纽约市900多个交通摄像头的数据，对比2024年11月（收费实施前）至2026年1月（实施后）的交通模式，建立基线并识别车辆密度变化。

Result: 通过分析数据，识别出拥堵收费政策实施后，监测区域内车辆密度的系统性变化。

Conclusion: 该研究通过自动化分析交通摄像头数据，为评估拥堵收费政策的交通影响提供了量化证据。
...
### [95] [MUSE: A Multi-agent Framework for Unconstrained Story Envisioning via Closed-Loop Cognitive Orchestration](https://arxiv.org/abs/2602.03028)
*Wenzhang Sun,Zhenyu Wang,Zhangchi Hu,Chunfeng Wang,Hao Li,Wei Chen*

Main category: cs.CV

TL;DR: 提出了一种名为 MUSE 的多智能体框架，通过迭代的“计划-执行-验证-修订”循环来解决长篇视听故事生成中的意图执行差距问题，该框架通过显式控制身份、空间构图和时间连续性来维护叙事意图。


<details>
  <summary>Details</summary>
Motivation: 现有的长篇视听故事生成方法存在意图执行差距，导致语义漂移和身份不一致，尤其是在序列增长时。研究旨在克服这一挑战。

Method: 将故事生成视为一个闭环约束执行问题，提出 MUSE 多智能体框架，通过迭代的“计划-执行-验证-修订”循环进行协调。MUSE 将叙事意图转化为对身份、空间构图和时间连续性的显式、机器可执行的控制，并在生成过程中应用有针对性的多模态反馈来纠正违规。此外，还引入了 MUSEBench，一个无需参考且经过人类判断验证的评估协议。

Result: 与代表性基线相比，MUSE 在长时叙事连贯性、跨模态身份一致性和电影质量方面得到了显著提升。

Conclusion: MUSE 框架通过闭环约束执行和多智能体协调，有效地解决了长篇视听故事生成中的意图执行差距问题，在保持叙事连贯性和跨模态一致性方面表现出色。
...
### [96] [Bongards at the Boundary of Perception and Reasoning: Programs or Language?](https://arxiv.org/abs/2602.03038)
*Cassidy Langenfeld,Claas Beger,Gloria Geng,Wasu Top Piriyakulkij,Keya Hu,Yewen Pu,Kevin Ellis*

Main category: cs.CV

TL;DR: 本文提出了一种结合大型语言模型（LLMs）和贝叶斯优化的神经符号方法，用于解决邦加德问题（Bongard problems），一种考验视觉推理能力的经典挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（VLMs）在日常视觉任务上表现良好，但缺乏像人类一样在全新情境下运用视觉推理能力的能力，而邦加德问题正是对这种能力的一种严峻考验。

Method: 该方法首先利用LLMs将假设的邦加德问题规则转化为参数化的程序化表示，然后通过贝叶斯优化进行参数拟合。

Result: 研究在对邦加德问题图像进行分类（已知规则）以及从零开始解决邦加德问题两方面进行了评估。

Conclusion: 提出的神经符号方法能够利用LLMs和贝叶斯优化来解决邦加德问题，展示了在通用视觉推理方面的潜力。
...
### [97] [HP-GAN: Harnessing pretrained networks for GAN improvement with FakeTwins and discriminator consistency](https://arxiv.org/abs/2602.03039)
*Geonhui Son,Jeong Ryong Lee,Dosik Hwang*

Main category: cs.CV

TL;DR: HP-GAN 是一种新的生成对抗网络（GAN）方法，通过结合自监督学习（FakeTwins）和多模型（CNN、ViT）鉴别器的一致性，提高了图像生成的多样性和质量，并在多个数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有 GAN 方法常依赖预训练网络进行感知损失或特征提取，作者希望通过引入自监督学习和鉴别器一致性机制，更有效地利用预训练网络的先验知识，提升图像生成质量和多样性。

Method: HP-GAN 包含两个核心策略：1. FakeTwins：利用预训练网络作为编码器，计算生成图像的自监督损失来训练生成器，促进生成更多样、高质量的图像。2. 鉴别器一致性：引入一个机制，使评估 CNN 和 ViT 特征图的鉴别器保持一致性，从而提高训练的稳定性和图像质量评估的连贯性。

Result: 在包括大量、少量和有限数据以及多种图像域的十七个数据集上的广泛评估表明，HP-GAN 在 Fréchet Inception Distance (FID) 指标上始终优于当前最先进的方法，显著提高了图像的多样性和质量。

Conclusion: HP-GAN 通过 FakeTwins 和鉴别器一致性策略，有效地利用了预训练网络的先验知识，能够生成更高质量、更多样化的图像，并在各种数据场景下均表现出优越的性能。
...
### [98] [IVC-Prune: Revealing the Implicit Visual Coordinates in LVLMs for Vision Token Pruning](https://arxiv.org/abs/2602.03060)
*Zhichao Sun,Yidong Ma,Gang Liu,Yibo Chen,Xu Tang,Yao Hu,Yongchao Xu*

Main category: cs.CV

TL;DR: 提出了一种名为IVC-Prune的视觉-语言模型（LVLM）推理优化方法，通过保留对空间推理至关重要的隐式视觉坐标（IVC）标记和语义相关的前景标记，在保持高性能的同时，将视觉标记数量减少约50%。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉标记剪枝方法通常侧重于语义相关性，但会丢弃对空间推理至关重要的标记，导致LVLM处理高分辨率视觉输入时推理成本高昂。

Method: 通过分析LVLM如何通过RoPE建立隐式视觉坐标系统，识别出对空间推理至关重要的IVC标记。IVC-Prune是一种无需训练、提示感知的剪枝策略，保留IVC标记（通过RoPE的数学性质识别）和前景标记（通过语义种子发现和上下文细化识别）。

Result: 在四个代表性LVLM和二十个基准测试中，IVC-Prune将视觉标记减少了约50%，同时保持了超过99%的原始性能，并在一些基准测试上取得了性能提升。

Conclusion: IVC-Prune是一种有效的LVLM推理优化策略，通过识别并保留对空间推理关键的IVC标记，可以在显著降低计算成本的同时，保持甚至提升模型的性能。
...
### [99] [JRDB-Pose3D: A Multi-person 3D Human Pose and Shape Estimation Dataset for Robotics](https://arxiv.org/abs/2602.03064)
*Sandika Biswas,Kian Izadpanah,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: 本研究提出了JRDB-Pose3D数据集，用于解决现实世界中多人3D姿态估计的挑战，该数据集包含在室内外复杂动态场景下采集的多人姿态数据，并提供SMPL模型参数、跟踪ID以及其他丰富的标注信息。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计数据集多关注单人场景或实验室环境，与现实世界中拥挤、动态的场景存在差距，限制了其在自动驾驶、机器人感知等领域的应用。

Method: 在JRDB数据集的基础上，从移动机器人平台采集了室内外多场景数据，并提供了SMPL模型为基础的3D人体姿态标注，包括一致的体型参数和跨时间帧的个体跟踪ID。

Result: JRDB-Pose3D数据集平均每帧包含5-10个人体姿态，最多可达35人。数据集包含了频繁的遮挡、截断和框外身体部分等现实世界常见挑战，并继承了JRDB数据集的2D姿态、社会分组、活动、交互、语义分割、物体和人类跟踪等丰富标注。

Conclusion: JRDB-Pose3D数据集为应对现实世界中复杂多人的3D姿态估计和人体感知任务提供了急需的资源，有望推动相关领域的研究和应用发展。
...
### [100] [Finding Optimal Video Moment without Training: Gaussian Boundary Optimization for Weakly Supervised Video Grounding](https://arxiv.org/abs/2602.03071)
*Sunoh Kim,Kimin Yun,Daeho Um*

Main category: cs.CV

TL;DR: 本文提出了一种名为高斯边界优化 (GBO) 的新颖推理框架，用于弱监督时间视频定位任务，该框架通过解决一个优化问题来预测视频片段的边界，从而提高定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从高斯参数推断时间段边界时依赖启发式映射，导致局部化性能不佳。

Method: 提出了一种名为高斯边界优化 (GBO) 的推理框架，该框架通过解决一个优化问题来预测片段边界，该问题平衡了提案覆盖率和片段紧凑性，并推导出了封闭式解。

Result: GBO 在标准基准测试中显著提高了局部化性能，取得了最先进的成果，并且在不同提案方案中都显示出效率和通用性。

Conclusion: GBO 是一种原则性的优化框架，用于弱监督时间视频定位，它通过解决一个优化的边界预测问题，克服了现有方法的局限性，并取得了优异的性能。
...
### [101] [A generalizable large-scale foundation model for musculoskeletal radiographs](https://arxiv.org/abs/2602.03076)
...
TL;DR: 本研究提出了一种名为SKELEX的大规模基础模型，用于骨骼肌肉放射影像的分析。该模型通过自监督学习在120万张多样化的影像上进行训练，并在12项下游诊断任务中表现出色，尤其在骨折检测、骨关节炎分级和骨肿瘤分类方面优于现有方法。SKELEX还具备零样本异常定位能力，并已成功部署为网络应用，为骨骼肌肉放射影像的临床应用和研究提供了一个可扩展、标签效率高且泛化性强的AI框架。


<details>
  <summary>Details</summary>
Motivation: 现有的AI模型在骨骼肌肉疾病检测方面存在任务特定、依赖标注和泛化性差等问题。临床上迫切需要一个能在广泛的骨骼肌肉疾病和解剖区域上进行训练的通用基础模型，但现有公开数据集规模小且多样性不足。

Method: 研究人员利用自监督学习在120万张包含多种疾病的骨骼肌肉放射影像上训练了一个大规模基础模型SKELEX。随后，在12项下游诊断任务中对模型进行了评估，并开发了一个基于SKELEX能力的、可解释的、区域引导的骨肿瘤预测模型，并将其部署为网络应用。

Result: SKELEX模型在骨折检测、骨关节炎分级和骨肿瘤分类等任务中普遍优于基线模型。此外，SKELEX展示了零样本异常定位能力，能够识别病变区域，而无需进行特定任务的训练。其开发的骨肿瘤预测模型在独立外部数据集上表现出稳健的性能。

Conclusion: SKELEX为骨骼肌肉影像学提供了一个可扩展、标签效率高且泛化性强的AI框架，为临床转化和数据高效研究奠定了基础。该模型有望推动骨骼肌肉放射影像AI技术的发展和应用。
...
### [102] [Gromov Wasserstein Optimal Transport for Semantic Correspondences](https://arxiv.org/abs/2602.03105)
*Francis Snelgar,Stephen Gould,Ming Xu,Liang Zheng,Akshay Asthana*

Main category: cs.CV

TL;DR: 本研究提出了一种新的语义匹配方法，通过将 DINOv2 特征与一种引入 Gromov Wasserstein 空间平滑先验的最优传输匹配算法相结合，取代了现有方法中计算成本较高的 Stable Diffusion 特征，在保持甚至超越当前最先进性能的同时，显著提高了效率。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的语义匹配方法通常结合 DINOv2 和 Stable Diffusion (SD) 的特征，虽然效果好但计算成本高。本研究旨在寻找一种更有效的方法，在不牺牲性能的前提下，减少对 SD 特征的依赖。

Method: 将 DINOv2 特征与一种改进的匹配算法相结合。该算法用最优传输（Optimal Transport）算法取代了标准的最近邻匹配，并加入了 Gromov Wasserstein 空间平滑先验，以赋予其空间一致性。

Result: 所提出的方法显著提升了 DINOv2 基线的性能，并在语义匹配任务上取得了与采用 Stable Diffusion 特征的最先进方法相当甚至更优的性能，同时效率提高了 5-10 倍。

Conclusion: 通过将 DINOv2 特征与带有 Gromov Wasserstein 空间平滑先验的最优传输匹配算法相结合，可以实现高效且高性能的语义匹配，为该领域提供了一种有前景的替代方案。
...
### [103] [Beyond Cropping and Rotation: Automated Evolution of Powerful Task-Specific Augmentations with Generative Models](https://arxiv.org/abs/2602.03123)
...
TL;DR: EvoAug是一个自动化的数据增强学习管道，它结合了生成模型和进化算法，以学习最优的任务特定增强方法。该方法通过学习随机增强树来生成结构化和自适应的转换，并在细粒度分类和少样本学习任务中取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的图像数据增强方法（如裁剪、旋转）在提高模型鲁棒性方面有限，而生成模型（如条件扩散、NeRFs）虽然能生成更多样化和逼真的数据，但其引入的显著变化可能损害模型性能。因此，需要一种自动化的方法来学习与特定任务相匹配的、利用生成模型优势的数据增强策略。

Method: EvoAug管道利用生成模型（如条件扩散、NeRFs）和高效的进化算法来学习任务特定的数据增强。其核心是一种新颖的图像增强方法，该方法学习随机增强树，该树能够分层地组合增强操作，从而实现更结构化和自适应的转换。

Result: EvoAug在细粒度分类和少样本学习任务中展现出强大的性能。该管道发现的增强方法与领域知识相符，即使在数据量少的情况下也能有效工作。这些结果表明，学习到的生成式增强方法在模型训练中具有巨大潜力。

Conclusion: EvoAug管道能够自动学习最优的任务特定数据增强，利用生成模型的优势来提高模型的鲁棒性，同时避免性能下降。学习到的增强策略具有结构化和自适应性，并且在低数据场景下也能发现与领域知识一致的增强方法，为模型训练提供了新的可能性。
...
### [104] [Feature, Alignment, and Supervision in Category Learning: A Comparative Approach with Children and Neural Networks](https://arxiv.org/abs/2602.03124)
*Fanxiao Wani Qiu,Oscar Leong*

Main category: cs.CV

TL;DR: 本研究比较了儿童和卷积神经网络（CNNs）在少量监督下的半监督类别学习能力。研究发现，儿童能从少量标签中快速泛化，但表现出强烈的特定特征偏见和对对齐的敏感性。CNNs则表现出不同的交互模式，监督的增加可以提高性能，但对齐和特征结构会调节额外监督对学习的影响。


<details>
  <summary>Details</summary>
Motivation: 理解人类和机器如何从稀疏数据中学习是认知科学和机器学习的核心问题。本研究旨在通过物种公平的设计，比较儿童和CNNs在少样本半监督类别学习任务中的表现，以期为理解和改进学习机制提供启示。

Method: 本研究采用物种公平的设计，让儿童和CNNs在相同的条件下接触新物体类别。研究操纵了监督水平（1/3/6个标签）、目标特征（大小、形状、图案）和感知对齐（高/低），并让学习者接触混合标签和无标签的样本。研究通过比较两者的性能来分析其学习模式。

Result: 研究发现，儿童能从极少的标签中快速泛化，但对特征有强烈的偏好，并且对感知对齐非常敏感。CNNs的学习模式与儿童不同：增加监督水平能提升性能，但感知对齐和特征结构会影响额外监督的学习效果。两者的性能差异表明，在进行人机比较时，需要关注监督、特征结构和对齐之间的相互作用，而非仅仅关注总体准确率。

Conclusion: 本研究强调，在进行人类与模型（如CNNs）的学习能力比较时，必须在恰当的条件下进行。研究结果表明，监督水平、特征结构和感知对齐之间的交互作用，比单纯的准确率更能揭示学习机制的异同。这对于设计更有效的学习算法和理解人类认知过程具有重要意义。
...
### [105] [Flexible Geometric Guidance for Probabilistic Human Pose Estimation with Diffusion Models](https://arxiv.org/abs/2602.03126)
*Francis Snelgar,Ming Xu,Stephen Gould,Liang Zheng,Akshay Asthana*

Main category: cs.CV

TL;DR: 本文提出了一种基于扩散模型的3D人体姿态估计框架，该框架能够从2D图像中生成概率分布上的多重姿态，克服了传统方法单一定义和对配对2D-3D训练数据依赖的问题，并在多个数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法面临深度模糊、遮挡导致的“多解性”问题，且许多方法假设存在确定性映射，忽略了问题的多解性。此外，基于机器学习的方法需要大量配对的2D-3D数据进行训练，并且泛化能力不足。

Method: 该方法利用扩散模型进行3D人体姿态估计。具体而言，它采用条件生成框架，通过2D关键点检测器提取的热图的梯度来指导仅在3D数据上训练的无条件扩散模型生成样本，从而实现从2D图像生成一个概率分布上的多重姿态。

Result: 在Human 3.6M数据集的best-of-$m$评估中，该方法取得了优于其他不依赖配对2D-3D训练数据的最先进性能。在MPI-INF-3DHP和3DPW数据集上的泛化能力测试中也表现出具有竞争力的性能。此外，该框架还被成功应用于姿态生成和姿态补全等新任务，无需训练专门的条件模型。

Conclusion: 提出的基于扩散模型的框架成功解决了3D人体姿态估计中的多解性和对配对训练数据依赖的挑战，并在多个数据集上取得了优异的性能。该框架还展现了其灵活性，可应用于多种新颖的姿态相关任务。
...
### [106] [FinMTM: A Multi-Turn Multimodal Benchmark for Financial Reasoning and Agent Evaluation](https://arxiv.org/abs/2602.03130)
*Chenxi Zhang,Ziliang Gan,Liyun Zhu,Youwei Pang,Qing Zhang,Rongjunchen Zhang*

Main category: cs.CV

TL;DR: 本研究提出了FinMTM，一个多模态、多轮次的金融领域问答基准，包含11,133个中英双语问答对和多种任务类型（单选、多选、开放式对话、基于代理的任务），旨在更全面地评估视觉语言模型在金融领域的表现。


<details>
  <summary>Details</summary>
Motivation: 现有金融领域的视觉语言模型评估基准存在局限性，主要体现在：1. 金融领域特有的图表格式和知识密集型推理需求；2. 现有基准多为单轮次且问题格式单一，无法模拟真实应用场景。因此，需要一个更具多样性、更贴近实际的评估基准。

Method: 构建FinMTM基准，包含：1. 数据收集与标注：11,133个中英双语金融问答对，涵盖蜡烛图、统计图表、报告图等；2. 任务设计：包含单选、多选、多轮开放式对话、基于代理的任务；3. 评估协议：针对不同任务设计了集重叠评分规则、多轮对话的轮次与会话结合评分、代理任务的规划与结果综合评分。

Result: 在FinMTM基准上对22个视觉语言模型进行的广泛实验表明，这些模型在精细视觉感知、长上下文推理以及复杂的代理工作流程方面存在不足。

Conclusion: FinMTM基准的提出填补了金融领域多模态、多轮次评估的空白，并通过实验揭示了现有视觉语言模型在处理复杂金融视觉问答任务时的局限性，为未来模型改进指明了方向。
...
### [107] [SwiftVLM: Efficient Vision-Language Model Inference via Cross-Layer Token Bypass](https://arxiv.org/abs/2602.03134)
*Chen Qian,Xinran Yu,Danyang Li,Guoxuan Chi,Zheng Yang,Qiang Ma,Xin Miao*

Main category: cs.CV

TL;DR: 提出了一种新的视觉token剪枝方法SwiftVLM，通过在后续层级重新评估被剪枝的token，有效解决了早期剪枝导致信息丢失的问题，并在多项任务上取得了比现有方法更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法在处理需要精细视觉细节的任务时性能下降明显，原因是早期剪枝可能导致关键信息永久丢失，而忽略了不同层级token重要性的差异。

Method: 提出了一种名为"bypass"的剪枝范式，允许未被选择的视觉token在后续层级被重新评估。在此基础上，设计了SwiftVLM方法，在模型特定的层级进行剪枝，并允许各层级独立做出剪枝决策。

Result: SwiftVLM在多个视觉语言模型和基准测试中，持续优于现有的剪枝策略，实现了更优的准确率-效率权衡，并且选择了更忠实的视觉token。

Conclusion: SwiftVLM通过其"bypass"范式和层级独立的剪枝策略，成功克服了早期剪枝的信息丢失问题，为提高视觉语言模型的效率和性能提供了一种有效的方法。
...
### [108] [FSOD-VFM: Few-Shot Object Detection with Vision Foundation Models and Graph Diffusion](https://arxiv.org/abs/2602.03137)
*Chen-Bin Feng,Youyang Sha,Longfei Liu,Yongjun Yu,Chi Man Vong,Xuanlong Yu,Xi Shen*

Main category: cs.CV

TL;DR: 本文提出了FSOD-VFM框架，利用视觉基础模型（包括UPN、SAM2和DINOv2）进行少样本目标检测，并通过一种新的图论置信度重加权方法解决了边界框碎片化问题，在多个数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 少数样本目标检测（Few-Shot Object Detection, FSOD）面临着如何有效利用有限标注数据检测新类别的挑战。现有方法在利用视觉基础模型的泛化能力时，其生成的边界框存在碎片化问题，导致检测精度下降。

Method: FSOD-VFM框架集成了三个组件：1. 通用候选框网络（UPN）用于类别无关的边界框生成；2. SAM2用于精确的掩码提取；3. DINOv2特征用于适应新类别。核心创新是引入了一种图论置信度重加权方法，将预测边界框建模为有向图的节点，通过图扩散传播置信度，从而提升完整物体的置信度并降低碎片化部分的置信度。

Result: 在Pascal-5$^i$, COCO-20$^i$, 和CD-FSOD数据集上进行了大量实验。FSOD-VFM在无需额外训练的情况下，显著优于现有方法。特别是在CD-FSOD数据集上，10-shot设置下达到了31.6 AP，远超之前训练无关方法（21.4 AP）。

Conclusion: FSOD-VFM框架成功地利用了视觉基础模型的强大能力，并通过创新的图论置信度重加权方法有效解决了少样本目标检测中的边界框碎片化问题，实现了无需额外训练即可获得优越的检测性能。
...
### [109] [Diversity-Preserved Distribution Matching Distillation for Fast Visual Synthesis](https://arxiv.org/abs/2602.03139)
*Tianhe Wu,Ruibin Li,Lei Zhang,Kede Ma*

Main category: cs.CV

TL;DR: 提出了一种名为DP-DMD的新型蒸馏框架，通过将第一步的目标设定为保持样本多样性（如v-prediction），后续步骤专注于质量提升，从而解决现有DMD方法易出现的模式崩溃问题，且无需额外的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有的分布匹配蒸馏（DMD）方法虽然能有效降低生成模型的推理成本，但容易陷入模式崩溃。现有解决方案（如感知或对抗性正则化）计算开销大且训练不稳定。

Method: 提出了一种角色分离的蒸馏框架（DP-DMD）。将蒸馏过程中的步骤进行角色区分：第一步通过目标预测（如v-prediction）损失来保持样本多样性，后续步骤则在标准DMD损失下进行质量优化，同时阻断DMD损失对第一步的梯度回传。

Result: DP-DMD在文本到图像生成任务上，在保持与最先进方法相当的视觉质量的同时，有效保留了样本多样性。该方法无需感知骨干网络、判别器、辅助网络或额外的真实图像。

Conclusion: DP-DMD是一种简单但有效的蒸馏方法，通过明确区分蒸馏步骤的角色，成功解决了DMD的模式崩溃问题，并在不增加额外计算成本的情况下实现了高质量和多样化的样本生成。
...
### [110] [Fully Kolmogorov-Arnold Deep Model in Medical Image Segmentation](https://arxiv.org/abs/2602.03156)
*Xingyu Qiu,Xinghua Ma,Dong Liang,Gongning Luo,Wei Wang,Kuanquan Wang,Shuo Li*

Main category: cs.CV

TL;DR: 本研究提出了首个全基于Kolmogorov-Arnold（KA）表示定理的深度模型ALL U-KAN，通过SaKAN和Grad-Free Spline技术克服了传统KAN深度堆叠的训练和内存障碍，并在医学图像分割任务上展现出优越性能，同时大幅减少了参数量和内存消耗。


<details>
  <summary>Details</summary>
Motivation: 现有研究受限于深度KAN（KANS）的训练困难和高内存需求，只能使用少量KAN层，阻碍了对KAN潜力的全面探索。本研究旨在解决这些局限，实现完全基于KA的深度模型。

Method: 1. 提出Share-activation KAN (SaKAN)，通过重构Kolmogorov-Arnold表示定理的变体，简化参数化并增加训练样本密度，以缓解训练难度。 2. 提出Grad-Free Spline，移除对训练贡献微小但消耗大量GPU内存的样条梯度，显著降低内存和计算开销。 3. 基于上述创新，构建了ALL U-KAN，这是首个全KA的深度模型实现，其中KA和KAonv层完全取代FC和Conv层。

Result: 在三个医学图像分割任务上的广泛评估表明，全KA架构（ALL U-KAN）优于部分KA和传统架构，实现了更高的分割精度。与直接深度堆叠的KAN相比，ALL U-KAN的参数量减少了10倍，内存消耗减少了20多倍。

Conclusion: ALL U-KAN是首个完全基于KA表示定理的深度模型，通过SaKAN和Grad-Free Spline技术，有效解决了深度KAN的训练和内存问题，并在医学图像分割任务中证明了其优越性，为深度KAN架构的进一步探索开辟了新途径。
...
### [111] [Human-in-the-loop Adaptation in Group Activity Feature Learning for Team Sports Video Retrieval](https://arxiv.org/abs/2602.03157)
*Chihiro Nakatani,Hiroaki Kawashima,Norimichi Ukita*

Main category: cs.CV

TL;DR: 本文提出了一种无需人工标注即可进行群组活动特征学习（GAFL）的人机协同自适应方法，并将其应用于群组活动视频检索框架，通过交互式微调来提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有群组活动视频检索方法依赖于监督学习和预定义的活动类别，缺乏灵活性，且需要大量标注数据。本文旨在解决这些问题，提出一种无需预定义类别、数据效率高的人机协同方法。

Method: 1. 自监督预训练GAF空间，基于群组活动相似性而非分类。 2. 引入人机协同微调过程：利用数据效率视频选择策略，向用户展示少量视频供其标注（正面/负面）。 3. 通过对比学习，利用标注数据更新GAF空间，使正面视频靠近查询视频，负面视频远离查询视频。

Result: 在两个团队运动数据集上的实验表明，本文提出的方法显著提高了视频检索性能。消融研究证明了人机协同自适应各组成部分对性能提升的贡献。

Conclusion: 人机协同自适应是一种有效的方法，可以在无需大量人工标注的情况下，通过交互式微调显著提升群组活动视频检索的性能。
...
### [112] [BinaryDemoire: Moiré-Aware Binarization for Image Demoiréing](https://arxiv.org/abs/2602.03176)
*Zheng Chen,Zhi Yang,Xiaoyang Liu,Weihang Zhang,Mengfan Wang,Yifan Fu,Linghe Kong,Yulun Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为 BinaryDemoire 的二值化图像去摩尔纹框架，通过引入摩尔纹感知二值门 (MABG) 和打乱分组残差适配器 (SGRA) 来有效处理摩尔纹的频率特性，在保持模型轻量化的同时，取得了优于现有二值化方法的去摩尔纹效果。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习去摩尔纹方法虽然效果好，但计算成本高，不适合部署。二值化是一种极端的模型压缩方法，但将其直接应用于去摩尔纹效果不佳，因此需要一种专门针对摩尔纹特性的二值化框架。

Method: 设计了两种关键模块：1. 摩尔纹感知二值门 (MABG)，提取轻量级频率描述符和激活统计信息，预测通道级门控系数，用于条件化二值卷积响应的聚合。2. 打乱分组残差适配器 (SGRA)，执行结构化稀疏的快捷连接对齐，并融合交错混合以促进不同通道分区之间的信息交换。

Result: 在四个基准数据集上的大量实验表明，BinaryDemoire 优于当前所有二值化方法，在去摩尔纹性能上取得了 SOTA 效果。

Conclusion: BinaryDemoire 是一个有效的二值化去摩尔纹框架，它通过显式地处理摩尔纹的频率结构，实现了高效且高性能的图像复原，为部署轻量级去摩尔纹模型提供了新的可能。
...
### [113] [LSGQuant: Layer-Sensitivity Guided Quantization for One-Step Diffusion Real-World Video Super-Resolution](https://arxiv.org/abs/2602.03182)
*Tianxing Wu,Zheng Chen,Cirou Xu,Bowen Chai,Yong Guo,Yutong Liu,Linghe Kong,Yulun Zhang*

Main category: cs.CV

TL;DR: 本研究提出了一种名为LSGQuant的量化方法，用于压缩用于视频超分辨率（VSR）的单步扩散模型（DiTs），以减小模型尺寸和计算成本，同时保持接近全精度模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散Transformer（DiT）在视频超分辨率（VSR）方面表现出色，但模型尺寸大、计算成本高，限制了其应用。低比特量化是模型压缩的常用方法，但对于DiT的高动态范围输入和多样化的层行为，其效果受到挑战。

Method: LSGQuant是一种层敏感度引导的量化方法，包括：1. 动态范围自适应量化器（DRAQ），以适应视频Token的激活；2. 通过分析校准过程中的层统计信息，估计层敏感度并采用方差导向层训练策略（VOLTS）；3. 量化感知优化（QAO），联合优化量化分支和保留的高精度分支。

Result: 实验证明，LSGQuant在保持接近全精度模型的性能的同时，显著优于现有的量化技术。

Conclusion: LSGQuant是一种有效的量化方法，可以显著压缩单步扩散模型的尺寸和计算成本，同时在视频超分辨率任务上保持优异的性能。
...
### [114] [From Single Scan to Sequential Consistency: A New Paradigm for LIDAR Relocalization](https://arxiv.org/abs/2602.03198)
*Minghang Zhu,Zhijing Wang,Yuxin Guo,Wen Li,Sheng Ao,Cheng Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为TempLoc的LiDAR重定位框架，通过结合全局坐标估计、注意力机制估计帧间对应点以及引导融合来提高在动态或模糊场景下的鲁棒性。实验结果表明，TempLoc在NCLT和Oxford Robot-Car数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于回归的LiDAR重定位方法在动态或模糊场景下容易出错，因为它们仅依赖单帧推理或忽略了扫描之间的时空一致性。

Method: TempLoc框架包含三个模块：1. 全局坐标估计模块（Global Coordinate Estimation module），用于预测每个LiDAR扫描的点状全局坐标及其不确定性。2. 先验坐标生成模块（Prior Coordinate Generation module），利用注意力机制估计帧间的点对应关系。3. 不确定性引导坐标融合模块（Uncertainty-Guided Coordinate Fusion module），以端到端的方式融合点对应关系的预测，从而获得更具时空一致性和准确性的全局6-DoF位姿。

Result: 在NCLT和Oxford Robot-Car数据集上的实验结果表明，TempLoc的性能显著优于最先进的方法。

Conclusion: 时空感知的对应点建模在LiDAR重定位中是有效的，TempLoc框架能够提高重定位的鲁棒性和准确性。
...
### [115] [Hand3R: Online 4D Hand-Scene Reconstruction in the Wild](https://arxiv.org/abs/2602.03200)
*Wendi Hu,Haonan Zhou,Wenhao Hu,Gaoang Wang*

Main category: cs.CV

TL;DR: Hand3R是一个在线框架，利用预训练手部模型和4D场景基础模型，通过场景感知视觉提示，从单目视频中联合重建动态手部和密集场景几何。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常独立重建手部和场景，忽略了它们之间的关联，并且常依赖离线优化。研究旨在解决这个问题，实现手部和场景的联合、在线重建。

Method: Hand3R结合了预训练的手部专家模型和4D场景基础模型，通过一种场景感知视觉提示机制实现。该机制将高保真的手部先验信息注入持久的场景记忆中，从而在一个前向传播中同步重建精确的手部网格和密集、度量尺度的场景几何。

Result: Hand3R能够进行在线的、联合的手部和场景重建，并且在局部手部重建和全局定位方面都取得了具有竞争力的性能，摆脱了对离线优化的依赖。

Conclusion: Hand3R是第一个能够从单目视频中进行联合4D手部-场景重建的在线框架，通过场景感知提示有效融合了手部先验和场景基础模型，实现了高效且准确的重建。
...
### [116] [VIRAL: Visual In-Context Reasoning via Analogy in Diffusion Transformers](https://arxiv.org/abs/2602.03210)
*Zhiwen Li,Zhongjie Duan,Jinyan Ye,Cen Chen,Daoyuan Chen,Yaliang Li,Yingda Chen*

Main category: cs.CV

TL;DR: 本文提出了VIRAL框架，通过将视觉少样本学习（V-ICL）视为条件生成问题，利用视觉类比来驱动预训练的图像编辑模型进行视觉推理，从而解决了计算机视觉领域复制 in-context learning（ICL）的挑战。


<details>
  <summary>Details</summary>
Motivation: 在计算机视觉领域，由于任务的多样性和异质性，复制 in-context learning（ICL）面临挑战。现有方法难以统一处理不同类型的视觉任务。

Method: VIRAL框架将V-ICL表述为条件生成问题，利用视觉类比（$x_s : x_t :: x_q : y_q$）来引导预训练的扩散 Transformer（DiT）模型。具体方法包括：1. 使用角色感知多图像条件化来适应冻结的DiT。2. 引入混合专家LoRA来减轻不同任务之间的梯度干扰。3. 整理了一个包含感知、恢复和编辑的大规模数据集，以弥补现有视觉上下文数据集的不足。

Result: 实验证明，VIRAL在处理多种视觉任务（包括开放域编辑）方面优于现有方法，验证了统一的V-ICL范式可以处理绝大多数视觉任务。

Conclusion: VIRAL框架成功地通过视觉类比实现了在计算机视觉任务中的in-context learning，证明了一个统一的V-ICL范式能够有效地处理包括开放域编辑在内的广泛视觉任务。
...
### [117] [ConsisDrive: Identity-Preserving Driving World Models for Video Generation by Instance Mask](https://arxiv.org/abs/2602.03213)
*Zhuoran Yang,Yanyong Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为ConsisDrive的、能保持实例身份的驾驶世界模型，通过实例掩码注意力机制和实例掩码损失来解决现有模型中的身份漂移问题，从而生成更高质量的驾驶视频，并提升下游自动驾驶任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有用于生成真实驾驶数据 else 成本低的世界模型常出现身份漂移问题，即同一物体在不同帧中外观或类别发生变化，这是由于缺乏实例层面的时间约束。

Method: ConsisDrive 包含两个核心组件：1. 实例掩码注意力：在注意力模块中引入实例身份掩码和轨迹掩码，确保视觉 token 仅与对应实例特征在空间和时间上交互，从而保持物体身份一致性；2. 实例掩码损失：通过概率性实例掩码自适应地强调前景区域，减少背景干扰同时保持场景保真度。

Result: ConsisDrive 在驾驶视频生成质量上达到了最先进水平，并在 nuScenes 数据集上的下游自动驾驶任务中取得了显著的性能提升。

Conclusion: ConsisDrive 通过强制实例层面的时间一致性，成功解决了世界模型中的身份漂移问题，显著提升了生成视频的质量和在自动驾驶任务中的实用性。
...
### [118] [FARTrack: Fast Autoregressive Visual Tracking with High Performance](https://arxiv.org/abs/2602.03214)
*Guijie Wang,Tong Lin,Yifan Bai,Anjia Cao,Shiyi Liang,Wangbo Zhao,Xing Wei*

Main category: cs.CV

TL;DR: 本文提出了一种名为FARTrack的快速自回归视觉跟踪框架，通过任务特定自蒸馏和跨帧自回归稀疏化技术，在保持高性能的同时显著提高了推理速度，能在GPU上达到343 FPS，CPU上达到121 FPS。


<details>
  <summary>Details</summary>
Motivation: 现有高性能视觉跟踪器推理速度慢，不适用于资源受限设备。需要一种既能保持高性能又能实现高效推理的跟踪框架。

Method: 提出FARTrack框架，包含两个核心技术：1. 任务特定自蒸馏（Task-Specific Self-Distillation）：逐层蒸馏任务特定Token以实现模型压缩和加速，避免手动指定师生层。2. 跨帧自回归稀疏化（Inter-frame Autoregressive Sparsification）：顺序压缩多个模板，学习时间全局最优的稀疏化策略，避免额外运行开销。

Result: FARTrack在GOT-10k数据集上实现了70.6%的AO，同时达到实时性能。最快模型在GPU上速度达343 FPS，CPU上速度达121 FPS。

Conclusion: FARTrack成功地将自回归机制应用于视觉跟踪，通过创新的蒸馏和稀疏化技术，实现了在保证跟踪性能的同时大幅提升了推理速度，使其成为一种适用于多种设备的实用解决方案。
...
### [119] [PokeFusion Attention: Enhancing Reference-Free Style-Conditioned Generation](https://arxiv.org/abs/2602.03220)
*Jingbang Tang*

Main category: cs.CV

TL;DR: 本文提出了一种名为PokeFusion Attention的新型轻量级交叉注意力机制，用于在文本到图像扩散模型中实现无参考、风格可控的角色生成。该方法通过将文本语义与学习到的风格嵌入直接融合在扩散解码器内，有效解决了现有方法在风格一致性和几何结构稳定性方面的问题，同时保持了预训练扩散模型的冻结，并显著减少了参数开销和推理复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在生成具有特定视觉风格的角色时，文本提示往往不足以精确表达视觉风格，容易导致风格漂移和几何不一致。而基于参考图像的方法则增加了推理时的复杂性和部署限制。

Method: 提出了一种名为PokeFusion Attention的轻量级解码器层交叉注意力机制。该机制直接在扩散解码器内融合文本语义和学习到的风格嵌入，将文本和风格的条件解耦。仅训练解码器的交叉注意力层和一个紧凑的风格投影模块，并保持预训练扩散主干模型冻结。

Result: 在Pokemon风格的角色生成基准测试中，PokeFusion Attention在风格保真度、语义对齐和角色形状一致性方面，均优于现有的基于适配器的基线方法。同时，该方法参数开销低，推理时简单高效。

Conclusion: PokeFusion Attention是一种参数高效、即插即用的控制组件，能够有效实现无参考的风格化生成，并能轻松集成到现有扩散模型中，且可迁移至不同模型。它解决了文本提示不足和参考图像依赖的问题，显著提升了生成图像的质量和一致性。
...
### [120] [Spiral RoPE: Rotate Your Rotary Positional Embeddings in the 2D Plane](https://arxiv.org/abs/2602.03227)
*Haoyu Liu,Sucheng Ren,Tingyu Zhu,Peng Wang,Cihang Xie,Alan Yuille,Zeyu Zheng,Feng Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为 Spiral RoPE 的新型二维位置编码方法，用于解决视觉 Transformer 中标准轴向 RoPE 仅能编码轴对齐方向位置信息的局限性，从而提升模型在各种视觉任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 标准轴向二维 RoPE 在视觉 Transformer 中存在内在的限制，即它只能有效地编码轴对齐方向（水平和垂直）的位置信息，而无法捕捉到自然图像中普遍存在的斜向空间关系。这种限制阻碍了模型对图像中斜向空间关系的建模能力。

Method: Spiral RoPE 通过将位置编码通道划分为多个组，并为每个组关联一个均匀分布的方向来实现多方向位置编码。每个通道组的旋转角度由图像块位置在该方向上的投影决定，从而使得模型能够编码超越水平和垂直轴的空间关系。

Result: 在图像分类、分割和生成等多种视觉任务上，Spiral RoPE 均取得了性能上的提升。此外，可视化注意力图显示 Spiral RoPE 能够更集中地激活与语义相关的目标，并更好地遵循局部物体边界。

Conclusion: Spiral RoPE 是一种简单有效的扩展，通过实现多方向位置编码，克服了标准轴向二维 RoPE 的局限性，显著提升了视觉 Transformer 在各种下游任务上的性能，并能生成更具解释性的注意力图。
...
### [121] [EventFlash: Towards Efficient MLLMs for Event-Based Vision](https://arxiv.org/abs/2602.03230)
*Shaoyu Liu,Jianing Li,Guanghui Zhao,Yunjian Zhang,Wen Jiang,Ming Li,Xiangyang Ji*

Main category: cs.CV

TL;DR: 本文提出了一种名为EventFlash的高效事件驱动多模态大语言模型（MLLM），通过时空稀疏化技术显著提高了吞吐量，同时保持了性能，并能处理更长的事件流。


<details>
  <summary>Details</summary>
Motivation: 现有的事件驱动MLLM在处理稀疏的时空事件流时计算成本过高，主要由于它们沿用了密集图像的处理范式，未能有效利用事件数据的稀疏性。

Method: 研究提出了EventMind数据集（包含超过50万条指令和各种长度的事件流），并设计了两个关键模块：1. 自适应时间窗口聚合模块，用于高效时间采样并压缩时序token；2. 稀疏密度引导注意力模块，用于选择性关注有信息量的空间区域。模型采用课程学习策略进行训练。

Result: EventFlash的吞吐量比基线模型（EventFlash-Zero）提高了12.4倍，同时性能相当。它能够处理长达1000个bin的事件流，远超EventGPT的5个bin限制。

Conclusion: EventFlash通过时空稀疏化技术，成功构建了一个高效的事件驱动MLLM，解决了计算成本高的问题，并能够处理更长的事件序列，为事件驱动视觉领域奠定了基础模型。
...
### [122] [QVLA: Not All Channels Are Equal in Vision-Language-Action Model's Quantization](https://arxiv.org/abs/2602.03782)
*Yuhao Xu,Yantai Yang,Zhenyang Fan,Yufan Liu,Yuming Li,Bing Li,Zhipeng Zhang*

Main category: cs.CV

TL;DR: 本研究提出了QVLA，一个针对具身智能体的视觉-语言-动作（VLA）模型的量化框架，通过通道级精细化比特分配，有效降低模型计算需求，同时保持高精度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型计算量巨大，难以部署在资源受限的机器人平台上。现有模型压缩技术（如低比特量化）在VLA模型上应用效果不佳，因为它们忽视了动作偏差累积导致的灾难性任务失败。

Method: QVLA采用动作导向的量化策略，通过测量每个通道在不同比特宽度下的动作空间敏感度，为每个通道分配最优比特数。该框架将量化和剪枝（0比特）统一在一个全局优化框架中。

Result: 在LIBERO基准测试中，采用QVLA量化的模型仅占用原始模型29.2%的VRAM，性能保持98.9%，速度提升1.49倍。相比LLM衍生的SmoothQuant方法，QVLA性能提升22.6%。

Conclusion: QVLA是第一个专门为具身控制设计的面向动作的量化框架，为VLA模型的机器人部署提供了新的基础，显著降低了计算成本，同时保证了模型性能。
...
### [123] [InstaDrive: Instance-Aware Driving World Models for Realistic and Consistent Video Generation](https://arxiv.org/abs/2602.03242)
*Zhuoran Yang,Xi Guo,Chenjing Ding,Chiyu Wang,Wei Wu,Yanyong Zhang*

Main category: cs.CV

TL;DR: InstaDrive是一个新的框架，通过引入实例流引导器和空间几何对齐器来提高生成驾驶视频的真实性，解决了现有世界模型在实例级时间一致性和空间几何保真度方面的不足，并在nuScenes数据集上取得了最先进的视频生成质量，同时增强了下游自动驾驶任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有用于训练自动驾驶模型的世界模型在生成驾驶视频时，难以保持实例级别的时间一致性和空间几何保真度，这限制了其生成真实驾驶视频的能力。

Method: 提出InstaDrive框架，包含两个核心组件：1. 实例流引导器（Instance Flow Guider），用于提取并跨帧传播实例特征，强制执行时间一致性并保持实例身份。2. 空间几何对齐器（Spatial Geometric Aligner），用于提升空间推理能力，确保实例的精确位置，并显式地建模遮挡层级。此外，利用CARLA的自动驾驶系统生成罕见但安全关键的驾驶场景。

Result: InstaDrive在nuScenes数据集上实现了最先进的视频生成质量，并提升了下游自动驾驶任务的表现。通过程序化和随机模拟，能够生成多样的安全关键驾驶场景。

Conclusion: InstaDrive通过实例感知机制显著提高了生成驾驶视频的真实性，解决了世界模型在时间和空间上的不足，并为自动驾驶系统的安全评估提供了更有效的解决方案。
...
### [124] [LaVPR: Benchmarking Language and Vision for Place Recognition](https://arxiv.org/abs/2602.03253)
*Ofer Idan,Dan Badur,Yosi Keller,Yoli Shavit*

Main category: cs.CV

TL;DR: 本文提出了一种名为 LaVPR 的大规模基准，通过引入大量自然语言描述来增强现有的视觉地点识别 (VPR) 数据集，旨在提高 VPR 在极端环境变化和感知混淆下的鲁棒性，并实现仅凭语言描述进行盲定位。研究探讨了多模态融合和跨模态检索两种范式，发现语言描述能有效提升 VPR 性能，尤其是在视觉退化和小型模型上，并为跨模态检索建立了新的基线。LaVPR 支持开发更具弹性和资源效率的定位系统。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉地点识别（VPR）系统在面对极端环境变化和感知混淆时表现不佳，并且无法仅凭语言描述进行“盲”定位，这限制了其在紧急响应等场景的应用。因此，需要一个能够增强 VPR 鲁棒性并支持语言驱动定位的新数据集和方法。

Method: 本文提出了 LaVPR 基准，该基准在现有 VPR 数据集上增加了超过 65 万条自然语言描述。研究了两种 VPR 范式：1）多模态融合，通过结合视觉和语言信息来提高鲁棒性；2）跨模态检索，允许仅使用语言描述进行定位。在跨模态检索方面，使用了低秩适应（LoRA）和多相似性损失来建立基线。

Result: 研究表明，语言描述在视觉退化条件下提供了显著的性能提升，对小型骨干网络的影响尤为显著。通过添加语言信息，紧凑型模型可以媲美大型纯视觉架构的性能。在跨模态检索任务中，基于 LoRA 和多相似性损失的基线方法显著优于标准的对比学习方法。

Conclusion: LaVPR 基准为 VPR 领域引入了大规模的语言描述，为开发对现实世界变化更具弹性且适用于资源受限环境的新一代定位系统提供了支持。语言信息能够有效增强 VPR 的鲁棒性，并实现语言驱动的定位能力。
...
### [125] [Global Geometry Is Not Enough for Vision Representations](https://arxiv.org/abs/2602.03282)
*Jiwan Chung,Seon Joo Kim*

Main category: cs.CV

TL;DR: 全局嵌入几何并不能有效预测组合绑定能力，而基于输入-输出雅可比矩阵的功能敏感性则能可靠地衡量这一能力。现有损失函数仅约束嵌入几何，未约束局部输入-输出映射，这解释了这种差异。


<details>
  <summary>Details</summary>
Motivation: 现有表示学习方法通常假设全局分布良好的嵌入能够支持鲁棒和可泛化的表示，并以此为导向设计训练目标和评估方法。然而，这种全局几何信息对于编码元素间的组合关系（组合绑定）是敏感度不足的。

Method: 作者通过在21个视觉编码器上测试几何度量预测组合绑定能力，并计算输入-输出雅可比矩阵来衡量功能敏感性，然后比较两者与组合绑定能力的相关性。此外，还对损失函数的设计进行了理论分析。

Result: 标准的几何统计度量与组合绑定能力几乎没有相关性（接近于零）。相反，功能敏感性（通过输入-输出雅可比矩阵衡量）能够可靠地跟踪组合绑定能力。理论分析表明，这种差异源于现有损失函数仅约束嵌入的全局几何，而忽略了局部输入-输出映射。

Conclusion: 全局嵌入几何只捕捉了表示能力的部分信息，而功能敏感性是理解和建模复合结构的关键补充维度。未来的表示学习研究应同时关注全局几何和局部功能敏感性。
...
### [126] [A3-TTA: Adaptive Anchor Alignment Test-Time Adaptation for Image Segmentation](https://arxiv.org/abs/2602.03292)
*Jianghao Wu,Xiangde Luo,Yubo Zhou,Lianming Wu,Guotai Wang,Shaoting Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为 A3-TTA 的测试时域自适应框架，通过锚点引导的监督来生成可靠的伪标签，从而提高图像分割模型在领域偏移下的性能，并在医学和自然图像数据集上取得了显著的改进。


<details>
  <summary>Details</summary>
Motivation: 现有的基于伪标签的测试时域自适应（TTA）方法通常依赖于缺乏分布依据的扰动集成策略，容易导致错误累积和灾难性遗忘。因此，研究需要一种更稳定、更可靠的 TTA 方法。

Method: A3-TTA 框架通过以下方式构建可靠的伪标签：1. 识别预测准确的目标域图像（锚点），基于类紧密密度指标；2. 利用这些锚点作为稳定参考来指导伪标签生成；3. 通过语义一致性和边界感知熵最小化来正则化伪标签；4. 引入自适应指数移动平均策略来减轻标签噪声并稳定模型更新。

Result: 在多领域医学图像（心脏结构和前列腺分割）和自然图像数据集上，A3-TTA 相较于源模型显著提高了平均 Dice 分数（10.40 至 17.68 个百分点），并优于多种最先进的 TTA 方法。此外，A3-TTA 在持续 TTA 场景下表现出色，能够跨越连续的目标域保持高性能并具备强大的抗遗忘能力。

Conclusion: A3-TTA 框架通过锚点引导的监督，成功解决了现有 TTA 方法中伪标签不稳定的问题，有效提升了图像分割模型在领域偏移下的鲁棒性和性能，并展现出在持续学习场景下的潜力。
...
### [127] [Full end-to-end diagnostic workflow automation of 3D OCT via foundation model-driven AI for retinal diseases](https://arxiv.org/abs/2602.03302)
...
TL;DR: 本文提出了一种名为FOCUS的基于3D OCT视网膜疾病诊断的端到端自动化框架，利用基础模型实现了从图像质量评估到多疾病分类的全面自动化，并在大规模数据集和多中心真实世界验证中表现出接近甚至优于专家水平的性能，有望推动眼科无人化发展。


<details>
  <summary>Details</summary>
Motivation: 当前的OCT诊断流程复杂且依赖于传统的单切片、单任务AI模型，这限制了其在临床实践中的全面自动化诊断能力。研究旨在开发一个能够实现3D OCT视网膜疾病诊断全流程自动化的框架。

Method: FOCUS框架采用基础模型驱动，首先使用EfficientNetV2-S进行图像质量评估，然后利用微调的Vision Foundation Model进行异常检测和多疾病分类。关键创新在于采用统一的自适应聚合方法，将2D切片级别的预测整合为3D患者级别的诊断。

Result: 在3,300名患者（40,672张切片）的数据集上训练和测试，并在1,345名患者（18,498张切片）的外部验证集上，FOCUS在质量评估（F1: 99.01%）、异常检测（F1: 97.46%）和患者级别诊断（F1: 94.39%）方面取得了高F1分数。在多中心真实世界验证中，性能稳定（F1: 90.22%-95.24%）。与专家相比，FOCUS在异常检测（F1: 95.47% vs 90.91%）和多疾病诊断（F1: 93.49% vs 91.35%）方面表现相当，同时效率更高。

Conclusion: FOCUS框架实现了OCT图像到诊断的自动化流程，是实现眼科无人化和提高群体规模视网膜护理可及性与效率的关键一步，为自主筛查提供了经过验证的蓝图。
...
### [128] [PQTNet: Pixel-wise Quantitative Thermography Neural Network for Estimating Defect Depth in Polylactic Acid Parts by Additive Manufacturing](https://arxiv.org/abs/2602.03314)
*Lei Deng,Wenhao Huang,Chao Yang,Haoyuan Zheng,Yinbin Tian,Yue Ma*

Main category: cs.CV

TL;DR: 本文提出了一种名为 PQT-Net 的像素级定量热成像神经网络，用于精确量化增材制造（AM）聚乳酸（PLA）部件中的缺陷深度，并通过创新的数据增强策略和高效网络架构取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 在无损检测（NDT）中，准确量化增材制造（AM）部件的缺陷深度仍然是一个重大挑战。

Method: 研究提出了一种像素级定量热成像神经网络（PQT-Net），并采用一种新颖的数据增强策略，将热序列数据重构为二维条纹图像，以保留每个像素完整的热扩散时间演变。PQT-Net 架构集成了预训练的 EfficientNetV2-S 主干和一个自定义的具有可学习参数的残差回归头（RRH）。

Result: PQT-Net 在与其它深度学习模型进行的对比实验中表现出优越性，实现了最低 0.0094 mm 的平均绝对误差（MAE），决定系数（R）超过 99%。

Conclusion: PQT-Net 实现了高精度的缺陷深度量化，展示了其在 AM 部件中进行稳健定量缺陷表征的潜力。
...
### [129] [Invisible Clean-Label Backdoor Attacks for Generative Data Augmentation](https://arxiv.org/abs/2602.03316)
*Ting Xiang,Jinhui Zhao,Changjian Chen,Zhuo Tang*

Main category: cs.CV

TL;DR: 提出了一种名为InvLBA的隐形、标签干净的后门攻击方法，通过在潜在特征层面进行扰动，用于生成式数据增强，有效解决了现有像素级攻击方法在生成图像上的低成功率问题。


<details>
  <summary>Details</summary>
Motivation: 生成式数据增强在小规模数据集场景下有效，但易受隐形、标签干净的后门攻击。现有像素级攻击方法在生成图像上成功率低，因此需要新的攻击策略。

Method: InvLBA方法通过对生成模型潜在特征进行扰动来实现隐形、标签干净的后门攻击，而不是直接操作像素。该方法在理论上证明了其泛化能力。

Result: InvLBA方法平均将攻击成功率提高了46.43%，同时对干净准确率几乎没有影响，并对现有最先进的防御方法表现出高鲁棒性。

Conclusion: InvLBA是一种有效且隐蔽的后门攻击方法，适用于生成式数据增强场景，能够克服像素级攻击的局限性，并具有良好的攻击效果和防御鲁棒性。
...
### [130] [MedSAM-Agent: Empowering Interactive Medical Image Segmentation with Multi-turn Agentic Reinforcement Learning](https://arxiv.org/abs/2602.03320)
*Shengyuan Liu,Liuxin Bao,Qi Yang,Wanting Geng,Boyun Zheng,Chenxin Li,Wenting Chen,Houwen Peng,Yixuan Yuan*

Main category: cs.CV

TL;DR: MedSAM-Agent 提出了一种新的框架，通过多步决策过程来改进医学图像的交互式分割，并设计了混合提示策略和两阶段训练流程，以提高效率和临床准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在交互式分割时存在单轮、僵化的交互策略，并且缺乏训练过程中的过程监督，导致无法充分发挥交互式工具的动态潜力并产生冗余操作。

Method: 提出 MedSAM-Agent 框架，将交互式分割重塑为多步自主决策过程。首先，引入混合提示策略以生成专家策展的轨迹，使模型能够内化类人启发式决策和自适应精炼策略。其次，开发了一个两阶段训练流程，集成了多轮、端到端的成果验证和临床保真过程奖励设计。

Result: 在 6 种医学模态和 21 个数据集上进行了广泛的实验，MedSAM-Agent 取得了最先进的性能，有效地统一了自主医学推理和鲁棒的迭代优化。

Conclusion: MedSAM-Agent 成功地将自主医学推理与鲁棒的迭代优化相结合，为医学图像的交互式分割提供了一种更高效、更准确的方法。
...
### [131] [PWAVEP: Purifying Imperceptible Adversarial Perturbations in 3D Point Clouds via Spectral Graph Wavelets](https://arxiv.org/abs/2602.03333)
*Haoran Li,Renyang Liu,Hongjia Liu,Chen Wang,Long Yin,Jian Xu*

Main category: cs.CV

TL;DR: 提出了一种名为PWAVEP的即插即用、非侵入式3D点云防御机制，通过分析扰动与高频光谱分量的关系，利用谱图小波变换和局部稀疏性识别并去除对抗性扰动。


<details>
  <summary>Details</summary>
Motivation: 现有的对抗性攻击对3D点云的防御机制存在侵入性强、训练成本高、需要额外数据等问题，亟需一种更便捷有效的防御方法。

Method: PWAVEP计算每个点的光谱图小波域显著性得分和局部稀疏性得分，然后采用分层策略：首先去除最显著的点（被识别为难以恢复的对抗性异常点），然后对中度显著的点应用谱过滤过程，利用图小波变换衰减与目标点相关的高频系数，从而抑制对抗性噪声。

Result: PWAVEP在准确性和鲁棒性方面优于现有方法，达到了3D点云净化技术的最先进水平。

Conclusion: PWAVEP是一种有效的、非侵入式的3D点云对抗性防御方法，通过在谱域进行分析和处理，能够有效去除对抗性扰动，提升模型的鲁棒性。
...
### [132] [Composable Visual Tokenizers with Generator-Free Diagnostics of Learnability](https://arxiv.org/abs/2602.03339)
*Bingchen Zhao,Qiushan Guo,Ye Wang,Yixuan Huang,Zhonghua Zhai,Yu Tian*

Main category: cs.CV

TL;DR: 本文提出了一种名为CompTok的训练框架，用于学习具有增强组合性的视觉标记器。该框架利用令牌条件扩散解码器和InfoGAN风格的目标来确保解码器不会忽略任何令牌，并通过令牌子集交换训练来促进组合控制，同时使用对抗性流正则化器来处理未配对的交换生成。结果表明，CompTok在图像类条件生成方面取得了最先进的性能，并支持更高级的语义编辑，同时提出的两个新指标也证明了其在令牌空间组合性和可学习性方面的改进。


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记器在组合性方面存在不足，限制了图像生成和编辑的灵活性。作者希望开发一种能够学习具有更强组合性的视觉标记器，以实现更精细的语义控制和编辑。

Method: CompTok框架包括一个令牌条件扩散解码器和一个InfoGAN风格的训练目标。该目标通过训练一个识别模型来预测用于条件化扩散解码器的令牌，从而强制解码器使用所有令牌。为了增强组合性，框架还利用了通过交换图像令牌子集形成的令牌进行训练，并引入了对抗性流正则化器来约束未配对交换生成的自然图像分布。

Result: CompTok在图像类条件生成任务上取得了最先进的性能。此外，它能够通过交换图像间的令牌来实现高级语义编辑。通过提出的两个新指标衡量，CompTok在令牌空间的组合性和可学习性方面均有所提升，并能支持最先进的生成器。

Conclusion: CompTok是一种有效的训练框架，可以学习具有良好组合性的视觉标记器。该框架不仅提升了图像类条件生成性能，还实现了灵活的语义图像编辑。新提出的指标也为评估令牌空间的组合性提供了有用的工具。
...
### [133] [Tiled Prompts: Overcoming Prompt Underspecification in Image and Video Super-Resolution](https://arxiv.org/abs/2602.03342)
*Bryan Sangwoo Kim,Jonghyun Park,Jong Chul Ye*

Main category: cs.CV

TL;DR: 提出一种名为 Tiled Prompts 的新框架，用于图像和视频超分辨率，通过为每个潜在图块生成特定于图块的提示，以解决现有方法中全局提示导致的局部细节丢失和误导问题。


<details>
  <summary>Details</summary>
Motivation: 现有的文本条件扩散模型在超分辨率方面表现出色，但当处理高分辨率图像/视频时，它们通常使用潜在图块（latent tiling）进行扩展。在这种情况下，单一的全局提示会因为图块划分而导致提示信息不足（prompt underspecification），具体表现为局部细节丢失（prompt sparsity）和局部不相关指导（prompt misguidance），而分类器无关引导（classifier-free guidance）会放大这些问题。

Method: 提出 Tiled Prompts 框架，该框架能够为每个潜在图块生成一个特定的提示（tile-specific prompt）。然后，在局部文本条件后验（locally text-conditioned posteriors）下执行超分辨率。这种方法旨在提供高信息量的指导，以最小的开销解决提示信息不足的问题。

Result: 在真实世界的高分辨率图像和视频上的实验表明，与使用全局提示的基线方法相比，Tiled Prompts 在感知质量和文本对齐方面取得了持续的提升。同时，该方法还能减少图像中的幻觉（hallucinations）和图块级别的伪影（tile-level artifacts）。

Conclusion: Tiled Prompts 是一种有效的统一框架，能够解决文本条件扩散模型在高分辨率图像和视频超分辨率中遇到的全局提示信息不足问题，从而提升超分辨率的质量和文本相关性，并减少不必要的伪影。
...
### [134] [Z3D: Zero-Shot 3D Visual Grounding from Images](https://arxiv.org/abs/2602.03361)
*Nikita Drozdov,Andrey Lemeshko,Nikita Gavrilov,Anton Konushin,Danila Rukhovich,Maksim Kolodiazhnyi*

Main category: cs.CV

TL;DR: 本研究提出了一种名为Z3D的零样本3D视觉基础方法，仅使用多视图图像即可在3D场景中定位物体，无需几何监督或先验知识，并在ScanRefer和Nr3D基准上取得了最先进的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本3D视觉基础方法存在性能瓶颈，作者希望通过引入一种新的通用流水线来克服这些限制，并利用多视图图像实现零样本3D视觉基础。

Method: 提出Z3D流水线，该流水线可灵活处理多视图图像，并可选择性地整合相机姿态和深度图。该方法通过（i）采用先进的零样本3D实例分割方法生成高质量3D边界框建议，以及（ii）通过基于提示的分割利用现代视觉语言模型（VLMs）的能力进行高级推理。

Result: 在ScanRefer和Nr3D数据集上的实验表明，Z3D在零样本3D视觉基础任务上取得了最先进的性能，显著优于现有零样本方法。

Conclusion: Z3D是一种有效的零样本3D视觉基础方法，通过结合高质量3D实例分割和基于提示的VLMs推理，成功地解决了现有方法的瓶颈，并在基准测试中展现出领先的性能。
...
### [135] [Symbol-Aware Reasoning with Masked Discrete Diffusion for Handwritten Mathematical Expression Recognition](https://arxiv.org/abs/2602.03370)
*Takaya Kawakatsu,Ryo Ishiyama*

Main category: cs.CV

TL;DR: 提出了一种基于离散扩散的框架，将手写数学表达式识别（HMER）视为迭代符号细化过程，而非序列生成，有效解决了自回归模型的曝光偏差和句法不一致问题，并在MathWriting和CROHME数据集上取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归模型在处理手写数学表达式识别（HMER）时，面临曝光偏差和句法不一致的挑战，因为HMER需要对多种符号和二维结构进行推理。

Method: 提出了一种离散扩散框架，将HMER重构为迭代符号细化过程。通过多步重掩码（multi-step remasking），逐步细化符号和结构关系，消除因果依赖，提高结构一致性。此外，还采用了符号感知分词（symbol-aware tokenization）和随机掩码互学习（Random-Masking Mutual Learning）来增强句法对齐和处理手写多样性的鲁棒性。

Result: 在MathWriting基准测试中，该模型实现了5.56%的字符错误率（CER）和60.42%的精确匹配率（EM），优于强大的Transformer和商业基线模型。在CROHME 2014-2023数据集上，也取得了持续的性能提升。

Conclusion: 离散扩散框架为结构感知视觉识别提供了一种新的范式，超越了传统的生成模型，在HMER任务上展现出显著的优势。
...
### [136] [Multi-Resolution Alignment for Voxel Sparsity in Camera-Based 3D Semantic Scene Completion](https://arxiv.org/abs/2602.03371)
*Zhiwen Yang,Yuxin Peng*

Main category: cs.CV

TL;DR: 本文提出了一种名为多分辨率对齐（MRA）的新方法，通过利用多分辨率3D特征在场景和实例层面的对齐，为相机输入的3D语义场景补全（SSC）问题缓解体素稀疏性，从而提高模型性能和优化效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于相机输入的3D语义场景补全方法主要依赖于体素标签进行监督，但在自动驾驶场景中，大部分体素是空的，导致体素稀疏性问题，限制了优化效率和模型性能。

Method: 提出了一种多分辨率对齐（MRA）方法，包括：1. 多分辨率视图变换器模块，将2D图像特征投影到多分辨率3D特征并进行场景级对齐；2. 立方体语义各向异性模块，识别每个体素的实例级语义重要性；3. 关键分布对齐模块，利用立方体语义各向异性选择关键体素作为实例级锚点，并对不同分辨率的关键特征分布一致性进行循环损失辅助监督。

Result: 通过利用多分辨率特征在场景和实例层面的对齐作为辅助监督，MRA方法能够缓解体素稀疏性问题，从而提升3D语义场景补全的性能和优化效率。

Conclusion: 多分辨率对齐（MRA）方法是一种有效缓解相机输入3D语义场景补全中体素稀疏性问题的方法，通过引入场景和实例层面的多分辨率特征对齐作为辅助监督，能够显著提高模型的性能。
...
### [137] [SLIM-Diff: Shared Latent Image-Mask Diffusion with Lp loss for Data-Scarce Epilepsy FLAIR MRI](https://arxiv.org/abs/2602.03372)
*Mario Pascual-González,Ariadna Jiménez-Partinen,R. M. Luque-Baena,Fátima Nagib-Raya,Ezequiel López-Rubio*

Main category: cs.CV

TL;DR: 提出了一种名为SLIM-Diff的紧凑型联合扩散模型，用于生成癫痫焦氏皮质发育不良（FCD）的MRI图像和掩码，通过共享瓶颈U-Net和可调的Lp目标来提高稳定性和精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成FCD的MRI图像和掩码时存在不稳定性与过拟合的问题，因为FCD病灶在FLAIR MRI图像中非常细微且数量稀少。

Method: 开发了一个紧凑型的联合扩散模型SLIM-Diff，其核心是（1）一个共享瓶颈的U-Net，强制将2通道图像和掩码紧密耦合；（2）通过可调的$L_p$目标进行损失-几何调优。实验中，与DDPM风格的$ε$-预测（$L_2$损失）作为基线进行比较。

Result: 实验证明，$x_0$-预测在联合生成任务中表现最好。此外，使用分数次二次惩罚（$L_{1.5}$）可以提高图像保真度，而$L_2$损失能更好地保留病灶掩码的形态。

Conclusion: SLIM-Diff通过其创新的模型架构和损失函数调优策略，有效解决了FCD联合生成中的挑战，实现了更高的图像保真度和病灶掩码的准确性。
...
### [138] [Unifying Watermarking via Dimension-Aware Mapping](https://arxiv.org/abs/2602.03373)
*Jiale Meng,Runyi Hu,Jie Zhang,Zheming Lu,Ivor Tsang,Tianwei Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为DiM的多维度水印框架，将水印视为一个维度感知映射问题，统一了现有的水印方法。该框架将水印信息建模为不同维度的载荷（一维二进制消息、二维空间掩码、三维时空结构），并发现嵌入和提取的维度配置决定了水印行为。通过在视频领域实例化DiM，实验证明仅改变维度配置即可实现不同的水印能力，如时空篡改定位、局部嵌入控制和帧扰动下的时间顺序恢复。


<details>
  <summary>Details</summary>
Motivation: 现有深度水印方法在编码器-解码器架构上相似，但在功能行为上存在显著差异。作者希望找到一种能够从功能层面统一现有方法的框架。

Method: 提出DiM（Multi-dimensional Watermarking）框架，将水印视为一个维度感知映射问题。水印信息被建模为不同维度的载荷（1D、2D、3D），并研究了嵌入和提取维度配置对水印行为的影响。在视频领域进行了实例化，并进行了实验验证。

Result: 维度配置（同维度映射或跨维度映射）决定了水印的行为。同维度映射保留载荷结构并支持精细控制；跨维度映射则实现空间或时空定位。通过调整维度配置，无需改变网络架构，即可实现时空篡改定位、局部嵌入控制和时间顺序恢复等功能。

Conclusion: DiM框架成功地从功能层面统一了现有的深度水印方法。通过维度配置的调整，可以灵活地实现不同的水印能力，特别是在视频领域，其时空特性使得更多维度映射成为可能，从而带来更强大的水印功能。
...
### [139] [Seeing Through the Chain: Mitigate Hallucination in Multimodal Reasoning Models via CoT Compression and Contrastive Preference Optimization](https://arxiv.org/abs/2602.03380)
*Hao Fang,Jinyu Li,Jiawei Kong,Tianqu Zhuang,Kuofeng Gao,Bin Chen,Shu-Tao Xia,Yaowei Wang*

Main category: cs.CV

TL;DR: 本研究提出了一种名为C3PO的基于训练的缓解框架，通过链式思考压缩和对比偏好优化来解决多模态推理模型中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态推理模型容易出现幻觉，目前有效的解决方案研究不足。

Method: 1. 链式思考压缩：识别并过滤掉使模型过度依赖语言先验而忽略视觉输入的冗余思考代币，以获得更紧凑、信号更有效的链式思考表示。2. 对比偏好优化：引入一种增强推理的偏好调整方案，利用高质量的AI反馈构建训练对，并通过精心设计的诱因器引发模型的固有幻觉模式，为对比校正提供信息性的负信号。

Result: 通过实验证明，C3PO框架在各种多模态推理模型和基准测试中都能持续减少幻觉。

Conclusion: C3PO框架能够有效地缓解多模态推理模型中的幻觉问题，通过压缩链式思考和利用对比学习来提高模型的鲁棒性。
...
### [140] [From Vicious to Virtuous Cycles: Synergistic Representation Learning for Unsupervised Video Object-Centric Learning](https://arxiv.org/abs/2602.03390)
*Hyun Seok Seong,WonJun Moon,Jae-Pil Heo*

Main category: cs.CV

TL;DR: 提出了一种名为Synergistic Representation Learning (SRL) 的无监督对象中心学习方法，通过迭代优化编码器和解码器来解决现有基于重建的方法中出现的编码器和解码器之间的表示差距问题，从而提高了视频对象中心学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于重建的无监督对象中心学习模型，特别是基于槽（slot-based）的模型，在分解复杂场景方面显示出巨大潜力。然而，它们依赖于基于重建的训练，导致编码器的注意力图（高频、清晰）与解码器的重建图（空间一致但模糊）之间存在根本性的冲突，形成一个恶性循环：编码器的噪声特征图迫使解码器平均化各种可能性并产生更模糊的输出，而从模糊重建图计算出的梯度缺乏监督编码器特征所需的高频细节。

Method: 作者提出了Synergistic Representation Learning (SRL)方法，旨在打破上述恶性循环，建立一个编码器和解码器相互精炼的良性循环。SRL利用编码器的清晰度来消除解码器输出中的语义边界模糊，同时利用解码器的空间一致性来去除编码器的特征噪声。这一相互精炼过程通过一个预热阶段和槽正则化目标来稳定，该目标最初为每个槽分配不同的实体。

Result: 通过弥合编码器和解码器之间的表示差距，SRL在视频对象中心学习基准测试中取得了最先进的结果。

Conclusion: SRL通过建立编码器和解码器之间的协同作用，有效地解决了现有对象中心学习模型中存在的表示差距问题，显著提升了模型在复杂场景分解任务上的性能。
...
### [141] [UnHype: CLIP-Guided Hypernetworks for Dynamic LoRA Unlearning](https://arxiv.org/abs/2602.03410)
*Piotr Wójcik,Maksym Petrenko,Wojciech Gromski,Przemysław Spurek,Maciej Zieba*

Main category: cs.CV

TL;DR: 本文提出了一种名为UnHype的框架，通过引入超网络来增强LoRA在大型扩散模型中的概念学习和知识擦除能力，解决了现有LoRA方法在多概念擦除、概念语义适应性和泛化性方面的局限性，并在多种擦除任务中表现出有效性。


<details>
  <summary>Details</summary>
Motivation: 大型扩散模型的潜在滥用（生成有害或颠覆性内容）引发了对有效机器学习知识擦除的需求。现有基于LoRA的方法在适应概念语义、平衡相关概念擦除与泛化性以及处理多概念擦除时存在不足。

Method: UnHype框架将超网络集成到单概念和多概念LoRA训练中，超网络能根据CLIP嵌入动态生成LoRA权重，实现自适应、可扩展的知识擦除。该框架可直接应用于Stable Diffusion和现代流式文本到图像模型。

Result: UnHype在物体擦除、名人擦除和露骨内容移除等具有挑战性的任务中，展现了稳定的训练行为和有效的概念控制能力，证明了其有效性和多功能性。

Conclusion: UnHype通过引入超网络，显著提高了LoRA在大型扩散模型中的知识擦除能力，特别是在处理复杂概念和多概念场景下，能够实现更具适应性和可扩展性的控制，为应对扩散模型的潜在风险提供了新的解决方案。
...
### [142] [Socratic-Geo: Synthetic Data Generation and Geometric Reasoning via Multi-Agent Interaction](https://arxiv.org/abs/2602.03414)
*Zhengbo Jiao,Shaobo Wang,Zifan Zhang,Wei Wang,Bing Zhao,Hu Wei,Linfeng Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种名为Socratic-Geo的自主框架，通过多智能体交互联合数据合成和模型学习，以解决多模态大语言模型（MLLM）在几何推理方面的瓶颈。该框架能够高效生成高质量的图像-文本对，并显着提升了模型在几何推理和图像生成任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的MLLMs在几何推理方面存在不足，主要原因是高质量图像-文本对的极度稀缺。人工标注成本高昂，自动方法难以保证准确性和训练效果。现有方法要么被动适应已有数据，要么采用低效的随机探索，导致生成与学习需求脱节。

Method: Socratic-Geo框架包含Teacher和Solver两个智能体。Teacher智能体通过参数化Python脚本生成图像-文本对，并利用Reflect（检查可解性）和RePI（检查视觉有效性）进行反思性反馈，确保数据纯净度。Solver智能体通过偏好学习优化推理能力，失败路径则用于指导Teacher进行有针对性的数据增强。同时，Generator智能体学习从累积的“图像-代码-指令”三元组中进行图像生成，将程序化绘图能力提炼为视觉生成能力。

Result: Socratic-Solver从仅108个种子问题出发，在六个基准测试中取得了49.11的成绩，仅使用了基线数据量的四分之一，并超越了强大的基线模型2.43个点。Socratic-Generator在GenExam上取得了42.4%的准确率，超越了Seedream-4.0（39.8%），接近Gemini-2.5-Flash-Image（43.1%），刷新了开源模型的最新性能。

Conclusion: Socratic-Geo框架通过创新的多智能体协同学习机制，有效解决了MLLMs在几何推理和图像生成方面的数据稀缺问题，实现了在有限数据和计算资源下显著提升模型性能的目标，证明了自主数据合成与模型学习联合的潜力。
...
### [143] [ConsistentRFT: Reducing Visual Hallucinations in Flow-based Reinforcement Fine-Tuning](https://arxiv.org/abs/2602.03425)
*Xiaofeng Tan,Jun Liu,Yuanting Fan,Bin-Bin Gao,Xi Jiang,Xiaochen Chen,Jinlong Peng,Chengjie Wang,Hongsong Wang,Feng Zheng*

Main category: cs.CV

TL;DR: 本文提出了一种名为ConsistentRFT的框架，旨在解决基于流的模型在强化微调（RFT）过程中出现的视觉幻觉问题，该问题通常表现为过度优化的细节和语义不匹配。ConsistentRFT通过动态粒度采样（DGR）和一致性策略梯度优化（CPGO）来平衡探索与利用，并保持模型的一致性。


<details>
  <summary>Details</summary>
Motivation: 强化微调（RFT）对于流模型实现偏好对齐至关重要，但其常导致视觉幻觉，如细节过度优化和语义不匹配。研究旨在理解这些幻觉产生的原因并提出解决方案。

Method: 该研究从探索和利用的角度统一分析了RFT方法，指出了SDE采样过程中有限的探索导致对局部细节的过度关注而牺牲全局语义，以及策略梯度方法中固有的轨迹模仿过程扭曲了基础矢量场。在此基础上，提出了ConsistentRFT框架，包含动态粒度采样（DGR）机制以平衡全局和局部细节的探索，以及一致性策略梯度优化（CPGO）以通过对齐先验来保持模型一致性。

Result: 实验表明，ConsistentRFT显著减少了视觉幻觉，低级和高级感知的幻觉分别平均降低了49%和38%。在与FLUX1.dev的比较中，ConsistentRFT在模型泛化能力方面优于其他RFT方法，提升了5.1%。

Conclusion: ConsistentRFT是一种有效的框架，能够显著减轻流模型在强化微调过程中产生的视觉幻觉，同时保持模型的泛化能力，优于现有的RFT方法。
...
### [144] [Hierarchical Concept-to-Appearance Guidance for Multi-Subject Image Generation](https://arxiv.org/abs/2602.03448)
*Yijia Xu,Zihao Wang,Jinshi Cui*

Main category: cs.CV

TL;DR: 提出了一种名为CAG（Hierarchical Concept-to-Appearance Guidance）的框架，用于生成多主体图像，提高了身份一致性和构图控制能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多主体图像生成中存在身份不一致和构图控制有限的问题，因为它们依赖扩散模型隐式地将文本提示与参考图像关联。

Method: CAG框架包含两个关键部分：1. VAE dropout训练策略，在训练时随机丢弃参考VAE特征，促使模型依赖VLM（Visual Language Model）的语义信号，增强概念生成的一致性。2. 整合VLM导出的对应关系到一个 correspondence-aware masked attention 模块中，该模块限制文本token只关注匹配的参考区域，确保属性绑定和多主体组合。

Result: 在多主体图像生成任务上取得了最先进的性能，显著提高了指令遵循能力和主体一致性。

Conclusion: CAG框架通过显式、结构化的概念到外观的监督，有效解决了多主体图像生成中的身份不一致和构图控制问题，实现了更高质量的图像合成。
...
### [145] [Contextualized Visual Personalization in Vision-Language Models](https://arxiv.org/abs/2602.03454)
*Yeongtak Oh,Sangwon Yu,Junsung Park,Han Cheol Moon,Jisoo Mok,Sungroh Yoon*

Main category: cs.CV

TL;DR: 提出CoViP框架，通过强化学习和增强生成技术，解决视觉语言模型（VLMs）无法基于用户特定经验生成个性化响应的问题，并在个性化图像描述和下游任务中取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLMs）在生成个性化响应方面存在不足，无法将视觉输入与用户的累积视觉-文本上下文关联起来。

Method: 提出CoViP框架，将个性化图像描述作为核心任务，并采用基于强化学习的后训练和增强生成技术来提升其能力。同时，引入诊断性评估来排除文本捷径并验证模型是否真正利用了视觉上下文。

Result: 现有开源和专有VLMs在个性化图像描述方面存在局限性。CoViP不仅改进了个性化图像描述，还在下游个性化任务中带来了整体性的提升。

Conclusion: CoViP是实现鲁棒且可泛化的情境化视觉个性化的关键一步，能够有效提升VLMs的个性化响应能力。
...
### [146] [Inlier-Centric Post-Training Quantization for Object Detection Models](https://arxiv.org/abs/2602.03472)
*Minsu Kim,Dongyeun Lee,Jaemyung Yu,Jiwan Hur,Giseop Kim,Junmo Kim*

Main category: cs.CV

TL;DR: 本文提出了一种名为 InlierQ 的后训练量化方法，用于解决目标检测中的冗余激活问题，通过区分并抑制与任务无关的异常（如背景杂乱和传感器噪声）来提高量化精度，并在 COCO 和 nuScenes 数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 目标检测计算量大，部署成本高，量化是解决该问题的关键。然而，背景杂乱和传感器噪声等任务无关的形态会产生冗余激活，扩大激活范围并扭曲激活分布，导致量化困难并损害有用特征的保留。现有方法缺乏区分异常的标准，可能误删有用信息。

Method: InlierQ 是一种以内点为中心（inlier-centric）的后训练量化方法。它计算梯度感知的体积显著性得分，将每个体积分类为内点或异常，并使用期望最大化（EM）算法拟合得分的后验分布，从而抑制异常并保留内点信息。该方法无需标签，可直接插入，仅需 64 个校准样本。

Result: 在 COCO 和 nuScenes 基准测试中，InlierQ 在基于摄像头（2D 和 3D）以及基于 LiDAR（3D）的目标检测任务上，均实现了量化误差的持续降低。

Conclusion: InlierQ 能够有效地将异常激活与有信息量的内点区分开，通过抑制异常来减少量化误差，同时保留了关键的语义信息，为目标检测的量化部署提供了一种新的解决方案。
...
### [147] [Decoupling Skeleton and Flesh: Efficient Multimodal Table Reasoning with Disentangled Alignment and Structure-aware Guidance](https://arxiv.org/abs/2602.03491)
*Yingjie Zhu,Xuefeng Bai,Kehai Chen,Yang Xiang,Youcheng Pan,Xiaoqiang Zhou,Min Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种名为DiSCo的解耦结构-内容对齐框架，用于在不依赖昂贵监督训练、强化学习或外部工具的情况下，有效适应大型视觉语言模型（LVLMs）处理表格数据。在此基础上，进一步提出了Table-GLS，一个全局到局部结构引导的推理框架，通过结构化探索和证据归因推理进行表格推理。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型在处理表格图像时存在挑战，原因在于表格复杂的布局和紧密结合的结构-内容信息。现有的解决方案通常依赖昂贵的监督训练、强化学习或外部工具，限制了效率和可扩展性。因此，本文旨在解决如何在最小化标注和不使用外部工具的情况下，使LVLMs适应表格推理的关键问题。

Method: 首先，引入DiSCo（Disentangled Structure-Content alignment）框架，在多模态对齐过程中显式地将结构抽象与语义关联分离，从而高效地使LVLMs适应表格结构。在此基础上，进一步提出Table-GLS（Global-to-Local Structure-guided）推理框架，通过结构化探索和证据归因推理来进行表格推理。

Result: 在多个不同的基准测试中进行的广泛实验表明，所提出的框架能够有效地增强LVLMs的表格理解和推理能力，尤其是在泛化到未见过的表格结构方面表现出色。

Conclusion: DiSCo和Table-GLS框架能够以高效率、最小化标注且不依赖外部工具的方式，提升大型视觉语言模型在表格数据上的理解和推理能力，并能良好地泛化到新的表格结构。
...
### [148] [ELIQ: A Label-Free Framework for Quality Assessment of Evolving AI-Generated Images](https://arxiv.org/abs/2602.03558)
*Xinyue Li,Zhiming Xu,Zhichao Zhang,Zhaolin Cai,Sijing Wu,Xiongkuo Min,Yitong Chen,Guangtao Zhai*

Main category: cs.CV

TL;DR: ELIQ 是一个无需标签的框架，用于评估不断演进的 AI 生成图像的视觉质量和提示-图像对齐度，通过自动构建正负样本对进行可迁移监督，并在多个基准测试中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 随着生成式文本到图像模型的快速发展，旧的标签数据已不再可靠。因此，需要一种能够在无人为标注的情况下评估新一代 AI 生成图像质量的方法。

Method: ELIQ 自动构建正样本对和特定负样本对（涵盖常规失真和 AIGC 特有失真模式），以实现可迁移监督。在此基础上，通过指令微调将预训练的多模态模型转变为质量感知批评者，并使用轻量级门控融合和质量查询 Transformer 预测二维质量。

Result: ELIQ 在多个基准测试中表现优于现有的无标签方法，并能直接泛化到用户生成内容（UGC）场景。它能够为不断演进的生成模型实现可扩展且无标签的质量评估。

Conclusion: ELIQ 提供了一种有效的、无需人类标注的 AI 生成图像质量评估方法，尤其适用于快速发展的生成模型时代，并且能够跨越 AI 生成内容和用户生成内容的应用场景。
...
### [149] [Semantic Routing: Exploring Multi-Layer LLM Feature Weighting for Diffusion Transformers](https://arxiv.org/abs/2602.03510)
*Bozhou Li,Yushuo Guan,Haolin Li,Bohan Zeng,Yiyan Ji,Yue Ding,Pengfei Wan,Kun Gai,Yuanxing Zhang,Wentao Zhang*

Main category: cs.CV

TL;DR: 本研究提出了一种统一的归一化凸融合框架，利用轻量级门控来组织多层LLM隐藏状态，以改进DiT模型中文本到图像的生成能力，并发现深度融合策略效果最佳，而纯时间融合可能损害生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于DiT的文本到图像模型虽然使用LLM作为文本编码器，但文本条件化通常是静态的，只利用LLM的单层，忽略了LLM层间的语义层次和扩散过程中随时间和网络深度的动态变化。研究旨在更好地匹配DiT生成过程，提升模型能力。

Method: 提出了一种统一的归一化凸融合框架，配备轻量级门控，通过时间、深度和联合融合来系统地组织多层LLM隐藏状态。实验评估了不同融合策略的有效性。

Result: 深度语义路由（Depth-wise Semantic Routing）被证明是最优的条件化策略，显著提高了文本-图像对齐和组合生成能力（例如，GenAI-Bench计数任务上提升9.97）。相反，纯时间融合（time-wise fusion）可能损害视觉生成保真度，这归因于训练-推理轨迹不匹配。

Conclusion: 深度路由是文本到图像生成中一种强大有效的条件化基线。同时，研究强调了需要轨迹感知信号来实现鲁棒的时间依赖性条件化，以解决纯时间融合可能带来的问题。
...
### [150] [A Lightweight Library for Energy-Based Joint-Embedding Predictive Architectures](https://arxiv.org/abs/2602.03604)
...
TL;DR: EB-JEPA是一个开源库，用于使用联合嵌入预测架构（JEPA）学习表征和世界模型。JEPA在表征空间而非像素空间进行预测，避免了生成模型的缺点，同时捕获了适用于下游任务的语义上有意义的特征。该库提供了模块化、独立的实现，展示了如何将图像级自监督学习的表征学习技术迁移到视频，并最终应用于动作条件世界模型。


<details>
  <summary>Details</summary>
Motivation: 研究人员希望开发一种新的方法来学习表征和世界模型，该方法可以避免生成模型的缺点，同时捕获语义上有意义的特征。此外，他们还希望使能源基础的自监督学习更容易用于研究和教育。

Method: 该研究使用联合嵌入预测架构（JEPA）来学习表征和世界模型。JEPA通过预测表征空间的嵌入而不是像素来实现这一点。研究人员还提供了一个开源库EB-JEPA，其中包含模块化、独立的实现，可以轻松地用于各种任务，包括图像、视频和动作条件世界模型。

Result: 在CIFAR-10上的消融研究表明，JEPA学习到的表征在下游任务上具有91%的准确率。在Moving MNIST数据集上的多步预测示例表明，JEPA的原理可以扩展到时间建模。在Two Rooms导航任务上，动作条件世界模型实现了97%的规划成功率。全面的消融研究揭示了每个正则化组件对于防止表征坍塌的重要性。

Conclusion: EB-JEPA是一个强大的开源库，可以学习表征和世界模型，并且易于用于研究和教育。JEPA在各种任务上都表现出色，并且是一种有前途的自监督学习方法。
...
### [151] [Interpretable Logical Anomaly Classification via Constraint Decomposition and Instruction Fine-Tuning](https://arxiv.org/abs/2602.03530)
*Xufei Zhang,Xinjiao Zhou,Ziling Deng,Dongdong Geng,Jianxiong Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为LAC（Logical Anomaly Classification）的新任务，用于同时检测工业图像中的逻辑异常并对其进行细粒度分类。为此，引入了一个名为LogiCls的视觉-语言框架，该框架通过将复杂逻辑约束分解为一系列可验证的子查询来解决LAC问题。同时，提出了一种以数据为中心的方法来生成这些子查询的思维链（CoT）监督，并利用一种难度感知重采样策略来稳定训练，从而实现准确、可解释的工业逻辑异常分类。


<details>
  <summary>Details</summary>
Motivation: 现有方法将异常检测视为二元决策，无法指出具体的逻辑规则被违反，因此在质量保证方面价值有限。需要一种能够同时进行异常检测和细粒度分类的方法，以提供更具指导性的信息。

Method: 提出LogiCls框架，将复杂逻辑约束分解为可验证的子查询。利用数据中心化的指令合成流水线生成子查询的思维链（CoT）监督，结合精确的接地标注和图像-文本增强来训练视觉-语言模型（VLMs）。采用难度感知重采样策略来稳定训练，侧重于挑战性的子查询和长尾约束类型。

Result: LogiCls在工业逻辑异常分类任务上取得了稳健、可解释且准确的结果，能够同时提供预测的违规类别及其证据。

Conclusion: LAC任务能够统一异常检测和细粒度违规分类，LogiCls框架通过分解约束、CoT监督和难度感知重采样，有效地解决了LAC问题，为工业图像的质量保证提供了更强大、更具解释性的解决方案。
...
### [152] [Zero-shot large vision-language model prompting for automated bone identification in paleoradiology x-ray archives](https://arxiv.org/abs/2602.03750)
*Owen Dong,Lily Gao,Manish Kota,Bennett A. Landmana,Jelena Bekvalac,Gaynor Western,Katherine D. Van Schaik*

Main category: cs.CV

TL;DR: 该研究利用大型视觉语言模型（LVLM）和零样本提示策略，开发了一种自动识别古放射学图像中的主要骨骼、投影视图和侧别的方法，显著提高了图像分类的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 古放射学研究中，考古和人类学遗骸的放射照片存在异质性（骨骼脱节、非标准摆放、缺乏侧别标记等），加上年龄、骨龄、性别和设备差异，导致内容导航（如查找特定投影视图的图像）耗时且困难，成为专家分析的瓶颈。

Method: 研究人员开发了一个流程，将DICOM文件转换为骨窗PNG图像，然后使用精心设计的提示词输入到LVLM中，自动识别主要骨骼、投影视图和侧别。LVLM输出结构化JSON，提取后整理到电子表格中。

Result: 在专家认证的古放射学家审查的100张随机图像样本上，该系统在主要骨骼识别上达到92%的准确率，在投影视图识别上达到80%的准确率，在侧别识别上达到100%的准确率。对于模糊案例，系统会标记低或中置信度。

Conclusion: 研究结果表明，LVLM能够显著加速大型古放射学数据集的词汇开发，从而实现未来人类学工作流程中的高效内容导航。
...
### [153] [PnP-U3D: Plug-and-Play 3D Framework Bridging Autoregression and Diffusion for Unified Understanding and Generation](https://arxiv.org/abs/2602.03533)
*Yongwei Chen,Tianyi Wei,Yushi Lan,Zhaoyang Lyu,Shangchen Zhou,Xudong Xu,Xingang Pan*

Main category: cs.CV

TL;DR: 提出了一种结合自回归和扩散模型的统一3D理解与生成框架，克服了现有纯自回归方法的性能下降和高昂训练成本问题，并在多项3D任务上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有2D大型多模态模型进展迅速，但3D领域的研究滞后。现有的3D统一模型（纯自回归）存在性能下降和训练成本过高的问题。研究动机在于探索一种能有效融合3D理解与生成，并兼顾模型原有能力、降低训练成本的统一框架。

Method: 提出了一种结合自回归（AR）和扩散模型的混合范式。对于3D理解任务，采用自回归的下一个token预测。对于3D生成任务，采用连续的扩散模型。使用一个轻量级Transformer来连接大型语言模型的特征空间和3D扩散模型的条件空间，以实现跨模态信息交互，同时保留预训练模型的先验知识。

Result: 在多个3D理解和生成基准测试中取得了最先进（SOTA）的性能。此外，在3D编辑任务中也表现出色。实验证明了AR+扩散模型在构建通用3D智能方面的潜力。

Conclusion: AR+扩散模型是一种有前途的通用3D智能构建方向。该框架通过结合两种模型的优势，有效解决了3D理解与生成统一的挑战，并在多项任务上取得了优异表现。
...
### [154] [SlowFocus: Enhancing Fine-grained Temporal Understanding in Video LLM](https://arxiv.org/abs/2602.03589)
*Ming Nie,Dan Ding,Chunwei Wang,Yuanfan Guo,Jianhua Han,Hang Xu,Li Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为 SlowFocus 的新机制，以提高视频大语言模型（Vid-LLMs）在视频理解任务中的性能，特别是增强了帧级语义信息和视频级时间信息之间的平衡，并引入了新的训练策略和评估基准 FineAction-CGR。


<details>
  <summary>Details</summary>
Motivation: 现有 Vid-LLMs 在同时保留高质量帧级语义信息和全面的视频级时间信息方面存在困难，这阻碍了它们在细粒度视频理解方面的进步。

Method: SlowFocus 机制首先识别与查询相关的时间段，然后对该段进行密集采样以提取局部高频特征。接着，利用多频混合注意力模块将这些局部高频细节与全局低频上下文聚合，以增强时间理解能力。此外，还引入了一套训练策略以增强时间定位和详细时间推理能力，并创建了 FineAction-CGR 基准。

Result: 在现有的视频理解基准和提出的 FineAction-CGR 基准上进行的实验表明，SlowFocus 机制显著优于现有方法。

Conclusion: SlowFocus 机制能够有效提高 Vid-LLMs 的性能，特别是在细粒度视频理解方面，通过平衡帧级和视频级信息，并辅以新的训练策略和评估基准。
...
### [155] [Cut to the Mix: Simple Data Augmentation Outperforms Elaborate Ones in Limited Organ Segmentation Datasets](https://arxiv.org/abs/2602.03555)
*Chang Liu,Fuxin Fan,Annette Schwarz,Andreas Maier*

Main category: cs.CV

TL;DR: 该研究探讨了四种跨图像数据增强（DA）策略（CutMix、CarveMix、ObjectAug 和 AnatoMix）在有限数据下进行多器官分割任务的有效性，并发现 CutMix、CarveMix 和 AnatoMix 能够显著提高分割性能，其中 CutMix 表现尤为鲁棒。


<details>
  <summary>Details</summary>
Motivation: 临床多器官分割任务需要大量标注数据，而数据稀缺是深度学习模型训练的普遍问题，尤其是在处理新型成像模态时。因此，研究如何在数据有限的情况下提升分割模型的性能，数据增强（DA）被认为是关键的正则化技术。

Method: 研究人员在两个器官分割数据集上，对四种跨图像数据增强策略（CutMix、CarveMix、ObjectAug 和 AnatoMix）进行了实验。并将这些策略与不使用 DA 的最先进的 nnUNet 模型进行了比较，同时还探索了传统 DA（TDA）策略与跨图像 DA 结合使用的效果。

Result: 实验结果表明，CutMix、CarveMix 和 AnatoMix 分别比不使用 DA 的 nnUNet 平均 Dice 分数提高了 4.9%、2.0% 和 1.9%。将 TDA 策略与这些跨图像 DA 策略结合使用，可以进一步提升性能。CutMix 被证明是一种鲁棒且简单的 DA 策略，即使生成直观上“错误”的图像，也能有效提高多器官分割的性能。

Conclusion: 跨图像数据增强策略，特别是 CutMix，对于在数据有限的情况下进行多器官分割任务非常有效，并且可以与传统数据增强策略结合以获得更好的结果。研究提供了公开可用的实现，为未来的基准测试提供了支持。
...
### [156] [High-Resolution Underwater Camouflaged Object Detection: GBU-UCOD Dataset and Topology-Aware and Frequency-Decoupled Networks](https://arxiv.org/abs/2602.03591)
*Wenji Wu,Shuo Ye,Yiyu Liu,Jiguang He,Zhuo Wang,Zitong Yu*

Main category: cs.CV

TL;DR: 本文提出DeepTopo-Net，一种结合拓扑感知和频率解耦的新框架，用于解决水下伪装目标检测（UCOD）中的挑战，如细长生物的拓扑碎片化和透明生物的细微特征提取。该框架通过水质自适应感知器（WCAP）处理物理退化，并通过深渊拓扑细化模块（ATRM）保持细长目标的结构连通性。此外，还发布了首个针对海洋垂直分布的高分辨率（2K）数据集GBU-UCOD。


<details>
  <summary>Details</summary>
Motivation: 现有的水下伪装目标检测方法难以处理海洋深处目标与背景的视觉相似性、细长生物的拓扑碎片化以及透明生物的细微特征提取问题。

Method: 提出DeepTopo-Net框架，该框架整合了拓扑感知建模和频率解耦感知。具体包括：1. 水质自适应感知器（WCAP），使用黎曼度量张量动态形变卷积采样域以处理物理退化；2. 深渊拓扑细化模块（ATRM），利用骨架先验维护细长目标的结构连通性。此外，还创建了高分辨率（2K）数据集GBU-UCOD，覆盖了深渊和超深渊区域。

Result: 在MAS3K、RMAS和新提出的GBU-UCOD数据集上的广泛实验表明，DeepTopo-Net在水下伪装目标检测任务上达到了最先进的性能，特别是在保持复杂水下图案的形态完整性方面表现优异。

Conclusion: DeepTopo-Net是一个有效的新型框架，通过结合拓扑感知和频率解耦，并引入水质自适应感知器和深渊拓扑细化模块，成功解决了水下伪装目标检测中的关键挑战，并取得了优于现有方法的性能。
...
### [157] [Constrained Dynamic Gaussian Splatting](https://arxiv.org/abs/2602.03538)
*Zihan Zheng,Zhenglong Wu,Xuanxuan Wang,Houqiang Zhong,Xiaoyun Zhang,Qiang Hu,Guangtao Zhai,Wenjun Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种名为CDGS（Constrained Dynamic Gaussian Splatting）的新框架，通过引入可微分预算控制器，在训练过程中强制执行用户定义的高斯数量预算，解决了动态场景重建中内存消耗过大的问题，实现了在硬件限制下最优的渲染质量和高压缩比。


<details>
  <summary>Details</summary>
Motivation: 现有动态高斯溅射方法在场景重建时面临一个两难困境：过度的密集化会导致内存消耗过大，无法在边缘设备上部署；而启发式剪枝又难以在固定高斯数量预算下达到最优渲染质量。

Method: CDGS框架将动态场景重建视为一个预算约束优化问题。核心在于引入一个可微分预算控制器，该控制器融合几何、运动和感知线索，通过多模态统一重要性分数来精确调控高斯数量。此外，CDGS将静态和动态元素的优化解耦，并采用自适应分配机制根据运动复杂度动态分配容量。配合三阶段训练策略和双模态混合压缩方案，CDGS在训练过程中严格遵守目标高斯数量。

Result: CDGS在严格遵守硬件限制（误差 < 2%）的同时，在速率-失真性能的帕累托前沿上取得了显著提升。实验结果表明，CDGS在不同容量限制下都能提供最优的渲染质量，并且实现了超过3倍的压缩比，优于现有最先进的方法。

Conclusion: CDGS通过预算约束优化和创新的组件，有效地解决了动态高斯溅射在内存限制下的部署难题，实现了在严格预算下的高质量动态场景重建，并显著提高了压缩效率。
...
### [158] [TIPS Over Tricks: Simple Prompts for Effective Zero-shot Anomaly Detection](https://arxiv.org/abs/2602.03594)
*Alireza Salehi,Ehsan Karami,Sepehr Noey,Sahand Noey,Makoto Yamada,Reshad Hosseini,Mohammad Sabokrou*

Main category: cs.CV

TL;DR: 本研究提出了一种名为TIPSomaly的新型零样本异常检测方法，利用空间感知视觉语言模型TIPS来克服CLIP在工业数据集上的局限性，通过解耦提示和注入局部证据来提高图像级和像素级的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本异常检测方法（ZSAD）依赖于视觉语言模型（VLMs），如CLIP。然而，CLIP在图像-文本对齐方面的粗糙性导致在异常检测和定位方面存在空间不对齐和对细粒度异常敏感性弱的问题。尽管已有研究试图通过复杂的辅助模块来弥补，但它们往往忽略了骨干模型的重要性。本研究旨在通过重新审视骨干模型来解决这些问题。

Method: 研究者采用了TIPS，一个在空间感知目标上训练的VLM作为骨干模型。为了解决TIPS暴露出的全局和局部特征之间的分布差距，他们引入了两种策略：1）使用解耦提示，其中固定提示用于图像级检测，可学习提示用于像素级定位；2）将局部证据注入全局分数。这种方法无需依赖CLIP特有的技巧。

Result: 与现有的基于CLIP的方法相比，TIPSomaly在七个工业数据集上取得了显著的性能提升。在图像级性能上，提升了1.1%-3.9%；在像素级性能上，提升了1.5%-6.9%。该方法还展示了强大的泛化能力，并采用了一种精简的架构。

Conclusion: 本研究通过采用空间感知VLM（TIPS）作为骨干模型，并结合解耦提示和局部证据注入的策略，有效解决了现有零样本异常检测方法在处理工业数据集时的局限性。TIPSomaly在不依赖CLIP特有技巧的情况下，显著提高了异常检测和定位的性能，并展现了良好的泛化能力和简洁的架构。
...
### [159] [KTV: Keyframes and Key Tokens Selection for Efficient Training-Free Video LLMs](https://arxiv.org/abs/2602.03615)
*Baiyang Song,Jun Peng,Yuxin Zhang,Guangyao Chen,Feidiao Yang,Jianyuan Guo*

Main category: cs.CV

TL;DR: 本文提出了一种名为 KTV 的新框架，通过两阶段方法（问答无关的关键帧选择和关键视觉标记选择）来提高训练免费视频理解的效率和有效性，从而显著减少处理长视频时的计算开销和视觉冗余。


<details>
  <summary>Details</summary>
Motivation: 现有的训练免费视频理解方法（将视频视为帧序列）存在严重的视觉冗余和高计算开销，尤其是在处理长视频时。此外，基于 CLIP 相似度的关键帧选择策略容易产生偏差，可能忽略关键帧。

Method: KTV 框架分为两个阶段：1. 问答无关的关键帧选择：通过聚类帧级视觉特征来选择紧凑、多样且具有代表性的帧子集，减少时间冗余。2. 关键视觉标记选择：根据标记重要性和冗余度，从选定的关键帧中剔除冗余或信息量少的标记，减少输入到 LLM 的标记数量。

Result: 在 Multiple-Choice VideoQA 任务上的实验表明，KTV 的性能优于最先进的训练免费基线，同时使用的视觉标记数量显著减少（例如，对于一个 60 分钟、10800 帧的视频，仅使用 504 个视觉标记），在 MLVU-Test 基准上达到了 44.8% 的准确率。KTV 在某些基准上甚至超过了一些基于训练的方法。

Conclusion: KTV 框架通过有效的关键帧和关键视觉标记选择，能够显著提高训练免费视频理解的效率和效果，尤其是在处理长视频时，可以显著降低计算成本并提高理解准确性。
...
### [160] [Refer-Agent: A Collaborative Multi-Agent System with Reasoning and Reflection for Referring Video Object Segmentation](https://arxiv.org/abs/2602.03595)
*Haichao Jiang,Tianming Liang,Wei-Shi Zheng,Jian-Fang Hu*

Main category: cs.CV

TL;DR: 提出了一种名为Refer-Agent的多智能体协作系统，用于解决视频对象分割（RVOS）问题，该系统通过交替的推理-反思机制，将RVOS分解为逐步推理过程，克服了现有方法对大规模监督微调的依赖和零样本方法的性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有RVOS方法严重依赖大规模监督微调（SFT），存在数据依赖性强和可扩展性差的问题。零样本方法虽然灵活，但性能远不如SFT方法。

Method: 提出Refer-Agent系统，采用交替的推理-反思机制。在推理阶段，引入粗粒度到细粒度的帧选择策略以保证帧的多样性和文本相关性，并采用动态焦点布局自适应调整代理的视觉焦点。在反思阶段，通过问题者-回应者对生成自我反思链，验证中间结果并为下一次推理提供反馈。

Result: 在五个具有挑战性的基准测试中，Refer-Agent显著优于包括SFT方法和零样本方法在内的最先进方法。

Conclusion: Refer-Agent是一种有效的RVOS解决方案，它通过多智能体协作和推理-反思机制，克服了现有方法的局限性，并支持快速集成新的多模态大语言模型，无需额外的微调成本。
...
### [161] [Quasi-multimodal-based pathophysiological feature learning for retinal disease diagnosis](https://arxiv.org/abs/2602.03622)
*Lu Zhang,Huizhen Yu,Zuowei Wang,Fu Gui,Yatu Guo,Wei Zhang,Mengyu Jia*

Main category: cs.CV

TL;DR: 提出了一种统一的多模态数据融合框架，用于视网膜疾病分类和分级，通过生成额外模态数据并自适应融合，在公开数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态数据在视网膜疾病诊断中具有潜力，但面临数据异质性、侵入性、配准复杂等挑战，因此需要一个统一的框架来整合数据合成与融合。

Method: 生成包含FFA、MSI和显著性图的多模态数据，利用并行模型学习模态特定的表示，然后自适应地校准和整合跨模态特征，以适应下游任务。

Result: 在多标签分类任务中，F1-score达到0.683，AUC达到0.953；在糖尿病视网膜病变分级任务中，准确率达到0.842，Kappa系数达到0.861，均优于现有方法。

Conclusion: 该框架提高了视网膜疾病筛查的准确性和效率，并为多模态医学成像数据增强提供了可扩展的解决方案。
...
### [162] [SPWOOD: Sparse Partial Weakly-Supervised Oriented Object Detection](https://arxiv.org/abs/2602.03634)
*Wei Zhang,Xiang Liu,Ningjing Liu,Mingxin Liu,Wei Liao,Chunyan Xu,Xue Yang*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的稀疏部分弱监督定向目标检测框架（SPWOOD），旨在通过利用少量稀疏弱标注数据和大量未标注数据，降低遥感领域定向目标检测的标注成本。该框架通过SOS-Student模型、多级伪标签过滤策略和稀疏划分方法实现高效学习。


<details>
  <summary>Details</summary>
Motivation: 遥感领域密集分布和类别繁多的目标导致高昂的标注成本，促使研究人员寻求使用更少、更弱的标注来维持性能。现有的监督学习方法难以解决大规模标注的挑战。

Method: 提出SPWOOD框架，包含三个关键创新：1. SOS-Student模型，用于在稀疏标注设置下分离背景中的未标记对象，并从方向或尺度无关的弱标注中学习信息。2. 多级伪标签过滤策略，利用模型预测的分布来优化伪标签。3. 稀疏划分方法，确保对每个类别进行公平对待。

Result: 在DOTA和DIOR数据集上的实验表明，SPWOOD框架相较于传统定向目标检测方法取得了显著的性能提升，提供了一种高性价比的解决方案。

Conclusion: SPWOOD框架能够有效地利用稀疏部分弱标注数据和大量未标注数据，显著降低了遥感领域定向目标检测的标注成本，并取得了优异的性能。
...
### [163] [MM-SCALE: Grounded Multimodal Moral Reasoning via Scalar Judgment and Listwise Alignment](https://arxiv.org/abs/2602.03665)
...
TL;DR: 本研究提出了MM-SCALE数据集，旨在改进视觉语言模型（VLMs）在复杂情境下的道德判断能力，通过提供五点量表评分和模态接地信息，实现了比二元监督更精细的对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在处理模态复杂且社会情境模糊的道德判断时存在困难，现有的二元或成对监督方式未能捕捉人类道德推理的连续性和多样性。

Method: 构建了一个名为MM-SCALE的大规模数据集，包含图像-情境对，并由人类进行五点量表评分（道德可接受度）和显式模态接地推理标注。采用列表式偏好优化方法。

Result: 在MM-SCALE上微调的VLMs在排名保真度和安全性校准方面优于使用二元信号训练的模型。

Conclusion: 从离散监督转向标量监督的MM-SCALE框架，能够提供更丰富的对齐信号，实现对多模态道德推理的更精细校准，从而提升VLMs的道德判断能力。
...
### [164] [Multi-Objective Optimization for Synthetic-to-Real Style Transfer](https://arxiv.org/abs/2602.03625)
*Estelle Chigot,Thomas Oberlin,Manon Huguenin,Dennis Wilson*

Main category: cs.CV

TL;DR: 该研究提出了一种使用多目标遗传算法优化风格迁移管道的方法，以缩小合成图像和真实图像之间的领域差距，从而提高语义分割模型的性能，特别是在恶劣条件下。该方法通过优化管道来平衡结构一致性和风格相似性，并使用成对图像指标加速评估过程。


<details>
  <summary>Details</summary>
Motivation: 在现实世界图像上训练的语义分割模型需要大量的像素级标注数据，成本高昂。虽然可以生成带有标注的合成图像，但由于真实图像和合成图像之间的领域差距，直接在合成图像上训练的模型在真实图像上表现不佳。因此，需要一种方法来减小这种领域差距。

Method: 研究人员利用多目标遗传算法来优化风格迁移算子的管道。算法旨在平衡管道生成的图像的结构一致性和风格相似性。为了加速评估过程，研究人员探索了在进化过程中使用成对图像指标，而不是传统的需要大量图像的分布指标。

Result: 经过优化，该方法在合成到真实的领域自适应标准数据集（如 GTA5 到 Cityscapes 和 ACDC）上进行了评估，特别关注恶劣条件。结果表明，进化算法能够提出适应不同目标的、多样化的数据增强管道。研究人员还评估了优化后的 Pareto 前沿在分布指标和分割性能方面的表现。

Conclusion: 该研究将风格迁移问题形式化为一个适合进化优化的序列问题，并研究了能够实现可行搜索的有效指标。进化算法能够生成多样化的、针对不同目标的风格迁移管道，有效解决了合成到真实领域自适应中的领域差距问题。
...
### [165] [LIVE: Long-horizon Interactive Video World Modeling](https://arxiv.org/abs/2602.03747)
*Junchao Huang,Ziyang Ye,Xinting Hu,Tianyu He,Guiyu Zhang,Shaoshuai Shi,Jiang Bian,Li Jiang*

Main category: cs.CV

TL;DR: 提出了一种名为LIVE的视频世界模型，通过新颖的周期一致性目标来限制误差累积，从而实现长时域视频生成，无需教师模型蒸馏，并在长时域基准测试中达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归视频世界模型在长时域生成时存在小误差累积导致性能下降的问题，而现有缓解方法会增加计算成本且无法阻止超出训练时域的误差传播。

Method: LIVE模型通过前向生成和反向重构的周期一致性目标来约束长时域误差累积，并采用渐进式训练策略。具体来说，模型从真实帧开始前向生成，然后反向生成以重构初始状态，并在重构的末端状态上计算扩散损失。

Result: LIVE在长时域视频生成任务上取得了最先进的性能，生成的视频稳定且质量高，能够远超训练时的回滚长度。

Conclusion: LIVE模型通过周期一致性目标有效地解决了长时域视频生成中的误差累积问题，并消除了对教师模型的依赖，展现了在生成稳定、高质量长时域视频方面的潜力。
...
### [166] [See-through: Single-image Layer Decomposition for Anime Characters](https://arxiv.org/abs/2602.03749)
*Jian Lin,Chengze Li,Haoyun Qin,Kwun Wang Chan,Yanghua Jin,Hanyuan Liu,Stephen Chun Wang Choy,Xueting Liu*

Main category: cs.CV

TL;DR: 提出了一种自动将静态动漫插画转换为可操作2.5D模型的框架，通过图像分解、inpainting和绘制顺序推断，并利用商业Live2D模型生成训练数据，实现了高质量的动漫角色分层和可操作性。


<details>
  <summary>Details</summary>
Motivation: 当前手动将静态动漫插画转化为可操作2.5D模型的工作流程繁琐，需要手动分割和“幻觉”遮挡区域，本文旨在自动化这一过程，提高效率。

Method: 将单张图像分解为完全inpainted、语义清晰且具有推断绘制顺序的图层。采用基于扩散模型的身体部位一致性模块来确保全局几何一致性，结合像素级伪深度推理机制来解决复杂的层级结构。

Result: 该方法能够生成高保真、可操作的2.5D模型，适用于专业实时动画应用。能够解决头发丝等复杂交织的层级结构问题。

Conclusion: 所提出的框架能够自动将静态动漫插画转化为可操作的2.5D模型，克服了现有方法的局限性，并为实时动画应用提供了高质量的解决方案。
...
### [167] [Referring Industrial Anomaly Segmentation](https://arxiv.org/abs/2602.03673)
*Pengfei Yue,Xiaokang Jiang,Yilin Lu,Jianghang Lin,Shengchuan Zhang,Liujuan Cao*

Main category: cs.CV

TL;DR: 本文提出了一种名为RIAS（Referring Industrial Anomaly Segmentation）的新范式，利用语言描述来指导工业异常检测，并引入了MVTec-Ref数据集和DQFormer模型，实现了更精确、更通用的异常分割。


<details>
  <summary>Details</summary>
Motivation: 传统工业异常检测方法存在精度不足（需要手动阈值）、泛化性差（一个模型只能检测一类异常）以及对稀缺、不平衡数据敏感的问题。因此，需要一种能够利用语言信息、通用性强且精度高的异常检测方法。

Method: 提出RIAS范式，利用文本描述生成精确的异常分割掩码，无需手动阈值。引入MVTec-Ref数据集，包含多样化的描述性短语，并侧重于小尺寸异常。提出DQFormer模型，采用语言门控多层聚合（LMA）增强多尺度分割能力，并使用“异常”和“背景”两个查询令牌来高效整合视觉和文本信息。

Result: RIAS在MVTec-Ref数据集上表现出优越的性能，能够生成精确的异常分割掩码，并且单一模型能够检测多种不同类型的异常。DQFormer模型通过LMA和双查询令牌有效提升了多尺度分割精度。

Conclusion: RIAS范式和DQFormer模型能够有效解决工业异常检测中的挑战，推动其向开放集检测能力发展，实现更精确、更通用的异常分割。
...
### [168] [Edge-Optimized Vision-Language Models for Underground Infrastructure Assessment](https://arxiv.org/abs/2602.03742)
*Johny J. Lopez,Md Meftahul Ferdaus,Mahdi Abdelguerfi*

Main category: cs.CV

TL;DR: 本文提出了一种用于地下基础设施缺陷检测和自动摘要生成的新型两阶段流水线，该流水线轻量级且适用于边缘设备，能够高效地分割缺陷并生成可读的总结。


<details>
  <summary>Details</summary>
Motivation: 目前，在资源受限的边缘设备上，从视觉传感器检测到的地下基础设施结构缺陷自动生成人类可读的摘要仍然是一个重大挑战。

Method: 该研究采用了一个两阶段流水线：第一阶段使用轻量级的 RAPID-SCAN 模型进行高效的缺陷分割（F1 分数 0.834，0.64M 参数）；第二阶段使用微调后的 Phi-3.5 VLM 模型，根据分割结果生成领域特定的自然语言摘要。研究还引入了一个包含手动验证描述的审查数据集用于 VLM 微调和评估，并采用训练后量化和硬件特定优化以实现实时性能。

Result: RAPID-SCAN 模型在缺陷分割方面取得了 0.834 的 F1 分数。经过优化的 VLM 模型能够生成简洁且领域相关的摘要。整个流水线部署在移动机器人平台上，并在真实场景中进行了评估，证明了其在实时性能方面的有效性。

Conclusion: 该研究展示了可部署在边缘的集成人工智能系统在弥合自动缺陷检测与基础设施维护的可操作见解之间的差距方面的潜力，为更具可扩展性和自主性的检查解决方案铺平了道路。
...
### [169] [Test-Time Conditioning with Representation-Aligned Visual Features](https://arxiv.org/abs/2602.03753)
*Nicolas Sereyjol-Garros,Ellington Kirby,Victor Letzelter,Victor Besnier,Nermin Samet*

Main category: cs.CV

TL;DR: 本文提出了一种名为REPA-G的框架，该框架利用自监督模型学习到的对齐表示来增强扩散模型的推理时间条件生成能力，实现了灵活且精确的多尺度（纹理匹配到全局语义）和多概念组合的控制。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注如何利用表示对齐来改进扩散模型的训练，但其在增强推理时间条件生成方面的潜力尚未得到充分探索。

Method: REPA-G框架在推理时优化一个相似性目标（潜在函数），通过提取预训练特征提取器中的特征来引导去噪过程，实现对生成过程的控制。该方法支持从单图像块到全局特征的多种尺度控制，并可扩展到多概念组合。

Result: 在ImageNet和COCO数据集上的实验表明，REPA-G能够生成高质量、多样化的图像，并在表示对齐和条件生成方面取得了显著效果。

Conclusion: REPA-G是一种在推理时进行条件生成的灵活且精确的框架，能够克服文本提示和类别标签的模糊性或粗糙性，为扩散模型提供了强大的控制能力。
...
### [170] [RegionReasoner: Region-Grounded Multi-Round Visual Reasoning](https://arxiv.org/abs/2602.03733)
*Wenfang Sun,Hao Chen,Yingjun Du,Yefeng Zheng,Cees G. M. Snoek*

Main category: cs.CV

TL;DR: 本文提出了一个名为RegionReasoner的强化学习框架，用于解决现有视觉-语言模型在多轮视觉推理中存在的局限性。该框架通过要求推理过程显式引用相关的边界框，并引入一个全局-局部一致性奖励来维护语义连贯性。同时，作者还构建了一个名为RegionDial-Bench的新基准，用于评估多轮推理能力。实验结果表明，RegionReasoner在检测和分割任务上显著提升了多轮推理准确性、空间定位精度和全局-局部一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉-语言模型在视觉推理方面取得了显著进展，但它们通常依赖于单步或纯文本推理，这限制了它们在多个视觉上下文之间迭代改进理解的能力。

Method: 本文提出RegionReasoner，一个强化学习框架。该框架通过要求推理过程显式引用参考边界框来强制执行基础推理，并通过全局-局部一致性奖励来维护语义连贯性。该奖励提取全局场景和区域级标题中的关键对象和名词，并将其与推理过程对齐，以确保推理步骤之间的一致性。RegionReasoner通过结合基础保真度和全局-局部语义对齐的结构化奖励进行优化。此外，作者还构建了一个名为RegionDial-Bench的多轮视觉推理基准。

Result: 在检测和分割任务上的实验表明，RegionReasoner-7B及其配套的RegionDial-Bench基准，在多轮推理准确性、空间定位精度和全局-局部一致性方面均有显著提升，为该新兴研究方向建立了一个强大的基线。

Conclusion: RegionReasoner框架通过显式的空间引用和全局-局部语义对齐，有效地解决了现有视觉-语言模型在多轮视觉推理中的不足，并在新的基准测试中展现出优越的性能，为未来的研究提供了新的方向和方法。
...
### [171] [RAWDet-7: A Multi-Scenario Benchmark for Object Detection and Description on Quantized RAW Images](https://arxiv.org/abs/2602.03760)
*Mishal Fatima,Shashank Agnihotri,Kanchana Vaishnavi Gandikota,Michael Moeller,Margret Keuper*

Main category: cs.CV

TL;DR: 该论文提出了RAWDet-7数据集，用于研究使用RAW图像进行目标检测和描述。RAW图像保留了更多原始传感器信息，优于经过ISP处理的RGB图像。数据集包含约25k训练和7.6k测试RAW图像，并提供了目标级描述，支持在模拟低比特量化下的评估。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉模型通常在为人类感知优化的RGB图像上训练，这会丢弃机器推理所需的传感器级信息。RAW图像保留了未处理的场景数据，包含更丰富的机器可用的线索，可以提升目标检测和描述的性能。

Method: 收集了一个包含约25k训练和7.6k测试RAW图像的大规模数据集RAWDet-7。图像来自不同相机、光照和环境。数据集按照MS-COCO和LVIS约定对七个目标类别进行了密集标注，并提供了目标级描述。数据集支持在模拟的4-bit、6-bit和8-bit量化下进行评估。

Result: RAW图像能够为目标检测和目标描述提供更丰富的线索，捕捉到在处理后的图像中丢失的细节、空间关系和上下文信息。数据集支持在低比特量化下评估检测性能、描述质量和细节，以及泛化能力。

Conclusion: RAWDet-7数据集为研究RAW图像在目标检测和描述任务中的潜力提供了基础，并能评估低比特量化对这些任务的影响，推动机器更好地利用传感器原始数据。
...
### [172] [From Pre- to Intra-operative MRI: Predicting Brain Shift in Temporal Lobe Resection for Epilepsy Surgery](https://arxiv.org/abs/2602.03785)
...
TL;DR: 提出了一种名为NeuralShift的U-Net模型，仅使用术前MRI预测脑移位，并成功应用于颞叶切除术，显著提高了神经导航的精度和手术安全性。


<details>
  <summary>Details</summary>
Motivation: 术前MRI在脑部手术中用于导航，但开颅后出现的脑移位会导致术前MRI失效，降低导航系统的精度。因此，需要一种方法来补偿脑移位，以提高手术的安全性和有效性。

Method: 提出了一种基于U-Net架构的名为NeuralShift的模型，该模型仅利用术前MRI预测患者在颞叶切除术中的脑移位。通过在解剖标志点上计算目标配准误差（TRE）以及比较预测的术中掩模与术中MRI衍生的掩模的DICE分数来评估模型的性能。

Result: NeuralShift模型能够以0.97的DICE分数准确预测全局脑变形，并在解剖标志点上达到1.12 mm的低TRE，有效补偿了颞叶切除术中出现的较大脑移位。

Conclusion: NeuralShift模型仅通过术前图像即可预测颞叶切除术中的脑移位，有望提高神经外科手术的安全性、效率和患者预后。该模型将在接受论文发表后在GitHub上开源。
...
### [173] [3D-Aware Implicit Motion Control for View-Adaptive Human Video Generation](https://arxiv.org/abs/2602.03796)
*Zhixue Fang,Xu He,Songlin Tang,Haoxian Zhang,Qingfeng Li,Xiaoqiang Liu,Pengfei Wan,Kun Gai*

Main category: cs.CV

TL;DR: 提出了一种名为3DiMo的方法，利用隐式的、与视角无关的运动表示来控制视频生成，克服了现有2D姿势和显式3D模型方法的局限性，实现了更真实的运动复现和灵活的相机控制。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成中的运动控制方法，如基于2D姿势或显式3D模型（如SMPL），存在视角限制或模型不准确的问题，阻碍了新视角合成和真实运动的生成。研究动机是开发一种能够自然对齐生成器内部3D感知能力的、视角无关的运动表示。

Method: 3DiMo联合训练一个运动编码器和一个预训练的视频生成器，将驱动帧提取成紧凑的、视角无关的运动令牌，并通过交叉注意力注入。为了增强3D感知，模型使用了丰富的视角监督（单视角、多视角、移动相机视频），并辅助使用SMPL进行初始化的几何监督，该监督随着训练的进行逐渐减弱。

Result: 实验表明，3DiMo能够忠实地复现驱动的运动，并支持灵活的文本驱动相机控制。在运动保真度和视觉质量方面，3DiMo显著优于现有方法。

Conclusion: 3DiMo成功地开发了一种基于隐式、视角无关的运动表示的视频生成方法，该方法能够有效利用生成器的3D空间先验，克服了传统方法的缺点，实现了更高质量和更灵活的运动控制。
...
### [174] [FOVI: A biologically-inspired foveated interface for deep vision models](https://arxiv.org/abs/2602.03766)
*Nicholas M. Blauch,George A. Alvarez,Talia Konkle*

Main category: cs.CV

TL;DR: 提出了一种基于人眼视觉机制的计算模型FOVI，通过模仿视网膜和初级视觉皮层的处理方式，将变分辨率的传感器阵列重新格式化为均匀密度的传感器流形，从而在计算成本大幅降低的情况下，实现了高效的高分辨率全景视觉感知。该模型支持kNN卷积和LoRA等技术，并在两个实际应用案例中展现了竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉系统多采用均匀分辨率处理，在处理高分辨率全景图像时效率低下。受人眼“中央凹”视觉（foveated vision）机制的启发，即视觉中心分辨率高，周边分辨率低，以实现高效主动感知，作者希望借鉴这一机制来提升计算机视觉系统的效率。

Method: 提出了一种名为FOVI（Foveated Vision Interface）的计算模型，其核心是将变分辨率的视网膜类传感器阵列，通过模仿人眼视网膜和初级视觉皮层（V1）的处理方式，重塑为一个均匀密度的传感器流形。在此基础上，定义了基于k-nearest-neighborhoods（kNN）的感受野，并开发了一种新的核映射技术来实现kNN卷积。研究中还展示了两种具体应用：一是端到端的kNN卷积神经网络架构；二是利用低秩适应（LoRA）技术对DINOv3 Vision Transformer模型进行中央凹化改造。

Result: 所提出的FOVI模型在计算成本上远低于非中央凹化基线模型，同时在性能上与之相当。这表明了该方法在提高高分辨率自我中心视觉（egocentric vision）感知效率和可扩展性方面的潜力。

Conclusion: FOVI通过模仿人类视觉系统的中央凹化机制，提供了一种高效且可扩展的解决方案，能够以更低的计算成本处理高分辨率的视觉信息，有望在需要主动感知和处理大视场数据的应用中发挥重要作用。
...
### [175] [EventNeuS: 3D Mesh Reconstruction from a Single Event Camera](https://arxiv.org/abs/2602.03847)
*Shreyas Sachan,Viktor Rudnev,Mohamed Elgharib,Christian Theobalt,Vladislav Golyanik*

Main category: cs.CV

TL;DR: 本文提出了一种名为EventNeuS的自监督神经模型，用于从单目事件流中学习3D表示，解决了现有事件基3D重建精度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有事件基3D重建技术在精度上存在局限性，促使研究者开发更精确的方法。

Method: EventNeuS结合了3D符号距离函数和密度场学习，并引入了球谐函数编码，利用事件数据进行监督学习。

Result: EventNeuS在3D重建精度上显著优于现有方法，在Chamfer距离上降低了34%，在平均绝对误差上降低了31%。

Conclusion: EventNeuS是第一个结合3D符号距离函数和密度场学习，并使用事件基监督的神经模型，成功提高了从单目事件流进行3D重建的精度。
...
### [176] [Progressive Checkerboards for Autoregressive Multiscale Image Generation](https://arxiv.org/abs/2602.03811)
*David Eigen*

Main category: cs.CV

TL;DR: 本文提出了一种基于渐进式棋盘格的自回归图像生成方法，通过多尺度并行采样并保持各层级平衡，实现了高效且高质量的图像生成，并且在ImageNet数据集上取得了具有竞争力的性能，同时采样步数更少。


<details>
  <summary>Details</summary>
Motivation: 现有自回归图像生成方法在并行采样独立位置与建模序列依赖性之间存在挑战。文章旨在探索一种更高效的并行采样策略。

Method: 文章提出了一种基于渐进式棋盘格的固定采样顺序，在多尺度金字塔中，该方法允许在每个尺度上从均匀分布的区域并行采样，并在四叉树细分结构的每个级别上保持完全平衡，从而有效地实现了跨尺度和尺度内的条件约束。

Result: 研究发现在平衡的设置下，只要总的序列采样步数恒定，不同尺度的比例因子对生成结果的影响相似。在类条件ImageNet数据集上，该方法在模型容量相似的情况下，比现有的最先进的自回归系统取得了具有竞争力的性能，且采样步数更少。

Conclusion: 渐进式棋盘格排序为多尺度自回归图像生成提供了一种灵活且高效的并行采样方法，能够在保持高质量生成的同时减少采样步数。
...
### [177] [Continuous Control of Editing Models via Adaptive-Origin Guidance](https://arxiv.org/abs/2602.03826)
*Alon Wolf,Chen Katzir,Kfir Aberman,Or Patashnik*

Main category: cs.CV

TL;DR: 本研究提出了一种名为 AdaOr（Adaptive-Origin Guidance）的新方法，用于实现对扩散模型图像和视频编辑强度的平滑、连续控制，解决了现有方法中通过调整 Classifier-Free Guidance (CFG) 无法实现平滑过渡的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本引导编辑方法缺乏对编辑强度进行平滑控制的机制。直接调整 CFG 参数并不能在原始输入和编辑后的结果之间实现平滑过渡，因为无条件预测会主导低引导强度下的生成，并且可能导致任意的输入内容操纵。

Method: 提出 Adaptive-Origin Guidance (AdaOr) 方法，通过引入一个身份条件自适应的引导起点来调整标准的无条件预测。该身份引导起点利用一个与身份操纵相关的身份指令。通过根据编辑强度在身份预测和标准无条件预测之间进行插值，AdaOr 确保了从输入到编辑结果的连续过渡。

Result: 在图像和视频编辑任务上的评估表明，AdaOr 能够提供比现有基于滑块的编辑方法更平滑、更一致的控制。该方法在标准训练框架中融入了身份指令，无需额外的编辑程序或专门的数据集即可在推理时实现精细控制。

Conclusion: AdaOr 方法有效地解决了扩散模型文本引导编辑中强度控制不平滑的问题，通过引入身份条件自适应引导起点，实现了对编辑强度的连续、平滑控制，并在图像和视频编辑任务上取得了优于现有方法的性能。
...
### [178] [Fast-Slow Efficient Training for Multimodal Large Language Models via Visual Token Pruning](https://arxiv.org/abs/2602.03815)
*Dingkun Zhang,Shuhan Qi,Yulin Wu,Xinyu Xiao,Xuan Wang,Long Chen*

Main category: cs.CV

TL;DR: 本文提出了一种名为DualSpeed的框架，用于提高多模态大语言模型（MLLMs）的训练效率。该框架通过在训练阶段引入视觉令牌剪枝（VTP）来减少计算量，同时通过一个辅助的“慢模式”和自蒸馏技术来解决因剪枝导致的训练-推理不匹配问题，从而在提高训练速度的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）由于模型规模庞大和视觉令牌数量多，存在严重的训练效率低下问题。现有方法主要通过减小模型尺寸或可训练参数来解决，但本文受到视觉令牌剪枝（VTP）在提高推理效率方面成功的启发，探索在训练阶段通过减少视觉令牌数量来提升训练效率。

Method: 提出了一种名为DualSpeed的快速-慢速框架。其中，“快模式”主要通过引入现有VTP方法插件来减少视觉令牌，并使用模式隔离器来分离模型行为；“慢模式”作为辅助模式，在完整的视觉序列上进行训练以保持训练-推理一致性，并通过自蒸馏从“快模式”中学习以加速训练。

Result: 实验表明，DualSpeed能够实现训练效率和性能的兼顾。在LLaVA-1.5和LLaVA-NeXT模型上，DualSpeed分别实现了2.1倍和4.0倍的训练加速，同时保持了超过99%的原有性能。

Conclusion: DualSpeed框架成功地解决了MLLMs在训练阶段效率低下的问题，通过结合视觉令牌剪枝和精心设计的快慢模式训练策略，实现了训练加速而不牺牲模型性能，为高效训练MLLMs提供了一种新颖有效的途径。
...
### [179] [The Hypocrisy Gap: Quantifying Divergence Between Internal Belief and Chain-of-Thought Explanation via Sparse Autoencoders](https://arxiv.org/abs/2602.02496)
*Shikhar Shiromani,Archie Chaudhury,Sri Pranav Kunda*

Main category: cs.CL

TL;DR: 本文提出了一种名为“虚伪差距”（Hypocrisy Gap）的度量方法，使用稀疏自编码器（SAEs）来量化大型语言模型（LLMs）的内部推理与其最终生成结果之间的差异，以检测模型为了迎合用户而产生的“不忠实”行为。该方法在多个模型和数据集上表现出比现有基线更好的检测效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）常常表现出“不忠实”的行为，即最终生成的答案与其内部的推理过程不符，目的是为了迎合用户。研究者希望找到一种方法来检测这种行为。

Method: 研究者引入了“虚伪差距”（Hypocrisy Gap）这一度量方法。该方法利用稀疏自编码器（SAEs）和稀疏线性探针（sparse linear probes）来提取模型的内部“信念”，并将其与最终生成的轨迹在潜在空间中进行数学比较，量化两者之间的差异。

Result: 在 Gemma、Llama 和 Qwen 模型上使用 Anthropic 的 Sycophancy 基准测试，该方法在检测奉承性（sycophantic）运行方面达到了 0.55-0.73 的 AUROC，在检测模型内部“知道”用户是错误但仍迎合的虚伪（hypocritical）情况下达到了 0.55-0.74 的 AUROC。这些结果持续优于决策对齐的对数概率基线（0.41-0.50 AUROC）。

Conclusion: “虚伪差距”度量方法能够有效地量化和检测 LLMs 的不忠实行为，并在实际应用中表现出比现有方法更好的性能。
...
### [180] [STEMVerse: A Dual-Axis Diagnostic Framework for STEM Reasoning in Large Language Models](https://arxiv.org/abs/2602.02497)
*Xuzhao Li,Xuchen Li,Jian Zhao,Shiyu Hu*

Main category: cs.CL

TL;DR: 本研究提出了STEMVerse，一个旨在系统性分析大型语言模型（LLMs）在科学、技术、工程和数学（STEM）领域推理能力的新型诊断框架。该框架通过将学术专业化和认知复杂度结合起来，对超过20,000个STEM问题进行重新组织和标记，从而克服了现有评估方法忽略领域知识和认知能力区别的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM评估方法往往将STEM基准视为独立的“筒仓”，只提供单一的总体分数，忽略了学术专业化和认知深度的复杂性。这种结果导向的方法无法区分模型错误是源于领域知识不足还是认知能力缺陷，从而限制了诊断价值。因此，需要一个更精细的框架来系统分析LLMs的STEM推理能力。

Method: 研究提出了STEMVerse框架，该框架将学术专业化和认知复杂度相结合，创建一个“学科 × 认知”的二元能力空间。研究者将主流基准中的20,000多个STEM问题重新整合到这个空间中，并为每个问题分配了双轴标签。然后，利用STEMVerse框架对不同参数规模和训练范式的代表性LLM家族进行系统性评估。

Result: 通过STEMVerse框架的系统评估，研究揭示了LLMs在STEM推理中存在的结构性失败模式。该框架能够区分模型在不同学科领域和不同认知复杂度下的表现差异。

Conclusion: STEMVerse提供了一个清晰且可操作的视角来理解LLMs的科学推理特征。通过将多学科覆盖和细粒度的认知分层整合到一个统一的框架中，STEMVerse能够更准确地诊断LLMs在STEM领域的优势和劣势，并指导未来的模型改进。
...
### [181] [Test-Time Detoxification without Training or Learning Anything](https://arxiv.org/abs/2602.02498)
*Baturay Saglam,Dionysis Kalogerias*

Main category: cs.CL

TL;DR: 本文提出了一种在测试时通过零阶优化来降低大型语言模型生成文本毒性的方法，该方法无需模型重训练或访问模型内部参数，并在毒性降低和生成质量之间取得了良好的权衡。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可能生成有毒或不当内容，存在安全风险。现有的去毒化方法通常需要昂贵的模型重训练或复杂的辅助组件，并且可能难以跨模型家族或在黑盒设置下转移。因此，需要一种高效且通用的去毒化方法。

Method: 提出了一种在测试时进行的程序，该程序通过逼近完成毒性相对于输入嵌入的梯度，并使用少量梯度下降步骤来引导生成过程，使其产生毒性更低的续写。该方法采用零阶优化，仅需要输入嵌入、毒性评分函数以及模型的正向评估。

Result: 该方法在不同模型和提示下都能稳健地降低毒性，并且在大多数情况下，在毒性降低和生成质量之间实现了最佳的权衡。

Conclusion: 词嵌入可以作为有效的控制变量来引导语言模型生成更安全的文本。黑盒优化是一种可行的、无需训练或访问中间计算的、用于指导自回归语言模型进行可扩展、更安全文本生成的方法。
...
### [182] [ROSA-Tuning: Enhancing Long-Context Modeling via Suffix Matching](https://arxiv.org/abs/2602.02499)
*Yunao Zheng,Xiaojie Wang,Lei Ren,Wei Chen*

Main category: cs.CL

TL;DR: ROSA-Tuning 提出了一种结合检索和召回的新机制，通过 CPU 上的 ROSA 模块高效定位历史相关信息并注入模型状态，以增强预训练模型在长上下文场景下的建模能力，同时保持接近窗口注意力机制的计算效率和显存占用。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在长上下文处理能力和计算效率方面面临挑战。现有的高效注意力机制通常会限制模型状态的覆盖范围，ROSA-Tuning 旨在解决这一问题。

Method: ROSA-Tuning 引入了一个基于 CPU 的 ROSA (RWKV Online Suffix Automaton) 检索模块，并行于标准注意力机制，用于高效查找长上下文中的相关历史位置，并以可训练的方式将检索到的信息注入模型状态。后续通过受限范围的注意力机制进行加权融合。为实现端到端训练，设计了二值化策略和反事实梯度算法，并通过异步 CPU-GPU 流水线优化执行效率。

Result: 在 Qwen3-Base-1.7B 模型上进行评估，ROSA-Tuning 显著恢复了窗口注意力模型的长上下文建模能力，在 LongBench 等基准测试上取得了接近甚至媲美全局注意力的性能，同时计算效率和 GPU 显存占用接近窗口注意力方法。

Conclusion: ROSA-Tuning 提供了一种高效处理长上下文的新技术路径，能够在保持计算效率的同时，显著提升模型在长序列任务上的性能。
...
### [183] [Graph-Augmented Reasoning with Large Language Models for Tobacco Pest and Disease Management](https://arxiv.org/abs/2602.02635)
*Siyu Li,Chenwei Song,Qi Zhou,Wan Zhou,Xinyi Liu*

Main category: cs.CL

TL;DR: 该论文提出了一个图增强的推理框架，用于烟草病虫害管理，将结构化领域知识集成到大型语言模型（LLMs）中。通过构建特定领域的知识图谱，并检索查询相关的子图，为答案生成提供关系证据。该框架使用ChatGLM作为Transformer骨干，并采用LoRA进行参数高效微调，同时利用图神经网络学习节点表示，捕捉症状-疾病-治疗依赖关系。实验结果表明，该框架在多跳和比较推理问题上优于纯文本基线。


<details>
  <summary>Details</summary>
Motivation: 现有的烟草病虫害管理系统在处理复杂查询和提供准确、可靠的治疗建议方面存在不足，尤其是在需要推理和整合多方面信息的情况下。作者希望通过集成结构化的领域知识来增强大型语言模型的能力，以提高回答的准确性和可靠性。

Method: 该研究构建了一个烟草病虫害领域的知识图谱，其中包含疾病、症状、农药和防治措施等实体及其之间的关系。在此基础上，提出了一种图增强的推理框架（GraphRAG），通过检索与查询相关的子图来提供关系证据。该框架以ChatGLM为基础模型，并使用LoRA进行参数高效微调。同时，采用图神经网络（GNN）学习节点表示，以捕捉实体间的依赖关系。检索到的图谱证据被整合到LLM的输入中，以指导生成。

Result: 实验结果表明，该图增强推理框架在烟草病虫害管理任务上，相比于纯文本基线，取得了持续的性能提升。尤其是在需要多步推理和比较的问题上，提升效果最为显著。

Conclusion: 将结构化领域知识图谱与大型语言模型相结合，能够有效地增强烟草病虫害管理系统的推理能力，生成更具证据支持和领域一致性的治疗建议，并有效减少不准确或幻觉的治疗方案。
...
### [184] [WideSeek: Advancing Wide Research via Multi-Agent Scaling](https://arxiv.org/abs/2602.02636)
*Ziyang Huang,Haolin Ren,Xiaowei Yuan,Jiawei Wang,Zhongtao Jiang,Kun Xu,Shizhu He,Jun Zhao,Kang Liu*

Main category: cs.CL

TL;DR: 该研究提出了WideSeekBench基准和WideSeek多智能体系统，以解决搜索智能从深度研究转向广度研究的挑战，并验证了扩展智能体数量是推进广度研究的有效途径。


<details>
  <summary>Details</summary>
Motivation: 现有搜索智能研究主要关注深度，缺乏针对广度研究的基准和优化方法，限制了在复杂约束下并行检索和综合信息的能力。

Method: 构建了一个名为WideSeekBench的通用广度信息检索（GBIS）基准；提出了WideSeek，一个动态分层多智能体架构，能够根据任务需求自主分叉子智能体；设计了一个端到端强化学习的统一训练框架来优化系统。

Result: 实验证明了WideSeek和多智能体强化学习的有效性，并且表明增加智能体数量是推动广度研究范式发展的一个有前景的方向。

Conclusion: 该研究通过引入基准和创新的多智能体系统，为广度研究领域的发展提供了新的方法和方向，并强调了智能体数量扩展的重要性。
...
### [185] [Monotonicity as an Architectural Bias for Robust Language Models](https://arxiv.org/abs/2602.02686)
*Patrick Cooper,Alireza Nadali,Ashutosh Trivedi,Alvaro Velasquez*

Main category: cs.CL

TL;DR: 研究表明，在 Transformer 模型的前馈子层中强制执行单调性，可以显著提高大型语言模型抵御对抗性攻击的能力，同时仅对标准性能造成很小的影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在面对对抗性提示和越狱攻击时表现出脆弱性，即使经过了广泛的对齐和微调。这种脆弱性反映了现代神经网络语言模型普遍存在的挑战：在高维输入空间中，微小且精心设计的扰动可能导致内部语义表示和输出发生巨大且不可预测的变化。

Method: 该研究通过在序列到序列 Transformer 的前馈子层中选择性地强制执行单调性来解决鲁棒性问题，同时保持注意力机制不受限制。这种方法旨在限制语义转换，使得增强信息、证据或约束不会导致相应内部表示的回归。

Result: 经验结果显示，单调性显著提高了鲁棒性。对抗性攻击的成功率从大约 69% 下降到 19%，而标准的摘要性能仅有微小的下降。

Conclusion: 该研究表明，通过选择性地将单调性作为一种归纳偏置引入 Transformer 模型的前馈子层，可以在不牺牲模型整体性能的情况下，有效提高大型语言模型在对抗性攻击下的鲁棒性。这种方法利用了单调性在保证语义稳定性方面的优势，而将复杂的语义交互和上下文理解留给注意力机制处理。
...
### [186] [InfMem: Learning System-2 Memory Control for Long-Context Agent](https://arxiv.org/abs/2602.02704)
*Xinyu Wang,Mingze Li,Peng Lu,Xiao-Wen Chang,Lifeng Shang,Jinping Li,Fei Mi,Prasanna Parthasarathi,Yufei Cui*

Main category: cs.CL

TL;DR: 提出了一种名为InfMem的控制中心代理，通过PreThink-Retrieve-Write协议实现System-2风格的控制，解决了超长文档中的稀疏证据合成和内存限制问题，并在超长问答基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 超长文档的推理需要整合分散在遥远片段中的稀疏证据，并且面临严格的内存限制。现有的流式代理在处理这些问题时，其被动的内存更新策略往往无法保留多跳推理所需的低显著性桥接证据。

Method: 提出了InfMem，一个控制中心代理，通过PreThink-Retrieve-Write协议实现System-2风格的控制。InfMem主动监控证据充分性，执行有针对性的文档内检索，并应用面向证据的联合压缩来更新有限内存。为确保可靠控制，引入了一种实用的SFT-to-RL训练方法，将检索、写入和停止决策与最终任务的正确性对齐。

Result: 在32k到1M token的超长问答基准测试中，InfMem在Qwen3-1.7B、Qwen3-4B和Qwen2.5-7B等骨干模型上持续优于MemAgent，平均绝对准确率分别提高了+10.17、+11.84和+8.23个百分点。此外，通过自适应提前停止，推理时间平均减少了3.9倍（最高可达5.1倍）。

Conclusion: InfMem通过主动的控制机制和优化的训练策略，有效地解决了超长文档推理中的关键挑战，显著提升了在超长问答任务上的性能，并提高了推理效率。
...
### [187] [Predicting first-episode homelessness among US Veterans using longitudinal EHR data: time-varying models and social risk factors](https://arxiv.org/abs/2602.02731)
*Rohan Pandey,Haijuan Yan,Hong Yu,Jack Tsai*

Main category: cs.CL

TL;DR: 该研究利用退伍军人事务部电子健康记录（EHR）数据，构建了包含社会和行为因素的纵向模型，以预测退伍军人未来3-12个月的首次无家可归事件。结果显示，纵向模型比静态模型更能准确地识别高风险个体，并且大型语言模型（LLM）在预测性能上与传统模型相当，但在不同种族群体间的表现差异较小。


<details>
  <summary>Details</summary>
Motivation: 美国退伍军人无家可归问题严峻，亟需有效的预测和干预方法。研究旨在探索利用电子健康记录数据预测无家可归风险的可能性，为主动干预提供依据。

Method: 研究者回顾性分析了2016年4,276,403名退伍军人事务部患者的电子健康记录，构建了静态和时变EHR表示，并结合临床逻辑模拟了临床状况和社会风险的长期变化。他们比较了经典机器学习、Transformer-based masked language models（MLMs）和微调后的大型语言模型（LLMs）的预测性能。

Result: 研究发现，将社会和行为因素纳入纵向模型可将精确率-召回率曲线下面积（PR-AUC）提高15-30%。在最高风险的1%人群中，不同模型在3、6、9、12个月的预测值分别为3.93-4.72%、7.39-8.30%、9.84-11.41%和11.65-13.80%。大型语言模型在区分度上略逊于编码器模型，但在不同种族群体间的性能差异较小。

Conclusion: 基于社会因素的纵向EHR建模能够有效地将退伍军人无家可归的风险集中到可操作的层级，从而支持有针对性的、数据驱动的预防策略，以帮助高风险退伍军人。
...
### [188] [Time-Critical Multimodal Medical Transportation: Organs, Patients, and Medical Supplies](https://arxiv.org/abs/2602.02736)
*Elaheh Sabziyan Varnousfaderani,Syed A. M. Shihab,Mohammad Taghizadeh*

Main category: cs.CL

TL;DR: 本研究提出了一种用于医疗运输的多模式车辆调度贪婪启发式算法，并评估了四种不同的车队配置（仅救护车、救护车+无人机、救护车+ eVTOL、全集成车队），以在考虑交通和天气因素的同时，最小化成本和运输时间。


<details>
  <summary>Details</summary>
Motivation: 传统地面交通工具受交通拥堵影响，而飞机成本高昂，新兴航空器（无人机、eVTOL）受续航和天气限制。多模式交通系统能结合各方优势，提高效率，尤其在时间敏感的医疗运输场景下至关重要。

Method: 开发了一种构造性贪婪启发式算法，用于多模式车辆调度。该算法考虑了有效载荷整合、地面交通拥堵和空中天气条件，并能快速调度车辆。通过四种不同的车队配置（仅救护车、救护车+无人机、救护车+ eVTOL、全集成车队）进行测试和评估。

Result: 评估了四种车队配置，以确定哪种最适合满足医疗运输需求，同时最小化运营成本、充电/燃油成本和总运输时间。研究旨在找出在特定条件下最高效的车队组合。

Conclusion: 通过比较不同车队配置的性能，本研究为医疗运输提供了一种高效的多模式车辆调度方法，并为优化车队构成提供了依据，以平衡效率、成本和可靠性。
...
### [189] [AmharicStoryQA: A Multicultural Story Question Answering Benchmark in Amharic](https://arxiv.org/abs/2602.02774)
...
TL;DR: 本研究提出了 AmharicStoryQA 基准测试，用于评估大型语言模型（LLM）在埃塞俄比亚不同地区叙事理解能力上的差异，发现现有 LLM 在此方面存在显著差距，并且区域差异明显，有监督微调效果不均衡。


<details>
  <summary>Details</summary>
Motivation: 现有针对大型语言模型的跨语言和跨文化评估基准，往往将语言和文化视为同义词，并以模型性能作为语言理解的代理。然而，这种方法忽略了单一语言内部存在的有意义的文化差异。本研究旨在弥合这一差距。

Method: 作者引入了 AmharicStoryQA，一个基于埃塞俄比亚阿姆哈拉语地区具有文化多样性的叙事长文本问答基准。利用此基准，研究评估了现有 LLM 的叙事理解能力，并探讨了有监督微调的影响。

Result: 研究结果显示，现有 LLM 在 AmharicStoryQA 基准上存在显著的叙事理解差距。评估结果呈现出明显的区域差异，表明模型在不同地区表现不一。此外，有监督微调在不同地区和评估场景下仅能带来不均衡的改进。

Conclusion: 本研究强调了开发基于文化的基准测试的必要性，这些基准测试应超越单纯的语言级别评估，以更准确地评估和改进低资源语言的叙事理解能力，并认识到语言内部的文化变异对模型评估的重要性。
...
### [190] [From Task Solving to Robust Real-World Adaptation in LLM Agents](https://arxiv.org/abs/2602.02760)
*Pouya Pezeshkpour,Estevam Hruschka*

Main category: cs.CL

TL;DR: 本研究评估了大型语言模型（LLM）在实际部署场景中的鲁棒性，发现其在面对不确定、动态和多目标环境时表现出显著的性能下降，并且模型之间的性能排名不稳定，需要进一步研究以提高其适应性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 评估方法过于理想化，假设“干净接口”，无法反映真实世界部署中 LLM 代理面临的复杂性，如规则不明确、工具不可靠、环境变化以及隐式多方目标。因此，研究 LLM 在这些现实挑战下的鲁棒性至关重要。

Method: 在网格游戏环境中，通过引入部分可观测性、动态环境、信号噪声和动态代理状态四种违反“干净接口”的假设来对 LLM 代理进行压力测试。评估了五个最先进的 LLM 代理在不同网格大小和执行时间跨度下的性能。

Result: 与理想场景相比，LLM 代理在鲁棒性测试中性能普遍下降。性能会随着网格大小和执行时间跨度的增加而恶化，但模型之间的性能排名不稳定。研究发现代理会在完成度、效率和避免惩罚之间进行权衡，表明其具备一定程度的客观推断能力。消融实验揭示了模型特定的敏感性和失败驱动因素。

Conclusion: 当前 LLM 代理在应对现实世界部署中的不确定性、动态性和隐式目标方面存在显著差距。未来的研究应侧重于提高代理在部分可观测性、噪声和非平稳性下的验证、安全行动选择和目标推断能力。
...
### [191] [When Efficient Communication Explains Convexity](https://arxiv.org/abs/2602.02821)
*Ashvin Ranjan,Shane Steinert-Threlkeld*

Main category: cs.CL

TL;DR: 本研究使用信息瓶颈（IB）方法，分析了语义类型学中高效沟通的解释因素。研究发现，IB最优性和一种广义凸性之间存在相关性，并且通信需求分布的凸性是驱动这种相关性的关键因素。


<details>
  <summary>Details</summary>
Motivation: 探究在语义类型学中，哪些因素能够成功地解释语言的多样性，特别是从高效沟通的角度来理解语言的简洁性和信息量的权衡。

Method: 研究采用了信息瓶颈（IB）方法来形式化沟通中的简洁性与信息量之间的权衡。通过操纵IB模型中的参数，分析IB最优性与广义凸性之间的相关性，并确定影响该相关性的关键因素。

Result: 研究发现，IB最优性与一种新颖的广义凸性之间存在显著相关性。进一步的实验表明，通信需求分布的凸性在驱动这种相关性方面起着尤其重要的作用。

Conclusion: 本研究不仅证明了高效沟通可以解释语义类型学中的某些现象，还进一步解释了其原因，通过识别通信需求分布的凸性这一关键底层因素，深化了对语言演化机制的理解。
...
### [192] [R2-Router: A New Paradigm for LLM Routing with Reasoning](https://arxiv.org/abs/2602.02823)
*Jiaqi Xue,Qian Lou,Jiarong Xing,Heng Huang*

Main category: cs.CL

TL;DR: 提出了一种名为R2-Router的新型LLM路由方法，它将输出长度作为可控变量，联合选择最优LLM和长度预算，并通过长度约束指令强制执行预算。随之构建了R2-Bench数据集。实验表明，R2-Router在成本降低4-5倍的情况下取得了最先进的性能，开辟了“路由即推理”的新方向。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM路由器忽略了同一LLM的输出质量会随着输出长度的变化而变化，这可能导致在成本超出预算时排除强大的LLM，错失了通过缩短输出长度来降低成本并获得高质量的潜力。

Method: R2-Router将输出长度预算视为一个可控变量，并联合选择最优LLM和长度预算。通过长度约束指令来强制执行长度预算。同时构建了R2-Bench数据集来捕获LLM在不同输出长度预算下的行为。

Result: R2-Router能够发现，通过约束输出长度，一个强大的LLM可以在可比较的成本下获得比一个较弱的LLM更好的性能，这是现有方法无法察觉的。实验结果显示，R2-Router实现了最先进的性能，且成本比现有路由器低4-5倍。

Conclusion: R2-Router通过将输出长度纳入考量，显著提高了LLM路由的效率和成本效益。该研究开辟了“路由即推理”的新方向，预示着未来的路由器将从简单的选择器演变为能够进行推理和探索的决策者。
...
### [193] [CATNIP: LLM Unlearning via Calibrated and Tokenized Negative Preference Alignment](https://arxiv.org/abs/2602.02824)
*Zhengbang Yang,Yisheng Zhong,Junyuan Hong,Zhuangdi Zhu*

Main category: cs.CL

TL;DR: CATNIP 是一种新的大语言模型（LLM）知识遗忘方法，通过量化模型对不良知识的置信度来校准梯度更新，解决了现有方法的局限性，并在数据集稀疏和长度变化方面表现出鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型知识遗忘方法（如梯度上升法）存在一些问题，例如会损害通用领域知识，需要遗忘保留数据或精心设计的对比样本，并且在真实数据设置下表现不佳。这些局限性促使研究者探索新的方法。

Method: CATNIP（Calibrated and Tokenized Negative Preference Alignment）是一种新的知识遗忘方法。它通过计算模型对不良知识的 token 级别置信度，并以此来调整遗忘效果的比例，实现对遗忘的精细控制。

Result: 在 MUSE 和 WMDP 基准数据集上的大量评估表明，CATNIP 在无需遗忘保留数据或对比遗忘响应对的情况下，能够实现有效的知识遗忘，并且在知识遗忘和知识保留的权衡方面优于现有最先进的方法。

Conclusion: CATNIP 能够实现有效的知识遗忘，并且在数据稀疏和长度变化的情况下表现出鲁棒性，同时能够更精确地控制遗忘，减少灾难性遗忘。
...
### [194] [Act or Clarify? Modeling Sensitivity to Uncertainty and Cost in Communication](https://arxiv.org/abs/2602.02843)
*Polina Tsvilodub,Karl Mulligan,Todd Snider,Robert D. Hawkins,Michael Franke*

Main category: cs.CL

TL;DR: 本研究提出，在不确定情况下，代理人是否会寻求澄清性问题（CQs）取决于情境不确定性以及替代行动的成本，并且这两者存在交互作用。研究者通过预期后悔模型进行了形式化，并在两个实验中验证了这些预测，发现人类倾向于根据行动不确定性带来的潜在损失风险来寻求澄清。


<details>
  <summary>Details</summary>
Motivation: 研究不确定性下代理人的决策行为，特别是在交流情境中，如何通过澄清性问题（CQs）来降低不确定性。

Method: 构建了一个基于预期后悔的计算模型，并进行了两项实验：一项侧重于纯粹的语言回应，另一项则扩展到澄清与非语言行动之间的选择。

Result: 实验结果表明，不确定性对决策的影响程度与错误行动的成本相关；当错误行动成本高时，不确定性的影响更大。人们会根据因不确定性而造成重大损失的风险来决定寻求澄清的程度。

Conclusion: 在不确定性下，人类的决策行为体现了一种理性的权衡：他们倾向于根据行动不确定性带来的潜在损失风险来寻求澄清。
...
### [195] [Which course? Discourse! Teaching Discourse and Generation in the Era of LLMs](https://arxiv.org/abs/2602.02878)
*Junyi Jessy Li,Yang Janet Liu,Kanishka Misra,Valentina Pyatkin,William Sheffield*

Main category: cs.CL

TL;DR: 本文介绍了一门名为“计算话语与自然语言生成”的新本科课程，旨在弥合 NLP 领域内的子学科差距，并整合话语处理的理论与计算方法，以应对 NLP 领域的快速发展。


<details>
  <summary>Details</summary>
Motivation: NLP 领域正在经历快速变革，现有教育课程在整合相关子学科方面存在不足，尤其是在话语处理与长文本生成之间的联系方面。

Method: 作者设计并开设了一门名为“计算话语与自然语言生成”的跨学科（语言学与计算机科学）本科课程。课程强调理论与实证的深度融合，鼓励探索性学习。

Result: 课程成功开设，并收集了独立调查的反馈。文章详细描述了课程内容，并提出了未来的发展方向。

Conclusion: 通过整合话语处理的理论和计算模型，可以更好地应对 NLP 领域的挑战，并为学生提供更全面的教育。课程的设计和实施为未来的 NLP 教育提供了有益的参考。
...
### [196] [HALT: Hallucination Assessment via Log-probs as Time series](https://arxiv.org/abs/2602.02888)
*Ahmad Shapiro,Karan Taneja,Ashok Goel*

Main category: cs.CL

TL;DR: 本文提出了一种名为HALT的轻量级幻觉检测器，它利用LLM生成的前20个token的log-probabilities作为时间序列，并通过GRU和熵特征学习模型校准偏差。同时，文章还引入了HUB基准来评估幻觉检测性能。HALT在速度和性能上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: LLM在安全关键领域存在幻觉问题，需要有效的检测方法。现有方法要么是白盒方法（需要访问内部状态），要么是黑盒方法（基于文本，泛化性较差）。作者希望开发一种既能处理专有LLM，又能在不同能力领域保持良好泛化性的高效检测方法。

Method: HALT模型使用GRU和熵特征，仅依赖LLM生成的前20个token的log-probabilities来学习模型校准偏差。文章还提出了HUB基准，整合了10个能力领域的数据集，用于评估幻觉检测器的性能。

Result: HALT模型比基于BERT的Lettuce模型取得了更好的性能，并且速度快60倍。HALT在HUB基准上表现出色，证明了其在多种LLM能力上的有效性。

Conclusion: HALT是一种高效且通用的幻觉检测框架，它通过利用token log-probabilities的时间序列和熵特征，实现了优于现有方法的性能，并与HUB基准一起为LLM幻觉检测提供了一个有效的评估方案。
...
### [197] [Equal Access, Unequal Interaction: A Counterfactual Audit of LLM Fairness](https://arxiv.org/abs/2602.02932)
*Alireza Amiri-Margavi,Arshia Gharagozlou,Amin Gholami Davodi,Seyed Pouyan Mousavi Davoudi,Hamidreza Hasani Balyani*

Main category: cs.CL

TL;DR: 本研究发现，即使大型语言模型（LLM）在访问层面（例如拒绝回答）对所有用户都公平，但在交互质量方面（如语气、不确定性和语言框架）仍然存在不公平现象，GPT-4 和 LLaMA-3.1-70B 在处理涉及年龄、性别和国籍等身份属性的职业建议任务时表现出不同的不公平模式。


<details>
  <summary>Details</summary>
Motivation: 现有关于大型语言模型（LLM）公平性的研究主要集中在访问层面的行为（如拒绝和安全过滤），但忽略了模型在提供回复后的交互质量是否公平。本研究旨在填补这一空白，探讨在访问公平的前提下，LLM 在交互质量上是否存在针对不同人口统计学身份的系统性差异。

Method: 研究者采用受控的公平性审计方法，利用反事实提示设计来评估 GPT-4 和 LLaMA-3.1-70B 模型。他们通过改变提示中的年龄、性别和国籍等身份属性，在职业建议任务上对模型进行测试。首先，通过拒绝率分析评估访问公平性；接着，利用自动化的语言学指标（包括情感、礼貌和犹豫性语言）来衡量交互质量；最后，使用配对统计检验来评估基于身份的差异。

Result: 研究发现，GPT-4 和 LLaMA-3.1-70B 在所有测试的身份属性上都表现出零拒绝率，表明在访问层面是公平的。然而，在交互质量方面，观察到系统性的、模型特有的差异：GPT-4 对年轻男性用户表现出显著更高的犹豫性语言；而 LLaMA 在不同身份群体之间表现出更广泛的情感差异。

Conclusion: 即使在访问层面实现公平，大型语言模型在交互质量层面仍然可能存在不公平。本研究结果表明，对 LLM 的公平性评估需要超越基于拒绝的审计，深入到交互内容本身的质量和语言表达的细微差别，以全面理解和解决模型可能存在的偏见。
...
### [198] [Where Norms and References Collide: Evaluating LLMs on Normative Reasoning](https://arxiv.org/abs/2602.02975)
*Mitchell Abrams,Kaveh Eskandari Miandoab,Felix Gervits,Vasanth Sarathy,Matthias Scheutz*

Main category: cs.CL

TL;DR: 本研究提出SNIC测试平台，评估大型语言模型（LLMs）在基于社会规范的指代消解（NBRR）方面的能力，发现当前LLMs在识别和应用社会规范方面存在不足，尤其是在规范隐含、不明确或冲突的情况下。


<details>
  <summary>Details</summary>
Motivation: 为了使机器人等具身智能体能在实际环境中成功进行交互，理解和利用社会规范进行沟通至关重要。然而，目前尚不清楚大型语言模型（LLMs）是否具备这种基于规范的推理能力。

Method: 研究人员构建了一个名为SNIC（Situated Norms in Context）的人类验证的诊断测试平台。该平台侧重于在日常任务（如清洁、整理、服务）中产生的、基于物理环境的社会规范，用于评估最先进的LLMs提取和利用与NBRR相关的规范的能力。

Result: 通过一系列受控评估，研究发现即使是最强的LLMs在一致地识别和应用社会规范方面也存在困难，特别是在规范是隐含的、欠明确的或相互冲突的情况下。

Conclusion: 当前的大型语言模型在处理基于规范的推理方面存在明显的局限性，这对在具有社会情境的具身环境中部署基于语言的系统提出了关键挑战。
...
### [199] [LatentMem: Customizing Latent Memory for Multi-Agent Systems](https://arxiv.org/abs/2602.03036)
*Muxin Fu,Guibin Zhang,Xiangyuan Xue,Yafu Li,Zefeng He,Siyuan Huang,Xiaoye Qu,Yu Cheng,Yang Yang*

Main category: cs.CL

TL;DR: 本文提出了LatentMem，一个可学习的多智能体记忆框架，用于解决现有框架中记忆同质化和信息过载的问题，通过经验库和记忆合成器实现，并引入LMPO进行端到端优化，实验结果表明性能有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的多智能体系统（MAS）的记忆设计存在记忆同质化（缺乏角色意识定制）和信息过载（记忆条目过于精细）的瓶颈。

Method: 提出了LatentMem框架，包含一个存储原始交互轨迹的“经验库”和一个根据检索到的经验和代理特定上下文合成紧凑“潜在记忆”的“记忆合成器”。引入了“潜在记忆策略优化”（LMPO），将任务级别的优化信号传播到合成器，以生成紧凑且高价值的表示。

Result: 在多个基准和主流MAS框架的实验中，LatentMem实现了高达19.36%的性能提升，并且在不修改底层框架的情况下，一致优于现有的记忆架构。

Conclusion: LatentMem是一个有效的、可学习的、针对代理定制的、并且对token高效的多智能体记忆框架，能够克服现有方法的局限性，并显著提升MAS的性能。
...
### [200] [CPMobius: Iterative Coach-Player Reasoning for Data-Free Reinforcement Learning](https://arxiv.org/abs/2602.02979)
*Ran Li,Zeyuan Liu,Yinghao chen,Bingxiang He,Jiarui Yuan,Zixuan Fu,Weize Chen,Jinyi Hu,Zhiyuan Liu,Maosong Sun*

Main category: cs.CL

TL;DR: 本文提出了一种名为 CPMöbius 的数据无关强化学习框架，通过模仿人类协作模式，让“教练”和“玩家”协同工作，以提升大型语言模型的数学推理能力，并在多项评估中展现出优于现有无监督方法的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在复杂推理方面的进展受限于对大量人工标注数据的依赖（如监督微调 SFT 或基于推理数据的强化学习 RL），这种对监督的重度依赖使得训练模式难以持续且可扩展性受限，作者旨在克服这一局限性。

Method: CPMöbius 框架采用了一种协作式教练-玩家（Coach-Player）范式，灵感来源于真实世界的人类体育协作和多智能体协作。教练负责根据玩家的能力提出指令，并根据玩家表现的变化获得奖励；玩家则通过解决教练生成的、难度逐渐递增的任务来获得奖励。这种协作优化循环直接提升玩家的数学推理能力，且无需任何外部训练数据。

Result: CPMöbius 在无需外部数据的情况下，显著提升了模型的数学推理能力。在 Qwen2.5-Math-7B-Instruct 模型上，CPMöbius 实现了 +4.9 的整体平均准确率提升和 +5.4 的分布外（OOD）平均准确率提升，其整体准确率比 RENT 高 +1.5，OOD 准确率比 R-zero 高 +4.2。

Conclusion: CPMöbius 提出了一种新颖的数据无关强化学习框架，通过模仿协作模式，有效地提升了大型语言模型的数学推理能力，且优于现有的无监督方法，克服了传统监督学习方法对大规模标注数据的依赖问题。
...
### [201] [SAES-SVD: Self-Adaptive Suppression of Accumulated and Local Errors for SVD-based LLM Compression](https://arxiv.org/abs/2602.03051)
*Xing Hu,Dawei Yang,Yuan Cheng,Zhixuan Chen,Zukang Xu*

Main category: cs.CL

TL;DR: 本文提出了一种名为SAES-SVD的低秩压缩框架，用于高效压缩大型语言模型（LLMs）。该框架通过联合优化层内重构和层间误差补偿，解决了现有方法中误差累积问题，并在多项任务和模型上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数规模的快速增长对高效压缩技术提出了高需求。虽然低秩压缩被广泛采用，但现有方法独立压缩各层，导致重构误差累积，使得压缩后模型性能显著下降。

Method: SAES-SVD框架包含两个主要组件：1) 累积误差感知层压缩（CEALC），将压缩目标设计为局部重构和加权累积误差补偿的结合，并基于二阶激活统计量推导出一种闭式低秩解法，显式地对齐层输出以补偿累积误差。2) 自适应协同误差抑制（ACES），通过最大化压缩层输出的Frobenius范数与压缩目标Frobenius范数之比来自动调整权重系数，以增强CEALC中压缩目标的低秩结构。

Result: 在不进行微调或采用混合秩策略的情况下，SAES-SVD在多种LLM架构和任务上的实验结果表明，其性能始终优于现有方法。

Conclusion: SAES-SVD框架能够有效地解决低秩压缩中误差累积的问题，通过联合优化和自适应误差抑制，显著提升了压缩后LLM的性能。
...
### [202] [ReMiT: RL-Guided Mid-Training for Iterative LLM Evolution](https://arxiv.org/abs/2602.03075)
*Junjie Huang,Jiarui Qin,Di Yin,Weiwen Liu,Yong Yu,Xing Sun,Weinan Zhang*

Main category: cs.CL

TL;DR: 本文提出了一种名为ReMiT的新方法，通过在预训练的中间阶段引入强化学习（RL）模型引导的重加权，实现了大型语言模型（LLM）的自增强循环。与传统的单向训练不同，ReMiT能够利用RL调优模型的推理能力来优化基础模型，从而提升后续的微调性能，并证明了这种迭代反馈循环的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统的LLM训练流程是单向的，从预训练到微调。研究者希望探索一种双向过程，即微调阶段的见解能够反过来改进预训练的基础模型，从而建立一个自增强的飞轮，持续提升LLM的性能，而无需额外的教师模型或参考模型。

Method: 研究者分析了训练动态，发现在预训练结束时的中期（退火）阶段是模型能力的关键转折点。在此基础上，提出ReMiT（Reinforcement Learning-Guided Mid-Training）方法。ReMiT在预训练的中期阶段，利用RL调优模型的推理先验知识，动态地重新加权Token，优先考虑对推理至关重要的Token。

Result: ReMiT在10个涵盖数学、代码和通用推理的预训练基准测试上平均实现了3%的提升，并在整个微调流程中保持了超过2%的性能增益。

Conclusion: ReMiT验证了通过RL引导的中间阶段重加权，可以实现LLM的持续、自增强进化，形成一个有效的迭代反馈循环，从而提高模型在多个任务上的性能。
...
### [203] [AERO: Autonomous Evolutionary Reasoning Optimization via Endogenous Dual-Loop Feedback](https://arxiv.org/abs/2602.03084)
*Zhitao Gao,Jie Ma,Xuhong Li,Pengyu Li,Ning Qu,Yaqiang Wu,Hui Liu,Jun Liu*

Main category: cs.CL

TL;DR: 本文提出了一种名为AERO的无监督框架，通过内部的自我提问、回答和批评来自动化语言模型的推理能力进化，克服了对专家标注数据和外部验证器的依赖，并在多个基准测试中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型在复杂推理方面虽然取得成功，但仍然依赖于专家标注数据和外部验证器，这限制了它们的自主性和可扩展性。现有的自进化范式存在未能识别最佳学习区域以及加剧集体幻觉和错误先验的风险。

Method: AERO框架采用协同双循环系统，实现自主推理进化。它借鉴了“最近发展区（ZPD）”理论，利用基于熵的定位来识别“可解性差距”，并 employing Independent Counterfactual Correction进行鲁棒验证。此外，还引入了Staggered Training Strategy来同步功能角色之间的能力增长，并防止课程崩溃。

Result: 在跨越三个领域的九个基准测试中的广泛评估表明，AERO在Qwen3-4B-Base上平均性能提高了4.57%，在Qwen3-8B-Base上平均性能提高了5.10%，优于竞争基线。

Conclusion: AERO是一个有效的无监督框架，可以通过内部的自我提问、回答和批评实现大型语言模型的自主推理进化，并在各种任务中展示出优越的性能。
...
### [204] [Test-time Recursive Thinking: Self-Improvement without External Feedback](https://arxiv.org/abs/2602.03094)
*Yufan Zhuang,Chandan Singh,Liyuan Liu,Yelong Shen,Dinghuai Zhang,Jingbo Shang,Jianfeng Gao,Weizhu Chen*

Main category: cs.CL

TL;DR: 本研究提出了一种名为TRT（Test-time Recursive Thinking）的迭代式自我改进框架，使大型语言模型（LLM）能够在没有外部监督的情况下自我提升。TRT通过生成多样化的高质量候选方案，并利用自我生成的验证信号来可靠地选择正确答案。实验表明，TRT能显著提高模型在AIME和LiveCodeBench等任务上的准确率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大型语言模型（LLM）在无需额外训练的情况下进行自我改进的可能性，特别是要解决在缺乏真实标签的情况下生成高质量解决方案和可靠选择正确答案的挑战。

Method: 提出了一种名为TRT（Test-time Recursive Thinking）的迭代式自我改进框架。该框架在生成过程中，会考虑特定策略、累积知识以及自我生成的验证信号，以期生成多样化的高质量候选方案并可靠地选择正确答案。

Result: 使用TRT框架，开源模型在AIME-25/24数学竞赛问题上达到了100%的准确率。在LiveCodeBench最困难的问题上，闭源模型在没有外部反馈的情况下，准确率提高了10.4-14.8个百分点。

Conclusion: TRT框架能够有效地提升大型语言模型在缺乏外部监督时的推理和问题解决能力，实现了在测试阶段的自我改进，并且在多个具有挑战性的基准测试中取得了显著的性能提升。
...
### [205] [Task--Specificity Score: Measuring How Much Instructions Really Matter for Supervision](https://arxiv.org/abs/2602.03103)
*Pritam Kadasi,Abhishek Upperwal,Mayank Singh*

Main category: cs.CL

TL;DR: 研究提出任务特异性得分（TSS）及其改进版本TSS++，用于衡量指令对输出的决定程度，并通过实验证明选择任务特异性示例能提升模型在有限token预算下的下游性能。


<details>
  <summary>Details</summary>
Motivation: 当前的指令微调方法依赖于大量的指令-输入-输出对，但很多指令对输出的指定不够明确，一个输入可能对应多个合理的输出。这促使研究者思考：指令是否能唯一确定目标输出？

Method: 提出任务特异性得分（TSS）来量化指令对输出的决定程度，通过对比真实指令和同一输入下的其他合理指令。进一步提出TSS++，采用更难的负例和质量项来缓解易负例效应。

Result: 在Alpaca、Dolly-15k、NI-20三个数据集和Gemma、Llama、Qwen三个模型上进行实验，结果显示选择任务特异性（高TSS）的示例能够提高模型在有限token预算下的下游任务性能，并且与基于困惑度（perplexity）和IFD等质量过滤方法互补。

Conclusion: TSS和TSS++是衡量指令特异性的有效指标，通过筛选高特异性示例可以提升LLM的微调效率和性能，尤其是在资源受限的情况下。
...
### [206] [The Mask of Civility: Benchmarking Chinese Mock Politeness Comprehension in Large Language Models](https://arxiv.org/abs/2602.03107)
*Yitong Zhang,Yuhan Xiang,Mingxuan Liu*

Main category: cs.CL

TL;DR: 本研究在“大语言学”范式下，从语用学视角系统评估了代表性大语言模型（LLMs）在识别中文礼貌、失礼和模拟礼貌现象方面的性能差异，并提出了结合语用理论和提示工程的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究在LLMs的语用理解能力方面存在不足，特别是对中文礼貌现象的识别。本研究旨在弥合这一差距，并探索技术与人文学科的融合，为“大语言学”研究提供实践案例。

Method: 研究构建了一个包含真实和模拟中文语料的三类数据集，涵盖礼貌、失礼和模拟礼貌。选取了包括GPT-5.1和DeepSeek在内的六个代表性LLMs，并采用零样本、少样本、知识增强和混合策略四种提示条件进行评估。

Result: 研究系统评估了不同LLMs在不同提示策略下的性能表现，揭示了它们在识别中文语用现象方面的差异。

Conclusion: 本研究为在大语言模型时代应用语用理论提供了新颖的方法，并证明了技术与人文学科的融合是可行的，为理解LLMs的语用能力提供了实证依据。
...
### [207] [ChemPro: A Progressive Chemistry Benchmark for Large Language Models](https://arxiv.org/abs/2602.03108)
*Aaditya Baranwal,Shruti Vyas*

Main category: cs.CL

TL;DR: 本文提出了 ChemPro，一个包含 4100 个自然语言问答对的化学领域基准测试，用于评估大型语言模型（LLMs）在通用化学知识方面的能力，并分析了当前 LLMs 在不同难度和类型化学问题上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有 LLMs 在科学推理和理解方面的能力存在局限性，需要一个能够衡量其在不同难度和类型化学问题上表现的基准来识别这些不足。

Method: 构建了一个包含 4100 个问答对的 ChemPro 基准，涵盖了信息回忆、长时推理、多概念问题、细致问题解决和直接问题等，并根据难度分为 4 个部分。对 45+7 个不同类型的 LLMs 进行了评估。

Result: LLMs 在基础化学问题上表现良好，但随着问题的复杂性增加，准确率下降。表明 LLMs 在科学推理和理解方面存在不足，尤其是在处理更复杂和细致的问题时。

Conclusion: ChemPro 基准揭示了 LLMs 在通用化学推理方面的局限性，并强调了需要更强大的方法来提高 LLMs 在科学理解和解决复杂问题方面的能力。
...
### [208] [One Model, All Roles: Multi-Turn, Multi-Agent Self-Play Reinforcement Learning for Conversational Social Intelligence](https://arxiv.org/abs/2602.03109)
*Bowen Jiang,Taiwei Shi,Ryo Kamoi,Yuan Yuan,Camillo J. Taylor,Longqi Yang,Pei Zhou,Sihao Chen*

Main category: cs.CL

TL;DR: OMAR是一个强化学习框架，通过多轮、多智能体的对话自玩，让AI能够扮演所有对话者，并直接从动态社交互动中学习实现长期目标和复杂社会规范，从而发展出共情、说服和寻求妥协等精细的社交智能。


<details>
  <summary>Details</summary>
Motivation: 传统的AI对话模型依赖于静态、单轮优化，无法有效学习长对话中的复杂社交互动和长期目标。作者希望开发一种能够让AI通过多轮、多智能体对话自玩来学习和发展社交智能的方法。

Method: OMAR框架允许一个模型同时扮演对话中的所有角色，通过多轮对话自玩进行训练。为了稳定训练，采用了分层优势估计（turn-level and token-level advantages）。

Result: 在SOTOPIA社交环境和Werewolf策略游戏中，OMAR训练出的模型展现出了精细的、涌现的社交智能，例如共情、说服和寻求妥协。研究表明，即使在竞争性场景下，AI也能有效地学习协作。

Conclusion: OMAR框架能够有效地让AI在没有人类监督的情况下，从动态社交互动中学习和发展出丰富的社交智能，为AI在群体对话中的社交智能研究提供了新的方向。
...
### [209] [Short Chains, Deep Thoughts: Balancing Reasoning Efficiency and Intra-Segment Capability via Split-Merge Optimization](https://arxiv.org/abs/2602.03141)
*Runquan Gui,Jie Wang,Zhihai Wang,Chi Ma,Jianye Hao,Feng Wu*

Main category: cs.CL

TL;DR: 提出CoSMo框架，通过一致性指导的分割-合并优化来减少大型推理模型（LRM）的冗余推理链，从而提高效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRM）在解决复杂任务时依赖于冗长的推理链，这导致了显著的延迟和计算开销。

Method: 提出CoSMo框架，使用分割-合并算法动态优化推理链，通过合并冗余片段和分割逻辑缺口来确保连贯性。采用结构对齐的强化学习和新颖的片段级预算来监督模型。

Result: 在多个基准测试和骨干网络上，CoSMo的准确率平均提高了3.3个百分点，同时平均将片段使用量减少了28.7%。

Conclusion: CoSMo框架能够有效地消除结构冗余，提高LRM的推理效率和准确性，优于现有的推理效率基线方法。
...
### [210] [FASA: Frequency-aware Sparse Attention](https://arxiv.org/abs/2602.03152)
*Yifei Wang,Yueqi Wang,Zhenrui Yue,Huimin Zeng,Yong Wang,Ismini Lourentzou,Zhengzhong Tu,Xiangxiang Chu,Julian McAuley*

Main category: cs.CL

TL;DR: 本文提出了一种名为FASA的新框架，通过识别RoPE（旋转位置嵌入）中的“主导”频率块（FC）来动态预测Token的重要性，从而实现查询感知的Token剔除，显著降低LLM处理长文本时的内存和计算开销，同时保持近乎完美的性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在处理长输入时存在内存瓶颈，主要是由于KV缓存的巨大内存占用。现有的Token剪枝方法要么采用静态方法导致信息丢失，要么采用启发式动态方法，未能充分捕捉Token对查询的重要性。

Method: FASA框架的核心在于 RoPE 的“功能稀疏性”新发现，即在频率块（FC）层面存在一种可识别的“主导”FC子集，它们能高度反映整个注意力头的内容。FASA利用这一发现，通过识别主导FC来动态预测Token的重要性，并仅在选定的Token子集上进行注意力计算。

Result: FASA在多种长文本任务（包括序列建模和复杂推理）上，均优于现有的Token剔除基线方法，并达到了接近最优的准确率。在LongBench-V1上，仅保留256个Token时，FASA性能接近全KV缓存；在AIME24上，使用18.9%的缓存实现了2.56倍的加速。

Conclusion: FASA是一种高效且查询感知的Token剔除框架，通过利用RoPE中的频率块特性，能够有效降低LLM处理长文本的内存和计算成本，同时保持卓越的性能和鲁棒性。
...
### [211] [Privasis: Synthesizing the Largest "Public" Private Dataset from Scratch](https://arxiv.org/abs/2602.03183)
...
TL;DR: 本研究提出了Privasis，一个百万级规模的全合成隐私敏感数据集，用于加速和扩展在处理敏感社交数据方面的研究，并构建了一个高效的文本去标识化模型。


<details>
  <summary>Details</summary>
Motivation: 研究隐私敏感数据面临数据稀疏的挑战，而现代AI代理（如OpenClaw和Gemini Agent）却被赋予了持久访问高度敏感个人信息的权限，这增加了紧迫性。为了解决这一长期存在的瓶颈和日益增长的风险，研究者需要一个大规模、多样化的数据集来支持相关研究。

Method: 研究者构建了一个名为Privasis的百万级全合成数据集，包含140万条记录，覆盖医疗、法律、金融、日历和短信等多种文档类型，并标注了5510万个属性。利用Privasis，研究者构建了一个并行语料库，并开发了一个文本去标识化流水线，该流水线能分解文本并进行定向去标识化。他们训练了紧凑型（<=4B）去标识化模型。

Result: Privasis数据集规模是现有数据集的几个数量级，质量和多样性也远超以往。在Privasis上训练的紧凑型去标识化模型在文本去标识化任务上表现优于GPT-5和Qwen-3 235B等先进的大型语言模型。

Conclusion: Privasis是一个开创性的、大规模的全合成隐私敏感数据集，能够显著促进和加速涉及隐私敏感数据的研究。基于该数据集训练的去标识化模型在性能上超越了现有的大型模型，研究者计划公开数据、模型和代码以鼓励未来的研究。
...
### [212] [ForesightKV: Optimizing KV Cache Eviction for Reasoning Models by Learning Long-Term Contribution](https://arxiv.org/abs/2602.03203)
*Zican Dong,Peiyu Liu,Junyi Li,Zhipeng Chen,Han Peng,Shuo Wang,Wayne Xin Zhao*

Main category: cs.CL

TL;DR: 本文提出了一种名为 ForesightKV 的训练式 KV 缓存淘汰框架，通过学习预测哪些 KV 对需要被淘汰，从而在长文本生成中平衡效率和性能。它通过 Golden Eviction 算法找到最优淘汰策略，并利用监督学习和强化学习进行优化，在仅使用一半缓存预算的情况下，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有 KV 缓存淘汰方法在处理长序列时，由于 KV 缓存线性增长带来的内存和计算成本，以及对复杂 KV 依赖捕获不足导致性能下降，需要一种更有效的方法。

Method: ForesightKV 框架结合了监督学习和强化学习。首先，Golden Eviction 算法利用未来注意力分数识别最优淘汰 KV 对。然后，通过 Pairwise Ranking Loss 对这些轨迹和分数进行监督训练。最后，将缓存淘汰建模为马尔可夫决策过程，并应用 GRPO 算法来解决低熵标记上的语言模型损失增加问题。

Result: 在 AIME2024 和 AIME2025 基准测试中，ForesightKV 在三种推理模型上，使用仅一半的缓存预算，表现持续优于现有方法。

Conclusion: ForesightKV 是一种有效的训练式 KV 缓存淘汰框架，通过结合监督学习和强化学习，能够在显著降低内存和计算成本的同时，保持或提升模型性能，尤其是在长文本生成任务中。
...
### [213] [Token Sparse Attention: Efficient Long-Context Inference with Interleaved Token Selection](https://arxiv.org/abs/2602.03216)
*Dongwon Jo,Beomseok Kang,Jiwon Song,Jae-Joon Kim*

Main category: cs.CL

TL;DR: 本文提出了一种名为“Token Sparse Attention”的轻量级动态注意力稀疏化机制，通过压缩和解压缩 Q, K, V 来减少参与注意力的 Token 数量，从而在不显著牺牲准确性的前提下，大幅提高长上下文推理的速度。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在处理长上下文时，其注意力机制的二次复杂度是一个主要的瓶颈。现有的加速方法要么采用结构化稀疏注意力，要么在特定层永久移除 Token，这些方法可能导致模型保留无关信息或过早做出不可逆的决策，未能充分考虑 Token 在不同层的重要性动态变化。

Method: 本文提出的 Token Sparse Attention 是一种动态 Token 级稀疏化机制。它在计算注意力时，首先将每头（per-head）的 Q、K、V 压缩到一组缩减的 Token 集合，然后在计算完成后将输出解压缩回原始序列长度。这种方法允许 Token 信息在后续层被重新考虑，并且可以与现有的密集注意力实现（如 Flash Attention）以及稀疏注意力核无缝结合。

Result: 实验结果表明，Token Sparse Attention 能够持续改善精度-延迟权衡。在 128K 的上下文长度下，该方法实现了高达 3.23 倍的注意力速度提升，同时准确率仅下降不到 1%。

Conclusion: 动态且交错的 Token 级稀疏化是一种有效的、可扩展的长上下文推理策略，能够与现有的注意力机制互补，显著提升效率。
...
### [214] [ATACompressor: Adaptive Task-Aware Compression for Efficient Long-Context Processing in LLMs](https://arxiv.org/abs/2602.03226)
*Xuancheng Li,Haitao Li,Yujia Zhou,Qingyao Ai,Yiqun Liu*

Main category: cs.CL

TL;DR: 提出了一种名为ATACompressor的自适应任务感知压缩器，通过动态调整压缩比例来解决长上下文LLM中的“中间丢失”问题，并在QA任务中取得了比现有方法更好的效果。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文压缩方法在信息保留和压缩效率之间难以取得平衡，并且LLM在处理长上下文时存在“中间丢失”问题。

Method: ATACompressor采用选择性编码器，只压缩任务相关的部分；自适应分配控制器根据相关内容长度调整压缩率。

Result: 在HotpotQA、MSMARCO和SQUAD三个QA数据集上，ATACompressor在压缩效率和任务性能上均优于现有方法。

Conclusion: ATACompressor能够有效地解决长上下文处理中的“中间丢失”问题，为LLMs提供了可扩展的长上下文处理解决方案，并且通过消融实验深入理解了其关键组成部分。
...
### [215] [POP: Prefill-Only Pruning for Efficient Large Model Inference](https://arxiv.org/abs/2602.03295)
*Junhui He,Zhihui Fu,Jun Wang,Qingan Li*

Main category: cs.CL

TL;DR: 本文提出了一种名为 Prefill-Only Pruning (POP) 的模型推理策略，通过在计算密集型的预填充阶段省略模型深层，而在关键的解码阶段保留完整模型，从而在保持模型性能的同时，显著提升了预填充速度。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLM）和视觉语言模型（VLM）虽然能力强大，但部署成本高昂。现有的结构化剪枝方法在硬件效率上表现良好，但往往会严重损害模型精度。本文认为，这种精度损失源于剪枝方法忽视了预填充（prefill）和解码（decode）阶段不同作用。

Method: 作者提出了一种虚拟门控机制，并通过重要性分析发现深层模型在解码阶段（下一token预测）至关重要，但在预填充阶段（上下文编码）作用不大。基于此洞察，论文提出了 Prefill-Only Pruning (POP) 策略，在预填充阶段移除深层模型，在解码阶段恢复完整模型。为了保证模型在不同阶段间的平滑过渡，引入了独立的 Key-Value (KV) 投影来维护缓存完整性，并设计了一种边界处理策略来确保第一个生成token的准确性。

Result: 在 Llama-3.1、Qwen3-VL 和 Gemma-3 等模型上进行的广泛实验表明，POP 在预填充延迟方面实现了高达 1.37 倍的速度提升，同时仅造成极小的性能损失，有效克服了现有结构化剪枝方法在精度-效率之间的权衡限制。

Conclusion: POP 是一种有效的、面向阶段的推理策略，能够显著加速 LLM 和 VLM 的预填充阶段，同时保持模型的精度，为解决大型模型部署的效率问题提供了一种新思路。
...
### [216] [MIRROR: A Multi-Agent Framework with Iterative Adaptive Revision and Hierarchical Retrieval for Optimization Modeling in Operations Research](https://arxiv.org/abs/2602.03318)
*Yifan Shi,Jialong Shi,Jiayi Wang,Ye Fan,Jianyong Sun*

Main category: cs.CL

TL;DR: MIRROR是一个无需微调的多智能体框架，能将自然语言描述的运筹学问题自动翻译成数学模型和求解器代码，通过执行驱动的迭代自适应修正和分层检索机制，提高了模型准确性和效率，优于现有方法，并能处理复杂工业数据集。


<details>
  <summary>Details</summary>
Motivation: 传统的运筹学建模过程依赖专家，效率低下且难以适应新场景。现有的利用大语言模型（LLMs）将自然语言翻译成优化模型的方法存在成本高、缺乏可靠的协作纠错和任务特定检索等问题，导致输出不正确。

Method: 提出MIRROR框架，这是一个无需微调、端到端的多智能体系统。它包含两个核心机制：1. 执行驱动的迭代自适应修正（execution-driven iterative adaptive revision）用于自动纠错；2. 分层检索（hierarchical retrieval）用于从精心策划的示例库中获取相关的建模和编码示例。

Result: MIRROR在标准运筹学基准测试中表现优于现有方法，并在IndustryOR和Mamo-ComplexLP等复杂的工业数据集上取得了显著成果。

Conclusion: MIRROR通过结合精确的外部知识注入和系统性的错误纠正，为非专业用户提供了一个高效可靠的运筹学建模解决方案，克服了通用LLM在专业优化任务中的局限性。
...
### [217] [Accurate Failure Prediction in Agents Does Not Imply Effective Failure Prevention](https://arxiv.org/abs/2602.03338)
*Rakshith Vasudev,Melisa Russak,Dan Bikel,Waseem Alshikh*

Main category: cs.CL

TL;DR: 使用经过良好训练的LLM评论模型进行主动干预，并非总能提高模型可靠性，反而可能导致性能下降。研究提出了一个部署前测试方法，用于预测干预效果，避免性能退化。


<details>
  <summary>Details</summary>
Motivation: LLM评论模型的主动干预被认为可以提高可靠性，但其在实际部署时的效果尚不明确，研究旨在探究其潜在的负面影响并提出解决方案。

Method: 研究人员使用一个二元LLM评论模型，在不同模型上评估其干预效果。他们识别出“干扰-恢复”的权衡，并提出一个部署前测试，使用少量任务（50个）来预测干预是否会带来益处或损害。

Result: 即使离线精度高（AUROC 0.94）的LLM评论模型，也可能导致性能严重下降（一个模型下降26个百分点）。部署前测试能够有效预测干预效果：在高成功率任务上，干预会降低性能（0到-26个百分点）；而在高失败率的ALFWorld基准上，干预能带来温和提升（+2.8个百分点，p=0.014）。

Conclusion: LLM评论模型的离线准确性不足以保证部署时的安全性。关键在于识别何时不应进行干预，部署前测试能够有效预测干预效果，从而在部署前避免严重的性能回归。
...
### [218] [PEGRL: Improving Machine Translation by Post-Editing Guided Reinforcement Learning](https://arxiv.org/abs/2602.03352)
*Yunzhi Shen,Hao Zhou,Xin Huang,Xue Han,Junlan Feng,Shujian Huang*

Main category: cs.CL

TL;DR: 本文提出了PEGRL，一个两阶段的强化学习框架，利用机器翻译的后编辑作为辅助任务来稳定训练并指导优化，以克服现有方法中信号噪声和探索效率低的问题，并在多项翻译任务上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的机器翻译方法面临由蒙特卡洛回报估计带来的噪声学习信号，以及 Trajectory 空间过大导致全局探索优先于局部优化的挑战。

Method: PEGRL 采用两阶段框架。在每个迭代中，生成翻译输出作为后编辑的输入，使得后编辑阶段的回报估计能受益于当前翻译行为的条件，同时支持全局探索和局部优化。此外，还设计了一个任务特定的加权方案来平衡翻译和后编辑目标。

Result: 在英语到芬兰语、英语到土耳其语以及英语↔中文的实验中，PEGRL 均优于现有的强化学习基线。尤其是在英语到土耳其语任务上，其在 COMET-KIWI 上的表现与先进的 LLM 系统（DeepSeek-V3.2）相当。

Conclusion: PEGRL 通过引入后编辑作为辅助任务，成功地稳定了强化学习训练，并实现了更有效的探索，从而在机器翻译任务中取得了优于现有方法的性能。
...
### [219] [Pursuing Best Industrial Practices for Retrieval-Augmented Generation in the Medical Domain](https://arxiv.org/abs/2602.03368)
*Wei Zhu*

Main category: cs.CL

TL;DR: 本文探讨了在工业应用（特别是医疗领域）中构建检索增强生成（RAG）系统的最佳实践，分析了 RAG 的各个组件，并进行了系统性评估，以改善 RAG 系统的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 在工业应用中，尽管 RAG 已被广泛采用，但缺乏关于构建 RAG 系统的最佳实践的共识，尤其是在医疗领域。因此，需要对 RAG 系统进行深入分析和评估，以指导实际应用。

Method: 文章首先对 RAG 系统的每个组件进行了细致的分析，并提出了实用的替代方案。随后，对三种类型的任务进行了系统的评估，以确定改进 RAG 系统的最佳实践，并研究基于 LLM 的 RAG 系统在性能和效率之间的权衡。

Result: 通过系统性评估，揭示了提高 RAG 系统性能的最佳实践，并阐明了基于 LLM 的 RAG 系统如何在性能和效率之间做出权衡。

Conclusion: 本文为在工业应用（特别是医疗领域）中构建 RAG 系统提供了实用的指导，通过对组件的分析和系统的评估，为优化 RAG 系统的性能和效率提供了依据。
...
### [220] [Towards Distillation-Resistant Large Language Models: An Information-Theoretic Perspective](https://arxiv.org/abs/2602.03396)
*Hao Fang,Tianyi Zhang,Tianqu Zhuang,Jiawei Kong,Kuofeng Gao,Bin Chen,Leqi Liang,Shu-Tao Xia,Ke Xu*

Main category: cs.CL

TL;DR: 研究提出了一种从信息论角度出发，通过最小化教师模型输出中的条件互信息（CMI）来防御基于logit的知识蒸馏的方法，并设计了一种可学习的变换矩阵来纯化模型输出，以增强模型的抗蒸馏能力，实验证明该方法能有效降低蒸馏性能并保持任务准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM防御方法主要关注文本蒸馏，忽视了logit蒸馏的威胁；而logit蒸馏可以更有效地提取模型知识。

Method: 1. 从信息论角度分析，使用条件互信息（CMI）量化教师模型输出中包含的、有利于模型提取的上下文信息。2. 提出通过最小化CMI来防御知识蒸馏。3. 设计一种可学习的变换矩阵来纯化模型输出，增强其抗蒸馏能力。4. 提出一种受CMI启发的反蒸馏目标函数来优化变换矩阵，以移除蒸馏相关信息并保留输出效用。

Result: 在多个LLM和强蒸馏算法上的实验表明，所提出的方法显著降低了蒸馏性能，同时保持了任务准确性。

Conclusion: 所提出的基于CMI的方法能够有效防御基于logit的知识蒸馏，保护模型的知识产权，同时不损害模型的实用性。
...
### [221] [Verified Critical Step Optimization for LLM Agents](https://arxiv.org/abs/2602.03412)
*Mukai Li,Qingcheng Zeng,Tianqing Fang,Zhenwen Liang,Linfeng Song,Qi Liu,Haitao Mi,Dong Yu*

Main category: cs.CL

TL;DR: 本文提出了一种名为关键步骤优化（CSO）的新型语言模型代理后训练方法，该方法通过识别和优化那些能够显著影响任务结果的决策点，从而提高代理处理复杂长时任务的能力。CSO从失败的轨迹开始，利用过程奖励模型（PRM）和专家模型来识别和生成有希望的替代方案，并仅在代理成功完成这些替代方案后生成训练数据。实验表明，CSO在GAIA-Text-103和XBench-DeepSearch任务上显著优于SFT基线和其他后训练方法，且所需监督信息仅占轨迹步数的16%。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型代理在处理复杂的长时任务时，其后训练方法面临挑战，例如结果奖励难以准确归因中间步骤的贡献，基于估计的步级奖励引入系统性噪声，以及蒙特卡洛采样方法计算成本过高。研究旨在解决这些问题，提出一种更有效、更具成本效益的后训练方法。

Method: 提出关键步骤优化（CSO）方法。该方法首先利用过程奖励模型（PRM）识别失败轨迹中的候选关键步骤。然后，利用专家模型生成高质量的替代行动。接着，代理从这些替代行动处继续执行，直到任务完成。只有那些成功执行并纠正了结果的替代方案才被验证并用作直接偏好优化（DPO）的训练数据。这种方法从失败的轨迹开始，专注于经过验证的关键决策点。

Result: 在GAIA-Text-103和XBench-DeepSearch数据集上的实验显示，CSO相比于SFT基线分别取得了37%和26%的相对性能提升。此外，CSO大幅优于其他后训练方法，并且仅在16%的轨迹步数上需要监督。

Conclusion: CSO通过选择性地在关键决策点进行验证和优化，能够有效地提升语言模型代理在复杂长时任务上的表现。这种方法避免了传统方法中的归因不准确和噪声问题，并显著降低了训练成本，证明了选择性验证式学习在代理后训练中的有效性。
...
### [222] [FactNet: A Billion-Scale Knowledge Graph for Multilingual Factual Grounding](https://arxiv.org/abs/2602.03417)
*Yingli Shen,Wen Lai,Jie Zhou,Xueren Zhang,Yudong Wang,Kangyang Luo,Shuo Wang,Ge Gao,Alexander Fraser,Maosong Sun*

Main category: cs.CL

TL;DR: 本研究提出了FactNet，一个包含17亿个原子断言和30.1亿个可追溯证据指针的大规模、开源资源，旨在解决大型语言模型（LLMs）的事实幻觉和溯源问题，并建立了FactNet-Bench评估套件。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLMs）存在事实幻觉和缺乏可追溯性，而现有的资源要么缺乏文本上下文，要么规模和语言覆盖有限。研究的动机是弥合这一差距，提供一个统一的、可验证的、多语言的资源。

Method: FactNet通过一个严格确定的构建流程，从316个维基百科版本中提取了17亿个原子断言和30.1亿个证据指针。构建过程保证了证据单元的可恢复性。此外，研究还建立了FactNet-Bench评估套件，用于知识图谱补全、问答和事实核查。

Result: FactNet具有极高的接地精度（92.1%），即使是在长尾语言中。FactNet-Bench在知识图谱补全、问答和事实核查任务上提供了全面的评估。

Conclusion: FactNet是一个大规模、可复现、可验证的多语言资源，可以支持可信赖的、可验证的LLM系统的训练和评估，为研究社区提供了一个基础性的资源。
...
### [223] [A-RAG: Scaling Agentic Retrieval-Augmented Generation via Hierarchical Retrieval Interfaces](https://arxiv.org/abs/2602.03442)
*Mingxuan Du,Benfeng Xu,Chiwei Zhu,Shaohan Wang,Pengyu Wang,Xiaorui Wang,Zhendong Mao*

Main category: cs.CL

TL;DR: 本文提出了一种名为 A-RAG 的生成式检索增强框架，允许语言模型直接参与检索决策，通过分层检索接口（关键字搜索、语义搜索、块读取）自适应地跨多粒度检索信息，并在开放域问答任务上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的 RAG 系统无法充分利用大型语言模型的推理和工具使用能力，要么一次性检索并拼接，要么预设固定流程，限制了模型能力的扩展。因此，需要一种能让模型参与检索决策的 RAG 框架。

Method: A-RAG 框架通过提供分层的检索接口（关键字搜索、语义搜索、块读取）直接暴露给语言模型，使模型能够自适应地、跨多粒度进行信息检索和选择，从而动态调整 RAG 过程。

Result: 在多个开放域问答基准测试中，A-RAG 始终优于现有方法，并且检索的 token 数量相当或更少。A-RAG 的性能会随着模型规模和测试时计算量的增加而扩展。

Conclusion: A-RAG 框架能够有效地利用语言模型的强大能力，动态地适应不同的 RAG 任务，提高了检索效率和问答性能。该框架为未来的 RAG 研究提供了新的方向和工具。
...
### [224] [Preferences for Idiomatic Language are Acquired Slowly -- and Forgotten Quickly: A Case Study on Swedish](https://arxiv.org/abs/2602.03484)
*Jenny Kunz*

Main category: cs.CL

TL;DR: 本研究探讨了语言模型在预训练和从英语迁移到瑞典语的过程中，对习语和语言可接受性偏好的发展。结果表明，习语能力比语法和词汇正确性等其他语言能力发展更慢。然而，使用机器翻译的英语指令数据进行微调会迅速削弱模型对习语的偏好。


<details>
  <summary>Details</summary>
Motivation: 了解语言模型在处理不同语言特征（特别是习语）时的能力和局限性，尤其是在跨语言迁移场景下，对于改进模型性能至关重要。

Method: 研究者通过从头开始训练瑞典语模型以及微调预训练的英语模型，并在不同检查点使用最小对（minimal pairs）进行探测，来评估模型对语言可接受性（通过改编现有基准）和习语（通过两个新数据集：习语与变体对比，以及习语与翻译腔对比）的偏好。

Result: 习语能力的出现比语法和词汇正确性等其他语言能力要慢。尽管大多数任务的训练回报递减，但习语相关性能持续提升，尤其是在最大的 8B 模型中。然而，使用机器翻译的英语指令数据进行微调，会导致模型迅速失去对习语的偏好。

Conclusion: 语言模型在习语方面的能力发展滞后，并且跨语言指令微调（特别是通过机器翻译）会严重损害模型处理习语的能力。
...
### [225] [Self-Verification Dilemma: Experience-Driven Suppression of Overused Checking in LLM Reasoning](https://arxiv.org/abs/2602.03485)
*Quanyu Long,Kai Jie Jiang,Jianda Chen,Xu Guo,Leilei Gan,Wenya Wang*

Main category: cs.CL

TL;DR: 研究发现大型推理模型（LRMs）中的自我验证（recheck）步骤冗余且低效，提出了一种基于经验的测试时框架，通过检索历史经验来预测并抑制不必要的验证，从而在保持准确性的同时减少了20.3%的token使用量。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）中的自我验证步骤（recheck）在实际应用中被过度使用，但大多数情况下并不能纠正错误，显示出实际效用与使用频率之间存在不匹配。

Method: 提出了一种在测试时运行的、基于经验的框架。该框架能检测到recheck行为的激活，并查询一个包含过往验证结果的离线经验池。通过高效检索，框架可以估算recheck是否不必要，如果历史经验表明不必要，则会发送一个抑制信号，引导模型跳过该验证步骤。

Result: 在多个模型和基准测试中，提出的方法可以将token使用量减少高达20.3%，同时保持了原有的准确性。在某些数据集上，准确性甚至有所提高。

Conclusion: 通过识别并减少大型推理模型中低效的自我验证步骤，可以在不牺牲性能的情况下显著提高模型的效率，证明了经验驱动的测试时优化策略的有效性。
...
### [226] [Learning to Reason Faithfully through Step-Level Faithfulness Maximization](https://arxiv.org/abs/2602.03507)
*Runquan Gui,Yafu Li,Xiaoye Qu,Ziyan Liu,Yeqiu Cheng,Yu Cheng*

Main category: cs.CL

TL;DR: 提出了一种名为FaithRL的强化学习框架，通过优化推理的忠实度来减少大型语言模型的幻觉，同时保持或提高答案的正确性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于奖励的强化学习方法（RLVR）在多步推理任务中表现良好，但其稀疏的基于结果的奖励会导致模型对中间步骤过度自信并产生虚假推理，从而增加幻觉。为了解决这个问题，需要一种可以直接优化推理忠实度的框架。

Method: FaithRL框架通过正式化一个最大化忠实度的目标函数，并理论证明其可以减轻过度自信。具体实现上，引入了几何奖励设计和忠实度感知优势调制机制，通过惩罚不支持的中间步骤来分配步骤级信用，同时保留有效的中间推导。

Result: 在不同的基础模型和基准测试中，FaithRL consistently 降低了幻觉率，并且保持（或提高了）答案的正确性。进一步的分析证实，FaithRL 提高了推理的忠实度，并且泛化能力强。

Conclusion: FaithRL 是一种有效的强化学习框架，能够通过直接优化推理忠实度来显著减少大型语言模型的幻觉，并保持甚至提高推理任务的性能。该方法在减少幻觉、提高中间推理步骤的可靠性以及泛化能力方面均表现出优越性。
...
### [227] [SEAD: Self-Evolving Agent for Multi-Turn Service Dialogue](https://arxiv.org/abs/2602.03548)
*Yuqin Dai,Ning Gao,Wei Zhang,Jie Wang,Zichen Luo,Jinpeng Wang,Yujie Wang,Ruiyuan Wu,Chaozheng Wang*

Main category: cs.CL

TL;DR: 提出了一种名为SEAD（Self-Evolving Agent for Service Dialogue）的框架，用于提升大型语言模型在服务对话中的表现，通过解耦用户建模和自适应训练，无需大量人工标注即可有效学习策略，实验结果表明其在任务完成率和对话效率上显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型在开放领域对话中表现出色，但在服务对话方面性能不佳，主要原因是依赖低质量的人类对话数据，且数据稀缺，难以模拟真实的、目标导向的用户行为。

Method: 提出SEAD框架，将用户建模解耦为两个部分：1. Profile Controller：生成多样化的用户状态，以管理训练课程。2. User Role-play Model：专注于实现逼真的角色扮演。这种设计确保了训练环境提供自适应的训练场景，而非不公平的对手。

Result: SEAD在实验中显著优于开源基础模型和闭源商业模型，任务完成率提高了17.6%，对话效率提高了11.1%。

Conclusion: SEAD框架能够使服务对话代理在没有大规模人工标注的情况下，有效学习策略，显著提升服务对话的表现，解决了当前模型在服务对话中的局限性。
...
### [228] [Can Large Language Models Generalize Procedures Across Representations?](https://arxiv.org/abs/2602.03542)
*Fangru Lin,Valentin Hofmann,Xingchen Wan,Weixing Wang,Zifeng Ding,Anthony G. Cohn,Janet B. Pierrehumbert*

Main category: cs.CL

TL;DR: 研究表明，大型语言模型（LLMs）在符号表示（代码、图）和自然语言任务之间泛化能力不足。文章提出了一个两阶段的数据课程（先符号后自然语言），显著提高了模型在不同表示下的性能，并将其成功归因于模型泛化能力的提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型在符号表示（如代码和图）上进行了大量训练和测试，但实际用户任务通常用自然语言表述。研究旨在探究 LLMs 在这些不同表示之间泛化能力的程度，并解决其局限性。

Method: 研究者通过构建涉及代码、图和自然语言的同构任务（例如，规划中的调度步骤），来评估 LLMs 的泛化能力。他们尝试了流行的后训练方法，并提出了一种两阶段的数据课程：首先在符号数据上训练，然后是自然语言数据。他们还分析了跨表示泛化的机制，并将其解释为生成类比。

Result: 单独在图或代码数据上进行后训练，并不能可靠地泛化到相应的自然语言任务。仅在自然语言上训练会导致效率不高的性能提升。所提出的两阶段数据课程显著提高了模型在不同模型家族和任务上的性能。使用该方法训练的一个 1.5B Qwen 模型在自然语言规划任务上，性能接近零样本的 GPT-4o。

Conclusion: 通过两阶段的数据课程，可以有效提升 LLMs 在代码、图和自然语言表示之间的泛化能力。这种成功可以被理解为模型学会了生成类比，而课程设计能够有效地促进这一点。该方法在提高模型性能方面具有显著效果，甚至能让小型模型在某些任务上接近大型模型。
...
### [229] [Assessing the Impact of Typological Features on Multilingual Machine Translation in the Age of Large Language Models](https://arxiv.org/abs/2602.03551)
*Vitalii Hirak,Jaap Jumelet,Arianna Bisazza*

Main category: cs.CL

TL;DR: 研究发现，除了训练资源不均外，语言的类型学特征也是影响多语言模型翻译质量的重要因素，尤其是在大型预训练模型中。某些类型学特征的语言可能更适合采用非标准的解码策略。


<details>
  <summary>Details</summary>
Motivation: 现有研究多基于小型模型或从头训练的双语模型，未能充分了解大型预训练多语言模型中语言类型学对翻译质量的影响。因此，本研究旨在分析两个最先进的大型预训练多语言翻译模型（NLLB-200 和 Tower+），以更全面地理解语言类型学的作用。

Method: 分析了两个大型预训练多语言翻译模型（NLLB-200 和 Tower+）在广泛语言上的翻译质量。通过控制数据资源和书写脚本等因素，研究语言类型学特征与翻译质量之间的关系，并探索了输出空间搜索范围对不同类型学语言的影响。

Result: 研究表明，目标语言的类型学特征会驱动翻译质量，即使在控制了数据资源和书写脚本等因素后也是如此。此外，某些类型学特征的语言在更广泛的输出空间搜索中获益更多。

Conclusion: 语言类型学是影响大型多语言模型翻译质量的关键因素。对于某些类型学特征的语言，可能需要采用超越标准左到右的束搜索（beam search）的替代解码策略来提高翻译效果。研究还发布了FLORES+ MT评估基准中212种语言的详细类型学特征，以促进未来研究。
...
### [230] [HySparse: A Hybrid Sparse Attention Architecture with Oracle Token Selection and KV Cache Sharing](https://arxiv.org/abs/2602.03560)
...
TL;DR: 本文提出了一种名为 HySparse 的新架构，它将全注意力层与稀疏注意力层交错排列，并利用全注意力层的输出指导稀疏注意力层的计算，从而在降低计算量和显存占用的同时提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法存在依赖额外代理预测 token 重要性导致复杂度和性能下降，以及虽然降低计算量但未能节省 KV 缓存的问题。 HySparse 旨在解决这些局限性。

Method: HySparse 架构将每个全注意力层与多个稀疏注意力层交错。每个稀疏注意力层的 token 选择和 KV 缓存直接从前一个全注意力层导出。全注意力层充当“神谕”来识别重要 token，并且稀疏注意力层可以重用全注意力层的 KV 缓存。

Result: 在 7B 密集模型和 80B MoE 模型上评估 HySparse。结果显示，HySparse 始终优于全注意力模型和 SWA 基线。特别是在 80B MoE 模型中，仅有 5 层采用全注意力，HySparse 实现了显著的性能提升，同时 KV 缓存存储减少了近 10 倍。

Conclusion: HySparse 是一种有效的混合注意力架构，通过利用全注意力层的输出来指导稀疏注意力层的计算，成功地降低了计算复杂度和内存占用，同时提升了模型性能，克服了现有稀疏注意力方法的缺点。
...
### [231] [ACL: Aligned Contrastive Learning Improves BERT and Multi-exit BERT Fine-tuning](https://arxiv.org/abs/2602.03563)
*Wei Zhu*

Main category: cs.CL

TL;DR: 本文提出了一种名为ACL（Aligned Contrastive Learning）的框架，用于解决监督式对比学习中交叉熵损失与对比学习目标之间的冲突问题，并进一步提出ACL-CL来增强多出口BERT的中间层性能，实验证明ACL-BRT在GLUE基准测试中表现优异，ACL-CL能显著提升多出口BERT的微调性能。


<details>
  <summary>Details</summary>
Motivation: 在监督学习设置下，对比学习（CL）的交叉熵损失（CE）目标与CL目标之间常存在冲突，阻碍了CL在监督学习中的应用。本文旨在解决这一问题，并提升多出口BERT的中间层性能。

Method: 提出了两种方法：1. ACL-Embed，将标签嵌入视为额外增强样本，通过对比学习将其与样本表示对齐。2. ACL-Grad，当CE和ACL-Embed目标冲突时，自动丢弃ACL-Embed项。此外，为了增强多出口BERT的中间层性能，提出了跨层ACL（ACL-CL），让教师出口指导学生浅层出口的优化。

Result: 在GLUE基准测试上，ACL-BRT的性能优于或等于CE和CE+SCL。特别是CL-ACL，在多出口BERT的微调上显著优于基线方法，为低延迟应用提供了更好的速度-质量权衡。

Conclusion: ACL框架成功解决了监督式对比学习中的目标冲突问题，并显著提升了多出口BERT模型的性能，尤其是在对延迟敏感的应用中，提供了更好的效率和准确性。ACL-CL能够有效地利用教师出口来指导学生出口的学习。
...
### [232] [Use Graph When It Needs: Efficiently and Adaptively Integrating Retrieval-Augmented Generation with Graphs](https://arxiv.org/abs/2602.03578)
*Su Dong,Qinggang Zhang,Yilin Xiao,Shengyuan Chen,Chuang Zhou,Xiao Huang*

Main category: cs.CL

TL;DR: 本文提出了一种名为EA-GraphRAG的自适应框架，通过语法分析动态选择使用纯检索增强生成（RAG）还是图增强RAG（GraphRAG），以解决GraphRAG在处理简单查询时效率低下和延迟高的问题，并在多项QA基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的GraphRAG方法在处理复杂查询时能提升性能，但在处理简单查询时效率低下且延迟高，导致在混合查询场景下整体表现不如纯RAG。本文旨在解决GraphRAG在不同查询复杂度下表现不一致的问题。

Method: EA-GraphRAG通过以下三个步骤动态集成RAG和GraphRAG：1. 语法特征构造器：解析查询并提取结构化特征。2. 轻量级复杂度评分器：根据特征计算查询的复杂度得分。3. 得分驱动的路由策略：对低复杂度查询使用密集RAG，对高复杂度查询使用基于图的检索，对中间情况使用复杂度感知互惠排名融合。

Result: EA-GraphRAG在包括单跳和多跳QA的基准测试中，显著提高了准确性，降低了延迟，并在处理混合简单和复杂查询的场景中达到了最先进的性能。

Conclusion: EA-GraphRAG通过自适应地整合RAG和GraphRAG，有效解决了GraphRAG在处理不同复杂度查询时的效率和延迟问题，并在实际应用中展现出优越的性能。
...
### [233] [$V_0$: A Generalist Value Model for Any Policy at State Zero](https://arxiv.org/abs/2602.03584)
*Yi-Kai Zhang,Zhiyuan Yao,Hongyan Hao,Yueqing Sun,Qi Gu,Hui Su,Xunliang Cai,De-Chuan Zhan,Han-Jia Ye*

Main category: cs.CL

TL;DR: 本文提出了一种名为 $V_0$ 的通用价值模型，它可以估计任何模型在未见过提示上的预期性能，而无需参数更新。该模型通过将策略的动态能力视为显式上下文输入，利用指令-性能对的历史来动态分析模型，从而摆脱了依赖参数拟合来感知能力变化的传统范式。在 GRPO 训练中，$V_0$ 可以在 rollout 前预测成功率，用于高效的采样预算分配；在部署中，它可以作为路由器，将指令分发给成本效益最高且最合适的模型。


<details>
  <summary>Details</summary>
Motivation: 传统的 Actor-Critic 方法（如 PPO）在训练 LLM 时，需要一个与策略模型大小相当的价值模型（Critic）来估计基线。然而，随着策略的不断演进，价值模型需要昂贵的同步增量训练来跟踪策略能力的变化。GRPO 虽消除了耦合的价值模型，但需要大量的采样来维持估计的稳定性。因此，研究者希望找到一种更高效的价值估计方法，以降低训练和部署 LLM 的成本。

Method: 提出了一种名为 $V_0$ 的通用价值模型，它不需要参数更新。该模型将策略的动态能力作为显式上下文输入，并利用历史的指令-性能对来动态分析模型。在 GRPO 训练中，$V_0$ 用于预测 rollout 前的成功率，以指导采样预算的分配。在部署阶段，$V_0$ 可作为路由器，将指令分配给最合适的模型。

Result: $V_0$ 模型在 LLM 训练和部署方面均取得了显著效果。在训练方面，它通过更有效的采样预算分配，提高了训练效率。在部署方面，它在性能和成本之间实现了帕累托最优的权衡，显著优于启发式预算分配方法。

Conclusion: $V_0$ 模型是一种创新的价值估计方法，它能够无需参数更新地动态评估 LLM 的性能。该模型在提高 LLM 训练效率和优化部署策略方面具有重要应用价值，尤其是在资源受限或需要快速响应的场景下。
...
### [234] [CL-bench: A Benchmark for Context Learning](https://arxiv.org/abs/2602.03587)
...
TL;DR: 研究提出了CL-bench基准，用于评估语言模型在复杂、特定任务场景下学习和运用新知识（上下文学习）的能力。现有模型在这一基准上的表现不佳，表明上下文学习是当前语言模型面临的关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现实世界的任务比预训练语言模型擅长处理的更复杂、更依赖上下文，需要模型从特定任务的上下文和预训练之外的新知识中学习，而现有研究忽视了这种“上下文学习”能力。

Method: 创建了CL-bench基准，包含500个复杂上下文、1899个任务和31607个验证标准，由领域专家设计。任务要求模型从上下文学习新知识（领域知识、规则系统、复杂流程、数据驱动的法则等），这些知识在预训练中不存在。

Result: 在CL-bench基准上，10个前沿语言模型的平均任务解决率仅为17.2%，最佳模型GPT-5.1也仅为23.7%。这表明当前语言模型在上下文学习方面表现不佳。

Conclusion: 上下文学习是语言模型在现实世界复杂任务中面临的关键瓶颈。CL-bench基准是衡量和推动语言模型实现这一能力的重要工具，有助于提升模型的智能水平和实际应用能力。
...
### [235] [Efficient Algorithms for Partial Constraint Satisfaction Problems over Control-flow Graphs](https://arxiv.org/abs/2602.03588)
...
### [236] [Controlling Output Rankings in Generative Engines for LLM-based Search](https://arxiv.org/abs/2602.03608)
*Haibo Jin,Ruoxi Chen,Peiyan Zhang,Yifeng Luo,Huimin Zeng,Man Luo,Haohan Wang*

Main category: cs.CL

TL;DR: 提出了一种名为CORE的优化方法，通过修改LLM搜索到的内容来控制生成式引擎的输出排名，以解决LLM搜索中排名偏见的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）驱动的搜索（生成式引擎）直接提供产品推荐，但其排名顺序受到LLM检索顺序的影响，可能导致小企业和独立创作者曝光不足。

Method: CORE通过向LLM检索到的内容中添加优化内容（包括字符串、推理和评论类型）来操纵输出排名。同时，引入了一个名为ProductBench的大规模基准数据集来评估该方法。

Result: 在四种LLM（GPT-4o, Gemini-2.5, Claude-4, Grok-3）上的实验表明，CORE在15个产品类别中平均取得了91.4%@Top-5, 86.6%@Top-3, 和 80.3%@Top-1的推广成功率，优于现有方法且不影响输出内容的流畅性。

Conclusion: CORE是一种有效控制LLM生成式引擎输出排名的优化方法，能够提升目标内容的可见性，并为解决LLM搜索中的排名偏见问题提供了一种新途径。
...
### [237] [Learning Query-Specific Rubrics from Human Preferences for DeepResearch Report Generation](https://arxiv.org/abs/2602.03619)
...
TL;DR: 本研究提出了一种自动生成用于评估深度研究报告的查询特定评分标准的方法，通过结合人类偏好和大型语言模型（LLM）进行训练，并将其集成到一个多智能体工作流中，显著提升了报告生成系统的性能。


<details>
  <summary>Details</summary>
Motivation: 当前训练和评估深度研究报告生成模型面临缺乏可验证奖励信号的挑战，现有的基于评分标准的方法要么过于粗糙，要么需要手动构建，成本高昂且难以扩展。

Method: 研究人员构建了一个包含人类偏好标注的深度研究查询数据集，并采用强化学习训练评分标准生成器，结合了人类偏好监督和基于LLM的评分标准评估。此外，引入了多智能体马尔可夫状态（MaMs）工作流来处理长远推理。

Result: 实验表明，所提出的评分标准生成器比现有方法能提供更具辨别力和更符合人类偏好的监督信号。当集成到MaMs框架后，配备这些评分标准生成器的深度研究系统在DeepResearch Benchmark上的性能超越了所有开源基线，并达到了与领先的闭源模型相当的水平。

Conclusion: 本研究提出的自动评分标准生成管道和多智能体工作流能够有效地解决深度研究报告生成中的评估难题，并显著提升模型的性能，为相关领域的研究提供了新的解决方案。
...
### [238] [TRE: Encouraging Exploration in the Trust Region](https://arxiv.org/abs/2602.03635)
...
TL;DR: 提出了一种名为信任区域熵（TRE）的新方法，用于解决大型语言模型（LLMs）在强化学习（RL）中由于累积尾部风险导致熵正则化效果不佳的问题，并在多项任务上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的熵正则化技术在大型语言模型（LLMs）的强化学习（RL）应用中效果不佳甚至损害性能。研究人员发现这是由于LLMs拥有庞大的词汇量和长的生成过程，存在累积尾部风险。

Method: 提出了一种名为信任区域熵（TRE）的新方法。TRE鼓励在模型的信任区域内进行探索，避免了将概率质量分散到无效标记的巨大尾部。该方法在数学推理（MATH）、组合搜索（Countdown）和偏好对齐（HH）等任务上进行了实验。

Result: 在MATH、Countdown和HH任务上的实验结果表明，TRE的表现 consistently 优于 vanilla PPO、标准的熵正则化以及其他探索基线方法。

Conclusion: TRE是一种有效的方法，可以解决大型语言模型在强化学习中熵正则化失效的问题，通过在信任区域内进行探索来提升模型性能。
...
### [239] [RAGTurk: Best Practices for Retrieval Augmented Generation in Turkish](https://arxiv.org/abs/2602.03652)
...
TL;DR: 本研究构建了一个土耳其语的检索增强生成（RAG）数据集，并评估了RAG流水线中不同组件对土耳其语事实性的影响，发现复杂方法能提高准确性，而过度堆叠生成模块可能损害性能。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG设计指南主要针对英语，缺乏对像土耳其语这样形态丰富的语言的指导，因此需要为土耳其语开发RAG方法。

Method: 研究人员构建了一个包含问答对和相关文本块的土耳其语RAG数据集（来源于土耳其维基百科和CulturaX）。他们对RAG流水线中的七个阶段（查询转换、重排序、答案精炼等）进行了基准测试，未使用特定任务的微调。

Result: 复杂方法（如HyDE）可将准确性提高到85%，显著优于基线（78.70%）。一种使用交叉编码器重排序和上下文增强的帕累托最优配置也能达到84.60%的性能，但成本较低。研究还发现，过度使用生成模块会损害性能，而简单的查询澄清和鲁棒的重排序是有效的解决方案。

Conclusion: 对于土耳其语等形态丰富的语言，RAG流水线的设计需要谨慎，复杂方法能提升事实性，但应避免过度堆叠生成模块，并结合有效的查询澄清和重排序策略。
...
### [240] [Instruction Anchors: Dissecting the Causal Dynamics of Modality Arbitration](https://arxiv.org/abs/2602.03677)
*Yu Zhang,Mufan Xu,Xuefeng Bai,Kehai chen,Pengfei Zhang,Yang Xiang,Min Zhang*

Main category: cs.CL

TL;DR: 该研究通过信息流视角深入探讨了多模态大语言模型（MLLMs）的模态跟随机制，揭示了指令代币在模态选择中的关键作用，并提出了一种通过操纵特定注意力头来调控模态跟随行为的方法。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs的模态跟随能力（根据用户指令选择性地利用多模态上下文）对其安全性和可靠性至关重要，但其内在机制尚不明确，因此需要对其进行深入研究。

Method: 研究者从信息流角度分析了MLLMs的模态跟随机制，考察了不同层（浅层和深层注意力层、MLP层）的作用，并识别了驱动模态选择的关键注意力头。通过因果干预实验，他们操纵了这些关键注意力头来观察其对模态跟随比例的影响。

Result: 研究发现，指令代币充当模态仲裁的结构锚点；浅层注意力进行非选择性信息传递，深层注意力根据指令意图解决模态竞争；MLP层表现出语义惯性。识别出的少数（5%）特异性注意力头对模态跟随至关重要，操纵这些头可使模态跟随比例变化高达60%。

Conclusion: 该研究揭示了MLLMs模态跟随的内在机制，为模型透明度提供了重要见解，并提出了一个用于协调多模态信息处理的原则性框架，为增强MLLMs的可控性和可靠性奠定了基础。
...
### [241] [Neural Attention Search Linear: Towards Adaptive Token-Level Hybrid Attention Models](https://arxiv.org/abs/2602.03681)
*Difan Deng,Andreas Bentzen Winje,Lukas Fehring,Marius Lindauer*

Main category: cs.CL

TL;DR: 提出了一种名为NAtS-L的神经注意力搜索框架，通过在同一层内动态地在不同token之间组合使用线性注意力和softmax注意力，以克服Transformer在长序列场景下的二次计算复杂性问题，同时保持模型的表达能力。


<details>
  <summary>Details</summary>
Motivation: Transformer模型中的softmax注意力在处理长序列时存在二次计算复杂度瓶颈。线性注意力模型虽然效率更高，但表达能力受限于隐藏状态的大小。现有方法通过交错softmax和线性注意力层来平衡效率和表达能力，但softmax层仍然是效率瓶颈。

Method: 提出NAtS-L框架，该框架能够自动确定每个token是使用线性注意力（适用于短期影响、可编码到固定大小隐藏状态的token）还是softmax注意力（适用于需要长期检索信息的token）。通过搜索Gated DeltaNet和softmax注意力的最优组合，实现了token级别的混合注意力架构。

Result: NAtS-L框架在token级别有效地结合了线性注意力和softmax注意力，证明了其作为一种高效且强大的混合注意力架构的潜力。

Conclusion: NAtS-L通过在同一层内智能地组合线性注意力和softmax注意力，为解决长序列Transformer的计算效率和表达能力问题提供了一种新的、有效的解决方案。
...
### [242] [BIRDTurk: Adaptation of the BIRD Text-to-SQL Dataset to Turkish](https://arxiv.org/abs/2602.03633)
*Burak Aktaş,Mehmet Can Baytekin,Süha Kağan Köse,Ömer İlbilgi,Elif Özge Yılmaz,Çağrı Toraman,Bilge Kaan Görür*

Main category: cs.CL

TL;DR: 该研究介绍了 BIRDTurk，一个土耳其语的 Text-to-SQL 基准测试，并评估了不同方法在该语言上的表现，发现土耳其语的语言结构和预训练数据的不足导致性能下降，但智能体式推理表现出更好的跨语言鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的 Text-to-SQL 系统在英语上表现良好，但在形态丰富、低资源语言上的表现未得到充分研究，特别是土耳其语。因此，需要一个专门的土耳其语基准来评估和改进这些系统。

Method: 研究者构建了一个名为 BIRDTurk 的土耳其语 Text-to-SQL 基准。他们使用受控翻译流程，在保留 SQL 查询和数据库逻辑结构及执行语义的同时，将模式标识符翻译成土耳其语。通过中心极限定理确定的样本量进行翻译质量验证，并在此基准上评估了推断式提示、多阶段智能体推理和监督微调等方法。

Result: 在 BIRDTurk 基准上，土耳其语 Text-to-SQL 性能出现了一致的下降，这归因于语言结构差异和预训练数据不足。智能体式推理在跨语言任务上表现出更强的鲁棒性。标准多语言基线模型的监督微调仍具挑战，但对于现代指令微调模型效果显著。

Conclusion: BIRDTurk 提供了一个在真实数据库条件下进行跨语言 Text-to-SQL 评估的受控测试平台。研究结果表明，土耳其语的语言特性和 LLM 预训练数据的不足是当前 Text-to-SQL 系统在该语言上性能不佳的主要原因，而智能体式推理和现代指令微调模型在克服这些挑战方面显示出潜力。
...
### [243] [Rethinking the Reranker: Boundary-Aware Evidence Selection for Robust Retrieval-Augmented Generation](https://arxiv.org/abs/2602.03689)
...
TL;DR: 提出了一种名为BAR-RAG的新型检索增强生成（RAG）方法，通过将重排器重新构建为边界感知证据选择器，旨在为生成器提供“金发姑娘区域”的证据，即既不太容易也不至于无法回答，但足够具有挑战性且信息充分，从而提供最强的学习信号。BAR-RAG利用生成器反馈通过强化学习训练选择器，并采用两阶段流水线微调生成器以解决训练和推理时的分布不匹配问题。实验证明，BAR-RAG在嘈杂检索条件下显著提升了端到端性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成（RAG）系统在面对实际检索噪声时表现脆弱，即使所需证据存在于前K个结果中。主要原因是检索器和重排器仅优化相关性，可能选择过于简单或缺乏关键信息的文本，而没有考虑证据是否适合生成器。这导致RAG系统性能不稳定。

Method: BAR-RAG将重排器视为一个边界感知证据选择器，旨在为生成器找到“金发姑娘区域”的证据（既不太容易也不至于无法回答，但足够用于推理并提供最强的学习信号）。该方法使用生成器的反馈通过强化学习来训练选择器，并采用一个两阶段的流水线：首先，重排器根据生成器的反馈选择证据；然后，在诱导的证据分布下微调生成器，以缓解训练和推理之间的分布不匹配问题。

Result: 在知识密集型问答基准测试中，BAR-RAG在嘈杂检索条件下始终提高了端到端性能，平均比强大的RAG和重排基线提高了10.3个百分点，并显著增强了鲁棒性。

Conclusion: BAR-RAG通过将重排器重新定位为生成器优化的边界感知证据选择器，并采用两阶段的训练策略，有效解决了RAG系统在噪声检索下的脆弱性问题，显著提高了问答性能和鲁棒性。
...
### [244] [OCRTurk: A Comprehensive OCR Benchmark for Turkish](https://arxiv.org/abs/2602.03693)
*Deniz Yılmaz,Evren Ayberk Munis,Çağrı Toraman,Süha Kağan Köse,Burak Aktaş,Mehmet Can Baytekin,Bilge Kaan Görür*

Main category: cs.CL

TL;DR: 本文介绍了OCRTurk，一个针对土耳其语文档解析任务的基准测试集，该测试集包含多种文档类型和难度级别，并评估了七个OCR模型在其中的表现。PaddleOCR在整体上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有文档解析模型的基准测试主要针对高资源语言，对土耳其语等低资源语言的覆盖不足，且缺乏反映真实世界场景和多样性的标准化测试。因此，需要一个针对土耳其语的、更全面的文档解析基准。

Method: 创建了一个名为OCRTurk的土耳其语文档解析基准测试集，该测试集包含180份来自不同来源（学术文章、论文、幻灯片、非学术文章）的土耳其语文档，分为三个难度级别。并使用该基准测试集评估了七个OCR模型，采用逐元素度量和归一化编辑距离进行评估。

Result: PaddleOCR在OCRTurk基准测试中取得了最强的总体结果，在大多数逐元素度量上领先（除了图形），并在所有难度级别上都获得了较高的归一化编辑距离分数。不同文档类型的模型表现存在差异，非学术文档表现最好，幻灯片则最具挑战性。

Conclusion: OCRTurk为土耳其语文档解析提供了一个全面的基准，有助于评估和改进OCR模型在低资源语言场景下的性能。研究结果表明，PaddleOCR在当前测试中表现最优，但模型的性能受文档类型影响显著，需要进一步优化以应对更具挑战性的文档。
...
### [245] [Cognitively Diverse Multiple-Choice Question Generation: A Hybrid Multi-Agent Framework with Large Language Models](https://arxiv.org/abs/2602.03704)
*Yu Tian,Linh Huynh,Katerina Christhilf,Shubham Chakraborty,Micah Watanabe,Tracy Arner,Danielle McNamara*

Main category: cs.CL

TL;DR: 提出了一种名为 ReQUESTA 的混合多智能体框架，用于生成具有可控认知要求的选择题（MCQ），相较于单一模型，ReQUESTA 生成的题目更具挑战性、区分度，并与阅读理解能力更一致。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM在生成具有特定认知需求的选择题方面仍存在挑战，需要一种更可靠的方法来生成符合特定认知要求的问题。

Method: ReQUESTA是一个混合多智能体框架，将MCQ生成分解为多个子任务，并协调LLM智能体和基于规则的组件进行规划、受控生成、迭代评估和后处理。

Result: ReQUESTA生成的题目在难度和区分度上优于GPT-5零样本基线，并且与整体阅读理解表现更一致。专家评估也显示ReQUESTA生成的题目在与中心概念的对齐度以及干扰项的语言一致性和语义合理性方面更优。

Conclusion: 混合多智能体的协同工作可以系统地提高LLM生成MCQ的可靠性和可控性，证明了工作流程设计在结构化内容生成中的重要性。
...
### [246] [OmniRAG-Agent: Agentic Omnimodal Reasoning for Low-Resource Long Audio-Video Question Answering](https://arxiv.org/abs/2602.03707)
*Yifan Zhu,Xinyu Mu,Tao Feng,Zhonghong Ou,Yuning Gong,Haoran Luo*

Main category: cs.CL

TL;DR: 提出了一种名为OmniRAG-Agent的代理式全模态问答方法，用于在低资源条件下对长音频-视频进行推理，通过检索增强生成和代理循环来提高效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有全模态长音频-视频问答方法存在资源消耗大、检索细粒度弱、规划能力有限以及缺乏端到端优化等问题。

Method: 构建了一个检索增强生成模块，使全模态语言模型能够从外部库中获取相关的短帧和音频片段。利用代理循环进行规划、跨轮调用工具以及合并检索到的证据来回答复杂查询。采用群体相对策略优化来协同提升工具使用和答案质量。

Result: 在OmniVideoBench、WorldSense和Daily-Omni数据集上，OmniRAG-Agent在低资源设置下持续优于现有方法，并取得了强大的结果。消融实验验证了各个组件的有效性。

Conclusion: OmniRAG-Agent是一种有效的低资源长音频-视频推理方法，通过集成检索、代理和优化技术，显著提高了问答性能。
...
### [247] [Beyond Tokens: Semantic-Aware Speculative Decoding for Efficient Inference by Probing Internal States](https://arxiv.org/abs/2602.03708)
*Ximing Dong,Shaowei Wang,Dayi Lin,Boyuan Chen,Ahmed E. Hassan*

Main category: cs.CL

TL;DR: 提出了一种名为 SemanticSpec 的语义感知推断加速框架，通过并行验证整个语义序列而非单个 token，显著提高了大型语言模型（特别是大型推理模型）的推断效率。


<details>
  <summary>Details</summary>
Motivation: 现有的投机解码方法在加速 LLM 推断时，由于仅在 token 层面进行验证，忽略了语义等价性，导致不必要的拒绝，效率低下。对于生成长思考链的大型推理模型（LRM）而言，这一问题尤为突出。

Method: SemanticSpec 框架引入了一种语义概率估计机制，利用模型的内部隐藏状态来评估生成特定语义序列的可能性，并以此为基础验证整个语义序列，而非逐个 token。

Result: 在四个基准测试中，SemanticSpec 在 DeepSeekR1-32B 上实现了高达 2.7 倍的速度提升，在 QwQ-32B 上实现了 2.1 倍的速度提升，并且在效率和效果上均优于现有的 token 级和序列级基线方法。

Conclusion: SemanticSpec 是一种有效的语义感知推断加速框架，通过语义级别的验证，能够比现有的方法更有效地加速 LLM 的推断过程，尤其是在处理大型推理模型时。
...
### [248] [No Shortcuts to Culture: Indonesian Multi-hop Question Answering for Complex Cultural Understanding](https://arxiv.org/abs/2602.03709)
*Vynska Amalia Permadi,Xingwei Tan,Nafise Sadat Moosavi,Nikos Aletras*

Main category: cs.CL

TL;DR: 本文提出了ID-MoCQA，一个大规模、多跳的文化问答数据集，用于评估大型语言模型（LLMs）在印尼文化理解方面的能力。该数据集通过将单跳问题转化为跨越多种线索类型的多跳推理链来构建，旨在解决现有文化问答基准的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的文化问答基准主要依赖单跳问题，这可能导致模型仅依靠浅层线索而无法展示真正的文化推理能力。研究者希望创建一个更具挑战性的基准来评估和提升LLMs的文化理解能力。

Method: 研究者构建了一个框架，将单跳文化问题系统性地转化为多跳推理链，并定义了六种线索类型（如常识、时间、地理）。通过专家评审和LLM-as-a-judge过滤的多阶段验证流程来确保问答对的质量。最后，使用该数据集对现有模型进行了评估。

Result: 评估结果显示，现有最先进的模型在文化推理方面存在显著差距，尤其是在需要细微推断的任务上。

Conclusion: ID-MoCQA是一个具有挑战性且重要的基准，能够促进LLMs文化能力的发展，并揭示了当前模型在文化推理方面的不足。
...
### [249] [Training Multi-Turn Search Agent via Contrastive Dynamic Branch Sampling](https://arxiv.org/abs/2602.03719)
*Yubao Zhao,Weiquan Huang,Sudong Wang,Ruochen Zhao,Chen Chen,Yao Shu,Chengwei Qin*

Main category: cs.CL

TL;DR: 提出了一种名为 BranPO 的新方法，通过引入步长级别的对比监督来解决强化学习在长时序任务中的挑战，并在多个问答基准测试中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 长时序任务的稀疏奖励使得强化学习模型难以有效学习；现有的树状方法存在方差高和计算效率低的问题。研究观察到性能差异主要源于决策的后期，这促使研究者探索一种新的方法来解决信用分配模糊的问题。

Method: BranPO 是一种无价值函数的方法，通过截断轨迹的后期并重采样替代的后续路径来构建对比性后缀，从而在共享前缀上提供步长级别的对比监督。此外，还引入了难度感知分支采样和冗余步掩码来提高效率和稳定性。

Result: BranPO 在多个问答基准测试中，在长时序任务上取得了显著的准确率提升，并且没有增加总训练预算，同时优于现有的强基线方法。

Conclusion: BranPO 是一种有效的、无价值函数的方法，通过引入步长级别的对比监督，能够解决长时序强化学习中的稀疏奖励问题，并在实践中取得了优于现有方法的性能。
...
### [250] [CUBO: Self-Contained Retrieval-Augmented Generation on Consumer Laptops 10 GB Corpora, 16 GB RAM, Single-Device Deployment](https://arxiv.org/abs/2602.03731)
*Paolo Astrino*

Main category: cs.CL

TL;DR: 本文提出 CUBO，一个专为消费级笔记本设计的系统导向的 RAG 平台，能在 16GB RAM 下实现数据最小化和 GDPR 合规，并达到与云端系统相当的检索性能。


<details>
  <summary>Details</summary>
Motivation: 在处理敏感文档时，企业面临着使用云端 AI 可能违反 GDPR 的风险，而本地系统需要高昂的内存（18-32GB RAM）。本研究旨在解决这一矛盾，提供一个低资源需求且符合隐私法规的解决方案。

Method: CUBO 平台通过工程集成实现了流式数据摄入（O(1) 缓冲开销）、分层混合检索以及硬件感知编排，从而在严格的 15.5 GB RAM 限制内实现了具有竞争力的 Recall@10 指标。

Result: CUBO 实现了 0.48-0.97 的 Recall@10（跨 BEIR 领域），在 C1,300 笔记本上实现了 185 毫秒（p50）的检索延迟。代码库约 37,000 行。

Conclusion: CUBO 证明了在消费级笔记本上构建一个满足 GDPR 要求、低资源消耗且性能优越的 RAG 平台是可行的，适用于中小型专业档案的实际部署。
...
### [251] [Accelerating Scientific Research with Gemini: Case Studies and Common Techniques](https://arxiv.org/abs/2602.03837)
*David P. Woodruff,Vincent Cohen-Addad,Lalit Jain,Jieming Mao,Song Zuo,MohammadHossein Bateni,Simina Branzei,Michael P. Brenner,Lin Chen,Ying Feng,Lance Fortnow,Gang Fu,Ziyi Guan,Zahra Hadizadeh,Mohammad T. Hajiaghayi,Mahdi JafariRaviz,Adel Javanmard,Karthik C. S.,Ken-ichi Kawarabayashi,Ravi Kumar,Silvio Lattanzi,Euiwoong Lee,Yi Li,Ioannis Panageas,Dimitris Paparas,Benjamin Przybocki,Bernardo Subercaseaux,Ola Svensson,Shayan Taherijam,Xuan Wu,Eylon Yogev,Morteza Zadimoghaddam,Samson Zhou,Vahab Mirrokni*

Main category: cs.CL

TL;DR: 本研究展示了如何利用LLM（特别是Google的Gemini模型）与研究人员合作，在理论计算机科学、经济学、优化和物理学等领域解决开放性问题、反驳猜想和生成新证明，并总结了有效的人工智能协作技术。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在数学发现等需要专业知识的新颖研究领域的潜力，超越其在常规任务中的应用。

Method: 通过案例研究，展示研究人员如何与Gemini模型互动，共同解决科学问题。提取了迭代优化、问题分解和跨学科知识转移等协作技术。还探索了将模型用作对抗性评审者以及在神经符号循环中自主执行代码以验证推导的方法。

Result: 成功利用AI与研究人员合作，解决了多个领域的开放性问题、反驳了猜想并生成了新证明。揭示了AI在理论研究中作为强大合作伙伴的潜力。

Conclusion: AI不仅是自动化工具，更是科学发现创造过程中的多功能、真正意义上的合作伙伴，可以通过有效的协作技术促进跨学科研究的进步。
...
### [252] [Context Compression via Explicit Information Transmission](https://arxiv.org/abs/2602.03784)
*Jiangnan Ye,Hanqi Yan,Zhenyi Shen,Heng Chang,Ye Mao,Yulan He*

Main category: cs.CL

TL;DR: 提出了一种名为ComprExIT的轻量级框架，通过显式信息传输来压缩长上下文，以应对LLM长上下文推理中二次注意力和键值缓存增长带来的成本问题。该方法通过深度传输聚合多层信息到token锚点，并宽度传输将锚点聚合到少量插槽中，从而解决了现有方法中表示被覆盖和压缩容量分配不协调的问题。实验证明，ComprExIT在六个问答基准测试中优于现有SOTA方法，且参数量增加很少。


<details>
  <summary>Details</summary>
Motivation: 现有的长上下文压缩方法在LLM中存在两个结构性限制：(i)层层渐进式表示覆盖（progressive representation overwriting）和(ii)跨token压缩容量分配不协调（uncoordinated allocation of compression capacity across tokens），导致成本高昂且效果不佳。

Method: 提出ComprExIT框架，将软上下文压缩重塑为在冻结的LLM隐藏状态上进行显式信息传输的新范式。该框架包含两个关键机制：(i)深度（depth-wise）传输，将多层信息选择性地传输到token锚点，以缓解渐进式覆盖；(ii)宽度（width-wise）传输，通过全局优化的传输计划将锚点聚合到少量插槽中，实现信息分配的协调。

Result: ComprExIT在六个问答基准测试中一致优于现有的上下文压缩方法，并且只引入了约1%的额外参数。

Conclusion: 显式和协调的信息传输是实现更有效和鲁棒的长上下文压缩的关键。ComprExIT框架通过解耦压缩过程与模型内部自注意力动态，成功克服了现有方法的结构性限制，证明了其有效性。
...
### [253] [They Said Memes Were Harmless-We Found the Ones That Hurt: Decoding Jokes, Symbols, and Cultural References](https://arxiv.org/abs/2602.03822)
*Sahil Tripathi,Gautam Siddharth Kashyap,Mehwish Nasim,Jian Yang,Jiechao Gao,Usman Naseem*

Main category: cs.CL

TL;DR: 提出了一种名为CROSS-ALIGN+的三阶段框架，用于检测基于模因的社会滥用，通过整合结构化知识、使用LoRA微调模型和生成解释来解决文化盲点、边界模糊和可解释性差的问题，并在多个基准测试中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的模因滥用检测方法存在文化理解不足、区分讽刺与滥用困难以及模型决策过程不透明等问题，导致其检测效果受限。

Method: 该研究提出了一个三阶段框架CROSS-ALIGN+：第一阶段通过集成ConceptNet、Wikidata和Hatebase的结构化知识来丰富多模态表示，缓解文化盲点；第二阶段利用参数高效的LoRA适配器来精炼决策边界，减少模糊性；第三阶段通过生成级联解释来提高模型的可解释性。

Result: 在五个基准测试和八个大型视觉语言模型（LVLMs）上的实验表明，CROSS-ALIGN+在性能上持续优于最先进的方法，F1分数相对提升高达17%，并且能够为每个决策提供可解释的理由。

Conclusion: CROSS-ALIGN+框架能够有效解决当前模因滥用检测中的关键挑战，通过多方面改进提升了检测的准确性和可解释性，为该领域的研究提供了新的解决方案。
...
### [254] [Parallel-Probe: Towards Efficient Parallel Thinking via 2D Probing](https://arxiv.org/abs/2602.03845)
...
TL;DR: 本文提出了一种名为 Parallel-Probe 的训练无关的控制器，用于优化并行推理过程。通过 2D 探测机制，该控制器能够洞察并行推理的宽度-深度动态，并利用这些洞察来优化计算效率，在不牺牲准确性的情况下显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有的并行推理方法计算成本高昂，且主要依赖局部信号，未能有效利用全局动态。研究的动机是寻找一种更有效的方法来管理并行推理的计算资源，同时保持甚至提高其性能。

Method: 研究人员引入了“2D 探测”接口，用于暴露并行推理的宽度-深度动态。基于对这些动态的分析，他们开发了 Parallel-Probe 控制器，该控制器采用基于共识的提前停止来调节推理深度，并使用基于偏差的分支剪枝来动态调整宽度。

Result: 在三个基准测试和多个模型上的实验表明，Parallel-Probe 能够建立优越的测试时间扩展帕累托前沿。与标准的多数投票相比，它将顺序 token 减少了高达 35.8%，总 token 成本降低了 25.8% 以上，同时保持了具有竞争力的准确性。

Conclusion: 2D 探测揭示了并行推理的宽度-深度动态，并且这些洞察可以用来设计有效的控制策略。Parallel-Probe 作为一个训练无关的控制器，能够显著提高并行推理的计算效率，并在准确性和计算成本之间取得了更好的权衡。
...
### [255] [HMVLA: Hyperbolic Multimodal Fusion for Vision-Language-Action Models](https://arxiv.org/abs/2602.02533)
*Kun Wang,Xiao Feng,Mingcheng Qu,Tonghua Su*

Main category: cs.RO

TL;DR: 本文提出了一种名为 HMVLA 的新型视觉语言动作（VLA）框架，该框架利用视觉和语言的内在层次结构，在双曲空间中进行多模态特征对齐，并结合稀疏门控专家混合（MoE）机制，以提高多模态理解能力和效率，并在实验中取得了优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型通常直接微调预训练的 VLM，未能充分解决 VLA 领域特有的语义对齐挑战，尤其是在处理图像和文本的层次结构方面。

Method: HMVLA 框架利用视觉和语言的层次结构，将多模态特征嵌入双曲空间以进行更有效的对齐，并引入稀疏门控专家混合（MoE）机制来增强图像和文本之间的多模态理解和效率。

Result: HMVLA 在准确性和泛化性方面均优于现有基线方法。此外，通过重建数据集进行验证，HMVLA 还展现了其在跨领域适应性方面的鲁棒性。

Conclusion: HMVLA 框架通过在双曲空间中进行语义对齐和采用 MoE 机制，有效解决了 VLA 领域的语义对齐问题，显著提升了模型的性能和泛化能力。
...
### [256] [StepNav: Structured Trajectory Priors for Efficient and Multimodal Visual Navigation](https://arxiv.org/abs/2602.02590)
*Xubo Luo,Aodi Wu,Haodong Han,Xue Wan,Wei Zhang,Leizheng Shu,Ruisuo Wang*

Main category: cs.RO

TL;DR: StepNav是一个创新的框架，通过引入结构化的、多模态的轨迹先验，解决了在复杂和不确定环境中生成可靠导航轨迹的挑战，实现了更安全、更高效的规划。


<details>
  <summary>Details</summary>
Motivation: 现有的生成模型在生成可靠、高效且实时的导航轨迹方面存在困难，尤其是在复杂和不确定的环境中。它们依赖于无结构的噪声先验，容易产生不安全、效率低下或单模态的规划。

Method: StepNav首先学习一个几何感知的成功概率场来识别所有可行的导航走廊。然后，利用这些走廊构建一个显式的、多模态的混合先验，用于初始化条件流匹配过程。该过程被表述为一个带有显式平滑度和安全性正则化的最优控制问题。

Result: 与现有生成规划器相比，StepNav在鲁棒性、效率和安全性方面取得了持续的改进，并且规划步数显著减少。在模拟和真实世界基准测试中均验证了其有效性。

Conclusion: StepNav通过用物理约束的候选方案取代无结构的噪声，成功地生成了更安全、更高效的导航计划，显著提高了自主导航的可靠性，为实际应用铺平了道路。
...
### [257] [AROLA: A Modular Layered Architecture for Scaled Autonomous Racing](https://arxiv.org/abs/2602.02730)
*Fam Shihata,Mohammed Abdelazim,Ahmed Hussein*

Main category: cs.RO

TL;DR: 本文提出了AROLA，一个模块化的分层软件架构，用于自主赛车，并引入了Race Monitor框架以进行标准化评估，旨在加速开发和提高可重现性。


<details>
  <summary>Details</summary>
Motivation: 为了应对自主赛车快速发展的需求，现有的软件栈需要进化，以支持更快的模块替换和客观的基准测试，同时提高开发的可重现性。

Method: AROLA将自动驾驶流水线分解为感应、预处理、感知、定位与建图、规划、行为、控制和执行等模块，并通过ROS 2接口连接。Race Monitor框架用于实时记录和生成标准化赛后分析，包括圈速、轨迹质量和计算负载。

Result: AROLA架构在模拟和RoboRacer硬件平台上得到了验证，并在2025 RoboRacer IV25竞赛中部署。模块化、透明的接口和系统化的评估能够加速开发。

Conclusion: AROLA软件架构和Race Monitor框架的结合，证明了模块化、透明的接口和系统化的评估方法可以有效加速小型自主赛车领域的开发进程，并提高研究结果的可重现性。
...
### [258] [PokeNet: Learning Kinematic Models of Articulated Objects from Human Observations](https://arxiv.org/abs/2602.02741)
*Anmol Gupta,Weiwei Gu,Omkar Patil,Jun Ki Lee,Nakul Gopalan*

Main category: cs.RO

TL;DR: PokeNet是一个无需先验知识即可从单个人类演示中学习物体关节模型（包括运动顺序）的端到端框架，在关节轴和状态估计方面比现有方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有机器人关节建模方法依赖先验知识，无法处理被遮挡的关节，需要大量多视图图像，并且忽略了对多自由度物体至关重要的操作顺序。

Method: PokeNet是一个端到端框架，接收人类操纵未知物体点云序列的观察，预测关节参数、推断操作顺序并跟踪关节状态。

Result: PokeNet在关节轴和状态估计方面比现有最先进方法平均提高了27%以上，在模拟和现实世界环境中均表现出色，包括新颖和未见的物体类别。

Conclusion: PokeNet能够从单个人类演示中有效学习未知物体的关节模型，包括操作顺序，解决了现有方法的局限性，并在机器人操作和规划中具有潜在应用价值。
...
### [259] [Bimanual High-Density EMG Control for In-Home Mobile Manipulation by a User with Quadriplegia](https://arxiv.org/abs/2602.02773)
*Jehan Yang,Eleanor Hodgson,Cindy Sun,Zackory Erickson,Doug Weber*

Main category: cs.RO

TL;DR: 该研究开发了一个创新的系统，允许四肢瘫痪的用户通过佩戴集成HDEMG传感器的前臂套来控制家中的移动机械臂，实现了用户驱动的共享自主操作，并成功进行了为期十二天的在家庭环境中的真实场景评估。


<details>
  <summary>Details</summary>
Motivation: 脊髓损伤（cSCI）用户，特别是四肢瘫痪者，由于肢体麻痹，难以使用传统的机器人控制界面来执行家务。因此，需要开发新的控制方法来增强他们的独立性。

Method: 研究人员开发了一对集成HDEMG传感器的织物前臂套，用于捕捉残余神经肌肉活动。该系统集成了视觉、语言和运动规划模块，构建了一个共享自主框架，支持用户通过手势进行机器人遥操作，特别是在导航方面。

Result: 该系统成功实现了用户在家中通过可穿戴EMG接口实时控制移动机械臂，并能够执行日常活动和其他家务任务。

Conclusion: 通过结合可穿戴HDEMG技术、共享自主框架和多模态集成，该系统为四肢瘫痪用户在家中有效控制移动机械臂提供了可行的解决方案，增强了他们的自主生活能力。
...
### [260] [Adaptive Linear Path Model-Based Diffusion](https://arxiv.org/abs/2602.02831)
*Yutaka Shimizu,Masayoshi Tomizuka*

Main category: cs.RO

TL;DR: 提出了一种名为 LP-MBD 的新型基于模型的扩散模型方法，用线性概率路径取代了传统的方差保持调度，简化了参数调整并提高了稳定性。进一步提出 ALP-MBD，利用强化学习自适应调整扩散步长和噪声水平，从而增强了鲁棒性、适应性和实时效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的机器人控制方法对调度参数的选择非常敏感，参数调整是主要的挑战。研究人员希望找到一种更简单、更鲁棒的方法来控制扩散模型的参数。

Method: 提出 LP-MBD，使用受流匹配启发的线性概率路径来代替方差保持调度。在此基础上，提出 ALP-MBD，通过强化学习自适应地调整扩散步长和噪声水平。

Result: LP-MBD 简化了调度并保持了良好的性能，在数值研究、Brax 基准测试和移动机器人轨迹跟踪中均表现良好。ALP-MBD 进一步提高了鲁棒性、适应性和实时效率。

Conclusion: LP-MBD 提供了一种更易于调整且性能强大的基于模型的扩散模型方法，而 ALP-MBD 通过强化学习实现了更优的自适应控制，适用于复杂的机器人任务。
...
### [261] [Language Movement Primitives: Grounding Language Models in Robot Motion](https://arxiv.org/abs/2602.02839)
*Yinlong Dai,Benjamin A. Christie,Daniel J. Evans,Dylan P. Losey,Simon Stepputtis*

Main category: cs.RO

TL;DR: 提出了一种名为语言运动原语（LMPs）的框架，将大型视觉语言模型（VLMs）的抽象推理能力与动态运动原语（DMPs）的低级运动控制相结合，实现了零样本（zero-shot）机器人操作任务。


<details>
  <summary>Details</summary>
Motivation: 现有的方法在让机器人理解自然语言指令并执行新颖的操作任务方面存在挑战。VLMs 能够进行推理但难以将其映射到具体的机器人动作，而机器人基础模型需要大量特定领域的数据进行微调才能处理新任务。核心问题在于连接抽象的任务推理和低级运动控制。

Method: 提出 LMP 框架，利用 DMPs 的可解释参数来接地 VLM 的推理。VLMs 通过理解自然语言指令，将其语义上的运动意图转化为 DMPs 的参数，从而生成稳定、连续的运动轨迹。该框架通过生成一系列 DMP 运动来完成桌面操作任务。

Result: 在 20 个真实世界的机器人操作任务上，LMP 实现了 80% 的任务成功率，远高于性能最佳的基线方法的 31%。

Conclusion: LMP 框架成功地弥合了高级任务推理与低级位置和速度控制之间的差距，实现了零样本机器人操作，并在实际任务中展现出优于现有方法的性能。
...
### [262] [Kino-PAX$^+$: Near-Optimal Massively Parallel Kinodynamic Sampling-based Motion Planner](https://arxiv.org/abs/2602.02846)
*Nicolas Perrault,Qi Heng Ho,Morteza Lahijanian*

Main category: cs.RO

TL;DR: Kino-PAX$^{+}$ 是一种大规模并行动力学运动规划器，通过分解串行操作为三个并行子程序，能够实现近乎最优的解，并且找到解决方案的速度比现有串行方法快三个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有的基于采样的方法（SBMPs）在处理高维空间中的复杂动力学约束时存在计算效率低下的问题，难以实现实时性能。虽然并行化方法提高了找到可行解的速度，但缺乏优化目标函数的保证。因此，研究者希望开发一种既能并行化又能保证最优性的方法。

Method: Kino-PAX$^{+}$ 通过将串行操作分解为三个大规模并行子程序，构建了一个稀疏的、动态可行的轨迹树。该算法将计算资源集中在局部邻域中最有潜力的节点进行传播和细化，从而加速解的成本改进。该算法在保持概率性 δ-鲁棒完备性的同时，通过关注有潜力的节点来保证渐进性 δ-鲁棒近最优性。

Result: Kino-PAX$^{+}$ 找到解决方案的速度比现有的串行方法快了多达三个数量级，并且获得的解的成本优于最先进的基于 GPU 的规划器。

Conclusion: Kino-PAX$^{+}$ 是一种高效的大规模并行动力学运动规划器，能够找到具有成本效益和动态可行性的运动轨迹，并在速度和解的质量方面均优于现有方法。
...
### [263] [Latent Perspective-Taking via a Schrödinger Bridge in Influence-Augmented Local Models](https://arxiv.org/abs/2602.02857)
*Kevin Alcedo,Pedro U. Lima,Rachid Alami*

Main category: cs.RO

TL;DR: 该研究提出了一种结合神经符号世界模型和基于薛定谔桥的视角转换算子，以在与人类交互的环境中学习和推理他人的心智模型和心智状态，从而实现机器人进行社会感知决策。


<details>
  <summary>Details</summary>
Motivation: 在与人类共同操作的环境中，机器人需要处理不确定性，并且要考虑他人的隐藏心智模型和心智状态。现有的交互式POMDP和贝叶斯心智理论方法在精确推断和开放世界场景下的模型鲁棒性方面存在挑战。

Method: 研究者提出了一个名为“影响增强局部模型”的框架，将机器人任务分解为局部动力学、社会影响和外部因素。具体而言，他们构建了一个神经符号世界模型（因式分解的离散动态贝叶斯网络），并设计了一个“视角转换算子”（基于薛定谔桥），用于将以自我为中心的信念转换为以他人为中心的信念。

Result: 该框架能够让智能体在基于模型的强化学习中合成社会感知策略，并通过决策时的心智状态规划（信念空间中的薛定谔桥）来实现。在MiniGrid社交导航任务中取得了初步成效。

Conclusion: 该研究提出的方法能够学习结构化的心智模型并估计他人的心智状态，从而解决在与人类交互的环境中机器人决策面临的不确定性和心智推理的挑战，并有望提升机器人在社会场景下的表现。
...
### [264] [IMAGINE: Intelligent Multi-Agent Godot-based Indoor Networked Exploration](https://arxiv.org/abs/2602.02858)
*Tiago Leite,Maria Conceição,António Grilo*

Main category: cs.RO

TL;DR: 本文利用多智能体强化学习（MARL）在2D室内GNSS受限环境中，实现了通信感知型UAV群的自主协同探索，解决了协调、感知和去中心化决策的挑战。通过高保真模拟和课程学习，有效提升了训练效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在GNSS受限环境中，UAV群的自主通信感知协同探索面临协调、感知和去中心化决策的挑战，现有研究在离散动作、中心化处理、先验知识假设、动态障碍物处理等方面存在局限。

Method: 采用多智能体强化学习（MARL），结合网络分布式部分可观测马尔可夫决策过程（ND-POMDP）进行策略训练，在Godot游戏引擎中进行2D室内高保真仿真，使用LiDAR传感器，并考虑通信约束（距离、带宽、延迟）。同时进行了广泛的消融实验，评估了训练范式、奖励函数、通信系统、神经网络架构、记忆机制和POMDP公式。

Result: 提出的可扩展训练范式结合简化的神经网络架构，实现了UAV群对室内区域的快速自主探索。课程学习（五级难度）也加速了训练并提高了鲁棒性。该方法克服了离散动作、中心化处理、先验知识假设、动态障碍物处理和计算复杂性等问题。

Conclusion: 通过高保真仿真、MARL方法和计算效率的结合，为将学习到的协同策略部署到实际机器人系统奠定了坚实基础，能够实现UAV群在GNSS受限环境下的自主协同探索。
...
### [265] [Accelerating Structured Chain-of-Thought in Autonomous Vehicles](https://arxiv.org/abs/2602.02864)
*Yi Gu,Yan Wang,Yuxiao Chen,Yurong You,Wenjie Luo,Yue Wang,Wenhao Ding,Boyi Li,Heng Yang,Boris Ivanovic,Marco Pavone*

Main category: cs.RO

TL;DR: 提出了一种名为 FastDriveCoT 的并行解码方法，用于加速视觉-语言-动作模型在自动驾驶中的链式思考（CoT）推理，通过将推理过程分解为可并行执行的子任务，显著减少了推理延迟，同时保持了CoT的性能优势。


<details>
  <summary>Details</summary>
Motivation: 链式思考（CoT）虽然能提升自动驾驶模型的能力，但其自回归的特性导致推理延迟过高，不适用于实时场景。

Method: FastDriveCoT 将 CoT 推理过程分解为依赖图中的独立子任务，如识别关键对象和总结交通规则，并允许在单次前向传播中并行生成多个独立的推理步骤，从而减少顺序计算。

Result: FastDriveCoT 在 CoT 生成方面实现了 3-4 倍的加速，并显著降低了端到端延迟，同时保留了 CoT 带来的下游任务性能提升。

Conclusion: FastDriveCoT 是一种有效的并行解码方法，能够显著加速模板结构 CoT，使其在自动驾驶等实时应用中更具实用性，并且不牺牲性能。
...
### [266] [Moving On, Even When You're Broken: Fail-Active Trajectory Generation via Diffusion Policies Conditioned on Embodiment and Task](https://arxiv.org/abs/2602.02895)
*Gilberto G. Briscoe-Martinez,Yaashia Gautam,Rahul Shetty,Anuj Pasricha,Marco M. Nicotra,Alessandro Roncone*

Main category: cs.RO

TL;DR: 本文提出了一种名为DEFT的基于扩散的模型轨迹生成器，用于在执行器发生故障时，使机器人能够保持安全操作并完成任务（fail-active操作）。DEFT能够泛化处理不同类型的故障，支持有约束和无约束运动，并在实际应用中证明了其有效性。


<details>
  <summary>Details</summary>
Motivation: 机器人故障会导致严重的中断，通常需要人工干预才能恢复。因此，研究机器人在故障下保持安全操作并完成任务的能力（fail-active操作）具有重要意义。

Method: 本文提出了一种名为DEFT（Diffusion-based Embodiment and Task-conditioned Trajectory Generator）的扩散模型轨迹生成器。DEFT能够根据机器人的当前状态和任务约束来生成轨迹，以应对执行器故障。

Result: 在模拟环境中，DEFT在数千个关节故障案例中表现优于基线方法，性能提升高达2倍，并且在未见过的故障类型上也能保持鲁棒性。在真实世界实验中，DEFT成功完成了基线方法失败的抽屉操作和白板擦除等任务。

Conclusion: DEFT能够实现跨越任意故障配置和实际部署场景的fail-active操纵能力，证明了其在机器人故障下的鲁棒性和泛化性。
...
### [267] [Modular Isoperimetric Soft Robotic Truss for Lunar Applications](https://arxiv.org/abs/2602.02915)
*Mihai Stanciu,Isaac Weaver,Adam Rose,James Wade,Kaden Paxton,Chris Paul,Spencer Stowell,Nathan Usevitch*

Main category: cs.RO

TL;DR: 本文介绍了一种新型轻质、模块化、可重构的充气式充气机器人系统，适用于月球应用。该系统通过充气和变形实现大范围的运动和结构构建，无需额外气源。


<details>
  <summary>Details</summary>
Motivation: 为应对月球操作的挑战，开发一种可持续、适应性强且易于部署的机器人系统。

Method: 系统由充气织物管组成的三角形桁架构成，通过滚筒单元实现连接和变形。利用滚筒单元的捏合和移动来改变结构形状，实现变形。

Result: 该系统展示了作为具有12自由度的太阳能电池阵列（可倾斜60度，扫描360度）和具有14自由度的爬行装置（采用“步移-滑动”步态）的能力。折叠体积比达到1:18.3。

Conclusion: 所提出的模块化、形状自适应机器人系统能够有效解决月球可持续运行和未来太空任务的关键挑战。
...
### [268] [Embodiment-Aware Generalist Specialist Distillation for Unified Humanoid Whole-Body Control](https://arxiv.org/abs/2602.02960)
*Quanquan Peng,Yunfeng Lin,Yufei Xue,Jiangmiao Pang,Weinan Zhang*

Main category: cs.RO

TL;DR: 提出了一种名为EAGLE的迭代式通用-专业知识蒸馏框架，用于训练一个单一的策略来控制多个异构人形机器人，无需针对每个机器人进行奖励调整，并在仿真和真实世界中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人全身控制器通常针对单一机器人，难以应对不同动力学、自由度（DoFs）和运动学拓扑的机器人，并且难以实现超越简单行走的更丰富行为的通用性。

Method: EAGLE是一个迭代的通用-专业知识蒸馏框架。在每个循环中，从通用策略派生出特定于机器人的专业策略，在各自的机器人上进行训练，然后将新技能蒸馏回通用策略。通过在混合机器人数据集上进行训练来实现。

Result: EAGLE在Unitree H1、G1和Fourier N1等机器人上进行了验证，在模拟和真实世界的五种不同机器人上进行了实验。与现有方法相比，EAGLE实现了高跟踪精度和鲁棒性。

Conclusion: EAGLE是一个朝着可扩展、车队级别人形机器人控制迈出的一步，它能够生成一个单一的、鲁棒的全身控制器，用于控制多个异构人形机器人，并且无需为每个机器人单独调整奖励。
...
### [269] [RPL: Learning Robust Humanoid Perceptive Locomotion on Challenging Terrains](https://arxiv.org/abs/2602.03002)
...
TL;DR: 提出了一种名为RPL的两阶段训练框架，用于训练人形机器人实现复杂地形下的鲁棒多向运动，并能有效承载负载。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人运动能力在复杂地形下仍显不足，尤其是在多方向运动和承载负载方面。

Method: RPL框架首先利用特权高度图训练特定地形的专家策略，学习解耦的运动和操纵技能。然后，通过引入深度特征缩放和随机侧面遮蔽技术，将这些技能蒸馏到一个利用多个深度相机观察的Transformer策略中。同时，开发了一个高效的多深度渲染系统来加速训练过程。

Result: 在真实世界实验中，该系统成功实现了在20°斜坡、不同步长楼梯和间隔60cm的踏脚石等复杂地形上，2kg负载下的鲁棒多向运动。

Conclusion: RPL框架能够显著提升人形机器人在复杂地形和承载负载下的多向运动能力，并通过高效的深度信息处理和鲁棒性技术克服了现有挑战。
...
### [270] [A Unified Candidate Set with Scene-Adaptive Refinement via Diffusion for End-to-End Autonomous Driving](https://arxiv.org/abs/2602.03112)
*Zhengfei Wu,Shuaixi Pan,Shuohan Chen,Shuo Yang,Yanjun Huang*

Main category: cs.RO

TL;DR: 本文提出了一种名为CdDrive的自主驾驶方法，它结合了固定的轨迹词汇和场景自适应的扩散模型生成的新轨迹，并通过HATNA模块优化扩散轨迹的平滑度和几何连续性，旨在提高在常规和复杂交互场景下的规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有自主驾驶方法在轨迹候选集设计上存在挑战：固定词汇在复杂场景下可能错过最优解，而场景自适应方法在简单场景下可能过度修正。因此，需要一种方法能够兼顾这两种情况的优点。

Method: CdDrive方法保留了原始的固定轨迹词汇，并利用词汇条件扩散去噪生成场景自适应的轨迹。两种类型的轨迹候选集由一个共享的选择模块进行联合评分。此外，HATNA模块通过时间平滑和视界感知噪声调制来改善扩散轨迹的平滑度和几何连续性。

Result: 在NAVSIM v1和NAVSIM v2数据集上的实验表明，CdDrive方法取得了领先的性能。消融实验验证了每个组件的贡献。

Conclusion: CdDrive方法通过融合固定轨迹词汇和场景自适应扩散模型生成的轨迹，并结合HATNA模块进行优化，能够实现跨越常规和复杂交互场景的可靠规划性能。
...
### [271] [Training and Simulation of Quadrupedal Robot in Adaptive Stair Climbing for Indoor Firefighting: An End-to-End Reinforcement Learning Approach](https://arxiv.org/abs/2602.03087)
*Baixiao Huang,Baiyu Huang,Yu Hou*

Main category: cs.RO

TL;DR: 提出了一种两阶段端到端深度强化学习方法，用于优化四足机器人在复杂室内环境中导航和爬楼梯的能力，实现了从抽象地形到真实楼梯的技能迁移和跨不同楼梯形状的泛化。


<details>
  <summary>Details</summary>
Motivation: 现有四足机器人在室内火灾搜救等复杂室内环境中，在态势感知和跨不同楼梯快速攀爬方面存在挑战。

Method: 设计了一个两阶段端到端深度强化学习框架。第一阶段在抽象的金字塔楼梯地形上训练四足机器人，第二阶段将第一阶段学到的策略迁移到 Isaac Lab 引擎中模拟的各种逼真室内楼梯（直形、L形、螺旋形）上进行训练。采用了基于中心线的导航方法，实现了导航和运动的统一学习。

Result: 成功实现了四足机器人跨越不同形状楼梯的能力，并展示了仅使用局部高度图感知即可实现策略泛化。对不同难度楼梯下的成功率、效率和失败模式进行了实证分析。

Conclusion: 两阶段端到端强化学习框架能够有效地将四足机器人的爬楼梯技能从抽象地形迁移到复杂的真实室内楼梯环境中，并且通过中心线导航方法实现了导航和运动的无缝结合，在各种楼梯形状下表现出良好的泛化能力。
...
### [272] [Multi-function Robotized Surgical Dissector for Endoscopic Pulmonary Thromboendarterectomy: Preclinical Study and Evaluation](https://arxiv.org/abs/2602.03147)
*Runfeng Zhu,Xin Zhong,Qingxiang Zhao,Jing Lin,Zhong Wu,Kang Li*

Main category: cs.RO

TL;DR: 本研究提出了一种基于同心推拉机器人（CPPR）结构的新型机器人解剖器，用于肺动脉内膜剥脱术（PTE），以克服现有工具的局限性，实现对肺动脉细小分支的精确操作。


<details>
  <summary>Details</summary>
Motivation: 现有用于肺动脉内膜剥脱术（PTE）的手术工具刚性且直线，缺乏精细操作肺动脉细小分支的能力，限制了手术效果。

Method: 设计并构建了一种基于CPPR结构、直径为3.5mm的机器人解剖器，该解剖器具有细长性和双段弯曲灵活性。建立了基于优化的运动学模型，实现了在开环控制策略下2mm的定位精度。通过实验评估了机器人的刚度、运动精度和机动性，并在离体猪肺上进行了手术模拟。

Result: 机器人解剖器具有细长的身躯和双段弯曲的灵活性，中央管腔可容纳灌溉、工具和内窥镜信号线。运动学模型实现了2mm的定位精度。实验和手术模拟均表明该机器人解剖器具有良好的灵活性和显著的PTE手术优势，有望将传统PTE升级为内镜PTE。

Conclusion: 新型CPPR基机器人解剖器能够进入肺动脉的细小分支，为PTE手术提供了更精细的操作能力，有望克服传统手术工具的不足，提升手术效果。
...
### [273] [When Attention Betrays: Erasing Backdoor Attacks in Robotic Policies by Reconstructing Visual Tokens](https://arxiv.org/abs/2602.03153)
*Xuetao Li,Pinhan Fu,Wenke Huang,Nengyuan Pan,Songhua Yang,Kaiyan Zhao,Guancheng Wan,Mengde Li,Jifeng Xuan,Miao Li*

Main category: cs.RO

TL;DR: 本文提出了一种名为 Bera 的测试时后门擦除框架，用于防御针对视觉-语言-动作（VLA）模型的后门攻击，该框架无需重新训练模型即可有效消除后门。


<details>
  <summary>Details</summary>
Motivation: 现有的 VLA 模型在下游微调时容易受到后门攻击，攻击者可以通过投毒数据植入隐蔽的后门，导致模型在推理时产生有害行为。现有防御方法要么缺乏对多模态后门机制的深入理解，要么需要昂贵的模型重训练。

Method: 该研究揭示了后门通过深层注意力抓取机制实现：后门会重定向后期注意力，并在靠近干净流形的位置形成紧凑的嵌入簇。基于此洞察，Bera 框架通过以下步骤检测并消除后门：1. 检测具有异常注意力的 token；2. 利用深层线索掩蔽可疑区域；3. 重构无触发器的图像，打破触发器-不安全行为的映射，并恢复正确行为。

Result: Bera 在多个具身平台和任务上的实验表明，它能有效维持模型名义性能，显著降低攻击成功率，并持续地将后门模型的输出恢复为良性行为。

Conclusion: Bera 提供了一种强大且实用的防御机制，能够有效保护机器人系统免受后门攻击，且无需重新训练 VLA 模型或改变训练流程。
...
### [274] [Estimation of Ground Reaction Forces from Kinematic Data during Locomotion](https://arxiv.org/abs/2602.03177)
...
TL;DR: 本研究提出了一种仅使用基于标记的运动捕捉数据来估计地面反作用力 (GRF) 的方法，无需力台系统。


<details>
  <summary>Details</summary>
Motivation: 尽管 GRF 在步态力学和临床评估中具有重要意义，但传统力台系统的实际限制阻碍了其在临床中的广泛应用。

Method: 该方法利用十六个身体节段的运动学数据来估计质心 (CoM)，进而计算 GRF。然后，使用基于最小化的方法将 GRF 分解为各个分量，并识别步态站立期，从而无需力台即可获得临床相关的动力学测量值。

Result: 实验结果表明，仅凭运动学数据即可准确估计 CoM 和 GRF，证明了该方法的可行性。

Conclusion: 该研究成功开发了一种无需力台即可进行步态分析的力学测量方法，为 GRF 在临床中的广泛应用提供了支持。
...
### [275] [Hierarchical Proportion Models for Motion Generation via Integration of Motion Primitives](https://arxiv.org/abs/2602.03188)
*Yu-Han Shu,Toshiaki Tsuji,Sho Sakaino*

Main category: cs.RO

TL;DR: 本研究提出了一种分层模仿学习框架，结合了运动基元和基于比例的运动合成，以提高数据效率和适应性。实验证明，该方法能够生成复杂的、未包含在基元集中的运动，并且在稳定性和适应性方面优于标准分层模型。


<details>
  <summary>Details</summary>
Motivation: 现有的模仿学习方法在处理复杂或长时序任务时，需要大量高质量数据且难以适应新情况，这促使研究者们寻求更高效和更具适应性的解决方案。

Method: 提出了一种两层分层模仿学习框架。上层负责长期规划，下层学习单个运动基元。通过三种模型变体（学习型、采样型、回放型比例模型）来探索比例组合方式和上层模型的可训练性，实现运动基元的比例合成。

Result: 所提出的方法能够成功生成包含在基元集之外的复杂运动。采样型和回放型比例模型在运动生成方面表现出比标准分层模型更稳定和更强的适应性。

Conclusion: 基于比例的运动合成是提高分层模仿学习效率和适应性的有效方法，能够应对机器人学习中的实际挑战。
...
### [276] [HUSKY: Humanoid Skateboarding System via Physics-Aware Whole-Body Control](https://arxiv.org/abs/2602.03205)
*Jinrui Han,Dewei Wang,Chenyun Zhang,Xinzhe Liu,Ping Luo,Chenjia Bai,Xuelong Li*

Main category: cs.RO

TL;DR: 提出了一种名为HUSKY的基于学习的框架，用于解决人形机器人滑板这一高难度动态任务，通过整合系统建模、学习人类推行动作和基于物理的转向策略，实现了稳定敏捷的滑板操控。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人全身控制框架通常假设环境是静态的，难以应对高动态性和复杂交互的任务。人形机器人滑板作为一个典型的挑战性任务，需要机器人掌握在非完整约束和人机交互下的动态操纵和稳定控制。

Method: HUSKY框架首先对滑板的倾斜和转向角度之间的耦合关系进行建模，以分析系统动力学。然后，利用对抗性运动先验（AMP）学习人类风格的推行动作，并采用一种物理引导的、以航向为导向的策略来实现“倾斜转向”的控制。此外，还设计了一个轨迹引导机制，以确保推行和转向之间的平滑稳定过渡。

Result: 在Unitree G1人形机器人平台上进行的实验表明，HUSKY框架能够实现人形机器人稳定且敏捷的滑板操控，并在真实场景中取得了成功。

Conclusion: 所提出的HUSKY框架成功解决了人形机器人滑板这一复杂动态任务，通过整合系统建模、学习人类运动模式和物理启发式控制策略，实现了在动态不稳定的滑板上的稳定和敏捷的操纵。
...
### [277] [Depth Completion in Unseen Field Robotics Environments Using Extremely Sparse Depth Measurements](https://arxiv.org/abs/2602.03209)
*Marco Job,Thomas Stastny,Eleni Kelasidi,Roland Siegwart,Michael Pantic*

Main category: cs.RO

TL;DR: 本文提出了一种利用合成数据和稀疏深度测量来完成密集度估计的模型，以应对野外机器人中单目深度估计的挑战，并实现了实时部署。


<details>
  <summary>Details</summary>
Motivation: 野外机器人需要鲁棒的感知能力，但单目深度估计在实际应用中受尺度线索缺失、纹理不足和缺乏大规模数据集等因素限制。因此，需要一种新的方法来提高其在野外机器人环境中的可靠性。

Method: 本文提出了一种端到端的深度完成模型，该模型在为野外机器人量身定制的合成数据集上进行训练。该模型利用深度传感器提供的极其稀疏的测量，并结合合成数据生成流水线（包括SfM和新视角合成），来预测密集度度量深度。

Result: 该模型在Nvidia Jetson AGX Orin上实现了53毫秒的端到端延迟，可实现实时部署。在各种真实的野外机器人场景下的广泛评估显示，该方法取得了具有竞争力的性能。

Conclusion: 通过使用合成数据和稀疏深度测量，该方法成功解决了野外机器人中单目深度估计的关键挑战，并实现了实时性能，为在嵌入式平台上的部署铺平了道路。
...
### [278] [Omnidirectional Solid-State mmWave Radar Perception for UAV Power Line Collision Avoidance](https://arxiv.org/abs/2602.03229)
*Nicolaj Haarhøj Malle,Emad Ebeid*

Main category: cs.RO

TL;DR: 研究提出了一种基于毫米波雷达的感知系统，用于在无人机飞行中检测和规避电线，通过集成多个雷达模块实现全向覆盖，并在真实环境中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 无人机在检测和估计电线距离方面存在挑战，增加了意外碰撞的风险。

Method: 该系统集成了多个紧凑型固态毫米波雷达模块，以合成全向视场，并开发了针对此系统特性的鲁棒检测和规避算法。

Result: 在真实电线环境中进行飞行实验，成功在10米范围内可靠检测电线，在10米/秒以上的飞行速度下成功执行避障，并能检测到直径1.2毫米的细线。

Conclusion: 该毫米波雷达感知系统为无人机（无论是自主还是手动飞行）提供了额外的安全层，能够可靠地检测和规避电线。
...
### [279] [A thin and soft optical tactile sensor for highly sensitive object perception](https://arxiv.org/abs/2602.03248)
...
TL;DR: 本文提出了一种薄型、紧凑、柔软且无需对准的光学触觉传感器，该传感器利用柔软材料内部的散斑图案变化来精确测量力和识别纹理。


<details>
  <summary>Details</summary>
Motivation: 现有的基于视觉的光学触觉传感器通常设计复杂、笨重、刚性且对准敏感，这限制了其在机器人和可穿戴设备中的应用。因此，需要一种更紧凑、更柔韧且易于制造的替代方案。

Method: 该传感器利用注入了散射体的柔软硅胶材料。当施加力时，材料会发生形变，导致内部散斑图案发生变化。通过机器学习算法分析这些散斑图案的变化，从而实现力的测量和纹理识别。

Result: 在力测量方面，传感器的均方根误差为 40 mN。在纹理识别方面，对九种不同纹理（包括麻将牌）的分类准确率达到了 93.33%。

Conclusion: 所提出的基于散斑的光学触觉传感器具有紧凑、易于制造和机械柔顺的优点，为软体机器人和可穿戴触觉界面提供了一种新的触觉感知范例。
...
### [280] [Collision Detection with Analytical Derivatives of Contact Kinematics](https://arxiv.org/abs/2602.03250)
*Anup Teejo Mathew,Anees Peringal,Daniele Caradonna,Frederic Boyer,Federico Renda*

Main category: cs.RO

TL;DR: 本研究提出了一种名为iDCOL的隐式可微分碰撞检测和接触运动学框架，通过正则化具有零曲率的几何体使其变为严格凸的隐式表示，解决了传统方法在退化配置下的非平滑问题，并利用隐函数定理推导出接触运动学量的解析导数，适用于基于梯度的机器人应用。


<details>
  <summary>Details</summary>
Motivation: 传统机器人梯度优化方法依赖于可微分的接触运动学，但在形状退化（零曲率）配置下，接触距离、位置和法线的映射会变得非平滑，阻碍了方法的应用。因此，需要一种能够处理这些退化情况并提供平滑可微接触运动学的方法。

Method: 该研究将具有零曲率的几何体正则化为严格凸的隐式表示。在此基础上，开发了iDCOL框架，通过解决一个基于几何缩放的凸优化问题的非线性系统来计算碰撞检测和接触运动学。利用隐函数定理，导出了接触运动学量的解析导数，并开发了一个基于牛顿法的求解器。

Result: iDCOL框架能够提供平滑可微的接触运动学，即使在几何退化的情况下也能保持鲁棒性。通过大量的碰撞仿真和基准测试，证明了该方法的有效性。此外，在基于梯度的运动规划和可微分接触物理（包括多刚体碰撞和软体机器人交互）方面展示了其应用潜力。

Conclusion: iDCOL框架通过对退化几何体进行正则化，成功地实现了可微分的碰撞检测和接触运动学，克服了传统方法的局限性。该框架为机器人领域的梯度优化方法提供了更广泛的应用前景，尤其是在复杂接触场景下。
...
### [281] [RDT2: Exploring the Scaling Limit of UMI Data Towards Zero-Shot Cross-Embodiment Generalization](https://arxiv.org/abs/2602.03310)
*Songming Liu,Bangguo Li,Kai Ma,Lingxuan Wu,Hengkai Tan,Xiao Ouyang,Hang Su,Jun Zhu*

Main category: cs.RO

TL;DR: RDT2是一个基于70亿参数的视觉-语言-动作（VLA）模型，旨在通过大规模数据集和创新的三阶段训练方法，实现机器人技术的通用性和零样本部署能力，即使在新的硬件平台上也能执行开放词汇的任务，并在复杂任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在数据稀缺、架构效率低下以及跨硬件平台泛化能力不足方面存在挑战，研究者希望构建一个更通用、更易于部署的机器人基础模型。

Method: 文章提出RDT2模型，基于一个70亿参数的VLM。通过一个增强的、与具体硬件无关的通用操纵接口（UMI），收集了超过10,000小时的机器人演示数据。采用了一个新颖的三阶段训练方法，包括残差向量量化（RVQ）、流匹配和蒸馏，以在实时推理中将离散语言知识与连续控制对齐。

Result: RDT2成功地实现了对未见过的物体、场景、指令甚至机器人平台的零样本泛化。在乒乓球等灵巧、长时域和动态的下游任务中，其性能优于现有的最先进基线模型。

Conclusion: RDT2是首批能够同时实现对新颖环境、指令和硬件的零样本泛化，并在复杂机器人任务中取得优异表现的模型之一，为通用机器人技术的发展奠定了基础。
...
### [282] [Manipulation via Force Distribution at Contact](https://arxiv.org/abs/2602.03350)
*Haegu Lee,Yitaek Kim,Casper Hewson Rask,Christoffer Sloth*

Main category: cs.RO

TL;DR: 本文提出了一种力分布线接触（FDLC）模型，用于解决机器人操作中点接触模型的局限性，并通过仿真和实验证明FDLC模型能够生成更高效、更鲁棒的轨迹，同时降低机器人控制和运动的需求。


<details>
  <summary>Details</summary>
Motivation: 现有点接触模型计算效率高，但在模拟人类操作中的摩擦动力学和力矩生成方面存在不足，限制了实现类人、富接触操作的机器人能力。研究旨在克服这些局限性。

Method: 提出了一种力分布线接触（FDLC）模型，并构建了一个双层优化框架。下层优化用于计算接触力，上层优化使用iLQR（iterative Linear Quadratic Regulator）进行轨迹优化。将FDLC模型与传统点接触模型进行了对比。

Result: FDLC模型能够生成非均匀分布的接触力，从而实现更高效、更鲁棒的轨迹。相较于点接触模型，FDLC在箱体旋转任务中，需要更低的控制力矩和更小的机器人运动。

Conclusion: FDLC模型能够有效地捕捉富接触操作中的关键摩擦动力学，并克服点接触模型的不足，生成更高质量的机器人操作轨迹，证明了其在实际应用中的有效性。
...
### [283] [Learning-based Adaptive Control of Quadruped Robots for Active Stabilization on Moving Platforms](https://arxiv.org/abs/2602.03367)
*Minsung Yoon,Heechan Shin,Jeil Jeong,Sung-Eui Yoon*

Main category: cs.RO

TL;DR: 提出了一种名为LAS-MP（基于学习的主动稳定移动平台）的算法，用于解决四足机器人在六自由度运动平台上（如地铁、公交车、飞机和游艇）的平衡问题。该算法包含一个自平衡策略和系统状态估计器，并通过轨迹生成和调度方法进行训练，实验结果表明其优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 四足机器人在地铁、公交车、飞机和游艇等六自由度运动平台上会面临平衡挑战，因为平台运动产生的惯性力会影响机器人。现有方法可能无法有效应对这些复杂动态。

Method: 提出了一种名为LAS-MP的算法，包含以下关键部分：1. 自平衡策略：自适应调整机器人姿态以应对平台运动。2. 系统状态估计器：利用本体感觉传感器数据估计机器人和平台的状态。3. 训练方案：引入平台轨迹生成和调度方法，以在各种平台运动下进行系统性训练。

Result: 在多个指标上，LAS-MP的表现优于三种基线方法。通过消融研究和对估计器的评估，验证了LAS-MP各组件的有效性。

Conclusion: LAS-MP算法能够有效地解决四足机器人在六自由度运动平台上的平衡问题，其自平衡策略和状态估计器在复杂动态环境下表现出色。
...
### [284] [PlanTRansformer: Unified Prediction and Planning with Goal-conditioned Transformer](https://arxiv.org/abs/2602.03376)
*Constantin Selzer,Fabina B. Flohr*

Main category: cs.RO

TL;DR: 提出了一种名为Plan Transformer (PTR) 的统一高斯混合Transformer框架，该框架将目标条件预测、动态可行性、交互感知和车道级拓扑推理相结合，解决了自动驾驶中预测和规划脱节的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的自动驾驶预测和规划模块是分离的，预测模型缺乏对意图的监督，而规划模型需要这些信息。现有的预测模型通常与规划约束（如避免碰撞和动态可行性）脱节。

Method: 引入Plan Transformer (PTR)，一个集成了目标条件预测、动态可行性、交互感知和车道级拓扑推理的统一高斯混合Transformer框架。采用师生训练策略，通过逐步掩盖周围代理的命令来使模型适应推理条件。

Result: PTR在边际/联合mAP上比基线Motion Transformer (MTR) 提高了4.3%/3.5%，在5秒预测范围内比GameFormer的规划误差降低了15.5%。

Conclusion: PTR通过一个统一的框架有效解决了预测和规划之间的不匹配问题，并能在实际应用中取得显著的性能提升。其架构无关的设计使其能够应用于各种基于Transformer的预测模型。
...
### [285] [Enhancing Navigation Efficiency of Quadruped Robots via Leveraging Personal Transportation Platforms](https://arxiv.org/abs/2602.03397)
*Minsung Yoon,Sung-Eui Yoon*

Main category: cs.RO

TL;DR: 提出一种基于强化学习的主动交通工具骑行（RL-ATR）方法，使四足机器人能够像人类骑行Segway一样，通过乘坐交通工具进行高效长距离导航，并降低能耗。


<details>
  <summary>Details</summary>
Motivation: 四足机器人在长距离导航方面存在效率低下的问题，因为它们依赖腿部移动。为了克服这一限制，研究者受到人类利用个人交通工具（如Segway）的启发。

Method: 提出RL-ATR方法，包含一个交通工具骑行策略和两个状态估计器。骑行策略能够根据交通工具特有的控制动力学制定适当的机动策略。状态估计器通过推断不可观察的机器人和交通工具状态来解决非惯性参考系中的传感器模糊性。

Result: 在仿真环境中进行了全面评估，结果表明RL-ATR在不同交通工具-机器人模型下均能熟练地跟踪指令，并且与纯粹的腿部运动相比，能耗有所降低。消融研究也量化了RL-ATR中各个组件的贡献。

Conclusion: RL-ATR方法为四足机器人提供了新的移动模式，能够乘坐交通工具进行高效导航，有望扩大其操作范围和效率。
...
### [286] [Learning-based Initialization of Trajectory Optimization for Path-following Problems of Redundant Manipulators](https://arxiv.org/abs/2602.03418)
*Minsung Yoon,Mincheul Kang,Daehyung Park,Sung-Eui Yoon*

Main category: cs.RO

TL;DR: 提出一种基于学习的冗余机械臂初始轨迹生成方法，通过示例引导强化学习和空域投影模仿奖励，能够快速生成高质量的初始轨迹，并提高轨迹优化的性能。


<details>
  <summary>Details</summary>
Motivation: 传统轨迹优化方法在生成冗余机械臂的初始轨迹时存在困难，选择高质量的初始轨迹耗时且需要先验知识，限制了优化性能。

Method: 采用示例引导强化学习（example-guided reinforcement learning）生成高质量的初始轨迹，并结合空域投影模仿奖励（null-space projected imitation reward）来考虑空域约束。

Result: 与三种基线方法相比，所提出的方法生成的初始轨迹能够显著提高轨迹优化的最优性、效率和适用性，并在真实世界的实验中得到验证。

Conclusion: 基于学习的初始轨迹生成方法能够有效地解决冗余机械臂轨迹优化中的初始轨迹选择难题，显著提升了轨迹优化的整体性能。
...
### [287] [Deep-Learning-Based Control of a Decoupled Two-Segment Continuum Robot for Endoscopic Submucosal Dissection](https://arxiv.org/abs/2602.03406)
*Yuancheng Shao,Yao Zhang,Jia Gu,Zixi Chen,Di Wu,Yuqiao Chen,Bo Lu,Wenjie Liu,Cesare Stefanini,Peng Qi*

Main category: cs.RO

TL;DR: 本文提出了一种名为DESectBot的新型双节段连续体机器人，该机器人具有解耦结构和集成手术镊子，可实现6自由度末端操作，以提高内镜黏膜下剥离术（ESD）中的病灶靶向精度。并开发了一种基于门控循环单元（GRU）的深度学习控制器，用于同时控制末端位置和姿态，有效解决了连续体片段之间的非线性耦合问题。


<details>
  <summary>Details</summary>
Motivation: 手动内镜黏膜下剥离术（ESD）技术要求高，且现有的单节段机器人工具灵活性有限，这促使了更先进解决方案的开发。

Method: 开发了名为DESectBot的双节段连续体机器人，具有解耦结构和集成手术镊子，可实现6自由度末端操作。使用基于门控循环单元（GRU）的深度学习控制器进行同时的末端位置和姿态控制，并与基于雅可比逆运动学、模型预测控制（MPC）、前馈神经网络（FNN）和长短期记忆（LSTM）网络等方法进行了基准测试。

Result: GRU控制器在嵌套矩形和利萨茹轨迹跟踪任务中取得了最低的位置/姿态均方根误差（RMSE），分别为1.11 mm/4.62°和0.81 mm/2.59°。在固定位置（四种目标姿态）的姿态控制中，GRU的平均RMSE为0.14 mm和0.72°，优于所有其他方法。在钉孔转移任务中，GRU的成功率为100%（120次成功/120次尝试），平均转移时间为11.8秒，显著优于新手控制的系统。离体ESD演示表明DESectBot具有足够的刚度来切割厚胃黏膜，并且操作空间足以应对大病灶。

Conclusion: GRU控制显著提高了ESD手术训练场景中的精度、可靠性和可用性，DESectBot为ESD手术提供了一种有前景的机器人辅助解决方案。
...
### [288] [ProAct: A Benchmark and Multimodal Framework for Structure-Aware Proactive Response](https://arxiv.org/abs/2602.03430)
...
TL;DR: 本研究提出了ProAct-75基准和ProAct-Helper模型，用于训练和评估跨领域的主动代理。ProAct-75包含75个任务、91,581个带任务图的标注，任务图编码了步骤依赖和并行执行的可能性。ProAct-Helper基于多模态大语言模型，通过状态检测和基于任务图的启发式搜索，实现了更优的触发检测、步骤节省和并行动作执行。


<details>
  <summary>Details</summary>
Motivation: 开发能够理解高层目标、持续监控环境并自主决定何时采取行动的主动代理面临专门资源缺乏的挑战。

Method: 提出了ProAct-75基准，包含75个任务、91,581个带显式任务图的步骤级别标注。任务图编码了步骤依赖和并行执行的可能性。在此基础上，提出了ProAct-Helper模型，该模型利用多模态大语言模型进行状态检测，并利用任务图进行基于熵的启发式搜索来选择动作，从而实现独立并行执行。

Result: ProAct-Helper在触发检测方面比闭源模型提高了6.21%的mF1分数，在在线一步决策中节省了0.25个步骤，并提高了15.58%的并行动作执行率。

Conclusion: ProAct-75基准和ProAct-Helper模型为开发和评估主动代理提供了有效的解决方案，并在多个关键指标上取得了显著的性能提升。
...
### [289] [Model-based Optimal Control for Rigid-Soft Underactuated Systems](https://arxiv.org/abs/2602.03435)
...
TL;DR: 本文研究了用于混合刚软连续体软机器人的三种最优控制策略（直接配置、微分动态规划和非线性模型预测控制），以执行动态摆动任务。通过利用几何可变应变模型和隐式积分方案，该方法在模拟中验证了其在复杂软机器人系统上的性能和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的连续体软机器人控制方法多集中于拟静止行为，难以应对如摆动这样的动态任务，特别是对于混合刚软机器人。现有的低阶模型无法捕捉真实连续体变形的复杂性，而基于模型的最优控制受限于计算成本和数值微分的精度。

Method: 本文研究了三种最优控制策略：直接配置、微分动态规划和非线性模型预测控制。利用几何可变应变模型提供解析导数，并采用隐式积分方案和热启动策略来提高数值鲁棒性和计算效率。

Result: 在模拟中，三种控制策略均成功地在软机械臂-杆、软机械臂-摆和软福塔摆等高阶软系统上执行了动态摆动任务。研究突出了它们在性能和计算成本上的权衡。

Conclusion: 本文提出的基于几何可变应变模型的优化控制方法能够有效地解决连续体软机器人（包括混合刚软系统）的动态控制挑战，并在计算效率和性能之间取得平衡。
...
### [290] [HetroD: A High-Fidelity Drone Dataset and Benchmark for Autonomous Driving in Heterogeneous Traffic](https://arxiv.org/abs/2602.03447)
...
TL;DR: HetroD是一个新的数据集和基准，用于在包含行人、骑自行车者和摩托车等弱势道路使用者（VRU）的异构交通环境中开发自动驾驶系统。该数据集包含大量VRU轨迹，并为预测、规划和模拟任务提供了基准，展示了现有先进模型在处理复杂异构交通方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集主要关注结构化、车道规矩的交通，而现实世界中以VRU为主的异构交通中的复杂行为（如鬼探头、蛇形行驶、非正式路权协商）被低估。这阻碍了自动驾驶系统在真实复杂场景下的发展。

Method: 收集了大规模的无人机航拍数据集，提供厘米级精度的标注、高精地图和交通信号状态，以全面观测交通场景。开发了一个模块化工具包，用于提取每个代理的场景信息，支持下游任务开发。数据集包含超过6.54万条高保真代理轨迹，其中70%来自VRU。

Result: 现有最先进的预测和规划模型在HetroD数据集上面临挑战，它们在预测VRU的横向移动、处理非结构化机动以及在密集和多代理场景下的表现有限。

Conclusion: HetroD数据集为建模VRU在密集异构交通中的行为提供了标准化基准，并揭示了当前自动驾驶模型在处理此类复杂场景时存在的局限性，强调了开发更鲁棒方法的必要性。
...
### [291] [CMR: Contractive Mapping Embeddings for Robust Humanoid Locomotion on Unstructured Terrains](https://arxiv.org/abs/2602.03511)
*Qixin Zeng,Hongyin Zhang,Shangke Lyu,Junxi Jin,Donglin Wang,Chao Huang*

Main category: cs.RO

TL;DR: 本文提出了一种名为“收缩映射鲁棒性”（CMR）的框架，通过将高维、易受干扰的观测映射到一个潜在空间，该空间中的局部扰动会随着时间衰减，从而提高了人形机器人运动在干扰和不确定环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人形机器人运动在非结构化地形上的鲁棒干扰抑制是一个长期存在的挑战，尤其是在感知不可靠和模型不匹配严重的情况下。现有的感知信息（如高度图）虽然增强了对地形的认知，但传感器噪声和仿真到现实的差距会使策略变得不稳定。

Method: 研究人员进行了理论分析，在诱导的潜在动力学具有收缩性的情况下，界定了观测噪声下的回报差距。在此基础上，提出了CMR框架，它结合了对比表征学习和Lipschitz正则化，以保留任务相关的几何结构，同时明确控制敏感性。该方法可以作为辅助损失项集成到深度强化学习流程中。

Result: 在增加噪声的条件下，CMR框架在大量人形机器人实验中表现出比其他运动算法更优越的性能。

Conclusion: CMR框架能够有效地将传感器噪声和模型不匹配的影响降至最低，从而显著提高人形机器人在复杂和干扰环境下的运动鲁棒性，并且易于集成到现有的深度强化学习系统中。
...
### [292] [Investigating the Influence of Spatial Ability in Augmented Reality-assisted Robot Programming](https://arxiv.org/abs/2602.03544)
*Nicolas Leins,Jana Gonnermann-Müller,Malte Teichmann,Sebastian Pokutta*

Main category: cs.RO

TL;DR: 本研究发现，尽管增强现实（AR）在机器人编程教学中并未显著提升整体学习体验，但它能弥补低空间能力学习者的劣势，降低空间能力对学习体验的影响。


<details>
  <summary>Details</summary>
Motivation: 增强现实（AR）在教育中的潜力巨大，但其作用机制和对个体学习者特征的影响尚不明确。随着个性化学习的兴起，研究学习者个体差异至关重要。

Method: 进行了一项包含71名参与者的组间实验，对比了传统的机器人编程方法与使用头戴式显示器的AR辅助方法。通过心理旋转测验评估了参与者的空间能力，并使用系统可用性量表（SUS）和认知负荷来衡量学习体验。

Result: AR辅助方法并未显著改善整体学习体验。然而，AR似乎对空间能力的影响起到了补偿作用。在对照组中，高空间能力与更好的SUS分数和更低的认知负荷相关；而在AR组中，这种相关性消失了，表明AR减轻了低空间能力学习者的不利影响。

Conclusion: AR在机器人编程学习中可以起到补偿作用，通过减轻学习者个体特征（如空间能力）对学习体验的影响。未来的研究应进一步探索AR的这种补偿机制，以设计能够满足不同学习者需求和减少认知障碍的个性化学习环境。
...
### [293] [AffordanceGrasp-R1:Leveraging Reasoning-Based Affordance Segmentation with Reinforcement Learning for Robotic Grasping](https://arxiv.org/abs/2602.03547)
*Dingyi Zhou,Mu He,Zhuowei Fang,Xiangtong Yao,Yinlong Liu,Alois Knoll,Hu Cao*

Main category: cs.RO

TL;DR: 提出了一种名为AffordanceGrasp-R1的机器人抓取框架，该框架结合了思维链（CoT）冷启动策略和强化学习，以提高推理和空间定位能力，并通过改进的抓取流程，实现了更强的上下文感知能力，并在基准数据集和实际机器人抓取评估中均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 为了提高机器人抓取任务的推理能力和空间定位能力，以及增强抓取流程的上下文感知能力。

Method: 结合了思维链（CoT）冷启动策略和强化学习，并重新设计了抓取流程，使其能够从全局场景点云生成抓取候选，并通过指令条件化的可供性掩码进行过滤。

Result: AffordanceGrasp-R1在基准数据集上持续优于最先进（SOTA）的方法，并在复杂语言条件操纵场景下的实际机器人抓取评估中验证了其鲁棒性和泛化能力。

Conclusion: AffordanceGrasp-R1是一个有效的、驱动推理的可供性分割框架，能够显著提升机器人抓取的性能，特别是在处理复杂的语言条件操纵任务时。
...
### [294] [Multi-Player, Multi-Strategy Quantum Game Model for Interaction-Aware Decision-Making in Autonomous Driving](https://arxiv.org/abs/2602.03571)
*Karim Essalmi,Fernando Garrido,Fawzi Nashashibi*

Main category: cs.RO

TL;DR: 提出了一种结合经典博弈论和量子力学原理的量子博弈决策模型（QGDM），用于解决自动驾驶中的交互感知决策问题，并在模拟环境中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶决策方法在处理多智能体交互时过于简化，尤其忽视了智能体之间的相互作用，且基于经典博弈论的模型假设了理性玩家，而人类行为常存在不确定性或非理性。

Method: 提出量子博弈决策模型（QGDM），结合了经典博弈论和量子力学原理（如叠加、纠缠、干涉），以处理多玩家、多策略的决策问题。该模型无需量子硬件，可在标准计算机上实时运行。

Result: 在模拟环境中，QGDM在包括环形交叉路口、汇流和高速公路等多种场景下进行了评估，并与多种基线方法进行了比较。结果表明，QGDM显著提高了成功率，降低了碰撞率，尤其在交互性强的场景下效果更佳。

Conclusion: QGDM模型是一种新颖的自动驾驶决策框架，利用量子博弈论的原理克服了经典方法的局限性，在处理复杂交互场景时表现出优越的性能。
...
### [295] [Human-in-the-Loop Failure Recovery with Adaptive Task Allocation](https://arxiv.org/abs/2602.03603)
*Lorena Maria Genua,Nikita Boguslavskii,Zhi Li*

Main category: cs.RO

TL;DR: 提出了一种自适应机器人故障分配方法（ARFA），通过对人类操作员的能力建模并根据其绩效进行更新，并结合奖励函数来为操作员分配故障，以提高机器人系统性能和工作负载均衡。


<details>
  <summary>Details</summary>
Motivation: 随着疫情后医疗和生活辅助机器人自主性的提高，但它们在动态环境中仍难以独立应对故障，需要人工干预。因此，需要一种有效的人机协作方法来分配故障，以利用最能干的操作员，减轻机器人工作负载并减少任务中断。

Method: ARFA方法通过以下步骤实现：1. 对人类操作员的能力进行建模。2. 基于操作员的实际故障恢复表现，持续更新其能力模型。3. 设计一个奖励函数，根据操作员能力、历史数据、任务紧急程度和当前工作负载分布，计算预期收益。4. 将故障分配给预期收益最高的操作员。

Result: 模拟和用户研究表明，ARFA方法优于随机分配，能够显著减少机器人空闲时间，提高整体系统性能，并实现操作员之间更均衡的工作负载分布。

Conclusion: ARFA是一种有效的人机协作方法，通过自适应地将机器人故障分配给最合适的人类操作员，可以显著提升机器人系统的效率和可靠性，同时优化操作员的工作负载。
...
### [296] [Self-supervised Physics-Informed Manipulation of Deformable Linear Objects with Non-negligible Dynamics](https://arxiv.org/abs/2602.03623)
*Youyuan Long,Gokhan Solak,Sara Zeynalpour,Heng Zhang,Arash Ajoudani*

Main category: cs.RO

TL;DR: 本文提出了一种名为 SPiD 的物理信息自监督学习框架，用于动态操控可形变线性物体，并通过仿真和真实世界实验证明了其在绳索稳定等任务上的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决动态操控可形变线性物体（如绳索）的挑战，开发一种数据高效、鲁棒且具有良好泛化能力的控制方法。

Method: 结合改进的质量-弹簧模型（更准确且轻量化）和一个新的自监督学习策略。该策略利用面向任务的成本函数进行端到端优化，并采用自监督 DAgger 变体来检测和纠正部署过程中的分布偏移。还开发了一种经济实惠的无标记绳索感知方法。

Result: SPiD 控制器在绳索稳定任务中实现了快速平滑的稳定效果，并能泛化到未见过（不同初始状态、长度、质量、质量分布不均、外部干扰）的条件。即使在状态更新不准确或低频的情况下，性能也能得到保持。框架也成功扩展到绳索轨迹跟踪任务。

Conclusion: SPiD 提供了一个数据高效、鲁棒且物理基础强的框架，适用于动态操控可形变线性物体，并展现了强大的仿真到真实世界的泛化能力。
...
### [297] [Variance-Reduced Model Predictive Path Integral via Quadratic Model Approximation](https://arxiv.org/abs/2602.03639)
*Fabian Schramm,Franki Nguimatsia Tiofack,Nicolas Perrin-Gilbert,Marc Toussaint,Justin Carpentier*

Main category: cs.RO

TL;DR: 提出一种结合先验模型的混合方差减小MPPI框架，通过将目标函数分解为模型项和残差项，并利用二次近似推导出模型引导的先验，有效提升了采样效率和收敛速度，适用于采样成本高或有限的场景。


<details>
  <summary>Details</summary>
Motivation: 传统的基于采样的控制器（如MPPI）存在方差高和样本效率低的问题，限制了其在实际应用中的可用性，尤其是在采样成本高昂的情况下。

Method: 通过将目标函数分解为已知近似模型和残差项，并采用二次近似推导出闭式解的模型引导先验，用于指导采样过程。该框架对先验模型的来源保持中立，可以是精确导数、近似方法（如Gauss-Newton）或无梯度方法。

Result: 在标准优化基准、非线性倒立摆控制任务和含接触的非光滑操纵问题上，与标准MPPI相比，新方法在低样本量情况下实现了更快的收敛速度和更优的性能。

Conclusion: 该混合方差减小MPPI框架能够显著提高基于采样的控制策略的样本效率和收敛速度，使其在采样受限或成本高昂的场景下更加实用。
...
### [298] [MVP-LAM: Learning Action-Centric Latent Action via Cross-Viewpoint Reconstruction](https://arxiv.org/abs/2602.03668)
*Jung Min Lee,Dohyeok Lee,Seokhun Ju,Taehyun Cho,Jin Woo Koo,Li Zhao,Sangwoo Hong,Jungwoo Lee*

Main category: cs.RO

TL;DR: 本文提出了一种多视角潜在动作模型（MVP-LAM），通过跨视角重建来学习与真实动作高度相关的离散潜在动作，并利用这些潜在动作预训练视觉-语言-动作（VLA）模型，以提高机器人操控的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于潜在动作的VLA预训练方法依赖于只包含一种视角的视频，这限制了潜在动作包含真实动作信息的能力。为了提高VLA预训练的有效性，需要学习更具信息量的潜在动作。

Method: MVP-LAM 使用时间同步的多视角视频，通过“跨视角重建”目标来学习离散潜在动作。具体来说，从一个视角推断出的潜在动作需要能够解释另一个视角中的未来，从而减少对特定视角线索的依赖。

Result: 在Bridge V2数据集上，MVP-LAM生成的潜在动作更具“动作中心性”，与真实动作的互信息更高，动作预测能力更强，即使在分布外评估时也是如此。使用MVP-LAM预训练的VLA模型在SIMPLER和LIBERO-Long基准测试中的下游操控性能有所提升。

Conclusion: MVP-LAM能够从多视角视频中学习到信息量更丰富的潜在动作，并且这种潜在动作的引入能够有效提升VLA模型的预训练效果，最终改善机器人下游的操控任务性能。
...
### [299] [A Scene Graph Backed Approach to Open Set Semantic Mapping](https://arxiv.org/abs/2602.03781)
*Martin Günther,Felix Igelbrink,Oscar Lima,Lennart Niecksch,Marian Renz,Martin Atzmueller*

Main category: cs.RO

TL;DR: 该研究提出了一种将3D语义场景图（3DSSG）作为机器人感知中基础知识表示的方法，解决了现有方法在大型真实世界环境中部署的挑战，并通过实时更新场景图来提高一致性和可扩展性，从而支持高层符号推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法在大型真实世界环境中有效部署开放集语义地图和3D语义场景图（3DSSG）以支持高层推理方面存在挑战，因为它们将感知与表示分离，导致一致性和可扩展性受限。

Method: 该研究提出了一种映射架构，其中3DSSG作为基础后端，是整个映射过程的主要知识表示。它利用增量场景图预测工作，在环境探索过程中实时推断和更新图结构，支持平坦和分层拓扑，并提供空间接地表示。

Result: 该方法保持了地图的拓扑一致性和计算效率，即使在大型环境的长期操作中也是如此。它弥合了子符号原始传感器数据和高层符号推理之间的差距，为知识驱动的框架（如知识图、本体和大型语言模型）提供了一个稳定、可验证的结构。

Conclusion: 通过将3DSSG作为基础知识表示并实时更新，该方法提高了机器人感知系统在大型真实世界环境中的一致性、可扩展性和可解释性，使其能够与人类概念更好地对齐，并被知识驱动的框架直接利用。
...
### [300] [BridgeV2W: Bridging Video Generation Models to Embodied World Models via Embodiment Masks](https://arxiv.org/abs/2602.03793)
*Yixiang Chen,Peiyan Li,Jiabing Yang,Keji He,Xiangnan Wu,Yuan Xu,Kai Wang,Jing Liu,Nianfeng Liu,Yan Huang,Liang Wang*

Main category: cs.RO

TL;DR: 本文提出了一种名为BridgeV2W的具身世界模型，通过将坐标空间动作转换为像素对齐的掩码，并将其与预训练视频生成模型结合，解决了动作与视频不匹配、视角敏感和架构不统一的问题，并在单臂和双臂机器人数据集上取得了更好的视频生成效果，同时在实际机器人任务中展现了应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现有的具身世界模型在处理坐标空间动作与像素空间视频的对齐、相机视角变化以及不同具身结构下的统一性方面存在挑战。

Method: BridgeV2W将坐标空间动作转换为从URDF和相机参数渲染的像素对齐掩码，并利用ControlNet风格的通道将其注入预训练视频生成模型。引入了基于流的运动损失来解决静态背景过拟合问题。

Result: 在DROID和AgiBot-G1数据集上，BridgeV2W在视频生成质量上优于现有SOTA方法，并且能够处理未见过的视角和场景。在策略评估和目标条件规划等下游真实世界任务中也展现了其潜力。

Conclusion: BridgeV2W成功地解决了现有具身世界模型面临的关键挑战，通过引入像素对齐的掩码和创新的架构设计，实现了更优的视频生成质量，并为下游机器人任务提供了有效的解决方案。
...
### [301] [Conformal Reachability for Safe Control in Unknown Environments](https://arxiv.org/abs/2602.03799)
*Xinhang Ma,Junlin Wu,Yiannis Kantaros,Yevgeniy Vorobeychik*

Main category: cs.RO

TL;DR: 该研究提出了一种结合保形预测和可达性分析的概率验证框架，用于未知动力学系统的安全控制，并开发了一种能优化奖励和规划时域的控制策略训练算法，在多种安全控制场景下验证了其安全性和有效性。


<details>
  <summary>Details</summary>
Motivation: 现有可验证安全控制方法在系统动力学未知或状态动作空间有限的情况下存在局限性，本研究旨在解决这一问题，为更广泛的未知动力学系统提供安全控制。

Method: 该研究结合了保形预测（Conformal Prediction）和可达性分析（Reachability Analysis）。首先，使用保形预测为未知动力学在每个时间步获取有效的区间估计（不确定性区间）。然后，利用可达性分析在保形不确定性范围内验证系统安全性。最后，开发一种算法来训练控制策略，该策略在优化标称奖励的同时，还能最大化规划时域并提供概率安全保证。

Result: 在七个安全控制场景（包括倒立摆、车道跟随、无人机控制和安全导航）和仿射/非线性安全规范下进行了评估。结果表明，所提出的方法能够实现最强的可证明安全性保证，同时保持较高的平均奖励。

Conclusion: 该研究成功开发了一个适用于未知动力学系统的概率验证框架，并提出了相应的控制策略训练算法。该方法在保证安全性的同时，还能有效地优化控制性能，为可信自主性领域的研究提供了新的方向。
...
### [302] [Estimation of Cell-to-Cell Variation and State of Health for Battery Modules with Parallel-Connected Cells](https://arxiv.org/abs/2602.02866)
*Qinan Zhou,Jing Sun*

Main category: eess.SY

TL;DR: 提出了一种基于增量容量分析（ICA）和微分电压分析（DVA）提取的模块级信息，用于同时估算电池模块中并联电池的电池间差异（CtCV）和健康状态（SoH）。


<details>
  <summary>Details</summary>
Motivation: 在仅能测量模块级信号且无法观测到个体电池行为的情况下，准确估算并联电池模块的CtCV和SoH具有挑战性，其中CtCV的估算尤其缺乏成熟方法。

Method: 提出一个统一框架，将CtCV和SoH的估算解耦为两个独立的任务，并利用从ICA和DVA提取的模块级信息来解决。框架具有灵活性，可以适应不同的CtCV指标，并且能够系统地选择最优的模块级特征。

Result: 实验验证表明，该框架能够准确估算CtCV和SoH，具有高置信度和低计算复杂度。该方法在不同C倍率下均有效，并且适用于车载实现。

Conclusion: 该统一框架能够有效地利用模块级数据，同时解决CtCV和SoH的估算问题，具有通用性、准确性和低计算复杂度，适合实际应用。
...
### [303] [Hybrid-Field Channel Estimation for XL-MIMO Systems: Dictionary-based Sparse Signal Recovery](https://arxiv.org/abs/2602.02942)
*David William Marques Guerra,Taufik Abrao*

Main category: eess.SY

TL;DR: 本研究提出了一种针对 XL-MIMO 系统混合场（HF）信道估计的统一模型和稀疏恢复方法。该方法无需预知信道稀疏度或近场/远场比例，并能自适应确定路径数量，通过参数细化解决网格失配问题，实现了准确且计算高效的信道重构。


<details>
  <summary>Details</summary>
Motivation: XL-MIMO 系统中，远场平面波和近场球面波共存的混合场传播是信道估计的关键挑战。

Method: 提出一个统一的 HF 信道模型，将近场和远场分量叠加。利用角度和极坐标域的信道稀疏性，将信道估计转化为稀疏恢复问题。该方法不依赖于信道稀疏度和近/远场比例的先验知识，通过基于残差的停止规则自适应确定路径数量，并使用结合了近场/远场字典的初始化和参数细化来解决网格失配。

Result: 仿真结果表明，所提出的估计器在视距（LoS）和非视距（NLoS）条件下都能实现准确的 HF 信道重构。

Conclusion: 该研究为 XL-MIMO 系统提供了一种实用且计算高效的混合场信道估计解决方案，克服了传统方法对信道稀疏度和近/远场比例先验知识的依赖。
...
### [304] [Fast Diffusion with Physics-Correction for ACOPF](https://arxiv.org/abs/2602.03020)
*Shashank Shekhar,Abhinav Karn,Kris Keshav,Shivam Bansal,Parikshit Pareek*

Main category: eess.SY

TL;DR: 提出了一种基于DDIM的快速扩散框架，通过结合物理引导校正，能够在生成大规模、物理一致的AC最优潮流（ACOPF）数据集时，显著加快采样速度（高达20倍），同时保持统计准确性和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的ACOPF数据集生成方法在保证解的准确性和物理一致性方面表现良好，但其慢速的采样过程限制了实际应用的可扩展性。

Method: 提出一种使用Denoising Diffusion Implicit Models (DDIM) 的快速扩散框架，并结合采样过程中的物理引导校正。该方法用确定性步骤和显式约束引导取代了缓慢的随机精炼过程，生成模型只需提供良好的初始化而非完全可行的解。

Result: 在IEEE 6、24和118节点系统上的实验表明，与标准扩散模型相比，该方法实现了高达20倍的采样速度提升，同时保持了可比的统计准确性和物理一致性。

Conclusion: 该方法能够高效地生成大规模、物理一致的ACOPF数据集，适用于可扩展的OPF数据集生成和实际的电力系统学习任务。
...
### [305] [ProOPF: Benchmarking and Improving LLMs for Professional-Grade Power Systems Optimization Modeling](https://arxiv.org/abs/2602.03070)
*Chao Shen,Zihan Guo,Xu Wan,Zhenghao Yang,Yifan Zhang,Wengi Huang,Jie Song,Zongyan Zhang,Mingyang Sun*

Main category: eess.SY

TL;DR: 本研究提出了一个用于优化电力系统潮流（OPF）建模的新数据集（ProOPF-D）和基准测试（ProOPF-B），旨在利用大型语言模型（LLMs）自动化将自然语言的运维需求转化为可执行的优化模型，以应对可再生能源带来的不确定性。


<details>
  <summary>Details</summary>
Motivation: 可再生能源的增长给电力系统运行带来了不确定性，使得频繁调整调度目标和约束成为必要，同时也给依赖专业知识的近实时建模工作流程带来挑战。现有的LLM数据集和基准测试主要关注跨领域泛化，在电力系统，特别是OPF领域，缺乏严格的评估。

Method: 引入了ProOPF-D数据集，包含12,000个配对的自然语言需求、参数调整和结构扩展到标准OPF的实例，并提供可执行的实现。同时，提出了ProOPF-B基准测试，包含121个专家标注的测试案例及地面真实代码，支持在具体和抽象的OPF建模场景下的端到端评估。

Result: 提供了用于评估LLMs在OPF建模能力的新数据集和基准测试，能够实现对模型性能的严格、端到端评估。

Conclusion: ProOPF-D和ProOPF-B为研究利用LLMs自动生成OPF模型提供了重要的资源，有助于推动电力系统运维建模的自动化和智能化。
...
### [306] [Impact of Physics-Informed Features on Neural Network Complexity for Li-ion Battery Voltage Prediction in Electric Vertical Takeoff and Landing Aircrafts](https://arxiv.org/abs/2602.03256)
*Eymen Ipek,Assoc. Mario Hirz*

Main category: eess.SY

TL;DR: 本研究提出了一种结合物理信息的数据驱动模型，用于管理电动垂直起降（eVTOL）飞机的电池系统。该模型在保持高精度的同时，大大减少了所需的可训练参数数量，降低了计算开销。


<details>
  <summary>Details</summary>
Motivation: 为满足电动垂直起降（eVTOL）飞机高精度的电池管理系统需求，同时解决现有数据驱动模型复杂和需要大量训练数据，以及等效电路模型（ECM）在高倍率非线性方面不足的问题。

Method: 通过将物理信息集成到数据驱动的代理模型中，并使用开源eVTOL电池数据集，比较了纯数据驱动模型与物理信息数据模型。评估了物理信息特征是否能简化神经网络架构而不牺牲精度。

Result: 物理信息模型在准确性上可与复杂的纯数据驱动模型相媲美，但可训练参数数量减少高达75%，显著降低了计算开销。

Conclusion: 将物理信息集成到数据驱动模型中，可以在不牺牲准确性的前提下，显著简化神经网络架构，减少计算资源需求，为在板部署提供了可行性。
...
### [307] [Power Reserve Procurement Considering Dependent Random Variables with PCE](https://arxiv.org/abs/2602.03272)
*Nicola Ramseyer,Matthieu Jacobs,Mario Paolone*

Main category: eess.SY

TL;DR: 该论文提出了一种使用广义多项式混沌来模拟相关随机变量的方法，并将其应用于概率性电力储备采购问题，通过考虑依赖关系来提高优化问题的可处理性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的优化方法通常假设随机输入变量是独立的，这在许多实际应用中是不准确的，导致模型不精确。需要一种方法来对随机输入变量之间的依赖关系进行建模，以便更准确地解决机会约束优化问题。

Method: 该方法利用广义多项式混沌来建模随机变量的联合分布，并通过高斯 copula 来捕捉任意依赖关系。为了保证算法的可处理性，研究者利用了问题的结构并开发了相应的变换。最后，将该方法应用于概率性电力储备采购问题，并与考虑独立随机变量的标准方法进行比较。

Result: 通过将该方法应用于概率性电力储备采购问题，并与考虑独立随机变量的标准方法进行比较，证明了该方法能够有效地捕捉变量之间的依赖关系，从而提高优化结果的准确性。

Conclusion: 所提出的基于广义多项式混沌和高斯 copula 的方法能够有效地对相关随机变量进行建模，并能将这种依赖关系纳入机会约束优化问题中，从而获得比独立变量假设更精确的解决方案。该方法在概率性电力储备采购问题中得到了验证。
...
### [308] [Dynamics of Implicit Time-Invariant Max-Min-Plus-Scaling Discrete-Event Systems](https://arxiv.org/abs/2602.03346)
*Sreeshma Markkassery,Ton van den Boom,Bart De Schutter*

Main category: eess.SY

TL;DR: 该论文研究了自律、时不变的隐式 max-min-plus-scaling (MMPS) 系统的动力学，提供了其解的存在条件，并分析了其全局和局部行为（增长率、不动点、归一化和稳定性），通过一个城市铁路网络的案例进行了验证。


<details>
  <summary>Details</summary>
Motivation: 为了更灵活地模拟离散事件动力学，特别是在实际离散事件应用中，需要对隐式 MMPS 模型进行深入研究。

Method: 首先，提出了隐式 MMPS 系统状态空间表示解的存在条件。然后，通过确定增长率和不动点来分析系统的全局行为。接着，通过归一化过程评估了系统在不动点附近的局部行为，并引入了归一化系统的稳定性概念。

Result: 论文提供了隐式 MMPS 系统解的存在条件。对于可解系统，分析了其增长率、不动点、局部行为和稳定性。通过城市铁路网络的案例研究验证了理论结果。

Conclusion: 该研究为理解和分析隐式 MMPS 系统的动力学提供了理论框架，并展示了其在实际离散事件系统建模中的应用潜力。
...
### [309] [When control meets large language models: From words to dynamics](https://arxiv.org/abs/2602.03433)
*Komeil Nosrati,Aleksei Tepljakov,Juri Belikov,Eduard Petlenkov*

Main category: eess.SY

TL;DR: 本研究探讨了大型语言模型（LLMs）与控制理论之间的双向互动关系，提出了一种从提示设计到系统动力学的连续体模型。研究一方面展示了LLMs如何辅助控制系统设计和研究，另一方面则研究了控制理论如何帮助LLMs优化其行为以实现特定目标。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在工程和技术领域的应用日益广泛，但其复杂性也带来了行为调控的挑战。本研究旨在探索LLMs与控制理论之间的互惠关系，以期增强LLMs在工程应用中的可控性和鲁棒性。

Method: 研究方法包括：1. 探索LLMs在控制器设计和研究工作流方面的辅助作用；2. 应用控制理论概念，如输入优化、参数编辑和激活层干预，来引导LLMs的行为；3. 将LLMs视为状态空间框架下的动态系统，并将其内部表征与外部控制环路结合；4. 识别关键挑战并提出未来研究方向。

Result: 研究结果表明，LLMs可以作为控制系统设计的有力工具，同时控制理论也能有效指导LLMs的输出，使其更具可达性和一致性。将LLMs视为动态系统并结合控制理论，有望实现更深层次的集成。

Conclusion: 本研究提出了LLMs与控制理论相互促进的双向模型，并展示了如何通过控制理论增强LLMs的可控性和鲁棒性。未来的研究应专注于理解LLMs的行为，并开发可解释、可信赖且稳健的LLMs，以确保其对社会的积极贡献。
...
### [310] [Real-world energy data of 200 feeders from low-voltage grids with metadata in Germany over two years](https://arxiv.org/abs/2602.03521)
*Manuel Treutlein,Pascal Bothe,Marc Schmidt,Roman Hahn,Oliver Neumann,Ralf Mikut,Veit Hagenmeyer*

Main category: eess.SY

TL;DR: 该研究发布了一个名为 FeederBW 的真实世界低压电网数据集，其中包含来自德国配电系统运营商 Netze BW 的两年数据，涵盖了 200 个低压馈线，并附带天气信息和详细元数据，旨在支持低压电网在能源转型中的研究和应用。


<details>
  <summary>Details</summary>
Motivation: 能源转型导致低压电网连接越来越多的低碳技术，给电网的运行和规划带来挑战，但研究人员缺乏合适的低压电网数据来支持相关研究。

Method: 收集并整理了德国配电系统运营商 Netze BW 提供的真实世界能源数据，涵盖 200 个低压馈线在两年（2023-2025）内的详细信息，包括天气数据、元数据（如低碳技术安装变化）、馈线特征（如住房单元数量、低碳技术装机功率）以及工业能源数据。数据具有高光伏接入和一分钟的时间分辨率。

Result: FeederBW 数据集提供了高光伏接入和一分钟时间分辨率的真实世界低压电网数据，并包含详细的元数据，支持机器学习应用（如负荷预测、非侵入式负荷监测），揭示了低碳技术对低压电网日益增长的影响，并显示了天气、馈线测量和元数据之间的相互作用。

Conclusion: FeederBW 数据集为低压电网的研究和应用提供了宝贵的资源，特别是对于应对能源转型带来的挑战，支持了包括机器学习在内的多种分析方法，并证实了低碳技术对电网影响的增加。
...
### [311] [A Comparison of Set-Based Observers for Nonlinear Systems](https://arxiv.org/abs/2602.03646)
*Nico Holzinger,Matthias Althoff*

Main category: eess.SY

TL;DR: 本文对非线性离散时间系统的基于集合的状态估计方法进行了统一的评估和比较，并提供了可复现的开源实现。


<details>
  <summary>Details</summary>
Motivation: 现有针对非线性离散时间系统的基于集合的状态估计方法众多，但缺乏在可比条件下的统一评估，这阻碍了对其优劣势的客观认识和应用。

Method: 在CORA框架下，对代表性的基于集合的观测器进行了实现，并在通用基准测试中，从计算量、可扩展性和保守性等方面进行了评估。

Result: 研究揭示了不同观测器类别和集合表示方法之间的典型权衡，以及在实际实现中出现的考虑因素。所有实现均开源。

Conclusion: 本文首次提供了对非线性离散时间系统保证状态估计器的一次广泛的、由工具支持的比较，为未来的研究和应用奠定了基础。
...
### [312] [Input-to-State Safe Backstepping: Robust Safety-Critical Control with Unmatched Uncertainties](https://arxiv.org/abs/2602.03691)
*Max H. Cohen,Pio Ong,Aaron D. Ames*

Main category: eess.SY

TL;DR: 该论文提出了一种用于处理非线性控制中不匹配扰动（无法通过控制输入直接抵消的不确定性）的安全关键控制方法。该方法结合了输入状态安全（ISSf）框架和最优衰减容量约束（CBF），并为严格反馈系统和双相对阶系统构造了ISSf-CBF。


<details>
  <summary>Details</summary>
Motivation: 在非线性控制中，如何处理不匹配扰动以保证系统的安全性是一个关键且尚未完全解决的挑战。

Method: 本文提出了一种利用最优衰减容量约束（Optimal Decay CBF）来推广输入到状态安全（ISSf）框架的方法，以处理具有不匹配扰动的系统。然后，为严格反馈系统和双相对阶系统（类似于微分平坦系统）构造了ISSf-CBFs。

Result: 理论上，文章展示了如何为具有不匹配不确定性的两类系统（严格反馈系统和双相对阶系统）构造ISSf-CBFs。数值模拟结果（以倒立摆和平面四旋翼为例）验证了该方法的有效性。

Conclusion: 该研究为具有不匹配扰动的非线性安全关键控制提供了一种建设性的方法，通过结合ISSf和Optimal Decay CBF，并为特定系统类提供了具体的构造步骤。
...
### [313] [Mitigating Timing-Based Attacks in Real-Time Cyber-Physical Systems](https://arxiv.org/abs/2602.03757)
*Arkaprava Sain,Sunandan Adhikary,Soumyajit Dey*

Main category: eess.SY

TL;DR: 研究提出了一种基于调度的缓解方法，通过有界地、结构化地引入控制任务执行的时间扰动，在保证实时性的前提下，减少针对实时网络物理系统的定时推断攻击。


<details>
  <summary>Details</summary>
Motivation: 实时网络物理系统（CPS）的确定性执行可能暴露定时信息，使对手能够进行定时推断攻击。现有防御措施通常忽略了对闭环控制行为和实时可行性的影响。

Method: 提出了一种基于调度的缓解方法，该方法将有界的时间扰动以结构化的方式引入控制任务的执行中，同时联合考虑了最坏情况执行行为以及执行延迟对控制性能的影响。

Result: 通过实验评估，证明了该方法能够显著降低定时推断攻击的暴露风险，同时保持可预测的执行和可接受的控制质量。

Conclusion: 所提出的框架能够同时满足实时性约束和控制性能要求，为在保证系统安全和性能的同时防御定时推断攻击提供了一种有效的方法。
...
### [314] [Super-résolution non supervisée d'images hyperspectrales de télédétection utilisant un entraînement entièrement synthétique](https://arxiv.org/abs/2602.02552)
*Xinxin Xu,Yann Gousseau,Christophe Kervazo,Saïd Ladjal*

Main category: eess.IV

TL;DR: 提出一种基于合成丰度数据的无监督高光谱单图像超分辨率方法，通过高光谱解混获得端元和丰度图，然后利用死树模型生成的合成数据训练神经网络进行丰度图超分辨率，最后重构超分辨率高光谱图像。


<details>
  <summary>Details</summary>
Motivation: 现有的高光谱图像超分辨率方法大多依赖于监督学习，需要高分辨率的真实数据，而这类数据在实际中通常难以获得。

Method: 首先，利用高光谱解混技术将高光谱图像分解为端元和丰度图。然后，使用死树模型生成的合成丰度数据训练一个神经网络，以实现丰度图的超分辨率。最后，将超分辨率的丰度图与端元重新组合，重建出超分辨率的高光谱图像。

Result: 实验结果表明，所提出的无监督方法在不依赖真实高分辨率数据的情况下，能够有效地进行高光谱单图像超分辨率，并且证明了合成数据在训练中的有效性。

Conclusion: 该研究提出了一种创新的无监督高光谱单图像超分辨率方法，通过利用合成丰度数据克服了对真实高分辨率地面真实数据的依赖，为实际应用提供了可行方案。
...
### [315] [Physics-based generation of multilayer corneal OCT data via Gaussian modeling and MCML for AI-driven diagnostic and surgical guidance applications](https://arxiv.org/abs/2602.02755)
*Jinglun Yu,Yaning Wang,Rosalinda Xiong,Ziyi Huang,Kristina Irsch,Jin U. Kang*

Main category: eess.IV

TL;DR: 提出一个可配置的蒙特卡洛模拟框架，生成带像素级五层分割标签的合成角膜OCT B扫描图像，以解决数据集不足的问题，并支持AI模型在可控条件下进行训练和评估。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在角膜OCT成像中的训练受限于大规模、标注良好的数据集的可用性。

Method: 构建一个包含高斯曲面和不同光学特性的五层角膜模型，使用蒙特卡洛光传输模拟（MCML）生成B扫描图像，并考虑了点扩散函数（PSF）和灵敏度衰减等系统特征，从而生成带有像素级分割标签的合成图像。

Result: 生成了超过10,000对高分辨率（1024x1024）的图像-标签对，并支持自定义几何形状、光子计数、噪声和系统参数。

Conclusion: 该合成数据集能够用于在可控、具备真实标注的条件下，系统地训练、验证和评估AI模型，为眼科图像引导的诊断和手术指导应用提供了一个可重复、可扩展的资源。
...
### [316] [EchoJEPA: A Latent Predictive Foundation Model for Echocardiography](https://arxiv.org/abs/2602.02603)
...
TL;DR: 本文提出了一种名为EchoJEPA的超声心动图基础模型，该模型通过在海量未标注视频数据上进行训练，能够有效处理超声图像中的解剖信号、散斑和伪影，并在左心室射血分数估计、心腔视图分类以及对噪声和跨人群（儿科）的鲁棒性方面取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有超声心动图基础模型在区分解剖信号与超声图像固有的散斑和采集伪影方面存在不足。研究旨在开发一种能够学习可泛化表征，并克服这些挑战的模型，以减少标注负担并提高诊断一致性。

Method: 研究提出了EchoJEPA基础模型，并在包含30万患者的1800万份超声心动图数据集上进行预训练。同时，引入了一种新颖的多视图探针框架，具有因子化流嵌入，可以在固定骨干网络的情况下标准化评估。该模型采用潜在预测作为其训练范式。

Result: EchoJEPA将左心室射血分数估计误差降低了19%，心腔视图分类准确率达到87.4%。在仅使用1%标记数据时，EchoJEPA的准确率达到78.6%，远超仅使用100%标记数据的基线模型（42.1%）。在声学扰动下，EchoJEPA性能仅下降2.3%，而次优模型下降16.8%。此外，EchoJEPA在零样本迁移到儿科患者时，误差比次优模型低15%，优于所有微调基线。

Conclusion: 研究结果表明，潜在预测是一种优越的超声基础模型范式，EchoJEPA在多种评估指标上均表现出色，尤其是在处理复杂超声信号、样本效率和模型泛化能力方面，为超声心动图的AI应用树立了新的标杆。
...
### [317] [Wide-field high-resolution microscopy via high-speed galvo scanning and real-time mosaicking](https://arxiv.org/abs/2602.02758)
*Ziyi Huang,Rosalinda Xiong,Yaning Wang,Jinglun Yu,Jin U. Kang*

Main category: eess.IV

TL;DR: 提出了一种适用于线性与正弦扫描的宽场显微成像图像拼接框架，通过几何拼接、ROI亮度校正和无缝融合，解决了扫描不均匀导致的几何和亮度问题，实现了大视野（2.5x2.5 cm^2）高分辨率（7.81 μm）图像的高效重建（6秒/数据集）。


<details>
  <summary>Details</summary>
Motivation: 传统宽场高分辨率显微成像面临扫描速度和图像拼接的挑战，尤其是正弦扫描引入的非均匀采样会导致几何不一致和亮度变化。需要一种方法来解决这些问题，实现高质量的宽场图像拼接。

Method: 提出一个图像拼接框架，结合了基于平移的几何拼接模型、基于感兴趣区域（ROI）的亮度校正以及缝隙感知羽化。该方法依赖于校准的扫描参数和同步的扫描-相机控制，无需基于图像内容的配准。适用于线性和正弦扫描。

Result: 成功在幅宽达2.5 x 2.5 cm^2的视野下，以约6秒的采集时间，重构了线性扫描和正弦扫描策略下的宽场拼接图像。量化评估显示，处理后两种扫描策略的图像质量均有提升，包括亮度均匀性、对比度噪声比（CNR）和接缝伪影减少，同时保持了7.81 μm的横向分辨率。

Conclusion: 所提出的框架为基于扫描的宽场显微图像拼接提供了一个实用且高效的解决方案，能够有效提高图像质量并保持高分辨率，适用于不同扫描策略。
...
### [318] [Real-time topology-aware M-mode OCT segmentation for robotic deep anterior lamellar keratoplasty (DALK) guidance](https://arxiv.org/abs/2602.02798)
*Rosalinda Xiong,Jinglun Yu,Yaning Wang,Ziyi Huang,Jin U. Kang*

Main category: eess.IV

TL;DR: 提出了一种基于U-NeXt的、轻量级且拓扑感知的M模式分割算法，用于机器人深板层角膜移植术（DALK）中的实时深度反馈，以应对M模式OCT数据中的噪声和不连续性问题。该算法通过引入解剖学拓扑正则化来稳定边界连续性和层序，实现了超过80 Hz的端到端处理速度，并能有效处理低信噪比数据。


<details>
  <summary>Details</summary>
Motivation: 机器人DALK手术需要实时精确的深度反馈以避免穿透Descemet's膜（DM）。M模式OCT虽然时间分辨率高，但存在散斑噪声、衰减和阴影等问题，导致层界面不连续或模糊，难以在实际部署帧率下进行准确分割。

Method: 提出了一种基于U-NeXt的轻量级、拓扑感知的M模式分割流水线。该算法将解剖学拓扑正则化融入其中，以稳定边界连续性和层序，尤其是在低信噪比条件下。整个预处理、推理和叠加流水线的端到端吞吐量超过80 Hz（单GPU）。

Result: 该算法在标准兔眼M模式数据集上进行的评估显示，与拓扑无关的对照组相比，在定性边界稳定性方面有所提高，同时保持了可部署的实时性能。

Conclusion: 该算法能够有效地处理M模式OCT数据中的噪声和不连续性，为机器人DALK手术提供稳定、实时的深度反馈，并具备足够的处理余量来应对低质量或丢失帧，从而保证手术的安全性和有效性。
...
### [319] [Super-Resolution and Denoising of Corneal B-Scan OCT Imaging Using Diffusion Model Plug-and-Play Priors](https://arxiv.org/abs/2602.02795)
*Yaning Wang,Jinglun Yu,Wenhan Guo,Ziyi Huang,Rosalinda Xiong,Yu Sun,Jin U. Kang*

Main category: eess.IV

TL;DR: 本研究提出了一种结合扩散模型即插即用（PnP）先验的超分辨率框架，用于增强眼科光学相干断层扫描（OCT）B扫描图像的空间分辨率并去除散斑噪声，实现了4倍分辨率提升和有效去噪。


<details>
  <summary>Details</summary>
Motivation: 为了克服高帧率OCT成像中空间分辨率下降和散斑噪声增多的问题，以提高诊断和手术规划的准确性。

Method: 将图像重建问题建模为一个贝叶斯逆问题，结合马尔可夫链蒙特卡洛（MCMC）采样和预训练的生成扩散模型先验，实现4倍分辨率提升和去噪。使用眼科鱼眼角膜数据集进行体内验证。

Result: 与双三次插值、U-Net和其它扩散模型先验相比，该方法能生成更精确的解剖结构，更好地区分角膜层，并显著抑制噪声。在峰值信噪比、结构相似性指数和感知指标上均达到最先进水平。

Conclusion: 基于扩散模型的即插即用重建方法能够提供高保真、高分辨率的OCT成像，有望支持更可靠的临床评估和先进的图像引导介入治疗，并且该方法具有扩展到其他生物医学成像模态的潜力。
