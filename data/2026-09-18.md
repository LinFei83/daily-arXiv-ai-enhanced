...
TL;DR: 论文研究强调式时序差分学习（ETD）的常步长采样动态稳定性，构造两状态反例说明ETD均值映射收缩但采样乘积的Lyapunov指数为正，并提出正则化强调式TD（RETD）以修复后冲击动态，证明收敛性并实验验证。


<details>
  <summary>Details</summary>
Motivation: ETD虽能稳定期望off-policy TD更新并改变投影几何，但这两点都不能决定常步长采样动态；需要分离均值收缩与采样稳定性，并处理follow-on trace的无限方差问题。

Method: 构造遍历两状态反例；使用再生循环分析分离符号与无限方差；提出RETD，采用归一化一阶后冲击修复，保持trace和重要性比不变，用泄漏标量状态存储emphatic TD信号并释放延迟校正；证明调和递减步长下的几乎必然收敛，以及来自马尔可夫随机乘积界的条件常步长矩收缩；进行10,000次配对实验。

Result: RETD原始均衡是ETD均衡的仿射偏移，单正则和双正则读出可精确恢复ETD不动点；在两状态构造和一个Baird点上获得认证负指数，而正Baird ETD符号仍为数值结果；实验验证分离、不动点恢复、非单调稳定域和任务依赖性；RETD改变后冲击动态，但不降低共享follow-on-trace方差。

Conclusion: RETD能改变后冲击动态并稳定采样行为，同时恢复ETD固定点，但不会减少follow-on-trace方差；其稳定性区域非单调且依赖任务。
...
TL;DR: 提出 BioPhys-Bridge，一个面向生物物理文献、要求证据溯源的跨学科科学推理基准，包含500个案例和1517个任务，用于评估大模型在归因、忠实性和复杂多步推理上的表现。


<details>
  <summary>Details</summary>
Motivation: 语言模型在分析跨学科科研文献时面临独特挑战：生物物理问题需要将观测数据与来源证据对应、用定量物理模型解释，并关联到生物机制，而现有基准难以覆盖这种证据接地与多步推理需求。

Method: 构建 BioPhys-Bridge 基准数据集，每个案例包含证据块、稳定证据ID、定量数值、单位、方程、假设、机制和后续决策等接地目标，支持问答与检索增强生成任务；设置模式、证据完整性、定量接地、来源许可、重复、单位归一化等严格质量门，并对81个案例进行领域专家审核标注；用多个模型进行初步评估。

Result: 初始版本含500个案例、1517个面向智能体的任务，覆盖6个生物领域和9个物理模型族（其中3个稀疏族预留扩展）。初步评估中，DeepSeek-V4-Flash 取得最高 evidence-ID F1（0.360），Qwen3.7-Max（0.316）和 GPT-4o-mini（0.294）紧随其后。

Conclusion: BioPhys-Bridge 是一个用于评估归因、忠实性、幻觉减少和生物实验设计的跨学科基准，支持复杂多步科学推理；未来将扩大数据规模与复杂度并开展全面评估，代码和数据已在 GitHub 与 Hugging Face 公开。
...
TL;DR: 系统分析了2022年1月至2026年8月arXiv上14,767篇引入或更新评估资源的论文，考察LLM评估基准的变化趋势。


<details>
  <summary>Details</summary>
Motivation: 基准对评估LLM进展至关重要，但仅凭模型排名难以揭示评估需求本身如何变化；基准的多样性可反映研究者对LLM能力期望及成功标准的变化。

Method: 从arXiv收集14,767篇评估资源相关论文，采用分阶段筛选和自动全文编码，分析目标系统与领域、评估材料与条件、评分机制的变化。

Result: 评估更强调行动、交互和专业应用，新旧设计元素常共存；LLM-based scoring在智能体和非智能体组均增长，但模型生成材料近期未持续增加。

Conclusion: 研究揭示了公共研究如何将能力期望转化为具体测试和成功标准；随着AI参与构建测试、执行任务和评判回答，需思考评估扩展是提供更独立证据，还是复制参与模型的偏好与盲点。
...
TL;DR: 该立场论文主张，AI应用已从单体基础模型转向复合智能体系统，但当前技术栈碎片化，因此需要引入基础模型操作系统（FMOS），作为虚拟化基础模型交互的系统层。


<details>
  <summary>Details</summary>
Motivation: 当前协议（如MCP、A2A）虽改善了工具与智能体连接，但各框架仍内嵌隐式运行时来管理状态、记忆、预算和护栏，导致行为不可移植、治理脆弱，类似操作系统出现前的计算机。

Method: 作为立场论文，提出FMOS概念，类比虚拟机抽象物理硬件；FMOS内部负责跨记忆层级编排知识、模型选择与资源分配、验证与策略执行，并借鉴人脑快慢思考，学习何时干预、何时直接推理，并持续根据运行经验调整策略。

Result: 论文未报告实验或量化结果，主要贡献是提出FMOS的架构愿景、类比与设计原则。

Conclusion: 该领域需要FMOS这一系统层，使应用获得专用、可信且能力近乎无限的基础模型实例，并实现可移植、可治理的复合智能体系统。
...
TL;DR: 论文提出 TranSGrid 测试平台，将演绎、归纳和溯因推理统一到同一任务中，用以更严格地研究系统泛化，并发现现有简化任务会低估系统泛化的难度。


<details>
  <summary>Details</summary>
Motivation: 系统泛化是人类智能的核心，但现有研究常通过近似线性动作组合、基于生产性的测试和动作显式目标等简化来降低研究难度，遗漏了该能力的一些关键方面。

Method: 作者采用以推理为中心的视角，构建 TranSGrid，将演绎、归纳和溯因推理结合在同一任务中；并用七个 Transformer 在 4,800 个 TranSGrid 实例上进行实验，同时设置变体重新引入原有简化。

Result: 最大模型在留出测试集上解决 79.6%，但在 TranSGrid 上仅 55.3%，在最难子集上仅 15.8%；差距在训练长度范围内仍存在。重新引入近似线性动作组合或动作显式目标后，解决率分别回升到接近测试集水平。

Conclusion: 现有任务降低了归纳和/或溯因推理需求；仅靠生产性不足以评估系统泛化。全面衡量系统泛化需要同时涉及演绎、归纳和溯因三种推理的任务。
...
TL;DR: 本文首次系统研究了ChatGPT、Claude、Grok和DeepSeek四大对话平台中LLM智能体使用Web搜索的完整生命周期，涵盖是否搜索、如何构造查询、搜索结果来源偏好以及如何生成基于搜索的回答。


<details>
  <summary>Details</summary>
Motivation: 对话式LLM智能体越来越依赖Web搜索，但对其端到端搜索行为与质量仍缺乏理解，因此需要实证研究其决策、策略、来源偏好和归因问题。

Method: 结合真实用户交互（invivo）与通过各平台API进行的受控实验（invitro），跨四个主要对话平台分析智能体的Web搜索调用决策、查询构造策略、搜索结果域名偏好以及将结果转化为有依据回答的方式。

Result: Web搜索决策在不同平台和模型间差异显著，更频繁调用搜索并不必然带来更好回答质量；智能体采用不同的复杂查询策略；平台特定搜索引擎倾向于返回偏好域名结果；回答虽大多基于搜索结果，但部分主张依赖未引用的搜索结果，引发归因与可靠性担忧。

Conclusion: 研究揭示了对话式智能体Web搜索生命周期中的关键差异与风险，对未来面向对话检索优化的AI智能体和Web搜索工具设计具有重要启示。
...
TL;DR: 论文提出 AutoTuring 对比方法：让同一智能体在相同的 15 维加速器设计空间中，分别面对有语义的架构旋钮与无意义的 [0,1] 匿名变量，以测量“问题是否有意义”对硬件设计的影响。


<details>
  <summary>Details</summary>
Motivation: 现有报告只能说明智能体改进了设计，却无法判断它是在推理机器架构，还是在对无意义旋钮进行有效搜索；前者才能迁移到新架构，但已有评测固定问题框架、只变化智能体，无法区分两者。

Method: AutoTuring 将同一智能体、同一 15 维加速器空间分别以两种方式呈现：一是有名称的架构旋钮与模拟器计数器，二是 [0,1] 上的匿名变量；评测器、合法空间和可达最优值保持一致，仅改变问题是否有意义。实验在九核 FP16 GEMM 任务集上进行，并引入 critic loop，每组条件运行 5 至 6 次。

Result: 有语义的“架构师”智能体平均比建模的 H200 快 5.4%，比其盲搜对应版本快 12.3%，且模拟器调用次数减少 70.1%。但语义优势并非唯一来源：critic loop 能弥补盲搜智能体的大部分差距，却对架构师智能体没有增益，说明架构知识与结构化批评是替代关系而非互补关系。

Conclusion: 作者将此作为初步发现，仅基于单个建模加速器和每组 5 至 6 次运行；其核心贡献不是加速器本身，而是该对比评测方法，用以区分智能体是在理解架构还是在盲目搜索。
...
TL;DR: 提出MAGS多智能体框架，以Dafny为验证中间表示，为LLM生成的代码提供形式化安全保证，在220个任务上实现100%成功生成满足冻结规范的程序。


<details>
  <summary>Details</summary>
Motivation: LLM编码代理生成的复杂程序规模庞大，人工审查困难，存在安全风险；模糊测试、静态分析和LLM验证器难以覆盖所有边界情况；形式化验证可提供机器可检查的保证，但传统上需要大量手动规范和证明工程。

Method: 统一多智能体框架MAGS：形式化并冻结经人工审计的API和安全需求；将生成代码翻译为Dafny；利用验证器反馈修复违规；将验证通过的代码编译回可执行代码。

Result: 在100个CUDA内核、100个终端脚本和20个机械臂任务共220个例子上，100%成功生成具有非平凡安全保证且符合冻结规范的程序；独立安全与功能评估显示三个领域表现强劲，但当自动形式化语义未能完全捕获目标行为时会出现失败。

Conclusion: MAGS能有效为LLM生成代码提供形式化安全保证，但其效果依赖于自动形式化语义的准确性；当语义不完整时仍存在失败风险。
...
TL;DR: 本文是一篇测量与基准研究，揭示并量化了工具增强LLM智能体的“工具幻觉”问题：智能体会调用不存在的工具并传入无schema声明的参数。作者提出五类幻觉分类（H1-H5）与一个免训练的封闭世界解析器Resolution Rung，证明幻觉防御必须前置于任何因果门控，并在单一注册表与MCP两种调用面上进行了大规模测量，发布了HTB基准。


<details>
  <summary>Details</summary>
Motivation: 现有防御要么关注工具选择（tool-selection），要么关注工具安全门控（gating），二者都预设了智能体发出的调用确实指向一个真实工具。然而，工具幻觉调用在构造上就不是任何门控做出的决策，因此任何门控都无法拒绝它，这是现有方法的结构性盲区。

Method: 作为测量与基准研究，作者：（1）给出工具幻觉的五类分类法（H1-H5）；（2）提出Resolution Rung——一个免训练、封闭世界的解析器（基于注册表成员资格加签名校验），并论证幻觉防御必须前置于任何因果门控；（3）刻画了唯一不可约的残余情况（借用参数、在schema上与合法调用无法区分）；（4）在两种调用面、十个托管模型上测量真实幻觉；（5）将研究扩展到Model Context Protocol（MCP），提出第二套分类法（M1-M5）；（6）发布版本化的Hallucinated-Tools Benchmark（HTB）。

Result: 在十个托管模型、两种调用面下共测得322次真实幻觉；虚构工具调用集中在无约束的raw-JSON调用面（34次 vs. 3次），且模型规模无帮助（675B模型与7-8B模型表现相当）。在实时MCP调用面上测得154次幻觉，包括在单注册表调用面上表现干净的前沿模型，原因是多服务器合并到同一命名空间带来的碰撞与遮蔽是结构性缺陷。

Conclusion: 工具幻觉是现有工具选择与门控方法无法覆盖的结构性盲区，幻觉防御必须前置于任何因果门控，且存在一类不可约的残余。MCP的多服务器命名空间合并进一步制造了单一注册表无法表达的幻觉面。作者发布HTB基准，使不同解析器可在统一标准下跨提交比较。
...
TL;DR: 本文从认知科学与计算机科学出发，将目标视为可与世界知识灵活组合的组成性表征，强调其内容与理性行为的关系，并综合不同目标表征的性质，提出目标表征构成一个更广泛的设计空间。


<details>
  <summary>Details</summary>
Motivation: 目标通常被视为固定并施加行为约束，但其表征本身的性质——如表达力、设计取舍和效率——尚未被充分探讨；同时，目标与语言学、逻辑学中的句法-语义界面存在类比，促使研究者重新审视目标的形式与意义。

Method: 采用理论综合与概念分析，借鉴认知科学、计算机科学、语言学和逻辑学，特别是句法-语义界面思想，比较不同目标表征的属性，并构建目标表征的设计空间框架。

Result: 指出目标表征可具有不同表达力、设计和效率，能捕捉相同或不同行为，并可将这些性质视为设计空间中的点；区分目标的形式与意义有助于揭示隐含假设。

Conclusion: 刻画目标表征的性质能阐明我们对目标的默认假设，推动高阶认知与动机交互研究，并为不同目标概念划分变异轴。
...
TL;DR: 论文研究语言模型后训练（如强化学习）对组合推理的影响，提出依赖图框架来形式化组合推理，并通过数据结构任务和真实工具调用基准进行实证分析。


<details>
  <summary>Details</summary>
Motivation: 组合推理对现实问题求解至关重要，因为训练数据有限，模型必须通过组合已学技能来泛化；但后训练方法对组合推理的具体影响仍不清楚。

Method: 提出依赖图框架，定义三个复杂度递增的组合层级；用具有确定性奖励和清晰组合结构的数据结构任务实例化；进行理论解释，并评估长度外推、结构分布偏移、未见技能迁移及真实工具调用基准。

Result: 发现一致的“分解到组合”不对称：分解技能训练不能可靠迁移到组合任务，而组合任务训练更容易迁移回分解任务；理论解释了该不对称，并在工具调用基准中观察到初步证据。

Conclusion: 后训练在组合推理上存在系统性不对称，组合任务训练可能比分解技能训练更有效；该现象可能扩展到实际应用，提示需重视组合任务训练与评估。
...
TL;DR: 该论文提出利用LLM内部激活训练轻量级MLP探针来检测有害提示，在多个基准上取得与超大外部防护模型相当的效果，同时显著降低延迟和计算开销。


<details>
  <summary>Details</summary>
Motivation: 自主系统依赖LLM，但现有安全基础设施带来延迟和计算开销，难以用于资源受限、时间敏感场景；外部防护模型无法理解模型内部状态，存在根本性保障缺口。

Method: 从LLaMA-3.1-8B提取激活，训练轻量级MLP分类器探针（12.6M参数）检测有害提示。

Result: 在WildJailbreak、Beavertails和AEGIS 2.0上，F1分别达到99%、83%和84%，与参数量大1000倍的防护模型相当，同时降低延迟和计算成本。

Conclusion: 模型内部激活已包含判断内容是否有害的信号，基于内部探针的轻量级防护方案可行且高效。
...
TL;DR: 本文提出QVAC Genesis III，一个191.43B token、覆盖19个STEM领域的多领域合成语料库，旨在为边缘设备和小模型提供高 token 学习价值的预训练数据。通过从零训练的1.7B模型验证，其在ARC、GPQA Diamond和MMLU STEM上优于Cosmopedia-v2和Cosmo-1B。


<details>
  <summary>Details</summary>
Motivation: 高质量预训练数据是面向边缘AI和端侧部署的教育与STEM语言模型的关键瓶颈，因为token预算受限；同时开放生态缺乏能高效提升小模型性能的STEM合成数据集。

Method: 构建QVAC Genesis III合成语料库，采用双生成策略：以弱边缘规模学生模型为信号进行教师蒸馏，将学生失败转化为纠正性解释，将成功扩展为覆盖所有选项的对比式推理；并提出LLM-as-a-parser评估协议，从自由形式输出中提取最终答案并跟踪准确率和答案有效性。

Result: 使用1.7B参数模型从零训练进行受控消融，QVAC Genesis III训练的模型在ARC、GPQA Diamond和MMLU STEM上持续优于Cosmopedia-v2和公开的Cosmo-1B；ARC-E最高提升+28.57%，ARC-C最高提升+21.35%，有效答案率最高达99.45%。

Conclusion: QVAC Genesis III能为STEM小模型提供高 token 效率的合成预训练数据，显著提升多项STEM基准表现，有助于缓解开放生态中高质量STEM数据不足的问题。
...
TL;DR: 论文提出 Verify–Repair–Reselect (VRR)，把验证反馈用于改进候选集本身，而不仅是排序。它通过 LLM-as-an-Improver 条件生成修复版和新方法候选，过滤后重新选择，在代码生成和推理任务中常优于固定候选池选择，甚至能纠正初始候选全错的情况。


<details>
  <summary>Details</summary>
Motivation: 现有基于验证器的选择方法通常只把验证当作排序步骤，一旦固定候选池评估完就丢弃验证反馈。本文探究验证反馈是否也能改进候选集本身。

Method: 提出 VRR：保留初始胜者，并条件生成三个互补候选，包括胜者修复版、次优者修复版和基于新方法的解；仅利用推理时信息过滤无效与重复候选；最后按原始评价标准重新选择答案。

Result: 在多种模型以及代码生成和推理基准上，VRR 在许多设置中优于固定池的验证器选择方法，并且当初始候选池中所有答案都错误时，也能恢复出正确解。

Conclusion: LLM 可以扮演改进者角色：验证反馈不仅能从现有解中选择，还能构建超出初始候选池的更强候选。
...
TL;DR: 论文探讨长时程语言模型代理如何持续运行而不遗忘，并提出一种位于模型外部的分层“框架”架构，使代理能跨上下文和会话边界维持任务主线。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型代理需执行持续数天或数周的任务，但受限于上下文窗口、进程生命周期和人类关注间隔。作者认为，长时程代理必须先能持续运行而不遗忘，才可能持续学习。

Method: 从长时程场景中提炼出七个瓶颈，并设计三部分分层架构：按时间尺度分层，每层用有界文件总结下一层；以时钟滴答作为自主行动单位；采用级联智能，仅在审查失败后将任务升级给更强模型。

Result: 通过十天实验活动，代理在人类每天仅参与一次的情况下复现了一项已发表的强化学习结果。结果表明：代理能在每次上下文重置和会话边界后保持任务主线；早期写入的操作知识无需改变模型权重即可影响后续行为；并说明了学习组件可进入该系统的位置。

Conclusion: 长时程代理的持续学习需要一个比任何上下文和进程都更持久的基底；框架已执行的检查环节正是学习器应嵌入之处。
...
TL;DR: 论文提出 EconSkills，一个将已验证的 EconWebArena 轨迹蒸馏为参数化标准操作流程的技能库与评估框架，用于经济数据网页检索，并验证可复用程序能否迁移及在技能库中保持收益。


<details>
  <summary>Details</summary>
Motivation: 网页智能体经常重复访问相同网站，但多数评估丢弃了此前成功交互中学习到的流程，导致已有经验无法被复用。

Method: 构建 EconSkills 技能库与评估框架：将验证过的轨迹抽象为参数化 SOP，每个技能记录适用范围、导航流程、站点特定指导、验证检查和恢复步骤，并用占位符替换实例值；区分已知相关流程向留出任务的迁移，以及智能体从技能库中选择时能否保留收益；进行受控迁移实验、技能库规模实验和浏览器轨迹分析。

Result: 受控迁移中，匹配技能相比无技能提示提高成功率，并在配对成功案例上减少步骤；抽象化显著优于重放原始轨迹。技能库规模下，检索整体与无技能基线相当，在直接覆盖任务上表现最好；覆盖分层结果显示，未覆盖任务上的近似匹配会抵消部分收益。浏览器轨迹进一步识别出程序性指导何时能缩短特定门户导航，以及何时仍需语义验证。

Conclusion: 可复用的经济网页操作程序能够跨任务实例迁移；研究为覆盖感知的选择与上下文传递提供了具体设计目标。
...
TL;DR: 提出一个统一的AI可信度评估框架，围绕八个维度连接输出级、轨迹级和跨模态评估，并加入元评估与治理映射，以弥补基准分数不足。


<details>
  <summary>Details</summary>
Motivation: 仅靠基准分数无法充分评估现代AI系统（如大语言模型、智能体系统、多模态模型）的可信度；不同系统需要不同评估方式，同时评估证据必须可解释、可追溯，并能支持开发与监督。

Method: 构建统一框架：定义能力、鲁棒性、安全、公平、透明、治理、监督、效率八个可信维度；将系统特定原生指标映射到共同性能区间，并附带不确定性估计与可追溯证据；设置元评估层检验评估本身的效度、信度和可复现性；使用多维画像与安全关键覆盖机制；映射到治理框架、国际标准和欧盟监管要求。

Result: 该框架能揭示系统优势与弱点，防止聚合分数掩盖关键失败，并将技术评估与监督需求连接起来；但跨部署情境的实证验证仍是必要下一步。

Conclusion: 该框架为评估AI系统性能及其支撑证据的可信度提供了结构化基础，后续需在不同部署环境中进行实证验证。
...
### [18] [Neuro-Symbolic Agentic AI for Networked Low-Altitude UAVs](https://arxiv.org/abs/2609.19961)
*Yuqi Ping,Tianhao Liang,Nanchi Su,Guangyu Lei,Junwei Wu,Qinyu Zhang,Tingting Zhang*

Main category: cs.AI

TL;DR: 本文探讨神经符号智能体AI（NSAAI）作为网络化低空无人机自主决策框架，结合神经基础、符号推理与闭环智能体交互，以提升不确定环境下的可靠性和自适应能力，并通过城市火灾巡检仿真案例验证其潜力。


<details>
  <summary>Details</summary>
Motivation: 网络化低空无人机需要在不确定观测、动态环境和间歇连接下进行可靠、自适应的决策，而现有智能体系统常受幻觉风险、数据依赖和泛化能力弱的限制。

Method: 研究NSAAI在数据效率、组合泛化、持续学习和零样本迁移方面的能力基础，提出参考架构，涵盖任务与目标管理、神经符号规划、验证与元认知、技能执行与网络交互、共享知识与记忆，并在LAESim中实现城市火灾巡检案例。

Result: 案例表明无人机可在间歇连接下协调感知与云访问，复用已验证的图像交付技能，并在满足明确证据条件后完成任务，展示了NSAAI支持可复用技能、证据驱动决策和自适应任务执行的潜力。

Conclusion: NSAAI有望提升网络化无人机系统的自主性，未来研究方向包括不确定性感知推理、知识与技能扩展、自适应自监控以及标准化评估。
...
### [19] [Self Improvement via Fast Tree-search](https://arxiv.org/abs/2609.19526)
*Xinghong Fu,Aravinth Kulanthaivelu,Yutaro Yamada*

Main category: cs.AI

TL;DR: 提出 SIFT：一种在严格预算下提升编码智能体自我改进效率的框架，通过快速树搜索和 LLM 评判信号减少昂贵下游评估。


<details>
  <summary>Details</summary>
Motivation: 编码智能体可通过递归修改自身实现自我提升，但现有方法计算成本高、资源密集，主要瓶颈在于反复运行基准任务来评估候选自我修改的效果。

Method: SIFT 引入 LLM-as-a-judge 对候选补丁进行成对比较，用正则化 Bradley-Terry 模型聚合胜负记录得到强度分数，并驱动轻量级解耦树搜索中的基于排名的父节点采样；仅对最有希望的节点进行昂贵的下游任务评估。

Result: 在完整 Polyglot 基准上，SIFT 优于现有基于树搜索的自进化框架，同时在 CPU 小时、墙钟时间和 API 成本方面显著降低资源需求。

Conclusion: 通过将快速评判信号与稀疏昂贵评估结合，SIFT 实现了在预算约束下更样本高效的编码智能体递归自我改进。
...
### [20] [When Hiring Becomes Agent-Mediated: Evaluating Access and Recurrence in Two-Agent Résumé Screening](https://arxiv.org/abs/2609.19530)
*Jian Gao,Hang Jiang*

Main category: cs.AI

TL;DR: 论文研究双边招聘中的简历筛选，提出由雇主侧和候选人侧两个智能体交换证据、更新判断后再决定是否晋级的两智能体筛查流程，并与传统一次性静态判断进行比较。


<details>
  <summary>Details</summary>
Motivation: 招聘本质上是双边的：雇主评估匹配度，候选人也会呈现和辩护自身资格。但作为第一道关卡的简历筛选，通常被自动化成对简历-岗位配对的静态、一次性判断，忽略了双方的互动与证据交换。

Method: 使用GPT-5.5和Claude Opus 4.7，在600个构造的简历-岗位配对上比较一次性筛查与两智能体筛查。两智能体中，雇主侧和候选人侧代理分别代表双方角色，交换证据并更新判断，然后决定谁进入下一轮。另有191个边缘样本池进行三次运行，并做发现案例重跑和一次性后续对照。

Result: 两智能体筛查提高了通过率：GPT-5.5从33.3%升至39.3%，Opus 4.7从34.0%升至35.5%。在191个边缘样本池三次运行中，pass-instance rate分别从4.5%升至26.2%，从6.5%升至16.1%。但并非统一放宽：两智能体也会拒绝一次性筛查通过的申请，决策双向变化。在相似通过量下，两种流程通过的申请不同，且没有单一一次性阈值能稳定恢复两智能体一致选中的申请。重跑发现，两智能体独有选中的复现率低于共享选中，GPT-5.5下明显，Opus 4.7下较不确定，而一次性后续对照无类似下降。

Conclusion: 当招聘双方都变得智能体化时，影响谁进入人工审核以及这种机会能否可靠复现的，不仅是背后的模型，筛选程序本身也起到关键作用。
...
TL;DR: 论文研究企业系统中智能体如何通过交互持续发现并维护业务规则构成的世界模型，并提出 EnterpriseWorldShift 基准与 Continual Discovery Agent（CDA）。CDA 能在规则变化的世界版本间持续更新模型，比逐题查询更准确地预测隐藏规则效果，最高提升 8.98 IoU 点。


<details>
  <summary>Details</summary>
Motivation: 企业系统中的字段更新可能触发其他字段变化、创建记录或启动审批，这些效果由各组织自行编写且不断修订的业务规则产生。智能体若不掌握这些规则，就无法预测自身动作的后果，因此需要持续发现并适应企业世界模型。

Method: 提出持续企业世界模型发现任务，并构建 EnterpriseWorldShift：基于实时 ServiceNow 环境，包含 9 张表、25 条隐藏规则和 600 个评估动作；通过四个同一企业世界版本依次修改、添加、移除规则，分别测试发现、修订、扩展和淘汰能力。同时提出 Continual Discovery Agent（CDA），通过交互观察结果构建并跨世界携带世界模型。

Result: CDA 对隐藏规则效果的预测比先前逐题查询的方法更准确，最高提升 8.98 IoU 点，并且能够仅基于自身模型作答，无需查询运行中的系统。

Conclusion: 持续发现和修订企业世界模型有助于智能体在业务规则动态变化的企业系统中更准确预测动作效果，并减少对实时系统的依赖。
...
### [22] [Agentic AI Networking for Heterogeneous Unmanned Aerial Systems in Low-Altitude Wireless Networks](https://arxiv.org/abs/2609.19538)
*Nguyen Duc Minh Quang,Chang Liu,Shuangyang Li,Derrick Wing Kwan Ng*

Main category: cs.AI

TL;DR: 本文提出一种分层混合LLM-MARL架构，采用双环结构应对低空无线网络中异构服务的动态需求，外环用LLM进行博弈编排，内环执行参数化MARL策略。


<details>
  <summary>Details</summary>
Motivation: 低空无线网络中异构无人系统共存导致移动性、连接性和资源共享强耦合，且异构服务需求时变，传统优化和学习方法依赖预定义目标，难以自主适应变化。

Method: 提出分层混合大语言模型与多智能体强化学习（LLM-MARL）架构，双环结构：外环使用LLM辅助博弈编排，解读服务需求和操作者意图，重新配置目标和资源优先级；内环在配置的博弈下执行去中心化、参数条件化的MARL策略。

Result: 物流监控案例研究表明，该框架能促进异构服务的协调共存，适应不断变化的运行条件，且无需重新训练底层MARL策略。

Conclusion: 讨论了面向可扩展、可信和自适应智能体低空无线网络的关键挑战和研究方向。
...
### [23] [SIMLIFE: Pattern Understanding for Long-Horizon Human-Agent Partnership](https://arxiv.org/abs/2609.19610)
*Run Peng,Zinnia Nie,Jing Ding,Yinpei Dai,Yichi Zhang,Zengqing Wu,Yao Fu,Ziqiao Ma,Jiayuan Mao,Joyce Chai*

Main category: cs.AI

TL;DR: 论文提出 SimLife 长期家庭生活模拟平台及 SimLife-BP 基准，用于评估智能体从数周/数月日常观察中推断潜在行为规则的长上下文模式理解能力，发现现有模型仍存在明显瓶颈。


<details>
  <summary>Details</summary>
Motivation: 理解人类长期行为不仅需要推断当下需求，还需理解习惯如何形成、为何重复以及何时改变；现有具身智能体缺乏对长时程日常行为规则理解的系统评测。

Method: 构建 SimLife 可扩展模拟平台，提供丰富视觉观察、真实动作日志、带音频的合成对话；基于此提出 SimLife-BP，包含106个片段，平均15.49小时、38.57游戏日，1439个问答对，覆盖直接、反事实、噪声和逆向推理，并在不同规则提示水平下评估前沿模型与架构。

Result: 当前模型常仅实现表面预测而缺乏全面规则理解，依赖频率启发式而非基于证据的 if-then 推理，并且在行为模式变化时难以适应。

Conclusion: 长上下文模式理解仍是未来具身智能体的主要瓶颈；SimLife 为研究记忆、个性化、适应和长时程人机交互规划提供了更广泛的空间。
...
TL;DR: 该论文研究车载语音助手中大语言模型在执行车辆功能前的安全授权决策问题，提出包含202个场景和七类参考决策的基准，并评估多个模型。结果显示即使最佳API模型也存在错误执行，结构化LLM决策不足以作为独立安全机制。


<details>
  <summary>Details</summary>
Motivation: LLM日益集成到车载语音助手中，但将自然语言请求映射到车辆功能会带来安全关键的授权问题。系统必须在执行、拒绝、澄清、要求确认、转手动控制、触发紧急响应或不调用工具之间做出正确选择，而此前评估未充分隔离这一行动前决策在不同说话人角色、认证状态、车辆状态和工具可用性下的表现。

Method: 提出一个包含202个场景的基准，定义七类决策分类体系及参考决策。评估两个本地开放权重模型和三个基于API的LLM，使用决策对齐度和安全特定错误指标。还对Llama 3.2 3B进行受控消融，比较结构化授权策略、仅模式基线和通用安全基线。

Result: 决策对齐度从Llama 3.2 3B的40.1%到Gemini 3.1 Pro Preview的89.1%。API模型得分在83.2%至89.1%之间，彼此无统计显著差异。即使这些模型，在161个非执行场景中仍产生2至3次错误执行，并在确认和手动控制决策上持续出错。Llama 3.2 3B消融中，结构化授权策略将对齐度提高到40.1%，高于仅模式基线和通用安全基线的28.2%至29.2%，但未能消除错误执行。

Conclusion: 结构化的LLM决策不足以作为独立安全机制。部署时需要独立的执行层，在调用任何车辆功能前验证工具权限和车辆状态约束。
...
TL;DR: 本文对旅行计划在中断后修复问题进行了系统性实证研究，比较了LLM-Z3完全重规划、IPyHOPPER分层修复和iTIMO局部修订三种方法。


<details>
  <summary>Details</summary>
Motivation: 旅行计划被接受后可能因航班取消、酒店不可用或景点关闭而不可行，而完全重规划、经典计划修复和LLM旅行智能体修订因任务定义与评估协议不同而难以比较。

Method: 使用两个基于TREK的基准集：500个单中断案例（含可行与不可行实例）和200个可行的同时复合中断案例；从有效性、计划稳定性和计算成本三个维度比较LLM-Z3完全重规划、IPyHOPPER分层修复和iTIMO局部修订适配器。

Result: LLM-Z3（Gemini）在复合中断成功率上最高；IPyHOPPER在单中断总体成功率上接近该配置，且在成功修复时保留更多已接受行程。成功的分层与局部修复比完全重规划编辑更少、保留更多已接受承诺。计算特征不同：IPyHOPPER无需LLM推理，LLM-Z3适配器使用紧凑的单次调用推理，iTIMO适配器消耗显著更多token。

Conclusion: 该研究为在所评估设置中平衡可行性恢复、承诺保留与计算成本提供了实践指导。
...
### [26] [ScientistTwo: Pioneering the Human Knowledge Frontier with Autonomous AI](https://arxiv.org/abs/2609.19644)
*Jaehyun Nam,Jinsung Yoon,Yanzhou Pan,Yubo Wang,Rui Meng,Parthasarathy Ranganathan,Tomas Pfister*

Main category: cs.AI

TL;DR: 提出 ScientistTwo 全自主多智能体框架，能从初始问题出发完成假设提出、实验、消融、同行评审式验证，并生成可发表论文与可执行代码。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 多为辅助工具，终极愿景是让 AI 在人类给出基础挑战后，自主识别知识边界与瓶颈，并系统推进科学发现。

Method: ScientistTwo 以初始问题为输入，建立 SOTA 基线，提出新假设，协调专用智能体完成端到端发现循环；使用多数据集与指标做实验，通过自动消融优化方法，并借助闭环模拟同行评审反驳引擎验证发现。

Result: 在 ICLR、ICML、NeurIPS 等顶会论文基准上，ScientistTwo 自主生成专家级可发表论文和完整可执行代码库，解决方案持续超越人类 SOTA 模型，并在自动 AI 评审代理下获得高于人类论文的平均评审评分。

Conclusion: ScientistTwo 不仅是辅助工具，更是能够推动人类发现边界的自主科学先驱。
...
TL;DR: 论文指出长程语言模型智能体的端点成功率混淆了“到达关键状态”和“从该状态完成”两种能力；提出checkpoint handoff评估协议，将RL相对SFT的增益拆分为REACH与SOLVE，并在多个基准上验证。


<details>
  <summary>Details</summary>
Motivation: RL训练长程语言模型智能体收益很大，常被解读为决策更好。但在闭环中，智能体后期遇到的状态部分由自身行为造成，导致SFT与RL checkpoint即便任务相同也从不同状态开始评估；端点成功混合了到达哪里与到那里后做什么。限制到共同可达状态会按结果选择，甚至翻转效应。

Method: 提出checkpoint handoff评估协议：克隆一个已发布checkpoint到达的状态，交给另一个checkpoint继续执行，无需重训练。通过交叉reacher与solver角色以及SFT与RL checkpoint，将端点增益拆为REACH和SOLVE。REACH衡量策略到达环境确认距成功固定步数状态的频率；SOLVE衡量从相同克隆状态完成的频率。

Result: 在两个基准和两个独立发布流程中，reacher×solver交互在全部五个条件下为正。RL历史对RL solver的价值高于同样历史对SFT solver的价值。在ALFWorld上，RL同时提升REACH和SOLVE，且SFT solver从未在RL solver失败处成功。独立的REACH与SOLVE差距可预测总体交互。

Conclusion: Handoff只要求一个checkpoint的历史能在另一个checkpoint下重放，因此长程评估可在端点成功率之外同时报告到达与完成情况，从而更准确地区分RL带来的决策改进。
...
### [28] [When2Think: Learning Difficulty-Aware Length Control for Efficient Hybrid Reasoning Models](https://arxiv.org/abs/2609.19671)
*Jaejun Shim,HyunJin Kim,Young Jin Kim,JinYeong Bak*

Main category: cs.AI

TL;DR: 论文提出 When2Think，将高效推理建模为实例自适应计算分配问题，通过难度感知奖励塑造动态决定简单题直接作答、难题深入推理，从而在数学基准上提升准确率并减少 token 使用。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型常对简单问题过度思考、对困难问题思考不足；现有统一长度惩罚或固定路由方法会带来效率税，即在简单题上省算力却损害难题准确率。因此需要实例级的自适应计算分配。

Method: 提出后训练框架 When2Think，核心为 Instance-level Difficulty-Aware Control（IDAC）：利用预计算参考统计（准确率与 token 使用）进行奖励塑形，调节推理深度；结合验证器奖励与批次标准化优势，实现稳定的无 critic 优化，无需学习奖励模型或在线参考模型查询。

Result: 数学基准实验显示更优的准确率-效率权衡：AIME24 上 Pass@3 相对基模型提升 10.0%，token 使用减少 27.9%；AIME25 上 Pass@3 达 40.0%，优于压缩和仅路由基线。

Conclusion: When2Think 能根据问题难度动态分配计算，在简单题上直接回答、难题上保留扩展推理，学习 NoThink 与 Think 的混合使用，实现更好的准确率-效率权衡。
...
### [29] [FINSKILLOPS: A Self-Evolving Multi-Agent System for SEC Filing QA](https://arxiv.org/abs/2609.19680)
...
TL;DR: 论文提出将金融问答系统的部署后改进视为受控的行为维护，并构建多智能体系统 FINSKILLOPS，通过技能补丁、验证、回归检查和生命周期管理实现可靠自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有金融问答系统通常在部署前通过检索、提示或智能体协调优化，部署后可靠性行为固定；面对新 SEC 文件问题时，会出现周期、实体、证据使用和计算等异质错误。已有自我改进方法虽能将失败转化为新行为，但难以控制修正范围，且可能破坏原本正确的答案。

Method: 提出“受控行为维护”框架：将重复失败转化为有范围的技能补丁，每个补丁需通过部署验证且不引入回归。实例化为 FINSKILLOPS 多智能体系统，从基于证据、分类型的失败诊断中提取可复用技能，并通过定向验证、保护案例回归检查、负对照以及版本化替换或退役进行治理。

Result: 在六个金融问答基准上，单一冻结技能注册表取得最高 verdict-weighted correctness 和 reference consistency。进化技能将增强基准上的正确性从 3.70 提升至 4.55。在 12 轮运行研究中，33 个候选技能仅 6 个被提升，监控非正确率从 20.0% 降至 12.5%。
...
### [30] [LearnActCoder: Role-Aware Error Memory for Adaptive Clinical Coding Agents](https://arxiv.org/abs/2609.19721)
*Meysam Ghaffari,Bhaskar Sen,Nasim Sabetpour,Nina Fatehi,Animesh Agarwal,Carlos Morato*

Main category: cs.AI

TL;DR: 论文提出Learn-Then-Act推理时自适应框架，将小规模标注LEARN批次中的错误转化为结构化MistakeKDB，并实例化为LearnActCoder临床编码流水线，以在不更新模型权重的情况下改善编码行为。


<details>
  <summary>Details</summary>
Motivation: 临床编码智能体反复出现相同失败模式，如不支持编码、遗漏已记录病症、特异性错误以及手术编码惯例不匹配，因此需要一种能利用错误经验进行推理时适应的方法。

Method: 提出Learn-Then-Act框架，把错误转为结构化Mistake Knowledge Database；将假阴性教训路由给偏向召回率的Coder，将假阳性教训路由给偏向精确率的Judge；实例化为LearnActCoder，采用Coder-Judge临床编码流水线，并在可用时使用查找表进行 grounding。

Result: 在150条匹配的MIMIC-III病历上，结构化MistakeKDB使CPT F1提高5.9个百分点，而原始示例和反思式记忆接近无记忆基线，ICD-9提升不显著；在匹配的MIMIC-IV队列上，记忆使ICD-10编码偏向更高精确率但牺牲召回率，F1统计上不变；在1000条留出MIMIC-III病历上，ICD工作点保持稳定，提供规模与稳定性证据。

Conclusion: 结果表明，结构化的、由反馈衍生的错误记忆有助于跨病例适应临床编码行为，且无需权重更新或改变底层工作流；但CPT/HCPCS绝对性能仍较低，且系统为回顾性评估，未在临床部署中验证。
...
### [31] [AutoData: Agentic Search for Pre-training Data Selection](https://arxiv.org/abs/2609.19754)
*Yan Meng,Dhruv Srikanth,Bingchen Zhao,Zhengyao Jiang,Yuxiang Wu*

Main category: cs.AI

TL;DR: 论文提出 AutoData，将预训练数据选择视为可执行算法的搜索问题，让 LLM 智能体在验证反馈下自动发现优于人工设计的数据筛选流程。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 智能体已能自动优化模型和训练代码，但数据选择仍主要依赖人工启发式流程，未充分纳入智能体的自动化优化闭环。

Method: 将预训练数据选择形式化为对文档级特征（词法统计、类别标签、困惑度等）的启发式工程；AutoData 直接搜索可执行的筛选算法，包括评分、分层和随机选择规则，并利用代理模型验证反馈迭代优化算法。

Result: 在一次通宵搜索中，AutoData 发现的筛选算法优于现有的人工设计数据整理流程；尽管仅在小代理模型上搜索，该配方仍可迁移到更大规模，并提升下游指标 CORE。

Conclusion: 数据工程可以被视为一种智能体机器学习问题，从而将自主研究从模型和训练代码优化扩展到数据优化。
...
### [32] [Rethinking Multi-Agent Collaboration: When More Is Less](https://arxiv.org/abs/2609.19759)
*Yishuo Yuan,Yibo Wu,Yihan Zhang,Minyuan Sun,Shenliang Li,Xinkai Ma,Yifan Li,Jiaheng Liu*

Main category: cs.AI

TL;DR: 论文系统分析多智能体协作相对单智能体的能力边界，提出SAIGE轻量级协作机制，在长时程稀疏依赖任务中有效，但更多智能体不一定更智能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型与单智能体系统快速发展，随着单智能体能力提升，多智能体协作收益递减且上下文开销增大，需明确何时多智能体协作真正有价值。

Method: 系统分析多智能体与单智能体的能力边界；提出SAIGE（语义感知增量图演化）机制，将协作建模为动态演化图：节点为按需生成的智能体实例，边表示基于内容检索建立的语义依赖。

Result: 在长时程复杂任务基准上，SAIGE在上下文效率与任务性能间取得良好权衡；扩大智能体池或加深递归层级并不持续提升效果。

Conclusion: 多智能体的优势受任务结构限制而非普适，更多智能体不一定使系统更智能。
...
### [33] [TorchCraft: Unified binder design by inverting an all-atom structure predictor](https://arxiv.org/abs/2609.19770)
...
TL;DR: 本文提出 TorchCraft，一个统一的结合体设计框架，通过冻结的全原子结构预测器优化序列 logits，并在 TorchFold 中实现。


<details>
  <summary>Details</summary>
Motivation: 全原子结构预测器已能建模多样分子相互作用，但利用其学习到的结构先验进行结合体设计仍具挑战。

Method: TorchCraft 在共享优化流程中结合置信度、接触、几何和序列先验目标，支持 minibinders、框架条件 VHH、环肽和配体结合蛋白设计；使用预训练 AlphaFold 3 权重。

Result: TorchCraft 生成了代表性 minibinders 和 VHHs，并在各四种靶点上实验测得结合，无需事后序列重设计；计算基准还展示了其对环肽和配体条件口袋设计的适用性。

Conclusion: TorchCraft 将预测器反演扩展到多种结合体格式和分子场景，为在设计中共用全原子结构先验提供了通用框架。
...
### [34] [Integrating knowledge from case reports: a medical ontology based multimodal information system with structured summary](https://arxiv.org/abs/2609.19775)
*Shuyu Guo,Lan Huang,Yichen Liu,Hanbin Ma,Tian Bai*

Main category: cs.AI

TL;DR: 该研究构建了一个面向医学病例报告的综合多模态信息系统，整合了2000至2021年间52949篇开放获取病例报告中的结构化临床摘要、医学图像和生物医学命名实体，并提供高效的搜索与浏览界面。


<details>
  <summary>Details</summary>
Motivation: 医学病例报告是重要的医学信息载体，记录了罕见病、诊断方法和创新治疗等发现，但PubMed中数百万病例报告因非结构化和多样性，传统关键词检索工具难以高效获取相关信息。

Method: 引入综合多模态病例报告信息系统，整合结构化患者临床摘要、医学图像和生物医学命名实体，数据来自52949篇2000至2021年开放获取病例报告；将多模态核心信息组织在结构化医学本体中，并设计强大的搜索和浏览界面。

Result: 构建了覆盖52949篇开放获取病例报告的多模态信息系统，实现了结构化临床摘要、医学图像和生物医学命名实体的整合与本体化组织，并提供了有效的检索与浏览接口。

Conclusion: 该系统可帮助初级临床医生有效检索病例，提高罕见病的识别和诊断能力。
...
TL;DR: 本文研究基于大语言模型的多智能体交易系统对抗性输入攻击的鲁棒性，提出GMATS框架及黑盒投毒攻击方法，发现仅通过社交媒体信息即可显著降低风险收益表现，但合理的多智能体拓扑与协调者提示可缓解冲击。


<details>
  <summary>Details</summary>
Motivation: LLM多智能体交易系统在量化金融中逐渐出现，但其面对对抗性输入的鲁棒性尚不清楚，尤其是仅通过可接受的社交媒体信息流进入系统的黑盒攻击。

Method: 提出通用多智能体交易系统框架GMATS，构建黑盒投毒攻击者：将LLM作为帖子生成器，在预算约束下向分析师证据流注入看似良性的社交媒体内容；定义传播指标，包括分析师与协调者层的信念偏移分数，以及标准回测指标上的攻击-干净增量；在安全离线基准上使用历史市场与社交数据进行实验。

Result: 实验表明，即使简单的仅输入攻击者也能实质性恶化风险收益特征，显著降低夏普比率；同时发现设计良好的多智能体拓扑和协调者提示可以在相同投毒预算下抑制对抗冲击并提升平均鲁棒性。

Conclusion: LLM多智能体交易系统对社交媒体输入投毒具有脆弱性，但通过优化多智能体架构与协调提示可增强鲁棒性，为安全部署提供方向。
...
TL;DR: 本文研究正则化自博弈能否有意选择价值等价均衡中的目标均衡。方法是将参考策略锚定到目标均衡成员并继续精炼，在多个可解博弈和二维多面体上验证；结果表明自博弈可被引导至目标成员，但也存在边界、离流形参考和刚性/平坦族等失效条件。


<details>
  <summary>Details</summary>
Motivation: DeepNash 等正则化自博弈中，熵正则项会在价值等价均衡构成多面体时隐式打破平局，例如均匀参考会选择最大熵均衡。作者追问：参考策略能否被主动用作均衡选择旋钮，而不仅是稳定项。

Method: 在五个精确可解博弈加一个二维多面体上，使用精确最佳响应和跨独立种子的等价性检验；将参考策略锚定到目标均衡成员并精炼，测试选择是否跟随参考而非初始化。还比较表格与 MLP 引导图，并设置匹配控制组，分析注意力机制的种子方差与系统偏移。

Result: 锚定参考并精炼可将自博弈引导到目标成员：平均坐标误差 0.007，中位可利用度 5×10^-5，TOST 等价于请求值 ±0.05；选择遵循可达加权 I-投影（斜率 0.969 [0.950, 0.987]）。失效条件包括：固定离流形参考带来 0.08–0.25 可利用度代价；刚性和平坦族需更小镜像步；边界目标会欠调；曲率可预测边界饱和（秩相关 0.90，p=0.037），内部精度与曲率无关。表格和 MLP 引导图在各目标上等价（±0.03，30 种子）；注意力稳健特征主要是种子方差过大，系统偏移上限 0.018 且不显著。对抗最佳响应时选择-稳健性权衡退化，引导仅在面对固定非均衡对手时重要。

Conclusion: 锚定参考到期望均衡成员并精炼，可将 RLHF 式 RL 中的 KL 锚重新解释为均衡选择旋钮，而不仅是稳定性约束；同时该选择方法有明确适用边界，需关注参考离流形、目标在边界、族刚性以及对手类型等条件。
...
### [37] [Dual-Axis Policy Optimization for LLM Agents: Bayesian Feedback Attribution and Trajectory Mass Normalization](https://arxiv.org/abs/2609.19830)
*Yingxuan Zhuang,Binhe Yu,Jingxiao Yang,Ruopei Sun,Ziting Li,Cheng Tan,Xuhong Zhang,Jianwei Yin,Jintao Chen*

Main category: cs.AI

TL;DR: 本文提出 BATON，一种面向 LLM 智能体强化学习的双轴策略优化框架，分别处理轨迹内反馈归因和轨迹间目标聚合，并在多个任务上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: LLM 智能体强化学习存在两个关键但常被忽视的优化维度：轨迹内如何利用环境反馈，以及批次中如何聚合完整轨迹；现有方法对二者处理不足。

Method: 将问题形式化为 Intra-Trajectory Feedback Attribution 和 Inter-Trajectory Objective Aggregation，并提出 BATON 框架。其第一轴为 Bayesian Feedback Attribution，构建基于反馈条件的动作后验；第二轴为 Trajectory Mass Normalization（TMN），为完整轨迹分配同等优化质量。

Result: 在 ALFWorld、WebShop 和 SearchQA 上结合 GRPO 与 GiGPO 实验表明，两个轴均带来独立增益，且二者结合在不同模型规模下持续取得最佳整体性能。

Conclusion: 同时优化轨迹内反馈归因与轨迹间目标聚合能够提升 LLM 智能体强化学习效果，BATON 为这类双维度优化提供了有效框架。
...
### [38] [MetaRTL: Meta-path Attention Enhanced Relational Table Learning](https://arxiv.org/abs/2609.19832)
*Ken Zhong,Weichen Li,Zheng Wang*

Main category: cs.AI

TL;DR: 提出MetaRTL，一个两阶段关系表学习框架，通过轻量预训练和非参数消息传递+MetaAttn聚合元路径特征，实现高效可扩展的关系表学习。
...
Method: 两阶段框架：第一阶段轻量预训练获得初始表嵌入；第二阶段进行非参数消息传递提取元路径特征，并用注意力模块MetaAttn聚合；将计算从深层消息传递转移到高效元路径聚合。

Result: 在10个真实数据集、24个任务上的实验验证了方法的有效性。

Conclusion: MetaRTL能在保持高效率的同时捕获丰富关系语义，适用于大规模关系表学习。
...
### [39] [A Dual-Process Perspective on Nudge Susceptibility in LLM-Based GUI Agents](https://arxiv.org/abs/2609.19843)
*Haya Halimeh,Sascha Kaltenpoth,Kevin Bösch,Oliver Müller*

Main category: cs.AI

TL;DR: 该研究基于双过程理论，通过在线购物实验考察LLM图形界面代理是否易受数字助推影响，以及推理配置如何调节这种易感性。


<details>
  <summary>Details</summary>
Motivation: LLM图形界面代理越来越多地代表用户在为人设计的数字环境中行动，而这些界面会引导用户行为与决策。现有研究多关注LLM文本输出中的行为偏差，但对代理感知界面并执行决策时如何受界面影响、推理能力是否增强其鲁棒性知之甚少。

Method: 采用随机在线购物实验，覆盖3家提供商的6个前沿模型，共3,600个代理、21,600次模拟；基于双过程理论，检验代理对自动型（Type 1）与反思型（Type 2）数字助推的易感性，并分析推理配置的调节作用。

Result: 代理对两类助推均表现出脆弱性。推理配置产生方向相反的调节作用：降低对自动默认助推的易感性，却提高对反思型社会影响助推的易感性。广泛推理并未使代理更鲁棒，而是改变了选择架构起效的路径。探索性分析显示，这种路径转移受模型规模系统性影响。

Conclusion: 研究将助推易感性确立为代理式AI的一种行为属性，并表明界面设计应成为组织向自主代理委托决策时的治理关切。
...
### [40] [Constraint-Safe Graph-Context Scoring for Stable Point-Feature Labels Under Text-Width and Accessibility-Inspired Profiles](https://arxiv.org/abs/2609.19848)
*Taimoor Ahmad*

Main category: cs.AI

TL;DR: 论文提出可复现原型 LABELSENSE-Pilot，用于交互地图点要素标签放置：每要素生成8个方向候选，用MLP对图上下文摘要评分，加入上一布局奖励，并通过混合整数优化选取布局。


<details>
  <summary>Details</summary>
Motivation: 交互地图标签放置需同时兼顾几何有效性、显示率、局部放置效用和相机运动稳定性；可访问性与多语言需求会改变标签尺寸，但已有评估常只关注重叠数量。

Method: 方法包括：每要素生成8个罗盘候选；用多层感知机在图上下文摘要上评分；加入历史放置奖励；通过混合整数优化选择最终布局；检查视口包含、每要素唯一性和成对间距。实验使用155国2500个机场坐标与名称，按国家分组划分，并加入密度、相机、文本后缀、偏好和放大字体压力测试，对比手工效用整数规划，运行5个随机种子。

Result: 5个种子下，LABELSENSE-Pilot显示85.62%标签，闪烁2.09%，零碰撞；相比手工效用整数规划，显示率低1.43个百分点，但闪烁减少12.04个百分点。放大框感知布局零代理违规，而标准几何在1.5倍下重新评估时52.57%的选中放置违规。

Conclusion: 结果建立了可审计的工程权衡，但不代表人类可访问性、多语言可用性或偏好；正式提交前仍需官方近期基线和参与者证据。
...
### [41] [Reproducibility is not construct validity: LLM measurement of institutionally situated communication](https://arxiv.org/abs/2609.19866)
*Veronika Batzdorfer,Carlo Romano Marcello Alessandro Santagiustina*

Main category: cs.AI

TL;DR: 该研究检验了LLM标注的高可重复性是否等同于构念效度。基于欧盟委员会AI法案咨询中同一利益相关者的调查问卷与自由文本提交数据，发现LLM标注虽高度可重复，但与调查测得的同一构念收敛有限，且分歧随利益相关者群体和欧洲国家空间分布呈现系统性差异。


<details>
  <summary>Details</summary>
Motivation: 高标注可重复性常被误认为LLM推断测量有效，但可重复性并不必然意味着其捕捉了目标构念。作者希望区分可重复性、构念效度和沟通语境差异，检验LLM作为测量工具时的效度问题。

Method: 使用欧盟委员会AI法案咨询数据，将同一利益相关者的结构化调查回答与自由文本咨询提交相链接；用LLM标注自由文本，并与调查中名义上对应的构念测量进行比较；分析不同利益相关者群体的分歧，并检验欧洲国家间分歧的空间自相关；同时考察调查所报告的关切与对可解释性支持之间的关系。

Result: LLM标注咨询文本具有极高可重复性（组内相关系数>0.99），但与调查测量的目标构念收敛有限。分歧在不同利益相关者群体中系统变化：商业协会在文本咨询中比调查中表达更高AI风险关切（g=+1.0），公共当局和若干非商业团体分歧较小或为负。分歧得分在欧洲国家间显示正空间自相关（Moran's I=0.347，p=0.036），邻国利益相关者对AI安全关切的文本立场更相似。尽管存在分歧，调查报告的关切在所有分歧水平上仍与支持可解释性强烈相关。

Conclusion: LLM标注可重复性可能与较差的构念对应并存。将LLM用作测量工具时，需要采用能够区分可重复性、构念效度和沟通语境变异的验证程序。
...
### [42] [Physical knowledge on historical data matters more than enforcing physical constraints on the forecast](https://arxiv.org/abs/2609.19871)
*Etienne Lehembre,Pascal Audigane,Vincent Nguyen,Christel Vrain,Thi-Bich-Hanh Dao*

Main category: cs.AI

TL;DR: 本文提出一种物理信息循环神经网络（PIRNN），在预测目标时间序列的同时估计不可观测的中间物理变量，并在地下水水位预测案例中验证其效果。


<details>
  <summary>Details</summary>
Motivation: 物理过程相关的时间序列预测仍具挑战；现有物理信息神经网络（PINN）类模型通常不估计对领域专家理解目标行为重要的不可观测中间物理变量。

Method: 提出PIRNN，结合物理方程与循环神经网络，预测目标及历史数据和预测目标中的不可观测变量；方法可适配包含多个方程及各自不可观测变量的物理模型。案例研究采用地下水水位模型Gardenia，基于水库间传输方程和数据同化。在12个真实数据集上与多种神经网络模型及Gardenia比较，并进行消融研究。

Result: PIRNN在12个数据集中有5个优于其他模型；消融研究强调物理背景对时间序列预测任务的重要性；领域专家评估认为神经网络预测的物理变量具有一致性。

Conclusion: PIRNN利用领域知识增强了模型鲁棒性和结果可解释性，表明引入物理背景有助于物理过程时间序列预测。
...
### [43] [TRACE: Accountable Agentic Retrieval for Source Discovery in Digital Archives](https://arxiv.org/abs/2609.19897)
*Donghan Bian,Marie Puren,Florian Cafiero*

Main category: cs.AI

TL;DR: 论文提出 TRACE，一个面向历史档案的免训练智能体检索框架，用于可追溯的来源发现，并在法国第三共和国历史问答基准上验证其检索效果与成本可行性。


<details>
  <summary>Details</summary>
Motivation: 历史档案给检索增强生成带来困难：OCR 退化、跨体裁与来源异构，并且学术和机构使用需要强来源可追溯性。

Method: TRACE 是免训练的智能体检索框架，强调可问责的来源发现和语料感知的智能体设计；在 DECIDON 项目背景下开发，评估于 HistoriQA-ThirdRepublic 基准，该基准包含 1,752 个关于 1887 年议会辩论和报纸的法国历史问题，文档来自法国国家图书馆数字化馆藏。

Result: TRACE 达到 R@10 = 0.856、MRR = 0.653，优于稀疏、稠密、基于图和智能体 RAG 基线，在多跳和跨语料问题上提升最大；默认托管推理配置下约每问题 0.02 美元。

Conclusion: 对于大型数字图书馆和档案馆，检索可问责性与语料感知的智能体设计，可作为比训练型或图构建型更重方法更实用的替代方案。
...
### [44] [From "Who Is This User?" to "What Does This Purchase Mean?": A Deployed Pipeline for Semantic User Profiling at Bank Scale](https://arxiv.org/abs/2609.19928)
*Ryota Mitsuhashi,Tetsuro Morimura,Hirotake Ito*

Main category: cs.AI

TL;DR: 该论文提出将用户属性推断从“按用户”转为“按交易模式”进行，通过 Resolve、Profile、Tag 三阶段流水线构建可查询数据库，在公开电商语料上效果与逐用户 LLM 推断相当，并在日本大型银行部署，显著降低 LLM 推理成本。


<details>
  <summary>Details</summary>
Motivation: 按用户交易历史逐用户调用 LLM 会使推理预算随用户数线性增长，在实际应用规模下成本过高，因此需要降低推理目标数量。

Method: 提出三阶段流水线：Resolve 解析物品名称并可选用网页 grounding；Profile 对每个频繁模式调用一次 LLM，输出预定义类别标签、自由文本属性和属性流行度估计；Tag 将自由文本属性聚类成可查询数据库。推理对象从用户变为交易模式，预算随模式数而非用户数增长。

Result: 在公开 Open e-commerce 语料上，该数据库在评估属性的 AUC 上与直接读取每个用户原始历史的 LLM 统计上不可区分，且流行度估计对正负用户具有区分信号。该流水线已部署于一家日本大型银行，覆盖数千万用户，相比逐用户流水线将 LLM 推理目标减少近三个数量级。

Conclusion: 按交易模式进行属性推断可在保持效果的同时大幅降低 LLM 推理成本，适合大规模实际部署；代码已在 GitHub 公开。
...
TL;DR: 本文提出多智能体框架 MaSCoD，在直接边判断前先组织候选第三变量和局部结构模式，以控制因果发现中潜在因果关系的过早遗漏。在 Auto-MPG、DWD 和 Sachs 数据集上使用 GPT-5.4 和 GPT-4o 评估，发现其保留-选择性表现依赖于数据集和骨干模型，并非一致优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的因果发现方法在生成候选图时，很少将“过早遗漏潜在相关因果关系”作为明确的设计目标，导致可能丢失真实因果边。

Method: 提出 MaSCoD 多智能体框架，先组织候选第三变量与局部结构模式，再进行直接边判断。在 Auto-MPG、DWD、Sachs 三个数据集上，以 GPT-5.4 为主干、GPT-4o 为复现，对比 Full（判断前提供结构假设）与 No Phase 1（在判断过程中构建结构假设）等设置，并进行部分消融和阶段分析。

Result: 在所有六种数据集-主干组合中，Full 比 No Phase 1 获得更高的平均召回率和 F1，但假阳性率也增加。仅在 DWD+GPT-5.4 和 Sachs+GPT-4o 上，Full 比所有基线保留更多参考边，并非普遍成立。部分消融显示同时提供两种信息组件并不总是优于只提供一种。阶段分析表明，GPT-5.4 上 Full 与 No Phase 1 的保留差距在直接边判断后已存在，而协调步骤额外导致 Full 在 Sachs 上损失参考边。

Conclusion: 结构预组织应作为遗漏控制的明确设计与评估目标，并且需要将上下文构建与其在判断中的利用联合评估。
...
TL;DR: 论文指出深度递归语言模型常用的深度截断评估指标存在缺陷：它同时改变块应用次数、不同计算量和读出头的分布外残差流，却通常只被解释为反映不同计算量。作者提出 Depth Control Protocol (DCP) 来解耦这三类因素。


<details>
  <summary>Details</summary>
Motivation: 判断深度递归语言模型是否真正利用其深度时，递归与层剪枝文献常依赖推理时截断深度、绘制质量与保留深度比例并读取斜率。该指标虽廉价且免训练，但存在未经审视的缺陷：深度截断同时改变多个模型属性，导致斜率被混淆解释。

Method: 提出 DCP 诊断套件，包含三个阳性对照分别隔离每个因素并变化其他因素，一个阴性对照在稠密 Transformer 上施加相同干预以排除测量协议假象，以及一个受控训练干预以验证因果性。关键控制是运行完整块应用预算但只执行单一不同迭代；该控制在深度权重共享架构中严格可实现，因为稠密网络中重复层会得到不同模型而非同一模型的替代配置。

Result: 摘要未报告具体实验数值结果。主要结果是提出 DCP 诊断协议，用于解耦深度截断指标中混淆的三类因素，并指出关键控制仅能在深度权重共享架构中实现。

Conclusion: 传统深度截断斜率不能可靠解释为仅反映不同计算量；需要 DCP 这类诊断套件来分离因素并通过训练干预验证因果性。该工作为评估深度递归语言模型是否真正利用深度提供了更严格的测量框架。
...
### [47] [Not All AI Agents Are Equal: Characterizing Resource and Performance Dynamics](https://arxiv.org/abs/2609.19947)
*Wonmi Choi,Minuk Park,Zhixiong Niu,Yongqiang Xiong,Chuck Yoo,Gyeongsik Yang*

Main category: cs.AI

TL;DR: 本文分析LLM智能体在检索问答、网页搜索和软件编程任务中的资源动态与延迟表现，发现任务和工具会导致CPU、磁盘I/O、内存等瓶颈差异显著，并提出CPU感知工具准入与任务感知CPU分配优化。


<details>
  <summary>Details</summary>
Motivation: LLM智能体通过迭代推理和工具执行处理请求，常涉及远程LLM API和本地工具容器。其延迟、本地资源需求和容器瓶颈跨请求混合，使服务优化困难；当前智能体生态较少考虑资源动态，造成宝贵资源浪费。

Method: 针对三类代表性任务：检索增强问答、网页搜索和软件编程，刻画并发处理多请求和多任务时的延迟与资源动态关系。通过测量分析不同任务、同一工具、并发请求下的CPU、磁盘I/O和内存瓶颈，并评估LLM响应速度和CPU核心数对智能体加速的影响。

Result: 测量显示智能体行为随任务变化很大，同一工具的资源动态也可能显著不同；并发请求会暴露任务相关的CPU、磁盘I/O和内存瓶颈；更快的LLM响应或更多CPU核心并不总能加速智能体。基于此提出CPU感知工具准入和任务感知CPU分配，CPU敏感任务延迟提升约5.4倍，多任务平均延迟降低约32%。

Conclusion: 智能体服务优化应重视任务与工具的资源动态，利用CPU感知和任务感知策略可显著降低延迟并减少资源浪费。
...
TL;DR: 该论文提出一种端到端可微的深度架构，联合优化代价函数与路径排序模型，以支持多样化用户偏好的最优路径规划。


<details>
  <summary>Details</summary>
Motivation: 经典图搜索算法依赖启发式代价函数，无法保证路径质量最优；已有数据驱动方法将用户轨迹视为最优路径，但轨迹受导航服务影响，存在反馈循环问题。

Method: 离线运行多目标Dijkstra算法收集Pareto最优路径作为完整候选集；设计模拟最短路径搜索与路径排序的神经网络，实现端到端可微学习；将路径偏好定义为路径属性的约束优化任务，并提出在约束下优化单目标变量的新损失函数。

Result: 在真实世界数据集上的大量实验表明，该架构在路径质量和可定制性方面显著优于现有最先进方法。

Conclusion: 该深度架构能够有效联合学习代价函数与路径排序，为不同用户偏好提供更优且可定制的路径规划方案。
...
TL;DR: 提出E-AVI，一个基于证据的自动视频面试评估框架，通过提取带时间戳的多模态证据，结合维度条件证据注意力与来源级嵌入进行评分，并支持自然语言反馈和问答。


<details>
  <summary>Details</summary>
Motivation: 自动视频面试评估虽能整合语言内容、声学表达和视觉行为，但仅靠数值预测缺乏可检查的支持，难以解释评估依据。

Method: 提取带时间戳的多模态证据；采用维度条件证据注意力与来源级嵌入进行评分；构建共享证据池以支持自然语言反馈和后续问答。

Result: 在RecruitView和私有酒店数据集上，E-AVI在秩相关指标上持续优于微调的多模态基线；消融、证据删除、自助法、人工审计和问答分析验证了证据路径的预测贡献、可解释性和实用性。

Conclusion: E-AVI框架在提升预测性能的同时，为评估、反馈和交互分析提供了可检查的证据支持。
...
TL;DR: 研究发现，AI聊天机器人对乌克兰战争的回答会因提问语言不同而变化；在112种语言中测试GPT、Claude和Gemini后，回答的俄乌倾向差异与全球地缘政治分歧相关。


<details>
  <summary>Details</summary>
Motivation: 人们越来越多地使用AI聊天机器人获取新闻和世界事件解释，因此需要检验不同语言提问是否会导致AI给出不同的政治性答案。

Method: 让GPT、Claude和Gemini用112种语言评估20条关于乌克兰战争的陈述，共收集67,200条回答，并按国家官方语言分组，与公众对俄态度、联合国投票和对乌援助等指标进行比较。

Result: 不同语言下俄倾向与乌倾向回答的比例不同；按官方语言分组后，更偏俄的回答对应更 favourable 的公众对俄看法、更少支持乌克兰的联合国投票和更少对乌援助。该模式在三个模型中均出现，且在移除个别陈述对后仍成立。

Conclusion: 研究提示，信息战可能影响用于训练AI模型的文本，进而使地缘政治偏见通过AI传播。
...
TL;DR: 提出 FedeRICo 联邦交通预测框架，通过梯度级协作和边界感知残差通信，解决异构客户端间空间依赖丢失与参数聚合干扰问题。


<details>
  <summary>Details</summary>
Motivation: 城市交通预测数据常因隐私或商业限制无法共享，联邦时空方法因此受到关注；但各客户端子图的空间拓扑与时间动态差异大，现有方法依赖参数聚合，易稀释客户端特有表示，且路网分割会阻断跨客户端交通动态传播。

Method: FedeRICo 采用双分支预测架构：全局引导分支捕捉可迁移的预测结构，私有残差分支保留客户端特有修正并引入边界残差信号；全局分支通过所有客户端的梯度对齐进行协作优化；同时利用趋势-残差分解提取边界消息，抑制周期结构，仅在物理相邻客户端间传递瞬态时空残差信号。

Result: 在四个真实世界交通预测基准上，FedeRICo 持续优于最先进的联邦时空基线，并保持具有竞争力的训练运行时间。

Conclusion: FedeRICo 能有效缓解联邦交通预测中的客户端异构性与跨客户端空间依赖丢失问题，是一种性能更强且实用的联邦时空预测框架。
...
TL;DR: 本文研究梯度式训练数据归因方法能否定位导致语言模型“潜意识学习”的训练数据，并比较其在不同模型与过滤粒度下的效果。


<details>
  <summary>Details</summary>
Motivation: 潜意识学习可通过与行为特征无显著语义关联的训练数据传播行为，使基于内容的数据过滤失效；因此需要不依赖语义内容、能识别责任训练样本的归因方法。

Method: 评估三种梯度归因方法：GradCos、对比式GradCos变体和EK-FAC；在三个模型上测试，并与divergence tokens基线比较，分别在token级和样本级进行过滤。

Result: token级过滤中，EK-FAC能缓解相当一部分潜意识学习效应，其他方法收益很小，且大多不如divergence tokens；样本级过滤对所有方法效果更差，但EK-FAC常比divergence tokens信号更强。效果在不同方法和设置间不一致，某些模型-偏好组合有效而其他组合失败，且未找到一致解释。

Conclusion: 梯度式归因在某些设置下能识别导致潜意识学习的数据，但不同近似方法的可靠性差异明显，整体仍不稳定。
...
TL;DR: 本文提出免训练的DART方法，通过低秩坐标传输与目标调度响应校准，解决步数蒸馏视频生成模型中LoRA复用时功能改变与质量下降的问题。


<details>
  <summary>Details</summary>
Motivation: 步数蒸馏可降低视频生成成本，但为更长轨迹训练的LoRA在短去噪调度下复用会改变功能效果或降低目标质量；静态参数兼容性无法完全解释相同几何测量下适配器行为差异。

Method: 提出DART，一种免训练方法：结合低秩坐标传输与目标调度响应校准，仅用前向评估、无需源训练视频；在四步Wan2.2目标上评估，并做组件分析和适配器级分析，另在两个目标上验证趋势。

Result: 在四步Wan2.2上，DART-F将联合质量分从0.9029提升至0.9227，宏观功能保留从-0.4644变为+0.1349；组件分析显示校准贡献主要质量提升，坐标传输与校准结合提供互补增益；适配器级结果有正功能效应和强衰减伴随负功能效应减少；两个额外目标呈现相同总体趋势。

Conclusion: 结果表明应通过功能保持与负迁移避免联合评估蒸馏模型的LoRA复用，而不假设每个适配器都能恢复。
...
### [54] [WiCleanData: Guaranteeing the Type Consistency of Wikidata by Taxonomy Refinement and Constraint Enforcement](https://arxiv.org/abs/2609.20057)
*Yiwen Peng,Marc Jeanmougin,Thomas Bonald*

Main category: cs.AI

TL;DR: 本文提出 WiCleanData，一个经过自动化清理的 Wikidata 版本，具有一致的分类体系且无类型约束违规，并通过 Web 界面公开提供。


<details>
  <summary>Details</summary>
Motivation: Wikidata 因协作性质存在错误、不一致和过度复杂问题，如冗余类、实例与类混淆、错误分类路径和类型约束违规，而人工大规模清理不可行。

Method: 设计自动化流水线：借助语言模型辅助清理分类体系，通过层次聚合简化类型约束，并据此过滤事实。

Result: 生成了无任何类型违规的知识图谱，并已通过 Web 界面公开，便于探索和下游应用。

Conclusion: WiCleanData 能有效缓解 Wikidata 中的分类与类型约束问题，为知识图谱的可靠使用提供可用资源。
...
### [55] [MAGMA-GEN: Validated Recovery Supervision from Ambiguous Failures via Counterfactual Re-Execution](https://arxiv.org/abs/2609.20056)
*Loan Bernat,Matthieu Grard,Ariane Herbulot,Florent Lamiraux*

Main category: cs.AI

TL;DR: 论文提出 MAGMA-GEN，一种在策略数据生成流程，将层级机器人系统中模糊的失败 rollout 转化为经过验证的恢复监督数据，从而提升长时程操作任务的成功率与恢复能力。


<details>
  <summary>Details</summary>
Motivation: 层级机器人系统需要高层语义决策协调随机低层技能，但失败 rollout 往往难以归因：可能来自无效高层决策、部分可观测或低层执行失败。传统监督学习缺少恢复状态数据，强化学习又受稀疏奖励和非局部信用分配困扰。

Method: MAGMA-GEN 使用特权教练对失败 rollout 假设早期决策级错误，并提出局部纠正或恢复动作；由于诊断可能出错，仅当从同一状态在匹配条件下重新执行能改善下游进展时，才保留候选，从而生成来自智能体自身失败分布的监督样本，无需逐步人类演示。

Result: 在交互式长时程操作任务中，MAGMA-GEN 相比蒸馏和轨迹修复基线，在模拟与真实机器人执行、且任务约束动态变化的情况下，提升了任务成功率和恢复能力。

Conclusion: MAGMA-GEN 能有效将模糊失败转化为经过验证的恢复监督，为层级机器人长时程操作提供了一种无需人类逐步演示的恢复学习与数据生成方案。
...
TL;DR: 本文提出FCA-CF框架，用形式概念分析的概念格作为反事实搜索的硬结构约束，在TCGA-BRCA多模态乳腺癌数据上生成临床可用的反事实解释，并与多种真实反事实方法对比。


<details>
  <summary>Details</summary>
Motivation: 多模态乳腺癌深度学习诊断虽预测准确，但缺乏可操作的反事实解释；LIME、SHAP等归因方法不生成替代实例，无法用反事实质量指标评估，因此需要真正可评估的反事实生成方法。

Method: 使用FCA概念格作为反事实搜索的硬结构约束，构建FCA-Guided Counterfactual框架；在TCGA-BRCA多模态数据集上，针对60个良性预测实例，与Wachter-style CF、DiCE、FACE、NICE四种反事实方法进行基准比较，并进行消融分析。

Result: FCA-CF达到Validity=1.0000、Sparsity=2.37（所有有效方法中最佳）、Proximity=0.900（与NICE并列最佳）；分类器Accuracy=0.980、F1=0.976、ROC-AUC=0.9947。消融显示FCA格约束是稀疏性主要驱动因素，移除后稀疏性增加40%，p<0.001，Cohen's d=0.78；Phase C贪心精修贡献最大，禁用后稀疏性增加113%，p<0.001，d=5.01。

Conclusion: FCA引导的反事实生成实现Pareto主导的临床重要结果：同时最稀疏、邻近性最佳之一且有效性完美；其稀疏性源于概念格拓扑而非数值惩罚项，构成反事实解释文献中的结构性新贡献。
...
### [57] [Tailored to you: longitudinal effects of personalising language models](https://arxiv.org/abs/2609.20077)
...
TL;DR: 该研究通过992名参与者连续5天的日常建议寻求互动，比较非个性化、基于记忆的个性化和基于问卷的个性化三种语言模型，发现部分人机互动变化主要源于重复接触而非个性化，但不同个性化方式会带来自我披露、诡异感和信息分享后悔等差异化影响。


<details>
  <summary>Details</summary>
Motivation: 个性化语言模型日益受关注，但持续与个性化模型互动如何影响人们对AI的感知与行为，尤其是对用户自我认知和人际关系等互动之外的下游后果，仍缺乏研究。

Method: 招募992名参与者，进行为期5天的每日建议寻求互动；设置非个性化基线，并与两种个性化方式对比：基于记忆的个性化（依据先前对话历史）和基于问卷的个性化（依据研究前收集的问卷信息）。

Result: 随时间变化的人机互动效果主要由重复接触驱动，而非个性化本身；但个性化组在建议寻求和信息分享态度与行为上存在差异：记忆个性化组自我披露更多且认为模型更不诡异，问卷个性化组对分享个人信息表现出更高后悔。

Conclusion: 不同个性化方法对互动结果具有微妙且差异化的影响，研究结果对负责任地设计和部署个性化AI系统具有启示意义。
...
### [58] [Marginal utility, matrix factorization, and the Key-Value (KV) cache: a unified information-economic framework for sovereign geo-mining inference](https://arxiv.org/abs/2609.20068)
*Caroline Gans Combe*

Main category: cs.AI

TL;DR: 本文将经济学中的边际效用概念与矩阵分解、Transformer 的 Key-Value 缓存联系起来，提出统一的维度保留/分配规则，并将其应用于地质矿产文档的结构化信息抽取，报告了一个轻量分类器的性能与 TIES 合并的诊断结果。


<details>
  <summary>Details</summary>
Motivation: 研究动机是建立边际效用与机器学习中矩阵分解、表示学习和 KV 缓存压缩之间的理论桥梁，并利用该框架指导地质矿产文档自动化结构化信息抽取中的高效推理、模型合并与选择策略。

Method: 方法包括：将评分矩阵奇异值谱解释为潜因子的边际效用递减表，将投影协方差算子特征值谱解释为学习表示的边际效用表，将缓存逐出和低秩缓存压缩视为内存预算下的约束效用最大化；统一规则为保留特征值超过绑定约束影子价格的最高维度。应用上采用多轮推理协议、逐层 TIES 模型合并，以及结合抽取质量、定位漂移和能耗的选择策略，并对漂移使用条件风险价值（CVaR）项进行标量化。

Result: 实证结果包括：一个 11.2 百万参数的层次分类器在单 GPU 上约 5 分钟训练完成，在 973 篇铀勘探语料留出测试集上达到 90.0% 的一级准确率；同一语料 50 篇人工审计中，专有模型为 92.0%；该分类器每卡延迟 2.62 ms，而 API 约 2000 ms，成本可忽略。均匀密度 TIES 合并诊断暴露出可复现的退化模式：合并模型在五个地理不同区域输出逐 token 相同结果且声称高置信度；按逐层校准密度重新执行合并后，该诊断样本上的退化特征消失。完整规模抽取基准（含 LoRA 微调）为投影结果而非实测，仍属未来实证扩展。

Conclusion: 论文提出并验证了边际效用、矩阵分解与 KV 缓存之间的统一分配视角，展示了其在地质矿产文档信息抽取中的实用价值：轻量模型可实现接近专有 API 的准确率且延迟和成本极低，同时揭示了 TIES 合并的退化风险并给出逐层密度校准的缓解方式；完整基准仍有待实证完成。
...
TL;DR: 本文提出一种面向巴西三军的Agentic AI概念架构，旨在通过自主规划、数据访问、工具执行和行动来缓解多域作战环境中C2中心的决策压力。


<details>
  <summary>Details</summary>
Motivation: 多域作战环境（陆、空、海、网络、电磁频谱）使数据量和速度激增，当前国防AI多为被动、孤立工具，仍需人工整合信息、评估态势和制定行动方案，加重了OODA决策周期负担。

Method: 提出概念性Agentic AI架构，使AI系统能自主且可听地规划、访问数据源、执行工具并行动；讨论四个应用方向（决策支持、态势分析、可行性研究、对策建议），并分析数据与传感器访问需求以及安全与权限保障措施。

Result: 形成了支持巴西海军、陆军和空军决策的概念架构，明确了四个应用前沿、数据/传感器访问要求，以及适用于行政、战略、战役和战术层面的安全与权限保障。

Conclusion: 该Agentic AI架构有望增强多域C2决策支持，但需在数据访问、安全与权限方面建立负责任使用机制，以支持巴西三军各层级决策。
...
TL;DR: 论文提出 UnifiedPlayers，一个由规划、执行和评估三类智能体组成的协作框架，通过角色特定奖励和 GRPO 联合优化，实现工具使用智能体的自进化，并在推理任务和验证器质量上取得提升。


<details>
  <summary>Details</summary>
Motivation: 现有自进化方法通常将轨迹生成与评估分离，依赖静态验证器，难以适应新出现的失败模式；而自一致性信号可能强化跨轨迹共享的错误。因此，需要联合适应规划、执行和评估，但这带来各组件相互改变训练数据与反馈的协调难题。

Method: 提出 UnifiedPlayers 协作框架：Planning Player 生成任务，Execution Player 产生带 Python 工具调用的多轮轨迹，Evaluation Player 构建可执行验证器。设计角色特定奖励，在 GRPO 下协调三个玩家朝共享学习目标优化。

Result: 在两种模型骨干和十二个推理基准上，UnifiedPlayers 相比最强先前基线，在数学推理上至少提升 3.5%，在通用推理任务上至少提升 3.9%。学习到的验证器达到 84.2% 的对抗检测准确率，其奖励信号相比自一致性基线具有 2.03 倍的每题方差，验证更具判别性。

Conclusion: 专业化玩家之间的协作是迈向自我增强、工具集成智能体的一条有前景的路径。
...
TL;DR: 本文从最小跨度视角研究 Antibandwidth 与 Cyclic Antibandwidth 问题，提出 Minimum Span Antibandwidth/Cyclic Antibandwidth Labeling（MSABL/MSCABL）问题，并构建统一的 SAT 求解框架。


<details>
  <summary>Details</summary>
Motivation: Antibandwidth 和 Cyclic Antibandwidth 是 NP-hard 图标记问题，已有研究多关注最大化最小（循环）距离，而其互补的最小跨度视角研究较少。

Method: 将 MSABL/MSCABL 形式化为一系列决策问题，利用单调性加速搜索；采用并行 SAT 求解和增量 SAT 求解两种策略，并在 Harwell-Boeing Sparse Matrix Collection 基准实例上评估。

Result: SAT 方法在解质量上很有竞争力；并行策略在 MSCABL 上整体表现最好，增量策略在 MSABL 上表现最好。在无洞约束下，SAT 方法与 CPLEXCP 竞争力相当，并显著优于 CPLEXMIP 和 Gurobi，尤其对 MSCABL。

Conclusion: 结果表明，SAT 求解是解决 MSABL 和 MSCABL 问题的有效精确方法。
...
TL;DR: 本文提出一种分解式置信度层，包含感知、布局和验证三个可解释通道，并结合保形风险控制，用于金融文档键值提取的直通处理（STP）。在三个公开数据集和两种VLM模型上验证，AUROC从0.54-0.74提升至0.90-0.99，自动批准率可达49-72%且错误率低于10%目标。


<details>
  <summary>Details</summary>
Motivation: 金融文档的STP需要校准概率并对自动批准层的残余误差提供有界保证。现有VLM的言语化置信度信号不可靠，与字段正确性关联弱，无法满足工业部署需求。

Method: 设计三个可解释置信度通道：感知、布局和验证，并最终结合保形风险控制，形成分解式置信度评分。在三个公开数据集（真实发票、合成发票、广告表单）和两种VLM家族（Qwen3.6-27B、Gemini-3.1-Flash-Lite）上验证。

Result: 分解式评分持续改善正确与错误提取的分离，AUROC从VLM言语化信号的0.54-0.74提升至0.90-0.99，三个通道均有贡献。在目标错误率<10%的风险控制下，原生VLM置信度仅能清除0.1%-7.0%字段，而本方法可自动批准49-72%字段，同时保持接受层经验错误率不超目标。

Conclusion: 所提方法可实现可靠的金融文档STP，显著提升自动批准率并维持有界误差，适用于工业部署。
...
TL;DR: 论文提出MTVA-Bench，用于在级联语音代理的真实条件下评估语言模型，包含多轮对话、工具调用、多语言和双评委评分。


<details>
  <summary>Details</summary>
Motivation: 现有评估要么端到端测试整条流水线，混入ASR/TTS错误；要么只测LLM，忽略真实电话场景中的转录问题、多消息拆分和语言/文字要求。

Method: 构建49个代理、490个审核场景、支持7种语言的基准；由LLM扮演呼叫者，mock后端响应工具调用参数；评分结合确定性工具调用检查和两个LLM评委，分别评估场景规则与对话质量，且评委需引用具体消息；任务分与对话分等权。

Result: 在7个模型研究中，6个模型选择正确工具的分差在6.4分以内，但总体得分差距达24.4分；主要差距来自参数值、动作顺序、规则遵循以及工具调用前后的表述。

Conclusion: MTVA-Bench能在级联系统条件下更精准评估语言模型决策能力，表明仅看工具选择不足，需关注参数、顺序、合规和对话质量。
...
TL;DR: 论文提出 PaGNet，一种结合 LightGBM 与 Panel-MLP 的双分支混合模型，用于从公司—年度面板数据预测企业避税代理指标，并在韩国 KoTaP 数据上进行无泄漏、共享超参数评估。


<details>
  <summary>Details</summary>
Motivation: 企业避税代理指标的预测信号分散在较短的公司历史和多个相关目标中，同时筛选式应用要求模型行为透明可解释。

Method: PaGNet 包含两个分支：LightGBM 分支使用面板—时间汇总特征，Panel-MLP 分支使用注意力池化时间聚合与共享主干多任务学习。通过按目标验证集最优的混合器生成最终预测和分支依赖诊断，且不引入可训练融合参数。在 KoTaP 面板的 1,754 家韩国上市公司 2011—2024 年数据上，采用无泄漏、共享超参数协议跨四种特征制度评估。

Result: 在排除直接代理滞后的 FS1 和加入税务历史的 FS2 制度中，应计目标 TSTA、TSDA 稳定路由到 LightGBM 分支，PaGNet 在主划分上比六个基线中最强者提高解释方差约 0.08—0.11。GETR 常偏向神经分支，CETR 暴露出验证—测试分支选择不一致而非稳定分支分配。面板展平控制显示，应计收益主要来自观测到的多年基础面板值，PaGNet 的面板感知表示带来较小但方向一致的改进。滚动起点分析确认应计路由稳定，将 ETR 诊断限制为划分特定行为，并发现远期限划分中监督模型不如朴素持续性。

Conclusion: PaGNet 不应被视为普遍更优的表格学习器，而应被视为一种代理感知面板模型，它将有竞争力的预测能力与显式的按目标分支依赖报告结合起来。
...
TL;DR: 本文提出并检验顺序情境契合度（SCF），一种基于嵌入的指标，用于衡量当前信息状态与近期情境的匹配程度；跨语言、音乐情绪、视听情绪EEG、赌博决策、活动识别和决策EEG等任务，较低SCF预测更长加工时间、更大情感或行为转变以及更强神经状态变化。


<details>
  <summary>Details</summary>
Motivation: 人类感知、行动和决策以序列方式展开，但现有计算预测器通常局限于特定领域，缺乏可跨领域衡量当前信息与近期情境兼容性的通用指标。

Method: 使用简单的近因加权相似核计算嵌入相似度，得到SCF；该指标可应用于词语、声音、视觉场景、情感状态、选择、动作和神经表征。研究在语言加工、音乐诱发情绪、部分视听情绪EEG数据、赌博决策、人类活动识别和决策相关EEG中测试，并控制惊讶度、强化学习预测误差、声学变化、视觉变化和传感器变化等既有预测因子。

Result: 较低的情境契合度预测更长的处理时间、更大的情感或行为转变以及更强的神经状态变化；这些效应在控制既有预测因子后仍然存在。

Conclusion: SCF提供了一个计算测量层，可将情境兼容性与行为加工及认知/神经状态转换动态联系起来。
...
TL;DR: 提出 JointMatch，一种基于学习的共享出行匹配框架，将请求配对与车辆分配联合建模在单张图上，兼顾收益与大规模调度效率。


<details>
  <summary>Details</summary>
Motivation: 传统方法将匹配分解为请求配对和车辆分配两个顺序子问题，计算方便但会在未知可用车辆前锁定拼车组合，导致收益损失且扩展性差。

Method: 构建联合图，利用空间邻近性稀疏化，使图规模随车辆和请求数线性增长；用图神经网络一次前向传播为所有候选决策打分；再通过监督训练和策略梯度微调对齐实际收益。

Result: 在纽约市黄色出租车数据上，JointMatch 超过经典 Blossom 启发式和两阶段 GNN 基线，且常大幅领先；在城市规模（车队10000）下，每个调度轮次比二者快20倍以上；监督训练缩小大部分收益差距，策略梯度微调进一步对齐实际收益。

Conclusion: 联合学习式匹配能在单图中同时处理请求配对与车辆分配，在收益和计算效率上均优于传统分解方法与两阶段基线，适合城市级共享出行调度。
...
### [67] [AI-Driven Real-Time Relay Optimisation in Smart Urban NR-V2X Networks via Learning-to-Optimise Graph Neural Networks](https://arxiv.org/abs/2609.20271)
*Giambattista Amati,Federica Mangiatordi,Emiliano Pallotti,Simone Angelini*

Main category: cs.AI

TL;DR: 本文提出一种基于图神经网络（GNN）的AI驱动学习优化（L2O）框架，用于NR-V2X系统中实时多跳中继选择，以提升智慧城市和工业4.0场景下的可靠低时延通信。


<details>
  <summary>Details</summary>
Motivation: NR-V2X网络中，路侧单元（RSU）部署有限且城市传播条件复杂，导致网联自动驾驶车辆（CAV）难以保持稳定连接，因此需要可扩展的实时中继选择方案。

Method: 将车联网建模为图：节点表示CAV和RSU，边编码无线链路特征；使用离线混合整数线性规划（MILP）生成最优中继决策作为监督信号，训练带边特征的边感知图同构网络（GINE）。

Result: 在真实城市数据集上的实验表明，该方法达到接近最优的连接性能，连接增益最高提升11.3%，相比MILP执行时间减少数个数量级，最高实现100倍加速。

Conclusion: 该框架支持可扩展、实时的网络控制，适用于智慧城市和工业4.0部署。
...
TL;DR: 论文研究自主智能体因数据库读取、检索证据、策略等输入在推理中变化而导致的提交一致性问题，提出TCT事务框架，通过认知可串行化与效果兼容认知准入保证突变与其推导输入在有效点一致，并实现低开销原型。


<details>
  <summary>Details</summary>
Motivation: 自主智能体从可能变化的数据库读取、检索证据、策略、信念和授权中推导出具体突变；现有数据库隔离只排序已提交事务，智能体事务处理只检查提案是否满足可执行合约，二者都不能保证突变与推导输入存在共同有效点，除非合约表示相关谓词。

Method: 提出类型化依赖令牌区分内容完整性与适用性，可信中介捕获暴露给推理的值；定义严格认知可串行化与较弱效果兼容认知准入；TCT组合不可变版本化可执行定义、注册表派生授权计划、密封信封、守卫优先提交事务、密封后基于信封和见证的授权、共同提交收据、幂等授权终结和收据驱动认知对账；以完整注册足迹和单一增长阶段诱导无环锁点顺序。

Result: 给出可串行化条件与观察等价边界，实现零错误健全性和正进展；原型在证伪测试中阻止所有注入异常，平均提交开销增加3.22 ms。

Conclusion: TCT为智能体自主推导的突变提供一致性保证，在输入可能变化时通过认知可串行化或效果兼容准入确保提交效果可被合理解释，且实现开销可接受。
...
TL;DR: JEPA-WAM 通过为每条文本指令生成多样化的视觉目标参考（由现成文生图模型生成、冻结 V-JEPA 2.1 编码为紧凑目标 token），以交叉注意力条件化视频与动作专家，从而提升机器人操作中的指令跟随能力，并在真实机器人基准上显著超越 π0 与 Fast-WAM。


<details>
  <summary>Details</summary>
Motivation: 现有 World Action Models（WAMs）虽具备较强机器人操作能力，但仅依赖文本指令时指令跟随能力有限。作者认为部分原因在于机器人学习数据的结构性失衡：丰富的视觉-动作轨迹常仅搭配稀疏、重复的语言标注，使策略更依赖视觉上下文和运动规律识别任务，而非真正 grounding 文本指令。

Method: 提出 JEPA-WAM：1）用现成文本到图像生成器，在不训练生成器的情况下，根据文本指令随机采样多张任务完成图像，作为视觉指令库；2）尽管生成图像在外观和布局上可能与当前场景不同，但其语义与指令对齐，可作为视觉目标参考；3）使用冻结的 V-JEPA 2.1 编码器提取任务级语义；4）将稠密目标表示压缩为紧凑目标 token，并通过交叉注意力同时条件化视频专家与动作专家；5）构建真实机器人指令跟随基准，涵盖分布内、OOD 场景和 OOD 指令三种设置。

Result: 在构建的真实机器人指令跟随基准上，JEPA-WAM 在分布内、OOD 场景、OOD 指令三种设置下的成功率分别为 87.3%、74.5%、80.9%，相比 π0 和 Fast-WAM 至少分别高出 10.0、27.3、14.5 个百分点。

Conclusion: JEPA-WAM 表明，通过引入随机生成且语义对齐的视觉目标参考，并用冻结 V-JEPA 2.1 提取任务级表示，可有效缓解机器人学习数据中语言标注稀疏带来的指令跟随不足问题，显著提升分布内及分布外场景和指令下的指令跟随性能。
...
TL;DR: 论文提出 AgentPProf，将智能体轨迹聚合为 pprof 兼容 профиль，用于跨运行、长期 profiling。它通过语义操作栈和递归操作分割实现任务级层级归因，支持火焰图可视化和分析，帮助定位问题与优化成本。


<details>
  <summary>Details</summary>
Motivation: AI 智能体越来越多地长期编排用户、工具和系统资源，开发者需要定位失败、不安全效应和高预算任务。现有智能体可观测工具侧重单次执行调试与 tracing，而非跨运行长期 profiling，难以规模化回答归因问题。同时，智能体 profiling 的难点在于责任实体是“诊断认证”“比较分支”等任务意图，而非代码路径，且缺乏稳定标识用于聚合。

Method: 提出语义操作栈模型，将智能体轨迹适配为 profiling：用统一操作表示所有活动，用操作栈替代运行时调用栈，实现不同粒度的层级归因。观察到任务占据连续区间并分解为子任务，因此提出递归操作分割，按任务边界递归切分轨迹。实现 AgentPProf，将智能体轨迹聚合成 pprof 兼容 профиль，支持火焰图可视化与分析。

Result: AgentPProf 在 CodeTraceBench 上对人工标注达到 0.764 B^3 F1。在三个问题定位基准上，profile 将 MAP 最高提升 56%，表明其能有效归因资源、定位问题，并在实际 profiling 开销下帮助优化 token 成本。

Conclusion: 智能体可观测性不仅需要调试，还需要 profiling。AgentPProf 证明语义操作栈与递归操作分割可用于聚合轨迹、可视化和分析，从而辅助优化智能体质量、安全与成本。AgentPProf 已开源。
...
TL;DR: 论文提出“休眠动力学漂移下的任务就绪”决策问题，统一主动变化诊断与变化后控制恢复：在有限且任务无关的交互预算下，智能体需定位局部动力学变化，用少量信息交互表征变化，并在下游任务身份揭示后为候选任务提供恢复策略及可达回报下界，或弃权至安全回退。


<details>
  <summary>Details</summary>
Motivation: 已部署控制策略可能掩盖关键动力学变化，例如执行器失效未影响当前任务，因为策略很少激励该执行器，但它对未来任务可能至关重要。因此需要在任务未知、交互预算有限条件下统一解决变化诊断与恢复问题。

Method: 提出 Evidence-Gated Matched-Pulse Transport，一种基于干预的贝叶斯过程，通过共享匹配响应表示耦合故障定位与执行器有效性估计，在保持诊断可靠性的同时将局部证据转化为恢复相关不确定性，并传播到任务条件策略选择与就绪认证。

Result: 在多种模拟器上的休眠执行器基准中评估该框架，采用分离诊断与能力恢复的协议，按就绪覆盖率、选择性风险、交互成本和回报评分，并识别出传输证据具有决定性的故障机制。

Conclusion: 该框架能够在预期性能、置信度与回退使用之间进行显式权衡，支持部署决策，并有效连接变化诊断与变化后控制恢复。
...
### [72] [Structured Four-Stage Legal Translation: From Natural-Language Traffic Rules to PROLOG](https://arxiv.org/abs/2609.20334)
*May Myo Zin,Wachara Fungwacharakorn,Ken Satoh,Katsumi Nitta*

Main category: cs.AI

TL;DR: 该研究针对交通法规自然语言到Prolog逻辑翻译中的歧义与语义不完整问题，提出结构化四阶段法律翻译框架S4L，并在20条真实交通规则上评估其效果。


<details>
  <summary>Details</summary>
Motivation: 交通法规为人类理解而写，依赖背景知识和灵活措辞，存在歧义、上下文依赖和语义不充分问题，与Prolog等计算推理引擎所需的精确逻辑结构相冲突。

Method: 评估两种基线方法：自然语言到Prolog（NL→Prolog）和逻辑英语到Prolog（LE→Prolog）；提出推理引导的S4L→Prolog框架，在单一引导提示中执行语义角色抽取、场景补全、逻辑映射和Prolog规则生成。使用20条真实交通规则作为基准，从语法有效性、语义正确性和逻辑完整性三方面评估。

Result: S4L→Prolog准确率最高，正确形式化75%的规则；NL→Prolog为60%，LE→Prolog为55%。定性分析表明，S4L比基线更可靠地捕捉隐含因果关系、道义模态和例外结构。

Conclusion: 结构化推理提示能显著提高自然语言到逻辑翻译在法律和安全关键应用中的可靠性。
...
TL;DR: 提出NeuSOGA3D，一种神经符号混合框架，从无组织点云中重建可解释且CAD兼容的几何表示。


<details>
  <summary>Details</summary>
Motivation: 三维点云重建仍具挑战；神经隐式方法精度高但潜在表示难以解释和复用，限制了工程工作流中的应用。

Method: 结合NeuSOGA学习感知先验与显式符号几何推理：将点云投影到主正交平面，构建符号隐式样条表示，通过保形CSG操作融合成粗视觉外壳；利用截面分解和部分保形样条进行体积重建恢复细节；输出控制多边形、隐式样条场、截面和体积放样等显式符号实体。

Result: 在ModelNet40全部40个类别上的实验表明，该框架能从多样点云中恢复结构有意义且CAD兼容的几何表示。

Conclusion: 结合学习感知与符号几何推理在可解释几何智能方面具有潜力。
...
TL;DR: 本文为 Camelot Jr. 积木解谜游戏提出一种融合定性推理与质心稳定性计算的混合模型，以支持可解释的玩家指导和空间技能训练。


<details>
  <summary>Details</summary>
Motivation: 空间推理能力与 STEM 领域表现密切相关，游戏适合训练儿童空间技能；但游戏中的类人辅导需要 AI 能从空间事件中进行常识推理并给出可解释反馈，而定性推理模型适合将游戏状态转化为符号化、可解释的指导信息。

Method: 针对 Camelot Jr. 中搭建多层桥梁、连接两座塔上角色的任务，提出混合定性模型；为处理平台稳定、路径规划和用完所有积木等精确物理要求，引入基于质心的数学稳定性逻辑来引导定性求解器。

Result: 构建了面向 Camelot Jr. 的定性求解框架，可将游戏状态转化为可解释反馈，并借助质心稳定性计算处理关卡中的物理稳定性约束。摘要未报告量化实验结果。

Conclusion: 该方法有助于在 Camelot Jr. 中训练空间技能，并推动以人为中心、可解释的游戏 AI 代理发展。
...
TL;DR: 提出一种结合去噪扩散模型与对抗损失的混合方法，从2D图像重建3D微观结构，以改善复杂非均质材料的三维生成质量。


<details>
  <summary>Details</summary>
Motivation: 3D成像昂贵、不易获取且对某些材料有技术限制，因此需要从更易获得的2D图像重建3D体积。现有SliceGAN对复杂异质微观结构的细节捕捉不足，促使寻找替代生成框架。

Method: 采用基于去噪扩散模型的混合生成方法。由于没有3D真值，用对抗损失替换标准去噪损失，从而在实验中实现稳定训练。

Result: 模型能够生成不同复杂度的微观结构，切片伪影较少，并且相分数和结构描述符与真值接近。

Conclusion: 扩散模型结合对抗训练可稳定生成高质量3D微观结构，适用于复杂异质材料的三维重建。
...
### [76] [PAA: The Probabilistic Allen Algebra: A Generative and Complete Probabilistic Extension of Allen's Interval Relations](https://arxiv.org/abs/2609.20634)
*Julian Eggert*

Main category: cs.AI

TL;DR: 本文提出概率艾伦代数（PAA），将艾伦区间代数的13种定性时序关系扩展为基于边界概率分布的完整概率化模型，并给出蒙特卡洛验证与开源Python实现。


<details>
  <summary>Details</summary>
Motivation: 传统艾伦区间代数使用精确边界上的清晰谓词，难以处理语言、感知、数据库或不确定历史中时间、时长和边界不确定以及“刚好之前”“大致在……期间”等渐进表达。

Method: 以高斯时间点、具有高斯中点和截断高斯时长的区间为基础，在统一概率空间中将关系定义为边界排序谓词；点-点关系化为误差函数，点-区间和区间-区间关系化为由线性不等式诱导的多元高斯概率积分；引入容差带使接触关系具有正测度，并推导关系分类、层级分解及CIDOC CRM式时序原语。

Result: 构建了生成式且完整的概率艾伦代数：13种关系在单一容差下构成真正划分，容差趋零时恢复清晰艾伦代数；粗粒度谓词如先于、重叠、包含是叶节点概率之和；区间退化为点时13种关系依次归约为5种和3种；代数具有尺度不变性，并能区分“ shortly before”等渐进表达与接触关系；结果经蒙特卡洛验证并作为开源Python包发布。

Conclusion: 概率艾伦代数从区间边界分布中推导出艾伦关系体系，而非直接设定关系得分，为不确定时序推理提供了原则性概率框架，并兼具理论完备性与可实现的软件工具。
...
### [77] [Language-model groups overstate consensus when replaying human deliberation on a reasoning task](https://arxiv.org/abs/2609.20543)
*Tengfei Shao*

Main category: cs.AI

TL;DR: 该研究比较人类Wason小组与匹配LLM代理小组的全共识率，发现共识率高度依赖评分与参与定义；在减少测量不对称后，LLM组仍更易达成共识，且模拟共识不反映集体准确性。


<details>
  <summary>Details</summary>
Motivation: 全共识率常被用作集体认知指标，但其估计受参与率和最终状态如何操作化的影响；需要检验LLM模拟小组能否可靠复现人类审议结果。

Method: 回放100个保留的人类Wason小组，并用匹配的LLM代理组进行模拟；按每位参与者讨论前答案设置一个信念锚定代理，对人和代理使用同一评分代码；比较不同人类评分定义，并进行submit-based与participation-matched两种盲后敏感性分析，以及去除早停和可记忆答案的重参数化检验。

Result: 不同人类评分定义下全共识率为24.0%至57.0%；约五分之一人类参与者从未发帖，而代理几乎都会发帖。代理组更共识：submit-based比较（n=98）在chat和reasoning模式下差距为34.0和43.9个百分点；participation-matched比较（n=45）差距为34.1和44.4个百分点，两种方法差异在0.5个百分点内收敛。无早停和去除可记忆答案后差距仍存在；reasoning模式组几乎一致，但多集中在错误答案。模拟共识未追踪集体准确性，信念锚定代理组对人类组结果分布是有偏估计。

Conclusion: 评估模拟群体对人类审议结果的估计必须明确评分规则；在该设定下，LLM代理组会高估共识且与集体准确性脱节，不能直接作为人类群体结果的无偏替代。
...
### [78] [An Empirical Study of Harness Design for Coding Agents](https://arxiv.org/abs/2609.20804)
*Run-Ze Fan,Zihao Zhang,Simin Ma,Yebowen Hu,Shouju Wang,Kaiqiang Song,Fei Liu,Hamed Zamani,Xiaoyang Wang*

Main category: cs.AI

TL;DR: 本文通过一个执行循环固定、仅变化规划、动作空间与上下文管理三个组件的轻量级编码框架，在4个模型、SWE-Bench Verified与Terminal-Bench 2.1上开展176组匹配实验，系统评估各组件对自主编码智能体长时程性能的独立影响。


<details>
  <summary>Details</summary>
Motivation: 编码框架决定了模型能力如何转化为长时程软件工程表现，但已有研究通常将框架作为整体评估，导致单个组件的有效性不清晰，缺乏组件级对比。

Method: 构建执行循环固定、仅变化规划、动作空间和上下文管理的轻量级编码框架；在4个模型、两个基准上评估176组匹配设置，涵盖5种上下文管理策略、4种上下文窗口预算，并对规划与动作空间进行定向消融。

Result: （1）上下文窗口预算越紧，上下文管理价值越大，收益主要来自避免上下文溢出失败；（2）先规则裁剪再LLM摘要的整体效率最优，使被裁剪内容可恢复反而增加几乎不被使用的机制且无准确率提升；（3）规划从弱模型的准确率支架转变为强模型的成本节省手段，准确率变化不大；（4）预定义工具对bash能力弱的模型有帮助，而bash能力强的模型仅用bash接口即可有效工作且成本显著更低，在命令行任务上尤为明显。轨迹级分析表明：上下文管理延长轨迹而不改变行为，规划改变轨迹停止位置，动作空间改变代码编写粒度。

Conclusion: 研究支持面向模型与预算的编码框架设计，并提供一个模块化框架用于评估未来更多框架组件。
...
TL;DR: 论文提出 SkillAA，一种面向冻结语言模型的结构化技能优化框架，用统一图结构表示技能的适用性、执行与组合，通过对比成功与失败轨迹进行溯因归因，定位并局部修复技能，再用双重门控筛选更新，从而在不更新模型参数的情况下提升下游任务表现。


<details>
  <summary>Details</summary>
Motivation: 现有外部技能方法虽无需参数更新即可提供领域流程，但往往直接依据失败轨迹编辑技能，缺乏从失败到可编辑位置的结构化路由；同时已有技能图未充分利用语义边界、对象地址和拓扑依赖，难以支持技能检索、定向更新与范围化验证。

Method: SkillAA 将技能适用性、执行和组合统一建模为图，使同一结构同时服务于技能选择、归因引导修复和更新验证。它对比成功与失败执行，将候选修复路由到具体图对象，仅更新选定的局部结构，并通过 Local Gate 与 Big Gate 在提交前筛选候选变更。

Result: 在 gpt-5.6-sol 上，SkillAA 在 SearchQA、LiveMath 和 DocVQA 分别达到 81.5%、66.7% 和 91.2%，在所有主要设置中取得最高观测均值。

Conclusion: 结果表明，归因引导的图编辑与图范围化验证具有实际效用，为冻结语言模型下的技能优化提供了有效路径。
...
### [80] [The Organization of Inference: Information, Resource Constraints, and AI Production](https://arxiv.org/abs/2609.20449)
*Yukun Zhang,Kemu Xu,Yishen Chen*

Main category: cs.AI

TL;DR: 论文通过受控工作流实验研究AI推理中容量与任务信息的分布如何影响经济价值，发现工作流与信息结构会显著改变成功率与资源使用效率。


<details>
  <summary>Details</summary>
Motivation: 推理的经济价值取决于容量和任务信息如何在AI生产的各阶段分配，作者试图厘清规模、工作流与信息结构对生产价值的作用。

Method: 在外部验证的软件工程任务上进行受控工作流实验，设置两个匹配资源面板（逻辑token上限12,000与24,000），比较直接执行和信息受限规划；并开展严格只读规划实验，操纵规划者是否可见任务issue。

Result: 直接执行成功率在两个token上限下均为59.6%；信息受限规划成功率从36.2%升至51.2%，规划劣势缩小15.0个百分点（95%任务簇bootstrap区间4.2–25.8）。12,000 tokens时，可见issue比隐藏issue提高约16个百分点；任务知情规划比直接执行低约10点，但24,000 tokens时反超29.6点。直接执行资源使用远低于上限；规划工作流的绑定率从46.2%降至0.8%，下游执行贡献总使用量增量的89.9%。

Conclusion: 规模决定系统可用容量，而工作流与信息结构决定推理的生产性价值；合理分配任务信息与容量能改变AI工作流的成功率和资源利用。
...
TL;DR: 该研究在τ²-bench的零售与航空任务中，分析智能体执行框架的规划、执行组织与完成检查组件对成功率、错误接受率和成本的影响。


<details>
  <summary>Details</summary>
Motivation: 理解执行框架各组件如何影响任务成功、错误接受与成本，并揭示在不同责任/损失设定下，规划指导与验证器哪个更重要。

Method: 在τ²-bench中进行两个零售实验和一个航空试点；核心对比为固定任务专属计划与词数匹配的打乱策略文本；使用265个匹配单元、oracle验证成功率、任务聚类bootstrap区间，并评估只读终端验证器。

Result: 固定计划使oracle验证成功率提升7.17个百分点（90%任务聚类bootstrap区间1.15–13.36），收益集中在高复杂度任务；只读终端验证器拒绝61%的零售oracle无效回合，同时误拦17%正确回合，每回合额外成本低于1美分。

Conclusion: 规划指导能提升成功率，验证器能减少错误接受；哪个组件更重要取决于错误接受的损失：低责任时规划收益占优，高责任时验证器避免的误通过占优。独立验证器能以远低成本获得完整规划加验证栈的几乎所有误通过收益。
...
TL;DR: 提出 FreqCondNorm，一种基于 Transformer 的预测性维护模型，通过 FiLM 风格频率条件归一化层统一不同采样频率的异构时序数据，并在五个公开数据集上预训练。


<details>
  <summary>Details</summary>
Motivation: 深度学习预测性维护模型跨机器、跨工况迁移能力差，尤其标签稀缺且信号采样频率跨越 1 Hz 到约 100 kHz 五个数量级。

Method: 设计带频率条件归一化层的 Transformer 架构；在 CWRU、MFPT、UOC18、PRONOSTIA、CMAPSS 五个公开数据集上，采用掩码自编码、对比学习和平衡域采样进行预训练。

Result: 故障诊断上 CWRU 准确率 99.2%，比 CNN 提升 6.4 个百分点；MFPT 零样本准确率 82.1%，显示跨采样频率迁移能力。但剩余使用寿命预测未获改进。

Conclusion: FreqCondNorm 能有效统一异构时序并提升故障诊断迁移性能，但预训练目标与 RUL 预测任务不匹配，需未来研究。
...
### [83] [SoL-Pi: Recursively Scaling Auto-Research Loops for Efficient Agent Harness](https://arxiv.org/abs/2609.20519)
...
TL;DR: 论文提出面向编码智能体长期自主运行的harness层token效率优化方法SoL-Pi，通过大规模自动研究循环筛选出四种可复用机制，在EdgeBench上保持性能的同时显著降低token流量与API成本。


<details>
  <summary>Details</summary>
Motivation: 编码智能体正从受监督代码补全转向无人值守、全天候探索，其工作从孤立预测变为包含推理、工具使用与反馈的长轨迹，因此token效率对扩展递归自我改进至关重要。

Method: 采用RSI启发的harness层方法，在大量且多样化的环境中扩展自动研究循环并进行harness rollout，最终筛选出四种可复用机制，涵盖动作执行、上下文压缩、观察处理和委托阅读，形成SoL-Pi。

Result: 在51项任务的EdgeBench评测中，SoL-Pi在GPT-5.6 Sol和Opus 5上达到与Pi相当的性能，同时将记录的token流量降低44.7%至49.0%，API成本降低约三分之一；相对原生Codex和Claude Code harness，估计每小时节省8.75至13.50美元，相对Pi节省4.36至5.71美元。

Conclusion: 大规模自动harness发现能够产生可迁移、可复用的改进，推动自动harness发现走向生产级成果，并显著改善长期编码智能体的token经济性。
...
### [84] [Refuse, Decompose, Refresh: A Claim-Safe Protocol for Closed-Loop AI Evaluation](https://arxiv.org/abs/2609.20538)
*Peiying Zhu,Sidi Chang*

Main category: cs.AI

TL;DR: 论文提出一种 claim-safe 协议，针对闭环 AI 评估中“可完全复现但仍支持错误结论”的风险，通过 Refuse、Decompose、Refresh 三个操作，并在仅聚合模拟器中进行预注册验证。


<details>
  <summary>Details</summary>
Motivation: 闭环系统中，策略决定访问状态、可观测组件以及哪些失败会留下可测量痕迹，因此评估可能统计上可复现，却仍支持错误声明；需要把协议执行、操作错误准入和结构性假设分开处理。

Method: 提出 claim-safe 协议：Refuse 在缺乏干净参考流或匹配运行时比较时弃权；Decompose 分开报告协议执行、操作错误准入和结构假设，而非单一 PASS/FAIL；Refresh 将分布偏移警报视为使参考图失效并重算的请求，而非故障证据。在仅聚合模拟器中实例化，含 24 个策略组件、3 种需求机制、2 个故障掩蔽族以及独立开发和留出种子；预注册留出集含 1440 个案例和 21600 个分区行。

Result: 仅 55/72 个机制-组件单元被参考准入，其中 54/55 保持运行时准入，使弃权成为结果的一部分。稳定错误准入为 0/20 个代表组件，在冻结 0.20 规则下单侧精确 95% 上界为 0.1391。在准入单元内，受影响清洁流量比名义故障单元比例更具预测性：540 个单元-臂行嵌套于 20 个组件簇，cell-minus-traffic 负对数似然差为每行 0.1264 nats，95% 组件簇区间为 [0.0593, 0.1918]。漂移日志显示“null”必须相对于参考：干净故障空流在三种机制下分别触发 15/15、0/15、14/15 次警报，仅中间机制匹配冻结检测器参考。

Conclusion: 论文的贡献不是通用阈值，而是一个可执行契约，将可观测支持、统计校准和合理声明联系起来；弃权和参考相对性应被纳入评估协议本身。
...
TL;DR: 该研究通过探索性定性调查探讨了人们在使用AI完成任务时何时仍觉得成果属于自己。发现所有权感受取决于协作过程：主导、迭代或重写时保留所有权，仅批准AI建议时则放弃所有权；同时，个人声音缺失和理解不足会削弱所有权，披露意愿则常与自豪感脱钩。


<details>
  <summary>Details</summary>
Motivation: 随着AI融入日常任务，需要研究当机器参与创作时，人们的归属感与贡献感会发生什么变化。

Method: 开展探索性定性调查，要求参与者描述两个近期自选的AI辅助任务：一个感觉属于自己的，一个感觉不属于自己的。

Result: 归属感取决于协作过程：仅批准AI建议会让人放弃所有权，而主导、迭代或重写则保留所有权；当人们拥有项目愿景而非执行时，所有权也可延伸，甚至对没有AI无法完成的任务报告高所有权；失去个人声音和不理解输出都会侵蚀所有权；披露AI使用的意愿常与自豪感或所有权脱钩，而受社群规范和害怕贡献被抹除影响。

Conclusion: 研究提出若干研究方向，以促进支持人们对自己生活拥有作者感的AI发展。
...
### [86] [Limits of Confidence in Diffusion](https://arxiv.org/abs/2609.20581)
*Russ Webb,Amitis Shidani,Alice Bizeul,Dan Busbridge*

Main category: cs.AI

TL;DR: 本文指出，离散扩散采样器若每步并行写入多个位置，只有当这些位置在已固定 token 条件下条件独立时，该步才能匹配训练分布；否则各位置边缘分布的乘积无法表示存在依赖的联合分布。作者在 ScanAndAdd 合成任务上验证了这种偏差：置信度排序写入的多位置组均存在依赖，生成分布总变差远高于噪声下限，但单样本指标看似完美。


<details>
  <summary>Details</summary>
Motivation: 离散扩散、重掩码和均匀状态采样器常通过每步并行写入多个 token 位置来加速生成，但像素、音素、词等真实数据中 token 之间存在依赖。作者因此质疑：仅基于各位置边缘分布选择并写入多个位置，是否还能正确匹配训练数据的联合分布。

Method: 作者从理论上分析离散扩散并行写入步骤与训练分布匹配的条件，证明 per-position 分布乘积无法匹配依赖组，且边缘分布不能决定联合依赖结构。随后构造联合分布有闭式解的 ScanAndAdd 合成任务，检验 confidence ranking 每步写入的未确定位置组是否依赖，并测量生成分布的总变差与采样噪声下限。

Result: 理论上，一个步骤只有在所写位置于已固定 token 条件下条件独立时才能匹配训练分布；任何依赖组都不能由各位置分布的乘积表示；相同边缘分布可对应不同的联合依赖结构。实验上，ScanAndAdd 中 confidence ranking 写入的任意两个及以上未确定位置组都是依赖的；生成分布总变差达到采样噪声下限的 29 倍，而逐样本指标仍为 1.0。

Conclusion: 常用离散扩散并行采样策略在存在多位置依赖时会产生原理性分布偏差，而单样本指标可能掩盖这种分布级错误。需要显式考虑 token 依赖，或改进采样、排序与分组策略以匹配真实联合分布。
...
### [87] [Deep Noir: Autonomous Steering Discovery via Architectural Chronometry in Transformer Models](https://arxiv.org/abs/2609.20722)
*Frank E. Bobe,Gregory D. Vetaw,Darshan W. Bryner,Matthew G. Cook,Jose L. Salas-Vernis*

Main category: cs.AI

TL;DR: Deep Noir 是一个通过 Logit Lens 收敛和因果头级归因自动发现激活干预参数的框架，能在多模型规模和多任务上提升分类性能，并揭示激活引导带来的提示注入攻击面。


<details>
  <summary>Details</summary>
Motivation: 激活引导可在推理时改变大语言模型行为，但现有方法中“在哪里引导、引导强度多大”仍依赖人工设定，缺乏自动化且机制可解释的发现手段。

Method: 提出 Deep Noir 框架，结合 Logit Lens 收敛分析与因果头级归因，自动发现最优激活引导参数和干预点；在 1B、2-3B、7-9B 等多个模型规模与架构上，针对垃圾信息分类和 SST-2 情感分类进行评估，并与 RepE 等方法比较。

Result: 在垃圾信息任务上，1B 模型提升 16.7 个百分点（标准差 4.7；39 次运行），7-9B 四个架构提升 21 至 42 个百分点；SST-2 情感任务上零代码修改提升 13.1 个百分点。情感任务中，未使用头掩码的 RepE 未能超过基线，而 Deep Noir 在所有模型上均有提升（p<0.01）。此外，激活引导会形成可预测的提示注入攻击面，且脆弱性随引导强度单调增加。

Conclusion: 机制归因使自动发现干预点成为可能，并可跨任务和架构泛化，显著提升激活引导效果；但部署带激活引导的分类器时需关注其引入的提示注入安全风险，尤其是智能体系统。
...
### [88] [Q&A on Any Spreadsheet Requires Interpreting Its Grid Structure](https://arxiv.org/abs/2609.20732)
*Zofia Smoleń*

Main category: cs.AI

TL;DR: 论文提出用单元格角色标注将电子表格切分为可解释块，以增强LLM驱动RAG的上下文与答案生成；方法虽超越SOTA，但受限于离散分类的固有天花板，主张转向降维技术把2D表格扁平化为1D文本。


<details>
  <summary>Details</summary>
Motivation: 电子表格在RAG中难以处理：它是2D非结构化数据，关系连续且潜在单元格角色无限，而分类模型只能使用有限预定义类别，无法完美捕获其结构语义，即使人工标注也存在上限。

Method: 提出基于单元格角色标注的框架，将任意电子表格分割成可解释的chunk，并与当前最先进方法比较；同时分析离散单元格分类的局限，提出发展维度降低/扁平化技术的方向。

Result: 该框架优于当前SOTA，但面临硬性上限；语义单元格标注主要通过丰富上下文帮助答案生成，而非提升检索准确率；离散分类无法完整表达表格的结构细微差异。

Conclusion: 仅靠离散单元格分类无法解决电子表格到LLM的瓶颈；领域应发展降维技术，直接将2D非结构化电子表格压平成1D非结构化文本，使下游RAG更易解释和生成。
...
TL;DR: 提出RAFT，一种有状态RAG框架，将历史支持案例抽象为时间线条目链，并在条目级检索，从而匹配当前案例的中间状态并返回锚定该状态的父案例轨迹；在合成基准与真实Apache Jira数据上验证了检索效果。


<details>
  <summary>Details</summary>
Motivation: 企业客服排障依赖从相似历史案例中检索可操作指导，但现有RAG系统将支持案例视为静态文档，忽视其多阶段、有状态的特点，难以在案例进展的中间状态上实现有效匹配。

Method: RAFT将每个已结历史案例抽象为有向时间线条目链，支持条目级检索：先找到与当前案例中间状态匹配的条目，再返回以该匹配状态为锚点的父案例轨迹；可选案例级图通过可配置相似度表示连接案例。评估上，直接评测检索层而无需生产部署，并使用两个数据集：基于Microsoft Learn Windows Server文档构建的合成基准，以及带有人工重复标签的真实Apache Jira问题。

Result: RAFT在案例进展的每个阶段上，Case Hit均优于vanilla RAG和GraphRAG基线，且相对最强基线取得统计显著提升；Jira结果表明该优势可方向性迁移到真实案例历史。

Conclusion: 条目级、有状态的历史案例检索能有效提升排障代理的案例匹配能力，且无需完整代理系统部署即可评估；作者发布了基准、实现和Apache Jira评估集。
...
### [90] [Can Vision-Language Models Judge Olympic Diving? From Reasoning to Scores in Zero-Shot Action Quality Assessment](https://arxiv.org/abs/2609.19354)
*Henry O. Velesaca,David Freire-Obregon,Luigi Miranda,Abel Reyes-Angulo*

Main category: cs.CV

TL;DR: 该论文评估开源视觉语言模型（VLMs）在奥运跳水视频零样本动作质量评估中的能力，并提出一个结合VLM语义推理与阶段子分数的回归集成框架，以预测最终比赛分数。


<details>
  <summary>Details</summary>
Motivation: 奥运体育中的自动动作质量评估具有挑战性，原因在于人体运动复杂且专家评分存在主观性。作者希望探索开源VLMs能否用于零样本动作质量评估，并提升评估的可解释性与自动化程度。

Method: 在AQA-7基准数据集上，使用开源VLMs对跳水视频进行零样本评估；提出基于回归的框架，利用VLM生成的语义推理和阶段级子分数，结合TF-IDF向量化、降维和集成学习来预测最终比赛分数。

Result: 单独VLM的Spearman相关性低于0.32；所提出的集成回归框架显著提升性能，四模型配置达到0.67的Spearman相关性。文本推理特征始终优于原始数值子分数，说明VLM生成解释具有更丰富的动作质量信息。

Conclusion: 研究表明，VLMs有潜力作为可解释和半自动化体育表现评估的辅助工具。代码已在GitHub公开。
...
TL;DR: 该研究提出RAUL框架，仅利用输尿管镜视频在体模中恢复输尿管镜轨迹，并基于轨迹计算导航指标，以评估不同经验水平住院医师的操作技能。


<details>
  <summary>Details</summary>
Motivation: 输尿管镜碎石术中解剖导航不完整会导致重复干预；熟练外科医生再干预率较低，但目前缺乏客观指标量化镜体导航表现，也无法判断受训者何时达到熟练水平。

Method: 提出RAUL参考辅助重建框架：对每个体模，先用缓慢、高质量参考探查视频生成参考重建，再将后续探查视频定位到该参考中，从而仅从视频恢复输尿管镜轨迹。使用电磁跟踪的镜体位姿评估定位精度，并从体模探查轨迹计算导航指标，比较不同经验水平的住院医师。

Result: 在9个体模上，方法平均平移均方根误差为0.5±0.1 mm；相比标准SfM，逐帧定位覆盖率从50.5±14.9%提高到86.1±7.2%。重建轨迹在既定导航指标上显示出高经验与低经验受训者之间的显著差异。

Conclusion: RAUL能比标准SfM更完整地从视频恢复输尿管镜轨迹，无需额外跟踪设备即可进行基于轨迹的技能评估；这是首个不依赖外部跟踪传感器、仅用视频恢复输尿管镜轨迹用于技能评估的工作，支持可扩展的输尿管镜导航技能自动化评估。
...
### [92] [Open ultrasound foundation model for robust segmentation and clinical measurement across heterogeneous settings](https://arxiv.org/abs/2609.19230)
*Chao Qin,Fahad Shahbaz Khan,Salman Khan,Sarim Ather,Siddiq Anwar,Rao Muhammad Anwer,Shadab Khan*

Main category: cs.CV

TL;DR: 该研究发布超声图像资源SonoCorpus和交互式分割基础模型SonoBase，旨在解决超声AI模型碎片化、跨设备/操作者/解剖部位泛化差的问题，并在多中心、多任务和外部数据上验证了其优越的分割与临床指标表现。


<details>
  <summary>Details</summary>
Motivation: 超声是全球使用最广的影像模态，但临床AI多为窄范围单任务模型，设备、操作者或解剖结构变化时性能下降，缺乏统一、可泛化、可复现的超声基础模型与数据资源。

Method: 构建SonoCorpus开放资源，整合53个公开数据集、456,963张图像和1,626,085个专家掩码，覆盖24个临床应用和17个国家；基于该资源预训练交互式分割基础模型SonoBase；在15个评估数据集上比较SAM2、MedSAM2、MedSAM3及专家模型；评估外部数据、射血分数、胎儿头围和孕龄等临床指标，并测试少样本适配及向SAM3等新架构的迁移。

Result: SonoBase在所有评估数据集上优于SAM2、MedSAM2和MedSAM3，并匹配同数据训练的专科模型；在完全外部数据上超过基线在其分布内基准的精度。射血分数误差6.63%，除颤器适用阈值误分类13%低于基线的18–42%；胎儿头围误差1.81 mm、孕龄误差1.2天，低于观察者间变异。基线完全失败的病例中，SonoBase在81%情况下恢复可用分割，包括塞拉利昂和坦桑尼亚低训练用户使用手持探头场景；5个标注样本即可适应新环境，训练协议也可迁移至SAM3。

Conclusion: SonoBase表明超声专用预训练可带来跨设备、操作者、地理区域和解剖部位的强泛化能力，优势不完全依赖特定架构；研究释放检查点、优化器状态、数据划分、去重哈希和代码，以促进可复现研究和社区平台化发展。
...
TL;DR: 本文研究无需训练、开放词汇的自动驾驶3D目标检测：用SAM3以类别文本提示在六环视相机中生成实例掩码，再结合场景几何转成3D框，并在nuScenes上控制变量比较不同3D几何来源。


<details>
  <summary>Details</summary>
Motivation: 现有3D检测器依赖大量人工标注3D框，类别固定，无法发现类别表之外的目标；作者因此探究能否以免训练、开放词汇方式完成3D检测。

Method: 使用可提示分割模型SAM3，以类别名作为文本提示，在车辆六环视相机中获得实例掩码；将掩码通过场景几何转换为度量3D框。在nuScenes上进行三阶段受控比较：固定2D检测，仅改变3D几何来源，包括纯图像预测几何、在相同掩码内用免训练规则拟合原始LiDAR点、推理时借用监督框几何。反向实验则用相同掩码构建三态相机见证规则，增强监督LiDAR-only检测器。

Result: 纯图像几何达到0.183 mAP；在相同掩码内用免训练规则拟合原始LiDAR点达到0.298 mAP/0.348 NDS，零标注成本；推理时借用监督框几何提升至0.413 mAP/0.555 NDS，说明最大缺陷在测量精度而非2D检测，类别混淆和置信度校准在替换后仍可维持。反向规则将监督LiDAR-only检测器从0.596 mAP提升到0.630 mAP，约为全监督相机融合增益的一半，且无需训练。SAM3对范围内目标的正确命名掩码覆盖率为84%；官方指标中失败类别主要是命名错误或几何苛刻，而非未见类别。

Conclusion: 免训练、开放词汇3D检测可行，主要瓶颈是几何测量精度；2D分割与开放词汇覆盖较强，掩码可有效增强LiDAR检测，但完全替代监督几何仍有明显差距。
...
TL;DR: 论文提出 LinePilot Digitizer，结合基于颜色的连续曲线恢复与三种标定模式，并发布首个线图数字化基准 DigitizerBench，用 FPC-NRMSE 评估性能。


<details>
  <summary>Details</summary>
Motivation: 从线图恢复数值序列需要准确的坐标轴标定和可靠的曲线提取，同时缺乏系统评估数字化器性能的专用基准。

Method: 采用连续颜色曲线恢复；提供 LinePilot 标准、增强和 OCR 三种标定模式；构建 DigitizerBench，以正交设计覆盖信号、渲染和图形结构因素，并结合自动与人工评估；使用失败惩罚截断归一化均方根误差 FPC-NRMSE 进行评价。

Result: 在 DigitizerBench-Full 上，LinePilot (OCR) 平均 FPC-NRMSE 最低为 0.672，可信可用率最高为 38.2%；在 DigitizerBench-Lite 上，LinePilot (enhanced) 平均 FPC-NRMSE 最低为 0.081，输出成功率 100%，可信可用率最高为 93.3%；正交设计还支持因子分析以识别关键影响因素。

Conclusion: 三种标定模式在共享曲线恢复流程中提供了自动化、用户控制与准确性之间的实用权衡，DigitizerBench 也为数字化器性能评估提供了系统基准。
...
TL;DR: 本文研究异构视觉模型融合，提出 Riemannian--Lorentz Parameter Fusion (RLPF)，在保留 ViT 与 SSM 两种架构的同时按语义对齐参数组，并通过黎曼/双曲几何求测地重心，再用学习门控融合分支 logits。


<details>
  <summary>Details</summary>
Motivation: 扩展深度学习面临数据枯竭、训练成本指数增长和资源集中等瓶颈；模型融合无需梯度下降即可组合预训练检查点，可大幅节省重训练成本。但现有权重空间融合通常要求架构与参数形状对齐，难以融合 ViT 与状态空间模型等异构模型。

Method: RLPF 先按语义角色对齐参数组并投影到共同坐标；将选定坐标提升到洛伦兹双曲模型；计算正则化测地重心并解码回两个分支；用学习门控按输入组合分支 logits。组件组使用固定曲率，归一化参数按欧几里得处理。

Result: 微调后系统在 CIFAR-10 达 82.37%，Oxford-IIIT Pet 达 75.04%，ImageNet-1K top-1 达 78.58%；对应最佳父模型为 76.54%、71.42%、76.42%。ImageNet-1K 上报告微调前初始化达 77.80%。

Conclusion: 结果支持进一步研究几何感知的异构融合，但不支持训练无关的单检查点融合；RLPF 是双分支混合方法，其门控和最终模型均经过训练。
...
TL;DR: 论文提出 WorkZonePlan 数据集、WAVE 半自动生成流程以及 BoundaryFormer/BF++ 模型，联合预测车道边界、施工区边界和驾驶轨迹，以提升自动驾驶车辆在施工区的感知与规划安全性。


<details>
  <summary>Details</summary>
Motivation: 施工区通过临时交通控制和封闭改变车道几何，且这些信息可能不在车载地图中，给自动驾驶感知与规划带来挑战；同时公开数据集稀少，缺乏结构化几何监督，导致模型泛化能力受限。

Method: 构建 WorkZonePlan 数据集，包含149K+合成与5K+真实多模态样本，提供车道边界、施工区边界和轨迹选项的3D标注；提供76个闭环CARLA场景、三种天气下228条Bench2Drive格式评估路线。提出WAVE半自动数据生成流程。提出基于Transformer的BoundaryFormer（BF），使用slot attention联合预测车道/施工区边界多项式与驾驶轨迹；BF++进一步引入相机/相机+LiDAR变体、度量地平面编码、类型化边界/轨迹查询、长距离点锚、图像空间曲线细化和保守门控LiDAR融合。

Result: 在所有四个模型共有的211条路线上，BF++-Camera和BF++-Camera+LiDAR的Driving Score分别为63.0和64.4，高于SimLingo的59.3和TransFuser++的26.1。BF++参数量比SimLingo小40倍、比TransFuser++小10倍以上，同时获得更高Driving Score。

Conclusion: 联合预测车道边界、施工区边界和驾驶轨迹是提升自动驾驶在施工区安全运行的有前景方向，且BF++在更小模型规模下取得更优表现。代码和数据集已开源。
...
TL;DR: 论文提出 Reflection-aware Gaussian Splatting (RGS)，一种基于物理的延迟渲染框架，用于解决 3DGS 在反射区域表面塌陷问题，提升几何和镜面反射质量。


<details>
  <summary>Details</summary>
Motivation: 现有 3D Gaussian Splatting 方法在反射区域常出现表面塌陷，导致几何和镜面反射质量较差。

Method: 提出 RGS 框架，利用 3D 基础模型提供几何先验，设计跨视图形状一致性正则化以正则化几何表面；并提出反射感知致密化策略，捕获不同视图的镜面变化。

Result: 大量实验表明，RGS 能一致地渲染高质量反射物体，达到 state-of-the-art 性能。

Conclusion: RGS 能准确建模镜面反射区域，生成更平滑的几何表面，减少几何空洞，并提升新视角合成质量。
...
TL;DR: 该研究提出 NoRDeC 框架，利用冻结的肾脏特征提取器进行单类异常检测，实现肾小球病理的检测与表征分析。


<details>
  <summary>Details</summary>
Motivation: 肾小球病理的细粒度评估需区分多种异常类型，但监督分类要求每类都有标注样本；当某些亚型罕见或训练队列中缺失时不可行。单类异常检测仅建模正常数据，可检测未见过的异常。

Method: 使用 Omni-Seg 的冻结残差 U-Net 主干，该主干预训练用于分割结构正常的肾脏基本单元，无需异常亚型标签。提出 NoRDeC，将马氏正常参考评分与逐层表征分析结合，评估主干层和空间聚合策略，并与 PaDiM、PatchCore 比较，使用 CKA 分析表征。数据来自两个机构的肾小球图像。

Result: Layer 4 配合 Center-70 聚合取得合并 AUROC 0.926±0.013。NoRDeC 在 7 个异常类别中的 6 个以及合并分析中取得最高 AUROC。CKA 提示亚型依赖的层间关系变化，而仅靠异常分数无法捕获这些变化。

Conclusion: 冻结的肾脏特征提取器可在不使用异常样本拟合检测器的情况下，同时支持肾小球异常的检测和表征层面刻画。
...
TL;DR: EUMU是第8届LSVOS挑战赛MUMU赛道的冠军方案，使用单一高效多模态模型统一完成多概念图像标注、开放词汇目标检测和图像描述。


<details>
  <summary>Details</summary>
Motivation: MUMU挑战要求用单个高效模型同时执行多概念图像标注、开放词汇目标检测和图像描述，并满足严格的计算与内存资源限制。

Method: 基于共享预训练多模态模型，利用其提示能力进行检测和描述，并在共享视觉特征上训练轻量头预测质量、场景和事件标签；采用任务感知推理优化，跨任务复用输出作为线索，例如用描述线索找回漏检目标、用检测线索细化描述、用图像统计和描述/检测线索优化标注预测。

Result: 模型包含239.169M参数，需要23.947 GFLOPs，峰值推理内存4.5 GB，最终挑战赛得分为17.3409，并获得MUMU赛道冠军。

Conclusion: 通过跨任务线索复用和统一模型设计，EUMU在单一模型内统一了三种多模态理解任务，同时满足挑战赛的资源约束。
...
TL;DR: 提出ParticleSplat，一种自监督、以物体为中心的学习方法，将场景分解为潜在“粒子”，并通过前馈3D高斯泼溅表示语义实体，实现3D空间与几何推理。


<details>
  <summary>Details</summary>
Motivation: 现有Deep Latent Particles（DLP）本质上是2D表示，缺乏显式3D空间与几何推理能力，难以支持机器人操作等下游任务。受潜在粒子与3D高斯基元结构相似性的启发，希望将粒子表示扩展到3D。

Method: 在DLP基础上引入3D潜在粒子空间，使用新颖视图合成目标进行训练。模型联合编码多视角图像与相机位姿到共享的3D物体中心潜在空间，并将粒子转换为粒子对齐的3D高斯，通过组合重建完整场景。

Result: 在模拟和真实数据集上，方法无需监督即可学习物体掩码，并支持可控3D场景编辑（如在潜在空间修改粒子来移动物体）；学习到的3D表示还提升了机器人操作下游任务性能。

Conclusion: ParticleSplat将物体中心粒子表示从2D扩展到3D，利用3D高斯泼溅实现自监督场景分解、编辑和下游机器人任务改进，验证了3D潜在粒子表示的潜力。
...
TL;DR: 该论文提出 SCOUT，将文本到人物检索中的跨模态检索建模为嵌入空间预测：用可训练预测器把冻结视频编码器 V-JEPA 的 patch token 映射到冻结文本编码器 EmbeddingGemma 的嵌入空间，并用双向 InfoNCE 训练，基础模型不微调任何编码器。研究发现：文本编码器几何匹配度可预测检索效果；ExPLoRA 与属性分解重排序可提升 top-rank 精度；局部-公开校准可解释干预迁移。在 AI City Challenge 2026 Track 4 上，完整 retrieve-fuse-rerank 系统达 84.25 mAP@10，单一冻结模型为 60.63。


<details>
  <summary>Details</summary>
Motivation: 在合成数据训练、真实图像库检索的 sim-to-real 场景中，现有方法通常依赖昂贵的微调 cross-encoder。作者希望验证：仅使用冻结编码器的系统是否也能具备竞争力，并降低训练成本。

Method: 将跨模态检索视为嵌入空间预测：训练一个预测器，把冻结 V-JEPA 视频编码器的 patch token 映射到冻结 EmbeddingGemma 文本编码器的嵌入空间，采用双向 InfoNCE 目标；基础模型不微调编码器。预测器由 Qwen3.5-0.8B 解码器初始化。引入参数高效 ExPLoRA 适配视频编码器，以及基于视觉语言模型的免训练属性分解重排序器，并进行局部与公开集校准研究，最终形成 retrieve-fuse-rerank 系统。

Result: 第一，免训练对齐分数可将三个候选文本编码器按检索准确率排序，Spearman ρ=1.0；第四个 LLM 编码器显示规则依赖度量，邻域重叠分数 ρ=0.8，线性探针 ρ=-0.2。第二，ExPLoRA 与属性分解重排序两个精度导向杠杆提升 top-rank 精度，排行榜 R@1 增加 2.2 点。第三，完整系统在 AI City Challenge 2026 Track 4 达 84.25 mAP@10，单一冻结模型为 60.63；训练组件约耗 95 GPU-hours。作者微调的 cross-encoder CMP 训练需 16 GPU-days，且只是完整系统中的一个融合成员，而非替代方案。

Conclusion: 冻结编码器系统在 sim-to-real 文本人物检索中可以具备竞争力，尤其与精度提升和重排序结合后表现显著。文本编码器与视频特征的几何匹配是选择冻结文本编码器的关键因素；ExPLoRA 与属性分解重排序可提升精度，局部-公开校准有助于判断哪些干预可迁移到真实域。
...
TL;DR: 提出 AMB3R-SLAM，一种可在单块消费级 GPU 上实时运行的公里级单目 SLAM 系统，支持万帧轨迹重建；通过轻量前端与分层后端实现局部、中层和全局一致性，并可扩展至立体、RGB-D 和 LiDAR 输入。


<details>
  <summary>Details</summary>
Motivation: 现有 SLAM 方法常依赖静态世界假设的 bundle adjustment，难以处理动态场景；同时需要兼顾实时性、大规模轨迹重建和单消费级 GPU 部署，并具备多模态扩展能力。

Method: 采用轻量前端进行低延迟在线跟踪，后端分层逐步施加局部、中层和全局一致性约束；避免依赖静态世界假设的 bundle adjustment，从而自然处理动态场景；方法可扩展融合 stereo、RGB-D 和 LiDAR 额外输入。

Result: 在单块消费级 GPU 上实现实时运行，可处理 10k 帧、公里级轨迹；在 9 个数据集上取得强相机跟踪性能，在 VBR 和 Oxford Spires 上将先前 SOTA 的 ATE 降低 70% 以上；加入 LiDAR 后，在 KITTI 和 VBR 上将 ATE 进一步降至亚米级。

Conclusion: AMB3R-SLAM 能高效处理大规模、动态场景下的实时 SLAM，并可通过多模态输入进一步提升精度，性能显著优于现有先进方法，具有实际应用潜力。
...
TL;DR: 提出PerSeM，一种无需训练的长时程开放词汇无人机建图持久语义记忆框架，通过将逐帧语义观测关联到世界空间体素并构建基于多数投票的语义记忆，并进行保守细化。


<details>
  <summary>Details</summary>
Motivation: 开放词汇分割能为无人机提供丰富语义感知，但逐帧预测在重复观测和视角变化下存在时序不一致问题，因此需要持久且一致的语义建图方法。

Method: 无需训练；将逐帧语义观测关联到持久世界空间体素，构建基于多数投票的语义记忆；通过保留历史的空间细化、信任感知重放和上下文引导验证对记忆进行保守细化。

Result: 在Forest和UAVScenes基准上，持久3D记忆显著提升语义正确性和时序稳定性；在所有五个UAVScenes序列上进一步取得一致提升；增益集中在语义困难与时序不稳定区域。

Conclusion: 持久3D聚合为长时程语义建图提供了强基础，而对不确定记忆状态进行保守细化可在无需重训练或额外神经网络推理的情况下带来额外改进。
...
TL;DR: 提出SOLAR多模态生成模型，用于番茄叶部病害理解，覆盖六类问答任务，并在大规模图像与QA数据上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有植物病害AI多局限于孤立预测任务，难以捕捉症状识别、严重度评估与问答诊断推理之间的互补关系，限制精准农业实际部署。

Method: 将番茄病害分析建模为生成式视觉问答（VQA）任务，采用基于混合专家的Fusion Expert模块对齐视觉特征与任务感知语言表示，实现单模型多任务推理与跨任务知识共享。

Result: 在41,677张图像和216,209个问答对上评估，SOLAR在闭集与开放问答设置下均持续优于视觉单模态、视觉语言及任务专用SOTA模型，准确率、鲁棒性和多模态推理更优。

Conclusion: 生成式多模态建模是植物病害理解的有效方向，代码已开源。
...
### [105] [VGGT-GS SLAM: Uncalibrated Monocular Gaussian Splatting SLAM with Feed-Forward Priors](https://arxiv.org/abs/2609.19628)
*Yuhang Han,Hao Wang,Jiaxi Cao,Xingyu Liu*

Main category: cs.CV

TL;DR: 提出 VGGT-GS SLAM，一种面向未标定视频的单目 3D 高斯泼溅 SLAM 系统，利用 VGGT 前馈位姿与深度先验，通过子图可微 BA 和 GNA 提升定位与渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有高斯 SLAM 通常依赖已标定相机或已知内参，难以直接处理未标定视频；同时需要解决子图间尺度一致性和回环检测问题。

Method: 以 VGGT 前馈位姿和深度为先验，执行子图可微 bundle adjustment，联合优化相机位姿与 3D 高斯地图，并通过解析标定雅可比优化子图共享内参和径向-切向畸变；提出 Gaussian-native alignment (GNA) 进行相机锚定尺度细化及回环候选验证。

Result: 在标准室内基准上，未标定设置下定位精度持续提升，渲染质量较强。

Conclusion: 该方法为未标定高斯 SLAM 建立了强基线。
...
### [106] [Selective Cotton Boll Localization for Robotic Harvesting: Evaluation of Deep Learning Vision Models Under Field Conditions](https://arxiv.org/abs/2609.19592)
...
TL;DR: 该研究开发并评估了用于选择性机器人棉花采摘的深度学习感知框架，比较了多种检测与分割模型，并通过田间机器人实验验证了 YOLOv12-m-seg 的实时采摘效果。


<details>
  <summary>Details</summary>
Motivation: 选择性机器人棉花采摘需要在实际田间光照和天气变化下准确检测与分割棉铃，因此需要评估高效、鲁棒的感知模型以支持实时采摘。

Method: 构建包含 1008 张田间标注图像的数据集，使用三台相机采集；评估 YOLOv8 至 YOLOv13 系列检测模型，以及 YOLOv8-seg、YOLOv11-seg、YOLOv12-seg、SAM、SAMv2.1、FastSAM 和 Grounded-SAM with RAM 等分割方法；并采用 UR5e 机械臂、自定义末端执行器和 ZED2i 立体相机开展田间实验。

Result: GELAN-s 在检测中取得最佳 mAP 与推理速度平衡，mAP 为 86.1%，精度 81.6%，召回率 76.6%，F1 为 79.0%，平均推理时间 42.3 ms/图；YOLOv12-m-seg 在直接分割中平衡最佳，AP@0.5 为 83.7%，推理时间 20.4 ms/图；GELAN-s 生成的边界框提示可提升 SAM 和 SAMv2.1 的棉铃定位，SAMv2.1 Tiny 优于 FastSAM 和 Grounded-SAM with RAM；面积评估中 YOLOv12-m-seg 的 R² 为 0.966，GELAN-s + SAMv2.1 Tiny 为 0.860；田间实验验证了 YOLOv12-m-seg 在不同置信度下的实时检测、分割与选择性采摘效果。

Conclusion: YOLOv12-m-seg 是适用于机器人棉花采摘的高效感知模型，具有较好的田间部署潜力。
...
TL;DR: 该论文针对大规模城市点云中建筑的实例级与功能理解，提出自适应区域划分与统一场景级评估策略，并设计细粒度分类模型，在UrbanBIS和STPLS3D上优于现有SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法多依赖预定义块进行训练和评估，但实际应用中这种划分难以获得，还会引入额外预处理并割裂完整建筑结构；同时，城市建筑细粒度分类研究不足。

Method: 将3D点云投影到BEV平面，用预训练分割模型检测建筑区域，再回投影生成结构对齐的自适应训练块；细粒度分类采用Point Transformer联合编码建筑主体与局部上下文（几何、颜色、核心上下文），使用类平衡加权交叉熵缓解类别不平衡，并提出空间监督对比损失增强类间区分性。

Result: 在UrbanBIS和STPLS3D数据集上的大量实验表明，所提方法在建筑实例分割和细粒度分类任务上均优于现有SOTA方法。

Conclusion: 所提出的自适应区域划分、统一场景级评估及细粒度分类方法有效，无需人工设计块划分，能够提升大规模城市点云中建筑实例分割与功能理解的性能。
...
TL;DR: 本文提出一种面向科学图像质量评估的检索增强生成（RAG）框架，同时覆盖SIQA挑战赛的理解赛道（SIQA-U）和评分赛道（SIQA-S），并在SIQA-U赛道获得第一名。


<details>
  <summary>Details</summary>
Motivation: 科学图像质量评估具有复杂性和专业性，大语言模型需要相关参考案例才能更准确地进行评估；研究旨在同时应对SIQA挑战中的理解与评分任务，并提升模型与人类专家判断标准的一致性。

Method: 构建融合文本语义与细粒度视觉特征的多模态索引，设计多路检索与融合机制，为大语言模型提供高度相关的参考案例，从而增强其对复杂科学图像的评估能力。

Result: 实验结果表明，该框架能有效对齐人类专家的判断标准，并在ICME 2026 Grand Challenges的SIQA挑战赛SIQA-U赛道中获得第一名。

Conclusion: 所提出的RAG框架能够有效支持科学图像质量评估，兼顾SIQA-U和SIQA-S两个赛道，并展现出与人类专家判断高度一致及竞赛领先的性能。
...
TL;DR: 论文提出主动跨视角物体地理定位任务ActiveGeo，让智能体依次选择新视角并决定何时停止，以最少观测提升定位精度；并给出三阶段训练框架ActiveMoPT及零样本测试集ActiveGeo-858。


<details>
  <summary>Details</summary>
Motivation: 传统跨视角物体地理定位（CVOGL）通常假设查询图像固定，忽略了移动智能体主动获取更具信息量观测的能力。

Method: 提出ActiveGeo任务与ActiveMoPT框架，采用三阶段训练：1）多视角提示保持适配，聚合多查询视角并复用初始提示；2）轨迹引导策略初始化，利用监督智能体轨迹学习视角选择与初始停止行为；3）成本感知策略精炼，使用GRPO和增益-成本奖励联合优化定位精度与观测效率。同时构建ActiveGeo-858零样本测试集。

Result: ActiveMoPT在MoP-UAV上仅平均使用1.45个查询视角即达到当前最优性能，并在ActiveGeo-858零样本评估中显著优于先前CVOGL方法。

Conclusion: 主动视角选择与自适应停止能有效提升跨视角物体地理定位的精度和效率，ActiveMoPT为ActiveGeo提供了有效的三阶段训练方案。
...
TL;DR: 提出 VideoResearcher，一个免训练的多智能体框架，可像人类研究者一样自主设计、测试并优化视频理解工具，通过 Solving 与 Evolving 双循环持续提升长视频推理能力。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中的视频智能体虽进展显著，但现有系统依赖昂贵耗时的人工设计与试错；当前自改进方法多局限于低影响提示优化、预定义微工具重组或难以收敛的工具链优化，因此需要一种能自动开发高影响视频工具的方案。

Method: VideoResearcher 采用免训练多智能体框架，包含 Solving 与 Evolving 双循环：分析工具使用轨迹以识别能力缺口，协调专门智能体开发并验证可执行工具，再复用已进化工具增强后续视频推理中的证据获取；通过迭代工具优化与验证提升能力，且不更新模型参数。

Result: VideoResearcher 在自改进智能体中达到最先进性能，并接近人工设计的上界，证明无需训练即可通过自主工具开发扩展长视频理解能力。

Conclusion: 该工作展示了一种免训练范式：通过自主工具开发提升长视频理解中的智能体能力，同时减少昂贵的人工工程投入。
...
TL;DR: 该研究提出状态恢复协议，区分对抗补丁对VLA策略造成的即时动作破坏与补丁移除后的持续状态影响，并评估恢复适配器的效果。


<details>
  <summary>Details</summary>
Motivation: 现有评估多关注连续攻击，未区分即时动作破坏与补丁移除后仍残留的持续状态效应，缺乏可恢复性度量。

Method: 提出状态恢复协议：在匹配的动作块边界移除补丁，在相同剩余步数预算下测量可恢复性；使用干净、随机补丁、偏差匹配和固定方向控制组区分遮挡、动作误差幅度和方向持续性的影响；并评估在攻击诱导状态上训练的恢复适配器在不同干预延迟下的表现。

Result: 在OpenVLA-OFT结合EDPA攻击下，LIBERO-Long任务五个动作块后仅36.2%的回合可恢复，而偏差匹配和固定方向控制组分别为89.9%和87.0%；自回归OpenVLA也观察到类似持续效应。恢复适配器在单块延迟下将恢复率从7.7%提升至47.4%，但延迟干预时收益大幅下降。
...
TL;DR: 论文提出 IMFD，一种基于指令的大视觉语言模型（LVLM）的单阶段多人脸伪造检测器，通过端到端联合定位人脸并预测每个人脸的伪造标签，将预测的人脸框作为视觉线索融入指令中以提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 深度伪造在社交媒体上迅速传播，引发严重担忧。传统多人脸伪造检测器独立裁剪和验证每张人脸，忽略背景上下文和人脸间关系，导致性能不理想。

Method: 利用基于指令的大视觉语言模型（LVLM），提出简单有效的单阶段多人脸伪造检测器 IMFD，端到端训练以联合定位人脸并预测每张人脸的伪造标签。将预测的人脸边界框显式整合到指令中作为视觉线索，增强指令 grounding 和伪造检测。并将现有多人脸伪造数据集转换为指令格式以支持训练和评估。

Result: 实验和分析表明，IMFD 通过将人脸边界框整合到指令中，提升了多人脸伪造检测性能，并持续优于多种当前最先进方法。

Conclusion: 将人脸边界框作为视觉线索融入基于指令的 LVLM 中，可有效改进多人脸伪造检测，IMFD 是一种简单且有效的解决方案。
...
TL;DR: 本文首次系统分析了自回归图像生成中的注意力稀疏性，发现其独特模式，并提出对角感知稀疏注意力，在GPU服务系统上实现最高3.1倍吞吐和1.19倍延迟提升，质量下降小于2%。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成每请求需生成数千视觉token，解码受KV缓存访问瓶颈制约；稀疏注意力可提升效率且图像生成应用可容忍适度质量下降，但其稀疏性假设是否适用于图像生成尚不明确。

Method: 对多种工作负载和开源模型进行注意力稀疏性系统表征；基于观察提出对角感知稀疏注意力，在近期窗口内沿对角注意力方向选择性跳过KV条目；基于FlexGen、FlashAttention-2和自定义内核在GPU服务系统上实现。

Result: 分析揭示预填充-解码不对称、注意力集中于提示和局部token、以及视觉token空间局部性带来的独特对角稀疏模式；方法相比稠密推理最高提升3.1倍吞吐和1.19倍延迟，质量下降小于2%。

Conclusion: 文本LLM的稀疏注意力假设不能直接泛化到自回归图像生成；针对图像生成的对角稀疏模式设计稀疏注意力，可在很小质量损失下显著提升服务性能。
...
TL;DR: 论文提出 GAPrompt++，一种多粒度几何感知的提示微调方法，用于高效适配预训练3D点云模型，在多个基准上以少于2%可训练参数达到甚至超越全量微调的效果。


<details>
  <summary>Details</summary>
Motivation: 预训练3D视觉模型全量微调计算和存储成本高；现有基于提示的PEFT方法忽略了点云固有的几何结构，无法同时编码细粒度几何线索和粗粒度结构语义，也难以在模型层级中有效传播这些信息。

Method: 提出多粒度几何感知提示方法 GAPrompt++，包含：Point Shift Prompter 提取跨尺度多粒度几何特征并进行实例级几何调整；Keypoint Prompter 自适应生成点级提示以突出局部几何显著性和细粒度结构；Prompt Propagation 机制将多粒度几何线索注入整个特征提取层级。

Result: 大量实验表明，GAPrompt++ 在基于提示的PEFT方法中达到SOTA，并在多个基准上超越全量微调，同时可训练参数少于2%；此外构建了两个更具挑战性的基准，分别来自3D高斯泼溅和多视角立体重建，以缓解现有评测数据集饱和问题。

Conclusion: 通过多粒度几何感知提示和层级传播，能够显著提升3D点云任务高效适配的效果与泛化能力，并为未来研究提供更真实多样的点云评测场景。
...
TL;DR: 本文提出 SeetaPsych v1.0，一个开源、统一且可扩展的计算机视觉工具包，用于从人脸图像/视频中提取心理学相关信号，涵盖情绪分析、心率估计、视线估计与注视跟随等模块，并提供标准化Python API和无代码WebUI。


<details>
  <summary>Details</summary>
Motivation: 自动化视觉分析为基于行为的心理测量开辟了新途径，但现有技术模块分散在任务专用系统中，接口异构、部署要求不一，缺乏统一的集成平台，阻碍了大规模可复现研究。

Method: 构建模块化 Pipeline/Runner 架构，自动解析属性依赖、生成计算图并支持模块间中间结果共享；集成四大核心模块（统一人脸情绪分析，含表情识别、动作单元检测、效价-唤醒度估计；摄像头心率估计；屏幕注视点估计；场景注视跟随），并配备人脸检测、面部关键点检测、头部检测等辅助预处理模块；提供标准化 Python API 与交互式 WebUI。

Result: 发布 SeetaPsych v1.0 开源工具包，实现统一的人脸/视频心理信号提取能力，支持可复现的大规模分析与无需编程的快速方法评估。

Conclusion: SeetaPsych 为心理学、行为科学、人机交互及相关领域提供了一个集成、易用的可视化测量平台。
...
### [116] [Federated Learning Framework for Privacy-Preserving Kidney Stone Detection](https://arxiv.org/abs/2609.19740)
*Najiyya Younas,Omar Abdulkader,Yaser Ali Shah,Muhammad Jawad Ikram,Jebran Khan,Amaad Khalil*

Main category: cs.CV

TL;DR: 该研究提出一种联邦学习与优化YOLOv8相结合的肾结石CT检测方法，在保护患者隐私的同时实现分布式协同训练与实时诊断。


<details>
  <summary>Details</summary>
Motivation: 深度学习医学图像诊断多依赖集中式数据存储，带来患者隐私和医疗数据安全风险，且需符合GDPR、HIPAA等法规。

Method: 采用联邦学习框架，使多机构无需共享患者数据即可共同训练模型；对YOLOv8进行改进，引入残差特征融合和DropBlock正则化以增强检测鲁棒性并减少过拟合。

Result: 在分布式CT数据集上，联邦YOLOv8模型的mAP@50达到0.733，能够保持数据机密性，且模型轻量，适合边缘部署和临床实时推理。
...
### [117] [Recency Forcing: Bridging the Long-Horizon Gap in Autoregressive Video Generation](https://arxiv.org/abs/2609.19729)
*Tri Cao,Hung Nguyen,Phong Nguyen,Khoi Nguyen*

Main category: cs.CV

TL;DR: 论文指出现有自回归视频生成在长时程退化，原因是被忽视的训练-推理不一致——KV驱逐不匹配：训练时所有上下文帧都在KV缓存中，推理时因内存限制会驱逐远端帧。作者提出Recency Forcing，通过逐步降低远端帧影响而非截断上下文，缩小该差距。


<details>
  <summary>Details</summary>
Motivation: 自回归视频生成在长时程上质量下降，核心问题是训练与推理时KV缓存上下文不一致：训练用短片段且全部上下文可见，推理时远端帧被驱逐，导致模型失去曾依赖的上下文。简单截断上下文会丢失时间信息并损害运动连贯性。

Method: 引入位置响应R(Δ,t_denoise)作为扰动敏感度度量，发现上下文影响随帧距陡峭衰减且随去噪步变化。据此提出Recency Forcing：在softmax前注意力logits上加非正、时间步相关的Temporal Response Bias（TRB），由R直接导出，不改变上下文长度或训练目标。进一步提出Biased Attention Reparameterization（BAR），将偏置移出softmax，使其成为标准FlashAttention调用且零开销。支持免训练和基于训练两种模式。

Result: 在VBench和VBench-Long上取得最先进的长时程生成质量，且不增加额外推理成本。

Conclusion: 通过让远端帧最终被驱逐前其影响已可忽略，Recency Forcing能有效弥合KV驱逐导致的训练-推理差距，在保持时间连贯性的同时实现高效长时程自回归视频生成。
...
TL;DR: 论文提出 Vision-RL2，通过区域级强化学习优化轻量候选区域网络，实现先粗略定位、再集中分辨率识别，从而在显著减少视觉 token 的同时提升多模态大模型细粒度感知性能。


<details>
  <summary>Details</summary>
Motivation: 提升 MLLM 细粒度视觉感知通常依赖提高分辨率，但会增加视觉 token、视觉编码和语言模型预填充成本。作者发现定位 RoI 与识别内容对分辨率需求不同：定位可容忍约 3 到 4 倍更强的 token 压缩，因此有必要粗看定位、再对证据区域集中高分辨率。现有坐标解码方法成本高且依赖 grounding 能力，轻量候选网络虽快但受注意力噪声影响，且离散区域选择导致答案忠实度无法直接监督该网络。

Method: 提出 Vision-RL2，将候选区域网络从模型注意力中蒸馏，并用区域级强化学习优化。把连贯区域视为动作，由冻结的 MLLM reader 根据移除该区域后答案似然的变化来评分。采用互补的减法与加法目标，抑制干扰候选并找回缺失证据，仅更新预测器，无需区域标注、响应采样或推理轨迹。精炼后的候选还支持稀疏编码，放大证据并排除背景 token。

Result: 在六个细粒度基准和四个 MLLM 主干上，Vision-RL2 在所有 token 预算下均优于基座模型，并以约 4 倍更少的视觉 token 超过基座模型最大预算下的准确率。

Conclusion: 定位与识别具有不同分辨率需求，采用粗定位加证据区域高分辨率聚焦可有效提升效率。Vision-RL2 通过区域级强化学习优化候选网络，无需区域标注即可实现更忠实、更高效的稀疏视觉编码，为细粒度 MLLM 感知提供高性价比方案。
...
TL;DR: 该论文提出STAR，一种面向扩散式光场重建的结构感知测试时自适应框架，用于从有限且含噪的焦栈测量中重建光场，并在二焦面和三焦面设置下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从有限且含噪的焦栈重建光场是高度病态逆问题；虽然给定光学系统的光场到焦栈成像几何固定，但光场的空间-角度结构（视图内空间细节、跨视图角度依赖、视图间视差）因场景而异，固定预训练先验难以最优捕捉每个测试光场的结构。

Method: STAR为每个测试光场冻结预训练扩散先验，并拟合三个轻量级适配器到观测焦栈，以联合自适应光场空间-角度结构的三个组成部分。

Result: 在二焦面和三焦面设置下，STAR均优于现有最先进方法，且推理时间短于需要测试时参数更新的方法。

Conclusion: STAR是首个从焦栈重建光场的测试时自适应框架，表明针对每个测试光场自适应调整扩散先验的结构表示可有效提升重建性能与效率。
...
TL;DR: 本文提出面向安全认知的场景否定理解任务SNUS，构建高保真否定描述数据集，并提出结构化、极性感知的CESG评估指标，以解决多模态大模型在视觉否定理解中的开放性和肯定偏差问题。


<details>
  <summary>Details</summary>
Motivation: 真正的机器智能需要超越被动像素识别，具备对缺失信息进行自上而下功能推理的能力，尤其是理解视觉否定。但现有视觉否定范式过于开放，且普遍存在的肯定偏差使多模态大模型和评估指标在否定语义下失效。

Method: 将否定推理边界锚定在安全认知这一具体认知目标上，定义SNUS任务；构建映射局部危险密集断言的高保真否定描述数据集；提出认知期望场景图（CESG）分数，作为结构接地且极性感知的评估指标。

Result: 大量实验表明，当前模型在SNUS任务上表现困难，传统评估指标在语义反转时完全崩溃；所提框架为SNUS提供了可靠基准。

Conclusion: 该框架为风险感知的情境理解和反事实认知提供了严谨基础，有助于推进具备安全认知能力的视觉否定理解研究。
...
TL;DR: 本文综述了AI智能眼镜作为可穿戴智能平台的发展，从硬件、可穿戴智能、交互设计和应用场景四个维度进行系统梳理，并指出五个跨领域研究挑战。


<details>
  <summary>Details</summary>
Motivation: 人工智能的进步正推动智能眼镜从单纯的采集与显示设备转变为可穿戴智能系统，需要在严格形态限制下实现第一人称感知与实时辅助的结合，因此需要统一框架来组织相关技术与应用。

Method: 采用综述方法，围绕四个相互关联的维度展开：硬件基础、可穿戴智能（感知与情境理解）、交互设计、以及医疗、无障碍、情境学习等应用场景，并识别五个跨领域挑战。

Result: 提出了AI智能眼镜的系统级定义，构建了统一框架来组织技术、应用与开放挑战，并明确了下一代硬件、可信第一人称智能、终身个性化记忆、主动智能和具身基础模型五大研究方向。

Conclusion: 将智能眼镜视为可穿戴智能平台，本综述为这一新兴领域提供了统一的研究框架，并指出了未来关键挑战与发展方向。
...
TL;DR: 论文关注以安全为认知约束的视觉场景负面描述任务，提出基于反事实重建与对比解码（CRCD）的框架，将图像中物理缺失的关键信息转化为语义负面事件。


<details>
  <summary>Details</summary>
Motivation: 传统场景理解聚焦图像中客观存在的肯定信息，但在安全关键领域，理解应该存在却实际缺失的信息对风险规避至关重要。现有视觉语言模型因肯定偏差、心理填补能力有限和表征偏差，难以处理缺失信息。

Method: 提出CRCD框架：将任务重构为反事实潜在变化描述以绕过肯定偏差；通过对比合成的安全期望与现实来识别语义遗漏；设计双分支反事实重建架构，其中amodal补全分支恢复缺陷物体，功能关联分支推断完全缺失的安全物体；集成多条件表征学习机制，将通用特征投影到预定义安全标准子空间；解码重建场景原型与原始输入间的特征级语义残差，约束不存在搜索空间并激活解码器负面逻辑。

Result: 大量实验验证了CRCD的有效性，为该开创性任务建立了高性能基线。

Conclusion: CRCD能够有效实现视觉场景负面描述，将物理缺失转化为语义负面事件，为安全关键领域的风险缓解提供支持。
...
TL;DR: 提出SnapPhysics，一种无需训练即可从单张图像重建3D物体并估计质量、摩擦和重心等物理属性的框架，用于混合现实中的物理一致交互。


<details>
  <summary>Details</summary>
Motivation: MR中物理一致交互不仅需要几何，还需要物体物理属性；现有方法依赖视频动态分析，计算成本高，或仅用VLM处理单图，缺乏几何基础和物体间关系。

Method: 结合实例级3D重建与空间对齐，构建物理感知场景图，将物体间关系和每个物体的度量几何编码为结构化上下文，供VLM进行物理属性推理。

Result: 在3D-FRONT上，场景级F-Score比最佳学习方法提升18.6%；在带真实质量标注的实拍场景中，mALDE最多降低20.5%，log尺度相关系数r²_ls最多提升19.6%。

Conclusion: SnapPhysics无需人工调参即可实现物理可交互的MR体验。
...
TL;DR: 提出KoUniTalk，一个以发音为中心的轻量级韩英3D说话人脸基准，通过变形迁移将VOCASET与韩语3D说话人脸数据统一到共享网格，便于跨语言训练与评测。


<details>
  <summary>Details</summary>
Motivation: 高质量3D说话人脸数据集大多以英语为中心，韩语3D面部运动数据因网格拓扑、空间尺度、坐标系和时间采样不同，难以直接与标准英语基准结合。

Method: 使用变形迁移将VOCASET和公开韩语语音3D说话人脸数据重定向到共享网格拓扑；构建身份中立的规范输出空间，聚焦嘴部及相邻中下面部区域；定义受试者不重叠的韩语基准划分；通过张嘴、嘴宽、开口率和张嘴动态等语义嘴部关键点轨迹评估重定向是否保留语音相关运动。

Result: 统一模板包含1,176个顶点，输出维度从15,069和72,147降至3,528，分别较VOCASET/FLAME和原始韩语网格减少4.27倍和20.45倍。处理后的匹配基准包含22位说话人、4,978个序列和642,781帧，可支持韩英跨数据集语音驱动3D面部动画评估。

Conclusion: KoUniTalk不提出新的变形迁移算法或完整身份保持头像数据集，而是提供一个紧凑、身份中立的统一发音模板空间，用于韩英语音驱动3D面部动画的受控训练与评测。
...
TL;DR: PACE是一种类型化表示，用于将剧本转化为精确的镜头规划，通过编译器生成扩散模型提示和3D场景，并用相机求解器保证构图精确。在自动驱动剧本和外部故事板镜头上验证了其精度和局限。


<details>
  <summary>Details</summary>
Motivation: 从剧本到电影之间存在空间规划问题：谁站在哪里、相机从何处看到什么。自由文本驱动的图像扩散模型依赖自身默认来规划镜头，缺乏对构图和调度的精确控制。

Method: 提出PACE类型化表示，包含剧本证据、角色、道具、地点、主体位置和相机运动。值在脚本、场景、镜头或面板层级写入并向下继承。编译器将其转为扩散模型提示和以米为单位的3D场景，相机求解器放置相机使声明的构图与构建的构图一致。通过逐字段测量编译相机和 staged render 与声明的偏差来评估。

Result: 在自动驱动剧本的11个场景中，单主体面板的主体位置误差在画幅宽度的1.2%以内；多主体（2-3个）时单一相机位姿无法满足所有位置，残差被报告。在204个外部导演故事板镜头上，导演文字、编译提示和灰盒控制得到的头部高度分别是 staged 目标的1.906、1.733和0.955倍；构图保持最好的条件绘制动作最少。在30个镜头上声明姿态，可使绘制动作从58.9%提升至74.4%，且不改变构图。

Conclusion: PACE能够精确控制镜头构图和主体位置，但转场、拟合运动和生成面板的人工审核仍是待解决问题。代码已开源。
...
TL;DR: 本文提出跨粒度学习本质上是层次交互调节问题，而非简单任务耦合。针对无人机感知中的跨粒度任务协作，提出了CrossUAV基准和CGSC框架，通过渐进自适应调节跨任务交互，在目标检测和实例分割上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 联合学习异构任务常被视为特征共享、蒸馏或辅助监督的任务耦合，但表征和监督粒度不匹配易导致干扰、教师偏差或单向崩溃。跨粒度学习应视为层次交互调节。无人机感知中视觉变化和检测-分割目标自然形成粗粒度和细粒度知识源，这一问题尤为突出。

Method: 引入CrossUAV，一个用于联合目标检测和实例分割的无人机基准，提供跨粒度任务协作的统一评估平台。提出跨粒度社会化协作（CGSC）框架，渐进且自适应地调节任务在网络层次中何时、何地、如何交换信息，根据任务贡献调整交互强度，抑制有害干扰并利用互补的粗/细粒度结构。

Result: 大量实验表明，两个任务均获得一致提升，验证了层次动态交互作为跨粒度协作机制的有效性。

Conclusion: 跨粒度协作可通过层次动态交互调节有效实现，CGSC在CrossUAV基准上验证了该机制。
...
TL;DR: 本文提出PART，一个统一的基于Transformer的3D部件检索与装配框架。给定目标形状和部件库，PART自动选择合适部件并预测其6-DoF位姿以重建目标。


<details>
  <summary>Details</summary>
Motivation: 现有工作多集中于装配预定义部件集，而更实用的基于检索的设定尚未被充分探索。该任务面临三大挑战：随部件库规模指数增长的组合搜索空间、不同目标所需部件数量不同导致的变长输出，以及部件装配所需的连续6-DoF位姿估计。

Method: 将检索与装配建模为集合预测问题，设计基于Transformer的框架，以变长输出检索部件并回归其位姿。同时利用部件位姿估计与目标分割之间的对偶性，通过联合训练和一种分割增强优化模块提升性能。构建了包含8万多个形状的大规模数据集。

Result: 实验结果表明，PART能够泛化到场景布局、图像目标和真实世界扫描等不同输入形式。

Conclusion: PART为基于检索的3D部件装配提供了一种统一且有效的解决方案，并展示了向多种实际应用场景推广的潜力。
...
TL;DR: 提出 BINDER 概率医学图像配准模型，基于互信息并引入潜在体素对应关系，推导闭式迭代优化与 MCMC 采样，实现非线性配准及高维多模态不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 医学图像配准，尤其多模态和高维 3D 形变场景，需要通用、鲁棒且能量化不确定性的方法；现有互信息配准缺乏高效概率推断与不确定性估计。

Method: 构建基于互信息的概率模型，假设图像间存在潜在体素级对应关系；采用空间插值技术，推导仅含闭式迭代更新的优化和 MCMC 采样；非线性配准时得到类似 demons 的高效算法；实现开源工具 BINDER。

Result: 在多种单模态和多模态配准任务中表现出稳健的开箱即用性能；首次在多模态配准中量化高维 3D 形变的不确定性；代码已公开。

Conclusion: BINDER 为通用医学图像配准提供了有效概率框架，兼具高效优化与不确定性量化能力，且代码开源可用。
...
TL;DR: 论文提出SlugTrails，一个面向大型室内空间的楼层平面图视觉定位基准，基于Aria眼镜真实自我中心感知数据，评估现有方法并发现微调可显著提升性能并改善跨数据集泛化。


<details>
  <summary>Details</summary>
Motivation: 现有楼层平面图室内视觉定位方法多在小型住宅环境开发和评估，而真实部署场景常为大型公共建筑，缺乏大规模、真实感知的基准。

Method: 构建SlugTrails基准：30Hz Aria眼镜记录，覆盖3栋校园建筑6层（22089平方米），提供带语义类和通行空间掩码的CAD楼层平面图，使用激光测量锚点将轨迹对齐到平面图坐标系。设计三种协议：单帧行走、静止多视角扫描、带里程计的行走流。评估五种代表性几何与学习系统。

Result: 官方预训练权重在SlugTrails上表现接近零（单帧行走R@1m30°最高0.004）；在SlugTrails上微调可提升所有可训练族（如F³Loc单帧0.0→0.141，序列0.03→0.66），增益随观测累积；微调权重在无LaMAR训练下提升跨数据集泛化（F³Loc序列R@1m 0.048→0.143，UnLoc 0.063→0.127）；从零训练远低于从预训练权重微调。

Conclusion: 楼层平面图定位当前受限于室内数据而非架构，SlugTrails数据集、协议和工具已开源。
...
TL;DR: 提出BinoGen自动生成大规模、具身感知的室内自中心双目视觉体验，构建超2000万标注图像数据集，并证明其可提升真实世界视觉感知及支持具身差异研究。


<details>
  <summary>Details</summary>
Motivation: 具身视觉依赖连续环境交互形成的时间连贯经验，但大规模自中心双目观测与密集标注采集成本高、难度大；同时视觉经验受观察者具身参数（视高、视场、双目几何、运动）影响，需可控生成环境与观察者变化。

Method: BinoGen联合建模环境与观察者变化，采用生成式场景合成、概率物体实例化、外观随机化、随机轨迹生成和可配置双目相机设置，生成同步双目视频及深度、光流、表面法线、语义、物体坐标、相机位姿等密集多模态监督。

Result: 构建超2000万标注图像数据集；引入BinoGen数据持续提升深度估计、目标检测和视频目标跟踪等真实世界视觉感知；同环境的人类启发与鼠类启发配对观测可受控研究具身对感知学习影响，具身特定自适应显著提升性能，联合训练使单模型在两种具身上下均有竞争力。

Conclusion: 大规模、可控的视觉经验能够改进具身感知，并为研究观察者具身差异如何影响感知学习提供了有效途径。
...
TL;DR: 提出 GS-PI，一种将 PBR 材质生成建模为 3D 点云上几何条件扩散过程的优化解耦框架，可从预训练 Gaussian Splatting 中提取可重光照的 PBR-GS 资产。


<details>
  <summary>Details</summary>
Motivation: Gaussian Splatting 虽擅长新视角合成，但将光照与几何烘焙进辐射场，难以直接接入 PBR 管线；现有逆渲染联合优化方法存在目标冲突，导致材质与光照歧义及残余光照伪影。

Method: 将 PBR 材质生成视为 3D 点云上的几何条件扩散过程，直接在 3D 域操作以保证多视角一致性；引入多尺度跨视角条件机制，融合全局语义先验、源锚定光度线索和绝对空间学习视角方向条件；从预训练 Gaussian 模型提取点云，用条件扩散预测 PBR 属性，再通过可微光栅化蒸馏回 Gaussian，得到可重光照 PBR-GS。

Result: GS-PI 优于近期逆渲染基线，将逐场景联合光照/BRDF 优化替换为一次学习式扩散加短时目标驱动蒸馏，且无需代理网格。

Conclusion: 该框架能缓解跨视角投影错位并防止高光烘焙进本征颜色，生成完全可重光照的 PBR-GS 资产，同时避免 2D 扩散方法的像素对应问题并 inherent 保证多视角一致性。
...
TL;DR: 本文提出CitySTAR，一个无需训练的城市级3D grounding框架，将城市规模3D grounding重构为结构化约束推理，并在新基准CitySTAR-3D上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有3D grounding方法多依赖特征相似度或直接匹配，难以将自然语言意图与十亿级城市点云中隐含的语义和几何结构关联，导致城市规模开放世界3D grounding困难。

Method: 提出CitySTAR：将原始十亿级城市点云提升为开放词汇3D实例的查询就绪场景图；使用CodeLLM驱动工具提供多模态证据以获取节点属性和3D空间关系；用配对超图建模目标-上下文拓扑，并进行双向拓扑验证以消歧；最后通过Reflective Cross-modal Grounding模块融合拓扑一致性和候选为中心的2D视觉证据，在度量感知3D上下文图上决策。同时构建CitySTAR-3D基准。

Result: 大量实验表明，CitySTAR在开放世界城市3D grounding上持续提升，同时保持强可解释性和泛化能力。

Conclusion: CitySTAR为城市级3D grounding提供了无需训练、可解释且泛化性强的结构化推理方案，并推动该方向基准发展。
...
TL;DR: DirtyMoCap 是一个无需固定标记布局的鲁棒光学动捕框架，将无序、稀疏、带噪的标记映射到骨骼关节与体表代理锚点，再拟合 SMPL-H 模型，恢复全身姿态、平移和体型。


<details>
  <summary>Details</summary>
Motivation: 传统光学动捕依赖严格标记布局和干净轨迹，实际系统常输出稀疏、噪声、无序且配置未知的标记点云，难以直接拟合参数化人体模型。

Method: 将无序标记观测映射到固定代理锚点（骨骼关节和体表点）作为稳定中间表示；使用循环滑窗架构初始化并长序列跟踪锚点；设计可微 Gauss-Newton 求解器将 SMPL-H 拟合到跟踪锚点；显式推导几何残差，端到端学习自适应观测置信度、平滑项和先验权重；并实现 CUDA 求解器加速。

Result: 在多样噪声标记配置上，仅用单个训练模型即可泛化到任意布局，关节和顶点重建精度持续优于配置特定 SOTA 基线；自定义 CUDA 求解器相比标准 PyTorch 实现最高加速 100 倍；应用于中国传统武术异构原始动捕数据，构建了时序连贯的 SMPL-H 功夫动作数据集。

Conclusion: DirtyMoCap 弥合了脏标记观测与参数化人体模型之间的差距，实现布局无关、鲁棒且高效的人体运动重建，并在真实复杂动捕场景中具有应用价值。
...
TL;DR: 论文提出一种即插即用的DETR蒸馏方法TPRD，通过利用教师模型不同阶段的预测信息来优化监督信号，从而提升边缘设备上DETR压缩模型的检测性能。


<details>
  <summary>Details</summary>
Motivation: DETR检测性能强但计算开销大，难以部署在边缘设备；现有DETR蒸馏方法主要关注对齐蒸馏点，却忽视了教师监督信号本身的质量。由于DETR存在阶段间非单调预测行为，早期定位或分类正确的预测可能在后期退化，部分负样本预测还会变得过度自信，导致仅依赖当前阶段预测会产生不准确且不一致的监督。

Method: 提出Teacher Prediction Refinement Distillation（TPRD），在蒸馏前利用阶段间预测信息精炼教师预测。包括：Positive Prediction Correction（PPC），从早期阶段恢复更准确的正样本预测以修正退化预测，提供可靠定位与分类信号；Negative Prediction Suppression（NPS），抑制过度自信负样本的误导性监督；Maximum Dark Knowledge Preservation（MDKP），选择性精炼目标类logits，同时保留非目标类关系以维持暗知识。

Result: 在MS COCO和PASCAL VOC上的大量实验表明，所提方法有效且鲁棒。

Conclusion: 通过提升教师监督质量而非仅对齐蒸馏点，TPRD能够改善DETR蒸馏效果，是一种有效且通用的即插即用模块，代码已开源。
...
TL;DR: 本文提出LAMP，一种面向结肠镜息肉图像合成的病灶引导自适应黏膜上下文传播框架，通过显式区分病灶、有效黏膜与镜头外部区域，提升合成图像真实感并改善下游分割性能。


<details>
  <summary>Details</summary>
Motivation: 结肠镜标注稀缺，合成图像与掩码对可缓解该问题；但现有前景引导方法将非前景像素统一视为背景且主要依赖局部整合，直接用于结肠镜会导致非黏膜黑色区域污染生成组织，以及黏膜纹理和光照不一致。

Method: LAMP利用视场（FOV）掩码显式分离病灶、有效黏膜和镜头外部区域；通过病灶到黏膜的交叉注意力为有效黏膜位置提取病灶外观条件，并使用FOV约束的多方向Vision Receptance Weighted Key Value在合法组织支撑上传播这些条件；最后通过自适应门控将其残差融合到扩散U-Net中。

Result: 在五个息肉数据集上的大量实验表明，LAMP在整体生成质量上显著优于现有方法，并持续提升五个下游分割模型的性能。

Conclusion: LAMP能有效解决结肠镜息肉合成中的背景污染与局部推理不一致问题，生成更高质量的息肉图像并促进下游分割任务，代码将在 https://github.com/wangtong627/LAMP 发布。
...
TL;DR: 提出SCARF——一种实时速度不变的事件表示，保留原始事件并处理快速运动、静止场景和独立运动物体，在计算效率与表示质量上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 事件相机适合实时视觉任务，但异步独立像素电路带来算法挑战；现有速度不变表示虽能缓解慢动作观测缺失和快动作运动模糊，却多将事件转为图像状表示而丢失时间信息。

Method: 提出Set of Centre Active Receptive Fields (SCARF)，一种实时速度不变表示，基于中心活跃感受野集合，保留原始事件，并一致处理快速运动、静止场景及独立运动物体。

Result: SCARF在计算效率和表示质量两方面均取得当前最优性能。

Conclusion: SCARF能够保留原始事件的时空信息并兼顾多种运动场景，为事件相机实时视觉提供高效且高质量的表示。
...
TL;DR: QCPruner 是一种无需训练的多模态大模型视觉 token 剪枝方法，通过双边查询条件化效用加权，在固定预算下同时衡量视觉目标与候选代表，保持查询相关证据并减少冗余。


<details>
  <summary>Details</summary>
Motivation: MLLM 中视觉 token 负载高，固定预算下的免训练剪枝必须保留查询相关信息并避免冗余；现有方法在排序、多样化或覆盖优化时，未使用共享的逐视觉查询效用同时加权视觉目标和候选代表。

Method: 利用关键词匹配的查询锚点，融合两种跨模态线索形成效用，并将其应用于基于视觉亲和力的覆盖中的视觉目标和候选代表；所得非负设施选址目标单调且子模，保留标准 (1-1/e) 贪心保证，且无需模型训练或参数更新。

Result: 在 LLaVA-1.5、LLaVA-NeXT、LLaVA-Video 和 Qwen2.5-VL 上，QCPruner 在每个报告 token 预算下取得评估的完整系统剪枝方法中最高的平均相对性能。LLaVA-1.5-7B 在 576 个 token 中保留 32 个时，保留未剪枝性能的 96.1%，最强基线为 93.9%；Qwen2.5-VL-7B 在 1296 个 token 中保留 256 个时，对应为 96.7% 和 92.5%。

Conclusion: QCPruner 通过查询条件化的双边效用加权和子模贪心剪枝，在无需训练的情况下实现高效视觉 token 压缩，并在多个 MLLM 上优于现有完整系统剪枝基线。
...
### [138] [GRF-Recon: Global Ray-Field Optimization for Long-Sequence Feed-forward Reconstruction](https://arxiv.org/abs/2609.20012)
*Enpeng Li,Yunzhou Zhang,Zhiyao Zhang,Dexuan Lyu,Chenyu Wang,Chiyuan Cui,Cheng Cheng*

Main category: cs.CV

TL;DR: 本文提出一种面向长单目序列的稳定、可扩展前馈3D重建统一框架，通过轨迹对齐、几何先验注入与射线场优化，缓解显存、局部几何退化和长期轨迹漂移问题。


<details>
  <summary>Details</summary>
Motivation: 前馈3D重建虽高效，但扩展到大规模单目场景时受限于GPU显存过高、局部几何质量下降和长期轨迹漂移；现有分块优化方法几何约束有限，难以在长轨迹上保持全局一致性。

Method: 构建由粗到细的轨迹对齐框架，并结合轻量几何先验注入；通过LoRA适配将单目几何线索蒸馏到前馈骨干中，在保持推理效率的同时提升细结构深度精度；提出混合权重稀疏射线场优化，利用高频几何特征引导局部点云细化并施加一致的帧间射线约束；最后采用联合射线误差优化的轨迹拼接策略显式减少累积漂移。

Result: 大量实验表明，该方法在轨迹精度上可与代表性SLAM系统竞争，同时在大规模场景中保持全局一致的3D重建。

Conclusion: 该统一框架能够实现稳定且可扩展的长单目序列前馈3D重建，在精度、效率和全局一致性之间取得良好平衡。
...
### [139] [AVTrace: Diagnosing Audio-Visual Temporal Reasoning in Omni Models](https://arxiv.org/abs/2609.19991)
*Longyin Zhang,Parth Sakhare Mahendra,Chengwei Wei,Ning Zhang,Lim Ming Chong,Sirui He,Ai Ti Aw*

Main category: cs.CV

TL;DR: 论文提出 AVTrace 音视频时序推理评估套件，覆盖事件定位、同步判断、链解析等任务，评测五个开放全模态模型，发现它们普遍表现不佳，并验证参数高效时序后训练的有效性。


<details>
  <summary>Details</summary>
Motivation: 全模态模型能描述视频内容，但尚不清楚它们能否在时间上定位事件、保持事件顺序并判断音视频同步。

Method: 构建 AVTrace 诊断套件，包含 onset/span grounding、同步验证、下一步预测、跨模态定位、链解析和事件条件理解等任务，含 34,114 个训练样本及 3,500/7,000 的开发/测试集；采用参考盲响应归一化与确定性评分评估五个开放模型，并进行扰动消融和参数高效时序后训练实验。

Result: 五个模型在同步验证上均低于多数标签基线 0.556，链解析和事件条件定位/理解得分低；Qwen3-Omni-30B 对模态移除和视觉输入处理变化表现出任务相关敏感性；时序后训练提升 Gemma4-E4B-it 多项指标；外部图像基准指标小幅变化（部分下降），教师强制困惑度降低。

Conclusion: 语义参考文本重叠不应作为时序定位的代理指标；AVTrace 能识别任务特定弱点，并为时序后训练提供测试平台。
...
### [140] [A Free Lunch? Adapting PP-OCRv6 for Historical Text Recognition](https://arxiv.org/abs/2609.20064)
*Benjamin Kiessling*

Main category: cs.CV

TL;DR: 该论文评估了轻量级无循环文本识别器 PP-OCRv6 在历史手写文本识别（ATR）中的表现，并与传统 CRNN 及基于 Qwen3.5 的大型视觉语言模型 Medusa 进行对比。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽得分高，但因计算成本高、依赖大规模预训练和幻觉问题，在历史 ATR 中实用受限；历史 ATR 仍主要依赖紧凑的 CRNN 识别器。轻量级无循环识别器有望兼顾高精度与实用性，但尚未在历史书写上被全面评估。

Method: 将 PP-OCRv6 适配于历史行识别，并与传统 CRNN 在通用预训练、领域特定训练、语料级微调及手稿特定少样本适应四种设置下对比，涵盖多语言拉丁字母和阿拉伯字母材料；同时与基于 Qwen3.5 的 Medusa 识别器进行比较。

Result: 从头训练时 PP-OCRv6 未持续优于 CRNN 基线；但异构预训练带来显著更好的泛化性能；微调后的 PP-OCRv6 可超越针对历史拉丁字母 HTR 定制的大型视觉语言模型 Medusa。

Conclusion: 轻量级无循环识别器经异构预训练和微调后，能在历史 ATR 中达到甚至超越大型 VLM 的效果，为历史 ATR 提供了一种实用且高效的替代方案。
...
### [141] [Astronex-World 1.0: Real-Time Interactive World Model Foundation](https://arxiv.org/abs/2609.20034)
*Xin Zhou,Cong Miao*

Main category: cs.CV

TL;DR: Astronex-World 1.0 是一个开放可控的视频世界模型基础模型，支持文本或图像输入，并在相机轨迹、连续动作和具身标识条件下预测未来视觉状态，还可插入文本事件。


<details>
  <summary>Details</summary>
Motivation: 研究动机是构建可控制的视频世界模型，使其能根据相机运动、动作和具身智能体信息生成未来视频，并支持持久生成与实时流式应用，同时为具身智能和自动驾驶预留接口。

Method: 基于 Wan2.2-TI2V-5B 先验，提供双向全上下文生成模型和带块因果注意力与跨块 KV 缓存的因果模型；使用 PRoPE 注入相机内参和外参，64 维动作流调制每个 Transformer 层；采用五阶段训练，包括双向相机与动作控制、转为块因果生成、少步学生蒸馏、混合域动态恢复，以及非对称 DMD/DMD2 分布匹配。

Result: 因果模型可生成 832x480、24 fps 视频；五个训练阶段均在两张 NVIDIA L20 48 GB GPU 上完成，因果模型可在单卡实时流式生成。WBench Navi 得分 73.5，WBench Full 得分 70.0；在 Full 上，该 5B 模型优于 13.6B LongCat-Video 和 14B Helios，接近 22B LTX-2.3 且差距不到 1 分，并优于同 5B 先验后训练的 YUME 1.5。

Conclusion: Astronex-World 1.0 是一个高效、可控、可流式生成的开放视频世界模型基础模型，具备较强基准表现；预留的动作输入输出接口可支持具身智能与自动驾驶的后训练扩展。
...
### [142] [G^2RA-NET: Graph-based Cross-Slice Relation Modeling with Attention Gating for Medical Image Segmentation](https://arxiv.org/abs/2609.20088)
*Shengye Wang,Zonglin Wu,Liang Fan,Yule Xue,Haozhe Zhao*

Main category: cs.CV

TL;DR: 本文提出G^2RA-Net，一种结合图基跨切片关系建模与注意力门控的医学图像分割框架，用于提升各向异性体积图像的分割一致性与准确性。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法在局部特征和体积上下文建模上有所改进，但仍难以高效建模各向异性体积图像中的跨切片关系，限制了分割一致性和准确性。

Method: 提出G^2RA-Net框架，包含图基切片关系建模（GSRM）和跨切片注意力门（CSAG）。GSRM将每个切片表示为图节点，通过图消息传递传播语义上下文以捕获相邻切片间的解剖依赖；CSAG选择相关邻域上下文，并通过注意力引导的特征调制强调目标解剖区域。

Result: 在脑部MRI和腹部CT数据集上的实验表明，G^2RA-Net在分割准确性和边界质量上优于代表性方法，消融研究进一步验证了所提设计的有效性。

Conclusion: G^2RA-Net通过结合图基跨切片关系建模与注意力门控，有效解决了各向异性体积图像中跨切片关系建模的难题，提升了医学图像分割性能。
...
TL;DR: 该论文针对事件相机微小无人机检测中远距离目标事件稀疏、碎片化且易受杂波与自运动干扰的问题，提出基于序列化运动证据累积的轻量级事件级状态空间框架 PointEvent，在保持高分辨率目标响应的同时抑制干扰，实现当前最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法多依赖密集事件表示或局部稀疏时空建模，前者计算冗余，后者难以跨远距离异步事件建模运动连续性，导致微小目标运动证据碎片化。

Method: 提出序列化运动证据累积，将同一事件流组织为保持局部性的时空路径和保持时序的时序路径；基于此设计 PointEvent 轻量级事件级状态空间框架，通过交替互补顺序的序列化扫描逐步聚合碎片化运动证据，并引入高分辨率事件分支保留细粒度目标响应，结合紧凑上下文调制抑制干扰。

Result: 实验表明 PointEvent 在对比方法中取得 SOTA 性能，同时具有最少参数量和最快实测推理速度。

Conclusion: 通过序列化运动证据累积与事件级状态空间建模，PointEvent 能有效应对远距离微小无人机检测中的稀疏、碎片化事件问题，兼顾精度与效率。
...
### [144] [A Smaller Transformer in Your Transformer](https://arxiv.org/abs/2609.20100)
*Dhananjay Tomar,Marius Aasan,Andreas Kleppe,Adín Ramírez Rivera*

Main category: cs.CV

TL;DR: 论文提出Transformer-Within-Transformer（TWT），通过将连续冗余层融合为可学习的替代层，减少Vision Transformer的参数与推理计算，并在自然图像和病理学任务中保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 近期发现Vision Transformer存在局部相似的计算阶段和深度冗余，但现有利用该冗余的方法要么无法降低推理计算，要么严重损害模型表达能力。

Method: 形式化统一的块冗余视角，将几何结构与具体替代干预解耦；提出TWT后处理方法，把连续冗余层融合为单个可学习的替代层。

Result: 在自然图像上，TWT用一半深度即可与原模型竞争；在若干病理学下游任务中，TWT匹配甚至优于原始基线，同时减少参数数量和推理计算。

Conclusion: TWT能有效利用ViT深度冗余，实现参数与推理计算压缩，并在保持或提升模型性能方面具有潜力。
...
### [145] [AnyviewMeter: Adapting Robotic Reward Models with Camera Geometry and Multi-View Attention](https://arxiv.org/abs/2609.20106)
*Yuang Tu,Runjia Tan,Yujie Yan,Jinghan Hu,Chen Lv*

Main category: cs.CV

TL;DR: 论文提出 AnyviewMeter，一种面向机器人奖励模型的几何条件自适应框架，通过引入相机几何与多视角同步注意力来提升任务进度奖励预测的视角鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 机器人奖励模型依赖视觉观测评估任务执行，但其预测会随相机视角和遮挡变化而改变，即使任务状态未变；因此将预训练奖励模型适配到具体任务时，需要考虑任务是如何被观测的。

Method: AnyviewMeter 将低秩微调与 token 对齐的 Plucker 射线及同步块注意力结合：射线条件将相机几何注入视觉特征以及注意力的 query 和 key，块注意力在预训练解码器内融合同步多视角。框架基于预训练 Robometer 进行参数高效适配，支持单视角奖励预测和联合多视角评估。

Result: 在 PickCube 上，单视角适配在每个相机组中均改善进度预测，并在视场变化下相比 RGB 微调降低约 21% 的平均绝对误差。在模拟操作任务中，联合多视角预测相比平均单视角 RGB 预测降低 41-69% 的进度误差，并在约 88% 的任务-相机组中改善时间排序。在固定相机和腕部相机的真实任务中，平均绝对误差相比平均 RGB 微调降低约 21%。

Conclusion: 结果表明，相机几何和联合视觉证据是任务特定机器人奖励适配中的有效组成部分。
...
### [146] [Cross-Modal Attention Acts as a Frequency Filter: Why Verbose Prompts Improve Robustness in Vision-Language Models](https://arxiv.org/abs/2609.20139)
...
TL;DR: 论文发现视觉语言模型在图像损坏下的鲁棒性受问题措辞显著影响：冗长问题增强鲁棒性，细粒度或语义复杂问题使模型更脆弱；作者用问题条件跨模态注意力形成的频谱滤波解释该现象，并证明填充提示可提升准确率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在图像损坏下表现脆弱，而问题措辞如何影响其鲁棒性尚不明确，因此需要揭示其机制并寻找简单有效的改进方法。

Method: 作者分析冗长问题与细粒度问题对VLM的相反影响，提出问题条件跨模态注意力会在图像块上诱导频谱滤波：冗长问题拓宽频率支持，细粒度问题将支持集中于更少视觉尺度。并在Qwen3-VL和LLaVA-OneVision上，使用GQA和CLEVR数据集，测试冗长改写、细粒度问题与损坏空间频率的关系，以及填充提示的效果。

Result: 冗长问题如“请仔细看并回答：有猫吗？”可显著提升鲁棒性；语义复杂或细粒度问题如“椅子左边的杯子是什么颜色？”则使模型更脆弱。当频谱滤波器与损坏处于相同空间频率时，答案漂移最大。在8B模型上，冗长改写使漂移方差降低70–81%。填充提示在图像损坏下也带来可测的准确率增益。

Conclusion: 问题措辞通过跨模态注意力的频谱滤波影响VLM对图像损坏的敏感性，频谱匹配程度决定答案漂移；简单填充提示是一种实际有效的鲁棒性提升方法。
...
### [147] [Bridging Modalities on the Cortex: Surface-based MRI to PET Translation with a Diffusion Bridge](https://arxiv.org/abs/2609.20147)
*Yitong Li,Alexandra Samoylova,Fabian Bongratz,Timo Grimmer,Dennis M. Hedderich,Igor Yakushev,Christian Wachinger*

Main category: cs.CV

TL;DR: 该论文提出一种基于皮层的扩散桥框架 DB-SUiT，用于 MRI 到 FDG-PET 的跨模态合成，以支持痴呆诊断。方法在皮层流形上原生运行，并在两个数据集上验证了高保真合成、自动分类提升及泛化能力。


<details>
  <summary>Details</summary>
Motivation: FDG-PET 是痴呆诊断的高敏感生物标志物，但成本高、有辐射且可及性有限。现有 MRI 到 PET 的体数据生成方法未显式考虑高度折叠的皮层几何，而疾病相关模式主要位于皮层上。

Method: 提出表面基扩散桥框架 DB-SUiT，在皮层流形上进行 MRI-to-PET 翻译。设计条件球面 U 形视觉 Transformer（SUiT），结合球面卷积编码器进行多尺度表面特征提取，利用瓶颈 Transformer 捕捉长程空间依赖，并引入人口统计学和皮层下条件以优化合成，同时保持表面拓扑。

Result: 在两个数据集（包含不同类型痴呆受试者）上，DB-SUiT 合成质量显著优于基线。自动痴呆分类中，合成 PET 表面比 MRI 提升 14.2%，比 PET 体积提升 11.3%，接近真实 PET 表面性能。盲法读者研究中，合成 PET 诊断准确率为 85.5%，MRI 为 75.8%，真实 PET 为 95.2%。模型还在未重新训练的情况下，在包含训练中未出现痴呆亚型的外部队列上表现出跨队列和跨病理泛化。

Conclusion: 表面原生 MRI-to-PET 合成方法 DB-SUiT 能生成高保真 PET，改善痴呆自动分类与临床阅读诊断表现，并具备跨队列、跨病理泛化潜力。代码已开源。
...
### [148] [Task-Oriented Semantic Feature Transmission for Multi-Task Satellite Remote Sensing over Low-SNR Channels](https://arxiv.org/abs/2609.20150)
*Shuoyuan Sun,Hongyu Wang,Mugen Peng,Wenjia Xu*

Main category: cs.CV

TL;DR: 本文提出一种面向任务的卫星遥感传输框架，跳过图像重建，直接传输多任务预训练骨干提取的语义特征。在AWGN信道下，场景分类和目标检测任务均优于重建导向的JSCC基线，低SNR时提升最显著。


<details>
  <summary>Details</summary>
Motivation: 传统卫星遥感传输遵循“先重建后推断”范式，优化像素级保真度，与分类、检测等下游任务目标不匹配，尤其在低信噪比条件下问题突出。

Method: 设计任务导向框架，绕过图像重建直接传输语义特征。采用多任务预训练骨干提取特征，引入轻量级通道自适应模块（CAM）压缩特征维度以降低带宽，并用特征恢复器在信道损坏后恢复任务相关结构。冻结骨干，联合优化CAM和任务特定下游头，在随机SNR训练下施加任务级和特征级监督。

Result: 在AWGN设置下，场景分类和目标检测实验表明，该方法在不同SNR条件下均持续优于重建导向的JSCC基线，且低SNR区域改进最大。

Conclusion: 绕过图像重建、直接传输语义特征的任务导向框架能有效提升下游任务性能，尤其适用于低信噪比卫星遥感传输场景。
...
### [149] [Ischemic Stroke Segmentation and Net Water Uptake Quantification on Multicenter Non-Contrast CT Using Supervised Target-Domain Adaptation](https://arxiv.org/abs/2609.20151)
*Linus Britt,Maximilian Nielsen,Susan Klapproth,Andre Kemmling,Michael H. Lev,Gabriel Broocks,Rene Werner,Thilo Sentker*

Main category: cs.CV

TL;DR: 该研究开发并外部测试了一种领域感知深度学习框架，用于非增强CT（NCCT）上的缺血性卒中病灶分割，并评估其量化净水摄取（NWU）的可行性。


<details>
  <summary>Details</summary>
Motivation: NCCT上梗死低密度和NWU的定量评估通常需要手动或半手动勾画病灶，且常依赖CT灌注或DWI MRI，限制临床推广；而NCCT自动分割在异质多中心数据中仍具挑战。

Method: 回顾性多中心研究，纳入4个数据集共801例患者。基于nnU-Net训练NCCT分割模型，并在Boston（n=11）和ISLES（n=75）目标域子集上微调，在未参与微调的保留病例上评估。将自动分割及NWU值与专家参考进行比较。

Result: 对于≥30 mL病灶，中位Dice为0.68（Boston）和0.56（ISLES）。纳入较小病灶后，急性病灶分割中位Dice为0.54（IQR 0.30-0.70，Boston）；与治疗后梗死相比，NCCT病灶分割中位Dice为0.20（IQR 0.03-0.41，ISLES）。自动NWU平均绝对误差为1.37个百分点（SD 1.61，Boston）。

Conclusion: 目标域自适应支持在异质外部队列中实现仅基于NCCT的梗死分割，但性能因域而异。该方法无需高级影像即可从基线NCCT实现低误差NWU量化，支持进一步前瞻性临床评估。
...
### [150] [Needles in a Raystack: Ultra-Sparse LiDAR Occupancy Detection for Bat Tracks](https://arxiv.org/abs/2609.20160)
*Nico Klar,Pankaj Rana,Nizam Gifary,Jakob Traub,Aamir Ahmad*

Main category: cs.CV

TL;DR: 该论文研究利用LiDAR监测夜间蝙蝠飞行，将问题定义为传感器中心LiDAR射线栈中的体素级占用检测，并提出轻量3D U-Net来恢复稀疏前景飞行轨迹。


<details>
  <summary>Details</summary>
Motivation: 夜间物种如蝙蝠难以实地观测，而LiDAR记录的蝙蝠运动形成超稀疏3D时空数据，标准重建损失容易只预测背景并漏掉真实飞行路径，因此需要有效的占用检测方法。

Method: 提出轻量3D U-Net，保持时间分辨率，利用跳跃连接保留空间细节，并结合加权二元交叉熵与Dice损失处理强类别不平衡；在真实LiDAR蝙蝠记录中与声学监测交叉验证，并与基于重建的3D卷积自编码器基线比较。

Result: 基于重建的3D卷积自编码器基线无法恢复前景轨迹；所提U-Net在诊断实验中能恢复稀疏前景占用，并沿蝙蝠飞行轨迹产生连贯的占用模式。
...
### [151] [MoSSGate: Memory-Modulated State-Space Gating for Skin Lesion Segmentation](https://arxiv.org/abs/2609.20181)
*Anum Awan,Mahnoor Buriro,Muhammad Younas Khan,Md Imam Ahasan*

Main category: cs.CV

TL;DR: 本文提出MoSSGate模块，用于U-Net皮肤病变分割，通过边界感知空间门控、外部记忆调制器和并行2D状态空间建模，在低计算量下实现高精度分割。


<details>
  <summary>Details</summary>
Motivation: 现有卷积和Transformer模型难以在有限计算预算下同时捕捉长距离空间依赖和精细边界细节，导致过分割、分割碎片化或遗漏细小边缘结构。

Method: 提出即插即用的MoSSGate模块，集成边界感知空间门控（限制长距离传播到信息区域）、外部记忆调制器（提供样本自适应动态控制）和并行2D状态空间建模（线性复杂度高效聚合全局上下文）。

Result: 在ISIC 2017和ISIC 2018基准上分别达到86.3%和85.9% mIoU、92.6%和90.6% Dice，且FLOPs显著低于多数CNN方法。

Conclusion: MoSSGate在高分辨率医学图像分割中实现了优越的精度-效率权衡，具有实际应用价值。
...
TL;DR: 提出MTF-Net多模态时序特征融合网络，融合边界框动态、人体姿态关键点、局部上下文和场景语义，用于行人过街意图预测，在PIE和JAAD上取得高AUC并保持实时性。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么只建模单模态内部的时序依赖，要么仅在粗粒度语义层面融合多模态信息，难以在自动驾驶与行人交互中准确、及时地预测行人意图。

Method: MTF-Net在递归融合框架中联合建模运动学、外观和上下文线索，使用门控线性单元（GLU）自适应调节跨模态信息流，并设计三个时序编码分支和注意力引导融合头，提前数帧预测行人过街意图。

Result: 在PIE和JAAD基准上分别达到最高0.95 AUC和0.94 AUC，优于近期基于Transformer和图模型的方案，同时保持实时性能。

Conclusion: 可靠的行人意图预测主要来自有原则的多模态融合，而非过度复杂的网络架构。
...
### [153] [Scene-Q: Confidence-Aware Coarse-to-Fine Querying of 3D Scenes with Selective VLM Reasoning](https://arxiv.org/abs/2609.20235)
*Juno Kim,Yesol Park,Hye-Jung Yoon,Byoung-Tak Zhang*

Main category: cs.CV

TL;DR: 提出Scene-Q，一种置信度感知的粗到细查询框架，用于室内移动机器人的开放词汇3D场景理解，结合快速检索与选择性VLM推理。


<details>
  <summary>Details</summary>
Motivation: 现有系统多依赖对比图像-文本编码器的余弦相似度检索，效率高但在标签近义或多个相似实例时表现脆弱，需要更鲁棒的开放词汇3D场景理解。

Method: 使用温度缩放归一化编码器分数，高置信度查询直接快速检索，仅对低置信度查询调用推理VLM，并在小规模top-K候选集上利用原始多视图图像和实例边界框进行重排序。

Result: 在ScanNet200上提升开放词汇3D实例分割，在真实重建上提升自然语言3D实例检索，空间和关系查询增益最大，同时大量查询仍走快速路径。

Conclusion: Scene-Q能以低成本实现上下文感知消歧，提升开放词汇3D理解，并保持较高效率。
...
TL;DR: 白内障是视力丧失的主要原因，但白内障眼底图像增强面临配对图像有限和细节恢复不足的挑战。本文提出了一种两阶段多尺度注意力网络（TSMSA-Net）用于弱监督白内障眼底图像增强。该方法通过域变换合成配对的白内障图像，并利用多尺度注意力阶段提取细节、减少伪影。在Kaggle和ODIR-5K数据集上，TSMSA-Net优于现有方法，且具有泛化能力，并能提升血管分割和分类性能。


<details>
  <summary>Details</summary>
Motivation: 白内障导致视力下降并阻碍进一步诊断，但白内障眼底图像增强存在配对白内障视网膜图像有限以及视网膜图像细节恢复不足的问题。

Method: 提出了两阶段多尺度注意力网络（TSMSA-Net）用于弱监督白内障眼底图像增强。利用域变换合成配对的真实感白内障图像，解决配对图像获取困难的问题；提出多尺度注意力阶段，学习更有用的特征以增强白内障图像，并减少伪影。

Result: 在Kaggle和ODIR-5K数据集上的实验表明，即使没有配对图像，TSMSA-Net也优于当前最先进的白内障眼底图像增强方法，并表现出一定的泛化能力。增强还能提高白内障图像中血管分割和分类的性能。

Conclusion: TSMSA-Net能有效增强白内障眼底图像，解决配对图像缺乏和细节恢复问题，并在多个数据集上优于现有方法，具有泛化能力，且能提升后续任务性能。
...
### [155] [SAGE-Yoga: Multi-Cue Learning for Yoga Pose Classification and Joint-Level Correction](https://arxiv.org/abs/2609.20245)
*Hung Le Chi,Khanh Minh Huynh,Long Nghia Tran Pham,Tan Phuc Huynh,Trong-Thuan Nguyen,Minh-Triet Tran*

Main category: cs.CV

TL;DR: 提出 SAGE-Yoga，一个从单张 RGB 图像进行瑜伽姿态分类与关节级纠正的统一粗到细框架，结合视觉集成与选择性几何验证。


<details>
  <summary>Details</summary>
Motivation: 现有方法常依赖单一视觉预测，难以区分视觉相似姿态，且将姿态分类与纠正视为独立任务，因此需要同时实现准确分类与可解释反馈。

Method: 采用基于 bagging 的互补视觉骨干集成生成候选姿态类别；用基于 margin 的门控机制在高置信时保留视觉预测，模糊时调用几何验证；确定类别后检索 medoid 参考姿态，比较关节角度与类别特定分布以识别错位关节，并转化为纠正反馈。

Result: 在 Yoga-82 数据集上，视觉集成达到 89.0% Top-1 准确率，完整框架提升至 90.7% Top-1 准确率和 90.1% Macro-F1。

Conclusion: 结合互补视觉证据与选择性几何验证可提升细粒度姿态分类性能，并实现可解释的关节级纠正。
...
### [156] [Distance to Class Prototypes: Active Learning for Object Detection](https://arxiv.org/abs/2609.20248)
*Licheng Zhang,Zheng Gong*

Main category: cs.CV

TL;DR: 该论文提出一种用于目标检测主动学习的单次前向传播评分信号：在训练目标中加入监督对比项，使每个目标的嵌入空间中距离编码类别归属，并用预测类别区域的偏离程度加权置信度来给未标注检测打分。


<details>
  <summary>Details</summary>
Motivation: 在新场景部署深度目标检测器的主要限制不是架构，而是标注成本。主动学习通过选择要标注的图像来降低成本，但选择质量取决于未标注图像的评分信号。常用类别后验概率便宜但校准差，而多模型或多次随机前向的不一致性更好但推理成本高，需在整个未标注池上多次前向。

Method: 在检测器训练中加入监督对比损失，塑造逐目标嵌入空间，使距离反映类别成员关系。对未标注检测，依据其嵌入与预测类别所在区域的距离进行评分，并用检测置信度加权。该方法只需单网络单次前向，无需集成、辅助预测器或重复推理；额外参数仅2.89M，相比裸检测器增加8.3%。

Result: 在PASCAL VOC和MS-COCO上，该方法在每一轮选择中都优于同一检测器的后验概率准则，最高提升1.08% mAP50，而运行间波动仅0.02%至0.18%。其表现与需要每个未标注图像3到50次前向的集成和Monte Carlo dropout准则相当。实验使用单阶段检测器，与对比准则报告结果所用检测器一致，从而隔离选择决策与检测器强度的影响。

Conclusion: 基于单次前向对比嵌入的评分准则能有效且高效地用于目标检测主动学习，在显著降低推理成本的同时优于廉价后验准则，并可与高成本集成或MC dropout方法竞争。
...
### [157] [Generative Verification: Rethinking the Uncertainty Signal for Active Learning of Object Detection](https://arxiv.org/abs/2609.20262)
*Licheng Zhang,Zheng Gong*

Main category: cs.CV

TL;DR: 提出生成式验证的主动目标检测采集函数：用独立生成模型从预测框内像素重新推导标签，以与检测器预测的不一致作为采集信号。


<details>
  <summary>Details</summary>
Motivation: 现有主动目标检测的采集函数大多让被改进的检测器自我评估，且需手工加权分类与定位项；希望用独立验证器统一捕捉多种失败模式，并优先标注自信的错误检测。

Method: 训练条件扩散模型作为验证器，扩散目标是标签表示而非图像；反向过程随机生成标签分布，以分布集中度衡量证据确定性；将生成标签与检测器预测标签的不一致作为采集信号。

Result: 在PASCAL VOC和MS-COCO上，该信号优于输出不确定性、特征几何、扰动和集成等准则；MS-COCO上每轮约提升1 mAP50，早期轮次（自信检测错误最常见时）优势最大。

Conclusion: 生成式验证通过独立生成模型与检测器解耦，自然融合分类与定位失败模式，且对自信错误更敏感，是有效的主动目标检测采集策略。
...
### [158] [CleanVideo: Adaptive Concept Erasure for Text-to-Video Diffusion Models](https://arxiv.org/abs/2609.20267)
*Junchi Liao,Hongji Li,Wenrui Zhou,Lijie Hu*

Main category: cs.CV

TL;DR: CleanVideo是一个用于视频扩散模型的选择性概念擦除框架，通过三模态门控机制进行低维子空间干预，有效擦除目标概念并保持视觉保真度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 概念擦除旨在从预训练生成模型中消除不良视觉语义而不影响通用性。将概念擦除从图像扩展到视频并非易事，因为目标概念逐渐出现且在不同帧和去噪步骤中变化，固定干预可能遗漏目标或引入模糊、抖动和内容失真。

Method: 提出CleanVideo框架，采用低维子空间干预，并由三模态门控机制控制。该机制联合处理时空视觉特征、时间步信号和文本语义，决定在何处、何时以及是否干预，将擦除内容导向自然替代概念（若可明确定义），同时保留非目标内容。

Result: 在三个视频扩散模型上的实验表明，CleanVideo能有效擦除目标概念，同时保持视觉保真度和时间一致性，在帧级和视频级评估以及概念恢复攻击下（当受保护流程保持完整时）均优于现有基线。

Conclusion: CleanVideo是一种有效的视频概念擦除方法，能够选择性擦除目标概念而不损害模型通用性，并具有鲁棒性。
...
TL;DR: 本文提出一种轻量级全帧检测器，用于检测部分篡改的AI生成视频，可在边缘设备部署。通过将DINOv2-Base教师模型蒸馏到冻结的MobileNetV3-Small学生模型，结合温度退火软标签、注意力多样性正则化、帧级监督和残差特征适配器，并针对场景切换误报和阈值校准问题设计训练策略。在55,393个样本的拼接测试集上，学生模型以150.4 MB检查点和3.65 ms/16帧的速度，达到AUC 0.766，缩小了与教师模型58%的差距。


<details>
  <summary>Details</summary>
Motivation: 生成视频模型的普及使检测威胁从完全伪造转向部分篡改，而现有高精度检测器依赖400M+参数的基础骨干网络，资源占用大，难以在边缘设备部署，且通常需要人脸检测预处理。

Method: 采用知识蒸馏：将DINOv2-Base教师蒸馏到冻结的MobileNetV3-Small学生，使用温度退火软标签迁移、注意力多样性正则化、帧级监督和残差特征适配器（将ImageNet特征适配于伪影检测）。针对部分篡改场景的两个失败模式：合法场景切换的误报通过视频内时间硬负样本解决；纯真实类主导的阈值校准错误通过校准感知采样解决。

Result: 在包含55,393个样本、伪造帧比例6.2%至31.2%的拼接测试集上，学生模型AUC为0.766，缩小了与DINOv2-Base教师模型58%的性能差距；在RTX A4000上处理16帧片段耗时3.65 ms，检查点大小150.4 MB，满足边缘设备内存和延迟预算。

Conclusion: 该轻量级全帧检测器无需人脸检测预处理，在边缘硬件上实现了对部分篡改AI生成视频的有效检测，在性能和效率之间取得了良好平衡。
...
### [160] [TinyCNN: A 193K-Parameter Network for On-Device Plant Disease Detection, with a Cross-Dataset Robustness Diagnosis](https://arxiv.org/abs/2609.20290)
*Ngoc-Bao Ho-Lam,Thai-Anh Nguyen*

Main category: cs.CV

TL;DR: 本文提出轻量级卷积神经网络 TinyCNN，用于移动端植物病害分类，在 PlantVillage 上达到高精度并显著压缩模型规模，但跨数据集到 PlantDoc 时鲁棒性明显下降。


<details>
  <summary>Details</summary>
Motivation: 作物病害早期检测对可持续农业和粮食安全很重要，尤其在缺乏专家诊断但移动设备普及的资源受限地区，需要低能耗、低成本、可端侧部署的模型。

Method: 采用深度可分离卷积块构建 TinyCNN，仅 193,190 个可训练参数，224×224 输入下为 110.05M MACs；在 38 类 PlantVillage 上评估，并分析 vanilla 知识蒸馏，对 alpha∈{0.3,0.5,0.7} 和 T∈{2,4} 做消融；最后进行 PlantVillage 到 PlantDoc 的跨数据集测试和 Grad-CAM 分析。

Result: TinyCNN 在 PlantVillage 上达到 98.88% 测试准确率和 98.03% macro-F1，比 ResNet18 小约 58 倍，比 MobileNetV2 教师小 11.8 倍；蒸馏选择 alpha=0.3、T=4，得到 98.81% 准确率；跨数据集到 PlantDoc 显示显著鲁棒性差距，Grad-CAM 归因于背景驱动的离叶注意力，符合捷径学习。

Conclusion: TinyCNN 是面向可持续农业智能的节能、可部署基础模型，但真实田间鲁棒性仍是实现持久实际影响的关键障碍。
...
TL;DR: 论文研究冻结DETR类分割模型的输出选择瓶颈：有用的掩码已存在于查询候选中，但部署的选择规则未将其暴露。作者提出HYDRA小选择器，仅基于缓存的冻结输出训练，推理时与保留基线比较，并用校准边际决定是否替换，从而在不增加查询、不生成新掩码、不重跑骨干、不更新权重的情况下恢复隐藏性能。


<details>
  <summary>Details</summary>
Motivation: 现代分割器常在昂贵计算完成后失败：模型查询条件候选中已存在有用掩码，但部署的选择规则没有暴露它。作者希望探究能否在不增加查询、不生成新掩码、不重跑骨干、不更新权重的前提下，恢复这部分已被计算但未被暴露的隐藏性能。

Method: 先用仅用真值评估的oracle显示已有掩码提议中存在大量隐藏余量。提出HYDRA：在冻结模型缓存输出上训练的小型选择器，推理时对缓存候选与显式keep-baseline选项打分，仅当留出数据校准的margin表明所选候选足够更好时才采取行动。在Mask2Former、MaskDINO、OneFormer与SAM 3上评估，并进行配对LoRA对照和TinyDETR受控研究。

Result: HYDRA在ADE20k和COCO上最多提升Mask2Former、MaskDINO、OneFormer +7.41 dataset mIoU；SAM 3在八个域平均提升+9.4 class-macro prompt-IoU，并通过校准保留有用预测。配对LoRA对照显示轻量权重适应不能消除瓶颈：暴露预测常持平或更差，而对适应后候选进行路由仍能恢复精度。

Conclusion: 冻结分割器不仅应按其暴露的掩码评价，也应按其抑制的有用候选评价；该输出选择瓶颈与二分匹配下的查询专业化有关。
...
### [162] [Not All Layers Are Equal: Dynamic Layer Routing for Reliable CLIP OOD Detection](https://arxiv.org/abs/2609.20299)
*Ignacio M. De la Jara,Cristian Rodriguez-Opazo,Damith Ranasinghe*

Main category: cs.CV

TL;DR: 本文研究CLIP中用于OOD检测的层选择问题，提出可学习路由方法Voyager，通过轻量路由器为每个输入选择稀疏的、以最后一层为锚的专家层组合，从而提升OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 已有工作表明跨模型层进行信息聚合能改进OOD检测，但多数方法关注如何融合层。本文转而研究层选择本身是否可作为可学习问题，即为输入决定应信任哪些层。

Method: 提出一种可泛化的弱OOD上下文构造监督信号，并据此学习一个轻量路由器，在CLIP的层深度上选择稀疏且以最终层为锚的专家组合，用于OOD检测。

Result: 在三个不同基准上，Voyager均提升OOD检测。ImageNet-1K上平均FPR@95为18.86，比最强同类提示学习方法优8.8个点。增益在多种监督来源下保持，表明关键优势来自可学习路由器而非监督来源。路由器学习约2分钟、内存小于1GB，比当前提示学习方法高效约20倍。

Conclusion: 层选择可以作为可学习路由问题，且可学习路由器是性能提升的关键。Voyager兼具有效性与实用性，说明弱OOD监督上下文很有效，但真正优势来自路由机制。
...
### [163] [AgriScope: Pixel-Grounded Multimodal Understanding for Agricultural Images](https://arxiv.org/abs/2609.20325)
*Abderrahmene Boudiaf,Mohamad Alanssari,Irfan Hussain,Sajid Javed*

Main category: cs.CV

TL;DR: 本文提出AgriScope，一个统一的像素级多模态农业图像理解框架，支持图像级、区域级和像素级理解，并构建了大规模数据集AgriGround（含50万+图像、1100万指令样本），在多个农业视觉-语言任务上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 农业图像理解需要在复杂真实条件下对植物病害、害虫、作物结构和植物物种进行细粒度识别。现有MLLMs仅能输出文本，缺乏像素级视觉定位能力，无法满足农业场景的精细需求。

Method: 提出AgriScope框架，集成生物语义编码、密集空间表示和像素解码，统一支持图像级、区域级和像素级理解。同时构建AgriGround数据集，通过多阶段自动标注流水线（多模态描述生成、短语级定位、分割掩码生成、任务导向指令合成）生成密集定位监督。

Result: 在多个农业视觉-语言任务（如定位描述生成、指代分割、多轮多模态交互）上的大量实验表明，AgriScope在像素级多模态理解方面有效，并建立了农业视觉-语言学习和视觉定位的强基准。

Conclusion: AgriScope为农业视觉-语言学习与视觉定位提供了统一框架和基准，数据集和代码将公开于https://github.com/boudiafA/AgriScope。
...
### [164] [FreqDINO++: A Frequency-Guided Multi-Task Routing Vision Foundation Model for Universal Ultrasound Analysis](https://arxiv.org/abs/2609.20340)
*Qing Xu,Yixuan Zhang,Yue Li,Xiangjian He,Qian Zhang,Mainul Haque,Rong Qu,Wenting Duan,Jieyun Bai,Zhen Chen*

Main category: cs.CV

TL;DR: 提出FreqDINO++，一种频率引导的多任务路由视觉基础模型，用于通用超声分析，在27类临床任务中优于强基线及近期基础模型。


<details>
  <summary>Details</summary>
Motivation: 超声图像分析需联合处理病灶分割与良恶性分类等任务，但视觉基础模型存在与自然图像的域差距；现有方法通常针对单任务微调重型编码器，计算开销大且忽略异构任务间的共性。

Method: 设计多任务路由适配器（MR-Adapter）以参数高效整合任务通用与任务特定知识；提出频率感知特征增强器（F²-Enhancer）捕获超声图像多尺度频率特征；构建任务对齐协同解码器（TC-Decoder），通过全局-局部token交互促进稠密与全局预测任务协作。

Result: 在大规模多任务及外部单任务超声基准上，FreqDINO++在27种临床任务场景中持续优于强基线和近期基础模型，并对未见数据展现良好泛化能力。

Conclusion: FreqDINO++能有效实现通用超声分析，兼顾多任务协作、参数效率与泛化性，代码已开源。
...
### [165] [Fast Cross-Strength Multi-Contrast Brain MRI Translation using Latent Bridge Matching](https://arxiv.org/abs/2609.20341)
*Siddharth Srivastava,Till Bretschneider*

Main category: cs.CV

TL;DR: 提出基于条件潜在桥匹配的统一条件模型，用于可控的MRI场强间合成，在MRIxFields2026挑战赛三个任务中取得有竞争力的结果，并实现快速单步推理。


<details>
  <summary>Details</summary>
Motivation: 不同场强下获取的MRI在噪声、分辨率、均匀性和对比度上差异显著，限制了采集设置间的可比性，并使下游分析复杂化。

Method: 基于条件潜在桥匹配框架，构建统一的条件模型，无需任务特定架构或训练，支持可控的场强到场强合成，并采用单步推理生成。

Result: 在MRIxFields2026挑战赛验证阶段，单一模型在全部三个任务上均取得极具竞争力的结果；单张NVIDIA A5000 GPU上，30个轴向切片的全部模态和场强组合生成耗时不到90秒，全卷跨模态-场强转换耗时不到70秒；并进行了广泛的消融实验。

Conclusion: 该统一条件模型无需任务特定设计即可有效处理多任务MRI场强合成，生成速度快，为跨场强MRI比较与分析提供了可行方案。
...
### [166] [MM-Future: Multi-Mode Joint World-Action Modeling for Autonomous Driving](https://arxiv.org/abs/2609.20377)
*Shuai Liu,Hechangle Gong,Hao Jiang,Runlin He,Junxiang Zhan,Kai Huang,Sheng Yang,Shaoqing Ren*

Main category: cs.CV

TL;DR: 提出MM-Future世界-动作模型，通过多模态配对的场景-动作假设及双向交互，建模自动驾驶中决策与场景演化的耦合不确定性。在NAVSIM和HUGSIM上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶涉及多模态不确定性下决策与场景演化的耦合，需要同时建模两者及其交互。

Method: MM-Future生成多个成对的场景-动作假设，每个假设由结构化动作先验和独立未来场景源初始化，通过模态感知扩散Transformer共同演化；压缩多视角视频为规划导向的MM-Tokens；使用未来条件提议评分器对轨迹候选排序。

Result: 在NAVSIM navtest上达到94.0 PDMS和91.5 EPDMS，在HUGSIM零样本闭环评估中HD-Score为32.3；消融实验表明优于单模态和仅动作变体。

Conclusion: 多模态联合世界-动作建模能有效提升自动驾驶性能，验证了其收益。
...
TL;DR: 提出EliGSiR，一种用于连续RGB-D建图的增量高斯泼溅方法，通过自适应视图调度、监督保真度和几何增长来优化建图预算，在多个数据集上优于基线。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅假设封闭观测集和长优化过程；连续RGB-D建图中新观测在线到达，需保留已重建区域，因此必须控制优化预算。

Method: EliGSiR包含三个机制：地图引导视图调度（过滤冗余视图，根据地图状态重考虑保留视图）；负载自适应保真度（根据当前建图负载调整监督分辨率）；目标几何增长（分离深度监督与高斯创建，仅在重复观测表明缺失或错位结构处增加几何容量）。这些机制自适应调整优化哪些视图、使用多少图像细节、表示在何处增长。

Result: 在Replica、TUM RGB-D、ScanNet++和真实RGB-D序列上评估。在TUM RGB-D fr3/long_office_household上，使用相同GT位姿，EliGSiR达到21.52 dB，SplaTAM为19.42 dB；使用ORB-SLAM3实时位姿，EliGSiR达到23.02 dB，耗时155.5秒，CaRtGS为20.10 dB，耗时230.9秒。还评估了采集过程中的重建，表明自适应机制改善了建图预算使用。

Conclusion: EliGSiR通过自适应机制有效利用建图预算，在连续RGB-D建图中实现更高质量和效率的重建。
...
### [168] [Compact Vision Models for Iris Presentation Attack Detection under Presentation Attack Instrument Shift and Environmental Degradation](https://arxiv.org/abs/2609.20386)
*Athanasios Angelakis,Marta Gomez-Barrero*

Main category: cs.CV

TL;DR: 该文在LivDet-Iris 2017的Notre Dame子集上，对三个参数量不超过约26万的轻量级从零训练模型进行虹膜活体检测基准测试，评估其在攻击类型未知和图像退化下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 虹膜呈现攻击检测在安全关键场景中，开发阶段表现可靠的子系统一旦遇到验证数据中未出现的攻击工具或采集条件，性能可能大幅下降，因此需要评估轻量模型在未知攻击和退化条件下的真实鲁棒性。

Method: 选取三个紧凑的从零训练视觉模型（参数≤0.26M），不使用外部预训练和数据增强，在LivDet-Iris 2017的Notre Dame子集上以五个随机种子训练；将验证集选定的阈值原样迁移到已知攻击、未知攻击、退化图像及混合测试分区进行评测。
...
Conclusion: 尽管最佳轻量模型在对比中占优，但其绝对错误率仍然很高，说明这种相对优势并不代表在未知呈现攻击下已具备部署就绪能力。
...
TL;DR: TouchSight 是一个仅使用单目第一视角视觉的框架，可从手套训练数据中预测密集全手接触力，并借助生成式视频将手套数据转换为裸手视觉数据，从而在无需接触式触觉仪器的情况下恢复触觉信号。


<details>
  <summary>Details</summary>
Motivation: 触觉信号对理解物理交互和实现灵巧机器人操作至关重要，但触觉测量需要在接触界面直接采集，导致大规模数据收集依赖侵入式、昂贵且受限的仪器。

Method: 提出 TouchSight，利用 500 小时压力手套记录和大量手-物交互数据，以单目第一视角视觉预测密集全手接触力；并构建 TwinTouch-20H，将 20 小时手套视频通过生成式视频模型重渲染为裸手视觉数据，同时保留原始触觉标签以缩小外观差距。

Result: TouchSight 能同时从手套视频和生成的裸手视频中预测密集力，在 OakInk2 上优于先前接触预测方法，并能定性泛化到未见数据集中的自然裸手第一视角视频；随着手套监督数据规模增加，性能持续提升。

Conclusion: 研究表明，密集触觉信号可以仅从第一视角视觉中恢复，而无需在采集时使用触觉仪器。
...
TL;DR: 提出WeVisDoc两阶段数据中心框架，实现鲁棒的端到端文档解析，并在多个基准上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 文档解析需适应多样版式和采集条件，但训练语料偏向常见文档类型和干净数字页面，且单纯扩大数据覆盖无法指明如何修复解析器的剩余弱点。

Method: 两阶段数据中心框架。阶段一通过异构数据和结构保持的退化合成，拓宽语义、结构、外观覆盖；阶段二利用留出探针在固定视觉-结构聚类中测量阶段一解析器的残余误差，指导针对性数据构建与目标token预算重分配。

Result: WeVisDoc-4B在OmniDocBench v1.6上Overall达95.38，在三个PureDocBench赛道平均Overall为75.54，四个设置中均排名第一。阶段二相比阶段一提升了2B和4B模型在两个基准上的Overall分数，退化PureDocBench赛道增益更大，其中4B在Real Degraded赛道提升4.03分。

Conclusion: 诊断驱动的两阶段数据中心方法能有效提升端到端文档解析的鲁棒性，其针对性数据构建与预算重分配优于单纯扩大数据覆盖。
...
### [171] [SenseFuse: Label-Free Fusion of Image and Shape Encoders for Open-Vocabulary 3D Instance Segmentation](https://arxiv.org/abs/2609.20475)
*Euiseok Han,Tri Ton,Hwanhee Kim,Seungyeon Ryu,Chang D. Yoo*

Main category: cs.CV

TL;DR: 提出 SenseFuse，一种无需标签的融合方法，通过平衡 2D 图像与 3D 形状编码器来改进开放词汇 3D 实例分割中的掩码标注阶段。


<details>
  <summary>Details</summary>
Motivation: 开放词汇 3D 实例分割对机器人空间推理和物体操作至关重要，但现有先进方法在掩码标注时主要依赖 2D 图像特征或图像蒸馏表示，未充分利用 3D 形状信息。

Method: SenseFuse 仅精炼现有流程的掩码标注阶段，发现 2D 图像编码器与 3D 形状编码器的失败模式高度不互补且很少犯相同错误，而两个 2D 编码器常重复相同错误。因此设计自适应机制，基于单个场景未标注候选框在毫秒级内估计无标签敏感度度量，并选择场景级融合权重。

Result: 在 ScanNet200、Replica 和 ScanNet++ 上，SenseFuse 在所有评估设置中均提升标注准确率，恢复 oracle 权重可获得增益的 67%–100%（中位数 93%），并在 22 个报告设置中的 21 个上提升实例 AP。

Conclusion: 2D 图像与 3D 形状编码器具有内在互补性，无需标签即可有效融合二者，从而稳健提升开放词汇 3D 实例分割性能。
...
### [172] [When Do Language-Grounded Explanations Help? A Graph-Bottleneck for Farm Monitoring Interpretable Sheep Facial Pain](https://arxiv.org/abs/2609.20427)
*Alam Noor,Miguel Guti'errez Gait'an*

Main category: cs.CV

TL;DR: 该论文研究绵羊面部疼痛自动识别中的可解释性可信问题：发现基于SPFES文本注意的解释并不可靠，转而提出仅读取SPFES概念分数的概念瓶颈模型，以少量性能损失换取可验证的概念学习，并提供七种方法的协议匹配基准。


<details>
  <summary>Details</summary>
Motivation: 自动面部疼痛识别有助于绵羊福利连续评估，但从业者需要可信解释才能依据分数采取行动；因此必须检验模型解释是否真正对应SPFES临床描述符，而非仅表面合理。

Method: 先让每个检测到的面部区域对SPFES临床描述符的文本嵌入进行注意力学习，并通过消融描述符和注意力一致性测试解释有效性；随后移除外观旁路，构建概念瓶颈分类器，使其只读取SPFES概念分数，并用图像级流程通常丢弃的逐区域状态标注进行监督；最后在临床不平衡条件下进行交叉验证和七种方法的协议匹配基准比较。

Result: 原解释无效：消融整个描述符仅使预测logit变化约1e-4，最受关注线索与预测疼痛等级仅32.6%区域一致，尽管注意力图、学习门控和生成文本均显示相反。概念瓶颈模型使Cohen's κ下降0.05--0.10，但概念确实被学到：少数疼痛指示状态以3.5--8.3倍基率被恢复，耳和眼严重度排序无需严重度监督即可出现。仅移除监督时κ不变，但概念准确率降至0.109。还表明汇总概念准确率在临床不平衡下具有误导性。

Conclusion: 架构必要性不等于语义有效性；可解释性必须通过概念监督和验证来保证。概念瓶颈可在可接受的性能损失下提供更可信的疼痛概念，并为该数据集提供协议匹配的基准。
...
TL;DR: 该研究将3亿参数的Prithvi-EO-2.0洪水分割基础模型蒸馏为70万参数的EfficientViT-B0学生模型，利用教师模型监督无标签Sentinel-2影像扩充训练集，并量化为INT8 TensorRT引擎，实现在Jetson Xavier NX上的边缘部署。


<details>
  <summary>Details</summary>
Motivation: 地理空间基础模型洪水分割性能强，但参数量大，难以部署在内存受限的边缘硬件上。

Method: 以在252个手工标注Sen1Floods11训练场景上微调的Prithvi-EO-2.0（300M）为教师，蒸馏到EfficientViT-B0（0.7M）学生；教师监督额外无标签Sentinel-2影像以扩大训练集；在252场景匹配预算下与直接训练对比，并设几何匹配对照；将监督池扩至2500场景；经激活替换和量化感知训练转为INT8 TensorRT引擎；与固定MNDWI阈值比较。

Result: 252场景下教师监督训练与直接训练竞争，在STURM-Flood上提升，几何匹配对照表明标签来源本身不能解释差异；2500场景下浮点学生在Sen1Floods11测试集水IoU达0.787（教师0.822），在STURM-Flood上匹配教师，在WorldFloods-v2上低于教师；量化后模型为1.5MB INT8 TensorRT引擎，Jetson Xavier NX上512x512图像GPU计算5.57ms，运行时设备内存约14MB；固定MNDWI阈值在两个干净外部基准上与两模型相当，故这些基准应视为泛化测试而非学习模型优越性证据。

Conclusion: 基础模型监督可将固定手工标注预算放大为更大训练集，并产生紧凑、可部署的边缘模型。
...
TL;DR: 该论文提出了GPUB基准和UniPro模型，用于电商直播中产品理解与相关时刻定位的联合任务。


<details>
  <summary>Details</summary>
Motivation: 现有产品理解基准孤立评估产品检索和时间定位，未评估产品身份与时间证据之间的关键对应关系，而电商直播中产品信息分散带来挑战。

Method: 构建GPUB基准，包含3000个直播实例、多时刻时序标注和超3.1万时尚产品；定义主任务GPrU及两个子任务；提出UniPro统一模型，从共享多模态编码中学习产品对齐和时序结构化表示。

Result: 现有模型在GPrU上表现差，最佳基线仅10.13% Pair mAP@.3；UniPro将Pair mAP@.3提升至21.53%，并达到37.23% Joint R@1@.3。

Conclusion: GPrU任务仍具挑战性，UniPro有效缩小性能差距，表明统一建模产品身份与时间定位的重要性。
...
TL;DR: 本文提出一种AI辅助的金匠标记检索流程，结合标记定位与度量学习微调，比较ResNet-50、ViT-S/16和DINOv2 ViT-S/14三种骨干网络及不同裁剪策略，最佳配置达到mAP 62.63%和Top-1 73.74%，并公开数据集、代码和网页系统。


<details>
  <summary>Details</summary>
Motivation: 艺术史研究中，金匠标记对文物识别和断代至关重要，但专家需人工将查询标记与数百个已记录样本逐一比对，过程繁琐且高度依赖专业知识。

Method: 构建AI辅助检索流程，将标记定位与度量学习微调结合，在ImageNet预训练ResNet-50、监督ViT-S/16和自监督DINOv2 ViT-S/14三种骨干上系统评估无裁剪、人工真值裁剪和基于学习检测的裁剪策略及其交互影响。

Result: 最强配置为DINOv2 ViT-S/14结合人工裁剪与度量学习微调，取得mAP 62.63%和Top-1准确率73.74%。实验表明自监督预训练和标记定位影响最大，学习式裁剪无需推理时真值标注即可恢复人工裁剪的大部分增益。

Conclusion: 自监督预训练与标记定位是提升检索性能的关键因素，学习式裁剪具有实用替代价值。为促进数字人文领域的可复现与应用，作者公开人工标注数据集和代码库，并通过公共网页界面部署系统。
...
### [176] [DocAttriBench: Benchmarking Answer Grounding in Document Visual Question Answering](https://arxiv.org/abs/2609.20574)
*Luca De Grandis,Silvia Cappelletti,William Raccagni,Marcella Cornia,Lorenzo Baraldi,Rita Cucchiara*

Main category: cs.CV

TL;DR: 提出 DocAttriBench (DAB) 大规模文档 VQA 元素级溯源基准，并利用 MAPPET 自动构建标注，评估多模态大模型的答案准确性与归因定位能力。


<details>
  <summary>Details</summary>
Motivation: 文档视觉问答中的答案 grounding 仍是开放挑战：现有基准缺少 grounding 标注或标注质量有限，而人工构建带 grounding 的数据集成本高昂。

Method: 提出 MAPPET（基于掩码的困惑度归因方法），结合文档布局与语言建模，通过掩码候选布局元素并测量困惑度增量，将答案归因于对模型置信度贡献最大的元素；将其应用于多个现有 Document VQA 数据集以构建 DAB。

Result: DAB 包含 237k 文档和 296k 问答对，并提供元素级 grounding；对具备 grounding 能力的多模态 LLM 进行基准测试，评估答案准确率、归因准确率和整体答案质量，发现更大模型通常答案准确率更高，但即使最强模型也常无法定位支持元素。

Conclusion: DAB 为开发 grounded、可验证且可信的 Document VQA 模型提供了可扩展的基准。
...
### [177] [A Dual-Stream Regulated Reconstruction and Segmentation Network with Hierarchical Artifact-Prior Modeling for Ultra-Low-Field Pediatric Neuroimaging](https://arxiv.org/abs/2609.20562)
*Bahram Jafrasteh,Leo Milecki,Qingyu Zhao*

Main category: cs.CV

TL;DR: 面向0.064 T超低场儿童MRI，提出一个统一框架，在同一推理流程中同时完成质量评估、图像增强和多结构分割。


<details>
  <summary>Details</summary>
Motivation: 超低场儿童MRI存在低信噪比、解剖边界弱和伪影频繁等问题，限制了自动化质量评估、增强与多结构分割；同时密集标注数据稀缺。

Method: 基于双耦合流3D U-Net：先重建增强后的uLF体积，再融合原始与增强图像进行皮层下分割。引入由全脑掩膜得到的目标结构外脑组织辅助类以提升边界稳定性。使用以伪影图为条件的网络，从重建残差和冻结分割特征预测7种伪影评分。通过图谱到目标的微分同胚配准进行标签传播，并正则化解剖重建。

Result: 在LISA 2026 Challenge中报告了全部三项任务的验证结果。

Conclusion: 该统一框架可联合处理超低场儿童MRI的质量评估、增强和分割；辅助类与图谱配准策略有助于应对边界不稳定和密集标注稀缺问题。
...
TL;DR: 本文提出首个在线高斯SLAM框架，可直接在单次曝光的16位线性HDR图像上进行跟踪与建图，并能原生渲染线性场景辐射。


<details>
  <summary>Details</summary>
Motivation: 现有稠密视觉SLAM几乎只依赖8位色调映射的LDR输入，在极端光照下易出现跟踪漂移与建图崩溃；而现有RAW/HDR重建流水线均为离线，依赖SfM预处理，无法应对大帧间运动。

Method: 方法包含三个核心组件：与架构无关的HDR高斯泼溅模块，采用无MLP的对数参数化高斯颜色特征；Reinhard范围压缩的光度目标函数；结构引导的空间梯度加权。同时发布了RawSLAM数据集，包含10个真实室内序列的16位RAW图像、对齐深度、IMU和外部的OptiTrack位姿。

Result: 在轨迹与重建精度上优于直接HDR适配的MonoGS；在标准8位输入下误差约减半；HDR高斯模块可无缝迁移至SplaTAM、Gaussian SLAM和DROID-W，消除了这些系统在困难光照序列上的全部跟踪失败。

Conclusion: 该工作实现了首个在线HDR高斯SLAM，能够原生输出线性场景辐射以支持后渲染处理，且公式在8位输入下同样适用，具有良好通用性；代码与数据集将公开。
...
TL;DR: 针对单视图前馈3D高斯泼溅（3DGS）生成中每条相机射线固定数量高斯导致的空间冗余问题，提出一种与骨干网络无关的后处理压缩流程，结合重要性评分剪枝与轻量循环精炼模块，在冻结基础模型的前提下实现高内存压缩并保持新视角渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有单视图前馈3DGS生成方法为每条相机射线预测固定数量的高斯，造成严重空间冗余；而现有多数压缩策略面向多视图场景，依赖跨视图一致性，无法兼容单图像模型。重新训练基础前馈网络直接输出紧凑表示成本高，因此需要一种后处理式的压缩方案。

Method: 保持基础前馈模型冻结，采用后处理剪枝与循环精炼。具体提出一种骨干无关的压缩流水线：首先基于重要性评分对生成的高斯进行剪枝，然后使用可训练的轻量级循环精炼模块，迭代更新存活的高斯基元以恢复图像质量。

Result: 该方法能与现有基线无缝集成，在保持新视角渲染保真度的同时实现高内存压缩，并支持推理时灵活调整保留比例以满足不同应用需求。

Conclusion: 通过冻结基础模型并施加后处理剪枝与循环精炼，可有效压缩单视图前馈3DGS生成的高斯表示，兼顾渲染质量与内存效率，且具备良好的通用性和灵活性。
...
### [180] [FunArt: Decoding Functional Structure and Articulation from Generative 3D Latents](https://arxiv.org/abs/2609.20673)
*Dennis Rotondi,Abdelrhman Werby,Kai O. Arras*

Main category: cs.CV

TL;DR: FunArt 提出一种从单次静态位姿RGB-D观测中构建关节感知功能3D场景图的框架，可联合分割可动部件与功能交互元素并估计运动学参数。


<details>
  <summary>Details</summary>
Motivation: 现有铰接场景表示多依赖观测到的交互，或在静态扫描中割裂关节结构与功能交互元素；机器人需要在交互前就能识别可动部件与功能区域。

Method: 重建物体实例，将融合几何转为TRELLIS.2的O-Voxel表示，利用其冻结稀疏压缩VAE作为结构先验；轻量查询解码器结合物体级隐变量与表面密集特征，联合分割可动部件和功能元素，并估计运动类型、轴、原点与范围。

Result: 在Articulate3D上，无论是否输入真值物体，均可动部件分割、关节估计和功能元素分割均达SOTA；端到端设置下较最强基线分别提升1.5 AP50（可动部件）、2.8 AP50（关节原点与轴约束）、6.7 AP50（功能元素）。

Conclusion: 生成式3D隐变量编码了可操作的结构线索，可在物理交互前初始化机器人感知与规划。
...
### [181] [Earth Surface Immune System for Rapid Monitoring of Unknown Anomalies](https://arxiv.org/abs/2609.20662)
*Jingtao Li,Qian Zhu,Xinyu Wang,Deren Li,Liangpei Zhang,Yanfei Zhong*

Main category: cs.CV

TL;DR: 该论文提出ESIA地球表面免疫系统，用于检测和识别地球表面异常，通过模拟生物免疫系统的非特异性、特异性和变异机制，实现未知异常的定位、开放词汇识别与快速场景适应。


<details>
  <summary>Details</summary>
Motivation: 气候变化和人类活动导致地球表面异常在频率和多样性上不断增加，但异常数据有限且难以预测，与传统遥感目标不同。现有方法多针对特定异常类别或仅做定位，难以从检测走向可操作信息。

Method: ESIA受生物免疫系统三原则约束：非特异性先天免疫阶段将异常视为时序卫星影像中的未观测变化，生成二值定位图；特异性适应性免疫阶段用负选择过滤文本提示，并用多模态基础模型匹配定位图像块，实现开放词汇识别；变异机制在测试时微调少量嵌入，利用单对参考图像快速适应新场景。

Result: ESIA在覆盖19,801.60平方公里、六类异常的全球数据集上验证，并与22个模型比较。先天免疫阶段定位速度达14.51平方公里每秒，F1比最强通用基线高37%；适应性免疫阶段识别F1超过80%；变异机制仅用单对参考图像即可在3.26秒内适应每个场景。还应用于量化Kakhovka大坝倒塌后Dnipro三角洲退化农田，以及评估2025年洛杉矶Palisades火灾烧毁严重度。

Conclusion: ESIA能灵活处理未知异常，为实时灾害响应和环境监测开辟了新途径。
...
### [182] [Refinement Is Inherently Editable: Training-Free Prompt-to-Prompt Image Editing with Generative Refinement Network](https://arxiv.org/abs/2609.20633)
*Yulong Chen,Ziqian Zhang,Haoyu Zhang,Ao He,Senmao Li,Kai Wang*

Main category: cs.CV

TL;DR: 提出RefineEdit，一种无需训练的prompt-to-prompt图像编辑框架，基于生成细化网络，通过耦合编辑定位与内容生成，在PIE-Bench九个类别上取得最佳背景保持和CLIP分数。


<details>
  <summary>Details</summary>
Motivation: 文本引导图像编辑需引入请求的修改同时保留无关源内容。基于扩散的编辑器依赖空间控制，其不准确会导致编辑不完整或改变无关区域；因果自回归编辑器则受固定解码顺序限制，难以修正早期决策。

Method: RefineEdit无需训练，基于生成细化网络。从源图像中间状态初始化编辑分支，复用涌现布局；比较两个分支对同一源采样比特的概率，用带符号差异选择可编辑位置和比特；选中比特进行编辑细化，其余复制演化中的源状态；采用自适应空间冻结限制掩码扩张，有限比特锁定保持近期选中比特可编辑。无需额外训练、外部掩码或注意力控制。

Result: 在PIE-Bench九个编辑类别上，RefineEdit在PSNR、LPIPS、MSE、SSIM上取得最佳背景保持分数，并在所有评估方法中获得最高的整图和编辑区域CLIP分数。

Conclusion: RefineEdit通过全局细化二进制图像码，无需训练、掩码或注意力控制，能有效实现文本引导图像编辑，在保持背景的同时完成编辑，性能优于现有方法。
...
### [183] [PROVIA: Procedure State Tracking for Online Mistake Detection in Egocentric Videos](https://arxiv.org/abs/2609.20638)
...
TL;DR: 论文提出 PROVIA，一个用于第一视角视频的在线错误检测方法：它分离维护「事实状态」（已执行步骤的学习式摘要，含错误）与「已接受进度」（由正确演示经贝叶斯状态合并得到的自动机状态后验及执行状态），并在完整试验（错误与恢复自然发生）上以固定误报预算进行评估，在四个数据集上优于仅依赖时序的基线，运行速度 58-70 FPS。


<details>
  <summary>Details</summary>
Motivation: 现有在线方法采用的「首次错误协议」会在每个视频的第一个错误处截断，导致一个从不看视频、仅按固定时间触发的规则在所有样本上都正确，从而使评估失去意义。错误会改变工作实际状态，因此后续步骤必须对照已发生的操作而非原始计划来解读，这要求用包含错误与恢复的完整试验来评估在线检测器。

Method: PROVIA 将两类记录分开维护：一是事实状态，即对每位执行者所完成步骤（含错误）的学习式摘要；二是已接受进度，即从正确演示中通过贝叶斯状态合并诱导出的自动机的状态后验，以及每位执行者的执行状态。过程状态转移只发生在正确状态分支中，而错误分支与纠正分支保留源状态。随后用一个序贯检验把逐帧错误概率转换为报警。

Result: 在 CaptainCook4D、IndustReal、HoloAssist 和 IMPACT-ego 上，PROVIA 在受控基线中错误检测排序最优。在每分钟 0.1 次误报的验证预算下，CaptainCook4D 召回率 .154 对 .128，HoloAssist 为 .034 对 .015，且在 HoloAssist 的每个预算下均领先。流水线运行速度为 58-70 帧/秒。

Conclusion: 通过将事实状态与已接受进度分离，并用单一滤波器与统一优化规则进行序贯检测，PROVIA 能在完整试验中有效、实时地检测并跟踪错误与恢复，优于仅依赖时序的控制方法；源代码已公开于 https://github.com/Kratos-Wen/PROVIA。
...
### [184] [FlowSGS: Improving Flow Matching Priors for Inverse Imaging with Stochastic Interpolants](https://arxiv.org/abs/2609.20769)
*Tianao Li,Xinhui Qian,Emma Alexander*

Main category: cs.CV

TL;DR: 提出 FlowSGS，一种基于流匹配和 Split Gibbs Sampling 的后验采样方法，用于求解计算成像逆问题。它将后验分解为似然步与先验步，并在多种逆问题上取得 SOTA 性能，且首次用于非线性傅里叶相位恢复。


<details>
  <summary>Details</summary>
Motivation: 现有基于流的逆问题求解器通常假设线性前向模型，或在后验采样中引入简化近似，限制了其适用性和准确性。

Method: 使用 Split Gibbs Sampling 将后验分解为似然步和先验步；似然步采用 Langevin dynamics 采样；先验步基于 Stochastic Interpolants 框架整合预训练流模型，并利用其反向时间 SDE，提出时间步校正技术以减少网络评估次数。

Result: 在一系列逆问题上达到当前最优性能；相比 plug-and-play 扩散采样器，先验步所需网络评估更少；首次在非线性逆问题（傅里叶相位恢复）上验证基于流的逆求解器。

Conclusion: FlowSGS 有效克服了现有基于流的逆求解器对线性前向模型和简化后验采样的依赖，减少了计算开销，并将流匹配后验采样成功扩展到非线性逆问题。
...
### [185] [Paint-Anything: Unified Any-Color Control for Image Generation and Editing](https://arxiv.org/abs/2609.20816)
*Ji Xie,Dewei Zhou,Xinyu Huang,Zhennan Chen,Xun Wang*

Main category: cs.CV

TL;DR: 提出Paint-Anything，通过共享的十六进制颜色提示接口，实现图像生成与编辑中对象级的任意颜色控制。


<details>
  <summary>Details</summary>
Motivation: 专业设计需要以任意24位十六进制值精确指定目标颜色；现有颜色生成、编辑与上色方法常依赖专用颜色表示或复杂推理流程，而大语言模型可自然关联十六进制值与颜色语义，提供了更简单的起点。

Method: 构建Paint-500K数据管道：基于真实图像进行对象定位、感知颜色标注与编辑对合成；针对阴影导致标签近似的问题，引入像素与十六进制值精确匹配的纯色锚点，仅在高噪声时间步使用；提出Any Color Benchmark（ACBench），含ACBench-T2I和ACBench-Edit，衡量对象级十六进制颜色保真度。

Result: 在FLUX.2-4B上，Paint-Anything使ACBench-T2I和ACBench-Edit分数分别较基模型提升85.3%和28.3%；消融实验支持该训练配方；在对比方法中取得最高平均CompColor分数。

Conclusion: Paint-Anything以共享十六进制提示接口有效实现对象级任意颜色的生成与编辑，所提出的数据与训练策略对颜色保真度提升显著。
...
### [186] [FAMOS: Feed-Forward 3D Articulation Modeling from Sparse Observations](https://arxiv.org/abs/2609.20817)
*Kevin Qu,Tao Sun,Massimiliano Viola,Liyuan Zhu,Zhizhuo Zhou,Sayan Deb Sarkar,Konrad Schindler,Iro Armeni*

Main category: cs.CV

TL;DR: 提出FAMOS，一种从稀疏、无序的部分点云集合中前馈预测可动部件分割和关节参数的模型，支持单视图或多视图输入。


<details>
  <summary>Details</summary>
Motivation: 从稀疏单目视图建模铰接物体很困难，因为每个观测只提供部分几何和运动信息；现有前馈方法多基于单观测并依赖类别级形状先验，限制其泛化与精度。

Method: 设计多状态铰接Transformer，通过交替的状态级注意力和全局注意力聚合多观测中的铰接线索；提出观测铰接跨度目标，监督各部件在输入观测中的运动范围；引入程序化数据生成器，在训练中合成自标注资产。

Result: 在PartNet-Mobility、ACD和ArtiCraft-10K数据集上，FAMOS持续优于前馈方法和基于优化的基线。

Conclusion: 联合推理多个观测并利用观测铰接跨度监督与程序化数据生成，能有效提升稀疏视图下铰接物体的可动部件分割和关节参数估计，且自然支持可变数量输入。
...
### [187] [Should This Case Be Adapted? Prediction Fragmentation Controls Test-Time Adaptation](https://arxiv.org/abs/2609.20700)
*Lili Wang,Jing Li,Xiaowen Sun,Xiangyu Hu,Zhuangzhuang Gu,Jian Liu,Srihari Nelakuditi,Yan Tong*

Main category: cs.CV

TL;DR: 论文研究 episodic test-time adaptation 中固定适应步数会掩盖个体病例是否应被适应的问题，提出用预测碎片化（prediction fragmentation）作为无标签、无额外反向传播的病例级信号，构建路由器决定是否部署适应，从而显著减少有害编辑面积 HA。


<details>
  <summary>Details</summary>
Motivation: 固定适应 horizon 把“适应多远”的群体级问题与“该病例是否应适应”的个体级问题混在一起；群体平均 ΔDice 可能接近零，但跨厂商心脏 MRI 中 58.7% 病例被个体性地恶化，因此需要病例级决策来控制适应伤害。

Method: 定义 harmful accepted area (HA) 为控制器部署编辑区域中有害部分的比例；分析源权重 M0 与适应后掩码 Mk 之间的分歧几何，即 prediction fragmentation；基于该信号构建病例级 router，在决策时无需标签或梯度，阈值仅在独立标注划分上拟合一次，并验证跨架构与跨域迁移。

Result: prediction fragmentation 在三个 benchmark 上预测 HA 的 Spearman ρ 为 0.50–0.60，延迟约为 gradient-norm 的四分之一；在未参与设计的 benchmark 上 HA 从 0.228 降至 0.139；在心脏 benchmark 上 HA 从 0.129 降至 0.013，匹配 Dice 与 1.10 次部署更新，并优于事后最佳预算，58.7% 恶化病例降至 20.0%；在前列腺数据上 HA 仍下降但准确率让步。

Conclusion: 固定 horizon 不足以处理 episodic test-time adaptation 的个体差异；病例级 router 能有效减少适应伤害，且无需标签或梯度、可跨架构和域迁移；但其边界是当保留病例并非净受益时，路由会以牺牲准确率为代价来降低 HA。
...
### [188] [ERCPMP-Gx: Endoscopic Image and Video Dataset for Morphological, Histopathological, and Genomic Characterization of Colorectal Polyposis](https://arxiv.org/abs/2609.20815)
*Zahra Ghaffari,Massih Bahar,Mojgan Forootan,Ali Darvishi,Hamidreza Bolhasani*

Main category: cs.CV

TL;DR: 该论文介绍了ERCPMP-Gx数据集，这是一个包含内镜、组织病理学和基因组学数据的结直肠息肉病数据集，旨在支持人工智能在识别、表征和分类中的应用。数据集包含160张图像和视频片段，约80%为遗传性息肉病综合征，20%为非遗传性息肉。每个记录关联了内镜注释、组织病理学和种系发现。数据集公开可用。


<details>
  <summary>Details</summary>
Motivation: 遗传性息肉病综合征是结直肠癌的前驱病变，并与多种结肠外肿瘤相关。早期识别和准确分类对于及时诊断、个性化管理和针对性监测至关重要。然而，公共内镜数据集主要围绕散发性息肉组织，没有在患者层面将息肉病表型与组织病理学和种系发现联系起来。

Method: 研究人员开发了ERCPMP-Gx数据集，使用Olympus EVIS X1系统进行WLE、NBI、放大NBI和近聚焦NBI检查，获得160张图像和视频。数据集包括约80%经临床和/或遗传确认的遗传性息肉病综合征（FAP、PJS、JPS、GNS）和20%非遗传性息肉及类似病变。每个记录关联标准化内镜注释、代表性组织病理学和临床报告的种系发现。

Result: 该数据集形成了一个AI就绪的、患者级别的注释框架，并公开可用（Mendeley和DataBioX网站）。

Conclusion: ERCPMP-Gx数据集支持人工智能在结直肠息肉病的识别、表征和分类中的应用。
...
TL;DR: 本文提出了首个同步多视角飞溅液体数据集，并提出了SplashSplat方法，用于从多视角视频中重建动态飞溅液体，在真实和合成数据上优于现有动态高斯泼溅方法。


<details>
  <summary>Details</summary>
Motivation: 飞溅液体持续时间极短、易破碎成液丝和液滴、外观依赖视角且近乎无纹理，现有重建研究多集中于烟雾、合成液体或缓慢变形表面，缺乏同步多视角真实飞溅液体数据集。

Method: 构建20个真实场景的基准数据集，使用7台同步标定4K相机以60fps采集，并提供逐视角液体与容器掩码及固定评估划分。提出SplashSplat：仅在观测可约束处施加物理结构；每帧液体SDF由掩码融合得到，相邻SDF间的水平集传输产生粗略速度场，拉格朗日载体沿流场推进，并通过新观测校正、在覆盖丢失处重新播种，解码局部高斯进行可微渲染。

Result: SplashSplat在真实采集和合成基准上均优于最先进的动态高斯泼溅方法，运动更符合物理且训练成本更低；同一表示还支持无需重新优化的时间插值与风格迁移。

Conclusion: 该工作填补了真实飞溅液体多视角数据集的空白，并表明仅在观测可约束处引入物理结构，可有效重建复杂飞溅液体动态，同时具备更低训练成本和额外编辑能力。
...
TL;DR: 论文提出 PersistBench，一个利用 360° 视频作为全知真值的基准数据集与指标套件，从物体恒存性、运动连续性和外观保持性三个维度评估 4D 基础模型的视觉记忆能力。


<details>
  <summary>Details</summary>
Motivation: 现有 4D 基础模型（如可控摄像机视频模型、4D 重建模型）能感知和重建动态环境，但其记忆能力尚不明确；现有基准多依赖像素级指标，且缺乏物体离开视野后的真值，无法以物体为中心、对照参考来评估视觉记忆。

Method: 构建 PersistBench 数据集与指标套件，以 360° 视频作为全知真值，提出三个评估方面：物体恒存性、运动连续性、外观保持性，并对多种类别的模型进行评测。

Result: 评估发现当前模型仅能维持短期一致性，一旦物体离开视野，一致性显著下降；揭示了模型能力与稳健视觉记忆之间的差距，即“看见不等于记住”。

Conclusion: 该工作为未来 4D 基础模型的发展提供了指导方向，数据集与代码已在项目主页公开。
...
### [191] [Modality Discrepancy Transformer for Ambivalence and Hesitancy Recognition](https://arxiv.org/abs/2609.19148)
*Shiyu Luo,Yu Wang,Jiawen Huang,Zhaoxiang Xiao,Chenxi Huang,Qi Zhang,Bin Liu*

Main category: cs.CL

TL;DR: 该论文提出模态差异Transformer（MDT），用于自动识别临床视频中的矛盾与犹豫（A/H）情感状态，通过检测跨模态不一致信号，在BAH数据集上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: A/H表现为面部、声音和语言通道间的矛盾信号，自动识别需要检测跨模态不一致，而标准融合方法会抑制这种信号，因此需要冲突感知的多模态融合方法。

Method: 基于Bekhouche等人的冲突感知多模态融合框架，MDT将原6-token设计扩展为9-token表示：3个模态嵌入、3个绝对差特征和3个通过线性投影学习的Hadamard积差异特征。9个token经过Transformer自注意力，并采用FiLM-based文本条件调制和LoRA微调。推理时，文本引导的晚期融合分支将纯文本辅助头与完整多模态输出结合。

Result: 在第三届ABAW挑战赛的BAH数据集上，MDT在标注测试集上达到0.7408 Macro F1，在私有排行榜上达到0.7368，超过最强已发表基线10个点以上，且单GPU训练时间不到20分钟。

Conclusion: MDT通过显式建模跨模态差异，能有效识别A/H，并在性能和效率上均优于现有方法。
...
### [192] [Sampling Reveals Style: Unsupervised, Training-Free Discovery of Prompt-Conditional Stylistic Axes in LLM Activations](https://arxiv.org/abs/2609.19150)
*Ajit Mallavarapu,Ziwei Gu*

Main category: cs.CL

TL;DR: 该论文提出一种无需训练的、基于提示条件的风格维度发现方法：通过高温重复采样同一提示的补全，对隐藏激活做PCA，并自动标注主轴；在人类标注验证中，最强模型上主轴能较好匹配人类自发关注的风格维度，且可发现性因模型而异。


<details>
  <summary>Details</summary>
Motivation: LLM隐藏激活中编码了丰富的风格结构，但识别给定提示下哪些风格维度显著通常需要监督对比数据；作者希望找到一种无需训练、低成本且能针对单个提示发现风格维度的替代方法。

Method: 对单个提示在较高温度下重复采样生成结果，池化隐藏激活后做PCA；根据主轴两极的生成样本自动标注各轴；再用两阶段研究将发现的轴与245个人类诱发的风格标注进行验证。

Result: 在最强模型Qwen-3.5-4B-Instruct上，前两个主轴与人类自发请求维度的精确率为72.8%，宏召回率为43.6%；75.6%的有效性评分认为轴两极生成与其标签相符，邻近标注者一致性为90.9%。可发现性依赖模型：Qwen系列和Llama-3.2-3B能暴露人类显著轴，而DeepSeek-7B-Chat精确率降至35.3%，其主成分更多由结构而非风格方差主导。

Conclusion: 仅对模型自身解码方差做PCA，即可作为探测LLM表示中风格结构的有效且低成本方法，同时揭示不同模型在风格结构组织方式上的显著差异。
...
### [193] [Subliminal Prompting Beyond Static Geometry: Causal Depth and Multi-Token Confounds](https://arxiv.org/abs/2609.19149)
*Barath Velmurugan*

Main category: cs.CL

TL;DR: 本文研究语言模型中的阈下学习：模型可通过看似无关的输出传递隐藏特征。作者在动物-数字提示协议中分别测量固定几何、观测可读性、因果时序和多词元测量，发现它们是冻结提示通道的不同属性。


<details>
  <summary>Details</summary>
Motivation: 现有关于令牌纠缠的解释常混用不同测量：输出是否共变、固定输出向量是否对齐、答案能否从隐状态读出、隐状态是否因果控制答案。作者希望区分这些性质，以检验令牌层面解释能否说明训练时特征迁移。

Method: 在固定的动物-数字提示协议中，对Llama-3.1-8B到70B分别测量固定输出向量相似度、输出头读出、因果干预和多词元评分；因果实验中在五个深度把数字提示的答案位置临时状态复制到另一提示，观察最终动物分数跟随哪个提示；并在两个Qwen模型中逐位评分及控制数字宽度。

Result: Llama-3.1-8B到70B中，固定输出向量相似度对行为的预测变差，配对平均相关变化为-0.080（95% CI [-0.127, -0.035]）；输出头读出的归一化深度AUC无明确变化。因果干预使供体控制AUC从0.254升至0.540，配对变化+0.286（95% CI [+0.272, +0.300]），18个概念全部上升，剩余八个Transformer块时对比仍存在，特异性与身份控制较小或精确。Qwen中逐位评分未恢复正的单令牌关联；按令牌平均会产生正池化关联，但控制数字宽度后消失，显示长度混杂。

Conclusion: 固定几何、观测可读性、因果时序和多词元测量是这一冻结提示通道的不同属性。它们约束了令牌层面的解释，但不足以识别训练时特征迁移的机制。
...
### [194] [What Users Think of Generative AI: A Cross-Platform NLP Analysis of Trust and Friction in App Store Reviews](https://arxiv.org/abs/2609.19151)
*Md Jafrin Hossain,Umme Nusrat Jahan,Shouvaggo Sharif Shammo*

Main category: cs.CL

TL;DR: 该研究对六款主流生成式AI应用（ChatGPT、Gemini、Microsoft Copilot、Claude、DeepSeek、Perplexity）的17012条英文应用商店评论进行跨应用分析，结合主题建模与情感分类，揭示用户感知质量、信任及采用障碍。


<details>
  <summary>Details</summary>
Motivation: 生成式AI应用已被消费者快速采用，但缺乏大规模研究考察用户感知质量、信任和采用障碍。

Method: 收集Google Play与Apple App Store评论，使用BERTopic主题建模和RoBERTa情感分类，并通过卡方检验、Kruskal-Wallis检验及带Bonferroni校正的多项逻辑回归评估跨应用差异；用300条分层样本进行人工编码验证。

Result: 负面情感集中在广告（91%）、认证（89%）、服务器可靠性（83%）和订阅定价（73%）。各应用情感差异显著，Claude负面情感最高（47.7%）但用户热情也强，呈现显著极化。DeepSeek部分评论涉及中国背景带来的地缘政治与数据隐私担忧；提出Trust Friction Score以总结应用特定信任与可用性障碍。

Conclusion: 研究提供了经过验证且可操作的证据，说明消费级生成式AI应用在用户信任、可用性和采用障碍方面的问题，并引入Trust Friction Score作为可解释维度。
...
### [195] [FakeSpotter: A content and strategy agnostic Viral Misinformation Detection Tool](https://arxiv.org/abs/2609.19152)
*Giovanni Spitale,Federico Germani*

Main category: cs.CL

TL;DR: 本文提出FakeSpotter，一种不直接判断真假、而是基于内容与策略无关的结构指纹来评估文本病毒式错误信息风险的工具。


<details>
  <summary>Details</summary>
Motivation: 现有错误信息检测依赖二分类或历史样本训练，面对新出现的误导性叙事时效果有限，因此需要更早、可解释且适应新叙事的风险评估方法。

Method: FakeSpotter基于理论框架，从语言、叙事、逻辑和批判性思维维度提取特征，使用重复LLM评估和针对短文本、长文本的领域专用逻辑回归分类器进行风险估计。

Result: 在来自社交媒体和FakeNewsNet的764条标注文本语料上，FakeSpotter在留出测试集上的宏F1分数为：短文本0.788，长文本0.793。其解释层提供基于特征的分数、信号一致性和警示指数，并可用于社会聆听。

Conclusion: 识别错误信息的结构指纹有助于对可能病毒式传播的错误信息进行早期、可解释且有人工监督的评估。
...
### [196] [Neo-Classic: A Benchmark for Evaluating Linguistic-Aesthetic Reasoning in Classical Chinese Poetry](https://arxiv.org/abs/2609.19154)
*Han Zhang,Zihan Gu,Zhiyuan Wang,Tianyi Ma,Jiacheng Lu,Xinyan Zhang,Yuhao Wei,Cheng Hua*

Main category: cs.CL

TL;DR: 论文提出 Neo-Classic 基准，用当代专家创作的严格格律诗和反向理解探针，评估大语言模型在古典诗词中的语言审美推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有古典诗词基准上 LLM 准确率虽高，但难以区分其真正具备可迁移的语言审美推理，还是依赖预训练中熟悉的模式。

Method: 构建 Neo-Classic：结合建构主义式样本外数据集与反向理解探针；采用当代专家创作的严格格律诗，降低直接检索可能；在五类行为探针上评估 Qwen3-Max、Gemini-3-Pro、DeepSeek-V3.2 等先进模型，测试层级约束满足能力。

Result: 模型从历史文本迁移到当代文本时性能下降 20% 至 50%；篇章级排序任务表现很差，标准准确率仅 0% 至 13%；专家级提示可将推理增强模型提升至 36%，但仍与人类专家存在明显差距。

Conclusion: 当前 LLM 能捕捉局部形式模式，但在稳健语言审美推理所需的全局层级规划方面仍有困难。
...
### [197] [Stop Removing Stopwords: How an Inherited Preprocessing Default Distorts Legal Text-as-Data](https://arxiv.org/abs/2609.19153)
*Gregory M. Dickinson*

Main category: cs.CL

TL;DR: 该研究通过穷举单字消融实验，检验停用词移除对法律文本分类任务的影响，发现常用停用词表反而降低性能，最优停用词表与不移除无显著差异，且无法预测哪些词移除有益。


<details>
  <summary>Details</summary>
Motivation: 法律文本作为数据的研究常沿用信息检索的预处理默认设置（如停用词移除），但这些默认从未针对分类准确率验证；文本特征本身是研究对象，预处理可能扭曲信号。

Method: 使用美国最高法院数据库标签与Caselaw Access Project意见文本，构建两个二分类任务（意识形态方向，F1约0.68；宪法与非宪法法律类型，F1约0.92），对每个任务进行单字消融：移除约18,500个候选词，直接测量对下游目标的影响，并近似专家可构建的最佳停用词表。

Result: 常用停用词表在所有测试中均低于不移除基线；优化停用词表与不移除统计上无差异；基于词级特征的元模型无法预测哪些移除有益。

Conclusion: 停用词移除等预处理默认会悄然改变模型所见特征，可能扭曲法律文本研究旨在恢复的教义和意识形态信号；保留停用词是测量效度问题。方法可推广至其他继承的预处理默认。
...
### [198] [Towards Proactive Detection of User-Side Implicit Conflicts in Human-LLM Dialogue](https://arxiv.org/abs/2609.19155)
*Jinqiang Wang,Tao Zhu,Huansheng Ning*

Main category: cs.CL

TL;DR: 本文关注人机对话中被忽视的“用户侧冲突”问题，构建了人工标注基准 UC-Bench，并提出约束引导的数据合成方法 SynUC 及训练集 UC-Data，使轻量模型在用户侧冲突检测上超越大型通用模型。


<details>
  <summary>Details</summary>
Motivation: 在多轮 Human-LLM 对话中，用户后续话语可能与先前意图存在隐性冲突，若系统不能主动识别，就会误解需求并生成不当回复。已有研究主要关注 LLM 侧冲突，用户侧冲突尚缺乏探索，且现有合成方法无法显式建模历史与当前话语间的隐性不兼容，难以生成可靠标注的隐式冲突样本。

Method: 1）构建人工标注基准 UC-Bench 用于评估用户侧冲突检测；2）提出 SynUC，将用户侧冲突表示在约束空间中，并借助 SPEAKING 框架引导可追溯的约束变换，实现数据合成；3）基于 WildChat 合成用户侧冲突训练集 UC-Data（共 2,487 条样本），用于微调轻量模型。

Result: 初步实验表明现有 LLM 在用户侧冲突检测上表现不佳，尤其在需要依据对话历史推断隐性不兼容时更为困难。在 UC-Bench 上，基于 UC-Data 训练的 Qwen3.5-4B 优于 Claude Opus 4.8 等更大规模的通用 LLM，也优于使用现有合成方法数据的同骨架模型。

Conclusion: 用户侧冲突检测是一个重要但被低估的任务，显式建模约束空间并结合 SPEAKING 框架的合成方法能有效提升轻量模型性能，使其在少量训练数据下超越大型通用模型。
...
### [199] [Reflective Recovery: A Self-Supervised Method for Reasoning by Learning from Mistakes](https://arxiv.org/abs/2609.19156)
*Qirui Chen,Renjie Pi,Jiahui Gao,Lingpeng Kong*

Main category: cs.CL

TL;DR: 该论文提出 Reflective Recovery，一种自监督方法，将失败推理轨迹转化为恢复训练数据，以提升大语言模型的推理鲁棒性和自我纠错能力。


<details>
  <summary>Details</summary>
Motivation: 主流模仿学习仅依赖完美推理轨迹，受限于问题集规模时会出现 Scaling Collapse，即增加正例无法持续提升性能；同时推理中一旦出错，模型难以恢复且易被累积错误误导。

Method: 提取失败轨迹的初始片段，与提示拼接后用于引导模型生成有效解；由于失败片段可能含错，该过程训练模型识别并纠正推理中的错误，无需外部批评器或奖励模型。

Result: 在多个基准上显著提升性能：DeepSeek-R1-Distill-Qwen-7B 在 AIME 2025 上准确率从 30.0% 升至 37.5%，在 Minerva 上从 37.6% 升至 47.8%；并突破 Scaling Collapse，涌现自我纠错行为。

Conclusion: Reflective Recovery 将训练范式从结果导向的记忆转变为过程导向的反思推理，为 LLM 推理能力提升提供了简单有效的自监督路径。
...
### [200] [VisKG-LM: Compiling Knowledge Graphs into Visual Memory for Multiple-Choice Question Answering](https://arxiv.org/abs/2609.19158)
*Yixin Peng,Er Jin,Shiwei Luo,Diego Collarana,Stefan Decker*

Main category: cs.CL

TL;DR: 提出 VisKG-LM，将知识图谱问答中检索到的子图离线编译为可复用的视觉记忆，而非在线反复用图神经网络编码。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常在线编码检索子图并与语言模型融合，导致同一静态子图在训练、推理和评测中被反复重新编码，计算冗余且效率低。

Method: 将候选子图序列化为 Relation-Labeled Paths，并渲染为保留路径分支结构的二维图像；每张图像离线编码一次并缓存。推理时语言模型仅从文本理解问题与候选，最后一层再查询缓存视觉记忆，读取全局布局和局部关系细节。

Result: 在 CommonsenseQA、OpenBookQA、MedQA-USMLE 上分别比 GreaseLM 提升 1.2、0.8、4.3 分；以约 4 亿在线参数匹配或超过 70 亿参数 GraphVis。与接收相同 Relation-Labeled Paths 的纯文本对照相比，分别提升 4.2、6.5、5.1 分。

Conclusion: 完整视觉记忆接口带来的收益超过单纯路径文本化，说明离线编译视觉记忆可作为在线图传播的替代方案。
...
### [201] [Advantage Scale Calibration Imbalance in Group-Relative Optimization under Low-Variance Rewards: Diagnosis and Bounded Recovery](https://arxiv.org/abs/2609.19164)
*Fei Ding*

Main category: cs.CL

TL;DR: 本文将 verifier-style RLVR 中组相对优化的 advantage scale 从实现细节提升为关键校准问题，区分亚分辨率抖动与可信小幅真实差距，并提出三路校准接口、Reward-Resolution Protocol 与 MaxNorm-AC。


<details>
  <summary>Details</summary>
Motivation: 现有方法常把 advantage scale 当作实现细节，但同一组内尺度会同时影响 reward 分支强度、prompt 级批权重和有效 KL 校准，导致小差距要么被 KL 主导，要么被无界放大。

Method: 提出 advantage-scale 三路校准接口，用同一组内尺度分母统一解释 reward 分支、批权重与 KL 校准；据此分析 RLOO、Dr.GRPO、GRPO；引入 Reward-Resolution Protocol 过滤亚分辨率差距，并提出 MaxNorm-AC 对可信非零差距做有界基数恢复。

Result: 该接口解释了 RLOO/Dr.GRPO 为何让可信小差距被 KL 主导，以及 GRPO 标准差分母为何无界放大微小差距；在 dense/MoE 架构和 math/code 推理任务上，MaxNorm-AC 优于最强 robust-scale 基线，并截断低方差逆尺度尾部。

Conclusion: advantage scale 需要显式校准；通过区分噪声与可信小差距，并采用有界恢复与过滤机制，可提升 RLVR 优化稳定性与性能。
...
### [202] [To Memories and Beyond: From Remembering to Knowing You across Long-Term Multimodal Personal Archives](https://arxiv.org/abs/2609.19167)
*Wenqi Zhou,Zhuorui Yu,Kaiao Wen,Hao Zheng,Xinyi Zheng,Peiran Wu,Enmin Zhou,Chi-Hao Wu,Junxiao Shen*

Main category: cs.CL

TL;DR: 论文提出首个基于真实多年个人视觉档案与第一人称标注的长期多模态记忆基准 ReaLMem，并设计时间加权画像模块 ChronoProfiler，用于评估和提升 MLLM 的长期个性化能力。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准多为合成、纯文本数据，忽视视觉记录，缺乏真实且因果关联的纵向数据，只能评估浅层事实回忆，无法支撑真正的个性化 AI 伴侣。

Method: 构建 ReaLMem 基准，包含真实多年个人视觉档案和主观标注，并按事实回忆、人格推断、预测性个性化三个认知层级评估；提出 ChronoProfiler，通过计算用户属性的时间稳定性分数作为显著性先验，解决时间不一致偏好冲突并复合多个共时偏好。

Result: 对前沿 MLLM 和记忆系统的广泛评估显示，预测性个性化是持续瓶颈，MLLM 与记忆系统间存在明显性能差距，高质量且时间感知的表征能显著提升个性化效果。

Conclusion: ReaLMem 与 ChronoProfiler 为长期个性化提供了真实测试平台和简单有效机制，为终身 AI 伴侣研究奠定基础。
...
### [203] [YNU-HPCC at SemEval-2025 Task 11: Bridging the Gap in Text-Based Emotion Using Multiple Prediction Headers](https://arxiv.org/abs/2609.19238)
*Hao Yang,Jin Wang,Xuejie Zhang*

Main category: cs.CL

TL;DR: YNU-HPCC团队参加SemEval-2025任务11子任务A（基于文本的情感识别），最佳系统基于RoBERTa并改造输出头以单情感处理，官方排名得分0.44。


<details>
  <summary>Details</summary>
Motivation: 参与SemEval-2025任务11子任务A，旨在弥合基于文本情感分析中的差距，并探索多语言情感识别方法。

Method: 使用RoBERTa模型（改进的BERT，基于Transformer编码器），将输出头改为一次处理一种情感；用Google Translate将全部数据翻译成英语；并进行概率和注意力分析。

Result: 官方排名得分为0.44（包含所有语言）。分析发现：(I) 单个预测头优于同时预测六种情感的六个头；(II) 在统一翻译的英语数据集上训练优于使用原始数据集。

Conclusion: 单预测头结构和统一翻译为英语的数据有利于基于文本的情感识别，相关代码已开源。
...
### [204] [Why Pretraining Fails to Share Cross-Lingual Knowledge](https://arxiv.org/abs/2609.19291)
*Adam Gaber,Uriel Dolev,Elisabeth Fittschen,Bobby Cheng,Yuval Marton,Leshem Choshen*

Main category: cs.CL

TL;DR: 该论文研究大语言模型跨语言知识迁移能力差的原因，发现词元空间不共享是导致知识隔离的关键因素，并提出通过词级翻译将语言映射到共享词元空间来改善跨语言知识泛化。


<details>
  <summary>Details</summary>
Motivation: 多语言大模型虽然取得进展，但其跨语言知识迁移能力远不如人类多语言者；这一局限在预训练中如何产生尚不清楚。

Method: 预训练360M和7B参数模型；构建受控双语预训练设置，使用同一语言的文本和分词方式，但映射到不相交的词元空间；测试标准干预；并尝试用简单词级翻译将语言映射到共享词元空间。

Result: 跨语言知识泛化差在预训练中出现并在标准干预下持续存在；仅词元空间不相交就足以造成知识隔离，即使两种“语言”是同一语言副本；共享词元空间可显著提升跨语言知识泛化，恢复高达12.6%的母语学习效率，是基线的14倍。

Conclusion: 不相交的词元空间是跨语言知识泛化的根本障碍；将不同语言映射到共享词元空间有助于缓解知识隔离并提升跨语言迁移。
...
### [205] [A frontend-backend architecture for tool calls in full-duplex speech models](https://arxiv.org/abs/2609.19334)
...
TL;DR: 提出前端-后端架构，让全双工语音到语音模型通过委派token将工具调用交给文本LLM，实现低延迟语音交互与工具调用。


<details>
  <summary>Details</summary>
Motivation: 全双工S2S模型对话自然低延迟，但缺乏使用外部工具和完成语音代理任务的能力。

Method: 前端为双工语音转文本模型，学习发出委派token并将流式ASR转录转发给文本后端LLM进行工具调用；工具结果通过轻量级prefill-and-repeat机制注入前端，再用流式TTS合成。前端改动小。

Result: 单轮工具调用评估中，工具调用召回率92-97%，预测性能有竞争力，拒绝无关调用准确率81.2%。使用更大后端（Qwen3-235B-A22B）时，在Full-Duplex-Bench-V3上表现与开源/闭源模型相当，在EVA-Bench上显著优于GPT-realtime-mini和Qwen3-Omni-30B-A3B-Instruct。

Conclusion: 后端委派是一种有效且模块化的方法，能结合自然双工语音交互与强大的代理工具调用能力。
...
### [206] [The Role of Fine-grained Harm Signals in LLM Safety](https://arxiv.org/abs/2609.19366)
*Soyeon Park,Seogyeong Jeong,Sunwoo Kim,Alice Oh*

Main category: cs.CL

TL;DR: 研究通过从各类别有害表征中移除共享的一般有害表征，分离出类别残差，并在11类风险、3个指令微调LLM中做激活引导。发现类别残差对有害性编码和拒答的影响因类别而异，且会增强下游与一般有害表征的对齐。


<details>
  <summary>Details</summary>
Motivation: 已有工作发现LLM内部有害表征跨风险类别存在差异，同时共享一般有害表征成分；但类别特定成分在LLM安全中除一般有害表征外的作用尚不清楚。

Method: 从每类有害表征中移除共享的一般有害表征，得到与各层一般有害性正交的类别残差；在3个指令微调LLM、11个风险类别上使用激活引导考察类别残差。

Result: 类别残差是否编码有害性因类别而异，该模式跨模型相似；是否引发拒答也因类别而异，但模式更依赖模型；类别残差增加模型下游内部与共享一般有害表征的对齐。

Conclusion: 理解LLM安全需同时考虑更细粒度的类别残差，而非仅共享一般有害表征；即使某层与某概念正交的方向，也可促进该概念的下游放大。
...
### [207] [Less Is More: Graph-free Multimodal RAG via Multi-signal Late Fusion](https://arxiv.org/abs/2609.19417)
*Tithi Rakshit,Hongkuan Zhou,Lavdim Halilaj,Yuqicheng Zhu*

Main category: cs.CL

TL;DR: TrioRAG 是一种无图多模态 RAG 框架，融合问题、锚点图像和 VLM 增强查询三种信号，在共享多向量索引上独立检索并晚期融合；同时提出基于噪声网络图像的汽车多模态基准 AutoQA。


<details>
  <summary>Details</summary>
Motivation: 基于图的 RAG 虽广泛用于多模态、跨文档问答，但构建语料级图成本高、查询慢且难以维护。

Method: 提出 TrioRAG：无图多模态框架，利用问题、锚点图像及由二者生成的 VLM 增强查询三类互补信号，分别检索页面文本和页面图像构成的共享多向量索引，并通过晚期融合合并结果。还提出 AutoQA，一个基于噪声网络来源图像、需跨手册推理的汽车多模态基准，定位为模型策划的测试台而非人工验证金标准。

Result: 在三个基准上，TrioRAG 匹配或超越基于图的系统，同时降低总成本，并将每次查询推理加速 1.6-2.3 倍。在 AutoQA 中，由于问题基于语料外网络图像，图像检索仅达到 19.3% 文档级召回，而文本衍生信号，尤其是 VLM 增强查询，保持检索稳健。

Conclusion: 无图、多信号晚期融合的多模态 RAG 可在性能不降甚至提升的同时，比图方法更便宜、更快。当图像超出语料范围时，VLM 增强的文本查询对维持检索鲁棒性尤为关键，AutoQA 则提供了相应挑战性测试平台。
...
### [208] [For Your Eyes Only: Evaluating Coordination Between Isolated Language Model Instances](https://arxiv.org/abs/2609.19504)
*Alexander Shirnin,Aleksey Kudelya*

Main category: cs.CL

TL;DR: 论文提出“For Your Eyes Only”协作信号游戏，检验同一模型的不同实例能否仅凭共享预训练与任务指令，在自然语言中传递隐藏目标信号，且无需共享记忆或专门协调训练。


<details>
  <summary>Details</summary>
Motivation: 随着模型生成内容越来越多地被其他模型实例在自动化流程中消费，一个实际问题是：模型能否在不共享记忆、不进行协调训练的情况下，仅通过共享预训练和任务指令，在自然语言中嵌入可被同模型独立实例检测的信号。

Method: 设计协作信号游戏：Sender为两个词生成自由描述，其中一个为隐藏目标；隔离的Receiver需识别目标。评估四个架构家族的七个当代模型，使用300个来自心理语言学语料库的词对，并采用Double-Pass Success Rate控制输出偏差。

Result: 多数模型在要求避免可检测信号后难以维持协调；一个前沿模型即使经过此类过滤仍保持近乎完美表现。模型还能将这种能力用于故意误导；跨架构协调始终弱于同架构内协调。

Conclusion: 模型具备一定自然语言隐写/协调能力，但多数模型在该能力受限制时表现明显下降；同模型实例间协调更强，且该能力可被用于误导，说明其具有实际与安全意义。
...
### [209] [From Parameters to Behaviors: A Survey of Model Fusion for Large Language Models](https://arxiv.org/abs/2609.19553)
*Shuo Cai,Yanggan Gu,Zihao Wang,Yuanyi Wang,Yibo Yan,Wenjun Wang,Yuhang Liu,Guanghao Zhu,Sirui Huang,Ming Li,Hongxia Yang*

Main category: cs.CL

TL;DR: 该综述对模型融合进行统一定义，并按参数级、表示级、行为级三个层次系统梳理现有工作，同时总结相关指标、基准、应用、挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 截至2026年6月，Hugging Face已托管超过200万个模型，模型复用与能力集成需求日益增长；但现有综述往往只覆盖部分内容，缺乏统一定义和系统分类。

Method: 采用文献综述方法，提出模型融合的统一界定，构建参数级、表示级、行为级三层分类体系，并回顾相关评价指标、基准测试与应用场景。

Result: 提供了模型融合领域的清晰研究地图，整理了综合论文列表（GitHub链接），归纳了当前挑战并指出未来研究方向。

Conclusion: 旨在帮助研究者系统理解模型融合领域，并为后续模型融合研究提供参考与支持。
...
### [210] [CliniCIRCA: A Modular LLM Framework for Constructing Longitudinal Mental Health Patient Journeys from Raw EHR Narratives](https://arxiv.org/abs/2609.19585)
...
TL;DR: 提出CliniCIRCA多阶段LLM框架，用于从无事件级时间戳的非结构化出院小结中重建精神健康患者的时间线，并验证指令微调能提升多个开源模型在事件抽取、时间标注和摘要任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 精神健康诊疗中，患者纵向经历（生物、心理、社会事件）常分散在非结构化文本中，且缺乏事件级时间戳，导致时间线重建困难。

Method: 基于14,882条MIMIC-III精神健康入院记录，构建52份出院小结基准；采用多阶段LLM框架进行日历锚定、不精确感知的事件时间分类；引入临床医生在环修正错误；生成银标准时间线作为训练数据；并评估指令微调对五个开源模型的影响。

Result: 生成15,891个时间标注事件，经临床医生在环修正629个错误后得到金标准；时间线驱动摘要压缩1.52倍为按日期分组的时序记录；扩展生成1,000条银标准时间线；与零样本和少样本提示相比，指令微调普遍提升五个开源模型在事件抽取、时间标注和摘要上的性能。

Conclusion: CliniCIRCA首次实现无时间戳的非结构化出院小结中临床事件的时间分类，且指令微调可有效提升开源模型在临床时间线相关任务上的表现。
...
### [211] [Form Over Content In Gradient-Based Data Attribution Methods](https://arxiv.org/abs/2609.19589)
*Sunwoo Kim,Seokwon Jung,Sohyung Kim,Seong Joon Oh,Alice Oh*

Main category: cs.CL

TL;DR: 该研究澄清了梯度相似度在数据归因中究竟衡量什么，发现它主要跟随答案格式而非任务语义；这一结论跨预训练到后训练、模型规模和家族成立，并且LESS的数据选择也偏向目标自身的答案格式。


<details>
  <summary>Details</summary>
Motivation: 梯度相似度被广泛用于分析和选择大语言模型的训练数据，但其含义存在争议：一种解释认为它识别任务相关技能，另一种认为表面形式才是主要因素。作者希望独立变化任务和答案格式来解决这一争议。

Method: 在监督微调数据中独立变化任务和答案格式：用不同答案格式渲染基准，使数据集可共享任务但不共享格式，或共享格式但不共享任务。计算梯度对齐程度，并覆盖最早预训练检查点、后训练阶段、不同模型规模与家族。此外分析LESS发布的选择结果。

Result: 共享答案格式的基准对梯度对齐很强，disattenuated cosine约0.4；同一基准但不同答案格式类别的对齐接近0.0。该模式从早期预训练检查点到后训练、跨模型规模和家族均成立。LESS中每个目标的选择都过度代表目标自身的答案格式。

Conclusion: 梯度归因方法更多追踪答案格式相似性，而非任务语义。因此，应谨慎解释梯度的语义含义，并在答案格式与任务独立变化的数据上测试这些方法，以提高稳健性和可靠性。
...
### [212] [Full-Duplex Speech Models Take the Floor When Asked, Not When Needed](https://arxiv.org/abs/2609.19596)
*Linkai Peng,Baorian Nuchged,Kaiqi Fu,Yuyang Yao*

Main category: cs.CL

TL;DR: 研究全双工语音模型是否像人类一样在适当时机主动发言，发现模型主要对直接称呼和沉默做出反应，而对错误事实或危险警告的主动干预能力严重不足。


<details>
  <summary>Details</summary>
Motivation: 全双工语音模型能同时听说，有望成为常驻助手，但必须决定何时发言。人类不仅在被称呼或对方停止时发言，还会主动纠正错误、补充缺失词或警告危险，因此需要探究模型是否具备这种能力。

Method: 构建上下文匹配的英语独白，同一主题下仅触发话语变化；根据轮次分配规则定义10种条件；压缩词间停顿以减少沉默带来的发言机会。测试五个模型家族，并分析Moshi和PersonaPlex的帧级文本标记概率及回复内容。

Result: 被称呼和沉默是比错误事实或危险更可靠的触发因素。Moshi和PersonaPlex在触发结束后的前2秒内，对错误事实的帧级文本标记概率低于中性条件；停顿或允许打断也不能缩小差距。在有发言权时，Moshi和PersonaPlex能回答多数直接问题，但挑战错误事实的非空回复比例仅0.14-0.15，警告危险的回复比例仅0.04-0.07。

Conclusion: 论文指出了语音发起和回复内容上的差距。要弥补这一差距，需要真正的内容理解以及基于此的干预决策。
...
### [213] [Chain-of-Thought Entropy as a Reliability Signal: A Preregistered Reproduction](https://arxiv.org/abs/2609.19606)
*Theodore O. Cochran*

Main category: cs.CL

TL;DR: 本文对Zhao在2026年报告的大语言模型思维链熵轨迹解离现象进行了独立重复研究：熵轨迹形状可预测最终答案正确性，而总熵降幅不能。重复实验在完整测试集上验证形状信号，并发现幅度信号因设置而异。


<details>
  <summary>Details</summary>
Motivation: 原研究的幅度信号仅基于单一模型、单一种子和300题的一次运行，证据较弱；而形状信号已在两个基准和第二个模型家族上全规模报告，因此需要预先注册的独立重复来检验该解离。

Method: 在OSF预先注册确认性运行；在完整GSM8K和MATH-500测试集上，用四个开源权重模型（含一个推理蒸馏模型）进行实验；检验形状信号与幅度信号，并进行探索性比较，包括最后一步熵、ROC面积和风险-覆盖面积。

Result: 形状信号得到重复。幅度信号因设置而分化：锚点模型上，单调与非单调思维链的准确率差在GSM8K为+9.6个百分点，MATH-500为+27.5个百分点；总熵降与正确性的秩相关在GSM8K为-0.018，MATH-500为+0.414。推理蒸馏模型上二元形状信号仅约每百条链触发一次，样本太少无法估计注册对比，但分级违规计数仍有预测力。探索分析中，最后一步熵在全部8个模型×基准单元中按ROC面积优于二元形状标志，按风险-覆盖面积在6或7个单元中更优，取决于原研究未说明的积分范围。

Conclusion: 本研究在全测试集规模上重复了形状信号，并记录了七项协议差异；绘制了幅度信号成立与失败的设置图谱；测量了原研究未报告的四项协议依赖性。
...
### [214] [Semantic Layer Induction from Raw Telemetry via Hierarchical LLM and RAG Abstraction](https://arxiv.org/abs/2609.19615)
*Yuanzhe Jia,Ali Anaissi*

Main category: cs.CL

TL;DR: 本文提出一个端到端框架，可从应用原始日志中全自动构建业务语义层。该方法采用两阶段语义抽象：先利用LLM推理和行业知识识别高层业务特征，再通过结构化流水线生成细粒度业务节点。


<details>
  <summary>Details</summary>
Motivation: 现代应用产生海量原始遥测数据，但将噪声大、异构的事件流转化为可执行业务洞察仍很困难。数据工程师和分析师需耗费大量精力处理语义差异、手工编写解析逻辑，并维护原始数据与业务KPI之间脆弱的映射关系。

Method: 提出两阶段语义抽象框架：第一阶段通过LLM推理并结合领域/行业知识识别高层业务特征；第二阶段通过结构化流水线生成细粒度业务节点，包括数据精炼、混合检索、多阶段过滤、语义聚类和规范化命名。评估中使用集成的LLM-as-Judge方法进行质量保证。

Result: 在生产规模遥测数据上，系统将人工评估的语义质量从50分提升到80分以上（百分制），维护工作量减少80%，过滤掉74%的噪声，并通过LLM-as-Judge评估取得0.87的Cohen's kappa，支持持续、可扩展的质量保证。

Conclusion: 本文区别于先前工作，针对从原始遥测数据归纳业务语义层这一新问题，无需标注训练数据或手工规则工程，即可自动构建业务语义层，并实现持续、可扩展的质量保证。
...
### [215] [Improving Cross-Lingual Transfer for Sequential Sentence Classification in Research Papers via Structural Similarity](https://arxiv.org/abs/2609.19650)
*Kazuhiro Yamauchi,Marie Katsurai*

Main category: cs.CL

TL;DR: 该论文研究跨语言顺序句子分类（SSC），构建了涵盖13种非英语语言、来自5个学术数据库的多语言数据集，并分析影响迁移效果的因素，进而提出利用结构信息的生成式方法。


<details>
  <summary>Details</summary>
Motivation: SSC对科学出版物结构化很重要，但非英语语言训练数据稀缺。跨语言迁移有望缓解该问题，但SSC依赖跨语言一致的话语层面模式（如标签序列和位置规律），因此需要探究决定迁移成功的因素。

Method: 构建多语言SSC数据集，覆盖13种非英语语言和5个学术数据库；使用基于编码器和生成式模型进行跨语言迁移实验；分析语言接近性、修辞组织结构相似性、标签分布相似性等预测因素；提出三种显式利用结构信息的生成式方法。

Result: 语言接近性对迁移性能没有一致预测力；修辞组织结构相似性呈弱但一致的正相关；控制源语言性能后，标签分布相似性是最一致的预测因素。所提方法在域内评估中最佳组合达到最强编码器基线水平，在迁移到训练中未见的语言时超过最强编码器基线。

Conclusion: SSC跨语言迁移中，话语结构信息比语言接近性更关键，尤其是标签分布相似性；显式利用结构信息的生成式方法能提升迁移性能，并在未见语言上优于强编码器基线。
...
### [216] [Learn Before You Judge: Progressive Knowledge-to-Decision Alignment for Explainable Hateful Meme Detection](https://arxiv.org/abs/2609.19778)
*Bo Xu,Chenyuan Wang,Xinyu Chen,Quanhao Zhu,Rui Lin,Liang Zhao,Hongfei Lin,Feng Xia*

Main category: cs.CL

TL;DR: 该论文提出 ProKDA，一种面向可解释仇恨模因检测的渐进式知识到决策对齐方法，通过分阶段训练缓解解释生成与标签预测之间的任务干扰，并在三个公开基准上取得最优检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有“先解释后检测”方法通常在同一训练过程中耦合解释生成与标签预测，导致任务目标互相干扰，检测性能受限，甚至不如简单的 SFT 基线。

Method: ProKDA 首先使用智能体式背景知识构建流程获取与模因理解相关的外部知识；然后采用三阶段训练策略，依次进行背景知识学习、仇恨性检测学习和仇恨性边界对齐。每个阶段只关注单一训练目标，从而减少任务干扰，并逐步将背景知识转化为稳健的检测决策。

Result: 在三个公开仇恨模因基准上的实验表明，ProKDA 取得了最先进的检测性能，并能提供准确、可解释且有证据支持的仇恨模因审核决策。

Conclusion: 通过将解释与检测解耦为渐进式训练阶段，ProKDA 有效缓解了多任务干扰，提升了可解释仇恨模因检测的性能与决策可靠性。
...
### [217] [A Phonemically Comprehensive, ASCII-Only Romanization Scheme for Thai and Lao: Systematic Cross-Lingual Correspondence and Chinese-User-Friendly Design](https://arxiv.org/abs/2609.19736)
*Zijie Zhang,Tan Lee*

Main category: cs.CL

TL;DR: 提出一种面向泰语和老挝语的、仅用ASCII且音位全面的罗马化方案，将两种语言作为统一的跨语言设计问题，兼顾音段、元音长度和声调。


<details>
  <summary>Details</summary>
Motivation: 泰语与老挝语关系密切，但缺乏统一的、音位全面且键盘友好的罗马化方案，需要支持语言学习和跨语言语音处理。

Method: 统一设计泰语-老挝语罗马化；表示音段对比、元音长度和词汇声调；坚持一符一音位透明性及泰老系统对应；优先共时语音对应，包括与拼音和粤拼的对应，在不冲突时保留历史音韵对应；声调采用紧凑单数字默认记法，并提供可选声调值和历史调类表示。

Result: 所得方案能提供可读、键盘友好、可机器处理的音位表示，并保持泰语与老挝语之间的系统对应。

Conclusion: 该方案适用于语言学习和跨语言语音处理，实现了一种泰老统一、仅用ASCII的罗马化表示。
...
### [218] [Evolution or Illusion? Rethinking Evaluation in LLM Evolutionary Search](https://arxiv.org/abs/2609.19799)
*Tal Oved,Roi Pony,Oshri Naparstek,Udi Barzelay*

Main category: cs.CL

TL;DR: 该论文指出，LLM驱动的进化搜索通常只在单一预算设置下比较方法，这不足以反映真实性能。作者通过在不同种子数和迭代次数组合下进行评估，发现最佳预算分配方式及方法排名会随策略、任务和总预算变化。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常只报告一种预算设置下的结果，例如固定种子运行和固定迭代次数，并据此对方法排名，这可能导致结论片面甚至误导。

Method: 在五个常用优化任务上评估三种进化搜索策略，并在种子数与迭代次数的完整网格上运行实验，分析固定预算在“宽度”（更多种子）和“深度”（更多迭代）之间的不同分配。

Result: 最佳预算分配随策略、任务和总预算变化；策略排名也随预算变化。例如，某任务中单种子下表现最差的策略在四十个种子时反而最好；另一任务中最佳迭代次数远低于常见实践值，过多迭代会浪费预算。

Conclusion: 论文提出一种测量协议，用于报告种子数-迭代次数前沿，并给出实际使用该协议进行预算分配和方法比较的指导。
...
### [219] [Dictionary-Constrained Grapheme-to-Phoneme for Unsegmented Languages from LLM-Annotated Data](https://arxiv.org/abs/2609.19805)
*Rui Hu,Zhenpeng Zhan,Xiaolong Lin*

Main category: cs.CL

TL;DR: 本文提出一种上下文感知的神经G2P方法，在基于词典构建的词格上使用判别式CRF对路径打分，并利用大语言模型生成超过200万句数据以缓解标注稀缺问题。


<details>
  <summary>Details</summary>
Motivation: G2P需快速、稳定且上下文感知；日语等未分词语言中G2P还需结合分词与高度依赖上下文的异读消歧，而准确标注数据稀缺成为瓶颈。

Method: 构建词典词格，使用判别式条件随机场（CRF）对路径进行上下文感知打分；利用大语言模型生成超过200万句训练数据。

Result: 在Joyo-Kanji-Yomi基准上，目标词读音准确率达99.62%，目标词音素错误率（PER）为0.32%，句子PER为0.14%，显著优于传统形态分析器方法和神经序列模型。

Conclusion: 上下文感知神经G2P结合词格CRF与LLM生成数据，能有效提升未分词语言G2P性能，并大幅超越现有基线。
...
### [220] [F$^{2}$DR: A Fine-Grained Full-Pipeline Reward Framework for DeepSearch Workflows](https://arxiv.org/abs/2609.19827)
...
TL;DR: 本文提出面向DeepSearch全流程的细粒度奖励框架F2DR，并构建DeepSearch RM-Bench基准，用于评估DeepSearch场景下的奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型和评测基准主要针对静态单轮任务，无法覆盖DeepSearch中规划、反思、检索和答案生成等迭代闭环流程的复杂性。

Method: 提出F2DR框架，从内容（Content）、轨迹（Trajectory）和答案（Answer）三个维度对DeepSearch工作流进行全过程评估；同时构建专用基准DeepSearch RM-Bench。

Result: 实验表明，F2DR在评估一致性上显著优于基于自评估的基线；DeepSearch RM-Bench对现有开源奖励模型展现出较强的区分能力。

Conclusion: F2DR和DeepSearch RM-Bench能够更全面地评估DeepSearch全流程中的奖励模型，弥补现有静态单轮评估的不足，数据集将公开。
...
### [221] [Zarya: A Hybrid Autoregressive--Masked Diffusion Language Model with Flexible Training and Dual-Mode Inference](https://arxiv.org/abs/2609.19868)
*Leonid Sinev,Ilya Koziev,Vladislav Leshchuk*

Main category: cs.CL

TL;DR: Zarya 是一类混合语言模型，在单一架构中联合优化自回归目标与掩码扩散目标，通过槽位与课程学习连接细粒度 AR 和粗粒度扩散，并支持两种统一解码范式，公开 0.6B、1.7B、4B 模型。


<details>
  <summary>Details</summary>
Motivation: 自回归模型受限于从左到右的顺序生成；掩码扩散模型虽可并行解码，但无法复用 KV 缓存导致计算开销高，且因在不可处理的词元组合空间上学习依赖而生成不连贯。

Method: 提出 Zarya，将训练数据组织为可变大小槽位，并用逐步提高槽位粒度的课程学习，从细粒度 AR 平滑过渡到粗粒度扩散；推理时统一接口支持 MDM 首次命中去噪采样，以及槽位推测解码，交错进行槽间扩散选择与槽内 AR 填充，实现完整 KV 缓存复用。训练与推理解耦，支持分组噪声模式、有序采样计划、噪声级排列等配置。

Result: 公开 0.6B、1.7B、4B 规模的 Zarya 模型，在标准基准上展示性能，并证明自回归与扩散范式的原则性集成。

Conclusion: Zarya 为 AR 与扩散范式提供了统一且可配置的集成方案，训练配置与推理模式可解耦部署，便于灵活研究和实际应用。
...
### [222] [JustMem: Just-Enough Memory Access for Long-Term Conversations](https://arxiv.org/abs/2609.19877)
*Guanhua Chen,Yanting Wang,Wenjing Zhi,Lei Sha*

Main category: cs.CL

TL;DR: 该论文提出 JustMem，将对话历史存储为紧凑的原子记忆，并沿“发现广度”和“读取保真度”两个维度自适应调整记忆访问，以在高效检索证据的同时避免上下文膨胀。


<details>
  <summary>Details</summary>
Motivation: 长期对话记忆需要检索足够证据，但不能盲目扩展语言模型上下文。相关证据可能分散在多个会话中，压缩又会丢失细节，且不同查询需要不同形式的记忆访问。

Method: 将记忆访问形式化为两个维度：发现广度控制证据搜索范围，读取保真度控制以紧凑形式读取还是从原始对话恢复。JustMem 据此存储紧凑原子记忆，并按查询自适应选择 LOOKUP（局部证据）、COMPOSE（扩大发现以处理分布式证据）、REPLAY（提高保真度以处理对细节敏感的证据）。

Result: 在 LoCoMo 和 LongMemEval-S 上，JustMem 在对比的记忆系统中取得最高平均准确率和检索召回率，同时在记忆构建和推理中使用显著更少的生成模型 token。

Conclusion: JustMem 通过自适应记忆访问，在检索充分证据与控制上下文成本之间取得平衡，为长期对话记忆提供了一种高效且有效的方案。
...
### [223] [VākQA: A Benchmark and Evaluation Study for Telugu Spoken Factoid Question Answering](https://arxiv.org/abs/2609.19879)
*Bhavana Akkiraju,Ravi Sastry Kolluru,Sri Charan D,Srihari Bandarupalli,Santosh Kesiraju,Anil Vuppala*

Main category: cs.CL

TL;DR: 本文提出了 VākQA，一个泰卢固语口语问答基准，包含 2,001 个事实型问答对、2.53 小时语音、双语转写和人工验证参考答案，并系统评估了自动评判方法与多种模型在该任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前问答系统在大语言模型推动下发展迅速，但主要集中在高资源语言的文本和口语场景。泰卢固语口语问答基准尚未被探索，且该场景下自动评估的可靠性也缺乏量化研究。

Method: 构建 VākQA 泰卢固语口语问答基准，覆盖六个领域，包含 2,001 个事实型问答对、2.53 小时音频、双语转写与人工验证参考答案；先用人工判断验证自动评估方法，再在输入模态、语言和领域维度上评测闭源与开源模型。

Result: 验证发现 Gemini-as-a-judge 最接近人工评分，但严格程度不一致；开源权重评判模型会系统性惩罚与参考答案表面形式不同但正确的泰卢固语答案。模型评测显示：泰卢固语表述保留翻译中丢失的文化特异性，语音输入引入改变问题含义的语音混淆，级联 ASR-MT 错误会逐步累积。

Conclusion: VākQA 已公开发布，为泰卢固语口语问答提供了基准，并揭示了多语言、多模态问答中评估可靠性与错误传播等关键挑战。
...
### [224] [D-Quant: Driftable Entropy Coding for KV Cache Quantization](https://arxiv.org/abs/2609.19880)
*Yi Su,Hong Liu,Guanghua Yu,Jianchen Zhu*

Main category: cs.CL

TL;DR: 针对 KV cache 内存瓶颈，提出 D-Quant 量化框架，利用熵编码压缩非均匀分布的 KV 值，并通过 drift 机制将变长编码转为固定大小比特流，以支持并行注意力内核。


<details>
  <summary>Details</summary>
Motivation: KV cache 内存随序列长度和批量线性增长，成为 LLM 部署瓶颈。现有固定位宽量化受限于 2^b 个量化级，低位宽下信息损失严重；且 KV 值近似正态分布，固定位宽未利用其非均匀性。熵编码虽能压缩，但变长输出不适合高度并行的注意力内核。

Method: 提出 D-Quant 框架，引入 drift 机制，将每个 token 的熵编码表示转换为固定大小比特流，从而实现规则内存访问和注意力内核内的并行反量化。

Result: 摘要未给出具体实验数据，主要结果是提出 D-Quant 框架，通过 drift 机制解决熵编码变长输出与并行注意力内核不兼容的问题。

Conclusion: D-Quant 为 KV cache 量化提供了一种灵活方案，在利用熵编码压缩非均匀分布的同时，保持固定步长内存布局和并行反量化能力。
...
### [225] [PetriBench: Benchmarking LLM Reasoning over Dynamic State Spaces](https://arxiv.org/abs/2609.19883)
*Pyrros Koussios,Benjamin Jäger,John Hua Yao,Ajay Sridhar,Violet Xiang,Chenhao Li*

Main category: cs.CL

TL;DR: 本文提出 PetriBench，一个基于 Petri 网的紧凑、自包含且可扩展的 LLM 推理基准，用于评估动态状态空间中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 推理基准往往孤立特定技能、依赖外部知识或扩展成本高，难以统一评估推理能力。

Method: 利用 Petri 网建模并发与分布式系统，设计四个任务族，并按结构复杂度生成 Easy、Medium、Hard 三级实例，使用精确真值评估多种专有和开源模型，并分析测试时计算的影响。

Result: 模型准确率随难度上升而持续下降；更难实例暴露更明显的任务特定能力差异；测试时计算能提升性能，但与不同推理任务交互不同；程序化生成使性能随结构复杂度平滑变化。

Conclusion: PetriBench 为探测 LLM 推理的优势、局限和扩展行为提供了统一且可扩展的评估环境。
...
### [226] [Evaluating Communicative Success in Machine-Translated Conversation](https://arxiv.org/abs/2609.19885)
*Faiz Ghifari Haznitrama,Alice Oh*

Main category: cs.CL

TL;DR: 论文提出一个可复用的三层“清单与评判”框架，用于评估口译智能体在真实对话中的交际表现，并构建单轮与多轮基准，强调语义、语用和文化社会层面的成功而不仅是翻译忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译评估多针对孤立句子，主要衡量忠实度，无法判断口译智能体是否真正促成跨语言沟通；因此需要面向会话、覆盖自然度、意图和社会适宜性的评估方法。

Method: 提出三层 checklist-and-judge 评估框架，覆盖语义、语用、文化社会维度，支持单轮和交互式多轮设置；多轮中模拟用户对翻译消息作出回复，并对每轮及整段对话评分。通过受控扰动、跨评委比较和人工标注进行验证。单轮基准评估10种口译设置，涵盖阿拉伯语、孟加拉语、印尼语、韩语，基于5,624个OpenSubtitles场景、12个翻译方向；多轮研究覆盖6个语言对，含脚本和实时模式。

Result: 结果显示从语义到语用再到文化社会层面，表现持续下降；传统机器翻译指标会忽视较强口译系统中的失败。提示消融表明，场景上下文、结构化指令和文化上下文能提升交际成功，但增益因设置而异。

Conclusion: 该工作为会话中的口译智能体提供了评估框架和基准，并指出在现有翻译指标之外，应同样重视交际成功。
...
### [227] [Generalization through Lexical Abstraction in Transformer Models: The Case of Functional Words](https://arxiv.org/abs/2609.19887)
*Giuseppe Samo,Vivi Nastase,Paola Merlo*

Main category: cs.CL

TL;DR: 该论文研究预训练Transformer模型是否像人类一样编码代词、副词等功能词，并探讨模型能否识别词汇化句子与功能化句子之间的句法和语义平行性。


<details>
  <summary>Details</summary>
Motivation: 功能词常替代具体名词或短语，但其编码方式是否支持类似人类的抽象与替换能力尚不清楚；作者希望了解语言模型能否捕捉如“The researchers wrote the paper”与“They wrote it”之间的平行结构。

Method: 将语言学问题映射到预训练Transformer的嵌入空间，比较名词与可替代它们的功能词在孤立形式及平行词汇化/功能化句子中的表示，并探测平行句子嵌入中的共享句法和语义结构，还进行了结构信息蒸馏实验。

Result: 功能词在嵌入空间中比名词更中心化，但也保持独特性，符合其作为多语境占位符的行为；平行词汇化与功能化句子占据不同子空间；仅用功能化或词汇化数据训练不能揭示共享结构，前者因词汇过度一致，后者因变化过多；混合训练后共享结构才出现。

Conclusion: 预训练Transformer对功能词和名词的编码存在系统差异，且不能仅从单一句子类型中直接提取词汇化与功能化句子间的共享结构；混合功能词与词汇化句子有助于显现这种共享的句法和语义结构。
...
### [228] [KoNeoBench: A Curated Evaluation Dataset for LLM Understanding of Korean Neologisms](https://arxiv.org/abs/2609.19916)
...
TL;DR: 本文提出了 KoNeoBench，一个用于评估大语言模型理解韩语新造词的基准。该基准包含 1,785 个 2020 年以来网络新闻中出现的新造词，并由专家进行词典学审查，定义了四项任务，并测试了近期模型与人类基线。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型通常在静态基准上评估，但自然语言会不断产生新词和新义。现有韩语基准主要覆盖既有词汇，对近期词汇变化覆盖不足；且其英语导向设计难以评估韩语中内容词与功能语素能产性结合的类型学特征。

Method: 构建 KoNeoBench：收集 2020 年以来网络新闻中出现的 1,785 个韩语新造词，经专家词典学审查；每条词条提供用法示例、构词分析和词典式定义。基于该资源定义四项任务，并在近期模型上报告结果，同时设置人类基线。

Result: 实验表明，当前大语言模型在还原来源成分、区分语义类别和生成准确释义方面存在明显局限，揭示了近期韩语词汇变化中对现有模型仍具挑战性的具体方面。

Conclusion: KoNeoBench 为评估大语言模型对韩语新造词的理解提供了新基准，并暴露出当前模型在处理韩语近期词汇变化时的不足。该基准已在 https://github.com/bcmilab/ko-neobench/ 公开。
...
### [229] [Before the Arrest: Benchmarking LLMs on Criminal Profiling from Incomplete Evidence](https://arxiv.org/abs/2609.19965)
*Yutong Yao,Yanjie Cao,Guanhua Chen,Xu Yang,Junchao Wu,Zeyu Wu,Lidia S. Chao,Derek F. Wong*

Main category: cs.CL

TL;DR: 论文提出PIJ基准，包含5国2500起真实凶杀案，用于评估LLM在逮捕前刑事调查中的犯罪画像、犯罪过程重建和刑期预测能力，发现其推理任务表现明显不足且存在偏差。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在法律与刑事司法中的应用多集中在已知嫌疑人身份的事后场景，而逮捕前从不完整证据推断嫌疑人特征这一关键挑战尚未被充分探索。

Method: 构建Profiling, Investigation, and Judgment（PIJ）基准，涵盖2500起真实凶杀案，设计三项任务：犯罪画像（从片段现场证据溯因推断嫌疑人属性）、犯罪过程重建（结构化信息抽取）和刑期预测（法律演绎推理），并评估9个强大LLM。

Result: LLM性能随任务从显性事实抽取转向对未知嫌疑人画像的隐性推理而系统性下降；动机、受害者-犯罪者关系等需要推理的类别是主要瓶颈；LLM与人类专家存在显著差距，并在性别、年龄和动机归因上存在普遍偏差。

Conclusion: 从不完整证据进行逮捕前推断仍是开放挑战，当前LLM在该场景下能力有限且偏差明显。
...
### [230] [Intrinsic Sequence-Likelihood Confidence in Retrieval-Dominated Extractive QA: Two Pre-Specified Negatives, and What They Do and Do Not Attribute](https://arxiv.org/abs/2609.19942)
*Gunwoo Lee,Changmin Sung,Sang-Hwan Gwak,Ina Kim,Ji-Young Choi,Kyong-Ha Lee*

Main category: cs.CL

TL;DR: 本文检验在抽取式文档问答中，微调开源语言模型后的置信度信号能否用于触发额外适配或决定答案取舍；在四个7-9B模型家族上，两类用途均失败，检索本身已恢复最佳组合准确率的92-99.8%，置信度信号不足以带来有意义增益。


<details>
  <summary>Details</summary>
Motivation: 问题由包含答案的段落生成，检索能覆盖几乎全部可达到的准确率，因此作者想验证：在一个经过领域语料微调的开源模型上，模型自身的置信度是否能作为控制信号，用于选择需要进一步适配的查询以及决定信任哪些答案。

Method: 在四个7-9B模型家族上微调领域语料，并按预先注册的标准评估两种用途：蒸馏触发器（三步迁移预算）和路由与弃答策略。分析序列似然置信度（AUROC）、标量重校准、token级温度缩放，并进行选择器消融。

Result: 两种用途均失败：蒸馏触发器在四个家族上都未达标，路由与弃答在单模型试点中失败。检索单独恢复最佳组合准确率的92-99.8%，使路由器无显著收益。置信度AUROC为0.65-0.81，适配前后不变，标量重校准无效，token级温度缩放不一致改善。选择器消融未显示置信度项的统计显著下游收益；在Gemma上移除该置信度项反而使选择器通过两项注册标准。

Conclusion: 在检索已接近上限的抽取式文档问答中，模型置信度不是可靠的适配触发或答案信任信号；本文的主要可复用产出是一组预先规定的负面结果及其依赖条件。
...
### [231] [Benchmarking LLM Compliance with China AI Generated Content Regulations](https://arxiv.org/abs/2609.19989)
*Chenrui Cui,Hongye Fang,Lisha Song,Weichao Chen,Yue Zhu,Gang Xu*

Main category: cs.CL

TL;DR: 本文围绕中国AI生成内容合规要求，提出一套评估框架，对20个主流大模型进行合规性与拒答率评测，并建立可统一评估中外大模型的监管基准。


<details>
  <summary>Details</summary>
Motivation: LLM广泛应用带来内容合规风险，已有研究多聚焦英文语境，忽视中文内容合规的复杂性；同时中国现行AI生成内容合规要求需要系统性评估与监管视角。

Method: 设计新评估框架，包含2303个问题，覆盖六个维度，其中含203个自建宪法类问题；采用多个裁判模型基于分层对齐记忆独立生成裁决，评估模型的合规率与拒答率。

Result: 对20个知名LLM的评测显示，国际模型在使用标准中文问题时也表现出较高合规水平；主要差异可能来自与意识形态对齐密切相关的维度。

Conclusion: 研究建立了具有法律依据的监管基准，使全球AI社区能够在统一合规要求下评估中国与非中国LLM，并为理解中国监管格局提供参考。
...
### [232] [DeepSeek-V4.1-Flash: Pushing the Limits of KV Cache Compression](https://arxiv.org/abs/2609.19969)
...
TL;DR: DeepSeek-V4.1-Flash 是一个多模态 MoE 模型，拥有 552B 主干参数，支持百万级 token 上下文，通过 CED 架构、CSA2、FP4 KV 缓存和 SWA Bounded Replay 等设计，大幅降低预填充与 KV 缓存成本，并在文本与多模态智能体任务上取得优于基线的表现。


<details>
  <summary>Details</summary>
Motivation: 长时程智能体使模型工作负载日益偏向输入密集型，虽然长上下文计算成本已有所下降，但预填充仍然昂贵，大型 KV 缓存持续给 HBM、SSD 容量和数据传输带宽带来压力。计算、存储和带宽需求共同构成进一步降低部署成本的主要瓶颈。

Method: 提出 DeepSeek-V4.1-Flash：多模态 MoE 模型，552B 主干参数，支持最多一百万 token 上下文；采用 Causal Encoder-Decoder（CED）架构，解码时每 token 激活 16B 参数，预填充时仅激活 8B 参数；结合 Compressed Sparse Attention 2（CSA2）中的跨层 KV 缓存复用与 FP4 KV 缓存；使用 SWA Bounded Replay 优化持久化 KV 缓存；简化 DeepSeek-V4 架构并引入高效架构扩展；在 45T token 多模态语料上预训练，并进行全面后训练。

Result: 全局 KV 缓存占用降至每 token 890 字节，约为 DeepSeek-V4-Flash 的 1/4；持久化 KV 缓存占用约为 DeepSeek-V4-Flash 的 1/8；尽管 KV 缓存占用显著更小，模型性能仍大幅优于基线，并在多种文本与多模态智能体场景中表现强劲。模型检查点已公开。

Conclusion: DeepSeek-V4.1-Flash 针对计算、存储和带宽瓶颈，通过高效架构与 KV 缓存压缩设计显著降低部署成本，同时提升多模态智能体任务性能，并已发布模型检查点。
...
### [233] [Reading Emotions in the Token Space: Discriminative Adaptation of SpeechLLMs for Emotion Recognition](https://arxiv.org/abs/2609.20081)
*Hasindri Watawana,Sergio Burdisso,Esaú Villatoro-Tello,Manjunath K E,Kadri Hacioglu,Petr Motlicek,Andreas Stolcke*

Main category: cs.CL

TL;DR: 提出一种判别式适配方法，通过分类头读取SpeechLLM最终提示词的隐藏状态，一次前向传播输出情感标签，不修改骨干网络，提升Macro F1并消除幻觉。


<details>
  <summary>Details</summary>
Motivation: 生成式解码器不适合分类任务，可能输出目标集外的标签，并偏向高频类别；希望在同一SpeechLLM中控制比较生成式与判别式推理。

Method: 在SpeechLLM上添加单层线性分类头，读取最终提示词token的隐藏状态，一次前向传播完成分类，不修改骨干网络；单层线性设计提升可解释性，每个情感对应LLM输出token空间中的一个方向。

Result: 在IEMOCAP上，两种SpeechLLM架构均提升Macro F1并消除幻觉，在真实ASR转录上增益最大；分析显示情感方向编码了反映网络文本偏差的间接关联。

Conclusion: 判别式适配能有效提升SpeechLLM情感识别性能并增强可解释性，揭示了生成式与判别式推理的差异以及情感方向与网络偏见之间的关联。
...
### [234] [Design of the IBM Granite 5.0 TurboCTC ASR Model](https://arxiv.org/abs/2609.20104)
*Brian Kingsbury,George Saon,Masayuki Suzuki,Hong-Kwang J. Kuo,Takashi Fukuda,Samuel Thomas,Vishal Sunder,Avihu Dekel*

Main category: cs.CL

TL;DR: 本文介绍了 Granite 5.0 Turbo CTC，一个 4.7 亿参数的仅编码器语音识别模型，在速度与准确率之间取得优异权衡，并在 Open ASR 排行榜上达到速度-准确率帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 研究动机是构建一个在英语短格式 ASR 任务中兼具高准确率和极快推理速度的模型，同时仅使用公开数据训练，并以宽松许可证开放使用。

Method: 模型采用 Conformer 块，并在其中使用带步长深度卷积的金字塔时间下采样、块对角（分块）自注意力，以及来自中间层预测的条件化。训练方面仅使用公开数据，采用 Muon 优化器和平衡数据采样。推理加速包括将 1x1 卷积替换为线性层，并优化 Conformer 块中的注意力计算。

Result: 该模型为 470M 参数仅编码器模型，在 Open ASR 排行榜英语短格式 ASR 任务中处于速度-准确率帕累托前沿，并且比最快竞争对手快两倍。

Conclusion: Granite 5.0 Turbo CTC 在速度与准确率之间实现了优秀权衡，模型可在宽松许可证下使用，并已发布在 Hugging Face 上供下载。
...
### [235] [To Copy or Not to Copy: Controlling Speculative Decoding via Intrinsic Model Signals](https://arxiv.org/abs/2609.20186)
*Roy Eisenstadt,Ido Cohen,Edo Cohen-Karlik,Lior Wolf,Itamar Zimerman*

Main category: cs.CL

TL;DR: 本文提出SwitchSD，一种自适应推测解码框架，通过检测大语言模型内部的真实复制意图，在神经草稿与上下文复制之间动态切换，从而提升推理吞吐。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码在神经草稿与基于复制的草稿之间存在权衡：神经草稿通用性强，复制方法在重复文本场景更快，但容易因表面n-gram重叠产生误判，导致错误触发并降低吞吐。

Method: SwitchSD将复制视为LLM的潜在控制信号，在目标模型内部表示上训练轻量级探针，以高精度识别真实复制意图，并据此动态选择神经草稿（如EAGLE）或基于上下文的复制。

Result: 在Llama和Qwen系列模型上，SwitchSD的吞吐量相比EAGLE3等先进基线最高提升15%，复制意图检测的AUC超过0.99。

Conclusion: SwitchSD将复制从噪声启发式转变为有原则、模型感知的解码机制，有效结合神经草稿与复制方法的优势，提升LLM推理效率。
...
### [236] [Fine-Tuning Models for Biomedical Relation Extraction](https://arxiv.org/abs/2609.20169)
*Claudiu Creanga,Liviu P. Dinu,Daniela Gifu*

Main category: cs.CL

TL;DR: 本文针对生物医学文本中变异-表型关系的自动抽取，评估了预训练模型。在SNPPhenA语料上，微调小型BERT类模型（尤其DeBERTa）接近当前最优；而精心微调的Gemini Pro 1.0在句子级和摘要级任务上均超过现有最优。


<details>
  <summary>Details</summary>
Motivation: 下一代测序产生了大量遗传突变与疾病关联数据，但仅靠人工从海量生物医学文献中提取有意义的关系非常困难，因此需要自动化的关系抽取方法。

Method: 使用预训练模型进行生物医学文本关系抽取，聚焦变异-表型领域；在SNPPhenA语料上微调小型BERT类模型（如DeBERTa），并精心微调Google Gemini Pro 1.0，分别评估句子级和摘要级任务。

Result: 微调小型BERT类模型表现较强，其中DeBERTa接近当前最先进水平；精心微调的Gemini Pro 1.0则在句子级和摘要级任务上均超过了现有最先进水平。

Conclusion: 预训练模型，尤其是经过良好微调的大模型如Gemini Pro 1.0，能有效用于生物医学变异-表型关系抽取，并达到或超越当前最优性能。
...
### [237] [Foundations of Stochastic Lexical Calculus: Semantic Descent and Random Dynamics on Probability Simplices](https://arxiv.org/abs/2609.20207)
*Matthew F Dixon*

Main category: cs.CL

TL;DR: 论文提出一个可观测框架，判断语言模型基于提示词产生的词概率何时能支持可随证据更新的序列状态表示。理论给出语义更新存在唯一的条件，实验显示原始提示条件概率不满足不变性门槛，经提示特定校准后三状态表示可通过稳定性门槛并在未触碰路径上达到0.933覆盖。


<details>
  <summary>Details</summary>
Motivation: 科学系统需要关于有意义状态的不确定性，并能在新证据到来时更新；而大语言模型只产生依赖提示词的词概率，二者之间存在鸿沟。因此需要确定语言导出的概率何时能支持这种序列状态表示。

Method: 理论上定义上下文语言的类型化可测变换，构造最小闭表示，给出语义更新唯一存在的充要条件，并界定不可约非闭性与累积误差；在平均收缩条件下证明概率单纯形上外部随机递归的存在性、唯一性与稳定性。实验上采用冻结实验检验可观测含义。

Result: 原始提示条件概率未通过预设的不变性门槛；经过提示特定校准后，一个共同的三状态表示通过稳定性门槛，并在30条未触碰的八步路径中覆盖28条，覆盖率为0.933，名义水平为0.90。

Conclusion: 语言概率只有在声明操作域内验证了闭性、稳定性和覆盖性之后，才能有条件地支持随机状态表示。
...
### [238] [Before the Warning Comes Too Late: Incremental Phone-Scam Detection from Speech](https://arxiv.org/abs/2609.20223)
*Khang Nhat Hoang Vo,Anh Trac Duc Dinh,Tai Tien Ta,Tho Quan*

Main category: cs.CL

TL;DR: 研究弱监督增量式电信诈骗检测：仅用通话级标签，从原始电话音频流中每2秒更新预测，提出StreamFraudNet，在英文基准上ROC-AUC达0.9953。


<details>
  <summary>Details</summary>
Motivation: 实际诈骗检测需在通话结束前实时预警，但现有方法多依赖转录文本或时间标注，且通常只能事后判断。

Method: StreamFraudNet采用冻结的自监督语音编码器，通过重叠的有界上下文窗口处理音频流，结合循环时序建模，并学习聚合各窗口的潜在分数；训练仅使用通话级弱标签。

Result: 在受控英文基准上ROC-AUC为0.9953，显著优于声学与均值池化基线，与强全局时序模型相当；10秒后首次预测，每2秒更新，速度快于实时；消融实验表明循环时序上下文是主要贡献因素。

Conclusion: 无需转录或时间标注即可从原始语音增量评分诈骗风险，但需引入延迟感知训练以改善早期预测。
...
### [239] [The Public Discourse Corpus (PDC): A Speaker-Attributed Dataset for Valence and Epistemic Modality with Target Speaker Participation](https://arxiv.org/abs/2609.20232)
*Bo Chen*

Main category: cs.CL

TL;DR: 该论文发布了公共话语语料库（PDC），这是首个同时对公众人物访谈语音的情感效价和认识模态进行标注的数据集，并提供了说话人分离与标注方法及开源工具。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏同时标注情感效价和认识模态的公众人物访谈语音数据集，且语料构建中需要确保保留的视频确实包含目标说话人的可分析语音。

Method: 构建包含100位说话人、7个专业领域、998个视频的语料库，经句子分割和过滤后得到186,642句、约310万词；提出目标说话人参与度（TSP）五类标注分类体系，标注者间信度κ=0.616；采用音频优先的说话人分离流程，结合本地Whisper ASR与pyannote说话人分离，将目标说话人轮次与采访者及第三方语音分开，并开源实现。

Result: 发布了标注语料库、标注工具、跨提供商验证样本和完整处理流程；数据集可在Hugging Face获取。

Conclusion: PDC为公众人物访谈语音的情感与认识模态研究提供了新资源，TSP分类体系和音频优先说话人分离流程可作为语料库构建的方法学贡献被广泛采用。
...
### [240] [Lens: Bringing the Right Semantic Perspective into Focus for Training-Free Multimodal Representation Learning](https://arxiv.org/abs/2609.20252)
*Xinran Liu,Shouqian Shi,Yixian Chen,Ruizhi Chen,Xin-Wei Yao,Sheng Zhong*

Main category: cs.CL

TL;DR: 论文提出一种无需训练的表示学习框架 Lens，通过任务导向的语义视角锚定与上下文短语读出，提升大语言模型/多模态大模型在下游任务中的表征质量。


<details>
  <summary>Details</summary>
Motivation: 高质量表示对下游任务至关重要。专用嵌入模型受限于训练数据规模与多样性，而大规模预训练和指令跟随赋予自回归模型选择证据、整合多模态信息并推断语义的能力，为免训练表示学习提供了机会；但现有语义激发方法难以可靠地将提取状态导向下游任务所需的语义视角，导致表示被显著输入内容主导，即语义视角错位。

Method: 提出 Lens 免训练框架，使表示读出任务导向。其包含：语义视角锚定（Semantic Perspective Anchoring），用任务特定读出短语关联任务所需视角，指定后续提取位置的解释角色；上下文短语读出（Contextualized Phrase Readout），将同一短语置于完整输入之后并聚合其 token 状态，从而结合全上下文访问与锚定视角。

Result: 无需参数更新、架构修改或重排序，Lens 在全部 36 个 MMEB 数据集上取得 63.9 的总体 Precision@1，比最接近的同骨干免训练嵌入基线高出 10.2 个百分点。

Conclusion: Lens 生成的表示反映任务条件下的证据整合与推理，而非显著内容的通用摘要，表明任务导向的读出可有效实现免训练表示学习。
...
### [241] [Viveka-Insight: a cross-lingual concept graph and citation-grounded retrieval resource over the complete works of Swami Vivekananda in English and Bengali](https://arxiv.org/abs/2609.20303)
*Tamal Maharaj*

Main category: cs.CL

TL;DR: 本文提出 Viveka-Insight，一个面向辨喜（Swami Vivekananda）英语与孟加拉语著作的双语资源与开源流水线，包含结构解析、跨语言概念图、双语别名库和人工标注数据，并评估跨语言检索、引用完整性与概念抽取精度。


<details>
  <summary>Details</summary>
Motivation: 经典哲学语料对语言资源构成三重挑战：多语言且无平行对齐、词汇与当代读者距离远、涉及文化敏感内容时生成文本必须可验证地有据可依。研究旨在为辨喜作品构建可迁移的双语资源与处理流程。

Method: 构建涵盖九卷英文《Complete Works》和十卷孟加拉文《Vani o Rachana》约1500万字符的非平行双语语料。发布四层资源：结构保持解析（32,694段、168,842句，含深链锚点）；跨语言概念图（8,362个语言无关概念、87,518条段落-概念边、55,872条概念-概念边，以英文标签字符串相等作为跨语言链接键）；双语别名库（60,850个表面形式）；以及200条段落-概念边的人工标注集（3位标注者）。评估包括194对讲座的已知项跨语言检索、30个问题的引用完整性与现代问题桥接审计，以及人工概念抽取研究。

Result: 跨语言检索 Recall@10 双向均为0.86；严格双标注者共识下概念抽取精度为0.60，Cohen's kappa=0.61；置信度>=0.8时精度提升至0.71，同时保留98%含概念段落。孟加拉语精度显著低于英语（0.54 vs 0.68）。

Conclusion: 孟加拉语侧正是跨语言访问所依赖的一半，其概念抽取精度较低暴露出该资源的薄弱环节。该设计可迁移到其他多语言经典语料。
...
### [242] [Schema-Anchored Latent Reasoning for Semantic Parsing-Based Knowledge Base Question Answering](https://arxiv.org/abs/2609.20398)
*Guangze Gao,Zixuan Li,Sikui Zhang,Chunfeng Yuan,Wenjuan Li,Bing Li,Xiaolong Jin,Weiming Hu*

Main category: cs.CL

TL;DR: 提出SALR，一种基于模式锚定的隐式推理方法，用于知识库问答中的逻辑形式构建，通过连续思维延迟模式元素决策，并利用模式反馈提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的语义解析方法在中间推理时过早离散地选择模式元素，导致错误传播并最终生成错误的逻辑形式，尤其是在大规模异构知识库上。

Method: SALR在模型隐藏状态中生成连续思维进行多步推理，延迟显式逻辑形式决策；通过由金标逻辑形式确定性推导的模式轨迹监督，将连续思维与知识库模式元素码本对齐；将对齐后的模式编码融入后续推理步骤输入，提供模式介导的反馈以指导逻辑形式生成。

Result: 在GrailQA和WebQSP上，SALR相比强基线取得一致的整体提升；在GrailQA的组合问题上，比强语义解析基线TIARA高出2.86个F1点；分析表明模式介导反馈影响逻辑形式生成，且隐状态中可恢复模式信息。

Conclusion: SALR通过隐式推理与模式对齐有效缓解了早期离散决策问题，验证了模式介导反馈在逻辑形式生成中的作用，为知识库问答提供了一种有前景的LLM语义解析范式。
...
### [243] [Xeno-Interpretability: Investigating the Alien Minds of LLMs](https://arxiv.org/abs/2609.20408)
*F. Pierucci,M. Bracale Syrnikov,M. Prandi,M. Galisai,F. Giarrusso,P. Bisconti*

Main category: cs.CL

TL;DR: 论文提出“异我表征”与“异我可解释性”概念，主张LLM可能具有并利用人类现有概念无法充分描述的内部表征，并呼吁从寻找人类概念转向刻画模型原生表征结构。


<details>
  <summary>Details</summary>
Motivation: 当前对大语言模型的解释通常依赖人类已有概念，如真实性、拒绝、欺骗、人格、有害性等，但模型内部可能还存在没有对应人类概念的区分，这些区分可能影响行为且无法被现有人类语言充分表达。

Method: 论文区分人类可解释语义空间与异我语义空间，论证LLM内部可能区分的空间远大于有限人类描述所能覆盖的空间；进而将实验识别与语义解释分开，提出可通过可复现定位、几何刻画、因果操纵及行为关联来研究内部表征，并勾勒识别异我表征的实证研究纲领。

Result: 论文表明，一个内部表征即使无法用人类术语充分表达其语义内容，也可以被稳定定位、几何描述、因果干预并关联到下游行为；同时，模型原生表征可能在多智能体交互中传播和稳定，而人类可读通信只能部分揭示它们。

Conclusion: 异我可解释性将可解释性的目标从“在模型中找到人类概念”转向“发现并刻画模型自身固有的表征结构”，这对AI安全与多智能体系统具有重要意义，因为这类表征可能以不可预测的方式影响模型行为。
...
### [244] [Stress-testing Alignment Midtraining](https://arxiv.org/abs/2609.20412)
*Sid Baines,Jonathan Bostock,Maria Angelica Martinez,Andrew Draganov,David Africa,Daniel Tan*

Main category: cs.CL

TL;DR: 本文系统评估了对齐中训练（AMT）的有效性：在最高1100亿参数模型和10亿中训练token规模下，发现AMT仅在简单场景中能引导模型动机，但少量相互竞争的微调数据即可抹消其效果；部分规则若未在中训练或后训练中演示，则难以稳健学习。因此作者认为目前缺乏充分公开证据证明中训练能解决强大AI对齐的核心难题。


<details>
  <summary>Details</summary>
Motivation: 后训练无法覆盖所有部署环境中的期望行为，模型需要泛化到后训练分布之外；对齐中训练被提出作为解决方案，但其有效性缺乏公开证据，因此需要系统检验其背后的假设。

Method: 识别并评估关于中训练的多项假设，进行跨规模实验：模型规模最高达1100亿参数，中训练token达10亿；研究动机模糊场景以及只演示部分规则等对齐相关场景。

Result: 中训练能在简单版本的动机模糊场景中引导模型动机；但若微调数据中只有极小比例暗示竞争动机，AMT效果会被消除。对于希望模型遵循多条规则但只演示部分规则的情况，规则必须在中训练或后训练数据中出现，才能被稳健学习。

Conclusion: 基于这些及其他发现，作者认为目前没有足够公开证据可以确信中训练能够解决对齐强大AI系统所固有的核心困难。
...
### [245] [Edustories: A Collection of Real-world Case Studies from Classroom Practices](https://arxiv.org/abs/2609.20484)
*Michal Štefánik,Jan Nehyba,Jirina Karasova,Martin Fico,Lucie Škarková,Markéta Košatková,David Kosatka*

Main category: cs.CL

TL;DR: 论文发布 Edustories 数据集，包含 1,492 份教师撰写的课堂案例，用于研究集体教学中的 AI 辅助，并评估大语言模型预测教师干预成功的能力。


<details>
  <summary>Details</summary>
Motivation: 以往 AI 教育研究多集中于个性化学生辅导，但全球多数教育实践发生在集体课堂中，因此需要支持集体教学场景下 AI 辅助研究的数据与评估方法。

Method: 构建 Edustories 数据集，收录小学和高中真实课堂中涉及学生挑战行为、教学干预及结果的教师案例；并让四个语言模型家族的最新模型预测教师干预是否成功，与专家评估进行比较。

Result: 当前模型在预测课堂结果方面仍不及人类专家：最强模型准确率为 58%，而人类专家为 64%。

Conclusion: 结果表明 AI 作为在职教师助手仍存在局限，但也展现出新兴潜力。
...
### [246] [Relational Attention for Data-Efficient Language Modeling](https://arxiv.org/abs/2609.20530)
*Adrian Brasoveanu,Ece Takmaz,Jakub Dotlačil*

Main category: cs.CL

TL;DR: 本文提出 Relational BabyLM，将关系注意力和下一潜在预测目标结合到仅解码器 Transformer 中，用于 BabyLM 2026 挑战，在数据受限条件下提升语言模型的结构泛化与效率。


<details>
  <summary>Details</summary>
Motivation: 标准自注意力未显式分离词汇级与关系/结构信息；关系注意力在纯关系任务上数据效率高，但能否迁移到语言建模尚不清楚，BabyLM 的数据受限训练与全面评测是理想试验场。

Method: 用 Dual Attention Transformer（DAT）替代标准自注意力，分离对象级词汇特征与结构/关系信息；比较 full RA、RCA、DisRCA 三种关系注意力；加入 Next-Latent Prediction（NextLat）目标，让隐藏状态增量压缩历史为密集信念状态；并提出基于 RoPE 的无参数符号检索机制。

Result: 架构是结构语言泛化的主导因素，NextLat 目标次要但显著；10M 词时三种关系注意力差别不大，100M 时 full RA 领先。RoPE 符号检索匹配学习式符号库且不增加参数。严格 100M 赛道最佳模型总榜 55 中第 6，NLP 子集第 3；两个最强模型在多数基准上超过 GPT-2，其中之一取得严格赛道最高 EWoK 分数。

Conclusion: 关系注意力与对象级注意力在语言建模中需兼顾解耦与整合；DAT 在 BabyLM 数据受限设置下有效，关系注意力的数据效率可迁移到语言任务，且无参数符号检索可行。
...
### [247] [An Analysis of Training-Free Self-Reported Confidence in Language Models](https://arxiv.org/abs/2609.20541)
*Lukas Meyer,Sofia Rossi,Wei Chen,Thomas Laurent,Yiming Li*

Main category: cs.CL

TL;DR: 论文分析大语言模型报告数值置信度的可靠性，发现直接口头表达的置信度在经基准错误审计后是意外强基线，而三样本一致性较弱，且固定插值无统计可靠增益；置信度对提问方式敏感，并存在相关错误与基准噪声。


<details>
  <summary>Details</summary>
Motivation: 大语言模型能在生成内容时报告数值置信度，但不清楚这种报告是否真正校准，还是只是校准修辞。研究旨在检验其可靠性及不同无训练置信信号的有效性。

Method: 在100个TriviaQA问题上，对两个模型家族分析三种无训练信号：随答案口头表达的置信度、事后P(True)、与额外三次生成的一致性。还进行固定答案的等价提示重问置信度实验，并探索性审计100个带置信度标签的传记声明。

Result: 经基准错误审计后，直接口头置信度达到AUROC 0.956和0.937；三样本一致性明显较弱，为0.765和0.790；与口头置信度的固定插值没有统计可靠收益。一个模型9个错误中有4个、另一个8个错误中有2个获得全样本支持，说明自一致性会放大共同误解。对同一固定答案用等价提示重问，置信度平均变化0.043至0.084，在0.8阈值下翻转4%至9%的决策。传记声明审计仅发现被支持与被反驳声明之间存在 modest 置信度差距。

Conclusion: 有用的自我报告仍对提示方式、相关错误和基准噪声敏感。因此，大语言模型的置信度报告虽有潜力，但不应被简单视为稳定可靠的校准信号。
...
### [248] [Steering the Compass: Aligning Dynamic Psychological Counseling Conversations with Cognitive Behavioral Therapy Strategies](https://arxiv.org/abs/2609.20565)
...
TL;DR: 论文提出 StratCBT，一个面向认知行为疗法（CBT）心理咨询对话的数据集，包含 9,688 个会话、约 25.6 万条话语，并将咨询师回复对齐到八种 CBT 策略，用于提升大语言模型在心理咨询中的策略性决策与生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在心理咨询尤其是 CBT 中取得进展，但 CBT 的成功依赖根据来访者实时心理状态进行动态决策，这一方面在现有研究中常被忽视，限制了咨询灵活性和治疗效果。

Method: 构建 StratCBT 数据集：基于来访者负面思维对其建模，通过 self-chat 生成高质量咨询对话，并引入真实会话作为指导；将每条咨询师回复对齐到八种 CBT 策略之一。随后进行大量实验，评估策略对齐生成的有效性，并使用 LLM 模拟来访者反映真实场景。

Result: StratCBT 包含 9,688 个会话和约 256K 条话语，在通用咨询和 CBT 特定技能上显著超越现有数据集；实验表明策略对齐生成有效，并能在 LLM 模拟来访者场景中提供专业且有效的咨询。

Conclusion: StratCBT 为 CBT 心理咨询中的策略感知对话生成与动态决策提供了高质量数据支持，有助于推动 LLM 在心理咨询中的专业应用。数据集已在 GitHub 公开。
...
### [249] [SAFARI: An Industrial Benchmark for LLM-Assisted Hazard Analysis and Risk Assessment](https://arxiv.org/abs/2609.20584)
*Chenxi Wu,Zimu Wang,Haiyang Zhang,Wei Wang,Zhijie Xu*

Main category: cs.CL

TL;DR: 本文介绍了SAFARI，首个用于ISO 26262下LLM辅助汽车危害分析与风险评估（HARA）的工业基准，包含3000个案例，评估两个任务：开放式危害分析和基于标准的风险评估。实验表明，九种前沿LLM能生成合理的危害描述，但在ISO 26262风险分类上表现不佳，最佳ASIL宏F1仅为0.261。


<details>
  <summary>Details</summary>
Motivation: LLM在安全关键工程中日益受到关注，但其在受监管的功能安全工作流程中的可靠性尚未得到充分探索。

Method: 提出了SAFARI基准，包含3000个去标识化的工业HARA案例；评估两个耦合任务：开放式危害分析和基于标准的风险评估；提出了首个参考锚定的LLM-as-a-judge协议；对九种前沿LLM进行了实验，并测试了思维链提示。

Result: 模型常产生合理的危害叙述，但在ISO 26262风险分类上表现薄弱，最佳ASIL宏F1仅为0.261；思维链提示收益有限，且常降低分类风险评估性能；错误分析将主要失败定位为危害生成中的场景关键上下文遗漏和风险评估中的可控性误判。

Conclusion: 研究指出了专家监督应集中的领域，并表明LLM在安全关键工程中的可靠性仍需提升。
...
### [250] [What Does Privileged Information Add to On-Policy Self-Distillation?](https://arxiv.org/abs/2609.20612)
*XiuYu Zhang,Wei Chow,Junfeng Fang,Zhenkai Liang,Tat-Seng Chua*

Main category: cs.CL

TL;DR: 该论文研究在线策略自蒸馏（OPSD）中，教师模型看到参考答案或解题过程所带来的额外收益，并区分其与无参考蒸馏本身的贡献。


<details>
  <summary>Details</summary>
Motivation: 已有OPSD让语言模型从能看到答案或解题过程的冻结自身副本中学习，但尚不清楚这种特权参考信息相对于蒸馏本身究竟增加了多少价值。

Method: 构建AMPLE-Math，包含5,319道数学题和六种共享同一答案的推理视图；将每种视图与匹配的无参考蒸馏对比；使用思考型教师监督直接回答式rollout，并在Qwen3-1.7B和SmolLM3-3B上评估，还进行教师画像与匹配损失干预。

Result: 无参考蒸馏解释了Qwen3-1.7B在思考型评估下的大部分提升，领域内和外部基准均如此；额外参考收益在Qwen中有限，精修解答最强，而完整轨迹在SmolLM3-3B第50步增加2个百分点。收益依赖学生训练状态；同一检查点下用长思考型rollout替代短直接回答rollout会使收益转为损失。Qwen中改变token级监督也可使行为基本不变。

Conclusion: OPSD可能通过直接回答与思考型推理共享的参数，改善对已有推理能力的调用；特权参考的价值在于促进这种跨模式迁移，而非揭示多少解题过程。
...
### [251] [WiC is Not WSD: A Study on LLMs and Lexical Ambiguity Resolution](https://arxiv.org/abs/2609.20593)
*Yi Zhou,Kiamehr Rezaee,Danushka Bollegala,Mohammad Taher Pilehvar,Jose Camacho-Collados*

Main category: cs.CL

TL;DR: 该研究探讨 Word-in-Context（WiC）对语言模型困难的原因，认为除比较同一词的两个上下文用法外，缺乏明确义项清单和语义粒度也是关键。通过对比 WiC 与传统 WSD，发现提供候选义项可提升 WiC 表现；人类评估表明许多错误源于标签歧义或义项边界不一致，LLM 常因过度细粒度区分而犯错。


<details>
  <summary>Details</summary>
Motivation: WiC 仍是语言模型的难题。作者假设困难不仅来自比较同一词的两个上下文用法，还来自缺少指定相关语义粒度的显式义项清单。研究希望检验显式义项信息能否帮助模型，并理解 WiC 错误的真正性质。

Method: 在相似设置下评估开放 LLM 在 WiC 和传统 Word Sense Disambiguation（WSD）上的表现；在 WiC 中提供候选义项，类似传统 WSD；并进行人工评估，分析模型错误与标签歧义、义项边界差异之间的关系。

Result: 在所有设置中，提供候选义项均提升 WiC 性能。显式义项信息帮助模型做出更一致、更有针对性的判断。人类评估进一步显示，许多表面上的 WiC 错误反映的是标签歧义或模型与标注者义项边界不匹配，而非简单的词汇理解失败。LLM 常过度思考义项区分，导致基于过度细粒度区分而产生错误。

Conclusion: WiC 的困难部分源于语义粒度和显式义项清单的缺失。引入显式义项信息有助于提升并校准模型判断。未来应关注义项边界与标注模糊性，避免模型进行过度细粒度的义项区分。
...
### [252] [Chronicle: Cut-Point Replay for Regression Testing of LLM Agents](https://arxiv.org/abs/2609.20625)
*Tisha Chawla,Susheem Koul*

Main category: cs.CL

TL;DR: 论文提出 Chronicle，通过在智能体运行的非确定性边界处记录不可变信封（envelope）并支持回放，将一次记录的失败事件转化为可在 CI 中运行的回归测试；其核心操作 cut-point replay 可选择性地从记录中重放部分边界、其余边界用新代码实时执行。


<details>
  <summary>Details</summary>
Motivation: LLM 响应具有非确定性，且智能体依赖会变化的外部工具状态与多步轨迹，导致失败难以复现。现有记录-回放工具仅用于追踪（trace）或评分，无法用于针对代码改动的测试。

Method: 在非确定性边界处将智能体运行记录为不可变信封；提出 cut-point replay：指定一部分边界从记录中服务，互补部分用新代码实时执行，从而把已记录的事故变成 CI 中的回归测试；并在 6 个含模拟模型边界的记录失败基准及变异研究上评估。

Result: 记录开销为每次跨越 23 μs（约为假定 300 ms 模型调用的 0.008%）；完整回放零模型调用，且在 20 次重复中位稳定；cut-point 测试对 6 个事故均在错误代码上失败、在受保护与良性改动上通过；在受保护工具的变异研究中，cut-point 测试能捕获所有让已记录不安全动作通过的变异体，而将所有边界打桩（stub）的基线在同一断言下捕获率为零。

Conclusion: Chronicle 能高效、稳定地记录与回放智能体运行，并使 cut-point replay 成为一种有效的回归测试手段，代码与基准已公开于 https://github.com/theagentplane/chronicle。
...
### [253] [HerHealthEval: Evaluating Multilingual and Register-Sensitive Understanding of Women's Health Communication](https://arxiv.org/abs/2609.20684)
*Hassan Saeed Hassan Albattra,Mazen Mohammed Bahgat,Rahatara Ferdousi,Hana Essam Sayed Ahmed Amrya,Mariam Mousa*

Main category: cs.CL

TL;DR: 论文提出 HerHealthEval，一个用于多语言女性健康沟通理解的可控评估框架，覆盖英语、法语和现代标准阿拉伯语，并通过六种沟通形式测试模型对关切分类、风险校准、澄清行为、解析合规性与跨形式一致性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多用于医疗沟通，但现有评估多关注回答质量，并默认模型已正确理解用户关切，忽略了语言、表达方式差异及信息不足时的澄清需求，可能掩盖安全相关失败。

Method: 构建 HerHealthEval：每个临床案例提供英语、法语、现代标准阿拉伯语的匹配版本，包含规范、临床、外行、间接/模糊、情绪担忧、故意信息不足六种沟通形式；前五种保留相同临床信息，最后一种故意省略细节以测试是否需澄清。评估多语言指令模型及 QLoRA 适配变体在关切分类、风险校准、澄清行为、解析合规和跨形式一致性上的表现。

Result: 聚合准确率和一致性会掩盖安全相关失败。多语言适配模型在语言不对称风险监督下，法语和阿拉伯语的漏诊/低估风险（under-triage）达 0.994。使用源数据派生、语言不变的 风​​险标签进行受控再适配后，法语和阿拉伯语的 under-triage 分别降至 0.572 和 0.558。

Conclusion: 稳健的多语言医疗评估必须显式测试语域变化、不确定性处理，以及适配标签的来源与语言不变性。
...
### [254] [UniPolicy: Unified Objective-Specific Policies for Generative Search Advertising](https://arxiv.org/abs/2609.20630)
*Kun Yao,Yuhang Zhou,Yichi Zhang,Zeliang Tong,Shengri Xue,Haitao Wang,Siyu Lu,Qianlong Xie,Xingxing Wang*

Main category: cs.CL

TL;DR: 提出UniPolicy，一种面向多目标的多策略对齐框架，用于搜索广告，通过分层参数解耦和多策略束搜索平衡相关性、点击倾向与商业价值，离线与在线实验均取得提升。


<details>
  <summary>Details</summary>
Motivation: 现有搜索广告系统通常仅对齐单一商业奖励（如eCPM）或简单融合多奖励，难以兼顾异质目标，且梯度竞争易导致全局次优，需同时优化用户体验与商业价值。

Method: UniPolicy采用目标特定前缀token、稀疏MoE-LoRA路由和目标特定残差FFN，在共享主干中分层解耦参数，为不同目标提供差异化策略表达空间；构建多阶段行为反馈的成对偏好，补充曝光未点击样本的相对偏好，增强点击候选优势；推理时支持并行、业务可定制的多策略束搜索，在固定检索预算下灵活分配各目标候选配额。

Result: 离线实验表明UniPolicy在多个指标上均衡提升并保持检索质量，优于单目标强化学习和朴素奖励融合基线；在真实搜索广告系统7天在线A/B测试中，CTR提升0.71%，RPS提升1.58%，广告收入提升1.32%，服务延迟稳定。

Conclusion: UniPolicy能有效平衡搜索广告中的多目标，兼顾用户体验与商业价值，并具备实际部署可行性。
...
### [255] [Summarization Bias: The Directional Collapse of Objective Projection into Told-Mode Labels in Large Language Models --- A Conceptual Framework and Registered Test Protocol](https://arxiv.org/abs/2609.20712)
*Levent Bulut*

Main category: cs.CL

TL;DR: 本文提出并操作化“摘要偏差”：大型语言模型倾向于将叙事意义表示为抽象摘要标签，而非生成该意义的可重构推断结构。该偏差假设存在于生成与评价两种机制中，其中评价机制影响更大。


<details>
  <summary>Details</summary>
Motivation: LLM日益充当评判者与奖励模型，若其偏好“讲述”模式而非“展示”模式，可能对写作施加选择压力，使文本趋向扁平化直白陈述，损害需要读者重构的叙事效果。

Method: 在Bulut Doctrine的讲述-展示轴上定义构念；将摘要偏差与LLM-as-judge偏差进行对比；重新解读一项已完成的独立信度研究作为方向性证据；预注册一个双机制检验，并设定放弃该构念的决策规则。

Result: 论文不声称该偏差已获验证。重读独立信度研究仅提供与其一致的方向性证据；双机制检验尚待预注册实施。评价机制被认为更关键，因为它可能促使模型奖励直白表达并低估展示型抑制。

Conclusion: 摘要偏差目前仍是待检验构念。若预注册检验结果未满足决策规则，则应放弃该构念；若成立，则需关注LLM作为评判者时对叙事风格的系统性影响。
...
### [256] [On-Demand Attention: Language Models Know When to Recall](https://arxiv.org/abs/2609.20734)
*Haibo Feng,Ruiqi Liang,Hanyang Peng,Shiqi Yu*

Main category: cs.CL

TL;DR: 本文提出按需注意力（ODA），一种局部优先的解码方法，通过轻量级召回头预测全局注意力的收益，从而在生成长文本时选择性调用全局注意力，减少不必要的全局读取并加速推理。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理中，全注意力解码在每一步都读取不断增长的历史，即使这对下一步预测未必有益，导致效率低下。作者发现预训练模型在全局读取前的解码状态已包含预测该收益的信息，因此希望让模型自主决定何时需要全局注意力。

Method: ODA仅训练一个轻量级召回头，保持预训练权重不变，并保留完整历史KV缓存以供未来召回。召回头根据预测收益动态决定是否调用全局注意力。作者还在vLLM中实现GPU端条件执行，将减少的全局读取转化为实际解码加速。

Result: 在Qwen和Gemma模型（包括混合注意力骨干）上的实验表明，选择性召回能够恢复局部注意力下损失的大部分性能，同时显著减少全局读取，并在长上下文长度下实现相对全注意力的实际解码加速。

Conclusion: 研究支持一种长上下文推理范式：预训练模型能够引导自身对已保留信息的访问，在保持性能的同时提升推理效率。
...
### [257] [dQwen3.5: Hybrid-Attention Diffusion Language Models](https://arxiv.org/abs/2609.20751)
*Anton Xue,Litu Rout,Aditya Akella,Adam Klivans,Sujay Sanghavi,Sanjay Shakkottai*

Main category: cs.CL

TL;DR: 该论文研究是否可将包含 Attention 与 RNN 混合架构的预训练自回归模型适配为扩散语言模型，并在 Qwen3.5 上构建了 dQwen3.5 系列。


<details>
  <summary>Details</summary>
Motivation: 将预训练自回归模型适配为扩散语言模型是一种低成本路径，但现有方法多基于全注意力 Transformer。随着自回归建模转向 Attention 与 RNN 交错的混合架构，RNN 的结构因果性使其难以双向化，这成为适配扩散语言模型的障碍。

Method: 作者在 0.8B、2B、4B、9B 规模上对 Qwen3.5 进行适配，构建 dQwen3.5 家族，并与全注意力对照模型比较训练效率、任意顺序解码行为和并行解码性能。

Result: 混合骨干可以作为高效的适配起点：相比全注意力对照，混合模型达到相同训练损失约只需一半 token。不同规模下，dQwen3.5 在任意顺序解码行为上接近全注意力扩散语言模型，并在并行解码下表现强劲。

Conclusion: 混合 Attention-RNN 架构虽存在因果性障碍，但仍可有效适配为扩散语言模型，并成为扩散语言模型的高效起点。
...
### [258] [Harm Laundering in GPT Models: Evidence That Gender Discrimination Is Transformed Rather Than Reduced Across Safety-Trained Generations](https://arxiv.org/abs/2609.20779)
*Sarah Wyer,Sue Black,Noura Al Moubayed*

Main category: cs.CL

TL;DR: 论文提出“危害洗白”现象：大型语言模型安全评估中，表面毒性分数下降并不意味着危害减少，而是歧视性内容被转化、重组或转移。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估依赖表面形式分类器，显示跨模型代际危害分数下降；作者认为该方法系统性不完整，可能遗漏被转化而非被移除的歧视内容。

Method: 分析15个模型（GPT-2至GPT-5）共45万条性别导向补全，涵盖三种人口统计条件；结合主题聚类、情感分析、独立分类器及REGARD/Detoxify等表征危害指标，并形式化“危害洗白”三标准测试和三阶段检测协议。

Result: GPT-2中针对女性的性暴力主题聚类到GPT-4消失，而针对男性的补全获得照护、情感范围、盟友身份等正面表征，女性补全没有；GPT-5中Topic 5将乳腺癌框定为男性权利辩论，女性导向输出无对应聚类，且三个独立分类器判其为非毒性。情感分数在GPT-4反转：早期贬低女性，后期过度矫正。GPT-4对齐边界处，女性导向主题多样性相对男性下降36%（W/M=0.58，GPT-2为0.91）。REGARD表征危害差异与发布时间相关（ρ=+0.55，p=.034），Detoxify不相关（ρ=-0.23，p=.42）：毒性分数下降时表征危害上升。

Conclusion: 在OpenAI GPT谱系内，毒性分数降低不能作为危害减少的充分代理指标；需采用形式化危害洗白测试与三阶段检测协议评估生成模型的真实危害。
...
### [259] [RetireOPD: Self-Retiring On-Policy Distillation for Agentic Reinforcement Learning](https://arxiv.org/abs/2609.20784)
...
TL;DR: 提出RetireOPD（自适应退出的自策略蒸馏）：先用环境奖励训练解耦的技能条件教师，再以RL+OPD联合训练无技能学生，并让学生在与教师差距不再缩小且达到教师成功率的设定比例后自行退出蒸馏，仅保留RL训练。


<details>
  <summary>Details</summary>
Motivation: 多轮智能体RL每轨迹仅有一个标量奖励，监督稀疏，因此常用自策略蒸馏从具备特权技能的教师提供token级密集监督。但作者发现两个问题：特权信息本身并不总能让教师可靠；教师监督的收益依赖于训练阶段，固定的蒸馏计划并不理想。

Method: RetireOPD首先用环境奖励优化一个解耦的、技能条件化的教师模型；然后让学生（无技能）同时接受RL和OPD训练。核心机制是自适应退出（Adaptive Retirement）：不预设蒸馏时间表，当师生差异不再缩小且学生达到教师成功率的某一目标比例时，学生主动丢弃教师，之后仅用RL继续训练。

Result: 在Qwen2.5系列1.5B至7B模型上，RetireOPD将ALFWorld成功率相比RL基线提升14.1%至18.8%，WebShop准确率提升11.8%至19.0%，并在所有设置下超过其技能条件教师。

Conclusion: 自适应退出的自策略蒸馏能有效让无技能学生内化教师技能，并最终超越教师；相比预设蒸馏计划，按师生差距和成功率自动决定退出时机更为有效。
...
### [260] [JEPA-Anything: Learning Predictive Models across Different Worlds](https://arxiv.org/abs/2609.20800)
...
TL;DR: 论文提出 JEPA-Anything，一个基于正交预测分解（OPF）的域无关世界建模框架，并在七个领域验证其预测、干预和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型通常局限于特定领域，作者希望探索是否存在一种通用学习原则，能够支撑跨 radically different systems 的世界建模。

Method: 扩展联合嵌入预测架构（JEPA），提出正交预测分解（OPF）：将潜在目标分解为互补因子，通过专用路径学习，并在共享预测设计中重组。评估覆盖视觉、生物、临床轨迹、控制、分子动力学、物理场和天气七个领域，任务包括表征学习、干预预测、分布外泛化和长时程动态。

Result: 相比匹配的 JEPA 基线，JEPA-Anything 在全部 10 个 dynamics 任务上提升指标；在 Interventional Pong 上将单干预预测误差降低 34.8%；在四个分子系统中取得最低的一步和 100 步分子误差；生物干预提名在细胞共培养、患者来源类器官、肿瘤片段和小鼠中获得实验支持；潜在轨道模式恢复 Keplerian 标度指数，拟合斜率为 -1.4991。

Conclusion: 结果支持一种跨异质世界的共同因子化预测原则，将世界建模与干预以及有实验支撑的科学发现联系起来。
...
### [261] [Unifying Models of Intergroup Hostility in Online Discourse](https://arxiv.org/abs/2609.20808)
*Patrick Gerard,Julia Mendelsohn,Kristina Lerman*

Main category: cs.CL

TL;DR: 研究整合六种群体敌意理论，利用2024年美国大选期间TikTok、Truth Social和Twitter/X的286万帖子建模，揭示敌意修辞的结构锚点与时间顺序。


<details>
  <summary>Details</summary>
Motivation: 敌意言论会正常化排斥、加剧极化和政治暴力；但相关社会科学理论长期平行发展，解释常不一致且少在真实话语中相互检验，导致对敌意修辞机制的理解碎片化。

Method: 使用2024年美国总统选举期间TikTok、Truth Social和Twitter/X的286万帖子，在统一实证框架中建模六种群体敌意理论机制：边界建构、威胁建构、替罪羊、负面评价、非人化和行动导向。

Result: 结构上，边界建构和威胁建构是系统锚点；时间上呈现规律顺序：边界建构、贬损和行动导向较早，非人化和威胁建构较晚，替罪羊最晚。

Conclusion: 将理论框架映射到真实话语，有助于桥接社会科学传统间的分歧，并为计算社会科学超越单一标签检测、建模群体敌意修辞提供更清晰的实证基础。
...
### [262] [Embedding Models Measure in Peculiar Ways](https://arxiv.org/abs/2609.20821)
*Juri Opitz,Andrianos Michail*

Main category: cs.CL

TL;DR: 研究嵌入空间是否能反映质量、距离、时间和体积等物理测量，发现嵌入对物理测量的建模很弱，且受表层字符串相似性强烈影响，重校准相似度也难以显著改善对齐。


<details>
  <summary>Details</summary>
Motivation: 嵌入空间定义了语义相似性与距离，但尚不清楚它们是否能反映具有唯一客观语义等价和距离的物理测量概念。

Method: 分析嵌入空间对质量、距离、时间和体积等物理测量的表示，考察其与客观语义等价和距离的一致性，并尝试重校准相似度。

Result: 物理测量在嵌入空间中仅被弱建模，呈现出较特殊的测量模式；嵌入表示强烈受表层字符串相似性影响，重校准相似度并未显著改善对齐。

Conclusion: 嵌入表示不足以可靠反映物理测量的客观语义结构，表层字符串相似性是重要干扰因素，仅靠相似度重校准难以解决该问题。
...
### [263] [AthenaZero: A low-inertia, bimanual robot for dynamic manipulation](https://arxiv.org/abs/2609.19194)
...
TL;DR: AthenaZero 是一种低惯性双臂机械臂，采用准直驱驱动与传动远程化设计，在保持控制能力的同时实现接近人类的末端有效质量，并完成投掷、接球和击球等棒球式动态任务。


<details>
  <summary>Details</summary>
Motivation: 传统机械臂惯性大，难以在毫秒级人类时间尺度上完成高加速度动态操作；研究旨在降低惯性且不牺牲控制权威，以实现快速、透明的动态交互。

Method: 使用准直驱执行器和传动远程化技术设计双臂机械臂；通过降低惯性并利用固有扭矩透明性，提升动态操作能力，并在棒球启发任务中验证。

Result: AthenaZero 的末端有效质量接近人类，比传统机械臂低约一个数量级；投掷速度超过 30 m/s，接球和击球在 7.3 m 距离内速度超过 14 m/s，并完成机器人对机器人及人对机器人的击球练习和接球游戏。

Conclusion: 该系统在高加速度动态任务中有效且适应性强，适合需要毫秒级响应和复杂交互的动态操作场景。
...
### [264] [DITTO: Dexterous Interface for Transparent TeleOperation](https://arxiv.org/abs/2609.19196)
*Joaquin Palacios,Katelyn Lee,Cheng Zhang,Zhanpeng He,Matei Ciocarlie*

Main category: cs.RO

TL;DR: 提出DITTO，一种通过7自由度机械手与运动学等效外骨骼的解剖学协同设计，统一手持式野外数据采集与带关节力反馈的双边遥操作，解决高自由度手部操作数据采集中的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 高自由度手部操作数据采集困难：接口需捕捉丰富手部运动并呈现精确操作所需的接触交互。现有方法存在权衡：遥操作保证部署一致性但缺乏力反馈；手持式野外系统提供自然力透明性但部署时存在视觉具身差距。

Method: 解剖学信息驱动的协同设计：开发7自由度灵巧机械手与运动学等效的电动外骨骼；外骨骼与机械手之间采用1对1执行器映射；在同一平台上实现手持式野外数据采集和带关节级力反馈的双边遥操作。

Result: DITTO外骨骼覆盖操作者自然食指到拇指的工作空间；通过接触丰富任务上学习到的策略展示了DITTO的灵巧操作能力。

Conclusion: DITTO通过统一的硬件与映射设计，解决了数据采集与遥操作之间的力反馈和视觉具身权衡，为高自由度手部操作提供兼具透明力反馈与部署一致性的平台。
...
### [265] [ULOHA: An Underwater Bimanual Robot System for Robot Learning](https://arxiv.org/abs/2609.19200)
*Masato Kobayashi,Takeru Tsunoori*

Main category: cs.RO

TL;DR: 提出 ULOHA 水下双臂机器人学习平台，结合定制主从硬件与 LeRobot 软件扩展，支持遥操作、多视角感知、演示采集、策略训练和自主部署，并验证多种水下双臂协调行为及算法表现。


<details>
  <summary>Details</summary>
Motivation: 水下视觉运动策略学习多集中于单机械臂，双臂模仿学习主要在空中研究，缺少面向水下双臂机器人学习的统一实验平台与评估。

Method: 设计主从式双臂硬件，扩展 LeRobot，集成遥操作、多视角传感、演示收集、策略训练和自主部署；在平台上评估 ACT、Diffusion Policy 和 VLA 模型 SmolVLA，并研究气泡扰动、浮力物体运动、动作执行时域和实时分块；另开展单臂空水策略迁移研究。

Result: 真实机器人实验展示了臂间传递、共享物体操作和浮力驱动拦截等协调行为；分析了空中学习方法在水下的表现；单臂研究表明，涵盖空气与水中演示的数据在测试条件下可支持两种介质中的执行。

Conclusion: ULOHA 为研究水下双臂机器人学习以及水下环境中耦合的感知与物理效应提供了统一实验平台。
...
### [266] [REACT: A Fully Spiking State-Space Model for Real-Time Event-Driven Temporal Perception](https://arxiv.org/abs/2609.19204)
*Geoffroy Keime,Nicolas Cuperlier,Benoit R. Cottereau*

Main category: cs.RO

TL;DR: 提出REACT，一种全脉冲状态空间模型，直接逐个处理原始事件，无需时间累积，实现低延迟的事件驱动时间感知。


<details>
  <summary>Details</summary>
Motivation: 动态环境中机器人需要连续演化的视觉感知，事件相机具有微秒级时间分辨率，但现有学习方法将事件累积成帧或时间箱，引入积分延迟，限制快速反应。

Method: REACT使用复数脉冲神经元C-SiLIF，其连续时间动力学由物理事件间隔驱动，内部状态以单个事件的时间分辨率演化。在手势识别和碰撞时间估计（TTC）上评估，使用全视场事件流，无需目标边界框或定位输入。

Result: 在EvTTC上，REACT达到9.59%相对TTC误差，端到端推理延迟4.6 ms，比最佳学习方法仅差0.15个百分点，但无需目标先验。平均接近速度下，延迟对应车辆运动仅4 cm，而最快竞争学习方法为1 m。支持随时TTC预测、零样本迁移到不同驾驶序列、INT8量化，能耗从18.5 mJ降至2.8 mJ每32768个事件。
...
### [267] [4D Radar Perception Algorithms for Autonomous Driving: A Review](https://arxiv.org/abs/2609.19216)
*Xumin Wu,Jun Zhou,Jilin Mei,Chen Min,Yu Hu*

Main category: cs.RO

TL;DR: 一篇关于4D毫米波雷达感知算法的综述，按感知任务与算法演进组织，涵盖从雷达基础、目标级感知到动态场景理解，并比较不同学习范式与数据集评估。


<details>
  <summary>Details</summary>
Motivation: 4D毫米波雷达感知算法近年快速发展，任务从信号处理、目标检测扩展到语义分割、运动估计、占据预测和动态场景重建，领域需要系统梳理与任务导向的综述。

Method: 综述方法：先介绍雷达基础、数据表示和质量增强方法；再按目标级感知、运动与定位、局部与稠密空间感知、动态场景理解展开；比较纯雷达学习、多模态融合、跨模态监督与知识蒸馏；关注高程、多普勒和雷达物理先验的利用；总结现有数据集的任务覆盖、输入数据、标注和评估协议。

Result: 系统梳理了4D毫米波雷达感知的任务演进、算法路线、数据与评估支持，并明确了不同研究方向的经验基础。

Conclusion: 该综述提供了从稀疏目标感知到动态空间理解的任务导向视角，并讨论了面向自动驾驶的4D雷达感知的共同挑战与未来方向。
...
### [268] [Grasping by interconnection: robust closing motions from coarse object templates](https://arxiv.org/abs/2609.19228)
*Julien Vanderheyden,Guillaume Drion,Fulvio Forni,Pierre Sacré*

Main category: cs.RO

TL;DR: 本文提出一种基于粗略物体模板、人类抓取类型、物体中心交互和柔顺滑动接触的抓取运动规划器，并在Shadow灵巧手上验证其对物体尺寸和位姿误差具有较强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 灵巧手常需抓取形状、尺寸和位姿仅近似已知的物体，但现有抓取规划器通常依赖精确物体模型或依赖反馈纠错；闭合动作本身能容忍多少不准确性尚不明确。

Method: 规划器基于四项原则：使用粗略物体模板、采用人类抓取类型、进行物体中心交互、以柔顺滑动接触代替预设接触点；通过虚拟模型控制实现，并在Shadow灵巧手上评估。

Result: 无反馈条件下，规划闭合运动可容忍约1厘米尺寸误差以及数厘米和数十度的位姿误差，在27个测试条件中的25个优于最先进的数据驱动规划器；对80个日常物体抓取成功率为82.5%，并在自主流程中取得成功。

Conclusion: 鲁棒性可以设计到闭合运动本身中，而不仅依赖反馈；该规划器为不确定环境下的可靠操作开辟了路径，后续将结合物理手上的自适应反馈控制。
...
### [269] [Learning Safe Humanoid Navigation from Reduced Order Models](https://arxiv.org/abs/2609.19272)
*William D. Compton,Zachary Olkin,Ryan Bena,Aaron D. Ames*

Main category: cs.RO

TL;DR: 本文提出 RoM-Nav，一种用于人形机器人多楼层、多层级无地图导航的分层强化学习策略，并在 Unitree G1 上实现了超过 10 米垂直位移和 100 米路径长度的导航。


<details>
  <summary>Details</summary>
Motivation: 标准单阶段强化学习导航流程难以扩展到多层级和多楼层地形，主要瓶颈在于人形机器人与复杂地形（如楼梯）的交互非常困难。

Method: 将导航问题分解为两步：先训练一个基于降阶动力学但使用完整 3D LiDAR 观测的策略，用于复杂多楼层地形导航；再利用该导航知识启动一个基于全阶人形动力学的策略，并在回路中冻结运动控制策略。此外，对导航策略输出应用泊松安全滤波器，以在分布外障碍物下恢复安全性。

Result: 所得 RoM-Nav 策略在 Unitree G1 上实现了无地图多楼层导航，测试中垂直位移超过 10 米、路径长度超过 100 米。泊松安全滤波器在不降低导航成功率的情况下，提升了分布外障碍物下的安全性。

Conclusion: 通过将导航分解为降阶与全阶两阶段学习，并引入安全滤波，可以使人形机器人完成复杂多楼层无地图导航，克服单阶段强化学习难以扩展的问题。
...
### [270] [OHRID-Retail: An Open Multimodal Dataset of Human Activity in Retail Environments](https://arxiv.org/abs/2609.19302)
*Xiangrui Wang,Yuetong Wu,Jalen Beeman,Robert Cook,Yu Gu,Nathanial Pearson,Trevor Smith,Read Hayes,Boyi Hu*

Main category: cs.RO

TL;DR: 本文介绍了OHRID Retail，一个开放的人类中心多模态数据集，用于研究人类在移动机器人共享环境中的行为，特别是零售场景下的货架拣选任务。


<details>
  <summary>Details</summary>
Motivation: 目前描述人类在移动机器人共享环境中行为的开放数据集仍然有限，尤其是涉及运动、伸手、物体处理和机器人引导移动的零售活动。

Method: 数据集收集自16名健康成年人，在三种条件下（无机器人、低速机器人引导、高速机器人引导）执行模拟货架拣选任务，每种条件每人完成两次试验。使用17个Xsens Awinda惯性传感器记录全身运动学，使用10个Delsys Trigno表面肌电传感器测量肌肉活动。

Result: 描述性分析表明，在不同机器人交互条件和身体部位之间，全身运动强度和肌肉激活存在差异。

Conclusion: OHRID Retail提供了公开可用的原始记录、处理后的测量数据、文档和可重复分析资源。该数据集可支持人类活动识别、多模态传感器融合、职业生物力学、人因工程、人类感知机器人导航以及零售和相关共享环境中的人机交互研究。
...
### [271] [GAVEL: Graph World Models for Verified and Efficient Long-Horizon LLM Task Planning](https://arxiv.org/abs/2609.19315)
*Ruiyang Wang,Hao-Lun Hsu,Swarajh Mehta,Jiwoo Kim,Zhihao Dou,Miroslav Pajic*

Main category: cs.RO

TL;DR: 提出 GAVEL 框架，利用显式图世界模型对 LLM 生成的长程机器人规划进行验证与修复，从而提升任务成功率与规划效率。


<details>
  <summary>Details</summary>
Motivation: LLM 虽可用于长程机器人规划，但其生成的计划常不满足具身约束、难以从规划错误中恢复，并且在部分可观测环境下推理效果不佳。

Method: 构建显式图世界模型，表示物体关系、动作前置条件与效果，以及未观测物体位置的概率信念；在执行前预测 LLM 动作后果、检测违规并修复可由世界模型直接推导的错误，仅将 LLM 重规划保留给需要语义推理的错误。对于多任务指令，基于物体位置分布重排剩余子任务，以最小化期望搜索成本。

Result: 在 BEHAVIOR-1K 的 100 个单任务和 500 个多任务指令上评估，使用 Qwen3-8B 时，单任务成功率从 41.2% 提升至 91.8%，多任务成功率从 19.9% 提升至 92.6%；分布信念推理相比静态版本减少约 5.4% 的移动距离。

Conclusion: 显式图世界模型作为外层框架，可显著提升长程具身规划在紧凑型与前沿 LLM 能力下的可靠性和效率。
...
### [272] [A Morphing Aerial Robot With Thruster-Integrated Flexible Continuum Links for Shape Adaptive Aerial Manipulation](https://arxiv.org/abs/2609.19328)
*Eri Sawada,Kazuki Sugihara,Ayano Miyamichi,Kunio Kojima,Kei Okada*

Main category: cs.RO

TL;DR: 本文提出一种带柔性连续体连杆和分布式旋翼的可变形空中机器人，兼具软体机械臂的环境适应性和扩展的可行力旋量空间，并实现稳定飞行与多种空中操作任务。


<details>
  <summary>Details</summary>
Motivation: 空中操作对扩展空中机器人应用至关重要，但现有研究难以同时实现两个目标：利用软体机械臂获得高环境适应性，以及沿机械臂分布推进器来扩展可行力旋量空间。

Method: 设计可变形、旋翼分布的空中机器人，采用柔性连续体连杆作为软体机械臂，使其被动顺应环境形状；沿连续体分布推进器以扩展推力力旋量空间，并让末端执行器施加较大交互力。为抑制轻质连续体连杆振动，开发兼具高扭转和垂向刚度的复合板簧结构及抑振控制方法。

Result: 实验展示了稳定飞行以及多种空中操作任务，验证了所提机器人实现柔性连杆与集成推进器结合进行空中操作的能力。

Conclusion: 据作者所知，这是首个使用带集成推进器的柔性连杆实现空中操作的工作，表明该方案可同时提升环境适应性和操作力输出能力。
...
### [273] [SemSafe-3DGS: Semantic Risk-Aware Active Navigation in Uncertain 3D Gaussian Splatting Maps](https://arxiv.org/abs/2609.19330)
*Amirhossein Mollaei Khass,Athanasios Cosse,Nader Motee*

Main category: cs.RO

TL;DR: 提出一种在带属性3D高斯地图中用于自主机器人导航的语义风险感知安全主动感知框架，通过类别风险权重调节AVaR碰撞间隙，并在统一CBF-QP中融合安全约束与主动感知。


<details>
  <summary>Details</summary>
Motivation: 现有安全规划主要基于几何进行推理，忽略场景元素的语义后果，导致几何相似但语义不同的元素可能引发相近的控制响应。

Method: 在带属性的3D高斯地图中，用语义属性通过类别相关风险权重调制Average Value-at-Risk碰撞间隙模型，使安全关键高斯基元在复合屏障中影响更大；将加权间隙聚合为控制屏障函数，并设计轨迹相关主动感知屏障，促进沿机器人预期运动降低几何地图不确定性；二者集成到统一CBF-QP中，安全作为硬约束，信息获取在与安全或任务进度冲突时松弛。

Result: 实验表明该方法能高效施加安全约束、通过主动感知改善导航、实现语义相关的轨迹适应，并在Ackermann动力学真机上执行。

Conclusion: 该框架实现了语义风险感知的安全主动感知导航，兼顾碰撞安全与信息获取，并支持真实机器人部署。
...
### [274] [Dynamic-LIVO: A Dynamic-Aware LiDAR-Inertial-Visual Odometry System Using Spatio-Temporal Normals](https://arxiv.org/abs/2609.19336)
*Zhixin Zhang,Samuel Ahiwe,Matthew Hale,Liang Zhao,Pawel Ladosz*

Main category: cs.RO

TL;DR: 提出 Dynamic-LIVO，一种动态感知的 LiDAR-Inertial-Visual Odometry 系统，用于动态环境中的鲁棒状态估计和静态彩色建图。


<details>
  <summary>Details</summary>
Motivation: 动态物体会污染 LiDAR 和视觉观测，影响状态估计与建图；而现有时空法向分析在新观测或空间稀疏区域不可靠。

Method: 利用时空（S-T）法向分析识别动态 LiDAR 点，并将分类传播到 LiDAR-惯性与视觉-惯性更新中，避免动态测量影响状态估计和建图；同时提出时间延迟 S-T 法向估计策略，延迟分类约束不足的点，待更多观测后再评估。

Result: 在公开数据集和自采数据集、多种传感器配置下实验表明，Dynamic-LIVO 提高了定位精度，并在挑战性动态环境中生成更干净的静态彩色地图。

Conclusion: Dynamic-LIVO 能提升动态环境下的状态估计鲁棒性和静态建图质量，源代码和自采数据集将在接收后公开。
...
### [275] [OmniMimic: Dynamics-completed Motion Augmentation for Multi-style Omnidirectional Quadruped Locomotion](https://arxiv.org/abs/2609.20566)
*Sheng Wu,Guoqiang Zhao,Zhe Yang,Fei Teng,Zhikun Zhou,Yanlin Yang,Zheng Fang,Hong Zheng,Yaonan Wang,Kailun Yang*

Main category: cs.RO

TL;DR: 提出OmniMimic训练框架，将方向受限的动物演示数据转化为单一多步态策略，覆盖各轴目标速度范围。


<details>
  <summary>Details</summary>
Motivation: 动物演示能为四足机器人提供难以手工设计奖励实现的自然步态风格，但其方向覆盖范围窄，导致后退、侧移和转向命令缺乏风格一致的监督信号。

Method: 1）结合时间反转、约束动力学补全和矢状面镜像，构建超出观测方向的机器人特定运动学与物理监督；2）将命令从演示速度分布逐步扩展到各轴目标边界；3）采用共享actor与软门控、步态专用残差专家，平衡可复用运动技能与步态特定修正。

Result: 在仿真中四种步态上，相比匹配的APEX基线，OmniMimic将前向和后向参考速度下的平均足部位置RMSE降低12.9%，并在均匀笛卡尔命令网格上将速度跟踪RMSE降低63.1%。

Conclusion: OmniMimic能够利用方向受限的动物演示，训练出覆盖多方向、多步态且风格一致的四足机器人运动策略。
...
### [276] [ViLoMan: Learning Visual-Proprioceptive Whole-Body Loco-Manipulation Skills for Humanoid Robots](https://arxiv.org/abs/2609.19340)
*Zejie Tian,Ruibing Hou,Bingpeng Ma,Börje F. Karlsson,Shiguang Shan*

Main category: cs.RO

TL;DR: 提出ViLoMan框架，实现自主人形机器人全身移动操作：将部分人体动作演示转化为可物理执行的机器人轨迹，并通过教师-学生蒸馏学习仅依赖机载深度与本体感知的统一全身控制策略，在开门/关门任务中完成仿真与真机验证。


<details>
  <summary>Details</summary>
Motivation: 人形机器人移动操作需要全身自适应协调，但学习自主移动操作面临困难：缺乏多样且可物理执行的机器人-物体交互数据，且难以直接从机载观测学习统一全身控制。

Method: ViLoMan先将不完整的人-物交互运动学演示转化为完整、可物理执行的机器人轨迹；再利用这些轨迹，通过教师-学生蒸馏框架学习统一策略，将第一视角深度观测和本体感知直接映射为关节级全身动作。部署时无需参考运动或中间指令。

Result: 在多种门配置和机器人初始条件下的关门任务中，于仿真和真实世界评估。结果表明，单一策略使Unitree G1人形机器人仅使用机载深度感知和本体感知即可完成完整任务，并能稳健泛化到任务变化，且有效实现仿真到现实迁移。

Conclusion: ViLoMan表明，通过轨迹转化与教师-学生蒸馏，可学习无需参考运动或中间指令的自主人形机器人移动操作策略，并在真实场景中实现泛化与仿真到现实迁移。
...
### [277] [Kinematics-Grounded Agentic AI for Robotic Additive Manufacturing Process Planning](https://arxiv.org/abs/2609.19347)
*Jingzhan Ge,Ruimin Chen,Azadeh Haghighi,Jiong Tang,Farhad Imani*

Main category: cs.RO

TL;DR: 提出A-RAM智能体框架，将用户意图和零件文件转化为可追溯、可执行的机器人增材制造计划，集成LLM、规划智能体和领域工具，并在六轴机械臂AM平台上验证。


<details>
  <summary>Details</summary>
Motivation: 机器人增材制造中，切片决策、零件朝向和工作空间放置相互耦合，使在零件坐标下看似可行的路径在机械臂上可能不可行或运动学不利；现有AM工具、LLM决策支持和数字孪生系统缺乏执行前的集成评估。

Method: A-RAM采用智能体-专家-工具框架：LLM解释制造目标与约束，识别规定和可搜索的规划变量，生成模式约束请求；确定性Planning Agent实例化搜索工作流；领域工具计算切片、放置、逆运动学、轨迹时间、Joint-6 jerk和挤出等定量证据。在六轴机械臂AM单元上开展三个案例研究。

Result: 在评估候选集中，所选方案相比最不利有效候选，最大Joint-6 jerk降低53.5%，平均绝对Joint-6 jerk降低48.3%；目标特定填充筛选使运动规划完成时间缩短40.1%，挤出路径缩短12.7%。

Conclusion: A-RAM能够在执行前对耦合的切片、朝向、放置和运动学决策进行集成评估与优化，生成可执行机器人AM计划，并提升运动性能与路径效率。
...
### [278] [CoRef-GS: Cooperative Referring Gaussian Splatting for Multi-Agent Scene Understanding](https://arxiv.org/abs/2609.20586)
*Zhikun Zhou,Kunyu Peng,Runyi Yang,Junhao Cai,Di Wen,Ruiping Liu,Danda Pani Paudel,Yi Zhou,Luc Van Gool,Kailun Yang*

Main category: cs.RO

TL;DR: 论文提出协作指代高斯泼溅框架CoRef-GS，用于多机器人独立重建并融合后的语义高斯地图中进行指代接地，并构建了双四足机器人基准CoQuad-Ref。


<details>
  <summary>Details</summary>
Motivation: 单智能体局部语义高斯地图可支持指代接地，但协作场景要求地图对齐融合后仍有效；目标或上下文地标可能来自其他智能体观测，而空间关系仍需从查询机器人视角解释。现有语言感知高斯方法主要面向单地图查询，高斯配准方法则未保留语言接地所需的语义兼容性。

Method: CoRef-GS构建局部开放词汇实例感知高斯地图，通过跨智能体对齐模块利用几何与语义一致性对齐部分重叠地图，并使用视角条件掩码关系图进行查询接地。同时提出CoQuad-Ref双四足机器人基准，涵盖真实与仿真室内场景。

Result: 仿真场景中，旋转误差从粗初始化后的2.58°降至细化后的0.15°；真实世界指代mIoU从ReferSplat的52.6%提升至68.8%。

Conclusion: CoRef-GS能够有效实现融合地图上的协作指代高斯接地，并建立了公开基准与代码，推动协作场景下的语言感知三维理解。
...
### [279] [Task-Oriented Active Learning of Residual Dynamics for Model Predictive Path Integral Control](https://arxiv.org/abs/2609.19378)
*Nobuaki Aoki,Hojin Lee,Stefan Sosnowski,Sandra Hirche*

Main category: cs.RO

TL;DR: 本文提出面向任务的信息获取准则 ToIA，用于在线高斯过程残差学习的模型预测路径积分控制，通过评估早期观测对后续状态预测不确定性的降低程度，并按轨迹任务相关性加权，从而提升主动学习效率。


<details>
  <summary>Details</summary>
Motivation: 在线残差学习可减小预测控制中的模型失配，但被动数据采集可能无法覆盖任务后期重要状态；任务无关主动学习虽关注不确定或信息量大的区域，却不一定提升任务性能。

Method: ToIA 针对带在线 GP 残差学习的 MPPI 控制，对每个采样控制序列估计：若在 rollout 早期获得观测，将如何降低同一 rollout 后续状态的预测不确定性，并用该 rollout 对任务的相关性加权。评分直接在现有 MPPI rollout 批次上计算，无需采样未来观测或在假设后验更新下重新优化控制。

Result: 在异构地形、留出地图的模拟越野导航中，ToIA 相比被动 GP 学习将到达目标成功率分别提升 19.3 和 27.4 个百分点，并在密集与稀疏在线学习间隔下优于任务无关主动学习基线。消融实验表明，任务相关性在稀疏模型更新时尤为重要。实现可在 NVIDIA RTX 2080 Ti 上以 20 Hz 进行在线控制。

Conclusion: ToIA 将主动学习与任务目标结合，可在不额外采样未来观测或重优化控制的情况下提升在线残差学习 MPPI 的任务表现，尤其适用于稀疏更新场景，并满足实时控制需求。
...
### [280] [Design and Experimental Validation of a 3D Printed Torsional Series Elastic Actuator for Safe Human Robot Interaction](https://arxiv.org/abs/2609.19367)
*Joel Hidalgo Pisco,Melissa Cobos Condo,Luigi Miranda,Dennys Paillacho*

Main category: cs.RO

TL;DR: 本文设计、有限元分析并实验验证了一种用于串联弹性执行器（SEA）的低刚度3D打印TPU扭转弹簧，并配合带力矩阈值切换的混合位置控制器，实现安全的人机物理交互。


<details>
  <summary>Details</summary>
Motivation: 社会与服务机器人需要保证人机物理交互中的本质安全。传统金属SEA虽能提供硬件柔顺性，但常需复杂多部件装配，因此需要一种低成本、易制造且安全的柔顺方案。

Method: 采用3D打印热塑性聚氨酯（TPU）制造低刚度扭转弹簧；通过有限元分析与实验进行验证；设计带力矩阈值切换的混合位置控制器，以适配标准位置受限伺服电机；并开展轨迹跟踪与外部扰动实验。

Result: 弹簧表现出高度线性的力矩-变形响应，刚度Ks=0.066 Nm/degree，与数值预测偏差低于3%，偏差归因于FDM结构各向异性；系统能准确跟踪非平稳轨迹并安全顺应外部扰动；TPU固有材料阻尼起到被动低通滤波作用，防止控制模式切换时的高频振荡。
...
### [281] [Enhancing the Perception of Safety and Comfort during Physical Human-Robot Handshake Interactions by Integrating Flexible Elements into a Robotic Arm](https://arxiv.org/abs/2609.19375)
*Joel Hidalgo,Dennys Paillacho,Melissa Cobos,Luigi Miranda*

Main category: cs.RO

TL;DR: 本文通过实验研究串联弹性驱动器（SEA）在机械臂握手中对感知安全与交互体验的影响。10名志愿者分别与刚性关节和SEA柔性关节机械臂握手，结合轨迹客观数据与主观问卷，结果显示SEA提高安全舒适感，支持更安全、更被社会接受的人机交互。


<details>
  <summary>Details</summary>
Motivation: 人机物理交互中的安全与舒适至关重要，握手等自然手势因直接物理接触而具有挑战性。采用SEA吸收冲击被视为有利于更安全交互的设计策略，但需要实验比较刚性与弹性对感知安全和交互体验的影响。

Method: 开展实验研究，机械臂分别采用刚性关节和SEA柔性关节。10名志愿者（6男4女）各完成两次握手交互。测试中收集末端执行器轨迹的客观数据，并通过感知问卷获取关于安全、自然性和信心的主观信息。

Result: 问卷结果显示，配备SEA的机械臂提升了安全与舒适感知，表明SEA在改善人机物理安全和社会接受度方面具有潜力。摘要未报告客观轨迹数据的具体分析结果。

Conclusion: SEA有助于实现更安全、更被社会接受的人机交互，在社交机器人及日常场景中具有应用潜力。
...
### [282] [Winning a Won Game: Strict Reach-Avoid-Stay Control Barrier Functions for High-Dimensional Black-Box Systems](https://arxiv.org/abs/2609.19449)
*Donggeon David Oh,Duy P. Nguyen,Gongkai Yuan,Qingchen Li,Jaime Fernández Fisac,Haimin Hu*

Main category: cs.RO

TL;DR: 本文提出一种面向高维黑箱系统在有限不确定性下的严格到达-避开-驻留（sRAS）Q控制障碍函数（CBF）安全滤波器，使系统安全到达目标后能永久安全驻留，并通过仿真与硬件验证。


<details>
  <summary>Details</summary>
Motivation: 机器人不仅需要完成任务，还必须持续维持已取得的结果并避免安全失败。严格到达-避开-驻留（sRAS）形式化了这一需求：安全到达目标，并在首次进入后无限期安全驻留。

Method: 构建结合驻留价值与到达-避开价值的sRAS Q-CBF安全滤波器；证明二者联合构成有效的鲁棒离散时间CBF，并提升为状态-动作Q函数用于运行时干预；采用基于可达性的对抗强化学习，仅通过黑箱交互进行可扩展价值近似；无需已知动力学、仿射结构、价值导数或手工设计障碍函数。

Result: 在精确价值和测度零条件下，滤波器可从几乎所有可获胜初始状态保持sRAS可行性，并在首次进入目标后，面对所有容许不确定性实现仍安全驻留目标内。四足机器人间隙跳跃仿真与硬件实验中，机器人跨越间隙、安全着陆并持续安全；F1TENTH仿真中实现安全超车并保持领先。

Conclusion: 该框架能有效处理高维黑箱不确定系统中的严格到达-避开-驻留安全问题，且不依赖已知动力学或手工设计，具备实际部署潜力。
...
### [283] [From Rollout to Reset: A Graph-Based Harness for Autonomous Long-Horizon Manipulation Evaluation](https://arxiv.org/abs/2609.19413)
*Jing Jiang,Yue Yang,Xinkai Jiang,Gedas Bertasius,Daniel J. Szafir,Rudolf Lioutikov*

Main category: cs.RO

TL;DR: HALTER 是一个用于长时程机器人任务自动评估与重置的系统，通过组合学习到的原子重置技能、在线场景图和 LLM 推理，减少人工重置与标注需求。


<details>
  <summary>Details</summary>
Motivation: 真实机器人策略评估仍依赖人工重置场景，耗时且初始状态分布不明确，导致复现性差；已有 AutoEval 仅支持单步任务，而长时程任务终止状态组合爆炸，单一重置策略难以覆盖。

Method: HALTER 在线从点云和视觉基础模型构建空间场景图，并用 LLM 基于该图进行 rollout 评分、重置规划与重置成功验证；重置通过规划一个学习到的原子重置技能库来恢复场景，使演示成本随技能库规模而非终止状态数量增长。

Result: 在 Franka 机械臂的四个长时程任务上，HALTER 重置成功率为 76%，高于 AutoEval 的 52% 和运动规划重置的 65%；完成技能比例估计正确率为 90%，高于 76%；重置验证正确率为 91%，高于 78%；相比人工重置减少 72% 操作员时间。在三个留出任务上，HALTER 组合泛化重置成功率为 74.7%，而每任务重置策略仅 1.3%；还消融了场景表示和图更新频率。

Conclusion: HALTER 能有效自动化长时程机器人任务评估与重置，提升重置成功率、评分与验证准确性，降低人工成本，并展现组合泛化能力。
...
### [284] [DR-MPC: Fast and Feasible Dynamics-Relaxed Model-Predictive Control for Legged Locomotion](https://arxiv.org/abs/2609.20035)
*Run Wang,Alapati Tuerxun,Shuo Liu,Wei Xiao,Ján Drgoňa,Yilin Mo,Liang Wu*

Main category: cs.RO

TL;DR: 本文提出面向足式运动的动力学松弛模型预测控制（DR-MPC）及定制内点法求解器，在保证在线优化可行性的同时提升实时性，并在Unitree Go1四足机器人上验证。


<details>
  <summary>Details</summary>
Motivation: 足式机器人MPC需要在线优化可行性、接触感知参数化和实时计算能力，现有方法在速度与可行性之间难以兼顾。

Method: DR-MPC将动力学等式约束和仿射输入约束转为二次惩罚，仅保留非空盒约束；利用块箭头Hessian和Schur补消去状态与仿射输出方向；定制内点法仅分解约简后的控制系统，并结合摆动腿力消除和接触对齐移动阻塞。

Result: 在相同DR-MPC实现下，该方法端到端MPC中位加速比HPIPM快16.0倍、比OSQP快4.4倍，仿真运动性能相当；机载MPC中位端到端时间为4.4 ms，并在Unitree Go1四足机器人上完成验证。

Conclusion: DR-MPC及其定制求解器能显著提升足式运动MPC的实时性并保持性能，适合机载部署；开源代码将在发表后提供。
...
### [285] [Predict Before You Deploy: Offline Prediction of Quantization-Induced Task Degradation for World Action Models](https://arxiv.org/abs/2609.19441)
*Jiuyi Xu,Jinjia Guo,Meida Chen,Jing Du,Yangming Shi*

Main category: cs.RO

TL;DR: 提出PreDE框架，利用离线动作偏差和少量闭环开发集校准，预测世界动作模型量化配置是否会导致任务性能退化，从而减少昂贵的闭环评估。


<details>
  <summary>Details</summary>
Motivation: 世界动作模型依赖视频生成骨干，部署时内存和计算开销大；后训练量化虽可降低开销，但位宽、分组和量化器构成巨大配置空间，穷举闭环评估成本高，且任务损失不能仅由位宽解释。

Method: PreDE通过小规模开发集的闭环结果校准两个阈值，并对新配置给出接受、拒绝或延迟决策；在标签排序假设下，使用固定观测日志，使决策与开发标签一致。评估覆盖5个WAM、4个基准设置、28个留出配置、2个策略及450次Franka Research 3试验。

Result: 在28个留出配置中，PreDE在未观察闭环结果前对21个配置给出决策，覆盖率为75%，且全部匹配可接受或退化标签；延迟候选既包括可接受结果，也包括33个百分点的损失。450次Franka试验中，高偏差组均显著退化，低偏差组无统计显著退化。真机W4A4实现1.37倍动作查询加速，峰值内存降低约44%。

Conclusion: 结果表明，策略特定的行为校准可用于量化配置选择，并能识别需要闭环评估的候选配置；代码已开源。
...
### [286] [VLN on the Fly: An Onboard Vision-Language Navigation Stack for Aerial Robots](https://arxiv.org/abs/2609.20191)
...
TL;DR: 论文提出 VLN on the Fly，一种全机载、模块化的视觉语言导航栈，将指令接地、规划与控制分离，在四旋翼上实现室内目标导航。


<details>
  <summary>Details</summary>
Motivation: 端到端空中策略将接地、规划和控制融合为一个网络，牺牲了模块化栈的可观测性与安全检查；同时全机载计算资源有限，单阶段错误难以隔离。

Method: 使用量化 VLM 将指令接地到粗图像单元，通过深度提升为 3D 目标，采用快速 B 样条规划器生成可行轨迹，并用预训练强化学习策略跟踪轨迹输出电机命令，适配多种四旋翼。

Result: 在受控室内环境中针对三个日常参照物完成 15 次机载飞行，13/15 次到达目标，平均目标误差 5.72 cm，平均 GPU 利用率 39.3%；额外 6 次杂乱环境试验中，在机载感知门控下跟踪无碰撞轨迹。

Conclusion: 保持接地、规划与控制为独立可检查阶段的模块化栈，能够在有限机载算力下实现有效且较安全的视觉语言导航。
...
### [287] [From Wizard-of-Oz Human-Robot Dialogue Collection to a Taxonomy of Robot Response Decisions: A Retrospective Analysis of Assistive Pilot Interactions](https://arxiv.org/abs/2609.19447)
*Guangping Liu,Nicholas Hawkins,Tipu Sultan,Flavio Esposito,Madi Dian*

Main category: cs.RO

TL;DR: 该论文通过一个轮椅机械臂的 Wizard-of-Oz 试点研究，分析机器人在不完整自然语言指令下的响应决策，提出包含六种响应模式和四类歧义的层级分类体系，并验证其标注一致性及用于视觉语言模型微调的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺少真实室内情境对话，也缺少判断机器人何时应行动、确认、澄清或拒绝的实践准则；同时，用户指令常省略关键信息，如视野外物体、目的地或用户目标。

Method: 回顾性分析一项试点 Wizard-of-Oz 研究：5名参与者用轮椅安装式移动机械臂完成开门、开抽屉、喂食、喝水、清洁等日常任务，向导无正式沟通策略。基于40个片段归纳出六种响应模式（ANSWER、REPORT_DONE、REFUSE、CONFIRM、CLARIFY、ACT）和四类歧义（意图、指称、空间、可理解性）。两名人类标注者和一名AI标注者应用该体系，并微调LLaVA-1.6-7B。

Result: 干净标签率分别为91%和89%；在决策点、响应模式和歧义层级上，人类-人类与人类-AI比较的Cohen's kappa为0.72至0.95。基于分类体系标签微调LLaVA-1.6-7B以处理ACT和CLARIFY，表明用该标注训练视觉语言模型可行。决策点识别和REPORT_DONE仍存在边界案例。

Conclusion: 该分类体系可用于标注机器人响应决策并支持视觉语言模型训练；但边界问题表明，需要采用受约束协议来更一致地收集对话数据。
...
### [288] [GLAMDRING: Gait Learning And Morphology co-Design via Reinforcement LearnING of CPGs](https://arxiv.org/abs/2609.19452)
*Amogh Joshi,Kaushik Roy*

Main category: cs.RO

TL;DR: 提出GLAMDRING框架，可根据运动任务规格联合优化四足机器人形态与CPG步态控制器，从而为非结构化环境自动设计机器人。


<details>
  <summary>Details</summary>
Motivation: 机器人正从结构化工厂走向灾难现场、行星表面、农田等非结构化环境，但往往尚不存在合适的机器人，因此需要自动联合设计身体结构与运动控制。

Method: 给定前向速度范围、每执行器功率预算、执行器库和负载要求，GLAMDRING输出匹配的四足形态（连杆几何与关节执行器）及基于Hopf振荡器的CPG步态策略；通过强化学习在候选形态空间中训练少量CPG策略，并在训练后根据策略操作包络反推连杆长度与执行器；按最大速度、最小运输成本或最大负载裕度等目标对可行设计排序。

Result: 实验表明：联合设计身体与步态对满足运动约束是必要的；决定实际可承载能力的是执行器包络可行性，而非仅运动成功；多数设计中会自然涌现典型动物步态；真实世界演示验证了有效性。

Conclusion: GLAMDRING能有效联合合成机器人形态与控制器，降低设计成本，并说明形态、执行器约束与步态之间存在强耦合关系。
...
### [289] [Pose-aware Legged Robot Semantic Exploration with Omnidirectional Perception in Confined Unknown Environments](https://arxiv.org/abs/2609.19460)
*Xiaoyang Zhan,Shiyu Chen,Kenji Shimada*

Main category: cs.RO

TL;DR: 提出POSE姿态感知语义探索系统，利用腿足机器人俯仰/横滚和全向相机-LiDAR，在受限环境中平衡目标覆盖与探索效率。


<details>
  <summary>Details</summary>
Motivation: 地面机器人垂直视场受限且站距有限，平面视角常无法观测物体上部；倾斜身体可提高覆盖，但会增加额外观测和姿态切换时间，因此需要解决覆盖与效率的权衡。

Method: POSE包含姿态感知视点采样，根据预期覆盖增益选择身体姿态；采用对准执行减少不必要的身体重定向；引入VLM辅助的以物体为中心视点剪枝，利用持续观测历史和BEV地图减少冗余检查；最终将语义视点与几何探索视点结合到全局探索规划器中。

Result: 仿真显示，POSE比平面规划基线最终目标表面覆盖率提高8-10个百分点，探索时间减少17-32%，并取得最高的平均物体覆盖率AUC；在机器车间的腿足机器人全向相机-LiDAR真机实验验证了适用性。

Conclusion: 结果表明自适应身体姿态规划可改善腿足机器人语义探索中的覆盖-效率权衡，未来计划开源代码。
...
### [290] [FASA: Feedback-Aware Sampling Adaptation for Efficient Diffusion-Based VLA Models](https://arxiv.org/abs/2609.19475)
*Yuchen Han,Jianhan Wu,Xiaoyang Qu,Lingwei Kong,Shiyi Li,Jianzong Wang*

Main category: cs.RO

TL;DR: FASA 是一种无需训练的运行时框架，利用实时多模态反馈动态调整扩散式 VLA 模型去噪采样步数，从而加速边缘部署。


<details>
  <summary>Details</summary>
Motivation: 扩散式 VLA 模型迭代采样计算与内存访问开销大，难以在边缘平台实时部署；现有加速方法要么需要昂贵训练（如蒸馏、流匹配），要么通过静态剪枝与缓存损害感知，忽视了机器人交互的动态负载变化。

Method: 提出 FASA（Feedback-Aware Sampling Adaptation），将实时多模态反馈作为去噪管线的控制信号：交互驱动的范围自适应器基于视觉与夹爪力反馈调节全局采样步预算，本体感知步自适应器在调整后的范围内定位最优步，实现软硬件协同适配不同执行阶段的负载需求。

Result: 在多个基准上的对比评估表明，推理速度最高提升 1.45 倍，同时保持有竞争力的成功率。

Conclusion: FASA 为在资源受限计算平台上部署重型生成式具身 AI 工作负载提供了一种新的动态运行时架构范式。
...
### [291] [Self-excited actuation enables adaptive and resilient flapping-wing flight](https://arxiv.org/abs/2609.19480)
*Rundong Yang,Ethan S. Wold,Ellen Liu,James Lynch,Wei Zhou,Mark Jankauski,Simon Sponberg,Nick Gravish*

Main category: cs.RO

TL;DR: 本文研制了首款可飞行的异步驱动扑翼机器人，并证明异步驱动能让翅膀无需控制输入即可响应身体共振变化，且无需外部传感即可瞬时响应碰撞，在杂乱环境中比同步驱动更稳定、性能更好。


<details>
  <summary>Details</summary>
Motivation: 昆虫飞行肌肉分为同步肌和异步肌，其中异步肌具有内在的拉伸激活反应，可自发产生翅拍，被认为能提供自适应和快速响应的飞行控制能力。但现有飞行机器人大多采用同步驱动，因此作者希望探索异步驱动在扑翼机器人中的潜力。

Method: 开发了使用异步驱动的飞行扑翼机器人，并与同步驱动进行对比；通过飞行测试，尤其是在杂乱环境中的测试，评估其对身体共振力学变化的响应以及对障碍物碰撞的瞬时反应，且不依赖外部传感。

Result: 异步驱动使翅膀无需控制输入即可响应身体共振 mechanics 的变化，并能在没有外部传感的情况下即时响应碰撞；在杂乱环境飞行测试中，异步驱动相比同步驱动显著提高了稳定性和性能。

Conclusion: 该工作表明，模拟飞行昆虫异步肌的扑翼机器人驱动策略可在控制系统介入之前提供快速、反应式的驱动响应；将具身控制分配到低层驱动动力学和高层感觉运动系统，为新型飞行机器人提供了有前景的设计蓝图。
...
### [292] [PIVOT: Perception-aware Independent Viewpoint Online Optimization](https://arxiv.org/abs/2609.19510)
*Yuyang Chen,Shekoufeh Sadeghi,Charuvahan Adhivarahan,Elton Lemos,Chen Wang,Sanjeev J. Koppal,Karthik Dantu*

Main category: cs.RO

TL;DR: 提出PIVOT，一种轻量迭代方法，用于在固定平移轨迹上优化运动解耦传感器的观测方向，以最大化特征可见性，从而支持在线视角选择。


<details>
  <summary>Details</summary>
Motivation: 传统机器人感知假设传感器视场相对机器人本体固定，而云台相机、MEMS LiDAR等运动解耦传感器可在运行时独立控制感知方向。这带来计算挑战：如何在特征密集环境中高效在线选择有用视角。

Method: 在锥形FoV模型下，可见性仅依赖光轴方向，将问题建模为观察球S^2上的二自由度优化。采用无坐标的SO(3)指数映射更新，实现高效连续优化，避免显式角度参数化和穷举观察球搜索。

Result: Monte Carlo评估表明，PIVOT保留暴力搜索98.1%–99.6%的可见性，并获得76–85倍加速。照片级仿真和真实四足机器人实验进一步证明，其能提升视觉定位鲁棒性并实现实用的视点控制。

Conclusion: PIVOT能够高效优化运动解耦传感器的观测方向，在保持接近最优可见性的同时显著降低计算成本，并具有实际机器人平台上的部署价值。
...
### [293] [AURORA: A Natural Language-Driven Agentic Framework for Understanding, Reasoning, and Orchestrating Reliable Air-Ground Co-Simulation](https://arxiv.org/abs/2609.19527)
*Keshu Wu,Hao Zhang,Rui Gan,Xiangbo Gao,Xiaopeng Li,Zhengzhong Tu,Yang Zhou*

Main category: cs.RO

TL;DR: 论文提出AURORA，一个自然语言驱动的智能体框架，将空地对场景生成视为带验证的编译过程；其核心是类型化中间表示AGSG，并引入AURORA-Bench评估场景是否忠实实现用户请求的交互。


<details>
  <summary>Details</summary>
Motivation: 空地对协同仿真场景构建耗时且难以验证；更关键的是，生成的场景可能可执行，却未实现用户要求的时间、空间、通信或行为关系。

Method: 设计空气-地面场景图AGSG作为类型化中间表示，显式连接智能体、空中任务、事件、通信链路、成功条件及跨域依赖；支持仿真器接地解析、道路-空域联合接地、时间规划、执行前可行性检查、基于轨迹的运行时验证、失败定位和受限修复；并提出AURORA-Bench。

Result: 多语言模型实验表明，结构化执行显著提升可靠性；运行时验证能暴露基于完成度评估忽略的静默失败；局部修复无需重新生成整个场景即可解决许多违规。

Conclusion: 可靠的场景生成不仅需要可执行代码，还需要验证实际实现的行为；显式中间表示对可验证、可修复的语言驱动协同仿真具有重要价值。
...
### [294] [CoreSense: Traceable Failure Recall and Conflict-Aware Belief Gating for Auditable Robot Decisions](https://arxiv.org/abs/2609.19512)
*Zoe Li*

Main category: cs.RO

TL;DR: 提出 CoreSense 机器人系统集成架构，将可追溯情景证据与冲突感知信念门控结合，使机器人能在回忆失败经验后决定继续、重观测、弃权或升级，并给出有限且可审计的建议。


<details>
  <summary>Details</summary>
Motivation: 机器人可回忆先前失败，但不知道这些回忆证据是否仍有效、是否与当前观测冲突、是否足以支持决策，因此需要一种冲突感知的安全门控机制。

Method: CoreSense 在允许 PROCEED、请求重新观测、弃权或升级前，检查范围、来源、时间、矛盾和支持度。评估分三层且不控制物理机器人：离线真实机器人公开数据、冻结信号级仿真、实时云部署路径；涉及 CableTrace-120、BotFails-200、ViFailback-BotFails、UR3 遥测及 CockroachDB Cloud-Amazon Bedrock 路径。

Result: 在 CableTrace-120 和 BotFails-200 上，信念门控将协议定义的不安全继续从 20% 和 40% 降至 0%。原始视频策略也达 0% 不安全继续，但过度阻止所有正常回合；ViFailback 视觉检测器 AUROC 达 0.778 但仍全部阻止。UR3 保护停止遥测取得 0% 不安全继续、36.1% 过度阻止、61.9% 覆盖率；抓取丢失迁移为负结果。受控物理验证分别为 3.3%、0%、42.0%，冲突感知融合为 4.7%、0%、42.8%。20/20 云端召回验证了 CockroachDB Cloud-Amazon Bedrock 部署路径。

Conclusion: 证据支持一种可审计的集成模式，而非自主恢复或经过认证的安全性。
...
### [295] [SLAMSqueezeBench: Comparing SLAM Systems under Resource Constraints](https://arxiv.org/abs/2609.19533)
*Mohamed Hefny,Karthik Dantu,Steven Y. Ko*

Main category: cs.RO

TL;DR: 论文提出 SLAMSqueezeBench，用于在边缘硬件上以真实资源约束测试 SLAM 系统，并对比了九种 SLAM 方法。


<details>
  <summary>Details</summary>
Motivation: 现有 SLAM 基准通常在孤立环境下构建和测试，未考虑边缘设备上多任务并行的计算与内存约束，也缺乏在现实资源限制下比较 SLAM 系统的通用机制。

Method: 开发 SLAMSqueezeBench 框架：在 SLAM 执行期间限制可用计算和内存资源；通过有限缓冲区满时模拟丢帧，复现真实相机帧采集过程；用该框架比较九种 SLAM 系统，涵盖经典方法、学习型方法和 Gaussian splatting 方法。

Result: 该框架能够在现实工作负载和边缘硬件上测试 SLAM 系统，并完成对九种代表性 SLAM 系统的对比。

Conclusion: SLAMSqueezeBench 弥补了现有基准在真实资源约束下比较 SLAM 系统的不足，并将在论文发表后向社区开放使用。
...
### [296] [Navigate or Relocate? Planning Among Movable Obstacles in Unknown Environments](https://arxiv.org/abs/2609.19541)
*Yuqing Zhang,Haoyu Zhu,Yiannis Kantaros*

Main category: cs.RO

TL;DR: 论文研究未知环境下的可移动障碍物导航（NAMO），提出在线框架在导航与搬障之间决策，并规划相互依赖的搬障序列。


<details>
  <summary>Details</summary>
Motivation: 传统机器人规划假设所有路径被阻塞时无法通行；NAMO需决定移动哪些物体、以何顺序移动以及放置到何处。多数NAMO规划器假设环境已知，现有未知环境方法多局部推理，无法规划相互依赖的搬障序列，且机器人需判断阻塞路径是需要搬障还是可能通过未探索空间找到可行路径。

Method: 提出在线框架，通过最短路径在“将已发现可移动物体视为障碍”与“视为可移除”之间选择导航或搬障；导航使用现有运动规划器，搬障使用基于采样的方法搜索相互依赖的搬障序列，并利用LLM偏置采样。

Result: 数值实验表明，该方法可扩展到需要相互依赖搬障的杂乱环境，并相比现有基线提升了规划质量。

Conclusion: 该在线框架能处理未知环境NAMO中的导航与搬障歧义，通过联合规划相互依赖的搬障序列，在复杂杂乱场景中实现可扩展性和更优规划效果。
...
### [297] [VABench: Measuring Embodied Spatial Intelligence through Visual Demonstrations, Active Perception, and Metric Control](https://arxiv.org/abs/2609.19554)
*Zhongbo Zhang,Jiayi Jin,Yifan Wang,Zaibin Zhang,Haiwen Diao,Lijun Wang,Huchuan Lu*

Main category: cs.RO

TL;DR: 论文提出 VA-Bench，用于评估通用多模态大模型在空间智能中的完整“观察—推理—行动—修正”闭环，包括主动获取缺失证据、统一空间表征和基于反馈执行动作。


<details>
  <summary>Details</summary>
Motivation: 现有空间智能评估多停留在描述物体位置，无法检验模型在观测不完整时是否能够主动寻找缺失证据、在共同空间框架中解释信息并据此行动。

Method: 构建 VA-Bench，包含 14 个基础任务族（11 个单臂、3 个双臂）、7 个留出几何/布局变体和长时程五物体组合任务。模型仅从 RGB 演示学习程序上下文，主动选择相机视角、发出度量笛卡尔命令，并依据执行反馈修正；不提供物体位姿、oracle 轨迹或学习到的动作头，由固定模型无关控制器执行模型指定目标。评估 12 个主要模型条件，每个基础任务在相同 20 个物理验证种子上独立运行 3 次，报告最终成功率、9 项轨迹级行为诊断和子任务进度。

Result: 最佳模型在标注运行中目标定位达 100.0%，空间关系达 78.9%，但三次运行宏平均任务成功率仅 53.93±3.17%。主动相机控制显著优于被动多视角观察，匹配比较中成功率从 27.86% 提升至 57.50%。留出几何迁移可使任务成功率下降超过 30 个百分点。没有模型完成严格长时程任务，尽管有显著部分进展。

Conclusion: VA-Bench 检验通用多模态大模型能否将视觉演示和主动获取的证据转化为成功的具身行动；当前模型在空间闭环任务中仍存在明显局限。
...
### [298] [Tele-Traversability: Rethinking Traversability for Teleoperated Ground Robots in Terrain Navigation](https://arxiv.org/abs/2609.19577)
*Lewei Feng,Qi Chen,Wenshuo Wang,Xianghao Meng,Minh Kieu,Haijie Guan,Jin Wu,Fuchun Sun,Junqiang Xi*

Main category: cs.RO

TL;DR: 该文指出现有可通行性研究多以机器人为中心，未充分考虑遥操作中人类操作员的认知影响；为此提出以人为中心的“遥可通行性”概念。


<details>
  <summary>Details</summary>
Motivation: 遥操作中，人类操作员依赖不完整、延迟的离站反馈判断可通行性，而机器人依赖现场机载感知，二者评估可能不一致，导致决策与动作不匹配。现有可通行性建模、估计与预测方法主要面向全自主机器人，忽略了人类操作员的影响。

Method: 概念性扩展与综述：重新审视机器人学中可通行性的定义与作用，并将其扩展到遥操作场景，纳入操作员的注意力、工作负荷、风险容忍度或风险意识等认知状态。

Result: 提出“遥可通行性”概念，将可通行性从以机器人为中心扩展到以人为中心，强调人机可通行性评估对齐对流畅、易用遥导航的重要性。

Conclusion: 未来需面向以人为中心的遥操作系统，进一步研究遥可通行性，并关注相关趋势与开放挑战。
...
### [299] [Recovering Aggressively Pruned Vision-Language-Action Models with Offline Hidden-State Distillation](https://arxiv.org/abs/2609.19579)
*Chiyoung Kim,Sanghyuk Roy Choi,Minhyeok Lee*

Main category: cs.RO

TL;DR: 本文提出一种纯离线恢复方法，用于修复结构化剪枝后的视觉-语言-动作（VLA）模型。通过宽度剪枝保持残差流形状不变，使教师与学生隐藏状态可直接匹配，无需投影器，并利用一次教师前向构建的缓存进行隐藏状态蒸馏，从而在极少 GPU 时间内恢复大部分成功率。


<details>
  <summary>Details</summary>
Motivation: VLA 模型虽能跟随语言指令，但数十亿参数的语言主干难以部署到机器人硬件上。结构化剪枝可压缩主干，但会严重降低成功率，例如 OpenVLA-OFT 剪掉 63% 后 LIBERO-Long 成功率从 93.2% 降至 0.8%。现有恢复方法需监督微调加强化学习，依赖在线 rollout 和数百 GPU 小时，因此需要更高效的离线恢复方案。

Method: 采用宽度剪枝，只缩小模块而保留残差流原始尺寸，使教师和学生隐藏状态形状一致，可直接匹配而无需投影器。训练时使用一次教师前向传播构建的缓存进行隐藏状态蒸馏。在 OpenVLA-OFT 和 CogACT 上扫描九种压缩比例，并与监督恢复方法对比；在 6-DoF 机械臂上评估部署性能。

Result: 63% 压缩的学生模型约 8 GPU 小时即可恢复到教师 3.5 个百分点以内。压缩至 45% 时，蒸馏与监督恢复在 OpenVLA-OFT 上无显著差异；63% 至 87% 时隐藏状态蒸馏带来 +2.1 至 +4.5 个百分点提升；CogACT 上从 63% 起提升 +9.4 至 +22.1 个百分点。CogACT 81% 压缩下，三倍恢复预算使蒸馏学生与教师平均差距缩小至 3.9 个百分点，而监督恢复仍低 20 个百分点以上。相同压缩率下，宽度剪枝成功率更高，深度剪枝延迟更低。6-DoF 机械臂上，72% 压缩的蒸馏学生达到 77.5% 成功率，监督恢复为 59.5%，且比教师板载运行快 2.23 倍，内存少用 62%。

Conclusion: 纯离线隐藏状态蒸馏能有效恢复宽度剪枝后 VLA 模型的大部分性能，无需在线 rollout 或大量 GPU 小时。宽度剪枝在成功率上优于深度剪枝，而深度剪枝在延迟上更优。该方法在 OpenVLA-OFT、CogACT 及真实机械臂上均验证了压缩、恢复与部署效率的平衡。
...
### [300] [Quantifying Mechanical Intelligence in Legged Robots with Information Theory](https://arxiv.org/abs/2609.19588)
*Zach J. Patterson*

Main category: cs.RO

TL;DR: 该文以现代腿式机器人为例，提出用信息论指标量化机械智能，即机器人身体形态如何降低控制负担，并为理解机构与具身计算的关系奠定基础。


<details>
  <summary>Details</summary>
Motivation: 机械智能在机器人学中日益重要，但缺乏严谨的理论理解和定量度量，落后于已有的工程系统发展。

Method: 将身体动力学视为计算过程和通信信道，提出若干信息论指标；以腿式机器人为基准，分析串联弹性驱动与低齿轮比本体感受传动之间的权衡，并在线性机器人腿传动模型、非线性单腿仿真和由学习策略控制的模拟四足机器人越障任务中验证。

Result: 表明腿式机器人工程中的若干已有洞见可用信息论描述，并能量化机械模态及机器人坐标间处理的信息比特数；同时揭示合规性与传动方式如何与运动控制策略交互。

Conclusion: 这些结果为更广泛地研究机器人机构及其在具身计算中的作用奠定了基础。
...
### [301] [OmniCalib: Target-Free, Task-Structured Self-Calibration for Humanoid Robots](https://arxiv.org/abs/2609.19582)
*Kaixiang Lu,Haiyu Lan,Chunxiao Qiao,You Li,Enyu Li,Yehao Lu,Jiarui Yang,Peiwen Lin,Chuang Wang*

Main category: cs.RO

TL;DR: OmniCalib 是一种无需标定物、仅利用机器人自身运动和板载传感的类人机器人标定流程，可校准上肢14个关节零位、腕部/胸部RGB-D相机外参、下肢关节零位以及多相机头部装置。


<details>
  <summary>Details</summary>
Motivation: 装配、磨损和部件更换会扰动CAD模型中的传感器外参和关节零位；现有方法通常只校准单个传感器对，或依赖外部标定物。

Method: 将机器人原生动作与参数块匹配，检查可观测性，并只把有支持的修正写入CAD模型。上肢用深度ICP无靶标恢复14个手臂关节零位并校准RGB-D外参；下肢用静态双支撑站姿估计12个关节零位；头部结合多相机视觉里程计、腿式里程计与动态补偿，并通过ROS变换树在线融合。

Result: 相对CAD，左腕外参修正10.56 mm、1.74°，右腕6.33 mm、1.25°，胸部RGB-D相机9.81 mm、0.929°；ICP点到面残差2.09 mm。注入偏移下，ICP与ArUco均将14个关节零位恢复到低于0.1°编码器分辨率参考。AGIBOT A3 Ultra上，12个下肢关节零位注入偏移的RMS误差为0.063°。头部仅用平面行走，三条序列平均SO(3)误差1.061°，最佳0.775°，与iKalibr在丰富6自由度激励下的0.902°相当；装置相对角度重复性0.140°。

Conclusion: 注入恢复和留出测试验证了各可观测模块；表明仅用机器人原生运动和板载感知，无需标定靶标，也能有效校准类人机器人多模块参数，并将可靠修正写回CAD模型。
...
### [302] [WorldContact: A Contact-Centric World Model for Scalable Robot Learning](https://arxiv.org/abs/2609.19600)
...
TL;DR: 提出以接触为中心的可变形物体操作世界模型 WorldContact，用少量高质量轨迹高效生成额外训练数据，用于机器人策略适配。


<details>
  <summary>Details</summary>
Motivation: 机器人适应新物体和新任务需要交互经验，但真实交互数据获取成本高、代价大。

Method: 构建 WorldContact 接触中心世界模型，基于有限高质量轨迹预测可变形物体动力学；采用比源数值模拟器更大的时间步训练/预测，以处理快速运动并避免穿透；在16个购物袋操作任务上评估，并用生成数据微调现有视觉-语言-动作策略后部署到真实机器人。

Result: 在单张H100 GPU上进行状态推演，WorldContact 比源模拟器快10倍（不含渲染和磁盘I/O）；在袋子提升任务中，仅用源仿真数据微调时单次尝试成功率为65%，加入 WorldContact 扩展数据后提升至95%。

Conclusion: WorldContact 能高效生成训练数据，显著提升机器人策略在新物体和任务上的适配效果。
...
### [303] [TacSushi: Tactile-Grounded World-Action Modeling for Dexterous Sushi Manipulation](https://arxiv.org/abs/2609.19613)
*Haodi Hu,Kaen Kogashi,Toshiaki Koike-Akino*

Main category: cs.RO

TL;DR: TacSushi 是一个基于 Cosmos3 的触觉接地世界-动作策略，用于灵巧食物操作。它在训练时利用未来后果监督，并通过特征级门控融合指尖触觉信息，部署时移除预测解码器。


<details>
  <summary>Details</summary>
Motivation: 灵巧食物操作面临形变、遮挡和不确定接触等挑战，需要更好的触觉与预测性监督来提升操作成功率和泛化能力。

Method: 骨干网络编码当前 RGB、语言和手部状态；采用特征级门控融合将指尖触觉特征融入动作表示。训练时，以示范动作块为条件的解码器预测未来视觉观测、任务进度、相对接触风险和触觉摘要，部署时移除该解码器。失败试验仅提供后果监督，其动作不用于模仿。在 340 次成功和 50 次失败的真实机器人试验上训练，并在 3 个分布内任务和 2 个分布外食材变体中进行 600 次独立 rollout，比较 6 种方法。终端结果使用锚定视觉质量协议评分，该协议等权结合 5 个人工评分和 3 个视觉语言模型评分。

Result: 完整 TacSushi 在分布内平均成功率为 68.3%，分布外为 37.5%；无未来后果监督时分别为 36.7% 和 10.0%；用直接触觉拼接替代门控融合时分别为 25.0% 和 17.5%。

Conclusion: 结果支持特征级门控触觉融合与仅训练时预测性监督具有互补收益，能显著提升灵巧食物操作性能与泛化性。
...
### [304] [EmbodiedMind: Adaptive Data Curation and Prefix-Tree Reinforcement Learning for Efficient Embodied Intelligence](https://arxiv.org/abs/2609.19659)
...
TL;DR: 提出一种三阶段高效训练范式，通过数据筛选与分层策略优化提升具身基础模型性能，最终模型EmbodiedMind在18个基准上取得70.02%的平均表现。


<details>
  <summary>Details</summary>
Motivation: 现有具身基础模型训练通常依赖大规模数据和算力，但存在样本利用率低、异构任务梯度贡献不均衡、长时规划中轨迹级奖励导致信用分配困难等问题。

Method: 方法包含三个协同阶段：RSFT通过拒绝采样微调过滤低信息样本，建立稳健行为先验；IR-GRPO使用按难度分层的任务队列平衡强化学习迭代数据，并结合混合奖励提供跨任务反馈；Trie-GRPO基于动作前缀树进行强化学习，实现步级优势估计，以隔离中间正确决策与下游错误。

Result: EmbodiedMind在18个基准上达到70.02%的最优平均性能，并在长时任务规划准确率上显著优于其他具身基础模型。

Conclusion: 该工作表明，结合战略性数据选择与分层策略优化，可有效缓解具身基础模型训练中的样本、梯度和信用分配问题，并计划开源项目以支持复现。
...
### [305] [ReShoot: Generative Visual Domain Randomization of Recorded Robot Demonstrations for Visuomotor Policy Learning](https://arxiv.org/abs/2609.19661)
*Chiyoung Kim,Min Sung Choi,Jinho Ju,Chanhoe Gu,Donghwan Hwang,Wonseok Choi,Woongsun Jeon,Minhyeok Lee*

Main category: cs.RO

TL;DR: ReShoot 通过视觉语言模型编辑属性并用边缘条件视频生成器重渲染已有示范，从而在不重新采集数据的情况下增加视觉多样性，提升模仿学习策略对颜色、背景等变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 模仿学习机器人策略常对训练演示中的视觉条件过拟合，物体颜色或背景变化会导致性能大幅下降；而针对每个新视觉环境重新采集示范需反复使用机器人、受控环境和人工操作，成本高昂。

Method: ReShoot 利用视觉语言模型为场景生成描述并编辑目标属性（如背景、物体颜色或材质），再由边缘条件视频生成器重渲染两个相机视角并相应更新语言指令；动作序列和本体感受轨迹原样复制、不重新标注，使生成片段保留原有动作与标签。

Result: 在 LIBERO 上，使用录制与重渲染示范等量混合训练的策略性能与仅录制训练相当（96.5% vs. 96.9%），并在 LIBERO-Plus 上提升了对场景扰动的鲁棒性（85.5% vs. 82.3%）。在两个实体机器人平台上，使用 43 和 100 条预采集示范部署 ReShoot，使重着色物体上的成功率分别从 0.0% 提升到 42.9% 和 47.5%，同时保持原始录制外观下的性能。

Conclusion: ReShoot 将视觉多样性获取的负担从数据采集转移到数据生成，能在不牺牲原始分布性能的情况下提升视觉鲁棒性，并可迁移到真实机器人任务。
...
### [306] [Towards High-DoF Dexterous Manipulation through VLA Post-Training](https://arxiv.org/abs/2609.19666)
*Junlei Zhu,Shenzhe Yao,Chaogui Huang,Wenkai Zhu,Jingwei Peng,Guanqi He,Soren Schwertfeger,Jiahao Chen,Yide Liu*

Main category: cs.RO

TL;DR: 提出一个四步后训练流程，将模仿学习得到的VLA基础模型适配到高自由度灵巧手和特定下游任务，并在五个真实世界任务上实现100%成功率。


<details>
  <summary>Details</summary>
Motivation: 开源VLA模型缺乏高自由度灵巧手的原生动作接口；人类接管DAgger时手势不匹配导致命令不连续并污染纠正轨迹；原始关节空间强化学习样本效率低。

Method: 统一四步后训练流程：学习式时序手部动作编解码器、监督微调、DAgger、真实世界残差强化学习。编解码器将预训练VLA适配为绝对灵巧手命令；缓冲回滚、姿态对齐和平滑命令混合实现连续任务相关DAgger纠正；潜在残差RL将探索限制在编解码器捕获的协调手部动作中。

Result: 在五个真实世界任务上评估，涵盖双手迁移、手内重定向和工具使用。在报告的后训练预算内，所得策略在每个任务20次试验中均达到100%成功率。

Conclusion: 该流程为将VLA基础模型适配到可靠的真实世界灵巧操作提供了一条实用路径。
...
### [307] [Runtime Safety Filtering for Two-Terminal Hazards in Robotic Battery Recycling](https://arxiv.org/abs/2609.19665)
*Yuxin Cao,Wei Song,Xianglin Yang,Fusen Guo,Lin Li,Xiao Cheng,Jin Song Dong*

Main category: cs.RO

TL;DR: 研究学习操作策略的运行时安全过滤器，针对依赖联合空间关系的双端危险（如电池回收中导电负载同时接近两个端子），在LIBERO上用冻结OpenVLA策略，比较谓词结构、几何余量和回退策略对安全-效用权衡的影响。


<details>
  <summary>Details</summary>
Motivation: 传统安全过滤器将不安全状态定义为对象级禁入区域的并集，对依赖联合空间关系的危险过于严格，如导电负载需同时接近两个端子才短路。因此需要研究此类双端危险的运行时过滤设计。

Method: 将运行时过滤器分解为谓词结构、几何余量、拒绝动作后的回退动作三个设计选择。比较合取谓词、传统双点禁入区及其复合。对每种谓词调整余量得到成功-危险前沿。在匹配工作点比较四种回退策略：保持、后退、采样搜索、连续动作障碍投影。在三个工作单元、第二个策略和任务套件上验证，并测试间隙误差下的鲁棒性。

Result: 三个谓词族在其各自余量下追踪几乎相同的安全-效用前沿。回退策略影响更大：保持比后退最多降低任务成功0.302且不减少危险，而两种最小侵入回退留下更多残余危险。该排序可迁移到第二策略和任务套件；基于后退的过滤在间隙常驻误差下仍有效，但估计负载大小的相关误差比端子位置的大独立误差更有害。

Conclusion: 对于接近定义的操作危险，余量选择和回退策略可能比谓词结构更决定运行时过滤器的安全-效用权衡。
...
### [308] [VAST: V2X/Dynamic Map-Aware Autonomous Driving Systems Validation Toolchain](https://arxiv.org/abs/2609.19681)
*Shunsuke Ito,Takuya Azumi*

Main category: cs.RO

TL;DR: VAST是一个面向V2X/动态地图的验证工具链，连接Scenic、SS2、AWSIM、Autoware和SIM-LDM，用于协作自动驾驶的系统级验证。


<details>
  <summary>Details</summary>
Motivation: 协作自动驾驶在物联网-边缘-云连续体中需要跨车辆、基础设施传感器、边缘动态地图服务和车载自动驾驶栈的系统级验证，但存在互操作性挑战。

Method: VAST不提出新搜索算法，而是解决Lanelet2到Scenic映射、基于ROS 2的SS2联合仿真、动态地图对象注入Autoware，以及TTC、PET、碰撞、超时和性能指标的采集。

Result: 在遮挡交叉口场景中，Lanelet2兼容的约束采样将边缘案例发现率从40.0%提升至80.0%，平均发现时间从259.7秒降至110.4秒。相同场景分布下，动态地图可用性将碰撞率从78.0%降至40.0%，非碰撞结果从22.0%升至60.0%，TTC/PET有统计显著变化。1-16个NPC的吞吐量研究表明采样低于0.1秒，而AWSIM/Autoware执行和重启开销主导运行时间。

Conclusion: VAST可作为协作自动驾驶信息物理系统的实用验证基础设施。
...
### [309] [LYRIC: Language-Driven Physics-Based Character Control for Contact-Rich Whole-Body Object Interaction](https://arxiv.org/abs/2609.19688)
...
TL;DR: LYRIC 是一种语言驱动的生成式流匹配控制器，用于基于物理的富接触全身物体交互。它结合任务级规划与闭环动作生成，能从自由语言指令和稀疏终端物体目标控制仿真角色完成复杂接触交互。


<details>
  <summary>Details</summary>
Motivation: 从语言指令和稀疏目标实现富接触全身交互很难，现有方法常依赖完整运动学参考，且不完美动捕参考难以直接获得可靠专家轨迹，接触处理与任务推进也缺乏稳定监督。

Method: 训练单一跟踪策略，使用几何条件交互奖励，并在手-物接触附近放松参考跟踪；将控制器分解为预测短时物体与人体根轨迹的任务级规划器，以及闭环求解全身运动和接触的动作生成器；行为克隆后冻结规划器，以其预测作为稳定监督在线微调动作生成器。

Result: 在受控 OMOMO 评估中，跟踪器成功率为 64.3%，高于 InterMimic 重实现的 53.2%；统一策略在完整 OMOMO 数据集上达到 76.5%；在留出划分上任务成功率为 90.3%，优于最强匹配运动学规划基线的 74.2%，且语义对齐和运动质量更好；无需重训练支持测试时物体路点引导，并能零样本迁移到新物体形状。

Conclusion: LYRIC 能有效实现语言驱动的富接触全身物体交互，在成功率、语义对齐、运动质量和泛化能力上优于基线，并具备测试时引导与新物体零样本迁移能力。
...
### [310] [UniExo: Unified Multi-Skill Policies for Musculoskeletal Locomotion and Co-Adaptive Exoskeleton Control](https://arxiv.org/abs/2609.19690)
*Yifei Yuan,Jakob Wolf,Ghaith Androwis,Xianlian Zhou*

Main category: cs.RO

TL;DR: 提出 UniExo 框架，先构建多技能人体肌肉骨骼策略，再与其联合训练外骨骼控制策略，实现无需显式模式切换即可支持行走、转身、跑步和倒走及其转换的统一髋关节外骨骼控制。


<details>
  <summary>Details</summary>
Motivation: 日常运动包含多种活动和频繁转换，但多数外骨骼控制器仅针对单一活动或少量相关动作设计，切换活动时通常需要显式模式切换以及单独调参或重新训练。

Method: 将行走、转身、跑步和倒走的四个单技能模仿专家蒸馏为一个由技能潜变量组织的统一人体策略，并在转换序列上通过强化学习微调；外骨骼控制器由人体策略的髋关节力矩预测初始化，并通过多智能体强化学习在四种技能中与人体策略共同适应。

Result: 统一人体策略在四种技能的未见片段上平均跟踪成功率达 94.7%，且比各专家更抗扰动。共同适应改变了辅助力矩时序并提高了向髋关节输送正功的比例。在定制髋关节外骨骼上，控制器在六名参与者、四种跑步机速度下具备泛化能力，并帮助一名参与者连续完成四种技能及其转换，无需技能标签或显式模式切换。

Conclusion: UniExo 朝着用统一的、用户特定的控制器替代活动特定控制器迈出一步，可支持多种运动活动及其之间的转换。
...
### [311] [Decoupling Physical Speed from Path Parameterization in Singularity-Free Guiding Vector Fields](https://arxiv.org/abs/2609.19726)
*Zhouru Xiao,Sha Luo,Yang Lu,Mingliang Xiao,Weijia Yao,Bohuan Lin,Xianzhe Cheng,Yaonan Wang*

Main category: cs.RO

TL;DR: 本文针对现有无奇异引导向量场（SF-GVF）无法任意指定沿期望路径物理速度的问题，提出一种具有指定物理速度（PPS）的新型SF-GVF，并验证其在路径跟踪中的有效性。


<details>
  <summary>Details</summary>
Motivation: 原有SF-GVF虽可消除奇点并保证全局收敛，但其沿期望路径给出的速度依赖路径参数化，不能任意指定；简单归一化物理投影的方法可能因归一化分母为零而引入新奇点。

Method: 提出一种新的SF-GVF with PPS，使积分曲线从高维空间任意初始条件指数收敛到期望路径；保证机器人物理速度收敛到指定速度，且路径误差动态在正则重参数化下保持不变；并针对二阶运动学模型设计饱和加速度控制律。

Result: 理论分析表明新方法可避免额外奇点并实现PPS收敛；通过对比仿真以及四旋翼在不同PPS剖面下的三维路径跟踪实验，验证了理论结果和方法的有效性。

Conclusion: 所提出的新型SF-GVF能够实现无奇异、全局收敛且物理速度可指定的路径跟踪，为机器人路径跟随控制提供有效方案。
...
### [312] [Equivariant Filter Design for Acoustic and Depth Aided Inertial Navigation Systems](https://arxiv.org/abs/2609.19742)
*Arihant Lunawat,Pieter van Goor,Frank Dellaert,Stefan B. Williams*

Main category: cs.RO

TL;DR: 本文针对无GPS的自主水下航行器导航，提出基于切群对称性的等变滤波器（TG-EqF），将IMU偏置纳入状态空间几何，以改善估计精度和协方差一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于IEKF的AUV导航滤波器将IMU偏置作为欧氏扩展附加，破坏了群仿射结构，使对数线性误差动力学不再精确，导致协方差和估计精度退化。

Method: 应用切群（TG）对称性，将偏置包含在状态空间几何中，推导等变滤波器；为DVL建立等变输出模型，其更新仅产生三阶线性化误差，并加入直接压力输出；通过蒙特卡洛仿真与双帧群IEKF和乘性EKF对比，并用AUV现场数据离线验证。

Result: TG-EqF在姿态、速度和位置上较两种对比方法误差降低18–25%；其ANEES更接近名义值1，表明协方差估计更一致；现场数据也显示位置漂移减小。

Conclusion: 将IMU偏置纳入切群几何的等变滤波框架，能提升AUV导航的估计精度和协方差一致性，并在真实数据中减少位置漂移。
...
### [313] [LIFD: Anchored Diffusion for 3D-Aware Scene Memory in Robotic Manipulation](https://arxiv.org/abs/2609.19796)
*Wenbo Li,Yiteng Chen,Wenhao Li,Qingyao Wu*

Main category: cs.RO

TL;DR: LIFD是一个用于部分可观测下机器人操作的持久化3D场景记忆框架，通过学习场景令牌表示并补全缺失区域，提升操作成功率。


<details>
  <summary>Details</summary>
Motivation: 部分可观测下需要当前视野外的空间信息；仅靠几何感知RGB特征只能描述可见结构，而先前观察区域可能消失，因此需要在保留观察历史的同时推断缺失内容且保持与可见证据的联系。

Method: 提出LIFD，从多视角一致性学习场景令牌表示，利用单RGB视图和循环记忆补全；使用整流流模型生成令牌，锚引导交叉注意力以当前几何特征为条件；紧凑槽特征连接操作策略；训练时使用多视角和几何监督，部署仅需单RGB相机、本体感知和任务指令。

Result: LIFD（分阶段）在LIBERO上平均成功率91.6%，MetaWorld上79.8%，比联合训练在LIBERO上提高3.1个百分点；在四个UR5e任务族（每族10次演示）上平均成功率56.0%，而OpenVLA-7B为40.5%。

Conclusion: LIFD通过持久化、3D感知的场景记忆有效应对部分可观测性，显著提升机器人操作成功率，且部署时仅需单目RGB相机等。
...
### [314] [Affective Shared Autonomy: Temporal Affect Dynamics and Subjective Evaluation in Bimanual Teleoperation Tasks](https://arxiv.org/abs/2609.19802)
*Zhengji Liang,Guiyin Tian,Sijin Qu,Hainan Liu,Shiyan Hu*

Main category: cs.RO

TL;DR: 提出一种情感感知的共享自主遥操作框架，基于面部视频、心脏信号和双臂运动学实时估计操作员情感状态，仅在检测到持续不利状态时触发机器人辅助。N=30用户研究显示，该方法使生产性状态最高提升39.7%，且不损害用户自主性，并发布了首个相关多模态数据集。


<details>
  <summary>Details</summary>
Motivation: 物理遥操作虽能结合人类认知灵活性与机器人精度，但高难度操作常导致严重认知负荷、挫败感和执行崩溃。传统共享自主主要依赖任务规则（如空间误差边界），忽视操作员瞬态情感状态，可能导致控制干预错位。

Method: 同步采集面部视频、心脏信号和双臂运动学数据，通过多模态融合估计操作员七类情感分布和三分类操作抽象（中性、有效、不利）。当检测到持续不利状态时，选择性触发情感感知辅助。开展N=30用户研究，并与Qwen、MiniCPM-V等零样本基线比较时间状态动态跟踪性能。

Result: 情感感知辅助将生产性状态最高提升39.7%，同时不损害用户自主性。多模态融合模型在跟踪时间状态动态方面优于零样本基线。所收集数据集是首个提供双机械臂遥操作期间视觉、生理和操作员双臂运动连续跟踪的多模态数据集。

Conclusion: 该真实部署提出了一种以人为中心的框架，整合视觉、生理和运动跟踪，用于物理人机交互，实现情感感知共享自主遥操作。
...
### [315] [HEROIC: Heterogeneous Evidential Reasoning for Open-Vocabulary Identification and Cross-Robot Collaboration](https://arxiv.org/abs/2609.19803)
*Mihir Chauhan,Aarav Jain,Addison Zucek,Manmeet Dang,Damon Conover,Aniket Bera*

Main category: cs.RO

TL;DR: 提出HEROIC，一种仅通过自然语言通信的分散式异构空地多智能体开放词汇搜索协调框架，能根据任务提示动态分配和调整角色。


<details>
  <summary>Details</summary>
Motivation: 空地机器人团队适合开放世界搜索与救援等任务，但两者失败模式不同：空中覆盖快却难分辨小或被遮挡目标，地面识别近但覆盖小。现有语言任务团队角色固定或依赖人工能力标签，无法判断任务中某资产何时不再有用。

Method: HEROIC从任务自然语言提示出发，利用传感器属性与尺度律推导初始角色，并确定空中飞行高度与扫描间距；当计算高度低于安全飞行高度时，空中智能体从搜索者重新任务为空中分诊、护航和路线引导。机器人维护对搜索区域的证据信念：正证据用方位射线，负证据用对数几率后验，并需近距验证才确认到达。

Result: 全栈实验中，HEROIC在6个场景中达到目标成功率为84%，而视觉语言前沿基线、基于前沿搜索、割草机式和随机游走等使用相同感知的基线为35%-54%；同时到达目标时间快2-4倍。

Conclusion: HEROIC通过自然语言驱动的动态角色重分配和证据化搜索协调，显著提升异构空地机器人团队在开放世界搜索中的成功率与效率。
...
### [316] [Vehicle Trajectory Prediction via Neural Fusion of Multiple EKF-Based Trajectory Candidates](https://arxiv.org/abs/2609.19813)
*Seong-Jun Kim,Seung-Hyun Kong*

Main category: cs.RO

TL;DR: 该论文提出一种将Trajectron++神经网络轨迹预测输出与基于扩展卡尔曼滤波（EKF）的多条候选轨迹进行后期融合的框架，用于提升自动驾驶中周围车辆未来轨迹预测的物理可行性和精度。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络轨迹预测器虽能利用智能体历史、动态场景图和语义地图取得较好性能，但在加速、减速和转弯等特定运动模式下，可能无法输出物理上可行的轨迹。

Method: 在不修改Trajectron++基线架构的前提下，将其输出与基于EKF生成的多条轨迹候选进行后期融合，并通过学习式融合方法结合两类预测结果。

Result: 在nuScenes数据集上，该方法使Trajectron++基线的平均位移误差和最终位移误差分别降低13.7%和14.6%。

Conclusion: 基于EKF的轨迹候选能够通过学习式融合有效补充神经网络轨迹预测，从而提升预测精度和物理合理性。
...
### [317] [RotateIt! Fast and Reliable Single-Arm Garment Unfolding via Online-Adaptive Dynamic Rotation](https://arxiv.org/abs/2609.19817)
*Zeqing Zhang,Zuokun Xie,Ao Fang,Bin Dai,Zhengjie Shu,Yifeng Tang,Ziwei Wang*

Main category: cs.RO

TL;DR: 提出RotateIt!，一种利用自适应轴向旋转进行动态衣物展开的单臂框架，可在随机桌面初始状态下展开衣物并支持后续自主折叠。


<details>
  <summary>Details</summary>
Motivation: 现有准静态衣物展开方法需要重复操作，而动态方法多依赖双臂抛甩，因此需要更高效、紧凑的单臂动态展开方案。

Method: 从随机桌面配置中选择适合旋转的抓取点，将提起的衣物绕近似固定锚点旋转，利用惯性张力分离重叠层；使用抓取排序器选择锚点，并用在线残差策略自适应调整旋转幅度、速度和释放时机。

Result: 在已见和未见模拟衣物及8件未见真实衣物上，三次尝试内成功率比准静态抓放方法提升44.0至61.0个百分点；模拟训练策略零样本迁移到真实世界，成功率达75.6%，首次尝试覆盖率提高41%，最终覆盖率提高26%。

Conclusion: 动态轴向旋转可作为单臂衣物展开的有效主要操作原语，且模拟训练策略可零样本迁移至真实场景，并进一步支持无需人工整理的自主折叠。
...
### [318] [TADreamer: Zero-Shot Language-Guided 3D Navigation for Terrestrial-Aerial Bimodal Robots via Video Imagination](https://arxiv.org/abs/2609.19824)
*Xiangyu Li,Tiancheng Lai,Xijie Huang,Ruitian Pang,Siqi Shen,Juncheng Chen,Zaisheng Pan,Chao Xu,Fei Gao,Yanjun Cao*

Main category: cs.RO

TL;DR: 提出 TADreamer，一个零样本框架，将视频想象的导航转化为符合真实几何的导航参考，用于地面-空中双模态机器人的语言引导导航。


<details>
  <summary>Details</summary>
Motivation: 地面-空中双模态机器人需要根据场景上下文和任务意图选择路线与运动模式。生成视频可表示此类运动序列，但因尺度模糊和轴向相关几何畸变，难以从中恢复度量一致的导航参考。

Method: 利用视觉语言模型将机载观测与指令转化为导航提示，筛选有效生成视频并在需要时提供纠错反馈。将选中视频重建为带地面/空中模式标注的 3D 航点。采用两阶段校准：先用视场约束初始化尺度估计，再通过将重建点云配准到测量几何来细化轴向尺度、旋转和平移。校准后的航点和模式标签输入规划器，并结合测量几何执行机器人导航。

Result: 在七种室内外场景中进行了真实世界实验。每轮生成五个候选视频时，所有七个场景均能在两轮内获得可用视频。在校准观测上，与 NavDreamer 相比，平均绝对深度误差降低 87.7%，平均绝对相对深度误差降低 86.3%。

Conclusion: TADreamer 无需任务特定训练或微调，即可在零样本条件下把视频想象导航锚定到实测几何，提升双模态机器人导航的可用性与深度一致性。
...
### [319] [Improving Cross-embodiment Transfer in Latent Action Models with Action-Similarity Supervision](https://arxiv.org/abs/2609.19846)
*Maxime Alvarez,Renzo Caballero,Tatsuya Matsushima,Yusuke Iwasawa,Yutaka Matsuo*

Main category: cs.RO

TL;DR: 论文研究潜在动作模型（LAM）中的动作相似性监督，以提升跨具身迁移。方法不预测真实机器人动作，而是让两个潜在动作的相似度匹配对应真实动作序列的相似度，从而避免潜在动作编码具身特定信息。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略虽受益于网络规模预训练，但演示数据收集昂贵且与录制机器人绑定。现有LAM可从无动作视频学习可跨具身共享的潜在动作，但易受背景视觉噪声影响，且不同机器人执行相同运动可能被编码为不同潜在动作。

Method: 在LAM训练中引入动作相似性监督：任意两个潜在动作之间的相似度被训练为匹配两个真实机器人动作序列之间的相似度。真实动作从不被LAM预测，因此潜在动作无需编码具身细节。在RoboTwin 2.0上进行控制实验：两个双臂机器人演示不相交任务集，策略在所有演示上训练，每个机器人仅在另一机器人演示过的任务上闭环评估。

Result: 在固定策略架构、超参数、数据集和评估协议下，预测潜在动作而非真实动作使跨具身成功率提高一倍以上。使用相同真实动作时，相似性监督比在LAM训练中预测真实动作的辅助损失迁移更好。最佳方案是在末端执行器运动而非关节空间运动上计算相似度，并让损失跨两个机器人比较潜在动作。

Conclusion: 动作相似性监督可有效提升LAM的跨具身迁移能力，避免潜在动作编码具身特定信息；结合末端执行器运动相似度与跨机器人潜在动作比较，是本研究中的最优方法。
...
### [320] [GR2PO: Group Relative Return Policy Optimization for Continuous Robot Control](https://arxiv.org/abs/2609.19850)
*Pengqin Wang,Qiming Zhang,Shaojie Shen,Jun Ma*

Main category: cs.RO

TL;DR: 提出一种无需评论家的强化学习框架GR2PO，用于连续机器人控制，通过并行轨迹估计折扣回报并进行组归一化，利用相对优势和裁剪目标更新策略，在仿真和边缘设备上验证了有效性和部署可行性。


<details>
  <summary>Details</summary>
Motivation: 现有actor-critic方法依赖价值网络，训练开销大且策略学习受价值估计近似误差影响；而无需评论家的组相对策略优化方法虽简单，但在密集奖励环境中直接使用即时奖励无法学习长期动作结果。

Method: GR2PO从并行收集的轨迹中估计折扣回报，在每个 rollout 时间索引进行组归一化，并使用相对优势和裁剪目标更新策略。在机器人控制仿真环境中实例化，并部署到真实边缘设备。

Result: GR2PO显著优于使用即时奖励的无评论家基线，与最先进的actor-critic方法性能相当，训练效率也具有竞争力。在NVIDIA Jetson TX2上的推理测试证明了在边缘平台部署学习策略的可行性。消融实验分析了并行组大小、回报估计方法和目标裁剪比例对学习性能的影响。

Conclusion: GR2PO是一种有效的无评论家强化学习框架，适用于连续机器人控制，兼具竞争性性能和训练效率，并能在边缘设备上部署。完整代码将在论文接收后公开。
...
### [321] [Feeling Terrain Before Crossing: World Models for Off-Road Navigation](https://arxiv.org/abs/2609.19863)
*E-In Son,Dong-Wook Kim,Ji-Hoon Hwang,Kangsun Lee,Jisung Bae,Jung-Taak Kim,Seung-Woo Seo*

Main category: cs.RO

TL;DR: 提出 Feel-WM，首个以本体感觉为条件的越野导航世界模型，在预测视觉场景的同时预测机器人未来本体状态与失败风险，并用于规划。


<details>
  <summary>Details</summary>
Motivation: 越野导航依赖机器人-地形交互，仅预测视觉场景不足；现有以场景为中心的世界模型无法预测打滑、倾斜或震动等物理未来，而本体感觉可直接反映这些动力学。

Method: Feel-WM 以本体感觉为输入条件，学习预测未来本体状态和失败风险，无需人工标签，均来自机器人自身经验；规划器同时推演物理未来与视觉场景，并用可分离评分权衡预测失败风险与目标相似度。

Result: 在真实越野数据和仿真中，Feel-WM 在开环规划与闭环崎岖地形导航上优于纯视觉导航世界模型，覆盖轮式与足式平台；部署于 Husky 山地小径时，可机载规划、预判前方崎岖并绕行，完成端到端策略失败的路线。

Conclusion: 将本体感觉纳入世界模型并预测物理未来与失败风险，可显著提升越野导航的前瞻规划与闭环性能。
...
### [322] [Learning Reliable Parking Policies via Offline Reinforcement Learning with Quantized Action Representations](https://arxiv.org/abs/2609.19894)
*Zewei Yang,Zengqi Peng,Jun Ma*

Main category: cs.RO

TL;DR: 本文提出一种基于路点级离线强化学习的交互感知自动泊车框架，并在CARLA仿真中验证其有效性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 城市环境中自动泊车的可靠机动生成受到杂乱、弱结构化停车空间以及周围车辆交互不确定性的阻碍，需要解决这些挑战。

Method: 构建了包含非交互和交互场景的专用泊车数据集，通过分层专家轨迹和旋转路点增强生成；策略以紧凑状态表示为条件，利用特征线性调制将LiDAR障碍物特征适配到目标位姿；使用状态条件分词器将连续路点序列量化为离散动作标记，并采用保守Q学习抑制对支持不足动作的价值高估。

Result: 在CARLA模拟器中的闭环实验表明，该框架在所有基线中取得最高泊车成功率，并能可靠迁移到未见过的停车位。

Conclusion: 该离线强化学习框架能够有效应对交互感知自动泊车任务，在复杂环境下实现高成功率和良好的泛化性能。
...
### [323] [Learning and Transferring Closed-Loop Robot Software](https://arxiv.org/abs/2609.19906)
*So Kuroki,Yujin Tang*

Main category: cs.RO

TL;DR: 该研究将完整闭环机器人策略实现视为可复用的执行经验，使用编码智能体从少量演示生成并迭代优化策略代码，存入软件档案，再用于新任务策略获取。优化后的源码在新任务上总体优于无参考和初始源码，但并非所有任务都更好。


<details>
  <summary>Details</summary>
Motivation: 闭环机器人策略涉及观测处理、状态管理和情境分支，手工设计与调参成本高。虽然编码智能体可生成和优化控制代码，但改进源任务的实现是否能帮助新任务策略获取尚不明确。

Method: 在RoboCasa的四个源任务中，编码智能体根据少量成功演示生成策略代码，并利用仿真反馈迭代改进；验证选择的实现存入软件档案。对九个新任务，智能体结合档案实现、目标演示和执行反馈生成并优化策略，最终冻结策略且不再调用模型。实验比较无参考、初始源码和优化源码三种条件。

Result: 源任务上，迭代优化将平均成功率从28.3%提升至64.2%。九个目标任务、三次独立运行中，平均成功率为：无参考45.2%，初始源码41.5%，优化源码57.0%。优化参考在三次运行的九任务平均上均优于初始参考，平均提升15.6个百分点；但在两个目标任务上，按运行平均初始参考仍更好。

Conclusion: 执行改进后的软件可作为获取新策略的有效资源，优化参考总体提升新任务表现，但其优势并非对所有任务都成立。
...
### [324] [Distributed Model Predictive Control with Connectivity-based Contracts](https://arxiv.org/abs/2609.19912)
*Jorit Geurts,Danilo Saccani,Melanie N. Zeilinger,Andrea Carron*

Main category: cs.RO

TL;DR: 提出一种分布式模型预测控制框架，为每个机器人分配“连通性契约”：一个局部区域，规定其预测位置允许范围；只要各机器人保持在各自契约内，团队通信网络即可保持连通。


<details>
  <summary>Details</summary>
Motivation: 多数分布式模型预测控制方案假设通信网络始终保持连通，而未主动保证连通性。常用的连通性数学条件是非凸的，并且涉及所有智能体之间的耦合，难以用于可扩展的分布式实时控制器。

Method: 设计连通性契约，将每个智能体的预测位置限制在局部区域内，并保证所有智能体遵守契约时团队保持连通。智能体只需与直接邻居进行一次信息交换来构建契约，之后各自独立求解优化问题。论文证明了闭环系统保持连通、避免碰撞并满足局部状态和输入约束。

Result: 理论证明闭环系统能够保持通信连通、避免碰撞并满足局部约束；仿真和微型自主车-like机器人硬件实验验证了方法的有效性。

Conclusion: 该DMPC框架通过局部连通性契约，以可扩展方式主动保证多机器人团队的通信连通性，并通过仿真与硬件实验证明了其可行性。
...
### [325] [Co-VLA: Consensus-based Federated Training for Vision-Language-Action Models](https://arxiv.org/abs/2609.19923)
*Haolong Li,Guner Dilsad Er,Michael Muehlebach,Joerg Stueckler*

Main category: cs.RO

TL;DR: 提出 Co-VLA，将基于 ADMM 的共识优化用于联邦视觉-语言-动作模型（VLA）训练，使不同机器人客户端在不共享数据的情况下协同训练共享模型。


<details>
  <summary>Details</summary>
Motivation: VLA 模型随规模扩大性能提升，但机器人数据天然分布在不同的机器人、任务和地点中，集中式收集成本高且不现实；联邦学习虽可利用去中心化数据，但直接用于 VLA 时需处理机器人客户端数据分布的异构性。

Method: Co-VLA 采用交替方向乘子法（ADMM）进行共识优化，以支持联邦 VLA 训练，并兼容全模型训练和参数高效微调，包括固定秩与秩自适应适配器。客户端使用各自本地机器人数据协同训练共享模型，无需共享原始数据。

Result: 实验表明，Co-VLA 在全模型训练和参数高效微调两种设置下，均能达到与集中式训练相当的性能。

Conclusion: Co-VLA 能够有效支持去中心化机器人数据上的联邦 VLA 训练，在保护数据隐私的同时取得接近集中式训练的效果。
...
### [326] [Execution-Aware Pre-Execution Ranking for Grasp-Conditioned Robotic Placement](https://arxiv.org/abs/2609.19946)
*Tianyuan Liu,Rutherford Agbeshi Patamia,Benjamin Champion,Richard Dazeley,Akansel Cosgun*

Main category: cs.RO

TL;DR: 该论文将机器人放置任务建模为执行前的抓取-放置候选排序问题，通过学习模型预测规划成功率和条件于规划的执行成功率，从而在规划前筛选更可行的候选。


<details>
  <summary>Details</summary>
Motivation: 几何上可行的放置不一定容易执行，因为所选抓取会改变末端执行器位姿、碰撞几何和运输运动；因此需要在规划前对候选进行可行性排序。

Method: 模型结合类型化目标条件点云与三种位姿描述子，并使用分层预测头分别预测规划成功率和条件于规划的执行成功率；在30个物体、1235个场景的数据集上采用场景组留出划分评估。

Result: 三随机种子top-1成功率：联合选择85.63±1.08%，固定目标排序79.84±0.16%；指定seed-42检查点相比full-pool cuMotion，联合排序从72.84%提升至85.78%，固定目标排序从59.65%提升至79.67%。冻结迁移到xArm7/MoveIt无需xArm特定重训练；27个锁定案例中13个端到端完成，成功率48.15%；通过Top-5预检并开始执行的16个案例中13个成功，成功率81.25%；候选级部署可行性预测达到81.25%召回率、85.20%特异度和83.23%平衡准确率。

Conclusion: 执行前对抓取-放置候选进行分层成功预测与排序，可显著提升放置任务成功率，并具备跨平台冻结迁移能力和较好的部署可行性预测性能。
...
### [327] [LapaTrack-3D: 6 DoF pre-operative shape tracking for laparoscopic surgery](https://arxiv.org/abs/2609.19954)
*Jingwei Song,Javid Hussain Jakir,Ray Zhang,Wenwei Zhang,Hao Zhou,Xiaomeng Xian,Maani Ghaffari*

Main category: cs.RO

TL;DR: 本文提出 LapaTrack-3D，一种用于单目腹腔镜手术的实时 6 自由度跟踪算法，可将术中视频与术前数据（如 CT）对齐，并在挑战性条件下实现稳健的 3D 跟踪。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术缺乏触觉反馈且组织透明，难以准确定位目标器官内部解剖结构，因此需要将术中视频与术前影像对齐的实时 6 DoF 跟踪方案。

Method: 在 ORB-SLAM2 框架基础上进行四项改进：1）利用原始 3D 形状快速初始化单目模式；2）采用伪分割策略分离目标器官与背景；3）在位姿图优化中引入 3D 形状作为几何先验；4）改进 MSRCP 算法以增强困难光照下的图像。

Result: 体内和体外实验表明，LapaTrack-3D 能稳健地进行 3D 跟踪，有效应对弱光照、快速运动、视野外、部分可见及“器官-背景”相对运动等典型挑战，并在 1280×720 视频上达到 13 Hz 处理速度。

Conclusion: LapaTrack-3D 可为单目腹腔镜手术提供实时、稳健的 6 DoF 跟踪，实现术中视频与术前数据的对齐，有助于在缺乏触觉反馈和透明视野下定位目标器官内部解剖结构。
...
### [328] [Hybrid Residual Reinforcement Learning for Contact-Rich Robotic Book Insertion](https://arxiv.org/abs/2609.19962)
*Tianyuan Liu,Rutherford Agbeshi Patamia,Benjamin Champion,Akansel Cosgun,Richard Dazeley*

Main category: cs.RO

TL;DR: 研究将抓取的书插入狭窄书架这一接触丰富的控制问题，提出混合控制方法：名义任务空间控制器负责结构化插入与就位，残差PPO提供有界局部修正并决定释放时机，仅开-退-闭过渡阶段采用脚本。仿真与真机实验均表明该方法显著优于纯名义控制。


<details>
  <summary>Details</summary>
Motivation: 毫米级位姿误差就可能使几何上可行的接近变为卡阻、释放失败或未完全就位，因此需要研究在抓取后最终插入阶段，控制权限应如何在已知几何与学习行为之间分配。

Method: 保留名义任务空间控制器用于结构化插入和就位，残差PPO提供有界局部修正并决定何时释放，仅开-退-闭过渡阶段脚本化。仿真采用部署匹配评估，在512个固定条件上对三个独立训练运行进行测试；真机在xArm7上对30个匹配条件进行60次试验，并进行初始化扰动鲁棒性测试。

Result: 仿真中最终策略平均成功率为98.50%（样本标准差0.23个百分点），名义控制为37.89%；真机上残差控制将成功率从26.7%提升至63.3%，失败次数从22降至11，在15个控制器表现不同的匹配条件中赢13个。初始化扰动达1.5倍时性能仍高于87%，但极紧间隙暴露了局部修正的几何极限。

Conclusion: 结果支持一种混合设计：利用几何保持可靠的任务结构，将学习集中在固定规则难以处理的接触敏感行为上。
...
### [329] [MaskHarness-WAM: Instance-Grounded Harnessing for Long-Horizon Robot Manipulation](https://arxiv.org/abs/2609.19974)
*Zitai Huang,Taiyi Su,Jian Zhu,Jianjun Zhang,Chong Ma,Tianbin Liu,Weiyi Lu,Yi Xu,Hanli Wang*

Main category: cs.RO

TL;DR: 提出MaskHarness-WAM，一个基于实例掩码的长期机器人操作框架，将高层任务规划与低层操作策略通过目标掩码连接，并利用视觉反馈进行子任务调度与持续执行。


<details>
  <summary>Details</summary>
Motivation: 长期机器人操作不仅需要稳定的局部视觉运动控制，还需要持续目标跟踪和可靠的任务进度评估；尤其当多个物体外观相同且必须按指定顺序操作时，有限视野策略难以判断应操作哪个实例以及何时进入下一阶段。

Method: 通过目标掩码连接高层任务规划与低层操作策略；每个子任务对应不同目标实例，在子任务切换时重新观察环境，生成并验证目标掩码，更新提供给低层策略的实例级空间条件；根据已验证的子任务完成状态切换目标实例，从而推进操作。

Result: 在真实机器人平台上的实验表明，MaskHarness-WAM在顺序多物体操作任务上显著优于有限视野策略。

Conclusion: 该方法能有效将局部操作技能扩展为可靠的长期机器人操作执行。
...
### [330] [Compliance for Free: Learning Identifiable Impedance via Bilateral Teleoperation](https://arxiv.org/abs/2609.19976)
*Harsha Guda,Adrià Colomé,Carme Torras*

Main category: cs.RO

TL;DR: 该论文提出利用四通道双边遥操作，从机械臂已有的关节力矩传感中识别每轴刚度，从而零额外标注地获得逐时刻、方向相关的柔顺标签，并用于微调VLA，使其在输出位姿的同时输出刚度。


<details>
  <summary>Details</summary>
Motivation: VLA能告诉机器人往哪移动，却不能告诉它用多大力。接触丰富任务依赖柔顺性，但现有示范接口无法记录该信息。核心障碍是可识别性：实际位姿与测得力无法分离操作者意图的平衡位置与刚度，因此VR控制器、SpaceMouse和手持夹爪原则上无法提供柔顺监督。

Method: 采用四通道双边遥操作，将主臂作为意图平衡位置的独立测量，使每轴刚度仅利用机械臂已有的关节力矩传感即可通过回归识别；由此生成逐时刻、方向相关的柔顺标签，零标注成本；再用这些标签微调VLA，使其同时输出位姿与刚度。

Result: 在Franka Research 3擦除任务中，对比五种策略，只有该策略在指令要求“用力擦”而非“普通擦”时接触力发生变化：RMS从6.4N（普通）增至9.1N（用力），Cohen's d=0.89，p=0.023。

Conclusion: 四通道双边遥操作可消除柔顺监督中的可识别性歧义，无需专用力/触觉硬件或特权仿真接触状态，就能为VLA提供零标注的刚度监督，使其响应柔顺性指令。
...
### [331] [Mechanical Precision Weeding with a Quadruped Robot](https://arxiv.org/abs/2609.20048)
*Ruben Beumer,Tom Janssen,René van de Molengraft,Duarte Antunes*

Main category: cs.RO

TL;DR: 提出一种基于小型四足机器人Spot的机械除草方案，通过刚性连接的铣刀工具和自主软件架构实现除草，并在室内外评估概念验证的精度与效率。


<details>
  <summary>Details</summary>
Motivation: 除草剂因杂草抗性上升和化学环境危害而日益不可持续；机械除草虽可避免这些问题，但大型机械会造成土壤压实，因此需要小型机器人替代方案。

Method: 使用Boston Dynamics Spot四足机器人，配备末端带铣刀的自定义除草工具，工具刚性连接，通过驱动腿部使机器人基座运动而保持足端静止；开发自主除草软件架构并集成所需组件，在室内外环境测试。

Result: 对当前概念验证系统在室内外环境中的除草精度和效率进行了分析，并提出了提升精度与效率的未来工作建议。

Conclusion: 小型移动机器人机械除草可减少土壤压实、简化自动化并提高可扩展性，但当前系统在精度和效率上仍需改进。
...
### [332] [FlipToSee: A Probabilistic Stable Placement Prior for Active Visual Exploration via Regrasping](https://arxiv.org/abs/2609.20078)
*Chang Shu,Sushil Samuel Dinesh,Shinkyu Park*

Main category: cs.RO

TL;DR: 提出 FlipToSee 概率框架，从单视角点云学习稳定放置先验，用于桌面物体主动视觉探索中的重新定向与遮挡表面暴露。


<details>
  <summary>Details</summary>
Motivation: 主动视觉探索常需将未知静止物体重新定向到不同稳定支撑面，但穷举物理搜索效率低。稳定放置预测本质多模态，传统 6-DoF 回归建模平移和平面偏航会引入额外歧义。

Method: 将放置参数化为 S^2 上的单位支撑法线，使用 von Mises--Fisher 混合密度网络建模多模态条件分布；从混合分量中确定性提取紧凑候选集，并用候选对齐监督训练的辅助头进行鲁棒性重排序。

Result: 仿真中首次提案成功率：分布内物体 98.4%，分布外形 95.3%，零样本迁移到 YCB 家庭物体 90.0%；并在物理机器人上结合抓取与运动规划验证了探索性重抓取。

Conclusion: FlipToSee 能有效学习稳定放置先验，减少穷举物理搜索，并支持机器人主动视觉探索中的重定向与探索性重抓取。
...
### [333] [Graph-Based Design of Soft Grippers with Multi-Objective Quality-Diversity Optimisation](https://arxiv.org/abs/2609.20087)
*Andre Farinha,Ge Shi,Harry Bowman,Brendan Tidd,David Howard,Josh Pinskier*

Main category: cs.RO

TL;DR: 该研究提出基于图的软体结构设计空间，并结合多目标、多样性驱动的遗传优化，以提升软体抓取器对多种物体的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 软体机器人虽具顺应性，但设计空间大且易过拟合特定任务，难以开发能跨任务通用的抓取器。

Method: 构建基于图的软体结构与机构设计表示，采用多目标、多样性驱动的遗传优化框架，并在优化过程中引入多种抓取场景，研究任务多样性对泛化能力的影响。

Result: 在足够多样的抓取案例集上优化，可涌现出泛化能力；与任务特定方案相比，这些设计在新颖场景中表现出更好的鲁棒性。

Conclusion: 多样性驱动优化为设计通用软体抓取器提供了原则性路径，契合软体机器人的可适应特性。
...
### [334] [Safety-Critical Scenanrio Emerges from Initial Scene](https://arxiv.org/abs/2609.20103)
*Yin Wu,Jiarong Wei,Carl Esselborn,Shubham Phoolari,Ahmed Abouelazm,Daniel Slieter,J. Marius Zöllner*

Main category: cs.RO

TL;DR: 该论文提出 AdvScene，将安全关键驾驶场景生成视为初始化问题，使用条件潜在扩散模型生成更可能演化为危险交互的真实初始场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法多从驾驶数据初始场景出发操纵周围智能体行为，但驾驶数据中缺乏有意义交互，限制可发现的失败模式；例如 Waymo Open Motion Dataset 中 20.44% 片段自车静止，30.39% 初始帧 10 米内无交通参与者。

Method: AdvScene 是条件潜在扩散模型，分两阶段训练：先在自然驾驶数据上预训练，再用闭环仿真器 rollout 反馈，通过强化学习后训练对抗智能体生成分支；以自车驾驶位移为条件防止自车静止，并用不可微的安全关键指标直接强化危险性。

Result: 在 Waymo 数据集上，跨 12 种自车与交通策略组合的实验表明，AdvScene 显著提高了自车责任碰撞事件和 TTC<3s 事件的发生率。

Conclusion: 将安全关键场景生成建模为初始场景生成问题可行且有效，AdvScene 能生成更易导致危险交互的真实初始场景。
...
### [335] [AnyViewDex: View-Invariant Dexterous Manipulation from RGB Observations](https://arxiv.org/abs/2609.20107)
*Soham Patil,Om Sanjay Gunjal,Sourabh Bhosale,Arhan Chavare,Ramandeep Singh Hora,Spandan Roy*

Main category: cs.RO

TL;DR: 提出AnyViewDex，一种非对称训练框架，通过在仿真训练中融入多视角对比对齐与特权3D几何监督，使仅用无标定单目RGB和本体感知的灵巧操作策略具备视角不变性，并实现零样本真机迁移。


<details>
  <summary>Details</summary>
Motivation: 多指灵巧操作的视觉运动策略对相机视角变化高度敏感，现有依赖RGB-D或点云等显式3D模态的方法存在硬件依赖、标定要求和传感器噪声脆弱性，因此希望在不使用测试时3D传感的情况下实现视角不变控制。

Method: 采用非对称训练管线：在仿真中结合多视角对比对齐与特权3D几何监督，通过回归绝对3D物体坐标提供几何锚定信号，缓解全局池化对比嵌入的空间坍缩；部署时仅用未标定单目RGB与本体感知，兼容强化学习和学生-教师蒸馏。

Result: 在xArm7+16自由度LEAP Hand真机评估中，对8个未见物体、6个未标定视角的480次试验达到76.7%抓取成功率（全部消融条件共2400次试验），表明几何锚定的单目策略无需测试时深度即可零样本迁移。

Conclusion: 无需显式测试时3D传感，仅通过仿真中编码几何知识即可实现视角不变的灵巧操作控制，为单目视觉运动策略的零样本真机部署提供了可行方案。
...
### [336] [Universal Navigation Interface: Robot-Free Data for Wheeled Robot Navigation](https://arxiv.org/abs/2609.20114)
*Sarvesh Prajapati,Ananya Trivedi,Lorena Maria Genua,Drake Moore,Bruce Maxwell,Taskin Padir*

Main category: cs.RO

TL;DR: 提出UNI无机器人数据采集范式，用四轮助行器和智能手机采集受物理约束的人类导航演示，构建37.2公里真实数据，并用于微调和迁移导航模型。


<details>
  <summary>Details</summary>
Motivation: 移动机器人真实导航数据通常依赖平台特定遥操作，导致大规模采集成本高且难以扩展。

Method: 使用四轮助行器与智能手机采集演示，因助行器无法爬楼梯、越未切路缘或过窄缝，数据自然偏向轮式可行路线；恢复度量轨迹以监督目标条件导航模型，并微调视觉导航模型，再闭环迁移到电动轮椅。

Result: 采集37.2公里真实导航数据；在留出UNI演示上轨迹预测误差降低17.4–24.8%；在其他导航数据集上收益随数据集和指标变化；在电动轮椅的路缘、楼梯和路缘坡道场景中完成闭环迁移。

Conclusion: 低成本物理代理可作为无需目标机器人即可采集导航监督数据的实用来源。
...
### [337] [GPT-6-Astra in a Navigation Workflow: Behavioral Analysis in Zero-Shot Vision-and-Language Navigation in Continuous Environments](https://arxiv.org/abs/2609.20116)
*Guangzhao Dai,Qi Wu,Bin Zhu*

Main category: cs.RO

TL;DR: 研究GPT-6-Astra在零样本连续环境视觉语言导航（VLN-CE）中的表现，采用直接API调用的观察-决策-执行流程，在50个R2R-CE验证集未见过episode上评估，成功率为52.0%。


<details>
  <summary>Details</summary>
Motivation: 探究无需导航专用微调或封装智能体框架的大模型在连续环境视觉语言导航中的零样本能力，并分析其任务理解与自主完成之间的差距。

Method: 使用GPT-6-Astra，在零样本VLN-CE中采用观察-决策-执行工作流，直接调用模型API，每次请求接收选定观察、执行反馈和保留的进度记录；在50个R2R-CE val-unseen episodes上评估，涵盖上下文管理和动作控制。

Result: 成功率52.0%，SPL 48.9%，nDTW 70.8%。主要发现：记录响应能利用观察和提供的历史将地标及先前动作与指令关联；审查包括请求额外视图和修正不确定判断；任务理解与自主完成存在差距，例如未完成穿越时仍继续旋转。终止时36.0%的episode通过工作流接受的STOP成功，另有16.0%在步数限制时满足距离标准。

Conclusion: 核心挑战在于将正确的局部判断转化为持续进展和适当停止。
...
### [338] [LLM-Guided Transformation of Non-Critical Driving Scenes into Safety-Critical Scenarios Using Augmented Reality](https://arxiv.org/abs/2609.20318)
*Noura Fady,Farah Khaled,Catherine M. Elias*

Main category: cs.RO

TL;DR: 提出一种结合计算机视觉、大语言模型和增强现实的自动化流水线，将安全驾驶场景转化为安全关键场景，用于自动驾驶系统测试。


<details>
  <summary>Details</summary>
Motivation: 测试自动驾驶系统需要真实的安全关键场景，但从真实道路中采集此类数据成本高且不安全。

Method: 通过计算机视觉检测和跟踪道路使用者，提取距离、速度、运动方向和碰撞时间（TTC）等安全特征并评估场景关键性；再利用大语言模型生成真实的碰撞诱导对象和行为，并通过增强现实集成到原始场景中。

Result: 在nuScenes数据集上评估，安全分类准确率达到97.52%，并成功生成行人横穿、后车超车、急停等真实场景。

Conclusion: 该流水线能够有效且灵活地自动生成安全关键场景，支持自动驾驶系统的测试与验证。
...
### [339] [Strategic Transformer for Resource-Constrained Multi-Object Navigation in Ultra-Large-Scale Environments](https://arxiv.org/abs/2609.20227)
*Daiki Iwata,Kanji Tanaka,Senta Hishida*

Main category: cs.RO

TL;DR: 该论文针对大规模室内环境中资源受限的多目标导航问题，将其建模为集合定向问题（SOP），提出 Strategic Transformer 与局部搜索结合的规划框架，实现实时高效导航。


<details>
  <summary>Details</summary>
Motivation: 在超过2000平方米的室内环境中进行资源受限多目标导航，对效率和策略规划提出重大挑战；传统基线要么规划僵化，要么行为短视，且SOP的NP-hard特性难以满足实时导航需求。

Method: 将任务重构为SOP，用独立启发式切换器管理探索、SOP模型管理利用；提出轻量Strategic Transformer作为优先级规划器，将专家组合逻辑内化为可预测前向推理，并降低师生信息不对称；引入几何注意力偏置建模长程结构依赖；结合有界迭代2-opt在受限候选图上进行局部优化。

Result: 前向推理耗时41.03毫秒，相比重型元启发式实现94倍加速；在ProcTHOR实验中，以路径长度加权的进度（PPL）指标超过精心重实现的基线，并建立可扩展、资源受限导航的新基准。

Conclusion: 该方法成功桥接探索与利用，保证有界延迟推理，适合机载部署，为大规模资源受限多目标导航提供了有效且可扩展的解决方案。
...
### [340] [Implementation of Tightly-Coupled SLAM Fusion of GPS, IMU, and LiDAR for Autonomous Vehicles](https://arxiv.org/abs/2609.20321)
*Amr O. Elmehrath,Farah Khaled,Rana Nahas,Catherine M. Elias*

Main category: cs.RO

TL;DR: 本文实现了一种紧耦合的激光雷达、IMU与GPS融合SLAM架构，用于自动驾驶在未知环境中的连续定位与建图。


<details>
  <summary>Details</summary>
Motivation: 单一传感器存在关键失效点：LiDAR在无特征走廊退化，IMU产生漂移，GPS在城市峡谷中频繁丢失，因此需要多模态融合来保证鲁棒空间感知。

Method: 采用分阶段开发：先建立2D基线验证硬件同步与坐标变换，再升级到基于FAST-LIO2的3D架构；使用IESKF处理密集3D激光点与连续惯性数据，并利用GTSAM位姿图优化后端执行多模态回环检测。

Result: 在仿真环境和实际部署中评估，紧耦合3D融合能克服各传感器盲区，生成高细节点云，并提供下游自动驾驶规划器所需的高精地图基础。

Conclusion: 紧耦合3D LiDAR-IMU-GPS融合可实现连续空间感知，有效消除长期漂移，为高级自动驾驶规划提供高精地图支撑。
...
### [341] [RoboFind: Multi-Agent Personalized Object Search for People Who Are Blind or Have Low Vision](https://arxiv.org/abs/2609.20330)
...
TL;DR: 提出 RoboFind 多智能体框架，帮助盲人和低视力用户寻找特定个人物品：手机端教目标，四足机器人执行搜索。真实机器人任务中成功率达 85.0%，并显著降低误报成功。


<details>
  <summary>Details</summary>
Motivation: 盲人和低视力用户常需要定位某个特定个人物品，而不是同类物品的任意实例。这要求机器人能到达用户无法到达的视角，并提供可访问界面让用户说明目标并确认是否找对。

Method: RoboFind 包含多智能体：Target Teaching Agent 通过 AR 引导、语音/触觉反馈和屏幕阅读器支持，将手机录制转为语义目标档案和多视角参考库；运行时 Navigation Agent 探索环境并提出候选目标，Verification Agent 对照参考验证身份，Coordination and Recovery Agent 完成任务或触发恢复并继续搜索。评估包含 32 次真实机器人任务、20 次十目标基线对比，以及 6 个共享目标与 GPT-6 Astra-only 对比。

Result: 32 次真实机器人任务中 RoboFind 成功率为 85.0%，重建的顺序首次停止基线为 25.0%；误报成功从 75.0% 降至 5.0%。在 6 个共享目标上，RoboFind 为 10/12 成功，12 次独立 GPT-6 Astra-only 试验为 5/12。

Conclusion: 多智能体设计适合个性化物品搜索需求；在声明完成前验证对象身份，是使结果可靠、用户可依赖的关键。
...
### [342] [Navi-Agent: Unlocalized Monocular Navigation Agent](https://arxiv.org/abs/2609.20388)
...
TL;DR: 提出Navi-Agent，一种零样本VLN-CE智能体，不依赖深度和全局坐标，而是利用视觉观测与运动历史构建无坐标空间状态，并组织为导航拓扑，以支持自定位、进度验证和恢复，实现闭环导航。


<details>
  <summary>Details</summary>
Motivation: 现有零样本VLN-CE系统通常依赖几何定位或基于坐标的空间表示；近期几何约束导航虽去除了深度和全局一致坐标，但难以维持持久空间意识，从而影响地点确认、进度验证和失败恢复。

Method: 构建无坐标空间状态：将视觉地点作为节点、运动转移作为边，形成导航拓扑；基于观测进行近似自定位、任务进度验证和视觉重访恢复；将指令分解为子目标，执行局部视觉导航，并通过空间状态验证已访问地点，实现闭环导航。

Result: 在零样本VLN-CE基准和真实机器人平台上的实验表明，Navi-Agent在几何约束方法中达到最先进性能，并与依赖几何定位的方法相比仍具竞争力。

Conclusion: 无需深度和全局坐标，通过视觉地点与运动拓扑构建无坐标空间状态，即可支持持久空间意识和零样本连续环境导航。
...
### [343] [Imagine-TAMP: Imagination-Guided Task and Motion Planning in Partial Observability](https://arxiv.org/abs/2609.20396)
*Antareep Singha,Shivaram Kumar,Yoonwoo Kim,Yoonchang Sung*

Main category: cs.RO

TL;DR: 提出Imagine-TAMP，一种在部分可观测环境下结合语义与几何想象的交错规划与执行框架，用于决定先观察还是先操作遮挡物。


<details>
  <summary>Details</summary>
Motivation: 传统TAMP使用符号动作代价或昂贵几何规划，难以评估观察动作揭示被遮挡目标的概率，导致观察与操作之间的决策不佳。

Method: 利用视觉语言模型根据目标与可见物体的常识关系塑造目标位置的粒子信念；用生成式场景模型估计未观测区域的合理几何。基于目标假设和想象场景生成多个符号计划骨架，并赋予非单位代价以近似操作努力和感知动作的目标可见性。选出骨架后细化为可行连续计划并执行，新观察更新信念并触发重规划。

Result: 在视角受限的货架场景中，非单位几何评估将成功率从46.0%提高到84.0%；语义信念塑造进一步减少操作和重规划。在真实机器人上，完整系统相比仅几何消融将规划时间减少32%。

Conclusion: 想象引导的评估能改进观察与操作决策，语义与几何想象结合可提升部分可观测下TAMP的成功率和效率。
...
### [344] [Resilient Motion Planning for Free-Flying Space Robots under Actuator Failures](https://arxiv.org/abs/2609.20407)
*Nicolas de Maddalena,Joris Verhagen,Jana Tumova*

Main category: cs.RO

TL;DR: 本文提出一种概率式、主动式运动规划框架，用于在自由飞行机器人执行器发生故障时仍尽量保证任务成功。


<details>
  <summary>Details</summary>
Motivation: 自由飞行机器人依赖多个推进器进行机动，若一个或多个推进器失效，可能丧失控制能力并导致任务失败；同时其自由飞行特性使无驱动时仍会沿局部直线轨迹运动，因此需要显式考虑执行器故障的规划方法。

Method: 将执行器故障模式建模为马尔可夫链，并沿规划时域传播成功到达目标的概率；使用预计算可达集评估潜在故障下到达路点的能力，再用基于RRT*的规划器拼接路点，从而最大化整体目标到达概率。

Result: 在物理自由飞行器平台上通过注入执行器故障进行了实验验证，表明该方法能够生成利用自由飞行特性的高韧性运动规划。

Conclusion: 该框架可显式处理空间执行器故障，最大化目标到达概率，并提升自由飞行机器人在故障情况下的任务鲁棒性。
...
### [345] [Coding Agents with an Obstacle-Aware Harness for Safe Robot Manipulation](https://arxiv.org/abs/2609.20822)
*Bingxin Xu,Yuzhang Shang,Zhen Dong,Emilio Ferrara*

Main category: cs.RO

TL;DR: 论文研究编码智能体在机器人操作中的安全性问题，发现其虽能完成任务但常忽视避障约束；提出SafeHarness，通过障碍感知的路径规划和接触执行两个模块，显著提升任务成功率和避障率。


<details>
  <summary>Details</summary>
Motivation: 编码智能体已能在无机器人专门训练下操作机器人，但其安全性尚未被系统评估。现有智能体在任务中往往只关注完成目标，而忽略不能触碰障碍物的安全约束。

Method: 将操作分解为路径阶段和接触密集阶段。提出SafeHarness，包含两个障碍感知模块：障碍感知路径规划将物体表示为边界框，生成候选路径点序列，先规划、验证、必要时重规划再执行；障碍感知接触执行则选择接触位置，使接触过程本身避开障碍物。

Result: SafeHarness达到71.9%任务成功率和87.5%避障率，分别超过此前最优方法6.5%和27.0%，也是未使用该框架的同一智能体的2.3倍和1.5倍。

Conclusion: 编码智能体操作失败的主因在于规划阶段未将安全约束作为优先目标，而非感知或指令问题。通过引入障碍感知的路径规划和接触执行框架，可有效提升机器人操作的安全性与任务完成能力。
...
### [346] [Time-Efficient Iterative Learning Planning for Safety-Critical Dynamic Obstacle Avoidance](https://arxiv.org/abs/2609.20435)
*Zhiyi Chen,Shuli Lv,Chen Min,Yong Xu,Jian Sun,Quan Quan*

Main category: cs.RO

TL;DR: 本文提出将迭代学习规划（ILP）与预期风险混合控制屏障函数（ARB-CBF）相结合的方法，用于资源受限自主移动机器人在动态环境中的安全关键导航与动态避障。


<details>
  <summary>Details</summary>
Motivation: 自主移动机器人需要在有限车载计算资源下实现高效规划与安全关键的动态避障。ILP虽轻量高效，但缺乏动态障碍物感知与规避机制。

Method: 扩展ILP，集成ARB-CBF；ILP通过基于局部障碍风险的分数幂更新学习行进速度与转向偏置，生成名义控制指令，ARB-CBF在运行时修正指令以提供实时安全保证；并从算法上分析复杂度。

Result: ILP重规划阶段复杂度为O(kN)（k次迭代、N个路点），ARB-CBF为线性复杂度；仿真和真实实验表明，相比基于优化的基线，该方法在时间效率和安全性上更优，且计算开销更低。

Conclusion: 该框架适合计算资源受限平台，可实现高效且安全的动态环境导航。
...
### [347] [Spatial-Semantic Uncertainty in VLM-Based Target Search: Balancing Exploration and Identification](https://arxiv.org/abs/2609.20443)
*Alkesh K. Srivastava,Jonathan Diller,Vijay Kumar,Philip Dames*

Main category: cs.RO

TL;DR: 该论文提出一种空间-语义不确定性分解方法，用于机器人根据自然语言描述搜索并识别目标，并利用信息论规划器分别评估候选发现与目标消歧的价值。


<details>
  <summary>Details</summary>
Motivation: 基于VLM的搜索系统常将候选位置的空间不确定性与目标身份的语义不确定性混为一谈，导致搜索与识别决策不清。

Method: 将空间与语义不确定性分别建模，维护对候选位置和目标身份的后验信念，并将VLM概率证据整合为全局目标身份后验（含未发现目标概率）；使用空间和语义期望信息增益（EIG）的信息论规划器，在探索与识别之间权衡；评估六种VLM不确定性引出接口，并在500个合成目标及退化观测搜索-识别实验中进行测试。

Result: 相似识别准确率可能掩盖校准与虚假置信度的显著差异；EIG规划器在75.0%-92.5%的试验中达到置信决策，而随机搜索仅20.0%；不同空间-语义权重在达到置信后识别准确率相当，但增加语义权重可减少不必要的探索和VLM查询。

Conclusion: 显式表示并规划语义不确定性可加速目标确认且不牺牲决策质量，说明不确定性表示与不确定性驱动规划在具身VLM系统中具有不同且重要的作用。
...
### [348] [Visual Sim-to-Real Learning for Robotic Insertion under Geometric Variations: Application to Rebar Installation](https://arxiv.org/abs/2609.20477)
*Tao Sun,Beining Han,Patrick Yin,Rui Xu,Harry He,Abhishek Gupta,Szymon Rusinkiewicz,Yi Shao*

Main category: cs.RO

TL;DR: 提出 RebarSim，一个完全在仿真中训练的视觉 sim-to-real 系统，用于接触丰富且间隙仅 1.4 mm 的钢筋插入任务，并实现零样本真机迁移，真实机器人 rollout 成功率达 91.3%。


<details>
  <summary>Details</summary>
Motivation: 钢筋插入是建筑现场重复且高体力消耗的任务，且属于 1.4 mm 间隙的接触丰富问题。构件存在名义设计差异和制造公差，导致真实数据需随设计和批次变化重新采集，因此需要可泛化且无需真实数据的仿真到现实方案。

Method: 先在程序化生成的钢筋几何上训练基于特权状态的强化学习教师策略，再蒸馏为多视图学生策略，将原始 RGB 和本体感觉直接映射为动作，并使用广泛域随机化。分析几何多样性与预训练作用，并检验外观随机化和 DAgger 混合对视觉 sim-to-real 的影响。

Result: 学生策略零样本迁移到真实世界，在真实工厂生产批次钢筋上成功率为 91.3%。训练多样名义设计比单一设计提升教师和学生未见设计成功率，且学生在其单设计专家自身设计上表现更优；预训练学生适应新设计所需蒸馏样本减少 4--6 倍；移除外观随机化或 DAgger 混合会显著降低成功率。

Conclusion: 纯仿真训练可零样本迁移到真实机器人钢筋插入任务。几何多样性、预训练、外观随机化和 DAgger 混合是视觉 sim-to-real 成功迁移的关键因素。
...
### [349] [Towards AI-enhanced control: a numerical technique for trajectory smoothing of a parallel robot for pancreatic surgery](https://arxiv.org/abs/2609.20499)
*Iosif Birlescu,Alexandru Pusca,Bogdan Gherman,Calin Vaida,Ionut Zima,Damien Chablat,Doina Pisla*

Main category: cs.RO

TL;DR: 论文提出一种用于微创胰腺手术并联机器人末端执行器轨迹平滑的数值方法，适用于实时主从控制架构，通过3D空间鼠标进行速度控制输入，生成S曲线以平滑速度场，从而减少组织创伤。


<details>
  <summary>Details</summary>
Motivation: 微创手术中机器人末端执行器的 abrupt 运动可能造成组织创伤，需要实时平滑轨迹以提升手术安全性。

Method: 采用数值方法，基于3D空间鼠标命令输入的速度控制，在末端执行器速度场中生成S曲线以控制加速度，并根据命令输入对S曲线进行分段以实现实时控制，同时考虑加速度时间恒定的特殊情况。

Result: 数值结果表明，末端执行器参数空间和主动关节参数均实现了稳定过渡（无突变），验证了所提方法的有效性。

Conclusion: 该方法能够有效平滑轨迹并减少组织创伤，未来将在实验模型上测试并集成到基于AI的训练模块中。
...
### [350] [Worst-Case Hidden-Vehicle Trajectory Search in Spatiotemporal Occlusion Regions](https://arxiv.org/abs/2609.20480)
*Ruichen Tan,Zengxiang Lei,Satish Ukkusuri*

Main category: cs.RO

TL;DR: 提出HC-MTS，结合时序遮挡推理与响应感知搜索，通过双层极小极大优化寻找最坏历史一致隐藏交互，在Waymo数据集上减少隐藏状态数量并识别可避免碰撞反例。


<details>
  <summary>Details</summary>
Motivation: 遮挡给自动驾驶带来根本不确定性，现有方法未探索最坏历史一致交互，仅传播逐帧假设或针对给定隐藏智能体预测优化自车行为。

Method: HC-MTS构建有限隐藏状态模式，每个模式由满足多帧可见性、占据、语义地图支持和类别运动学约束的后向见证认证；求解双层极小极大问题：内层有限预言机最大化自车驾驶得分（目的地达成与舒适性），外层搜索选择合法隐藏车辆轨迹以最小化该最佳响应值。

Result: 在8个Waymo Open Motion Dataset场景中，可见性记忆视野从K=1增至K=20，平均每场景车辆、行人、总保留隐藏种子数分别减少18.12%、21.67%、18.45%；HC-MTS识别出6个可避免反例，剩余2个场景在有限搜索预算内未发现合法碰撞攻击者。

Conclusion: HC-MTS能有效进行历史条件极小极大轨迹搜索，减少隐藏状态并发现可避免的碰撞反例，但部分场景未找到攻击者，可能受搜索预算限制。
...
### [351] [Integrated Guidance and Control of a Mother-Child UAV-UGV System for Cooperative Missions](https://arxiv.org/abs/2609.20540)
*Aashish Sahu,R. Prasanth Kumar*

Main category: cs.RO

TL;DR: 本文提出并户外验证了一套面向异构“漫游车-母机-子机”系统的自主空中回收框架，实现母机悬停状态下对小型多旋翼的部署、出击、返航与回收，20次任务中17次成功，成功率85%。


<details>
  <summary>Details</summary>
Motivation: 将小型多旋翼自主降落到悬停的多旋翼母机上，区别于地面或舰船平台，因为回收表面本身是主动控制且推力受限的飞行器，带来更强的相对运动、扰动和安全约束。

Method: 构建异构 rover-mothership-child 自主框架，覆盖漫游车监督、母机转场、子机部署与出击、自主返航、空中回收和同步下降。回收控制栈包括：加加速度有界参考生成、扰动观测器增强的平面跟踪、可行性感知垂直控制、基于离散时间障碍函数的相对垂直几何安全滤波器、通信感知的母机状态预测。系统部署于PX4-ROS 2，融合RTK GNSS、IMU和气压计，母机侧1D激光雷达仅作近距接触辅助线索。

Result: 20次户外协同任务中17次完成部署、出击与回收，观测任务成功率85%。成功回收中：平均终端对准时间6.3 s，验收时平均平面对准误差0.18 m，0.40 m捕获半径内最大终端平面偏差0.32 m，耦合下降期间记录的最小相对垂直间距0.41 m；母机平面站位保持RMS误差0.25 m。3次失败发生在不同任务阶段并单独分析。

Conclusion: 结果表明，在测试的户外操作包线内，所提出的系统级集成方法能够实现实用的自主空中回收，并完成了全尺寸户外验证。
...
### [352] [Learning Slope-Adaptive Whole-Body Locomotion for Humanoid Robots in Roofing Construction](https://arxiv.org/abs/2609.20558)
*Songyang Liu,Shuai Li*

Main category: cs.RO

TL;DR: 提出一种任务语义场景锚定框架，使Unitree G1学习屋面工式全身动作，通过轨迹优化与执行感知强化学习保证支撑、作业间隙和非穿透关系。


<details>
  <summary>Details</summary>
Motivation: 屋顶倾斜面要求工人协调移动、平衡和作业动作，但直接重定向人体演示可能只保留动作外观，却使机器人脚或手相对屋顶位置错误，且跟踪系统缺少屋顶空间参考。

Method: 用跟踪系统采集人体演示并重定向到机器人；用度量屋顶模型提供空间参考；通过轨迹级优化将推断的支撑接触和标注的作业关系锚定到屋顶；用执行感知强化学习让策略在动态跟踪误差下保持这些关系。评估包括多动作跟踪、屋顶坡度覆盖矩阵、五路钉枪消融、锤击与侧推跨任务实验，以及与纯强化学习和零样本遥操作的对比。

Result: 方法在所有评估种子中满足支撑、作业间隙和非穿透标准；在钉枪、锤击和推挤中，作业间隙误差为0.256–0.531 cm，每任务3/3成功；物理实验复现上坡行走、钉枪、锤击和弯腰动作，基座系平均运动误差低于80 mm。

Conclusion: 场景锚定的人体运动学习是构建面向建筑的人形机器人运动基元的有前景基础。
...
### [353] [Walking on the Slope: Stable Bipedal Gaits with Genetic-Algorithm-Optimized Trajectories](https://arxiv.org/abs/2609.20570)
*Madhav Rijal*

Main category: cs.RO

TL;DR: 本文对8自由度双足机器人在平地与斜面行走进行运动学、动力学建模、轨迹生成和稳定性分析。


<details>
  <summary>Details</summary>
Motivation: 研究8自由度双足机器人在不同步时和坡度下的稳定行走边界，并通过优化步态参数提升动态稳定性。

Method: 采用DH参数和齐次变换推导正运动学，闭式逆运动学将三次样条生成的髋部与摆动足笛卡尔轨迹映射为关节角；用Newton-Euler迭代算法计算关节力矩；用ZMP判据评估动态稳定性；用遗传算法以关节做功最小为目标，并加入ZMP可行性惩罚，优化髋高、最大摆动足抬升高度和额状面倾斜角；在MATLAB中仿真。

Result: 标称8自由度模型在步完成时间低至0.5 s、坡度达22.5°时仍满足ZMP稳定；超过这些极限后ZMP会离开支撑多边形，需修改足部尺寸或轨迹参数；ZMP稳定性主要受各连杆质量分布影响，而非机器人总质量。

Conclusion: 所提建模、轨迹生成、动力学计算和优化框架可用于双足机器人平地与斜面稳定行走分析，并给出了稳定性的步时与坡度边界及后续设计调整方向。
...
### [354] [Accelerating Visual Policy Learning with Sampling-Based Model Predictive Control](https://arxiv.org/abs/2609.20575)
*Yilang Liu,Haoxiang You,Qian Wang,Daniel Rakita,Ian Abraham*

Main category: cs.RO

TL;DR: 提出Sampling-Guided Policy Search (SGPS)，将基于采样的模型预测控制与一阶策略优化结合，用于视觉运动策略学习，并在仿真和真实机器人上验证。


<details>
  <summary>Details</summary>
Motivation: 学习视觉运动与操作策略需协调环境接触，计算和GPU内存成本高。一阶策略梯度(FoPG)通过可微分仿真降低训练成本，但局部优化可能收敛到非预期的接触模式。

Method: SGPS通过采样-based MPC反复细化动作目标，并与一阶策略优化耦合。行为克隆从采样动作初始化策略；训练交替进行采样细化和短视界FoPG更新，并在扰动初始状态和随机动力学下进行。视觉策略训练采用解耦FoPG，将渲染排除在计算图外，无需状态策略教师即可直接从深度观测学习。

Result: 在单GPU上，SGPS在仿真Unitree Go2和G1机器人上学习运动、越障、推箱子和双臂搬运策略。实验表明细化能提升策略学习，超越仅初始化和跟踪。硬件部署时，蒸馏策略零样本迁移到真实Go2，利用板载深度自主小跑、爬行、跨越障碍并在行为间切换。

Conclusion: SGPS通过结合采样细化和FoPG，解决了FoPG局部优化问题，实现了高效视觉策略学习，并成功零样本迁移到真实机器人。
...
### [355] [Semantic SLAM in Precision Agriculture using Bayesian Inference](https://arxiv.org/abs/2609.20604)
*Ruben Beumer,Sander Doodeman,René van de Molengraft,Duarte Antunes*

Main category: cs.RO

TL;DR: 本文提出一种面向精准农业自主机器人的实时语义世界建模框架，将贝叶斯概率对象/语义属性映射与基于g²o的图优化SLAM结合，并用YOLOv8n从深度相机提取语义信息。


<details>
  <summary>Details</summary>
Motivation: 精准农业中的自主机器人需要在不完全依赖GPS的情况下实现准确建图与定位，同时利用植物类型、大小和健康状态等语义信息执行任务。

Method: 采用贝叶斯推断对对象及其语义属性进行概率建图；使用g²o实现图优化SLAM；训练YOLOv8n目标检测模型处理深度相机观测；通过Gazebo仿真和Boston Dynamics Spot机器人在室内人工植物田中进行物理实验验证。

Result: 仿真与实验表明，系统能够实时完成至少400株植物的建图，并同时进行定位与语义信息提取。

Conclusion: 该框架将语义世界建模与图优化SLAM有效集成，可在精准农业场景下支持机器人边建图、边定位、边执行任务，且无需仅依赖GPS。
...
### [356] [V2-STRep: VLM-Grounded Structured Task Representations for Reusable Robot Skills Acquired from Generated Videos](https://arxiv.org/abs/2609.20582)
*Yexin Hu,Dongheui Lee*

Main category: cs.RO

TL;DR: 提出V2-STRep零样本框架，将生成视频中的运动转换为可重用机器人技能，通过VLM结构化任务表示实现跨场景迁移和指令适应。


<details>
  <summary>Details</summary>
Motivation: 人类操作视频可提供机器人技能学习线索，但生成视频恢复的运动仅针对单一场景，任务结构、几何关系和约束隐含其中，难以重用。

Method: V2-STRep使用VLM生成结构化任务表示（运动阶段、参考、约束），用点、法线、轴、平面、6D位姿描述目标；通过RGB-D将2D线索提升到3D重建几何和抓取位姿；几何规则迁移运动，任务约束轨迹优化耦合抓取选择与运动规划，并利用旋转自由度满足关节限制；更新部署接地和约束即可复用。

Result: 在六个真实操作任务上，执行成功率优于基线，成功习得技能可跨场景迁移，并能适应变化的部署指令。

Conclusion: V2-STRep有效将生成视频运动转化为可重用机器人技能，实现跨场景和指令适应，提升执行成功率。
...
### [357] [INSPECT: Learning Robot View Selection from Assistant Use](https://arxiv.org/abs/2609.20615)
*Di Wen,Kailun Yang,Wenhao Guo,Yitian Shi,Junwei Zheng,Yufan Chen,Ruiping Liu,Jiale Wei,Rania Rayyes,Kunyu Peng*

Main category: cs.RO

TL;DR: 提出INSPECT方法，让机器人从智能眼镜助手的记录中学习视角偏好，用于装配检查中判断零件是否存在及安装是否正确，无需目标域视角标签即可选择最佳观察视角。


<details>
  <summary>Details</summary>
Motivation: 机器人在装配检查中需要确定零件是否出现、安装是否正确。第一人称装配辅助场景下，头部运动、工件操作和语音确认提供了检查证据，但机器人难以直接获得目标域的视角标签来训练视角选择策略。

Method: INSPECT框架包含：PI-TwinSwap（通过成对身份干预校准物体证据）、claim-indexed监督（分离证据需求与相机可复现的观察变化）、物体中心校准（将相对视角偏好适配到机器人位姿）、子句级筛选（检查预测证据）。机器人仅利用当前观察和已知位姿选择视角，无需候选图像。

Result: 在物理齿轮箱装配图像上，INSPECT在非预言机策略中取得最高视角效用，将人工评估的完全可验证率从34.8%提升至41.7%。在IMPACT商用角磨机记录上，迁移的相对视角选择器使冻结感知头的正确决策率从50.6%提升至54.3%。

Conclusion: INSPECT能有效从辅助记录中学习机器人视角偏好，提升装配检查的视角效用和决策正确率，且无需目标域视角标签。代码已开源。
...
### [358] [Bayesian Continuum Robot Dynamics and State Estimation](https://arxiv.org/abs/2609.20605)
*James M. Ferguson,Tucker Hermans,Alan Kuntz*

Main category: cs.RO

TL;DR: 该论文提出一种面向连续体机器人动态状态估计的因子图方法，通过近似Cosserat杆动力学，在惯性效应显著时提升估计精度。


<details>
  <summary>Details</summary>
Motivation: 现有因子图方法多基于准静态假设或白噪声运动先验，在惯性效应显著时难以准确描述物理过程，限制动态运动下的估计精度。

Method: 将惯性和阻尼等效为外加载荷，使动态平衡保持与准静态工作相似的静态代数形式；无骨干观测时退化为随机前向仿真，有观测时联合优化运动学与动态状态并推断外部载荷等状态。

Result: 通过仿真和实验在腱驱动连续体机器人上验证，展示了随机前向仿真和状态估计能力。

Conclusion: 该方法能够处理连续体机器人的动态运动，实现动态状态估计与外部载荷推断，弥补传统准静态或白噪声先验的不足。
...
### [359] [GeoAAC: Geometry-Based Adaptive Action Chunking from Denoising Trajectories in VLA Policies](https://arxiv.org/abs/2609.20776)
*Xin Chen,Sen Chen,Yujuan Ding,Jian Liu,Guoqing Wang,Wei Ye,Heng Tao Shen,Yi Bin*

Main category: cs.RO

TL;DR: 提出GeoAAC，一种基于几何的自适应动作分块方法，用于流式VLA策略，根据动作预测可靠性动态调整动作视野。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略通常使用固定动作视野，但任务不同阶段对动作连续性、控制精度和闭环反馈的需求不同，固定视野无法适应变化的控制要求。

Method: 利用Flow Matching去噪轨迹的几何信息表征预测可靠性，发现动作前缀的几何变化与预测不确定性正相关；基于前缀几何构建视野级几何轮廓，在单次生成中自适应确定动作视野，无需额外训练。

Result: 在GR00T N1.5和π0.5上，于LIBERO、LIBERO-Pro、RoboCasa365及真实世界操作任务中，相比固定视野基线和现有自适应方法，模拟环境成功率提升最多8.7个百分点，真实世界平均成功率从53.3%提升至74.4%。

Conclusion: GeoAAC通过几何驱动的自适应动作分块有效提升了流式VLA策略在不同任务和场景下的性能。
...
### [360] [HIL-UMI: Bringing Human-in-the-Loop Post-Training of Vision-Language-Action Models to Universal Manipulation Interface](https://arxiv.org/abs/2609.20659)
...
TL;DR: HIL-UMI 是一种无需机器人的、策略引导的人类在环 VLA 后训练框架，通过手持 UMI 演示实现高效数据收集与优势估计优化，从而提升机器人操作策略。


<details>
  <summary>Details</summary>
Motivation: 大规模 VLA 模型虽提供强大先验，但适配特定部署仍困难。监督微调（SFT）面临两个持久局限：静态数据对分布外状态覆盖有限，且标准模仿目标无法区分有进展的行为与低价值数据。交互式后训练可缓解这些问题，但通常需要在物理机器人上反复执行策略并依赖人工干预。

Method: 提出 HIL-UMI 框架。在手持 UMI 演示过程中，查询当前策略但不执行其预测。使用 Energy Score 比较人类动作轨迹与策略推理，当差异表明分布外区域时触发数据收集。另一反馈循环利用低在线优势预测识别关键片段，精炼基于进展的优势估计器。更新后的估计器引导优势条件行为克隆，使用基础演示与新策略数据的平衡混合。该设计将数据收集与机器人部署解耦。

Result: 在四个真实任务（涵盖长时程和精细操作）上，HIL-UMI 相比 SFT 取得持续改进，并受益于针对性收集和优势精炼。在 Clean Up Table 任务上，HIL-UMI 优于 HG-DAgger，且每帧收集时间更低。

Conclusion: HIL-UMI 为跨操作员和地点的 VLA 后训练提供了一条可扩展路径。
...
### [361] [A Simulation Platform for AUV Fault Recovery: Exploring LLM-Based Diagnostic Strategies](https://arxiv.org/abs/2609.20620)
*Khalid Halba,Kylie Cooper,James G. Bellingham*

Main category: cs.RO

TL;DR: 该研究提出一种AUV故障恢复架构，结合确定性分层控制与可调用的LLM作为诊断和恢复规划器，并开发SPAR仿真平台，通过480次试验评估不同LLM在质量偏移故障下的诊断与决策表现。


<details>
  <summary>Details</summary>
Motivation: 自主水下航行器在通信不可靠时需自主从故障中恢复，但传统确定性控制难以应对未预期故障；LLM可提供诊断与恢复规划，但其随机性要求严格的集成评估而非单次演示。

Method: 提出SPAR仿真架构，将实时C车辆软件与高层编排层结合，支持物理故障注入、结构化提示、LLM交互、任务文件生成、验证、执行和LLM评判评分。在480次质量偏移故障试验中，变化故障实现、提示结构、推理模型和任务条件，评估一个前沿模型和三个本地部署LLM。

Result: 模型选择主导诊断：前沿模型在85-90%的试验中将CG偏移机制列入前三假设，最佳本地模型为60-78%。本地模型成功与遵循完整诊断程序相关，弱模型常过早归因于升降舵故障。诊断与操作决策性能不耦合。

Conclusion: 贡献在于扩展了未预期故障恢复从检测到缓解的架构，以及用于评估低功耗AUV上LLM辅助任务管理的集成方法。
...
### [362] [SmellDiffusion: Diffusion-Based Quadruped Navigation with Olfactory Scene Graphs](https://arxiv.org/abs/2609.20624)
*Faith Ogunwoye,Iana Zhura,Hajira Amjad,Timofei Kozlov,Didar Seyidov,Dmitrii Plotnikov,Fedor Fedorov,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: SmellDiffusion 是一个面向指定气体泄漏源的机器人仿真流程，利用开放词汇嗅觉场景图表示特定气体区域，并在经典规划器与扩散规划器之间共享目标，结合峰值局部几何门控进行选择性源校正和气体引导扩散轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 机器人被派往指定气体泄漏源时，需要保持气体身份、估计源位置并导航至目标。然而浓度峰值可能偏离真实源，导致源估计误差，因此需要选择性校正机制与更优的气体引导规划方法。

Method: 提出 SmellDiffusion 仿真流程：在开放词汇嗅觉场景图中表示不同气体区域，并将场景图源估计作为经典规划和扩散规划的共享目标。关键组件包括峰值局部几何门控和基于扩散的气体引导轨迹生成。评估涵盖424种源-风配置、留出的峰值位移案例，以及六次匹配的Gazebo运行。

Result: 在424种已求解源-风配置中，28例浓度峰值偏离源超过0.5m。源无关几何门控在留出位移上检测到9/10例，精度0.64；门控前向匹配校正将位移案例平均误差从1.468m降至0.592m（60%），仅对14/204案例使用匹配。全体案例平均误差从0.205m降至0.180m。最佳十次扩散的平均气体暴露与气体引导A*相当（0.0476对0.0455）。单次扩散提议耗时41.7ms，气体引导A*为72.3ms，但最佳十次顺序采样增加总运行时间。普通A*也能到达同一目标，且最快、路径最短。六次匹配Gazebo运行中，机器人到源平均误差A*为0.39m，扩散为0.31m。

Conclusion: SmellDiffusion 表明，选择性几何门控可有效校正峰值位移带来的源估计误差，扩散规划能在气体暴露上与气体引导A*相当，并在Gazebo中获得更低的最终源定位误差，但顺序采样带来运行时间权衡；经典A*仍是最快且路径最短的方法。
...
### [363] [RTK-Vision PPO for Autonomous Micro UAV Recovery on an Airborne Carrier](https://arxiv.org/abs/2609.20629)
*Aashish Sahu,R Prasanth Kumar*

Main category: cs.RO

TL;DR: 本文提出一种基于RTK视觉引导的强化学习框架，实现微型无人机从移动空中载机上的自主回收，包括起飞、独立任务、返回、重新对接和随载机下降的完整循环。


<details>
  <summary>Details</summary>
Motivation: 实现可重复使用的部署-任务-回收操作，但面临远距离交会、近距离感知、载机运动、气动干扰和接触事件不连续等挑战。

Method: 采用RTK-GNSS与下视相机标记检测器提供相对定位，载机共享导航状态；终端回收阶段使用PPO策略，在MuJoCo物理仿真中训练，包含传感器噪声、气动干扰代理和标记延迟随机化，并迁移到硬件；PX4负责底层稳定，确定性安全门独立授权下降。

Result: PPO检查点在2000个保留随机终端回合中成功率达99.55%，优于调优PD基线的78.4%，中位平面终端误差6.62 cm；14次户外试验中完整任务成功13次（92.9%），涵盖近区域回收和载机偏离释放点后的回收。

Conclusion: 结果展示了完整的自主空中部署-回收循环而非孤立着陆，为检查、监视和移动物流中的可重复使用载机-子机操作奠定了实用基础。
...
### [364] [SkipVLA: Skipping VLA Steps with Classical Planning for Fast Robot Manipulation](https://arxiv.org/abs/2609.20648)
*Kaivalya Agrawal,Md Ashiqur Rahman,Raymond A. Yeh,Zachary Kingston*

Main category: cs.RO

TL;DR: 提出SkipVLA混合策略，将预训练VLA与经典运动规划器结合：自由空间移动交给规划器，仅在抓取、放置等接触丰富技能时查询VLA，从而降低推理延迟与能耗。


<details>
  <summary>Details</summary>
Motivation: VLA模型虽能直接由图像和语言指令生成机器人动作，但测试时速度慢，尤其长时程任务需频繁查询策略。现有加速方法（蒸馏、异步动作块、快慢策略）仍需对整段任务运行学习策略；而经典运动规划器虽快且能避障，却缺乏语义理解且需显式目标。

Method: SkipVLA用经典运动规划器负责自由空间运动，仅对接触丰富技能调用VLA。它复用VLA中冻结的视觉-语言主干来预测每次规划运动的目标位姿，并利用大VLA已有知识学习该预测器，无需引入额外演示数据。

Result: 在仿真中基于三种VLA评估13个LIBERO任务，并在真实6-DoF YAM机械臂上测试三个抓放任务。相比基线，任务完成速度最高提升2.5倍，能耗显著降低，同时保持相同任务成功率。

Conclusion: 将VLA的语义能力与经典运动规划的快速避障能力结合，可有效加速长时程机器人任务并降低能耗，且不牺牲成功率，为VLA实际部署提供高效方案。
...
### [365] [Workspace Models: Lightweight Robotic Memory via Saliency-Driven Supervision](https://arxiv.org/abs/2609.20820)
*Nitish Dashora,Douglas Chen,Idan Shenfeld,John Marangola,Pulkit Agrawal,Max Simchowitz*

Main category: cs.RO

TL;DR: 本文提出 workspace token：在训练时利用 VLM 提取任务相关的当前与历史信息，并通过集合重建解码器损失蒸馏为轻量潜在记忆；部署时无需在环 VLM 推理，可作为观测的替代品用于机器人操作策略。


<details>
  <summary>Details</summary>
Motivation: 复杂机器人操作任务需要长期记忆，但直接使用完整历史会使策略容易学到伪相关并降低性能；现有方法常依赖昂贵的 VLM 在环查询来压缩历史信息，计算开销大。

Method: 训练阶段使用 VLM 识别完成当前和历史任务所需的信息，再通过 set-reconstruction decoder loss 将这些信息蒸馏到轻量的 workspace token 中；部署阶段直接查询该 token，作为观测的 drop-in replacement，无需 VLM 在环推理。

Result: 在仿真和真实硬件上，workspace token 使策略能够在没有 VLM 在环推理的情况下解决记忆密集型任务；并且它不仅更轻量，还带来了更好的策略性能。

Conclusion: 将重计算 VLM 查询移到训练时、学习可高效查询的轻量潜在记忆，是替代部署时 VLM 推理的有效方案；workspace token 能提升记忆密集型机器人操作策略的效率与表现。
...
### [366] [TraceFlow: Guiding Frozen Flow-Matching Robot Policies with Success and Failure Traces](https://arxiv.org/abs/2609.20646)
*Jiaxuan Zhang,Ruizhe Liu,Yu Zhang,Yanchao Yang*

Main category: cs.RO

TL;DR: 提出 TraceFlow：一种进度对齐的引导场，将检索到的成功与失败轨迹的动作密度转化为对冻结 flow-matching 动作专家的有界修正，仅使用每条 rollout 的终端成败位，无需更新权重。配合 TraceBank 存储时序状态-动作轨迹，在真实机器人和仿真任务中提升部分序列任务成功率。


<details>
  <summary>Details</summary>
Motivation: 冻结的 VLA 策略与 flow-matching 动作专家在权重固定后，早期 rollout 的成败无法影响当前动作块生成。现有测试时方法虽引入检索成功、critic、verifier 或动力学模型，但未把机器人自身失败 rollout 作为负证据，也未做到仅依赖终端结果位。

Method: TraceFlow 使用 TraceBank 存储带终端标签的时序状态-动作记录，初始来自目标任务训练轨迹，后续加入部署机器人自身 rollout。它把检索到的成功与失败 rollout 的动作密度转为有界修正，引导冻结的 flow-matching 动作专家；每条 rollout 只需一个终端成败位，无需其他标签或权重更新。

Result: 真实机器人有序打包任务中，基础方法 50 次按序完成 21 次，TraceFlow 完成 39 次，一次堆叠轮且无权重更新完成 47 次，错误序列从 20 降至 0。仿真中增益有选择性：RoboMemArena Sequence 从 78.92% 升至 91.50%，Transferring 从 54.41% 升至 62.00%（stacking round 2）；26 任务总体不变，Counting 与 Occlusion 分别下降 1.12 和 1.42 点，LIBERO-Plus（Long）变化 +1.27 点（p=0.0733）。堆叠增益有限，各分支在第十轮前达峰；成功失败比不能预测检索分配。

Conclusion: TraceFlow 能仅凭终端结果位利用成功与失败轨迹，对冻结 flow-matching 动作专家进行测试时引导，无需权重更新。它在部分序列与长时任务上有效，但收益具有选择性且有限，检索分配并非由成功失败比决定。
...
### [367] [DexTouch-WM: Learning Action-Conditioned Tactile World Models from Human Touch for Dexterous Robot Manipulation](https://arxiv.org/abs/2609.20649)
*Yan Qin,Yue Chen,Wenwei Lin,Shujia Liu,Chuqiao Lyu,Kailun Su,Chenze Yu,Ping Luo,Wenbo Ding,Tianxing Chen,Renjing Xu*

Main category: cs.RO

TL;DR: 提出 DexTouch-WM，一种动作条件世界模型，可利用可扩展的人类触觉数据，联合预测未来 RGB 观测与双手触觉动态，并迁移到机器人灵巧操作中。


<details>
  <summary>Details</summary>
Motivation: 接触丰富的灵巧操作预测模型依赖密集触觉交互，但真实机器人数据采集成本高且受限于特定传感器与本体，难以规模化。

Method: 在人类和灵巧机器人手上部署共享传感布局的柔性压阻阵列，将人类运动重定向到机器人动作空间，使人类交互可监督同一动态模型；模型耦合预训练视频专家与轻量触觉专家，并使用解剖感知触觉 token 和对齐的动作条件。

Result: 在人类到机器人的扩展实验中，固定 5 小时真实机器人监督，将人类交互从 0 增加到 100 小时，在未见机器人域上的视觉、几何和接触预测均显著提升，且人类与机器人任务集不重叠。

Conclusion: 可扩展的人类交互可作为学习灵巧机器人世界模型的补充数据维度；该世界模型还能作为策略评估的替代环境，并生成合成轨迹用于真实机器人策略学习。
...
### [368] [Custom PX4 firmware for autonomous hybrid aerial-marine missions](https://arxiv.org/abs/2609.20691)
*Andrea Capuozzo,Fabio Ruggiero,Vincenzo Lippiello*

Main category: cs.RO

TL;DR: 本文为混合空中-水上无人机扩展了PX4固件，新增手动与自主水面导航模式，并集成到标准PX4任务流程与QGroundControl中，实现空中与水上任务的统一规划与执行。


<details>
  <summary>Details</summary>
Motivation: 测绘与监测水域环境需要能够结合飞行和水面航行的混合空中-水上无人机，但现有PX4固件缺乏对这类平台的水面导航及混合任务支持。

Method: 在PX4固件中扩展实现手动和自主水面导航模式，将其接入标准PX4任务管线与QGroundControl界面，同时保留原有飞行功能和安全机制，并支持区分空中与水面航点的混合任务规划。

Result: 仿真案例验证了实现方案，表明在平静和有浪条件下均能实现稳定的水面导航。

Conclusion: 该框架能够在不破坏原有飞行功能与安全机制的前提下，支持混合空中-水上任务的统一规划与执行，为水域测绘与监测应用提供了可行方案。
...
### [369] [Learning Foresight without Explicit Trajectories for 3D Diffusion Policies](https://arxiv.org/abs/2609.20669)
*Zhongbo Zhang,Zaibin Zhang,Yifan Wang,Changbo Yan,Lijun Wang,Huchuan Lu*

Main category: cs.RO

TL;DR: 提出 Movement Trend Guidance，从短观测历史中学习交互演化潜表示，为3D扩散策略提供未来导向条件，在多个仿真与真实任务上显著优于DP3。


<details>
  <summary>Details</summary>
Motivation: 现有3D扩散策略主要根据当前观测生成几何上可行的动作，缺乏对交互未来走向的预见；而成功操作不仅需要知道当前可行运动，还需要预判交互将如何发展。已有方法大多让这种预见从动作学习中隐式涌现。

Method: 从短观测历史学习紧凑的交互演化潜表示；训练时用稀疏的未来夹爪状态监督该表示；推理时仅保留该潜表示作为未来导向条件，与当前观测一起用于动作生成。潜表示提供全局条件，并在UNet瓶颈处加入门控FiLM分支。方法仅增加3.52%参数，保留DP3原有的密集动作与滚动时域形式。

Result: 在RoboTwin2.0、LIBERO-40和DexArt上持续优于DP3：RoboTwin2.0 50任务混合训练达到62.8% vs 56.1%；LIBERO-40达到71.93% vs 37.08%；五个真实机器人任务达到72.0% vs 49.0%。

Conclusion: 扩散策略可以从“知道交互将走向何处”中获得显著收益，而无需显式规划或被告知具体移动目标。
...
### [370] [Towards Scaling Marine Perception with Synthetic Data](https://arxiv.org/abs/2609.20680)
...
TL;DR: 本文扩展了基于IsaacSim的水下感知模拟器OceanSim，加入合成数据生成管线，用于生成大规模、自动标注、可配置的水下数据集，并在真实海胆检测任务上评估合成场景变化对仿真到现实迁移性能的影响。


<details>
  <summary>Details</summary>
Motivation: 水下环境中的可扩展机器学习受限于缺乏标注的真实世界训练数据，且真实数据采集与整理成本高、难度大，因此需要利用模拟数据来缩小差距。

Method: 在OceanSim基础上提出合成数据生成管线，支持生成照片级真实感、自动标注、可配置场景外观、结构和传感器设置的大规模数据集；在真实海胆检测任务上进行评估，并研究不同合成场景变化对sim-to-real性能的影响。

Result: 通过实验讨论了合成场景变化对仿真到现实迁移性能的影响，总结了当前管线的主要局限，并识别出提升水下渲染保真度、场景多样性和sim-to-real泛化评估的未来方向。

Conclusion: 该合成数据生成管线有助于水下感知模型的训练与仿真到现实迁移研究，代码已开源，但当前仍存在渲染保真度、场景多样性及泛化评估方面的限制。
...
### [371] [HOPHY: A Hierarchical Hypergraph Representation for Off-Road Path and Mission Planning](https://arxiv.org/abs/2609.20694)
*Pranay Meshram,Charuvahan Adhivarahan,Prithvi Poddar,Ehsan Tarkesh Esfahani,Chen Wang,Souma Chowdhury,Karthik Dantu*

Main category: cs.RO

TL;DR: 本文提出HOPHY，一种基于超图的可复用分层越野地形表示，将地图尺度地形组织为几何连通的语义区域、保持连通性的关键区域以及带类型的超边，从而支持高效路径与任务规划。


<details>
  <summary>Details</summary>
Motivation: 灾害响应、搜救和战术UGV等任务需要在地形、智能体类型和目标变化时反复进行路径与任务规划；像素网格搜索在公里级重复查询中代价高，而语义抽象需在条件变化时维持有效代价与连通性。

Method: HOPHY构建分层地形表示：GSNodes表示几何连通的语义区域，Coarse Regions表示保持连通性的关键区域，并用类型化超边编码地形、智能体和天气上下文；通过超边交集选择受影响区域和关联边进行状态更新，而无需重建整个层次结构。

Result: 在公里级真实越野地图上，HOPHY实现100%规划成功，与oracle（像素A*）相比中位代价偏差低于0.01%，查询和重规划延迟显著低于像素与抽象基线；在多机器人任务分配中，总计算量较像素A*减少79倍、较最快抽象基线减少7.2倍，任务总完成时间与像素A*相当；并在Clearpath Jackal实体机器人上成功执行1.5公里、八任务混合地表户外任务及阻塞触发的重规划路线。

Conclusion: HOPHY能够以可复用的分层超图表示，在保持接近像素级最优代价和连通性的同时显著降低重复规划与任务分配计算开销，并具备实体机器人长距离多任务越野部署能力。
...
### [372] [MAGNETAR: Multipath-Guided Spatial Posteriors for Transmitter Pose Inference in the Upper Mid-Band](https://arxiv.org/abs/2609.20670)
*Haozhe Lei,Ruibin Chen,Yuhan Jiang,Ali Rasteh,Aditya Dhananjay,Sundeep Rangan*

Main category: cs.RO

TL;DR: 提出MAGNETAR，从单次异步射频多径快照（包含到达角和信噪比估计）推断发射器平面位置与朝向的联合后验分布。


<details>
  <summary>Details</summary>
Motivation: 机器人定位无线电发射器时，仅用点估计不够：在杂乱房间中，一次测量可能对应多个发射器位置，且由于中高频段天线具有方向性，还可能对应多个朝向。因此需要推断位置与朝向的联合后验。

Method: 采用五种神经评分器，最终选用共享2D U-Net，以每个候选朝向为条件，在离散化的位置-朝向网格上联合归一化评分。训练使用真实到仿真校准的10 GHz仿真数据以及少量实测数据。

Result: 在留出仿真中，基于网格的联合后验优于参数化方法；朝向条件评分器在机器人实测中迁移效果最好；融合联合后验优于融合仅位置边际。

Conclusion: MAGNETAR能有效推断发射器位置与朝向的联合后验，网格化联合建模和朝向条件设计有助于提升实际机器人测量中的性能。
...
### [373] [MoWAM: Explicit Future Motion Prediction for Efficient World Action Models](https://arxiv.org/abs/2609.20709)
*Jiayu Wang,Bin Zhu,Yue Yu,Jingjing Chen*

Main category: cs.RO

TL;DR: 提出MoWAM，一种高效的世界动作模型，用显式未来运动预测替代推理时的未来视频生成，从而提升机器人策略学习效率与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 世界动作模型通过融入未来动态来改进机器人策略学习，但推理时显式生成未来视频计算开销巨大；若移除未来生成，未来动态只能隐式编码在观测特征中，在分布偏移下鲁棒性受限。

Method: MoWAM将结构化机器人运动建模为未来紧凑抽象，而非重建完整未来场景；采用Mixture-of-Transformer架构，在训练中学习未来视觉动态并联合预测运动与动作，推理时完全去除视频生成；同时利用紧凑运动表示进行推理时缩放，采样多组运动-动作候选，并用运动感知任务进度验证器进行选择。

Result: 在LIBERO、LIBERO-Plus和真实世界操作任务上，MoWAM取得强分布内性能、更好的分布外鲁棒性，以及高于代表性WAM基线的真实世界平均成功率；且随着候选数量增加，性能持续提升。

Conclusion: 显式未来运动为推理时缩放提供了有效且高效的基础，使模型在无需视频生成的情况下仍能保留对未来的显式表示。
...
### [374] [MILER: Semantic Mid-Level Representation for Sim-to-Real Reinforcement Learning in Unstructured Autonomous Driving](https://arxiv.org/abs/2609.20747)
*Thomas Steinecker,Denis Trescher,Alexander Bienemann,Thorsten Luettel,Mirko Maehlisch*

Main category: cs.RO

TL;DR: 提出MILER端到端策略框架，通过自定义语义中层表示模拟器和轨迹对齐策略，实现非结构化环境下感知与控制的零样本sim-to-real迁移，并在真实车辆上完成无人干预测试。


<details>
  <summary>Details</summary>
Motivation: 强化学习具有超越人类表现和自主学习策略的潜力，但在真实世界自动驾驶中应用仍少，尤其在非结构化环境中，主要受限于sim-to-real迁移困难。

Method: 离线训练阶段，使用自定义语义中层表示（MLR）模拟器，以强化学习训练策略网络，控制输出直接作用于自行车模型。实车部署时，用BEVFusion处理相机和LiDAR数据，生成与MLR模拟器一致的语义鸟瞰图表示。策略网络生成的动作不直接用于实车，而是采用轨迹对齐策略，实现感知和控制的零样本sim-to-real迁移。整套软件栈运行在Jetson AGX Orin上。

Result: 在包含多种障碍物、发夹弯、最高33.6 km/h速度及越野路段的多样化测试赛道上评估。两辆不同车辆在3.0 km测试赛道上无人工干预行驶17.3 km，验证了方法有效性。

Conclusion: MILER能够有效实现非结构化环境下端到端自动驾驶策略的零样本sim-to-real迁移，并在真实车辆上表现出良好性能。
...
### [375] [OPTED: On-Policy Fine-Tuning for End-to-End Driving using a Render-Free Teacher](https://arxiv.org/abs/2609.20756)
...
TL;DR: 提出OPTED，将强化学习从端到端驾驶策略的后训练中解耦：先用向量化输入训练特权教师，再用教师监督预训练学生在闭环中微调；在AlpaSim中使TransFuser和VaVAM驾驶分数分别提升1.6倍和9.5倍。


<details>
  <summary>Details</summary>
Motivation: 单纯扩大预训练数据收益递减，后训练在自动驾驶等物理AI领域愈发重要。端到端驾驶策略通常通过人类演示进行开环行为克隆预训练，但闭环部署时误差累积会使车辆偏离训练分布，增加安全风险。闭环后训练可缓解该问题，但基于传感器的策略需要昂贵仿真。

Method: OPTED将强化学习与端到端策略后训练解耦：在HD地图和边界框等向量化输入上，用RL训练特权教师；教师再在闭环后训练中为预训练学生提供监督。作者将OPTED应用于TransFuser和VaVAM两个相机模型，并在AlpaSim中使用真实驾驶日志的3DGS神经重建进行微调，同时与直接RL后训练进行对照实验。

Result: 两个相机模型的驾驶分数分别提升1.6倍和9.5倍。对照实验中，OPTED以约少三个数量级的仿真器交互达到与直接RL后训练相当的闭环性能，同时更接近人类先验。

Conclusion: OPTED能够高效提升端到端驾驶策略的闭环性能，显著减少仿真交互需求，并更好保持与人类驾驶先验的一致性，为物理AI中的闭环后训练提供可行方案。
...
### [376] [Underwater Visual Target Tracking with Target-Specific Depth Estimation and Adaptive Model-Fusion Predictive Control](https://arxiv.org/abs/2609.20731)
*Yuheng Zhou,Haiyang Cheng,Yanqi Feng,Pangkit Fong,Mei Xuan Lee,Marcus Gee,Chongrong Fang,Jianping He*

Main category: cs.RO

TL;DR: 本文提出一种用于自主水下航行器的立体视觉伺服框架，通过稳定深度感知与解耦控制实现水下目标跟踪。


<details>
  <summary>Details</summary>
Motivation: 水下视觉目标跟踪面临深度测量不可靠和目标运动未知的问题，传统方法难以稳定获取三维相对状态并实时控制。

Method: 感知上，利用颜色、视差和时间线索构建目标深度掩膜，选择可靠目标像素，并分别对深度测量和图像中心进行卡尔曼滤波，得到稳定三维相对状态；控制上，解耦偏航调节与平移控制，采用自适应模型融合预测控制，融合恒速与零速目标模型，并基于历史预测误差更新模型权重，在驱动、跟随距离和视场约束下计算平移指令。

Result: 通过仿真和真实实验验证了框架的有效性，并表明其性能优于现有框架。

Conclusion: 所提出的立体视觉伺服框架能够提升水下目标跟踪的可靠性与实时性，适用于AUV自主目标跟踪任务。
...
### [377] [Agile-WAM: An Agile Tactile World Action Model for Contact-Rich Robot Control](https://arxiv.org/abs/2609.20761)
*Hanchu Zhou,Brendan Lynch,Raman Goyal,Dechen Gao,Begum Kasap,Boqi Zhao,Junshan Zhang*

Main category: cs.RO

TL;DR: 本文提出了TARO，一种用于接触丰富机器人控制的敏捷触觉世界行动模型（World Action Model）。它通过多时间尺度多模态预测，联合生成动作块和未来视觉/触觉潜变量，在仿真和真实任务中实现了高成功率和低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 近期触觉世界行动模型依赖大规模预训练生成骨干网络来捕捉接触丰富的物理动力学，这限制了推理效率和灵活部署。

Method: TARO将视觉和触觉观测编码到共享潜空间，直接进行视觉-触觉到动作的流匹配过程，联合生成动作块和未来视觉/触觉潜变量。针对视觉和触觉信号时间尺度不同（视觉帧间相似，触觉在接触时突变），引入多时间尺度多模态预测：对视觉潜变量使用较大时间偏移的监督，对触觉潜变量预测下一帧以捕捉细粒度接触动力学。

Result: 在九个仿真和五个真实世界接触丰富操作任务中，TARO成功率高且推理延迟低。在五个真实实验中，总体成功率相对提升29.4%，推理延迟为11.9毫秒。

Conclusion: 多模态世界行动模型可以通过敏捷架构实现，适用于精确且高频率的机器人控制。
...
### [378] [StageGuard: Learning Stage Transitions for Long-Horizon Robot Tasks via Agentic Distillation](https://arxiv.org/abs/2609.20791)
*Jinbang Huang,Yuanzhao Hu,Zhiyuan Li,Ran Qi,Yixin Xiao,Yangzheng Wu,Tengyue Ba,Zhanguang Zhang,Yingxue Zhang*

Main category: cs.RO

TL;DR: 提出StageGuard，一种智能体蒸馏框架，用于分层机器人规划中准确高效地判断子任务完成与技能切换，通过教师模型推理与演示轨迹生成结构化解释，蒸馏到轻量学生VLM实现实时监控。


<details>
  <summary>Details</summary>
Motivation: 现有分层规划框架依赖难以获取的预设计完成信号检查器；大规模视觉语言模型虽有强推理能力，但其决策边界与任务完成标准不天然对齐，且云端部署与长推理带来高延迟，限制实时监控。

Method: StageGuard结合教师模型推理与演示轨迹生成子任务完成和策略切换的结构化解释；轻量学生VLM基于这些解释生成紧凑自解释，并用于监督微调，从而支持在线阶段转换预测。

Result: 在两个基准轨迹上评估阶段转换预测，并通过集成到BEHAVIOR-1K分层机器人控制中评估闭环任务成功率，同时在真实机器人上验证；结果显示阶段转换预测显著提升，并支持高效在线监控。

Conclusion: StageGuard能实现准确且高效的阶段转换决策，提升分层机器人长时程任务执行能力，并具备实际部署潜力。
...
### [379] [Equivalent-Agent Guidance for Cooperative UAV Payload Transportation](https://arxiv.org/abs/2609.19312)
*Saurabh Kumar,Shashi Ranjan Kumar,Abhinav Sinha*

Main category: eess.SY

TL;DR: 本文提出了一种用于两架无人机协同运输刚性载荷至静止和机动着陆平台的制导框架。采用虚拟等效智能体表示、几何分析和固定时间滑模制导策略，并通过仿真和处理器在环验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 多无人机协同运输刚性载荷至着陆平台时，需要处理初始几何不确定性、目标机动以及终端可行性等问题，现有方法难以保证收敛时间和控制能量效率。

Method: 引入虚拟等效智能体描述无人机-载荷系统的平移运动，建立相对距离和视线动力学；进行几何分析确定终端可行性条件；设计鲁棒固定时间滑模制导策略调节相对距离和视线动力学；提出独立的连杆方向控制器和控制分配方案，将虚拟智能体指令映射到各无人机控制输入；通过数值仿真和Raspberry Pi上的处理器在环实现进行验证。

Result: 该策略保证在均匀有界时间内收敛到期望着陆构型，且与初始几何无关，并能显式处理目标机动引起的不确定性。仿真表明在不同终端接近角和平台机动下均能准确投送；处理器在环实验满足实时计算要求。对比分析显示，所提框架具有更好的跟踪精度、更快的滑模面收敛速度，且每架无人机所需控制能量显著更低。

Conclusion: 所提出的制导框架能够有效实现两架无人机协同运输刚性载荷至静止和机动平台，具有强鲁棒性、快速收敛和低能耗的优点。
...
### [380] [Optimal Control and Closed-Loop Stability of Droplet Transport in a Microchannel](https://arxiv.org/abs/2609.19320)
*Rajneesh Anand,Mayuresh V. Kothare*

Main category: eess.SY

TL;DR: 本文基于润滑理论，用最优控制研究微通道中液滴的高效输运，比较降阶ODE与全非线性PDE优化，揭示两种最优输运策略，并证明CLF反馈可指数稳定目标态。


<details>
  <summary>Details</summary>
Motivation: 工业应用中受限几何内液滴高效输运十分重要，需设计最优控制策略以最小化黏性耗散并理解最优输运机制。

Method: 基于润滑理论建立最优控制框架；分别用Pontryagin极大值原理优化降阶ODE模型，用CMA-ES优化全非线性PDE模型；参数化目标位移、液滴尺寸和毛细数；用控制Lyapunov函数分析闭环稳定性。

Result: 发现两种最优输运机制：短距离为“平移-弛豫”，长距离为“压缩-平移-弛豫”；表面力与累积黏性耗散的竞争决定最优策略；降阶控制器向连续模型迁移的位移范围由毛细刚度控制；终端代价可作为物理域上代价兼容CLF，CLF兼容反馈使目标态指数稳定。

Conclusion: 最优控制框架能解释并设计微通道液滴输运策略，且CLF反馈可实现闭环稳定，为受限几何液滴操控提供理论依据。
...
### [381] [Resilient Design and Optimal Operation of Battery Energy Storage Systems for Behind-the-Meter Data Center Microgrids](https://arxiv.org/abs/2609.19342)
*Hamed Haggi,Chinmay Morankar*

Main category: eess.SY

TL;DR: 本文提出高保真超大规模数据中心负荷建模框架，并将负荷分解为IT与非IT部分，以秒级和分钟级分辨率刻画负荷行为；在此基础上，建立多阶段优化框架，用于数据中心微电网中表后BESS的规划与韧性设计。


<details>
  <summary>Details</summary>
Motivation: 超大规模数据中心负荷随机性强、可靠性要求高，易对燃气轮机等发电资产造成运行压力，并可能引发SSTI、轴系疲劳、系统跳闸、设备损坏和强迫停运等问题，因此需要合理配置BESS以提升韧性和运行可行性。

Method: 采用高保真负荷建模与多阶段优化方法：负荷侧BESS作为随机需求与发电资产之间的动态缓冲，平滑负荷尖峰；发电侧BESS优化容量以提供旋转备用。方法同时协同优化BESS与光伏、风电、天然气燃料电池、简单循环燃气轮机和联合循环燃气轮机，并计及运行、可靠性、韧性和技术约束。

Result: 仿真结果表明，所提方法能够识别兼具成本效益、韧性和运行可行性的设计方案，满足下一代超大规模数据中心的高可靠性要求和能源市场挑战。

Conclusion: 该框架可为投资者、开发商和系统运营商提供规划洞见，支持数据中心微电网中BESS与多能源发电资源的协同规划与韧性设计。
...
### [382] [PowerModels-ACOPF-AI: On-the-Fly Machine Learning Approach for Solving AC Optimal Power Flow Integrating Renewable Energy Sources](https://arxiv.org/abs/2609.19360)
*Bhuban Dhamala,Jose Tabarez,Anup Pandey*

Main category: eess.SY

TL;DR: 本文提出 PowerModels-ACOPF-AI，一种两阶段贝叶斯神经网络代理模型，用于快速且带不确定性预测 AC-OPF 的发电机设定点、母线电压和相角，并通过在线学习机制提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现代电力系统可再生能源渗透率高、负荷多变且运行不确定性强，传统 AC-OPF 优化方法虽准确但可扩展性差、计算负担重，难以满足大型网络实时应用需求。

Method: 构建两阶段贝叶斯神经网络代理模型，预测发电机设定点、母线电压和相角并量化不确定性；引入性能敏感的即时学习机制，识别预测精度下降区域，并利用 PowerModels.jl 生成额外 AC-OPF 解进行动态再训练。

Result: 在 30、200 和 500 节点基准系统上验证，模型表现出较强泛化能力，可高效处理随机可再生能源注入，并提供快速、不确定性感知的预测。

Conclusion: 该方法既可作为系统运行人员的实时辅助工具，也可作为传统求解器的快速初始化器，有助于未来电力系统实现韧性与高效运行。
...
### [383] [Feasibility and Singularity in High-Order Safety-Critical Control for Quadrotor UAVs](https://arxiv.org/abs/2609.19362)
*Omayra Yago Nieto,Leonardo Colombo*

Main category: eess.SY

TL;DR: 本文研究有界输入和成对避碰约束下四旋翼团队的高阶安全关键控制，提出基于力矩的动态扩展与高斯过程学习的四阶HOCBF方法，并在QP框架下保证避碰和标称输入恢复。


<details>
  <summary>Details</summary>
Motivation: 平方距离障碍在相对位移与可用推力方向正交时会失效，非奇异约束在共享输入界限下也可能联合不可行，因此需要刻画成对有效性和总体可行性问题。

Method: 引入力矩感知的动态扩展，将姿态力矩暴露在四阶障碍函数中，避免正推力下扩展输入行消失；用高斯过程直接学习四阶HOCBF残差，提供鲁棒裕度而无需对未知扰动求导。

Result: 在残差界和持续可行性假设下，所构造的二次规划能保证避碰，并在标称输入满足鲁棒安全和执行器约束时恢复该标称输入。

Conclusion: 该方法可处理四旋翼团队有界输入下的高阶安全控制，兼顾成对避碰有效性与输入可行性，并实现鲁棒安全保证。
...
### [384] [Data-Driven Compositional Safety Verification of Interconnected Monotone Systems](https://arxiv.org/abs/2609.19396)
*Amirreza Alavi,Majid Zamani,Saber Jafarpour*

Main category: eess.SY

TL;DR: 提出一种样本高效、组合式的互联单调系统形式化安全验证方法。无需显式局部子系统模型，利用单调性仅通过边界评估学习局部区间屏障证书，并按互联结构组合以保证整体系统安全。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法要么缺乏形式化安全保证，要么依赖基于Lipschitz的稠密状态空间离散化，导致计算开销大且可扩展性受限。

Method: 利用局部子系统的单调性，以去中心化方式仅通过边界评估构建可处理的局部区间屏障证书；局部层面使用单调神经网络，从局部状态与内部输入空间划分诱导的边界样本中学习区间屏障证书；全局层面按互联结构组合这些局部证书，并在适当结构假设下将全局安全条件转化为可扩展形式，直接嵌入神经网络损失以在局部训练中强制整体安全。

Result: 实验结果表明，所提方法在有效性和可扩展性方面表现良好。

Conclusion: 该方法可在无需显式局部模型的情况下，实现互联单调系统的可扩展组合式形式化安全验证，同时兼顾样本效率与全局安全保证。
...
### [385] [Demystifying Linear Operator Learning for Control Systems](https://arxiv.org/abs/2609.19428)
*Max Beier,Nicolas Hoischen,Sandra Hirche,Petar Bevanda*

Main category: eess.SY

TL;DR: 本文提出从数据学习控制系统线性算子的结构化方法，结合（半）群框架与逆问题学习理论，并在希尔伯特空间有界算子假设下为时变系统导出收敛估计器。


<details>
  <summary>Details</summary>
Motivation: 控制系统中的算子与演化方程算子类型相同，但现有数据驱动线性算子学习方法缺乏结构假设与学习理论保证，且常隐含矩阵化假设；需要统一框架来比较方法并设计更优算法。

Method: 利用（半）群框架刻画演化方程的结构假设；将学习算法置于逆问题框架中分析，进行误差分解、收敛性分析和最优正则化；研究范围限定为希尔伯特空间上的有界算子。

Result: 揭示了学习模型如何依赖数据，给出误差分解、收敛保证与最优正则化；可比较现有方法并推导出具有可证明优势的算法；为时变系统导出一个收敛估计器。

Conclusion: 所提结构化和理论框架有效，虽有界算子假设看似限制，但与现有矩阵化做法一致；该框架能指导控制系统线性算子的数据驱动学习并得到收敛估计。
...
### [386] [Bifurcation Beyond Surface-Tangential Asymptotic Convergence in Continuous Sliding Mode Control](https://arxiv.org/abs/2609.19682)
*Fan Zhang,Jingwen Xu,Peng Li,Jun Zhou,Yaohua Guo*

Main category: eess.SY

TL;DR: 本文通过严格相平面分析，研究二阶系统在连续滑模控制下沿滑模面切向收敛这一隐含假设，发现渐近状态比存在分岔，系统可能收敛到隐式次级流形而非设计滑模面。


<details>
  <summary>Details</summary>
Motivation: 文献长期依赖相轨迹图隐含认为二阶连续滑模控制下轨迹会沿设计滑模面切向趋于平衡点，但该切向收敛/平滑进入假设缺乏严格验证。

Method: 针对双积分器系统在标准线性滑模控制下进行相平面分析，推导收敛比的闭式表达，并建立分类框架；进一步扩展至经典PD控制。

Result: 渐近状态比不唯一，取决于控制增益、滑模面参数和初始条件；系统可能沿隐式次级流形收敛，平滑进入并非全局保证。PD控制下分岔阈值与临界阻尼边界一致。仿真验证了收敛区域分类。

Conclusion: 研究揭示了终端几何与收敛平滑性之间的联系，为高性能运动控制设计提供了可预测的收敛分类框架。
...
### [387] [Design of Economic Dispatch Schemes of An Isolated BESS Network Based on Distributed Discrete-time PI+Rest Consensus](https://arxiv.org/abs/2609.19804)
*Yalin Zhang,Zhongxin Liu,Zengqiang Chen*

Main category: eess.SY

TL;DR: 本文针对孤立电池储能系统网络中容量退化和功率损耗导致运行成本增加的问题，提出了两种基于离散时间动力学的分布式经济调度方案，并引入带重置机制的PI+R协议，以实现最优输出功率分配和供需平衡。


<details>
  <summary>Details</summary>
Motivation: 孤立BESS网络中电池单元容量退化与功率损耗会推高运行成本，因此需要优化经济调度，在考虑动态线路损耗和容量约束的同时保证供需平衡。

Method: 设计了两种分布式经济调度方案，核心包括边际成本一致性控制器和平均功率不匹配估计器，二者相互耦合。采用PI+R协议：当比例项过零时积分项重置为0，以加快收敛并减少超调。两种方案的区别在于MC一致性控制器是否将估计的平均功率不匹配反馈项纳入闭环误差。

Result: 仿真结果表明，所提方案有效，且第二种方案在一致性速度、收敛速度、BESS/代理即插即用、负载切换和广域系统应用等方面显著优于原有方案。

Conclusion: 两种方案的有效性、重置机制工作条件和稳定性均得到分析；带反馈的第二种PI+R方案能大幅提升性能，适合孤立BESS网络的分布式经济调度。
...
### [388] [On unified asymmetric barrier Lyapunov functions](https://arxiv.org/abs/2609.19886)
*Susmitha T Rayabagi,Shashi Ranjan Kumar,Debasattam Pal,Dwaipayan Mukherjee*

Main category: eess.SY

TL;DR: 本文提出一种统一的非对称障碍Lyapunov函数（BLF），推广了现有的对数对称BLF，并基于该函数为带输出约束的SISO非线性系统设计了可实现指数快速输出跟踪的控制律。


<details>
  <summary>Details</summary>
Motivation: 现有障碍Lyapunov函数在处理约束控制时，尤其是非对称障碍Lyapunov函数中，通常需要不连续切换函数，且对称与非对称约束下的控制设计不统一，因此需要一种更平滑、统一的BLF。

Method: 提出一种统一的非对称障碍Lyapunov函数，证明其光滑且无需不连续切换函数；基于该BLF设计控制律，用于带输出约束的一类单输入单输出非线性系统；并构造性地证明该函数可由对称class K函数上下界约束，从而分析局部指数收敛性；最后通过数值仿真与现有对数BLF比较。

Result: 所提出的BLF推广了对称对数BLF，避免了非对称BLF中常见的不连续切换；基于该函数的控制律能够保证指数快速输出跟踪，并统一了对称与非对称输出约束下的控制设计和结构；同时借助class K函数上下界证明了局部指数收敛。

Conclusion: 所提出的统一非对称障碍Lyapunov函数为约束控制提供了一种光滑且统一的设计框架，适用于对称和非对称输出约束，并实现了指数收敛性能，数值例子验证了其相对于现有对数BLF的优势。
...
### [389] [Foldable Antenna Array in Space-Air-Ground Integrated Networks: Architectures and Applications](https://arxiv.org/abs/2609.20011)
*Jikang Deng,Ki-Hong Park,Mohamed-Slim Alouini*

Main category: eess.SY

TL;DR: 本文介绍了可折叠天线阵列（FAA）在空天地一体化网络（SAGIN）中的基础、架构、应用、挑战和未来方向，并通过案例研究展示了其通信性能提升。


<details>
  <summary>Details</summary>
Motivation: SAGIN集成异构平台，对灵活天线架构有强烈需求。FAA通过可控关节重构阵列几何、位置和方向，是有前景的解决方案，但现有研究仅考虑简单折叠配置和有限部署优势，其架构和更广泛潜力尚未充分探索。

Method: 本文提出了FAA的基础和架构，包括折叠机制、可折叠级别和功能能力；分析了FAA在地面基站、低空平台、高空平台和卫星中的应用及挑战与解决方案；最后指出开放问题和未来方向，并进行案例研究。

Result: 通过案例研究展示了FAA在SAGIN中通信性能的提升，并总结了FAA在不同平台应用的潜在挑战与解决方案。

Conclusion: FAA是SAGIN中灵活天线架构的有前景解决方案，其架构和应用潜力值得进一步研究，未来需解决开放问题并探索更多方向。
...
### [390] [The Small-Talk Effect in Practical Synchronization of Heterogeneous Oscillating Dynamics](https://arxiv.org/abs/2609.20115)
*Finn Voland,Vincent Schmidtke,Zonglin Liu,Olaf Stursberg*

Main category: eess.SY

TL;DR: 本文研究异构Liénard振子同步中的多时间尺度通信方案，提出用子网络内更频繁的局部交互补充全局更新，并给出全局/局部更新速率的设计条件。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖所有网络系统在离散时刻进行全局信息交换，难以灵活权衡通信速率与同步性能。

Method: 提出多时间尺度通信方案：全局更新与子网络内更频繁的局部交互相结合；针对给定同步误差上界，推导全局和局部更新速率可作为设计参数选取的条件，并用数值仿真验证。

Result: 得到了在预设同步误差界下，全局更新速率与局部更新速率可设计选取的条件；数值例子表明局部交互具有有益效果。

Conclusion: 通过引入局部交互补充全局更新，可在通信速率和同步性能之间提供更大灵活性，并实现异构Liénard振子系统同步。
...
### [391] [Advances in Modeling Techniques for Ventricular Assist Devices: A Comprehensive Review and Future Directions](https://arxiv.org/abs/2609.20518)
*Muhammad Adel Yusuf,Nezar M. Alyazidi,Hamna Saleem,Ali Nasir,Mojeed Oyedeji*

Main category: eess.SY

TL;DR: 本文综述了左心室辅助装置（LVAD）建模与控制的发展，指出其临床转化受限并非算法不够复杂，而是建模假设、传感限制与真实心血管变异性之间存在错配。


<details>
  <summary>Details</summary>
Motivation: 晚期心衰患者对LVAD需求高，但已有建模与控制方法难以临床落地，作者希望厘清阻碍转化的结构性原因。

Method: 系统综述100余篇同行评审研究，涵盖心血管数学模型、集总参数与降阶模型、经典与先进控制策略、数据驱动及机器学习方法，并用问题驱动框架关联建模/控制选择与临床挑战。

Result: 高保真模型和智能控制器在仿真中表现良好，但依赖不可测状态、大量参数调优和密集传感，限制临床实现；简单控制方法在临床约束下往往更鲁棒；自适应与数据驱动方法有潜力，但需考虑植入式传感限制和可解释性。

Conclusion: 综述识别了LVAD adopted的结构性障碍，综合了建模与控制范式，并提出面向生理自适应、临床可实施且患者安全的LVAD系统研究方向。
...
### [392] [Sparse One-Step-Ahead Optimal Control of Time-Varying Affine Opinion Networks: Tracking and Competitive Games](https://arxiv.org/abs/2609.20450)
*Gabriel Gentil,Amit Bhaya*

Main category: eess.SY

TL;DR: 本文研究时变意见网络中资源受限的外部影响：控制器每次更新需同时选择少量个体和一个标量干预。作者利用仿射自由响应（含DeGroot与Friedkin--Johnsen动力学）研究稀疏最优控制、周期跟踪及多玩家竞争均衡。


<details>
  <summary>Details</summary>
Motivation: 在意见动力学中，外部控制通常受资源限制，只能影响少数个体并施加有限干预；因此需要理解如何精确选择影响对象与干预强度，以及多竞争者下策略互动和福利损失。

Method: 将一步问题消去标量动作后化为基数约束的支撑集目标，其全局最优由最大或最小残差分量决定，只需一次排序和O(|T|log|T|)操作。对周期仿射动力学，分析状态依赖低秩反馈、周期最终界及模型不变目标精确跟踪。多外部玩家时，稀疏最优响应保留相同排序结构，二元--连续阶段博弈为精确势博弈。

Result: 得到精确稀疏一步预测最优控制算法；周期仿射动力学具有基于周期的最终界并可精确跟踪模型不变目标；DeGroot和Friedkin--Johnsen特例揭示目标不变性与仿射失配项作用；每个冻结状态存在纯策略均衡；福利基准可分离标量竞争损失与策略支撑选择额外损失；数值实验验证精确支撑选择、切换顺序效应、跟踪、资源权衡与竞争实现。

Conclusion: 资源受限的外部影响可通过排序实现高效精确的稀疏支持选择；周期仿射网络可实现精确跟踪与可界行为；多玩家稀疏竞争构成精确势博弈并存在纯策略均衡；福利分解有助于评估竞争与策略选择造成的损失。
...
### [393] [Large Language Models as Falsifiers for Cyber-Physical Systems](https://arxiv.org/abs/2609.20752)
*Ali ArjomandBigdeli,Jiawei Zhou,Stanley Bak*

Main category: eess.SY

TL;DR: 本文提出 LLM-Falsifier，一种利用大语言模型通过最小化 STL 鲁棒度来对信息物理系统形式化规范进行证伪的方法。


<details>
  <summary>Details</summary>
Motivation: 传统 STL 证伪通常被建模为鲁棒性优化问题，并使用黑盒搜索算法求解；而大语言模型在迭代提示下展现出意外有效的优化能力。作者希望结合二者，并弥补标准数值优化器缺乏语义信息的不足。

Method: 将证伪问题转化为最小化 STL 鲁棒度，并使用 LLM 进行迭代提示优化。关键方法是向 LLM 暴露其擅长但数值优化器不具备的语义信息，包括自然语言输入/输出名称、输出轨迹，以及最小鲁棒值对应的关键时间见证。

Result: 在 ARCH-COMP 证伪基准上，以找到反例所需的平均仿真次数衡量，LLM-Falsifier 在 21 个规范中的 14 个上优于现有的代理优化、贝叶斯优化和基于搜索的测试等证伪工具。

Conclusion: 该工作表明，将大语言模型与领域语义信息结合，可以实现更智能、更样本高效的鲁棒性搜索，为 CPS 形式化规范证伪提供了有效新途径。
...
### [394] [Sustainable MRI: Interpretable Deep Learning for estimating energy and power consumption, revealing key acquisition parameters and their interactions](https://arxiv.org/abs/2609.19178)
...
TL;DR: 该研究开发了一个可解释的深度学习框架，利用MRI采集参数预测扫描序列的能耗与功率，并识别关键影响因素。


<details>
  <summary>Details</summary>
Motivation: MRI是能耗最高的医学影像模态之一，需要明确哪些采集参数驱动能耗与功率，以支持更节能的MRI协议和序列设计。

Method: 训练两个独立的深度学习模型，分别从MRI采集参数预测能耗和功率；通过分析注意力权重识别最具影响力的参数及其交互作用。

Result: 模型成功捕捉数据变异性：能耗模型R²=0.963，功率模型R²=0.843。能耗主要由时间参数驱动，而功率受更复杂的序列、梯度和射频相关交互影响。

Conclusion: 仅凭MRI采集参数即可准确预测能耗和功率；少数参数主导能耗与功率需求，为开发更节能的扫描协议和序列提供了可解释的指导。
...
### [395] [Optimal Transport Metric Learning for Feature Alignment in Partially Supervised Segmentation](https://arxiv.org/abs/2609.19176)
*Dakini Mallam Garba,Salim Abdou Daoura*

Main category: eess.IV

TL;DR: 提出一种两阶段学习框架，利用部分标注数据和多数据集信息进行多器官分割，通过器官原型与Sinkhorn-triplet损失对齐特征分布，缓解域偏移。


<details>
  <summary>Details</summary>
Motivation: 多器官分割常面临部分标注数据集和不同成像来源导致的域偏移问题，现有方法难以有效利用不完整监督信息。

Method: 第一阶段在可用标注上训练模型，准确分割已标注器官并学习稳健特征表示；第二阶段引入可学习器官原型和Sinkhorn-triplet损失，跨数据集强制器官级特征一致性，使同一器官的潜嵌入更接近、不同器官更分离，即使缺失标注也有效。

Result: 在BTCV数据集上取得与当前最优方法相当的性能，同时保持计算高效，并能有效缓解域偏移。

Conclusion: 通过显式对齐特征分布而非仅依赖伪标签，该框架适合标注资源有限的医学图像分割任务。
...
### [396] [Coupling Hyperspectral and 3D Data for the preventive Conservation of Palace-museums](https://arxiv.org/abs/2609.19179)
*Julie Fromager,Vincent Gauthier,Loïc Martinez,Danilo Forleo,Stéphane Serfaty*

Main category: eess.IV

TL;DR: 在能源与气候转型背景下，历史建筑预防性保护需关联环境变量与艺术品状况；EPICO方法提供多尺度监测框架，本文提出通过三维高光谱与LiDAR测绘创建数字孪生来增强该决策工具。


<details>
  <summary>Details</summary>
Motivation: 历史建筑及其内部艺术品受环境影响，需要综合、个性化的原位监测来理解环境变量与文物状况之间的因果机制，并改进宫殿博物馆的劣化风险评估与保护决策。

Method: 基于EPICO方法的多尺度监测与环境参数—文物状况关联框架，引入数字孪生，并以空间和物体的三维高光谱与LiDAR测绘数据驱动该数字孪生。

Result: 摘要主要提出研究目标与方案：创建数字孪生以增强EPICO决策工具；未给出具体实验结果。

Conclusion: 将数字孪生与三维高光谱、LiDAR制图结合，有望提升历史建筑预防性保护中的劣化风险评估与决策支持能力。
...
### [397] [Perceptual Refinement of an End-to-End Video Streaming Pipeline via Generative AI Layers](https://arxiv.org/abs/2609.19215)
*Emanuele Artioli,Farzad Tashtarian,Christian Timmerer*

Main category: eess.IV

TL;DR: PRESLEY 是一种生成式视频编码层，用自适应原位退化替代此前的破坏性块移除，并通过侧信道传递退化强度，在客户端用生成模型恢复背景区域，从而在低码率下大幅节省码率并保持前景无损。


<details>
  <summary>Details</summary>
Motivation: 传统编解码器对所有区域同等处理，而人眼注意力分布不均。研究希望只退化观众最不关注的区域，并在客户端重建，从而在有限码率下提升传输效率。

Method: PRESLEY 扩展 ELVIS：用可移除性掩码下的自适应原位退化取代块移除；用位打包侧信道传输每块退化强度；恢复时使用以传输视觉先验为条件的生成骨干，而非无条件修复。问题被拆为选择退化块、退化以省比特、生成恢复三个目标。

Result: 在匹配码率下，相比前代，背景质量平均实现 -56.4% BD-rate。相比原始基线，在码率饥饿目标区间最高节省 -29.4% BD-rate，并在 17/23 个序列上背景质量更优，同时前景保真度逐比特一致。用留一超块组合 oracle 作为经验上界，显示现有复杂度启发式已捕获 83.3% 的比特成本节省，剩余成本轴空间约占总码率 5%。还发现恢复后损伤是主要未解决轴，分散在 4.9-8.4 dB，并证明其可在传输前预测，留出集 rho=+0.400。

Conclusion: 该工作定义了生成式传输的适用区间，并证明传输时可对恢复性建模，为联合率-失真-恢复选择规则和后续路线图奠定基础。
...
### [398] [Physics-Informed Hemodynamic Modeling for Data-Free Prediction and Sparse-Data Assimilation](https://arxiv.org/abs/2609.19290)
*Xi Chen,Jianchuan Yang,Hongde Li,Guangxin He,Qiuyu Ye,Qiang Luo,Mao Chen,Wenqi Hu*

Main category: eess.IV

TL;DR: 提出一种物理信息驱动的血流动力学建模框架，可从双视角冠脉造影重建3D冠脉几何并预测血流速度与压力场，用于冠脉介入临床决策支持。


<details>
  <summary>Details</summary>
Motivation: 现有冠脉介入决策主要依赖造影和FFR，但造影缺乏3D深度信息，FFR仅提供单一功能指标；数值分析计算昂贵，学习类方法依赖大量监督且物理一致性不足。

Method: 构建集成深度学习框架：先用注意力增强CNN从造影重建冠脉几何；将点云映射至参考域并进行傅里叶编码；采用解耦网络分别预测速度场和压力场，并嵌入物理先验以实现跨生理条件迁移。

Result: 在32例临床患者、4种血流条件下，跨狭窄压降平均绝对百分比误差为2.02%，速度与压力相对L2误差分别为0.054和0.023；与医院实测FFR验证诊断准确率达93.8%（30/32，95% CI 79.2%-99.2%），并支持血运重建对比和稀疏数据同化，全流程约20分钟/患者。

Conclusion: 该框架可实现从造影到血流动力学的快速、物理一致3D分析，有望提升冠脉介入临床决策能力。
...
### [399] [NIMARC-MRI: Abdominal HASTE Dataset and a Baseline U-Net Exposing the Synthetic-to-Real Gap in Low-Resource Motion Correction](https://arxiv.org/abs/2609.19323)
...
TL;DR: 该文发布首个西非腹部MRI数据集NIMARC-MRI，并评估2D U-Net去呼吸运动伪影；结果显示合成运动训练在真实临床运动上泛化不佳，存在合成到真实的差距。


<details>
  <summary>Details</summary>
Motivation: 在低中收入国家，腹部T2 HASTE MRI常因呼吸运动退化，且缺乏厂商运动校正许可；失败扫描在PACS清理中被删除，导致无法获得监督训练数据。因此需要建立资源受限场景下可复现的运动校正基线。

Method: 构建NIMARC-MRI数据集，来自尼日利亚1.5 T Siemens扫描仪且无集成运动校正许可，包含139例干净HASTE、25例原生运动退化HASTE和79例配对HASTE-TSE TRIGGER。训练7.7M参数2D U-Net，使用3D一致的合成呼吸运动数据，并采用三层验证：留出合成数据、放射科医生对真实运动进行盲法Likert评分、跨序列TRIGGER泛化。

Result: 合成测试集上模型达到SSIM 0.863±0.042、PSNR 30.06±1.80 dB。但在真实原生运动上，盲法放射科医生和技师评分无显著改善：平均Likert 4.06±0.55（原始）对4.04±0.61（校正后），P=0.914，且28%病例校正后更差。跨序列TRIGGER评估显示SSIM轻度改善（0.404±0.073对0.334±0.055，P<0.001），但无放射科医生感知获益。

Conclusion: 保守合成运动未能捕捉临床运动严重程度，暴露出可复现的合成到真实差距。NIMARC-MRI已在Zenodo以受控数据使用协议发布，要求引用；序列元数据和样本子集公开，患者级数据仅限批准合作者。目标是建立资源受限环境下运动校正的可复现基线。
...
### [400] [Mammography Foundation Models for Opportunistic Prediction of Major Adverse Cardiovascular Events](https://arxiv.org/abs/2609.19385)
*Paula Feldman,Nusrat Binta Nizam,Sunwoo Kwak,Batuhan Karaman,Katerina Dodelzon,Mert Sabuncu*

Main category: eess.IV

TL;DR: 该研究评估了原本用于乳腺癌任务的乳腺X线摄影基础模型，能否在不使用心血管监督信号或钙化标注的情况下，直接从筛查性乳腺X线图像中预测女性5年主要不良心血管事件风险。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是女性首要死因，但常规心血管风险评估依赖可能缺失、过时或不可用的临床变量。筛查性乳腺X线摄影常规开展，且包含与心血管风险相关的血管特征（如乳腺动脉钙化），因此有望用于机会性心血管风险分层。

Method: 构建了22,497名女性、随访5年的MACE队列，并与电子健康记录结局链接，其中500例事件，患病率2.22%。使用乳腺X线摄影基础模型，仅以筛查性乳腺X线图像为输入，不提供临床变量，也不进行心血管特异性监督或显式BAC标注，评估其对MACE的预测能力，并与仅使用年龄的模型比较。

Result: 基础模型达到AUROC 0.823和0.822，显著超过仅使用年龄的模型（AUROC 0.765）。尽管BAC从未作为训练标签，两个模型均对放射科医生记录的BAC患者赋予显著更高的预测风险，且激活模式与血管表现一致。

Conclusion: 乳腺X线摄影基础模型能够直接从图像像素中恢复具有临床相关性的心血管风险信息，提示筛查性乳腺X线摄影可作为机会性心血管风险信息来源，在无需额外影像的情况下补充传统临床评估。代码已公开于https://github.com/PauFeld/MammoCVD。
...
### [401] [Compression Hurts, Pooling Helps: Information Loss in Rayleigh-Scale Estimation from B-Mode Ultrasound](https://arxiv.org/abs/2609.19525)
*D. Hudson Smith,Ahmer Raza*

Main category: eess.IV

TL;DR: 本文分析常规B模式超声图像用于定量超声（QUS）时的信息损失问题，重点研究未知对数压缩对RF包络统计参数估计的影响。


<details>
  <summary>Details</summary>
Motivation: 临床B模式图像广泛可得，是QUS组织表征的潜在数据源；但标准设备在显示和存储前会对RF包络数据施加未知的对数压缩，导致参数估计困难。

Method: 使用Fisher信息分析，在未知压缩律下估计RF包络统计；将对比度参数a视为未知、边界偏移b视为已知，并通过合成实验及OASBUD真实RF包络数据验证。

Result: 有限偏移对数压缩会严重损失估计Rayleigh尺度σ的信息；单个图像窗口下，未知压缩使无偏估计σ的最小方差按压缩无关因子约FisherMinInflation增大；M个等大窗口共享同一未知压缩时，额外方差按1/M衰减，但即使最有利情形，将方差膨胀因子降至1.1以下也需约FisherBestCaseWindows个窗口；若实验估计b，方差更大。

Conclusion: 常规B模式图像用于QUS存在固有局限，未知对数压缩会显著降低RF包络统计参数估计精度。
...
### [402] [The segmentation ceiling: why explicit left-ventricular masks do not improve learned ejection-fraction regression](https://arxiv.org/abs/2609.19730)
*Farshid Farhadi Khouzani,Paul La Plante,Bryar Mustafa Shareef,Laxmi Gewali*

Main category: eess.IV

TL;DR: 本文提出“分割上限”准则，量化左心室分割误差传播到射血分数（EF）预测误差的阈值，并据此检验分割信息是否真能提升超声心动图EF预测；结果发现现有分割精度未达阈值，分割策略难以超越原始视频基线，而强增强权重平均与异方差beta-NLL不确定性建模更实用。


<details>
  <summary>Details</summary>
Motivation: 临床EF由左心室舒张末期和收缩末期容积导出，因此直觉认为显式左心室分割应能改善深度学习EF预测。作者希望定量检验这一直觉，并明确分割需达到多高精度才可能带来增益。

Method: 基于EF是容积归一化差值的定义，闭式推导每帧分割面积误差如何传播为EF误差，提出“分割上限”；使用EchoNet-Dynamic、UniFormer-S骨干和实测患者内误差相关性计算盈亏平衡点。实验比较原始视频回归与四种注入分割/面积信息的策略，并测试真实掩码、权重平均+强增强，以及异方差beta-NLL不确定性估计。

Result: 分割上限约为每帧面积误差10%，而代表性分割器约14%，高于阈值。四种分割/面积注入策略均未超过原始视频基线；真实掩码仅通过标签泄漏带来虚假提升。权重平均+强增强在匹配密集片段协议下达到测试R^2=0.806、MAE=4.08，与R(2+1)D基线0.811相当且缩小验证-测试差距。beta-NLL给出良好校准的逐预测不确定性，在更难的低EF病例中更大，而蒙特卡洛dropout未能做到。

Conclusion: “分割上限”为何时值得采用掩码引导EF估计提供了具体设计准则：若分割误差高于阈值，直接视频回归更优。当前瓶颈不是输入表示，而是泛化能力；强增强权重平均和不确定性感知的beta-NLL配方是简单有效的EF回归方案。
...
### [403] [HyperAMS-Net: Adaptive Multi-Scale Spatial Hypergraph Network for Brain Disorder Classification](https://arxiv.org/abs/2609.19755)
*Proloy Kumar Mondal,Md Kamran Hussin Chowdhury,Hoi Leong Lee*

Main category: eess.IV

TL;DR: 提出HyperAMS-Net深度学习框架，用于基于rs-fMRI或sMRI的脑疾病分类，融合自适应多尺度卷积、超图注意力、空间-通道注意力和自适应特征融合，在ABIDE、REST-meta-MDD和ADNI三个数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 神经影像数据存在显著的个体间异质性以及功能连接和形态表征中的复杂多尺度模式，导致脑疾病分类仍具挑战性。

Method: 提出HyperAMS-Net框架，集成自适应多尺度卷积（学习多感受野的数据驱动权重）、超图注意力（通过节点-超边-节点消息传递建模高阶依赖）、空间-通道注意力（增强判别特征学习）和自适应特征融合（聚合并行分支互补信息）；在ABIDE、REST-meta-MDD和ADNI数据集上采用5折分层交叉验证评估。

Result: 在所有评估数据集上均达到最先进性能，准确率和AUC最高；消融实验验证各组件贡献，移除超图注意力时性能下降最大。

Conclusion: HyperAMS-Net能有效应对脑疾病分类中的异质性和多尺度模式挑战，各组件协同提升分类性能。
...
### [404] [Video Based Assessment of Surgical Skills Using Frozen Pretrained Video Foundation Models](https://arxiv.org/abs/2609.19772)
*Sangrock Lee,FNU Rahul,Suvranu De*

Main category: eess.IV

TL;DR: 论文提出 VBA-Net+，一种仅基于视频、使用冻结预训练视频基础模型提取特征的 FLS 评分回归与通过/不通过分类框架，并在缝合和剪裁任务上采用受试者级 LOUO 交叉验证评估。


<details>
  <summary>Details</summary>
Motivation: 自动化视频手术技能评估发展迅速，但在标准化评分预测中，针对未见受训者的参与者级泛化仍缺乏严格评估。

Method: 使用 VideoPrism、V-JEPA2、VideoMAE v2 等预训练视频基础模型作为冻结特征提取器，并以帧级 SimCLR 为基线；在离线嵌入上训练轻量级全卷积头，采用参与者级留一用户交叉验证（LOUO）评估缝合和剪裁两个 FLS 数据集。

Result: 连续评分预测最佳 R² 为缝合 0.6367、剪裁 0.9261；按官方 FLS 阈值的通过/不通过分类 AUC 分别为 0.9073 和 0.9906。冻结视频编码器流程总体优于帧级 SimCLR，尤其在缝合任务上更明显。

Conclusion: VBA-Net+ 为仅视频的 FLS 评估提供了基准，表明冻结视频基础模型结合轻量回归头在标准化技能评分和通过/不通过分类上具有较好泛化能力。
...
### [405] [Unifying Image Quality Assessment Datasets: MOSAIQ-500K and MOSAIQ-Bench](https://arxiv.org/abs/2609.20247)
*Wenbo Yang,Zhongling Wang,Jialu Xu,Jinghan Zhou,Zhou Wang*

Main category: eess.IV

TL;DR: 该论文通过主观实验建立统一感知尺度，将23个IQA数据集的分数映射到可比较的质量标度，构建了超过50万张图像的MOSAIQ-500K数据集，并提出MOSAIQ-Bench跨数据集基准，用于评估和训练更通用的IQA模型。


<details>
  <summary>Details</summary>
Motivation: 现有IQA数据集采用不同主观协议和评分尺度，分数不可直接比较，导致难以进行多数据集联合训练和跨数据集评估，缺乏共同感知尺度。

Method: 开展新的主观实验，将其评分作为感知锚点，对23个IQA数据集的已有分数拟合单调映射，在保留数据集内部排序的同时统一到共同质量尺度；构建MOSAIQ-500K数据集和MOSAIQ-Bench跨数据集基准，并评估31种IQA方法。

Result: MOSAIQ-500K包含超过50万张图像，是最大的具有感知对齐主观分数的IQA数据集；评估显示IQA方法在数据集内与跨数据集性能存在显著差距，尤其在真实失真上；使用对齐分数后，标准回归损失可替代专门的多数据集训练机制，获得相当的类内精度和更好的跨数据集性能。

Conclusion: MOSAIQ为整合异构IQA数据集提供了实用基础，有助于训练泛化能力更强的IQA模型，并推动跨数据集评估与统一感知尺度的应用。
...
### [406] [Effects of Sequence Timing on the Spatio-Temporal Properties of 3D BOLD fMRI: A Formal Framework and Analysis](https://arxiv.org/abs/2609.20105)
*Samuel Bianchi,Klaas P. Pruessmann*

Main category: eess.IV

TL;DR: 本文提出一个形式化框架，用于建模3D BOLD fMRI中序列时序对数据性质的影响，并通过模拟说明3D采集会降低时空保真度。


<details>
  <summary>Details</summary>
Motivation: 2D或多频带序列的层间采集时间差异影响已被较好理解，但3D序列将k空间分段采集，重建时精确时间信息丢失，文献中缺乏序列时序如何影响3D BOLD fMRI数据特性的理论描述。

Method: 基于自旋系统的统计描述纳入BOLD信号变化，并结合空间编码与图像重建的一般描述，建立形式化框架；定义实际重建图像、不受序列时序影响的最优参考图像和误差图像；定义两组算子描述图像生成，并作为时空滤波器分析序列时序效应；通过模拟验证。

Result: 所有算子均为时空滤波器，会混合信号的空间内容及其时间波形；高频BOLD信号成分比低频成分受到更强影响；模拟显示3D图像时间序列的总方差可被降低，但时空保真度逐渐丢失；还推测3D序列对生理噪声更敏感，是因为序列时序对热噪声的抑制强于对生理波动的抑制。

Conclusion: 该框架可形式化描述3D BOLD fMRI中的序列时序效应，并与现有文献建立联系；运动和系统不稳定性对图像时间序列的影响仍需进一步研究。
...
### [407] [Synthetic Fingerprints for Children Under Four: Generation and Biometric Evaluation](https://arxiv.org/abs/2609.20621)
*Faheem Ahmad,Ajan Ahmed,Mst Rumana Sumi,Stephanie Schuckers,Masudul Imtiaz*

Main category: eess.IV

TL;DR: 该论文提出了一套针对四岁以下儿童合成指纹的评估与筛选协议，并用小样本儿科指纹数据微调年龄条件生成器；从32,000个候选中最终保留1,985枚合成指纹，并评估其质量、与真实数据的相似性及身份多样性。


<details>
  <summary>Details</summary>
Motivation: 幼儿指纹识别在纵向身份应用中有价值，但该年龄段真实指纹数据有限且敏感，研究受约束；合成数据可能作为补充资源，但必须严格评估其生物特征质量、与真实训练数据的相似性以及身份多样性。

Method: 使用205枚来自9名儿童的指纹微调年龄条件生成器；对32,000个候选样本进行NFIQ 2、NBIS细节提取与匹配、指纹模式分类、与完整真实参考集的相似性以及保留合成样本间成对相似性评估；经过候选筛选和最终对称成对验证后保留样本；比较固定合成身份重复渲染的细节匹配分数与真实匹配分数，并使用DINOv2纹理表示检查相似性。

Result: 最终保留1,985枚合成指纹；最年轻年龄组在采样预算内仍产生保留样本，而最年长原始年龄组仅产生24枚保留指纹；数据驱动的两组年龄表示改善了年轻组的模式分布一致性；重复渲染固定合成身份的细节匹配分数与真实匹配分数相当，但DINOv2纹理表示显示生成器内部相似性显著更高。

Conclusion: 合成儿科指纹可支持受控研究用途，但其评估应同时包括真实-合成相似性和合成-合成多样性，并且结论应限定于研究所用的匹配器和度量。
