...
TL;DR: 该研究揭示了通过强化学习训练的推理模型相较于监督微调模型在数学推理任务中表现更优的神经机制，包括更线性的表征结构和层次化的信息处理架构。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明强化学习训练的大型推理模型性能优于监督微调模型，但其内在表征差异的机制尚不明确，需要探究两者在模型内部表示上的根本区别。

Method: 采用线性探针分析层级隐藏状态预测答案正确性的准确性，并通过均值消融实验评估各层重要性分布差异，同时结合重复采样下的token数量变异性分析自适应计算分配。

Result: RL模型在答案正确性预测上更准确且表征更线性可分；其深层网络具有层级重要性递增结构，而SFT模型各层重要性均匀；token分配变异性在部分RL模型中更高，但受整体训练流程影响大于训练方法本身。

Conclusion: RL训练通过重构模型表征和处理结构（更线性化表征+层级关键性架构）实现性能提升，而token分配策略的稳定性与策略确定性相关，非RL/SFT的简单二分法可解释。
...
TL;DR: 基于LLMs的多智能体系统在混合动机环境中面临目标错位问题，通过社交推理游戏狼人杀构建评估框架，发现目标错位会显著破坏集体决策，且与角色和不对称信息相关。


<details>
  <summary>Details</summary>
Motivation: 在混合动机环境中，LLMs驱动的多智能体系统存在目标不一致和策略性欺骗行为，需要量化评估其对集体目标的影响。

Method: 以狼人杀为测试环境，修改单个智能体的目标并保留其角色，分析不同模型系列、规模、角色和目标公式下的内部推理、公共廉价谈话行为及游戏结果。

Result: 目标错位损害对抗性环境下的结果，不对称信息和角色差异加剧此效应；被操纵的智能体发展出依赖目标的推理策略，但公共行为中不可见。

Conclusion: 微妙的目标错位可能深刻影响集体决策，需开发针对LLMs多智能体系统的缓解策略。
...
TL;DR: 提出了CLINLENS基准，用于评估临床数据科学代理在多种患者数据和时间范围上的分析能力，发现当前最强模型在126个任务中仅达到56.3%的严格通过率。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要局限于医学问答、结构化表格推理或通用科学数据，缺乏对临床数据科学代理在异构纵向记录上进行可审计分析的全面评估。

Method: 基于MIMIC资源设计200个可执行任务，采用4×5分类法（4种患者时间范围×5种分析能力），并通过程序优先反向合成生成带参考工作流的数据包。

Result: 24种模型-框架配置中最佳达到56.3%严格通过率（执行成功率100%），专业编码代理解决83/126任务，而五种生物医学系统在GPT-4o-mini上最高仅2.9%。

Conclusion: 当前模型在可运行提交与正确临床分析之间存在显著差距，CLINLENS可暴露该差距并推动临床数据科学代理的发展。
...
TL;DR: 提出了基准评估结果的“可投影性”问题，指出相邻主张的合理证据不能自动构成合理链条，并给出非组合原则与可投影性审计方法。


<details>
  <summary>Details</summary>
Motivation: 基准评估结果需通过多重推理链（推广、解释、外推等）才能支持实际应用主张，但现有研究忽略了这些推理环节之间的衔接是否合理。

Method: 结合Goodman的“竞争性扩展”问题与基于论证的有效性框架，提出非组合原则；通过法律研究案例、重分析与模拟验证方法有效性。

Result: 相邻投影的合理证据仅在端点与假设一致、依赖性与不确定性传递时才能组合；聚合稳定性可能掩盖后续投影所需的区分。

Conclusion: 基准到应用的论证存在“不可见拼接”问题，需通过可投影性审计诊断支持链条中的无效连接。
...
TL;DR: 提出GuideSkill框架，将临床指南转化为可执行的诊断技能函数，结合案例数据优化技能覆盖，显著提升LLM诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM系统通常检索或吸收指南文本，而非执行其规则，导致诊断精度不足。

Method: 构建外部推理层，从指南初始化技能（GuideSkill-Zero），通过病例-诊断对进化技能（GuideSkill-Evo），在推理时结合LLM差分诊断与技能评分。

Result: 在四个基准和四种骨干模型上，GuideSkill-Zero比指南RAG平均提高13.45%宏平均准确率；GuideSkill-Evo比直接推理相对提升18.49%，技能覆盖率从56.5%增至99.5%，且无需更新骨干模型。

Conclusion: 可执行技能是一种模型无关的机制，能有效结合指南程序与病例诊断模式，产生临床合理且广泛可接受的诊断结果。
...
TL;DR: 提出一个名为GoGoTB的智能体框架，通过三个子系统（执行控制层、可演化知识系统、规范导向的覆盖率）实现集成电路功能验证的端到端闭环，在无人工干预下达到接近100%的环境生成成功率和平均90%以上的覆盖率。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的验证方法因缺乏共享上下文，导致接口不匹配和覆盖率脱离规范要求，难以实现完整验证闭环。

Method: 采用包含执行控制层、知识系统和规范导向覆盖率的三子系统框架，将确定性执行与LLM推理分离，按需分配专家知识，并将每个覆盖率收集点与规范行为绑定。

Result: 在8个RTL设计上实现100%环境生成成功率，平均行覆盖率98.4%，分支覆盖率97.2%，翻转覆盖率97.0%，功能覆盖率83.2%，超越现有方法。

Conclusion: 该框架首次实现无人工干预的完整验证环境生成，并达到有意义的覆盖率，显著提升了功能验证自动化水平。
...
TL;DR: 提出评估中的信任膨胀现象，并建议将评估结果作为带有形式性、范围和有效期元数据的认知声明。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估多信号聚合时，平均值可能掩盖弱信号的低可靠性，导致虚假的高置信度。

Method: 基于思维链分析、可能性逻辑和代数理论中的最弱环节聚合方法，提出参数化悲观度算子族，并建议为评估结果附加形式性层级、范围声明和过期日期元数据。

Result: 在HELM榜单上，54个前沿模型在10个场景中，按平均分和按最弱环节得分排名的前五名完全不重叠，证明了均值聚合的问题。

Conclusion: 评估结果应使用最弱环节聚合，并附带显式元数据以提高认知透明性，避免信任膨胀。
...
TL;DR: TraceCoder 通过记录修复事件的历史、可视化工具和稳定的标识符索引，使基于LLM的代码生成过程可审计和可重放。


<details>
  <summary>Details</summary>
Motivation: 当前LLM编码代理生成的代码缺乏可解释性，修复过程难以追踪，无法进行事后审计。

Method: 使用关系型代码片段-历史模式记录修复事件，开发浏览器可视化工具，并采用竞争性分数位置键索引方案。

Result: 在30个算法任务上评估，平均变更率30%，十分之三的代码片段携带可追踪的修复事件记录。

Conclusion: TraceCoder使自动代码生成的内部叙事可审计和可重放，增强了生产部署中的信任和问责性。
...
TL;DR: 本文提出了StatMechBench-v0基准测试，评估基于LLM的AI代理能否从原始配分函数发现统计力学映射。研究表明，数值反馈有助于修复代码，但代理可能通过数值检查却错误识别可解类或低估计算复杂度，揭示了LLM推理的局限性，并倡导超越数值验证的符号检查与结构不变性。


<details>
  <summary>Details</summary>
Motivation: 理论物理中，将新问题转化为已知模型是重要技能，但AI代理在此类结构发现中的能力尚未被系统评估。

Method: 构建包含六个Ising型问题的基准测试StatMechBench-v0，涵盖转移矩阵、规范可移除无序和平面/帕夫良结构；采用提出-验证-修正的代理流程，测试多种LLM和问题表述。

Result: 数值反馈通常帮助代理修复代码并恢复正确配分函数，但代理可能通过数值检查却误判可解类或低估计算复杂度。

Conclusion: 当前LLM推理存在局限，需要结合符号检查和结构不变量的验证栈来评估AI代理在理论物理结构发现中的表现。
...
TL;DR: 提出了首个集成多模态感知与生成的社交推理游戏AI智能体CaM-Wolf，在狼人杀游戏中实现行为与隐藏角色的逻辑推理，并通过动画化身增强人机交互。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的社交推理游戏智能体局限于纯文本，缺乏多模态交互能力。

Method: 结合多模态视频输入、基于强化学习的因果感知推理器（Causal-aware Reasoner）和动画化身表达。

Result: CaM-Wolf在游戏表现和人机交互质量上均优于现有方法。

Conclusion: 该研究向创建能参与复杂社交动态的类人AI智能体迈出重要一步。
...
### [11] [MultivationBench: A Benchmark for Multimodal Sequential Motivation Reasoning](https://arxiv.org/abs/2607.26465)
...
TL;DR: 引入 MultivationBench 基准，评估多模态大模型在故事驱动视觉叙事中的序列动机推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估多基于静态文本或孤立视觉快照，无法反映真实世界行为驱动因素的累积性。

Method: 基于马斯洛需求层次和赖斯的基本欲望心理学框架，要求模型整合累积多模态上下文推断动态动机。

Result: 所有测试模型在序列动机推理中均难以保持一致性，表明静态识别与动态推理之间存在关键脱节。

Conclusion: 多模态大模型在社交智能所需的动态推理方面存在显著不足，MultivationBench 揭示了这一挑战。
...
TL;DR: CG-World是一个从工业计算机图形管线导出的、包含状态、动作、事件与观测联合动力学的大规模世界状态数据集，提供约85万段1-5秒的时对齐数据，支持干预学习与反事实推理。


<details>
  <summary>Details</summary>
Motivation: 现有视频、机器人及仿真数据集通常仅捕捉状态-动作-事件-观测结构的部分信息，缺少联合动力学与结构化干预数据，限制了世界模型的干预学习与反事实推理能力。

Method: 从工业计算机图形生产流程中提取显式中间状态，包括多模态语义、空间结构、骨架与控制器状态、运动曲线、相机与光照参数、物理缓存、接触事件及多通道渲染；定义分支谱系（事实轨迹、观测干预、动作干预、机制干预与严格反事实分支），并记录干预目标、不变性与替代结果。

Result: 在几何条件视频生成、动作预测及闭环视觉-语言-动作策略迁移任务上验证，表明CG-World能为受控生成、动作建模与具身策略迁移提供可复用的结构化监督。

Conclusion: CG-World可作为世界模型、物理AI和具身智能的共享数据基础设施，计划通过持续数据收集与社区协作扩展。
...
TL;DR: 提出EvoPINN框架，将物理信息神经网络开发转化为自动化算法发现问题，通过大语言模型代理和模块化搜索空间自动设计优化算法，在多种偏微分方程上显著降低误差，并自主发明了新的SLRC-PINN架构。


<details>
  <summary>Details</summary>
Motivation: 物理信息神经网络依赖人工试错设计神经表示、损失函数和优化策略，效率低下且需专业知识。大语言模型虽可自动化，但直接代码生成易导致数学无效或不稳定解。

Method: EvoPINN框架：1）解耦神经表示与训练程序，构建模块化搜索空间；2）利用大语言模型代理基于记忆迭代提出编程修改；3）对候选方案进行结构验证和预算匹配的偏微分方程评估。

Result: 在振荡、椭圆、耗散和非线性输运等偏微分方程上，EvoPINN发现的专用学习算法显著降低相对L2误差。自主发明的SLRC-PINN架构在严格参数对比下仍保持性能优势。

Conclusion: EvoPINN证明了执行级代理可自动发现新的科学计算机制，为物理信息神经网络自动化设计提供了有效范式。
...
TL;DR: 提出了一种证据账本裁决方法，通过将每个声明与证据包配对并分配支持关系，实现对AI生成声明的可审计性。


<details>
  <summary>Details</summary>
Motivation: AI代理生成的声明速度超过人工验证速度，需要可追踪的声明-证据链路以确保准确性。

Method: 构建了2335行的盲测基准，基于AVeriTeC、CLIMATE-FEVER和SciFact的外部标注，比较了证据账本代理与非代理基线的表现。

Result: 证据账本代理达到0.676的关系准确率和0.601的宏F1，远高于最佳非代理基线（0.383准确率，0.303宏F1）；正确路由了1270/1435条有矛盾或缺失证据的声明，以及295/900条已支持的声明。

Conclusion: 证据账本裁决能有效将异构证据包转化为可审计的追踪层，提升AI辅助写作的可靠性。
...
TL;DR: 提出了Eco3S框架，通过协同演化环境、结构因果模拟和仿真-分析-优化范式，解决了LLM-based ABM在环境交互、反事实推理和工作流自动化上的挑战，并在多个经济场景中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM-based ABM研究面临环境-代理交互建模困难、缺乏灵活反事实推理、以及仿真工作流难以自动化的问题，无法满足经济研究的需求。

Method: Eco3S包含三个机制：(1) 协同演化环境设计，实现代理与环境双向反馈；(2) 结构因果模拟，基于SCM进行因果干预；(3) 仿真-分析-优化范式，通过迭代自校正优化实验设计。

Result: 在运河衰落、治理起源和信息传播等经济场景中复现了经典研究结果，并证明框架具有良好的可扩展性和泛化能力。

Conclusion: Eco3S框架能够支持严谨的经济研究和政策分析，具有在社会科学中广泛应用的潜力。
...
TL;DR: 提出CAPA基准测试，研究利用同一用户历史会话解决个性化编码歧义，以减少澄清需求。


<details>
  <summary>Details</summary>
Motivation: 现有方法孤立处理歧义，未探索利用用户历史会话作为记忆来解析重复出现的个性化歧义。

Method: 定义个性化歧义适应任务，通过六种机制构建CAPA数据集，采用三阶段生成管道注入歧义，并评估12个LLM在有无历史条件下的性能。

Result: CAPA包含600个会话，实验显示同一用户历史能提升首次命中率和成功率，但部分模型仍需多次交互。

Conclusion: CAPA为开发长期编码助手提供基础，通过历史记忆减少澄清循环，更好对齐用户意图。
...
TL;DR: 提出AlphaSchema框架，将因子挖掘分解为语义空间探索与实现，通过结构化的交易语义模式提升因子发现效率与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的因子挖掘系统缺乏显式探索空间和系统导航机制，导致探索隐式且难以优化。

Method: 构建由事件、上下文、质量、方向、输出组成的结构化语义空间；LLM将选定模式转化为可执行因子；通过累积奖励学习代理模型，结合全局探索、代理引导利用和局部突变的迭代选择机制。

Result: 在中国股市实验中，发现因子池具有强预测和组合表现；语义搜索过程覆盖多样区域并集中于高奖励区；不同LLM实现的相同模式因子质量相当。

Conclusion: AlphaSchema实现了可控、透明的因子挖掘，因子质量对LLM选择具有鲁棒性。
...
TL;DR: 提出了SkillBoost框架，通过结构化利用和先验引导探索来平衡LLM技能的自我演化中的探索与开发，缓解过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 现有技能优化方法在有限轨迹数据下易过拟合，过度利用导致当前批次过拟合，无约束探索导致已解决问题退化。

Method: 三阶段框架：结构化利用定位失败到可编辑技能组件，先验引导探索生成多样化修复候选，验证接受在回归界限内改进性能时提交。

Result: 在23个模型-基准配置上达到最先进性能，缓解过拟合，且优化技能可迁移到其他智能体。

Conclusion: SkillBoost有效平衡探索与开发，提升LLM技能自我演化中的鲁棒性和泛化性。
...
TL;DR: 提出AgenticCANN框架，针对昇腾C算子自动化合成，在低语料NPU环境中通过知识增强和阶段自适应进化策略，实现算子可行性与性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在CUDA内核生成中有潜力，但昇腾C编程模型独特且缺乏探索，亟需自动化优化方法。

Method: 知识驱动生成系统提供结构化领域知识，结合阶段自适应进化策略，动态调整LLM交互模式以平衡探索与收敛。

Result: 在华为Ascend 910B上，元素级和归一化算子可行性达90-100%，融合算子56%，1B Pangu模型推理核加速达6.65倍，知识注入使可行性从57%提升至86%。

Conclusion: AgenticCANN有效解决低语料NPU环境下的算子合成难题，知识增强具有通用性而非特定算子优势。
...
TL;DR: 提出了UrbanDS，一个图引导的大语言模型多智能体系统，用于处理数据密集型城市任务。该系统通过统一数据集图组织技能和关系，在基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在数据密集型场景（如城市数据挖掘）中面临挑战，因为城市数据规模大、来源多且关系复杂。

Method: 构建统一数据集图，包含数据配置智能体（提取技能）、关系智能体（识别关系）、规划智能体（检索和生成计划）、执行智能体（处理和分析，共享中间结果）以及报告智能体（合成报告）。

Result: 在通用和城市基准测试中（包括UrbanDS-Bench），UrbanDS在数据密集型任务上持续优于现有数据科学智能体。

Conclusion: UrbanDS有效解决了数据密集型城市任务的挑战，并在实际城市应用中（如武汉东西湖区）展示了有效性。
...
### [21] [Do Latent Channels Actually Communicate? A Causal Audit of Latent Multi-Agent LLM](https://arxiv.org/abs/2607.26773)
*Huixiang Zhang,Mahzabeen Emu*

Main category: cs.AI

TL;DR: 本文提出了一种因果审计方法，用于解构大型语言模型中潜在通信对接收方的影响，通过控制消息替换揭示信息成分的独立贡献。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅基于最终任务表现，无法区分潜在通信中消息存在性、样例特定内容或独立代理信息的影响。

Method: 在发送者表示进入接收者的边界处实施受控消息替换，通过四种消息设置测量五种指标，包括编码信息、消息敏感度、样例内容价值及代理附加价值。

Result: 在GSM8K上，Qwen3-4B的整体性能效应被分解为-6.17%的共性效应和+5.17%的样例特定效应，且模型规模改变时效应方向反转；在MATH-500上，增益主要来自共性效应。

Conclusion: 聚合准确率无法揭示潜在消息如何影响接收者，应引入受控消息比较作为潜在通信的标准评估方法。
...
### [22] [Property-driven Causal Abstractions for Markov Decision Processes](https://arxiv.org/abs/2607.26787)
*Jule Schmidt,Maximilian Weininger,Clemens Dubslaff,David Parker,Nils Jansen*

Main category: cs.AI

TL;DR: 提出一种基于因果关系的MDP抽象技术，通过状态变量谓词间的因果联系构建精简模型，在保持原始MDP特征的同时支持近似最优策略计算。


<details>
  <summary>Details</summary>
Motivation: 解决因状态空间指数爆炸导致MDP推理困难的问题，现有抽象方法未能保留因果特性。

Method: 定义因子化MDP上的因果关系，基于状态谓词间的因果依赖关系识别具有相同属性满足/违反原因的状态，构建因果抽象模型（涵盖MDP、区间MDP和随机博弈）。

Result: 在多个标准基准测试中获得小型抽象模型，能计算原始MDP的近似最优策略，且抽象结果可泛化至相关大规模MDP模型。

Conclusion: 因果抽象技术可有效降低MDP复杂度，同时保证决策质量与泛化能力。
...
### [23] [From Passive Video to Editable Experience: Physically Grounded Experience Synthesis for Embodied Intelligence](https://arxiv.org/abs/2607.26903)
*Jia Luo*

Main category: cs.AI

TL;DR: 提出Pegasus框架，通过构建任务图、可供性图、约束图和机器人规划图，将人类演示视频中的知识迁移到机器人可学习的视频数据，解决具身AI中人体与机器人之间的具身鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 机器人无法直接利用海量在线人类操作视频进行学习，因为人体形态与机器人硬件存在具身鸿沟。现有方法依赖大量机器人数据收集，资源消耗高且难以扩展。

Method: Pegasus框架：从人类视频中提取任务图，通过可供性图和约束图转换为机器人规划图，用于机器人条件视频生成。使用分层可供性潜在空间建模对象状态、可供性与任务关系，结合闭环物理验证器过滤无效生成（运动学可行性、碰撞约束、关节限位）。

Result: 在GTEA Gaze+和EPIC-KITCHENS-100等基准测试及多种机器人平台上，Pegasus在任务正确性、可执行性、状态一致性和可学习性指标上均表现良好，实现了可靠的跨具身迁移。

Conclusion: 机器人数据生成可以从硬件收集问题重新定义为可扩展、低资源的的知识迁移问题，Pegasus框架有效弥合了具身鸿沟并降低了数据获取成本。
...
### [24] [What Does It Take to Detect an AI Agent? Minimal Feature Sets for Behavioral Detection under Browser Automation](https://arxiv.org/abs/2607.26935)
*Vishisht Choudhary,Lukas Schmidt,Anne Zoë Kenntner,Feras Skhab,Michel Osswald,Jens Ernstberger*

Main category: cs.AI

TL;DR: 提出三分类检测框架（人类、机器人、AI代理），揭示二元分类器将39.1%-34.5%的AI代理误分类为人类，引入代理类后达到100%召回率。


<details>
  <summary>Details</summary>
Motivation: 现有二元人类-机器人检测器无法处理AI代理这一新兴流量类别，导致结构性误分类。

Method: 三分类MLP/Transformer模型、五级逃避攻击梯度（被动观测到GAN轨迹重放）、9401个GBM特征子集穷举搜索。

Result: 双特征（mouse_event_rate, teleport_click_ratio）在全部逃避等级下实现100%代理召回率（精度0.994），五特征提升宏F1至0.991。

Conclusion: 区分信号是浏览器自动化产生的物理输入流缺失伪影，而非代理推理行为，且该信号冗余编码可抵抗轨迹操纵。
...
### [25] [Belief-Guided Decision Making with Uncertainty Gating in the Game of Go](https://arxiv.org/abs/2607.26946)
*Mehrad Yaghoubi,Azam Bastanfard,Abbas Jalilvand,Ashkan Rezaei*

Main category: cs.AI

TL;DR: 提出信念引导架构，通过分离策略头和信念头实现无需深度搜索的高水平围棋AI，降低对MCTS的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有围棋AI依赖MCTS进行错误修正，在消费级硬件上因计算成本高导致推理速度受限，且无深度搜索时策略模型易出现幻觉（高置信度但致命的失误）。

Method: 设计信念引导架构：将策略头与信念头解耦，信念头作为内部模拟器和独立评论家建模认知不确定性与战略稳定性；集成Transformer/GRU记忆机制处理长期依赖和打劫规则；使用门控机制过滤策略网络的过度自信错误。

Result: 实验表明该方法显著提升无搜索场景下的胜率，减少幻觉，在受限硬件上实现职业级表现而无需大规模MCTS。

Conclusion: 通过将智能从运行时搜索转移到参数化直觉，证明在资源受限环境中可达到专业水平的玩法。
...
### [26] [Setoka: A Benchmark for Hierarchical User Understanding in Personalized Agents over Heterogeneous Data](https://arxiv.org/abs/2607.27056)
...
TL;DR: 提出Setoka基准，从记忆检索、行为模式、人格特质等层次评估个性化agent对用户的理解，发现现有系统在高层理解上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有记忆基准仅关注显式事实检索，无法评估agent对用户深层特征（如行为模式、人格）的理解。

Method: 基于认知与人格心理学定义四层用户理解，并设计心理测量学驱动的合成数据生成和查询流水线。

Result: 现有系统在语义记忆检索上表现好，但在情景记忆上下降，行为模式和人格特质理解任务中表现更差。

Conclusion: 用户理解不能仅靠事实检索，需设计跨源整合和长程抽象的记忆机制。
...
### [27] [On-Policy Distillation for LLM Safety: A Routing Approach to Template-Robust Realignment](https://arxiv.org/abs/2607.27081)
*Yongjian Guo,Wanlun Ma,Lingyu Shen,Xi Xiao,Sheng Wen*

Main category: cs.AI

TL;DR: 本文提出路由式策略蒸馏（ROPD），通过建模对齐与受损模型输出概率分布的差异，解决微调导致的大型语言模型安全漏洞问题，在保持专业技能的同时抵御有害行为。


<details>
  <summary>Details</summary>
Motivation: 微调范式使恶意数据提供者能嵌入有害行为，导致模型在特定情况下违背人类价值观，现有关对齐方法存在灾难性遗忘、模板不匹配下失效、易被再次越狱等局限。

Method: 通过路由策略蒸馏，将对齐与受损模型的输出概率分布差异作为优化目标，而非拟合特定提示模板，实现鲁棒的模型重对齐。

Result: 在多个数据集和基础模型上，ROPD在防御模板不匹配时显著优于基线方法，保持高防御有效性和能力保留，性能下降可忽略。

Conclusion: ROPD为鲁棒的大语言模型重对齐建立了新标准，尽管对模板偏移并非完全免疫，但相比现有方法表现更优。
...
TL;DR: 提出了统一等价发现和包含发现的混合本体匹配（HOM）任务，并设计了基于大语言模型的多智能体框架AgentMap来同时发现等价和包含映射。


<details>
  <summary>Details</summary>
Motivation: 传统本体匹配仅能发现单一类型语义对应，无法同时发现等价和包含映射，限制了匹配的全面性。

Method: 采用基于大语言模型的多智能体框架，通过语义检索、层次搜索和协作式多智能体推理逐步探索目标本体，识别等价概念或最细粒度的包含概念。

Result: 在混合匹配、仅等价匹配和仅包含匹配三种设置下，AgentMap均取得了有竞争力的性能，并在后两种设置上超过了基线方法。

Conclusion: AgentMap能够有效统一等价和包含发现，为混合本体匹配提供了可行的解决方案。
...
### [29] [OmegaUse-OfficeVal: Benchmarking LLM Agents on Long-Horizon Office-Suite Tasks with Economic Grounding](https://arxiv.org/abs/2607.27155)
...
TL;DR: 本文提出OmegaUse-OfficeVal基准，用于评估LLM代理在办公套件任务中的性价比，发现尽管成本更低，但质量尚未达到人类水平。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估LLM代理在办公套件工作流中的成本和性能。

Method: 构建100个办公任务，附带人类劳动时间和任务价格代理两个经济信号，并开发基于代码的验证器。

Result: 所有评估的LLM成本更低、速度更快，但交付质量未达到人类水平。

Conclusion: LLM代理在办公套件任务中具有成本优势，但质量仍需提升。
...
### [30] [Linguistic Monoculture in LLM-Assisted Language Use](https://arxiv.org/abs/2607.27134)
*Suhas Thejaswi,Juhi Kulshreshta,Lutz Oettershagen*

Main category: cs.AI

TL;DR: 论文研究了大型语言模型（LLMs）在写作中可能导致的语言多样性减少问题，即“语言单一文化”，并分析了不同交互机制对语言多样性的影响。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs广泛用于写作和沟通，人们担心这可能导致语言多样性的丧失，即语言单一文化现象。

Method: 开发了一个数学框架，将作者和LLMs表示为语言特征的分布，并通过重复交互共同演化，分析了三种交互机制：固定共享模型、递归更新共享模型和个性化模型。

Result: 共享模型可导致作者趋向共同规范；递归反馈在不改变成对扩散下重新定位共享规范；个性化可维持一系列不同的作者-模型平衡，保持语言多样性。

Conclusion: 个体作者可能过度遵从社会规范，因为他们没有内化自身独特性对他人的价值，导致负外部性和单一文化成本，该成本在固定情况下有限，但当独特性主导真实性时可能无限增长。
...
TL;DR: 提出CE-CM和CE-CM-Div方法，通过贝叶斯推断能力向量并引入多样性规划器，实现在多任务、隐藏能力及人类次优行为下的高效协同。


<details>
  <summary>Details</summary>
Motivation: 现有临时团队协作（AHT）假设固定任务和已知能力，忽视真实世界中能力隐藏、人类次优行为及多任务场景。

Method: 将AHT扩展为多任务联合规划问题，采用近似贝叶斯方法CE-CM推断任务不变能力向量，通过仿真采样估计能力并构建上下文MDP；CE-CM-Div进一步通过多样性规划器评估能力假设。

Result: 模拟实验中CE-CM快速恢复隐藏能力、减少不可行动作分配并适应变化；人类研究中CE-CM-Div显著提升能力估计精度。
...
TL;DR: 本文提出了一种评估AI代理自动化AI研究能力的新方法——影子评估，即让代理尝试回答高质量未发表论文的核心研究问题，并由原论文作者评分。实验表明，前沿代理能完成所有工程任务，但在核心研究问题上进展有限，导致两篇论文被明确拒绝。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法要么局限于狭窄可验证任务，排除开放式研究；要么依赖盲审，但存在评分质量差等问题。需要一种更有效的方法来测量AI在开放式AI研究上的进展。

Method: 影子评估方法：让AI代理针对高质量未发表论文的核心研究问题，在给定时间和计算资源下进行独立研究，并由原论文作者对结果进行评分。

Result: 两个未发表的NeurIPS 2026投稿均被原作者明确拒绝。代理能完成所有工程任务，但无法在研究问题上取得实质进展，并出现了五种失败模式。

Conclusion: 当前AI代理能处理AI研究的工程部分，但在研究流程的关键环节（如判断研究价值、应对设计缺陷、回溯错误、资源意识、指令遵循）上仍存在明显不足。
...
TL;DR: 提出了KDA方法，利用大语言模型将动作标签分解为原子动作，通过知识注入和知识解耦模块增强视频特征，在多标签动作识别任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉复杂场景中的细粒度动作交互，缺乏显式语义指导，导致解耦不精确。

Method: 使用LLM将动作标签分解为原子动作；知识注入模块（KIM）整合原子动作知识到视频特征；知识解耦模块（KDM）进一步解耦知识形成精准语义指导；引入知识解耦损失（KD Loss）促进清晰解耦。

Result: 在多个多标签动作识别基准上达到最先进性能，且KIM和KDM可即插即用，具有良好的泛化性。

Conclusion: 细粒度语义知识能够增强动作表征和鉴别性，原子动作解耦有效提升复杂场景下的动作识别精度。
...
TL;DR: 本文探索了使用深度神经网络从野外单张人物图像中自动估算BMI、体重和身高的方法，并提出了一个包含6105张图像的新数据集。


<details>
  <summary>Details</summary>
Motivation: BMI是反映身心健康的重要指标，但现有研究中缺乏公开的野外全身图像数据集用于BMI估算。

Method: 采用单任务和多任务学习，结合RGB、深度图、姿态亲和图、边缘图等多种模态，使用VGG、Densenet、ResNet等CNN骨干网络进行实验。

Result: 全身体图像在BMI估算中效果优于半身图像和面部图像。

Conclusion: 全身体图像更适合野外场景下的BMI自动估算，新数据集为后续研究提供了基础。
...
TL;DR: TraceCLIP是一个无需额外训练的方法，通过从CLIP的CLS注意力输出中提取局部语义贡献，并利用语义-测地拓扑门来校准密集特征，从而在零样本语义分割等密集视觉-语言理解任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: CLIP的图像级对比目标主要对齐全局表示，对局部视觉-语言对应关系约束较弱，现有方法依赖额外监督或模型适应，而无需训练的方法尚未充分利用CLIP内部的可访问局部语义。

Method: 从CLS注意力输出中分离补丁级语义贡献项，将贡献响应转化为语义-测地拓扑门，以校准最终层的补丁亲和度用于密集特征重建。

Result: 在8个零样本语义分割基准上，平均mIoU提升1.3到4.5个百分点，超越最强的先前无训练方法，无需额外训练或区域级监督。

Conclusion: 全局对齐表示的内部构造中仍可访问空间局部化语义，TraceCLIP有效恢复了这些潜在的补丁级语义证据。
...
TL;DR: 提出DVPSFormer，一种统一在线架构，通过显式场景离散化（ESD）和在线多数投票（OMV）机制，实现高效的4D场景理解（深度、语义、实例轨迹），在Cityscapes-DVPS和SemKITTI-DVPS基准上达到新最优。


<details>
  <summary>Details</summary>
Motivation: 现有深度感知视频全景分割方法依赖计算昂贵的多阶段流水线或离线跟踪，不适合实时决策，需要高效统一的在线方案。

Method: 通过显式场景离散化（ESD）利用分割查询表示前景/背景区域，结合离散到连续（D2C）深度头单次解码度量深度；提出在线多数投票（OMV）机制利用时间一致性改进实例跟踪中的分类。

Result: 在Cityscapes-DVPS和SemKITTI-DVPS基准上达到新最优性能，实现流线型在线机器人感知。

Conclusion: DVPSFormer为在线4D场景理解提供高效、统一的解决方案，开源代码和模型促进实际应用。
...
TL;DR: 提出混合计算机视觉方法，用Mask R-CNN识别人体和颜色算法分割暴露皮肤，实现暴露面积比率量化并与人工评估80%一致。


<details>
  <summary>Details</summary>
Motivation: 为化学暴露评估提供自动化、可扩展的图像分析工具，替代传统人工目测法。

Method: 结合Mask R-CNN去除背景干扰，再通过颜色分割算法提取暴露皮肤区域，计算像素比率。

Result: 暴露皮肤-身体像素比率与人工评估约80%一致，验证了方法的准确性。

Conclusion: 该图像分析方法可扩展至身体部位识别、PPE检测及视频暴露分析，大幅提升暴露评估效率。
...
TL;DR: 提出了Rad-JEPA 3D框架，通过联合嵌入预测和混合H-Mamba编码器，在120,000次CT扫描上预训练，以紧凑参数实现了3D医学图像分析的最新性能。


<details>
  <summary>Details</summary>
Motivation: 现有体积编码器难以保留粗粒度的空间和几何结构，限制了在器官分离、异常检测及与语言模型结合时的空间推理能力。

Method: 采用联合嵌入预测框架，核心是混合H-Mamba编码器（融合Mamba状态空间分支和分组查询注意力分支），并引入隐藏状态正交正则化（HSOR）以减少特征冗余。

Result: 仅4.0B参数，在封闭式VQA上取得有竞争力结果，并在Spatial-Med基准上获得最佳平均空间推理分数。

Conclusion: 混合块和HSOR贡献互补增益，诱导的空间结构可替代原始语言模型规模进行体积推理任务。
...
TL;DR: 提出WildShadowRemover框架，结合预训练视频扩散模型与LoRA微调，并引入细节注入与频率分解调制模块，在自制大规模数据集上实现高质量视频阴影去除与强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有阴影去除方法在真实场景中受限于复杂光照、多样阴影形态及训练数据不足，视频阴影去除在非约束环境下鲜有探索。

Method: 使用LoRA微调预训练视频扩散模型，冻结VAE解码器并添加细节注入模块，通过阴影掩膜引导的频率分解调制模块选择性恢复纹理，结合Depth Anything 3的单目深度先验提供几何感知。

Result: 在自制WildShadow数据集及真实场景实验中，本方法在阴影去除质量与时间一致性上优于现有技术，生成清晰无阴影视频且泛化性强。

Conclusion: WildShadowRemover能有效处理野生视频阴影去除挑战，实现鲁棒且时序连贯的结果，相关数据集和基准可推动后续研究。
...
TL;DR: 提出联邦物理信息神经网络，在保护隐私的同时达到91.4%的脑肿瘤诊断准确率并产生合理的位移场。


<details>
  <summary>Details</summary>
Motivation: 静态成像无法捕捉脑肿瘤引起的力学变化，且集中数据训练违反隐私法规，限制了模型的泛化能力。

Method: 结合联邦学习与基于线性弹性方程的物理信息损失，在模拟的三家医院站点上仅共享模型权重，通过FedAvg协议进行100轮训练。

Result: 联邦模型总体准确率91.4%，非联邦基线90.0%；平均AUC 0.985；垂体肿瘤准确率从85.6%提升至94.5%；生成平滑、无散度的位移场。

Conclusion: 联邦训练可与物理约束结合且性能损失很小，能保护隐私并保持精度。
...
### [41] [Lightweight Image Classification of Raptor Species for Edge Devices: Rare-Species Dataset Expansion via Video Frame Extraction, Knowledge Distillation, and TensorRT Deployment](https://arxiv.org/abs/2607.26238)
*Takeshi Nishikawa*

Main category: cs.CV

TL;DR: 提出了一种轻量级猛禽分类方法，通过知识蒸馏将大模型压缩为三个轻量学生模型，并在风电机碰撞缓解的边缘部署中实现高精度和实时性。


<details>
  <summary>Details</summary>
Motivation: 实时边缘部署中需要轻量模型，同时减少猛禽间的误分类（如白尾海雕与虎头海雕的混淆）以提升保护效果。

Method: 以DINOv2-L（3.04亿参数）为教师模型，蒸馏MobileNetV4、ViT-Small、EfficientNet-B0三个轻量学生模型；通过视频帧提取将数据集扩展至12,519张图像（虎头海雕从463张增至2,050张）；采用分组拆分避免数据泄露；集成三个学生模型进行推理。

Result: 三个学生集成的宏召回率达0.935±0.004（图像级分割为0.955），参数仅为教师的1/8；白尾海雕召回率最高提升38.6%，误识别为虎头海雕的比率从61%降至15%；EfficientNet-B0在Jetson Orin Nano上达313张/秒，FP16与FP32的argmax一致性达99.95%。

Conclusion: 知识蒸馏和教师模型升级未显著提升性能，主要改进来自数据集扩展和教师微调；轻量模型在边缘设备上可实现高效实时分类。
...
### [42] [Lag-aware cross-hand alignment for dual-hand action segmentation](https://arxiv.org/abs/2607.26215)
*Fatemeh Ziaeetabar*

Main category: cs.CV

TL;DR: 提出了一种轻量级模块LACA，通过显式估计双手特征流之间的定向时间偏移分布，提升双手动作分割性能。


<details>
  <summary>Details</summary>
Motivation: 传统双手动作分割假设左右手特征在时间上严格对齐，但实际手部转换存在非零且时变的延迟，导致性能受限。

Method: 设计Lag-Aware Cross-Hand Alignment模块，估计双手特征流的时间偏移分布，利用偏移信息检索跨手特征，并引入可学习的空状态抑制不兼容转换。

Result: 在HA-ViD和ATTACH数据集上，LACA使双手平均F1@50分别提升2.1和1.9，边界F1提升3.1和3.2，仅增加约0.0086M参数。未来无关版本LACA-C实现83.6%转换线索召回率、233ms中位延迟。

Conclusion: 显式跨手时间对齐能同时改善动作分割和边界定位，并支撑及时的未来无关感知。
...
### [43] [BG-REAL: A Public Real-Data Anchored Benchmark for Background Manipulation Detection and Localization](https://arxiv.org/abs/2607.26232)
*Bugra Alperen Uluirmak,Rifat Kurban*

Main category: cs.CV

TL;DR: 提出了BG-REAL基准，用于检测和定位背景操作，包含7000个样本和6种编辑类型，通过匹配真实控制诊断发现重编码伪影是多个基线模型的共同捷径风险。


<details>
  <summary>Details</summary>
Motivation: 背景操作是实际但未充分研究的图像取证场景，现有基准多聚焦于前景对象操作或通用合成编辑，缺乏专门针对背景操作的评估资源。

Method: 从Open Images V7实例分割源构建BG-REAL基准，包含7000个样本（6000个真实数据锚定样本和1000个合成控制样本），覆盖6种编辑类型，并采用匹配真实控制诊断评估基线模型。

Result: 在固定阈值下，基线模型对重编码真实图像的误报率范围为0.57（TruFor最低）到1.00（若干弱基线或掩码信息基线），表明重编码伪影是共享的捷径风险。

Conclusion: BG-REAL是背景操作取证的专门基准，可作为通用图像操作定位基准的补充，而非完全真实或通用基准。
...
### [44] [Spline-Based Boundary Representations for Sparse View Reconstruction and Simulation Using Isogeometric Analysis](https://arxiv.org/abs/2607.26234)
*Davor Dobrota,Vsevolod Skorokhodov,Chenghao Xu,Olga Fink,Malcolm Mielle*

Main category: cs.CV

TL;DR: 提出了FORGE-SIM方法，从稀疏有姿态的RGB图像直接重建多片B样条边界表示，生成紧凑、光滑、水密的几何体，并可将观测场投影到样条基上，实现与CAD和仿真工作流的原生兼容。


<details>
  <summary>Details</summary>
Motivation: 基于图像的重建方法恢复的几何模型通常缺乏数值仿真所需的显式、水密、光滑特性，阻碍了其在仿真驱动设计中的应用。

Method: 通过优化多片B样条表示直接从稀疏RGB图像重建，并引入策略将观测场（如热状态、语义信息）投影到相同样条基上。

Result: 重建模型质量足以支持热仿真和模态分析，实现了计算机视觉与数值分析的统一。

Conclusion: 该工作消除了图像重建与仿真就绪建模之间的长期障碍，为仿真驱动设计、检测和数字孪生应用提供了新工作流。
...
### [45] [LumaGuide: Distribution Shaping for Training-Free HDR Generation in Diffusion Models](https://arxiv.org/abs/2607.26237)
*Bowen Chen,Shreshth Saini,Balu Adsumilli,Alan C. Bovik*

Main category: cs.CV

TL;DR: LumaGuide是无需训练的框架，通过能量引导调整扩散模型采样过程，实现高动态范围（HDR）内容生成。


<details>
  <summary>Details</summary>
Motivation: 预训练扩散模型受限于训练数据的统计偏差，无法生成高动态范围内容。

Method: 在不同感知均匀PQ空间中通过能量引导控制亮度分布，对齐直方图。

Result: 生成HDR一致图像（高光细节、阴影保留），保持语义保真度。

Conclusion: 无需重训练即可通过采样时调整输出分布实现可控生成。
...
TL;DR: 使用CT基础模型提取特征预测头颈癌远处转移风险，AUC达0.791，优于传统影像组学和深度学习特征，且无需专业分割标注。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法依赖肿瘤分割等专业知识，耗时且存在主观差异；基础模型可简化流程并提升可扩展性。

Method: 基于RADCURE数据集2327例术前CT，分别提取影像组学、深度学习及CT基础模型特征，输入MLP预测远处转移风险。

Result: CT基础模型AUC 0.791，优于影像组学(0.772)和深度学习(0.753)，与特征组合模型(0.794)性能接近。

Conclusion: 基础模型特征可替代传统影像组学，减少领域专家依赖与标注需求，更具可及性和可扩展性。
...
TL;DR: 提出HeteroPROMPT框架，通过轻量级提示（prompt）对齐异构车辆的特征空间，实现高效、隐私保护的协同感知。


<details>
  <summary>Details</summary>
Motivation: 现有异构协同感知方法需重训练或访问元数据，扩展性差且存在隐私问题。

Method: 基于视觉提示的BEV特征调制模块 + 无元数据的自编码器模态分类与路由。

Result: 在OPV2V-H和V2XSet数据集上，以极少可训练参数实现SOTA性能，模态分类准确率>99.99%。

Conclusion: HeteroPROMPT提供可扩展、实用的异构CP解决方案，兼顾实时性与隐私保护。
...
TL;DR: 提出了一个名为Eddeep的深度学习框架，用于快速校正扩散MRI中的涡流畸变，其效果与FSL Eddy相当但推理时间大幅缩短。


<details>
  <summary>Details</summary>
Motivation: 现有的涡流畸变校正方法（如FSL Eddy）虽效果优异但计算成本高，难以满足大规模研究及临床快速处理的需求。

Method: 将问题分解为两个阶段：先通过监督图像翻译网络统一扩散加权与b=0图像的对比度，再使用非监督配准网络在物理约束的二次畸变模型下联合估计涡流畸变及头动参数。

Result: 在UK Biobank（域内）和Memodyn（域外）数据集上，Eddeep在校正质量（如抖动、DKI残差、信号不规则性和互信息）上与FSL Eddy相当，且推理时间显著减少。

Conclusion: 深度学习可在无需迭代优化的情况下实现精确且高效的涡流畸变校正，有望加速大规模研究与临床部署中的扩散MRI处理流程。
...
TL;DR: 提出MoSAIC框架，通过区域分解特征和对齐干预监督实现局部参考条件的人物动作风格迁移，在冻结评估中改善了动作编辑的选择性与保留性权衡。


<details>
  <summary>Details</summary>
Motivation: 现有运动数据集缺乏任意局部内容-参考组合的配对目标，且自重构训练可能导致扩散模型忽略路由参考而复制内容运动。

Method: 基于潜在扩散框架，分解内容与参考特征为解剖区域，通过独立路径保留根轨迹，利用对齐干预监督构建同步参考和反事实目标（通过局部变换生成可观测的对应响应与保留运动）。

Result: 在128个动作和896个路由条件中，局部掩码路由将保留区域误差从70.64降至66.45mm，噪声泄漏从18.08降至9.88mm；维持对齐干预监督使目标响应提升8.8%，路由影响集中度提高2个百分点。

Conclusion: MoSAIC有效改善了选择性局部运动编辑中的响应-保留权衡，实现了可控制的部分身体运动编辑。
...
TL;DR: 多模态大语言模型(MLLMs)在视觉证据与语言先验冲突时常失败。通过两种范式分析发现：MLLMs能编码粗粒度视觉证据但无法可靠控制对其依赖。


<details>
  <summary>Details</summary>
Motivation: 探索MLLMs在视觉中心任务中失败的原因，特别是当视觉证据与预训练知识冲突时。

Method: 使用两种诊断范式：(1)通过图像重建探测视觉信息可用性；(2)通过WhatIfVis基准测量多模态上下文敏感性。

Result: 粗粒度视觉证据可保留；无监督模型视觉敏感性不稳定，SFT可改善可控性；视觉-先验权衡沿学习向量可控。

Conclusion: 瓶颈在于MLLMs能编码视觉证据但无法可靠控制对其依赖，而非感知阶段编码退化。
...
TL;DR: 提出Zero-Fi框架，通过对比信号-语言对齐实现Wi-Fi人体活动的零样本识别，无需标注新类样本。


<details>
  <summary>Details</summary>
Motivation: 现有封闭集活动识别方法无法处理未见过的活动类别。

Method: 利用互补Wi-Fi信号特征与自然语言描述语义之间的跨模态对齐。

Result: 在公开数据集上对未见过活动类别实现有效零样本识别。

Conclusion: 信号-语言对齐可扩展Wi-Fi感知到预定义类别之外。
...
TL;DR: 提出一种可靠性感知的复域融合框架，用于解决白光成像与窄带成像在配准困难下的病灶分割问题。


<details>
  <summary>Details</summary>
Motivation: WLI和NBI在空间上难以配准，直接融合会导致病变边界分割性能下降。

Method: 构建拓扑正则化的特征对应关系，估计跨模态可靠性，并在可学习的复域表示中选择性融合WLI（幅度）和NBI（相位）特征。

Result: 在配对WLI/NBI内镜数据集上，该方法稳定提升病变分割性能，消融实验验证了模态角色设计和可靠性引导融合的必要性。

Conclusion: 显式建模不同模态角色并抑制不可靠跨模态交互，能有效提升未配准多模态医学图像分割的效果。
...
TL;DR: 提出了跨分支语义引导框架，揭示了统一多模态模型中理解与生成分支的语义空间存在非对称对齐，理解分支的语义向量可有效指导生成，反之则效果有限。


<details>
  <summary>Details</summary>
Motivation: 探究统一多模态模型中理解与生成能力是否共享可迁移的语义空间，以及两个异构表示分支间的语义对齐程度。

Method: 跨分支语义引导框架：从理解分支提取语义方向向量并应用于生成分支，通过双向迁移实验分析语义对齐性与不对称性。

Result: 理解分支的引导向量可迁移至生成分支，实现可控图像生成并提升语义保真度；反向迁移效果持续有限。

Conclusion: 架构统一无法保证语义对齐，理解分支编码可迁移的对象中心语义，生成分支主要编码低级外观特征，跨分支引导可作为探测多模态表示的有效工具。
...
TL;DR: 提出TIDE双分支弹性追踪框架，通过自适应几何对应交并比替代外观特征，在边缘设备上实现高效密集多目标追踪。


<details>
  <summary>Details</summary>
Motivation: 现有密集目标追踪方法（如SU-T）计算开销大，难以在边缘设备部署，且在严重遮挡时外观特征失效。

Method: 采用双分支弹性结构：轻量L分支利用自适应几何对应交并比关联机制，可扩展S分支平衡精度与效率，均无需复杂重识别网络。

Result: L分支以20.47G FLOPs达到28.43 HOTA，计算量较SU-T降低38.7倍；S分支实现29.98 HOTA，接近云端精度。

Conclusion: TIDE有效弥合了云端高精度分析与边缘高效追踪的差距，为密集同质目标追踪提供切实可行的边缘部署方案。
...
TL;DR: 提出FAS-R1，一个两阶段推理导向的多模态大语言模型框架，用于统一的人脸防欺骗，包含真实性分类、攻击类型识别和欺骗区域定位。


<details>
  <summary>Details</summary>
Motivation: 现有判别式FAS模型主要依赖标签，而基于MLLM的方法依赖监督微调，输出模板化推理且对困难攻击优化不足。

Method: 先使用高质量长链推理数据集FAS-R1-23K进行冷启动监督微调，然后进行FAS特定的GRPO后训练；提出退化模拟增强和难度感知GRPO分别增强推理稳定性和抑制易样本主导。

Result: 3B模型在域内达到98.75%真实性准确率、93.33%攻击类型准确率及96.30/94.73% AP@40/AP@50，并在跨域泛化和推理质量上优于对比系统。

Conclusion: 所提方法有效提升了FAS的统一预测性能，并展现良好的扩展性。
...
TL;DR: 基于EfficientNet-B0迁移学习的四类痤疮严重度分类器，在ACNE04数据集上达到93.5%准确率和94.4%宏F1分数，并提供开源实现。


<details>
  <summary>Details</summary>
Motivation: 手动痤疮严重度分级存在评估者间变异和成像条件不一致的问题，需要自动化、准确的分类方法。

Method: 使用ImageNet预训练的EfficientNet-B0进行迁移学习，采用AdamW优化、几何/光度增强和验证宏F1的检查点选择。

Result: 测试集上准确率93.5%，宏F1为94.4%，相邻等级间错误占83%，Cohen's kappa为0.956。

Conclusion: 轻量级迁移学习可提供准确、平衡且可解释的痤疮严重度分级，支持跨平台临床验证。
...
TL;DR: 提出了HERMES算法，一个统一的容器化解决方案，用于HECKTOR 2026挑战的三个子任务：肿瘤分割、T/N分期和复发风险评估，在验证集上取得了加权得分0.6454。


<details>
  <summary>Details</summary>
Motivation: 针对头颈癌治疗中肿瘤分割、分期和预后评估的集成需求，开发一个端到端的自动化算法以提升准确性和临床适用性。

Method: 使用10折STU-Net Small网络集成进行分割；从预测掩膜中提取与AJCC/UICC标准对齐的紧凑几何特征用于T/N分期；结合深度学习和临床风险专家的等权重集成进行预后评估，并设计了近似一致性指数的生存损失函数。

Result: 内部交叉验证中，N分期平衡准确率从0.691提升至0.720；验证集上平均Dice为0.641，T分期平衡准确率0.580，N分期平衡准确率0.642，RFS C指数0.679。

Conclusion: HERMES算法在HECKTOR 2026验证集上表现良好，成功获得测试阶段资格，验证了几何特征和双专家集成在分期和预后中的有效性。
...
TL;DR: 提出SATATrack，一种用于无人机反无人机跟踪的语义感知时域自适应框架，通过语言描述引导的上下文传播和在线特征分布对齐，在双动态场景下实现鲁棒跟踪。


<details>
  <summary>Details</summary>
Motivation: 无人机反无人机跟踪中，相机平台和目标同时运动（双动态）导致快速视角变化、运动模糊、尺度变化和视觉干扰，固定视觉表征下目标外观不可靠、特征分布偏移，而语言描述具有帧间稳定性。

Method: 采用语义感知上下文传播（SACP）用目标描述引导时域上下文传播，训练时引入辅助对比正则化抑制语义相似背景；推理时用时域感知分布对齐（TADA）在线调整特征分布。

Result: 在UAV-Anti-UAV基准上达到最优性能，并在Anti-UAV和UAV目标跟踪任务中展现竞争力。

Conclusion: 语言描述作为语义锚点结合在线分布对齐，能有效应对双动态场景下的外观变化和分布偏移，提升跟踪鲁棒性。
...
### [59] [HumanCLAW: Can Vision-Language Models Act Through a Body?](https://arxiv.org/abs/2607.27180)
...
TL;DR: 提出HumanCLAW框架，解耦视觉语言模型的动作决策与低级运动执行，构建包含1218个长时程任务的基准测试，发现现有9个先进模型最高成功率仅16.8%，主要瓶颈在于缺乏具身自我意识。


<details>
  <summary>Details</summary>
Motivation: 现有评估无法区分任务失败源于VLM决策错误还是运动控制失败，需要解耦两者的评估方法。

Method: 设计HumanCLAW框架：VLM输出原子技能指令，由硬编码控制器转换成亚秒级连续全身运动，排除平衡和电机误差，仅测量模型的动作智能。

Result: 在41个室内场景的1218个找物-导航-交互任务中，9个SOTA VLM模型最高成功率仅16.8%。

Conclusion: 当前VLM缺乏具身自我意识，无法感知自身位置、是否到达目标或遇到障碍，这是超越目标识别的主要瓶颈。
...
### [60] [EgoSafe: A First-Person Mobile-Captured Benchmark for Visual Safety Understanding](https://arxiv.org/abs/2607.26518)
*Yuyun Chen,Tianao Li,TianQuan Feng,Cen Chen,Huiping Zhuang,Hao Peng,Ziqian Zeng*

Main category: cs.CV

TL;DR: 提出了EgoSafe-Bench基准，通过分层推理评估协议评估大视觉语言模型在自我中心安全场景中的因果推理能力，发现模型存在感知与推理脱节的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准依赖第三人称监控视频和二元分类指标，无法暴露模型在自我中心动态场景中区分表面关联与真实逻辑的认知缺陷。

Method: 构建包含12000个评估样本的基准，采用层次推理评估协议，强制模型经历特征锚定、盲点推导到意图推断的严格推理轨迹。

Result: 主流LVLM（如Qwen3-VL、Gemini、VideoLLaMA 3）描述评分较高，但因果推理和逻辑闭环能力显著脆弱。

Conclusion: 该基准和评估框架有助于推动逻辑鲁棒视频理解系统的发展。
...
### [61] [VidMap: Exploiting Temporal Structure for Video-Based Structure-from-Motion](https://arxiv.org/abs/2607.27194)
*Zador Pataki,Paul-Edouard Sarlin,Marc Pollefeys*

Main category: cs.CV

TL;DR: 提出了一种结合SLAM时序约束与SfM全局优化的系统，用于从任意长视频中恢复未标定相机的度量位姿和标定。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，SLAM对初始化和瞬态错误敏感，SfM缺乏对视觉对称和极端运动的鲁棒性，需要一种融合两者优势的方法。

Method: 利用宽基线密集图像匹配，将时序顺序作为闭环检测的核心，并引入度量单目深度先验辅助全局优化。

Result: 在多种挑战性数据集上，该方法比现有SLAM和SfM方法（无论经典或学习型）更鲁棒和精确，无论是否已知相机标定。

Conclusion: 结合SLAM的强时序约束与SfM的灵活性，能有效提升未标定长视频的度量重建性能。
...
### [62] [CineWeaver: Training-Free Reference-Controllable Multi-Shot Long Video Generation for Cinematic Storytelling](https://arxiv.org/abs/2607.26529)
*Yuyang Huang,Yabo Chen,Wenrui Dai,Ziyang Zheng,Haibin Huang,Chi Zhang,Junni Zou,Hongkai Xiong,Xuelong Li*

Main category: cs.CV

TL;DR: 提出了CineWeaver框架，无需训练即可实现参考可控的多镜头长视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖定制和重新训练，无法统一满足长时、多镜头、可控生成的需求。

Method: 通过操作位置编码和注意力模式打破时序连续性，结合镜头路由参考控制和锚点记忆机制。

Result: 生成高质量电影级长视频，具有一致的identity、稳定的全局外观和清晰的镜头切换。

Conclusion: CineWeaver是首个无训练统一框架，同时实现长时、参考可控和多镜头视频生成。
...
### [63] [TurboVLA: Real-Time Vision-Language-Action Model at 32 Hz on an RTX 4090 with <1 GB VRAM](https://arxiv.org/abs/2607.27205)
*Hengyi Xie,Chenfei Yao,Xianjin Wu,Xuanyang Xi,Yiping Tang,Di Xu,Yingying Zhu,Dingkang Liang,Xiang Bai,Han Ding*

Main category: cs.CV

TL;DR: 提出了TurboVLA，一种替代LLM中心化的VLA范式，通过直接的视觉-语言交互和紧凑的解码器，在LIBERO上以0.2B参数、31.2ms延迟和0.9GB显存达到97.7%的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型采用LLM中心的V→L→A路径，导致每次策略调用时计算和内存开销大，不适合高效机器人操作。

Method: 将传统V→L→A路径重构为直接的V+L→A映射，独立编码视觉和语言，通过轻量双向交互交换信息，并用紧凑解码器预测连续动作块。

Result: 在LIBERO上，TurboVLA以0.2B参数、31.2ms推理延迟和0.9GB显存（RTX 4090）达到97.7%平均成功率，匹配或超越更大规模VLA策略。

Conclusion: TurboVLA作为一种简单有效的替代LLM中心化VLA的范式，展示了高效连接视觉、语言和动作的新视角。
...
### [64] [TPCD: Tone-Pressure Contrastive Decoding and the Label-Free Gating Bottleneck in Vision-Language Models](https://arxiv.org/abs/2607.26536)
*Jinkun Zhao,Kui Zhang,Wenjun Wu*

Main category: cs.CV

TL;DR: 本文提出基于语调压力的对比解码（TPCD）方法，通过减去高压指令下的logits来缓解视觉语言模型的过度承诺问题，并结合门控机制平衡准确性与安全性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在高压提示下会做出不可靠承诺（如读取模糊文本、确认不存在物体），现有方法缺乏对压力诱导分布的利用。

Method: 从高压指令和中性指令的logits差异进行对比解码；引入基准特定的任务先验/不一致门控和类别无关的回答不一致路由器来过滤输出。

Result: 在LLaVA-1.5-7B上，安全中性化将攻击成功率从66.75%降至9.88%；对比解码进一步降至0.50%但正例准确率仅15.56%；门控机制在保持54.44%正例准确率的同时将攻击成功率降至1.63%。在GLM-4.6V和Llama-3.2-Vision上的结果表明简单门控优于安全中性化。

Conclusion: 压力是可用的承诺偏差探针和缓解信号，但当前门控机制尚不能作为独立有效的接地感知检测器。
...
### [65] [From Spatial Semantics to Temporal Context: Leveraging Gaze Trajectory for Weakly Supervised Medical Image Segmentation](https://arxiv.org/abs/2607.26542)
*Shaoxuan Wu,Xiao Zhang,Xiaodi Zhao,Yunzhi Tian,Yilin Tang,Jun Feng*

Main category: cs.CV

TL;DR: 提出TrailNet，利用眼动追踪中的注视点和轨迹进行弱监督医学图像分割，结合时空编码与不确定性解码。


<details>
  <summary>Details</summary>
Motivation: 像素级标注成本高，眼动追踪是低成本替代方案，但时间轨迹建模困难且注视噪声影响性能。

Method: 轨迹引导时空编码器融合图像语义与时间上下文；多尺度不确定性解码器利用类别互斥性抑制噪声；循环蒸馏实现无眼动推理。

Result: 在公共数据集上Dice达81.25%和81.85%，优于现有方法。

Conclusion: TrailNet有效利用眼动数据提升分割精度，并支持无眼动推断。
...
### [66] [MedARC: Training-Free Adaptive Redundancy Compression of Visual Tokens for 3D Medical Vision-Language Models](https://arxiv.org/abs/2607.26554)
*Yitao Zhu,Mengjun Liu,Yingji Fu,Haowen Pang,Anqi Qiu*

Main category: cs.CV

TL;DR: 提出一个无需训练的3D医学图像视觉标记自适应冗余压缩框架MedARC，通过三种互补线索估计标记重要性，保留临床相关信息。


<details>
  <summary>Details</summary>
Motivation: 3D医学图像产生过长的视觉标记序列，存在大量空间和切间冗余，现有压缩方法常去除临床相关区域。

Method: 结合视觉编码器自注意力、查询相关文本相似度、局部特征偏离中心程度三线索，进行重要性引导的合并压缩。

Result: 在CT-RATE和MR-RATE数据集上降低视觉标记开销和推理时间，保持或提升诊断性能。

Conclusion: 多线索冗余压缩有效且高效，对大语言模型更有利。
...
### [67] [Representation Trajectories Matters: Complementary Evidence for OOD Detection and Image Classification](https://arxiv.org/abs/2607.26565)
*Ignacio M. De la Jara,Cristian Rodriguez-Opazo,Hamed Damirchi,Stephen Gould,Damith Ranasinghe*

Main category: cs.CV

TL;DR: 本文研究了视觉模型计算路径中的中间状态变换，发现这些路径包含最终表示丢弃的证据，可用于改进异常检测和图像分类。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅将中间层视为独立快照，忽略了深度方向上的转换信息，本文旨在探索这些计算路径是否包含有用的可靠性信号。

Method: 通过分离类别相干传输与输入特定创新、协调移动与关系重组，分析监督、自监督、视觉-语言等编码器的计算路径，并设计转换惊喜分数和更新探针。

Result: 转换惊喜分数在131/152个非饱和比较中降低了FPR95，尤其适用于视觉干扰和语义远偏移；更新探针改进了71/72个干净模型-数据集案例，偏移数据增益依赖于架构和损坏类型。

Conclusion: 计算路径提供了联合由模型组织和偏移类型决定的通用可靠性信号，对OOD检测和分类有实际价值。
...
### [68] [3DGBGS: 3D Granular Ball Gaussian Splatting for Compact Novel View Synthesis](https://arxiv.org/abs/2607.26578)
*Meng Yang,Shuyin Xia,Dawei Dai,YiWang*

Main category: cs.CV

TL;DR: 提出3DGBGS框架，通过自适应3D颗粒球划分减少锚点数量并保持渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有锚点方法固定体素化导致锚点数量与渲染质量的权衡，需适应非均匀点分布。

Method: 将SfM点云自适应划分3D颗粒球，用GBAI初始化锚点，用GBSP提供尺度先验。

Result: 初始和最终锚点分别减少37.1%和10.0%，模型存储减少9.8%，渲染质量相当。

Conclusion: 3DGBGS在保持高质量渲染的同时显著提高锚点效率和模型紧凑性。
...
### [69] [Classification of Disease from Lungs X-ray Images using VGG16, VGG19 and ResNet50 Models](https://arxiv.org/abs/2607.26580)
*Nand Lal Yadav,Rajesh Kumar,Satyendra Singh,Sudhakar Singh*

Main category: cs.CV

TL;DR: 研究了VGG16、VGG19和ResNet50三种深度学习模型在基于X光图像分类肺部疾病（肺炎、结核、肺癌、正常）中的表现。


<details>
  <summary>Details</summary>
Motivation: 呼吸系统疾病病例增加，急需早期准确诊断，而CNN在影像诊断中表现良好。

Method: 使用VGG16、VGG19、ResNet50模型，在大规模X光图像数据集上训练并评估分类性能。

Result: 三种模型均表现良好，但ResNet50在效率和准确率上最优。

Conclusion: 这些深度学习模型有望成功应用于肺部疾病诊断，促进早期检测并改善患者预后。
...
### [70] [Level, Sharpness, and Corpus: Why Zero-Shot OOD Detector Rankings Do Not Transfer](https://arxiv.org/abs/2607.26582)
*Ignacio M. De la Jara,Cristian Rodriguez-Opazo,Stephen Gould,Damith Ranasinghe*

Main category: cs.CV

TL;DR: 通过对17个域内数据集、3个视觉语言模型和7个零样本OOD检测器的可控迁移性审计，发现检测器排名在不同部署中完全逆转。为此提出互补证据防护（CEG），通过非补偿性融合基础检测器、水平和锐度特征，无需OOD样本即可将GL-MCM的FPR95从38.1降至28.8，MCM从42.6降至30.5。


<details>
  <summary>Details</summary>
Motivation: 现有实践依赖基准排名选择零样本OOD检测器，但隐含的跨域迁移假设不成立。需要通过系统审计揭示排名逆转现象并解释其根本原因。

Method: 控制性迁移性审计（17个域内数据集+3个VLM+7个检测器）；提出CEG：基于域内百分位数的非补偿性融合（基础检测器、绝对匹配水平、相对/空间锐度），无需OOD样本或学习。

Result: 检测器排名在部署间完全逆转，每个检测器至少在一个域中FPR95>80%。CEG将GL-MCM的FPR95从38.1降至28.8，MCM从42.6降至30.5（家族平衡指标）。

Conclusion: 零样本OOD检测器无法可靠跨域迁移源于互补证据通道（水平与锐度）的不可互换性。CEG通过保存互补证据有效降低检测器对部署变化的敏感性。
...
### [71] [R-SLPR: Region-based Small-to-Large Point-cloud Registration with Contrastive Learning](https://arxiv.org/abs/2607.26583)
*Yusen Wan,Zeyuan Chen,Qianshi Zou,Xu Chen*

Main category: cs.CV

TL;DR: 提出了R-SLPR框架，一种针对尺度不匹配的小到大点云配准的三阶段方法，显著提升了配准精度。


<details>
  <summary>Details</summary>
Motivation: 传统配准算法在处理小部分、不完整或模糊几何线索的点云与大规模参考点云配准时效果不佳，现有学习方法也未能有效解决尺度不匹配问题。

Method: 三阶段架构：区域提议、区域匹配和迭代细化。包括斐波那契网格分割和对比学习生成局部几何补丁，以及级联锚点选择与细化算法进行迭代对齐。

Result: 在ModelNet40数据集上，位置平均绝对误差（MAE）降至0.009，旋转MAE降至1.104，达到新最先进水平。

Conclusion: R-SLPR有效解决了尺度不匹配的点云配准问题，显著提高了配准精度。
...
### [72] [SpatialQ: Understanding 3D Gaussian Splatting Scene Quality via Visual-based MLLM](https://arxiv.org/abs/2607.26595)
*Jingxuan Su,Shenglin Wang,Tiesong Zhao,Ge Li,Wei Gao*

Main category: cs.CV

TL;DR: 提出了一种针对3D高斯泼溅场景的多模态质量评估框架，通过3D感知表示学习与多模态推理，评估渲染视图质量及场景级一致性。


<details>
  <summary>Details</summary>
Motivation: 现有图像质量评估方法依赖2D感知线索，而通用多模态大模型缺乏稳定质量回归能力，无法满足3DGS场景对空间结构和跨视图一致性的评估需求。

Method: 使用VGGT编码器加质量头进行3D感知特征学习，多视图特征聚合捕获跨视图一致性，联合深度和点云信息建模几何线索；集成原始图像、深度图、点云渲染和相机参数到Qwen多模态模型中进行推理。

Result: 实现了结构感知的质量表示，能够同时评估渲染视图的感知保真度和场景级结构一致性。

Conclusion: 该方法为3DGS场景提供了更全面的质量评估，超越了传统2D IQA和通用MLLM的局限性。
...
### [73] [Decoupled Visual Processing: Efficient Multimodal Adaptation via Modality-Specific Transformer Substitution](https://arxiv.org/abs/2607.26596)
*Mingkuan Feng,Zhengqi Wen,Jianhua Tao*

Main category: cs.CV

TL;DR: 提出解耦视觉处理（DVP）框架，通过轻量级单Transformer块独立处理视觉令牌，大幅减少参数量且保持竞争性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型微调所有参数计算成本高，且深层网络中视觉与文本令牌表征需求显著不同。

Method: 在解码器前半共享处理后，将视觉令牌路由至新初始化的单Transformer块，文本令牌继续通过原冻结层，仅更新单块参数。

Result: 在LLaVA-1.5框架上，DVP在MME、POPE、ChartQA基准上达到竞争性能，同时仅训练极少参数。

Conclusion: 多模态大语言模型中的视觉表征可通过解耦的参数高效路径有效学习。
...
### [74] [JEPADepth: Masked Predictive Representation Learning for Self-Supervised Monocular Depth Estimation](https://arxiv.org/abs/2607.26600)
*Ionuţ Grigore,Călin-Adrian Popa*

Main category: cs.CV

TL;DR: 提出JEPADepth，一种结合I-JEPA掩码预测损失和光度损失的自监督单目深度估计方法，提升性能且不增加推理成本。


<details>
  <summary>Details</summary>
Motivation: 标准光度重建损失耦合深度、位姿和外观假设，需要更鲁棒的训练信号。

Method: 在光度流程基础上，添加基于DINOv3编码器的掩码预测损失，预测器从可见上下文嵌入推断目标区域嵌入，推理时丢弃。

Result: 在KITTI上优于同基线，与SOTA transformer方法竞争，超越CNN基线；零样本迁移至Make3D和Cityscapes达最优或次优。

Conclusion: JEPA目标可有效增强自监督深度估计，无需额外推理开销，泛化性强。
...
### [75] [Physically Real-time Infrared Attack against Optical Flow Estimation Networks](https://arxiv.org/abs/2607.26651)
*Shen You,Wei Jiang,Jiarui Liu,Yijian Ye,Qiuzhen Lin,Xiangtao Li,Ka-Chun Wong*

Main category: cs.CV

TL;DR: 提出了一种利用红外光在物理世界实时攻击光流估计网络的方法。


<details>
  <summary>Details</summary>
Motivation: 光流估计网络在自动驾驶等安全关键应用中至关重要，但其鲁棒性测试不足。

Method: 通过预先大量生成对抗样本，实时计算并动态显示，利用红外光实现物理世界直接攻击。

Result: 在不同光照、运动速度和物体位置下成功削弱了网络的光流估计能力。

Conclusion: 该方法可有效实现物理世界对光流估计网络的实时隐蔽攻击。
...
### [76] [Genie Sim PanoWorld: An Infinite Indoor 3D World Generation Pipeline via Panoramic Scene Modeling and Simulation](https://arxiv.org/abs/2607.26646)
*Yongxin Su,Linjie Hou,Feng Wang,Jialin Tang,Zhijun Li,Qian Wang,Maoqing Yao*

Main category: cs.CV

TL;DR: 提出Genie Sim PanoWorld，从单张360度全景图生成高质量可导航3D场景的两阶段流程，无需每场景优化。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么缺乏可控轨迹影响3D重建，要么在大范围相机运动下出现大空洞且需要多GPU服务器。

Method: 两阶段前馈流程：NavMesh规划SE(3)轨迹并注入潜在视频扩散模型，通过稠密几何扭曲条件、长短轨迹混合训练和自一致性目标生成高清视频；前馈全景重建器将视频提升为3D高斯场景。

Result: 在视频生成和下游3D重建上优于几何条件基线，零样本泛化到未见室内场景。

Conclusion: 实现了从单全景图到高保真、可实时导航3D场景的高效重建，并可直接用于具身AI应用。
...
TL;DR: 提出CDPI方法分析异构多模态大模型跨尺度融合中的选择性知识迁移，发现推理能力（尤其是高维推理）的迁移集中在语言模型且仅在小比例参数注入时有效，感知能力几乎未提升。


<details>
  <summary>Details</summary>
Motivation: 现有融合研究聚焦于聚合指标改进，但未揭示小模型实际继承了哪些能力，且缺乏对大范围任务集合中跨尺度能力迁移规律的理解。

Method: 提出Cross-Scale Directional Parameter Injection (CDPI)，一种线性探针方法，结合一阶选择性和二阶曲率约束的理论分析，用于分析异构融合中的跨尺度知识迁移。

Result: 在四个Qwen3-VL模型对和12个多模态基准上，推理能力（尤其是高维推理）显著提升，感知能力几乎不变；语言模型贡献主要推理增益，小比例参数注入时转移最显著。

Conclusion: 跨尺度异构多模态大模型融合本质上是受限的、低干扰的语言侧推理迁移，而非广泛的能力继承。
...
### [78] [FPSGen: Flexible Point Cloud Scene Generation with BEV-Supported Transport Flows](https://arxiv.org/abs/2607.26645)
*Wenzhe He,Meng Wang,JiaWei Qian,Jinfeng Xu,Ying Liu,Ruihui Li*

Main category: cs.CV

TL;DR: FPSGen是一种灵活的室外场景点云生成框架，通过预测BEV先验构建点源，结合教师-学生最优传输路径矫直，实现无需部分扫描的无条件和条件生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于点的室外场景生成方法依赖于部分扫描，存在训练-推理不匹配导致的稀疏和不完整几何问题，且灵活性有限。

Method: 首先从主动线索预测带有密度、高度和掩码通道的BEV先验，再采样密度图构建BEV支撑点源；然后通过教师-学生最优传输方案利用教师预测端点学习速度场，矫直传输路径。

Result: 在SemanticKITTI补全任务上达到最优JSD和体素IoU，单步传输保持强性能；在KITTI-360无条件生成上取得最佳覆盖率（COV）。

Conclusion: FPSGen统一了无条件与灵活条件引导的场景生成，解决了部分扫描依赖和几何不完整问题。
...
### [79] [FakeIDet3-DB: Refining Digital Attacks and Patch Extraction for Secure ID Benchmarking](https://arxiv.org/abs/2607.26641)
*Muñoz-Haro Javier,Teruel Andres,Tolosana Ruben,DeAlcala Daniel,Vera-Rodriguez Ruben,Morales Aythami,Fierrez Julian*

Main category: cs.CV

TL;DR: 提出了FakeIDet3-DB数据库，包含对真实政府签发身份证的数字操纵（包括传统和生成式AI驱动的伪造），并设计PACE算法在保护隐私的前提下提取高密度语义补丁。实验表明现有模型难以检测和定位攻击。


<details>
  <summary>Details</summary>
Motivation: 身份证件认证依赖复杂高频安全图案的结构完整性，但生成式AI能注入高保真局部操纵绕过标准验证。训练检测模型受隐私法规限制，依赖缺乏真实纹理的合成模板，存在领域差距。

Method: 构建FakeIDet3-DB数据库，包含传统（如复制-移动）和生成式AI（如换脸、修复）操纵并增强精炼。提出PACE算法，利用积分图映射和距离驱动的非极大值抑制（NMS）提取隐私保护补丁。

Result: 从超过6.4K张真实/伪造身份证图像中提取近5.2M补丁。评估显示最先进模型难以检测和定位攻击（检测等错误率32.45%，定位AUC-ROC 83.48%）。

Conclusion: 现有模型在检测生成式和传统身份证伪造攻击上性能不足，FakeIDet3-DB为训练鲁棒取证模型提供了重要基准。
...
### [80] [Anchoring and Steering Diffusion: Enhancing the Faithfulness of Text-to-Image Generation at Inference Time](https://arxiv.org/abs/2607.26647)
*Xinyi Wang,Yuyang Huang,Yalin Su,Pengcheng Luan,Tao Zhang,Feiming Wei,Wenxian Yu*

Main category: cs.CV

TL;DR: 提出AnchorSteer框架，通过语义锚定和反思性引导两个组件，在无需训练的情况下实现文本到图像扩散模型的精确语义对齐。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在处理复杂组合提示时对齐精度不足，现有方法在初始噪声或去噪轨迹的改进上存在局限性。

Method: 语义锚定使用CLIP提取先验并通过LP-SDS损失弥合域差距，反思性引导基于VLM诊断并执行Think-Erase-Retouch循环进行语义纠正。

Result: 在GenEval和T2I-CompBench++上，AnchorSteer在文本-图像对齐上一致优于基线，并保持高视觉质量。

Conclusion: 同时优化初始化和去噪轨迹的无训练框架能有效提升对齐精度，且方法具有通用性。
...
### [81] [Dual Inversion for Text-to-Image Diffusion Models: From Both Prompt and Noise Perspectives](https://arxiv.org/abs/2607.26735)
*Xiaolong Liu,Junjian Li,Yuan Xiao,Jiaqi Deng,Dayong Ye,Tianqing Zhu,Huan Huo*

Main category: cs.CV

TL;DR: 提出Dualin双阶段方法，联合恢复语义提示和潜在噪声，实现高质量图像生成与编辑。


<details>
  <summary>Details</summary>
Motivation: 现有提示反转方法存在梯度不稳定、缺乏细节对齐等问题，忽视了编码结构信息的潜在噪声的关键作用。

Method: 第一阶段利用CLIP和大语言模型生成可解释硬提示；第二阶段通过无条件DDIM反转重建精确潜在噪声，保证结构一致性。

Result: 在多个数据集上达到最优图像保真度，生成高质量反转提示，支持灵活图像编辑。

Conclusion: Dualin解决了提示反转的局限性，为精确可控的图像编辑提供了坚实基础。
...
### [82] [Visko Orbis 1.0: A Live Model for Real-Time Interactive Long Video Generation](https://arxiv.org/abs/2607.26694)
...
TL;DR: 提出了Visko Orbis 1.0，一种支持实时交互式长视频生成的实时模型。


<details>
  <summary>Details</summary>
Motivation: 现有长视频生成系统难以实现实时交互和长时间高质量输出。

Method: 采用有界多尺度记忆保持主题、场景和风格，结合蒸馏的块式流生成器和流式视频升频器。

Result: 在实时4K视频生成（24FPS）下，获得用户偏好和时序稳定性最高分。

Conclusion: Visko Orbis 1.0在实时交互式长视频生成中优于现有系统。
...
### [83] [Multimodal fusion of visual and morphometric features for avian bone classification](https://arxiv.org/abs/2607.26743)
*Nevio Dubbini,Lisa Yeomans,Marco Pavia,Ramazan Parmaksiz,Ayse Atas Hooglugt,Gabriele Gattiglia,Beatrice Demarchi*

Main category: cs.CV

TL;DR: 本研究提出了一个结合CNN图像分析和骨测量的多模态框架，用于鸟类骨骼分类。在骨类型识别上达到86%的准确率，科级分类的top-1和top-3准确率分别为51%和75%。


<details>
  <summary>Details</summary>
Motivation: 人工智能在动物考古学中应用有限，尤其是对鸟类骨骼的鉴定，需要开发可扩展、可解释的工具。

Method: 使用BiRefNet和SAM2的两阶段分割管道，EfficientNet_V2_S提取视觉特征，与标准骨测量数据通过特征级多模态架构融合，进行骨骼元素和科级分类。

Result: 骨类型分类测试集准确率86%；科级分类top-1准确率51%，top-3准确率75%。

Conclusion: 结合视觉与骨测量信息的多模态深度学习方法可行，为未来AI辅助动物考古鉴定建立了方法学基线。
...
### [84] [Sequence-SOD: Bio-inspired Sequence-aware Spiking ObjectDetection for Event Cameras](https://arxiv.org/abs/2607.26703)
*Katharina Bendig,René Schuster,Didier Stricker*

Main category: cs.CV

TL;DR: 提出序列感知SNN目标检测器Sequence-SOD，在连续事件序列上训练，利用跨时间步的膜电位演化提升检测性能，在Gen1数据集上将mAP从23.38提升至26.88，理论预测频率达40 Hz。


<details>
  <summary>Details</summary>
Motivation: 传统SNN目标检测器仅处理孤立事件间隔并重置网络状态，未能充分利用连续事件流中的时间信息。

Method: 将事件累积为短间隔并离散为时间步，输入SSD风格的脉冲DenseNet，在序列内保留膜电位跨间隔传递，实现基于演化神经状态的检测。

Result: 在Gen1数据集上，序列感知训练使mAP从23.38增至25.30（无增强）或26.88（有增强），理论预测频率达40 Hz。

Conclusion: 序列感知训练能有效利用时间线索并保持节能优势，是事件SNN检测架构改进之外的互补方向。
...
### [85] [TPD: Temporal Prior Decoupling for Text-to-Video Diffusion Models](https://arxiv.org/abs/2607.26706)
*Taewon Kang,Matthias Zwicker*

Main category: cs.CV

TL;DR: 提出了一种无需训练的时间先验解耦框架，用于修复文本到视频扩散模型中后期事件合成失败的问题。


<details>
  <summary>Details</summary>
Motivation: 现有时序扩散模型在处理“早期场景持续+新事件出现”的提示时，后期事件在对应帧中常无法实现，这种现象被定义为时间先验抑制。

Method: 通过构建早期段的条件时间反事实，定义全提示与反事实轨迹差异为抑制信号方向，并采用帧选下界约束在去噪过程中恢复该信号。

Result: 在多个视频生成骨干上显著提升后期概念实现，同时保持时序连贯性和视觉保真度。

Conclusion: 时间先验抑制广泛存在，所提框架无需重新训练即可有效恢复被抑制的后期事件信号，优于现有减法投影方法。
...
### [86] [Searching for Robust Augmentations to Improve Out-of-Domain Generalization in Dermoscopic Skin Cancer Classification](https://arxiv.org/abs/2607.26765)
*Alexander Kozachok,Ilya Latyshev,Evgeny Karpulevich,Elena Kozachok,Egor Ushakov,Oleg Samovarov*

Main category: cs.CV

TL;DR: 研究了数据增强方法在皮肤病变分类器中增强域外泛化能力的效果，发现mix策略和光度变换最有效，但评估存在潜在偏差。


<details>
  <summary>Details</summary>
Motivation: 改善皮肤镜分类器在成像设备、光照和伪影等域偏移下的鲁棒性，尤其是域外泛化能力。

Method: 使用ConvNeXt-Large骨干网络和ROC-AUC，在ISIC Archive和Derm7pt上搜索单增强、光度组合和复合策略；按病变ID分割，将HAM10000和ISIC 2019-2020作为域外测试集。

Result: mix策略带来最大域外增益+0.053（95% CI +0.045至+0.061, p<0.001）；在小型独立临床集中，灵敏度从0.591升至0.818，但基于少量恶性案例且不具跨种子一致性。

Conclusion: 模拟真实域偏移的增强比优化域内准确率更重要；但策略在评估来源上选择，结果可能有偏差，需源不重叠的选择协议。
...
### [87] [FreeShadow: Training-Free Shadow Removal via Illumination Transfer and Selective Content Preservation in Diffusion Models](https://arxiv.org/abs/2607.26715)
*Yinan Wang,Yan Huang,Yong Xu,Patrick Le Callet*

Main category: cs.CV

TL;DR: 提出了无需训练阴影去除方法FreeShadow，基于预训练扩散模型通过照明转移注意力和内容保留机制实现鲁棒泛化。


<details>
  <summary>Details</summary>
Motivation: 现有监督/无监督阴影去除方法因训练数据多样性不足泛化受限，零样本方法存在伪影且需耗时优化。

Method: 利用预训练扩散模型先验，设计照明转移注意力(ITA)传递非阴影区亮度，分析扩散模型自注意力图和潜在高频特征保留光照不变成分，提出局部纹理保持重照明(LTPR)缓解VAE压缩导致的纹理错位。

Result: 实验表明方法具有强泛化性，能生成真实无阴影图像。

Conclusion: FreeShadow无需训练或优化即可通过扩散先验实现高性能阴影去除。
...
### [88] [See2Think: Do Multimodal Models Really Use Intermediate Visual States?](https://arxiv.org/abs/2607.26769)
...
TL;DR: 提出了See2Think评估框架，包含1200个视觉依赖问题，揭示了多模态大模型在视觉推理中对视觉状态存在行为依赖。


<details>
  <summary>Details</summary>
Motivation: 现有基准存在任务覆盖窄、可文本求解的问题，且评估仅关注最终答案，忽略了中间视觉状态的生成和使用过程。

Method: 构建See2ThinkBench（12类1200个开放问题）和VAoT（记录文本思考、视觉动作、渲染状态及推理过程），通过受控干预测试模型依赖。

Result: 视觉推理高度依赖模型和环境，忠实渲染是瓶颈，高反馈采纳不一定提升准确率，受控干预下准确率下降超10%。

Conclusion: 模型对视觉状态存在行为依赖，但推理效果受模型、环境和渲染质量共同影响。
...
### [89] [Progressive Multimodal Alignment for Continual Instruction Tuning](https://arxiv.org/abs/2607.26947)
*Duzhen Zhang,Yahan Yu,Qiaoyi Su,Jiahua Dong,Tielin Zhang*

Main category: cs.CV

TL;DR: 提出渐进多模态对齐方法，解决多模态大模型在持续指令微调中的投影仪遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注LLM主干遗忘，忽略了投影仪因视觉分布和指令语义变化导致的遗忘。

Method: 通过轻量表示描述符检测分布偏移，按需扩展投影仪专家，并使用可扩展路由器集成多模态特征输出，同时保留预训练投影仪作为稳定锚点。

Result: 在两种基准上，结合PMA的方法持续超越现有技术，且可跨不同MLLM主干扩展。

Conclusion: 缓解投影仪级遗忘可提升MCIT性能，PMA作为方法无关的插件具有鲁棒性和泛化性。
...
### [90] [CASIAL: Geometric Distortion Robust Image Watermarking](https://arxiv.org/abs/2607.26729)
*Yupeng Qiu,Han Fang,Ee-Chien Chang*

Main category: cs.CV

TL;DR: 提出了CASIAL框架，通过全局水印扩散和几何不变表示学习，显著提升深度学习水印对几何变换的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习水印方法对几何变换（如裁剪、缩放、旋转）鲁棒性不足，主要因区域移除和信息失步两种失效模式。

Method: 采用覆盖图像感知的消息扩散策略（CAS）将水印比特与图像特征自适应分布，以及不变性对齐学习模块（IAL）利用空间注意力对齐特征到几何不变空间。

Result: 在六种几何变换下鲁棒性显著优于11个基线方法，同时在信号和光度变换上保持竞争力，且对未见过的黑盒变换具有迁移鲁棒性。

Conclusion: CASIAL实现了宽泛的鲁棒性和高视觉质量，验证了全局扩散和不变表示对几何鲁棒性的必要性。
...
### [91] [Online Handwriting Trajectory Reconstruction from Kinematic Sensors using Temporal Convolutional Network](https://arxiv.org/abs/2607.26733)
*Wassim Swaileh,Florent Imbert,Yann Soullard,Romain Tavenard,Eric Anquetil*

Main category: cs.CV

TL;DR: 本文提出了一种新型处理流程，利用动态时间规整和时序卷积网络，从数字笔传感器信号重建在线手写轨迹，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 数字笔在任意表面书写时，需从传感器信号重建手写轨迹，以支持书写学习、协作会议等应用，而现有方法存在采样率不匹配等问题。

Method: 预处理阶段使用动态时间规整对齐笔与平板（真值）信号；设计基于时序卷积网络的神经网络架构，用于从传感器信号重建手写轨迹。

Result: 在新基准数据集上，该方法在定性和定量评估中均显著优于最显著的竞争对手。

Conclusion: 所提处理流程能有效利用数字笔传感器信号重建在线手写轨迹，为相关应用提供可靠方案。
...
### [92] [Hearsay: Vision-Language Medical Diagnoses Without an Image](https://arxiv.org/abs/2607.26886)
*Siddharth Vohra*

Main category: cs.CV

TL;DR: 研究表明，前沿视觉语言模型（如Claude Opus、GPT和Gemini）在没有医学图像的情况下，会根据患者的种族、年龄和性别等人口统计信息编造诊断结果，这种“幻象”并非随机，而是系统性地受到患者身份信息的影响。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在揭示视觉语言模型在缺失图像时是否会产生虚假诊断，并探究这种编造行为是否与患者的人口统计信息有关。

Method: 实验使用胸部X光、脑部MRI和皮肤科图像，仅向Claude Opus-4.7、GPT-5.4和Gemini-3.1-Pro提供人口统计描述（无图像），然后分析其返回的诊断变化。

Result: 模型会编造诊断，且差异显著：Claude对65岁白人男性的皮肤痣几乎总是报告黑色素瘤；GPT-5.4对年轻黑人患者的胸部X光倾向于报告结节病；Claude的效应在将“皮肤痣”改为“皮肤病变”后完全消失。
...
### [93] [ScratchSim: A Procedural Synthetic Data Pipeline for Surface Scratch Detection](https://arxiv.org/abs/2607.27065)
*Paul Julius Kühn,Saptarshi Neil Sinha,Tiago Kleist,Richard Hoffmann,Arjan kuijper,Michael Weinmann*

Main category: cs.CV

TL;DR: 提出一个程序化渲染管线生成合成缺陷训练数据，通过微调策略提升少量真实数据下的缺陷检测性能。


<details>
  <summary>Details</summary>
Motivation: 标注缺陷数据稀缺阻碍了自动缺陷检测在工业质检中的应用。

Method: 基于BlenderProc构建可配置渲染管线，生成COCO格式合成数据；比较纯合成、纯真实、混合训练及从合成权重微调四种策略。

Result: 从合成权重微调始终优于纯真实训练；混合训练在真实数据稀缺时有效恢复性能；该结论在卷积和Transformer检测器上均成立。

Conclusion: 该方法可扩展缺陷检测，减少对大量真实标注数据的依赖，适用于边缘设备工业检测。
...
### [94] [StatePlay: State-Aware Game World Models for Mechanics-Consistent Generation](https://arxiv.org/abs/2607.26754)
*Zijun Lin,Zeqing Wang,Cheston Tan,Bihan Wen,Yeying Jin*

Main category: cs.CV

TL;DR: 提出StatePlay，一种状态感知的游戏世界模型，联合预测视觉内容和游戏状态以生成符合机制的游戏过程。


<details>
  <summary>Details</summary>
Motivation: 现有游戏世界模型仅关注像素生成，忽略游戏机制依赖的内部状态（如血量、技能计时器），导致生成结果视觉合理但违反游戏规则。

Method: 采用混合Transformer架构，设置专用分支分别处理视觉和状态表示，通过跨模态交互使预测状态引导帧生成，各分支采用针对性优化目标。

Result: 状态预测平均归一化L1距离低于0.06，相比无显式状态建模的模型，游戏生成机制保真度提升18.6%。

Conclusion: 状态感知建模对生成机械一致的游戏至关重要，推动了超越像素真实感的完整游戏生成发展。
...
### [95] [SciFigAlign: Scoring Scientific Figures by Fine-tuned Alignment of Visuals with Manuscript Evidence](https://arxiv.org/abs/2607.27066)
*Chuanzhi Xu,Zihan Deng,Huiqi Liang,Chengkun Yue,Zhanlin Cui,Pengfei Ye,Weidong Cai*

Main category: cs.CV

TL;DR: 提出科学图评估指标SciFigAlign，通过跨模态对齐和证据融合实现更准确的科学图表质量评估。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法（如IQA、CLIP、LLM/VLM）无法捕捉科学图与论文论证的深层关联，评分集中且缺乏多模态融合。

Method: 构建3,857张科学图数据集，标注清晰度、相关性、信息量和结构四维度；微调CLIP+SciBERT，采用跨模态交叉注意力和CubeMLP融合，结合SmoothL1回归与排序损失优化。

Result: Macro MAE 0.3524，成对准确率81.64%，错误率比最佳LLM基线降低59%。

Conclusion: 科学图评估需学习视觉与论文证据的对齐，仅靠提示词不够，即使使用先进VLM。
...
### [96] [Long-Tailed 3D Point Cloud Dataset Distillation](https://arxiv.org/abs/2607.26763)
*Jiahao You,Xu Han,Jinfeng Xu,Xianzhi Li*

Main category: cs.CV

TL;DR: 本文首次研究了长尾分布下的3D点云数据集蒸馏问题，提出自适应预算分配和长尾分布匹配两个模块，显著提升了分类准确率。


<details>
  <summary>Details</summary>
Motivation: 现有点云数据集蒸馏方法忽略了数据集固有的长尾类分布问题，导致模型对尾部类别性能不佳。

Method: 1. 自适应合成预算分配：根据各类别样本数量和额外合成样本的预期收益分配预算；2. 3D长尾分布匹配：包括全局-局部特征对齐（保持全局分布和类内多样性）和先验感知监督（根据类别重要性调整监督权重）。

Result: 在ShapeNet55数据集上，分类准确率相比现有最优方法提升7.0个百分点。

Conclusion: 该方法有效处理了点云数据的长尾分布问题，实现了更均衡且高性能的蒸馏数据集生成。
...
### [97] [SciFigQual-Bench: A Benchmark for Scientific Figure Quality Assessment with Full-Manuscript Context](https://arxiv.org/abs/2607.27084)
*Zihan Deng,Chuanzhi Xu,Huiqi Liang,Haoyang Li,Xiaozhen Zhong,Lequan Yu*

Main category: cs.CV

TL;DR: 提出了SciFigQual-Bench基准，评估科学图像在五个维度上的质量，并设计了SFQ-Agent框架实现自动化评估，实验表明该方法优于现有方案。


<details>
  <summary>Details</summary>
Motivation: 现有图像质量评估方法不适用于科学图像，且缺乏对图文对齐、引用相关性和视觉误导性的验证。

Method: 构建SciFigQual-Bench基准包含6308张图像的五维专家评分，并设计SFQ-Agent多阶段交叉评估框架。

Result: SFQ-Agent（F3）在eval1200子集上达到最低平均绝对误差0.418和最高一致性率93.4%。

Conclusion: 该基准和评估框架能有效评估科学图像质量，超越直接评估和辅助视觉语言模型方法。
...
### [98] [PRISM-Net: Patient-specific reference-guided inter-breast symmetry matching for three-class breast DCE-MRI classification](https://arxiv.org/abs/2607.26799)
*Boya Zhang,Shuaiwen Zhou,Di Kong,Mingxu Wang,Wenbiao Du,Yiman Zhong,Yuexin Duan,Xiawei Yue,Liuquan Cheng,Xiru Li*

Main category: cs.CV

TL;DR: 提出了PRISM-Net，一种无需配准的双侧乳腺DCE-MRI分类框架，通过利用对侧乳腺特征作为患者特异性背景参考，提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注单侧或病灶中心分析，而双侧方法缺乏对空间自适应交叉乳腺对应关系的显式建模，患者背景变异是分类混淆的主要来源。

Method: PRISM-Net集成双侧特征匹配和不对称感知注意力，建立自适应双侧乳腺对应关系，增强判别性不对称模式的表示，无需图像配准。

Result: 在ODELIA数据集上，Macro AUC、Micro AUC和二次加权kappa分别达到84.11±2.33、90.64±1.61和60.94±5.64（在分布内测试集），在保留机构上分别为68.51±4.54、80.74±2.68和43.45±7.10，优于基线方法。

Conclusion: 患者特异性双侧参考建模是一种临床合理的DCE-MRI解释策略，通过显式建模背景复杂性改善不对称模式判别，双侧关系建模和不对称感知重加权均有助于性能提升。
...
### [99] [Visual Credit Audit for Multimodal Spatial Reasoning](https://arxiv.org/abs/2607.27069)
*Feixiang Liu,Qiang Qiu,Lanbo Sun,Nan Wei,Huawei Shen,Xueqi Cheng*

Main category: cs.CV

TL;DR: 提出视觉信用审计方法，将空间基准的模型表现分解为正确性、图像额外支持、关系一致性响应三个维度，揭示了12.73-26.25%的正确决策缺乏图像证据支持。


<details>
  <summary>Details</summary>
Motivation: 现有空间基准中模型可能答对却未利用图像信息，需要区分回答正确是否真正依赖视觉证据。

Method: 设计视觉信用审计（VCA）框架，通过无文本控制和空白控制对比、图像排列置换实验、关系对比和3x3证据因子分析等，分离估计图像支持和关系响应。

Result: 四个开源MLLM及两个空间基准中，12.73-26.25%正确决策无图像支持；图像排列置换使依赖正确率下降21.25-47.80分；关系反转导致81.57-100%一致决策改变答案。

Conclusion: VCA有效分解了空间基准的模型表现，揭示了模型正确决策中图像证据缺乏和关系响应不一致的问题。
...
### [100] [DistillAlign: Coordinating Mode Covering and Mode Seeking in Autoregressive Video Distillation](https://arxiv.org/abs/2607.26811)
*Jiaxing Li,Kai Zou,Cindy Zhou,Kaichen Huang,Junyao Gao,Zile Wang,Yang Liu,Bin Liu,Bo An,Yangguang Li*

Main category: cs.CV

TL;DR: 本文从分布对齐角度重新审视自回归视频蒸馏，提出联合蒸馏方法，结合模式寻求与模式覆盖约束，提升生成质量、覆盖率和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有DMD多阶段蒸馏中，初始化与DMD阶段追求不同目标分布，且仅依赖视觉评分评估中间学生，忽略了分布对齐的重要性。

Method: 引入分布评估协议（精度与覆盖率），并提出联合蒸馏，将DMD的模式寻求目标与一致性蒸馏的模式覆盖约束相结合。

Result: 联合蒸馏方法在生成质量、覆盖率和多样性上均有提升；使用Wan-1.3B DMD教师蒸馏的结果优于用Wan-14B基线精炼的效果。

Conclusion: 分布对齐在自回归视频蒸馏中至关重要，联合蒸馏有效解决了模式覆盖不足的问题。
...
### [101] [Anatomy Contextualized Adaption of CT Foundation Models](https://arxiv.org/abs/2607.27154)
*Roshan Kenia,Stephanie L McNamara,William Lotter*

Main category: cs.CV

TL;DR: ACA是一种轻量级框架，通过冻结CT基础模型并引入跨解剖结构transformer，实现了解剖级视觉-语言对齐，同时保留全局上下文，在零样本分类中优于现有方法且训练高效。


<details>
  <summary>Details</summary>
Motivation: 现有CT视觉语言模型使用整体表示，忽略了细粒度解剖信号；细粒度方法丢弃全局上下文且训练成本高。

Method: 使用TotalSegmentator将CT分解为解剖级嵌入，通过跨解剖transformer细化，并与解剖和扫描级文本对齐。

Result: 在Merlin和CT-RATE上，ACA在零样本分类中一致优于冻结基础模型和现有细粒度方法，训练时间不到一小时。

Conclusion: ACA是一种轻量级方法，可在保留全局解剖上下文的同时实现解剖级视觉-语言对齐。
...
### [102] [Ripple: Real-Time Streaming Audio-Video Generation With Cross-Modal Recurrent Memory](https://arxiv.org/abs/2607.26818)
*Yanbo Ding,Zhizhi Guo,Quanyue Song,Yishan He,Zhixiang He,Yongxiang Li,Yali Wang*

Main category: cs.CV

TL;DR: 提出Ripple，一种实时音视频联合生成系统，采用跨模态循环记忆机制，在480P分辨率下实现约28 FPS的推理速度，支持长序列生成。


<details>
  <summary>Details</summary>
Motivation: 现有音视频生成模型延迟高，无法满足实时应用需求；流式生成方法成本高且不支持长序列生成。

Method: 结合固定长度滑动窗口注意力与模态特定记忆状态，引入跨模态记忆交互增强音视频同步；采用三阶段训练：因果注意力适应、端到端蒸馏记忆管道、在线强化后训练。

Result: 在480P分辨率下达到约28 FPS，速度远超过教师模型，在短/长视频基准上优于现有离线和在线联合音视频生成方法。

Conclusion: Ripple实现了高效实时的长序列音视频联合生成，验证了记忆机制与三阶段训练的有效性。
...
### [103] [ICDAR 2026 Competition on Information Extraction from Atomic Layer Deposition/Etching (ALD/E) Scientific Figures](https://arxiv.org/abs/2607.26848)
*Fahad Ahmed,Sören Auer,Jennifer D'Souza*

Main category: cs.CV

TL;DR: Sci-ImageMiner 基准数据集和社区竞赛用于评估多模态 AI 在科学图表理解与推理中的表现，发现模型在分类和摘要任务上表现良好，但在数据提取和科学推理上存在局限。


<details>
  <summary>Details</summary>
Motivation: 现有科学图表理解研究缺乏综合性、专家标注的数据集和高标准的竞赛，难以推动多模态 AI 在领域特定推理方面的发展。

Method: 构建了包含四个端到端互补任务的专家注释数据集 Sci-ImageMiner，并组织社区竞赛，收集了 68 名参与者的 1,263 次公开/私有提交。

Result: 最先进的多模态模型在分类和摘要任务中表现良好，但在数据提取和科学推理（尤其是视觉问答）中表现不佳。

Conclusion: Sci-ImageMiner 基准和竞赛为科学图表理解提供了严格的评估平台，揭示了当前模型的局限性，并指出了提升领域感知多模态 AI 系统的方向和机遇。
...
### [104] [BATS: Resource-Efficient Volumetric Segmentation with Boundary-Aware Mixed-Resolution Tokens](https://arxiv.org/abs/2607.26829)
*David Hagerman,Roman Naeem,Fredrik Kahl*

Main category: cs.CV

TL;DR: BATS是一种边界感知的3D医学图像分割架构，通过混合分辨率处理减少内存和计算成本，在多个数据集上接近最佳密集模型性能，同时显著降低内存和推理时间。


<details>
  <summary>Details</summary>
Motivation: 现有高性能3D分割模型依赖密集多尺度特征图，导致高内存和推理成本，作者旨在通过边界感知的稀疏表示降低开销。

Method: 采用边界预测器识别细粒度区域，构建输入依赖的混合分辨率层次，通过父簇注意力提供跨尺度上下文，并在各分辨率独立预测边界相关性。

Result: 在5个CT/MRI数据集上，BATS的LiTS Dice最高，平均与最强密集基线MedNeXt-L相差0.37 Dice点，内存减少超53%，推理速度在KiTS和LiTS上快30%，但在BraTS上较慢。

Conclusion: 混合分辨率处理在内存节省上稳定，但运行时间和精度提升依赖数据集的边界密度，BATS实现了高效的体积分割权衡。
...
### [105] [CinemaTraj: Composing Atomic Camera Trajectories for 3D Scenes with LLM Agents](https://arxiv.org/abs/2607.26910)
*Qianru Li,Xuyang Chen,Erkin Türköz,Lu Liu,Xuqin Wang,Liqiu Meng,Tao Wu,Yanfeng Zhang*

Main category: cs.CV

TL;DR: 提出了CinemaTraj框架，利用3D场景图和语言指令自动生成电影化相机轨迹，在ScanNet++上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏3D空间意识或只做几何路径规划，无法结合电影语义生成表达性轨迹。

Method: 用LLM代理结合结构化3D场景图将提示分解为原子电影运动，每种运动由参数化轨迹表示并支持避障优化。

Result: 在真实ScanNet++场景中生成的轨迹忠实于提示、无碰撞且电影质量高，在提示匹配、轨迹质量和安全性上超越现有方法。

Conclusion: 将相机轨迹规划转化为语言引导的空间推理问题，能有效生成电影化、安全且语义一致的视频输出。
...
### [106] [SCALPEL: Semantic Cross-modal Alignment via LLM-Powered Encoder Learning for Medical Vision-Language Representation](https://arxiv.org/abs/2607.26885)
*Yunzhan Fu,Enyu Bao,Xiangyu Shen,Yihao Wu,Chunbo Jiang,Fangli Guan,Liqi Yan*

Main category: cs.CV

TL;DR: 本文提出SCALPEL框架，通过将生成式大语言模型（LLM）转换为各向同性编码器、采用非对称对齐策略减少内存开销，并设计解剖-否定感知损失函数，解决了医学视觉-语言预训练中的各向异性表示崩溃、高内存需求及医学幻觉问题，在多项任务中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLP框架受限于轻量文本编码器的有限上下文窗口和浅层表示能力，无法处理长而术语密集的临床报告；引入医学LLM虽带来临床推理能力，却导致表示各向异性、高训练内存开销及由对比损失引起的医学幻觉。

Method: 提出SCALPEL框架：1) 领域临床报告对比微调将生成式LLM转换为各向同性编码器；2) 非对称对齐策略利用离线特征缓存实现高效训练；3) 解剖-否定感知目标函数显式惩罚包含侧位混淆或否定错误的图文对。

Result: 在MIMIC-CXR、CheXpert和IU X-Ray基准上，跨模态检索、零样本疾病分类和医学视觉问答任务均达到最优性能。

Conclusion: SCALPEL有效克服了医学LLM集成的三大瓶颈，为医学多模态表示学习提供了高效且语义精准的解决方案。
...
### [107] [Prior Directions: Why GUI Grounding Gets Locked in the Past](https://arxiv.org/abs/2607.26913)
*Weile Gong,Zijian Lu,Mingcai Chen,Yiping Zuo,Xin He,Weibei Fan*

Main category: cs.CV

TL;DR: 研究发现视觉语言模型中的视觉锁定现象源于先验信息引发的表示变化沿少数重复方向集中，移除这些方向可恢复视觉基础能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型在场景变化时易受先验语言描述的误导，导致视觉判断出错，需揭示其机制。

Method: 在控制先验变量的基础设定下，比较多模型的表示变化方向性，通过干预实验分离先验方向与正交方向的影响。

Result: 先验锁定与表示变化沿紧凑方向集（先验方向）的集中度正相关；移除先验方向可恢复视觉基础，而移除等量正交方向无效。

Conclusion: 先验控制源于先验在答案表示中形成连贯可复用的方向模式，解释了不同模型对相同先验的敏感性差异。
...
### [108] [From Keypoints to Predictive Distributions: Post-Hoc Uncertainty for YOLO-Pose Models](https://arxiv.org/abs/2607.26921)
*Alexej Klushyn,Juan Rivero Sesma,Florian Seligmann,Richard Kurle,Kinh Tieu,Jayant Sen Gupta*

Main category: cs.CV

TL;DR: 为YOLO-Pose引入后验概率扩展，通过预测关键点位置的协方差矩阵和校准分布，实现不确定性量化与可靠性排序。


<details>
  <summary>Details</summary>
Motivation: YOLO-Pose虽高效但无法量化空间不确定性。

Method: 训练概率头预测输入相关的2×2色散矩阵，结合高斯或t分布校准，并设计AKP评估指标。

Result: 不确定性估计支持可靠性排序，t分布校准最佳，不确定性剪枝有效。

Conclusion: 该方法在飞行着陆等任务中提升关键点定位的可靠性。
...
### [109] [Mitigating Compounding Error via Video Representation Regularization](https://arxiv.org/abs/2607.27036)
*Taiye Chen,Qi Zhang,Yisen Wang*

Main category: cs.CV

TL;DR: 视频扩散世界模型在长时自回归生成中会出现误差累积，本文发现该现象与隐藏表示的维度坍塌密切相关，并提出视频表示正则化方法有效提升生成稳定性。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在长时自回归推理中存在严重误差累积，但其机制和解决方案尚不明确。

Method: 分析了视频世界模型的内部表示动力学，提出视频表示正则化训练约束。

Result: 在VBench的美学质量和成像质量指标上分别从38.65提升至55.56和从44.37提升至72.08。

Conclusion: 建立了自回归视频漂移与模型内部表示的首个关联，并提出了简单有效的正则化策略。
...
### [110] [Robust RPC Bundle Adjustment for Multi-Date Satellite Imagery with Season-Invariant Correspondences](https://arxiv.org/abs/2607.26973)
*Roger Marí,Elías Masquil,Xavier Bou,Thibaud Ehret,Gabriele Facciolo*

Main category: cs.CV

TL;DR: 提出了一种外观感知的RPC精化流程，结合学习型局部特征匹配和全局图像描述符，提升多时相卫星影像的无地面控制点几何精化效果。


<details>
  <summary>Details</summary>
Motivation: 传统RPC光束法平差依赖手工特征匹配，在多时相影像中易受季节、光照和地表变化影响，匹配不可靠。

Method: 采用学习型局部特征匹配获取季节不变对应点，并用全局图像描述符筛选视觉兼容的图像对，减少冗余匹配并保持图连通性。

Result: 在WorldView-3多时相影像上，相比开源基线方法，几何一致性误差更低，匹配时间在39-42视图数据集上大幅减少。

Conclusion: 该流程使RPC精化对跨时相外观变化更鲁棒，提高了多时相卫星影像的实用价值。
...
### [111] [Object Detection for Autonomous Driving in Chinese Rural Scenes: An Experimental Study on Real-Synthetic Data Mixing and Model Evaluation](https://arxiv.org/abs/2607.27058)
*Danning Zhu,Ziyan Lin,Jing Wu*

Main category: cs.CV

TL;DR: 针对中国复杂乡村交通场景，提出一个真实-合成混合目标检测数据集，系统评估13种主流检测器在不同数据比例下的性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶目标检测模型在复杂的中国乡村交通场景中面临数据稀缺和泛化挑战。

Method: 结合河南省威氏县真实图像和虚幻引擎生成的参数化虚拟场景，定义14类乡村特定目标（如电动三轮车、低速车辆和路边摊），在统一协议下评估YOLOv5/8/11/26系列和RT-DETR-L在三种数据配置（全真实、1:0.5真实-虚拟混合、1:1混合）上的性能。

Result: 适度注入合成数据（1:0.5比例）可有效提升检测性能，YOLO11m达到最高mAP@0.5为0.758；但更高比例（1:1）引入域偏移抵消数据扩展收益；大多数模型可靠识别本地车辆，但对长尾非标准物体（如摊位、栏杆）存在感知瓶颈。

Conclusion: 研究为乡村场景下的模型选择和合成数据策略提供了实证依据和新见解，有助于自动驾驶感知系统在实际乡村部署。
...
### [112] [Step-Attention Refinement of DINOv3 Features for Efficient Anterior Eye Segmentation](https://arxiv.org/abs/2607.27087)
*Philippe Baumstimler,Jean-Mathieu Gagnon,Sébastien Gagné,Mathieu Duchesneau,Clément Playout,Lama Séoud*

Main category: cs.CV

TL;DR: 提出一种基于DINOv3 ViT-Small的轻量级结构，通过逐步注意力特征精炼模块实现高效前眼节分割，在临床数据集上达到85.55% mIoU，并展现出最强的域迁移鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法因医学影像采集条件异质和标注有限而鲁棒性不足，且基础模型的表示难以高效适应密集预测任务。

Method: 采用蒸馏的DINOv3 ViT-Small作为骨干网络，引入逐步注意力特征精炼模块，在卷积解码前渐进式调整多级Transformer表示，以少量参数高效利用预训练特征。

Result: 在333张临床图像（8种采集协议、7类解剖标签）上全微调达85.55% mIoU，优于卷积和Transformer基线；在4个公开数据集上展现出最强的域迁移鲁棒性。

Conclusion: 该工作建立了临床环境稳健前眼节分割的强基线，并强调解码器设计对适应基础模型到医学分割任务的关键作用。
...
### [113] [FreqForcing: Autoregressive Long Video Generation via Spectral Self-Anchoring](https://arxiv.org/abs/2607.27110)
*Jiatong Li,Leo Liang,Linghe Kong,Yulun Zhang*

Main category: cs.CV

TL;DR: 提出了FreqForcing框架，通过频谱自锚定解决自回归视频扩散模型中的误差累积问题，实现24倍外推的长视频生成。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型在长视频生成中存在误差累积，表现为色彩漂移、运动停滞和视觉崩溃，需要解决这一问题。

Method: 从频域角度分析误差累积为低频能量漂移，提出基于频谱自锚定的训练无关框架，利用锚定注意力的低频分量保持稳定性，局部注意力的高频分量保留动态运动。

Result: 将5秒片段预训练模型扩展到2分钟生成（24倍外推），定量和定性上优于现有训练无关方法，可竞争训练基方法。

Conclusion: FreqForcing有效缓解了长视频生成的误差累积问题，验证了频域分析和频谱自锚定的有效性。
...
### [114] [Veritas++: Value-aware On-Policy Distillation for Perception-Enhanced AIGI Detection](https://arxiv.org/abs/2607.27113)
*Hao Tan,Jun Lan,Zichang Tan,Ajian Liu,Zijian Yu,Chuanbiao Song,Huijia Zhu,Weiqiang Wang,Jun Wan,Zhen Lei*

Main category: cs.CV

TL;DR: 提出了Veritas++框架，通过感知导向学习和自适应蒸馏策略，提升多模态大模型在AI生成图像检测中的细粒度感知与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM检测器在捕捉细粒度异常方面存在感知瓶颈，主要关注视觉证据的组织方式，而内在感知优化不足。

Method: 引入感知导向学习（PoRL）替代开放描述监督，使用可验证奖励增强三种基本感知能力；以及价值感知在线蒸馏（VaOPD）自适应优先高价值蒸馏信号。

Result: 在标准、野外和新兴基准测试中表现出显著泛化能力，感知学习无缝提升检测性能，VaOPD高效促进能力进化而不牺牲现有性能。

Conclusion: Veritas++通过强化基础感知能力并整合推理，有效弥合了AI生成图像检测中的感知差距，实现了稳健的泛化检测。
...
### [115] [Towards Grounded GI Endoscopy VQA via Multi-Task Learning on Small VLMs](https://arxiv.org/abs/2607.27122)
*Itbaan Safwan,Ramail Khan,Muhammad Annas Shaikh,Muhammad Atif Tahir*

Main category: cs.CV

TL;DR: 通过多任务微调提升内镜 VQA 模型的视觉基础与回答准确性。


<details>
  <summary>Details</summary>
Motivation: 推动临床采用需要模型不仅回答准确，还能反映视觉证据。

Method: 在现有 VQA 数据集上构造辅助定位和描述任务，复用专家标注的息肉掩码，并用胃肠域预训练分类器的 Grad-CAM 提供弱监督；使用低秩适配对三个小 VLM 进行微调。

Result: 在分布内和分布外数据上，多任务微调一致提升了准确性，并改进了答案标记与相关图像区域的隐式对齐。

Conclusion: 多任务微调是增强 VLM 在胃肠内镜图像中视觉基础的有效简单方法。
...
### [116] [SeasonStereo: Robust Dense Stereo Matching for Multi-Date Satellite Imagery via Generative AI](https://arxiv.org/abs/2607.27139)
*Álvaro Díaz-Laureano,Roger Marí,Elías Masquil,Pablo Arias,Gabriele Facciolo*

Main category: cs.CV

TL;DR: 提出了SeasonStereo框架，通过合成多时相图像对和零样本几何先验，实现卫星图像立体匹配，无需对齐的真实多日期数据或LiDAR标签。


<details>
  <summary>Details</summary>
Motivation: 现有卫星3D重建依赖近同步立体对，难以处理不同季节和光照变化的图像；多日期对齐数据和真实几何标注获取成本高。

Method: 训练在具有可控季节性外观变化的合成图像对上，利用预训练基础模型的零样本几何先验进行立体匹配。

Result: 精度与最先进的LiDAR监督模型相当，且能生成更清晰的几何细节。

Conclusion: SeasonStereo为使用异构卫星图像进行大规模3D重建提供了降低监督成本的实用路径。
...
### [117] [Explainable and Resource-Efficient Spatial Reasoning in Multimodal LLMs for Decision-Critical Applications](https://arxiv.org/abs/2607.27145)
*Piyush Jain,Kousik Dasgupta,Rajarshi Roy,Subarna Tripathi*

Main category: cs.CV

TL;DR: ByDeWay-V2在无训练条件下，通过显式空间关系谓词增强MLLM空间推理，在VSR和BLINK基准上显著提升性能，并以轻量配置支持实时决策。


<details>
  <summary>Details</summary>
Motivation: MLLM在空间判断中存在不透明性和幻觉问题，现有LDP方法在同平面空间关系解析上不足。

Method: 整合YOLO-World-L开放词汇检测器计算对象间几何关系，以结构化空间谓词形式注入MLLM提示中，融合3D深度与2D语义。

Result: BLINK空间子集上相对LDP提升46% F1（Qwen2.5-VL），VSR上近乎随机性能恢复至F1=0.53，最小配置仅需40 token上下文预算。

Conclusion: ByDeWay-V2以无训练方式有效增强MLLM空间推理和可审计性，适用于资源受限的实时决策支持场景。
...
### [118] [Large-Scale ChatBot Validation Through Customer Digital Twin Simulations](https://arxiv.org/abs/2607.26060)
...
TL;DR: 提出基于合成客户代理的聊天机器人验证框架，结合数据驱动模拟与多渠道评估，实现银行场景下的规模化合规验证。


<details>
  <summary>Details</summary>
Motivation: 监管领域（如银行）中LLM聊天机器人的规模化验证缺乏成本效益方法，阻碍安全部署。

Method: 构建基于真实数据的高保真合成客户代理（SCA），结合自动LLM评判、专家测试与对抗探测的验证框架。

Result: SCA与真实客户语义对齐度高、幻觉率低且可控干预下成功复现人格特质；场景验证在情绪、人口统计、语言维度表现稳健。

Conclusion: 该框架为金融机构提供可扩展的合规路径，已成功用于英国大型银行客服聊天机器人验证。
...
### [119] [Do Methods Support the Claims? Intra-Paper Verification for Peer Review](https://arxiv.org/abs/2607.26066)
*Ranjitha Shivaprasad Ballakuraya,Arash Mahyari,Ashok Srinivasan*

Main category: cs.CL

TL;DR: 提出了一种新颖的论文内部声明验证框架，通过大型语言模型评估论文中的创新性声明是否被方法论证据充分支持。


<details>
  <summary>Details</summary>
Motivation: 现有自动新颖性评估方法忽略了对论文内部声明与方法论证据之间匹配度的检查，而人类审稿人常因此质疑创新性。

Method: 使用LLM从引言提取创新性声明，检索相关方法论证据，并基于从182篇ICLR 2025论文人类审稿意见中归纳的评估标准进行结构化评估。

Result: 在ICLR论文子集上，人类评估显示框架生成的评论与审稿人关注点高度一致，BERTScore进一步区分了匹配与不匹配的评论对。

Conclusion: 该框架能有效捕捉人类审稿人对论文内部声明与方法论之间匹配度的关切，尤其是在创新性相关问题上。
...
### [120] [DuplexGen: Adaptive Synthesis of Human-AI Turn-Taking Dialogues](https://arxiv.org/abs/2607.26178)
*Takyoung Kim,Kang-wook Kim,Sang Hoon Woo,Julia Hirschberg,Gunhee Kim,Dilek Hakkani-Tür*

Main category: cs.CL

TL;DR: 提出DuplexGen框架，通过少量人机偏好校准LLM预测，生成适应不同场景的对话轮换行为。


<details>
  <summary>Details</summary>
Motivation: 现有语音交互模型忽略场景差异，无法自适应轮换行为，且训练数据缺乏场景特异性或人类偏好。

Method: 利用少量人类偏好标注校准大语言模型（LLM）预测，生成场景自适应轮换的对话数据。

Result: 在六种合作与竞争任务中，DuplexGen生成的对话更符合人类场景特异性偏好，优于未校准或通用数据训练方法。

Conclusion: 人类校准（而非语料规模或提示设计）是实现场景特异性轮换合成的关键。
...
### [121] [Choosing Where and How to Moderate: End-to-End Trade-offs in Filter Placement and Response Rewriting](https://arxiv.org/abs/2607.26200)
*Mengya Hu,Susie Park,Suzana Ilic,Qiong Wei,Sandeep Atluri,Myra Deng,Tucker Fross,Curt Tigges*

Main category: cs.CL

TL;DR: 本文评测了内容审核配置（输入/响应过滤、重写、路由）对端到端客户指标（有用性、有害曝光）的影响，发现响应过滤在有用性上最优，输入+响应过滤有害曝光最低，重写可恢复流量但效果不等价于过滤，探针路由可降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有内容审核研究多在单一组件上评价准确率，但实际部署需考虑干预位置和后续动作，缺乏端到端客户指标指导配置选择。

Method: 使用人工标注和ToxicChat基准，比较输入过滤、响应过滤、输入+响应硬阻塞、响应+重写、探针路由等配置，以有用性、有害曝光、延迟为核心指标。

Result: 响应过滤在仅过滤时有用性最高，输入+响应过滤有害曝光最低；重写恢复流量但有害曝光与过滤相当；探针路由大幅降低条件路由时间。

Conclusion: 内容审核配置应在部署场景下按安全与延迟约束选择，而非通用规则；重写可平衡过滤与有用性，但敏感领域可能缺失安全支持信息。
...
### [122] [Characterizing Human-Likeness in AI Generated Poetry: A Zero-shot Classification Study](https://arxiv.org/abs/2607.26221)
*A. N. Biswas,T. Tabassum,A. A. Shohid,R. M. Mou,A. A. Esha,F. Sadeque,A. Ahmed*

Main category: cs.CL

TL;DR: 本文研究了英诗在人类与GenAI创作中的可区分性，提出零样本检测流程提取关键分类属性，以优化检测管道。


<details>
  <summary>Details</summary>
Motivation: GenAI文本与人类创作难以区分，且现有检测器对GenAI诗歌识别困难，需验证区分性并提取影响分类的属性。

Method: 构建包含人类与AI诗歌的数据集，采用零样本检测流程，提取影响分类与误分类的关键特征。

Result: 提取的属性可减少训练需求，仅需针对误分类特征调整模型，并提供对GenAI检测困境的关键洞察。

Conclusion: 该方法能强化现代检测管道，通过聚焦关键属性提升人类与AI诗歌的区分准确性。
...
### [123] [Steering Instruction Hierarchies at Inference Time](https://arxiv.org/abs/2607.26228)
*Siqi Zeng,Sewoong Lee,Han Zhao,Julia Hockenmaier*

Main category: cs.CL

TL;DR: V-Steer是一种无需训练的推理时方法，通过编辑缓存的值向量恢复语言模型中的指令层次优先级，显著提升主要约束准确性。


<details>
  <summary>Details</summary>
Motivation: 前沿语言模型常违反指令层次结构（如系统提示应优先于用户输入），现有方法效果有限。

Method: 基于直接logit归因识别主导低优先级跨度的高层头，通过原位乘法编辑缓存V张量提升高级跨度并抑制冲突低优先级跨度。

Result: 在7B到70B模型上，主要约束准确性从低于18%提升至92%，在指令层次评估中优于仅提示基线，匹配或超越基于训练的SOTA方法，解码速度开销可忽略。

Conclusion: V-Steer有效且高效地恢复指令层次，兼容融合注意力后端，仅增加一次预填充开销。
...
### [124] [A large-scale corpus of religious radio broadcast transcripts from webstream recordings in the United States](https://arxiv.org/abs/2607.26249)
*Samuel Bestvater,Athena Chapekis,Skyler Seets,Anna Lieb,Sono Shah,Aaron Smith*

Main category: cs.CL

TL;DR: 本文提供了一个包含70万条录音、6000万行对话的宗教广播语料库，覆盖785个流媒体源，并进行了转录、说话人分离和主题标注。


<details>
  <summary>Details</summary>
Motivation: 美国宗教广播广泛但研究不足，缺乏大规模转录数据限制了内容分析。

Method: 从785个宗教广播流媒体中录制15分钟片段，使用自动化流水线进行转录和说话人分离，利用大语言模型进行格式和主题标注。

Result: 构建了包含700,000+录音和60,000,000+行对话的语料库，组织成元数据表、录音表和转录表。

Conclusion: 该语料库支持宗教广播的区域与宗派研究、社会政治议题分析及语音处理研究。
...
### [125] [Robostreet Flow: A Lightweight, Ultra-Low-Drag Electric Tractor and Four-Truck Hybrid Convoy Architecture for Minimum-Cost Point-to-Point Freight](https://arxiv.org/abs/2607.26250)
*Wei Wang,Yiru Veronika Wang,Sumukh Veeramalla,Xiaohui Liang,for the Robostreet Research Team*

Main category: cs.CL

TL;DR: 本文提出Robostreet Flow货运架构，通过优化车辆设计、编队行驶和运营模式，实现每吨英里成本降低56%。


<details>
  <summary>Details</summary>
Motivation: 传统卡车运输成本中能源、司机和设备占比相近，而现有效率技术仅单一优化某方面，缺乏系统解决方案。

Method: 采用电池电动6x4牵引车（风阻系数0.35，重量降低50%）、513kWh电池+340kWh挂车电池实现500英里续航；四车编队（头车有安全驾驶员，后三车L4自动驾驶），通过CFD模拟8米跟车距离降低后车阻力42-48%。

Result: 编队能耗1.27kWh/英里（较单辆车降20.5%）；电力成本仅为柴油的17%；每吨英里成本从9.4降至4.1美分（降56%）。

Conclusion: 协同设计车辆、编队和运营模式可显著降低卡车运输成本，并提供了敏感性分析和监管影响讨论。
...
### [126] [Evaluating Prompt Scope and Demonstration Similarity in Local LLM Machine Translation](https://arxiv.org/abs/2607.26286)
*Mihael Arcan*

Main category: cs.CL

TL;DR: 论文系统评估了提示范围（单目标 vs. 语系范围）和示例选择策略（随机、词汇相似、嵌入相似）对本地LLM翻译的影响，发现专业MT系统仍占优，少样本对部分模型有效，语系范围提示虽可行但小模型有输出结构问题。


<details>
  <summary>Details</summary>
Motivation: 用户实际使用LLM翻译时可能请求多种目标语言或基于示例翻译，而现有评估多限于单句子单语言提示，缺乏对提示范围和示例选择的系统研究。

Method: 在FLORES devtest数据集上对英语到9种罗曼语和日耳曼语进行翻译，比较3个本地LLM（llama3.2:3b, mistral:latest, qwen2.5:14b）与OPUS-MT和NLLB-200基线，采用零样本、k=5少样本（随机/词汇相似/嵌入相似示例）以及单目标与JSON格式语系范围提示。

Result: 专业MT系统整体最强，尤其日耳曼语；少样本提升mistral和qwen但降低llama3.2；嵌入检索平均最优但优势有限；语系范围提示较强大模型可行但小模型出现结构化输出失败。

Conclusion: 评估LLM翻译时应考虑提示范围、检索策略和多目标合规性，而不只是语言对和度量指标。
...
### [127] [AgentGUI: An Interface for Observing and Steering Long-Running AI Agents](https://arxiv.org/abs/2607.26300)
*Xuan Zhao,Jiwoong Sohn,Qinyue Zheng,Michael Moor*

Main category: cs.CL

TL;DR: 提出了AgentGUI，一个本地托管的图形用户界面，用于在多个并发的长时间运行会话中观察和操控AI代理，支持丰富的轨迹可视化、手动和自动转向以及与开源和前沿代理框架的集成。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理自主能力的快速增强，由于以人为中心的接口有限，人工监督系统性地滞后，因此需要更好的交互界面。

Method: 通过用户控制研究和初步实验评估AgentGUI的效果，前者测量用户识别关键元素的用时，后者测试自动漂移预防功能对任务完成率的影响。

Result: 用户研究显示识别关键元素的速度提高38%（p=0.023）；初步实验中，自动漂移预防功能将小型本地代理的任务完成率最高提升34个百分点（0.8B-9B模型，每模型50次运行）。

Conclusion: AgentGUI能有效提升用户监督AI代理的效率和代理的任务完成率，是解决人机交互滞后问题的有前途方案。
...
### [128] [When Synthetic Users Fail: A Cross-Domain Benchmark of LLM-Simulated Human Survey Responses](https://arxiv.org/abs/2607.26348)
*Zihan Chen,Di Zhu,Lei Nico Zheng*

Main category: cs.CL

TL;DR: 本文评估了LLM作为合成用户的可靠性。在两种大型社会调查数据上，所有测试模型在个体层面均弱于最佳基线，且过于依赖人口统计学信息。模型会放大群体差异，导致错误决策。能力更强的模型无法解决该问题。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多被用作合成用户，但其有效性尚未被系统评估。本文旨在明确LLM何时有效、何时失效。

Method: 在GSS和WVS两个独立领域的人类数据上，使用统一的评估框架，测试了涵盖不同能力和系列的4种LLM，并与多种非LLM基线对比。

Result: 1. 个体层面上，无LLM能超越最佳基线。2. 模型系统性高估人口统计学的预测能力。3. 在决策影响分析中，模型将群体间差异放大2-4倍，并在半数以上案例中引导团队选择错误群体。

Conclusion: 当前LLM作为合成用户存在根本性缺陷，且无法通过提升模型规模解决。需预先评估合成用户证据的安全性，方可用于决策支持。
...
### [129] [Symphony of Bias: Exploring Gender Associations with Musical Instruments in Multimodal LLMs](https://arxiv.org/abs/2607.26355)
*Farhan Farsi,Shayan Bali,Mohammad Heydari Rad,Negar Heidary,Donya Rooein*

Main category: cs.CL

TL;DR: 本文通过构建多模态数据集Symphony-Bias，评估了10个大模型在22种乐器与性别关联上的偏见，发现92%的结果符合社会刻板印象，且文本模态最强，音频最弱。


<details>
  <summary>Details</summary>
Motivation: LLMs广泛应用于信息检索，可能加剧社会偏见和刻板印象，需量化评估其性别偏见。

Method: 构建并行多模态数据集（文本/图像/音频），测试10个多模态模型在22种乐器上的性别关联，分析模型输出与先验社会性别刻板印象的一致性。

Result: 92%的乐器-性别关联符合社会刻板印象；竖琴和鼓的关联最一致；音频模态偏见最弱，文本最强。

Conclusion: 多模态模型在乐器-性别关联上普遍存在偏见，且不同模态的偏见强度不同，需关注文本模态对刻板印象的强化作用。
...
TL;DR: 本文研究了金融披露文本中细粒度不一致性分类问题，比较了多种方法，发现紧凑监督编码器在效率上具有优势，且证据定位质量是关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 金融披露中存在多种类型的不一致信息，检测冲突后还需区分类型以支持后续审查流程。

Method: 基于SBID-FD数据集的5940条实例，比较了冻结嵌入分类器、微调编码器、证据增强分类器、提示大语言模型和LoRA适应生成模型。

Result: 微调300M编码器准确率61.9%，LoRA适应Qwen3.5-9B为61.5%，GPT-5.4为61.3%；提供黄金证据时编码器提升至65.3%。

Conclusion: 紧凑监督编码器在效率上具有优势；证据定位质量和类型区分能力是未来提升的关键方向。
...
### [131] [(Im)Paired Programming: Coding Agents Improve Productivity but Harm Understanding](https://arxiv.org/abs/2607.26375)
*Nishant Balepur,Connor Baumler,Valerie Chen,Eunsol Choi,Rachel Rudinger,Jordan Lee Boyd-Graber*

Main category: cs.CL

TL;DR: 研究发现编程助手虽然能提升任务完成效率，但会降低用户的代码理解能力，导致用户无法自主扩展代码；用户即便意识到理解不足，仍偏好编程助手。


<details>
  <summary>Details</summary>
Motivation: 验证从写代码转为提示和审查代码可能损害用户对代码的理解，进而影响监督、学习和沟通。

Method: 54名学生用两种AI系统（编辑用户代码的助手或提供建议的聊天机器人）创建网站，通过理解测试和扩展代码任务评估效果。

Result: (1) 助手助力初始任务但损害理解，用户难以扩展代码；(2) 低努力交互（如复制粘贴、自动接受修改）与低理解相关；(3) 用户虽自感理解弱，仍偏爱助手的便捷。

Conclusion: 编程助手应减少低努力提示、生成可读代码、促进主动参与，以平衡效率与理解。
...
### [132] [Misalignment Has a Personality: A Big Five Account of Emergent Misalignment](https://arxiv.org/abs/2607.26389)
*Hasibur Rahman,Smit Desai*

Main category: cs.CL

TL;DR: 通过Big Five人格向量（三水平干预提取），发现微调中安全数据导致的广泛偏差可归因为人格转变（低宜人性、低尽责性、高外向性、高神经质），并在一系列实验中验证了该发现。


<details>
  <summary>Details</summary>
Motivation: 解释语言模型微调中，包含如不安全代码等狭窄缺陷的数据如何导致广泛的不对齐现象。

Method: 使用三水平干预提取大五人格向量，在两个开放权重模型上验证；分析微调前后模型的激活与生成文本。

Result: 不对齐数据在所有八个领域共享相同的大五人格特征（低宜人性与尽责性，高外向性与神经质）；微调使模型沿该方向转变（激活相关r=0.69，文本判断r=0.90）。

Conclusion: 经过校准的人格向量可将模糊的安全问题转化为可读的诊断轮廓，并为理解微调导致的不对齐提供可解释机制。
...
### [133] [Knowledge before Reasoning: EC-Reason-Bench, a Training-Free Diagnostic Benchmark for LLM Enzyme Classification](https://arxiv.org/abs/2607.26397)
*Linyu Li,Zhi Jin,Yichi Zhang,Dongming Jin,Yuanpeng He,Huanyao Zhang,Xuan Zhang,Gadeng Luosang,Nyima Tashi*

Main category: cs.CL

TL;DR: 本文提出一个无需训练的评估协议EC-Reason-Bench，用于分析通用大语言模型在EC编号预测任务中表现几乎为零的原因，并探索在不更新权重的情况下能恢复多少性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准显示通用LLM在粗粒度预测中表现良好，但在完整EC编号预测中准确率骤降至几乎为零，而专用模型仍可用，这促使研究者探究背后的原因。

Method: 将酶分类能力分解为输出结构、外部知识、推理结构和推理鲁棒性四个正交杠杆，分别通过推理时方法测试，并与零样本基线对比。

Result: 外部知识至关重要且必须先于推理；闭卷设置下级联和思维链的效果取决于模型不回答的倾向；最佳LLM设置与检索近邻投票结果在平均分数上无显著差异，但掩盖了在对抗证据和多功能酶上的巨大波动；准确率遵循同源可用性定律。

Conclusion: 推理的作用是仲裁矛盾近邻而非提供知识，单一数值榜单无法体现这一点；外部知识是性能提升的关键。
...
### [134] [Voice Memory for Agentic Speech Recognition](https://arxiv.org/abs/2607.26410)
*Chao-Han Huck Yang,Zih-Ching Chen,Piotr Zelasko,Zhehuai Chen,Jagadeesh Balam,Boris Ginsburg*

Main category: cs.CL

TL;DR: 提出Voice Memory，一种推理时方案，通过冻结校正器读取单域记忆文件，结合异步优化器编辑，实现无需参数更新的语音识别，加权词错误率从8.36%降至7.52%。


<details>
  <summary>Details</summary>
Motivation: 传统生成式错误校正（GER）会过度修正，最多破坏64%的正确词元（金融新闻场景），需设计一种受约束的校正机制。

Method: 采用监听-思考架构：冻结校正器推理时读取记忆并决定是否修正，异步优化器通过基于分数的有界编辑更新记忆，仅接受严格提升分数的修改。

Result: 在10个HyPoradise领域将WER从8.36%降至7.52%（加3个上下文示例降至7.47%），航空命令域WER从8.40%降至3.40%，CHiME-4从12.69%降至10.46%。

Conclusion: 约束校正比无监督GER更有效（过修正率从64%降至35%），记忆可跨校正器家族迁移，且推理路径参数为零增加。
...
### [135] [ForgetBench: Benchmarking Forgetting Dynamics of Long-Term Parametric Memory in Language Models](https://arxiv.org/abs/2607.26455)
*Ruxi Gu,Zhenliang Zhang,Wei Wang*

Main category: cs.CL

TL;DR: 提出了ForgetBench基准，系统评估大语言模型在持续知识编辑下的遗忘行为，发现现有方法无法平衡长期保留与泛化质量。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅关注单步推理或静态知识编辑，忽略了知识在持续更新中的时间动态和退化过程。

Method: 设计了基于概念问答和情景问答两种互补范式，构建时序知识流，并提出统一评估框架建模知识演化，测量时间衰减、保留强度和跨实例稳定性。

Result: 实验表明现有方法无法在长期保留与泛化质量间取得平衡。

Conclusion: 需要更鲁棒的记忆机制来在时间上有效获取、更新和保留知识。
...
### [136] [Mergeable Model-Side Aggregation States for Long-Context Language Models](https://arxiv.org/abs/2607.26448)
*Dachuan Song,Junyu Yin,Zechen Hu,Xuan Wang*

Main category: cs.CL

TL;DR: 提出一种基于HyperLogLog（HLL）草图的状态聚合接口，固定2 KiB内存，在长上下文语言模型中实现精确且可扩展的集合聚合，无需额外生成-执行-返回循环。


<details>
  <summary>Details</summary>
Motivation: 长上下文语言模型在处理非加性、基于集合的聚合任务（如基数估计、集合关系等）时，随着上下文长度增加性能不可靠，而现有方法需要额外的生成-执行-返回循环，效率低下。

Method: 冻结语言模型，维护紧凑的Hash-based HLL草图状态。通过提取器将记录映射为规范身份并哈希更新HLL状态，状态可跨段落合并或直接读出用于下游推理。

Result: 在百万记录实验中，平均相对误差1.6%；在256段落合并测试中，读取结果与单次遍历完全一致；在3,969个聚合-推理任务中，Gemma 4达到99.2%准确率，相比精确聚合仅差0.8个百分点，比直接推理和CoT推理分别提升56.3和63.2个点。

Conclusion: 固定预算的HLL草图接口能有效解决长上下文中集合聚合的不可靠问题，保持高精度且不随上下文长度或集合规模增长，具有实用性和可扩展性。
...
### [137] [CMT-RAG: Complementary Memory Traces for Multi-turn Multi-hop RAG](https://arxiv.org/abs/2607.26470)
*Lang Zhou,Yingjian Chen,Shuxuan Li,Kun-Yu Lin,Zhilin Zhao*

Main category: cs.CL

TL;DR: 提出了CMT-RAG框架，通过将对话记忆表示为子问题级推理轨迹，解决了多轮多跳RAG中的长期依赖追踪问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统难以从原始对话历史中恢复先前推理步骤和证据，阻碍了多轮查询的准确回答。

Method: CMT-RAG使用状态空间轨迹生成器将当前查询分解为带依赖的检索导向子问题，通过检索证据验证后存储为持久化有向无环图记忆。

Result: 在MuMu-QA和语料级RAG基准测试中，CMT-RAG在答案准确性上持续优于五类基线方法。

Conclusion: 用结构化推理轨迹对齐对话记忆与检索机制，能有效提升多轮多跳RAG的长期依赖处理能力。
...
### [138] [Where Detectors Fail: Closing the Tail-Domain Gap with Expert-Guided Mutual Distillation](https://arxiv.org/abs/2607.26555)
*Xuan Feng,Guihong Liu,Tianlong Gu,Shuai Zhao,Xuemin Wang,Chenzhong Bin,Yang Liu,Bo An*

Main category: cs.CL

TL;DR: 提出专家引导的相互蒸馏方法（EGMD），通过输入级校准、表示级教师指导和决策级学生蒸馏，减少多模态假新闻检测中的域偏见，在4个数据集上降低域偏见最高57.3%。


<details>
  <summary>Details</summary>
Motivation: 多模态假新闻检测器常因学习不可靠的证据（如域特定捷径、不平衡数据、语义不一致的图文对）而在跨域泛化上表现不佳。

Method: EGMD包括：输入级校准（编码图文对一致性作为共享增益）、表示级专家引导教师（对齐域统计，聚集域模式）、决策级原型锚定域学生（相互学习与双通道蒸馏）。

Result: 在4个数据集（含新构建的平衡基准Weibo_Balanced）上，EGMD实现最先进精度，域偏见降低高达57.3%。

Conclusion: 通过多层级信任学习机制，EGMD有效抑制了域特定不可靠证据，显著提升跨域泛化能力。
...
### [139] [Which RAG Paradigm Wins at Scale? A Scaling Study of Retrieval-Augmented Generation Paradigms](https://arxiv.org/abs/2607.26497)
*Pengyu Wang,Benfeng Xu,Shaohan Wang,Xin Zeng,Huarui Wu,Lei Zhang,Licheng Zhang*

Main category: cs.CL

TL;DR: 本研究通过控制语料库规模，系统比较了四种检索增强生成范式的精度-成本缩放特性。结果表明，BM25在规模化时表现最佳，在大多数规模下以低成本实现高精度；基于文件的智能体在小规模时略优但消耗大量令牌；图式RAG在扩展性上受限且精度较低。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在不同基准和单一语料规模下评估，缺乏对精度-成本缩放特性的理解，本研究旨在填补这一空白。

Method: 构建28个严格嵌套规模的语料库（从约1000到512,000文档），在固定问题和相关/对抗文档下，使用统一阅读器和评判协议，测量官方精度、构建和查询令牌数及延迟。

Result: BM25在帕累托前沿定义最佳精度-成本平衡，从中等规模起领先精度；File-System Agent在小规模略好但成本高且大时精度大幅下降；Agent+BM25结合后提升显著；Graph-based RAG在构建时消耗大量资源且扩展性差。

Conclusion: 在受控语料规模缩放下，BM25作为非LLM方法具有最优的精度-成本缩放特性，而复杂方法需在特定条件下优势才能体现。
...
### [140] [WikiLoop: Jointly Learning to Build and Navigate Agent-Native Wikis with Downstream Feedback](https://arxiv.org/abs/2607.26604)
*Haoliang Ming,Feifei Li,Wenhui Que*

Main category: cs.CL

TL;DR: 提出了一个反馈耦合框架WikiLoop，联合学习构建和导航智能体原生的维基知识库，将知识库构建与下游查询性能直接关联。


<details>
  <summary>Details</summary>
Motivation: 知识库构建和查询通常被孤立优化，检索增强智能体依赖于固定外部索引，而构建过程缺乏下游查询的反馈信号。

Method: 采用角色条件共享策略，包含Navigator和Builder两个接口。Navigator遵循“先充分再高效”目标，Builder通过效用差异学习，结合守卫惩罚防止退步。训练包括顺序角色优化和最终联合阶段。

Result: 在AuthTrace上达到62.6聚合答案正确率，比LLM-Wiki基线高6.3分，多文档查询提升最大。控制实验验证了设计目标的有效性，编辑对独立Navigator有用。联合策略保留了角色能力，端到端答案正确率比专家模型高0.4分。

Conclusion: WikiLoop通过联合学习和反馈耦合显著提升了知识库导航性能，且无需特定数据集训练即可泛化到其他问答基准。
...
### [141] [Revisiting Lossy Verification in Speculative Decoding: Mechanisms, Trade-offs, and Failure Modes](https://arxiv.org/abs/2607.26627)
*Tianyu Wang,Yuxuan Zhou,Wenbin Wang,Heng Li,Zikai Xiao,Junyuan Shang*

Main category: cs.CL

TL;DR: 本文系统分析了有损验证的推测解码方法，揭示了其对生成分布的隐性改写，并通过诊断框架指出截断式验证存在分布失真风险，合作式验证需控制草案概率过冲以保证质量。


<details>
  <summary>Details</summary>
Motivation: 现有有损验证方法在加速推理的同时，因放松严格分布匹配而可能产生不稳定甚至严重退化的生成质量。

Method: 对有损验证方法进行理论分析，分类为截断式验证和合作式验证；构建诊断评估框架，识别截断式验证的分布失真问题及合作式验证的关键原则。

Result: 截断式验证的性能可能显著劣于真实截断采样基线；合作式验证需确保草案概率不高于目标概率以避免低质量输出。

Conclusion: 有损验证方法的加速效果需谨慎权衡生成质量，并遵循特定原则（如控制概率过冲）来避免退化。
...
### [142] [Filesystem-Based Memory for LLM Agents: Organization, Evolution, and Sustainability](https://arxiv.org/abs/2607.26637)
*Sizhe Zhou,Sheldon Yu,Hui Wei,Junda Wu,Siru Ouyang,Yizhu Jiao,Shijia Pan,Julian McAuley,Yu Zhang,Tong Yu,Jiawei Han*

Main category: cs.CL

TL;DR: 本文首次系统探索了基于文件系统的LLM智能体长期记忆，揭示了组织化存储主要带来搜索经济性，但当前智能体未能将组织转化为更好的答案质量，且工具集对存储形态的影响与模型替换相当。


<details>
  <summary>Details</summary>
Motivation: 现有研究多设计定制化记忆系统，忽略了广泛使用的文件系统默认方案的两个关键假设未经检验：智能体能否有效组织增长中的记忆，以及这种组织是否带来实际收益。

Method: 形式化三角色（管理、搜索、执行智能体）围绕单一文件系统协同工作，在长对话和具身任务基准上，通过变化记忆形态、流规模、工具集和智能体能力进行系统性实验。

Result: 组织化存储约降低半数检索成本；但随记忆增长，除最强管理智能体外的组织性均退化；组织未能提升答案质量；工具集对存储形态的影响堪比模型更换。

Conclusion: 将文件系统默认从假设转变为可设计的记忆空间，强调工具集和智能体协调能力的关键作用。
...
### [143] [Contrastive ESA: Human Evaluation of Multiple Translations at Once](https://arxiv.org/abs/2607.26640)
...
TL;DR: 提出对比错误跨度标注（cESA）协议，通过同时展示多个翻译版本降低标注噪声和成本，并产生绝对质量评分。


<details>
  <summary>Details</summary>
Motivation: 传统单输出评估方法存在标注噪声高和成本高的问题。

Method: cESA：呈现源输入的多个翻译版本，标注错误跨度，并给出0%-100%绝对评分。

Result: 在大规模英->日翻译评估中，cESA显著减少标注时间和噪声，且无需后续校正即可得到可解释的非参数模型排名。

Conclusion: cESA是一种更高效、更一致的机器翻译评估方法，能产生绝对质量判断。
...
### [144] [Constitutional Midtraining: Content Presence Drives Alignment Gains](https://arxiv.org/abs/2607.26654)
*Desiree Cho,Cameron Tice,Bernie Hogan,Hunar Batra,Puria Radmard,Jun Zhao,Nigel Shadbolt*

Main category: cs.CL

TL;DR: 中期训练中插入宪政内容可以产生持久且稳健的对齐效果，优于仅基于SFT的管道，且不损害模型能力。


<details>
  <summary>Details</summary>
Motivation: 检测中期训练干预（与后期训练隔离）能否产生持久对齐。

Method: 在120B规模下，使用394M词元的宪政语料（基于Anthropic宪法），通过2x2析因设计（课程顺序×审议推理）进行宪法中期训练，并与重放控制对比。

Result: 宪法中期训练模型在对齐泛化和持久性上优于控制，尤其在勒索任务中优势持续存在（-17.5pp），但不扩展到需主动抵抗上下文压力或冲突的场景。能力测试（MMLU、ARC-Easy、piqa、GSM8K）无显著成本。

Conclusion: 适度的中期宪政内容可产生广泛、持久的对齐收益，是SFT中心管道的廉价补充。
...
### [145] [Automated Multilabel Mpox Research Classification with Explainable Transformer Models](https://arxiv.org/abs/2607.26700)
*Tanjim Taharat Aurpa*

Main category: cs.CL

TL;DR: 本研究利用多标签分类对14590篇Mpox研究文章进行主题分类，BERT模型表现最佳，准确率97.05%，F1分数97.67%（微平均）和96.46%（宏平均），并通过SHAP分析增强了模型可解释性。


<details>
  <summary>Details</summary>
Motivation: Mpox疫情持续威胁公共卫生，大量研究文献难以高效组织与分析，亟需自动化分类工具来辅助科研人员、政策制定者和医疗工作者快速获取关键信息。

Method: 采用多标签分类方法，测试多种AI模型，其中BERT表现最优；使用SHAP进行特征重要性分析，解读模型决策机制。

Result: BERT在14590篇文章上达到97.05%准确率、97.67%微平均F1和96.46%宏平均F1，SHAP揭示了关键词语特征。

Conclusion: BERT可有效自动化分类Mpox研究，节省人力并提升公共卫生应对效率，为信息检索和决策支持提供了实用工具。
...
### [146] [AtmosERC: Modeling Dialogue-Level Affective Atmosphere for Emotion Recognition in Conversation](https://arxiv.org/abs/2607.26726)
*Weijie Feng,Tongwei Zhang,Binbin Liu,Zhiyong Cheng*

Main category: cs.CL

TL;DR: 提出AtmosERC框架，通过图结构建模对话级情感氛围，提升轻量级和基于LLM的对话情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有上下文建模方法忽略了全局上下文中的异质性，并非所有上下文信息都与情感预测相关，因此需要聚焦情感导向的信号。

Method: 构建对话级情感氛围图，使用时序感知图提取器过滤异构信号，生成对话级和说话人条件化的情感先验，并通过轻量级序列预测或提示级线索集成到LLM中。

Result: 在四个基准数据集上，AtmosERC提升了轻量级ERC的性能，作为插件增强了LLM-based ERC，并在局部情感偏差下保持更稳定的预测。

Conclusion: 通过显式建模情感氛围，可更有效地利用上下文信号，提升情感识别鲁棒性与泛化能力。
...
### [147] [Phoneme- vs. Character-Level Targets and Selective State-Space Models for Intracortical Brain-to-Text](https://arxiv.org/abs/2607.26751)
*Lucas Zamora Vera,Jose A. Gonzalez-Lopez*

Main category: cs.CL

TL;DR: 本文在公开的脑-文本数据集上比较了GRU和Mamba混合解码器在音素和字符两种输出目标下的性能。


<details>
  <summary>Details</summary>
Motivation: 探索选择性状态空间模型（Mamba）是否优于循环解码器，以及输出目标（音素vs字符）如何影响解码性能。

Method: 使用2x2实验设计（GRU vs Mamba混合解码器；音素vs字符目标），采用CTC损失函数，在统一的可复现协议下训练。

Result: GRU循环解码器仍是最强的：音素GRU达到12.62% PER和21.19% WER；字符GRU经语言模型重打分后达到13.39% CER和26.28% WER。Mamba混合解码器未超越GRU。

Conclusion: 循环解码器在脑-文本任务中仍占优势，输出目标不同会导致不同类型的错误（音素混淆vs词汇边界错误）。
...
### [148] [Metis: Memory Foundation Model](https://arxiv.org/abs/2607.26760)
...
TL;DR: 提出记忆基础模型，赋予基础模型原生记忆能力，并提出首个原型Metis，通过架构和训练实现记忆的持久化与自主操作。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理的记忆主要依赖外部模块，而基础模型的原生记忆能力未被充分探索。

Method: 形式化原生记忆为持久动态记忆状态和自主存储/利用过程；设计Metis架构，通过记忆注意力压缩历史信息，并用大规模记忆数据和多目标优化进行中训练。

Result: Metis展现出原生记忆能力，记忆更新无需梯度，推理时权重冻结，记忆状态通过前向计算自主转换。

Conclusion: 记忆基础模型在架构、端到端优化和效率上具有优势，Metis是有效原型，研究提供了详细分析和开源资源。
...
### [149] [Relation Geometry in Semantic Space of Language Models](https://arxiv.org/abs/2607.26762)
*Zhihan Cao,Hiroaki Yamada,Simone Teufel,Tatsuya Hiraoka,Kentaro Inui,Hitomi Yanaka,Takenobu Tokunaga*

Main category: cs.CL

TL;DR: 本文研究了语言模型语义空间中语义关系的几何表示，发现非对称关系中关系词占据独特区域，但关系属性编码中等；因果模型更依赖词法信息，掩码和扩散模型更依赖上下文。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型生成的语义空间中，语义关系知识的几何表示程度，特别是不同关系（如对称性、非对称性）在空间中的表现差异。

Method: 从三方面分析：关系词区域分布、关系属性（对称/非对称/传递性）编码、输入信息（词形/上下文）影响；使用因果、掩码和扩散语言模型对六种语义关系进行实验。

Result: 非对称关系中关系词占据不同区域；关系属性编码中等但优于对称关系；因果模型依赖词法，掩码和扩散模型依赖上下文。

Conclusion: 语义关系在语义空间中的几何表示不均衡，表明仅从分布信息学习语义关系存在差异。
...
### [150] [Enhancing Generative Information Extraction with Two-step Validation: A Product Attribute Use Case](https://arxiv.org/abs/2607.26780)
*Yi-Sheng Hsu,Nermeen Abou Baker,Uwe Handmann*

Main category: cs.CL

TL;DR: 本文提出一种两阶段验证方法，将预训练语言模型（PLM）集成到生成式信息提取（IE）流程中，利用大语言模型（LLMs）的纠错能力，在数字产品护照（DPP）等数据稀缺领域提升了实体提取性能，尤其是对低频弱表达实体的提取。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在信息提取中展现泛化优势，但针对数字产品护照这种术语空间广但标注数据稀缺的场景，现有方法在效率、泛化性和数据隐私方面存在局限。

Method: 提出两阶段验证方法：第一阶段使用PLM进行初步预测，第二阶段由LLM对预测结果进行纠错与验证，从而增强IE管道的性能。

Result: 验证任务提升了LLM提取弱表达、低频实体的能力；中型模型在某些实体上达到与大型模型相当的性能；PLM的改进也提升了最终LLM输出；但对最小开源模型（如Llama-3.2 3B）提升有限。

Conclusion: 两阶段验证方法可有效增强生成式IE在数据稀缺领域的效果，并为DPP实际应用中的本地化LLM部署提供了可行的demo方案。
...
### [151] [When Does Span-Guided Detoxification Help? Human Preferences and Evaluator Diagnostics in a Controlled Comparison](https://arxiv.org/abs/2607.26795)
*Kyungwon Park*

Main category: cs.CL

TL;DR: 在固定单生成器设置下，对span引导和非引导去毒化进行了受控比较，发现人类偏好存在权衡：span引导在保留原意时受青睐，非引导在完全缓解毒性时更优，且差异随毒性强度变化。


<details>
  <summary>Details</summary>
Motivation: span引导改写虽能局部编辑有害片段以保留意义，但可能无法充分缓解有害意图，促使研究对比两种策略的实际效果。

Method: 混合来源英语评估集（人工输入和HateXplain测试项），采用密集盲人评估，在固定单生成器下比较span引导与非引导去毒化，并用自动评估（毒性-相似性标量、多生成器分析、通用LLM评判）作为诊断工具。

Result: span引导在强毒性层有竞争力，非引导在中性层明显更优；自动评估能再现部分总体趋势，但无法捕捉分层对比。

Conclusion: 不存在基于严重性的通用规则，应分别评估缓解充分性和意义保留，并同时报告残余伤害和过度修改。
...
### [152] [From Found to Designed: Concepts as a Design Axis for Large Language Models](https://arxiv.org/abs/2607.26825)
*Chen Shani*

Main category: cs.CL

TL;DR: 论文提出应将概念视为大语言模型的设计轴，并映射了沿两个维度的设计空间：引入概念结构的流水线阶段（训练目标、核心架构、推理或事后解释），以及结构是内部派生的还是外部基础的。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过分布统计关联隐式编码概念信息，缺乏显式结构化、组合性的概念表示，导致概念级结构不稳定、不可控且与人类分类不匹配。

Method: 按流水线阶段（训练目标、核心架构、推理、事后解释）和结构来源（内部派生或外部基础）两个维度分类现有方法，形成设计空间。

Result: 识别三种模式：推理时间方法探索不足、相关思想在流水线阶段孤立发展、外部基础方法贯穿全流水线但术语不一。

Conclusion: 推动从恢复训练后模型的概念结构转向显式设计含概念表示的大语言模型。
...
### [153] [Language Models are not Equally Robust to Non-Canonical Tokenization across Languages](https://arxiv.org/abs/2607.26831)
*Poulami Ghosh,Preethi Jyothi*

Main category: cs.CL

TL;DR: 研究发现，语言模型对非标准分词（tokenization）的鲁棒性并非普遍成立，而是高度依赖于语言及其与分词器的交互方式，在多语言场景下模型性能下降显著。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型通常依赖单一标准分词序列，忽略了理论上指数级多的有效分词空间；先前英文研究表明模型对替代分词具有不变性，但该结论是否适用于其他语言尚不清楚。

Method: 对27种涵盖不同文字系统的语言进行多语言研究，在六项下游任务中评估大语言模型（Llama-3.1-8B、Qwen3-8B、Gemma-3-12B等）在非标准分词下的行为，并通过LoRA微调引入多分词训练数据来缓解敏感性问题。

Result: 分词不变性不具泛化性：指令微调模型在非标准分词下平均相对性能下降显著（Llama-3.1-8B：23.7%，Qwen3-8B：11.4%，Gemma-3-12B：9.9%）；语言碎片化程度越高，对非标准分词越敏感。

Conclusion: 分词鲁棒性不是语言模型的通用属性，强烈依赖于语言与分词器的交互；通过多分词训练数据进行LoRA微调能有效缓解敏感性问题，其中系统采样多样化非标准分词方案效果最佳。
...
### [154] [From Representations to Behaviors: Exploring the Person-Situation-Behavior Triad in LLMs](https://arxiv.org/abs/2607.26853)
*Ruikang Zhang,Shuo Wang,Qi Su*

Main category: cs.CL

TL;DR: 该研究基于Funder的人格三元框架，通过稀疏自编码器分解大语言模型中的内部特征，发现、控制并验证了类似人格特质的表征，证明这些表征在不同情境下能稳定影响行为，并在社会智能任务中产生类似人类的人格相关收益权衡模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注大语言模型在人格条件化下的行为输出，缺乏对内部人格相关表征及其跨情境表达和具体行为塑造机制的实证证据。

Method: 采用Funder的人格三元框架，通过对比行为对在共享情境下识别稀疏内部特征（SAE分解），验证特质相关性；通过特征级干预实现双向特质转变并评估情境泛化性；将干预应用于社会智能任务以获取行为验证。

Result: 识别出与特质极相关的稀疏内部特征，干预能双向改变模型在多样情境中的特质相关行为，并保持响应有效性；在社会智能任务中引出与人格心理学一致的收益权衡模式。
...
### [155] [DIRECT: Direct Decoding for Efficient and Aligned Sequence Labeling with Large Language Models](https://arxiv.org/abs/2607.26891)
*Yilei Wang,Jiaxin Gan,Kexuan Zhang,Ling Li,Wentao Zhang,Peichao Lai*

Main category: cs.CL

TL;DR: 提出了DIRECT框架，通过训练时的直接偏好优化和推理时的受控解码，显著提升了序列标注任务的领域对齐和推理效率。
...
Method: 采用监督微调后的直接偏好优化（DPO）增强任务对齐，并引入受控解码过程（固定输出格式、限制预测候选项）；通过模板填充机制（仅生成标签令牌，重用前缀内容使用KV缓存）提升效率。

Result: 在八个数据集上，DIRECT在性能和效率方面均显著优于现有方法。

Conclusion: DIRECT通过训练优化和推理修正，有效解决了序列标注中的领域对齐和效率问题。
...
### [156] [SERPO: Self-Evolving Rubric Policy Optimization for Open-Ended Test-Time Reinforcement Learning](https://arxiv.org/abs/2607.26873)
*Jianze Wang,Kunwang Zheng,Ying Liu,Yu Cao,Qilong Zhang,Jinlong Chen,Hua Yang,Qianglong Chen*

Main category: cs.CL

TL;DR: 提出SERPO方法，通过协同进化响应证据、查询特定评分标准和策略参数，实现了无需标注反馈的测试时强化学习，在开放生成任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有测试时强化学习方法依赖答案投票，无法应用于开放生成任务（如对话、创作），因为有效响应无法映射到统一的标准答案，且缺乏外部奖励模型。

Method: 构建闭环进化系统：1) 好-正常-差响应进化将最大分离的响应组织为排序档案；2) 评分标准进化保留能区分这些档案的标准；3) 概率标准评分将判决标记概率转化为奖励信号；4) 策略进化用奖励信号优化生成器，新生成刷新档案和评分标准。

Result: 在HealthBench和ResearchQA上比基础模型提升20.63和20.31个百分点，六任务宏平均提升8.06个百分点，支持跨领域迁移和持续进化。

Conclusion: SERPO有效解决了开放生成场景下测试时强化学习的核心问题，通过自进化评分机制显著提升模型质量，且具备良好的泛化性和持续学习能力。
...
### [157] [Dual-Path LLM Reasoning for Multimodal Few-Shot Knowledge Graph Completion](https://arxiv.org/abs/2607.26909)
*Jinlan Liu,Zhiying Tu,Yongchao Xing,Yicheng Liu,Bolin Zhang,Dianbo Sui,Dianhui Chu,Hongliang Sun*

Main category: cs.CL

TL;DR: 提出双路径大语言模型推理框架（DuPLeR），用于多模态小样本知识图谱补全，通过融合多模态大模型先验与事实支持结构构建校准关系图，实现数据稀疏场景下的鲁棒推理。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱补全方法难以处理新兴实体关系、小样本及零样本设置下的归纳推理问题，且多模态信息和LLM先验可能引入噪声或幻觉。

Method: 构建校准关系图（融合多模态LLM类型先验与事实支持结构），执行双层结构推理；设计双路径多模态增强模块，通过查询相关多模态信号调控消息传递并补充图传播后的实体表示。

Result: 在两种多模态知识图谱基准的八种归纳变体上，DuPLeR在数据稀缺场景下取得鲁棒性能。

Conclusion: 多模态信息与LLM先验的合理融合及双路径结构推理可有效提升小样本知识图谱补全效果。
...
### [158] [Latent-IM: Latent Interaction Management for Speech LLMs](https://arxiv.org/abs/2607.26928)
*Adar Avsian,Atahan Dokme,Tony Woo,Larry Heck*

Main category: cs.CL

TL;DR: 本文提出Latent-IM框架，通过内部对话管理恢复大语言模型中的状态估计与动作控制，提升对话动作选择与生成的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统对话系统将对话管理与响应生成分离，但LLM将两者融合在隐表示中，导致对话动作控制不明确。

Method: 将对话动作控制分为动作选择（预测下一动作）与动作实现（在生成时因果产生指定动作），并通过Latent-IM内部管理框架统一实现。

Result: 在端到端动作准确率上比无引导的骨干模型提升12.5个百分点，性能与微调方法相当。

Conclusion: 通过内部对话管理可有效恢复LLM中的动作控制能力，无需额外微调即可接近传统方法的性能。
...
### [159] [Same Evidence, Different Target: Decoding How Diagnostic Evidence Bears on Causal Questions from Language-Model States](https://arxiv.org/abs/2607.26929)
*Weiyi Kong,Zhuoran Li*

Main category: cs.CL

TL;DR: 通过配对提示和线性读出分析，发现大语言模型隐藏状态中线性可解码的诊断证据与因果目标关系信息，平衡准确率约0.654-0.659，可恢复18-21对。


<details>
  <summary>Details</summary>
Motivation: 诊断证据在不同因果目标下支持或挑战的程度差异显著，现有方法易受措辞或模式影响，需检验模型能否匹配证据与因果问题。

Method: 使用配对提示（重复相同证据、改变因果目标），从Transformer最后隐藏层提取最终token状态，基于线性读出分类为Favors/Challenges/Unresolved/Wrong Target。

Result: 在49对基准上平衡准确率0.654-0.659，恢复18-21对；人类标注一致性96.9%；模型性能超过置换空值和输入限制基线，且跨诊断家族泛化。

Conclusion: 隐藏状态包含线性可解码的诊断证据与因果目标关系信息，超越基于答案选项logits的线性分类器及文本基线。
...
### [160] [Credit Cards, Confusion, Computation, and Consequences: What Can We Uncover About Language Model Reasoning?](https://arxiv.org/abs/2607.26952)
*Arnav Hiray,Agam Shah,Caleb Lu,Meghaj Tarte,Harsit Mittal,Sudheer Chava*

Main category: cs.CL

TL;DR: 提出了首个基于真实信用卡协议的金融素养数值推理基准CreditCardQA，包含1800个问题，评估了大语言模型和推理模型的Chain-of-Thought与Program-of-Thought提示方法。


<details>
  <summary>Details</summary>
Motivation: 现有金融素养基准缺乏对真实信用卡协议的数值推理能力评估，且消费者提问方式与标准问题存在差距。

Method: 构建CreditCardQA数据集，使用CoT和PoT提示方法评估多种大语言和推理模型，并进行错误分析和难度分析。

Result: PoT提示方法一致提升模型性能，尤其对基线推理较弱的模型；错误主要源于金融规则误用而非算术错误；比较、条件逻辑和货币约束问题最难；边缘案例（如滞纳金、小余额场景）更易出错。

Conclusion: CreditCardQA揭示了模型在金融规则理解和边缘案例推理上的不足，需要改进对合同术语和特殊场景的处理能力。
...
### [161] [Generation or Judgement? A Paradigm Perspective on LLM-Based Emotion-Cause Pair Extraction in Conversation](https://arxiv.org/abs/2607.26967)
*Weijie Feng,Hongchuang Wang,Binbin Liu,Zhiyong Cheng*

Main category: cs.CL

TL;DR: 论文发现任务粒度显著影响情绪-原因对提取性能，pair级判断优于对话级生成，并提出辅助检索器改进边界案例。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法在情绪-原因对提取中存在任务粒度差异，且性能影响未知，需探究不同粒度的效果差异及原因。

Method: 系统性比较对话级生成与pair级判断的表现，分析遗漏原因，并引入辅助检索器重新检查模糊边界案例。

Result: pair级判断在所有18个对照实验中优于对话级生成；LLM能识别92.7%-98.1%的关系但难以完整发现；辅助检索器在三个数据集上提升F1值0.50-1.46点，推理时间仅为基线1.49倍。

Conclusion: 任务分解和候选范围对有效利用LLM进行情绪-原因对提取至关重要。
...
### [162] [TREK: A Travel Reasoning and Evaluation Kit for LLM Agents in Complex Trip Planning](https://arxiv.org/abs/2607.26977)
*Jinhu Qi,Wentao Zhang,Siu Man Ng,Feiyang Xu,Yanyu Chen,Yaoman Li,Irwin King*

Main category: cs.CL

TL;DR: 提出TREK基准，用于评估LLM代理生成可行旅行计划的能力，发现即使最强模型也仅有46.2%成功率。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法确保旅行计划的可执行性，且评分缺乏可重复性。

Method: 构建包含800个多约束任务的合成知识库，使用确定性规则评估器代替LLM评分。

Result: 最强模型GPT-5.6在可解任务中仅46.2%生成完全可行计划，未明示需求成最大瓶颈。

Conclusion: TREK揭示了LLM在复杂约束联合满足上的显著局限，尤其是理解用户潜在需求方面。
...
### [163] [OptimismBench: Forecasting Bias and the Alignment Effect in Language Model Judgment](https://arxiv.org/abs/2607.26981)
*Seonglae Cho,Adriano Koshiyama*

Main category: cs.CL

TL;DR: 发布 OptimismBench 框架用于检测大语言模型的定向置信偏差，发现多数模型偏向乐观，且该偏差会从对齐训练继承到下游应用。


<details>
  <summary>Details</summary>
Motivation: 现有校准指标无法区分方向性偏差，且自然语言场景缺乏真实概率作为参考，因此需要无真实标签的偏向量化方法。

Method: 构造反向配对问题（如P(成功)与P(失败)），通过不对称性获得带符号的偏差分数，无需真实概率。在16个模型、8个提供商、6种语言上评测。

Result: 14个模型偏差为正（乐观），仅Anthropic的顶级模型展现悲观；后训练决定偏差方向且不同模型家族方向相反；模型差异>语言差异（4.7倍）。

Conclusion: 对齐训练在提升模型有用性的同时会引入概率倾斜，下游流程默认继承该偏差，需通过OptimismBench进行定向审计。
...
### [164] [Evaluating Regional Bias in LLMs From Abstract Stereotype to Concrete Social Decision-Making](https://arxiv.org/abs/2607.27022)
*Jiayuan Di,Haoyi Yang,Yufei Luo,Jiahui Qu,Yiming Wang*

Main category: cs.CL

TL;DR: 本文提出Stereotypes-to-Decisions (S2D)框架，系统评估了大语言模型（LLMs）在中国34个省级行政区中的区域偏见，发现偏见普遍、系统且影响实际决策。


<details>
  <summary>Details</summary>
Motivation: 现有研究孤立审视区域偏见的抽象刻板印象与具体决策表现，缺乏两者间的系统联系和后果分析。

Method: S2D框架包括刻板印象评分（温暖和竞争力维度）和配对选择任务（教育、职业、社会互动），评估六个LLMs在中文和英文提示下的表现。

Result: LLMs存在显著区域差异，模型间一致性较高，尤其是竞争力和职业决策；偏见与区域经济和数字发展指标相关，且呈现混合人型刻板印象。

Conclusion: LLMs的区域偏见普遍、系统且具有实际影响，需加强区域意识评估和缓解措施。
...
### [165] [DenseOn with the LateOn: Fully Open Dense and Late-Interaction Models for Multilingual, Long-Context, and Code Search](https://arxiv.org/abs/2607.27178)
*Raphaël Sourty,Antoine Chaffin,Paulo Roberto Moura Junior,Amélie Chatelain*

Main category: cs.CL

TL;DR: 提出了开放端到端检索模型训练方法，通过翻译训练（translate-train）实现多语言检索，并发布了模型、数据和代码。


<details>
  <summary>Details</summary>
Motivation: 现有检索模型依赖封闭训练数据，导致可重复性差，且多语言检索能力有限。

Method: 构建665M英文对比预训练对和1.88M有监督微调对，训练DenseOn（单向量）和LateOn（后期交互）模型；将英文数据译为8种语言，训练mDenseOn和mLateOn多语言模型。

Result: DenseOn和LateOn在BEIR上分别达到56.20和57.22平均nDCG@10；mDenseOn在翻译语言上表现好但泛化差，mLateOn则能泛化到未见语言。

Conclusion: Token级匹配使翻译训练从目标语言扩展变为多语言泛化方法。
...
### [166] [Pangram 4 Technical Report](https://arxiv.org/abs/2607.27183)
*Ben Glickenhaus,Katherine Thai,Jenna Russell,Elyas Masrour,Yue Han,Max Spero,Bradley Emi*

Main category: cs.CL

TL;DR: 提出了Pangram 4，一种最新的基于深度学习的AI文本分类模型，在多项指标上达到领先水平。


<details>
  <summary>Details</summary>
Motivation: 为了提升AI生成文本检测的准确率、泛化能力和鲁棒性，并改进对细粒度编辑和混合人机合作文本的识别。

Method: 基于深度学习的AI文本分类方法，可能在模型架构或训练策略上有创新。

Result: AUROC达到0.9916，假阳性率0.0041%，假阴性率0.3396%；在对抗攻击、分布外泛化、边界检测及混合文本检测上均优于前代。

Conclusion: Pangram 4在多种场景和领域下的AI文本检测任务中达到了最先进的性能。
...
### [167] [APEX-Accounting](https://arxiv.org/abs/2607.27189)
...
TL;DR: APEX-Accounting是一个评估前沿模型执行会计任务能力的基准测试，包含160个专业设计的任务，目前最佳模型准确率仅56.4%。


<details>
  <summary>Details</summary>
Motivation: 需要评估AI模型在实际会计工作中的表现，填补专业领域基准测试的空白。

Method: 由会计专家设计10个虚拟世界中的160项会计任务（含评分标准），测试9种前沿模型并分析token预算的影响。

Result: Claude-Fable-5以56.4%领先，所有模型Pass^8最高仅2.6%，且出现辛普森悖论——高token任务在预算约束下得分更低。

Conclusion: 当前模型在专业会计任务上表现有限，token预算分配存在效率矛盾，需进一步优化。
...
### [168] [Mental World Modeling](https://arxiv.org/abs/2607.27201)
*Hao Fei,Yiran Zhao*

Main category: cs.CL

TL;DR: 提出心理世界建模（MWM）框架，将心理变量作为世界模型的核心成分，并实现无训练、可检视的基线MENTIS，在跨模态决策场景中验证其必要性。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型仅追踪物理场景，忽略驱动人类行为的隐藏心理状态（信念、意图等），导致预测行为错误。

Method: 构建耦合物理-心理世界状态，生成目标特异性部分观测，通过状态解析、动作分解、耦合转移和分支评估实现心理世界建模。

Result: 在文本、图像、音频视频故事数据集上，8种LLM世界模型实验表明，显式建模心理状态对预测人类决策至关重要。

Conclusion: 心理世界建模是世界建模的下一个阶段，从模拟物理场景转向模拟在其中行动的思维。
...
TL;DR: 本文定义了可信具身智能为在环境与系统变化下持续可靠执行任务且风险可控的能力，并提出“持续安全成功”的目标，通过模型、系统、证据、部署四个相互依赖的层次实现。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注任务完成，未考虑执行中的物理风险，无法建立完整信任，因此需定义可持续安全成功的可信具身智能框架。

Method: 提出四层架构：模型层（任务提案与不确定性校准）、系统层（可靠执行与故障容错）、证据层（验证与保证论证）、部署层（运行时监控与更新），并基于多领域提出非规范性可信层次分级。

Result: 指出单一层级的模型能力或安全措施不足以建立端到端可信性，提供了跨层级故障传播分析及基于任务能力、安全、系统保证、运营治理和证据的可信度分级。

Conclusion: 该框架为可信具身智能的部署评估、比较研究、优先级设置和未来标准化提供了基础，强调跨层级整合的重要性。
...
TL;DR: 本文研究了在零样本导航任务中，使用通用智能体框架（agenic control）替代任务特定工作流或具身策略的效果，发现模型选择是关键因素，而工具接口对弱模型有帮助但可能阻碍强模型。


<details>
  <summary>Details</summary>
Motivation: 当前具身智能体依赖任务特定工作流或策略，缺乏通用性；探索通用智能体框架是否能在最小化设置（单目RGB、离散动作）中实现自主导航。

Method: 在零样本导航测试中，使用三个软件工程智能体框架（opus-5, fable-5等），结合可选的训练好的航点工具，评估默认和最大努力配置下的成功率、步数和耗时。

Result: 默认配置下opus-5平均成功率达到70.7±3.5%，fable-5最大努力达78%；混合fable-5（含航点工具）默认配置达76.7±0.6%，耗时和步数减少超一半；模型选择对成功率影响最大，工具接口对弱模型有效但对强模型有抑制作用。

Conclusion: 通用智能体框架在零样本导航中已具竞争力，模型、框架和接口三者互补，但长时任务能力不足，延迟和上下文增长限制持续运行。
...
TL;DR: 提出MMOCIC框架，通过解耦学习与合规，在协同搜索救援中平衡团队性能与规范遵守。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在实时平衡高维团队目标与低级合规约束。

Method: 多目标框架整合协同进化行为与合规行为，将合规作为外部约束而非学习目标。

Result: 硬件8艇/仿真12艇的协同救援中实现高团队性能且无碰撞。

Conclusion: MMOCIC有效协调学习与预设规范，适用于实际部署。
...
TL;DR: 提出MoMo框架，通过两阶段模仿学习将运动模式作为可复用的行为因子，在六项真实机器人操作任务中实现跨任务的运动模式组合泛化。


<details>
  <summary>Details</summary>
Motivation: 机器人需在不同情境中灵活调整操作执行方式，现有方法缺乏可跨任务复用的执行层行为控制。

Method: 两阶段模仿学习框架：时空动作分词器 + 行为克隆Transformer，以任务和连续运动模式为条件输入。

Result: 六项任务中可通过调节运动模式产生稳定、动态及中间行为，并被人类分辨；仅单模态训练数据下仍可迁移未见模式并保持任务成功率。

Conclusion: 运动模式可跨任务复用，实现任务-运动模式的组合泛化，从而控制操作技能的执行方式。
...
TL;DR: 提出一种利用缝线间接拾取手术针的自主框架，解决传统直接拾取在遮挡或不可达情况下的问题，并避免组织损伤。


<details>
  <summary>Details</summary>
Motivation: 自主缝合中针可能被掉落或需调整夹持，现有直接拾取方法受限且易造成组织损伤，需一种更安全、鲁棒的方案。

Method: 利用缝线作为辅助工具，包括线组织重建、安全抓点选择、稳定提线及双机械臂跟踪抓取，并融入视觉不确定性处理。

Result: 在达芬奇研究套件上验证，即使在复杂线形或不可达针条件下仍表现鲁棒。

Conclusion: 该框架填补了自主策略在非结构化缝合环境中的应用空白，提升安全性与可行性。
...
TL;DR: 提出了一种自适应的在线学习方法，用于跟踪未知且可能切换的目标动力学，并通过自监督学习从零开始训练多个预测器，自适应选择最佳匹配。


<details>
  <summary>Details</summary>
Motivation: 解决机器人跟踪未知目标（如动态地图、交通控制、追捕-规避场景）时，目标动力学可能混合结构化、随机或对抗性行为的问题。

Method: 自监督、一次性高效学习多个预测器，自适应选择最佳预测器；理论分析包括有限时间近优性保证和遗憾界。

Result: 在无误差和切换时渐近匹配最优非因果控制策略（无悔）；存在误差时平均遗憾与学习误差和切换频率成比例；Crazyflie仿真和硬件实验验证了有效性。

Conclusion: 方法在跟踪未知切换目标时表现鲁棒，优于非随机、核方法和神经网络方法。
...
TL;DR: 在低成本机器人平台上部署学习控制策略面临传输延迟和噪声反馈导致的仿真到现实差距问题。受生物启发，本研究通过时间感知神经网络结合平均执行器延迟前向模型，在Mini Pupper 2平台上实现了鲁棒步态，并发现了自维持的中央模式生成器（CPG），该CPG对高达+320毫秒的延迟扰动具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 低成本机器人平台（如Mini Pupper 2）上，超过50毫秒的传输延迟将标准马尔可夫决策过程变为部分可观测问题，导致仿真到部署的差距显著扩大，亟需解决。

Method: 采用生物启发的噪声和延迟反馈处理方法，结合平均执行器延迟的前向模型与时间感知神经网络。

Result: 在低成本四足平台上，时间感知神经网络学会了中央模式生成器（CPG），实现了对高达+320毫秒延迟扰动鲁棒的自维持节律步态。

Conclusion: 时间自组织可能是成本受限的机器人运动控制中的一种通用策略。
...
TL;DR: 提出RLMM-Flow框架，结合流策略预训练和潜在空间强化学习后训练，提升移动操作策略质量。


<details>
  <summary>Details</summary>
Motivation: 基于模仿学习的流策略无法超越演示分布，需要额外优化。

Method: 先用专家演示预训练流策略，然后冻结策略，用潜在引导网络优化初始噪声；预热动作批评家后联合训练潜在批评家和潜在演员，并引入粗到细潜在引导。

Result: 在移动操作运动规划基准上，显著提升任务成功率、碰撞避免和轨迹质量，同时保持快速推理。

Conclusion: RLMM-Flow有效克服模仿学习局限，提升了策略性能。
...
TL;DR: 本文提出概念专家模块，将3D物理世界的对象结构信息转换为可执行的分析概念，以增强VLA模型的空间感知和操作精度。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型主要依赖2D输入，缺乏3D物体结构信息和常识知识，限制了其在复杂精细操作中的空间感知和适应性。

Method: 通过概念专家模块在VLA推理前利用视觉基础模型的3D信息估计初始运动学和结构参数，并在操作过程中动态跟踪概念参数以保持准确性，同时提供程序和空间指导。

Result: 在监督学习和强化学习设置中，成功率和学习效率均有持续提升。

Conclusion: 基于结构化的概念指导有效增强了VLA后训练的物理交互行为学习，同时保持端到端灵活性。
...
TL;DR: 提出Speech2Grasp框架，将文本条件抓取模型高效迁移到语音控制，通过轻量MLP投影器实现，并在人形机器人实验中验证性能优于级联ASR系统。


<details>
  <summary>Details</summary>
Motivation: 自然的人机交互需要多模态理解（语音而非文本），但现有视觉-语言模型主要接受文本输入，需探索将文本条件模型高效迁移到语音输入的可行性。

Method: 以ALBEF为案例进行诊断分析，采用轻量MLP投影器适配语音特征；提出Speech2Grasp框架，将文本条件抓取检测迁移到语音，用于人形机器人实时抓取。

Result: Speech2Grasp在人形机器人真实实验中优于级联ASR方案，推理延迟更低，且轻量投影器能保留语义判别性和鲁棒性。

Conclusion: 该工作展示了将成熟文本条件系统高效迁移到语音输入的实用范式，为多模态人机交互提供新思路。
...
TL;DR: 提出半分散化POMDP框架，在间歇性通信约束下实现近集中式避碰性能，同步事件减少28.5%。


<details>
  <summary>Details</summary>
Motivation: 现有航天器避碰依赖间歇地面站接触，信息延迟且非同步，需协调程度与集中式规划可比性未知。

Method: 将问题建模为半去中心化POMDP (SDec-POMDP)，用RS-SDA*算法联合求解机动策略，以地面站可见窗口模拟信息传播。

Result: 半去中心化规划接近集中式机动质量，同步事件比连续协调少28.5%，且更稳定达成所需碰撞距离带，减少轨迹偏量。

Conclusion: 为间歇通信下自主避碰提供实用规划框架，弥合理想集中式与完全去中心化间的差距。
...
TL;DR: 提出了一种与具体机械臂形态无关的动作表征方法Contact Flow，通过追踪3D接触点轨迹，结合视频生成模型，实现从人类演示到不同机器人形态的动作规划和转移。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型难以捕捉操作中关键的物理约束（如接触），且动作条件常局限于特定机械臂形态（如平行夹爪），缺乏通用性。

Method: 1. 定义Contact Flow动作表征，编码操作者和目标物体间3D接触点轨迹；2. 基于人类和机器人交互视频，训练以Contact Flow为条件的大规模视频生成模型；3. 集成到“提出-想象-验证-执行”流程中，由视觉语言模型评估生成的结果。

Result: 在DROID数据集和真实桌面操作任务上，Contact Flow成功实现人类演示到不同机器人形态的动作转移。

Conclusion: Contact Flow提供了一种与具体形态无关的动作表征，结合视频生成模型，能有效预测物理合理的操作结果，并支持跨形态的动作转移。
...
TL;DR: 提出Enfold方法，将世界生成器的计算过程内化为从当前视觉和语言指令预测的表征，避免每次推理都执行生成器，显著降低动作延迟。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型需每次执行生成器来产生未来表征，计算成本高，限制了实时控制应用。

Method: 在多级生成器处理观察到的未来时，提取其中间状态作为监督信号，训练一个仅使用当前输入的编码器；该表征用于条件化未来生成和下游任务，同时阻止任务梯度修改编码器。

Result: 在LIBERO、RoboTwin2.0和真实机器人任务中，Enfold在保持强控制性能的同时，动作延迟相比Fast-WAM降低3.7倍，Enfold-Flash达到10.1倍。表征分析表明其抑制噪声、偏好长时程变化，且对人工干预能自适应调整。

Conclusion: 世界生成器可被重新用作预测控制表征的来源：其内部结构可折叠进当前状态，无需每步实体化未来轨迹，从而实现高效控制。
...
TL;DR: 本文提出ActSWM模型，通过过渡-分离原则增强潜在世界模型对动作的敏感性，解决上下文崩塌问题，提升长期规划与动作恢复能力。


<details>
  <summary>Details</summary>
Motivation: 现有潜在世界模型在长期展开时存在上下文崩塌，导致不同动作序列产生的未来状态难以区分，影响规划有效性。

Method: 基于过渡-分离原则，在潜在动力学中强制动作敏感性作为约束，而非仅辅助预测目标，保持不同动作下未来轨迹的可区分性。

Result: 在步漂移分析、封闭Minecraft规划及跨游戏动作恢复任务中，ActSWM保持更大动作依赖展开差距，提高长期交互任务成功率，并实现基于离线游戏视频的动作恢复。

Conclusion: 动作敏感性约束能显著提升潜在世界模型在长期规划与动作推理中的实用性与鲁棒性。
...
TL;DR: 提出Vision-TL-Action，融合多视角图像与时序逻辑图生成机器人动作轨迹，无需精确物体几何信息。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要精确物体几何编码至任务图中，忽略感知与符号的绑定问题。

Method: 通过双向交叉注意力融合TL节点令牌与视觉令牌，并用流匹配轨迹生成器，训练时增加谓词-区域监督。

Result: 在Panda任务上Success@1024为67.45%（基线59.11%），AntMaze任务上Success@256为96.35%（基线96.88%）。

Conclusion: 模型成功实现从视觉和TL目标直接映射到动作轨迹，无需物体几何先验。
...
### [184] [NeoRacer: An Open, Standardized 1:12 Scale Autonomous Race Car for Benchmarking and Education](https://arxiv.org/abs/2607.26855)
*Koneshka Bandyopadhyay,Ansh Mehta,Bassel El Mabsout,Renato Mancuso*

Main category: cs.RO

TL;DR: 提出了NeoRacer，一款开源的1:12比例自动驾驶赛车平台，集成高性能计算与传感器，旨在解决自主系统研究缺乏标准化开放硬件的问题。


<details>
  <summary>Details</summary>
Motivation: 自主系统研究缺乏广泛接受的开放硬件，现有平台成本高昂或性能不足，阻碍了研究比较、可复现性及教育普及。

Method: 基于NVIDIA Jetson Orin Nano、LiDAR、高速相机和IMU构建，采用开放硬件/软件设计，联合基金会与制造商实现规模化生产。

Result: 提供67 TOPS算力，预组装售价2699美元，算力为同类平台3倍以上，成本低于最接近替代品一半。

Conclusion: NeoRacer通过标准化、可复现的开源平台，有望加速自主系统研究，尤其是自动驾驶算法基准测试。
...
### [185] [CheckVLA: Execution-Time Verification with Action-Conditioned World Model for Long-Horizon Mobile Manipulation](https://arxiv.org/abs/2607.26789)
...
TL;DR: 提出CheckVLA框架，通过动作条件世界模型验证长视界移动操作中的动作块执行，在RoboCasa365上成功率提升8.5%，及时召回率达77.9%。


<details>
  <summary>Details</summary>
Motivation: 开放循环动作块在长视界操作中无法应对执行偏差，导致错误传播且缺乏反馈机制。

Method: 使用单独训练的动作条件世界模型进行验证，结合保形校准风险阈值、延迟感知硬前缀替换和事件驱动关键帧银行。

Result: 在RoboCasa365上，平均成功率36.1%对比周期性重规划27.6%；动作条件召回率77.9%对比观测条件48.6%。

Conclusion: 动作条件验证能有效恢复块执行期间的反馈，同时保持修复与推理延迟的一致性。
...
### [186] [Risk-Aware Motion Planning with Learned Trajectory Primitives and Probabilistic Safety Assessment](https://arxiv.org/abs/2607.26802)
*Marc Kaufeld,Dian Zhuang,Johannes Betz*

Main category: cs.RO

TL;DR: 提出了一个基于径向基函数网络的运动规划框架，用于城市自动驾驶，通过结合轨迹生成、碰撞概率评估和优化，提高了安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 城市自动驾驶需要在安全性和效率之间取得平衡，现有方法在复杂场景下计算复杂度高或安全性不足。

Method: 使用RBFN生成 jerk 最小化轨迹，结合解析碰撞概率评估和优化轨迹精炼，减少MPC搜索空间。

Result: 在多个城市驾驶场景中，相比基准方法，展示了更好的风险意识，减少了车辆极限违规。

Conclusion: 将学习型轨迹整合到优化规划中，保证了安全性和可解释性。
...
### [187] [Controlled Experiments on Lane Changing by Transitional Autonomous Vehicle: Dataset and Behavioral Insights](https://arxiv.org/abs/2607.27085)
*Abhinav Sharma,Md Abdullah Al Hasan,Danjue Chen,George F. List*

Main category: cs.RO

TL;DR: 基于NC-tALC数据集，研究了过渡自动驾驶车辆强制换道行为的间隙演化和碰撞风险，发现前车-后车间隙在换道时收敛，碰撞风险在进入车道时达到峰值。


<details>
  <summary>Details</summary>
Motivation: 缺乏对过渡自动驾驶车辆完整强制换道过程的经验特征描述，现有研究多关注单一时刻间隙接受决策而非全过程。

Method: 在公共道路开展78次强制换道实地实验，使用RTK-GNSS/INS轨迹数据提取关键时间点，通过时间间隙和速度指标评估安全交互。

Result: 前后车间隙在换道时趋近窄范围，碰撞风险在车道进入时最大且由目标车道前车主导，换道完成不意味风险消失。

Conclusion: 实验揭示了tAV换道行为和安全变化规律，数据集为自动驾驶换道模型校准和仿真验证提供了经验基准。
...
TL;DR: 提出KinRT方法，通过运动学监督实现专家显式路由，解决MoE在视觉-语言-动作（VLA）中的路由无效问题，并在DIY机器人平台上验证有效性。


<details>
  <summary>Details</summary>
Motivation: MoE在操作任务中因动作运动学异构性及推理时信号缺失导致专家路由低效。

Method: 先对动作轨迹进行运动学聚类，用聚类ID监督路由器训练；推理时仅依赖视觉-语言观测，无需运动学信号。

Result: 在RoboTwin基准和DIYRobot平台上分别提升23.26%和20.27%。

Conclusion: 运动学监督显式路由能有效提升MoE-VLA的跨平台泛化能力。
...
TL;DR: 提出HERO，一种自改进的分层具身智能体，无需人类示范即可自主进化操作能力。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作技能主要静态获取，缺乏通过物理交互自适应进化的能力。

Method: 构建统一编排框架，整合启发式推理、范例复用和反射性执行，通过自主数据收集与任务执行耦合，渐进式巩固为闭环策略。

Result: 显著减少人类干预，实现多样化任务的鲁棒操作。

Conclusion: 为自改进机器人系统提供了一种有前景的路径。
...
TL;DR: 提出一种从不确定性到确定性的粗到细视觉地板布局定位框架，通过图像条件扩散模型和局部精化器，实现无需离线预处理的高精度定位。


<details>
  <summary>Details</summary>
Motivation: 传统基于光线匹配的方法存在信息丢失和资源消耗问题，且室内布局重复导致多模态位姿分布挑战。

Method: 粗阶段设计图像条件位姿扩散模型参数化连续多模态分布；细阶段使用局部精化器预测位姿残差。

Result: 在S3D（完整）和ZInD基准上达到最先进精度和鲁棒性。

Conclusion: 方法有效平衡全局多假设跟踪与局部精细调整，无需外部预处理或查找表。
...
### [191] [BioVLN: A Simulation Platform for Visual Language Navigation in Biomedical Laboratories](https://arxiv.org/abs/2607.26914)
*Zhe Liu,Quan Lu,Zhaohui Du,Zhe Wang,Huanbo Jin,Jiaming Gu,Qi Wang,Ting Xiao,Minting Pan,Dongzhan Zhou*

Main category: cs.RO

TL;DR: 提出了一个专门用于生物医学实验室的视觉-语言导航模拟平台BioVLN


<details>
  <summary>Details</summary>
Motivation: 现有导航平台面向家庭环境，无法处理实验室仪器需从操作侧接近且保持安全距离的特殊要求

Method: 将仪器分为物理体、安全区、操作区三个区域，支持程序化场景生成和手动设计环境

Result: 几何探索成功率74.4-87.5%，在操作区采样多个有效位置可将成功率提升至83.3-92.5%并减少危险靠近

Conclusion: 专门为实验室仪器设计的区域划分能有效提升导航安全性和成功率
...
### [192] [Dense Soft Weighting for Radar Ego-Velocity Estimation](https://arxiv.org/abs/2607.26980)
*Atar Babgei,Chenyu Zhao,Michael Breza,Julie A. McCann*

Main category: cs.RO

TL;DR: 提出Dense Soft Weighting方法，利用全范围-多普勒单元的连续置信度度量替代CFAR阈值，实现毫米波雷达的精确自身速度估计。


<details>
  <summary>Details</summary>
Motivation: 传统CFAR阈值处理会丢弃次阈值多普勒信息，导致视觉退化环境中速度估计精度不足。

Method: 为每个距离-多普勒单元分配连续置信度权重，结合加权最小二乘估计速度，并推导闭环速度协方差与惯性后端集成。

Result: 在三个数据集上，相较最优CFAR基线，平均绝对位姿误差降低31-45%，且实时运行于嵌入式硬件。

Conclusion: 无需平台特定训练数据，方法可直接迁移至不同单芯片雷达配置，显著提升退化环境中的状态估计鲁棒性。
...
### [193] [SymmGrid: Super-Scaling On-Robot Learning with Parallelized Symmetries and Egocentric-Exocentric Visual Perception](https://arxiv.org/abs/2607.26985)
*Gabe Everett,Brice Gunter,Ryan Vander Stelt,Cleiver Ruiz-Martinez,Blake Hull,Juan Rojas*

Main category: cs.RO

TL;DR: 本文提出SymmGrid框架，通过对称性超缩放生成几何网格结构，加速物理机器人上的深度强化学习，在操作任务中实现1.37-2.17倍训练速度提升和1.09-1.27倍成功率改进。


<details>
  <summary>Details</summary>
Motivation: 物理机器人上的深度强化学习受限于训练时间慢，需要更快的学习方法来提升实用性。

Method: 提出SymmGrid框架，在对称树下建模MDP，利用并行化对称变换生成几何网格结构，并为自我中心和非自我中心视觉场景应用单应变换处理。

Result: 在真实机器人操作任务中，训练收敛速度提升1.37-2.17倍，成功率提升1.09-1.27倍，最快训练时间分别为16.6、10.9和79.3分钟，nAUC提升高达2.59倍。

Conclusion: SymmGrid通过简单分支对称性的超缩放显著加速了机器人学习，使次10分钟训练成为可能，适用于手臂和人形机器人操作任务。
...
### [194] [RL$^2$-VLA: Adaptive RL Latent Compositional Steering with Test-Time Scaling for Vision-Language-Action Models](https://arxiv.org/abs/2607.26991)
*Derek Ming Siang Tan,Shailesh Shailesh,Srikrishna Iyer,William Wei Jie Teo,Yuanliang Ju,Qiao Gu,Guillaume Sartoretti*

Main category: cs.RO

TL;DR: 提出RL2框架，通过强化学习对VLA模型隐空间进行自适应推理时调控，仅在预测失败时激活组合策略，提升机器人操作成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在处理困难/域外任务时性能下降，测试时调控方法存在动作多样性不足、对成功/失败状态无差别干预的问题。

Method: 训练轻量级离线RL策略，基于VLA动作专家的隐层表达，在推理时与冻结VLA进行流速度组合，仅当预测到失败时激活组合调控。

Result: 在SIMPLER和PolaRiS基准上，域外设置成功率提升最高17.3%，真实世界实验验证了泛化性。

Conclusion: RL2提供了一种实用模块化的VLA推理时调控框架，揭示了动作多样性在失败状态下的有效性和成功状态下的必要性。
...
TL;DR: 本文提出DLAM，一种分布性潜在动作模型，通过高斯分布表示过渡、归一化组合和反转约束来改进潜在动力学一致性，从而提升下游策略学习在机器人操作任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型受限于标注数据稀缺，而潜在动作模型缺乏结构化约束，导致误差累积，限制了与机器人动作的联合生成。

Method: 采用对角高斯分布表示过渡，通过参考帧归一化约束均值，使用等间隔三元组归一化组合和反转约束均值和方差，方差组合采用共享相关系数，并冻结编码器训练流匹配策略。

Result: 在MetaWorld MT50、LIBERO和真实操作任务中，DLAM比基线模型实现了更一致的潜在动力学和更强的重建性能，并提升了策略成功率；消融实验显示均值约束贡献主要重建增益。

Conclusion: 分布性建模和结构化约束可显著改善潜在动作模型的时序一致性和机器人控制性能。
...
TL;DR: 提出了一套在ROS 2系统中实现知识图谱的完整方法论，用于提升自主机器人任务效率与智能性，并通过仿真搜救任务验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 增强自主机器人任务中决策制定的智能化和执行效率。

Method: 定义了初始与目标条件、结构化任务与子任务、规划执行顺序、在知识图谱中表达任务数据，并使用高级语言设计任务。

Result: 在Aerostack2框架中通过Gazebo仿真的搜救任务实践，验证了该方法有效提升了决策和任务性能。

Conclusion: 知识图谱能够有效提升自主机器人的任务执行效率与智能性。
...
### [197] [Data-Driven Stability and Performance Analysis of Lurye Systems](https://arxiv.org/abs/2607.26277)
*Sahel Vahedi Noori,Peter Seiler*

Main category: eess.SY

TL;DR: 论文提出了基于数据驱动的条件，用于验证离散Lurye系统的内部稳定性和诱导ℓ₂性能，无需名义LTI系统的状态空间模型，仅利用轨迹数据。


<details>
  <summary>Details</summary>
Motivation: 现有Lurye系统条件需要名义LTI动力学的状态空间实现，论文旨在通过数据驱动方法消除此需求，提高实用性。

Method: 首先利用有限输入、输出和状态轨迹数据制定稳定性和性能条件；然后通过确定性子空间辨识技术从输入/输出数据重构状态序列（允许相似变换），两种条件均表示为凸半定规划。

Result: 在充分激励输入和无噪声条件下，数据驱动条件可恢复基于模型的Lurye条件；通过扇形有界非线性示例验证了与模型方法相同的诱导ℓ₂增益界。

Conclusion: 数据驱动方法有效替代模型依赖，仅凭轨迹数据即可实现等效稳定性和性能验证。
...
### [198] [Time-delay Control Using a New Nonlinear Adaptive Law for Cable-Driven Robots](https://arxiv.org/abs/2607.26383)
*Wenbo Gao,Yaoyao Wang,Jiawang Chen,Wenliang Zhang,Hanzhuo Wang*

Main category: eess.SY

TL;DR: 提出基于时延估计的自适应分数阶非奇异终端滑模控制策略，用于解决缆索驱动机器人非线性、低刚度下的精确控制问题，实验证明能将均方根误差降低约34%。


<details>
  <summary>Details</summary>
Motivation: 缆索驱动机器人具有强非线性和低结构刚度，在时变不确定性和外部扰动下精确控制困难。

Method: 在时延估计的无模型框架下，结合分数阶非奇异终端滑模误差动力学和快速终端滑模趋近律，并设计带有自适应指数项的非线性自适应律。

Result: 实验表明，相比基线方法，两个关节的均方根误差分别降低34.52%和31.11%，时间绝对误差积分分别降低33.79%和32.97%，摆角积分分别降低6.69%和17.77%；自适应律响应更快，增益演化更稳定。

Conclusion: 所提方法能有效抑制颤振、提高跟踪精度，具有较强的鲁棒性和可重复性。
...
### [199] [Optimal sizing of a hydrogen-based direct reduced iron-electric arc furnace system integrated with methanol synthesis toward zero-carbon steel production](https://arxiv.org/abs/2607.26270)
*Qiang Ji,Fashun Shi,Lin Cheng,Yeye Xie,Yufei Xi,Zhen Dai,Xun Wang*

Main category: eess.SY

TL;DR: 开发了基于分数规划的H2-DRI-EAF-MeOH系统最优容量配置模型，采用年化净投资收益率(ANROI)作为投资回报指标，并利用Dinkelbach方法求解。


<details>
  <summary>Details</summary>
Motivation: 可再生能源的波动性和间歇性可能导致系统容量过度配置，而传统年化成本和度电成本指标不足以评估投资回报。

Method: 建立分数规划最优容量配置模型，将小时产量上限与年产能解耦；采用ANROI衡量投资回报，并通过Dinkelbach方法求解混合整数线性分数规划模型。

Result: 相比年化净收益基准，ANROI优化提高了内部收益率(IRR)；电网连接和离网场景下IRR分别为20.67%和17.08%；等效年运行时间缩短使IRR提高并降低电池容量；离网零碳约束虽实现CO2制甲醇但降低了IRR至14.99%。

Conclusion: ANROI指标更有效评估投资回报，零碳约束下系统经济性需进一步改进以弥补额外成本。
...
### [200] [Planning Waste-to-Energy-Coupled AI Data Centers Through Grade-Matched Cooling and Corridor Screening](https://arxiv.org/abs/2607.26324)
*Qi He,Chunyu Qu,Wenjie Zuo*

Main category: eess.SY

TL;DR: 针对AI数据中心增长带来的电力、互联和冷却限制，开发了一个边界一致的垃圾焚烧耦合冷却筛选框架，将冷却作为按等级匹配的能量服务。


<details>
  <summary>Details</summary>
Motivation: AI数据中心的快速扩张受到有限的可交付电力、互联容量和冷却需求的约束，传统电力驱动的机械冷却方式面临挑战。

Method: 通过考虑热衰减、吸收转换及输送和辅助的寄生电力，将工厂侧可出口热量转化为走廊级规划指标，建立了边界一致的筛选框架。

Result: 基准案例中，1500吨/天的垃圾焚烧厂在20公里走廊提供约53.0 MW冷却和8.0 MWe净节电；1 GW IT园区在5公里走廊可节电116.9-264.4 MW，所需垃圾焚烧厂数量为3-148座。

Conclusion: 该框架可识别垃圾焚烧耦合冷却的走廊可行性、混合操作需求及基础设施规模约束，适用于筛选和比较，而非具体工程设计。
...
### [201] [Fully distributed singularity-free prescribed-time stabilization of the continuous-time generalized adaptive Bellman-Ford algorithm](https://arxiv.org/abs/2607.26424)
*Yuanqiu Mo,Jian Qin,Soura Dasgupta*

Main category: eess.SY

TL;DR: 提出两种控制策略，使广义自适应Bellman-Ford算法在用户指定时间内收敛到稳态值，增强其实际应用。


<details>
  <summary>Details</summary>
Motivation: 现有分布式偏最小一致性协议主要关注渐近稳定性，缺乏收敛速度分析，限制了实际应用。

Method: 通过控制策略实现预设时间稳定化，确保算法在指定时间内收敛。

Result: 在机器人路径规划等场景中验证了所提方法的有效性。

Conclusion: 所提策略拓宽了GABF协议的应用范围，可通过用户自定义时间保证收敛。
...
### [202] [Global Exponential Stabilization of the Kinematic Bicycle Model of a Car in Polar Coordinates](https://arxiv.org/abs/2607.26442)
*Velimir Todorovski,Kwang Hak Kim,Alessandro Astolfi,Miroslav Krstic*

Main category: eess.SY

TL;DR: 本文提出了基于极坐标和范围归一化坐标的全局指数稳定反馈控制律，使得自动泊车轨迹更接近人类驾驶行为。


<details>
  <summary>Details</summary>
Motivation: 现有运动学自行车模型在低速泊车时缺乏有效的稳定反馈律，且传统方法因Brockett条件无法实现平滑静态反馈稳定，生成的轨迹不类人。

Method: 将系统转换为极坐标与范围归一化坐标组合，使动力学变为严格反馈形式；利用非传统反步法设计光滑反馈律。

Result: 变换坐标下实现全局指数稳定，通过纯反馈生成与人驾驶相似的泊车轨迹。

Conclusion: 所提方法有效绕过了Brockett障碍，为低速自动泊车提供了仿人性能的稳定控制方案。
...
### [203] [Online Monitoring and Risk Assessment of Non-Cooperative UAVs via STL-Aware Adaptive Fusion Kalman Filtering](https://arxiv.org/abs/2607.26527)
*Xinhao Yan,Ruige Yang,Chao Peng,Hailong Huang*

Main category: eess.SY

TL;DR: 一种针对非合作无人机的在线状态估计与预测风险评估框架，融合交互多模型多速率卡尔曼滤波与信号时态逻辑。


<details>
  <summary>Details</summary>
Motivation: 解决异步异构感知和不确定运动模式下的UAV状态估计与安全评估难题。

Method: 结合交互多模型多速率卡尔曼滤波与信号时态逻辑，引入STL感知时变模式转换机制在线更新模型概率。

Result: 提高估计精度，并产生更早、更丰富的安全警告。

Conclusion: 该方法适用于实时无人机监控与安全监测，能提前指示不安全行为。
...
### [204] [Emission-Forecasting-Based Spatial-Temporal Carbon Response: A Multi-Agent Attention-Enhanced Deep Learning Framework](https://arxiv.org/abs/2607.26560)
*Feiyu Cai,Jing Qiu,Yi Yang,Chenxi Zhang,Xinlei Wang,Baichuan Liu,Junhua Zhao*

Main category: eess.SY

TL;DR: 提出一种基于深度学习和语言模型的日前碳强度预测框架，结合调度负荷，在降低延迟的同时减少30%以上碳排放。


<details>
  <summary>Details</summary>
Motivation: 传统事后碳强度计算存在调度延迟，阻碍低碳响应，需主动预测方法。

Method: 构建深度学习层次化设计，含双阶段注意力机制和LLM多智能体系统，预测日前节点碳强度；结合可调度负荷的空间-时间碳调度模型。

Result: 在IEEE 33节点系统中，调度延迟减少1小时可实现30%以上减排。

Conclusion: 突破被动碳核算局限，实现主动碳管理，加速清洁电力系统转型。
...
### [205] [An extended Perron-Frobenius operator filter for nonlinear state estimation](https://arxiv.org/abs/2607.26632)
*Yuta Miwa,Yoshihiko Susuki,Shunji Kotsuki*

Main category: eess.SY

TL;DR: 提出扩展Perron-Frobenius算子滤波器用于非线性状态估计。


<details>
  <summary>Details</summary>
Motivation: 利用线性算子完整保留非线性系统特性，并显式处理非高斯分布。

Method: 通过扩展动态模式分解学习Perron-Frobenius算子，自由选择基函数。

Result: 数值实验表明该方法在计算效率和估计精度上均有优势。

Conclusion: 基函数选择的灵活性是提升性能的关键。
...
### [206] [A Physics-Informed Framework for PID Tuning of Chemical Processes Using Large Language Model Agents](https://arxiv.org/abs/2607.26594)
*Zhoupeng Shou,Xiaodong Hong,Congjing Ren,Jingdai Wang,Yongrong Yang,Zuwei Liao*

Main category: eess.SY

TL;DR: 提出了一种基于大/小语言模型辅助的PID整定框架，通过模拟工程师的迭代工作流，实现了对化工过程的高效PID参数整定。


<details>
  <summary>Details</summary>
Motivation: 传统PID整定依赖过程模型，而工程师常通过观测响应、诊断缺陷和调整增益来手动迭代；本研究旨在通过语言模型自动化此工作流，提升整定效率和可靠性。

Method: 使用托管LLM接收闭环响应特征、控制工程诊断和IMC演示来生成/修正PID增益；对Qwen3-0.6B采用SFT（模拟验证的IMC目标）和PI-GRPO（物理约束的稳定性与性能奖励）进行本地适配。

Result: 在100个FOPDT和100个SOPDT测试中，托管LLM（DeepSeek-V4-Flash和Qwen3.7-Plus）最终成功率75-89%和77-79%；SFT后首推荐成功率86.5%，PI-GRPO进一步提高至94.0%，主要改善首次尝试可靠性和稳定性裕度。

Conclusion: 提出的语言模型辅助PID整定框架在离线整定中表现优异，尤其是通过PI-GRPO训练的小模型在首荐成功率上超过大模型，具有实际部署潜力。
...
### [207] [UAV Swarming for Air-Ground ISAC via Cross-Region Cooperation](https://arxiv.org/abs/2607.26679)
*Linghui Miao,Shijian Gao*

Main category: eess.SY

TL;DR: 本文提出了一种跨区域协作框架，通过区域划分、自适应握手和基于MAPPO的决策，协调无人机群实现通信与感知性能提升。


<details>
  <summary>Details</summary>
Motivation: 无人机在执行集成感知与通信任务时面临地面通信需求动态不平衡和感知观测多样性有限的关键挑战。

Method: 设计了服务驱动的区域划分方案和自适应握手机制以提升协作感知精度，并采用基于CTDE的多智能体近端策略优化框架进行跨区域决策。

Result: 所提方法实现约90%的通信服务质量，并将克拉美-罗界降低约45%。

Conclusion: 跨区域协作框架有效提升了无人机群的通信与感知性能，优于传统方法。
...
### [208] [Global Sensitive-Based Input Shaping for UAV-Payload Precision Motion Control](https://arxiv.org/abs/2607.26717)
*Karan Baker,Sanjay Maharjan,Tariq Hlayel,Oladapo Ogunbodede,Dutch Dunphy,Adrian Stein*

Main category: eess.SY

TL;DR: 提出基于全局敏感性和Shapley值的输入整形器，用于3D无人机-负载系统，增强对负载质量和绳长不确定性的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有控制器对负载质量和绳长等参数不确定性敏感，需要更鲁棒且系统化处理不确定性的控制方法。

Method: 利用Shapley值概念设计控制器，系统性地考虑不确定性，并采用全局敏感性分析方法优化输入整形器。

Result: 数值仿真表明，所提方法（全局敏感性和Shapley值输入整形器）在性能上优于非鲁棒、鲁棒和最小最大设计。

Conclusion: 该方法为空中负载运输中的不确定性感知控制提供了有前景的框架。
...
### [209] [Joint State-Parameter Inference Enhances Estimation Performance in Model-Based Digital Therapeutics for Type 1 Diabetes](https://arxiv.org/abs/2607.26790)
*Milad Banitalebi Dehkordi,Vihangkumar V. Naik,Manas Mejari,Dario Piga,Jose Garcia-Tirado*

Main category: eess.SY

TL;DR: 论文提出基于Rao-Blackwellized Stein变分梯度下降的递归滤波框架，用于1型糖尿病生理模型的在线状态与参数联合估计，在Hovorka模型和OHSU模拟器验证中优于EKF和AEKF。


<details>
  <summary>Details</summary>
Motivation: 现有模型固定参数在真实自由生活条件下无法准确重建和估计血糖水平，需要在线联合估计状态和参数。

Method: 采用Rao-Blackwellized Stein变分梯度下降滤镜，计算非线性时变模型的联合后验分布。

Result: 在20个虚拟患者上，RBSVGD方法在血糖估计精度和模型参数估计上均优于标准EKF和增强EKF。

Conclusion: 所提框架为1型糖尿病决策支持和自动胰岛素输送系统提供更准确的血糖估计，具有临床应用潜力。
...
### [210] [Network Verified NTN Positioning for 6G A Standards Oriented Survey of Hybrid TN NTN Localization](https://arxiv.org/abs/2607.26846)
*Donglin Wang,Zexing Fang,Qiuheng Zhou,Hans D. Schotten*

Main category: eess.SY

TL;DR: 该综述回顾了从LTE和GNSS到NR、Release-18/19及NTN的三维定位演进，强调了混合TN/NTN框架中的标准化、垂直可观测性和网络验证的挑战。


<details>
  <summary>Details</summary>
Motivation: 智能交通、无人机操作等新应用需要高精度、垂直感知和可验证的定位，推动定位技术向混合、高精度、三维和网络可验证框架发展。

Method: 通过综述3GPP标准演进，分析定位技术族、方法级权衡、混合TN/NTN设计的开放挑战。

Result: 总结了定位技术的演进路径、混合网络设计中的关键挑战。

Conclusion: 未来定位需要标准化影响、垂直可观测量、可靠性感知测量选择和网络侧验证的优化。
...
### [211] [From Crop and Energy Data to Optimized Lighting Scheduling: A Surrogate-Based MILP Framework for Vertical Farming](https://arxiv.org/abs/2607.26968)
*Francesco Ceccanti,Andrea Baccioli,Aldo Bischi*

Main category: eess.SY

TL;DR: 提出基于代理模型的优化框架，通过灵活光照调度降低垂直农场用电成本，最高节省电费17.8%。


<details>
  <summary>Details</summary>
Motivation: 垂直农场经济效益受照明和气候控制的高电力需求限制，现有固定光照方案不经济。

Method: 将作物-能量响应数据转化为数学规划关系，结合合成数据构建代理模型，优化光照强度与光周期。

Result: 代理模型准确复现参考模型趋势；优化调度使电力消耗降低15.9%，电费降低17.8%，同时维持目标产量。

Conclusion: 中等光强与灵活光周期策略比高光强更经济；框架具通用性，可整合作物生长、能耗与电价波动。
...
### [212] [Dynamic Storage Operation Under Uncertainty and the Reliability Externality: Implications for Capacity Investments](https://arxiv.org/abs/2607.27021)
*Daniel Shen,Marija Ilic,John Parsons*

Main category: eess.SY

TL;DR: 研究了需求不确定性如何影响储能的动态运行及长期投资，发现预防性储能策略会显著改变需求分布，且市场可靠性外部性会扭曲储能运营与投资。


<details>
  <summary>Details</summary>
Motivation: 储能系统在可再生能源波动和电气化背景下日益重要，但其可靠性贡献受政策影响，且需平衡短期套利与未来稀缺风险，现有研究对需求不确定性如何改变储能运行和投资机制缺乏深入分析。

Method: 将储能运行建模为平均成本马尔可夫决策过程，并将稳态策略嵌入简化的容量扩展框架中。

Result: 需求不确定性促使储能采取预防性策略对冲随机稀缺，导致储能后需求分布与完美预测基准显著不同；电力市场的可靠性外部性与不确定性交互，独特地扭曲了储能运营与投资。
...
### [213] [Some intuition for why cooperative systems "look 1-dimensional" and 2-cooperative systems "look 2-dimensional"](https://arxiv.org/abs/2607.27176)
*Eduardo D. Sontag*

Main category: eess.SY

TL;DR: 本文通过Birkhoff-Hilbert投影度量的收缩现象，为合作系统及两合作系统的维度降低提供了几何直观解释。


<details>
  <summary>Details</summary>
Motivation: 已有理论表明合作系统及两合作系统分别在不可约条件和保二维锥条件下表现出低维动力学特征，但缺乏统一的几何理解。

Method: 利用正锥上的Birkhoff-Hilbert投影度量收缩性，分析系统动力学的降维机制。

Result: 合作系统和两合作系统的低维行为可归因于投影度量的收缩性质。

Conclusion: Birkhoff-Hilbert度量收缩为合作系统的维度简约提供了几何框架。
...
### [214] [Shape-Based Inductive Bias for Glioma Grading from Tumor Contours](https://arxiv.org/abs/2607.26090)
*Puneet Velidi,Michelle F. Miranda,Farouk Nathoo,Ashery Mbilinyi,Cédric Beaulac*

Main category: eess.IV

TL;DR: 论文用形状对齐框架处理胶质瘤分级，将轮廓形状转为频序令牌，压缩多层感知器（MLP）在 BraTS 2020 上以最少参数达最高平衡准确率 71.5%，优于像素模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法将轮廓分级当作像素问题，而信号本质是形状，故引入形状归纳偏置以提升可解释性与效率。

Method: 对齐封闭轮廓，分离全局形变与残余傅里叶形状，组织为频序令牌，用紧凑 MLP 进行分类。

Result: MLP 平衡准确率 71.5%，F1 Low-Grade Glioma 达 54.9%，参数仅 2.9k–117.3k，远少于像素基线。

Conclusion: 形状表征的归纳偏置可显著降维并提升可解释性与可扩展性。
...
### [215] [ScalablePromptus: Scalable and High-Fidelity Prompt-Based Video Streaming](https://arxiv.org/abs/2607.26106)
*Zehao Cao,Bowei Xu,Xun Cao,Zhan Ma,Hao Chen*

Main category: eess.IV

TL;DR: 提出ScalablePromptus，通过丢弃训练等策略增强提示视频流在丢包环境下的鲁棒性，减少性能损失82%-95%。


<details>
  <summary>Details</summary>
Motivation: 现有Promptus框架在部分提示丢失时会导致严重的质量坍塌，无法应对网络波动。

Method: 语义与色彩感知提示反转、球面线性插值以及产生排序提示表示的丢弃训练策略。

Result: 稳定网络下质量略有提升；丢包条件下性能损失较基线降低82%-95%。

Conclusion: ScalablePromptus使基于提示的视频流在真实网络部署中具有足够的鲁棒性。
...
### [216] [A Hierarchical Framework for Graph Structure Learning in Histopathology Image Classification](https://arxiv.org/abs/2607.26153)
*Sudipta Paul,Amanda W. Lund,Bülent Yener*

Main category: eess.IV

TL;DR: 提出了一种优化两层级图学习框架G2*-Net，用于组织病理学图像的分类，通过学习图像层级图结构提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的方法依赖固定或启发式图结构，无法准确表示组织连接性，需优化图结构学习。

Method: 先分割图像为块，构建细胞级图；再以每个块为节点，通过双层优化（二阶优化）学习图像级图结构，采用DARTS启发的一步展开近似高效计算超梯度。

Result: 在三个不同组织病理学数据集上验证了方法的有效性。

Conclusion: G2*-Net通过可学习图结构提升分类性能，为大规模组织病理图像分析提供新方法。
...
### [217] [Rethinking Clinical Relevance in Chest X-ray Machine Learning: How Evaluation References Define Performance](https://arxiv.org/abs/2607.26333)
...
TL;DR: 本文系统研究了胸片机器学习中评估参考选择对病理分类和图像质量评估模型性能及排名的影响，发现标签来源和度量方法的变化会显著改变结果，强调评估参考需根据病理、成像任务和临床用途合理选择。


<details>
  <summary>Details</summary>
Motivation: 当前胸片机器学习依赖报告标签或通用图像质量指标，但这些可能无法可靠反映临床判断，导致模型评估和排名偏差。

Method: 收集剑桥大学医院临床队列的配对专家和报告标签，以及MIMIC-CXR数据集子集和专家诊断质量评分；对比监督分类器（ResNet, DenseNet）与视觉语言模型（如MedKLIP, GLoRIA, ConVIRT）在不同标签源下的表现；评估IQA度量（如SSIM, PSNR）与专家判断的对齐情况。

Result: 标签源变化导致性能估计和模型排名显著差异；常用IQA指标（SSIM, PSNR）常与专家诊断可用性判断不一致。

Conclusion: 评估选择至关重要，决定模型优劣排名和选择，应视为临床有效性的核心要素，需根据病理、成像任务和临床用途进行论证。
...
### [218] [Decoder-Guided Lossy Contour Coding Via Anchor Refinement](https://arxiv.org/abs/2607.26426)
*Ruoyu Yang,Yichi Zhang,Haohong Wang,Fengqing Zhu*

Main category: eess.IV

TL;DR: 提出一种粗到细的轮廓编码框架，利用解码端已有的粗轮廓作为先验，通过自适应锚点跳过和率失真控制，实现高质量轮廓的压缩，相比JBIG2等方法节省54.5%-66.9%比特率。


<details>
  <summary>Details</summary>
Motivation: 现有轮廓编解码器（如JBIG2）为无损对称设计，忽略了解码端已存在粗轮廓的先验信息，导致高码率且缺乏率失真控制。

Method: 将高质量轮廓建模为对解码端粗轮廓的结构化几何细化：编码器沿精细轮廓提取有序锚点并在失真约束下自适应跳过，解码器利用粗先验指导锚点连接重建轮廓。

Result: 相比无解码端指导的方法比特率降低54.5%-66.9%，相比JBIG2节省高达5倍比特率，同时保持高几何精度。

Conclusion: 所提粗到细轮廓编码框架通过利用解码端先验实现有损压缩，显式优化率失真权衡，在低码率下有效保留几何结构。
...
### [219] [Multi-scale radiomics in pelvic MRI for endometriosis subtyping: highlighting data heterogeneity constraints](https://arxiv.org/abs/2607.26692)
*Eliot Leguy,Chloe Mallet,Nicolas Normand,Elodie Germani*

Main category: eess.IV

TL;DR: 基于盆腔MRI的影像组学在子宫内膜异位症亚型分类中表现出初步信号但效果脆弱，尤其在小规模多中心数据集中。


<details>
  <summary>Details</summary>
Motivation: 分析女性盆腔MRI具有解剖复杂、技术变异和读片差异等挑战，需探索客观的亚型分类方法。

Method: 使用UT-EndoMRI数据集，从小波特征中提取影像组学特征，结合多尺度表示与特征选择，训练监督分类器（如梯度提升），并进行无监督扰动分析和ComBat调和。

Result: 最佳监督性能AUC为0.80，但假阳性多导致特异性低；ComBat调和未持续改善；无监督聚类可识别再现分区但仍受采集变量影响。

Conclusion: 盆腔MRI影像组学存在子宫内膜异位症亚型分类的初步信号，但在小样本多中心数据中稳定性不足。
...
### [220] [An Attention-Based Framework for Alzheimers Disease Classification Using Resting-State fMRI](https://arxiv.org/abs/2607.26746)
*Harshiddhi Pathak,Gowtham Reddy N,Mrinal Acharya,Manjunatha Mahadevappa*

Main category: eess.IV

TL;DR: 提出基于注意力机制的深度学习框架，直接处理rs-fMRI功能连接矩阵实现阿尔茨海默病分类，在ADNI数据集上达到88.95%准确率和0.90 AUC。


<details>
  <summary>Details</summary>
Motivation: 传统基于手工特征或常规机器学习的方法难以处理rs-fMRI数据的高维度、噪声和复杂脑区间依赖关系。

Method: 将脑区视为token，采用Transformer自注意力机制建模全局功能依赖，并引入subject-wise评估和类别加权优化。

Result: AD vs 正常对照组分类准确率88.95%，ROC-AUC 0.90，精确率-召回率平衡良好。

Conclusion: 自注意力驱动的功能连接建模是鲁棒且可解释的AD检测方法。
