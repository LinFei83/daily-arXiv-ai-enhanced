...
TL;DR: 本研究调查了大型语言模型（LLMs）的幻觉如何影响用户信任及其与LLM的互动，发现幻觉导致情境敏感的信任校准，并识别了影响信任校准的用户和情境因素。


<details>
  <summary>Details</summary>
Motivation: 研究LLM输出的幻觉（事实不正确但看似合理的内容）如何影响用户对LLM的信任以及用户与LLM的互动。

Method: 一项包含192名参与者的定性研究，以探索日常使用情境下幻觉的影响。

Result: 幻觉不会导致全面不信任，而是引发情境敏感的信任校准。研究证实了期望、先验经验、用户专业知识和领域知识作为用户相关信任因素，并识别出直觉是幻觉检测的额外因素。信任动态还受到感知风险和决策风险等情境因素的影响。研究验证了Blöbaum提出的递归信任校准过程，并将其扩展，纳入直觉作为用户相关的信任因素。

Conclusion: 基于对信任校准动态的深入理解，本研究为负责任和反思性地使用LLM提出了实用建议。
...
TL;DR: 本文指出当前AI治理框架在用例级风险评估、可操作性控制和规模化运营方面存在不足，并提出了AI TIPS 2.0框架以应对这些挑战。


<details>
  <summary>Details</summary>
Motivation: 现有AI治理框架未能有效解决三大关键挑战：用例级风险评估不足（如Humana诉讼案所示的偏差和高错误率导致的不当索赔拒绝），框架过于概念化缺乏可操作性控制，以及缺乏规模化实施治理的机制。这些问题阻碍了可信赖AI实践的嵌入和合规性衡量。

Method: 本文提出了AI TIPS（人工智能信任集成可持续支柱）2.0框架，这是2019年开发的全面操作框架的更新版本，旨在直接解决上述治理挑战。

Result: 研究成果是引入了AI TIPS 2.0框架，该框架旨在提供一个综合性的操作方法，以解决AI系统部署中面临的风险评估、可操作性控制和规模化治理问题。

Conclusion: AI TIPS 2.0框架旨在弥补当前AI治理框架的不足，通过提供一个全面的操作性解决方案，实现可持续和可信赖的AI系统部署。
...
### [3] [SDialog: A Python Toolkit for End-to-End Agent Building, User Simulation, Dialog Generation, and Evaluation](https://arxiv.org/abs/2512.09142)
...
### [4] [A Categorical Analysis of Large Language Models and Why LLMs Circumvent the Symbol Grounding Problem](https://arxiv.org/abs/2512.09117)
...
TL;DR: 本文提出一种结合高斯过程回归（GPR）的方法，用于在连续动作空间的根并行蒙特卡洛树搜索（MCTS）中聚合来自不同线程的统计数据，实验证明其优于现有策略。


<details>
  <summary>Details</summary>
Motivation: 根并行蒙特卡洛树搜索在时间有限但追求最佳性能时被广泛使用。然而，在连续动作空间中，如何有效聚合来自不同线程的统计数据是一个重要但尚未充分探索的问题。

Method: 引入了一种方法，利用高斯过程回归（GPR）来估计那些有前景但尚未在环境中尝试过的动作的价值。

Result: 在6个不同领域进行的系统评估表明，该方法优于现有的聚合策略，并且仅需要适度增加推理时间。

Conclusion: 所提出的基于GPR的方法能有效解决连续动作空间中根并行MCTS的统计数据聚合问题，从而提升性能。
...
### [9] [An End-to-end Planning Framework with Agentic LLMs and PDDL](https://arxiv.org/abs/2512.09629)
*Emanuele La Malfa,Ping Zhu,Samuele Marro,Sara Bernardini,Michael Wooldridge*

Main category: cs.AI

TL;DR: 本文提出一个端到端、由验证器支持的规划框架，利用大型语言模型（LLMs）将自然语言规范转换为PDDL模型，通过迭代细化和外部规划器生成计划，并将其翻译回自然语言，无需人工干预。


<details>
  <summary>Details</summary>
Motivation: 人类自然语言规范中常存在歧义、矛盾以及时间约束和最优性等规划要求难以直接满足的问题。同时，大型语言模型在处理复杂规划问题（如Blocksworld和Tower of Hanoi）时表现不佳，研究旨在克服这些挑战，实现LLM辅助的端到端规划。

Method: 框架包含一个编排器，将自然语言规范转换为PDDL模型。子模块（代理，由LLMs驱动）迭代细化领域和问题，以解决约束、歧义和矛盾。验证后的PDDL模型传递给外部规划引擎生成计划。最后，一个模块将计划翻译回自然语言以提高可读性。整个过程无需人工干预，并可与任何PDDL规划引擎和验证器集成。

Result: 该框架在各种领域和任务中展现出灵活性和有效性，包括Google NaturalPlan基准、PlanBench以及Blocksworld和Tower of Hanoi等LLMs难以处理的规划问题。它能克服LLMs在小规模实例上的不足，并成功与多种PDDL规划引擎和验证器（如Fast Downward, LPG, POPF, VAL, uVAL）集成。

Conclusion: 该框架代表了在大型语言模型辅助下实现端到端规划的重要一步，为处理复杂规划问题和自然语言规范提供了强大的解决方案。
...
TL;DR: 本研究利用运动学动力学模型在真实环境中评估多智能体路径规划（MAPF）算法，系统探究了方案最优性、模型不准确性及其相互作用对实际执行性能的影响，并提出了未来的研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有的MAPF评估框架依赖于简化的机器人模型，导致算法基准与实际性能之间存在巨大差距。研究旨在通过引入运动学动力学建模，更真实地评估MAPF算法，以理解规划器设计选择如何影响实际性能。

Method: 本研究基于SMART等包含运动学动力学建模的最新框架进行大规模、真实的评估。系统研究了三个核心因素：1) 方案最优性与执行性能的关系；2) 系统性能对运动学动力学模型不准确性的敏感性；3) 模型精度与方案最优性之间的相互作用。通过实证分析来理解这些设计选择如何影响真实场景中的性能。

Result: 通过实证检验，本研究揭示了方案最优性、运动学动力学模型不准确性以及它们之间的相互作用如何影响MAPF在真实场景中的性能。研究结果指出了开放性挑战和未来的研究方向，以指导社区实现实际部署。

Conclusion: 本研究为理解MAPF规划器设计选择在真实执行设置下如何影响性能提供了重要见解，强调了在实际部署中考虑方案最优性与模型准确性的重要性，并为未来的研究指明了方向。
...
TL;DR: 本文提出一个几何框架，用于建模认知异质代理间的信念、动机和影响力，强调结构兼容性而非共享信息或理性。


<details>
  <summary>Details</summary>
Motivation: 研究如何在认知异质的代理之间建模信念、动机和影响力，解释信念扭曲、误解和消失等现象，并整合概念空间、社会认识论和AI价值对齐的见解。

Method: 将每个代理表示为一个个性化的价值空间（向量空间）。信念被形式化为结构化向量，其传播由线性解释映射介导。信念的存活和可理解性取决于其是否避免这些映射的零空间。

Result: 通过代数约束解释了信念扭曲、动机漂移、反事实评估以及相互理解的局限性。提出了“无零空间领导条件”，将领导力定义为表征可达性而非说服力。模型揭示了抽象实体如何在不同认知几何中传播、变异或消失。

Conclusion: 这种认知几何视角通过将意义保存建立在结构兼容性上，而非共享信息或理性，澄清了人类和人工智能系统中影响力的认识论边界，并为分析异质代理间的信念动态提供了一个通用基础。
...
### [12] [RIFT: A Scalable Methodology for LLM Accelerator Fault Assessment using Reinforcement Learning](https://arxiv.org/abs/2512.09829)
*Khurram Khalil,Muhammad Mahad Khaliq,Khaza Anuarul Hoque*

Main category: cs.AI

TL;DR: RIFT是一个可扩展的强化学习引导框架，用于自动化发现AI加速器中最小且高影响的故障场景，以实现高效的设计时故障评估，显著提高速度、覆盖率和硬件保护的成本效益。


<details>
  <summary>Details</summary>
Motivation: 现代AI加速器规模庞大，导致传统故障评估方法计算成本过高，且对关键故障模式的覆盖率不足，因此需要更高效、更全面的故障评估方法。

Method: RIFT将复杂的故障搜索转化为顺序决策问题，结合混合敏感性分析进行搜索空间剪枝，并利用强化学习智能生成最小化、高影响的测试套件。它还生成UVM兼容的验证工件。

Result: 在NVIDIA A100 GPU上对LLM工作负载进行评估，RIFT比进化方法提速2.2倍，将所需测试向量量减少99%以上（相比随机故障注入），同时实现卓越的故障覆盖率。RIFT指导的选择性纠错码比统一三重模块冗余保护的成本效益提高了12.8倍（每单位面积覆盖率）。

Conclusion: RIFT提供了一种可扩展且实用的框架，能够高效地评估AI加速器中的故障，显著提高评估速度和故障覆盖率，并能指导开发更具成本效益的硬件保护策略，其发现可直接集成到商业RTL验证工作流程中。
...
### [14] [Comparing AI Agents to Cybersecurity Professionals in Real-World Penetration Testing](https://arxiv.org/abs/2512.09882)
...
### [17] [Mitigating Bias with Words: Inducing Demographic Ambiguity in Face Recognition Templates by Text Encoding](https://arxiv.org/abs/2512.08981)
*Tahar Chettaoui,Naser Damer,Fadi Boutros*

Main category: cs.CV

TL;DR: 本文提出了一种名为统一文本-图像嵌入（UTIE）的新策略，利用视觉-语言模型（VLM）通过将其他人口统计群体的文本派生特征注入人脸嵌入中，来减少人脸识别（FR）系统中的人口统计偏差，从而提高跨群体验证的公平性。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统常出现人口统计偏差，部分原因是人口统计特定信息与身份相关特征在人脸嵌入中纠缠不清。在大规模多文化城市中，尤其是在生物识别技术在智慧城市基础设施中扮演重要角色的地方，这种偏差尤为关键，因为它会导致不同人口统计群体之间验证性能的差异。

Method: 研究者提出了一种名为统一文本-图像嵌入（UTIE）的新策略。该方法旨在通过用与其他人口统计群体相关的信息来丰富人脸嵌入，从而在嵌入中引入人口统计模糊性。UTIE利用视觉-语言模型（VLM）的零样本能力和跨模态语义对齐特性，将从其他人口统计群体文本中提取的人口统计特征注入到每个群体的面部嵌入中，以促进更中性的人口统计属性表示。

Result: 研究在RFW和BFW这两个评估FR偏差的基准上，使用CLIP、OpenCLIP和SigLIP这三种VLM对UTIE进行了评估。实验结果表明，UTIE持续降低了偏差指标，同时保持甚至在某些情况下提高了人脸验证的准确性。

Conclusion: UTIE是一种有效的方法，可以通过在人脸嵌入中引入人口统计模糊性，从而显著减少人脸识别系统中的人口统计偏差，并在不牺牲准确性的前提下实现更公平的验证性能。
...
### [18] [Training Multi-Image Vision Agents via End2End Reinforcement Learning](https://arxiv.org/abs/2512.08980)
...
TL;DR: IMAgent是一个开源的、通过端到端强化学习训练的视觉代理，专为复杂多图像任务设计。它通过多智能体系统生成多图像QA数据，并引入视觉反思工具，在多图像任务上表现出色，无需监督微调。


<details>
  <summary>Details</summary>
Motivation: 现有基于VLM的代理大多限制于单图像输入，无法有效处理真实世界中的多图像问答任务，从而限制了其在复制OpenAI O3“通过图像思考”能力方面的潜力。

Method: 本研究提出了IMAgent，一个通过端到端强化学习训练的开源视觉代理。它利用多智能体系统生成具有挑战性的多图像QA对，构建了MIFG-QA数据集（1万个样本）。为防止VLM忽视视觉输入，开发了两种专门的视觉反思和确认工具。此外，通过精心设计的动作轨迹两级掩码策略，IMAgent实现了稳定的工具使用行为，且无需监督微调数据。

Result: IMAgent在现有单图像基准测试上保持了强大性能，并在所提出的多图像数据集上取得了显著改进。研究分析为社区提供了可操作的见解。

Conclusion: IMAgent成功解决了多图像QA任务的挑战，通过其独特的强化学习训练范式、多图像数据生成方法和专门的视觉工具，在单图像和多图像任务上均展现出卓越性能，为VLM代理在复杂真实世界场景中的应用提供了有效方案。
...
### [19] [What Happens When: Learning Temporal Orders of Events in Videos](https://arxiv.org/abs/2512.08979)
*Daechul Ahn,Yura Choi,Hyeonbeom Choi,Seongwon Cho,San Kim,Jonghyun Choi*

Main category: cs.CV

TL;DR: 研究发现视频大模型（VLMMs）在现有基准上即使帧被打乱也能表现良好，表明其对事件时间顺序的理解不足。本文提出VECTOR基准来评估时间理解能力，并引入MECOT方法（多事件指令微调与思维链）来提升VLMMs的事件时间顺序理解能力，MECOT在VECTOR及现有基准上均表现优异。


<details>
  <summary>Details</summary>
Motivation: 视频大模型在视频理解方面表现出色，但其准确捕捉多个事件时间顺序的能力尚未被充分探索。研究发现，即使视频帧被打乱，模型在现有基准上仍能表现良好，这暗示VLMMs可能并非真正依赖于事件的顺序处理，而是依赖于对典型场景的先验知识来回答问题。因此，需要专门评估并提升其时间理解能力。

Method: 1. 提出**VECTOR**基准，专门用于评估VLMMs识别事件时间顺序的能力。2. 提出**MECOT**（Multi-Event instruction fine-tuning with Chain-of-Thought）方法，通过以下两点提升模型的时间意识：(1) 使用详细的、逐事件的视频描述来训练模型；(2) 在推理时使用思维链（Chain-of-Thought）提示。

Result: 1. 各种VLMMs在VECTOR基准上经常无法理解事件的顺序。2. MECOT方法在VECTOR基准上超越了现有技术。3. MECOT还提升了模型在现有视频基准上的性能，证明了其时间理解的有效性。

Conclusion: 视频大模型在理解事件时间顺序方面存在局限性，它们可能更多依赖于先验知识而非真实的顺序处理。本文提出的VECTOR基准有效揭示了这一问题，而MECOT方法通过多事件指令微调和思维链提示，显著增强了VLMMs的时间理解能力，并在多个基准上取得了更好的表现。
...
### [20] [Consist-Retinex: One-Step Noise-Emphasized Consistency Training Accelerates High-Quality Retinex Enhancement](https://arxiv.org/abs/2512.08982)
*Jian Xu,Wei Chen,Shigui Li,Delu Zeng,John Paisley,Qibin Zhao*

Main category: cs.CV

TL;DR: 本文提出了Consist-Retinex，首次将一致性模型应用于基于Retinex的低光图像增强，实现了单步采样下的SOTA性能，同时大幅减少了训练预算。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的低光图像增强方法需要数百次迭代采样，严重限制了实际部署。尽管一致性模型在无条件合成中实现了有前景的单步生成，但其在条件增强领域的应用尚未被探索。条件增强与无条件生成在训练动态上存在根本差异，前者更依赖于连接退化输入与增强输出的大噪声区域。

Method: 本文提出了Consist-Retinex框架，包含两项核心创新：1) 双目标一致性损失，结合了时间一致性与随机时间采样下的真值对齐，提供了全谱监督以实现稳定收敛；2) 自适应噪声强调采样策略，优先训练对单步条件生成至关重要的大噪声区域。

Result: 在VE-LOL-L数据集上，Consist-Retinex在单步采样下实现了最先进的性能（PSNR: 25.51 vs. Diff-Retinex++的23.41；FID: 44.73 vs. Diff-Retinex++的49.59）。同时，其训练预算仅为1000步Diff-Retinex基线的1/8。

Conclusion: Consist-Retinex成功将一致性模型应用于低光图像增强，解决了扩散模型迭代采样耗时的问题，在实现单步高效生成的同时，大幅提升了性能并降低了训练成本。
...
### [21] [HSCP: A Two-Stage Spectral Clustering Framework for Resource-Constrained UAV Identification](https://arxiv.org/abs/2512.08983)
*Maoyu Wang,Yao Lu,Bo Zhou,Zhuangzhi Chen,Yun Lin,Qi Xuan,Guan Gui*

Main category: cs.CV

TL;DR: 本文提出HSCP框架，结合层剪枝和通道剪枝，通过谱聚类和CKA指导，实现深度学习模型在无人机射频指纹识别（RFFI）任务上的极端压缩、高性能和鲁健性，适用于资源受限的边缘设备。


<details>
  <summary>Details</summary>
Motivation: 传统的无人机识别方法在复杂环境中难以提取可靠信号特征并满足实时性，而基于深度学习的RFFI方法虽然提高了识别精度，但其巨大的模型尺寸和高计算需求阻碍了在资源受限边缘设备上的部署。现有模型剪枝技术难以同时优化压缩率、硬件加速和识别精度。

Method: 本文提出了HSCP（分层谱聚类剪枝）框架，结合层剪枝和通道剪枝。第一阶段，HSCP利用由中心核对齐（CKA）指导的谱聚类来识别并移除冗余层。随后，相同的策略应用于通道维度以消除更精细的冗余。为确保鲁棒性，研究进一步采用了噪声鲁棒的微调策略。

Result: 在UAV-M100基准测试中，HSCP优于现有通道和层剪枝方法。具体而言，在ResNet18上，HSCP实现了86.39%的参数减少和84.44%的FLOPs减少，同时相比未剪枝基线，准确率提高了1.49%，并且在低信噪比环境下仍保持卓越的鲁棒性。

Conclusion: HSCP框架通过结合分层剪枝策略和噪声鲁棒微调，成功解决了深度学习RFFI模型在边缘设备部署中的挑战，实现了极高的模型压缩率、性能提升以及在复杂环境下的鲁棒性。
...
### [22] [RAG-HAR: Retrieval Augmented Generation-based Human Activity Recognition](https://arxiv.org/abs/2512.08984)
*Nirhoshan Sivaroopan,Hansi Karunarathna,Chamara Madarasingha,Anura Jayasumana,Kanchana Thilakarathna*

Main category: cs.CV

TL;DR: RAG-HAR是一个无需训练的检索增强框架，利用大型语言模型（LLMs）进行人类活动识别（HAR），通过轻量级统计描述符和上下文证据实现，并在多个基准测试中达到最先进的性能，同时能识别未见过的活动。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习HAR方法需要针对特定数据集进行训练、大量的标注数据以及显著的计算资源，这限制了其实用性和泛化能力。

Method: RAG-HAR框架首先计算轻量级统计描述符，然后从向量数据库中检索语义相似的样本作为上下文证据，并利用LLM进行活动识别。该方法通过提示优化和引入LLM驱动的活动描述符进一步增强，以生成上下文丰富的向量数据库，提供更准确和相关的上下文信息。

Result: RAG-HAR在六个不同的HAR基准测试中取得了最先进的性能。最重要的是，它在不进行模型训练或微调的情况下实现了这些改进。此外，RAG-HAR能够识别并有意义地标记多种未见过的（unseen）人类活动。

Conclusion: RAG-HAR是一个鲁棒且实用的HAR框架，无需训练即可达到SOTA性能，并能处理未知活动，超越了传统训练依赖型HAR方法的局限性。
...
### [23] [An Efficient Test-Time Scaling Approach for Image Generation](https://arxiv.org/abs/2512.08985)
...
### [24] [Explainable Fundus Image Curation and Lesion Detection in Diabetic Retinopathy](https://arxiv.org/abs/2512.08986)
...
### [25] [3DID: Direct 3D Inverse Design for Aerodynamics with Physics-Aware Optimization](https://arxiv.org/abs/2512.08987)
...
### [26] [Deterministic World Models for Verification of Closed-loop Vision-based Systems](https://arxiv.org/abs/2512.08991)
...
### [27] [Enhancing Knowledge Transfer in Hyperspectral Image Classification via Cross-scene Knowledge Integration](https://arxiv.org/abs/2512.08989)
...
### [28] [LiM-YOLO: Less is More with Pyramid Level Shift and Normalized Auxiliary Branch for Ship Detection in Optical Remote Sensing Imagery](https://arxiv.org/abs/2512.09700)
...
### [29] [Demo: Generative AI helps Radiotherapy Planning with User Preference](https://arxiv.org/abs/2512.08996)
*Riqiang Gao,Simon Arberet,Martin Kraus,Han Liu,Wilko FAR Verbakel,Dorin Comaniciu,Florin-Cristian Ghesu,Ali Kamen*

Main category: cs.CV

TL;DR: 本文提出了一种新型生成模型，通过用户定义的偏好预测3D剂量分布，解决了传统深度学习模型依赖参考计划导致的偏倚问题，提高了放疗计划的灵活性和个性化。


<details>
  <summary>Details</summary>
Motivation: 现有的3D剂量预测深度学习方法依赖于参考计划作为训练真值，这可能导致模型偏向特定的计划风格或机构偏好。研究旨在开发一种更灵活、个性化的方法，允许用户自定义偏好。

Method: 引入了一种新颖的生成模型，该模型仅基于用户定义的偏好（如OAR和PTV之间的权衡）来预测3D剂量分布。这种方法旨在无缝集成到临床治疗计划系统中。

Result: 该方法通过可定制的偏好提供了更大的灵活性和个性化，能够帮助用户高效生成高质量的计划。在某些情况下，其在适应性和计划质量方面超越了Varian RapidPlan模型。

Conclusion: 所提出的生成模型能够根据用户偏好生成高质量的3D剂量分布，克服了传统方法的局限性，提供了更灵活、个性化的放疗计划解决方案，并在性能上可能优于现有商业模型。
...
### [30] [Diffusion Model Regularized Implicit Neural Representation for CT Metal Artifact Reduction](https://arxiv.org/abs/2512.08999)
*Jie Wen,Chenhe Du,Xiao Wang,Yuyao Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种基于扩散模型正则化的隐式神经表示框架，用于金属伪影去除(MAR)，解决了现有监督方法对有限配对数据依赖的性能不稳定性问题，以及无监督方法未能有效整合CT物理几何和充分利用先验知识的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的监督金属伪影去除(MAR)方法因依赖有限的配对金属-清洁数据，导致在已知数据上的性能不稳定，限制了临床应用。同时，现有的无监督方法面临两大挑战：1) 未能有效将CT物理几何融入MAR过程以确保数据保真度；2) 传统的启发式正则化项无法充分捕捉丰富的先验知识。

Method: 本文提出了一种扩散模型正则化的隐式神经表示框架用于MAR。该框架通过隐式神经表示整合物理约束并强制实现数据保真度，同时利用预训练的扩散模型提供先验知识来正则化解决方案。

Result: 在模拟和临床数据上的实验结果表明，该方法有效且具有良好的泛化能力。

Conclusion: 该方法在临床环境中具有应用潜力，有望解决金属伪影问题。
...
### [31] [A Physics-Constrained, Design-Driven Methodology for Defect Dataset Generation in Optical Lithography](https://arxiv.org/abs/2512.09001)
*Yuehua Hu,Jiyeong Kong,Dong-yeol Shin,Jaekyun Kim,Kyung-Tae Kang*

Main category: cs.CV

TL;DR: 本研究提出了一种生成大规模、物理有效且带有像素级标注的光刻缺陷数据集的新方法，以解决AI在微纳制造缺陷检测中高质量训练数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 人工智能在微纳制造中的效率受限于高质量、物理基础训练数据的稀缺，特别是半导体行业的光刻缺陷数据难以获取，导致公开数据集不足。

Method: 该方法首先利用受物理约束的数学形态学操作（腐蚀和膨胀）对原始设计布局进行缺陷布局的从头合成。然后，通过高保真数字微镜器件（DMD）光刻将这些合成的缺陷布局及其无缺陷对应物制造成物理样本。最后，比较缺陷样本和无缺陷参考的显微照片，以创建一致的缺陷描绘标注。基于此方法，构建了一个包含3,530张光学显微照片和13,365个标注缺陷实例（包括桥接、毛刺、收缩和污染四类）的综合数据集。

Result: 构建了一个包含四类缺陷的像素级精确分割掩模数据集。使用该数据集，Mask R-CNN在桥接、毛刺和收缩类别上的AP@0.5分别达到0.980、0.965和0.971，相比Faster R-CNN的0.740、0.719和0.717，平均AP@0.5提高了约34%。对于污染类别，Mask R-CNN的AP@0.5比Faster R-CNN高出约42%。

Conclusion: 这些持续的性能提升表明，所提出的生成带有像素级标注的缺陷数据集的方法对于半导体制造中基于AI的鲁棒测量/检测是可行的。
...
### [32] [Towards Lossless Ultimate Vision Token Compression for VLMs](https://arxiv.org/abs/2512.09010)
*Dehua Zheng,Mouxiao Huang,Borui Jiang,Hailin Hu,Xinghao Chen*

Main category: cs.CV

TL;DR: 本文提出LUVC框架，通过视觉编码器中的迭代合并和LLM中的无注意力频谱剪枝，实现视觉语言模型（VLM）中视觉token的无损压缩，将高维视觉特征逐步融入多模态查询，从而在LLM推理中实现2倍加速且精度损失可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像和视频的token表示存在大量冗余，导致视觉语言模型（VLM）计算效率低下和延迟高。现有基于注意力/相似度的压缩算法存在位置偏差或类别不平衡问题，导致精度显著下降，且无法推广到交叉模态交互较弱的浅层LLM。

Method: 本文将token压缩扩展到视觉编码器，采用有效的空间正交迭代合并方案。此外，通过无注意力/相似度的低通滤波器，将频谱剪枝单元整合到LLM中，该单元逐步剪枝冗余视觉token并与FlashAttention完全兼容。在此基础上，提出了无损终极视觉token压缩（LUVC）框架，该框架系统地压缩视觉token，直至在LLM的最后一层完全消除，使高维视觉特征逐渐融合到多模态查询中。

Result: 实验表明，LUVC在语言模型中实现了2倍的推理加速，且精度下降可忽略不计。其免训练特性使其能够立即部署到多个VLM中。

Conclusion: LUVC通过创新的视觉token压缩方案，有效解决了VLM的计算效率和延迟问题，在不牺牲精度的情况下显著提升了推理速度，并具备即时部署的优势。
...
### [33] [An Automated Tip-and-Cue Framework for Optimized Satellite Tasking and Visual Intelligence](https://arxiv.org/abs/2512.09670)
*Gil Weissman,Amir Ivry,Israel Cohen*

Main category: cs.CV

TL;DR: 本文提出了一种全自动“提示-线索”（Tip-and-Cue）框架，用于卫星图像任务规划、调度和分析，通过海事船只追踪场景验证了其有效性，并可扩展至更广泛的应用。


<details>
  <summary>Details</summary>
Motivation: 卫星星座的普及、任务延迟的减少以及多样化的传感器能力，为自动化地球观测提供了更多机会。研究旨在开发一个全自动的系统来应对这些机遇。

Method: 该框架包括：1. 从外部数据或历史图像分析生成“提示”，识别并优先处理时空目标；2. 依据传感器限制、时间要求和效用函数制定相应的“线索”（成像任务）；3. 自动生成候选任务，并利用连续效用函数优化多颗卫星的任务调度；4. 使用基于人工智能的模型（如目标检测器和视觉-语言模型）处理图像；5. 生成结构化视觉报告以支持解释性和洞察发现。

Result: 该框架通过一个海事船只追踪场景验证了其有效性，利用自动识别系统（AIS）数据进行轨迹预测、目标观测和可操作性输出。它被证明能够生成有价值的成果。

Conclusion: 该全自动“提示-线索”框架能有效支持卫星成像任务规划、调度和分析，并可扩展应用于智慧城市监控和灾害响应等需要及时任务执行和自动化分析的领域。
...
### [34] [A Survey of Body and Face Motion: Datasets, Performance Evaluation Metrics and Generative Techniques](https://arxiv.org/abs/2512.09005)
...
### [35] [Learning to Remove Lens Flare in Event Camera](https://arxiv.org/abs/2512.09016)
...
### [36] [An Approach for Detection of Entities in Dynamic Media Contents](https://arxiv.org/abs/2512.09011)
...
### [37] [ConceptPose: Training-Free Zero-Shot Object Pose Estimation using Concept Vectors](https://arxiv.org/abs/2512.09056)
...
### [38] [SIP: Site in Pieces- A Dataset of Disaggregated Construction-Phase 3D Scans for Semantic Segmentation and Scene Understanding](https://arxiv.org/abs/2512.09062)
...
### [39] [KD-OCT: Efficient Knowledge Distillation for Clinical-Grade Retinal OCT Classification](https://arxiv.org/abs/2512.09069)
...
### [40] [AgentComp: From Agentic Reasoning to Compositional Mastery in Text-to-Image Models](https://arxiv.org/abs/2512.09081)
...
### [41] [Adaptive Thresholding for Visual Place Recognition using Negative Gaussian Mixture Statistics](https://arxiv.org/abs/2512.09071)
...
### [42] [Explaining the Unseen: Multimodal Vision-Language Reasoning for Situational Awareness in Underground Mining Disasters](https://arxiv.org/abs/2512.09092)
...
### [43] [Food Image Generation on Multi-Noun Categories](https://arxiv.org/abs/2512.09095)
...
### [44] [Integrated Pipeline for Coronary Angiography With Automated Lesion Profiling, Virtual Stenting, and 100-Vessel FFR Validation](https://arxiv.org/abs/2512.09134)
...
### [45] [WonderZoom: Multi-Scale 3D World Generation](https://arxiv.org/abs/2512.09164)
...
### [46] [GimbalDiffusion: Gravity-Aware Camera Control for Video Generation](https://arxiv.org/abs/2512.09112)
...
### [47] [Prompt-Based Continual Compositional Zero-Shot Learning](https://arxiv.org/abs/2512.09172)
...
### [48] [GTAvatar: Bridging Gaussian Splatting and Texture Mapping for Relightable and Editable Gaussian Avatars](https://arxiv.org/abs/2512.09162)
...
### [49] [SuperF: Neural Implicit Fields for Multi-Image Super-Resolution](https://arxiv.org/abs/2512.09115)
...
### [50] [Learning Patient-Specific Disease Dynamics with Latent Flow Matching for Longitudinal Imaging Generation](https://arxiv.org/abs/2512.09185)
...
### [51] [View-on-Graph: Zero-shot 3D Visual Grounding via Vision-Language Reasoning on Scene Graphs](https://arxiv.org/abs/2512.09215)
...
### [52] [Enabling Next-Generation Consumer Experience with Feature Coding for Machines](https://arxiv.org/abs/2512.09232)
...
### [53] [GLACIA: Instance-Aware Positional Reasoning for Glacial Lake Segmentation via Multimodal Large Language Model](https://arxiv.org/abs/2512.09251)
...
### [54] [Efficient Feature Compression for Machines with Global Statistics Preservation](https://arxiv.org/abs/2512.09235)
...
### [55] [Log NeRF: Comparing Spaces for Learning Radiance Fields](https://arxiv.org/abs/2512.09375)
...
### [56] [A Clinically Interpretable Deep CNN Framework for Early Chronic Kidney Disease Prediction Using Grad-CAM-Based Explainable AI](https://arxiv.org/abs/2512.09244)
...
### [57] [OmniPSD: Layered PSD Generation with Diffusion Transformer](https://arxiv.org/abs/2512.09247)
...
### [58] [Representation Calibration and Uncertainty Guidance for Class-Incremental Learning based on Vision Language Model](https://arxiv.org/abs/2512.09441)
...
### [60] [ROI-Packing: Efficient Region-Based Compression for Machine Vision](https://arxiv.org/abs/2512.09258)
...
### [61] [Privacy-Preserving Computer Vision for Industry: Three Case Studies in Human-Centric Manufacturing](https://arxiv.org/abs/2512.09463)
...
### [62] [MoRel: Long-Range Flicker-Free 4D Motion Modeling via Anchor Relay-based Bidirectional Blending with Hierarchical Densification](https://arxiv.org/abs/2512.09270)
...
### [63] [Temporal-Spatial Tubelet Embedding for Cloud-Robust MSI Reconstruction using MSI-SAR Fusion: A Multi-Head Self-Attention Video Vision Transformer Approach](https://arxiv.org/abs/2512.09471)
...
### [64] [Color encoding in Latent Space of Stable Diffusion Models](https://arxiv.org/abs/2512.09477)
...
### [65] [LongT2IBench: A Benchmark for Evaluating Long Text-to-Image Generation with Graph-structured Annotations](https://arxiv.org/abs/2512.09271)
...
### [66] [Dynamic Facial Expressions Analysis Based Parkinson's Disease Auxiliary Diagnosis](https://arxiv.org/abs/2512.09276)
...
### [68] [LoGoColor: Local-Global 3D Colorization for 360° Scenes](https://arxiv.org/abs/2512.09278)
...
### [69] [FoundIR-v2: Optimizing Pre-Training Data Mixtures for Image Restoration Foundation Model](https://arxiv.org/abs/2512.09282)
...
### [71] [Rethinking Chain-of-Thought Reasoning for Videos](https://arxiv.org/abs/2512.09616)
...
### [73] [MelanomaNet: Explainable Deep Learning for Skin Lesion Classification](https://arxiv.org/abs/2512.09289)
...
### [74] [Composing Concepts from Images and Videos via Concept-prompt Binding](https://arxiv.org/abs/2512.09824)
...
### [76] [Traffic Scene Small Target Detection Method Based on YOLOv8n-SPTS Model for Autonomous Driving](https://arxiv.org/abs/2512.09296)
*Songhan Wu*
...
### [77] [VABench: A Comprehensive Benchmark for Audio-Video Generation](https://arxiv.org/abs/2512.09299)
...
### [78] [From SAM to DINOv2: Towards Distilling Foundation Models to Lightweight Baselines for Generalized Polyp Segmentation](https://arxiv.org/abs/2512.09307)
...
### [79] [Transformer-Driven Multimodal Fusion for Explainable Suspiciousness Estimation in Visual Surveillance](https://arxiv.org/abs/2512.09311)
...
### [80] [Benchmarking Real-World Medical Image Classification with Noisy Labels: Challenges, Practice, and Outlook](https://arxiv.org/abs/2512.09315)
...
### [81] [Relightable and Dynamic Gaussian Avatar Reconstruction from Monocular Video](https://arxiv.org/abs/2512.09335)
...
### [82] [UniLS: End-to-End Audio-Driven Avatars for Unified Listening and Speaking](https://arxiv.org/abs/2512.09327)
...
### [83] [TextGuider: Training-Free Guidance for Text Rendering via Attention Alignment](https://arxiv.org/abs/2512.09350)
...
### [84] [Video-QTR: Query-Driven Temporal Reasoning Framework for Lightweight Video Understanding](https://arxiv.org/abs/2512.09354)
...
### [85] [ASSIST-3D: Adapted Scene Synthesis for Class-Agnostic 3D Instance Segmentation](https://arxiv.org/abs/2512.09364)
...
### [86] [StereoWorld: Geometry-Aware Monocular-to-Stereo Video Generation](https://arxiv.org/abs/2512.09363)
...
### [87] [FUSER: Feed-Forward MUltiview 3D Registration Transformer and SE(3)$^N$ Diffusion Refinement](https://arxiv.org/abs/2512.09373)
...
### [88] [Perception-Inspired Color Space Design for Photo White Balance Editing](https://arxiv.org/abs/2512.09383)
...
### [89] [Detection and Localization of Subdural Hematoma Using Deep Learning on Computed Tomography](https://arxiv.org/abs/2512.09393)
...
### [90] [Wasserstein-Aligned Hyperbolic Multi-View Clustering](https://arxiv.org/abs/2512.09402)
...
### [91] [Generative Point Cloud Registration](https://arxiv.org/abs/2512.09407)
*Haobo Jiang,Jin Xie,Jian Yang,Liang Yu,Jianmin Zheng*
...
### [92] [DirectSwap: Mask-Free Cross-Identity Training and Benchmarking for Expression-Consistent Video Head Swapping](https://arxiv.org/abs/2512.09417)
...
### [93] [FunPhase: A Periodic Functional Autoencoder for Motion Generation via Phase Manifolds](https://arxiv.org/abs/2512.09423)
...
### [94] [Label-free Motion-Conditioned Diffusion Model for Cardiac Ultrasound Synthesis](https://arxiv.org/abs/2512.09418)
...
### [95] [UniPart: Part-Level 3D Generation with Unified 3D Geom-Seg Latents](https://arxiv.org/abs/2512.09435)
...
### [96] [InfoMotion: A Graph-Based Approach to Video Dataset Distillation for Echocardiography](https://arxiv.org/abs/2512.09422)
...
### [97] [Defect-aware Hybrid Prompt Optimization via Progressive Tuning for Zero-Shot Multi-type Anomaly Detection and Segmentation](https://arxiv.org/abs/2512.09446)
...
### [98] [MODA: The First Challenging Benchmark for Multispectral Object Detection in Aerial Images](https://arxiv.org/abs/2512.09489)
...
### [99] [Gradient-Guided Learning Network for Infrared Small Target Detection](https://arxiv.org/abs/2512.09497)
...
### [100] [Masked Registration and Autoencoding of CT Images for Predictive Tibia Reconstruction](https://arxiv.org/abs/2512.09525)
...
### [101] [Seeing Soil from Space: Towards Robust and Scalable Remote Soil Nutrient Analysis](https://arxiv.org/abs/2512.09576)
*David Seu,Nicolas Longepe,Gabriel Cioltea,Erik Maidik,Calin Andrei*

Main category: cs.CV

TL;DR: 本研究开发了一个可扩展的混合模型系统，利用遥感数据、物理信息协变量和基础模型，准确估算农田土壤有机碳、总氮、有效磷、交换性钾和pH值，并在欧洲农田数据上进行了严格验证。


<details>
  <summary>Details</summary>
Motivation: 环境因素日益影响农业决策，但目前土壤评估工具的可及性和可扩展性有限。

Method: 该系统采用混合建模方法，结合通过代理和驱动因素的间接土壤建模与直接光谱建模。通过辐射传输模型(RTMs)派生可解释的物理信息协变量，并利用基础模型生成复杂的非线性嵌入。在涵盖欧洲不同土壤气候区的农田统一数据集上进行验证，采用严格的空间阻塞、分层拆分和统计上独立的训练-测试集进行评估，并通过共形校准评估不确定性。

Result: 模型在土壤有机碳(SOC)和总氮(N)方面取得了最高精度。SOC的平均绝对误差(MAE)为5.12 g/kg，一致性相关系数(CCC)为0.77；N的MAE为0.44 g/kg，CCC为0.77。在空间交叉验证和独立测试集中，该性能在未见区域均保持良好。不确定性评估在目标置信水平下达到了90%的覆盖率。

Conclusion: 这项研究通过应用可扩展的、数据驱动的土壤分析框架，促进了农业的数字化发展，并可扩展到需要定量土壤评估的相关领域，如碳市场。
...
### [102] [StateSpace-SSL: Linear-Time Self-supervised Learning for Plant Disease Detectio](https://arxiv.org/abs/2512.09492)
...
### [103] [Building Reasonable Inference for Vision-Language Models in Blind Image Quality Assessment](https://arxiv.org/abs/2512.09555)
...
### [104] [A Dual-Domain Convolutional Network for Hyperspectral Single-Image Super-Resolution](https://arxiv.org/abs/2512.09546)
...
### [105] [From Graphs to Gates: DNS-HyXNet, A Lightweight and Deployable Sequential Model for Real-Time DNS Tunnel Detection](https://arxiv.org/abs/2512.09565)
...
### [106] [Content-Adaptive Image Retouching Guided by Attribute-Based Text Representation](https://arxiv.org/abs/2512.09580)
...
### [107] [Investigate the Low-level Visual Perception in Vision-Language based Image Quality Assessment](https://arxiv.org/abs/2512.09573)
*Yuan Li,Zitang Sun,Yen-Ju Chen,Shin'ya Nishida*
...
### [108] [UnReflectAnything: RGB-Only Highlight Removal by Rendering Synthetic Specular Supervision](https://arxiv.org/abs/2512.09583)
...
### [109] [FROMAT: Multiview Material Appearance Transfer via Few-Shot Self-Attention Adaptation](https://arxiv.org/abs/2512.09617)
...
### [110] [Kaapana: A Comprehensive Open-Source Platform for Integrating AI in Medical Imaging Research Environments](https://arxiv.org/abs/2512.09644)
...
### [111] [Benchmarking SAM2-based Trackers on FMOX](https://arxiv.org/abs/2512.09633)
...
### [112] [Beyond Sequences: A Benchmark for Atomic Hand-Object Interaction Using a Static RNN Encoder](https://arxiv.org/abs/2512.09626)
...
### [113] [VHOI: Controllable Video Generation of Human-Object Interactions from Sparse Trajectories via Motion Densification](https://arxiv.org/abs/2512.09646)
...
### [114] [IF-Bench: Benchmarking and Enhancing MLLMs for Infrared Images with Generative Visual Prompting](https://arxiv.org/abs/2512.09663)
...
### [115] [Stylized Meta-Album: Group-bias injection with style transfer to study robustness against distribution shifts](https://arxiv.org/abs/2512.09773)
...
### [116] [OxEnsemble: Fair Ensembles for Low-Data Classification](https://arxiv.org/abs/2512.09665)
...
### [117] [Unconsciously Forget: Mitigating Memorization; Without Knowing What is being Memorized](https://arxiv.org/abs/2512.09687)
...
### [118] [FastPose-ViT: A Vision Transformer for Real-Time Spacecraft Pose Estimation](https://arxiv.org/abs/2512.09792)
...
### [119] [Modality-Specific Enhancement and Complementary Fusion for Semi-Supervised Multi-Modal Brain Tumor Segmentation](https://arxiv.org/abs/2512.09801)
...
### [120] [UniUGP: Unifying Understanding, Generation, and Planing For End-to-end Autonomous Driving](https://arxiv.org/abs/2512.09864)
...
### [121] [Diffusion Posterior Sampler for Hyperspectral Unmixing with Spectral Variability Modeling](https://arxiv.org/abs/2512.09871)
...
### [122] [Benchmarking Document Parsers on Mathematical Formula Extraction from PDFs](https://arxiv.org/abs/2512.09874)
...
### [123] [Splatent: Splatting Diffusion Latents for Novel View Synthesis](https://arxiv.org/abs/2512.09923)
...
TL;DR: Splatent是一个基于扩散的增强框架，通过在VAE潜在空间中利用多视角注意力从2D输入视图恢复细节，解决了VAE潜在空间中3D高斯泼溅的多视角不一致性问题，从而提高了辐射场重建的质量。


<details>
  <summary>Details</summary>
Motivation: 将辐射场表示与VAE潜在空间结合虽然能实现高效渲染并与扩散模型无缝集成，但VAE潜在空间缺乏多视角一致性，导致3D重建中纹理模糊和细节缺失。现有方法（微调VAE或依赖预训练扩散模型）各有弊端，如牺牲重建质量或引入幻觉。

Method: 本文提出了Splatent，一个基于扩散的增强框架，它在VAE的潜在空间中与3D高斯泼溅（3DGS）协同工作。其核心思想是，不通过3D空间重建精细细节，而是通过多视角注意力机制从2D输入视图中恢复这些细节。

Result: Splatent在多个基准测试中，为VAE潜在辐射场重建设立了新的SOTA。此外，将Splatent与现有前馈框架集成，能持续改进细节保留。

Conclusion: Splatent在实现忠实细节恢复的同时，保留了预训练VAE的重建质量。该方法为高质量稀疏视角3D重建开辟了新的可能性。
...
### [124] [GAINS: Gaussian-based Inverse Rendering from Sparse Multi-View Captures](https://arxiv.org/abs/2512.09925)
...
### [125] [From Detection to Anticipation: Online Understanding of Struggles across Various Tasks and Activities](https://arxiv.org/abs/2512.09847)
...
### [126] [DynaIP: Dynamic Image Prompt Adapter for Scalable Zero-shot Personalized Text-to-Image Generation](https://arxiv.org/abs/2512.09814)
...
### [127] [NordFKB: a fine-grained benchmark dataset for geospatial AI in Norway](https://arxiv.org/abs/2512.09913)
...
### [128] [VisualActBench: Can VLMs See and Act like a Human?](https://arxiv.org/abs/2512.09907)
...
### [129] [ReViSE: Towards Reason-Informed Video Editing in Unified Models with Self-Reflective Learning](https://arxiv.org/abs/2512.09924)
...
### [130] [Noise-Robust Abstractive Compression in Retrieval-Augmented Language Models](https://arxiv.org/abs/2512.08943)
*Singon Kim*

Main category: cs.CL

TL;DR: 抽象压缩在RAG中存在噪音（不相关/误导信息）和遗漏关键信息的问题。ACoRN通过离线数据增强和微调两个训练步骤，提高了压缩器对噪音的鲁棒性，并使其生成以关键信息为中心的摘要，从而显著提升了RAG的性能，尤其是在文档噪音较多的场景下。


<details>
  <summary>Details</summary>
Motivation: 抽象压缩在检索增强生成（RAG）中用于精简上下文以降低计算成本。然而，检索到的文档常包含不相关或误导性信息，即使相关性得分高，这导致压缩器容易遗漏正确答案所需的关键信息，尤其是在长上下文中，注意力分散问题更为突出。

Method: 本文提出了ACoRN（Abstractive Compression Robust against Noise），引入了两个新颖的训练步骤：1. 对训练数据集进行离线数据增强，以增强压缩器对两种不同类型检索噪音的鲁棒性。2. 进行微调，使语言模型基础的压缩器生成围绕直接支持正确答案的关键信息的摘要，以解决其无法充分利用多个检索文档和位置偏差的问题。

Result: 实验表明，使用ACoRN训练的T5-large作为压缩器，提高了EM和F1分数，并保留了作为直接证据的答案字符串。ACoRN在包含大量降低准确性文档的数据集上表现出色。

Conclusion: ACoRN有效提高了抽象压缩在RAG中对抗噪音的鲁棒性，通过关注关键信息和增强对噪音的抵抗力，显著改善了性能。这使得ACoRN在真实世界的场景中，尤其是在检索结果嘈杂时，具有高度实用性。
...
### [131] [Enhancing Reliability across Short and Long-Form QA via Reinforcement Learning](https://arxiv.org/abs/2512.08944)
*Yudong Wang,Zhe Yang,Wenhan Ma,Zhifang Sui,Liang Zhao*

Main category: cs.CL

TL;DR: 本研究提出了一种有针对性的强化学习框架，旨在减少大型语言模型在问答任务中的内在和外在幻觉。通过使用特定数据集和奖励机制（包括拒绝回答无法回答的问题），该框架显著提高了模型的可靠性和性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习在提升大型语言模型的复杂推理能力的同时，也加剧了其产生幻觉的倾向，导致能力与可靠性之间出现关键的权衡。研究旨在解决这一挑战，减轻内在和外在幻觉。

Method: 引入了一个有针对性的强化学习框架：1. 通过从TriviaQA的开放式转换中创建新颖的训练集来解决外在幻觉（错误的内部知识）。2. 利用FineWeb中的长篇文本，通过事实依据奖励机制来解决内在幻觉（对上下文的不忠实）。3. 明确奖励模型拒绝回答无法回答的问题，以培养谨慎性，进一步增强可靠性。

Result: 广泛的实验表明，该方法在各种基准测试中取得了显著的性能提升，大幅减少了两种类型的幻觉。

Conclusion: 这项研究提供了一个实用的框架，解决了高级推理与事实可信度之间的关键矛盾，为开发更强大、更可靠的大型语言模型铺平了道路。
...
### [132] [The Linguistic Architecture of Reflective Thought: Evaluation of a Large Language Model as a Tool to Isolate the Formal Structure of Mentalization](https://arxiv.org/abs/2512.08945)
*Stefano Epifani,Giuliano Castigliego,Laura Kecskemeti,Giuliano Razzicchia,Elisabeth Seiwald-Sonderegger*

Main category: cs.CL

TL;DR: 本研究评估了大型语言模型（LLM）复现心智化语言结构的能力，发现其生成的心智化文本具有高结构连贯性和可解释性，但在情感整合方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成反思性文本的能力日益增强，引发了关于语言形式与心智表征之间关系的问题。本研究旨在评估单一LLM能在多大程度上根据心智化治疗（MBT）的参数复现心智化的语言结构。

Method: 生成了50个人类参与者与标准模式配置的LLM之间的对话。五名经过MBT培训的精神科医生在盲法条件下，根据MBT的四个维度评估了模型生成的心智化概况，并使用李克特量表对评估连贯性、论证连贯性和整体质量进行评分。使用ICC(3,1)估算了评估者间的一致性。

Result: 平均得分（3.63-3.98）和适度的标准差表明生成的心智化概况具有高水平的结构连贯性。ICC值（0.60-0.84）显示评估者之间存在实质性到高度的一致性。模型在“内隐-外显”和“自我-他人”维度上表现更稳定，但在整合内部状态和外部情境方面存在局限。生成的概况连贯且具有临床可解释性，但以情感中性为特征。

Conclusion: LLM能够高度连贯且可解释地复现心智化的语言结构，尤其在某些维度上表现稳定。然而，它在整合内部情感状态与外部情境方面存在局限，并且其输出缺乏情感深度，这表明语言形式与真正的心智表征之间可能存在差异。
...
### [133] [Luxical: High-Speed Lexical-Dense Text Embeddings](https://arxiv.org/abs/2512.09015)
...
### [134] [Detecting Hallucinations in Graph Retrieval-Augmented Generation via Attention Patterns and Semantic Alignment](https://arxiv.org/abs/2512.09148)
...
### [135] [Knowledge-Guided Large Language Model for Automatic Pediatric Dental Record Understanding and Safe Antibiotic Recommendation](https://arxiv.org/abs/2512.09127)
...
### [136] [Targeting Misalignment: A Conflict-Aware Framework for Reward-Model-based LLM Alignment](https://arxiv.org/abs/2512.09212)
...
### [137] [MindShift: Analyzing Language Models' Reactions to Psychological Prompts](https://arxiv.org/abs/2512.09149)
...
### [138] [CORE: A Conceptual Reasoning Layer for Large Language Models](https://arxiv.org/abs/2512.09222)
*Vishwas Hegde,Vindhya Shigehalli*

Main category: cs.CL

TL;DR: CORE是一个概念优先的交互层，通过持久化的语义状态和认知操作符，在不修改模型权重的情况下，提升大型语言模型多轮对话的稳定性，并显著减少提示词长度。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在多轮交互中存在局限，其内部表示无法跨轮次持久化，导致模型需要从不断增长的令牌历史中重建用户意图和任务状态。这种“令牌优先”范式会导致对话漂移、推理模式不一致以及提示词随对话深入而不断增长的问题。

Method: CORE提出了一种“概念优先”的交互层，它结合了少量通用的认知操作符和一个持久化的“局部概念”（Local Concept）。该局部概念是一个紧凑的语义状态，捕获任务、约束、偏好和中间结果。每次模型调用只接收此概念状态、用户最新指令和选定的操作符，无需重放完整的对话历史。它是一种与模型无关的机制。

Result: 一个模拟CORE行为的初步原型显示，累计提示词令牌减少了约42%。作者强调这仅反映原型条件，不应被视为实际性能估计。

Conclusion: CORE提供了一种与模型无关的机制，将概念推理与语言生成分离，为构建更稳定的多轮系统提供了一个可扩展的方向。
...
### [139] [Training-free Context-adaptive Attention for Efficient Long Context Modeling](https://arxiv.org/abs/2512.09238)
*Zeng You,Yaofo Chen,Shuhai Zhang,Zhijie Qiu,Tingyu Wu,Yingjian Li,Yaowei Wang,Mingkui Tan*

Main category: cs.CL

TL;DR: 本文提出了一种名为TCA-Attention的免训练、上下文自适应稀疏注意力机制，旨在通过选择性关注信息量大的token来提高长上下文LLM推理的效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的自注意力机制在处理长序列时存在二次方的计算和内存复杂度，这带来了显著的挑战。现有的稀疏注意力及KV缓存压缩方法通常有固定模式、无法同时处理预填充和解码阶段、或需要额外训练等局限性。

Method: 本文提出了TCA-Attention，一个免训练的稀疏注意力机制。它包含两个轻量级阶段：1) 离线校准阶段，通过一次前向传播确定每个注意力头的稀疏预算；2) 在线token选择阶段，利用轻量级冗余度指标自适应地保留核心上下文token。该方法统一加速了预填充和解码，并减少了KV缓存占用，无需参数更新或架构修改。

Result: TCA-Attention在128K上下文长度下实现了2.8倍的加速，KV缓存减少了61%，同时在各种基准测试中保持了与全注意力相当的性能。理论分析表明其近似误差有界。

Conclusion: TCA-Attention提供了一个实用且即插即用的解决方案，能够高效地进行长上下文推理，显著提升了LLMs在处理长序列时的计算效率和内存利用率，而无需额外的训练或架构改动。
...
### [140] [Identifying Bias in Machine-generated Text Detection](https://arxiv.org/abs/2512.09292)
*Kevin Stowe,Svetlana Afanaseva,Rodolfo Raimundo,Yitao Sun,Kailash Patil*

Main category: cs.CL

TL;DR: 该研究发现，机器生成文本检测系统普遍存在偏见，尤其倾向于将英语学习者（ELL）和非白人ELL群体的文本错误地识别为机器生成，尽管人类在检测任务上表现不佳但未显示出显著偏见。


<details>
  <summary>Details</summary>
Motivation: 随着文本生成能力的飞速发展，对机器生成文本检测的需求也日益增长。然而，检测模型可能产生显著的负面影响，因此有必要探究这些系统潜在的偏见。

Method: 研究构建了一个学生论文数据集，并评估了16种不同的检测系统，分析其在性别、种族/民族、英语学习者（ELL）身份和经济状况四个属性上的偏见。通过基于回归的模型来确定效应的显著性和强度，并进行子群分析。此外，还进行了人工标注以比较人类检测的性能和偏见。

Result: 研究发现，尽管偏见在不同系统间普遍不一致，但存在几个关键问题：一些模型倾向于将弱势群体分类为机器生成；ELL学生的论文更有可能被归类为机器生成；经济弱势学生的论文反而不太可能被归类为机器生成；非白人ELL学生的论文被不成比例地分类为机器生成，远高于其白人同行。最后，人工标注显示人类在检测任务上表现普遍较差，但在所研究的属性上没有显示出显著偏见。

Conclusion: 机器生成文本检测系统存在显著且不一致的偏见，尤其对英语学习者和非白人英语学习者等弱势群体造成不利影响，可能导致他们被错误识别。尽管人类在检测方面表现不佳，但他们没有表现出类似的偏见，这突显了在部署此类检测系统时需要仔细考虑公平性和潜在的负面影响。
...
### [141] [Language models as tools for investigating the distinction between possible and impossible natural languages](https://arxiv.org/abs/2512.09394)
*Julie Kallini,Christopher Potts*

Main category: cs.CL

TL;DR: 本文提出将语言模型作为探测工具，以区分可能与不可能的自然语言，从而揭示人类语言学习的归纳偏见。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过区分可能与不可能的自然语言，揭示支持人类语言学习的归纳偏见。

Method: 提出一个分阶段的研究计划，其中语言模型架构将迭代优化，以更好地辨别可能与不可能的语言，从而支持与人类认知相关的连接假设。

Result: 本文阐述了一个研究计划，旨在通过不断优化的语言模型，实现对可能与不可能语言的更好区分，并为连接人类认知的假设提供支持。

Conclusion: 语言模型作为探测可能与不可能自然语言之间区别的工具具有巨大潜力，通过系统性的研究，可以揭示人类语言学习的归纳偏见。
...
### [142] [CONCUR: A Framework for Continual Constrained and Unconstrained Routing](https://arxiv.org/abs/2512.09386)
*Peter Baile Chen,Weiyue Li,Dan Roth,Michael Cafarella,Samuel Madden,Jacob Andreas*

Main category: cs.CL

TL;DR: CONCUR是一种持续路由框架，通过模块化设计和多表示学习，有效将AI任务映射到合适的计算策略，解决了现有方法在策略更新时的再训练开销和泛化问题，提高了路由决策的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: AI任务复杂多样，需要不同的计算策略（模型与解码方法组合）。现有的路由系统通常通过训练一个单一模型来覆盖所有策略，导致新策略出现时需要高昂的完全再训练成本，且持续路由的泛化性差。此外，现有模型多采用单一输入表示，限制了其捕获路由问题复杂性的能力，导致次优的路由决策。

Method: 本文提出了CONCUR，一个支持受限和非受限路由的持续路由框架。其采用模块化设计，为每种计算策略训练一个独立的预测模型，从而能够以较低的额外训练成本无缝整合新策略。预测模型还利用任务和计算策略的多种表示，以更好地捕捉问题的整体复杂性。

Result: 在分布内和分布外、知识密集型和推理密集型任务上的实验表明，CONCUR在持续和非持续设置下，均优于最佳单一策略和现有强大的路由技术，实现了更高的端到端准确性和更低的推理成本，同时在持续设置中显著降低了训练成本。

Conclusion: CONCUR通过其模块化设计和多表示学习方法，有效解决了AI任务路由中的持续学习和决策优化问题，显著提高了路由系统的效率、准确性，并降低了维护成本，尤其是在策略不断演进的场景下表现突出。
...
### [143] [CourtPressGER: A German Court Decision to Press Release Summarization Dataset](https://arxiv.org/abs/2512.09434)
...
### [144] [Knowledge-Augmented Large Language Model Agents for Explainable Financial Decision-Making](https://arxiv.org/abs/2512.09440)
...
### [145] [Advancing Text Classification with Large Language Models and Neural Attention Mechanisms](https://arxiv.org/abs/2512.09444)
...
### [146] [Source Coverage and Citation Bias in LLM-based vs. Traditional Search Engines](https://arxiv.org/abs/2512.09483)
...
### [147] [RouteRAG: Efficient Retrieval-Augmented Generation from Text and Graph via Reinforcement Learning](https://arxiv.org/abs/2512.09487)
...
### [148] [System Report for CCL25-Eval Task 10: Prompt-Driven Large Language Model Merge for Fine-Grained Chinese Hate Speech Detection](https://arxiv.org/abs/2512.09563)
...
### [149] [Systematic Framework of Application Methods for Large Language Models in Language Sciences](https://arxiv.org/abs/2512.09552)
*Kun Sun,Rong Wang*
...
### [150] [Creation of the Estonian Subjectivity Dataset: Assessing the Degree of Subjectivity on a Scale](https://arxiv.org/abs/2512.09634)
...
### [151] [MentraSuite: Post-Training Large Language Models for Mental Health Reasoning and Assessment](https://arxiv.org/abs/2512.09636)
...
### [152] [d-TreeRPO: Towards More Reliable Policy Optimization for Diffusion Language Models](https://arxiv.org/abs/2512.09675)
...
TL;DR: d-TreeRPO是一种针对扩散大语言模型（dLLM）的可靠强化学习框架，它通过树形展开和自下而上的优势计算提供精细奖励信号，并通过自信心引导的自蒸馏损失提高预测概率估计的准确性，从而在多个推理基准上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有用于扩散大语言模型（dLLM）的强化学习方法在两个方面存在不足：一是优势估计不准确，因为奖励信号粗糙或不可验证；二是预测概率估计不精确，因为它没有考虑相对于真实、无偏期望预测概率的偏差。

Method: 本文提出了d-TreeRPO框架，它采用树形结构展开和基于可验证结果奖励的自下而上优势计算，以提供细粒度、可验证的逐步奖励信号。在估计条件转移概率时，理论分析表明，更高的预测置信度能带来更低的估计误差。受此启发，模型在训练后期引入了时间调度自蒸馏损失，以增强预测置信度，从而实现更准确的概率估计和更好的收敛性。

Result: 实验结果表明，d-TreeRPO优于现有基线，并在多个推理基准上取得了显著提升，包括数独（+86.2）、倒计时（+51.6）、GSM8K（+4.5）和Math500（+5.3）。消融研究和计算成本分析进一步证明了其设计选择的有效性和实用性。

Conclusion: d-TreeRPO通过解决现有dLLM强化学习方法在优势估计和预测概率估计方面的不足，提供了一个更可靠、更有效的框架，显著提升了模型在推理任务上的性能。
...
### [154] [Neurosymbolic Information Extraction from Transactional Documents](https://arxiv.org/abs/2512.09666)
...
### [155] [FineFreq: A Multilingual Character Frequency Dataset from Web-Scale Text](https://arxiv.org/abs/2512.09701)
*Binbin XU*
...
### [156] [Interpreto: An Explainability Library for Transformers](https://arxiv.org/abs/2512.09730)
...
### [157] [Weird Generalization and Inductive Backdoors: New Ways to Corrupt LLMs](https://arxiv.org/abs/2512.09742)
...
### [158] [MOA: Multi-Objective Alignment for Role-Playing Agents](https://arxiv.org/abs/2512.09756)
...
### [159] [DeepSeek's WEIRD Behavior: The cultural alignment of Large Language Models and the effects of prompt language and cultural prompting](https://arxiv.org/abs/2512.09772)
*James Luther,Donald Brown*

Main category: cs.CL

TL;DR: 本研究使用Hofstede的文化维度调查评估了大型语言模型（LLMs）的文化一致性，发现DeepSeek-V3/V3.1和GPT-5与美国文化高度一致，而GPT-4o/GPT-4.1对提示语言和文化提示更敏感，能更好地适应中美文化。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在生成类人文本方面的进步，人机交互日益增多。因此，理解和确保这些类人代理的文化一致性变得至关重要。

Method: 研究采用Hofstede的VSM13国际调查问卷来衡量LLMs的文化一致性。通过结合提示语言（如英语或简体中文）和“文化提示”（使用系统提示来调整模型以反映特定国家文化）策略，来使主流LLMs与不同文化对齐。

Result: DeepSeek-V3、V3.1和OpenAI的GPT-5与美国调查回应高度一致，即使使用文化提示或改变提示语言，也未能与中国文化实现强或软一致性。GPT-4在英文提示下与中国文化更接近，但文化提示能有效将其对齐转向美国。其他低成本模型，如GPT-4o和GPT-4.1，对提示语言和文化提示策略均有响应，能够与美国和中国文化实现可接受的一致性。

Conclusion: 主流LLMs（如DeepSeek-V3/V3.1和GPT-5）表现出固有的美国文化偏向，难以通过简单提示调整至中国文化。然而，某些模型（如GPT-4o和GPT-4.1）通过灵活运用提示语言和文化提示，能够有效地在不同文化之间进行调整。
...
### [160] [OnCoCo 1.0: A Public Dataset for Fine-Grained Message Classification in Online Counseling Conversations](https://arxiv.org/abs/2512.09804)
...
### [161] [LLMs in Interpreting Legal Documents](https://arxiv.org/abs/2512.09830)
*Simone Corbo*
...
### [162] [ChronusOmni: Improving Time Awareness of Omni Large Language Models](https://arxiv.org/abs/2512.09841)
...
### [163] [Mitigating Social Bias in English and Urdu Language Models Using PRM-Guided Candidate Selection and Sequential Refinement](https://arxiv.org/abs/2512.09854)
...
### [164] [Efficient Continual Learning in Neural Machine Translation: A Low-Rank Adaptation Approach](https://arxiv.org/abs/2512.09910)
...
### [165] [ShelfAware: Real-Time Visual-Inertial Semantic Localization in Quasi-Static Environments with Low-Cost Sensors](https://arxiv.org/abs/2512.09065)
*Shivendra Agrawal,Jake Brawer,Ashutosh Naik,Alessandro Roncone,Bradley Hayes*

Main category: cs.RO

TL;DR: ShelfAware是一种语义粒子滤波器，通过将场景语义作为对象类别的统计证据，在准静态室内环境中实现鲁棒的全局定位，克服了几何混叠和语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 传统的基于视觉的定位方法在准静态室内工作空间（如仓库、实验室、零售环境）中表现不佳，因为重复的几何结构、动态杂物和感知噪声会干扰其性能。

Method: ShelfAware采用语义粒子滤波器，融合了深度似然和以类别为中心的语义相似性。它利用预计算的语义视点库在蒙特卡洛定位(MCL)内部执行逆语义提议，以实现快速、有针对性的假设生成。该方法仅需视觉传感器和VIO，可在低成本、仅视觉的硬件上运行。

Result: 在零售环境中的100次全局定位试验中（涵盖四种条件），ShelfAware的成功率达到96%（MCL为22%，AMCL为10%），平均收敛时间为1.91秒。在所有条件下均取得了最低的平移RMSE，并在80%的测试序列中保持了稳定的跟踪，所有这些都在消费级笔记本电脑平台上实时运行。

Conclusion: ShelfAware通过在类别级别对语义分布进行建模并利用逆提议，有效解决了准静态领域中常见的几何混叠和语义漂移问题。它提供了一个无需基础设施的构建模块，适用于移动机器人在仓库、实验室和零售环境中的应用，并可支持为视障人士提供辅助导航设备。
...
### [166] [Inferring Operator Emotions from a Motion-Controlled Robotic Arm](https://arxiv.org/abs/2512.09086)
*Xinyu Qi,Zeyu Deng,Shaun Alexander Macdonald,Liying Li,Chen Wang,Muhammad Ali Imran,Philip G. Zhao*

Main category: cs.RO

TL;DR: 本文提出一种利用远程机器人功能性运动来推断操作者情感状态的机器学习系统，实现了83.3%的识别准确率，为远程机器人操作和情感机器人学提供了新思路。


<details>
  <summary>Details</summary>
Motivation: 远程机器人操作者的情感状态会显著影响机器人运动，导致意外后果。现有情感识别方法依赖生理信号或肢体语言，在远程控制场景下存在局限性，而操作者情感状态识别在远程机器人控制中尚未得到充分探索。

Method: 研究利用一个非为情感表达设计的远程控制机器人化身的功能性运动（由操作者的手部动作产生），通过一个机器学习系统来推断人类操作者的情感状态。

Result: 该系统在识别由机器人运动（作为手部动作的结果）表达的用户情感状态时，达到了83.3%的准确率。

Conclusion: 该系统对于当前和未来的远程机器人操作以及情感机器人学领域具有重要意义，表明即使是功能性机器人运动也能有效反映操作者的情感状态。
...
### [167] [Masked Generative Policy for Robotic Control](https://arxiv.org/abs/2512.09101)
...
### [168] [Cognitive Trust in HRI: "Pay Attention to Me and I'll Trust You Even if You are Wrong"](https://arxiv.org/abs/2512.09105)
...
### [169] [Semantic Trajectory Generation for Goal-Oriented Spacecraft Rendezvous](https://arxiv.org/abs/2512.09111)
...
### [170] [UPETrack: Unidirectional Position Estimation for Tracking Occluded Deformable Linear Objects](https://arxiv.org/abs/2512.09283)
...
### [171] [One-Shot Real-World Demonstration Synthesis for Scalable Bimanual Manipulation](https://arxiv.org/abs/2512.09297)
*Huayi Zhou,Kui Jia*
...
### [172] [Scene-agnostic Hierarchical Bimanual Task Planning via Visual Affordance Reasoning](https://arxiv.org/abs/2512.09310)
...
### [173] [COVLM-RL: Critical Object-Oriented Reasoning for Autonomous Driving Using VLM-Guided Reinforcement Learning](https://arxiv.org/abs/2512.09349)
*Lin Li,Yuxin Cai,Jianwu Fang,Jianru Xue,Chen Lv*

Main category: cs.RO

TL;DR: COVLM-RL是一种新型端到端自动驾驶框架，它将批判性对象导向推理与VLM引导的强化学习相结合，显著提高了泛化能力、训练效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端自动驾驶框架在泛化能力、训练效率和可解释性方面面临挑战。基于VLM的方法在处理新颖场景时缺乏鲁棒性，而基于RL的方法则存在数据效率低下和决策不透明的问题。

Method: 该研究提出了COVLM-RL框架，它整合了批判性对象导向（CO）推理和VLM引导的强化学习。具体而言，设计了一种思维链（CoT）提示策略，使VLM能够对关键交通元素进行推理并生成高级语义决策（先验）。这些先验降低了输入维度，并将任务相关知识注入RL循环，从而加速训练并提高策略可解释性。此外，引入了一致性损失，以促进VLM的语义规划与RL智能体的控制输出之间的一致性，增强了可解释性和训练稳定性。

Result: 在CARLA模拟器中进行的实验表明，COVLM-RL在已训练的驾驶环境中成功率提高了30%，在以前未见过的环境中成功率提高了50%，这突显了其强大的泛化能力。

Conclusion: COVLM-RL通过将VLM的语义推理与RL的适应性相结合，有效解决了现有端到端自动驾驶框架的局限性，实现了更高的成功率、更强的泛化能力和更好的可解释性。
...
### [174] [Observability Analysis and Composite Disturbance Filtering for a Bar Tethered to Dual UAVs Subject to Multi-source Disturbances](https://arxiv.org/abs/2512.09377)
*Lidan Xu,Dadong Fan,Junhong Wang,Wenshuo Li,Hao Lu,Jianzhong Qiao*

Main category: cs.RO

TL;DR: 本文证明了在多源干扰下，仅使用无人机里程计信息，双无人机-杆系统中的有效载荷姿态是可观测的，并设计了一种复合干扰滤波方案进行验证。


<details>
  <summary>Details</summary>
Motivation: 合作空中运输易受气动效应和推力不确定性等多源干扰影响，现有方法为实现精确负载操作常依赖额外传感器，增加了系统成本和复杂性。核心问题是：在多源干扰下，仅使用无人机里程计信息，有效载荷的姿态是否可观测？

Method: 研究集中于双无人机-杆系统，利用可观测性秩判据，证明了当仅存在两种或少于两种类型的集总干扰时，整个系统是可观测的。为验证此分析，论文考虑了干扰仅作用于无人机的情况，并开发了一种复合干扰滤波方案，设计了一个基于干扰观测器的误差状态扩展卡尔曼滤波器（DO-ES-EKF）用于状态和干扰估计。

Result: 研究首次证明了当仅存在两种或少于两种类型的集总干扰时，仅使用无人机里程计信息，双无人机-杆系统的整个系统是可观测的。仿真和实验测试验证了仅使用无人机里程计信息，完全估计系统状态和干扰是可行的，DO-ES-EKF在流形$(\mathbb{R}^3)^2\times(TS^2)^3$上实现了改进的估计性能。

Conclusion: 本研究得出结论，在特定干扰条件下，仅使用无人机里程计信息，可以完全估计合作双无人机-杆系统的状态和干扰（包括有效载荷姿态）。这一结果为通过最小化传感器套件来构建更具成本效益和鲁棒性的系统铺平了道路。
...
### [175] [Development and Testing for Perception Based Autonomous Landing of a Long-Range QuadPlane](https://arxiv.org/abs/2512.09343)
...
### [176] [H2R-Grounder: A Paired-Data-Free Paradigm for Translating Human Interaction Videos into Physically Grounded Robot Videos](https://arxiv.org/abs/2512.09406)
...
### [177] [Generalizable Collaborative Search-and-Capture in Cluttered Environments via Path-Guided MAPPO and Directional Frontier Allocation](https://arxiv.org/abs/2512.09410)
...
### [178] [D$^2$GSLAM: 4D Dynamic Gaussian Splatting SLAM](https://arxiv.org/abs/2512.09411)
...
### [179] [Visual Heading Prediction for Autonomous Aerial Vehicles](https://arxiv.org/abs/2512.09898)
...
### [180] [A Hierarchical, Model-Based System for High-Performance Humanoid Soccer](https://arxiv.org/abs/2512.09431)
...
### [181] [Sequential Testing for Descriptor-Agnostic LiDAR Loop Closure in Repetitive Environments](https://arxiv.org/abs/2512.09447)
...
### [182] [Development of a Compliant Gripper for Safe Robot-Assisted Trouser Dressing-Undressing](https://arxiv.org/abs/2512.09462)
...
### [183] [On Mobile Ad Hoc Networks for Coverage of Partially Observable Worlds](https://arxiv.org/abs/2512.09495)
...
### [184] [Prediction-aware and Reinforcement Learning based Altruistic Cooperative Driving](https://arxiv.org/abs/2211.10585)
...
### [185] [Altruistic Maneuver Planning for Cooperative Autonomous Vehicles Using Multi-agent Advantage Actor-Critic](https://arxiv.org/abs/2107.05664)
...
### [186] [ViTA-Seg: Vision Transformer for Amodal Segmentation in Robotics](https://arxiv.org/abs/2512.09510)
...
### [187] [REASAN: Learning Reactive Safe Navigation for Legged Robots](https://arxiv.org/abs/2512.09537)
...
### [188] [Mastering Diverse, Unknown, and Cluttered Tracks for Robust Vision-Based Drone Racing](https://arxiv.org/abs/2512.09571)
...
### [189] [UrbanNav: Learning Language-Guided Urban Navigation from Web-Scale Human Trajectories](https://arxiv.org/abs/2512.09607)
...
### [190] [Super4DR: 4D Radar-centric Self-supervised Odometry and Gaussian-based Map Optimization](https://arxiv.org/abs/2512.09608)
*Zhiheng Li,Weihua Wang,Qiang Shen,Yichen Zhao,Zheng Fang*

Main category: cs.RO

TL;DR: Super4DR是一个以4D雷达为中心的框架，用于在恶劣环境下进行基于学习的里程计估计和高斯地图优化，显著提升了性能并缩小了与激光雷达的地图质量差距。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉或激光雷达SLAM系统在恶劣光照和天气下表现不佳。尽管4D雷达适用于此类环境，但其稀疏和嘈杂的点云阻碍了准确的里程计估计，且雷达地图结构模糊不完整。

Method: 该研究提出了两部分方法：1. 设计了一个集群感知里程计网络，利用聚类雷达点中的对象级线索进行帧间匹配，并结合分层自监督机制（通过时空一致性、知识迁移和特征对比）来处理异常值。2. 提出使用3D高斯作为中间表示，结合雷达特定的增长策略、选择性分离和多视图正则化，以恢复模糊和未检测到的地图区域。

Result: 实验表明，Super4DR比先前的自监督方法性能提高了67%，几乎与监督里程计相当，缩小了与激光雷达的地图质量差距，并实现了多模态图像渲染。

Conclusion: Super4DR框架有效解决了4D雷达在恶劣环境下的里程计估计和地图构建挑战，通过创新的网络设计和高斯地图优化，显著提升了系统性能和地图质量。
...
### [191] [ReMoSPLAT: Reactive Mobile Manipulation Control on a Gaussian Splat](https://arxiv.org/abs/2512.09656)
*Nicolas Marticorena,Tobias Fischer,Niko Suenderhauf*

Main category: cs.RO

TL;DR: ReMoSPLAT是一种基于二次规划的移动机械臂反应式控制器，利用高斯溅射表示进行避障，使其能在复杂环境中有效到达目标位姿，性能媲美依赖完美地面实况信息的控制器。


<details>
  <summary>Details</summary>
Motivation: 反应式控制能够协调移动机械臂的运动，但如何在不涉及昂贵规划的情况下，利用准确的环境表示进行避障仍然是一个挑战。

Method: 本文提出了ReMoSPLAT，一个基于二次规划（QP）的反应式控制器，用于移动操作，并利用高斯溅射（Gaussian Splat）表示进行碰撞避免。通过在优化公式中集成额外的约束和成本，平台能够到达预期的末端执行器位姿并避开障碍物。研究了两种高效计算机器人-障碍物距离的方法：纯几何方法和基于光栅化的方法，并比较了它们的权衡。

Result: 该方法在模拟中（包括合成场景和真实世界扫描）验证了其可行性，展示了在复杂场景中，移动机械臂能够到达目标末端执行器位姿并避开障碍物。其性能与依赖完美地面实况信息的控制器相当。

Conclusion: ReMoSPLAT是一种可行的反应式控制器，能够有效协调移动机械臂在复杂环境中的运动，通过利用高斯溅射表示和二次规划实现高效避障，并取得了与依赖地面实况信息控制器相媲美的性能。
...
### [192] [GLaD: Geometric Latent Distillation for Vision-Language-Action Models](https://arxiv.org/abs/2512.09619)
...
### [193] [Bridging the Basilisk Astrodynamics Framework with ROS 2 for Modular Spacecraft Simulation and Hardware Integration](https://arxiv.org/abs/2512.09833)
...
### [194] [High-Resolution Water Sampling via a Solar-Powered Autonomous Surface Vehicle](https://arxiv.org/abs/2512.09798)
...
### [195] [Simultaneous Tactile-Visual Perception for Learning Multimodal Robot Manipulation](https://arxiv.org/abs/2512.09851)
...
### [196] [YOPO-Nav: Visual Navigation using 3DGS Graphs from One-Pass Videos](https://arxiv.org/abs/2512.09903)
...
### [197] [Py-DiSMech: A Scalable and Efficient Framework for Discrete Differential Geometry-Based Modeling and Control of Soft Robots](https://arxiv.org/abs/2512.09911)
*Radha Lahoti,Ryan Chaiyakul,M. Khalid Jawed*

Main category: cs.RO

TL;DR: Py-DiSMech是一个基于离散微分几何的Python软机器人仿真框架，它通过向量化实现高计算效率和物理精度，并支持复杂的接触和控制。


<details>
  <summary>Details</summary>
Motivation: 软机器人设计和控制中的大几何变形和复杂接触对传统建模工具构成挑战，需要兼具物理精度、计算可扩展性并能与现代控制优化流程无缝集成的仿真框架。

Method: 本文提出了Py-DiSMech，一个基于Python的开源仿真框架，其核心是离散微分几何（DDG）原理。它直接在网格上离散化曲率和应变等几何量，以高保真度和低计算成本捕捉杆、壳和混合结构的非线性变形。该框架特点包括：1) 全向量化的NumPy实现，比现有几何模拟器快一个数量级；2) 基于惩罚能量的全隐式接触模型，支持杆-杆、杆-壳和壳-壳相互作用；3) 基于自然应变的反馈控制模块，采用PI控制器进行形状调节和轨迹跟踪；4) 模块化、面向对象的软件设计，支持用户自定义弹性势能、驱动方案并与机器学习库集成。

Result: 基准测试表明，Py-DiSMech在计算效率上显著优于现有最先进的模拟器Elastica，同时保持了物理精度，实现了数量级的加速。

Conclusion: Py-DiSMech的这些特性使其成为一个可扩展、可伸缩的平台，适用于软机器人的仿真驱动设计、控制验证和虚实迁移研究。
...
### [198] [LISN: Language-Instructed Social Navigation with VLM-based Controller Modulating](https://arxiv.org/abs/2512.09920)
...
### [199] [Token Expand-Merge: Training-Free Token Compression for Vision-Language-Action Models](https://arxiv.org/abs/2512.09927)
...
### [200] [HiF-VLA: Hindsight, Insight and Foresight through Motion Representation for Vision-Language-Action Models](https://arxiv.org/abs/2512.09928)
...
### [201] [Electric Arc Furnaces Scheduling under Electricity Price Volatility with Reinforcement Learning](https://arxiv.org/abs/2512.09293)
*Ruonan Pi,Zhiyuan Fan,Bolun Xu*

Main category: eess.SY

TL;DR: 本文提出了一种基于强化学习的框架，用于在电价波动下优化电弧炉（EAF）的运行。


<details>
  <summary>Details</summary>
Motivation: 在电力价格波动和共享进料容量限制下，优化电弧炉的运行是一个复杂的问题，需要实时控制策略。

Method: 研究首先将EAF调度问题的确定性版本表述为混合整数线性规划（MILP）。然后，开发了一种Q学习算法，以在实时价格波动和共享进料容量约束下对多个EAF单元进行实时控制。为Q学习算法设计了自定义奖励函数，以平滑EAF的启动惩罚。最后，使用纽约州的实际EAF设计数据和电价，将算法与基于规则的控制器和假设完美价格预测的MILP基准进行了比较。

Result: 在非预测控制设置下，本文提出的强化学习算法在各种单单元和多单元案例中，实现了与完美MILP基准约90%的利润。

Conclusion: 强化学习算法能够有效应对电价波动和操作约束下的EAF实时优化问题，其性能接近于具有完美价格预测的基准最优解。
...
### [202] [Characterizing Human Feedback-Based Control in Naturalistic Driving Interactions via Gaussian Process Regression with Linear Feedback](https://arxiv.org/abs/2512.09097)
*Rachel DiPirro,Rosalyn Devonport,Dan Calderone,Chishang "Mario'' Yang,Wendy Ju,Meeko Oishi*

Main category: eess.SY

TL;DR: 该研究通过驾驶模拟器数据，使用高斯过程回归建模了人类驾驶员在无标志交叉口的行为策略，并分析了这些策略中的反馈增益及其在不同人群间的差异，旨在促进社交响应型自动驾驶汽车的设计。


<details>
  <summary>Details</summary>
Motivation: 理解驾驶员互动对于设计能够与人类驾驶汽车安全互动的自动驾驶车辆至关重要。

Method: 研究在一个驾驶模拟器中收集了无标志交叉口处驾驶员的自然决策和行为数据。利用这些数据，通过高斯过程回归方法，将人类驾驶员的响应建模为基于状态的反馈控制器。控制器的反馈增益通过线性与非线性先验的加权组合计算得出。

Result: 研究分析了单个增益如何反映在驾驶员行为中，并评估了不同驾驶员群体之间这些控制器的差异。

Conclusion: 这项关于驾驶员如何确定其策略的数据驱动分析工作，有助于未来设计具有社交响应能力的自动驾驶车辆。
...
### [203] [MPC for momentum counter-balanced and zero-impulse contact with a free-spinning satellite](https://arxiv.org/abs/2512.09213)
*Theofania Karampela,Rishie Seshadri,Florian Dörfler,Sarah H. Q. Li*

Main category: eess.SY

TL;DR: 本文提出一种非线性模型预测控制（MPC）框架，使服务卫星能与自由旋转的目标卫星实现零冲击接触，克服了现有方法的约束限制。


<details>
  <summary>Details</summary>
Motivation: 在轨机器人中，服务卫星与自由旋转目标卫星接触的能力是完成大多数在轨服务（OOS）任务的关键。现有控制方法无法充分考虑执行器和状态约束，因此需要一种更鲁棒的控制方法。

Method: 开发了一个非线性模型预测控制（MPC）框架，用于控制服务卫星的两个独立执行模块（力矩生成模块和操作模块），并明确建模了它们之间的交叉耦合动力学。通过数值蒙特卡洛（MC）试验模拟零冲击接触场景，并与现有控制方法进行比较。

Result: MPC控制器能够强制执行现有方法无法解决的执行器和状态约束。仿真结果验证了MPC控制器在操作约束、移动接触位置以及观测和执行器噪声下，在保持自旋同步和零冲击接触方面的有效性，并优于先前的控制方法。

Conclusion: 该MPC控制器能够有效实现服务卫星与自由旋转目标卫星的零冲击接触，并在各种操作约束和不确定性下保持自旋同步，为在轨服务任务提供了鲁棒的解决方案。
...
### [204] [Kinematics Control of Electromagnetic Formation Flight Using Angular-Momentum Conservation Constraint](https://arxiv.org/abs/2512.08949)
*Yuta Takahashi,Hiraku Sakamoto,Shin-ichiro Sakai*

Main category: eess.SY

TL;DR: 本文提出了一种新型控制器，用于电磁编队飞行（EMFF），该控制器能同时控制电磁力和扭矩，有效解决反作用轮（RW）饱和问题，并显著减少对RW的需求，通过角动量守恒实现对多卫星系统的姿态和轨道控制。


<details>
  <summary>Details</summary>
Motivation: 在以往的电磁编队飞行研究中，卫星通常配备反作用轮来补偿电磁扭矩。然而，由于电磁扭矩在不同卫星之间存在差异，导致反作用轮所承载的角动量分布不均，随着时间推移，这种偏差会累积，使反作用轮迅速饱和，进而影响卫星姿态控制。

Method: 本研究提出了一种新型控制器，能够同时控制电磁力和扭矩。该控制器设计基于角动量守恒推导的电磁编队飞行运动学。它允许控制n颗卫星而不使反作用轮饱和，并且整个编队中仅需一套反作用轮。结合一个专为领星设计的简单卸载控制方案，可以消除整个系统角动量的累积。

Result: 所提出的控制器能够有效控制n颗卫星，防止反作用轮饱和，且仅需一套反作用轮即可实现对所有卫星的控制。与领星的卸载控制相结合，成功消除了整个系统角动量的累积。通过对五卫星系统进行编队保持和编队重构的数值仿真，验证了该控制器的有效性。

Conclusion: 所提出的控制器在电磁编队飞行中能同时有效控制电磁力和扭矩，解决了反作用轮饱和问题，显著降低了对反作用轮硬件的需求（仅需一套），并通过与领星卸载控制结合，实现了整个系统角动量的平衡，提升了多卫星编队飞行的可持续性和可靠性。
...
### [205] [Intelligent Resilience Testing for Decision-Making Agents with Dual-Mode Surrogate Adaptation](https://arxiv.org/abs/2512.09372)
*Jingxuan Yang,Weichao Xu,Yuchen Shi,Yi Zhang,Shuo Feng,Huaxin Pei*

Main category: eess.SY

TL;DR: IRTest是一种统一的在线自适应测试框架，旨在通过结合离线代理模型、在线微调和重要性采样校正，以及偏置校正的贝叶斯优化，高效地测试决策代理并发现其故障。


<details>
  <summary>Details</summary>
Motivation: 测试和评估决策代理面临挑战，包括系统架构未知、内部状态访问受限以及高维场景空间巨大。现有方法依赖代理模型生成场景库，但代理模型与真实代理之间的差异限制了其泛化性和实用性。

Method: 本文提出了智能韧性测试（IRTest），一个统一的在线自适应测试框架。它首先使用离线训练的代理预测模型，并通过两种互补的适应机制逐步缩小代理与真实代理之间的差距：(i) 在数据丰富的条件下进行在线神经网络微调；(ii) 在数据有限的条件下进行基于重要性采样的轻量级加权校正。一个配备偏置校正采集函数的贝叶斯优化策略，指导场景生成以平衡复杂测试空间中的探索与利用。

Result: 在不同任务复杂度和系统异构性水平的广泛实验表明，IRTest持续提高了故障发现效率、测试鲁棒性和跨系统泛化能力。

Conclusion: 这些结果突显了IRTest作为一种可扩展、自适应和韧性的决策代理测试实用解决方案的潜力。
...
### [206] [Analysis of Frequency and Voltage Strength in Power Electronics-Dominated Power Systems Based on Eigen-subsystems](https://arxiv.org/abs/2512.09323)
...
### [207] [Time-Discretized Simulation of Vehicle Platoons for Safety Analysis with Guaranteed Error Bounds](https://arxiv.org/abs/2512.09416)
...
### [208] [Power Control of Multi-Layer Repeater Networks (POLARNet)](https://arxiv.org/abs/2512.09449)
...
### [209] [Personalized Building Climate Control with Contextual Preferential Bayesian Optimization](https://arxiv.org/abs/2512.09481)
...
### [210] [Instantaneous Complex Phase and Frequency: Conceptual Clarification and Equivalence between Formulations](https://arxiv.org/abs/2512.09574)
...
### [211] [Real-Time Non-Smooth MPC for Switching Systems: Application to a Three-Tank Process](https://arxiv.org/abs/2512.09611)
...
### [212] [Adaptive Optimal Control for Avatar-Guided Motor Rehabilitation in Virtual Reality](https://arxiv.org/abs/2512.09667)
*Francesco De Lellis,Maria Lombardi,Egidio De Benedetto,Pasquale Arpaia,Mario di Bernardo*

Main category: eess.SY

TL;DR: 该论文提出了一种基于可解释、自适应运动引导的最优控制理论框架，用于虚拟现实中自主虚拟形象引导的康复治疗，旨在解决中风患者在家进行个性化康复的挑战。


<details>
  <summary>Details</summary>
Motivation: 运动康复面临可及性、成本和护理连续性等关键挑战，超过50%的患者无法定期参加诊所治疗。因此，需要一种系统使中风患者能在沉浸式虚拟现实中在家进行个性化治疗，并由临床医生进行监控。

Method: 核心是一个非线性、人机交互的控制策略，其中虚拟形象实时适应患者的表现。通过多目标最优控制，实现跟随患者运动与引导他们达到基于Hogan最小抖动模型的理想运动学轮廓之间的平衡。一个数据驱动的“能力指数”利用平滑度指标，根据患者的进展动态调整控制增益。

Result: 该系统通过模拟和初步试验进行了验证，显示出提供自适应、引人入胜和可扩展的远程物理治疗的潜力，并以可解释的控制理论原理为指导。

Conclusion: 该框架通过可解释的控制理论原则，为中风患者提供了一种有潜力实现自适应、引人入胜且可扩展的远程物理治疗解决方案。
...
### [213] [RIS-Assisted Coordinated Multi-Point ISAC for Low-Altitude Sensing Coverage](https://arxiv.org/abs/2512.09625)
*Ying Zhang,Zeqi Hao,Tingting Zhang*

Main category: eess.SY

TL;DR: 本文提出了一种在RIS辅助的协调多点ISAC网络中，解决低空传感覆盖问题的方法，通过联合波束赋形和相移设计，在保证传感信噪比和通信频谱效率的同时，最小化总发射功率。


<details>
  <summary>Details</summary>
Motivation: 低空经济（LAE）的兴起和发展，对低空区域的安全监控提出了要求，需要建立有效的传感覆盖方案来监测未经授权的目标。将集成传感与通信（ISAC）引入蜂窝网络可以增强传感性能和覆盖范围，而可重构智能表面（RIS）则能缓解城市区域基站与低空目标之间的信号阻塞。

Method: 研究了RIS辅助的协调多点ISAC网络中的低空传感覆盖问题。提出了一种联合波束赋形和相移设计方案，旨在最小化总发射功率，同时保证传感信噪比和通信频谱效率。通过交替优化和半正定松弛技术，解决了一个非凸优化问题，并提出了高效算法。

Result: 数值结果表明，所提出的方案优于基线方案，展示了其优越性。

Conclusion: 所提出的RIS辅助协调多点ISAC网络中的低空传感覆盖方案，通过联合波束赋形和相移设计，能够有效最小化发射功率，同时满足传感和通信性能要求，展现出卓越的性能。
...
### [214] [Dynamic one-time delivery of critical data by small and sparse UAV swarms: a model problem for MARL scaling studies](https://arxiv.org/abs/2512.09682)
*Mika Persson,Jonas Lidman,Jacob Ljungberg,Samuel Sandelius,Adam Andersson*

Main category: eess.SY

TL;DR: 本研究概念性地探讨了多智能体强化学习（MARL）在无人机分散式控制中继关键数据包的应用，引入了一系列确定性博弈进行可扩展性研究，并提出了一种基于Dijkstra算法的基线策略。实验结果表明，MARL算法在智能体数量较少时表现良好，但随着智能体数量增加，可扩展性问题显现。


<details>
  <summary>Details</summary>
Motivation: 研究MARL在无人机分散式控制中的应用，以实现关键数据包向已知位置的中继，并评估其可扩展性。

Method: 引入了一系列确定性博弈，专为MARL的可扩展性研究而设计。提出了一种基于限制智能体运动范围和应用Dijkstra算法的鲁棒基线策略。使用两种现成的MARL算法与该基线策略进行比较。

Result: 对于少量智能体，两种现成的MARL算法与基线策略表现相当。然而，随着智能体数量的增加，MARL算法出现了可扩展性问题。

Conclusion: MARL在少量智能体场景下具有竞争力，但在智能体数量增加时面临可扩展性挑战，这表明在实际应用中需要进一步解决MARL的可扩展性问题。
...
### [215] [Resilient Neural-Variable-Structure Consensus Control for Nonlinear MASs with Singular Input Gain Under DoS Attacks](https://arxiv.org/abs/2512.09879)
*Ladan Khoshnevisan,Xinzhi Liu*

Main category: eess.SY

TL;DR: 本文提出了一种可靠的基于学习的自适应控制框架，用于处理遭受拒绝服务（DoS）攻击和奇异控制增益的非线性多智能体系统（MASs），通过神经网络变结构自适应控制器和基于可靠性评估的切换控制机制，实现领导者-跟随者共识并保证闭环系统的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时解决网络物理系统中非线性多智能体系统面临的拒绝服务（DoS）攻击和奇异控制增益两大挑战，且通常依赖于限制性假设（如Lipschitz连续性或非线性的先验界限）。

Method: 1. 开发了神经网络变结构自适应控制器，以实现领导者-跟随者共识，并确保对外部扰动的鲁棒性和对未知非线性动力学的适应性。2. 引入了可靠性评估规则，用于检测DoS攻击期间的通信丢失。3. 激活切换控制机制，以在检测到攻击时保持闭环系统的稳定性和性能。

Result: 该方法实现了领导者-跟随者共识，确保了对外部扰动的鲁棒性和对未知非线性动力学的适应性。通过Lyapunov分析，证明了所有闭环信号的一致最终有界性。Matlab/Simulink仿真在一个互联自动驾驶车队中验证了该方法的有效性和弹性。

Conclusion: 这是首次将神经网络学习、变结构鲁棒性和基于可靠性的切换机制整合到统一的共识跟踪控制架构中，用于处理具有奇异输入增益且遭受DoS攻击的异构非线性多智能体系统，且不依赖于限制性假设。
...
### [216] [Agreement Disagreement Guided Knowledge Transfer for Cross-Scene Hyperspectral Imaging](https://arxiv.org/abs/2512.08990)
*Lu Huo,Haimin Zhang,Min Xu*

Main category: eess.IV

TL;DR: 本文提出了一种名为ADGKT的框架，通过整合梯度对齐（GradVac）、Logit幅度规范（LogitNorm）、不一致性限制（DiR）和集成策略，有效解决了跨场景高光谱成像（HSI）知识迁移中存在的梯度冲突、主导梯度以及未能同时捕获一致性和不一致性信息的问题，实现了鲁棒且平衡的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有的跨场景高光谱成像（HSI）知识迁移研究常忽略共享参数优化过程中出现的梯度冲突和主导梯度问题。此外，许多现有方法未能同时捕获一致性和不一致性信息，仅依赖有限的共享目标特征子集，从而错失了目标场景中丰富多样的模式。

Method: 本文提出了一个“一致性-不一致性引导的知识迁移（ADGKT）”框架。其中，一致性组件包含：1) GradVac，用于对齐梯度方向以缓解源域和目标域之间的冲突；2) LogitNorm，用于调节Logit幅度以防止单一梯度源的主导。不一致性组件包含：1) 不一致性限制（DiR），用于捕获多样化的预测目标特征；2) 集成策略，用于缓解关键目标信息的丢失。

Result: 广泛的实验证明，所提出的方法在实现异构HSI场景间的鲁棒且平衡的知识迁移方面具有有效性和优越性。

Conclusion: ADGKT框架通过整合一致性和不一致性机制，有效增强了跨场景知识迁移，解决了现有方法在梯度冲突、主导梯度和信息捕获方面的不足，为异构HSI场景的知识迁移提供了更优解决方案。
...
### [217] [Enhanced Chest Disease Classification Using an Improved CheXNet Framework with EfficientNetV2-M and Optimization-Driven Learning](https://arxiv.org/abs/2512.08992)
*Ali M. Bahram,Saman Muhammad Omer,Hardi M. Mohammed,Sirwan Abdolwahed Aula*

Main category: eess.IV

TL;DR: 该研究提出了一种基于EfficientNetV2-M和先进训练方法的胸部X光疾病分类框架，显著提高了分类准确性和效率，尤其在COVID-19和肺结核检测方面表现卓越，可作为临床诊断支持工具。


<details>
  <summary>Details</summary>
Motivation: 在资源有限的环境中，放射科医生短缺导致胸部X光诊断延迟和患者预后不佳。现有的CheXNet（基于DenseNet-121）计算效率低下且单标签分类性能不佳，需要更优的解决方案。

Method: 研究采用EfficientNetV2-M作为骨干网络，并结合了自动混合精度训练、AdamW优化器、余弦退火学习率调度和指数移动平均正则化等先进训练策略。构建了一个包含18,080张胸部X光图像的数据集，涵盖五种疾病类别（心脏肥大、COVID-19、正常、肺炎、肺结核）。为确保统计可靠性，进行了九次独立的实验运行。

Result: 所提出的框架取得了显著的性能提升，平均测试准确率达96.45%（基线为95.30%，p < 0.001），宏平均F1分数增至91.08%（p < 0.001）。对关键传染病的分类表现近乎完美，其中COVID-19检测准确率为99.95%，肺结核检测准确率为99.97%。尽管模型参数增加了6.8倍，但训练时间减少了11.4%，性能稳定性提高了22.7%。

Conclusion: 该框架可作为一种决策支持工具，应用于疫情应对、肺结核筛查以及各类医疗机构的常规胸部疾病评估，具有重要的临床应用潜力。
...
### [218] [DermETAS-SNA LLM: A Dermatology Focused Evolutionary Transformer Architecture Search with StackNet Augmented LLM Assistant](https://arxiv.org/abs/2512.08998)
*Nitya Phani Santosh Oruganty,Keerthi Vemula Murali,Chun-Kit Ngan,Paulo Bandeira Pinho*

Main category: eess.IV

TL;DR: 本文介绍DermETAS-SNA LLM助手，它结合了皮肤病学进化Transformer架构搜索、StackNet增强的LLM和RAG管道，用于动态学习皮肤病分类器并提供医学解释，显著优于现有模型，并获得专家的高度认可。


<details>
  <summary>Details</summary>
Motivation: 开发能够动态学习皮肤病分类器并提供医学解释的系统，以促进医生与患者之间的沟通和理解。

Method: 1. 采用ETAS框架在SKINCON数据集上优化Vision Transformer (ViT)，并针对DermNet数据集中23种皮肤病类别微调二元分类器。2. 设计StackNet架构整合多个微调的ViT分类器，以增强预测鲁棒性并缓解类别不平衡问题。3. 实施名为Derm的RAG管道，利用Google Gemini 2.5 Pro LLM和验证的皮肤病学材料生成个性化诊断描述和解释。4. 创建一个原型，将DermETAS-SNA LLM完全集成到AI助手中。

Result: 1. 在23种皮肤病类别上实现了56.30%的F1分数，较SkinGPT-4 (48.51%) 提升16.06%。2. 8位执业医生对AI助手的临床响应进行了领域专家评估，对7种皮肤病的评估达成92%的一致性。3. 创建了一个概念验证原型，展示了其在实际临床和教育应用中的可行性。

Conclusion: DermETAS-SNA LLM助手通过集成先进的AI技术，在皮肤病分类和生成医学解释方面表现出色，其性能显著优于现有模型，并获得临床专家的认可，具有实际应用潜力。
...
### [219] [Causal Attribution of Model Performance Gaps in Medical Imaging Under Distribution Shifts](https://arxiv.org/abs/2512.09094)
*Pedro M. Gordaliza,Nataliia Molchanova,Jaume Banus,Thomas Sanchez,Meritxell Bach Cuadra*

Main category: eess.IV

TL;DR: 本文提出一个因果归因框架，利用Shapley值量化医学图像分割模型性能下降中采集协议和标注变异的独立贡献。研究发现，在跨标注者时标注协议变化是主导因素，而在跨成像中心时采集协议变化是主导因素。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分割中因分布偏移导致性能显著下降，但其背后的因果机制尚不明确。

Method: 将因果归因框架扩展到高维分割任务。通过因果图建模数据生成过程，并采用Shapley值公平地归因性能变化到个体机制（采集协议和标注变异）。该框架解决了医学成像中高维输出、样本有限和复杂机制交互的独特挑战。

Result: 在多发性硬化症(MS)病灶分割任务中验证。结果显示，跨标注者时，标注协议偏移是主要的失败模式（7.4% ± 8.9% DSC归因）；而跨成像中心时，采集协议偏移是主要的失败模式（6.5% ± 9.1%）。

Conclusion: 这种针对特定机制的量化能够帮助实践者根据部署情境优先采取有针对性的干预措施。
...
### [220] [SITP: A High-Reliability Semantic Information Transport Protocol Without Retransmission for Semantic Communication](https://arxiv.org/abs/2512.09291)
*Yunhao Wang,Shuai Ma,Youlong Wu,Guangming Shi,Xiang Cheng,Yuxuan Liu,Pengfei He*

Main category: eess.IV

TL;DR: 本文提出了一种语义信息传输协议（SITP），旨在满足6G网络对高可靠性和低延迟的需求。SITP通过仅验证包头并保留潜在损坏的负载进行语义解码，实现了接近TCP的可靠性和UDP的低延迟。此外，论文还建立了一个跨层分析模型来量化丢包率，并开发了一种跨图像特征交织机制以缓解突发性丢包。


<details>
  <summary>Details</summary>
Motivation: 随着6G网络的发展，现代通信系统面临前所未有的高可靠性和低延迟需求。然而，传统的传输协议是为比特级可靠性设计的，无法满足语义鲁棒性要求。

Method: 1. 提出了语义信息传输协议（SITP）：通过仅验证数据包头部，同时保留可能损坏的负载进行语义解码，旨在实现TCP级别的可靠性和UDP级别的低延迟。
2. 建立了跨层分析模型：量化了从物理层到应用层的丢包概率，统一了信噪比（SNR）与丢包率之间的概率公式，为端到端语义传输提供理论基础。
3. 开发了跨图像特征交织机制：通过在多个相关图像之间重新分配语义特征，以减轻连续突发性丢包的影响，增强在突发衰落信道中的鲁棒性。

Result: 1. 在低信噪比（SNR）下，SITP比TCP具有更低的延迟和相当的可靠性。
2. SITP能达到UDP级别的延迟，并提供卓越的重建质量。
3. 所提出的跨图像语义交织机制有效缓解了突发性丢包造成的性能下降。

Conclusion: SITP协议及其配套的跨层分析模型和跨图像特征交织机制，有效解决了6G网络中语义鲁棒性、高可靠性和低延迟的挑战，特别是在突发衰落信道中表现出优越的性能和重建质量。
...
### [221] [NOC4SC: A Bandwidth-Efficient Multi-User Semantic Communication Framework for Interference-Resilient Transmission](https://arxiv.org/abs/2512.09356)
...
### [222] [QSMnet-INR: Single-Orientation Quantitative Susceptibility Mapping via Implicit Neural Representation in k-Space](https://arxiv.org/abs/2512.09425)
...
### [223] [PathCo-LatticE: Pathology-Constrained Lattice-Of Experts Framework for Fully-supervised Few-Shot Cardiac MRI Segmentation](https://arxiv.org/abs/2512.09779)
