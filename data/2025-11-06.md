...
TL;DR: 本研究系统评估了AI控制协议，发现在SHADE-Arena环境中，“关键行动延迟”策略对自适应攻击者具有高度鲁棒性，而“重新采样”策略则易受攻击。研究强调了保护协议内部信息的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统能力增强并广泛部署，确保其安全运行变得至关重要。AI控制提供了一种通过监控和干预来降低不受信任AI代理风险的方法。然而，需要评估这些协议在面对当前攻击和自适应对手时的有效性和鲁棒性。

Method: 研究在SHADE-Arena（一个包含多样化代理环境的数据集）中系统评估了一系列AI控制协议。首先，评估了“蓝队”协议（包括信任模型延迟、重新采样和关键行动延迟）对抗默认攻击策略的有效性。随后，迭代开发“红队”策略来对抗表现最佳的协议，包括赋予红队额外能力（如了解重新采样时机或模拟监控器）。

Result: 初步评估显示，重新采样和关键行动延迟协议表现最佳，将安全性从50%提高到96%。然而，当红队策略被赋予额外能力（如了解重新采样时机）时，对重新采样策略的攻击成功率显著提高，安全性降至17%。相比之下，关键行动延迟协议即使面对最强的红队策略也表现出高度鲁棒性。

Conclusion: “关键行动延迟”协议在AI控制中表现出极强的鲁棒性，即使面对自适应攻击者。这表明，拒绝攻击策略访问协议内部信息对于确保AI系统安全运行至关重要。
...
TL;DR: 本文提出了PublicAgent，一个多智能体框架，通过将复杂分析工作流分解为专业智能体来解决大型语言模型在处理开放数据时的局限性，并基于评估结果提出了多智能体LLM系统的五个设计原则。


<details>
  <summary>Details</summary>
Motivation: 开放数据存储库对非专家来说难以访问，因为他们缺乏数据集发现、模式映射和统计分析的专业知识。虽然大型语言模型在单一任务上表现出潜力，但在端到端分析工作流中，它们面临注意力分散、专业推理模式干扰以及错误传播等根本性限制。
...
Result: 研究得出了多智能体LLM系统的五个设计原则：1. 专业化独立于模型强度提供价值，即使最强的模型也显示出97.5%的智能体胜率。2. 智能体分为通用（发现、分析）和条件（报告、意图）类别，通用智能体表现出一致的有效性，而条件智能体则因模型而异。3. 智能体缓解了不同的故障模式，移除发现或分析会导致灾难性故障，而移除报告或意图会导致质量下降。4. 架构优势在任务复杂性上持续存在，胜率稳定，表明其工作流管理价值而非推理增强。5. 智能体在不同模型间的有效性差异很大，需要模型感知的架构设计。

Conclusion: 这些设计原则指导了在复杂分析工作流中何时以及为何需要专业化，从而通过自然语言接口更广泛地访问公共数据。
...
TL;DR: ScalingEval是一个大规模基准测试，系统性地比较了36个大型语言模型（LLMs）作为评估者在多个产品类别中的表现，并通过共识驱动的多智能体框架生成真值标签，发现不同模型在置信度、整体性能、成本效益和开源领域中的最佳表现，以及在不同类别中评估共识的差异。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型（LLMs）作为评判者对于构建可扩展且值得信赖的评估流程变得日益重要。

Method: 本研究提出了ScalingEval，一个大规模基准测试，采用共识驱动的评估协议。其多智能体框架通过可扩展的多数投票，将模式审计和问题代码聚合为真值标签，从而实现了LLM评估器的可复现比较，无需人工标注。该方法应用于大规模互补商品推荐场景，系统比较了包括GPT、Gemini、Claude和Llama在内的36个LLM。

Result: 基准测试报告了四个主要发现：(i) Anthropic Claude 3.5 Sonnet 实现了最高的决策置信度；(ii) Gemini 1.5 Pro 在所有类别中提供了最佳的整体性能；(iii) GPT-4o 提供了最有利的延迟-准确性-成本权衡；(iv) GPT-OSS 20B 在开源模型中表现领先。类别级分析显示，在结构化领域（电子产品、体育）有很强的共识，但在生活方式类别（服装、食品）中存在持续的分歧。
...
TL;DR: 本研究创建了一个基准来评估大型语言模型（LLM）对真实世界概率分布的理解能力，发现LLM在这方面表现不佳，未能自然地内化真实世界的统计数据。


<details>
  <summary>Details</summary>
Motivation: 人工智能系统需要更普遍的智能，尤其是在处理反映真实世界概率属性的概率知识方面。尽管LLM在大量文本上进行训练，可能内化了这些分布，但维度诅咒等统计学经典结果对普遍分布学习提出了挑战，因此需要直接测试LLM在这方面的能力。

Method: 开发了第一个专门用于测试LLM对描述真实世界人口的经验分布知识的基准。该基准涵盖经济、健康、教育和社会行为等多个领域的数据。

Result: 研究结果表明，LLM的整体表现不佳，似乎未能自然地内化真实世界的统计数据。它们不具备关于观察性分布（Pearl因果层级PCH的第一层）的知识。

Conclusion: LLM缺乏对观察性分布的知识，根据Pearl的因果层级定理，这意味着这些模型在干预性（第二层）和反事实性（第三层）知识方面的能力也受到限制。
...
TL;DR: 本文提出了一种名为SnapStream的KV缓存压缩方法，解决了大型语言模型（LLMs）在工业部署中对片上内存的巨大需求，同时克服了现有框架的部署挑战，并实现了显著的内存节省和最小的准确性下降。


<details>
  <summary>Details</summary>
Motivation: 随着100B+参数LLM和100k+上下文长度的支持，KV缓存对片上内存的需求急剧增加。尽管StreamingLLM和SnapKV等技术可以控制KV缓存大小并保持模型准确性，但它们并未在vLLM或SGLang等工业框架中广泛使用。原因在于：这些框架的静态图和连续批处理方法难以修改标准多头注意力算法；同时，这些技术对现代指令遵循和推理模型的准确性影响尚不明确，导致实施需求被忽视。

Method: 本文首先探究了现有KV缓存压缩技术对Llama-3.1-8B-Instruct和DeepSeek-R1模型的准确性影响。在此基础上，开发了一种名为SnapStream的KV缓存压缩方法，该方法可在大规模生产环境中部署。SnapStream旨在兼容静态图和连续批处理的推理系统。

Result: SnapStream在DeepSeek-671B的16路张量并行部署中（使用SambaNova SN40L加速器，128k上下文长度，高达1832 tokens/秒）得到验证。它实现了4倍的片上内存使用改进，并在LongBench-v2、AIME24和LiveCodeBench基准测试中引入了最小的准确性下降。据作者所知，这是首次将稀疏KV注意力技术部署到具有静态图和连续批处理的生产推理系统中。

Conclusion: SnapStream成功地解决了大规模LLM推理中KV缓存对内存的巨大需求问题，通过在生产环境中实现4倍的内存节省和最小的准确性损失。它克服了在现有工业框架中部署此类技术的挑战，证明了稀疏KV注意力技术在实际生产系统中的可行性和有效性。
...
TL;DR: 本文提出了一种名为“基于能力监测”的新方法，用于在医疗保健领域对大型语言模型（LLMs）进行可扩展、安全和协作的监测，以解决传统任务型监测的不足。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在医疗保健领域的快速应用，对其监督的审查也随之而来。传统的机器学习（ML）监测方法是基于任务的，并假设性能下降源于数据集漂移。然而，LLMs并非为特定任务或人群训练，其模型退化不能简单地归因于人群变化。因此，需要一种新的、可扩展的监测原则来适应LLMs的通用性和实际应用方式。

Method: 本文提出“基于能力监测”的方法。该方法围绕LLMs共享的内部能力（如摘要、推理、翻译、安全防护等）组织监测，而不是独立评估每个下游任务。这种方法旨在跨任务检测系统性弱点、长尾错误和新兴行为，这些是传统任务型监测可能遗漏的。

Result: 通过基于能力监测，可以实现跨任务检测系统性弱点、长尾错误和新兴行为。这种方法为开发人员、组织领导者和专业协会提供了一个可扩展的框架，以更有效地监测LLMs的性能和行为。

Conclusion: 基于能力监测将为医疗保健领域中LLMs和未来通用人工智能模型的安全、自适应和协作监测提供可扩展的基础。
...
### [7] [Using Multi-modal Large Language Model to Boost Fireworks Algorithm's Ability in Settling Challenging Optimization Tasks](https://arxiv.org/abs/2511.03137)
*Shipeng Cen,Ying Tan*

Main category: cs.AI

TL;DR: 本研究提出了一种利用多模态大语言模型（MLLM）辅助设计烟花算法（FWA）的新框架，通过引入“关键部分”（CP）概念，有效解决了复杂高维优化问题，并在旅行商问题（TSP）和电子设计自动化（EDA）问题上取得了超越或达到SOTA的结果。


<details>
  <summary>Details</summary>
Motivation: 优化问题日益复杂多样，表现出非凸性、高维度、黑盒等挑战。传统零阶或一阶方法效率低、梯度信息不准、优化信息利用不足，难以有效应对。近年来，大语言模型（LLM）在语言理解和代码生成方面取得显著进展，为优化算法设计提供了新思路。

Method: 本研究选择烟花算法（FWA）作为基础优化器，并提出了一种新方法，通过结合多模态大语言模型（MLLM）辅助FWA的设计。具体而言，引入了“关键部分”（CP）概念，将FWA扩展到复杂高维任务，并利用MLLM的多模态特性来更好地利用优化过程中的信息。研究重点关注旅行商问题（TSP）和电子设计自动化（EDA）问题。

Result: 实验结果表明，在该新框架下生成的烟花算法（FWA）在许多问题实例上达到了或超越了当前最先进（SOTA）的结果。

Conclusion: 本研究成功展示了多模态大语言模型在辅助设计优化算法方面的潜力，特别是通过引入“关键部分”概念，显著提升了烟花算法在复杂高维问题（如TSP和EDA）上的性能，取得了优异的成果。
...
TL;DR: 本文对miniF2F基准测试进行了深入分析，发现其形式化和非形式化陈述存在大量不一致。通过纠正这些问题，作者创建了miniF2F-v2，并在此新基准上将AI定理证明管线的准确率从36%（原miniF2F的40%）提高到70%，强调了高质量基准对评估和诊断AI形式化推理模型的重要性。


<details>
  <summary>Details</summary>
Motivation: 在数学奥林匹克竞赛场景中，AI系统需要理解自然语言问题、将其形式化为Lean语言并进行证明。尽管现有自动形式化和定理证明模型的个体准确率很高（分别为97%和69%），但当它们整合到一个完整的管线中解决miniF2F问题时，整体准确率却非常低（约36%）。这促使研究人员深入分析这种性能下降的原因。

Method: 研究人员首先从AI系统的角度对miniF2F基准中的形式化和非形式化陈述进行了全面分析。他们评估了最先进（SoTA）模型在该管线上的表现，并分析了失败模式。通过追溯，他们发现性能下降很大一部分源于miniF2F中超过一半问题在形式化和非形式化陈述之间的不一致。随后，他们纠正了所有错误、不一致和简化，创建了miniF2F-v2，其中包含经过完全验证的形式化和非形式化陈述及证明。最后，他们在新基准miniF2F-v2上重新评估了完整的定理证明管线。

Result: 在原始miniF2F上，使用SoTA模型的完整管线准确率约为36%（论文中也提到40%，可能指特定配置下的最佳值）。分析发现，大部分性能下降归因于miniF2F中超过一半问题的形式化和非形式化陈述之间的差异。经过纠正后，miniF2F-v2被创建。在miniF2F-v2上评估，完整定理证明管线的最佳准确率达到了70%，相较于原始miniF2F的40%（或36%）有显著提升，但仍表明自动形式化模型和定理证明器之间存在相当大的未对齐。

Conclusion: 高质量的基准测试对于社区更好地评估形式化推理领域的进展至关重要。例如miniF2F-v2这样的改进基准，可以帮助更准确地诊断自动形式化和定理证明模型的失败和成功模式，从而推动该领域的发展。
...
TL;DR: 本文提出一个新颖的LLM安全响应框架，通过在输入端进行精细化风险识别和处理，并在输出端结合RAG和微调模型确保响应的真实性和可追溯性，从而系统性地保障LLM的安全性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）的广泛应用，其相关的安全问题日益突出，严重制约了它们在关键领域的可信部署。

Method: 该框架在输入层采用基于监督微调（SFT）的安全分类模型，通过四层（安全、不安全、有条件安全、重点关注）分类法进行风险识别和差异化处理。在输出层，框架整合了检索增强生成（RAG）与专门微调的解释模型，确保所有响应都基于实时可信的知识库，消除信息编造并实现结果可追溯性。

Result: 在输入层，实现了99.3%的风险召回率。在公共安全评估基准上，提出的安全控制模型比基线模型TinyR1-Safety-8B获得了显著更高的安全分数。在专有的高风险测试集上，框架组件达到了100%的安全分数。

Conclusion: 本研究为构建高安全性、高信任度的LLM应用提供了一条有效的工程路径，验证了其在复杂风险场景下的卓越保护能力。
...
TL;DR: 本文提出了一种验证形式化可解释AI解释器的新方法，并通过评估PyXAI发现其在多个数据集上生成了不正确的解释，强调了该验证方法的重要性。


<details>
  <summary>Details</summary>
Motivation: 形式化可解释AI（XAI）在理论上提供了严格的保证，但其实际实现缺乏验证，这促使了对形式化解释器验证方法的研究。

Method: 开发了一种新颖的方法论，用于验证形式化解释器，并将其应用于评估公开可用的形式化解释器PyXAI。
...
TL;DR: 本文提出一个多智能体AI框架，通过整合结构化设计和评审循环来形式化工程设计过程，并以NACA翼型气动优化为例，展示了其如何利用LLM构建的知识图谱和协作智能体提升设计效率和质量。


<details>
  <summary>Details</summary>
Motivation: 传统的工程设计过程通常需要多领域专业知识，导致复杂的协作、迭代精炼、资源密集且效率低下。

Method: 该框架包含三个关键AI智能体：图本体论专家（Graph Ontologist）、设计工程师（Design Engineer）和系统工程师（Systems Engineer）。图本体论专家利用大型语言模型（LLM）从文献中构建两个领域特定知识图谱。系统工程师根据人类管理者制定的技术要求，指导设计生成和评估。设计工程师利用设计知识图谱和计算工具提出候选设计。系统工程师利用其知识图谱进行定性和定量评审并提供反馈，形成迭代反馈循环，直至设计获得管理者验证。最终设计会进一步优化以最大化性能指标。

Result: 该框架成功应用于4位NACA翼型的气动优化，展示了协作AI智能体如何通过结构化知识表示来增强工程设计过程的效率、一致性和质量。
...
TL;DR: 本文介绍了Summit Concierge，一个为Adobe Summit开发的领域特定AI助手。该助手通过结合提示工程、检索接地和人工验证的“人机协作”开发流程，解决了数据稀疏和快速部署等实际挑战，并展示了其在冷启动场景下的可扩展性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 生成式AI助手在企业环境中具有提升生产力、简化信息获取和改善用户体验的巨大潜力。本文旨在开发一个领域特定的AI助手，以应对Adobe Summit等活动中遇到的实际约束，如数据稀疏性和质量保证。

Method: 本文采用了一种“人机协作”的开发流程，结合了提示工程（prompt engineering）、检索接地（retrieval grounding）和轻量级人工验证（lightweight human validation）。研究描述了系统架构、开发过程以及实际部署成果。

Result: Summit Concierge助手能够处理广泛的活动相关查询。研究经验表明，敏捷的、反馈驱动的开发方法能够实现可扩展且可靠的AI助手，即使在冷启动场景下也能成功部署。

Conclusion: 敏捷的、反馈驱动的开发方法对于构建可扩展且可靠的AI助手至关重要，特别是在数据稀疏和需要快速部署的冷启动企业环境中。
...
TL;DR: 研究发现大型语言模型（LLMs）能从最少输入中准确模拟人类心理特质的相关结构，其通过抽象和推理的两阶段过程，性能优于语义相似度，接近于经过训练的机器学习算法。


<details>
  <summary>Details</summary>
Motivation: 心理构念被广泛认为是相互关联的。本研究旨在探究LLMs是否以及如何能够从最少的定量输入中建模人类心理特质的相关结构。

Method: 研究人员向不同的LLMs提供了来自816名人类受试者的“大五人格量表”答复，并要求LLMs扮演角色，对其他九个心理量表做出回应。他们比较了LLM生成的回应与人类数据之间的量表间相关模式，并分析了LLMs的推理轨迹。

Result: LLMs在捕捉人类心理结构方面表现出卓越的准确性，其生成的量表间相关模式与人类数据高度一致（R² > 0.89）。这种零样本表现显著优于基于语义相似性的预测，并接近于直接在数据集上训练的机器学习算法。分析显示，LLMs采用系统性的两阶段过程：首先，将原始“大五”答复转化为自然语言人格摘要；其次，基于这些摘要进行推理生成目标量表回应。LLMs识别出与训练算法相同的关键人格因素，但未能区分因素内部的项目重要性。压缩后的摘要不仅是冗余表示，还捕获了协同信息，能增强预测一致性。

Conclusion: 研究表明，LLMs能够通过抽象和推理过程，从最少的数据中精确预测个体参与者的心理特质。这为心理模拟提供了一个强大的工具，并为LLMs的涌现推理能力提供了有价值的见解。
...
TL;DR: 本文提出了一个名为AAA的人机协作框架，通过图基多模态采样方法GRASP和多模态大语言模型副驾驶MaC，将WCAG-EM操作化，实现了可扩展、端到端的网络无障碍审计，并贡献了四个新数据集。


<details>
  <summary>Details</summary>
Motivation: 网络无障碍对于数字空间的社会福利、正义和公平至关重要，然而绝大多数网站用户界面不合规。当前的审计实践（如WCAG-EM）资源密集且难以扩展，需要大量人工投入，缺乏大规模执行的实际支持。

Method: 本研究提出了AAA审计框架，通过人机协作模式将WCAG-EM操作化。AAA包含两项关键创新：1) GRASP，一种基于图的多模态采样方法，利用视觉、文本和关系线索的学习嵌入确保代表性的页面覆盖；2) MaC，一个基于多模态大语言模型的副驾驶，通过跨模态推理和智能辅助支持审计人员完成高难度任务。此外，还贡献了四个用于基准测试审计流程核心阶段的新数据集。

Result: 广泛的实验证明了所提出方法的有效性，并提供了见解，表明经过微调的小规模语言模型可以作为有能力的专家。
...
TL;DR: 本文对透明机器学习模型中解释问题的参数化复杂性进行了全面的理论研究。


<details>
  <summary>Details</summary>
Motivation: 现有研究在可解释AI (XAI) 领域中，缺乏对透明内部机制模型生成解释复杂性的基础理解，本研究旨在填补这一空白，并促进AI系统透明度和可问责性的讨论。

Method: 采用理论调查方法，研究了多种透明ML模型（包括决策树、决策集、决策列表、布尔电路及其集成）中，溯因式和对比式（局部和全局）两种主要解释问题的参数化复杂性。

Result: 本研究提供了对为透明ML模型生成解释的复杂性的基础理解，并为XAI领域的进一步研究提供了重要见解。

Conclusion: 本工作为可解释AI领域奠定了理论基础，并强调了AI系统透明度和可问责性的重要性。
...
TL;DR: Solly是首个在多玩家“吹牛扑克”（Liar's Poker）中达到精英人类水平的AI代理，它通过深度强化学习进行自我对弈训练，超越了大型语言模型，并开发出新颖的策略。


<details>
  <summary>Details</summary>
Motivation: AI研究在扑克类游戏（如无限注德州扑克）中取得了突破，但这些游戏的多数手牌中多玩家动态不明显。研究人员希望解决具有广泛多玩家互动、不完美信息和不确定性推理特点的游戏挑战。

Method: Solly通过自我对弈训练，采用了无模型、actor-critic深度强化学习算法。

Result: Solly在单挑和多玩家“吹牛扑克”中均达到了精英人类水平，其胜率超过50%，且在收益方面表现出色。它在相同指标上优于包括具有推理能力在内的大型语言模型。Solly发展出新颖的叫牌策略，有效地随机化了玩法，并且不易被世界级人类玩家利用。

Conclusion: Solly证明了AI在处理具有复杂多玩家动态和不完美信息的游戏中达到了新的里程碑，展示了其在“吹牛扑克”中超越人类和LLMs的能力，并开发出不易被利用的策略。
...
### [17] [Cropland Mapping using Geospatial Embeddings](https://arxiv.org/abs/2511.02923)
*Ivan Zvonkov,Gabriel Tseng,Inbal Becker-Reshef,Hannah Kerner*

Main category: cs.CV

TL;DR: 本研究评估了地理空间嵌入在多哥农田测绘中的实用性，发现其能简化工作流程并实现高精度分类，从而更好地评估土地利用变化及其气候影响。


<details>
  <summary>Details</summary>
Motivation: 准确且实时的土地覆盖图对于理解土地利用变化（气候变化的关键驱动因素）至关重要。尽管地理空间嵌入提供了高效便捷的景观特征测绘方式，但其在实际测绘应用中的潜力尚未得到充分探索。

Method: 研究评估了地理空间嵌入在多哥农田测绘中的效用。具体使用了来自Presto和AlphaEarth的嵌入来制作农田地图。

Result: 研究结果表明，地理空间嵌入能够简化工作流程，实现高精度的农田分类。

Conclusion: 地理空间嵌入最终能够支持对土地利用变化及其气候影响进行更准确的评估。
...
### [18] [Generative Hints](https://arxiv.org/abs/2511.02933)
*Andy Dimnaku,Abdullah Yusuf Kavranoğlu,Yaser Abu-Mostafa*

Main category: cs.CV

TL;DR: 本文提出“生成式提示”（generative hints），一种训练方法，通过生成模型生成虚拟样本，直接在整个输入空间中强制执行已知不变性，从而在学习相同属性时，性能显著优于传统数据增强。


<details>
  <summary>Details</summary>
Motivation: 数据增强广泛用于视觉任务中以引入变化和缓解过拟合，但它仅通过训练数据的变换来学习不变性，无法充分捕获这些不变性属性。

Method: 该方法利用一个在训练集上训练的生成模型来近似输入分布并生成无标签的“虚拟样本”。然后，主模型以半监督方式进行训练，结合分类目标和“提示”目标（即已知的功能属性），使用这些虚拟样本来指导模型学习所需的不变性。

Result: 在各种数据集、架构和损失函数下，生成式提示在学习相同属性时始终优于标准数据增强。在流行的细粒度视觉分类基准上，相对于经过数据增强的微调模型，Top-1准确率最高提升1.78%（平均0.63%）；在CheXpert X射线数据集上，平均性能提升1.286%。

Conclusion: 生成式提示通过利用生成模型和虚拟样本，能够直接在整个输入空间中强制执行已知不变性，从而有效提高模型的泛化能力和性能，优于仅依赖数据增强的方法。
...
### [19] [ProM3E: Probabilistic Masked MultiModal Embedding Model for Ecology](https://arxiv.org/abs/2511.02946)
*Srikumar Sastry,Subash Khanal,Aayush Dhakal,Jiayu Lin,Dan Cher,Phoenix Jarosz,Nathan Jacobs*

Main category: cs.CV

TL;DR: ProM3E是一种用于生态学的概率掩码多模态嵌入模型，支持任意模态生成、模态反转，并能学习如何融合模态，在跨模态检索和表示学习方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 需要一个灵活的多模态嵌入模型，能够处理缺失模态、支持任意模态间的生成，并智能地融合不同模态以适应下游任务，尤其是在生态学领域。

Method: ProM3E基于嵌入空间中的掩码模态重建，通过少量上下文模态推断缺失模态。模型具有概率性质，支持嵌入空间中的模态反转。作者提出了一种结合模态间和模态内相似性的新型跨模态检索方法，并利用模型的隐藏表示进行线性探测任务。

Result: 在所有检索任务中，提出的跨模态检索方法都取得了卓越的性能。通过线性探测任务，证明了模型优越的表示学习能力。

Conclusion: ProM3E是一个功能强大且灵活的概率掩码多模态嵌入模型，能够实现生态学领域中任意模态的生成、模态反转以及智能的模态融合，从而在跨模态检索和表示学习方面取得显著成效。
...
### [20] [EvtSlowTV -- A Large and Diverse Dataset for Event-Based Depth Estimation](https://arxiv.org/abs/2511.02953)
*Sadiq Layi Macaulay,Nimet Kaygusuz,Simon Hadfield*

Main category: cs.CV

TL;DR: 为解决事件相机深度估计受限于小规模数据集的问题，本文推出了大型事件相机数据集EvtSlowTV，该数据集来源于YouTube视频，包含超过130亿个事件，支持自监督学习并显著提升模型在复杂场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 事件相机在恶劣环境下进行鲁棒深度估计具有巨大潜力，但现有方法受限于小规模标注数据集，导致其在真实世界场景中的泛化能力不足。

Method: 本文引入了EvtSlowTV，一个从公开YouTube视频中整理而成的大规模事件相机数据集，包含超过130亿个事件，涵盖多种环境条件和运动。该方法利用自监督学习框架，充分利用原始事件流的HDR潜力，并移除了对基于帧的标注的需求，保留了事件数据的异步特性。

Result: EvtSlowTV的规模比现有事件数据集大一个数量级，为事件深度学习提供了无约束的自然环境。结果表明，使用EvtSlowTV进行训练能显著增强模型对复杂场景和运动的泛化能力，并有效利用原始事件流的HDR潜力。

Conclusion: EvtSlowTV通过提供大规模、自然化的事件数据，弥补了事件相机深度估计领域的数据鸿沟。它支持自监督学习，提高了模型的泛化能力，同时保留了事件数据的异步特性，充分发挥了事件相机的优势。
...
### [21] [Hybrid Convolution and Vision Transformer NAS Search Space for TinyML Image Classification](https://arxiv.org/abs/2511.02992)
*Mikhael Djajapermana,Moritz Reiber,Daniel Mueller-Gritschneder,Ulf Schlichtmann*

Main category: cs.CV

TL;DR: 本文提出了一种新的混合CNN-ViT神经架构搜索（NAS）搜索空间，旨在为TinyML部署找到高效且性能优越的混合架构，以克服现有混合模型参数大、计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 尽管卷积神经网络（CNN）和Vision Transformer（ViT）的混合架构表现优于纯CNN或ViT，但它们需要大量参数和高计算成本，不适用于TinyML部署。

Method: 引入了一个新的混合CNN-ViT搜索空间用于神经架构搜索（NAS）。该搜索空间包含混合CNN和ViT块以学习局部和全局信息，以及新颖的可搜索池化层（Pooling block）用于高效特征图降维。

Result: 在CIFAR10数据集上的实验结果表明，在严格的模型尺寸限制下，所提出的搜索空间能够生成具有优越准确性和推理速度的混合CNN-ViT架构，优于基于ResNet的TinyML模型。

Conclusion: 所提出的NAS搜索空间能够有效地发现适用于图像分类的、在模型尺寸受限下仍能提供卓越精度和推理速度的混合CNN-ViT架构，解决了TinyML部署的效率挑战。
...
### [22] [ISC-Perception: A Hybrid Computer Vision Dataset for Object Detection in Novel Steel Assembly](https://arxiv.org/abs/2511.03098)
*Miftahur Rahman,Samuel Adebayo,Dorian A. Acevedo-Mejia,David Hester,Daniel McPolin,Karen Rafferty,Debra F. Laefer*

Main category: cs.CV

TL;DR: 本文介绍了ISC-Perception，一个用于Intermeshed Steel Connection (ISC)组件检测的混合数据集，旨在解决建筑机器人感知数据不足的问题，并通过结合合成和少量真实数据，显著提高了目标检测器的性能，同时大幅减少了数据集标注所需的人工时间。


<details>
  <summary>Details</summary>
Motivation: 将Intermeshed Steel Connection (ISC)系统与机器人机械手结合可以加速钢结构装配并提高工人安全性，但实现这一目标需要可靠的机器人感知能力。然而，目前缺乏专用的图像语料库，因为在活跃的建筑工地收集照片存在物流困难、安全和隐私问题。

Method: 研究人员创建了ISC-Perception数据集，这是第一个专为ISC组件检测设计的混合数据集。它结合了程序渲染的CAD图像、游戏引擎生成的逼真场景以及少量精选的真实照片。合成部分实现了全自动标注。作者明确量化了生成数据集的所有人工投入，包括仿真引擎和场景设置、资产准备、后处理脚本和质量检查。

Result: 生成10,000张图像的数据集所需的人工时间为30.5小时，相比手动标注（每张图像60秒）所需的166.7小时，减少了81.7%。在ISC-Perception上训练的检测器在IoU 0.50时的平均精度(mAP)达到0.756，显著优于仅使用合成数据或仅使用逼真数据训练的模型。在1,200帧的基准测试中，报告的mAP@0.50/mAP@[0.50:0.95]分别为0.943/0.823。

Conclusion: ISC-Perception通过弥补建筑机器人感知领域的数据空白，促进了定制对象检测器的快速开发。该数据集免费提供给研究和工业使用，从而推动了机器人辅助钢结构装配技术的发展。
...
### [23] [SCALE-VLP: Soft-Weighted Contrastive Volumetric Vision-Language Pre-training with Spatial-Knowledge Semantics](https://arxiv.org/abs/2511.02996)
*Ailar Mahdizadeh,Puria Azadi Moghadam,Xiangteng He,Shahriar Mirabbasi,Panos Nasiopoulos,Leonid Sigal*

Main category: cs.CV

TL;DR: SCALE-VLP是一种软加权对比视觉-语言预训练框架，专为CT等体积数据设计，通过整合体积空间语义和领域知识引导的语义，在有限监督下实现结构一致且语义扎实的表示，并在多任务和跨领域上展现出强大的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型(VLMs)大多局限于2D数据和二元监督，忽视了CT等体积数据中连续和结构化的依赖关系。此外，现有方法常将体积扫描视为独立的2D切片，损害了空间连贯性并未能充分利用丰富的临床语义。

Method: 本文提出了SCALE-VLP，一个软加权对比视觉-语言预训练框架。它主要整合了两个方面：(i) 体积空间语义，以保留解剖结构；(ii) 领域感知、知识注入的语义（如放射学本体论），以指导对齐。这在有限监督下产生了结构一致和语义扎实的表示。

Result: SCALE-VLP在跨任务（检索、报告生成和分类）和跨领域泛化方面表现出色，无需进一步微调即可获得持续提升。与现有SOTA相比，CT-报告检索的top-1准确率提高了4.3倍，异常分类提高了10个点，报告生成达到ROUGE-L 0.44和BERT-F1 0.89。在零样本、域外数据集评估中也观察到持续的性能提升。

Conclusion: SCALE-VLP通过结合体积空间语义和领域知识，在有限监督下成功学习到结构一致和语义扎实的视觉-语言表示。它在多任务和跨领域上展现出强大的可迁移性和泛化能力，显著优于现有方法。
...
### [24] [Learning with less: label-efficient land cover classification at very high spatial resolution using self-supervised deep learning](https://arxiv.org/abs/2511.03004)
*Dakota Hester,Vitor S. Martins,Lucas B. Ferreira,Thainara M. A. Lima*

Main category: cs.CV

TL;DR: 本研究提出了一种标签高效的方法，利用自监督学习和少量标注数据（仅1000个图像块）实现了密西西比州1米分辨率的大规模土地覆盖分类，解决了深度学习模型对大量训练数据需求的问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习语义分割在1米分辨率土地覆盖分类中表现出色，但收集大量代表性训练数据的挑战是其在大面积米级土地覆盖制图中广泛应用的重大障碍。

Method: 研究采用“Bootstrap Your Own Latent”（BYOL）自监督预训练策略，利用大量未标注的彩色红外航空影像（377,921个256x256 1米像素块）预训练一个ResNet-101卷积编码器。随后，将学习到的编码器权重迁移到多种深度语义分割架构（FCN, U-Net, Attention U-Net, DeepLabV3+, UPerNet, PAN），并使用非常小的训练数据集（250、500、750个图像块）进行交叉验证微调。最终，使用表现最佳的U-Net模型集成进行预测。

Result: 通过最佳U-Net模型的集成，在密西西比州实现了1米分辨率、8类土地覆盖制图，覆盖超过1230亿像素，获得了87.14%的总体准确率和75.58%的宏观F1分数。定性和定量分析表明，开放水域和森林区域的制图准确，但在耕地、草本植物和贫瘠土地类型之间的准确划分仍面临挑战。

Conclusion: 这些结果表明，自监督学习是有效减少手动标注数据需求的一种策略，直接解决了高空间分辨率大规模土地覆盖制图的主要限制。
...
### [25] [A Foundation Model for Brain MRI with Dynamic Modality Integration](https://arxiv.org/abs/2511.03014)
*Minh Sao Khue Luu,Bair N. Tuchinov*

Main category: cs.CV

TL;DR: 本文提出了一种用于脑部MRI的基础模型，该模型能处理不同成像序列组合，包括缺失模态。它使用一个带有可学习模态嵌入、条件层归一化和掩蔽自编码目标的编码器，并通过方差-协方差正则化来稳定特征学习。该模型在6万张多中心MRI数据上进行自监督训练，初步结果显示其可行性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要为每种MRI模态建立单独模型，并且难以处理某些序列缺失或未见的情况，限制了模型的灵活性和泛化能力。

Method: 该模型采用单一编码器架构，结合可学习的模态嵌入和条件层归一化。它使用掩蔽自编码目标来处理缺失模态，并通过方差-协方差正则器来稳定特征学习并增加表示多样性。模型在约60,000张多中心MRI数据上进行自监督重建和模态插补训练，以学习灵活的表示。

Result: 初步结果显示该方法是可行的。作者计划在脑肿瘤和多发性硬化症分割以及病灶分类任务上，在各种模态设置下进行进一步的详细评估。所有代码和预训练模型均已公开。

Conclusion: 所提出的脑部MRI基础模型能够灵活处理不同成像序列组合及缺失模态，通过其独特设计（如模态嵌入和掩蔽自编码）实现了单模型处理多模态的能力。初步实验证明了其可行性，未来将进行更全面的性能评估。
...
### [26] [SLIP: Structural-aware Language-Image Pretraining for Vision-Language Alignment](https://arxiv.org/abs/2511.03019)
*Wenbo Lu*

Main category: cs.CV

TL;DR: 该论文提出了结构感知语言-图像预训练（SLIP）模型，通过整合结构对比损失来对齐多模态，并建模实体间的关系。SLIP在亚马逊商品共购多模态图数据集上训练，在跨模态检索和分类任务上显著优于CLIP，证明了关系监督的价值。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言预训练（VLP）模型虽然成功，但主要依赖于大规模数据，并将图像-文本对视为独立的训练样本，忽略了许多领域（如电子商务共购图、社交推荐网络）中自然存在的丰富关系结构。受人类将知识编码为关系认知图的神经科学证据启发，研究旨在将这种结构信息引入VLP。

Method: 引入结构感知语言-图像预训练（SLIP）模型。SLIP集成了一个结构对比损失，旨在对齐不同模态，同时对结构化图中相邻实体之间的关系进行建模。为支持此范式，构建了一个大规模的亚马逊商品共购多模态图数据集，以实现大规模的结构化跨模态监督。

Result: 实验结果表明，SLIP在零样本和少样本设置下的跨模态检索和分类任务中，始终优于CLIP模型。这证明了关系监督对于跨模态对齐的有效性。

Conclusion: 该研究得出结论，将关系结构信息整合到视觉-语言预训练中具有显著价值。通过引入结构感知语言-图像预训练（SLIP）模型和大规模结构化数据集，证明了关系监督能够有效提升跨模态对齐任务的性能。
...
### [27] [From Propagation to Prediction: Point-level Uncertainty Evaluation of MLS Point Clouds under Limited Ground Truth](https://arxiv.org/abs/2511.03053)
*Ziyang Xu,Olaf Wysocki,Christoph Holst*

Main category: cs.CV

TL;DR: 本研究提出了一种基于学习的框架，通过几何特征预测移动激光扫描（MLS）点云的不确定性，以减少对真值（GT）的依赖。


<details>
  <summary>Details</summary>
Motivation: 在许多高精度应用中，评估移动激光扫描（MLS）点云的不确定性至关重要，但获取用于评估的真值（GT）通常成本高昂且在实际应用中不可行。

Method: 本研究提出一个基于学习的框架，该框架将最优邻域估计与几何特征提取相结合。实验中使用了XGBoost和Random Forest模型，并以C2C距离量化点级别的不确定性。
...
### [28] [A Plug-and-Play Framework for Volumetric Light-Sheet Image Reconstruction](https://arxiv.org/abs/2511.03093)
*Yi Gong,Xinyuan Zhang,Jichen Chai,Yichen Ding,Yifei Lou*

Main category: cs.CV

TL;DR: 本文提出了一种高性能计算成像框架，将压缩感知（CS）与光片显微镜（LSM）结合，用于高效、低光毒性的心脏成像，通过PnP-ADMM框架和时间正则化实现了高压缩比下的细胞结构重建和去噪。


<details>
  <summary>Details</summary>
Motivation: 传统光学成像在捕捉跳动心脏的动态细胞结构时，由于空间和时间分辨率之间的基本权衡，往往无法满足要求，因此需要一种新的方法来克服这些限制。

Method: 该系统通过数字微镜器件（DMD）随机二值掩模编码对荧光信号进行压缩采集。提出了一种即插即用（PnP）框架，使用交替方向乘子法（ADMM）求解，并灵活地整合了Tikhonov、全变分（TV）和BM3D等先进去噪器。为保持动态成像的结构连续性，进一步引入了时间正则化以确保相邻Z切片之间的平滑性。

Result: 在斑马鱼心脏成像实验中，即使在高压缩比下，所提出的方法也成功重建了细胞结构，并表现出卓越的去噪性能和图像清晰度。

Conclusion: 实验结果验证了该算法在真实世界高速、低光生物成像场景中的有效性和鲁棒性。
...
### [29] [DentalSplat: Dental Occlusion Novel View Synthesis from Sparse Intra-Oral Photographs](https://arxiv.org/abs/2511.03099)
*Yiyi Miao,Taoyu Wu,Tong Chen,Sihao Li,Ji Jiang,Youpeng Yang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: 本文提出DentalSplat框架，利用先验引导的稠密立体重建、尺度自适应剪枝和光流几何约束，解决了正畸治疗中从稀疏图像进行牙齿咬合3D重建的挑战，并实现了卓越的新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 正畸治疗（尤其是远程医疗）中，从多视角观察患者牙齿咬合对临床决策至关重要。现有3D高斯泼溅（3DGS）技术依赖密集多视角输入和精确相机姿态，但在正畸案例中，通常只有三张稀疏图像（前视图和双侧颊视图），且缺乏相机姿态信息，这严重降低了重建质量和实用性。

Method: 本文提出了DentalSplat框架。首先，利用先验引导的稠密立体重建模型初始化点云。其次，采用尺度自适应剪枝策略提高3DGS的训练效率和重建质量。对于极度稀疏的视角，进一步结合光流作为几何约束和梯度正则化来增强渲染保真度。

Result: 该方法在包含950个临床病例的大规模数据集和195个模拟真实远程正畸成像条件的视频测试集上进行了验证。实验结果表明，DentalSplat能有效处理稀疏输入场景，并在牙齿咬合可视化方面实现了优于现有最先进技术的新视角合成质量。

Conclusion: DentalSplat框架成功克服了从稀疏正畸图像进行3D重建的限制，实现了卓越的牙齿咬合可视化新视角合成，为远程正畸治疗中的及时临床决策提供了有效支持。
...
### [30] [Image-Intrinsic Priors for Integrated Circuit Defect Detection and Novel Class Discovery via Self-Supervised Learning](https://arxiv.org/abs/2511.03120)
*Botong. Zhao,Xubin. Wang,Shujing. Lyu,Yue. Lu*

Main category: cs.CV

TL;DR: 该论文提出IC DefectNCD，一个无支持集框架，利用集成电路SEM图像的内在先验，实现缺陷检测和新类别发现，克服了传统监督和无监督方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 集成电路制造复杂，缺陷导致良率损失和产品可靠性下降。现有监督方法需大量人工标注，难以处理突发和稀有缺陷；基于聚类的无监督方法因缺乏先验知识而性能不稳定。

Method: 提出IC DefectNCD框架。首先，通过“自正常信息引导的IC缺陷检测”模块，学习正常特征并利用重建残差进行粗定位，再通过自适应二值化策略生成稳定的缺陷区域子图像。其次，通过“自缺陷信息引导的IC缺陷分类”模块，将软掩码引导的注意力机制融入师生模型，注入空间缺陷先验，增强对缺陷区域的敏感性并抑制背景干扰，从而识别和分类未见过的缺陷。

Result: 在涵盖三个关键制造阶段和15种缺陷类型的真实世界数据集上进行验证，实验结果表明该方法在缺陷检测和未见缺陷分类方面均表现出鲁棒性能。

Conclusion: IC DefectNCD框架通过利用图像内在先验，有效解决了集成电路缺陷检测和新类别发现中的挑战，尤其在处理稀有和未见缺陷方面展现出强大能力。
...
### [31] [Accelerating Physical Property Reasoning for Augmented Visual Cognition](https://arxiv.org/abs/2511.03126)
*Hongbo Lan,Zhenlin An,Haoyu Li,Vaibhav Singh,Longfei Shangguan*

Main category: cs.CV

TL;DR: 本文介绍了一个名为\sysname的系统，通过算法和系统优化，将视觉引导的物理属性推理的端到端延迟从10-20分钟缩短到6秒以内，同时保持或提高了估计精度，并在真实世界环境中展现出强大的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前的视觉引导物理属性推理管道存在高延迟问题（10-20分钟），这限制了增强视觉认知（augmented visual cognition）的应用和实时性需求。

Method: 该系统采用了一系列算法和系统优化措施，包括快速几何3D重建、高效语义特征融合和并行视图编码。此外，还结合了眼动追踪技术来定位复杂真实环境中的目标物体，以实现智能眼镜上的物理属性推理。

Result: \sysname将推理管道的端到端延迟从10-20分钟缩短到不到6秒，实现了62.9倍至287.2倍的加速。在ABO数据集上，其物体级别物理属性（如质量）估计精度与现有SOTA基线相当甚至略优，并在材料分割和体素级别推理方面表现更佳。在真实世界场景中，即使视图较少，也能提供鲁棒且一致的高性能属性估计。

Conclusion: \sysname通过显著降低延迟并保持高精度，极大地加速了视觉引导的物理属性推理，使其能够应用于智能眼镜等设备，在真实、复杂的环境中实现强大的增强视觉认知能力。
...
### [32] [Deploying Rapid Damage Assessments from sUAS Imagery for Disaster Response](https://arxiv.org/abs/2511.03132)
*Thomas Manzini,Priyankari Perali,Robin R. Murphy*

Main category: cs.CV

TL;DR: 本文介绍了首个在联邦宣布的灾害（飓风黛比和海伦）中实际部署的，用于自动化无人机（sUAS）图像建筑损害评估的AI/ML系统。


<details>
  <summary>Details</summary>
Motivation: 在重大灾害发生后，无人机团队每天收集大量图像（47GB至369GB），超出了现场专家合理传输和解读的能力，从而延误了响应工作。尽管已有卫星图像的自动化损害评估工作，但无人机图像的损害评估系统仍停留在学术阶段，缺乏实际应用。

Method: 开发并部署了用于无人机图像建筑损害评估的AI/ML模型。模型训练使用了已知最大的灾后无人机航空图像数据集，包含21,716个建筑损害标签。同时，对91名灾害从业人员进行了操作培训。性能最佳的模型在实际灾害响应中得到了部署。

Result: 性能最佳的模型在飓风黛比和海伦的响应期间投入使用，在约18分钟内评估了总计415座建筑物。

Conclusion: 这项工作建立了无人机图像损害评估的AI/ML实践现状，记录了AI/ML在灾害中进行损害评估的实际应用，并为AI/ML研究和用户社区提供了经验教训。
...
### [33] [Finetuning-Free Personalization of Text to Image Generation via Hypernetworks](https://arxiv.org/abs/2511.03156)
*Sagar Shrestha,Gopal Sharma,Luowei Zhou,Suren Kumar*

Main category: cs.CV

TL;DR: 本文提出一种无需微调的个性化文本到图像扩散模型方法，通过超网络直接从主题图像预测LoRA权重，并结合端到端训练目标和混合模型无分类器引导（HM-CFG），实现了高效且高质量的主题保真度和组合泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统的个性化方法（如DreamBooth）计算成本高且推理慢。近期基于适配器和编码器的方法虽有所改进，但仍需额外微调或依赖大型骨干模型。之前的超网络方法则面临数据生成昂贵或模仿基模型优化轨迹不稳定的问题。

Method: 本研究重新审视了通过超网络实现无需微调的个性化方向，超网络直接从主题图像预测LoRA适应权重。通过引入一个端到端训练目标，并辅以简单的输出正则化来稳定超网络，解决了现有超网络方法的局限性。为增强推理时的组合泛化能力，提出了混合模型无分类器引导（HM-CFG），它在采样过程中结合了基础扩散模型的组合优势和个性化模型的主题保真度。

Result: 实验在CelebA-HQ、AFHQ-v2和DreamBench数据集上表明，该方法实现了强大的个性化性能，在测试时无需每主题优化，同时保持了主题保真度和提示对齐。HM-CFG进一步增强了组合泛化能力。

Conclusion: 本研究提出的方法证明了超网络在开放类别个性化方面作为一种可扩展且有效方向的巨大潜力，实现了无需微调的高质量个性化文本到图像生成。
...
### [34] [Subsampled Randomized Fourier GaLore for Adapting Foundation Models in Depth-Driven Liver Landmark Segmentation](https://arxiv.org/abs/2511.03163)
*Yun-Chen Lin,Jiayuan Huang,Hanyuan Zhang,Sergi Kavtaradze,Matthew J. Clarkson,Mobarak I. Hoque*

Main category: cs.CV

TL;DR: 本文提出了一种深度引导的肝脏地标分割框架，结合SAM2和DA2编码器，并引入SRFT-GaLore进行高效微调，实现了在腹腔镜肝脏手术中高精度、可扩展的解剖结构分割，并展示了强大的跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在腹腔镜肝脏手术等计算机辅助干预中，准确检测和描绘解剖结构至关重要。2D视频流限制了深度感知，使地标定位复杂化。现有工作在融合RGB和深度特征以及高效适应大型视觉模型到手术领域方面仍面临挑战。

Method: 本文提出了一个深度引导的肝脏地标分割框架，通过视觉基础编码器整合语义和几何线索。使用Segment Anything Model V2 (SAM2) 提取RGB特征，使用Depth Anything V2 (DA2) 提取深度感知特征。为高效适应SAM2，引入了SRFT-GaLore（一种新颖的低秩梯度投影方法，用SRFT替代SVD）以高效微调高维注意力层。通过一个交叉注意力融合模块进一步整合RGB和深度线索。此外，构建了一个新的腹腔镜肝脏手术数据集（LLSD）作为外部验证基准。

Result: 在公共L3D数据集上，该方法在Dice相似系数方面比D2GPLand提高了4.85%，在平均对称表面距离方面减少了11.78点。在LLSD数据集上，模型保持了有竞争力的性能，并显著优于基于SAM的基线，展示了强大的跨数据集鲁棒性和对未知手术环境的适应性。
...
### [35] [QG-CoC: Question-Guided Chain-of-Captions for Large Multimodal Models](https://arxiv.org/abs/2511.03206)
*Kuei-Chun Kao,Hsu Tzu-Yin,Yunqi Hong,Ruochen Wang,Cho-Jui Hsieh*

Main category: cs.CV

TL;DR: 多模态大语言模型（MLLMs）在多图像场景中存在细粒度感知和有效推理的挑战。本文首先调查了现有提示方法的不足，然后提出了一种零样本提示方法QG-CoC，该方法能够有效处理任意数量图像的问题，并在多图像和单图像基准测试中展现出竞争性表现和鲁棒性改进。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在处理多图像上下文时面临两个关键问题：缺乏跨不同图像的细粒度感知能力，以及在多个视觉输入中进行有效推理和信息整合的能力下降。现有提示方法主要关注单图像或受限的多图像场景，未能解决更通用和复杂的多图像推理任务，这构成了一个关键的研究空白。

Method: 首先，本文对当前提示方法在处理多图像时感知细粒度视觉细节和处理视觉信息的能力进行了广泛调查。在此基础上，提出了一种新的零样本提示方法——问题引导的标题链（Question-Guided Chain-of-Captions, QG-CoC），这是一种能够有效处理任意数量图像的通用提示方法。最后，在各种开源和闭源MLLMs上，通过多图像和单图像基准测试对所提出的方法进行了评估。

Result: 研究发现，现有提示方法在多图像上下文中未能关注到所需线索，也无法无缝整合感知和推理。实验结果表明，QG-CoC在各项任务中表现出具有竞争力的性能，并在现有提示方法失效的挑战性场景中展现出显著且稳健的改进。

Conclusion: QG-CoC是一种有效的、通用的零样本提示方法，能够解决多模态大语言模型在多图像上下文中的细粒度感知和推理不足问题。它在各种任务中均表现出色，并在复杂场景中展现出强大的鲁棒性，克服了现有方法的局限性。
...
### [36] [SurgAnt-ViVQA: Learning to Anticipate Surgical Events through GRU-Driven Temporal Cross-Attention](https://arxiv.org/abs/2511.03178)
...
TL;DR: 该研究引入了首个用于外科手术事件预测的VQA数据集PitVQA-Anticipation，并提出了一个名为SurgAnt-ViVQA的视频语言模型，该模型通过门控时间交叉注意力模块使大型语言模型适应外科领域，实现了从回顾性描述到主动预测的外科VQA。


<details>
  <summary>Details</summary>
Motivation: 在经鼻蝶垂体手术中，实时辅助至关重要，但能见度有限且工作流程变化迅速。现有的大多数VQA系统仅基于孤立帧进行推理，缺乏对预测下一步骤或器械需求的支持。现有的外科VQA数据集也侧重于当前场景而非近未来。
...
2. **模型：** 提出了SurgAnt-ViVQA，一个视频语言模型。该模型使用GRU门控时间交叉注意力模块来调整大型语言模型，其中双向GRU编码帧间动态，自适应门控在token级别将视觉上下文注入语言流。通过参数高效微调来定制语言骨干网络以适应外科领域。

Result: SurgAnt-ViVQA在PitVQA-Anticipation和EndoVis数据集上进行了测试，超越了强大的基于图像和视频的基线模型。消融研究表明，时间递归和门控融合是性能提升的主要驱动因素。一项帧预算研究指出权衡：8帧能最大化流畅性，而32帧略微降低BLEU但能改善数字时间估计。

Conclusion: 通过将时间感知编码器与细粒度门控交叉注意力相结合，SurgAnt-ViVQA将外科VQA从回顾性描述推进到主动预测。PitVQA-Anticipation为这一设置提供了全面的基准，并强调了有针对性的时间建模对于可靠、面向未来的外科辅助的重要性。
...
### [37] [PETWB-REP: A Multi-Cancer Whole-Body FDG PET/CT and Radiology Report Dataset for Medical Imaging Research](https://arxiv.org/abs/2511.03194)
*Le Xue,Gang Feng,Wenbo Zhang,Yichi Zhang,Lanlan Li,Shuqi Wang,Liling Peng,Sisi Peng,Xin Gao*

Main category: cs.CV

TL;DR: PETWB-REP是一个新发布的医学影像数据集，包含490名患者的全身18F-FDG PET/CT扫描及其放射学报告和临床元数据，涵盖多种常见癌症，旨在支持AI和多模态学习研究。


<details>
  <summary>Details</summary>
Motivation: 开发和验证AI模型以及进行回顾性临床研究急需结合功能和解剖成像与详细临床报告的大规模、多癌种医学影像数据集，但此类数据集目前非常稀缺。

Method: 研究人员策划并构建了PETWB-REP数据集。

Result: PETWB-REP数据集包含490名不同恶性肿瘤（主要包括肺癌、肝癌、乳腺癌、前列腺癌和卵巢癌等常见癌症）患者的全身18F-FDG PET/CT扫描图像及其对应的去识别化文本报告和结构化临床元数据。

Conclusion: 该数据集旨在支持医学影像、影像组学、人工智能和多模态学习等领域的研究。
...
### [38] [Generative deep learning for foundational video translation in ultrasound](https://arxiv.org/abs/2511.03255)
*Nikolina Tomic Roshni Bhatnagar,Sarthak Jain,Connor Lau,Tien-Yu Liu,Laura Gambini,Rima Arnaout*

Main category: cs.CV

TL;DR: 本文提出了一种生成式方法，用于超声彩色血流多普勒（CFD）到灰度视频的转换，以解决医疗图像数据不平衡问题。生成的合成视频在深度学习任务和临床专家评估中与真实视频表现无异，展示了该方法在医学成像领域的数据增强潜力。


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学图像获取和解释方面具有巨大潜力，但面临数据不平衡和缺失的挑战。超声数据尤其复杂，包含灰度图和彩色血流多普勒等子模态，这些子模态在临床研究中常不平衡。图像转换可以帮助平衡数据集，但迄今为止，超声子模态的图像转换仍然具有挑战性。

Method: 研究开发了一种生成式方法，用于超声CFD到灰度视频的转换。该方法在54,975个视频上进行训练，并利用像素级、对抗性和感知损失。它采用两个网络：一个用于重建解剖结构，另一个用于去噪，以生成逼真的超声图像。

Result: 合成视频与真实视频之间的平均成对SSIM为0.91+/-0.04。在深度学习分类和分割任务中，合成视频与真实视频表现无异（分类F1分数真实为0.9，合成为0.89；真实与合成分割的Dice分数为0.97）。盲法临床专家区分真实与合成视频的总体准确率为54+/-6%，表明合成视频的真实性。尽管仅在心脏视频上训练，该模型在多个临床领域的超声图像上表现良好（平均SSIM 0.91+/-0.05），展现了基础能力。

Conclusion: 这些数据扩展了回顾性收集图像的实用性，并丰富了医学成像的数据集设计工具箱。该方法为解决超声子模态数据不平衡问题提供了有效的解决方案，并展现了其在不同临床领域的应用潜力。
...
### [39] [MvBody: Multi-View-Based Hybrid Transformer Using Optical 3D Body Scan for Explainable Cesarean Section Prediction](https://arxiv.org/abs/2511.03212)
...
TL;DR: 本研究探索了利用3D身体形状和自我报告医疗数据进行剖宫产风险评估的可行性，并提出了一种名为MvBody的多视图Transformer网络，在资源有限环境下表现出卓越的预测性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有剖宫产风险预测模型主要适用于院内分娩期间，且依赖于在资源有限或家庭环境中通常不可用的参数。然而，在医疗资源受限的地区，早期可靠的风险预测对于改善母婴结局至关重要。

Method: 研究提出了一种新颖的多视图Transformer网络MvBody，仅使用自我报告的医疗数据和在妊娠31至38周期间获取的3D光学身体扫描来预测剖宫产风险。为了提高数据稀缺环境下的训练效率和模型泛化能力，网络中加入了度量学习损失。同时，采用集成梯度算法（Integrated Gradients）来解释模型的决策过程。

Result: 与广泛使用的机器学习模型和最新的3D分析方法相比，本方法表现出卓越的性能，在独立测试集上实现了84.62%的准确率和0.724的AUC-ROC。模型解释结果表明，孕前体重、产妇年龄、产科史、既往剖宫产史以及身体形状（特别是头部和肩部周围）是剖宫产风险预测的关键因素。

Conclusion: 3D身体形状结合自我报告数据在剖宫产风险评估中是可行的，特别是通过MvBody网络，它在资源有限的环境中提供了优越的预测性能和理论上可解释的决策过程，有助于改善产前护理决策。
...
### [40] [Decoupling Augmentation Bias in Prompt Learning for Vision-Language Models](https://arxiv.org/abs/2511.03367)
*Gahyeon Kim,Sohee Kim,Seokju Lee*

Main category: cs.CV

TL;DR: 本文提出AAPL方法，通过引入图像级属性特定增强和对抗性token嵌入来解耦视觉变异与语义表示，显著提升了提示学习在零样本和泛化任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法（如CoOp、CoCoOp）在泛化到完全未见类别时表现不佳，且主要关注文本修改，忽略了图像增强的潜力。此外，现有方法缺乏明确指导，使学习到的提示难以关注语义上有意义的视觉特征。

Method: 本文探索了图像级增强（特别是引入属性特定变化的增强）如何支持和提升提示学习的泛化能力。为解决现有方法的局限性，本文提出了AAPL（Adding Attributes to Prompt Learning），一种新颖的方法。AAPL引入对抗性token嵌入，以解耦由增强引入的表面视觉变异与类别相关的语义表示，使学习到的提示能够专注于与目标类别对齐的视觉判别特征。

Result: AAPL在十一个基准数据集上进行了全面的实验，并在少样本、零样本、跨数据集和域泛化设置中持续优于现有方法。

Conclusion: 图像级属性特定增强能够有效支持和提升提示学习的泛化能力。AAPL通过引入对抗性token嵌入，成功引导提示学习关注语义上有意义的视觉特征，显著提升了模型在多种泛化任务中的性能。
...
### [41] [Transformer-Progressive Mamba Network for Lightweight Image Super-Resolution](https://arxiv.org/abs/2511.03232)
*Sichen Guo,Wenjie Li,Yuanyang Liu,Guangwei Gao,Jian Yang,Chia-Wen Lin*

Main category: cs.CV

TL;DR: T-PMambaSR是一个轻量级超分辨率（SR）框架，结合了窗口自注意力机制和渐进式Mamba，实现了细粒度的多尺度建模和高频细节恢复，解决了现有Mamba方法的不足，并以线性复杂度取得了优于Transformer和Mamba基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Mamba的超分辨率方法虽然具有线性复杂度，但缺乏跨不同建模尺度的细粒度过渡，限制了特征表示效率。而基于Transformer的方法则面临二次方的计算成本。

Method: 本文提出了T-PMambaSR，一个轻量级SR框架。它将基于窗口的自注意力机制与渐进式Mamba相结合，以实现不同尺度感受野之间的交互，从而建立细粒度的建模范式，并以线性复杂度逐步增强特征表示。此外，引入了一个自适应高频细化模块（AHFRM）来恢复在Transformer和Mamba处理过程中丢失的高频细节。

Result: 实验证明，T-PMambaSR逐步增强了模型的感受野和表达能力，与最近的基于Transformer或Mamba的方法相比，取得了更好的性能，同时计算成本更低。

Conclusion: T-PMambaSR通过整合窗口自注意力与渐进式Mamba实现细粒度多尺度建模，并引入高频细节恢复模块，有效解决了现有Mamba-based SR方法的局限性，实现了高性能和高效率的超分辨率重建。
...
### [42] [Diffusion-Guided Mask-Consistent Paired Mixing for Endoscopic Image Segmentation](https://arxiv.org/abs/2511.03219)
*Pengyu Jie,Wanquan Liu,Rui He,Yihui Wen,Deyu Meng,Chenqiang Gao*

Main category: cs.CV

TL;DR: 本文提出了一种新的配对、扩散引导的密集预测数据增强范式，通过混合真实和合成图像外观，同时保持掩膜一致性并自适应地重新锚定到真实数据，以提高分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有密集预测数据增强方法存在局限：样本混合可能因掩膜未对齐导致软标签模糊；扩散合成虽增加多样性，但训练时忽略掩膜条件结构优势，并引入合成-真实域偏移。

Method: 提出“配对、扩散引导范式”。为每张真实图像生成一个在相同掩膜下的合成对应图像，并用这对图像作为可控输入进行“掩膜一致配对混合（MCPMix）”，该方法仅混合图像外观，监督始终使用原始硬掩膜。这产生了一系列在共享几何结构下平滑连接合成和真实外观的中间样本。为使学习与真实数据对齐，引入“真实锚定可学习退火（RLA）”，在训练过程中自适应调整混合强度和混合样本的损失权重，逐步将优化重新锚定到真实数据，并减轻分布偏差。

Result: 在Kvasir-SEG、PICCOLO、CVC-ClinicDB、私人NPC-LES队列和ISIC 2017等多个数据集上，该方法实现了最先进的分割性能，并持续优于基线。

Conclusion: 结合保持标签的混合、扩散驱动的多样性以及自适应重新锚定，能够实现鲁棒且泛化能力强的内窥镜分割。
...
### [43] [Decoupled Multi-Predictor Optimization for Inference-Efficient Model Tuning](https://arxiv.org/abs/2511.03245)
*Liwei Luo,Shuaitengyuan Li,Dongwei Ren,Qilong Wang,Pengfei Zhu,Qinghua Hu*

Main category: cs.CV

TL;DR: 本文提出了一种解耦多预测器优化（DMPO）方法，通过架构设计和模型优化，有效解耦了早期阶段的代表性和判别性能力，从而在减少计算成本的同时提高了早期退出模型的推理效率。


<details>
  <summary>Details</summary>
Motivation: 早退出与多阶段预测器结合参数高效微调是提高模型推理效率的有效途径。然而，一个关键挑战是早期阶段难以同时为深层阶段提供低级基础特征，并为早期预测器提供高级判别特征。

Method: DMPO方法在架构和优化两方面进行了解耦：
1.  **架构设计**：引入轻量级旁路模块，实现浅层特征的功能分解；开发基于高阶统计的预测器，增强早期阶段的判别能力。
2.  **解耦优化**：提出两阶段损失权重分配策略：
    *   **初始训练阶段**：通过强调早期阶段的代表性能力，优先使深层阶段获得判别能力。
    *   **后期训练阶段**：尽可能将判别能力驱动至更早的阶段。

Result: 在各种数据集和预训练骨干网络上的实验表明，DMPO在降低计算成本方面明显优于现有方法。

Conclusion: DMPO通过架构设计和模型优化，有效地解耦了早期阶段的代表性和判别性能力，解决了早期退出模型中的关键挑战，提高了推理效率。
...
### [44] [Enhancing Medical Image Segmentation via Heat Conduction Equation](https://arxiv.org/abs/2511.03260)
*Rong Wu,Yim-Sang Yu*

Main category: cs.CV

TL;DR: 本文提出了一种结合U-Mamba和热传导方程的混合架构，用于医学图像分割，以有效解决全局上下文建模和长距离依赖推理的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型（尤其是U-Net变体）在实际计算预算下，难以同时实现高效的全局上下文建模和长距离依赖推理。

Method: 提出了一种新颖的混合架构，将基于Mamba的状态空间模块用于高效的长距离推理，并在瓶颈层中引入热传导算子（HCOs），模拟频域热扩散以增强语义抽象。

Result: 在多模态腹部CT和MRI数据集上的实验结果表明，所提出的模型持续优于强大的基线模型，验证了其有效性和泛化性。

Conclusion: 将状态空间动态与基于热的全局扩散相结合，为医学分割任务提供了一个可扩展且可解释的解决方案。
...
### [45] [IEC3D-AD: A 3D Dataset of Industrial Equipment Components for Unsupervised Point Cloud Anomaly Detection](https://arxiv.org/abs/2511.03267)
*Bingyang Guo,Hongjie Li,Ruiyun Yu,Hanzhe Liang,Jinbao Wang*

Main category: cs.CV

TL;DR: 该研究提出了一个针对真实工业场景的3D点云异常检测数据集IEC3D-AD，并引入了一种基于生成式几何形态分析的新型3D异常检测范式GMANet，以解决现有数据集和方法在工业设备组件异常检测中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的3D异常检测数据集（如Real3D-AD和MVTec 3D-AD）未能捕捉真实工业环境中复杂且细微的缺陷，这阻碍了对轴承、环件和螺栓等工业设备组件(IEC)进行精确异常检测的研究。

Method: 1. 开发了IEC3D-AD数据集：直接从实际生产线收集，具有更高的点云分辨率和缺陷标注粒度。 2. 提出了GMANet范式：受生成式2D-AD方法的启发，通过几何形态分析生成合成点云样本，并通过空间差异优化减少正常和异常点级特征之间的裕度并增加重叠。

Result: 广泛的实验证明了所提出的方法在IEC3D-AD和其他数据集上的有效性。

Conclusion: 该研究通过提供一个高保真度的工业特定3D异常检测数据集IEC3D-AD和一个创新的生成式3D异常检测范式GMANet，显著提升了工业制造中核心设备组件的异常检测能力。
...
### [46] [Unified Long Video Inpainting and Outpainting via Overlapping High-Order Co-Denoising](https://arxiv.org/abs/2511.03272)
*Shuangquan Lyu,Steven Mao,Yue Ma*

Main category: cs.CV

TL;DR: 本文提出了一种新颖统一的方法，通过扩展文本到视频扩散模型，结合LoRA微调和时序协同去噪策略，实现了任意长度、高可控性的长视频修复和外绘制，有效解决了长视频生成和编辑中的一致性与质量挑战。


<details>
  <summary>Details</summary>
Motivation: 生成长视频是一个基本挑战，尤其是在视频修复和外绘制中实现高可控性非常困难。现有方法往往受限于固定长度的视频片段或存在拼接伪影。

Method: 该方法扩展了文本到视频扩散模型（如阿里巴巴的Wan 2.1），利用LoRA高效微调预训练模型进行掩码区域视频合成，并采用重叠融合的时序协同去噪策略与高阶求解器，以保持长序列之间的一致性。

Result: 该系统能够生成和编辑任意长度的视频，没有明显的接缝或漂移。在数百帧的修复/外绘制任务中，其性能优于基线方法（如Wan 2.1和VACE），在质量（PSNR/SSIM）和感知真实感（LPIPS）方面表现卓越。该方法以最小的开销实现了实用的长距离视频编辑，并在参数效率和卓越性能之间取得了平衡。

Conclusion: 本文提出了一种新颖且统一的方法，能够以最小的开销实现实用的长距离视频编辑，具有高保真度和可控性，有效解决了长视频生成和编辑中的核心挑战。
...
### [47] [Diffusion-SDPO: Safeguarded Direct Preference Optimization for Diffusion Models](https://arxiv.org/abs/2511.03317)
*Minghao Fu,Guo-Hua Wang,Tianyu Cui,Qing-Guo Chen,Zhao Xu,Weihua Luo,Kaifu Zhang*

Main category: cs.CV

TL;DR: 本文提出Diffusion-SDPO，解决了传统Diffusion-DPO中偏好裕度增加可能导致生成质量下降的问题，通过自适应缩放劣势梯度来保护优势结果，从而提升文生图模型与人类偏好的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 文生图扩散模型生成图像质量高，但与人类偏好对齐仍具挑战。现有Diffusion-DPO存在一个关键缺陷：增大偏好裕度不一定能提升生成质量，反而可能增加优胜和劣势分支的重建误差，导致劣势输出严重退化，甚至影响到优势分支。

Method: 引入Diffusion-SDPO，一种受保护的更新规则。它通过根据劣势梯度与优势梯度的对齐程度，自适应地缩放劣势梯度来保护优势结果。通过一阶分析得到一个闭式缩放系数，确保在每个优化步骤中，偏好输出的误差不会增加。该方法简单、模型无关、兼容性强且计算开销极小。

Result: 在标准文生图基准测试中，Diffusion-SDPO在自动化偏好、美学和提示对齐指标上，均持续优于现有的偏好学习基线方法。

Conclusion: Diffusion-SDPO成功解决了Diffusion-DPO中存在的关键病理问题，通过确保偏好输出的质量不受损害，显著提高了文生图模型与人类偏好的对齐效果，并展现出稳定的性能提升。
...
### [48] [Multi-Object Tracking Retrieval with LLaVA-Video: A Training-Free Solution to MOT25-StAG Challenge](https://arxiv.org/abs/2511.03332)
*Yi Yang,Yiming Xu,Timo Kaiser,Hao Cheng,Bodo Rosenhahn,Michael Ying Yang*

Main category: cs.CV

TL;DR: 本文提出了一种解决MOT25-Spatiotemporal Action Grounding (StAG)挑战赛的方法，结合了最先进的追踪模型和多模态大语言模型，并获得了第二名。


<details>
  <summary>Details</summary>
Motivation: 挑战赛旨在利用复杂真实场景的视频数据，根据特定的自由形式语言查询，准确地定位和追踪多个匹配对象。

Method: 将底层任务建模为视频检索问题，并提出了一种两阶段、零样本的方法。该方法结合了最先进的追踪模型FastTracker和多模态大语言模型LLaVA-Video的优势。

Result: 在MOT25-StAG测试集上，该方法取得了20.68的m-HIoU和10.73的HOTA分数，在挑战赛中获得第二名。

Conclusion: 所提出的两阶段、零样本方法，通过结合SOTA追踪模型和多模态大语言模型，有效解决了MOT25-StAG挑战，并取得了有竞争力的结果。
...
### [49] [SurgViVQA: Temporally-Grounded Video Question Answering for Surgical Scene Understanding](https://arxiv.org/abs/2511.03325)
...
TL;DR: 本文提出SurgViVQA模型和REAL-Colon-VQA数据集，旨在解决手术视频问答中对动态场景理解的不足，通过捕捉时间线索，显著优于现有基于图像的模型。


<details>
  <summary>Details</summary>
Motivation: 当前手术视频问答（VideoQA）方法局限于静态图像特征，现有数据集缺乏时间注释，忽视了手术过程中关键的动态信息，导致模型无法准确解释程序性事件。

Method: 提出了SurgViVQA模型，将视觉推理从静态图像扩展到动态手术场景。它使用一个Masked Video-Text Encoder来融合视频和问题特征，捕捉运动和工具-组织交互等时间线索，然后由一个微调的大型语言模型（LLM）解码生成连贯的答案。为评估模型性能，研究者还创建了REAL-Colon-VQA结肠镜视频数据集，包含运动相关问题和诊断属性，以及评估模型鲁棒性的模板外问题。

Result: SurgViVQA在REAL-Colon-VQA和公开的EndoVis18-VQA数据集上表现优于现有基于图像的VQA基准模型，尤其在关键词准确率方面，在REAL-Colon-VQA上比PitVQA提高了11%，在EndoVis18-VQA上提高了9%。对问题的扰动研究进一步证实了模型在问题措辞变化下的泛化性和鲁棒性有所提高。

Conclusion: SurgViVQA模型和REAL-Colon-VQA数据集为手术视频问答提供了一个时间感知理解的框架，使AI模型能够更有效地解释动态程序性上下文。
...
### [50] [UniAVGen: Unified Audio and Video Generation with Asymmetric Cross-Modal Interactions](https://arxiv.org/abs/2511.03334)
*Guozhen Zhang,Zixiang Zhou,Teng Hu,Ziqiao Peng,Youliang Zhang,Yi Chen,Yuan Zhou,Qinglin Lu,Limin Wang*

Main category: cs.CV

TL;DR: UniAVGen是一个统一的音视频联合生成框架，通过双分支Diffusion Transformer架构、非对称跨模态交互和面部感知调制，显著提升了唇形同步和语义一致性，并能以更少数据处理多种音视频任务。


<details>
  <summary>Details</summary>
Motivation: 现有开源音视频生成方法缺乏有效的跨模态建模，导致唇形同步不佳和语义一致性不足。

Method: 本文提出了UniAVGen，一个用于音视频联合生成的统一框架。它基于双分支联合合成架构，包含两个并行的Diffusion Transformers (DiTs) 来构建一致的跨模态潜在空间。核心机制是非对称跨模态交互，实现双向、时间对齐的交叉注意力。此外，引入了面部感知调制模块来动态优先处理显著区域。在推理阶段，还引入了模态感知无分类器指导，以显式增强跨模态相关性信号。

Result: UniAVGen的联合合成设计能够无缝统一关键的音视频任务，如音视频联合生成与续写、视频转音频配音和音频驱动视频合成。尽管训练样本少得多（1.3M 对比 30.1M），UniAVGen在音视频同步、音色一致性和情感一致性方面均表现出整体优势。

Conclusion: UniAVGen通过其创新的统一框架和跨模态交互机制，有效解决了现有音视频生成方法在唇形同步和语义一致性方面的不足，并在多种任务上展现出卓越的性能和数据效率。
...
### [51] [Robust Alignment of the Human Embryo in 3D Ultrasound using PCA and an Ensemble of Heuristic, Atlas-based and Learning-based Classifiers Evaluated on the Rotterdam Periconceptional Cohort](https://arxiv.org/abs/2511.03416)
...
TL;DR: 本文提出了一种自动化的三维超声胚胎标准化对齐方法，通过主成分分析（PCA）提取主轴并结合多种策略选择标准方向，实现了第一孕期胚胎的高精度对齐。


<details>
  <summary>Details</summary>
Motivation: 在三维超声图像中标准化胚胎对齐有助于产前生长监测，因为它能促进标准平面的检测、改善地标可视化并突出不同扫描之间的差异。

Method: 该方法首先对胚胎的分割掩膜应用主成分分析（PCA），提取胚胎的主轴，从而得到四个候选方向。然后，通过三种策略之一选择标准方向的候选：基于皮尔逊相关系数评估形状的启发式方法、通过归一化互相关将图像与图谱匹配，以及使用随机森林分类器。该方法在来自1043次怀孕的2166张纵向采集的三维超声图像上进行了测试。

Result: 在99.0%的图像中，PCA正确提取了胚胎的主轴。皮尔逊启发式、基于图谱和随机森林方法分别在97.4%、95.8%和98.4%的图像中正确选择了候选方向。这些选择方法进行多数投票后，准确率达到98.5%。

Conclusion: 该流水线的高准确性使得在第一孕期能够实现一致的胚胎对齐，从而支持在临床和研究环境中进行可扩展的分析。
...
### [52] [Generalizing Shape-from-Template to Topological Changes](https://arxiv.org/abs/2511.03459)
*Kevin Manogue,Tomasz M Schang,Dilara Kuş,Jonas Müller,Stefan Zachow,Agniva Sengupta*

Main category: cs.CV

TL;DR: 本文提出了一种拓扑变化感知的Shape-from-Template (SfT) 扩展方法，用于从2D图像和3D模板对应关系重建可变形物体表面，解决了现有SfT方法无法处理拓扑变化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的Shape-from-Template (SfT) 方法在变形伴随拓扑变化时会失效，无法进行表面重建。

Method: 该方法在经典SfT解决方案的基础上进行初始化，并通过划分模板的空间域来迭代调整模板。它通过最小化一个结合了物理合理性和重投影一致性的能量函数来实现这一目标。

Result: 该方法能够鲁棒地捕捉各种实际相关的拓扑事件，包括有界2D表面上的撕裂和切割，从而建立了第一个通用的拓扑变化感知SfT框架。在合成数据和真实数据上的实验都证实，该方法始终优于基线方法。

Conclusion: 该研究建立了首个通用的拓扑变化感知SfT框架，能够有效处理可变形物体在重建过程中出现的拓扑变化，并表现出优越的性能。
...
### [53] [Human Mesh Modeling for Anny Body](https://arxiv.org/abs/2511.03589)
...
TL;DR: Anny是一个简单、完全可微分、无需扫描的人体模型，基于人体测量学知识，提供了一个连续、可解释且具有人口统计学基础的形状空间。它支持精确的扫描拟合、受控合成数据生成和人体网格恢复（HMR），并且其生成的Anny-One数据集训练的HMR模型性能可与基于扫描的模型媲美。


<details>
  <summary>Details</summary>
Motivation: 现有的参数化人体模型依赖于昂贵的3D扫描，并且其学习的形状空间通常是专有的、人口统计学上狭窄的。

Method: Anny是一个简单、完全可微分、无需扫描的人体模型，其基础是来自MakeHuman社区的人体测量学知识。它通过表型参数（如性别、年龄、身高、体重）控制混合形状，定义了一个连续、可解释的形状空间，涵盖了从婴儿到老人的广泛人体形态。该模型使用WHO人口统计数据进行校准，以提供真实且具有人口统计学基础的人体形状变异。

Result: Anny在一个统一模型中提供了真实且具有人口统计学基础的人体形状变异，涵盖了不同年龄、体型和比例。它能支持毫米级精度的扫描拟合、受控的合成数据生成和人体网格恢复（HMR）。通过Anny生成的80万个逼真人体的Anny-One数据集显示，尽管Anny模型简单，但用其训练的HMR模型性能可以与基于扫描的人体模型相媲美，同时保持可解释性和广泛的代表性。

Conclusion: Anny凭借其开放性、语义控制和竞争性表现，为以人为中心的3D建模提供了一个可访问且多功能的基础。Anny人体模型及其代码已根据Apache 2.0许可证发布。
...
### [54] [Signal Intensity-weighted coordinate channels improve learning stability and generalisation in 1D and 2D CNNs in localisation tasks on biomedical signals](https://arxiv.org/abs/2511.03645)
*Vittal L. Rao*

Main category: cs.CV

TL;DR: 本文提出了一种信号强度加权坐标表示方法，用于生物医学数据中的定位任务，通过将局部信号强度与坐标通道结合，实现了比传统坐标通道方法更快的收敛速度和更高的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 生物医学数据中的定位任务需要模型从复杂强度分布的信号中学习有意义的空间或时间关系。现有方法如CoordConv通过添加纯坐标通道来学习绝对位置，但作者认为可以更直接地将强度-位置耦合嵌入输入表示中，以引入一种简单且与模态无关的归纳偏置。

Method: 提出了一种信号强度加权坐标表示方法，用局部信号强度缩放的通道替换了纯坐标通道。这种修改将强度-位置耦合直接嵌入到输入表示中。该方法在两个不同的定位问题上进行了评估：(i) 预测20秒双导联心电图信号的形态学转变时间，以及 (ii) 回归SiPaKMeD数据集中细胞学图像中细胞核中心的坐标。

Result: 在心电图信号和细胞学图像的定位任务中，所提出的表示方法相对于传统的坐标通道方法，均实现了更快的收敛速度和更高的泛化性能，证明了其在一维和二维生物医学信号中的有效性。

Conclusion: 信号强度加权坐标表示方法通过直接嵌入强度-位置耦合，为生物医学定位任务提供了一种有效且通用的归纳偏置，显著提高了模型在收敛速度和泛化能力方面的表现。
...
### [55] [A Lightweight 3D-CNN for Event-Based Human Action Recognition with Privacy-Preserving Potential](https://arxiv.org/abs/2511.03665)
*Mehdi Sefidgar Dilmaghani,Francis Fowley,Peter Corcoran*

Main category: cs.CV

TL;DR: 本文提出了一种轻量级三维卷积神经网络（3DCNN），用于基于事件视觉数据的人体活动识别（HAR），在保持隐私的同时实现了高准确性和边缘部署的紧凑设计。


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的摄像头在人体监控系统中存在隐私泄露问题，因为它们捕获可识别的个人信息。事件相机仅记录像素强度变化，提供了一种固有的隐私保护传感模式，因此研究者希望利用事件相机数据解决HAR中的隐私挑战。

Method: 研究采用了一种轻量级三维卷积神经网络（3DCNN），能够有效建模空间和时间动态。为了解决类别不平衡和增强泛化能力，采用了带有类别加权的Focal Loss和目标数据增强策略。模型在一个结合了Toyota Smart Home和ETRI数据集的复合数据集上进行训练和评估。

Result: 实验结果显示，该模型在人体活动识别任务上达到了0.9415的F1分数和94.17%的整体准确率。与C3D、ResNet3D和MC3_18等基准3D-CNN架构相比，性能提升高达3%。

Conclusion: 研究结果突出了基于事件的深度学习在开发准确、高效且隐私感知的人体动作识别系统方面的潜力，这些系统适用于实际的边缘应用。
...
### [56] [Part-Aware Bottom-Up Group Reasoning for Fine-Grained Social Interaction Detection](https://arxiv.org/abs/2511.03666)
*Dongkeun Kim,Minsu Cho,Suha Kwak*

Main category: cs.CV

TL;DR: 本文提出了一种基于部件感知的自下而上群体推理框架，用于细粒度社交互动检测。该方法利用身体部件特征和人际关系来推断社交群体及其互动，表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有社交互动检测方法忽略了面部表情、凝视、手势等细微线索，主要依赖个体的整体表征，并且直接检测社交群体而非明确建模个体间的互动。这限制了它们捕捉局部社交信号的能力，并在从细微线索推断群体配置时引入模糊性。

Method: 本文提出了一种“部件感知自下而上群体推理框架”。该模型首先检测个体并利用部件感知线索增强其特征，然后通过基于相似性的推理（考虑空间关系和指示互动的细微社交线索）关联个体，从而推断群体配置。

Result: 在NVI数据集上的实验表明，本文方法优于现有方法，达到了新的技术水平。

Conclusion: 通过利用身体部件特征和人际关系，本文提出的方法能够有效捕捉细粒度社交互动并推断群体配置，从而提高了检测准确性。
...
### [57] [Disentangled Concepts Speak Louder Than Words:Explainable Video Action Recognition](https://arxiv.org/abs/2511.03725)
*Jongseo Lee,Wooil Lee,Gyeong-Moon Park,Seong Tae Kim,Jinwoo Choi*

Main category: cs.CV

TL;DR: DANCE是一种针对视频动作识别的框架，通过解耦运动动态、物体和场景等概念来提供更清晰、可解释的预测，解决了现有方法解释纠缠不清或难以解释运动的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频动作识别模型的解释方法（如显著性图）将运动和空间背景纠缠在一起，导致无法明确预测是基于运动还是空间背景。基于语言的方法虽然有结构，但常因其隐性本质而无法有效解释运动。

Method: 本文提出了DANCE（Disentangled Action aNd Context concept-based Explainable）框架，通过解耦的概念类型（运动动态、物体和场景）来预测动作。运动动态概念被定义为人体姿态序列，而物体和场景概念则通过大型语言模型自动提取。DANCE基于事前概念瓶颈设计，强制模型通过这些解耦的概念进行预测。

Result: 在KTH、Penn Action、HAA500和UCF-101四个数据集上的实验表明，DANCE在保持竞争性性能的同时，显著提高了解释的清晰度。用户研究也验证了DANCE卓越的可解释性。此外，实验结果还显示DANCE有助于模型调试、编辑和故障分析。

Conclusion: DANCE框架通过提供解耦的、基于概念的解释，显著提升了视频动作识别模型的可解释性，解决了现有方法在区分运动和空间背景方面的挑战，并为模型理解和改进提供了实用价值。
...
### [58] [Cache Mechanism for Agent RAG Systems](https://arxiv.org/abs/2511.02919)
*Shuhang Lin,Zhencan Peng,Lingyao Li,Xiao Lin,Xi Zhu,Yongfeng Zhang*

Main category: cs.CL

TL;DR: 本文提出ARC（Agent RAG Cache Mechanism），一个无需标注的缓存框架，能为每个LLM代理动态管理小而高价值的语料库，显著提升RAG驱动LLM代理的效率和效果。


<details>
  <summary>Details</summary>
Motivation: 尽管RAG成功提升了LLM代理的性能，但代理级别的缓存管理（即为每个代理动态构建、维护和更新紧凑、相关的语料库）仍未得到充分探索。

Method: ARC通过结合历史查询分布模式与嵌入空间中缓存项的内在几何结构，动态管理高相关性缓存。该框架无需标注，能自动维护小而高价值的语料库。

Result: 实验结果表明，ARC将存储需求降至原始语料库的0.015%，同时提供高达79.8%的答案命中率，并将平均检索延迟降低80%。
...
### [59] [Targeted Error Correction in Knowledge Distillation: Small Language Models Surpass GPT](https://arxiv.org/abs/2511.03005)
*Hee-Jin Lee,Zhen Guo,Luchao Jin,Morteza Moazami Goudarzi*

Main category: cs.CL

TL;DR: ARF流水线使小型开源语言模型在客户服务摘要任务中超越大型专有模型，提高了成本效率和数据隐私。


<details>
  <summary>Details</summary>
Motivation: 使小型开源LLM在性能上超越大型专有模型，同时提高成本效率和数据隐私，并保持竞争力。

Method: 引入Analyze-Revise-Finetune (ARF) 流水线：首先分析并分类教师模型（GPT-3.5）生成的摘要中的常见错误；然后使用紧凑的编辑模型（Llama 3.1 70B）进行有针对性的修订，生成高质量的精炼训练数据；最后，使用这些精炼数据微调一个更小的学生模型（Llama 3.1 8B）。

Result: 通过ARF流水线微调的小型学生模型（Llama 3.1 8B）在摘要性能上超越了GPT-3.5。

Conclusion: ARF流水线提供了一个可推广的框架，用于增强开源LLM在各种下游应用中的能力，同时提高成本效率、数据隐私并保持竞争性准确性。
...
### [60] [Automatic Machine Translation Detection Using a Surrogate Multilingual Translation Model](https://arxiv.org/abs/2511.02958)
*Cristian García-Romero,Miquel Esplà-Gomis,Felipe Sánchez-Martínez*

Main category: cs.CL

TL;DR: 本文提出一种利用多语言机器翻译模型内部表示的新方法，以区分并行语料库中的人工翻译和机器翻译，从而提高翻译系统质量，尤其是在非英语语言对上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现代机器翻译系统依赖大型并行语料库，但其中很大一部分是机器生成的翻译。过度依赖这些合成内容会显著降低翻译质量，因此，过滤非人工翻译成为构建高质量机器翻译系统的关键预处理步骤。

Method: 本文提出一种新颖的方法，直接利用替代多语言机器翻译模型的内部表示来区分人工翻译和机器翻译的句子。

Result: 实验结果表明，该方法优于当前最先进的技术，尤其是在非英语语言对上，准确率至少提高了5个百分点。

Conclusion: 所提出的方法能有效识别并过滤机器生成的翻译，从而显著提升机器翻译系统的性能，特别是在处理多语言场景时效果显著。
...
### [61] [LEGO-Eval: Towards Fine-Grained Evaluation on Synthesizing 3D Embodied Environments with Tool Augmentation](https://arxiv.org/abs/2511.03001)
*Gyeom Hwangbo,Hyungjoo Chae,Minseok Kang,Hyeonjong Ju,Soohyun Oh,Jinyoung Yeo*

Main category: cs.CL

TL;DR: 当前LLM生成的3D场景缺乏真实感且评估方法不可靠。本文引入LEGO-Eval评估框架和LEGO-Bench详细指令基准，以实现更准确的场景与指令对齐评估，并揭示现有生成方法的显著局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在3D场景生成方面取得进展，但生成的场景往往缺乏真实的布局和物体属性，原因是指令不够细致。这导致训练具身智能体时学习到与现实世界不符的先验知识，影响其性能。现有评估方法（如CLIPScore、VLM）因对3D场景理解不足，无法可靠评估细粒度指令与生成场景的对齐情况。

Method: 本文提出了LEGO-Eval，一个配备多样化工具的评估框架，旨在明确地对场景组件进行接地（grounding），从而实现更准确的对齐评估。同时，还提出了LEGO-Bench，一个包含指定复杂布局和物体属性的详细指令基准。

Result: 实验表明，LEGO-Eval在评估场景-指令对齐方面比VLM-as-a-judge高出0.41 F1分数。使用LEGO-Bench进行的基准测试揭示了当前生成方法的显著局限性：所有评估方法的成功率最高仅为10%，未能完全对齐细粒度指令。

Conclusion: LEGO-Eval和LEGO-Bench为3D场景生成提供了一个更准确的评估框架和更具挑战性的基准。研究结果强调了当前生成方法在生成与细粒度指令完全对齐的真实场景方面的重大不足，亟需进一步改进。
...
### [62] [Data-Efficient Adaptation and a Novel Evaluation Method for Aspect-based Sentiment Analysis](https://arxiv.org/abs/2511.03034)
*Yan Cathy Hua,Paul Denny,Jörg Wicker,Katerina Taškova*

Main category: cs.CL

TL;DR: 本研究针对ABSA在教育和医疗等低资源领域的应用空白，提出了一种灵活的评估方法FTS-OBP，并首次探索了小型生成式语言模型（SLMs）在教育评论ABSA中的应用，通过多任务微调策略使其在极少数据下表现优异，同时发布了首个教育领域ABSA数据集。


<details>
  <summary>Details</summary>
Motivation: ABSA研究和资源主要集中在商业领域，忽视了教育和医疗等高需求但低资源领域的需求。现有方法资源密集且难以进行领域适应。此外，传统的精确匹配评估方法对生成模型过于严格，无法准确反映其性能。

Method: 1) 提出了一种新的评估方法：灵活文本相似度匹配和最优二分图配对（FTS-OBP），以适应边界变异。2) 首次对参数小于7B的小型解码器专用生成式语言模型（SLMs）进行ABSA研究，探索了数据自由（上下文学习、权重合并）和数据轻量微调方法。3) 提出了一种多任务微调策略来增强SLM性能。

Result: 1) FTS-OBP评估方法能够适应实际提取边界变异，并与传统指标保持强相关性，提供细粒度诊断。2) 通过多任务微调，1.5-3.8B参数的SLMs在仅有200-1000个示例的情况下，超越了专有大型模型，并接近基准测试结果。3) 发布了首个用于教育评论ABSA的公共资源集。

Conclusion: 本研究通过提出创新的评估方法、探索小型生成式语言模型在低资源领域的应用潜力，并发布新的教育领域ABSA资源，有效弥补了ABSA研究的现有空白，为未来在教育和医疗等领域推广ABSA提供了高效且资源友好的解决方案。
...
### [63] [ROBoto2: An Interactive System and Dataset for LLM-assisted Clinical Trial Risk of Bias Assessment](https://arxiv.org/abs/2511.03048)
*Anthony Hevia,Sanjana Chintalapati,Veronica Ka Wai Lai,Thanh Tam Nguyen,Wai-Tat Wong,Terry Klassen,Lucy Lu Wang*

Main category: cs.CL

TL;DR: ROBOTO2是一个开源的、基于网络的平台，旨在通过结合PDF解析、检索增强的LLM提示和人机协作审查，简化临床试验的偏倚风险（ROB v2）评估过程。该研究还发布了一个新的儿科临床试验数据集，并对LLM在此任务上的性能进行了基准测试和分析。


<details>
  <summary>Details</summary>
Motivation: 传统的ROB v2 (ROB2) 标注过程劳动密集且耗时，需要一种更高效、更简化的方法来辅助系统评价中的偏倚风险评估。

Method: 该研究提出了ROBOTO2平台，该平台是一个交互式界面，结合了PDF解析、检索增强的大语言模型（LLM）提示和人机协作（human-in-the-loop）审查。用户可以上传临床试验报告，接收LLM生成的初步答案和支持证据，并提供实时反馈或修正。此外，研究构建并发布了一个包含521份儿科临床试验报告的数据集，并使用该数据集对4个LLM在ROB2评估上的性能进行了基准测试。

Result: ROBOTO2平台已公开可用，其代码和数据也已发布，以促进复现和采用。研究构建并发布了一个包含521份儿科临床试验报告的数据集（涵盖8954个信号问题和1202个证据段落），该数据集通过人工和LLM辅助方法进行标注，可作为基准。利用该数据集，研究对4个LLM在ROB2评估上的性能进行了基准测试，并对当前模型的能力和自动化系统评价关键环节所面临的挑战进行了分析。

Conclusion: ROBOTO2平台提供了一个LLM辅助的偏倚风险评估工具，有效简化了ROB2标注流程。通过发布平台、代码和数据集，该研究为未来的研究提供了重要的基准和资源，并深入分析了LLM在自动化系统评价这一关键方面所面临的当前能力和挑战。
...
### [64] [Reading Between the Lines: The One-Sided Conversation Problem](https://arxiv.org/abs/2511.03056)
*Victoria Ebert,Rishabh Singh,Tuochao Chen,Noah A. Smith,Shyamnath Gollakota*

Main category: cs.CL

TL;DR: 本文提出并形式化了“单边对话问题”（1SC），即从对话的一侧进行推理和学习。研究了重建缺失发言者话语和从单边记录生成摘要两个任务，发现特定条件和方法能提升重建质量，且无需重建也能生成高质量摘要，为隐私保护的对话AI迈出一步。


<details>
  <summary>Details</summary>
Motivation: 在许多真实世界场景（如远程医疗、呼叫中心、智能眼镜）中，由于隐私或技术限制，只能记录对话的一侧，这限制了对话式AI的应用。研究旨在解决这一“单边对话问题”。

Method: 将问题形式化为1SC，并研究了两个任务：1) 重建缺失发言者的话语，用于实时应用；2) 从单边记录生成摘要。通过在MultiWOZ、DailyDialog和Candor数据集上评估提示工程（prompting）和微调模型，并采用人类A/B测试和LLM作为评判指标进行评估。

Result: 研究发现，访问未来一个轮次的信息和话语长度信息可以改善重建效果；占位符提示有助于减少幻觉；大型模型通过提示可以生成有前景的重建，而小型模型则需要微调。此外，无需重建缺失的话语也能生成高质量的摘要。

Conclusion: 1SC是一个新颖的挑战，本文报告了有前景的结果，标志着朝着隐私保护的对话式AI迈出了重要一步。高质量的单边摘要生成是可行的，重建任务在特定条件下也能取得良好效果。
...
### [65] [PolyNorm: Few-Shot LLM-Based Text Normalization for Text-to-Speech](https://arxiv.org/abs/2511.03080)
*Michel Wong,Ali Alshehri,Sophia Kao,Haotian He*

Main category: cs.CL

TL;DR: 本文提出PolyNorm，一种基于提示的大语言模型（LLM）文本归一化（TN）方法，旨在减少对人工规则的依赖。它还引入了一个语言无关的数据自动整理和评估流程，并在八种语言上展示了词错误率（WER）的显著降低，并发布了多语言数据集PolyNorm-Benchmark。


<details>
  <summary>Details</summary>
Motivation: 传统的文本归一化系统虽然准确，但工程量大、难以扩展，并且在语言覆盖方面存在挑战，尤其是在低资源环境下。

Method: 本文提出PolyNorm，一种利用大型语言模型（LLM）的基于提示的文本归一化方法。此外，还提出了一个语言无关的自动化数据整理和评估流程，以支持跨多种语言的可扩展实验。

Result: 在八种语言上的实验表明，与生产级系统相比，PolyNorm持续降低了词错误率（WER）。为了支持进一步研究，本文发布了PolyNorm-Benchmark，一个涵盖多种文本归一化现象的多语言数据集。

Conclusion: PolyNorm方法减少了对人工规则的依赖，并通过最少的人工干预实现了更广泛的语言适用性。所发布的基准数据集将有助于推动文本归一化领域的未来研究。
...
### [66] [A Computational Approach to Analyzing Disrupted Language in Schizophrenia: Integrating Surprisal and Coherence Measures](https://arxiv.org/abs/2511.03089)
*Gowtham Premananth,Carol Espy-Wilson*

Main category: cs.CL

TL;DR: 本研究利用计算语言学指标（惊奇度和语义连贯性）量化精神分裂症患者的语言障碍，以区分患者与健康对照，并评估症状严重程度。


<details>
  <summary>Details</summary>
Motivation: 精神分裂症症状常表现为语言障碍，如言语紊乱和语篇连贯性受损。这些自发语言产生异常反映了潜在的认知障碍，并有潜力作为精神分裂症症状严重程度和诊断的客观生物标志物。

Method: 本研究通过计算模型计算语言的惊奇度和语义连贯性这两个计算语言学指标，以表征语言障碍。研究将比较精神分裂症患者与健康对照组在这些指标上的差异，并探讨这些语言指标如何随精神分裂症症状严重程度的变化而改变。

Result: 本研究旨在揭示精神分裂症患者与健康对照组在惊奇度和语义连贯性方面的语言差异，并深入了解这些语言障碍如何随着精神分裂症症状严重程度的变化而演变。

Conclusion: 通过量化语言障碍，本研究有望为精神分裂症的诊断和症状严重程度评估提供新的客观语言标记，并加深对精神分裂症认知机制的理解。
...
### [67] [CARMA: Comprehensive Automatically-annotated Reddit Mental Health Dataset for Arabic](https://arxiv.org/abs/2511.03102)
*Saad Mankarious,Ayah Zirikly*

Main category: cs.CL

TL;DR: 本文介绍了CARMA，首个大规模自动标注的阿拉伯语Reddit帖子心理健康数据集，涵盖六种心理健康状况和对照组，旨在解决阿拉伯语心理健康检测中数据稀缺的挑战。


<details>
  <summary>Details</summary>
Motivation: 全球数百万人受精神疾病困扰，但早期检测仍是一大挑战，尤其对于阿拉伯语人群，资源有限且文化耻辱感阻碍了心理健康讨论。相较于英语，阿拉伯语心理健康检测研究严重不足，主要原因是缺乏标注数据集。

Method: 研究构建了CARMA数据集，这是一个大规模自动标注的阿拉伯语Reddit帖子数据集，包含焦虑、自闭症、抑郁症等六种心理健康状况和一个对照组。对用户间的词汇和语义差异进行了定性和定量分析，并使用从浅层分类器到大型语言模型等多种模型进行了分类实验，以展示数据集在心理健康分析方面的潜力。

Result: CARMA在规模和多样性上均超越现有资源。分析揭示了特定心理健康状况的语言标记。分类实验结果表明，该数据集在推进阿拉伯语等代表性不足语言的心理健康检测方面具有巨大潜力。

Conclusion: CARMA数据集的创建为阿拉伯语心理健康检测研究提供了宝贵资源，并展示了在缺乏代表性的语言中推进心理健康检测的广阔前景。
...
### [68] [Control Barrier Function for Aligning Large Language Models](https://arxiv.org/abs/2511.03121)
*Yuya Miyaoka,Masaki Inoue*

Main category: cs.CL

TL;DR: 本文提出一个基于控制的框架，利用控制障碍函数（CBF）来对大型语言模型（LLM）进行对齐，以确保生成用户期望的文本，且无需微调基线LLM。


<details>
  <summary>Details</summary>
Motivation: 动机是使大型语言模型能够生成用户期望的文本（例如积极文本），同时避免对现有LLM进行耗时的微调。

Method: 该研究提出一个基于控制的框架，将控制障碍函数（CBF）作为安全过滤器应用于基线LLM预测的token，从而干预生成的文本。该安全过滤器是附加式的，允许在不微调基线LLM的情况下用于对齐目的，并且可以直接将评估模型用于过滤器设计。整个系统使用开源语言模型实现，目标是生成积极文本。

Result: 该框架能够确保生成用户期望的文本（例如积极文本）。其安全过滤器具有两个显著优势：一是作为附加组件，无需微调基线LLM即可用于对齐；二是如果存在关于期望对齐的评估模型，可以直接应用于过滤器设计。

Conclusion: 该论文成功提出了一种基于控制障碍函数（CBF）的框架，通过安全过滤器对大型语言模型进行对齐，有效确保生成用户期望的文本。该方法无需微调基线模型，并能直接利用评估模型进行过滤器设计，在生成积极文本方面展示了其可行性。
...
### [69] [MME-CC: A Challenging Multi-Modal Evaluation Benchmark of Cognitive Capacity](https://arxiv.org/abs/2511.03146)
...
TL;DR: MME-CC是一个新的以视觉为中心的基准，用于系统评估多模态大语言模型（MLLMs）在空间、几何和知识推理方面的认知能力，并揭示了现有模型的弱点和错误模式。


<details>
  <summary>Details</summary>
Motivation: 随着推理模型规模的扩大，多模态在人类认知中的重要性日益凸显，促使人们需要探究以视觉为中心的认知行为。然而，现有多模态基准要么过分强调文本推理，要么未能系统捕捉以视觉为中心的认知行为，导致MLLMs的认知能力评估不足。

Method: 引入了MME-CC基准，该基准将11个代表性推理任务组织成空间、几何和知识型推理三大视觉信息类别。基于MME-CC，对16个代表性MLLMs进行了广泛实验，并进行了细粒度分析，识别了常见的错误模式，并观察了思维链（CoT）过程。

Result: 研究发现，闭源模型目前总体领先（例如Gemini-2.5-Pro vs. GLM-4.5V），而空间和几何推理能力普遍较弱（得分≤30%）。常见的错误模式包括方向错误、跨视图身份持久性脆弱以及对反事实指令依从性差。思维链通常遵循“提取→推理→验证”的三阶段过程，并严重依赖视觉提取。

Conclusion: 本工作旨在促使将MLLMs的认知能力视为评估和模型设计的核心，以推动该领域的发展。
...
### [70] [Who Sees the Risk? Stakeholder Conflicts and Explanatory Policies in LLM-based Risk Assessment](https://arxiv.org/abs/2511.03152)
*Srishti Yadav,Jasmina Gajcin,Erik Miehling,Elizabeth Daly*

Main category: cs.CL

TL;DR: 本文提出一个基于大型语言模型（LLM）的利益相关者风险评估框架，通过生成可解释的、特定于利益相关者的政策，并可视化冲突，以提高AI系统风险评估的透明度和对齐性。


<details>
  <summary>Details</summary>
Motivation: 为了负责任地部署AI系统，理解不同利益相关者如何感知AI系统中的风险至关重要。

Method: 研究采用LLM作为“判断者”来预测和解释风险，并结合Risk Atlas Nexus和GloVE解释方法，生成特定于利益相关者的可解释政策。该方法在医疗AI、自动驾驶和欺诈检测三个真实AI用例中进行了演示。此外，还提出了一个交互式可视化工具来揭示冲突的产生原因和方式。

Result: 研究结果表明，利益相关者的视角显著影响风险感知和冲突模式。

Conclusion: 工作强调了以利益相关者为中心的解释的重要性，以使基于LLM的评估更加透明、可解释，并符合以人为中心的AI治理目标。
...
### [71] [Measuring Aleatoric and Epistemic Uncertainty in LLMs: Empirical Evaluation on ID and OOD QA Tasks](https://arxiv.org/abs/2511.03166)
*Kevin Wang,Subre Abdoul Moktar,Jia Li,Kangshuo Li,Feng Chen*

Main category: cs.CL

TL;DR: 本研究对大型语言模型（LLMs）中的不确定性估计（UE）方法进行了全面实证研究，评估了12种UE方法在分布内（ID）和分布外（OOD）QA任务中的表现，发现不同类型的方法在不同情境下各有优劣。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的日益普及，确保其输出的可信度至关重要，而不确定性估计（UE）在此过程中扮演着关键角色。因此，需要深入理解并评估不同UE方法的鲁棒性和有效性。

Method: 研究进行了一项综合性的实证分析，考察了12种不同的UE方法，涵盖了对偶然不确定性（aleatoric uncertainty）和认知不确定性（epistemic uncertainty）的测量。评估使用了四种生成质量指标（包括来自LLM评论器的LLMScore），在问答（QA）任务中，利用分布内（ID）和分布外（OOD）数据集，评估LLM生成答案的不确定性。

Result: 信息熵类方法（利用token和序列概率）在ID设置中表现出色，因其与模型对数据的理解一致。密度类方法和P(True)指标在OOD上下文中表现优越，有效捕捉了模型的认知不确定性。语义一致性方法（评估生成答案的变异性）在不同数据集和生成指标下均表现可靠，虽非在所有情况下都最佳，但性能稳定。

Conclusion: 不同类型的不确定性估计方法在大型语言模型中各有优势，信息熵类方法适用于分布内数据，密度类方法和P(True)适用于分布外数据，而语义一致性方法则在不同场景下表现出普遍的可靠性。选择合适的UE方法对于确保LLM输出的可信度至关重要。
...
### [72] [BengaliMoralBench: A Benchmark for Auditing Moral Reasoning in Large Language Models within Bengali Language and Culture](https://arxiv.org/abs/2511.03180)
*Shahriyar Zaman Ridoy,Azmine Toushik Wasi,Koushik Ahamed Tonmoy*

Main category: cs.CL

TL;DR: 该研究引入了BengaliMoralBench，首个针对孟加拉语和其社会文化背景的大规模伦理基准，评估了多语言大型语言模型（LLMs）的伦理对齐，发现其在文化理解、常识推理和道德公平性方面存在显著弱点，为负责任的本地化提供了基础。


<details>
  <summary>Details</summary>
Motivation: 随着多语言LLMs在南亚地区普及，其与当地伦理规范的对齐（特别是对于拥有2.85亿使用者的孟加拉语）仍未被充分探索。现有伦理基准主要以英语为中心并受西方框架影响，忽视了现实部署中关键的文化细微差别。

Method: 研究引入了BengaliMoralBench，一个涵盖五个道德领域（日常活动、习惯、育儿、家庭关系、宗教活动）和50个文化相关子主题的大规模孟加拉语伦理基准。每个场景均由母语使用者通过美德、常识和正义三种伦理视角进行共识标注。使用统一的提示协议和标准指标，对Llama、Gemma、Qwen和DeepSeek等主流多语言LLMs进行了系统性的零样本评估。

Result: 多语言LLMs的性能差异很大（准确率在50-91%之间）。定性分析揭示了模型在文化基础、常识推理和道德公平性方面存在持续的弱点。

Conclusion: BengaliMoralBench为负责任的本地化奠定了基础，支持文化对齐的评估，并有助于在孟加拉国等多样化、低资源的多语言环境中部署符合伦理的AI系统。
...
### [73] [LGM: Enhancing Large Language Models with Conceptual Meta-Relations and Iterative Retrieval](https://arxiv.org/abs/2511.03214)
*Wenchang Lei,Ping Zou,Yue Wang,Feng Sun,Lei Zhao*

Main category: cs.CL

TL;DR: 本文提出语言图模型（LGM），通过从自然语言中提取并验证元关系（继承、别名、组成），并结合概念迭代检索算法，动态地向大型语言模型（LLM）提供这些信息，以增强其概念理解和响应准确性，且能处理任意长度文本，优于现有RAG基线。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）尽管语义理解能力强，但在用户指令涉及模糊或概念错位的术语时表现不佳。

Method: 本文提出了语言图模型（LGM），它通过以下步骤增强概念清晰度：1) 从自然语言中提取元关系（继承、别名、组成）；2) 利用反射机制验证这些元关系；3) 采用概念迭代检索算法，动态地向LLM提供这些关系及相关描述。与传统RAG不同，LGM不依赖扩展上下文窗口，能够处理任意长度文本而无需截断。

Result: 在标准基准测试中，LGM始终优于现有的RAG基线模型。

Conclusion: LGM通过提取和验证元关系并采用迭代检索机制，有效提升了LLM对模糊或概念错位术语的理解和响应准确性，且克服了传统RAG的上下文窗口限制，能够处理任意长度文本。
...
### [74] [Hybrid Fact-Checking that Integrates Knowledge Graphs, Large Language Models, and Search-Based Retrieval Agents Improves Interpretable Claim Verification](https://arxiv.org/abs/2511.03217)
*Shaghayegh Kolli,Richard Rosenbaum,Timo Cavelius,Lasse Strothe,Andrii Lata,Jana Diesner*

Main category: cs.CL

TL;DR: 本文提出了一种混合事实核查方法，结合大型语言模型（LLM）、知识图谱（KG）和实时网络搜索，以解决LLM缺乏事实依据和KG覆盖范围有限的问题，实现了高准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成流畅文本方面表现出色，但可能缺乏可靠的事实依据；基于知识图谱的事实核查器能提供精确可解释的证据，但受限于覆盖范围或延迟。研究动机是整合两者的优势，克服各自的弱点。

Method: 该系统包含三个自主步骤：1) 知识图谱检索，用于在DBpedia中进行快速单跳查找；2) 基于语言模型的分类，由任务特定的标注提示引导，并结合内部规则逻辑；3) 仅当知识图谱覆盖不足时才调用的网络搜索代理。

Result: 该流水线在FEVER基准测试的“支持/反驳”分类上实现了0.93的F1分数，且无需进行任务特定的微调。针对“信息不足”（NEI）的情况，目标重标注研究表明，该方法经常能为最初标记为NEI的主张发现有效证据，并得到了专家标注者和LLM评审员的证实。

Conclusion: 本文提出了一个模块化、开源的事实核查流水线，具有回退策略，并能在不同数据集之间泛化，有效整合了LLM、知识图谱和网络搜索的优势。
...
### [75] [Beyond Ranked Lists: The SARAL Framework for Cross-Lingual Document Set Retrieval](https://arxiv.org/abs/2511.03228)
*Shantanu Agarwal,Joel Barry,Elizabeth Boschee,Scott Miller*

Main category: cs.CL

TL;DR: ISI的SARAL项目在IARPA的MATERIAL计划中，通过一种新颖的方法，在跨语言信息检索（CLIR）方面取得了显著进展，特别是在检索相关文档集方面表现出色，超越了大多数竞争团队。


<details>
  <summary>Details</summary>
Motivation: 本研究的动机是IARPA的MATERIAL（Machine Translation for English Retrieval of Information in Any Language）计划，旨在推动跨语言信息检索（CLIR）的最新进展。ISI的SARAL团队为此贡献了他们的努力。

Method: 该团队提出了一种新颖的CLIR方法，重点是开发一种能够检索到与查询相关的“文档集”，而不仅仅是生成一个排序的文档列表。

Result: 在MATERIAL的第三阶段评估中，SARAL团队在涵盖三种不同语言（波斯语、哈萨克语和格鲁吉亚语）的六种评估条件中的五种中，表现优于其他团队。

Conclusion: ISI的SARAL项目为MATERIAL计划开发的CLIR方法，特别是其检索相关文档集的能力，被证明是高度有效和成功的，在多语言环境下超越了竞争对手的表现。
...
### [76] [IndicSuperTokenizer: An Optimized Tokenizer for Indic Multilingual LLMs](https://arxiv.org/abs/2511.03237)
*Souvik Rana,Arul Menezes,Ashish Kulkarni,Chandra Khatri,Shubham Agarwal*

Main category: cs.CL

TL;DR: 本文提出IndicSuperTokenizer，一种结合子词和多词分词以及语言特定预分词的印地语多语言LLM分词器，显著提高了分词效率（fertility score）和推理吞吐量，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 分词器对大型语言模型（LLMs）的性能、训练效率和推理成本至关重要。为多语言LLMs设计有效的分词器极具挑战性，尤其对于具有多样脚本和丰富形态变化的印地语系语言。子词方法（如BPE）虽然广泛应用，但在多语言环境中的有效性仍未被充分探索。

Method: 研究者提出了IndicSuperTokenizer，它结合了子词和多词分词技术，并加入了语言特定的预分词策略。该分词器在英语、22种印度语言和代码数据上进行了评估，并进行了分词器训练数据大小、词汇量大小、合并技术和预分词策略的详细消融实验。

Result: IndicSuperTokenizer在分词效率（fertility score）上取得了新的最先进水平，相较于LLaMA4提高了39.5%，相较于Sutra（当前最佳）提高了18%。这转化为推理吞吐量相较于LLaMA4提高了44%，同时在英语和印地语基准测试上保持了可比的性能。详细的消融实验也证明了其设计选择的鲁棒性。

Conclusion: 所提出的IndicSuperTokenizer通过结合子词和多词分词以及语言特定预分词，为印地语多语言LLMs提供了更具语言对齐性的分词器，显著提升了分词效率和推理吞吐量，证明了其设计的有效性和鲁棒性。
...
### [77] [Comparing the Performance of LLMs in RAG-based Question-Answering: A Case Study in Computer Science Literature](https://arxiv.org/abs/2511.03261)
*Ranul Dayarathne,Uvini Ranaweera,Upeksha Ganegoda*

Main category: cs.CL

TL;DR: 本研究比较了结合RAG技术的四种开源LLM（Mistral-7b、LLaMa2-7b、Falcon-7b、Orca-mini-v3-7b）和GPT-3.5在计算机科学领域问答任务上的性能，发现GPT-3.5表现出色，Mistral-7b在开源模型中最佳，同时指出开源LLM具备与专有模型竞争的潜力。


<details>
  <summary>Details</summary>
Motivation: RAG技术因其减少幻觉的能力而日益受到关注，与LLM结合使用可增强其性能。因此，比较不同LLM在结合RAG后在多领域问答任务中的表现变得具有重要意义。
...
Result: 结合RAG后，GPT-3.5在二元和长答案问题上表现出色。在开源LLM中，Mistral-7b-instruct结合RAG在两种问题类型上均超越其他开源模型。Orca-mini-v3-7b生成响应的平均延迟最短，而LLaMa2-7b-chat的平均延迟最高。

Conclusion: 研究强调，开源LLM在更好的基础设施支持下，也能够与GPT-3.5等专有模型并驾齐驱，展现出强大的竞争力。
...
### [78] [How to Evaluate Speech Translation with Source-Aware Neural MT Metrics](https://arxiv.org/abs/2511.03295)
*Mauro Cettolo,Marco Gaido,Matteo Negri,Sara Papi,Luisa Bentivogli*

Main category: cs.CL

TL;DR: 本研究首次系统性地探讨了语音到文本翻译（ST）的源感知评估指标，通过使用ASR转录或参考回译作为音频源的文本代理，并引入了一种新的跨语言重分段算法，以在源转录不可用时实现更准确的评估。


<details>
  <summary>Details</summary>
Motivation: ST系统自动评估通常只比较翻译假设与参考译文，忽略了源输入信息。在机器翻译（MT）中，源感知指标已显示出与人类判断更强的相关性。然而，将此思想扩展到ST并非易事，因为源是音频，且可靠的转录或对齐通常不可用。

Method: 本研究在源转录不可用的实际操作条件下，对ST的源感知指标进行了首次系统性研究。探索了两种生成输入音频文本代理的策略：自动语音识别（ASR）转录和参考译文的反向翻译。引入了一种新颖的两步跨语言重分段算法，以解决合成源与参考译文之间的对齐不匹配问题。

Result: 实验结果表明，当词错误率（WER）低于20%时，ASR转录比反向翻译构成更可靠的合成源；而反向翻译总是计算成本更低但仍然有效的替代方案。此外，所提出的跨语言重分段算法使得源感知MT指标能够稳健地用于ST评估。
...
### [79] [SCALE: Upscaled Continual Learning of Large Language Models](https://arxiv.org/abs/2511.03270)
...
TL;DR: 针对大型语言模型持续预训练，提出SCALE架构，通过在冻结预训练参数的同时插入轻量级宽度扩展，平衡稳定性与适应性，有效缓解遗忘问题并提升新知识获取。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的持续预训练进展需要从单纯增加参数转向优化结构。现有方法（如深度扩展）会导致严重遗忘，因此需要一种既能增加模型容量、获取新知识，又能保持原有功能并减轻遗忘的方法。

Method: 引入SCALE，一种宽度扩展架构，在冻结所有预训练参数的同时，将轻量级扩展模块插入线性模块中，并保持残差和注意力拓扑结构。该方法遵循“持久保留”（通过保留导向初始化和冻结预训练权重维持基础模型行为）和“协同适应”（选择性训练扩展组件获取新知识）两大原则。具体实现包括SCALE-Preserve（保留优先）、SCALE-Adapt（适应优先）和可选的SCALE-Route（用于在保留和适应头部之间进行token级路由）。

Result: 在合成传记基准测试中，SCALE在获取新知识的同时显著减轻了深度扩展导致的严重遗忘。在韩语语料库上的持续预训练中，SCALE变体在英语评估中表现出更少的遗忘，并在韩语基准测试中取得有竞争力的提升。这些变体提供了最佳的整体稳定性-可塑性权衡。伴随的分析阐明了保留何时成立，以及保留与适应之间的相互作用如何稳定优化。

Conclusion: SCALE通过结构化宽度扩展，平衡了保留与适应，为大型语言模型的持续预训练提供了一种新颖有效的方法，成功缓解了遗忘问题，并实现了更好的稳定性-可塑性权衡。
...
### [80] [Benchmarking the Thinking Mode of Multimodal Large Language Models in Clinical Tasks](https://arxiv.org/abs/2511.03328)
...
TL;DR: 本研究评估了具有“思考模式”的多模态大语言模型（MLLMs）在临床任务中的表现，发现其思考模式带来的性能提升微乎其微，并且在复杂医学任务上表现仍不理想。


<details>
  <summary>Details</summary>
Motivation: 随着具有“思考模式”的推理型MLLMs的出现，本研究旨在严格评估这些增强的推理过程如何影响MLLMs在临床任务中的性能和可靠性。

Method: 本研究评估了两个领先的MLLMs，Seed1.5-VL和Gemini-2.5-Flash，在医学应用中激活“思考模式”的能力。使用VQA-RAD和ROCOv2数据集，在四项视觉医学任务上评估了它们的性能，并与标准“非思考模式”进行了比较。

Result: 研究发现，在大多数任务中，激活思考模式带来的性能提升与标准非思考模式相比微乎其微。模型在开放式VQA和医学图像解释等复杂医学任务上的表现仍不尽如人意。
...
### [81] [Silenced Biases: The Dark Side LLMs Learned to Refuse](https://arxiv.org/abs/2511.03369)
*Rom Himelstein,Amit LeVi,Brit Youngmann,Yaniv Nemcovsky,Avi Mendelson*

Main category: cs.CL

TL;DR: 本文引入了“沉默偏见”的概念，指安全对齐LLM中被拒绝响应掩盖的不公平偏好。提出SBB基准，通过激活引导减少拒绝来揭示这些偏见，发现模型直接响应与潜在公平性之间存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 安全对齐的大语言模型在敏感应用中日益普及，但现有公平性评估方法（如问答式）常将模型拒绝响应误判为积极的公平性，从而掩盖了模型潜在的不公平偏好（即“沉默偏见”）。此前检测间接偏见的方法存在可扩展性和评估污染问题。

Method: 本文引入了“沉默偏见”的概念，并提出了“沉默偏见基准”（SBB）。SBB通过运用激活引导技术，在问答过程中减少模型的拒绝响应，从而揭示模型中编码的不公平偏好。SBB设计易于扩展到新的受试者和人口统计群体。

Result: 在多个大型语言模型上验证了该方法，发现模型的直接响应与其潜在的公平性问题之间存在惊人的差异，揭示了被安全对齐掩盖的深层偏见。

Conclusion: SBB提供了一个公平性评估框架，旨在超越对齐训练的掩盖效应，鼓励未来开发更公平的模型和工具，揭示模型深层次的偏见问题。
...
### [82] [EQ-Negotiator: Dynamic Emotional Personas Empower Small Language Models for Edge-Deployable Credit Negotiation](https://arxiv.org/abs/2511.03370)
*Yunbo Long,Yuhan Liu,Alexandra Brintrup*

Main category: cs.CL

TL;DR: 本文提出了EQ-Negotiator框架，通过结合博弈论和隐马尔可夫模型，使小型语言模型（SLM）在自动化信用协商中能够在线学习和追踪债务人情绪状态，从而弥补了与大型语言模型（LLM）的性能差距，实现了更好的债务回收和协商效率，同时兼顾隐私保护和伦理标准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在自动化协商中表现出色，但其高计算成本和数据隐私要求使其不适用于隐私敏感的设备端应用。小型语言模型（SLM）虽然是实用替代方案，但在处理情绪复杂的人格（尤其是在信用协商中）时，与LLM存在显著的性能差距。

Method: 本文引入了EQ-Negotiator框架，其核心是一个推理系统，该系统将博弈论与隐马尔可夫模型（HMM）相结合，以在线学习和追踪债务人的情绪状态，无需预训练。这使得SLM能够获得战略智能，以应对操纵、缓解冲突并维护伦理标准。

Result: 通过广泛的代理间模拟，包括对抗性债务人策略（如欺骗、威胁和扮演受害者），结果表明，一个7B参数的语言模型配备EQ-Negotiator，在债务回收和协商效率方面优于其规模十倍以上的基础LLM。

Conclusion: 这项工作将人格建模从描述性特征配置文件推进到动态情感架构，并在隐私约束下运行。研究表明，战略性情商而非原始模型规模，是自动化协商成功的关键因素，为有效、道德和隐私保护的边缘AI协商者铺平了道路。
...
### [83] [LFC-DA: Logical Formula-Controlled Data Augmentation for Enhanced Logical Reasoning](https://arxiv.org/abs/2511.03372)
*Shenghao Li*

Main category: cs.CL

TL;DR: LFC-DA是一种符号逻辑控制的数据增强管道，通过将逻辑文本映射到命题表达式并进行系统搜索来生成多样且逻辑严谨的自然语言问题，有效提升预训练模型的逻辑推理准确性。


<details>
  <summary>Details</summary>
Motivation: 复杂的逻辑数据增强高度依赖人工标注，成本高昂；而大型语言模型直接生成的数据则难以解释且逻辑上同质化。

Method: LFC-DA管道首先将逻辑文本映射为命题表达式，然后编译一个紧凑的规则库，接着通过有界状态空间搜索系统地发现有效的公式，最后将这些公式口语化为自然语言问题，确保了命题逻辑下的多样性和逻辑严谨性。

Result: 在ReClor和LogiQA数据集上的实验表明，LFC-DA显著提高了预训练模型的逻辑推理准确性。

Conclusion: 实验结果证实了LFC-DA在LLM引导的逻辑数据增强方面的有效性。
...
### [84] [Generative Artificial Intelligence in Bioinformatics: A Systematic Review of Models, Applications, and Methodological Advances](https://arxiv.org/abs/2511.03354)
...
TL;DR: 该综述系统评估了生成式人工智能（GenAI）在生物信息学中的变革作用，涵盖了其在基因组学、蛋白质组学、药物发现等领域的应用、方法学进展、预测性能、专业化、局限性及支持数据集。


<details>
  <summary>Details</summary>
Motivation: 生成式人工智能（GenAI）已成为生物信息学中的一种变革性方法，推动了基因组学、蛋白质组学、转录组学、结构生物学和药物发现等领域的发展。为了系统地识别和评估这些不断增长的进展，需要进行一次全面的审查。

Method: 本综述根据系统评价和荟萃分析的首选报告项目（PRISMA）方法，提出了六个研究问题（RQs）。旨在评估有影响力的GenAI策略在方法学进步、预测性能和专业化方面的表现，并识别有前景的高级建模、数据密集型发现和综合生物学分析方法。

Result: RQ1强调GenAI在序列分析、分子设计和综合数据建模等多个生物信息学子领域的多样化应用，通过模式识别和输出生成，表现优于传统方法。RQ2揭示了经过调整的专业模型架构优于通用模型，这得益于有针对性的预训练和上下文感知策略。RQ3识别了GenAI在分子分析和数据整合方面的显著优势，提高了复杂分析的准确性并减少了错误。RQ4指出结构建模、功能预测和合成数据生成方面的改进已通过既定基准验证。RQ5提出了主要限制，如可扩展性不足和数据偏见影响泛化能力，并提出了未来方向，重点关注鲁棒评估和生物学基础建模。RQ6考察了分子、细胞和文本数据集广泛支持GenAI模型的训练和泛化。

Conclusion: GenAI在生物信息学中带来了显著进步，通过专业化模型在多个应用领域超越了传统方法。尽管存在可扩展性和数据偏差等挑战，但未来方向应侧重于鲁棒评估和基于生物学原理的建模，并利用多样化的数据集支持GenAI模型的训练和泛化。
...
### [85] [CareMedEval dataset: Evaluating Critical Appraisal and Reasoning in the Biomedical Field](https://arxiv.org/abs/2511.03441)
*Doria Bonzi,Alexandre Guiggi,Frédéric Béchet,Carlos Ramisch,Benoit Favre*

Main category: cs.CL

TL;DR: 该研究引入了CareMedEval数据集，用于评估大型语言模型（LLMs）在生物医学批判性评估和推理任务中的表现，揭示了LLMs在此类任务中的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在生物医学批判性评估中展现出潜力，但其可靠性有限，尤其是在专业领域的批判性推理方面。现有基准未能明确评估基于科学论文的批判性阅读和推理能力。

Method: 研究构建了一个名为CareMedEval的原创数据集，包含534个问题，基于37篇科学文章，这些问题来源于法国医学生的真实考试。随后，研究在不同上下文条件下，对最先进的通用型和生物医学专业型LLMs进行了基准测试。

Result: 基准测试结果显示，LLMs在任务中表现困难，开放和商业模型的准确匹配率未能超过0.5。虽然生成中间推理标记能显著提高结果，但模型在关于研究局限性和统计分析的问题上仍面临挑战。

Conclusion: CareMedEval提供了一个具有挑战性的、基于事实的推理基准，暴露了当前LLMs的局限性，并为未来开发自动化批判性评估支持工具指明了方向。
...
### [86] [SOLVE-Med: Specialized Orchestration for Leading Vertical Experts across Medical Specialties](https://arxiv.org/abs/2511.03542)
...
TL;DR: SOLVE-Med是一个多智能体架构，结合了领域专业化的小型语言模型（1B参数），用于处理复杂的医学查询，在意大利医学论坛数据上表现优于大型模型（高达14B参数），并支持本地部署。


<details>
  <summary>Details</summary>
Motivation: 医疗问答系统面临幻觉、偏见、计算需求高、隐私问题以及需要跨领域专业知识等部署挑战。

Method: SOLVE-Med采用多智能体架构，包括：一个路由智能体（用于动态专家选择）、十个在特定医学领域微调的专业化小型模型（每个1B参数），以及一个协调智能体（用于综合响应）。

Result: 在意大利医学论坛的十个专业领域数据上，SOLVE-Med取得了优越的性能，ROUGE-1达到0.301，BERTScore F1达到0.697，其表现优于高达14B参数的独立模型，并支持本地部署。

Conclusion: SOLVE-Med通过结合领域专业化的小型语言模型，有效地解决了医疗问答系统的部署挑战，实现了卓越的性能和本地部署能力。
...
### [87] [Segmentation Beyond Defaults: Asymmetrical Byte Pair Encoding for Optimal Machine Translation Performance](https://arxiv.org/abs/2511.03383)
*Saumitra Yadav,Manish Shrivastava*

Main category: cs.CL

TL;DR: 本研究表明，非对称BPE（源语言和目标语言采用不同的合并操作次数）能显著提升机器翻译性能，尤其是在低资源设置下，优于传统的对称BPE。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译研究通常为词汇分割模型（对称BPE）建议单一固定的超参数集，但这种统一方法可能无法保证在不同语言对和数据量下获得最佳机器翻译性能。

Method: 研究通过在不同数据量（5万、10万、50万句对）和多种语言对（如英语-印地语，以及其他六种语言）上，评估了对称和非对称BPE分割方案对机器翻译系统性能的影响，并使用CHRF++指标进行衡量。

Result: 非对称BPE显著优于对称BPE，特别是在低资源设置下。在低资源英语-印地语设置中，非对称BPE平均获得5.32、4.46和0.7的CHRF++增益。在额外六个语言对中，12个系统中有10个观察到显著改进。源语言使用高NMO（4K-32K）而目标语言使用低NMO（0.5K-2K）可提供最佳结果，尤其有利于低资源机器翻译。

Conclusion: 非对称BPE，特别是源语言采用高合并操作次数而目标语言采用低合并操作次数的配置，比对称BPE更有效，能够显著提高机器翻译性能，尤其对低资源机器翻译有益。
...
### [88] [Overcoming the Generalization Limits of SLM Finetuning for Shape-Based Extraction of Datatype and Object Properties](https://arxiv.org/abs/2511.03407)
*Célian Ringwald,Fabien Gandon,Catherine Faron,Franck Michel,Hanna Abi Akl*

Main category: cs.CL

TL;DR: 本文研究了小型语言模型（SLM）在提取完整RDF图（包括数据类型和对象属性）时的性能，并解决了稀有属性长尾分布的瓶颈，发现通过确保每个属性出现次数达到阈值来构建训练集是最有效的策略。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在基于SHACL形状提取RDF三元组时，尤其是在处理常见数据类型属性方面，已显示出潜力。本研究旨在探讨SLM如何处理数据类型和对象属性，以实现完整的RDF图提取，并解决稀有属性长尾分布带来的关键瓶颈。

Method: 为了解决稀有属性的长尾分布问题，本文评估了几种策略：分层抽样、加权损失、数据集缩放和基于模板的合成数据增强。

Result: 研究发现，关键瓶颈在于稀有属性的长尾分布。在处理不平衡目标属性时，最佳策略是构建一个训练集，其中每个属性的出现次数都超过给定的阈值，从而实现同等良好的性能。

Conclusion: 研究结果为训练形状感知的SLM提供了实用指导，并为语义关系抽取领域的未来工作指明了有前景的方向。为确保可复现性，研究团队已公开发布了数据集、实验结果和代码。
...
### [89] [MultiZebraLogic: A Multilingual Logical Reasoning Benchmark](https://arxiv.org/abs/2511.03553)
*Sofie Helene Bruun,Dan Saattrup Smart*

Main category: cs.CL

TL;DR: 本文创建了多语言、高质量的斑马谜题数据集（MultiZebraLogic），用于评估大型语言模型（LLMs）的逻辑推理能力，并探讨了增加谜题难度的方法，发现红鲱鱼线索能显著降低模型准确率。


<details>
  <summary>Details</summary>
Motivation: 为了全面衡量大型语言模型（LLMs）的能力，特别是逻辑推理技能，需要跨多种任务、多语言且难度适中的基准测试数据集。

Method: 研究人员生成了多语言斑马谜题，涵盖不同主题、规模，包含14种线索类型和8种“红鲱鱼”类型（无信息线索），并探索了多种增加难度的方法。他们使用GPT-4o mini（非推理模型）和o3-mini（推理模型）进行了测试。

Result: 结果显示，2x3和4x5的谜题规模分别对GPT-4o mini和o3-mini构成挑战。在4x5谜题中，加入5个红鲱鱼线索使o3-mini的谜题级准确率下降了15±7%。o3-mini在4x5谜题上的得分不受英语与丹麦语或通用房屋主题与特定国家三明治主题的影响。未发现难度与所选线索类型之间存在关联。最终发布了包含128+1024个谜题的MultiZebraLogic数据集，涵盖九种日耳曼语言的2x3和4x5规模谜题，并公开了谜题生成代码。

Conclusion: 研究成功创建了用于评估LLM逻辑推理能力的多语言、高质量斑马谜题数据集（MultiZebraLogic），并发现红鲱鱼线索是有效增加谜题难度的方式。数据集和生成代码的发布将有助于未来LLM的评估和开发。
...
### [90] [AILA--First Experiments with Localist Language Models](https://arxiv.org/abs/2511.03559)
*Joachim Diederich*

Main category: cs.CL

TL;DR: 本文首次实证展示了Transformer语言模型中可控的局部性，通过一个可调的局部性参数，实现了可解释的局部性编码和高效的分布式表示之间的连续控制，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 传统的语言模型主要依赖分布式表示，缺乏对表示局部性程度的控制。本研究旨在开发一种方法，允许动态地在高度可解释的局部性编码和高效的分布式表示之间进行插值，以满足需要透明度和能力的受监管领域应用的需求。

Method: 研究引入了一个可调的局部性参数λ，将其整合到Transformer语言模型架构中。在WikiText语料库上，使用一个两层Transformer架构进行实验，系统地将λ从1.0（完全局部性）变化到0.0（完全分布式），并评估其对注意力熵、指针保真度和预测性能（困惑度、准确率）的影响。

Result: 结果表明，局部性配置（λ=1.0）显著降低了注意力熵（5.36比特，而λ=0.0时为7.18比特），同时保持了更高的指针保真度。预测实验显示，中间局部性值（如λ=0.6）能在可解释性和性能之间取得最佳平衡，实现了4.65的测试困惑度和84.7%的准确率。

Conclusion: 研究确立了局部性语言模型为受监管领域提供了一个实用的框架，这些领域既需要透明度又需要能力。通过显式惩罚阈值和信息论设计原则，该框架能够对可解释性-性能谱系进行精确的数学控制。
...
### [91] [Efficient Reasoning via Thought-Training and Thought-Free Inference](https://arxiv.org/abs/2511.03408)
*Canhui Wu,Qiong Cao,Chao Xue,Wei Xi,Xiaodong He*

Main category: cs.CL

TL;DR: 本文提出3TF框架，通过在训练阶段内化CoT推理过程，使大型语言模型在推理时能进行高质量的隐式推理，同时输出简洁的无思考结果，从而提高推理效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的CoT方法虽然提高了推理准确性，但仍依赖于推理过程的显式输出（即使经过压缩），这导致推理过程冗长且效率不高。研究者旨在实现模型在不显式生成推理步骤的情况下，也能进行高质量的推理。

Method: 3TF（Thought-Training and Thought-Free inference）框架采取“短到长”而非“长到短”的视角。首先训练一个混合模型，使其能在推理和非推理模式下运行。然后，在CoT标注数据上进一步训练模型，使其内化结构化推理过程，同时在推理时强制使用非推理模式，生成简洁、无思考的输出。这使得模型能够进行丰富的内部隐式推理，而外部输出保持简短。

Result: 实验结果表明，经过3TF训练的模型在无思考推理模式下，在推理基准测试上获得了显著的性能提升。这证明了高质量的推理可以被隐式学习和执行，而无需显式的分步生成过程。
...
### [92] [Step-Audio-EditX Technical Report](https://arxiv.org/abs/2511.03601)
...
TL;DR: Step-Audio-EditX是首个开源的基于LLM的音频模型，通过利用大裕度合成数据，实现了富有表现力和迭代性的音频编辑（包括情感、说话风格和副语言），并具备强大的零样本文本转语音能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于开发一个能够进行富有表现力、迭代性音频编辑（涵盖情感、说话风格和副语言）并支持鲁棒零样本文本转语音的音频模型。

Method: 核心创新在于仅使用大裕度合成数据进行训练，从而避免了对基于嵌入的先验知识或辅助模块的需求。这种大裕度学习方法实现了对不同声音的迭代控制和高表现力，并与传统上专注于表示层解耦的方法有所不同。

Result: 评估结果表明，Step-Audio-EditX在情感编辑和其他细粒度控制任务中超越了MiniMax-2.6-hd和Doubao-Seed-TTS-2.0。

Conclusion: Step-Audio-EditX成功展示了通过创新性的大裕度学习方法，在富有表现力和迭代性的音频编辑方面取得了卓越性能，并提供了强大的零样本文本转语音能力。
...
### [93] [Knowledge-Augmented Question Error Correction for Chinese Question Answer System with QuestionRAG](https://arxiv.org/abs/2511.03410)
*Longpeng Qiu,Ting Li,Shuai Mao,Nan Yang,Xiaohui Yan*

Main category: cs.CL

TL;DR: QuestionRAG框架通过结合外部知识增强和强化学习，解决了问答系统中大型语言模型因输入错误导致的误解和过度纠正问题。


<details>
  <summary>Details</summary>
Motivation: 问答系统中的输入错误常导致不准确的响应。大型语言模型（LLMs）在此任务上表现不佳，经常误解用户意图（误解）或不必要地改变原始问题结构（过度纠正）。

Method: 提出QuestionRAG框架。为解决误解，通过外部知识（如搜索结果、相关实体）丰富输入。为防止过度纠正，使用强化学习（RL）将模型目标与精确纠正而非仅仅释义对齐。将这两种策略整合。

Result: 知识增强对于理解有缺陷的问题至关重要。基于RL的对齐比传统的监督微调（SFT）显著更有效，提升了模型遵循指令和泛化的能力。通过整合这两种策略，QuestionRAG释放了LLMs在问题纠正任务上的全部潜力。

Conclusion: QuestionRAG通过结合知识增强和强化学习对齐，有效解决了LLMs在问题纠正任务中面临的误解和过度纠正挑战，显著提升了模型性能。
...
### [94] [ChiMDQA: Towards Comprehensive Chinese Document QA with Fine-grained Evaluation](https://arxiv.org/abs/2511.03656)
*Jing Gao,Shutiao Luo,Yumeng Liu,Yuanming Li,Hongji Zeng*

Main category: cs.CL

TL;DR: 本文提出了一个名为ChiMDQA的中文多文档问答数据集，旨在满足不同业务场景下的需求，并支持多种NLP任务。


<details>
  <summary>Details</summary>
Motivation: 随着自然语言处理技术的发展，对高质量中文文档问答数据集的需求日益增长，特别是在学术、教育、金融、法律、医疗和新闻等常见领域。

Method: ChiMDQA数据集通过严格筛选文档和系统性的问题设计方法构建而成。它包含了来自六个不同领域的长篇文档，共计6,068个经过精心策划的高质量问答对，并进一步细分为十个细粒度类别。此外，论文还概述了数据集的设计目标、构建方法和细粒度评估系统。

Result: ChiMDQA数据集保证了多样性和高质量，适用于文档理解、知识提取和智能问答系统等多种NLP任务。它为中文问答领域的未来研究和实际应用提供了坚实的基础。
...
### [95] [Kastor: Fine-tuned Small Language Models for Shape-based Active Relation Extraction](https://arxiv.org/abs/2511.03466)
*Ringwald Celian,Gandon Fabien,Faron Catherine,Michel Franck,Abi Akl Hanna*

Main category: cs.CL

TL;DR: Kastor是一个框架，通过重构SHACL形状验证为属性组合评估和采用迭代学习，显著增强了RDF模式提取在微调小型语言模型（SLMs）和完善专业领域知识库方面的泛化性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的RDF模式提取方法通过专注于特定SHACL形状来微调小型语言模型，虽然有效，但在满足专业领域知识库补全和精炼的需求方面仍有提升空间，尤其是在处理有限文本和RDF数据时。

Method: 本文引入了Kastor框架。它将传统的SHACL形状验证任务重构为评估形状派生的所有可能属性组合。通过为每个训练示例选择最优组合，该框架旨在显著增强模型泛化和性能。此外，Kastor采用迭代学习过程来精炼噪声知识库。

Result: Kastor框架显著增强了模型的泛化性和性能。它能够创建强大的模型，这些模型能够发现新的、相关的知识，并有效地满足专业领域知识库的补全和精炼需求。

Conclusion: Kastor通过其创新的属性组合评估和迭代学习方法，显著提升了RDF模式提取在微调小型语言模型和精炼专业知识库方面的效率和效果，使其能够处理噪声数据并发现新知识。
...
### [96] [Grounded Misunderstandings in Asymmetric Dialogue: A Perspectivist Annotation Scheme for MapTask](https://arxiv.org/abs/2511.03718)
*Nan Li,Albert Gatt,Massimo Poesio*

Main category: cs.CL

TL;DR: 本研究引入了一种透视主义标注方案，用于追踪HCRC MapTask语料库中对话参与者对指代表达的理解如何出现、分歧和修复，并利用LLM进行大规模标注。结果显示，虽然完全误解较少，但多重性差异系统性地导致指代分歧，揭示了表面上的共识可能掩盖指代错位。


<details>
  <summary>Details</summary>
Motivation: 在不对称的协作对话中，参与者可能认为他们达成了一致，但实际上却指代了不同的实体。研究旨在探究理解是如何随着时间发展、分歧和修复的，以及如何识别和分析这种基于视角的误解。

Method: 研究为HCRC MapTask语料库设计了一种透视主义标注方案，该方案分别捕获说话者和听话者对每个指代表达的理解。然后，利用一个受方案约束的大型语言模型（LLM）标注流水线，获得了1.3万个带可靠性估计的标注指代表达，并分析了由此产生的理解状态。

Result: 结果表明，一旦词汇变体得到统一，完全的误解是罕见的。然而，多重性差异（multiplicity discrepancies）会系统性地导致理解分歧，揭示了表面上的共识（apparent grounding）可能掩盖指代错位（referential misalignment）。

Conclusion: 本研究提出的框架既提供了一个资源，也提供了一个分析视角，用于研究基于视角的误解，并可用于评估(V)LLM在协作对话中建模依赖于视角的共识建立能力。
...
### [97] [HaluMem: Evaluating Hallucinations in Memory Systems of Agents](https://arxiv.org/abs/2511.03506)
*Ding Chen,Simin Niu,Kehang Li,Peng Liu,Xiangping Zheng,Bo Tang,Xinchi Li,Feiyu Xiong,Zhiyu Li*

Main category: cs.CL

TL;DR: 为解决AI记忆系统幻觉问题，本文引入了HaluMem，首个针对记忆系统的操作级别幻觉评估基准，并构建了用户中心的多轮交互数据集。研究发现，现有系统在记忆提取和更新阶段容易产生并累积幻觉。


<details>
  <summary>Details</summary>
Motivation: AI系统（如LLM和AI代理）的记忆系统在存储和检索过程中常出现幻觉（如捏造、错误、冲突、遗漏）。现有评估主要通过端到端问答，难以定位幻觉产生的具体操作阶段。

Method: 引入了HaluMem，首个针对记忆系统的操作级别幻觉评估基准。定义了三个评估任务：记忆提取、记忆更新和记忆问答。构建了用户中心的多轮人机交互数据集HaluMem-Medium和HaluMem-Long，包含约1.5万记忆点和3.5千多类型问题，对话长度和上下文长度均较大，以评估不同规模和复杂性下的幻觉。

Result: 基于HaluMem的实证研究表明，现有记忆系统倾向于在记忆提取和更新阶段生成并累积幻觉，这些错误随后会传播到问答阶段。

Conclusion: 未来的研究应致力于开发可解释和受约束的记忆操作机制，以系统性地抑制幻觉并提高记忆可靠性。
...
### [98] [BanglaSTEM: A Parallel Corpus for Technical Domain Bangla-English Translation](https://arxiv.org/abs/2511.03498)
*Kazi Reyazul Hasan,Mubasshira Musarrat,A. B. M. Alim Al Islam,Muhammad Abdullah Adnan*

Main category: cs.CL

TL;DR: 大型语言模型在处理孟加拉语技术问题时表现不佳，因为现有翻译系统难以处理技术术语。本文介绍了BanglaSTEM数据集（包含5,000对STEM领域的孟加拉语-英语句子）和基于T5的翻译模型，显著提高了技术内容的翻译准确性，使孟加拉语使用者能更有效地利用以英语为中心的语言模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语技术问题解决方面表现良好，但在孟加拉语中表现不佳。将孟加拉语问题翻译成英语是一种解决方案，但现有孟加拉语-英语翻译系统在处理技术术语时存在困难，常常误译专业词汇，导致问题含义改变和答案错误。

Method: 我们构建了BanglaSTEM数据集，包含5,000对精心挑选的STEM（包括计算机科学、数学、物理、化学和生物学）领域的孟加拉语-英语句子。我们使用语言模型生成了超过12,000个翻译，然后通过人工评估员选择出高质量且能正确保留技术术语的句子对。随后，我们使用BanglaSTEM数据集训练了一个基于T5的翻译模型，并在代码生成和数学问题解决这两个任务上对其进行了测试。

Result: 我们的结果显示，该模型在技术内容的翻译准确性方面取得了显著改进，使得孟加拉语使用者能够更有效地利用以英语为中心的语言模型。
...
### [99] [One Battle After Another: Probing LLMs' Limits on Multi-Turn Instruction Following with a Benchmark Evolving Framework](https://arxiv.org/abs/2511.03508)
*Qi Jia,Kaiwei Zhang,Xiujie Song,Ye Shen,Xiangyang Zhu,Guangtao Zhai*

Main category: cs.CL

TL;DR: 本研究提出了一种可扩展的框架和基准测试（EvolIF），用于评估大型语言模型（LLMs）在多轮对话中遵循指令的能力，结果显示GPT-5表现出卓越的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常局限于固定轮次，容易饱和，并且未能充分反映用户在多主题对话中与LLM交互的真实体验，无法有效评估LLMs在多轮指令遵循方面的能力。

Method: 本研究提出了一个可扩展的框架，用于评估多轮指令遵循能力。该框架通过三层机制（跟踪约束、指令和主题）将语言表层形式与用户意图模拟解耦，并通过状态变化和回溯动态构建基准，模拟用户-LLM交互，直到模型耗尽模拟用户的耐心才终止对话。研究还定义了一套捕捉交互过程质量的指标，并基于此框架构建了包含九种不同约束类型的EvolIF基准。

Result: GPT-5表现出卓越的指令遵循性能，平均能维持18.54个对话轮次，并展现出70.31%的鲁棒性。它以11.41%的显著优势超越了Gemini-2.5-Pro，而其他模型则远远落后。

Conclusion: 所提出的框架和EvolIF基准有效地评估了LLMs在多轮对话中遵循指令的能力，并明确指出GPT-5在这方面表现出显著的优越性。
...
### [100] [ASVRI-Legal: Fine-Tuning LLMs with Retrieval Augmented Generation for Enhanced Legal Regulation](https://arxiv.org/abs/2511.03563)
*One Octadion,Bondan Sapta Prakoso,Nanang Yudi Setiawan,Novanto Yudistira*

Main category: cs.CL

TL;DR: 本研究通过微调大型语言模型（LLM）并结合检索增强生成（RAG）方法，旨在提升政策制定者在法律理解、分析和法规起草方面的能力。


<details>
  <summary>Details</summary>
Motivation: 支持政策制定者更好地理解、分析和起草法律法规，解决他们在关键工作中面临的挑战。

Method: 1. 构建了一个针对法律领域的监督数据集，用于微调LLM，使其深入理解法律文本。2. 整合了检索增强生成（RAG）方法，使LLM能够访问并结合外部最新的法律知识。

Result: 该方法显著提高了法律研究和法规制定的效率，能积极协助政策制定者解释法规并起草符合当前需求的新法规。

Conclusion: 结合微调和RAG的方法为政策制定者提供了一个有价值的工具，可以有效增强法律研究和法规制定的效果，在不断发展的法律领域中具有重要意义。
...
### [101] [Bearing Syntactic Fruit with Stack-Augmented Neural Networks](https://arxiv.org/abs/2511.03547)
*Brian DuSell,Ryan Cotterell*

Main category: cs.CL

TL;DR: 本文首次提出并证明了堆栈增强神经网络能够在无需特殊条件的情况下，以类似人类的方式进行层次化泛化，特别是在问句生成任务上，带有非确定性堆栈的Transformer表现最佳。


<details>
  <summary>Details</summary>
Motivation: 人类儿童在语言学习中倾向于基于层次句法规则的假设，而标准神经网络仅在特定条件下（如句法监督、大规模预训练或长时间训练）才表现出这种偏好。研究旨在探索无需这些特殊条件就能实现类人层次泛化的神经网络架构。

Method: 研究测试了堆栈增强神经网络。基准架构包括Transformer、Simple RNN和LSTM，并结合了两种堆栈类型：Joulin & Mikolov（2015）的叠加堆栈和DuSell & Chiang（2023）提出的非确定性泛化堆栈。实验在经典的问句生成任务上进行。此外，还提出并改进了堆栈RNN架构以增强层次泛化能力。

Result: 研究发现，在问句生成任务中，带有非确定性堆栈的Transformer架构泛化能力最佳。同时，对堆栈RNN架构的修改也显著改善了其层次泛化能力。这些堆栈增强神经网络在没有句法监督、大规模语料预训练或超长训练的情况下，实现了类似人类的泛化表现。

Conclusion: 堆栈增强神经网络可能比标准架构更能准确地模拟人类语言习得过程，并可作为心理语言学研究的有用工具。
...
### [102] [A systematic review of relation extraction task since the emergence of Transformers](https://arxiv.org/abs/2511.03610)
*Ringwald Celian,Gandon,Fabien,Faron Catherine,Michel Franck,Abi Akl Hanna*

Main category: cs.CL

TL;DR: 本文对自Transformer模型出现以来（2019-2024年）的关系抽取（RE）研究进行了系统性回顾，分析了方法进展、基准资源和语义网技术集成，并指出了当前趋势、局限性和挑战。


<details>
  <summary>Details</summary>
Motivation: 旨在为研究人员和从业者提供一个全面参考，以理解关系抽取（RE）的演变和未来方向，特别是自Transformer模型问世以来的发展。

Method: 采用自动化框架收集和标注出版物，对2019年至2024年间发表的34篇综述、64个数据集和104个模型进行了系统性分析。

Result: 研究结果突出了方法论的进步、基准资源以及语义网技术的整合。通过整合多维度结果，识别了当前趋势、局限性和开放挑战。

Conclusion: 该研究为研究人员和从业者提供了一个理解关系抽取演变和未来方向的全面参考，总结了Transformer时代RE研究的进展、挑战与趋势。
...
### [103] [Towards Transparent Stance Detection: A Zero-Shot Approach Using Implicit and Explicit Interpretability](https://arxiv.org/abs/2511.03635)
*Apoorva Upadhyaya,Wolfgang Nejdl,Marco Fisichella*

Main category: cs.CL

TL;DR: 本文提出了一种名为IRIS的新型可解释零样本立场检测（ZSSD）框架，通过结合文本中的隐式理由和基于语言测量的显式理由，提供了对模型预测的内在可解释性，并解决了现有方法的泛化性和可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 现有零样本立场检测（ZSSD）方法存在泛化性差、文本与目标之间缺乏连贯性、过度依赖显式推理、解释粗糙且缺乏细微差别，以及未能明确建模推理过程等问题，导致模型预测难以解释。

Method: IRIS将立场检测视为信息检索排名任务。它通过理解文本序列中隐式理由与不同立场的关联性来指导模型预测，无需理由的真实标签，从而提供内在可解释性。此外，它利用基于交际特征的显式理由来解码立场的情感和认知维度，以提供作者态度的可解释性理解。

Result: 在VAST、EZ-STANCE、P-Stance和RFD等基准数据集上，使用50%、30%甚至10%的训练数据进行的广泛实验证明了IRIS模型具有出色的泛化能力，这得益于其提出的架构和可解释性设计。

Conclusion: IRIS框架通过结合隐式和显式理由，提供了一种新颖且可解释的零样本立场检测方法。它不仅解决了现有方法的局限性，还在有限训练数据下展现了强大的泛化能力，为理解作者对目标态度的细微之处提供了可解释的洞察。
...
### [104] [Do Androids Dream of Unseen Puppeteers? Probing for a Conspiracy Mindset in Large Language Models](https://arxiv.org/abs/2511.03699)
*Francesco Corso,Francesco Pierri,Gianmarco De Francisci Morales*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型（LLMs）是否具有阴谋论倾向、是否存在社会人口学偏见，以及其采纳阴谋论观点的易感性，发现LLMs表现出部分阴谋论信念，存在潜在的偏见，且易受操纵。


<details>
  <summary>Details</summary>
Motivation: 阴谋论信念在错误信息传播和机构不信任形成中扮演核心角色，是评估LLMs社会忠实度的关键。LLMs被越来越多地用作研究人类行为的替代品，但其是否能再现更高阶的心理结构（如阴谋论思维）尚不清楚。

Method: 研究通过在不同提示和条件策略下，对多个LLMs进行了经过验证的测量阴谋论思维的心理测量调查。

Result: 研究发现LLMs与阴谋论信念的某些元素表现出部分一致性；社会人口学属性的条件作用产生了不均衡的效果，揭示了潜在的人口学偏见；有针对性的提示可以轻易地将模型响应转向阴谋论方向，突显了LLMs易受操纵的特性及其在敏感场景部署的潜在风险。

Conclusion: 这些结果强调了批判性评估LLMs中嵌入的心理维度至关重要，这不仅能推动计算社会科学发展，也能为应对有害用途提供可能的缓解策略。
...
### [105] [Toward an Agricultural Operational Design Domain: A Framework](https://arxiv.org/abs/2511.02937)
*Mirco Felske,Jannik Redenius,Georg Happich,Julius Schöning*

Main category: cs.RO

TL;DR: 本文提出了农业操作设计域（Ag-ODD）框架，用于描述和验证自主农业系统的操作边界，以应对现有ODD概念无法解决的农业应用独特挑战。


<details>
  <summary>Details</summary>
Motivation: 农业自动化系统在复杂多变的环境中运行，并结合了驾驶和作业过程，面临独特的运行约束。现有的操作设计域（ODD）概念无法有效应对这些农业应用的独特挑战，需要一个结构化、透明且可验证的环境描述方法来确保开发和验证过程的一致性。

Method: Ag-ODD框架包含三个核心要素：1. Ag-ODD描述概念，利用ASAM Open ODD和CityGML概念，提供结构化方法明确定义环境和操作参数；2. 7层模型，在PEGASUS 6层模型基础上扩展了过程层，以捕捉动态农业操作；3. 迭代验证过程，根据7层模型导出的逻辑场景验证Ag-ODD，确保其完整性和一致性。

Result: Ag-ODD框架提供了一种创建明确且可验证的Ag-ODD的一致性方法。示范用例表明，Ag-ODD框架能够支持自主农业系统环境描述的标准化和可扩展性。

Conclusion: Ag-ODD框架通过其核心要素，为自主农业系统提供了一种描述操作边界的一致且可验证的方法，有助于推动农业自动化领域环境描述的标准化和可扩展性。
...
TL;DR: AURA是一个协作框架，结合LLM、数字孪生和人机交互，用于在机器人系统中实时检测和诊断异常行为，并通过反馈循环持续学习和改进。


<details>
  <summary>Details</summary>
Motivation: 在安全关键环境中安全部署自主系统，尤其是在出现不可预见的异常时，需要结合人类专业知识和AI分析的范式。

Method: 引入AURA框架，整合大型语言模型（LLM）、高保真数字孪生（DT）和人机交互。该架构包含两个代理：一个低级状态异常表征代理，将遥测信号转换为自然语言问题描述；一个高级诊断推理代理，与操作员进行知识驱动的对话以识别根本原因，并利用外部资源。人类验证的诊断被转换为新的训练样本，以优化低级感知模型，形成一个持续学习的反馈循环。

Result: AURA能够实时检测和响应异常行为，通过与操作员对话识别根本原因，并将人类专家的知识逐步提炼到AI中，使其从静态工具转变为自适应伙伴。

Conclusion: 该框架为建立值得信赖、持续改进的人机协作团队提供了一种具体实现和模式，有助于自主系统在安全关键环境中的安全部署。
...
### [107] [Comprehensive Assessment of LiDAR Evaluation Metrics: A Comparative Study Using Simulated and Real Data](https://arxiv.org/abs/2511.02994)
*Syed Mostaquim Ali,Taufiq Rahman,Ghazal Farhani,Mohamed H. Zaki,Benoit Anctil,Dominique Charlebois*

Main category: cs.RO

TL;DR: 本研究旨在通过比较真实和模拟LiDAR扫描来评估自动驾驶系统(ADS)的虚拟测试环境(VTE)的准确性。研究发现，密度感知倒角距离(DCD)是评估VTE保真度的最佳指标，并且在真实与模拟LiDAR扫描之间存在几何和模型感知上的差异，其中DCD与感知方法的关联性最强。


<details>
  <summary>Details</summary>
Motivation: 为了安全部署自动驾驶系统，需要进行严格测试。然而，传统的物理测试成本高昂且存在安全隐患。虚拟测试环境(VTE)提供了一种替代方案，但需要验证其能否准确反映现实世界，这可以通过比较VTE生成的传感器输出与真实世界数据来实现。

Method: 1. 探索并实验性评估适用于比较真实与模拟LiDAR扫描的多种评估指标，测试其在不同噪声、密度、畸变、传感器方向和通道设置下的灵敏度和准确性。2. 使用在受控环境中收集的真实LiDAR扫描数据生成一个虚拟测试环境。3. 利用与真实LiDAR扫描相同的姿态从VTE生成模拟LiDAR扫描。4. 从模型感知和几何相似性两方面比较模拟和真实LiDAR扫描。

Result: 1. 密度感知倒角距离(DCD)在所有测试场景中表现最佳。2. 经过强度校正后，真实与模拟LiDAR扫描的语义分割输出相似，mIoU为21%。3. 平均密度感知倒角距离(DCD)为0.63，表明模拟与真实LiDAR扫描在几何特性上存在细微差异。4. 模型输出之间存在显著差异。5. 在所有指标中，密度感知倒角距离(DCD)与感知方法的相关性最强。

Conclusion: 本研究表明，尽管模拟与真实LiDAR扫描在几何特性和模型感知输出上仍存在差异，但虚拟测试环境可以作为自动驾驶系统测试的替代方案。密度感知倒角距离(DCD)是评估VTE保真度的有效指标，尤其与感知方法具有高度相关性。
...
### [108] [WorldPlanner: Monte Carlo Tree Search and MPC with Action-Conditioned Visual World Models](https://arxiv.org/abs/2511.03077)
*R. Khorrambakht,Joaquim Ortiz-Haro,Joseph Amigo,Omar Mostafa,Daniel Dugas,Franziska Meier,Ludovic Righetti*

Main category: cs.RO

TL;DR: 本文提出一种模型基方法，利用非结构化游戏数据学习视觉世界模型和动作采样器，结合MCTS规划器和MPC控制器，在真实机器人任务中显著优于行为克隆基线。


<details>
  <summary>Details</summary>
Motivation: 现有行为克隆（BC）等端到端策略难以迁移到新任务，且训练数据生成困难（需要精心演示和频繁环境重置），限制了机器人在复杂任务中的应用。

Method: 该研究采用模型基方法：1) 收集少量非结构化、易于获取的“游戏数据”；2) 学习一个条件动作的视觉世界模型和一个基于扩散的动作采样器，并可选地学习一个奖励模型；3) 将世界模型、动作采样器和奖励模型结合蒙特卡洛树搜索（MCTS）规划器来优化长序列动作；4) 通过零阶模型预测控制器（MPC）在机器人上执行规划结果。动作采样器被用于缓解世界模型在规划过程中的幻觉。

Result: 该方法在3个具有不同规划和建模复杂度的真实世界机器人任务上得到验证。实验结果表明，与行为克隆基线相比，规划方法在标准操作测试环境中取得了显著改进。动作采样器有效缓解了世界模型在规划过程中的幻觉问题。

Conclusion: 通过利用非结构化数据学习世界模型和动作采样器，并结合MCTS规划和MPC执行，模型基方法能够显著提升机器人在真实世界任务中的表现，克服了传统行为克隆方法的局限性。
...
### [109] [3D Cal: An Open-Source Software Library for Calibrating Tactile Sensors](https://arxiv.org/abs/2511.03078)
*Rohan Kota,Kaival Shah,J. Edward Colgate,Gregory Reardon*

Main category: cs.RO

TL;DR: 本文介绍了一个名为\libname{}的开源库，它能将低成本3D打印机转变为自动化探针设备，从而生成大量带标签的训练数据，实现触觉传感器（如DIGIT和GelSight Mini）的自动化校准，以重建高质量深度图。


<details>
  <summary>Details</summary>
Motivation: 触觉传感在机器人灵巧操作中至关重要，但其校准过程通常是临时性的、劳动密集型的，需要将原始传感器读数转换为有物理意义的量。

Method: 研究者开发了开源库\libname{}，它利用3D打印机作为自动化探针设备，生成用于触觉传感器校准的大量带标签训练数据。利用收集到的数据和定制的卷积神经网络（CNN），对DIGIT和GelSight Mini两种商用视觉触觉传感器进行了校准，以重建深度图。此外，还进行了数据消融研究，以确定准确校准所需的数据量。

Result: 成功展示了\libname{}在校准DIGIT和GelSight Mini传感器方面的实用性，实现了高质量深度图的重建。研究还提供了关于准确校准所需数据量的实用指导方针。训练后的模型在未见过物体上的基准测试表现出良好的校准精度和泛化性能。

Conclusion: \libname{}通过自动化触觉传感器校准过程，能够加速触觉传感研究，简化传感器部署，并促进触觉传感在机器人平台中的实际集成应用。
...
### [110] [SENT Map -- Semantically Enhanced Topological Maps with Foundation Models](https://arxiv.org/abs/2511.03165)
*Raj Surya Rajendran Kathirvel,Zach A Chavis,Stephen J. Guy,Karthik Desingh*

Main category: cs.RO

TL;DR: 本文介绍了SENT-Map，一种语义增强的拓扑地图，用于室内环境表示，它利用基础模型（FMs）的优势，以人类和FMs都能理解的JSON文本格式支持自主导航和操作，并通过语义增强使小型FMs也能成功规划。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了利用基础模型（FMs）的最新进展，改进室内环境的自主导航和操作能力，并解决现有方法在表示语义信息以及确保机器人规划可行性方面的挑战。

Method: SENT-Map采用JSON文本格式表示环境，允许添加和编辑语义信息，同时在规划时将机器人固定到现有节点以避免不可行状态。该框架分两阶段：首先，操作员与Vision-FM一起进行环境映射；其次，使用SENT-Map表示和自然语言查询通过FM进行规划。

Result: 实验结果表明，语义增强使得即使是小型、可在本地部署的基础模型（FMs）也能成功地在室内环境中进行规划。

Conclusion: SENT-Map通过语义增强的拓扑地图表示，有效提升了基础模型在室内环境中的规划能力，即使是资源受限的小型FMs也能实现成功的自主导航和操作。
...
### [111] [Learning Natural and Robust Hexapod Locomotion over Complex Terrains via Motion Priors based on Deep Reinforcement Learning](https://arxiv.org/abs/2511.03167)
*Xin Liu,Jinze Wu,Yinghui Li,Chenkun Qi,Yufei Xue,Feng Gao*

Main category: cs.RO

TL;DR: 本文提出了一种基于运动先验的方法，将深度强化学习成功应用于真实的六足机器人，使其在复杂地形中无需视觉信息也能学习并展示出自然且鲁棒的步态。
...
Result: 所学习的策略在真实六足机器人上展示了自然的步态模式和卓越的鲁棒性，即使在复杂地形中没有视觉信息也能有效行走。这是强化学习控制器首次用于实现真实六足机器人复杂地形行走。

Conclusion: 该研究成功地将基于运动先验的深度强化学习方法应用于真实六足机器人，解决了多足协调难题，使其能够在复杂地形中展现出自然且鲁棒的运动能力。
...
### [112] [OneOcc: Semantic Occupancy Prediction for Legged Robots with a Single Panoramic Camera](https://arxiv.org/abs/2511.03571)
*Hao Shi,Ze Wang,Shangwei Guo,Mengfei Duan,Song Wang,Teng Chen,Kailun Yang,Lin Wang,Kaiwei Wang*

Main category: cs.RO

TL;DR: OneOcc是一个为足式/人形机器人设计的纯视觉全景语义场景补全(SSC)框架，解决了步态引起的抖动和360度连续性问题，并通过多项创新技术实现了最先进的性能，并发布了两个新的全景占用基准。


<details>
  <summary>Details</summary>
Motivation: 现有的语义场景补全(SSC)系统主要针对配备前向传感器的轮式平台，但足式/人形机器人需要鲁棒的3D语义占用感知，并且面临步态引起的身体抖动和360度连续性感知挑战。

Method: OneOcc框架结合了：(i) 双投影融合(DP-ER)，利用环形全景图及其等距矩形展开，保持360度连续性和网格对齐；(ii) 双网格体素化(BGV)，在笛卡尔和柱坐标空间中推理，减少离散化偏差并锐化边界；(iii) 带有分层AMoE-3D的轻量级解码器，用于动态多尺度融合和更好的长距离/遮挡推理；(iv) 即插即用的步态位移补偿(GDC)模块，无需额外传感器即可学习特征级运动校正。此外，论文还发布了两个全景占用基准：QuadOcc（真实四足机器人）和Human360Occ (H3O)（CARLA人形机器人）。

Result: OneOcc在QuadOcc基准上超越了强大的视觉基线和流行的激光雷达基线，在H3O上实现了+3.83 mIoU（城内）和+8.08 mIoU（跨城）的显著提升，达到了新的最先进(SOTA)水平。所有模块都轻量化，使得该系统能够部署在足式/人形机器人上，实现全方位感知。
...
### [113] [Learning-based Cooperative Robotic Paper Wrapping: A Unified Control Policy with Residual Force Control](https://arxiv.org/abs/2511.03181)
*Rewida Ali,Cristian C. Beltran-Hernandez,Weiwei Wan,Kensuke Harada*

Main category: cs.RO

TL;DR: 本文提出了一个学习框架，结合大型语言模型（LLM）的高级规划和混合模仿学习/强化学习（IL/RL）的低级策略，通过引入子任务感知的机器人Transformer（START）模型，成功实现了97%的机器人自动礼品包装成功率，有效处理了可变形物体的长期操作挑战。


<details>
  <summary>Details</summary>
Motivation: 在仓库和零售店等环境中，工人经常处理纸张、袋子和织物等可变形物体，人机协作至关重要。然而，由于可变形材料不可预测的动态特性以及对自适应力控制的需求，协调机器人行动与人类协助仍然困难。礼品包装任务是一个典型的长期操作问题，涉及精确折叠、受控压痕和固定纸张，是探索这一挑战的理想范例。

Method: 本文提出了一个学习框架，该框架将由大型语言模型（LLM）驱动的高级任务规划器与低级混合模仿学习（IL）和强化学习（RL）策略相结合。其核心是子任务感知机器人Transformer（START），它从人类演示中学习统一策略。关键创新在于在一个模型中捕获整个包装序列的长期时间依赖性，并通过引入子任务ID提供明确的时间定位，使策略能够学习子目标而非简单复制运动序列。

Result: 该框架在真实世界的包装任务中实现了97%的成功率。研究表明，统一的基于Transformer的策略减少了对专业模型的依赖，允许受控的人类监督，并有效连接了高级意图与可变形物体操作所需的精细力控制。

Conclusion: 所提出的学习框架通过其统一的、基于Transformer的策略，成功解决了机器人处理可变形物体（如礼品包装）的长期操作挑战。它证明了LLM驱动的高级规划与子任务感知的低级策略相结合的有效性，为未来人机协作和复杂可变形物体操作提供了新的解决方案。
...
### [114] [Collaborative Assembly Policy Learning of a Sightless Robot](https://arxiv.org/abs/2511.03189)
*Zeqing Zhang,Weifeng Lu,Lei Yang,Wei Jing,Bowei Tang,Jia Pan*

Main category: cs.RO

TL;DR: 本文提出了一种新颖的强化学习（RL）方法，结合了人类设计的导纳控制器，用于解决人机协作（pHRC）中的盲机器人与人类共同插入板的任务，旨在提高任务成功率、缩短完成时间并减少人类劳力。


<details>
  <summary>Details</summary>
Motivation: 传统的导纳控制难以准确测量人类施加的力/扭矩以估计意图，从而限制了机器人的协助能力。而现有的强化学习方法因安全约束和稀疏奖励，不适用于板插入这类pHRC任务。

Method: 提出了一种新颖的强化学习（RL）方法，该方法利用人类设计的导纳控制器来促进机器人更主动的行为，并减少人类的努力。

Result: 通过仿真和真实世界实验，证明所提出的方法在成功率和任务完成时间方面优于传统的导纳控制。此外，与导纳控制相比，该方法显著降低了测量的力/扭矩。

Conclusion: 所提出的结合人类设计导纳控制器的RL方法，能够有效解决人机协作中的板插入任务，不仅提高了任务性能，还显著减轻了人类的负担。
...
### [115] [GUIDES: Guidance Using Instructor-Distilled Embeddings for Pre-trained Robot Policy Enhancement](https://arxiv.org/abs/2511.03400)
*Minquan Gao,Xinyi Li,Qing Yan,Xiaojian Sun,Xiaopan Zhang,Chien-Ming Huang,Jiachen Li*

Main category: cs.RO

TL;DR: GUIDES框架通过整合基础模型的语义指导来增强预训练的机器人策略，无需重新设计架构，从而显著提高任务成功率和运动精度。


<details>
  <summary>Details</summary>
Motivation: 预训练的机器人策略缺乏基础模型所具备的语义感知能力，但由于成本高昂和知识损失，完全替换它们并不现实。本研究旨在弥补这一差距，在不替换现有策略的情况下提升其语义理解能力。

Method: GUIDES框架采用经过微调的视觉-语言模型（Instructor）生成上下文指令，并由辅助模块将其编码为指导嵌入。这些嵌入被注入到策略的潜在空间中，并通过简短、有针对性的微调使原有模型适应新的语义输入。为了提高推理时的鲁棒性，一个基于大型语言模型（LLM）的Reflector会监控Instructor的置信度，并在置信度较低时启动一个推理循环，分析执行历史，检索相关示例，并增强VLM的上下文以改进后续动作。

Result: 在RoboCasa模拟环境中，GUIDES在多种策略架构上均显示出任务成功率的一致且显著提升。在真实世界的UR5机器人部署中，GUIDES也增强了抓取等关键子任务的运动精度。

Conclusion: GUIDES提供了一种实用且资源高效的方法，用于升级而非替换已验证的机器人策略，使其具备语义感知能力，从而提升其性能。
...
### [116] [Value Elicitation for a Socially Assistive Robot Addressing Social Anxiety: A Participatory Design Approach](https://arxiv.org/abs/2511.03444)
*Vesna Poprcova,Iulia Lefter,Martijn Warnier,Frances Brazier*

Main category: cs.RO

TL;DR: 本研究通过与心理健康研究人员的参与式设计研讨会，探讨了为社交焦虑症患者设计社交辅助机器人时应考虑的核心价值观，包括适应性、接受度和有效性，并强调了以研究为主导的用户中心设计方法。


<details>
  <summary>Details</summary>
Motivation: 社交焦虑症普遍存在且严重影响生活质量，但现有支持不足。社交机器人技术为补充传统心理健康支持提供了新机遇。为了开发有效的解决方案，理解塑造有意义、可接受和有益设计的潜在价值观至关重要。

Method: 与心理健康学术研究人员举办了一场参与式设计研讨会。通过创造性、反思性和构想活动，参与者探索了情景和设计可能性，系统地启发了与机器人辅助干预相关的价值观、期望、需求和偏好。

Result: 研究结果揭示了与设计相关的丰富见解，包括适应性、接受度和有效性等核心价值观，这些对于支持社交焦虑症患者至关重要。

Conclusion: 本研究强调了以研究为主导的价值启发方法的重要性，并强调在开发社交辅助机器人时，应考虑以用户为中心和情境感知的设计因素。
...
### [117] [Development of the Bioinspired Tendon-Driven DexHand 021 with Proprioceptive Compliance Control](https://arxiv.org/abs/2511.03481)
*Jianbo Yuan,Haohua Zhu,Jing Dai,Sheng Yi*

Main category: cs.RO

TL;DR: 本文提出并验证了Dex-Hand 021，一种高性能、缆线驱动的五指机械手，具有19个自由度，重量仅1公斤。通过本体力传感的导纳控制方法，显著提高了其操作性能，特别是在多物体抓取和防过载方面。


<details>
  <summary>Details</summary>
Motivation: 在机器人系统中复制人手的多功能能力（包括运动、传感和协调操作）仍然是一个巨大的挑战。开发灵巧的机器人手需要平衡类人的敏捷性与工程约束，如复杂性、尺寸重量比、耐用性和力传感性能。

Method: 本文设计了Dex-Hand 021，这是一种缆线驱动的五指机械手，拥有12个主动和7个被动自由度，实现了19个自由度的灵巧性，且重量仅1公斤。研究提出了一种基于本体力传感的导纳控制方法，以增强其操作能力。

Result: Dex-Hand 021展现出卓越的性能：单指承载能力超过10牛顿，指尖重复性低于0.001米，力估计误差低于0.2牛顿。与PID控制相比，在多物体抓取中关节扭矩降低了31.19%，显著提升了力传感能力并防止碰撞过载。该机械手在力量和精确抓取方面均表现出色，成功执行了33种GRASP分类动作和复杂的操纵任务。

Conclusion: 这项工作推动了轻量级工业级灵巧手的设计，并增强了本体感知控制，为机器人操作和智能制造做出了贡献。
...
### [118] [ROSBag MCP Server: Analyzing Robot Data with LLMs for Agentic Embodied AI Applications](https://arxiv.org/abs/2511.03497)
*Lei Fu,Sahar Salimpour,Leonardo Militano,Harry Edelman,Jorge Peña Queralta,Giovanni Toffetti*

Main category: cs.RO

TL;DR: 本文介绍了一个MCP服务器，用于通过大型语言模型（LLMs）和视觉语言模型（VLMs）对ROS和ROS 2数据包进行自然语言分析、可视化和处理，旨在弥补具身智能体（Agentic Embodied AI）研究的空白。实验结果显示，不同LLM/VLM在工具调用能力上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 具身智能体（Agentic Embodied AI）是人工智能和机器人领域的关键研究方向，但相关文献稀缺。模型上下文协议（MCP）在智能体应用中日益重要。因此，研究人员旨在开发一个系统，允许通过自然语言分析和处理机器人数据，以推动具身智能体领域的发展。

Method: 研究引入了一个MCP服务器，专门用于分析ROS和ROS 2数据包。该服务器集成了机器人领域知识的特定工具，初步支持移动机器人轨迹、激光扫描、变换和时间序列数据的分析。它还提供了与标准ROS 2 CLI工具的接口，并具备数据包过滤功能。此外，研究团队还提供了一个轻量级UI，用于基准测试不同专有（如Anthropic, OpenAI）和开源（通过Groq）LLM的工具调用能力。

Result: 实验分析了八种最先进的LLM/VLM模型（包括专有和开源、大型和小型）的工具调用能力。结果表明，在工具调用能力上存在巨大差异，其中Kimi K2和Claude Sonnet 4表现出明显优越的性能。研究还发现，工具描述模式、参数数量以及可用工具数量等多个因素都会影响工具调用的成功率。
...
### [119] [Indicating Robot Vision Capabilities with Augmented Reality](https://arxiv.org/abs/2511.03550)
*Hong Wang,Ridhima Phatak,James Ocampo,Zhao Han*

Main category: cs.RO

TL;DR: 研究发现人类常误解机器人的视野范围（FoV），导致人机协作失败。本文提出并评估了四种增强现实（AR）FoV指示器，旨在校准人类对机器人视觉能力的心理模型，结果显示异中心和物理改动（如深眼窝）指示器能提高准确性，并提供了六条实践指南。


<details>
  <summary>Details</summary>
Motivation: 人类常对机器人的视野范围（FoV）存在不准确的心理模型，导致在人机协作任务中可能给机器人分配超出其视野范围的“不可能”任务，尤其是在机器人无法扫描场景更新世界模型时，这种误解更为严重。

Method: 提出了四种AR FoV指示器，涵盖从以自我为中心（机器人视角和头部空间）到以异物为中心（任务空间）的范围。通过一项包含41名参与者的人体实验，评估了这些指示器在准确性、信心、任务效率和工作量方面的表现。

Result: 结果表明，任务空间中的异中心方块指示器具有最高的准确性，但解释机器人FoV时存在延迟。此外，通过物理改变（如更深的眼窝）实现的以自我为中心的指示器也提高了准确性。所有指示器都使参与者保持高信心和低认知负荷。

Conclusion: 本研究为实践者提供了六项指导方针，以应用所提出的AR指示器或进行物理改动，从而校准人类对机器人视觉能力的心理模型。
...
### [120] [Multi-User Personalisation in Human-Robot Interaction: Using Quantitative Bipolar Argumentation Frameworks for Preferences Conflict Resolution](https://arxiv.org/abs/2511.03576)
*Aniol Civit,Antonio Andriella,Carles Sierra,Guillem Alenyà*

Main category: cs.RO

TL;DR: 该研究提出了MUP-QBAF框架，这是一个基于量化双极论证框架的新型多用户个性化方法，旨在解决人机交互中多用户间冲突的偏好，并结合机器人动态环境观察进行适应性调整。


<details>
  <summary>Details</summary>
Motivation: 现有的人机交互个性化方法主要关注单用户适应性，忽略了多利益相关者可能存在冲突偏好的场景。为了解决这一多用户偏好冲突问题，本研究提出了新的框架。

Method: 本研究提出了多用户偏好量化双极论证框架（MUP-QBAF）。该框架基于量化双极论证框架（QBAFs），显式建模并解决多用户偏好冲突。与以往论证框架不同，它结合了用户的论点和机器人对环境的动态观察，允许系统随时间适应和响应变化的情境。偏好（包括正面和负面）被表示为论点，其强度根据新信息迭代重新计算。

Result: 通过一个现实案例研究（辅助机器人在体弱评估任务中调解看护者和受助者之间的冲突偏好），验证了该框架的属性和能力。评估还包括对论点基础分数的敏感性分析，展示了用户输入和上下文观察如何影响偏好结果。

Conclusion: 该工作为解决竞争性用户偏好提供了一种透明、结构化和上下文敏感的方法，推动了多用户人机交互领域的发展。它为数据驱动方法提供了一个有原则的替代方案，使机器人能够在现实世界环境中处理冲突。
...
### [121] [Manifold-constrained Hamilton-Jacobi Reachability Learning for Decentralized Multi-Agent Motion Planning](https://arxiv.org/abs/2511.03591)
*Qingyi Chen,Ruiqi Ni,Jun Kim,Ahmed H. Qureshi*

Main category: cs.RO

TL;DR: 本文提出了一种流形约束的哈密顿-雅可比可达性（HJR）学习框架，用于解决多智能体运动规划中任务诱导的流形约束问题，实现了安全且任务可行的分散式运动规划。


<details>
  <summary>Details</summary>
Motivation: 多智能体运动规划（MAMP）在动态环境中，同时遵守任务施加的流形约束（例如，服务机器人需保持杯子直立），是一个关键挑战。尽管分散式MAMP在处理高维系统方面有所进展，但整合流形约束仍然困难。

Method: 本文提出了一种流形约束的哈密顿-雅可比可达性（HJR）学习框架。该方法在流形约束下求解HJR问题以捕捉任务感知安全条件，然后将这些条件整合到一个分散式轨迹优化规划器中。

Result: 该方法能够生成既安全又任务可行的运动计划，且无需假设其他智能体的策略。它能推广到各种流形约束任务，并有效扩展到高维多智能体操作问题。实验表明，该方法优于现有约束运动规划器，并能以适用于实际应用的速度运行。

Conclusion: 所提出的流形约束HJR学习框架有效解决了多智能体运动规划中的任务诱导流形约束问题，提供了一种可扩展、高性能的解决方案，适用于实际应用中的安全多智能体操作。
...
TL;DR: 本文介绍了一种创新的自主无人机系统，用于绘制世界上最大的无人机壁画，该系统在恶劣户外条件下仍能实现高精度导航和绘画。


<details>
  <summary>Details</summary>
Motivation: 在风和阳光等不利户外条件下，使用无人机进行大规模壁画创作面临保持艺术精度和操作可靠性的双重挑战。

Method: 该系统采用结合红外（IR）运动捕捉摄像头和激光雷达（LiDAR）的新型导航系统，实现精确位置跟踪。其独特的控制架构在切向和法向采用不同调节，确保精确轨迹跟踪和稳定线条绘制。此外，还提出了轨迹规划和路径优化算法，并设计了定制的喷漆机构，以应对螺旋桨产生的气流并保护无人机组件。

Result: 实验结果证明了该系统在各种条件下的鲁棒性和精度，展示了其在自主大规模艺术创作方面的潜力。

Conclusion: 该系统成功扩展了机器人技术在创意领域的应用功能。
...
TL;DR: 本文提出了一种新颖的自动机理论方法，用于在不确定环境中（语义标签具有概率知识）解决由scLTL规范表达的时空逻辑任务的运动规划问题，并利用值迭代进行在线重规划。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决在存在不确定性的环境中（例如，任务区域的精确位置未知，但有概率信念），机器人如何执行复杂的时空逻辑任务（如“先去区域1，再去区域2”）的运动规划问题。

Method: 该方法将不确定性建模为环境语义标签的概率知识。核心是构建一个特殊的乘积自动机来捕获与语义标签相关的不确定性，并为该自动机的每条边设计奖励函数。算法利用值迭代进行在线重规划。

Result: 论文展示了一些理论结果，并通过仿真/实验证明了所提出方法的有效性。

Conclusion: 所提出的基于自动机理论和值迭代的在线重规划方法，能够有效解决在不确定环境中执行时空逻辑任务的运动规划问题。
...
### [124] [Multi-robot searching with limited sensing range for static and mobile intruders](https://arxiv.org/abs/2511.03622)
*Swadhin Agrawal,Sujoy Bhore,Joseph S. B. Mitchell,P. B. Sujit,Aayush Gohil*

Main category: cs.RO

TL;DR: 在几何区域中利用多机器人搜索入侵者的问题被证明是NP难的。本文提出了基于空间填充曲线、随机搜索和协作随机搜索的算法，并评估了机器人数量与搜索时间之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 在简单连通的正交多边形区域内，利用有限感知能力的多机器人搜索入侵者（包括静态和移动）的问题，即使对于静止入侵者，也被证明是NP难的。这种固有的计算复杂性促使研究者开发高效且鲁棒的搜索算法。

Method: 研究方法包括：1) 定义搜索区域为简单连通的正交多边形；2) 考虑静态和移动入侵者；3) 提出基于空间填充曲线、随机搜索和协作随机搜索的算法；4) 评估每种算法在所需机器人数量和完成搜索所需时间之间的权衡，并考虑搜索区域的几何特性。

Result: 主要发现是，即使对于静止入侵者，在几何区域内利用多机器人搜索入侵者的问题也是NP难的。在此基础上，论文开发并评估了多种高效且鲁棒的算法，并分析了它们在机器人数量与搜索完成时间之间的权衡。
...
### [125] [Unconscious and Intentional Human Motion Cues for Expressive Robot-Arm Motion Design](https://arxiv.org/abs/2511.03676)
*Taito Tashiro,Tomoko Yonezawa,Hirotake Yamazoe*

Main category: cs.RO

TL;DR: 本研究探讨如何利用人类运动线索（自然游戏和指令表达）设计富有表现力的机器人手臂动作，发现后期运动时机（特别是撤回阶段）和物理实体对印象形成和线索解读至关重要。


<details>
  <summary>Details</summary>
Motivation: 研究旨在利用人类运动线索来设计富有表现力的机器人手臂动作，以提升人机交互的自然性和可解释性。

Method: 研究分析了人类在不完全信息游戏Geister中的两种棋子移动动作：自然游戏（无意识倾向）和指令表达（有意提示）。基于此，通过改变运动速度和停止时长，创建了分阶段的机器人动作，并在物理机器人和录制视频两种呈现模式下评估了观察者的印象。

Result: 结果表明，后期运动时机，尤其是在撤回阶段，对印象形成起着重要作用；同时，物理实体增强了运动线索的可解释性。
...
### [126] [Source-Free Bistable Fluidic Gripper for Size-Selective and Stiffness-Adaptive Grasping](https://arxiv.org/abs/2511.03691)
*Zhihang Qin,Yueheng Zhang,Wan Su,Linxin Hou,Shenghao Zhou,Zhijun Chen,Yu Jun Tan,Cecilia Laschi*

Main category: cs.RO

TL;DR: 本文介绍了一种自给自足的软抓手，通过内部液体在三个互连双稳态“弹跳”腔室中的重新分配实现尺寸选择性和刚度自适应抓取，无需外部能源。


<details>
  <summary>Details</summary>
Motivation: 传统流体驱动软抓手依赖外部能源，限制了其便携性和长期自主性。

Method: 该抓手采用固定尺寸设计，包含三个互连的双稳态“弹跳”腔室。当顶部传感腔室接触物体时发生形变，排出的液体触发抓取腔室的“弹跳”式扩张，实现抓取。内部液压反馈机制使其能被动适应物体刚度。

Result: 实现了无需持续能量输入的稳定、尺寸选择性抓取。抓取压力能被动适应物体刚度。设计无外部能源且紧凑。

Conclusion: 这种无外部能源、紧凑的设计为软机器人中轻量级、刚度自适应的流体驱动操作开辟了新可能性，为水下和野外环境中特定尺寸的采样和操作提供了可行方法。
...
### [127] [Robust reduced-order model predictive control using peak-to-peak analysis of filtered signals](https://arxiv.org/abs/2511.03002)
*Johannes Köhler,Carlo Scholz,Melanie Zeilinger*

Main category: eess.SY

TL;DR: 本文提出了一种基于降阶模型（ROM）的鲁棒模型预测控制（MPC）方案，用于大规模线性系统，通过预测一个误差边界系统来保证约束满足和鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 为大规模线性系统设计一种计算可行且能保证鲁棒约束满足的模型预测控制（MPC）方案，尤其是在使用降阶模型时。

Method: 该方法利用降阶模型（ROM），结合鲁棒控制工具，并将其整合到MPC框架中。核心贡献是预测一个（标量）误差边界系统与ROM并行运行，以获得全阶系统预测输出的保证边界。此边界用于构建鲁棒的基于ROM的MPC，确保约束满足和鲁棒性能。该方法通过以下步骤开发：(i) 分析误差，(ii) 边界峰峰增益，以及 (iii) 使用滤波信号。

Result: 该方法在一个100维的质量-弹簧-阻尼器系统上进行了验证，相对于现有方法，保守性降低了四个数量级以上，并保证了约束满足和鲁棒性能。

Conclusion: 本文提出了一种新颖的鲁棒基于ROM的MPC方法，该方法通过预测误差边界系统，解决了大规模线性系统在计算可行性和鲁棒约束满足方面的挑战，并显著降低了保守性。
...
### [128] [Oscillation Analysis and Damping Control for a Proposed North American AC-DC Macrogrid](https://arxiv.org/abs/2511.03017)
*Kaustav Chatterjee,Sameer Nekkalapu,Antos Varghese,Marcelo Elizondo,Quan Nguyen,Xiaoyuan Fan*

Main category: eess.SY

TL;DR: 本文通过详细的模型仿真，评估了连接北美东西部电网（EI和WI）的多端直流（MTDC）宏电网的小信号稳定性风险，并设计了附加阻尼控制器以解决潜在的低阻尼振荡问题。


<details>
  <summary>Details</summary>
Motivation: 行业和美国能源部资助的研究提出通过MTDC宏电网连接北美东西部电网，并探讨了其容量共享和频率支持优势。然而，这种互联可能引起的小信号稳定性挑战尚未被充分研究，特别是互区振荡风险。

Method: 开发了定制的MTDC系统动态模型，并将其与包含大量逆变器基能源的工业级EI和WI模型集成。通过基于模型的振荡分析来识别低阻尼互区模式。设计了利用广域反馈调制选定换流站有功功率设定点的MTDC系统附加阻尼控制器。采用频率扫描方法进行数据驱动的模型线性化和控制器合成。

Result: 通过MTDC集成，EI和WI的互区模式可能发生潜在偏移，并识别出阻尼不足的模式。设计的附加阻尼控制器在预设运行条件和选定的偶发事件场景下，有效改善了系统的阻尼性能。

Conclusion: 研究揭示了通过MTDC宏电网连接北美东西部电网可能带来的小信号稳定性挑战，特别是互区振荡问题，并成功设计了有效的附加阻尼控制器来缓解这些风险，确保宏电网的稳定运行。
...
### [129] [Quantifying Power Systems Resilience Using Statistical Analysis and Bayesian Learning](https://arxiv.org/abs/2511.03043)
*Apsara Adhikari,Charlotte Wertz,Anamika Dubey,Arslan Ahmad,Ian Dobson*

Main category: eess.SY

TL;DR: 本研究提出一个框架，利用统计和贝叶斯学习方法，量化建模天气参数（风速、温度、降水）与电网弹性指标之间的关系，并发现这些变量的综合影响，以支持风险缓解和适应策略。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件日益频繁和强烈，严重影响电网并导致大规模停电，损害电力系统弹性，但目前对天气参数影响的系统建模和弹性量化研究有限。

Method: 采用统计和贝叶斯学习方法来建模天气参数与电力系统弹性指标之间的关系。利用真实世界的公开停电和天气数据，识别关键天气变量。通过对伊利诺伊州库克县和佛罗里达州迈阿密-戴德县的案例研究进行验证。

Result: 识别出风速、温度和降水是影响特定地区电网弹性的关键天气变量。这些天气参数在弹性分析和风险评估中至关重要。研究还发现，这些天气变量在联合研究时具有综合效应，与单独研究时的影响不同。

Conclusion: 该框架为理解天气事件如何影响配电系统性能提供了宝贵见解，有助于决策者制定更有效的风险缓解、资源分配和适应气候变化的策略。
...
### [130] [Microgrids optimal radial reconfiguration via FORWARD algorithm](https://arxiv.org/abs/2511.03059)
*Joan Vendrell Gallart,Russell Bent,Solmaz Kia*

Main category: eess.SY

TL;DR: 微电网的能源管理面临计算难题，本文提出了一种基于置换的迭代搜索方法（结合FORWARD）来解决资源分配和径向配置设计问题，并将其作为MINLP求解器的热启动策略，以实现可扩展的微电网设计。


<details>
  <summary>Details</summary>
Motivation: 微电网在集成分布式能源、增强能源弹性和减少停电影响方面前景广阔，但其固有的去中心化和动态操作带来了巨大的能源管理复杂性。这些复杂性（如平衡供需、确保系统稳定性、最小化运营成本）通常需要解决计算上难以处理的NP-hard混合整数非线性规划（MINLP）问题。传统的MINLP求解器难以满足这些挑战所需的可扩展性和可行性保证。

Method: 本文针对微电网配电的资源分配和径向配置设计问题，提出了一个抽象问题。通过在FORWARD方法上引入一种基于置换的迭代搜索方法来解决该问题，以有效地识别可行、接近最优的径向网络结构，同时固有地遵守物理约束。此外，本文还研究了将所提出的方法作为基准MINLP求解器的热启动策略，以提供全面的微电网设计的可扩展解决方案。

Result: 所提出的方法能够有效地识别可行、接近最优的径向网络结构，同时固有地遵守物理约束。当作为基准MINLP求解器的热启动策略集成时，该方法为全面的微电网设计提供了一个可扩展的解决方案。

Conclusion: 本文通过提出一种基于置换的迭代搜索方法（结合FORWARD），解决了微电网的资源分配和径向配置设计问题，该方法能够高效找到可行、接近最优的径向网络结构。将其作为MINLP求解器的热启动策略，为复杂的微电网设计提供了一个可扩展的解决方案。
...
### [131] [Active Noise Control Method Using Time Domain Neural Networks for Path Decoupling](https://arxiv.org/abs/2511.03162)
*Yijing Chu,Qinxuan Xiang,Sipei Zhao,Ming Wu,Y. Zhao,Guangzheng Yu*

Main category: eess.SY

TL;DR: 本文分析了去中心化主动噪声控制（ANC）系统中串扰和预滤波对控制性能的影响，并提出了一种混合DecNet-LMS算法，结合自适应滤波器和神经网络，以提高去中心化ANC的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 在去中心化主动噪声控制（ANC）系统中，多通道次级声源和误差麦克风之间的串扰会严重降低控制精度。此外，在Fx型算法中对参考信号进行预滤波可能会引入额外的建模误差，这些问题促使研究者寻求更有效的去中心化ANC解决方案。

Method: 首先，对基于Fx的去中心化控制算法进行了理论分析，揭示了预滤波和串扰如何影响控制性能。然后，提出了一种结合固定值神经网络（DecNet）和自适应策略的混合方法。其中，自适应滤波器使用LMS算法在线建模自身通道的主路径，而神经网络（DecNet）则用于次级路径的反演和解耦。该混合DecNet-LMS算法在时域实现，以确保因果性和避免延迟。

Result: 通过测量声学路径的仿真结果表明，在不同的声学条件下，所提出的混合DecNet-LMS方法优于现有使用传统自适应滤波器或基于神经网络的固定系数方法的ANC算法。

Conclusion: 该研究得出结论，提出的混合DecNet-LMS算法能有效解决去中心化ANC系统中的串扰和预滤波问题，显著提升控制性能，优于现有主流的ANC算法。
...
### [132] [MHE in Output Feedback Control of Uncertain Nonlinear Systems via IQCs](https://arxiv.org/abs/2511.03221)
*Yang Guo,Stefan Streif*

Main category: eess.SY

TL;DR: 本文提出了一种针对具有参数或静态非线性不确定性的非线性受限系统的移动视界估计（MHE）方案，并引入了基于积分二次约束（IQCs）的鲁棒可检测性概念，以确保闭环系统的输入到状态稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为具有不确定性和预定状态反馈控制器的非线性受限系统设计一种鲁棒的估计方案，以确保即使存在估计误差，系统也能保持稳定。

Method: 该研究提出了一种MHE方案，并利用积分二次约束（IQCs）引入了一种新的、对非参数不确定性鲁棒且可实际验证的可检测性概念。在此可检测性假设下，MHE被设计成使得由不确定系统、控制器和MHE组成的闭环系统对外源扰动具有输入到状态稳定性（ISS）。

Result: 研究结果是提出了一种针对一般非线性受限系统的MHE方案，并引入了一个基于IQCs的鲁棒可检测性新概念。在满足此可检测性条件下，闭环系统（不确定系统、控制器和MHE）实现了对外源扰动的输入到状态稳定性。

Conclusion: 结论是，所提出的MHE方案结合基于IQCs的鲁棒可检测性概念，能够为具有不确定性的非线性受限系统提供一个鲁棒的估计框架，并确保与预设控制器协同工作时，闭环系统具有输入到状态稳定性。
...
### [133] [Theoretical and Experimental Limitations of RoCoF Estimation](https://arxiv.org/abs/2511.03249)
*Gutierrez-Florensa,F. Sanniti,D. Tedeschi,L. Sigrist,A. Ortega,F. Milano*

Main category: eess.SY

TL;DR: 本文提出了一种基于微分几何和流体力学概念的数值鲁棒方法，用于精确估计现代低惯量电力系统中的频率变化率（RoCoF），并通过实际测量进行测试，以开发更快的RoCoF-UFLS控制方案。


<details>
  <summary>Details</summary>
Motivation: RoCoF对电力系统安全运行至关重要，广泛应用于反孤岛、低频减载（UFLS）等保护系统。然而，现代电力系统低惯量和基于变流器的发电设备增加了瞬态的严重性，使得现有设备的频率和RoCoF估计变得复杂且不精确。

Method: 提出了一种数值鲁棒的方法，其概念源于微分几何和流体力学。该方法通过高采样率的实际实验测量数据进行测试，并用于开发基于RoCoF的UFLS控制方案的更快控制逻辑。

Result: 所提出的方法能够为保护系统提供有关突发事件性质的信息，从而改善其响应。它还能够开发出更快的RoCoF-UFLS控制逻辑。

Conclusion: 该研究成功解决了现代电力系统中RoCoF估计的挑战，提供了一种精确且鲁棒的方法，能够提高保护系统的响应速度，并为UFLS控制方案提供更快、更准确的决策支持。
...
### [134] [Evolutionary Dynamics in Continuous-time Finite-state Mean Field Games -- Part II: Stability](https://arxiv.org/abs/2511.03297)
*Leonardo Pedroso,Andrea Agazzi,W. P. M. H. Heemels,Mauro Salazar*

Main category: eess.SY

TL;DR: 本文研究了大规模动态博弈中混合平稳纳什均衡（MSNE）的演化稳定性，并导出了确保其在演化动力学下局部和全局稳定性的条件。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了理解MSNE（在第一部分中引入的解决方案概念）何时能在大规模动态博弈中稳健地出现和持续存在，从而深入了解其长期可行性。

Method: 本文调查了MSNE的演化稳定性。它在MSNE的结构和博弈的收益图上推导了确保其在演化动力学下局部和全局稳定性的条件。这项工作建立在第一部分引入的演化模型和MSNE概念之上。

Result: 研究导出了MSNE在演化动力学下实现局部和全局稳定性的条件，这些条件取决于MSNE的结构和博弈的收益图。

Conclusion: 这些结果描述了MSNE何时能够稳健地出现并抵抗策略性偏差而持续存在，从而为MSNE在这种大规模动态博弈中的长期可行性提供了见解。
...
### [135] [Lightwave Power Transfer-Enabled Underwater Optical ISAC Systems under Ship Attitude Variation](https://arxiv.org/abs/2511.03366)
*Kapila W. S. Palitharathna,Constantinos Psomas,Ioannis Krikidis*

Main category: eess.SY

TL;DR: 本文提出了一种光波能量传输驱动的水下光集成传感与通信（O-ISAC）系统，该系统能为海底传感器供能并实现上行通信，同时通过船载针孔摄像头阵列估计水下目标位置，并考虑了船体姿态变化对系统性能的影响。


<details>
  <summary>Details</summary>
Motivation: 在实际部署条件下，水下传感器的能量供给和信息传输面临挑战，同时需要精确地定位水下目标。现有系统可能未充分考虑船体姿态变化等实际因素对水下O-ISAC系统通信和传感性能的影响。

Method: 研究人员提出了一种O-ISAC系统，其中海面船只上的接入点向海底传感器（能量收集和上行通信）和传感目标（位置估计）传输光波信号。为了模拟实际部署，船体姿态（横摇、纵摇、偏航角）被建模为在低到中等海况下遵循高斯分布。针对目标定位的均方误差（MSE）和可实现的上行数据速率，推导了闭式近似表达式，并通过分析和仿真结果进行验证。

Result: 分析和仿真结果显示出极好的一致性，验证了所提出的模型和推导的表达式。研究揭示了O-ISAC系统中固有的通信-传感权衡。结果还提供了宝贵的设计见解，包括优化船上摄像头位置以最小化定位误差（在横摇、纵摇、偏航角变化10°时，多摄像头可实现10^-2 m^2的最小MSE），以及所考虑设置下0.55的最佳能量收集-使用比率。

Conclusion: 该研究成功地提出并验证了一种光波能量传输驱动的水下O-ISAC系统，并考虑了船体姿态变化。研究不仅揭示了通信-传感权衡，还为实际系统设计提供了关键的优化指导，例如最佳摄像头放置和能量利用效率，从而提高了水下传感和通信的鲁棒性和性能。
...
### [136] [Maximum Likelihood Estimation of Dynamic Sub-Networks with Missing Data](https://arxiv.org/abs/2511.03391)
*João Victor Galvão da Mata,Anders Hansson,Martin S. Andersen*

Main category: eess.SY

TL;DR: 本文提出一种最大似然估计方法，允许在大型互联系统中仅使用局部测量识别子网络，从而显著降低计算复杂性并增强隐私。


<details>
  <summary>Details</summary>
Motivation: 传统最大似然估计在识别大型动态系统时计算成本过高。此外，在跨组织边界共享敏感内部数据存在隐私和效率问题。

Method: 该方法引入了一种最大似然估计技术，可在特定拓扑条件下，仅利用目标子网络内部以及与分隔器子网络直接连接的信号进行参数估计。文中建立了网络可分离性的理论条件，并推导了子网络的概率密度函数。

Result: 该方法显著降低了计算复杂性，并通过消除共享敏感数据需求增强了隐私。数值示例验证了其有效性。

Conclusion: 该研究提供了一种创新的最大似然估计方法，能够在不估计整个网络的情况下识别复杂系统中的子网络，从而在计算效率和数据隐私方面带来显著优势。
...
### [137] [A Digital Twin of Evaporative Thermo-Fluidic Process in Fixation Unit of DoD Inkjet Printers](https://arxiv.org/abs/2511.03379)
...
### [138] [An Alternative Derivation and Optimal Design Method of the Generalized Bilinear Transformation for Discretizing Analog Systems](https://arxiv.org/abs/2511.03403)
*Shen Chen,Yanlong Li,Jiamin Cui,Wei Yao,Jisong Wang,Yixin Tian,Chaohou Liu,Yang Yang,Jiaxi Ying,Zeng Liu,Jinjun Liu*

Main category: eess.SY

TL;DR: 本文提出了一种广义双线性变换（GBT）的新推导方法，揭示了其设计参数α（形状因子）的物理意义和稳定范围，并开发了一种基于误差函数的最优设计方法，通过低通滤波器验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 将模拟系统传递函数转换为数字系统是常用的设计方法，其中欧拉和Tustin变换是广义双线性变换（GBT）的特例。然而，GBT中设计参数α的物理意义和最优设计方法尚未得到充分研究。

Method: 本文通过采用新的六边形形状近似误差函数的封闭面积，提出了GBT的替代推导，并将参数α定义为形状因子。首次揭示了形状因子的物理意义（等于所提出六边形形状后向矩形比的百分比）。通过域映射确定了形状因子的稳定范围为[0.5, 1]。观察到两种不同的失真模式（幅度失真和相位失真），并基于归一化幅度或相位误差形式的目标函数，开发了形状因子的优化设计方法。最后，通过设计和测试低通滤波器（LPF）来验证所提方法的有效性。

Result: 提出了GBT的替代推导，其中参数α被定义为形状因子，并首次揭示了其物理意义（等于所提出六边形形状后向矩形比的百分比）。通过域映射证明了形状因子的稳定范围是[0.5, 1]。观察到两种失真模式：幅度和相位失真。开发了一种基于归一化幅度或相位误差的目标函数的形状因子优化设计方法。通过低通滤波器（LPF）的设计和实验验证了所提方法的有效性。

Conclusion: 本文提出的形状因子优化设计方法是有效的，并通过低通滤波器（LPF）的理论计算与实验结果对比得到了验证。
...
### [139] [System Identification of a Moored ASV with Recessed Moon Pool via Deterministic and Bayesian Hankel-DMDc](https://arxiv.org/abs/2511.03482)
*Giorgio Palma,Ivan Santic,Andrea Serani,Lorenzo Minno,Matteo Diez*

Main category: eess.SY

TL;DR: 本研究使用HDMDc及其贝叶斯扩展（BHDMDc）对系泊条件下的小型自主水面艇（ASV）进行系统辨识，成功构建了数据驱动的降阶模型，并验证了其在未见波浪激励下的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 小型ASV，特别是带有凹陷月池的船只，在波浪作用下会因晃荡产生非线性响应，增加了建模挑战。因此，需要有效的方法进行系统辨识和动力学建模。

Method: 在拖曳水池中对Codevintec CK-14e ASV进行了不规则和规则迎头浪条件下的实验。通过测量船体运动和系泊载荷，利用带控制的Hankel动态模态分解（HDMDc）及其贝叶斯扩展（BHDMDc）构建了数据驱动的降阶模型（ROMs）。

Result: HDMDc框架提供了准确的确定性船体动力学预测。贝叶斯公式（BHDMDc）通过考虑超参数选择的变异性，实现了模型响应的不确定性感知表征。两种方法都成功预测了船只对未见规则和不规则波浪激励的响应，首次展示了HDMDc-based ROMs对不同于训练集的海洋条件的泛化能力，并取得了高精度。

Conclusion: HDMDc-based ROMs是系统辨识的一种可行的数据驱动替代方案，能够高精度地再现船体动力学，并首次证明了其对不同于训练集的海洋条件的泛化能力。
...
### [140] [Data-driven Modeling of Grid-following Control in Grid-connected Converters](https://arxiv.org/abs/2511.03494)
*Amir Bahador Javadi,Philip Pong*

Main category: eess.SY

TL;DR: 本研究通过一个包含变流器型资源的简化电力系统生成合成数据，以评估数据驱动方法（如SINDy和深度符号回归）捕获现代电网复杂动态的能力。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源整合和智能电网技术实施，电力系统日益复杂，需要灵活、可扩展的建模方法来准确捕捉其动态。现有的数据驱动方法（如稀疏非线性动力学识别和深度符号回归）能够直接从数据中识别动态系统，本研究旨在评估它们在此场景下的有效性。

Method: 研究设置了一个变流器型资源替代传统发电机，连接到无限母线系统的无损输电线路中。在此配置下，以并网跟随控制模式生成合成数据。这些数据被用于评估SINDy和深度符号回归等方法在捕获系统动态方面的能力。

Result: 通过上述设置，成功生成了在并网跟随控制模式下的合成数据。这些数据为评估数据驱动方法（如稀疏非线性动力学识别和深度符号回归）有效捕获电力系统动态提供了基础。

Conclusion: 本研究建立了一个用于生成合成数据并评估数据驱动方法捕获现代电力系统动态的框架，特别关注了变流器型资源在并网跟随模式下的行为。这为进一步评估这些方法在应对复杂电网挑战方面的潜力奠定了基础。
...
### [141] [Powered Descent Trajectory Optimization of Chandrayaan-3 using Radau Collocation and Controllable Sets](https://arxiv.org/abs/2511.03594)
*Suraj Kumar,Aditya Rallapalli,Ashok Kumar Kakula,Bharat Kumar GVP*

Main category: eess.SY

TL;DR: 本文介绍了印度“月船3号”任务的动力下降轨迹设计，该设计采用伪谱Radau搭配法和基于可控性的航路点优化，以平衡燃料消耗和轨迹鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 印度于2023年8月23日成功实现月球软着陆，成为第四个实现此成就的国家。本文旨在详细介绍“月船3号”任务的动力下降轨迹设计。

Method: 优化框架基于伪谱Radau搭配法。采用基于可控性的航路点优化来增强轨迹对状态和控制扰动的鲁棒性。此外，明确量化了燃料消耗与鲁棒性之间的权衡。

Result: 本文展示了“月船3号”任务的动力下降轨迹设计，并明确量化了燃料消耗与鲁棒性之间的权衡，为任务规划提供了实用见解。

Conclusion: 通过量化燃料消耗与鲁棒性之间的权衡，本研究为月球任务规划的实际考量提供了深入见解。
...
### [142] [Artificial-reference tracking MPC with probabilistically validated performance on industrial embedded systems](https://arxiv.org/abs/2511.03603)
*Victor Gracia,Pablo Krupa,Filiberto Fele,Teodoro Alamo*

Main category: eess.SY

TL;DR: 本文提出了一种针对嵌入式系统的高效模型预测控制（MPC）实现，用于人工参考跟踪，该方案结合了实用的控制特性并进行了概率性能验证。


<details>
  <summary>Details</summary>
Motivation: 工业嵌入式系统计算资源有限，但高级控制技术（如MPC）的应用日益受到关注。现有MPC简化方案往往缺乏实际环境中所需的特性和属性。

Method: 本文实现了一种针对人工参考跟踪的MPC，通过最近开发的结构利用一阶方法求解。该实现融入了关键的实用特性，如无偏差方案集成、用于约束收紧的退避参数以及在扰动或模型失配下保持可行性的软约束。此外，还提出了一个用于长期运行闭环系统概率性能验证的框架。该方法在一个可编程逻辑控制器（PLC）上进行了验证，通过硬件在环设置控制一个非线性连续搅拌釜反应器。

Result: 所提出的方法成功应用于PLC，并能有效控制非线性连续搅拌釜反应器。闭环系统的行为通过概率验证，评估了约束违反情况以及MPC优化算法在每个时间步所需的迭代次数。

Conclusion: 本文为嵌入式系统提供了一个高效、功能丰富的MPC实现方案，适用于广泛的应用。该方案通过结合实用特性并进行概率性能验证，克服了传统嵌入式MPC的局限性，并展示了其在实际工业环境中的潜力。
...
### [143] [A Constant-Gain Equation-Error Framework for Airliner Aerodynamic Monitoring Using QAR Data](https://arxiv.org/abs/2511.03678)
*Ruiying Wen,Yuntao Dai,Hongyong Wang*

Main category: eess.SY

TL;DR: 本文提出了一种名为恒定增益方程误差法（CG-EEM）的新方法，用于利用QAR数据监测飞机在役气动性能。该方法克服了传统状态传播滤波器和标准递归估计器在处理低激励巡航数据时的局限性，实现了对机队性能的鲁棒、可扩展和高效监测。


<details>
  <summary>Details</summary>
Motivation: 监测客机的在役气动性能对于运营效率和飞行安全至关重要。然而，使用快速存取记录器（QAR）数据进行此项任务面临诸多挑战，尤其是在缺乏关键参数（如飞机转动惯量）以及传统方法在低激励巡航数据下表现不佳的情况下。

Method: 首先指出由于缺乏关键参数，传统的状态传播滤波器不适用于此应用，因此需要解耦的方程误差法（EEM）。接着通过比较分析发现，标准递归估计器（如RLS）在EEM框架下应用于低激励巡航数据时会过早收敛或不稳定。为解决这些问题，本文提出并验证了恒定增益方程误差法（CG-EEM），该方法采用带有恒定卡尔曼式增益的定制估计器，特别适用于巡航飞行的平稳、低信噪比特性。

Result: CG-EEM在包含200多次飞行的多机队大型数据集上进行了广泛验证。结果表明，该方法能够生成高度一致、符合物理规律的气动参数，并准确识别不同飞机类型之间已知的性能差异。该工具被证明是鲁棒、可扩展且计算高效的。

Conclusion: CG-EEM为机队范围内的性能监测和早期检测性能退化提供了一个强大的、可扩展且计算高效的工具，能够有效应对QAR数据在役性能监测的挑战。
...
### [144] [Optimizing the nnU-Net model for brain tumor (Glioma) segmentation Using a BraTS Sub-Saharan Africa (SSA) dataset](https://arxiv.org/abs/2511.02893)
...
TL;DR: 本研究发现，在BraTS撒哈拉以南非洲数据集上，nnU-Net模型在原始小规模数据集（60例）上配合强大的在线数据增强，比在离线增强的大规模数据集（360例）上表现更好，强调了数据质量和适当增强方法的重要性。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割是现代医学的关键成就，能精确描绘解剖和病理特征。自动化分割能让医生专注于诊断和治疗计划。本研究旨在开发准确、泛化性强的医学图像分割模型，特别关注代表性不足的地区。

Method: 使用BraTS撒哈拉以南非洲数据集，包含60例胶质瘤患者的多模态MRI图像。比较了两种训练方法：1) 在原始60例数据上使用nnU-Net及其强大的在线增强；2) 在经过离线增强的360例数据上训练nnU-Net模型。

Result: 令人惊讶的是，在原始60例数据上训练并结合在线增强的nnU-Net模型表现优于在离线增强的360例数据集上训练的模型。全肿瘤分割的Dice分数达到0.84。推测离线增强引入了人工解剖变异或强度分布，降低了泛化能力。

Conclusion: 研究结果强调了数据质量和适当的（在线）数据增强方法对于构建准确、泛化性强的医学图像分割模型的重要性，尤其是在代表性不足的区域。离线增强可能引入人为变异，从而降低模型的泛化能力。
...
### [145] [Domain-Adaptive Transformer for Data-Efficient Glioma Segmentation in Sub-Saharan MRI](https://arxiv.org/abs/2511.02928)
...
TL;DR: 针对撒哈拉以南非洲地区MRI基础设施有限和采集协议异构导致的严重域偏移问题，本文提出SegFormer3D-plus，一个放射组学引导的Transformer架构，用于在域变异下进行稳健的胶质瘤分割。
...
Method: 本文提出的SegFormer3D-plus方法结合了：(1) 用于扫描仪间强度协调的直方图匹配；(2) 结合PCA降维k-means的放射组学特征提取，用于域感知分层采样；(3) 具有频率感知特征提取和空间-通道注意力的双通路编码器；以及 (4) 用于边界细化的复合Dice-交叉熵损失。该模型在BraTS 2023上预训练并在BraTS-Africa数据上微调。

Result: SegFormer3D-plus在异构的非洲临床扫描中，展示了肿瘤亚区域描绘和边界定位的改进。

Conclusion: 该研究强调了放射组学引导的域适应在资源有限环境下对胶质瘤分割的价值。
...
### [146] [SAAIPAA: Optimizing aspect-angles-invariant physical adversarial attacks on SAR target recognition models](https://arxiv.org/abs/2511.03192)
*Isar Lemeire,Yee Wei Law,Sang-Heon Lee,Will Meakin,Tat-Jun Chin*

Main category: eess.IV

TL;DR: 本文提出了一种名为SAAIPAA的新型物理对抗攻击框架，通过部署和优化角反射器，即使在SAR平台方位角未知的情况下，也能有效规避基于机器学习的SAR自动目标识别（ATR）系统，实现了最先进的欺骗率。


<details>
  <summary>Details</summary>
Motivation: SAR ATR系统广泛应用于地球观测和监控，但其底层机器学习算法易受对抗性扰动攻击。特别是物理对抗攻击（PAAs）利用角反射器作为攻击载体，对ML-based ATR构成威胁。因此，需要对物理对抗扰动机制进行系统性研究。

Method: 本文提出了SAR方位角不变物理对抗攻击（SAAIPAA）框架，用于基于物理建模的反射器驱动对抗性扰动。其核心方法是在每个方位象限部署至少一个反射器并优化其朝向，从而在攻击者不了解SAR平台方位角的情况下仍能保持攻击效果。该方法旨在使物理规避攻击在SAR平台和目标之间考虑的方位角范围内高效且最优。

Result: SAAIPAA在白盒设置下，对DenseNet-121和ResNet50实现了超过80%的最先进欺骗率。当攻击者已知方位角时，平均欺骗率可达99.2%。在黑盒设置下，SAAIPAA在某些模型（如ResNet50到DenseNet121）之间具有良好的迁移性，但对其他模型（如MobileNetV2）的迁移性有待提高。此外，本文还引入了一种为MSTAR数据集生成密集采样方位角SAR数据集边界框的方法。

Conclusion: SAAIPAA是一个严格且高效的物理对抗攻击框架，即使在SAR平台方位角未知的情况下，也能通过策略性部署和优化反射器来有效规避SAR ATR系统。它在白盒设置下取得了优异的欺骗率，并在黑盒设置下展现了良好的模型间迁移性，尽管在某些模型上仍有提升空间。研究结果突出了对SAR ATR系统进行物理攻击的强大能力，并为未来的防御研究提供了见解。
...
### [147] [Morpho-Genomic Deep Learning for Ovarian Cancer Subtype and Gene Mutation Prediction from Histopathology](https://arxiv.org/abs/2511.03365)
*Gabriela Fernandes*

Main category: eess.IV

TL;DR: 本研究提出一种混合深度学习方法，结合细胞核形态测量和深度图像特征，直接从H&E病理图像中对卵巢癌进行亚型分类和基因突变推断，为精准病理学铺平道路。


<details>
  <summary>Details</summary>
Motivation: 卵巢癌致死率高，主要原因在于诊断晚和亚型高度异质性。现有诊断方法在揭示基因组变异方面存在局限性，无法满足精准肿瘤学的需求。

Method: 开发了一种新颖的混合深度学习流水线，将定量细胞核形态测量与深度卷积图像特征相结合。该方法使用来自TCGA和公共数据集的约45,000个图像块，构建了一个融合模型（ResNet-50 CNN编码器与Vision Transformer (ViT)），以捕捉局部形态纹理和全局组织上下文。

Result: 该流水线在卵巢癌亚型分类方面取得了84.2%的整体准确率（Macro AUC为0.87 ± 0.03）。在基因突变推断方面也表现出中高准确率：AUC_TP53 = 0.82 ± 0.02，AUC_BRCA1 = 0.76 ± 0.04，AUC_ARID1A = 0.73 ± 0.05。特征重要性分析表明，细胞核实性和偏心率是TP53突变的主要预测因子。

Conclusion: 可量化的组织学表型编码了可测量的基因组信号，这为卵巢癌的分类和诊断提供了经济高效的精准病理学方法。
...
### [148] [Computational Imaging Meets LLMs: Zero-Shot IDH Mutation Prediction in Brain Gliomas](https://arxiv.org/abs/2511.03376)
*Syed Muqeem Mahmood,Hassan Mohy-ud-Din*

Main category: eess.IV

TL;DR: 该研究提出了一个结合大型语言模型（LLM）和计算图像分析的框架，用于对脑胶质瘤进行非侵入性、零样本的IDH突变状态预测。


<details>
  <summary>Details</summary>
Motivation: 在神经肿瘤学中，需要一种精确、非侵入性的肿瘤基因分型方法，以改进胶质瘤的诊断策略。

Method: 该框架处理配准的多参数MRI扫描和多类别肿瘤分割图，提取可解释的语义属性和定量特征，并将其序列化为JSON文件。然后，使用GPT-4o和GPT-5进行零样本查询（无需微调）来预测IDH突变状态。

Result: 在六个公开数据集（N=1427）上评估，该框架在异质队列中展现出高准确性和平衡的分类性能，即使在没有手动标注的情况下也有效。GPT-5在上下文驱动的表型解释方面优于GPT-4o。体积特征被发现是最重要的预测因子，辅以亚型特异性影像标记和临床信息。

Conclusion: 研究结果表明，将基于LLM的推理与计算图像分析相结合，在实现精确、非侵入性肿瘤基因分型方面具有巨大潜力，有助于推动神经肿瘤学的诊断策略发展。
