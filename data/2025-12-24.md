...
TL;DR: 提出PhysMaster，一个基于LLM的自主理论物理学家代理，通过结合抽象推理与数值计算、利用知识库LANDAU和自适应探索策略，在开放科学场景中实现端到端问题解决。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理主要在封闭基准或文献检索等通用任务上评估，缺乏在抽象、数学密集且需结合分析推理与代码计算的物理学开放场景中的端到端问题解决能力。

Method: 构建PhysMaster代理，耦合抽象推理与数值计算；设计LANDAU知识库存储文献、先验知识和方法轨迹以提升决策可靠性；采用自适应探索策略平衡效率与开放探索。

Result: 在高能理论、凝聚态理论和天体物理等问题上验证，实现：(i) 加速：将数月研究压缩至数小时；(ii) 自动化：自主执行假设驱动循环；(iii) 自主发现：独立探索开放问题。

Conclusion: PhysMaster展示了LLM代理在复杂科学领域实现研究加速、自动化和自主发现的潜力，为开放科学场景中的智能研究助手提供了可行框架。
...
TL;DR: 本文提出了一种双目标优化模型，用于灾后救援物资配送中的车辆路径规划与有限物资分配，旨在平衡效率与公平性。


<details>
  <summary>Details</summary>
Motivation: 重大灾害中预置物资常无法满足所有需求，需在有限资源下规划车辆路径并分配物资，以兼顾配送效率与需求公平性。

Method: 建立基于基尼系数的双目标混合整数规划模型，采用ε-约束法处理多目标，推导最优解性质并引入有效不等式，设计分支定价算法求解。

Result: 在土耳其凡城地震和伊斯坦布尔卡塔尔地区数据上的计算表明，分支定价算法显著优于商业求解器，双目标方法可将援助分配不公平性降低34%而不损失效率。

Conclusion: 时间约束极宽松或极严格时，优先满足需求的词典优化有效；中等时间约束下需采用平衡方法以避免不公平结果。
...
TL;DR: 该研究利用插值解码技术，通过成对对立提示词模拟人格维度，使大语言模型在经济学游戏中复现人类决策行为，并尝试将个体玩家行为映射到插值空间。


<details>
  <summary>Details</summary>
Motivation: 现有研究使用大语言模型模拟人类行为时，需为每个人格特征创建独立提示词，导致实验成本高且可复现性差。

Method: 采用插值解码技术，将人格五维度各表示为对立提示词对，通过插值参数在维度上连续调节模拟行为。

Result: 插值解码能可靠调节人格五维度得分；使大语言模型在经济学游戏中复现人类心理学研究结果；初步实现通过插值空间搜索匹配个体玩家行为。

Conclusion: 插值解码为高效模拟人格影响决策提供了可扩展方法，支持大语言模型在行为经济学中替代人类受试者的应用潜力。
...
TL;DR: 本文提出了一种针对PlantClef 2025挑战赛的细粒度多标签物种识别方法，通过使用训练数据集中的类别原型作为指导，训练分割视觉Transformer（ViT）模型，在测试集图像上进行领域自适应，最终在比赛中获得第五名。


<details>
  <summary>Details</summary>
Motivation: PlantClef 2025挑战赛需要从高分辨率图像中进行细粒度多标签物种识别，传统方法难以直接适应从单物种分类到植被群落多标签分类的领域转换，因此需要一种有效的领域自适应方法。

Method: 方法包括：1）从训练数据集中提取特征并使用K-Means聚类生成类别原型；2）构建定制化的窄ViT分割模型，使用冻结的DinoV2作为补丁嵌入层；3）训练模型从测试图像重建训练数据的类别原型；4）利用注意力分数识别感兴趣区域并指导分类过程。

Result: 该方法在PlantCLEF 2025挑战赛私有排行榜上获得第五名，F1分数为0.33331，与最佳提交结果仅相差0.03，表现出竞争力。

Conclusion: 提出的基于类别原型指导的分割ViT方法能够有效实现从单物种分类到植被群落多标签分类的领域自适应，在细粒度植物识别任务中取得了有竞争力的结果。
...
TL;DR: 提出一种通过类内聚类生成伪标签进行分层分类的新方法，以缓解细粒度视觉分类中的类内差异问题，在PlantNet300k数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 细粒度视觉分类任务中，类内差异（同一类别内图像间的差异程度）会阻碍深度学习模型的学习，尤其当这些类别样本不足时。现有方法难以有效处理这种类内差异。

Method: 提出FGDCC方法：对每个类别单独进行聚类以发现编码图像间相似度的伪标签，然后利用这些伪标签进行分层分类，从而学习更细粒度的视觉特征。

Result: 在PlantNet300k数据集上的初步实验表明，即使部分组件尚未完全优化，该方法仍能达到最先进的性能，并揭示了未来需要改进的关键点。

Conclusion: 通过类内聚类生成伪标签进行分层分类的方法能有效缓解类内差异问题，提升细粒度视觉分类性能，但需要进一步优化以获得更确凿的证据。
...
TL;DR: 本文提出了S³IT基准测试，用于评估具身智能体在三维环境中整合社交规范与物理约束的能力，通过座位安排任务发现当前大语言模型仍存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法要么只关注脱离实体的社交推理（如纯文本），要么只关注无社交意识的物理任务，无法评估智能体在真实具身环境中综合权衡社交与物理约束的能力。

Method: 设计了空间情境化社交智能测试（S³IT），核心为座位排序任务：要求智能体在3D环境中为具有不同身份、偏好和复杂人际关系的LLM驱动NPC安排座位。采用可扩展的程序化框架生成多样化场景，要求智能体通过主动对话获取偏好、自主探索感知环境，并在复杂约束网络中进行多目标优化。

Result: 评估发现当前最先进的LLM在此任务上仍存在困难，与人类基线相比有明显差距。结果表明LLM在空间智能方面存在缺陷，但在处理具有明确文本线索的冲突时能接近人类水平。

Conclusion: S³IT基准测试揭示了现有LLM在整合社交与物理智能方面的局限性，强调了开发具身社交智能的必要性，并为未来研究提供了可扩展的评估框架。
...
TL;DR: 提出SOF框架，通过光流表示从无动作视频中学习潜在技能，实现高级规划并提升机器人任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以转化为低层动作，而潜在动作模型缺乏高级规划能力，需要弥合视觉预测与动作执行之间的鸿沟。

Method: 基于光流中间表示学习潜在技能空间，该表示同时对齐视频动态与机器人动作，支持从原始视觉数据直接获取和组合技能。

Result: 实验表明SOF在多任务和长时域场景中均能提升性能，验证了从视频直接学习技能的有效性。

Conclusion: SOF框架成功连接了视频生成与动作执行，为从视觉数据学习机器人技能提供了可行路径。
...
### [8] [Discovering Lie Groups with Flow Matching](https://arxiv.org/abs/2512.20043)
*Jung Yeon Park,Yuxuan Chen,Floor Eijkelboom,Jan-Willem van de Meent,Lawson L. S. Wong,Robin Walters*

Main category: cs.AI

TL;DR: 提出LieFlow方法，通过李群上的流匹配直接从数据中学习对称性，能发现更广泛的群类型且假设更少，在2D/3D点云上成功识别了离散群（包括复域上的反射对称）。


<details>
  <summary>Details</summary>
Motivation: 对称性对理解物理系统和提升机器学习性能都很重要，但通常需要先验知识。现有方法在群类型和假设条件上受限，需要更灵活、数据驱动的对称性发现方法。

Method: 将对称性发现建模为在更大假设群上学习分布，使学习分布与数据中观察到的对称性匹配。提出LieFlow方法：1）基于李群的流匹配；2）针对目标模式对称排列导致的“最后一刻收敛”问题，设计了新的流匹配插值方案。

Result: 在2D和3D点云数据上的实验表明，该方法能成功发现离散群（包括复域中的反射对称），且比先前方法更灵活、假设更少。

Conclusion: LieFlow提供了一种有效的数据驱动对称性发现框架，能处理更广泛的群类型，并通过改进的流匹配机制解决了对称模式学习中的收敛挑战。
...
TL;DR: 提出ProbGLC方法，结合概率与确定性模型进行跨视角地理定位，提升灾害响应中的定位准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 气候变化导致灾害频发，快速精准定位灾害位置对应急响应和资源调配至关重要，现有方法在准确性和可解释性方面存在不足。

Method: 设计统一的概率-确定性跨视角地理定位框架，引入不确定性量化，生成概率分布和可定位性评分，在MultiIAN和SAGAINDisaster数据集上进行多灾害类型测试。

Result: 在1公里和25公里精度下分别达到0.86和0.97的定位准确率，同时通过概率分布和可定位性评分提供模型可解释性。

Conclusion: ProbGLC方法在灾害地理定位中表现出优越性能，为快速灾害响应提供了可解释的生成式跨视角定位方案，具有实际应用潜力。
...
TL;DR: 本文通过强化学习训练大语言模型进行内容审核，在数据稀缺场景下实现高效政策对齐分类，相比监督微调提升100倍数据效率。


<details>
  <summary>Details</summary>
Motivation: 当前海量用户和AI生成内容需要政策合规审核，但现有方法面临标注稀疏、政策动态变化、需要深度推理等挑战，缺乏对强化学习实际应用的系统性研究。

Method: 采用强化学习训练通用语言模型为专业内容分类器，系统评估多种RL训练方案和奖励塑造策略（包括可验证奖励和LLM作为评判框架），在三个真实内容审核任务上进行实验。

Result: RL呈现S型扩展规律，性能随训练数据、采样次数和优化步骤增加平滑提升后逐渐饱和；在需要复杂政策推理的任务上显著提升性能，数据效率比监督微调高100倍。

Conclusion: 强化学习为工业级内容审核系统提供可行方案，特别适用于专家标注稀缺或昂贵的领域，能有效实现政策对齐的内容分类。
...
TL;DR: 提出Reason2Decide两阶段训练框架，解决临床决策支持系统中预测准确性与解释对齐的挑战，在医疗数据集上优于现有方法，且模型规模更小。


<details>
  <summary>Details</summary>
Motivation: 当前临床决策支持系统在实现高预测准确性的同时，难以生成与预测一致的解释，存在暴露偏差和任务分离问题。

Method: 采用两阶段训练框架：第一阶段训练模型生成解释；第二阶段联合训练标签预测和解释生成，使用计划采样从依赖黄金标签逐步过渡到依赖模型预测。

Result: 在三个医疗数据集上，Reason2Decide在预测指标（F1）和解释保真度（BERTScore等）上优于其他微调基线和部分零样本大语言模型；在分诊任务中对不同来源的解释具有鲁棒性；仅使用LLM生成解释进行预训练即可取得良好效果。

Conclusion: Reason2Decide能以比现有基础模型小40倍的规模实现高性能，降低对人工标注的依赖，使临床推理更易于在资源受限环境中部署，同时提供可解释的决策支持。
...
TL;DR: 提出一种结合大语言模型与股市反馈的自适应金融情感分析框架，通过指令微调、检索增强生成和强化学习优化情感预测与市场回报的对齐。


<details>
  <summary>Details</summary>
Motivation: 现有金融情感分析研究未考虑股价或市场反馈的影响，需要开发能动态适应市场行为的鲁棒情感建模方法。

Method: 1) 使用SentiFin数据集对LLaMA 3.2 3B模型进行指令微调；2) 基于句子嵌入余弦相似度的RAG管道动态选择多源上下文信息；3) 通过比较预测情感与实际次日股票回报的反馈驱动模块调整信源可靠性；4) 引入PPO强化学习代理优化跨时序数据的信源加权策略。

Result: 在2024-2025年NIFTY 50新闻标题数据上的实验表明，该系统在分类准确率、F1分数和市场对齐度上显著优于基线模型和静态检索方法。

Conclusion: 指令微调大语言模型与动态反馈、强化学习相结合的方法，能够实现鲁棒且具有市场感知能力的金融情感建模。
...
TL;DR: ActionFlow是一个针对资源受限边缘平台的系统级推理框架，通过跨请求流水线调度和内存优化技术，显著提升VLA模型的推理速度，使其能够在边缘设备上实现实时动态操作。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在动态真实环境中的部署受到高推理延迟的严重阻碍，其推理频率（3-5 Hz）远低于机器人平滑交互所需的控制频率（20-30 Hz），且现有优化方法通常需要大量重新训练或会损害模型精度。

Method: 提出ActionFlow框架，核心是跨请求流水线策略，将VLA推理重新定义为微请求的宏流水线；同时提出跨请求状态打包前向算子和统一KV环形缓冲区，将碎片化的内存操作融合为高效的密集计算。

Result: 实验表明，ActionFlow在OpenVLA-7B模型上实现了2.55倍的FPS提升，无需重新训练即可在边缘硬件上实现实时动态操作。

Conclusion: ActionFlow通过创新的系统级优化，有效解决了VLA模型在边缘设备上的推理延迟问题，为实时机器人控制提供了可行的解决方案。
...
TL;DR: MolAct是一个基于智能体强化学习的分子设计框架，通过两阶段训练实现分子编辑与优化，在多项任务中超越现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 分子编辑与优化是多步骤问题，需要迭代改进性质同时保持化学有效性和结构相似性。现有方法缺乏将分子设计形式化为智能体学习问题的框架，无法有效结合推理、工具使用与优化过程。

Method: 提出MolAct框架：1) 将分子设计形式化为智能体强化学习问题；2) 采用两阶段训练范式：先学习编辑能力，再重用编辑行为进行性质优化；3) 智能体通过多轮交互调用化学工具进行有效性检查、性质评估和相似性控制；4) 实例化出MolEditAgent（编辑任务）和MolOptAgent（优化任务）两个模型家族。

Result: 1) 分子编辑任务：MolEditAgent-7B在添加、删除、替换编辑中分别达到100、95、98的有效率，优于DeepSeek-R1等基线；MolEditAgent-3B性能接近Qwen3-32B-think。2) 分子优化任务：MolOptAgent-7B在LogP指标上超越Claude 3.7，在溶解度指标上保持竞争力，且在其他目标上表现均衡。

Conclusion: 将分子设计视为多步骤、工具增强的过程是实现可靠且可解释改进的关键。MolAct框架通过智能体学习结合推理与工具使用，为分子设计提供了新的有效范式。
...
TL;DR: 本文提出一种简单有效的方法，通过在时间序列数据tokenization前注入噪声，提升冻结大语言模型在零样本时间序列预测中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究在利用冻结大语言模型进行时间序列预测时，性能高度依赖于输入数据的文本表示，且模型参数无法适应分布偏移。

Method: 在原始时间序列数据tokenization前注入噪声，作为一种推理时数据增强手段，迫使冻结大语言模型基于鲁棒的时间模式而非表面数值特征进行预测。

Result: 该方法在多个基准测试中有效提升了性能，并在两个新构建的、确保未在LLM预训练中出现的时间序列数据集上验证了其有效性。

Conclusion: 噪声注入是一种非侵入式干预策略，能够增强冻结大语言模型在时间序列预测中的鲁棒性，为直接利用现成大语言模型进行时序预测提供了进一步支持。
...
TL;DR: 提出基于双向门控循环单元（BiGRU）的数据中心能效预测模型，通过特征选择和超参数优化，相比GRU模型在PUE预测上表现更优。


<details>
  <summary>Details</summary>
Motivation: 数据中心能耗高、碳足迹大，边缘计算和AI发展推动其存储需求增长。提升能效可降本增效、促进可持续发展，PUE是衡量数据中心运行效率的关键指标，预测PUE有助于识别影响能耗的关键特征并针对性优化。

Method: 使用EnergyPlus模拟新加坡数据中心生成52,560个样本的117维数据集；采用递归特征消除交叉验证（RFECV）筛选关键特征子集；构建BiGRU预测模型，通过超参数优化与GRU模型对比。

Result: 优化后的BiGRU模型在均方误差（MSE）、平均绝对误差（MAE）和R平方指标上均优于GRU模型，验证了其更高的PUE预测精度。

Conclusion: BiGRU模型能有效预测数据中心PUE，为能效优化提供数据支持，有助于通过调整关键特征降低能耗，推动数据中心可持续发展。
...
TL;DR: 比较人类与大型语言模型在数字游戏中的概念推理能力，发现人类更灵活地结合规则与相似性推理，而LLMs更依赖数学规则且需要更多样本进行泛化。


<details>
  <summary>Details</summary>
Motivation: 探究人类与大型语言模型在数学概念推理任务中的归纳偏好与泛化策略差异，以理解两者认知机制的根本区别。

Method: 使用数字游戏作为概念推理任务，以贝叶斯模型为分析框架，对比人类与LLMs的推理行为。

Result: 贝叶斯模型能更好拟合人类行为；人类灵活结合规则与相似性推理，LLMs更依赖数学规则；人类仅需单样本即可泛化，LLMs需要更多样本。

Conclusion: 人类与LLMs在数学概念推理中存在根本性差异，体现在归纳偏好、推理策略和少样本泛化能力上。
...
TL;DR: 本文提出PreSa框架，通过直接学习策略而非间接学习奖励和成本模型，解决离线偏好强化学习中安全策略学习问题，在连续控制任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于人类反馈的安全强化学习方法先学习奖励和成本模型，再用约束强化学习优化策略，但在长时域连续控制任务中，奖励和成本误差会累积，导致性能下降。

Method: 提出PreSa方法：1) 直接基于行为偏好对和轨迹段安全标签学习策略；2) 将偏好学习与安全对齐结合为约束优化问题，在拉格朗日框架下直接学习安全策略，无需显式学习奖励/成本模型；3) 在合成和真实人类反馈的连续控制任务中评估。

Result: 实验表明，该方法能成功学习高奖励的安全策略，在性能上优于最先进的基线方法，甚至超过使用真实奖励和成本的离线安全强化学习方法。

Conclusion: 通过直接学习策略而非间接建模奖励/成本，PreSa框架能有效解决长时域任务中的误差累积问题，实现安全与偏好的对齐，为离线偏好强化学习提供了更可靠的安全解决方案。
...
TL;DR: 本文介绍了TongSIM——一个用于训练和评估具身智能体的高保真通用平台，包含多样化的室内场景和开放式户外城镇模拟，旨在解决现有仿真平台功能单一、缺乏通用性的问题。


<details>
  <summary>Details</summary>
Motivation: 随着AI向多模态和具身智能发展，现有仿真平台多为特定任务设计，缺乏能够支持从底层导航到高层复合活动（如多智能体社交模拟、人机协作）的通用训练环境。

Method: 开发TongSIM平台，提供100多个多样化多房间室内场景和开放式交互丰富的户外城镇模拟；包含定制化场景、任务自适应保真度、多样智能体类型和动态环境模拟等特性；建立综合评估框架与基准测试。

Result: TongSIM实现了高保真、通用化的具身智能训练环境，支持感知、认知、决策、人机协作、空间与社会推理等能力的评估，为研究者提供了灵活可扩展的统一平台。

Conclusion: TongSIM填补了通用具身智能训练平台的空白，通过其广泛适用性和综合评估体系，有望加速通用具身智能的研究与发展。
...
TL;DR: 提出MemR^3记忆检索系统，通过路由器选择和证据缺口追踪机制实现闭环控制，提升LLM代理的答案质量。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统过度关注压缩存储而忽视检索过程的显式闭环控制，导致检索效率和质量受限。

Method: 1) 路由器机制在检索、反思、回答间动态选择；2) 全局证据缺口追踪器使回答过程透明化；3) 基于LoCoMo基准测试验证。

Result: 在LLM-as-a-Judge评分上超越基线，RAG提升7.29%，Zep提升1.94%（GPT-4.1-mini后端），兼容现有记忆存储。

Conclusion: MemR^3通过闭环控制实现自主记忆检索，为现有记忆系统提供即插即用的控制器，显著提升答案质量。
...
TL;DR: 提出G-SPEC神经符号框架，通过确定性验证约束概率规划，解决5G/6G网络中LLM智能体的随机风险问题，在模拟450节点5G核心网中实现零安全违规和94.1%修复成功率。


<details>
  <summary>Details</summary>
Motivation: 5G独立组网和6G演进中，静态自动化和深度强化学习无法满足编排需求，而大型语言模型智能体虽能实现意图驱动网络，但会引入拓扑幻觉和政策违规等随机风险。

Method: 采用神经符号框架G-SPEC，包含治理三元组：电信适配智能体(TSLAM-4B)、网络知识图谱(NKG)和SHACL约束，通过确定性验证约束概率规划过程。

Result: 在模拟450节点5G核心网中实现零安全违规，修复成功率达94.1%（基线为82.4%）；NKG验证贡献68%安全增益，SHACL策略贡献24%；验证延迟随子图规模呈O(k^1.2)增长，处理开销142ms。

Conclusion: G-SPEC框架能有效约束LLM智能体的随机行为，确保网络策略合规性，其可扩展性和低延迟特性使其适用于SMO层网络运营。
...
TL;DR: 本文通过案例研究，探索了如何让大语言模型从被动工具使用者转变为主动工作流架构师，并解决了自动技能生成中的四个结构性瓶颈，实现了自主编写生产级代码技能。


<details>
  <summary>Details</summary>
Motivation: 尽管CodeMem确立了可执行代码作为代理程序记忆的最佳表示形式，但从零开始自主合成这种记忆的机制仍未得到充分探索。研究旨在推动大语言模型从被动工具使用者向主动工作流架构师的转变。

Method: 通过涉及Outlook和OneDrive的跨服务编排任务的高保真案例研究，识别并解决了自动技能生成中的四个结构性瓶颈：发现差距（导航大型工具注册表）、验证差距（基础工具响应结构）、分解差距（用线性状态锚定替代低效搜索）和扩展差距（关注并发性和持久性）。

Result: 研究表明，通过强制执行假设、探测和编码的科学方法，代理能够自主编写健壮的生产级代码技能。

Conclusion: 通过解决自动技能生成中的关键瓶颈，大语言模型可以成功过渡为主动的工作流架构师，实现自主代码技能的合成与部署。
...
TL;DR: SynCraft是一个基于大语言模型推理能力的框架，通过原子级编辑而非直接生成SMILES来优化分子的可合成性，在保持结构新颖性的同时显著提升合成可行性。


<details>
  <summary>Details</summary>
Motivation: 当前生成式AI在化学空间探索中面临关键瓶颈：大量生成分子难以合成。现有方法（如后过滤或基于模板的投影）往往损害结构新颖性或破坏关键药效团。

Method: 将可合成性优化重构为精确的结构编辑问题而非序列翻译任务；利用大语言模型的推理能力预测可执行的原子级编辑序列；采用交互感知提示机制模拟药物化学专家直觉。

Result: 在广泛基准测试中超越现有最优方法，能生成具有高结构保真度的可合成类似物；成功编辑PLK1抑制剂并挽救先前被丢弃的RIPK1候选分子。

Conclusion: SynCraft通过推理驱动的结构编辑有效解决了“合成悬崖”问题，在保持生成分子创新性的同时显著提升其合成可行性，为AI驱动的药物发现提供了新范式。
...
### [24] [Generative Digital Twins: Vision-Language Simulation Models for Executable Industrial Systems](https://arxiv.org/abs/2512.20387)
*YuChe Hsu,AnJui Wang,TsaiChing Ni,YuanFu Yang*

Main category: cs.AI

TL;DR: 提出视觉语言仿真模型VLSM，通过布局草图和自然语言提示生成可执行的FlexScript代码，构建首个大规模生成式数字孪生数据集，并设计三个专用评估指标验证模型性能。


<details>
  <summary>Details</summary>
Motivation: 工业仿真系统需要跨模态推理能力，现有方法缺乏将视觉布局与语言描述结合生成可执行仿真代码的统一框架，且缺乏相关数据集和评估标准。

Method: 1. 提出VLSM模型统一视觉与文本理解；2. 构建包含12万组提示-草图-代码三元组的数据集；3. 设计SVR、PMR、ESR三个评估指标；4. 对视觉编码器、连接器和代码预训练语言骨干进行系统消融实验。

Result: 模型在结构准确性上接近完美，执行鲁棒性高；新评估指标能全面评估结构完整性、参数保真度和仿真器可执行性。

Conclusion: 本研究为生成式数字孪生奠定基础，实现了视觉推理与语言理解在可执行工业仿真系统中的集成。
...
TL;DR: 开发了基于DeepSeek Janus-Pro的Janus-Pro-CXR胸部X光解读系统，通过多中心前瞻性试验验证，在报告生成质量、关键发现检测和工作流程效率方面优于现有模型，并计划开源以促进AI辅助放射学临床转化。


<details>
  <summary>Details</summary>
Motivation: 全球放射科医生短缺，胸部X光工作量巨大，现有多模态大语言模型缺乏严格的前瞻性临床验证，需要开发可靠且高效的AI辅助诊断系统。

Method: 基于DeepSeek Janus-Pro模型开发Janus-Pro-CXR系统，采用轻量级架构和领域特定优化，通过多中心前瞻性临床试验（NCT07117266）进行严格验证，包括自动化报告生成、六种关键放射学发现检测，并与ChatGPT 4o等模型进行对比。

Result: 系统在自动化报告生成方面优于现有最先进模型（包括参数更大的ChatGPT 4o），能可靠检测六种临床关键放射学发现；前瞻性临床部署显示，AI辅助显著提高报告质量评分，减少解读时间18.3%（P<0.001），54.3%的案例中专家更偏好AI辅助结果。

Conclusion: Janus-Pro-CXR通过轻量级架构和领域优化提升了诊断可靠性和工作流程效率，特别适用于资源有限环境；模型架构和实施框架将开源，以促进AI辅助放射学解决方案的临床转化。
...
TL;DR: 提出Bohrium+SciMaster基础设施与生态系统方案，通过标准化AI资产管理和工作流编排，解决规模化代理科学面临的观测、复用、追溯等挑战，并在实际科学工作流中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: AI代理正推动科学工作流向多步骤、工具调用与验证结合的‘规模化代理科学’转变，但现有系统存在工作流难以观测与复现、工具非代理就绪、执行难以追溯、原型系统定制化导致复用困难等问题，阻碍了规模化应用。

Method: 提出Bohrium作为AI4S资产的托管追溯中心（类似AI for Science的HuggingFace），将科学数据、软件、计算和实验系统转化为代理就绪能力；SciMaster负责将这些能力编排为长周期科学工作流；中间通过‘科学智能基板’组织可复用模型、知识和组件，支持工作流推理与行动。

Result: 通过11个代表性主代理在实际工作流中验证该架构，实现了端到端科学周期时间的数量级缩减，并从百万级规模的实际工作负载中生成执行锚定的信号。

Conclusion: 规模化代理科学需要基础设施与生态系统协同支持，Bohrium+SciMaster通过资产标准化、工作流编排与智能基板的结合，为代理科学的可组合性、可审计性和持续改进提供了可行路径。
...
TL;DR: 本研究比较了大型语言模型（LLMs）与小型语言模型（SLMs）在预测危重患者休克方面的表现，发现LLMs在预测临床事件方面并不固有优于SLMs，尽管GatorTron-Base取得了最高的加权召回率（80.5%）。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的发展，其在自然语言处理领域的应用日益广泛，但在预测性任务中的应用研究较少。及时预测休克可以促进早期干预，从而改善患者预后。

Method: 使用MIMIC III数据库中17,294次ICU住院的文本数据，筛选出住院时间>24小时且休克指数（SI）>0.7的患者（正常SI指数355例，异常87例）。比较了GatorTron-Base（基于临床数据训练）、Llama 8B、Mistral 7B等LLMs与BioBERT、DocBERT、BioClinicalBERT、Word2Vec、Doc2Vec等SLMs。在微调过程中使用焦点损失和交叉熵损失来处理类别不平衡问题。

Result: GatorTron-Base取得了最高的加权召回率（80.5%），但LLMs和SLMs的整体性能指标相当。这表明LLMs在预测未来临床事件方面并不固有优于SLMs。

Conclusion: LLMs在预测临床事件方面并不固有优于SLMs。为实现有意义的临床结果，未来训练LLMs的努力应优先开发能够预测临床轨迹的模型，而不是专注于命名实体识别或表型分析等较简单的任务。
...
<details>
  <summary>Details</summary>
Motivation: 教师情感状态对教学效果和学生学业成就具有重要影响，但现有研究常因教师情感的表演性特征而难以准确捕捉，且忽略了教学信息对情感表达的关键影响。

Method: 1. 采用人机协同标注流程构建T-MED数据集，包含14,938个教师情感实例；2. 提出AAM-TSA模型，引入非对称注意力机制和分层门控单元，实现差异化跨模态特征融合。

Result: AAM-TSA模型在T-MED数据集上的实验结果表明，其在准确性和可解释性方面显著优于现有最先进方法。

Conclusion: 本研究通过构建高质量数据集和创新模型，为教师情感分析提供了有效解决方案，强调了多模态数据和教学情境信息在情感识别中的重要性。
...
TL;DR: 研究开发了基于大语言模型的SAGE系统用于立体定向放射外科治疗计划，发现推理模型在保持剂量学指标与人工计划相当的同时，能降低耳蜗剂量并展示系统性的规划行为。


<details>
  <summary>Details</summary>
Motivation: 当前黑盒AI系统在临床应用中因透明度不足而受限，需要开发更透明、可解释的自动化治疗计划系统。

Method: 在41例脑转移瘤患者的回顾性队列中，比较了推理模型与非推理模型生成的SRS治疗计划，评估剂量学指标并分析规划行为。

Result: 推理模型在主要终点指标（靶区覆盖率、最大剂量、适形指数、梯度指数）上与人工计划相当（p>0.21），但显著降低了耳蜗剂量（p=0.022）。推理模型展现出前瞻性约束验证和权衡审议等系统性规划行为。

Conclusion: 链式思维推理能提升自动化治疗计划的透明度和可解释性，优化轨迹可作为可审计日志，为透明自动化规划提供了可行路径。
...
TL;DR: 提出多智能体框架解决长视频问答问题，通过主LLM协调定位代理和视觉代理，在LongTVQA数据集上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法压缩内容导致信息丢失或工具集有限，削弱时序定位能力并忽略细粒度视觉线索，需要更精确的长视频理解方案。

Method: 采用强化学习训练的主LLM协调两个代理：定位代理负责定位问题相关片段，视觉代理提取针对性文本观察；主代理通过步数限制进行规划。

Result: 在LongTVQA和LongTVQA+数据集上，多智能体系统显著优于非智能体基线；强化学习进一步提升了智能体的推理和规划能力。

Conclusion: 多智能体协作框架能有效提升长视频问答的时序定位和细粒度理解，强化学习优化了多智能体合作的效率和准确性。
...
### [31] [Progressive Learned Image Compression for Machine Perception](https://arxiv.org/abs/2512.20070)
*Jungwoo Kim,Jun-Hyuk Kim,Jong-Seok Lee*

Main category: cs.CV

TL;DR: 提出PICM-Net，一种面向机器感知的渐进式学习图像编解码器，基于三值平面编码实现细粒度可扩展性，并通过自适应解码控制器动态调整解码级别以维持下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有学习型图像编解码器主要面向人类感知，而面向机器感知的渐进式压缩（支持单比特流多质量级别解码）尚未被探索，需要研究机器导向的码率-失真优先级策略。

Method: 基于三值平面编码构建PICM-Net，系统分析机器导向的潜在优先级策略，并设计自适应解码控制器在推理时动态确定所需解码级别以维持下游任务置信度。

Result: 实验表明该方法能实现高效自适应的渐进传输，同时在下游分类任务中保持高性能，为机器感知的渐进图像压缩建立了新范式。

Conclusion: PICM-Net成功填补了机器感知渐进压缩的空白，通过优先级策略分析和自适应控制机制，平衡了压缩效率与机器任务性能。
...
### [32] [PHANTOM: PHysical ANamorphic Threats Obstructing Connected Vehicle Mobility](https://arxiv.org/abs/2512.19711)
*Md Nahid Hasan Shuvo,Moinul Hossain*

Main category: cs.CV

TL;DR: 提出PHANTOM框架，利用变形艺术生成视角依赖的对抗样本，攻击自动驾驶车辆的视觉DNN和V2X通信系统，在多种条件下保持高攻击成功率并引发网络级连锁反应。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶系统依赖视觉DNN和V2X通信，但仍易受物理对抗攻击威胁，需要研究更隐蔽、可迁移且能引发系统性风险的攻击方法。

Method: 基于变形艺术原理设计视角依赖的对抗图案，在无需模型内部信息的黑盒设置下生成对抗样本，通过CARLA仿真测试不同速度、天气和光照条件，并用SUMO-OMNeT++联合仿真评估V2X网络影响。

Result: 在最优条件下攻击成功率超90%，恶劣环境仍保持60-80%；攻击在6-10米内激活，使车辆来不及反应；V2X虚假信息传播导致信息峰值年龄增加68-89%，破坏安全通信。

Conclusion: PHANTOM揭示了CAV生态系统在感知和通信层面的关键漏洞，对抗样本具有强迁移性和系统性破坏力，需加强针对视角依赖攻击的防御研究。
...
### [33] [Generating the Past, Present and Future from a Motion-Blurred Image](https://arxiv.org/abs/2512.19817)
*SaiKiran Tedla,Kelly Zhu,Trevor Canham,Felix Taubner,Michael S. Brown,Kiriakos N. Kutulakos,David B. Lindell*

Main category: cs.CV

TL;DR: 本文提出一种新方法，利用预训练的视频扩散模型从运动模糊图像中恢复视频，揭示拍摄时刻及前后的复杂场景动态，优于现有技术并支持多种下游任务。


<details>
  <summary>Details</summary>
Motivation: 运动模糊图像虽降低视觉质量，但编码了场景和相机运动信息。现有方法依赖手工先验或网络架构，难以处理复杂动态，且未尝试恢复拍摄前后的信息，因此需要更强大的技术来利用大规模数据先验。

Method: 采用预训练的视频扩散模型，基于互联网规模数据集，从单张运动模糊图像中恢复视频序列，无需特定训练，直接利用模型先验处理逆问题。

Result: 该方法在恢复复杂场景动态方面优于先前方法，能泛化到野外挑战性图像，并支持恢复相机轨迹、物体运动和动态3D场景结构等下游任务。

Conclusion: 通过预训练视频扩散模型，能够有效从运动模糊图像中提取过去、现在和未来的场景信息，为运动模糊分析提供了更鲁棒和通用的解决方案。
...
TL;DR: 提出一种多模态注意力网络，融合探地雷达时序模式与热成像空间特征，用于桥梁板分层检测，引入不确定性量化，在多数数据集上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有基础设施自动化检测技术中，单模态方法（探地雷达与红外热成像）存在互补性局限：雷达对湿度和浅层缺陷敏感，热成像受天气影响且探测深度有限，需开发多模态融合方法提升检测能力。

Method: 设计多模态注意力网络，包含雷达时序注意力、热成像空间注意力及跨模态融合模块；采用可学习嵌入发现互补缺陷模式；通过蒙特卡洛dropout和方差估计进行不确定性量化，分解为认知不确定性和偶然不确定性。

Result: 在五个桥梁数据集上，该方法在平衡至中度不平衡数据中准确率和AUC显著优于单模态及简单融合基线；跨模态注意力带来关键性能提升；不确定性量化降低校准误差，支持选择性预测；但在极端类别不平衡时注意力机制易出现多数类主导问题。

Conclusion: 注意力架构在典型场景下表现优异，极端不平衡需专门技术；系统具备部署效率，可实现实时检测，并为安全关键决策提供不确定性量化支持。
...
### [35] [Learning to Refocus with Video Diffusion Models](https://arxiv.org/abs/2512.19823)
...
<details>
  <summary>Details</summary>
Motivation: 自动对焦系统常无法准确捕捉目标主体，用户需要在拍摄后调整对焦，现有方法在感知质量和鲁棒性方面存在不足。

Method: 使用视频扩散模型从单张失焦图像生成对焦堆栈（视频序列），并发布大规模真实智能手机拍摄的对焦堆栈数据集。

Result: 在多种挑战性场景下，该方法在感知质量和鲁棒性方面均优于现有方法，支持交互式重聚焦和下游应用。

Conclusion: 该方法为日常摄影中的高级对焦编辑功能开辟了新途径，代码和数据集已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统像素空间压缩方法在低码率下难以同时保证高真实感与高保真度，因为像素失真与人类感知不一致。

Method: 1. 在VQ-VAE的生成式潜在空间进行变换编码；2. 引入分类超模块降低超信息比特成本；3. 使用基于码字预测的监督增强语义一致性。

Result: 1. 自然图像<0.04 bpp、人脸图像<0.01 bpp下保持高视觉质量；2. CLIC2020测试集上以比MS-ILLM少45%的比特数达到相同FID；3. 支持图像修复、风格迁移等应用。

Conclusion: 生成式潜在空间具有稀疏性、语义丰富和感知对齐优势，GLC架构能实现高效感知对齐的图像压缩，并支持扩展应用。
...
### [37] [Degradation-Aware Metric Prompting for Hyperspectral Image Restoration](https://arxiv.org/abs/2512.20251)
*Binfeng Wang,Di Wang,Haonan Guo,Ying Fu,Jing Zhang*

Main category: cs.CV

TL;DR: 提出DAMP框架，通过设计空间-光谱退化度量作为退化提示，结合空间-光谱自适应模块，实现无需显式退化先验的统一高光谱图像恢复。


<details>
  <summary>Details</summary>
Motivation: 现有统一高光谱图像恢复方法依赖难以获取的显式退化先验（如退化标签），而真实场景中退化复杂且混合，需要一种不依赖显式先验的鲁棒方法。

Method: 1. 设计空间-光谱退化度量作为连续量化多维退化的退化提示；2. 引入空间-光谱自适应模块动态调制特征提取；3. 将SSAM作为专家集成到混合专家架构，以退化提示作为门控路由。

Result: 在自然和遥感高光谱数据集上的实验表明，DAMP达到最先进性能，并在多样、混合或未见退化下展现出优异的泛化能力。

Conclusion: DAMP框架通过度量驱动的退化提示和自适应特征调制，实现了无需显式退化先验的高效、鲁棒统一高光谱图像恢复，具有强泛化性。
...
### [38] [RANSAC Scoring Functions: Analysis and Reality Check](https://arxiv.org/abs/2512.19850)
*A. Shekhovtsov*

Main category: cs.CV

TL;DR: 本文重新审视了RANSAC几何拟合中的模型评分问题，提出了球形噪声下的几何误差扩展，分析了MAGSAC++方法的理论缺陷，并建立了统一的评分函数评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有RANSAC方法中模型评分函数缺乏统一的理论框架，特别是MAGSAC++虽在基准测试中表现优异，但其理论推导存在缺陷，需要系统性的理论分析和实验验证。

Method: 1) 将高斯噪声下的几何误差扩展到球形噪声；2) 建立包含均匀分布异常值的混合模型，通过阈值参数化统一似然估计和鲁棒M估计；3) 分析MAGSAC++的理论推导问题；4) 提出基于大验证集或小随机验证集的评分函数实验评估方法。

Result: 1) 发现MAGSAC++的评分函数在数值上等价于简单的高斯-均匀似然模型；2) 所有评分函数（包括学习的内点分布）在实验中表现相同；3) MAGSAC++既不优于简单方法，也不对阈值超参数更不敏感。

Conclusion: 本文通过理论和实验分析全面重新评估了现有方法，为未来改进鲁棒拟合方法提供了重要基础，指出当前评分函数的理论差异在实际应用中可能并不显著。
...
### [39] [TAVID: Text-Driven Audio-Visual Interactive Dialogue Generation](https://arxiv.org/abs/2512.20296)
*Ji-Hoon Kim,Junseok Ahn,Doyeop Kwak,Joon Son Chung,Shinji Watanabe*

Main category: cs.CV

TL;DR: 提出TAVID统一框架，从文本和参考图像同步生成交互式视频和对话语音，实现视听模态的紧密耦合。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常孤立地研究说话头/倾听头生成或对话语音生成，忽略了人类对话中紧密耦合的多模态交互特性。

Method: 通过两个跨模态映射器（运动映射器和说话人映射器）集成人脸和语音生成流程，实现音频与视觉模态间的双向互补信息交换。

Result: 在四个维度（说话面部真实感、倾听头响应性、二元交互流畅性、语音质量）的广泛实验证明了该方法的有效性。

Conclusion: TAVID框架能够同步生成高质量的交互式面部和对话语音，为构建类人对话系统提供了统一的多模态解决方案。
...
### [40] [HyGE-Occ: Hybrid View-Transformation with 3D Gaussian and Edge Priors for 3D Panoptic Occupancy Prediction](https://arxiv.org/abs/2512.19871)
*Jong Wook Kim,Wonseok Roh,Ha Dam Baek,Pilhyeon Lee,Jonghyun Choi,Sangpil Kim*

Main category: cs.CV

TL;DR: 提出HyGE-Occ框架，通过融合3D高斯和边缘先验的混合视图变换分支，提升3D全景占据预测的几何一致性和边界感知能力，在Occ3D-nuScenes数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在3D全景占据预测中难以保持精确几何结构并捕捉3D实例的空间范围，影响全景分离的鲁棒性。

Method: 采用混合视图变换分支，融合连续高斯深度表示与离散深度仓表示以生成几何一致的BEV特征；同时从BEV特征提取边缘图作为辅助信息学习边缘线索。

Result: 在Occ3D-nuScenes数据集上超越现有方法，展现出更优的3D几何推理能力。

Conclusion: HyGE-Occ通过增强几何一致性和边界感知，有效提升了3D全景占据预测的精度与鲁棒性。
...
### [41] [Widget2Code: From Visual Widgets to UI Code via Multimodal LLMs](https://arxiv.org/abs/2512.19918)
...
TL;DR: 本文提出Widget2Code任务，针对紧凑、无上下文的小部件界面生成代码，开发了WidgetFactory系统，通过组件化设计和领域特定语言提升视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有UI2Code研究主要关注网页和移动界面，而小部件（widget）作为紧凑、无上下文的微界面，缺乏公开标注数据和专门方法，需要针对其密集布局和图标化特点进行研究。

Method: 1) 建立仅图像的小部件基准数据集和细粒度评估指标；2) 提出联合感知理解与结构化代码生成的基线方法，包括基于设计原则的组件组装、图标检索和可视化模块；3) 设计端到端系统WidgetFactory，包含框架无关的领域特定语言WidgetDSL和可编译为多前端实现的编译器；4) 引入自适应渲染模块优化空间布局。

Result: 基准测试显示，通用多模态大语言模型优于专用UI2Code方法，但仍生成不可靠且视觉不一致的代码。提出的WidgetFactory系统显著提升了视觉保真度，为Widget2Code建立了强基线。

Conclusion: 研究填补了小部件代码生成的研究空白，提出的系统框架和评估基准为未来Widget2Code研究提供了统一基础设施，通过结合感知理解与结构化生成解决了小界面的独特挑战。
...
### [42] [Unified Brain Surface and Volume Registration](https://arxiv.org/abs/2512.19928)
...
TL;DR: 提出NeurAlign深度学习框架，通过统一体积-表面表示联合配准大脑MRI的皮层和皮层下区域，在准确性和速度上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法将体积和表面配准分开处理，导致不一致性，限制了神经科学跨被试分析的效果。

Method: 利用中间球坐标空间桥接解剖表面拓扑与体积解剖结构，通过深度学习框架整合球面配准，确保体积与表面域的几何一致性。

Result: 在域内和域外数据集上均优于经典和机器学习方法，Dice分数提升高达7分，保持规则形变场，速度比标准方法快数个数量级。

Conclusion: NeurAlign以其高精度、快速推理和易用性，为联合皮层和皮层下配准设立了新标准。
...
### [43] [Vehicle-centric Perception via Multimodal Structured Pre-training](https://arxiv.org/abs/2512.19934)
*Wentao Wu,Xiao Wang,Chenglong Li,Jin Tang,Bin Luo*

Main category: cs.CV

TL;DR: 提出VehicleMAE-V2，一种面向车辆感知的预训练大模型，通过引入对称性、轮廓和语义三种结构化先验知识指导掩码重建，显著提升车辆感知表征学习能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在预训练阶段缺乏对车辆相关知识的有效学习，导致车辆感知表征建模能力不足，需要设计专门针对车辆特性的预训练框架。

Method: 1. 设计对称性引导掩码模块（SMM）利用车辆对称性选择高质量掩码区域；2. 轮廓引导表征模块（CRM）通过最小化轮廓特征与重建特征分布差异保持车辆结构信息；3. 语义引导表征模块（SRM）通过对比学习和跨模态蒸馏解决语义混淆问题；4. 构建包含400万车辆图像和1.2万文本描述的大规模数据集Autobot4M。

Result: 在五个下游任务上的实验表明，VehicleMAE-V2相比现有方法具有显著优越性能，验证了车辆结构化先验知识对提升表征学习效果的有效性。

Conclusion: 通过系统整合车辆特有的结构化先验知识，VehicleMAE-V2能够学习到更具泛化能力的车辆感知表征，为智能监控、交通管理和自动驾驶等应用提供了更强大的视觉基础模型。
...
### [44] [Block-Recurrent Dynamics in Vision Transformers](https://arxiv.org/abs/2512.19941)
*Mozes Jacobs,Thomas Fel,Richard Hakim,Alessandra Brondetta,Demba Ba,T. Andy Keller*

Main category: cs.CV

TL;DR: 本文提出块循环假设（BRH），认为训练后的视觉Transformer（ViT）具有块循环深度结构，可以用远少于原始块数的循环块来准确重写计算。通过Raptor模型验证了该假设，并利用该假设发展了动态可解释性分析程序。


<details>
  <summary>Details</summary>
Motivation: 随着视觉Transformer成为标准视觉骨干网络，需要对其计算现象学进行机制性解释。尽管架构线索暗示了动态结构，但缺乏将Transformer深度解释为特征化流的框架。

Method: 提出块循环假设（BRH）；通过层间表示相似性矩阵分析ViT的连续阶段；训练块循环代理模型Raptor来验证假设；利用该假设开发动态可解释性分析程序，包括分析方向收敛、令牌特定动态和低秩更新等现象。

Result: 小规模实验表明随机深度和训练促进循环结构；成功训练Raptor模型仅用2个块恢复DINOv2 ImageNet-1k线性探测96%的准确率；动态分析发现：i）类别依赖角度盆地的方向收敛，ii）令牌特定动态（cls令牌晚期重定向，补丁令牌晚期向均值方向强一致性），iii）后期深度更新低秩崩溃。

Conclusion: ViT深度中出现了紧凑的循环程序，指向低复杂度的规范解决方案，使这些模型能够通过原则性动力系统分析进行研究。
...
### [45] [SE360: Semantic Edit in 360$^\circ$ Panoramas via Hierarchical Data Construction](https://arxiv.org/abs/2512.19943)
*Haoyi Zhong,Fang-Lue Zhang,Andrew Chalmers,Taehyun Rhee*

Main category: cs.CV

TL;DR: 提出了SE360框架，用于360°全景图中的多条件引导对象编辑，通过自主数据生成管道和两阶段数据优化策略，实现了高质量的全景图编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像编辑方法在处理360°全景图时，在等距柱状投影和透视视图中常产生不合理结果，需要专门解决全景图编辑的挑战。

Method: 1. 提出粗到细的自主数据生成管道，利用视觉语言模型和自适应投影调整进行层次分析；2. 引入两阶段数据优化策略提升数据真实感；3. 基于构建的数据集训练基于Transformer的扩散模型。

Result: 实验表明，该方法在视觉质量和语义准确性方面优于现有方法，能够实现文本、掩码或参考图像引导的灵活对象编辑。

Conclusion: SE360框架通过创新的数据生成和优化方法，有效解决了360°全景图编辑的挑战，为全景图编辑提供了高质量解决方案。
...
### [46] [How Much 3D Do Video Foundation Models Encode?](https://arxiv.org/abs/2512.19949)
*Zixuan Huang,Xiang Li,Zhaoyang Lv,James M. Rehg*

Main category: cs.CV

TL;DR: 本文提出首个模型无关框架，通过浅层读出器从视频基础模型特征中估计3D属性，评估其3D理解能力。研究发现先进视频生成模型虽未经3D数据训练，却展现出超越专业3D模型的3D场景理解能力。


<details>
  <summary>Details</summary>
Motivation: 探究在大量视频数据上预训练后，视频基础模型是否能自然涌现出全局3D理解能力，并量化评估现有模型的3D感知水平。

Method: 设计模型无关评估框架，通过浅层读出器从不同视频基础模型的特征中估计多种3D属性（如深度、法线、光流等），系统比较各模型在3D理解任务上的表现。

Result: 1. 先进视频生成模型展现出对3D物体和场景的强理解能力，甚至超越专门训练的3D专家模型；2. 不同视频基础模型在3D感知维度上存在显著差异；3. 大规模视频预训练可有效促进3D理解能力的涌现。

Conclusion: 视频基础模型通过大规模预训练可获得超越专业模型的3D理解能力，该研究为构建可扩展的3D模型提供了重要基准和设计启示。
...
### [47] [HistoWAS: A Pathomics Framework for Large-Scale Feature-Wide Association Studies of Tissue Topology and Patient Outcomes](https://arxiv.org/abs/2512.19954)
...
TL;DR: 提出了HistoWAS框架，通过整合拓扑和空间特征，将组织空间结构与临床结果关联，应用于肾脏病理图像分析。


<details>
  <summary>Details</summary>
Motivation: 现有工具难以量化组织微环境的空间交互及其与临床参数的关系，限制了病理图像分析的临床相关性。

Method: 结合30个源自GIS的拓扑和空间特征，构建增强特征空间；采用类似PheWAS的关联研究引擎进行大规模单变量回归和统计校正。

Result: 在KPMP的385张肾脏病理图像上验证了框架，分析了102个特征，代码和数据已开源。

Conclusion: HistoWAS能有效关联组织空间组织与临床结果，为病理图像分析和生物标志物发现提供了新工具。
...
### [48] [Enhancing annotations for 5D apple pose estimation through 3D Gaussian Splatting (3DGS)](https://arxiv.org/abs/2512.20148)
*Robert van de Ven,Trim Bresilla,Bram Nelissen,Ard Nieuwenhuizen,Eldert J. van Henten,Gert Kootstra*

Main category: cs.CV

TL;DR: 提出一种基于3D高斯泼溅重建果园场景的新流程，通过简化标注和自动投影大幅减少苹果姿态估计所需的标注工作量，实验表明该方法能有效处理遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 果园环境复杂且遮挡严重，传统苹果姿态估计方法依赖关键点标注，但关键点（如花萼）常被遮挡，导致标注困难、耗时且存在冲突标注问题。

Method: 使用3D高斯泼溅重建果园场景，通过简化标注和自动投影将3D标注映射到图像，生成大规模训练数据集，并训练评估姿态估计模型。

Result: 仅需105次手动标注即可生成28,191个训练标签（减少99.6%）；使用遮挡≤95%的果实标签训练效果最佳，在原始图像和渲染图像上的F1分数分别为0.927和0.970；位置估计精度随遮挡增加而下降，但测试方法未能正确学习苹果方向估计。

Conclusion: 所提流程能显著降低标注成本并有效处理遮挡问题，但当前姿态估计方法在方向估计上存在局限，未来需改进方向学习能力。
...
### [49] [WSD-MIL: Window Scale Decay Multiple Instance Learning for Whole Slide Image Classification](https://arxiv.org/abs/2512.19982)
*Le Feng,Li Xiao*

Main category: cs.CV

TL;DR: 提出WSD-MIL方法，通过窗口尺度衰减注意力模块和区域门控模块，在降低计算成本的同时提升对多尺度肿瘤区域的建模能力，在病理图像分类任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多示例学习方法忽视全切片图像中实例间的复杂语义关系，Transformer方法因二次计算复杂度难以扩展至大规模WSI，且固定尺度注意力机制无法适应不同WSI中肿瘤区域尺度的显著差异。

Method: 1) 窗口尺度衰减注意力模块：采用聚类采样降低计算成本，通过渐进衰减注意力窗口尺度捕获多尺度局部实例关系；2) 基于压缩激励的区域门控模块：动态调整窗口权重以增强全局信息建模。

Result: 在CAMELYON16和TCGA-BRCA数据集上达到最先进性能，同时减少62%的计算内存消耗。

Conclusion: WSD-MIL能有效建模多尺度肿瘤区域并提升计算效率，为计算病理学中的多示例学习提供了更高效的解决方案。
...
### [50] [LEAD: Minimizing Learner-Expert Asymmetry in End-to-End Driving](https://arxiv.org/abs/2512.20563)
*Long Nguyen,Micha Fauth,Bernhard Jaeger,Daniel Dauner,Maximilian Igl,Andreas Geiger,Kashyap Chitta*

Main category: cs.CV

TL;DR: 本文研究了模拟器中专家演示与学生观察之间的不对齐如何限制模仿学习效果，提出了TransFuser v6策略，在CARLA基准测试中达到新SOTA，并在真实世界基准上表现一致提升。


<details>
  <summary>Details</summary>
Motivation: 模拟器能生成无限驾驶数据，但模仿学习策略在闭环性能上仍不鲁棒。研究发现专家（高可见性、低不确定性）与学生（传感器观测、导航意图不明确）之间的不对称性严重限制了驾驶性能。

Method: 1. 实证研究专家-学生不对齐问题；2. 提出实际干预措施缩小差距；3. 开发TransFuser v6学生策略；4. 在共享仿真到真实流程中集成感知监督。

Result: 1. TransFuser v6在CARLA所有主要闭环基准测试中达到新SOTA：Bench2Drive上95 DS，Longest6~v2和Town13性能翻倍；2. 在NAVSIM和Waymo视觉端到端驾驶基准上表现一致提升。

Conclusion: 通过缩小专家与学生之间的差距，可以有效提升模仿学习在自动驾驶中的性能。该方法在仿真和真实世界基准测试中均表现出色，代码、数据和模型已开源。
...
### [51] [A Novel CNN Gradient Boosting Ensemble for Guava Disease Detection](https://arxiv.org/abs/2512.19989)
*Tamim Ahasan Rijon,Yeasin Arafath*

Main category: cs.CV

TL;DR: 该研究提出了一种结合CNN与机器学习（CNN-ML）的集成模型，用于检测孟加拉国本地种植番石榴的疾病，在GFDD24数据集上实现了约99.99%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 番石榴是孟加拉国重要的热带水果，但炭疽病和果蝇感染会降低其品质和产量。早期疾病检测系统可减少损失、保护收成，从而支持该国经济发展。

Method: 使用来自孟加拉国Rajshahi和Pabna种植园的GFDD24数据集（包含健康、果蝇感染、炭疽病三类图像），构建CNN与传统机器学习结合的集成模型，特别是CNN-ML级联框架与梯度提升机（Gradient Boosting Machine）的组合。

Result: 提出的集成模型在番石榴疾病分类中达到了约99.99%的最高准确率，表现出强大的检测性能。

Conclusion: CNN-ML级联框架能够实现高精度的番石榴疾病检测，适用于实时农业监测系统，有助于提升孟加拉国番石榴种植的生产力和经济价值。
...
### [52] [A Dual-Branch Local-Global Framework for Cross-Resolution Land Cover Mapping](https://arxiv.org/abs/2512.19990)
*Peng Gao,Ke Li,Di Wang,Yongshan Zhu,Yiming Zhang,Xuemei Luo,Yifeng Wang*

Main category: cs.CV

TL;DR: 提出DDTM双分支弱监督框架，通过扩散分支细化局部语义和Transformer分支保持全局上下文一致性，解决跨分辨率土地覆盖制图中分辨率不匹配问题，在基准测试中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督方法在跨分辨率土地覆盖制图中难以对齐细粒度空间结构与粗粒度标签，导致噪声监督和制图精度下降，需要更有效的学习框架。

Method: 1. 设计双分支框架：扩散分支在粗监督下逐步细化局部语义，Transformer分支强制大空间范围内的长程上下文一致性；2. 引入伪标签置信度评估模块，选择性地利用可靠监督信号减轻噪声。

Result: 在Chesapeake Bay基准测试中达到66.52% mIoU，显著优于现有弱监督方法，建立了新的最优性能。

Conclusion: DDTM通过解耦局部语义细化和全局上下文推理，有效缓解了跨分辨率监督中的不匹配问题，为弱监督土地覆盖制图提供了更准确的解决方案。
...
### [53] [Few-Shot-Based Modular Image-to-Video Adapter for Diffusion Models](https://arxiv.org/abs/2512.20000)
...
TL;DR: 提出MIVA模块化适配器，解决扩散模型在图像动画中数据稀缺和泛化不足的问题，实现高效训练和精确运动控制。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像动画应用中面临训练数据稀缺导致记忆而非提示遵循、以及难以泛化到未见运动模式的问题，现有方法在有限数据下的微调研究不足。

Method: 设计轻量级模块化图像到视频适配器（MIVA），每个模块学习单一运动模式，可并行扩展，仅需约10个样本在消费级GPU上高效训练，推理时通过选择模块组合指定运动。

Result: 实验表明MIVA在保持或超越大数据集训练模型生成质量的同时，实现了更精确的运动控制，且训练效率显著提升。

Conclusion: MIVA通过模块化设计有效解决了扩散模型在图像动画中的泛化和数据依赖问题，为有限数据下的运动学习提供了高效解决方案。
...
### [54] [PaveSync: A Unified and Comprehensive Dataset for Pavement Distress Analysis and Classification](https://arxiv.org/abs/2512.20011)
*Blessing Agyei Kyem,Joshua Kofi Asamoah,Anthony Dontoh,Andrews Danyo,Eugene Denteh,Armstrong Aboah*

Main category: cs.CV

TL;DR: 提出了一个标准化的路面缺陷检测基准数据集，整合了多个公开数据集，包含52747张图像和135277个标注框，覆盖13种缺陷类型和7个国家，用于统一训练和评估。


<details>
  <summary>Details</summary>
Motivation: 现有路面缺陷检测数据集在标注风格、缺陷定义和格式上存在差异，导致模型难以泛化到多样化的真实场景，缺乏统一的训练和评估基准。

Method: 整合多个公开数据集，统一标注格式和缺陷类型定义，构建包含52747张图像、135277个标注框的标准化数据集，涵盖13种缺陷类型和7个国家的多样场景。使用YOLOv8-YOLOv12、Faster R-CNN和DETR等先进目标检测模型进行基准测试。

Result: 构建的数据集在多样场景下实现了竞争性性能，支持零样本迁移到新环境，为路面缺陷检测提供了首个全球代表性基准。

Conclusion: 该标准化数据集解决了现有数据集不一致的问题，为路面缺陷检测提供了公平的模型比较基准，促进了模型在真实场景中的泛化能力。
...
### [55] [SegEarth-R2: Towards Comprehensive Language-guided Segmentation for Remote Sensing Images](https://arxiv.org/abs/2512.20013)
*Zepeng Xin,Kaiyu Li,Luodi Chen,Wanchen Li,Yuchen Xiao,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.CV

TL;DR: 本文提出了LaSeRS数据集和SegEarth-R2模型，用于解决遥感图像中复杂语言指令引导的分割任务，填补了现有数据集过于简化、模型在真实场景中表现脆弱的空白。


<details>
  <summary>Details</summary>
Motivation: 现有模型只能处理简单的单目标指令，但在面对多粒度、多目标、需要推理和语言多样性的复杂地理空间场景时表现不佳，这限制了在灾害响应、环境监测等实际应用中的有效性。

Method: 构建了大规模数据集LaSeRS，涵盖分层粒度、目标多重性、推理需求和语言变异性四个关键维度；提出了SegEarth-R2多模态大语言模型架构，采用空间注意力监督机制处理小目标定位，以及灵活高效的分割查询机制处理单目标和多目标场景。

Result: SegEarth-R2在LaSeRS和其他基准测试中表现出色，为新一代地理空间分割建立了强大的基线。

Conclusion: LaSeRS数据集和SegEarth-R2模型共同推动了遥感图像中复杂语言引导分割的进展，通过更全面的训练和评估框架提升了模型在真实世界复杂场景中的鲁棒性和实用性。
...
### [56] [A Contextual Analysis of Driver-Facing and Dual-View Video Inputs for Distraction Detection in Naturalistic Driving Environments](https://arxiv.org/abs/2512.20025)
*Anthony Dontoh,Stephanie Ivey,Armstrong Aboah*

Main category: cs.CV

TL;DR: 该研究探讨了在分心驾驶检测中结合驾驶员视角和道路视角图像的效果，发现多视角输入的性能提升取决于模型架构设计，简单的视觉上下文添加可能产生干扰。


<details>
  <summary>Details</summary>
Motivation: 现有分心驾驶检测模型主要依赖驾驶员视角，忽略了影响驾驶行为的环境上下文信息，研究者希望验证结合道路视角能否提升自然驾驶条件下的检测准确率。

Method: 使用真实驾驶场景中的同步双摄像头数据，基于三种时空动作识别架构（SlowFast-R50、X3D-M、SlowOnly-R50）进行基准测试，比较单视角（驾驶员视角）和双视角（驾驶员+道路视角堆叠）两种输入配置。

Result: SlowOnly-R50模型在双视角输入下准确率提升9.8%，而SlowFast-R50模型因表征冲突导致准确率下降7.2%，表明性能提升高度依赖底层架构。

Conclusion: 简单地增加视觉上下文不足以改善检测效果，可能产生干扰；未来多模态驾驶员监控系统需要专门支持多视角融合的架构设计。
...
### [57] [MAPI-GNN: Multi-Activation Plane Interaction Graph Neural Network for Multimodal Medical Diagnosis](https://arxiv.org/abs/2512.20026)
*Ziwei Qin,Xuhui Song,Deqing Huang,Na Qin,Jun Li*

Main category: cs.CV

TL;DR: 提出MAPI-GNN框架，通过从语义解耦特征子空间学习多面图结构，克服传统单图方法在建模患者特异性病理关系上的局限，显著提升多模态医疗诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络在多模态医疗诊断中通常依赖单一静态图结构，该图基于无差别的特征构建，无法有效建模患者特定的病理关系，限制了诊断的准确性和个性化。

Method: 1. 使用多维判别器挖掘潜在的图感知模式；2. 基于这些模式动态构建一组激活图；3. 通过关系融合引擎聚合并上下文化多面图特征，实现鲁棒诊断。

Result: 在两个多样化任务（涵盖1300多个患者样本）上的实验表明，MAPI-GNN显著优于现有最先进方法。

Conclusion: MAPI-GNN通过重构单图范式为多面图学习，能够更有效地捕捉患者特异性病理关系，为多模态医疗诊断提供了更强大的图神经网络框架。
...
### [58] [$\text{H}^2$em: Learning Hierarchical Hyperbolic Embeddings for Compositional Zero-Shot Learning](https://arxiv.org/abs/2512.20029)
*Lin Li,Jiahui Li,Jiaming Lei,Jun Xiao,Feifei Shao,Long Chen*

Main category: cs.CV

TL;DR: 提出H2em框架，利用双曲几何学习层次化嵌入来解决组合零样本学习问题，在封闭和开放世界场景中均达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有组合零样本学习方法在欧几里得空间中建模层次结构时，无法适应现实世界大规模分类体系的需求——平坦几何的多项式体积增长无法匹配指数级结构，限制了泛化能力。

Method: 提出H2em框架：利用双曲几何嵌入树状结构的优势；设计双重层次蕴含损失（使用双曲蕴含锥强制预定义层次结构）和判别对齐损失（通过困难负样本挖掘增大语义相似组合间的测地距离）；开发双曲跨模态注意力机制实现双曲几何内的实例感知跨模态融合。

Result: 在三个基准测试上的广泛消融实验表明，H2em在封闭世界和开放世界场景中均建立了新的最先进性能。

Conclusion: 双曲几何能有效建模组合零样本学习中的层次结构，H2em框架通过精心设计的损失函数和注意力机制解决了层次塌缩和细粒度区分问题，显著提升了泛化能力。
...
### [59] [VALLR-Pin: Dual-Decoding Visual Speech Recognition for Mandarin with Pinyin-Guided LLM Refinement](https://arxiv.org/abs/2512.20032)
*Chang Sun,Dongliang Xie,Bo Qin,Hong Yang*

Main category: cs.CV

TL;DR: 提出VALLR-Pin框架，通过双解码器联合预测汉字与拼音，并利用大语言模型结合拼音上下文纠正同音字错误，提升中文唇语识别性能。


<details>
  <summary>Details</summary>
Motivation: 中文唇语识别面临视觉音素高度模糊和同音字普遍存在的挑战，需要结合语音和语义信息解决歧义问题。

Method: 两阶段框架：1）共享视频编码器连接双解码器，同时预测汉字序列和拼音罗马化；2）推理时用拼音和候选文本构建提示，输入大语言模型进行纠错；3）使用合成噪声数据微调大语言模型以适应模型错误模式。

Result: 通过多任务学习增强视觉语义表示，利用拼音上下文有效纠正同音字错误，提升中文唇语识别的准确性。

Conclusion: VALLR-Pin融合视觉特征、语音信息和语言上下文，通过大语言模型纠错机制显著改善中文唇语识别性能。
...
### [60] [Beyond Vision: Contextually Enriched Image Captioning with Multi-Modal Retrieva](https://arxiv.org/abs/2512.20042)
*Nguyen Lam Phu Quy,Pham Phu Hoa,Tran Chi Nguyen,Dao Sy Duy Minh,Nguyen Hoang Minh Ngoc,Huynh Trung Kiet*

Main category: cs.CV

TL;DR: 提出一个多模态管道，通过外部文本知识增强图像描述，生成更丰富、上下文感知的标题，在OpenEvents v1数据集上表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界图像描述常缺乏上下文深度，如事件背景、时间线索、结果和不可见命名实体，限制了在新闻、教育和数字档案等领域的应用效果。

Method: 使用BEIT-3和SigLIP检索语义相似图像，ORB和SIFT重排几何对齐，语义搜索提取相关文章上下文，结合Instruct BLIP生成的基础描述，通过微调Qwen3模型（QLoRA）集成生成增强描述。

Result: 在OpenEvents v1数据集上，该方法生成的信息更丰富的描述显著优于传统方法。

Conclusion: 该方法在需要深度视觉-文本理解的实际应用中具有强潜力，能有效提升图像描述的上下文丰富性。
...
### [61] [FlashLips: 100-FPS Mask-Free Latent Lip-Sync using Reconstruction Instead of Diffusion or GANs](https://arxiv.org/abs/2512.20033)
*Andreas Zinonos,Michał Stypułkowski,Antoni Bigata,Stavros Petridis,Maja Pantic,Nikita Drobyshev*

Main category: cs.CV

TL;DR: FlashLips是一个两阶段、无需掩码的唇形同步系统，通过解耦唇部控制与渲染实现实时性能（>100 FPS），视觉质量媲美大型SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 现有唇形同步系统通常依赖复杂的生成模型（如GANs或扩散模型），导致计算开销大、推理速度慢。需要一种既能保持高质量视觉效果，又能实现实时性能的轻量级解决方案。

Method: 1. 第一阶段：基于潜在空间的单步图像编辑器，使用参考身份、掩码目标帧和低维唇部姿态向量进行重建，仅用重建损失训练。通过自监督生成嘴部变体作为伪真值微调，实现无掩码推理。2. 第二阶段：基于流匹配目标的音频-姿态变换器，从语音预测唇部姿态向量。两阶段结合形成确定性重建与音频控制的稳定流程。

Result: 系统在单GPU上实现超过100 FPS的实时性能，视觉质量与大型SOTA模型相当，同时保持简单稳定的训练流程，无需GANs或扩散模型。

Conclusion: FlashLips通过解耦唇部控制与渲染、结合自监督微调和流匹配音频驱动，实现了高质量、实时、无需掩码的唇形同步，为轻量级实时应用提供了有效方案。
...
### [62] [Item Region-based Style Classification Network (IRSN): A Fashion Style Classifier Based on Domain Knowledge of Fashion Experts](https://arxiv.org/abs/2512.20088)
*Jinyoung Choi,Youngchae Kwon,Injung Kim*

Main category: cs.CV

TL;DR: 提出IRSN网络，通过分析单品特征及其组合来改进时尚风格分类，在多个数据集上显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 时尚风格分类因同类风格视觉差异大、不同风格视觉相似而具有挑战性，需要同时考虑全局外观、单品属性及其组合。

Method: 使用单品区域池化提取各区域特征，通过门控特征融合分析组合特征，并采用结合领域专用特征提取器和通用预训练提取器的双主干架构。

Result: 在六个主流骨干网络上应用IRSN，在FashionStyle14和ShowniqV3数据集上平均提升准确率6.9%/7.6%，最高提升14.5%/15.1%。可视化分析证实模型能更好区分相似风格。

Conclusion: IRSN通过有效利用单品级特征及其组合，显著提升了时尚风格分类性能，双主干架构进一步增强了特征提取能力。
...
### [63] [LiDARDraft: Generating LiDAR Point Cloud from Versatile Inputs](https://arxiv.org/abs/2512.20105)
*Haiyun Wei,Fan Lu,Yunwei Zhu,Zehan Zheng,Weiyi Xue,Lin Shao,Xudong Zhang,Ya Wu,Rong Fu,Guang Chen*

Main category: cs.CV

TL;DR: 提出LiDARDraft方法，通过3D布局桥接多种控制信号与LiDAR点云，实现高质量、可控的自动驾驶仿真点云生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在复杂LiDAR点云分布与简单控制信号之间取得平衡，无法同时实现高质量生成与多样化控制。

Method: 将文本、图像和点云统一表示为3D布局，转换为语义和深度控制信号，采用基于rangemap的ControlNet引导点云生成。

Result: 像素级对齐方法在可控LiDAR点云生成中表现优异，支持从任意文本描述、图像和草图创建自动驾驶环境。

Conclusion: LiDARDraft实现了“从零开始仿真”，为自动驾驶仿真提供了灵活高效的点云生成方案。
...
### [64] [Effect of Activation Function and Model Optimizer on the Performance of Human Activity Recognition System Using Various Deep Learning Models](https://arxiv.org/abs/2512.20104)
...
TL;DR: 本文研究了激活函数与优化器组合对基于深度学习的HAR系统性能的影响，发现ConvLSTM结合Adam或RMSprop在医疗相关活动识别中表现最佳，准确率可达99%。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注HAR系统架构设计，而激活函数与优化器的组合选择及其对模型性能的影响尚未得到充分分析，尤其在医疗等实际应用场景中缺乏系统评估。

Method: 使用BiLSTM和ConvLSTM两种循环架构，测试ReLU、Sigmoid和Tanh三种激活函数与SGD、Adam、RMSprop、Adagrad四种优化器的组合，在HMDB51和UCF101数据集的六个医疗相关活动类别上进行实验。

Result: ConvLSTM整体优于BiLSTM，结合Adam或RMSprop时准确率最高达99%；BiLSTM在UCF101上准确率约98%，但在HMDB51上降至60%，表现不稳定且对激活函数与优化器变化敏感度较低。

Conclusion: ConvLSTM具有更强的时空学习能力和稳定性，适合医疗等需要快速精准识别的实际HAR应用；激活函数与优化器的组合选择对模型性能有显著影响，为优化实际HAR系统提供了实用指导。
...
### [65] [UMAMI: Unifying Masked Autoregressive Models and Deterministic Rendering for View Synthesis](https://arxiv.org/abs/2512.20107)
*Thanh-Tung Le,Tuan Pham,Tung Nguyen,Deying Kong,Xiaohui Xie,Stephan Mandt*

Main category: cs.CV

TL;DR: 提出了一种结合确定性渲染和随机扩散的混合框架，用于高效高质量的新视角合成，在保持图像质量的同时大幅减少渲染时间。


<details>
  <summary>Details</summary>
Motivation: 现有确定性网络渲染速度快但未观测区域模糊，而基于扩散的随机方法能生成合理内容但计算成本高昂，需要结合两者优势。

Method: 使用双向Transformer编码多视图图像标记和Plucker射线嵌入，生成共享潜在表示；采用两个轻量级头：前馈回归头渲染几何约束良好的像素，掩码自回归扩散头补全遮挡或未观测区域；端到端训练，结合光度损失和扩散损失。

Result: 实验表明，该方法在图像质量上达到最先进水平，同时相比完全生成式基线将渲染时间减少了一个数量级。

Conclusion: 所提出的混合框架有效统一了确定性和随机方法的优势，实现了高效且高质量的新视角合成，无需手工3D归纳偏置，具有良好的可扩展性。
...
TL;DR: 提出基于自然语言的文档图像检索新基准NL-DIR，包含4.1万真实文档图像及细粒度语义查询，评估现有视觉语言模型性能并探索两阶段检索方法。


<details>
  <summary>Details</summary>
Motivation: 现有文档图像检索方法主要依赖图像查询，难以处理现实场景中常见的细粒度文本查询需求，需要建立更符合实际应用的评估基准。

Method: 构建包含4.1万真实文档图像的数据集，每张图像配5个由大语言模型生成并经人工验证的细粒度语义查询；对主流对比视觉语言模型和OCR-free视觉文档理解模型进行零样本和微调评估；探索兼顾时空效率的两阶段检索方法。

Result: 建立了首个自然语言驱动的文档图像检索基准，揭示了现有模型在细粒度语义检索上的局限性，两阶段检索方法在性能与效率间取得平衡。

Conclusion: NL-DIR基准为视觉文档理解社区提供了新的研究机遇，推动文档检索向自然语言交互方向发展，数据集和代码已开源。
...
TL;DR: 提出DDAVS框架，通过解耦音频语义和延迟双向对齐机制，解决音频-视觉分割中的多源纠缠和视听错位问题，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉分割方法存在多源纠缠和视听错位问题，导致模型偏向响度大或体积大的物体，而忽略较弱、较小或共现的声源。

Method: 1. 使用可学习查询从音频原型记忆库提取音频语义并锚定在结构化语义空间中；2. 通过对比学习增强区分性和鲁棒性；3. 引入延迟模态交互的双重交叉注意力机制改善多模态对齐。

Result: 在AVS-Objects和VPO基准测试中，DDAVS在单源、多源和多实例场景下均优于现有方法，展现出优异的泛化能力。

Conclusion: DDAVS框架通过解耦音频语义和延迟对齐机制，有效提升了复杂真实场景下音频-视觉分割的准确性和鲁棒性。
...
TL;DR: 提出milliMamba框架，利用毫米波雷达进行2D人体姿态估计，通过联合建模时空依赖关系解决雷达信号稀疏性问题，在多个数据集上显著超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达具有保护隐私和光照不变性的优势，但其信号常因镜面反射而稀疏，导致特征提取困难，需要开发能有效处理稀疏信号并建模时空依赖的方法。

Method: 采用Cross-View Fusion Mamba编码器高效提取长序列时空特征（线性复杂度），结合Spatio-Temporal-Cross Attention解码器预测多帧关节点坐标，训练时引入速度损失以增强运动平滑性。

Result: 在TransHuPR和HuPR数据集上分别超过基线11.0 AP和14.6 AP，在保持合理复杂度的同时实现显著性能提升。

Conclusion: milliMamba通过联合时空建模有效利用相邻帧和关节的上下文信息推断缺失关节，为雷达基姿态估计提供了高效解决方案，代码已开源。
...
### [69] [${D}^{3}${ETOR}: ${D}$ebate-Enhanced Pseudo Labeling and Frequency-Aware Progressive ${D}$ebiasing for Weakly-Supervised Camouflaged Object ${D}$etection with Scribble Annotations](https://arxiv.org/abs/2512.20260)
*Jiawei Ge,Jiuxin Cao,Xinyi Li,Xuelin Zhu,Chang Liu,Bo Liu,Chen Feng,Ioannis Patras*

Main category: cs.CV

TL;DR: 提出D³ETOR框架解决弱监督伪装目标检测问题，通过两阶段方法提升伪标签质量和缓解标注偏差，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督伪装目标检测方法因两个主要限制落后于全监督方法：1) 通用分割模型生成的伪标签不可靠；2) 涂鸦标注的固有偏差阻碍模型捕捉全局结构。

Method: 两阶段框架：1) 辩论增强伪标签生成，采用自适应熵驱动点采样和多智能体辩论机制增强SAM模型；2) 频率感知渐进去偏，设计FADeNet融合多级频率特征并动态调整监督权重。

Result: D³ETOR显著缩小了弱监督与全监督COD之间的性能差距，在多个基准测试中实现了最先进的性能。

Conclusion: 通过联合利用伪标签和涂鸦语义的监督信号，提出的两阶段框架有效解决了弱监督伪装目标检测中的关键挑战，为弱监督分割任务提供了新思路。
...
### [70] [UbiQVision: Quantifying Uncertainty in XAI for Image Recognition](https://arxiv.org/abs/2512.20288)
*Akshat Dubey,Aleksandar Anžel,Bahar İlgen,Georges Hattab*

Main category: cs.CV

TL;DR: 本研究提出一种结合Dirichlet后验采样和Dempster-Shafer理论的框架，用于量化医学影像中SHAP解释的不确定性，并在三类医学影像数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学影像领域应用广泛，但复杂模型（如ResNet、Vision Transformer）的可解释性不足。SHAP虽能提供可视化解释，但在存在认知和随机不确定性时，其解释可能不稳定且不可靠。

Method: 使用Dirichlet后验采样和Dempster-Shafer理论，通过信念图、合理图和融合图方法，结合统计定量分析，量化SHAP解释中的不确定性。

Result: 框架在病理学、眼科和放射学三类不同类别分布、图像质量和模态的医学影像数据集上进行了评估，能够有效处理由图像分辨率和模态差异引入的显著认知不确定性。

Conclusion: 所提出的框架能够可靠地量化SHAP解释中的不确定性，增强深度学习模型在医学影像应用中的可解释性和可信度。
...
### [71] [CoDi -- an exemplar-conditioned diffusion model for low-shot counting](https://arxiv.org/abs/2512.20153)
*Grega Šuštar,Jer Pelhan,Alan Lukežič,Matej Kristan*

Main category: cs.CV

TL;DR: 提出CoDi，首个基于潜在扩散模型的少样本物体计数方法，通过新的示例条件模块生成高质量密度图，在FSC和MCAC基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有少样本计数方法在密集小物体场景中存在局限：基于密度的方法定位能力差，而基于点检测的方法因预训练查询数量有限，在物体数量极大时性能下降，需采用上采样和分块等临时技术。

Method: 使用潜在扩散模型生成高质量密度图，核心创新是示例条件模块，该模块提取物体原型并调整到去噪网络的中间层，实现精确的物体位置估计。

Result: 在FSC基准测试中，少样本、单样本和无参考场景下分别以15%、13%和10%的MAE优势超越现有最佳方法；在MCAC基准测试中以44%的MAE优势刷新记录。

Conclusion: CoDi通过扩散模型和示例条件模块的结合，有效解决了密集小物体场景下的少样本计数问题，在多个基准测试中实现了新的最优性能。
...
### [72] [JDPNet: A Network Based on Joint Degradation Processing for Underwater Image Enhancement](https://arxiv.org/abs/2512.20213)
*Tao Ye,Hongbin Ren,Chongbing Zhang,Haoran Chen,Xiaosong Li*

Main category: cs.CV

TL;DR: 提出JDPNet网络，通过联合特征挖掘模块和概率引导分布策略处理水下图像中非线性耦合的多种退化问题，在多个数据集上取得先进性能。


<details>
  <summary>Details</summary>
Motivation: 水下图像存在多种非线性耦合的退化现象，现有方法多针对单一退化设计，难以有效处理耦合退化中的潜在信息。

Method: 提出JDPNet网络，包含联合特征挖掘模块、概率引导分布策略和新型AquaBalanceLoss损失函数，用于挖掘和统一调整耦合退化特征。

Result: 在六个公开数据集和两个新建数据集上验证，JDPNet在性能、参数量和计算成本之间取得更好平衡，达到先进水平。

Conclusion: JDPNet能有效处理水下图像中的耦合退化问题，为水下图像处理提供了更高效的解决方案。
...
### [73] [DETACH : Decomposed Spatio-Temporal Alignment for Exocentric Video and Ambient Sensors with Staged Learning](https://arxiv.org/abs/2512.20409)
*Junho Yoon,Jaemo Jung,Hyunju Kim,Dongman Lee*

Main category: cs.CV

TL;DR: 提出DETACH框架，通过分解时空特征解决外中心视频与环境传感器对齐问题，提升动作识别性能


<details>
  <summary>Details</summary>
Motivation: 现有自我中心视频与可穿戴传感器对齐方法存在用户不适、隐私问题和可扩展性限制，需要探索非侵入式的外中心视频与环境传感器替代方案

Method: 提出DETACH分解时空框架：1) 通过在线聚类发现传感器-空间特征；2) 两阶段对齐方法：先通过相互监督建立空间对应，再使用时空加权对比损失进行时间对齐

Result: 在Opportunity++和HWU-USP数据集的下游任务实验中，相比适应的自我中心-可穿戴基线方法有显著提升

Conclusion: DETACH框架通过分解时空特征和上下文感知对齐，有效解决了外中心-环境设置中的局部细节捕捉和时空模式误对齐问题
...
### [74] [HEART-VIT: Hessian-Guided Efficient Dynamic Attention and Token Pruning in Vision Transformer](https://arxiv.org/abs/2512.20120)
*Mohammad Helal Uddin,Liam Seymour,Sabur Baidya*

Main category: cs.CV

TL;DR: HEART-ViT是一个基于Hessian引导的高效动态注意力与令牌剪枝框架，首次统一了视觉Transformer中令牌和注意力头的二阶输入自适应剪枝，在显著降低计算开销的同时保持或提升模型精度。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer（ViT）虽然精度领先，但其二次注意力计算成本和冗余操作严重阻碍了在延迟和资源受限平台上的部署。现有剪枝方法通常孤立处理令牌或注意力头，依赖启发式或一阶信号，往往牺牲精度或无法泛化到不同输入。

Method: 提出HEART-ViT框架，通过高效的Hessian-向量积估计令牌和注意力头的曲率加权敏感度，在显式损失预算下实现原则性剪枝决策。该框架统一处理令牌剪枝（主导计算节省）和注意力头剪枝（提供细粒度冗余消除），并结合两者实现更优权衡。

Result: 在ImageNet-100和ImageNet-1K数据集上，使用ViT-B/16和DeiT-B/16模型，HEART-ViT实现了高达49.4%的FLOPs减少、36%的延迟降低和46%的吞吐量提升，同时在微调后保持或超越基线精度（例如在40%令牌剪枝时恢复4.7%精度）。在AGX Orin等边缘设备上的部署验证了计算和延迟减少直接转化为实际推理速度和能效提升。

Conclusion: HEART-ViT弥合了理论与实践的差距，提供了首个统一、曲率驱动的剪枝框架，既保持精度又适合边缘部署，为视觉Transformer的高效优化提供了新方向。
...
### [75] [AMoE: Agglomerative Mixture-of-Experts Vision Foundation Model](https://arxiv.org/abs/2512.20157)
...
TL;DR: 本文系统研究了视觉基础模型的多教师蒸馏方法，提出了AMoE框架，结合非对称关系知识蒸馏、令牌平衡批处理和分层聚类采样，显著提升了训练效率和表示质量，并发布了OpenLVD200M数据集和蒸馏模型。


<details>
  <summary>Details</summary>
Motivation: 多教师蒸馏方法在构建统一视觉表示方面具有潜力，但其学习动态和数据效率尚未得到充分探索。本研究旨在降低训练成本，提高知识蒸馏的效率与效果。

Method: 提出AMoE框架，同时从SigLIP2和DINOv3教师模型蒸馏知识到混合专家学生模型；采用非对称关系知识蒸馏损失保持教师模型的几何特性；使用令牌平衡批处理统一不同分辨率图像的训练；通过分层聚类和采样提升数据效率。

Result: 结合上述方法，构建了OpenLVD200M图像语料库，在多教师蒸馏中表现出更高的效率；蒸馏得到的模型在保持性能的同时显著降低了计算成本。

Conclusion: 多教师蒸馏可通过优化损失函数、批处理策略和数据采样方法实现高效训练；AMoE框架和OpenLVD200M数据集为视觉基础模型的蒸馏提供了有效解决方案。
...
### [76] [LiteFusion: Taming 3D Object Detectors from Vision-Based to Multi-Modal with Minimal Adaptation](https://arxiv.org/abs/2512.20217)
*Xiangxuan Ren,Zhongdao Wang,Pin Tang,Guoqing Wang,Jilai Zheng,Chao Ma*

Main category: cs.CV

TL;DR: 提出LiteFusion多模态3D检测器，将激光雷达数据作为几何信息补充增强相机检测，无需3D主干网络，提升部署友好性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态3D检测器过度依赖激光雷达，在激光雷达缺失时性能骤降，且依赖GPU优化的3D稀疏卷积算子，难以部署到NPU/FPGA等多样化硬件平台。

Method: 重新思考激光雷达在相机-激光雷达融合中的作用，将激光雷达点云作为几何信息补充而非独立模态；在四元数空间中融合激光雷达与图像特征，保持正交约束以建模跨模态关系。

Result: 在nuScenes数据集上，LiteFusion将基线视觉检测器的mAP提升20.4%，NDS提升19.7%，参数仅增加1.1%；即使激光雷达输入缺失仍保持较强性能。

Conclusion: LiteFusion通过轻量化跨模态融合设计，在保持高精度的同时显著提升部署灵活性和系统鲁棒性，适用于实际自动驾驶场景。
...
### [77] [IndicDLP: A Foundational Dataset for Multi-Lingual and Multi-Domain Document Layout Parsing](https://arxiv.org/abs/2512.20236)
*Oikantik Nath,Sahithi Kukkala,Mitesh Khapra,Ravi Kiran Sarvadevabhatla*

Main category: cs.CV

TL;DR: 提出了IndicDLP，一个大规模多语言文档布局数据集，涵盖11种印度语言和英语，以解决现有数据集在细粒度标注和多语言多样性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有大规模数据集（如PubLayNet、DocBank）缺乏细粒度区域标签和多语言多样性，而人工标注数据集（如M6Doc、D4LA）规模太小且多语言覆盖不足，特别是印度语言文档在现有数据集中代表性不足，限制了文档布局分析的发展。

Method: 1. 构建IndicDLP数据集，涵盖11种代表性印度语言和英语，覆盖12个常见文档领域；2. 从DocLayNet和M6Doc中整理UED-mini数据集用于预训练；3. 通过在IndicDLP上微调现有英语模型进行实验验证。

Result: 实验表明，在IndicDLP上微调现有英语模型能显著提升性能，验证了数据集的有效性；同时，在IndicDLP上训练的模型能很好地泛化到非印度语言文档布局。

Conclusion: IndicDLP填补了文档布局数据集在规模、多样性和标注粒度方面的空白，为文档数字化提供了宝贵资源，推动了包容且高效的文档理解研究。
...
### [78] [BiCoR-Seg: Bidirectional Co-Refinement Framework for High-Resolution Remote Sensing Image Segmentation](https://arxiv.org/abs/2512.20255)
*Jinghao Shi,Jianing Song*

Main category: cs.CV

TL;DR: 提出BiCoR-Seg框架，通过热图驱动的双向信息协同模块和分层监督策略，解决高分辨率遥感图像语义分割中类间相似性高、类内差异大的问题，在多个数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像语义分割长期面临类间相似性高、类内差异大的挑战，现有方法难以将抽象且具有强判别性的语义知识有效注入像素级特征学习，导致复杂场景中边界模糊和类别混淆。

Method: 1. 设计热图驱动的双向信息协同模块（HBIS），通过生成类别级热图建立特征图与类别嵌入之间的双向信息流；2. 引入分层监督策略，将每个HBIS模块生成的可解释热图直接用作低分辨率分割预测进行监督；3. 提出跨层类别嵌入Fisher判别损失，增强嵌入表示的判别性。

Result: 在LoveDA、Vaihingen和Potsdam数据集上的大量实验表明，BiCoR-Seg实现了出色的分割性能，同时提供了更强的可解释性。

Conclusion: BiCoR-Seg框架通过双向协同精炼机制有效提升了高分辨率遥感图像语义分割的精度和可解释性，为解决类间相似和类内差异问题提供了新思路。
...
### [79] [Linking Faces and Voices Across Languages: Insights from the FAME 2026 Challenge](https://arxiv.org/abs/2512.20376)
...
TL;DR: 本文介绍了ICASSP 2026的FAME挑战赛，该赛事关注多语言环境下的人脸-语音关联技术，旨在解决训练和测试语言不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 全球超过一半人口使用双语或多语，实际交流常涉及多语言场景，因此需要开发在训练和测试语言不同的情况下仍有效的人脸-语音关联方法。

Method: 通过举办FAME挑战赛，征集并评估不同团队提出的多语言人脸-语音关联算法，重点关注跨语言泛化能力。

Result: 报告提供了挑战赛的简要总结，包括赛事目标、数据集设置和评估标准，但未具体列出各参赛方法的结果。

Conclusion: FAME挑战赛旨在推动多语言环境下人脸-语音关联技术的研究，为解决实际应用中的语言不匹配问题提供平台和方向。
...
### [80] [LADLE-MM: Limited Annotation based Detector with Learned Ensembles for Multimodal Misinformation](https://arxiv.org/abs/2512.20257)
*Daniele Cardullo,Simone Teglia,Irene Amerini*

Main category: cs.CV

TL;DR: 提出LADLE-MM模型，一种在有限标注数据和计算资源下工作的多模态虚假信息检测器，通过集成单模态分支和多模态分支，在减少参数的同时达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 随着多媒体生成工具普及，跨模态合成的虚假内容威胁加剧，现有检测方法通常依赖计算密集型架构或大量标注数据，难以在资源受限场景下应用。

Method: 设计三分支架构：两个单模态分支（图像、文本）和一个多模态分支，利用BLIP提取的多模态嵌入作为固定参考空间增强表征；采用模型汤初始化策略，在有限标注设置下训练。

Result: 在DGM4基准测试中，参数减少60.3%仍实现竞争性二分类和多标签分类性能；在VERITE数据集上超越使用大型视觉语言模型的现有方法，展现开放集泛化能力和抗单模态偏差鲁棒性。

Conclusion: LADLE-MM证明了在有限标注和计算资源下，通过高效架构设计可实现有效的多模态虚假信息检测，为实际部署提供了可行方案。
...
### [81] [SmartSplat: Feature-Smart Gaussians for Scalable Compression of Ultra-High-Resolution Images](https://arxiv.org/abs/2512.20377)
*Linfei Li,Lin Zhang,Zhong Wang,Ying Shen*

Main category: cs.CV

TL;DR: 提出SmartSplat框架，利用梯度-颜色引导的变分采样和尺度自适应高斯颜色采样，在超高分辨率图像压缩中平衡压缩比与重建质量。


<details>
  <summary>Details</summary>
Motivation: 生成式AI快速发展导致超高分辨率视觉内容激增，现有2D高斯图像模型在压缩比与重建保真度之间难以平衡，需要更高效的压缩方案。

Method: 采用梯度-颜色引导变分采样与基于排除的均匀采样相结合的策略，提出尺度自适应高斯颜色采样方法，通过联合优化空间布局、尺度和颜色初始化实现高效压缩。

Result: 在DIV8K和新建16K数据集上的实验表明，SmartSplat在相同压缩比下优于现有方法，并能突破其压缩极限，展现出强扩展性和实用价值。

Conclusion: SmartSplat通过特征感知的自适应高斯表示，有效解决了超高分辨率图像压缩中效率与质量的平衡问题，为实时解码应用提供了可行方案。
...
### [82] [The devil is in the details: Enhancing Video Virtual Try-On via Keyframe-Driven Details Injection](https://arxiv.org/abs/2512.20340)
...
TL;DR: 提出KeyTailor框架和ViT-HD数据集，通过关键帧驱动的细节注入策略解决视频虚拟试穿中服装动态细节和背景一致性问题，同时避免扩散变换器架构的额外修改。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散变换器的视频虚拟试穿方法难以捕捉细粒度服装动态、保持背景一致性，且因引入额外交互模块导致计算成本高；现有公开数据集规模和质量有限也限制了模型泛化能力。

Method: 1. 提出指令引导的关键帧采样策略筛选信息帧；2. 设计两个关键帧驱动模块：服装细节增强模块提取服装动态特征，协作背景优化模块优化背景一致性；3. 将增强细节与姿态、掩码、噪声潜在变量共同注入标准扩散变换器块；4. 构建包含15,070个高清视频样本的ViT-HD数据集。

Result: 实验表明KeyTailor在动态和静态场景下，在服装保真度和背景完整性方面均优于现有先进基线方法。

Conclusion: KeyTailor通过关键帧驱动的细节注入策略，在不修改扩散变换器架构的前提下，实现了高效且真实的视频虚拟试穿合成，同时ViT-HD数据集为模型训练提供了高质量数据支持。
...
### [83] [Chain-of-Anomaly Thoughts with Large Vision-Language Models](https://arxiv.org/abs/2512.20417)
*Pedro Domingos,João Pereira,Vasco Lopes,João Neves,David Semedo*

Main category: cs.CV

TL;DR: 提出Chain-of-Anomaly-Thoughts（CoAT）多智能体推理框架，通过引入异常分类层解决大视觉语言模型在视频监控中的犯罪检测偏差问题，显著提升异常检测和分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型在自动化视频监控中存在对正常场景的固有偏差，难以有效检测犯罪；同时链式思维推理缺乏异常归纳偏差，进一步导致模型偏向正常解释。

Method: 提出Chain-of-Anomaly-Thoughts（CoAT）多智能体推理框架，在推理过程中通过最终的异常聚焦分类层引入归纳性犯罪偏差。

Result: 在低分辨率视频中异常检测F1分数提升11.8个百分点，在高分辨率视频中异常分类性能提升3.78个百分点。

Conclusion: CoAT框架通过引入异常归纳偏差有效解决了大视觉语言模型在犯罪检测中的偏差问题，显著提升了视频监控中的异常检测与分类能力。
...
### [84] [CRAFT: Continuous Reasoning and Agentic Feedback Tuning for Multimodal Text-to-Image Generation](https://arxiv.org/abs/2512.20362)
*V. Kovalev,A. Kuvshinov,A. Buzovkin,D. Pokidov,D. Timonin*

Main category: cs.CV

TL;DR: 本文提出了CRAFT框架，一种无需训练、模型无关的推理时结构化推理方法，通过将提示分解为依赖结构化的视觉问题、使用视觉语言模型验证生成图像、并通过LLM代理仅在约束失败时进行针对性提示编辑，迭代改进文本到图像生成的质量和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有推理时推理和反思方法通常依赖隐式、整体的批评或无约束的提示重写，导致其行为难以解释、控制或可靠停止。而大型语言模型已从基于验证、针对性纠正和提前停止的显式结构化思维中受益，因此希望将这种结构化推理范式引入多模态图像生成。

Method: CRAFT框架包含三个核心步骤：1) 将提示分解为依赖结构化的视觉问题；2) 使用视觉语言模型验证生成图像是否满足约束；3) 通过LLM代理仅在约束失败时进行针对性提示编辑。该过程迭代进行，并在所有约束满足时基于明确停止标准终止。

Result: 在多个模型家族和具有挑战性的基准测试中，CRAFT一致提高了组合准确性、文本渲染和基于偏好的评估，特别是在轻量级生成器上表现出显著增益。这些改进仅带来可忽略的推理时间开销，使较小或较便宜的模型能够接近更昂贵系统的质量。

Conclusion: 显式结构化、约束驱动的推理时推理是提高多模态生成模型可靠性的关键因素。CRAFT框架提供了一种可解释、可控的推理时细化循环，能够在不重新训练的情况下显著提升文本到图像生成的性能。
...
### [85] [Skin Lesion Classification Using a Soft Voting Ensemble of Convolutional Neural Networks](https://arxiv.org/abs/2512.20431)
*Abdullah Al Shafi,Abdul Muntakim,Pintu Chandra Shill,Rowzatul Zannat,Abdullah Al-Amin*

Main category: cs.CV

TL;DR: 提出一种基于CNN软投票集成和混合双编码器分割的早期皮肤癌分类方法，在三个基准数据集上取得高准确率。


<details>
  <summary>Details</summary>
Motivation: 皮肤癌早期检测对提高生存率至关重要，但传统诊断方法存在局限性。人工智能技术，特别是基于标注皮肤图像和卷积神经网络的系统，能够提升诊断准确性，因此研究高效准确的自动分类方法具有重要临床意义。

Method: 采用三个基准数据集（HAM10000、ISIC 2016、ISIC 2019），通过重平衡、图像增强和滤波技术预处理数据。使用混合双编码器进行迁移学习分割，聚焦临床相关特征。分类阶段采用MobileNetV2、VGG19和InceptionV3的软投票集成模型，平衡准确率与速度。

Result: 在三个数据集上的病灶识别准确率分别为96.32%、90.86%和93.92%。系统在皮肤病灶检测标准指标评估中表现优异，证明了方法的有效性。

Conclusion: 所提出的集成CNN方法结合精确分割技术，能有效提升皮肤癌早期分类的准确性，减少背景干扰，且兼顾计算效率，具备实际部署潜力，为临床辅助诊断提供了可靠工具。
...
### [86] [High Dimensional Data Decomposition for Anomaly Detection of Textured Images](https://arxiv.org/abs/2512.20432)
*Ji Song,Xing Wang,Jianguo Wu,Xiaowei Yue*

Main category: cs.CV

TL;DR: 提出一种纹理基集成平滑分解方法，用于纹理图像的高效异常检测，相比传统方法具有更低的误识别率、更小的训练数据需求和更好的检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统异常检测方法在处理纹理缺陷图像时存在误识别不可忽视、鲁棒性低、过度依赖大规模结构化数据集等局限性，需要更高效的纹理图像异常检测技术。

Method: 提出纹理基集成平滑分解方法，首先研究准周期性的数学公式和理论特性以估计图像纹理，然后通过两个主要过程：学习纹理基函数以提取准周期纹理模式，随后利用纹理基作为先验知识进行异常检测。

Result: 该方法在仿真和真实数据集上均超越了基准方法，表现出更少的误识别、更小的训练数据集需求以及更优的异常检测性能。

Conclusion: 纹理基集成平滑分解方法能够有效解决纹理图像异常检测中的关键问题，为制造系统中纹理缺陷检测提供了高效可靠的技术方案。
...
### [87] [UTDesign: A Unified Framework for Stylized Text Editing and Generation in Graphic Design Images](https://arxiv.org/abs/2512.20479)
*Yiming Zhao,Yuanpeng Gao,Yuxuan Luo,Jiwei Duan,Shisong Lin,Longfei Xiong,Zhouhui Lian*

Main category: cs.CV

TL;DR: 提出UTDesign框架，用于设计图像中的高精度风格化文本编辑和条件文本生成，支持中英文，通过DiT模型生成透明文本前景，并集成到全自动文本到设计流程中。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本渲染方面存在局限，尤其是小规模排版和非拉丁文字（如中文），需要提升设计图像中文本生成的准确性和风格一致性。

Method: 使用基于DiT的文本风格迁移模型，在合成数据集上训练生成透明RGBA文本前景；扩展为条件文本生成框架，通过多模态条件编码器结合背景图像、提示和布局规范；集成预训练文本到图像模型和基于MLLM的布局规划器，构建全自动文本到设计流程。

Result: UTDesign在开源方法中实现了风格一致性和文本准确性的最先进性能，相比专有商业方法展现出独特优势。

Conclusion: UTDesign为AI辅助图形设计提供了统一的高精度文本生成和编辑解决方案，有效支持中英文，推动了自动化设计工具的发展。
...
### [88] [AlignPose: Generalizable 6D Pose Estimation via Multi-view Feature-metric Alignment](https://arxiv.org/abs/2512.20538)
*Anna Šárová Mikeštíková,Médéric Fourmy,Martin Cífka,Josef Sivic,Vladimir Petrik*

Main category: cs.CV

TL;DR: 提出AlignPose方法，通过多视角RGB图像进行6D物体姿态估计，无需物体特定训练或对称标注，在工业数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 单视角RGB姿态估计方法存在深度模糊、遮挡和杂乱问题；现有多视角方法依赖精确单视角估计或泛化能力不足，需解决这些限制。

Method: 1. 提出AlignPose多视角姿态估计框架；2. 设计多视角特征度量优化，通过最小化渲染特征与观测特征差异，优化全局一致的世界坐标系物体姿态；3. 使用外参标定的多视角RGB图像，无需物体特定训练。

Result: 在YCB-V、T-LESS、ITODD-MV、HouseCat6D四个数据集上评估，AlignPose优于现有方法，尤其在多视角易获取的工业数据集上表现突出。

Conclusion: AlignPose通过多视角特征优化有效解决了单视角方法的局限性，实现了对未见物体的泛化，在多视角场景中具有实用优势。
...
### [89] [Multi-Grained Text-Guided Image Fusion for Multi-Exposure and Multi-Focus Scenarios](https://arxiv.org/abs/2512.20556)
*Mingwei Tang,Jiahao Nie,Guang Yang,Ziqing Cui,Jie Li*

Main category: cs.CV

TL;DR: 提出多粒度文本引导图像融合（MTIF）方法，通过分层跨模态调制模块，利用细粒度、结构性和语义性文本描述指导图像融合，显著提升多曝光和多焦点图像融合质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用粗粒度文本描述作为辅助指导，但难以理解图像细粒度细节，且跨模态对齐不精确，限制了图像融合质量的提升。

Method: 1. 引入多粒度文本描述（细节、结构、语义）；2. 设计分层跨模态调制模块；3. 采用各粒度监督信号促进跨模态对齐；4. 使用显著性驱动增强模块扩充训练数据。

Result: 在多曝光和多焦点图像融合任务上，MTIF方法均优于现有方法。
...
### [90] [Bridging Modalities and Transferring Knowledge: Enhanced Multimodal Understanding and Recognition](https://arxiv.org/abs/2512.20501)
*Gorjan Radevski*

Main category: cs.CV

TL;DR: 本文通过五章内容探讨多模态对齐、翻译、融合与迁移，旨在提升机器对复杂输入的理解能力，涵盖空间关系解码、医学文本定位、知识图谱构建、动作识别等多个应用领域。


<details>
  <summary>Details</summary>
Motivation: 多模态机器学习面临复杂输入理解的挑战，需要开发能够处理文本、图像、视频等多种模态数据的方法，以增强计算系统在空间推理、医学导航、知识提取和动作识别等任务中的能力。

Method: 分别提出：1）Spatial-Reasoning Bert将文本空间关系转换为2D布局；2）基于医学术语空间共现的损失函数实现文本到3D解剖图谱的映射；3）构建自然语言到知识图谱实体与谓词的链接基准；4）融合视频帧与物体检测表示的动作识别方法；5）通过多模态知识蒸馏使RGB模型获得多模态融合能力。

Result: 实现了空间语言的自动化场景生成、医学文本的精确定位、知识图谱的清晰提取、动作识别的鲁棒性提升，以及通过知识迁移在保持性能的同时降低计算成本。

Conclusion: 研究提出的多模态处理方法显著推进了空间语言理解、医学文本解释、知识图谱丰富化和动作识别技术的发展，增强了计算系统处理复杂多模态输入的能力，具有广泛的应用潜力。
...
### [91] [Beyond Motion Pattern: An Empirical Study of Physical Forces for Human Motion Understanding](https://arxiv.org/abs/2512.20451)
*Anh Dao,Manh Tran,Yufei Zhang,Xiaoming Liu,Zijun Cui*

Main category: cs.CV

TL;DR: 该研究探讨了将物理推断的关节驱动力融入人体运动理解任务（步态识别、动作识别、视频描述）中，实验表明力信号能有效提升模型性能，尤其在动态、遮挡或外观变化条件下。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉的运动理解方法大多忽略了关节驱动力等物理线索，而这些线索在生物力学中至关重要。研究旨在探索物理推断的力是否以及何时能增强运动理解能力。

Method: 将力信号整合到已有的运动理解流程中，在步态识别（CASIA-B、Gait3D）、动作识别（Penn Action）和细粒度视频描述（Qwen2.5-VL）三类任务上系统评估其影响，共涉及8个基准数据集。

Result: 加入力信号后所有任务均获得一致性能提升：步态识别在CASIA-B上Rank-1准确率提升0.87%（穿外套时+2.7%，侧视图+3.0%），Gait3D提升1.3%；动作识别中CTR-GCN在Penn Action上提升2.00%，高用力动作（如拳击/拍打）提升6.96%；视频描述的ROUGE-L分数从0.310提升至0.339。

Conclusion: 物理推断的力线索能显著补充视觉和运动学特征，在动态、遮挡或外观变化的挑战性条件下提升运动理解任务的性能，证明了力信号在运动分析中的补充价值。
...
### [92] [Multi-temporal Adaptive Red-Green-Blue and Long-Wave Infrared Fusion for You Only Look Once-Based Landmine Detection from Unmanned Aerial Systems](https://arxiv.org/abs/2512.20487)
*James E. Gallagher,Edward J. Oughton,Jana Kosecka*

Main category: cs.CV

TL;DR: 本研究评估了自适应RGB与LWIR融合技术用于无人机探测地表地雷，发现YOLOv11在特定融合参数下性能最优，并揭示了不同模型在精度与效率间的权衡。


<details>
  <summary>Details</summary>
Motivation: 地雷造成严重的人道主义威胁，全球每年约2.6万人伤亡。现有探测方法需提升效率与准确性，无人机结合热成像技术有望通过热对比增强地表地雷识别。

Method: 采用自适应RGB与长波红外（LWIR）融合技术，利用地雷与土壤的热对比特征。使用YOLO系列架构（v8、v10、v11）在114张测试图像上进行评估，共生成35,640次模型条件测试，并比较了RF-DETR、Faster R-CNN、RetinaNet等模型性能。

Result: YOLOv11在10-30%热融合比例、5-10米飞行高度下达到最佳性能（86.8% mAP）。模型比较显示RF-DETR精度最高（69.2% mAP），但YOLOv11训练速度快17.7倍。多时相训练数据集比季节特定方法提升1.8-9.6%。反坦克地雷检测准确率（61.9%）显著高于反步兵地雷（19.2%）。

Conclusion: 自适应热融合能有效提升地表地雷探测性能，YOLOv11在精度与效率间取得较好平衡。未来需研究不同埋藏深度和土壤类型对热对比的影响，以扩展技术适用场景。
...
### [93] [SirenPose: Dynamic Scene Reconstruction via Geometric Supervision](https://arxiv.org/abs/2512.20531)
*Kaitong Cai,Jensen Zhang,Jing Yang,Keze Wang*

Main category: cs.CV

TL;DR: 提出SirenPose方法，结合周期性激活网络与关键点几何监督，从单目视频实现动态3D场景的精确、时序一致重建，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在快速运动、多物体交互、遮挡和场景突变等挑战性场景中，常难以保持运动保真度和时空一致性，需要更鲁棒的动态3D重建方案。

Method: 1. 设计几何感知损失函数，整合正弦表示网络的周期性激活特性与基于关键点的几何监督；2. 引入物理启发约束，增强时空维度的关键点预测一致性；3. 利用高频信号建模捕捉几何细节；4. 扩展UniKPT数据集至60万标注实例；5. 结合图神经网络建模关键点关系与结构关联。

Result: 1. 在Sintel、Bonn和DAVIS基准测试中均优于现有方法；2. 在DAVIS上相比MoSCA：FVD降低17.8%、FID降低28.7%、LPIPS提升6.0%；3. 在姿态估计中优于Monst3R，绝对轨迹误差及平移/旋转相对姿态误差更低；4. 显著提升时序一致性、几何精度、用户评分和运动平滑度。

Conclusion: SirenPose能有效处理快速运动、复杂动态和物理合理重建，在动态3D场景重建中实现了更优的精度与时空一致性，为单目视频理解提供了可靠解决方案。
...
### [94] [Learning to Reason in 4D: Dynamic Spatial Understanding for Vision Language Models](https://arxiv.org/abs/2512.20557)
*Shengchao Zhou,Yuxin Chen,Yuying Ge,Wei Huang,Jiehong Lin,Ying Shan,Xiaojuan Qi*

Main category: cs.CV

TL;DR: 本文提出DSR Suite，包括自动化生成动态空间推理数据集DSR-Train和评估基准DSR-Bench的方法，以及轻量级几何选择模块GSM，显著提升了视觉语言模型在动态空间推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在动态空间推理（DSR）方面表现较弱，主要原因是缺乏可扩展的四维感知训练资源。DSR涉及对三维空间中物体几何和关系随时间演变的推理，这在真实世界视频理解中至关重要。

Method: 1) 提出自动化流水线，从真实世界视频生成DSR多选问答对，利用现代视觉基础模型提取几何和运动信息；2) 构建DSR-Train训练集和人工精炼的DSR-Bench评估集；3) 设计轻量级几何选择模块（GSM），将几何先验知识压缩为几何标记并集成到视觉语言模型中。

Result: 实验表明，将DSR-Train和GSM集成到Qwen2.5-VL-7B模型中，显著提升了其动态空间推理能力，同时在通用视频理解基准上保持了准确性。

Conclusion: DSR Suite通过数据集、基准和模型的协同设计，有效解决了视觉语言模型在动态空间推理方面的瓶颈，为四维感知理解提供了系统化解决方案。
...
### [95] [FlashVLM: Text-Guided Visual Token Selection for Large Multimodal Models](https://arxiv.org/abs/2512.20561)
*Kaitong Cai,Jusheng Zhang,Jing Yang,Yijia Fan,Pengtao Xie,Jian Wang,Keze Wang*

Main category: cs.CV

TL;DR: FlashVLM是一种文本引导的视觉令牌选择框架，通过跨模态相似性计算和视觉显著性融合，动态适应查询需求，在保持性能的同时大幅减少视觉令牌数量。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型处理大量视觉令牌时存在二次注意力成本和冗余问题，现有令牌缩减方法常忽略文本查询或依赖不稳定的注意力图，导致语义对齐退化。

Method: 提出文本引导的视觉令牌选择框架，通过投影图像令牌与归一化文本嵌入的跨模态相似性计算，结合视觉显著性进行对数域加权和温度控制锐化，并采用多样性保留分区维持全局上下文。

Result: 在相同令牌预算下，FlashVLM在LLaVA 1.5上实现超越无损压缩的效果，最高减少77.8%视觉令牌，在94.4%压缩率下仍保持92.8%准确率，在14个图像视频基准测试中达到最优效率性能平衡。

Conclusion: FlashVLM通过显式跨模态相似性计算和多样性保留机制，实现了高效且鲁棒的视觉令牌压缩，为视觉语言模型提供了可扩展的加速方案。
...
### [96] [Repurposing Video Diffusion Transformers for Robust Point Tracking](https://arxiv.org/abs/2512.20606)
...
TL;DR: 提出DiTracker方法，利用预训练的视频扩散变换器（DiT）进行点跟踪，通过注意力匹配、轻量调优和特征融合，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于浅层卷积网络（如ResNet）的点跟踪方法缺乏时间连贯性，在动态运动和遮挡场景下匹配不可靠，而预训练视频DiT具有时空注意力机制，天然具备点跟踪潜力。

Method: 1. 使用查询-键注意力匹配提取点对应关系；2. 采用轻量级LoRA微调适应跟踪任务；3. 将DiT特征与ResNet主干网络成本融合。

Result: 在ITTO基准测试中达到最先进性能，在TAP-Vid基准测试中匹配或超越现有最佳模型，且训练批次大小仅为对比方法的1/8。

Conclusion: 视频DiT特征可作为点跟踪任务的有效且高效基础，其时空注意力机制能鲁棒处理动态运动和频繁遮挡。
...
### [97] [SemanticGen: Video Generation in Semantic Space](https://arxiv.org/abs/2512.20619)
...
TL;DR: 提出SemanticGen方法，通过在语义空间生成视频来提升生成效率和质量，相比传统VAE潜在空间方法收敛更快且计算成本更低。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在VAE潜在空间中学习视频分布存在收敛慢、生成长视频时计算成本高的问题，需要更高效的生成方法。

Method: 采用两阶段生成过程：第一阶段用扩散模型生成紧凑的语义视频特征（定义视频全局布局），第二阶段用另一个扩散模型基于语义特征生成VAE潜在表示以产生最终输出。

Result: 实验表明SemanticGen能生成高质量视频，在收敛速度上优于传统方法，且扩展到长视频生成时仍保持高效计算性能，超越现有先进方法和基线模型。

Conclusion: 在语义空间进行视频生成能有效提升模型收敛效率和计算性能，为高质量视频生成提供了更优的解决方案。
...
### [98] [Active Intelligence in Video Avatars via Closed-loop World Modeling](https://arxiv.org/abs/2512.20615)
*Xuanhua He,Tianyu Yang,Ke Cao,Ruiqi Wu,Cheng Meng,Yong Zhang,Zhuoliang Kang,Xiaoming Wei,Qifeng Chen*

Main category: cs.CV

TL;DR: 本文提出L-IVA基准和ORCA框架，首次实现视频数字人具备主动智能，通过内部世界模型和闭环OTAR循环，在随机生成环境中自主完成多步骤目标导向任务。


<details>
  <summary>Details</summary>
Motivation: 现有视频数字人生成方法虽能保持身份一致性和动作对齐，但缺乏真正的自主性，无法通过自适应环境交互实现长期目标追求。

Method: 提出ORCA框架：1) 闭环OTAR循环（观察-思考-行动-反思），在生成不确定性下保持鲁棒状态跟踪；2) 分层双系统架构（系统2进行战略推理，系统1将抽象计划转换为具体行动描述）；3) 将数字人控制建模为POMDP问题，实现持续信念更新与结果验证。

Result: 实验表明ORCA在任务成功率和行为一致性上显著优于开环和非反思基线方法，验证了内部世界模型设计的有效性。

Conclusion: ORCA框架将视频数字人智能从被动动画提升到主动目标导向行为，为具身智能研究提供了新方向。
...
### [99] [FedPOD: the deployable units of training for federated learning](https://arxiv.org/abs/2512.20610)
*Daewoon Kim,Si Young Yie,Jae Sung Lee*

Main category: cs.CV

TL;DR: 本文提出FedPOD方法，通过比例编排导数优化联邦学习的效率和通信成本，在保持与FedPIDAvg相当性能的同时解决了其数据利用和参与者依赖限制，并借鉴Kubernetes的POD概念实现灵活扩展。


<details>
  <summary>Details</summary>
Motivation: FedPIDAvg方法虽然通过PID控制器和泊松分布建模提升了性能并降低了通信成本，但存在两个主要问题：1) 基于泊松分布排除异常参与者限制了数据利用；2) PID控制器需要相同参与者持续参与以利用历史学习信息。FedPOD旨在解决这些限制。

Method: 1) 定义轮次任务增强训练效率；2) 纳入被排除的异常参与者；3) 消除对历史轮次学习信息的依赖；4) 每轮计算验证损失；5) 借鉴Kubernetes的POD概念，将任务扩展到POD单元，支持类似自动扩缩的灵活设计。

Result: FedPOD在Dice分数上与FedPIDAvg表现相当：WT平均0.78、ET平均0.71、TC平均0.72；平均收敛分数为0.74。同时解决了数据利用限制和参与者依赖问题，并实现了与Kubernetes自动扩缩的兼容性。

Conclusion: FedPOD通过改进效率、灵活性和性能指标，展现了增强联邦学习的潜力，特别是在处理倾斜数据分布和动态参与者场景时更具优势，为实际部署提供了更可行的解决方案。
...
### [100] [SpatialTree: How Spatial Abilities Branch Out in MLLMs](https://arxiv.org/abs/2512.20617)
*Yuxi Xiao,Longfei Li,Shen Yan,Xinhang Liu,Sida Peng,Yunchao Wei,Xiaowei Zhou,Bingyi Kang*

Main category: cs.CV

TL;DR: 本文提出了一个受认知科学启发的空间能力层次结构SpatialTree，包含四个层级，并构建了首个能力中心化的分层基准来评估多模态大语言模型。研究发现低层能力相对独立，高层能力相互依赖，并揭示了负迁移和跨层迁移现象。通过提出自动思考策略，实现了强化学习在所有层级上的稳定提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的空间能力研究缺乏系统性框架，大多聚焦于狭窄的任务集，未能理解空间能力从感知到推理的渐进发展层次。

Method: 1. 提出SpatialTree层次结构（L1感知、L2心理映射、L3模拟、L4智能体能力）；2. 构建分层基准评估27种子能力；3. 进行针对性监督微调研究迁移动态；4. 提出自动思考策略优化强化学习训练。

Result: 1. 评估显示L1能力相对正交，高层能力高度相关；2. 微调发现L1存在负迁移，但低层到高层存在强跨层迁移与协同效应；3. 朴素强化学习会损害直觉感知，而自动思考策略能抑制不必要思考，使强化学习在所有层级稳定提升性能。

Conclusion: SpatialTree为理解和系统扩展多模态大语言模型的空间能力提供了概念验证框架，揭示了空间能力的层次结构与迁移规律，并通过自动思考策略实现了全层次能力优化。
...
### [101] [HARMON-E: Hierarchical Agentic Reasoning for Multimodal Oncology Notes to Extract Structured Data](https://arxiv.org/abs/2512.19864)
...
TL;DR: 提出一种基于大语言模型的智能体框架，用于从电子健康记录的非结构化肿瘤学笔记中提取结构化临床数据，在真实数据集上达到高准确率并显著降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的非结构化肿瘤学笔记包含重要临床信息，但人工提取成本高昂且不可扩展，现有自动化方法局限于狭窄场景，无法处理跨文档的矛盾信息与大规模患者数据。

Method: 采用大语言模型作为推理智能体，结合上下文感知检索和迭代合成能力，将复杂的肿瘤数据提取任务分解为模块化、自适应的子任务，构建端到端的提取框架。

Result: 在包含40万份临床笔记和扫描报告的大规模数据集上，平均F1分数达0.93，103个肿瘤特异性临床变量中有100个超过0.85，关键变量（如生物标志物和药物）超过0.95，集成到数据整理工作流后手动批准率达0.94。

Conclusion: 该研究首次实现了基于LLM智能体的大规模端到端肿瘤学结构化数据提取，证明其能高效处理真实世界临床文档的复杂性，显著提升数据提取的准确性与可扩展性。
...
### [102] [How well do Large Language Models Recognize Instructional Moves? Establishing Baselines for Foundation Models in Educational Discourse](https://arxiv.org/abs/2512.19903)
*Kirk Vanacore,Rene F. Kizilcec*

Main category: cs.CL

TL;DR: 本研究评估了六种大语言模型在零样本、单样本和少样本提示下对真实课堂转录中教学行为分类的基线性能，发现少样本提示显著提升性能，但模型表现仍存在局限性和可靠性约束。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在教育技术中广泛应用，了解其在未经定制的情况下对真实教育场景的解读能力，对于设定预期和建立基准至关重要。

Method: 比较六种大语言模型，使用零样本、单样本和少样本提示方法，评估其对真实课堂转录中教学行为分类的基线性能。

Result: 零样本表现中等，少样本提示显著提升先进模型的性能（最佳配置Cohen's Kappa=0.58），但性能因教学行为而异，高召回率常伴随假阳性增加。

Conclusion: 基础模型对教学话语的解读能力有意义但有限，提示设计有助于展现能力，但无法消除根本的可靠性约束。
...
### [103] [Counterfactual LLM-based Framework for Measuring Rhetorical Style](https://arxiv.org/abs/2512.19908)
*Jingyi Qiu,Hong Chen,Zongyi Li*

Main category: cs.CL

TL;DR: 本文提出了一种基于LLM的反事实框架，用于量化机器学习论文中的修辞风格，发现愿景式修辞能显著预测论文影响力，且2023年后修辞强度因LLM写作辅助工具的使用而急剧上升。
...
Method: 提出基于LLM的反事实框架：使用多个LLM修辞角色基于相同实质内容生成反事实文本，通过LLM评委进行成对比较，并采用Bradley-Terry模型聚合结果。该方法应用于2017-2025年的8,485篇ICLR投稿样本。

Result: 1. 愿景式修辞能显著预测下游关注度（引用和媒体关注），即使控制同行评审评价后仍成立；2. 2023年后修辞强度急剧上升，实证表明这主要由LLM写作辅助工具的采用驱动；3. 框架对角色选择具有鲁棒性，且LLM判断与人工标注高度相关。

Conclusion: LLM可作为测量和改进科学评估的工具，该框架为量化修辞风格提供了可靠方法，揭示了修辞风格对科学传播的影响及LLM技术对学术写作的变革作用。
...
### [104] [PRISM: A Personality-Driven Multi-Agent Framework for Social Media Simulation](https://arxiv.org/abs/2512.19933)
*Zhixiang Lu,Xueyuan Deng,Yiran Liu,Yulong Li,Qiang Yan,Imran Razzak,Jionglong Su*

Main category: cs.CL

TL;DR: 提出了PRISM模型，结合随机微分方程和人格条件部分可观测马尔可夫决策过程，通过MBTI人格类型模拟在线舆论极化中的心理异质性。


<details>
  <summary>Details</summary>
Motivation: 传统基于智能体的舆论动力学模型因简化同质性假设，无法捕捉驱动在线极化的心理异质性，阻碍了对意识形态分化放大机制的理解。

Method: 开发了PRISM混合框架：使用随机微分方程模拟连续情感演化，结合人格条件部分可观测马尔可夫决策过程处理离散决策；为多模态大语言模型智能体分配基于MBTI的认知策略，并通过大规模社交媒体数据初始化先验。

Result: PRISM在人格一致性方面显著优于传统同质模型和五因素模型基准，能有效复现理性抑制和情感共鸣等涌现现象。

Conclusion: PRISM为分析复杂社交媒体生态系统提供了稳健工具，能更好地模拟个体认知偏见与信息传播的相互作用。
...
### [105] [Bias Beneath the Tone: Empirical Characterisation of Tone Bias in LLM-Driven UX Systems](https://arxiv.org/abs/2512.19950)
*Heet Bodara,Md Masum Mushfiq,Isma Farah Siddiqui*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在对话中存在系统性语调偏见，即使中性提示下也会表现出过度礼貌、乐观或谨慎的倾向，通过合成对话数据集和分类器可有效检测这些偏见。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数字个人助理等对话系统中广泛应用，但其回应常带有微妙语调偏见（如过度礼貌、乐观或谨慎），可能影响用户对信任、共情和公平性的感知，需要系统性地检测和量化这种隐藏的行为特征。

Method: 1. 创建两个合成对话数据集：中性提示生成集和明确引导生成正/负语调集；2. 使用预训练DistilBERT模型进行弱监督标注；3. 训练多个分类器检测语调模式；4. 采用集成模型评估性能。

Result: 1. 即使中性数据集也呈现一致的语调偏斜；2. 集成模型的宏观F1分数最高达0.92；3. 证明语调偏见具有系统性、可测量性，且与设计公平可信的对话AI相关。

Conclusion: 大型语言模型的语调偏见是其底层对话风格的固有特征，通过可控对话合成与语调分类模型结合的方法，能够实现稳健且符合伦理的情感识别，为构建更公平、可信的对话系统提供重要依据。
...
### [106] [Schoenfeld's Anatomy of Mathematical Reasoning by Language Models](https://arxiv.org/abs/2512.19995)
*Ming Li,Chenrui Fan,Yize Cheng,Soheil Feizi,Tianyi Zhou*

Main category: cs.CL

TL;DR: 提出ThinkARM框架，基于Schoenfeld的Episode Theory将大语言模型的推理轨迹抽象为功能步骤（如分析、探索、实施、验证），用于系统分析数学问题解决中的思维动态和结构差异。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以深入识别和分析大语言模型推理轨迹的认知结构与步骤，仅依赖表层统计信息，需要一种能显式抽象推理步骤的中观尺度分析框架。

Method: 采用Schoenfeld的Episode Theory作为归纳框架，提出ThinkARM，将推理轨迹映射为功能步骤；应用于多个模型的数学问题求解，并进行诊断性案例研究。

Result: 抽象化揭示了推理与非推理模型间可复现的思维动态和结构差异；探索步骤是关键分支点，与正确性相关；效率导向方法会选择性地抑制评估反馈步骤而非均匀缩短响应。

Conclusion: Episode级别的表征使推理步骤显式化，支持对现代语言模型中推理的结构化、稳定化和变化进行系统分析。
...
### [107] [Memory-T1: Reinforcement Learning for Temporal Reasoning in Multi-session Agents](https://arxiv.org/abs/2512.20092)
...
TL;DR: 提出Memory-T1框架，通过强化学习学习时间感知的记忆选择策略，以解决长对话历史中的时序推理问题，在Time-Dialog基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文模型在处理冗长、多轮次对话历史时，难以准确识别时序相关信息，导致推理性能显著下降。

Method: 采用强化学习框架，通过粗到细策略：先用时间和相关性过滤器修剪对话历史生成候选集，再用RL代理选择精确证据会话；训练使用多级奖励函数优化答案准确性、证据基础和时序一致性。

Result: 在Time-Dialog基准上，Memory-T1将7B模型提升至67.0%总分，超越14B基线10.2%；消融研究表明时序一致性和证据基础奖励共同贡献15.0%性能提升；框架在128k令牌内保持鲁棒性。

Conclusion: Memory-T1能有效处理长对话历史中的噪声，通过时间感知记忆选择显著提升时序推理能力，代码和数据集已开源。
...
### [108] [A Novel Graph-Sequence Learning Model for Inductive Text Classification](https://arxiv.org/abs/2512.20097)
*Zuo Wang,Ye Yuan*

Main category: cs.CL

TL;DR: 提出TextGSL模型，通过构建多边类型文本图并结合Transformer层，解决现有GNN文本分类方法在结构信息利用和序列信息捕获上的不足，实现归纳式文本分类。


<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的文本分类方法存在两个主要局限：1）未能充分利用词对间的多样化结构信息（如共现、句法、语义）；2）在文本图结构学习中忽略序列信息，且无法处理新词和新关系。

Method: 1）为每个文本构建单文本级图，基于词对间多样化关系定义不同边类型；2）设计自适应多边消息传递机制聚合多样化结构信息；3）引入Transformer层捕获文本序列信息；4）提出TextGSL模型实现归纳式文本分类。

Result: 在多个基准数据集上的实验表明，TextGSL在准确率方面优于多个强基线模型。

Conclusion: TextGSL通过融合多样化图结构信息和序列信息，能够学习更具判别性的文本表示，有效提升文本分类性能，并支持归纳式学习场景。
...
### [109] [ABBEL: LLM Agents Acting through Belief Bottlenecks Expressed in Language](https://arxiv.org/abs/2512.20111)
*Aly Lidayan,Jakob Bjorner,Satvik Golechha,Kartik Goyal,Alane Suhr*

Main category: cs.CL

TL;DR: 提出ABBEL框架，通过语言表达的信念瓶颈来维护简洁的上下文，以解决长序列决策任务中历史交互信息过载的问题，并通过RL后训练进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着序列决策任务长度增加，在上下文中保留完整交互历史变得计算上不切实际，需要一种既能压缩信息又能保持性能的方法。

Method: 1. 提出ABBEL框架：用自然语言信念状态（对任务相关未知信息的总结）替代长交互历史；2. 每个步骤先更新先验信念形成后验信念，仅基于后验选择动作；3. 引入RL后训练，包括信念质量奖励和长度惩罚机制。

Result: 1. ABBEL能在保持近恒定内存使用的同时生成可解释的信念；2. 瓶颈方法易产生错误传播，导致性能低于完整上下文设置；3. RL训练能提升ABBEL性能，超越完整上下文设置，同时内存使用低于现有方法。

Conclusion: ABBEL框架通过信念瓶颈有效压缩交互历史，结合RL训练能克服错误传播问题，在减少内存使用的同时提升决策性能，为长序列任务提供了可扩展的解决方案。
...
### [110] [M$^3$KG-RAG: Multi-hop Multimodal Knowledge Graph-enhanced Retrieval-Augmented Generation](https://arxiv.org/abs/2512.20136)
*Hyeongcheol Park,Jiyoung Seo,Jaewon Mun,Hogun Park,Wonmin Byeon,Sung June Kim,Hyeonsoo Im,JeungSub Lee,Sangpil Kim*

Main category: cs.CL

TL;DR: 提出M³KG-RAG方法，通过构建多跳多模态知识图谱和GRASP检索机制，增强多模态大语言模型的音频-视觉推理能力与答案忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有多模态检索增强生成在音频-视觉领域面临两个挑战：1）现有多模态知识图谱的模态覆盖有限且多跳连接不足；2）仅基于共享嵌入空间相似性的检索无法过滤无关或冗余知识。

Method: 1）设计轻量级多智能体流水线构建多跳多模态知识图谱（M³KG），包含上下文丰富的多模态实体三元组；2）提出GRASP机制，实现查询实体精准锚定、答案支持相关性评估和冗余上下文剪枝。

Result: 在多个多模态基准测试上的广泛实验表明，M³KG-RAG显著提升了多模态大语言模型的多模态推理和基础能力，优于现有方法。

Conclusion: M³KG-RAG通过结构化多模态知识检索和选择性剪枝机制，有效解决了多模态检索增强生成中的知识覆盖不足和检索噪声问题，为音频-视觉领域推理提供了可靠框架。
...
### [111] [Multi-hop Reasoning via Early Knowledge Alignment](https://arxiv.org/abs/2512.20144)
*Yuxin Wang,Shicheng Fang,Bo Wang,Qi Luo,Xuanjing Huang,Yining Zheng,Xipeng Qiu*

Main category: cs.CL

TL;DR: 提出早期知识对齐（EKA）模块，在迭代式检索增强生成系统中先对齐检索知识再规划，提升多跳问题处理性能


<details>
  <summary>Details</summary>
Motivation: 现有迭代式RAG系统在规划问题分解时未充分利用检索语料库信息，导致检索效率低和级联错误

Method: 在强化学习增强的迭代RAG系统中引入EKA模块，在规划前用检索到的上下文相关知识对齐LLM，减少推理过程中的不必要探索

Result: 在六个标准RAG数据集上显著提升检索精度、减少级联错误，提高性能和效率；熵分析表明能更聚焦相关子集

Conclusion: EKA通过结构化推理与高效探索的协同作用推进了迭代RAG技术，且作为无需训练的策略可扩展到大型模型
...
### [112] [Retrieval-augmented Prompt Learning for Pre-trained Foundation Models](https://arxiv.org/abs/2512.20145)
*Xiang Chen,Yixin Ou,Quan Feng,Lei Li,Piji Li,Haibo Ye,Sheng-Jun Huang,Shuofei Qiao,Shumin Deng,Huajun Chen,Ningyu Zhang*

Main category: cs.CL

TL;DR: 提出RetroPrompt方法，通过检索机制从知识库中获取上下文信息，减少对死记硬背的依赖，提升预训练基础模型在零样本和少样本场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统提示学习仍遵循参数化学习范式，在有限数据下可能难以充分利用非典型实例并避免对浅层模式的过拟合，导致记忆与泛化不平衡。

Method: RetroPrompt方法：1）从训练数据构建公开可访问的知识库；2）在输入、训练和推理阶段引入检索机制，动态检索相关上下文信息；3）通过解耦知识与单纯记忆来平衡记忆与泛化。

Result: 在自然语言处理和计算机视觉任务的多数据集实验中，RetroPrompt在零样本和少样本场景下均表现优异；记忆模式分析表明其有效降低了对死记硬背的依赖。

Conclusion: RetroPrompt通过检索增强机制改善了预训练基础模型的泛化能力，为平衡记忆与泛化提供了有效解决方案。
...
### [113] [Fun-Audio-Chat Technical Report](https://arxiv.org/abs/2512.20156)
...
TL;DR: 提出Fun-Audio-Chat大音频语言模型，通过双分辨率语音表示和核心鸡尾酒训练解决语音-文本模型中的分辨率不匹配和灾难性遗忘问题，在多个音频任务上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 现有联合语音-文本模型面临语音标记（25Hz）与文本标记（~3Hz）的时间分辨率不匹配问题，导致语义信息稀释、计算成本高以及文本LLM知识的灾难性遗忘。

Method: 1. 双分辨率语音表示：共享LLM以高效5Hz处理音频（通过标记分组），语音精炼头生成25Hz高质量标记；2. 核心鸡尾酒训练：两阶段微调与中间合并缓解灾难性遗忘；3. 多任务DPO训练增强鲁棒性、音频理解、指令跟随和语音共情能力。

Result: Fun-Audio-Chat 8B和MoE 30B-A3B在语音转文本和语音转语音任务上表现竞争性，在口语问答基准测试中位居同类模型前列，在音频理解、语音功能调用、指令跟随和语音共情方面达到竞争至优越水平。

Conclusion: 该模型通过多阶段后训练在保留文本LLM知识的同时获得强大的音频理解、推理和生成能力，无需大规模音频-文本预训练，并开源了8B版本及训练推理代码。
...
### [114] [AI Security Beyond Core Domains: Resume Screening as a Case Study of Adversarial Vulnerabilities in Specialized LLM Applications](https://arxiv.org/abs/2512.20164)
*Honglin Mu,Jinghao Liu,Kaiyang Wan,Rui Xing,Xiuying Chen,Timothy Baldwin,Wanxiang Che*

Main category: cs.CL

TL;DR: 研究发现LLMs在简历筛选等任务中容易受到隐藏的对抗指令攻击，攻击成功率超过80%。论文提出FIDS防御方法，结合训练时防御可降低26.3%攻击成功率。


<details>
  <summary>Details</summary>
Motivation: LLMs在代码审查等领域已有防御措施，但在简历筛选、同行评审等常见应用中缺乏对抗指令攻击的防护，存在安全漏洞。

Method: 建立简历筛选场景的基准测试，评估两种防御机制：基于提示的防御和提出的FIDS方法（通过LoRA适配实现对抗指令分离检测）。

Result: 某些攻击类型的成功率超过80%；提示防御降低10.1%攻击但增加12.5%误拒率；FIDS降低15.4%攻击且仅增加10.4%误拒率；组合方法降低26.3%攻击。

Conclusion: 训练时防御（如FIDS）在安全性和实用性保护上均优于推理时缓解措施，组合方法能有效降低对抗指令攻击风险。
...
### [115] [FaithLens: Detecting and Explaining Faithfulness Hallucination](https://arxiv.org/abs/2512.20182)
...
TL;DR: 提出FaithLens模型，用于检测大语言模型输出中的忠实性幻觉，能同时提供二元预测和解释，在12个任务上超越GPT-4等先进模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在检索增强生成和摘要等实际应用中存在忠实性幻觉问题，需要高效可靠的检测方法来提高可信度。

Method: 1. 使用先进LLM合成带解释的训练数据，并通过数据过滤确保标签正确性、解释质量和数据多样性；2. 在精心整理的数据上微调模型作为冷启动；3. 采用基于规则的强化学习进一步优化，奖励预测正确性和解释质量。

Result: 8B参数的FaithLens在12个多样化任务上表现优于GPT-4.1和o3等先进模型，并能生成高质量解释，在可信度、效率和效果之间达到良好平衡。

Conclusion: FaithLens是一种成本效益高且有效的忠实性幻觉检测模型，通过数据合成、过滤和强化学习优化，实现了优越的性能和解释能力。
...
### [116] [Corpus of Cross-lingual Dialogues with Minutes and Detection of Misunderstandings](https://arxiv.org/abs/2512.20204)
*Marko Čechovič,Natália Komorníková,Dominik Macháček,Ondřej Bojar*

Main category: cs.CL

TL;DR: 本文创建了一个跨语言对话语料库，用于评估自动语音翻译系统在无共同语言会议中的应用，并探索了误解自动检测方法。


<details>
  <summary>Details</summary>
Motivation: 为评估跨语言会议中自动语音翻译系统的性能，需要构建真实、多语言的评估语料库，并解决跨语言交流中误解检测的挑战。

Method: 构建包含12种语言、5小时语音的跨语言对话语料库，提供ASR转录、人工校对文本及英译版本；同时人工标注误解现象，并测试大语言模型（如Gemini）的自动检测能力。

Result: 语料库包含多语言会议录音、转录文本、翻译及会议纪要；Gemini模型在误解检测任务中达到77%的召回率和47%的精确率。

Conclusion: 该语料库支持跨语言翻译与摘要研究；大语言模型在误解检测中展现出潜力，但精确率仍有提升空间，需进一步优化。
...
### [117] [SlideTailor: Personalized Presentation Slide Generation for Scientific Papers](https://arxiv.org/abs/2512.20292)
*Wenzheng Zeng,Mingyu Ouyang,Langyuan Cui,Hwee Tou Ng*

Main category: cs.CL

TL;DR: 提出SlideTailor框架，通过用户提供的论文-幻灯片示例对和视觉模板来个性化生成演示幻灯片，无需详细文本描述偏好，并引入链式语音机制增强内容与口头叙述的匹配。


<details>
  <summary>Details</summary>
Motivation: 现有幻灯片自动生成方法因用户偏好差异而难以满足个性化需求，需要一种能隐式捕捉用户内容与视觉偏好的更自然、易用的生成方式。

Method: 1. 提出基于人类行为启发的代理框架SlideTailor，逐步生成可编辑幻灯片；2. 仅需用户提供论文-幻灯片示例对和视觉模板作为隐式偏好输入；3. 引入链式语音机制（chain-of-speech）对齐幻灯片内容与口头叙述规划。

Result: 构建了包含多样化用户偏好的基准数据集，设计可解释评估指标，实验证明框架能有效提取和泛化用户偏好，生成质量显著提升，并支持视频演示等下游应用。

Conclusion: SlideTailor通过隐式偏好学习和链式语音机制，实现了用户对齐的个性化幻灯片生成，为自动化内容创作提供了更灵活、实用的解决方案。
...
### [118] [AprielGuard](https://arxiv.org/abs/2512.20293)
...
TL;DR: 提出了AprielGuard，一个8B参数的安全防护模型，统一处理LLM的安全风险和对抗威胁，在多项基准测试中表现优于现有开源防护方案。


<details>
  <summary>Details</summary>
Motivation: 现有防护工具将安全风险（如毒性、偏见）和对抗威胁（如提示注入、越狱）视为独立问题，限制了其鲁棒性和泛化能力，需要统一的防护框架。

Method: 构建统一分类和学习框架，使用开放和合成数据混合训练，涵盖单轮提示、多轮对话和智能体工作流，并加入结构化推理轨迹以提高可解释性。

Result: 在多个公开和专有基准测试中，AprielGuard在检测有害内容和对抗操作方面表现强劲，特别是在多步骤和推理密集型场景中优于Llama-Guard和Granite Guardian等现有开源防护方案。

Conclusion: 通过发布该模型，旨在推动LLM可靠防护的透明和可重复研究，为对话和智能体场景提供统一的安全保障方案。
...
### [119] [Patterns vs. Patients: Evaluating LLMs against Mental Health Professionals on Personality Disorder Diagnosis through First-Person Narratives](https://arxiv.org/abs/2512.20298)
*Karolina Drożdż,Kacper Dudzic,Anna Sterna,Marcin Moskalewicz*

Main category: cs.CL

TL;DR: 本研究首次直接比较了先进LLM与心理健康专家在利用波兰语第一人称自传体叙述诊断边缘型人格障碍（BPD）和自恋型人格障碍（NPD）的表现。结果显示，最佳LLM模型在整体诊断准确率上超越人类专家21.91个百分点，但在NPD诊断中存在严重漏诊，且表现出对价值负载术语的回避倾向。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在精神科自我评估中的应用日益增多，需要评估其解释定性患者叙述的能力，特别是针对人格障碍这类复杂临床数据的诊断可靠性。

Method: 采用波兰语第一人称自传体叙述作为诊断材料，对比state-of-the-art LLM（包括Gemini Pro模型）与心理健康专家对BPD和NPD的诊断表现，使用准确率和F1分数进行量化评估，并辅以诊断理由的质性分析。

Result: 1. Gemini Pro模型整体诊断准确率达65.48%，显著高于人类专家的43.57%；2. 双方对BPD诊断表现相当（模型F1=83.4，人类F1=80.0）；3. 模型对NPD严重漏诊（F1=6.7 vs. 人类50.0），表现出对“自恋”术语的回避；4. 质性分析显示模型诊断理由自信详尽但偏重形式分类，人类专家则更简洁谨慎，关注患者自我感知和时间体验。

Conclusion: LLM虽能有效解读复杂的第一人称临床数据，但在诊断可靠性（特别是对价值负载障碍）和系统性偏差方面存在显著局限，目前尚不能替代人类专业判断。
...
### [120] [SpidR: Learning Fast and Stable Linguistic Units for Spoken Language Models Without Supervision](https://arxiv.org/abs/2512.20308)
...
TL;DR: 提出了SpidR自监督语音表示模型，通过掩码预测、自蒸馏和在线聚类高效学习语音表示，在语言建模任务上优于现有方法，且训练时间大幅缩短。


<details>
  <summary>Details</summary>
Motivation: 语言建模和语音表示学习的并行进展使得直接从语音学习语言成为可能，这需要从语音中直接提取语义表示，而现有方法在效率和效果上仍有提升空间。

Method: 使用掩码预测目标结合自蒸馏和在线聚类训练模型，学生模型中间层预测教师模型中间层的聚类分配，稳定在线聚类过程，生成更高质量的码本。

Result: SpidR在sWUGGY、sBLIMP、tSC等语言建模基准上优于wav2vec 2.0、HuBERT、WavLM和DinoSR；验证了语音单元质量指标与语言建模性能的相关性；训练时间从一周缩短至一天（16 GPU）。

Conclusion: SpidR通过改进的训练方法和高效实现，在语音表示学习和无文本语言建模任务上取得更好性能，同时大幅降低训练成本，为相关研究提供了实用工具。
...
### [121] [Can LLMs Solve My Grandma's Riddle? Evaluating Multilingual Large Language Models on Reasoning Traditional Bangla Tricky Riddles](https://arxiv.org/abs/2512.20324)
*Nurul Labib Sayeedi,Md. Faiyaz Abdullah Sayeedi,Khushnur Binte Jahangir,Swakkhar Shatabda,Sarah Masud Preum*

Main category: cs.CL

TL;DR: 本文介绍了BanglaRiddleEval基准测试，包含1,244个孟加拉语谜语，评估大语言模型在低资源比喻推理任务上的表现。结果显示模型性能远低于人类基线，表明该领域仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在标准NLP任务上表现优异，但在比喻性、文化相关及低资源语言（如孟加拉语）的推理能力尚未充分探索，因此需要建立专门的评估基准。

Method: 构建包含4个任务的孟加拉语谜语数据集（共4,976个样本），使用LLM生成思维链解释、语义干扰项和歧义标注，并评估开源与闭源模型在不同提示策略下的表现。

Result: 模型在生成式问答中仅达到中等语义重叠但正确率低；多项选择题准确率最高约56%（人类基线83%）；歧义消解准确率在26%-68%之间，仅最强模型能生成高质量解释。

Conclusion: 当前LLM能捕捉部分孟加拉语谜语推理线索，但远未达到人类水平。BanglaRiddleEval可作为低资源比喻推理的挑战性新基准，所有数据与代码已开源。
...
### [122] [Multi-LLM Thematic Analysis with Dual Reliability Metrics: Combining Cohen's Kappa and Semantic Similarity for Qualitative Research Validation](https://arxiv.org/abs/2512.20352)
*Nilesh Jain,Seyi Adeyinka,Leor Roseman,Aza Allsop*

Main category: cs.CL

TL;DR: 提出一个结合集成验证与双可靠性指标的多视角验证框架，用于评估LLM主题分析的可靠性，并在三个主流LLM上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统质性研究的编码者间一致性评估方法依赖多人标注、耗时且一致性通常不高，需要更高效可靠的验证方法。

Method: 采用多视角验证框架，结合集成验证（1-6种子、温度0.0-2.0可调）与双可靠性指标（Cohen's Kappa评估编码一致性、余弦相似度评估语义一致性），支持自定义提示结构和任意JSON格式的共识主题提取。

Result: 在致幻艺术治疗访谈文本上测试三个LLM（各运行6次）：Gemini可靠性最高（κ=0.907，余弦相似度95.3%），GPT-4o次之（κ=0.853，92.6%），Claude第三（κ=0.842，92.1%）；所有模型κ>0.80，Gemini提取6个共识主题（一致性50-83%），GPT-4o提取5个，Claude提取4个。

Conclusion: 该框架通过透明可靠性指标、灵活配置和结构无关的共识提取，为可靠的AI辅助质性研究奠定了方法学基础，开源实现可供研究者使用。
...
### [123] [Sentiment-Aware Extractive and Abstractive Summarization for Unstructured Text Mining](https://arxiv.org/abs/2512.20404)
*Junyi Liu,Stanley Kok*

Main category: cs.CL

TL;DR: 提出一种情感感知的文本摘要框架，通过将情感信号嵌入到抽取式和生成式摘要方法中，提升对用户生成短文本的情感细微差别和主题相关性的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体、评论和论坛等非结构化数据的快速增长，文本挖掘在信息系统中变得至关重要。现有摘要方法主要针对结构化新闻优化，难以处理噪声大、非正式的短文本，且情感线索对品牌监控和市场分析等任务很重要，但很少有研究将情感建模整合到用户生成文本的摘要中。

Method: 扩展抽取式（TextRank）和生成式（UniLM）摘要方法，通过将情感信号嵌入到排名和生成过程中，形成双设计框架。

Result: 该框架能够更好地捕捉情感细微差别和主题相关性，生成简洁且情感丰富的摘要。

Conclusion: 情感感知摘要框架可增强动态在线环境中及时干预和战略决策的能力。
...
### [124] [Distilling to Hybrid Attention Models via KL-Guided Layer Selection](https://arxiv.org/abs/2512.20569)
*Yanhong Li,Songlin Yang,Shawn Tan,Mayank Mishra,Rameswar Panda,Jiawei Zhou,Yoon Kim*

Main category: cs.CL

TL;DR: 提出了一种基于层重要性评分的简单高效方法，用于在蒸馏预训练Transformer时选择将哪些层转换为线性注意力层，以提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 为了在不从头开始昂贵预训练的情况下，通过蒸馏预训练的softmax注意力Transformer为混合架构（交替使用softmax和线性注意力层）来提升LLM的推理效率，其中层选择是关键因素。

Method: 使用少量通用文本数据训练得到的层重要性评分进行层选择，然后采用RADLADS蒸馏流程，包括注意力权重转移、隐藏状态对齐、基于KL散度的分布匹配和少量微调。

Result: 该方法在层选择上比现有方法更有效，包括基于固定比例均匀插入线性注意力的启发式方法，以及依赖专用诊断数据集的复杂方法。

Conclusion: 通过层重要性评分指导的层选择结合蒸馏流程，能高效地将预训练Transformer转换为混合注意力架构，优化推理效率。
...
### [125] [Step-DeepResearch Technical Report](https://arxiv.org/abs/2512.20491)
...
TL;DR: 提出了Step-DeepResearch端到端智能体，通过原子能力数据合成策略和渐进式训练路径提升深度研究能力，并在中文领域建立ADR-Bench评估基准，实验表明中等规模模型能达到专家级性能。


<details>
  <summary>Details</summary>
Motivation: 现有学术基准（如BrowseComp）无法满足开放研究对意图识别、长程决策和跨源验证的现实需求，且中文领域缺乏深度研究评估标准。

Method: 1. 基于原子能力的数据合成策略强化规划与报告撰写；2. 从智能体中期训练到SFT和RL的渐进式训练路径；3. 引入清单式评判器提升鲁棒性；4. 构建中文深度研究场景评估基准ADR-Bench。

Result: Step-DeepResearch（32B）在Scale AI研究量表中得分61.4%；在ADR-Bench上显著优于同类模型，性能接近OpenAI和Gemini DeepResearch等闭源SOTA模型。

Conclusion: 精细化训练能使中等规模模型以行业领先的性价比实现专家级深度研究能力，验证了所提方法的有效性。
...
### [126] [Can LLMs Predict Their Own Failures? Self-Awareness via Internal Circuits](https://arxiv.org/abs/2512.20578)
*Amirhosein Ghasemabadi,Di Niu*

Main category: cs.CL

TL;DR: 提出Gnosis，一种轻量级自感知机制，使冻结大语言模型能够通过解码隐藏状态和注意力模式的内部信号进行内在自我验证，以预测自身错误，且计算开销极低。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成流畅复杂的输出，但常无法识别自身错误和幻觉。现有方法依赖外部评估器、多样本一致性或基于文本的自我批判，这些方法计算成本高或与真实正确性关联弱。研究探索是否可通过推理时检查内部状态来预测模型自身失败。

Method: 引入Gnosis机制：被动观察内部轨迹（隐藏状态和注意力模式），将其压缩为固定预算描述符，并预测正确性。该方法仅增加约500万参数，独立于序列长度，推理成本可忽略。

Result: 在数学推理、开放域问答和学术知识基准测试中，对1.7B至20B参数的冻结主干模型，Gnosis在准确性和校准方面均持续优于强内部基线和大型外部评估器。此外，它能够零样本泛化到部分生成，实现失败轨迹的早期检测和计算感知控制。

Conclusion: 可靠的正确性线索内在于生成过程，无需外部监督即可高效提取。Gnosis证明了通过内部状态自验证的可行性，为模型自我纠错提供了新途径。
...
### [127] [MoE-DiffuSeq: Enhancing Long-Document Diffusion Models with Sparse Attention and Mixture of Experts](https://arxiv.org/abs/2512.20604)
*Alexandros Christoforos,Chadbourne Davis*

Main category: cs.CL

TL;DR: 提出MoE-DiffuSeq框架，通过混合专家架构和稀疏注意力机制提升扩散模型在长文档生成中的效率与性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的文本生成模型（如DiffuSeq）在处理长序列时存在计算成本高、内存开销大的问题，限制了其在长文档生成场景的应用。

Method: 1. 结合稀疏注意力与混合专家架构，实现高效的长序列建模；2. 设计定制化稀疏注意力机制以降低计算复杂度；3. 在扩散过程中引入软吸收态以加速序列重建并提升生成精度。

Result: 实验表明，MoE-DiffuSeq相比现有扩散模型显著提升了训练效率和采样速度，在科学文章生成、代码库建模和长对话生成等任务中表现出色，同时兼顾了效率、速度、准确性和表达力。

Conclusion: MoE-DiffuSeq通过架构创新有效解决了扩散模型在长文本生成中的效率瓶颈，推动了高质量长文本生成的实际应用。
...
### [128] [Cube Bench: A Benchmark for Spatial Visual Reasoning in MLLMs](https://arxiv.org/abs/2512.20595)
*Dhruv Anand,Ehsan Shareghi*

Main category: cs.CL

TL;DR: Cube Bench是一个基于魔方的多模态大语言模型基准测试，用于评估空间和序列推理能力，包含五个技能维度，测试显示模型性能随魔方复杂度增加而急剧下降。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在空间和序列推理方面的能力缺乏系统评估，需要设计一个紧凑、可复现的基准来量化模型在复杂空间任务中的表现差距。

Method: 设计包含五个技能维度的魔方基准测试：(1)从图像/文本重建魔方面，(2)选择最优下一步，(3)预测候选移动结果，(4)执行多步计划并纠错，(5)错误检测与修正；使用统一提示词、解析器和解决距离指标，在七个MLLMs上测试不同打乱深度的表现。

Result: 所有模型准确率随打乱深度增加急剧下降；轨迹一旦停滞或偏离，模型难以恢复；高面重建准确率不保证动作选择或多步执行能力；闭源模型在单步感知和多步控制任务上均领先，开源模型在困难场景下接近随机水平；最佳模型在高复杂度下性能仍会退化；简单自校正仅带来有限提升且可能引发过度思考。

Conclusion: Cube Bench能有效揭示MLLMs在序列空间推理中的系统性缺陷，闭源与开源模型存在显著差距，当前模型在复杂多步任务中仍缺乏鲁棒性，该基准为评估和改进MLLMs的推理能力提供了标准化工具。
...
### [129] [Gaussian Variational Inference with Non-Gaussian Factors for State Estimation: A UWB Localization Case Study](https://arxiv.org/abs/2512.19855)
*Andrew Stirling,Mykola Lukashchuk,Dmitry Bagaev,Wouter Kouw,James R. Forbes*

Main category: cs.RO

TL;DR: 本文扩展了ESGVI算法，使其适用于矩阵李群状态估计并处理重尾/偏斜噪声，在UWB定位实验中验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有ESGVI算法无法直接处理含方向的状态估计（如旋转），且实际UWB定位中NLOS和多径效应导致噪声呈重尾/偏斜分布，需要更鲁棒的噪声建模。

Method: 1. 将ESGVI推广至矩阵李群以保持群结构；2. 引入因子处理重尾和偏斜噪声分布；3. 在UWB NLOS场景实验验证；4. 开源Python实现。

Result: 扩展后的ESGVI框架保持了稀疏性和无导数特性，在NLOS丰富的UWB定位实验中实现了更高精度和相当的一致性。

Conclusion: 所提方法自然融合了李群结构和鲁棒噪声建模，有效提升了复杂噪声环境下的状态估计性能，开源代码促进相关研究。
...
### [130] [A Time-efficient Prioritised Scheduling Algorithm to Optimise Initial Flock Formation of Drones](https://arxiv.org/abs/2512.19914)
*Sujan Warnakulasooriya,Andreas Willig,Xiaobing Wu*

Main category: cs.RO

TL;DR: 提出一种基于优先级的调度算法，用于提升无人机集群初始编队效率，避免碰撞并优化轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 现有集群算法在初始编队时效率低、可扩展性差，尤其在面临潜在碰撞时易产生次优轨迹，限制了大规模无人机协同应用。

Method: 基于无人机潜在碰撞数量及无障碍到达目标位置的可能性分配优先级，通过计算延迟时间实现分层调度，确保无碰撞路径。

Result: 仿真验证算法可为多达5000架无人机生成无碰撞轨迹，在性能和计算效率上均优于基于耦合度的启发式优先级规划方法（CDH-PP）。

Conclusion: 该优先级调度算法能有效解决大规模无人机集群初始编队的碰撞问题，提升编队效率与可扩展性。
...
### [131] [Bring My Cup! Personalizing Vision-Language-Action Models with Visual Attentive Prompting](https://arxiv.org/abs/2512.20014)
*Sangoh Lee,Sangwoo Mo,Wook-Shin Han*

Main category: cs.RO

TL;DR: 提出视觉注意力提示（VAP），一种无需训练的方法，通过视觉记忆和开放词汇检测增强视觉-语言-动作模型对个性化物体（如“拿我的杯子”）的识别与操控能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型能处理通用指令，但难以执行涉及用户特定物体的个性化命令（如从多个相似物体中识别并操作属于用户的那个），因为训练时未见过该具体实例。

Method: VAP作为冻结模型的感知适配器，将用户提供的参考图像作为非参数视觉记忆，通过开放词汇检测和嵌入匹配在场景中定位目标物体，并通过高亮物体和重写指令将其作为视觉提示注入模型。

Result: 在仿真基准（Personalized-SIMPLER和Personalized-VLABench）和真实桌面任务上的实验表明，VAP在成功率和正确物体操作率上均优于通用策略和基于令牌学习的方法。

Conclusion: VAP能有效桥接语义理解与实例级控制，提升模型对个性化物体的操作能力，且无需额外训练。
...
### [132] [LoLA: Long Horizon Latent Action Learning for General Robot Manipulation](https://arxiv.org/abs/2512.20166)
*Xiaofan Wang,Xingyu Gao,Jianlong Fu,Zuolei Li,Dean Fortier,Galen Mullins,Andrey Kolobov,Baining Guo*

Main category: cs.RO

TL;DR: 提出了LoLA框架，通过整合长期多视角观测和机器人本体感知，实现多步推理和动作生成，在长时程操作任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型常忽视历史信息利用和连贯动作序列生成能力，难以完成长时程、语言引导的机器人操作任务。

Method: 使用视觉语言模型编码历史序列和多视角观测的上下文特征；提出状态感知潜在重表示模块，将视觉输入和语言指令转换到可操作的机器人运动空间；通过可学习的具身锚定潜在空间显式地将视觉-语言表示与物理尺度对齐。

Result: 在仿真基准和真实机器人任务上的评估表明，LoLA显著优于现有方法，尤其在长时程操作任务中表现突出。

Conclusion: LoLA框架通过整合多模态历史信息和机器人状态，有效提升了长时程语言引导操作任务的性能，为机器人操作提供了新思路。
...
### [133] [Asynchronous Fast-Slow Vision-Language-Action Policies for Whole-Body Robotic Manipulation](https://arxiv.org/abs/2512.20188)
*Teqiang Zou,Hongliang Zeng,Yuxuan Nong,Yifan Li,Kehui Liu,Haotian Yang,Xinyang Ling,Xin Li,Lianyang Ma*

Main category: cs.RO

TL;DR: 提出异步快慢双通路VLA框架DuoCore-FS，通过分离高频动作生成与低频语义推理，解决传统VLA模型因同步执行导致的实时性瓶颈，在全身机器人操控中实现更高频率与稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统VLA模型将视觉语言模型与动作生成模块以单一频率同步运行，受限于大模型推理速度，难以满足全身机器人操控对高频控制与实时响应的需求。

Method: 1. 设计异步双通路架构：快通路负责高频动作生成（30Hz），慢通路进行VLM语义推理；2. 引入潜在表示缓冲区，对齐场景-指令上下文，为快通路提供高层指导；3. 设计全身动作分词器，提供紧凑统一的动作表示；4. 保持VLM与动作专家端到端联合训练。

Result: 1. 支持30亿参数VLM的同时实现30Hz全身动作块生成，速度达到同类VLA模型的约3倍；2. 真实世界全身操控实验显示任务成功率提升，响应性显著优于同步基线模型。

Conclusion: DuoCore-FS通过异步执行机制有效解耦语义推理与动作生成，在保持端到端学习优势的同时，显著提升机器人全身操控的实时性能与控制稳定性，已集成于Astribot机器人平台。
...
### [134] [UrbanV2X: A Multisensory Vehicle-Infrastructure Dataset for Cooperative Navigation in Urban Areas](https://arxiv.org/abs/2512.20224)
...
TL;DR: 提出了UrbanV2X数据集，这是一个包含车辆和路边基础设施多传感器数据的C-V2X数据集，用于支持密集城市环境下的智能交通应用研究。


<details>
  <summary>Details</summary>
Motivation: 由于真实世界中支持车辆-基础设施协同导航的复杂城市环境数据集稀缺，限制了C-V2X技术在完全自动驾驶中的应用研究。

Method: 在香港C-V2X测试平台采集车辆和路边基础设施的同步多传感器数据，包括工业相机、激光雷达、4D雷达、UWB、IMU和高精度GNSS-RTK/INS导航系统，使用PTP协议进行时间同步并提供传感器标定数据。

Result: 构建了包含丰富传感器数据的UrbanV2X数据集，并公开提供；通过基准测试评估了协同导航算法的性能。

Conclusion: UrbanV2X数据集填补了复杂城市环境下车辆-基础设施协同导航数据集的空白，为智能交通应用研究提供了重要资源。
...
### [135] [KnowVal: A Knowledge-Augmented and Value-Guided Autonomous Driving System](https://arxiv.org/abs/2512.20299)
*Zhongyu Xia,Wenhao Chen,Yongtao Wang,Ming-Hsuan Yang*

Main category: cs.RO

TL;DR: 提出KnowVal自动驾驶系统，通过开放世界感知与知识检索的协同整合实现视觉语言推理，结合知识图谱和基于LLM的检索机制，并利用价值模型进行轨迹评估，显著提升规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶方法主要依赖数据驱动学习，难以通过模仿或有限奖励捕获决策背后的复杂逻辑，需要解决视觉语言推理、驾驶知识和价值对齐的整合问题。

Method: 构建包含交通法规、防御性驾驶原则和伦理规范的驾驶知识图谱；设计基于LLM的高效检索机制；创建人类偏好数据集并训练价值模型进行可解释的轨迹评估。

Result: 在nuScenes数据集上获得最低碰撞率，在Bench2Drive基准测试中达到最先进水平，同时保持与现有架构的兼容性。

Conclusion: KnowVal系统通过知识增强的视觉语言推理有效提升了自动驾驶的规划性能和价值对齐能力，为复杂决策逻辑的建模提供了新途径。
...
### [136] [Pneumatic bladder links with wide range of motion joints for articulated inflatable robots](https://arxiv.org/abs/2512.20322)
*Katsu Uchiyama,Ryuma Niiyama*

Main category: cs.RO

TL;DR: 提出了一种采用Hillberry关节的多段气动膀胱连杆结构可充气机器人，具有大运动范围和高负载能力，并展示了其在机械臂和腿部运动中的应用。


<details>
  <summary>Details</summary>
Motivation: 探索可充气机器人在多样化应用中的前沿研究，通过新型关节设计解决传统可充气关节运动范围有限的问题。

Method: 设计由双层结构（防水布+聚氨酯）气动膀胱连杆通过滚动接触式Hillberry关节连接的关节式机器人，实现±150°大范围运动。

Result: 3自由度机械臂可搬运500g负载；2自由度和1自由度机械臂分别提升3.4kg和5kg负载；3自由度腿部结构成功实现带推车的行走运动。

Conclusion: Hillberry关节与双层膀胱结构的结合为可充气机器人提供了优异的运动性能和负载能力，拓展了其在复杂任务中的应用潜力。
...
### [137] [FAR-AVIO: Fast and Robust Schur-Complement Based Acoustic-Visual-Inertial Fusion Odometry with Sensor Calibration](https://arxiv.org/abs/2512.20355)
*Hao Wei,Peiji Wang,Qianhao Wang,Tong Qin,Fei Gao,Yulin Si*

Main category: cs.RO

TL;DR: 提出FAR-AVIO框架，一种基于舒尔补的紧耦合声学-视觉-惯性里程计方法，通过EKF嵌入舒尔补实现高效优化，并引入自适应传感器权重调整与在线校准，提升水下机器人定位精度与计算效率。


<details>
  <summary>Details</summary>
Motivation: 水下环境因光线衰减、浑浊水体及弱激励运动导致视觉-惯性里程计易失效，现有声学-视觉-惯性紧耦合方法计算复杂度高，难以在资源受限平台上实时运行。

Method: 1. 在扩展卡尔曼滤波中嵌入舒尔补公式，实现位姿-路标点联合优化并边缘化路标状态以保持恒定时间更新；2. 设计AWARE模块在线评估视觉、惯性及DVL测量可靠性并自适应调整权重；3. 开发无需专用标定动作的DVL-IMU外参在线联合标定方案。

Result: 数值仿真与真实水下实验表明，FAR-AVIO在定位精度和计算效率上均优于现有水下SLAM基线方法，可在低功耗嵌入式平台上稳定运行。

Conclusion: FAR-AVIO通过紧耦合传感器融合与高效优化策略，解决了水下机器人长期运行中的状态估计难题，实现了精度与实时性的平衡，代码已开源。
...
### [138] [Drift-Corrected Monocular VIO and Perception-Aware Planning for Autonomous Drone Racing](https://arxiv.org/abs/2512.20475)
*Maulana Bisyir Azhari,Donghun Han,Je In You,Sungjun Park,David Hyunchul Shim*

Main category: cs.RO

TL;DR: 本文介绍了阿布扎比自主赛车联盟与无人机冠军联赛竞赛中使用的自主无人机竞速系统，该系统仅使用单目摄像头和低质量IMU，通过融合视觉惯性里程计与基于YOLO的闸门检测器输出，结合感知感知规划器，在多项比赛中获得优异成绩。


<details>
  <summary>Details</summary>
Motivation: 竞赛要求仅使用单目摄像头和低质量IMU进行高速自主无人机竞速，这种传感器配置与人类飞手类似，但视觉惯性里程计在高速激烈机动中容易产生漂移，需要解决漂移问题以实现稳定竞速。

Method: 1. 使用卡尔曼滤波器融合视觉惯性里程计输出与基于YOLO的闸门检测器提供的全局位置测量，以校正漂移；2. 设计感知感知规划器，生成平衡速度与闸门可见性的轨迹；3. 构建完整的系统架构用于单目视觉自主无人机飞行。

Result: 系统在竞赛中表现优异：AI Grand Challenge第三名（最高速度43.2 km/h），AI Drag Race第二名（速度超过59 km/h），AI Multi-Drone Race第二名。

Conclusion: 该系统证明了仅使用单目视觉和低质量IMU实现高速自主无人机竞速的可行性，通过传感器融合和感知感知规划有效解决了漂移问题，为基于单目视觉的自主无人机飞行系统提供了成功范例。
...
### [139] [LightTact: A Visual-Tactile Fingertip Sensor for Deformation-Independent Contact Sensing](https://arxiv.org/abs/2512.20591)
*Changyi Lin,Boda Huo,Mingyang Yu,Emily Ruppel,Bingqing Chen,Jonathan Francis,Ding Zhao*

Main category: cs.RO

TL;DR: 提出LightTact光学触觉传感器，通过环境光阻隔机制直接可视化接触，无需依赖表面形变，实现对液体、软材料等轻接触的鲁棒感知。


<details>
  <summary>Details</summary>
Motivation: 现有触觉传感器依赖宏观形变检测接触，难以感知液体、半液体或超软材料等无显著形变的轻接触场景。

Method: 采用环境光阻隔光学结构，抑制非接触区域的内外光源，仅透射真实接触产生的漫反射光，生成高对比度原始图像；基于此实现像素级接触分割。

Result: 传感器非接触像素灰度值<3，接触像素保留表面自然外观；接触分割对材料特性、接触力、表面外观和环境光照具有鲁棒性；成功应用于水面铺展、面霜蘸取、薄膜交互等轻接触操控任务。

Conclusion: LightTact通过光学原理实现形变无关的接触可视化，为轻接触交互提供可靠感知方案，其空间对齐的视觉-触觉图像可直接被视觉语言模型解析，拓展了机器人精细操作能力。
...
### [140] [A Class of Axis-Angle Attitude Control Laws for Rotational Systems](https://arxiv.org/abs/2512.19846)
*Francisco M. F. R. Gonçalves,Ryan M. Bena,Néstor O. Pérez-Arancibia*

Main category: eess.SY

TL;DR: 提出了一种基于欧拉轴角表示的新型姿态控制律，通过李雅普诺夫稳定性理论和扩展K∞函数确保全局渐近稳定性，相比传统四元数方法在高速翻滚恢复机动中具有更短的稳定时间和更低的控制能耗。


<details>
  <summary>Details</summary>
Motivation: 传统四元数姿态控制方法在设计灵活性上存在局限，特别是在需要结合角速度依赖的切换策略时，需要一种更通用的轴角表示框架来提升控制性能。

Method: 基于欧拉轴角表示构建控制律，利用李雅普诺夫稳定性理论和扩展K∞函数分析闭环系统的全局渐近稳定性，并通过切换策略结合系统角速度状态进行控制优化。

Result: 仿真和实时实验表明，在高速翻滚恢复机动中，新方法相比四元数基准方法和几何控制方法，稳定时间更短且控制能耗更低。

Conclusion: 广义轴角方法为旋转系统控制提供了更灵活的设计框架，在动态性能和控制效率方面优于传统方法，特别适用于需要快速稳定和高能效的应用场景。
...
### [141] [HeylandCircle: A Computational Framework for the Geometric Reconstruction of the Heyland Circle Diagram](https://arxiv.org/abs/2512.20015)
*Anubhav Gupta,Abhinav Gupta*

Main category: eess.SY

TL;DR: 本文提出了HeylandCircle计算框架，将传统的Heyland圆图从手工绘图工具转化为标准化计算模型，可直接根据电机测试参数重构圆图并计算稳态性能指标。


<details>
  <summary>Details</summary>
Motivation: Heyland圆图作为感应电机稳态分析的经典图形工具，长期依赖手工绘制且缺乏标准化计算表述，限制了其可重复性和系统性应用。

Method: 建立确定性几何操作序列的计算框架，将传统图解构造转化为可编程的几何运算，通过测量参数与几何对象的映射关系推导功率因数、转差率、输出功率等性能指标。

Result: 使用典型教科书案例验证表明，该框架计算结果与传统方法高度一致，实现了Heyland圆图的可计算化重构。

Conclusion: HeylandCircle框架为传统Heyland圆图提供了计算实现，适用于教学、工程分析和系统性扩展，提升了该经典方法的可重复性和实用性。
...
### [142] [From Dissipativity Property to Data-Driven GAS Certificate of Degree-One Homogeneous Networks with Unknown Topology](https://arxiv.org/abs/2512.20076)
*Abolfazl Lavaei,David Angeli*

Main category: eess.SY

TL;DR: 提出了一种数据驱动的分治策略，用于分析模型和拓扑完全未知的互联齐次非线性网络的稳定性。该方法通过子系统轨迹数据构建存储函数，并组合成李雅普诺夫函数，为未知互联网络提供稳定性证书。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法在处理未知模型和拓扑的互联网络稳定性分析时面临样本复杂度高、只能提供概率性置信保证的问题，需要一种能提供确定性保证且高效的方法。

Method: 1) 将存储函数构建条件表述为鲁棒凸规划问题；2) 基于子系统轨迹数据建立场景凸规划并转化为线性规划求解；3) 利用新提出的数据驱动组合性条件，将子系统存储函数组合成互联网络的李雅普诺夫函数。

Result: 1) 提出的分治策略提供了正确性保证（非概率性置信）；2) 显著缓解了数据驱动方法中的样本复杂度问题；3) 在三个不同案例研究中成功验证了未知互联网络的全局渐近稳定性。

Conclusion: 该方法通过数据驱动的分治策略，为模型和拓扑完全未知的互联齐次非线性网络提供了具有确定性保证的稳定性分析框架，有效平衡了计算复杂度和可靠性。
...
### [143] [Joint Design of Embedded Index Coding and Beamforming for MIMO-based Distributed Computing via Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2512.20201)
*Heekang Song,Wan Choi*

Main category: eess.SY

TL;DR: 本文提出了一种多智能体强化学习框架，用于联合优化MIMO系统中的嵌入式索引编码和波束成形，以最小化传输时间，相比传统方法显著降低了复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入式索引编码研究主要关注有线无差错环境中的码长最小化，这在考虑信道条件和空间复用增益的无线MIMO系统中并非最优，需要联合设计编码和波束成形以优化系统性能。

Method: 首先提出基于穷举搜索的传统优化方法，然后针对其高复杂度和动态无线环境适应性不足的问题，设计了一种低复杂度的多智能体强化学习框架，支持基于局部观测的分布式决策，并能处理离散编码选择和连续波束成形的混合动作空间。

Result: 仿真结果表明，所提出的MARL方法能够实现接近最优的性能，同时显著降低了计算复杂度，证明了其在真实无线系统中的有效性和实用性。

Conclusion: 多智能体强化学习框架为MIMO系统中嵌入式索引编码和波束成形的联合优化提供了一种高效且实用的解决方案，特别适用于动态无线环境。
...
### [144] [Robust safety design for strict-feedback nonlinear systems via observer-based linear time varying feedback](https://arxiv.org/abs/2512.20226)
*Imtiaz Ur Rehman,Moussa Labbadi,Amine Abadi,Lew Lew Yan Voon*

Main category: eess.SY

TL;DR: 提出了一种针对具有不匹配扰动的非线性严格反馈系统的鲁棒安全临界控制方法，通过状态变换和线性时变扰动观测器确保安全集的前向不变性。


<details>
  <summary>Details</summary>
Motivation: 非线性严格反馈系统在实际应用中常受到不匹配扰动的影响，传统安全控制方法难以保证系统安全性，需要开发能够处理此类扰动的鲁棒控制策略。

Method: 采用状态变换将系统转换为适合安全控制设计的形式，结合线性时变扰动观测器估计和补偿扰动，设计确保安全集前向不变性的控制律。

Result: 该方法在存在不匹配扰动和扰动自由情况下均能保证系统安全性，数值仿真验证了控制方法的有效性。

Conclusion: 所提出的方法为非线性严格反馈系统提供了一种鲁棒的安全控制解决方案，能够处理不匹配扰动并确保系统安全运行。
...
### [145] [Finite-Time Control Based on Differential Flatness for Wheeled Mobile Robots with Experimental Validation](https://arxiv.org/abs/2512.20229)
*Imtiaz Ur Rehman,Moussa Labbadi,Amine Abadi,Lew Lew Yan Voon*

Main category: eess.SY

TL;DR: 提出一种基于积分非线性超平面的滑模控制方法，用于增强轮式移动机器人在干扰环境下的路径跟踪能力，并通过实际实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 轮式移动机器人在强风、不平整路面等干扰下跟踪性能下降，需要一种能在实际场景中鲁棒运行的跟踪控制方法。

Method: 1. 利用运动学模型的微分平坦性将系统转换为线性规范型；2. 提出新型积分非线性超平面滑模控制方法；3. 进行稳定性分析与验证；4. 在TurtleBot3平台上进行对比实验。

Result: 所提方法在存在干扰的实际室内环境中表现出良好的跟踪性能，验证了其可行性和有效性。

Conclusion: INH-SMC方法能有效提升轮式移动机器人在干扰环境下的路径跟踪鲁棒性，具备实际应用潜力。
...
### [146] [Sliding Mode Control for a Parabolic-Elliptic PDE System with Boundary Perturbation](https://arxiv.org/abs/2512.20244)
*Moussa Labbadi,Ilyasse Lamrani*

Main category: eess.SY

TL;DR: 本文研究了边界控制下抛物-椭圆系统的鲁棒性，提出了一种滑模控制策略来抑制匹配扰动，证明了滑动流形的有限时间收敛性和闭环系统的指数稳定性，并通过数值算例验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于提高边界控制下抛物-椭圆系统对匹配扰动的鲁棒性，确保系统在存在扰动时的稳定性和控制性能。

Method: 采用滑模控制策略处理匹配扰动，通过稳定性分析证明滑动流形的有限时间收敛性，并建立闭环系统的指数稳定性，同时证明了不连续闭环系统的适定性。

Result: 主要结果包括：滑动流形在有限时间内收敛，闭环系统具有指数稳定性，且不连续系统的适定性得到证明，数值算例验证了控制策略的有效性。

Conclusion: 结论表明所提出的滑模控制策略能有效抑制边界控制下抛物-椭圆系统的匹配扰动，确保系统的鲁棒稳定性和有限时间收敛性。
...
### [147] [Inference in Latent Force Models Using Optimal State Estimation](https://arxiv.org/abs/2512.20250)
*Tobias M. Wolff,Victor G. Lopez,Matthias A. Müller,Thomas Beckers*

Main category: eess.SY

TL;DR: 本文提出了两种最优状态估计框架，用于重构隐力模型中的未知输入力并估计系统状态，特别考虑了系统固有约束，并在数值和生物医学实例中验证了性能。


<details>
  <summary>Details</summary>
Motivation: 现有隐力模型方法在估计时未充分考虑系统固有约束，限制了其在具有物理限制的实际系统中的应用。

Method: 设计了两种基于隐力模型的最优状态估计框架，将未知输入建模为高斯过程，并整合系统动力学约束进行状态重构。

Result: 新框架在多个数值示例中表现出色，特别是在下丘脑-垂体-甲状腺轴生物医学实例中，能够有效利用激素测量数据估计状态和隐力。

Conclusion: 所提出的约束感知估计框架扩展了隐力模型的应用范围，为具有物理约束的混合建模问题提供了有效的解决方案。
...
### [148] [Data-based Moving Horizon Estimation under Irregularly Measured Data](https://arxiv.org/abs/2512.20259)
*Tobias M. Wolff,Isabelle Krauss,Victor G. Lopez,Matthias A. Müller*

Main category: eess.SY

TL;DR: 提出一种基于样本和数据的线性系统移动水平估计框架，适用于测量稀少且不规则的应用场景，不依赖标准数学模型，而是基于测量数据的隐式系统表示。


<details>
  <summary>Details</summary>
Motivation: 针对测量成本高或耗时长（如医疗监测）的应用场景，传统状态估计方法需要大量规则采样数据，而实际中往往只能获得少量不规则测量数据，需要开发适应这种限制的估计方法。

Method: 采用基于样本和数据的移动水平估计框架，利用少量不规则输出测量值，通过数据驱动的隐式系统表示替代传统数学模型，并证明了估计器的样本实用鲁棒指数稳定性。

Result: 在温和假设下证明了估计器的样本实用鲁棒指数稳定性，并将该方法成功应用于胃肠道吸收系统的状态估计，验证了其有效性。

Conclusion: 所提出的数据驱动估计框架能够有效处理测量稀少且不规则的实际问题，为高成本或耗时测量应用提供了可行的状态估计解决方案。
...
### [149] [Divergence Method to Stability Study of Andronov-Vyshnegradsky Problem. Hidden Oscillations](https://arxiv.org/abs/2512.20418)
*I. B. Furtat,N. V. Kuznetsov*

Main category: eess.SY

TL;DR: 使用发散方法解决了经典安德罗诺夫-维什涅格拉茨基问题，确定了瓦特调节器控制系统的稳定性和振荡区域，获得了全局稳定性区域的精确隐藏边界。


<details>
  <summary>Details</summary>
Motivation: 经典安德罗诺夫-维什涅格拉茨基问题涉及瓦特调节器控制系统的稳定性和振荡区域定位，需要研究系统在有/无自调节效应下的行为，并确定全局稳定性的精确边界。

Method: 采用发散方法研究动态系统稳定性，结合线性矩阵不等式可解性分析稳定性准则，并通过计算机建模验证系统行为。

Result: 获得了全局稳定性区域的精确隐藏边界值；计算机建模显示系统在有无自调节效应时均存在隐藏振荡；瓦特调节器模型的三个参数决定了计算全局稳定性隐藏边界的条件。

Conclusion: 通过发散方法和线性矩阵不等式分析，成功解决了瓦特调节器控制系统的稳定性区域定位问题，确定了全局稳定性的精确边界条件，为控制系统稳定性分析提供了有效工具。
...
### [150] [Fast Fixed-time Convergence in Nonlinear Dynamical Systems](https://arxiv.org/abs/2512.20427)
*Igor B. Furtat*

Main category: eess.SY

TL;DR: 提出了一种非线性动力系统固定时间快速收敛方法，通过特定时间区间内对二次函数导数施加负幂次实现快速收敛，并应用于反步法控制设计。


<details>
  <summary>Details</summary>
Motivation: 现有方法在收敛速度和收敛时间估计方面存在不足，需要开发能保证固定时间内收敛且收敛速度更快的控制策略。

Method: 基于二次函数导数满足特定条件的非线性系统，在系统演化过程中对二次函数导数施加负幂次；利用反步法设计任意阶线性对象的控制律。

Result: 获得了系统解在固定时间内收敛到零和给定区域的条件；数值模拟验证了所提方法相比现有方案具有更快的收敛速度。

Conclusion: 所提出的固定时间收敛方法能有效提高系统收敛速度，适用于非线性动力系统控制设计，并通过反步法实现了对任意阶线性对象的控制。
...
### [151] [Modeling Economic Systems as Multiport Networks](https://arxiv.org/abs/2512.20600)
*Coen Hutters,Max B. Mendel*

Main category: eess.SY

TL;DR: 本文展示了如何将多端口网络理论作为经济学建模工具，通过端口概念将商品流（电流）与主体激励（电压）配对，构建从宏观到微观的多层次抽象模型，并利用电路仿真软件分析从简单到复杂的经济系统。


<details>
  <summary>Details</summary>
Motivation: 传统经济学模型在处理复杂宏观经济系统时，难以有效整合微观主体行为与宏观涌现现象，需要一种能够统一描述多层次经济互动的建模框架。

Method: 采用多端口网络理论，将经济主体间的交互建模为端口连接（商品流对应电流，激励对应电压），构建多层次网络模型，并使用LTSpice电路仿真软件进行系统设计与分析。

Result: 成功构建了从经典的鲁滨逊经济到完整经济体的系列模型，展示了该方法能够有效模拟微观行为如何涌现为宏观经济动态，验证了网络理论对经济系统建模的适用性。

Conclusion: 多端口网络理论为经济学提供了强大的跨层次建模工具，能够统一描述经济系统的微观机制与宏观现象，为复杂经济动态分析开辟了新途径。
...
### [152] [Neural Compression of 360-Degree Equirectangular Videos using Quality Parameter Adaptation](https://arxiv.org/abs/2512.20093)
*Daichi Arai,Yuichi Kondo,Kyohei Unno,Yasuko Sugito,Yuichi Kusakabe*

Main category: eess.IV

TL;DR: 提出一种无需重新训练即可压缩360度全景视频的实用方法，通过纬度自适应质量参数和潜在调制技术，在DCVC-RT框架上实现5.2%的BD-Rate节省。


<details>
  <summary>Details</summary>
Motivation: 传统神经视频压缩模型处理360度等距柱状投影视频时，未考虑投影中空间采样密度的变化，导致压缩效率不高。

Method: 将传统视频编码中的量化参数自适应技术扩展到神经视频压缩，采用基于纬度的自适应质量参数，并通过向量库插值实现潜在调制以支持任意质量参数。

Result: 在JVET S1测试序列上，加权球面峰值信噪比获得5.2%的BD-Rate节省，处理时间仅增加0.3%。

Conclusion: 该方法有效提升了神经视频压缩模型对360度视频的压缩效率，且无需修改模型架构或重新训练，具有实用价值。
...
### [153] [CLIP Based Region-Aware Feature Fusion for Automated BBPS Scoring in Colonoscopy Images](https://arxiv.org/abs/2512.20374)
*Yujia Fu,Zhiyu Dong,Tianwen Qian,Chenye Zheng,Danian Ji,Linhai Zhuo*

Main category: eess.IV

TL;DR: 本文提出了一种基于CLIP模型和适配器迁移学习的自动化BBPS评分框架，通过融合全局视觉特征和粪便相关文本先验，无需显式分割即可准确评估肠道清洁度。


<details>
  <summary>Details</summary>
Motivation: 波士顿肠道准备量表（BBPS）在手动评分时存在主观性和观察者间差异，需要一种客观、自动化的评估方法来提高结肠镜检查中肠道清洁度评估的准确性和一致性。

Method: 构建了包含2,240张图像的高质量结肠镜数据集；提出基于CLIP模型的适配器迁移学习框架，结合专用的粪便特征提取分支，融合视觉特征与文本先验进行BBPS评分。

Result: 在自建数据集和公开NERTHU数据集上的实验表明，该方法优于现有基线方法，展示了在计算机辅助结肠镜分析中的临床部署潜力。

Conclusion: 所提出的自动化BBPS评分框架能够有效减少人工评分的主观性，提高肠道清洁度评估的准确性和可靠性，具有临床推广应用价值。
...
TL;DR: 本文提出了一种基于双编码器TransUNet的缺血性卒中病灶分割方法，在ISLES 2022数据集上取得了最佳性能，Dice分数达到85.4%。


<details>
  <summary>Details</summary>
Motivation: 扩散磁共振成像（DWI和ADC）对急性及亚急性缺血性病变具有互补信息，但由于病灶外观的变异性，自动分割仍然具有挑战性，而准确的病灶分割对临床决策和预后评估至关重要。

Method: 研究了多种卷积和Transformer架构（包括U-Net变体、Swin-UNet和TransUNet），提出双编码器TransUNet从DWI和ADC输入中学习模态特异性表示，并采用三切片输入配置整合相邻切片的空间上下文信息。

Result: 基于Transformer的模型优于卷积基线，所提出的双编码器TransUNet在测试集上达到85.4%的Dice相似系数，表现最佳。

Conclusion: 该框架为扩散MRI的缺血性卒中病灶自动分割提供了稳健的解决方案，Transformer架构在医学图像分割任务中展现出优势。
...
### [155] [Branch Learning in MRI: More Data, More Models, More Training](https://arxiv.org/abs/2512.20330)
*Yuyang Li,Yipin Deng,Zijian Zhou,Peng Hu*

Main category: eess.IV

TL;DR: 研究两种多对比度心脏磁共振重建策略：物理一致的数据空间增强（DualSpaceCMR）和参数高效容量扩展（VQPrompt和Moero）。在CMRxRecon25基准测试中评估小样本和分布外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决心脏磁共振重建中数据稀缺和泛化能力不足的问题，探索数据增强与模型容量扩展的互补策略，以提升小样本和跨中心场景下的重建性能。

Method: 1. DualSpaceCMR：结合图像级变换与k空间噪声及运动模拟，保持前向模型一致性；2. VQPrompt：添加轻量级瓶颈提示模块；3. Moero：在深度展开网络中嵌入稀疏混合专家模块，采用基于直方图的路由机制。

Result: 1. 小数据集上k空间运动加噪声增强提升重建性能，但在大规模基准测试中性能下降，显示对增强比例和调度敏感；2. VQPrompt带来稳定小幅性能提升且内存开销可忽略；3. Moero在早期平台期后持续改进，保持接近基线的泛化能力，但稀疏路由降低计算吞吐量，使实际运行时间成为瓶颈。

Conclusion: 数据增强需考虑规模敏感性，提示式容量扩展是实用路径，稀疏专家模型需优化效率以发挥潜力。
