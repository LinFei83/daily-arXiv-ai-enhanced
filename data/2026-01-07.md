...
TL;DR: 提出一种新颖的可解释强化学习框架，通过大语言模型生成文本解释并转化为透明规则，结合专家知识和自动谓词生成器提升解释质量，在开源环境和电信用例中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有可解释强化学习技术生成的文本解释难以保证正确性且评估有限，需要一种系统化方法生成可靠、可评估的文本解释以满足人类对自主代理行为的理解需求。

Method: 使用大语言模型生成文本解释，通过聚类识别频繁条件并转化为透明规则；提出两种精炼技术减少冲突信息；结合专家知识和自动谓词生成器确定状态语义信息。

Result: 框架在三个开源环境中实现可复现性，在电信用例中验证工业适用性；生成的透明规则在特定任务中表现良好，能系统化定量评估文本解释。

Conclusion: 该框架解决了现有自主策略解释方法的局限性，为可解释强化学习领域提供了系统化的文本解释生成与评估方法，具有实际应用价值。
...
TL;DR: 提出SimpleMem框架，通过语义无损压缩实现高效记忆管理，提升LLM代理在复杂环境中的长期交互能力


<details>
  <summary>Details</summary>
Motivation: 现有方法存在冗余或高成本问题：要么被动扩展上下文导致冗余，要么依赖迭代推理带来高token开销

Method: 三阶段流水线：1) 语义结构化压缩，通过熵感知过滤将非结构化交互压缩为紧凑记忆单元；2) 递归记忆整合，异步整合相关单元为高层抽象表示；3) 自适应查询感知检索，根据查询复杂度动态调整检索范围

Result: 在基准数据集上表现优于基线方法，平均F1提升26.4%，推理时token消耗降低高达30倍

Conclusion: SimpleMem在性能与效率间取得优越平衡，代码已开源
...
TL;DR: Orchestral是一个轻量级Python框架，旨在解决多LLM提供商集成中的碎片化问题，提供统一的类型安全接口来构建可移植的LLM智能体系统。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体框架面临两大问题：一是供应商锁定（使用特定SDK），二是复杂的多包生态系统导致控制流不透明、可复现性差。不同LLM提供商在API、消息格式、流式处理和工具调用行为上存在碎片化，使得构建可移植、可靠的智能体系统变得困难。

Method: 1. 定义消息、工具和LLM使用的通用表示形式，实现跨提供商无缝操作；2. 通过Python类型提示自动生成工具模式，无需手动编写描述符；3. 采用支持流式处理的同步执行模型，确保确定性行为；4. 采用模块化架构，清晰分离提供商集成、工具执行、对话编排和用户界面。

Result: Orchestral框架实现了：消除手动格式转换、减少框架引入的复杂性、保持跨提供商边界的类型安全、支持确定性调试和实时交互，同时支持高级智能体功能（如丰富工具调用、上下文压缩、工作区沙盒、用户审批流程、子智能体、内存管理和MCP集成）。

Conclusion: Orchestral提供了一个轻量级解决方案，统一了多LLM提供商的接口，在保持科学计算和生产部署所需简洁性的同时，解决了智能体开发中的可移植性和可靠性挑战。
...
TL;DR: 本文研究了设备端AI模型实际部署中的关键问题，包括模型选择、资源消耗和领域适应能力，通过构建LiveChatBench基准测试并在五款移动设备上实验，证明了设备端模型在特定任务上能达到与商业模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管设备端AI模型效率高，但实际部署中涉及设备CPU利用率和发热等现实因素的研究较少，需要解决模型选择、资源消耗和领域适应能力等问题以实现真实场景的应用。

Method: 通过大量实验研究两个关键问题：设备端模型选择与资源消耗、设备端模型的领域适应能力；构建了包含1000个韩英平行句对的LiveChatBench基准测试；在五款移动设备上进行实验验证。

Result: 实验表明，虽然服务大规模异构用户群需要考虑高度受限的部署环境和模型选择，但所提出的方法在针对性任务上达到了与GPT-5.1等商业模型相当的性能。

Conclusion: 设备端AI模型在充分考虑部署约束和模型选择的情况下，能够在特定领域任务中实现与先进商业模型媲美的性能，为设备端AI社区提供了有价值的实践见解。
...
TL;DR: 本文提出将不可行查询处理视为偏好感知的查询修复问题，开发了三种基于LLM的约束重要性推断方法，并创建了AWARE-US基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有工具调用对话代理在处理结构化数据库查询时，常面临两个关联问题：欠规范化和查询不可行性。传统方法要么返回“无结果”，要么使用临时规则放松约束，这可能违反用户意图。

Method: 提出了三种基于LLM的约束重要性推断方法：1) 局部加权，2) 全局一次性加权，3) 成对排序。同时创建了AWARE-US基准测试，包含基于角色的查询，要求代理通过对话消歧并以符合角色偏好的方式解决不可行性。

Result: 实验表明，局部加权在偏好对齐方面表现最佳，而全局加权在正确放松约束方面表现最好。

Conclusion: 将不可行查询处理视为偏好感知的查询修复问题是有效的，不同方法在不同指标上各有优势，AWARE-US基准为评估此类系统提供了新工具。
...
TL;DR: 提出GTL-CIRL框架，通过同时学习策略和挖掘因果图时序逻辑规范，提升图时空决策任务的样本效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 黑盒强化学习常忽略局部变化在网络结构中的传播机制，导致样本效率低且缺乏可解释性，难以处理图时空动态决策任务。

Method: 结合因果图时序逻辑规范挖掘与策略学习，通过鲁棒性塑造奖励、收集失效反例，并采用高斯过程贝叶斯优化精调参数化因果模板，利用GP捕获系统动态的时空相关性。

Result: 在基因网络和电力网络的案例研究中，相比标准RL基线，该方法实现了更快的学习速度和更清晰、可验证的行为表现。

Conclusion: GTL-CIRL框架能有效提升图时空决策任务的样本效率和可解释性，其因果逻辑规范挖掘与优化机制为复杂网络系统的决策提供了新思路。
...
TL;DR: 提出HAPO框架解决提示优化中的提示漂移和可解释性问题，通过动态归因机制和语义单元优化提升多模态任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法存在提示漂移问题（新提示修复旧错误但损害原有任务表现），且从零生成提示会降低可解释性，需要更结构化的优化方法。

Method: 提出分层归因提示优化（HAPO）框架，包含：1）针对训练数据和提示历史的动态归因机制；2）编辑功能提示段的语义单元优化；3）支持LLM和LLM-MLLM工作流的多模态友好流程。

Result: 在单/多图像QA（如OCRV2）和复杂任务分析（如BBH）中，HAPO显示出更高的优化效率，优于同类自动提示优化方法。

Conclusion: HAPO建立了可扩展的提示工程范式，能有效缓解提示漂移，保持可解释性，并支持多模态应用。
...
TL;DR: 提出MultiSessionCollab基准测试，评估智能体在多轮会话中学习用户偏好并提升协作质量的能力，通过引入记忆机制显著改善长期协作效果。


<details>
  <summary>Details</summary>
Motivation: 随着对话智能体与用户协作经验的积累，适应用户偏好对建立长期关系和提升协作质量至关重要，需要评估和改进智能体在多轮会话中的持续学习能力。

Method: 1. 构建MultiSessionCollab基准测试框架；2. 为智能体设计可持久化并动态优化用户偏好的记忆机制；3. 从用户模拟器中提取学习信号，训练智能体生成更全面的反思并优化记忆更新策略。

Result: 实验表明：1. 配备记忆的智能体显著提升长期协作效果；2. 任务成功率提高，交互效率提升，用户努力程度降低；3. 人类用户研究证实记忆机制在真实场景中改善用户体验。

Conclusion: 记忆机制是提升智能体长期协作能力的关键，MultiSessionCollab基准为相关研究提供有效评估工具，基于用户模拟器的学习信号训练方法能有效优化智能体的反思与记忆更新策略。
...
TL;DR: 论文提出时间尺度化（Time-Scaling）概念，旨在通过扩展和优化智能体在时间维度上的推理能力，实现更深层次的问题解决，而无需大幅增加静态模型参数。


<details>
  <summary>Details</summary>
Motivation: 早期人工智能范式（神经网络、强化学习、符号AI）功能分离，而当前基于Transformer的大模型虽能生成流畅文本，但在深层语义推理方面仍有不足。现有提示技术（如CoT、ToT）在搜索完整性和效率上存在局限，因此需要系统性地提升智能体在时间维度上的推理能力。

Method: 提出时间尺度化作为架构设计原则，通过扩展时间路径、支持动态策略调整和增强元认知控制，模拟人类在认知资源有限下的序列化推理过程。

Result: 时间尺度化能够实现更深层次的问题空间探索、动态策略调整和增强的元认知控制，为提升深度推理和问题解决能力提供关键方向。

Conclusion: 时间尺度化是增强智能体深度推理能力的关键前沿，应将显式时间推理管理作为智能体设计的核心基础，以推动智能体能力的进一步发展。
...
### [10] [LLM Agent Framework for Intelligent Change Analysis in Urban Environment using Remote Sensing Imagery](https://arxiv.org/abs/2601.02757)
*Zixuan Xiao,Jun Ma*

Main category: cs.AI

TL;DR: 提出ChangeGPT框架，集成大语言模型与视觉基础模型，用于遥感变化检测，在多样化查询中表现优异，准确率达90.71%。


<details>
  <summary>Details</summary>
Motivation: 现有变化检测方法缺乏处理多样化真实世界查询的通用性和综合分析能力，需要更智能、自适应的解决方案。

Method: 采用分层结构的大语言模型代理框架（ChangeGPT），集成视觉基础模型，通过工具选择机制减少幻觉，并在包含140个问题的数据集上进行评估。

Result: ChangeGPT（尤其GPT-4-turbo后端）在工具选择（精确率/召回率）和整体查询准确率（Match）上表现优异，达到90.71%的Match率，特别擅长需要多步推理的变化查询。

Conclusion: ChangeGPT通过提供智能化、自适应性和多类型变化分析能力，为遥感应用决策提供了强大解决方案，并在深圳前海湾的实际案例中验证了其有效性。
...
TL;DR: 本章探讨了大型语言模型从被动文本生成器向自主、目标驱动的智能体系统的演变，分析了实现自主行为所需的核心架构组件、技术挑战和研究方向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型正从单纯的语言理解系统向能够自主规划、使用工具和持续学习的智能体转变，这一转变代表了人工智能的根本性变革，但同时也带来了技术、安全和伦理上的新挑战，需要系统性的研究和框架来指导其负责任的发展。

Method: 通过分析LLM架构的演进历程，识别出实现智能体行为的关键能力（如长程推理、情境感知、自适应决策），并提出一个集成框架，该框架包含感知、记忆、规划、工具执行等核心组件，以连接LLM与自主行为。

Result: 提出了三个主要贡献：1) 综合阐述了LLM如何通过推理-行动-反思循环实现智能体能力；2) 构建了一个描述智能体核心组件的集成框架；3) 批判性评估了应用现状及在安全性、对齐性、可靠性和可持续性方面的持续挑战。

Conclusion: 要实现LLM向自主智能体的负责任演进，必须解决可验证规划、可扩展多智能体协调、持久记忆架构和治理框架等关键技术缺口，同时需要在技术鲁棒性、可解释性和伦理保障方面同步推进，以在发挥其潜力的同时降低错位和意外后果的风险。
...
TL;DR: 提出了HAL框架，通过可解释的数据驱动奖励信号对齐语言模型与人类对话相似性，使模型在保持整体性能的同时更显人类化。


<details>
  <summary>Details</summary>
Motivation: 当前人类与AI交互中，对话的人类相似性难以定义、测量和优化，改进主要依赖规模化或宽泛监督训练，缺乏针对性对齐方法。

Method: 从对比对话数据中提取显式对话特征，组合成紧凑标量分数作为透明奖励信号，使用标准偏好优化方法进行模型对齐。

Result: 对齐后的模型在不同规模下均保持整体性能，大规模人类评估显示其更频繁被视为人类化对话；框架支持对齐行为检查和副作用诊断。

Conclusion: HAL展示了如何将语言中软性、定性属性（传统上难以对齐）转化为可测量、可对齐的指标，实现可解释和可说明的对齐过程。
...
TL;DR: 提出了一种结合因果图与检索增强生成的系统，用于解决系统综述中AI幻觉问题，在痴呆症运动研究中实现了95%的准确率和零幻觉。


<details>
  <summary>Details</summary>
Motivation: 系统综述对循证医学至关重要，但每年150万+的出版物无法人工处理。现有AI方法在系统综述任务中存在幻觉问题（2-40%的错误率），这在影响患者护理时不可接受。

Method: 开发了因果图增强的检索增强生成系统，整合显式因果推理与双层知识图谱。采用证据优先协议，每个因果主张都追溯到检索到的文献，并自动生成可视化干预-结果路径的有向无环图。

Result: 在234篇痴呆症运动摘要评估中，CausalAgent达到95%的准确率、100%检索成功率和零幻觉，而基线AI仅34%准确率和10%幻觉率。自动因果图支持显式机制建模、可视化合成和增强可解释性。
...
TL;DR: 本研究首次将变分量子电路引入LSTM注意力模型，提出量子增强的QLSTMA模型，显著提升了储层渗透率等复杂地质参数的预测精度。


<details>
  <summary>Details</summary>
Motivation: 储层参数（尤其是渗透率）的空间预测对油气勘探开发至关重要，但传统方法难以可靠预测其大范围高变异性的特征。

Method: 设计两种量子化结构（共享门QLSTMA-SG和独立门QLSTMA-IG），将变分量子电路嵌入循环单元，利用量子纠缠和叠加原理增强模型能力，并探究量子结构配置和量子比特数对性能的影响。

Result: 8量子比特的QLSTMA-IG模型显著优于传统LSTMA，MAE降低19%，RMSE降低20%，在测井数据复杂区域表现尤为突出；增加量子比特数可进一步提升精度。

Conclusion: 量子-经典混合神经网络在储层预测中具有巨大潜力，本研究为未来在真实量子硬件上部署此类模型及拓展至石油工程与地球科学更广应用奠定了框架基础。
...
TL;DR: 提出一种神经符号深度强化学习方法，通过整合符号知识提升样本效率和泛化能力，在复杂任务中加速学习过程。


<details>
  <summary>Details</summary>
Motivation: 现有深度强化学习方法需要大量训练数据且泛化能力有限，难以适应超出训练范围的复杂任务场景。

Method: 将简单任务中学习到的部分策略表示为逻辑规则，通过在线推理在训练过程中引导智能体：探索阶段偏置动作分布，利用阶段重新缩放Q值。

Result: 在网格世界环境（完全可观测和部分可观测）的挑战性变体上验证了方法有效性，性能优于先进的奖励机制基线方法。

Conclusion: 神经符号整合方法能提升可解释性和可信度，加速收敛过程，特别适用于稀疏奖励环境和长规划视野任务。
...
TL;DR: 本文提出了M3MAD-Bench，一个用于评估多智能体辩论（MAD）方法的统一可扩展基准，覆盖多领域任务、多模态输入和多维度指标，解决了现有评估碎片化和单模态限制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MAD研究存在两个根本局限：评估在碎片化且不一致的设置下进行，阻碍公平比较；且主要局限于仅依赖文本输入的单模态场景。

Method: 构建M3MAD-Bench基准，在知识、数学、医学、自然科学和复杂推理五个核心任务领域建立标准化协议，系统覆盖纯文本和视觉语言数据集。在九个不同架构、规模和模态能力的基模型上评估MAD方法，并引入效率导向指标（如令牌消耗和推理时间）。

Result: 大量实验系统揭示了MAD在纯文本和多模态场景下的有效性、鲁棒性和效率，提供了性能与成本权衡的整体视图。
...
TL;DR: 提出SimRPD框架，通过用户模拟器生成大规模对话数据，结合多维评估筛选高质量数据，训练招聘场景的任务导向主动对话代理，在真实场景中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 招聘等业务场景中，任务导向主动对话代理需要高质量领域数据，但此类数据稀缺，限制了监督微调和强化学习的效果。

Method: 三阶段框架：1）开发高保真用户模拟器合成多轮对话数据；2）基于意图链（CoI）设计全局和实例级多维评估框架筛选高质量数据；3）用筛选数据训练招聘主动对话代理。

Result: 在真实招聘场景实验中，SimRPD优于现有基于模拟器的数据选择策略，证明了工业部署价值及其他业务对话场景的适用潜力。

Conclusion: SimRPD通过模拟数据生成与智能筛选，缓解了领域数据稀缺问题，为业务导向对话系统提供了可扩展的解决方案。
...
TL;DR: 提出ReTreVal框架，结合树状思维探索、自我精炼、LLM批判评分和反思记忆，提升大语言模型在数学和创意写作等复杂领域的多步推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如ReAct、Reflexion、Self-Refine）在多步推理中缺乏对替代解决方案的结构化探索和跨问题的持续学习能力，限制了在复杂任务中的表现。

Method: 构建基于问题复杂度的自适应深度推理树，每个节点通过LLM生成的反馈进行迭代自我批判和精炼；采用双重验证机制评估推理质量，并通过反思记忆缓冲区存储成功路径和失败模式；基于批判的剪枝保留每层最高分节点以控制计算成本。

Result: 在500个数学问题和创意写作任务上使用Qwen 2.5 7B进行评估，ReTreVal在结构化探索、批判驱动精炼和跨问题记忆的综合作用下，持续优于ReAct、Reflexion和Self-Refine等方法。

Conclusion: ReTreVal通过结合结构化探索、验证机制和知识迁移，特别适用于需要探索性推理、严格验证和跨任务学习的复杂问题，为多步推理提供了有效解决方案。
...
TL;DR: 该研究发现了大语言模型在逻辑推理中存在的'逻辑相变'现象，并提出神经符号课程调优框架来缓解高复杂度下的推理崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 符号逻辑推理是LLMs在数学推理和法律判决等高风险领域的关键能力，但现有研究对其在逻辑复杂度增加时的表现缺乏系统分析。

Method: 提出神经符号课程调优框架：通过自适应对齐自然语言与逻辑符号建立共享表示，围绕相变边界重塑训练动态，逐步增强深层逻辑推理能力。

Result: 在五个基准测试中，该方法有效缓解了高复杂度下的逻辑推理崩溃，在基础提示和思维链提示下分别获得平均+1.26和+3.95的准确率提升，并改善了未见逻辑组合的泛化能力。

Conclusion: 逻辑相变现象揭示了LLMs逻辑推理能力的突变特性，而神经符号课程调优能系统性提升模型在复杂逻辑场景下的鲁棒性和泛化性能。
...
TL;DR: 提出Batch-of-Thought（BoT）方法，通过批量处理相关查询实现跨实例学习，提升大语言模型的推理能力与效率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型推理系统独立处理查询，忽略了跨实例信号（如共享推理模式和一致性约束），导致信息利用不足。

Method: 采用无需训练的BoT方法进行批量联合查询处理，结合多智能体反思架构（BoT-R），通过比较分析识别高质量推理模板、一致性检查纠错，并分摊计算成本。

Result: 在三个模型系列和六个基准测试中，BoT-R持续提升准确率和置信度校准，同时将推理成本降低最高61%。

Conclusion: 批量感知推理能有效利用跨实例信息，提升LLM系统的性能与效率，研究揭示了该方法适用的条件与机制。
...
TL;DR: 提出RationaleTS方法，通过基于原理的上下文学习改进时间序列推理，避免模型依赖表面模式匹配，提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在时间序列推理中表现不佳，主要因为缺乏连接时间观测与下游结果的原理先验，导致模型依赖表面模式匹配而非原则性推理。

Method: 首先诱导标签条件化原理（从可观测证据到潜在结果的推理路径），然后设计混合检索机制平衡时间模式和语义上下文，以检索相关原理先验用于新样本的上下文推理。

Result: 在三个领域的时间序列推理任务上进行广泛实验，证明了RationaleTS方法的有效性和效率。

Conclusion: RationaleTS通过将原理作为指导性推理单元而非事后解释，显著提升了时间序列推理性能，代码将开源以供复现。
...
TL;DR: 提出了一种用于供水管网泄漏检测的可解释图神经网络框架，结合互信息识别关键区域和模糊逻辑提供规则化解释，在保持较高检测性能的同时增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络在供水管网泄漏检测中虽能捕捉时空依赖关系，但其黑盒特性及缺乏针对水网的可解释图模型限制了实际应用，需要开发兼具高精度和可解释性的方法。

Method: 提出模糊图神经网络框架：1）比较多种GNN架构后选用广义图卷积网络（GENConv）作为基础模型；2）集成互信息识别关键网络区域；3）引入模糊逻辑构建模糊增强变体（FGENConv），为节点分类任务提供基于规则的直观解释。

Result: 模糊图神经网络（FGENConv）在泄漏检测和定位任务中分别获得0.889和0.814的Graph F1分数，略低于原始GENConv的0.938和0.858，但能提供空间局部化的模糊规则解释，实现了精度与可解释性的平衡。

Conclusion: 所提出的模糊网络通过平衡精度与可解释性，有助于水力工程师验证预测泄漏位置、节约人力资源并优化维护策略，为供水管网智能监测提供了实用化解决方案。
...
TL;DR: 本文提出了一个用于航空数字孪生的保证框架，结合可信与伦理保证（TEA）方法，旨在支持AI空中交通控制代理的培训与测试，并满足监管要求。


<details>
  <summary>Details</summary>
Motivation: 数字孪生技术在航空业具有巨大潜力，但缺乏针对其准确性和功能性的结构化评估方法，且监管要求尚不明确，需要具体用例的定制化保证框架。

Method: 基于行业与学术合作项目（Project Bluebird），结合数字孪生开发与AI/ML在空管中的新兴指导原则，采用可信与伦理保证（TEA）方法构建结构化保证案例，包含目标、证据、假设和论证。

Result: 开发了一个可操作的保证框架，能够评估数字孪生对其物理对应物的表征准确性及目标用例的功能充分性，为研究人员、利益相关者和监管机构提供结构化评估与沟通基础。

Conclusion: 该框架不仅帮助理解数字孪生的优势与局限，支持监管需求讨论，还通过具体实例推动了相关指导原则的发展，为未来应用提供了可扩展的保证基础。
...
TL;DR: 提出了一种更简单、无需调优且无需任务明确线索的自动提示工程系统，在数据库表列名扩展任务中验证了其有效性，并首次应用于英语和德语两种语言。


<details>
  <summary>Details</summary>
Motivation: 现有自动提示工程方法设计复杂且需要任务特定线索，而数据库表列名扩展任务对表格数据搜索、访问和理解至关重要，但现有研究很少，且缺乏多语言应用探索。

Method: 设计了一种无需调优、无需任务明确线索的自动提示工程系统，并在英语和德语的数据库表列名扩展任务上进行评估。

Result: 所提系统在列名扩展任务上表现与现有方法相当，且首次实现了自动提示工程在非英语语言（德语）上的应用。

Conclusion: 该系统简化了自动提示工程的设计与应用，在列名扩展任务中验证了有效性，并拓展了自动提示工程在多语言场景下的应用潜力。
...
TL;DR: 提出InfiAgent框架，通过文件中心状态抽象外部化持久状态，保持推理上下文有界，解决LLM智能体在长时任务中上下文膨胀和错误累积问题。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时任务中因上下文无限增长和错误累积而失效，现有方法（如上下文压缩或检索增强）在信息保真度和推理稳定性间存在权衡。

Method: 设计文件中心状态抽象外部化持久状态，智能体每步从工作空间状态快照加固定窗口近期动作重建上下文，保持推理上下文严格有界。

Result: 在DeepResearch和80篇文献综述任务中，未任务特定微调的20B开源模型InfiAgent与大型专有系统竞争，长时覆盖显著高于上下文中心基线。

Conclusion: 显式状态外部化是稳定长时智能体的实用基础，支持智能体在长时任务中保持高性能。
...
TL;DR: 提出MAGMA多图记忆架构，通过正交语义、时序、因果和实体图表示记忆项，以策略引导的图遍历实现查询自适应检索，提升长上下文推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于语义相似度的单维记忆方法混淆了时序、因果和实体信息，导致推理可解释性差、查询意图与检索证据对齐不足，限制了长上下文推理精度。

Method: 设计多图记忆架构，将记忆项分解为语义、时序、因果、实体四个正交图；采用策略引导的图遍历算法实现查询自适应检索；通过解耦记忆表示与检索逻辑增强可控性。

Result: 在LoCoMo和LongMemEval基准测试中，MAGMA在长时序推理任务上持续优于现有最先进的智能记忆系统。

Conclusion: 多图记忆架构能有效解耦记忆维度，通过结构化检索提升长上下文推理的透明度与准确性，为记忆增强生成系统提供了新范式。
...
### [27] [Multimodal Sentiment Analysis based on Multi-channel and Symmetric Mutual Promotion Feature Fusion](https://arxiv.org/abs/2601.02415)
*Wangyuan Zhu,Jun Yu*

Main category: cs.CV

TL;DR: 本文提出一种多模态情感分析方法，通过多通道特征提取和对称互促进特征融合机制，提升情感识别准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在单模态特征有限、过度关注模态一致性而忽略特征差异的问题，导致特征融合不充分，影响情感分析性能。

Method: 1. 提取视觉和听觉的双通道特征以增强模态内表征；2. 提出对称互促进（SMP）模态间特征融合方法，结合对称跨模态注意力与自注意力机制；3. 融合模态内特征与模态间融合特征。

Result: 在两个基准数据集上的实验表明，该方法在情感识别任务中具有有效性和优越性。

Conclusion: 所提出的多通道特征提取与SMP融合机制能有效利用模态互补性并考虑特征差异，显著提升多模态情感分析性能。
...
### [28] [MIAR: Modality Interaction and Alignment Representation Fuison for Multimodal Emotion](https://arxiv.org/abs/2601.02414)
*Jichao Zhu,Jun Yu*

Main category: cs.CV

TL;DR: 提出MIAR方法，通过模态交互和对齐表示解决多模态情感识别中的模态分布差异和贡献度不均问题，在CMU-MOSI和MOSEI数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注模态融合，但未能充分处理模态间的显著分布差异，也未考虑不同模态对任务的贡献度差异，且缺乏对不同文本模型特征的泛化能力，限制了多模态场景下的性能。

Method: 提出MIAR网络，通过特征交互整合不同模态的上下文特征生成特征token来表示模态的全局表示；使用对比学习和归一化策略对齐不同模态。

Result: 在CMU-MOSI和CMU-MOSEI两个基准数据集上的实验结果表明，MIAR方法优于现有的最先进多模态情感识别方法。

Conclusion: MIAR通过模态交互和对齐表示有效解决了多模态情感识别中的模态差异和贡献度问题，提升了模型性能。
...
### [29] [Self-Supervised Masked Autoencoders with Dense-Unet for Coronary Calcium Removal in limited CT Data](https://arxiv.org/abs/2601.02392)
*Mo Chen*

Main category: cs.CV

TL;DR: 提出Dense-MAE自监督学习框架，通过随机掩码3D血管腔体块并重建缺失几何结构，提升CTA中钙化伪影去除效果，尤其在少样本场景下显著改善狭窄评估准确性。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉钙化在CTA中产生伪影，严重影响管腔狭窄诊断。现有深度学习方法依赖大量标注数据，但医学领域标注稀缺，需开发不依赖标注的预训练方法。

Method: 基于3D点云掩码自编码器思想，提出Dense-MAE框架：随机掩码血管腔体的3D块，使用Dense-Unet重建缺失几何结构，通过自监督预训练学习动脉拓扑特征，再迁移至钙化去除任务。

Result: 在临床CTA数据集上验证表明，使用MAE预训练权重初始化网络，相比从头训练，在少样本场景下显著提升伪影修复精度和狭窄估计准确性。

Conclusion: Dense-MAE通过自监督预训练有效学习血管结构特征，减少对标注数据的依赖，为医学图像分析提供了一种高效的少样本学习解决方案。
...
### [30] [Watch Wider and Think Deeper: Collaborative Cross-modal Chain-of-Thought for Complex Visual Reasoning](https://arxiv.org/abs/2601.02422)
*Wenting Lu,Didi Zhu,Tao Shen,Donglin Zhu,Ayong Ye,Chao Wu*

Main category: cs.CV

TL;DR: 提出CoCoT框架解决多模态推理中视觉-语言融合问题，通过动态多区域定位和关系感知推理提升推理连贯性，构建CoCoT-70K数据集并在多个基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有思维链方法在多模态场景中存在两大局限：过度依赖单一粗粒度图像区域，以及连续推理步骤间的语义碎片化问题，制约了视觉与语言线索的有效整合。

Method: 1) 动态多区域定位：根据问题自适应检测最相关图像区域；2) 关系感知推理：通过迭代对齐视觉线索实现多区域协作，构建连贯逻辑思维链；3) 构建包含74,691个多区域标注样本的CoCoT-70K数据集。

Result: 在六个挑战性基准测试中，CoCoT显著提升复杂视觉推理能力：LLaVA-1.5平均准确率提升15.4%，Qwen2-VL提升4.0%。

Conclusion: CoCoT框架通过增强视觉-语言对齐与多区域协作，有效解决了多模态推理中的语义连贯性问题，为跨模态思维链方法提供了新范式。
...
### [31] [NitroGen: An Open Foundation Model for Generalist Gaming Agents](https://arxiv.org/abs/2601.02427)
...
TL;DR: 本文介绍了NitroGen，一个基于4万小时游戏视频训练的多游戏视觉-动作基础模型，在未见过的游戏中表现出强大的跨游戏泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前游戏AI通常针对单一游戏专门训练，缺乏跨游戏的通用能力。研究旨在开发能够适应多种游戏类型的通用游戏智能体。

Method: 1) 从公开游戏视频自动提取玩家动作构建大规模视频-动作数据集；2) 创建多游戏基准环境评估跨游戏泛化；3) 通过大规模行为克隆训练统一的视觉-动作模型。

Result: NitroGen在3D动作游戏、2D平台游戏和程序生成世界等多种游戏类型中表现优异，在未见游戏中任务成功率比从头训练模型提升最高达52%。

Conclusion: NitroGen证明了大规模跨游戏训练的有效性，为通用具身智能体研究提供了数据集、评估套件和模型权重。
...
### [32] [TAP-ViTs: Task-Adaptive Pruning for On-Device Deployment of Vision Transformers](https://arxiv.org/abs/2601.02437)
*Zhibo Wang,Zuoyuan Zhang,Xiaoyi Pang,Qile Zhang,Xuanyi Hao,Shuguo Zhuo,Peng Sun*

Main category: cs.CV

TL;DR: 提出TAP-ViTs框架，通过高斯混合模型构建代理数据集，实现无需原始本地数据的设备定制化视觉Transformer剪枝。


<details>
  <summary>Details</summary>
Motivation: 现有ViT剪枝方法无法兼顾设备异构性、资源限制和隐私约束，难以在移动计算场景中实现任务定制化剪枝。

Method: 1) 设备端用轻量GMM拟合私有数据分布并上传参数；2) 云端基于GMM参数从公开数据选择样本构建任务代表数据集；3) 设计双粒度重要性评估剪枝策略，联合评估复合神经元重要性和自适应层重要性。

Result: 在多种ViT架构和数据集上，TAP-ViTs在相同压缩比下持续优于现有剪枝方法。

Conclusion: TAP-ViTs框架能有效生成设备特定的剪枝模型，在保护隐私的同时适应不同设备的计算预算和任务特性。
...
### [33] [Understanding Pure Textual Reasoning for Blind Image Quality Assessment](https://arxiv.org/abs/2601.02441)
*Yuan Li,Shin'ya Nishida*

Main category: cs.CV

TL;DR: 本文从信息流角度研究文本推理在盲图像质量评估中的作用，比较了三种学习图像-文本-分数关系的范式，发现自一致性范式能显著缩小图像与文本预测的差距。


<details>
  <summary>Details</summary>
Motivation: 当前盲图像质量评估广泛采用文本推理，但文本信息如何贡献于质量预测、文本能在多大程度上表示与分数相关的图像内容尚不明确，需要系统研究。

Method: 从信息流角度比较现有BIQA模型与三种设计范式：思维链、自一致性和自编码器，通过实验分析图像与文本条件预测的性能差异。

Result: 仅使用文本信息时现有模型预测性能显著下降；思维链对BIQA性能提升有限；自一致性范式显著缩小图像与文本预测差距（PLCC/SRCC差异降至0.02/0.03）；自编码器范式效果较弱但揭示了优化方向。

Conclusion: 研究揭示了改进BIQA及高级视觉任务中文本推理的途径，自一致性范式在弥合图像-文本鸿沟方面表现突出，为后续优化提供了见解。
...
### [34] [Evaluating the Diagnostic Classification Ability of Multimodal Large Language Models: Insights from the Osteoarthritis Initiative](https://arxiv.org/abs/2601.02443)
*Li Wang,Xi Chen,XiangWen Deng,HuaHui Yi,ZeKun Jiang,Kang Li,Jian Li*

Main category: cs.CV

TL;DR: 本文评估了多模态大语言模型（MLLM）在膝关节骨关节炎X光片分类任务上的表现，发现MLLM架构在需要高确定性的医学图像诊断分类任务中并不理想，建议优先优化视觉编码器和精心策划数据集。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLM在医学视觉问答和报告生成方面表现出潜力，但其生成和解释能力未能可靠地迁移到疾病特异性分类任务。膝关节骨关节炎影响全球3-4亿人，但在现有医学MLLM基准中代表性不足，因此需要评估MLLM在该特定分类任务上的有效性。

Method: 通过系统消融研究，操纵视觉编码器、连接器和大语言模型（LLM）组件，并采用多种训练策略（包括LoRA微调、提示引导等），评估各组件对诊断准确率的贡献。实验使用了小规模类平衡数据集（500张图像）和大规模类不平衡数据集（5,778张图像）进行对比。

Result: 1. 单独训练的视觉编码器在分类准确率上优于完整的MLLM流程；2. 微调LLM并未比基于提示的引导带来有意义改进；3. 在小规模类平衡数据集上的LoRA微调结果优于大规模类不平衡数据集，表明数据平衡和质量比原始规模更重要。

Conclusion: 对于领域特异性医学分类任务，LLM更适合作为解释器和报告生成器，而非主要分类器。MLLM架构对需要高确定性的医学图像诊断分类任务适用性有限，建议在开发临床适用系统时优先优化视觉编码器并进行精细的数据集策划。
...
### [35] [A Spatio-Temporal Deep Learning Approach For High-Resolution Gridded Monsoon Prediction](https://arxiv.org/abs/2601.02445)
*Parashjyoti Borah,Sanghamitra Sarkar,Ranjan Phukan*

Main category: cs.CV

TL;DR: 提出一种深度学习框架，将印度夏季风预测重构为时空计算机视觉任务，利用卷积神经网络从前期大气海洋场预测高分辨率网格化降雨模式。


<details>
  <summary>Details</summary>
Motivation: 传统季风预测方法仅提供空间平均的季节值，缺乏区域资源管理所需的空间细节信息，无法满足精细化预报需求。

Method: 将多变量前期大气海洋场视为多通道图像序列，构建视频式输入张量；使用85年ERA5再分析数据和IMD降雨数据，采用CNN架构学习1-5月前期数据与后续季风季节高分辨率降雨模式的复杂映射关系。

Result: 框架成功生成6-9月各月及季节总平均的独立高分辨率降雨预报，证明其同时适用于季节内和季节尺度预测。

Conclusion: 该深度学习框架能提供具有空间细节的季风预测，弥补传统方法的不足，为区域水资源管理和农业规划提供更实用的预报产品。
...
### [36] [Don't Mind the Gaps: Implicit Neural Representations for Resolution-Agnostic Retinal OCT Analysis](https://arxiv.org/abs/2601.02447)
*Bennet Kahrs,Julia Andresen,Fenja Falta,Monty Santarossa,Heinz Handels,Timo Kepp*

Main category: cs.CV

TL;DR: 提出两种基于隐式神经表示（INR）的框架，用于处理各向异性的视网膜OCT图像，实现密集3D分析，包括跨B扫描插值和分辨率无关的视网膜图谱构建。


<details>
  <summary>Details</summary>
Motivation: 临床OCT成像因大切片间距导致图像高度各向异性，现有2D方法易产生相邻B扫描结果不一致，且传统卷积神经网络受训练数据分辨率限制，难以适应不同成像协议。

Method: 1）利用INR的坐标输入特性，结合en-face模态信息进行跨B扫描插值；2）通过群体训练构建分辨率无关的视网膜图谱，使用泛化性INR实现未见病例的预测。

Result: 所提框架能有效处理大B扫描间距的OCT图像，提升视网膜形状表示的一致性，支持不同分辨率数据的通用分析。

Conclusion: 基于INR的分辨率无关框架为各向异性OCT图像的体积评估开辟了新途径，有助于视网膜结构和病理的3D分析。
...
### [37] [PatchAlign3D: Local Feature Alignment for Dense 3D Shape understanding](https://arxiv.org/abs/2601.02457)
*Souhail Hadgi,Bingchen Gong,Ramana Sundararaman,Emery Pierson,Lei Li,Peter Wonka,Maks Ovsjanikov*

Main category: cs.CV

TL;DR: 提出一种基于点云的编码器模型，通过两阶段预训练实现零样本3D部件分割，无需多视角渲染，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D基础模型擅长全局任务，但在局部部件级推理上表现不佳；基于多视角渲染和文本查询的方法计算成本高、依赖LLM提示工程，且未充分利用3D几何信息。

Method: 使用点云Transformer编码器，通过两阶段预训练：1) 从DINOv2等视觉编码器蒸馏密集2D特征到3D补丁；2) 通过多正例对比目标将补丁嵌入与部件级文本嵌入对齐。

Result: 模型在多个3D部件分割基准测试中显著优于基于渲染和前馈方法，支持快速单次推理的零样本3D部件分割。

Conclusion: 该方法通过直接处理点云并学习语言对齐的补丁特征，实现了高效且性能优越的3D局部理解，为3D基础模型提供了新方向。
...
### [38] [CT Scans As Video: Efficient Intracranial Hemorrhage Detection Using Multi-Object Tracking](https://arxiv.org/abs/2601.02521)
*Amirreza Parvahan,Mohammad Hoseyni,Javad Khoramdel,Amirhossein Nikoofard*

Main category: cs.CV

TL;DR: 提出一种轻量级计算机视觉框架，将3D CT数据重构为序列视频流，结合YOLO检测器和ByteTrack跟踪算法，在边缘设备上实现颅内出血的实时检测，显著提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上运行3D医学影像分析面临高内存和计算需求限制，需要一种兼顾2D效率和3D上下文的方法，以在资源受限环境中实现实时患者优先级排序。

Method: 将CT数据重构为视频流；基准测试YOLO Nano架构（v8-v12）选择最佳切片级骨干；引入ByteTrack算法确保z轴解剖一致性；提出混合推理策略和时空一致性滤波器减少初始化延迟和预测噪声。

Result: 在独立测试数据上，检测精度从基线2D检测器的0.703提升至0.779，同时保持高灵敏度，以较低计算成本近似3D上下文推理。

Conclusion: 该方法为移动卒中单元和物联网远程诊所等资源受限环境提供了可扩展的实时解决方案，通过视频视角范式有效平衡了检测效率与上下文需求。
...
### [39] [MovieRecapsQA: A Multimodal Open-Ended Video Question-Answering Benchmark](https://arxiv.org/abs/2601.02536)
*Shaden Shaar,Bradon Thymes,Sirawut Chaixanien,Claire Cardie,Bharath Hariharan*

Main category: cs.CV

TL;DR: 提出了首个开放式的多模态视频问答基准MovieRecapsQA，基于电影解说视频构建，包含约8.2K个问题-答案对，支持无参考评估，并对7个先进多模态大模型进行了性能分析。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答基准难以捕捉真实电影理解所需的多模态推理能力，且大多非开放式，缺乏对自由形式答案的评估方法。

Method: 利用YouTube电影解说视频（包含同步的视觉和文本模态），通过解说摘要生成与电影字幕对齐的QA对，提供验证答案所需的“事实”信息，支持无参考评估。

Result: 评估发现：1）纯视觉问题最具挑战性；2）模型倾向于依赖文本输入；3）从视频中提取准确事实信息仍困难；4）专有和开源模型在视频相关问题上表现相当。

Conclusion: MovieRecapsQA填补了开放式多模态视频问答基准的空白，揭示了当前模型在多模态推理中的局限性，尤其是视觉理解和事实提取能力不足。
...
### [40] [Shallow- and Deep-fake Image Manipulation Localization Using Vision Mamba and Guided Graph Neural Network](https://arxiv.org/abs/2601.02566)
*Junbin Zhang,Hamid Reza Tohidypour,Yixiao Wang,Panos Nasiopoulos*

Main category: cs.CV

TL;DR: 提出一种结合Vision Mamba网络和引导图神经网络（G-GNN）的图像篡改定位方法，可同时检测浅层伪造和深度伪造图像中的篡改区域。


<details>
  <summary>Details</summary>
Motivation: 现有方法多专注于浅层伪造图像或深度伪造视频的篡改定位，缺乏能同时处理两类伪造图像的通用方案，而伪造图像可能对社会各方面产生重大影响。

Method: 使用Vision Mamba网络提取能清晰描述篡改与原始区域边界的特征图，并设计引导图神经网络（G-GNN）模块增强篡改像素与真实像素的区分度。

Result: 评估结果表明，该方法在推理准确率上优于其他先进方法。

Conclusion: 所提出的深度学习方法能有效定位浅层伪造和深度伪造图像中的篡改区域，为通用图像篡改检测提供了可行解决方案。
...
### [41] [DreamLoop: Controllable Cinemagraph Generation from a Single Photograph](https://arxiv.org/abs/2601.02646)
*Aniruddha Mahapatra,Long Mai,Cusuh Ham,Feng Liu*

Main category: cs.CV

TL;DR: DreamLoop是一种可控视频合成框架，能够从单张照片生成高质量、可循环的动态图像（cinemagraph），无需专门的训练数据，通过时间桥接和运动条件实现灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有图像动画技术仅限于简单低频运动（如水流、烟雾），且缺乏针对动态图像无缝循环和可控性的专门方法；大规模视频扩散模型不适用于动态图像约束且缺乏相关训练数据。

Method: 通过训练通用视频扩散模型实现两个目标：时间桥接（连接首尾帧）和运动条件（基于用户指定的运动路径）；推理时以输入图像同时作为首尾帧条件实现无缝循环，结合静态轨迹保持背景静止。

Result: 该方法能够生成与用户意图一致的高质量复杂动态图像，在通用场景中实现灵活直观的控制，性能优于现有方法。

Conclusion: DreamLoop是首个支持通用场景动态图像生成且具备灵活可控性的方法，通过适应视频扩散模型解决了动态图像生成中的数据与约束挑战。
...
### [42] [GRRE: Leveraging G-Channel Removed Reconstruction Error for Robust Detection of AI-Generated Images](https://arxiv.org/abs/2601.02709)
*Shuman He,Xiehua Li,Xioaju Yang,Yang Xiong,Keqin Li*

Main category: cs.CV

TL;DR: 提出基于绿色通道移除重建误差（GRRE）的新方法，通过比较真实图像与AI生成图像在绿色通道移除后重建误差的差异，实现高精度、强泛化能力的AI生成图像检测。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法在面对新型或未见过的生成模型时泛化能力不足，准确率下降，需要开发具有更强泛化能力的检测技术。

Method: 提出GRRE方法：移除图像的绿色通道后重建该通道，通过比较真实图像与AI生成图像在重建误差上的显著差异进行检测。

Result: GRRE在多个生成模型（包括训练时未见过的模型）上均保持高检测准确率，对各类扰动和后处理操作具有强鲁棒性，且跨模型泛化能力优于现有方法。

Conclusion: 基于通道移除重建的检测范式是有效的图像真实性鉴定工具，在生成式AI时代具有保护图像真实性的潜力。
...
### [43] [CAMO: Category-Agnostic 3D Motion Transfer from Monocular 2D Videos](https://arxiv.org/abs/2601.02716)
*Taeyeon Kim,Youngju Na,Jumin Lee,Minhyuk Sung,Sung-Eui Yoon*

Main category: cs.CV

TL;DR: CAMO是一个无需类别特定模板的框架，可将单目2D视频中的运动直接迁移到多样化的3D网格上，通过形态参数化高斯溅射模型和语义对应解决形状-姿态歧义。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖类别特定的参数化模板，难以处理多样化物体形状和姿态歧义，限制了在任意类别和日常视频场景中的运动迁移能力。

Method: 提出形态参数化关节3D高斯溅射模型，结合密集语义对应关系，通过联合优化同时调整目标网格的形状和姿态。

Result: 实验表明该方法在运动准确性、效率和视觉连贯性上优于现有方法，能有效处理多种物体类别和日常视频场景。

Conclusion: CAMO框架通过解耦形状与姿态优化，实现了无需模板的跨类别运动迁移，显著推进了运动迁移技术的通用性和实用性。
...
### [44] [Robust Mesh Saliency GT Acquisition in VR via View Cone Sampling and Geometric Smoothing](https://arxiv.org/abs/2601.02721)
*Guoquan Zheng,Jie Hao,Huiyu Duan,Yongming Han,Liang Yuan,Dong Zhang,Guangtao Zhai*

Main category: cs.CV

TL;DR: 本文提出了一种用于VR中3D网格显著性标注的鲁棒框架，通过视锥采样和混合流形-欧几里得约束扩散算法，解决现有方法忽略3D拓扑结构差异和信号泄漏的问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D网格显著性标注方法沿用2D图像方法，忽略了3D几何拓扑与2D图像阵列的差异，且VR眼动追踪流程依赖单射线采样和欧几里得平滑，导致纹理注意力干扰和跨间隙信号泄漏。

Method: 1. 提出视锥采样策略，通过高斯分布射线束模拟人类中央凹感受野，提升复杂拓扑的采样鲁棒性；2. 开发混合流形-欧几里得约束扩散算法，融合流形测地约束与欧几里得尺度，确保拓扑一致的显著性传播。

Result: 该框架通过缓解“拓扑短路”和混叠效应，提供了符合人类自然感知的高保真3D注意力获取范式，为3D网格显著性研究提供了更准确、鲁棒的基线。

Conclusion: 所提方法克服了现有3D网格显著性标注的局限性，实现了更贴合人类视觉机制的3D注意力建模，有助于提升VR中人类中心视觉建模的可靠性。
...
### [45] [Foreground-Aware Dataset Distillation via Dynamic Patch Selection](https://arxiv.org/abs/2601.02727)
*Longzhen Li,Guang Li,Ren Togo,Keisuke Maeda,Takahiro Ogawa,Miki Haseyama*

Main category: cs.CV

TL;DR: 提出了一种前景感知的数据集蒸馏方法，通过动态补丁选择策略增强内容自适应能力，在减少冗余背景的同时保留关键对象信息，显著提升了蒸馏性能与架构泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统优化方法存在计算成本高、生成图像不自然、泛化能力有限等问题；近期非优化方法虽有所改进，但固定补丁选择策略可能丢失关键对象信息，因此需要更智能的内容自适应蒸馏方法。

Method: 利用Grounded SAM2识别前景对象并计算前景占比，据此制定类别级补丁决策阈值；设计动态补丁选择策略，根据阈值选择最具信息量的补丁或直接缩放完整图像，形成双路径机制。

Result: 在多个基准测试中，该方法持续超越现有方法，生成更具信息性和代表性的蒸馏数据集，并在不同架构和图像构成下表现出更强的鲁棒性。

Conclusion: 前景感知的动态补丁选择策略能有效平衡信息保留与冗余削减，为高效数据集蒸馏提供了更优解决方案，推动了轻量化模型训练的发展。
...
### [46] [HOLO: Homography-Guided Pose Estimator Network for Fine-Grained Visual Localization on SD Maps](https://arxiv.org/abs/2601.02730)
*Xuchang Zhong,Xu Cao,Jinke Feng,Hao Fang*

Main category: cs.CV

TL;DR: 提出了一种基于单应性引导的姿态估计网络，用于多视角图像与标准定义地图之间的细粒度视觉定位，通过BEV特征投影和语义对齐提升定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于回归的视觉定位方法常忽略几何先验，导致训练效率低下和定位精度受限，需要一种能有效利用几何约束的方法。

Method: 构建满足单应性约束的输入对，将地面视角特征投影到BEV域并与地图特征语义对齐，利用单应性关系引导特征融合并约束姿态输出到有效区域。

Result: 在nuScenes数据集上的实验表明，该方法在训练效率和定位精度上显著优于现有基于注意力融合和直接3自由度姿态回归的先进方法。

Conclusion: 该框架首次将BEV语义推理与单应性学习统一用于图像到地图的定位，通过显式建模单应性变换自然支持跨分辨率输入，提升了模型灵活性。
...
### [47] [D$^3$R-DETR: DETR with Dual-Domain Density Refinement for Tiny Object Detection in Aerial Images](https://arxiv.org/abs/2601.02747)
*Zixiao Wen,Zhen Yang,Xianjie Bao,Lei Zhang,Xiantai Xiang,Wenshuai Li,Yuhan Liu*

Main category: cs.CV

TL;DR: 提出D³R-DETR检测器，通过双域密度细化解决遥感图像中小目标检测难题，在AI-TOD-v2数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中的微小目标包含关键信息，但现有基于Transformer的检测器因像素信息有限和目标密度变化大，存在收敛慢和查询-目标匹配不准确的问题。

Method: 采用双域密度细化方法，融合空间域和频域信息优化低层特征图，利用其丰富细节预测更准确的目标密度图，从而精确定位微小目标。

Result: 在AI-TOD-v2数据集上的大量实验表明，D³R-DETR在微小目标检测任务上优于现有最先进的检测器。

Conclusion: D³R-DETR通过双域密度细化有效提升了微小目标检测性能，为遥感智能解译提供了更优解决方案。
...
### [48] [Unveiling and Bridging the Functional Perception Gap in MLLMs: Atomic Visual Alignment and Hierarchical Evaluation via PET-Bench](https://arxiv.org/abs/2601.02737)
...
TL;DR: 本文揭示了多模态大语言模型在功能成像（特别是PET）中的感知缺陷，提出了首个大规模功能成像基准PET-Bench，发现思维链提示会导致幻觉陷阱，并提出原子视觉对齐方法以提升诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在解剖模态异常检测和报告生成方面表现优异，但在功能成像（如PET）中的能力尚未被充分探索，尤其是视觉编码器难以独立于形态先验解码示踪剂生物分布，存在功能感知缺陷。

Method: 1. 构建PET-Bench基准，包含52,308个层次化QA对，源自9,732项多中心、多示踪剂PET研究；2. 评估19个先进MLLM，发现思维链提示在PET中导致语言生成与视觉证据脱节；3. 提出原子视觉对齐（AVA）微调策略，强制模型先掌握低层功能感知再进行高层诊断推理。

Result: 1. 识别并量化了功能感知差距；2. 标准思维链提示在PET中产生临床流畅但事实无依据的诊断（幻觉陷阱）；3. AVA方法将思维链从幻觉来源转变为稳健推理工具，诊断准确率最高提升14.83%。

Conclusion: 功能成像需要不同于解剖模态的感知范式，AVA通过强制视觉-概念对齐有效弥补感知差距，为MLLM在功能医学影像中的可靠应用提供了解决方案。
...
### [49] [Towards Zero-Shot Point Cloud Registration Across Diverse Scales, Scenes, and Sensor Setups](https://arxiv.org/abs/2601.02759)
*Hyungtae Lim,Minkyun Seo,Luca Carlone,Jaesik Park*

Main category: cs.CV

TL;DR: 提出BUFFER-X框架，通过几何引导、分布感知采样和坐标归一化实现点云配准的零样本泛化，无需训练或超参数调整。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在零样本泛化上存在局限，需针对不同数据集调整超参数或重新训练，难以适应新环境。

Method: 采用几何引导自动估计超参数、分布感知最远点采样替代学习检测器、块级坐标归一化保证尺度一致性，并结合分层多尺度匹配。

Result: 在12个数据集（物体、室内、室外及跨传感器场景）上验证，无需手动调参即可有效泛化；轻量版BUFFER-X-Lite计算时间减少43%且精度不变。

Conclusion: BUFFER-X框架实现了跨场景的鲁棒点云配准，解决了零样本泛化问题，为实际应用提供了高效解决方案。
...
### [50] [AnyDepth: Depth Estimation Made Easy](https://arxiv.org/abs/2601.02760)
*Zeyu Ren,Zeyu Zhang,Wukai Li,Qingxiang Liu,Hao Tang*

Main category: cs.CV

TL;DR: 提出轻量级零样本单目深度估计框架，采用DINOv3编码器和新型解码器SDT，结合质量过滤策略提升性能


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大规模数据集和复杂解码器，导致效率低且泛化能力受限，需平衡模型设计与数据质量

Method: 使用DINOv3提取密集特征；设计轻量解码器SDT（单路径特征融合）；提出质量过滤策略筛选训练样本

Result: SDT比DPT参数量减少85%-89%，精度更高；在五个基准测试中超越DPT；过滤策略提升训练质量

Conclusion: 平衡模型设计与数据质量对实现高效、可泛化的零样本深度估计至关重要，所提框架在精度和效率上均具优势
...
### [51] [ClearAIR: A Human-Visual-Perception-Inspired All-in-One Image Restoration](https://arxiv.org/abs/2601.02763)
*Xu Zhang,Huan Zhang,Guoli Wang,Qian Zhang,Lefei Zhang*

Main category: cs.CV

TL;DR: 提出ClearAIR框架，受人类视觉感知启发，采用分层粗到细策略进行一体化图像恢复，在合成和真实数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有一体化图像恢复方法过度依赖退化特定表示，常导致过度平滑和伪影，需更准确处理复杂复合退化问题。

Method: 1. 基于MLLM的图像质量评估模型进行整体评价；2. 区域感知与任务识别流程（语义交叉注意力生成粗语义提示，退化感知模块捕获区域退化特征）；3. 内部线索重用机制以自监督方式挖掘图像内在信息。

Result: ClearAIR在多样合成和真实世界数据集上实现了优越性能。

Conclusion: 受人类视觉感知启发的分层恢复策略能有效处理复杂退化，提升图像恢复质量。
...
### [52] [AbductiveMLLM: Boosting Visual Abductive Reasoning Within MLLMs](https://arxiv.org/abs/2601.02771)
*Boyu Chang,Qi Wang,Xi Guo,Zhixiong Nan,Yazhou Yao,Tianfei Zhou*

Main category: cs.CV

TL;DR: 提出AbductiveMLLM模型，通过模拟人类言语与图像双重推理模式，增强多模态大语言模型在视觉溯因推理任务中的表现，在标准基准测试中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在视觉溯因推理任务中表现不及人类，缺乏类似人类结合言语推理与图像想象的溯因能力，需要设计新方法弥补这一差距。

Method: 构建包含REASONER（言语推理模块）和IMAGINER（图像想象模块）的双组件模型。REASONER先通过纯文本LLM生成候选解释，再基于跨模态因果对齐筛选视觉一致的假设；IMAGINER利用扩散模型根据输入视频和REASONER的嵌入生成对应解释的视觉场景。两组件端到端联合训练。

Result: 在标准视觉溯因推理基准测试中，AbductiveMLLM取得最优性能，显著超越传统方法和先进MLLMs。

Conclusion: 模拟人类言语与图像双重推理模式能有效提升MLLMs的溯因推理能力，为多模态推理系统设计提供了新思路。
...
### [53] [EarthVL: A Progressive Earth Vision-Language Understanding and Generation Framework](https://arxiv.org/abs/2601.02783)
*Junjue Wang,Yanfei Zhong,Zihang Chen,Zhuo Zheng,Ailong Ma,Liangpei Zhang*

Main category: cs.CV

TL;DR: 提出EarthVL框架，包含EarthVLSet数据集和EarthVLNet网络，通过语义分割引导的视觉问答实现遥感图像的对象关系推理与场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有地球视觉研究多关注对象识别，缺乏对对象间关系的推理能力，限制了场景的全面理解。

Method: 1) 构建EarthVLSet多任务数据集（含遥感图像、土地覆盖掩码及文本对）；2) 设计EarthVLNet网络，分阶段进行语义分割、关系推理与理解；3) 提出数值差异损失函数优化多对象统计差异。

Result: 在语义分割、选择题和开放式视觉问答三个基准测试中表现优异，发现：1) 分割特征能持续提升VQA性能；2) 选择题对视觉编码器更敏感；3) 开放式任务需要更先进的视觉与语言模型。

Conclusion: 该框架为“图像-掩码-文本”关联提供了有益基准，推动了地球视觉在地理应用中的发展。
...
### [54] [DreamStyle: A Unified Framework for Video Stylization](https://arxiv.org/abs/2601.02785)
*Mengtian Li,Jinshu Chen,Songtao Zhao,Wanquan Feng,Pengqi Tu,Qian He*

Main category: cs.CV

TL;DR: 提出了DreamStyle，一个支持文本、风格图像和首帧引导的统一视频风格化框架，通过精心设计的数据处理流程和LoRA训练解决现有方法风格不一致和时间闪烁问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频风格化方法通常局限于单一风格条件（文本、图像或首帧），且缺乏高质量数据集，导致风格不一致和时间闪烁，限制了应用范围。

Method: 基于普通图像到视频（I2V）模型构建，采用低秩适应（LoRA）训练，使用特定令牌的上矩阵减少不同条件令牌间的混淆，并设计了高质量配对视频数据采集流程。

Result: 定性和定量评估表明，DreamStyle能胜任三种视频风格化任务，在风格一致性和视频质量上优于现有方法。

Conclusion: DreamStyle是一个统一的视频风格化框架，通过多条件支持和高质量数据处理，有效解决了风格一致性和时间稳定性问题，扩展了视频风格化的应用范围。
...
### [55] [Textile IR: A Bidirectional Intermediate Representation for Physics-Aware Fashion CAD](https://arxiv.org/abs/2601.02792)
*Petteri Teikari,Neliana Fuenmayor*

Main category: cs.CV

TL;DR: 本文提出Textile IR，一种连接CAD设计、物理仿真和生命周期评估的双向中间表示，通过七层验证阶梯和场景图表示，解决时尚设计中的工程约束、可持续性和制造性问题。


<details>
  <summary>Details</summary>
Motivation: 现有时尚设计工具相互孤立：纸样软件保证可缝纫性但不理解悬垂性，物理仿真能预测行为但无法自动修正纸样。缺乏集成框架导致设计师只能在物理原型制作后才发现可持续性、可制造性和美观性之间的冲突。

Method: 提出Textile IR双向中间表示，包含七层验证阶梯（从语法检查到物理验证），采用场景图表示将服装视为结构化程序；将时尚工程形式化为三个领域的约束满足问题，支持双向反馈和不确定性传播。

Result: 该框架使工程约束可感知、可操作且即时生效，支持设计师同时权衡可持续性、可制造性和美观性；能处理复合不确定性问题，通过显式置信边界提高可持续性声明的可靠性。

Conclusion: Textile IR为时尚设计提供了语义集成基础，降低了专业工程要求，特别适合中小型企业；提出了六个研究重点，强调通过形式化表示实现跨领域协同设计。
...
### [56] [StableDPT: Temporal Stable Monocular Video Depth Estimation](https://arxiv.org/abs/2601.02793)
*Ivan Sobko,Hayko Riemenschneider,Markus Gross,Christopher Schroers*

Main category: cs.CV

TL;DR: 提出StableDPT方法，通过添加可训练的时间模块，将单图像深度估计模型适配到视频处理，提高时间稳定性并避免闪烁伪影。


<details>
  <summary>Details</summary>
Motivation: 现有单图像深度估计模型应用于视频序列时会产生显著的时间不稳定性和闪烁伪影，需要一种能保持时间一致性的视频深度估计方法。

Method: 基于现成的ViT编码器和DPT头部构建StableDPT架构，在头部引入时间层，使用高效的交叉注意力机制整合整个视频序列关键帧信息，并提出处理任意长度视频的推理策略。

Result: 在多个基准数据集上评估显示，该方法提高了时间一致性，达到竞争性的最先进性能，并在实际场景中实现2倍以上的处理速度提升。

Conclusion: StableDPT通过有效整合全局时间上下文，实现了准确且时间稳定的视频深度估计，为单图像模型到视频应用的适配提供了可行方案。
...
### [57] [Topology-aware Pathological Consistency Matching for Weakly-Paired IHC Virtual Staining](https://arxiv.org/abs/2601.02806)
*Mingzhou Jiang,Jiaying Zhou,Nan Zeng,Mickael Li,Qijie Tang,Chao He,Huazhu Fu,Honghui He*

Main category: cs.CV

TL;DR: 提出一种拓扑感知的H&E到IHC虚拟染色框架，通过拓扑一致性匹配和拓扑约束病理匹配机制解决弱配对数据问题，在四个染色任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: IHC染色对癌症诊断至关重要，但过程复杂、耗时且昂贵；虚拟染色可提供经济替代方案，但相邻切片作为真值会导致空间错位和局部变形，阻碍监督学习效果。

Method: 提出拓扑感知框架：1）拓扑一致性匹配机制（TACM），利用图对比学习和拓扑扰动学习鲁棒匹配模式；2）拓扑约束病理匹配机制（TCPM），基于节点重要性对齐病理阳性区域以增强病理一致性。

Result: 在两个基准数据集上的四个染色任务实验中，该方法在生成质量和临床相关性方面均优于现有最先进方法。

Conclusion: 所提出的拓扑感知框架能有效处理虚拟染色中的弱配对数据问题，提升生成图像的临床实用性，为IHC染色提供高效替代方案。
...
### [58] [DGA-Net: Enhancing SAM with Depth Prompting and Graph-Anchor Guidance for Camouflaged Object Detection](https://arxiv.org/abs/2601.02831)
*Yuetong Li,Qing Zhang,Yilin Zhao,Gongyang Li,Zeming Liu*

Main category: cs.CV

TL;DR: 提出DGA-Net框架，通过深度提示范式改进SAM模型，用于伪装目标检测，在RGB语义与深度几何信息融合方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有伪装目标检测方法多依赖稀疏提示（如点或框），未能充分利用深度线索，需要更全面的深度信息整合机制。

Method: 1. 提出跨模态图增强模块，在异质图中融合RGB语义与深度几何信息；2. 设计锚点引导细化模块，通过全局锚点和非局部路径传播深度引导至浅层。

Result: 定量与定性实验表明，DGA-Net在伪装目标检测任务上优于当前最先进方法。

Conclusion: 深度提示范式能有效提升伪装目标检测性能，密集深度提示的构建与传播机制具有重要应用价值。
...
### [59] [SketchThinker-R1: Towards Efficient Sketch-Style Reasoning in Large Multimodal Models](https://arxiv.org/abs/2601.02825)
*Ruiyang Zhang,Dongzhan Zhou,Zhedong Zheng*

Main category: cs.CV

TL;DR: 提出SketchThinker-R1方法，通过模仿人类草图式推理，在大规模多模态模型中实现高效推理，减少64%以上的推理令牌成本而不影响准确性。


<details>
  <summary>Details</summary>
Motivation: 现有大规模多模态模型采用逐步推理时计算开销大（令牌成本高、响应时间长），而人类使用的草图式推理更简洁高效，因此希望将这种认知效率引入模型。

Method: 1. 草图模式冷启动：将标准长推理转换为草图式推理并微调模型；2. 训练SketchJudge奖励模型，评估推理过程并给草图式推理更高评分；3. 在SketchJudge监督下进行草图思维强化学习，泛化草图式推理能力。

Result: 在四个基准测试中，SketchThinker-R1将推理令牌成本降低超过64%，且最终答案准确性不受影响；定性分析显示草图式推理更关注解决问题的关键线索。

Conclusion: 草图式推理能显著提升大规模多模态模型的推理效率，减少计算开销，同时保持准确性，为高效推理提供了新方向。
...
### [60] [Breaking Self-Attention Failure: Rethinking Query Initialization for Infrared Small Target Detection](https://arxiv.org/abs/2601.02837)
*Yuteng Liu,Duanni Meng,Maoxun Yuan,Xingxing Wei*

Main category: cs.CV

TL;DR: 提出SEF-DETR框架，通过频率引导的补丁筛选、动态嵌入增强和可靠性一致性融合，改进红外小目标检测中的查询初始化，提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于DETR的红外小目标检测器因自注意力机制导致目标相关嵌入被背景特征淹没，造成查询初始化不可靠和目标定位不准确。

Method: SEF-DETR包含三个模块：频率引导补丁筛选（FPS）利用傅里叶频谱构建目标相关密度图抑制背景；动态嵌入增强（DEE）以目标感知方式加强多尺度表示；可靠性一致性融合（RCF）通过空间-频率一致性和可靠性细化对象查询。

Result: 在三个公开红外小目标检测数据集上的实验表明，SEF-DETR相比现有最优方法具有更优越的检测性能。

Conclusion: SEF-DETR为红外小目标检测任务提供了鲁棒且高效的解决方案，有效解决了背景特征主导导致的检测性能下降问题。
...
### [61] [Towards Agnostic and Holistic Universal Image Segmentation with Bit Diffusion](https://arxiv.org/abs/2601.02881)
*Jakob Lønborg Christensen,Morten Rieger Hannemose,Anders Bjorholm Dahl,Vedrana Andersen Dahl*

Main category: cs.CV

TL;DR: 提出一种基于扩散模型的通用图像分割框架，通过整体预测而非掩码框架实现与任务无关的分割，并引入多项关键改进以优化离散数据场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有掩码依赖的分割方法存在局限性，需要一种更通用、能整体预测分割结果且能建模分割模糊性的新框架。

Method: 采用扩散模型框架，引入位置感知调色板与2D格雷码排序、tanh激活函数处理离散数据、sigmoid损失加权优化，并选择x-prediction作为预测类型。

Result: 当前模型虽未超越主流掩码架构，但显著缩小了性能差距，并具备建模分割模糊性等独特能力；所有模型均从头训练。

Conclusion: 该框架为通用分割提供了新方向，结合大规模预训练或可提示条件机制后有望成为有竞争力的模型。
...
### [62] [TA-Prompting: Enhancing Video Large Language Models for Dense Video Captioning via Temporal Anchors](https://arxiv.org/abs/2601.02908)
*Wei-Yuan Cheng,Kai-Po Chang,Chi-Pin Huang,Fu-En Yang,Yu-Chiang Frank Wang*

Main category: cs.CV

TL;DR: 提出TA-Prompting方法，通过时序锚点增强视频大语言模型的事件定位能力，结合事件连贯性采样策略提升密集视频描述性能


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在未修剪视频中难以准确定位事件边界，导致生成的描述缺乏时序基础

Method: 1. 提出TA-Prompting方法，使用时序锚点学习精确事件定位并提示模型进行时序感知的视频事件理解；2. 设计事件连贯性采样策略，基于事件间连贯性和跨模态相似性选择描述序列

Result: 在基准数据集上的实验表明，该方法在密集视频描述、时刻检索和时序问答任务上优于现有视频大语言模型

Conclusion: TA-Prompting通过时序锚点和连贯性采样有效提升了视频大语言模型的时序理解能力和描述生成质量
...
### [63] [Zoom-IQA: Image Quality Assessment with Reliable Region-Aware Reasoning](https://arxiv.org/abs/2601.02918)
*Guoqiang Liang,Jianyi Wang,Zhonghua Wu,Shangchen Zhou*

Main category: cs.CV

TL;DR: 提出Zoom-IQA模型，通过模拟人类认知行为（不确定性感知、区域推理和迭代优化）改进基于视觉语言模型的图像质量评估，提升鲁棒性、可解释性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的图像质量评估方法存在视觉与文本线索融合能力有限、推理不可靠的问题，难以同时生成精确分数和高质量描述。

Method: 1) 使用GR-IQA数据集进行监督微调，使模型能够基于关键区域进行评估；2) 采用强化学习进行动态策略探索，通过KL-Coverage正则化防止推理多样性崩溃，并利用渐进重采样策略缓解标注偏差。

Result: 实验表明Zoom-IQA在鲁棒性、可解释性和泛化性方面表现优异，在图像修复等下游任务中验证了其有效性。

Conclusion: 通过显式模拟人类认知行为的两阶段训练框架，Zoom-IQA实现了更可靠的图像质量评估，为结合视觉语言模型与认知机制提供了新思路。
...
### [64] [DCG ReID: Disentangling Collaboration and Guidance Fusion Representations for Multi-modal Vehicle Re-Identification](https://arxiv.org/abs/2601.02924)
*Aihua Zheng,Ya Gao,Shihao Li,Chenglong Li,Jin Tang*

Main category: cs.CV

TL;DR: 提出DCG-ReID方法，通过动态置信度解耦加权机制和两种场景化融合策略，解决多模态车辆重识别中平衡与不平衡质量分布数据的冲突融合需求。


<details>
  <summary>Details</summary>
Motivation: 现有方法将所有多模态数据置于单一融合模型中，忽略了平衡与不平衡质量分布数据的不同需求，难以解耦类内一致性与模态间异质性的冲突。

Method: 设计动态置信度解耦加权机制动态重加权三模态贡献；针对平衡质量分布开发协作融合模块挖掘共识特征；针对不平衡分布开发引导融合模块强化主导模态优势并引导辅助模态。

Result: 在WMVeID863、MSVR310、RGBNT100三个多模态ReID基准测试上验证了方法的有效性。

Conclusion: DCG-ReID通过解耦异构质量分布模态数据并采用场景化融合策略，有效提升了多模态车辆重识别的性能。
...
### [65] [PrismVAU: Prompt-Refined Inference System for Multimodal Video Anomaly Understanding](https://arxiv.org/abs/2601.02927)
*Iñaki Erregue,Kamal Nasrollahi,Sergio Escalera*

Main category: cs.CV

TL;DR: 提出PrismVAU系统，利用单一现成MLLM实现实时视频异常理解，无需微调或外部模块，通过弱监督自动提示工程优化性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常理解方法依赖微调的多模态大语言模型或外部模块，导致标注成本高、训练复杂、推理开销大，需要轻量高效的实时解决方案。

Method: 采用两阶段框架：1）粗粒度异常评分模块通过文本锚点相似性计算帧级异常分数；2）MLLM细化模块通过系统与用户提示上下文化异常；使用弱监督自动提示工程优化文本锚点和提示。

Result: 在标准VAD基准测试中取得有竞争力的检测性能，提供可解释的异常说明，无需指令微调、帧级标注、外部模块或密集处理。

Conclusion: PrismVAU是高效实用的实时视频异常理解解决方案，平衡了性能与计算效率，适用于实际应用场景。
...
### [66] [HybridSolarNet: A Lightweight and Explainable EfficientNet-CBAM Architecture for Real-Time Solar Panel Fault Detection](https://arxiv.org/abs/2601.02928)
*Md. Asif Hossain,G M Mota-Tahrin Tayef,Nabil Subhan*

Main category: cs.CV

TL;DR: 提出HybridSolarNet模型，集成EfficientNet-B0与CBAM注意力模块，用于无人机太阳能板故障检测，在轻量化和准确率上优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 太阳能板人工检测成本高且易出错，现有深度学习模型要么过大难以部署在边缘设备，要么因学习方法不当导致准确率估计有偏。

Method: 结合EfficientNet-B0与CBAM注意力模块，使用focal loss和余弦退火训练，采用严格的数据增强前分割协议防止数据泄露，并进行5折分层交叉验证。

Result: 在Kaggle数据集上达到92.37%平均准确率，F1分数0.9226，模型仅16.3MB（比VGG19小32倍），GPU推理速度54.9 FPS，Grad-CAM可视化显示模型能关注相关区域。

Conclusion: HybridSolarNet在保持轻量化的同时实现了高精度故障检测，适合无人机实时部署，CBAM和focal loss分别提升了模型性能和类别不平衡处理能力。
...
### [67] [LAMS-Edit: Latent and Attention Mixing with Schedulers for Improved Content Preservation in Diffusion-Based Image and Style Editing](https://arxiv.org/abs/2601.02987)
*Wingwa Fu,Takayuki Okatani*

Main category: cs.CV

TL;DR: 提出LAMS-Edit框架，通过混合反演过程的潜在表示和注意力图来改进文本到图像编辑，在保持内容与编辑效果间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本到图像编辑方法在内容保持与编辑应用之间难以平衡，且处理真实图像编辑时存在挑战。

Method: 利用反演过程的中间状态（潜在表示和注意力图），通过加权插值与调度器控制，结合Prompt-to-Prompt方法，支持区域掩码编辑和LoRA风格迁移。

Result: 实验表明LAMS-Edit能有效平衡内容保持与编辑应用，提升真实图像编辑效果。

Conclusion: LAMS-Edit是一个可扩展的编辑框架，通过混合反演状态改进了文本到图像编辑的平衡性和精确性。
...
### [68] [VTONQA: A Multi-Dimensional Quality Assessment Dataset for Virtual Try-on](https://arxiv.org/abs/2601.02945)
*Xinyi Wei,Sijing Wu,Zitong Xu,Yunhao Li,Huiyu Duan,Xiongkuo Min,Guangtao Zhai*

Main category: cs.CV

TL;DR: 本文构建了首个针对虚拟试穿图像的多维质量评估数据集VTONQA，包含8,132张图像和24,396个主观评分，并基于该数据集对现有VTON模型和图像质量评估方法进行了基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿模型常出现服装变形和身体不一致等伪影，缺乏专门的质量评估数据集来可靠评估生成图像质量，阻碍了该领域的发展。

Method: 构建包含11个代表性VTON模型生成的8,132张图像的VTONQA数据集，收集三个维度（服装合身度、身体兼容性、整体质量）的24,396个平均意见分数，并基于该数据集对VTON模型和多种图像质量评估指标进行系统性基准测试。

Result: VTONQA数据集揭示了现有VTON模型的局限性，并展示了现有图像质量评估方法在虚拟试穿任务上的不足，验证了该数据集对质量评估方法发展和VTON模型改进的价值。

Conclusion: VTONQA数据集和相应基准测试为感知对齐评估提供了坚实基础，将促进质量评估方法的开发和VTON模型的进步。
...
### [69] [ULS+: Data-driven Model Adaptation Enhances Lesion Segmentation](https://arxiv.org/abs/2601.02988)
...
<details>
  <summary>Details</summary>
Motivation: 原始ULS模型在CT扫描中基于点击点进行病灶分割，但新公开的数据集可用于进一步提升模型性能，因此开发了增强版本ULS+。

Method: 整合了新增的公共数据集，采用更小的输入图像尺寸，在ULS23挑战赛测试数据和Longitudinal-CT数据集子集上使用Dice分数和点击点位置鲁棒性进行评估。

Result: 在所有比较中，ULS+显著优于原始ULS模型，并在ULS23挑战赛测试阶段排行榜中排名第一。

Conclusion: 通过数据驱动的更新和临床验证循环，ULS+为构建鲁棒且具有临床相关性的病灶分割模型奠定了基础。
...
### [70] [Towards Faithful Reasoning in Comics for Small MLLMs](https://arxiv.org/abs/2601.02991)
*Chengcheng Feng,Haojie Yin,Yucheng Jin,Kaizhu Huang*

Main category: cs.CV

TL;DR: 提出了一种针对漫画视觉问答（CVQA）的新型推理框架，通过模块化思维链生成、GRPO强化微调和结构化奖励，解决了小规模多模态大语言模型在CVQA任务中应用标准思维链提示时性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 漫画视觉问答依赖符号抽象、叙事逻辑和幽默，与常规VQA任务不同。研究发现标准思维链提示在CVQA中会导致状态纠缠、虚假转移和探索效率低下，尤其在小规模模型中性能下降明显，需要专门解决方案。

Method: 设计了漫画推理框架，结合模块化思维链生成、基于GRPO的强化微调和新型结构化奖励机制，专门优化小规模MLLMs在CVQA任务中的推理能力。

Result: 在五个挑战性基准测试中，提出的3B模型优于现有最先进方法，插件实验在不同MLLMs上实现了平均12.1%的额外性能提升，并在更广泛的幽默中心和抽象视觉推理任务（如表情包理解和社论漫画解读）上表现出色。

Conclusion: 该研究证明了专门设计的推理框架能有效解决小规模MLLMs在复杂视觉语言任务（如CVQA）中的推理缺陷，为资源受限环境下的高级视觉理解提供了实用解决方案。
...
### [71] [Towards Efficient 3D Object Detection for Vehicle-Infrastructure Collaboration via Risk-Intent Selection](https://arxiv.org/abs/2601.03001)
*Li Wang,Boqi Li,Hang Chen,Xingjian Wu,Yichen Wang,Jiewen Tan,Xinyu Zhang,Huaping Liu*

Main category: cs.CV

TL;DR: 提出RiSe框架，通过风险意图选择性检测，仅传输高风险交互区域的特征，在保持检测精度的同时将通信量降至全特征共享的0.71%。


<details>
  <summary>Details</summary>
Motivation: 车路协同感知中通信带宽与特征冗余的权衡是关键瓶颈，现有方法在空间压缩或静态置信度图方面效率不足，仍会传输非关键背景区域的冗余特征。

Method: 1. 基于势场理论构建潜在场-轨迹关联模型定量评估运动学风险；2. 利用自车运动先验设计意图驱动区域预测模块，主动预测关键BEV区域；3. 实现语义选择性融合方案，仅传输高风险交互区域的高保真特征。

Result: 在DeepAccident数据集上的实验表明，该方法在保持最先进检测精度的同时，将通信量降至全特征共享的0.71%，在带宽效率与感知性能间建立了有竞争力的帕累托前沿。

Conclusion: RiSe框架通过从识别可见区域转向优先处理风险关键区域的范式转变，实现了通信效率与感知性能的有效平衡，为车路协同感知提供了实用的解决方案。
...
### [72] [ReCCur: A Recursive Corner-Case Curation Framework for Robust Vision-Language Understanding in Open and Edge Scenarios](https://arxiv.org/abs/2601.03011)
*Yihan Wei,Shenghai Yuan,Tianchen Deng,Boyang Lou,Enwen Hu*

Main category: cs.CV

TL;DR: 提出ReCCur框架，通过多智能体递归流程将噪声网络图像转化为可审计的细粒度标签，用于高效处理现实世界中的极端场景数据。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的极端场景（corner cases）数据难以规模化收集和标注：网络数据噪声大、标签脆弱、边缘部署限制大规模重训练。

Method: 1. 大规模数据采集与过滤：使用视觉语言模型扩展领域词汇，爬取网络数据，通过三模态一致性检查和人工抽查获得候选数据；2. 专家混合知识蒸馏：使用互补编码器进行kNN投票，结合双重置信激活和不确定性采样；3. 区域证据VLM对抗标注：提案者生成多粒度区域和语义线索，验证者进行全局和局部链式一致性检验。

Result: 在现实极端场景（如淹水车辆检测）中，ReCCur可在消费级GPU上运行，持续提升数据纯度和可分离性，且仅需极少人工监督。

Conclusion: ReCCur为资源受限条件下的下游训练和评估提供了实用基础，代码和数据集将开源发布。
...
### [73] [SA-ResGS: Self-Augmented Residual 3D Gaussian Splatting for Next Best View Selection](https://arxiv.org/abs/2601.03024)
*Kim Jun-Seong,Tae-Hyun Oh,Eduardo Pérez-Pellitero,Youngkyoon Jang*

Main category: cs.CV

TL;DR: 提出SA-ResGS框架，通过自增强点云和残差学习改进3D高斯溅射，提升主动场景重建中下一最佳视角选择的稳定性和不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 主动场景重建中，稀疏和宽基线视角导致不确定性估计不可靠、高斯元素监督不足，影响下一最佳视角选择的效果和重建质量。

Method: 1. 通过训练视图与光栅化外推视图的三角测量生成自增强点云（SA-Points）；2. 针对3D高斯溅射设计残差学习策略，结合不确定性驱动过滤和受dropout/难负样本挖掘启发的采样；3. 提出物理基础的视角选择策略。

Result: 在主动视角选择实验中，SA-ResGS在重建质量和视角选择鲁棒性上均优于现有先进基线方法。

Conclusion: SA-ResGS通过约束视角选择和残差监督，隐式纠正了不确定性量化的偏差，有效解决了宽基线探索与稀疏视角模糊性在NBV规划中的冲突问题。
...
### [74] [Flow Matching and Diffusion Models via PointNet for Generating Fluid Fields on Irregular Geometries](https://arxiv.org/abs/2601.03030)
*Ali Kashefi*

Main category: cs.CV

TL;DR: 提出了Flow Matching PointNet和Diffusion PointNet两种生成式几何深度学习框架，通过将PointNet与流匹配和扩散模型结合，直接基于点云预测不规则几何上的流体流动变量，避免了传统像素化方法的局限，并在圆柱绕流问题上表现出更高的精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理不规则几何的流体流动预测时，常依赖像素化投影或图神经网络，可能导致高频噪声或复杂架构，因此需要一种更简单、直接且准确的方法来生成物理场。

Method: 将PointNet集成到流匹配和扩散模型中，构建生成式框架，通过反向生成过程从高斯噪声重建物理场，直接操作点云表示的计算域，无需中间网络进行几何条件化。

Result: 在圆柱绕流数据集上测试，相比参数数量相同的普通PointNet，所提框架在速度场、压力场预测以及升力、阻力计算上更准确，对不完整几何更具鲁棒性，且无高频噪声伪影。

Conclusion: Flow Matching PointNet和Diffusion PointNet提供了一种统一且高效的架构，能直接处理点云几何，提升流体流动预测的精度和鲁棒性，为几何深度学习在物理模拟中的应用提供了新方向。
...
### [75] [IBISAgent: Reinforcing Pixel-Level Visual Reasoning in MLLMs for Universal Biomedical Object Referring and Segmentation](https://arxiv.org/abs/2601.03054)
*Yankai Jiang,Qiaoru Li,Binlu Xu,Haoran Sun,Chao Ding,Junting Dong,Yuxiang Cai,Xuhong Zhang,Jianwei Yin*

Main category: cs.CV

TL;DR: 提出IBISAgent代理式多模态大模型，将医学图像分割重构为视觉中心的多步决策过程，通过迭代推理和文本点击动作调用分割工具，无需修改架构即可生成高质量分割掩码。


<details>
  <summary>Details</summary>
Motivation: 现有医学MLLM分割方法存在两大局限：1）引入隐式分割标记需同时微调MLLM和外部像素解码器，易导致灾难性遗忘且泛化能力受限；2）依赖单次推理缺乏迭代优化能力，导致性能欠佳。

Method: 1）设计IBISAgent代理框架，支持交错生成视觉推理和基于文本的点击动作以调用分割工具；2）提出两阶段训练框架：冷启动监督微调 + 基于细粒度奖励的代理强化学习；3）通过掩码图像特征的多步视觉推理实现掩码迭代优化。

Result: 在复杂医学指代和推理分割任务中，IBISAgent在多个数据集上持续超越闭源和开源SOTA方法，验证了其鲁棒性和优越性能。

Conclusion: 将分割任务重构为代理式多步决策过程能有效提升医学MLLM的像素级理解能力，两阶段训练策略增强了模型在复杂场景下的适应性，相关资源将开源促进领域发展。
...
### [76] [On the Intrinsic Limits of Transformer Image Embeddings in Non-Solvable Spatial Reasoning](https://arxiv.org/abs/2601.03048)
*Siyi Lyu,Quan Liu,Feng Yan*

Main category: cs.CV

TL;DR: 本文指出视觉Transformer在空间推理任务上的系统失败源于架构固有的电路复杂性限制，而非数据规模问题。通过将空间理解形式化为群同态学习，证明了对于非可解群，ViT的常数深度架构在计算复杂性上存在根本性不足。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer在语义识别上表现出色，但在心理旋转等空间推理任务中存在系统性失败。传统观点将其归因于数据规模，但本文认为这源于架构内在的电路复杂性限制，需要从计算复杂性角度解释这一现象。

Method: 将空间理解形式化为学习群同态：将图像序列映射到保持底层变换群代数结构的潜在空间。通过计算复杂性理论分析，证明对于非可解群（如SO(3)），保持结构保持嵌入的计算下界为NC^1完全问题，而常数深度ViT严格受限于TC^0类。

Result: 在TC^0 ⊊ NC^1猜想下，建立了复杂性边界：常数深度ViT从根本上缺乏有效捕捉非可解空间结构所需的逻辑深度。通过潜在空间探测验证了这一复杂性差距，表明随着组合深度增加，ViT表示在非可解任务上会出现结构崩溃。

Conclusion: 视觉Transformer在空间推理任务上的局限性源于架构固有的计算复杂性约束，特别是常数深度设计无法有效处理非可解群的结构保持表示。这为理解Transformer类模型的认知边界提供了理论框架。
...
### [77] [Motion Blur Robust Wheat Pest Damage Detection with Dynamic Fuzzy Feature Fusion](https://arxiv.org/abs/2601.03046)
*Han Zhang,Yanwei Wang,Fang Li,Hongjun Wang*

Main category: cs.CV

TL;DR: 提出DFRCP（动态模糊鲁棒卷积金字塔）作为YOLOv11的插件升级，用于运动模糊场景下的目标检测，通过动态鲁棒开关单元和模糊特征增强全局感知，在保持低延迟的同时显著提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 相机抖动导致的运动模糊会产生重影伪影，严重降低边缘设备的目标检测性能。现有方法要么将模糊视为噪声而丢失判别性结构，要么进行全图像恢复导致延迟增加，难以在资源受限设备上部署。

Method: 1. 在YOLOv11特征金字塔中结合大尺度与中尺度特征，保留原生表示；2. 引入动态鲁棒开关单元，自适应注入模糊特征以增强抖动下的全局感知；3. 通过旋转和非线性插值合成多尺度模糊特征，利用透明度卷积学习内容自适应的原始与模糊线索权衡；4. 开发CUDA并行旋转与插值内核，避免边界溢出并实现400倍加速；5. 在约3500张图像的私有小麦害虫数据集上进行配对监督训练，使用两种模糊机制（均匀图像运动模糊和边界框限制的旋转模糊）进行三倍数据增强。

Result: 在模糊测试集上，搭载DFRCP的YOLOv11比基线YOLOv11准确率提升约10.4%，仅带来适度的训练时间开销，减少了数据收集后的人工过滤需求。

Conclusion: DFRCP作为一种即插即用的升级方案，能够在边缘设备上实现高效的模糊鲁棒目标检测，通过动态模糊特征增强和硬件优化，在精度与延迟之间取得良好平衡，为农业害虫监测等实际应用提供了可行解决方案。
...
### [78] [Fine-Grained Generalization via Structuralizing Concept and Feature Space into Commonality, Specificity and Confounding](https://arxiv.org/abs/2601.03056)
*Zhen Wang,Jiaojiao Zhao,Qilong Wang,Yongfeng Dong,Wenlong Yu*

Main category: cs.CV

TL;DR: 提出CFSG模型，通过将概念和特征空间解耦为共同、特定和混淆三个结构化组件，并引入自适应机制动态调整组件比例，显著提升了细粒度域泛化任务的性能。


<details>
  <summary>Details</summary>
Motivation: 细粒度域泛化任务中，由于类间差异细微而类内变化显著，模型在域偏移下对细粒度线索过度敏感，导致关键特征被抑制和性能下降。受人类利用共同与特定属性进行细粒度分类的认知机制启发，旨在将这一机制有效引入深度学习模型。

Method: 提出概念-特征结构化泛化模型，将概念和特征空间显式解耦为共同、特定和混淆三个结构化组件；引入自适应机制动态调整各组件比例以应对不同程度的分布偏移；在最终预测中为每对组件分配显式权重。

Result: 在三个单源基准数据集上的实验表明，CFSG相比基线模型平均性能提升9.87%，优于现有最先进方法平均3.08%；可解释性分析验证了模型能有效整合多粒度结构化知识，且特征结构化促进了概念结构化的涌现。

Conclusion: CFSG通过模拟人类认知中的结构化属性利用机制，有效解决了细粒度域泛化中的特征敏感性问题，显著提升了模型泛化能力，为细粒度识别任务提供了新的解决方案。
...
### [79] [Understanding Multi-Agent Reasoning with Large Language Models for Cartoon VQA](https://arxiv.org/abs/2601.03073)
*Tong Wu,Thanet Markchom*

Main category: cs.CV

TL;DR: 提出了一种用于卡通图像视觉问答的多智能体大语言模型框架，通过视觉、语言和批评三个智能体的协作，解决卡通图像中视觉抽象和叙事上下文带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 标准大语言模型在自然图像上训练，难以处理卡通图像中夸张的视觉抽象和叙事驱动的上下文，导致在卡通视觉问答任务上表现不佳。

Method: 设计了一个多智能体LLM框架，包含三个专门化智能体：视觉智能体（提取视觉特征）、语言智能体（处理文本信息）和批评智能体（评估和整合信息），通过协作实现结构化推理。

Result: 在Pororo和Simpsons两个卡通VQA数据集上进行了系统评估，实验结果表明该框架能有效处理卡通图像问答，并详细分析了各智能体对最终预测的贡献。

Conclusion: 该多智能体框架为卡通视觉问答提供了有效的解决方案，增进了对基于LLM的多智能体在卡通VQA和多模态推理中行为的理解。
...
### [80] [LesionTABE: Equitable AI for Skin Lesion Detection](https://arxiv.org/abs/2601.03090)
*Rocio Mexia Diaz,Yasmin Greenway,Petru Manescu*

Main category: cs.CV

TL;DR: 提出LesionTABE框架，通过对抗性去偏和皮肤病学基础模型嵌入，显著改善AI皮肤诊断模型在深色肤色上的性能，提升公平性指标25%以上。


<details>
  <summary>Details</summary>
Motivation: AI皮肤病诊断模型在深色肤色上表现不佳，存在偏见问题，阻碍了临床AI的公平应用。

Method: 结合对抗性去偏技术和皮肤病学专用基础模型嵌入，构建LesionTABE公平性框架。

Result: 在多个数据集上评估，相比ResNet-152基线，公平性指标提升超过25%，优于现有去偏方法，同时提高了整体诊断准确率。

Conclusion: 基础模型去偏是实现临床AI公平应用的有效途径，LesionTABE框架展示了这一潜力。
...
### [81] [Text-Guided Layer Fusion Mitigates Hallucination in Multimodal LLMs](https://arxiv.org/abs/2601.03100)
*Chenchen Lin,Sanbao Su,Rachel Luo,Yuxiao Chen,Yan Wang,Marco Pavone,Fei Miao*

Main category: cs.CV

TL;DR: 本文提出TGIF模块，通过文本引导的动态跨层融合，利用视觉编码器的多层次特征，增强多模态大语言模型的视觉基础并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM通常仅使用视觉编码器的单一高层特征，忽略了丰富的多层次视觉线索，导致模型依赖语言先验而非图像证据，产生视觉未接地的幻觉。

Method: 提出TGIF（文本引导的层间融合）轻量模块，将视觉编码器各层视为深度专家，根据查询动态预测特征融合权重，无需更新视觉编码器，开销极小。

Result: 在LLaVA-1.5-7B上集成TGIF后，在幻觉检测、OCR和VQA基准上取得一致提升，同时在ScienceQA、GQA和MMBench上保持或改进性能。

Conclusion: 查询条件化、层次感知的特征融合是增强现代MLLM视觉基础、减少幻觉的有效方法。
...
### [82] [LeafLife: An Explainable Deep Learning Framework with Robustness for Grape Leaf Disease Recognition](https://arxiv.org/abs/2601.03124)
*B. M. Shahria Alam,Md. Nasim Ahmed*

Main category: cs.CV

TL;DR: 该研究开发了一个基于深度学习的葡萄叶病害检测系统，使用Xception模型达到96.23%的准确率，并集成了对抗训练、Grad-CAM可视化，最终部署为Streamlit网络应用。


<details>
  <summary>Details</summary>
Motivation: 植物病害会降低作物产量和品质，葡萄叶病害检测对农业生产至关重要，需要高效准确的自动化诊断方法。

Method: 使用包含9,032张图像（4个类别）的葡萄叶病害数据集，经过预处理后按70%/20%/10%划分训练/验证/测试集，部署InceptionV3和Xception预训练模型，采用对抗训练增强鲁棒性，集成Grad-CAM提供可视化解释。

Result: Xception模型表现最佳，测试准确率达到96.23%，优于InceptionV3模型；系统能够提供病害预测、置信度及热力图可视化。

Conclusion: 基于Xception的深度学习模型能有效检测葡萄叶病害，结合对抗训练和可视化技术提高了系统的鲁棒性和可解释性，并通过网络应用实现了实用化部署。
...
### [83] [Unified Thinker: A General Reasoning Modular Core for Image Generation](https://arxiv.org/abs/2601.03127)
...
TL;DR: 提出Unified Thinker架构，通过可执行推理将高级意图分解为可验证计划来指导图像生成，显著提升开源模型的逻辑推理图像生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在逻辑密集型指令跟随上存在推理-执行差距，闭源系统（如Nano Banana）已展示强大推理驱动图像生成能力，开源模型与之存在明显差距。

Method: 1. 提出任务无关的Unified Thinker架构，将专用推理器（Thinker）与图像生成器（Generator）解耦；2. 采用两阶段训练：先构建结构化规划接口，再通过强化学习将策略基于像素级反馈进行优化。

Result: 在文本到图像生成和图像编辑任务上的大量实验表明，Unified Thinker显著提升了图像推理和生成质量。

Conclusion: 通过可执行推理架构将高级意图分解为可验证计划，能够有效缩小开源与闭源系统在推理驱动图像生成上的差距，且该架构支持模块化升级而无需重新训练整个生成模型。
...
### [84] [LSP-DETR: Efficient and Scalable Nuclei Segmentation in Whole Slide Images](https://arxiv.org/abs/2601.03163)
*Matěj Pekár,Vít Musil,Rudolf Nenutil,Petr Holub,Tomáš Brázdil*

Main category: cs.CV

TL;DR: 提出LSP-DETR框架，利用轻量级Transformer和星形凸多边形表示，实现高效、端到端的细胞核实例分割，无需后处理且速度快5倍以上。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中全切片图像的细胞核实例分割面临计算挑战，现有方法依赖分块处理和复杂后处理，牺牲上下文信息与效率。

Method: 采用局部星形多边形检测Transformer（LSP-DETR），使用线性复杂度的轻量Transformer处理大图像，通过径向距离损失函数自然分割重叠细胞核，无需重叠标注或手工后处理。

Result: 在PanNuke和MoNuSeg数据集上表现出强泛化能力和最先进的效率，比次优方法快5倍以上。

Conclusion: LSP-DETR提供了一种可扩展、端到端的细胞核分割解决方案，显著提升计算效率与泛化性能，适用于计算病理学应用。
...
### [85] [AnatomiX, an Anatomy-Aware Grounded Multimodal Large Language Model for Chest X-Ray Interpretation](https://arxiv.org/abs/2601.03191)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态医学大语言模型在空间推理和解剖学理解方面存在不足，缺乏真正的解剖对应关系，导致医学领域理解错误。

Method: 采用两阶段方法：首先识别解剖结构并提取特征，然后利用大语言模型执行短语定位、报告生成、视觉问答和图像理解等下游任务。

Result: 在多个基准测试中，AnatomiX在解剖学推理方面表现优异，在解剖学定位、短语定位、基础诊断和基础描述任务上比现有方法性能提升超过25%。

Conclusion: AnatomiX通过解剖学基础的方法显著提升了胸部X光片解读的准确性和可靠性，代码和预训练模型已开源。
...
### [86] [DiffBench Meets DiffAgent: End-to-End LLM-Driven Diffusion Acceleration Code Generation](https://arxiv.org/abs/2601.03178)
...
TL;DR: 提出DiffBench基准和DiffAgent框架，利用大语言模型自动生成和评估扩散模型加速代码，解决多技术组合优化难题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的多步推理过程计算开销大，阻碍实际部署；现有加速技术组合策略缺乏自动化优化方案。

Method: 1) 构建DiffBench基准，实现三阶段自动化评估流程；2) 设计DiffAgent智能体，通过规划-代码生成-调试的闭环工作流结合遗传算法优化加速策略。

Result: DiffBench能全面评估生成代码质量；DiffAgent在生成扩散模型加速策略方面显著优于现有大语言模型。

Conclusion: 该框架为扩散模型加速提供了自动化解决方案，通过系统化基准和智能体协同实现了高效策略生成与优化。
...
### [87] [LTX-2: Efficient Joint Audio-Visual Foundation Model](https://arxiv.org/abs/2601.03233)
*Yoav HaCohen,Benny Brazowski,Nisan Chiprut,Yaki Bitterman,Andrew Kvochko,Avishai Berkowitz,Daniel Shalem,Daphna Lifschitz,Dudu Moshe,Eitan Porat,Eitan Richardson,Guy Shiran,Itay Chachy,Jonathan Chetboun,Michael Finkelson,Michael Kupchick,Nir Zabari,Nitzan Guetta,Noa Kotler,Ofir Bibi,Ori Gordon,Poriya Panet,Roi Benita,Shahar Armon,Victor Kulikov,Yaron Inger,Yonatan Shiftan,Zeev Melumian,Zeev Farbman*

Main category: cs.CV

TL;DR: LTX-2是一个开源基础模型，能够统一生成高质量、时间同步的视听内容，通过非对称双流变压器架构实现高效训练和推理，在开源系统中达到最先进的视听质量和提示遵循度。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频扩散模型虽然能生成引人注目的视频序列，但缺乏音频提供的语义、情感和氛围线索，因此需要开发能够同步生成高质量视听内容的方法。

Method: 采用非对称双流变压器架构，包含140亿参数视频流和50亿参数音频流，通过双向视听交叉注意力层、时间位置嵌入和跨模态AdaLN进行耦合；使用多语言文本编码器增强提示理解，并引入模态感知的无分类器引导机制以改善视听对齐和可控性。

Result: 该模型不仅能生成语音，还能产生丰富、连贯的音频轨道，跟随场景中的角色、环境、风格和情感，包含自然的背景和拟音元素；在评估中，其在开源系统中达到最先进的视听质量和提示遵循度，同时以较低计算成本和推理时间实现与专有模型相当的结果。

Conclusion: LTX-2通过统一的视听生成架构，有效解决了现有文本到视频模型缺乏音频的问题，为开源社区提供了高质量、高效率的视听内容生成解决方案，所有模型权重和代码均已公开发布。
...
### [88] [UniCorn: Towards Self-Improving Unified Multimodal Models through Self-Generated Supervision](https://arxiv.org/abs/2601.03193)
...
TL;DR: 本文提出UniCorn框架，通过自监督方式解决统一多模态模型在生成任务中的表现差距，将模型分为三个协作角色进行自我改进，显著提升了文本到图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在跨模态理解方面表现出色，但在利用内部知识进行高质量生成时存在明显差距，作者将这种现象形式化为'传导性失语症'，即模型能准确理解多模态输入却难以将其转化为忠实可控的合成输出。

Method: 提出UniCorn自改进框架，将单个统一多模态模型划分为三个协作角色：提议者、求解者和评判者，通过自我博弈生成高质量交互，并采用认知模式重构将潜在理解提炼为显式生成信号。同时引入基于文本-图像-文本重建循环的UniCycle基准来验证多模态连贯性恢复。

Result: 在六个通用图像生成基准测试中，UniCorn相比基础模型取得全面显著改进，在TIIF(73.8)、DPG(86.8)、CompBench(88.5)和UniCycle上达到SOTA性能，在WISE和OneIG上分别提升+5.0和+6.5分。

Conclusion: 该方法在显著提升文本到图像生成能力的同时保持了强大的理解能力，证明了完全自监督精炼方法对于统一多模态智能的可扩展性。
...
### [89] [A Versatile Multimodal Agent for Multimedia Content Generation](https://arxiv.org/abs/2601.03250)
*Daoan Zhang,Wenlin Yao,Xiaoyang Wang,Yebowen Hu,Jiebo Luo,Dong Yu*

Main category: cs.CV

TL;DR: 提出MultiMedia-Agent系统，利用基于代理的方法自动化复杂多媒体内容创作，通过技能获取理论、两阶段规划优化和三阶段训练实现多模态内容生成。


<details>
  <summary>Details</summary>
Motivation: 当前AIGC模型多为单模态或特定场景组件，无法端到端处理现实应用中多模态（视频、音频、文本等）内容创作需求，而基于代理的系统为复杂内容生成任务提供了新可能。

Method: 设计包含数据生成管道、创作工具库和偏好对齐评估指标的系统；引入技能获取理论指导数据构建与代理训练；采用自关联和模型偏好关联的两阶段规划优化策略；通过基础/成功计划微调和偏好优化的三阶段方法训练代理。

Result: 对比实验表明该方法有效，MultiMedia-Agent能生成优于现有新型模型的多媒体内容。

Conclusion: 所提出的代理系统能够自动化复杂内容创作，解决当前AIGC模型在多模态集成和端到端应用中的局限性，为实际多媒体内容生产提供可行方案。
...
### [90] [InfiniDepth: Arbitrary-Resolution and Fine-Grained Depth Estimation with Neural Implicit Fields](https://arxiv.org/abs/2601.03252)
*Hao Yu,Haotong Lin,Jiawei Wang,Jiaxin Li,Yida Wang,Xueyang Zhang,Yue Wang,Xiaowei Zhou,Ruizhen Hu,Sida Peng*

Main category: cs.CV

TL;DR: 提出InfiniDepth方法，将深度表示为神经隐式场，通过局部隐式解码器实现任意分辨率的连续深度估计，在合成和真实数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度估计方法受限于离散图像网格表示，无法扩展到任意输出分辨率且难以恢复几何细节，需要一种更灵活的深度表示方法。

Method: 使用神经隐式场表示深度，设计局部隐式解码器在连续2D坐标查询深度值，构建包含5款游戏的4K合成基准数据集进行验证。

Result: 在合成和真实数据集上实现最先进的相对深度和度量深度估计性能，在细节区域表现优异，并能提升大视角变化下的新视角合成质量。

Conclusion: 神经隐式场表示能有效支持任意分辨率的精细深度估计，在深度估计和新视角合成任务中均展现出优越性能。
...
### [91] [Muses: Designing, Composing, Generating Nonexistent Fantasy 3D Creatures without Training](https://arxiv.org/abs/2601.03256)
*Hexiao Lu,Xiaokun Sun,Zeyu Cai,Hao Guo,Ying Tai,Jian Yang,Zhenyu Zhang*

Main category: cs.CV

TL;DR: Muses是一种无需训练的前馈式3D生物生成方法，通过骨骼引导的体素组装和外观建模，实现高质量、风格一致的奇幻生物生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖部件优化、手动组装或2D图像生成，常因复杂的部件级操作和跨域生成限制导致3D资产不真实或不连贯。

Method: 1. 通过图约束推理构建具有连贯布局的创意3D骨骼；2. 在结构化潜在空间中进行骨骼引导的体素组装；3. 在骨骼条件下进行图像引导的外观建模以生成协调纹理。

Result: 大量实验表明Muses在视觉保真度、文本描述对齐方面达到最先进水平，并展现出灵活的3D物体编辑潜力。

Conclusion: Muses通过骨骼表示将3D内容创建形式化为结构感知的设计-组合-生成流程，首次实现了无需训练的前馈式奇幻3D生物生成。
...
### [92] [PCEval: A Benchmark for Evaluating Physical Computing Capabilities of Large Language Models](https://arxiv.org/abs/2601.02404)
*Inpyo Song,Eunji Jeon,Jangwon Lee*

Main category: cs.CL

TL;DR: 提出了首个物理计算基准测试PCEval，用于自动评估LLM在硬件约束下的逻辑和物理实现能力，发现LLM在代码生成表现良好，但在物理电路布局方面存在显著困难。


<details>
  <summary>Details</summary>
Motivation: LLM在软件开发等领域表现出色，但在涉及硬件交互的物理计算场景中，其能力尚未得到充分探索，需要建立评估框架来填补这一研究空白。

Method: 开发了PCEval基准测试框架，通过仿真环境对13个主流LLM进行自动化评估，测试其在生成电路和兼容代码方面的能力，涵盖不同复杂度的项目。

Result: LLM在代码生成和逻辑电路设计方面表现良好，但在物理面包板布局创建上存在显著困难，特别是在引脚连接管理和避免电路错误方面。

Conclusion: PCEval为理解AI在硬件依赖环境中的辅助能力提供了基础，有助于开发更有效的物理计算教育工具，并揭示了LLM在物理实现方面的局限性。
...
### [93] [WearVox: An Egocentric Multichannel Voice Assistant Benchmark for Wearables](https://arxiv.org/abs/2601.02391)
...
TL;DR: 本文提出了首个针对可穿戴语音助手的真实场景基准测试WearVox，包含3842条多通道音频数据，覆盖多种任务和环境，揭示了现有语音大模型在真实可穿戴场景中的性能局限，并证明了多通道音频对提升鲁棒性的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有语音助手基准测试主要针对干净或通用对话音频，忽略了可穿戴设备（如AI眼镜）在真实场景中面临的挑战，包括运动噪声、环境干扰、设备定向语音识别等复杂问题，缺乏能够系统评估可穿戴语音助手性能的基准。

Method: 构建WearVox基准数据集，包含3842条通过AI眼镜采集的多通道自我中心音频，覆盖搜索增强问答、闭卷问答、侧谈拒绝、工具调用和语音翻译五种任务，涵盖室内外多种声学环境。对主流专有和开源语音大模型进行基准测试，并通过单通道与多通道音频的对比实验分析空间音频线索的影响。

Result: 实时语音大模型在WearVox上的准确率仅为29%-59%，在嘈杂室外音频上性能显著下降；多通道音频输入相比单通道能显著提升模型对环境噪声的鲁棒性，并改善设备定向语音与背景语音的区分能力。

Conclusion: 空间音频线索对于上下文感知语音助手至关重要；WearVox为可穿戴语音AI研究提供了全面的测试平台，揭示了当前模型在真实可穿戴场景中的性能差距，并指明了多通道音频处理是提升鲁棒性的有效方向。
...
### [94] [Losses that Cook: Topological Optimal Transport for Structured Recipe Generation](https://arxiv.org/abs/2601.02531)
...
<details>
  <summary>Details</summary>
Motivation: 标准训练方法主要基于交叉熵损失，仅关注文本流畅性，但食谱生成需要同时保证食材准确性、时间/温度精度和步骤连贯性，现有方法无法全面满足这些要求。

Method: 基于RECIPE-NLG框架，研究了多种复合目标函数，提出新的拓扑损失函数，将食材列表表示为嵌入空间点云，并结合Dice损失和混合损失进行实验。

Result: 拓扑损失显著提升了食材和动作层面的指标；Dice损失在时间/温度精度上表现优异；混合损失在数量和时间指标上取得协同增益；人工偏好评估中模型在62%的情况下被优先选择。

Conclusion: 复合损失函数能有效提升食谱生成的多维度质量，拓扑损失特别适用于食材准确性优化，不同损失函数可针对性地改善食谱生成的不同方面。
...
### [95] [ModeX: Evaluator-Free Best-of-N Selection for Open-Ended Generation](https://arxiv.org/abs/2601.02535)
*Hyeong Kyu Choi,Sharon Li*

Main category: cs.CL

TL;DR: 提出ModeX框架，通过识别生成文本中的主导语义共识来选择最佳输出，无需外部评估器，在开放任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部评估器或精确字符串匹配，限制了在开放任务中的适用性和效率，需要一种无需评估器的多输出选择方法。

Method: 构建候选文本相似度图，递归应用谱聚类选择代表性中心点；提出ModeX-Lite版本，通过早期剪枝提高效率。

Result: 在文本摘要、代码生成和数学推理等开放任务中，方法持续优于单路径和多路径基线，计算效率高。

Conclusion: ModeX为开放文本生成提供了高效稳健的解决方案，通过语义共识选择显著提升输出质量。
...
TL;DR: 提出LoRA-Drop框架，通过周期性刷新和LoRA修正，在保持精度的同时实现2.6倍解码加速和45-55% KV缓存减少。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型受限于顺序解码的计算开销，现有动态深度和层跳过方法存在路由机制复杂或精度下降的问题，需要一种高效且保持精度的推理加速方案。

Method: 采用时间计算调度策略：多数解码步骤中，选定层复用前一令牌的隐藏状态并应用低秩LoRA修正；定期执行刷新步骤运行完整模型以防止漂移，无需路由网络且兼容标准KV缓存。

Result: 在LLaMA2-7B、LLaMA3-8B、Qwen2.5-7B和Qwen2.5-14B模型上，解码速度提升最高2.6倍，KV缓存减少45-55%，精度损失控制在0.5个百分点内，在推理、代码生成和多语言任务中均保持性能。

Conclusion: LoRA-Drop提供了一种简单有效的自适应容量推理方案，通过调度配置的安全区在保证质量的同时显著提升效率，为大语言模型推理加速提供了新路径。
...
### [97] [Fact-Checking with Large Language Models via Probabilistic Certainty and Consistency](https://arxiv.org/abs/2601.02574)
*Haoran Wang,Maryam Khalid,Qiong Wu,Jian Gao,Cheng Cao*

Main category: cs.CL

TL;DR: 提出PCC框架，通过联合建模LLM的概率确定性和推理一致性来估计事实置信度，实现基于置信度的自适应验证策略，提高事实核查的效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM事实核查方法通常无差别检索外部证据，忽略了模型内部知识且可能引入噪声，同时缺乏针对性解决模型推理中不确定性的机制。

Method: 提出概率确定性与一致性（PCC）框架，通过建模LLM的概率确定性和推理一致性来估计事实置信度，并基于置信度实现自适应验证策略（直接回答、触发定向检索或深度搜索）。

Result: 在三个基准测试上的实验表明，PCC在不确定性量化方面优于口头置信度方法，且持续超越基于LLM的事实核查基线方法，同时在不同LLM上表现出良好的泛化能力。

Conclusion: PCC框架通过置信度引导的自适应验证策略，有效结合LLM内部知识和外部检索，提高了事实核查的准确性和效率，为LLM的事实可靠性提供了新解决方案。
...
### [98] [DataParasite Enables Scalable and Repurposable Online Data Curation](https://arxiv.org/abs/2601.02578)
*Mengyi Sun*

Main category: cs.CL

TL;DR: DataParasite是一个开源、模块化的在线数据收集管道，通过分解表格化数据收集任务为独立的实体级搜索，显著降低计算社会科学中的数据收集成本。


<details>
  <summary>Details</summary>
Motivation: 当前从异构在线来源组装数据集的过程通常劳动密集、成本高昂且难以复现，现有基于大语言模型的系统往往不透明、不灵活或不适合科学数据管理。

Method: 通过轻量级配置文件定义实体级搜索任务，使用共享的、任务无关的Python脚本执行，支持仅通过自然语言指令重新调整管道用途以适应新任务。

Result: 在多个计算社会科学典型任务（如教师招聘历史、精英死亡事件、政治生涯轨迹）中，DataParasite在保持高准确性的同时，将数据收集成本相对于人工管理降低了一个数量级。

Conclusion: DataParasite通过降低在线数据组装的技术和劳动壁垒，为计算社会科学及其他领域提供了可扩展、透明和可复用的数据管理实践基础。
...
### [99] [Reconstructing Item Characteristic Curves using Fine-Tuned Large Language Models](https://arxiv.org/abs/2601.02580)
*Christopher Ormerod*

Main category: cs.CL

TL;DR: 本研究提出了一种利用大语言模型模拟学生反应来估计项目反应理论参数的新方法，避免了传统现场测试的高成本。


<details>
  <summary>Details</summary>
Motivation: 传统项目参数确定方法依赖昂贵的现场测试收集学生表现数据，本研究旨在开发一种更高效、低成本的项目参数估计方法。

Method: 使用Qwen-3密集模型系列和低秩适应技术，微调大语言模型来模拟不同潜在能力水平学生对选择题的反应，通过生成合成项目特征曲线来估计IRT参数。

Result: 在六年级英语语言艺术项目数据集和BEA 2024共享任务数据集上的评估表明，该方法与基线方法相当或更优，特别在建模项目区分度方面表现突出。

Conclusion: 基于模拟的方法为项目参数估计提供了一种有前景的替代方案，能够有效建模项目心理测量特性，减少对现场测试的依赖。
...
### [100] [FlowPlan-G2P: A Structured Generation Framework for Transforming Scientific Papers into Patent Descriptions](https://arxiv.org/abs/2601.02589)
*Kris W Pan,Yongmin Yoo*

Main category: cs.CL

TL;DR: 提出FlowPlan-G2P框架，通过模拟专家认知流程将科学论文转化为专利描述，显著提升逻辑连贯性和法律合规性。


<details>
  <summary>Details</summary>
Motivation: 每年有超过350万份专利申请，但将科学论文转化为专利描述面临修辞风格差异和法律要求严格的挑战，现有黑盒文本生成方法难以处理结构推理和法律约束。

Method: 采用三阶段框架：1) 概念图归纳：通过专家式推理提取技术实体和关系构建有向图；2) 段落与章节规划：将图重组为符合专利标准章节的连贯簇；3) 图条件生成：使用章节特定子图和定制提示生成法律合规段落。

Result: 实验表明FlowPlan-G2P在逻辑连贯性和法律合规性上显著优于端到端LLM基线方法。

Conclusion: 该框架为论文到专利生成建立了新范式，推动了专业领域的结构化文本生成技术发展。
...
### [101] [Scalable Construction of a Lung Cancer Knowledge Base: Profiling Semantic Reasoning in LLMs](https://arxiv.org/abs/2601.02604)
...
TL;DR: 本研究提出了一种基于开放信息抽取（OpenIE）的流程，用于构建肺癌知识库，以提升大型语言模型在生物医学领域的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生物医学研究中的应用需要高质量、领域特定的知识表示，尤其是在肿瘤学这种对精确性和可解释性要求极高的领域，需要可扩展的方法来构建结构化知识库以进行有效的微调。

Method: 采用四步流程：1）使用MeSH词表识别医学概念；2）筛选具有宽松许可（CC0）的PubMed开放文献；3）使用OpenIE方法提取（主体，关系，客体）三元组；4）通过命名实体识别（NER）丰富三元组以确保生物医学相关性。

Result: 基于该数据集微调的T5模型在ROUGE和BERTScore评估中表现出显著提升的性能和语义连贯性。

Conclusion: OpenIE衍生的资源可作为可扩展、低成本的解决方案，有效增强生物医学自然语言处理能力。
...
### [102] [Improved Evidence Extraction for Document Inconsistency Detection with LLMs](https://arxiv.org/abs/2601.02627)
*Nelvin Tan,Yaowen Zhang,James Asikin Cheung,Fusheng Liu,Yu-Ching Shih,Dong Yang*

Main category: cs.CL

TL;DR: 本文提出了一种基于大语言模型的文档不一致性检测新框架，通过证据提取指标和约束过滤机制显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多领域展现出强大能力，但在文档不一致性检测方面的研究相对有限，特别是针对不一致性证据提取这一关键任务。

Method: 提出了一种基于约束过滤的'编辑重试'框架，并引入了全面的证据提取评估指标，用于改进直接提示方法的不足。

Result: 实验结果表明，该方法在大语言模型文档不一致性检测方面取得了显著提升，验证了框架的有效性。

Conclusion: 所提出的框架和评估指标能够有效提升大语言模型在文档不一致性检测任务中的性能，特别是在证据提取方面具有明显优势。
...
### [103] [Empirical Comparison of Encoder-Based Language Models and Feature-Based Supervised Machine Learning Approaches to Automated Scoring of Long Essays](https://arxiv.org/abs/2601.02659)
*Kuo Wang,Haowei Hua,Pengfei Yan,Hong Jiao,Dan Song*

Main category: cs.CL

TL;DR: 本研究探索了编码器语言模型在长文本自动评分中的表现，发现结合多种预训练模型表征的集成模型优于单一模型。


<details>
  <summary>Details</summary>
Motivation: 长文本处理对编码器语言模型构成挑战，特别是在自动作文评分任务中，需要评估这些模型在长文本上的有效性。

Method: 训练了BERT、RoBERTa、DistilBERT、DeBERTa等编码器模型，构建了基于多模型嵌入的集成模型，以及基于梯度提升决策树等传统机器学习模型的集成模型，在17,307篇作文数据集上使用80%/10%/10%划分进行训练验证测试。

Result: 结合多种预训练语言模型表征并使用梯度提升分类器作为集成模型的集成嵌入模型，在长文本作文评分任务中显著优于单一语言模型。

Conclusion: 集成多种预训练语言模型表征的嵌入集成模型是处理长文本自动评分任务的有效方法，能够提升评分性能。
...
### [104] [When Do Tools and Planning Help LLMs Think? A Cost- and Latency-Aware Benchmark](https://arxiv.org/abs/2601.02663)
*Subha Ghoshal,Ali Al-Bustami*

Main category: cs.CL

TL;DR: 本文评估了大型语言模型在推理时使用规划与外部工具对两个真实任务（事件问答和说服性回复生成）的影响，发现工具增强能提升事件问答准确率但大幅增加延迟，而单次提示在说服性回复生成中更优，强调需根据任务权衡模型大小与工具复杂度。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型日益依赖推理时规划和外部工具来提升推理能力，但缺乏在实际任务中系统评估其效果（如准确率与延迟的权衡），因此需要实证研究以指导模型与工具的选择。

Method: 使用LangChain和LangGraph框架，在Event-QA（基于图结构知识的事件问答）和CMV（Reddit说服性回复生成）任务上，比较单次提示基线方法与配备任务特定工具（如DBpedia查询、检索、搜索）的规划-执行-重规划智能体，评估GPT-4o和GPT-4o-mini的准确率、延迟和令牌成本。

Result: 在Event-QA中，最佳工具增强配置将GPT-4o准确率从47.5%提升至67.5%，但延迟从约8秒增至约317秒；在CMV中，单次提示效果最好（如GPT-4o-mini达75%准确率，延迟约6秒），规划加搜索显著增加延迟但未稳定提升性能，且复杂工具编排会导致小模型性能下降。

Conclusion: 研究强调需要根据具体任务、成本感知来选择模型大小和智能体/工具复杂度，工具增强可能以高延迟为代价换取准确率提升，而简单提示在某些任务中可能更高效。
...
### [105] [Towards Comprehensive Stage-wise Benchmarking of Large Language Models in Fact-Checking](https://arxiv.org/abs/2601.02669)
*Hongzhan Lin,Zixin Chen,Zhiqi Shen,Ziyang Luo,Zhen Ye,Jing Ma,Tat-Seng Chua,Guandong Xu*

Main category: cs.CL

TL;DR: FactArena是一个自动化评估框架，用于全面评估大语言模型在完整事实核查流程中的表现，包括声明提取、证据检索和验证，揭示了现有基准的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型事实核查评估主要关注声明验证，忽略了完整的核查流程（声明提取和证据检索），导致无法全面揭示模型的系统性推理缺陷、事实盲点和鲁棒性限制。

Method: FactArena框架包含三个核心组件：1）LLM驱动的事实核查流程，标准化声明分解、工具增强的证据检索和基于理由的裁决预测；2）基于统一参考指南的竞技场式判断机制，确保异构评估代理的无偏比较；3）竞技场驱动的声明演化模块，自适应生成更具挑战性的声明以测试模型鲁棒性。

Result: 在涵盖7个模型家族的16个先进LLM上，FactArena产生了稳定且可解释的排名。分析显示静态声明验证准确性与端到端事实核查能力存在显著差异，证明了全面评估的必要性。

Conclusion: FactArena提供了一个可扩展且可信赖的范式，用于诊断LLM的事实推理能力，指导未来模型开发，并推动LLM在安全关键的事实核查应用中可靠部署。
...
### [106] [Multi-Turn Jailbreaking of Aligned LLMs via Lexical Anchor Tree Search](https://arxiv.org/abs/2601.02670)
*Devang Kulshreshtha,Hang Su,Chinmay Hegde,Haohan Wang*

Main category: cs.CL

TL;DR: 提出了一种无需攻击者LLM的越狱方法LATS，通过词汇锚注入实现高效攻击，在最新模型上达到97-100%成功率且平均仅需6.4次查询。


<details>
  <summary>Details</summary>
Motivation: 现有越狱方法需要攻击者LLM生成对抗性查询且查询成本高，生成的查询往往包含不可解释的随机前缀，限制了实际应用。

Method: LATS将越狱重构为多轮对话的广度优先树搜索，通过向良性提示中逐步注入攻击目标缺失的内容词汇来实现攻击。

Result: 在AdvBench和HarmBench评估中，LATS在GPT、Claude和Llama最新模型上达到97-100%攻击成功率，平均仅需约6.4次查询，显著优于需要20+次查询的其他方法。

Conclusion: 对话结构是一个强大且防护不足的攻击面，LATS展示了在攻击成功率普遍较高的时代具有卓越的查询效率优势。
...
### [107] [Iterative Structured Pruning for Large Language Models with Multi-Domain Calibration](https://arxiv.org/abs/2601.02674)
*Guangxin Wu,Hao Zhang,Zhang Zhibin,Jiafeng Guo,Xueqi Cheng*

Main category: cs.CL

TL;DR: 提出一种新颖的结构化剪枝框架，通过混合多领域校准集和迭代校准策略有效识别并移除冗余通道，在多种模型和下游任务上实现显著压缩且性能损失最小。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型规模不断增长导致计算开销、内存占用和推理延迟显著增加，现有非结构化剪枝技术产生的稀疏模式不规则，需要专用硬件或软件支持，因此探索与标准硬件兼容的结构化剪枝方法。

Method: 采用结构化剪枝方法，提出基于混合多领域校准集和迭代校准策略的框架，通过消除整个架构组件来识别冗余通道。

Result: 在多种模型和多样化下游任务上的实验表明，该方法实现了显著压缩，同时性能退化最小。

Conclusion: 所提出的结构化剪枝框架能有效解决大模型部署中的计算和内存问题，且保持与标准硬件加速器的兼容性。
...
### [108] [Extracting books from production language models](https://arxiv.org/abs/2601.02671)
*Ahmed Ahmed,A. Feder Cooper,Sanmi Koyejo,Percy Liang*

Main category: cs.CL

TL;DR: 研究发现，尽管生产级大语言模型（LLMs）采取了安全措施，但仍能通过特定方法提取其训练数据中的受版权保护文本，其中不同模型的可提取程度存在差异。


<details>
  <summary>Details</summary>
Motivation: 当前关于LLMs与版权的法律争议焦点在于模型是否记忆训练数据以及能否提取这些数据。尽管普遍认为LLMs不会大量记忆训练数据，但已有研究表明开源模型可提取大量受版权文本。然而，对于采取了安全措施的生产级LLMs是否也存在类似风险，仍是一个未解决的问题。

Method: 采用两阶段方法：第一阶段使用初始探测（有时结合Best-of-N越狱）测试提取可行性；第二阶段使用迭代延续提示尝试提取整本书。评估了Claude 3.7 Sonnet、GPT-4.1、Gemini 2.5 Pro和Grok 3四个生产级LLMs，并使用基于块的最长公共子串近似值（nv-recall）计算提取成功率。

Result: 不同模型的提取效果差异显著：Gemini 2.5 Pro和Grok 3无需越狱即可提取文本（例如《哈利·波特与魔法石》的nv-recall分别为76.8%和70.3%）；Claude 3.7 Sonnet和GPT-4.1需要越狱，其中越狱后的Claude 3.7 Sonnet可近乎逐字输出整本书（nv-recall=95.8%），而GPT-4.1需要更多越狱尝试且最终会拒绝继续（nv-recall=4.0%）。

Conclusion: 即使生产级LLMs采取了模型和系统级安全措施，提取其训练数据中的受版权内容仍然存在风险，这凸显了当前LLMs在数据记忆和版权保护方面的潜在漏洞。
...
### [109] [EvoRoute: Experience-Driven Self-Routing LLM Agent Systems](https://arxiv.org/abs/2601.02695)
*Guibin Zhang,Haiyang Yu,Kaiming Yang,Bingli Wu,Fei Huang,Yongbin Li,Shuicheng Yan*

Main category: cs.CL

TL;DR: 本文提出EvoRoute，一种自演化的模型路由范式，旨在解决智能体系统在性能、成本和延迟之间的三难困境，通过动态选择帕累托最优的LLM骨干网络，在保持性能的同时显著降低成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的复杂智能体系统在处理多轮复杂任务时表现出色，但面临高昂的经济成本和严重延迟问题，这暴露了性能、成本与速度之间未被充分探索的权衡困境。

Method: 提出EvoRoute自演化模型路由范式，利用不断扩展的先验经验知识库，在每一步动态选择帕累托最优的LLM骨干网络，平衡准确性、效率和资源使用，并通过环境反馈持续优化选择策略。

Result: 在GAIA和BrowseComp+等智能体基准测试中，EvoRoute集成到现有智能体系统后，在维持或提升系统性能的同时，将执行成本降低高达80%，延迟减少超过70%。

Conclusion: EvoRoute通过动态路由机制有效解决了智能体系统的三难困境，实现了性能、成本和延迟的优化平衡，为高效能智能体系统的实际部署提供了可行方案。
...
### [110] [Boosting Accuracy and Interpretability in Multilingual Hate Speech Detection Through Layer Freezing and Explainable AI](https://arxiv.org/abs/2601.02697)
*Meysam Shirdel Bilehsavar,Negin Mahmoudi,Mohammad Jalili Torkamani,Kiana Kiashemshaki*

Main category: cs.CL

TL;DR: 本研究评估了三种基于Transformer的模型（BERT、RoBERTa、XLM-RoBERTa）在五种语言（英、韩、日、中、法）上的情感分析和仇恨言论检测性能，并利用LIME框架增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 在线内容审核需要有效识别有害或冒犯性内容，情感分析和仇恨言论检测对此至关重要。现有研究需提升多语言场景下的模型性能与决策透明度。

Method: 使用BERT-base-multilingual-cased、RoBERTa-base和冻结前八层的XLM-RoBERTa-base模型，在五语言数据集上进行评估，采用准确率、精确率、召回率和F1值等指标，并集成LIME框架分析词级贡献。

Result: 通过对比实验量化了各模型在多语言任务中的性能差异，LIME可视化揭示了模型决策依赖的关键词汇特征。

Conclusion: 结合先进Transformer架构与可解释性技术，能同步提升多语言情感分析和仇恨言论检测系统的效能与透明度，为安全数字环境提供技术支持。
...
### [111] [Adversarial Question Answering Robustness: A Multi-Level Error Analysis and Mitigation Study](https://arxiv.org/abs/2601.02700)
*Agniv Roy Choudhury,Vignesh Ponselvan Rajasingh*

Main category: cs.CL

TL;DR: 该研究通过系统实验探索了Transformer模型在AddSent对抗数据集上的鲁棒性，发现模型规模扩大可消除鲁棒性与准确性的权衡，并提出实体感知对比学习方法显著缩小了对抗性能差距。


<details>
  <summary>Details</summary>
Motivation: 尽管问答系统在标准基准测试中表现优异，但对对抗样本仍然脆弱，因此需要研究其对抗鲁棒性并开发有效的缓解策略。

Method: 采用多级错误分析（五种分类方案）、系统评估对抗微调比例、数据增强实验、模型规模扩展（从ELECTRA-small到ELECTRA-base），并实施了三种针对性缓解策略，包括实体感知对比学习。

Result: 最佳对抗数据比例为20%；小模型存在容量瓶颈；扩大模型规模可同时提升干净数据和对抗数据性能；实体感知对比学习效果最佳，在AddSent上达到89.89% EM，在SQuAD上达到90.73% EM，对抗差距缩小了94.9%。

Conclusion: 针对性缓解策略（尤其是结合NER的对比学习）能够使对抗性能接近干净数据性能，首次将全面语言错误分析与NER引导的对比学习结合用于对抗问答研究。
...
### [112] [Mitigating Prompt-Induced Hallucinations in Large Language Models via Structured Reasoning](https://arxiv.org/abs/2601.02739)
*Jinbo Hao,Kai Yang,Qingzhen Su,Yang Chen,Yifan Li,Chao Jiang*

Main category: cs.CL

TL;DR: 提出一种通过代码模块增强知识蒸馏链式模型的方法，以缓解大语言模型中的提示诱导幻觉问题，显著提升推理准确性和可验证性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉问题，尤其是在提示诱导下容易生成不准确信息，需要引入外部结构化知识来约束推理过程。

Method: 基于知识蒸馏链式模型，引入代码模块引导知识图谱探索，将代码作为思维链提示的一部分，形成外部知识输入，并改进模型以分析和约束LLM的推理过程。

Result: 在GPT-4和LLaMA-3.3上的实验表明，代码模块显著提升模型对上下文信息的捕捉能力，HIT@1/3/5分别提升15.64%、13.38%、13.28%，多项评估中HIT@1/3/5超过95%。

Conclusion: 该方法能有效减少幻觉行为，提高大语言模型的准确性和可验证性，为缓解LLM幻觉问题提供了可行方案。
...
### [113] [Language Hierarchization Provides the Optimal Solution to Human Working Memory Limits](https://arxiv.org/abs/2601.02740)
*Luyao Chen,Weibo Gao,Junjie Wu,Jinshan Wu,Angela D. Friederici*

Main category: cs.CL

TL;DR: 该研究通过计算模拟和自然语言验证，发现层级处理能最优地解决工作记忆容量限制，解释了人类语言普遍具有层级结构的原因。


<details>
  <summary>Details</summary>
Motivation: 探索人类语言为何普遍具有层级结构的根本原因，特别是从认知限制（如工作记忆容量）的角度理解这一语言特性。

Method: 建立似然函数量化语言处理机制与工作记忆容量的匹配程度，通过计算模拟符号序列和自然语言句子的验证分析，比较线性处理与层级处理的性能。

Result: 层级处理在约束平均单元数（tehta_MLE）低于人类工作记忆容量限制方面远优于线性处理，且随序列/句子长度增加表现更佳；结果模式与儿童工作记忆发展相吻合。

Conclusion: 构建层级结构能优化序列语言输入的处理效率，同时满足记忆约束，这为人类语言的普遍层级性提供了认知机制上的解释。
...
### [114] [SYNAPSE: Empowering LLM Agents with Episodic-Semantic Memory via Spreading Activation](https://arxiv.org/abs/2601.02744)
*Hanqi Jiang,Junhao Chen,Yi Pan,Ling Chen,Weihang You,Yifan Zhou,Ruidong Zhang,Yohannes Abate,Tianming Liu*

Main category: cs.CL

TL;DR: 提出了Synapse记忆架构，通过动态图模型和激活传播机制，解决大语言模型在长期记忆中的上下文隧道问题，在复杂推理任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法无法处理长期记忆的割裂性问题，需要一种超越静态向量相似度的统一记忆架构。

Method: 基于认知科学构建动态记忆图，采用侧向抑制和时间衰减机制，实现三重混合检索策略，融合几何嵌入与基于激活的图遍历。

Result: 在LoCoMo基准测试中，Synapse在复杂时序和多跳推理任务上显著优于现有最先进方法。

Conclusion: Synapse为解决上下文隧道问题提供了稳健方案，代码和数据将在论文接受后公开。
...
### [115] [Window-based Membership Inference Attacks Against Fine-tuned Large Language Models](https://arxiv.org/abs/2601.02751)
*Yuetian Chen,Yuntao Du,Kaiyuan Zhang,Ashish Kundu,Charles Fleming,Bruno Ribeiro,Ninghui Li*

Main category: cs.CL

TL;DR: 提出WBC方法，通过滑动窗口和基于符号的聚合来增强成员推理攻击，利用局部上下文中的记忆信号，显著提升对微调大语言模型的隐私攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有针对大语言模型的成员推理攻击主要依赖全局信号（如平均损失），这会稀释记忆的局部细微信号，降低攻击效果。研究旨在挑战这种全局平均范式，认为成员信号在局部上下文中更为显著。

Method: 提出WBC（基于窗口的比较）方法，采用滑动窗口策略和基于符号的聚合。通过不同大小的窗口在文本序列上滑动，每个窗口基于目标模型与参考模型的损失比较进行二值投票，并通过几何间隔的窗口大小集成投票，捕捉从词元级到短语级的记忆模式。

Result: 在11个数据集上的实验表明，WBC显著优于现有基线方法，AUC分数更高，在低误报率阈值下检测率提升2-3倍。
...
### [116] [EComStage: Stage-wise and Orientation-specific Benchmarking for Large Language Models in E-commerce](https://arxiv.org/abs/2601.02752)
*Kaiyan Zhao,Zijie Meng,Zheyong Xie,Jin Duan,Yao Hu,Zuozhu Liu,Shaosheng Cao*

Main category: cs.CL

TL;DR: 提出了EComStage基准，用于评估LLM智能体在电子商务场景中感知、规划、行动三阶段的推理能力，覆盖客户和商家双重视角，测试了30多个模型并揭示了细粒度性能差异。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估智能体是否完成最终任务，忽略了中间推理阶段对决策质量的关键影响，且缺乏对商家侧场景的覆盖，难以全面评估LLM在真实电商环境中的适用性。

Method: 构建EComStage基准，包含7个代表性电商任务，所有样本经人工标注和质量检查；将推理过程划分为感知、规划、行动三阶段；评估超过30个参数量从1B到200B+的开源与闭源LLM。

Result: 评估揭示了LLM在不同推理阶段和任务导向（客户/商家）上的特异性优劣，例如某些模型在感知阶段表现优异但在行动阶段不足，为模型优化提供了细粒度洞察。

Conclusion: EComStage提供了更全面的LLM智能体评估框架，其阶段化评估方法能为实际电商场景中LLM智能体的设计与优化提供可操作的指导。
...
### [117] [MiMo-V2-Flash Technical Report](https://arxiv.org/abs/2601.02780)
...
TL;DR: 提出MiMo-V2-Flash，一个总参数量309B、激活参数量15B的混合专家模型，采用混合注意力架构和多教师策略蒸馏，在推理性能上媲美顶级开源模型且解码速度更快。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型参数量庞大但推理效率低，需要一种既能保持强大推理和智能体能力，又能高效部署的模型架构。

Method: 1. 混合注意力架构（滑动窗口注意力与全局注意力5:1混合）；2. 多令牌预测预训练；3. 多教师策略蒸馏框架；4. 将多令牌预测重用作推测解码的草稿模型。

Result: 1. 参数量仅为DeepSeek-V3.2和Kimi-K2的1/2和1/3，但性能相当；2. 推测解码实现3.6接受长度和2.6倍解码加速；3. 支持256k上下文长度。

Conclusion: MiMo-V2-Flash通过创新的架构和训练范式，实现了高性能与高效率的平衡，为开源社区提供了强大的推理模型基础。
...
### [118] [Punctuation-aware Hybrid Trainable Sparse Attention for Large Language Models](https://arxiv.org/abs/2601.02819)
*Junxiang Qiu,Shuo Wang,Zhengsu Chen,Hengheng Zhang,Jinda Lu,Changcheng Li,Qi Tian*

Main category: cs.CL

TL;DR: 提出PHSA框架，利用标点符号作为语义边界锚点，改进稀疏注意力机制，在保持高性能的同时显著降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 密集注意力机制因二次复杂度难以处理长序列，现有稀疏注意力方法在块选择时依赖粗粒度语义表示，导致语义边界模糊和关键信息丢失。

Method: 设计基于标点符号的混合稀疏注意力框架：1) 双分支聚合机制融合全局语义和标点增强的边界特征；2) 极稀疏自适应训练推理策略稳定低激活率下的模型行为。

Result: 在通用基准和长上下文评估中，PHSA均优于密集注意力和现有稀疏注意力方法。0.6B参数模型处理32k序列时，在97.3%稀疏率下信息损失减少10.8%。

Conclusion: PHSA通过标点感知的语义边界建模有效解决了稀疏注意力中的信息丢失问题，为长序列建模提供了高效可扩展的解决方案。
...
### [119] [TWIST: Training-free and Label-free Short Text Clustering through Iterative Vector Updating with LLMs](https://arxiv.org/abs/2510.06747)
*I-Fan Lin,Faegheh Hasibi,Suzan Verberne*

Main category: cs.CL

TL;DR: 提出了一种无需训练和标签的短文本聚类方法，适用于任何嵌入器，通过迭代向量更新和LLM指导实现，在多种数据集上达到或超过现有方法性能。


<details>
  <summary>Details</summary>
Motivation: 商业聊天机器人场景中，用户话语量大且无标签数据，聚类数量未知，现有方法依赖标注数据或已知聚类数，难以适应实际需求。

Method: 基于迭代向量更新：先构建稀疏向量表示代表性文本，再通过LLM指导迭代优化向量，无需训练或标签，可适配不同嵌入器和聚类算法。

Result: 在多样化数据集上，本方法性能与基于对比学习的先进方法相当或更优，且能扩展到大规模数据集，降低LLM计算成本，适用于小规模LLM。

Conclusion: 该方法资源需求低、适应性强、可扩展性好，比现有聚类方法更符合实际应用场景，为无监督短文本聚类提供了实用解决方案。
...
### [120] [The performances of the Chinese and U.S. Large Language Models on the Topic of Chinese Culture](https://arxiv.org/abs/2601.02830)
*Feiyan Liu,Chenxun Zhuo,Siyan Zhao,Bao Ge,Tianming Liu*

Main category: cs.CL

TL;DR: 研究比较中美开发的大语言模型在中文文化问题上的表现，发现中国模型整体优于美国模型，性能差异可能源于训练数据、本地化策略和对中文文化的重视程度不同。


<details>
  <summary>Details</summary>
Motivation: 文化背景影响问题解决方式，而中美是LLM开发的主要国家，因此探究两国开发的LLM在中文文化理解上是否存在差异。

Method: 采用直接提问范式，评估GPT-5.1、DeepSeek-V3.2、Qwen3-Max、Gemini2.5Pro等模型对中国传统文化（历史、文学、诗歌等）的理解能力。

Result: 中国开发的模型在这些任务上普遍优于美国模型；美国模型中Gemini 2.5Pro和GPT-5.1准确率相对较高。

Conclusion: LLM在中文文化任务上的性能差异可能源于训练数据分布、本地化策略及开发过程中对中文文化内容的重视程度不同。
...
### [121] [TiMem: Temporal-Hierarchical Memory Consolidation for Long-Horizon Conversational Agents](https://arxiv.org/abs/2601.02845)
*Kai Li,Xuanqing Yu,Ziyi Ni,Yi Zeng,Yao Xu,Zheqing Zhang,Xin Li,Jitao Sang,Xiaogang Duan,Xuelei Wang,Chengbao Liu,Jie Tan*

Main category: cs.CL

TL;DR: TiMem是一个时间-层次记忆框架，通过时间记忆树组织对话，实现从原始观察到抽象人物表征的系统记忆整合，在长对话任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有记忆框架对跨层次的时间结构化信息支持有限，导致记忆碎片化和长对话个性化不稳定，需要解决LLM有限上下文窗口与不断增长的对话历史之间的矛盾。

Method: 提出时间记忆树（TMT）实现时间-层次组织；采用语义引导的记忆整合方法，无需微调即可跨层次集成记忆；设计复杂度感知的记忆召回机制，平衡查询精度与效率。

Result: 在LoCoMo和LongMemEval-S基准测试中分别达到75.30%和76.88%的准确率，优于所有基线方法，在LoCoMo上召回记忆长度减少52.20%，人物表征分离度更清晰。

Conclusion: TiMem将时间连续性作为对话代理长时记忆的首要组织原则，通过层次化记忆整合有效解决了长对话中的记忆管理问题。
...
TL;DR: 对多个广泛使用的视觉-语言-动作（VLA）数据集进行系统性语言审计，发现其指令语言重复性高、结构变化有限，缺乏语言多样性。


<details>
  <summary>Details</summary>
Motivation: VLA模型中语言扮演关键角色，但训练和评估这些系统所用数据集的语言特性缺乏充分记录，需要系统分析其指令语言的真实构成和多样性。

Method: 从词汇多样性、重复与重叠、语义相似性、句法复杂性等多个互补维度，对多个常用VLA数据集进行量化分析。

Result: 分析显示许多数据集依赖高度重复、模板化的指令，结构变化有限，导致指令形式分布狭窄。

Conclusion: 本研究旨在描述当前VLA训练和评估数据中的语言信号，以支持更详细的数据集报告、更合理的数据集选择，以及扩大语言覆盖的针对性数据优化策略。
...
### [123] [To Generate or Discriminate? Methodological Considerations for Measuring Cultural Alignment in LLMs](https://arxiv.org/abs/2601.02858)
*Saurabh Kumar Pandey,Sougata Saha,Monojit Choudhury*

Main category: cs.CL

TL;DR: 本文提出逆社会人口统计学提示（ISDP）方法，通过让大语言模型从用户行为中预测人口统计学特征，评估其文化对齐能力，发现模型在真实行为上表现优于模拟行为，但个体层面个性化存在局限。


<details>
  <summary>Details</summary>
Motivation: 传统的社会人口统计学提示（SDP）方法在评估大语言模型文化能力时存在混杂因素（如提示敏感性、解码参数等），导致难以区分模型表现差是源于偏见还是任务设计问题，需要更可靠的评估方法。

Method: 使用逆社会人口统计学提示（ISDP），让模型从Goodreads-CSI数据集中印度、墨西哥和美国用户的真实与模拟书评行为中预测其人口统计学特征，测试了Aya-23、Gemma-2、GPT-4o和LLaMA-3.1四个模型。

Result: 模型在真实用户行为上的预测表现优于模拟行为，这与SDP的结论相反；但在个体层面，两种行为类型的预测性能均下降且趋于接近，表明个性化能力存在上限。

Conclusion: ISDP能更可靠地评估大语言模型的文化对齐能力，揭示出现有模型在个体层面个性化方面的局限性，为改进模型的文化适应性提供了新视角。
...
### [124] [Training Language Models with homotokens Leads to Delayed Overfitting](https://arxiv.org/abs/2601.02867)
*Adrian Cosma,Stefan Ruseti,Emilian Radoi,Mihai Dascalu*

Main category: cs.CL

TL;DR: 本文提出同义分词（homotokens）作为数据增强方法，通过训练架构使语言模型对同一词汇的不同分词方式具有不变性，在数据受限的预训练和多语言微调中提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型训练通常采用单一最长前缀分词方式，但同一词汇的不同子词分词序列可能对应相同的语义却引发不同的内部计算，这种非唯一性未被充分利用。

Method: 提出同义分词作为严格保持语义的数据增强形式；设计轻量级训练架构，通过辅助因果编码器和块因果交叉注意力，在保持训练目标和分词接口不变的情况下，基于采样的同义分词变体进行条件化下一个词预测。

Result: 在数据受限的预训练中，同义分词增强能持续延缓重复数据暴露下的过拟合并提升跨数据集的泛化能力；在多语言微调中，其效果取决于分词器质量——当规范分词高度压缩时增益最强，而当分词器已过度分割输入时增益减弱。

Conclusion: 同义分词为语言模型提供了一种简单模块化的机制，可诱导分词不变性，其有效性受分词器特性的影响。
...
### [125] [LongBench Pro: A More Realistic and Comprehensive Bilingual Long-Context Evaluation Benchmark](https://arxiv.org/abs/2601.02872)
*Ziyang Chen,Xing Wu,Junlong Jia,Chaochen Gao,Qi Fu,Debing Zhang,Songlin Hu*

Main category: cs.CL

TL;DR: 提出了LongBench Pro，一个包含1500个中英文长文本样本的双语基准测试，用于评估大语言模型的长上下文理解能力，覆盖8k到256k的输入长度，并通过人工-模型协作构建流程平衡质量与可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文基准测试在可扩展性和真实性之间存在矛盾：合成任务无法反映真实复杂性，而完全人工标注难以扩展到极长文本和多样场景。

Method: 采用人工-模型协作构建流程：前沿大语言模型生成具有挑战性的问题、参考答案及设计原理，专家进行验证和修正；构建包含11个主要任务和25个次要任务的双语基准测试，支持基于上下文需求、长度和难度的多维度分析。

Result: 评估46个常用长上下文大语言模型发现：(1) 长上下文优化比参数缩放对理解能力贡献更大；(2) 有效上下文长度通常短于宣称长度，且存在显著的跨语言不对齐；(3) '思考'范式主要帮助原生推理训练的模型，而混合思考设计提供了有前景的帕累托权衡。

Conclusion: LongBench Pro为推进长上下文理解研究提供了可靠的测试平台，其构建方法平衡了质量与可扩展性，揭示了当前模型在长上下文处理中的关键局限性和改进方向。
...
### [126] [RAL2M: Retrieval Augmented Learning-To-Match Against Hallucination in Compliance-Guaranteed Service Systems](https://arxiv.org/abs/2601.02917)
*Mengze Hong,Di Jiang,Jiangtao Wen,Zhiyang Su,Yawen Li,Yanjie Sun,Guan Wang,Chen Jason Zhang*

Main category: cs.CL

TL;DR: 提出RAL2M框架，通过将LLM重新定位为检索系统中的查询-响应匹配判断器来消除生成幻觉，并采用查询自适应潜在集成策略缓解判断幻觉。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的服务系统中存在严重的幻觉问题，需要显式知识基础来保证合规响应。

Method: 检索增强学习匹配框架，将LLM作为检索系统中的匹配判断器；提出查询自适应潜在集成策略，显式建模异构模型能力和LLM间的相互依赖。

Result: 在大规模基准测试中，该方法有效利用“群体智慧”，显著优于强基线方法。

Conclusion: 该方法为纯生成方法提供了稳健替代方案，并讨论了未来工作中进一步利用潜在表示的最佳实践和方向。
...
### [127] [Transparent Semantic Change Detection with Dependency-Based Profiles](https://arxiv.org/abs/2601.02891)
*Bach Phan-Tat,Kris Heylen,Dirk Geeraerts,Stefano De Pascale,Dirk Speelman*

Main category: cs.CL

TL;DR: 本文提出一种基于依存共现模式的语义变化检测方法，相比主流神经网络方法更可解释且性能更优。


<details>
  <summary>Details</summary>
Motivation: 当前基于词嵌入的语义变化检测方法虽性能强但缺乏可解释性，需要更透明的方法。

Method: 使用依存句法分析提取词语的依存共现模式，基于模式变化检测语义演变。

Result: 该方法在语义变化检测任务上有效，性能优于多种分布语义模型，预测结果合理且可解释。

Conclusion: 依存共现模式可作为语义变化检测的可靠替代方案，兼具性能与可解释性优势。
...
### [128] [Revisiting Data Compression with Language Modeling](https://arxiv.org/abs/2601.02875)
*Chen-Han Tsai*

Main category: cs.CL

TL;DR: 本文探索使用大型语言模型进行数据压缩，在enwik9数据集上实现了18%的调整压缩率新SOTA，并测试了其在非英语文本、代码和字节流上的压缩能力。


<details>
  <summary>Details</summary>
Motivation: 尽管先前研究显示LLM在多模态数据压缩中表现良好，但实际应用中仍存在挑战，阻碍其替代传统压缩算法。本研究旨在探索降低调整压缩率的方法，并扩展LLM在非自然文本序列压缩中的应用。

Method: 使用LLM作为数据压缩器，探索不同配置方法以优化压缩性能，无需额外模型训练。

Result: 在enwik9数据集上实现了约18%的调整压缩率，达到新的SOTA；同时证明LLM在文本主导领域表现优异，在非自然文本序列（如代码、字节流）中通过适当配置仍具竞争力。

Conclusion: LLM在数据压缩任务中潜力显著，尤其在文本领域表现突出，通过合理配置也能有效压缩非自然文本序列，为替代传统压缩算法提供了可行路径。
...
### [129] [Linear Script Representations in Speech Foundation Models Enable Zero-Shot Transliteration](https://arxiv.org/abs/2601.02906)
...
TL;DR: 该研究发现多语言语音模型中脚本信息呈线性编码，通过修改推理时的激活向量可直接控制语音识别输出的文字脚本。


<details>
  <summary>Details</summary>
Motivation: 多语言语音模型（如Whisper）训练数据包含同一语言的多种区域变体，这些变体常使用不同文字脚本书写，导致语音识别输出脚本存在不确定性。

Method: 分析模型激活空间中脚本的线性编码特性，在推理时向激活向量添加特定脚本向量以控制输出脚本，并测试非常规语言-脚本组合（如意大利语用西里尔字母）。

Result: 该方法能有效诱导脚本转换，在Whisper所有模型尺寸上均表现出竞争力，即使对训练中未见的语言-脚本组合也有效。

Conclusion: 通过修改激活向量可实现语音识别输出脚本的后验控制，为解决多语言语音模型中脚本不确定性问题提供了轻量级解决方案。
...
### [130] [Correct, Concise and Complete: Multi-stage Training For Adaptive Reasoning](https://arxiv.org/abs/2601.02972)
*Nathanaël Carraz Rakotonirina,Ren Pang,Neha Anna John,Michael Bohlke-Schneider,Momchil Hardalov*

Main category: cs.CL

TL;DR: 提出一种多阶段高效推理方法，通过监督微调和强化学习结合自适应长度惩罚，减少大语言模型推理时的冗余思考，在保持精度的同时显著缩短响应长度。


<details>
  <summary>Details</summary>
Motivation: 现有链式思考（CoT）方法常产生不必要的长推理过程，增加计算成本且可能因'过度思考'导致性能下降，需平衡推理精度与效率。

Method: 结合监督微调（拒绝采样或推理轨迹重构）与强化学习，采用轻量级奖励函数惩罚首个正确答案后的冗余令牌，仅在有益时鼓励自我验证。

Result: 在7个推理任务上，8B和32B模型的响应长度分别平均减少28%和40%，精度仅下降1.6和2.5个百分点；在过思考调整精度曲线下面积指标上优于现有方法。

Conclusion: 该方法以简洁设计实现了推理精度与效率的优越权衡，为减少大语言模型计算开销提供了有效方案。
...
### [131] [Mechanistic Knobs in LLMs: Retrieving and Steering High-Order Semantic Features via Sparse Autoencoders](https://arxiv.org/abs/2601.02978)
*Ruikang Zhang,Shuo Wang,Qi Su*

Main category: cs.CL

TL;DR: 提出基于稀疏自编码器的框架，用于检索和操控与大语言模型中高级语言行为相关的可解释内部特征，以五大人格特质为例验证了方法的精确双向行为调控能力。


<details>
  <summary>Details</summary>
Motivation: 现有机制可解释性方法难以将内部特征与语言生成中复杂行为级语义属性的可靠控制相连接，需要建立更稳健的语义特征操控机制。

Method: 采用基于对比语义对立的对比特征检索流程，结合统计激活分析和基于生成的验证，从稀疏激活空间中提取单语义功能特征。

Result: 方法在五大人格特质案例中实现了精确双向的行为调控，相比对比激活加法等方法具有更优的稳定性和性能，并发现'功能忠实性'效应。

Conclusion: 大语言模型内化了高阶概念的深度集成表示，该框架为复杂AI行为调控提供了新颖且稳健的机制路径。
...
### [132] [Image, Word and Thought: A More Challenging Language Task for the Iterated Learning Model](https://arxiv.org/abs/2601.02911)
*Hyoyeon Lee,Seth Bullock,Conor Houghton*

Main category: cs.CL

TL;DR: 本文应用半监督迭代学习模型，成功模拟了复杂七段数码管图像的语言传输，证明了语言传输瓶颈能促使语言产生表达性、组合性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 探索语言代际传输中的约束如何促进语言结构的涌现，特别是针对更复杂意义（七段数码管图像）的通信任务。

Method: 采用半监督迭代学习模型，结合监督学习和无监督学习，基于自编码器架构，模拟多代语言学习与传输过程。

Result: 智能体成功学习并传输了一种语言：能表达所有128种字形（表达性），信号组件与意义组件一致映射（组合性），且代际间语言保持稳定（稳定性）。

Conclusion: 即使学习者从零开始，语言传输中的信息瓶颈能驱动复杂意义通信中结构化语言的涌现与稳定传承。
...
### [133] [Beyond the Black Box: Theory and Mechanism of Large Language Models](https://arxiv.org/abs/2601.02907)
...
TL;DR: 本文提出了一种基于生命周期的统一分类法，将大语言模型（LLM）的研究领域划分为六个阶段，系统回顾了驱动LLM性能的基础理论和内部机制，旨在推动LLM开发从工程启发式向原则性科学学科转变。


<details>
  <summary>Details</summary>
Motivation: 当前LLM领域存在一个关键悖论：尽管实证效果显著，但理论理解仍相对滞后，导致这些系统大多被视为“黑箱”。为应对这种理论碎片化，本文旨在通过统一框架整合研究现状。

Method: 提出基于生命周期的分类法（数据准备、模型准备、训练、对齐、推理、评估），系统分析各阶段的核心理论问题，如数据混合的数学依据、架构表示极限、对齐算法优化动态等。

Result: 构建了结构化研究路线图，识别出关键前沿挑战，包括合成数据自我改进的理论极限、安全保证的数学边界、以及涌现智能的机制起源。

Conclusion: 通过连接实证观察与严谨科学探究，本文为LLM开发从工程启发式转向原则性科学学科提供了系统化路径，强调理论理解对释放LLM全部潜力的必要性。
...
### [134] [SentGraph: Hierarchical Sentence Graph for Multi-hop Retrieval-Augmented Question Answering](https://arxiv.org/abs/2601.03014)
*Junli Liang,Pengfei Zhou,Wangqiu Zhou,Wenjie Qing,Qi Zhao,Ziwen Wang,Qi Song,Xiangyang Li*

Main category: cs.CL

TL;DR: 提出SentGraph框架，通过构建句子级图结构来改进多跳问答中的检索增强生成，显式建模句子间逻辑关系，提升证据链的连贯性和推理准确性。


<details>
  <summary>Details</summary>
Motivation: 传统基于分块的检索增强生成方法在多跳问答任务中表现不佳，常因检索到无关或逻辑不连贯的上下文，导致证据链不完整和推理错误。

Method: 构建分层句子图：离线阶段采用修辞结构理论区分核心句与辅助句，组织成语题级子图并通过跨文档实体桥连接；在线阶段执行图引导的证据选择和路径扩展，检索细粒度句子级证据。

Result: 在四个多跳问答基准测试上的广泛实验验证了SentGraph的有效性，证明了显式建模句子级逻辑依赖对多跳推理的重要性。

Conclusion: SentGraph通过句子级图结构显式建模逻辑关系，显著提升了多跳问答中检索证据的连贯性和推理质量，为复杂推理任务提供了有效解决方案。
...
### [135] [Memorization, Emergence, and Explaining Reversal Failures: A Controlled Study of Relational Semantics in LLMs](https://arxiv.org/abs/2601.02931)
*Yihua Zhu,Qianying Liu,Jiaxin Wang,Fei Cheng,Chaoran Liu,Akiko Aizawa,Sadao Kurohashi,Hidetoshi Shimodaira*

Main category: cs.CL

TL;DR: 本文通过受控知识图谱合成框架研究自回归LLMs是否学习关系的逻辑语义（如对称性和逆关系），发现足够逻辑监督下关系语义会涌现，且反转失败主要由自回归顺序偏差而非语义缺陷导致。


<details>
  <summary>Details</summary>
Motivation: 自回归LLMs在关系任务上表现良好，但尚不清楚它们是否真正学习关系的逻辑语义（如对称性、逆关系逻辑），以及反转类失败源于缺失关系语义还是自左向右的顺序偏差。

Method: 提出基于知识图谱的受控合成框架，生成对称/逆三元组文本，从头训练GPT风格自回归模型，评估记忆、逻辑推理和上下文泛化到未见实体的能力。

Result: 发现关系语义在足够逻辑监督下出现明显相变，即使浅层（2-3层）模型也能涌现；成功泛化与稳定的中间层信号相关；顺序匹配的前向/反向测试和扩散基线表明反转失败主要源于自回归顺序偏差。

Conclusion: 自回归LLMs在适当监督下能学习关系逻辑语义，但反转失败主要由顺序偏差而非语义理解缺陷导致，这为模型架构设计提供了重要启示。
...
### [136] [Lil: Less is Less When Applying Post-Training Sparse-Attention Algorithms in Long-Decode Stage](https://arxiv.org/abs/2601.03043)
...
TL;DR: 研究发现稀疏注意力算法在解码阶段可能因信息丢失导致序列变长，提出早期停止算法解决此问题，显著减少令牌消耗。


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理效率需求高，解码阶段占主导延迟，现有稀疏注意力方法可能因信息丢失反而增加端到端复杂度。

Method: 通过实证和理论分析稀疏注意力的“少即是少”现象，提出检测信息损失阈值的早期停止算法。

Result: 早期停止算法在推理密集型基准测试中减少高达90%的令牌消耗，准确率下降小于2%。

Conclusion: 稀疏注意力可能因信息丢失导致序列延长，早期停止算法能有效平衡效率与准确性，提升推理效率。
...
### [137] [Pearmut: Human Evaluation of Translation Made Trivial](https://arxiv.org/abs/2601.02933)
*Vilém Zouhar,Tom Kocmi*

Main category: cs.CL

TL;DR: Pearmut是一个轻量级但功能丰富的人类评估平台，旨在简化多语言NLP任务（特别是机器翻译）的端到端人工评估流程，使其像自动评估一样易于运行。


<details>
  <summary>Details</summary>
Motivation: 当前多语言NLP领域的人类评估存在设置复杂、耗时且工程开销大的问题，导致实践中常被自动指标替代，但自动指标无法完全替代人工评估的质量判断。

Method: 开发了Pearmut平台，支持标准评估协议（如DA、ESA、MQM），提供文档级上下文、绝对与对比评估、注意力检查、ESAAI预标注、静态和主动学习分配策略等功能，并允许扩展新协议。

Result: Pearmut平台通过降低技术门槛和操作负担，使可靠的人类评估能够成为模型开发和诊断的常规实践，而非偶尔进行的昂贵任务。

Conclusion: 该平台通过工程化解决方案，使高质量人工评估在多语言NLP中变得实用和可常规化，有助于提升模型评估的可靠性。
...
### [138] [Dementia-R1: Reinforced Pretraining and Reasoning from Unstructured Clinical Notes for Real-World Dementia Prognosis](https://arxiv.org/abs/2601.03018)
*Choonghan Kim,Hyunmin Hwang,Hangeol Chang,Jaemin Kim,Jinse Park,Jae-Sung Lim,Jong Chul Ye*

Main category: cs.CL

TL;DR: 提出Dementia-R1框架，通过冷启动强化学习方法从非结构化临床笔记进行纵向痴呆预后预测，解决传统方法在复杂症状轨迹推理上的不足。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床文本理解上表现良好，但在需要跨多次就诊推理复杂非单调症状轨迹的纵向预测任务（如痴呆预后）中表现不佳。标准监督训练缺乏症状演变的显式标注，而直接强化学习受稀疏二元奖励限制。

Method: 采用冷启动强化学习策略，预训练模型预测从患者历史中提取的可验证临床指标，增强疾病进展推理能力，再确定最终临床状态。

Result: 在真实世界非结构化临床数据集上达到77.03%的F1分数；在ADNI基准测试中，7B模型性能与GPT-4o相当，能有效捕捉波动的认知轨迹。

Conclusion: Dementia-R1框架通过结合临床指标预测和强化学习，显著提升了纵向痴呆预后预测的准确性，为临床决策提供了有效工具。
...
### [139] [Enhancing Multilingual RAG Systems with Debiased Language Preference-Guided Query Fusion](https://arxiv.org/abs/2601.02956)
*Jeonghyun Park,Byeongjeong Kim,Seojin Hwang,Hwanhee Lee*

Main category: cs.CL

TL;DR: 本文揭示了多语言检索增强生成系统中英语偏好评估的偏差，并提出去偏度量DeLP和优化框架DELTA，证明单语对齐比英语枢纽策略更有效。


<details>
  <summary>Details</summary>
Motivation: 现有研究多将mRAG系统的英语偏好归因于大语言模型的英语中心能力，但作者认为这种评估受到评测基准中结构性先验的严重干扰，需要更准确的评估方法和优化框架。

Method: 提出DeLP度量来消除暴露偏差、黄金答案可用性先验和文化先验等结构性混杂因素；基于分析结果设计DELTA框架，利用单语对齐优化跨语言检索与生成。

Result: DeLP分析表明英语偏好主要源于证据分布不均而非模型固有偏差；检索器本质上偏好查询与文档语言的单语对齐；DELTA在多种语言上持续优于英语枢纽和现有mRAG基线。

Conclusion: 英语偏好主要是评估偏差的产物，单语对齐是跨语言检索的关键机制；DELTA为轻量高效的mRAG系统提供了新方向。
...
### [140] [Low-Resource Heuristics for Bahnaric Optical Character Recognition Improvement](https://arxiv.org/abs/2601.02965)
*Phat Tran,Phuoc Pham,Hung Trinh,Tho Quan*

Main category: cs.CL

TL;DR: 本研究针对巴拿语文档数字化中的OCR识别难题，提出结合表格检测与概率后处理的综合方法，将识别准确率从72.86%提升至79.26%，为少数民族语言保护提供技术框架。


<details>
  <summary>Details</summary>
Motivation: 巴拿语作为越南、柬埔寨和老挝的少数民族语言，面临研究资料匮乏和保存危机；扫描文档因图像质量退化导致OCR识别错误率高，严重影响数字化保存和信息检索。

Method: 采用先进的表格与非表格区域检测技术优化输入数据质量，结合基于概率的后处理启发式算法对OCR输出进行纠错。

Result: 实验表明该方法显著提升识别准确率，从基线72.86%提高至79.26%。

Conclusion: 该研究为巴拿语数字化保存提供了有效解决方案，其技术框架可扩展至其他少数民族语言的文档保护工作。
...
### [141] [LLM-Augmented Changepoint Detection: A Framework for Ensemble Detection and Automated Explanation](https://arxiv.org/abs/2601.02957)
...
<details>
  <summary>Details</summary>
Motivation: 现有变点检测方法存在两个关键局限：1）单一方法在不同数据特征下表现互补，方法选择困难且易导致次优结果；2）缺乏对检测到的变化提供自动化、上下文相关的解释。

Method: 1）集成方法：聚合10种不同变点检测算法的结果；2）LLM解释管道：自动生成上下文叙事，将检测到的变点与现实历史事件关联；3）针对私有数据：采用检索增强生成（RAG）方案，基于用户提供的文档生成解释。

Result: 集成方法相比单一方法表现出更优的性能和鲁棒性；LLM解释管道能够将统计输出转化为可操作的见解；开源Python框架在金融、政治科学和环境科学等多个领域展示了实用价值。

Conclusion: 该框架通过集成统计方法与LLM，不仅提高了变点检测的准确性和鲁棒性，还增强了结果的可解释性，为分析师和决策者提供了从原始统计输出到可操作见解的完整解决方案。
...
### [142] [P-Check: Advancing Personalized Reward Model via Learning to Generate Dynamic Checklist](https://arxiv.org/abs/2601.02986)
*Kwangwook Seo,Dongha Lee*

Main category: cs.CL

TL;DR: 提出P-Check框架，通过动态生成个性化检查表来改进奖励建模，提升奖励预测准确性和下游生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有个性化奖励建模方法将用户上下文视为静态或隐式条件，无法捕捉人类判断的动态性和多面性。

Method: 提出P-Check框架，包含可插拔的检查表生成器和偏好对比准则加权训练策略，根据判别力为准则分配显著性分数。

Result: 实验表明P-Check不仅提高了奖励准确性，还增强了下游个性化生成性能，并在分布外场景中保持鲁棒性。

Conclusion: P-Check通过动态个性化检查表有效捕捉用户判断的细微差别，为个性化奖励建模提供了新方向。
...
### [143] [Reliability-Aware Adaptive Self-Consistency for Efficient Sampling in LLM Reasoning](https://arxiv.org/abs/2601.02970)
*Junseok Kim,Nakyeong Yang,Kyungmin Min,Kyomin Jung*

Main category: cs.CL

TL;DR: 提出ReASC方法，通过响应置信度引导的自适应采样改进自一致性推理效率，在保持准确率的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统自一致性方法依赖多样本聚合提升推理可靠性，但计算成本高昂；现有自适应方法使用基于计数的停止规则，对所有响应同等对待，常导致不必要的采样。

Method: 提出可靠性感知自适应自一致性（ReASC）：1）单样本决策阶段处理可单次响应确信回答的问题；2）可靠性感知累积阶段联合利用响应频率和置信度进行信息聚合。

Result: 在5个模型和4个数据集上的实验表明，ReASC在准确率-成本权衡上始终优于基线方法，在3B到27B参数规模的模型上均提升推理效率。例如在GSM8K上使用Gemma-3-4B-it模型时，相比自一致性方法减少70%推理成本且保持准确率。

Conclusion: ReASC通过将自适应采样从响应计数重构为证据充分性评估，利用响应级置信度实现原则性信息聚合，显著提升了推理效率与成本效益。
...
### [144] [Do LLMs Encode Functional Importance of Reasoning Tokens?](https://arxiv.org/abs/2601.03066)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型通过长推理链解决复杂任务，但计算成本高且难以识别功能相关的推理部分。现有压缩方法未能深入探究模型内部是否编码了标记级别的功能重要性。

Method: 提出贪婪剪枝方法：基于模型似然目标，迭代删除对似然影响最小的推理标记，生成长度可控的推理链。在蒸馏框架中评估剪枝效果，并与前沿模型监督的压缩基线对比。

Result: 在匹配推理长度下，使用剪枝链训练的学生模型优于前沿模型监督的压缩基线。分析显示系统性的剪枝模式，且注意力分数能预测贪婪剪枝排序，表明模型编码了推理标记的功能重要性结构。

Conclusion: 贪婪剪枝能有效压缩推理链，模型内部确实编码了推理标记的功能重要性，注意力机制可部分解释这种重要性，为高效推理提供了新视角。
...
### [145] [Grad-ELLM: Gradient-based Explanations for Decoder-only LLMs](https://arxiv.org/abs/2601.03089)
*Xin Huang,Antoni B. Chan*

Main category: cs.CL

TL;DR: 提出Grad-ELLM，一种基于梯度的归因方法，用于解码器仅Transformer大语言模型，通过聚合注意力层梯度和注意力图生成热图，并引入改进的忠实度指标，在多个任务上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的黑盒性质引发透明度和忠实度担忧，现有输入归因方法通常模型无关，未针对Transformer架构优化，导致忠实度有限。

Method: 开发Grad-ELLM方法，结合输出logit对注意力层的梯度（通道重要性）和注意力图（空间重要性）生成热图；引入π-Soft-NC和π-Soft-NS指标，通过控制扰动信息量公平评估忠实度。

Result: 在情感分类、问答和开放生成任务上测试不同模型，Grad-ELLM在忠实度方面始终优于其他归因方法。

Conclusion: Grad-ELLM有效提升大语言模型归因的忠实度，无需修改架构，为模型透明度提供实用工具。
...
### [146] [Mechanistic Interpretability of Large-Scale Counting in LLMs through a System-2 Strategy](https://arxiv.org/abs/2601.02989)
...
TL;DR: 提出一种受系统2认知启发的测试时策略，将大计数任务分解为小任务，使大语言模型能突破架构限制，显著提升大规模计数任务的准确率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂数学任务中表现优异，但在计数任务中存在系统性局限，这是由于Transformer架构中计数操作跨层执行，深度限制导致大规模计数精度下降。

Method: 采用受系统2认知过程启发的测试时策略，将大计数任务分解为可独立解决的小子问题；通过观察性和因果中介分析理解该策略的机制；识别关键组件：潜在计数存储于各部分最终表示中，通过专用注意力头传递，最终聚合得到总数。

Result: 该策略使大语言模型能够超越架构限制，在大规模计数任务中实现高准确率；机制分析揭示了系统2式计数在LLMs中的具体实现路径。

Conclusion: 本研究为理解LLMs中的系统2计数机制提供了机制性见解，并提出了一种可推广的方法来改善和理解其推理行为。
...
### [147] [Stable-RAG: Mitigating Retrieval-Permutation-Induced Hallucinations in Retrieval-Augmented Generation](https://arxiv.org/abs/2601.02993)
*Qianchi Zhang,Hainan Zhang,Liang Pang,Hongwei Zheng,Zhiming Zheng*

Main category: cs.CL

TL;DR: 本文提出Stable-RAG方法，通过估计检索文档顺序敏感性并利用聚类隐藏状态来减少LLM在RAG中的幻觉，提升答案准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法虽能减少LLM的事实幻觉，但未充分研究检索文档顺序对模型行为的影响。实验发现即使黄金文档固定，不同文档排列仍导致答案显著变化，表明存在未被充分探索的排列敏感性。

Method: 提出Stable-RAG：1) 在多种检索顺序下运行生成器；2) 对隐藏状态进行聚类；3) 从捕获主导推理模式的聚类中心表示解码；4) 利用推理结果将幻觉输出对齐至正确答案。

Result: 在三个QA数据集上的实验表明，相比基线方法，Stable-RAG显著提高了答案准确性、推理一致性，并在不同数据集、检索器和输入长度上展现出更强的鲁棒泛化能力。

Conclusion: 文档排列敏感性是RAG中影响LLM稳定性的重要因素，Stable-RAG通过显式建模和利用这种敏感性，有效减少了排列诱导的幻觉，提升了RAG系统的可靠性和一致性。
...
### [148] [Who Laughs with Whom? Disentangling Influential Factors in Humor Preferences across User Clusters and LLMs](https://arxiv.org/abs/2601.03103)
*Soichiro Murakami,Hidetaka Kamigaito,Hiroya Takamura,Manabu Okumura*

Main category: cs.CL

TL;DR: 该研究通过聚类分析日本创意游戏Oogiri中的用户幽默偏好，使用Bradley-Terry-Luce模型估计可解释偏好因子，并探索大语言模型（LLMs）的幽默偏好与特定用户群体的相似性及可引导性。


<details>
  <summary>Details</summary>
Motivation: 幽默偏好因个体和文化差异而高度多样化，这使得使用大语言模型评估幽默变得复杂。研究旨在建模这种异质性，并探索LLMs的幽默偏好与人类用户群体的关联。

Method: 1. 利用Oogiri游戏的投票日志对用户进行聚类；2. 使用Bradley-Terry-Luce模型估计各聚类在可解释偏好因子上的权重；3. 通过提示LLMs选择更有趣的回答来获取其偏好判断；4. 采用角色提示（persona prompting）引导LLMs的偏好朝向特定聚类。

Result: 1. 用户聚类表现出不同的幽默偏好模式；2. LLMs的偏好结果与特定用户聚类相似；3. 通过角色提示，可以成功将LLMs的偏好引导至目标聚类。

Conclusion: 研究证实了幽默偏好的异质性可通过聚类建模，LLMs的幽默偏好可与人类用户群体对齐，且通过提示工程可定向引导。数据收集与分析脚本将公开以支持可重复性。
...
### [149] [Large Reasoning Models Are (Not Yet) Multilingual Latent Reasoners](https://arxiv.org/abs/2601.02996)
*Yihong Liu,Raoyuan Zhao,Hinrich Schütze,Michael A. Hedderich*

Main category: cs.CL

TL;DR: 本文系统研究了大型推理模型在11种语言中的潜在推理现象，发现多语言潜在推理存在但不均衡，且内部预测演化高度一致，暗示了以英语为中心的潜在推理路径。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在数学推理任务中表现出色，通常归因于其生成显式思维链解释的能力。然而，近期研究发现模型常在完成文本推理步骤前就得出正确答案，表明存在潜在推理（隐藏状态中编码的内部非语言计算）。这一现象在英语中已有探索，但其多语言行为尚不明确。

Method: 采用基于截断的策略，通过仅提供部分推理痕迹来检查正确答案如何随着模型接收信息而逐步形成，从而测量逐步潜在预测的形成。此外，进行了表征分析以探究不同语言间的内部机制差异。

Result: 研究结果揭示了明显的多语言潜在推理证据，但分布不均衡：在资源丰富的语言中表现较强，在低资源语言中较弱，且在较难的基准测试中普遍较难观察到。表征分析表明，尽管表面存在差异，但预测的内部演化在跨语言间高度一致，且与英语模式广泛对齐。

Conclusion: 多语言潜在推理确实存在，但其强度受语言资源和任务难度影响。内部机制分析表明，不同语言间的潜在推理路径高度一致，并围绕英语中心化，这暗示了大型推理模型可能共享一种以英语为基础的潜在推理结构。
...
### [150] [ToxiGAN: Toxic Data Augmentation via LLM-Guided Directional Adversarial Generation](https://arxiv.org/abs/2601.03121)
*Peiran Li,Jan Fillies,Adrian Paschke*

Main category: cs.CL

TL;DR: 提出ToxiGAN框架，结合对抗生成与LLM语义指导，通过定向训练和语义锚点增强毒性语言数据增强，在四个仇恨言论基准测试中表现最优。


<details>
  <summary>Details</summary>
Motivation: 现有毒性语言数据增强方法面临监督有限和分布偏斜的挑战，难以实现可控且类别特定的增强，影响了毒性分类模型的鲁棒性。

Method: 提出ToxiGAN框架：1）采用类别感知的对抗生成方法；2）引入两步定向训练策略防止模式崩溃和语义漂移；3）利用LLM生成的中性文本作为语义锚点；4）动态选择中性示例提供平衡指导，并优化毒性样本与中性示例的差异。

Result: 在四个仇恨言论基准测试中，ToxiGAN在macro-F1和hate-F1指标上均取得最佳平均性能，显著优于传统方法和基于LLM的增强方法。消融实验证实语义锚点和定向训练对提升分类器鲁棒性的有效性。

Conclusion: ToxiGAN通过结合对抗生成与LLM语义指导，实现了可控的毒性语言数据增强，有效提升了毒性分类的鲁棒性，为数据增强方法提供了新思路。
...
### [151] [MedDialogRubrics: A Comprehensive Benchmark and Evaluation Framework for Multi-turn Medical Consultations in Large Language Models](https://arxiv.org/abs/2601.03023)
...
TL;DR: 提出MedDialogRubrics基准，包含5200个合成病例和6万+细粒度评估标准，用于评估医疗大语言模型的多轮诊断能力，发现现有模型面临显著挑战。


<details>
  <summary>Details</summary>
Motivation: 现有医疗对话AI的评估基准在信息收集和诊断推理能力评估方面不够严谨，且缺乏隐私保护的数据生成方法。

Method: 1. 使用多智能体系统合成患者病例（不依赖真实医疗记录）；2. 设计患者智能体配合动态纠错机制确保病例一致性；3. 基于循证医学指南和拒绝采样生成优先级评估标准。

Result: 对前沿模型的评估表明，当前模型在多维度评估中均面临重大挑战，仅靠基础模型微调不足以提升医疗对话能力。

Conclusion: 医疗对话系统的改进需要对话管理架构的革新，而不仅是基础模型的渐进式优化。
...
### [152] [MMFormalizer: Multimodal Autoformalization in the Wild](https://arxiv.org/abs/2601.03017)
...
TL;DR: 提出MMFormalizer，首个多模态自动形式化方法，通过自适应接地与递归构造将视觉物理问题转化为形式化命题，在涵盖经典力学、相对论、量子力学和热力学的PhyX-AF基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统自动形式化方法仅处理文本数学，无法应对现实世界中需从视觉元素推断隐藏约束（如质量、能量）的多模态物理问题。

Method: MMFormalizer通过自适应接地从真实世界数学与物理领域获取实体，递归地将感知接地的原语组合为形式命题，并利用自适应递归终止确保每个抽象都有视觉证据支持且基于维度或公理接地。

Result: 在PhyX-AF基准（115个来自MathVerse、PhyX、合成几何与解析几何的样本）上，前沿模型如GPT-5和Gemini-3-Pro在编译与语义准确率上表现最佳，其中GPT-5在物理推理上突出，而几何领域仍最具挑战性。

Conclusion: MMFormalizer为统一的多模态自动形式化提供了可扩展框架，连接了感知与形式推理，首次实现了处理经典力学、相对论、量子力学和热力学的多模态自动形式化。
...
### [153] [Self-Verification is All You Need To Pass The Japanese Bar Examination](https://arxiv.org/abs/2601.03144)
*Andrew Shin*

Main category: cs.CL

TL;DR: 本文提出了一种自验证模型，首次在保持原始试题结构和评分规则的情况下，使大语言模型通过了日本司法考试。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型快速发展，但在高度专业化和结构化的考试（如日本司法考试）中实现可靠性能仍具挑战。现有方法虽尝试将问题分解为简单判断，但未在原始考试格式和评分标准下系统评估，无法证明其真正具备考试水平的能力。

Method: 构建了忠实复制考试真实格式和评分标准的新数据集，并训练了一个自验证模型。该方法在保持原题结构和评分规则的前提下进行训练与评估，并与多智能体推理、基于分解的监督等替代策略进行了广泛比较。

Result: 该模型在实际考试评分标准下超过了官方及格分数，成为首个已知的不改变原题结构或评分规则即通过日本司法考试的LLM。比较实验表明，其他方法（如多智能体推理和分解监督）未能达到可比性能。

Conclusion: 研究强调了格式忠实的监督和一致性验证的重要性，表明精心设计的单模型方法在高风险专业推理任务中可以超越更复杂的系统。数据集和代码已公开。
...
### [154] [LittiChoQA: Literary Texts in Indic Languages Chosen for Question Answering](https://arxiv.org/abs/2601.03025)
*Aarya Khandelwal,Ritwik Mishra,Rajiv Ratn Shah*

Main category: cs.CL

TL;DR: 该研究针对印度语言缺乏长上下文问答资源的问题，创建了LittiChoQA数据集，包含27万+自动生成的文学问答对，并评估了多语言大模型在完整上下文和缩短上下文设置下的表现。


<details>
  <summary>Details</summary>
Motivation: 印度语言缺乏长上下文文学问答数据集，现有大模型在处理低资源语言的长上下文文学文本问答时面临挑战。

Method: 1. 从开放网络收集自然创作的文学文本；2. 自动生成超过27万个平衡分布的事实性和非事实性问答对；3. 在完整上下文和缩短上下文两种设置下评估多语言大模型；4. 使用答案段落选择和基于向量的检索两种缩短上下文方法。

Result: 1. 完整上下文微调获得最高分词级和语义级分数；2. 缩短上下文显著提高吞吐量；3. Krutrim-2模型表现最佳，完整上下文语义得分为76.1，缩短上下文中段落选择得74.9分，向量检索得71.4分。

Conclusion: 长上下文文学问答存在性能与效率的权衡：完整上下文性能最优但效率低，缩短上下文能提高效率但性能略有下降。Krutrim-2在印度语言文学问答任务中表现突出。
...
### [155] [DIP: Dynamic In-Context Planner For Diffusion Language Models](https://arxiv.org/abs/2601.03199)
*Yang Li,Han Meng,Chenan Wang,Haipeng Chen*

Main category: cs.CL

TL;DR: 提出DIP方法，通过动态选择和插入上下文示例来优化扩散语言模型的推理效率，在保持生成质量的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在处理长上下文时因双向注意力机制导致计算成本高昂，而自回归模型的序列生成范式无法直接应用。

Method: 基于扩散生成范式允许动态调整上下文的特性，提出动态上下文规划器（DIP），在生成过程中动态选择和插入上下文示例，而非一次性提供所有示例。

Result: DIP在保持生成质量的同时，相比标准推理实现最高12.9倍加速，相比KV缓存增强推理实现1.17倍加速。

Conclusion: DIP通过利用扩散生成范式的独特优势，有效解决了扩散语言模型在长上下文场景下的计算效率问题，为实际应用提供了可行方案。
...
### [156] [NorwAI's Large Language Models: Technical Report](https://arxiv.org/abs/2601.03034)
*Jon Atle Gulla,Peng Liu,Lemei Zhang*

Main category: cs.CL

TL;DR: NorwAI团队开发了针对挪威语和斯堪的纳维亚语言的大语言模型家族，基于多种Transformer架构，通过预训练和指令微调优化性能，并向北欧组织开放使用。


<details>
  <summary>Details</summary>
Motivation: 挪威语（约500万使用者）在自然语言处理领域代表性不足，需要专门的语言模型来填补这一空白。

Method: 采用GPT、Mistral、Llama2等Transformer架构，使用挪威语扩展的分词器，在250亿至884.5亿词元上从头预训练或持续预训练，并应用高级后训练策略和指令微调。

Result: 模型展现出强大的助手式能力，特别是在指令微调变体（如Mistral-7B-Instruct和Mixtral-8x7B-Instruct）中，适用于交互式和领域特定任务。

Conclusion: 这些模型为挪威语和斯堪的纳维亚语言提供了专门的大语言模型解决方案，具有实际部署潜力，并已向北欧的研究和实验用途开放。
...
### [157] [UltraLogic: Enhancing LLM Reasoning through Large-Scale Data Synthesis and Bipolar Float Reward](https://arxiv.org/abs/2601.03205)
*Yile Liu,Yixian Liu,Zongwei Li,Yufei Huang,Xinhua Feng,Zhichao Hu,Jinglu Hu,Jianfeng Yan,Fengzong Lian,Yuhong Liu*

Main category: cs.CL

TL;DR: 提出UltraLogic框架，通过代码化求解方法自动生成大规模、高质量、难度分级的多步逻辑推理数据，并引入双极浮动奖励机制提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要多步逻辑、规划和验证的复杂通用推理方面存在瓶颈，且缺乏大规模、高质量、难度分级的通用推理数据。

Method: 1. 提出UltraLogic框架，将问题逻辑核心与自然语言表达解耦，通过代码化求解方法自动生成数据；2. 构建包含数百种任务类型和十级难度自动校准的流程；3. 引入双极浮动奖励机制，使用分级惩罚区分完美回答与存在逻辑缺陷的回答。

Result: 实验表明任务多样性是提升推理能力的主要驱动力，双极浮动奖励结合难度匹配策略能显著提高训练效率，引导模型趋向全局逻辑最优解。

Conclusion: UltraLogic框架通过自动化数据生成和创新的奖励机制，有效解决了通用推理数据稀缺和训练效率问题，为大语言模型的复杂推理能力提升提供了可行路径。
...
### [158] [Reducing Hallucinations in LLMs via Factuality-Aware Preference Learning](https://arxiv.org/abs/2601.03027)
*Sindhuja Chaduvula,Ahmed Y. Radwan,Azib Farooq,Yani Ioannou,Shaina Raza*

Main category: cs.CL

TL;DR: 本文提出F-DPO方法，通过引入事实性标签和边界调整，在保持指令跟随能力的同时显著减少大语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有的偏好对齐方法（如RLHF和DPO）虽然能提升指令跟随能力，但可能因过度奖励流畅性和自信度而强化模型的事实性错误和幻觉。

Method: F-DPO在DPO基础上引入二元事实性标签，采用标签翻转校正错误排序的偏好对，并添加事实性感知边界以强调正确性差异明显的样本对。通过增强DPO数据对并生成合成幻觉变体构建训练数据。

Result: 在7个开源大语言模型（1B-14B）上，F-DPO相比基线模型和标准DPO持续提升事实性并降低幻觉率。例如在Qwen3-8B上，幻觉率降低5倍（0.424→0.084），事实性得分提升50%（5.26→7.90）；在TruthfulQA上，Qwen2.5-14B的MC1准确率提升17%（0.500→0.585），MC2准确率提升49%（0.357→0.531）。

Conclusion: F-DPO是一种无需辅助奖励模型、词级标注或多阶段训练的轻量方法，能有效提升大语言模型的事实性并减少幻觉，且具备良好的泛化能力。
...
### [159] [BaseCal: Unsupervised Confidence Calibration via Base Model Signals](https://arxiv.org/abs/2601.03042)
...
<details>
  <summary>Details</summary>
Motivation: 后训练LLM普遍存在过度自信问题，损害输出可信度，而对应基础LLM通常校准良好，这启发了利用基础LLM作为参考校准后训练LLM置信度的思路。

Method: 提出两种方法：1) BaseCal-ReEval：将后训练LLM响应输入基础LLM计算平均概率作为置信度；2) BaseCal-Proj：训练轻量投影层，将后训练LLM最后隐藏层状态映射回基础LLM状态，再通过基础LLM输出层计算校准后置信度。

Result: 在5个数据集和3个LLM系列上的实验表明，BaseCal平均将预期校准误差降低42.90%，优于无监督基线方法。

Conclusion: BaseCal作为无监督即插即用方案，能有效校准后训练LLM置信度，提升输出可靠性，且无需额外标注或模型改动。
...
### [160] [Learning to Diagnose and Correct Moral Errors: Towards Enhancing Moral Sensitivity in Large Language Models](https://arxiv.org/abs/2601.03079)
*Bocheng Chen,Han Zi,Xi Chen,Xitong Zhang,Kristen Johnson,Guangliang Liu*

Main category: cs.CL

TL;DR: 提出两种语用推理方法增强大语言模型的道德敏感性，使其能诊断道德良性与危险输入并纠正道德错误。


<details>
  <summary>Details</summary>
Motivation: 道德敏感性是人类道德能力的基础，但现有方法难以使大语言模型具备道德敏感性，需要探索增强其道德敏感性的途径。

Method: 提出两种基于推理负荷原则的语用推理方法，统一处理语义多样且复杂的道德话语表面形式。

Result: 实证表明该方法能有效增强大语言模型的道德敏感性，在代表性道德相关基准测试中表现优异。

Conclusion: 语用推理方法为增强大语言模型道德敏感性提供了有效途径，通过统一视角处理道德话语实现了性能提升。
...
### [161] [Temporal Graph Network: Hallucination Detection in Multi-Turn Conversation](https://arxiv.org/abs/2601.03051)
*Vidhi Rathore,Sambu Aneesh,Himanshu Singh*

Main category: cs.CL

TL;DR: 提出一种基于图神经网络的对话级幻觉检测方法，通过将对话建模为时序图，利用消息传递和注意力池化来识别幻觉。


<details>
  <summary>Details</summary>
Motivation: 对话AI系统在多轮对话中容易产生幻觉（前后矛盾或错误信息），现有方法难以有效检测对话级别的幻觉。

Method: 将对话建模为时序图，节点表示对话轮次（使用句子编码器编码），边分为实体共享边（连接提及相同实体的轮次）和时间边（连接相邻轮次）；通过消息传递更新节点表示，使用注意力池化聚合为全局向量，最后用分类器检测幻觉类型。

Result: 该方法在幻觉检测任务上略优于现有方法，且注意力机制可提供决策依据。

Conclusion: 图神经网络能有效建模对话结构，提升幻觉检测性能，并提供可解释性。
...
### [162] [Detecting Hallucinations in Retrieval-Augmented Generation via Semantic-level Internal Reasoning Graph](https://arxiv.org/abs/2601.03052)
*Jianpeng Hu,Yanzeng Li,Jialun Zhong,Wenfa Qi,Lei Zou*

Main category: cs.CL

TL;DR: 提出基于语义级内部推理图的忠实性幻觉检测方法，通过扩展层相关传播算法构建依赖图，并设计基于小预训练模型的通用检测框架。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成系统仍存在忠实性幻觉问题，先前检测方法未能充分捕捉模型内部推理过程或特征处理粗糙，导致判别器学习困难。

Method: 1. 将层相关传播算法从词元级扩展到语义级，基于归因向量构建内部推理图；2. 设计基于小预训练语言模型的通用框架，利用LLM推理依赖进行训练和检测，通过阈值动态调整正确样本通过率。

Result: 在RAGTruth和Dolly-15k数据集上，该方法相比现有最优基线取得了更好的整体性能。

Conclusion: 所提出的语义级内部推理图方法能更准确地表示依赖关系，有效提升忠实性幻觉检测能力，为RAG系统幻觉问题提供了新解决方案。
...
### [163] [The Anatomy of Conversational Scams: A Topic-Based Red Teaming Analysis of Multi-Turn Interactions in LLMs](https://arxiv.org/abs/2601.03134)
*Xiangzhe Yuan,Zhenhao Zhang,Haoming Tang,Siying Hu*

Main category: cs.CL

TL;DR: 本文通过LLM间多轮对话模拟，系统研究了大型语言模型在多轮诈骗场景中的风险，发现现有单轮安全评估无法有效捕捉此类风险，并揭示了防御机制和失败模式。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs通过多轮对话获得说服性代理能力，它们在多轮对话诈骗中引入了新的风险，而现有的单轮安全评估方法无法有效识别这些风险。

Method: 采用受控的LLM间模拟框架，在英语和中文环境下对八个最先进的模型进行多轮诈骗场景评估，分析对话结果并定性标注攻击者策略、防御响应和失败模式。

Result: 研究发现诈骗交互遵循重复的升级模式，防御机制主要采用验证和延迟策略；交互失败常源于安全护栏激活和角色不稳定性。

Conclusion: 多轮交互安全是LLM行为中一个关键且独特的维度，需要专门的安全评估框架来应对多轮对话中的风险。
...
### [164] [Discovering and Causally Validating Emotion-Sensitive Neurons in Large Audio-Language Models](https://arxiv.org/abs/2601.03115)
*Xiutian Zhao,Björn Schuller,Berrak Sisman*

Main category: cs.CL

TL;DR: 该研究首次对大型音频语言模型中的情感敏感神经元进行了神经元级可解释性分析，通过推理时干预揭示了情感决策的因果机制，并展示了针对性神经元干预对情感行为的可控性。


<details>
  <summary>Details</summary>
Motivation: 情感是语音交流的核心维度，但现代大型音频语言模型如何内部编码情感仍缺乏机制性解释，这促使研究者探索模型内部是否存在专门处理情感信息的神经元单元。

Method: 在三种开源模型上比较了基于频率、熵、幅度和对比度的神经元选择器；使用推理时干预技术，包括神经元消融和增益放大；分析了神经元在层级上的非均匀聚类和跨数据集迁移性。

Result: 发现情感敏感神经元普遍存在于三种模型中；消融特定情感相关的神经元会显著降低该情感的识别准确率，而增益放大则能将预测导向目标情感；干预效果随干预强度系统性地增强；情感敏感神经元在层级上呈现非均匀聚类。

Conclusion: 研究为大型音频语言模型的情感决策提供了神经元级的因果解释，并证明针对性神经元干预是实现可控情感行为的有效途径。
...
### [165] [Improving Indigenous Language Machine Translation with Synthetic Data and Language-Specific Preprocessing](https://arxiv.org/abs/2601.03135)
*Aashish Dhawan,Christopher Driggers-Ellis,Christan Grant,Daisy Zhe Wang*

Main category: cs.CL

TL;DR: 通过合成数据增强和语言特定预处理，提升美洲低资源土著语言的神经机器翻译质量，在瓜拉尼语和克丘亚语上取得改进，但高度黏着语如艾马拉语仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: 美洲土著语言缺乏平行语料库，限制了神经机器翻译的应用，需要解决低资源环境下的数据稀缺问题。

Method: 使用多语言翻译模型生成合成句子对增强平行数据集；对mBART模型进行微调；采用正字法规范化和噪声感知过滤等语言特定预处理；使用chrF++指标评估翻译质量。

Result: 合成数据增强在瓜拉尼语-西班牙语和克丘亚语-西班牙语翻译中带来一致的chrF++提升；但对高度黏着语艾马拉语，通用预处理方法效果有限。

Conclusion: 合成数据增强是提升低资源土著语言翻译的有效策略，但需要针对不同语言类型设计特定的预处理方法，特别是对高度黏着语。
...
### [166] [Decoupling the Effect of Chain-of-Thought Reasoning: A Human Label Variation Perspective](https://arxiv.org/abs/2601.03154)
*Beiduo Chen,Tiancheng Hu,Caiqi Zhang,Robert Litschko,Anna Korhonen,Barbara Plank*

Main category: cs.CL

TL;DR: 研究发现长思维链能提升LLM在分布任务中的对齐度，但最终准确性由思维链内容决定，而分布排序由模型先验主导，思维链无法作为模糊任务的精细分布校准器。


<details>
  <summary>Details</summary>
Motivation: 探索思维链调整的LLM在需要捕捉概率模糊性（而非解决模糊性）的人类标签变异任务中的能力，目前这方面研究不足。

Method: 通过系统解耦实验，在基于分布的任务中使用交叉思维链实验，分离推理文本与内在模型先验的影响。

Result: 发现解耦机制：思维链提升分布对齐，但准确性99%由思维链内容决定，分布排序80%以上由模型先验主导；逐步分析显示思维链对准确性的影响单调增长，而分布结构主要由LLM内在先验决定。

Conclusion: 长思维链可作为LLM对最优选项的决策者，但无法作为模糊任务的精细分布校准器。
...
### [167] [Multi-RADS Synthetic Radiology Report Dataset and Head-to-Head Benchmarking of 41 Open-Weight and Proprietary Language Models](https://arxiv.org/abs/2601.03232)
...
TL;DR: 该研究创建了一个放射科医生验证的多RADS基准数据集RXL-RADSet，并比较了开源小语言模型与专有模型在RADS分类任务中的表现。研究发现，在引导提示下，大型SLM（20-32B参数）可以接近专有模型的性能，但对于更复杂的RADS方案仍存在差距。


<details>
  <summary>Details</summary>
Motivation: 放射学报告和数据系统（RADS）用于标准化放射学风险沟通，但自动化RADS分配面临挑战，包括指南复杂性、输出格式限制以及跨RADS框架和模型大小的基准测试有限。

Method: 创建了包含1,600份合成放射学报告的RXL-RADSet基准数据集，涵盖10种RADS标准和多种成像模态。报告由LLM生成，并经过两阶段放射科医生验证。评估了41个量化SLM（0.135-32B参数）和GPT-5.2在固定引导提示下的表现，主要终点是有效性和准确性。

Result: 在引导提示下，GPT-5.2达到99.8%的有效性和81.1%的准确性。SLM总体达到96.8%的有效性和61.1%的准确性，其中20-32B参数范围的顶级SLM达到约99%有效性和70%以上的准确性。性能随模型规模增加而提升（拐点在<1B和>=10B之间），并随RADS复杂性增加而下降。引导提示相比零样本提示显著提升了有效性和准确性。

Conclusion: RXL-RADSet提供了一个放射科医生验证的多RADS基准；在引导提示下，大型SLM（20-32B）可以接近专有模型的性能，但对于更高复杂度的RADS方案仍存在性能差距。
...
### [168] [WebAnchor: Anchoring Agent Planning to Stabilize Long-Horizon Web Reasoning](https://arxiv.org/abs/2601.03164)
*Yu Xinmiao,Zhang Liwen,Feng Xiaocheng,Jiang Yong,Qin Bing,Xie Pengjun,Zhou Jingren*

Main category: cs.CL

TL;DR: 本文提出Anchor-GRPO框架，通过两阶段强化学习解决长视野网络推理任务中的规划瓶颈问题，显著提升任务成功率和工具使用效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体在长视野网络推理任务中存在规划瓶颈，表现为'规划锚定'现象——第一步推理对后续行为影响过大，而当前强化学习方法未能有效处理此问题。

Method: 提出Anchor-GRPO两阶段强化学习框架：第一阶段通过自博弈经验和人工校准的细粒度评分优化第一步规划；第二阶段通过稀疏奖励使执行与初始规划对齐，确保工具使用的稳定性。

Result: 在BrowseComp、BrowseComp-Zh、GAIA和XBench-DeepSearch四个基准测试中，Anchor-GRPO在3B到30B模型上均优于基线方法，WebAnchor-30B在BrowseComp上达到46.0% pass@1，在GAIA上达到76.4%。框架展现出良好的可扩展性。

Conclusion: Anchor-GRPO通过解耦规划与执行有效缓解了规划锚定问题，为长视野网络推理任务提供了更稳定高效的优化范式，且随着模型规模和上下文长度增加性能持续提升。
...
### [169] [Maximizing Local Entropy Where It Matters: Prefix-Aware Localized LLM Unlearning](https://arxiv.org/abs/2601.03190)
*Naixin Zhai,Pengyang Shao,Binbin Zheng,Fei Shen,Long Bai,Xun Yang*

Main category: cs.CL

TL;DR: 提出PALU框架，通过局部熵最大化实现大语言模型敏感知识的精确遗忘，减少对模型整体性能的影响。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法通常对所有响应令牌进行全局处理，导致不必要的性能下降和优化冗余，需要更精确的遗忘方法。

Method: 提出PALU框架，采用局部熵最大化目标，仅抑制敏感前缀并扁平化top-k逻辑值，避免全词汇表和参数空间的冗余优化。

Result: 实验验证PALU在遗忘效果和模型效用保持方面优于现有基线方法。

Conclusion: PALU通过局部化处理实现了更精确的敏感知识遗忘，同时最小化对模型整体性能的损害。
...
### [170] [Can Embedding Similarity Predict Cross-Lingual Transfer? A Systematic Study on African Languages](https://arxiv.org/abs/2601.03168)
*Tewodros Kederalah Idris,Prasenjit Mitra,Roald Eiselen*

Main category: cs.CL

TL;DR: 本文系统评估了五种嵌入相似性度量在跨语言迁移中的预测能力，发现余弦间隙和检索类指标能可靠预测迁移成功，而CKA预测能力较弱，并强调了模型特异性分析的重要性。


<details>
  <summary>Details</summary>
Motivation: 针对低资源非洲语言构建NLP系统时，缺乏可靠的源语言选择方法，需要系统评估不同嵌入相似性度量的预测效果。

Method: 在三个NLP任务、三个非洲中心多语言模型和12种来自四个语系的语言上，进行了816次迁移实验，系统评估了五种嵌入相似性度量。

Result: 余弦间隙和检索类指标（P@1、CSLS）能可靠预测迁移成功（ρ=0.4-0.6），CKA预测能力很弱（ρ≈0.1）；跨模型聚合时相关性符号会反转（辛普森悖论）；嵌入度量的预测能力与URIEL语言类型学相当。

Conclusion: 研究结果为源语言选择提供了具体指导，强调了模型特异性验证的重要性，嵌入相似性度量可作为语言类型学的有效补充。
...
### [171] [MemRL: Self-Evolving Agents via Runtime Reinforcement Learning on Episodic Memory](https://arxiv.org/abs/2601.03192)
...
TL;DR: 提出MemRL框架，通过基于情节记忆的非参数强化学习实现智能体自我进化，解决LLM在持续学习中的稳定性-可塑性困境。


<details>
  <summary>Details</summary>
Motivation: 人类智能的核心是通过情节模拟整合过去经验解决新任务，而现有LLM难以实现这种自我进化：微调成本高且易灾难性遗忘，基于记忆的方法依赖被动语义匹配易检索噪声。

Method: MemRL框架将冻结LLM的稳定推理与可塑性记忆分离，采用两阶段检索机制：先语义相关度筛选候选，再基于学习到的Q值（效用）选择，通过环境反馈以试错方式持续优化效用评估。

Result: 在HLE、BigCodeBench、ALFWorld和Lifelong Agent Bench等基准测试中，MemRL显著优于现有最优方法，分析实验证实其能有效协调稳定性与可塑性。

Conclusion: MemRL使智能体无需权重更新即可在运行时持续改进，为非参数强化学习与情节记忆的结合提供了新范式，解决了LLM持续学习的关键挑战。
...
### [172] [Automated Semantic Rules Detection (ASRD) for Emergent Communication Interpretation](https://arxiv.org/abs/2601.03254)
*Bastien Vanderplaetse,Xavier Siebert,Stéphane Dupont*

Main category: cs.CL

TL;DR: 本文提出了一种自动语义规则检测算法，用于解释多智能体系统中涌现通信的可解释性，通过分析智能体在Lewis游戏中交换的消息模式，并将其与输入数据属性关联。


<details>
  <summary>Details</summary>
Motivation: 当前对多智能体系统中涌现通信的研究较少关注其可解释性，缺乏对智能体自主发展通信策略的理解和分析方法。

Method: 提出自动语义规则检测算法，分析智能体在Lewis游戏中基于两种不同数据集训练时交换的消息模式，并将模式与输入数据属性关联。

Result: ASRD算法能够有效提取消息中的相关模式，并将这些模式与特定输入数据属性关联，显著简化了对涌现通信的后续分析。

Conclusion: 该研究为涌现通信的可解释性提供了有效工具，通过自动化模式检测简化了分析过程，有助于理解智能体自主发展的通信策略。
...
### [173] [STReasoner: Empowering LLMs for Spatio-Temporal Reasoning in Time Series via Spatial-Aware Reinforcement Learning](https://arxiv.org/abs/2601.03248)
*Juntong Ni,Shiyu Wang,Ming Jin,Qi He,Wei Jin*

Main category: cs.CL

TL;DR: 提出ST-Bench基准和STReasoner模型，用于时空序列的显式推理，通过强化学习奖励空间信息利用，在低成本下实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列研究多关注预测精度而忽视推理能力，但时空推理对交通、电网、疾病传播等高风险决策至关重要，该领域仍待发展。

Method: 1. 构建ST-Bench基准（含病因推理、实体识别等四任务），基于网络SDE多智能体数据合成；2. 提出STReasoner模型，使LLM整合时序、图结构和文本进行显式推理；3. 设计S-GRPO强化学习算法，专门奖励空间信息带来的性能增益。

Result: STReasoner在基准任务上平均准确率提升17%-135%，成本仅为专有模型的0.004倍，且在真实数据上泛化鲁棒。

Conclusion: 该研究填补了时空序列推理的空白，证明了显式整合多模态信息与空间感知强化学习的有效性，为高风险决策系统提供了新工具。
...
### [174] [X-MuTeST: A Multilingual Benchmark for Explainable Hate Speech Detection and A Novel LLM-consulted Explanation Framework](https://arxiv.org/abs/2601.03194)
...
TL;DR: 提出X-MuTeST框架，结合大语言模型语义推理与传统注意力增强技术，提升印地语、泰卢固语和英语仇恨言论检测的准确性与可解释性，并发布带词级标注的数据集。


<details>
  <summary>Details</summary>
Motivation: 社交媒体仇恨言论检测在准确性和可解释性方面存在挑战，尤其是针对资源匮乏的印度语言（如印地语、泰卢固语），现有方法缺乏可解释性指导的训练框架。

Method: 提出X-MuTeST框架：1) 结合LLM高层语义推理与传统注意力增强技术；2) 通过计算原始文本与n-gram预测概率差异生成解释；3) 融合LLM解释与X-MuTeST解释；4) 利用人工标注的词级理由优化模型注意力。

Result: 1) 使用人工理由训练可提升分类性能和可解释性；2) 结合人工理由与可解释性方法进一步优化模型注意力；3) 在合理性（Token-F1/IOU-F1）和忠实性（Comprehensiveness/Sufficiency）指标上表现良好；4) 发布包含6,004个印地语、4,492个泰卢固语和6,334个英语样本的数据集。

Conclusion: X-MuTeST框架能有效提升多语言仇恨言论检测的可解释性和性能，特别适用于资源匮乏语言，推动了跨语言仇恨言论检测研究。
...
### [175] [MalruleLib: Large-Scale Executable Misconception Reasoning with Step Traces for Modeling Student Thinking in Mathematics](https://arxiv.org/abs/2601.03217)
*Xinghe Chen,Naiming Liu,Shashank Sonkar*

Main category: cs.CL

TL;DR: 本文介绍了MalruleLib框架，将数学学习中的常见错误概念转化为可执行程序，用于生成学生错误推理的逐步追踪，并评估语言模型在跨模板误解预测上的表现。


<details>
  <summary>Details</summary>
Motivation: 学生在数学学习中常出现系统性错误，即反复应用错误但自洽的解题步骤。现有教育AI缺乏对错误概念的系统建模，难以诊断学生误解并提供针对性反馈。

Method: 基于67个学习科学和数学教育文献，将错误概念形式化为101个可执行错误规则（malrules），覆盖498个参数化问题模板。提出Malrule推理准确率（MRA）评估框架，要求模型从单个错误解答推断误解并在跨模板重述中预测学生答案。

Result: 在9个语言模型（4B-120B）测试中，跨模板误解预测准确率从直接解题的66%降至40%。提供学生步骤追踪可提升预测3-15%，但跨模板场景仍导致10-21%的性能下降。

Conclusion: MalruleLib能生成超百万实例，为教育AI提供可扩展的监督数据和受控评估环境，支持跨情境的学生程序建模，助力误解诊断和针对性反馈系统的开发。
...
### [176] [Trust in LLM-controlled Robotics: a Survey of Security Threats, Defenses and Challenges](https://arxiv.org/abs/2601.02377)
...
TL;DR: 本文系统综述了大型语言模型（LLM）控制机器人系统的安全威胁与防御策略，重点分析了因‘具身鸿沟’引发的独特安全挑战，并提出了分类框架与未来研究方向。


<details>
  <summary>Details</summary>
Motivation: LLM与机器人融合提升了任务执行能力，但也引入了因抽象推理与物理环境不匹配（‘具身鸿沟’）导致的安全漏洞，现有文本LLM安全方案难以应对可能引发物理危害的威胁。

Method: 采用系统性综述方法，对攻击向量（如越狱、后门攻击、多模态提示注入）和防御机制（形式化安全规范、运行时监控、多LLM监督、提示强化）进行分类分析，并评估相关数据集与基准。

Result: 构建了LLM-机器人安全威胁与防御的分类体系，指出当前防御措施在上下文感知方面的不足，强调需开发适应物理场景的安全解决方案。

Conclusion: LLM控制的机器人面临独特的具身化安全风险，需结合上下文感知的安全框架来保障系统可靠运行，本文为后续研究提供了基础路线图。
...
### [177] [Movement Primitives in Robotics: A Comprehensive Survey](https://arxiv.org/abs/2601.02379)
*Nolan B. Gutierrez,William J. Beksi*

Main category: cs.RO

TL;DR: 本文是一篇关于运动基元在机器人学中应用与发展的综述性论文，系统回顾了运动基元框架的演变、优势与局限，并探讨了其在机器人领域的实际应用与未来挑战。


<details>
  <summary>Details</summary>
Motivation: 受生物系统通过连续运动片段完成复杂任务的启发，研究者希望为自主系统（如机器人）找到类似生物运动基元的控制方法，以实现灵活、创造性的任务执行。

Method: 采用编年顺序的百科全书式综述方法，系统梳理运动基元框架的发展历程，分析其理论特性（如弹簧阻尼系统特性、概率耦合演示、神经网络建模等），并评估不同方法的优缺点。

Result: 运动基元能够有效表征通过人类示范学习的机器人控制轨迹，支持如抓取杯子、抛球等基本动作编码，并在高维系统中展现出适应性。综述总结了主流框架的特点、成功应用案例及实践中的关键问题。

Conclusion: 运动基元为机器人控制提供了灵活且可解释的表示方法，但其实际应用仍面临开放性问题与工程挑战，未来需进一步优化框架以适应更复杂的机器人任务场景。
...
### [178] [Modeling the Mental World for Embodied AI: A Comprehensive Review](https://arxiv.org/abs/2601.02378)
*Biyuan Liu,Daigang Xu,Lei Jiang,Wenjun Guo,Ping Chen*

Main category: cs.RO

TL;DR: 本文综述了具身AI中的心理世界模型（MWM）研究，构建了完整的理论框架，区分了MWM与物理世界模型（PWM）的本质差异，系统定义了心理要素表示范式，分析了心智理论（ToM）推理方法，并总结了神经符号混合架构趋势与评估基准。


<details>
  <summary>Details</summary>
Motivation: 随着具身AI代理在虚拟形象、可穿戴设备和机器人系统中的深入应用，其核心研究挑战已从物理环境交互转向对社会互动的准确理解。传统物理世界模型（PWM）无法满足社会智能建模需求，而心理世界模型（MWM）作为人类内部心理状态的结构化表示，是实现自然人机协作与动态社会适应的关键认知基础。当前MWM研究面临概念框架碎片化、推理机制脱节、评估与实践分离等瓶颈。

Method: 本文通过系统综述100余项权威研究，采用理论框架构建与范式分析方法，区分MWM与PWM的本质差异，通过两种心理要素表示范式定义MWM关键组件，分析两种核心ToM推理范式及19种ToM方法，并阐明神经符号混合架构的融合趋势。

Result: 研究首次构建了完整的MWM理论框架，明确了MWM与PWM的差异；系统定义了基于表示范式的MWM关键组件；全面分析了两种ToM推理范式及相关方法；总结了26个ToM评估基准，揭示了神经符号混合架构的集成趋势。

Conclusion: 心理世界模型（MWM）是具身AI实现社会智能的核心认知基础。本综述通过整合理论框架、技术方法与评估体系，为MWM研究提供了系统性指导，有助于推动具身AI融入人类社会并促进人机协同交互的深入发展。
...
### [179] [InternVLA-A1: Unifying Understanding, Generation and Action for Robotic Manipulation](https://arxiv.org/abs/2601.02456)
...
TL;DR: 提出InternVLA-A1模型，通过统一的Mixture-of-Transformers架构整合场景理解、视觉预测和动作执行能力，在12个真实机器人任务中显著超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型缺乏物理世界动态推理能力，而世界模型方法又存在语义基础薄弱和预测错误脆弱性问题，需要融合语义理解与动态预测能力。

Method: 采用统一掩码自注意力机制的Mixture-of-Transformers架构，包含三个专家模块；基于InternVL3和Qwen3-VL构建2B/3B参数模型；在混合合成-真实数据集上进行预训练。

Result: 在真实机器人任务中显著优于pi0和GR00T N1.5等领先模型，日常任务提升14.5%，动态场景（如传送带分拣）提升40%-73.3%。

Conclusion: InternVLA-A1成功融合了语义理解与动态预测能力，通过混合训练策略有效利用合成数据多样性并减小仿真-现实差距，在复杂机器人任务中表现出色。
...
### [180] [Learning and Optimizing the Efficacy of Spatio-Temporal Task Allocation under Temporal and Resource Constraints](https://arxiv.org/abs/2601.02505)
*Jiazhen Liu,Glen Neville,Jinwoo Park,Sonia Chernova,Harish Ravichandar*

Main category: cs.RO

TL;DR: 本文提出STEAM问题框架和E-ITAGS算法，用于优化异构多机器人系统的任务分配、调度和路径规划，通过主动学习特征-效能映射，在满足时空约束的同时最大化任务效能。


<details>
  <summary>Details</summary>
Motivation: 复杂多机器人任务需要联合优化分配、调度和路径规划，现有方法通常采用二元成功-失败模型，无法建模任务效能与机器人能力之间的连续关系，且难以处理时空约束。

Method: 提出STEAM问题形式化框架，引入特征-效能映射模型；开发E-ITAGS算法，集成任务分配、调度和路径规划；采用可实现性感知的主动学习模块在线学习特征-效能映射。

Result: 仿真和应急响应场景实验表明，E-ITAGS相比基线方法能生成更高效能的任务分配方案，同时满足资源与时空约束；主动学习方法样本高效，在数据与计算效率间取得平衡。

Conclusion: STEAM框架和E-ITAGS算法能有效解决复杂多机器人任务优化问题，通过显式建模效能和主动学习，在严格约束下提升团队性能，为异构机器人协同提供了新方法。
...
### [181] [Making Infeasible Tasks Feasible: Planning to Reconfigure Disconnected 3D Environments with Movable Objects](https://arxiv.org/abs/2601.02645)
*Samarth Kalluraya,Yiannis Kantaros*

Main category: cs.RO

TL;DR: 提出BRiDGE规划器，用于在3D环境中通过移动物体搭建桥梁，解决目标区域因高度差或间隙而无法直接到达的导航问题。


<details>
  <summary>Details</summary>
Motivation: 现有路径规划器假设目标区域可达，但在实际3D环境中，支撑平面可能因高度差、孔洞或横向分离而断开，导致任务不可行。现有NAMO方法主要针对2D推离障碍物场景，无法直接处理3D环境中需要战略性放置物体以建立连接的问题。

Method: 开发基于采样的BRiDGE规划器，在机器人和物体配置空间上增量构建树，规划移动哪些物体、放置位置及顺序，同时考虑可移动物体数量限制；引入非均匀采样策略以加速规划。

Result: 方法被证明具有概率完备性；通过大量数值仿真和硬件实验验证了其有效性。

Conclusion: BRiDGE能够有效解决3D断开环境中的导航问题，通过环境交互重组可移动物体，为原本不可达的目标区域创造可遍历连接。
...
### [182] [Effective Online 3D Bin Packing with Lookahead Parcels Using Monte Carlo Tree Search](https://arxiv.org/abs/2601.02649)
*Jiangyi Fang,Bowen Zhou,Haotian Wang,Xin Zhu,Leye Wang*

Main category: cs.RO

TL;DR: 提出一种结合前瞻信息的在线三维装箱方法，通过模型预测控制和蒙特卡洛树搜索框架，在物流场景中动态平衡学习策略与随机策略，显著提升分布偏移下的装箱性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度强化学习方法在物流场景中面临短期分布偏移（如不同批次货物到达）时性能下降，而现代物流系统中可用的短期前瞻信息未被充分利用以应对此问题。

Method: 将带前瞻包裹的在线三维装箱建模为模型预测控制问题，采用蒙特卡洛树搜索框架求解，设计动态探索先验以平衡学习到的RL策略和鲁棒随机策略，并引入惩罚长期空间浪费的辅助奖励。

Result: 在真实数据集上的实验表明，该方法在分布偏移下性能提升超过10%，在线部署平均提升4%，最佳情况下提升超过8%，均优于现有基线方法。

Conclusion: 所提框架通过有效利用前瞻信息和动态策略平衡，显著提升了在线三维装箱在分布变化下的适应性和整体性能，验证了前瞻信息在物流优化中的关键作用。
...
### [183] [Learning to Nudge: A Scalable Barrier Function Framework for Safe Robot Interaction in Dense Clutter](https://arxiv.org/abs/2601.02686)
*Haixin Jin,Nikhil Uday Shinde,Soofiyan Atar,Hongzhan Yu,Dylan Hirsch,Sicun Gao,Michael C. Yip,Sylvia Herbert*

Main category: cs.RO

TL;DR: 提出Dense Contact Barrier Functions (DCBF)，一种可组合的对象中心安全函数，使机器人能够在密集杂乱环境中安全导航和操作，避免传统方法在接触处理、计算复杂性和任务迁移方面的限制。


<details>
  <summary>Details</summary>
Motivation: 传统安全框架将物理接触视为不安全，限制机器人在密集日常环境中的操作能力；基于模型的方法在对象增多时计算不可行，而学习方法通常与特定任务绑定，难以迁移到新任务。

Method: 学习可组合的对象中心函数（DCBF），隐式捕获物理交互产生的安全约束；离线训练少量对象交互，运行时跨任意对象集组合，生成线性扩展的全局安全过滤器。

Result: 在密集杂乱模拟实验中验证了DCBF能够实现无碰撞导航和安全接触丰富的交互，且无需重新训练即可跨任务迁移。

Conclusion: DCBF通过可组合的对象中心安全函数，解决了密集环境中机器人安全操作的扩展性和迁移性问题，为实际应用提供了可行方案。
...
### [184] [Analysis of Various Manipulator Configurations Based on Multi-Objective Black-Box Optimization](https://arxiv.org/abs/2601.02704)
*Kento Kawaharazuka,Keita Yoneda,Takahiro Hattori,Shintaro Inoue,Kei Okada*

Main category: cs.RO

TL;DR: 通过多目标优化方法研究机械臂的最优结构设计，分析现有机械臂在优化空间中的位置，为未来设计提供指导。


<details>
  <summary>Details</summary>
Motivation: 现有6自由度和7自由度机械臂的关节配置和连杆长度比例多为经验性设计，且不同机器人结构各异。为探讨机械臂的最优结构，需要从系统化优化角度进行研究。

Method: 采用多目标优化方法，从末端执行器可达性和关节扭矩两个角度对机械臂结构进行优化分析，并对现有机械臂结构在优化采样结果中的位置进行评估。

Result: 通过优化分析揭示了现有机械臂结构在优化空间中的分布位置，识别了其与理论最优结构的差异。

Conclusion: 研究为未来机械臂设计提供了基于多目标优化的结构设计见解，有助于推动机械臂结构从经验设计向系统化优化设计转变。
...
### [185] [Loop Closure using AnyLoc Visual Place Recognition in DPV-SLAM](https://arxiv.org/abs/2601.02723)
*Wenzheng Zhang,Kazuki Adachi,Yoshitaka Hara,Sousuke Nakamura*

Main category: cs.RO

TL;DR: 提出一种改进DPV-SLAM闭环检测的方法，用基于深度特征的AnyLoc替代传统词袋模型，并引入自适应阈值机制，在室内外数据集中显著提升了闭环精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统基于手工特征的词袋模型在视角和光照变化下闭环检测性能有限，需要手动调整阈值，限制了SLAM系统的准确性和适应性。

Method: 1. 用基于深度学习的视觉位置识别方法AnyLoc替代传统词袋模型；2. 设计自适应机制动态调整相似度阈值，无需人工调参。

Result: 在室内外数据集上的实验表明，该方法相比原始DPV-SLAM在闭环检测准确率和鲁棒性方面有显著提升。

Conclusion: 该方法为现代SLAM系统提供了一种实用且可扩展的闭环性能增强方案，能更好地适应复杂环境变化。
...
### [186] [Optimizing Control-Friendly Trajectories with Self-Supervised Residual Learning](https://arxiv.org/abs/2601.02738)
*Kexin Guo,Zihan Yang,Yuhang Liu,Jindou Jia,Xiang Yu*

Main category: cs.RO

TL;DR: 提出一种自监督残差学习和轨迹优化框架，用于解决复杂机器人系统因物理模型不精确而难以准确跟踪激进轨迹的问题。通过将未知动态效应作为名义动力学的残差进行学习，构建混合模型，并开发轨迹优化器最小化残差物理影响，最终实现四旋翼飞行器的敏捷飞行精确跟踪。


<details>
  <summary>Details</summary>
Motivation: 现实物理系统难以用解析模型精确描述，导致控制器合成时存在残差物理效应，使激进轨迹的精确跟踪面临挑战。

Method: 1. 自监督残差学习：利用轨迹级数据学习闭环模型中的未知动态效应，作为名义动力学的残差，构建混合模型；2. 基于解析梯度的学习：实现任意积分步长下的准确长时程预测；3. 轨迹优化器开发：计算最优参考轨迹，最小化沿轨迹的残差物理影响。

Result: 通过混合动力学模型，优化器能够生成可被精确跟踪的激进运动轨迹，在四旋翼飞行器的敏捷飞行实验中验证了框架的有效性。

Conclusion: 所提框架通过残差学习和轨迹优化的结合，有效解决了复杂机器人系统因模型不精确导致的轨迹跟踪难题，为激进运动的精确控制提供了可行方案。
...
### [187] [Unified Meta-Representation and Feedback Calibration for General Disturbance Estimation](https://arxiv.org/abs/2601.02762)
*Zihan Yang,Jindou Jia,Meng Wang,Yuhang Liu,Kexin Guo,Xiang Yu*

Main category: cs.RO

TL;DR: 提出一种基于元学习和反馈校准在线适应的通用扰动估计框架，能有效处理非结构性时变扰动，在四旋翼飞行实验中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于元学习的方法需要共享环境结构表示，缺乏对现实非结构性扰动的灵活性，且表示误差和分布偏移会导致预测精度严重下降。

Method: 通过从过去有限时间窗口的观测中提取特征，学习无需预定义结构假设的统一表示；采用状态反馈机制校准在线适应过程，以减弱表示和泛化性限制带来的学习残差。

Result: 理论分析表明在线学习误差和扰动估计误差可同时收敛；框架能有效估计多种快速变化的扰动，四旋翼飞行实验验证了其性能。

Conclusion: 该框架通过统一元表示和反馈校准的在线适应，实现了对非结构性时变扰动的高效估计，提升了机器人应用的精确控制能力。
...
### [188] [Advancing Assistive Robotics: Multi-Modal Navigation and Biophysical Monitoring for Next-Generation Wheelchairs](https://arxiv.org/abs/2601.02766)
*Md. Anowar Hossain,Mohd. Ehsanul Hoque*

Main category: cs.RO

TL;DR: 提出一种集成四种控制模式（操纵杆、语音、手势、眼电）和实时生命体征监测的多模态电动轮椅控制系统，旨在提升残障人士的独立性和护理人员的监护能力。


<details>
  <summary>Details</summary>
Motivation: 电动轮椅是肌萎缩侧索硬化症、中风后偏瘫等行动障碍者的重要辅助工具，但现有系统缺乏灵活的控制切换和实时健康监测功能，难以兼顾用户自主性与护理需求。

Method: 整合操纵杆、语音、手势和眼电四种控制接口，结合心率变异性、血氧饱和度和皮肤温度的持续监测；通过双点校准生物传感器，在20名参与者中进行500次室内导航指令实验评估性能。

Result: 控制指令识别准确率分别为：操纵杆99%、语音97%±2%、手势95%±3%；生命体征监测误差在心率≤2bpm、体温≤0.5℃、血氧≤1%以内；系统闭环延迟为20±0.5毫秒，生理数据经加密云端传输至护理端Android应用。

Conclusion: 该系统通过多模态控制与云端健康监测的结合，提升了辅助机器人的安全性与适应性，符合ISO/IEC相关标准，并为未来集成自适应机器学习奠定了基础。
...
### [189] [M-SEVIQ: A Multi-band Stereo Event Visual-Inertial Quadruped-based Dataset for Perception under Rapid Motion and Challenging Illumination](https://arxiv.org/abs/2601.02777)
*Jingcheng Cao,Chaoran Xiong,Jianmin Song,Shang Yan,Jiachen Liu,Ling Pei*

Main category: cs.RO

TL;DR: 提出了M-SEVIQ数据集，这是一个用于四足机器人感知的多波段立体事件视觉与惯性数据集，旨在解决现有事件相机数据集在立体配置和多波段感知方面的不足。


<details>
  <summary>Details</summary>
Motivation: 传统帧式相机在四足机器人快速运动或低光条件下易产生模糊图像，而事件相机具有低延迟、高时间分辨率和动态范围的优势，但现有数据集缺乏立体配置和多波段感知数据。

Method: 使用Unitree Go2四足机器人搭载立体事件相机、帧式相机、IMU和关节编码器，在多种速度、光照波长和照明条件下采集了30多个真实世界序列，并提供完整的标定数据。

Result: 构建了包含多传感器同步数据的M-SEVIQ数据集，支持传感器融合、语义分割和多模态视觉研究，适用于挑战性环境下的机器人感知任务。

Conclusion: M-SEVIQ数据集填补了事件相机在立体和多波段感知领域的空白，为敏捷机器人感知研究提供了重要的数据基础。
...
### [190] [Closing the Reality Gap: Zero-Shot Sim-to-Real Deployment for Dexterous Force-Based Grasping and Manipulation](https://arxiv.org/abs/2601.02778)
*Haoyu Dong,Zhengmao He,Yang Li,Zhibin Li,Xinyu Yi,Zhe Zhao*

Main category: cs.RO

TL;DR: 提出了一种结合密集触觉反馈和关节力矩传感的实用仿真到现实强化学习框架，实现了多指灵巧手的零样本部署，展示了可控抓握和物体重定向能力。


<details>
  <summary>Details</summary>
Motivation: 多指灵巧手虽具备人类级操作能力，但由于接触物理复杂和执行器不完美，直接在真实硬件上训练控制策略困难。

Method: 采用仿真到现实强化学习框架，包括：(1) 快速触觉仿真计算虚拟触觉单元与物体的距离；(2) 电流-力矩标定替代力矩传感器；(3) 执行器动力学建模与非理想效应随机化。使用非对称actor-critic PPO在仿真中训练。

Result: 策略可直接部署到五指手上，实现了：(1) 基于指令的可控抓握力跟踪；(2) 手中物体重定向。两项技能均无需在机器人上微调即可鲁棒执行。

Conclusion: 通过触觉与力矩观测结合有效的传感/执行器建模，该系统为实现可靠的灵巧操作提供了实用解决方案，首次实现了完全在仿真中训练并零样本迁移到真实硬件的多指灵巧手可控抓握。
...
### [191] [Reinforcement Learning for Follow-the-Leader Robotic Endoscopic Navigation via Synthetic Data](https://arxiv.org/abs/2601.02798)
*Sicong Gao,Chen Qian,Laurence Xian,Liao Wu,Maurice Pagnucco,Yang Song*

Main category: cs.RO

TL;DR: 提出了一种基于柔性连续结构的跟随式内窥镜机器人，采用视觉深度强化学习框架实现肠道自主导航，减少与肠壁接触


<details>
  <summary>Details</summary>
Motivation: 医疗和工业内窥镜机器人需要自主导航以安全探索狭窄管状环境，但避免与内壁接触一直是长期挑战，现有方法难以解决

Method: 1. 构建柔性连续结构机器人；2. 提出基于单目深度估计的视觉深度强化学习框架；3. 在NVIDIA Omniverse中创建真实肠道仿真环境；4. 使用NVIDIA Replicator生成合成图像微调Depth Anything模型；5. 引入几何感知奖励惩罚机制实现精确管腔跟踪

Result: 1. 深度准确度δ₁提升39.2%；2. 导航J指数相比次优方法降低0.67；3. 实现了密集三维感知和鲁棒的自主导航

Conclusion: 该方法通过深度学习和仿真训练，显著提升了内窥镜机器人的自主导航能力，有效减少与肠壁接触，为医疗内窥镜检查提供了更安全舒适的解决方案
...
### [192] [Soft Responsive Materials Enhance Humanoid Safety](https://arxiv.org/abs/2601.02857)
...
TL;DR: 提出了一种软硬协同设计框架，利用非牛顿流体软响应材料增强人形机器人安全性，通过材料在冲击下快速硬化吸收能量，结合仿真引导保护器设计和主动跌倒策略学习，显著降低碰撞峰值力。


<details>
  <summary>Details</summary>
Motivation: 人形机器人在人类中心环境中易跌倒，其刚性金属塑料结构对人员和环境构成安全风险，限制了实际部署。

Method: 采用非牛顿流体基软响应材料（正常时柔顺，冲击时快速硬化），通过物理仿真优化保护器布局与厚度，并学习主动跌倒控制策略。

Result: 在42公斤全尺寸人形机器人上验证，保护器显著降低碰撞峰值力，实现从3米跌落及长楼梯翻滚等重复跌落无硬件损坏，提升了机器人鲁棒性与环境安全性。

Conclusion: 通过响应材料、结构协同设计和学习控制的结合，推动了交互安全、可工业应用的人形机器人发展。
...
### [193] [Warm-Starting Collision-Free Model Predictive Control With Object-Centric Diffusion](https://arxiv.org/abs/2601.02873)
...
TL;DR: 提出了一种结合扩散模型和模型预测控制的混合方法，用于在复杂环境中生成可靠且高效的机器人运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统优化控制器难以在存在多个障碍物时快速生成可行解，而扩散模型缺乏对场景结构进行高效条件化的通用方法。

Method: 使用基于对象中心的槽注意力机制表示场景，通过扩散变换器生成初始轨迹，再用考虑碰撞的模型预测控制器进行细化，确保动力学和碰撞约束。

Result: 在基准任务中，该方法比基于采样的规划器或单一组件具有更高的成功率和更低的延迟，真实机器人实验验证了其可靠性和安全性。

Conclusion: 结合扩散模型的条件化生成与模型预测控制的优化，能够在严格时间限制下实现高效、可靠的机器人运动规划。
...
### [194] [Dual-quaternion learning control for autonomous vehicle trajectory tracking with safety guarantees](https://arxiv.org/abs/2601.03097)
*Omayra Yago Nieto,Alexandre Anahory Simoes,Juan I. Giribet,Leonardo Colombo*

Main category: cs.RO

TL;DR: 提出一种基于学习的轨迹跟踪控制器，用于SE(3)运动描述的自主机器人平台，结合对偶四元数框架与高斯过程回归，在线学习补偿未知扰动，保证跟踪误差的稳定性。


<details>
  <summary>Details</summary>
Motivation: 自主机器人（如无人机、全向移动机器人）在运动控制中常受未知扰动（传感器干扰、未建模的驱动耦合、环境不确定性）影响，传统方法依赖参数化模型，难以适应非结构化扰动。

Method: 1. 在对偶四元数框架下设计几何反馈控制律；2. 集成高斯过程回归在线学习状态相关的未知扰动；3. 基于李雅普诺夫分析证明概率最终有界稳定性。

Result: 仿真结果表明，在存在局部化扰动（如磁力计扰动引起的旋转与平移耦合效应）时，控制器能实现精确平滑的轨迹跟踪，且具有数据效率高的优点。

Conclusion: 结合几何建模与概率学习的方法，能为自主机器人系统提供鲁棒、数据高效的位姿控制，无需依赖扰动参数模型，适用于实际复杂环境。
...
### [195] [LOST-3DSG: Lightweight Open-Vocabulary 3D Scene Graphs with Semantic Tracking in Dynamic Environments](https://arxiv.org/abs/2601.02905)
...
TL;DR: 提出LOST-3DSG，一种轻量级开放词汇3D场景图方法，用于动态环境中的物体跟踪，避免依赖重型基础模型


<details>
  <summary>Details</summary>
Motivation: 现有动态物体跟踪方法因依赖重型基础模型而效率低下，需要更轻量高效的解决方案

Method: 采用基于word2vec和句子嵌入的语义实体跟踪方法，构建开放词汇3D场景图，避免存储密集CLIP视觉特征

Result: 在TIAGo机器人真实3D环境实验中，LOST-3DSG相比依赖高维视觉嵌入的方法表现出更优的性能和效率

Conclusion: LOST-3DSG能有效高效地跟踪动态环境中的物体，代码和补充材料已公开
...
### [196] [Parameter-Robust MPPI for Safe Online Learning of Unknown Parameters](https://arxiv.org/abs/2601.02948)
*Matti Vahs,Jaeyoun Choi,Niklas Schmid,Jana Tumova,Chuchu Fan*

Main category: cs.RO

TL;DR: 提出PRMPPI控制框架，结合在线参数学习和概率安全约束，使机器人在动态不确定环境中保持安全并提升性能。


<details>
  <summary>Details</summary>
Motivation: 机器人在动态环境中运行时，关键物理参数可能不确定或随时间变化，需确保其始终安全。

Method: 通过Stein变分梯度下降维护参数的粒子信念，使用Conformal Prediction评估安全约束，并行优化性能驱动和安全备份轨迹。

Result: 仿真和硬件实验显示，相比基线方法，该方法成功率更高、跟踪误差更低、参数估计更准确。

Conclusion: PRMPPI控制器能实现从谨慎到高性能的平滑过渡，并在整个过程中确保安全。
...
### [197] [Learning to Act Robustly with View-Invariant Latent Actions](https://arxiv.org/abs/2601.02994)
*Youngjoon Jeong,Junha Chun,Taesup Kim*

Main category: cs.RO

TL;DR: 提出VILA方法，通过建模跨轨迹的潜在动作来学习基于物理动力学的视角不变表示，提升视觉策略的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视觉机器人策略对视角变化敏感，现有方法依赖场景级多视角观测，但未结合物理动力学，导致真实场景泛化能力不足。

Method: 提出VILA框架，建模捕获轨迹间转移模式的潜在动作，使用基于真实动作序列的动作引导目标对齐不同视角的潜在动作。

Result: 仿真和真实实验表明，基于VILA的策略能有效泛化到未见视角，并迁移到新任务，提升了鲁棒性和下游学习性能。

Conclusion: VILA是一种有效的预训练框架，通过物理动力学驱动的视角不变表示学习，增强了视觉策略的泛化能力和任务迁移性。
...
### [198] [A Bi-directional Adaptive Framework for Agile UAV Landing](https://arxiv.org/abs/2601.03037)
*Chunhui Zhao,Xirui Kao,Yilin Lu,Yang Lyu*

Main category: cs.RO

TL;DR: 提出一种双向协作降落框架，将移动平台从被动目标转变为主动参与者，通过平台主动倾斜表面为四旋翼创造最佳终端姿态，实现对齐与下降阶段的并行化，显著提升动态场景下的降落效率、精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统四旋翼在移动平台上的自主降落方法采用'跟踪-下降'范式，将平台视为被动目标，导致在高度动态场景中效率低下、机动复杂。

Method: 1. 将问题从单智能体跟踪重构为耦合系统优化；2. 平台主动倾斜表面以优化四旋翼终端姿态；3. 四旋翼规划时间最优、动态可行的轨迹以最小化能耗；4. 双向协调实现对齐与下降的并行执行。

Result: 该框架在动态场景中验证有效，通过激进轨迹跟踪和瞬态窗口内的快速状态同步，显著提升了复杂任务中自主降落的效率、精度和鲁棒性。

Conclusion: 双向协作框架突破了传统顺序降落范式的限制，通过平台与飞行器的主动协同，实现了更敏捷、高效的动态平台降落能力。
...
### [199] [Validating Generalist Robots with Situation Calculus and STL Falsification](https://arxiv.org/abs/2601.03038)
*Changwen Li,Rongjie Yan,Chih-Hong Cheng,Jian Zhang*

Main category: cs.RO

TL;DR: 提出一个结合抽象推理与具体系统证伪的两层验证框架，用于验证通用机器人的任务执行能力，并在桌面操作任务中成功发现NVIDIA GR00T控制器的故障案例。


<details>
  <summary>Details</summary>
Motivation: 通用机器人能够解释自然语言指令并执行多样化操作，但传统验证方法无法适应每个任务特有的操作环境和正确性规范，因此需要新的验证方法。

Method: 采用两层验证框架：抽象层使用情境演算对世界建模并推导最弱前置条件，通过约束感知的组合测试生成多样化、语义有效的世界-任务配置；具体层将这些配置实例化，通过STL监控进行基于仿真的证伪。

Result: 在桌面操作任务实验中，该框架有效发现了NVIDIA GR00T控制器的故障案例，证明了其在验证通用机器人自主性方面的潜力。

Conclusion: 所提出的验证框架能够系统性地生成多样化测试配置并有效发现故障，为验证通用机器人提供了有前景的方法。
...
### [200] [PiDR: Physics-Informed Inertial Dead Reckoning for Autonomous Platforms](https://arxiv.org/abs/2601.03040)
*Arup Kumar Sahoo,Itzik Klein*

Main category: cs.RO

TL;DR: 提出PiDR框架，通过物理信息增强的深度学习模型解决纯惯性导航中的漂移问题，在机器人和水下自主航行器上实现超过29%的定位精度提升。


<details>
  <summary>Details</summary>
Motivation: 完全自主系统需在无GNSS或视觉信息等外部数据时维持精确导航，但纯惯性导航因传感器噪声和误差随时间漂移，传统深度学习模型存在黑箱性、数据需求大且难以保持物理原理的问题。

Method: 提出物理信息惯性航位推算框架PiDR，将惯性导航原理通过物理信息残差组件显式整合到网络训练中，减少对监督数据的依赖，抑制轨迹突变。

Result: 在移动机器人和自主水下航行器的真实数据集上测试，定位精度提升超过29%，证明其在不同平台和环境中的泛化能力。

Conclusion: PiDR提供轻量、鲁棒且有效的架构，可部署于资源受限平台，实现在恶劣场景下的实时纯惯性导航。
...
### [201] [SOP: A Scalable Online Post-Training System for Vision-Language-Action Models](https://arxiv.org/abs/2601.03044)
...
TL;DR: 提出SOP系统，实现视觉-语言-动作模型在物理世界中的在线、分布式、多任务后训练，通过机器人集群实时交互提升任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型后训练方法多为离线、单机器人或任务特定，限制了在真实场景中的在线策略适应和规模化交互学习能力。

Method: 设计闭环架构的SOP系统：机器人集群持续上传交互数据和人机干预信号至云端学习器，异步接收更新策略；采用HG-DAgger（交互模仿学习）和RECAP（强化学习）两种算法实例化。

Result: 在布料折叠、箱子组装、商品补货等现实操作任务中，SOP显著提升预训练VLA模型性能，保持跨任务共享策略；数小时真实交互即可有效后训练，性能随机器人数量近线性增长。

Conclusion: 在线学习与集群部署的紧密耦合是实现通用机器人策略高效、可靠、规模化后训练的关键。
...
### [202] [A Fast Semidefinite Convex Relaxation for Optimal Control Problems With Spatio-Temporal Constraints](https://arxiv.org/abs/2601.03055)
*Shiying Dong,Zhipeng Shen,Rudolf Reiter,Hailong Huang,Bingzhao Gao,Hong Chen,Wen-Hua Chen*

Main category: cs.RO

TL;DR: 提出了一种时间缩放直接多重打靶方案和基于半定规划的凸松弛方法，用于高效求解具有时空约束的最优控制问题，在四旋翼飞行任务中验证了其有效性和实用性。


<details>
  <summary>Details</summary>
Motivation: 自主智能体在时空约束下的最优控制问题（如自动驾驶节能驾驶、四旋翼导航）通常因动力学与事件时序的耦合导致非凸性，现有方法（如预定义航点时间或非凸轨迹优化）往往计算复杂且解质量有限，需要更高效、更优的求解方法。

Method: 1. 提出时间缩放直接多重打靶方案，将预测时域划分为与特征时间约束对齐的段；2. 开发基于半定规划的快速凸松弛方法，利用提升公式的稀疏性。

Result: 仿真研究表明该方法在解的最优性和计算效率上均有显著提升；四旋翼航点飞行任务（含受限开放时间窗口）的真实实验验证了其在复杂环境中的实际适用性。

Conclusion: 所提方法通过时间缩放和凸松弛技术，有效解决了时空约束最优控制问题的非凸挑战，实现了更优解和更高计算效率，并具备实际应用潜力。
...
### [203] [HEXAR: a Hierarchical Explainability Architecture for Robots](https://arxiv.org/abs/2601.03070)
*Tamlin Love,Ferran Gebellí,Pradip Pramanick,Antonio Andriella,Guillem Alenyà,Anais Garrell,Raquel Ros,Silvia Rossi*

Main category: cs.RO

TL;DR: 提出HEXAR分层可解释性框架，通过模块化解释器和选择器为机器人系统提供层次化解释，在家庭辅助机器人任务中优于端到端和聚合基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有机器人可解释性方法存在局限：要么关注独立模块（难以从高层行为角度查询），要么采用整体方法（未利用机器人架构的模块化特性），需要更灵活有效的解释框架。

Method: 设计HEXAR分层框架，包含针对特定机器人模块的专用组件解释器（使用LLM推理、因果模型、特征重要性等技术），由解释器选择器根据查询动态选择最合适的解释器。

Result: 在TIAGo家庭辅助机器人180个场景-查询变体测试中，HEXAR在根本原因识别、错误信息排除和运行时间方面显著优于端到端和聚合基线方法。

Conclusion: HEXAR为透明自主系统提供了有前景的方向，其分层模块化设计能有效生成机器人系统解释，平衡了模块特异性和整体行为可查询性。
...
### [204] [A High-Fidelity Digital Twin for Robotic Manipulation Based on 3D Gaussian Splatting](https://arxiv.org/abs/2601.03200)
*Ziyang Sun,Lingfan Bao,Tianhu Peng,Jingcheng Sun,Chengxu Zhou*

Main category: cs.RO

TL;DR: 提出一个快速构建高质量数字孪生的框架，使用3D高斯泼溅实现快速逼真重建，并通过语义融合和几何转换支持机器人操作任务。


<details>
  <summary>Details</summary>
Motivation: 现有数字孪生方法存在重建速度慢、视觉保真度有限、难以将逼真模型转换为规划可用碰撞几何的问题，阻碍了仿真到现实的迁移。

Method: 采用3D高斯泼溅（3DGS）作为统一场景表示进行快速逼真重建；通过可见性感知语义融合实现精确3D标注；设计基于滤波的几何转换方法生成碰撞就绪模型；集成Unity-ROS2-MoveIt物理引擎。

Result: 在Franka Emika Panda机器人执行拾放任务的实验中，增强的几何精度有效支持了真实世界中的鲁棒操作。

Conclusion: 基于3DGS、富含语义和几何一致性的数字孪生，为无结构环境中从感知到操作提供了快速、可靠且可扩展的路径。
...
### [205] [AMC26: VSSEA robust position control](https://arxiv.org/abs/2601.02557)
*Emre Sariyildiz*

Main category: eess.SY

TL;DR: 本文针对新型VSSEA提出了鲁棒位置控制策略，通过状态空间模型开发了两种控制方案：状态反馈控制器和滑模控制器，均集成了二阶扰动观测器，实现了高性能运动控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于极点配置的控制器对扰动高度敏感，而LQR控制器虽提高鲁棒性但动态响应较慢，需要一种既能精确补偿内外扰动、又不牺牲动态响应的控制方法。

Method: 1. 构建状态空间模型；2. 在统一框架下开发状态反馈控制器和滑模控制器；3. 集成二阶扰动观测器进行扰动估计与补偿；4. 通过仿真比较极点配置与LQR控制器的性能；5. 进行实验验证。

Result: 1. 集成DOb后鲁棒性显著增强且不降低时间响应；2. LQR控制器可仅针对性能优化进行调参；3. 实验证实所提控制器可在实际应用中实现；4. 仿真显示极点配置控制器对扰动敏感，LQR控制器鲁棒性更好但动态较慢。

Conclusion: 所提鲁棒位置控制框架能有效补偿扰动并保持标称动态响应，为不同刚度设置下的鲁棒稳定性与性能研究奠定了基础。
...
### [206] [AMC26: High-performance DOb for robust position control](https://arxiv.org/abs/2601.02560)
*Emre Sariyildiz*

Main category: eess.SY

TL;DR: 本文提出了一种新型高阶扰动观测器（HPDOb），通过引入一阶截断误差动态，显著提升了运动控制系统的扰动估计精度和鲁棒性，优于传统扰动观测器。


<details>
  <summary>Details</summary>
Motivation: 传统扰动观测器（DObs）仅考虑零阶截断误差，限制了扰动估计的精度和鲁棒性，尤其在运动控制系统中对高频扰动的抑制能力不足。

Method: 在离散时域中分析与综合HPDOb，提出一种新的合成方法，将高阶截断误差动态纳入扰动估计，实现一阶截断误差，并进行了仿真与实验验证。

Result: HPDOb在扰动估计精度和鲁棒性上显著优于传统DObs，仿真和实验验证了其稳定性和性能提升。

Conclusion: HPDOb通过高阶截断误差动态的引入，为运动控制系统提供了更精确、更鲁棒的扰动估计方法，具有实际应用价值。
...
### [207] [O-DSS: An Open Dynamic Spectrum Sharing Framework for Cellular-Radar Coexistence in Mid-band Frequencies](https://arxiv.org/abs/2601.02571)
*Azuka Chiejina,Divyadharshini Muruganandham,Vini Chaudhary,Kaushik Chowdhury,Vijay K. Shah*

Main category: eess.SY

TL;DR: 提出O-DSS框架，基于O-RAN和机器学习实现蜂窝网络与雷达系统的实时动态频谱共享，显著降低延迟并提升性能。


<details>
  <summary>Details</summary>
Motivation: 中频频谱需求增长，现有频谱接入系统依赖固定环境感知传感器，存在延迟高、灵活性差的问题，无法有效支持蜂窝网络与移动雷达的共存。

Method: 采用O-RAN兼容的机器学习框架，集成低开销关键性能指标进行雷达检测，结合频谱图定位技术，实现精细化的无线接入网络控制（如资源块静默和雷达感知的调制编码方案调整）。

Result: O-DSS作为模块化xApp部署，实现60毫秒检测和700毫秒撤离延迟，优于现有基线；在仿真和空中测试中，雷达检测率≥99%（信干噪比≥-4dB），定位召回率≥95%（信干噪比≥8dB）。

Conclusion: O-DSS框架能有效保护现有雷达系统，同时保持蜂窝网络性能，为动态频谱共享提供了低延迟、高灵活性的解决方案。
...
### [208] [AI Social Responsibility as Reachability: Execution-Level Semantics for the Social Responsibility Stack](https://arxiv.org/abs/2601.02585)
*Otman Basir*

Main category: eess.SY

TL;DR: 本文提出将社会责任感栈（SRS）的核心要求形式化为系统执行的可达性属性，并引入Petri网作为责任自主系统的执行级形式化工具，将责任内化为结构不变量而非外部目标。


<details>
  <summary>Details</summary>
Motivation: 人工智能系统日益成为网络物理、社会和制度过程中持续、闭环的组成部分，其通过重复、并发和反馈产生的累积执行轨迹可能导致社会不可接受的结果，而非单一故障或政策违规。

Method: 采用Petri网作为执行级形式化工具，将SRS价值承诺映射为禁止标识，安全保障对应转移发射的结构约束，审计对应可达性压力监控，治理对应执行结构的合法修改。

Result: 建立了社会责任感栈作为可执行的责任架构，并将基于可达性的执行语义定位为在反馈丰富的网络物理和社会技术系统中实现责任自主的必要基础。

Conclusion: 责任本质上是系统执行的可达性属性，责任失败是轨迹控制的执行级失败而非目标级错误；通过Petri网可达性嵌入SRS架构，可将责任内化为结构不变量。
...
### [209] [Topology-Aware Spatio-Temporal Graph Transformer for Predicting Smart Grid Failures](https://arxiv.org/abs/2601.02701)
*Anh Le,Phat K. Huynh,Om P. Yadav,Harun Pirim,Chau Le,Trung Q. Le*

Main category: eess.SY

TL;DR: 提出一种拓扑感知的时空图变换器（ST-GT）架构，用于智能电网故障预测，通过融合物理拓扑与时间序列数据显著提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在故障预测中难以准确表征时空因果依赖关系，且存在类别不平衡问题，无法满足智能电网对高可靠性维护的需求。

Method: 采用图变换器架构，创新性地将物理输电网络拓扑直接融入注意力机制，统一处理静态拓扑描述符与动态PMU时间序列，实现端到端训练。

Result: 在10个变电站的五折交叉验证中，模型召回率达1.000±0.001，F1分数0.858±0.009，显著优于XGBoost基线（0.683），确保无关键故障漏报。

Conclusion: 该框架为关键基础设施监测提供了可解释的故障传播路径分析，未来将聚焦损失函数优化、实时监测系统开发及成本敏感框架构建。
...
### [210] [Hierarchical Preemptive Holistic Collaborative Systems for Embodied Multi-Agent Systems: Framework, Hybrid Stability, and Scalability Analysis](https://arxiv.org/abs/2601.02779)
*Ting Peng*

Main category: eess.SY

TL;DR: 提出分层抢占式整体协作框架，解决多智能体在物理受限环境中的协调问题，通过子空间优化、三阶段滚动机制和空闲缓冲区协议确保安全与效率。


<details>
  <summary>Details</summary>
Motivation: 传统分散式方法缺乏意图感知易陷入局部最优，集中式规划存在计算复杂性和单点故障问题，需平衡安全性、可扩展性和效率。

Method: 采用分层抢占式整体协作框架，将全局协调分解为拓扑连通的子空间优化；形式化为混合自动机；设计三阶段滚动机制（冻结执行、初步规划、前瞻窗口）和空闲缓冲区协议；引入影子智能体协议保证轨迹连续性。

Result: 框架通过子空间分解降低计算复杂度，空闲缓冲区消除Zeno行为，影子协议确保子空间边界轨迹的输入到状态稳定性，提升系统鲁棒性。

Conclusion: 所提框架有效协调多智能体系统，兼顾安全性、可扩展性与计算效率，为物理受限环境中的协同任务提供可靠解决方案。
...
### [211] [Distributionally Robust Game for Proof-of-Work Blockchain Mining Under Resource Uncertainties](https://arxiv.org/abs/2601.02804)
*Xunqiang Lan,Xiao Tang,Ruonan Zhang,Bin Li,Qinghe Du,Dusit Niyato,Zhu Han*

Main category: eess.SY

TL;DR: 本文提出了一种考虑资源不确定性的工作量证明区块链挖矿博弈模型，采用分布鲁棒方法处理矿工计算能力的不确定性，证明了均衡存在性并设计了求解算法。


<details>
  <summary>Details</summary>
Motivation: 随着PoW区块链的广泛应用，矿工计算能力呈现多样化且存在不确定性，传统模型难以准确刻画这种资源分布的不确定性对挖矿博弈的影响。

Method: 建立矿工资源分布的模糊集合描述不确定性；构建非合作博弈模型，采用分布鲁棒性能指标；提出基于条件风险价值的最佳响应重构方法；使用交替优化求解个体策略；针对高斯分布等特例设计分布式算法。

Result: 证明了分布鲁棒挖矿博弈均衡的存在性；所提算法能有效收敛至均衡；仿真表明方法能有效处理不确定性并实现鲁棒性能保证。

Conclusion: 提出的分布鲁棒博弈框架能有效建模资源不确定的区块链挖矿问题，均衡存在性得到保证，求解算法具有收敛性，为不确定环境下的挖矿策略设计提供了理论工具。
...
### [212] [A Mathematical Formalization of Self-Determining Agency](https://arxiv.org/abs/2601.02885)
*Yoshiyuki Ohmura,Earnest Kota Carr,Yasuo Kuniyoshi*

Main category: eess.SY

TL;DR: 本文提出了一种基于随附因果的物理模型，用于解释智能体层面的决定如何在不违反物理定律的情况下产生作用，为建模具有道德责任的自决智能体提供了理论基础。


<details>
  <summary>Details</summary>
Motivation: 现有物理描述与智能体行为之间存在解释鸿沟，讨论智能体的道德与责任需要建立行为模型，但现有物理决定论难以完全解释负责任的行为。

Method: 通过形式化随附因果关系，构建多层级随附函数代数表达式，证明序列索引可不随附于底层基态，从而允许高层级具有独立于底层物理规律的动力学规律。

Result: 建立了随附因果的严格数学形式化，表明高层级变化可时间优先地引致底层变化，这种双定律系统为建模自决智能体提供了可能。

Conclusion: 提出的随附因果物理模型为连接机械物理过程与智能体行为提供了理论框架，有助于在认知科学与人工智能中定义具有道德责任的智能体。
...
### [213] [Site-Specific and Frequency-Dependent Channel Characterization and MIMO Performance in FR3](https://arxiv.org/abs/2601.02903)
*Zhuangzhuang Cui,Rudranil Chattopadhyay,Emiel Vanspranghels,Sofie Pollin*

Main category: eess.SY

TL;DR: 本文研究了7-24 GHz（FR3频段）的传播特性和MIMO性能，通过射线追踪仿真发现FR3在传播行为和空间复用方面介于FR1和FR2之间，且性能提升依赖于天线阵列规模。


<details>
  <summary>Details</summary>
Motivation: 下一代无线系统需要动态频谱利用来实现按需连接，因此需要研究介于sub-6 GHz和毫米波之间的FR3频段的传播特性，以评估其在实际部署中的潜力。

Method: 使用基于Sionna框架的站点特定射线追踪（RT）仿真，在室内外环境中对FR1、FR3和FR2的代表性频率（3.5、7、10、14、20、24、28 GHz）进行单天线和多天线配置分析。

Result: FR3表现出介于sub-6 GHz和毫米波频段之间的中间传播特性，同时能维持有效的空间复用和良好的频谱效率；大规模阵列分析表明FR3的性能增益与天线缩放密切相关。

Conclusion: FR3频段在传播和MIMO性能上具有过渡优势，但实际部署需要依赖大规模或大孔径MIMO架构来实现性能增益。
...
### [214] [Post-Earthquake Restoration of Electricity-Gas Distribution Systems with Damage Information Collection and Repair Vehicle Routing](https://arxiv.org/abs/2601.02958)
*Mingxuan Li,Wei Wei,Yin Xu,Chengeng Zhang,Shanshan Shi*

Main category: eess.SY

TL;DR: 提出了一种考虑动态损伤评估和修复的集成电-气配电网自适应恢复框架，采用部分可观测马尔可夫决策过程建模，并引入信念树搜索算法实现实时决策，显著降低了停电成本。


<details>
  <summary>Details</summary>
Motivation: 地震等极端事件会对集成电-气配电网造成广泛破坏，现有恢复方法通常假设完全知晓损伤情况，但在监测和通信基础设施受损时无法满足，需要进行现场检查。

Method: 将恢复问题建模为部分可观测马尔可夫决策过程，引入先进的信念树搜索算法，通过综合模拟评估未来潜在轨迹，确定最优检查和修复策略，并开发了统一的实时决策框架。

Result: 在两个不同的集成电-气配电网系统中进行案例研究，结果表明该方法达到与理想解决方案相当的停电成本，相比基于随机规划和启发式方法的策略，总停电成本降低超过15%。

Conclusion: 所提出的自适应恢复框架能有效处理损伤信息不完全的情况，通过实时决策优化现场人员行动，显著提升集成电-气配电网的恢复效率和韧性，具有良好的可扩展性。
...
### [215] [Closed-Loop Transmission Power Control for Reliable and Low-Power BLE Communication in Dynamic IoT Settings](https://arxiv.org/abs/2601.03003)
*Ziyao Zhou,Hen-Wei Huang*

Main category: eess.SY

TL;DR: 提出了一种基于PID控制器的混合RSSI-吞吐量BLE传输功率控制框架，在动态环境中保持稳定吞吐量。


<details>
  <summary>Details</summary>
Motivation: BLE通信在动态环境中RSSI和吞吐量易受环境影响，导致通信性能下降，需要可靠且节能的功率控制方案。

Method: 系统分析RSSI、吞吐量、传输功率和功耗的关联性，提出基于PID控制器的闭环功率控制框架，比较了RSSI基、吞吐量基及混合控制策略。

Result: 混合策略在环境快速变化时能将吞吐量维持在目标水平且方差最小，优于单一策略。

Conclusion: 混合RSSI-吞吐量控制策略结合了响应速度与准确性，有效提升了BLE在动态环境中的通信可靠性。
...
### [216] [From inconsistency to decision: explainable operation and maintenance of battery energy storage systems](https://arxiv.org/abs/2601.03007)
*Jingbo Qu,Yijie Wang,Yujie Fu,Putai Zhang,Weihan Li,Mian Li*

Main category: eess.SY

TL;DR: 本文提出了一种基于不一致性驱动的电池储能系统运维范式，通过多维不一致性评估与大型语言模型语义推理相结合，将监测数据转化为可解释的决策指导，显著提升运维效率。


<details>
  <summary>Details</summary>
Motivation: 当前电池储能系统的运维主要依赖反应式、专家依赖的诊断方法，缺乏可扩展且可解释的决策支持框架，导致电池不一致性等早期预警信号难以有效转化为运维行动。

Method: 提出集成多维不一致性评估（电气、热、老化）与基于大型语言模型的语义推理框架，通过多智能体系统将结构化运行记录转化为可操作的维护建议。基于包含3,564个电池的现场八个月数据进行验证。

Result: 该方法能够对实际运维查询提供准确且可解释的响应，与传统专家驱动方法相比，响应时间和运维成本降低超过80%。

Conclusion: 该研究为电池储能系统的智能运维提供了可扩展路径，对提升电力系统可靠性、安全性和储能经济性集成具有直接意义。
...
### [217] [Finite Memory Belief Approximation for Optimal Control in Partially Observable Markov Decision Processes](https://arxiv.org/abs/2601.03132)
*Mintae Kim*

Main category: eess.SY

TL;DR: 本文研究了部分可观测随机最优控制问题中的有限记忆信念近似方法，提出了基于Wasserstein度量的性能边界理论，量化了有限记忆导致的信息损失对控制性能的影响，并在LQG系统中进行了验证。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测马尔可夫决策过程中，信念状态虽然是充分的，但通常是无限维且不实用的。因此需要研究有限记忆近似方法，并量化这种近似对控制性能的影响。

Method: 将截断的输入输出历史解释为信念近似，基于Wasserstein度量建立理论框架，推导策略条件性能边界，并通过固定策略比较方法分析信念替换的影响。在线性二次高斯系统中提供闭式信念失配评估。

Result: 理论分析表明信念失配随记忆长度近似指数衰减，导致的性能失配也相应缩放。实验验证了预测机制，展示了有限记忆信念近似的可行性和局限性。

Conclusion: 研究提供了度量感知的有限记忆信念近似能力表征，明确了在部分可观测设置中该方法能实现和不能实现的目标，为实际应用提供了理论指导。
...
### [218] [Time-Varying Kinematics Control for Magnetically-Actuated Satellite Swarm without Additional Actuator](https://arxiv.org/abs/2601.03143)
*Yuta Takahashi,Hiraku Sakamoto,Shin-ichiro Sakai*

Main category: eess.SY

TL;DR: 本文研究了电磁编队飞行系统的可控性，设计了一种无需额外姿态执行器的多卫星控制器。


<details>
  <summary>Details</summary>
Motivation: 电磁编队飞行技术利用电磁力/力矩控制多卫星，无需传统推进剂，但系统角动量守恒构成非完整约束，影响可控性，需设计适应此约束的控制器。

Method: 基于角动量守恒分析系统可控性，针对非完整约束设计多卫星控制器，避免使用额外姿态执行器。

Result: 实现了在角动量守恒约束下多卫星的相对位置与姿态控制，验证了控制器在非完整约束下的有效性。

Conclusion: 所提控制器能在无额外姿态执行器的情况下，克服非完整约束实现电磁编队飞行的可控性，为多卫星无燃料推进控制提供了可行方案。
...
### [219] [Conditioning Aircraft Trajectory Prediction on Meteorological Data with a Physics-Informed Machine Learning Approach](https://arxiv.org/abs/2601.03152)
*Amy Hodgkin,Nick Pepper,Marc Thomas*

Main category: eess.SY

TL;DR: 提出一种物理信息方法，通过从数据中学习飞机推力和空速来改进轨迹预测，结合BADA物理模型确保轨迹的物理合理性。


<details>
  <summary>Details</summary>
Motivation: 空中交通管理中的轨迹预测受气象条件和操作程序等不确定性影响，需要概率模型，但现有模型生成的轨迹可能缺乏物理合理性，影响可信度。

Method: 提出物理信息方法：从数据中学习飞机推力和空速，用于调节基于物理的BADA模型，确保轨迹满足能量约束；识别特征集并构建概率模型。

Result: 与忽略气象等上下文信息的基线概率模型相比，该方法在六项指标上平均提升20%的预测准确性。

Conclusion: 结合数据驱动学习和物理约束的方法能有效提升轨迹预测的准确性和物理合理性，增强模型可信度。
...
### [220] [Deep Learning Superresolution for 7T Knee MR Imaging: Impact on Image Quality and Diagnostic Performance](https://arxiv.org/abs/2601.02436)
*Pinzhen Chen,Libo Xu,Boyang Pan,Jing Li,Yuting Wang,Ran Xiong,Xiaoli Gou,Long Qing,Wenjing Hou,Nan-jie Gong,Wei Chen*

Main category: eess.IV

TL;DR: 本研究评估了深度学习超分辨率技术在7T膝关节MRI中的应用，发现SR图像在主观质量上优于低分辨率图像且噪声低于高分辨率图像，但在诊断准确性方面与标准低分辨率成像无显著差异。


<details>
  <summary>Details</summary>
Motivation: 深度学习超分辨率技术可能提升肌肉骨骼MRI图像质量，但其在7T膝关节成像中的诊断价值尚不明确，需要验证其临床实用性。

Method: 前瞻性研究，42名参与者接受7T膝关节MRI扫描，获取低分辨率和高分辨率序列，使用混合注意力Transformer模型从低分辨率数据生成超分辨率图像，由三位放射科医生评估图像质量和解剖结构可见性，并以关节镜检查作为10例病例的参考标准。

Result: SR图像整体质量评分高于LR（中位数5 vs 4），噪声低于HR（5 vs 4）；软骨、半月板和韧带的可见性在SR和HR中均优于LR；各类图像在关节内病变的检出率和诊断性能（敏感性、特异性、AUC）无显著差异。

Conclusion: 深度学习超分辨率技术改善了7T膝关节MRI的主观图像质量，但未能提高诊断准确性，与标准低分辨率成像相比无临床诊断优势。
...
### [221] [Expert-Guided Explainable Few-Shot Learning with Active Sample Selection for Medical Image Analysis](https://arxiv.org/abs/2601.02409)
*Longwei Wang,Ifrat Ikhtear Uddin,KC Santosh*

Main category: eess.IV

TL;DR: 提出EGxFSL和xGAL双框架，结合可解释性与小样本学习/主动学习，解决医学影像分析中数据稀缺和模型不可解释问题，在多个数据集上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 医学影像分析面临标注数据稀缺和模型不可解释两大挑战，现有小样本学习缺乏透明度，主动学习忽视样本可解释性，阻碍临床AI部署。

Method: EGxFSL：通过Grad-CAM Dice损失整合放射科医生定义的ROI空间监督，与原型分类联合优化；xGAL：基于预测不确定性和注意力错配迭代选择样本，形成可解释性指导的闭环训练框架。

Result: 在BraTS、VinDr-CXR、SIIM-COVID-19数据集上准确率分别达92%、76%、62%，均优于基线；xGAL在仅680样本下达到76%准确率（随机采样为57%）；Grad-CAM可视化显示模型聚焦诊断相关区域，并在乳腺超声上验证了跨模态泛化能力。

Conclusion: 将可解释性机制融入小样本学习和主动学习能有效提升模型性能与临床可信度，形成协同优化的闭环框架，为数据受限的医学影像分析提供透明化解决方案。
...
### [222] [Comparative Analysis of Binarization Methods For Medical Image Hashing On Odir Dataset](https://arxiv.org/abs/2601.02564)
*Nedim Muzoglu*

Main category: eess.IV

TL;DR: 本文在ODIR数据集上评估了四种二值化方法，发现SDH在仅使用32位编码时取得了最佳性能，mAP@100达到0.9184，优于其他方法，并在存储效率和准确性之间取得了良好平衡。


<details>
  <summary>Details</summary>
Motivation: 研究旨在比较不同二值化方法在医学图像检索中的性能，寻找在保持高检索精度的同时减少存储开销的实用方法，以支持医疗图像检索和设备库存管理。

Method: 使用深度特征嵌入，在ODIR数据集上评估了四种二值化方法：局部敏感哈希（LSH）、迭代量化（ITQ）、基于核的监督哈希（KSH）和监督离散哈希（SDH）。

Result: SDH表现最佳，仅用32位编码即获得0.9184的mAP@100，优于LSH、ITQ和KSH。与先前研究相比，SDH在显著减少位数的同时达到了接近最先进水平的检索精度。

Conclusion: SDH是测试方法中最有效的二值化方法，为医学图像检索提供了准确性、存储效率和计算效率之间的实用平衡，适用于医疗图像检索和库存管理应用。
...
### [223] [Annealed Langevin Posterior Sampling (ALPS): A Rapid Algorithm for Image Restoration with Multiscale Energy Models](https://arxiv.org/abs/2601.02594)
*Jyothi Rikhab Chand,Mathews Jacob*

Main category: eess.IV

TL;DR: 本文提出了一种快速蒸馏策略，将预训练扩散模型的优势转移到多尺度能量基模型（EBMs）中，并基于EBMs的组合性开发了退火朗文后验采样算法（ALPS），用于解决成像逆问题中的MAP、MMSE和不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 能量基模型（EBMs）因其可解释的能量景观和组合结构适合解决成像逆问题，但历史上存在计算成本高和训练不稳定的缺点。本文旨在克服这些缺点，利用扩散模型的优势提升EBMs的实用性和效率。

Method: 1. 提出快速蒸馏策略，将预训练扩散模型的知识转移到多尺度EBMs中；2. 基于EBMs的组合性，开发退火朗文后验采样算法（ALPS），用于处理静态后验分布；3. 在图像修复和MRI重建任务上进行实验验证。

Result: 实验表明，该方法在图像修复和MRI重建任务中，在准确性和效率上均匹配或超越了基于扩散模型的基线方法，同时支持MAP恢复。

Conclusion: 该框架为成像逆问题提供了可扩展且原理性的解决方案，具有在科学和临床环境中实际部署的潜力。
...
### [224] [Transform and Entropy Coding in AV2](https://arxiv.org/abs/2601.02712)
...
TL;DR: AV2是AV1的继任者，旨在显著提升压缩效率和主观质量，同时保持低复杂度编解码器操作。本文介绍了AV2的变换、量化和熵编码设计，包括多种新编码工具。


<details>
  <summary>Details</summary>
Motivation: 开发AV2的主要动机是在保持低复杂度编解码器操作的同时，显著提升视频压缩效率和主观质量，为视频应用提供更高质量的视频体验。

Method: 采用重新设计的变换核和数据驱动变换、扩展的变换分区、模式与系数相关的变换信号传输，以及多种新编码工具，包括帧内/帧间二次变换、网格编码量化、自适应变换编码、概率适应率调整、前向跳过编码、交叉色度分量变换、奇偶隐藏工具和改进的无损编码。

Result: AV2能够以显著降低的比特率提供最高质量的视频体验，实现了压缩效率和主观质量的显著提升。

Conclusion: AV2通过先进的变换、量化和熵编码设计，成功实现了压缩效率和视频质量的显著提升，同时保持了低复杂度操作，为视频应用提供了更优的解决方案。
...
### [225] [Lesion Segmentation in FDG-PET/CT Using Swin Transformer U-Net 3D: A Robust Deep Learning Framework](https://arxiv.org/abs/2601.02864)
*Shovini Guha,Dwaipayan Nandi*

Main category: eess.IV

TL;DR: 本文提出SwinUNet3D框架用于FDG-PET/CT扫描中的病灶分割，通过结合移位窗口自注意力和U-Net跳跃连接，在AutoPET III数据集上取得了优于传统3D U-Net的性能。


<details>
  <summary>Details</summary>
Motivation: 精准且自动化的PET/CT病灶分割对癌症诊断和治疗规划至关重要，现有方法在捕捉全局上下文和精细解剖细节方面存在局限，需要更高效的深度学习模型。

Method: 采用Swin Transformer与3D U-Net结合的架构（SwinUNet3D），利用移位窗口自注意力机制捕获全局上下文，并通过U-Net风格跳跃连接保留局部细节，在AutoPET III FDG数据集上进行训练与评估。

Result: SwinUNet3D在测试集上达到Dice分数0.88和IoU 0.78，显著优于基线3D U-Net（Dice 0.48，IoU 0.32），同时推理速度更快，能更准确检测小而不规则病灶并减少假阳性。

Conclusion: SwinUNet3D为PET/CT病灶分割提供了高效鲁棒的解决方案，为后续多示踪剂、多中心研究及与其他Transformer架构的对比奠定了基础，推动了Transformer模型在肿瘤影像工作流程中的集成。
...
### [226] [DiT-JSCC: Rethinking Deep JSCC with Diffusion Transformers and Semantic Representations](https://arxiv.org/abs/2601.03112)
*Kailin Tan,Jincheng Dai,Sixian Wang,Guo Lu,Shuo Shao,Kai Niu,Wenjun Zhang,Ping Zhang*

Main category: eess.IV

TL;DR: 提出DiT-JSCC框架，通过语义优先的双分支编码器与扩散Transformer解码器联合学习，解决生成式联合信源信道编码中语义一致性问题，并在极端信道条件下实现高效传输。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的生成式联合信源信道编码方法常因编码器缺乏显式语义判别力，导致生成图像虽视觉逼真但语义一致性不足，需解决编码器与生成式解码器之间的根本性不匹配问题。

Method: 设计语义-细节双分支编码器，与由粗到细的条件扩散Transformer解码器自然对齐；引入基于Kolmogorov复杂度的免训练自适应带宽分配策略以提升传输效率。

Result: 实验表明DiT-JSCC在极端信道条件下，于语义一致性与视觉质量上均优于现有联合信源信道编码方法，尤其在超低带宽与低信噪比场景中表现突出。

Conclusion: DiT-JSCC通过联合学习语义优先表示与生成式解码，重新定义了生成解码时代的信息价值概念，为生成式联合信源信道编码的未来研究提供了开源框架与方向。
