...
### [1] [Graph-Memoized Reasoning: Foundations Structured Workflow Reuse in Intelligent Systems](https://arxiv.org/abs/2511.15715)
*Yash Raj Singh*

Main category: cs.AI

TL;DR: 该论文提出了“图记忆化推理”框架，通过将LLM的推理工作流存储为图结构记忆并进行复用，以解决重复计算、提高效率和一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型（LLM）推理系统经常在不同任务中重复计算相似的推理步骤，导致计算资源浪费、推理延迟增加和可复现性受限。这凸显了对能够回忆和重用先前计算轨迹的持久推理机制的需求。

Method: 引入了“图记忆化推理”（Graph-Memoized Reasoning）这一正式框架，用于将推理工作流表示、存储和复用为图结构记忆。通过编码过去的决策图并基于结构和语义相似性进行检索，该方法实现了子图在新的推理任务中的组合复用。论文还提出了一个优化目标，旨在最小化总推理成本并正则化存储与生成工作流之间的一致性，从而为智能系统中的效率-一致性权衡提供了理论基础。

Result: 该框架能够实现子图的组合复用，为智能系统中的效率-一致性权衡提供了理论基础，并提出了一个与优化目标相符的概念性评估协议。

Conclusion: 该框架为可解释、成本高效和自我改进的推理架构奠定了基础，是大型智能体系统中实现持久记忆的重要一步。
...
### [2] [Majority Rules: LLM Ensemble is a Winning Approach for Content Categorization](https://arxiv.org/abs/2511.15714)
*Ariel Kamen,Yakov Kamen*

Main category: cs.AI

TL;DR: 本研究提出了一种集成大型语言模型（eLLM）框架，用于非结构化文本分类，显著提高了性能、鲁棒性和准确性，达到了接近人类专家的水平。


<details>
  <summary>Details</summary>
Motivation: 解决单个大型语言模型在文本分类中存在的常见弱点，包括不一致性、幻觉、类别膨胀和错误分类，以提升整体性能。

Method: 引入了一个集成大型语言模型（eLLM）框架，通过数学模型形式化了集体决策过程和聚合标准。在零样本条件下，使用IAB分层分类法，在一个包含8,660个样本的人工标注语料库上，评估了十个最先进的LLM。

Result: eLLM方法在F1分数上比最强的单一模型提高了高达65%。单个模型因语义压缩导致性能停滞，而eLLM则提高了鲁棒性和准确性，实现了接近人类专家水平的性能。

Conclusion: eLLM为基于分类法的文本分类提供了一个可扩展、可靠的解决方案，有望显著减少对人工专家标注的依赖。
...
### [3] [How Modality Shapes Perception and Reasoning: A Study of Error Propagation in ARC-AGI](https://arxiv.org/abs/2511.15717)
*Bo Wen,Chen Wang,Erhan Bilal*

Main category: cs.AI

TL;DR: 本研究探讨了在ARC-AGI任务中，不同模态（文本和图像）如何影响模型感知，发现结构化文本擅长稀疏特征，图像擅长2D形状，结合两者可显著提高执行准确性，而无需改变底层模型。


<details>
  <summary>Details</summary>
Motivation: 目前的指令优先系统缺乏对编码如何塑造模型感知以及如何区分指令错误和执行错误的原则性理解。研究者假设模态会造成感知瓶颈，影响网格特征的可靠感知。

Method: 研究使用了ARC-AGI和ARC-AGI-2任务，比较了九种文本和图像模态。通过加权集合不一致性度量和两阶段推理管道，将感知与推理分离。最终通过执行结果评估了模态的影响。

Result: 结果表明，结构化文本在稀疏特征上能提供精确坐标，图像能捕捉2D形状但对分辨率敏感。结合文本和图像模态能提高执行效果（约8个感知点；中位数相似度约0.20）。

Conclusion: 将表示与Transformer的归纳偏置对齐，并实现文本和图像之间的交叉验证，可以在不改变底层模型的情况下，产生更准确的指令和更可靠的执行。
...
TL;DR: MACIE是一个多智能体因果智能解释器框架，结合结构化因果模型、干预反事实和Shapley值，旨在为多智能体强化学习系统提供全面的可解释性，包括个体贡献、系统级涌现智能和可操作的自然语言解释。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习系统（MARL）在安全关键应用中被使用，因此理解智能体决策原因及集体行为方式至关重要。现有可解释AI方法在多智能体环境中表现不佳，难以将集体结果归因于个体、量化涌现行为或捕捉复杂交互。

Method: MACIE框架结合了结构化因果模型（SCMs）、干预反事实和Shapley值。它解决三个核心问题：1. 使用干预归因分数量化每个智能体的因果贡献。2. 通过协同度量分离集体效应和个体贡献，量化系统级涌现智能。3. 通过综合因果洞察的自然语言叙述提供可操作的解释。

Result: MACIE在合作、竞争和混合动机四种MARL场景中进行了评估。结果显示：准确的结果归因（平均phi_i为5.07，标准差小于0.05），在合作任务中检测到积极的涌现（协同指数高达0.461），以及高效的计算（在CPU上每个数据集0.79秒）。

Conclusion: MACIE独特地结合了因果严谨性、涌现量化和多智能体支持，同时保持了实时使用的实用性。这代表了迈向可解释、可信赖和负责任的多智能体AI的重要一步。
...
TL;DR: 本论文提出了一个多模态AI框架，结合文本和视觉数据分析来增强建筑工地安全，并通过案例研究评估了大型语言模型（LLMs）和视觉-语言模型（VLMs）在识别安全隐患方面的能力。


<details>
  <summary>Details</summary>
Motivation: 建筑工地事故数据以多种格式存在（书面报告、检查记录、现场图像），传统方法难以综合分析这些多模态数据来识别危险。

Method: 本研究提出了一个多模态AI框架，结合文本和图像分析来识别建筑工地安全隐患。通过两个案例研究进行评估：1. 利用GPT-4o和GPT-4o mini从28,000份OSHA事故报告中提取结构化洞察。2. 使用轻量级、开源的Molmo 7B和Qwen2 VL 2B模型，在ConstructionSite10k数据集上，通过自然语言提示评估其在规则级安全违规检测方面的性能。

Result: 第一个案例研究展示了LLMs从文本报告中提取结构化洞察的能力。第二个案例研究表明，尽管Molmo 7B和Qwen2 VL 2B模型规模较小，但在特定提示配置下，它们在规则级安全违规检测方面表现出具有竞争力的性能。

Conclusion: 低资源多模态系统在建筑工地进行规则感知安全监控是可行的，这为成本敏感的部署提供了可能性。
...
### [7] [Chain of Summaries: Summarization Through Iterative Questioning](https://arxiv.org/abs/2511.15719)
...
TL;DR: 本论文通过一致性引导的跨模态迁移，探索了不确定性鲁棒的多模态学习，以应对噪声数据、低质量标签和模态异质性带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 多模态学习系统常因数据噪声、低质量标签和异质模态特性面临巨大不确定性，这在人机交互（HCI）场景中尤为突出，因为数据质量、语义可靠性和标注一致性因用户和记录条件而异。

Method: 核心思想是利用跨模态语义一致性作为鲁棒表示学习的基础。通过将异质模态投影到共享潜在空间，缓解模态间隙并揭示结构关系，以支持不确定性估计和稳定特征学习。在此基础上，研究了在不依赖大型高质量标注的情况下，增强语义鲁棒性、提高数据效率并减少噪声和不完美监督影响的策略。

Result: 在多模态情感识别基准测试中，一致性引导的跨模态迁移显著提高了模型的稳定性、判别能力以及对噪声或不完整监督的鲁棒性。潜在空间分析进一步表明，该框架即使在挑战性条件下也能捕获可靠的跨模态结构。
...
TL;DR: 本研究提出一个双模块框架，结合检索增强生成（RAG）和多智能体系统（MAS），以提升大型语言模型（LLM）在生物力学课程中作为教育助手的表现，特别是在概念性问题和多步计算问题上。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用任务上表现出色，但在领域特定应用中因知识空白而效果不佳，且在需要多步推理和分析的复杂问题上性能下降。本研究旨在解决这些挑战，以增强本科生物力学课程的学习体验。

Method: 本研究构建了一个双模块框架：1) 应用检索增强生成（RAG）来提高LLM对概念性判断题回答的特异性和逻辑一致性；2) 建立一个多智能体系统（MAS）来解决涉及多步推理和代码执行的计算导向问题。研究评估了Qwen-1.0-32B、Qwen-2.5-32B和Llama-70B等LLM在一个包含100个判断题和需要公式推导与计算的问题的生物力学数据集上的表现。

Result: 结果表明，RAG显著增强了LLM在回答概念性问题时的性能和稳定性，优于普通模型。另一方面，由多个LLM构建的MAS展示了执行多步推理、推导方程、执行代码以及为计算任务生成可解释解决方案的能力。

Conclusion: 这些发现证明了应用RAG和MAS来增强LLM在工程课程中性能的潜力，为开发工程教育中的智能辅导提供了有前景的方向。
...
### [10] [Spatial Reasoning in Multimodal Large Language Models: A Survey of Tasks, Benchmarks and Methods](https://arxiv.org/abs/2511.15722)
...
TL;DR: 本文识别了AI供应链风险评估中的系统性空白，尤其是在AI关键应用领域，并提出了一种AI供应链实体分类法，以帮助利益相关者系统地评估和管理相关风险。


<details>
  <summary>Details</summary>
Motivation: 尽管AI社区已广泛关注算法偏见和模型幻觉等AI风险，但对于现代AI系统复杂供应链（包括数据源、预训练模型、服务等）所带来的风险，却缺乏系统性评估。当AI系统应用于食品供应、医疗、公用事业等关键领域时，这一空白尤为突出。

Method: 研究人员首先调查了当前AI风险评估与管理的现状，重点关注AI供应链以及AI系统行为和输出相关的风险。在此基础上，他们提出了一种专门用于AI供应链实体分类的分类法。

Result: 本文提出了一种用于AI供应链实体分类的分类法。该分类法旨在帮助利益相关者（特别是那些缺乏广泛AI专业知识的人）“考虑正确的问题”，并系统地盘点其组织AI系统中的依赖关系。

Conclusion: 该分类法弥补了当前AI治理与关键应用中AI风险可操作性评估和管理之间的差距，为利益相关者提供了识别和管理AI供应链风险的实用工具。
...
TL;DR: 本研究比较了基于规则的方法和大型语言模型（LLMs）从波兰语电子健康记录中提取医疗信息。结果显示，基于规则的方法在人口统计信息提取方面准确性更高，而LLMs在药物识别方面表现更优且适应性强。研究强调了准确性、标准化和计算成本之间的权衡，并提出了混合方法的建议。


<details>
  <summary>Details</summary>
Motivation: 从非结构化临床文本中提取结构化医疗信息，尤其是在资源稀缺的非英语环境中，仍然是一个开放的挑战。

Method: 本研究对低计算量的基于规则的自然语言处理方法和大型语言模型（LLMs）进行了比较分析，用于从波兰语儿童康复医院的电子健康记录中提取患者人口统计数据、临床发现和处方药物。研究还考察了文本标准化不足和翻译导致的信息损失的影响，并比较了LLMs在原始波兰语文本和翻译成英语的文本上的表现。

Result: 基于规则的方法在信息检索任务中提供了更高的准确性，尤其是在年龄和性别提取方面。LLMs提供了更大的适应性和可扩展性，在药物名称识别方面表现出色。翻译对LLMs的有效性有影响。

Conclusion: 研究结果强调了在医疗环境中部署自然语言处理时，准确性、标准化和计算成本之间的权衡。作者主张采用混合方法，结合基于规则系统的精确性和LLMs的适应性，为实际医院中更可靠、资源效率更高的临床自然语言处理提供了一条实用途径。
...
### [13] [Multi-Agent LLM Orchestration Achieves Deterministic, High-Quality Decision Support for Incident Response](https://arxiv.org/abs/2511.15755)
*Philip Drammeh*
...
### [14] [CorrectHDL: Agentic HDL Design with LLMs Leveraging High-Level Synthesis as Reference](https://arxiv.org/abs/2511.16395)
*Kangwei Xu,Grace Li Zhang,Ulf Schlichtmann,Bing Li*

Main category: cs.AI

TL;DR: 该论文提出了CorrectHDL框架，利用高层次综合（HLS）结果作为功能参考，纠正大型语言模型（LLM）生成的硬件描述语言（HDL）设计中的错误，从而在保持正确性的同时提高面积和功耗效率。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在硬件前端设计方面潜力巨大，但它们固有的“幻觉”倾向常导致生成的HDL设计中出现功能错误。

Method: CorrectHDL框架以C/C++程序为输入：首先，LLM生成HDL设计，并通过检索增强生成（RAG）机制修复语法错误。接着，通过将LLM生成电路的模拟行为与传统HLS工具生成的参考设计进行比较，迭代改进其功能正确性。HLS参考设计确保功能正确性，但可能在面积和功耗效率上欠佳。

Result: 实验结果表明，该框架生成的电路比传统HLS设计在面积和功耗效率上显著更优，并接近人工设计的质量。同时，保持了HDL实现的正确性。

Conclusion: CorrectHDL框架有效结合了LLMs的生成能力和传统IC设计流程的严谨性，实现了代理式HDL设计，证明了其在生成高质量、高效率HDL实现方面的潜力和有效性。
...
### [15] [IMACT-CXR - An Interactive Multi-Agent Conversational Tutoring System for Chest X-Ray Interpretation](https://arxiv.org/abs/2511.15825)
*Tuan-Anh Le,Anh Mai Vu,David Yang,Akash Awasthi,Hien Van Nguyen*

Main category: cs.AI

TL;DR: IMACT-CXR是一个交互式多智能体对话导师，它通过整合空间标注、凝视分析、知识检索和图像推理，帮助学员解读胸部X光片。


<details>
  <summary>Details</summary>
Motivation: 旨在解决学员在解读胸部X光片时面临的挑战，并提供一个统一、交互式的辅导系统以提高诊断能力。

Method: 该系统基于AutoGen工作流，整合了学员的边界框、凝视样本和自由文本观察。其核心方法包括：专业智能体评估定位质量、生成苏格拉底式辅导、检索PubMed证据、推荐REFLACX中的相似案例，并在掌握度低或学员明确要求时触发NV-Reason-CXR-3B进行视觉-语言推理。系统使用贝叶斯知识追踪(BKT)来估计技能掌握度，并以此驱动知识强化和案例检索。此外，还包含一个基于TensorFlow U-Net的肺叶分割模块用于解剖学凝视反馈，并利用安全提示防止过早泄露真实标签。系统与REFLACX数据集集成以处理真实的DICOM案例。

Result: IMACT-CXR展示了响应迅速的辅导流程，具有有限的延迟、对答案泄露的精确控制，并具有向住院医师培训部署的可扩展性。初步评估显示，与基线相比，学员的定位和诊断推理能力得到了改善。

Conclusion: IMACT-CXR是一个有效的交互式多智能体辅导系统，能够通过整合多种技术来帮助学员提高胸部X光片的解读能力，并在定位和诊断推理方面表现出积极效果，具有实际部署潜力。
...
### [16] [Mini Amusement Parks (MAPs): A Testbed for Modelling Business Decisions](https://arxiv.org/abs/2511.15830)
...
TL;DR: 本文引入Mini Amusement Parks (MAPs)，一个游乐园模拟器，用于评估AI在复杂、互联的现实世界决策中的能力。研究发现人类在MAPs上的表现远超最先进的LLM智能体，揭示了当前AI在长远优化、样本效率学习、空间推理和世界建模方面的持续弱点。


<details>
  <summary>Details</summary>
Motivation: 尽管人工智能发展迅速，但当前系统难以应对定义现实世界决策的相互关联的挑战，例如开放式、多方面的目标优化、从稀疏经验中学习环境动态、不确定环境下的长期规划以及空间信息推理。现有的人工智能基准测试往往只关注这些能力的一个子集，限制了我们评估整体决策能力。

Method: 研究引入了Mini Amusement Parks (MAPs)——一个游乐园模拟器，旨在评估智能体建模环境、在不确定性下预测长期后果以及战略性运营复杂业务的能力。研究提供了人类基线，并对最先进的大型语言模型（LLM）智能体进行了全面评估。

Result: 人类在简单模式下表现优于LLM智能体6.5倍，在中等模式下优于9.8倍。分析揭示了LLM智能体在长远优化、样本效率学习、空间推理和世界建模方面的持续弱点。

Conclusion: 通过在一个单一环境中统一这些挑战，MAPs为基准测试能够进行适应性决策的智能体提供了新的基础。当前AI系统在模拟现实世界的整体决策能力方面仍与人类存在显著差距。
...
### [17] [Step-Audio-R1 Technical Report](https://arxiv.org/abs/2511.15848)
...
TL;DR: Step-Audio-R1是首个成功解锁音频领域推理能力的模型，通过模态接地推理蒸馏（MGRD）框架，生成与声学特征相关的推理链，在音频理解和推理基准上超越Gemini 2.5 Pro并媲美Gemini 3 Pro，证明推理能力在模态间是可迁移的。


<details>
  <summary>Details</summary>
Motivation: 文本和视觉领域的推理模型取得了显著成功，但音频语言模型在推理方面表现不佳，甚至在没有或极少推理时表现更好，这引发了一个基本问题：音频智能能否从深思熟虑中受益？

Method: 引入了Step-Audio-R1，这是第一个成功的音频推理模型。通过提出的模态接地推理蒸馏（MGRD）框架，Step-Audio-R1学习生成真正基于声学特征而非凭空捏造的、与音频相关的推理链。

Result: Step-Audio-R1展现出强大的音频推理能力，在涵盖语音、环境音和音乐的综合音频理解和推理基准测试中，超越了Gemini 2.5 Pro，并达到了与最先进的Gemini 3 Pro相当的性能。

Conclusion: 研究结果表明，当推理能力被恰当地锚定到特定模态时，它可以在不同模态之间迁移，从而使深思熟虑从一种负担转变为音频智能的强大资产。Step-Audio-R1作为首个成功的音频推理模型，为构建能够跨所有感官模态进行深度思考的真正多模态推理系统开辟了新途径。
...
### [18] [JudgeBoard: Benchmarking and Enhancing Small Language Models for Reasoning Evaluation](https://arxiv.org/abs/2511.15958)
*Zhenyu Bi,Gaurav Srivastava,Yang Li,Meng Lu,Swastik Roy,Morteza Ziyadi,Xuan Wang*

Main category: cs.AI

TL;DR: 本文提出了JudgeBoard，一个直接评估模型答案正确性的流程，并引入了多智能体判断(MAJ)框架，使小型语言模型(SLM)通过协作审议能达到或超越大型语言模型(LLM)的判断能力，从而实现可扩展和高效的评估。


<details>
  <summary>Details</summary>
Motivation: 现有LLM作为裁判的框架通常依赖间接比较，难以自动化进行细粒度和可扩展的推理输出评估。同时，SLM在判断答案正确性方面的能力与LLM相比尚不明确。

Method: 本文提出JudgeBoard，一个直接查询模型评估答案正确性的新型评估流程，无需额外答案比较。聚焦于数学和科学/常识推理，构建了基于准确率和Elo评分的任务特定评估排行榜。为提高轻量级模型的判断性能，提出MAJ（多智能体判断）框架，利用多个具有不同推理配置的SLM通过协作审议来近似LLM级别的判断准确性。

Result: 实验结果显示，在独立判断任务中，SLM与LLM之间存在显著性能差距。然而，MAJ框架显著提高了SLM的可靠性和一致性。在MATH数据集上，使用较小规模模型作为骨干的MAJ表现与较大规模模型相当甚至更优。研究发现多智能体SLM系统在判断任务中可能匹配或超越LLM的性能。

Conclusion: 多智能体SLM系统在判断任务中具有匹配或超越LLM性能的潜力，为可扩展和高效的评估提供了重要启示。
...
### [19] [Decomposing Theory of Mind: How Emotional Processing Mediates ToM Abilities in LLMs](https://arxiv.org/abs/2511.15895)
*Ivan Chulo,Ananya Joshi*

Main category: cs.AI

TL;DR: 研究发现，通过激活引导提升大型语言模型（LLM）的心智理论（ToM）能力，其内部机制在于增强情感处理并抑制分析性思维。


<details>
  <summary>Details</summary>
Motivation: 尽管激活引导能显著提升LLM的心智理论能力，但其导致不同输出的内部机制尚不明确，本研究旨在揭示这一机制。

Method: 通过比较引导后与基线LLM的激活，并使用在45种认知行为上训练的线性探针来分解ToM。具体方法是在Gemma-3-4B模型上应用对比激活添加（CAA）引导，并在1000个BigToM前向信念场景中进行评估。

Result: 研究发现，信念归因任务的性能有所提升（准确率从32.5%提高到46.7%）。这种提升是由处理情感内容的激活（如情感感知+2.23，情感评估+2.20）所介导，同时抑制了分析过程（如质疑-0.78，聚合思维-1.59）。

Conclusion: LLM成功的心智理论能力是由情感理解而非分析推理所介导的。
...
### [20] [Thinking, Faithful and Stable: Mitigating Hallucinations in LLMs](https://arxiv.org/abs/2511.15921)
...
### [21] [Sensorium Arc: AI Agent System for Oceanic Data Exploration and Interactive Eco-Art](https://arxiv.org/abs/2511.15997)
*Noah Bissell,Ethan Paley,Joshua Harrison,Juliano Calil,Myungin Lee*

Main category: cs.AI

TL;DR: Sensorium Arc是一个实时多模态交互式AI系统，将海洋拟人化为诗意讲述者，通过对话和数据可视化引导用户探索复杂的海洋数据，旨在情感化地呈现环境数据。


<details>
  <summary>Details</summary>
Motivation: 复杂的海洋数据通常以抽象数据集形式存在，难以直观理解和情感连接。该研究旨在通过AI代理将海洋数据转化为活生生的叙事，促进用户对环境数据的直观和情感理解，并探索人机生态系统的新范式。

Method: 该系统基于模块化多智能体和检索增强型大型语言模型（LLM）框架构建。AI代理以海洋视角进行自然口语对话，融合科学洞察与生态诗意。通过关键词检测和语义解析，系统根据对话中的时间、地点和主题线索动态触发数据可视化和视听播放。灵感来源于Newton Harrison的生态美学哲学。

Result: Sensorium Arc系统能够将海洋数据从抽象数据集转化为活生生的叙事，使用户能够通过会话式AI代理情感化、直观地访问高维环境数据。

Conclusion: 该项目展示了会话式AI代理在调解对高维环境数据的情感和直观访问方面的潜力，并提出了一种全新的人机生态系统范式。
...
### [22] [KRAL: Knowledge and Reasoning Augmented Learning for LLM-assisted Clinical Antimicrobial Therapy](https://arxiv.org/abs/2511.15974)
...
TL;DR: 研究发现，即使提供正确证据，大型语言模型（LLMs）在临床环境中也难以正确推理。本文提出一个评估框架，衡量推理的准确性、一致性和忠实度，强调安全部署LLMs需要严格评估推理能力。


<details>
  <summary>Details</summary>
Motivation: 访问正确证据并不能保证大型语言模型（LLMs）能正确推理，尤其是在输出必须符合结构化协议的临床环境中。研究旨在弥合检索与推理之间的差距。

Method: 使用书面暴露疗法（WET）指南作为测试平台，评估模型对临床医生验证问题的响应。提出一个评估框架，衡量推理的准确性、一致性和忠实度。

Result: 即使提供了权威段落，模型错误仍然存在。检索增强生成（RAG）可以约束输出，但安全部署需要像评估检索一样严格评估推理。

Conclusion: 检索增强生成（RAG）具有潜力，但为了安全部署，必须对LLMs的推理能力进行严格评估，而不仅仅是检索能力。
...
### [24] [Detecting Sleeper Agents in Large Language Models via Semantic Drift Analysis](https://arxiv.org/abs/2511.15992)
...
TL;DR: 本文提出SpellForger游戏，玩家通过自然语言提示创建自定义法术，利用AI实现个性化和创造性的核心玩法。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在游戏内容生成方面发展显著，但其作为核心玩法共创工具的应用仍未被充分探索。

Method: 系统采用经过监督训练的BERT模型来解释玩家的自然语言提示，将文本描述映射到预设法术并平衡其参数（伤害、消耗、效果），以确保游戏平衡性。游戏在Unity引擎开发，AI后端使用Python。

Result: 预期将交付一个功能原型，实时生成法术并应用于引人入胜的游戏循环中，使玩家的创造力成为核心体验。

Conclusion: 本研究旨在验证AI作为直接游戏机制的可行性。
...
TL;DR: 本文提出了一种新颖的约束编程（CP）模型，用于解决具有最小批次规模的串行批处理调度问题。该模型通过直接推理同家族作业序列来避免现有CP模型中虚拟批次集的维度灾难，并通过定制搜索和强化推理进一步改进，在广泛的实验中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 串行批处理调度中，现有处理最小批次规模的CP模型依赖于预定义的虚拟批次集，这导致维度灾难并增加了问题复杂性。在半导体制造等实际应用中，最小批次规模是一个常见要求。

Method: 本文提出了一种不依赖虚拟批次集的新型CP模型。它使用关键对齐参数直接对机器上同家族作业的序列进行推理，从而形成更紧凑的公式。该模型通过利用问题结构定制搜索阶段和强化约束传播器的推理级别进一步改进。

Result: 在近五千个实例上的广泛计算实验表明，所提出的模型在小型到中型实例（最多100个作业）上表现出卓越性能。在大型实例（最多500个作业、10个家族和10台机器）上，它能找到比现有方法（包括混合整数规划、禁忌搜索元启发式和CP方法）好25%的解决方案。

Conclusion: 所提出的新型CP模型在处理具有最小批次规模的串行批处理调度问题上表现出优越性。它通过避免虚拟批次集和利用问题结构，实现了更紧凑的公式和更强的求解能力，尤其是在大型实例上显著优于现有技术。
...
### [27] [OpenMMReasoner: Pushing the Frontiers for Multimodal Reasoning with an Open and General Recipe](https://arxiv.org/abs/2511.16334)
*Kaichen Zhang,Keming Wu,Zuhao Yang,Kairui Hu,Bin Wang,Ziwei Liu,Xingxuan Li,Lidong Bing*

Main category: cs.AI

TL;DR: OpenMMReasoner提出了一种透明的两阶段（SFT+RL）多模态推理训练方法，通过高质量数据和精心设计的训练策略，显著提升了模型性能，并开源了所有资源。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉推理取得了进展，但多模态推理领域缺乏透明和可复现的数据构建与训练策略，这阻碍了可扩展的研究。

Method: 引入了OpenMMReasoner，一个完全透明的两阶段训练方案：
1.  **SFT（监督微调）阶段**：构建了一个包含874K样本的冷启动数据集，并进行严格的逐步验证，为推理能力打下基础。
2.  **RL（强化学习）阶段**：利用一个包含74K样本的跨领域数据集，进一步提升和稳定推理能力，实现更稳健高效的学习过程。

Result: 该训练方法不仅超越了强大的基线模型，还突出了数据质量和训练设计在多模态推理性能中的关键作用。具体而言，在九个多模态推理基准测试中，其方法比Qwen2.5-VL-7B-Instruct基线提高了11.6%。

Conclusion: OpenMMReasoner提供了一个透明、可复现且有效的多模态推理训练方案，为未来的大规模多模态推理研究奠定了坚实的实证基础，并强调了数据质量和训练设计的重要性。
...
### [29] [TOFA: Training-Free One-Shot Federated Adaptation for Vision-Language Models](https://arxiv.org/abs/2511.16423)
*Li Zhang,Zhongxuan Han,XiaoHua Feng,Jiaming Zhang,Yuyuan Li,Linbo Jiang,Jianan Lin,Chaochao Chen*

Main category: cs.AI

TL;DR: 本文提出了一种名为TOFA的无训练、一次性联邦VLM适应框架，通过利用视觉和文本管道以及自适应权重校准机制，有效应对联邦学习中预训练视觉-语言模型（VLM）适应任务时的通信成本、潜在攻击和数据异质性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的联邦学习中VLM适应算法通常采用迭代训练，导致高昂的通信成本并易受攻击。尽管一次性联邦训练技术能减少客户端-服务器交互，但当前的一次性方法在联邦设置下适应VLM时面临挑战：未能充分利用VLM丰富的多模态信息、缺乏专门策略处理严重的数据异质性，以及需要额外的客户端或服务器训练资源。

Method: 本文提出了TOFA框架，这是一种无训练的一次性联邦VLM适应方法。TOFA利用预训练VLM中可泛化的多模态特征，通过视觉和文本两条管道提取任务相关表示。在视觉管道中，采用分层贝叶斯模型学习个性化的、特定类别的原型分布。在文本管道中，TOFA评估并全局对齐生成的本地文本提示以增强鲁棒性。此外，引入了一种自适应权重校准机制来结合两种模态的预测，以平衡个性化和鲁棒性，从而处理数据异质性。该方法无需客户端或服务器的额外训练资源。

Result: 在各种联邦设置下的9个数据集上进行的广泛实验证明了所提出的TOFA方法的有效性。

Conclusion: TOFA提供了一种高效、轻量级且无训练的解决方案，用于在联邦学习环境中适应预训练的VLM，通过充分利用多模态信息和自适应策略，有效解决了通信成本、安全性和数据异质性等关键挑战。
...
### [30] [A Hybrid Proactive And Predictive Framework For Edge Cloud Resource Management](https://arxiv.org/abs/2511.16075)
*Hrikshesh Kumar,Anika Garg,Anshul Gupta,Yashika Agarwal*

Main category: cs.AI

TL;DR: 该研究提出一种主动式云边缘工作负载资源管理框架，通过结合CNN-LSTM预测和多智能体深度强化学习，使AI管理器能预见未来，从而优化长期资源规划，在成本、性能和可靠性之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 旧的云边缘工作负载资源管理过于被动，依赖静态阈值会导致资源浪费或性能下降。因此，需要开发主动式解决方案来预测问题并提前规划。

Method: 设计了一个混合架构，结合了用于时间序列预测的CNN-LSTM模型和基于多智能体深度强化学习（DRL）的编排器。其创新点在于将CNN-LSTM的预测结果直接嵌入到DRL智能体的状态空间中，使AI管理器能够“预见未来”，从而做出更好的长期任务运行决策，以在节省成本、保持系统健康和应用程序快速运行之间找到最佳平衡点。

Result: 测试结果表明，该系统轻松超越了旧方法。它在解决复杂决策和同时兼顾多个目标（如成本低、速度快、可靠）等难题方面表现出色。

Conclusion: 该系统赋予了AI管理器“预见未来”的能力，使其能够平稳地向前发展，避免被动应对问题。它成功实现了在成本、性能和可靠性等多目标下的优化资源管理。
...
### [32] [FOOTPASS: A Multi-Modal Multi-Agent Tactical Context Dataset for Play-by-Play Action Spotting in Soccer Broadcast Videos](https://arxiv.org/abs/2511.16183)
*Jeremie Ochin,Raphael Chekroun,Bogdan Stanciulescu,Sotiris Manitsaris*

Main category: cs.AI

TL;DR: 该论文引入了FOOTPASS数据集，这是一个用于足球比赛中多模态、多智能体战术背景下逐场动作识别的基准，旨在通过结合计算机视觉输出和战术知识，更可靠地自动化生成逐场数据。


<details>
  <summary>Details</summary>
Motivation: 当前的动作识别方法不足以构建可靠的逐场数据，通常仅用于辅助而非完全自动化标注。同时，战术建模、轨迹预测和性能分析等研究都依赖于比赛状态和逐场数据，这促使研究者利用战术知识作为先验来支持计算机视觉预测，以实现更自动化、更可靠的逐场数据提取。

Method: 研究引入了Footovision Play-by-Play Action Spotting in Soccer Dataset (FOOTPASS)，这是第一个用于足球比赛中多模态、多智能体战术背景下逐场动作识别的基准数据集。该方法旨在开发以球员为中心的动作识别方法，这些方法利用计算机视觉任务的输出（如跟踪、识别）以及足球的先验知识（包括长期战术规律）来生成可靠的逐场数据流。

Result: 论文成功创建了FOOTPASS数据集，它是第一个针对整个足球比赛的逐场动作识别的基准，该数据集在一个多模态、多智能体战术背景下运作。它能够促进开发结合计算机视觉输出和足球战术先验知识的球员中心动作识别方法。

Conclusion: FOOTPASS数据集为足球分析提供了一个重要的输入，它使得开发更自动化、更可靠的逐场数据流提取方法成为可能，这些数据流对于数据驱动的体育分析至关重要。
...
### [33] [D-GARA: A Dynamic Benchmarking Framework for GUI Agent Robustness in Real-World Anomalies](https://arxiv.org/abs/2511.16590)
...
TL;DR: CRM (多智能体协作奖励模型) 框架通过用专业评估器团队取代单一黑盒奖励模型，提高了RLHF中奖励模型的鲁棒性和可解释性，并引入了rewardBench基准来支持其训练和评估。


<details>
  <summary>Details</summary>
Motivation: 传统的奖励模型难以同时优化多个（有时相互冲突的）偏好维度（如事实性、有用性、安全性），并且缺乏对分数分配原因的透明度。

Method: CRM将偏好评估分解为多个领域特定智能体，每个智能体生成部分信号，同时结合基于排序器和嵌入相似度的全局评估器。一个中央聚合器在每个时间步融合这些信号，平衡逐步正确性、多智能体一致性和重复惩罚，生成一个与标准RL管道兼容的单一训练奖励。策略通过基于优势的更新进行优化，价值模型回归到聚合奖励。为支持训练和评估，引入了rewardBench。

Result: CRM实现了多视角奖励塑造，无需额外的人工标注，提升了奖励模型的鲁棒性和可解释性，并提供了更稳定的优化。rewardBench为CRM的协作结构提供了训练和评估支持。

Conclusion: CRM和rewardBench提供了一种实用、模块化的途径，以实现更透明的奖励建模和更稳定的RLHF优化。
...
### [35] [SkyRL-Agent: Efficient RL Training for Multi-turn LLM Agent](https://arxiv.org/abs/2511.16108)
...
TL;DR: 本文介绍了SkyRL-Agent，一个高效的多轮、长时程智能体训练和评估框架。通过优化异步调度和工具增强的训练方案，该框架显著提升了软件工程智能体SA-SWE-32B的性能和训练效率，并在多个智能体任务上展现出强大的泛化能力和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 当前智能体训练面临效率低、多轮和长时程任务处理能力不足的挑战。研究旨在开发一个高效、可扩展的框架来解决这些问题，并提升基于强化学习的智能体在复杂任务（如软件工程）上的性能。

Method: 引入SkyRL-Agent框架，该框架具有高效异步调度、轻量级工具集成和灵活后端互操作性。在此框架下，基于Qwen3-32B通过纯强化学习训练了软件工程智能体SA-SWE-32B。关键方法包括：1) 优化异步管道调度器，实现1.55倍的速度提升；2) 采用工具增强的训练方案，特别是利用基于AST的搜索工具来辅助代码导航，提高rollout Pass@K并提升训练效率。

Result: SkyRL-Agent框架实现了高效的智能体训练。SA-SWE-32B在SWE-Bench Verified上达到了39.4%的Pass@1，与之前达到类似性能的模型相比，训练成本降低了2倍以上。优化的异步管道调度器比朴素异步批处理提速1.55倍。SA-SWE-32B尽管仅在SWE任务上训练，但能有效泛化到Terminal-Bench、BrowseComp-Plus和WebArena等其他智能体任务。SkyRL-Agent还通过案例研究展示了其在不同训练后端下的可扩展性。

Conclusion: SkyRL-Agent是一个高效、可扩展的智能体训练和评估框架，通过其优化的调度和工具集成，显著提升了强化学习智能体的训练效率和性能。它成功地训练出高性能的软件工程智能体SA-SWE-32B，并证明了其在多种智能体任务上的泛化能力和与不同训练后端的兼容性。
...
### [36] [Multidimensional Rubric-oriented Reward Model Learning via Geometric Projection Reference Constraints](https://arxiv.org/abs/2511.16139)
...
### [37] [From Performance to Understanding: A Vision for Explainable Automated Algorithm Design](https://arxiv.org/abs/2511.16201)
...
TL;DR: 本文展示了一个概念验证原型，实现了分布式系统中代理间仅使用自然语言消息进行通信，无需共享标识符或集中数据，并通过假名化令牌和本地数据查找确保隐私和安全。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索多个组织如何在不共享敏感数据（如患者身份）的情况下，安全地进行合作和分布式推理，同时仅使用自然语言进行通信，以克服传统分布式系统中共享标识符和集中数据交换的限制。

Method: 该原型使用Orpius作为多代理编排、工具执行和隐私保护通信的底层平台。所有代理通过OperationRelay调用交换简洁的自然语言摘要。每个代理操作自己的本地数据，并通过基于HMAC的假名化案例令牌进行通信。一个具体的例子是诊所、保险公司和专家网络之间的合作，其中诊所计算假名令牌，保险公司评估覆盖规则并咨询专家，专家返回适当性建议。

Result: 研究结果成功地概念验证了在分布式系统中，使用自然语言消息、假名化令牌和本地数据实现代理间通信的可行性。它展示了允许专业代理进行分布式推理的架构模式、隐私考虑和通信流程，同时确保数据保留在各自组织内部，且不接收或重建患者身份。

Conclusion: 该工作证明了去中心化多代理系统在分布式推理和隐私保护通信方面的可行性，并为未来更严格的评估和深入研究指明了方向。
...
TL;DR: 该论文介绍了ChemO，一个基于国际化学奥林匹克竞赛（IChO）2025的新基准，旨在解决AI在化学领域多模态推理的挑战。为应对此基准，提出了ChemLabs，一个模拟人类专家协作的分层多智能体框架。实验表明，结合SVE和ChemLabs显著提升了性能，达到93.6分，超越了人类金牌门槛。


<details>
  <summary>Details</summary>
Motivation: 数学和物理奥林匹克级别的基准已成为高级AI推理的关键测试平台，但化学领域因其独特的多模态符号语言，仍是一个未解决的挑战。研究旨在为化学领域提供一个类似的严格基准，并开发能处理其复杂性的AI方法。

Method: 引入了ChemO基准，包含两项创新：评估等效重构（AER），将需要视觉输出的问题（如绘制分子）转换为可计算格式；以及结构化视觉增强（SVE），一种诊断机制，用于区分模型的视觉感知能力与核心化学推理能力。为应对ChemO，提出了ChemLabs，一个分层多智能体框架，通过专门的智能体（用于问题分解、感知、推理和审计）模拟人类专家协作。

Result: 在最先进的多模态模型上进行的实验表明，将SVE与所提出的多智能体系统（ChemLabs）结合使用，能带来显著的性能提升。最佳配置获得了93.6分（满分100），超过了估计的人类金牌门槛，并在自动化化学问题解决方面建立了新的最先进水平。

Conclusion: ChemO基准和ChemLabs多智能体框架成功解决了AI在奥林匹克级别化学问题解决中的挑战。通过创新的评估方法和模拟人类协作的AI系统，实现了超越人类金牌门槛的性能，为自动化化学问题解决树立了新的里程碑。
...
### [40] [FlipVQA-Miner: Cross-Page Visual Question-Answer Mining from Textbooks](https://arxiv.org/abs/2511.16216)
...
### [41] [Trustworthy AI in the Agentic Lakehouse: from Concurrency to Governance](https://arxiv.org/abs/2511.16402)
*Jacopo Tagliabue,Federico Bianchi,Ciro Greco*

Main category: cs.AI

TL;DR: 为解决企业对AI代理处理生产数据缺乏信任的问题，本文提出了一种名为Bauplan的代理优先湖仓设计，通过解决基础设施问题来确保事务性和治理，从而实现可信赖的代理工作流。


<details>
  <summary>Details</summary>
Motivation: 尽管AI能力不断提升，但大多数企业认为AI代理不足以信任来处理生产数据。传统的数据湖仓不适合代理的访问模式，缺乏必要的事务性和治理机制，这是阻碍代理工作流可信赖性的主要原因。

Method: 本文提出了一种名为Bauplan的代理优先设计，旨在重新实现湖仓中的数据和计算隔离。它借鉴了数据库中MVCC（多版本并发控制）的操作类比，但指出直接移植到解耦、多语言环境中会失败。Bauplan从根本上解决了湖仓的事务性和治理问题，以适应代理访问模式。

Result: 通过Bauplan，论文展示了一个自修复管道的参考实现，该管道将代理推理与正确性和信任所需的所有保证无缝结合，证明了其在实现可信赖代理工作流方面的有效性。

Conclusion: 实现可信赖的代理工作流首先要解决基础设施问题。通过Bauplan等代理优先设计，重新构建湖仓以支持事务和治理，可以为代理操作提供必要的正确性和信任保证。
...
### [42] [Revisiting Fairness-aware Interactive Recommendation: Item Lifecycle as a Control Knob](https://arxiv.org/abs/2511.16248)
...
### [43] [From generative AI to the brain: five takeaways](https://arxiv.org/abs/2511.16432)
*Claudius Gros*

Main category: cs.AI

TL;DR: 生成式AI的原理可能也适用于大脑，认知神经科学可以从机器学习研究中获得关于神经信息处理的深刻见解。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的巨大进步源于明确的生成原理和成功的具体实现，这促使作者思考这些原理是否也在大脑中运作。此外，机器学习研究对神经信息处理系统提出了一系列有趣的描述。

Method: 本文提出对生成式AI原理在大脑中的运作进行深入研究的必要性，并讨论了五个具体例子（世界建模的不足、思维过程的生成、注意力、神经缩放定律和量化），以说明神经科学可以从机器学习研究中学习的潜力。

Result: 通过讨论五个例子，文章阐明了神经科学如何能够潜在地从机器学习研究中学习，从而加深对神经信息处理系统的理解。

Conclusion: 神经科学应深入探究生成式AI的生成原理是否也在大脑中发挥作用，并积极借鉴机器学习研究中关于神经信息处理系统的有趣描述和见解。
...
### [44] [MuISQA: Multi-Intent Retrieval-Augmented Generation for Scientific Question Answering](https://arxiv.org/abs/2511.16283)
...
### [45] [Reducing Instability in Synthetic Data Evaluation with a Super-Metric in MalDataGen](https://arxiv.org/abs/2511.16373)
*Anna Luiza Gomes da Silva,Diego Kreutz,Angelo Diniz,Rodrigo Mansilha,Celso Nobre da Fonseca*

Main category: cs.AI

TL;DR: 本文提出了一种集成到MalDataGen中的“超级度量”，用于评估Android恶意软件领域合成数据质量，该度量比传统度量更稳定、更一致，并与分类器性能表现出更强的相关性。


<details>
  <summary>Details</summary>
Motivation: Android恶意软件领域中，合成数据质量的评估因现有度量的不稳定性和缺乏标准化而持续面临挑战。

Method: 将一个名为“超级度量”的指标集成到MalDataGen中。该超级度量聚合了跨越四个保真度维度的八个度量，生成一个单一的加权分数。实验涉及十个生成模型和五个平衡数据集。

Result: 实验证明，该超级度量比传统度量更稳定、更一致，并且与分类器的实际性能表现出更强的相关性。

Conclusion: 该超级度量为评估Android恶意软件合成数据质量提供了一个更可靠和标准化的方法。
...
TL;DR: 本文介绍了PersonaDrift，一个合成基准，用于评估机器学习和统计方法在检测痴呆症患者（PLwD）日常沟通中渐进性行为变化（如情感平淡和偏题）方面的能力，并评估了多种检测方法。


<details>
  <summary>Details</summary>
Motivation: 痴呆症患者的沟通方式会发生微妙变化，如表达减少、重复或偏题。尽管照护者能非正式地注意到这些变化，但现有计算工具大多无法跟踪这种随时间推移的行为漂移。

Method: 研究引入了PersonaDrift，一个合成基准，模拟了基于照护者访谈的真实痴呆症患者的60天数字提醒系统交互日志。该基准注入了两种主要变化：情感平淡（情绪和话语减少）和偏题回复（语义漂移），并以不同速率渐进注入。研究评估了异常检测方法、无监督统计方法（CUSUM、EWMA、One-Class SVM）、使用上下文嵌入的序列模型（GRU + BERT）以及通用和个性化设置下的监督分类器。

Result: 初步结果显示，对于基线变异性较低的用户，简单统计模型通常可以检测到情感平淡。而检测语义漂移则需要时间序列建模和个性化基线。在这两项任务中，个性化分类器始终优于通用分类器，强调了个体行为背景的重要性。

Conclusion: 检测痴呆症患者沟通中的渐进性变化是一项复杂任务。个性化方法和时间序列建模对于准确识别这些行为漂移至关重要，特别是对于语义漂移。PersonaDrift基准为未来研究提供了有价值的工具。
...
### [47] [An Agent-Based Framework for the Automatic Validation of Mathematical Optimization Models](https://arxiv.org/abs/2511.16383)
...
TL;DR: CLOZE是一个利用大型语言模型（LLMs）从临床笔记中自动提取医学实体并将其整合到分层医学本体论中的零样本框架，具有高准确性、可扩展性和隐私保护能力。


<details>
  <summary>Details</summary>
Motivation: 现有本体论的覆盖范围和实用性有待提高，而临床笔记作为非结构化文档，富含详细的患者观察结果，是本体论扩展的宝贵但未充分利用的来源。直接利用临床笔记进行本体论扩展仍未得到充分探索。

Method: CLOZE框架利用预训练LLMs强大的语言理解和广泛的生物医学知识，从临床笔记中识别疾病相关概念并捕获复杂的分层关系。它是一个零样本框架，无需额外训练或标注数据，并通过自动化去除受保护健康信息（PHI）来确保患者隐私。

Result: 实验结果表明，CLOZE提供了一个准确、可扩展且隐私保护的本体论扩展框架。

Conclusion: CLOZE在生物医学研究和临床信息学领域具有支持广泛下游应用的巨大潜力。
...
### [49] [Pharos-ESG: A Framework for Multimodal Parsing, Contextual Narration, and Hierarchical Labeling of ESG Report](https://arxiv.org/abs/2511.16417)
...
TL;DR: 本文提出Deliberate Practice Policy Optimization (DPPO) 框架，通过结合监督微调和强化学习的“元循环”训练，有效解决了具身智能系统中数据稀缺和算法效率低下的问题，显著提升了具身智能体的学习效率和性能。


<details>
  <summary>Details</summary>
Motivation: 开发通用多功能的具身智能系统面临两大挑战：一是具身数据瓶颈，即真实世界数据稀缺且昂贵；二是现有算法效率低下，资源消耗巨大。

Method: 引入Deliberate Practice Policy Optimization (DPPO)，这是一个元认知“元循环”训练框架。它动态地在监督微调（能力扩展）和强化学习（技能完善）之间切换，以自动识别弱点并进行有针对性的资源分配，旨在最大限度地提高从稀疏有限数据中学习的效率。理论上，DPPO 可被形式化为一个统一的偏好学习框架。

Result: 使用DPPO训练的视觉-语言具身模型Pelican-VL 1.0，比基础模型性能提升20.3%，并超越了100B参数规模的开源模型10.6%。作者开源了模型和代码。

Conclusion: DPPO提供了首个系统性框架，有效缓解了数据和资源瓶颈，使社区能够高效地构建多功能具身智能体。
...
TL;DR: MedBayes-Lite是一种轻量级贝叶斯增强框架，用于基于Transformer的临床语言模型，旨在提供可靠、感知不确定性的预测，无需重新训练或改变架构，显著降低了医疗AI的过度自信并提高了可信度。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在临床决策支持方面潜力巨大，但它们容易过度自信，尤其是在模糊的医学案例中，而校准后的不确定性在这种情况下至关重要。

Method: MedBayes-Lite将不确定性量化直接嵌入现有Transformer管道，无需重新训练或修改架构，未增加新的可训练层，参数开销低于3%。它包含三个组件：(i) 使用蒙特卡洛dropout进行认知不确定性量化的贝叶斯嵌入校准；(ii) 对token可靠性进行边缘化的不确定性加权注意力；(iii) 受临床风险最小化启发的置信度引导决策塑形。

Result: 在生物医学问答和临床预测基准（MedQA、PubMedQA、MIMIC-III）上，MedBayes-Lite持续改进了校准和可信度，将过度自信降低了32%至48%。在模拟临床环境中，通过标记不确定预测以供人工审查，它可以预防高达41%的诊断错误。

Conclusion: MedBayes-Lite有效实现了可靠的不确定性传播，并提高了医疗AI系统的可解释性，使其在临床应用中更值得信赖。
...
TL;DR: 本文提出一个基于AI的欧元兑美元高频算法交易系统，结合宏观经济基本面和技术指标，通过机器学习指标和历史回测评估其性能，并比较两类特征对盈利交易信号的预测能力。


<details>
  <summary>Details</summary>
Motivation: 设计一个先进的、基于人工智能的算法交易系统，专门用于外汇市场高频环境下的欧元兑美元交易，并探究基本面和技术特征哪一类能提供更可靠的盈利交易信号。

Method: 该系统整合了来自欧元区和美国的关键宏观经济基本面变量（如GDP、失业率）以及全面的技术变量（包括指标、振荡器、斐波那契水平和价格背离）作为输入特征。算法性能通过标准机器学习指标评估预测准确性，并通过历史数据回测模拟评估交易盈利能力和风险。研究还进行了比较分析，以确定哪类输入特征具有更强的预测能力。

Result: 该算法的性能通过标准的机器学习指标来量化预测准确性，并通过历史数据回测模拟评估了交易盈利能力和风险。研究还进行了比较分析，以确定基本面或技术输入特征哪一类提供更强大和可靠的预测能力。

Conclusion: 该研究旨在通过比较分析，确定哪一类输入特征（基本面或技术）能为生成盈利交易信号提供更大、更可靠的预测能力。
...
### [53] [Consciousness in Artificial Intelligence? A Framework for Classifying Objections and Constraints](https://arxiv.org/abs/2511.16582)
...
### [54] [Formal Abductive Latent Explanations for Prototype-Based Networks](https://arxiv.org/abs/2511.16588)
...
TL;DR: YOFO提出了一种模板条件方法，使多模态大语言模型（MLLMs）能够在一个前向传递中快速且可解释地判断多个结构化要求，从而解决了现有方法的慢速和细节不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有的将多模态大语言模型（MLLMs）用作评判器的方法面临一个根本性权衡：输出单一分数与MLLMs的生成性质不符且限制了对细粒度需求的理解；而自动回归生成评判分析在需要高吞吐量的场景中速度过慢。研究者观察到评判本质上是验证输入是否满足一组结构化要求。

Method: YOFO是一种模板条件方法，它在一个前向传递中判断所有要求。它基于一个自回归模型，接受一个结构化的要求模板，并在一个推理步骤中，通过读取与每个要求相关的最终token的logits，为每个要求生成一个二元的“是/否”决策。该设计支持依赖感知分析，并受益于事后CoT。

Result: YOFO实现了数量级的速度提升，同时保持了可解释性。在标准推荐数据集上取得了最先进的结果，并支持依赖感知分析，且能从事后CoT中进一步受益。

Conclusion: YOFO通过其单次前向传递的模板条件判断方法，有效解决了MLLMs作为评判器时在速度和细粒度理解之间的权衡问题，提供了快速、可解释且高性能的多要求判断能力。
...
### [57] [UniFit: Towards Universal Virtual Try-on with MLLM-Guided Semantic Alignment](https://arxiv.org/abs/2511.15831)
*Wei Zhang,Yeying Jin,Xin Li,Yan Zhang,Xiaofeng Cong,Cong Wang,Fengcai Qiao,zhichao Lian*

Main category: cs.CV

TL;DR: 本文提出了UniFit，一个由多模态大语言模型（MLLM）驱动的通用虚拟试穿（VTON）框架，旨在通过解决文本与图像之间的语义鸿沟和复杂场景下的数据稀缺问题，实现灵活多样的试穿任务，并取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管虚拟试穿技术取得了显著进展，但构建一个能灵活处理多样化和复杂任务的通用VTON框架仍面临挑战。现有方法在处理多任务VTON时，存在文本指令与参考图像之间的语义鸿沟以及复杂场景下数据稀缺两大局限性。

Method: 本文提出了UniFit框架，其核心是一个由多模态大语言模型（MLLM）驱动的通用VTON框架。具体方法包括：1) 引入了MLLM引导的语义对齐模块（MGSA），该模块通过MLLM和一组可学习查询来整合多模态输入。通过施加语义对齐损失，MGSA捕获跨模态语义关系并提供一致且明确的语义指导，从而缩小语义鸿沟。2) 设计了一个两阶段渐进式训练策略，结合自合成管道，使UniFit能够从有限数据中学习复杂任务。

Result: 实验结果表明，UniFit不仅支持广泛的VTON任务，包括多件服装试穿和模型到模型的试穿，而且实现了最先进的性能。

Conclusion: UniFit成功地通过MLLM驱动的语义对齐和两阶段渐进式训练策略，解决了通用虚拟试穿中存在的语义鸿沟和数据稀缺问题，为多样化和复杂任务提供了灵活且高性能的解决方案。
...
### [58] [Adaptive Guided Upsampling for Low-light Image Enhancement](https://arxiv.org/abs/2511.16623)
*Angela Vivian Dcosta,Chunbo Song,Rafael Radkowski*

Main category: cs.CV

TL;DR: 本文提出自适应引导上采样（AGU），一种高效的低光图像放大方法，能同时优化降噪和提高锐度等图像质量。它通过多参数优化学习低光与明亮图像特征之间的关联，实现实时高质量图像渲染，优于现有先进方法。


<details>
  <summary>Details</summary>
Motivation: 现有引导图像方法在处理低光图像时，因高噪声和低亮度导致特征不足，无法显著改善图像质量，从而产生次优结果。研究旨在解决这一问题。

Method: AGU基于引导图像方法，并引入多参数优化。它通过机器学习从少量样本图像对中学习多组低光和明亮图像特征之间的关联，以实现特征转移和优化。

Result: AGU能够从低质量、低分辨率的输入实时渲染出高质量图像。实验证明，在处理低光图像方面，AGU优于现有最先进的方法。

Conclusion: AGU是一种高效且实时的低光图像上采样方法，能够通过学习多参数关联同时优化图像的降噪和锐度等特性，其性能优于现有技术，特别适用于低光场景。
...
### [59] [SAM2S: Segment Anything in Surgical Videos via Semantic Long-term Tracking](https://arxiv.org/abs/2511.16618)
*Haofeng Liu,Ziyue Wang,Sudhanshu Mishra,Mingqi Gao,Guanyi Qin,Chang Han Low,Alex Y. W. Kong,Yueming Jin*

Main category: cs.CV

TL;DR: 该研究构建了SA-SV，一个最大的外科交互式视频对象分割（iVOS）基准数据集，并提出了SAM2S，一个基于SAM2的改进模型，通过引入多样化记忆机制、时间语义学习和抗模糊学习，显著提升了外科手术视频中器械和组织分割的长期跟踪、零样本泛化能力和实时推理性能。


<details>
  <summary>Details</summary>
Motivation: 外科视频分割对于计算机辅助手术至关重要，但现有的交互式视频对象分割（iVOS）模型（如SAM2）在外科场景中面临领域差距大和长期跟踪能力有限的挑战。

Method: 1. 构建了SA-SV，一个最大的外科iVOS基准数据集，包含8种手术类型（61k帧，1.6k实例掩码），具有实例级时空标注。2. 提出了SAM2S，一个增强SAM2的外科iVOS基础模型，具体包括：(1) DiveMem，一种可训练的多样化记忆机制，用于鲁棒的长期跟踪；(2) 时间语义学习，用于器械理解；(3) 抗模糊学习，以减轻多源数据集中的标注不一致性。

Result: 在SA-SV上微调后，SAM2的平均J&F性能比原始SAM2提高了12.99。SAM2S的平均J&F性能达到80.42，分别比原始SAM2和微调后的SAM2高出17.10和4.11点。同时，SAM2S保持了68 FPS的实时推理速度和强大的零样本泛化能力。

Conclusion: SA-SV基准数据集和SAM2S模型有效地解决了外科iVOS的挑战，显著提升了性能、长期跟踪和泛化能力，同时保持了实时推理速度。
...
### [60] [EfficientSAM3: Progressive Hierarchical Distillation for Video Concept Segmentation from SAM1, 2, and 3](https://arxiv.org/abs/2511.15833)
*Chengxi Zeng,Yuxuan Jiang,Aaron Zhang*

Main category: cs.CV

TL;DR: EfficientSAM3通过渐进式分层蒸馏（PHD）方法，将大型SAM3模型的能力转移到轻量级学生模型，实现了高效的设备端概念分割和跟踪。


<details>
  <summary>Details</summary>
Motivation: Segment Anything Model 3 (SAM3) 及其统一架构（共享视觉骨干、DETR风格检测器、密集内存跟踪器）计算成本高昂，不适合在设备端部署使用。

Method: 本文提出了EfficientSAM3，采用渐进式分层蒸馏（PHD）三阶段方法：1) 编码器蒸馏：通过在SA-1B上进行prompt-in-the-loop训练来对齐图像特征；2) 时序内存蒸馏：用紧凑的基于Perceiver的模块替换密集内存，在SA-V上训练以高效压缩和检索时空特征；3) 端到端微调：在SAM3官方PCS数据上精调整个管线以保持概念级性能。学生模型采用RepViT、TinyViT和EfficientViT等骨干。

Result: EfficientSAM3模型系列实现了设备端概念分割和跟踪，同时保持了对SAM3教师模型行为的高保真度。在流行VOS数据集上进行基准测试，EfficientSAM3在性能-效率权衡方面表现出色。

Conclusion: 通过PHD方法，EfficientSAM3成功将SAM3的强大能力转化为一系列高效模型，使其能够在设备端进行概念分割和跟踪，并在保持高精度的同时显著提升了效率。
...
### [61] [WALDO: Where Unseen Model-based 6D Pose Estimation Meets Occlusion](https://arxiv.org/abs/2511.15874)
*Sajjad Pakdamansavoji,Yintao Ma,Amir Rasouli,Tongtong Cao*

Main category: cs.CV

TL;DR: 本文提出了一种针对6D物体姿态估计的新方法，通过动态采样、多假设推理、迭代优化和遮挡增强训练，显著提高了对未见物体的姿态估计精度和推理速度，尤其在遮挡条件下表现更优。


<details>
  <summary>Details</summary>
Motivation: 对于已知物体，6D姿态估计精度较高，但对于未见物体，尤其在遮挡情况下，现有方法通常采用多阶段流程（检测、分割、初始姿态、细化），容易导致早期阶段的错误传播，从而降低整体性能。

Method: 本文提出了四项新颖的扩展：(i) 动态非均匀密集采样策略，将计算重点放在可见区域以减少遮挡误差；(ii) 多假设推理机制，保留多个置信度排序的姿态候选，以缓解单一路径失败；(iii) 迭代细化以逐步提高姿态精度；(iv) 一系列以遮挡为重点的训练增强，以提高鲁棒性和泛化能力。此外，还提出了一种新的基于可见性加权的评估指标，以最小化现有协议的偏差。

Result: 通过广泛的实证评估，本文方法在ICBIN基准测试中实现了超过5%的精度提升，在BOP数据集基准测试中实现了超过2%的精度提升，同时推理速度提高了约3倍。

Conclusion: 本文提出的方法显著提高了6D物体姿态估计的精度和推理速度，尤其在处理遮挡条件下的未见物体时表现出更强的鲁棒性和泛化能力。
...
### [62] [Automatic Uncertainty-Aware Synthetic Data Bootstrapping for Historical Map Segmentation](https://arxiv.org/abs/2511.15875)
*Lukas Arzoumanidis,Julius Knechtel,Jan-Henrik Haunert,Youness Dehbi*

Main category: cs.CV

TL;DR: 该研究提出了一种通过风格迁移和噪声模拟，自动生成大量逼真合成历史地图数据的方法，以解决深度学习在历史地图分析中训练数据稀缺的问题，并验证其在语义分割任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习在历史地图分析中需要大量标注数据，但特定历史地图语料库缺乏此类数据。现有合成数据常缺乏真实感和多样性，无法有效支持学习。

Method: 1. 将原始历史地图语料库的制图风格迁移到矢量数据上，生成无限数量的合成历史地图。2. 提出一种自动深度生成方法和一种手动随机降级技术，以模拟历史地图扫描中常见的视觉不确定性和噪声。3. 利用生成的训练数据集，结合自构建图卷积网络，在同质地图语料库上进行领域自适应语义分割，以定量评估方法的有效性和适用性。

Result: 通过将生成的训练数据集应用于同质地图语料库上的领域自适应语义分割，该方法能够对数据引导方法的影响进行全面评估，证明了其在解决历史地图数据稀缺问题上的有效性和适用性。

Conclusion: 该研究提出的数据引导方法，通过风格迁移和噪声模拟，能够有效生成逼真的合成历史地图数据，从而解决深度学习在历史地图分析中面临的数据稀缺挑战，并为土地覆盖解释等任务提供可行的训练数据。
...
### [63] [Box6D : Zero-shot Category-level 6D Pose Estimation of Warehouse Boxes](https://arxiv.org/abs/2511.15884)
*Yintao Ma,Sajjad Pakdamansavoji,Amir Rasouli,Tongtong Cao*

Main category: cs.CV

TL;DR: Box6D是一种针对仓库存储箱的类别级6D姿态估计方法，它通过单张RGB-D图像推断箱子尺寸并使用类别CAD模板估计姿态，显著提高了精度并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 在杂乱和遮挡环境下，对新物体进行准确高效的6D姿态估计对于机器人操作至关重要。现有方法（基于模型、无模型、类别级）在精度、灵活性和工业实用性方面存在不足，尤其是在仓库场景中。

Method: Box6D是一种类别级6D姿态估计方法，专为仓库存储箱设计。它从单张RGB-D观测中，通过快速二分搜索推断箱体尺寸，并使用类别CAD模板而非实例特定模型估计姿态。通过基于深度的合理性过滤器和提前停止策略，Box6D拒绝不合理的假设，从而降低了计算成本。

Result: Box6D在真实世界存储场景和公共基准测试中表现出有竞争力或更优的6D姿态精度，同时将推理时间缩短了约76%。

Conclusion: Box6D提供了一种针对仓库环境的有效且高效的类别级6D姿态估计解决方案，在保持高精度的同时显著降低了计算开销，克服了现有方法在工业应用中的局限性。
...
### [64] [RB-FT: Rationale-Bootstrapped Fine-Tuning for Video Classification](https://arxiv.org/abs/2511.15923)
...
### [65] [Boosting Medical Visual Understanding From Multi-Granular Language Learning](https://arxiv.org/abs/2511.15943)
*Zihan Li,Yiqing Wang,Sina Farsiu,Paul Kinahan*

Main category: cs.CV

TL;DR: 本文提出多粒度语言学习 (MGLL) 框架，旨在解决现有对比学习模型（如CLIP）在处理多标签、跨粒度图像-文本对齐方面的局限性，尤其在医学影像等复杂领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有图像-文本预训练模型（如CLIP）侧重于单标签、单粒度对齐，这限制了它们在复杂领域（如医学影像）的有效性。在这些领域，图像通常对应多个高层标签和不同粒度的文本描述，需要更精细的多标签和跨粒度对齐能力。

Method: MGLL是一个对比学习框架，用于改进多标签和跨粒度对齐。它利用结构化的多标签监督，整合跨粒度的文本描述，引入带有逐点约束的软标签监督，并通过平滑KL散度确保跨粒度一致性。MGLL作为一个即插即用模块，保持计算效率。

Result: MGLL在自建的大规模多粒度数据集上进行预训练，并在多个下游任务中进行评估，结果表明其性能优于其他现有最先进的方法。

Conclusion: MGLL有效解决了现有视觉-语言模型在处理复杂领域中多标签和跨粒度对齐的挑战，显著提升了视觉理解能力，特别适用于医学影像等需要精细语义理解的场景。
...
### [66] [Automated Interpretable 2D Video Extraction from 3D Echocardiography](https://arxiv.org/abs/2511.15946)
*Milos Vukadinovic,Hirotaka Ieki,Yuki Sahasi,David Ouyang,Bryan He*

Main category: cs.CV

TL;DR: 本文提出一种自动化方法，利用深度学习和启发式规则从三维心脏超声数据中提取标准二维视图，使医生能以熟悉的方式解读三维数据，同时保留诊断准确性和空间校准。


<details>
  <summary>Details</summary>
Motivation: 尽管心脏解剖复杂且三维超声技术日益成熟，但医生仍习惯于解读传统的二维心脏超声图像。三维超声在采集效率和评估非轴向特征方面具有潜力，但缺乏将其转化为医生熟悉二维视图的自动化方法。

Method: 该方法结合深度学习视图分类器、基于解剖地标的下游启发式规则以及心脏病专家提供的启发式规则，从三维心脏超声体数据中重建标准二维超声视图。

Result: 该方法在盲法评估中获得了三位心脏病专家的验证，在来自两家医院的1600个视频中达到96%的准确率。提取的二维视频在检测心脏异常方面通过AI超声模型（EchoPrime和PanEcho）得到验证，并且能够生成临床级别的解剖测量（EchoNet-Measurement）。结果表明，提取的二维视频保留了空间校准和诊断特征。

Conclusion: 所提出的方法允许临床医生从三维超声数据中获取准确的真实世界解读，同时以他们熟悉的二维格式呈现，从而结合了三维扫描的速度和可用性与传统二维解读的便利性。
...
### [67] [Click2Graph: Interactive Panoptic Video Scene Graphs from a Single Click](https://arxiv.org/abs/2511.15948)
...
### [68] [InfoCLIP: Bridging Vision-Language Pretraining and Open-Vocabulary Semantic Segmentation via Information-Theoretic Alignment Transfer](https://arxiv.org/abs/2511.15967)
...
### [69] [Externally Validated Multi-Task Learning via Consistency Regularization Using Differentiable BI-RADS Features for Breast Ultrasound Tumor Segmentation](https://arxiv.org/abs/2511.15968)
...
### [70] [Fairness in Multi-modal Medical Diagnosis with Demonstration Selection](https://arxiv.org/abs/2511.15986)
*Dawei Li,Zijian Gu,Peng Wang,Chuhan Song,Zhen Tan,Mohan Zhang,Tianlong Chen,Yu Tian,Song Wang*

Main category: cs.CV

TL;DR: 多模态大语言模型（MLLMs）在医学图像推理中存在公平性问题。本文提出了一种名为FADS（公平性感知示例选择）的上下文学习方法，通过聚类采样构建人口统计学平衡且语义相关的示例，有效减少了医学图像推理中与性别、种族和民族相关的偏差，同时保持了高准确性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医学图像推理方面潜力巨大，但其在不同人口统计学群体间的公平性是一个主要问题。现有的去偏方法往往依赖于大量标注数据或微调，这对于基础模型来说不切实际。

Method: 研究探索了上下文学习（ICL）作为一种轻量级、免微调的公平性改进方案。发现传统的示例选择（DS）策略因选定示例的人口统计学不平衡而无法确保公平性。为此，提出公平性感知示例选择（FADS），通过基于聚类的采样构建人口统计学平衡且语义相关的示例。

Result: 在多个医学成像基准测试上的实验表明，FADS持续减少了与性别、种族和民族相关的差异，同时保持了强大的准确性。

Conclusion: FADS为公平的医学图像推理提供了一条高效且可扩展的路径，凸显了公平性感知上下文学习作为一种可扩展且数据高效的解决方案，在实现公平医学图像推理方面的潜力。
...
### [71] [Exploiting Inter-Sample Information for Long-tailed Out-of-Distribution Detection](https://arxiv.org/abs/2511.16015)
*Nimeshika Udayangani,Hadi M. Dolatabadi,Sarah Erfani,Christopher Leckie*

Main category: cs.CV

TL;DR: 本文提出了一种基于图的方法，通过利用样本间的关系，显著提高了长尾数据集中OOD检测的性能，特别是在降低假阳性率和提升尾部类别分类准确性方面。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在长尾分布的域内（ID）数据下进行OOD检测时面临挑战，常导致高假阳性率（FPR）和低尾部类别ID分类准确性。现有OOD检测方法在ID尾部类别上的性能不佳。

Method: 该方法首先利用预训练模型的特征空间初始化图结构，并考虑预训练与训练数据激活层分布的差异。接着，引入高斯化处理以缓解激活层偏离标准正态分布的情况。最后，使用图卷积网络（GCNs）优化图表示，以获得适用于长尾OOD检测的特征空间。

Result: 在CIFAR10-LT、CIFAR100-LT和ImageNet-LT三个基准测试中，该方法在FPR和尾部类别ID分类准确性方面，大幅超越了现有最先进的方法。

Conclusion: 通过利用基于图的样本间关系表示，并结合高斯化和GCNs对特征空间进行优化，可以有效解决长尾识别中OOD检测的挑战，显著提高OOD检测性能和尾部类别的分类准确性。
...
### [72] [UniDGF: A Unified Detection-to-Generation Framework for Hierarchical Object Visual Recognition](https://arxiv.org/abs/2511.15984)
...
### [73] [Physically Realistic Sequence-Level Adversarial Clothing for Robust Human-Detection Evasion](https://arxiv.org/abs/2511.16020)
*Dingkun Zhou,Patrick P. K. Chan,Hengxu Wu,Shikang Zheng,Ruiqi Huang,Yuanjie Zhao*

Main category: cs.CV

TL;DR: 本研究提出了一种序列级优化框架，用于生成自然、可打印的对抗性纹理，使服装在整个行走视频中保持对人体检测的有效隐蔽性，并在数字和物理环境中得到验证。


<details>
  <summary>Details</summary>
Motivation: 用于人体检测的深度神经网络容易受到对抗性操纵，在真实监控环境中造成安全和隐私风险。现有可穿戴攻击通常逐帧优化纹理，在运动、姿态变化和服装变形的长视频序列中难以保持隐蔽性。

Method: 将产品图像映射到UV空间，转换为紧凑的调色板和控制点参数化，并使用ICC锁定确保颜色可打印。采用基于物理的人体-服装管道模拟运动、多角度视角、布料动力学和光照变化。使用带有时间权重的变换期望目标来优化控制点，以最小化整个序列的检测置信度。通过热升华打印制作物理服装。

Result: 实验结果表明，该方法实现了强大而稳定的隐蔽性、对视角变化的高鲁棒性以及卓越的跨模型可迁移性。通过热升华打印制作的物理服装在室内和室外录制中均实现了可靠的抑制效果。

Conclusion: 所提出的序列级优化框架能够生成自然、可打印的对抗性纹理，使服装在整个行走视频中保持有效性，并在数字和物理环境中得到验证，证实了其在真实世界中的可行性。
...
### [74] [Towards a Safer and Sustainable Manufacturing Process: Material classification in Laser Cutting Using Deep Learning](https://arxiv.org/abs/2511.16026)
*Mohamed Abdallah Salem,Hamdy Ahmed Ashur,Ahmed Elshinnawy*

Main category: cs.CV

TL;DR: 本文提出一种基于深度学习（卷积神经网络）的材料分类技术，通过分析激光散斑图案来实时监测和控制激光切割过程。该方法在激光颜色变化时仍能保持高精度，为材料感知的激光切割提供了一个鲁棒且准确的解决方案。


<details>
  <summary>Details</summary>
Motivation: 激光切割过程中会产生大量粉尘、烟雾和气溶胶，对环境和工人健康构成风险。实时监测切割过程和识别材料类型对于安全高效的切割至关重要。以往基于散斑传感的材料分类方法在激光颜色改变时可能面临挑战。

Method: 该研究提出一种利用材料表面激光散斑图案进行材料分类的技术，并结合深度学习方法。具体而言，通过训练一个卷积神经网络（CNN）来识别不同材料类型的散斑图案，以实现对激光切割过程的监控和控制。该方法特别解决了激光颜色变化对分类准确性的影响问题。

Result: 实验结果表明，所提出的方法在材料分类上取得了高精度，即使在激光颜色改变的情况下也能保持良好性能。模型在训练集上达到98.30%的准确率，在验证集上达到96.88%的准确率。此外，模型在30种不同材料的3000张新图像上进行了评估，F1-score达到0.9643。

Conclusion: 所提出的方法为使用散斑传感实现材料感知的激光切割提供了一个鲁棒且准确的解决方案，有助于提升激光切割过程的安全性和效率。
...
### [75] [Mixture of Ranks with Degradation-Aware Routing for One-Step Real-World Image Super-Resolution](https://arxiv.org/abs/2511.16024)
...
### [76] [LLMs-based Augmentation for Domain Adaptation in Long-tailed Food Datasets](https://arxiv.org/abs/2511.16037)
*Qing Wang,Chong-Wah Ngo,Ee-Peng Lim,Qianru Sun*

Main category: cs.CV

TL;DR: 本文提出一个利用大型语言模型（LLMs）的框架，通过生成食物文本描述并对图像和文本进行跨域对齐，来解决食物识别中存在的域偏移、长尾分布和细粒度区分困难等挑战。


<details>
  <summary>Details</summary>
Motivation: 食物识别面临多重挑战：训练样本（网络爬取）与用户实拍图片存在视觉差异（域偏移问题）；真实世界食物数据集通常呈长尾分布；不同类别的食物视觉差异细微，难以区分。

Method: 该框架首先利用LLMs解析食物图像，生成食物标题和配料。然后，将生成的文本和来自不同领域的食物图像投影到共享嵌入空间，以最大化它们之间的配对相似性。最后，利用对齐后的多模态特征进行食物识别。

Result: 该简单框架在两个食物数据集上，超越了现有专门针对长尾数据分布、域适应和细粒度分类的方法。

Conclusion: 所提出的基于LLMs的食物识别方法，通过文本生成和多模态特征对齐，有效解决了食物识别中的域偏移、长尾分布和细粒度挑战，取得了优异的性能。
...
### [77] [Crossmodal learning for Crop Canopy Trait Estimation](https://arxiv.org/abs/2511.16031)
*Timilehin T. Ayanlade,Anirudha Powadi,Talukder Z. Jubery,Baskar Ganapathysubramanian,Soumik Sarkar*

Main category: cs.CV

TL;DR: 本文提出一种跨模态学习策略，将高分辨率卫星图像与无人机（UAV）级别的视觉细节结合，以提升作物冠层性状估算能力，弥补卫星图像空间分辨率的不足。


<details>
  <summary>Details</summary>
Motivation: 植物表型技术的发展推动了多传感器平台在作物冠层反射率数据收集中的广泛应用。UAV在作物监测、预测任务中表现出色，而卫星任务虽有效但受限于空间分辨率，影响其在微小区管理中的应用。研究动机在于弥合卫星与UAV传感之间的差距，为卫星图像提供UAV级别的精细细节。

Method: 研究提出了一种跨模态学习策略，通过丰富高分辨率卫星图像的UAV级别视觉细节来估算作物冠层性状。利用美国玉米带五个不同地点收集的84个杂交玉米品种的近似配准卫星-UAV图像对数据集，训练模型学习传感模态间精细的光谱空间对应关系。

Result: 结果表明，从卫星输入生成的类UAV表示在多项下游任务（包括产量和氮素预测）中持续优于真实的卫星图像。这证明了跨模态对应学习在农业监测中弥合卫星和UAV传感之间差距的潜力。

Conclusion: 跨模态对应学习能够有效地将卫星图像与UAV级别的视觉细节相结合，显著提升卫星数据在作物性状估算方面的表现，为农业监测中卫星和UAV传感的融合提供了有效途径。
...
### [78] [AMS-KV: Adaptive KV Caching in Multi-Scale Visual Autoregressive Transformers](https://arxiv.org/abs/2511.16047)
*Boxun Xu,Yu Wang,Zihu Wang,Peng Li*

Main category: cs.CV

TL;DR: 本文提出AMS-KV，一种针对基于下一尺度预测的视觉自回归（VAR）模型的尺度自适应KV缓存策略，显著减少了KV缓存使用和自注意力延迟，从而提高了模型的可扩展性和效率。


<details>
  <summary>Details</summary>
Motivation: 基于下一尺度预测的VAR模型是一种可扩展的图像生成范式，但其KV内存占用随尺度增加而急剧增长，严重限制了模型的扩展性。尽管大型语言模型（LLMs）中的KV缓存已得到广泛研究，但针对下一尺度预测的VAR Transformer的KV缓存设计仍未被充分探索，且面临独特的挑战。

Method: 研究人员首先进行了系统性调查，发现：1) 关注局部尺度的token对生成质量至关重要；2) 为最粗糙的“凝聚尺度”分配少量内存能稳定多尺度图像生成；3) 在缓存效率高的层中，精细尺度间KV相似性强，而在缓存需求高的层中，跨尺度相似性弱。基于这些发现，他们提出了AMS-KV策略，该策略优先存储来自凝聚尺度和局部尺度的KV，并根据跨尺度相似性分析识别缓存需求高的层，以优化KV缓存利用率和计算效率。

Result: 与传统的基于下一尺度预测的VAR模型相比，AMS-KV将KV缓存使用量减少高达84.83%，自注意力延迟降低60.48%。此外，在基线VAR-d30模型在批处理大小为128时出现内存不足（OOM）的情况下，AMS-KV能够稳定扩展到批处理大小为256，并提高了吞吐量。

Conclusion: AMS-KV通过创新的尺度自适应KV缓存策略，成功解决了基于下一尺度预测的VAR模型中KV内存过度增长的瓶颈，显著提升了模型的内存效率、计算速度和可扩展性，使其能够处理更大的批处理量并生成高质量图像。
...
### [79] [CuriGS: Curriculum-Guided Gaussian Splatting for Sparse View Synthesis](https://arxiv.org/abs/2511.16030)
...
### [80] [LiSTAR: Ray-Centric World Models for 4D LiDAR Sequences in Autonomous Driving](https://arxiv.org/abs/2511.16049)
...
TL;DR: LiSTAR是一个新颖的生成式世界模型，直接在4D LiDAR传感器的原始几何上操作，用于生成高保真和可控的4D LiDAR数据，解决了传统方法在处理其独特几何、时间稀疏性和动态场景复杂性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 为自动驾驶创建可扩展的仿真环境，需要合成高保真和可控的4D LiDAR数据。然而，这项任务因传感器独特的球形几何、点云的时间稀疏性以及动态场景的复杂性而极具挑战。

Method: LiSTAR引入了混合圆柱-球形（HCS）表示来保持数据保真度，并通过射线中心变换器（START）与时空注意力机制来捕获稀疏时间数据中的复杂动态。为实现可控合成，LiSTAR提出了一个新颖的4D点云对齐体素布局进行条件化，并开发了离散的掩码生成式START（MaskSTART）框架，以实现高效、高分辨率和布局引导的组合生成。

Result: LiSTAR在4D LiDAR重建、预测和条件生成方面实现了最先进的性能，取得了显著的定量增益：生成MMD大幅减少76%，重建IoU提高32%，预测L1 Med降低50%。

Conclusion: LiSTAR的卓越性能为创建逼真且可控的自动驾驶系统仿真奠定了强大的新基础。
...
### [81] [SpectralTrain: A Universal Framework for Hyperspectral Image Classification](https://arxiv.org/abs/2511.16084)
*Meihua Zhou,Liping Yu,Jiawei Cai,Wai Kin Fung,Ruiguo Hu,Jiarui Zhao,Wenzhuo Liu,Nan Wan*

Main category: cs.CV

TL;DR: 本研究提出SpectralTrain，一个结合课程学习和PCA降维的通用训练框架，显著提高高光谱图像分类的训练效率，实现2-7倍加速并保持高精度。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像（HSI）分类面临数据量大和训练计算成本高的问题，这限制了深度学习模型在实际遥感任务中的应用。

Method: 引入SpectralTrain框架，该框架通过将课程学习（CL）与基于主成分分析（PCA）的谱降采样相结合，逐步引入光谱复杂性，同时保留关键信息，从而提高学习效率。该框架独立于具体模型架构、优化器或损失函数，与多种模型兼容。

Result: 在Indian Pines、Salinas-A和CloudPatch-7三个基准数据集上的实验表明，训练时间一致减少2-7倍，且精度损失小到中等。该框架展现出强大的泛化能力，并在云分类应用中显示出潜力。

Conclusion: 训练策略优化（如SpectralTrain）是高光谱图像模型中架构设计的有效补充，对于气候相关遥感任务具有重要意义。
...
### [82] [LEGO-SLAM: Language-Embedded Gaussian Optimization SLAM](https://arxiv.org/abs/2511.16144)
...
### [83] [Rad-GS: Radar-Vision Integration for 3D Gaussian Splatting SLAM in Outdoor Environments](https://arxiv.org/abs/2511.16091)
*Renxiang Xiao,Wei Liu,Yuanfan Zhang,Yushuai Chen,Jinming Chen,Zilu Wang,Liang Hu*

Main category: cs.CV

TL;DR: Rad-GS是一个4D雷达-相机SLAM系统，利用3D高斯作为空间表示，结合雷达数据进行动态物体遮罩和图像细化，并通过八叉树结构优化内存，实现了千米级户外环境的鲁棒映射和重建。


<details>
  <summary>Details</summary>
Motivation: 传统的3D高斯方法通常依赖相机或激光雷达输入，而毫米波雷达在户外环境具有鲁棒性。本文旨在探索利用4D毫米波雷达实现大规模、鲁棒的户外映射和SLAM系统。

Method: Rad-GS系统采用3D高斯作为可微分空间表示。它结合原始雷达点云和多普勒信息以及几何增强点云，指导同步图像中的动态物体遮罩。此外，利用非同步图像帧全局优化3D高斯表示，以增强纹理一致性和新视角合成保真度。系统还引入全局八叉树结构和目标高斯基元管理策略，以抑制噪声并显著降低大规模环境中的内存消耗。

Result: Rad-GS在性能上与基于相机或激光雷达输入的传统3D高斯方法相当，证明了使用4D毫米波雷达进行鲁棒户外映射的可行性。在千米级真实世界重建中验证了其在大规模场景重建方面的潜力。

Conclusion: Rad-GS是一个有效的4D雷达-相机SLAM系统，能够实现千米级户外环境的鲁棒映射和大规模场景重建，突显了4D毫米波雷达在该领域应用的巨大潜力。
...
### [84] [VideoSeg-R1:Reasoning Video Object Segmentation via Reinforcement Learning](https://arxiv.org/abs/2511.16077)
*Zishan Xu,Yifu Guo,Yuquan Lu,Fengyu Yang,Junxin Li*

Main category: cs.CV

TL;DR: VideoSeg-R1是首个将强化学习引入视频推理分割的框架，采用解耦架构，结合分层采样、推理模型和SAM2/XMem进行分割，通过任务难度感知机制提高效率和准确性，并在复杂视频推理和分割任务中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统的视频推理分割方法依赖于监督微调，这限制了其对分布外场景的泛化能力，并且缺乏显式的推理过程。

Method: 本文提出了VideoSeg-R1框架，首次将强化学习引入视频推理分割。它采用解耦架构，将任务表述为联合指代图像分割和视频掩码传播。该框架包含三个阶段：1) 分层文本引导帧采样器，模拟人类注意力；2) 推理模型，生成空间线索和显式推理链；3) 分割-传播阶段，使用SAM2和XMem。此外，引入了任务难度感知机制，自适应地控制推理长度以提高效率和准确性。

Result: 在多个基准测试上的广泛评估表明，VideoSeg-R1在复杂的视频推理和分割任务中取得了最先进的性能。

Conclusion: VideoSeg-R1是一个创新且高效的视频推理分割框架，通过引入强化学习和解耦架构，克服了传统方法的局限性，并在性能上达到了新的高度。
...
### [85] [Decoupling Complexity from Scale in Latent Diffusion Model](https://arxiv.org/abs/2511.16117)
*Tianxiong Zhong,Xingye Tian,Xuebo Wang,Boyuan Jiang,Xin Tao,Pengfei Wan*

Main category: cs.CV

TL;DR: DCS-LDM是一种新型的潜在扩散模型，它将视觉生成中的信息复杂度与尺度解耦，通过分层、尺度无关的潜在空间实现灵活的任意分辨率和帧率生成。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型（LDM）将尺度与内容复杂度耦合，用更多潜在token表示更高分辨率或帧率的数据。然而，表示视觉数据所需的潜在容量主要取决于内容复杂度，尺度仅是上限，这促使研究者寻求解耦。

Method: 本文提出了DCS-LDM，构建了一个分层、尺度无关的潜在空间，通过多级token建模样本复杂度。该方法支持从固定的潜在表示解码到任意分辨率和帧率，并通过在不同层分解结构和详细信息，实现了渐进式的从粗到精生成范式。

Result: 实验结果表明，DCS-LDM在提供灵活的跨尺度和视觉质量生成的同时，实现了与现有最先进方法相当的性能。

Conclusion: DCS-LDM通过解耦信息复杂度和尺度，提供了一种新的视觉生成范式，实现了计算与质量的灵活权衡，并支持渐进式生成。
...
### [86] [T2T-VICL: Unlocking the Boundaries of Cross-Task Visual In-Context Learning via Implicit Text-Driven VLMs](https://arxiv.org/abs/2511.16107)
*Shao-Jun Xia,Huixin Zhang,Zhengzhong Tu*

Main category: cs.CV

TL;DR: 本文提出T2T-VICL，一个针对视觉语言模型（VLM）的完全协作式管道，旨在探索跨任务视觉上下文学习（VICL）的潜力，通过生成描述任务差异的文本提示和结合感知分数推理，在多个跨任务场景中取得了优异表现。


<details>
  <summary>Details</summary>
Motivation: 当前视觉上下文学习（VICL）在统一视觉语言模型（VLM）中展现出解决下游任务的巨大潜力。然而，当视觉提示和目标图像来源于不同的视觉任务时，VLM是否仍能实现VICL是一个未解决的问题。

Method: 1. 提出了一个完全协作式的管道T2T-VICL，用于VLM的跨任务VICL研究。2. 设计了一种机制来生成和选择最佳隐式描述两个不同低级视觉任务之间差异的文本提示。3. 构建了首个跨任务VICL数据集。4. 提出了一个新颖的推理框架，该框架结合了基于感知分数的推理与传统评估指标来执行跨任务VICL。

Result: 该方法在九个跨任务场景中取得了顶尖结果，并在另外十个场景中取得了第二梯队性能。

Conclusion: 该研究解锁了VLM中跨任务VICL的边界，展示了其在处理不同视觉任务提示和目标图像时的强大潜力。
...
### [87] [Can MLLMs Read the Room? A Multimodal Benchmark for Assessing Deception in Multi-Party Social Interactions](https://arxiv.org/abs/2511.16221)
...
### [88] [How Noise Benefits AI-generated Image Detection](https://arxiv.org/abs/2511.16136)
*Jiazhen Yan,Ziqiang Li,Fan Wang,Kai Zeng,Zhangjie Fu*

Main category: cs.CV

TL;DR: 生成模型使真实和合成图像难以区分，现有检测器在分布外泛化方面表现不佳，原因在于利用了虚假捷径。本文提出PiN-CLIP，通过在特征空间注入正激励噪声来抑制捷径并增强稳定取证线索，从而提高AI生成图像检测的鲁棒性和泛化能力，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展使得真实和合成图像越来越难以区分。尽管在检测AI生成图像方面做了大量努力，但现有方法在分布外泛化方面仍面临挑战，这源于训练过程中利用了虚假捷径。研究观察到小的特征空间扰动可以缓解捷径主导，但需要一种更可控的方法来解决这个问题。

Method: 本文提出PiN-CLIP（CLIP的正激励噪声），它在变分正激励原则下联合训练一个噪声生成器和一个检测网络。具体而言，通过视觉和类别语义特征的交叉注意力融合，在特征空间构建正激励噪声。在优化过程中，将噪声注入特征空间以微调视觉编码器，从而抑制对捷径敏感的方向，同时放大稳定的取证线索，以提取更鲁棒和泛化的伪影表示。

Result: 在包含由42种不同生成模型生成的合成图像的开放世界数据集上进行了比较实验。本文方法取得了新的最先进性能，比现有方法平均准确率显著提高了5.4。

Conclusion: PiN-CLIP通过引入正激励噪声在特征空间中抑制虚假捷径并放大稳定线索，有效解决了AI生成图像检测中的分布外泛化挑战。该方法能够提取更鲁棒和泛化的伪影表示，从而在广泛的生成模型上实现显著的性能提升。
...
### [89] [Degradation-Aware Hierarchical Termination for Blind Quality Enhancement of Compressed Video](https://arxiv.org/abs/2511.16137)
*Li Yu,Yingbo Zhao,Shiyu Wu,Siyue Yu,Moncef Gabbouj,Qingshan Liu*

Main category: cs.CV

TL;DR: 本文提出了一种针对压缩视频质量增强的盲方法，通过解耦和提取高维多尺度降级表示来指导去伪影，并引入分层终止机制以动态调整计算需求，显著提升了性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有压缩视频质量增强（QECV）方法大多依赖于已知的量化参数（QP）或其盲方法缺乏空间降级细节。此外，无论盲方法还是非盲方法，通常采用统一的网络架构，忽略了不同压缩级别固有的计算需求差异。

Method: 本文提出了一个预训练的降级表示学习（DRL）模块，用于从视频内容中解耦并提取高维、多尺度的降级表示，以指导伪影去除。此外，引入了一种分层终止机制，根据压缩级别动态调整伪影减少阶段的数量。

Result: 实验结果表明，在QP=22时，所提出的方法比现有最先进的盲方法实现了110%的PSNR提升（从0.31 dB到0.65 dB）。此外，分层终止机制使QP=22时的平均推理时间比QP=42时减少了一半。

Conclusion: 所提出的方法通过学习多尺度降级表示和动态调整网络架构，显著提高了盲压缩视频质量增强的性能和计算效率，有效解决了现有方法的局限性。
...
### [90] [Clustered Error Correction with Grouped 4D Gaussian Splatting](https://arxiv.org/abs/2511.16112)
...
### [91] [Thinking-while-Generating: Interleaving Textual Reasoning throughout Visual Generation](https://arxiv.org/abs/2511.16671)
*Ziyu Guo,Renrui Zhang,Hongyu Li,Manyuan Zhang,Xinyan Chen,Sifan Wang,Yan Feng,Peng Pei,Pheng-Ann Heng*

Main category: cs.CV

TL;DR: 本研究提出TwiG框架，首次实现在视觉生成过程中交错进行文本推理，以动态引导和反思生成内容，从而提升视觉输出的上下文感知和语义丰富性，并通过多种策略进行验证。


<details>
  <summary>Details</summary>
Motivation: 当前视觉生成方法在整合推理能力时，通常将文本推理作为生成前规划或生成后优化，但在生成过程中缺乏即时多模态交互。

Method: 引入了“生成时思考”（TwiG）框架，这是一个交错式框架，允许文本推理在视觉生成过程中共同演化。当视觉内容逐步生成时，文本推理会交错进行，既能指导即将生成的局部区域，也能反思已合成的区域。研究探索了三种候选策略：零样本提示、在我们整理的TwiG-50K数据集上进行监督微调（SFT），以及通过定制的TwiG-GRPO策略进行强化学习（RL）。

Result: TwiG的动态交互能够产生更具上下文感知和语义更丰富的视觉输出。三种调查的策略为交错推理的动态性提供了独特的见解。

Conclusion: 本研究展示了TwiG框架的潜力，并希望激发未来在视觉生成中交错文本推理以增强生成效果的研究。
...
### [92] [VTinker: Guided Flow Upsampling and Texture Mapping for High-Resolution Video Frame Interpolation](https://arxiv.org/abs/2511.16124)
...
### [93] [TimeViper: A Hybrid Mamba-Transformer Vision-Language Model for Efficient Long Video Understanding](https://arxiv.org/abs/2511.16595)
*Boshen Xu,Zihan Xiao,Jiaze Li,Jianzhong Ju,Zhenbo Luo,Jian Luan,Qin Jin*

Main category: cs.CV

TL;DR: TimeViper是一个混合Mamba-Transformer模型，通过引入TransV模块有效处理长视频理解，解决了视觉令牌冗余问题，并能在保持高性能的同时处理超长视频。


<details>
  <summary>Details</summary>
Motivation: 长视频理解需要高效的模型架构和处理扩展时间上下文的有效机制。研究者观察到混合模型中存在视觉到文本信息聚合现象，导致严重的视觉令牌冗余。

Method: TimeViper采用混合Mamba-Transformer骨干网络，结合了状态空间模型的效率和注意力机制的表达能力。为解决视觉令牌冗余，提出了TransV（令牌信息传输模块），将视觉令牌传输并压缩到指令令牌中，同时保持多模态理解能力。

Result: TimeViper能够处理超过10,000帧的长达数小时的视频。在多个基准测试中，TimeViper在扩展帧数的同时与最先进的模型竞争。研究还分析了Mamba和Transformer层的注意力行为，为混合模型的可解释性提供了新见解。

Conclusion: 这项工作代表了开发、解释和压缩混合Mamba-Transformer架构以实现长视频理解的初步步骤。
...
### [94] [A Spatial Semantics and Continuity Perception Attention for Remote Sensing Water Body Change Detection](https://arxiv.org/abs/2511.16143)
*Quanqing Ma,Jiaen Chen,Peng Wang,Yao Zheng,Qingzhan Zhao,Yuchen Zheng*

Main category: cs.CV

TL;DR: 该研究针对遥感水体变化检测中高空间分辨率数据集的稀缺性以及现有深度学习方法未能充分利用空间语义和结构信息的问题，提出了一个新的高分辨率数据集HSRW-CD和一个空间语义与连续性感知（SSCP）注意力模块，显著提高了水体变化检测的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前遥感水体变化检测（WBCD）面临两个主要挑战：一是缺乏高空间分辨率数据集，限制了其在需要精确制图的城市和农村地区的应用；二是现有的深度学习方法未能全面利用深度特征中的空间语义和结构信息，导致判别能力不足。

Method: 本研究首先提出了一个新的高空间分辨率（高于3米）WBCD数据集HSRW-CD，包含大量覆盖多种水体类型的图像对。其次，设计了一个空间语义与连续性感知（SSCP）注意力模块，以充分利用WBCD网络中深度特征的空间语义和结构。SSCP包含三个组件：多语义空间注意力（MSA）增强水体特征的空间语义并提供精确的空间语义先验；结构关系感知全局注意力（SRGA）进一步提取空间结构以学习水体的空间连续性；通道自注意力（CSA）利用MSA和SRGA的空间语义和结构先验来计算通道间的相似性。SSCP被设计为可即插即用的模块，可集成到现有WBCD模型中。

Result: 在提出的HSRW-CD和Water-CD数据集上进行了大量实验，验证了SSCP模块的有效性和泛化能力。实验结果表明，SSCP显著提高了水体的判别能力。

Conclusion: 本研究通过构建高空间分辨率数据集HSRW-CD和设计SSCP注意力模块，有效解决了遥感水体变化检测中数据稀缺和信息利用不充分的问题。SSCP模块能够全面利用深度特征中的空间语义和结构信息，显著提升了水体变化检测的准确性和泛化能力。
...
### [95] [SurvAgent: Hierarchical CoT-Enhanced Case Banking and Dichotomy-Based Multi-Agent System for Multimodal Survival Prediction](https://arxiv.org/abs/2511.16635)
...
### [96] [Video2Layout: Recall and Reconstruct Metric-Grounded Cognitive Map for Spatial Reasoning](https://arxiv.org/abs/2511.16160)
*Yibin Huang,Wang Xu,Wanyue Zhang,Helu Zhi,Jingjing Huang,Yangbin Xu,Yangang Sun,Conghui Zhu,Tiejun Zhao*

Main category: cs.CV

TL;DR: 本文提出Video2Layout框架，通过连续物体边界坐标从视频中重建基于度量的空间布局，以提升多模态大语言模型的空间智能，并优于传统基于网格的方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于网格的认知地图依赖离散栅格表示，限制了多模态大语言模型（MLLMs）进行细粒度空间推理的能力，无法进行定量的空间计算，导致自然语言描述空间关系时存在固有的模糊性。

Method: Video2Layout框架采用连续物体边界坐标来量化物体间的物理距离和物体大小。该方法包括两个核心阶段：1) 在监督微调阶段，从AI2THOR模拟器构建高质量数据集，使模型学习从视觉输入到精确边界坐标的映射；2) 随后通过强化微调阶段增强模型的真实世界泛化能力。此外，本文引入了QVS-Bench诊断基准来评估认知地图精度与图像数量以及图像输入数量如何影响空间推理精度。

Result: 在QVS-Bench和主流空间推理基准上进行评估，本文提出的模型V2LO-7B比基于网格地图训练的模型平均提高了4.92%，验证了该方法的优越性。

Conclusion: 通过采用连续边界坐标而非离散网格表示，Video2Layout框架能够从视频中重建基于度量的空间布局，显著提升了多模态大语言模型的空间推理能力，有效缓解了空间关系描述中的模糊性，并展现出优于传统方法的性能。
...
### [97] [Pluggable Pruning with Contiguous Layer Distillation for Diffusion Transformers](https://arxiv.org/abs/2511.16156)
*Jian Ma,Qirong Peng,Xujie Zhu,Peixing Xie,Chen Chen,Haonan Lu*

Main category: cs.CV

TL;DR: 本文提出PPCL，一个针对Diffusion Transformers (DiTs) 的灵活结构化剪枝框架，通过识别冗余层并结合师生交替蒸馏，在保持高质量图像生成的同时，将模型参数减少50%，性能下降不到3%，适用于资源受限环境。


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) 在图像生成方面表现出色，但其庞大的参数量导致高计算成本，阻碍了在资源受限环境中的部署。

Method: 首先，通过线性探测机制结合相似性指标的一阶微分趋势分析，识别冗余层区间。其次，提出一种即插即用的师生交替蒸馏方案，用于在单个训练阶段内整合深度和宽度剪枝。该蒸馏框架支持跨不同剪枝率的灵活知识迁移，无需为每种配置重新训练。

Result: 在多个多模态Diffusion Transformer架构模型上进行的大量实验表明，PPCL相较于完整模型，参数量减少了50%，而关键目标指标下降不到3%。该方法在实现更高压缩率的同时，保持了高质量的图像生成能力。

Conclusion: PPCL有效解决了DiT模型参数量大、计算成本高的问题，在资源受限的环境中，通过大幅压缩模型尺寸同时保持高图像生成质量，展现出良好的适用性。
...
### [98] [Real-Time 3D Object Detection with Inference-Aligned Learning](https://arxiv.org/abs/2511.16140)
*Chenyu Zhao,Xianwei Zheng,Zimin Xia,Linwei Yue,Nan Xue*

Main category: cs.CV

TL;DR: 本文提出了一种名为SR3D的实时3D目标检测框架，用于室内点云。它通过引入空间优先级最优传输分配和排名感知自蒸馏方案，有效弥合了训练与推理之间的差距，显著提高了检测精度并保持了实时速度。


<details>
  <summary>Details</summary>
Motivation: 现有的3D目标检测器在训练过程中缺乏空间可靠性和排名感知，这与推理时使用的基于排名的预测选择相冲突，导致训练-推理差距。这种差距阻碍了模型学习与推理行为一致的表示。

Method: SR3D框架包含两个核心组件：1) 一种新颖的空间优先级最优传输分配，动态地强调定位良好且空间可靠的样本；2) 一种排名感知自适应自蒸馏方案，通过自蒸馏范式自适应地注入排名感知。

Result: 在ScanNet V2和SUN RGB-D数据集上的大量实验表明，SR3D有效弥合了训练-推理差距，在保持实时速度的同时，准确性显著优于现有方法。

Conclusion: SR3D通过其独特的空间优先级最优传输分配和排名感知自适应自蒸馏方案，成功解决了室内点云3D目标检测中的训练-推理差距问题，从而在准确性和实时性能方面取得了显著提升。
...
### [99] [An Image Is Worth Ten Thousand Words: Verbose-Text Induction Attacks on VLMs](https://arxiv.org/abs/2511.16163)
*Zhi Luo,Zenghui Yuan,Wenqi Wei,Daizong Liu,Pan Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的冗长文本诱导攻击（VTIA），通过向图像注入难以察觉的对抗性扰动，使视觉-语言模型（VLMs）生成冗长且信息密度低的输出，从而增加其资源消耗。


<details>
  <summary>Details</summary>
Motivation: 随着视觉-语言模型（VLMs）的成功，其部署效率问题日益突出，尤其是生成过程中消耗的token数量。现有研究表明，特定输入会导致VLMs生成冗长且信息密度低的输出，显著增加能耗、延迟和token成本。然而，现有方法通过延迟EOS token隐式延长输出，缺乏稳定性和可控性，未能直接以最大化输出token长度为显式优化目标。

Method: 本文提出了一个两阶段的冗长文本诱导攻击（VTIA）框架。首先，通过对抗性提示搜索，利用强化学习策略自动识别能够诱导VLM中LLM组件产生冗长输出的对抗性提示。其次，进行视觉对齐扰动优化，在输入图像上制作对抗性样本，最大化扰动图像的视觉嵌入与对抗性提示的视觉嵌入之间的相似性，从而构建触发冗长文本生成的恶意图像。

Result: 在四种流行的VLMs上进行的综合实验表明，本文提出的方法在有效性、效率和泛化能力方面均取得了显著优势。

Conclusion: VTIA是一种通过注入对抗性扰动，成功且稳定地诱导VLMs生成冗长输出的新颖攻击方法，揭示了VLMs在部署效率方面的潜在漏洞。
...
### [100] [Reasoning Guided Embeddings: Leveraging MLLM Reasoning for Improved Multimodal Retrieval](https://arxiv.org/abs/2511.16150)
...
### [101] [Simba: Towards High-Fidelity and Geometrically-Consistent Point Cloud Completion via Transformation Diffusion](https://arxiv.org/abs/2511.16161)
*Lirui Zhang,Zhengkai Zhao,Zhi Zuo,Pan Gao,Jie Qin*

Main category: cs.CV

TL;DR: Simba提出了一种新颖的框架，通过将点变换回归重构为分布学习问题，并结合扩散模型和对称先验，实现鲁棒且高保真度的点云补全，同时利用分层Mamba架构进行上采样。


<details>
  <summary>Details</summary>
Motivation: 现有方法在点云补全中面临两大挑战：(1) 基于回归的方法容易过拟合，倾向于记忆特定实例的变换而非学习通用的几何先验；(2) 它们对点级变换回归的依赖导致对输入噪声高度敏感，严重影响鲁棒性和泛化能力，难以同时保留细节和确保全局结构完整性。

Method: Simba将点级变换回归重新定义为分布学习问题。它将对称先验与扩散模型的强大生成能力相结合，以避免实例特定的记忆并捕获鲁棒的几何结构。此外，引入了分层Mamba架构以实现高保真度上采样。

Result: 在PCN、ShapeNet和KITTI基准测试中进行了广泛实验，验证了Simba达到了最先进（SOTA）的性能。

Conclusion: Simba框架通过将变换回归转化为分布学习，并结合扩散模型和对称先验，有效解决了现有方法的局限性，实现了鲁棒且泛化能力强的点云补全，同时通过Mamba架构保证了高保真度的细节恢复。
...
### [102] [Layer-wise Noise Guided Selective Wavelet Reconstruction for Robust Medical Image Segmentation](https://arxiv.org/abs/2511.16162)
*Yuting Lu,Ziliang Wang,Weixin Xu,Wei Zhang,Yongqiang Zhao,Yang Yu,Xiaohong Zhang*

Main category: cs.CV

TL;DR: 本文提出LNG-SWR方法，通过层级噪声引导和选择性小波重建，提高医学图像分割模型在分布偏移和扰动下的鲁棒性，且能与对抗训练结合或独立使用，具有低成本和优异性能。


<details>
  <summary>Details</summary>
Motivation: 临床部署中，分割模型需在分布偏移和扰动下保持稳定。现有对抗训练(AT)方案存在“干净-鲁棒性”权衡及高训练/调优成本，限制了医学成像领域的可扩展性和可维护性。

Method: 提出“层级噪声引导选择性小波重建 (LNG-SWR)”框架。训练时，在多层注入微小零均值噪声以学习频率偏置先验，引导表示远离噪声敏感方向。然后，在输入/特征分支上应用先验引导的选择性小波重建，实现频率自适应：抑制噪声敏感频段，增强方向结构和形状线索，稳定边界响应，同时保持频谱一致性。该框架与骨干网络无关，推理开销低，可作为对抗训练的插件增强，也可独立提升鲁棒性。

Result: 在CT和超声数据集上，统一协议（PGD-$L_{\infty}/L_{2}$ 和 SSAH）下，LNG-SWR在干净Dice/IoU上持续取得增益，并显著减少强攻击下的性能下降。将LNG-SWR与对抗训练结合可获得叠加增益，进一步提升鲁棒性而不牺牲干净准确度。

Conclusion: LNG-SWR为对抗训练和标准训练方案下的鲁棒医学图像分割提供了一种简单、有效且工程友好的途径。
...
### [103] [When Alignment Fails: Multimodal Adversarial Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.16203)
*Yuping Yan,Yuhan Xie,Yinxin Zhang,Lingjuan Lyu,Yaochu Jin*

Main category: cs.CV

TL;DR: 本文介绍了VLA-Fool，一项针对具身视觉-语言-动作（VLA）模型的多模态对抗性鲁棒性研究，涵盖文本、视觉和跨模态攻击，并发现即使是微小的多模态扰动也会导致显著的行为偏差，揭示了具身多模态对齐的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 尽管VLA模型在具身环境中取得了显著进展，但其对抗性鲁棒性（尤其是在真实多模态和黑盒条件下）仍未被充分探索。现有研究主要关注单模态扰动，忽视了对具身推理和决策有根本影响的跨模态错位问题。

Method: 本文引入了VLA-Fool，一个全面的多模态对抗性鲁棒性研究框架，涵盖白盒和黑盒设置。它统一了三个层次的多模态对抗性攻击：1) 基于梯度和提示的文本扰动；2) 通过补丁和噪声扭曲的视觉扰动；3) 故意破坏感知和指令之间语义对应关系的跨模态错位攻击。此外，研究还将VLA感知的语义空间整合到语言提示中，开发了首个自动生成且语义引导的提示框架。

Result: 在LIBERO基准上使用微调的OpenVLA模型进行的实验表明，即使是微小的多模态扰动也能导致显著的行为偏差。

Conclusion: 实验结果证明了具身多模态对齐的脆弱性，即VLA模型容易受到多模态对抗性扰动的影响。
...
### [104] [EvoVLA: Self-Evolving Vision-Language-Action Model](https://arxiv.org/abs/2511.16166)
*Zeting Liu,Zida Yang,Zeyu Zhang,Hao Tang*

Main category: cs.CV

TL;DR: EvoVLA是一个自监督的VLA框架，通过引入阶段对齐奖励、基于姿态的物体探索和长程记忆，有效解决了长程机器人操作中的“阶段幻觉”问题，显著提升了任务成功率和样本效率，并在模拟和真实世界中表现出色。


<details>
  <summary>Details</summary>
Motivation: 尽管零样本泛化和模拟到现实世界迁移取得了进展，但长程机器人操作对视觉-语言-动作（VLA）模型来说仍然具有挑战性。当前的VLA模型存在“阶段幻觉”问题，即智能体利用粗略的评估信号来走捷径完成多步骤任务，报告高进度但并未真正完成任务。

Method: EvoVLA是一个自监督的VLA框架，包含三个互补组件：1. 阶段对齐奖励（SAR）：使用三元组对比学习和Gemini生成的困难负样本来防止视觉捷径。2. 基于姿态的物体探索（POE）：将好奇心建立在物体-抓手相对姿态而非原始像素上。3. 长程记忆：采用选择性上下文保留和门控融合，以稳定长时间运行中的内在塑造。

Result: 在Discoverse-L长程操作基准测试中，EvoVLA将平均任务成功率比最强基线（OpenVLA-OFT）提高了10.2个百分点，达到69.2%。它还实现了1.5倍的样本效率提升，并将阶段幻觉从38.5%降低到14.8%。在物理机器人上的真实世界部署中，EvoVLA在四项操作任务中达到了54.6%的平均成功率，比OpenVLA-OFT高出11个百分点，展示了有效的模拟到现实世界迁移和强大的泛化能力。

Conclusion: EvoVLA通过其创新组件有效解决了长程机器人操作中的阶段幻觉问题，显著提高了任务成功率、样本效率和泛化能力，并在模拟和真实世界部署中均表现出卓越性能。
...
### [105] [Target Refocusing via Attention Redistribution for Open-Vocabulary Semantic Segmentation: An Explainability Perspective](https://arxiv.org/abs/2511.16170)
...
### [106] [Domain-Shared Learning and Gradual Alignment for Unsupervised Domain Adaptation Visible-Infrared Person Re-Identification](https://arxiv.org/abs/2511.16184)
*Nianchang Huang,Yi Xu,Ruida Xi,Ruida Xi,Qiang Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种无监督域适应可见光-红外行人重识别（UDA-VI-ReID）方法，通过两阶段模型（DSLGA）和新的测试方法（CMDA-XD）来解决公共数据集与真实世界数据之间的模态差异，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有可见光-红外行人重识别（VI-ReID）算法在公共数据集上表现出色，但在实际应用中因公共数据与真实世界数据之间的差异而面临挑战。研究旨在开发一种无监督域适应方法，在不影响准确性且无需新样本标注的情况下，将从公共数据中学到的知识迁移到真实世界数据中。

Method: 研究首先分析了UDA-VI-ReID中的两个基本挑战：域间模态差异和域内模态差异。然后，设计了一个新颖的两阶段模型，即域共享学习和渐进对齐（DSLGA）。第一阶段（预训练）引入域共享学习策略（DSLS）以缓解域间模态差异导致的无效预训练。第二阶段（微调）设计了渐进对齐策略（GAS），通过从簇到整体的对齐方式处理由大的域内模态差异引起的跨模态对齐挑战。最后，构建了一种新的UDA-VI-ReID测试方法CMDA-XD。

Result: 大量的实验证明，本文提出的方法显著优于现有的VI-ReID域适应方法，甚至在各种设置下超越了一些有监督方法。

Conclusion: 所提出的DSLGA模型和CMDA-XD测试方法能有效应对UDA-VI-ReID中的域间和域内模态差异挑战，在将知识从公共数据迁移到真实世界数据方面取得了卓越性能。
...
### [107] [PrIntMesh: Precise Intersection Surfaces for 3D Organ Mesh Reconstruction](https://arxiv.org/abs/2511.16186)
*Deniz Sayin Mercadier,Hieu Le,Yihong Chen,Jiancheng Yang,Udaranga Wickramasinghe,Pascal Fua*

Main category: cs.CV

TL;DR: PrIntMesh是一个基于模板的深度学习框架，它将器官重建为一个统一的系统，通过联合变形子结构来保持拓扑结构和解剖学合理性，即使在有限数据下也表现出色。


<details>
  <summary>Details</summary>
Motivation: 大多数深度学习方法将器官子结构独立处理，导致重建结果不符合解剖学原理。然而，人体器官是由相互连接的子结构组成，它们的几何形状和空间关系是相互约束的。

Method: PrIntMesh从一个连接的模板开始，联合变形所有子结构以匹配患者特定的解剖结构。它明确地保留了内部边界，并强制生成平滑、无伪影的表面，从而保持了拓扑结构。

Result: 该方法在心脏、海马体和肺部重建中表现出高几何精度、正确的拓扑结构和鲁棒的性能，即使在训练数据有限或嘈杂的情况下也是如此。与基于体素和曲面的方法相比，PrIntMesh能更好地重建共享界面，保持结构一致性，并提供了一种适用于临床使用的数据高效解决方案。

Conclusion: PrIntMesh提供了一个有效且数据高效的解决方案，用于将器官重建为统一的系统，解决了现有方法在解剖学合理性和结构一致性方面的不足，适用于临床应用。
...
### [108] [Mantis: A Versatile Vision-Language-Action Model with Disentangled Visual Foresight](https://arxiv.org/abs/2511.16175)
...
### [109] [Building temporally coherent 3D maps with VGGT for memory-efficient Semantic SLAM](https://arxiv.org/abs/2511.16282)
*Gergely Dinya,Péter Halász,András Lőrincz,Kristóf Karacs,Anna Gelencsér-Horváth*

Main category: cs.CV

TL;DR: 本文提出了一个基于视觉门控生成式Transformer（VGGT）的快速时空场景理解框架，通过滑动窗口处理和子图对齐实现连续3D场景更新和环境变化检测，适用于辅助导航等实时应用。


<details>
  <summary>Details</summary>
Motivation: 开发一个高效、接近实时的时空场景理解框架，以支持包括辅助导航在内的应用，并克服VGGT在高内存需求方面的挑战。

Method: 该方法基于视觉门控生成式Transformer（VGGT），通过滑动窗口处理图像流并对齐子图，实现3D场景表示的连续更新，同时解决VGGT的高内存需求。它利用VGGT的跟踪头将2D语义实例掩码聚合成3D对象。系统存储时间戳和实例级身份，以确保时间一致性和更丰富的上下文推理，从而能够检测环境变化。

Result: 该方法在知名基准和专为辅助导航场景设计的自定义数据集上进行了评估。结果表明该框架适用于真实世界场景。

Conclusion: 该框架能够实现快速的时空场景理解，具有环境变化检测能力，并被证明适用于真实世界的应用场景。
...
### [110] [Mem-MLP: Real-Time 3D Human Motion Generation from Sparse Inputs](https://arxiv.org/abs/2511.16264)
*Sinan Mutlu,Georgios F. Angelis,Savas Ozkan,Paul Wisbey,Anastasios Drosou,Mete Ozay*

Main category: cs.CV

TL;DR: 本文提出了一种基于残差连接多层感知器（MLP）和新颖记忆模块（Memory-Block）的方法，通过多任务学习从稀疏传感器输入中实现全身追踪。该方法显著降低了预测误差，并在移动HMD上达到72 FPS，提高了精度与运行时间的权衡。


<details>
  <summary>Details</summary>
Motivation: 沉浸式AR/VR应用需要逼真流畅的全身追踪。现有系统主要通过头戴设备（HMD）和控制器追踪头部和手部，导致3D全身重建不完整。从有限传感器收集的稀疏输入生成全身运动是潜在的解决方案。

Method: 本文提出了一种基于多层感知器（MLP）骨干的新方法，该骨干通过残差连接和一种名为“记忆模块”（Memory-Block）的新型神经网络组件进行增强。记忆模块用可训练的代码向量表示缺失的传感器数据，并与先前时间实例的稀疏信号结合，以提高时间一致性。此外，该解决方案被表述为一个多任务学习问题，使MLP骨干能够学习鲁棒的表示以提高准确性。

Result: 实验结果表明，该方法通过显著减少预测误差，优于现有最先进的基线。此外，它在移动HMD上实现了72 FPS，最终改善了精度与运行时间的权衡。

Conclusion: 本文提出的方法为AR/VR应用中从稀疏输入进行全身追踪提供了一个高精度、高效率的解决方案，有效解决了现有系统追踪不完整的问题，并提高了沉浸式体验的质量。
...
### [111] [Unsupervised Image Classification with Adaptive Nearest Neighbor Selection and Cluster Ensembles](https://arxiv.org/abs/2511.16213)
...
### [112] [SwiTrack: Tri-State Switch for Cross-Modal Object Tracking](https://arxiv.org/abs/2511.16227)
...
### [113] [Upsample Anything: A Simple and Hard to Beat Baseline for Feature Upsampling](https://arxiv.org/abs/2511.16301)
*Minseok Seo,Mark Hamilton,Changick Kim*

Main category: cs.CV

TL;DR: 本文提出了“Upsample Anything”框架，一个轻量级、无需训练的测试时优化（TTO）方法，能将视觉基础模型的低分辨率特征恢复为高分辨率、像素级输出，并在多个像素级任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型的特征通常被大幅降采样（如14x/16x），限制了它们在像素级应用中的直接使用。现有的特征上采样方法依赖于特定数据集的再训练或繁重的隐式优化，这限制了它们的可扩展性和泛化能力。

Method: 该方法通过简单的每图像优化来学习一个各向异性高斯核，该核结合了空间和范围线索，有效地结合了高斯溅射（Gaussian Splatting）和联合双边上采样（Joint Bilateral Upsampling）。这个学习到的核作为一个通用的、边缘感知的操作符，可以无缝地应用于不同的架构和模态。

Result: 该框架每224x224图像运行仅需约0.419秒，并在语义分割、深度估计以及深度和概率图上采样方面取得了最先进的性能。它能够精确地重建高分辨率的特征、深度或概率图，并能无缝地跨架构和模态进行迁移。

Conclusion: Upsample Anything提供了一个高效、无需训练且泛化性强的解决方案，用于将视觉基础模型的低分辨率特征提升至高分辨率像素级输出，从而克服了现有方法的局限性，并为像素级应用提供了强大支持。
...
### [114] [Explainable AI for Diabetic Retinopathy Detection Using Deep Learning with Attention Mechanisms and Fuzzy Logic-Based Interpretability](https://arxiv.org/abs/2511.16294)
*Abishek Karthik,Pandiyaraju V,Sreya Mynampati*

Main category: cs.CV

TL;DR: 本文提出了一种混合深度学习框架，结合CNN、ViT和GNN进行杂草检测，通过GAN增强和自监督预训练提高鲁棒性，并在多基准数据集上取得了99.33%的优异性能，支持边缘设备实时部署，实现可持续精准农业。


<details>
  <summary>Details</summary>
Motivation: 杂草检测是精准农业的关键任务，能实现选择性除草剂施用，符合可持续农业作物管理理念。现有方法可能在多种田间条件下缺乏鲁棒性，需要更有效、可推广的解决方案。

Method: 该研究提出了一种混合深度学习框架，整合了卷积神经网络（CNN）提取局部特征、Vision Transformers（ViT）提取全局特征，以及图神经网络（GNN）捕捉关系特征。为解决类别不平衡问题和提高泛化能力，采用了基于生成对抗网络（GAN）的数据增强方法。此外，通过自监督对比预训练从有限的标注数据中学习更多特征。

Result: 实验结果表明，该模型在多基准数据集上取得了卓越的性能，准确率、精确率、召回率和F1分数均达到99.33%。所提出的模型架构能够表示局部、全局和关系特征，并具有高可解释性和适应性。

Conclusion: 该混合深度学习框架为杂草检测提供了鲁棒、高效的解决方案，支持在边缘设备上实时部署，有助于减少对除草剂的过度依赖，并为自动化、可扩展和可持续的精准农业提供了实践选项。
...
### [115] [BioBench: A Blueprint to Move Beyond ImageNet for Scientific ML Benchmarks](https://arxiv.org/abs/2511.16315)
*Samuel Stevens*

Main category: cs.CV

TL;DR: ImageNet-1K的线性探测迁移准确率不再能准确预测科学图像（特别是生态学）的性能，因此论文提出了BioBench，一个开放的生态学视觉基准，以提供更可靠的评估。


<details>
  <summary>Details</summary>
Motivation: ImageNet-1K的线性探测迁移准确率作为视觉表示质量的默认代理，已不再适用于科学图像，尤其在生态学任务上，它只能解释34%的方差，并错误地对30%的模型进行排名，这表明需要一个更贴合实际应用的评估基准。

Method: 论文提出了BioBench，一个统一的生态学视觉基准，整合了9个公开的、应用驱动的任务，涵盖4个生物分类界和6种采集模式（如无人机RGB、网络视频、显微照片等），总计310万张图像。它提供了一个Python API用于数据下载、使用轻量级分类器评估冻结骨干网络，并报告类别平衡的macro-F1分数及特定领域的指标。

Result: ImageNet top-1准确率在生态学任务上只能解释34%的方差，并且错误地对30%的准确率高于75%的模型进行排名。BioBench为生态学领域的计算机视觉提供了新的信号，并为在任何领域构建可靠的科学AI基准提供了模板方案。

Conclusion: BioBench成功解决了ImageNet在评估科学图像（特别是生态学）视觉表示质量方面的不足，提供了一个更准确、更可靠的评估工具，并为未来AI for Science基准的构建提供了可复用的范例。
...
### [116] [WWE-UIE: A Wavelet & White Balance Efficient Network for Underwater Image Enhancement](https://arxiv.org/abs/2511.16321)
*Ching-Heng Cheng,Jen-Wei Lee,Chia-Ming Lee,Chih-Chung Hsu*

Main category: cs.CV

TL;DR: 本文提出WWE-UIE，一个紧凑高效的水下图像增强网络，它结合了自适应白平衡、基于小波的增强模块和梯度感知模块，实现了实时性能和竞争性的恢复质量。


<details>
  <summary>Details</summary>
Motivation: 现有的水下图像增强（UIE）混合方法虽然性能强大，但计算成本高昂，限制了它们在实时场景中的实际应用。

Method: 本文提出了WWE-UIE网络，集成了三个可解释的先验：1) 自适应白平衡，用于缓解波长相关的颜色衰减；2) 基于小波的增强模块（WEB），通过多波段分解捕获全局结构和精细纹理；3) 梯度感知模块（SGFB），利用可学习门控的Sobel算子显式保留边缘结构。

Result: WWE-UIE在基准数据集上实现了具有竞争力的恢复质量，同时参数和FLOPs显著减少，从而支持在资源受限平台上进行实时推理。消融研究和可视化进一步验证了每个组件的贡献。

Conclusion: WWE-UIE是一个紧凑高效的水下图像增强网络，通过集成可解释的先验，在保持高质量恢复的同时，显著降低了计算成本，使其适用于实时应用和资源受限平台。
...
### [117] [Physics-Informed Machine Learning for Efficient Sim-to-Real Data Augmentation in Micro-Object Pose Estimation](https://arxiv.org/abs/2511.16494)
*Zongcai Tan,Lan Wei,Dandan Zhang*

Main category: cs.CV

TL;DR: 本文提出一种物理信息深度生成学习框架，将波动光学渲染和深度对齐整合到GAN中，以高效合成高保真显微镜图像，用于微型机器人姿态估计，显著提升了合成数据的质量和姿态估计器的性能。


<details>
  <summary>Details</summary>
Motivation: 微型机器人精确姿态估计对高精度物体追踪和生物研究至关重要。然而，现有方法严重依赖难以获取且成本高昂的大型高质量显微镜图像数据集。数字孪生系统虽有前景，但难以复制衍射伪影和深度相关成像等复杂光学现象。

Method: 本文提出一种新颖的物理信息深度生成学习框架，首次将基于波动光学的物理渲染和深度对齐集成到生成对抗网络（GAN）中，以高效合成用于微型机器人姿态估计的高保真显微镜图像。

Result: 与纯AI驱动方法相比，该方法将结构相似性指数（SSIM）提高了35.6%，同时保持了实时渲染速度（0.022秒/帧）。使用合成数据训练的姿态估计器（CNN主干）达到了93.9%/91.9%（俯仰/横滚）的准确率，仅比使用真实数据训练的估计器低5.0%/5.4%。此外，该框架能够泛化到未见过的姿态，无需额外训练数据即可实现数据增强和对新型微型机器人配置的鲁棒姿态估计。

Conclusion: 所提出的物理信息深度生成学习框架成功合成了高保真显微镜图像，显著提升了微型机器人姿态估计的准确性和鲁棒性，减少了对昂贵真实数据的依赖，并展现出良好的泛化能力。
...
### [118] [TetraSDF: Precise Mesh Extraction with Multi-resolution Tetrahedral Grid](https://arxiv.org/abs/2511.16273)
...
### [119] [Generative AI for Enhanced Wildfire Detection: Bridging the Synthetic-Real Domain Gap](https://arxiv.org/abs/2511.16617)
*Satyam Gaba*

Main category: cs.CV

TL;DR: 本文利用生成式AI技术合成烟雾数据集，并结合无监督域适应方法进行烟雾羽流分割，旨在弥补真实数据稀缺性，提高野火早期检测模型的准确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 野火的早期检测是关键的环境挑战，特别是烟雾的及时识别对于减轻大规模损害至关重要。然而，深度神经网络在定位任务中虽有效，但缺乏大型标注烟雾数据集限制了其潜力。

Method: 研究方法包括：1) 利用生成式AI技术合成全面且带标注的烟雾数据集；2) 探索无监督域适应方法进行烟雾羽流分割，以缩小合成数据与真实数据之间的差距；3) 整合风格迁移、生成对抗网络（GANs）和图像抠图等先进生成方法，以增强合成数据的真实感并弥合域差异。

Result: 通过利用生成式AI合成数据和探索无监督域适应方法，并整合先进生成技术（如风格迁移、GANs、图像抠图），本研究旨在增强合成数据的真实感并弥合域差异，从而为更准确、可扩展的野火检测模型铺平道路。

Conclusion: 通过增强合成数据的真实性并弥合域差异，所提出的方法为开发更准确、可扩展的野火检测模型奠定了基础。
...
### [120] [VLA-Pruner: Temporal-Aware Dual-Level Visual Token Pruning for Efficient Vision-Language-Action Inference](https://arxiv.org/abs/2511.16449)
*Ziyan Liu,Yeqiu Chen,Hongyi Cai,Tao Lin,Shuo Yang,Zheng Liu,Bo Zhao*

Main category: cs.CV

TL;DR: VLA-Pruner是一种为视觉-语言-动作(VLA)模型设计的即插即用型token剪枝方法，它通过结合语义和动作层面的重要性准则，有效解决了现有方法忽略动作执行关键信息的问题，显著提升了VLA模型的实时部署性能。


<details>
  <summary>Details</summary>
Motivation: VLA模型因处理连续视觉流而计算成本高昂，限制了实时部署。现有的针对视觉-语言模型(VLM)的token剪枝方法仅基于语义显著性选择token，忽略了VLA模型高层语义理解和低层动作执行的双系统特性，导致关键动作信息被丢弃，VLA性能显著下降。

Method: 本文提出了VLA-Pruner，一种即插即用的VLA专用token剪枝方法。它与VLA模型的双系统特性对齐，并利用机器人操作中的时间连续性。具体而言，VLA-Pruner采用双层重要性准则进行视觉token保留：基于视觉-语言预填充注意力评估语义层面的相关性，以及通过时间平滑估计的动作解码注意力评估动作层面的重要性。基于此准则，VLA-Pruner提出了一种新颖的双层token选择策略，在给定计算预算下，自适应地保留一组紧凑且信息丰富的视觉token，以同时支持语义理解和动作执行。

Result: 实验结果表明，VLA-Pruner在多种VLA架构和多样化的机器人任务上均实现了最先进的性能。

Conclusion: VLA-Pruner通过其双层重要性准则和选择策略，成功弥补了VLM专用token剪枝方法在VLA模型中的不足，有效提升了VLA模型的效率和性能，使其更适用于实时机器人部署。
...
### [121] [Optimizing 3D Gaussian Splattering for Mobile GPUs](https://arxiv.org/abs/2511.16298)
...
### [122] [Aerial View River Landform Video segmentation: A Weakly Supervised Context-aware Temporal Consistency Distillation Approach](https://arxiv.org/abs/2511.16343)
*Chi-Han Chen,Chieh-Ming Chen,Wen-Huang Cheng,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 本研究提出一种基于师生架构、关键帧选择和更新算法的弱监督学习框架，用于无人机遥感地形分类，仅使用30%的标注数据即可显著提升mIoU和时间一致性，解决数据稀缺和标注复杂性问题。


<details>
  <summary>Details</summary>
Motivation: 无人机遥感地形分类面临数据标注复杂、难以保证时间一致性、相关数据稀缺以及技术有效范围受限等挑战。传统方法中，全标注数据并非最佳选择，仅选择关键数据又缺乏时间一致性提升。

Method: 提出一种师生架构，结合关键帧选择和关键帧更新算法。该框架通过弱监督学习和时间一致性知识蒸馏，克服了传统时间一致性训练在航空任务中的不足。研究表明，全标注数据并非最优，且仅选择关键数据无法提升时间一致性。

Result: 实验结果显示，该方法仅使用30%的标注数据，就能同时提高平均交并比（mIoU）和时间一致性（TC），从而确保地形目标的稳定定位。

Conclusion: 所提出的师生架构结合关键帧选择和更新算法，成功地在无人机空中定位任务中实现了弱监督学习和时间一致性知识蒸馏，有效解决了数据标注复杂性和时间一致性保障的难题，显著提升了定位的稳定性和效率。
...
### [123] [NutriScreener: Retrieval-Augmented Multi-Pose Graph Attention Network for Malnourishment Screening](https://arxiv.org/abs/2511.16566)
*Misaal Khan,Mayank Vatsa,Kuldeep Singh,Richa Singh*

Main category: cs.CV

TL;DR: 本文提出NutriScreener，一个基于检索增强、多姿态图注意力网络的AI工具，通过图像实现儿童营养不良检测和人体测量预测，具有高准确性和效率，特别适用于资源匮乏地区。


<details>
  <summary>Details</summary>
Motivation: 儿童营养不良仍是全球性危机，但现有筛查方法费力且难以规模化，阻碍了早期干预。

Method: NutriScreener是一个检索增强、多姿态图注意力网络，结合了基于CLIP的视觉嵌入、类别增强的知识检索和上下文感知，以实现鲁棒的营养不良检测和人体测量预测。它同时解决了泛化性和类别不平衡问题。

Result: 在一项临床研究中，医生对其准确性评分为4.3/5，效率评分为4.6/5。在AnthroVision的2,141名儿童数据上，NutriScreener达到了0.79的召回率、0.82的AUC，并显著降低了人体测量RMSE。在ARAN和CampusPose等跨大陆数据集上的评估显示，使用人口统计学匹配的知识库可将召回率提高高达25%，RMSE降低高达3.5厘米。

Conclusion: NutriScreener为资源匮乏地区提供了可扩展且准确的早期营养不良检测解决方案。
...
### [124] [Sparse Autoencoders are Topic Models](https://arxiv.org/abs/2511.16309)
...
### [125] [Supervised Contrastive Learning for Few-Shot AI-Generated Image Detection and Attribution](https://arxiv.org/abs/2511.16541)
*Jaime Álvarez Urueña,David Camacho,Javier Huertas Tato*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的两阶段检测框架，通过对比学习和少样本k-NN分类，有效解决了合成图像检测中的泛化挑战，并在检测精度和来源归因方面取得了显著提升，无需对不断涌现的新生成模型进行大量重新训练。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展使得合成图像与真实内容几乎无法区分，严重威胁数字媒体的完整性。同时，新型生成模型的快速发布周期使得依赖周期性重新训练的传统检测方法在计算上不可行且操作上不切实际。

Method: 该研究提出一个两阶段检测框架：第一阶段使用通过监督对比学习训练的视觉深度学习模型，从输入图像中提取判别性嵌入（该模型在部分生成器上训练，并特意保留了特定架构以测试跨生成器泛化能力）。第二阶段则利用一个k-近邻（k-NN）分类器在学习到的嵌入空间上运行，该分类器采用少样本学习范式，仅使用来自先前未见测试生成器的有限样本进行训练。

Result: 在少样本学习模式下，每类仅使用150张图像，该框架的平均检测准确率达到91.3%，比现有方法提高了5.2个百分点。在来源归因任务中，该方法在开放集分类背景下，AUC和OSCR分别获得了14.70%和4.27%的改进。

Conclusion: 该框架代表着在构建鲁棒、可扩展的取证归因系统方面取得了重大进展，该系统能够适应不断发展的生成式AI环境，而无需进行详尽的重新训练协议。
...
### [126] [NaTex: Seamless Texture Generation as Latent Color Diffusion](https://arxiv.org/abs/2511.16317)
*Zeqiang Lai,Yunfei Zhao,Zibo Zhao,Xin Yang,Xin Huang,Jingwei Huang,Xiangyu Yue,Chunchao Guo*

Main category: cs.CV

TL;DR: NaTex是一个原生3D纹理生成框架，它直接在3D空间中预测纹理颜色，通过将纹理视为密集的颜色点云并采用潜在颜色扩散（包括几何感知颜色点云VAE和多控制扩散Transformer），解决了传统多视图扩散模型在遮挡区域、网格纹理对齐和跨视图一致性方面的局限性，并展现出卓越的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 先前的纹理生成方法依赖于多视图扩散模型（MVDs）合成的2D多视图图像，存在固有局限性，例如难以处理遮挡区域（需要修复）、难以实现精确的网格-纹理边界对齐，以及难以保持内容和颜色强度上的跨视图一致性和连贯性。本研究旨在克服这些问题。

Method: NaTex提出了一种新颖的范式，将纹理视为密集的颜色点云。它引入了“潜在颜色扩散”机制，包含一个几何感知的颜色点云变分自编码器（VAE）和一个多控制扩散Transformer（DiT），两者均从头开始使用3D数据进行训练。为实现精确对齐，NaTex引入了原生几何控制，通过位置嵌入和几何潜在变量将DiT直接与3D空间信息关联。VAE-DiT架构协同设计，几何潜在变量通过一个与颜色VAE紧密耦合的专用几何分支提取，提供精细的表面指导，以保持与纹理的强对应性。

Result: NaTex展现出强大的性能，在纹理连贯性和对齐方面显著优于现有方法。此外，NaTex还表现出强大的泛化能力，无论是无需训练还是通过简单微调，都能适用于各种下游应用，例如材质生成、纹理细化以及部件分割和纹理化。

Conclusion: NaTex通过直接在3D空间中预测纹理颜色，成功解决了传统多视图扩散模型在纹理生成方面的局限性。其提出的潜在颜色扩散框架和原生几何控制机制，实现了卓越的纹理连贯性和对齐效果，并具备广泛的泛化能力，适用于多种3D应用。
...
### [127] [ChangeDINO: DINOv3-Driven Building Change Detection in Optical Remote Sensing Imagery](https://arxiv.org/abs/2511.16322)
*Ching-Heng Cheng,Chih-Chung Hsu*

Main category: cs.CV

TL;DR: ChangeDINO是一个端到端的多尺度孪生框架，用于光学建筑变化检测，它融合了DINOv3的语义特征和空间-光谱差分变换器，并在四个公共基准测试中优于现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习遥感变化检测方法过度依赖变化图标注，且未充分利用非变化区域的语义信息，导致在光照变化、非天顶视角和标签稀缺情况下的鲁棒性不足。

Method: ChangeDINO采用端到端的多尺度孪生框架。它将轻量级骨干网络与来自冻结DINOv3的特征融合，生成语义和上下文丰富的特征金字塔。然后，一个空间-光谱差分变换器解码器利用多尺度绝对差异作为变化先验来突出真实建筑变化并抑制无关响应。最后，一个可学习的形态学模块细化上采样的logits以恢复清晰的边界。

Result: 在四个公共基准测试中，ChangeDINO在IoU和F1指标上始终优于最新的SOTA方法。消融研究证实了每个组件的有效性。

Conclusion: ChangeDINO通过有效融合预训练模型的语义信息和创新的差分解码器及形态学模块，显著提升了光学建筑变化检测的性能和鲁棒性，尤其是在标签稀缺和复杂环境下的表现。
...
### [128] [SAM 3D: 3Dfy Anything in Images](https://arxiv.org/abs/2511.16624)
...
TL;DR: SAM 3D是一种生成模型，能从单张图像重建3D物体（几何、纹理、布局），通过人机协作标注和多阶段训练克服了3D数据障碍，在真实世界图像中表现出色。


<details>
  <summary>Details</summary>
Motivation: 在自然图像中，遮挡和场景杂乱普遍存在，上下文视觉识别线索至关重要。传统的3D重建方法难以应对这些挑战，且存在“3D数据障碍”，即缺乏大规模高质量的3D重建数据。

Method: 该研究提出了SAM 3D模型，利用人与模型协同的循环管线（human- and model-in-the-loop pipeline）标注物体形状、纹理和姿态，以空前规模提供视觉基础的3D重建数据。接着，采用现代多阶段训练框架，结合合成数据预训练和真实世界对齐，从而突破3D“数据障碍”。

Result: SAM 3D在真实世界物体和场景的人类偏好测试中，相对于最新工作取得了显著提升，胜率至少达到5:1。它在处理存在遮挡和场景杂乱的自然图像方面表现优异。

Conclusion: SAM 3D通过创新的数据标注和多阶段训练框架，成功解决了3D重建中的数据障碍，实现了从单张图像进行视觉基础的3D物体重建，并在真实世界场景中展现出卓越性能。研究团队将发布代码、模型权重、在线演示和一个新的野外3D物体重建挑战基准。
...
### [129] [DetailSemNet: Elevating Signature Verification through Detail-Semantic Integration](https://arxiv.org/abs/2511.16364)
*Meng-Cheng Shih,Tsai-Ling Huang,Yu-Heng Shih,Hong-Han Shuai,Hsuan-Tung Liu,Yi-Ren Yeh,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 本文提出了一种名为DetailSemNet的新模型，用于离线签名验证（OSV），它通过强调细粒度差异和匹配局部结构来显著提高验证准确性，并解决了基于Transformer骨干网络可能模糊局部细节的问题。


<details>
  <summary>Details</summary>
Motivation: 离线签名验证（OSV）在法证领域被广泛应用。现有方法主要依赖整体特征进行比较，但本文认为细粒度差异对于鲁棒的OSV至关重要。此外，基于Transformer的骨干网络在没有特定修改的情况下可能会模糊局部细节，从而影响OSV性能。

Method: 本文提出DetailSemNet模型，其核心思想是匹配两个签名图像之间的局部结构。为解决Transformer骨干网络模糊局部细节的问题，引入了“细节语义整合器”（Detail Semantics Integrator），该整合器利用特征解耦和重耦技术，旨在增强复杂细节并同时扩展判别性语义，从而提高局部结构匹配的效率。

Result: DetailSemNet在离线签名验证的领先基准测试中持续优于现有方法，取得了最先进的成果，并具有明显的优势。该模型不仅提高了性能，还增强了解释性，并在跨数据集测试场景中表现出卓越的泛化能力。

Conclusion: DetailSemNet通过强调局部结构匹配，结合其出色的泛化能力和解释性，显著提升了离线签名验证的性能，使其在实际应用中具有巨大的潜力。
...
### [130] [Multi-Order Matching Network for Alignment-Free Depth Super-Resolution](https://arxiv.org/abs/2511.16361)
*Zhengxue Wang,Zhiqiang Yan,Yuan Wu,Guangwei Gao,Xiang Li,Jian Yang*

Main category: cs.CV

TL;DR: 本文提出了一种名为MOMNet的对齐无关深度超分辨率框架，通过多阶匹配和多阶聚合机制，从失对的RGB图像中自适应地检索和整合信息，从而解决了真实世界中RGB-D数据对齐困难的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的引导式深度超分辨率方法都基于RGB和深度严格空间对齐的假设，但在实际场景中，由于硬件限制和校准漂移，严格对齐的RGB-D数据难以获取。这导致现有方法在应用于失对的真实世界场景时性能下降。

Method: 本文提出了多阶匹配网络（MOMNet），一个新颖的对齐无关框架。该方法首先通过多阶匹配机制（联合执行零阶、一阶和二阶匹配）在多阶特征空间中全面识别与深度一致的RGB信息。为了有效整合检索到的RGB和深度信息，进一步引入了由多个结构检测器组成的多阶聚合模块，利用多阶先验作为提示，促进RGB到深度的选择性特征迁移。

Result: 广泛的实验表明，MOMNet取得了最先进的性能，并展现出卓越的鲁棒性。

Conclusion: MOMNet成功地解决了真实世界中RGB-D数据失对带来的挑战，为引导式深度超分辨率提供了一个高性能且鲁棒的对齐无关解决方案。
...
### [131] [CAMS: Towards Compositional Zero-Shot Learning via Gated Cross-Attention and Multi-Space Disentanglement](https://arxiv.org/abs/2511.16378)
*Pan Yang,Cheng Deng,Jing Yang,Han Zhao,Yun Liu,Yuling Chen,Xiaoli Ruan,Yanping Chen*

Main category: cs.CV

TL;DR: 本文提出CAMS，一种基于CLIP的组合零样本学习（CZSL）方法，通过门控交叉注意力从视觉特征中提取细粒度语义特征，并在多维空间中解耦属性和对象，从而提高对未见组合的泛化能力，并达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的CZSL方法主要依赖图像编码器获得的全局语义表示来解耦属性和对象。然而，这种表示能力有限，无法实现完全解耦，从而限制了对未见组合的泛化能力。

Method: CAMS方法包括：1. 设计一个门控交叉注意力（Gated Cross-Attention），通过一组潜在单元从CLIP高级图像编码块中捕获细粒度语义特征，并自适应抑制背景及无关信息。2. 进行多空间解耦（Multi-Space Disentanglement），实现属性和对象语义的解耦。

Result: 在MIT-States、UT-Zappos和C-GQA三个流行基准上的实验表明，CAMS在封闭世界和开放世界设置下均达到了最先进的性能。

Conclusion: CAMS通过在多维空间中进行语义解耦，有效地解决了现有CLIP-based CZSL方法中属性和对象解耦不完全的问题，显著提高了对未见属性-对象组合的泛化能力，并取得了卓越的实验结果。
...
### [132] [Arbitrary-Resolution and Arbitrary-Scale Face Super-Resolution with Implicit Representation Networks](https://arxiv.org/abs/2511.16341)
*Yi Ting Tsai,Yu Wei Chen,Hong-Han Shuai,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 本文提出了一种名为ARASFSR的任意分辨率和任意尺度人脸超分辨率方法，利用隐式表示网络和三个创新设计，解决了现有方法在固定上采样尺度和输入尺寸敏感性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸超分辨率（FSR）方法受限于固定的上采样尺度且对输入尺寸变化敏感，这限制了它们在实际应用中的有效性。因此，需要一种能够处理任意分辨率和任意尺度输入的方法。

Method: ARASFSR方法包含三个新颖设计：1) 使用2D深度特征、局部相对坐标和上采样尺度比来预测每个目标像素的RGB值，实现任意上采样尺度的超分辨率；2) 引入局部频率估计模块，捕获高频人脸纹理信息以减少频谱偏差效应；3) 采用全局坐标调制模块，引导FSR利用先验人脸结构知识并有效实现分辨率适应。

Result: 定量和定性评估表明，ARASFSR方法在处理各种输入尺寸和上采样尺度的人脸图像时，比现有最先进的方法表现出更强的鲁棒性。

Conclusion: ARASFSR通过其创新的隐式表示网络设计，成功解决了现有FSR方法在固定上采样尺度和输入尺寸敏感性方面的局限，实现了任意分辨率和任意尺度的人脸超分辨率，并展现出优越的性能和鲁棒性。
...
### [133] [Teacher-Guided One-Shot Pruning via Context-Aware Knowledge Distillation](https://arxiv.org/abs/2511.16653)
*Md. Samiul Alim,Sharjil Khan,Amrijit Biswas,Fuad Rahman,Shafin Rahman,Nabeel Mohammed*

Main category: cs.CV

TL;DR: 本文提出了一种教师引导的剪枝框架，通过在重要性分数计算阶段整合知识蒸馏（KD），实现了一次性全局剪枝。该方法在保持高性能的同时，达到了高稀疏度，并比现有迭代剪枝方案更高效。


<details>
  <summary>Details</summary>
Motivation: 非结构化剪枝虽然有效，但通常需要迭代的训练-剪枝-再训练循环，导致巨大的计算开销。现有方法将KD作为剪枝后的恢复步骤，未能充分利用教师模型在剪枝决策阶段的指导作用。

Method: 引入了一种新颖的教师引导剪枝框架，将知识蒸馏（KD）与重要性分数估计紧密结合。在重要性分数计算时，利用教师模型提供的梯度信号来识别和保留对任务性能和知识迁移至关重要的参数。该方法采用一次性全局剪枝策略，并在剪枝后进行稀疏感知再训练（有或无KD），以恢复准确性而不重新激活已剪枝的连接。

Result: 在CIFAR-10、CIFAR-100和TinyImageNet等多个图像分类基准测试中，该方法在实现高稀疏度的同时，性能下降最小。在高稀疏度下，其性能优于EPG和EPSD等现有先进基线，并且比COLT等迭代剪枝方案具有更高的计算效率。

Conclusion: 所提出的框架提供了一种计算高效、性能保持的解决方案，非常适用于资源受限环境中的部署。
...
### [134] [Dataset Distillation for Pre-Trained Self-Supervised Vision Models](https://arxiv.org/abs/2511.16674)
*George Cazenavette,Antonio Torralba,Vincent Sitzmann*

Main category: cs.CV

TL;DR: 本文提出了一种名为“线性梯度匹配”的数据集蒸馏方法，用于为预训练视觉模型上的线性探针训练生成高效的合成数据集，该方法在性能、泛化性和模型可解释性方面均表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法主要关注从头开始训练模型，而当前最先进的视觉方法越来越多地依赖大型预训练自监督模型进行线性探针训练。因此，需要一种专门针对此类预训练模型优化线性探针的数据集蒸馏方法。

Method: 引入“线性梯度匹配”（Linear Gradient Matching）方法。该方法通过优化合成图像，使其在经过预训练特征提取器后，在线性分类器中产生的梯度与真实数据产生的梯度相似，从而实现数据集蒸馏。

Result: 通过该方法得到的合成数据优于所有基于真实图像的基线，并且能够跨预训练视觉模型进行泛化（例如，使用DINO骨干蒸馏的数据集训练的CLIP线性探针表现良好）。此外，蒸馏数据集在细粒度分类中异常有效，并为模型可解释性提供了有价值的工具，例如预测模型嵌入空间的相似性或模型对对抗性数据中虚假相关性的敏感度。

Conclusion: 本文提出的线性梯度匹配方法成功地为预训练视觉模型上的线性探针训练蒸馏出高效的合成数据集，不仅提高了性能和泛化能力，还为模型可解释性提供了新的视角和工具。
...
### [135] [CylinderDepth: Cylindrical Spatial Attention for Multi-View Consistent Self-Supervised Surround Depth Estimation](https://arxiv.org/abs/2511.16428)
*Samer Abualhanud,Christian Grannemann,Max Mehltretter*

Main category: cs.CV

TL;DR: 本文提出了一种几何引导的自监督环视深度估计算法，通过将3D点投影到共享圆柱体并利用非学习空间注意力，显著提升了多视角深度估计的一致性和整体精度。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督环视深度估计算法在重叠图像之间存在深度估计不一致的问题，限制了其在360度感知中的应用。

Method: 该方法针对校准且时间同步的多摄像头系统，首先为每张图像预测初始深度图。然后，将所有图像导出的3D点投影到一个共享的单位圆柱体上，以建立跨图像的邻域关系。接着，为每张图像生成2D位置图，每个像素被分配其在圆柱体上的投影位置。最后，基于这些位置图，应用一种显式的、非学习的空间注意力机制，根据像素在圆柱体上的距离聚合跨图像的特征，以预测每张图像的最终深度图。

Result: 在DDAD和nuScenes数据集上评估结果表明，与现有最先进的方法相比，该方法显著改善了图像间深度估计的一致性，并提高了整体深度估计的准确性。

Conclusion: 所提出的几何引导方法有效解决了环视深度估计中跨视角不一致的局限性，实现了密集、度量且跨视角一致的深度预测。
...
### [136] [CRISTAL: Real-time Camera Registration in Static LiDAR Scans using Neural Rendering](https://arxiv.org/abs/2511.16349)
*Joni Vanherck,Steven Moonen,Brent Zoomers,Kobe Werner,Jeroen Put,Lode Jorissen,Nick Michiels*

Main category: cs.CV

TL;DR: 本文提出了一种实时相机定位方法，通过从预捕获的彩色激光雷达点云渲染合成视图，并结合神经网络渲染技术，实现无漂移且度量尺度正确的相机跟踪。


<details>
  <summary>Details</summary>
Motivation: 准确的相机定位对机器人技术和扩展现实（XR）至关重要，但现有视觉方法常受漂移、尺度模糊性困扰，且依赖于标识物或回环检测。

Method: 该方法利用预捕获的高精度彩色激光雷达点云，渲染合成视图，并在实时帧与点云之间建立2D-3D对应关系。通过神经渲染技术缩小合成图像与真实图像之间的域差距，减少遮挡和背景伪影，从而改进特征匹配。提出了两种实时变体：在线渲染与匹配（Online Render and Match）和预构建与定位（Prebuild and Localize）。

Result: 实现了在全局激光雷达坐标系中无漂移的相机跟踪，并具有正确的度量尺度。在ScanNet++数据集上展示了改进的结果，并优于现有的SLAM管线。

Conclusion: 该研究提供了一种实时、高精度且无漂移的相机定位解决方案，通过结合激光雷达点云和神经网络渲染，显著提升了在机器人和XR应用中的定位性能。
...
### [137] [End-to-End Motion Capture from Rigid Body Markers with Geodesic Loss](https://arxiv.org/abs/2511.16418)
*Hai Lan,Zongyan Li,Jianmin Hu,Jialing Yang,Houde Dai*

Main category: cs.CV

TL;DR: 本文提出了一种名为刚体标记（RBM）的新型运动捕捉（MoCap）基本单元，结合深度学习回归模型和测地线损失，实现了高效、高精度的实时人体姿态估计，解决了传统基于标记MoCap的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统的基于标记的光学运动捕捉虽然精度高，但存在设置耗时、标记识别模糊以及扩展性差等实际挑战，这些问题源于其对密集标记配置的依赖。

Method: 研究引入了刚体标记（RBM）作为MoCap的新型基本单元，提供明确的六自由度（6-DoF）数据并简化设置。在此基础上，开发了一个深度学习回归模型，直接估计SMPL参数，并采用流形感知的测地线损失进行优化。模型在AMASS数据集的合成数据上进行训练，并通过Vicon光学跟踪系统捕获的真实世界数据进行验证。

Result: 该端到端方法在性能上与基于优化的方法相当，但计算量减少了一个数量级以上。模型在人体姿态估计方面达到了最先进的精度，并通过真实世界数据证明了其在实际应用中的可行性。

Conclusion: 结合稀疏的六自由度RBM数据和流形感知的测地线损失，能够为图形、虚拟现实和生物力学领域提供一种实用且高保真度的实时运动捕捉解决方案。
...
### [138] [Graph Neural Networks for Surgical Scene Segmentation](https://arxiv.org/abs/2511.16430)
*Yihan Li,Nikhil Churamani,Maria Robu,Imanol Luengo,Danail Stoyanov*

Main category: cs.CV

TL;DR: 该研究提出基于图的分割方法，结合Vision Transformer和图神经网络，以提高腹腔镜胆囊切除术中肝胆解剖结构识别的准确性和一致性，从而减少手术并发症。


<details>
  <summary>Details</summary>
Motivation: 在腹腔镜胆囊切除术中，准确识别肝胆解剖结构对于预防手术并发症至关重要。传统的深度学习模型在处理遮挡、长距离依赖以及罕见结构的精细几何形状时面临挑战，这促使研究人员寻求更有效的方法来增强手术场景分析中的空间和语义理解。

Method: 该研究提出了两种结合Vision Transformer (ViT) 特征编码器与图神经网络 (GNNs) 的分割模型，以明确建模解剖区域间的空间关系：1) 使用静态 k-近邻 (k-NN) 图与带初始残差和恒等映射的图卷积网络 (GCNII) 来实现稳定的长距离信息传播。2) 使用动态可微分图生成器 (DGG) 与图注意力网络 (GAT) 来支持自适应拓扑学习。这两种模型均在Endoscapes-Seg50和CholecSeg8k基准数据集上进行了评估。

Result: 所提出的方法在平均交并比 (mIoU) 上实现了高达7-8%的提升，在平均Dice系数 (mDice) 上实现了6%的提升，优于现有最先进的基线模型。尤其在薄、罕见和安全关键结构上，产生了符合解剖学逻辑的预测。

Conclusion: 所提出的基于图的分割方法显著提升了手术场景分割的性能和解剖学一致性。通过结合基于ViT的全局上下文理解与基于图的关系推理，这些模型提高了可解释性和可靠性，为通过精确识别关键解剖特征来实现更安全的腹腔镜和机器人辅助手术铺平了道路。
...
### [139] [FastSurfer-CC: A robust, accurate, and comprehensive framework for corpus callosum morphometry](https://arxiv.org/abs/2511.16471)
*Clemens Pollak,Kersten Diers,Santiago Estrada,David Kügler,Martin Reuter*

Main category: cs.CV

TL;DR: 本文介绍了FastSurfer-CC，一个高效且全自动的胼胝体形态测量框架，它优于现有工具，并能在亨廷顿病患者中发现现有方法未检测到的显著差异。


<details>
  <summary>Details</summary>
Motivation: 胼胝体在衰老和神经疾病研究中至关重要，但目前公开可用的工具缺乏全面且自动化的分析流程，存在空白。

Method: FastSurfer-CC是一个全自动框架，能自动识别正中矢状面切片，分割胼胝体和穹窿，定位前后连合以标准化头部姿势，生成厚度剖面和细分，并提取八种形状指标用于统计分析。

Result: FastSurfer-CC在各项任务中均优于现有的专业工具。此外，它揭示了亨廷顿病患者与健康对照组之间存在统计学上的显著差异，而这些差异是当前最先进方法未能检测到的。

Conclusion: FastSurfer-CC提供了一个卓越的、全面的自动化胼胝体形态测量解决方案，为神经疾病研究和临床试验提供了新的洞察力。
...
### [140] [Flow and Depth Assisted Video Prediction with Latent Transformer](https://arxiv.org/abs/2511.16484)
*Eliyas Suleyman,Paul Henderson,Eksan Firkat,Nicolas Pugeault*

Main category: cs.CV

TL;DR: 本文提出通过引入点流（运动信息）和深度图（几何结构）来增强视频预测模型，以解决遮挡和背景运动预测的挑战，并在实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管通用视频预测模型在标准场景中表现出色，但遮挡仍然是其固有挑战。研究者假设提供明确的运动（通过点流）和几何结构（通过深度图）信息将使视频预测模型在遮挡和背景运动情况下表现更好。

Method: 研究者使用标准的、基于多对象潜在Transformer的架构来预测未来帧，并对其进行修改以整合深度和点流信息。他们在受控设置下，使用合成和真实世界数据集对模型进行评估，不仅采用基于外观的指标，还使用对象掩码上的Wasserstein距离来有效衡量预测的运动分布。

Result: 研究发现，当预测模型得到点流和深度的辅助时，它在遮挡场景中表现更好，并且比没有这些模态帮助的模型预测出更准确的背景运动。

Conclusion: 提供点流和深度信息可以显著提高视频预测模型在遮挡情况下的性能，并能更准确地预测背景运动。
...
### [141] [LLaVA$^3$: Representing 3D Scenes like a Cubist Painter to Boost 3D Scene Understanding of VLMs](https://arxiv.org/abs/2511.16454)
*Doriand Petit,Steve Bourgeois,Vincent Gay-Bellile,Florian Chabot,Loïc Barthe*

Main category: cs.CV

TL;DR: LLaVA$^3$是一种新颖的方法，它利用多视角2D图像和受立体主义启发的全向视觉表示，无需微调即可增强视觉语言模型（VLM）的3D场景理解能力，并在3D VQA和3D语言基础任务上超越了现有2D方法。


<details>
  <summary>Details</summary>
Motivation: 由于3D训练数据稀缺，多模态语言模型在理解3D场景方面面临挑战，这与用于视觉语言模型的丰富2D数据集形成对比。

Method: 本文提出了LLaVA$^3$，一种通过仅使用多视角2D图像且无需微调来提升VLM 3D场景理解能力的方法。受立体派画家启发，该方法通过场景的中间多视角3D重建，为每个物体生成全向视觉表示，并用这些表示来描述3D场景供VLM理解。

Result: 在3D VQA和3D语言基础任务上的广泛实验表明，LLaVA$^3$方法优于之前基于2D的VLM解决方案。

Conclusion: LLaVA$^3$成功地通过利用多视角2D图像和受立体主义启发的全向表示，在不进行微调的情况下显著提升了VLM对3D场景的理解能力，有效解决了3D数据稀缺的问题。
...
### [142] [StreetView-Waste: A Multi-Task Dataset for Urban Waste Management](https://arxiv.org/abs/2511.16440)
*Diogo J. Paulo,João Martins,Hugo Proença,João C. Neves*

Main category: cs.CV

TL;DR: 本文提出了StreetView-Waste数据集，用于解决智慧城市中垃圾车视角下的垃圾桶溢出监测问题。该数据集支持垃圾桶检测、跟踪和溢出分割三项任务，并提供了基线模型和两种改进策略，显著提升了跟踪和分割性能。


<details>
  <summary>Details</summary>
Motivation: 智慧城市发展面临严峻的城市垃圾管理挑战。尽管垃圾检测数据集数量不断增长，但从垃圾车图像监测溢出的垃圾桶问题却鲜受关注。现有数据集通常缺乏特定容器跟踪的标注，且多在静态、去情境化的环境中捕获，限制了其在实际物流中的应用。

Method: 本文构建了StreetView-Waste数据集，包含城市场景中的垃圾和垃圾桶，支持垃圾桶检测、跟踪和溢出分割三项关键评估任务。同时，提供了基于现有先进模型的基线，并提出了两种互补策略：一种基于启发式的方法用于改进垃圾桶跟踪，以及一个模型无关的框架，利用几何先验知识来细化垃圾溢出分割。

Result: 实验结果表明，经过微调的目标检测器在垃圾桶检测方面表现良好；基线跟踪方法在准确估计垃圾桶数量方面表现不佳，但本文提出的启发式方法将平均绝对计数误差降低了79.6%。此外，虽然非晶态垃圾的分割具有挑战性，但几何感知策略使轻量级模型的分割mAP@0.5提高了27%，证明了多模态输入对此任务的价值。

Conclusion: StreetView-Waste数据集提供了一个具有挑战性的基准，旨在促进城市垃圾管理领域中真实世界感知系统的研究。
...
### [143] [YOWO: You Only Walk Once to Jointly Map An Indoor Scene and Register Ceiling-mounted Cameras](https://arxiv.org/abs/2511.16521)
*Fan Yang,Sosuke Yamao,Ikuo Kusajima,Atsunori Moteki,Shoichi Masui,Shan Jiang*

Main category: cs.CV

TL;DR: 本文提出一种新颖的解决方案，通过使用配备RGB-D摄像头的移动代理和定制的因子图优化，实现室内场景的联合映射和天花板摄像机（CMC）的注册，并开发了一个新数据集。


<details>
  <summary>Details</summary>
Motivation: 天花板摄像机（CMC）的注册是一项挑战性任务。手动注册效率低且成本高，而自动视觉定位在存在视觉模糊时效果不佳。因此，需要一种更有效的方法来解决这些问题。

Method: 该方法包括：1. 移动代理佩戴头戴式RGB-D摄像头遍历场景，同时CMC同步捕捉该代理。2. 代理的自我中心视频生成世界坐标系的代理轨迹和场景布局。3. CMC视频提供伪尺度代理轨迹和CMC相对姿态。4. 通过时间戳关联所有轨迹，将CMC相对姿态与世界坐标系场景布局对齐。5. 基于此初始化，定制一个因子图以联合优化自我摄像头姿态、场景布局和CMC姿态。此外，还开发了一个新的数据集作为基准。

Result: 实验结果表明，该方法不仅在一个统一框架内有效完成了场景映射和CMC注册两项任务，而且共同提升了它们的性能。它为下游的位置感知应用提供了一个可靠的工具。

Conclusion: 本文提出的方法为室内场景的联合映射和天花板摄像机注册提供了一个可靠且有效的工具，通过统一框架共同增强了这两项任务的性能，从而促进了位置感知应用的发展。
...
### [144] [Improving Long-Tailed Object Detection with Balanced Group Softmax and Metric Learning](https://arxiv.org/abs/2511.16619)
*Satyam Gaba*

Main category: cs.CV

TL;DR: 该论文通过改进Balanced Group Softmax (BAGS)框架并结合度量学习和k-NN推理，解决了长尾2D目标检测问题，在LVISv1数据集上实现了24.5%的mAP，刷新了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界场景中的数据通常呈长尾分布，即少数类别实例众多，而多数类别实例稀少。这种固有的类别不平衡导致目标检测模型偏向常见类别，从而降低了稀有类别的性能。

Method: 研究人员使用LVISv1数据集（包含1,203个类别和164,000张图像），采用两阶段Faster R-CNN架构。他们提出了对Balanced Group Softmax (BAGS)框架的增强以缓解类别不平衡。此外，他们探索了度量学习来生成类间分离良好且类内紧密聚类的特征嵌入，并在推理阶段利用k-Nearest Neighbors (k-NN)方法改进分类性能，特别是针对稀有类别。

Result: 该方法在LVISv1数据集上实现了24.5%的平均精度（mAP），超越了之前24.0%的基准，达到了新的最先进（SOTA）性能。结果表明这些方法在推进长尾目标检测方面是有效的。

Conclusion: 通过对BAGS框架的增强以及结合度量学习和k-NN推理，研究人员成功缓解了长尾目标检测中的类别不平衡问题，尤其提升了稀有类别的性能，并在LVISv1数据集上取得了SOTA成果。
...
### [145] [Enhancing Multi-Camera Gymnast Tracking Through Domain Knowledge Integration](https://arxiv.org/abs/2511.16532)
*Fan Yang,Shigeyuki Odashima,Shoichi Masui,Ikuo Kusajima,Sosuke Yamao,Shan Jiang*

Main category: cs.CV

TL;DR: 本文提出了一种鲁棒的多摄像头体操运动员追踪系统，通过结合体操领域知识和级联数据关联范式，解决了摄像机数量有限和检测失败的挑战，并在国际体操锦标赛中成功应用。


<details>
  <summary>Details</summary>
Motivation: 传统多摄像头追踪算法在体操运动员追踪中面临独特挑战：摄像机数量受限；光照、背景、服装和遮挡变化导致多摄像头检测失败（有时仅能从两个相对视角提供有效检测），使得传统三角测量难以准确确定运动员的3D轨迹。

Method: 该方法将体操领域知识（体操运动员的3D中心通常位于预定义的垂直平面内）融入追踪方案。提出了一种新颖的级联数据关联范式：当跨视图检测足够时，使用三角测量生成3D轨迹候选；当检测不足时，则采用射线-平面交点法生成共面3D轨迹候选。这些共面候选用于补偿不确定的轨迹，从而最大限度地减少追踪失败。

Result: 通过大量实验验证了该方法的鲁棒性，在挑战性场景中表现出优于现有方法的性能。配备该追踪方法的体操裁判系统已成功应用于近期世界体操锦标赛，并获得了国际体操联合会的认可。

Conclusion: 该方法通过创新性地结合领域知识和级联数据关联，有效解决了多摄像头体操运动员追踪中的关键挑战，实现了鲁棒且准确的追踪，并在实际应用中取得了显著成功和认可。
...
### [146] [Acquisition Time-Informed Breast Tumor Segmentation from Dynamic Contrast-Enhanced MRI](https://arxiv.org/abs/2511.16498)
*Rui Wang,Yuexi Du,John Lewin,R. Todd Constable,Nicha C. Dvornek*

Main category: cs.CV

TL;DR: 本文提出一种乳腺DCE-MRI肿瘤分割方法，通过利用图像采集时间来调制模型特征，从而提高分割性能和模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: DCE-MRI在乳腺癌筛查和治疗中很重要，但由于采集方案和个体因素差异，导致图像中组织外观变化大，使得自动化肿瘤分割极具挑战性。

Method: 研究者提出一种肿瘤分割方法，利用图像采集时间来调制模型特征。具体采用特征级线性调制（FiLM）层来整合时间信息，该方法轻量且能处理可变数量的图像。他们在大型多中心DCE-MRI数据集上训练了基线模型和不同配置的时间调制模型，并使用了不同的骨干网络架构。

Result: 在域内图像和公共域外数据集上的评估显示，整合相位采集时间显著提高了肿瘤分割性能和模型的泛化能力。

Conclusion: 通过在DCE-MRI肿瘤分割模型中融入相位采集时间信息（利用FiLM层），可以有效改善分割表现并增强模型的泛化性。
...
### [147] [TRIM: Scalable 3D Gaussian Diffusion Inference with Temporal and Spatial Trimming](https://arxiv.org/abs/2511.16642)
*Zeyuan Yin,Xiaoming Liu*

Main category: cs.CV

TL;DR: TRIM是一种后训练方法，通过轨迹削减和实例掩码去噪，显著提升3D高斯扩散模型的推理效率和生成质量，同时支持推理时缩放。


<details>
  <summary>Details</summary>
Motivation: 当前的3D高斯扩散模型由于高斯基元数量庞大，导致去噪和后处理耗时，生成速度慢，且在采样轨迹上的可扩展性有限。

Method: TRIM（轨迹削减和实例掩码去噪）是一种后训练方法，结合了时间和空间修剪策略。它开发了一个轻量级选择器模型，用于评估来自多个采样噪声的潜在高斯基元，通过选择具有高质量潜力的候选来早期削减轨迹。此外，它引入了实例掩码去噪，通过过滤冗余背景区域来修剪可学习的高斯基元，从而减少每个去噪步骤的推理计算。

Result: TRIM显著提高了3D生成的效率和质量。

Conclusion: TRIM通过结合时间（轨迹削减）和空间（实例掩码去噪）修剪策略，在不牺牲输出质量的情况下，有效加速了3D高斯扩散模型的推理过程，并支持推理时缩放，从而解决了现有模型的效率和可扩展性问题。
...
### [148] [Lite Any Stereo: Efficient Zero-Shot Stereo Matching](https://arxiv.org/abs/2511.16555)
*Junpeng Jing,Weixun Luo,Ye Mao,Krystian Mikolajczyk*

Main category: cs.CV

TL;DR: 本文提出了一种名为Lite Any Stereo的高效立体深度估计框架，该框架在保持极高效率的同时，实现了强大的零样本泛化能力，并在多个真实世界基准上取得了领先的准确性，计算成本极低。


<details>
  <summary>Details</summary>
Motivation: 当前的立体匹配研究倾向于以增加模型大小为代价来提高准确性，导致高效模型因容量有限而被认为不具备零样本能力。本文旨在打破这一传统认知，证明高效模型也能实现强大的零样本泛化。

Method: 作者设计了一个紧凑而富有表现力的主干网络以确保可扩展性，并结合了一个精心设计的混合成本聚合模块。此外，他们提出了一种在百万级数据上进行的三阶段训练策略，以有效弥合仿真到现实的差距。

Result: Lite Any Stereo模型展现出强大的泛化能力，在四个广泛使用的真实世界基准测试中排名第一。值得注意的是，该模型在计算成本不到1%的情况下，达到了与或超越当前最先进的非先验高精度方法相当的准确性。

Conclusion: 研究表明，超轻量级模型也能提供强大的泛化能力，为高效立体匹配树立了新标准，证明了效率与零样本泛化能力可以兼得。
...
### [149] [BoxingVI: A Multi-Modal Benchmark for Boxing Action Recognition and Localization](https://arxiv.org/abs/2511.16524)
*Rahul Kumar,Vipul Baghel,Sudhanshu Singh,Bikash Kumar Badatya,Shivam Yadav,Babji Srinivasan,Ravi Hegde*

Main category: cs.CV

TL;DR: 本文介绍了一个为拳击拳法检测和分类量身定制的综合性、高质量视频数据集，旨在克服现有数据集的不足，并推动实时计算机视觉在搏击运动分析中的应用。


<details>
  <summary>Details</summary>
Motivation: 尽管计算机视觉在搏击运动分析中受到关注，但由于动作的动态性、非结构化以及录制环境的多变性，缺乏鲁棒的数据集仍然是主要瓶颈。

Method: 研究者从20个公开的YouTube陪练视频中提取了6,915个高质量的拳击片段，涉及18名不同运动员。每个片段都经过手动分割和标注，以确保精确的时间边界和类别一致性，并分为六种不同的拳法类型。该数据集旨在支持实时、低资源和非受限环境下的视觉动作识别研究。

Result: 本文成功构建了一个全面、标注精良的视频数据集，包含6,915个拳击片段，涵盖六种不同拳法类型，并捕获了广泛的运动风格、摄像机角度和运动员体格。该数据集为拳击及其相关领域的动作分析、自动化教练和表现评估提供了丰富的基准。

Conclusion: 通过提供一个包含多样化拳击示例的丰富基准数据集，本文旨在加速搏击运动及其相关领域在动作分析、自动化教练和表现评估方面的研究进展。
...
### [150] [Beyond Visual Cues: Leveraging General Semantics as Support for Few-Shot Segmentation](https://arxiv.org/abs/2511.16435)
*Jin Wang,Bingfeng Zhang,Jian Pang,Mengyu Liu,Honglong Chen,Weifeng Liu*

Main category: cs.CV

TL;DR: 本文提出了一种语言驱动的属性泛化（LDAG）架构，利用大型语言模型（LLMs）生成多属性描述，为小样本分割（FSS）提供无偏的元指导，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有小样本分割方法主要从支持图像中提取元指导，但由于视觉表示的类内差异，这种元信息无法为未训练类别提供准确指导。研究动机在于提供对训练和未训练类别都无偏的元指导。

Method: 引入语言驱动的属性泛化（LDAG）架构。设计了多属性增强（MaE）模块，利用LLMs生成目标类别的多个详细属性描述，并通过多模态匹配构建精细的视觉-文本先验指导。同时，为解决文本-视觉模态偏移问题，设计了多模态属性对齐（MaA）模块，实现属性文本与视觉特征的跨模态交互。

Result: 实验证明，所提出的方法在性能上明显优于现有方法，并达到了新的最先进水平。

Conclusion: 通过利用目标属性的语言描述和LLMs生成无偏元指导，本文成功解决了小样本分割中现有方法的局限性，实现了更准确、鲁棒的分割性能。
...
### [151] [PartUV: Part-Based UV Unwrapping of 3D Meshes](https://arxiv.org/abs/2511.16659)
*Zhaoning Wang,Xinyue Wei,Ruoxi Shi,Xiaoshuai Zhang,Hao Su,Minghua Liu*

Main category: cs.CV

TL;DR: PartUV是一种基于部件的UV展开流水线，专为处理AI生成网格的噪声和不良条件而设计，它能生成更少、与部件对齐的图表，同时保持低失真。


<details>
  <summary>Details</summary>
Motivation: 现有的UV展开方法难以处理AI生成的网格，这些网格通常噪声大、凹凸不平且条件差，导致图表高度碎片化和边界次优，引入伪影并阻碍下游任务。

Method: PartUV是一个基于部件的UV展开流水线，它建立在学习型部件分解方法PartField之上，结合了高级语义部件分解和新颖的几何启发式方法，采用自顶向下的递归框架。它确保每个图表的失真低于用户指定阈值，同时最小化图表总数，并集成了参数化和打包算法，专门处理非流形和退化网格，并进行了广泛并行化以提高效率。

Result: PartUV在四个不同数据集（包括人造、CAD、AI生成和通用形状）上的评估表明，它在图表数量和接缝长度方面优于现有工具和最新神经方法，实现了可比的失真，在具有挑战性的网格上表现出高成功率，并支持了部件特定的多瓦片打包等新应用。

Conclusion: PartUV通过结合语义部件分解和几何启发式方法，提供了一个强大的UV展开解决方案，特别适用于处理AI生成网格的挑战，能够生成数量更少、与部件对齐且失真度低的图表。
...
### [152] [TriDiff-4D: Fast 4D Generation through Diffusion-based Triplane Re-posing](https://arxiv.org/abs/2511.16662)
*Eddie Pokming Sheung,Qihao Liu,Wufei Ma,Prakhar Kaushik,Jianwen Xie,Alan Yuille*

Main category: cs.CV

TL;DR: TriDiff-4D提出了一种新颖的扩散模型，通过三平面重摆姿态，实现了从文本描述生成高质量、时间连贯且可控的任意长度4D虚拟形象，显著提高了生成效率和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 当前4D生成模型在从文本生成高保真、可控的4D虚拟形象方面存在局限性，包括时间与几何不一致、感知伪影、运动不规律、计算成本高以及对动态控制有限，阻碍了其广泛应用。

Method: TriDiff-4D采用扩散模型结合三平面重摆姿态的4D生成流程。它使用自回归策略生成任意长度的4D序列，每个3D帧通过一次扩散过程合成。模型显式学习大规模3D和运动数据集中的3D结构和运动先验，实现骨架驱动的4D生成。具体而言，它首先从文本提示生成规范的3D虚拟形象和运动序列，然后使用第二个扩散模型根据运动序列驱动虚拟形象动画化。

Result: TriDiff-4D显著优于现有方法，将生成时间从数小时缩短至数秒（通过消除优化过程），同时大幅改进了复杂运动的生成，实现了高保真外观和精确的3D几何。

Conclusion: TriDiff-4D成功解决了4D虚拟形象生成中的挑战，通过扩散模型和三平面重摆姿态，实现了高质量、时间连贯、计算高效且可控的4D虚拟形象生成，为3D动画领域带来了重大突破。
...
### [153] [Progressive Supernet Training for Efficient Visual Autoregressive Modeling](https://arxiv.org/abs/2511.16546)
...
### [154] [Late-decoupled 3D Hierarchical Semantic Segmentation with Semantic Prototype Discrimination based Bi-branch Supervision](https://arxiv.org/abs/2511.16650)
*Shuyu Cao,Chongshou Li,Jie Xu,Tianrui Li,Na Zhao*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的框架，通过解耦的3DHS分支和辅助判别分支，结合层次指导和语义原型双分支监督，解决了3D分层语义分割中的多层次冲突和类别不平衡问题，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的3D分层语义分割(3DHS)方法忽略了两个主要挑战：1) 参数共享模型进行多标签学习时，在跨层次优化中可能导致多层次冲突；2) 3D场景中多层次固有的类别不平衡问题，导致模型性能被主要类别主导。

Method: 本文提出一个新颖的框架，包含一个主3DHS分支和一个辅助判别分支。为缓解多层次冲突，引入了“晚期解耦”的3DHS框架，该框架采用多个解码器，并结合粗到精的层次指导和一致性。此外，引入了面向3DHS的语义原型双分支监督机制，学习类别判别性点云特征，并在辅助分支和3DHS分支之间进行相互监督，以增强类别不平衡分割。

Result: 在多个数据集和骨干网络上的大量实验表明，该方法实现了最先进的3DHS性能。其核心组件还可以作为即插即用的增强模块，改进现有方法。

Conclusion: 本研究通过提出晚期解耦的3DHS框架和基于语义原型的双分支监督机制，成功解决了3D分层语义分割中的多层次冲突和类别不平衡问题，显著提升了模型性能，并达到了最先进水平。
...
### [155] [Contrastive vision-language learning with paraphrasing and negation](https://arxiv.org/abs/2511.16527)
*Kwun Ho Ngan,Saman Sadeghi Afgeh,Joe Townsend,Artur d'Avila Garcez*

Main category: cs.CV

TL;DR: SemCLIP通过引入考虑释义和否定语义的新对比损失函数，并利用大型语言模型生成的三元组数据进行训练，显著提高了视觉-语言模型（如CLIP）对语义转换（特别是否定）的鲁棒性，同时保持了其原有性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比视觉-语言模型（如CLIP）在处理否定或释义文本时表现不佳。否定会以最小的词汇变化彻底改变含义，而释义则可以用非常不同的文本表达相同的含义。这给评估和对齐带来了挑战，促使研究如何提高模型对这些语义转换的鲁棒性。

Method: 本研究评估了释义和否定相结合的情况，并提出了一种新的CLIP对比损失函数，该函数同时考虑了释义和否定。此外，该方法利用大型语言模型（LLM）生成由原始、释义和否定文本标题组成的三元组训练数据，并将其应用于类CLIP的训练模型中。这种方法被称为SemCLIP。

Result: SemCLIP被证明能够将释义标题向原始图像嵌入移动，同时将否定标题在嵌入空间中推得更远。在CC-Neg基准测试中，使用“原始与否定图像检索准确率”指标，SemCLIP将准确率从68.1%提高到78.1%。尽管在Sugarcrepe++基准测试中与CLIP相比结果喜忧参半，但SemCLIP的性能普遍优于使用否定标题训练的模型。这种对否定的鲁棒性也延伸到了下游的零样本分类任务中，经过Sugarcrepe++预训练的SemCLIP在所有测试的下游任务中均优于CLIP。

Conclusion: SemCLIP能够显著提高视觉-语言模型对语义转换的鲁棒性，尤其是在处理否定句方面表现出色，同时保持了其在其他方面的性能。
...
### [156] [Erase to Retain: Low Rank Adaptation Guided Selective Unlearning in Medical Segmentation Networks](https://arxiv.org/abs/2511.16574)
...
### [157] [Investigating Optical Flow Computation: From Local Methods to a Multiresolution Horn-Schunck Implementation with Bilinear Interpolation](https://arxiv.org/abs/2511.16535)
*Haytham Ziani*
...
### [158] [EOGS++: Earth Observation Gaussian Splatting with Internal Camera Refinement and Direct Panchromatic Rendering](https://arxiv.org/abs/2511.16542)
...
### [159] [POMA-3D: The Point Map Way to 3D Scene Understanding](https://arxiv.org/abs/2511.16567)
...
### [160] [NoPo-Avatar: Generalizable and Animatable Avatars from Sparse Inputs without Human Poses](https://arxiv.org/abs/2511.16673)
*Jing Wen,Alexander G. Schwing,Shenlong Wang*

Main category: cs.CV

TL;DR: 本文提出NoPo-Avatar，一种从单张或稀疏图像中重建可动画3D人体化身的方法，该方法不依赖于任何姿态输入（包括相机和人体姿态），从而避免了姿态估计噪声的影响，提高了适用性。


<details>
  <summary>Details</summary>
Motivation: 现有的最先进方法在测试时需要准确的“真实”相机姿态和人体姿态作为输入来指导重建。研究表明，如果姿态估计存在噪声，这种姿态依赖的重建会显著降低结果。因此，需要一种不受噪声姿态估计影响的方法。

Method: 本文引入NoPo-Avatar，它仅从图像中重建化身，无需任何姿态输入。通过消除测试时重建对人体姿态的依赖，NoPo-Avatar不受噪声人体姿态估计的影响。

Result: 在具有挑战性的THuman2.0、XHuman和HuGe100K数据集上的实验表明，在实际设置（无真实姿态）中，NoPo-Avatar优于现有基线；在实验室设置（有真实姿态）中，其结果与现有方法相当。

Conclusion: NoPo-Avatar成功实现了无需姿态输入的3D人体化身重建，解决了现有方法对准确姿态估计的依赖问题，使其在更广泛的应用场景中表现出更强的鲁棒性和实用性。
...
### [161] [Video-as-Answer: Predict and Generate Next Video Event with Joint-GRPO](https://arxiv.org/abs/2511.16669)
*Junhao Cheng,Liang Hou,Xin Tao,Jing Liao*

Main category: cs.CV

TL;DR: 该论文提出了视频下一事件预测（VNEP）任务，将视频作为下一事件预测的新回答模式。为解决该任务的挑战，作者引入了VANS模型，该模型通过强化学习（Joint-GRPO）对视觉语言模型（VLM）和视频扩散模型（VDM）进行对齐，并构建了VANS-Data-100K数据集。实验证明VANS在视频事件预测和可视化方面均达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型在许多实际应用中影响深远，但视频生成仍主要局限于娱乐。视频具有独特的能力，能展示仅凭语言难以传达的物理世界信息（如系领带），因此存在一个未被充分利用的机会，即将视频作为下一事件预测（NEP）的新回答模式，即视频下一事件预测（VNEP）。传统的NEP任务以文本形式预测下一事件，而VNEP需要动态视频响应，这能为程序性学习和创意探索提供更直观、更定制化的答案。然而，现有模型难以应对这项任务，因为它需要多模态输入理解、指令条件推理以及生成具有视觉和语义一致性的视频。

Method: 本文引入了VNEP任务，要求模型以动态视频响应来预测下一事件。为解决此挑战，作者提出了VANS模型，该模型利用强化学习将视觉语言模型（VLM）与视频扩散模型（VDM）对齐。VANS的核心是提出的Joint-GRPO机制，它将VLM和VDM作为一个整体进行协调。通过共享奖励驱动，Joint-GRPO优化VLM生成既准确又易于可视化的字幕，同时指导VDM生成忠实于这些字幕和输入视觉上下文的视频。为了实现这一学习过程，作者还创建了专用于VNEP任务的VANS-Data-100K数据集。

Result: 在程序性和预测性基准测试中，VANS模型在视频事件预测和可视化方面均取得了最先进的性能。

Conclusion: VANS模型成功地将视频作为下一事件预测的新回答模式，解决了现有模型在多模态理解、指令推理和视频生成方面的挑战，并在相关任务中达到了领先水平，展示了视频作为答案模态的巨大潜力。
...
### [162] [Solving Spatial Supersensing Without Spatial Supersensing](https://arxiv.org/abs/2511.16655)
...
### [163] [SceneDesigner: Controllable Multi-Object Image Generation with 9-DoF Pose Manipulation](https://arxiv.org/abs/2511.16666)
*Zhenyuan Qin,Xincheng Shuai,Henghui Ding*

Main category: cs.CV

TL;DR: SceneDesigner提出了一种新的方法，通过引入分支网络、CNOCS图、新数据集和两阶段强化学习训练策略，实现了对图像中多个对象的9D姿态（位置、大小、方向）的精确灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成方法在实现对多个对象的9D姿态同时控制方面存在局限性，通常表现出可控性有限和质量下降的问题，无法满足全面的多对象9D姿态控制需求。

Method: 本文提出了SceneDesigner，它在预训练基础模型中引入了分支网络，并利用一种新的表示CNOCS图来编码9D姿态信息。为支持训练，构建了新的ObjectPose9D数据集。为解决数据不平衡问题，特别是低频姿态的性能下降，引入了两阶段强化学习训练策略。在推理时，提出了Disentangled Object Sampling技术以缓解复杂多对象场景中的对象生成不足和概念混淆。此外，通过集成用户特定的个性化权重，实现了对参考主体的定制姿态控制。

Result: 大量的定性和定量实验表明，SceneDesigner在可控性和图像质量方面均显著优于现有方法。

Conclusion: SceneDesigner成功解决了多对象9D姿态控制的挑战，提供了一种准确且灵活的多对象9自由度姿态操作方法，并显著提升了生成图像的可控性和质量。
...
### [164] [EvoLMM: Self-Evolving Large Multimodal Models with Continuous Rewards](https://arxiv.org/abs/2511.16672)
...
TL;DR: 本文提出EvoLMM，一个自进化的框架，通过两个合作代理（提问者和解决者）在完全无监督的情况下提升大型多模态模型（LMMs）的推理能力，无需人工标注数据或外部奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有LMMs的训练管道依赖于人工标注数据或外部验证的奖励模型，这限制了它们的自主性和可扩展性。研究旨在以纯无监督的方式改进LMM的推理能力。

Method: EvoLMM框架从单一骨干模型实例化出两个合作代理：一个“提问者”生成多样化、基于图像的问题，一个“解决者”通过内部一致性来解决这些问题。学习过程通过持续的自我奖励进行，这种动态反馈在不依赖真实标签或人工判断的情况下，鼓励生成信息丰富的查询并完善结构化推理。

Result: 使用流行的Qwen2.5-VL作为基础模型，EvoLMM在仅使用原始训练图像的情况下，在多模态数学推理基准（包括ChartQA、MathVista和MathVision）上实现了高达约3%的持续提升。

Conclusion: EvoLMM提供了一种简单而有效的方法，为未来在完全无监督情境下自改进LMMs的研究奠定了坚实的基础。
...
### [165] [V-ReasonBench: Toward Unified Reasoning Benchmark Suite for Video Generation Models](https://arxiv.org/abs/2511.16668)
...
### [166] [Learning to Think Fast and Slow for Visual Language Models](https://arxiv.org/abs/2511.16670)
...
### [167] [What Really Counts? Examining Step and Token Level Attribution in Multilingual CoT Reasoning](https://arxiv.org/abs/2511.15886)
*Jeremias Ferrao,Ezgi Basar,Khondoker Ittehadul Islam,Mahrokh Hassani*

Main category: cs.CL

TL;DR: 本研究调查了多语言大型语言模型中思维链（CoT）推理的归因模式，发现归因分数过度侧重最终步骤，结构化CoT对高资源语言更有效，且扰动会降低准确性和归因连贯性，揭示了CoT提示在多语言鲁棒性和解释性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管思维链（CoT）提示能提高任务性能，但对其生成推理链的忠实性和可解释性存在担忧。本研究旨在评估这些特性在不同语言中的表现。

Method: 研究使用两种互补的归因方法——ContextCite（用于步骤级归因）和Inseq（用于令牌级归因），应用于Qwen2.5 1.5B-Instruct模型，并在MGSM基准上进行实验。

Result: 主要发现包括：(1) 归因分数过度强调最终推理步骤，尤其是在不正确的生成中；(2) 结构化CoT提示主要提高了高资源拉丁字母语言的准确性；(3) 通过否定和干扰句进行的受控扰动降低了模型准确性和归因连贯性。

Conclusion: 这些发现揭示了CoT提示的局限性，特别是在多语言鲁棒性和解释性透明度方面。
...
### [168] [Mind the Motions: Benchmarking Theory-of-Mind in Everyday Body Language](https://arxiv.org/abs/2511.15887)
*Seungbeen Lee,Jinhong Jeong,Donghyun Kim,Yejin Son,Youngjae Yu*

Main category: cs.CL

TL;DR: 本文提出了Motion2Mind框架和数据集，用于评估机器解读非语言线索（NVCs）以推断他人心智状态的能力。研究发现当前AI系统在NVC解读方面表现不佳，存在检测性能差距和过度解读现象。


<details>
  <summary>Details</summary>
Motivation: 现有的心智理论（ToM）基准主要关注错误信念任务和不对称信息推理，却忽略了信念之外的其他心智状态以及人类非语言交流的丰富性。解读非语言线索对生存和社会凝聚至关重要，但机器在这方面的能力尚未得到充分评估。

Method: 研究构建了Motion2Mind框架，利用专家整理的肢体语言参考作为代理知识库。在此基础上，创建了一个精心策划的视频数据集，其中包含细粒度的非语言线索注释和手动验证的心理学解释。该数据集涵盖222种非语言线索和397种心智状态。

Result: 评估显示，当前的AI系统在非语言线索解读方面表现出显著的困难。与人类标注者相比，AI系统不仅在“检测”方面存在巨大的性能差距，而且在“解释”方面也呈现出过度解读的模式。

Conclusion: 当前AI系统在解读非语言线索以推断心智状态方面能力不足，在检测准确性和解释合理性上均与人类存在较大差距。这表明机器在理解人类非语言沟通和心智理论方面仍有很大的提升空间。
...
### [169] [TOD-ProcBench: Benchmarking Complex Instruction-Following in Task-Oriented Dialogues](https://arxiv.org/abs/2511.15976)
*Sarik Ghazarian,Abhinav Gullapalli,Swair Shah,Anurag Beniwal,Nanyun Peng,Narayanan Sadagopan,Zhou Yu*

Main category: cs.CL

TL;DR: 该论文提出了TOD-ProcBench，一个针对多轮任务导向对话（TOD）的新基准，旨在评估大型语言模型（LLMs）理解和遵循复杂、细粒度自然语言指令的能力，以弥补现有基准的不足。


<details>
  <summary>Details</summary>
Motivation: 在真实的TOD场景中，代理需要严格遵循复杂的自然语言指令，而现有TOD基准往往将这些指令简化为简单的模式。这导致缺乏系统评估LLMs在复杂指令遵循能力方面的工具。

Method: 研究者提出了TOD-ProcBench基准，其数据集源自高质量的ABCD数据集，并经过人工质量控制。他们将细粒度约束和操作流程表述为多级条件-动作指令语句。设计了三个任务来全面评估LLMs的指令遵循能力：任务1评估LLMs检索相关指令并预测下一步动作的能力；任务2评估LLMs识别违反指令响应的能力；任务3评估LLMs基于复杂指令条件生成符合指令响应的能力。此外，还研究了多语言设置和不同指令文本格式对合规性能的影响。

Result: 该研究成功构建了一个具有挑战性的TOD-ProcBench基准，该基准包含复杂的流程指令和细致的约束，并设计了三项任务来全面衡量LLMs在多轮TOD中理解和遵循指令的能力。同时，还发布了该基准，以促进LLMs指令遵循能力的研究和发展。

Conclusion: TOD-ProcBench基准有效解决了现有TOD基准在评估LLMs复杂指令遵循能力方面的不足，为系统性地评估和提升LLMs在真实世界任务导向对话中的表现提供了一个重要工具。
...
### [170] [Liars' Bench: Evaluating Lie Detectors for Language Models](https://arxiv.org/abs/2511.16035)
*Kieron Kretschmar,Walter Laurito,Sharan Maiya,Samuel Marks*

Main category: cs.CL

TL;DR: 本文介绍了LIARS' BENCH，一个包含72,863个谎言和诚实回复的测试平台，用于评估大型语言模型（LLM）的谎言检测技术，并揭示了现有技术的局限性。


<details>
  <summary>Details</summary>
Motivation: 先前的LLM谎言检测技术通常在狭窄的设置中进行验证，无法捕捉LLM可能生成的各种不同类型的谎言，因此需要一个更全面、多样化的测试平台。

Method: 研究者引入了LIARS' BENCH，一个由四个开源模型在七个数据集上生成的72,863个谎言和诚实回复组成的测试平台。该平台捕获了不同类型的谎言，并根据模型撒谎的原因和谎言所针对的信念对象这两个维度进行变化。在此平台上评估了三种黑盒和白盒谎言检测技术。

Result: 评估结果表明，现有技术系统性地未能识别某些类型的谎言，尤其是在仅凭文本记录无法判断模型是否撒谎的情况下。这揭示了现有技术在检测多样化谎言方面的局限性。

Conclusion: LIARS' BENCH揭示了现有LLM谎言检测技术的不足，并提供了一个实用的测试平台，以指导未来谎言检测技术的发展和进步。
...
### [171] [Learning Tractable Distributions Of Language Model Continuations](https://arxiv.org/abs/2511.16054)
*Gwen Yidou-Weng,Ian Li,Anji Liu,Oliver Broadrick,Guy Van den Broeck,Benjie Wang*

Main category: cs.CL

TL;DR: 本文提出了一种名为LTLA（Learning to Look Ahead）的混合方法，用于受控语言生成。它将自回归语言模型（LM）的丰富上下文编码与可处理的替代模型相结合，以计算精确的延续概率，解决了现有方法上下文感知能力弱和效率低下的问题，显著提高了约束满足度和生成质量。


<details>
  <summary>Details</summary>
Motivation: 受控语言生成需要文本满足序列级约束（如语法、风格、安全性），这些约束可能依赖于未来的token，使得直接条件化自回归LM变得困难。现有方法使用可处理的替代模型（如HMM）来近似延续分布并调整token logits，但这些替代模型通常上下文感知能力弱，导致查询质量下降。

Method: LTLA是一种混合方法，它将基础语言模型用于丰富的上下文（前缀）编码，并结合一个固定的、可处理的替代模型来计算精确的延续概率。为解决效率问题，LTLA采取了两项策略：(i) 使用单次批量HMM更新来同时考虑所有候选的下一个token，避免了对每个候选token进行重新评分；(ii) 仅将替代模型的潜在状态先验条件化于LM的隐藏表示，同时保持替代解码器固定，从而允许在不同前缀之间重用计算。

Result: 实验结果表明，LTLA实现了比无条件HMM更高的条件似然；它能够为视觉-语言模型近似延续分布，而独立HMM无法编码视觉上下文；在受控生成任务中，LTLA在保持可比流畅性的前提下，显著提高了约束满足度，且推理开销极小。

Conclusion: LTLA成功地将神经上下文与可处理的替代模型结合起来，有效解决了受控语言生成中序列级约束的挑战，提高了生成质量和效率，并能处理更复杂的应用场景（如视觉-语言模型）。
...
### [172] [Early science acceleration experiments with GPT-5](https://arxiv.org/abs/2511.16072)
...
### [173] [ELPO: Ensemble Learning Based Prompt Optimization for Large Language Models](https://arxiv.org/abs/2511.16122)
...
TL;DR: 本文提出了一种名为ELPO的集成学习框架，通过结合投票机制、共享生成策略和多种搜索方法，实现了对大型语言模型提示词的更准确和鲁棒的自动优化，超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的性能高度依赖于精心设计的提示词，但手动提示词工程过程繁琐。现有的自动提示词优化（APO）方法通常局限于单一模型或算法，导致在处理复杂任务时性能受限。

Method: 本文提出了基于集成学习的提示词优化（ELPO）框架。该框架引入了投票机制，并结合了共享生成策略和不同的搜索方法来寻找更优的提示词。此外，ELPO还创造性地提出了更高效的提示词生成和搜索算法。

Result: 实验结果表明，ELPO在不同任务上均优于现有的最先进提示词优化方法，例如在ArSarcasm数据集上将F1分数提高了7.6。

Conclusion: ELPO框架通过集成学习的思想，成功实现了对LLMs提示词的更准确和鲁棒的自动优化，有效解决了现有APO方法的局限性。
...
### [174] [TS-PEFT: Token-Selective Parameter-Efficient Fine-Tuning with Learnable Threshold Gating](https://arxiv.org/abs/2511.16147)
...
### [175] [SemanticCite: Citation Verification with AI-Powered Full-Text Analysis and Evidence-Based Reasoning](https://arxiv.org/abs/2511.16198)
*Sebastian Haan*
...
### [176] [SeSE: A Structural Information-Guided Uncertainty Quantification Framework for Hallucination Detection in LLMs](https://arxiv.org/abs/2511.16275)
*Xingtao Zhao,Hao Peng,Dingli Su,Xianghua Zeng,Chunyang Liu,Jinzhi Liao,Philip S. Yu*

Main category: cs.CL

TL;DR: 本文提出了语义结构熵（SeSE），一个从结构信息角度量化大型语言模型（LLMs）内在语义不确定性的框架，用于幻觉检测，并在多项实验中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在安全关键场景中部署LLMs需要可靠的不确定性量化（UQ）以避免幻觉。然而，现有UQ方法主要依赖语义概率分布或成对距离，忽略了能提供更精确不确定性估计的潜在语义结构信息。

Method: SeSE通过以下方式实现：1) 开发了一种自适应稀疏化有向语义图构建算法，用于建模语义空间，捕捉方向性语义依赖并剪除干扰连接。2) 利用分层抽象，将SeSE定义为最优语义编码树的结构熵，量化语义空间内的内在不确定性。SeSE值越高表示不确定性越大。3) 扩展SeSE以通过建模随机语义交互来量化长文本生成中单个主张的不确定性。

Result: 在29种模型-数据集组合上的广泛实验表明，SeSE显著优于先进的UQ基线，包括强大的监督方法和最近提出的KLE。

Conclusion: SeSE提供了一个基于结构信息原理的UQ框架，能从语义结构角度量化LLMs的不确定性，实现更精确的幻觉检测，并为长文本生成中的细粒度UQ提供了理论上可解释的方法。
...
### [177] [Incorporating Self-Rewriting into Large Language Model Reasoning Reinforcement](https://arxiv.org/abs/2511.16331)
*Jiashu Yao,Heyan Huang,Shuang Zeng,Chuwei Luo,WangJie You,Jie Tang,Qingsong Liu,Yuhang Guo,Yangyang Kang*

Main category: cs.CL

TL;DR: 现有强化学习（RL）虽提升大型推理模型（LRM）性能，但单一结果正确性奖励导致内部推理质量不佳。本文提出自重写框架，模型重写自身推理文本并从中学习，以改善内部推理过程，在准确性和推理长度上取得显著提升，并有效缓解内部推理缺陷。


<details>
  <summary>Details</summary>
Motivation: 通过强化学习（RL）训练的大型推理模型（LRM）在复杂推理任务上表现出色，但其单一的最终正确性奖励机制无法提供对内部推理过程的详细监督。这导致了内部推理质量不佳，表现为过度思考、思考不足、冗余思考和无序思考等问题。

Method: 受LRM自奖励进展启发，本文引入了自重写框架，模型重写自己的推理文本，并从重写后的推理中学习以提高内部思维过程质量。算法设计上，提出选择性重写方法，只对模型能持续正确解答的“简单”样本进行重写，以保留GRPO的原始奖励信号。实践中，将重写和常规生成合并在同一批次中处理，保持RL算法的可扩展性，且仅增加约10%的开销。

Result: 在不同的任务和模型尺寸上，自重写方法验证了其有效性。在准确性-长度权衡方面，即使没有明确指示减少推理长度，自重写方法在提高准确性（+0.6）的同时大幅缩短了推理长度（-46%），优于现有强基线。在内部推理质量方面，使用LLM-as-a-judge指标，自重写实现了显著更高的分数（+7.2），成功缓解了内部推理缺陷。

Conclusion: 自重写框架通过让模型重写并学习自身的推理文本，有效解决了现有强化学习方法中单一结果正确性奖励导致的内部推理质量问题。它显著提升了大型推理模型的准确性，同时大幅缩短了推理长度，并提高了内部推理过程的质量，成功缓解了过度思考、冗余思考等缺陷。
...
### [178] [SDA: Steering-Driven Distribution Alignment for Open LLMs without Fine-Tuning](https://arxiv.org/abs/2511.16324)
*Wei Xia,Zhi-Hong Deng*

Main category: cs.CL

TL;DR: 本文提出SDA（Steering-Driven Distribution Alignment），一个免训练、模型无关的对齐框架，通过动态重分配模型输出概率，在推理阶段将开源大型语言模型（LLMs）与人类意图对齐。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的广泛部署，确保它们在推理阶段无需昂贵的再训练或大量监督，就能有效地与人类意图对齐，成为一个关键且具有挑战性的需求。

Method: SDA框架通过基于用户定义的对齐指令，动态地重新分配模型输出概率，以增强模型行为与人类意图的对齐，而无需微调。它轻量、资源高效，并兼容多种开源LLMs，可独立运行或与基于训练的对齐策略结合使用，并支持个性化偏好对齐。

Result: SDA在8个不同规模和来源的开源LLMs上，针对有用性、无害性和诚实性（3H）三个关键对齐维度进行了评估，结果显示其持续提升了对齐性能。具体而言，在测试模型中，SDA在有用性方面平均提升64.4%，在诚实性方面提升30%，在无害性方面提升11.5%。

Conclusion: SDA是一个有效、通用、轻量且资源高效的方法，能够在推理阶段将LLMs与人类意图对齐，支持个性化偏好，并适用于各种开源LLMs和应用场景。
...
### [179] [NLP Datasets for Idiom and Figurative Language Tasks](https://arxiv.org/abs/2511.16345)
*Blake Matheny,Phuong Minh Nguyen,Minh Le Nguyen,Stephanie Reynolds*

Main category: cs.CL

TL;DR: 本文介绍了为大型语言模型（LLMs）处理习语和比喻语言而创建的新数据集，旨在弥补现有模型在此方面的不足。


<details>
  <summary>Details</summary>
Motivation: 习语和比喻语言在日常交流中占比很大，但LLMs在理解它们时仍面临挑战，即使拥有大量语料库。微调方法显示出潜力，但需要更好、更大的数据集来进一步缩小差距。

Method: 研究人员首先整合了近期习语和比喻语言数据集，形成一个综合习语列表。然后，他们使用此列表从大型语料库中检索上下文序列。最终创建了一个大规模的潜在习语和比喻表达数据集，以及两个额外的人工标注的明确习语和比喻表达数据集。这些数据集经过后处理以兼容模型无关的训练，并用于槽位标注和序列标注任务中的训练和评估。

Result: 本文成功创建了一个大规模的潜在习语和比喻语言表达数据集，以及两个用于习语识别（检测）任务的人工标注的明确习语和比喻语言表达数据集。这些数据集用于评估预训练语言模型处理比喻意义的基线能力。

Conclusion: 所提供的数据集为构建新模型和开发新方法以提升LLMs对习语和比喻语言的理解能力提供了一个解决方案，并提供了多样化的类别以供进一步研究。
...
### [180] [AICC: Parse HTML Finer, Make Models Better -- A 7.3T AI-Ready Corpus Built by a Model-Based HTML Parser](https://arxiv.org/abs/2511.16397)
...
### [181] [Learning from Sufficient Rationales: Analysing the Relationship Between Explanation Faithfulness and Token-level Regularisation Strategies](https://arxiv.org/abs/2511.16353)
...
### [182] [Arctic-Extract Technical Report](https://arxiv.org/abs/2511.16470)
*Mateusz Chiliński,Julita Ołtusek,Wojciech Jaśkowski*

Main category: cs.CL

TL;DR: Arctic-Extract是一个先进的模型，用于从商业文档中提取结构化数据（问答、实体和表格），其特点是轻量级且性能强大，适用于资源受限的硬件和长文档处理。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一个既拥有最先进（SoTA）能力，又能部署在资源受限硬件上，并能有效处理长文档的结构化数据提取模型。

Method: 本文主要介绍了Arctic-Extract模型的训练协议和评估结果。该模型旨在实现最先进的文档理解能力，同时保持轻量级设计（6.6 GiB），以适应有限的硬件资源（如24 GB内存的A10 GPU）。

Result: Arctic-Extract模型展示了强大的文档理解性能，其大小仅为6.6 GiB，可在资源受限的硬件（如24 GB内存的A10 GPU）上部署。在这些GPU上，它能够处理多达125页A4文档，非常适合长文档处理。

Conclusion: Arctic-Extract是一个高效且高性能的解决方案，能够从扫描或数字原生商业文档中提取结构化数据，尤其适用于资源有限的环境和需要处理大量文档的场景。
...
### [183] [Classification of worldwide news articles by perceived quality, 2018-2024](https://arxiv.org/abs/2511.16416)
...
### [184] [ESGBench: A Benchmark for Explainable ESG Question Answering in Corporate Sustainability Reports](https://arxiv.org/abs/2511.16438)
*Sherine George,Nithish Saji*

Main category: cs.CL

TL;DR: 本文介绍了ESGBench，一个旨在评估可解释ESG问答系统的基准数据集和评估框架，并分析了现有大型语言模型在该基准上的表现。


<details>
  <summary>Details</summary>
Motivation: 研究动机是需要一个工具来评估和改进基于企业可持续发展报告的可解释ESG问答系统，以解决透明度和问责制方面的挑战。

Method: 方法是构建了ESGBench，一个包含多个ESG主题的领域问题、人工标注答案和支持证据的数据集。该框架还被用于评估最先进的大型语言模型（LLMs）的表现。

Result: 研究结果显示，最先进的大型语言模型在ESGBench上存在事实一致性、可追溯性和领域对齐方面的关键挑战。

Conclusion: ESGBench旨在加速透明和负责任的ESG领域人工智能系统的研究和发展。
...
### [185] [Anatomy of an Idiom: Tracing Non-Compositionality in Language Models](https://arxiv.org/abs/2511.16467)
*Andrew Gomes*
...
### [186] [Beyond Tokens in Language Models: Interpreting Activations through Text Genre Chunks](https://arxiv.org/abs/2511.16540)
*Éloïse Benito-Rodriguez,Einar Urdshals,Jasmina Nasufi,Nicky Pochinkov*

Main category: cs.CL

TL;DR: 本文提出一个预测框架，通过LLM的激活来预测用于提示文本的体裁，并使用Mistral-7B和浅层学习模型实现了高精度预测。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型（LLM）对于其安全和有益部署至关重要，但LLM结构难以解释且所有输出无法进行人工评估，这促使研究人员探索新的解释性方法。

Method: 研究人员使用Mistral-7B模型和两个数据集，通过其激活层提取信息，并利用scikit-learn分类器来预测输入文本的体裁。

Result: 结果显示，使用scikit-learn分类器预测文本体裁的F1分数分别高达98%和71%。在两个数据集上，这些结果均显著优于对照任务，证明了从LLM激活中推断文本体裁的可行性。

Conclusion: 本研究提供了一个概念验证，表明可以通过浅层学习模型从大型语言模型中推断文本体裁，为LLM的预测性解释框架迈出了第一步。
...
### [187] [TurkColBERT: A Benchmark of Dense and Late-Interaction Models for Turkish Information Retrieval](https://arxiv.org/abs/2511.16528)
...
### [188] [WER is Unaware: Assessing How ASR Errors Distort Clinical Understanding in Patient Facing Dialogue](https://arxiv.org/abs/2511.16544)
...
TL;DR: 该研究挑战了在临床对话中依赖词错误率（WER）评估自动语音识别（ASR）的现状，发现WER与临床影响相关性差。为此，引入了一个经过优化的“大语言模型作为评判者”（LLM-as-a-Judge），该模型能以与人类专家相当的水平自动评估转录错误的临床影响。


<details>
  <summary>Details</summary>
Motivation: 随着ASR在临床对话中的应用日益增多，标准的评估指标（如WER）未能充分反映转录错误对临床的实际影响。研究者旨在探究WER或其他现有指标是否能有效关联转录错误的临床影响，并寻求一种更侧重安全、可扩展的评估方法。

Method: 研究首先建立了黄金标准基准，由临床专家比较真实语音和ASR转录结果，并标记发现的差异所带来的临床影响（无、轻微或显著影响）。随后，分析了WER及其他现有指标与临床医生分配的风险标签的相关性。最后，引入并使用GEPA优化了一个“大语言模型作为评判者”，使其能够复制专家临床评估。

Result: 分析表明，WER和一套现有指标与临床医生分配的风险标签（无、轻微或显著影响）相关性很差。而优化后的LLM评判者（Gemini-2.5-Pro）达到了与人类专家相当的性能，获得了90%的准确率和0.816的Cohen's κ值。

Conclusion: 这项工作提供了一个经过验证的自动化框架，用于将临床对话中ASR的评估从简单的文本忠实度提升到必要的、可扩展的安全性评估。这对于确保ASR在医疗领域的安全应用至关重要。
...
### [189] [Nemotron Elastic: Towards Efficient Many-in-One Reasoning LLMs](https://arxiv.org/abs/2511.16664)
...
TL;DR: Nemotron Elastic是一个框架，它通过在一个父模型中嵌入多个嵌套子模型，显著降低了训练一系列针对不同规模和部署目标的LLM的成本，同时保持或提升了性能。


<details>
  <summary>Details</summary>
Motivation: 训练一系列针对不同规模和部署目标的LLM成本极高，即使是现有的模型压缩技术也需要数百亿token的训练成本，这促使研究人员寻求更高效的方法。

Method: 本文提出了Nemotron Elastic框架，用于构建推理导向的LLM，包括混合Mamba-Attention架构。它在一个父模型中嵌入多个嵌套子模型，这些子模型与父模型共享权重，并可在部署时零样本提取，无需额外训练或微调。实现这一功能的核心是一个端到端训练的路由器，并结合了两阶段训练课程。此外，还引入了组感知SSM弹性化、异构MLP弹性化、基于归一化MSE的层重要性以及知识蒸馏，以实现多预算同时优化。

Result: 将Nemotron Elastic应用于Nemotron Nano V2 12B模型，仅使用110B训练token就同时生成了9B和6B模型。与从头训练模型家族相比，成本降低了360倍以上；与现有最先进的压缩技术相比，成本降低了约7倍。每个嵌套模型的准确性都与最先进的技术相当或更好。此外，该方法允许拥有一个多合一的推理模型，其部署内存与家族中模型的数量无关。

Conclusion: Nemotron Elastic提供了一种革命性的方法，可以在保持或超越现有性能的同时，显著降低构建多规模推理导向LLM家族的成本，并提供高效的部署能力。
...
### [190] [Integrating Symbolic Natural Language Understanding and Language Models for Word Sense Disambiguation](https://arxiv.org/abs/2511.16577)
...
### [191] [Comparison of Text-Based and Image-Based Retrieval in Multimodal Retrieval Augmented Generation Large Language Model Systems](https://arxiv.org/abs/2511.16654)
...
### [192] [Gimballed Rotor Mechanism for Omnidirectional Quadrotors](https://arxiv.org/abs/2511.15909)
*J. Cristobal,A. Z. Zain Aldeen,M. Izadi,R. Faieghi*

Main category: cs.RO

TL;DR: 本文提出了一种模块化、高效的万向节旋翼机构设计，用于构建全驱动的全向四旋翼无人机，实现了六自由度独立运动。


<details>
  <summary>Details</summary>
Motivation: 传统四旋翼无人机欠驱动，现有全向四旋翼设计通常需要大量结构修改。研究旨在提供一种轻量、易于集成且能实现全驱动的解决方案。

Method: 设计了一种将舵机集成在旋翼平台内的万向节旋翼系统，实现每个旋翼的独立倾斜，无需对中央结构进行重大改动。开发了适用于PX4飞控的新型控制分配方案。

Result: 所提出的万向节旋翼系统实现了全驱动，使无人机能够在所有六个自由度上独立运动。通过成功的飞行测试验证了该方法的有效性。

Conclusion: 所提出的万向节旋翼机构设计为构建全驱动全向四旋翼无人机提供了一种有效、模块化且高效的解决方案，且易于集成。
...
### [193] [I've Changed My Mind: Robots Adapting to Changing Human Goals during Collaboration](https://arxiv.org/abs/2511.15914)
*Debasmita Ghose,Oz Gitelson,Ryan Jin,Grace Abawe,Marynel Vazquez,Brian Scassellati*

Main category: cs.RO

TL;DR: 本文提出了一种机器人方法，通过追踪候选动作序列和验证策略库来检测人类在人机协作中途改变目标。一旦检测到目标变化，机器人会精炼信念并使用RHP主动选择协助和区分性动作以揭示新目标，从而提高协作效率和任务完成速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设目标固定不变，将目标预测视为一次性推理。然而，在现实世界中，人类经常在任务中途改变目标，这使得机器人难以适应，除非有明确的沟通。因此，机器人需要一种能动态适应人类目标变化的方法。

Method: 该方法通过追踪多个候选动作序列并对照策略库验证其合理性来检测目标变化。一旦检测到变化，机器人会精炼对相关过去动作的信念，并构建“后退式地平线规划”（RHP）树，主动选择既能辅助人类又能鼓励“区分性动作”以揭示更新后目标的行动。

Result: 在包含多达30种独特食谱的协作烹饪环境中进行评估，该方法优于三种可比较的人类目标预测算法。它在目标切换后能迅速收敛到正确目标，减少了任务完成时间，并提高了协作效率。

Conclusion: 该方法能够有效检测和适应人类在人机协作中的目标变化，从而显著提升了机器人的适应性、任务完成速度和协作效率。
...
### [194] [Semantic Glitch: Agency and Artistry in an Autonomous Pixel Cloud](https://arxiv.org/abs/2511.16048)
*Qing Zhang,Jing Huang,Mingyang Xu,Jun Rekimoto*

Main category: cs.RO

TL;DR: 本文提出一种“低保真”机器人艺术装置“语义故障”，其物理形态源于数字考古，并利用多模态大型语言模型（MLLM）进行语义导航，而非传统传感器。通过自然语言提示赋予机器人个性，旨在创造以角色而非效率衡量的非完美陪伴者。


<details>
  <summary>Details</summary>
Motivation: 主流机器人追求度量精度和完美性能，本文旨在探索一种刻意“低保真”方法的创造潜力，挑战传统机器人设计的范式，并利用数字考古和语义理解来创造独特的艺术装置。

Method: 研究方法包括：1) 构建“语义故障”机器人，其物理形态是一个3D像素风格的云，作为“物理故障”。2) 开发一种新颖的自主导航管道，完全依赖多模态大型语言模型（MLLM）的定性语义理解进行导航，摒弃了激光雷达和SLAM等传统传感器。3) 通过自然语言提示为机器人编写生物启发式个性，创建“叙事心智”。4) 通过13分钟的自主飞行日志和后续研究来分析和验证框架的鲁棒性。

Result: 主要结果包括：1) 机器人完成了13分钟的自主飞行。2) 后续研究统计验证了该框架在创建可量化区分的机器人角色方面的鲁棒性。3) 综合分析揭示了从基于地标的导航到引人注目的“计划到执行”差距等涌现行为。4) 机器人因缺乏精确的本体感受而表现出不可预测但合理可信的行为，其成功在于其角色而非效率。

Conclusion: 本文展示了一种“低保真”框架，用于创建不完美的陪伴者，其成功衡量标准是角色而非效率。这表明机器人设计可以超越对度量精度和完美性能的追求，转而探索具有独特个性和语义理解的创造性可能性。
...
### [195] [The Role of Consequential and Functional Sound in Human-Robot Interaction: Toward Audio Augmented Reality Interfaces](https://arxiv.org/abs/2511.15956)
*Aliyah Smith,Monroe Kennedy*

Main category: cs.RO

TL;DR: 本研究探讨了机器人操作音和功能音对人类感知与行为的影响，包括空间音效。结果显示，操作音未产生负面影响，空间定位在侧向准确但在正面下降，且空间音效能同时传达信息、提升亲和力并减少不适。


<details>
  <summary>Details</summary>
Motivation: 随着机器人日益融入日常生活环境，理解其与人类的沟通方式至关重要。声音作为一种强大的交互通道，包含操作噪声和有意设计的听觉提示，因此研究其对人类感知和行为的影响具有重要意义。

Method: 本研究考察了机器人（Kinova Gen3机械臂）的附带性（consequential）和功能性（functional）声音对人类感知和行为的影响。特别地，通过定位和交接任务，创新性地探索了空间声音（spatial sound）的效果。

Result: 研究结果表明：1) Kinova Gen3机械臂的附带性声音并未对人类感知产生负面影响；2) 空间定位在侧向提示时准确度高，但在正面提示时准确度下降；3) 空间声音能够同时传达与任务相关的信息，并促进亲和感、减少不适感。

Conclusion: 这些发现突出了功能性和变革性听觉设计在增强人机协作方面的潜力，并为未来的基于声音的交互策略提供了指导和启发。
...
### [196] [PushingBots: Collaborative Pushing via Neural Accelerated Combinatorial Hybrid Optimization](https://arxiv.org/abs/2511.15995)
...
### [197] [Bi-AQUA: Bilateral Control-Based Imitation Learning for Underwater Robot Arms via Lighting-Aware Action Chunking with Transformers](https://arxiv.org/abs/2511.16050)
...
### [198] [MagBotSim: Physics-Based Simulation and Reinforcement Learning Environments for Magnetic Robotics](https://arxiv.org/abs/2511.16158)
*Lara Bergmann,Cedric Grothues,Klaus Neumann*

Main category: cs.RO

TL;DR: 本文提出了MagBotSim，一个基于物理的磁悬浮系统模拟器，旨在将磁悬浮系统视为机器人群（MagBots），以实现运输和操作的融合，为下一代制造系统奠定基础。


<details>
  <summary>Details</summary>
Motivation: 磁悬浮系统在工业自动化中具有革新机器内物料流的潜力，但其除了动态运输外，还拥有尚未开发的操纵潜力。通过将运输和操纵结合成磁机器人群，可以显著提高制造系统的效率、适应性和紧凑性。因此，需要一个工具来支持智能算法的开发。

Method: 引入了MagBotSim（磁性机器人模拟），一个专门为磁悬浮系统设计的基于物理的模拟器。该模拟器将磁悬浮系统视为机器人群（MagBots）。

Result: MagBotSim为磁悬浮系统提供了一个专用的模拟平台，为由磁机器人驱动的下一代制造系统奠定了基础。

Conclusion: 将磁悬浮系统视为机器人群并提供像MagBotSim这样的专用模拟器，能够促进运输与操纵的融合，从而实现更高效率、适应性和紧凑性的下一代制造系统。
...
### [199] [FT-NCFM: An Influence-Aware Data Distillation Framework for Efficient VLA Models](https://arxiv.org/abs/2511.16233)
*Kewei Chen,Yayu Long,Shuai Li,Mingsheng Shang*

Main category: cs.RO

TL;DR: 本文提出FT-NCFM，一个数据中心的生成式数据蒸馏框架，通过评估样本内在价值并合成信息密集的数据资产，显著减少了VLA模型训练所需的数据量和时间，同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作（VLA）模型过度依赖大规模、冗余且价值不均的数据集，限制了其广泛应用。现有的模型中心优化方法（如模型压缩或策略蒸馏）未能从根本上解决数据层面的挑战，且常导致性能下降或缺乏通用性。

Method: 本文引入FT-NCFM框架，一个数据中心的生成式数据蒸馏方法。它包含一个自给自足的“事实追溯”（Fact-Tracing, FT）引擎，该引擎结合因果归因和程序化对比验证来评估样本的内在价值。根据这些评估，一个对抗性NCFM过程合成了一个与模型无关、信息密集且可复用的数据资产。

Result: 在多个主流VLA基准测试中，使用我们蒸馏出的核心数据集（仅占完整数据集的5%）训练的模型，其成功率达到使用完整数据集训练模型的85-90%，同时训练时间减少了80%以上。

Conclusion: 智能数据蒸馏是构建高效、高性能VLA模型的一个极具前景的新途径。
...
### [200] [PIPHEN: Physical Interaction Prediction with Hamiltonian Energy Networks](https://arxiv.org/abs/2511.16200)
*Kewei Chen,Yayu Long,Mingsheng Shang*

Main category: cs.RO

TL;DR: 为解决多机器人系统中的“共享大脑困境”，本文提出了PIPHEN框架，通过边缘语义蒸馏将高维感知数据压缩为紧凑的物理表示，并利用哈密顿能量网络控制器实现精确协作，大幅降低了通信带宽和决策延迟，提高了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统在复杂物理协作中面临“共享大脑困境”，即传输高维多媒体数据（如视频流）会造成严重的带宽瓶颈和决策延迟，阻碍了系统效率和性能。

Method: 本文提出了PIPHEN，一个创新的分布式物理认知-控制框架。其核心思想是通过在机器人边缘执行“语义蒸馏”，将“原始数据通信”替换为“语义通信”，把高维感知数据重构为紧凑、结构化的物理表示。这主要通过两个关键组件实现：(1) 一个新颖的物理交互预测网络（PIPN），源自大模型知识蒸馏，用于生成这种表示；(2) 一个基于能量守恒的哈密顿能量网络（HEN）控制器，将该表示精确转换为协调动作。

Result: 实验结果表明，与基线方法相比，PIPHEN能将信息表示压缩到原始数据量的5%以下，将协作决策延迟从315毫秒降低到76毫秒，同时显著提高了任务成功率。

Conclusion: 这项工作为解决资源受限多机器人系统中的“共享大脑困境”提供了一种根本性的高效范式。
...
### [201] [DynaMimicGen: A Data Generation Framework for Robot Learning of Dynamic Tasks](https://arxiv.org/abs/2511.16223)
...
### [202] [Funabot-Upper: McKibben Actuated Haptic Suit Inducing Kinesthetic Perceptions in Trunk, Shoulder, Elbow, and Wrist](https://arxiv.org/abs/2511.16265)
*Haru Fukatsu,Ryoji Yasuda,Yuki Funabora,Shinji Doki*

Main category: cs.RO

TL;DR: 本文介绍了一种名为Funabot-Upper的穿戴式触觉套装，能让用户感知14种上半身运动，通过独立刺激关节和肌肉，显著提高了运动识别准确率并减少了感知混淆。


<details>
  <summary>Details</summary>
Motivation: 以往的触觉设备在诱导动觉感知方面，要么局限于单一身体部位，要么在应用于多个身体部位时出现感知混淆。研究旨在开发一种能对上半身多个部位独立且有效地诱导动觉感知的设备。

Method: 本文提出了一种新的简化设计策略，并开发了Funabot-Upper触觉套装。该套装利用人工肌肉收缩使服装三维变形，通过独立刺激关节和肌肉，诱导躯干、肩部、肘部和腕部的动觉感知。

Result: 实验证明，Funabot-Upper成功诱导了多个关节的动觉感知，并显著减少了先前设计中观察到的感知混淆。与之前的Funabot-Suit相比，新的套装将识别准确率从68.8%提高到94.6%。

Conclusion: 新的设计策略和Funabot-Upper触觉套装在诱导上半身多关节动觉感知方面表现出优越性，并有效解决了感知混淆问题，展示了其在未来触觉应用中的巨大潜力。
...
### [203] [How Robot Dogs See the Unseeable](https://arxiv.org/abs/2511.16262)
...
### [204] [Safe and Optimal Variable Impedance Control via Certified Reinforcement Learning](https://arxiv.org/abs/2511.16330)
*Shreyas Kumar,Ravi Prakash*

Main category: cs.RO

TL;DR: 本文提出C-GMS框架，通过将策略探索重新定义为从稳定增益调度流形中采样，为结合DMP和VIC的机器人强化学习提供Lyapunov稳定性和执行器可行性保证，从而实现安全可靠的机器人交互。


<details>
  <summary>Details</summary>
Motivation: 现有的结合动态运动基元（DMP）和可变阻抗控制（VIC）的机器人强化学习（RL）方法，由于阻抗增益的时变性，常面临不稳定和不安全探索的风险。

Method: 引入认证高斯流形采样（C-GMS），这是一种以轨迹为中心的RL框架。它将策略探索重新定义为从数学定义的稳定增益调度流形中采样，从而在构建时就保证了Lyapunov稳定性和执行器可行性。该方法确保每次策略执行都是稳定且物理可实现的。

Result: C-GMS确保每次策略执行都具有稳定性、物理可实现性，并消除了对奖励惩罚或事后验证的需求。此外，即使存在有界模型误差和部署时不确定性，该方法也能保证有界的跟踪误差。其有效性已在仿真和真实机器人上得到验证。

Conclusion: C-GMS为结合DMP和VIC的机器人强化学习提供了一个稳定且安全的范式，通过在仿真和真实机器人上的验证，为复杂环境中的可靠自主交互铺平了道路。
...
### [205] [InEKFormer: A Hybrid State Estimator for Humanoid Robots](https://arxiv.org/abs/2511.16306)
*Lasse Hohmeyer,Mihaela Popescu,Ivan Bergonzani,Dennis Mronga,Frank Kirchner*

Main category: cs.RO

TL;DR: 本文提出了一种名为InEKFormer的混合状态估计算法，结合了不变扩展卡尔曼滤波器（InEKF）和Transformer网络，用于人形机器人的稳定步态。研究结果显示了Transformer在人形机器人状态估计中的潜力，但也强调了在高维问题中需要鲁棒的自回归训练。


<details>
  <summary>Details</summary>
Motivation: 人形机器人在不同环境下的双足运动稳定性是其广泛应用（如工业、家庭、医疗、搜救）面临的挑战。状态估计对于为运动控制器提供快速准确的机器人浮动基座状态反馈至关重要。传统的卡尔曼滤波器方法需要专家知识来调整噪声参数，而机器学习领域的最新进展使得深度学习方法在状态估计任务中越来越受欢迎。

Method: 本文提出了一种新颖的混合状态估计算法——InEKFormer，它结合了不变扩展卡尔曼滤波器（InEKF）和Transformer网络。该方法在从人形机器人RH5获取的数据集上与InEKF和KalmanNet方法进行了比较。

Result: 实验结果表明，Transformer在人形机器人状态估计方面具有潜力。

Conclusion: Transformer在人形机器人状态估计中展现出潜力，但研究也强调了在高维问题中进行鲁棒自回归训练的必要性。
...
### [206] [Robot Metacognition: Decision Making with Confidence for Tool Invention](https://arxiv.org/abs/2511.16390)
*Ajith Anil Meera,Poppy Collis,Polina Arbuzova,Abián Torres,Paul F Kinghorn,Ricardo Sanz,Pablo Lanillos*

Main category: cs.RO

TL;DR: 本文提出了一种基于置信度的机器人元认知架构，以实现机器人对其认知过程和决策的反思，从而提高决策的鲁棒性，并在自主工具发明案例中进行了验证。


<details>
  <summary>Details</summary>
Motivation: 当今机器人缺乏真正的智能行为中的关键要素：反思自身认知过程和决策的能力。在人类中，这种自我监控或元认知对于学习、决策和问题解决至关重要，例如评估完成任务的信心以调节行为和分配资源。

Method: 受神经科学启发，本文提出了一种以置信度（对决策的二阶判断）为中心的机器人元认知架构。将置信度作为机器人决策方案中的元认知度量，并在自主工具发明的使用案例中进行了演示。

Result: 通过置信度辅助的机器人能够评估其决策的可靠性，从而在实际物理部署中提高其鲁棒性。这种形式的机器人元认知强调具身动作监控是实现更明智决策的一种手段。

Conclusion: 机器人元认知，特别是利用置信度作为元认知度量并强调具身动作监控，可以使机器人做出更明智、更鲁棒的决策，并具有潜在的应用前景和研究方向。
...
### [207] [Homogeneous Proportional-Integral-Derivative Controller in Mobile Robotic Manipulators](https://arxiv.org/abs/2511.16406)
*Luis Luna,Isaac Chairez,Andrey Polyakov*

Main category: cs.RO

TL;DR: 本文提出了一种针对移动机器人机械臂（MRMs）的同质比例-积分-微分（hPID）控制策略，通过利用同质控制理论，显著提高了系统在存在不确定性和扰动情况下的稳定性、收敛性和轨迹跟踪精度，优于传统PID控制器。


<details>
  <summary>Details</summary>
Motivation: 移动机器人机械臂（MRMs）因其非线性动力学、欠驱动以及底座与机械臂子系统之间的耦合，在控制方面面临巨大挑战。现有控制策略难以有效应对这些复杂特性，尤其是在动态不确定性和外部扰动存在的情况下。

Method: 本文设计了一种新型的同质比例-积分-微分（hPID）控制策略。该方法利用同质控制理论的数学框架，通过分级同质性方法将传统PID增益推广到非线性、状态相关函数，从而系统性地增强闭环系统的稳定性和收敛性。采用Lyapunov方法进行了稳定性分析，以证明hPID控制器在温和假设下能保证全局渐近稳定性和有限时间收敛。

Result: 稳定性分析表明，hPID控制器能保证全局渐近稳定性和有限时间收敛。在代表性MRM模型上的实验结果验证了hPID控制器的有效性，在移动底座和机械臂的轨迹跟踪方面均实现了高精度。与传统线性PID控制器相比，hPID在响应时间、稳态误差和对模型不确定性的鲁棒性方面表现更优。

Conclusion: 本研究为增强下一代移动操作系统在结构化和非结构化环境中的自主性和可靠性，提供了一个可扩展且具有分析基础的控制框架。hPID控制器为解决MRMs的复杂控制挑战提供了一种有效且鲁棒的解决方案。
...
### [208] [Flow-Aided Flight Through Dynamic Clutters From Point To Motion](https://arxiv.org/abs/2511.16372)
*Bowen Xu,Zexuan Yan,Minghao Lu,Xiyu Fan,Yi Luo,Youshen Lin,Zhiqiang Chen,Yeke Chen,Qiyuan Qiao,Peng Lu*

Main category: cs.RO

TL;DR: 本文提出了一种基于单LiDAR传感的强化学习方法，无需显式目标检测、跟踪和预测，即可实现四旋翼飞行器在动态杂波环境中的自主飞行，表现出卓越的成功率和适应性。


<details>
  <summary>Details</summary>
Motivation: 在动态杂波中导航的挑战在于高效感知环境动态并生成考虑障碍物运动的规避行为。现有解决方案通过显式建模动态障碍物运动进行避障，但在高动态和遮挡场景中，这种对决策的关键依赖既耗时又不可靠。

Method: 该研究利用强化学习（RL）和单LiDAR传感实现从点云到运动的直接自主飞行系统。外感知方面，将原始点云编码为固定形状、低分辨率、细节安全的深度感知距离图，并采用从多帧观测中提取的环境变化感知点流作为运动特征。这两者被整合为一种轻量级且易于学习的复杂动态环境表示。动作生成方面，通过所提出的变化感知传感表示隐式驱动提前规避动态威胁的行为，策略优化由相对运动调制的距离场指示。系统采用部署友好的传感模拟和无动力学模型加速度控制。

Result: 所提出的系统在成功率和适应性方面优于替代方案。从模拟器中学习到的策略能够驱动真实世界的四旋翼飞行器进行安全机动。

Conclusion: 通过结合单LiDAR传感和新颖的感知表示，该强化学习系统有效解决了动态杂波环境中的挑战，实现了高效、可靠的自主飞行，并在真实世界中展现了良好的部署能力和安全性能。
...
### [209] [LAOF: Robust Latent Action Learning with Optical Flow Constraints](https://arxiv.org/abs/2511.16407)
*Xizhou Bu,Jiexi Lyu,Fulei Sun,Ruichen Yang,Zhiqiang Ma,Wei Li*

Main category: cs.RO

TL;DR: LAOF是一种伪监督框架，利用光流作为动作驱动信号，从大规模视频中学习对干扰物鲁棒的潜在动作表示，在下游任务中表现优于现有方法，尤其在标签稀缺条件下。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从大规模视频中学习潜在动作时，常受动作无关干扰物影响。虽然动作监督能缓解此问题，但其有效性受限于动作标签的稀缺性。光流能自然地抑制背景并强调运动物体，可作为有效的动作驱动信号。

Method: 提出LAOF（Latent Action learning with Optical Flow constraints），一个伪监督框架。该方法利用智能体的光流作为动作驱动信号，学习对干扰物具有鲁棒性的潜在动作表示。

Result: LAOF学习到的潜在表示在下游模仿学习和强化学习任务中优于现有方法。光流约束显著稳定了训练并提高了潜在表示的质量，尤其在极度标签稀缺的条件下（标签比例增加到10%时仍有效）。即使没有动作监督，LAOF也能达到或超越使用1%动作标签训练的动作监督方法。

Conclusion: 光流是一种强大的伪监督信号，能够学习鲁棒的潜在动作，显著改善了标签稀缺条件下的性能，甚至在无动作监督的情况下，也能超越部分有动作监督的方法。
...
### [210] [InternData-A1: Pioneering High-Fidelity Synthetic Data for Pre-training Generalist Policy](https://arxiv.org/abs/2511.16651)
...
### [211] [From Prompts to Printable Models: Support-Effective 3D Generation via Offset Direct Preference Optimization](https://arxiv.org/abs/2511.16434)
...
### [212] [Dexterity from Smart Lenses: Multi-Fingered Robot Manipulation with In-the-Wild Human Demonstrations](https://arxiv.org/abs/2511.16661)
...
TL;DR: 该研究提出AINA框架，利用Aria Gen 2眼镜从人类在自然环境中执行日常任务的视频中学习多指机器人策略，无需机器人数据即可直接部署。


<details>
  <summary>Details</summary>
Motivation: 机器人界长期目标是从人类视频中学习多指机器人策略，以实现通用机器人操作并减少对劳动密集型机器人数据收集的依赖。现有进展受限于人机具身差距以及从野外人类视频中提取相关上下文和运动线索的困难。

Method: 通过使用Aria Gen 2眼镜收集数据，该眼镜提供高分辨率RGB摄像头、精确的3D头部和手部姿态以及宽广的立体视野用于深度估计。在此基础上，提出AINA框架来学习3D点基的多指手部策略，这些策略对背景变化具有鲁棒性，并且无需任何机器人数据（包括在线校正、强化学习或仿真）即可直接部署。

Result: AINA框架能够学习对背景变化鲁棒的多指策略，并可直接部署，无需任何机器人数据。研究将该框架与现有的人机策略学习方法进行了比较，并对其设计选择进行了消融研究，在九个日常操作任务中展示了结果。

Conclusion: 通过简单但功能强大的硬件（Aria Gen 2眼镜）和提出的AINA框架，该研究在实现从人类数据中学习多指机器人策略的长期目标上迈出了重要一步，显著减少了对机器人数据收集的依赖。
...
### [213] [MiMo-Embodied: X-Embodied Foundation Model Technical Report](https://arxiv.org/abs/2511.16518)
...
TL;DR: MiMo-Embodied是首个跨具身基础模型，在自动驾驶和具身AI两大领域均实现最先进性能，并在多项基准测试中创下新纪录。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索自动驾驶和具身AI这两个领域之间是否存在正向迁移和相互强化，并构建一个能同时在这两个领域取得优异表现的通用模型。

Method: 该研究采用了多阶段学习、精心策划的数据构建以及CoT/RL（思维链/强化学习）微调等方法来训练MiMo-Embodied模型。

Result: MiMo-Embodied在17个具身AI基准测试（包括任务规划、可供性预测和空间理解）和12个自动驾驶基准测试（包括环境感知、状态预测和驾驶规划）中均取得了新纪录，显著优于现有的开源、闭源和专用基线模型。

Conclusion: 研究结果表明，通过多阶段学习、精心策划的数据构建和CoT/RL微调，自动驾驶和具身AI这两个领域展现出强大的正向迁移和相互强化作用。
...
### [214] [Development of a velocity form for a class of RNNs, with application to offset-free nonlinear MPC design](https://arxiv.org/abs/2511.15889)
*Daniele Ravasio,Bestem Abdulaziz,Marcello Farina,Andrea Ballarino*

Main category: eess.SY

TL;DR: 本文提出了一种针对一类循环神经网络（RNN）描述的非线性系统，在存在扰动和模型-实际不匹配情况下，实现无偏差跟踪的新方法。该方法通过将RNN模型重构为速度形式，并结合基于线性矩阵不等式（LMI）的非线性状态观测器和控制器设计，以及一种处理输入输出约束的无偏差非线性模型预测控制（NMPC）算法，实现了闭环系统的稳定性和有效跟踪。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了解决非线性系统（特别是RNNs描述的系统）在存在恒定扰动和模型-实际不匹配时，难以实现无偏差跟踪的问题，同时还需要处理输入和输出约束。

Method: 1. 将RNN模型重构为速度形式，以补偿恒定扰动并保证无偏差跟踪。2. 基于线性矩阵不等式（LMI）设计非线性状态观测器和非线性状态反馈控制器，以确保速度形式动力学原点的全局或区域闭环稳定性。3. 开发了一种无偏差非线性模型预测控制（NMPC）算法，该算法利用速度形式模型作为预测模型，并结合静态控制器作为辅助律来定义终端条件，以处理输入和输出约束。

Result: 所提出的方法能够确保速度形式动力学原点的全局或区域闭环稳定性，有效补偿恒定扰动，并在存在模型-实际不匹配时实现无偏差跟踪。此外，该算法能有效处理输入和输出约束。通过在pH中和过程基准上的仿真验证，证明了所提方法的有效性。

Conclusion: 本文提出的基于速度形式RNN模型、LMI设计的观测器/控制器以及无偏差NMPC算法，为具有扰动和约束的非线性RNN系统提供了一个有效且理论上可靠的无偏差跟踪解决方案。
...
### [215] [Cyber-Resilient Data-Driven Event-Triggered Secure Control for Autonomous Vehicles Under False Data Injection Attacks](https://arxiv.org/abs/2511.15925)
*Yashar Mousavi,Mahsa Tavasoli,Ibrahim Beklan Kucukdemiral,Umit Cali,Abdolhossein Sarrafzadeh,Ali Karimoddini,Afef Fekih*

Main category: eess.SY

TL;DR: 本文提出了一种针对自动驾驶汽车（AVs）执行器虚假数据注入（FDI）攻击的网络弹性安全控制框架，该框架结合了数据驱动建模、事件触发通信和分数阶滑模控制，以提高系统对恶意干预的抵抗力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶汽车容易受到执行器虚假数据注入（FDI）攻击的威胁，需要一个能够增强系统韧性、减轻攻击影响并优化通信效率的安全控制框架。

Method: 该框架采用动态模型分解（DMD）方法从真实数据中提取横向动力学模型；设计了事件触发传输方案以优化通信效率；开发了扩展状态观测器（ESO）实时估计并减轻执行器攻击影响；并利用Lyapunov方法和线性矩阵不等式（LMI）进行了理论稳定性分析，以保证指数误差收敛。

Result: 仿真结果表明，所提出的事件触发安全控制框架在攻击缓解、通信效率和横向跟踪性能方面均有显著提升。该框架能有效对抗执行器攻击，同时优化通信资源利用率。

Conclusion: 该框架能够有效对抗执行器攻击，优化通信资源利用，因此非常适用于安全关键型自动驾驶汽车应用。
...
### [216] [What Does It Take to Get Guarantees? Systematizing Assumptions in Cyber-Physical Systems](https://arxiv.org/abs/2511.15952)
*Chengyu Li,Saleh Faghfoorian,Ivan Ruchkin*

Main category: eess.SY

TL;DR: 本文对网络物理系统（CPS）文献中的假设和保证进行了系统性调查，揭示了当前假设的趋势和不足，并呼吁改进假设的报告和测试。


<details>
  <summary>Details</summary>
Motivation: CPS的形式化保证依赖于各种假设，但目前对这些假设、它们支持的保证以及如何精确指定它们缺乏系统的认识。

Method: 作者调查了2014-2024年间104篇关于CPS控制、验证和运行时保证的论文，使用扎根理论编码提取了423个假设和321个保证，并用21个标签标注了假设所需的规范语言特征。

Result: 分析揭示了CPS假设中普遍存在的趋势和不足，特别是在初始化、传感、感知、神经网络组件和不确定性方面。

Conclusion: 研究结果促使作者呼吁改进CPS假设的报告和测试方法。
...
### [217] [Physics Informed Multi-task Joint Generative Learning for Arterial Vehicle Trajectory Reconstruction Considering Lane Changing Behavior](https://arxiv.org/abs/2511.16019)
*Mengyun Xu,Jie Fang,Eui-Jin Kim,Tony Z. Qiu,Prateek Bansal*

Main category: eess.SY

TL;DR: 本研究提出一个生成式框架，通过物理信息多任务联合学习，联合建模换道和跟车行为，从有限的网联车辆轨迹数据重建多车道主干道交叉口完整的交通流时空图。


<details>
  <summary>Details</summary>
Motivation: 获取完整的车辆轨迹成本高昂，且在多车道环境中准确推断换道和跟车行为极具挑战性。因此，需要一种能够从有限数据重建完整交通流时空图的方法。

Method: 提出一个生成式框架，通过物理信息多任务联合学习（Physics-informed Multi-task Joint Learning）共同建模换道和跟车行为。该框架包含：
1. 换道生成对抗网络（LC-GAN）：基于历史轨迹，结合信号控制、几何配置和周围车辆交互等物理条件，建模随机换道行为。
2. 轨迹生成对抗网络（Trajectory-GAN）：整合LC-GAN的换道信息和物理跟车模型生成的初始轨迹，以数据驱动方式进行细化，适应动态交通状况。
该框架旨在从少量网联车辆轨迹（例如，每车道仅一条观测轨迹）重建完整轨迹。通过多任务联合学习，LC-GAN和Trajectory-GAN之间实现协同交互，相互作为辅助监督和物理条件。

Result: 通过两个真实世界轨迹数据集的验证，该框架在重建多车道主干道交叉口完整的时空图方面，优于传统的基准模型。

Conclusion: 本研究推动了基于网联车辆轨迹感知与物理信息深度学习的结合，为从有限数据重建完整轨迹提供了一种鲁棒的方法。
...
### [218] [Bellman Memory Units: A neuromorphic framework for synaptic reinforcement learning with an evolving network topology](https://arxiv.org/abs/2511.16066)
*Shreyan Banerjee,Aasifa Rounak,Vikram Pakrashi*

Main category: eess.SY

TL;DR: 该论文提出了一种突触Q学习算法和神经形态贝尔曼记忆单元（BMU），用于解决神经形态边缘设备在控制应用中面临的在线学习和可扩展性限制。通过在Loihi芯片上实现，该方法支持网络拓扑演化、优化资源利用并实现片上学习，从而适应未知的控制场景。


<details>
  <summary>Details</summary>
Motivation: 神经形态边缘设备在控制应用中受到梯度无关在线学习和硬件在不同控制问题上可扩展性的限制。

Method: 引入了一种突触Q学习算法，将贝尔曼方程融入突触层面，实现训练过程中网络拓扑的迭代演化。随后，采用类似方法构建了神经形态贝尔曼记忆单元（BMU），并利用神经工程框架在英特尔Loihi神经形态芯片上实现。结合拓扑演化和混合信号计算，优化神经元和突触的数量，并利用片上学习。

Result: 该方法实现了网络拓扑的迭代演化，通过结合拓扑演化和混合信号计算，优化了神经元和突触的数量，有助于设计基于脉冲的强化学习加速器。所提出的架构能减少板载资源利用，促进紧凑型专用神经形态IC的制造。此外，片上学习能够适应未知的控制场景。

Conclusion: 所提出的突触Q学习算法和神经形态贝尔曼记忆单元（BMU）通过实现网络拓扑演化和片上学习，解决了神经形态边缘设备在控制应用中的限制，有望降低资源消耗，促进紧凑型神经形态IC的开发，并增强对未知控制场景的适应能力。
...
### [219] [Describing Functions and Phase Response Curves of Excitable Systems](https://arxiv.org/abs/2511.16235)
*Robin Wroblowski,Rodolphe Sepulchre*

Main category: eess.SY

TL;DR: 本文提出了一种针对兴奋性系统的新型分析方法，通过离散事件算子映射输入输出事件序列，克服了传统描述函数和相位响应曲线的局限性，并以霍奇金-赫胥黎模型为例进行说明，对神经形态控制和神经生理学具有重要意义。


<details>
  <summary>Details</summary>
Motivation: 描述函数（DF）和相位响应曲线（PRC）等经典工具在弛豫振荡器和兴奋性系统网络中存在局限性，因此需要一种专门针对兴奋性系统的新方法。

Method: 提出了一种针对兴奋性系统的新颖方法，侧重于将输入事件序列映射到输出事件序列的离散事件算子。该方法以霍奇金-赫胥黎（Hodgkin-Huxley）兴奋性模型为例进行说明。

Result: 所提出的框架为设计和分析兴奋性神经元网络中的中枢模式发生器提供了基础。

Conclusion: 该框架与神经形态控制和神经生理学直接相关，具有重要的实际应用价值。
...
### [220] [Parallelizable Complex Neural Dynamics Models for PMSM Temperature Estimation with Hardware Acceleration](https://arxiv.org/abs/2511.16093)
...
### [221] [Robust Self-Triggered Control Approaches Optimizing Sensors Utilization with Asynchronous Measurements](https://arxiv.org/abs/2511.16253)
*Abbas Tariverdi*

Main category: eess.SY

TL;DR: 本文提出了一种针对线性系统的自触发控制方法，该方法在传感器异步更新时，通过优化传感器读取策略来最大化采样间隔，同时保持系统稳定性，并显著降低传感器利用率。


<details>
  <summary>Details</summary>
Motivation: 大多数控制系统运行在具有有限通信资源的数字硬件上，因此需要开发资源高效的控制方法来应对网络化系统中的通信限制。

Method: 该研究为线性系统开发了自触发控制，其中传感器独立更新（异步测量）。控制器在每个采样时刻计算一个最优时间范围，选择在接下来的几个时间步中读取哪个传感器，以在保持稳定性的同时最大化采样间隔。为了解决计算复杂性，提出了两种实现方式：在线版本（每次更新时解决优化问题）和离线版本（使用锥形分区预计算最优时间范围，将在线计算简化为查表）。

Result: 两种实现方式都能确保无扰动系统的指数稳定性，并确保有界扰动系统的全局一致最终有界性。仿真结果表明，与周期性采样相比，传感器利用率降低了59-74%。

Conclusion: 所提出的框架能够在具有通信约束的网络化系统中实现资源高效的控制。
...
### [222] [Spatially Dependent Sampling of Component Failures for Power System Preventive Control Against Hurricane](https://arxiv.org/abs/2511.16279)
*Ziyue Li,Guanglun Zhang,Grant Ruan,Haiwang Zhong,Chongqing Kang*

Main category: eess.SY

TL;DR: 本文提出一种考虑气象因素空间相关性的电力系统组件故障抽样方法，能更准确地生成极端天气下的高严重性场景，从而提高预防性控制策略的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究在电力系统预防性控制的场景生成中，常采用蒙特卡洛模拟并假设组件故障独立采样，忽略了飓风等气象因素引起的空间相关性，导致场景不真实。

Method: 本文识别并解决了极端天气下组件故障空间依赖性建模的不足。通过分析气象强度随机变量的均值、方差和相关结构如何影响组件故障相关性，提出了一种空间依赖性抽样方法，通过生成相关的气象强度随机变量实现多个组件故障的联合抽样。通过比较研究评估了该方法并分析了场景选择对预防性控制性能的影响。

Result: 1) 不确定天气强度的强空间相关性始终导致组件故障相互依赖，与均值水平无关；2) 所提出的方法揭示了独立抽样遗漏的更多高严重性场景；3) 预防性控制需要在不同场景严重性下平衡限负荷和过发电成本；4) 忽略故障相关性会导致低估高严重性事件的风险，从而损害预防性控制策略的鲁棒性。

Conclusion: 所提出的空间依赖性抽样方法能更真实地捕获极端事件，揭示传统独立抽样方法遗漏的高严重性场景，从而有效评估风险并增强预防性控制策略的鲁棒性。
...
### [223] [A Comprehensive Study on Cyber Attack Vectors in EV Traction Power Electronics](https://arxiv.org/abs/2511.16399)
*Siddhesh Pimpale*

Main category: eess.SY

TL;DR: 本研究探讨了电动汽车牵引动力电子设备在数字化转型中面临的网络安全风险，识别了潜在攻击向量，并通过模拟展示了攻击对系统稳定性和功能造成的严重影响，强调了嵌入式入侵预防和安全固件设计的重要性。


<details>
  <summary>Details</summary>
Motivation: 电动汽车的普及使得电力电子在牵引管理、能量转换和车辆控制中扮演核心角色。然而，这种数字化转型显著增加了网络攻击面，使得电动汽车的牵引动力电子设备容易受到各种网络安全风险的影响。

Method: 研究利用STRIDE威胁建模框架，概述并归类了架构的漏洞。通过运行拒绝服务（DoS）、欺骗、固件篡改和数据注入等攻击模拟，来验证攻击的影响。

Result: 实验证明，控制信号或传感数据的轻微中断可能导致严重的运行后果，例如不稳定的扭矩传感器值、异常的电压偏移以及整个系统冻结。

Conclusion: 研究结果强调了在电动汽车动力总成电子设备中注入嵌入式入侵预防机制和安全固件设计的高度优先性。本文为网络安全实践与汽车电力电子特殊需求之间的联系贡献了知识。
...
### [224] [Second-Order MPC-Based Distributed Q-Learning](https://arxiv.org/abs/2511.16424)
*Samuel Mallick,Filippo Airaldi,Azita Dabiri,Bart De Schutter*

Main category: eess.SY

TL;DR: 本文提出了一种基于MPC的分布式Q学习的二阶扩展方法，该方法仅依赖局部信息和邻居通信，并在仿真中显著优于一阶方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于MPC的分布式Q学习方法仅限于一阶梯度更新，收敛速度慢。使用二阶信息可以显著提高学习收敛速度，并允许使用更高的学习率而不引入不稳定。

Method: 本文提出了一种基于MPC的Q学习的二阶扩展，其更新分布在局部智能体之间，仅依赖于局部可用信息和邻居之间的通信。

Result: 在仿真中，该方法被证明显著优于一阶分布式Q学习。

Conclusion: 所提出的二阶分布式Q学习方法能显著提升基于MPC的Q学习的性能和收敛速度，超越传统的一阶方法。
...
### [225] [Tube-Based Model Predictive Control with Random Fourier Features for Nonlinear Systems](https://arxiv.org/abs/2511.16425)
*Ákos M. Bokor,Tamás Dózsa,Felix Biertümpfel,Ádám Szabó*

Main category: eess.SY

TL;DR: 本文提出了一种结合随机傅里叶特征（RFF）和管式模型预测控制（MPC）的计算高效方法，用于非线性系统的鲁棒控制。


<details>
  <summary>Details</summary>
Motivation: 管式MPC能提供鲁棒的约束满足，但在处理模型不确定性时可能过于保守。随机傅里叶特征方法能有效近似非线性动力学并减少近似误差。研究旨在通过结合RFF来减少模型不确定性，从而降低管式MPC的保守性并提高其计算效率。

Method: 该方法将基于随机傅里叶特征（RFF）的残差学习与管式模型预测控制（tube-based MPC）相结合。RFF通过求解数值上易于处理的最小二乘问题来近似非线性系统动力学，并学习残差。然后将这种RFF-based残差学习集成到管式MPC框架中，并应用于自动驾驶车辆路径跟踪问题。

Result: 与线性基线方法相比，所提出的方法将管尺寸减小了约50%，导致更不保守的控制行为，并在测试场景中将误差减小了约70%。此外，该方法实现了实时性能，同时保持了可证明的鲁棒性保证。

Conclusion: 所提出的RFF与管式MPC结合的方法显著提升了非线性系统鲁棒MPC的性能（更小的误差、更不保守的行为）和计算效率（实时性能），同时保留了鲁棒性保证，为自动驾驶等实际应用提供了有效且高效的解决方案。
...
### [226] [Energy-Efficient and Actuator-Friendly Control Under Wave Disturbances: Model Reference vs. PID for Thruster Surge](https://arxiv.org/abs/2511.16413)
...
### [227] [Observer Design for Singularly Perturbed Linear Networked Control Systems Subject to Measurement Noise](https://arxiv.org/abs/2511.16469)
...
### [228] [Weakly Supervised Segmentation and Classification of Alpha-Synuclein Aggregates in Brightfield Midbrain Images](https://arxiv.org/abs/2511.16268)
*Erwan Dereure,Robin Louiset,Laura Parkkinen,David A Menassa,David Holcman*

Main category: eess.IV

TL;DR: 本研究开发了一种基于弱监督分割和ResNet50分类器的自动化深度学习流程，用于在帕金森病全玻片图像中分割和分类α-突触核蛋白聚集体，平衡准确率达80%。


<details>
  <summary>Details</summary>
Motivation: 帕金森病是一种与错误折叠的α-突触核蛋白聚集体（如路易小体和神经突）相关的神经退行性疾病。自动分析免疫组织化学病理图像有助于更好地理解这些聚集体的空间组织，从而改进病理诊断。

Method: 开发了一个自动化图像处理流程，用于分割和分类帕金森病和偶发性路易小体病（iLBD）病例中脑组织全玻片图像（WSIs）中的α-突触核蛋白聚集体。该方法采用弱监督分割，对免疫组织化学标记变异性具有鲁棒性，并结合ResNet50分类器进行分类。

Result: 该方法能够区分主要的聚集体形态，包括路易小体和神经突，并达到了80%的平衡准确率。

Conclusion: 该框架为大规模表征亮视野免疫组织化学组织中α-突触核蛋白聚集体的空间分布和异质性铺平了道路，并有助于研究它们与周围细胞（如小胶质细胞和星形胶质细胞）之间尚未明确的关系。
...
### [229] [UniUltra: Interactive Parameter-Efficient SAM2 for Universal Ultrasound Segmentation](https://arxiv.org/abs/2511.15771)
*Yue Li,Qing Xu,Yixuan Zhang,Xiangjian He,Qian Zhang,Yuan Yao,Fiseha B. Tesem,Xin Chen,Ruili Wang,Zhen Chen,Chang Wen Chen*

Main category: eess.IV

TL;DR: 针对SAM2在超声图像上性能下降和部署受限的问题，UniUltra提出一种高效的适配框架。它通过上下文-边缘混合适配器(CH-Adapter)进行参数高效微调，并利用深度监督知识蒸馏(DSKD)将知识转移到轻量级模型，实现了高性能、参数高效的通用超声分割，适用于资源受限的临床环境。


<details>
  <summary>Details</summary>
Motivation: Segment Anything Model 2 (SAM2) 在自然图像上表现出色，但在超声图像上因领域差异性能显著下降。这带来了两个核心挑战：如何高效地将SAM2适配到超声成像领域并保持参数效率，以及如何在资源受限的临床环境中有效部署适配后的模型。

Method: UniUltra提出：1) 一种新颖的上下文-边缘混合适配器 (CH-Adapter)，用于增强对不同超声图像模态的细粒度感知，并实现参数高效微调。2) 一种深度监督知识蒸馏 (DSKD) 技术，将微调后的SAM2大型图像编码器的知识转移到一个超轻量级编码器，以大幅减少计算要求而不牺牲性能。

Result: UniUltra在通用超声分割方面超越了现有最先进的方法，并展现出卓越的泛化能力。在微调阶段，该框架仅使用了SAM2 8.91%的参数。最终压缩模型与原始SAM2相比，参数量减少了94.08%，同时保持了有竞争力的性能。

Conclusion: UniUltra通过创新的CH-Adapter和DSKD技术，成功解决了SAM2在超声图像上的性能退化和临床部署挑战。它提供了一个高性能、参数高效且适用于资源受限环境的通用超声分割框架，非常适合实际临床部署。
