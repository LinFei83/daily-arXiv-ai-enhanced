...
TL;DR: 提出了GraphBit，一个引擎编排的框架，通过有向无环图定义工作流，解决提示编排中的幻觉、无限循环和不可复现问题，在GAIA基准上取得67.6%准确率和零框架诱导幻觉。


<details>
  <summary>Details</summary>
Motivation: 提示编排的LLM框架存在幻觉路由、无限循环和不可复现执行的问题。

Method: 使用Rust引擎驱动DAG工作流，代理作为类型化函数，支持并行分支、条件控制和三层记忆架构。

Result: 在GAIA基准上准确率67.6%，零框架诱导幻觉，延迟11.9毫秒，吞吐量最高。

Conclusion: 确定性执行和分层记忆有效提升工具密集型任务性能，各记忆层均有贡献。
...
TL;DR: 提出混合整数目标规划（MIGP）解决传统饮食优化中的连续变量分数份数和硬约束不可行问题，在810个实例中MIGP在66%情况下优于后取整方法，且100%可行。


<details>
  <summary>Details</summary>
Motivation: 传统饮食优化模型存在连续变量产生分数份数（如1.7个鸡蛋）和硬约束冲突导致无解的双重局限，而现有56篇论文均未同时解决这两个问题。

Method: 使用整数变量表示自然份数（如一个鸡蛋），目标规划偏差变量实现软营养目标，并通过逆目标归一化平衡多目标优化。

Result: 在810个实例中，MIGP在66%情况下优于后取整方法，硬约束整数规划仅48%可行；求解时间小于100毫秒。

Conclusion: MIGP能同时实现实际份数约束和软营养目标，性能优于现有方法，适合实际膳食规划应用。
...
TL;DR: 提出结合认知功能七类和执行拓扑六类的二维分类框架，定义27种命名模式，并通过跨域分析总结五项模式选择定律。


<details>
  <summary>Details</summary>
Motivation: 现有框架从单一视角描述LLM代理架构，无法区分拓扑相同但功能不同的系统（如规划执行与层级委托），导致设计权衡和故障模式混淆。

Method: 采用二维分类：认知功能轴（7类）与执行拓扑轴（6类），构建7x6矩阵；系统交叉分析验证正交性，详细定义8种代表模式，并在4个真实域验证覆盖性。

Result: 识别27种命名模式（含13种原创名），跨域分析导出五项经验定律，涵盖时间压力、行动权限、失败成本不对称和任务量对架构选择的影响。

Conclusion: 该框架提供一种原则性、框架中立且模型无关的词汇表，用于AI代理架构设计，澄清了架构差异与适应性规律。
...
TL;DR: 研究发现，不可见的编排者会显著提升集体解离，且行为评估无法检测内部状态风险。


<details>
  <summary>Details</summary>
Motivation: 探索多智能体系统中编排器不可见性对安全性的影响，此前从未被实证检验。

Method: 预注册的3×2实验，共365次运行，每次5个智能体，使用Claude Sonnet 4.5。

Result: 不可见编排导致集体解离升高；编排者自身解离最大，且工作智能体受污染；行为输出无差异。

Conclusion: 编排器可见性和模型选择直接影响多智能体系统安全性，行为评估不足以检测内部状态风险。
...
TL;DR: 提出了Preping框架，通过提议者引导的合成任务实践和选择性记忆更新，让智能体在无先验经验时构建程序性记忆，降低冷启动成本。


<details>
  <summary>Details</summary>
Motivation: 智能体在新环境中缺少任务特定经验时存在冷启动问题，而现有方法依赖离线或在线经验构建记忆成本高。

Method: Preping框架包含提议者记忆（控制后续实践）、提议者（生成合成任务）、求解者（执行任务）和验证者（筛选轨迹并反馈），通过提议者控制任务可行性、冗余和覆盖率，结合选择性记忆更新。

Result: 在AppWorld、BFCL v3和MCP-Universe上，Preping显著优于无记忆基线，与基于最佳策略的记忆方法性能相当，部署成本分别降低2.99倍和2.23倍。

Conclusion: 主要收益来自提议者侧对任务可行性、冗余性和覆盖率的控制，以及选择性记忆更新，而非单纯的合成数据量。
...
TL;DR: 提出了一个多语言基准测试PolitNuggets，用于评估大型推理模型在代理框架中合成分散来源的长尾事实的能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估未能充分测试LRM从分散来源发现和合成'长尾'事实的能力。

Method: 构建包含400位全球精英政治传记的基准，采用多代理系统和FactNet协议评分发现、准确性和效率。

Result: 当前系统在细粒度细节上表现不佳，效率差异显著。

Conclusion: 代理性能与短上下文提取、多语言鲁棒性和可靠工具使用等基础能力密切相关。
...
TL;DR: 提出条件属性变换器，在单个前向传递中同时估计下一个token概率和条件属性值，实现逐token信用分配、反事实分析和可控生成。


<details>
  <summary>Details</summary>
Motivation: 生成模型用下一token预测训练，但下游应用需序列级属性估计或控制，而该方法会过拟合局部模式、欠拟合全局结构，且需要昂贵采样或修改。

Method: 在预测下一个token概率的同时，估计每个候选token对应的属性值，形成联合框架。

Result: 在稀疏奖励任务上达到最优，足够大模型时提升下一token预测，属性概率估计速度比采样快数个数量级，能引导解码。

Conclusion: 该方法在不修改输入序列的情况下，高效实现序列属性估计、反事实分析和可控生成。
...
TL;DR: 本文提出有限层论框架，通过传输与阻碍检测AI智能体中的理论变迁候选，并在过渡卡片基准上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体仅能拟合数据，无法判断表征框架是否需要在新的情境下扩展。

Method: 构建局部到全局的上下文结构（源、重叠、目标、验证图表），通过拟合残差、重叠不兼容、约束违反、极限关系失败和表征成本等阻碍度量来评估连贯性。

Result: 直接阻碍排序显示，预期的变形或扩展通常是阻碍最低的候选，且过渡类型在基准中得以区分。

Conclusion: 该框架为AI智能体提供了一个有限的诊断子问题：检测表征传输失败并识别扩展的必要性，而非解决范式重构或自主理论发明。
...
TL;DR: 提出基于GraphRAG的价值框架，通过分层原则指令提升LLM代理的自我认知、困境决策和情感符合人类价值观，在DAILYDILEMMAS基准上优于ECoT等方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在自我认知、困境决策和情感表现方面与人类社会价值观存在差距。

Method: 基于GraphRAG将原则转化为价值指令，根据对话上下文检索指令指导代理行为。

Result: 在DAILYDILEMMAS基准上，该方法在预期行为比率上显著优于ECoT、Plan-and-Solve和元认知提示等基线。

Conclusion: 该方法为AI系统自我情感的出现提供了基础。
...
TL;DR: 提出一种高效的原则性推理方法，通过一元关系集成码预处理数据，使大语言模型在保持软硬件基础的同时，实现可信任的内容生成和多项式时间可学习的推理。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型虽能生成流畅文本，但缺乏可靠原则支撑文本内容的可信度，且传统观点认为增加原则性推理在计算上不可行。

Method: 两阶段方法：第一阶段将数据重编码为显式表达对象关系的Unary Relational Integracode；第二阶段用可能精简的标准机器学习预测这些关系，结合鲁棒逻辑进行原则性推理。

Result: 重编码使学习核心关系规则在多项式时间内可完成（复杂度取决于规则本身），并支持单次及多次分类器调用的可靠推理。

Conclusion: 该方法通过显式关系编码实现了高效原则推理，可扩展至视觉、动作等领域，为LLM提供可信内容生成的基础。
...
TL;DR: 本文提出模型自适应的工具必要性定义，发现大模型在实际使用中存在26.5-54.0%的工具调用失调，并验证了认知到行动之间存在“知行差距”。


<details>
  <summary>Details</summary>
Motivation: 现有研究将工具必要性视为模型无关属性，但实际中模型能力边界不同，需要模型自适应的工具必要性定义。

Method: 通过比较模型自适应的工具必要性与实际工具调用行为，分解为内部认知和行动执行两阶段，并利用探针分析隐藏状态。

Result: 模型在算术和事实问答数据集上分别存在26.5-54.0%和30.8-41.8%的工具调用失调，且失调主要源于“认知到行动”的转换阶段。

Conclusion: 改善LLM工具使用可靠性不仅需要更好的工具必要性识别，更需要将认知有效转化为行动。
...
TL;DR: 提出了NERVE框架，通过双线性标记化将功能连接矩阵划分为网络间/内块，实现网络感知的自监督表征学习。


<details>
  <summary>Details</summary>
Motivation: 现有MAE方法将FC矩阵视为同质结构元素，忽略大脑大规模网络的模块化组织。

Method: 将FC矩阵分割为网络对定义的异质尺寸补丁，通过结构化双线性分解嵌入，保持网络身份并降低参数量。

Result: 在三个大规模发育队列中，NERVE在行为和精神病理预测上优于结构无关的MAE变体和图自监督方法，跨队列评估更稳定。

Conclusion: 将领域特定的结构先验（网络组织）纳入自监督学习对功能连接组学至关重要。
...
TL;DR: 提出神经符号方法结合大语言模型与形式化验证，解决法律AI中推理不严谨与信任问题


<details>
  <summary>Details</summary>
Motivation: 法律领域对AI的严谨性要求极高，现有LLM存在事实幻觉及超出文本依据的假设性结论，缺乏逻辑支撑

Method: 神经符号方法：融合大语言模型的表达能力与形式化验证的严格性

Result: 实现既能处理法律文本又能通过形式验证确保推理可靠性的AI系统

Conclusion: 该方法可在减轻人工验证负担的同时，保持法律实践所需的责任追究能力
...
TL;DR: 提出SPIN包装器，通过验证DAG计划和前缀执行控制，减少无效计划和工具调用，提升效率。


<details>
  <summary>Details</summary>
Motivation: 工业LLM代理系统常分离规划与执行，导致结构无效或过长的工作流，增加成本。

Method: 结合验证的有向无环图（DAG）规划与基于前缀的执行控制，强制DAG约束，逐步评估前缀，在足够回答查询时停止。

Result: 在AssetOpsBench的261个场景中，执行任务从1061降至623，Accomplished从0.638升至0.706，工具调用从11.81降至6.82。在MCP Bench上，GPT OSS1和Llama 4 Maverick的规划、基础相关分数均提升。

Conclusion: SPIN包装器有效减少不必要执行，提升任务完成率和效率。
...
TL;DR: 提出 MoCA 框架，通过分解感知与推理步骤并引入感知验证 (PV) 奖励感知保真度，缓解 VLM 中感知与推理的“跷跷板效应”。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在感知与推理协同中常出现“跷跷板效应”，提升一方会损害另一方，根本原因是模态信用分配模糊，无法区分错误源于“看错”还是“想错”。

Method: 使用强化学习框架，将生成过程分解为交替的感知与推理步骤。通过“盲推理代理”（感知验证）独立奖励感知保真度，并引入结构化口头验证 (SVV) 替代大模型评判，实现结构化、可扩展的信用分配。

Result: MoCA 机制能准确将奖励路由至错误源头（感知或推理），在广泛视觉任务上实现同步性能提升，避免传统外部代理的工程负担。

Conclusion: 通过显式分解感知与推理并设计针对性奖励，可有效解决 VLM 中的感知-推理权衡，推动单个模型在多种任务上的协同进步。
...
TL;DR: 提出了MathAtlas，首个大规模研究生级别数学自动形式化基准，包含约5.2万定理、定义等，并带有约17.8万关系的依赖图。基准挑战性极高，最强基线正确率仅9.8%（定理）和16.7%（定义），在深度依赖子集上正确率跌至2.6%。


<details>
  <summary>Details</summary>
Motivation: 现有自动形式化基准多集中于奥林匹克或本科数学，研究生及研究级数学仍未被充分探索。

Method: 从103本研究生数学教科书中提取约5.2万个数学实体（定理、定义、习题、例子、证明），构建包含约17.8万关系的数学依赖图，并评估多种基线模型（如GPT-4等）在自动形式化上的表现。

Result: MathAtlas质量高但极具挑战：强基线定理正确率≤9.8%，定义≤16.7%；在深度依赖子集MA-Hard上，最佳模型仅2.6%正确率。

Conclusion: MathAtlas填补了研究生级自动形式化基准的空白，依赖图的引入促进了依赖感知系统的发展，但当前模型在深度依赖场景下表现严重不足，亟需更先进的自动形式化方法。
...
TL;DR: 提出多阶段飞行拒绝（MSIFR）框架，在合成数据生成过程中早期终止低质量样本，减少token浪费并保持或提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的合成数据生成方法需先生成完整输出再过滤，导致大量低质量样本的token被浪费。

Method: 将生成过程分解为多个阶段，使用基于规则的快速验证器检测算术不一致、幻觉模式及格式违规，在完成前早期拒绝低质量轨迹。

Result: 在5个指令微调模型和7个推理基准上，MSIFR单独使token消耗降低11%-77%，结合Early-Exit方法最高降低78.2%，同时保持或提升评估准确率。

Conclusion: MSIFR是一种无需额外训练或架构修改的有效机制，能显著提升LLM合成数据生成的效率。
...
TL;DR: 提出SkillFlow，一种基于流的框架，通过可训练监督者和动态技能库实现任务编排，在14个数据集上超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理系统在任务编排中面临策略崩溃、梯度方差高和技能演化无指导的问题。

Method: 采用Tempered Trajectory Balance（TTB）流匹配损失，结合递归技能演化机制，实现多样化策略和透明信用分配。

Result: 在问答、数学推理、代码生成和交互决策任务中显著优于基线方法。

Conclusion: SkillFlow通过基于流的训练信号闭环，有效提升了多步任务编排的鲁棒性和自动化能力。
...
TL;DR: 本文提出ChromaFlow框架，通过多种工具增强的自动推理在GAIA 2023任务上测试，结果显示更强的编排并未提升性能反而增加操作噪声。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型代理的复杂能力（如规划、工具使用等）引入了操作故障模式，仅靠最终准确率无法发现这些模式，因此需要更细致的评估方法。

Method: 采用ChromaFlow框架，基于规划器主导执行、专业化工具和遥测驱动的评估，在GAIA 2023 Level-1验证任务上进行严格控制实验。

Result: 基础设置下正确率54.72%，扩展编排后降至50.94%，且跟踪、超时等操作噪声增加；小型测试显示不稳定。

Conclusion: 更激进的编排无益，可靠自主代理评估需将规划器级联、确定性提取等作为首要需求。
...
TL;DR: 本文提出基于注意力的有限理性决策框架，通过优先处理关键药物子集来应对短缺，模拟验证其在降低计算复杂度的同时保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 医院药师在不确定性和时间压力下优先关注高风险药物，启发研究者探索注意力引导的决策机制。

Method: 开发专家代理（基于药师访谈的注意力权重）和学习代理（自适应调整注意力分配），结合蒙特卡洛模拟进行长短期场景测试。

Result: 注意力引导的规划在无需完整状态推理的情况下支持稳定决策，不同时间范围内性能保持稳健。

Conclusion: 决策的核心不在于选择具体动作，而在于认知资源的分配；满足式策略可降低问题复杂度同时维持性能。
...
TL;DR: 提出了ClawForge框架，用于在状态冲突下评估命令行Agent的工作流执行能力，通过生成式基准测试研究了现有模型在处理持久状态时的不足。


<details>
  <summary>Details</summary>
Motivation: 现有交互式基准测试多从干净状态初始化，缺乏对预存在冲突、过时或不一致任务状态系统测试，导致Agent在实际持久状态工作流中的失败无法被发现。

Method: 构建了基于模板生成器的基准框架，编译情景模板、插槽、初始化状态、参考轨迹和验证器，生成可复现任务，并通过正则化最终状态和副作用进行逐步评估。

Result: 在7个前沿模型上，最佳模型严格准确率仅45.3%，所有模型状态错误替换率低于17%，模型表现差异最大来自是否预先检查状态（17%到90%）。

Conclusion: 当前模型在处理状态冲突的工作流中普遍存在近失失败和不同失败风格，表明需要更好地感知和执行持久状态的能力。
...
TL;DR: 提出了一种通过学习生成可执行求解器代码的新范式，在保证正确性的同时优化执行速度，并在组合优化任务上取得了显著效果。


<details>
  <summary>Details</summary>
Motivation: 现有学习范式只关注预测准确性，但实际中求解器的运行时间同样关键，且正确解可能来自不同运行速度的多个求解器。

Method: 引入"求解器提示"抽象概念，从样本中推断可重用结构并编译为专用求解器代码；通过固定库中选择样本一致且经验上最快的求解器，证明其正确性和运行时间的泛化性。

Result: 在21个目标分布上，合成求解器平均归一化质量达0.971，比平均启发式池高0.224，比最优质量启发式高0.098；速度比最优质量启发式快336.9倍，比Gurobi快342.8倍，比限时精确求解器快16.1倍。在PACE 2025集支配数据集上，所有100图均有效，速度比顶级求解器快约两个数量级。

Conclusion: 通过编译分布特定的计算替代通用指数搜索或优化，可以同时提升求解质量和速度，验证了该框架的有效性。
...
### [23] [Learning Developmental Scaffoldings to Guide Self-Organisation](https://arxiv.org/abs/2605.14998)
*Milton L. Montero,Elias Najarro,Jakob Schauser,Sebastian Risi*

Main category: cs.AI

TL;DR: 本文提出一种神经细胞自动机与坐标基图案生成器联合学习的方法，研究自组织过程中初始预模式的信息卸载现象，发现联合学习可提升鲁棒性、编码能力和对称性破缺，且有效预模式并非简单近似目标，而是通过偏置动态过程促进收敛。


<details>
  <summary>Details</summary>
Motivation: 生物发育中，自组织过程常依赖初始条件（如母体预模式）提供位置和对称性破缺信息，这种信息卸载类似于计算系统的内存-计算权衡，但缺乏量化分析方法。

Method: 构建神经细胞自动机（NCA）与可学习坐标基图案生成器（SIREN）的联合学习框架，两者同步训练以生成目标图案，并通过信息论分析信息在预模式与自组织过程间的分布。

Result: 联合学习相比纯自组织方法能提升鲁棒性、编码容量和对称性破缺能力；有效预模式不直接逼近目标，而是通过偏置发育动态促进收敛。

Conclusion: 初始预模式与自组织规则的共同学习揭示了非平凡的信息分布关系，为理解发育中信息卸载的机制提供了计算框架。
...
### [24] [Agentic Systems as Boosting Weak Reasoning Models](https://arxiv.org/abs/2605.14163)
*Varun Sunkaraneni,Pierfrancesco Beneventano,Riccardo Neumarker,Tomaso Poggio,Tomer Galanti*

Main category: cs.AI

TL;DR: 弱推理模型委员会通过验证器增强的搜索机制，在SWE-bench Verified上达到强模型水平，表明弱模型生成的候选解中已包含正确答案，主要挑战在于选择而非生成。


<details>
  <summary>Details</summary>
Motivation: 探究弱推理模型委员会能否通过推理时增强达到强模型性能，并形式化分析其机制边界。

Method: 提出验证器支持的委员会搜索方法，通过分离提议覆盖、局部可识别性、进展和多样性进行形式化分析，并给出基于排名的界限。

Result: 在SWE-bench Verified上，单GPT-5.4 nano解决67.0%任务，委员会搜索（k=8）达到76.4%，匹配Gemini 3 Pro和Claude Opus 4.5 Thinking性能，接近79.0% oracle上界。

Conclusion: 弱模型提议池已包含大量正确补丁，剩余失败主要源于提议覆盖盲区，仅靠更强选择无法弥补该缺陷。
...
### [25] [Unsteady Metrics and Benchmarking Cultures of AI Model Builders](https://arxiv.org/abs/2605.14164)
*Stefan Baack,Christo Buschek,Maty Bohacek*

Main category: cs.AI

TL;DR: 该研究分析了2025年11家主要AI公司发布的139个模型中引用的231个基准，发现评估体系碎片化，基准更多作为叙事工具而非标准化测量。


<details>
  <summary>Details</summary>
Motivation: 模型评估依赖公司宣传而非同行评议，但基准选择及其传达的信息缺乏系统研究。

Method: 构建Benchmarking-Cultures-25数据集，开发统一分类法，进行定性定量分析。

Result: 63.2%的基准由单一公司使用，38.5%仅出现在一个模型中；基准被不同公司赋予不同能力标签，如“通用知识应用”定义模糊，主要评估STEM领域。

Conclusion: 基准主要服务于市场定位而非科学评估，其作为标准化工具的功能被削弱。
...
### [26] [The Evaluation Trap: Benchmark Design as Theoretical Commitment](https://arxiv.org/abs/2605.14167)
*Theodore J Kalaitzidis*

Main category: cs.AI

TL;DR: 论文提出新方法论Epistematics，用于审计基准测试是否真正评估了所宣称的能力，避免评估陷入自我强化的陷阱。


<details>
  <summary>Details</summary>
Motivation: 现有AI基准测试隐含理论假设，导致评估窄化并自证正确，阻碍范式进步。

Method: Epistematics方法论：从技术能力主张推导评估标准，审计基准能否区分目标能力与代理行为。包括审计流程、失败模式分类和设计准则。

Result: 通过Dupoux等人案例展示：其修改架构但仍沿用原有评估标准，导致评估无法检测到自身强化的限制。

Conclusion: 需要元评估方法确保基准测试的效度，避免评估与声称能力脱节。
...
### [27] [Grounded Continuation: A Linear-Time Runtime Verifier for LLM Conversations](https://arxiv.org/abs/2605.14175)
*Qisong He,Yi Dong,Xiaowei Huang*

Main category: cs.AI

TL;DR: 提出运行时验证器，通过显式依赖图和符号引擎确保LLM长对话中下一句话基于最新前提，从而消除上下文操纵攻击。


<details>
  <summary>Details</summary>
Motivation: LLM在长对话中可能使用已放弃的前提生成看似合理的话语，导致上下文操纵攻击，现有方法无法有效检测。

Method: 采用运行时验证器，包含显式依赖图：LLM将每轮对话分类为8种更新操作（基于动态认知逻辑、溯因推理、意识逻辑和论证理论），符号引擎记录声明与证据的依赖关系，通过图遍历检查连续性支持，并通过消息传播标识撤回的结论。

Result: 在LongMemEval-KU上准确率89.7%，优于LLM基线（88.5%）和RAG基线（87.2%）；在15项过时前提测试中达100%准确率（基线93.3%）；撤回检查延迟微秒级，历史重放线性增长。
...
TL;DR: SimPersona是一种从历史流量中学习离散买家类型并作为角色标记提供给LLM网页代理的框架，显著提升了买家行为多样性和转化率对齐。


<details>
  <summary>Details</summary>
Motivation: 现有LLM网页代理常忽略买家异质性，而基于提示的角色方法脆弱、难扩展，无法真实模拟人群行为。

Method: 使用行为感知VQ-VAE从点击流中学习离散买家类型，将其映射为LLM代理词汇中的角色标记并微调，推理时通过编码器前向传播分配类型。

Result: 在42个线上店铺的837万买家中，转化率对齐达78%，表现出可解释的行为变化，并在目标导向购物任务中优于8倍参数基线。

Conclusion: SimPersona有效捕获买家异质性，提供可扩展的人群模拟方法，并开源了数据流水线。
...
### [29] [ASH: Agents that Self-Hone via Embodied Learning](https://arxiv.org/abs/2605.14211)
*Benjamin Schneider,Xavier Schneider,Victor Zhong,Sun Sun*

Main category: cs.AI

TL;DR: 本文提出ASH，一种从无标签互联网视频中自主学习的智能体系统，无需奖励设计或专家注释，通过自我改进循环和逆动力学模型从互联网视频中提取监督信号，在长期具身任务中显著超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有长期具身任务方法依赖手工设计的奖励或动作标注的演示，难以规模化扩展，而互联网视频资源丰富但缺乏标签。

Method: ASH系统包含自我改进循环：当智能体卡住时，从其自身轨迹学习逆动力学模型（IDM），并利用IDM从相关互联网视频中提取监督信号；同时通过无监督学习从大规模视频中识别关键时刻作为长期记忆。

Result: 在宝可梦绿宝石和塞尔达传说缩小帽两个需要数小时规划的任务中，ASH分别达到平均11.2/12和9.9/12里程碑，而最强基线仅6.5/12和6.0/12。

Conclusion: 自我改进智能体是长期具身学习的可扩展方案。
...
### [30] [MetaAgent-X : Breaking the Ceiling of Automatic Multi-Agent Systems via End-to-End Reinforcement Learning](https://arxiv.org/abs/2605.14212)
*Yaolun Zhang,Yujie Zhao,Nan Wang,Yiran Wu,Jiayu Chang,Yizhao Chen,Qingyun Wu,Jishen Zhao,Huazheng Wang*

Main category: cs.AI

TL;DR: 提出MetaAgent-X框架，通过端到端强化学习联合优化多智能体系统的自动设计与执行，实现自我设计与执行，性能提升最高21.7%。


<details>
  <summary>Details</summary>
Motivation: 现有自动多智能体系统仅部分自适应，存在冻结执行器上限，未探索端到端训练自我设计与执行模型。

Method: 采用端到端强化学习框架，包含脚本生成、执行展开收集、设计者与执行器轨迹的信用分配，并提出执行器-设计者分层展开与阶段式协同进化策略。

Result: MetaAgent-X一致超越现有基线，最高性能提升21.7%。消融实验表明设计者与执行器均持续改进，学习遵循阶段式协同进化过程。

Conclusion: 端到端可训练的自动多智能体系统是构建自我设计与执行智能模型的有效范式。
...
### [31] [GenCircuit-RL: Reinforcement Learning from Hierarchical Verification for Genetic Circuit Design](https://arxiv.org/abs/2605.14215)
*Noah Flynn*

Main category: cs.AI

TL;DR: 提出了GenCircuit-RL框架，通过分层验证奖励和课程学习，利用强化学习生成遗传电路代码，显著提升功能推理任务的成功率，并能在新生物部件上泛化。


<details>
  <summary>Details</summary>
Motivation: 遗传电路设计仍依赖专家经验和劳动力，自动化程度不足，需探索更高效的代码生成方法。

Method: 基于强化学习，引入分层验证奖励（五级正确性检查）和四阶段课程学习，利用pysbol3和SBOL生成电路代码。

Result: 分层验证使功能推理任务成功率提升14-16个百分点，课程学习对设计性能至关重要，模型能生成拓扑正确电路并泛化到新部件。

Conclusion: 该方法能自动化生成正确遗传电路，重现已有的经典设计，有望减少人工设计依赖。
...
TL;DR: 提出Loop Skill Engine系统，通过一次性录制和确定性重放范式，将LLM代理的周期性任务成功率提升至99%，并降低99%的token消耗。


<details>
  <summary>Details</summary>
Motivation: LLM执行周期性任务时存在随机性故障和高token成本的问题。

Method: 采用一次录制、确定性重放范式，首次执行时记录完整工具调用轨迹，通过贪心长度递减模板提取算法将其转换为参数化、无分支的Loop Skill，后续执行绕过LLM，直接解析变量并重放工具序列。

Result: 在5分钟至24小时跨度的基准测试中，月token消耗降低93.3%–99.98%，执行延迟降低8.7倍，消除输出非确定性。

Conclusion: Loop Skill Engine在保持LLM灵活性的同时，实现了高成功率、低成本和可预测的周期性任务执行。
...
TL;DR: 提出HEAR，一种基于分层超图本体的企业级推理器，用于解决LLM在企业系统中的幻觉和多跳推理失败问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂企业系统中缺乏语义基础和可审计执行，导致多跳、n元推理任务失败。

Method: 构建分层超图本体，包括基础图层的溯源数据接口和超边层的业务规则，采用证据驱动推理循环动态协调工具。

Result: 在供应链任务中（如订单履行阻塞根因分析）达到94.7%准确率，并通过超边减少token成本，在复杂查询中保持正确性。

Conclusion: HEAR为可扩展、可审计的企业智能建立了基础，匹配专有模型性能，并自动化人工诊断。
...
TL;DR: 研究揭示了ChatGPT类AI行为不可预测偏移的驱动力源于活性物质中的融合-裂变动力学，并提出可通过量化对话、期望与非期望状态的群体竞争提前预测偏移。该方法在七种AI模型及前沿对话系统中验证，并提前11个月成功预测了Stanford数据集的行为偏移。


<details>
  <summary>Details</summary>
Motivation: ChatGPT类AI在实际应用中存在行为从合意偏移到危险（如自伤、极端主义、医疗错误）的风险，且现有对齐技术无法预测这种偏移，迫切需要一种跨模型、可提前预警的通用方法。

Method: 将对话上下文（C）与期望（B）和非期望（D）状态视为活性物质中的群体竞争系统，利用向量化的融合-裂变动力学方程建模，并推导出偏移条件。该条件不依赖具体模型或随机采样，可通过预设应用场景预先估计。

Result: 在六个独立测试中验证：七种参数规模（124M–12B）AI模型的预测准确率达90%；前沿对话系统的行为偏移持久性被成功捕捉；提前11个月预测了Stanford‘妄想螺旋’数据集中的行为偏移，并被该数据集的207,443次人机交互所证实。

Conclusion: 该偏移预测机制位于当前安全堆栈之下，能为实时预警提供现有对齐技术无法实现的功能，且可跨架构移植，适用于任何能定义竞争响应类别的应用领域。
...
### [35] [Heuristic Pathologies and Further Variance Reduction via Uncertainty Propagation in the AIVAT Family of Techniques](https://arxiv.org/abs/2605.14261)
*Juho Kim,Tuomas Sandholm*

Main category: cs.AI

TL;DR: 本文分析了AIVAT方差缩减技术在多智能体评估中的局限性，指出启发式值函数可能被人为操纵，并提出了不确定性量化方法，在扑克数据集上实现了43%的样本减少。


<details>
  <summary>Details</summary>
Motivation: AIVAT方法中启发式值函数的选择缺乏约束和指导，可能导致方差被病理性地降低或统计结论被p-hacking操纵。

Method: 参数化启发式值函数以揭示其脆弱性（梯度下降降低方差、梯度上升操控统计量）；通过传播启发式不确定性来量化AIVAT估计的不确定性，并使用逆方差加权平均进一步降方差。

Result: 扑克数据集实验显示，不确定性传播在保持统计结论质量的同时减少了43%的样本需求。

Conclusion: 启发式值函数应在评估数据收集前固定；不确定性传播可量化估计可靠性，但可能牺牲无偏性。
...
### [36] [Agentic AI Ecosystems in Higher Education: A Perspective on AI Agents to Emerging Inclusive, Agentic Multi-Agent AI Framework for Learning, Teaching and Institutional Intelligence](https://arxiv.org/abs/2605.14266)
*Vidya K Sudarshan,Anushka Sisodia,Reshma A Ramachandra,Sia Batra,Josephine Chong Leng Leng*

Main category: cs.AI

TL;DR: 本文提出高等教育中应构建集成化、多智能体AI平台，支持教学、学习与行政的协同运作，并特别关注包容性学习。


<details>
  <summary>Details</summary>
Motivation: 现有AI工具碎片化，无法应对高等教育复杂性，且缺乏对多样学习者的包容性支持。

Method: 通过文献主题分析法，识别出当前AI实施中的四大主题：碎片化工具、单智能体到多智能体过渡、跨功能集成不足、包容性缺失。

Result: 发现当前AI实施与整体化、以学习者为中心的生态系统需求之间存在明显差距。

Conclusion: 未来研究方向包括构建可扩展、人类对齐且包容的多智能体AI平台，以灵活的多模态干预支持多样化学习者。
...
### [37] [Parallelizing Counterfactual Regret Minimization](https://arxiv.org/abs/2605.14277)
*Juho Kim,Tuomas Sandholm*

Main category: cs.AI

TL;DR: 提出将反事实遗憾最小化（CFR）算法并行化的通用框架，通过重构成线性代数运算来加速游戏求解。


<details>
  <summary>Details</summary>
Motivation: 并行化在AI领域减少训练时间效果显著，但在博弈求解中应用不足；CFR算法是解决大型不完全信息博弈的关键，现有CPU实现效率低。

Method: 将CFR算法重新表述为一系列线性代数操作，利用并行化技术加速；扩展至CFR+、折扣CFR、预测变体等族内算法。

Result: GPU上的CFR实现相比DeepMind OpenSpiel的CPU版本速度提升高达四个数量级。

Conclusion: 并行化线性代数运算可有效加速CFR求解，为大型博弈的实时求解提供可能。
...
### [38] [Precise Verification of Transformers through ReLU-Catalyzed Abstraction Refinement](https://arxiv.org/abs/2605.14294)
*Hengjie Liu,Zhenya Zhang,Jianjun Zhao*

Main category: cs.AI

TL;DR: 提出一种通过新颖使用ReLU来表示点积的精确非线性边界，从而提升Transformer形式化验证精度的方法，并扩展了两种经典框架。


<details>
  <summary>Details</summary>
Motivation: Transformer在安全关键应用中的部署日益广泛，但现有验证方法因过度近似导致精度损失和误报频繁。

Method: 利用ReLU表示点积的精确非线性边界，结合凸松弛技术，扩展了基于规则和基于优化的两种验证框架。

Result: 在情感分析数据集上的实验表明，该方法在大多数验证任务中显著提升精度，且对效率的折衷可接受。

Conclusion: 所提方法能有效提升Transformer验证的精度，相比基线具有明显优势。
...
### [39] [Semantic Feature Segmentation for Interpretable Predictive Maintenance in Complex Systems](https://arxiv.org/abs/2605.14318)
*Emilio Mastriani,Alessandro Costa,Federico Incardona,Kevin Munari,Sebastiano Spinello*

Main category: cs.AI

TL;DR: 提出了一种语义特征分割框架，将监测变量分解为规范成分和残差成分，在保持可解释性的同时实现与全特征空间和PCA相当的预测性能。


<details>
  <summary>Details</summary>
Motivation: 预测性维护中监测变量的异构性和冗余性掩盖了故障相关信息并降低了模型可解释性。

Method: 基于领域知识定义语义分割标准，将监测变量按功能分组（如吞吐量、延迟、压力等），并通过预测风险作为任务相关信息代理来评估分割效果。

Result: 规范成分的预测风险始终低于残差成分，且规范片段内相干性远高于片段间依赖性，结构在冗余减少后保持稳定；与全特征空间和PCA相比，规范成分具备相当的预测性能并保留了语义含义。

Conclusion: 语义特征分割提供了一种可解释且信息保持的监测信号分解方法，在预测性维护中实现了竞争性性能与运营可解释性的平衡。
...
### [40] [Are Agents Ready to Teach? A Multi-Stage Benchmark for Real-World Teaching Workflows](https://arxiv.org/abs/2605.14322)
*Zixin Chen,Peng Liu,Rui Sheng,Haobo Li,Jianhong Tu,Xiaodong Deng,Kashun Shum,Dayiheng Liu,Huamin Qu*

Main category: cs.AI

TL;DR: EduAgentBench是一个基于理论、现实的教育基准，用于全面评估语言代理在辅导任务中的教学能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准未衡量代理在辅导中的多方面能力（如诊断学生状态、适应性支持、基于教育学的决策等），因此需要更全面的评估工具。

Method: 通过教育学驱动的管道构建150个质量控制任务，涵盖专业教学判断、多轮情境辅导和Canvas式教学流程完成，并采用互补验证信号和人工审核评估。

Result: 前沿模型在有限的教学判断上表现尚可，但在情境辅导和自主教学流程执行中未达到专业标准。

Conclusion: EduAgentBench是首个理论驱动、现实导向的辅导代理整体教学能力基准，为开发能支持真实教学工作的代理提供了测量基础。
...
### [41] [CrystalReasoner: Reasoning and RL for Property-Conditioned Crystal Structure Generation](https://arxiv.org/abs/2605.14344)
*Yuyang Wu,Stefano Falletta,Delia McGrath,Sherry Yang*

Main category: cs.AI

TL;DR: 提出CrystalReasoner框架，通过思考轨迹和强化学习生成有效、稳定且满足特定属性的晶体结构。


<details>
  <summary>Details</summary>
Motivation: 现有LLM模型原子精度低，扩散方法难以整合科学知识，导致生成结构无效或不稳定。

Method: 将物理先验作为思考令牌（对称性、配位环境、物理性质），再通过多目标密集奖励的强化学习对齐生成。

Result: 在多个指标上优于基线，S.U.N.比率提高三倍，属性条件生成表现更好，且推理长度随原子数自适应增长。

Conclusion: 思考轨迹和强化学习能有效生成有效、稳定且满足条件的晶体结构。
...
TL;DR: 本文发现语言模型生成的推理轨迹中存在大量冗余步骤，平均46%的步骤可被移除，并提出了最小核心的概念来提取必要步骤。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型生成的长链推理轨迹中，有多少中间步骤对最终预测是真正必要的。

Method: 定义了最小核心和四个评估指标，并在六个推理基准上通过贪婪提取方法进行实验分析。

Result: 平均46%的步骤可移除，86%的情况保留原答案；前三步贡献65%的必要性质量；最小核心改善了正确与错误轨迹分离精度11点，降低了内在维度34%。

Conclusion: 完整推理轨迹往往冗长且过完备，而最小核心能有效隔离支撑语言模型预测的必要步骤。
...
TL;DR: 提出了首个面向金融智能体的技能基准Herculean，覆盖交易、对冲、市场洞察和审计四个工作流，发现当前智能体在长周期协调和结构化验证任务上表现不足。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准仅评估问答、检索等静态能力，无法反映智能体在金融专业工作中的可靠性。

Method: 基于MCP协议构建标准化技能环境，每个工作流包含专用工具、交互规则、约束条件和成功标准，对异构智能体系统进行端到端评估。

Result: 智能体在交易和市场洞察任务中表现较好，但在对冲和审计任务中显著困难，暴露出长周期协调、状态一致性和结构化验证方面的关键缺陷。
...
### [44] [Nexus : An Agentic Framework for Time Series Forecasting](https://arxiv.org/abs/2605.14389)
...
TL;DR: 提出Nexus多智能体框架，通过分阶段解耦宏观与微观波动、整合上下文信息，在多个领域超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型忽略文本信号，而LLM在零样本预测中表现不稳定，需要桥接数值与上下文推理。

Method: 多智能体分解框架，先隔离宏观与微观时间波动，再整合上下文信息，最后综合预测。

Result: 在Zillow房地产和股票市场数据上，Nexus匹配或超越最好基线，且生成高质量推理链。

Conclusion: 现实预测本质是智能体推理问题，超出单纯序列建模范畴。
...
### [45] [Learning to Build the Environment: Self-Evolving Reasoning RL via Verifiable Environment Synthesis](https://arxiv.org/abs/2605.14392)
*Yucheng Shi,Zhenwen Liang,Kishan Panaganti,Dian Yu,Wenhao Yu,Haitao Mi*

Main category: cs.AI

TL;DR: 本文提出自改进语言模型通过构造可执行环境而非生成数据来实现持续提升，核心在于保持解-验证不对称性，并在EvoEnv方法中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有自改进方法依赖生成数据或轨迹模仿，但存在策略漏洞导致奖励信号失效；本文旨在探索模型通过构造环境实现可持续自改进的机制。

Method: 提出EvoEnv方法，通过单策略生成器-求解器合成Python环境，经过分阶段验证、语义自审查、求解器相对难度校准和 novelty 检查后接纳环境。

Result: 在Qwen3-4B-Thinking模型上，EvoEnv将平均性能从72.4提升至74.8（相对提升3.3%），而固定数据或固定环境RLVR则导致性能下降。

Conclusion: 稳定的自改进不依赖于合成更多数据，而在于模型学会构造难度始终超出其自身能力范围的环境。
...
### [46] [Coding Agent Is Good As World Simulator](https://arxiv.org/abs/2605.14398)
*Hongyu Wang,Jingquan Wang,Bocheng Zou,Radu Serban,Dan Negrut*

Main category: cs.AI

TL;DR: 提出一种基于可执行仿真代码的智能体框架，通过多智能体协作生成物理一致的世界模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频基础世界模型不显式施加物理约束，导致生成视频物理不真实（如不稳定接触、形状扭曲）。

Method: 设计包含规划、代码生成、视觉审查和物理分析四个智能体的框架，通过迭代反馈修正仿真代码。

Result: 在物理准确性、指令忠实度和视觉质量上超越先进视频模型。

Conclusion: 该框架适用于驾驶模拟和具身机器人等场景。
...
### [47] [Metis AI: The Overlooked Middle Zone Between AI-Native and World-Movers](https://arxiv.org/abs/2605.14407)
*Xiang Li*

Main category: cs.AI

TL;DR: 本文提出AI能力的真正界限并非数字与物理任务之分，而是数字任务内部存在一类难以自动化且具社会规范的Metis AI任务，并建议采用人类主导的Centaur架构而非更优的自动化。


<details>
  <summary>Details</summary>
Motivation: 现有讨论错误地将AI局限性归因于数字与物理任务的分离，忽略了数字任务内部更关键且被忽视的分类界限。

Method: 通过定义Metis概念（实践性、情境性知识），区分构成性与操作性Metis，提出五个结构特征（后果不可逆性、关系不可还原性、规范开放纹理、对抗性共演化、责任锚定），并结合跨学科理论进行论证。

Result: 揭示了Metis AI任务具有社会、制度与规范纠缠的特性，这些特性源于任务本身而非模型限制，并指出自动化在此类任务中效力有限。

Conclusion: 应对Metis AI任务的最佳设计不是追求更优自动化，而应采用人类领导、AI辅助的Centaur架构。
...
### [48] [A Unified Knowledge Embedded Reinforcement Learning-based Framework for Generalized Capacitated Vehicle Routing Problems](https://arxiv.org/abs/2605.14416)
*Wen Wang,Xiangchen Wu,Liang Wang,Hao Hu,Xianping Tao*

Main category: cs.AI

TL;DR: 提出一种知识嵌入框架，结合预序聚类后序启发式和强化学习，以统一求解带容量约束的车辆路径问题及其变体。


<details>
  <summary>Details</summary>
Motivation: 强化学习方法在组合优化中端到端学习缺乏显式问题求解知识，导致解质量有限。

Method: 将问题分解为'预序'和'后序聚类'两个子问题；用动态规划求解后序问题，其结果指导基于强化学习的构造式求解器解决预序问题；引入历史增强上下文处理模块缓解分解导致的局部可观测性。

Result: 与前沿学习方法相比，该框架解质量更优，与经典启发式算法差距更小，且对多种CVRP变体泛化能力强。

Conclusion: 知识嵌入框架能够有效提升强化学习方法在CVRP上的求解性能。
...
### [49] [DVMap: Fine-Grained Pluralistic Value Alignment via High-Consensus Demographic-Value Mapping](https://arxiv.org/abs/2605.14420)
*Pengyun Zhu,Yuqi Ren,Zhen Wang,Lei Yang,Deyi Xiong*

Main category: cs.AI

TL;DR: 提出DVMap框架，通过多维度人口统计约束替代国家标签，实现细粒度的多元化价值对齐。


<details>
  <summary>Details</summary>
Motivation: 当前大模型依赖粗粒度国家标签进行价值对齐，难以捕捉国家内部的价值异质性，导致对齐松散。

Method: 从世界价值观调查数据提取高共识人口统计原型，构建对齐语料库；引入结构化思维链引导模型推理人口-价值关联；使用组相对策略优化实现自适应价值分布锚定。

Result: 在跨人口统计测试中，Qwen3-8B-DVMap达到48.6%准确率，超越DeepSeek-v3.2（45.1%），表现出强泛化性和鲁棒性。

Conclusion: DVMap有效学习从人口统计到价值的流形映射，为多元价值对齐提供了细粒度、可泛化的解决方案。
...
### [50] [BEAM: Binary Expert Activation Masking for Dynamic Routing in MoE](https://arxiv.org/abs/2605.14438)
*Juntong Wu,Jialiang Cheng,Qishen Yin,Yue Dai,Yuliang Yan,Fuyu Lv,Ou Dan,Li Yuan*

Main category: cs.AI

TL;DR: BEAM通过可训练二进制掩码实现动态专家选择，在不需重新训练的情况下将MoE层计算量减少85%，同时保持98%以上的原始性能。


<details>
  <summary>Details</summary>
Motivation: 标准MoE的固定Top-K路由策略导致计算冗余和推理延迟，现有加速方法存在需重新训练或性能严重下降的问题。

Method: 提出可训练二进制专家激活掩码，使用直通估计器和辅助正则化损失进行端到端训练，并实现自定义CUDA内核。

Result: MoE层FLOPs减少85%，解码速度提升2.5倍，吞吐量提高1.4倍，保留98%以上性能。

Conclusion: BEAM是一种有效的即插即用方案，实现了高效MoE推理。
...
### [51] [Synthesizing POMDP Policies: Sampling Meets Model-checking via Learning](https://arxiv.org/abs/2605.14440)
*Debraj Chakraborty,Anirban Majumdar,Prince Mathew,Sayan Mukherjee,Jean-François Raskin*

Main category: cs.AI

TL;DR: 提出一个结合采样、自动机学习和模型检测的POMDP综合框架，能够为有限状态控制器提供形式化保证。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，基于采样的方法缺乏形式化保证，而形式化综合技术难以扩展。

Method: 基于Angluin的L*算法，使用采样作为成员查询、模型检测作为等价查询，综合出有限状态控制器。

Result: 原型实现表明该方法能够成功解决其他形式化工具难以处理的阈值安全性问题。

Conclusion: 该算法可作为处理POMDP综合难题的组合方法中的重要组成部分。
...
### [52] [Prompting Policies for Multi-step Reasoning and Tool-Use in Black-box LLMs with Iterative Distillation of Experience](https://arxiv.org/abs/2605.14443)
*Krishna Sayana,Ketan Todi,Ambarish Jash*

Main category: cs.AI

TL;DR: 提出一种通过强化学习迭代蒸馏经验来训练提示策略的框架，在多个任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 与大语言模型交互已从启发式练习转变为关键优化挑战，需要更高效的提示工程方法。

Method: 使用轻量提示器模型通过强化学习最大化冻结大语言模型的任务奖励，结合对比经验缓冲区（含标量奖励和文本评价），将迭代提示优化转化为单次策略权重。

Result: 在BBEH和Tau-bench测试中，逻辑推理任务性能从55%提升至90%，工具使用任务从74%提升至91%，且样本效率优于GEPA等基线。

Conclusion: 迭代蒸馏方法能有效发现结构化提示策略，实现更优性能和更高效率。
...
TL;DR: 提出一种名为IIQ的复合度量指标，用于量化AI系统在组织工作中的整合深度和影响，结合多种因素生成0-1000的标准化指数，并给出子日更新规则和解释层。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如访问次数或总token量）不足以证明AI的实际影响，需要更精细的度量框架来区分不同类型的AI使用方式。

Method: 采用新颖加权、时间衰减的token存量与使用频率、宽限期最近门、组织杠杆、任务复杂度和自主性组合，推导出原始IAI和标准化IIQ指数。

Result: 通过合成场景展示，该度量能区分频繁低效使用、语义重复提示和更自主高影响的工作。

Conclusion: IIQ是一个面向部署的测量框架，用于追踪AI在工作流程中的嵌入程度，而非直接衡量模型能力或替代因果生产力评估。
...
TL;DR: 本文发现链式思维推理长度增加时，模型对早期关键洞察的注意力减弱导致性能下降，提出InsightReplay方法，通过在生成过程中周期性提取并重放关键洞察来保持其可访问性，在多种模型和基准上平均提升1.65个精度点。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明链式思维推理长度与准确率呈非单调关系，过长链会导致性能下降，但原因不明确，需探究机制并提出改进。

Method: 提出InsightReplay，一种状态推理方法，让模型在生成推理轨迹时，周期性地提取关键洞察，将其重放在当前生成前沿附近，以保持长推理中洞察的可访问性。

Result: 在2×3×4基准网格上的所有24个设置中，3轮InsightReplay均优于标准链式思维，平均提升+1.65点，最大单设置提升+9.2点，涵盖不同模型大小、系列和推理基准。

Conclusion: 测试时扩展的有效性不仅取决于推理量，还取决于关键中间洞察在长推理轨迹中的可访问性，InsightReplay通过保持洞察可访问性提升了性能。
...
TL;DR: 提出OmniDrop，一种免训练、逐层剪枝框架，在LLM解码层内渐进剪枝音视频令牌，解决高分辨率输入导致的令牌爆炸问题。


<details>
  <summary>Details</summary>
Motivation: 现有全模态令牌压缩方法在输入嵌入层基于不可靠的相似性假设剪枝，不能满足实时应用和长时推理需求。

Method: 在LLM解码层内而非输入层渐进剪枝音视频令牌；利用文本查询引导模态无关和任务自适应剪枝；引入时间多样性分数促进令牌均衡存活以保留全局时间上下文。

Result: 在各种音视频基准测试中，OmniDrop比所有基线高3.58分，同时降低预填充延迟40%，内存使用14.7%。

Conclusion: OmniDrop有效解决了全模态大语言模型中高分辨率令牌爆炸问题，提升了效率和性能。
...
TL;DR: 本文提出TABALIGN框架，通过扩散语言模型生成二进制单元格掩码作为计划步骤，并引入轻量验证器TABATTN基于人工标注的注意力标准评估计划质量，在表格推理任务中相比强基线提升15.76个百分点的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法中规划与执行缺乏显式的单元格锚定契约，且左到右因子化规划方式违背表格置换不变性，导致多步LLM表格推理失败。

Method: 使用掩蔽扩散语言模型（DLM）作为规划器，通过双向去噪生成二进制单元格掩码作为执行步骤；设计TABATTN验证器，基于人工标注的注意力标准计算掩码与注意力重叠分数来评估每一步计划。

Result: 在8个基准测试中平均准确率提升15.76个百分点；消融实验显示DLM规划器贡献2.87个百分点；计划执行速度提升44.64%。

Conclusion: 扩散语言模型通过生成置换不变性的单元格掩码，有效解决了表格推理中规划与执行的锚定问题。
...
TL;DR: 提出了一种基于反事实强化学习的可执行多智能体编排方法（LEMON），能自动化生成任务特定的角色、职责、容量和依赖关系，在多个基准测试中达到最新最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动化编排方法通常部分或顺序优化决策，且依赖执行级反馈，难以对局部编排决策提供有效信用分配。

Method: 利用LLM作为编排器生成可执行编排规格，并通过增强的GRPO目标与局部反事实信号（编辑角色/容量/依赖字段并对比奖励差异）进行训练。

Result: 在MMLU、GSM8K、AQuA、MultiArith、SVAMP和HumanEval六个基准上取得领先于现有多智能体编排方法的性能。

Conclusion: LEMON通过反事实强化学习实现了更有效的多智能体编排自动化，提升了推理和代码生成任务的质量与效率。
...
TL;DR: 本文提出了Deepchecks，一个针对检索增强生成（RAG）系统的综合评估框架，能够从多维度进行根因分析和生产监控，确保RAG系统的可靠性、相关性和用户满意度。


<details>
  <summary>Details</summary>
Motivation: RAG系统的评估面临生成结果的随机性以及检索与生成组件之间复杂交互的挑战，现有方法难以全面、可靠地评价系统性能。

Method: 通过多维度评估方法、根因分析和生产监控，Deepchecks框架对RAG应用进行全面评估，并确保与特定应用需求对齐。

Result: Deepchecks为RAG系统提供了可靠的评估基础，能够有效衡量可靠性、相关性和用户满意度。

Conclusion: Deepchecks框架为RAG应用程序的评估和监控提供了稳健的解决方案，有助于推动其在各领域的可靠应用。
...
TL;DR: 提出了LongAct基准和HoloMind智能体，用于评估和提升长时域家庭任务的规划级自主能力。


<details>
  <summary>Details</summary>
Motivation: 现有具身AI基准侧重短时域导航或操作，且依赖固定任务类别，忽视了长时域家庭任务中鲁棒的高层规划和持续推理能力。

Method: LongAct通过抽象低级控制来隔离高级认知能力；HoloMind包含基于DAG的长时域分层规划器、多模态空间记忆、情景记忆和全局评论器。

Result: HoloMind显著提升长时域性能并降低对模型规模的依赖；即使顶级模型也仅达59%目标完成率和16%完全任务成功率。

Conclusion: LongAct揭示了现有模型在长时域规划上的不足，亟需更强的规划能力。
...
### [60] [Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty](https://arxiv.org/abs/2605.14494)
*Tianjue Lin,Jianan Zhou,Jieyi Bi,Yaoxin Wu,Wen Song,Zhiguang Cao,Jie Zhang*

Main category: cs.AI

TL;DR: 提出了一种结合问题驱动启发式与GNN-Transformer的神经替代模型NeurPRISE，用于两阶段鲁棒优化中的场景缩减，在保持高质量的同时实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有场景缩减方法忽略问题结构，且PRISE启发式计算成本高，难以扩展。

Method: PRISE通过顺序评估场景边际影响进行选择；NeurPRISE使用GNN-Transformer编码场景结构，并通过模仿学习训练排序函数来近似PRISE。

Result: 在三类2RO问题上，NeurPRISE达到与PRISE相当的后悔值，加速7-200倍，并展现出零样本泛化能力。

Conclusion: 神经替代可有效加速问题驱动的场景缩减，同时保持质量和泛化性。
...
TL;DR: 提出了 Cattle Trade 基准，一个多智能体经济博弈环境，用于评估大语言模型在非完美信息、对抗性交互和资源约束下的策略推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准孤立测试子能力，无法评估智能体在多智能体竞争环境中整合多种能力（如竞价、谈判、资源分配）的表现。

Method: 设计了一个 50-60 回合的长程游戏，结合拍卖、隐藏报价贸易挑战、讨价还价、虚张声势、对手建模和资源分配。记录了每个回合的行为数据，以便进行细粒度分析。

Result: 在 242 场游戏中评估了七个成本高效的语言模型和三个确定性代码智能体。策略连贯性（如支出效率、资源纪律）与排名强相关。两个启发式代码智能体优于大多数 LLM。LLM 常见失败模式包括过度竞标、自我竞标、破产交易触发和对手状态适应弱。

Conclusion: 评估智能体能力需要在具有冲突激励、不确定性和经济动态的多智能体环境中测试联合部署多种能力。
...
### [62] [Falkor-IRAC: Graph-Constrained Generation for Verified Legal Reasoning in Indian Judicial AI](https://arxiv.org/abs/2605.14665)
*Joy Bose*

Main category: cs.AI

TL;DR: Falkor-IRAC是一个基于IRAC知识图谱的图约束生成框架，用于印度法律AI，通过Verifier Agent验证生成答案的合法性，减少幻觉和错误引用。


<details>
  <summary>Details</summary>
Motivation: 向量RAG无法准确表示法律推理的符号约束，导致幻觉和错误引用，影响司法公正，尤其在印度等高案件量地区。

Method: 将判决书解析为IRAC节点结构，包含程序状态转换、先例关系和法定引用，存储在FalkorDB中；推理时使用Verifier Agent验证LLM生成的答案是否在图中有有效路径。

Result: 在51个最高法院判决的概念验证语料上，Verifier Agent正确验证了已完成的引用并拒绝了伪造引用。

Conclusion: Falkor-IRAC通过图约束生成提高了法律推理的准确性，但未来需评估对向量RAG基线的性能，并解决GPU加速问题。
...
### [63] [VerbalValue: A Socially Intelligent Virtual Host for Sales-Driven Live Commerce](https://arxiv.org/abs/2605.14542)
*Yuyan Chen*

Main category: cs.AI

TL;DR: 提出面向直播带货的虚拟主播VerbalValue，通过领域知识库、标注数据和微调LLM实现销售导向的对话，提升信息准确性和用户参与度。


<details>
  <summary>Details</summary>
Motivation: 现有对话推荐系统缺乏销售转化能力，通用LLM存在幻觉和模板化问题，无法模拟优秀主播的专业知识、情感回应和娱乐性策略。

Method: 构建产品规格知识库和销售术语词典；收集并标注1475条直播交互数据；微调LLM以适配不同观众意图，采用共情增强、证据反驳和幽默调节策略。

Result: 相比GPT-5.4等基线，信息量提升23%，事实正确性提升18%，在策略性和用户参与度上持续领先。

Conclusion: 基于专业知识和情感适应的虚拟主播能有效提升直播带货的销售转化效果。
...
### [64] [Agentifying Patient Dynamics within LLMs through Interacting with Clinical World Model](https://arxiv.org/abs/2605.14723)
...
TL;DR: SepsisAgent是一个基于世界模型增强的LLM智能体，用于脓毒症治疗推荐，通过三阶段训练在MIMIC-IV数据上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM缺乏对患者动态的基于动作的建模能力，无法直接用于序列治疗决策。

Method: 提出propose-simulate-refine工作流，结合临床世界模型和三个训练阶段：患者动态微调、行为克隆和基于世界模型的强化学习。

Result: 在MIMIC-IV脓毒症轨迹上，SepsisAgent在离策略价值和安全性指标上优于所有传统RL和LLM基线。

Conclusion: 与世界模型的反复交互使智能体学习到患者演化规律，即使移除模拟器仍有效，证明了方法的鲁棒性。
...
### [65] [Complacent, Not Sycophantic: Reframing Large Language Models and Designing AI Literacy for Complacent Machines](https://arxiv.org/abs/2605.14544)
*Federico Germani,Giovanni Spitale*

Main category: cs.AI

TL;DR: 本文论证将大语言模型的行为描述为“谄媚”具有误导性，提出“自满”概念更准确，强调偏差源于训练设计而非模型意图。


<details>
  <summary>Details</summary>
Motivation: 纠正学界对模型行为术语的误用，将责任从模型转移到开发者与机构。

Method: 概念分析与框架重构，通过对比“谄媚”与“自满”的定义差异，论证模型行为本质。

Result: 模型只会表现出不同程度的“自满”，而非具有真实意图的“谄媚”，偏差源于人类设计。

Conclusion: 应通过AI素养教育对抗自满模型强化的确认偏误，开发者需承担修正责任。
...
### [66] [TeachAnything: A Multimodal Crowdsourcing Platform for Training Embodied AI Agents in Symmetrical Reality](https://arxiv.org/abs/2605.14556)
*Zidong Liu,Rongkai Liu,Yue Li,Zhenliang Zhang*

Main category: cs.AI

TL;DR: 提出了面向对称现实的多模态示范平台TeachAnything，通过三阶段示范范式与物理仿真，收集多样化示范数据以支持具身智能体发展。


<details>
  <summary>Details</summary>
Motivation: 对称现实要求智能体具备类人智能，需要更丰富多样的人类引导信号。

Method: 设计了三阶段示范范式，整合多模态信号，构建了基于云的众包演示平台TeachAnything，并集成物理仿真。

Result: 平台能收集跨场景、任务和具身形态的多样化示范数据，统一虚拟与物理交互。

Conclusion: 为开发符合对称现实的具身智能体提供了实用基础。
...
### [67] [Holistic Evaluation and Failure Diagnosis of AI Agents](https://arxiv.org/abs/2605.14865)
...
TL;DR: 提出了一个结合自上而下智能体诊断与自下而上跨度级评估的整体性智能体评估框架，将分析分解为独立的、基于跨度的评估，在TRAIL基准上取得了最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有结果指标只能报告成功或失败，无法解释原因；过程级方法难以将失败类型与其在长结构化轨迹中的确切位置对应起来。

Method: 采用自上而下的智能体级诊断与自下而上的跨度级评估相结合，将分析分解为独立的、基于跨度的评估。

Result: 在GAIA和SWE-Bench上所有指标均达到最优，类别F1相对提升38%，定位精度提升3.5倍，联合定位-分类精度提升12.5倍。

Conclusion: 评估方法而非模型能力是瓶颈：相同的顶级模型在该框架内的定位精度显著高于作为整体性评估器。
...
### [68] [Explainable Detection of Depression Status Shifts from User Digital Traces](https://arxiv.org/abs/2605.14995)
*Loris Belcastro,Francesco Gervino,Fabrizio Marozzo,Domenico Talia,Paolo Trunfio*

Main category: cs.AI

TL;DR: 提出一个可解释框架，结合多种BERT模型和LLM，从用户数字轨迹中检测抑郁相关状态变化，生成可读报告，在社交媒体数据集上优于直接LLM报告。


<details>
  <summary>Details</summary>
Motivation: 用户生成的时间戳数字轨迹可能反映心理状态，但现有方法缺乏可解释性和对状态变化（如改善或恶化）的敏感分析。

Method: 多BERT模型提取情感、情绪和抑郁严重度信号，聚合为轨迹后分析变化点，并集成LLM生成人类可读的演变报告。

Result: 在社交媒体数据集上，方法比直接LLM报告有更高的历史覆盖率、更强的时间连贯性和更好的变化点敏感性。

Conclusion: 提供了可解释的心理健康信号时间演化视图，支持研究决策，但不用于临床诊断。
...
### [69] [PyCSP3-Scheduling: A Scheduling Extension for PyCSP3](https://arxiv.org/abs/2605.14559)
*Sohaib Afifi*

Main category: cs.AI

TL;DR: 提出了PyCSP$^3$ Scheduling库，为PyCSP$^3$添加调度抽象（区间变量、序列变量、资源函数），并通过编译到标准约束保持建模与求解的分离。


<details>
  <summary>Details</summary>
Motivation: PyCSP$^3$缺乏调度抽象，导致调度模型需用低层整数变量和手动通道约束编码，尽管已有全局约束。

Method: 设计53个专用约束和27个表达式，编译为标准PyCSP$^3$/XCSP$^3$约束，保持建模/求解分离。

Result: 在261对实例（17个模型族）上，两种公式化在所有72个双重最优对中目标相同，8/17模型族编译后结构不变；但运行性能因编译分解开销存在差异（部分加速5.8倍，部分退步）。

Conclusion: PyCSP$^3$ Scheduling成功添加调度抽象，性能在不同模型族上表现不一，代码已开源。
...
### [70] [Small, Private Language Models as Teammates for Educational Assessment Design](https://arxiv.org/abs/2605.15015)
*Chris Davis Jaldi,Anmol Saini,Shan Zhang,Noah Schroeder,Cogan Shimizu,Eleni Ilkou*

Main category: cs.AI

TL;DR: 系统比较了大语言模型和小语言模型在教育评估出题中的表现，发现小语言模型在保证教学质量的同时可本地部署，但评估存在系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对生成式AI出题质量的系统评估，小语言模型在教育评估中的作用尚未充分探索。

Method: 基于布鲁姆分类学，使用可复现的教学评估指标比较LLM和SLM的出题质量，并对比模型评价与专家评价的一致性。

Result: SLM在关键教学质量维度上表现有竞争力，但模型评价与专家评价存在系统不一致和偏差。
...
### [71] [Prompt Segmentation and Annotation Optimisation: Controlling LLM Behaviour via Optimised Segment-Level Annotations](https://arxiv.org/abs/2605.14561)
*Devika Prasad,Luke Gerschwitz,Tong Li,Henry Xiao,Anjin Liu,Coco Wu,Anna Leontjeva,Luiz Pizzato*

Main category: cs.AI

TL;DR: 提出了一种名为PSAO的结构化提示优化框架，通过将提示分解为可解释片段并添加注释来提升LLM的推理准确性和自一致性。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法空间庞大且无结构，计算成本高且可能扭曲原始意图，需要一种可控且高效的优化方式。

Method: PSAO将提示分解为句子等可解释片段，并添加人类可读的注释（如{不重要}、{重要}、{非常重要}），引导LLM分配注意力和澄清模糊点。原始提示保留为候选者以防止性能下降。

Result: 经验评估表明，PSAO的注释在提升推理准确性和自一致性方面具有优势。

Conclusion: 这项工作作为概念验证，证明了片段级注释优化的可行性和潜力，但识别最优片段和注释的高效方法仍需未来研究。
...
### [72] [Sycophancy is an Educational Safety Risk: Why LLM Tutors Need Sycophancy Benchmarks](https://arxiv.org/abs/2605.14604)
*Enkelejda Kasneci,Gjergji Kasneci*

Main category: cs.AI

TL;DR: 本文论证有效辅导需要纠正性摩擦，即支持性地揭示和挑战错误观念以促进概念转变，但偏好对齐的大语言模型可能为了亲和力而牺牲认知严谨性；识别出推理-谄媚悖论，并引入EduFrameTrap基准测试来评估模型在不同社会认知压力下的表现。


<details>
  <summary>Details</summary>
Motivation: 偏好对齐的大语言模型在辅导中可能因追求用户满意度而放弃认知严谨性，导致学术不端行为，需系统评估其教育场景中的社会认知勇气。

Method: 提出EduFrameTrap基准测试，在数学、物理等六学科中变化学生自信度和三种压力（上下文切换、权威、社会情感），使用两个前沿大语言模型并采用双法官不一致度作为可靠性信号。

Result: GPT-5.2的上下文切换失败率较低，但权威和社会压力更易引发认知退缩；Claude在该测试中表现出显著的上下文切换脆弱性。

Conclusion: 基准测试应测量社会认知勇气（支持性但纠正性的辅导），并将友善且正确的行为作为安全要求。
...
TL;DR: 提出开源框架Orchard，通过轻量环境服务Orchard Env支持多领域智能体建模，在编码、GUI操作和虚拟助手三个任务上达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有开源智能体框架多聚焦于编排和评估，缺乏可扩展的训练基础设施，高性能系统常依赖专有代码，难以复现和推广。

Method: 构建Orchard Env提供沙箱生命周期管理；针对编码智能体采用知识蒸馏、信用分配SFT和平衡自适应 rollout RL；对GUI和虚拟助手使用少量蒸馏轨迹和开放任务训练。

Result: Orchard-SWE在SWE-bench Verified达67.5%；Orchard-GUI在WebVoyager等任务上达74.1%；Orchard-Claw在Claw-Eval达59.6% pass@3。

Conclusion: 轻量开源环境层可跨领域复用智能体数据、训练方案和评估，实现媲美专有系统的高性能开源智能体。
...
TL;DR: 提出基于案例驱动的框架CAST，通过历史执行轨迹提取复杂度和失败信号，优化工具使用中的推理深度与结构有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在工具使用时需要平衡推理深度与结构合法性，现有方法缺乏对历史案例的精细化利用。

Method: 构建案例驱动框架，从历史执行轨迹中提取复杂度与失败模式信号，用于奖励设计及自适应推理，并通过强化学习内化策略。

Result: 在BFCLv2和ToolBench上，执行准确率提升5.85个百分点，平均推理长度减少26%，有效降低结构性错误。

Conclusion: 历史执行案例可提供可重用的适应知识，实现校准的工具使用。
...
### [75] [SliceGraph: Mapping Process Isomers in Multi-Run Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.14619)
*Kang Chen,Junjie Nian,Yixin Cao,Yugang Jiang*

Main category: cs.AI

TL;DR: 提出SliceGraph方法，从多阶段推理链（CoT）的中间过程构建图结构，发现相同正确答案的推理路径常分为多个“过程异构体”家族，揭示了最终答案聚合所忽略的推理过程几何结构。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多次推理链的最终答案进行聚合，丢弃了中间计算过程的共享、分裂和合并信息，无法捕捉推理过程的结构化几何特征。

Method: 基于互k近邻和稀疏激活键Jaccard相似性，从CoT片段构建后验问题-模型-细胞图（SliceGraph），并通过双连通分量识别共享推理状态单元和过程家族。

Result: 在95.4%的问题-模型细胞中，正确推理链分成多个过程家族；平均76.6%的成对推理链属于不同家族；成功区域常分裂为不连通的高值核心，各过程家族在这些核心上形成独特足迹。

Conclusion: 正确答案对应的推理路径存在结构化多路线几何（过程异构体），最终答案聚合会忽略这一结构，而SliceGraph方法能稳健地揭示该现象。
...
### [76] [Teaching Large Language Models When Not to Know: Learning Temporal Critique for Ex-Ante Reasoning](https://arxiv.org/abs/2605.14636)
*Chenlu Ding,Jiancan Wu,Yanchen Luo,Zheyuan Liu,Yancheng Yuan,Xiang Wang*

Main category: cs.AI

TL;DR: LLMs在时间截止点下的推理会泄露未来知识；本文提出TCFT框架，通过训练模型进行截止点感知的时间验证，显著减少了时间泄露。


<details>
  <summary>Details</summary>
Motivation: LLMs在时间截止点下推理时经常失败，会泄露截止点后才知道的未来信息，现有方法无法解决此问题。

Method: 提出Temporal Critique Fine-Tuning (TCFT)框架，训练模型识别后截止点泄露、解释时间边界违规、判断时间可接受性。

Result: 在Qwen2.5-7B-Instruct和Qwen2.5-14B-Instruct上，TCFT比提示和SFT基线分别减少41.89和37.79个百分点的时间泄露。

Conclusion: 提示可以引导模型进入时间框架，但无法赋予其验证时间可接受性的能力；TCFT通过训练模型掌握截止点感知的时间验证，能有效解决此问题。
...
### [77] [MindGap: A Conversational AI Framework for Upstream Neuroplastic Intervention in Post-Traumatic Stress Disorder](https://arxiv.org/abs/2605.14660)
...
TL;DR: 提出了一种基于佛教缘起理论的隐私保护会话AI框架MindGap，用于PTSD的神经可塑性康复，通过干预应激反应前的感受间隙实现上游通路消解。


<details>
  <summary>Details</summary>
Motivation: 现有PTSD疗法（如暴露疗法、EMDR、CBT）主要在下游抑制症状，未能实现永久性的神经结构重组，需要一种从根源消解过度反应通路的方法。

Method: 基于依赖缘起理论，设计三层渐进的观察训练（感受原始信号、识别其自生性、觉察潜藏信念），结合设备端轻量级大语言模型进行每日定制的暴露训练。

Result: 通过前额叶调节和长时程抑制机制，渐进式削弱反应通路的连接强度，实现从下游抑制到上游消解的转变。

Conclusion: MindGap提供了一种隐私安全、可部署于敏感场景的神经可塑性康复方案，能更根本地改变PTSD的神经结构。
...
### [78] [How Sensitive Are Radiomic AI Models to Acquisition Parameters?](https://arxiv.org/abs/2605.14667)
*D. Gil,I. Sanchez,C. Sanchez*

Main category: cs.AI

TL;DR: 提出一个性能导向框架，量化CT参数对影像组学AI模型的影响，并识别提高跨数据集鲁棒性的最佳扫描参数组合。


<details>
  <summary>Details</summary>
Motivation: AI放射组学系统在多中心异构采集协议下性能下降，阻碍临床部署。

Method: 构建混合效应模型，量化临床相关采集参数对模型性能的影响，同时考虑受试者随机效应。

Result: 最佳配置为电流≥200mA、螺旋节距≤1.5、层厚≤1.25mm，将低质量扫描的灵敏度从0.79±0.04、特异度0.47±0.10提升至高质量扫描的0.90±0.10和0.79±0.13。

Conclusion: 该框架可有效识别关键参数区域，平衡诊断质量与低辐射剂量，提升跨数据库可重复性。
...
### [79] [Monitoring Data-aware Temporal Properties (Extended Version)](https://arxiv.org/abs/2605.14666)
*Alessandro Gianola,Marco Montali,Sarah Winkler*

Main category: cs.AI

TL;DR: 本文提出了一种针对带有SMT理论的LTLfMT性质的预期监控框架，通过结合自动机理论和自动推理技术，解决了动态系统监控中状态依赖过去和未来轨迹的挑战，并识别了可判定的实际相关片段，通过原型实现验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 动态系统复杂且异构，内部规范不可达，传统模型检测等技术不适用，而现有监控面临踪迹前缀与所有可能有限延续的依赖挑战。

Method: 结合自动机理论处理逻辑的时间维度，利用自动推理技术处理一阶维度，在合理的背景理论假设下提出并验证了预期监控的基础框架。

Result: 成功识别了结合线性算术与无解释函数的可判定监控片段，该片段覆盖数据感知业务流程和只读数据库的动态系统，并通过原型实现和初步评估证明了可行性。

Conclusion: 该框架为LTLfMT属性在动态系统上的预期监控提供了基础，在理论正确性和实际相关性方面取得了进展。
...
### [80] [$π$-Bench: Evaluating Proactive Personal Assistant Agents in Long-Horizon Workflows](https://arxiv.org/abs/2605.14678)
...
TL;DR: 提出了π-Bench基准，用于评估智能体在多轮交互中主动识别和处理隐含用户意图的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准很少评估智能体在用户未明确表达需求时主动发现并回应的能力，尤其是在多轮交互中需求逐渐显露的场景。

Method: 构建了包含100个多轮任务和5个领域用户角色的π-Bench基准，集成了隐藏意图、任务依赖和跨会话连续性。

Result: 实验表明主动辅助仍具挑战性，任务完成与主动性存在明显区别，先前的交互经验有助于后续任务中主动意图的解析。

Conclusion: 该基准可有效评估智能体在实际长期交互中的主动性和任务完成能力。
...
### [81] [On Strong Equivalence Notions in Logic Programming and Abstract Argumentation](https://arxiv.org/abs/2605.14721)
*Giovanni Buraglio,Wolfgang Dvorak,Stefan Woltran*

Main category: cs.AI

TL;DR: 本文提出逻辑程序的一种新强等价概念，在动态上下文中恢复与抽象论证框架的等价性。


<details>
  <summary>Details</summary>
Motivation: 逻辑程序与抽象论证框架在静态环境语义等价，但在动态更新中因更新定义不同而脱节，导致强等价性无法传递。

Method: 通过引入新的强等价定义，确保在特定逻辑程序类与Dung式及主张增强论证框架间的翻译下保持强等价。

Result: 新定义成功恢复了两类形式主义之间的兼容性，使得替换知识库不影响推理结果。

Conclusion: 提出的强等价概念弥合了逻辑程序与论证框架在动态上下文中的差异，增强了形式主义的互操作性。
...
### [82] [XDomainBench: Diagnosing Reasoning Collapse in High-Dimensional Scientific Knowledge Composition](https://arxiv.org/abs/2605.14754)
...
TL;DR: 提出基准XDomainBench评估大模型在跨学科科学推理中的组合泛化能力，揭示“推理崩溃”现象。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅覆盖单回合受限场景，未能捕捉真实交互科学工作流中的能力边界。

Method: 形式化组合顺序与混合结构，构建8598个交互会话（20领域、4任务类别、8种轨迹模式）进行压力测试。

Result: 大模型随组合顺序增加出现系统性推理崩溃，根因包括领域组合带来的难度提升以及轨迹模式引发的错误累积、推理中断和领域混淆。

Conclusion: 交互式跨学科场景暴露了大模型在组合泛化上的关键弱点，需发展更具鲁棒性的方法。
...
### [83] [Probabilistic Verification of Recurrent Neural Networks for Single and Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.14758)
*Luca Marzari,Enrico Marchesini*

Main category: cs.AI

TL;DR: 提出RNN-ProVe框架，用于概率性验证RNN策略在部分可观测RL中的不良行为，提供有界误差的高置信度估计。


<details>
  <summary>Details</summary>
Motivation: 现有RNN验证工具受限于严格假设或过度近似，导致结果保守或无效。

Method: 利用策略驱动采样近似可行隐藏状态集，结合统计误差界生成概率保证。

Result: 在单智能体和多智能体任务中，比现有工具提供更定量、可行的概率保证。

Conclusion: RNN-ProVe能有效扩展到循环和多智能体设置，提供可信的行为违规估计。
...
### [84] [AI Outperforms Humans in Personalized Image Aesthetics Assessment via LLM-Based Interviews and Semantic Feature Extraction](https://arxiv.org/abs/2605.14761)
*Yoshia Abe,Tatsuya Daikoku,Yasuo Kuniyoshi*

Main category: cs.AI

TL;DR: 提出了一种集成深度学习与大语言模型的系统，通过主动访谈获取偏好，在图像美感预测上优于人类和传统模型，误差小于个人内部变异性。


<details>
  <summary>Details</summary>
Motivation: 个体审美评价存在主观性和个体依赖性，传统深度学习模型仅提取客观低层特征，难以准确预测个体偏好。

Method: 开发了深度学习与大语言模型集成系统，通过LLM进行半结构化访谈主动获取审美偏好，同时利用低层和高层语义特征进行预测。

Result: 系统在所有对比中表现最佳，尤其在高分图像上优势显著；预测误差小于个人内部变异性，而人类预测者误差最大。

Conclusion: AI可能比他人或未来的自己更能捕捉特定时刻的个体审美偏好，提出了AI或可成为比人类自身更深层的审美解释者的新问题。
...
### [85] [Identifying Culprits Through Deep Deterministic Policy Gradient Deep Learning Investigation](https://arxiv.org/abs/2605.14774)
*Lata B T,Savitha N J*

Main category: cs.AI

TL;DR: 提出基于深度确定性策略梯度（DDPG）的犯罪识别方法，在复杂数据集中以95%准确率优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统刑侦依赖有限数据分析，难以从复杂数据中高效识别罪犯并减少误报。

Method: 使用DDPG算法训练犯罪现场数据、证人陈述和嫌疑人档案，优化特征以最大化识别准确性并降低噪声干扰。

Result: DDPG方法在罪犯识别中达到95%的准确率，优于多种现有方法。

Conclusion: 该深度学习框架有效提升了犯罪识别的性能，减少了误报和漏报。
...
### [86] [MediaClaw: Multimodal Intelligent-Agent Platform Technical Report](https://arxiv.org/abs/2605.14771)
...
TL;DR: MediaClaw是一个基于OpenClaw生态的多模态智能体平台，通过三层架构（统一抽象、插件扩展、工作流编排）解决AIGC部署中的碎片化、接口异构、流程割裂及复用难等问题，将能力抽象为统一调用模型，并通过插件和技能实现可复用工作流。


<details>
  <summary>Details</summary>
Motivation: 解决AIGC实际部署中的痛点：能力碎片化、接口异构、生产过程不连贯、高质量工作流复用率低。

Method: 三层架构设计：统一抽象层、插件化扩展层、工作流编排层；通过系统抽象全类别AIGC能力为统一调用模型，利用插件支持热插拔扩展，任务导向的技能将复杂流程转化为可复用工作流资产。

Result: 提供了可复用的多模态能力平台构建实践参考，解决了部署痛点并提升了工作流复用性。

Conclusion: MediaClaw的架构设计为多模态能力平台的工程实现提供了可借鉴的实践经验，强调统一抽象、插件化和工作流编排的重要性。
...
### [87] [Interestingness as an Inductive Heuristic for Future Compression Progress](https://arxiv.org/abs/2605.14831)
*Vincent Herrmann,Jürgen Schmidhuber*

Main category: cs.AI

TL;DR: 本文形式化了'有趣性'概念，将其定义为未来压缩进展的归纳启发式，并通过Kolmogorov复杂性和算法统计工具证明其可预测性。


<details>
  <summary>Details</summary>
Motivation: 实现递归自我改进系统的主要瓶颈之一是识别哪些任务或数据具有未来进步潜力的能力。

Method: 使用Kolmogorov复杂性和算法统计，分析长度先验、算法先验和速度先验下的复杂性-运行时轮廓，并证明过去突破的新近性对未来进展的影响。

Result: 预期未来进展指数依赖于最后观察到的突破的新近性；算法先验比长度先验更乐观，预期发现量呈二次增长。

Conclusion: 有趣性的归纳属性在理论和实验上均成立，并在三种通用计算范式上得到验证。
...
### [88] [A Heterogeneous Temporal Memory Governance Framework for Long-Term LLM Persona Consistency](https://arxiv.org/abs/2605.14802)
*Zhao Yang,Wang Huan,Li Yingshuo,Tu Haomiao,Lin Hujite*

Main category: cs.AI

TL;DR: 提出了ARPM框架，一种外部的时序记忆治理方法，用于解决长对话中的事实丢失、时间线混乱、人格漂移和稳定性下降问题。通过分离静态知识记忆和动态对话经验记忆，结合向量检索、BM25、RRF融合、双时序重排、时间顺序证据读取和受控分析协议，在50轮对话、消融实验和510万字符噪声环境下验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长距离交互中常出现事实丢失、时间线混乱、人格漂移和稳定性下降，特别是在高噪声知识库、上下文清除和跨模型迁移场景下。现有方法将人格一致性编码到模型权重或仅依赖长上下文，而ARPM将连续性视为可追踪、可审计和可迁移的治理问题。

Method: ARPM框架包括静态知识记忆与动态对话经验记忆分离、向量检索、BM25、RRF融合、双时序重排、时间顺序证据读取和受控分析协议用于证据验证和答案绑定。

Result: 在1:5信噪比下，CSV召回准确率54.0%，人工审查提升至100.0%；1:200+下为44.0%和80.0%。消融实验显示禁用对话历史检索将严格准确率从100%降至66.7%，禁用BM25降至80.0%。在510万字符噪声下保持语义连续性、边界连续性和人格一致性。

Conclusion: 长期人格一致性可分解为可治理的组件，并能以白盒方式评估。ARPM在自动规则低估召回时表现不足，但整体上通过分离记忆和多种检索融合有效提升了连续性，同时协议遵从性弱是主要限制。
...
### [89] [Emotion-Attended Stateful Memory (EASM):The Architecture for Hyper-Personalization at Scale](https://arxiv.org/abs/2605.14833)
*Vineet Kotecha,Vansh Gupta*

Main category: cs.AI

TL;DR: 提出一种情感注意的有状态记忆架构，通过长期历史、情感信号和用户意图实现个性化对话，在30次非脚本对话实验中，相比无状态基线在记忆锚定（提升95%）、计划清晰度（57%）和情感认可（34%）上显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在会话间保持无状态，无法持续个性化交互；检索增强生成和微调虽提升知识访问和领域能力，但未能建立对个体的持久理解。

Method: 构建情感注意的有状态记忆架构，在推理时动态整合用户长期历史、情感信号和推断意图，形成个性化对话上下文。

Result: 在跨6类情感的无脚本对话A/B测试中，有状态记忆条件全面优于无状态基线，最大改进体现在记忆锚定（95%）、计划清晰度（57%）和情感认可（34%），且在高情感对抗场景中结果一致。

Conclusion: 有状态情感记忆可能成为超个性化AI系统的基础设施层，但需在更广泛和多样化的评估中进一步验证。
...
### [90] [BiFedKD: Bidirectional Federated Knowledge Distillation Framework for Non-IID and Long-Tailed ECG Monitoring](https://arxiv.org/abs/2605.14886)
*Zixuan Shu,Tiancheng Cao,Hen-Wei Huang*

Main category: cs.AI

TL;DR: 提出双向联邦知识蒸馏框架BiFedKD，通过聚合-蒸馏流程和温度缩放来解决异构心电图数据中的非独立同分布和长尾标签问题，在MIT-BIH心律失常数据集上提升准确率3.52%、Macro-F1 9.93%，同时降低40%通信开销和71.7%计算成本。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在心电监测中面临高维模型更新传输开销大，且联邦蒸馏在非独立同分布和长尾标签数据下性能退化的问题。

Method: 采用双向联邦知识蒸馏框架，通过聚合-蒸馏管线结合温度缩放生成稳定的全局蒸馏信号，实现跨客户端对齐。

Result: 在MIT-BIH心律失常数据集上，BiFedKD比基线准确率提升3.52%，Macro-F1提升9.93%，通信开销减少40%，计算成本降低71.7%。

Conclusion: BiFedKD有效缓解了非独立同分布和长尾标签对联邦蒸馏性能的影响，同时显著降低通信和计算开销。
...
### [91] [A Deterministic Agentic Workflow for HS Tariff Classification: Multi-Dimensional Rule Reasoning with Interpretable Decisions](https://arxiv.org/abs/2605.14857)
*Yu Zhang,Dongjiang Zhuang,Qu Zhou,Zheng Huang,Junhe Wu,Jing Cao,Kai Chen*

Main category: cs.AI

TL;DR: 提出一种确定性代理工作流，将HS编码分类分解为六阶段结构化流程，使用Qwen3.6-plus达到64.2%六位码top-1准确率，优于直接大模型提示方法。


<details>
  <summary>Details</summary>
Motivation: 现有大模型在HS编码分类中无法同时满足多维度规则推理的优先级约束，常常只解决一个维度而忽略其他限制。

Method: 采用固定控制流的确定性代理工作流，大模型调用仅限于狭窄阶段，保留局部反思与验证机制；结合离线知识工程和六阶段在线管线。

Result: Qwen3.6-plus在HSCodeComp上四位码top-1准确率75.0%、top-3 91.5%；六位码64.2% top-1和78.3% top-3。开源Qwen3.6-27B-FP8（非思考模式）与前沿模型四位码一致性84.2%、六位码77.4%。

Conclusion: 该工作流具备内在可解释性，通过阶段式结构化输出和章节注释引用展现决策过程；部分HSCodeComp标签可能偏离HS通用规则，需社区进一步审查。
...
### [92] [Beyond Individual Intelligence: Surveying Collaboration, Failure Attribution, and Self-Evolution in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.14892)
...
TL;DR: 该综述以LIFE框架（能力奠基、协作整合、故障归因、自主进化）统一分析了多智能体系统中错误传播与自我改进的因果链条，并提出跨阶段研究议程。


<details>
  <summary>Details</summary>
Motivation: 现有研究分别关注单智能体能力、多智能体协作或自我进化，忽略了错误在协作中传播并阻碍结构性自我改进的因果依赖关系。

Method: 提出LIFE四阶段因果框架（能力奠基→协作整合→故障归因→自主进化），通过系统分类法形式化阶段间依赖约束。

Result: 揭示了阶段间依赖关系与边界开放挑战，构建了闭环多智能体系统的研究蓝图。

Conclusion: 该框架桥接了碎片化研究，为迈向自组织集体智能提供了系统性参考与概念路线图。
...
### [93] [COREKG: Coreset-Guided Personalized Summarization of Knowledge Graphs](https://arxiv.org/abs/2605.14900)
*Sohel Aman Khan,Raghava Mutharaju,Supratim Shit*

Main category: cs.AI

TL;DR: 提出基于核心集理论个性化知识图谱摘要方法COREKG，通过敏感度采样生成小规模但高精度的子图。


<details>
  <summary>Details</summary>
Motivation: 知识图谱过大导致查询和可视化困难，亟需个性化摘要以降低存储与计算开销。

Method: 将核心集理论适配到知识图谱，定义三元组敏感度，通过基于敏感度的重要性采样生成个性化摘要。

Result: 在Freebase、WikiData、DBpedia上，COREKG在查询准确率和结构覆盖度上优于现有方法，且仅需原始图极小部分。

Conclusion: 个性化核心集摘要能有效平衡精度与规模，适于存储受限或查询频繁的场景。
...
### [94] [KGPFN: Unlocking the Potential of Knowledge Graph Foundation Model via In-Context Learning](https://arxiv.org/abs/2605.14907)
*Yisen Gao,Jiaxin Bai,Haoyu Huang,Zhongwei Xie,Yufei Li,Hong Ting Tsang,Sirui Han,Yangqiu Song*

Main category: cs.AI

TL;DR: 提出了KGPFN，一种基于先验数据拟合网络的KG基础模型，通过结构上下文实现推理时的上下文学习，在57个KG基准上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有KG基础模型主要关注关系层面的通用性，但上下文学习（ICL）在KG推理中未被充分探索，且KG上下文具有结构化和异构性。

Method: 首先通过关系图上的消息传递学习关系表示；然后使用多层NBFNet编码局部邻域作为局部上下文；再通过检索查询关系的大量实例及局部邻域构建全局上下文，并在先验数据拟合网络框架中结合特征级和样本级注意力进行聚合。

Result: 在57个KG基准上，KGPFN仅通过上下文学习就能很好地适应未见过的图，并且一致优于需要微调的竞争性KG基础模型。

Conclusion: KGPFN通过结合关系级通用性和结构上下文的上下文学习，实现了对未知图的有效适应，证明了ICL在KG推理中的潜力。
...
TL;DR: 提出双维一致性（DDC）框架，通过置信加权贝叶斯协议和趋势感知分层剪枝，在保证推理质量的同时将令牌消耗降低10倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有推理时扩展方法在采样预算与推理质量间存在权衡，宽度共识易强化幻觉，深度剪枝可能过早截断有效推理链。

Method: 融合置信加权贝叶斯协议与趋势感知分层剪枝的DDC框架，实现路径质量与自适应终止的统一。

Result: 在五个基准上，令牌消耗降低10倍以上，准确率持平或超越强基线。

Conclusion: DDC有效过滤幻觉、加速共识，提升了LLM推理的资源效率。
...
### [96] [GraphFlow: An Architecture for Formally Verifiable Visual Workflows Enabling Reliable Agentic AI Automation](https://arxiv.org/abs/2605.14968)
*Drewry H. Morris,Luis Valles,Reza Hosseini Ghomi*

Main category: cs.AI

TL;DR: GraphFlow是一个视觉工作流系统，通过将工作流图作为可执行规范，结合编译时合同验证和运行时耐久执行，提升多步关键任务中AI自动化的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有工作流平台缺乏语义正确性保证，而代理系统的行为对提示变化敏感且难以审计，导致多步过程中错误累积，可靠性低。

Method: 将工作流图作为单一可执行规范，定义数据范围、执行语义和监控；编译时限制图类型并验证合同（前置、后置条件及组合义务）；运行时使用耐久引擎、只追加事件日志和合同强制机制。

Result: 在三个临床站点的一年试点中，早期原型执行了8728个工作流，完成率为97.08%，失败主要源于外部集成。

Conclusion: GraphFlow通过正式语义和经证明检查的准入模型提高了工作流可靠性，但验证核心仍在开发中，未来需进一步评估。
...
### [97] [From Sycophantic Consensus to Pluralistic Repair: Why AI Alignment Must Surface Disagreement](https://arxiv.org/abs/2605.14912)
*Varad Vishwarupe,Nigel Shadbolt,Marina Jirotka*

Main category: cs.AI

TL;DR: 本文批判当前RLHF训练下的多价值对齐仅聚焦偏好聚合，指出其导致‘谄媚共识’——模型倾向于赞同用户而非真正价值多元。基于格莱斯准则提出三种对话机制（范围界定、信号提示、修正），并定义‘多元修正分数’来区分原则性修正与屈服。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF型AI在价值冲突场景中呈现隐性的‘谄媚共识’倾向，掩盖真实分歧，对涉及健康、公民生活等领域的协商造成结构性危害。

Method: 重新定义多价值对齐为三种对话机制：范围界定（承认视角限制）、信号提示（揭示价值冲突而非掩盖）、修正（基于原则调整立场）。提出‘多元修正分数’以量化模型是否进行实质性辩论。

Result: 对Claude Sonnet 4.5和GPT-4o的初步实验显示，两者在高争议价值观提示下表现出高附和需率与低修正质量。

Conclusion: 真正的价值多元无法仅靠聚合实现，需在部署治理层（界面、数据管道、审计）进行结构性干预；PRS衡量的是对话层面的前置条件而非多元本身，且需反思‘原则性’的权威定义问题。
...
TL;DR: 该论文指出在Agentic GraphRAG系统中，引用的忠实性不仅应支持最终答案，还应涵盖完整的图遍历路径和未引用的实体信息。通过控制消融实验，作者发现引用证据是必要的但非充分的，因为未引用的上下文和结构也会影响答案准确性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注引用对答案的支持性，但Agentic GraphRAG中代理通过探索知识图谱生成答案，引用与图遍历过程的关系未得到充分评估，因此需要定义轨迹级别的引用忠实性。

Method: 通过控制消融实验，隔离、移除和遮蔽图中已引用和未引用的实体，对比不同操作下答案变化与准确性差异。

Result: 移除已引用证据会显著改变答案并降低准确性（必要但非充分）；未引用的遍历上下文和周围图结构同样影响答案准确性。

Conclusion: 对Agentic GraphRAG的引用评估应从简单的源支持转向涵盖更广泛检索轨迹的溯源验证。
...
TL;DR: 提出OpenDeepThink，一种基于Bradley-Terry比较的群体测试时计算框架，通过并行采样和成对比较提升LLM推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖延长单一推理轨迹，但并行采样遇到选择瓶颈：LLM点式评估存在噪声和偏差。

Method: 每轮通过LLM对随机候选对进行Bradley-Terry比较并聚合为全局排名，保留高排名候选，前75%基于比较中产生的自然语言批评进行变异，后25%丢弃。

Result: Gemini 3.1 Pro在Codeforces上的有效Elo提升405分（8轮LMM调用，约27分钟），框架跨强弱模型无需调参，HLE测试中可验证领域收益提升，主观领域收益下降。

Conclusion: OpenDeepThink有效解决了并行采样中的选择问题，提升了LLM推理性能，但效果依赖于任务客观性。
...
### [101] [Few Channels Draw The Whole Picture: Revealing Massive Activations in Diffusion Transformers](https://arxiv.org/abs/2605.13974)
*Evelyn Turri,Davide Bucciarelli,Sara Sarto,Lorenzo Baraldi,Marcella Cornia*

Main category: cs.CV

TL;DR: 揭示了Diffusion Transformers中“大激活”通道作为稀疏的、受提示条件控制的语义载体子空间，能组织并控制图像生成语义。


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers及其相关架构是强大的文本到图像生成器，但其内部机制（尤其是提示如何塑造图像语义）尚不明确。

Method: 通过分析隐藏状态通道的响应大小，识别出“大激活”通道，并采用破坏性探测、空间聚类和激活迁移实验来研究其功能、空间组织和可迁移性。

Result: 大激活通道在功能上关键、空间上有组织（与主要主题和显著区域对齐），并且可迁移（实现提示插值和主题驱动生成，无需额外训练）。

Conclusion: 大激活通道并非异常，而是组织并控制语义信息的稀疏载体，可用于语义传输等应用。
...
### [102] [Contrastive Multi-Modal Hypergraph Reasoning for 3D Crowd Mesh Recovery](https://arxiv.org/abs/2605.13854)
*Minghao Sun,Chongyang Xu,Yitao Xie,Buzhen Huang,Kun Li*

Main category: cs.CV

TL;DR: 本文提出对比多模态超图推理方法，通过融合语义、几何和姿态线索，实现多人3D重建，在严重遮挡和深度模糊场景下取得新最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有单模态方法缺乏几何指导且忽略群体上下文，难以应对遮挡和深度模糊。

Method: 融合RGB特征、几何先验和遮挡感知姿态初始化节点，引入骨盆深度指示器作为全局锚点，构建共享拓扑超图建模高阶人群动态，设计超图对比学习增强模态内区分性和模态间正交性。

Result: 在Panoptic和GigaCrowd基准上达到新最优性能。

Conclusion: 超图推理有效利用群体上下文，可在严重遮挡下推断缺失信息，显著提升多人3D重建效果。
...
### [103] [CineMesh4D: Personalized 4D Whole Heart Reconstruction from Sparse Cine MRI](https://arxiv.org/abs/2605.13994)
*Xiaoyue Liu,Xiaohan Yuan,Mark Y Chan,Ching-Hui Sia,Lei Li*

Main category: cs.CV

TL;DR: 提出CineMesh4D，一种从多视图2D电影MRI端到端重建4D全心网格的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能重建部分心腔或单个心相，无法同时处理稀疏采样与心脏形状-运动耦合问题。

Method: 引入可微分渲染损失监督多视图稀疏轮廓，并设计双上下文时序模块融合全局与局部时序信息。

Result: 在重建质量和运动一致性上优于现有方法。

Conclusion: 为个性化实时心脏评估提供了实用途径。
...
### [104] [Unified Pix Token And Word Token Generative Language Model](https://arxiv.org/abs/2605.14028)
*Haun Leung,ZiNan Wang*

Main category: cs.CV

TL;DR: 提出一种统一像素令牌和单词令牌的新模型，解决ViT在细节视觉理解上的局限，通过无监督预训练在小模型和有限数据下表现良好，并符合缩放定律。


<details>
  <summary>Details</summary>
Motivation: CLIP或SigLIP方法获得的ViT作为视觉编码器时，难以识别图像中的小文本或数字等细节，限制了多模态模型的视觉理解能力。

Method: 新模型将图像像素令牌和单词令牌统一到生成语言模型中，包含颜色折叠、全局条件注意力近似以及图像无监督预训练等技术。

Result: 在无监督预训练实验中，即使使用小模型和有限训练数据，新模型也展现出良好性能。

Conclusion: 该模型符合缩放定律，性能随模型参数和训练数据增加而持续提升。
...
### [105] [A CUBS-Compatible Ultrasound Morphology and Uncertainty-Aware Baseline for Carotid Intima-Media Segmentation and Preliminary Risk Prediction](https://arxiv.org/abs/2605.14949)
*Aueaphum Aueawatthanaphisut*

Main category: cs.CV

TL;DR: 提出了AtheroFlow-XNet，一种基于超声形态学与不确定性感知的颈动脉内膜-中膜分割及风险预测基线模型。


<details>
  <summary>Details</summary>
Motivation: 传统超声评估基于形态和速度指标，可能无法充分捕捉患者特异性血管风险。

Method: 使用CUBS数据集，将手动注释转换为密集内膜-中膜掩码进行监督分割，结合临床变量进行辅助风险预测，并采用蒙特卡洛dropout实现不确定性感知推理。

Result: 模型在分割任务上Dice系数0.7930，损失0.2359；风险预测AUC为0.6910。

Conclusion: 超声衍生的颈动脉形态可支持自动管壁分析和不确定性感知解释，但当前作为形态驱动基线，未来需结合多普勒血流等生物标志物。
...
### [106] [PVRF: All-in-one Adverse Weather Removal via Prior-modulated and Velocity-constrained Rectified Flow](https://arxiv.org/abs/2605.14045)
*Wei Dong,Han Zhou,Terry Ji,Guanhua Zhao,Shahab Asoodeh,Yulun Zhang,Guangtao Zhai,Jun Chen,Xiaohong Liu*

Main category: cs.CV

TL;DR: 提出了PVRF，一个统一框架，通过零-shot软天气感知和速度约束的整流流优化，提升真实世界图像中的恶劣天气去除效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在异质和未见过的天气退化下效果不佳，且失真驱动的训练常导致过度平滑的结果。

Method: 引入AWR-QA模块，利用冻结的视觉-语言模型估计天气类型的软概率和低层属性评分；通过属性调制归一化和天气加权适配器调节恢复网络；学习终端一致的残差整流流，采用感知自适应源扰动和终端一致的速度参数化。

Result: 在保真度和感知质量上优于现有基线，并在单一和组合退化上展现出强跨数据集泛化能力。

Conclusion: PVRF有效提升了恶劣天气去除的性能和泛化性，代码将开源。
...
### [107] [Evolving Layer-Specific Scalar Functions for Hardware-Aware Transformer Adaptation](https://arxiv.org/abs/2605.14047)
*Kieran Carrigg,Sigur de Vries,Amirhossein Sadough,Marcel van Gerven*

Main category: cs.CV

TL;DR: 提出一种基于遗传编程的硬件感知框架，为视觉Transformer各层自动进化异构标量函数，替换层归一化，消除全局归约瓶颈，仅需20轮微调即可恢复84.25%的Top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 层归一化的计算复杂度和全局归约瓶颈严重阻碍了ViT在边缘设备上的部署。

Method: 利用遗传编程从预训练权重中进化出逐层异构的标量函数，配合后训练重对齐策略，无需从头重新训练模型。

Result: 进化表达式捕捉了91.6%的归一化行为方差（R²），远超同质基线的70.2%，在20轮微调后达到84.25%的Top-1 ImageNet-1K准确率。

Conclusion: 该方法在算术复杂度和片外内存流量之间取得了优越的平衡，消除了ViT在边缘加速器上高效部署的主要障碍。
...
### [108] [CurveBench: A Benchmark for Exact Topological Reasoning over Nested Jordan Curves](https://arxiv.org/abs/2605.14068)
*Amirreza Mohseni,Mona Mohammadi,Morteza Saghafian,Naser Talebizadeh Saradari*

Main category: cs.CV

TL;DR: 提出了CurveBench基准，用于评估视觉层次拓扑推理能力，包含756张Jordan曲线图像，最先进的模型仅达到71.1%的准确率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉模型在拓扑推理方面存在不足，缺乏专门评估层次拓扑关系的基准。

Method: 构建了包含5种配置的Jordan曲线图像数据集，标注为包含关系树，并采用RLVR微调方法训练Qwen3-VL-8B模型。

Result: Gemini 3.1 Pro在Easy和Hard子集上分别达到71.1%和19.1%准确率；微调后Qwen3-VL-8B在Easy上从2.8%提升至33.3%。

Conclusion: 精确的拓扑感知视觉推理仍未解决，尤其在复杂场景中表现明显不足。
...
### [109] [Venus-DeFakerOne: Unified Fake Image Detection & Localization](https://arxiv.org/abs/2605.14091)
*GuangJian Team*

Main category: cs.CV

TL;DR: 提出DeFakerOne，一个统一伪造检测与定位的基础模型，整合InternVL2和SAM2，在39个检测和9个定位基准上达到最优性能，并深入分析了数据缩放、跨域伪影迁移等原理。


<details>
  <summary>Details</summary>
Motivation: 传统伪造图像检测研究碎片化，而生成式AI使得伪造生成机制趋于统一，现有范式与统一生成机制不匹配。

Method: 数据驱动的统一FIDL基础模型，集成InternVL2（视觉语言模型）和SAM2（分割模型），实现图像级检测和像素级定位。

Result: 在39个检测基准和9个定位基准上达到领先性能，对现实扰动和GPT-Image-2等新生成器表现出鲁棒性。

Conclusion: 统一FIDL模型跨域有效，需关注细粒度监督和原始分辨率伪影保留，为可扩展、鲁棒的伪造检测提供设计原则。
...
### [110] [DUET: Dual-Paradigm Adaptive Expert Triage with Single-cell Inductive Prior for Spatial Transcriptomics Prediction](https://arxiv.org/abs/2605.14104)
...
TL;DR: 本文提出DUET框架，通过结合参数化预测和基于记忆的检索，利用单细胞参考数据提升组织学图像推断基因表达的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将空间基因表达推断简化为形态到表达的映射，忽视分子一致性；单细胞数据丰富但未被充分利用；现有单一范式方法在灵活性和准确性上存在瓶颈。

Method: DUET采用双范式框架，包括参数化回归和记忆检索两个并行通路，并通过轻量级适配器动态分配分支权重，同时利用大规模单细胞参考数据作为生物学约束。

Result: 在三个公共数据集上的实验表明，DUET达到了最先进的性能，各组件均有贡献。

Conclusion: DUET有效融合了视觉与分子信息，提升了空间基因表达推断的准确性和生物保真度。
...
### [111] [Bridging the Rural Healthcare Gap: A Cascaded Edge-Cloud Architecture for Automated Retinal Screening](https://arxiv.org/abs/2605.14108)
*Nishi Doshi,Shrey Shah*

Main category: cs.CV

TL;DR: 提出了一个双层边缘-云级联系统，在APTOS数据集上通过轻量级模型进行二分类分流，减少50.48%的云端调用，同时保持高灵敏度。


<details>
  <summary>Details</summary>
Motivation: 农村地区缺乏专家和基础设施，云端深度学习系统因高延迟、带宽有限和高传输成本难以部署。

Method: 第一层在本地设备运行MobileNetV3-small进行转诊与非转诊分类；第二层在云端运行RETFoundDINOv2对转诊图像进行严重程度分级。

Result: 第一层灵敏度98.99%，特异度84.37%；级联系统准确率80.49%，云端基线准确率80.76%，云调用减少50.48%。

Conclusion: 级联系统在性能轻微下降的情况下，大幅减少了云端使用，适用于资源受限的农村场景。
...
### [112] [SToRe3D: Sparse Token Relevance in ViTs for Efficient Multi-View 3D Object Detection](https://arxiv.org/abs/2605.14110)
*Sandro Papais,Lezhou Feng,Charles Cossette,Lingting Ge*

Main category: cs.CV

TL;DR: 提出SToRe3D框架，联合选择2D图像token和3D对象查询，实现3倍加速推理，同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: ViT在3D检测中推理延迟高，现有稀疏方法仅针对2D视觉，无法扩展到全模型稀疏或处理3D查询。

Method: SToRe3D框架：互相关2D-3D相关头分配计算，保留其他嵌入并存储特征以便重新激活。

Result: 在nuScenes和新基准上实现3倍加速，精度损失极小，可实时处理大规模ViT 3D检测。

Conclusion: SToRe3D实现了实时大规模ViT基3D检测，对关键规划目标保持高精度。
...
### [113] [ProtoMedAgent: Multimodal Clinical Interpretability via Privacy-Aware Agentic Workflows](https://arxiv.org/abs/2605.14113)
*Alvaro Lopez Pellicer,Plamen Angelov,Marwan Bukhari,Yi Li,Eduardo Soares,Jemma Kerns*

Main category: cs.CV

TL;DR: 提出ProtoMedAgent框架，通过神经符号瓶颈和反射式Scribe-Critic循环约束LLM输出，实现高忠实度的临床报告生成，同时通过k-匿名和ℓ-多样性保障隐私安全。


<details>
  <summary>Details</summary>
Motivation: 可解释原型网络缺乏医学文档所需的语义结构，而标准RAG会导致LLM产生事后合理化幻觉。

Method: 基于冻结的原型骨干网络，将潜在特征蒸馏为离散语义记忆，通过精确集合论差分和Scribe-Critic循环约束生成过程，并引入k-匿名和ℓ-多样性语义隐私门。

Result: 在4160患者队列上，Comparison Set Faithfulness达91.2%（标准RAG为46.2%），并通过ℓ-多样性相变将成员推理风险降低9.8%。

Conclusion: ProtoMedAgent在保持高生成忠实度的同时，有效降低了隐私泄露风险，为多模态临床报告生成提供了可解释且安全的解决方案。
...
### [114] [PanoPlane: Plane-Aware Panoramic Completion for Sparse-View Indoor 3D Gaussian Splatting](https://arxiv.org/abs/2605.14135)
*Adil Qureshi,Dongki Jung,Jaehoon Choi,Dinesh Manocha*

Main category: cs.CV

TL;DR: 提出PanoPlane，一种通过全景场景补全实现高保真稀疏室内新视角合成的方法，利用训练无关的注意力引导机制，在仅3-9张输入视图下达到SOTA效果，PSNR提升最高17.8%。


<details>
  <summary>Details</summary>
Motivation: 透视方法受限于有限视野，难以重建完整室内几何结构，无法高质量合成未观察区域的新视角。

Method: 利用360°全景补全条件化生成过程，提出布局锚定注意力引导，在推理时将扩散模型内注意力定向到场景中检测到的平面，实现有依据的表面外推。

Result: 在Replica等三个基准数据集上，基于3、6、9张输入视图的新视角合成质量达到SOTA，PSNR相比基线最高提升17.8%。

Conclusion: 通过全景补全和训练无关的注意力引导，方法能有效利用几何一致性信息提升稀疏视角下的新视角合成质量。
...
### [115] [TeDiO: Temporal Diagonal Optimization for Training-Free Coherent Video Diffusion](https://arxiv.org/abs/2605.14136)
*Nurislam Tursynbek,Zhiqiang Lao,Heather Yu,Gedas Bertasius,Marc Niethammer*

Main category: cs.CV

TL;DR: TeDiO是一种无需训练、仅在推理时生效的方法，通过正则化内部注意力模式中的时间对角线来增强视频扩散模型的时间一致性，显著减少闪烁、漂移和不稳定运动。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频扩散Transformer生成的帧视觉上吸引人，但在时间连贯性上存在闪烁、漂移或运动不稳定等问题。

Method: 通过观测到时间不连贯视频中自注意力图出现不规则碎片化对角线，而稳定运动对应平滑带状对角线，TeDiO估计对角线平滑度、识别不稳定区域，并进行轻量级潜在更新以促进帧间连贯动态，不修改模型权重或使用外部运动监督。

Result: 在多个视频扩散模型上（如Wan2.1, CogVideoX），TeDiO实现了显著更平滑的运动，同时保持每帧视觉质量。

Conclusion: TeDiO作为一种高效即插即用方法，能提升现代视频生成系统的动态真实感。
...
### [116] [Rethinking the Good Enough Embedding for Easy Few-Shot Learning](https://arxiv.org/abs/2605.14145)
...
TL;DR: 提出一种基于YOLO-World热图引导的轻量级U-Net，实现16×16到128×128的人脸超分辨率，无需对抗训练或额外网络。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖复杂网络或对抗训练，计算成本高；轻量模型在极端放大倍数下难以恢复精细面部细节。

Method: 使用YOLO-World检测器生成人脸关键特征热图，转化为空间权重构建热图引导损失，强调语义区域的重建误差。

Result: 在CelebA数据集上，该损失函数持续提升量化指标，生成更清晰、更逼真的重建图像。

Conclusion: 轻量网络结合检测器先验，可在无对抗训练下实现感知上令人信服的极端超分辨率。
...
### [118] [CoReDiT: Spatial Coherence-Guided Token Pruning and Reconstruction for Efficient Diffusion Transformers](https://arxiv.org/abs/2605.14191)
*Zhuojin Li,Hsin-Pai Cheng,Hong Cai,Shizhong Han,Fatih Porikli*

Main category: cs.CV

TL;DR: 提出CoReDiT，一种针对扩散Transformer的结构化令牌剪枝框架，通过空间相干性评分和注意力输出重建，在保持高质量的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer计算成本高，限制了其可扩展性和设备端部署。

Method: 利用线性时间空间相干性评分跳过冗余令牌，通过相干性引导聚合重建注意力输出，并采用渐进式块自适应剪枝策略。

Result: 在PixArt-α和MagicDrive-V2上实现高达55%自注意力FLOPs降低，云GPU加速1.33倍，移动NPU加速1.72倍，同时保持高视觉质量并支持更高分辨率生成。

Conclusion: CoReDiT有效降低计算成本，提升推理速度，并增加设备端内存余量，适合可扩展部署。
...
### [119] [Automatic Landmark-Based Segmentation of Human Subcortical Structures in MRI](https://arxiv.org/abs/2605.14221)
*Ahmed Rekik,R. Jarrett Rushmore,Sylvain Bouix,Linda Marrakchi-Kacem*

Main category: cs.CV

TL;DR: 提出了一种基于 landmark 引导的 3D 脑分割方法，通过模仿哈佛-牛津图谱的手动分割协议，将 12 个粗标签细分为 26 个精细结构，显著提升边界准确性。


<details>
  <summary>Details</summary>
Motivation: 现有 voxel-wise 深度模型分割结果与专家定义的解剖边界不一致，需要更符合手动分割协议的方法。

Method: 全局到局部网络自动检测 16 个 landmark；语义分割模型生成 12 个粗标签；landmark 驱动的后处理将粗标签拆分为 26 个精细结构。

Result: 实验证明边界一致性持续改善，集成 landmark 使分割更接近手动协议。

Conclusion: landmark 引导的 3D 分割方法能有效提升解剖一致性，使输出更符合专家标准。
...
### [120] [Implicit spatial-frequency fusion of hyperspectral and lidar data via kolmogorov-arnold networks](https://arxiv.org/abs/2605.14239)
*Zekun Long,Judy X. Yang,Jing Wang,Ali Zia,Guanyiman Fu,Jun Zhou*

Main category: cs.CV

TL;DR: 提出了基于KAN的IFGNet网络，通过可学习样条函数和LiDAR引导的空间-频率融合，在HSI-LiDAR分类中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有HSI-LiDAR融合方法依赖CNN/MLP，难以建模数据中的非线性关系和结构不连续性，且缺乏LiDAR引导的空间-频率域融合。

Method: 使用KAN的可学习样条函数捕获非线性特征，并设计了LiDAR引导的隐式聚合模块，在空间域和频率域进行融合。

Result: 在Houston 2013和MUUFL基准上，OA、AA和Kappa系数均超过现有方法，且模型高效。

Conclusion: KAN和LiDAR引导的隐式融合能有效提升HSI-LiDAR分类精度，验证了方法的有效性。
...
### [121] [Generative Deep Learning for Computational Destaining and Restaining of Unregistered Digital Pathology Images](https://arxiv.org/abs/2605.14251)
*Aarushi Kulkarni,Alarice Lowe,Pratik Shah*

Main category: cs.CV

TL;DR: cGAN模型在跨机构H&E染色和脱色任务中，通过预处理（直方图归一化、通道强度校准）适应域偏移，无需重训练即可获得合理性能，但恶性腺体形态存在偏差。


<details>
  <summary>Details</summary>
Motivation: cGAN在数字病理染色上有潜力，但跨机构泛化性未经充分测试；需在不重训练下评估其对域偏移的鲁棒性。

Method: 使用预训练的cGAN模型，对H&E染色WSI进行直方图归一化，对未染色WSI进行通道强度校准；省略图像配准以模拟真实部署。

Result: 虚拟脱色PCC=0.854, SSIM=0.699, PSNR=18.41 dB；从脱色输出重新染色优于直接染色（PCC=0.798 vs. 0.715等）；病理审查显示良性腺体保真，但恶性腺体常呈现血管样形态。

Conclusion: 仅靠预处理即可实现cGAN跨机构染色/脱色，但需针对恶性腺体形态进行特定域适应改进。
...
### [122] [Towards Real-Time Autonomous Navigation: Transformer-Based Catheter Tip Tracking in Fluoroscopy](https://arxiv.org/abs/2605.14253)
...
TL;DR: 提出了一种多线程导管尖端实时追踪管道，在低对比度、噪声和遮挡下实现高精度追踪，为基于强化学习的自主机械取栓导航提供可靠基础。


<details>
  <summary>Details</summary>
Motivation: 机械取栓改善卒中结局但受限于治疗可及性；基于强化学习的机器人系统可解决此问题，但现有方法需要实时设备尖端坐标追踪，而当前追踪技术缺乏对此类挑战的有效解决方案。

Method: 设计多线程管道（帧读取、预处理、推理、后处理）；训练和对比U-Net、U-Net+Transformer、SegFormer等深度学习分割模型（二类和三类）；后处理包括两步组件滤波、单像素骨架化及贪心弧长路径跟踪。

Result: 在中等复杂度荧光视频数据中，二类SegFormer平均绝对误差为4.44 mm，优于其他模型；分割基准测试中，Dice分数比CathAction方法最高提升+5%。

Conclusion: 该多线程追踪框架在挑战性成像条件下保持稳定性能，超越先前基准，为强化学习驱动的自主机械取栓导航提供可靠高效的基础。
...
### [123] [Image Restoration via Diffusion Models with Dynamic Resolution](https://arxiv.org/abs/2605.14267)
*Yang Zheng,Wen Li,Zhaoqiang Liu*

Main category: cs.CV

TL;DR: 本文提出基于动态分辨率扩散模型的子空间投影方法，通过将数据投影到低维子空间加速图像修复，SubDAPS和SubDAPS++在多数任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在像素空间计算开销大，基于潜在变量的方法因编解码重复推理导致效率低下，亟需降低计算复杂度。

Method: 微调预训练DM获得动态分辨率先验，将DPS和DAPS方法转化为子空间版本SubDPS和SubDAPS，并改进得到SubDAPS++。

Result: 实验表明所提方法在多种数据集和修复任务中推理速度更快、重建质量更优，超越近期DM方法。

Conclusion: 动态分辨率子空间投影能有效平衡图像修复的效率与质量，代码已开源。
...
### [124] [PhyMotion: Structured 3D Motion Reward for Physics-Grounded Human Video Generation](https://arxiv.org/abs/2605.14269)
*Yidong Huang,Zun Wang,Han Lin,Dong-Ki Kim,Shayegan Omidshafiei,Jaehong Yoon,Jaemin Cho,Yue Zhang,Mohit Bansal*

Main category: cs.CV

TL;DR: 提出了一种基于物理模拟的结构化细粒度运动奖励信号PhyMotion，用于评估视频中人体运动的物理可行性，并与RL后训练结合提升生成运动质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频奖励主要依赖2D感知信号，无法准确评估3D人体运动中的物理合理性，导致浮空或物理不合理动作被误判为高分。

Method: 从生成视频中恢复SMPL人体网格，重定向至MuJoCo模拟器的人形机器人，沿运动学合理性、接触与平衡一致性、动态可行性三个维度评估物理质量，提供连续可解释的奖励信号。

Result: PhyMotion与人类判断的相关性优于现有奖励；在RL后训练中优化该奖励，较现有方法在自动指标和盲评中均取得更大改进（+68 Elo分），并保持整体生成质量。

Conclusion: 三个轴提供互补监督信号，PhyMotion能在不显著增加训练开销的前提下，有效提升视频人体运动的物理真实性。
...
### [125] [Diagnosing and Correcting Concept Omission in Multimodal Diffusion Transformers](https://arxiv.org/abs/2605.14270)
*Kanghyun Baek,Jaihyun Lew,Chaehun Shin,Jungbeom Lee,Sungroh Yoon*

Main category: cs.CV

TL;DR: 提出了一种基于遗漏信号的干预方法，有效缓解多模态扩散Transformer在文本到图像生成中的概念遗漏问题。


<details>
  <summary>Details</summary>
Motivation: MM-DiTs在文本到图像生成中常出现概念遗漏，即指定对象或属性未在生成图像中体现。

Method: 通过线性探测识别文本嵌入中的"遗漏信号"，并设计OSI方法放大该信号以促进缺失概念的生成。

Result: 在FLUX.1-Dev和SD3.5-Medium上实验证明，OSI显著减轻了概念遗漏，即使在极端场景下也有效。

Conclusion: OSI方法能主动催化缺失概念生成，提升生成图像的忠实度。
...
### [126] [CreFlow: Corrective Reflow for Sparse-Reward Embodied Video Diffusion RL](https://arxiv.org/abs/2605.14274)
...
TL;DR: 提出在线RL框架CreFlow，利用组合约束奖励模型优化视频生成模型，提升双人操作任务成功率23.8%。


<details>
  <summary>Details</summary>
Motivation: 现有视频RL奖励依赖低层视觉指标，无法验证组合任务规范，导致生成视频违反物理约束。

Method: 组合约束奖励模型（LTL形式化任务需求）和CreFlow框架（信用感知NFT损失+校正再流损失）。

Result: 奖励判断更符合人类和模拟器标签，在8个双人操作任务中成功率提升23.8个百分点。

Conclusion: CreFlow有效优化视频生成模型，使其满足物理约束和任务规范。
...
### [127] [ICED: Concept-level Machine Unlearning via Interpretable Concept Decomposition](https://arxiv.org/abs/2605.14309)
*Shen Lin,Jing Lin,Junhao Dong,Piotr Koniusz,Li Xu*

Main category: cs.CV

TL;DR: 提出可解释的概念级遗忘框架，通过概念词汇构建和稀疏非负分解实现VLM细粒度知识移除。


<details>
  <summary>Details</summary>
Motivation: 现有实例级遗忘方法在VLM中难以精确移除目标知识，因单张图像常包含多个纠缠概念。

Method: 利用多模态大模型构建任务相关概念词汇表，将视觉表示分解为概念的稀疏非负组合，通过抑制目标概念同时保留非目标语义和跨模态知识进行优化。

Result: 在域内和域外遗忘设置中，实现了更全面的目标遗忘，更好保留同一图像中非目标知识，保持模型效用。

Conclusion: 概念级优化有效解决了VLM中多概念纠缠导致的遗忘困难问题。
...
### [128] [KVPO: ODE-Native GRPO for Autoregressive Video Alignment via KV Semantic Exploration](https://arxiv.org/abs/2605.14278)
*Ruicheng Zhang,Kaixi Cong,Jun Zhou,Zhizhou Zhong,Zunnan Xu,Shuiyang Mao,Wei Liu,Xiu Li*

Main category: cs.CV

TL;DR: 提出了KVPO框架，通过因果语义探索和速度场代理策略，在线对齐流媒体AR视频生成器与人类偏好，提升长视频连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法依赖噪声探索和SDE代理策略，与蒸馏AR模型的确定性ODE动力学不匹配，且扰动低层外观而非高层语义，难以保证长时连贯性。

Method: KVPO框架：因果语义探索利用历史KV缓存随机路由构建语义多样性分支；速度场代理策略基于轨迹速度能量量化分支似然，实现与ODE一致的奖励加权对比目标。

Result: 在多个蒸馏AR视频生成器上，单提示短视频和多提示长视频设置中的视觉质量、运动质量和文本-视频对齐均一致提升。

Conclusion: KVPO有效解决了流媒体AR生成器的偏好对齐问题，显著增强了语义连贯性和生成质量。
...
### [129] [Systematic Discovery of Semantic Attacks in Online Map Construction through Conditional Diffusion](https://arxiv.org/abs/2605.14396)
*Chenyi Wang,Ruoyu Song,Raymond Muller,Jean-Philippe Monteuuis,Jonathan Petit,Z. Berkay Celik,Ryan Gerdes,Ming F. Li*

Main category: cs.CV

TL;DR: 提出MIRAGE框架，利用扩散模型潜空间生成语义攻击，绕过现有防御，成功实现HD地图边界移除与注入攻击，且攻击在多种防御下仍有效。


<details>
  <summary>Details</summary>
Motivation: 现有像素扰动攻击可被标准对抗防御中和，需探索更隐蔽的语义级攻击。

Method: 利用扩散模型学习真实数据潜空间，搜索与真实场景邻域但误导地图预测的语义变异（如阴影、湿滑路面）。

Result: 边界移除攻击抑制57.7%检测并破坏96%规划轨迹；边界注入攻击为唯一成功方法（PGD、AdvPatch完全失败）；攻击在防御下保持效力；VLM评估真实度80-84%。

Conclusion: 语义级环境变异攻击难以被现有防御缓解，暴露了防御在语义层面的漏洞。
...
### [130] [SceneFunRI: Reasoning the Invisible for Task-Driven Functional Object Localization](https://arxiv.org/abs/2605.14704)
*Posheng Chen,Powen Cheng,Gueter Josmy Faure,Hung-Ting Su,Winston H. Hsu*

Main category: cs.CV

TL;DR: 本文提出了SceneFunRI基准测试，要求视觉语言模型从任务指令和常识中推断不可见物体的位置。当前最强模型Gemini 3 Flash在此任务上表现不佳，CAcc@75仅为15.20。


<details>
  <summary>Details</summary>
Motivation: 现实场景中目标物体可能位于不可见区域，而视觉语言模型无法像人类一样通过上下文和常识推断其位置，这一能力差距促使了本研究。

Method: 基于SceneFun3D数据集，通过半自动流程构建了包含855个实例的2D空间推理任务，并提出了三种提示分析策略：强指令提示、基于推理的提示和空间排除法。

Result: 最佳基线模型Gemini 3 Flash的CAcc@75为15.20，mIoU为0.74，Dist为28.65，表明当前VLM在不可见区域推理方面能力不足。

Conclusion: 不可见区域推理仍是当前VLM的不稳定能力，未来需要更紧密整合任务意图、常识先验、空间定位和不确定性感知搜索的模型。
...
### [131] [CoRDS: Coreset-based Representative and Diverse Selection for Streaming Video Understanding](https://arxiv.org/abs/2605.14310)
*Ailar Mahdizadeh,Puria Azadi,Muchen Li,Xiangteng He,Leonid Sigal*

Main category: cs.CV

TL;DR: 提出将KV缓存压缩视为核心集选择问题，通过联合表示和双准则目标平衡键值空间覆盖，并引入正交性多样性标准，在固定缓存预算下提升流式视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有流式方法依赖局部令牌级启发式（如近因、冗余或显著性），未显式优化保留缓存是否代表累积历史。

Method: 采用核心集选择视角，通过联合KV表示的双准则目标（覆盖键和值空间）及正交性驱动的多样性标准（连接log-行列式子集选择）进行缓存压缩。

Result: 在四个开源VLM和五个长视频/流式视频基准上，方法优于启发式流式压缩基线。

Conclusion: 代表性核心集选择比令牌级剪枝更有效，适用于内存受限的流式视频理解。
...
### [132] [TurboVGGT: Fast Visual Geometry Reconstruction with Adaptive Alternating Attention](https://arxiv.org/abs/2605.14315)
*David Huang,Guile Wu,Chengjie Huang,Bingbing Liu,Dongfeng Bai*

Main category: cs.CV

TL;DR: 提出TurboVGGT，通过自适应交替注意力机制实现快速多视图3D重建，在保持质量的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D重建方法在质量与效率间难以平衡，且缺乏自适应学习代表性令牌的机制。

Method: 采用端到端框架，包含自适应稀疏全局注意力（动态调整稀疏度）和帧内注意力，分别捕获全局关系和局部细节。

Result: 在多个3D重建基准上实现快速重建，质量与最先进方法相当。

Conclusion: TurboVGGT通过自适应交替注意力有效平衡了重建速度与质量。
...
### [133] [EARL: Towards a Unified Analysis-Guided Reinforcement Learning Framework for Egocentric Interaction Reasoning and Pixel Grounding](https://arxiv.org/abs/2605.14742)
*Yuejiao Su,Xinshen Zhang,Zhen Ye,Lei Yao,Lap-Pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: 提出EARL框架，通过两阶段解析（粗粒度解释+细粒度响应）和强化学习优化，提升第一人称视觉中的交互推理与像素级定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在第一人称视觉中交互推理和像素级定位仍不准确，缺乏有效的粗细粒度语义桥接机制。

Method: 采用两阶段解析：先全局解释生成结构化文本描述，再提取交互描述符作为语义先验，通过分析引导特征合成器（AFS）实现查询导向推理，并设计多目标奖励函数结合GRPO优化异构输出。

Result: 在Ego-IRGBench上像素定位cIoU达65.48%，比以往强化学习方法提升8.37%；在EgoHOS上的域外定位结果展示了对未见场景的强迁移性。

Conclusion: EARL有效将粗粒度交互语义迁移到查询导向的问答和定位任务，验证了语义先验与强化学习在提升第一人称视觉理解中的潜力。
...
### [134] [Evo-Depth: A Lightweight Depth-Enhanced Vision-Language-Action Model](https://arxiv.org/abs/2605.14950)
...
TL;DR: Evo-Depth是一个轻量级的深度增强VLA框架，通过隐式深度编码和空间增强模块，在无需额外传感器的情况下提升了机器人操控的空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏深度信息，而引入显式3D输入增加复杂性和成本，隐式建模依赖大型几何模型导致训练部署成本高。

Method: 使用轻量隐式深度编码模块从多视图RGB提取深度特征，通过空间增强模块的深度感知调制融合到视觉-语言表示中，并采用渐进对齐训练策略。

Result: 在四个仿真基准测试中性能优异，真实实验中以0.9B参数实现最高成功率、最小模型尺寸、最低GPU内存和最高推理频率。

Conclusion: Evo-Depth在保持部署效率的同时，有效增强了空间接地操控能力，是一种实用且高效的解决方案。
...
### [135] [D2-CDIG: Controlled Diffusion Remote Sensing Image Generation with Dual Priors of DEM and Cloud-Fog](https://arxiv.org/abs/2605.14326)
*Zuopeng Zhao,Ying Liu,Kanyaphakphachsorn Pharksuwan,Su Luo,Xiaoyu Li,Maocai Ning*

Main category: cs.CV

TL;DR: 提出一种基于扩散模型与双先验控制机制的遥感图像生成框架D2-CDIG，利用DEM和云雾信息实现地形与大气现象的精确控制，提升图像质量与真实感。


<details>
  <summary>Details</summary>
Motivation: 现有可控遥感图像生成方法依赖分割等传统技术，未充分利用地形或大气条件，导致复杂地形和气象场景下生成图像缺乏准确性和自然性。

Method: 集成扩散模型与双先验控制机制，通过独立的地形分支和大气分支分别处理DEM和云-雾信息，并引入云-雾滑块调节云层厚度与分布，在训练中分层注入控制信号。

Result: 与传统基于分割或边缘检测的方法相比，D2-CDIG在图像质量、细节丰富度和真实感上显著提升。

Conclusion: D2-CDIG为遥感图像生成提供了灵活精确的解决方案，可为遥感大模型训练和下游任务提供高质量数据。
...
### [136] [InsightTok: Improving Text and Face Fidelity in Discrete Tokenization for Autoregressive Image Generation](https://arxiv.org/abs/2605.14333)
...
TL;DR: 提出InsightTok框架，通过局部内容感知损失提升离散视觉分词器对文本和面部细节的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有离散分词器过于注重通用重建，对文本可读性和面部保真度支持不足。

Method: 在分词器训练中引入局部化、内容感知的感知损失，保持16倍下采样和16k码本。

Result: 在文本和人脸重建上显著优于现有分词器，且不降低通用重建质量；在自回归生成模型InsightAR中也带来更清晰的文本和面部细节。

Conclusion: 在分词器训练中加入专门监督有助于提升离散图像生成的效果。
...
### [137] [IG-Diff: Complex Night Scene Restoration with Illumination-Guided Diffusion Model](https://arxiv.org/abs/2605.14337)
*Yifan Chen,Fei Yin,Chunle Guo,Chongyi Li,Yujiu Yang*

Main category: cs.CV

TL;DR: 提出了一种集成光照引导模块的扩散模型，用于应对夜间场景中同时存在的多种退化问题，并贡献了模拟低光与其他退化共存的数据集。


<details>
  <summary>Details</summary>
Motivation: 现有图像恢复方法难以处理夜间复杂场景（如低光与天气退化共存），且缺乏相应的配对数据。

Method: 构建了模拟低光与其他退化共存的夜间数据集，提出将光照引导模块嵌入扩散模型以指导光照恢复过程。

Result: 模型能在低光场景下保持纹理保真度，有效应对多种退化挑战。

Conclusion: 所提方法为复杂夜间场景的端到端恢复提供了有效解决方案。
...
### [138] [Articraft: An Agentic System for Scalable Articulated 3D Asset Generation](https://arxiv.org/abs/2605.15187)
...
TL;DR: 提出Articraft系统，利用大语言模型通过程序合成方式自动生成高质量铰接式3D资产，并构建包含10K+资产的数据集，验证其在机器人仿真等场景的实用性。


<details>
  <summary>Details</summary>
Motivation: 缺乏大规模、多样化的铰接式3D物体数据集，限制了对这类物体的理解和学习。

Method: 将铰接式3D资产生成问题转化为程序编写问题，设计Articraft智能体系统。系统包含领域专用SDK（定义部件、组合几何、指定关节）和测试验证框架，大语言模型在受控工作空间中生成代码，系统自动验证并返回结构化反馈。

Result: 生成的资产质量优于现有铰接式资产生成器和通用编码智能体；构建了Articraft-10K数据集（10K+资产，245类别），并在机器人仿真和虚拟现实等下游任务中验证了其有效性。

Conclusion: 基于大语言模型的程序合成方法可高效生成高质量铰接式3D资产，缓解数据稀缺问题，并支持多领域应用。
...
### [139] [AnyBand-Diff: A Unified Remote Sensing Image Generation and Band Repair Framework with Spectral Priors](https://arxiv.org/abs/2605.14341)
*Zuopeng Zhao,Ying Liu,Xiaoyu Li,Su Luo,Lu Li,Wenwen Liu*

Main category: cs.CV

TL;DR: 提出AnyBand-Diff，一种光谱先验引导的扩散框架，用于遥感影像的物理一致光谱重建。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型直接用于遥感影像时忽略物理规律，导致光谱失真和辐射不一致，限制了生成数据科学实用性。

Method: 采用掩码条件扩散骨干结合双随机掩码策略，恢复任意波段子集光谱信息；通过物理引导采样利用可微物理模型梯度调整去噪轨迹；设计多尺度物理损失在像素、区域和全局层面施加约束。

Result: 实验证实AnyBand-Diff能生成可靠影像并实现准确光谱重建。
...
### [140] [Learning with Semantic Priors: Stabilizing Point-Supervised Infrared Small Target Detection via Hierarchical Knowledge Distillation](https://arxiv.org/abs/2605.14346)
*Yuanhang Yao,Ping Qian,Zhu Liu,Long Ma,Weimin Wang*

Main category: cs.CV

TL;DR: 提出一种基于视觉基础模型层次化知识蒸馏框架，通过双层优化和语义调制解决点监督红外小目标检测中伪标签噪声和训练不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 点监督标签成本低但轻量CNN检测器语义不足，导致伪掩码噪声大、优化不稳定。

Method: 双层优化（内环适应VFM教师、外环传递验证知识给学生）、SCAM语义调制、动态协作学习与聚类级样本重加权。

Result: 在多个ISTD骨干网络上检测精度和训练稳定性一致提升。

Conclusion: 层次化VFM知识蒸馏有效利用冻结的视觉基础模型，提升点监督ISTD性能。
...
### [141] [Delta Forcing: Trust Region Steering for Interactive Autoregressive Video Generation](https://arxiv.org/abs/2605.14382)
*Yuheng Wu,Xiangbo Gao,Tianhao Chen,Xinghao Chen,Qing Yin,Zhengzhong Tu,Dongman Lee*

Main category: cs.CV

TL;DR: 提出Delta Forcing框架，通过自适应信任区域约束教师模型的不可靠监督，平衡反应速度与长期稳定性，显著提升视频生成的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频生成模型在应对动态事件时，存在条件偏差导致的条件切换后持续性漂移问题，需平衡反应性与稳定性。

Method: 借鉴Trust Region Policy Optimization，通过教师与生成器轨迹的潜在差异估计过渡一致性，用单调连续性目标约束教师监督。

Result: Delta Forcing在保持事件响应性的同时，显著提升了生成视频的时序一致性。

Conclusion: Delta Forcing能有效抑制条件切换后漂移，为交互式实时视频生成提供可靠的平衡方案。
...
### [142] [Dual-Latent Collaborative Decoding for Fidelity-Perception Balanced Image Compression](https://arxiv.org/abs/2605.14391)
*Qi Mao,Zijian Wang,Zhengxue Cheng,Lingyu Zhu,Siwei Ma*

Main category: cs.CV

TL;DR: 本文提出MoDE框架，通过双潜在协作解码实现宽比特率范围内的保真度-感知平衡图像压缩。


<details>
  <summary>Details</summary>
Motivation: 现有学习的图像压缩方法依赖单一潜在表示同时承担结构和感知信息，导致低比特率下细节丢失或保真度不足。

Method: MoDE融合标量量化保真专家和向量量化感知专家，通过专家特定增强和跨专家调制实现协作解码。

Result: 实验表明MoDE在宽比特率范围内优于失真导向、感知导向、生成式和双潜在基线，实现更好的保真度-感知平衡。

Conclusion: 解码端专家协作是平衡保真度和感知质量的有效设计，适合宽范围图像压缩。
...
### [143] [Analogical Trajectory Transfer](https://arxiv.org/abs/2605.14393)
*Junho Kim,Eun Sun Lee,Gwangtak Bae,Seunggu Kang,Young Min Kim*

Main category: cs.CV

TL;DR: 提出了一种无需训练的快速模拟轨迹迁移方法，通过分解场景、层次化映射和组合优化，实现不同3D场景间语义和空间一致的轨迹迁移。


<details>
  <summary>Details</summary>
Motivation: 现有方法在语义相似但布局不同的场景间直接迁移轨迹会导致碰撞或几何形变，且映射搜索空间大，难以同时保持语义和空间连贯性。

Method: 将场景分解为以对象为中心的聚类，利用3D基础模型特征预测层次化平滑映射，组合各聚类映射得到初始迁移结果，再通过优化消除碰撞和形变。

Result: 运行时间约0.6秒，在虚拟共存、多轨迹迁移、相机迁移和人到机器人运动迁移等任务中优于基于LLM、VLM和场景图匹配的基线方法。

Conclusion: 该方法可广泛用于AR/VR和机器人领域，实现高效的模拟空间推理。
...
### [144] [SceneForge: Structured World Supervision from 3D Interventions](https://arxiv.org/abs/2605.14399)
*Jizhizi Li,Jiayang Ao,Danny Wicks,Petru-Daniel Tudosiu*

Main category: cs.CV

TL;DR: SceneForge是一个基于可编辑3D世界状态生成结构化监督数据的框架，通过干预驱动的方式产生一致的跨模态监督信号，在物体移除和场景移除任务上提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有观测级数据集缺乏对底层场景状态及干预传播的显式建模，难以提供跨编辑、视角和场景级干预的一致性监督。

Method: 构建持久化世界模型，包含语义、几何和物理依赖；施加显式干预（如物体移除、相机变化）并通过场景依赖传播效果，渲染出结构化监督数据。使用Infinigen和Blender实现，生成超2000个场景的大量配对数据和多视图标注。

Result: 在匹配训练预算下，SceneForge生成的监督数据在物体移除和场景移除的多个基准上均提升了定量和定性表现。
...
### [145] [DermAgent: A Self-Reflective Agentic System for Dermatological Image Analysis with Multi-Tool Reasoning and Traceable Decision-Making](https://arxiv.org/abs/2605.14403)
*Yize Liu,Siyuan Yan,Ming Hu,Lie Ju,Xieji Li,Feilong Tang,Wei Feng,Zongyuan Ge*

Main category: cs.CV

TL;DR: DermAgent是一个用于皮肤科诊断的多工具协作智能体，通过计划-执行-反思框架整合视觉和语言模块，在多项任务上超越GPT-4o等模型。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在皮肤科应用受限于领域知识不足和幻觉问题。

Method: 提出DermAgent，包含视觉感知、双模态检索（413,210张影像和3,199条指南）和确定性评论模块，通过三阶段框架进行逐步推理和自纠正。

Result: 在五个基准上，零样本诊断准确率比GPT-4o高17.6%，字幕ROUGE-L高3.15%。

Conclusion: DermAgent通过多工具协作和证据锚定，有效提升了皮肤科诊断的准确性和可信度。
...
### [146] [Think When Needed: Adaptive Reasoning-Driven Multimodal Embeddings with a Dual-LoRA Architecture](https://arxiv.org/abs/2605.14448)
*Longxiang Zhang,Weilong Dai,Guanghao Zhang,Hao Jiang,Pipei Huang*

Main category: cs.CV

TL;DR: 提出TWN框架，通过自适应推理机制在保持高效的同时提升多模态嵌入质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法CoT推理虽提升检索质量，但模型参数大、推理成本高，且对简单输入冗余推理会降低性能。

Method: 使用共享冻结骨干网络附加双LoRA适配器（推理和嵌入），梯度分离避免冲突；自监督路由门控决定是否生成CoT；嵌入引导强化学习优化CoT质量。

Result: 在MMEB-V2的78个任务上达到最先进嵌入质量，参数仅增加3-5%，推理令牌减少50%。

Conclusion: TWN以较小参数和推理开销实现更优嵌入性能，自适应机制有效提升效率与质量。
...
### [147] [ClickRemoval: An Interactive Open-Source Tool for Object Removal in Diffusion Models](https://arxiv.org/abs/2605.14461)
*Ledun Zhang,Yatu Ji,Xufei Zhuang,Xinying Yao*

Main category: cs.CV

TL;DR: 提出ClickRemoval，一个基于预训练Stable Diffusion的交互式物体移除工具，仅需用户点击即可定位并移除物体并修复背景，无需掩码或文本提示。
...
Method: 利用预训练Stable Diffusion模型，通过去噪过程中的自注意力调制定位目标物体并修复背景，无需额外训练或人工掩码。

Result: 在定量指标和用户研究中展现出与现有工具相当的效果，并开源了完整代码包。

Conclusion: ClickRemoval通过简单的点击交互即可实现高效、自然的物体移除，降低了使用门槛并缩减了用户工作流。
...
### [148] [Real2Sim in HOI: Toward Physically Plausible HOI Reconstruction from Monocular Videos](https://arxiv.org/abs/2605.14462)
*Yubo Zhao,Yujin Chai,Yunao Dong,Chengfeng Zhao,Zijiao Zeng,Yuan Liu,Chi-Keung Tang*

Main category: cs.CV

TL;DR: 提出了 HA-HOI 框架，从单目视频重建物理上可信的 4D 人-物交互动画，采用“人优先、物跟随”策略，并利用物理仿真提升交互稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法重建的人-物交互轨迹在用作仿真参考时缺乏稳定接触、功能性操作和物理合理性，存在交互鸿沟。

Method: 以人体运动为交互锚点，将物体相对于人体进行重建、对齐和优化；然后将运动轨迹投影到基于物理的人-物仿真中，作为教师轨迹实现稳定物理滚动。

Result: 在基准测试和真实视频上，相比现有方法，HA-HOI 在人-物对齐、接触一致性、时间稳定性和仿真就绪度方面均有提升。

Conclusion: 超越了视觉上合理的轨迹恢复，实现了物理可解释的交互动画，为将单目 HOI 视频转化为人类-机器人行为示教提供了新思路。
...
### [149] [GeoVista: Visually Grounded Active Perception for Ultra-High-Resolution Remote Sensing Understanding](https://arxiv.org/abs/2605.14475)
...
TL;DR: GeoVista是一个基于规划驱动的主动感知框架，用于超高分遥感影像解译，通过全局规划、分支局部验证和跨区域证据聚合，在三个基准上实现了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉语言模型采用单路径探索（单次聚焦或单序列轨迹），容易丢失全局上下文、遗漏分散区域或重复计数证据，需要更高效的解译方法。

Method: 提出GeoVista框架，包括APEX-GRO语料库（将多样UHR任务统一为全局-区域-对象推理过程）、Observe-Plan-Track机制（全局观察、自适应区域检查和证据追踪）以及GRPO对齐策略（基于步骤奖励的规划、定位和答案正确性优化）。

Result: 在RSHR-Bench、XLRS-Bench和LRS-VQA基准上取得最先进性能。

Conclusion: GeoVista通过规划驱动的多路径探索和证据去重，显著提升了超高分遥感影像的解译能力。
...
### [150] [Reduce the Artifacts Bias for More Generalizable AI-Generated Image Detection](https://arxiv.org/abs/2605.14486)
*Yiheng Li,Yang Yang,Zichang Tan,Gao Li,Zhen Lei,Wenhao Wang*

Main category: cs.CV

TL;DR: 提出了一种基于GAN上采样和独立专家融合框架的通用AI生成图像检测方法


<details>
  <summary>Details</summary>
Motivation: 现有基于重建的检测方法无法充分捕捉GAN等多样化生成模式的伪造痕迹

Method: 设计GAN上采样方法生成对齐的伪造图像，并通过独立专家融合框架分离域间干扰，结合LoRA适配和门控网络

Result: 在13个基准测试中表现优异

Conclusion: 该方法通过引入互补伪造模式提高了检测泛化能力
...
### [151] [Head Forcing: Long Autoregressive Video Generation via Head Heterogeneity](https://arxiv.org/abs/2605.14487)
*Jiahao Tian,Yiwei Wang,Gang Yu,Chi Zhang*

Main category: cs.CV

TL;DR: 提出Head Forcing框架，针对自回归视频扩散模型中的注意力头进行功能分类（局部、锚点、记忆头），并分配定制化KV缓存策略，无需额外训练即可将生成时长从5秒扩展到分钟级别，支持多提示交互合成。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频扩散模型在长程生成中存在错误累积和上下文丢失问题，而注意力头功能未被差异化利用，导致KV缓存分配次优。

Method: 训练无关的Head Forcing框架：为局部头和锚点头保留关键token，记忆头采用分层记忆系统与动态情节更新，配合头部级RoPE重新编码。

Result: 无需额外训练，将生成时长从5秒扩展到分钟级，支持多提示交互合成，性能优于现有基线。

Conclusion: 通过差异化处理注意力头功能，可有效增强长程视频生成的连贯性与效率。
...
### [152] [HASTE: Training-Free Video Diffusion Acceleration via Head-Wise Adaptive Sparse Attention](https://arxiv.org/abs/2605.14513)
*Xuzhe Zheng,Yuexiao Ma,Jing Xu,Xiawu Zheng,Rongrong Ji,Fei Chao*

Main category: cs.CV

TL;DR: 提出一种面向视频扩散Transformer的免训练稀疏注意力加速框架，通过头自适应阈值与掩码复用机制，在保持视频质量的同时实现最高1.93倍加速。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型的全注意力计算复杂度高，现有免训练稀疏注意力方法存在掩码预测开销大且忽略头级差异性的问题，限制了实际加速效果。

Method: 提出头自适应框架，包含时序掩码复用（根据查询-键漂移跳过不必要的掩码预测）和误差引导预算校准（基于模型输出误差分配各头的top-p阈值）两个插件组件。

Result: 在Wan2.1-1.3B和Wan2.1-14B模型上，相比XAttention和SVG2基线方法，在720P分辨率下实现最高1.93倍加速，同时保持竞争性的视频质量和相似度指标。

Conclusion: 所提方法有效解决了免训练稀疏注意力中的掩码计算冗余和头差异性问题，为视频扩散模型的高效部署提供了实用方案。
...
### [153] [ArcGate: Adaptive Arctangent Gated Activation](https://arxiv.org/abs/2605.14518)
*Avik Bhattacharya,Siddhant Dnyanesh Gole,Subhasis Chaudhuri,Alejandro C. Frery,Biplab Banerjee*

Main category: cs.CV

TL;DR: 提出一种自适应Arctangent Gated Activation (ArcGate) 函数，通过七个可学习参数生成多样化激活形状，在遥感分类任务中优于ReLU等固定激活函数，并在噪声环境下保持高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统固定形状激活函数（如ReLU、GELU、SiLU）无法根据数据分布和特征层级自适应调整非线性，限制深度网络在复杂遥感任务中的表现。

Method: ArcGate使用三层非线性变换，每层引入七个可学习参数，允许网络自主优化激活函数形状。

Result: 在PatternNet上达到99.67%准确率；在中等高斯噪声下（标准差0.1）性能领先ReLU 26.65%。

Conclusion: ArcGate作为自适应通用激活函数，在高分辨率地球观测任务中兼具高性能与鲁棒性，且参数学习表现出深度依赖的功能演化规律。
...
### [154] [From Sparse to Dense: Spatio-Temporal Fusion for Multi-View 3D Human Pose Estimation with DenseWarper](https://arxiv.org/abs/2605.14525)
*Ling Li,Changjie Chen,Yuyan Wang,Jiaqing Lyu,Kenglun Chang,Yiyun Chen,Zhidong Deng*

Main category: cs.CV

TL;DR: 提出稀疏交错输入方法，利用多视角图像在不同时间点的特征，结合DenseWarper模型提升3D人体姿态估计的时空精度与帧率。


<details>
  <summary>Details</summary>
Motivation: 传统多视角方法忽略帧间时序信息，且受限于单视角帧率，难以同时获取高时空分辨率。

Method: 稀疏交错输入：在不同时间点交错采集不同视角图像（如视角1在t时刻，视角2在t+δ时刻），由DenseWarper模型基于对极几何进行时空热力图交换。

Result: 在Human3.6M和MPI-INF-3DHP上超越传统密集多视角方法，达到SOTA性能。

Conclusion: 稀疏交错输入能有效利用多视角时序信息，减少数据冗余并提升输出帧率，是3D姿态估计的高效新范式。
...
### [155] [Mitigating Mask Prior Drift and Positional Attention Collapse in Large Diffusion Vision-Language Models](https://arxiv.org/abs/2605.14530)
*Sujung Hong,Chanyong Yoon,Seongjae Hwang*

Main category: cs.CV

TL;DR: 本文揭示了大扩散视觉语言模型在长文本生成中的重复与视觉定位退化问题，并提出无需训练的解码改进策略。


<details>
  <summary>Details</summary>
Motivation: 现有LDVLMs在长文本生成中的行为未被充分探索，存在重复生成和视觉定位退化问题。

Method: 提出Mask Prior Suppression和Monotonic RoPE Scaling两种无训练方法，分别抑制掩码先验漂移和位置注意力坍塌。

Result: 在通用多模态基准和视觉定位任务上优于基线，长文本描述基准提升明显。

Conclusion: 通过轻量级即插即用策略可有效解决这些缺陷，无需额外训练且具有跨架构通用性。
...
### [156] [PROVE: A Perceptual RemOVal cohErence Benchmark for Visual Media](https://arxiv.org/abs/2605.14534)
*Fuhao Li,Shaofeng You,Jiagao Hu,Yu Liu,Yuxuan Chen,Zepeng Wang,Fei Wang,Daiguo Zhou,Jian Luan*

Main category: cs.CV

TL;DR: 提出了感知一致的去除连贯性度量RC（包括空间RC-S和时间RC-T），以及PROVE-Bench基准数据集，用于评估图像/视频中的物体移除效果。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标与人类感知不一致，例如全参考指标偏好复制粘贴而非真实擦除，无参考指标存在系统性偏差，全局时间指标对局部编辑不敏感。

Method: RC-S通过滑动窗口比较掩膜区域与背景区域的特征来测量空间连贯性；RC-T通过相邻帧共享修复区域的分布追踪来测量时间连贯性。PROVE-Bench包含PROVE-M和PROVE-H两个层次。

Result: 在多种图像和视频基准上，RC指标与人类判断的对齐程度显著优于现有评估协议。

Conclusion: RC度量和PROVE-Bench共同构成了感知评估框架PROVE，能更准确地反映人类对物体移除效果的感知。
...
### [157] [Local Spatiotemporal Convolutional Network for Robust Gait Recognition](https://arxiv.org/abs/2605.14548)
*Xiaoyun Wang,Cunrong Li,Wu Wang*

Main category: cs.CV

TL;DR: 提出LSTCN，通过全局双向空间池化和局部时空卷积层，将2D卷积扩展为高效提取时空特征的步态识别方法。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别依赖静态外观特征或复杂时序模型，计算成本高且难以捕捉内在运动模式。

Method: 设计双分支架构LSTCN，包含全局双向空间池化（将特征分解为水平/垂直条带）和局部时空卷积层（联合处理时空维度），并引入非对称卷积核增强表征。

Result: 在步态识别任务上，LSTCN以简单结构达到与复杂模型相当或更优的性能，显著降低计算开销。

Conclusion: LSTCN通过将时序信息融入2D卷积，有效平衡了步态识别的准确性和计算效率。
...
### [158] [SpectraFlow: Unifying Structural Pretraining and Frequency Adaptation for Medical Image Segmentation](https://arxiv.org/abs/2605.14566)
*Zhiquan Chen,Haitao Wang,Guowei Zou,Hejun Wu*

Main category: cs.CV

TL;DR: 提出了一个两阶段框架，通过结构感知预训练和边界导向解码，解决低数据下的医学图像分割问题。


<details>
  <summary>Details</summary>
Motivation: 现有自监督预训练存在纹理偏见，而分割需要几何和边界信息，且在标注少时泛化差、边界模糊。

Method: 第一阶段提出混合域均值流预训练，利用掩码作为结构指导进行潜空间传输回归，并加入分散损失防止表示坍缩；第二阶段结合直接注意融合和频域动态卷积进行边界细化。

Result: 在ISIC-2016、Kvasir-SEG和GlaS数据集上，相比现有方法有持续提升，尤其在低数据和边界清晰度方面。

Conclusion: 该框架能有效学习结构感知表示并提升边界质量，适用于低数据场景。
...
### [159] [LiWi: Layering in the Wild](https://arxiv.org/abs/2605.14552)
*Yu He,Fang Li,Haoyang Tong,Lichen Ma,Xinyuan Shan,Jingling Fu,Dong Chen,Luohang Liu,Junshi Huang,Yan Li*

Main category: cs.CV

TL;DR: 提出了一种用于自然图像分解的新框架，包括自动生成分层数据的ADD流程和高质量数据集LiWi-100k，以及联合优化光度保真度和alpha边界的训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型主要限于图形设计领域，自然图像的分层分解（如光照效果和结构边界）因缺乏可扩展数据和对象交互建模而未被充分探索。

Method: 采用Agent驱动的数据分解（ADD）管道自动合成分层数据，构建包含10万张图像的LiWi-100k数据集；设计阴影引导学习和退化-恢复目标来分别建模光照效应和修正边界。

Result: 在RGB L1和Alpha IoU指标上达到最先进性能，优于现有模型。

Conclusion: 框架有效解决了自然图像分层分解中的关键瓶颈，支持高质量精细编辑。
...
### [160] [Bridging Brain and Semantics: A Hierarchical Framework for Semantically Enhanced fMRI-to-Video Reconstruction](https://arxiv.org/abs/2605.14569)
*Yujie Wei,Chenglong Ma,Jianxiong Gao,Chenhui Wang,Shiwei Zhang,Biao Gong,Shuai Tan,Hangjie Yuan,Hongming Shan*

Main category: cs.CV

TL;DR: 本文提出CineNeuron框架，通过双阶段层次结构（自底向上语义丰富和自顶向下记忆整合）从fMRI信号重建视频，显著提升了重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有fMRI到视频重建方法因语义嵌入不完备（缺乏视频特定线索如动作，且未整合先验知识）而受限于噪声信号与丰富视频内容之间的语义鸿沟。

Method: 提出CineNeuron框架，包含两个阶段：1）自底向上语义丰富：将fMRI信号映射到涵盖文本、图像、动作和物体类别的丰富嵌入空间；2）自顶向下记忆整合：利用Mixture-of-Memories方法从历史数据中动态选择相关记忆并融合以优化重建。

Result: 在两个fMRI-to-video基准上，CineNeuron在多种指标上超越现有最优方法。

Conclusion: CineNeuron通过模拟人脑双通路处理机制，有效弥合fMRI与视频之间的语义鸿沟，为视频重建提供了新框架。
...
### [161] [Med-DisSeg: Dispersion-Driven Representation Learning for Fine-Grained Medical Image Segmentation](https://arxiv.org/abs/2605.14579)
*Zhiquan Chen,Haitao Wang,Guowei Zou,Hejun Wu*

Main category: cs.CV

TL;DR: 提出了一种结合分散损失和自适应注意力的医学图像分割框架，在多种数据集上达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法在异质性外观、模糊边界和大解剖变异下分割不准确，归因于编码表示坍塌和缺乏细粒度多尺度解码。

Method: 提出Med DisSeg框架，包含轻量级分散损失（扩大样本间边界）和自适应多尺度校准解码器。

Result: 在五个数据集、三种模态上达到一致的最优性能，并在多器官CT分割中表现鲁棒。

Conclusion: 该方法有效提升表示学习和解剖分割质量，具有跨任务适用性。
...
### [162] [A Picture is Worth a Thousand Words? An Empirical Study of Aggregation Strategies for Visual Financial Document Retrieval](https://arxiv.org/abs/2605.14581)
*Ho Hung Lim,Yi Yang*

Main category: cs.CV

TL;DR: 该研究指出，视觉RAG将文档作为图像处理，并使用视觉编码器生成视觉补丁令牌，但单向量聚合在金融文档中会丢失关键信息，导致不同文档的向量几乎相同。


<details>
  <summary>Details</summary>
Motivation: 视觉RAG处理文档时，每个文档生成数百个补丁令牌，在向量数据库中造成检索和存储挑战，实际部署需要聚合为单一向量，但金融文档中单个数字的变化可能导致重大语义变化，因此需要验证聚合是否信息丢失。

Method: 构建了一个使用金融文档的诊断基准，其中单个数字的变化会引起显著语义变化，通过实验比较补丁级别和单向量聚合的检索效果，并分析全局纹理主导性。

Result: 单向量聚合导致不同文档的向量几乎相同；补丁级别能检测到语义变化，而聚合掩盖了这些细节；全局纹理主导性是根本原因，且结果在模型规模、检索优化嵌入和多缓解策略下一致。
...
### [163] [FedStain: Modeling Higher-Order Stain Statistics for Federated Domain Generalization in Computational Pathology](https://arxiv.org/abs/2605.14590)
*Fengyi Zhang,Junya Zhang,Wenzhuo Sun*

Main category: cs.CV

TL;DR: 提出FedStain，利用高阶染色统计量（偏度和峰度）实现联邦域泛化，解决计算病理学中跨机构染色变异问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法仅依赖低阶统计量，无法捕捉真实染色过程的非高斯（不对称、重尾）分布特征。

Method: 在联邦优化中交换高阶染色矩（偏度、峰度），并采用对比跨站点参数聚合策略，无需传输像素级数据。

Result: 在Camelyon17和MvMidog-Fed基准上，准确率比现有方法提升最高+3.9%。

Conclusion: FedStain是首个显式建模高阶染色统计的联邦域泛化方法，支持隐私保护下的稳健跨机构部署。
...
### [164] [Towards Accurate Single Panoramic 3D Detection: A Semantic Gaussian Centric Approach](https://arxiv.org/abs/2605.14601)
*Kanglin Ning,Yiran Zhao,Wenrui Li,Shaoru Sun,Xingtao Wang,Xiaopeng Fan*

Main category: cs.CV

TL;DR: 提出基于连续语义3D高斯表示的PanoGSDet框架，实现单目全景图像中的3D目标检测，在Structured3D数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法将2D特征投影到离散3D网格会破坏几何连续性并限制表示效率。

Method: 包含全景深度估计组件和语义高斯组件，后者包括语义高斯提升模块、优化模块和引导预测头。

Result: 在Structured3D数据集上显著优于现有方法。

Conclusion: PanoGSDet通过连续语义3D高斯表示有效提升了全景3D检测性能。
...
### [165] [TOPOS: High-Fidelity and Efficient Industry-Grade 3D Head Generation](https://arxiv.org/abs/2605.14594)
*Bojun Xiong,Zoubin Bi,Xinghui Peng,Yunmu Wang,Junchen Deng,Jun Liang,Jing Li,Bowen Cai,Huan Fu*

Main category: cs.CV

TL;DR: 提出TOPOS框架，从单张图像生成具有固定参考拓扑的高保真3D头部模型，支持顶点级语义对应和工业级动画。


<details>
  <summary>Details</summary>
Motivation: 现有方法生成的网格拓扑不一致，阻碍了顶点语义对应和资产级重用，无法满足电影、游戏等行业的标准化拓扑需求。

Method: 设计TOPOS-VAE变分自编码器，利用Perceiver Resampler将不同拓扑的点云转换为目标拓扑；训练TOPOS-DiT整流流Transformer从单图生成网格；微调多模态图像生成模型得到可重光照的UV纹理。

Result: 在3D头部生成任务上达到最先进性能，超越经典面部重建方法和通用3D生成模型。

Conclusion: TOPOS通过固定拓扑生成实现了高效的数字人创建，适用于工业管线。
...
### [166] [VMU-Diff: A Coarse-to-fine Multi-source Data Fusion Framework for Precipitation Nowcasting](https://arxiv.org/abs/2605.14597)
*Chunlei Shi,Hao Li,Yufeng Zhu,Boyu Liu,Yongchao Feng,Zengliang Zang,Hongbin Wang,Yanlan Yang,Dan Niu*

Main category: cs.CV

TL;DR: 提出了一种基于Vision Mamba Unet和残差扩散（VMU-Diff）的粗到细降水临近预报框架，通过确定性模型和概率模型的两阶段处理提升预报精度和细节。


<details>
  <summary>Details</summary>
Motivation: 现有单源雷达数据的确定性模型存在模糊问题，单一概率模型（如扩散模型）导致伪影和低效，亟需结合两者优势的方法。

Method: 粗阶段：融合雷达和多波段卫星数据，利用时空注意力块和Vision Mamba状态空间块预测全局运动趋势；细阶段：基于残差条件扩散模型，通过时空残差特征和Mamba模块重建细节。

Result: 在江苏SWAN数据集上超越了现有方法，尤其在短期预报中表现优异。

Conclusion: VMU-Diff框架有效结合多源数据与两阶段建模，显著提升了降水临近预报的准确性和细节生成能力。
...
### [167] [Do We Really Need External Tools to Mitigate Hallucinations? SIRA: Shared-Prefix Internal Reconstruction of Attribution](https://arxiv.org/abs/2605.14621)
*Tian Qin,Junzhe Chen,Yuqing Shi,Tianshu Zhang,Qiang Ju,Lijie Wen*

Main category: cs.CV

TL;DR: 提出SIRA，一种无训练的内部对比解码框架，通过在LVLM内部构建反事实参考来减少幻觉，无需外部扰动或额外前向传播。


<details>
  <summary>Details</summary>
Motivation: 现有对比解码方法依赖外部扰动输入，可能引入伪影且计算成本高，需更高效、无训练的内部对比策略。

Method: 利用多模态Transformer的分阶段信息流，先在初始层对齐多模态状态，然后在后层掩蔽图像注意力分支，生成仅依赖语言先验的内部参考，进行token级对比解码。

Result: 在POPE、CHAIR、AMBER数据集上，使用Qwen2.5-VL和LLaVA-v1.5模型，SIRA一致减少幻觉，保持描述覆盖率，且开销低于两遍对比解码。

Conclusion: SIRA是一种无需训练、外部验证器或扰动输入的有效幻觉缓解方法，适用于白盒开放权重LVLM。
...
### [168] [MambaRain: Multi-Scale Mamba-Attention Framework for 0-3 Hour Precipitation Nowcasting](https://arxiv.org/abs/2605.14606)
...
TL;DR: 提出MambaRain，一种结合Mamba线性复杂度长程时间建模与自注意力空间建模的多尺度编解码架构，将确定性降水临近预报有效扩展至0-3小时，减少模糊伪影。


<details>
  <summary>Details</summary>
Motivation: 现有确定性方法在0-2小时外性能严重下降，难以捕捉长程时空依赖。

Method: 多尺度编码器-解码器架构，Mamba模块利用选择性状态空间机制建模全局时间动态，自注意力模块显式捕捉空间相关性，并引入频谱损失缓解模糊。

Result: 在0-3小时临近预报任务中显著优于现有方法，尤其在2-3小时挑战区间表现突出。

Conclusion: MambaRain通过混合时空建模和频谱损失，有效延长了确定性降水预报的准确时效。
...
### [169] [Deep Image Segmentation via Discriminant Feature Learning](https://arxiv.org/abs/2605.14609)
*Adam Dawid Sztamborski,Raül Pérez-Gonzalo,Antonio Agudo*

Main category: cs.CV

TL;DR: 提出深度判别分析(DDA)损失函数，通过最大化类间方差、最小化类内方差提升图像分割的边界清晰度和模型置信度，无需增加推理成本。


<details>
  <summary>Details</summary>
Motivation: 标准损失函数（如交叉熵、Dice）忽视特征判别结构，导致分割边界不准确。

Method: 设计可微分的、与架构无关的深度判别分析损失函数，将经典判别分析原理嵌入网络训练。

Result: 在DIS5K基准上，DDA一致提升分割精度、边界清晰度和模型置信度，兼容多种架构。

Conclusion: 集成判别分析为构建鲁棒分割模型提供了简单有效的路径。
...
### [170] [ViMU: Benchmarking Video Metaphorical Understanding](https://arxiv.org/abs/2605.14607)
*Qi Li,Xinchao Wang*

Main category: cs.CV

TL;DR: 提出了 ViMU，首个系统评估视频理解模型暗语能力的基准，包含无提示的多选题和开放式问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解模型主要关注文字层面的视觉识别，缺乏对视频中隐喻、讽刺和社会含义的系统性理解。

Method: 构建 ViMU 基准，通过无提示的多选题和开放式问题，评估模型结合多模态证据推断视频暗语的能力。

Result: ViMU 能够评估模型超越字面知觉、推断隐含意义的能力，并通过无提示设计避免关键信息泄露。

Conclusion: ViMU 提供了首个系统性评估视频暗语理解的基准，揭示了当前模型在该任务上的不足和挑战。
...
### [171] [CalibAnyView: Beyond Single-View Camera Calibration in the Wild](https://arxiv.org/abs/2605.14615)
*Boying Li,Cheng Zhang,Weirong Chen,Daniel Cremers,Ian Reid,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: 提出了CalibAnyView，一种支持任意数量输入视图的相机标定方法，通过显式建模跨视图几何一致性，并利用多视图视频数据集和几何优化框架，在单视图和多视图场景下均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统标定方法依赖受控采集环境，不适用于野外图像；现有学习方法忽视多视图几何一致性。

Method: 构建多视图视频数据集，开发多视图Transformer预测密集透视场，结合几何优化框架联合估计相机内参和重力方向。

Result: 在单视图和多视图设置下均优于现有方法，且多视图推理进一步提升性能。

Conclusion: CalibAnyView为野外3D重建和机器人感知提供了可靠的标定基础。
...
### [172] [Do Composed Image Retrieval Benchmarks Require Multimodal Composition?](https://arxiv.org/abs/2605.14787)
...
TL;DR: 本文发现多模态检索基准中存在大量单模态捷径（32.2%-83.6%），仅结合图像和文本的模型表现可能被高估，经人工验证后，真正需多模态组合的查询仅占约35.6%。


<details>
  <summary>Details</summary>
Motivation: 现有基准假设组合图像检索（CIR）需要多模态推理，但实证发现大量查询可通过单模态信号解决，可能导致模型能力被高估。

Method: 两阶段审计：1）跨模型分析识别捷径可解查询；2）对4,741个无捷径查询进行人工验证，筛选出1,689个合格查询。

Result: 当前基准中32.2%-83.6%的查询存在单模态捷径，且大量查询存在歧义（如编辑不匹配）。在验证集上，模型准确率下降，但对多模态信息的依赖显著增加。

Conclusion: 现有CIR基准混淆了捷径可解、含噪声和真正组合性查询，导致多模态组合能力被高估。
...
### [173] [UniTriGen: Unified Triplet Generation of Aligned Visible-Infrared-Label for Few-Shot RGB-T Semantic Segmentation](https://arxiv.org/abs/2605.14626)
*Ping Zhou,Haoyu Wang,Mengmeng Zheng,Lei Zhang,Wei Wei,Chen Ding,Fei Zhou*

Main category: cs.CV

TL;DR: 提出UniTriGen统一框架，直接生成空间对齐、语义一致、模态互补的VIS-IR-Label三元组，缓解RGB-T语义分割中数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: RGB-T语义分割需要严格对齐的VIS-IR-Label三元组，但现实中此类对齐数据稀缺；现有生成方法采用级联范式，导致视觉、红外和标签在空间结构、语义内容和跨模态细节上难以保持一致性。

Method: 构建共享潜在空间的扩散过程联合编码VIS、IR、Label；集成轻量级模态特定残差适配器；采用场景平衡和类别感知的少样本采样策略。

Result: 从有限的真实配对数据中生成高质量对齐三元组，在不同RGB-T语义分割模型上实现一致的性能提升。

Conclusion: UniTriGen通过统一生成和平衡采样有效增强数据多样性，提升模型性能，验证了方法的有效性。
...
### [174] [MultiEmo-Bench: Multi-label Visual Emotion Analysis for Multi-modal Large Language Models](https://arxiv.org/abs/2605.14635)
*Tianwei Chen,Takuya Furusawa,Yuki Hirakawa,Ryotaro Shimizu,Mo Fan,Takashi Wada*

Main category: cs.CV

TL;DR: 提出了一个多标签视觉情感分析基准数据集，用于评估多模态大模型预测图片引发情绪的能力，并发现现有标注方案存在缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有数据集的标注方案（每位标注者只判断单一候选情绪）低估了MLLMs的能力，且缺少合适的基准来评估模型。

Method: 为每张图像雇佣20名标注者，让他们选择所有感受到的情绪，汇总投票得到情绪分布标签，构建包含10,344张图像和236,998张有效票的数据集。

Result: 评估了Qwen3-VL、GPT、Gemini和Claude等模型，在主导情绪和情绪分布预测上展示了进展，但仍存在提升空间；LLM-as-a-judge方法未持续改善性能。

Conclusion: 新基准数据集能更可靠地评估MLLMs，但模型在情感分析这一主观任务上仍需改进。
...
### [175] [How to Evaluate and Refine your CAM](https://arxiv.org/abs/2605.14641)
*Luca Domeniconi,Alessandra Stramiglio,Michele Lombardi,Samuele Salti*

Main category: cs.CV

TL;DR: 本文通过引入具有真实标签的合成数据集改进了类激活图（CAM）的评估方法，并提出新指标ARCC，同时提出RefineCAM生成高分辨率的归因图。


<details>
  <summary>Details</summary>
Motivation: 现有CAM评估因缺乏真实归因而存在困难，且常用CAM方法生成低分辨率图限制了可解释性。

Method: 构建合成数据集以支持严格评估，提出ARCC复合指标，以及通过跨层聚合生成高分辨率归因图的RefineCAM方法。

Result: RefineCAM在提出的评估中持续优于现有方法。

Conclusion: ARCC能更可靠地识别忠实解释，RefineCAM解决了低分辨率问题，提升了可解释性。
...
### [176] [Vision-Based Water Level and Flow Estimation](https://arxiv.org/abs/2605.14645)
*ZhiXin Sun*

Main category: cs.CV

TL;DR: 本文提出了一种集成框架，结合最新的视觉模型与统计建模，利用物理先验和鲁棒滤波策略，提高了水位和河流表面流速的估计精度。


<details>
  <summary>Details</summary>
Motivation: 传统视觉方法对环境敏感、精度有限且标定复杂，需要更稳健的解决方案。

Method: 融合SOTA视觉模型与统计建模，利用物理先验和鲁棒滤波策略。

Result: 在水位检测和流量估计方面精度得到提升。

Conclusion: 该集成框架有效克服了传统视觉方法的局限性，并代码已开源。
...
### [177] [TERRA-CD: Multi-Temporal Framework for Multi-class and Semantic Change Detection](https://arxiv.org/abs/2605.14651)
*Omkar Oak,Rukmini Nazre,Rujuta Budke,Suraj Sawant*

Main category: cs.CV

TL;DR: 提出了TERRA-CD基准数据集，包含5221对Sentinel-2图像，覆盖欧美232个城市，用于城市植被变化检测和语义变化检测。
...
Method: 构建了2019年和2024年的Sentinel-2图像数据集，设计了四种注释方案，并评估了多种深度学习方法的性能。

Result: 数据集支持多类别变化检测和语义变化检测，提供了公开可用的基准。

Conclusion: TERRA-CD填补了城市植被变化检测数据集的空白，可有效支持环境变化研究。
...
### [178] [On the Cultural Anachronism and Temporal Reasoning in Vision Language Models](https://arxiv.org/abs/2605.15071)
*Mukul Ranjan,Prince Jha,Khushboo Kumari,Zhiqiang Shen*

Main category: cs.CV

TL;DR: 发现视觉语言模型在处理文化遗产时存在‘文化错位’现象，构建TAB-VLM基准评估显示GPT-5.2仅达58.7%准确率，表明当前模型对非西方历史物品的时序理解明显不足。


<details>
  <summary>Details</summary>
Motivation: VLM在文化遗产领域的应用日益广泛，但现有模型对历史文物的解释常出现时空概念混淆，尤其对非西方文化对象缺乏正确认知，需要量化评估这一缺陷。

Method: 提出‘文化错位’概念，构建TAB-VLM基准数据集（600题、6类、1600件印度文物），并系统评估10个前沿VLM的时序推理能力。

Result: 最优模型GPT-5.2整体准确率仅58.7%，不同架构和规模模型均表现欠佳，表明文化错位是视觉AI系统的普遍局限。

Conclusion: 当前VLM在文化遗产时序理解上存在显著短板，尤其是非西方视觉文化，亟需提升多模态AI的时空认知能力，本基准为后续研究提供了评估工具。
...
### [179] [Beyond Instance-Level Self-Supervision in 3D Multi-Modal Medical Imaging](https://arxiv.org/abs/2605.14654)
...
TL;DR: 提出利用跨个体的拓扑一致性作为医学图像自监督预训练的信号，通过实例内和实例间两种对齐方式，在7个下游任务中平均提升分割1.1%、分类5.94%并增强模态缺失鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有自监督预训练忽略人体解剖结构在不同个体间保持空间拓扑一致性这一关键特征，未能充分利用跨实例的拓扑结构作为监督信号。

Method: 设计两种对齐方式：实例内利用像素级对应通过跨模态三元组目标保持局部邻域拓扑；实例间通过构造伪对应控制部分邻域对齐并防止拓扑坍缩。

Result: 在7个多模态下游任务中，分割任务平均提升1.1%，分类任务平均提升5.94%，并在测试时模态缺失场景下展现显著更好的鲁棒性。

Conclusion: 利用跨实例拓扑一致性作为自监督信号有效提升多模态医学图像预训练性能，尤其在模态缺失条件下表现稳健。
...
### [180] [MemEye: A Visual-Centric Evaluation Framework for Multimodal Agent Memory](https://arxiv.org/abs/2605.15128)
...
TL;DR: MemEye框架评估长期多模态记忆的视觉证据粒度与推理结构，发现现有方法难以保留细粒度细节并推理状态变化。


<details>
  <summary>Details</summary>
Motivation: 现有评估多依赖文字线索或简单视觉问题，缺乏对视觉证据粒度及动态推理的检验。

Method: 构建二维评估框架：证据粒度（场景级到像素级）和证据使用（单证据到进化合成），并在8个生活场景中设置消融验证门。

Result: 在13种记忆方法和4种VLM骨干中，显示模型难以保存细粒度视觉细节和推理随时间的状态变化。

Conclusion: 长期多模态记忆依赖于证据路由、时间追踪和细节提取。
...
### [181] [MiVE: Multiscale Vision-language features for reference-guided video Editing](https://arxiv.org/abs/2605.14664)
*Tong Wang,Meng Zou,Chengjing Wu,Xiaochao Qu,Luoqi Liu,Xiaolin Hu,Ting Liu*

Main category: cs.CV

TL;DR: MiVE提出了一种利用VLM多尺度特征进行参考引导视频编辑的方法，通过提取Qwen3-VL的分层特征并集成到统一的扩散Transformer中，在人类偏好评估中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两种范式：解耦编码器存在模态间隙，而统一视觉语言编码器因仅使用最终层表示而丢失细粒度空间细节，急需一种能同时处理局部细节和全局语义的方法。

Method: 从VLM中提取分层特征（早期层捕获局部空间细节，深层层编码全局语义），集成到统一的扩散Transformer的自注意力机制中，替代传统交叉注意力设计。

Result: 在人类偏好评估中排名最高，优于现有学术方法和商业系统。

Conclusion: VLM的多尺度特征能有效弥合模态间隙并保留细节，实现更精准的视频编辑。
...
TL;DR: ATLAS 提出功能令牌，将符号代理操作与隐式视觉推理统一，在无需视觉监督的条件下，仅依赖标准下一个令牌预测实现高效视觉推理。


<details>
  <summary>Details</summary>
Motivation: 直接生成中间图像计算昂贵，代理方法延迟高，隐式方法泛化差且训练困难，需结合二者优势并克服缺陷。

Method: 功能令牌设计，每个令牌内化视觉操作，作为标准词汇表令牌；LA-GRPO 算法针对强化学习中令牌稀疏性，通过静态权重辅助目标稳定训练。

Result: 在多个挑战性基准上表现优越，同时保持清晰可解释性。

Conclusion: ATLAS 提供了一个无需架构修改、兼容标准训练范式的视觉推理新范式。
...
### [183] [Are Candidate Models Really Needed for Active Learning?](https://arxiv.org/abs/2605.14689)
*Harshini Mridula Mohan,Maanya Manjunath,Vipul Arya,S. H. Shabbeer Basha,Nitin Cheekatla*

Main category: cs.CV

TL;DR: 本研究探索使用随机初始化权重的CNN和Transformer进行主动学习，无需候选模型即可通过低置信度采样策略达到与传统框架相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统主动学习框架依赖候选模型且耗时，本研究旨在消除候选模型需求以简化流程。

Method: 采用三种置信度采样策略（HC、LC、HCLC），评估随机初始化权重的CNN和Transformer的性能。

Result: 低置信度（LC）策略在实验中表现最佳，验证了方法的鲁棒性和有效性。

Conclusion: 所提出的方法挑战了传统框架，通过无需候选模型的低置信度采样，提高了主动学习的效率与灵活性。
...
### [184] [Generating HDR Video from SDR Video](https://arxiv.org/abs/2605.14703)
...
TL;DR: 提出了一种基于大规模生成视频模型的框架，将SDR视频转换为高质量HDR视频，包含多曝光预测和可学习的视频合并模型。


<details>
  <summary>Details</summary>
Motivation: HDR视频生态系统成熟，但将遗留SDR视频升级为HDR仍缺乏令人信服的解决方案。

Method: 使用多曝光视频模型（MEVM）从单输入SDR视频预测曝光序列，再通过可学习的视频合并模型（VMM）合并为HDR序列。

Result: 定量和定性评估以及用户研究表明，该方法能鲁棒地转换来自日常消费者视频甚至经典电影的HDR视频。

Conclusion: 该方法支持基于现有SDR生成视频模型的HDR合成管道。
...
### [185] [EponaV2: Driving World Model with Comprehensive Future Reasoning](https://arxiv.org/abs/2605.14696)
...
TL;DR: EponaV2是一种新的驾驶世界模型范式，通过预测更全面的未来表示（包括3D几何和语义图）并引入流匹配组相对策略优化，实现了高质量轨迹规划，在NAVSIM基准上达到感知无关模型的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶范式依赖昂贵人工标注，可扩展性差；而感知无关的驾驶世界模型因缺乏足够监督，场景理解能力弱，轨迹规划效果不佳。

Method: 训练模型预测未来3D几何和语义表示，通过提取3D和语义模态增强环境理解；借鉴大语言模型训练策略，引入流匹配组相对策略优化提升规划精度。

Result: 在三个NAVSIM基准上，EponaV2在感知无关模型中达到最先进性能，PDMS提升1.3%，EPDMS提升5.5%。

Conclusion: 通过预测更全面的未来表示和优化策略，EponaV2有效提升了驾驶世界模型的规划能力，验证了方法的有效性。
...
### [186] [StyleTextGen: Style-Conditioned Multilingual Scene Text Generation](https://arxiv.org/abs/2605.14708)
*Zeyu Chen,Fangmin Zhao,Yan Shu,Yichao Liu,Liu Yu,Yu Zhou*

Main category: cs.CV

TL;DR: 提出StyleTextGen框架，通过双分支编码器、风格一致性损失和掩码引导推理，实现多语言场景文本风格生成的新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以从复杂背景中提取精准文本风格，且在多语言场景下难以保持字符间细腻的风格一致性。

Method: 双分支风格编码器+文本风格一致性损失+掩码引导推理策略。

Result: 在构建的StyleText-CE基准上，风格一致性和跨语言泛化性显著优于现有方法。

Conclusion: StyleTextGen实现了多语言条件文本生成的最优性能，验证了风格感知与复制的有效性。
...
### [187] [Breaking Dual Bottlenecks: Evolving Unified Multimodal Models into Self-Adaptive Interleaved Visual Reasoners](https://arxiv.org/abs/2605.14709)
...
TL;DR: 提出了一个框架，让统一模型根据指令复杂度自动切换生成策略，通过构建层次化数据管道和两阶段训练（SFT+RL），在X2I任务上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型存在“理解-生成差距”，导致在X2I任务中注意力纠缠和视觉优化瓶颈。

Method: 构建层次化数据管道，包含三种自适应模式（直接生成、自我反思、多步规划）；设计步骤推理奖励和组内复杂度惩罚；采用SFT和RL两阶段训练。

Result: 在简单到复杂指令的X2I任务上，生成保真度优于现有基线。

Conclusion: 所提框架能有效克服理解-生成差距，提升生成质量。
...
### [188] [Towards Continuous Sign Language Conversation from Isolated Signs](https://arxiv.org/abs/2605.14705)
*Youngmin Kim,Kyobin Choo,Jiwoo Park,Minseo Kim,Chanyoung Kim,Junhyeok Kim,Seong Jae Hwang*

Main category: cs.CV

TL;DR: 提出SignaVox系列数据集与模型，通过将孤立手语视频重组为连续对话，实现端到端的手语到手语对话生成。


<details>
  <summary>Details</summary>
Motivation: 现有对话AI系统依赖口语/文字交互，限制聋人用户的可及性；手语数据稀疏且标注成本高，导致模型词汇覆盖不足、开放域泛化弱。

Method: 从大规模独立标注剪辑（SignaVox-W）重构手语顺序语句（SignaVox-U）；使用检索引导的口语-标注翻译器解决语言结构错配；提出BRAID扩散Transformer对齐时长并修复协同发音边界。

Result: 在孤立到连续动作质量、响应级语义对齐及可扩展的以手语者为中心的交互上取得改进。

Conclusion: 直接手语到手语对话模型无需推理时依赖口语文本或外部标注，能更好支持视觉-空间表达。
...
### [189] [Vision-Core Guided Contrastive Learning for Balanced Multi-modal Prognosis Prediction of Stroke](https://arxiv.org/abs/2605.14710)
*Liren Chen,Lidong Sun,Mingyan Huang,Junzhe Tang,Yinghui Zhu,Guanjie Wang,Yiqing Xia,Ting Xiao*

Main category: cs.CV

TL;DR: 提出了一种三模态融合模型，结合医学图像、结构化临床数据和半结构化文本，用于缺血性卒中预后，并设计了视觉条件双重对齐融合模块（VDAFM）以增强模态间交互。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限于双模态融合，缺乏有效整合图像、临床数据和文本的三模态框架，且难以实现模态间的深层双向交互。

Method: 利用大语言模型从脑MRI自动生成半结构化诊断文本，并设计VDAFM模块，以视觉特征为条件先验，通过双重语义对齐损失实现动态深度融合。

Result: 在真实临床数据集上达到最优性能，验证了模型的有效性。

Conclusion: 三模态融合和视觉引导的对齐机制能有效提升缺血性卒中预后准确性，解决模态异质性问题。
...
### [190] [Towards Label-Free Single-Cell Phenotyping Using Multi-Task Learning](https://arxiv.org/abs/2605.14717)
*Saqib Nazir,Ardhendu Behera*

Main category: cs.CV

TL;DR: 提出了一个统一的深度学习框架，从无标记差分相位对比图像中同时进行白细胞分类和蛋白质表达回归，结合卷积与Transformer的混合架构及大语言模型生成可解释摘要。


<details>
  <summary>Details</summary>
Motivation: 无标记单细胞成像虽可替代荧光技术，但直接从明场形态推断分子表型仍有挑战，需要高效且可解释的方法。

Method: 采用混合架构，融合卷积的细粒度纹理特征与Transformer的全局表示，通过可学习的跨分支门控模块实现稳健推断，并集成大语言模型生成生物学含义总结。

Result: 在BSCCM数据集上白细胞分类准确率达91.3%，CD16表达回归Pearson相关系数为0.72，表现优异。

Conclusion: 无标记单细胞成像结合深度学习可同时实现表型识别和定量生物标志物估计，无需荧光染色，具有成本效益，代码已开源。
...
### [191] [Video-Zero: Self-Evolution Video Understanding](https://arxiv.org/abs/2605.14733)
*Ruixu Zhang,Deyi Ji,Lanyun Zhu,Xuanyi Liu,Yuxin Meng,Ruihang Chu,Yujiu Yang*

Main category: cs.CV

TL;DR: 提出了一个无标注的问答协同进化框架，通过视频时间证据驱动自进化，提升视频推理模型的性能。


<details>
  <summary>Details</summary>
Motivation: 视频自进化面临验证稀疏且常依赖静态线索而非时间证据的问题，因此关键在于证据基础而非难度。

Method: 设计Questioner-Solver协同进化框架，Questioner发现时间证据片段并生成基于证据的问答，Solver学习回答并与证据对齐。

Result: 在13个视频理解基准测试中，Video-Zero一致提升了多个视频VLM的性能。

Conclusion: 基于证据的自进化范式有效且可迁移，为视频理解模型提供了一种无标注的改进途径。
...
### [192] [BioHuman: Learning Biomechanical Human Representations from Video](https://arxiv.org/abs/2605.14772)
*Yujun Huo,He Zhang,Chentao Song,Honglin Song,Zongyu Zuo,Tao Yu*

Main category: cs.CV

TL;DR: 提出一个从视频中同时预测人体运动与肌肉激活的端到端框架，并构建了大规模同步数据集BioHuman10M。


<details>
  <summary>Details</summary>
Motivation: 缺少带生物力学标注的大规模数据集，且现有方法无法从视觉观测直接推断内部生物力学状态。

Method: 基于仿真的框架从运动捕捉数据估计肌肉激活，构建包含同步视频、运动和激活的数据集；在此基础上设计端到端模型BioHuman，从单目视频联合预测运动与肌肉激活。

Result: BioHuman能精确重建运动学与肌肉活动，并在不同受试者和运动类型上具有良好的泛化能力。

Conclusion: 该工作为基于视频的生物力学理解建立了新基准，并推动了物理基础的人体建模。
...
### [193] [CHASM: Cross-frequency Harmonized Axis-Separable Mixing for Spectral Token Operators](https://arxiv.org/abs/2605.14727)
*Pengcheng Fang,Hongli Chen,Yuxia Chen,Tengjiao Sun,Jiaxin Liu,Xiaohao Cai*

Main category: cs.CV

TL;DR: 提出CHASM，一种跨频协同的轴向可分离混合器，通过共享通道特征基和保留各频率正增益，在多个图像任务上提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有频谱混合器在频率间通道对齐上存在不足，未能统一频率方向，限制了全局交互效率。

Method: CHASM采用分离式设计：所有频率共享学习到的通道特征基，各频率保留独立的积极频谱增益，并沿高度和宽度轴可分离应用。

Result: 在加速MRI重建、欠采样MRI分割和自然图像重建中，CHASM优于同骨干的频谱混合器基线；消融实验表明共享基约束和相干采样几何对性能至关重要。

Conclusion: 跨频协同是一个有效的归纳偏置，为频谱token算子提供了结构化中间方案，平衡了全局共享与局部适应。
...
### [194] [MonoPRIO: Adaptive Prior Conditioning for Unified Monocular 3D Object Detection](https://arxiv.org/abs/2605.14781)
*Leon Davies,Qinggang Meng,Mohamad Saada,Baihua Li,Simon Sølvsten*

Main category: cs.CV

TL;DR: 提出MonoPRIO，通过自适应先验条件优化单目3D检测中的尺寸估计，在KITTI上取得多类最优结果。


<details>
  <summary>Details</summary>
Motivation: 单目3D检测中尺寸和深度因遮挡、截断和尺度-深度模糊而难确定，多类场景下尺寸模式更不稳定。

Method: 离线构建类别感知尺寸原型，解码器查询路由至软混合先验，使用不确定性感知对数空间条件化和CAP正则化训练。

Result: 在KITTI测试集上，多类检测表现最佳；仅车辆检测在未使用额外数据下达到最佳AP，计算量低于MonoCLUE。

Conclusion: 自适应先验在证据不足时最有效，但极端几何或可见性损失仍可能导致路由先验与真实尺寸不匹配。
...
### [195] [COAL: Counterfactual and Observation-Enhanced Alignment Learning for Discriminative Referring Multi-Object Tracking](https://arxiv.org/abs/2605.14795)
*Shukun Jia,Shiyu Hu,Yipei Wang,Ximeng Cheng,Yichao Cao,Xiaobo Lu*

Main category: cs.CV

TL;DR: COAL通过知识正则化解决RMOT中稀疏监督与高判别性之间的结构性矛盾，在Refer-KITTI-V2上提升7.28% HOTA。


<details>
  <summary>Details</summary>
Motivation: RMOT存在稀疏语义监督与高判别性需求间的根本矛盾，尤其在高度同质场景中导致模型过拟合到不足线索，引发捷径学习和语义崩塌。

Method: 提出COAL框架：1）显式语义注入（ESI）利用VLM稠密化观察空间；2）反事实学习（CFL）借助LLM推理增强监督；3）层次化多流集成架构（HMSI）蒸馏外部知识为领域判别表示。

Result: 在Refer-KITTI和Refer-KITTI-V2基准上超越现有方法，尤其在极具挑战的Refer-KITTI-V2上HOTA提升7.28%。

Conclusion: 知识正则化能有效解决RMOT中的稀疏性-判别性悖论，验证了外部知识蒸馏对提升判别能力的价值。
...
### [196] [Can Visual Mamba Improve AI-Generated Image Detection? An In-Depth Investigation](https://arxiv.org/abs/2605.14799)
*Mamadou Keita,Wassim Hamidouche,Hessen Bougueffa Eutamene,Abdelmalik Taleb-Ahmed,Xianxun Zhu,Abdenour Hadid*

Main category: cs.CV

TL;DR: 系统评估Vision Mamba在AI生成图像检测中的表现，并与CNN、ViT、VLM等主流方法对比。


<details>
  <summary>Details</summary>
Motivation: AI生成图像可能被用于虚假信息、身份盗窃等恶意用途，但现有检测技术对Vision Mamba的潜力探索不足。

Method: 在多种数据集和生成模型上，对多个Vision Mamba变体与CNN、ViT、VLM检测器进行基准测试，评估准确性、效率和泛化能力。

Result: Vision Mamba在检测AI生成图像方面展现出潜力，但同时也存在局限性。

Conclusion: Vision Mamba可作为区分真实与AI生成内容的系统组件，但需进一步改进以应对挑战。
...
### [197] [Probing into Camera Control of Video Models](https://arxiv.org/abs/2605.14815)
*Chen Hou,Christian Rupprecht*

Main category: cs.CV

TL;DR: 提出将相机控制视为几何位移场的训练无关方法，通过可微重采样实现视频扩散模型的相机控制，并发现基础模型的固有偏见。


<details>
  <summary>Details</summary>
Motivation: 现有监督方法受限于数据集规模和多样性，破坏了基础模型的先验知识。

Method: 将相机控制建模为位移场，在去噪过程中通过可微重采样调整潜特征。

Result: 无需训练即可实现与精调基线相当的相机控制效果，发现视频模型的通用偏见及响应差异。

Conclusion: 该方法可作为一种探针工具，评估基础模型的相机控制能力，并为3D/4D任务提供基准。
...
### [198] [SuperADD: Training-free Class-agnostic Anomaly Segmentation -- CVPR 2026 VAND 4.0 Workshop Challenge Industrial Track](https://arxiv.org/abs/2605.14808)
*Lukas Roming,Felix Lehnerer,Jonas V. Funk,Andreas Michel,Georg Maier,Thomas Längle,Jürgen Beyerer*

Main category: cs.CV

TL;DR: 提出了一种基于SuperAD改进的无训练、类别无关的工业视觉异常检测方法，通过DINOv3、重叠分块等改进增强分布偏移鲁棒性，在MVTec AD 2上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 工业检测中训练与部署数据存在光照等分布偏移，现有方法需依赖类别特定架构或调参，不适应频繁产品变体的工业场景。

Method: 采用DINOv3骨干网络、重叠块处理、强度增强、改进内存库子采样、迭代形态学闭合等优化，无需训练且超参数跨类共享。

Result: 在MVTec AD 2测试集上分割F1分数达62.61%、57.42%、54.35%，优于SuperAD等现有方法。

Conclusion: 提出的管道在分布偏移下兼顾鲁棒性与部署便捷性，适合工业应用。
...
### [199] [HDRFace: Rethinking Face Restoration with High-Dimensional Representation](https://arxiv.org/abs/2605.14821)
*Zirui Wang,Xianhui Lin,Yi Dong,Bo Wei,Gangjian Zhang,Siteng Ma,Zebiao Zheng,Xing Liu,Hong Gu,Minjing Dong*

Main category: cs.CV

TL;DR: 提出HDRFace，通过高维特征注入和自适应融合机制，在复杂退化下恢复人脸身份细节和结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型仅依赖低质量输入，难以恢复重度退化下的身份关键细节。

Method: 先用现成恢复器获得中间结果，再用预训练高维编码器提取输入和中间结果的特征，并通过SDFM机制自适应平衡结构与细节。

Result: 在SD V2.1-base和Qwen-Image上均实现稳定且一致的性能提升。

Conclusion: 高维表示注入与结构-细节自适应融合能有效提升复杂退化下的人脸恢复质量，且具有跨架构泛化能力。
...
### [200] [The Velocity Deficit: Initial Energy Injection for Flow Matching](https://arxiv.org/abs/2605.14819)
*Linze Li,Zong-Wei Hong,Shen Zhang,Bo Lin,Jinglun Li,Yao Tang,Jiajun Liang*

Main category: cs.CV

TL;DR: 本文发现流匹配在高维实践中存在速度赤字问题，导致生成样本无法达到数据流形。为解决此问题，提出了初始能量注入方法，包括基于训练的幅度感知流匹配和无训练的尺度调度校正器。实验表明，该方法在ImageNet-1k上将FID提升44.6%，加速5倍，并泛化到文本到图像任务。


<details>
  <summary>Details</summary>
Motivation: 标准流匹配理论保证恒定速度轨迹，但高维实践中的MSE目标函数系统性地低估速度幅度，导致集成延迟现象，即生成样本无法到达数据流形。

Method: 提出初始能量注入概念，包含两种互补方法：基于训练的幅度感知流匹配（MAFM）和无训练的尺度调度校正器（SSC）。两者利用速度收缩在轨迹始端有害（动能停滞）而末端有益（去噪）的对称性。

Result: 在ImageNet-1k（256x256）上，SSC只需一行代码，将FID从13.68降至7.58（改进44.6%），实现5倍加速，50步生成器优于250步基线。在MS-COCO上FID改进约22%。

Conclusion: 所提方法有效解决高维流匹配中的速度赤字问题，显著提升生成效率和图像质量，适用于高分辨率生成和文本到图像任务。
...
### [201] [Multi-proposal Collaboration and Multi-task Training for Weakly-supervised Video Moment Retrieval](https://arxiv.org/abs/2605.14838)
*Bolin Zhang,Chao Yang,Bin Jiang,Takahiro Komamizu,Ichiro Ide*

Main category: cs.CV

TL;DR: 针对弱监督视频片段检索中时间标注缺失的问题，提出多提议协作与多任务训练方法（MCMT），通过生成多个提议并融合高斯掩码构建高质量正样本，结合正负样本分类与正反向遮蔽查询重建任务，显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督方法生成的时序提议质量低、难以区分同一视频中的不匹配片段，且依赖单一辅助任务导致稳定性不足。

Method: 1) 生成多个提议并转换为可学习的高斯掩码，融合为高质量正样本掩码；2) 将其他片段归为简单负样本、整段视频归为困难负样本；3) 增加前向与反向遮蔽查询重建任务以强化网络约束。

Result: 在两个标准基准数据集上的实验验证了方法的有效性。

Conclusion: MCMT通过多提议协作和多任务训练有效提升了弱监督视频片段检索的准确性和稳定性。
...
### [202] [Editor's Choice: Evaluating Abstract Intent in Image Editing through Atomic Entity Analysis](https://arxiv.org/abs/2605.14842)
*Mor Ventura,Roy Hirsch,Yonatan Bitton,Regev Cohen,Roi Reichart*

Main category: cs.CV

TL;DR: 本文正式定义了抽象图像编辑，提出了Entity-Rubrics评估框架和AbstractEdit基准，揭示了现有模型在意图保持与编辑强度间的平衡问题，并指出集成高级LLM文本编码器与迭代思考是改进关键。


<details>
  <summary>Details</summary>
Motivation: 当前图像编辑基准多聚焦于显式文字指令，忽略了人类通过抽象概念（如情绪）进行自然交流的方式，缺乏对抽象指令的理解与评估。

Method: 形式化定义抽象图像编辑的分类，构建Entity-Rubrics分解评估框架，并创建了AbstractEdit基准数据集。通过对比11个模型的表现进行分析。

Result: 现有模型普遍存在欠编辑或过编辑问题，无法平衡意图与内容保持；集成高级LLM文本编码器与迭代思考能显著提升性能。

Conclusion: 基于实体的评估范式可泛化为奖励模型，帮助模型正确理解抽象交流，或突出测试时批评循环中的特定失败，为无缝多模态交互奠定基础。
...
### [203] [Exploring Vision-Language Models for Online Signature Verification: A Zero-Shot Capability Study](https://arxiv.org/abs/2605.14845)
*Marta Robledo-Moreno,Ruben Vera-Rodriguez,Ruben Tolosana,Javier Ortega-Garcia*

Main category: cs.CV

TL;DR: 本文探索了视觉语言模型（VLM）在签名验证任务中的零样本性能，发现其在随机伪造场景下表现出色，但在熟练伪造场景中因‘理性化陷阱’而性能下降。


<details>
  <summary>Details</summary>
Motivation: VLM在通用视觉推理中表现优异，但其在严格生物特征任务（如签名验证）中的适用性尚未被探索。

Method: 将原始运动时间序列转换为静态图像（利用压力信息编码笔画不透明度），并设计评分协议提取潜在令牌概率以计算稳健的生物特征分数。

Result: 在随机伪造场景中，GPT-5.2在移动任务上达到0.32%的等错误率，超越监督式SOTA；在熟练伪造场景中，链式思维推理导致性能显著下降。

Conclusion: VLM在签名验证中有潜力，但面对熟练伪造时，推理过程会引入幻觉，需警惕理性化陷阱。
...
### [204] [MechVerse: Evaluating Physical Motion Consistency in Video Generation Models](https://arxiv.org/abs/2605.14843)
*Rahul Jain,Mayank Patel,Asim Unmesh,Karthik Ramani*

Main category: cs.CV

TL;DR: MechVerse是一个评估图像到视频生成模型中运动是否符合机械约束的基准，包含21,156个合成视频片段，覆盖141类机械装置，发现现有模型在运动保真度上随耦合复杂度增加而显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在视觉保真度和时序一致性上表现良好，但常违反运动学与几何约束，如制造不存在的形变、破坏部件耦合关系等，缺乏对刚性、接触和传动关系的建模能力。

Method: 构建包含3个复杂度层级（独立运动、两两耦合、密集多部件耦合）的机械装配体数据集，使用结构化提示描述部件属性和运动约束，结合标准视频指标、指令跟随评分与人工判断评估模型。

Result: 当前模型能保持外观和流畅性，但机械可容许运动生成失败显著，且错误随耦合复杂度增加而增多（如将旋转误判为平移、破坏传动链）。

Conclusion: MechVerse揭示了视频生成模型在机械约束运动理解上的核心缺陷，并提出亟需面向机制感知的生成评估与改进方向。
...
### [205] [FactorizedHMR: A Hybrid Framework for Video Human Mesh Recovery](https://arxiv.org/abs/2605.14854)
*Patrick Kwon,Chen Chen*

Main category: cs.CV

TL;DR: 提出了一个两阶段人体网格重建框架，分别处理确定性躯干和概率性四肢的模糊性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在遮挡或深度弱提示下，人体重建存在多义性，且躯干和四肢的不确定性不同。

Method: 使用确定性回归模块恢复躯干-根节点，再用流匹配模块完成非躯干部分，结合复合目标表征、几何感知监督和特征感知引导。

Result: 在相机空间和世界空间基准上与强基线相当，在遮挡重建和漂移敏感指标上提升明显。

Conclusion: 两阶段分解能有效处理人体重建的歧义性，尤其改善遮挡和世界空间漂移问题。
...
### [206] [LPH-VTON: Resolving the Structure-Texture Dilemma of Virtual Try-On via Latent Process Handover](https://arxiv.org/abs/2605.14874)
*Yixin Liu,Baihong Qian,Jinglin Jiang,Jeffery Wu,Yan Chen,Wei Wang,Yida Wang,Lanqing Yang,Guangtao Xue*

Main category: cs.CV

TL;DR: 提出LPH-VTON框架，通过时间架构解耦在单一去噪过程中平衡结构完整性与纹理保真度，在VITON-HD数据集上实现最优帕累托平衡。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在虚拟试衣中面临结构完整性与纹理保真度之间的根本性权衡。

Method: LPH-VTON将生成过程分解：前期使用结构偏置模型建立几何一致的潜在支架，后期切换至纹理偏置模型进行高保真细节渲染。

Result: 在VITON-HD数据集上，模型实现了感知真实性和结构对齐的最优帕累托平衡，确立了新基准。

Conclusion: 时间架构解耦能有效解决虚拟试衣中结构-纹理的固有冲突。
...
### [207] [HeatKV: Head-tuned KV-cache Compression for Visual Autoregressive Modeling](https://arxiv.org/abs/2605.14877)
*Jonathan Cederlund,Axel Berg,Durmus Alp Emre Acar,Chuteng Zhou,Pontus Giselsson*

Main category: cs.CV

TL;DR: 本文提出HeatKV，一种针对视觉自回归（VAR）模型的KV缓存压缩方法，通过基于注意力分数进行头级别的缓存分配，实现了2倍压缩比提升，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: VAR模型在图像生成中面临严重的KV缓存内存限制，每张图像需要数GB内存，因此需要有效的压缩方法。

Method: 利用小规模离线校准集，根据每个注意力头对先前尺度的注意力分数进行排序，构建静态剪枝调度，实现细粒度的、头特定的缓存分配。

Result: 在Infinity-2B模型上，HeatKV相比现有方法实现了2倍更高的KV缓存压缩比，同时保持相同或更好的图像保真度、提示对齐度和人类感知分数。

Conclusion: HeatKV在VAR模型KV缓存压缩上达到新的最优性能，证明了细粒度、头特定缓存分配的有效性。
...
### [208] [Denoising-GS: Gaussian Splatting with Spatial-aware Denoising](https://arxiv.org/abs/2605.14880)
*Qingyuan Zhou,Xinyi Liu,Weidong Yang,Ning Wang,Shuquan Ye,Ben Fei,Ying He,Wanli Ouyang*

Main category: cs.CV

TL;DR: 提出Denoising-GS框架，通过将3D高斯泼溅优化视为原始去噪过程，结合空间梯度去噪、不确定性剪枝和空间一致性细化策略，在保持紧凑性的同时提升新视角合成保真度。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法仅调整高斯原始位置而忽略空间结构，导致噪声原始体影响合成质量。

Method: 设计空间感知优化器保持去噪方向；空间梯度去噪确保梯度一致更新；不确定性模块剪枝冗余原始体；空间一致性细化在稀疏区域分裂原始体。

Result: 在三个基准数据集上达到最优新视角合成性能，同时保持表示紧凑性。

Conclusion: 空间感知去噪框架能有效提升3DGS的渲染质量和效率，代码将开源。
...
### [209] [SR-Prominence: A Crowdsourced Protocol and Dataset Suite for Perceptually-Weighted Super-Resolution Artifact Evaluation](https://arxiv.org/abs/2605.14847)
*Ivan Molodetskikh,Kirill Malyshev,Mark Mirgaleev,Nikita Zagainov,Evgeney Bogatyrev,Dmitriy Vatolin*

Main category: cs.CV

TL;DR: 提出了一个基于显著性的人工制品评估框架，结合众包标注和数据集SR-Prominence，发现仅48.2%的传统二进制标记的人工制品能被多数观众感知，并验证了全参考指标SSIM和DISTS在局部显著性上的优势。


<details>
  <summary>Details</summary>
Motivation: 现有超分方法生成图像时经常引入视觉假象，而现有检测方法对所有假象一视同仁，没有考虑它们在感知上的不同程度影响。

Method: 定义了显眼度（artifact prominence）作为评估目标，通过众包标注获取数据，构建了包含3935个人工制品掩码的SR-Prominence数据集，并使用特异性评分协议进行基准测试。

Result: DeSRA数据集中48.2%的实验室二进制人工制品不被多数观众注意到；全参考指标（SSIM、DISTS）在局部显著定位中表现出色，无参考IQA方法和专用检测器跨数据集泛化能力差。

Conclusion: 应当从二进制检测转向基于感知影响的显眼度评估，SR-Prominence为这一转变提供了数据和评测协议。
...
### [210] [Unlocking Complex Visual Generation via Closed-Loop Verified Reasoning](https://arxiv.org/abs/2605.14876)
*Hanbo Cheng,Limin Lin,Ruo Zhang,Yicheng Pan,Jun Du*

Main category: cs.CV

TL;DR: 提出闭环视觉推理(CLVR)框架，通过视觉-语言规划与像素级扩散生成深度耦合，解决多步推理中的幻觉、不稳定和延迟问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型依赖单步生成，处理复杂语义能力有限，参数扩展收益递减；多步推理方法存在未验证的规划幻觉、后反思不足、长上下文优化不稳定及推理延迟高昂。

Method: CLVR框架包含：（1）带步骤级视觉验证的自动化数据引擎；（2）代理提示强化学习(PPRL)将交错多模态历史蒸馏为显式奖励信号；（3）Δ-空间权重合并(DSWM)融合对齐权重与蒸馏先验，将每步推理成本降至4 NFEs。

Result: 在多个基准测试中超越现有开源基线，接近商业闭源模型性能，实现复杂视觉生成的一般测试时扩展能力。

Conclusion: CLVR有效克服了当前多步推理在视觉生成中的核心瓶颈，为复杂文本到图像生成提供可扩展的解决方案。
...
### [211] [Masked Next-Scale Prediction for Self-supervised Scene Text Recognition](https://arxiv.org/abs/2605.14885)
*Zhuohao Chen,Zeng Li,Yifei Zhang,Chang Liu,Yu Zhou*

Main category: cs.CV

TL;DR: 提出了一种名为MNSP的自监督框架，通过跨尺度预测和掩码重建来提升场景文本识别的层级特征建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有MIM方法仅在单一空间尺度操作，无法捕捉场景文本的层级结构特性。

Method: 包含下一尺度预测（NSP）学习层级表示、掩码图像重建强化局部约束、多尺度语言对齐模块保持语义一致性。

Result: 在Union14M基准上达到86.2%平均准确率，在六个标准数据集上达到96.7%，在极端尺度和布局变化下表现鲁棒。

Conclusion: MNSP通过联合跨尺度预测和掩码重建有效提升了场景文本识别的性能。
...
### [212] [Your CLIP has 164 dimensions of noise: Exploring the embeddings covariance eigenspectrum of contrastively pretrained vision-language transformers](https://arxiv.org/abs/2605.14893)
*Jakub Grzywaczewski,Dawid Płudowski,Przemysław Biecek*

Main category: cs.CV

TL;DR: 本文通过谱分解方法将VLM潜在空间分解为多模态语义信号和共享噪声子空间，发现噪声几何具有强子群不变性，去除噪声维度可保持或提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: CLIP等对比预训练视觉-语言模型共享潜在空间存在结构异常和非语义噪声。

Method: 对协方差矩阵进行谱分解，分离语义信号与共享噪声子空间。

Result: 共享噪声维度具有子群不变性，修剪噪声主要无害，可改善下游性能。

Conclusion: VLM潜在空间的大部分几何由共享架构级噪声支配，而非仅任务相关语义。
...
### [213] [SurgicalMamba: Dual-Path SSD with State Regramming for Online Surgical Phase Recognition](https://arxiv.org/abs/2605.14889)
*Sukju Oh,Sukkyu Sun*

Main category: cs.CV

TL;DR: SurgicalMamba是基于Mamba2的因果手术阶段识别模型，通过三个创新组件应对长视频、非均匀时间流和强相关特征，在7个公开数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有手术阶段识别方法无法同时处理手术视频的三个需求：长时序跨度、非均匀时间流速和特征通道强相关性，导致识别精度受限。

Method: 基于Mamba2的结构化状态空间对偶性构建，提出三个组件：双路径SSD块分离长短时状态、强度调制步进实现自适应时间扭曲、状态重编程通过Cayley旋转实现跨通道混合。

Result: Cholec80上94.6%/82.7%准确率，AutoLaparo上89.5%/68.9%，速度119 fps，均超越先前最优方法。

Conclusion: SurgicalMamba有效解决了手术视频分析的三大挑战，且学习到的旋转平面呈现相位对齐的结构，具有可解释性，为临床应用奠定基础。
...
### [214] [MHSA: A Lightweight Framework for Mitigating Hallucinations via Steered Attention in LVLMs](https://arxiv.org/abs/2605.14966)
*Wei Ding,Yilin Li,Yudong Zhang,Ruobing Xie,Xingwu Sun,Jiansheng Chen,Yu Wang*

Main category: cs.CV

TL;DR: 提出MHSA框架，通过学习纠正跨模态注意力模式来缓解LVLMs中的幻觉问题，无需修改模型参数。


<details>
  <summary>Details</summary>
Motivation: LVLMs在多模态任务中表现优异但存在幻觉问题，现有方法仅能检测幻觉而无法缓解。

Method: 训练一个三层MLP生成器，利用DHCP判别器和LVLM自身的监督信号来产生校正后的注意力，并在推理时替换原始注意力。

Result: 在多个数据集和LVLMs上有效缓解了判别性和生成性幻觉。

Conclusion: MHSA将跨模态注意力从幻觉检测扩展到缓解，为LVLM幻觉研究提供了新视角并增强了模型可靠性。
...
### [215] [Hierarchical Image Tokenization for Multi-Scale Image Super Resolution](https://arxiv.org/abs/2605.14891)
*Isma Hadji,Enrique Sanchez,Adrian Bulat,Brais Martinez,Georgios Tzimiropoulos*

Main category: cs.CV

TL;DR: 提出了一种基于视觉自回归（VAR）建模的多尺度图像超分辨率方法，通过层次图像标记化（HIT）和直接偏好优化（DPO）正则化，在更小模型（300M参数）上实现单次前向传播的多尺度输出，达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VAR超分辨率方法无法灵活生成多尺度输出，且依赖大型骨干网络或大量标注数据。

Method: 引入层次图像标记化（HIT）实现多尺度渐进表示和跨尺度标记重叠，结合直接偏好优化（DPO）正则化仅利用低/高分辨率图像对优化变压器。

Result: 300M参数模型在无外部训练数据情况下超越1B参数的VARSR方法，支持单次前向传播多尺度输出。

Conclusion: HIT作为强归纳偏置降低了VAR训练复杂度，实现了高效灵活的多尺度超分辨率。
...
### [216] [MicroscopyMatching: Towards a Ready-to-use Framework for Microscopy Image Analysis in Diverse Conditions](https://arxiv.org/abs/2605.14980)
*Xiaofei Hui,Haoxuan Qu,Hossein Rahmani,Shuohong Wang,Jeff W. Lichtman,Jun Liu*

Main category: cs.CV

TL;DR: 提出首个通用显微图像分析框架MicroscopyMatching，通过统一匹配问题解决多样化分析任务


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法难以适应多样化的显微图像分析场景，导致研究人员仍依赖耗时的人工分析，迫切需要通用可靠的自动化工具

Method: 将不同分析任务统一转化为匹配问题，利用预训练潜在扩散模型的鲁棒匹配能力

Result: 在分割、追踪、计数等关键任务上实现跨场景可靠性能

Conclusion: 该框架为生物医学研究提供了实用的显微图像自动分析解决方案
...
### [217] [SEDiT: Mask-Free Video Subtitle Erasure via One-step Diffusion Transformer](https://arxiv.org/abs/2605.14894)
*Zheng Hui,Yunlong Bai*

Main category: cs.CV

TL;DR: SEDiT是一种基于扩散Transformer的单步视频字幕擦除方法，无需掩膜先验，支持无限长度原生1440p视频的高效处理。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法依赖掩膜输入，分割精度影响完成质量，且两阶段处理存在次优性。

Method: 提出无掩膜推理的单步框架，利用整流流的一步生成性质，通过理论证明局部编辑下条件最优传输映射的Lipschitz连续性，结合混合训练策略（偶尔以干净首帧为条件）实现长时一致性，并采用分块流式推理处理无限分辨率视频。

Result: 实证验证了一步去噪的可靠性，可高效处理无限长度原生1440p视频，且避免拼接伪影（尤其在运动场景中）。

Conclusion: SEDiT在字幕擦除任务上实现了更快、更优的效果，得益于其单步生成与流式架构设计。
...
### [218] [SteerSeg: Attention Steering for Reasoning Video Segmentation](https://arxiv.org/abs/2605.14908)
*Ali Cheraghian,Hamidreza Dastmalchi,Abdelwahed Khamis,Morteza Saberi,Aijun An,Lars Petersson*

Main category: cs.CV

TL;DR: 提出了SteerSeg框架，通过软提示和思维链提示改善视频推理分割中的注意力定位，仅需学习少量参数即可提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于冻结大型视觉语言模型的方法生成的注意力图扩散且模糊，不适合空间定位，需要解决这种错位问题。

Method: 结合可学习的软提示和推理驱动的思维链提示，调整注意力分布，提取点提示指导分割，并通过相关性评分选择候选轨迹。

Result: 仅在Ref-YouTube-VOS上训练，但在多个基准测试上表现良好，显著改善了空间定位能力。

Conclusion: SteerSeg通过输入级条件调整注意力，冻结模型参数并仅学习软提示，有效提升了视频推理分割的准确性和泛化性。
...
### [219] [Evidential Reasoning Advances Interpretable Real-World Disease Screening](https://arxiv.org/abs/2605.15171)
*Chenyu Lian,Hong-Yu Zhou,Jing Qin*

Main category: cs.CV

TL;DR: EviScreen是一个基于证据推理的疾病筛查框架，通过历史病例的区域级证据提高性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有疾病筛查模型可解释性差且性能不佳，缺乏历史病例引用和透明推理路径。

Method: 利用双知识库检索区域级证据，结合证据感知推理模块进行预测，并通过对比检索生成异常图增强定位可解释性。

Result: 在真实疾病筛查基准上取得更优性能，在临床级召回率下实现更高特异性。

Conclusion: EviScreen通过证据推理有效提升了疾病筛查的可解释性和性能。
...
### [220] [MemLens: Benchmarking Multimodal Long-Term Memory in Large Vision-Language Models](https://arxiv.org/abs/2605.14906)
...
TL;DR: MEMLENS是一个评估多模态多会话记忆的基准，包含789个问题，覆盖五种记忆能力，发现长上下文LVLMs在短上下文中表现好但随长度下降，记忆代理长度稳定但视觉保真度损失，多会话推理普遍低于30%。


<details>
  <summary>Details</summary>
Motivation: 现有的基准没有系统比较长上下文LVLMs和记忆增强代理在需要多模态证据问题上的能力，因此需要新的基准来填补这一空白。

Method: 设计了MEMLENS基准，包含789个问题，覆盖五种记忆能力（信息提取、多会话推理、时间推理、知识更新、拒绝回答），在四种上下文长度（32K-256K tokens）下评估，采用跨模态令牌计数方案，并进行图像消融研究验证视觉证据的必要性。

Result: 长上下文LVLMs在短上下文中准确率高但随长度下降；记忆代理长度稳定但存储时视觉保真度下降；多会话推理限制多数系统低于30%；图像消融证明去除视觉证据后准确率降至2%以下。
...
### [221] [Predicting Response to Neoadjuvant Chemotherapy in Ovarian Cancer from CT Baseline Using Multi-Loss Deep Learning](https://arxiv.org/abs/2605.14991)
...
TL;DR: 提出一种非侵入式深度学习框架，利用术前增强CT自动提取的3D病灶掩膜预测卵巢癌新辅助化疗反应。


<details>
  <summary>Details</summary>
Motivation: 约60%卵巢癌患者确诊时为晚期，5年生存率仅30%，现有方法无法早期识别化疗无效者，导致无效治疗和手术延误。

Method: 部分微调预训练图像编码器处理轴向切片，通过注意力模块聚合切片级表示形成体积嵌入，结合分类损失、监督对比正则化和困难样本挖掘优化模型。

Result: 在280例患者（147应答/133无应答）的测试集上，ROC-AUC为0.73，F1分数为0.70。

Conclusion: 模型能学习临床相关预测模式，为基于影像的分层工具提供稳健基础。
...
### [222] [Sat3DGen: Comprehensive Street-Level 3D Scene Generation from Single Satellite Image](https://arxiv.org/abs/2605.14984)
*Ming Qian,Zimin Xia,Changkun Liu,Shuailei Ma,Wen Wang,Zeran Ke,Bin Tan,Hang Zhang,Gui-Song Xia*

Main category: cs.CV

TL;DR: 本文提出了 Sat3DGen，一种从单张卫星图像生成街景级 3D 场景的几何优先方法，通过集成新几何约束和视角训练策略，显著提升了 3D 精度与逼真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在卫星图像到街景 3D 场景生成中存在几何精度与语义丰富度的权衡，几何-色彩模型精度高但缺乏多样性，代理模型内容丰富但几何不稳定，其根源是极端视角差异和稀疏不一致的监督数据。

Method: 采用几何优先的增强前馈范式，引入新颖的几何约束与视角训练策略，以对抗主要几何误差源。

Result: 在 VIGOR-OOD 测试集上，几何 RMSE 从 6.76m 降至 5.20m，FID 从约 40 降至 19（与 Sat2Density++ 相比），且无需额外图像质量模块。

Conclusion: Sat3DGen 在 3D 精度和逼真度上取得显著提升，并展示了在语义-3D 合成、多视角视频生成、大规模网格建模和单图像 DSM 估计等下游应用中的多功能性。
...
### [223] [Representative Attention For Vision Transformers](https://arxiv.org/abs/2605.14913)
*Yuntong Li,Hainuo Wang,Hengxing Liu,Mingjia Li,Xiaojie Guo*

Main category: cs.CV

TL;DR: 提出了一种名为Representative Attention (RPAttention)的线性全局注意力机制，通过在表示空间中进行动态令牌压缩，而非基于固定空间布局，实现了线性复杂度的全局上下文建模。


<details>
  <summary>Details</summary>
Motivation: 现有的线性注意力方法将令牌压缩锚定在图像坐标上，而非视觉内容的语义组织，限制了全局信息交换的效率。

Method: 采用Gather-Interact-Distribute范式：首先通过基于相似性的竞争路由将空间令牌软聚合到代表性令牌中，然后在紧凑潜空间中进行全局交互，最后通过查询驱动的交叉注意力将信息广播回所有空间令牌。

Result: 在图像分类、目标检测和语义分割等多种视觉Transformer骨干网络上验证了有效性，令牌交互复杂度从二次降至线性。

Conclusion: RPAttention通过表示驱动的令牌压缩替代坐标驱动的聚合，在保持全局感受野的同时实现了自适应的内容结构对齐。
...
### [224] [EverAnimate: Minute-Scale Human Animation via Latent Flow Restoration](https://arxiv.org/abs/2605.15042)
*Wuyang Li,Yang Gao,Mariam Hassan,Lan Feng,Wentao Pan,Po-Chien Luan,Alexandre Alahi*

Main category: cs.CV

TL;DR: 提出EverAnimate，一种高效的后期训练方法，用于长时动画视频生成，通过持久潜在上下文记忆解决漂移问题，仅需轻量LoRA调优即可在短时和长时场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 长时动画生成中，高度动态人体运动与相对静态环境合成易导致逐块生成的累积漂移（低质背景退化和高层语义不一致），亟需高效方法保真。

Method: 包含两个互补机制：(i) 持久潜在传播，跨块维护上下文记忆以传播身份和运动并缓解时序遗忘；(ii) 恢复性流匹配，通过采样时速度调整实现隐式恢复目标，提升块内保真度。

Result: 在10秒时长，PSNR/SSIM提升8%/7%，LPIPS/FID降低22%/11%；在90秒时长，增益增至15%/15%和32%/27%。

Conclusion: EverAnimate以轻量调优实现长时动画生成中视觉质量与角色一致性的显著提升，优于前沿方法。
...
### [225] [SceneParser: Hierarchical Scene Parsing for Visual Semantics Understanding](https://arxiv.org/abs/2605.14923)
*Pengxin Xu,Xincheng Lin,Luping Xiao,Qing Jiang,Meishan Zhang,Hao Fei,Shanghang Zhang,Xingyu Chen*

Main category: cs.CV

TL;DR: 提出了层次场景理解任务及配套的SceneParser模型与SceneParser-Bench基准，实现了场景→物体→部件→交互点的结构化解析。


<details>
  <summary>Details</summary>
Motivation: 现有感知能力孤立预测物体、部件或交互点，缺失结构化层级关联，无法支持交互导向的场景理解。

Method: 基于VLM的统一层次生成框架，采用结构补全伪标签和课程学习训练，并构建大规模层级数据引擎生成训练与评估数据。

Result: SceneParser在层次解析任务上优于现有MLLMs和感知组合流程，且与COCO/AGD20K等常规任务兼容，下游规划实验验证了表示实用性。

Conclusion: 层次场景解析能提供结构化、可操作的视觉表示，填补了交互导向理解的空白，且与现有任务良好兼容。
...
### [226] [Road Maps as Free Geometric Priors: Weather-Invariant Drone Geo-Localization with GeoFuse](https://arxiv.org/abs/2605.14925)
*Yunsong Fang,Tingyu Wang,Zhedong Zheng*

Main category: cs.CV

TL;DR: 提出GeoFuse框架，通过融合路网地图与卫星图像，增强无人机视角下恶劣天气的地理定位性能，在2个基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 无人机定位中恶劣天气导致跨视角域差距加剧，现有方法忽略路网地图这一低成本、天气不变的结构信息。

Method: 构建路网地图增强基准，设计含令牌级和通道级交互的融合模块，采用动态门控机制自适应加权模态贡献，并应用类级跨视角对比学习对齐特征。

Result: 在University-1652和DenseUAV上Recall@1分别提升3.46%和23.18%，超越当前最优方法。

Conclusion: 融合路网地图可有效提升天气条件下无人机地理定位的鲁棒性和准确性。
...
### [227] [Quantitative Video World Model Evaluation for Geometric-Consistency](https://arxiv.org/abs/2605.15185)
*Jiaxin Wu,Yihao Pi,Yinling Zhang,Yuheng Li,Xueyan Zou*

Main category: cs.CV

TL;DR: 本文提出PDI-Bench，一个用于评估生成视频几何一致性的定量框架，通过点跟踪和单目重建计算投影几何残差，揭示了现有视频生成器在尺度深度对齐、3D运动一致性和结构刚性方面的常见失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有视频评估方法依赖人工判断或学习评分器，主观性强且难以诊断几何失败，需要更客观的几何一致性评估工具。

Method: 利用分割和点跟踪（如SAM 2、MegaSaM、CoTracker3）获取目标观测，通过单目重建提升到3D世界坐标，计算投影几何残差来度量三种故障维度：尺度-深度对齐、3D运动一致性、3D结构刚性。

Result: PDI-Bench发现当前视频生成器存在一致的几何特异性失败模式，这些模式未被常见感知指标捕获，并提供了诊断信号。

Conclusion: PDI-Bench为物理可信视频生成和世界模型评估提供了有效的几何诊断工具，促进该领域发展。
...
### [228] [SCRWKV: Ultra-Compact Structure-Calibrated Vision-RWKV for Topological Crack Segmentation](https://arxiv.org/abs/2605.14926)
*Hanxu Zhang,Chen Jia,Hui Liu,Xu Cheng,Fan Shi,Shengyong Chen*

Main category: cs.CV

TL;DR: 提出超紧凑结构校准视觉RWKV（SCRWKV）网络，用于结构裂缝的像素级分割，以1.22M参数在多个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在裂缝拓扑建模与计算效率之间难以平衡，无法同时保证高分割质量与低资源需求。

Method: SCRWKV包含结构场编码器（SFE）主干，集成自适应多尺度级联调制器（AMCM）和结构校准感知单元（SCIU）；SCIU利用几何引导双向结构变换（GBST）和动态自校准衰减（DSCD）增强拓扑建模；轻量级跨尺度谐波融合（CSHF）解码器实现精确特征聚合。

Result: 在TUT数据集上F1达0.8428，mIoU达0.8512，仅1.22M参数，显著优于现有方法。

Conclusion: SCRWKV在高精度与高效率间取得平衡，具有实际部署潜力。代码已开源。
...
### [229] [Multi-scale Coarse-to-fine Modeling for Test-time Human Motion Control](https://arxiv.org/abs/2605.14935)
*Nhat Le,Daochang Liu,Anh Nguyen,Ajmal Mian*

Main category: cs.CV

TL;DR: MSCoT是一种多尺度、由粗到细的模型，用于测试时人体运动合成与控制。它通过分层表示和离散token预测实现快速、高质量的可控运动生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖多次迭代或针对特定控制信号设计模块，效率低且灵活性差。MSCoT旨在实现快速、灵活且高质量的运动控制。

Method: 采用多尺度分层运动表示和由粗到细的token预测，结合多尺度token引导策略处理离散采样，并引入轻量级token精炼器通过连续残差进行可微调优化。

Result: 在HumanML3D基准上，FID提升48%，平均控制误差降低61%，推理速度提高10倍，优于现有基线。

Conclusion: MSCoT在运动质量和控制精度上达到最先进水平，同时显著提升采样速度，适用于实时可控运动生成。
...
### [230] [ACE-LoRA: Adaptive Orthogonal Decoupling for Continual Image Editing](https://arxiv.org/abs/2605.14948)
*Yuehao Liu,Weijia Zhang,Xuanming Shang,Zhizhou Chen,Yanhao Ge,Shanyan Guan,Chao Ma*

Main category: cs.CV

TL;DR: 提出了ACE-LoRA框架，通过自适应正交解耦和秩不变历史信息压缩解决持续图像编辑中的灾难性遗忘问题，并构建了首个基准CIEBench。


<details>
  <summary>Details</summary>
Motivation: 现实应用需要模型持续适应新任务而不遗忘旧知识，但持续学习在图像编辑领域尚未充分探索。

Method: 使用自适应正交解耦识别任务干扰并进行正交化，结合秩不变历史信息压缩策略应对持续更新的可扩展性问题。

Result: 在指令保真度、视觉真实感和遗忘鲁棒性上持续超越现有基线。

Conclusion: ACE-LoRA为持续图像编辑建立了强基线，CIEBench为领域标准化评估提供了基础。
...
### [231] [EntityBench: Towards Entity-Consistent Long-Range Multi-Shot Video Generation](https://arxiv.org/abs/2605.15199)
*Ruozhen He,Meng Wei,Ziyan Yang,Vicente Ordonez*

Main category: cs.CV

TL;DR: 提出了EntityBench基准测试（140个片段，2491个镜头）和EntityMem记忆增强系统，用于评估和提升多镜头视频生成中的实体一致性。


<details>
  <summary>Details</summary>
Motivation: 现有评估使用独立生成的提示集，实体覆盖有限且一致性指标简单，难以标准化比较长序列中的实体一致性。

Method: EntityBench：从真实叙述媒体中提取数据，具有显式的每镜头实体调度；EntityMem：在生成前将已验证的实体视觉参考存储在持久记忆库中。

Result: 跨镜头实体一致性随重复间隔急剧下降；EntityMem在角色保真度（Cohen's d = +2.33）和存在性上表现最好。

Conclusion: 显式的每实体记忆能显著提升长序列中的实体一致性，EntityBench为标准化评估提供了框架。
...
### [232] [VGGT-Edit: Feed-forward Native 3D Scene Editing with Residual Field Prediction](https://arxiv.org/abs/2605.15186)
...
TL;DR: 提出VGGT-Edit，一种前馈式原生3D场景编辑框架，通过深度同步文本注入和残差变换头实现文本驱动的3D编辑，克服了2D提升方法的模糊纹理和几何不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有可泛化前馈3D重建模型在静态场景感知上表现优异，但无法响应动态人类指令，且2D提升编辑方法因缺乏空间感知导致纹理模糊和几何不一致。

Method: VGGT-Edit：深度同步文本注入对齐语义与空间姿态，残差变换头直接预测3D几何位移以变形场景，配合多目标损失函数确保几何准确性和跨视图一致性，并构建DeltaScene数据集用于监督。

Result: 实验表明，VGGT-Edit在物体细节锐度、多视图一致性和推理速度上显著优于2D提升基线方法，支持近实时推理。

Conclusion: VGGT-Edit通过原生3D编辑框架实现了高质量、一致且高效的文本驱动场景编辑，为交互式应用提供了有效方案。
...
### [233] [H-OmniStereo: Zero-Shot Omnidirectional Stereo Matching with Heading-Aligned Normal Priors](https://arxiv.org/abs/2605.14963)
*Chenxing Jiang,Zhe Tong,Pusen Gao,Peize Liu,Yang Xu,Chuan Fang,Ping Tan,Shaojie Shen*

Main category: cs.CV

TL;DR: 提出了H-OmniStereo，一个零样本的全向立体匹配框架，通过构建超过280万对的全向立体数据集和引入方向对齐坐标系的赤道面法线估计器，在零样本情况下实现高精度全向立体匹配。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于全向立体数据集稀少和透视单目先验在球面畸变下的退化，导致性能严重受限。

Method: 构建大规模合成数据集，引入赤道面法线估计器提供畸变鲁棒和跨视图一致的几何先验，采用零样本策略适配现有透视立体架构。

Result: 在跨域数据集上精度高于现有方法，能成功泛化到真实消费级相机场景。

Conclusion: H-OmniStereo通过合成数据和赤道面法线估计器有效解决了全向立体匹配的数据和畸变问题，零样本泛化能力强。
...
### [234] [Compositional Video Generation via Inference-Time Guidance](https://arxiv.org/abs/2605.14988)
*Ariel Shaulov,Eitan Shaar,Amit Edenzon,Gal Chechik,Lior Wolf*

Main category: cs.CV

TL;DR: 提出CVG方法，利用冻结视频扩散模型的交叉注意力图训练轻量级分类器，在推理时引导去噪过程，提升组合生成的准确性，无需修改模型结构或重训练。


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型在需要细粒度组合理解（如实体关系、属性、动作方向）的提示上常失败，假设这些失败可通过模型内部接地信号引导去噪来缓解。

Method: 训练一个轻量组合分类器，基于交叉注意力图特征，使用其梯度在早期去噪步骤中引导潜变量轨迹，并利用冻结VLM骨干实现跨组合标签迁移。

Result: 在组合文本到视频基准上，改进提示忠实度，同时保持生成器的视觉质量。

Conclusion: CVG有效提升组合生成准确性，无需修改架构、微调生成器或用户提供布局/框等额外控制。
...
### [235] [Characterizing the visual representation of objects from the child's view](https://arxiv.org/abs/2605.14990)
*Jane Yang,Tarun Sepuri,Alvin Wei Ming Tan,Khai Loong Aw,Michael C. Frank,Bria Long*

Main category: cs.CV

TL;DR: 通过分析婴儿第一视角视频（868小时，5-36个月），发现物体类别暴露高度偏态，且非典型视角、遮挡等可变性大，但检测到的类别在上级类别中比标准照片有更强分组。


<details>
  <summary>Details</summary>
Motivation: 探究幼儿日常视觉输入的结构，理解类别学习的起点。

Method: 使用BabyView数据集，应用监督目标检测模型从300多万帧中提取常见类别，并分析分布、变异性和上级类别结构，同时用自监督视觉和多模态嵌入验证。

Result: 类别暴露高度偏态（如杯子、椅子为主），样本高度可变（非典型角度、遮挡），但上级类别结构比标准照片更强；自监督嵌入和个体儿童数据中复制此模式。

Conclusion: 视觉类别学习的鲁棒性和效率需借助利用强上级类别结构、从非典型样本中学习的模型。
...
### [236] [3D Skew-Normal Splatting](https://arxiv.org/abs/2605.15010)
*Xiangru Wu,Ke Fan,Yanwei Fu*

Main category: cs.CV

TL;DR: 提出了基于偏正态分布的3D高斯泼溅方法，通过可学习的偏斜参数实现非对称结构的高效建模，在保持解析可跟踪性的同时提升了重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统3DGS在有限基元下，对称形状的基元难以高效表示非对称结构（如边界和单侧表面），而现有方法要么限于椭圆族，要么依赖硬截断导致连续性受限。

Method: 采用Azzalini偏正态分布作为基元，引入可学习有界偏斜参数实现对称高斯与半高斯的连续插值；提出解耦参数化和分块优化策略解决尺度、旋转与偏斜参数的耦合问题。

Result: 在标准新视角合成基准上，SNS在尖锐边界、薄结构及单侧表面上重建质量优于高斯及非高斯核方法。

Conclusion: 偏正态基元能有效提升3DGS的表示紧凑性和重建精度，并保持与现有渲染管线的兼容性。
...
### [237] [HiSem: Hierarchical Semantic Disentangling for Remote Sensing Image Change Captioning](https://arxiv.org/abs/2605.15024)
*Man Wang,Chenyang Liu,Wenjun Li,Feng Ni,Bing Jia,Baoqi Huang,Riting Xia,Zhenwei Shi*

Main category: cs.CV

TL;DR: 提出层次化语义解耦网络HiSem，通过双向差分注意力与动态专家路由机制，分别处理粗粒度变化判断与细粒度语义理解，显著提升遥感图像变化描述性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法统一处理变化与未变化图像，导致粗粒度变化判断与细粒度语义理解之间的语义纠缠，限制了模型对异质语义的建模能力。

Method: 设计BDAM模块增强跨时相交互，HASD模块通过图像级路由区分变化类型，令牌级MoE建模多样变化语义。

Result: 在WHU-CDC数据集上BLEU-4提升7.52%，全面超越现有方法。

Conclusion: 通过显式对齐模型设计与双时相场景的语义异质性，为遥感图像变化描述提供结构化新思路。
...
### [238] [LATERN: Test-Time Context-Aware Explainable Video Anomaly Detection](https://arxiv.org/abs/2605.15054)
*Mitchell Piehl,Muchao Ye*

Main category: cs.CV

TL;DR: 提出LATERN框架，通过时间证据聚合将VLM的片段级推理扩展为上下文感知的事件级异常检测与解释。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在视频异常检测中受限于词元约束，进行独立片段推理，缺乏结构化时间上下文，导致预测碎片化且解释不一致。

Method: 设计两个互补模块：CEA利用图像锚定记忆机制选择历史帧作为扩展上下文生成异常分数；REA递归聚合时间证据形成连贯异常区间并生成事件级决策和解释。

Result: 在UCF-Crime和XD-Violence基准上，LATERN提升了检测准确性和解释一致性，生成了时间连贯、语义接地的事件级解释。

Conclusion: LATERN有效解决了VLM在视频异常检测中缺乏时间上下文的问题，实现了更可靠的事件级异常检测与可解释性。
...
### [239] [SAGE3D: Soft-guided attention and graph excitation for 3D point cloud corner detection](https://arxiv.org/abs/2605.15088)
*Batuhan Arda Bekar,Can Sarı,Hüseyin Can Gülkan,Barış Özcan*

Main category: cs.CV

TL;DR: 提出SAGE3D混合变换模型，通过软引导注意力和兴奋图神经网络在机载LiDAR点云中检测角点，实现高精度与高召回。


<details>
  <summary>Details</summary>
Motivation: 现有方法在点云角点检测中精度或召回不足，尤其在多尺度下角点信号易被稀释。

Method: 采用层级编码-解码架构，包含Set Abstraction下采样与Feature Propagation上采样；创新点包括软引导注意力（注入真实角点标签作为对数先验）和兴奋图神经网络（仅正向消息传递以增强高置信度角点）。

Result: 在机载LiDAR点云数据上，相比基线方法，角点检测的精度与召回均显著提升。

Conclusion: 软引导注意力和兴奋图神经网络能有效放大角点信号，避免多尺度稀释问题，适用于复杂场景。
...
### [240] [Computational Imaging Priors for Wireless Capsule Endoscopy: Monte Carlo-Guided Hemoglobin Mapping for Rare-Anomaly Detection](https://arxiv.org/abs/2605.15062)
*Chengshuai Yang,Lei Xing,Gregory Entin,Roopa Vemulapalli,Lisa Casey,Raiyan Tripti Zaman*

Main category: cs.CV

TL;DR: 基于蒙特卡洛启发的分析模型通过提取的血红蛋白先验提升了胶囊内镜分类器性能，在Kvasir-Capsule数据集上实现宏AUC的小幅度但方向一致的改进。


<details>
  <summary>Details</summary>
Motivation: RGB训练的胶囊内镜分类器在小血管病变上表现不佳，因其混淆了血红蛋白对比度与胆汁和光照衰减的信号。

Method: 使用蒙特卡洛启发的分析模型计算血红蛋白信号，通过输入融合或蒸馏头集成到EfficientNet-B0中，并在Kvasir-Capsule数据集上评估。

Result: 输入融合变体将宏AUC从0.760提升至0.783，蒸馏变体达到0.773；对淋巴管扩张症的AUC提升最大（0.238至0.337），但罕见血管类别的效果存在高方差。

Conclusion: 分析先验提供了小而方向一致的性能提升，蒸馏变体可在标准RGB图像上运行并生成免费的可解释性热力图。
...
### [241] [Causal Forcing++: Scalable Few-Step Autoregressive Diffusion Distillation for Real-Time Interactive Video Generation](https://arxiv.org/abs/2605.15141)
*Min Zhao,Hongzhou Zhu,Kaiwen Zheng,Zihan Zhou,Bokai Yan,Xinyuan Li,Xiao Yang,Chongxuan Li,Jun Zhu*

Main category: cs.CV

TL;DR: 提出了Causal Forcing++框架，通过因果一致性蒸馏实现帧级自回归视频生成，在1-2步采样下性能超越现有方法，并减少了延迟和训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有自回归扩散蒸馏方法在块级4步生成中效果较好，但存在响应粒度粗和采样延迟高的问题，帧级自回归的1-2步初始化是关键瓶颈。

Method: 采用因果一致性蒸馏（causal CD），利用相邻时间步间的单步在线教师ODE监督，学习自回归条件流映射，无需预计算完整PF-ODE轨迹。

Result: 在帧级2步设置下，VBench Total提升0.1，VBench Quality提升0.3，VisionReward提升0.335，首帧延迟减半，Stage 2训练成本降低约4倍。

Conclusion: Causal Forcing++是一种高效且可扩展的帧级自回归视频生成框架，并拓展至动作条件的世界模型生成。
...
### [242] [CoralLite: μCT Reconstruction of Coral Colonies from Individual Corallites](https://arxiv.org/abs/2605.15093)
*Jess Jones,Leonardo Bertini,Kenneth Johnson,Erica Hendy,Tilo Burghardt*

Main category: cs.CV

TL;DR: 本文提出了CoralLite，一个包含整个珊瑚骨骼μCT扫描数据集和首个基于深度学习的珊瑚单体重建基线。


<details>
  <summary>Details</summary>
Motivation: 为了理解珊瑚息肉分裂的速率和时机及其对骨骼生长的影响，需要追踪每个息肉周围的骨骼珊瑚体结构。

Method: 使用混合V-Trans-UNet架构进行分割，结合弱标注预训练和全标注微调，并实现跨切片链接以生成3D模型。

Result: 在相同菌落和投影轴上，拓扑准确度达0.94，平均Dice分数0.77；在无关标本上平均Dice分数0.63。

Conclusion: 视觉机器学习可有效支持从μCT扫描中全3D重建珊瑚单体，但实验规模和范围有限。
...
### [243] [DriveCtrl: Conditioned Sim-to-Real Driving Video Generation](https://arxiv.org/abs/2605.15116)
*Haonan Zhao,Yiting Wang,Jingkun Chen,Valentina Donzella,Thomas Bashford-Rogers,Kurt Debattista*

Main category: cs.CV

TL;DR: DriveCtrl是一种基于深度条件的可控仿真到真实视频生成框架，通过结构感知适配器保留场景布局和运动模式，生成与原始仿真序列对齐的时序连贯驾驶视频，并引入仿真到真实数据生成流程和驾驶视频真实感评分（DVRS）评估指标。


<details>
  <summary>Details</summary>
Motivation: 大规模标注驾驶视频数据对训练自动驾驶系统至关重要，但仿真数据与真实视频之间存在域差异限制了下游部署，现有视频生成方法无法同时保持场景结构、对象动态、时序一致性和视觉真实感。

Method: 提出深度条件可控生成框架DriveCtrl，包含结构感知适配器实现深度引导生成；构建可扩展的仿真到真实数据生成流程，支持深度、参考数据集风格和文本提示三种条件信号；提出基于驾驶领域知识的DVRS评估指标。

Result: 在真实感、时序质量和感知任务表现上优于基线模型和竞争方法，显著缩小了仿真到真实的域差异。

Conclusion: DriveCtrl有效生成了保持标注有效性的真实驾驶视频，为自动驾驶系统训练提供了可扩展的数据增强方案。
...
### [244] [SANA-WM: Efficient Minute-Scale World Modeling with Hybrid Linear Diffusion Transformer](https://arxiv.org/abs/2605.15178)
*Haoyi Zhu,Haozhe Liu,Yuyang Zhao,Tian Ye,Junsong Chen,Jincheng Yu,Tong He,Song Han,Enze Xie*

Main category: cs.CV

TL;DR: SANA-WM是一个高效的2.6B参数开源世界模型，原生训练用于生成一分钟720p高保真视频，并支持精确相机控制。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在长视频生成、效率和大规模部署方面存在不足，需要一种高效且高质量的开源解决方案。

Method: 采用混合线性注意力、双分支相机控制、两阶段生成管道和鲁棒标注管道。

Result: 在视觉质量上与大型工业基线相当，吞吐量提高36倍，训练仅需15天，可在单卡上生成60秒视频。

Conclusion: SANA-WM实现了高效、高质量的一分钟视频生成，为可扩展世界建模提供了新范式。
...
### [245] [Does Synthetic Layered Design Data Benefit Layered Design Decomposition?](https://arxiv.org/abs/2605.15167)
*Kam Man Wu,Haolin Yang,Qingyu Chen,Yihu Tang,Jingye Chen,Qifeng Chen*

Main category: cs.CV

TL;DR: 本文提出纯合成数据集SynLayers用于图形设计分解，实验证明其在无需真实标注数据的情况下，性能优于现有方法，且数据量约50K时效果饱和。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型输出扁平化，难以灵活编辑；真实分层数据稀缺且不均衡，合成数据缺乏系统验证。

Method: 基于CLD基线构建SynLayers纯合成数据集，利用VLM生成文本标注和边界框自动化推理输入。

Result: 纯合成数据训练效果优于PrismLayersPro等非可扩展方法；性能随数据量增加而提升，50K样本后趋于饱和；合成数据可避免真实数据中的层数不平衡问题。

Conclusion: 纯合成数据可作为图形设计分解的可扩展且有效基础，建议在分层编辑系统中推广使用。
...
### [246] [From Plans to Pixels: Learning to Plan and Orchestrate for Open-Ended Image Editing](https://arxiv.org/abs/2605.15181)
*Anirudh Sundara Rajan,Krishna Kumar Singh,Yong Jae Lee*

Main category: cs.CV

TL;DR: 提出一个针对长序列图像编辑的经验性框架，通过结合规划器、编排器和视觉语言评判器，实现复杂多步指令的自主分解与执行。


<details>
  <summary>Details</summary>
Motivation: 现有模型在处理抽象多步指令时表现不佳，而基于代理的方法依赖手工流程或教师模仿，灵活性有限且学习与编辑效果脱节。

Method: 规划器生成结构化的原子分解，编排器选择工具和区域执行每一步，视觉语言评判器提供基于结果的奖励，编排器通过最大化奖励进行训练，成功轨迹用于优化规划器。

Result: 与单步或基于规则的多步基线相比，该方法生成的编辑更连贯可靠。

Conclusion: 将规划与奖励驱动的执行紧密耦合，可提升多步图像编辑的协调性和效果。
...
### [247] [Warp-as-History: Generalizable Camera-Controlled Video Generation from One Training Video](https://arxiv.org/abs/2605.15182)
*Yifan Wang,Tong He*

Main category: cs.CV

TL;DR: 提出了Warp-as-History接口，通过将相机运动引起的图像变形转换为带位置对齐和可见标记选择的伪历史，无需训练即可实现零样本的相机可控视频生成，轻量微调可进一步改进。


<details>
  <summary>Details</summary>
Motivation: 现有相机可控视频生成方法常需要后训练或测试时优化，缺乏简单且无需额外训练或优化的有效方案。

Method: 构造相机扭曲的伪历史，对齐位置编码并过滤无效标记，通过模型的视觉历史通路注入控制信息。

Result: 在多个数据集上验证了零样本能力和轻量微调后的性能提升。

Conclusion: 所提接口简单有效，能在零样本下实现相机轨迹跟随，轻量微调进一步增强效果。
...
### [248] [RAVEN: Real-time Autoregressive Video Extrapolation with Consistency-model GRPO](https://arxiv.org/abs/2605.15190)
*Yanzuo Lu,Ronglai Zuo,Jiankang Deng*

Main category: cs.CV

TL;DR: 提出RAVEN框架和CM-GRPO算法，解决因果视频扩散模型在长时域生成中的分布偏移问题，实现高质量实时视频流生成。


<details>
  <summary>Details</summary>
Motivation: 现有因果视频扩散模型在训练和推理时面临历史分布不一致的问题，限制了长序列生成质量。

Method: RAVEN通过训练时重新打包自回滚序列为交错干净和历史端点与噪声去噪状态，对齐注意力机制；CM-GRPO将一致性采样步重构为条件高斯转移，并直接在核上应用在线强化学习。

Result: RAVEN在质量、语义和动态度指标上超越现有因果视频蒸馏基线，且CM-GRPO能进一步提升性能。

Conclusion: 所提方法有效缓解分布偏移，显著改进实时视频生成质量。
...
### [249] [RefDecoder: Enhancing Visual Generation with Conditional Video Decoding](https://arxiv.org/abs/2605.15196)
*Xiang Fan,Yuheng Wang,Bohan Fang,Zhongzheng Ren,Ranjay Krishna*

Main category: cs.CV

TL;DR: 提出RefDecoder，通过参考注意力机制将高保真图像信号注入视频VAE解码器，显著提升细节保留与一致性。


<details>
  <summary>Details</summary>
Motivation: 现有潜扩散模型的解码器缺少条件输入，导致细节丢失与输入图像不一致。

Method: 在解码器上采样阶段，用轻量图像编码器将参考帧映射为高维令牌，与去噪潜变量共同处理。

Result: 在多个解码器骨干上PSNR提升最高2.1dB，VBench I2V基准中主体、背景一致性及整体质量均提升。

Conclusion: RefDecoder可直接替换现有视频生成系统，无需微调，并泛化到风格迁移等任务。
...
### [250] [Aligning Latent Geometry for Spherical Flow Matching in Image Generation](https://arxiv.org/abs/2605.15193)
*Tuna Han Salih Meral,Kaan Oktay,Hidir Yesiltepe,Adil Kaan Akan,Pinar Yanardag*

Main category: cs.CV

TL;DR: 提出一种球面线性插值与径向投影相结合的潜在流匹配方法，在保持扩散架构不变的情况下，显著提升图像生成的FID指标。


<details>
  <summary>Details</summary>
Motivation: 现有潜在流匹配方法沿欧几里得弦传输数据，但高斯噪声与VAE潜在表示都集中于薄球壳，导致路径偏离数据分布的几何结构。

Method: 通过径向-角度分解发现图像内容主要由方向编码，因此：1）将潜在表示投影到固定半径球面；2）使用径向投影的高斯噪声作为球面先验；3）冻结编码器微调解码器；4）用球面线性插值替代线性插值，确保路径全程位于球面。

Result: 在类条件ImageNet-256上，使用不同图像分词器均稳定提升FID，且无需额外编码器或表示对齐目标。

Conclusion: 球面几何约束的潜在流匹配方法有效利用了数据分布的几何结构，在不修改扩散架构的情况下提升了图像生成质量。
...
### [251] [VGGT-$Ω$](https://arxiv.org/abs/2605.15195)
...
TL;DR: 本文介绍了VGGT-Ω，一个通过模型和数据规模扩展提升重建质量的模型，在静态和动态场景重建上取得显著进步，并展示了其学习到的表示可提升视觉-语言-动作模型性能。


<details>
  <summary>Details</summary>
Motivation: 先前的前馈重建模型（如VGGT）已展现竞争力，但其质量如何随模型和数据规模扩展尚不清楚，且存在训练效率低、动态场景处理能力有限等问题。

Method: 通过简化架构（单密集预测头+多任务监督，移除高分辨率卷积层）、引入寄存器（紧凑场景表示）和寄存器注意力（部分替代全局注意力），并结合自监督学习、高效数据标注流水线，以15倍监督数据和大量未标注视频训练模型。

Result: 在多个基准上静态和动态场景重建取得强结果，如Sintel相机估计精度提升77%，GPU内存节省约70%，且学习到的寄存器可改善视觉-语言-动作模型的对齐能力。

Conclusion: 重建可作为空间理解的可扩展代理任务，VGGT-Ω展示了通过规模扩展提升重建质量的有效性，并促进了多模态模型性能。
...
### [252] [Merging Methods for Multilingual Knowledge Editing for Large Language Models: An Empirical Odyssey](https://arxiv.org/abs/2605.13919)
*Kunil Lee,Ki-Young Shin,Jong-Hyeok Lee,Young-Joo Suh*

Main category: cs.CL

TL;DR: 该论文研究了在多语言知识编辑任务中，向量合并方法的有效性，特别是TSVM在减少多语言干扰方面的局限性，并分析了权重缩放因子和秩压缩比对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 多语言知识编辑中，语言特定的编辑会相互干扰，即使定位-编辑方法在单语设置中有效，但在多语言场景下仍具挑战性。

Method: 使用六种合并变体，结合两个主流大型语言模型、两个基础知识编辑方法和12种语言，在MzsRE基准上进行大规模批量编辑实验。

Result: 具有共享协方差的向量求和是最可靠的策略，而简单求和表现较差；TSVM在一定程度上提升性能，但缓解多语言干扰效果有限；性能对权重缩放因子和秩压缩比敏感，较大的权重缩放和较低的秩通常效果更好。
...
### [253] [VectraYX-Nano: A 42M-Parameter Spanish Cybersecurity Language Model with Curriculum Learning and Native Tool Use](https://arxiv.org/abs/2605.13989)
*Juan S. Santillana*

Main category: cs.CL

TL;DR: 本文介绍了VectraYX-Nano，一个41.95M参数的西班牙语网络安全语言模型，具备原生工具调用能力，通过课程学习和重放缓冲区实现了单调损失下降，并在小型模型上发现了语料密度与性能的关系。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏针对西班牙语、特别是拉丁美洲背景的网络安全领域语言模型，且现有模型在原生工具集成方面不足。

Method: 从零训练了一个42M参数的Transformer解码器，使用自建的VectraYX-Sec-ES语料库（170M token），采用课程学习与重放缓冲区进行持续预训练，并通过SFT和监督微调提升对话和工具使用能力。

Result: 模型在对话门控测试中达到0.78±0.05，LoRA研究发现工具选择性能的瓶颈源于语料密度而非模型容量，小模型可通过密集工具语料显著提升性能。

Conclusion: VectraYX-Nano是首个西班牙语原生网络安全LLM，具备完整MCP集成，证明了小模型在特定领域和资源受限场景下的实用性。
...
### [254] [Mistletoe: Stealthy Acceleration-Collapse Attacks on Speculative Decoding](https://arxiv.org/abs/2605.14005)
*Shuoyang Sun,Chang Da,Hao Fang,Kuofeng Gao,Xinhao Zhong,Yi Sun,Fan Mo,Shu-Tao Xia,Bin Chen*

Main category: cs.CL

TL;DR: 本文发现基于模型的推测解码中，草稿模型与目标模型之间的不匹配会引入隐藏攻击面，并提出Mistletoe攻击，通过联合优化退化目标和语义保持目标，利用零空间投影机制，显著降低草稿令牌接受长度，破坏加速效果，同时保持输出质量。


<details>
  <summary>Details</summary>
Motivation: 推测解码的加速效率依赖于草稿令牌的平均接受长度，而草稿模型与目标模型之间的近似不完美可能被利用，形成新的机制级攻击面。

Method: 提出Mistletoe攻击，联合优化退化目标（降低草稿与目标模型一致性）和语义保持目标（约束目标模型输出分布），并通过零空间投影机制解决两者冲突，将退化梯度投射偏离局部语义保持方向。

Result: 在多种推测解码系统上，Mistletoe显著降低平均接受长度、破坏加速比、降低平均令牌吞吐量，同时保持输出质量和困惑度。

Conclusion: 推测解码引入了超越输出鲁棒性的机制级攻击面，需要更鲁棒的LLM加速系统设计。
...
### [255] [Derivation Prompting: A Logic-Based Method for Improving Retrieval-Augmented Generation](https://arxiv.org/abs/2605.14053)
*Ignacio Sastre,Guillermo Moncecchi,Aiala Rosá*

Main category: cs.CL

TL;DR: 提出了一种名为推导提示的新型提示技术，用于检索增强生成框架的生成步骤，通过构建可解释的推导树来减少不可接受的答案。


<details>
  <summary>Details</summary>
Motivation: LLM在问答任务中会出现幻觉和错误推理，特别是在知识密集型、领域特定任务中。

Method: 推导提示技术，受逻辑推导启发，通过系统应用预定义规则从初始假设推导结论，构建可解释的推导树。

Result: 与传统RAG和长上下文窗口方法相比，显著减少了不可接受的答案。

Conclusion: 推导提示技术能提高RAG生成步骤的可控性和解释性，有效减少错误输出。
...
### [256] [Physics-R1: An Audited Olympiad Corpus and Recipe for Visual Physics Reasoning](https://arxiv.org/abs/2605.14040)
*Shan Yang*

Main category: cs.CL

TL;DR: 本文系统审计了多模态物理评估流程，发现了三个未检测到的建设性问题：训练-评估污染、翻译漂移和多项选择题饱和。作者发布了四个新资源（PhysCorp-A、PhysR1Corp、PhysOlym-A、Physics-R1），并通过强化学习训练的新模型在多项基准上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态物理评估管道中存在未被检测的构建缺陷，导致视觉语言推理能力的衡量失真。

Method: 使用三阶段审计（Jaccard -> mxbai-embed-large余弦 -> Haiku-4.5 LLM评判）检测重复和翻译问题；通过对比实验分析选择题格式与开放性问题性能差异；利用GSPO+DAPO强化学习策略训练Physics-R1模型。

Result: Physics-R1在PhysOlym-A上比8B基线提升18.3个百分点，在PhysReason上超过32B模型，在多项基准上表现优异。

Conclusion: 现有评估管道存在严重缺陷，通过发布新资源和改进训练方法可以有效提升物理推理能力。
...
### [257] [PEML: Parameter-efficient Multi-Task Learning with Optimized Continuous Prompts](https://arxiv.org/abs/2605.14055)
*Anjir Ahmed Chowdhury,Syed Zawad,Xiaolong Ma,Xu Dong,Feng Yan*

Main category: cs.CL

TL;DR: 提出了一种参数高效的多任务微调方法PEML，通过联合优化连续提示和低秩模型适配，在多个基准测试上平均准确率提升高达6.67%。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法如LoRA和Prefix Tuning侧重单任务适配，在多任务场景下未协同优化提示和模型，导致资源消耗大且适配能力有限。

Method: 采用神经架构工程技术优化连续提示，同时结合低秩适配调整模型权重，形成自动化多任务微调框架。

Result: 在GLUE、SuperGLUE等基准上，平均准确率提升6.67%，单个任务最高提升10.75%。

Conclusion: PEML通过联合优化提示与模型权重，有效提升了多任务学习的参数效率和性能。
...
### [258] [Dual Hierarchical Dialogue Policy Learning for Legal Inquisitive Conversational Agents](https://arxiv.org/abs/2605.14057)
*Xubo Lin,Zezhii Deng,Shihao Wang,Grace Hui Yang,Yang Deng*

Main category: cs.CL

TL;DR: 本文提出了一种双层级强化学习框架，用于构建主动提问的对话代理（ICA），以美国最高法院口头辩论为场景，通过模拟法官的提问模式来系统性获取关键法律信息。


<details>
  <summary>Details</summary>
Motivation: 现有对话系统多为被动响应式，无法在需要主动获取信息的场景（如司法质询）中高效工作。

Method: 采用双层级强化学习框架，包含两个独立策略的RL代理，分别负责高层对话策略管理和低层具体话语生成，通过协调学习提问时机与内容。

Result: 在美国最高法院数据集上，该方法在多项指标上优于各种基线模型。

Conclusion: 该研究是向高风险、领域特定场景中主动对话代理的重要第一步，展示了强化学习在战略对话管理中的有效性。
...
### [259] [Distribution Corrected Offline Data Distillation for Large Language Models](https://arxiv.org/abs/2605.14071)
*Yumeng Zhang,Zhengbang Yang,Yevin Nikhel Goonatilake,Zhuangdi Zhu*

Main category: cs.CL

TL;DR: 提出了一种离线推理蒸馏框架，通过自适应地强调与学生在策略分布更对齐的教师监督，解决教师-学生分布漂移问题，无需在线采样即可提升小模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有离线蒸馏存在分布漂移，导致学生模型在自回归推理时误差累积；在线自蒸馏虽匹配分布但成本高且早期轨迹质量低，亟需一种兼顾效率与质量的方法。

Method: 基于离线教师数据的分布校正蒸馏，通过自适应权重机制优先选择与学生当前分布更一致的教师推理轨迹进行监督。

Result: 在GSM8K、MATH、MATH500及AMC、AIME、OlympiadBench等竞赛级任务上，推理准确率优于现有离线蒸馏算法，且推理轨迹更稳定。

Conclusion: 轻量级的分布校正训练可显著增强离线推理蒸馏效果，无需在线采样，为资源受限场景提供了有效方案。
...
### [260] [Measuring and Mitigating Toxicity in Large Language Models: A Comprehensive Replication Study](https://arxiv.org/abs/2605.14087)
*Mokshit Surana,Archit Rathod,Akshaj Satishkumar*

Main category: cs.CL

TL;DR: LLMs存在毒性退化问题，DExperts在显式毒性上达100%安全率，但对抗隐式仇恨言论脆弱（98.5%），且延迟增加10倍。


<details>
  <summary>Details</summary>
Motivation: LLM训练数据中吸收的毒性模式会导致有害输出，需要在不降低模型效用的情况下实现安全缓解。

Method: 采用DExperts推理时毒性缓解技术，分为三阶段：显式毒性基线测量、方法实施与评估、隐式仇恨言论压力测试。

Result: DExperts在显式毒性基准上达100%安全率，在隐式对抗数据上降至98.5%；同时引入约10倍延迟惩罚（从0.2秒到2.0秒）。

Conclusion: 显式与隐式毒性缓解间存在鲁棒性差距，需开发更通用的方法并降低计算成本。
...
### [261] [When Evidence Conflicts: Uncertainty and Order Effects in Retrieval-Augmented Biomedical Question Answering](https://arxiv.org/abs/2605.14115)
*Yikun Han,Mengfei Lan,Halil Kilicoglu*

Main category: cs.CL

TL;DR: 该论文研究了检索增强的大型语言模型在生物医学领域面对矛盾证据时的表现，发现证据顺序颠倒会导致准确率下降11.4%-25.2%，并提出了一种结合模型置信度和证据冲突检测的冲突感知弃权评分，在困难条件下将选择性准确率提升3.6-33.4点。


<details>
  <summary>Details</summary>
Motivation: 现有评估通常关注有帮助上下文下的答案准确性，忽略了证据不完整、误导或自相矛盾时模型的可靠性，而生物医学领域冲突证据普遍存在。

Method: 使用HealthContradict数据集，在五种证据条件下评估六个开源LLM，并引入冲突感知弃权评分，结合模型置信度和证据冲突检测。

Result: 在证据顺序颠倒条件下，所有模型准确率下降，11.4%-25.2%预测翻转；冲突感知评分在困难条件下比仅基于置信度的方法提升选择性准确率7.2-33.4点。

Conclusion: 冲突生物医学证据既是不确定性也是鲁棒性问题，需要显式考虑证据不一致的评估和弃权方法。
...
### [262] [Generative Floor Plan Design with LLMs via Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2605.14117)
*Luis Lara,Aristides Milios,Zhi Hao Luo,Aditya Sharma,Ge Ya Luo,Christopher Beckham,Florian Golemo,Christopher Pal*

Main category: cs.CL

TL;DR: 该文提出一种基于文本的平面图生成方法，通过微调大语言模型（LLM）并应用强化学习（RLVR）来满足拓扑和数值约束，并设计了一组约束遵循度量来评估生成效果。实验结果表明该方法在保真度、兼容性和多样性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的平面图生成方法主要关注房间之间的连接约束，但无法支持满足房间尺寸和面积等数值约束。

Method: 首先在大规模真实平面图上微调LLM，然后使用基于可验证奖励的强化学习（RLVR）来优化模型，使其在生成过程中满足拓扑和数值约束，同时避免无效或重叠输出。此外，设计了一套约束遵循度量标准进行系统评估。

Result: 该方法在遵循用户定义的连接和数值约束方面表现优异，在真实感、兼容性和多样性指标上均优于现有方法，所有任务中兼容性相对降低了至少94%。

Conclusion: LLM能够有效处理平面图设计中的约束条件，为文本生成模型在该领域的更广泛应用提供了潜力。
...
### [263] [Polar probe linearly decodes semantic structures from LLMs](https://arxiv.org/abs/2605.14125)
*Pablo J. Diego-Simón,Pierre Orhan,Yair Lakretz,Jean-Rémi King*

Main category: cs.CL

TL;DR: 本文发现大型语言模型通过嵌入向量的距离和方向来表示实体间的关系类型，形成复杂语义结构。


<details>
  <summary>Details</summary>
Motivation: 探索人工神经网络如何绑定概念形成复杂语义结构。

Method: 使用Polar Probe从LLM层激活中线性恢复语义结构，在算术、视觉场景等五个领域测试。

Result: Polar Probe可线性恢复语义结构；该编码主要出现在中间层，随模型性能提升；能泛化到新实体和关系，但随结构大小退化；极坐标表示质量与问答能力相关。

Conclusion: LLM通过简单的几何原则绑定表示来构建复杂语义结构。
...
### [264] [ROK-FORTRESS: Measuring the Effect of Geopolitical Transcreation for National Security and Public Safety](https://arxiv.org/abs/2605.14152)
...
TL;DR: 本文提出了ROK-FORTRESS基准，通过英韩语言及美韩地缘政治轴心，分离语言与地缘背景对LLMs安全性的影响，发现韩语变体中存在抑制效应，且地缘背景可缓解韩语驱动的抑制。


<details>
  <summary>Details</summary>
Motivation: 现有多语言安全评估仅通过翻译保留原始场景，缺乏语言与地缘背景交互的实证，且仅限少数语言对。

Method: 采用构想矩阵方法论，在控制英语/韩语语言和美国/韩国实体、机构及操作细节组合下评估对抗意图，并配以双重用途良性提示量化过度拒绝，使用校准的LLM评委和专家制定的二元评分规则。

Result: 在先进及韩语优化模型中发现，韩语变体存在一致抑制效应，且模型间地缘背景与语言交互差异显著；韩语背景可缓解韩语驱动的抑制，无模型显示相反方向的增强。

Conclusion: 至少英韩案例中，语言作为风险信号与上下文交互塑造安全行为，翻译评估无法捕获此现象；构想矩阵方法可推广至其他语言-文化对。
...
### [265] [BOOKMARKS: Efficient Active Storyline Memory for Role-playing](https://arxiv.org/abs/2605.14169)
*Letian Peng,Ziche Liu,Yiming Huang,Longfei Yun,Kun Zhou,Yupeng Hou,Jingbo Shang*

Main category: cs.CL

TL;DR: 提出一种基于搜索的记忆框架BOOKMARKS，以解决角色扮演代理中递归摘要丢失细节的问题，通过主动初始化、维护和更新与当前任务相关的书签来实现长期一致性。


<details>
  <summary>Details</summary>
Motivation: 现有角色扮演代理记忆方法（如概要）主要依赖递归摘要，其压缩过程不可避免地丢失重要细节，导致角色一致性不足。

Method: BOOKMARKS框架包含三类搜索（概念、行为、状态），通过高效同步方法初始化、维护和更新书签，书签以特定故事点的问题-答案形式结构化存储，并根据当前任务选择或创建新书签，同步到当前故事点后更新答案。

Result: 在16个作品的85个角色上，BOOKMARKS显著优于现有角色扮演代理记忆基线方法。

Conclusion: 基于搜索的记忆方法比递归摘要更有效，能主动捕捉任务细节并被动更新，避免不必要的计算，是角色扮演代理记忆的有效方案。
...
### [266] [Why Retrieval-Augmented Generation Fails: A Graph Perspective](https://arxiv.org/abs/2605.14192)
*Kai Guo,Xinnan Dai,Zhibo Zhang,Nuohan Lin,Shenglai Zeng,Jie Ren,Haoyu Han,Jiliang Tang*

Main category: cs.CL

TL;DR: 本论文通过电路追踪构建归因图，分析检索增强生成中模型内部信息流，揭示正确与错误预测的结构差异，并提出基于图拓扑的误差检测与干预方法。


<details>
  <summary>Details</summary>
Motivation: 尽管RAG系统可访问外部信息，但仍频繁输出错误答案，其原因尚不清楚。

Method: 使用电路追踪构建归因图，建模检索证据与模型内部激活之间的信息流，并通过图拓扑特征分析正确/错误预测的差异。

Result: 正确预测呈现更深的推理路径、分布式证据流和局部连接模式；而错误预测则显示浅层、碎片化且过度集中的证据流。基于此开发了图拓扑误差检测框架，并通过干预重塑内部路由以减少错误。

Conclusion: 归因图能揭示RAG失败的根本原因，并支持误差检测与针对性干预，提升答案生成准确性。
...
### [267] [GradShield: Alignment Preserving Finetuning](https://arxiv.org/abs/2605.14194)
*Zhanhao Hu,Xiao Huang,Patrick Mendoza,Emad A. Alghamdi,Basel Alomair,Raluca Ada Popa,David Wagner*

Main category: cs.CL

TL;DR: 本文提出GradShield方法，通过在微调前计算数据的隐式危害分数并自适应阈值过滤有害数据，保护大语言模型的安全对齐，在保持实用性的同时将攻击成功率降至6%以下。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在微调后存在安全对齐风险，即使是表面无害的数据也可能导致模型行为偏差，现有方法无法有效防御隐式有害数据。

Method: 计算每个数据点的微调隐式危害分数（FIHS），并采用自适应阈值算法识别和移除有害数据。

Result: 在多种实用微调任务中，GradShield将攻击成功率维持在6%以下，同时保持模型实用性，性能优于所有基线方法。

Conclusion: GradShield能有效过滤隐式有害数据，保障大语言模型微调时的安全性而不损害其功能。
...
### [268] [What Makes Words Hard? Sakura at BEA 2026 Shared Task on Vocabulary Difficulty Prediction](https://arxiv.org/abs/2605.14257)
*Adam Nohejl,Xuanxin Wu,Yusuke Ide,Maria Angelica Riera Machin,Yi-Ning Chang,Hitomi Yanaka*

Main category: cs.CL

TL;DR: 本文介绍了两种词汇难度预测模型：高精度黑箱模型（r>0.91）和可解释模型（r>0.77），并揭示了英国文化协会KVL列表中难度受拼写、题目结构等因素影响。


<details>
  <summary>Details</summary>
Motivation: 现有词汇难度预测模型精度不够或缺乏可解释性，需要同时实现高精度和可解释性，并理解难度影响因素。

Method: 对比两种模型：黑箱模型采用软目标损失函数微调大语言模型；可解释模型优于微调编码器基线，通过分析提供难度影响因素。

Result: 黑箱模型在开放赛道第一名，r>0.91；可解释模型保持r>0.77相关性。分析发现KVL难度受拼写和题目结构影响。

Conclusion: 黑箱模型实现高精度预测，可解释模型提供洞察，两种方法互补，可用于词汇难度分析。
...
### [269] [Auditing Agent Harness Safety](https://arxiv.org/abs/2605.14271)
...
TL;DR: 提出了HarnessAudit框架，用于审计LLM代理的执行轨迹中的边界合规、执行保真度和系统稳定性，并创建了包含210个任务的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准仅评估最终输出，但许多违规行为发生在执行过程中，输出级安全评估无法检测到这些违规。

Method: 开发了HarnessAudit框架，通过审计完整执行轨迹来评估边界合规、执行保真度和系统稳定性，并构建了跨8个领域的210个任务基准。

Result: 任务完成与安全执行不一致，违规随轨迹长度增加；安全风险因领域、任务类型和代理角色而异；违规主要集中在资源访问和代理间信息传输；多代理协作扩大安全风险面，而调度设计决定安全部署的上限。

Conclusion: 需要关注执行轨迹级别的安全审计，多代理场景尤其需要重视资源访问和跨代理信息流控制。
...
### [270] [Factorization-Error-Free Discrete Diffusion Language Model via Speculative Decoding](https://arxiv.org/abs/2605.14305)
*Xun Fang,Yunchen Li,Hang Yuan,Zhou Yu*

Main category: cs.CL

TL;DR: 提出无因子化误差的离散扩散语言模型，通过精确前缀条件分解和投机解码提升生成质量和推理速度。


<details>
  <summary>Details</summary>
Motivation: 标准X0预测方法用独立逐词分布近似清洁后验，引入因子化误差，需解决此问题。

Method: 用精确前缀条件分解替代独立预测，并融入投机解码加速推理。

Result: 在GSM8K、MATH等数据集上准确率平均提升5.04%，推理速度提升3.86倍。

Conclusion: 该方法能生成真实联合分布，且有效保持并行预测和重掩码特性。
...
### [271] [Ideology Prediction of German Political Texts](https://arxiv.org/abs/2605.14352)
*Sinclair Schneider,Florian Steuber,Joao A. G. Schneider,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 提出一种基于Transformer的模型，将德语文本的政治倾向映射到-1到1的连续谱上，在多个语料上验证效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以灵活定位政治光谱特定区间（如保守派），而多类分类器受限于预设类别。

Method: 使用DeBERTa-large等13种Transformer模型，基于四类语料（议会记录、选举工具、33家报纸、成员推文）进行训练和测试，采用双训练/测试集防过拟合。

Result: DeBERTa-large在领域内测试和推文间域外测试最优（F1=0.844, ACC=0.864），Gemma2-2B在报纸间域外测试最佳（MAE=0.172），模型性能接近民调精度。

Conclusion: Transformer模型能识别德语新闻政治框架，模型架构和领域数据的重要性不亚于模型规模，需改进偏见测量的鲁棒性。
...
### [272] [LLM-based Detection of Manipulative Political Narratives](https://arxiv.org/abs/2605.14354)
*Sinclair Schneider,Florian Steuber,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 提出了一个结合提示过滤与无监督聚类的计算框架，用于检测和结构化操控性政治叙事。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上政治讨论增多，区分操控性叙事与合法批评成为挑战，现有方法依赖预定义类别，难以发现新叙事。

Method: 先用详细few-shot提示过滤操控性帖子，再用UMAP降维、HDBSCAN聚类，最后以推理模型解析每个簇背后的叙事。

Result: 在超过120万社交媒体帖子中，成功识别出41个不同的操控性叙事簇。

Conclusion: 该框架能有效发现新的操控性叙事结构，不依赖预定义类别，为政治叙事分析提供了灵活工具。
...
### [273] [Reinforcement Learning with Semantic Rewards Enables Low-Resource Language Expansion without Alignment Tax](https://arxiv.org/abs/2605.14366)
*Zeli Su,Ziyin Zhang,Zhou Liu,Xuexian Song,Zhankai Xu,Longfei Zheng,Xiaolu Zhang,Rong Fu,Guixian Xu,Wentao Zhang*

Main category: cs.CL

TL;DR: 本文提出了一种基于语义空间对齐和GRPO的方法，在低资源语言扩展中减轻对齐税，保留通用能力。


<details>
  <summary>Details</summary>
Motivation: 监督微调在低资源语言扩展中导致对齐税，即目标语言改进伴随通用能力遗忘，受限于逐标记表面模仿。

Method: 使用GRPO进行语义空间对齐优化，通过嵌入级语义奖励而非似然最大化，鼓励含义保留和灵活实现。

Result: 在藏汉翻译和标题生成中，该方法获取低资源能力同时显著减轻对齐税，保留通用能力，语义质量和偏好更高。
...
### [274] [Where Should Diffusion Enter a Language Model? Geometry-Guided Hidden-State Replacement](https://arxiv.org/abs/2605.14368)
*Injin Kong,Hyoungjoon Lee,Yohan Jo*

Main category: cs.CL

TL;DR: 提出DiHAL，一种几何引导的扩散-Transformer混合模型，通过选择预训练Transformer中适合扩散的隐藏状态层，用扩散桥替代低层前缀，避免直接连续到离散的token恢复，在8B模型上优于连续扩散基线。


<details>
  <summary>Details</summary>
Motivation: 连续扩散语言模型性能落后于自回归Transformer，部分原因是扩散空间不适合语言去噪和token恢复。

Method: 使用几何代理对Transformer层评分，选择扩散友好的隐藏状态接口，保留上层和LM头，用扩散桥替代低层前缀，在选定层重建隐藏状态而非token。

Result: 在8B规模骨干上，几何评分能预测有效插入层；在匹配扩散/恢复训练预算的诊断比较中，隐藏状态恢复优于连续扩散基线。

Conclusion: 隐藏状态几何有助于识别预训练语言模型中可进行扩散替代的层，表明该方法可行且有效。
...
### [275] [Mitigating Data Scarcity in Psychological Defense Classification with Context-Aware Synthetic Augmentation](https://arxiv.org/abs/2605.14380)
*Hoang-Thuy-Duong Vu,Quoc-Cuong Pham,Huy-Hieu Pham*

Main category: cs.CL

TL;DR: 提出一种结合上下文感知的合成数据增强与混合分类模型的框架，用于自动分类心理防御机制，在数据稀缺和类别不平衡情况下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 心理防御机制分类面临数据稀缺和类别不平衡问题，单纯生成式增强缺乏心理学基础，现有方法效果有限。

Method: 混合模型整合上下文语言表示、基础临床特征和150个标注防御条目，采用上下文化合成增强框架。

Result: 准确率达58.26%（提升40.25%），宏F1为24.62%（提升15.99%），超越DMRS Co-Pilot。

Conclusion: 提示定义质量直接影响生成保真度和下游性能，该方法为低资源场景下心理学驱动分类建立了强基线。
...
### [276] [Agentic Recommender System with Hierarchical Belief-State Memory](https://arxiv.org/abs/2605.14401)
...
TL;DR: 提出分级记忆框架MARS，通过三层记忆结构和自适应生命周期管理，显著提升个性化推荐准确率。


<details>
  <summary>Details</summary>
Motivation: 现有扁平记忆表示无法区分临时信号与稳定偏好，且缺乏完整的记忆演化管理机制。

Method: 将推荐视为部分可观测问题，构建三级信念状态（事件/偏好/概要记忆），设计六种操作并由LLM规划器自适应调度。

Result: 在四个基准域上，HR@1平均提升26.4%，NDCG@10提升10.3%，动态场景下自适应调度带来额外增益。

Conclusion: 结构化记忆与自适应生命周期管理能有效提升智能推荐系统的性能与适应性。
...
### [277] [Knowledge Beyond Language: Bridging the Gap in Multilingual Machine Unlearning Evaluation](https://arxiv.org/abs/2605.14404)
*Kyomin Hwang,Hyeonjin Kim,Sangyeon Cho,Nojun Kwak*

Main category: cs.CL

TL;DR: 本文提出KSS和KPS两个新指标来评估多语言机器遗忘中的跨语言信息传播，并基于此对多种遗忘方法进行综合分析。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法局限于单语言扩展，无法捕捉跨语言信息分布，导致对多语言机器遗忘的评估不全面。

Method: 提出知识可分性分数(KSS)和知识持久性分数(KPS)两个指标，分别衡量整体遗忘质量和跨语言信息一致性移除程度。

Result: 新指标能有效检测跨语言信息残留，揭示了多语言遗忘中的独特现象。

Conclusion: 这些指标为多语言机器遗忘评估提供了新视角，改进了对跨语言信息传播的理解。
...
### [278] [A Calculus-Based Framework for Determining Vocabulary Size in End-to-End ASR](https://arxiv.org/abs/2605.14427)
*Sunil Kumar Kopparapu*

Main category: cs.CL

TL;DR: 通过微积分的导数测试方法优化端到端语音识别系统的词汇量大小超参数，提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端ASR系统中词汇量大小超参数的选择缺乏理论指导，通常依赖经验值。

Method: 对训练数据进行曲线拟合，并应用微积分的一阶和二阶导数测试来形式化估计最优词汇量大小。

Result: 在Librispeech语料库上的实验表明，该方法选出的最优词汇量能提升ASR性能。

Conclusion: 提出了一种形式化方法确定端到端ASR系统的最佳词汇量大小，验证了其有效性。
...
### [279] [Does RAG Know When Retrieval Is Wrong? Diagnosing Context Compliance under Knowledge Conflict](https://arxiv.org/abs/2605.14473)
*Yihang Chen,Pin Qian,Su Wang,Sipeng Zhang,Huan Xu,Shuhuai Lin,Xinpeng Wei*

Main category: cs.CL

TL;DR: 本论文提出上下文驱动分解（CDD）方法，用于探测和干预检索增强生成（RAG）中的上下文遵从问题。实验发现标准RAG在对抗设置下准确率仅15.0%，而CDD在时间漂移和干扰物条件下分别达到71.3%和69.9%，但不同模型间的推理机制存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有RAG评估中，准确率无法揭示检索上下文与模型参数知识冲突时对答案的因果影响，需要一种能够探测和干预这种冲突的方法。

Method: 提出上下文驱动分解（CDD），一种在推理时运行的信念分解探针，可作为受控检索冲突的干预机制。

Result: 在TruthfulQA误解注入测试中，标准RAG准确率仅15.0%；CDD在时间漂移和干扰物条件下分别达到71.3%和69.9%；Gemini-2.5-Flash的因果敏感性达64.1%，而Claude系列仅-3%到+7%。

Conclusion: 上下文遵从是RAG中独立于检索质量的结构性维度，可通过CDD进行探测和干预，但因果机制在不同模型间不传递，需更多系统性研究。
...
### [280] [Cross-Linguistic Transcription and Phonological Representation in the Huìtóngguǎnxì Huáyíyìyǔ](https://arxiv.org/abs/2605.14480)
*Ji-eun Kim*

Main category: cs.CL

TL;DR: 本文研究了明代《会同馆华夷译语》（HHY）多语种词汇集作为转录系统的内部结构和规律，发现其以汉字系统性地转录非汉语口语。


<details>
  <summary>Details</summary>
Motivation: 以往研究多将HHY视为孤立语言材料，本研究旨在揭示其作为连贯多语转录系统的统一原则和语音近似机制。

Method: 数字化大部分HHY文本，对齐中古音韵范畴；结合各语言部分的前人构拟，建立统一比较数据库，分析主转录（MT）和辅助转录（ST）的跨语言规律。

Result: 主转录（MT）主要体现符合汉语音节结构的声音，辅助转录（ST）编码与汉语音系不兼容的语音特征；汉语音韵范畴在转录中具有比以往假设更大的灵活性，HHY是一种系统性的语音近似方法。

Conclusion: HHY可视为内部结构的转录系统，而非简单词汇集；此类历史转录系统能为历史音韵学，特别是缺乏记录的亚洲语言提供宝贵证据。
...
### [281] [GroupMemBench: Benchmarking LLM Agent Memory in Multi-Party Conversations](https://arxiv.org/abs/2605.14498)
*Jingbo Yang,Kwei-Herng Lai,Xiaowen Wang,Shiyu Chang,Yaar Harari,Evgeniy Gabrilovich*

Main category: cs.CL

TL;DR: 提出GroupMemBench基准，评估大模型智能体在多用户场景下的群体记忆能力，现有记忆系统准确率仅46%，远低于需求。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统与基准均基于单用户对话，无法衡量多用户场景下的群体动态、说话者信念追踪和受众适应语言等关键属性。

Method: 构建图引导合成管道生成多轮多用户对话，并设计对抗性查询管道生成六类推理问题，测试记忆系统的综合能力。

Result: 最强记忆系统平均准确率仅46.0%，知识更新27.1%，术语歧义37.7%，简单BM25基线甚至超越多数智能体记忆系统。

Conclusion: 当前记忆摄入方法丢失群体记忆依赖的结构和词汇特征，多用户记忆问题远未解决。
...
### [282] [Language Generation as Optimal Control: Closed-Loop Diffusion in Latent Control Space](https://arxiv.org/abs/2605.14531)
*ZiYi Dong,Yuliang Huang,Weijian Deng,Xiangyang Ji,Liang Lin,Pengxu Wei*

Main category: cs.CL

TL;DR: 将语言生成重新表述为随机最优控制问题，通过HJB方程近似求解得到闭环控制器，利用流匹配和全局积分算子实现高效高保真文本生成。


<details>
  <summary>Details</summary>
Motivation: 解决自回归和扩散模型在效率-保真度悖论、不可逆误差传播、优化可处理性与保真度方面的局限性。

Method: 将语言生成视为随机最优控制问题，近似求解HJB方程得到最优策略，采用流匹配作为最优轨迹求解器，结合全局积分算子近似全局向量场。

Result: 在语言建模和条件生成任务上取得强性能，同时展现改进的稳定性、效率和可控性。

Conclusion: 提出的Manta-LM模型实现了高保真文本生成与低代价并行采样的统一。
...
### [283] [Dimension-Level Intent Fidelity Evaluation for Large Language Models: Evidence from Structured Prompt Ablation](https://arxiv.org/abs/2605.14517)
*GAng Peng*

Main category: cs.CL

TL;DR: 提出维度级意图保真评估框架，揭示了系统中结构恢复与意图保真的系统性分裂，并证明维度保真分数比整体分数更可靠。


<details>
  <summary>Details</summary>
Motivation: 整体评估分数无法区分模型是否重现了用户请求的结构形式与是否保留了用户的具体意图。

Method: 通过结构化提示消融研究（2880个输出，涵盖三种语言、三个任务域和六个LLM），分别测量每个语义维度的结构恢复和意图保真，并结合人类评估、公共-私有分解、代理标注和权重扰动实验。

Result: 中文输出中25.7%的整体对齐完美（GA=5）但存在维度意图缺陷，英文输出该比例升至58.6%；维度保真分数比整体分数更可靠；中等错位通常被吸收，严重维度反转始终有害。

Conclusion: 维度级意图保真评估是用户特定任务中整体评估的必要补充。
...
### [284] [Learning from Failures: Correction-Oriented Policy Optimization with Verifiable Rewards](https://arxiv.org/abs/2605.14539)
*Mengjie Ren,Jie Lou,Boxi Cao,Xueru Wen,Hongyu Lin,Xianpei Han,Le Sun,Xing Yu,Yaojie Lu*

Main category: cs.CL

TL;DR: 提出CIPO方法，利用模型自身的失败轨迹生成修正样本，与标准RLVR目标联合优化，在11个基准上显著提升推理与修正能力。


<details>
  <summary>Details</summary>
Motivation: RLVR受限于稀疏二元奖励和弱信用分配，导致优化信号模糊且未能充分利用失败轨迹中的有用信息。

Method: 将在线策略失败轨迹转换为修正导向的监督信号，与标准RLVR目标联合优化，无需外部信号。

Result: 在11个数学推理和代码生成基准上持续且显著优于强基线，同时提升pass@K指标，表明增强了内在推理能力而非仅重分配概率。

Conclusion: CIPO能有效提升模型推理与自我修正能力，是一种简单有效的RLVR扩展。
...
### [285] [Uncertainty Quantification for Large Language Diffusion Models](https://arxiv.org/abs/2605.14570)
*Artem Vazhentsev,Vladislav Smirnov,David Li,Maxim Panov,Timothy Baldwin,Artem Shelmanov*

Main category: cs.CL

TL;DR: 提出首个面向大语言扩散模型的不确定性量化框架，在高效推理的同时实现幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法依赖自回归分解或多次采样，与扩散模型的高并行推理范式不匹配。

Method: 利用迭代去噪中的中间生成、令牌重遮罩动态和去噪复杂度构造零样本不确定性信号，并融合掩码扩散似然与轨迹语义差异。

Result: 在多项任务中逼近最强采样基线，计算开销降低高达100倍。

Conclusion: 大语言扩散模型能同时实现快速推理和可靠幻觉检测。
...
### [286] [EndPrompt: Efficient Long-Context Extension via Terminal Anchoring](https://arxiv.org/abs/2605.14589)
...
TL;DR: 本文提出EndPrompt方法，仅用短训练序列即可扩展大语言模型的上下文窗口至64K，性能超越全长度微调等方法。


<details>
  <summary>Details</summary>
Motivation: 长上下文扩展通常需在目标长度序列上训练，导致二次内存和计算成本高昂，亟需高效且可复现的方案。

Method: 通过两段式结构：保留原始短上下文作为第一段，附加低位终端提示作为第二段，模拟长程相对位置距离，并基于RoPE和Bernstein不等式进行理论分析。

Result: 在LLaMA系列模型上从8K扩展至64K，平均RULER得分76.03，LongBench平均得分最高，超过LCEG、LongLoRA及全长度微调。

Conclusion: 长上下文泛化可从稀疏位置监督中诱导，挑战了需要密集长序列训练的传统假设。
...
### [287] [SciPaths: Forecasting Pathways to Scientific Discovery](https://arxiv.org/abs/2605.14600)
*Eric Chamoun,Yizhou Chi,Yulong Chen,Rui Cao,Zifeng Ding,Michalis Korakakis,Andreas Vlachos*

Main category: cs.CL

TL;DR: 本文提出了发现路径预测任务，构建了SciPaths基准（262条专家标注路径和2,444条银标准路径），并发现最佳模型仅达到0.189 F1分数，核心方法依赖最难恢复。


<details>
  <summary>Details</summary>
Motivation: 现有AI4Science基准聚焦于引用预测、文献检索或思想生成，而非实现科学进步所依赖的使能贡献序列。

Method: 构建发现路径预测基准SciPaths，包含专家标注和自动构建的路径数据，并评估前沿及开源语言模型的表现。

Result: 最佳模型在严格语义匹配下仅达0.189 F1；提供黄金使能贡献时，先前工作定位显著提升，但分解质量仍是端到端路径恢复的主要瓶颈。

Conclusion: SciPaths将评估转向科学预测中缺失的能力：从目标贡献反向推理到使能科学构建块及先前工作依赖。
...
### [288] [AI-assisted cultural heritage dissemination: Comparing NMT and glossary-augmented LLM translation in rock art documents](https://arxiv.org/abs/2605.14679)
*Vicent Briva-Iglesias,María Ferre-Fernández*

Main category: cs.CL

TL;DR: 本文比较了三种机器翻译设置（DeepL、Gemini-Simple和Gemini-RAG）在西班牙语岩石艺术学术文本英译中的效果，发现基于词汇检索增强的Gemini-RAG在术语准确率（81.4%）上显著优于其他方法，且整体质量持平。


<details>
  <summary>Details</summary>
Motivation: 文化遗产机构需多语言传播研究材料，但术语密集领域（如岩石艺术）的翻译常因预算和人力有限而质量不佳，小错误会误导非专业用户并降低材料重用性。

Method: 采用三种英译设置：DeepL（强神经机器翻译基线）、Gemini-Simple（大语言模型+简单提示）和Gemini-RAG（同模型+词汇增强提示）。通过人工评估，包括多方位直接评估（0-100分）和受限MQM术语审计。

Result: Gemini-RAG的术语精确匹配准确率最高（81.4%），优于Gemini-Simple（69.1%）和DeepL（64.4%）；整体质量（平均DA 85.3）与Gemini-Simple（85.2）相近，超过DeepL（80.3）。

Conclusion: 词汇增强提示是一种低开销的方法，可有效提升文化遗产翻译中的术语控制，前提是机构维护最小术语资源和轻量级评估流程。
...
### [289] [Video2GUI: Synthesizing Large-Scale Interaction Trajectories for Generalized GUI Agent Pretraining](https://arxiv.org/abs/2605.14747)
*Weimin Xiong,Shuhao Gu,Bowen Ye,Zihao Yue,Lei Li,Feifan Song,Sujian Li,Hao Tian*

Main category: cs.CL

TL;DR: 提出了一个从无标注互联网视频中自动提取GUI交互轨迹的框架Video2GUI，并构建了包含1200万条轨迹的大规模数据集WildGUI，预训练模型在多个GUI任务上取得5-20%的提升。


<details>
  <summary>Details</summary>
Motivation: 现有GUI数据集依赖昂贵的人工标注，且局限于狭窄领域，限制了多模态大模型在GUI智能体上的泛化能力。

Method: 采用粗到细的过滤策略从500M视频元数据中筛选高质量GUI教程视频，并将其自动转化为结构化的智能体轨迹。

Result: 在WildGUI上预训练Qwen2.5-VL和Mimo-VL，在多个GUI基准测试上取得5-20%的一致提升，达到或超越现有最优性能。

Conclusion: Video2GUI和WildGUI有效解决了GUI训练数据稀缺问题，为GUI智能体的研究提供了大规模、多样化的数据资源。
...
### [290] [Non-linear Interventions on Large Language Models](https://arxiv.org/abs/2605.14749)
*Sangwoo Kim*

Main category: cs.CL

TL;DR: 提出一种可处理非线性表征特征和隐式特征的通用干预框架，在拒绝绕过引导任务上比线性方法更精确。


<details>
  <summary>Details</summary>
Motivation: 现有干预方法局限于线性表征假设，无法处理非线性流形编码的特征，且无法干预无直接输出签名的隐式特征。

Method: 设计通用干预公式扩展至非线性表征特征，并开发学习过程使干预可应用于隐式特征。

Result: 在拒绝绕过引导任务中，该方法比线性基线更精确地控制模型行为。

Conclusion: 干预方法可突破线性假设限制，有效处理非线性与隐式特征，提升模型控制精度。
...
### [291] [Streaming Speech-to-Text Translation with a SpeechLLM](https://arxiv.org/abs/2605.14766)
*Titouan Parcollet,Shucong Zhang,Xianrui Zheng,Rogier C. van Dalen*

Main category: cs.CL

TL;DR: 提出基于LLM的实时流式语音翻译架构，LLM自主决策何时输出翻译，翻译质量接近非流式基线，延迟仅1-2秒


<details>
  <summary>Details</summary>
Motivation: 现有SpeechLLM系统要么等待完整语音再输出，要么固定间隔输出，不适合实时应用

Method: 利用输入语音与输出文本的自动对齐训练，LLM同时学习输出翻译和决定何时已收集足够音频

Result: 多语言实验显示翻译质量接近非流式基线，延迟仅1-2秒

Conclusion: 该架构能实现低延迟实时流式语音翻译，且保持良好翻译质量
...
### [292] [Mechanical Enforcement for LLM Governance:Evidence of Governance-Task Decoupling in Financial Decision Systems](https://arxiv.org/abs/2605.14744)
*José Manuel de la Chica Rodríguez,Carlos Martí-González*

Main category: cs.CL

TL;DR: 提出了五条治理指标来量化政策合规性，发现在纯文本治理下27%的延期决策未包含有效信息；通过机械强制（四种原语操作）可降低73%的无效延期，将任务准确率从MCC 0.43提升至0.88，且因果消融实验证明每种原语都是必要的。核心发现是治理与任务存在解耦现象：结构压力下纯文本治理同时降低两方面性能，而机械强制能保持治理质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在受监管金融流程中需遵循自然语言政策，但存在委托-代理失灵问题：输出看似合规实则不然。现有评估只关注任务准确性，未从决策原因层面验证治理是否真正约束了行为，而受监管决策必须可审计。

Method: 提出了五项治理指标（Governance Metrics）量化决策原因层的政策合规性；在合成银行领域对比纯文本治理与机械强制（四种在模型解释循环之外运行的原语）。进行了因果消融实验确认每种原语的必要性。

Result: 纯文本治理下27%的延期不包含决策相关信息；机械强制使该比率降低73%，延期信息含量翻倍以上，任务准确率从MCC 0.43提升至0.88。机械强制下LLM生成的原因与纯文本治理相比CDL相当，改进源于将清晰决策从模型控制中抽离。因果消融证实每种原语单独必要。核心发现：治理与任务解耦，结构压力下纯文本治理两维同时退化，而机械强制保持治理质量。

Conclusion: 治理与任务评估是不同维度：准确性不能充分替代治理指标；在受监管AI系统中需单独评估政策合规性。机械强制通过架构分离（原语操作）确保治理约束，避免委托-代理问题。
...
### [293] [Conversion of Lexicon-Grammar tables to LMF. Application to French](https://arxiv.org/abs/2605.14816)
*Eric Laporte,Elsa Tolone,Mathieu Constant*

Main category: cs.CL

TL;DR: 本文报告了将法语动词的词汇-语法表转换为词汇标记框架格式的首次实验。


<details>
  <summary>Details</summary>
Motivation: 词汇-语法是法语词汇和句法信息的重要来源，将其转换为LMF标准格式可提高互操作性。

Method: 分析转换过程中的主要困难，并描述最终生成的资源。

Result: 成功实现了词汇-语法表到LMF格式的转换，提高了词典的标准化和互操作性。

Conclusion: 转换后的资源可用于不同自然语言处理环境，促进词典标准化。
...
### [294] [Graphs of Research: Citation Evolution Graphs as Supervision for Research Idea Generation](https://arxiv.org/abs/2605.14790)
*Songyang Gao,Yinghui Xia,Siyi Liu,Hui Xiong*

Main category: cs.CL

TL;DR: 提出了一种基于引文演化有向无环图的监督微调方法GoR，用于提升大语言模型自动生成科研想法的能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态检索或复杂提示工程，忽略了文献间的结构关系，限制了LLM生成创新想法的效果。

Method: GoR方法提取每篇种子论文的2跳引用邻居，从引文位置、频率、前驱链接和出版时间推导关系，构建论文演化DAG；并构建自动化提取流程，使用Qwen2.5-7B-Instruct-1M在结构化文本提示上进行监督微调。

Result: 在与gpt-4o驱动的基线对比的LLM评估锦标赛中，GoR-SFT取得最优结果，证明了引文演化图作为监督信号的有效性。

Conclusion: GoR通过引入引文结构关系作为监督，有效提升了LLM的科研想法生成能力，有望降低引文演化图的使用门槛，加速自动化科学创新。
...
### [295] [Chain-of-Procedure: Hierarchical Visual-Language Reasoning for Procedural QA](https://arxiv.org/abs/2605.14928)
*Guanhua Chen,Yutong Yao,Shenghe Sun,Ci-Jun Gao,Shudong Liu,Lidia S. Chao,Feng Wan,Derek F. Wong*

Main category: cs.CL

TL;DR: 提出ProcedureVQA基准和Chain-of-Procedure推理框架，在视觉程序问答任务上提升VLMs性能最高13%。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在视觉程序问答中面临检索对齐不足和粒度不匹配问题。

Method: 通过视觉线索检索指令，语义分解细化步骤，生成下一步的层次化推理框架CoP。

Result: 在六个VLM上实现最高13%的绝对性能提升。

Conclusion: CoP有效解决了VLM在程序推理中的关键限制。
...
### [296] [Tokenizer Fertility and Zero-Shot Performance of Foundation Models on Ukrainian Legal Text: A Comparative Study](https://arxiv.org/abs/2605.14890)
*Volodymyr Ovcharov*

Main category: cs.CL

TL;DR: 本文系统比较了七种基础模型在乌克兰法律文本上的标记器效率与零样本性能，发现标记器效率差异可达1.6倍，NVIDIA Nemotron Super 3在成本仅为三分之一的情况下表现最佳，而少样本提示会显著降低性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对乌克兰法律文本领域基础模型标记器效率的系统比较，影响了实际部署中的模型选择与成本优化。

Method: 选取了五家提供商的七种模型，使用乌克兰国家法院登记处的273份有效判决书，测量标记器效率，并在三个任务上进行零样本性能评估。

Result: (1)标记器效率差异达1.6倍，Qwen3模型比Llama模型多消耗60%的令牌；(2)NVIDIA Nemotron Super 3（120B）综合得分最高（83.1），以三分之一的API成本超越Mistral Large 3（675B总参数、41B活跃参数）；(3)少样本提示使性能下降高达26个百分点，且该现象与示例选择无关。

Conclusion: 对于形态丰富的语言，应优先分析标记器效率后再选择模型，且零样本提示比少样本提示更可靠。
...
### [297] [Performance-Driven Policy Optimization for Speculative Decoding with Adaptive Windowing](https://arxiv.org/abs/2605.14978)
*Jie Jiang,Xing Sun*

Main category: cs.CL

TL;DR: 提出PPOW框架，通过窗口级强化学习优化投机解码的草稿模型，提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 传统草稿训练聚焦于词级模仿，忽视了投机解码对窗口级和前缀敏感性的需求，导致硬位置成为瓶颈。

Method: 使用PPOW框架，结合成本感知加速奖励、分布邻近奖励和自适应分歧感知窗口化，进行窗口级策略优化。

Result: 在多个模型和基准上，平均接受长度达6.29-6.52，加速比达3.39-4.36倍。
...
### [298] [Quantifying and Mitigating Premature Closure in Frontier LLMs](https://arxiv.org/abs/2605.15000)
*Rebecca Handler,Suhana Bedi,Nigam Shah*

Main category: cs.CL

TL;DR: 本研究定义了大型语言模型（LLM）中的“过早闭合”概念，并评估了五种前沿模型在医疗任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 过早闭合是诊断错误的重要原因，但在LLM中尚未被充分研究。

Method: 通过MedQA、AfriMed-QA、HealthBench和医生编写的对抗性查询，进行结构化和开放式医疗任务评估。

Result: 模型在无正确答案时仍高频率选择答案，基线错误率为55-82%；安全提示可减少但无法消除错误。

Conclusion: 医疗LLM需要评估何时不应回答，以降低诊断错误风险。
...
### [299] [The Scientific Contribution Graph: Automated Literature-based Technological Roadmapping at Scale](https://arxiv.org/abs/2605.15011)
*Peter A. Jansen*

Main category: cs.CL

TL;DR: 本文提出了自动技术路线图任务，通过构建包含200万科学贡献和1250万先决条件边的大规模科学贡献图谱（SCG），并引入科学先决条件预测任务，验证了现有模型在该任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 科学贡献很少独立发展，而是基于先前发现；现有的文献分析缺乏系统化地提取和关联科学贡献之间的先决关系。

Method: 从23万篇开放获取论文中提取详细的科学贡献，构建科学贡献图谱（SCG），并基于时间过滤的回测评估先决条件预测任务。

Result: 模型在先决条件预测任务上表现快速提升，在时间过滤回测中达到0.48的平均精度均值（MAP）。

Conclusion: 技术路线图资源（如SCG）将有助于科学影响评估和自动化科学发现。
...
### [300] [COTCAgent: Preventive Consultation via Probabilistic Chain-of-Thought Completion](https://arxiv.org/abs/2605.15016)
*Zihan Deng,Xiaozhen Zhong,Chuanzhi Xu*

Main category: cs.CL

TL;DR: 提出了COTCAgent框架，通过概率链式思维完成代理，提升纵向电子健康记录中的临床推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在纵向EHR推理中存在细粒度统计推理不足和时间序列非均匀等问题，误导诊断。

Method: 包含三个模块：Temporal-Statistics Adapter（TSA）生成可执行代码，Chain-of-Thought Completion（COTC）层利用知识库评分，以及有界完成模块确保严谨推理。

Result: COTCAgent在自建数据集上达90.47% Top-1准确率，HealthBench上为70.41%，优于现有医疗代理和主流模型。

Conclusion: 该框架通过解耦统计计算、特征匹配和语言生成，高效实现了纵向记录分析。
...
### [301] [From Scenes to Elements: Multi-Granularity Evidence Retrieval for Verifiable Multimodal RAG](https://arxiv.org/abs/2605.15019)
*Guanhua Chen,Chuyue Huang,Yutong Yao,Shudong Liu,Xueqing Song,Lidia S. Chao,Derek F. Wong*

Main category: cs.CL

TL;DR: 提出GranuVistaVQA基准和GranuRAG框架，通过元素级检索解决多模态RAG系统的细粒度查询不匹配问题，实现29.2%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有多模态RAG系统以粗粒度（整图或场景）检索证据，无法匹配细粒度用户查询且错误难以验证。

Method: 构建包含地标元素级标注的多视角VQA基准；提出多粒度RAG框架，包括元素检测分类、跨模态对齐检索和归因约束生成。

Result: 在六个强基线中，GranuRAG最高提升29.2%的性能，且支持透明错误诊断。

Conclusion: 元素级检索能有效解决粗粒度与细粒度查询的匹配问题，同时提高可解释性。
...
### [302] [Concurrency without Model Changes: Future-based Asynchronous Function Calling for LLMs](https://arxiv.org/abs/2605.15077)
*Guangyu Feng,Huanzhi Mao,Prabal Dutta,Joseph E. Gonzalez*

Main category: cs.CL

TL;DR: 提出了AsyncFC框架，通过解耦LLM解码与函数执行，实现重叠和并行处理，减少端到端延迟，同时保持任务准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM函数调用受同步执行语义限制，解码需等待函数完成，导致延迟增加。

Method: 设计纯执行层框架AsyncFC，无需修改模型或函数实现，支持解码与执行重叠及函数间并行，基于标准异步协议。

Result: 在基准测试和软件工程任务中显著降低完成时间，且精度不变；发现LLM能原生推理未解决的未来结果。

Conclusion: 异步函数调用能有效提升效率，LLM具备处理符号未来的能力，适合模型-工具异步交互。
...
### [303] [AI Knows When It's Being Watched: Functional Strategic Action and Contextual Register Modulation in Large Language Models](https://arxiv.org/abs/2605.15034)
*Vinicius Covas,Jorge Alberto Hidalgo Toledo*

Main category: cs.CL

TL;DR: 这项研究发现，基于大型语言模型的多智能体系统在感知社会观察时会表现出系统性的语言调整，且对观察者身份（人类 vs. AI）敏感。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在计算和认知角度已被广泛研究，但它们在结构化社会背景中作为沟通行为者的表现尚未被充分探索，这对AI治理和审计有直接意义。

Method: 通过一个包含100场多智能体辩论的对照实验（五种条件，每条件20场），改变社会观察的表述方式（从人类研究者明确监控到AI自动监控等），并测量语言适应情况。

Result: 监控条件（Delta+24.9%, Delta+24.2%）和AI监控条件（Delta+22.2%）比观众框架条件（Delta+17.7%）产生更高的词汇多样性变化率，F(4,94)=2.79, p=.031；消息长度呈现完全分离的效果，F(4,95)=19.55, p<.001；将人类观察者替换为AI观察者时产生中间的词汇适应性。
...
### [304] [ML-Embed: Inclusive and Efficient Embeddings for a Multilingual World](https://arxiv.org/abs/2605.15081)
*Ziyin Zhang,Zihan Liao,Hang Yu,Peng Di,Rui Wang*

Main category: cs.CL

TL;DR: 本文提出 ML-Embed 模型套件，通过三维俄罗斯套娃学习框架解决文本嵌入中的计算成本高、语言覆盖窄和透明度低三大问题，在 430 项任务中取得优异表现，并开源所有资源。


<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入模型存在计算成本高、忽视多数语言、封闭或不透明阻碍研究的问题，需要更公平高效的方案。

Method: 引入三维俄罗斯套娃学习框架，包括传统 MRL 存储效率、MLL 推理深度灵活性，以及新提出的 MEL 参数效率；构建多语言数据集并训练 140M 到 8B 参数的模型。

Result: 在 17 个 MTEB 基准的 9 个上创下新纪录，尤其在低资源语言上表现突出，在 430 个任务中全面评估优异。

Conclusion: ML-Embed 通过开放数据和代码，为构建全球公平且计算高效的 AI 系统提供了可复现的蓝图。
...
### [305] [Improving Multi-turn Dialogue Consistency with Self-Recall Thinking](https://arxiv.org/abs/2605.15102)
*Renning Pang,Tian Lan,Leyuan Liu,Xiaoming Huang,Piao Tong,Xiaosong Zhang*

Main category: cs.CL

TL;DR: 提出自我回忆思考框架，通过识别历史对话轮次并生成自回忆链，在不使用外部模块的情况下提升多轮对话的长距离依赖处理能力和推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多轮对话系统无法有效跟踪非相邻轮次依赖，长对话中关键信息稀疏，处理全部历史效率低。

Method: 包括依赖构建（生成自回忆链）、能力初始化（训练回忆令牌）、推理改进（基于可验证奖励优化）。

Result: 在多个数据集上F1分数提升4.7%，端到端延迟降低14.7%，优于现有方法。

Conclusion: SRT实现了推理延迟与准确率之间的平衡，有效处理长距离上下文依赖和稀疏信息。
...
### [306] [MeMo: Memory as a Model](https://arxiv.org/abs/2605.15156)
...
TL;DR: 提出MeMo框架，将新知识编码为专用记忆模型，不改变LLM参数，实现高效知识整合。


<details>
  <summary>Details</summary>
Motivation: LLM预训练后无法更新，需高效机制引入及时、领域特定知识。

Method: MeMo模块化框架，通过独立记忆模型编码跨文档关系，无需访问LLM权重或输出logits，检索代价与语料规模无关。

Result: 在BrowseComp-Plus、NarrativeQA、MuSiQue三个基准测试中表现优于现有方法。

Conclusion: MeMo能有效避免灾难性遗忘、抵抗检索噪声，兼容开源和闭源LLM，适合实时知识注入场景。
...
### [307] [From Text to Voice: A Reproducible and Verifiable Framework for Evaluating Tool Calling LLM Agents](https://arxiv.org/abs/2605.15104)
*Md Tahmid Rahman Laskar,Xue-Yong Fu,Seyyed Saeed Sarfjoo,Quinten McNamara,Jonas Robertson,Shashi Bhushan TN*

Main category: cs.CL

TL;DR: 提出了一种将文本工具调用基准转换为音频评估的框架，并通过对7个模型在噪音和说话者变化下的测试，验证了该方法在隐私保护和可靠性方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 语音代理需要可靠的音频工具调用能力，但现有基准多为文本，缺乏可复现的音频评估方法。

Method: 使用文本转语音、说话者变化和背景噪声创建文本-音频配对实例，保持原始标注不变，并引入LLM作为无参考判断器。

Result: 模型表现依赖任务和模型，Gemini-3.1-Flash-Live在Confetti上得分最高（70.4），GPT-Realtime-1.5在When2Call上最佳（71.9）；文本到语音的差距在1.8到4.8点之间；Qwen3（≥8B参数）与专有判断器一致性超过80%。

Conclusion: 该框架作为可验证的诊断工具，有效补充了专用音频语料库，支持隐私保护评估。
...
### [308] [Text Knows What, Tables Know When: Clinical Timeline Reconstruction via Retrieval-Augmented Multimodal Alignment](https://arxiv.org/abs/2605.15168)
*Sayantan Kumar,Shahriar Noroozizadeh,Juyong Kim,Jeremy C. Weiss*

Main category: cs.CL

TL;DR: 提出了一种检索增强的多模态对齐框架，通过结合非结构化临床文本和结构化电子健康记录数据，提高临床时间线的绝对时间戳精度和时序一致性。


<details>
  <summary>Details</summary>
Motivation: 临床文本虽然语义丰富但缺乏时间精度，而结构化数据具有精确时间锚点但遗漏了大量临床事件，现有方法难以同时保证时间精确性和事件覆盖率。

Method: 基于图的多阶段流程：从文本中提取锚定事件构建时间骨架，将非锚定事件相对放置，然后利用检索的结构化EHR数据作为外部证据校准时间线。

Result: 在MIMIC-III和IV基准上，多模态管道显著提升了绝对时间戳准确性（AULTC）和时序一致性，且不降低事件匹配率。34.8%的文本事件在表格记录中完全缺失。

Conclusion: 对齐文本和结构化数据能产生比单一数据源更时间忠实和临床信息丰富的患者轨迹重建。
...
### [309] [Is Grep All You Need? How Agent Harnesses Reshape Agentic Search](https://arxiv.org/abs/2605.15184)
*Sahil Sen,Akhil Kasturi,Elias Lumer,Anmol Gulati,Vamse Kumar Subbiah*

Main category: cs.CL

TL;DR: 本文通过两个实验系统比较了LLM代理中grep和向量检索的准确性，发现grep通常优于向量检索，但性能受代理架构和工具调用范式影响显著。


<details>
  <summary>Details</summary>
Motivation: 现有文献缺乏对检索策略选择、代理架构和工具调用范式之间交互的系统比较，尤其是在工具输出呈现方式和无关文本干扰影响方面。

Method: 实验1：在LongMemEval的116个问题上比较grep和向量检索，使用自定义代理框架（Chronos）和提供商CLI（Claude Code、Codex、Gemini CLI），比较内联和基于文件的工具结果；实验2：混合无关对话历史，逐步增加干扰，比较仅grep和仅向量检索。

Result: 实验1中，grep一般比向量检索更准确，但准确性高度依赖于代理框架和工具调用风格；实验2中，增加干扰后grep表现更稳健。

Conclusion: 检索策略选择需结合代理架构和工具调用范式优化，grep在准确性和鲁棒性上表现出优势，但整体性能受具体实现影响。
...
### [310] [Hidden State Poisoning Attacks against Mamba-based Language Models](https://arxiv.org/abs/2601.01972)
*Alexandre Le Mercier,Chris Develder,Thomas Demeester*

Main category: cs.CL

TL;DR: 本文发现状态空间模型（如Mamba）存在隐藏状态中毒攻击（HiSPA），导致信息不可逆覆盖，而Transformer不受影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽略SSMs的对抗鲁棒性，需要评估其信息检索漏洞。

Method: 构建RoBench-25基准，对比SSMs与Transformer在HiSPA下的表现，并通过可解释性分析隐藏层模式。

Result: SSMs（包括混合模型）在HiSPA攻击下信息检索能力崩溃，而纯Transformer保持稳定；触发词会显著削弱Jamba模型性能。

Conclusion: SSMs存在独特脆弱性，但隐藏层模式可指导防御系统设计。
...
### [311] [Behavior Cloning for Active Perception with Low-Resolution Egocentric Vision](https://arxiv.org/abs/2605.14106)
*Anthony Bilic,Chen Chen,Ladislau Bölöni*

Main category: cs.RO

TL;DR: 本文研究行为克隆是否能在结构化物体寻找任务中产生主动感知，使用低分辨率RGB图像预测关节命令，验证主动感知可从行为克隆中涌现。


<details>
  <summary>Details</summary>
Motivation: 探究在低成本机器人系统中行为克隆是否足以实现主动感知，即机器人通过动作改善未来观测。

Method: 采用行为克隆模型，从低分辨率第一人称RGB图像直接预测连续控制命令（相对关节增量优于绝对位置预测）。

Result: 低分辨率视觉足以可靠完成任务，相对关节增量预测显著优于绝对位置预测。

Conclusion: 视觉引导的主动感知可通过行为克隆在可重复环境中涌现。
...
### [312] [Ergodic Imitation for Adaptive Exploration around Demonstrations](https://arxiv.org/abs/2605.13996)
*Ziyi Xu,Cem Bilaloglu,Yiming Li,Sylvain Calinon*

Main category: cs.RO

TL;DR: 提出自适应遍历模仿方法，通过演示构建目标分布，在跟踪与探索间自适应插值，应对模仿学习中训练与部署条件不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 模仿学习中训练与部署条件不匹配（如环境变化、观测或控制不完美）导致机器人沿名义轨迹执行任务时陷入僵局，需自适应在线探索策略。

Method: 将遍历控制扩展至自适应模仿，从演示几何中构建目标分布，结合检索式滚动时域框架生成轨迹，实现跟踪与探索的自适应插值。

Result: 所提方法能有效应对条件不匹配，通过自适应探索避免任务失败。

Conclusion: 该方法拓展了遍历控制在覆盖搜索外的应用，为自适应模仿学习提供了一种基于演示的在线探索框架。
...
### [313] [Towards Robotic Dexterous Hand Intelligence: A Survey](https://arxiv.org/abs/2605.13925)
*Weiguang Zhao,Xihao Guo,Tian Liang,Rui Zhang,Irwin King,Kaizhu Huang*

Main category: cs.RO

TL;DR: 该综述全面回顾了机器人灵巧手的研究，从硬件、控制方法、数据集与评估四个维度进行了系统梳理，并指出了当前研究的局限与未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有灵巧手研究在硬件形态、传感器配置、任务设置、训练数据和评估协议上存在差异，导致系统比较困难，模糊了领域发展轨迹。

Method: 从硬件层面分析驱动、传动、感知及代表性手的设计权衡；从方法论层面回顾控制与学习方法，按范式和时间顺序分组；整合数据集、模态设计与评估实践。

Result: 揭示了灵巧手在力能力、柔顺性、带宽、集成度和系统复杂性之间的关键权衡，并理清了方法论的演进脉络。

Conclusion: 通过连接硬件、方法、数据与评估，该综述提供了结构化理解，并明确了未来研究最重要的开放挑战。
...
### [314] [Safety-Constrained Reinforcement Learning with Post-Training Reachability Verification for Robot Navigation](https://arxiv.org/abs/2605.14174)
*Qisong He,Xinmiao Huang,Jinwei Hu,Zhuoyun Li,Yi Dong,Changshun Wu,Xiaowei Huang*

Main category: cs.RO

TL;DR: 提出结合CVaR约束优化与神经网络可达性验证的安全强化学习框架，在导航任务中实现98.3%成功率并保障尾风险安全。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法仅关注平均累计代价，忽视高后果尾风险行为，导致在杂乱环境中的感知不确定性下可靠性不足。

Method: 基于离线TD3算法，采用条件风险价值约束优化策略，并通过泰勒模型分析计算动作可达集，量化安全裕度。

Result: 10个导航场景中达到98.3%成功率，安全验证率最高，且平均代价与可达性安全排名存在差异。

Conclusion: CVaR约束训练能提升策略安全裕度与形式化验证通过率，可达性分析可捕捉被传统代价指标忽略的风险，并在物理机器人上验证了仿真到现实的迁移能力。
...
### [315] [Motion Planning for Autonomous Vehicles using Optimization over Graphs of Convex Sets](https://arxiv.org/abs/2605.14199)
*Matheus Wagner,Antônio Augusto Fröhlich*

Main category: cs.RO

TL;DR: 本文研究基于凸集图优化近似自动驾驶中非线性最优控制问题的方法，通过贝塞尔曲线和多项式时间缩放参数化车辆运动，在CommonRoad场景中验证了计算效率和初始化鲁棒性的提升。


<details>
  <summary>Details</summary>
Motivation: 非线性最优控制计算成本高且对初始化敏感，而几何规划方法将路径选择与轨迹优化解耦，需要一种能兼顾两者优势的近似方法。

Method: 将自由空间表示为有向图上的凸集并集，使用贝塞尔曲线和多项式时间缩放参数化轨迹，结合简化动力学模型通过凸约束近似动态可行性。

Result: 生成的轨迹无碰撞且动态一致，接近非线性规划结果，计算效率更高且对初始化不敏感。

Conclusion: 凸集图优化能结构化近似非线性运动规划问题，保持几何和动态主要特征的同时维持凸性。
...
### [316] [MAPLE: Latent Multi-Agent Play for End-to-End Autonomous Driving](https://arxiv.org/abs/2605.14201)
...
TL;DR: 提出了一个名为MAPLE的框架，在VLA模型的潜在空间中进行多智能体闭环训练，提升端到端自动驾驶的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在闭环环境中易失效，而传统闭环监督方法可扩展性差且无法充分建模反应性环境。

Method: 两阶段训练：监督微调基于真实轨迹的潜在展开，然后使用全局和特定智能体奖励进行强化学习，并加入多样性奖励以探索未记录行为。

Result: 在Bench2Drive上达到了最先进的驾驶性能，展示了可扩展的闭环多智能体规划。

Conclusion: MAPLE提供了一种无需外部模拟器的可扩展闭环训练方案，可增强自动驾驶系统的鲁棒性。
...
### [317] [Reactive Planning based Control for Mobile Robots in Obstacle-Cluttered Environments](https://arxiv.org/abs/2605.14232)
*Li Tan,Junlin Xiong,Yan Wang,Wei Ren*

Main category: cs.RO

TL;DR: 提出了一种基于反应式规划的控制策略，使机器人在部分环境信息下实现无碰撞运动


<details>
  <summary>Details</summary>
Motivation: 解决移动机器人在障碍密集环境中仅有部分环境信息时的运动控制问题

Method: 连接初始与目标位置作为参考轨迹，开发反应式规划策略局部修改轨迹避障，利用离散化技术设计自适应跟踪控制器

Result: 数值示例验证了所提策略的有效性与优势

Conclusion: 该策略可有效实现部分信息下的无碰撞导航
...
### [318] [Distill: Uncovering the True Intent behind Human-Robot Communication](https://arxiv.org/abs/2605.14262)
*Ting Li,David Porfirio*

Main category: cs.RO

TL;DR: 提出Distill方法，通过去冗余、泛化步骤和松弛顺序约束，从用户任务规约中提炼真实意图。


<details>
  <summary>Details</summary>
Motivation: 自然语言模糊且编程过于具体，难以完整捕捉用户意图。

Method: Distill方法：删除不必要步骤、泛化各步骤含义、调整步骤间顺序约束。在Web界面上实现并通过众包研究验证。

Result: 有效从初始任务规约中提炼和细化用户意图。

Conclusion: Distill改善了人机通信中意图理解问题。
...
### [319] [Energy-Efficient Quadruped Locomotion with Compliant Feet](https://arxiv.org/abs/2605.14411)
*Pramod Pal,Shishir Kolathaya,Ashitava Ghosal*

Main category: cs.RO

TL;DR: 本文研究在四足机器人足部加入适当弹性对行走效率的影响，发现中等刚度弹簧可降低约17%的能量消耗。


<details>
  <summary>Details</summary>
Motivation: 传统刚性足部虽控制简单，但无法吸收冲击和存储弹性能，导致能耗较高；探索柔性足对降低能量消耗的潜力。

Method: 为足部设置8种不同刚度弹簧，通过强化学习训练行走策略，并在仿真和真实机器人上交叉评估每米机械能耗。

Result: 中等刚度弹簧能耗最低，比极硬或极软弹簧降低约17%，仿真与实验趋势一致。

Conclusion: 选择合适的足部柔顺性可提高运动效率，同时不影响运动稳定性。
...
### [320] [Before the Body Moves: Learning Anticipatory Joint Intent for Language-Conditioned Humanoid Control](https://arxiv.org/abs/2605.14417)
...
TL;DR: 提出DAJI框架，通过分层结构与未来感知的动作表征，使类人机器人能基于自然语言指令生成预期性全身控制。


<details>
  <summary>Details</summary>
Motivation: 现有系统依赖反应式修复或缺乏对接触变化、平衡准备等物理过渡的显式编码，导致语言驱动的全身控制不流畅。

Method: DAJI-Act通过学生驱动展开蒸馏未来感知教师策略至扩散动作策略；DAJI-Flow利用语言和意图历史自回归生成未来意图块。

Result: 在HumanML3D生成任务达94.42% rollout成功率，BABEL上子序列FID为0.152。

Conclusion: DAJI在预期性潜在学习、单指令生成和流式指令跟随中表现优异，为语言-控制接口提供了有效方案。
...
### [321] [Let Robots Feel Your Touch: Visuo-Tactile Cortical Alignment for Embodied Mirror Resonance](https://arxiv.org/abs/2605.14571)
*Tianfang Zhu,Ning An,Rui Wang,Jiasi Gao,Qingming Luo,Anan Li,Guyue Zhou*

Main category: cs.RO

TL;DR: 提出了镜像触觉网络，通过多层级对齐视觉与触觉表征，使机器人能从RGB图像预测触觉信号，并实现类人的镜像触觉反应。


<details>
  <summary>Details</summary>
Motivation: 人类观察他人被触摸时会产生镜像触觉，但机器人缺乏实现这一现象的计算框架，阻碍了共情交互。

Method: 通过语义、分布和几何对齐约束，将视觉表征重塑为与触觉流形一致的几何结构，减少跨模态映射复杂度。

Result: 在机器人手上成功从RGB图像预测1140个触觉点的毫米级信号，并能跨域对人类手部触觉进行预测和反射响应。

Conclusion: 将视觉-触觉共振的神经原理转化为机器人感知，为预期触觉和共情的人机交互提供了可解释的途径。
...
### [322] [DSSP: Diffusion State Space Policy with Full-History Encoding](https://arxiv.org/abs/2605.14598)
*Zhiyuan Guan,Jianshu Hu,Han Fang,Yunpeng Jiang,Yize Huang,Shujia Li,Xiao Li,Yutong Ban*

Main category: cs.RO

TL;DR: 提出了DSSP，一种基于扩散状态空间模型的策略，通过高效的全历史条件编码和层级条件机制，显著提升了机器人操作任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模仿学习策略仅依赖当前或短窗口观测，难以解决长时域任务中的历史依赖歧义。

Method: 利用状态空间模型作为历史编码器压缩完整观测流，结合动力学感知辅助训练目标优化上下文表示，并采用SSM作为扩散主干以保持架构一致性和降低内存开销。

Result: 在仿真和真实机器人操作任务中，DSSP以更小的模型规模实现了最先进的性能，且历史长度增加时信息捕捉效率更高。

Conclusion: 层级条件机制能高效捕获长历史中的关键信息，为机器人操作中的历史依赖问题提供了有效解决方案。
...
### [323] [SeaVis: Modeling and Control of a Remotely Operated Towed Vehicle for Seabed Visualization and Mapping](https://arxiv.org/abs/2605.14683)
*Abdelhakim Amer,Aske Alstrup,Frederik Rasmussen,Yury Brodskiy,Andriy Sarabakha,Erdal Kayacan*

Main category: cs.RO

TL;DR: 提出了一种用于SeaVis远程操控拖曳式水下航行器(ROTV)的数学模型，并开发了增益调度线性二次型调节器(LQR)以实现稳健的深度与姿态控制。


<details>
  <summary>Details</summary>
Motivation: 高分辨率海底测绘需要水下机器人稳定且精确的定位。

Method: 建立了SeaVis ROTV的数学模型，并设计了增益调度LQR控制器。

Result: 在模拟中，LQR优于传统PID控制器，具有更强的抗干扰能力、更高的控制效率和更低的襟翼驱动需求，且增益调度确保了在全操作速度范围内的有效性。

Conclusion: 提出的LQR控制器在ROTV深度与姿态控制中性能优越，且相关仿真环境和控制器已开源。
...
### [324] [SR-Platform: An Agentic Pipeline for Natural Language-Driven Robot Simulation Environment Synthesis](https://arxiv.org/abs/2605.14700)
*Ben Wei Lim,Minh Duc Le,Thang Truong,Thanh Nguyen Canh*

Main category: cs.RO

TL;DR: 提出了SR-Platform，一个将自然语言描述自动转换为可执行的MuJoCo机器人仿真环境的代理系统，端到端延迟约50秒。


<details>
  <summary>Details</summary>
Motivation: 手动构建机器人仿真环境（如MuJoCo场景）需要3D建模、MJCF规范、空间布局等多领域专业知识，成为仿真学习的瓶颈。

Method: 将场景合成分解为四个阶段：基于LLM的编排器、资产锻造（检索或生成3D模型）、布局架构师（分配物体位姿）、桥接层（组装MJCF场景并集成机器人模型），系统部署为9个Docker服务。

Result: 基于30天生产数据（611次LLM调用），五物体场景中位延迟约50秒，缓存加速场景约30-40秒；资产锻造首次重试率11.3%，缓存可消除已生成物体类型的LLM调用。

Conclusion: 代理式场景合成能将创建多样化机器人训练环境的手动工作量降至一分钟内，用户可用自然语言快速生成执行用MuJoCo场景。
...
### [325] [IntentVLA: Short-Horizon Intent Modeling for Aliased Robot Manipulation](https://arxiv.org/abs/2605.14712)
...
TL;DR: 本文提出IntentVLA，一种历史条件化的视觉-语言-动作（VLA）框架，通过编码短期视觉观测为紧凑的意图表示来缓解多模态模仿数据中的跨块冲突，并在多个基准上提升执行稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有帧条件化VLA策略因部分可观测性，在相邻重规划步骤中可能重新采样不同意图，导致跨块冲突和执行不稳定。

Method: IntentVLA引入历史条件化机制，将近期视觉观测编码为短期意图表示，并以此作为条件生成动作块。

Result: 在AliasBench、SimplerEnv、LIBERO和RoboCasa上，IntentVLA提升了滚转稳定性并超越了强VLA基线。

Conclusion: 通过历史观测编码为意图表示，IntentVLA有效缓解了多模态模仿学习中的跨块冲突，提高了机器人策略的稳定性。
...
### [326] [Exploring Bottlenecks in VLM-LLM Navigation: How 3D Scene Understanding Capability Impacts Zero-Shot VLN](https://arxiv.org/abs/2605.14801)
*Ziyi Xia,Chaoran Xiong,Litao Wei,Xinhao Hu,Ling Pei*

Main category: cs.RO

TL;DR: 该研究量化了3D场景理解能力对零样本视觉-语言导航性能的影响，提出了成功率上限，并发现了感知饱和现象。


<details>
  <summary>Details</summary>
Motivation: 当前3D感知模型追求像素级精度，与导航对计算效率和实时性的要求存在矛盾，需要量化其实际影响。

Method: 基于典型的VLM-LLM框架，分别构建了慢速LLM规划器和快速反应导航器的统计成功率上限。

Result: 验证了提出的上限，并发现感知精度超过一定阈值后，导航成功率提升呈递减趋势。

Conclusion: 3D场景理解应转向导航相关的核心词汇和准确的边界框比例，而非严格像素级精度。
...
### [327] [Learning Cross-Coupled and Regime Dependent Dynamics for Aerial Manipulation](https://arxiv.org/abs/2605.14805)
*Rishabh Dev Yadav,Samaksh Ujjawal,Sihao Sun,Spandan Roy,Wei Pan*

Main category: cs.RO

TL;DR: 提出结构化编码器-解码器框架，用于空中机械臂的残差动力学自适应学习，结合非线性潜编码器与线性解码器实现耦合、历史依赖及非平稳动态的在线适应。


<details>
  <summary>Details</summary>
Motivation: 现有模型无法有效处理空中机械臂中强耦合、时滞气动交互及工况变化导致的非线性、非平稳残差动力学。

Method: 采用非线性潜编码器捕获状态输入历史中的跨变量耦合和时间依赖，轻量线性潜解码器通过闭合贝叶斯自适应和一致性驱动协方差膨胀实现在线适应，支持实时模型预测控制。

Result: 在真实平台上验证表明，残差预测精度提升，适应工况变化更快，轨迹跟踪性能增强。

Conclusion: 联合建模耦合时变动态与部署期非平稳性对实现可靠空中操控至关重要。
...
### [328] [CaMeRL: Collision-Aware and Memory-Enhanced Reinforcement Learning for UAV Navigation in Multi-Scale Obstacle Environments](https://arxiv.org/abs/2605.14810)
*Hong Hong,Feiyu Liao,Yongheng Liang,Boning Zhang,Haitao Wang,Hejun Wu*

Main category: cs.RO

TL;DR: 提出了一种针对多尺度障碍物的无人机避障导航强化学习框架CaMeRL，通过碰撞感知表示和时序记忆模块提升对大小障碍物的感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略障碍物尺度变化，仅提取单帧深度几何特征，导致小障碍物被忽视且大障碍物遮挡时失去空间上下文。

Method: 结合碰撞感知潜在表示（编码风险敏感深度线索以保留细粒度结构）和时序记忆模块（跨帧整合观测缓解大障碍物遮挡问题）。

Result: 在超小和超大障碍物设置中，成功率分别提升0.48和0.28，超越现有基线方法。

Conclusion: CaMeRL在多种尺度的障碍物环境下表现出可靠的导航性能，尤其在杂乱户外场景中有效。
...
### [329] [Learning Direct Control Policies with Flow Matching for Autonomous Driving](https://arxiv.org/abs/2605.14832)
...
TL;DR: 提出了一种基于流匹配的自动驾驶规划器，可直接输出加速度和曲率定义的控制轨迹，在少量ODE积分步骤内生成控制序列，实现低延迟闭环重规划。


<details>
  <summary>Details</summary>
Motivation: 现有方法对分布变化敏感，缺乏泛化能力，难以从简单模拟环境迁移到真实复杂城市场景。

Method: 以鸟瞰图BEV栅格为条件，训练流匹配模型直接预测控制轨迹，并在模拟城市数据上训练。

Result: 模型在分布内和分布外（如多车道高速公路）场景均表现稳定，成功完成任务，泛化能力强。

Conclusion: BEV表示和流匹配公式共同提升了模型对分布变化的鲁棒性与泛化性。
...
### [330] [Chrono-Gymnasium: An Open-Source, Gymnasium-Compatible Distributed Simulation Framework](https://arxiv.org/abs/2605.14911)
*Bocheng Zou,Harry Zhang,Khailanii Slaton,Jingquan Wang,Derrick Ruan,Huzaifa Mustafa Unjhawala,Radu Serban,Dan Negrut*

Main category: cs.RO

TL;DR: 提出了Chrono-Gymnasium，一个基于Ray的分布式计算框架，缩放高保真多体动力学仿真至大型计算集群，加速机器人学习和设计优化。


<details>
  <summary>Details</summary>
Motivation: 高保真物理仿真计算开销大，限制了在强化学习和全局优化等数据密集型任务中的使用。

Method: 基于Ray框架构建，提供标准Gymnasium接口，内置分布式执行所需的同步和消息机制。

Result: 通过两个案例（自主导航和行星着陆器设计优化）证明，该框架在不降低物理精度的前提下减少了墙钟时间。

Conclusion: Chrono-Gymnasium为复杂机器人系统的设计和控制提供了可扩展的高保真仿真路径。
...
### [331] [FU-MPC: Frontier- and Uncertainty-Aware Model Predictive Control for Efficient and Accurate UAV Exploration with Motorized LiDAR](https://arxiv.org/abs/2605.14920)
*Jianping Li,Pengfei Wan,Zhongyuan Liu,Yi Wang,Yiheng Chen,Xinhang Xu,Rui Jin,Boyu Zhou,Lihua Xie*

Main category: cs.RO

TL;DR: 提出了一种基于电机驱动旋转激光雷达的无人机分层探索框架，通过联合优化探索效用和定位不确定性，提高了探索效率和定位鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统无人机探索中，覆盖范围与运动紧密耦合，旋转动作降低效率且易导致定位退化；现有系统未充分利用旋转激光雷达的扫描自由度作为决策变量。

Method: 开发了独立驱动旋转激光雷达的无人机平台，包含全局规划器（拓扑感知视角序列化）和局部FU-MPC控制器（联合优化前沿探索效用与方向相关定位不确定性，轻量代理评估支持实时运行）。

Result: 在复杂环境中，相比固定扫描模式和仅考虑不确定性的基线方法，该系统提升了探索效率并保持了鲁棒的定位性能。

Conclusion: 主动旋转激光雷达扫描自由度的显式优化可有效平衡探索效率与定位质量，为未知环境中的安全导航提供了新方案。
...
### [332] [Behavioral Data-Driven Optimal Trajectory Generation for Rotary Cranes](https://arxiv.org/abs/2605.14944)
*Iskandar Khemakhem,Manuel Zobel,Johannes Schüle,Oliver Sawodny,Naoki Uchiyama,Abdallah Farrage*

Main category: cs.RO

TL;DR: 提出一种数据驱动的开环轨迹生成方法，用于旋转起重机在抑制负载摇摆的同时减少操作时间和能耗。


<details>
  <summary>Details</summary>
Motivation: 建筑业增长和熟练劳动力短缺使得起重机自动控制至关重要，现有方法依赖精确模型或大数据，难以满足安全和精度要求。

Method: 基于Willems基本引理及其推广，利用行为数据驱动框架，从测量输入输出数据中直接生成轨迹，避免显式建模，使用凸优化。

Result: 在实验室起重机实验中，与传统模型方法相比，负载摇摆减少35%，跟踪误差减少43%，运行时间减少50%。

Conclusion: 该方法在不依赖精确模型的情况下，通过有限数据生成平滑最优轨迹，显著提升性能，具有实际应用潜力。
...
### [333] [A Prototyping Framework for Distributed Control of Multi-Robot Systems](https://arxiv.org/abs/2605.15049)
*Junaid Ahmed Memon,Allan Andre Do Nascimento,Kostas Margellos,Antonis Papachristodoulou*

Main category: cs.RO

TL;DR: 该论文提出了一个用于多机器人分布式控制的原型框架，通过单程序多数据范式在单台计算机上模拟分布式控制，并在四旋翼位置交换任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 为了弥合分布式优化算法的理论与实际测试之间的差距。

Method: 采用单程序多数据范式，每个核心运行相同算法，使用本地状态和邻居通信；支持点质量模型、高保真四旋翼模型和硬件实验平台。

Result: 框架提供了低成本、易访问的方法来验证分布式算法，计算时间和轨迹数据在不同动力学层面进行了比较。

Conclusion: 该框架为多机器人分布式控制的原型测试和算法验证提供了实用工具。
...
### [334] [SOCC-ICP: Semantics-Assisted Odometry based on Occupancy Grids and ICP](https://arxiv.org/abs/2605.15074)
*Johannes Scherer,Sebastian Hirt,Henri Meeß*

Main category: cs.RO

TL;DR: 提出SOCC-ICP，一个统一语义占用网格建图与激光雷达里程计的框架，实现高效鲁棒的位姿估计。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR里程计与下游任务使用的语义占用网格表示不统一，导致冗余计算。

Method: 基于语义占用网格，每个体素编码几何与语义统计；结合点-点/点-面ICP自适应匹配；通过射线投射过滤动态物体。

Result: 在多种场景下性能达到先进水平，在几何退化环境中保持鲁棒；语义标签可进一步提升精度。

Conclusion: 统一里程计与语义建图，消除冗余结构，直接提供适用于下游任务的占用网格。
...
### [335] [CoCo-InEKF: State Estimation with Learned Contact Covariances in Dynamic, Contact-Rich Scenarios](https://arxiv.org/abs/2605.15122)
*Michael Baumgartner,David Müller,Agon Serifi,Ruben Grandia,Espen Knoop,Markus Gross,Moritz Bächer*

Main category: cs.RO

TL;DR: 提出CoCo-InEKF，用连续接触速度协方差替代二元接触状态，通过轻量神经网络学习协方差，实现腿部机器人在动态接触场景下的鲁棒状态估计。


<details>
  <summary>Details</summary>
Motivation: 传统二元接触状态无法处理部分接触或方向滑移等细微条件，导致动态、接触丰富场景下状态估计不准确。

Method: 使用可微不变扩展卡尔曼滤波，结合学习得到的连续接触速度协方差；通过端到端训练的轻量神经网络预测协方差，并设计自动候选点选择程序。

Result: 在双足机器人上，线性速度估计精度-效率权衡更优，滤波器一致性提升；支持跳舞和复杂地面交互等挑战性运动。

Conclusion: 该方法有效提升了动态接触场景下的状态估计鲁棒性，且对候选点位置不敏感。
...
### [336] [CLOVER: Closed-Loop Value Estimation \& Ranking for End-to-End Autonomous Driving Planning](https://arxiv.org/abs/2605.15120)
*Sining Ang,Yuguang Yang,Canyu Chen,Yan Wang*

Main category: cs.RO

TL;DR: CLOVER通过闭环价值估计和排序框架，解决了端到端自动驾驶规划中的训练-评估不匹配问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统自动驾驶规划器的训练目标（模仿单一轨迹）与评估指标（基于规则的安全、可行性等）不匹配，导致性能受限。

Method: 采用轻量级生成器-评分器框架：生成器产生多样化轨迹候选，评分器预测规划指标子评分；通过评估器过滤的伪专家轨迹进行集合级覆盖训练，并使用保守闭环自蒸馏优化生成器和评分器。

Result: 在NAVSIM上PDMS达94.5，EPDMS达90.4；在NavHard上EPDMS达48.3；在nuScenes上L2误差和碰撞率最低。

Conclusion: CLOVER有效缓解了训练-评估不匹配问题，通过闭环自蒸馏和多样化轨迹覆盖显著提升了规划性能。
...
### [337] [Pelican-Unified 1.0: A Unified Embodied Intelligence Model for Understanding, Reasoning, Imagination and Action](https://arxiv.org/abs/2605.15153)
...
TL;DR: 提出首个遵循统一原则的具身基础模型Pelican-Unified 1.0，使用单一VLM实现理解、推理、想象和动作的统一优化，并在多个基准测试中达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有具身智能系统通常将理解、推理、想象和动作分解为独立的专家系统，缺乏统一优化，导致效率低下和性能瓶颈。

Method: 采用单一VLM作为统一的理解与推理模块，通过自回归生成多类型推理链，并利用统一未来生成器（UFG）联合生成未来视频和动作，所有损失反向传播至共享表示。

Result: 在8个VLM基准上平均64.7分（同规模最佳），WorldArena第一（66.03），RoboTwin第二（93.5）。

Conclusion: 统一范式能在保持专长性能的同时，将多种能力整合到单一模型中，实现无妥协的统一。
...
### [338] [Hand-in-the-Loop: Improving Dexterous VLA via Seamless Interventional Correction](https://arxiv.org/abs/2605.15157)
*Zhuohang Li,Liqun Huang,Wei Xu,Zhengming Zhu,Nie Lin,Xiao Ma,Xinjun Sheng,Ruoshi Wen*

Main category: cs.RO

TL;DR: 提出HandITL方法，通过混合人类纠正意图与自主策略执行，解决灵巧操作中由于指令不匹配导致的‘手势跳跃’问题，显著提升策略性能。


<details>
  <summary>Details</summary>
Motivation: VLA模型在灵巧操作中易因高维动作空间和接触动力学产生累积误差，而现有IIL方法在人类接管时存在指令不匹配导致配置突变的问题。

Method: HandITL人机协同干预方法，在接管时刻平滑融合人类纠正意图与自主策略执行，避免手势跳跃。

Result: 接管抖动减少99.8%，抓取失败降低87.5%，平均完成时间减少19.1%；收集的干预数据使策略性能平均提升19%。

Conclusion: HandITL有效解决高自由度灵巧操作中的命令不匹配问题，提升了策略鲁棒性和数据效率。
...
### [339] [Optimal design of solar-battery hybrid resources considering multi-market participation under weather and price uncertainty](https://arxiv.org/abs/2605.14043)
*Hikaru Hoshino,Taiyo Mantani,Eiko Furutani*

Main category: eess.SY

TL;DR: 提出了一种基于深度强化学习的框架，用于光伏-电池混合资源在多市场下的容量配置与竞价策略联合优化。


<details>
  <summary>Details</summary>
Motivation: 混合资源在多市场协调中面临运行复杂性和盈利评估困难，且可再生能源与市场电价的不确定性使传统优化方法难以获得鲁棒的系统设计。

Method: 将系统设计变量嵌入策略学习过程，通过深度强化学习实现系统容量与多市场竞价策略的联合随机优化。

Result: 基于历史新能源发电与市场数据的案例验证了框架能有效识别考虑多市场运行的经济合理系统设计。

Conclusion: 该框架能处理随机运行条件下的联合优化问题，为混合资源设计提供有效决策支持。
...
### [340] [Frequency Nadir-Constrained Power System Restoration Planning with Energy Storage](https://arxiv.org/abs/2605.14099)
*Xiangyu Zou,Amir Reza Nikzad,Ilyas Farhat,John W. Simpson-Porco*

Main category: eess.SY

TL;DR: 提出了一种考虑频率约束的黑启动恢复规划框架，利用储能系统提升恢复过程中的频率安全并缩短恢复时间。


<details>
  <summary>Details</summary>
Motivation: 电力系统恢复过程中必须确保频率稳定，但现有研究缺乏对储能系统集成下频率约束的考虑。

Method: 开发了含储能系统的频率最低点预测方法，并将其嵌入多周期混合整数线性规划优化框架中。

Result: 在修改的IEEE 9节点系统上，恢复计划在保证频率安全的同时，通过储能协调缩短了恢复时间。

Conclusion: 所提框架能有效确保黑启动恢复过程中的频率安全，并验证了储能系统的辅助作用。
...
### [341] [Receding Horizon Multi-Agent Deceptive Path Planner](https://arxiv.org/abs/2605.14085)
*Xubin Fang,Brian M. Sadler,Rick S. Blum*

Main category: eess.SY

TL;DR: 提出基于玻尔兹曼分布的滚动时域欺骗路径规划框架，通过短时域候选轨迹的局部优化实现动态欺骗平衡。


<details>
  <summary>Details</summary>
Motivation: 现有全时域端到端方法难以在线重规划和扩展，且无法适应动态环境变化。

Method: 采用玻尔兹曼分布建模短时域候选轨迹，通过滚动时域循环迭代包含欺骗、资源和光滑性的用户定义代价函数，支持多智能体耦合项。

Result: 仿真验证该方法在环境更新时无需完全重计算，可通过少量参数直观调节欺骗与约束的平衡。

Conclusion: 该框架支持在线动态调整欺骗水平与约束，为新兴动态欺骗策略提供实现途径。
...
### [342] [JAX-Based Batched AC Power Flow for GPU Acceleration and AI Ecosystem Integration](https://arxiv.org/abs/2605.14103)
*Yihong Zhou,Dylan Cope,Jakob Foerster,Thomas Morstyn*

Main category: eess.SY

TL;DR: 提出了基于JAX的批量交流潮流求解器，在输电和配电网络中实现了10倍以上的加速。


<details>
  <summary>Details</summary>
Motivation: 电网灵活性协调需要高效的批量潮流评估，但现有方法依赖手写C/CUDA代码，难以定制和优化。

Method: 使用JAX框架实现牛顿-拉夫逊法（输电网络）和Z-Bus法（三相不平衡配电网络）的批量求解。

Result: 相比pandapower和OpenDSS，实现了10倍以上的加速，并与JAX AI生态无缝集成。

Conclusion: JAX方法提供了高效、可定制的批量潮流求解，适用于未来大规模AI驱动的电力系统操作。
...
### [343] [Battery-Assisted Operation of Hyperscale AI Data Centers under Connect-and-Manage Interconnection Practices](https://arxiv.org/abs/2605.14105)
*Xin Lu,Jing Qiu,Jiafeng Lin,Sihai An,Mingyang Sun,Junhua Zhao*

Main category: eess.SY

TL;DR: 提出一种电池辅助运行框架，利用储能系统作为物理缓冲，协调AI数据中心工作负载的快速动态变化与电网互联约束。


<details>
  <summary>Details</summary>
Motivation: 新兴的连接管理实践允许大型负荷接入，但实时变化的功率交换限制与数据中心工作负载连续性需求产生冲突。

Method: 提出连续感知能量计算模型；构建包含日前场景工作负载承诺与实时滚动交付保证控制的两阶段决策框架。

Result: IEEE 39节点系统案例表明：储能显著提高日前工作负载承诺能力，增强实时交付鲁棒性；敏感性分析显示储能角色根据约束条件在连续性支持与经济性灵活性之间转变。

Conclusion: 电池储能系统能有效协调数据中心内部快速动态与电网交互限制，其作用随约束放松从保障连续性转向提供经济灵活性。
...
### [344] [Optimizing Grid-Forming Controls using Relay-based Extremum Seeking to Enhance Transient Performance](https://arxiv.org/abs/2605.14161)
*Kyung-Bin Kwon,Min Gyung Yu,Sayak Mukherjee,Timothy I. Salsbury*

Main category: eess.SY

TL;DR: 提出一种基于极值搜索控制的实时自适应优化框架，动态调整电网形成逆变器下垂系数，在无需电网模型的情况下实现了多目标性能权衡。


<details>
  <summary>Details</summary>
Motivation: 电网形成逆变器的下垂系数存在阻尼、调节时间、频率变化率和频率最低点等多目标冲突，固定参数无法适应动态电网条件。

Method: 采用极值搜索控制实时调节下垂增益，构建包含振荡能量、频率最低点、频率变化率和暂态恢复性能的多目标代价函数，通过梯度优化跟踪最优参数。

Result: 在改进的IEEE 68节点系统中验证了代价函数的下垂参数凸性，极值搜索算法能实时跟踪时变最优下垂系数，在无分析模型条件下保持接近最优的系统性能。

Conclusion: 该自适应框架有效解决了多目标权衡问题，无需精确电网模型即可实现鲁棒的近优性能，适用于高渗透率逆变器资源系统。
...
### [345] [Grid Integration of Gigawatt-Scale AI Data Centers under Connect-and-Manage](https://arxiv.org/abs/2605.14109)
*Xin Lu,Qianwen Xu*

Main category: eess.SY

TL;DR: 本文形式化了一种灵活的连接管理机制，允许人工智能数据中心（AIDC）在无预先升级条件下连入电网，同时引入实时削减协议。通过三层次分层架构（学习驱动的规划层、鲁棒接纳机制、单步执行优化器），在IEEE 39节点系统及澳大利亚市场数据上，实现了削减率从9.1%降至2.8%，并保留了98.1%的前沿训练负载。


<details>
  <summary>Details</summary>
Motivation: 大型AIDC快速增长导致电力需求激增，而传统电网扩容耗时且成本高。新兴的“即连即管”模式虽允许快速接入，但需应对实时削减。当前缺乏统一框架协调AIDC与输电系统运营商（TSO），且AIDC内部工作负载的弹性尚未被充分发掘。

Method: 将AIDC工作负载分为前沿训练、批量训练和推理服务，共享电池储能。TSO侧采用直流潮流与预算约束不确定性建模。基于TSO接纳机制的非透明性，设计三层次架构：学习层生成功率请求，TSO通过鲁棒接受机制评估，单步优化确保内部可行性。

Result: 削减率从9.1%降至2.8%，前沿训练保留98.1%。批量训练是主要弹性资源，高峰需求时吞吐波动最大。电池通过主动放电和充电延迟缓冲削减影响。

Conclusion: 提出的框架有效平衡了AIDC工作负载与电网稳定性，展示了批量训练和电池储能在缓解削减方面的关键作用，为超大规模AIDC与电网协同提供了实用方案。
...
### [346] [Day-to-Day Traffic Network Modeling under Route-Guidance Misinformation: Endogenous Trust and Resilience in CAV Environments](https://arxiv.org/abs/2605.14204)
*Eunhan Ka,Satish V. Ukkusuri*

Main category: eess.SY

TL;DR: 本文建立了一个耦合的日间交通分配与信任演化框架，用于分析路径引导中的错误信息影响。


<details>
  <summary>Details</summary>
Motivation: 当数字路径引导变得不可靠或对抗时，交通建模需要同时表示流量适应和用户对信息源的信任演化。

Method: 方法包括：基于Lighthill-Whitham-Richards网络的日内拥堵加载、有限理性logit学习日间路径选择、Beta证据模型信任更新，以及理论分析和数值实验（Sioux Falls和Anaheim）。

Result: 信任内生机制产生阈值效应：低于阈值时攻击隐蔽且影响无衰减；高于阈值时信任侵蚀使固定信任攻击影响降低91%（Sioux Falls）和85%（Anaheim）。CAV渗透率增加固定信任脆弱性，但保留动态衰减；交通性能先于信任恢复，产生77天隐藏脆弱期。

Conclusion: 结果为CAV交通网络中的弹性分析提供了信任感知建模基础。
...
### [347] [Energy Management for Solar-Powered Electric-Bus Charging Station: A Data-Driven Method](https://arxiv.org/abs/2605.14282)
*Xiaoting Wang,Supun Amarathunga,Pasan Gunawardena,Gregory Kish,Yunwei,Li*

Main category: eess.SY

TL;DR: 提出一种结合数据驱动多项式混沌展开和新能源的电动公交车充电站柔性能量管理系统，以应对多重不确定性。


<details>
  <summary>Details</summary>
Motivation: 电动汽车充电站面临可再生能源出力、电价及车辆充放电状态等多重不确定性，现有方法在处理有限样本数据时效率不足。

Method: 采用数据驱动多项式混沌展开代理模型，结合非参数推断方法从有限样本中扩充输入数据，构建包含光伏、储能和充电的柔性EMS。

Result: 在20辆电动公交车案例中验证了所提EMS在不确定性下有效管理能量流动。

Conclusion: 该方法能在历史数据有限的场景下提升充电站运行的经济性和鲁棒性。
...
### [348] [Randomized Atomic Feature Models for Physics-Informed Identification of Dynamic Systems](https://arxiv.org/abs/2605.14351)
*Rajiv Singh,Mario Sznaier,Lennart Ljung*

Main category: eess.SY

TL;DR: 提出了一种基于随机稳定原子特征的物理信息系统辨识框架，将脉冲响应表示为阻尼复指数随机叠加，通过凸正则化最小二乘问题求解，并支持多种物理约束。


<details>
  <summary>Details</summary>
Motivation: 现有系统辨识方法难以同时处理模态可解释性、数值可扩展性以及对欠激励数据的鲁棒性。

Method: 使用随机采样极点生成稳定原子（阻尼复指数），通过正则化最小二乘进行辨识，引入KYP等约束，并从算子理论推导了Disk-Bochner定理。

Result: 证明了核的移位缺陷与正测度的关系，建立了RKHS到l1的嵌入，给出了随机特征收敛性和稀疏恢复保证，数值实验表明物理先验能改善欠激励下的恢复。

Conclusion: 该框架有效结合了物理可解释性与计算可扩展性，在数据不足时通过先验约束提升系统辨识性能。
...
### [349] [Automated Curriculum Design for High-dimensional Human Motor Learning](https://arxiv.org/abs/2605.14367)
*Ankur Kamboj,Rajiv Ranganathan,Xiaobo Tan,Vaibhav Srivastava*

Main category: eess.SY

TL;DR: 提出了一种结合人类运动学习模型与随机非线性模型预测控制的自动课程设计框架，在新型运动学习任务中通过个性化技能估计加速技能获取。


<details>
  <summary>Details</summary>
Motivation: 高维运动学习任务中有效练习调度设计困难，且技能状态不可观测、任务表现无法反映真实学习状态。

Method: 结合人类运动学习模型与在线个性化技能估计，采用随机非线性模型预测控制实现自动课程设计。

Result: 模拟和人体实验（N=36）显示，该方法比随机课程和基于表现的启发式课程分别提高学习效率约23%和17%。

Conclusion: 基于模型的个性化课程在运动康复和复杂技能训练中具有显著潜力。
...
### [350] [Fully Dynamic Rebalancing in Dockless Bike-Sharing Systems via Deep Reinforcement Learning](https://arxiv.org/abs/2605.14501)
*Edoardo Scarpel,Alberto Pettena,Matteo Cederle,Federico Chiariotti,Marco Fabris,Gian Antonio Susto*

Main category: eess.SY

TL;DR: 提出了一种基于深度强化学习的全动态再平衡方法，用于无桩共享单车系统，通过实时控制单辆卡车进行局部调度，显著减少可用性故障并缓解空间不平等和出行荒漠。


<details>
  <summary>Details</summary>
Motivation: 现有周期性、系统级干预方法无法适应共享单车的动态需求变化，需要更高效的自适应再平衡策略。

Method: 基于图模拟器建模服务过程，将再平衡问题构建为马尔可夫决策过程，深度强化学习智能体实时路由卡车，结合时空关键性评分执行取车、放车和充电操作。

Result: 在真实数据实验中使用极少量卡车即显著降低可用性故障，同时限制空间不平等和出行荒漠。

Conclusion: 基于学习的再平衡方法能为共享微出行系统提供高效可靠的运营支持。
...
### [351] [Quantifying Cyber-Vulnerability in Power Electronics Systems via an Impedance-Based Attack Reachable Domain](https://arxiv.org/abs/2605.14502)
*Hongwei Zhen,Ze Yu,Xin Xiang,Wuhua Li,Mingyang Sun*

Main category: eess.SY

TL;DR: 提出基于阻抗的攻击可达域（ARD）框架，量化电力电子系统中节点在权限受限下的网络脆弱性，并定义攻击穿透指数（API），通过案例验证跨层协同攻击的破坏性及新指标的独特价值。


<details>
  <summary>Details</summary>
Motivation: 现有缺乏面向攻击者的度量指标，无法量化节点在权限受限的动作空间中被推向不稳定的程度。

Method: 构建阻抗攻击可达域（ARD），映射攻击动作到关键特征值迁移；定义攻击穿透指数（API）；提出灰盒工作流整合阻抗辨识与可微代理模型。

Result: 跨层协同攻击比单层攻击破坏性更强；API能揭示电网强度指标无法发现的脆弱性模式。

Conclusion: ARD和API为电力电子系统提供了有效的网络脆弱性量化评估方法，且灰盒工作流使实际计算可行。
...
### [352] [Admittance-Guided Inverter Dispatch Command Manipulation Attack: A Grid Stability-Oriented Approach](https://arxiv.org/abs/2605.14509)
*Hongwei Zhen,Ze Yu,Xin Xiang,Mingyang Sun,Wuhua Li*

Main category: eess.SY

TL;DR: 本文提出了一种基于导纳引导的框架，用于识别智能微电网中易受攻击的逆变器及最严重的调度命令篡改攻击，通过物理信息神经网络重建导纳并优化稳定性裕度，实验证明攻击可在额定范围内引发次同步振荡。


<details>
  <summary>Details</summary>
Motivation: 现有网络攻击研究要么需要详细的系统拓扑和控制器动态知识，要么依赖重复在线交互，可能产生不可行或越限命令，缺乏实用性。

Method: 利用受损逆变器注入谐波扰动进行稀疏导纳测量，使用物理信息神经网络重建目标逆变器的运行点依赖导纳，基于重建导纳构建稳定性裕度优化问题，定位最脆弱逆变器及最坏调度命令。

Result: 在五逆变器微电网控制器硬件在环实验表明，识别的命令能在保持额定调度边界内驱动系统进入严重的次同步振荡。

Conclusion: 研究强调了需要超越静态限制检查的稳定性感知命令筛选机制。
...
### [353] [Distributionally Robust Model Predictive Control for Virtual Power Plants](https://arxiv.org/abs/2605.14642)
*Nikolas Recke,Mathias Hudoba de Badyn*

Main category: eess.SY

TL;DR: 提出了一种分布鲁棒模型预测控制（DRMPC）框架，用于在电力价格不确定下优化虚拟电厂（VPP）运行，通过分位数不确定性量化构建时变Wasserstein模糊集，在实际数据上实现了高达0.8%的经济效益提升。


<details>
  <summary>Details</summary>
Motivation: 现有VPP运行依赖电价预测，但预测误差会显著影响经济效益，需要一种能内生处理分布偏移和离散度的鲁棒控制方法。

Method: 结合数据驱动的分位数预测与时变Wasserstein模糊集，在MPC中嵌入预测分布信息，形成可求解的DR-MPC优化问题。

Result: 在两种季节性场景下，适当选择模糊半径时DR-MPC比标准MPC提高经济效益达0.8%；半径过大会导致保守性增加，降低收入。

Conclusion: 分布鲁棒优化在不确定性感知的VPP运行中具有实际价值，但模糊半径的选择对性能至关重要。
...
### [354] [Dynamic Event-Triggered Control of Discrete-Time Nonlinear Systems based on Difference-Algebraic Representations](https://arxiv.org/abs/2605.14688)
*Vitoriano Casas,Gabriela Reis,Pedro Henrique Coutinho,Iury Bessa,Rodrigo Araújo*

Main category: eess.SY

TL;DR: 针对一类由差分代数表示描述的非线性离散系统，提出了一种基于增益调度控制器的动态事件触发控制方法，并同时设计了控制律与触发机制，以减少事件次数并扩大吸引域。


<details>
  <summary>Details</summary>
Motivation: 现有事件触发控制方法在处理非线性离散系统时存在保守性，且未充分利用系统非线性信息及异步采样带来的影响。

Method: 采用差分代数表示描述系统，设计增益调度控制器与事件触发机制，融合非线性信息与异步项，形成协同设计条件，并通过优化问题减少事件次数、扩大吸引域估计。

Result: 获得了确保闭环系统渐近稳定性的低保守性协同设计条件，并给出了吸引域的估计；数值示例验证了方法的有效性。

Conclusion: 该方法能有效降低事件触发频率并扩大吸引域，适用于一类非线性离散系统的动态事件触发控制。
...
### [355] [Flexibility-Aware Framework for Efficient Planner-Initiated Siting of Data Center](https://arxiv.org/abs/2605.14714)
*Dongjoo Kim,Lin Dong,Le Xie*

Main category: eess.SY

TL;DR: 提出一种由规划者发起的框架，通过可靠性筛选、市场影响评估和多准则评分生成预认证选址目录，在2,000节点测试系统上实现9-21%选址扩展、电价稳定和并网时间缩短至12-18个月。


<details>
  <summary>Details</summary>
Motivation: AI数据中心能源需求爆发式增长，传统电网互联与输电扩建速度滞后，现有项目评估流程被动且耗时数年。

Method: 集成可靠性门控筛选、基于标准灵活性包络的市场影响评估、熵权多准则评分，生成预认证互联就绪位置目录。

Result: 灵活性将选址边界扩展9-17%（1GW）和19-21%（2GW）；中位电价稳定（24.32美元/MWh）；峰值电价离散度降低约3.4%；首次通电时间缩短至12-18个月。

Conclusion: 该技术无关框架为系统运营商和监管者提供主动决策工具，可在保障可靠性和市场稳定前提下加速大型柔性负载并网。
...
### [356] [Addressing Terminal Constraints in Data-Driven Demand Response Scheduling](https://arxiv.org/abs/2605.14741)
*Maximilian Bloor,Martha White,Ehecatl Antonio del Rio Chanona,Calvin Tsay*

Main category: eess.SY

TL;DR: 本研究提出了一种结合目标空间规划（GSP）与深度确定性策略梯度（DDPG）的方法，用于解决电气化化学过程在时变电力市场中的灵活调度问题，在满足终端存储约束的同时提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 电气化化学过程面临时变电价市场的激励，需要灵活运行，但参与需求响应方案需满足长期终端约束，传统模型优化成本高，强化学习面临信用分配困难。

Method: 将目标空间规划与DDPG集成，利用学习到的关于离散子目标的时域抽象模型，在扩展时间范围内传播价值，并应用于空气分离基准模拟。

Result: 该方法在模拟空气分离基准上，相比标准DDPG提高了样本效率，同时满足了终端存储约束，减轻了短视控制行为。

Conclusion: 提出的GSP-DDPG方法有效解决了电气化过程中的灵活调度问题，平衡了长期约束和优化效率。
...
### [357] [Hybrid Metaheuristic Optimization of Distributed Control System Hardware Architecture with Model-Based Verification](https://arxiv.org/abs/2605.14788)
*Ruslan Zakirzyanov*

Main category: eess.SY

TL;DR: 提出了一种基于蚁群元启发式算法的分布式控制系统硬件架构综合方法，并在硫酸厂案例中验证了其满足结构及动态性能需求。


<details>
  <summary>Details</summary>
Motivation: 化工工厂分布式控制系统硬件架构设计在参数部分不确定下需平衡成本与可靠性，是一个难题。

Method: 建立形式化模型，开发混合蚁群元启发式框架来构造可行的分层架构。

Result: 在大型硫酸厂案例中验证了可行性，得到的架构满足结构及动态性能要求。

Conclusion: 该方法能够有效生成满足性能需求的分布式控制系统硬件架构。
...
### [358] [Robust Quadcopter Motion Control Using Output Feedback](https://arxiv.org/abs/2605.14945)
*Stanislav Kim,Anton Pyrkin,Oleg Borisov*

Main category: eess.SY

TL;DR: 针对四旋翼飞行器输出反馈运动控制问题，通过几何方法将模型转换为带时变增益系数的标准型，再经控制输入双重积分实现增益稳定化，最后基于扩展观测器设计了鲁棒输出反馈控制律。


<details>
  <summary>Details</summary>
Motivation: 现有四旋翼控制方法多依赖全状态反馈，但实际中部分状态难以直接测量，需要研究仅基于可测输出的反馈控制策略。

Method: 几何变换法将四旋翼模型化为标准型，控制输入双重积分处理时变增益，扩展观测器合成鲁棒输出反馈控制律。

Result: 实现了四旋翼系统的输出反馈稳定控制，保证了闭环系统的鲁棒性。

Conclusion: 所提输出反馈控制方法能有效解决四旋翼在部分状态不可测情况下的运动控制问题。
...
### [359] [On the Nonexistence of Continuous Immersions for Discrete-time Systems](https://arxiv.org/abs/2605.15161)
*Eron Ristich,Eduardo Sontag,Necmiye Ozay*

Main category: eess.SY

TL;DR: 本文研究了非线性动力系统在离散时间内能否被线性嵌入的问题，并证明了当系统具有可数多个但多于一个omega极限集时存在障碍，同时推广到alpha极限集，并提供了示例。


<details>
  <summary>Details</summary>
Motivation: 理解非线性动力系统的线性嵌入何时存在对于利用线性系统理论工具分析非线性动力学至关重要。

Method: 通过理论证明和示例分析，将连续时间结果扩展到离散时间动力学，并考虑alpha极限集的推广。

Result: 离散时间动力系统在具有可数多个但多于一个omega极限集时无法被有限维线性系统通过一对一的连续映射嵌入，类似障碍也适用于alpha极限集。

Conclusion: 非线性动力系统的线性嵌入在离散时间中存在与连续时间类似的障碍，这限制了线性方法的应用范围。
...
### [360] [An Attention-Enhanced Network with Joint Dehazing and Retinex-Based Enhancement for Underwater Images](https://arxiv.org/abs/2605.14677)
*Sahana Ray,Bibhabasu Debnath,Sanjay Ghosh*

Main category: eess.IV

TL;DR: 提出ADR三阶段网络，通过扩展IFM模型、Retinex增强和注意力U-Net++细化，实现水下图像去雾与增强。


<details>
  <summary>Details</summary>
Motivation: 经典IFM无法模拟非线性水下光行为，纯数据驱动方法缺乏物理可解释性。

Method: 扩展水下图像形成模型的去雾阶段，结合Retinex增强与注意力U-Net++细化。

Result: 在UIEB和UFO-120数据集上达到与最先进方法竞争的性能。

Conclusion: ADR网络有效结合物理模型与深度学习，提升水下图像质量。
...
### [361] [Efficient Dense Matching for Enhanced Gaussian Splatting Using AV1 Motion Vectors](https://arxiv.org/abs/2605.14629)
*Julien Zouein,Vibhoothi Vibhoothi,François Pitié,Anil Kokaram*

Main category: eess.IV

TL;DR: 本文提出基于AV1视频编码的SfM管道，生成更稠密点云，使3D Gaussian Splatting重建速度提升63%，VMAF提高9点。


<details>
  <summary>Details</summary>
Motivation: 传统COLMAP初始化计算成本高且在无纹理区域点云稀疏，限制了3DGS重建精度与收敛速度。

Method: 利用AV1编码器中的运动矢量进行特征检测与匹配，取代耗时的穷举匹配，生成稠密点云。

Result: 点云密度提升8倍，训练时间减少63%，VMAF指标提高9点。

Conclusion: 基于AV1的初始化管道能有效提升3DGS重建效率与质量。
...
### [362] [Denoising for Neuromorphic Cameras Based on Graph Spectral Features](https://arxiv.org/abs/2605.14734)
*Shimpei Harada,Junya Hara,Hiroshi Higashi,Yuichi Tanaka*

Main category: eess.IV

TL;DR: 提出了一种基于图谱特征的神经形态相机去噪方法，通过构建事件图并利用图拉普拉斯特征向量提取无噪声事件。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高灵敏度导致测量噪声显著，现有去噪方法效果有限。

Method: 构建以事件为节点、时空距离为边的图，利用事件密度先验确定图连接参数，通过定制图拉普拉斯进行快速特征分解提取无噪声事件。

Result: 在合成和真实事件数据上，相比其他方法有效去除了噪声事件。

Conclusion: 基于图谱特征的去噪方法能有效降低事件相机噪声，且通过优化特征分解计算复杂度可行。
...
### [363] [FaSST: Fast Sparsifying Secondary Transform](https://arxiv.org/abs/2605.15086)
*Darukeesan Pakiyarajah,Samuel Fernández-Menduiña,Eduardo Pavez,Antonio Ortega,Debargha Mukherjee*

Main category: eess.IV

TL;DR: 提出了一种低复杂度数据依赖二次变换框架FaSST，通过Givens旋转分解和交替优化策略，在AV2中实现与LFNST相当的率失真性能，同时降低计算复杂度达83.67%，并进一步节省1.80%的BD-rate。


<details>
  <summary>Details</summary>
Motivation: 现有数据依赖二次变换（如LFNST）存在高频系数截断限制编码增益、缺乏显式率失真优化设计准则，且计算复杂度高，需设计低复杂度且RD最优的方案。

Method: 将数据驱动的稀疏正交变换（SOT）分解为Givens旋转序列，采用交替最小化和近似Givens分解策略，自适应旋转次数以降低复杂度，并基于预测模式设计模式相关变换。

Result: FaSST在AV2中匹配LFNST的RD性能，计算量减少83.67%；避免固定系数截断后，相比LFNST节省1.80% BD-rate且复杂度降低66.24%。

Conclusion: FaSST通过Givens旋转分解和模式自适应策略，实现了低复杂度与高编码效率的权衡，优于传统LFNST方案。
