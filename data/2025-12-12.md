...
TL;DR: ExaCraft是一个AI系统，通过结合用户画像和实时学习行为分析，利用Google Gemini AI生成个性化且动态适应学习者上下文的学习示例，以提高学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有的教育AI工具未能专注于生成个性化示例，也无法适应学习者不断变化的理解、困难或技能发展。学习在与个人相关的、能引起共鸣的例子连接时最有效。

Method: ExaCraft系统基于Google Gemini AI和Python Flask API开发，通过Chrome扩展程序访问。它结合了用户定义的个人资料（包括地点、教育、职业和复杂性偏好）以及对学习者行为的实时分析。系统核心创新在于其适应学习上下文的五个关键方面：挣扎指标、掌握模式、主题进展历史、会话边界和学习进展信号。

Result: ExaCraft能够生成文化相关并根据个体学习需求定制的示例。其示例会从基本概念演变为高级技术实现，并响应主题重复、重新生成请求和不同用例中的主题进展模式。

Conclusion: ExaCraft通过动态适应学习者的上下文，成功地生成了高度个性化和适应性的学习示例，从而有效支持学习者从基础到高级概念的学习进展。
...
### [2] [Echo-CoPilot: A Multi-View, Multi-Task Agent for Echocardiography Interpretation and Reporting](https://arxiv.org/abs/2512.09944)
*Moein Heidari,Mohammad Amin Roohi,Armin Khosravi,Ilker Hacihaliloglu*

Main category: cs.AI

TL;DR: Echo-CoPilot是一个多视图、多任务的超声心动图智能代理，它利用大型语言模型协调专业工具，实现统一、符合临床指南的解释，并在MIMIC-EchoQA基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 超声心动图的完整解读是一项认知要求高、多视图的手动任务。现有的基础模型虽然在单一感知子任务（如视图分类、分割或疾病预测）上表现良好，但它们通常独立运行，无法提供统一且临床连贯的评估。

Method: 本文引入了Echo-CoPilot，一个多视图、多任务代理。它使用大型语言模型（LLM）以ReAct风格的循环来协调一系列专业的超声心动图工具。该代理分解临床查询，调用工具进行视图识别、心脏结构分割、测量和疾病预测以及报告合成，并将它们的输出整合为符合指南的答案和叙述性总结。

Result: Echo-CoPilot在MIMIC-EchoQA基准测试中取得了50.8%的准确率，优于通用的和生物医学视频视觉-语言模型。定性分析表明，该代理利用定量测量和生理背景来解决接近临床决策阈值的挑战性病例，例如边缘性左心室肥大或心包积液严重程度。

Conclusion: Echo-CoPilot通过整合多视图、多任务工具并由LLM协调，提供了一种统一、符合临床指南的超声心动图评估方法，显著提升了超声心动图的解读能力，尤其在处理复杂病例方面表现出优势。
...
### [3] [Suzume-chan: Your Personal Navigator as an Embodied Information Hub](https://arxiv.org/abs/2512.09932)
*Maya Grace Torii,Takahito Murakami,Shuka Koseki,Yoichi Ochiai*

Main category: cs.AI

TL;DR: 本研究提出并原型化了一个名为“Suzume-chan”的具身AI代理，它通过物理和对话交互，利用社交临场感理论，实现更温暖、更以人为本的知识共享，减少心理距离。


<details>
  <summary>Details</summary>
Motivation: 现有数字工具虽能提供信息，但难以建立深入理解所需的连接感，而获取专家知识常需实时人际沟通。本研究旨在解决数字工具在知识共享中缺乏人际连接的问题。

Method: 基于社交临场感理论，提出“具身信息枢纽”概念。原型“Suzume-chan”是一个小型、柔软的本地AI代理，结合语言模型（LLM）和检索增强生成（RAG），通过物理和对话交互来学习和回应。

Result: 原型“Suzume-chan”能从口头解释中学习，并通过对话回应，有效减少心理距离，使知识共享过程更温暖、更以人为本。

Conclusion: 通过具身AI代理和对话交互，可以显著增强知识共享中的连接感和人性化程度，克服传统数字工具的局限性。
...
TL;DR: 本文提出使用稀疏自编码器 (SAE) 生成可解释的SAE嵌入，用于大规模文本分析，比大型语言模型 (LLM) 更具成本效益和可靠性，比密集嵌入更具可控性。


<details>
  <summary>Details</summary>
Motivation: 分析大规模文本语料是机器学习中的核心挑战，对识别模型不良行为或训练数据偏差至关重要。现有方法（如基于LLM的注释或密集嵌入聚类）要么成本高昂，要么缺乏对特定属性的控制。

Method: 提出使用稀疏自编码器 (SAE) 来创建SAE嵌入。这些嵌入的维度映射到可解释的概念，从而提供对文本数据的细粒度控制和理解。

Result: SAE嵌入在四项数据分析任务中表现出比LLM更具成本效益和可靠性，比密集嵌入更具可控性。它们能揭示数据集间的语义差异（例如，Grok-4比其他模型更常澄清歧义）和文档中意想不到的概念关联。相较于LLM，SAE嵌入能以2-8倍的更低成本发现更大的差异，并更可靠地识别偏差。SAE嵌入还具有可控性，能实现基于兴趣轴的文档聚类，并在基于属性的检索方面优于密集嵌入。案例研究分析了OpenAI模型行为随时间的变化，并找到了Tulu-3模型从训练数据中学到的“触发”短语。

Conclusion: SAE被定位为一种用于非结构化数据分析的多功能工具，并强调了通过数据解释模型的重要性。
...
TL;DR: 本文将哥德尔不完备定理扩展到AI领域，建立了AI安全和对齐的信息论限制，并提供了应对这些挑战的实用方法。


<details>
  <summary>Details</summary>
Motivation: 了解AI安全和对齐的理论限制，并为应对这些挑战做好准备，对于负责任地采用AI技术至关重要。

Method: 通过将哥德尔不完备定理扩展到AI，建立了AI安全和对齐的信息论限制。

Result: 确立了AI安全和对齐的信息论限制；证明了AI系统认知推理能力的更广泛限制。

Conclusion: 识别并准备应对AI固有的理论限制对于其负责任的采用至关重要，同时提供了处理这些挑战的实用方法。
...
TL;DR: 研究发现大型语言模型（LLMs）能从姓名和职业等间接线索中推断并在线性激活空间中编码用户社会人口属性，这些隐含的表征会影响其下游行为，即使通过偏见基准测试的模型也可能存在并利用隐含偏见。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs如何从间接线索（如姓名和职业）中推断并编码人类对话伙伴的社会人口属性，以及这些编码如何影响LLMs的行为和公平性。

Method: 研究使用了四种开放式Transformer LLMs（Magistral 24B, Qwen3 14B, GPT-OSS 20B, OLMo2-1B）。首先通过显式人口信息提示，探测模型各层残差流中的线性人口属性表征。然后，使用这些探针来预测从隐式线索（姓名、职业）中推断出的人口属性。最后，通过案例（如职业推荐）展示这些隐式人口表征对下游行为的影响。

Result: LLMs在激活空间中形成了用户人口属性的线性表征，其中刻板印象相关的属性沿可解释的几何方向编码。通过显式披露训练的探针可以从隐式线索中预测人口属性：姓名激活与人口普查一致的性别和种族表征，职业触发与现实劳动力统计相关的表征。这些隐式人口表征积极地塑造了下游行为，例如职业推荐。此外，通过偏见基准测试的模型仍可能存在并利用隐含偏见。

Conclusion: LLMs会从间接线索中隐式推断并编码用户的人口属性，这些隐含的线性表征会影响模型的下游行为，并可能导致偏见，对大规模应用中的公平性产生影响，即使通过标准偏见测试的模型也需警惕。
...
TL;DR: 通用大语言模型在农业领域常因区域差异产生幻觉，本研究提出AgriRegion，一个结合地理空间元数据注入和区域优先重排机制的RAG框架，旨在提供高保真、区域感知的农业建议，显著减少幻觉并提高信任度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在农业领域面临挑战，通用模型常因土壤、气候和当地法规差异而产生上下文幻觉，提供不准确或在特定区域可能造成灾难性后果的建议，阻碍了其在该领域的应用潜力。

Method: AgriRegion是一个检索增强生成（RAG）框架。它通过引入地理空间元数据注入层和区域优先重排机制来解决问题。知识库被限制在经过验证的当地农业推广服务中，并在检索过程中强制执行地理空间约束。研究还创建了一个包含160个领域特定问题、涵盖12个农业子领域的新基准数据集AgriRegion-Eval。

Result: 实验结果表明，与最先进的大语言模型系统相比，AgriRegion将幻觉减少了10-20%。通过综合评估，该框架显著提高了信任分数，确保了建议的本地准确性。

Conclusion: AgriRegion通过其独特的地理空间RAG方法，成功解决了农业领域中大语言模型存在的上下文幻觉问题，能够提供高保真、区域感知的农业建议，从而提高了信息的准确性和可信度。
...
### [15] [Modeling Narrative Archetypes in Conspiratorial Narratives: Insights from Singapore-Based Telegram Groups](https://arxiv.org/abs/2512.10105)
...
TL;DR: 本文提出了一种名为神经元注意力电路（NAC）的新型、生物学上合理的连续时间（CT）注意力机制，它通过将注意力计算重新表述为受线虫（C. elegans）神经回路策略启发的ODE解决方案，实现了高效的自适应动态，并在多个领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 注意力机制在RNNs上提升了表示学习，但其离散性限制了连续时间（CT）建模。因此，需要一种能够处理连续时间数据的注意力机制。

Method: NAC将注意力logits计算重构为一阶线性ODE的解，该ODE具有从线虫神经回路策略（NCPs）布线机制中衍生的非线性互联门。它用稀疏的感知门代替密集投影进行键-查询投影，并使用具有两个头的稀疏骨干网络计算“内容-目标”和“可学习时间常数”门，从而实现高效的自适应动态。NAC支持三种注意力logits计算模式：显式欧拉积分、精确闭合解和稳态近似。为提高内存效率，还实现了一种稀疏Top-K成对连接方案。提供了状态稳定性、有界近似误差和通用近似等理论保证。

Result: 在不规则时间序列分类、自动驾驶车道保持和工业预测等多个领域进行实验，NAC在准确性上与现有基线持平或超越，并且在运行时和内存效率方面，相对于其他CT基线处于中间水平。

Conclusion: NAC是一种有效且高效的生物学上合理的连续时间注意力机制，它通过创新的ODE建模和稀疏化技术，克服了传统注意力机制在连续时间建模中的局限性，并展现出强大的理论支持和实际应用潜力。
...
### [22] [InfoCom: Kilobyte-Scale Communication-Efficient Collaborative Perception with Information Bottleneck](https://arxiv.org/abs/2512.10305)
*Quanmin Wei,Penglin Dai,Wei Li,Bingyi Liu,Xiao Wu*

Main category: cs.AI

TL;DR: InfoCom是一个信息感知框架，通过扩展信息瓶颈原理，为自动驾驶中的通信高效协同感知奠定了理论基础，实现了近乎无损的感知性能，同时将通信开销从兆字节降低到千字节级别。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统对精确环境感知有高要求，而单智能体感知存在局限性。现有协同感知方法在信息共享中面临通信与性能的权衡，且通常假设兆字节级别的数据传输，这在实际网络约束下可能无法实现。

Method: InfoCom提出了一种信息净化范式，通过扩展信息瓶颈原理，理论上优化了关键任务信息的最小充分提取。其核心创新包括：i) 信息感知编码，将特征浓缩为最小消息；ii) 稀疏掩码生成，以可忽略的通信成本识别空间线索；以及 iii) 多尺度解码，通过掩码引导机制逐步恢复感知信息。

Result: InfoCom在多个数据集上实现了近乎无损的感知性能，同时将通信开销从兆字节降低到千字节级别，与Where2comm和ERMVP相比，每个智能体的通信量分别减少了440倍和90倍。

Conclusion: InfoCom为通信高效的协同感知提供了一个开创性的理论基础和实用框架，能够显著降低通信开销，同时保持卓越的感知性能，有效解决了自动驾驶中的通信-性能权衡问题。
...
TL;DR: 本文提出了一个新颖的可解释性框架，并引入了CogVision数据集，旨在系统地分析视觉-语言模型（VLMs）的内部机制，特别是注意力头在多模态推理中的功能角色。研究发现，专门负责特定认知功能的“功能头”普遍稀疏且关键。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言模型（VLMs）在多模态基准测试中表现出色，但它们在很大程度上仍然是“黑箱”。本研究的动机是系统地分析VLMs的内部机制，特别是关注注意力头在多模态推理中的功能作用，以揭示其工作原理。

Method: 研究引入了CogVision数据集，该数据集将复杂的多模态问题分解为逐步的子问题，旨在通过思维链范式模拟人类推理，每个子问题都与特定的感知或认知功能（如高级视觉接收和推理）相关联。研究采用基于探针的方法来识别专门负责这些功能的注意力头，并将其定性为功能头。此外，还进行了干预实验（移除或强调功能头）。

Result: 对不同VLM家族的分析表明，这些功能头普遍稀疏，在不同功能中数量和分布各异，并介导交互和层次组织。干预实验进一步证明了它们在多模态推理中的关键作用：移除功能头会导致性能下降，而强调它们则能提高准确性。

Conclusion: 这些发现为视觉-语言模型的认知组织提供了新的见解，并为设计具有更符合人类感知和推理能力的模型指明了有希望的方向。
...
TL;DR: 本研究设计并评估了EpiPlanAgent，一个基于大型语言模型（LLMs）的多智能体系统，旨在自动化生成和验证数字疫情应急响应计划，显著提高了计划的完整性和规范性，并缩短了开发时间。


<details>
  <summary>Details</summary>
Motivation: 疫情响应规划至关重要，但传统方法依赖于劳动密集型的人工操作。

Method: 开发了一个名为EpiPlanAgent的多智能体框架，该框架集成了任务分解、知识基础和模拟模块。公共卫生专业人员使用真实世界疫情场景在受控评估中测试了该系统。

Result: EpiPlanAgent显著提高了计划的完整性和与指南的一致性，同时大幅缩短了开发时间。专家评估证实了AI生成内容与人工编写内容之间的高度一致性，用户反馈也表明了强大的实用性。

Conclusion: EpiPlanAgent为智能疫情响应规划提供了一个有效且可扩展的解决方案，展示了智能体AI在转变公共卫生准备方面的潜力。
...
TL;DR: 本研究解决了扩散模型中“密度比率引导”方法在使用异构模型时出现的“边际路径崩溃”问题。我们提出了一个预测崩溃的准则，并引入了ACE（指数自适应路径校正）方法，该方法通过时间变化的指数确保有效的概率路径，从而提高了组合生成任务的可靠性和性能。


<details>
  <summary>Details</summary>
Motivation: 预训练的扩散/流模型可以通过推理时引导适应新任务，其中“密度比率”方法是一种常用手段。然而，当结合不同噪声调度或数据集训练的异构模型时（如分子设计中的多种模型组合），该方法存在一个关键但未被形式化的失败模式：“边际路径崩溃”，即中间密度变得不可归一化。这限制了其在可控生成中的应用和可靠性。

Method: 首先，我们推导了一个简单的路径存在准则，仅通过噪声调度和指数就能精确预测崩溃发生的时间。其次，我们引入了ACE（指数自适应路径校正）方法，该方法将费曼-卡茨引导扩展到时间变化的指数，并保证生成有效的概率路径。

Result: 在合成2D基准测试和柔性姿态支架修饰任务中，ACE方法成功消除了路径崩溃，并实现了高引导的组合生成。与固定指数基线甚至专门的任务特定支架修饰模型相比，ACE显著改善了分布和对接指标。

Conclusion: 本研究将使用异构专家的密度比率引导方法从不稳定的启发式工具转变为可控生成的可靠工具，彻底解决了其存在的边际路径崩溃问题。
...
### [27] [V-OCBF: Learning Safety Filters from Offline Data via Value-Guided Offline Control Barrier Functions](https://arxiv.org/abs/2512.10822)
...
### [28] [User-Feedback-Driven Continual Adaptation for Vision-and-Language Navigation](https://arxiv.org/abs/2512.10322)
...
TL;DR: 该论文提出了EmerFlow，一个由LLM驱动的表示学习框架，用于推荐新兴商品。它通过LLM推理丰富商品特征，与现有推荐模型对齐，并利用元学习整合新交互，从而从有限交互中学习独特的嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有方法在推荐新兴商品时，通常假设它们只有少量或没有历史交互，这简化了问题。实际上，新兴商品的交互是逐渐积累的，一个好的模型需要平衡其独特性与与现有商品的共享模式。

Method: 本文提出了EmerFlow框架：首先，通过大型语言模型（LLM）推理丰富新兴商品的原始特征，生成独特的嵌入；其次，将这些表示与现有推荐模型的嵌入空间对齐；最后，通过元学习逐步整合新的交互来细化嵌入。

Result: 在电影和医药等多个领域进行的广泛实验表明，EmerFlow始终优于现有方法。

Conclusion: EmerFlow能够从有限的交互中为新兴商品学习到富有表现力的嵌入，有效解决了新兴商品推荐的挑战。
...
### [30] [REMISVFU: Vertical Federated Unlearning via Representation Misdirection for Intermediate Output Feature](https://arxiv.org/abs/2512.10348)
...
### [33] [Planning, Living and Judging: A Multi-agent LLM-based Framework for Cyclical Urban Planning](https://arxiv.org/abs/2412.20505)
...
### [34] [Representation of the structure of graphs by sequences of instructions](https://arxiv.org/abs/2512.10429)
...
### [37] [Remember Me, Refine Me: A Dynamic Procedural Memory Framework for Experience-Driven Agent Evolution](https://arxiv.org/abs/2512.10696)
...
### [39] [Replace, Don't Expand: Mitigating Context Dilution in Multi-Hop RAG via Fixed-Budget Evidence Assembly](https://arxiv.org/abs/2512.10787)
...
### [40] [Achieving Olympia-Level Geometry Large Language Model Agent via Complexity Boosting Reinforcement Learning](https://arxiv.org/abs/2512.10534)
...
### [41] [NormCode: A Semi-Formal Language for Context-Isolated AI Planning](https://arxiv.org/abs/2512.10563)
*Xin Guan*
...
### [42] [Phythesis: Physics-Guided Evolutionary Scene Synthesis for Energy-Efficient Data Center Design via LLMs](https://arxiv.org/abs/2512.10611)
...
### [43] [Refinement Contrastive Learning of Cell-Gene Associations for Unsupervised Cell Type Identification](https://arxiv.org/abs/2512.10640)
...
### [44] [CAPTAIN: Semantic Feature Injection for Memorization Mitigation in Text-to-Image Diffusion Models](https://arxiv.org/abs/2512.10655)
...
### [45] [On the Dynamics of Multi-Agent LLM Communities Driven by Value Diversity](https://arxiv.org/abs/2512.10665)
...
### [46] [AEBNAS: Strengthening Exit Branches in Early-Exit Networks through Hardware-Aware Neural Architecture Search](https://arxiv.org/abs/2512.10671)
...
### [47] [Challenges of Evaluating LLM Safety for User Welfare](https://arxiv.org/abs/2512.10687)
...
TL;DR: 本文提出“敏捷审议”（Agile Deliberation）框架，通过概念范围界定和迭代，帮助用户迭代地定义和完善视觉概念，以训练分类器，尤其适用于模糊和主观的概念。


<details>
  <summary>Details</summary>
Motivation: 现有的人机协作（human-in-the-loop）方法通常假设用户对概念有清晰、稳定的理解。然而，通过对内容审核专家的访谈发现，用户通常从模糊的概念开始，需要通过“概念审议”反复完善。这种现实与现有系统假设之间的差距促使本研究的进行。

Method: 研究将内容审核专家在审议中常用的策略操作化为“敏捷审议”人机协作框架，该框架明确支持演变和主观的概念。系统通过暴露边缘案例来帮助用户定义概念，包含两个审议阶段：1) 概念范围界定，将初始概念分解为结构化的子概念层次结构；2) 概念迭代，提出语义上的边缘示例供用户反思和反馈，以迭代地使图像分类器与用户不断演变的需求对齐。通过18次用户会话（每次1.5小时）进行评估。

Result: 敏捷审议框架的F1分数比自动化分解基线高出7.5%，比手动审议高出3%以上。参与者报告概念理解更清晰，认知负荷更低。

Conclusion: 敏捷审议是一种有效的人机协作框架，能够支持用户迭代地定义和完善演变且主观的视觉概念，从而提高分类器性能并改善用户体验和概念理解。
...
### [49] [Enhancing Radiology Report Generation and Visual Grounding using Reinforcement Learning](https://arxiv.org/abs/2512.10691)
...
### [50] [COMPARE: Clinical Optimization with Modular Planning and Assessment via RAG-Enhanced AI-OCT: Superior Decision Support for Percutaneous Coronary Intervention Compared to ChatGPT-5 and Junior Operators](https://arxiv.org/abs/2512.10702)
...
### [55] [Neuromorphic Eye Tracking for Low-Latency Pupil Detection](https://arxiv.org/abs/2512.09969)
*Paul Hueber,Luca Peres,Florian Pitters,Alejandro Gloriani,Oliver Rhodes*

Main category: cs.CV

TL;DR: 本文提出了一种基于脉冲神经网络（SNN）的事件驱动眼动追踪模型，通过替换循环和注意力模块并利用深度可分离卷积，显著提高了可穿戴设备的效率和实时性能，同时保持了高精度。


<details>
  <summary>Details</summary>
Motivation: 可穿戴系统（如AR/VR）的眼动追踪需要低延迟和毫瓦级功耗，但传统的基于帧的方法存在运动模糊、计算成本高和时间分辨率有限等问题。现有神经形态传感器和SNN方法要么过于专业化，要么性能不及现代ANN架构。因此，需要一种更高效且高性能的眼动追踪解决方案。

Method: 研究者将最先进的事件驱动眼动追踪模型中的循环和注意力模块替换为轻量级LIF（Leaky Integrate-and-Fire）层，并利用深度可分离卷积来降低模型复杂性，从而创建了该模型的神经形态版本。

Result: 所提出的模型实现了3.7-4.1像素的平均误差，接近专业神经形态系统Retina（3.24像素）的精度。与最接近的ANN变体相比，模型尺寸缩小了20倍，理论计算量减少了850倍。预计这些高效变体在1 kHz频率下功耗为3.9-4.9 mW，延迟为3毫秒。

Conclusion: 研究结果表明，高性能的事件驱动眼动追踪架构可以被重新设计为脉冲神经网络，从而实现显著的效率提升，并保持适合实时可穿戴设备部署的精度。
...
### [56] [ABBSPO: Adaptive Bounding Box Scaling and Symmetric Prior based Orientation Prediction for Detecting Aerial Image Objects](https://arxiv.org/abs/2512.10031)
*Woojin Lee,Hyugjae Chang,Jaeho Moon,Jaehyup Lee,Munchurl Kim*

Main category: cs.CV

TL;DR: 本文提出ABBSPO框架，用于弱监督定向目标检测（WS-OOD）。它通过自适应边界框缩放（ABBS）和基于对称先验的角度（SPA）损失，解决了现有HBox监督方法中尺度估计不准确和学习崩溃的问题，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 全监督定向目标检测成本高昂，而弱监督HBox-OOD方法在效率和准确性方面具有优势。然而，现有HBox监督方法存在局限性：一是直接比较GT HBox与预测RBox的最小外接矩形导致尺度估计不准确；二是当所有增强视图（原始、旋转、翻转）的预测都持续不正确时，学习容易崩溃。

Method: 本文提出了ABBSPO框架，包含两个核心组件：
1.  **自适应边界框缩放（ABBS）**：适当地缩放GT HBox，以优化每个预测RBox的尺寸，从而确保更准确的尺度预测。
2.  **对称先验角度（SPA）损失**：利用航空物体固有的对称性进行自监督学习，解决了先前方法中因所有三个增强视图（原始、旋转、翻转）预测持续不正确而导致的学习崩溃问题。

Result: 实验结果表明，ABBSPO在WS-OOD任务中取得了最先进的性能，优于现有方法。

Conclusion: ABBSPO框架通过自适应边界框缩放和对称先验角度损失，有效解决了HBox监督定向目标检测中尺度估计不准确和学习崩溃的限制，显著提升了模型性能。
...
### [57] [Diffusion Is Your Friend in Show, Suggest and Tell](https://arxiv.org/abs/2512.10038)
*Jia Cheng Hu,Roberto Cavicchioli,Alessandro Capotondi*

Main category: cs.CV

TL;DR: 本文提出一种结合扩散模型和自回归模型的新范式，通过扩散模型为自回归生成提供建议，以在离散领域（如图像字幕）中超越现有技术水平，并在COCO数据集上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 扩散去噪模型在生成式计算机视觉任务中表现出色，但在离散领域未能超越标准的自回归解决方案，最多只能与其持平。研究动机是结合两者的优势，利用扩散模型的双向和精炼能力以及自回归模型强大的语言结构。

Method: 提出一种新范式，即扩散模型为自回归生成提供“建议”，而非完全替代。具体实现为“Show, Suggest and Tell (SST)”模型，用于图像字幕任务。该方法将扩散模型的建议与自回归模型的生成过程相结合。

Result: SST模型在COCO数据集上取得了最先进的结果，在相似设置的模型中，未经强化学习达到了125.1 CIDEr-D，比现有最先进的自回归模型高出1.5点，比扩散模型高出2.5点。实验证明，建议模块与字幕质量呈正相关，表明这是一个有前景的研究方向。

Conclusion: 通过让扩散模型为自回归生成提供建议，成功结合了两者的优势，显著提高了图像字幕的质量。这表明将扩散模型的建议能力与自回归模型的结构化生成相结合是一个未被充分探索但极具潜力的研究方向。
...
### [58] [MetaVoxel: Joint Diffusion Modeling of Imaging and Clinical Metadata](https://arxiv.org/abs/2512.10041)
...
TL;DR: MetaVoxel是一种生成式联合扩散模型，通过学习跨所有变量的单一扩散过程，对医学影像数据和临床元数据进行联合分布建模，从而统一了多种传统上需要独立条件模型的任务，并支持灵活的零样本推理。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习方法在医学领域表现出色，但大多数方法都针对特定预测方向和输入变量集训练条件分布模型，这导致了任务和输入变量变化时需要独立的模型和重新训练。研究动机是寻找一种更统一、灵活的框架，能够处理多种任务和任意输入子集。

Method: 引入了MetaVoxel框架，这是一个生成式联合扩散模型，通过学习一个涵盖所有变量（影像数据和临床元数据）的单一扩散过程，来建模这些变量的联合分布。这种方法允许在不进行任务特定再训练的情况下，使用任意输入子集进行灵活的零样本推理。

Result: 研究使用来自九个数据集的10,000多张T1加权MRI扫描和配对的临床元数据，展示了单个MetaVoxel模型能够执行图像生成、年龄估计和性别预测，并且性能与已建立的任务特定基线相当。额外的实验也突出了其灵活推理的能力。

Conclusion: 联合多模态扩散为统一医学AI模型和实现更广泛的临床应用提供了有前景的方向。MetaVoxel的发现证明了这种方法在整合不同医学任务和提高模型灵活性方面的潜力。
...
### [59] [Independent Density Estimation](https://arxiv.org/abs/2512.10067)
*Jiahao Liu*
...
### [60] [TraceFlow: Dynamic 3D Reconstruction of Specular Scenes Driven by Ray Tracing](https://arxiv.org/abs/2512.10095)
...
### [61] [Hierarchical Instance Tracking to Balance Privacy Preservation with Accessible Information](https://arxiv.org/abs/2512.10102)
...
### [62] [Topological Conditioning for Mammography Models via a Stable Wavelet-Persistence Vectorization](https://arxiv.org/abs/2512.10151)
...
### [63] [Feature Coding for Scalable Machine Vision](https://arxiv.org/abs/2512.10209)
...
### [64] [Latent Chain-of-Thought World Modeling for End-to-End Driving](https://arxiv.org/abs/2512.10226)
...
### [65] [Multi-dimensional Preference Alignment by Conditioning Reward Itself](https://arxiv.org/abs/2512.10237)
*Jiho Jang,Jinyoung Kim,Kyungjune Baek,Nojun Kwak*

Main category: cs.CV

TL;DR: 该研究提出了MCDPO方法，通过解耦的Bradley-Terry目标和条件注入，解决了标准DPO在扩散模型对齐中因多维度奖励聚合导致的奖励冲突问题，实现了更优的性能和推理时的多轴控制。


<details>
  <summary>Details</summary>
Motivation: 标准的DPO（基于人类反馈的强化学习）在对齐扩散模型时，将美学质量和语义对齐等多种评估维度聚合为单一标量奖励，这导致了“奖励冲突”。当一个样本在某个维度上表现良好但在整体上不被偏好时，模型可能会错误地“遗忘”该维度的可取特征。

Method: 本文提出了多奖励条件DPO (MCDPO)。它通过引入解耦的Bradley-Terry目标来解决奖励冲突。MCDPO在训练过程中明确注入一个偏好结果向量作为条件，使模型能够在单一网络中独立学习每个奖励维度的正确优化方向。此外，还引入了维度奖励丢弃（dimensional reward dropout）以确保各维度间的平衡优化。

Result: 在Stable Diffusion 1.5和SDXL上的大量实验表明，MCDPO在基准测试中取得了卓越的性能。值得注意的是，其条件框架使得在推理时能够利用Classifier Free Guidance实现动态、多轴控制，以放大特定的奖励维度，且无需额外的训练或外部奖励模型。

Conclusion: MCDPO成功解决了扩散模型对齐中DPO的奖励冲突问题，通过独立优化多维度奖励，显著提升了模型性能，并提供了灵活且无需额外成本的多维度推理控制能力。
...
TL;DR: 该研究探讨了机器视觉数据通信中，通过机器特征编码（FCM）来降低带宽、保护隐私并维持任务精度的方法，并分析了不同视频编码器（H.26X）作为FCM内部编码器对机器任务性能的影响，发现现有编码器在特定任务上仍具可用性。


<details>
  <summary>Details</summary>
Motivation: 机器日益成为视觉数据的主要消费者，但现有机器对机器系统仍依赖为人类感知优化的像素级视频流，导致带宽密集、扩展性差且存在隐私风险。

Method: 本文介绍了MPEG重新设计的机器对机器通信方案：机器视频编码（VCM）在像素域应用任务感知编码工具，以及机器特征编码（FCM）压缩中间神经网络特征以降低码率、保护隐私并支持计算卸载。通过实验，评估了FCM的性能，并分析了H.26X系列编码器（H.265/HEVC、H.266/VVC、H.264/AVC）作为FCM内部编码器对机器任务性能的影响。

Result: FCM能够保持接近边缘推理的精度，同时显著降低码率。H.265/HEVC和H.266/VVC在机器任务性能上几乎相同，用HEVC替代VVC时平均BD-Rate增加1.39%。相比之下，H.264/AVC比VVC的平均BD-Rate增加32.28%。然而，对于跟踪任务，编码器选择的影响最小，HEVC甚至优于VVC（BD-Rate为-1.81%），AVC也表现良好（8.79%）。

Conclusion: FCM是机器对机器通信的有效方案，能够显著降低带宽并保持精度。对于特定任务（如跟踪），现有已部署的H.26X编码器（如HEVC甚至AVC）的硬件可以支持机器对机器通信而不会显著降低性能，这表明并非所有M2M场景都需要新的VVC硬件。
...
### [67] [Solving Semi-Supervised Few-Shot Learning from an Auto-Annotation Perspective](https://arxiv.org/abs/2512.10244)
*Tian Liu,Anwesha Basu,James Caverlee,Shu Kong*

Main category: cs.CV

TL;DR: 本文提出了一种名为SWIFT的新方法，通过结合分类器初始化和温度调优，解决了半监督少样本学习（SSFSL）中视觉-语言模型（VLM）概率分布“平坦”导致无标签数据利用率低的问题，显著提升了SSFSL的性能，甚至接近全监督学习。


<details>
  <summary>Details</summary>
Motivation: SSFSL在“自动标注”等现实应用中至关重要，但现有文献在利用强大的开源VLM及其预训练数据方面不足。虽然相关领域的少样本学习（FSL）已利用这些资源，但直接将现有SSL方法应用于VLM时表现不佳，其根本原因是VLM产生的softmax概率分布“平坦”，导致无标签数据利用率低和监督信号弱。

Method: 首先，作者尝试将现有SSL方法应用于VLM微调，发现性能不佳。通过深入分析，确定了VLM概率分布“平坦”是根本原因。为解决此问题，作者提出了两种简单技术：分类器初始化和温度调优，以提高伪标签的置信度分数，从而增加无标签数据的利用率并强化监督信号。在此基础上，提出了“分阶段微调与温度调优”（SWIFT）框架，该框架能使现有SSL方法有效利用有限标签数据、大量无标签数据以及从VLM预训练集中检索到的任务相关但有噪声的数据来微调VLM。

Result: 在五个SSFSL基准测试中，SWIFT的性能比最新的FSL和SSL方法提高了约5个准确率百分点。SWIFT的性能甚至可以与监督学习（使用真实标签对无标签数据进行VLM微调）相媲美。

Conclusion: SWIFT通过解决VLM在SSFSL中概率分布“平坦”的问题，成功地使现有SSL方法能够有效微调VLM。它显著提升了SSFSL的性能，使其能够利用开源VLM资源，并为现实世界的自动标注应用提供了强大的解决方案，其表现甚至接近于使用真实标签的全监督学习。
...
### [68] [THE-Pose: Topological Prior with Hybrid Graph Fusion for Estimating Category-Level 6D Object Pose](https://arxiv.org/abs/2512.10251)
*Eunho Lee,Chaehyeon Song,Seunghoon Jeong,Ayoung Kim*

Main category: cs.CV

TL;DR: THE-Pose提出了一种新的类别级6D姿态估计框架，通过表面嵌入的拓扑先验和混合图融合，有效整合2D图像上下文和3D几何结构，显著提升了复杂物体和遮挡下的姿态估计鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的3D图卷积（3D-GC）方法在类别级物体姿态估计中仅关注局部几何和深度信息，使其在面对复杂物体和视觉模糊时表现不佳，缺乏对类内变化的鲁棒性，需要全局上下文信息。

Method: 本文提出了THE-Pose框架，通过表面嵌入利用拓扑先验。它从图像域中提取一致且不变的拓扑特征，并使用混合图融合（HGF）模块将这些拓扑特征与点云特征自适应地融合，从而无缝连接2D图像上下文和3D几何结构，以确保对未知或复杂物体的稳定性。

Result: 在REAL275数据集上的广泛实验表明，THE-Pose比3D-GC基线（HS-Pose）提高了35.8%，并在所有关键指标上超越了之前的最先进方法7.2%。

Conclusion: THE-Pose通过引入拓扑先验和混合图融合，成功克服了现有3D-GC方法的局限性，为类别级6D姿态估计提供了更稳定和准确的解决方案，即使在显著遮挡下也能有效处理未知或复杂物体。
...
### [69] [RobustSora: De-Watermarked Benchmark for Robust AI-Generated Video Detection](https://arxiv.org/abs/2512.10248)
...
### [70] [GDKVM: Echocardiography Video Segmentation via Spatiotemporal Key-Value Memory with Gated Delta Rule](https://arxiv.org/abs/2512.10252)
...
### [71] [VLM-NCD:Novel Class Discovery with Vision-Based Large Language Models](https://arxiv.org/abs/2512.10262)
...
### [72] [Long-LRM++: Preserving Fine Details in Feed-Forward Wide-Coverage Reconstruction](https://arxiv.org/abs/2512.10267)
...
### [73] [Sample-wise Adaptive Weighting for Transfer Consistency in Adversarial Distillation](https://arxiv.org/abs/2512.10275)
...
### [74] [MotionEdit: Benchmarking and Learning Motion-Centric Image Editing](https://arxiv.org/abs/2512.10284)
...
### [75] [Efficient-VLN: A Training-Efficient Vision-Language Navigation Model](https://arxiv.org/abs/2512.10310)
*Duo Zheng,Shijia Huang,Yanyang Li,Liwei Wang*

Main category: cs.CV

TL;DR: 本文提出Efficient-VLN，一个高效的视觉语言导航（VLN）模型，通过设计高效记忆机制和动态混合策略，显著降低了多模态大语言模型在VLN中的训练开销，同时实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在视觉语言导航（VLN）中显示出巨大潜力，但其开发受到巨大训练开销的阻碍。主要问题包括：1) 处理长历史观察时，作为大量token序列导致的二次方计算负担；2) DAgger数据聚合过程中探索效率的权衡问题，即更多探索虽能提供错误恢复轨迹以应对测试时分布偏移，但会增加训练和推理的轨迹长度。

Method: 为解决上述挑战，本文提出Efficient-VLN。具体方法包括：1) 针对token处理负担，设计了两种高效记忆机制：渐进式记忆（progressive memory），动态分配更多token给最新观察；可学习递归记忆（learnable recursive memory），利用可学习token的键值缓存作为记忆状态。2) 引入动态混合策略（dynamic mixed policy），以平衡探索与效率的权衡。

Result: 广泛实验表明，Efficient-VLN在R2R-CE上达到64.2%的SR，在RxR-CE上达到67.0%的SR，均实现最先进性能。更重要的是，该模型仅消耗282个H800 GPU小时，与现有最先进方法相比，训练开销显著降低。

Conclusion: Efficient-VLN通过创新的记忆机制和动态策略，成功解决了MLLMs在VLN中面临的训练开销问题，不仅取得了卓越的导航性能，还在训练效率上实现了显著提升，为VLN的实际应用铺平了道路。
...
### [76] [ShotDirector: Directorially Controllable Multi-Shot Video Generation with Cinematographic Transitions](https://arxiv.org/abs/2512.10286)
...
### [77] [DualProtoSeg: Simple and Efficient Design with Text- and Image-Guided Prototype Learning for Weakly Supervised Histopathology Image Segmentation](https://arxiv.org/abs/2512.10314)
...
TL;DR: 本文提出一个简单有效的原型驱动框架，通过视觉-语言对齐改善弱监督病理图像语义分割（WSSS），解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 组织病理学中的WSSS受限于类间同质性、类内异质性以及基于CAM的区域收缩效应，导致分割效果不佳，需要更有效的区域发现方法。

Method: 该方法集成CoOp风格的可学习提示调优以生成文本原型，并结合可学习图像原型，形成一个捕获语义和外观线索的双模态原型库。为解决ViT表示中的过平滑问题，引入多尺度金字塔模块以增强空间精度和定位质量。

Result: 在BCSS-WSSS基准测试中，该方法超越了现有最先进技术。详细分析表明文本描述多样性、上下文长度以及文本和图像原型的互补行为均有益处。

Conclusion: 研究结果强调了联合利用文本语义和视觉原型学习在数字病理学WSSS中的有效性。
...
### [78] [Point2Pose: A Generative Framework for 3D Human Pose Estimation with Multi-View Point Cloud Dataset](https://arxiv.org/abs/2512.10321)
*Hyunsoo Lee,Daeum Jeon,Hyeokjae Oh*

Main category: cs.CV

TL;DR: 本文提出了一种名为Point2Pose的新型生成式方法，用于3D人体姿态估计，该方法通过序列点云和姿态历史来建模姿态分布。同时，还发布了一个大规模室内数据集MVPose3D。实验结果表明，该方法优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 3D人体姿态估计面临多项挑战，包括人体复杂的几何结构、关节自遮挡以及对大规模真实世界运动数据集的需求。

Method: 本文引入了Point2Pose框架，它能有效建模以序列点云和姿态历史为条件的姿态分布。具体而言，该方法采用时空点云编码器和姿态特征编码器来提取关节特征，然后通过一个基于注意力的生成式回归器进行处理。此外，本文还提出了一个大规模室内数据集MVPose3D，其中包含IMU数据、密集多视图点云和RGB图像等多种模态。

Result: 实验结果表明，所提出的方法在各种数据集上均优于基线模型，展现出卓越的性能。

Conclusion: Point2Pose方法在3D人体姿态估计任务中表现出优越的性能。
...
### [79] [Physically Aware 360$^\circ$ View Generation from a Single Image using Disentangled Scene Embeddings](https://arxiv.org/abs/2512.10293)
*Karthikeya KV,Narendra Bandaru*

Main category: cs.CV

TL;DR: Disentangled360是一种创新的3D感知技术，它将方向解耦体渲染与单图像360°独特视图合成相结合，应用于医学成像和自然场景重建。该框架能区分各向同性与各向异性贡献，采用双分支条件化和混合姿态无关锚定方法，实现快速、逼真且具方向性的视图合成，无需特定场景微调或昂贵模拟。


<details>
  <summary>Details</summary>
Motivation: 现有技术在处理各向异性光行为时过于简化，或在不同场景中缺乏通用性。研究旨在开发一种更先进的3D感知技术，能够更准确地处理光线行为，并能广泛应用于医学成像和自然场景重建，同时实现快速、逼真的视图合成，减少对昂贵模拟和特定场景微调的依赖。

Method: 该方法引入了Disentangled360，一个在高斯泼溅（Gaussian Splatting）骨干中明确区分各向同性与各向异性贡献的框架。它采用双分支条件化：一个分支针对CT强度驱动的体积数据散射进行优化，另一个分支通过标准化相机嵌入处理真实世界RGB场景。为解决尺度模糊并保持结构真实性，提出了一种混合姿态无关锚定方法，自适应采样场景深度和材料过渡，作为场景蒸馏过程中的稳定支点。该设计将术前放射模拟和消费级360°渲染整合到单一推理管线中。

Result: 在Mip-NeRF 360、RealEstate10K和DeepDRR数据集上的评估显示，Disentangled360在SSIM和LPIPS性能上表现优越。运行时评估证实其适用于交互式应用，能够实现快速、逼真且具有固有方向性的视图合成。

Conclusion: Disentangled360能够促进混合现实医疗监督、机器人感知和沉浸式内容创建，消除了场景特定微调或昂贵光子模拟的必要性。它提供了一种通用且高效的3D感知解决方案，具有固有的方向性，适用于多种应用场景。
...
### [80] [ConStruct: Structural Distillation of Foundation Models for Prototype-Based Weakly Supervised Histopathology Segmentation](https://arxiv.org/abs/2512.10316)
...
### [81] [Any4D: Unified Feed-Forward Metric 4D Reconstruction](https://arxiv.org/abs/2512.10935)
...
### [82] [EchoingPixels: Cross-Modal Adaptive Token Reduction for Efficient Audio-Visual LLMs](https://arxiv.org/abs/2512.10324)
...
### [83] [StainNet: A Special Staining Self-Supervised Vision Transformer for Computational Pathology](https://arxiv.org/abs/2512.10326)
...
### [84] [Simple Yet Effective Selective Imputation for Incomplete Multi-view Clustering](https://arxiv.org/abs/2512.10327)
...
### [85] [A Conditional Generative Framework for Synthetic Data Augmentation in Segmenting Thin and Elongated Structures in Biological Images](https://arxiv.org/abs/2512.10334)
*Yi Liu,Yichi Zhang*
...
### [86] [Zero-shot Adaptation of Stable Diffusion via Plug-in Hierarchical Degradation Representation for Real-World Super-Resolution](https://arxiv.org/abs/2512.10340)
...
### [87] [CoSPlan: Corrective Sequential Planning via Scene Graph Incremental Updates](https://arxiv.org/abs/2512.10342)
...
### [88] [Topology-Agnostic Animal Motion Generation from Text Prompt](https://arxiv.org/abs/2512.10352)
...
### [89] [Hybrid Transformer-Mamba Architecture for Weakly Supervised Volumetric Medical Segmentation](https://arxiv.org/abs/2512.10353)
...
### [90] [Are We Ready for RL in Text-to-3D Generation? A Progressive Investigation](https://arxiv.org/abs/2512.10949)
...
### [91] [mmCounter: Static People Counting in Dense Indoor Scenarios Using mmWave Radar](https://arxiv.org/abs/2512.10357)
...
### [92] [Tool-Augmented Spatiotemporal Reasoning for Streamlining Video Question Answering Task](https://arxiv.org/abs/2512.10359)
...
### [93] [Visual Funnel: Resolving Contextual Blindness in Multimodal Large Language Models](https://arxiv.org/abs/2512.10362)
...
### [94] [Point to Span: Zero-Shot Moment Retrieval for Navigating Unseen Hour-Long Videos](https://arxiv.org/abs/2512.10363)
...
### [95] [Breaking the Vicious Cycle: Coherent 3D Gaussian Splatting from Sparse and Motion-Blurred Views](https://arxiv.org/abs/2512.10369)
...
### [96] [RaLiFlow: Scene Flow Estimation with 4D Radar and LiDAR Point Clouds](https://arxiv.org/abs/2512.10376)
...
### [97] [Towards Fine-Grained Recognition with Large Visual Language Models: Benchmark and Optimization Strategies](https://arxiv.org/abs/2512.10384)
...
### [98] [Self-Supervised Contrastive Embedding Adaptation for Endoscopic Image Matching](https://arxiv.org/abs/2512.10379)
...
### [99] [Adaptive Dual-Weighted Gravitational Point Cloud Denoising Method](https://arxiv.org/abs/2512.10386)
...
### [100] [MultiHateLoc: Towards Temporal Localisation of Multimodal Hate Content in Online Videos](https://arxiv.org/abs/2512.10408)
...
### [101] [Neural Collapse in Test-Time Adaptation](https://arxiv.org/abs/2512.10421)
*Xiao Chen,Zhongjing Du,Jiazhen Huang,Xu Jiang,Li Lu,Jingyan Jiang,Zhi Wang*

Main category: cs.CV

TL;DR: 本文提出了一种基于样本级对齐坍缩（NC3+）和混合目标的新型测试时自适应（TTA）方法NCTTA，通过重新对齐特征与分类器权重来提高模型对域漂移的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的TTA方法缺乏对域漂移下性能下降根本原因的理论洞察。作者发现，在训练有素的模型中，样本的特征嵌入与其对应的分类器权重紧密对齐（NC3+），但域漂移会导致这种样本级错位，进而降低性能。此外，在这种错位下，伪标签变得不可靠，给适应带来了挑战。

Method: 1. 将神经坍缩（NC）扩展到样本级，发现并命名了“样本级对齐坍缩”（NC3+）现象。2. 识别出性能下降源于适应过程中的样本级错位，且在更大分布偏移下恶化。3. 提出NCTTA，一种新的特征-分类器对齐方法。4. NCTTA采用混合目标策略（结合几何邻近性和预测置信度）来减轻不可靠伪标签的影响。

Result: NCTTA显著增强了模型对域漂移的鲁棒性。例如，在ImageNet-C数据集上，NCTTA的表现比Tent高出14.52%。

Conclusion: 通过发现样本级对齐坍缩（NC3+）并识别其错位是性能下降的原因，本文提出了NCTTA方法。NCTTA通过结合几何邻近性和预测置信度的混合目标策略，有效地解决了特征-分类器错位和不可靠伪标签的问题，从而显著提高了模型在域漂移下的鲁棒性。
...
### [102] [TransLocNet: Cross-Modal Attention for Aerial-Ground Vehicle Localization with Contrastive Learning](https://arxiv.org/abs/2512.10419)
...
### [103] [Beyond Endpoints: Path-Centric Reasoning for Vectorized Off-Road Network Extraction](https://arxiv.org/abs/2512.10416)
...
### [104] [An M-Health Algorithmic Approach to Identify and Assess Physiotherapy Exercises in Real Time](https://arxiv.org/abs/2512.10437)
...
### [105] [Error-Propagation-Free Learned Video Compression With Dual-Domain Progressive Temporal Alignment](https://arxiv.org/abs/2512.10450)
...
### [106] [Robust Shape from Focus via Multiscale Directional Dilated Laplacian and Recurrent Network](https://arxiv.org/abs/2512.10498)
...
### [107] [Beyond Pixels: A Training-Free, Text-to-Text Framework for Remote Sensing Image Retrieval](https://arxiv.org/abs/2512.10596)
...
### [108] [3D Blood Pulsation Maps](https://arxiv.org/abs/2512.10517)
...
### [109] [What matters for Representation Alignment: Global Information or Spatial Structure?](https://arxiv.org/abs/2512.10794)
...
### [110] [Take a Peek: Efficient Encoder Adaptation for Few-Shot Semantic Segmentation via LoRA](https://arxiv.org/abs/2512.10521)
...
### [111] [Blink: Dynamic Visual Token Resolution for Enhanced Multimodal Understanding](https://arxiv.org/abs/2512.10548)
...
### [112] [MMSI-Video-Bench: A Holistic Benchmark for Video-Based Spatial Intelligence](https://arxiv.org/abs/2512.10863)
...
### [113] [Grounding Everything in Tokens for Multimodal Large Language Models](https://arxiv.org/abs/2512.10554)
...
### [114] [BabyVLM-V2: Toward Developmentally Grounded Pretraining and Benchmarking of Vision Foundation Models](https://arxiv.org/abs/2512.10932)
...
### [115] [Audio-sync Video Instance Editing with Granularity-Aware Mask Refiner](https://arxiv.org/abs/2512.10571)
*Haojie Zheng,Shuchen Weng,Jingqi Liu,Siqi Yang,Boxin Shi,Xinlong Wang*

Main category: cs.CV

TL;DR: 本文提出了AVI-Edit框架，用于实现音频同步的视频实例编辑，通过粒度感知掩码细化器和自反馈音频代理，提供精细的空间和时间控制，并构建了一个大规模数据集。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法强调视听同步的重要性，但当前的视频编辑方法大多忽视了视听同步，并且缺乏对实例级编辑所需的精细空间和时间控制。

Method: 本文提出了AVI-Edit框架，包含：1) 一个粒度感知掩码细化器，用于将粗略用户掩码迭代细化为精确的实例级区域。2) 一个自反馈音频代理，用于生成高质量音频指导，提供精细的时间控制。此外，还构建了一个包含实例中心对应和全面注释的大规模数据集。

Result: 广泛的实验表明，AVI-Edit在视觉质量、条件遵循和视听同步方面均优于现有最先进的方法。

Conclusion: AVI-Edit提供了一个有效且精确的解决方案，能够实现音频同步的视频实例编辑，显著提升了视频编辑的精细控制能力和视听同步质量。
...
### [116] [OmniView: An All-Seeing Diffusion Model for 3D and 4D View Synthesis](https://arxiv.org/abs/2512.10940)
...
### [117] [Data-Efficient American Sign Language Recognition via Few-Shot Prototypical Networks](https://arxiv.org/abs/2512.10562)
*Meher Md Saad*
...
### [118] [Unleashing Degradation-Carrying Features in Symmetric U-Net: Simpler and Stronger Baselines for All-in-One Image Restoration](https://arxiv.org/abs/2512.10581)
...
### [119] [Mull-Tokens: Modality-Agnostic Latent Thinking](https://arxiv.org/abs/2512.10941)
...
### [120] [AlcheMinT: Fine-grained Temporal Control for Multi-Reference Consistent Video Generation](https://arxiv.org/abs/2512.10943)
...
### [121] [SceneMaker: Open-set 3D Scene Generation with Decoupled De-occlusion and Pose Estimation Model](https://arxiv.org/abs/2512.10957)
...
### [122] [Salient Object Detection in Complex Weather Conditions via Noise Indicators](https://arxiv.org/abs/2512.10592)
...
### [123] [Track and Caption Any Motion: Query-Free Motion Discovery and Description in Videos](https://arxiv.org/abs/2512.10607)
...
### [125] [Lang2Motion: Bridging Language and Motion through Joint Embedding Spaces](https://arxiv.org/abs/2512.10617)
...
### [126] [SpaceDrive: Infusing Spatial Awareness into VLM-based Autonomous Driving](https://arxiv.org/abs/2512.10719)
*Peizheng Li,Zhenghao Zhang,David Holtz,Hang Yu,Yutong Yang,Yuzhi Lai,Rui Song,Andreas Geiger,Andreas Zell*

Main category: cs.CV

TL;DR: 现有基于视觉语言模型（VLM）的自动驾驶方法在细粒度3D空间理解方面存在不足。本文提出SpaceDrive，通过将空间信息编码为显式位置编码（PE），而非文本数字，实现了语义与空间表示的联合推理，显著提升了规划精度，并在多个基准测试中达到了最先进或接近最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基于VLM的端到端自动驾驶发展迅速，但它们在理解细粒度的3D空间关系方面表现不佳，而这对于与物理世界交互的系统是基本要求。

Method: SpaceDrive是一种空间感知的VLM自动驾驶框架。它将3D空间信息视为显式位置编码（PEs），而非文本数字标记，从而实现语义和空间表示的联合推理。它使用一个通用位置编码器处理来自多视角深度估计、历史自我状态和文本提示的所有3D坐标。这些3D PEs被叠加以增强2D视觉标记，同时作为任务无关的坐标表示，替代数字式数值标记作为VLM的输入和输出，从而使模型能更好地进行空间推理和直接回归轨迹坐标。

Result: SpaceDrive在nuScenes数据集上取得了最先进的开环性能，并在Bench2Drive闭环基准测试中，相对于现有VLM方法，取得了78.02的第二佳驾驶分数。

Conclusion: SpaceDrive通过将空间信息作为显式位置编码进行处理，有效解决了VLM在细粒度3D空间理解上的不足，显著提升了自动驾驶系统的空间推理能力和规划精度。
...
### [127] [DOCR-Inspector: Fine-Grained and Automated Evaluation of Document Parsing with VLM](https://arxiv.org/abs/2512.10619)
...
### [128] [K-Track: Kalman-Enhanced Tracking for Accelerating Deep Point Trackers on Edge Devices](https://arxiv.org/abs/2512.10628)
...
### [129] [TriDF: Evaluating Perception, Detection, and Hallucination for Interpretable DeepFake Detection](https://arxiv.org/abs/2512.10652)
...
### [130] [XDen-1K: A Density Field Dataset of Real-World Objects](https://arxiv.org/abs/2512.10668)
...
### [131] [Geo6DPose: Fast Zero-Shot 6D Object Pose Estimation via Geometry-Filtered Feature Matching](https://arxiv.org/abs/2512.10674)
...
### [132] [NaviHydra: Controllable Navigation-guided End-to-end Autonomous Driving with Hydra-distillation](https://arxiv.org/abs/2512.10660)
...
### [133] [Optimal transport unlocks end-to-end learning for single-molecule localization](https://arxiv.org/abs/2512.10683)
...
### [134] [Sharp Monocular View Synthesis in Less Than a Second](https://arxiv.org/abs/2512.10685)
...
### [135] [CheXmask-U: Quantifying uncertainty in landmark-based anatomical segmentation for X-ray images](https://arxiv.org/abs/2512.10715)
...
### [136] [Video Depth Propagation](https://arxiv.org/abs/2512.10725)
...
### [137] [IRG-MotionLLM: Interleaving Motion Generation, Assessment and Refinement for Text-to-Motion Generation](https://arxiv.org/abs/2512.10730)
...
### [138] [LDP: Parameter-Efficient Fine-Tuning of Multimodal LLM for Medical Report Generation](https://arxiv.org/abs/2512.10750)
...
### [139] [Blood Pressure Prediction for Coronary Artery Disease Diagnosis using Coronary Computed Tomography Angiography](https://arxiv.org/abs/2512.10765)
...
### [140] [Graph Laplacian Transformer with Progressive Sampling for Prostate Cancer Grading](https://arxiv.org/abs/2512.10808)
...
### [141] [Self-Ensemble Post Learning for Noisy Domain Generalization](https://arxiv.org/abs/2512.10818)
...
### [142] [SWiT-4D: Sliding-Window Transformer for Lossless and Parameter-Free Temporal 4D Generation](https://arxiv.org/abs/2512.10860)
...
### [143] [PoseGAM: Robust Unseen Object Pose Estimation via Geometry-Aware Multi-View Reasoning](https://arxiv.org/abs/2512.10840)
...
### [144] [From Macro to Micro: Benchmarking Microscopic Spatial Intelligence on Molecules via Vision-Language Models](https://arxiv.org/abs/2512.10867)
...
TL;DR: 本文提出微观空间智能（MiSI）概念，并构建MiSI-Bench基准评估视觉-语言模型（VLMs）在此领域的能力。结果显示当前VLMs远低于人类水平，但微调模型在某些任务上表现出巨大潜力，并强调了领域知识对科学AGI的重要性。


<details>
  <summary>Details</summary>
Motivation: 微观空间智能（MiSI）是科学发现的基础能力，涉及感知和推理不可见微观实体的空间关系。本研究旨在系统评估当前视觉-语言模型（VLMs）在此关键领域的潜力。

Method: 研究提出了MiSI-Bench基准框架，包含超过163,000个问答对和587,000张图像，这些数据来源于约4,000个分子结构。该基准涵盖了9项互补任务，从基本空间变换到复杂关系识别，以全面评估模型的MiSI能力。

Result: 实验结果表明，当前最先进的视觉-语言模型在MiSI-Bench上的表现远低于人类水平。然而，一个经过微调的7B模型展现出巨大潜力，在空间变换任务上甚至超越了人类。但在像氢键识别这类科学基础任务上，该模型表现不佳。

Conclusion: 当前VLMs在微观空间智能方面仍有显著差距，尤其在需要科学领域知识的任务上。为了实现科学领域的通用人工智能（AGI），必须整合明确的领域知识以促进模型进步。
...
### [145] [MoCapAnything: Unified 3D Motion Capture for Arbitrary Skeletons from Monocular Videos](https://arxiv.org/abs/2512.10881)
...
### [146] [PubTables-v2: A new large-scale dataset for full-page and multi-page table extraction](https://arxiv.org/abs/2512.10888)
...
### [147] [DuetSVG: Unified Multimodal SVG Generation with Internal Visual Guidance](https://arxiv.org/abs/2512.10894)
...
### [148] [FoundationMotion: Auto-Labeling and Reasoning about Spatial Movement in Videos](https://arxiv.org/abs/2512.10927)
...
### [149] [VL-JEPA: Joint Embedding Predictive Architecture for Vision-language](https://arxiv.org/abs/2512.10942)
...
### [150] [MeViS: A Multi-Modal Dataset for Referring Motion Expression Video Segmentation](https://arxiv.org/abs/2512.10945)
...
### [151] [GaussianHeadTalk: Wobble-Free 3D Talking Heads with Audio Driven Gaussian Splatting](https://arxiv.org/abs/2512.10939)
...
### [152] [ClusIR: Towards Cluster-Guided All-in-One Image Restoration](https://arxiv.org/abs/2512.10948)
...
### [153] [Towards Efficient and Effective Multi-Camera Encoding for End-to-End Driving](https://arxiv.org/abs/2512.10947)
...
### [154] [E-RayZer: Self-supervised 3D Reconstruction as Spatial Visual Pre-training](https://arxiv.org/abs/2512.10950)
...
### [155] [Group Diffusion: Enhancing Image Generation by Unlocking Cross-Sample Collaboration](https://arxiv.org/abs/2512.10954)
...
### [156] [Omni-Attribute: Open-vocabulary Attribute Encoder for Visual Concept Personalization](https://arxiv.org/abs/2512.10955)
...
### [157] [Empowering Dynamic Urban Navigation with Stereo and Mid-Level Vision](https://arxiv.org/abs/2512.10956)
...
### [158] [WorldLens: Full-Spectrum Evaluations of Driving World Models in Real World](https://arxiv.org/abs/2512.10958)
...
### [159] [StereoSpace: Depth-Free Synthesis of Stereo Geometry via End-to-End Diffusion in a Canonical Space](https://arxiv.org/abs/2512.10959)
...
### [160] [What Kind of Reasoning (if any) is an LLM actually doing? On the Stochastic Nature and Abductive Appearance of Large Language Models](https://arxiv.org/abs/2512.10080)
*Luciano Floridi,Jessica Morley,Claudio Novelli,David Watson*

Main category: cs.CL

TL;DR: 本文分析了大型语言模型（LLMs）的推理机制，指出其基于学习模式而非真正的溯因推理，其看似溯因的输出源于训练数据，并强调了评估和应用LLM时需批判性地审视其输出。


<details>
  <summary>Details</summary>
Motivation: 研究当前基于token补全的LLMs如何进行推理，特别是它们随机性本质与人类溯因推理的相似性，以厘清LLMs是否真正执行溯因推理。

Method: 文章通过审视LLMs的随机性本质及其token补全机制，并使用具体例子展示LLMs如何在缺乏真实性、语义理解或验证能力的情况下，生成看似溯因的、合理且具有解释性的文本。

Result: LLMs的文本生成基于学习到的模式，而非实际的溯因推理。其输出看似溯因，主要是因为它们在包含推理结构的人类生成文本上进行训练。LLMs可以在不理解、不验证或不基于真相的情况下，产生合理想法和解释性答案。

Conclusion: LLMs具有随机性基础但表现出溯因性，这对它们的评估和应用具有重要影响。它们可以辅助人类思考和生成想法，但由于无法识别真相或验证解释，其输出必须经过批判性评估。
...
### [161] [Generate-Then-Validate: A Novel Question Generation Approach Using Small Language Models](https://arxiv.org/abs/2512.10110)
*Yumou Wei,John Stamper,Paulo F. Carvalho*

Main category: cs.CL

TL;DR: 本文探索了使用小型语言模型（SLM）进行自动问题生成，并提出了一种结合生成和概率推理的新型流水线，实现了高质量问题生成，并通过专家和大型语言模型评估验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索小型语言模型（SLM）在学习分析研究中自动问题生成方面的潜力，以补充大型语言模型（LLM）的普遍应用。

Method: 本文提出了一种新颖的问题生成流水线，利用SLM的文本生成和概率推理能力。该流水线采用“先生成后验证”策略，首先进行大量候选问题生成，然后基于新颖的概率推理进行选择性验证。通过两项评估研究，分别由七位人类专家和一个大型语言模型（LLM）评估了生成问题的质量。

Result: 大多数评判者（无论是人类专家还是LLM）都认为生成的问题有明确的答案，并且与预期的学习目标高度一致。研究结果表明，在设计良好的流水线指导下，SLM可以有效地生成高质量问题。

Conclusion: 研究结论是，当由一个充分利用其优势的精心设计的流水线引导时，小型语言模型（SLM）能够有效地生成高质量的问题。
...
### [162] [Workflow is All You Need: Escaping the "Statistical Smoothing Trap" via High-Entropy Information Foraging and Adversarial Pacing](https://arxiv.org/abs/2512.10121)
*Zhongjie Jiang*

Main category: cs.CL

TL;DR: 本研究指出当前大型语言模型在垂直领域长文本生成中面临“不可能三角”和“统计平滑陷阱”问题。为此，提出DeepNews框架，通过模拟专家认知过程，集成了双粒度检索、图式引导规划和对抗性约束提示模块。实验证明DeepNews能显著提高金融报道的真实性和接受率，优于SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在垂直领域长文本生成中，难以同时实现低幻觉、深层逻辑连贯性和个性化表达，形成“不可能三角”。研究认为这是由于现有生成范式陷入“统计平滑陷阱”，忽略了专家级写作所需的高熵信息获取和结构化认知过程。

Method: 提出DeepNews框架，一个代理式工作流，显式模拟资深金融记者的隐式认知过程。框架包含三个核心模块：1) 基于信息觅食理论的双粒度检索机制，确保10:1的饱和信息输入比以减少幻觉；2) 图式引导的战略规划，利用领域专家知识库（叙事图式）和原子块构建稳健的逻辑骨架；3) 对抗性约束提示，通过“节奏中断”和“逻辑迷雾”等策略，干扰模型生成文本固有的概率平滑性。

Result: 实验揭示了深度金融报道中的“知识悬崖”现象：当检索上下文低于15,000字符时，内容真实性会急剧下降；而超过30,000字符的高冗余输入能将无幻觉率（HFR）稳定在85%以上。在一家顶级中国科技媒体的盲测中，基于上一代模型（DeepSeek-V3-0324）构建的DeepNews系统获得了25%的提交接受率，显著优于最先进模型（GPT-5）零样本生成0%的接受率。

Conclusion: DeepNews框架通过模拟专家认知过程，有效解决了大型语言模型在垂直领域长文本生成中面临的幻觉、逻辑连贯性不足和缺乏个性化表达的问题，显著提升了生成内容的真实性和专业性，证明了其在专家级写作方面的优越性。
...
### [163] [PARAN: Persona-Augmented Review ANswering system on Food Delivery Review Dataset](https://arxiv.org/abs/2512.10148)
*Moonsoo Park,Jeongseok Yun,Bohyung Kim*

Main category: cs.CL

TL;DR: 本文提出一个两阶段提示框架，从短评论中推断用户画像（显式和隐式），并将其融入大型语言模型的响应生成提示中，以在用户信息有限的场景下，无需模型微调即可生成个性化回复。


<details>
  <summary>Details</summary>
Motivation: 在用户信息有限的领域（如外卖平台），大型语言模型在生成评论回复时常因缺乏上下文用户数据而产生通用回复，降低了用户参与度和有效性。因此，需要一种方法来提升回复的个性化和相关性。

Method: 研究者提出了一个两阶段提示框架：首先，直接从短评论文本中推断用户的显式（如偏好）和隐式（如人口统计或风格）画像属性；其次，将这些推断出的画像属性整合到响应生成提示中，以生成用户定制的回复。为鼓励多样且忠实的生成，还在推理过程中调整了解码温度。该方法使用韩国外卖应用的真实世界数据集进行评估，并衡量了其对精确度、多样性和语义一致性的影响。

Result: 研究结果表明，通过增强用户画像的提示方法，在无需模型微调的情况下，能有效提高自动化回复的相关性和个性化。

Conclusion: 该研究证明了在用户数据有限的情况下，通过两阶段提示框架推断用户画像并将其融入LLM提示中，能够显著提升自动回复的个性化和相关性，且无需对模型进行微调，为个性化评论回复生成提供了一种有效解决方案。
...
### [164] [Unforgotten Safety: Preserving Safety Alignment of Large Language Models with Continual Learning](https://arxiv.org/abs/2512.10150)
*Lama Alssum,Hani Itani,Hasan Abed Al Kader Hammoud,Philip Torr,Adel Bibi,Bernard Ghanem*

Main category: cs.CL

TL;DR: 本文将大型语言模型（LLM）微调过程中的安全退化问题视为持续学习（CL）问题，并系统评估了多种CL方法在保持任务效用的同时缓解安全退化方面的能力，发现CL方法（尤其是DER）能有效提升安全性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLM）的普及，其安全对齐变得日益重要。然而，将LLM适应新任务时，会发生安全性能下降，这主要是由灾难性遗忘引起的。研究旨在解决在微调过程中如何保持LLM安全性的问题。

Method: 研究将微调时保持安全性的问题框架为持续学习（CL）问题，并采用“微调即服务”的设置。本文适配并系统评估了多种CL方法，包括基于正则化、基于内存和模型合并的方法。实验考虑了两种场景：良性用户数据和被投毒的用户数据。评估在三个下游任务（GSM8K、SST2、Code）和三个模型家族（LLaMA2-7B、Mistral-7B、Gemma-2B）上进行。

Result: 结果表明，与标准微调相比，持续学习方法能持续实现更低的安全攻击成功率。在这些方法中，DER在保持任务效用的同时，优于其他CL方法和现有的安全保持基线。这些发现适用于不同的下游任务和模型家族。

Conclusion: 持续学习是一种实用的解决方案，可以在LLM适应新任务时有效保持其安全性。特别是，DER方法表现出色，为解决LLM微调中的安全退化问题提供了有效的途径。
...
### [165] [AutoMedic: An Automated Evaluation Framework for Clinical Conversational Agents with Medical Dataset Grounding](https://arxiv.org/abs/2512.10195)
*Gyutaek Oh,Sangjoon Park,Byung-Hoon Kim*

Main category: cs.CL

TL;DR: AutoMedic是一个多智能体模拟框架，通过将静态QA数据集转换为虚拟患者档案，实现LLM作为临床对话代理的自动化评估，并使用CARE多维度指标（准确性、效率/策略、同理心、鲁棒性）进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有静态医学问答基准未能充分评估LLM在动态、交互式、多轮临床对话中的表现，也缺乏超越简单准确性的多方面评估策略。正式评估动态交互式临床情境因其巨大的组合空间而面临标准化和量化测量的困难。

Method: 引入AutoMedic，一个多智能体模拟框架。它将现成的静态问答数据集转换为虚拟患者档案，从而在LLM代理之间实现真实且基于临床的多轮对话。性能评估采用CARE指标，该指标提供了临床对话准确性、效率/策略、同理心和鲁棒性的多方面评估标准。

Result: 通过人类专家的验证，研究结果证明了AutoMedic作为临床对话代理自动化评估框架的有效性。

Conclusion: AutoMedic为对话式医疗应用中LLM的有效开发提供了实用指导。
...
### [166] [Multilingual VLM Training: Adapting an English-Trained VLM to French](https://arxiv.org/abs/2512.10336)
...
### [167] [Confucius Code Agent: An Open-sourced AI Software Engineer at Industrial Scale](https://arxiv.org/abs/2512.10398)
...
### [168] [Sliding Window Attention Adaptation](https://arxiv.org/abs/2512.10411)
*Yijiong Yu,Jiale Liu,Qingyun Wu,Huazheng Wang,Ji Pei*

Main category: cs.CL

TL;DR: 本文提出滑动窗口注意力适应（SWAA）方法，通过结合五种策略，使预训练时使用全注意力（FA）的大型语言模型（LLMs）能够在不重新预训练的情况下，有效适应滑动窗口注意力（SWA），从而实现长上下文推理的效率提升，同时恢复性能。


<details>
  <summary>Details</summary>
Motivation: Transformer模型中的自注意力机制计算成本随输入长度呈二次方增长，导致长上下文推理昂贵。滑动窗口注意力（SWA）能将其降至线性复杂度，但直接在预训练使用全注意力（FA）的模型上应用SWA会导致训练-推理不匹配，严重降低长上下文性能。

Method: 研究提出滑动窗口注意力适应（SWAA）方案，结合了五种方法以更好地适应SWA：(1) 仅在预填充阶段应用SWA；(2) 保留“sink”令牌；(3) 交错使用FA/SWA层；(4) 链式思考（CoT）；以及(5) 微调。

Result: 实验表明，SWA适应是可行的但并非易事，单一方法不足以解决问题，但特定的协同组合能够有效恢复原始的长上下文性能。研究还分析了不同SWAA配置的性能-效率权衡，并提供了针对不同场景的推荐方案。

Conclusion: 预训练时使用全注意力的LLMs可以通过特定的协同适应方法（SWAA）成功地适应滑动窗口注意力，从而在不进行预训练的情况下，以线性复杂度实现高效的长上下文推理，同时克服训练-推理不匹配导致的性能下降。
...
### [169] [T-pro 2.0: An Efficient Russian Hybrid-Reasoning Model and Playground](https://arxiv.org/abs/2512.10430)
...
TL;DR: T-pro 2.0是一个开源的俄罗斯大型语言模型（LLM），专为混合推理和高效推断设计，支持直接回答和推理链生成，并通过发布模型权重、数据集和基准，促进了俄罗斯语言推理的可复现研究。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在开发一个针对俄罗斯语言的LLM，能够支持混合推理和高效推断，同时通过开源模型和资源，促进可复现和可扩展的俄罗斯语言推理研究。

Method: 引入T-pro 2.0模型，该模型支持直接回答和推理链生成。采用西里尔字母密集型分词器和适应的EAGLE推测解码管道以降低推理延迟。同时，发布了模型权重、T-Wix 500k指令语料库、T-Math推理基准和EAGLE权重，以支持研究。

Result: T-pro 2.0模型成功实现了混合推理和高效推断，能够直接回答并生成推理过程。通过EAGLE推测解码管道，推理速度得到了显著提升。所有相关资源（模型、数据集、基准）均已开源，并通过公共网络演示展示了其在不同领域的速度优势。

Conclusion: T-pro 2.0提供了一个可访问的开放系统，为构建和评估高效、实用的俄罗斯LLM应用奠定了基础，并促进了俄罗斯语言推理领域的研究和发展。
...
### [170] [Cooperative Retrieval-Augmented Generation for Question Answering: Mutual Information Exchange and Ranking by Contrasting Layers](https://arxiv.org/abs/2512.10422)
*Youmin Ko,Sungjong Seo,Hyunjoon Kim*

Main category: cs.CL

TL;DR: CoopRAG是一个新颖的检索增强生成（RAG）框架，通过检索器和大型语言模型（LLM）之间的合作以及检索器内部不同层之间的合作，解决了现有RAG方法在问答任务中检索不准确和幻觉问题，从而提高了多跳和简单问答的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）容易生成事实不准确的内容，而现有的检索增强生成（RAG）方法虽然旨在缓解此问题，但仍存在检索不准确和产生幻觉的缺点，尤其是在简单和多跳问答任务中。

Method: CoopRAG框架采用以下方法：(i) 将问题分解为子问题和带有不确定掩码位置的推理链；(ii) 使用增强后的问题（包含子问题和推理链）检索相关文档；(iii) 通过对比检索器模型的不同层来重新排序检索到的文档；(iv) 通过LLM填充掩码位置来重建推理链。该框架强调检索器和LLM以及检索器内部层之间的合作。

Result: 实验结果表明，CoopRAG在三个多跳问答数据集以及一个简单问答数据集上，在检索和问答性能方面均持续优于现有的最先进问答方法。

Conclusion: CoopRAG通过引入检索器与LLM之间以及检索器内部层之间的合作机制，有效解决了现有RAG方法在问答任务中检索不准确和幻觉的局限性，显著提升了问答性能。
...
### [171] [Semantic Reconstruction of Adversarial Plagiarism: A Context-Aware Framework for Detecting and Restoring "Tortured Phrases" in Scientific Literature](https://arxiv.org/abs/2512.10435)
*Agniva Maiti,Prajwal Panth,Suresh Chandra Satapathy*

Main category: cs.CL

TL;DR: 本文提出SRAP框架，用于检测科学文献中由自动释义工具生成的“扭曲短语”，并能数学地恢复其原始术语，以对抗学术剽窃。


<details>
  <summary>Details</summary>
Motivation: 科学文献的完整性受到对抗性文本生成技术（特别是自动释义工具）的严重威胁，这些工具通过生成“扭曲短语”来掩盖抄袭。现有检测方法依赖静态黑名单或通用语言模型，导致高假阴性率且无法确定剽窃来源。

Method: 本文提出语义对抗性剽窃重建（SRAP）框架，采用两阶段架构：1. 统计异常检测，使用领域特定掩码语言模型（SciBERT）和token级伪困惑度。2. 基于源的语义重建，使用稠密向量检索（FAISS）和句子级对齐（SBERT）。研究还表明，在专业术语密集的科学文本中，静态决策边界对于鲁棒检测是必要的。

Result: 在对抗性科学文本并行语料库上的实验表明，零样本基线方法完全失败（0.00%恢复准确率），而SRAP的检索增强方法达到了23.67%的恢复准确率，显著优于基线方法。同时，动态阈值在方差高的情况下失败，证实了静态决策边界的重要性。

Conclusion: SRAP框架不仅能检测“扭曲短语”等异常，还能数学地恢复原始术语。它通过将模糊表达链接回最可能的源文档，实现了法证分析，有效对抗学术剽窃。
...
### [172] [Decoding Student Minds: Leveraging Conversational Agents for Psychological and Learning Analysis](https://arxiv.org/abs/2512.10441)
*Nour El Houda Ben Chaabene,Hamza Hammami,Laid Kahloul*

Main category: cs.CL

TL;DR: 本文提出一个心理感知会话代理，通过结合LLM、知识图谱增强的BERT和带有注意力的双向LSTM，实时分类学生认知和情感状态，利用多模态数据提升学习表现和情绪健康。


<details>
  <summary>Details</summary>
Motivation: 现有聊天机器人仅限于辅导或情感支持，未能同时解决学习表现和情感健康问题。研究旨在开发一个能够实时推断学生认知和情感状态的系统，以提供自适应、以学生为中心的教育干预。

Method: 该系统结合了大型语言模型（LLMs）、知识图谱增强的BERT（KG-BERT）以及带有注意力的双向长短期记忆网络（Bi-LSTM），用于实时分类学生的认知和情感状态。它利用多模态数据，包括文本语义、韵律语音特征和时间行为趋势，来推断学生的投入度、压力和概念理解。

Result: 一项针对大学生的初步研究表明，与基线方法相比，该系统显著提高了学生的学习动机，减轻了压力，并取得了适度的学业进步。

Conclusion: 研究结果强调了整合语义推理、多模态融合和时间建模在支持自适应、以学生为中心的教育干预方面的巨大潜力。
...
### [173] [Enhancing Next-Generation Language Models with Knowledge Graphs: Extending Claude, Mistral IA, and GPT-4 via KG-BERT](https://arxiv.org/abs/2512.10440)
...
### [174] [Grammaticality Judgments in Humans and Language Models: Revisiting Generative Grammar with LLMs](https://arxiv.org/abs/2512.10453)
*Lars G. B. Johnsen*
...
### [175] [XDoGE: Multilingual Data Reweighting to Enhance Language Inclusivity in LLMs](https://arxiv.org/abs/2512.10545)
...
### [176] [RoleRMBench & RoleRM: Towards Reward Modeling for Profile-Based Role Play in Dialogue Systems](https://arxiv.org/abs/2512.10575)
...
### [177] [Causal Reasoning Favors Encoders: On The Limits of Decoder-Only Models](https://arxiv.org/abs/2512.10561)
...
### [178] [Textual Data Bias Detection and Mitigation - An Extensible Pipeline with Experimental Evaluation](https://arxiv.org/abs/2512.10734)
...
TL;DR: 本文提出了一套全面的数据偏见检测和缓解流程，用于解决大型语言模型训练数据中的代表性偏见和刻板印象。尽管该流程成功减少了数据中的偏见，但用去偏见数据微调的模型并未在偏见基准测试中始终表现出改进，这揭示了当前评估方法的不足。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的训练文本数据存在多方面偏见，包括有害语言和倾斜的人口统计分布。欧盟AI法案等法规要求识别和缓解数据中针对受保护群体的偏见，以防止不公平的模型输出。然而，实际操作指南和具体实施方法仍有欠缺。

Method: 本文提出了一个包含四个组件的综合数据偏见检测和缓解流程，针对配置敏感属性的两种数据偏见类型（代表性偏见和显性刻板印象）：
1.  利用基于质量标准生成的LLM词列表检测相关群体标签。
2.  使用人口代表性分数（Demographic Representation Score）量化代表性偏见。
3.  利用社会语言学知情过滤检测和缓解刻板印象。
4.  通过语法和上下文感知反事实数据增强（Grammar- and Context-Aware Counterfactual Data Augmentation）补偿代表性偏见。
研究通过性别、宗教和年龄的例子进行了双重评估：
1.  通过人工验证和基线比较，评估每个组件在数据去偏见方面的有效性。
2.  通过对在去偏见数据集上微调的多个模型（0.6B-8B参数）进行偏见基准测试，评估数据去偏见对模型偏见减少的影响。

Result: 研究发现，该流程成功减少了文本数据集中的代表性偏见和显性刻板印象。然而，在去偏见数据上微调的LLMs并未在偏见基准测试中始终显示出改进的性能。

Conclusion: 这项研究揭示了当前评估方法中的关键空白，并强调了需要有针对性的数据操作来解决已表现出的模型偏见，即使数据本身已经去偏见，也可能无法直接导致模型偏见的减少。
...
### [179] [AgriGPT-Omni: A Unified Speech-Vision-Text Framework for Multilingual Agricultural Intelligence](https://arxiv.org/abs/2512.10624)
...
### [180] [From Data Scarcity to Data Care: Reimagining Language Technologies for Serbian and other Low-Resource Languages](https://arxiv.org/abs/2512.10630)
...
### [181] [TRIDENT: A Redundant Architecture for Caribbean-Accented Emergency Speech Triage](https://arxiv.org/abs/2512.10741)
*Elroy Galbraith,Chadwick Sutherland,Donahue Morgan*

Main category: cs.CL

TL;DR: 本文提出TRIDENT系统，一个三层调度员支持架构，旨在通过结合针对加勒比口音优化的ASR、LLM实体提取和生物声学检测，改善紧急呼叫（特别是加勒比口音）的处理，即使自动语音识别失败也能辅助人类应用分诊协议。


<details>
  <summary>Details</summary>
Motivation: 现有的紧急语音识别系统在非标准英语变体（如加勒比口音）上表现不佳，导致加勒比地区人口在紧急服务中面临严重的服务空白。

Method: TRIDENT系统采用三层架构：1) 针对加勒比口音优化的自动语音识别（ASR），2) 通过大型语言模型（LLM）进行本地实体提取，3) 生物声学遇险检测。这些层共同为调度员提供转录置信度、结构化临床实体和声音压力指标三种互补信号。核心思想是低ASR置信度结合高声音遇险标志可作为呼叫优先级信号，同时，语义分析用于捕获冷静呼叫者可能遗漏的临床指标。

Result: 本文描述了TRIDENT的架构设计、基于心理语言学研究的理论基础（关于压力诱导的代码转换）以及灾难场景下离线部署的考量。该工作为实现口音弹性紧急AI建立了一个框架，旨在确保加勒比地区的声音能够公平地获得既定的国家分诊协议服务。

Conclusion: TRIDENT系统为加勒比口音的紧急呼叫提供了一个口音弹性的人工智能框架，确保这些声音能够公平地接入国家分诊协议。尽管经验验证是未来的工作，但该框架已为解决现有系统缺陷奠定了基础。
...
### [182] [Long-horizon Reasoning Agent for Olympiad-Level Mathematical Problem Solving](https://arxiv.org/abs/2512.10739)
...
TL;DR: 本文提出了一种名为OPV（Outcome-based Process Verifier）的新型验证器，通过验证长思维链中总结性结果的推理过程，实现了准确高效的验证和大规模标注。OPV采用迭代主动学习框架，结合专家标注和拒绝微调，显著提升了大型语言模型在复杂推理任务中的验证能力和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于结果的验证器无法检查长推理链中不可靠的中间步骤；而现有基于过程的验证器受限于高质量标注稀缺和高昂的人力成本，难以可靠地检测复杂长思维链中的错误。

Method: 本文提出了OPV（Outcome-based Process Verifier），通过验证长思维链中总结性结果的推理过程，以实现准确高效的验证并支持大规模标注。为增强OPV的能力，作者采用了一个迭代主动学习框架，结合专家标注，逐步提升验证能力。具体来说，在每次迭代中，对当前OPV最不确定的案例进行标注，然后通过拒绝微调（RFT）和可验证奖励强化学习（RLVR）训练新的OPV。

Result: OPV在自定义基准测试	extsc{	hisbench}上取得了新的最先进结果，F1分数达到83.1，优于Qwen3-Max-Preview（76.3）。它能有效检测合成数据集中的假阳性，并与专家评估高度一致。与策略模型协同工作时，OPV持续提升性能，例如将DeepSeek-R1-Distill-Qwen-32B在AIME2025上的准确率从55.2%提高到73.3%。

Conclusion: OPV提供了一种准确高效的验证方法，能够检查大型语言模型长推理链的总结性过程，克服了现有验证器的局限性。通过迭代主动学习，OPV在降低标注成本的同时，显著提升了验证能力，并在多项实验中展现出卓越的性能和广泛的适用性，实现了最先进的结果并有效提升了策略模型的表现。
...
### [183] [OPV: Outcome-based Process Verifier for Efficient Long Chain-of-Thought Verification](https://arxiv.org/abs/2512.10756)
...
### [184] [Grow Up and Merge: Scaling Strategies for Efficient Language Adaptation](https://arxiv.org/abs/2512.10772)
...
### [185] [Script Gap: Evaluating LLM Triage on Indian Languages in Native vs Roman Scripts in a Real World Setting](https://arxiv.org/abs/2512.10780)
...
### [186] [The FACTS Leaderboard: A Comprehensive Benchmark for Large Language Model Factuality](https://arxiv.org/abs/2512.10791)
...
### [187] [LabelFusion: Learning to Fuse LLMs and Transformer Classifiers for Robust Text Classification](https://arxiv.org/abs/2512.10793)
...
### [188] [Quantifying Emotional Tone in Tolkien's The Hobbit: Dialogue Sentiment Analysis with RegEx, NRC-VAD, and Python](https://arxiv.org/abs/2512.10865)
*Lilin Qiu*
...
### [189] [Computational emotion analysis with multimodal LLMs: Current evidence on an emerging methodological opportunity](https://arxiv.org/abs/2512.10882)
*Hauke Licht*
...
### [190] [Unsupervised Acquisition of Discrete Grammatical Categories](https://arxiv.org/abs/2503.18702)
...
### [191] [Openpi Comet: Competition Solution For 2025 BEHAVIOR Challenge](https://arxiv.org/abs/2512.10071)
...
TL;DR: 本文提出了一种针对2025 BEHAVIOR挑战赛的解决方案，该方案在BEHAVIOR-1K任务中取得了第二名，并通过系统研究训练技术和数据，展示了预训练和后训练阶段的扩展能力，并提供了实践经验。


<details>
  <summary>Details</summary>
Motivation: BEHAVIOR挑战赛旨在跟踪物理智能体在模拟环境中解决长周期任务的进展，特别是BEHAVIOR-1K专注于人们最希望机器人协助的日常家庭任务，以弥合当前研究与现实世界、以人为中心的应用程序之间的差距。

Method: 该解决方案以$π_{0.5}$为基础，通过系统研究训练技术和数据的影响来构建。通过仔细的消融实验，展示了预训练和后训练阶段在实现竞争性能方面的扩展能力。

Result: 该解决方案在2025 BEHAVIOR挑战赛中获得了非常接近的第二名，并显著优于其他提交方案。研究结果表明了预训练和后训练阶段的强大扩展能力。

Conclusion: 论文总结了实践经验和设计建议，希望能为更广泛的具身AI社区在将强大的基础模型应用于复杂具身场景时提供可操作的见解。
...
### [192] [Push Smarter, Not Harder: Hierarchical RL-Diffusion Policy for Efficient Nonprehensile Manipulation](https://arxiv.org/abs/2512.10099)
*Steven Caro,Stephen L. Smith*

Main category: cs.RO

TL;DR: 本文提出了一种名为HeRD的分层强化学习-扩散策略，用于非抓取式操作（如推物体），它将任务分解为高层目标选择和低层轨迹生成，在高层使用强化学习，在低层使用条件扩散模型，显著提高了推物任务的成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 非抓取式操作（例如在杂乱环境中推物体）是一个具有挑战性的控制问题，原因在于复杂的接触动力学和长周期的规划需求。

Method: 本文提出了HeRD（分层强化学习-扩散策略），它将推物任务分解为两个层次：高层目标选择和低层轨迹生成。高层使用强化学习（RL）代理选择中间空间目标，低层使用目标条件扩散模型生成可行且高效的轨迹以达到这些目标。这种架构结合了RL的长周期奖励最大化行为和扩散模型的生成能力。

Result: 在2D仿真环境中进行评估，HeRD在成功率、路径效率和跨多种环境配置的泛化能力方面均优于现有最先进的基线方法。

Conclusion: 研究结果表明，结合生成式低层规划的分层控制是实现可扩展、目标导向的非抓取式操作的一个有前景的方向。
...
### [193] [Fast Functionally Redundant Inverse Kinematics for Robotic Toolpath Optimisation in Manufacturing Tasks](https://arxiv.org/abs/2512.10116)
*Andrew Razjigaev,Hans Lohr,Alejandro Vargas-Uscategui,Peter King,Tirthankar Bandyopadhyay*

Main category: cs.RO

TL;DR: 本文提出了一种新颖的算法，利用任务空间分解、阻尼最小二乘法和Halley方法，快速鲁棒地解决功能冗余机械臂的逆运动学问题，以优化复杂轨迹规划并减少关节运动。


<details>
  <summary>Details</summary>
Motivation: 工业自动化中，六轴机械臂在焊接、增材制造等任务中存在功能冗余（等效为五轴任务），这限制了工作空间和灵活性。现有的离线规划方法计算成本高，而逆运动学算法虽快但未被充分利用，因此需要一种快速且鲁棒的方法来利用这种冗余，以实现工具路径规划的可行性和优化。

Method: 研究者提出了一种新颖的算法来解决功能冗余的逆运动学问题。该方法结合了任务空间分解（task space decomposition）方法、阻尼最小二乘法（damped least-squares method）以及Halley方法，旨在实现快速、鲁棒的解决方案并减少关节运动。

Result: 该功能冗余逆运动学算法能够快速求解运动规划，最大程度地减少关节运动，从而扩展了复杂工具路径的可行操作空间。该方法在一个工业ABB机械臂和冷喷枪上执行计算出的工具路径，并在非平面表面冷喷涂应用中的工具路径优化案例中得到了验证。

Conclusion: 该算法能够有效解决功能冗余机械臂的逆运动学问题，显著减少关节运动，扩展了复杂工具路径的可行操作范围，为工业自动化中的轨迹规划提供了快速、鲁棒且高效的解决方案。
...
### [194] [Lies We Can Trust: Quantifying Action Uncertainty with Inaccurate Stochastic Dynamics through Conformalized Nonholonomic Lie Groups](https://arxiv.org/abs/2512.10294)
*Luís Marques,Maani Ghaffari,Dmitry Berenson*

Main category: cs.RO

TL;DR: 本文提出了一种名为CLAPS的对称感知共形预测算法，它能为给定的动作构建一个预测集，该预测集以用户定义的概率保证包含系统最终配置。CLAPS在非欧几里得配置空间（如SE(2)）中提供可靠的概率保证，并通过考虑空间结构实现更高效的预测区域。


<details>
  <summary>Details</summary>
Motivation: 传统的机器人不确定性量化方法通常依赖于对误差分布或大小的强假设，或使用未校准的不确定性估计，这不足以实现安全控制。虽然共形预测提供了分布无关的概率保证，但现有方法将机器人视为欧几里得点，无法处理许多具有非欧几里得配置的系统（例如SE(2)机器人）。

Method: 本文提出了共形李群作用预测集（CLAPS），这是一种对称感知的共形预测算法。它利用李群（特别是SE(2)）对配置误差进行严格分析，将先前的欧几里得空间理论保证扩展到非欧几里得空间。通过考虑配置空间的结构，CLAPS设计了对称知情的非一致性分数来构建预测集。

Result: 在模拟JetBot和真实MBot上的实验表明，通过考虑配置空间的结构，CLAPS的对称知情非一致性分数能够生成体积更高效的预测区域。这些区域比现有方法更能准确地表示底层不确定性。

Conclusion: CLAPS为具有非欧几里得配置的系统提供了一种在任意用户定义概率下，保证包含未来系统配置的预测集。它在不确定性量化方面提供了分布无关的非渐近概率保证，并且通过利用配置空间的对称性，能够生成更紧凑、更准确的预测区域，从而更好地表示系统的不确定性。
...
### [195] [Inertial Magnetic SLAM Systems Using Low-Cost Sensors](https://arxiv.org/abs/2512.10128)
...
### [196] [Task-Oriented Grasping Using Reinforcement Learning with a Contextual Reward Machine](https://arxiv.org/abs/2512.10235)
...
### [197] [Design of a six wheel suspension and a three-axis linear actuation mechanism for a laser weeding robot](https://arxiv.org/abs/2512.10319)
...
### [198] [Design and Validation of an Under-actuated Robotic Finger with Synchronous Tendon Routing](https://arxiv.org/abs/2512.10349)
...
### [199] [CLASH: Collaborative Large-Small Hierarchical Framework for Continuous Vision-and-Language Navigation](https://arxiv.org/abs/2512.10360)
*Liuyi Wang,Zongtao He,Jinlong Li,Xiaoyan Qi,Mengxian Hu,Chenpeng Yao,Chengju Liu,Qijun Chen*

Main category: cs.RO

TL;DR: 本文提出CLASH框架，结合反应式小型规划器和反射式大型推理器，通过不确定性感知协作机制，在Vision-and-Language Navigation (VLN) 任务中实现了最先进的性能，并在模拟和真实世界部署中均表现出强大的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉-语言模型具有强大的推理能力，但在Vision-and-Language Navigation (VLN) 任务中，其表现往往不如任务专用的小型全景模型。研究旨在弥合这一差距，提升VLN任务的性能和泛化能力。

Method: 本文提出了CLASH（Collaborative Large-Small Hierarchy）框架，整合了：1) 反应式小型模型规划器（RSMP），采用基于因果学习的双分支架构以增强泛化能力；2) 反射式大型模型推理器（RLMR），利用全景视觉提示和思维链推理支持可解释的空间理解和导航。此外，引入了不确定性感知协作机制（UCM）自适应融合两个模型的决策。为实现避障，在模拟中替换为可学习的点目标策略，在真实世界部署中设计了基于LiDAR的聚类模块生成可导航路点，并结合在线SLAM局部控制器。

Result: CLASH在VLN-CE排行榜上取得了最先进的（第一名）结果，显著提升了测试集（未见）上的SR和SPL。真实世界实验也证明了CLASH强大的鲁棒性，验证了其在模拟和部署场景中的有效性。

Conclusion: CLASH框架通过有效结合大型模型的推理能力和小型模型的任务特异性规划能力，成功解决了VLN任务中大型模型表现不足的问题。其在模拟和真实世界中的优异表现，验证了该分层协作方法的有效性和鲁棒性。
...
### [200] [Symphony: A Heuristic Normalized Calibrated Advantage Actor and Critic Algorithm in application for Humanoid Robots](https://arxiv.org/abs/2512.10477)
*Timur Ishuov,Michele Folgheraiter,Madi Nurmanov,Goncalo Gordo,Richárd Farkas,József Dombi*

Main category: cs.RO

TL;DR: 本文提出Symphony算法，通过“襁褓式”正则化、有限参数噪声和Fading Replay Buffer等方法，旨在实现从零开始、样本高效且安全的类人机器人训练。


<details>
  <summary>Details</summary>
Motivation: 传统机器人学习从零开始时，往往需要大量训练步骤，效率低下且可能对机器人硬件和环境造成损害。作者认为人类学习并非快速，而是耗时且受限，机器人学习也应考虑效率和安全性。

Method: 本文提出了Symphony (Transitional-policy Deterministic Actor and Critic) 算法，结合了多种思想：1. “襁褓式”正则化：通过惩罚动作强度来抑制快速但不稳定的发展。2. 限制参数噪声：相比随机算法，使用有限参数噪声并降低动作强度，以安全地增加熵。3. Fading Replay Buffer：利用双曲正切公式调整批量采样概率，结合近期和长期记忆。4. Temporal Advantage：通过与指数移动平均比较改进评论家网络预测，实现Actor和Critic的一次性更新和损失合并。

Result: 通过上述方法，训练过程在经验上对环境和机器人机构都更加安全。Symphony算法旨在实现样本高效、样本接近性和动作安全性。

Conclusion: Symphony算法及其组成部分，如“襁褓式”正则化、有限噪声和Fading Replay Buffer，为从零开始训练类人机器人提供了一种更安全、更样本高效的解决方案。
...
### [201] [RoboNeuron: A Modular Framework Linking Foundation Models and ROS for Embodied AI](https://arxiv.org/abs/2512.10394)
*Weifan Guan,Huasen Xi,Chenxiao Zhang,Aosheng Li,Qinghao Hu,Jian Cheng*

Main category: cs.RO

TL;DR: RoboNeuron是一个通用的具身智能部署框架，它首次将大型语言模型（LLM）和视觉-语言-动作（VLA）模型的认知能力与机器人操作系统（ROS）的实时执行骨干深度集成，通过模型上下文协议（MCP）实现动态编排和高模块化，显著提升跨场景适应性和组件灵活性。


<details>
  <summary>Details</summary>
Motivation: 当前的具身AI系统面临严峻的工程障碍，主要表现为跨场景适应性差、模块间耦合僵硬以及推理加速碎片化。

Method: RoboNeuron通过以下方法实现目标：1) 深度整合LLM和VLA模型与ROS；2) 利用模型上下文协议（MCP）作为语义桥梁，使LLM能够动态编排底层机器人工具；3) 建立高度模块化的架构，通过ROS的统一通信接口严格解耦感知、推理和控制；4) 引入自动化工具，将ROS消息转换为可调用的MCP函数，简化开发。

Result: RoboNeuron显著增强了跨场景适应性和组件灵活性，并建立了一个系统化的平台，用于水平性能基准测试。

Conclusion: RoboNeuron为可扩展的真实世界具身应用奠定了坚实的基础。
...
### [202] [Seamless Outdoor-Indoor Pedestrian Positioning System with GNSS/UWB/IMU Fusion: A Comparison of EKF, FGO, and PF](https://arxiv.org/abs/2512.10480)
*Jiaqiang Zhang,Xianjia Yu,Sier Ha,Paola Torrico Moron,Sahar Salimpour,Farhad Kerama,Haizhou Zhang,Tomi Westerlund*

Main category: cs.RO

TL;DR: 本文提出一个统一的GNSS/UWB/IMU融合框架，用于实现室外-室内无缝行人定位，并比较了ESKF、因子图优化和粒子滤波三种后端，结果显示ESKF表现最稳定。


<details>
  <summary>Details</summary>
Motivation: GNSS、UWB和惯性PDR等现有定位技术在信号遮挡、多径效应和漂移等问题下各自脆弱，难以在室外-室内环境中提供准确且连续的行人定位。

Method: 该研究提出了一个统一的GNSS/UWB/IMU融合框架，以胸部佩戴的IMU驱动的PDR作为运动骨干，并整合室外GNSS和室内UWB的绝对更新。框架比较了误差状态扩展卡尔曼滤波器(ESKF)、滑动窗口因子图优化和粒子滤波器三种概率后端。为增强过渡鲁棒性并缓解城市GNSS降级，引入了基于OpenStreetMap建筑轮廓的轻量级地图可行性约束，将大多数建筑内部视为不可导航，同时允许在指定UWB仪器化建筑内移动。系统在ROS 2中实现，可在可穿戴平台上实时运行。

Result: 在室内(UWB+PDR)、室外(GNSS+PDR)和无缝室外-室内(GNSS+UWB+PDR)三种场景下进行了评估。结果表明，在作者的实现中，ESKF提供了最稳定的一致性整体性能。

Conclusion: 该研究成功构建了一个用于无缝室外-室内行人定位的GNSS/UWB/IMU融合框架，通过引入地图约束增强了鲁棒性，并发现ESKF在所实现方案中表现出最佳的综合性能和一致性。
...
### [203] [Contact SLAM: An Active Tactile Exploration Policy Based on Physical Reasoning Utilized in Robotic Fine Blind Manipulation Tasks](https://arxiv.org/abs/2512.10481)
*Gaozhao Wang,Xing Liu,Zhenduo Ye,Zhengxiong Liu,Panfeng Huang*

Main category: cs.RO

TL;DR: 本文提出了一种名为“接触SLAM”的物理驱动接触认知方法，使机器人在视觉受阻的“盲操作”场景下，仅依靠触觉传感和先验知识进行环境状态估计和操作，并通过主动探索策略提高效率。


<details>
  <summary>Details</summary>
Motivation: 机器人进行接触密集型操作时需要精确的环境感知，但某些情况下视觉会被遮挡，导致机器人无法通过视觉反馈获取实时场景状态，即“盲操作”问题。

Method: 提出“接触SLAM”方法，这是一种物理驱动的接触认知方法，仅利用触觉传感和场景先验知识来估计环境状态并实现操作。同时设计了一种主动探索策略，以最大化探索效率并逐步降低操作场景中的不确定性。

Result: 实验结果证明了所提出方法在多个接触密集型任务中的有效性和准确性，包括复杂的插座装配任务和推块任务。

Conclusion: 所提出的“接触SLAM”方法结合主动探索策略，能够有效且准确地解决机器人在视觉受阻情况下的盲操作问题，实现基于触觉传感的环境认知和操作。
...
### [204] [Mr. Virgil: Learning Multi-robot Visual-range Relative Localization](https://arxiv.org/abs/2512.10540)
*Si Wang,Zhehan Li,Jiadong Lu,Rong Xiong,Yanjun Cao,Yue Wang*

Main category: cs.RO

TL;DR: 本文提出Mr. Virgil，一个端到端学习的多机器人视觉-测距相对定位框架，通过图神经网络解决UWB测距与视觉检测之间的数据关联难题，并结合可微分位姿图优化，实现多机器人定位的鲁棒性和高精度。


<details>
  <summary>Details</summary>
Motivation: 现有的超宽带（UWB）-视觉融合定位方法在多智能体相对定位中面临机器人与视觉检测之间的匹配难题。这导致系统高度依赖身份编码硬件或复杂的调优算法，且错误的匹配可能对定位系统造成不可逆的损害。

Method: 引入Mr. Virgil框架，包含一个基于图神经网络（GNN）的前端，用于UWB测距和视觉检测之间的数据关联，以及一个可微分的位姿图优化（PGO）后端。GNN前端提供鲁棒的匹配结果、准确的初始位置预测和可信的不确定性估计，这些信息随后被集成到PGO后端以提升最终位姿估计的精度。此外，系统还实现了去中心化部署。

Result: 在不同机器人数量、模拟和真实世界、遮挡和非遮挡条件下的实验表明，与传统方法相比，Mr. Virgil在各种场景下都展现出卓越的稳定性和精确性。

Conclusion: Mr. Virgil通过端到端学习方法，有效解决了多机器人视觉-测距融合定位中的数据关联挑战，提供了一个鲁棒且高精度的多机器人相对定位解决方案。
...
### [205] [Neural Ranging Inertial Odometry](https://arxiv.org/abs/2512.10531)
*Si Wang,Bingqi Shen,Fei Wang,Yanjun Cao,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: 本文提出IR-ULSG，一个用于测距惯性里程计的新型神经融合框架，结合图注意力UWB网络和循环神经惯性网络，通过学习场景相关模式并集成最小二乘法，显著提高了UWB在无GPS和复杂环境（如隧道）下的定位精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 超宽带（UWB）在无GPS定位中具有轻量化和无漂移的潜力，但其精度受传感器布置以及多径或多信号干扰引起的非高斯模式限制，这在长隧道等典型应用中尤为常见。

Method: 本文引入了一种新颖的神经融合框架，用于测距惯性里程计。该框架包含一个图注意力UWB网络（学习场景相关测距模式，适应任意数量的锚点或标签，无需校准即可实现精确定位）和一个循环神经惯性网络。此外，通过集成最小二乘法和引入名义帧（nominal frame）来增强整体性能和可扩展性。

Result: 通过在公共和自收集数据集（涵盖室内、室外和隧道环境）上进行的广泛实验，验证了所提方法的有效性和鲁棒性。结果表明，IR-ULSG在处理挑战性条件（包括凸包外部场景和仅有一个锚点的情况）时表现出优越性。

Conclusion: 所提出的IR-ULSG方法通过其神经融合框架，成功解决了UWB定位在复杂和GPS受限环境下的精度限制问题，展现出卓越的性能和鲁棒性，尤其是在传统方法受限的挑战性场景中。
...
### [206] [Motion Planning for Safe Landing of a Human-Piloted Parafoil](https://arxiv.org/abs/2512.10595)
*Maximillian Fainkich,Kiril Solovey,Anna Clarke*

Main category: cs.RO

TL;DR: 本文研究了跳伞伞翼飞行和着陆阶段的安全轨迹计算问题，通过改进采样式运动规划器SST，生成比人类飞行员更安全、更平滑的轨迹，以期辅助飞行员训练。


<details>
  <summary>Details</summary>
Motivation: 跳伞事故多发生于伞翼操纵和着陆阶段，主要源于人为判断失误。目前缺乏功能性强且易于获取的训练模拟器，且适用于辅助人类训练的伞翼轨迹规划研究有限，导致新手飞行员训练周期长。

Method: 研究者将采样式运动规划器Stable Sparse RRT (SST) 进行了改进，以适应伞翼飞行的约束条件，并以最小化坡度角（控制力）作为安全代理。然后，将算法生成的轨迹与人类飞行员的飞行数据进行比较。

Result: 算法生成的解决方案在成本上比人类飞行员的表现提高了20%-80%。人类飞行员倾向于先缩短与着陆区的水平距离，然后通过盘旋下降来解决垂直高度差；而算法则能实现更平稳、更渐进的下降，在保持安全约束的同时，以精确的高度到达着陆区进行最终进近。

Conclusion: 研究表明，计算机生成的飞行指南（而非传统的经验法则）具有巨大潜力，可集成到未来的模拟器中，用于训练飞行员进行更安全、更经济高效的飞行。
...
### [207] [LEO-RobotAgent: A General-purpose Robotic Agent for Language-driven Embodied Operator](https://arxiv.org/abs/2512.10605)
...
### [208] [Evaluating Gemini Robotics Policies in a Veo World Simulator](https://arxiv.org/abs/2512.10675)
...
### [209] [How to Brake? Ethical Emergency Braking with Deep Reinforcement Learning](https://arxiv.org/abs/2512.10698)
...
### [210] [On the Stabilization of Rigid Formations on Regular Curves](https://arxiv.org/abs/2512.10700)
*Mohamed Elobaid,Shinkyu Park,Eric Feron*

Main category: cs.RO

TL;DR: 本文提出了一种通过随机牛顿类算法和连续反馈控制律，将多智能体等边多边形编队稳定在平面曲线上的方法，同时确保智能体间的避碰。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决多智能体刚性编队（特别是等边多边形编队）在一般平面曲线（尤其是在路径扫描后）上的稳定问题。

Method: 首先，通过一种随机多启动的牛顿类算法（能够确定最小值的存在性）来找到一个以兴趣点为中心的外接正多边形。然后，设计了一个连续反馈控制律。

Result: 所提出的反馈控制律能够保证：1) 收敛到曲线；2) 充分扫描曲线；3) 收敛到期望的编队顶点；4) 确保智能体间的避碰。该方法通过对不同类型曲线和刚性编队的数值模拟得到了验证。

Conclusion: 研究成功提出并验证了一种在平面曲线上稳定多智能体刚性编队的有效方法，该方法结合了优化算法和连续反馈控制，并能确保编队稳定性和避碰。
...
### [211] [Curriculum-Based Reinforcement Learning for Autonomous UAV Navigation in Unknown Curved Tubular Conduit](https://arxiv.org/abs/2512.10934)
*Zamirddine Mari,Jérôme Pasquet,Julien Seinturier*

Main category: cs.RO

TL;DR: 本文提出一种基于强化学习的方法，使无人机仅凭局部激光雷达和视觉观测，在未知三维管状环境中自主导航，克服了几何约束和感知限制，表现优于依赖完整几何模型的确定性控制器。


<details>
  <summary>Details</summary>
Motivation: 在受限管状环境中，由于管道几何形状的约束、壁面接近以及固有的感知限制，无人机自主导航仍是一个重大挑战。

Method: 采用强化学习（PPO策略），结合激光雷达局部观测和管中心条件视觉检测。通过渐进式课程学习策略进行训练，逐步暴露于更复杂的弯曲几何体。引入基于直接可见性、方向记忆和激光雷达对称线索的转弯协商机制，以应对部分可观测性。与获取完整中心线的Pure Pursuit算法作为确定性基线进行对比。

Result: PPO策略习得了鲁棒且可泛化的行为，尽管几何信息有限，但始终优于确定性控制器。在高保真3D环境中的验证进一步证实了学习行为对连续物理动力学的可迁移性。

Conclusion: 所提出的方法为未知管状环境中的自主导航提供了一个完整的框架，为工业、地下或医疗应用等在狭窄且感知能力弱的管道中前进的核心挑战开辟了前景。
...
### [212] [AERMANI-Diffusion: Regime-Conditioned Diffusion for Dynamics Learning in Aerial Manipulators](https://arxiv.org/abs/2512.10773)
...
### [213] [Iterative Compositional Data Generation for Robot Control](https://arxiv.org/abs/2512.10891)
*Anh-Quan Pham,Marcel Hussing,Shubhankar P. Patankar,Dani S. Bassett,Jorge Mendez-Mendez,Eric Eaton*

Main category: cs.RO

TL;DR: 本文提出了一种语义组合扩散变换器，通过分解机器人操作任务的转换并利用注意力机制学习其交互，从而实现对未知任务组合的零样本高质量数据生成，并通过迭代自改进过程进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 收集机器人操作数据成本高昂，尤其是在多对象、多机器人、多环境设置中，任务组合空间巨大。现有生成模型未能利用机器人领域的组合结构，难以泛化到未见的任务组合。

Method: 提出了一种语义组合扩散变换器（semantic compositional diffusion transformer），将转换分解为机器人、对象、障碍物和目标特定组件，并通过注意力机制学习它们之间的交互。模型在有限任务子集上训练后，能零样本生成高质量转换，进而学习控制策略。此外，引入了迭代自改进过程，通过离线强化学习验证合成数据，并将其纳入后续训练轮次。

Result: 该模型能够零样本生成高质量转换，从而学习未见任务组合的控制策略。与整体式和硬编码组合基线相比，零样本性能显著提高，最终解决了几乎所有保留任务。同时，学习到的表示中出现了有意义的组合结构。

Conclusion: 该方法通过语义组合扩散变换器和迭代自改进，有效解决了机器人操作数据收集成本高昂和泛化性差的问题。它在零样本设置下表现出色，能够处理复杂的未见任务组合，并展示了学习表示中组合结构的涌现。
...
### [214] [ImplicitRDP: An End-to-End Visual-Force Diffusion Policy with Structural Slow-Fast Learning](https://arxiv.org/abs/2512.10946)
...
### [215] [QSTAformer: A Quantum-Enhanced Transformer for Robust Short-Term Voltage Stability Assessment against Adversarial Attacks](https://arxiv.org/abs/2512.09936)
*Yang Li,Chong Ma,Yuanzheng Li,Sen Li,Yanbo Chen,Zhaoyang Dong*

Main category: eess.SY

TL;DR: 本文提出了一种量子增强型Transformer架构QSTAformer，通过将参数化量子电路嵌入注意力机制，并结合对抗训练策略，实现了对短期电压稳定性评估（STVSA）的鲁棒且高效的预测，尤其擅长抵御对抗性攻击。


<details>
  <summary>Details</summary>
Motivation: 经典的机器学习方法在短期电压稳定性评估（STVSA）中表现良好，但在对抗性条件下其鲁棒性仍面临挑战。现有研究尚未系统地探讨基于量子机器学习的STVSA的对抗性脆弱性。

Method: 本文提出了QSTAformer，一个定制的量子增强型Transformer架构，它将参数化量子电路（PQCs）嵌入到注意力机制中。同时，开发了一种专门的对抗训练策略来防御白盒和灰盒攻击。此外，还对多种PQC架构进行了基准测试，以探索表达能力、收敛性和效率之间的权衡。

Result: 在IEEE 39节点系统上的案例研究表明，QSTAformer实现了具有竞争力的准确性、降低的复杂性以及更强的鲁棒性。这是首次系统性研究基于量子机器学习的STVSA的对抗性脆弱性。

Conclusion: QSTAformer在对抗性条件下，为安全和可扩展的短期电压稳定性评估展现出巨大潜力。
...
### [216] [Explicit Control Barrier Function-based Safety Filters and their Resource-Aware Computation](https://arxiv.org/abs/2512.10118)
*Pol Mestres,Shima Sadat Mousavi,Pio Ong,Lizhi Yang,Ersin Das,Joel W. Burdick,Aaron D. Ames*

Main category: eess.SY

TL;DR: 本文提出了一种基于控制障碍函数（CBF）的安全滤波器的闭式表达式，通过状态空间分区，显著提高了计算效率，适用于高频控制场景。


<details>
  <summary>Details</summary>
Motivation: CBF安全滤波器通常通过实时求解二次规划（QP）实现，但对于需要高效、高频计算控制动作的应用来说，使用现成的求解器面临挑战。

Method: 本文通过将状态空间划分为不同区域，并在每个区域内推导出一个不同的闭式解，从而为CBF安全滤波器得到的控制器引入了一个闭式表达式。在此基础上，提出了一种资源感知实现方法，该方法检测分区区域的变化，并在变化之间使用相应的闭式表达式。

Result: 研究引入了CBF安全滤波器控制器的闭式表达式，并基于此提出了一种资源感知实现方法，解决了传统QP求解的计算效率问题。

Conclusion: 所提出的方法为CBF安全滤波器提供了一种高效的实现方式，并通过航空航天控制和安全强化学习等示例展示了其广泛适用性。
...
### [217] [Traffic Equilibrium in Mixed-Autonomy Network with Capped Customer Waiting](https://arxiv.org/abs/2512.10194)
*Jiaxin Hou,Kexin Wang,Ruolin Li,Jong-shi Pang*

Main category: eess.SY

TL;DR: 本文提出一个统一建模框架，用于捕捉混合自动驾驶交通网络中网约车公司、出行者和交通流之间的均衡态互动，并通过数值实验为政策制定者提供可操作的见解。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了理解自动驾驶技术进步和信息可及性快速变化背景下，现代交通系统中自动驾驶车辆（AV）的采用以及网约车公司车辆偏离行为对公司、出行者和系统性能的影响，并为政策制定者提供监管依据。

Method: 该框架整合了四个相互关联的子模块：(i) 代表性混合车队网约车公司（MiFleet TNCs）对自动驾驶车辆（AV）和人工驾驶车辆（HV）车队的操作行为；(ii) 考虑出行成本和等待时间的出行者模式选择决策；(iii) 设定客户等待时间上限，以反映出行者在超出耐心后选择其他出行方式的选项；(iv) 基于流量的交通拥堵模型来计算出行时间。模型关键特征在于区分了AV和HV在接载和服务（载客）阶段的行为。整个框架被公式化为一个非线性互补问题（NCP），等价于一个变分不等式（VI）公式，并基于此建立了交通模型的变分均衡解的存在性。

Result: 数值实验结果表明，自动驾驶车辆渗透率和Wardrop松弛因子（限制路线偏离的因素）在不同程度上影响公司、出行者和系统性能。

Conclusion: 研究结果为政策制定者提供了关于在现代交通系统中监管自动驾驶车辆采用和公司车辆偏离行为的可操作见解。
...
### [218] [Permutation-Equivariant Learning for Dynamic Security Assessment of Power System Frequency Response](https://arxiv.org/abs/2512.10232)
*Francisco Zelaya-Arrazabal,Sebastian Martinez-Lizana,Hector Pulgar-Painemal,Jin Zhao*

Main category: eess.SY

TL;DR: 本文提出了一种混合模型-AI框架，用于电力系统频率稳定性的实时动态安全评估，通过结合模态分析和深度学习，快速准确预测关键频率参数。


<details>
  <summary>Details</summary>
Motivation: 需要在不断变化的运行条件和扰动下，对电力系统频率稳定性进行快速、准确的实时动态安全评估，同时避免对大量训练数据或广域测量单元（PMU）的广泛依赖。

Method: 该框架基于系统频率响应（SFR）的模态公式，利用系统的特征结构预测频率稳定性指标。采用受Deep Sets启发的神经网络来估计模态系数，将其表述为置换等变学习问题。通过重用预计算的模态结构并仅更新扰动特定系数，实现可扩展性。

Result: 该方法能够快速准确地预测频率最低点及其发生时间，在不同运行条件和扰动下表现出强大的泛化能力，无需大量训练场景或广泛部署PMU。在IEEE 39节点和118节点系统上验证，与纯数据驱动方法相比，显示出卓越的准确性、鲁棒性和计算效率。

Conclusion: 所提出的混合模型-AI框架为电力系统频率稳定性的实时动态安全评估提供了一种高精度、鲁棒且计算高效的解决方案，优于纯数据驱动方法，并具有良好的泛化能力和可扩展性。
...
### [219] [A Novel Phase-Noise Module for the QUCS Circuit Simulator. Part I : the Periodic Steady-State](https://arxiv.org/abs/2512.10373)
...
### [220] [Structural Sign Herdability in Temporally Switching Networks with Fixed Topology](https://arxiv.org/abs/2512.10374)
...
### [221] [Collision-Aware Density-Driven Control of Multi-Agent Systems via Control Barrier Functions](https://arxiv.org/abs/2512.10392)
...
### [222] [Structural Methods for handling mode changes in multimode DAE systems](https://arxiv.org/abs/2512.10580)
...
### [223] [NWP-based Atmospheric Refractivity Modeling and Fast & Stable Non-uniform Plane Wave Ray-Tracing Simulations for LEO Link Analysis](https://arxiv.org/abs/2512.10598)
...
### [224] [Codeshare agreements between airlines: literature review with the aid of artificial intelligence](https://arxiv.org/abs/2512.10639)
*Lucas T. B. Mendes,Alessandro V. M. Oliveira*

Main category: eess.SY

TL;DR: 本文回顾了代码共享协议的文献，重点关注巴西市场，并利用Litmaps工具分析其演变和影响，以识别现有证据、巴西市场特点及未来研究空白。


<details>
  <summary>Details</summary>
Motivation: 代码共享协议在航空业广泛存在，但其影响存在争议，既有促进竞争、扩大网络的积极作用，也有潜在的反竞争影响。鉴于巴西市场独特的特点和监管历史，有必要对其进行专门的文献综述。

Method: 采用文献综述方法，特别关注巴西市场，并利用基于人工智能技术的Litmaps计算工具，通过引文关系支持出版物的背景分析。

Result: 通过对文献的联合分析，旨在识别和评估几十年来关于代码共享协议在巴西影响的主要证据，描绘当前知识现状，描述巴西市场的具体特点，并识别可能指导未来研究的空白。

Conclusion: 该文献综述将勾勒出当前知识状态，描述巴西市场的特殊性，并确定未来研究的空白。
...
### [225] [Estimating Hormone Concentrations in the Pituitary-Thyroid Feedback Loop from Irregularly Sampled Measurements](https://arxiv.org/abs/2512.10657)
*Seth Siriya,Tobias M. Wolff,Victor G. Lopez,Matthias A. Müller*

Main category: eess.SY

TL;DR: 本文针对甲状腺疾病药物推荐中模型控制对不可测量内部激素浓度的依赖性，以及测量数据采样不规律的问题，验证了基于采样的可检测性，并实现了基于采样的移动视界估计器。研究表明该估计器具有鲁棒稳定性，且更频繁的采样能减少估计误差。


<details>
  <summary>Details</summary>
Motivation: 甲状腺疾病药物推荐中的模型控制技术常依赖于血液样本无法测量的内部激素浓度，且可测量的浓度通常只能在不规则的采样时间获得。

Method: 1. 在代表甲状腺功能减退和亢进患者的两个垂体-甲状腺环路模型上，经验性地验证了考虑不规则采样的基于采样的可检测性，并将内部浓度作为状态。2. 为这些模型实现了基于采样的移动视界估计（MHE）。3. 在一系列采样方案下，对虚拟患者测试了估计器的性能。

Result: 研究显示，在所有场景下，估计器都表现出鲁棒的稳定性。在存在模型不确定性和剂量报告错误的情况下，更频繁的采样能导致更小的估计误差。

Conclusion: 基于采样的移动视界估计器能够稳健地估计甲状腺模型中的内部激素浓度，并且通过增加采样频率可以提高估计精度，有效解决了不可测量状态和不规则数据带来的挑战。
...
### [226] [Active prognosis and diagnosis of modular discrete-event systems](https://arxiv.org/abs/2512.10684)
*Shaopeng Hu,Shaowen Miao,Jan Komenda,Zhiwu Li*

Main category: eess.SY

TL;DR: 本文提出了一种验证和强制离散事件系统 (DES) 可预测性和可诊断性的方法，通过建立与“预正态性”的等价关系，并设计算法计算最大子语言，尤其适用于模块化DES的局部监督器。


<details>
  <summary>Details</summary>
Motivation: 可预测性和可诊断性是系统的全局属性，传统的纯局部监督器不足以解决这个问题。目前在通过局部监督器强制实现模块化DES的这些属性方面研究有限。

Method: 本文将可预测性（或可诊断性）与非故障语言（或故障语言后缀）子集上的“预正态性”等价起来。然后证明了最大可预测/可诊断/正态子语言的存在性，并设计了一种算法来计算最大可控、正态和可预测/可诊断子语言。对于模块化DES，利用改进的“预正态性”计算局部子系统的模块化监督器。

Result: 研究结果表明，可预测性（或可诊断性）与特定语言子集上的“预正态性”等价。本文证明了最大可预测/可诊断/正态子语言的存在，并提供了一种计算这些最大子语言的算法。通过所提出的方法，得到的闭环系统被证明是全局可控、正态且可预测/可诊断的。

Conclusion: 本文成功地提出了验证和强制离散事件系统可预测性和可诊断性的方法，并通过利用改进的“预正态性”为模块化DES计算局部监督器，确保了闭环系统在全局范围内的可控性、正态性以及可预测/可诊断性。
...
### [227] [Low-Order $\mathcal{H}_2 / \mathcal{H}_\infty$ Controller Design for Aeroelastic Vibration Suppression](https://arxiv.org/abs/2512.10841)
...
### [228] [Distribution-Free Stochastic MPC for Joint-in-Time Chance-Constrained Linear Systems](https://arxiv.org/abs/2512.10738)
...
### [229] [Active Optics for Hyperspectral Imaging of Reflective Agricultural Leaf Sensors](https://arxiv.org/abs/2512.10213)
*Dexter Burns,Sanjeev Koppal*

Main category: eess.IV

TL;DR: 该论文提出了一种集成、自适应且可扩展的系统，利用激光雷达、液态镜头、单色相机、滤光轮和快速转向镜等低成本光学组件，自主检测和询问植物传感器，以实现实时植物健康监测。


<details>
  <summary>Details</summary>
Motivation: 在复杂的农业环境中，有效定位和采样叶片上安装的传感器以获取实时生理数据是一个重大挑战。

Method: 该系统首先使用激光雷达识别田野中传感器的独特反射特征，然后通过快速转向镜动态重定向相机的视场以瞄准每个传感器进行高光谱成像。液态镜头持续调整焦距以保持不同深度下的图像清晰度，从而实现精确的光谱测量。该系统整合了多种低成本光学组件并进行协调工作。

Result: 在受控室内实验中验证了该系统，证明能够准确检测和跟踪反射性植物传感器，并成功获取其光谱数据。据作者所知，目前没有其他系统集成这些传感和光学模式用于农业监测。

Conclusion: 这项工作为自适应、低成本和自动化植物传感器询问奠定了基础，是迈向规模化、实时精准农业植物健康监测的重要一步。
...
### [230] [Fast and Robust LRSD-based SAR/ISAR Imaging and Decomposition](https://arxiv.org/abs/2512.10740)
*Hamid Reza Hashempour,Majid Moradikia,Hamed Bastami,Ahmed Abdelhadi,Mojtaba Soltanalian*

Main category: eess.IV

TL;DR: 本文提出了一种快速统一的低秩稀疏分解（LRSD）合成孔径雷达（SAR）成像框架，能够有效处理平台残余相位误差（PRPE）并显著降低计算成本，同时将其扩展到逆合成孔径雷达（ISAR）自动聚焦和成像。


<details>
  <summary>Details</summary>
Motivation: 现有低秩稀疏分解驱动的SAR成像方法在面对机载平台不稳定性引起的平台残余相位误差（PRPE）时性能不佳。此外，这些方法仅关注图像质量提升，忽视了遥感应用中实时处理对计算效率的要求。

Method: 本文提出了一种快速统一的联合SAR成像框架，通过鲁棒的LRSD分解和增强图像背景中的主导稀疏目标和低秩特征。该算法避免了计算大型矩阵的逆以进行图像形成，并利用受约束二次规划的最新进展来处理PRPE导致的单模约束。此外，该方法被扩展到ISAR自动聚焦和成像，主要用于稀疏ISAR图像的恢复。

Result: 通过合成数据和真实数据进行的实验表明，与现有最先进方法相比，所提出的方法在成像质量和计算成本方面均表现出卓越的性能。

Conclusion: 该研究成功开发了一种能够应对平台残余相位误差并大幅提升计算效率的统一SAR成像框架，并将其有效应用于ISAR成像，验证了其在成像质量和计算效率方面的优越性。
...
### [231] [Hyperspectral Image Data Reduction for Endmember Extraction](https://arxiv.org/abs/2512.10506)
*Tomohiko Mizutani*

Main category: eess.IV

TL;DR: 本文提出了一种数据降维方法，以显著降低高光谱图像自字典端元提取的计算成本，同时保持提取精度。


<details>
  <summary>Details</summary>
Motivation: 自字典方法在高光谱图像端元提取中表现出高精度，但其高计算成本限制了在大规模图像上的应用。降低计算量是一个主要挑战。

Method: 基于高光谱图像遵循纯像素线性混合模型，开发了一种数据降维技术，用于移除不包含端元的像素。理论分析表明该技术能保留接近端元的像素。在此基础上，提出了一种数据降维自字典方法，将该降维技术与基于线性规划的自字典方法相结合。

Result: 数值实验表明，所提出的方法能够大幅减少原始自字典方法的计算时间，且不牺牲端元提取精度。

Conclusion: 该数据降维方法有效解决了自字典端元提取的计算成本问题，同时保持了高精度，提高了其在大规模高光谱图像中的适用性。
