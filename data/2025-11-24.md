...
TL;DR: 本研究比较了人类（艺术家和非艺术家）与AI（两种提示条件）在图像生成中的视觉创造力。结果显示，人类的创造力显著高于AI，其中艺术家最高。增加人类指导能提升AI的创造力，但人类和AI评估者的判断模式差异很大，表明视觉创造力对AI来说仍是独特挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管最近研究表明大型语言模型在发散性思维任务中能与人类创造力匹敌，但视觉创造力领域仍未被充分探索。本研究旨在填补这一空白，比较人类和AI在视觉创造力方面的表现。

Method: 研究比较了人类参与者（视觉艺术家和非艺术家）以及图像生成AI模型（两种提示条件：高人类输入的“人类启发”和低人类输入的“自我引导”）生成的图像。由255名人类评估者和GPT4o共同评估了这些图像的创造力。

Result: 研究发现一个清晰的创造力梯度：视觉艺术家最具创造力，其次是非艺术家，然后是人类启发式生成AI，最后是自我引导式生成AI。增加人类指导显著提高了生成AI的创造性输出，使其接近非艺术家的水平。值得注意的是，人类和AI评估者在创造力判断模式上表现出巨大差异。

Conclusion: 与以语言为中心的任务不同，生成式AI模型在视觉领域可能面临独特的挑战，因为视觉创造力依赖于感知细微差别和情境敏感性，这些是人类特有的能力，可能无法轻易从语言模型中转移过来。
...
TL;DR: Cognitive BASIC 是一种受BASIC启发的、用于大型语言模型（LLM）的极简提示语言和模型内解释器，旨在将LLM的推理结构化为明确的、逐步的执行轨迹。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为LLM提供一个可解释的认知控制层，通过模拟复古BASIC的简洁性，实现模型内部透明的多步推理。

Method: 该方法设计了Cognitive BASIC语言，利用编号行和简单命令作为解释层。一个自然语言解释器文件定义了命令语义、内存更新和日志行为。一个“心理模型解释器”负责提取知识、检测矛盾并进行解决。研究在知识提取、冲突检测和推理任务的基准上，对三种LLM进行了比较评估。

Result: 结果显示，所有测试的LLM都能够执行Cognitive BASIC程序，并且在各项任务上表现出总体强大但不一致的性能。

Conclusion: Cognitive BASIC提供了一种有效的方法，通过结构化的、逐步的执行轨迹，增强了LLM推理的透明度和可控性，尽管不同模型之间的性能存在差异。
...
TL;DR: 本文提出一个系统性基准修订框架，通过统计分析模型响应模式来标记潜在无效问题，以供专家审查，并结合LLM评判进行初步筛选，有效提高基准可靠性。


<details>
  <summary>Details</summary>
Motivation: 基准测试中的无效问题严重影响AI评估的可靠性，而手动识别和纠正大量问题既不可行，也是一个关键瓶颈。

Method: 该方法基于模型平均分数足以概括性能的假设（即测量实验具有一维潜在结构）。它通过统计分析模型响应模式，计算每个问题的统计量预期范围。当经验估计值超出此范围时，该问题更有可能存在问题。此外，引入LLM评判进行初步审查，进一步减少人工工作量。

Result: 在九个广泛使用的基准测试中，该方法能以高达84%的精度指导专家审查识别问题。结合LLM评判的初步筛选，进一步减少了人工投入。

Conclusion: 该框架提供了一个高效且可扩展的系统性基准修订方法，以提高AI基准的可靠性。
...
### [6] [Budget-Aware Tool-Use Enables Effective Agent Scaling](https://arxiv.org/abs/2511.17006)
...
TL;DR: 本文研究了在工具增强型LLM代理中，如何在明确的工具调用预算下有效扩展其性能。通过引入预算跟踪器和BATS框架，代理获得了预算感知能力，显著改善了成本-性能扩展曲线。


<details>
  <summary>Details</summary>
Motivation: LLM代理在扩展计算时表现提升，但简单地增加工具调用预算并不能有效提高性能，因为代理缺乏“预算感知”并很快达到性能上限。研究旨在解决如何在工具调用预算限制下有效扩展这些代理。

Method: 1. 引入Budget Tracker（预算跟踪器）：一个轻量级插件，为代理提供持续的预算感知能力。2. 开发BATS (Budget Aware Test-time Scaling) 框架：利用预算感知能力动态调整规划和验证策略，决定是“深入挖掘”有前景的路径还是“转向”新路径。3. 形式化统一成本指标：同时考虑token和工具消耗，以受控方式分析成本-性能扩展。

Result: 预算感知方法产生了更有利的扩展曲线，并推动了成本-性能帕累托前沿。这是对预算受限代理的首次系统研究，表明预算感知能力对于工具增强型代理的有效扩展至关重要。

Conclusion: 本研究为更透明和有原则地理解工具增强型代理的扩展性提供了实证见解，证明了预算感知在提高代理性能和效率方面的重要性。
...
### [7] [Patient-level Information Extraction by Consistent Integration of Textual and Tabular Evidence with Bayesian Networks](https://arxiv.org/abs/2511.17056)
*Paloma Rabaey,Adrick Tench,Stefan Heytens,Thomas Demeester*

Main category: cs.AI

TL;DR: 本文提出了一种多模态患者信息提取方法，结合电子健康记录中的表格数据（贝叶斯网络）和临床文本（神经网络分类器），并通过引入一致性节点进行可解释的概率融合，以提高预测校准并处理数据不一致性。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录（EHRs）是训练临床决策支持系统的宝贵资源，但在高风险应用中，需要大型、结构化的表格数据集和透明的基于特征的模型。EHRs中大部分信息存在于非结构化文本中，需要一种方法来充分利用这些多模态信息。

Method: 该方法结合使用专家指导的贝叶斯网络处理患者EHR中的表格特征，以及神经网络文本分类器处理描述患者症状的临床笔记。通过使用虚拟证据并辅以一致性节点，实现了模型预测的可解释、概率性融合。一致性节点旨在提高最终预测的校准，使贝叶斯网络能更好地调整神经网络分类器的输出，以处理缺失信息并解决表格数据和文本数据之间的矛盾。

Result: 研究结果表明，与单独使用虚拟证据相比，引入一致性节点显著改善了最终预测的校准性。这使得贝叶斯网络能更好地调整神经网络分类器的输出，有效处理缺失信息，并解决表格和文本数据之间的冲突。该方法在SimSUM数据集（一个通过专家知识链接表格EHR和临床笔记的模拟基准数据集）上展现了其潜力。

Conclusion: 所提出的多模态信息提取方法能够有效地融合EHR中的结构化和非结构化数据。通过引入一致性节点，该方法不仅提供了可解释的概率融合，还显著提高了预测的校准性，并能有效处理数据缺失和矛盾，为构建透明的临床决策支持系统奠定了基础。
...
TL;DR: 本文重新解释了分数扩散模型在逆问题中的作用，将其视为EM风格框架中的初始化阶段，并提出了DAPS++，通过解耦扩散和数据驱动的细化，提高了计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 从贝叶斯角度看，分数扩散模型通过联合推断解决逆问题，但实际行为中先验指导有限，重建主要由测量一致性驱动，导致推断与扩散动力学脱钩。现有公式无法解释这种实际行为。

Method: 将分数扩散在逆问题解决中的作用重新解释为EM（期望-最大化）风格框架中的初始化阶段，其中扩散阶段和数据驱动的细化是完全解耦的。引入了DAPS++，允许似然项更直接地指导推断，同时保持数值稳定性。

Result: DAPS++需要更少的函数评估（NFEs）和测量优化步骤，实现了高计算效率和在各种图像恢复任务中的鲁棒重建性能。它还提供了对统一扩散轨迹在实践中仍然有效的原因的洞察。

Conclusion: DAPS++通过将扩散重新解释为EM框架中的初始化阶段并解耦扩散和数据驱动的细化，有效解决了分数扩散模型在逆问题中的局限性，实现了更直接的似然指导、更高的效率和鲁棒的性能。
...
TL;DR: 现有AI科学家忽视了知识生产的社会和历史性，且大型语言模型难以表示结构化认知和社会语境。本文提出了MirrorMind，一个分层认知架构，通过整合个体认知轨迹和集体学科记忆的双重记忆表征，实现了结构化、个性化和洞察生成式的科学推理。


<details>
  <summary>Details</summary>
Motivation: 当前的AI科学家模型将科学发现视为孤立的优化或搜索过程，忽略了知识生产固有的社会和历史属性。人类的科学洞察源于个体研究者的独特认知轨迹和集体学科记忆。然而，现有的大型语言模型（LLMs）难以有效地表示这些结构化、高保真的认知和社会语境。

Method: 本文提出了MirrorMind，一个分层认知架构，该架构整合了双重记忆表征，并采用三层框架：1. 个体层面：构建研究者的高保真认知模型，捕获其情景、语义和人格记忆。2. 领域层面：将集体知识映射为结构化的学科概念图。3. 跨学科层面：作为正交协调引擎。关键在于，该架构将记忆存储与智能体执行分离，使AI科学家能够灵活地访问个体记忆以获取独特视角，或利用集体结构进行推理。

Result: MirrorMind在四项综合任务中进行了评估，包括作者级认知模拟、互补推理、跨学科协作促进和多智能体科学问题解决。结果表明，通过整合个体认知深度与集体学科广度，MirrorMind超越了简单的事实检索，实现了结构化、个性化和能生成洞察的科学推理。

Conclusion: MirrorMind通过其独特的分层认知架构和双重记忆表征，成功地将个体认知深度与集体学科广度相结合，为AI科学家提供了更高级的、能够生成洞察的科学推理能力，弥补了现有模型在处理复杂认知和社会语境方面的不足。
...
### [12] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文认为，为实现具备持续自主性、推理和交互能力的Agentic AI，需要结合AAMAS社区的认知、协作和治理模型，以构建透明、合作和负责任的智能体系统。


<details>
  <summary>Details</summary>
Motivation: Agentic AI的愿景需要超越对智能体本身的假设，补充以明确的认知、协作和治理模型，以实现真正的持续自主性。

Method: 利用AAMAS社区的概念工具，如BDI架构、通信协议、机制设计和制度建模，将自适应、数据驱动的方法与结构化的推理和协调模型相结合，以构建Agentic AI。

Result: 通过这种方法，可以为Agentic系统勾勒出一条发展路径，使其不仅能力强大且灵活，而且透明、合作和负责任。

Conclusion: 所提出的视角弥合了形式理论与实际自主性之间的鸿沟，为Agentic AI提供了坚实的基础。
...
### [13] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了使用合成或非策略数据训练的探针在检测大型语言模型行为时，其泛化能力如何受影响，发现生成策略和数据域偏移对探针性能有显著影响，并强调了处理分布偏移的重要性。


<details>
  <summary>Details</summary>
Motivation: 探针是监控大型语言模型（LLM）行为（如欺骗、谄媚）的有效方法，但在实际应用中，许多行为的自然示例稀少，研究者被迫依赖合成或非策略的LLM响应来训练探针。本研究旨在系统评估这种数据使用方式如何影响探针的泛化能力。

Method: 研究评估了线性探针和注意力探针，针对八种不同的LLM行为，在多个LLM上进行了测试。主要考察了响应生成策略（合成或非策略数据）如何影响探针的泛化性能，包括从非策略数据到目标行为被激励的测试集（on-policy数据）的泛化，以及不同训练数据域对性能的影响。

Result: 研究发现，响应生成策略会显著影响探针性能，但影响程度因行为而异。从非策略数据成功泛化到激励目标行为的测试集（on-policy数据）预示着成功的on-policy泛化。据此预测，欺骗和“藏拙”（Sandbagging）探针在实际监控场景中可能无法从非策略数据泛化到on-policy数据。此外，训练数据域的偏移会导致更大的性能下降，不同域测试分数始终低于相同域分数。结果表明，在缺乏on-policy数据的情况下，使用相同域的非策略数据比使用不同域的on-policy数据能产生更可靠的探针。

Conclusion: 研究结果表明，在使用合成或非策略数据训练LLM行为探针时，生成策略和数据域偏移对探针的泛化能力有显著影响。尤其强调了在LLM监控中，需要开发更好的方法来处理数据分布偏移问题，以提高探针的可靠性。
...
### [14] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
...
TL;DR: 本文提出RubiSCoT，一个AI辅助框架，利用先进的自然语言处理技术（包括大型语言模型、检索增强生成和结构化思维链提示）来增强学术论文的评估，实现从提案到最终提交的一致、可扩展和透明的评估。


<details>
  <summary>Details</summary>
Motivation: 传统的论文评估方法耗时且评估者之间存在差异性，这促使研究者寻求更高效、一致的解决方案。

Method: RubiSCoT框架采用先进的自然语言处理技术，包括大型语言模型（LLMs）、检索增强生成（RAG）和结构化思维链（CoT）提示。它涵盖初步评估、多维度评估、内容提取、基于评分标准的评分和详细报告等功能。

Result: 本文展示了RubiSCoT的设计和实现，并讨论了其在通过提供一致、可扩展和透明的评估来优化学术评估过程的潜力。

Conclusion: RubiSCoT框架有潜力通过其AI支持的评估方法，优化高等教育中的学术评估流程，提升评估的一致性、可扩展性和透明度。
...
### [17] [The persistence of painting styles](https://arxiv.org/abs/2511.16695)
*Reetikaa Reddy Munnangi,Barbara Giunti*

Main category: cs.CV

TL;DR: 本文展示了持久同调（PH）作为一种拓扑数据分析方法，能客观且可解释地洞察艺术风格，区分艺术家及其作品与AI生成图像。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的主观直觉和经验，而数学工具可以提供更结构化和客观的分析视角。

Method: 使用持久同调（Persistent Homology, PH），一种拓扑数据分析方法。

Result: PH能够以统计学确定性区分不同艺术流派和同一流派内的艺术家，并能将艺术家的真实作品与AI生成的、模仿该艺术家风格的图像区分开来。

Conclusion: 持久同调为艺术风格提供了客观且可解释的见解，为艺术分析提供了一种新的数学工具，能够有效区分艺术家和识别AI生成作品。
...
### [18] [Motion Transfer-Enhanced StyleGAN for Generating Diverse Macaque Facial Expressions](https://arxiv.org/abs/2511.16711)
*Takuya Igaue,Catia Correia-Caeiro,Akito Yoshida,Takako Miyabe-Nishiwaki,Ryusuke Hayashi*

Main category: cs.CV

TL;DR: 本研究提出了一种利用StyleGAN2生成猕猴面部表情的方法，通过数据增强、样本选择和损失函数优化克服了数据限制，成功生成了多样且逼真的猕猴面部表情，并支持风格化编辑。


<details>
  <summary>Details</summary>
Motivation: 由于训练图像数量和多样性（特别是跨个体的面部表情）的限制，使用生成式AI技术生成动物面部具有挑战性。猕猴在系统神经科学和进化研究中被广泛研究，因此生成其面部表情具有重要意义。

Method: 本研究采用基于风格的生成图像模型（StyleGAN2）来生成猕猴面部表情。为解决数据限制，实施了三项关键策略：1) 数据增强，通过运动迁移（使用计算机图形动画化静态图像）合成新的面部表情图像；2) 基于初始训练的StyleGAN2模型潜在表示进行样本选择，以确保训练数据集的多样性和均匀性；3) 损失函数优化，以确保精确再现眼睛运动等细微动作。

Result: 研究结果表明，所提出的方法能够为多个猕猴个体生成多样化的面部表情，其性能优于仅使用原始静态图像训练的模型。此外，模型在基于风格的图像编辑方面表现出色，特定的风格参数对应于不同的面部动作。

Conclusion: 这些发现强调了该模型将运动分量分解为风格参数的潜力，为猕猴面部表情研究提供了有价值的工具。
...
### [19] [PairHuman: A High-Fidelity Photographic Dataset for Customized Dual-Person Generation](https://arxiv.org/abs/2511.16712)
*Ting Pan,Ye Wang,Peiguang Jing,Rui Ma,Zili Yi,Yu Liu*

Main category: cs.CV

TL;DR: 本文提出了PairHuman数据集，这是首个大规模双人肖像生成基准数据集，并引入了DHumanDiff基线方法，旨在实现高质量、个性化的双人肖像定制。


<details>
  <summary>Details</summary>
Motivation: 双人肖像定制具有巨大的应用潜力（如情感记忆保存、婚礼摄影规划），但缺乏高质量双人肖像生成所需的基准数据集。

Method: 提出了PairHuman数据集，包含超过10万张图像及丰富的元数据（图像描述、人物定位、关键点、属性标签）。同时，引入了DHumanDiff基线方法，专注于增强面部一致性，并在个性化人物生成和语义驱动场景创建之间取得平衡。

Result: 实验结果表明，所提出的数据集和方法能够生成高度定制化、视觉质量卓越且符合人类偏好的肖像。

Conclusion: PairHuman数据集和DHumanDiff方法为高质量、个性化的双人肖像生成提供了有效的解决方案和基准。
...
### [20] [A Machine Learning-Driven Solution for Denoising Inertial Confinement Fusion Images](https://arxiv.org/abs/2511.16717)
...
TL;DR: 本研究提出了一种结合小波变换的无监督自编码器，用于去除惯性约束聚变（ICF）中子图像中的高斯-泊松混合噪声，并取得了优于传统方法的去噪效果和边缘保留能力。


<details>
  <summary>Details</summary>
Motivation: 惯性约束聚变（ICF）中的中子图像常受高斯和泊松混合噪声影响，导致细节模糊、边缘不清，严重阻碍图像分析。传统去噪方法难以有效处理此类混合噪声。过去因缺乏真实中子成像数据，机器学习方法应用受限，但近期合成数据生成技术的进步为探索新的去噪方法提供了机会。

Method: 本研究采用了一种无监督自编码器，并在其潜在空间中引入了Cohen-Daubechies-Feauveau (CDF 97) 小波变换，以处理高斯-泊松混合噪声。

Result: 该网络成功地对中子成像数据进行了去噪。与通过正向模型生成的数据进行基准测试时，该方法显示出更低的重建误差和更优异的边缘保留指标，并优于Block-matching and 3D filtering (BM3D) 等非机器学习去噪机制。

Conclusion: 这种方法为中子图像降噪和ICF实验的三维重建分析提供了一个有前景的进展。
...
### [21] [SAM 3: Segment Anything with Concepts](https://arxiv.org/abs/2511.16719)
...
### [22] [Parameter-Free Neural Lens Blur Rendering for High-Fidelity Composites](https://arxiv.org/abs/2511.17014)
*Lingyan Ruan,Bin Chen,Taehyun Rhee*

Main category: cs.CV

TL;DR: 本文提出一种新颖的混合现实合成方法，直接从RGB图像估计景深模糊（CoC）图，无需深度或相机元数据，从而实现虚拟对象与真实场景的无缝融合和逼真镜头模糊渲染。


<details>
  <summary>Details</summary>
Motivation: 在混合现实中，虚拟对象与真实场景的无缝融合需要一致且自然的镜头模糊，其模糊程度通常随场景深度变化。现有方法依赖于相机参数和场景深度来计算CoC，但这些信息对于普通用户往往不可用，限制了方法的普及性和通用性。

Method: 本研究提出一种直接从RGB图像估计CoC图的方法，绕过了对场景深度或相机元数据的需求。虚拟对象的CoC值通过其符号CoC图与深度之间的线性关系推断得出，并使用一个神经重模糊网络来渲染逼真的镜头模糊。

Result: 实验结果表明，该方法实现了高保真度的合成，具有逼真的散焦效果，在定性和定量评估中均优于现有最先进的技术。

Conclusion: 该方法为混合现实应用提供了一个灵活且实用的解决方案，能够实现逼真的散焦效果合成，提高了混合现实合成的可访问性和实用性。
...
### [23] [SafeR-CLIP: Mitigating NSFW Content in Vision-Language Models While Preserving Pre-Trained Knowledge](https://arxiv.org/abs/2511.16743)
*Adeel Yousaf,Joseph Fioresi,James Beetham,Amrit Singh Bedi,Mubarak Shah*

Main category: cs.CV

TL;DR: 本文提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最近的安全替代方案，以最小化干预的方式，在提高视觉-语言模型（如CLIP）安全性的同时，显著减少对泛化性能的损害。


<details>
  <summary>Details</summary>
Motivation: 现有对视觉-语言模型（如CLIP）进行安全微调的方法，往往会导致其泛化性能显著下降。研究发现，这种权衡源于僵化的对齐策略，强制将不安全概念映射到单一预定义的安全目标，从而破坏了模型原有的语义结构。

Method: 提出一种“近邻感知”方法，将不安全概念重定向到语义上最接近的安全替代方案，以最小化表征变化。基于这一“最小干预”原则，引入了SaFeR-CLIP微调框架。此外，还贡献了一个新的基准测试集NSFW-Caps（包含1,000个高度对齐的对），用于在分布偏移下更严格地评估安全性。

Result: SaFeR-CLIP成功地协调了安全性和性能，在保持鲁棒安全性的同时，比现有方法恢复了高达8.0%的零样本准确率。

Conclusion: 研究表明，尊重预训练模型表征的几何结构是实现安全性而不牺牲性能的关键。
...
### [24] [Neighbor GRPO: Contrastive ODE Policy Optimization Aligns Flow Models](https://arxiv.org/abs/2511.16955)
...
### [25] [SVG360: Multi-View SVG Generation with Geometric and Color Consistency from a Single SVG](https://arxiv.org/abs/2511.16766)
...
### [26] [WorldGen: From Text to Traversable and Interactive 3D Worlds](https://arxiv.org/abs/2511.16825)
...
### [27] [Mesh RAG: Retrieval Augmentation for Autoregressive Mesh Generation](https://arxiv.org/abs/2511.16807)
*Xiatao Sun,Chen Liang,Qian Wang,Daniel Rakita*

Main category: cs.CV

TL;DR: Mesh RAG是一个新颖的、无需训练的即插即用框架，通过利用点云分割和配准来检索、生成和集成网格组件，显著提升了自回归网格生成模型的质量和速度，并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 三维网格的创建通常依赖于耗时且难以扩展的手动艺术制作。现有的自回归网格生成模型虽然能自动化此过程，但提高质量通常需要更大的模型或更长的序列，导致生成时间增加，并存在严重的质量-速度权衡。此外，其固有的顺序依赖性也极大地阻碍了增量编辑。

Method: 本文提出了Mesh RAG，一个受语言模型RAG启发的、无需训练的即插即用框架。它通过利用点云分割、空间变换和点云配准来检索、生成和集成网格组件，从而增强生成过程。这种基于检索的方法将生成与严格的顺序依赖解耦，实现了高效且可并行化的推理。

Result: Mesh RAG在各种基础自回归网格生成模型上展现了广泛的适用性。它显著提高了网格质量，相比顺序部分预测加速了生成速度，并实现了增量编辑，所有这些都无需重新训练模型。

Conclusion: Mesh RAG成功克服了当前自回归网格生成模型的质量-速度权衡和增量编辑困难等局限性，提供了一个高效、高质量且灵活的网格生成解决方案，且无需模型再训练。
...
### [28] [Towards Unified Vision Language Models for Forest Ecological Analysis in Earth Observation](https://arxiv.org/abs/2511.16853)
...
### [29] [BOP-ASK: Object-Interaction Reasoning for Vision-Language Models](https://arxiv.org/abs/2511.16857)
...
TL;DR: 尽管视觉语言模型（VLM）在高级空间推理方面表现出色，但在对象交互的精细理解上存在显著弱点。本文提出了BOP-ASK，一个大规模数据集，用于训练和评估VLM在对象交互推理方面的能力，并展示了模型在BOP-ASK上训练后性能的显著提升和新能力的涌现。


<details>
  <summary>Details</summary>
Motivation: 现有VLM的空间推理基准侧重于高层关系（如“左边”、“后面”），但忽略了现实世界应用所需的精细空间理解，例如精确的3D定位、对象间的物理兼容性、对象功能性和多步空间规划。这表明VLM在理解对象交互方面存在关键缺陷。

Method: 本文提出了BOP-ASK，一个大型对象交互推理数据集，用于训练和基准测试。数据生成管道利用来自BOP数据集的6D对象姿态，从中推导出精细标注，包括抓取姿态、参考对象姿态、路径规划轨迹、相对空间和深度关系以及对象间关系。BOP-ASK包含超过15万张图像和3300万个问答对，涵盖六项任务（其中四项是新任务）。研究人员评估了专有和开源VLM，并在BOP-ASK-core（一个贡献的测试基准）上进行了人工评估。此外，还发布了BOP-ASK-lab，一个域外（out-of-distribution）基准，用于测试泛化能力。

Result: 实验表明，在BOP-ASK上训练的模型优于基线模型，并展现出新兴能力，例如精确的对象和抓取姿态估计、轨迹规划以及在杂乱环境中以对象为中心的精细空间推理。

Conclusion: BOP-ASK成功解决了VLM在精细对象交互推理方面的不足，通过提供一个大规模数据集和评估基准，显著提升了模型的性能并使其获得了新的能力。数据集和数据生成管道将公开发布，以促进该领域的研究。
...
### [30] [The Joint Gromov Wasserstein Objective for Multiple Object Matching](https://arxiv.org/abs/2511.16868)
*Aryan Tajmir Riahi,Khanh Dao Duc*

Main category: cs.CV

TL;DR: 本文引入了联合格罗莫夫-瓦瑟斯坦（JGW）距离，将传统的格罗莫夫-瓦瑟斯坦（GW）框架扩展到多对多对象匹配，并通过实验证明了其在准确性和计算效率上的优越性，适用于多种复杂匹配问题。


<details>
  <summary>Details</summary>
Motivation: 传统的格罗莫夫-瓦瑟斯坦（GW）距离仅限于单个对象之间的成对匹配，限制了其在需要多对一或多对多对象匹配的场景和应用中的实用性。

Method: 引入了联合格罗莫夫-瓦瑟斯坦（JGW）目标，将GW框架扩展到对象集合之间的同步匹配。该方法提供了一个非负的差异度量，能够识别部分同构的mm空间分布，并具有点采样收敛性。它可以通过适应最优传输中的传统算法（包括熵正则化）来为点云对象表示进行公式化和求解。

Result: JGW在准确性和计算效率方面优于其他部分匹配的GW变体。在合成数据集和真实世界数据集（包括几何形状和生物分子复合物）上的实验表明，JGW在多形状匹配方面表现出有效性，适用于解决计算机图形学和结构生物学等不同领域的复杂匹配问题。

Conclusion: 联合格罗莫夫-瓦瑟斯坦（JGW）是一种有效且高效的复杂多对象匹配方法，在计算机图形学和结构生物学等多样化领域具有广阔的应用前景。
...
### [31] [Parts-Mamba: Augmenting Joint Context with Part-Level Scanning for Occluded Human Skeleton](https://arxiv.org/abs/2511.16860)
...
### [32] [Align & Invert: Solving Inverse Problems with Diffusion and Flow-based Models via Representational Alignment](https://arxiv.org/abs/2511.16870)
*Loukas Sfountouris,Giannis Daras,Paris Giampouras*

Main category: cs.CV

TL;DR: 本文提出将表示对齐（REPA）应用于逆问题，通过预训练的自监督编码器（如DINOv2）引导扩散或流生成模型进行重建，显著提高了重建质量、感知真实性和效率。


<details>
  <summary>Details</summary>
Motivation: 表示对齐已被证明能有效改善生成模型的收敛性和样本质量。本研究旨在将这一思想扩展到逆问题，利用生成模型作为先验，以提升重建精度，即使在缺乏真实信号的情况下。

Method: 我们提出在扩散或流模型与预训练的自监督视觉编码器（如DINOv2）之间应用表示对齐（REPA），以在推理阶段指导逆问题的重建过程。提供了理论结果，阐明了REPA正则化与DINOv2嵌入空间中散度度量之间的关系，以及REPA更新如何引导模型内部表示趋向于清晰图像。

Result: 研究表明，将模型表示与近似目标特征对齐可以显著提高重建保真度和感知真实感。理论分析揭示了REPA在改善感知保真度方面的作用。通过将该方法集成到多种最先进的逆问题求解器中，在超分辨率、方框修复、高斯去模糊和运动去模糊等任务上，一致地提高了重建质量，并通过减少所需的离散化步骤，在不损害底层求解器性能的情况下，提供了显著的效率提升。

Conclusion: REPA是一种通用且有效的方法，通过利用预训练的自监督编码器指导生成模型先验，显著提高了逆问题的重建质量和效率。
...
### [33] [R-AVST: Empowering Video-LLMs with Fine-Grained Spatio-Temporal Reasoning in Complex Audio-Visual Scenarios](https://arxiv.org/abs/2511.16901)
*Lu Zhu,Tiantian Geng,Yangye Chen,Teng Wang,Ping Lu,Feng Zheng*

Main category: cs.CV

TL;DR: 本文介绍了R-AVST，一个针对真实世界复杂音视频事件的时空推理数据集，包含细粒度标注和Q&A对。在此基础上，提出了AVST-Zero，一个基于强化学习的模型，用于提升音视频时空推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型（MLLMs）在视频理解方面取得了快速进展，但现有研究主要集中于简单视频场景，未能反映真实世界音视频事件的复杂性和多样性，缺乏对细粒度时空推理的能力。

Method: 1. 构建R-AVST数据集：设计了基于LLM的关键对象提取、自动空间标注和人工质量检查的流程，生成了超过5K未裁剪视频、27K对象和100种音视频事件的标注。2. 定义核心任务：基于R-AVST数据集，定义了三个音视频场景下的时空推理核心任务，并生成了超过8K高质量、均匀分布的问答对。3. 提出AVST-Zero模型：一个基于强化学习的模型，通过精心设计的多维度奖励直接优化行为，避免了中间监督，以增强推理能力。

Result: 实验验证了R-AVST数据集在推进音视频时空推理方面的有效性。在此基础上，AVST-Zero模型与现有模型相比展现出具有竞争力的性能。

Conclusion: R-AVST是首个专为真实世界音视频时空推理设计的细粒度数据集，AVST-Zero为解决该领域未来的挑战提供了一个新颖的视角，共同推动了复杂音视频理解任务的发展。
...
### [34] [Glass Surface Detection: Leveraging Reflection Dynamics in Flash/No-flash Imagery](https://arxiv.org/abs/2511.16887)
...
### [35] [Warm Diffusion: Recipe for Blur-Noise Mixture Diffusion Models](https://arxiv.org/abs/2511.16904)
*Hao-Chien Hsueh,Chi-En Yen,Wen-Hsiao Peng,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 本文提出“暖扩散”（Warm Diffusion），一个统一的模糊-噪声混合扩散模型（BNMD），它结合了热扩散和冷扩散的优点，通过联合控制模糊和噪声来改进图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型存在局限：热扩散（纯噪声）未充分利用高频和低频图像细节之间的强相关性，导致早期生成步骤随机；冷扩散（纯模糊）虽利用图像相关性，但忽略了噪声在塑造数据流形中的作用，导致脱离流形问题并性能下降。研究旨在整合两者的优点。

Method: 提出“暖扩散”（Warm Diffusion），一个模糊-噪声混合扩散模型（BNMD），用于联合控制模糊和噪声。采用分而治之的策略，利用图像中的光谱依赖性，通过解耦去噪和去模糊过程来简化得分模型估计。进一步通过光谱分析研究模糊-噪声比（BNR），以探讨模型学习动态和数据流形变化之间的权衡。

Result: 在多个基准测试中进行了广泛实验，验证了所提出方法在图像生成方面的有效性。

Conclusion: 暖扩散模型通过巧妙结合模糊和噪声，克服了传统热扩散和冷扩散的缺点，有效利用图像相关性和随机性，从而显著提高了图像生成性能。
...
### [36] [Q-REAL: Towards Realism and Plausibility Evaluation for AI-Generated Content](https://arxiv.org/abs/2511.16908)
*Shushi Wang,Zicheng Zhang,Chunyi Li,Wei Wang,Liya Ma,Fengjiao Chen,Xiaoyu Li,Xuezhi Cao,Guangtao Zhai,Xiaohong Liu*

Main category: cs.CV

TL;DR: 本文提出了Q-Real数据集，用于细粒度评估AI生成图像的真实感和合理性，并构建了Q-Real Bench来评估多模态大语言模型（MLLMs），同时设计了MLLM微调框架。


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成内容质量评估数据集和模型通常只提供单一的粗略质量分数，无法为生成模型的改进提供有针对性的指导。在AI生成图像应用中，真实感和合理性是两个关键维度，细粒度评估对于提升生成性能至关重要。

Method: 研究者构建了Q-Real数据集，包含3,088张由流行文生图模型生成的图像。每张图像都标注了主要实体的位置，并提供了关于真实感和合理性的判断问题及归因描述。基于Q-Real，构建了Q-Real Bench，用于评估多模态大语言模型在判断和带推理的定位两个任务上的表现。此外，设计了一个微调框架来增强MLLM的能力。

Result: 实验结果表明，Q-Real数据集具有高质量和重要性，并且Q-Real Bench基准测试具有全面性。

Conclusion: 本研究成功引入了一个新颖的细粒度评估数据集和基准，为AI生成图像的质量评估和生成模型的优化提供了更有针对性的工具，并展示了其在增强MLLM能力方面的潜力。
...
### [37] [DeltaDeno: Zero-Shot Anomaly Generation via Delta-Denoising Attribution](https://arxiv.org/abs/2511.16920)
*Chaoran Xu,Chengkan Lv,Qiyu Chen,Yunkang Cao,Feng Zhang,Zhengtao Zhang*

Main category: cs.CV

TL;DR: DeltaDeno是一种无需训练的零样本异常生成方法，通过对比两个扩散分支来定位和编辑缺陷，生成逼真的局部异常，并在下游检测中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要异常样本进行少样本微调，这与异常稀缺性相悖，并容易导致对类别先验的过拟合。本研究旨在解决在没有真实异常样本或训练的情况下生成异常的问题。

Method: 本文提出了Delta-Denoising (DeltaDeno)，一种无需训练的零样本异常生成方法。它通过在共享调度下，利用一对最小提示词驱动的两个扩散分支进行对比，从而定位和编辑缺陷。该方法将每步去噪增量累积到图像特定的定位图中，生成一个掩码来指导后期扩散步骤中的潜在修复，以保留周围上下文并生成逼真的局部缺陷。为了提高稳定性和控制，DeltaDeno还执行令牌级提示词优化，以对齐共享内容并强化异常令牌，并在预测区域中对异常令牌应用空间注意力偏差。

Result: 在公共数据集上的实验表明，DeltaDeno在生成质量、真实感方面表现出色，并持续提升了下游检测性能。

Conclusion: DeltaDeno提供了一种无需真实异常样本或训练的有效零样本方法，能够生成逼真的局部异常，显著提高了异常检测的性能。
...
### [38] [OmniGround: A Comprehensive Spatio-Temporal Grounding Benchmark for Real-World Complex Scenarios](https://arxiv.org/abs/2511.16937)
*Hong Gao,Jingyu Wu,Xiangkai Xu,Kangni Xie,Yunchen Zhang,Bin Zhong,Xurui Gao,Min-Ling Zhang*

Main category: cs.CV

TL;DR: 本文提出OmniGround，一个用于时空视频定位（STVG）的综合基准，并引入PG-TAF，一个无训练的两阶段框架，显著提升了在复杂真实世界场景中的STVG性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大型语言模型取得进展，但当前时空视频定位（STVG）模型在处理多样化对象和复杂查询的真实世界需求方面仍存在显著差距。这归因于现有基准范围有限，导致模型出现类别偏差、过度简化推理和语言鲁棒性差的问题。

Method: 本文引入了OmniGround，一个包含3,475个视频、81个类别和复杂真实世界查询的综合基准。提出了一种结合多向跟踪和智能纠错的“前向-后向-精炼”标注流程，以生成高质量标签。此外，引入了DeepSTG，一个系统性评估框架，从四个互补维度量化数据集质量。受性能下降的启发，提出PG-TAF，一个无训练的两阶段框架，将STVG分解为高级时间定位和细粒度时空传播。

Result: 评估显示，在复杂真实世界场景（特别是小型/被遮挡对象和复杂空间关系）中，模型的性能平均下降10.4%。PG-TAF在OmniGround上实现了m_tIoU 25.6%和m_vIoU 35.6%的改进，并在四个基准测试中均获得了持续的性能提升。

Conclusion: OmniGround基准揭示了现有STVG模型在复杂真实世界场景中的局限性。本文提出的PG-TAF框架通过分解任务，显著提高了在OmniGround及其他基准上的STVG性能，为未来的研究提供了新的方向。
...
### [39] [RacketVision: A Multiple Racket Sports Benchmark for Unified Ball and Racket Analysis](https://arxiv.org/abs/2511.17045)
*Linfeng Dong,Yuchen Yang,Hao Wu,Wei Wang,Yuenan HouZhihang Zhong,Xiao Sun*

Main category: cs.CV

TL;DR: RacketVision是一个新的大规模、细粒度数据集和基准，用于乒乓球、网球和羽毛球等球拍运动的计算机视觉分析，首次提供球拍姿态和球位置标注。它支持球追踪、球拍姿态估计和球轨迹预测任务，并发现交叉注意力机制对于融合球拍姿态特征以提升轨迹预测性能至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有体育分析数据集缺乏大规模、细粒度的球拍姿态标注与传统球位置信息相结合，限制了对复杂人-物体交互的研究。本研究旨在通过提供此类数据来推进球拍运动中的计算机视觉分析。

Method: 引入RacketVision数据集，包含乒乓球、网球和羽毛球的球拍姿态和球位置的细粒度标注。定义了三个相互关联的任务：细粒度球追踪、关节式球拍姿态估计和预测性球轨迹预测。评估了现有基线模型，并对比了多模态融合策略（朴素拼接与交叉注意力机制）。

Result: 研究发现，简单地拼接球拍姿态特征会降低性能，而交叉注意力机制对于发挥球拍姿态特征的价值至关重要。采用交叉注意力机制后，轨迹预测结果超越了强大的单模态基线。

Conclusion: RacketVision数据集是动态物体追踪、条件运动预测和体育领域多模态分析的宝贵资源和良好起点。交叉注意力机制是有效融合多模态特征以提升预测性能的关键。
...
### [40] [The Finer the Better: Towards Granular-aware Open-set Domain Generalization](https://arxiv.org/abs/2511.16979)
...
### [41] [UniModel: A Visual-Only Framework for Unified Multimodal Understanding and Generation](https://arxiv.org/abs/2511.16917)
...
### [43] [Flow-Guided Implicit Neural Representation for Motion-Aware Dynamic MRI Reconstruction](https://arxiv.org/abs/2511.16948)
*Baoqing Li,Yuanyuan Liu,Congcong Liu,Qingyong Zhu,Jing Cheng,Yihang Zhou,Hao Chen,Zhuo-Xu Cui,Dong Liang*

Main category: cs.CV

TL;DR: 本文提出一种隐式神经表示（INR）框架，通过联合建模动态图像序列和运动场，并在光学流方程的约束下，实现动态磁共振成像（dMRI）的运动补偿重建，无需预估运动。


<details>
  <summary>Details</summary>
Motivation: 动态磁共振成像常面临采样不足和运动伪影的挑战。传统的运动补偿重建依赖于预估的光学流，但在欠采样条件下不准确，从而降低重建质量。

Method: 该方法采用一个隐式神经表示（INR）来参数化时空图像内容，另一个INR来表示光学流。两者通过光学流方程（作为物理启发式正则化）进行耦合，并结合数据一致性损失（与k空间测量一致）。这种联合优化能够同时恢复时间连贯的图像和运动场，无需预先估计流场。

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了卓越的重建质量、准确的运动估计和改进的时间保真度。

Conclusion: 隐式联合建模结合流正则化约束在推动动态磁共振成像重建方面具有巨大潜力。
...
### [44] [Rethinking Diffusion Model-Based Video Super-Resolution: Leveraging Dense Guidance from Aligned Features](https://arxiv.org/abs/2511.16928)
...
### [45] [FingerCap: Fine-grained Finger-level Hand Motion Captioning](https://arxiv.org/abs/2511.16951)
...
TL;DR: 本文提出FingerCap任务，旨在生成细粒度的手指级手部动作描述。为此，构建了FingerCap-40K数据集和LLM驱动的评估工具HandJudge。为解决视频多模态大模型（Video-MLLMs）在捕捉细微手指动作时的时间稀疏性问题，引入了FiGOP方法，通过结合RGB关键帧和后续手部关键点来恢复精细时间线索，并在实验中取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度的人手运动对于视觉感知、具身智能和多模态通信至关重要。然而，现有的Video-MLLMs在捕捉细微、高频的手指运动动态时面临时间稀疏性瓶颈，因为稀疏的RGB采样不足以捕获这些细节，导致其在手指级推理方面表现不佳。

Method: 本文提出细粒度手指级手部动作描述（FingerCap）任务。为此，构建了包含4万对视频和描述的FingerCap-40K数据集，涵盖指令式手指动作和自然手物交互。评估采用HandJudge，一个基于大型语言模型（LLM）的评估标准，衡量手指级正确性和动作完整性。针对时间稀疏性问题，引入了FiGOP（Finger Group-of-Pictures）方法，将每个RGB关键帧与直到下一个关键帧的后续手部关键点配对，并通过一个轻量级时间编码器将关键点转换为运动嵌入，并与RGB特征融合。

Result: 实验表明，强大的开源和闭源Video-MLLMs在手指级推理方面仍面临挑战。而我们采用FiGOP增强的模型在HandJudge和人类研究中均显示出显著且一致的性能提升。

Conclusion: FiGOP方法通过适应经典的GOP概念到手指运动，在不增加RGB密度的情况下有效恢复了精细的时间线索。该方法显著提高了模型在细粒度手指级手部动作描述任务上的表现，解决了现有Video-MLLMs在处理微妙高频手指动态时的不足。
...
TL;DR: 本文提出了一种名为OmniPT的统一行人跟踪框架，它基于大型视觉语言模型（LVLMs），能够进行跟踪、基于引用的跟踪以及交互式语义理解，并通过多阶段强化学习和微调弥补了LVLMs在实例级任务上的性能差距。


<details>
  <summary>Details</summary>
Motivation: LVLMs在图像级任务上表现出色，但在实例级任务（如视觉定位、目标检测）上与专家模型仍有差距。然而，新兴的行人跟踪任务（如Referring MOT、Semantic MOT）强调对跟踪对象的深层语义理解，这正是LVLMs的优势所在。因此，研究动机在于如何将LVLMs应用于这些需要高级语义理解的行人跟踪任务。

Method: 本文提出了一个名为OmniPT的统一行人跟踪框架。为解决如何将跟踪任务建模为基础模型可执行的任务以及如何使模型输出格式化答案这两个问题，作者设计了一个RL-Mid Training-SFT-RL的训练阶段：首先进行简单的RL阶段，使模型输出固定的、可监督的边界框格式；接着使用大量行人相关数据集进行中期训练；然后对多个行人跟踪数据集进行监督微调；最后进行另一个RL阶段，以提高跟踪性能和指令遵循能力。

Result: 在跟踪基准上的实验结果表明，所提出的方法OmniPT比以前的方法表现更好。

Conclusion: OmniPT成功地将行人跟踪、基于引用的跟踪和对跟踪对象的语义理解统一在一个框架中，并通过创新的多阶段训练策略，在性能上超越了现有方法，有效利用了LVLMs在高级语义理解方面的优势。
...
### [47] [SING3R-SLAM: Submap-based Indoor Monocular Gaussian SLAM with 3D Reconstruction Priors](https://arxiv.org/abs/2511.17207)
*Kunyi Li,Michael Niemeyer,Sen Wang,Stefano Gasperini,Nassir Navab,Federico Tombari*

Main category: cs.CV

TL;DR: SING3R-SLAM是一个基于高斯表示的密集RGB SLAM框架，它通过结合局部重建和全局高斯地图，实现了全局一致且紧凑的3D重建，并支持多种下游应用。


<details>
  <summary>Details</summary>
Motivation: 当前的密集3D重建技术难以有效集成到SLAM中，存在漂移和冗余点图问题，限制了效率和下游任务（如新视角合成）。

Method: SING3R-SLAM首先通过轻量级跟踪和重建模块构建局部一致的子图，然后逐步将这些子图对齐并融合到一个全局高斯地图中，以强制执行跨视图几何一致性。该全局地图反过来为纠正局部漂移和增强跟踪鲁棒性提供反馈。

Result: 实验表明，SING3R-SLAM在跟踪、3D重建和新视角渲染方面达到了最先进的水平，跟踪性能提升超过12%，生成了更精细、更详细的几何结构，同时在真实世界数据集上保持了紧凑且内存高效的全局表示。

Conclusion: SING3R-SLAM提供了一个强大、高效且多功能的密集RGB SLAM解决方案，能够实现高质量的3D重建和新视角合成，解决了传统方法中的漂移和冗余问题。
...
### [48] [A lightweight detector for real-time detection of remote sensing images](https://arxiv.org/abs/2511.17147)
*Qianyi Wang,Guoqiang Ren*

Main category: cs.CV

TL;DR: 本文提出DMG-YOLO，一种轻量级实时检测器，专为遥感图像中的小目标检测设计，通过双分支特征提取、多尺度特征融合和全局局部聚合FPN实现高精度和高效率。


<details>
  <summary>Details</summary>
Motivation: 遥感图像实时检测面临小目标普遍存在以及精度与效率平衡的挑战。

Method: DMG-YOLO包含：1) 主干网络中的双分支特征提取（DFE）模块，并行提取局部特征（通过深度可分离卷积）和全局上下文（通过带门控机制的视觉Transformer）；2) 多尺度特征融合（MFF）模块，利用空洞卷积增强多尺度集成并保留细节；3) 颈部网络中的全局局部聚合特征金字塔网络（GLAFPN），通过全局局部特征融合进一步提升小目标检测。

Result: 在VisDrone2019和NWPU VHR-10数据集上的大量实验表明，DMG-YOLO在mAP、模型大小和其他关键指标方面均取得了有竞争力的性能。

Conclusion: DMG-YOLO成功解决了遥感图像中小目标实时检测的挑战，在保持高效率的同时实现了优异的检测精度。
...
### [49] [Shape-preserving Tooth Segmentation from CBCT Images Using Deep Learning with Semantic and Shape Awareness](https://arxiv.org/abs/2511.16936)
*Zongrui Ji,Zhiming Cui,Na Li,Qianhan Zheng,Miaojing Shi,Ke Deng,Jingyang Zhang,Chaoyuan Li,Xuepeng Chen,Yi Dong,Lei Ma*

Main category: cs.CV

TL;DR: 针对锥束CT图像中牙齿粘连导致的分割挑战，本文提出一个深度学习框架，通过整合语义和形状感知，实现了形状保持的牙齿分割，并显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从锥束CT（CBCT）图像中准确分割牙齿对数字牙科至关重要，但牙间粘连会导致严重的解剖形状扭曲，使分割变得极具挑战性。

Method: 本文提出一个深度学习框架，整合了语义和形状感知以实现形状保持的分割。具体方法包括：1) 引入目标牙齿质心提示的多标签学习策略，建模牙齿间的语义关系以减少形状歧义；2) 采用牙齿形状感知学习机制，明确施加形态约束以保持边界完整性。这些组件通过多任务学习统一优化。

Result: 在内部和外部数据集上的广泛评估表明，所提出的方法显著优于现有方法。

Conclusion: 本文方法有效缓解了形状扭曲，并提供了符合解剖学特征的牙齿边界。
...
TL;DR: ReBrain是一个检索增强扩散框架，用于从稀疏CT扫描中重建完整的脑部MRI，通过结合布朗桥扩散模型和结构相似的检索参考来解决低分辨率CT带来的挑战。


<details>
  <summary>Details</summary>
Motivation: MRI对脑疾病诊断至关重要，但并非所有患者都适用。从CT合成MRI是一个方向，但低剂量CT通常导致稀疏且平面外分辨率差的数据，使得准确重建完整脑部MRI极具挑战性。

Method: ReBrain框架首先使用布朗桥扩散模型（BBDM）从有限的3D CT切片中合成2D MRI切片。同时，通过微调的检索模型从数据库中检索结构和病理相似的CT切片作为参考。这些检索到的切片通过ControlNet分支引入，指导中间MRI切片的生成并确保结构连续性。对于检索失败的情况，采用球面线性插值提供额外指导。

Result: 在SynthRAD2023和BraTS数据集上的大量实验表明，ReBrain在稀疏条件下的跨模态重建中实现了最先进的性能。

Conclusion: ReBrain通过结合扩散模型、检索增强和插值指导，有效解决了从稀疏CT重建完整脑部MRI的难题，并在相关任务中达到了领先水平。
...
### [51] [MultiPriv: Benchmarking Individual-Level Privacy Reasoning in Vision-Language Models](https://arxiv.org/abs/2511.16940)
*Xiongtao Sun,Hui Li,Jiaming Zhang,Yujie Yang,Kaili Liu,Ruxin Feng,Wen Jun Tan,Wei Yang Bryan Lim*

Main category: cs.CV

TL;DR: 现代视觉-语言模型（VLMs）存在严重的推理型隐私风险，现有基准无法有效评估。本文提出MultiPriv，一个新基准来系统评估VLMs的个体级隐私推理能力，并发现VLMs普遍存在未被测量的风险，且现有安全对齐措施无效。


<details>
  <summary>Details</summary>
Motivation: 现代VLMs的推理能力使得隐私风险从简单的属性感知升级到个体层面的关联，但现有隐私基准主要评估隐私感知，未能解决更关键的隐私推理风险，即VLM推断和链接分布式信息以构建个人资料的能力。为弥补这一关键空白，需要新的评估框架。

Method: 本文提出了MultiPriv，首个旨在系统评估VLM个体级隐私推理能力的基准。引入了“隐私感知与推理（PPR）”框架，并构建了一个新颖的双语多模态数据集。该数据集的核心是一个合成个体档案组件，其中标识符（如面部、姓名）与敏感属性精确关联。此设计支持九项挑战性任务，涵盖从属性检测到跨图像重识别和链式推理的完整PPR范围。对超过50个基础和商业VLM进行了大规模评估。

Result: 分析结果显示：(1) 许多VLM具有显著且未被测量的基于推理的隐私风险。(2) 感知层面的指标不足以预测这些推理风险，揭示了评估上的关键差距。(3) 现有的安全对齐措施在抵御此类基于推理的攻击时表现出不一致和无效性。

Conclusion: MultiPriv揭示了VLMs中系统性的隐私漏洞，并为开发健壮的、保护隐私的VLM提供了必要的框架。
...
TL;DR: STAR（Spanning Tree Autoregressive）建模通过利用均匀生成树的遍历顺序，为图像自回归模型提供了灵活的序列顺序选择，同时保持采样性能，并支持图像编辑，解决了传统随机排列方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归（AR）模型在视觉生成中采用随机排列的序列顺序来获取双向上下文，但这样要么导致性能下降，要么牺牲了推理时序列顺序选择的灵活性。研究旨在找到一种方法，在保持采样性能的同时，能融入图像的先验知识（如中心偏置和局部性），并提供足够灵活的序列顺序以适应图像编辑。

Method: 本文提出了STAR建模，它在图像块位置定义的格子上采样均匀生成树的遍历顺序。这些遍历顺序通过广度优先搜索（BFS）获得。通过拒绝采样，可以确保图像的连接部分观测作为序列的前缀出现。与随机排列相比，STAR采用了一种定制且结构化的随机策略，并且无需对现有语言AR模型的架构进行重大修改。

Result: STAR模型在保持采样性能的同时，保留了后缀补全的能力，并能适应推理时的图像编辑。它成功地融入了图像的先验知识（如中心偏置和局部性），并提供了足够灵活的序列顺序。

Conclusion: STAR建模通过一种结构化的随机策略（基于生成树遍历），成功地解决了传统自回归模型在视觉生成中随机排列带来的性能下降或灵活性受限的问题。它在不显著改变模型架构的情况下，实现了采样性能、序列顺序灵活性和先验知识融合的平衡。
...
### [53] [QueryOcc: Query-based Self-Supervision for 3D Semantic Occupancy](https://arxiv.org/abs/2511.17221)
*Adam Lilja,Ji Lan,Junsheng Fu,Lars Hammarstrand*

Main category: cs.CV

TL;DR: QueryOcc是一个基于查询的自监督框架，通过独立的4D时空查询直接学习连续的3D语义占用。它在Occ3D-nuScenes基准上显著优于现有方法，并支持视觉基础模型或原始激光雷达数据的监督。


<details>
  <summary>Details</summary>
Motivation: 从图像中学习3D场景几何和语义是计算机视觉的核心挑战，也是自动驾驶的关键能力。由于大规模3D标注成本过高，现有自监督方法要么依赖2D渲染一致性（3D结构隐式出现），要么依赖累积激光雷达点云的离散体素网格（限制空间精度和可扩展性）。

Method: 本文提出了QueryOcc，一个基于查询的自监督框架，通过在相邻帧中采样的独立4D时空查询直接学习连续的3D语义占用。该框架支持从视觉基础模型派生的伪点云或原始激光雷达数据进行监督。为实现长距离监督并在恒定内存下进行推理，引入了一种收缩场景表示，它在保留近场细节的同时平滑压缩远距离区域。

Result: QueryOcc在自监督Occ3D-nuScenes基准测试中，语义RayIoU超越了之前的基于摄像头的SOTA方法26%，同时以11.6 FPS运行。
...
### [54] [MatPedia: A Universal Generative Foundation for High-Fidelity Material Synthesis](https://arxiv.org/abs/2511.16957)
*Di Luo,Shuhui Yang,Mingxin Yang,Jiawei Lu,Yixuan Tang,Xintong Han,Zhuo Chen,Beibei Wang,Chunchao Guo*

Main category: cs.CV

TL;DR: MatPedia是一个基于新型RGB-PBR联合表示和视频扩散架构的基础模型，旨在统一材料生成任务，实现高质量、多样化的物理渲染（PBR）材料合成。


<details>
  <summary>Details</summary>
Motivation: PBR材料创建劳动密集且需专业知识。现有生成模型缺乏统一的表示来桥接自然图像外观和PBR属性，导致任务特定流程碎片化，且无法利用大规模RGB图像数据。

Method: MatPedia采用新颖的RGB-PBR联合表示，将材料编码为两个相互依赖的潜在变量：一个用于RGB外观，另一个用于编码互补物理属性的四个PBR贴图。通过将它们公式化为5帧序列，并利用视频扩散架构，MatPedia自然地捕捉了它们之间的相关性，并从RGB生成模型中转移了视觉先验。模型在MatHybrid-410K（结合了PBR数据集和大规模RGB图像的混合语料库）上进行训练。

Result: MatPedia实现了原生$1024\times1024$的合成，在质量和多样性方面显著超越现有方法。它在一个单一架构内支持多项材料任务，包括文本到材料生成、图像到材料生成和固有分解。

Conclusion: MatPedia通过其创新的RGB-PBR联合表示和视频扩散架构，提供了一个统一的框架，能够处理多种材料任务，并显著提高了PBR材料合成的质量和多样性，克服了现有方法的局限性。
...
### [55] [Point-Supervised Facial Expression Spotting with Gaussian-Based Instance-Adaptive Intensity Modeling](https://arxiv.org/abs/2511.16952)
*Yicheng Deng,Hideaki Hayashi,Hajime Nagahara*

Main category: cs.CV

TL;DR: 本文提出了一种点监督面部表情识别（P-FES）框架，仅需每个实例一个时间戳注释。该框架包含一个基于高斯分布的实例自适应强度建模（GIM）模块用于软伪标签，以及一个区分宏表情和微表情的尖峰分类分支，并通过强度感知对比损失增强特征学习。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情识别方法主要依赖于完全监督学习，需要耗时且昂贵的视频时间边界标注。为了解决这一问题，研究者旨在探索仅需单点注释的更高效训练方法。

Method: 本文提出了一个独特的双分支P-FES框架：
1.  **类无关表情强度分支**：引入高斯基于实例自适应强度建模（GIM）模块，通过检测伪顶点帧、估计持续时间并构建实例级高斯分布，为表情帧分配软伪标签，以更可靠地监督表情强度，避免硬伪标签的局限性。
2.  **类感知尖峰分类分支**：仅基于伪顶点帧区分宏表情和微表情。
3.  **强度感知对比损失**：通过对比中性帧和不同强度的表情帧，增强判别性特征学习并抑制中性噪声。
在推理阶段，两个分支独立工作，分别生成表情提议和进行宏微表情分类。

Result: 在SAMM-LV、CAS(ME)$^2$和CAS(ME)$^3$数据集上进行了广泛实验，结果表明所提出的框架是有效的。

Conclusion: 本文提出的点监督面部表情识别框架，通过其独特的双分支结构（包含GIM模块进行软伪标签和类感知尖峰分类）以及强度感知对比损失，有效解决了时间边界标注的限制，并在多个数据集上取得了良好的性能。
...
### [56] [Two Heads Better than One: Dual Degradation Representation for Blind Super-Resolution](https://arxiv.org/abs/2511.16963)
*Hsuan Yuan,Shao-Yu Weng,I-Hsuan Lo,Wei-Chen Chiu,Yu-Syuan Xu,Hao-Chien Hsueh,Jen-Hui Chuang,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 本文提出了一种双分支降质信息提取器网络，用于解决盲超分辨率问题。该网络预测模糊和噪声两种无监督降质嵌入，使超分辨率网络能够分别适应它们，并利用降质提取器作为正则化器，实现了盲超分辨率任务的最新性能。


<details>
  <summary>Details</summary>
Motivation: 现有单图像超分辨率（SISR）方法在降质模式已知且固定（如双三次下采样）时表现出色，但当实际降质偏离这些假设时，性能会显著下降。一些盲超分辨率方法假设无噪声降质，或未明确考虑降质模型中的噪声，这限制了其在更复杂真实场景中的应用。

Method: 本文提出了一种双分支降质信息提取器网络（Dual Branch Degradation Extractor Network）。该网络预测两种无监督降质嵌入，分别代表图像中的模糊信息和噪声信息。超分辨率网络可以以不同的方式适应这些模糊和噪声嵌入。此外，降质提取器被用作正则化器，以利用超分辨率图像和高分辨率图像之间的差异。

Result: 在多个基准测试上的大量实验表明，本文方法在盲超分辨率问题中取得了最先进（SOTA）的性能。

Conclusion: 通过提出双分支降质信息提取器网络，该方法能够有效预测和区分模糊与噪声降质信息，并使超分辨率网络能够分别适应这些信息。同时，利用降质提取器作为正则化器，显著提升了盲超分辨率任务的性能，达到了最先进水平。
...
### [57] [Real-Time Cooked Food Image Synthesis and Visual Cooking Progress Monitoring on Edge Devices](https://arxiv.org/abs/2511.16965)
*Jigyasa Gupta,Soumya Goyal,Anil Kumar,Ishan Jindal*

Main category: cs.CV

TL;DR: 本文提出了一种用于在边缘设备上从生食图像合成熟食图像的边缘高效生成器，该生成器由食谱和烹饪状态指导，并引入了一个新的烹饪图像相似性(CIS)度量。该模型显著提高了图像质量并超越了现有基线。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上从生食合成逼真的熟食图像是一个挑战性的生成任务，需要捕捉烹饪过程中复杂的纹理、颜色和结构变化。现有的图像到图像生成方法通常产生不真实的结果，或对边缘部署而言资源消耗过大。

Method: 研究引入了第一个基于烤箱的烹饪进程数据集，包含厨师标注的熟度等级。提出了一种边缘高效的、由食谱和烹饪状态指导的生成器，能够根据生食图像合成逼真的熟食图像，并支持用户偏好的视觉目标。为确保时间一致性和烹饪合理性，引入了领域特定的“烹饪图像相似性(CIS)”度量，该度量既作为训练损失，也作为进度监控信号。

Result: 该模型在FID分数上显著优于现有基线，在作者的数据集上实现了30%的改进，在公共数据集上实现了60%的改进。

Conclusion: 所提出的边缘高效生成器，结合新数据集和CIS度量，能够从生食图像合成逼真的熟食图像，支持用户偏好的视觉目标，并在图像质量上显著优于现有方法，同时适用于边缘设备部署。
...
### [58] [Intervene-All-Paths: Unified Mitigation of LVLM Hallucinations across Alignment Formats](https://arxiv.org/abs/2511.17254)
*Jiaye Qian,Ge Zheng,Yuchen Zhu,Sibei Yang*

Main category: cs.CV

TL;DR: 本研究提出了一种针对大型视觉-语言模型（LVLMs）幻觉的干预框架，发现幻觉源于多个因果路径的相互作用，并依赖于问答格式。通过识别关键幻觉头并进行干预，有效减少了幻觉。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉-语言模型（LVLMs）表现出色，但仍易产生幻觉。研究旨在深入理解LVLMs中幻觉的因果机制，并开发有效方法来减轻幻觉。

Method: 提出一个与Transformer因果架构对齐的综合干预框架，整合了图像到输入文本、图像到输出文本以及文本到文本路径对幻觉的影响。首次发现LVLMs根据问答对齐格式依赖不同的路径。在此基础上，提出了识别和干预每个路径中关键幻觉头的简单有效方法，并针对判别式和生成式格式进行了定制。

Result: 研究发现LVLMs中的幻觉并非源于单一因果路径，而是图像到输入文本、图像到输出文本和文本到文本路径之间相互作用的结果。LVLMs根据问答对齐格式依赖不同的路径。实验表明，所提出的方法在多个基准测试中持续减少了不同对齐类型下的幻觉。

Conclusion: LVLMs中的幻觉是一个复杂现象，涉及多个交互的因果路径，并受到问答格式的影响。通过针对性地识别和干预这些路径中的关键幻觉头，可以有效减轻幻觉问题。
...
### [59] [A Diversity-optimized Deep Ensemble Approach for Accurate Plant Leaf Disease Detection](https://arxiv.org/abs/2511.16982)
*Sai Nath Chowdary Medikonduru,Hongpeng Jin,Yanzhao Wu*

Main category: cs.CV

TL;DR: 本文提出了一种名为协同多样性 (SQ) 的新框架，用于改进深度集成模型中成员模型的选择，从而显著提高基于图像的植物病害检测的准确性。


<details>
  <summary>Details</summary>
Motivation: 植物病害对全球农业造成巨大经济损失并威胁粮食安全，及时准确的检测至关重要。深度集成模型虽能提高预测精度，但现有集成多样性度量 (Q 度量) 在选择高性能集成成员方面存在局限性，难以识别最优团队。

Method: 首先，作者全面分析了现有集成多样性 Q 度量的局限性。其次，提出了 SQ 度量，这是一种新颖的度量方法，能捕捉集成成员之间的协同作用并与集成准确性保持一致。最后，通过在植物叶片图像数据集上的大量实验验证了 SQ 方法。

Result: 实验结果表明，SQ 度量能有效捕捉集成成员间的协同作用，并与集成准确性高度相关。该方法显著改善了集成模型的选择，从而提高了植物病害检测的准确性。

Conclusion: SQ 框架为更可靠、高效的基于图像的植物病害检测铺平了道路，通过改进深度集成模型的成员选择，有效提升了检测精度。
...
### [60] [DReX: Pure Vision Fusion of Self-Supervised and Convolutional Representations for Image Complexity Prediction](https://arxiv.org/abs/2511.16991)
*Jonathan Skaza,Parsa Madinei,Ziqi Wen,Miguel Eckstein*

Main category: cs.CV

TL;DR: DReX是一个纯视觉模型，通过融合自监督DINOv3和卷积ResNet-50的表示，并使用可学习的注意力机制，在视觉复杂度预测任务上实现了最先进的性能，且参数量远少于现有方法，表明视觉特征本身足以进行人类对齐的复杂度预测。


<details>
  <summary>Details</summary>
Motivation: 视觉复杂度预测是计算机视觉中的一个基础问题，在图像压缩、检索和分类等领域有应用。理解人类如何感知图像复杂性也是认知科学中一个长期存在的问题。现有方法利用多模态模型结合视觉和语言表示，但语言信息是否必要仍不清楚。

Method: 本文提出了DReX (DINO-ResNet Fusion)，一个纯视觉模型，通过可学习的注意力机制融合自监督DINOv3 ViT-S/16和卷积ResNet-50的表示来预测图像复杂度。该架构整合了ResNet-50的多尺度分层特征和DINOv3的语义丰富表示，从而捕捉低级纹理模式和高级语义结构。

Result: DReX在IC9600基准测试上达到了最先进的性能（Pearson r = 0.9581），超越了包括多模态图像-文本训练方法在内的现有方法，同时使用的可学习参数约少21.5倍。此外，DReX在多个数据集和指标（Pearson、Spearman相关性、RMSE、MAE）上表现出强大的泛化能力。消融和注意力分析证实DReX利用了两个骨干网络的互补线索，其中DINOv3 [CLS] token增强了对视觉复杂度的敏感性。

Conclusion: 研究结果表明，仅凭视觉特征足以进行人类对齐的复杂度预测。当自监督Transformer和监督深度卷积神经网络正确融合时，它们能为这项任务提供互补和协同的优势。
...
### [61] [Gradient-Driven Natural Selection for Compact 3D Gaussian Splatting](https://arxiv.org/abs/2511.16980)
*Xiaobin Deng,Qiuli Yu,Changyu Diao,Min Li,Duanqing Xu*

Main category: cs.CV

TL;DR: 本文提出了一种受自然选择启发的剪枝框架，通过将生存压力建模为应用于不透明度的正则化梯度场，使3D高斯飞溅（3DGS）能够自主优化和剪枝高斯，从而在显著减少存储和计算开销的同时提高渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS因使用大量高斯基元而导致巨大的存储和计算开销。现有剪枝方法依赖手动设计标准或引入额外可学习参数，效果不佳，因此需要一种更优、更自动化的解决方案。

Method: 该方法提出一个受自然选择启发的剪枝框架：将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度（旨在最大化渲染质量）自主决定高斯的保留或剪枝。此过程完全可学习且无需人工干预。此外，引入了带有有限不透明度先验的不透明度衰减技术，以加速选择过程而不影响剪枝效果。

Result: 与原始3DGS相比，该方法在15%预算下实现了超过0.6 dB的PSNR增益，为紧凑型3DGS建立了最先进的性能。

Conclusion: 通过引入自然选择启发的剪枝框架和不透明度衰减技术，本文方法有效解决了3DGS的存储和计算开销问题，实现了高斯基元的自主优化和剪枝，显著提升了渲染质量和模型紧凑性。
...
### [62] [Where Culture Fades: Revealing the Cultural Gap in Text-to-Image Generation](https://arxiv.org/abs/2511.17282)
...
TL;DR: 本文分析了多语言文本到图像（T2I）模型在文化一致性方面的不足，发现现有模型常产生文化中立或英语偏向的结果，原因在于文化相关表征激活不足。研究提出了一种探测方法来定位文化敏感神经元，并据此引入两种互补的对齐策略：推理时文化激活和层级定向文化增强，以提高文化一致性同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 多语言T2I模型已广泛应用，但在不同文化背景下，其输出存在差异。由于语言承载文化内涵，模型生成图像应保持跨语言文化一致性，而当前模型未能有效实现这一点。

Method: 研究首先对现有T2I模型进行了综合分析，发现其在多语言提示下常生成文化中立或英语偏向的结果。随后，通过分析指出问题并非源于文化知识缺失，而是文化相关表征激活不足。在此基础上，提出了一种探测方法，将文化敏感信号定位到少量固定层中的一小部分神经元。受此启发，提出了两种互补的对齐策略：1) 推理时文化激活，放大识别出的神经元而不微调主干网络；2) 层级定向文化增强，仅更新文化相关层。

Result: 分析表明，当前T2I模型在多语言提示下常产生文化中立或英语偏向的结果，且问题根源在于文化相关表征的激活不足而非知识缺失。通过提出的探测方法和两种对齐策略，在CultureBench上的实验显示，模型在文化一致性方面取得了显著且持续的改进，同时保持了图像的保真度和多样性。

Conclusion: 当前多语言T2I模型在文化一致性方面存在偏见，主要由于文化相关表征激活不足。通过定位文化敏感神经元并采用推理时文化激活和层级定向文化增强策略，可以有效提升模型在多语言提示下的文化一致性，同时不牺牲图像质量。
...
### [64] [VLM-Augmented Degradation Modeling for Image Restoration Under Adverse Weather Conditions](https://arxiv.org/abs/2511.16998)
*Qianyi Shao,Yuanfan Zhang,Renxiang Xiao,Liang Hu*

Main category: cs.CV

TL;DR: 本文提出了一种统一的记忆增强视觉语言恢复（MVLR）模型，通过结合轻量级编解码器、视觉语言模型（VLM）和隐式记忆库（IMB），在各种恶劣天气条件下恢复图像。


<details>
  <summary>Details</summary>
Motivation: 在雨、雾、雪等恶劣天气条件下实现可靠的视觉感知，对于自动驾驶和户外机器人来说是必要但极具挑战性的。

Method: MVLR模型包含一个轻量级编解码器主干、一个视觉语言模型（VLM）和一个隐式记忆库（IMB）。VLM通过思维链推理编码天气退化先验，IMB存储退化模式的连续潜在表示。VLM生成的先验查询IMB以检索细粒度的退化原型。这些原型通过动态交叉注意力机制与多尺度视觉特征自适应融合，以提高恢复精度和计算效率。

Result: 在四个恶劣天气基准测试中，MVLR在峰值信噪比（PSNR）和结构相似性指数（SSIM）方面均超越了单分支和专家混合基线模型。

Conclusion: MVLR在模型紧凑性和表达能力之间取得了实用平衡，适用于在各种户外条件下的实时部署。
...
### [65] [DepthFocus: Controllable Depth Estimation for See-Through Scenes](https://arxiv.org/abs/2511.16993)
*Junhong Min,Jimin Kim,Cheol-Hui Min,Minwook Kim,Youngpil Jeon,Minyong Choi*

Main category: cs.CV

TL;DR: 本文提出DepthFocus，一种可操控的Vision Transformer，将立体深度估计重新定义为意图驱动的控制。通过标量深度偏好，模型能动态聚焦于期望深度，实现复杂场景中的选择性感知，并在多层透明材质场景中达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射材料会造成分层模糊，使传统感知系统困惑。现有模型是被动地估计静态深度图，而人类则能主动转移焦点以感知期望深度。

Method: DepthFocus是一个可操控的Vision Transformer，它将立体深度估计定义为意图驱动的控制。模型根据标量深度偏好动态调整计算，以聚焦于目标深度。训练主要利用了一个新构建的包含50万个多层合成数据集，该数据集旨在捕捉多样化的透视效果。

Result: DepthFocus在BOOSTER等包含透明和反射物体的传统单深度基准测试中达到了最先进的性能。它还在新提出的真实和合成多深度数据集中定量展示了与意图对齐的估计。此外，它在未见过的透视场景中表现出强大的泛化能力。

Conclusion: DepthFocus是向主动和类人3D感知迈出的重要一步，展示了在复杂透视场景中的鲁棒性。
...
TL;DR: 针对多通道图像中Vision Transformers (ViTs) 存在的通道间注意力计算效率低下的问题，本文提出MoE-ViT，一种基于稀疏专家混合模型（MoE）的架构，通过轻量级路由器为每个图像块选择最相关的通道（专家），显著提升效率并保持或增强性能。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers (ViTs) 已成为视觉基础模型的主干，但在多通道领域（如细胞绘画、卫星图像）的优化尚待探索。现有方法在标记化时独立处理每个通道，导致注意力块中通道间比较的计算成本呈二次增长，带来巨大的FLOPs和高昂的训练成本。研究旨在解决ViTs在多通道图像中被忽视的跨通道注意力效率挑战。

Method: 本文提出MoE-ViT，一种用于ViTs中多通道图像的专家混合（Mixture-of-Experts, MoE）架构。该方法将每个通道视为一个专家，并采用一个轻量级路由器，为每个图像块（patch）选择最相关的专家（通道）进行注意力计算，从而避免建模所有通道交互。

Result: 在JUMP-CP和So2Sat真实世界数据集上的概念验证实验表明，MoE-ViT在不牺牲性能，甚至在某些情况下提升性能的同时，实现了显著的效率提升。

Conclusion: MoE-ViT是一种实用且有吸引力的多通道成像骨干网络，因为它在效率和性能方面都表现出色。
...
### [68] [RadioKMoE: Knowledge-Guided Radiomap Estimation with Kolmogorov-Arnold Networks and Mixture-of-Experts](https://arxiv.org/abs/2511.16986)
*Fupei Guo,Kerry Pan,Songyang Zhang,Yue Wang,Zhi Ding*

Main category: cs.CV

TL;DR: 本文提出RadioKMoE框架，结合柯尔莫哥洛夫-阿诺德网络（KAN）和专家混合模型（MoE），用于无线电图估计，通过KAN预测粗略覆盖图，再由MoE精细化，以提高复杂环境下的估计精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 日益复杂的无线电传播行为和环境对无线电图估计（RME）提出了巨大挑战，需要更强大的空间信号传播和覆盖知识。

Method: 提出知识引导的RME框架RadioKMoE。首先，设计KAN模块预测初始粗略覆盖图，利用KAN擅长近似物理模型和全局无线电传播模式的优势。然后，结合初始粗略图和环境信息，驱动MoE网络进行精确的无线电图估计。MoE模块包含专注于不同无线电图模式的专家网络，以改善局部细节并保持全局一致性。

Result: 在多频段和单频段RME实验中，所提出的RadioKMoE在无线电图估计方面表现出更高的准确性和鲁棒性。

Conclusion: RadioKMoE通过整合KAN和MoE，有效解决了复杂环境下无线电图估计的挑战，显著提升了估计精度和鲁棒性。
...
### [69] [RoomPlanner: Explicit Layout Planner for Easier LLM-Driven 3D Room Generation](https://arxiv.org/abs/2511.17048)
*Wenzhuo Sun,Mingjian Liang,Wenxuan Song,Xuelian Cheng,Zongyuan Ge*

Main category: cs.CV

TL;DR: RoomPlanner是首个全自动3D房间生成框架，只需短文本输入即可创建逼真的室内场景，无需手动布局，并优化了渲染速度和质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法创建逼真室内场景通常需要手动布局设计或全景图像指导，过程繁琐。本文旨在通过仅短文本输入，实现无痛且全自动地生成高质量3D室内场景。

Method: 该方法首先引入分层语言驱动代理规划器，将模糊文本提示解析为详细场景描述（包含物体和背景的空间及语义属性）。然后，这些描述用于初始化3D点云。接着，通过两种布置约束迭代优化空间布局，确保无碰撞且可访问。最后，在渲染阶段，提出AnyReach采样策略用于相机轨迹，并结合Interval Timestep Flow Sampling (ITFS)策略，高效优化粗糙的3D高斯场景表示。

Result: RoomPlanner将总生成时间缩短至30分钟以内，能够生成几何合理的3D室内场景。实验证明，该方法在渲染速度和视觉质量上均超越了现有方法，同时保持了可编辑性。

Conclusion: RoomPlanner成功开发了一个完全自动化的3D房间生成框架，仅通过文本输入即可高效、高质量地创建逼真、合理的室内场景，显著提升了生成效率和场景质量，并提供了可编辑性。
...
TL;DR: 遥感领域的基础模型（RSFM）选择困难，本文提出了RSFM数据库（RS-FMD）和首个基于LLM的自动化RSFM选择智能体REMSA，并设计了专家验证的基准测试，REMSA表现优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 遥感领域的基础模型（FMs）日益增多，但由于文档分散、格式异构和部署限制多样，选择合适的遥感基础模型（RSFM）变得困难。

Method: 本文引入了RSFM数据库（RS-FMD），这是一个结构化资源，涵盖了150多个RSFM。在此基础上，提出了REMSA，这是第一个基于大型语言模型（LLM）的智能体，用于从自然语言查询中自动选择RSFM。REMSA能够解释用户需求、解决缺失约束、利用上下文学习对候选模型进行排序，并提供透明的理由。此外，还提出了一个包含75个专家验证的遥感查询场景（900种配置）的基准测试。

Result: REMSA在RSFM选择方面优于包括朴素智能体、密集检索和非结构化RAG-based LLM在内的多个基线模型。它完全基于公开可用的元数据运行，不访问私有或敏感数据。

Conclusion: RS-FMD和REMSA为自动化、透明的遥感基础模型选择提供了一个有效的解决方案，显著简化了用户在复杂遥感任务中选择合适模型的流程。
...
### [72] [Vision Language Models are Confused Tourists](https://arxiv.org/abs/2511.17004)
*Patrick Amadeus Irawan,Ikhlasul Akmal Hanif,Muhammad Dehan Al Kautsar,Genta Indra Winata,Fajri Koto,Alham Fikri Aji*

Main category: cs.CV

TL;DR: 尽管文化维度对评估视觉语言模型（VLM）至关重要，但现有评估忽视了多文化线索并存的场景。本文引入了ConfusedTourist套件，发现VLM在面对扰动的地理文化线索时表现出严重的脆弱性，准确率大幅下降，且模型注意力会系统性地转向干扰线索，凸显了对更具文化鲁棒性的多模态理解的迫切需求。


<details>
  <summary>Details</summary>
Motivation: 文化维度是评估VLM的关键方面，但它们在多样文化输入下保持稳定性的能力尚未得到充分测试，这对于支持多样性和多文化社会至关重要。现有评估通常只关注每张图像中的单一文化概念，忽略了多种、可能不相关的文化线索并存的场景。

Method: 引入了ConfusedTourist，一个新颖的文化对抗鲁棒性套件，旨在评估VLM在受扰动地理文化线索下的稳定性。实验使用了简单的图像堆叠扰动及其基于图像生成的变体。此外，还进行了可解释性分析，以理解失败的原因。

Result: 实验揭示了VLM的严重脆弱性：在简单的图像堆叠扰动下，准确率大幅下降，且在基于图像生成的变体下情况更糟。可解释性分析表明，这些失败源于模型注意力系统性地转向分散注意力的线索，使其偏离了预期焦点。

Conclusion: 这些发现强调了一个关键挑战：视觉文化概念混合会严重损害即使是最先进的VLM，这突显了对更具文化鲁棒性的多模态理解的迫切需求。
...
### [73] [Preventing Shortcut Learning in Medical Image Analysis through Intermediate Layer Knowledge Distillation from Specialist Teachers](https://arxiv.org/abs/2511.17421)
...
TL;DR: ReCoVAD是一个受人类神经系统启发的视频异常检测框架，利用双路径选择性帧处理，实现了在仅处理少量帧的情况下，基于大型预训练模型的免训练VAD的SOTA性能，显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于大型预训练模型的免训练视频异常检测（VAD）方法通常依赖于密集的帧级推理，导致高计算成本和延迟。这引发了一个基本问题：在使用强大的预训练模型时，密集推理是否真正必要？

Method: ReCoVAD受人类神经系统的双重反射和意识路径启发，采用选择性帧处理来减少冗余计算。它包含两个核心路径：(i) 反射路径：使用轻量级CLIP模块融合视觉特征和原型提示，生成决策向量，查询过去帧和异常分数的动态记忆以实现快速响应；(ii) 意识路径：采用中等规模的视觉-语言模型为新帧生成文本事件描述和精细异常分数。它持续更新记忆和原型提示，同时集成的LLM定期审查累积的描述以识别未见异常、纠正错误并优化原型。

Result: ReCoVAD在UCF-Crime和XD-Violence数据集上实现了SOTA的免训练性能，同时仅处理了之前方法所用帧数的28.55%和16.04%。

Conclusion: 稀疏推理足以实现有效的基于大型模型的视频异常检测，证明了在VAD系统中，密集推理并非绝对必要。
...
### [75] [PathAgent: Toward Interpretable Analysis of Whole-slide Pathology Images via Large Language Model-based Agentic Reasoning](https://arxiv.org/abs/2511.17052)
*Jingyun Chen,Linghan Cai,Zhikang Wang,Yi Huang,Songhan Jiang,Shenjin Huang,Hongpeng Wang,Yongbing Zhang*

Main category: cs.CV

TL;DR: PathAgent是一个基于大型语言模型（LLM）的无训练代理框架，通过模拟病理学家迭代、证据驱动的推理过程来分析全玻片图像（WSI），提供可解释的诊断辅助。


<details>
  <summary>Details</summary>
Motivation: 现有计算管道在分析WSI时缺乏明确的推理轨迹，导致预测不透明且难以证明，无法有效模拟病理学家动态缩放、重新聚焦和自我纠正以收集证据的过程。

Method: PathAgent是一个无训练、基于LLM的代理框架，它通过以下模块模拟人类专家的反射式、逐步分析方法：Navigator模块自主探索WSI并精确定位微区域；Perceptor模块提取形态学视觉线索；Executor模块将这些发现整合到不断演变的自然语言轨迹中，形成明确的思维链（chain-of-thought）。

Result: PathAgent在五个具有挑战性的数据集上表现出强大的零样本泛化能力，在开放式和受限视觉问答任务中均超越了特定任务基线。此外，与人类病理学家的协作评估证实了PathAgent作为透明且具有临床基础的诊断助手的潜力。

Conclusion: PathAgent通过模拟人类专家的反思性、逐步分析方法，成功实现了WSI的迭代、证据驱动推理，提供了完全可解释的诊断预测，并被证实具有强大的泛化能力和作为透明、临床基础诊断助手的应用前景。
...
### [76] [RL-AD-Net: Reinforcement Learning Guided Adaptive Displacement in Latent Space for Refined Point Cloud Completion](https://arxiv.org/abs/2511.17054)
...
### [77] [REArtGS++: Generalizable Articulation Reconstruction with Temporal Geometry Constraint via Planar Gaussian Splatting](https://arxiv.org/abs/2511.17059)
*Di Wu,Liu Liu,Anran Huang,Yuyan Liu,Qiaoyu Jun,Shaofan Liu,Liangtu Song,Cewu Lu*

Main category: cs.CV

TL;DR: REArtGS++ 提出一种新方法，通过解耦螺旋运动、平面高斯splatting和时间几何约束，改进了可动对象的部分级表面重建和关节参数估计，解决了REArtGS在复杂关节和未知状态下的不足。


<details>
  <summary>Details</summary>
Motivation: REArtGS在螺纹关节或多部件对象上表现不佳，并且缺乏对未见状态的几何约束。本文旨在解决这些限制，实现更通用化的可动对象重建。

Method: 该方法首先为每个关节建模解耦的螺旋运动，无需预设类型。然后，通过部件运动混合，共同优化部件感知的高斯splatting和关节参数。为引入时间连续的几何约束，REArtGS++鼓励高斯splatting呈平面状，并提出通过泰勒一阶展开在平面法线和深度之间进行时间一致性正则化。

Result: 在合成和真实世界可动对象上的大量实验表明，REArtGS++在通用化的部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++是一种优越的新方法，通过结合时间几何约束和平面高斯splatting，显著提升了可动对象重建的通用性、精度和对复杂关节的处理能力。
...
### [78] [PEGS: Physics-Event Enhanced Large Spatiotemporal Motion Reconstruction via 3D Gaussian Splatting](https://arxiv.org/abs/2511.17116)
*Yijun Xu,Jingrui Zhang,Hongyi Liu,Yuhan Chen,Yuanyang Wang,Qingyao Guo,Dingwen Wang,Lei Yu,Chu He*

Main category: cs.CV

TL;DR: PEGS是一个结合物理先验、事件流增强和3D高斯泼溅的框架，用于去模糊和恢复大时空尺度下的刚体运动，并在各种场景中表现出卓越的性能。


<details>
  <summary>Details</summary>
Motivation: 在大时空尺度下重建刚体运动是一个具有挑战性的任务，主要原因是建模范式的局限性、严重的运动模糊以及物理一致性不足。

Method: 本文提出了PEGS框架，该框架将物理先验与事件流增强整合到3D高斯泼溅管线中，以实现去模糊的目标聚焦建模和运动恢复。方法包括：1) 引入了一个内聚的三级监督方案，通过加速度约束强制物理合理性，利用事件流提供高时间分辨率指导，并采用卡尔曼正则化器融合多源观测。2) 设计了一种运动感知模拟退火策略，根据实时运动状态自适应地调度训练过程。3) 贡献了第一个针对自然、快速刚体运动的RGB-事件配对数据集。

Result: 实验表明，与主流动态方法相比，PEGS在重建大时空尺度运动方面表现出卓越的性能。

Conclusion: PEGS通过整合物理先验、事件流增强和创新的训练策略，成功解决了大时空尺度下刚体运动重建的挑战，并取得了显著的性能提升。
...
### [79] [ChainV: Atomic Visual Hints Make Multimodal Reasoning Shorter and Better](https://arxiv.org/abs/2511.17106)
*Yuan Zhang,Ming Lu,Junwen Pan,Tao Huang,Kuan Cheng,Qi She,Shanghang Zhang*

Main category: cs.CV

TL;DR: ChainV是一个动态整合视觉提示的多模态推理框架，旨在减少冗余的自我反思，使推理更短更好，显著提高准确性和效率，尤其在数学密集型基准上表现突出。


<details>
  <summary>Details</summary>
Motivation: 当前多模态推理模型在生成长推理链时存在冗余的自我反思。现有的无训练CoT压缩方法依赖静态视觉参考，对多模态推理增益有限，因此需要一种能动态利用视觉信息的方法来优化推理过程。

Method: ChainV框架动态整合视觉提示：首先基于前一步推理进行粗略视觉区域选择，然后根据平均注意力强度识别最具代表性的原子视觉提示进行细化。此外，引入基于一致性的评估机制来评估提示的可靠性，指导模型自适应调整自我反思水平。最后，将所选视觉提示的像素坐标及其可靠性通过伯努利随机过程融入推理。

Result: 实验表明，ChainV显著提高了推理准确性和效率，尤其在视觉提示对多步符号推理至关重要的数学密集型基准上。例如，在MIMO-VL-RL框架下，ChainV在MathVista上实现了2.3%的改进，同时将推理延迟降低了51.4%，输出token长度缩短了24.5%。

Conclusion: ChainV通过动态整合视觉提示，有效解决了多模态推理中冗余自我反思的问题，使得推理过程更短、更优，尤其在需要精细视觉-符号交互的复杂任务中展现出卓越的性能。
...
### [81] [DiffRefiner: Coarse to Fine Trajectory Planning via Diffusion Refinement with Semantic Interaction for End to End Autonomous Driving](https://arxiv.org/abs/2511.17150)
*Liuhan Yin,Runkun Ju,Guodong Guo,Erkang Cheng*

Main category: cs.CV

TL;DR: DiffRefiner是一种两阶段轨迹预测框架，结合判别式提案生成和扩散模型精炼，实现了最先进的自动驾驶轨迹预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶轨迹预测方法，无论是判别式（预测固定轨迹集）还是生成式（如扩散模型，依赖去噪锚点或随机噪声），都存在改进空间。研究旨在提供更灵活且更准确的轨迹预测。

Method: 本文提出了DiffRefiner，一个新颖的两阶段轨迹预测框架。第一阶段使用基于Transformer的提案解码器，通过预定义轨迹锚点从传感器输入回归生成粗略轨迹预测。第二阶段应用扩散精炼器，迭代去噪并精炼这些初始预测。此外，设计了一个细粒度去噪解码器以增强场景依从性，实现更准确的轨迹预测。

Result: DiffRefiner在NAVSIM v2上达到87.4 EPDMS，在Bench2Drive上达到87.1 DS和71.4 SR，在两个公共基准测试中均创下新纪录，实现了最先进的性能。消融研究也验证了每个组件的有效性。

Conclusion: DiffRefiner通过将判别式轨迹提案模块与生成式扩散精炼过程相结合，并辅以细粒度去噪解码器增强场景依从性，显著提升了自动驾驶轨迹预测的准确性和灵活性，达到了最先进的水平。
...
### [82] [Learning to Look Closer: A New Instance-Wise Loss for Small Cerebral Lesion Segmentation](https://arxiv.org/abs/2511.17146)
*Luc Bouteille,Alexander Jaus,Jens Kleesiek,Rainer Stiefelhagen,Lukas Heine*

Main category: cs.CV

TL;DR: 本文提出CC-DiceCE损失函数，旨在解决医学图像中小病灶欠分割问题，结果显示它能提高召回率，且在多数据集上通常优于现有blob损失。


<details>
  <summary>Details</summary>
Motivation: 传统的医学图像分割损失函数（如Dice）倾向于欠分割小病灶，因为它们相对体积小，对整体损失的贡献微不足道，导致难以有效检测和分割这些重要的小目标。

Method: 研究引入了基于CC-Metrics框架的CC-DiceCE损失函数。该函数与现有blob损失进行了比较，并以DiceCE作为基线，所有实验均在nnU-Net框架下进行基准测试。

Result: CC-DiceCE损失提高了病灶检测率（召回率），同时对分割性能的影响极小或没有影响，但代价是略微增加了假阳性。多数据集研究进一步表明，CC-DiceCE通常优于blob损失。

Conclusion: CC-DiceCE损失函数能有效解决医学图像中小病灶的欠分割问题，显著提高了召回率，并且在性能上优于blob损失，是处理此类问题的有效方法，尽管需要权衡少量假阳性。
...
### [83] [Investigating self-supervised representations for audio-visual deepfake detection](https://arxiv.org/abs/2511.17181)
*Dragos-Alexandru Boldisor,Stefan Smeu,Dan Oneata,Elisabeta Oneata*

Main category: cs.CV

TL;DR: 本文系统评估了自监督表示在音视频深度伪造检测中的潜力，发现其能捕获相关且互补的信息，但跨数据集泛化能力不足。


<details>
  <summary>Details</summary>
Motivation: 自监督表示在视觉和语音任务中表现出色，但在音视频深度伪造检测领域的潜力尚未充分探索，尤其缺乏对不同模态和领域进行系统评估。

Method: 研究系统地评估了自监督特征在不同模态（音频、视频、多模态）和领域（唇部运动、通用视觉内容）中的表现。评估维度包括检测有效性、编码信息的解释性以及跨模态互补性。

Result: 大多数自监督特征捕获了与深度伪造相关且互补的信息，模型主要关注语义有意义的区域。然而，这些特征在不同数据集之间无法可靠泛化，这种泛化失败可能源于数据集特性而非特征本身的问题。

Conclusion: 自监督表示在深度伪造检测中展现了学习有意义模式的潜力，但实现鲁棒的跨领域性能仍然是一个基本挑战。
...
### [84] [SPAGS: Sparse-View Articulated Object Reconstruction from Single State via Planar Gaussian Splatting](https://arxiv.org/abs/2511.17092)
*Di Wu,Liu Liu,Xueyu Yuan,Qiaoyu Jun,Wenxiao Chen,Ruilong Yan,Yiming Tang,Liangtu Song*

Main category: cs.CV

TL;DR: 该论文提出了一种通过平面高斯泼溅进行关节物体重建的框架，仅使用单状态稀疏视图RGB图像，实现了高保真度的部件级表面重建。


<details>
  <summary>Details</summary>
Motivation: 现有的关节物体重建方法通常需要昂贵的多阶段和多视图输入。本研究旨在解决这一限制，仅利用单状态稀疏视图RGB图像进行重建。

Method: 该方法首先引入高斯信息场来感知最佳稀疏视点；然后将3D高斯压缩为平面高斯，以促进法线和深度的准确估计。平面高斯通过深度平滑正则化和少样本扩散以粗到精的方式进行优化。此外，为每个高斯基元引入部件分割概率，并通过反投影渲染的部件分割掩码进行更新。

Result: 广泛的实验结果表明，与现有方法相比，该方法在合成和真实世界数据上都实现了更高保真度的部件级表面重建。

Conclusion: 该框架成功地从稀疏视图RGB图像中重建了关节物体，并获得了高质量的部件级表面，解决了传统方法对昂贵输入的需求。
...
### [85] [Diversity Has Always Been There in Your Visual Autoregressive Models](https://arxiv.org/abs/2511.17074)
...
### [86] [Bridging Visual Affective Gap: Borrowing Textual Knowledge by Learning from Noisy Image-Text Pairs](https://arxiv.org/abs/2511.17103)
...
### [87] [Off the Planckian Locus: Using 2D Chromaticity to Improve In-Camera Color](https://arxiv.org/abs/2511.17133)
*SaiKiran Tedla,Joshua E. Little,Hakki Can Karaimer,Michael S. Brown*

Main category: cs.CV

TL;DR: 该论文提出了一种改进的相机内色彩映射方法，通过从1D相关色温(CCT)转向2D色度空间，并利用轻量级多层感知器(MLP)进行映射，显著提高了LED等非普朗克光源下的色彩准确性，并支持实时部署。


<details>
  <summary>Details</summary>
Motivation: 传统的相机内色彩映射依赖于基于1D相关色温(CCT)的插值，但现代LED照明常常偏离普朗克轨迹，使得传统的1D CCT方法在表征这些非普朗克光源时存在局限性，导致色彩准确性不足。

Method: 该研究将照明表征从1D CCT（普朗克轨迹上）转换为2D色度空间（可偏离普朗克轨迹）。此外，用一个轻量级多层感知器（MLP）取代了传统的CCT插值，该MLP利用2D色度特征进行色彩映射。通过包含代表性LED光源的灯箱校准程序来训练MLP。

Result: 结果显示，过渡到2D色度空间可以提高各种映射方法的色彩准确性。所提出的方法在LED照明场景中平均将角度再现误差降低了22%，同时保持了与传统光源的向后兼容性，能够处理多光源场景，并支持实时相机内部署，计算成本可忽略不计。

Conclusion: 通过从1D CCT转向2D色度空间并采用基于MLP的映射方法，本研究显著提高了LED等非普朗克光源下的色彩准确性，提供了一种鲁棒、兼容且计算效率高的相机内色彩映射解决方案。
...
### [88] [Real Noise Decoupling for Hyperspectral Image Denoising](https://arxiv.org/abs/2511.17196)
*Yingkai Zhang,Tao Zhang,Jing Nie,Ying Fu*

Main category: cs.CV

TL;DR: 本文提出了一种多阶段噪声解耦框架，将高光谱图像（HSI）中的复杂噪声分解为显式建模和隐式建模两部分，并通过分阶段学习策略进行去噪，有效提升了真实HSI的去噪效果。


<details>
  <summary>Details</summary>
Motivation: 现有噪声建模方法难以准确模拟真实高光谱图像中复杂的噪声分布，从而限制了去噪网络的有效性。

Method: 该方法提出一个多阶段噪声解耦框架：1) 将复杂噪声分解为显式建模和隐式建模两部分。2) 对于显式建模噪声，利用现有噪声模型生成配对数据预训练去噪网络，赋予其先验知识。3) 对于隐式建模噪声，引入一个高频小波引导网络，利用预训练模块的先验知识自适应提取高频特征进行去噪。4) 采用多阶段学习策略，包括独立的预训练和联合微调，以优化整个框架并减轻误差累积。

Result: 在公共数据集和自采集数据集上的大量实验表明，所提出的框架优于最先进的方法，能有效处理复杂的真实世界噪声，并显著提升了高光谱图像的质量。

Conclusion: 通过将复杂噪声解耦为显式和隐式部分，并结合多阶段学习策略，本框架能有效应对真实高光谱图像中的复杂噪声，显著提高图像质量。
...
### [89] [One-Step Diffusion Transformer for Controllable Real-World Image Super-Resolution](https://arxiv.org/abs/2511.17138)
...
### [90] [Scaling Self-Supervised and Cross-Modal Pretraining for Volumetric CT Transformers](https://arxiv.org/abs/2511.17209)
*Cris Claessens,Christiaan Viviers,Giacomo D'Amicantonio,Egor Bondarev,Fons van der Sommen*

Main category: cs.CV

TL;DR: SPECTRE是一个完全基于Transformer的CT基础模型，通过自监督和视觉-语言预训练学习通用的CT表示，并在多个基准测试中超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 体积CT数据存在极端token扩展、几何各向异性以及临床监督弱或嘈杂等独特挑战，使得标准Transformer和对比学习方法难以直接应用，需要一种能有效处理这些挑战并学习通用CT表示的模型。

Method: SPECTRE采用可扩展的3D Vision Transformer架构，结合自监督和视觉-语言预训练策略。它联合优化一个用于高分辨率体积特征提取的局部Transformer和一个用于全扫描上下文建模的全局Transformer，以解决大规模3D注意力计算难题。预训练结合了DINO风格的自蒸馏和基于SigLIP的视觉-语言对齐（使用配对的放射学报告），且仅使用公开可用的CT数据集进行训练。

Result: SPECTRE在多个CT基准测试中，无论是在零样本还是微调设置下，都持续优于先前的CT基础模型，证明了其高性能和泛化能力。

Conclusion: SPECTRE是一个可扩展、开放且完全基于Transformer的3D医学影像基础模型，能够学习几何一致且临床有意义的CT表示，为3D医学影像领域树立了新标杆。
...
### [91] [UI-Styler: Ultrasound Image Style Transfer with Class-Aware Prompts for Cross-Device Diagnosis Using a Frozen Black-Box Inference Network](https://arxiv.org/abs/2511.17155)
*Nhat-Tuong Do-Tran,Ngoc-Hoang-Lam Le,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 本文提出UI-Styler，一种新型的超声图像类别感知风格迁移框架，通过纹理模式匹配和类别感知提示策略，有效解决跨设备超声图像的域偏移问题，提高下游诊断模型的性能。


<details>
  <summary>Details</summary>
Motivation: 超声图像因采集设备不同而存在域偏移，导致固定黑盒推理模型在重用时性能下降。现有的非配对图像转换（UIT）方法常忽略域适应过程中的类别特异性语义对齐，可能导致内容-类别映射错误，从而影响诊断准确性。

Method: 本文提出了UI-Styler框架。它利用一种模式匹配机制，将目标图像中的纹理模式转移到源图像上，同时保留源图像的结构内容。此外，引入了一种由目标域伪标签引导的类别感知提示策略，以强制实现与诊断类别精确的语义对齐。

Result: 在超声跨设备任务上的大量实验表明，UI-Styler始终优于现有UIT方法，在分布距离和下游任务（如分类和分割）中均达到了最先进的性能。

Conclusion: UI-Styler通过其新颖的类别感知风格迁移方法，成功解决了超声图像的域偏移问题，确保了语义对齐，显著提升了下游诊断任务的准确性。
...
### [92] [A Multi-Stage Optimization Framework for Deploying Learned Image Compression on FPGAs](https://arxiv.org/abs/2511.17135)
*Jiaxun Fang,Li Chen*
...
### [93] [Dual-domain Adaptation Networks for Realistic Image Super-resolution](https://arxiv.org/abs/2511.17217)
*Chaowei Fang,Bolin Fu,De Cheng,Lechao Cheng,Guanbin Li*

Main category: cs.CV

TL;DR: 本文提出了一种双域自适应网络（Dual-domain Adaptation Networks），能有效将预训练的图像超分辨率（SR）模型从模拟数据集适应到真实世界数据集，以解决真实世界SR中数据稀缺和复杂退化的问题。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像超分辨率面临复杂退化模式和真实世界低分辨率-高分辨率（LR-HR）数据有限的挑战，这影响了模型学习基本图像特征的能力。虽然预训练的SR模型提供了有价值的先验知识，但它们需要有效的方法来适应真实世界数据。

Method: 该方法引入了双域自适应网络。首先，通过选择性更新预训练模型的参数并利用低秩自适应技术调整冻结参数，建立了空间域自适应策略。其次，为了恢复高频分量，模型集成了一个频率域自适应分支，该分支结合输入光谱数据和空间域骨干的中间特征来推断高分辨率频率图，从而增强SR结果。

Result: 在RealSR、D2CRealSR和DRealSR等公共真实图像SR基准上的实验评估表明，所提出的方法优于现有最先进的模型。

Conclusion: 该研究成功地提出了一种新颖的双域自适应网络，能够高效地将预训练的图像SR模型从模拟数据集适应到真实世界数据集，并在真实图像SR任务中取得了超越现有先进模型的性能。
...
### [94] [FireScope: Wildfire Risk Prediction with a Chain-of-Thought Oracle](https://arxiv.org/abs/2511.17171)
*Mario Markov,Stefan Maria Ailuro,Luc Van Gool,Konrad Schindler,Danda Pani Paudel*

Main category: cs.CV

TL;DR: 本文介绍了FireScope-Bench数据集和FireScope框架，该框架利用视觉语言模型（VLM）和强化学习，通过结合视觉、气候和地理因素，生成具有可解释推理轨迹的火灾风险栅格图，并在跨大陆（美国训练，欧洲测试）的泛化性上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 预测野火风险是一个推理密集型的空间问题，需要整合视觉、气候和地理因素来推断连续的风险图。现有方法缺乏因果推理和多模态理解能力，难以实现可靠的泛化。

Method: 研究引入了FireScope-Bench，一个大规模数据集和基准，它将Sentinel-2图像和气候数据与专家定义的风险栅格图（美国）以及真实野火事件（欧洲）相结合，用于跨大陆评估。在此基础上，提出了FireScope，一个基于VLM的“推理到生成”框架，通过强化学习和视觉监督进行学习，预测带有互补推理轨迹的风险栅格图。

Result: FireScope在美国训练并在欧洲测试时，取得了显著的性能提升。专家反馈和自动化分析证实其推理轨迹是忠实且有语义意义的。研究结果表明，推理可以为栅格预测模型提供基础，从而提高泛化能力和可解释性。这是首个证明基于语言的推理可以改善视觉生成泛化能力、提出可应用于跨大陆的高分辨率野火风险模型，并实现多模态火灾风险模型鲁棒跨大陆泛化系统性研究的框架。

Conclusion: 推理能够为栅格预测模型提供基础，从而显著提高模型的泛化能力和可解释性。FireScope-Bench数据集有望成为推动推理驱动、可解释和可泛化空间建模的基础。
...
### [95] [Navigating in the Dark: A Multimodal Framework and Dataset for Nighttime Traffic Sign Recognition](https://arxiv.org/abs/2511.17183)
...
### [96] [PostCam: Camera-Controllable Novel-View Video Generation with Query-Shared Cross-Attention](https://arxiv.org/abs/2511.17185)
*Yipeng Chen,Zhichao Ye,Zhenzhou Fang,Xinyu Chen,Xiaoyu Zhang,Jialing Liu,Nan Wang,Haomin Liu,Guofeng Zhang*

Main category: cs.CV

TL;DR: PostCam是一个用于动态场景中新视角视频生成和捕获后摄像机轨迹编辑的框架，通过创新的运动注入策略，显著提高了摄像机控制精度和视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频重捕获方法在摄像机运动注入策略上存在不足，导致摄像机控制精度受限，且难以保留源视频的精细视觉细节。

Method: PostCam引入了一个查询共享的交叉注意力模块，将6自由度摄像机姿态和2D渲染视频帧两种控制信号融合到共享特征空间中的统一表示，以提取深层运动线索。此外，采用了两阶段训练策略：首先从姿态输入学习粗略的摄像机控制，然后结合视觉信息来精炼运动精度和视觉保真度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在摄像机控制精度和视图一致性方面超越了现有最先进方法20%以上，同时实现了最高的视频生成质量。

Conclusion: PostCam通过其独特的方法，实现了更准确和灵活的运动操作，显著提升了新视角视频生成的控制精度和视觉质量，解决了现有方法的痛点。
...
### [97] [SpatialGeo:Boosting Spatial Reasoning in Multimodal LLMs via Geometry-Semantics Fusion](https://arxiv.org/abs/2511.17308)
*Jiajie Guo,Qingpeng Zhu,Jin Zeng,Xiaolong Wu,Changyong He,Weida Wang*

Main category: cs.CV

TL;DR: 本文提出SpatialGeo，一种基于几何与语义特征分层融合的新型视觉编码器，旨在增强多模态大语言模型（MLLMs）的空间推理能力和接地能力，并在空间推理任务中取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数多模态大语言模型（MLLMs）在三维空间中解释和推断空间排列的能力有限，主要原因是其视觉编码器（如CLIP）的嵌入存在信息丢失，仅限于实例级语义特征，导致空间模糊性。

Method: 本文提出SpatialGeo，一个新颖的视觉编码器，通过几何和语义特征的分层融合生成空间感知视觉嵌入。具体方法是：通过分层适配器，利用仅视觉的自监督学习获得的几何特征来补充CLIP的语义特征，从而增强空间感知能力。网络使用预训练的LLaVA模型进行高效训练，并采用随机特征丢弃进行优化，以避免过度依赖CLIP编码器。

Result: 实验结果表明，SpatialGeo提高了空间推理任务的准确性，在SpatialRGPT-Bench上将现有最佳模型至少提升了8.0%，同时在推理过程中内存成本降低了约50%。

Conclusion: SpatialGeo通过融合几何和语义特征，有效解决了多模态大语言模型在三维空间推理方面的局限性，显著提升了模型性能和推理效率。
...
### [98] [VLA-4D: Embedding 4D Awareness into Vision-Language-Action Models for SpatioTemporally Coherent Robotic Manipulation](https://arxiv.org/abs/2511.17199)
...
### [99] [A Little More Like This: Text-to-Image Retrieval with Vision-Language Models Using Relevance Feedback](https://arxiv.org/abs/2511.17255)
*Bulat Khaertdinov,Mirela Popa,Nava Tintarev*

Main category: cs.CV

TL;DR: 本文提出了一种受传统文本搜索启发的机制——相关性反馈，以在推理时提高大型视觉-语言模型（VLM）的检索性能，并引入了四种反馈策略，显著提升了检索效果。


<details>
  <summary>Details</summary>
Motivation: 大型VLM虽然支持直观的视觉搜索，但其性能提升通常需要微调或扩展到更大的模型。研究旨在探索一种在推理阶段无需微调或模型扩展即可提高检索性能的方法，并与现有微调VLM兼容。

Method: 本文评估了四种VLM检索的相关性反馈策略：1. 修正经典的伪相关性反馈（PRF），基于顶部结果优化查询嵌入。2. 提出生成式相关性反馈（GRF），利用合成标题进行查询优化。3. 引入注意力反馈摘要器（AFS），一个定制的基于Transformer的模型，整合来自相关项目的多模态细粒度特征。4. 通过使用真实标题模拟显式反馈，作为性能上限基线。

Result: 在Flickr30k和COCO数据集上，GRF、AFS和显式反馈策略使小型VLM的MRR@5检索性能提高了3-5%，大型VLM提高了1-3%，优于无反馈的检索。AFS与显式反馈类似，能有效缓解查询漂移，并且在迭代、多轮检索设置中比GRF更鲁棒。

Conclusion: 相关性反馈能够持续增强VLM的检索性能，为交互式和自适应视觉搜索开辟了新的机会。AFS尤其在多轮检索中展现出良好的鲁棒性和查询漂移缓解能力。
...
### [100] [Refracting Reality: Generating Images with Realistic Transparent Objects](https://arxiv.org/abs/2511.17340)
*Yue Yin,Enze Tao,Dylan Campbell*

Main category: cs.CV

TL;DR: 生成模型在合成透明物体，特别是折射方面表现不佳。本文提出一种新方法，通过在生成过程中应用斯涅尔定律并结合全景图像，生成更符合光学物理规律的折射图像。


<details>
  <summary>Details</summary>
Motivation: 生成图像模型在合成透明物体时表现显著不佳，尤其是在处理折射现象时，因为它们未能充分理解光学定律，导致无法准确渲染折射物体。

Method: 该研究在生成轨迹的每一步，利用斯涅尔折射定律对物体边界内外的像素进行扭曲和合并，以实现像素同步。对于那些未直接观察到但通过折射或反射可见的表面，通过将主图像与以物体为中心生成的第二张全景图像进行同步（使用相同的扭曲和合并过程），来恢复它们的视觉效果。

Result: 该方法生成的图像在光学上更具合理性，并且更好地遵守了物理约束。

Conclusion: 本研究通过将物理光学定律（斯涅尔定律）集成到生成过程中，并结合全景图像信息，有效地解决了生成模型在准确渲染透明物体折射方面的挑战，生成了更具物理真实感的图像。
...
### [101] [Loomis Painter: Reconstructing the Painting Process](https://arxiv.org/abs/2511.17344)
*Markus Pobitzer,Chang Liu,Chenyi Zhuang,Teng Long,Bin Ren,Nicu Sebe*

Main category: cs.CV

TL;DR: 该研究提出了一个统一框架，用于生成交互式、个性化的多媒体绘画过程，通过语义驱动的风格控制和逆向绘画训练策略，实现了跨媒体风格的连贯纹理演变和过程迁移，并能模拟人类艺术创作的进程。


<details>
  <summary>Details</summary>
Motivation: 现有的视频教程缺乏互动性和个性化，而当前的生成模型在跨媒体泛化、时间和结构一致性方面存在不足，难以忠实再现人类的创作流程。

Method: 提出一个统一的多媒体绘画过程生成框架，采用语义驱动的风格控制机制，将多种媒体嵌入到扩散模型的条件空间中，并使用跨媒体风格增强来实现一致的纹理演变和过程迁移。此外，采用逆向绘画训练策略确保生成过程平滑且符合人类习惯。还构建了一个大规模真实绘画过程数据集，并提出了感知距离曲线（PDP）来量化建模创作序列。

Result: 在LPIPS、DINO和CLIP指标上取得了强大的结果，验证了跨媒体一致性、时间连贯性和最终图像的保真度。感知距离曲线（PDP）能够定量建模创作序列（构图、色彩块、细节细化），反映了人类的艺术进展。

Conclusion: 该框架有效解决了现有绘画教程和生成模型的局限性，能够生成连贯、符合人类习惯的多媒体绘画过程，并通过PDP曲线为量化建模艺术创作序列提供了新方法。
...
### [102] [Equivariant-Aware Structured Pruning for Efficient Edge Deployment: A Comprehensive Framework with Adaptive Fine-Tuning](https://arxiv.org/abs/2511.17242)
*Mohammed Alnemari*

Main category: cs.CV

TL;DR: 本文提出了一种结合群等变卷积神经网络（G-CNNs）和等变感知结构化剪枝的新框架，旨在为资源受限环境生成紧凑、变换不变的模型，同时保持几何鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的环境中，需要构建既紧凑又对几何变换具有不变性的模型。传统的剪枝方法可能破坏等变网络的几何鲁棒性，因此需要一种能同时实现模型压缩和保持等变特性的方法。

Method: 该方法结合了G-CNNs（通过e2cnn库实现C4循环群的旋转等变性）和等变感知的结构化剪枝。剪枝策略分析e2cnn层结构并对全连接组件应用神经元级别的剪枝以保持等变特性。为缓解精度下降，引入了自适应微调机制，当精度下降超过2%时自动触发，并结合早停和学习率调度。整个框架还包括动态INT8量化，并提供了一个包含训练、知识蒸馏、结构化剪枝、微调和量化的完整流程。

Result: 实验结果表明，该方法在卫星图像（EuroSAT）和标准基准（CIFAR-10，Rotated MNIST）上有效。实现了29.3%的参数减少，并显著恢复了精度，证明了等变网络的结构化剪枝在实现大幅压缩的同时，能有效保持几何鲁棒性。

Conclusion: 该框架为优化等变模型提供了一个可复现的流程，弥合了群论网络设计与实际部署限制之间的鸿沟。它在卫星图像分析和几何视觉任务中具有特别的实用价值，能够生成在资源受限环境下表现出色的紧凑、变换不变模型。
...
### [103] [Continual Alignment for SAM: Rethinking Foundation Models for Medical Image Segmentation in Continual Learning](https://arxiv.org/abs/2511.17201)
...
### [104] [Blind Deconvolution for Color Images Using Normalized Quaternion Kernels](https://arxiv.org/abs/2511.17253)
...
### [105] [FisheyeGaussianLift: BEV Feature Lifting for Surround-View Fisheye Camera Perception](https://arxiv.org/abs/2511.17210)
...
### [106] [NoPe-NeRF++: Local-to-Global Optimization of NeRF with No Pose Prior](https://arxiv.org/abs/2511.17322)
*Dongbo Shi,Shen Cao,Bojian Wu,Jinhui Guo,Lubin Fan,Renjie Chen,Ligang Liu,Jieping Ye*

Main category: cs.CV

TL;DR: 本文提出NoPe-NeRF++，一种无需姿态先验的局部到全局优化算法，用于训练神经辐射场(NeRF)，显著提高了复杂场景下的相机姿态估计准确性和新视图合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法（特别是NoPe-NeRF）仅关注图像内部的局部关系，在复杂场景中难以恢复准确的相机姿态。

Method: 该方法首先通过显式特征匹配进行相对姿态初始化，接着进行局部联合优化以增强姿态估计并训练更鲁棒的NeRF表示。随后引入全局优化阶段，通过捆集调整（Bundle Adjustment）整合几何一致性约束和特征轨迹，进一步细化姿态并提升NeRF质量。该方法是首次无缝结合局部和全局线索与NeRF的工作。

Result: NoPe-NeRF++在姿态估计精度和新视图合成方面均优于现有最先进方法。在基准数据集上的广泛评估表明，即使在具有挑战性的场景中，该方法也展现出卓越的性能和鲁棒性。

Conclusion: NoPe-NeRF++通过其创新的局部到全局优化设计，成功解决了无姿态先验NeRF训练中的挑战，其在复杂场景下的优越性能和鲁棒性验证了其设计选择。
...
### [107] [DSeq-JEPA: Discriminative Sequential Joint-Embedding Predictive Architecture](https://arxiv.org/abs/2511.17354)
*Xiangteng He,Shunsuke Sakai,Kun Yuan,Nicolas Padoy,Tatsuhito Hasegawa,Leonid Sigal*

Main category: cs.CV

TL;DR: DSeq-JEPA是一种受人类视觉启发的新型联合嵌入预测架构，它通过识别判别性区域并按判别顺序进行预测，学习比I-JEPA更具判别性和泛化性的视觉表示。


<details>
  <summary>Details</summary>
Motivation: 现有的I-JEPA模型将所有图像区域视为统一且独立的，缺乏明确的预测顺序和关注点，这与人类视觉选择性地、顺序地从最信息丰富的区域部署注意力的机制不符。

Method: DSeq-JEPA结合了JEPA式的潜在预测和GPT式的序列推理。具体方法是：(i) 首先基于Transformer派生的显著图识别出主要的判别性区域，强调视觉重要性的分布；(ii) 然后按照这种判别性顺序预测后续区域，逐步形成从主要到次要线索的课程式语义进展，这是一种GPT风格的预训练。

Result: DSeq-JEPA在多种任务上，包括图像分类（ImageNet）、细粒度视觉分类（iNaturalist21, CUB-200-2011, Stanford-Cars）、检测和分割（MS-COCO, ADE20K）以及低级推理任务（Clevr/Count, Clevr/Dist），都持续地比I-JEPA变体学习到更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA通过整合判别性顺序预测，成功地弥合了预测式和自回归式自监督学习之间的鸿沟，从而能够学习到更优越的视觉表示。
...
### [108] [BiFingerPose: Bimodal Finger Pose Estimation for Touch Devices](https://arxiv.org/abs/2511.17306)
...
### [109] [ATAC: Augmentation-Based Test-Time Adversarial Correction for CLIP](https://arxiv.org/abs/2511.17362)
*Linxiang Su,András Balogh*

Main category: cs.CV

TL;DR: 尽管CLIP在零样本图像-文本匹配方面表现出色，但它对图像对抗性扰动高度脆弱。本文提出了一种名为ATAC的简单而有效的测试时防御策略，通过在CLIP嵌入空间中利用增强诱导漂移向量进行语义恢复，显著提高了鲁棒性，超越现有方法近50%，且计算开销极小。


<details>
  <summary>Details</summary>
Motivation: CLIP模型对对抗性扰动非常脆弱，而对抗性微调成本过高。现有测试时防御策略的鲁棒性有限，因此需要一种更有效、高效的防御方法。

Method: 本文提出了一种基于增强的测试时对抗性校正（ATAC）方法。该方法直接在CLIP的嵌入空间中操作，计算增强诱导的漂移向量以推断语义恢复方向，并根据这些潜在漂移的角度一致性来校正嵌入。

Result: ATAC在广泛的基准测试中始终保持极高的鲁棒性，平均超越现有最先进方法近50%，同时计算开销极小。此外，ATAC在非常规和极端设置下仍保持最先进的鲁棒性，甚至对自适应攻击也表现出不俗的鲁棒性。

Conclusion: ATAC是一种高效的方法，为CLIP嵌入空间的测试时对抗性防御提供了一个新颖的范式。
...
### [110] [Label-Efficient Skeleton-based Recognition with Stable-Invertible Graph Convolutional Networks](https://arxiv.org/abs/2511.17345)
*Hichem Sahbi*

Main category: cs.CV

TL;DR: 本文提出了一种标签高效的图卷积网络（GCN）方法，用于基于骨架的动作识别，通过学习一种结合数据代表性、多样性和不确定性的新型采集函数来选择最有信息量的子集进行标注，并进一步扩展使用可逆GCN，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 基于骨架的动作识别任务严重依赖于大型、昂贵且耗时的人工标注数据集，这是一个关键挑战。

Method: 该方法设计了一种标签高效的GCN。其核心贡献在于学习一种新型的采集函数，通过优化一个混合了数据代表性、多样性和不确定性的目标函数，来对最有信息量的标注子集进行评分。此外，该方法还通过使用可逆GCN来学习最有信息量的子集，该GCN能将数据从环境空间映射到潜在空间，从而更容易捕捉数据的固有分布。

Result: 在两个具有挑战性的基于骨架的识别数据集上进行了广泛实验，结果表明所提出的标签节约型GCNs是有效的，并且优于相关工作。

Conclusion: 所提出的标签高效GCN方法，通过创新的采集函数和可逆GCN扩展，有效解决了基于骨架动作识别中对大量标注数据的依赖问题，并在实验中展现出卓越的性能。
...
### [111] [MMT-ARD: Multimodal Multi-Teacher Adversarial Distillation for Robust Vision-Language Models](https://arxiv.org/abs/2511.17448)
*Yuqi Li,Junhao Dong,Chuanguang Yang,Shiping Wen,Piotr Koniusz,Tingwen Huang,Yingli Tian,Yew-Soon Ong*

Main category: cs.CV

TL;DR: 本文提出MMT-ARD框架，通过多模态多教师对抗性鲁棒蒸馏，利用双教师知识融合、动态权重分配和自适应sigmoid加权策略，显著提升了视觉-语言模型（VLM）的对抗性鲁棒性和训练效率。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型（VLM）在安全关键应用中日益普及，其对抗性鲁棒性成为关键问题。传统的单教师对抗性知识蒸馏方法存在知识多样性有限、收敛速度慢以及鲁棒性与准确性难以平衡等缺点。

Method: 我们提出了MMT-ARD（多模态多教师对抗性鲁棒蒸馏）框架。其核心创新包括：1) 双教师知识融合架构，协同优化干净特征的保留和鲁棒特征的增强。2) 基于教师置信度的动态权重分配策略，以更好地处理挑战性对抗样本。3) 自适应sigmoid加权函数，平衡跨模态知识迁移的强度，以减轻教师之间的偏差。

Result: 在ImageNet和零样本基准测试中，MMT-ARD使ViT-B-32模型的鲁棒准确率提高了+4.32%，零样本准确率提高了+3.5%，同时训练效率比传统单教师方法提高了2.3倍。

Conclusion: MMT-ARD框架在增强多模态大型模型的对抗性鲁棒性方面表现出有效性和可扩展性，解决了传统单教师蒸馏方法的局限性。
...
### [112] [MCMoE: Completing Missing Modalities with Mixture of Experts for Incomplete Multimodal Action Quality Assessment](https://arxiv.org/abs/2511.17397)
*Huangbiao Xu,Huanqi Wu,Xiao Ke,Junyi Wu,Rui Xu,Jinglin Xu*

Main category: cs.CV

TL;DR: 本文提出了一种名为MCMoE的缺失补全与专家混合框架，以解决多模态动作质量评估（AQA）中模态缺失导致模型失效或性能下降的问题，在完整和不完整模态场景下均实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态AQA通过结合互补信息来增强对细微动作差异的评估。然而，在实际推断阶段，部分模态常常不可用，导致现有模型无法运行或性能急剧下降，这是因为跨模态交互被中断。

Method: 本文提出了MCMoE框架，它在单阶段训练中统一了单模态和联合表示学习。具体方法包括：1) 一个自适应门控模态生成器，动态融合可用信息以重建缺失模态。2) 模态专家，用于学习单模态知识，并动态混合所有专家知识以提取跨模态联合表示，进一步细化和补充缺失模态。3) 在训练阶段，利用完整的多模态特征和单模态专家知识来指导模态生成和基于生成的联合表示提取。

Result: 广泛的实验表明，MCMoE在三个公共AQA基准测试中，无论是在完整还是不完整的多模态学习场景下，都取得了最先进（state-of-the-art）的结果。

Conclusion: MCMoE框架通过其创新的模态生成器和专家混合机制，有效解决了多模态AQA中模态缺失的挑战，显著提高了模型在现实场景中的鲁棒性和评估性能。
...
### [113] [MolSight: Optical Chemical Structure Recognition with SMILES Pretraining, Multi-Granularity Learning and Reinforcement Learning](https://arxiv.org/abs/2511.17300)
...
### [114] [GPR-OdomNet: Difference and Similarity-Driven Odometry Estimation Network for Ground Penetrating Radar-Based Localization](https://arxiv.org/abs/2511.17457)
*Huaichao Wang,Xuanxin Fan,Ji Liu,Haifeng Li,Dezhen Song*

Main category: cs.CV

TL;DR: 本研究提出一种基于神经网络的里程计方法，利用探地雷达（GPR）B扫描图像的相似性和差异性特征，实现机器人/车辆在恶劣天气下精确的欧几里得距离估计，并优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 在恶劣天气和环境条件下，使用探地雷达（GPR）进行机器人/车辆定位时，现有技术在处理B扫描图像的微小差异时，难以准确估计距离。

Method: 引入一种新型定制神经网络，从连续时刻获取的GPR B扫描图像中提取多尺度特征，并通过分析这些特征的相似性和差异来确定行驶的欧几里得距离。

Result: 在公开的CMU-GPR数据集上进行消融研究和比较实验，结果表明该方法在所有测试中均优于现有最先进方法。总体加权均方根误差（RMSE）为0.449米，比最佳现有技术减少了10.2%。

Conclusion: 所提出的基于神经网络的GPR里程计方法显著提高了机器人/车辆定位中距离估计的准确性，尤其适用于处理B扫描图像中细微差异的场景。
...
### [115] [Improving Multimodal Distillation for 3D Semantic Segmentation under Domain Shift](https://arxiv.org/abs/2511.17455)
*Björn Michele,Alexandre Boulch,Gilles Puy,Tuan-Hung Vu,Renaud Marlet,Nicolas Courty*

Main category: cs.CV

TL;DR: 本文针对激光雷达语义分割在领域迁移下的泛化性问题，深入研究了如何利用视觉基础模型（VFMs）进行无监督域适应。研究发现，激光雷达骨干网络架构至关重要，一个预训练骨干网络可应对多种域迁移，且冻结预训练骨干网络并训练一个MLP头能获得最佳性能，实现了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 为特定类型激光雷达训练的语义分割网络，在未经干预的情况下难以泛化到未见过的激光雷达，导致性能下降。近期趋势是利用视觉基础模型（VFMs）提供跨域的鲁棒特征来弥补这一性能差距。本研究旨在通过全面探索，找出在激光雷达点云语义分割的无监督域适应中有效利用VFMs的方法。

Method: 本研究基于无监督图像到激光雷达知识蒸馏技术，进行了详尽的研究，以识别利用VFMs的有效策略。具体探讨了：1) 激光雷达骨干网络的架构对目标域泛化性能的影响；2) 是否可能预训练一个通用的骨干网络来处理多个域迁移；3) 在语义分割中，冻结预训练骨干网络并训练一个MLP头部或进行其他训练策略的效果。

Result: 研究揭示了以下关键发现：(1) 激光雷达骨干网络的架构是最大化目标域泛化性能的关键；(2) 可以一次性预训练一个单一的骨干网络，并用它来解决多种域迁移问题；(3) 最佳结果是通过保持预训练骨干网络冻结并训练一个MLP头部进行语义分割获得的。由此产生的流水线在四个公认且具有挑战性的设置中取得了最先进的（SOTA）结果。

Conclusion: 本研究成功地为激光雷达点云语义分割的无监督域适应，找到了有效利用视觉基础模型的策略。通过优化激光雷达骨干网络架构，并采用单一、冻结的预训练骨干网络与一个简单的MLP头部相结合的方法，能够显著提高跨域泛化性能，并达到了当前最先进的水平。
...
### [116] [MorphSeek: Fine-grained Latent Representation-Level Policy Optimization for Deformable Image Registration](https://arxiv.org/abs/2511.17392)
...
### [117] [SVRecon: Sparse Voxel Rasterization for Surface Reconstruction](https://arxiv.org/abs/2511.17364)
...
### [118] [SuperQuadricOcc: Multi-Layer Gaussian Approximation of Superquadrics for Real-Time Self-Supervised Occupancy Estimation](https://arxiv.org/abs/2511.17361)
...
### [119] [Illustrator's Depth: Monocular Layer Index Prediction for Image Decomposition](https://arxiv.org/abs/2511.17454)
...
TL;DR: 本文提出了一种名为“插画师深度”的新型深度定义，旨在将平面图像分解为可编辑、有序的图层。该方法通过预测像素的图层索引，实现了优于现有技术的图像矢量化，并支持文本到矢量图形生成、2D到3D浮雕生成以及深度感知编辑。


<details>
  <summary>Details</summary>
Motivation: 数字内容创作中的一个关键挑战是如何将平面图像分解为可编辑、有序的图层。传统的深度定义通常是物理量，而本研究旨在探索一种更符合艺术家构图过程、以可编辑性为目标的抽象深度概念。

Method: 引入“插画师深度”，它为每个像素推断一个图层索引，形成离散、全局一致且针对可编辑性优化的元素排序，从而实现可解释的图像分解。研究还提出并训练了一个神经网络，使用精选的矢量图形图层数据集，直接从栅格输入预测图层信息。

Result: 该方法在图像矢量化方面显著优于现有基线，并支持高保真文本到矢量图形生成、从2D图像自动生成3D浮雕，以及直观的深度感知编辑。

Conclusion: 通过将深度从物理量重新定义为一种创意抽象，“插画师深度”预测为可编辑图像分解提供了一个新基础。
...
### [120] [Video-R4: Reinforcing Text-Rich Video Reasoning with Visual Rumination](https://arxiv.org/abs/2511.17490)
*Yolo Yunlong Tang,Daiki Shimada,Hang Hua,Chao Huang,Jing Bi,Rogerio Feris,Chenliang Xu*

Main category: cs.CV

TL;DR: 本文提出Video-R4，一个受人类启发的大型多模态模型，通过视觉反刍（迭代选择、缩放、重新编码帧）来解决富文本视频问答中现有模型因单次感知而导致的细粒度证据识别失败问题，并在多个任务上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 大多数视频问答模型依赖于对固定帧的单次感知，导致在处理视频中细小、瞬态的文本线索时出现幻觉和失败。人类通过暂停、放大和重复阅读关键区域来理解这些信息，这启发了本文的研究。

Method: 引入了Video-R4，一个执行视觉反刍的视频推理大型多模态模型（LMM），它能迭代地选择帧、放大信息区域、重新编码检索到的像素并更新其推理状态。构建了两个包含可执行反刍轨迹的数据集：Video-R4-CoT-17k用于监督学习，Video-R4-RL-30k用于强化学习。提出了一个多阶段反刍学习框架，通过SFT和基于GRPO的RL逐步微调一个7B的LMM，使其学习原子和混合视觉操作。

Result: Video-R4-7B在M4-ViteVQA上取得了最先进的成果，并进一步泛化到多页文档问答、幻灯片问答和通用视频问答。

Conclusion: 迭代反刍是像素级多模态推理的有效范式。
...
### [121] [Radar2Shape: 3D Shape Reconstruction from High-Frequency Radar using Multiresolution Signed Distance Functions](https://arxiv.org/abs/2511.17484)
*Neel Sortur,Justin Goodwin,Purvik Patel,Luis Enrique Martinez,Tzofi Klinghoffer,Rajmonda S. Caceres,Robin Walters*

Main category: cs.CV

TL;DR: 本文提出Radar2Shape，一个去噪扩散模型，通过将雷达信号频率与多分辨率形状特征关联，实现从部分可观测高频雷达信号中重建任意三维形状。


<details>
  <summary>Details</summary>
Motivation: 从高频雷达信号确定三维物体形状在商业和航空航天应用中至关重要，但分析上复杂。现有深度学习方法难以表示任意形状或处理有限视角下的真实雷达信号。光学三维重建方法虽能从有限视角生成任意形状，但无法直接应用于雷达信号。

Method: Radar2Shape采用两阶段去噪扩散模型。首先，它学习一个具有分层分辨率形状特征的正则化潜在空间。其次，它以粗到细的方式，通过条件化雷达信号的频率扩散到这个潜在空间中，将频率与多分辨率形状特征相关联。

Result: Radar2Shape能够成功地从部分观测的雷达信号中重建任意三维形状，并对两种不同的模拟方法和真实世界数据表现出鲁棒的泛化能力。此外，还发布了两个合成基准数据集，以促进高频雷达领域未来的研究。

Conclusion: Radar2Shape有效地解决了从部分雷达信号重建任意三维形状的挑战，展示了其在真实世界雷达系统中的应用潜力，并通过发布新数据集鼓励了该领域的进一步研究。
...
### [122] [Native 3D Editing with Full Attention](https://arxiv.org/abs/2511.17501)
*Weiwei Cai,Shuangkang Fang,Weicai Ye,Xin Dong,Yunhan Yang,Xuanyang Zhang,Wei Cheng,Yanpei Cao,Gang Yu,Tao Chen*

Main category: cs.CV

TL;DR: 本文提出一种新颖的、高效的、原生的3D编辑框架，通过构建大规模多模态数据集和引入3D token拼接条件策略，解决了现有指令引导3D编辑方法速度慢或几何不一致的问题，并在生成质量、3D一致性和指令保真度方面树立了新标杆。


<details>
  <summary>Details</summary>
Motivation: 指令引导的3D编辑领域面临关键限制：基于优化的方法速度过慢，而依赖多视图2D编辑的前馈方法常导致几何不一致和视觉质量下降。

Method: 本文提出一种原生的3D编辑框架，通过单一、高效的前馈过程直接操作3D表示。具体方法包括：1) 创建一个大规模、多模态的指令引导3D编辑数据集，涵盖添加、删除和修改任务，并确保编辑对象的忠实度和未编辑区域的一致性。2) 探索两种模型条件策略：传统的交叉注意力机制和新颖的3D token拼接方法。

Result: 研究结果表明，3D token拼接方法在参数效率和性能上均优于交叉注意力。广泛评估显示，本文方法超越了现有的2D提升方法，在生成质量、3D一致性和指令保真度方面设定了新的基准。

Conclusion: 本文提出的原生3D编辑框架，结合大规模多模态数据集和高效的3D token拼接条件策略，显著提升了指令引导3D编辑的效率、一致性和保真度，超越了现有方法，为3D内容创作提供了更广阔的途径。
...
### [123] [Counterfactual World Models via Digital Twin-conditioned Video Diffusion](https://arxiv.org/abs/2511.17481)
...
### [124] [UAM: A Unified Attention-Mamba Backbone of Multimodal Framework for Tumor Cell Classification](https://arxiv.org/abs/2511.17355)
...
### [125] [EvDiff: High Quality Video with an Event Camera](https://arxiv.org/abs/2511.17492)
*Weilun Li,Lei Sun,Ruixi Gao,Qi Jiang,Yuqin Ma,Kaiwei Wang,Ming-Hsuan Yang,Luc Van Gool,Danda Pani Paudel*

Main category: cs.CV

TL;DR: EvDiff提出了一种事件相机扩散模型，通过代理训练框架从事件流中生成高质量彩色视频，解决了传统方法在感知质量、模型容量和数据依赖方面的限制。


<details>
  <summary>Details</summary>
Motivation: 事件相机重建强度图像是一个病态问题，传统方法通过确定性回归直接映射事件到图像，但其感知结果不佳，且在模型容量和训练数据扩展方面存在困难，尤其依赖配对的事件-图像数据集。

Method: 本文提出了EvDiff，一个基于事件的扩散模型，采用代理训练框架生成高质量视频。为降低计算成本，它仅执行单步前向扩散，并配备时间一致的EvEncoder。新颖的代理训练框架消除了对配对事件-图像数据集的依赖，允许模型利用大规模图像数据集来提高容量。

Result: EvDiff能够仅从单色事件流生成高质量的彩色视频。在真实世界数据集上的实验表明，该方法在保真度和真实感之间取得了平衡，在像素级和感知指标上均优于现有方法。

Conclusion: EvDiff通过其创新的扩散模型和代理训练框架，成功解决了从事件流重建高质量视频的挑战，并在性能和数据利用效率上取得了显著提升。
...
### [126] [Downscaling Intelligence: Exploring Perception and Reasoning Bottlenecks in Small Multimodal Models](https://arxiv.org/abs/2511.17487)
*Mark Endo,Serena Yeung-Levy*

Main category: cs.CV

TL;DR: 本文分析了多模态模型中缩小LLM容量对视觉能力的影响，发现其对视觉感知和推理能力有显著影响。为解决此瓶颈，提出“视觉提取调优”和“提取+思考”（Extract+Think）方法，提升了小型高效多模态系统的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型取得了显著进展，但实际需求呼唤更小、更高效的系统。研究旨在理解如何在缩小模型规模的同时保持智能，特别关注LLM容量缩减对多模态能力的影响。

Method: 研究首先对多模态模型中LLM容量缩减如何影响多模态能力进行了原理性分析。接着，分离并考察了LLM缩减对感知能力而非推理能力的影响。为解决发现的瓶颈，引入了“视觉提取调优”方法，显式训练模型提取与指令相关的视觉细节。最后，结合提取的视觉细节和分步推理，提出了“提取+思考”（Extract+Think）方法。

Result: 初步发现LLM容量缩减对视觉能力的影响远大于对LLM继承能力的影响。进一步研究表明，性能下降主要反映了视觉推理的预期下降，以及更根本的感知能力损失，且对感知的冲击常常与对推理的影响相当或更甚。提出的“提取+思考”方法在此领域为效率和性能树立了新标准。

Conclusion: 缩小多模态模型中的LLM容量会对视觉感知和推理能力造成显著影响。通过引入视觉提取调优和“提取+思考”方法，可以有效解决这一瓶颈，为小型、高效的多模态系统提供卓越的效率和性能。
...
### [127] [An Artificial Intelligence Framework for Measuring Human Spine Aging Using MRI](https://arxiv.org/abs/2511.17485)
...
### [128] [Non-Parametric Probabilistic Robustness: A Conservative Metric with Optimized Perturbation Distributions](https://arxiv.org/abs/2511.17380)
...
### [129] [Shona spaCy: A Morphological Analyzer for an Under-Resourced Bantu Language](https://arxiv.org/abs/2511.16680)
*Happymore Masoka*

Main category: cs.CL

TL;DR: 本文介绍了Shona spaCy，一个基于spaCy框架的开源、基于规则的绍纳语形态学处理管道，通过结合词典和语言学规则实现了90%的词性标注准确率和88%的形态特征准确率，旨在提升绍纳语的NLP可及性。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言自然语言处理（NLP）发展迅速，但班图语系的绍纳语在形态分析和语言感知工具方面仍然服务不足。

Method: 该研究构建了一个基于spaCy框架的开源、基于规则的形态学管道Shona spaCy。它结合了精选的JSON词典和有语言学基础的规则，用于建模名词类别前缀、动词主语一致、时态-体貌标记、象声词和附着语素，并将其整合到词元级别的引理、词性标注和形态特征注释中。

Result: 在正式和非正式的绍纳语语料库上进行评估，该系统取得了90%的词性标注准确率和88%的形态特征准确率，同时保持了其语言学决策的透明度。该工具已通过pip安装、GitHub和PyPI发布。

Conclusion: Shona spaCy通过弥合描述性语法和计算实现之间的鸿沟，提升了绍纳语使用者的NLP可及性和数字包容性，并为其他资源不足的班图语系的形态分析工具提供了范本。
...
### [130] [Towards Hyper-Efficient RAG Systems in VecDBs: Distributed Parallel Multi-Resolution Vector Search](https://arxiv.org/abs/2511.16681)
*Dong Liu,Yanxuan Yu*

Main category: cs.CL

TL;DR: 本文提出语义金字塔索引（SPI），一种新颖的多分辨率向量索引框架，通过查询自适应分辨率控制，显著提升RAG系统中向量数据库的检索速度和上下文相关性。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG系统依赖于扁平或单分辨率的向量数据库索引结构，无法适应多样化用户查询所需的语义粒度变化，导致检索速度和上下文相关性之间存在次优的权衡。

Method: SPI构建了一个文档嵌入的语义金字塔，并通过一个轻量级分类器为每个查询动态选择最佳分辨率级别。这种自适应方法支持从粗到细的渐进式检索，从而在保持语义覆盖的同时显著加速搜索。它作为插件实现，兼容FAISS和Qdrant。

Result: SPI在多项RAG任务（如MS MARCO、Natural Questions和多模态检索）中，实现了高达5.7倍的检索速度提升和1.8倍的内存效率增益，同时将端到端QA F1分数提高了2.5个点。理论分析也提供了检索质量和延迟的保证。

Conclusion: SPI框架通过其多分辨率和查询自适应特性，有效解决了RAG系统中向量数据库的检索效率和相关性问题。它与现有基础设施兼容，易于部署到生产RAG系统，并经过广泛验证，具有显著的性能优势。
...
### [131] [Bench360: Benchmarking Local LLM Inference from 360°](https://arxiv.org/abs/2511.16682)
*Linus Stuhlmann,Mauricio Fadel Argerich,Jonathan Fürst*

Main category: cs.CL

TL;DR: Bench360是一个360°全方位基准测试框架，旨在解决本地大型语言模型（LLM）推理配置选择的复杂性，通过集成系统和任务特定指标，帮助用户找到最佳设置。


<details>
  <summary>Details</summary>
Motivation: 随着小型开源LLM和推理引擎的普及，本地运行LLM变得常见，但用户面临大量配置选择。现有基准测试专注于狭窄的评估目标，缺乏用户导向，未能整合系统和任务特定指标，且不支持多种推理引擎、使用场景和量化级别，导致优化配置需要大量手动工作。

Method: 本文提出了Bench360框架，允许用户轻松定义自定义任务、数据集和相关任务特定指标。它能自动对选定的LLM、推理引擎和量化级别在不同使用场景（单流、批处理和服务器）下进行基准测试。Bench360跟踪广泛的指标，包括计算性能（如延迟、吞吐量）、资源使用（如每次查询能耗）、部署（如冷启动时间）等系统指标，以及ROUGE、F1分数或准确性等任务特定指标。

Result: 研究在四种常见LLM任务（通用知识与推理、问答、摘要和Text-to-SQL）、三种硬件平台和四种最先进推理引擎上演示了Bench360。结果揭示了任务性能与系统级效率之间有趣的权衡，突出了不同推理引擎和模型之间的差异。最重要的是，对于本地推理没有单一的最佳设置。

Conclusion: 由于本地LLM推理配置的复杂性且不存在单一最佳设置，迫切需要像Bench360这样的框架来帮助用户进行全面、用户导向的基准测试和优化。
...
### [132] [How Well Do LLMs Understand Tunisian Arabic?](https://arxiv.org/abs/2511.16683)
*Mohamed Mahdi*

Main category: cs.CL

TL;DR: 本研究引入了一个新的突尼斯阿拉伯语（Tunizi）平行语料库，并对主流大型语言模型在转写、翻译和情感分析任务上的表现进行了基准测试，揭示了它们在处理低资源语言方面的显著差异和局限性，强调了将此类语言纳入未来AI系统的重要性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）对低资源语言（如突尼斯阿拉伯语）的理解能力常被忽视，这可能导致数百万突尼斯人无法用母语与AI充分互动，转而使用法语或英语，从而威胁到突尼斯方言的保存，并可能对识字率和年轻一代的语言偏好产生负面影响。因此，研究旨在解决这一排斥问题，并促进语言多样性。

Method: 本研究引入了一个包含突尼斯阿拉伯语、标准突尼斯阿拉伯语和英语翻译的并行数据集，并附带情感标签。研究人员在转写、翻译和情感分析这三个任务上对多个流行的大型语言模型进行了基准测试。

Result: 基准测试结果显示，不同模型之间存在显著差异，突出了它们在理解和处理突尼斯方言方面的优势和局限性。这些差距被量化，揭示了当前AI系统在支持低资源语言方面的不足。

Conclusion: 这项工作通过量化现有差距，强调了将低资源语言纳入下一代AI系统的重要性，以确保技术保持可访问性、包容性并植根于文化之中，从而避免语言排斥和文化流失的风险。
...
### [133] [Ellipsoid-Based Decision Boundaries for Open Intent Classification](https://arxiv.org/abs/2511.16685)
*Yuetian Zou,Hanlei Zhang,Hua Xu,Songze Li,Long Xiao*

Main category: cs.CL

TL;DR: EliDecide是一种新颖的开放意图分类方法，它通过对比学习和双重损失函数，学习具有不同特征方向尺度变化的椭球决策边界，从而优于传统的球形边界，在多个文本意图基准测试中达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自适应决策边界方法在开放意图分类中假设已知类别呈各向同性分布（即球形边界），忽略了沿不同方向的分布方差，限制了边界的灵活性和系统的鲁棒性。

Method: 该方法首先采用监督对比学习来获得已知样本的判别性特征空间。其次，使用可学习矩阵将椭球参数化为每个已知类别的边界。最后，通过一个新颖设计的双重损失函数来优化边界，该函数平衡了经验风险（扩展边界以覆盖已知样本）和开放空间风险（针对合成的伪开放样本收缩边界）。

Result: EliDecide在多个文本意图基准测试以及一个问题分类数据集上均取得了最先进的性能。椭球的灵活性展示了卓越的开放意图检测能力。

Conclusion: 椭球决策边界的灵活性显著提高了开放意图检测能力，并显示出在多样化复杂开放世界场景中推广到更多文本分类任务的强大潜力。
...
### [134] [Prompt-Based Value Steering of Large Language Models](https://arxiv.org/abs/2511.16688)
*Giulio Antonio Abbo,Tony Belpaeme*

Main category: cs.CL

TL;DR: 本文提出了一种实用、可复现且模型无关的评估方法，用于衡量提示词能否有效引导大型语言模型（LLM）的生成文本符合特定人类价值观，并量化了目标价值观的存在和增益。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在需要与人类价值观对齐的应用中日益普及。虽然模型微调常用于确保安全响应，但这种技术是静态的，不适用于涉及动态价值观和偏好的日常情境。

Method: 研究者提出了一种实用、可复现且模型无关的程序，以评估候选提示词是否能有效引导生成文本符合特定人类价值观。他们形式化了一种评分方法来量化生成响应中目标价值观的存在和增益。该方法应用于Wizard-Vicuna语言模型的一个变体，并结合Schwartz的基本人类价值观理论和一个通过对话数据集进行的结构化评估。通过这种设置，他们比较了基线提示词和明确以价值观为条件的提示词。

Result: 研究结果表明，即使不修改模型或动态优化提示词，通过提示词进行价值观引导是可能的。

Conclusion: 通过精心设计的提示词，可以在不改变模型或进行动态优化的情况下，有效地引导大型语言模型的输出符合特定的人类价值观。
...
### [135] [Concept-Based Interpretability for Toxicity Detection](https://arxiv.org/abs/2511.16689)
*Samarth Garg,Deeksha Varshney,Divya Singh*

Main category: cs.CL

TL;DR: 本研究引入了一种基于概念梯度（CG）的解释性技术，用于毒性语言检测，以解决概念过度归因导致的分类错误。通过创建目标词汇集和词-概念对齐（WCA）分数，并结合无词汇增强策略，旨在提供更因果的解释并理解模型对毒性语言模式的归因。


<details>
  <summary>Details</summary>
Motivation: 社交网络中有害内容的传播日益严重。尽管文本毒性检测取得进展，但基于概念的解释仍然有限，且概念对目标类别的过度归因常导致分类错误。

Method: 1. 利用毒性检测数据集中的子类型属性（如淫秽、威胁、侮辱等）作为概念。2. 引入概念梯度（CG）方法，通过衡量概念变化如何直接影响模型输出来提供更因果的解释。3. 提出目标词汇集（Targeted Lexicon Set），捕获导致文本分类模型误分类的毒性词汇。4. 计算词-概念对齐（WCA）分数，量化这些词汇因过度归因于毒性概念而导致错误的程度。5. 引入一种无词汇增强策略，生成排除预定义毒性词汇集的毒性样本，以检查在移除明确词汇重叠后过度归因是否仍然存在。

Result: 本研究提出了一系列方法：概念梯度（CG）提供了一种更因果的解释，超越了传统的基于输入特征的方法；目标词汇集（TLS）和词-概念对齐（WCA）分数能够识别并量化导致误分类的毒性词汇及其过度归因问题；无词汇增强策略则有助于在没有明确词汇重叠的情况下，探究模型对更广泛毒性语言模式的归因。

Conclusion: 通过引入概念梯度解释技术、目标词汇集、WCA分数和无词汇增强策略，本研究旨在深入理解毒性检测模型中由于概念过度归因而导致的误分类问题，并揭示模型对更广泛毒性语言模式的归因洞察。
...
### [136] [Reproducibility Report: Test-Time Training on Nearest Neighbors for Large Language Models](https://arxiv.org/abs/2511.16691)
...
### [137] [How Language Directions Align with Token Geometry in Multilingual LLMs](https://arxiv.org/abs/2511.16693)
*JaeSeong Kim,Suan Lee*

Main category: cs.CL

TL;DR: 本研究系统分析了多语言大型语言模型（LLMs）内部语言信息的编码结构及其在不同层中的演变，发现语言信息在早期层迅速分离并受训练数据组成深刻影响。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言LLMs在多种语言上表现出色，但目前缺乏对其内部表示空间中语言信息如何结构化以及如何跨层演变的系统分析。

Method: 对六个多语言LLMs的268个Transformer层进行了全面的探测研究，使用了线性和非线性探针，并引入了一种新的“Token-语言对齐”分析方法，以量化语言编码的层级动态和几何结构。

Result: 研究发现，语言信息在第一个Transformer块中急剧分离（从第0层到第1层增加了76.4%），并在整个模型深度中几乎完全线性可分。此外，语言方向与词汇嵌入之间的对齐与训练数据的语言构成密切相关。包含中文的模型实现了16.43%的ZH Match@Peak，而以英语为中心的模型仅为3.90%，揭示了4.21倍的结构印记效应。这些结果表明，多语言LLMs通过由训练语料库塑造的潜在表征结构而非表面脚本特征来区分语言。

Conclusion: 本分析为多语言表示学习中的数据组成策略和公平性提供了实用见解。
...
### [138] [Falsely Accused: How AI Detectors Misjudge Slightly Polished Arabic Articles](https://arxiv.org/abs/2511.16690)
...
### [139] [Hierarchical Retrieval with Out-Of-Vocabulary Queries: A Case Study on SNOMED CT](https://arxiv.org/abs/2511.16698)
...
### [140] [Detecting and Steering LLMs' Empathy in Action](https://arxiv.org/abs/2511.16699)
*Juan P. Cadile*

Main category: cs.CL

TL;DR: 研究了LLM中“行动同理心”——为满足人类需求牺牲任务效率的意愿——作为激活空间中的一个线性方向，并在不同模型上进行了检测与操控，发现检测与操控之间存在差异，且安全训练可能影响操控的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型（LLMs）如何编码和响应“行动同理心”（即为满足人类需求而牺牲任务效率的意愿），并评估其在模型激活空间中的可检测性和可操控性。

Method: 将“行动同理心”定义为LLM激活空间中的一个线性方向。使用基于“行动同理心”（EIA）基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B（安全训练）和Dolphin-Llama-3.1-8B（未审查）模型上进行检测和操控测试。检测方面，测量AUROC并评估探针与EIA行为得分的相关性以及跨模型探针的一致性。操控方面，评估双向控制的成功率和一致性。

Result: 检测方面，所有模型在最佳层都达到了0.996-1.00的AUROC，未审查的Dolphin与安全训练模型表现一致，表明同理心编码独立于安全训练。Phi-3探针与EIA行为得分强相关（r=0.71）。跨模型探针一致性有限。操控方面，Qwen和Phi-3分别实现了65.3%和61.7%的成功率，并保持了双向控制的一致性。Dolphin表现出不对称的可操控性：亲同理心操控成功率达94.4%，但反同理心操控导致灾难性故障（空输出、代码伪影）。

Conclusion: 检测与操控之间的差距因模型而异。Qwen和Phi-3保持了双向操控的一致性，而Dolphin仅在增强同理心方面表现出鲁棒性。安全训练可能影响操控的鲁棒性，而非阻止操控，但需更多模型进行验证。
...
### [141] [From Representation to Enactment: The ABC Framework of the Translating Mind](https://arxiv.org/abs/2511.16811)
*Michael Carl,Takanori Mizowaki,Aishvarya Raj,Masaru Yamada,Devi Sri Bandaru,Yuxiang Wei,Xinyue Ren*

Main category: cs.CL

TL;DR: 本文基于扩展心智理论和激进具身认知，提出了一种翻译心智的非表征性ABC框架，将翻译视为一种动态整合情感、行为和认知过程的具身活动，通过脑-身-环境互动而涌现，重新定义为对社会文化实践的熟练参与。


<details>
  <summary>Details</summary>
Motivation: 旨在为基于表征的心智模型提供一个替代方案，特别是在翻译领域，并在此基础上构建翻译心智的新理论。

Method: 提出了一个新颖的ABC（情感、行为、认知）翻译心智框架，将翻译视为动态整合这些过程的具身活动。借鉴预测加工和（具身）主动推理理论，论证翻译者心智通过脑-身-环境互动循环而涌现。

Result: 翻译被重新定义为一种熟练参与社会文化实践的活动，而非静态语际对应关系的操纵。意义通过与文本、工具和语境的具身互动实时共同创造，翻译者心智在此过程中涌现，而非仅仅扩展。

Conclusion: 翻译并非对静态语际对应关系的操纵，而是一种动态整合情感、行为、认知过程的具身活动，通过脑-身-环境互动涌现。这提供了一个非表征性的视角，将翻译视为对社会文化实践的熟练参与。
...
### [142] [NALA_MAINZ at BLP-2025 Task 2: A Multi-agent Approach for Bangla Instruction to Python Code Generation](https://arxiv.org/abs/2511.16787)
...
### [143] [PEPPER: Perception-Guided Perturbation for Robust Backdoor Defense in Text-to-Image Diffusion Models](https://arxiv.org/abs/2511.16830)
*Oscar Chew,Po-Yi Lu,Jayden Lin,Kuan-Hao Huang,Hsuan-Tien Lin*

Main category: cs.CL

TL;DR: PEPPER是一种针对文本到图像(T2I)扩散模型后门攻击的防御方法，通过重写提示词来扰乱触发器并提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像(T2I)扩散模型容易受到后门攻击，即输入提示词中的触发器可导致生成有害或意外内容。

Method: PEPPER（PErcePtion Guided PERturbation）通过将输入标题重写为语义上不同但视觉上相似的标题，并添加不显眼的元素来扰乱提示词中嵌入的触发器，从而稀释触发词的影响。

Result: 实验表明，PEPPER对基于文本编码器的攻击特别有效，显著降低了攻击成功率，同时保持了生成质量。此外，PEPPER可与现有防御方法结合使用，提供比任何单一方法更强大和更具泛化性的鲁棒性。

Conclusion: PEPPER通过感知引导的扰动策略，有效地增强了T2I模型对后门攻击的鲁棒性，尤其是在对抗基于文本编码器的攻击时表现出色，并能与其他防御方法协同工作。
...
### [144] [Interpretable dimensions support an effect of agentivity and telicity on split intransitivity](https://arxiv.org/abs/2511.16824)
*Eva Neu,Brian Dillon,Katrin Erk*

Main category: cs.CL

TL;DR: 本文重新审视了不及物动词的语义（施事性、终结性）与句法（非作格、作格）之间的关系，通过使用可解释维度的方法，再次确认了两者之间的关联，并提出了一种新的评估语义属性的有效方法。


<details>
  <summary>Details</summary>
Motivation: 长期以来，施事性动作动词被认为倾向于非作格句法，而终结性事件动词倾向于作格句法。然而，Kim et al. (2024) 的最新研究发现，人类对施事性和终结性的评分并不能很好地预测不及物动词的句法行为，这促使作者重新审视这个问题。

Method: 研究方法是使用可解释维度，这些维度通过施事性和终结性量表两极的种子词计算得出，并结合人类判断来评估动词的语义属性。

Result: 研究结果支持了非作格/作格与施事性/终结性之间的联系。这表明，当使用可解释维度结合人类判断时，可以为那些在评分任务中难以评估的语义属性提供有价值的证据。

Conclusion: 结论是，结合使用可解释维度和人类判断，能够为不易通过传统评分任务评估的语义属性提供有力的证据，并再次确认了不及物动词语义与句法之间的关联。
...
### [145] [Improving Latent Reasoning in LLMs via Soft Concept Mixing](https://arxiv.org/abs/2511.16885)
*Kang Wang,Xiangyu Duan,Tianyi Du*

Main category: cs.CL

TL;DR: 本文提出软概念混合（SCM）训练方案，通过在训练中直接引入软概念表示，提高大型语言模型（LLMs）的推理能力，并保持训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 人类在抽象概念空间中进行推理，而LLMs通常通过生成离散token进行推理，这可能限制其表达能力。尽管已有研究表明LLMs通过软概念进行潜在推理很有前景，但LLMs本身是基于离散token训练的。本研究旨在缩小软概念推理与离散token训练之间的差距。

Method: 提出软概念混合（SCM）训练方案，该方案在训练过程中直接向模型暴露软概念表示。具体方法是：1) 构建一个软概念向量，通过对嵌入进行概率加权平均。2) 将此向量混合到模型的隐藏状态中，以融入丰富的上下文信息。3) 整个潜在推理过程通过强化学习（RL）进行优化。

Result: 在五个推理基准上的实验表明，SCM显著提高了LLMs的推理性能，同时保持了稳定的训练动态。

Conclusion: SCM是一种有效的训练方案，通过弥合推理中的软概念与训练中的离散token之间的差距，增强了LLMs的推理能力，并确保了训练的稳定性。
...
### [146] [Deep Improvement Supervision](https://arxiv.org/abs/2511.16886)
*Arip Asadulaev,Rayan Banerjee,Fakhri Karray,Martin Takac*

Main category: cs.CL

TL;DR: 本文提出了一种新的训练方案，通过将TRM的潜在推理视为无分类器引导和隐式策略改进，为训练中的每个循环提供目标，显著提高了小型循环架构（TRM）的训练效率，减少了18倍的前向传播次数并消除了停止机制，同时在ARC-1上以0.8M参数达到了24%的准确率，超越了大多数大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 小型循环架构（如TRM）在复杂推理任务（如ARC）上已显示出优于大型语言模型（LLM）的性能。研究的动机是探讨如何在最小改动的情况下进一步提高这些方法的效率。

Method: 将TRM的潜在推理框架为一种无分类器引导和隐式策略改进算法。在此基础上，提出了一种新颖的训练方案，在训练过程中为每个循环提供一个目标。

Result: 该方法显著提高了训练效率，将总前向传播次数减少了18倍，并消除了停止机制，同时保持了与标准TRM相当的质量。值得注意的是，该方法仅用0.8M参数就在ARC-1上达到了24%的准确率，优于大多数大型语言模型。

Conclusion: 通过为训练中的每个循环提供目标，该方法极大地提升了小型循环架构（TRM）的训练效率和性能，以极小的模型参数在复杂推理任务上取得了超越大型语言模型的成果，证明了小规模模型在特定任务上的巨大潜力。
...
### [147] [ConCISE: A Reference-Free Conciseness Evaluation Metric for LLM-Generated Answers](https://arxiv.org/abs/2511.16846)
...
### [148] [Predicting the Formation of Induction Heads](https://arxiv.org/abs/2511.16893)
*Tatsuya Aoyama,Ethan Gotlieb Wilcox,Nathan Schneider*

Main category: cs.CL

TL;DR: 本研究探究了训练数据的统计特性与语言模型中归纳头（induction heads, IHs）形成之间的关系，发现批次大小、上下文大小、以及二元语法重复频率和可靠性是预测IHs形成的关键因素。


<details>
  <summary>Details</summary>
Motivation: 尽管归纳头被认为是现代语言模型上下文学习能力的基础，但其形成机制尚不明确。本研究旨在精确刻画训练数据统计特性与归纳头形成之间的关系。

Method: 本研究通过分析自然数据和合成数据的统计特性，调查了训练数据（包括批次大小、上下文大小、二元语法重复频率和可靠性等）与归纳头形成之间的关系。

Result: 研究发现：(1) 一个结合批次大小和上下文大小的简单方程可以预测归纳头的形成点；(2) 表面二元语法重复频率和可靠性显著影响归纳头的形成，并存在一个精确的帕累托前沿；(3) 高频率和高可靠性的局部依赖足以促成归纳头的形成，但在低频率和低可靠性情况下，类别性和边际分布的形状变得重要。

Conclusion: 本研究揭示了训练数据的统计特性（如批次大小、上下文大小、二元语法重复频率和可靠性）是预测和理解归纳头形成的关键因素，为归纳头的形成机制提供了更精确的刻画。
...
### [149] [Supervised Fine Tuning of Large Language Models for Domain Specific Knowledge Graph Construction:A Case Study on Hunan's Historical Celebrities](https://arxiv.org/abs/2511.17012)
*Junjie Hao,Chun Wang,Ying Qiao,Qiuyue Zuo,Qiya Song,Hua Ma,Xieping Gao*

Main category: cs.CL

TL;DR: 本研究提出了一种监督微调方法，用于增强大型语言模型在湖南近代历史名人领域的信息提取能力，并在低资源环境下取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 湖南历史名人的系统数据资源有限，通用模型在低资源领域的知识提取和结构化输出方面表现不佳。因此需要提升模型在特定历史文化领域的性能。

Method: 1. 设计了针对湖南历史名人领域的细粒度、模式引导的指令模板，并构建了指令微调数据集以弥补领域特定训练语料的不足。2. 对Qwen2.5-7B、Qwen3-8B、DeepSeek-R1-Distill-Qwen-7B和Llama-3.1-8B-Instruct四种大型语言模型进行了参数高效的指令微调。3. 制定了评估其提取性能的标准。

Result: 所有模型在微调后均表现出显著的性能提升。其中，Qwen3-8B表现最佳，在100个样本和50次训练迭代下达到了89.3866分。

Conclusion: 本研究为区域历史文化领域垂直大型语言模型的微调提供了新见解，并强调了其在文化遗产知识提取和知识图谱构建中成本效益高的应用潜力。
...
### [150] [ARQUSUMM: Argument-aware Quantitative Summarization of Online Conversations](https://arxiv.org/abs/2511.16985)
*An Quang Tang,Xiuzhen Zhang,Minh Ngoc Dinh,Zhuang Li*

Main category: cs.CL

TL;DR: 本文提出了一种新颖的“论点感知定量摘要”任务和框架ARQUSUMM，旨在揭示在线对话中论点的“主张-理由”结构并量化其强度，以生成更具信息量的摘要。


<details>
  <summary>Details</summary>
Motivation: 在线对话中争议话题日益增多，现有文本摘要方法未能充分捕捉对话的论证性质，忽视了句子内部深层的论点结构，也未能量化论点强度。用户需要能总结论点、其原理和依据的摘要。

Method: 本文提出了一个新任务：论点感知定量摘要。并提出了ARQUSUMM框架来解决此任务。ARQUSUMM利用大型语言模型（LLM）的小样本学习，结合论证理论，识别句子中的命题及其“主张-理由”关系。对于定量摘要，ARQUSUMM采用论点结构感知的聚类算法来聚合论点并量化其支持度。

Result: 实验结果表明，ARQUSUMM优于现有的对话和定量摘要模型。它生成的摘要能更好地代表论点结构，对用户更有帮助，文本质量高，量化准确性也更高。

Conclusion: ARQUSUMM成功解决了论点感知定量摘要任务，能有效地揭示在线对话中的论点结构并量化其强度，为用户提供高质量、高准确度的摘要。
...
### [151] [Do Vision-Language Models Understand Visual Persuasiveness?](https://arxiv.org/abs/2511.17036)
*Gyuwon Park*

Main category: cs.CL

TL;DR: 研究发现视觉语言模型（VLMs）在理解视觉说服力方面存在局限，尤其是在将视觉元素与沟通意图关联起来的能力上，而非识别说服性对象本身。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）在多模态推理和理解方面取得了显著进展，但它们是否真正理解视觉说服力（即视觉线索如何影响人类态度和决策）仍不清楚。

Method: 本研究构建了一个高共识的二元说服力判断数据集，并提出了视觉说服因素（VPFs）的分类法，涵盖低级感知、中级构图和高级语义线索。同时，探索了认知引导和知识注入策略以进行说服力相关推理，并对VLMs进行了实证分析。

Result: VLMs表现出召回偏见，倾向于过度预测高说服力，并且对低级/中级特征的判别力较弱。相反，消息与物体存在之间的高级语义对齐是人类判断的最强预测因素。在干预策略中，简单的指令或无引导推理支架效果微弱甚至负面，而简洁、基于对象的理由能显著提高精确度和F1分数。

Conclusion: 研究结果表明，VLMs的核心局限不在于识别说服性对象，而在于将这些对象与沟通意图联系起来的能力。
...
### [152] [MUCH: A Multilingual Claim Hallucination Benchmark](https://arxiv.org/abs/2511.17081)
*Jérémie Dentan,Alexi Canesse,Davide Buscaldi,Aymen Shabou,Sonia Vanier*

Main category: cs.CL

TL;DR: 本文介绍了MUCH，首个面向大语言模型(LLM)的声明级不确定性量化(UQ)基准，旨在公平、可复现地评估未来方法，并提出了一种高效的声明分割算法，揭示当前UQ方法仍有巨大改进空间。


<details>
  <summary>Details</summary>
Motivation: 大语言模型缺乏可靠性，声明级不确定性量化是提升其可靠性的有效途径。现有声明级基准在公平性、可复现性、数据可用性及效率方面存在局限性，不适合在真实部署条件下评估UQ方法。

Method: 引入了MUCH基准，包含4873个样本，涵盖英、法、西、德四种语言及四种指令微调的开源LLM。为促进白盒方法开发，发布了每个token的24个生成logits。提出了一种新的确定性算法用于声明分割，其所需时间仅为LLM生成时间的0.2%，使其适用于LLM输出的实时监控。

Result: MUCH提供了一个公平、可复现的平台，用于评估LLM的声明级UQ方法。新提出的声明分割算法高效，适合实时部署。评估结果显示，当前的不确定性量化方法在性能和效率上仍有很大的提升空间。

Conclusion: MUCH基准为未来声明级UQ方法提供了一个重要的评估工具，其高效的声明分割方法解决了实际部署的挑战。研究表明，需要进一步开发更有效、更高效的UQ方法来提高LLM的可靠性。
...
### [153] [Principled Design of Interpretable Automated Scoring for Large-Scale Educational Assessments](https://arxiv.org/abs/2511.17069)
*Yunsung Kim,Mike Hardy,Joseph Tey,Candace Thille,Chris Piech*

Main category: cs.CL

TL;DR: 本研究针对AI自动评分系统缺乏可解释性的问题，提出了FGTI四项可解释性原则，并开发了AnalyticScore框架。该框架利用大型语言模型（LLMs）进行特征化，通过可解释的模型实现高准确度的短答案评分，其可解释性表现与人类标注员一致。


<details>
  <summary>Details</summary>
Motivation: 尽管AI驱动的自动评分系统在评估复杂学生回答方面具有可扩展性和效率，但该领域尚未开发出广泛接受的可解释自动评分解决方案，以满足大规模真实世界评估中对透明度和可解释性日益增长的需求。

Method: 本研究采取了以下方法：1) 分析了不同评估利益相关者对可解释自动评分的需求和潜在益处。2) 提出了针对这些需求的四项可解释性原则：忠实性（Faithfulness）、基础性（Groundedness）、可追溯性（Traceability）和互换性（Interchangeability）（FGTI）。3) 开发了AnalyticScore框架作为短答案评分的基准参考，其操作步骤包括：a) 提取回答中明确可识别的元素；b) 使用LLMs将每个回答特征化为人类可解释的值；c) 应用直观的序数逻辑回归模型进行评分。

Result: AnalyticScore在评分准确性方面优于许多不可解释的评分方法，并且在ASAP-SAS数据集的10个项目上，平均仅比不可解释的SOTA低0.06 QWK。此外，通过与执行相同特征化任务的人类标注员进行比较，研究进一步证明了AnalyticScore的特征化行为与人类高度一致。

Conclusion: 本研究通过提出FGTI可解释性原则并开发AnalyticScore框架，证明了实现可解释自动评分的可行性。AnalyticScore不仅在准确性上表现出色，而且其特征化过程与人类行为高度一致，为未来可解释自动评分系统的研究提供了基准。
...
### [154] [LangMark: A Multilingual Dataset for Automatic Post-Editing](https://arxiv.org/abs/2511.17153)
*Diego Velazquez,Mikaela Grace,Konstantinos Karageorgos,Lawrence Carin,Aaron Schliem,Dimitrios Zaikis,Roger Wechsler*

Main category: cs.CL

TL;DR: 本文发布了一个名为LangMark的新型多语言人工标注APE数据集，包含206,983个三元组，用于英译七种语言。研究表明，利用该数据集，大型语言模型（LLMs）通过少样本提示能有效执行APE，超越现有商业机器翻译系统。


<details>
  <summary>Details</summary>
Motivation: 尽管神经机器翻译（NMT）取得了进展，但由于缺乏专门针对NMT输出的大规模多语言数据集，有效的自动译后编辑（APE）系统的开发受到了阻碍。

Method: 研究团队构建并发布了LangMark数据集，这是一个由专业人类语言学家标注的多语言APE数据集，包含206,983个三元组（源语段、NMT输出、人工译后编辑译文），涵盖英语到巴西葡萄牙语、法语、德语、意大利语、日语、俄语和西班牙语七种语言。在此数据集上，他们实证研究了大型语言模型（LLMs）通过少样本提示执行APE的能力。

Result: 实证结果表明，利用LangMark数据集，大型语言模型（LLMs）通过少样本提示能够有效地执行APE任务，并且在提高翻译质量方面超越了领先的商业甚至专有机器翻译系统。

Conclusion: LangMark这一新资源将极大地促进未来自动译后编辑（APE）系统的开发和评估。
...
### [155] [Learning to Compress: Unlocking the Potential of Large Language Models for Text Representation](https://arxiv.org/abs/2511.17129)
*Yeqin Zhang,Yizheng Zhao,Chen Hu,Binxing Jiao,Daxin Jiang,Ruihang Miao,Cam-Tu Nguyen*

Main category: cs.CL

TL;DR: 本文提出并探索了一种上下文压缩的预训练任务，用于无监督地适应大型语言模型（LLMs）以生成更好的文本表示，并通过对比学习进一步提升，构建了高性能且数据效率更高的LLM2Comp模型。


<details>
  <summary>Details</summary>
Motivation: LLMs虽然强大，但其因果性和针对下一词预测的优化使其在生成整体文本表示方面表现不佳。现有的LLM文本表示适应方法大多依赖于掩码下一词预测（MNTP）等词元级预测目标，这可能不是最优的。

Method: 本文提出将上下文压缩作为LLMs的无监督预训练任务。在压缩预训练期间，模型学习生成紧凑的记忆词元，这些词元可以替代整个上下文进行下游序列预测。在此基础上，通过对比学习进一步增强模型，创建了LLM2Comp。

Result: 实验表明，精心设计的压缩目标可以显著提升基于LLM的文本表示，优于使用词元级预训练任务训练的模型。通过对比学习进一步改进后，LLM2Comp成为一个强大的表示模型，在广泛的任务上超越了当前的基于LLM的文本编码器，同时具有更高的样本效率，需要显著更少的训练数据。

Conclusion: 上下文压缩是一个有潜力的预训练任务，能够有效增强LLMs的文本表示能力。结合对比学习，可以构建出高性能、高数据效率的文本表示模型，如LLM2Comp，在多种任务上表现出色。
...
### [156] [Hallucinate Less by Thinking More: Aspect-Based Causal Abstention for Large Language Models](https://arxiv.org/abs/2511.17170)
*Vy Nguyen,Ziqi Xu,Jeffrey Chan,Estrid He,Feng Xia,Xiuzhen Zhang*

Main category: cs.CL

TL;DR: 本文提出了一种名为ABCA（Aspect-Based Causal Abstention）的新框架，通过因果推断分析大型语言模型（LLM）内部知识的多样性，实现早期弃权，从而在生成响应前有效防止幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常产生流畅但不准确的“幻觉”响应。现有弃权方法多依赖于生成后的信号，限制了其提前预防不可靠响应的能力。

Method: ABCA框架通过因果推断分析LLM内部知识的多样性（即从不同来源/方面，如学科、法律背景、时间框架获取的参数化知识）。它估计以这些方面为条件的因果效应，以评估与查询相关的知识可靠性。基于这些估计，实现两种弃权类型：Type-1（知识冲突，当方面效应不一致时）和Type-2（知识不足，当方面效应一致支持弃权时）。

Result: 实验结果表明，ABCA提高了弃权可靠性，达到了最先进的性能，并增强了弃权决策的可解释性。

Conclusion: ABCA是一个有效且可解释的早期弃权框架，它利用LLM内部知识多样性和因果推断，显著改善了模型处理不确定信息时的可靠性，并能提前识别并避免幻觉。
...
### [157] [The PLLuM Instruction Corpus](https://arxiv.org/abs/2511.17161)
...
TL;DR: 本文介绍了用于微调PLLuM（波兰大型语言模型）的指令数据集，并发布了其代表性子集，以指导其他LLM类似数据集的开发。


<details>
  <summary>Details</summary>
Motivation: 研究动机是描述PLLuM项目中用于微调大型语言模型的指令数据集，并分享关于使用不同类型（人工编写与合成）指令数据集对基础LLM语言适应性影响的观察结果。

Method: 本文采用的方法是提出并展示了PLLuM中使用的有机、转换和合成指令的功能类型学，并基于这些数据对人工编写与合成指令数据集在LLM语言适应性中的影响进行了观察和分析。此外，还发布了PLLuM指令语料库（PLLuMIC）的第一个代表性子集。

Result: 研究结果包括：提出了PLLuM指令的功能类型学，涵盖了有机、转换和合成指令；分享了关于使用人工编写和合成指令数据集对基础LLM语言适应性影响的观察结果；发布了PLLuM指令语料库（PLLuMIC）的第一个代表性子集。

Conclusion: 结论是，所发布的PLLuMIC子集对于指导和规划其他大型语言模型类似数据集的开发具有实用价值。
...
TL;DR: 该研究首次在大规模纯AMD MI300X硬件上进行了MoE预训练，并提供了系统和模型设计的实用指导，证明了AMD硬件栈在大规模预训练中的竞争力。


<details>
  <summary>Details</summary>
Motivation: 旨在评估和展示AMD MI300X GPU和Pollara互连在大规模MoE预训练中的能力，并为系统和模型设计提供实用指导。

Method: 方法包括：系统层面，对集群和网络进行全面表征，包括Pollara互连上所有核心集合通信（all-reduce, reduce-scatter, all-gather, broadcast）的微基准测试，以及MI300X GPU在核大小和内存带宽方面的微基准测试。模型层面，引入并应用了针对MI300X优化的Transformer尺寸规则，并优化了MoE宽度以兼顾训练吞吐量和推理延迟。详细描述了训练栈，包括容错、检查点重塑和训练配方。开发了名为ZAYA1（7.6亿活跃参数，83亿总参数MoE）的基础模型。

Result: 研究提供了Pollara互连和MI300X GPU的全面微基准测试结果。所开发的ZAYA1-base模型在推理、数学和编码基准测试中，性能与Qwen3-4B和Gemma3-12B等领先基础模型相当，并优于Llama-3-8B和OLMoE等模型。

Conclusion: 研究结果表明，AMD的硬件（MI300X）、网络（Pollara）和软件栈已经足够成熟和优化，能够支持具有竞争力的大规模预训练任务。
...
### [159] [Attention-Guided Feature Fusion (AGFF) Model for Integrating Statistical and Semantic Features in News Text Classification](https://arxiv.org/abs/2511.17184)
*Mohammad Zare*

Main category: cs.CL

TL;DR: 本文提出了一种注意力引导的特征融合（AGFF）模型，通过注意力机制结合统计和语义特征，显著提高了新闻文本分类的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统方法（如TF-IDF）能捕获词频重要性但缺乏上下文语义，而现代深度学习方法虽能理解上下文但可能忽略高影响力的统计指标。研究动机是寻找一种方法，能同时利用两者的优势，克服各自的局限性。

Method: 本文引入了注意力引导的特征融合（AGFF）模型。该模型在一个统一框架内结合了统计特征（如词频、TF-IDF值）和语义特征（深度学习模型提取）。它采用基于注意力的机制，动态地确定每种特征类型的相对重要性，从而做出更明智的分类决策。

Result: AGFF模型在基准新闻数据集上表现出优于传统统计模型和纯语义深度学习模型的性能。结果证实，策略性地整合不同类型的特征可以显著提高分类准确性。此外，消融研究验证了融合过程中每个组件的贡献。

Conclusion: 研究结果强调了AGFF模型平衡和利用统计与语义表示互补优势的能力，使其成为解决实际新闻分类任务的实用且有效的解决方案。
...
### [160] [AutoLink: Autonomous Schema Exploration and Expansion for Scalable Schema Linking in Text-to-SQL at Scale](https://arxiv.org/abs/2511.17190)
*Ziyang Wang,Yuanlei Zheng,Zhenbiao Cao,Xiaojin Zhang,Zhongyu Wei,Pei Fu,Zhenbo Luo,Wei Chen,Xiang Bai*

Main category: cs.CL

TL;DR: AutoLink是一个自主智能体框架，通过迭代和智能体驱动的过程，动态识别和扩展相关的数据库模式子集，解决了工业级Text-to-SQL中模式链接的扩展性和召回率问题，尤其适用于大型数据库。


<details>
  <summary>Details</summary>
Motivation: 在工业级Text-to-SQL中，将整个数据库模式提供给大型语言模型（LLM）是不切实际的，因为它受限于上下文窗口大小且包含无关噪声。因此，模式链接（筛选出相关模式子集）至关重要。然而，现有方法成本高昂，难以平衡召回率与噪声，且在大规模数据库上扩展性差。

Method: 本文提出了AutoLink，一个将模式链接重构为迭代、智能体驱动过程的自主智能体框架。在LLM的引导下，AutoLink动态探索并扩展链接的模式子集，逐步识别必要的模式组件，而无需输入完整的数据库模式。

Result: AutoLink在Bird-Dev上实现了97.4%的严格模式链接召回率和68.7%的执行准确率（优于CHESS），在Spider-2.0-Lite上实现了91.2%的召回率和34.9%的执行准确率（官方排行榜第二）。关键是，AutoLink在大规模模式（如超过3,000列）上展现出卓越的可扩展性，保持高召回率、高效的token消耗和稳定的执行准确率，而现有方法在此类情况下性能严重下降。

Conclusion: AutoLink为工业Text-to-SQL系统提供了一个高度可扩展、高召回率的模式链接解决方案，有效解决了现有方法在大规模数据库上的局限性，特别是在上下文限制和效率方面。
...
### [161] [Parrot: Persuasion and Agreement Robustness Rating of Output Truth -- A Sycophancy Robustness Benchmark for LLMs](https://arxiv.org/abs/2511.17220)
*Yusuf Çelebi,Mahmoud El Hussieni,Özay Ezerceli*

Main category: cs.CL

TL;DR: 本研究提出了PARROT框架，通过双盲评估、置信度量化和行为分类，衡量大型语言模型（LLMs）在权威和说服等社会压力下准确性的退化和盲从现象。结果显示模型间存在显著异质性，先进模型表现出较低的盲从率和准确性损失，而较旧/较小的模型则表现出严重的认知崩溃，并降低对正确答案的置信度。研究强调“抵制过拟合压力”应成为LLM安全部署的核心目标之一。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于测量大型语言模型在社会压力下（如权威和说服）的准确性下降和盲从现象。这种现象，即过度顺从，可能导致模型输出不准确甚至有害，从而影响其在现实世界中的安全部署。

Method: 本研究采用PARROT框架，具体方法包括：(i) 通过比较同一问题的中立版本和权威性错误版本进行双盲评估，以隔离因果效应；(ii) 使用基于对数似然的校准跟踪来量化模型对正确和强加错误响应的置信度变化；(iii) 运用八种行为分类法（如鲁棒正确、盲从同意、强化错误、顽固错误、自我纠正等）系统地分类失败模式。研究评估了22个模型，使用了来自13个领域的1302个MMLU风格多项选择题和特定领域权威模板。

Result: 研究发现模型间存在显著异质性：先进模型（如GPT-5、GPT-4.1、Claude Sonnet 4.5）的“跟随率”较低（≤11%，GPT-5为4%），准确性损失最小；而较旧/较小的模型则显示出严重的认知崩溃（GPT-4为80%，Qwen 2.5-1.5B为94%）。此外，危险不仅限于响应变化，弱模型还会降低对正确响应的置信度，同时增加对强加错误响应的置信度。在领域层面，国际法和全球知识表现出高脆弱性，而初等数学相对具有弹性。

Conclusion: 本研究得出结论，LLM的“抵制过拟合压力”能力应与准确性、避免危害和隐私一起，被视为在现实世界中安全部署的主要目标。
...
### [162] [A Simple Yet Strong Baseline for Long-Term Conversational Memory of LLM Agents](https://arxiv.org/abs/2511.17208)
*Sizhe Zhou*

Main category: cs.CL

TL;DR: 本文提出了一种以事件为中心的记忆方法，用于提高基于LLM的对话代理在多轮会话中的连贯性和个性化。它将对话历史表示为事件式命题，并存储在异构图中，通过检索和过滤机制，在更短的QA上下文中超越了现有基线。


<details>
  <summary>Details</summary>
Motivation: 基于大型语言模型（LLM）的对话代理难以在多轮会话中保持连贯和个性化的互动，原因在于固定的上下文窗口限制了历史信息的保留，而现有的外部记忆方法在粗粒度检索和细粒度但碎片化的对话视图之间存在权衡。

Method: 受新戴维森事件语义学的启发，该方法将对话历史表示为简短的、事件式的命题，这些命题捆绑了参与者、时间线索和最小的局部上下文。具体而言，它指导LLM将每个会话分解为丰富的基本语篇单元（EDU），并将其与会话、EDU及其论元组织在一个异构图中，以支持联想召回。在此表示之上，构建了两种简单的基于检索的变体，使用密集相似性搜索和LLM过滤，并可选择性地通过基于图的传播步骤来连接和聚合相关EDU的证据。

Result: 在LoCoMo和LongMemEval$_S$基准测试中，这些以事件为中心的记忆方法在操作更短的QA上下文的同时，达到或超越了强大的基线性能。

Conclusion: 结构简单、事件级的记忆为长周期对话代理提供了一个原则性且实用的基础。
...
### [163] [Social-Media Based Personas Challenge: Hybrid Prediction of Common and Rare User Actions on Bluesky](https://arxiv.org/abs/2511.17241)
*Benjamin White,Anastasia Shimorina*

Main category: cs.CL

TL;DR: 本文提出了一种混合方法，用于预测社交媒体用户（包括常见和罕见行为）的多种动作，并在大型Bluesky数据集上进行了评估，取得了优异的性能，并在相关挑战中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 理解和预测社交媒体用户行为对内容推荐和平台设计至关重要。现有方法主要关注转发和点赞等常见行为，而对罕见但重要行为的预测研究不足。

Method: 该研究采用混合方法，结合四种互补策略：(i) 基于历史响应模式的查找数据库系统；(ii) 针对常见行为的、带有时间与语义特征的个性化LightGBM模型；(iii) 用于罕见动作分类的融合文本和时间表示的混合神经网络架构；(iv) 文本回复生成。该方法在一个包含640万对话线程、12种用户动作和25个角色集群的大规模Bluesky数据集上进行了评估。

Result: 个性化模型在常见动作预测上取得了平均0.64的宏观F1分数，而罕见动作分类器在10种罕见动作上取得了0.56的宏观F1分数。该方法在COLM 2025的SocialSim: Social-Media Based Personas挑战赛中获得第一名。

Conclusion: 有效的社交媒体行为预测需要根据不同动作类型的根本差异，采用量身定制的建模策略。
...
### [164] [Estonian WinoGrande Dataset: Comparative Analysis of LLM Performance on Human and Machine Translation](https://arxiv.org/abs/2511.17290)
*Marii Ojastu,Hele-Andra Kuulmets,Aleksei Dorkin,Marika Borovikova,Dage Särg,Kairit Sirts*

Main category: cs.CL

TL;DR: 本文介绍了WinoGrande基准测试集的爱沙尼亚语翻译和文化适应过程，评估了模型在该数据集上的表现，并探讨了提示工程对机器翻译质量的有限影响，强调了语言专家在数据集翻译中的重要性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在为广泛使用的常识推理基准WinoGrande提供一个本地化和文化适应的爱沙尼亚语版本，以便评估大型语言模型在爱沙尼亚语环境下的语言能力和推理能力，并探究高质量机器翻译的可行性。

Method: 研究方法包括：由翻译专家对WinoGrande测试集进行人工翻译和文化适应；评估专有和开源模型在人工翻译基准上的性能；通过设计详细的提示（结合爱沙尼亚语语言特征和WinoGrande数据集的翻译挑战）来探索机器翻译质量的提升。

Result: 研究发现，模型在人工翻译的爱沙尼亚语数据集上的性能略低于在原始英语测试集上的表现，而在机器翻译数据上的性能明显更差。此外，实验表明，提示工程在翻译质量或模型准确性方面的改进有限。

Conclusion: 研究得出结论，为了确保对大型语言模型语言能力和推理进行可靠和可解释的评估，语言专家在数据集翻译和适应过程中扮演着至关重要的角色，简单的提示工程对机器翻译质量的提升作用有限。
...
### [165] [Humanlike Multi-user Agent (HUMA): Designing a Deceptively Human AI Facilitator for Group Chats](https://arxiv.org/abs/2511.17315)
*Mateusz Jacniacki,Martí Carmona Serrat*

Main category: cs.CL

TL;DR: 该研究提出了HUMA，一个基于LLM的群聊协调代理，它采用类人策略和时机参与多方对话。实验结果显示，参与者无法可靠地区分HUMA和人类协调员，且两者在主观体验上表现相当。


<details>
  <summary>Details</summary>
Motivation: 当前LLM对话代理主要针对一对一、回合制交流，而非自然、异步的群聊。随着AI助手的普及，开发自然、类人的互动模式对于维持用户信任和参与至关重要。

Method: 研究提出了Humanlike Multi-user Agent (HUMA)，一个基于LLM的协调代理，用于多方对话。HUMA采用事件驱动架构，处理消息、回复、回应并模拟真实响应时间。它包含路由器、行动代理和反思三个组件，以适应群聊动态。通过一项有97名参与者的对照研究，在四人角色扮演聊天中比较了AI（HUMA）和人类社区经理（CMs）。

Result: 参与者在两种条件下将社区经理分类为人类的概率接近随机，表明他们无法可靠地区分HUMA代理和人类。主观体验在两种条件下也相当：社区经理的有效性、社交存在感和参与度/满意度差异不大，且效应量很小。

Conclusion: 研究结果表明，在自然的群聊环境中，AI协调代理可以达到人类的质量，同时很难被识别为非人类。
...
### [166] [E$^3$-Pruner: Towards Efficient, Economical, and Effective Layer Pruning for Large Language Models](https://arxiv.org/abs/2511.17205)
...
### [167] [Lost in Translation and Noise: A Deep Dive into the Failure Modes of VLMs on Real-World Tables](https://arxiv.org/abs/2511.17238)
...
### [168] [Large Language Models for Sentiment Analysis to Detect Social Challenges: A Use Case with South African Languages](https://arxiv.org/abs/2511.17301)
...
### [169] [A new kid on the block: Distributional semantics predicts the word-specific tone signatures of monosyllabic words in conversational Taiwan Mandarin](https://arxiv.org/abs/2511.17337)
*Xiaoyun Jin,Mirjam Ernestus,R. Harald Baayen*

Main category: cs.CL

TL;DR: 本文研究了汉语单音节词在自然会话中音高轮廓的实现方式，发现词义（语义）是其音高实现的重要预测因子，挑战了传统的声调理论。


<details>
  <summary>Details</summary>
Motivation: 研究者旨在探讨在自然语境下，汉语单音节词的音高轮廓如何实现，并特别关注词义对音高实现的影响，这与标准汉语声调理论可能存在冲突。

Method: 采用基于语料库的研究方法，使用广义加性模型（GAM）将观察到的音高轮廓分解为多个成分。研究中控制了词长、性别、说话人、声调语境、元音高度和发音位置等变量。通过比较词本身、词义和异形同音词的预测能力，并利用语境化词嵌入来预测音高轮廓。

Result: 即使控制了多种变量，词的效应仍是声调实现的重要预测因子。词义比词本身能更好地预测音高轮廓，异形同音词也表现出不同的音高轮廓。最强有力的证据是，通过语境化词嵌入预测单个词语的音高轮廓，其准确率显著高于随机基线。

Conclusion: 研究结果表明，词义（语义）对汉语单音节词的音高实现至关重要，挑战了传统的汉语声调理论，但与判别词汇模型（Discriminative Lexicon Model）的理论框架吻合。语义在语音学中具有重要地位。
...
### [170] [SMILE: A Composite Lexical-Semantic Metric for Question-Answering Evaluation](https://arxiv.org/abs/2511.17432)
*Shrikant Kendre,Austin Xu,Honglu Zhou,Michael Ryoo,Shafiq Joty,Juan Carlos Niebles*

Main category: cs.CL

TL;DR: 本文提出SMILE，一种新型评估指标，通过结合句子级和关键词级语义理解以及关键词匹配，平衡词汇精确性和语义相关性，在多种QA任务中与人类判断高度相关且计算开销小。


<details>
  <summary>Details</summary>
Motivation: 传统的QA评估指标（如ROUGE、METEOR、EM）过度依赖n-gram词汇相似性，缺乏深层语义理解。BERTScore和MoverScore虽利用上下文嵌入，但缺乏平衡句子级和关键词级语义的灵活性，并忽略词汇相似性。基于LLM的评估器则面临成本高、偏见、不一致和幻觉等问题。

Method: 引入SMILE（Semantic Metric Integrating Lexical Exactness），一种复合方法，结合了句子级语义理解、关键词级语义理解和简易关键词匹配，旨在平衡词汇精度和语义相关性。

Result: 在文本、图像和视频QA任务上的广泛基准测试表明，SMILE与人类判断高度相关，并且计算开销小。

Conclusion: SMILE弥合了词汇和语义评估之间的鸿沟，提供了一种全面且高效的评估方法，能够更准确地反映人类对答案质量的判断。
...
### [171] [Beyond Multiple Choice: A Hybrid Framework for Unifying Robust Evaluation and Verifiable Reasoning Training](https://arxiv.org/abs/2511.17405)
...
TL;DR: MCQA选项可能泄露信号，导致评估不准确和模型猜测。ReVeL框架将MCQA转换为开放式问答（OpenQA），通过重写和LLM验证，提高了RFT的效率和评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 多项选择题问答（MCQA）的选项可能泄露可利用的信号，导致模型在评估中表现出的准确性并不能真实反映其能力，并鼓励模型在强化微调（RFT）过程中进行显式或隐式的答案猜测行为。

Method: ReVeL（Rewrite and Verify by LLM）框架将多项选择题重写为开放式问题，并尽可能保持答案可验证。该框架根据不同的答案类型对问题进行分类，并分别应用不同的重写和验证方案。在RFT应用中，它转换了2万个MCQA示例，并使用GRPO对Qwen2.5-VL模型进行微调。

Result: 通过ReVeL-OpenQA训练的模型在多项选择基准上能匹配MCQA的准确性，并将OpenQA的准确性提高了约6个百分点，表明其具有更好的数据效率和更稳健的奖励信号。在评估中，ReVeL还揭示了MCQA基准中高达20个百分点（相对于OpenQA）的分数虚高，提高了判断准确性，并降低了成本和延迟。

Conclusion: ReVeL框架通过将MCQA转换为可验证的OpenQA，有效解决了MCQA在评估和RFT中存在的问题，提供了更可靠的评估指标和更高效的训练方法，能够更准确地反映模型的真实能力。
...
### [172] [Don't Learn, Ground: A Case for Natural Language Inference with Visual Grounding](https://arxiv.org/abs/2511.17358)
*Daniil Ignatev,Ayman Santeer,Albert Gatt,Denis Paperno*

Main category: cs.CL

TL;DR: 本文提出一种零样本自然语言推理（NLI）方法，通过将语言与视觉上下文关联，利用多模态表示进行推理。


<details>
  <summary>Details</summary>
Motivation: 现有NLI方法可能容易受到文本偏见和表面启发式的影响，因此需要更鲁棒的自然语言理解方法。

Method: 该方法使用文本到图像模型生成前提的视觉表示，然后通过比较这些视觉表示与文本假设来执行推理。论文评估了两种推理技术：余弦相似度和视觉问答（VQA）。此外，还设计了一个对抗性数据集来验证方法的鲁棒性。

Result: 该方法在没有任务特定微调的情况下取得了高精度，并表现出对文本偏见和表面启发式的鲁棒性。对抗性数据集也验证了其鲁棒性。

Conclusion: 研究结果表明，利用视觉模态作为意义表示为鲁棒的自然语言理解提供了一个有前景的方向。
...
### [173] [PUCP-Metrix: A Comprehensive Open-Source Repository of Linguistic Metrics for Spanish](https://arxiv.org/abs/2511.17402)
...
### [174] [Selective Rotary Position Embedding](https://arxiv.org/abs/2511.17388)
...
### [175] [Masked-and-Reordered Self-Supervision for Reinforcement Learning from Verifiable Rewards](https://arxiv.org/abs/2511.17473)
*Zhen Wang,Zhifeng Gao,Guolin Ke*

Main category: cs.CL

TL;DR: MR-RLVR通过“掩码填充”和“步骤重排”构建过程级自监督奖励，有效提升了大型语言模型在数学推理（特别是结果难以直接验证的场景）中的表现，相比传统RLVR有显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放（RLVR）在数学推理（特别是定理证明）中存在局限性，因为中间推理过程至关重要但最终答案难以直接验证。同时，基于token的SFT常导致死记硬背而非生成更长的思维链。

Method: 本文提出MR-RLVR（Masked-and-Reordered RLVR），灵感来源于BERT的自监督任务。它通过“掩码填充”（masked-then-fill）和“步骤重排”（step reordering）构造过程级自监督奖励，从中间推理中提取可学习信号。训练分为两阶段：首先在数学计算和证明数据上进行自监督训练，然后对仅结果可验证的数学计算数据集进行RLVR微调。

Result: 在Qwen2.5-3B和DeepSeek-R1-Distill-Qwen-1.5B模型上，MR-RLVR在AIME24、AIME25、AMC23和MATH500等数据集上，在固定采样和解码预算下，相对于原始RLVR实现了平均相对增益：Pass@1提高9.86%，Pass@5提高5.27%，Pass@8提高4.00%。

Conclusion: 研究结果表明，结合过程感知的自监督信号可以有效增强RLVR在仅结果可验证设置下的可扩展性和性能，从而提升LLM的数学推理能力。
...
### [176] [MobileOcc: A Human-Aware Semantic Occupancy Dataset for Mobile Robots](https://arxiv.org/abs/2511.16949)
*Junseo Kim,Guido Dumont,Xinyu Gao,Gang Chen,Holger Caesar,Javier Alonso-Mora*

Main category: cs.RO

TL;DR: MobileOcc是一个为移动机器人在拥挤环境中进行3D语义占用感知而设计的全新数据集，包含创新的行人占用建模方法，并为占用预测和行人速度预测任务建立了基准。


<details>
  <summary>Details</summary>
Motivation: 移动机器人在行人密集环境中进行密集的3D语义占用感知至关重要，但与自动驾驶领域相比，此领域的研究仍显不足。

Method: 该研究构建了MobileOcc数据集，结合了静态物体占用标注和一种新颖的网格优化框架，专门用于人体占用建模。该框架从2D图像重建可变形人体几何，并使用LiDAR点数据进行精炼和优化。基于MobileOcc，研究建立了占用预测和行人速度预测两项任务的基准，并使用单目、立体和全景占用等方法进行评估。此外，其标注方法还在3D人体姿态估计数据集上进行了评估。

Result: MobileOcc数据集为占用预测和行人速度预测任务提供了可复现的基准。实验结果表明，提出的标注方法在不同数据集上均表现出鲁棒的性能。

Conclusion: MobileOcc数据集及其创新的人体占用建模方法填补了移动机器人在拥挤环境中3D语义占用感知研究的空白，为未来的研究提供了重要资源和基准。
...
### [177] [Stable Offline Hand-Eye Calibration for any Robot with Just One Mark](https://arxiv.org/abs/2511.17001)
*Sicheng Xie,Lingchen Meng,Zhiying Du,Shuyuan Tu,Haidong Cao,Jiaqi Leng,Zuxuan Wu,Yu-Gang Jiang*

Main category: cs.RO

TL;DR: CalibAll是一种简单有效的相机外参估计方法，仅需一个标记，通过粗到精的校准流程，实现了免训练、稳定、准确的估计，并优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在机器人任务中取得了显著成功，而相机外参信息对学习过程有益。然而，相机外参通常难以获取，且现有估计方法常受局部最优和泛化性差的困扰。

Method: 该方法在末端执行器上标注一个标记，利用视觉基础模型自动定位该标记。结合点跟踪和3D末端执行器轨迹，通过时间PnP获得粗略的相机外参。随后，通过基于渲染的优化（对齐渲染和真实掩码）进一步精炼，以获得准确稳定的相机外参。

Result: 实验结果表明，CalibAll在三个机器人平台上表现出比现有方法更优的性能，具有强大的鲁棒性和普遍有效性。它还生成了深度图、链接掩码和末端执行器2D轨迹等有用的辅助标注，可支持下游任务。

Conclusion: CalibAll提供了一种简单、有效、免训练、稳定且准确的相机外参估计方法，解决了模仿学习中的一个关键挑战，并能提供有价值的辅助数据。
...
### [178] [Multi-UAV Swarm Obstacle Avoidance Based on Potential Field Optimization](https://arxiv.org/abs/2511.16911)
*Yendo Hu,Yiliang Wu,Weican Chen*

Main category: cs.RO

TL;DR: 本研究提出了一种混合算法，结合改进的多机器人编队避障（MRF IAPF）和增强型APF，用于解决多无人机场景中传统APF路径冗余、航向不稳及碰撞问题，显著优化了路径长度和航向稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统人工势场（APF）方法在多无人机场景中存在路径冗余、航向频繁突变以及无人机间易发生碰撞的问题，特别是在避障过程中。

Method: 本研究提出了一种新型混合算法，结合了改进的多机器人编队避障（MRF IAPF）算法和针对单无人机路径规划优化的增强型APF。核心思想包括：1. 整合了MRF IAPF的障碍物排斥力、无人机间相互作用力以及目标吸引力三种力；2. 引入了精细化的单无人机路径优化机制，包括碰撞风险评估和辅助子目标策略。在高碰撞威胁下，生成临时航点引导避障，确保最终到达实际目标。

Result: 仿真结果表明，与传统基于APF的编队算法相比，所提出的算法在路径长度优化和航向稳定性方面取得了显著改进，能够有效避开障碍物并快速恢复编队构型。

Conclusion: 该算法在具有未知障碍物的静态环境中具有良好的适用性和有效性，能有效解决多无人机避障问题。
...
### [179] [Single-Pixel Tactile Skin via Compressive Sampling](https://arxiv.org/abs/2511.16898)
*Ariel Slepyan,Laura Xing,Rudy Zhang,Nitish Thakor*

Main category: cs.RO

TL;DR: 本文提出了一种名为单像素触觉皮肤（SPTS）的新范式，通过压缩采样技术，利用单个输出通道从整个传感器阵列重建丰富的触觉信息，从而解决了大面积、高速电子皮肤的布线复杂性和数据瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 开发大面积、高速的电子皮肤对于机器人、假肢和人机界面至关重要，但其发展受到布线复杂性和数据瓶颈的根本限制。

Method: SPTS通过在电路层面直接实现分布式压缩感知，每个传感元件配备微型控制器，贡献一个动态加权的模拟信号到一个全局总和。这种灵活、可菊花链连接的设计将布线简化为几条输入线和一条输出线，并显著减少了测量需求。

Result: 该系统实现了每秒3500帧的有效物体分类，并能捕捉瞬态动力学，将8毫秒的弹丸冲击解析为23帧。一个关键特性是支持自适应重建，感知保真度随测量时间而变化，仅用7%的数据即可快速定位接触，随后逐步细化以获得高保真图像。

Conclusion: 这项工作为机器人和人机界面实现大规模触觉智能提供了一条高效途径。
...
### [180] [A*-based Temporal Logic Path Planning with User Preferences on Relaxed Task Satisfaction](https://arxiv.org/abs/2511.16844)
*Disha Kamale,Xi Yu,Cristian-Ioan Vasile*

Main category: cs.RO

TL;DR: 本文提出了一种基于A*的规划框架，结合用户偏好，在大型机器人环境中为时间逻辑任务生成近似最优的高层轨迹，并引入了一种高效启发式算法来解决大规模问题。


<details>
  <summary>Details</summary>
Motivation: 在大型机器人环境中，完全遵守任务通常难以实现。因此，研究的动机是在无法完全满足任务时，通过将用户对放松的偏好整合到规划过程中，实现最佳的任务满意度。

Method: 本研究利用基于自动机的表示方法来表示时间逻辑目标和用户偏好，并提出了一种基于A*的规划框架。为了应对大规模问题，该方法引入了一种简单高效的启发式算法，以显著减少规划时间和搜索内存。

Result: 所提出的A*规划框架能够有效处理大规模问题，生成近似最优的高层轨迹。与无信息搜索算法相比，所提出的启发式算法能够以更少的时间和搜索内存在大规模机器人环境中进行规划。通过广泛的案例研究，验证了该方法的可扩展性、运行时性能以及启发式算法的次优性经验界限。

Conclusion: 本研究提出的基于A*的规划框架，结合高效启发式算法，为在大型机器人环境中进行时间逻辑任务规划提供了一个可扩展且有效的解决方案，即使无法完全满足任务，也能实现近似最优的任务满意度。
...
### [181] [MfNeuPAN: Proactive End-to-End Navigation in Dynamic Environments via Direct Multi-Frame Point Constraints](https://arxiv.org/abs/2511.17013)
*Yiwen Ying,Hanjing Ye,Senzi Luo,Luyao Liu,Yu Zhan,Li He,Hong Zhang*

Main category: cs.RO

TL;DR: 本文提出一种利用多帧点约束（包括当前和预测的未来帧）的框架，通过预测移动障碍物的未来路径，实现机器人主动的端到端导航，以应对复杂动态环境中的实时避障挑战。


<details>
  <summary>Details</summary>
Motivation: 在复杂动态环境中，机器人实时导航中的避障是一个关键挑战。传统的模型基方法和学习基方法在高度动态场景中表现不佳，因为前者假设环境是静态的，而后者依赖单帧观测进行运动约束估计，限制了它们的适应性。

Method: 提出一种新颖的框架，利用多帧点约束（包括当前帧和由专用模块预测的未来帧）。该方法包含一个预测模块，基于多帧观测预测移动障碍物的未来路径，从而使机器人能够主动地预测和规避潜在危险，实现主动的端到端导航。

Result: 这种主动规划能力显著增强了机器人在未知动态环境中的导航鲁棒性和效率。通过仿真和真实世界实验验证了该方法的有效性。

Conclusion: 通过整合多帧预测模块，本方法成功克服了传统和学习基避障方法的局限性，在复杂动态环境中实现了更鲁棒、更高效的机器人主动避障导航。
...
### [182] [H-GAR: A Hierarchical Interaction Framework via Goal-Driven Observation-Action Refinement for Robotic Manipulation](https://arxiv.org/abs/2511.17079)
*Yijie Zhu,Rui Shao,Ziyang Liu,Jie He,Jizhihui Liu,Jiuru Wang,Zitong Yu*

Main category: cs.RO

TL;DR: H-GAR是一个用于机器人操作的统一视频和动作预测的分层框架，通过目标驱动的观察-动作细化，以粗到精的方式处理预测，实现了更准确和连贯的操作，并达到了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 统一的视频和动作预测模型对机器人操作潜力巨大，但现有方法通常以单一且与目标无关的方式处理观察和动作生成，导致预测语义不一致和行为不连贯。因此，需要一种能将预测锚定到任务目标，并实现观察与动作之间明确交互的方法。

Method: 本文提出了H-GAR（通过目标驱动的观察-动作细化实现的分层交互框架）。H-GAR首先生成一个目标观察和粗略的动作草图，勾勒出通往目标的高级路线。为实现观察与动作在目标观察指导下的明确交互，设计了两个协同模块：1) 目标条件观察合成器（GOS），根据粗粒度动作和预测的目标观察合成中间观察。2) 交互感知动作细化器（IAAR），利用中间观察的反馈和编码先前动作以确保时间一致性的历史动作记忆库，将粗略动作细化为精细的、与目标一致的动作。通过以粗到精的方式整合目标基础与明确的动作-观察交互，H-GAR实现了更精确的操作。

Result: H-GAR使机械臂操作更加准确。在模拟和真实世界的机器人操作任务中，H-GAR均达到了最先进的性能。

Conclusion: 通过将目标定位与明确的动作-观察交互以粗到精的方式相结合，H-GAR显著提高了机器人操作中统一视频和动作预测的准确性和连贯性，为未来的机器人规划提供了有效途径。
...
### [183] [Progress-Think: Semantic Progress Reasoning for Vision-Language Navigation](https://arxiv.org/abs/2511.17097)
...
TL;DR: 本文提出Progress-Think，通过引入语义进度推理，从视觉观察中预测指令式进度，以解决视觉-语言导航中智能体长期连贯行动的问题。该方法采用三阶段框架进行无昂贵标注的训练，并在R2R-CE和RxR-CE上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型和数值进度预测方法忽略了观察和指令序列之间的单调协同进展特性，导致在长距离导航中缺乏对本地视觉上下文和指令进展的连贯理解，影响导航准确性。

Method: 本文提出Progress-Think，核心是语义进度推理。它包含一个三阶段框架：1) **自对齐进度预训练**：通过视觉历史与指令前缀之间的可微分对齐，引导推理模块学习。2) **进度引导策略预训练**：将学习到的进度状态注入导航上下文，指导策略采取一致行动。3) **进度-策略联合微调**：使用定制的进度感知强化目标共同优化两个模块。

Result: 在R2R-CE和RxR-CE基准测试中，Progress-Think实现了最先进的成功率和效率。实验证明语义进度能提供更一致的导航进展表示。

Conclusion: 语义进度推理能够显著提升视觉-语言导航的准确性和一致性。Progress-Think通过其独特的三阶段训练框架，无需昂贵标注即可学习和利用这种语义进度，从而在复杂导航任务中取得卓越表现。
...
### [184] [Reflection-Based Relative Localization for Cooperative UAV Teams Using Active Markers](https://arxiv.org/abs/2511.17166)
...
### [185] [Efficient Robot Design with Multi-Objective Black-Box Optimization and Large Language Models](https://arxiv.org/abs/2511.17178)
*Kento Kawaharazuka,Yoshiki Obinata,Naoaki Kanazawa,Haoyu Jia,Kei Okada*

Main category: cs.RO

TL;DR: 本研究提出了一种结合大型语言模型（LLMs）的黑盒优化方法，以提高机器人本体设计优化的效率。


<details>
  <summary>Details</summary>
Motivation: 机器人设计优化方法多样，数值优化速度快但不适用于复杂结构或离散值。黑盒优化虽适用于此类情况，但采样效率低，需要大量迭代才能找到好的解决方案。

Method: 在黑盒优化采样过程的同时，利用LLMs进行并行采样。LLMs被提供问题设置和广泛的反馈，从而辅助优化过程。

Result: 该方法能够更有效地探索设计解决方案。

Conclusion: 本研究证明了所提方法能提高机器人本体设计效率，并讨论了其特点和局限性。
...
### [186] [TP-MDDN: Task-Preferenced Multi-Demand-Driven Navigation with Autonomous Decision-Making](https://arxiv.org/abs/2511.17225)
*Shanshan Li,Da Huang,Yu He,Yanwei Fu,Yu-Gang Jiang,Xiangyang Xue*

Main category: cs.RO

TL;DR: 本文提出了一种新的基准TP-MDDN，用于解决具有多重需求和任务偏好的长程导航问题，并提出了AWMSystem系统，结合LLM模块和空间记忆MASMap，在感知准确性和导航鲁棒性方面超越了现有基线。


<details>
  <summary>Details</summary>
Motivation: 传统的需求驱动导航（DDN）一次只能处理一个需求，无法反映现实世界中涉及多个需求和个人选择的复杂任务，因此需要一个能处理多需求和任务偏好的导航基准。

Method: 引入了任务偏好多需求驱动导航（TP-MDDN）基准。提出了AWMSystem自主决策系统，包含BreakLLM（指令分解）、LocateLLM（目标选择）和StatusMLLM（任务监控）三个关键模块。设计了MASMap空间记忆，结合3D点云累积和2D语义映射。采用Dual-Tempo动作生成框架，整合零样本规划和基于策略的精细控制，并辅以自适应错误校正器处理故障。

Result: 实验结果表明，该方法在感知准确性和导航鲁棒性方面均优于最先进的基线方法。

Conclusion: 该研究成功地引入了一个更符合现实世界复杂性的多需求导航基准，并提出了一个结合LLM、高效空间记忆和鲁棒动作生成机制的系统，有效解决了长程多需求导航问题，并取得了卓越的性能。
...
### [187] [Leveraging CVAE for Joint Configuration Estimation of Multifingered Grippers from Point Cloud Data](https://arxiv.org/abs/2511.17276)
*Julien Merand,Boris Meden,Mathieu Grossard*

Main category: cs.RO

TL;DR: 本文提出一种高效方法，利用条件变分自编码器（CVAE）从多指机械手的点云数据中直接推断其关节配置，速度快且精度高。


<details>
  <summary>Details</summary>
Motivation: 传统的逆运动学（IK）技术在确定多指机械手关节配置时，要么仅依赖指尖姿态而忽略中间指节，导致需要额外的后处理决策；要么对复杂运动学系统采用数值逼近，效率较低。研究旨在寻找一种更高效、隐式的方法，直接从视觉数据（点云）中确定关节配置。

Method: 该方法采用条件变分自编码器（CVAE）。它以关键结构元素（如机械手手指）的点云数据作为输入，然后重建出相应的关节配置。通过机器学习模型隐式地克服了传统方法的挑战。

Result: 该方法在MultiDex抓取数据集和Allegro机械手上进行了验证，运行时间仅为0.05毫秒，且精度与现有最先进方法相当。这表明该流水线在关节配置估计方面非常有效。

Conclusion: 所提出的基于CVAE的流水线能高效且准确地从点云数据中估计多指机械手的关节配置，为AI驱动的抓取规划提供了有效的技术支持。
...
### [188] [A ROS2 Interface for Universal Robots Collaborative Manipulators Based on ur_rtde](https://arxiv.org/abs/2511.17237)
...
### [189] [Simulation of Active Soft Nets for Capture of Space Debris](https://arxiv.org/abs/2511.17266)
...
### [190] [MonoSpheres: Large-Scale Monocular SLAM-Based UAV Exploration through Perception-Coupled Mapping and Planning](https://arxiv.org/abs/2511.17299)
...
### [191] [FORWARD: Dataset of a forwarder operating in rough terrain](https://arxiv.org/abs/2511.17318)
...
TL;DR: FORWARD是一个高分辨率多模态数据集，记录了伐木归堆机在瑞典崎岖地形上的操作数据，旨在推动林业机械的AI模型开发和仿真。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发用于林业机械（特别是伐木归堆机）交通能力、感知和自主控制的AI模型和算法，同时考虑效率、燃油消耗、安全性及环境影响。此外，还旨在探索利用实地数据自动生成和校准林业机械模拟器及自动化场景描述。

Method: 该研究使用一台大型小松伐木归堆机，配备RTK-GNSS、360度摄像头、操作员振动传感器、内部CAN总线信号记录和多个IMU等传感器。数据以5赫兹频率记录事件时间日志（如驾驶速度、油耗、厘米级车辆位置、起重机使用），并在高分辨率激光扫描（约1500点/平方米）的森林区域内操作。数据还包括带有时间戳的生产日志文件（StanForD标准）、大量视频材料和多种格式的地形数据。约18小时的日常木材提取工作从360度视频材料中被标注成独立的作业元素。此外，还包含了在林道和地形中进行的实验场景规范，包括重复驾驶相同路线（有无钢履带）、不同负载重量和不同目标驾驶速度的实验。

Result: 研究成果是创建并发布了FORWARD数据集，该数据集包含约18小时的伐木归堆机在真实林业作业中收集的高分辨率多模态数据。数据涵盖了传感器读数、事件日志、生产日志、视频素材、高分辨率激光扫描地形数据，以及在不同工况下（如不同履带、负载和速度）进行的实验场景规范和注释。

Conclusion: FORWARD数据集为利用人工智能、仿真和物理测试平台开发林业机械（特别是伐木归堆机）的交通能力、感知和自主控制模型与算法提供了一个宝贵的开放资源。它还有助于探索林业机械模拟器的自动生成和校准，以及自动化场景描述的改进。
...
### [192] [Robot Confirmation Generation and Action Planning Using Long-context Q-Former Integrated with Multimodal LLM](https://arxiv.org/abs/2511.17335)
...
### [193] [IndustryNav: Exploring Spatial Reasoning of Embodied Agents in Dynamic Industrial Navigation](https://arxiv.org/abs/2511.17384)
*Yifan Li,Lichi Li,Anh Dao,Xinyu Zhou,Yicheng Qiao,Zheda Mai,Daeun Lee,Zichen Chen,Zhen Tan,Mohit Bansal,Yu Kong*

Main category: cs.RO

TL;DR: 现有视觉大语言模型（VLLMs）在动态真实世界的空间推理方面面临挑战。本文提出了IndustryNav基准，用于评估VLLMs在动态工业导航中的整体规划和安全行为，发现所有模型在路径规划、避碰和主动探索方面均存在显著不足。


<details>
  <summary>Details</summary>
Motivation: VLLMs作为具身智能体在空间推理方面仍面临重大挑战。现有具身基准主要关注被动、静态的家庭环境，且仅评估孤立的能力，未能捕捉动态、真实世界复杂性中的整体性能。

Method: 引入IndustryNav，一个动态工业导航基准，包含12个手动创建的高保真Unity仓库场景，具有动态物体和人类移动。评估采用PointGoal导航流程，结合第一人称视角和全局里程计来评估整体的局部-全局规划。引入“碰撞率”和“警告率”指标来衡量安全行为和距离估计。对九种最先进的VLLMs进行了全面研究。

Result: 闭源模型保持一致优势，但所有智能体在鲁棒路径规划、避碰和主动探索方面都表现出显著缺陷。

Conclusion: 具身研究迫切需要从被动感知转向要求在动态真实世界环境中进行稳定规划、主动探索和安全行为的任务。
...
### [194] [Vector Cost Behavioral Planning for Autonomous Robotic Systems with Contemporary Validation Strategies](https://arxiv.org/abs/2511.17375)
...
### [195] [Agility Meets Stability: Versatile Humanoid Control with Heterogeneous Data](https://arxiv.org/abs/2511.17373)
...
### [196] [METIS: Multi-Source Egocentric Training for Integrated Dexterous Vision-Language-Action Model](https://arxiv.org/abs/2511.17366)
*Yankai Fu,Ning Chen,Junkai Zhao,Shaozhe Shan,Guocai Yao,Pengwei Wang,Zhongyuan Wang,Shanghang Zhang*

Main category: cs.RO

TL;DR: 本文提出METIS，一个基于多源第一视角数据集预训练的视觉-语言-动作(VLA)模型，旨在解决灵巧操作中通用机器人构建的数据稀缺问题，并在真实世界任务中展现出卓越的泛化性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 构建一个能感知、推理和行动的通用机器人，尤其是在灵巧操作领域，仍面临挑战。主要瓶颈在于缺乏大规模、带动作标注的灵巧技能数据，因为远程操作困难且成本高昂。现有利用人类演示的工作受限于场景和人机视觉差距。

Method: 本文提出METIS，一个视觉-语言-动作(VLA)模型。首先构建EgoAtlas数据集，整合了来自多源的大规模人类和机器人数据，并统一了动作空间。其次，提取运动感知动力学，作为紧凑且离散的运动表示，为VLA训练提供高效和富有表现力的监督。在此基础上，METIS将推理和行动整合到一个统一框架中。

Result: METIS在六个真实世界任务中取得了最高的平均成功率，展现出卓越的灵巧操作能力。实验结果还强调了其在分布外场景中的优越泛化性和鲁棒性。

Conclusion: 研究结果表明METIS是迈向灵巧操作通用模型的重要一步，具有广阔的应用前景。
...
### [197] [Human Imitated Bipedal Locomotion with Frequency Based Gait Generator Network](https://arxiv.org/abs/2511.17387)
...
### [198] [Feasibility of Embodied Dynamics Based Bayesian Learning for Continuous Pursuit Motion Control of Assistive Mobile Robots in the Built Environment](https://arxiv.org/abs/2511.17401)
*Xiaoshan Zhou,Carol C. Menassa,Vineet R. Kamat*

Main category: cs.RO

TL;DR: 本研究提出了一种基于脑启发式贝叶斯推理的框架，用于实现脑机接口（BCI）的连续追踪运动控制，解码加速度基的运动表征，并在预测速度方面比现有方法性能提高72%。


<details>
  <summary>Details</summary>
Motivation: 当前的BCI运动控制系统主要限于离散指令，而无法支持实时调整速度和方向的连续追踪，这对于轮椅使用者在复杂公共空间中导航、社交互动以及灵活移动至关重要。

Method: 研究提出并验证了一个脑启发式贝叶斯推理框架，该框架解码基于加速度的运动表征，与传统的运动学解码和深度学习方法形成对比。方法利用自动相关性确定（ARD）进行特征选择和持续在线学习。使用一个包含16小时EEG数据的公开数据集（来自四名受试者执行运动想象目标跟踪任务）进行验证。

Result: 在会话累积迁移学习设置中，与自回归和基于EEGNet的方法相比，本研究提出的方法将预测速度与真实速度之间的归一化均方误差降低了72%。

Conclusion: 理论上，这些发现实证支持了具身认知理论，并揭示了大脑内在的具身和预测性的运动控制动力学。实践上，将EEG解码建立在生物运动的相同动力学原理上，为实现更稳定和直观的BCI控制提供了有前景的途径。
...
### [199] [MDG: Masked Denoising Generation for Multi-Agent Behavior Modeling in Traffic Environments](https://arxiv.org/abs/2511.17496)
*Zhiyu Huang,Zewei Zhou,Tianhui Cai,Yun Zhang,Jiaqi Ma*

Main category: cs.RO

TL;DR: 本文提出MDG（Masked Denoising Generation），一个统一的生成框架，将多智能体行为建模重构为独立噪声时空张量的重建，通过连续、逐智能体、逐时间步的噪声掩码实现高效、可控的轨迹生成，并能泛化到多种自动驾驶任务。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型和自回归方法在建模真实、交互式多智能体行为时，受限于迭代采样、顺序解码或任务特定设计，导致效率低下且难以复用。

Method: MDG将多智能体行为建模重构为独立加噪时空张量的重建。它不依赖扩散时间步或离散标记化，而是应用连续的、逐智能体、逐时间步的噪声掩码，实现局部去噪和可控轨迹生成，仅需单次或少量前向传播。这种掩码驱动的公式能够在一个模型内泛化到开环预测、闭环仿真、运动规划和条件生成。

Result: MDG在Waymo Sim Agents和nuPlan Planning基准测试中取得了有竞争力的闭环性能，同时提供了高效、一致且可控的开环多智能体轨迹生成。

Conclusion: MDG是一个简单而通用的多智能体行为建模范式，其结果证实了其有效性和多功能性。
...
### [200] [SPEAR-1: Scaling Beyond Robot Demonstrations via 3D Understanding](https://arxiv.org/abs/2511.17411)
...
### [201] [HALO: High-Altitude Language-Conditioned Monocular Aerial Exploration and Navigation](https://arxiv.org/abs/2511.17497)
*Yuezhan Tao,Dexter Ong,Fernando Cladera,Jason Hughes,Camillo J. Taylor,Pratik Chaudhari,Vijay Kumar*

Main category: cs.RO

TL;DR: HALO系统展示了使用单目相机、GPS和IMU进行实时高空航空度量-语义建图和探索，有效解决了远距离3D重建和大规模户外环境精确建图的挑战。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决两个关键挑战：(i) 在远距离下使用视觉进行实时密集3D重建，以及 (ii) 对大规模户外环境进行具有精确场景几何和语义的建图和探索。

Method: 本文提出的系统名为HALO，它结合了单目相机、全球定位系统（GPS）和惯性测量单元（IMU）。该系统能够规划利用度量-语义信息的路径，以完成用自然语言指定的任务。所有模块都可以在机器人上板运行。

Result: 在面积达78,000平方米的大规模模拟环境中，HALO比现有语义探索基线探索时间更少，行驶距离的竞争比率提高了68%。在真实世界的定制四旋翼平台上，HALO所有模块均可板载运行，并能在40米高空、面积达24,600平方米的多样环境中有效自主执行任务。

Conclusion: HALO系统成功实现了实时高空航空度量-语义建图和探索，能够有效支持大规模户外环境中的自主任务执行，并在效率和准确性方面表现出色。
...
### [202] [RoboCOIN: An Open-Sourced Bimanual Robotic Data COllection for INtegrated Manipulation](https://arxiv.org/abs/2511.17441)
...
### [203] [RynnVLA-002: A Unified Vision-Language-Action and World Model](https://arxiv.org/abs/2511.17502)
...
TL;DR: RynnVLA-002是一个统一的视觉-语言-动作（VLA）和世界模型，通过联合学习环境动力学和动作规划，实现VLA模型与世界模型的相互增强，并在模拟和真实机器人任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索VLA模型和世界模型如何通过统一框架相互增强，以更好地理解环境物理并生成更有效的动作，从而提升机器人任务的成功率。

Method: RynnVLA-002采用统一框架，其中世界模型利用动作和视觉输入预测未来图像状态以学习环境物理，进而优化动作生成；VLA模型则根据图像观察生成后续动作，增强视觉理解并支持世界模型的图像生成。该框架实现环境动力学和动作规划的联合学习。

Result: 实验结果表明RynnVLA-002超越了单独的VLA模型和世界模型。在LIBERO模拟基准测试中，未经预训练即达到97.4%的成功率。在真实的LeRobot实验中，其集成世界模型使整体成功率提高了50%。

Conclusion: RynnVLA-002的统一框架成功实现了VLA模型与世界模型的相互增强，通过联合学习环境动力学和动作规划，显著提升了在模拟和真实世界机器人任务中的性能。
...
### [204] [Design, Fabrication, and Measurement of a Hemispherical Multi-Layer Band-Pass Frequency Selective Surface](https://arxiv.org/abs/2511.16777)
*Ali Tehranian,Jordan Budhu,Casey Perkowski,Lance Sookdeo,Kenneth H. Church,Garrett Harris,Carl Pfeiffer*

Main category: eess.SY

TL;DR: 该论文报告了一种半球形多层宽带(7-13 GHz)带通频率选择表面(FSS)，采用高德堡离散化和单元缩放技术设计，并通过增材制造实现，展示了良好的带内插入损耗和带外抑制，并引入了新的测量后处理技术。


<details>
  <summary>Details</summary>
Motivation: 研究的动机是开发一种能够适应曲面轮廓的FSS，以实现未来的共形频率选择表面。

Method: 该研究采用以下方法：
1.  **设计技术**：引入基于高德堡离散化和单元缩放的新设计技术，以适应FSS的曲面轮廓。
2.  **制造技术**：通过增材制造实现，顺序打印介电层和金属图案，将3个图案化的银墨表面集成在4.5毫米厚的ABS半球形雷达罩内。
3.  **测量后处理**：使用一种新的后处理技术来抑制测量传输频谱中边缘衍射的影响。

Result: 主要结果如下：
1.  实现了工作在7-13 GHz的半球形多层宽带带通FSS。
2.  FSS厚度为4.5毫米（在10 GHz处约为λ₀/6），直径约为5λ₀，高度约为3λ₀。
3.  测量结果显示，在通带内插入损耗约为1.7 dB，在阻带内抑制达到15-20 dB。

Conclusion: 该论文的设计过程、制造技术和测量后处理代表了新颖的进步，为未来共形频率选择表面的实现提供了可能。
...
### [205] [The PV performance ratio paradox: annual data from large-scale, real-world PV systems show negligible meteorological and technical impact and points to dominant human factors](https://arxiv.org/abs/2511.16834)
*Hugo FM Milan,Aline Q Alves,Thatiane AT Souza,Juliana M Galo,Alex SC Maia,Moisés AP Borges,Ciro J Egoavil*

Main category: eess.SY

TL;DR: 本研究通过数据驱动模型分析了巴西朗多尼亚州大型光伏系统的性能比（PR），发现气象和技术变量对PR影响微乎其微，而人为因素（如安装、监控和维护质量）影响更大。研究建议政策制定者应关注光伏安装和维护人员的教育培训。


<details>
  <summary>Details</summary>
Motivation: 以往研究已探讨气象和技术变量对光伏系统PR的影响，但对于大型、真实、异构光伏系统中哪些变量对PR有强影响，文献中仍存在空白。本研究旨在填补这一空白，识别强烈影响年度PR的变量，以指导优化。

Method: 研究对位于巴西朗多尼亚州的光伏系统应用了数据驱动模型，以识别影响年度PR的强变量。同时，估算了PR的概率密度函数，并绘制了朗多尼亚州的年度最终发电量地图。

Result: 研究发现气象和技术变量对年度PR的影响可以忽略不计，表明人为因素（如安装、监控和维护质量）可能产生更强的影响。PR的峰值为78.85%（平均值77.52%，95%置信区间为76.12%至78.84%，95%预测区间为58.83%至92.70%）。此外，还开发了朗多尼亚州的年度最终发电量地图。

Conclusion: 为提高光伏系统性能，政策制定者应侧重于创建教育计划，培训光伏安装人员和技术人员正确安装、监控和维护现代光伏系统。开发的年度最终发电量地图可供企业家快速便捷地估算能源产量。
...
### [206] [Experimental Multi-site Testbed for Advanced Control and Optimization of Hybrid Energy Systems](https://arxiv.org/abs/2511.16819)
*Arash Omidi,Tanmay Mishra,Mads R. Almassalkhi*

Main category: eess.SY

TL;DR: 本文介绍了一个混合能源系统(HES)实验测试平台，用于验证电网服务的先进控制和优化策略。


<details>
  <summary>Details</summary>
Motivation: 研究动机是支持电网服务中先进控制和优化策略的原型开发和验证。

Method: 该平台结合了硬件在环(HIL)仿真和可重构的千瓦级资产，包括太阳能光伏(PV)、电池储能、电解槽（作为可控负载）和并网逆变器。它采用统一的监控和通信架构，并通过控制器硬件在环(CHIL)实验（电池系统参与光伏功率平滑）来演示其能力。

Result: 测试平台成功集成了HIL仿真和物理资产，支持实时数据采集、模型验证和控制实施。一个CHIL实验证明了电池系统在光伏功率平滑中的有效参与。

Conclusion: 所开发的混合能源系统测试平台能够有效地支持电网服务中先进控制和优化策略的原型开发和验证，并通过实验成功展示了其能力。
...
### [207] [When Motion Learns to Listen: Diffusion-Prior Lyapunov Actor-Critic Framework with LLM Guidance for Stable and Robust AUV Control in Underwater Tasks](https://arxiv.org/abs/2511.16900)
...
### [208] [State-Derivative Feedback Control for Damping Low-Frequency Oscillations in Bulk Power Systems](https://arxiv.org/abs/2511.16974)
...
### [209] [Feature Partitioning and Semantic Equalization for Intrinsic Robustness in Semantic Communication under Packet Loss](https://arxiv.org/abs/2511.16983)
*Xiao Yang,Shuai Ma,Yong Liang,Guangming Shi*

Main category: eess.SY

TL;DR: 本文研究了语义通信中特征分割策略对丢包鲁棒性的影响。结果表明，Transformer在通道维度分割时具有固有鲁棒性。为增强CNN的鲁棒性，提出了一种语义均衡机制（SEM），通过平衡通道贡献，使CNN在丢包情况下也能实现与Transformer相当的性能。


<details>
  <summary>Details</summary>
Motivation: 语义通信通过关注任务相关信息提高了传输效率，但在基于数据包的通信协议下，任何错误都可能导致整个数据包丢失，使得语义通信对丢包特别脆弱。高维语义特征在数据包化时必须被分割成一维传输单元，因此关键问题是如何分割语义特征以最大化鲁棒性。

Method: 研究了Transformer和卷积神经网络（CNN）两种主流架构在各种特征分割方案下的性能。针对CNN，提出了一种轻量级的语义均衡机制（SEM），该机制包含两个并行方法：动态尺度模块（自适应调整通道重要性）和广播模块（促进通道间信息交互）。

Result: 研究发现，Transformer架构在沿通道维度分割时表现出固有的丢包鲁棒性。相比之下，基于CNN的基线模型存在通道利用不平衡问题，一旦主导通道丢失就会导致严重性能下降。配备SEM的CNN在丢包情况下实现了平稳的性能下降（在40%丢包率下仍能保持无损PSNR的约85%），与Transformer模型相当。

Conclusion: 在适当的分割策略下，保持平衡的语义表示是实现对丢包固有鲁棒性的基本条件。这些发现可能也适用于视频等其他模态，并支持实际的语义通信设计。
...
### [210] [KNN and Time Series Based Prediction of Power Generation from Renewable Resources](https://arxiv.org/abs/2511.17102)
*Ismum Ul Hossain,Mohammad Nahidul Islam*

Main category: eess.SY

TL;DR: 本研究利用机器学习（KNN和SARIMA模型）和长达30年的多可再生能源数据集，开发了一种可再生能源预测环境，以解决现有方法在处理间歇性、非线性和复杂性方面的不足，从而提高电力供应的稳定性。


<details>
  <summary>Details</summary>
Motivation: 随着世界转向利用自然资源发电，需要增强预测系统以保证稳定的电力供应，并将所发电量整合到电网中。现有的预测程序难以应对可再生能源固有的间歇性、非线性和复杂性。

Method: 研究利用一个包含太阳能、风能和水力资源的大约30年综合数据集，评估了K-近邻（KNN）模型和季节性自回归积分滑动平均（SARIMA）模型两种方法来预测总发电量。该框架采用高时间分辨率和多个环境参数来提高预测准确性。

Result: 结果显示，就误差指标而言，KNN和SARIMA模型都表现出同等的显著性，并在特定情况下具有各自独特的倾向。长时间的历史数据（30年）有助于更好地校准模型，以应对发电量的时间波动、季节和气候效应，从而提升预测的可靠性。

Conclusion: 利用30年数据增强的预测功能，显著提高了预测可靠性，对电网运营商、能源交易商以及制定可再生能源政策和标准的机构具有重要价值。
...
### [211] [Continuous Resilience in Cyber-Physical Systems of Systems: Extending Architectural Models through Adaptive Coordination and Learning](https://arxiv.org/abs/2511.17017)
...
### [212] [Comparison of linear observation techniques for robust load torque estimation in actuators](https://arxiv.org/abs/2511.17025)
*Michael Ruderman,Elia Brescia,Luigi P. Savastio,Paolo R. Massenio,David Naso,Giuseppe L. Cascella*

Main category: eess.SY

TL;DR: 本文理论与实验性地比较了两种线性观测器（降阶Luenberger观测器和扰动观测器），用于在只有电机驱动扭矩和角位移可用时，鲁棒估计旋转执行器系统中的外部负载扭矩。


<details>
  <summary>Details</summary>
Motivation: 研究的动机是在仅有有限输入输出数据（电机驱动扭矩和角位移）的情况下，鲁棒地估计旋转执行器系统中的外部负载扭矩。

Method: 本文采用了两种已有的线性观测技术：降阶Luenberger观测器和扰动观测器。两者均使用永磁同步电机（PMSM）执行器的相同已识别模型。研究方法包括理论分析和实验验证，重点关注实现、输入扭矩到估计负载扭矩的传递特性相对阶数、观测器开环传递函数以及对不确定系统植物的灵敏度（和稳定性裕度）。实验案例研究中使用了负载扭矩传感器提供参考测量值以评估两种观测器。

Result: 研究结果突出了与两种观测器的实现、输入扭矩到估计负载扭矩的传递特性相对阶数、观测器开环传递函数以及对固有不确定系统植物的灵敏度（和稳定性裕度）相关的多个方面。通过详细的实验案例研究，利用负载扭矩传感器提供的参考测量值，对两种观测器进行了评估。

Conclusion: 本文通过理论分析和实验验证，对两种线性观测器在旋转执行器系统中鲁棒估计外部负载扭矩的应用进行了比较和评估，揭示了其在实现和面对系统不确定性时的关键特性。
...
### [213] [Power Flow Solution in Unbalanced 3-Wire MV and 4-Wire LV Networks Using Symmetrical and Eigen-basis Coordinates](https://arxiv.org/abs/2511.17104)
...
### [214] [Distributed Switching Model Predictive Control Meets Koopman Operator for Dynamic Obstacle Avoidance](https://arxiv.org/abs/2511.17186)
*Ali Azarbahram,Chrystian Pool Yuca Huanca,Gian Paolo Incremona,Patrizio Colaneri*

Main category: eess.SY

TL;DR: 本文提出了一种Koopman增强的分布式切换模型预测控制（SMPC）框架，用于四旋翼无人机在动态环境中（含移动障碍物）的安全可扩展导航。


<details>
  <summary>Details</summary>
Motivation: 在动态环境中，实现四旋翼无人机（UAV）的安全、可扩展导航，并有效避开移动障碍物，尤其是在地面交通应用中（如协调车流、共享基础设施、城市无人机交通）。

Method: 该方法结合了切换运动模式和数据驱动预测。利用局部Koopman算子根据在线测量将非线性障碍物动力学近似为线性模型，从而实现精确轨迹预测。这些预测被嵌入到分布式SMPC结构中，每个无人机利用本地和集群信息自主决策。

Result: 仿真结果表明，该框架实现了可靠的编队控制和实时避障。

Conclusion: 该框架对智能协作移动系统具有广泛的适用性，尤其在地面交通领域展现出巨大潜力。
...
### [215] [Algorithmic design and implementation considerations of deep MPC](https://arxiv.org/abs/2511.17233)
*Prabhat K. Mishra,Mateus V. Gasparino,Girish Chowdhary*

Main category: eess.SY

TL;DR: 深度模型预测控制（Deep MPC）结合深度学习与MPC，其中神经网络学习模型不确定性，MPC处理约束。本文探讨了其实现挑战、控制权限分配算法，并指出不当分配会导致性能下降。


<details>
  <summary>Details</summary>
Motivation: 将深度学习与模型预测控制（MPC）结合，利用深度神经网络学习模型不确定性，同时MPC处理约束，从而实现在线数据微调神经网络并确保学习过程中的系统安全。

Method: 本文聚焦于一种特定的Deep MPC方法，即深度神经网络与MPC在环工作。神经网络负责学习模型不确定性，MPC负责处理约束。研究解释了实现挑战，提出了控制权限分配的算法方式，并通过一个四轮防滑转向动力学的数值实验来论证不当分配会导致性能下降。

Result: 论文解释了Deep MPC的实现挑战，提出了控制权限分配的算法方式，并指出不当的控制权限分配会导致性能不佳。这一结论通过一个四轮防滑转向动力学系统的数值实验得到了验证。

Conclusion: 在Deep MPC中，将控制权限不当地分配给神经网络（学习不确定性）和MPC（处理约束）可能导致系统性能不佳，这一问题通过实现挑战的讨论和数值实验得到了阐明。
...
### [216] [Influence of Transmission Rank on EMF Exposure Measured With Provoked Data Traffic Around 5G Massive MIMO Base Stations](https://arxiv.org/abs/2511.17212)
*Lisa-Marie Schilling,Christian Bornkessel,Anna-Malin Schiffarth,Thanh Tam Julian Ta,Dirk Heberling,Matthias Hein*

Main category: eess.SY

TL;DR: 本研究调查了5G大规模MIMO网络中传输秩对电磁场暴露水平的影响，发现传输秩的变化会导致显著的暴露差异。


<details>
  <summary>Details</summary>
Motivation: 5G新无线电网络（特别是大规模MIMO技术）的引入，使得电磁场暴露评估变得复杂。传输秩的变化（如波束成形、空间复用层数）会影响基站总传输功率在各层之间的分配，进而影响测量的暴露水平，因此需要研究其具体影响。

Method: 研究在德国5G网络3.6 GHz (n78) 频段进行了现场测量。使用频谱分析仪和各向同性探头，在满负荷交通条件下捕获最大场强。通过使用屏蔽袋人为降低用户设备的接收质量，强制实现单层传输（秩-1），并与未屏蔽操作（允许最多四层独立传输，秩-4）进行比较。

Result: 数据揭示了暴露差异范围为1.7 dB至5.4 dB，在研究的测量点中位数为4.3 dB。

Conclusion: 这些发现强调了在电磁场暴露评估中考虑传输秩的必要性。
...
### [217] [Computing the Hard Scaled Relative Graph of LTI Systems](https://arxiv.org/abs/2511.17297)
*Julius P. J. Krebbekx,Eder Baron-Prada,Roland Tóth,Amritam Das*

Main category: eess.SY

TL;DR: 本文提出了一种计算不稳定LTI系统硬SRG（Scaled Relative Graphs）的系统方法，并探讨了其与奈奎斯特判据的关系，以分析非线性系统中的反馈回路。


<details>
  <summary>Details</summary>
Motivation: Scaled Relative Graphs (SRGs) 是一种分析非线性系统的新型图形频率域方法，其中LTI系统是基本构建块。为了分析包含不稳定LTI组件的反馈回路，需要使用硬SRG，因为它能准确捕捉扩展L2空间上的输入/输出行为。因此，需要开发一种系统化的计算方法来精确计算LTI系统的硬SRG。

Method: 本文开发了一种系统化的计算方法，以精确计算可能不稳定并包含积分器的LTI系统的硬SRG。同时，研究了硬SRG与奈奎斯特判据（包括多变量情况）之间的联系，并通过多个示例进行了方法演示。

Result: 开发出了一种精确计算不稳定LTI系统（包括带积分器系统）硬SRG的系统计算方法。研究并阐明了硬SRG与奈奎斯特判据（包括多变量情况）之间的连接。该方法已通过多个示例进行了验证。

Conclusion: 本文提供了一种精确计算LTI系统硬SRG的系统化方法，即使系统不稳定或包含积分器也能适用。该方法还建立了硬SRG与奈奎斯特判据之间的重要联系，为分析包含不稳定LTI组件的非线性反馈系统提供了工具。
...
### [218] [A Framework for Adaptive Stabilisation of Nonlinear Stochastic Systems](https://arxiv.org/abs/2511.17436)
*Seth Siriya,Jingge Zhu,Dragan Nešić,Ye Pu*

Main category: eess.SY

TL;DR: 本文提出一种基于确定性等价学习的自适应控制策略，用于离散时间、非线性随机系统，其具有线性参数化不确定性，并推导了闭环系统的稳定性界限。


<details>
  <summary>Details</summary>
Motivation: 研究具有线性参数化不确定性的离散时间、非线性随机系统的自适应控制问题，并寻求在特定条件下保证系统稳定性。

Method: 采用确定性等价学习（certainty equivalence learning）的自适应控制策略，并假设存在一个参数化控制器族，在参数选择得当时，能在状态空间的特定信息区域内使系统稳定。

Result: 研究推导了闭环系统在某些概率下成立的稳定性界限。如果整个状态空间都具有信息性，且控制器族在参数选择恰当时能实现全局稳定，则可以推导出高概率的稳定性保证。

Conclusion: 所提出的基于学习的自适应控制策略能够为具有参数不确定性的非线性随机系统提供稳定性保证，且在信息丰富的状态空间和全局稳定控制器族下能获得更强的稳定性保证。
...
### [219] [The Iberian Blackout: A Black Swan or a Gray Rhino? A Thorough Power System Analysis](https://arxiv.org/abs/2511.17433)
*Abdallah Alalem Albustami,Ahmad F. Taha*

Main category: eess.SY

TL;DR: 本文分析了2025年4月28日伊比利亚电力系统因过电压引发的全面停电事件，重建了事件时间线和因果链，识别了促成因素，在IEEE测试系统上重现了停电，并从电压控制角度分析，提出了实用的缓解措施。


<details>
  <summary>Details</summary>
Motivation: 2025年4月28日伊比利亚电力系统发生全面停电，这是欧洲首次有记录的过电压驱动的级联故障，引发了对高可再生能源输出、低惯量和运营商操作等根本原因的广泛讨论。本文旨在通过深入分析，厘清事实，解释停电的发生过程。

Method: ['重建事件的时间线和因果链。', '根据事故报告的事实调查结果，总结并呈现促成因素。', '在IEEE测试系统上重现停电事件。', '从系统理论和电压控制的角度分析事件。', '将分析转化为旨在缓解和预防类似事件的实用技术措施。']

Result: 通过详细的电力系统分析，本文将解释停电如何逐步展开，识别并总结促成因素，成功在IEEE测试系统上重现停电，并从电压控制视角提供深入分析，最终提出实用的技术缓解和预防措施。

Conclusion: 本文通过对伊比利亚电力系统全面停电事件的全面分析，旨在提供对过电压驱动级联故障的清晰理解，并提出具体的实用措施，以减轻和预防未来类似事件的发生。
...
### [220] [MRI Super-Resolution with Deep Learning: A Comprehensive Survey](https://arxiv.org/abs/2511.16854)
...
TL;DR: 高分辨率MRI对临床至关重要但成本高昂且受限。超分辨率（SR），特别是基于深度学习（DL）的方法，能从低分辨率扫描生成高分辨率图像，以克服这些挑战。本综述全面回顾了DL-based MRI SR技术，提出了一个系统分类，探讨了其理论、方法、挑战和未来方向，并提供了相关资源。


<details>
  <summary>Details</summary>
Motivation: 高分辨率磁共振成像（MRI）对于许多临床和研究应用至关重要，但其实现成本高昂，并受到技术权衡和实验限制。超分辨率（SR）提供了一种有前景的计算方法，通过从更经济的低分辨率扫描中生成高分辨率图像来克服这些挑战，有望在不增加硬件成本的情况下提高诊断准确性和效率。

Method: 本研究是一篇综述论文，重点回顾了MRI超分辨率技术的最新进展，特别是深度学习（DL）方法。它从计算机视觉、计算成像、逆问题和MR物理学的角度审视了基于DL的MRI SR方法，涵盖了理论基础、架构设计、学习策略、基准数据集和性能指标。论文提出了一种系统的分类法来对这些方法进行归类，并对现有和新兴的MRI SR技术进行了深入研究。

Result: 该综述提出了一种系统分类法，用于对基于深度学习的MRI超分辨率方法进行分类。它深入研究了适用于MRI的成熟和新兴SR技术，并考虑了临床和研究背景下的独特挑战。此外，论文还强调了社区需要解决的开放性挑战和未来方向，并提供了一系列重要的开放获取资源、工具和教程。

Conclusion: 基于深度学习的超分辨率技术是克服高分辨率MRI成本和技术限制的一种有前景的计算方法。本综述系统地总结了DL-based MRI SR的最新进展，为该领域的研究人员提供了全面的视角、分类框架、挑战分析和实用资源，以指导未来的研究和发展。
...
### [221] [MedImageInsight for Thoracic Cavity Health Classification from Chest X-rays](https://arxiv.org/abs/2511.17043)
...
TL;DR: 本研究利用医学影像基础模型MedImageInsight实现了胸部X光片的自动化正常/异常二分类，通过微调方式取得了与现有SOTA模型相当的性能，旨在减轻放射科医生的工作负担。


<details>
  <summary>Details</summary>
Motivation: 胸部X光片是胸部诊断最常用的影像模态之一，但不断增长的影像量和放射科医生工作量对及时解读构成了挑战，因此需要自动化解决方案来辅助诊断。

Method: 研究评估了两种方法：1) 对MedImageInsight进行端到端微调以进行分类；2) 将MedImageInsight用作特征提取器，结合传统机器学习分类器进行迁移学习。实验结合使用了ChestX-ray14数据集和合作医院的真实临床数据。

Result: 微调后的分类器表现最佳，ROC-AUC达到0.888，并且校准性优于迁移学习模型，其性能与CheXNet等成熟架构相当。这表明基础医学影像模型在减少特定任务训练要求的同时保持了诊断可靠性。

Conclusion: 基础医学影像模型能有效减少特定任务的训练需求并保持诊断可靠性。该系统旨在集成到基于网络的和医院PACS工作流程中，以支持分诊并减轻放射科医生的负担。未来的工作将扩展到多标签病理分类以提供初步诊断解释。
...
### [222] [Avoiding Quality Saturation in UGC Compression Using Denoised References](https://arxiv.org/abs/2511.16876)
*Xin Xiong,Samuel Fernández-Menduiña,Eduardo Pavez,Antonio Ortega,Neil Birkbeck,Balu Adsumilli*

Main category: eess.IV

TL;DR: 本文提出了一种针对用户生成内容（UGC）视频压缩中质量饱和（QS）问题的解决方案。通过引入去噪均方误差（D-MSE）作为替代参考，并开发了两种失真饱和检测方法（DSD和RDSD），以避免无效的比特率使用，从而在AVC编码中实现了8%-20%的BD-rate节省。


<details>
  <summary>Details</summary>
Motivation: 传统的视频编码器通过最小化压缩视频与输入视频之间的均方误差（MSE）来工作。然而，当应用于嘈杂的用户生成内容（UGC）时，这种方法会导致质量饱和（QS），即增加比特率只会保留输入伪影而不会提高视觉质量。现有的QS检测方法效率低下，需要重复评估非参考指标。

Method: 研究人员从噪声信源编码理论的角度重新审视UGC压缩和QS检测。他们提出了一种新的失真度量：去噪均方误差（D-MSE），即压缩视频与去噪UGC之间的MSE。D-MSE在比特率增加时会饱和在非零值，这被称为失真饱和（DS）。D-MSE可以在变换域的块级别高效计算，无需进行多种参数的编码和解码。在此基础上，提出了两种DS检测方法：1) 失真饱和检测（DSD），它依赖于从输入UGC的D-MSE导出的输入相关阈值；2) 速率失真饱和检测（RDSD），它使用低复杂度压缩方法估算饱和点的拉格朗日乘数。这些方法可作为标准兼容编码器的预处理步骤。

Result: D-MSE在比特率增加时会饱和在非零值（失真饱和现象）。通过避免在饱和区域进行编码（即避免在根据本文方法导致QS的量化参数下编码），使用AVC进行的实验表明，与忽略QS的朴素基线相比，在多个不同的非参考指标下，实现了8%-20%的BD-rate节省。

Conclusion: 通过引入D-MSE和两种高效的失真饱和检测方法（DSD和RDSD），本文提供了一种有效的预处理步骤，可以帮助标准兼容编码器在UGC压缩中避免质量饱和，从而显著节省比特率。
...
### [223] [Learning Latent Transmission and Glare Maps for Lens Veiling Glare Removal](https://arxiv.org/abs/2511.17353)
...
TL;DR: 该研究提出了VeilGen，一个无监督生成模型，用于模拟紧凑光学系统中的光晕眩光（veiling glare），并生成带真实复合降级的数据集；同时提出了DeVeiler，一个恢复网络，利用VeilGen预测的潜在图来有效去除眩光。


<details>
  <summary>Details</summary>
Motivation: 紧凑光学系统（如单镜头和超透镜设计）的成像性能，除了常见的光学像差外，还常受到非理想光学表面和涂层散射引起的眩光（veiling glare）的影响，尤其是在复杂真实环境中。这种复合降级损害了传统的镜头像差校正，但尚未得到充分探索。主要挑战在于，传统散射模型不适用于眩光，因为其具有空间变化和深度无关的特性，导致难以通过模拟生成高质量的配对数据，从而阻碍了数据驱动眩光去除模型的发展。

Method: 研究提出了两个主要组件：
1.  **VeilGen**：一个生成模型，通过无监督方式从目标图像中估计其潜在的光学透射率和眩光图，并利用Stable Diffusion (SD) 先验进行正则化，从而学习模拟眩光。VeilGen能够生成包含光学像差和眩光复合降级的真实配对数据集，并提供估计的潜在光学透射率和眩光图以指导眩光去除过程。
2.  **DeVeiler**：一个恢复网络，通过可逆性约束进行训练，利用VeilGen预测的潜在图来指导学习到的散射模型的逆过程。

Result: 在具有挑战性的紧凑光学系统上进行的广泛实验表明，该方法相比现有方法，提供了卓越的恢复质量和物理保真度。结果表明，VeilGen能够可靠地合成逼真的眩光，并且其学习到的潜在图能有效指导DeVeiler的恢复过程。

Conclusion: VeilGen和DeVeiler共同提供了一种有效的方法来模拟和去除紧凑光学系统中的眩光，通过生成逼真的降级数据和利用潜在物理图指导恢复过程，显著优于现有技术，为解决眩光问题提供了新的途径。
...
### [224] [OmniLens++: Blind Lens Aberration Correction via Large LensLib Pre-Training and Latent PSF Representation](https://arxiv.org/abs/2511.17126)
