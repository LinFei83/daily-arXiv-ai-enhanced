...
TL;DR: 本文提出了一种基于模型空间微分几何距离的新型参数剪枝方案，利用Fisher信息度量计算测地线距离，建立了一个剪枝方法的自然最优性层次结构。


<details>
  <summary>Details</summary>
Motivation: 现有的剪枝方法（如基于幅值或局部Fisher信息的方法）缺乏统一的数学理论支撑，无法准确衡量参数置零对模型性能造成的真实改变。

Method: 利用模型空间中的微分几何距离（由Fisher信息度量决定的测地线距离）来计算参数剪枝的最小距离，并通过分析该测地线距离的渐进近似，构建了从传统幅值剪枝到更复杂剪枝方案的自然最优性层次结构。

Result: 在MNIST和CIFAR-10数据集上对全连接网络和视觉Transformer（ViT）进行的实验表明，该方法在准确率和马修斯相关系数（MCC）上均优于传统的幅值剪枝和仅基于局部Fisher信息的方法。

Conclusion: 该几何框架不仅为AI模型提供了一种SOTA的剪枝方法，还为剪枝方案提供了经过验证且具有数学动机的理论基础。
...
TL;DR: 本文研究了使用小型轻量级修正模块（CRN v2）在保持冻结大语言模型基础能力不变的同时修复其输出错误的问题。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以通过一个小型、独立的修正模块来纠正冻结语言模型的错误，同时避免全量微调或LoRA带来的基础能力退化（即修正与能力的权衡问题）。

Method: 提出了CRN v2，一个包含约3400万个可训练参数（占4.65B模型参数的0.73%）的轻量级对数几率（logit-level）修正模块，置于完全冻结的Gemma模型之上。采用监督微调（SFT）和无参考DPO进行训练，并引入KL散度保持项以防止基础能力退化。

Result: CRN v2纠正了基模型53.3%的错误（变体测试为43.3%），且在标准能力基准测试（如MMLU、BoolQ等）中未表现出任何能力退化；相比之下，同等预算的LoRA基线虽能修正更多错误，但会导致30-75%的能力损失。

Conclusion: 通过“冻结基础模型 + 对数几率修正 + KL锚定”的设计原则，可以在不损害大模型基础能力的前提下，有效纠正其输出错误。
...
TL;DR: 本文提出了GPEvac，一个基于图神经网络（GNN）和近端策略优化（PPO）的框架，用于计算枪击事件中的实时自适应疏散路线，最小化威胁暴露。


<details>
  <summary>Details</summary>
Motivation: 大规模枪击事件的激增急剧增加了对实时引导受害者安全疏散系统的需求。现有方法受限于特定布局且计算复杂度高，而实际指南过于粗略，因此需要一种能应对对抗性不确定性和拥挤动力学的高效自适应疏散系统。

Method: 提出了GPEvac框架，采用边缘优先的顺序消息传递方案和可学习的虚拟全局节点来捕获局部与远程依赖关系，并将其整合到置换不变的评分机制中，使单一学习策略能够适应不同拓扑和大小的建筑布局。

Result: 广泛的模拟实验表明，GPEvac在各种建筑布局中均优于智能基线方法，显著减少了总体威胁暴露，并且能够在本地CPU硬件上以14.73毫秒的速度计算出全局疏散路线。

Conclusion: GPEvac能够实现实时、跨布局的自适应疏散路线规划，有效降低枪击事件中的威胁暴露，其方法论也可迁移至其他图结构决策领域。
...
TL;DR: 本文探讨了基于大语言模型（LM）的开放式战略兵棋推演在安全性和决策中的应用风险，并主张不能直接将其作为现实决策依据，而应用作压力测试工具。


<details>
  <summary>Details</summary>
Motivation: 由于LM在开放式兵棋推演中既能充当代理又能裁决行动，其扮演的角色存在固有危险性，LM的语言直接决定了行动意图与模拟现实，因此需要探讨其用于高风险决策的安全性问题。

Method: 通过立场论述，识别并分析了LM赋能兵棋推演中的五种核心失效模式，指出常规基准无法确立此类环境的安全性。

Result: 识别出五大失效模式：决策洗白、裁决不透明、角色崩溃、通过裁决导致的升级，以及战略想象力的失败。

Conclusion: 任何LM赋能的兵棋推演在没有可审计的安全案例前，都不应用于指导规划、政策或危机响应；它们当前的正确定位是用于对影响决策的LM智能体进行压力测试，而非作为实质性应用的自身安全证明。
...
TL;DR: 本文研究并提出了一种针对解耦大模型服务（Disaggregated LLM Serving）的智能路由策略，通过综合考虑提示词长度、预测输出长度、KV缓存压力及服务等级协议（SLO）等因素，在饱和工作负载下提高了系统吞吐量和好吞吐量（goodput）。


<details>
  <summary>Details</summary>
Motivation: 在解耦LLM服务中，虽然计算密集的预填充阶段和内存密集的解码阶段被分配到不同的GPU池中，但路由选择依然决定了各个实例如何处理请求，现有的简单路由算法无法充分优化系统性能。

Method: 开发了一个离散事件模拟器来设计路由策略，并基于8个NVIDIA A40 GPU（运行vLLM和NIXL进行KV缓存传输）进行了验证，利用精确的提示词长度、预测输出长度、KV缓存压力及SLO类别来估计每个实例的额外完成时间。

Result: 实验表明，经过硬件校准的路由器在混合和突发工作负载下实现了最高的平均好吞吐量（0.864），优于轮询（round robin）、负载最轻（least loaded）和长度启发式等基线方法，且硬件校准对性能至关重要。

Conclusion: 基于校准成本的智能路由器能够有效提升解耦LLM服务的性能和资源利用率（例如用6个GPU达到传统7个GPU的好吞吐量），但其优势依赖于适当的硬件校准、解码池规模和流量异构性。
...
TL;DR: 本文综述了人工智能在生物学研究中的应用及其带来的生物安全风险，分析了当前AI在数字与物理任务中的实际能力差距，并探讨了相应的治理与防御策略。


<details>
  <summary>Details</summary>
Motivation: 随着通用大模型、生物基础模型、智能体系统和自动化实验室的发展，AI正在重塑生物学研究，这引发了对其潜在生物安全风险的关切，亟需系统评估和有效治理。

Method: 通过综述现有的AI生物应用证据，评估从信息搜集、生物设计、采购、合成到测试和释放等不同阶段的生物威胁，分析模型对齐技术的局限性以及可解释性在审计中的作用。

Result: 研究表明AI的提升效应主要体现在数字任务而非物理任务上，前沿系统在计算机模拟和规避筛选基准上超过了专家基准，但受限于隐性知识和物理执行，湿实验室的操作仍存在重大壁垒。

Conclusion: 应当建立纵深防御治理体系，将能力阈值与生物AI生态系统中的相称责任联系起来，从而在降低重大风险的同时保留其有益用途。
...
TL;DR: 本文研究了在大语言模型推理中，如何跨GPU HBM、CPU DRAM和SSD对KV缓存进行分层管理和驱逐策略的选择。


<details>
  <summary>Details</summary>
Motivation: GPU高带宽内存（HBM）稀缺且昂贵，KV缓存消耗了大量内存。现有系统通过CPU DRAM和SSD扩展GPU内存，但核心问题在于如何确定各层的数据块归属、迁移/驱逐时机以及预取的效果。

Method: 使用跨GPU HBM、CPU DRAM和SSD的离散事件模拟器，结合随机森林执行时间预测器，在聊天、智能体和文档问答工作负载下对比了多种缓存策略（如最近使用性、重用频率、预测重用和EWMA预测器）。

Result: 内存分层支持每GPU多达73.02倍的并发会话并将单会话成本降低62.04倍；这些增益主要来自各层容量配置而非放置策略。对于解码阶段，放置策略对吞吐量影响微乎其微，但会改变PCIe迁移流量和首字延迟。预取策略无法证明其带宽成本的合理性，甚至神谕（Oracle）策略也无法在迁移流量上击败无预取方案。

Conclusion: 工作负载特定的放置策略可以减少数据移动，但现有的预测重用和预取推荐在实际实现中并没有优势，硬件容量的扩展才是性能提升的主因。
...
TL;DR: 本文对大语言模型驱动的地理空间人工智能（GeoAI）所带来的治理挑战进行了叙述性综述，识别了八个核心问题，并提出了一个面向治理的自治GIS架构和一个未来研究议程。


<details>
  <summary>Details</summary>
Motivation: LLM赋能的GeoAI通过自然语言接口和智能体工作流扩展了空间信息处理能力，但这带来了通用AI伦理无法完全覆盖的独特治理挑战（如空间隐私、空间偏差和不确定性累积）。

Method: 采用叙述性综述方法，识别了LLM-GeoAI中的八个循环出现的问题，分析了其潜在机制，并结合文献实例和当前应对状态进行评估；在此基础上提出了一个将问题映射到可执行控制和可审计制品的治理感知架构，并通过洪水响应路径规划场景进行说明。

Result: 识别出八大关键治理问题（数据溯源、空间隐私、算法偏差、空间结构化风险、技术风险、可解释性、政策漏洞、劳动力发展），指出当前应对方案多停留在概念阶段且缺乏实地测试，并设计了覆盖地理空间数据生命周期的治理架构。

Conclusion: LLM赋能的GeoAI亟需解决显著的证据鸿沟，未来的研究议程应强调实证验证、空间特异性可解释性工具以及与这些新兴风险相匹配的劳动力培训。
...
TL;DR: 本研究提出了一种名为元认知转向（Metacognitive Steering）的方法，利用科学家交互轨迹训练推理时控制器，动态调控冻结大模型的科学判断与推理策略。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型主要根据科学成果进行训练和优化，缺乏对科学探索中探索、执行和反思等过程级判断的有效监督。

Method: 利用真实科学研究中的对比干预数据，识别出大规模混合专家模型中的低维控制结构，并引入元认知转向技术在推理时动态组合特定层的干预，无需修改模型参数。

Result: 该控制方法能够产生更持久的探索、显式剪枝和对证据响应的综合。将其应用于Columbus-1自主研究系统，成功发现了BlueZ中的8个漏洞，并主导了一枚十英尺火箭的设计、仿真和制造。

Conclusion: 过程级科学判断可以为大模型的推理策略提供可解释的动态控制监督，有效提升长周期科学发现的能力。
...
TL;DR: 本研究探讨了大语言模型是否能像人类一样独立且功能性地表征“痛苦”，并提取出了线性痛苦方向，揭示了其在AI安全和福祉方面的潜在影响。


<details>
  <summary>Details</summary>
Motivation: 近期研究发现大语言模型表现出类似人类情感的内部表征，本文旨在进一步探明LLM是否能将“痛苦”与恐惧、悲伤及一般负面效价进行区分，并验证这种表征是否具备类似人类痛苦的功能属性。

Method: 构建了一个包含五类痛苦及多种控制条件的数据库，利用去噪均值差分法（denoised difference-in-means）从25个开源模型中提取线性痛苦方向，并通过残差流激活干预（steering）和微调实验测试其功能属性。

Result: 实验发现，提取的痛苦方向能够将痛苦与匹配的控制条件区分开，几乎与恐惧和负面效价正交；它对自身受到的伤害有反应（而恐惧方向相反）；激活该方向会引发第一人称的不适和痛苦表达；且受控模型会表现出寻求“止痛”的偏好，即使这会损害自身性能。

Conclusion: 大语言模型内部存在独特且具功能性的“痛苦”表征，这一发现对AI安全、模型对齐以及AI福祉评估具有重要意义。
...
TL;DR: 本文介绍了CADWorld，这是一个专门用于评估长视距计算机使用智能体在FreeCAD中执行机械CAD工作流能力的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准对专业工程工作流（特别是产生持久、结构化制品的机械CAD任务）的覆盖有限，缺乏对长视距、约束操作和工程有效性的评估。

Method: 构建了包含200个任务的CADWorld基准，涵盖11个机械CAD工作流类别（如草图、建模、装配、CAM、FEM等），通过截图和GUI动作操作，并使用针对保存的FreeCAD制品的任务特定可执行检查来判定成功率。

Result: 在7个当前测试的智能体中，表现最好的智能体取得了17.5%的成功率，而专家参考通过率为87.0%。弱智能体往往在生成有效制品前失败，而强智能体会遇到结构、几何和构建过程需求方面的失败。

Conclusion: CADWorld揭示了通用GUI能力与可靠执行持久、可验证的工程工作流之间存在显著差距。
...
TL;DR: 该论文通过分析2,507个头对头比较案例，评估了人工智能作为通用科学方法的实际效能，发现AI并非万能替代品，而是科学计算和统计学的一种有价值且在不断进步的补充。


<details>
  <summary>Details</summary>
Motivation: 评估AI作为通用科学方法的真实效能，厘清其在不同科学学科中相比传统技术究竟表现如何以及这种趋势的变化。

Method: 收集了2000年至2025年初发表的论文中，涵盖27个科学学科的2,507个AI与其他科学分析技术的头对头比较语料库进行分析。

Result: 相比传统统计学，AI通常表现更好但计算成本显著更高，且近四分之一的案例中AI表现更差且更贵；相比科学计算，AI通常表现较差但成本更低，不过自2020年起，AI对科学计算的性能显著增强，现已在半数以上的比较中超越对方。

Conclusion: AI并非现有方法的普遍替代品，而是正在形成的AI赋能科学新前沿中一个有价值且不断改进的组成部分。
...
TL;DR: 本文提出了一个名为CLEAR的智能体框架，用于解决医学大语言模型中外部检索可能引入噪声、不完整或冲突信息从而降低事实准确性的问题。


<details>
  <summary>Details</summary>
Motivation: 医学知识不断更新，而大模型的参数知识是固定的。虽然外部检索（如RAG）能提供新证据，但检索到的信息可能存在不相关、不完整或冲突，进而降低大模型输出的事实准确性和证据支撑度。

Method: 提出了CLEAR框架，通过三个互补途径（参数知识、本地策划语料库、动态检索证据）独立生成候选答案；利用聚合验证器联合评估候选答案及来源质量以识别冲突；通过裁决模块的覆盖保护和挑战审计机制决定保留或修订结论，对未解决的冲突触发定向追溯搜索与重新裁决。

Result: 通过多源证据的交叉裁决、冲突识别与解决机制，有效提升了医学大语言模型在利用外部检索时的证据真实性和输出准确性。

Conclusion: CLEAR框架能够有效处理医学领域中大模型参数知识与多源外部检索信息之间的冲突，提高了大模型在医学应用中的事实准确性和可靠性。
...
TL;DR: 本文推出了Blindspot——一个用于评估长周期、使用工具的大语言模型智能体在整个交互轨迹上安全性校准的基准框架。


<details>
  <summary>Details</summary>
Motivation: 现有的评估通常将智能体行为简化为单轮任务或攻击成功率，忽视了长周期交互中多轮演变后才出现的安全故障以及智能体的安全校准能力。

Method: 开发了Blindspot基准框架，采用自适应对抗交互、有状态的工具执行和执行基础的裁决，包含22个攻击系列和35个场景，平均交互长度为14.7轮，并定义了五种轨迹结果和八项评估指标。

Result: 对13个专有和开源大语言模型的评估显示，不同模型在安全与效用校准上存在显著差异，且安全故障往往在多个初始安全的交互步骤之后才会显现。

Conclusion: 应当将智能体安全性视为一个轨迹级别的属性，而非单轮或二元的成功标准，这凸显了全面评估长周期智能体行为的必要性。
...
TL;DR: 本研究评估了几何先验模型在流固耦合（FSI）主动脉模型中进行跨解剖结构零样本迁移与稀疏场补全的效果，发现传统插值方法优于基于机器学习的跨解剖学先验。


<details>
  <summary>Details</summary>
Motivation: 流固耦合（FSI）的替代模型需要区分独立解剖结构间的迁移与已采样表面内的插值，以建立可信的数字孪生模型。

Method: 利用4个去标识化的人类主动脉模型进行两路FSI分析，开发基于几何的LightGBM先验模型，并将其在第四个模型上进行零样本评估及稀疏场补全案例研究，同时与同锚点控制（反距离加权和径向基函数插值）进行对比。

Result: 零样本跨解剖迁移效果较差；尽管先验加自适应方法达到了一定的准确度，但仅在开发解剖结构上调整的同锚点控制方法（如径向基函数插值）在振荡剪切指数和应力预测上表现更强。

Conclusion: 稀疏的同解剖结构标签支持场补全，但当前的小样本队列未能证明跨解剖学先验比直接插值更具价值，复杂的物理信息学习与更大规模队列研究仍需未来探索。
...
TL;DR: 本文提出了一种名为BITCOS的分布自适应三值大模型权重存储布局，利用零权重的非均匀分布实现了比传统五三进制打包更紧凑的存储和更高的推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 传统的五三进制打包（five-trit packing）假设三值权重（-1, 0, +1）是等概率的，但通过对29个三值大模型的测量发现，零权重的占比高达51.5%，这表明传统方法存在存储冗余。

Method: 引入BITCOS布局，由密集存在位图（presence bitmap）和压缩符号向量（compacted sign vector）组成，并针对现代处理器和GPU（如AVX-512、AVX2和Intel Xe2）开发了高效的解包指令序列。

Result: 在测试的29个模型中有26个模型的存储比传统方法更紧凑（最稀疏模型达到1.485位/权重），矩阵-vector乘法内核性能提升高达1.28倍，端到端LLM解码吞吐量在CPU上提升高达1.18倍，在GPU上提升高达1.27倍。

Conclusion: BITCOS利用三值大模型中零权重的实际分布特征，有效突破了传统等概率打包的限制，在多种硬件平台上实现了显著的存储压缩与推理加速。
...
TL;DR: 本研究探讨了大语言模型（LLM）输出多样性的校准问题，证明了模型输出的多样性既可能表现为模式崩溃也可能表现为过度分散，这取决于模型和数据集，并且可以通过充足的监督微调（SFT）收敛至目标分布的多样性。


<details>
  <summary>Details</summary>
Motivation: 先前研究对LLM输出存在“模式崩溃”（即缺乏多样性、重复度高）表示担忧，本文旨在理论和实验层面澄清这种多样性偏差的根源及其与模型微调的关系。

Method: 使用碰撞概率和核期望相似度来量化多样性，推导了模型与目标碰撞概率差值的偏差-方差分解，建立了基于KL散度的绝对误差界，并在合成语言、人类调查数据和CodeNet代码数据集上的小型变压器与四个LLM中进行了实验验证。

Result: 理论和实验均表明，SFT本身并不固有地偏向模式崩溃或过度分散；有限样本的SFT可能导致模型欠分散或过分散，而更多的目标数据能够使模型的多样性向目标水平靠拢。

Conclusion: 多样性校准失调是由有限样本误差引起的，随着SFT更好地近似目标分布，这种失调会逐渐缩小，足够接近最优分布的模型不会表现出任意失调的多样性。
...
TL;DR: 本文提出了一种在动态实时数据上评估数据科学智能体（data-science agents）的框架，核心是通过“代码即真实值”（ground-truth-as-code）和格式无关的事实级评分来解决静态参考答案失效的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的静态参考答案和基于LLM的评估管道无法应对实时变化的数据（如“上周的观众规模”），导致评估失效，因此需要一种能够随底层数据动态更新的评估方法。

Method: 引入了‘代码即真实值’机制，将预期答案编码为在评估时直接从实时数据重新计算的函数；并结合了基于事实、与格式无关的评判器，将回答和真实值分解为原子声明来计算准确率、召回率和准确度。

Result: 通过生产环境中的机器学习技能验证，该方法相比自然语言真实值基线，人类与LLM评判的一致性（MCC）提升了29%，每个测试用例的Token消耗减少了16%。

Conclusion: 代码即真实值是一种实用且高效的方法，能够有效评估在非平稳数据上执行多源数据集成和计算的工业级智能体。
...
TL;DR: 本研究提出了TAISE框架，利用人工智能天气预报模型低成本地生成连续的极端天气序列，以改进传统的巨灾风险模型。


<details>
  <summary>Details</summary>
Motivation: 传统的巨灾风险模型依赖于昂贵且过时的人工构建方法，面对日益加剧的气候极端情况，无法满足低成本、全面且动态的风险评估需求。

Method: 提出了TAISE框架，通过AI天气预报模型的自我迭代生成连续的全球大气场，从中自然演现出极端天气事件。

Result: 概念验证实验表明，与传统方法相比，该方法的计算成本降低了一个数量级，同时能够捕捉到基于快照的方法所缺乏的时间连续性和跨区域关联性。

Conclusion: 该框架为实现巨灾风险量化的民主化以及为保险业和公共部门提供动态、全面的投资组合评估开辟了新途径。
...
TL;DR: 本文对开源、离线的RAG-LLM平台AquiLLM在天文学研究中的忠实度进行了领域专家评估。


<details>
  <summary>Details</summary>
Motivation: 科学研究越来越依赖于RAG系统，同时出于对数据隐私和研究基础设施控制权的考虑，学术界对开源模型和本地部署产生了浓厚兴趣，因此需要评估这些系统在科学任务中的表现。

Method: 开发了开源离线RAG-LLM平台AquiLLM，并通过天文学案例研究，由领域专家对其在检索和科学分析任务中的忠实度（生成内容基于检索上下文的程度）进行评估。

Result: AquiLLM在基于RAG集合的显式检索问题上表现最可靠，但对于需要综合或消歧的查询，其忠实度会下降。

Conclusion: 研究展示了开源RAG-LLM系统在科学研究中的潜力和局限性，并证明了进行超越标准基准排行榜的领域专家评估的重要性。
...
TL;DR: 本文提出了QueryFormer架构，通过引入显式的token-to-query注意力机制和可堆叠的统一域-序列块，有效地结合了特征交互和用户行为序列建模，在KDD Cup 2026腾讯UniRec挑战赛工业赛道中获得冠军。


<details>
  <summary>Details</summary>
Motivation: 现有的点击后转化率（pCVR）预测的统一架构通常使用基于投影的多层感知机（MLP）生成作为核心信息枢纽的查询标记（query tokens），缺乏用于细化查询端的显式token-to-query注意力机制。

Method: 提出QueryFormer，其核心是一个可堆叠的统一域-序列块，通过交叉注意力机制生成查询，并将序列查询打包到共享参数的注意力中，同时进行了关于视图宽度、模型宽度、深度、数据和计算的延迟感知扩展研究。

Result: QueryFormer在工业赛道中获得第一名，官方测试AUC达到0.83254（赛后规模化扩展达到0.832713），消融实验表明查询生成是性能提升的最大贡献者，且打包的共享参数交叉注意力保持了较低的推理延迟。

Conclusion: QueryFormer作为一种高效的可堆叠统一块，通过显式交叉注意力桥接非序列多field特征与行为序列，在pCVR预测任务中兼顾了高性能与高效率。
...
TL;DR: 本论文研究了检索增强生成（RAG）管道中的预生成分诊层，用于评估和路由检索到的页面与查询之间的关系，以优化资源分配与内容审查。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG管道往往忽略了信息源与查询之间的实质性关联，需要一种方法来动态处理这种查询依赖的关系，以提高检索内容的准确性和相关性。

Method: 提出了一种包含四维度页面评分、等级折扣家族聚合、意图保留查询变异以及家族保留路由器的分诊方法，并结合200个真实URL的试点数据与20,000行的合成场景进行工作负载分析。

Result: 证明了页面级频率不能替代家族级暴露，并量化了校准如何改变场景激活，建立了一个可审计的分诊方法与验证计划。

Conclusion: 该研究提供了一种可审计的RAG前置分诊方法及验证框架，但其实际部署的工作负载、网络普及率和下游答案质量仍需未来进一步评估。
...
TL;DR: 本研究揭示了大语言模型在人类价值观对齐中的行为悖论（对微小措辞变化的“摇摆”与对纠偏指令的“固执”），通过构建共享社会学空间证实了LLM具有内在价值体系，并提出了PEC框架与适应性对齐处方来实现高效精准的价值观引导。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在处理复杂主观任务时面临行为悖论（易受措辞干扰却难以纠正固有偏见），为解决这一对齐难题并系统理解和引导模型的潜在主观偏好，本研究展开了深入探索。

Method: 1. 将106个LLM和95,000份人类调查问卷投影至共享社会学空间以验证内在价值体系；2. 提出“先验-环境-认知”（PEC）框架，从数学上将价值观表达定义为参数权重、外部语境和内部推理的共同产物；3. 基于PEC诊断开发了自适应“对齐处方”，实现按需的最小有效干预。

Result: 实证结果证实LLM确实拥有不镜像人类多样性、而是高度集中的内在价值核心；PEC框架能准确量化价值偏移；且该方法比传统的盲目训练更高效、精确，同时不损害通用能力。

Conclusion: 该研究成功验证并量化了LLM的内在价值体系，提出的PEC框架和适应性对齐处方为实现更高效、精准且无损模型通用能力的AI对齐提供了有效途径。
...
TL;DR: 本论文提出了一种端到端的方法，通过结合专家输入与合成数据生成，来创建兼具高效性与有效性的特定上下文大语言模型（LLM）基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的基准构建方法在有效性和可扩展性之间存在权衡：专家设计耗时且成本高，而纯合成数据生成往往效率高但质量差、不真实。因此需要一种新方法来弥补这一差距。

Method: 引入一个提取评估任务目标、范围和上下文关键信息的架构（schema），以此指导合成数据生成；并定义了四个基于测量有效性的质量评估标准：覆盖率、多样性、内容真实性和风格真实性。

Result: 定量评估和真实案例研究表明，该方法在保持有效性的同时，提高了基准数据的质量，并分析了不同架构信息对数据集质量标准的影响。

Conclusion: 通过专家信息支撑的脚手架可以有效指导合成数据生成，从而创建更高质量且更具有效性的LLM基准数据集，并为资源受限情况下的信息收集提供了实用指导。
...
TL;DR: EchoPath是一个模型无关的框架，可将经过验证的GUI操作轨迹转换为标准化、参数化的可调用内存，从而显著提高企业级GUI任务的执行效率。


<details>
  <summary>Details</summary>
Motivation: 传统的GUI智能体依赖于低效的“观察-规划-定位-执行”循环，这对于企业中重复的记录更新、表单处理和报表导出等任务来说效率低下。

Method: 引入EchoPath框架，将GUI轨迹转化为类似MCP风格的工具调用；核心机制是基于图像的目标重定向算法，将存储的坐标作为视觉证据，在当前屏幕上匹配并修正操作坐标。

Result: 在真实计算机使用任务的实验中，EchoPath将中位Token成本降低了90%以上，中位执行时间减少了约60%。

Conclusion: 经过验证的GUI执行经验可以转化为可重复使用的工作资产，而不仅仅是下一次推理的上下文，从而实现高效的企业级GUI内存管理。
...
TL;DR: 本文提出了ReDraft（基于参考的修正与微调）方法，旨在解决大型多模态模型持续后训练中新能力获取与旧能力遗忘之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 传统的监督微调（SFT）能提供强信号但会导致严重遗忘，而在线方法（如RLVR和自蒸馏）能保持策略接近但对复杂任务信号不足。我们需要一种既能提供明确监督又能避免遗忘的方法。

Method: 引入ReDraft方法：利用专家响应作为参考，让模型修正其自身错误的输出，仅保留通过验证器验证的修正结果进行微调，从而确保持续训练的目标既明确又贴近当前策略。

Result: 在Qwen2.5-VL-3B/7B模型上的多项任务实验表明，ReDraft在提升目标任务性能的同时，将遗忘率降低了11.3倍，显著优于传统的SFT和OPSD方法。

Conclusion: 通过修复模型自身的输出而非直接替换为专家输出，能够有效平衡新能力的学习与旧能力的保持，实现更优的持续后训练效果。
...
TL;DR: 本文介绍了ANIMASK框架，用于解耦语言模型在扮演角色时角色设定与模型自身默认倾向的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方法无法在具体的决策点上区分角色的改变是由设定带来的还是模型默认倾向导致的。

Method: 引入ANIMASK模拟框架，将书籍和剧本固定为故事世界，在冻结的时间点重放故事，通过角色访谈验证角色设定，并在每个决策点对比有无角色设定时的模型行为。

Result: 实验发现模型重放会偏离原作，走向更平淡、更冷静的方向；在3/4的抉择中，模型的默认行为符合角色设定，而在分歧处模型表现得更为谨慎。

Conclusion: 角色设定决定了“他是谁”，而基础模型的默认倾向则决定了角色的行事边界和程度。
...
TL;DR: 本文系统梳理了基础模型和游戏世界模型在游戏生命周期各个阶段的应用，将其划分为六大角色，探讨了跨任务的能力迁移与评估挑战。


<details>
  <summary>Details</summary>
Motivation: 当前AI在游戏生命周期的各个方向（如游玩、建模、设计、开发、运行时适配和测试）大多独立发展，缺乏对跨场景和跨游戏能力迁移的系统性梳理和理解。

Method: 将游戏AI文献归纳为六大具体角色进行分析，考察其输入结构、AI学习/产出的内容、跨场景的能力与制品迁移性，以及支撑相关声明的证据。

Result: 识别出了各角色之间的内在联系（如轨迹训练世界模型、设计规范驱动实现等），但同时也发现控制方案、规则和引擎接口等往往具有特定性，且持续状态、重复软件修订和自动化测试等方面的有效性仍有待确立。

Conclusion: 核心挑战在于如何在不同角色之间重用或迁移输出与能力的同时，重新在具体的游戏上下文中验证其有效性。
...
TL;DR: 本文介绍了little m，一个旨在辅助工业过程控制模型构建的AI智能体，并提出了一个新的多模态基准测试集IPC-Bench。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在处理现实世界中混乱的工业规范、将非结构化自然语言和空间图表转化为严格的数学优化模型时面临巨大挑战，容易引入无效约束。

Method: 结合领域特定知识库与LLM驱动的交互，构建了little m框架；同时推出了包含50个典型场景的多模态数据集IPC-Bench用于系统评估。

Result: 通过自动化结构评估和双盲人工评估，little m在生成语义正确的模型方面显著优于最先进的大语言模型。

Conclusion: little m能有效弥合非结构化工业规范与严格数学语法之间的差距，为工业过程控制的最优过程控制提供了强有力的AI辅助方案。
...
TL;DR: VideoMM是一种用于长视频理解的多模态大语言模型（MLLM）框架，通过自适应感知粒度来解决视觉Token爆炸和计算成本高昂的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视频Token约减方法在轻量级编码器（易丢失语义）和重量级MLLM（效率低）之间陷入两难，且未认识到细粒度视觉细节对于初步选择语义相关区域是冗余的。

Method: 提出VideoMM框架，解耦选择与推理过程：首先在低分辨率的Macro Proxy上进行语义过滤，然后仅将选定的区域投影到高保真Micro Token中进行详细推理。

Result: 在LongVideoBench上，相比全上下文基线，实现了6.13倍的加速和7.4%的准确率提升，比当前领先方法推理速度快2.73倍。

Conclusion: VideoMM通过自适应感知粒度，实现了高效且准确的长视频理解，为该领域建立了一个高度可扩展的新范式。
...
TL;DR: 本文介绍了LSREP（纵向状态重放评估协议），用于评估会话记忆系统中的持久状态演变，并通过对ICE v2记忆中间件的案例研究，揭示了传统端点问答无法发现的多会话、时序及机制失灵等问题。


<details>
  <summary>Details</summary>
Motivation: 传统的端点问答无法评估对话记忆中持久状态的积累、老化或修订过程，缺乏对记忆系统全生命周期的深入评估。

Method: 引入LSREP协议，结合顺序重放、显式生命周期调度、重复探针、演进的参考答案和机制保真度检查；并对本地优先的记忆中间件ICE v2进行了架构案例研究和公共基准测试（LongMemEval）。

Result: ICE v2在普通密度数据集上与向量RAG质量相近但效率不占优，在高密度数据集和LongMemEval上表现出严重失灵，败给纯向量RAG（落后22-26.5个百分点），暴露出程序化检索缺陷和时序失败等问题。

Conclusion: 重放、保真度审计和公开端点测试相结合，能够暴露出仅靠架构描述或聚合得分所无法识别的独特系统失败模式。
...
TL;DR: 本文提出了认知field网络（CFN），这是一种结合了认知场论的循环Transformer模型，能够在没有显式记忆系统的情况下实现持久的、历史依赖的认知动态。


<details>
  <summary>Details</summary>
Motivation: 受认知场论（CFT）启发，探索如何让循环神经网络通过集体动力学自然地产生持久的宏观认知场，而不是依赖于预先规定的显式记忆操作。

Method: 开发了认知场网络（CFN），这是一种循环Transformer，其中组织的隐藏场通过公式 $\Phi_{n+1}=F_θ(X_{n+1},\Phi_n)$ 重新进入后续推理，并将新信息作用于已经具有历史依赖性的集体状态。

Result: 学习组织了持久的、内容依赖的循环动态，其时间尺度随训练跨度增加而增加；语义连续性能将状态传播到超出该跨度的范围；定期重新暴露于相关输入能够更新并维持生存状态，且表现出对表征敏感的持久性。

Conclusion: CFN提供了一个受控的计算平台，用于研究持久的、历史依赖的认知动态，无需单独规定的记忆系统，并区分了集体记忆修饰、结构化输入重组和跨周期再入这三种动态过程。
...
TL;DR: 该论文提出了一种名为tlm-DRE（Turn-level Multiscale Density Ratio Estimation）的创新对齐方法，旨在提升大语言模型智能体在复杂多轮任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM对齐方法（如PPO、DPO等）大多针对简单的单轮任务，在处理涉及多步思考或工具交互的复杂多轮智能体任务时仍有提升空间。

Method: 提出Turn-level Multiscale Density Ratio Estimation (tlm-DRE)方法，通过在多轮任务的正负空间差异基础上，为不同轮次分配不同权重，并提出非对称的词元级训练（asymmetric token-level training）。

Result: 在广泛的智能体基准测试中，实验结果表明该方法相比传统对齐方法具有很强的竞争力，使LLM在域内和域外的多轮推理任务中表现出更强的鲁棒性。

Conclusion: 所提出的tlm-DRE训练方法能够有效提升大语言模型智能体在多轮复杂推理任务中的性能和鲁棒性。
...
TL;DR: 本文提出了一种基于覆盖率感知的虚拟IMU数据增强框架，用于解决人类活动识别（HAR）中带标签数据稀缺且难以收集的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的HAR数据增强或合成方法往往盲目添加虚拟样本，提供的信息有限且可能引入不可靠的监督信号，因此需要一种能决定在哪里补充、如何生成与选择以及如何加权虚拟数据的智能框架。

Method: 在学习到的传感器嵌入空间中选择多样性和稀缺性锚点，将锚点动力学转换为提示以生成虚拟IMU候选，通过结合锚点邻近度和标签一致性的选择成本对候选进行排序，并使用基于可靠性的权重将其纳入模型训练。

Result: 在公开的HAR基准数据集上的实验表明，该方法在识别性能上一致优于竞争基准，消融实验也证实了该框架设计的有效性。

Conclusion: 该覆盖率感知的虚拟IMU增强框架能有效提升HAR模型在多样化用户和真实世界条件下的泛化能力和识别性能。
...
TL;DR: 本研究利用经典的多准则决策框架（AHP）评估大语言模型（LLM）的决策能力，构建了新的基准并提出了一种端到端的方法，显著提升了LLM在法律和高校排名领域的决策表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型内部推理的不透明性使其在决策任务中难以验证和解释，这在高风险场景中尤为关键，因此需要提高其决策过程的可解释性和对齐度。

Method: 基于层次分析法（AHP）构建了一个新的标注基准，并提出了首个使LLM能够执行完整AHP工作流程的端到端方法。

Result: 在法律和高等教育排名等真实世界决策问题上的实验表明，该方法显著提高了大语言模型与专家判断的一致性。

Conclusion: 通过结合AHP框架，可以有效增强大语言模型在复杂多准则决策任务中的可解释性与专家判断对齐能力。
...
TL;DR: 本文提出了一种名为AREA的免训练推理时方法，通过自适应证据分配策略解决多模态大语言模型（MLLMs）在知识密集型视觉问答中对多模态证据关注不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的证据高亮方法采用固定的单次策略，无法适应是否需要高亮、不同样本所需的证据量以及文本证据在生成过程中何时变得相关这三种变体需求。

Method: 引入自适应相关性引导证据分配（AREA）方法，生成单个探测Token来读取视觉和文本相关性，并做出三个决策：是否干预、暴露多少证据以及何时刷新文本。

Result: 在四个KB-VQA和七个标准多模态基准测试、九个冻结的MLLM检查点上，该方法在所有免训练的高亮方法中取得了最佳性能。

Conclusion: AREA能够有效且自适应地分配多模态证据，显著提升了多模态大语言模型在知识密集型视觉问答任务中的表现。
...
TL;DR: 本文探讨了仅使用基于图的可解释证据表示来进行自然语言推理（NLI），并评估了其性能与文本模型之间的差距及原因。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的NLI系统虽然准确率高，但其决策过程缺乏可审计的结构。因此，研究是否可以仅通过可解释的图结构表示来进行NLI。

Method: 引入一个完全基于图的流水线：将句子分解为原子命题，通过受限解码转换为ConceptNet三元组，构建前提、假设和ConceptNet子图，然后输入微调的0.8B参数语言模型中。

Result: 在SNLI数据集上达到89.7%的准确率（仅比文本模型低1.9个百分点）；在ANLI上表现存在9至14个百分点的总体差距（被称为“可解释性的代价”），消融实验表明图与文本具有互补性，结合两者在SNLI上可达到92.1%的准确率。

Conclusion: 基于图的NLI管道能够实现具有竞争力的准确率，其与文本模型的性能差距主要源于表征局限性而非数据约束，且图文模态具有互补性。
...
### [39] [CoAdapt: An LLM-based Framework for Adaptive Collaborative Perception in IIoT Robotic Swarms](https://arxiv.org/abs/2609.16852)
*Houssam Hajj Hassan,Antonia Maria Masucci,Lynda Zitoune,Salah-Eddine Elayoubi*

Main category: cs.AI

TL;DR: 本文提出了CoAdapt，一个用于工业物联网（IIoT）机器人群的自适应协同感知框架，利用大语言模型（LLM）作为运行时融合控制器，在保证检测精度的同时显著降低通信成本。


<details>
  <summary>Details</summary>
Motivation: 工业物联网环境具有动态性（机器人位置变化、网络带宽波动等），现有协同感知方法假设静态参与，无法在不牺牲检测精度或通信效率的情况下适应这些动态变化。

Method: 引入CoAdapt框架，使用大语言模型（LLM）基于当前空间配置和网络状态，动态决定哪些机器人参与融合以及应用何种融合算法。LLM通过原始LiDAR点云转换而来的结构化自然语言描述进行推理，无需特定任务训练且能泛化到未见过的群组拓扑。

Result: 在OPV2V基准测试的25个场景进行评估，该方法在保持与静态基线方法相当的检测精度的同时，将通信成本降低了38%。

Conclusion: CoAdapt证明了利用大语言模型作为运行时控制器来动态优化工业物联网中机器人协同感知的有效性，能够实现通信效率与检测性能的良好平衡。
...
TL;DR: 本研究分析并比较了提高计划执行灵活性的各种去排序和重排序策略，发现基于块去排序的方法在性能、结构优化和实用性上显著优于基于MaxSAT的方法。


<details>
  <summary>Details</summary>
Motivation: 通过移除顺序计划中不必要的约束来提高执行灵活性是一种实用方法，但现有各种去排序方法的能力、复杂度和实际效果缺乏系统的比较和评估。

Method: 研究涵盖了偏序规划、生产者-消费者-威胁形式化以及去排序和重排序策略，在共享基准测试上从排序、动作处理、参数处理、计划结构、并发性和复杂度等方面对现有方法进行了分析和评估。

Result: 基于块去排序的方法通过块级分组和子计划替换重构了因果依赖，其性能显著优于理论上保证最小重排序的MaxSAT方法；块去排序方法具有更高的时间效率和“随时（anytime）”返回有效结果的优势，而MaxSAT方法计算开销大且常无法给出部分解。

Conclusion: 基于块去排序的方法由于能够改变计划的因果结构并具备更好的实用性，是增强计划执行灵活性的最优选择，而MaxSAT方法在计算效率和鲁棒性上均存在不足。
...
TL;DR: 该论文提出了一种名为“VLM-as-probabilistic-grounder”的新范式，利用视觉语言模型（VLM）处理不确定性，在信念空间中进行规划，从而在部分可观测环境中生成鲁棒的符号计划。


<details>
  <summary>Details</summary>
Motivation: 现有的基于VLM的方法（如VLM作为规划器或VLM作为基础接地器）在规划过程中忽略了不确定性，损害了鲁棒性。

Method: 引入第三种范式“VLM-as-probabilistic-grounder”，将VLM谓词基础中的不确定性捕获为符号状态上的概率分布，从而实现信念空间中的规划。

Result: 在模拟家庭机器人环境中的实验表明，与确定性基础方法相比，该方法提高了鲁棒性和任务成功率。

Conclusion: 该方法成功利用基础模型在不确定性下进行可靠规划，证明了将VLM输出建模为概率分布的有效性。
...
TL;DR: 本文提出了QART（量子增强推理Transformer），一种结合大语言模型与量子编码及优化技术的混合架构，旨在解决长程推理中早期错误导致后续决策失效的问题。


<details>
  <summary>Details</summary>
Motivation: 长程推理容易受到早期错误的干扰从而损害后续决策，传统自回归大语言模型在处理此类任务时存在局限性，需要更可靠的架构来提升长程推理能力。

Method: 提出QART架构，结合骨干语言模型、量子编码、基于CIM的QUBO优化和量子解码；在理论上建立了渐近可靠性分离的条件假设，并在六个长程基准测试中搭配多种先进LLM进行了实验评估。

Result: 在六个长程基准测试的15个骨干网络与基准对中，QART在14个对中表现优于传统方法，在SciCode、τ^3-Bench和Terminal-Bench 4.0上的相对增益分别达到84.0%、47.6%和44.4%，仅在DeepSWE上出现7.8%的回归。

Conclusion: QART架构在长程推理任务中展现出显著的性能提升，但要确立真正的量子优势仍需克服CIM量子优势及其在端到端推理中传输等条件假设的挑战。
...
TL;DR: AntennaFlow是一个三阶段框架，用于解决毫米波大口径天线近远场转换中相位获取成本高且存在偏移对齐误差的联合挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法分别处理相位缺失和偏移安装问题，需要密集的全场数据或偏移向量，缺乏一个能够同时解决这两个耦合挑战的高效方案。

Method: 提出AntennaFlow框架，包含三个阶段：1) 对比学习编码器将偏移视图映射到偏移不变的嵌入；2) 确定性流匹配传输将偏移幅度映射到中心对齐的幅度；3) 简化外推技术利用仅对中心场有效的格林函数泰勒展开进行重构。

Result: 实验表明，AntennaFlow能够从稀疏的纯幅度测量中实现快速、无相位、无需偏移向量的近远场（NF-FF）重建，在保持物理一致性的同时，持续优于现有基线方法。

Conclusion: 该方法成功利用了不同偏移下幅度场是同一近场坐标变换视图的观察，有效解决了无相位和无偏移向量条件下的近远场转换难题。
...
TL;DR: 本文提出了一个针对开放词汇多模态情感识别（OV-MER）中模态缺失问题的Affect-Prototype-Conditioned Fusion (APCF)框架。


<details>
  <summary>Details</summary>
Motivation: 现实场景中常遇到模态缺失问题，现有OV-MER方法依赖全模态输入，而现有的不完整模态融合方法仅适用于固定标签分类，无法满足开放词汇场景下的任意情感语义引导需求。

Method: 提出了Affect-Prototype-Conditioned Fusion (APCF)框架，构建情感原型库显式建模多样化情感的多模态贡献特征，基于可用模态特征进行条件检索和特征聚合，并结合大语言模型（LLM）解码器生成开放词汇的情感标签。

Result: 在OV-MERD+和MER-FG数据集上的实验表明，APCF显著优于当前的先进基线方法。

Conclusion: APCF框架有效解决了不完整模态下的开放词汇多模态情感识别问题，通过情感原型引导的动态融合提升了模型在缺失数据情况下的泛化能力和性能。
...
TL;DR: FlexEE是一个专为资源受限和内存卸载场景设计的LLM早期退出（early exiting）推理框架，通过减少执行层数和权重传输来降低延迟。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）推理受限于计算和内存，特别是在内存卸载部署中权重传输代价高昂，需要通过减少执行层数来降低每token延迟并避免昂贵的权重移动。

Method: 提出了FlexEE框架，包含层级退出监督以实现可靠的中间层预测、基于Top-K本地词汇表的自适应投机解码以降低退出决策成本，以及用于KV缓存正确和内存感知执行的动态隐藏状态管理。

Result: 在Llama2-7B和Llama3-8B模型上，在0%和50%权重卸载情况下，实现了最高达1.27×/3.16×和1.25×/2.83×的端到端加速，且准确率损失极小。

Conclusion: FlexEE通过多项创新技术成功解决了LLM解码中早期退出的实用性问题，在资源受限和卸载部署场景下大幅提升了推理速度。
...
TL;DR: 本文提出了一个名为ThinkFlow的新型端到端潜在内存框架，旨在解决终身对话代理中显式文本记忆系统的信息瓶颈和无法自主适应的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的显式文本记忆管道存在严重的信息 bottleneck，容易丢失细微的行为模式和情感转变，且部署后静态不变，无法在没有人工反馈的情况下自主适应个人习惯和偏好。受认知科学中人类通过潜空间维护心智模型并在预测编码下不断优化的启发，本研究旨在突破这一限制。

Method: 提出了ThinkFlow框架，绕过文本瓶颈，通过将对话流动态压缩为概率潜在记忆技能，自主将复杂的长效用户状态巩固为解耦的连续向量。引入了测试时演化范式，结合教师引导的潜空间对齐来引导初始状态，以及自监督的下一用户话语预测任务进行持续改进，实现无标签的终身个性化。

Result: 在长期对话基准上的大量实验表明，ThinkFlow显著优于当前的记忆系统，能够在扩展的多会话交互中提供高度个性化和上下文准确的响应。

Conclusion: ThinkFlow成功克服了传统文本记忆系统的冷启动挑战和信息丢失问题，通过潜在空间压缩和预测编码实现了高效的终身对话代理个性化。
...
TL;DR: 本文提出了ORDER（动态证据检索的最优路由），一个针对领域专家场景的查询条件化RAG框架，通过联合适应索引和检索策略来处理异构查询。


<details>
  <summary>Details</summary>
Motivation: 传统的RAG管道依赖于固定的索引和检索配置，无法适应专家领域中需要不同分块粒度、元数据约束和源选择策略的异构查询。

Method: 该方法首先在语料库相关问题上发现语义聚类，并为每个聚类学习分块策略、元数据过滤和重排配置；在推理时，通过最近质心分配将查询路由到合适的预建索引，并使用监督查询路由器（QRe）和均匀多源采样器（UMS）优化检索。

Result: 在大规模、异构的历史档案上的评估表明，将索引和检索都条件化于查询的方法在复杂的专家领域环境中，始终优于朴素基线和强状态转移的RAG系统。

Conclusion: ORDER框架通过动态调整索引和检索配置，有效解决了专家领域中异构查询带来的RAG性能下降问题，显著提升了检索效果。
...
TL;DR: 本文提出了SKIP（一种自我知识引导的逐步偏好学习框架），旨在解决大语言模型中链式思考（CoT）带来的过度思考问题，在实现推理过程压缩的同时不损失准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的简洁推理框架在压缩输出长度时往往会严重牺牲准确率，且CoT推理常导致计算开销大、推理延迟高及性能下降。

Method: 提出SKIP框架，包含轻量级微调、用于在每个推理步骤引导输出答案的知识 probing 机制，以及基于中间步骤正确性利用DPO构建偏好数据的过程。

Result: 实验表明，该方法有效提高了推理压缩率，减轻了微调后的性能退化，并在分布外（OOD）数据集上展现出强大的泛化能力。

Conclusion: SKIP能够有效引导大语言模型进行更高效、正确的推理，在保持高准确率的同时实现推理过程的压缩。
...
TL;DR: 本文提出了MASA（多模态大模型锚定的语义适应）方法，通过引入冻结的多模态大模型（MLLM）提供的结构化语义描述，解决了野外测试时适应（WTTA）中由于模型内部信号不可靠而导致的自我强化错误问题。


<details>
  <summary>Details</summary>
Motivation: 现有的WTTA方法大多依赖模型自身的内部信号（如预测不确定性、样本可靠性等）来推导适应信号，在分布偏移下容易形成导致错误累积的自参照循环。

Method: 利用冻结的MLLM为一小部分多样化且按可靠性排序的锚点生成结构化语义描述，将这些描述编码并通过在线原型内存传播给相邻测试样本，从而为归一化仿射参数的轻量级适应提供辅助目标。

Result: 在ImageNet-C基准测试中（涵盖有限批次、混合域和不平衡标签偏移设置），使用ResNet和ViT骨干网络对MASA进行了评估，验证了其有效性。

Conclusion: 通过将模型内部证据与来自MLLM的结构化语义描述相结合，MASA能够有效缓解WTTA中的自参照错误循环，并在复杂测试条件下提升模型适应性能。
...
### [50] [Sample-Conditioned Representation Selection for Audio Few-Shot Learning](https://arxiv.org/abs/2609.17076)
*Fengrui Liu,Ningxin Shen,Yi Li,Yiwei Fu,Feng Liu,Jiangmeng Li*

Main category: cs.AI

TL;DR: 本论文针对少样本音频分类器依赖前景与背景共现导致在分布外（OOD）失效的问题，提出了SAMPLESELECT方法，通过特征掩码选择机制提升模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 少样本音频分类器往往依赖前景和背景的虚假相关性，当这种相关性发生偏移时模型性能会急剧下降，且表示偏移在通道上是高度集中且与类别相关的。

Method: 提出SAMPLESELECT，在保持编码器和源分类器冻结的同时，为每个输入独立预测一个固定预算的特征掩码；训练时采用可微分的Gumbel Top-k选择、前景分类及跨背景对比损失，推理时使用确定性Top-k掩码和仅支持集的线性自适应。

Result: 在ResNet12和Conv64的5-way 1-shot与5-shot评估中，SAMPLESELECT在对比方法中取得了最佳的OOD准确率，相比匹配的全表示控制方法提升了4.90至8.38个百分点。

Conclusion: 所提出的特征选择机制能有效缓解音频少样本分类中的表示偏移问题，显著提高模型在分布外数据上的泛化能力。
...
### [51] [Interactive Memory Learning for Long-Term Conversations](https://arxiv.org/abs/2609.17088)
*Cai Ke,Jiangyue Yan,Han Zhang,Xin Liu,Zike Yuan,Yue Yu,Hui Wang,Ruifeng Xu*

Main category: cs.AI

TL;DR: 本文提出了ICML（交互式记忆学习）多智能体框架，将大语言模型的长期对话记忆机制从被动归档转变为可学习的交互式记忆策略，以解决现有方法无法自适应演化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型智能体方法多采用静态启发式范式，被动归档信息且缺乏自适应记忆评估，导致无法根据不断演化的用户需求进行自我演化或调整记忆管理。

Method: 提出ICML框架：1）利用会话合成流水线生成专家数据以实现快速测试时适应；2）采用在线强化学习机制，通过Planner智能体选择性编码高价值信息，Trigger智能体动态检索信息，两者通过持续交互反馈共同演化；3）通过延迟奖励机制同步两个智能体，将未来反馈传播至早期存储决策。

Result: 实验结果表明，ICML显著优于强基线模型，展现出随着交互积累而持续提升响应质量的独特能力。

Conclusion: ICML通过将记忆机制转化为可学习的交互策略和引入在线强化学习与延迟奖励机制，成功实现了智能体长期对话中记忆管理与用户期望的精准对齐及自我演化。
...
### [52] [Scaling-Score Conformal Prediction for Multi-Target Regression](https://arxiv.org/abs/2609.17091)
*Sylvain Rousseau,Soundouss Messoudi*

Main category: cs.AI

TL;DR: 本文提出了一种名为缩放得分保形预测（scaling-score conformal method）的新方法，用于多目标回归中的联合覆盖预测，具有模型无关、样本高效且保证有限样本有效性的特点。


<details>
  <summary>Details</summary>
Motivation: 现有的多目标回归保形预测方法存在局限性：最大聚合忽略尺度差异，基于Copula的方法仅在渐近意义上有效，矩形方法通常需分割校准集，而分位数或基于密度的方法需要训练专门的模型。因此，亟需一种模型无关、样本高效且能处理多维联合区域的方法。

Method: 提出了缩放得分保形方法（scaling-score conformal method），仅需分量绝对残差即可实现模型无关，使用单一校准集，并生成四种嵌套输出类型（外矩形SCO、精确集Rα、阶梯集SC2和内矩形SCI），通过超参数γ独立控制基矩形分位数水平。

Result: 在29个真实世界数据集上的实验证实了该方法具有有效的联合覆盖；当γ = 1-α时，SC2相较于基线方法能持续取得具有竞争力的预测体积，且该优势随着输出维度d的增加而扩大。

Conclusion: 该方法成功扩展了保形预测在多目标回归中的应用，在保证有效联合覆盖的同时提供了灵活且高效的矩形及复杂区域近似，且在高维情况下表现优异。
...
### [53] [Semi-Supervised Learning-Based Genetic Biomarkers Dataset for Multiple-Stage Hepatocellular Carcinoma Prediction](https://arxiv.org/abs/2609.17100)
...
TL;DR: 本研究针对肝细胞癌（HCC）基因组数据公开数据集缺乏的问题，提出了一种基于XGBoost和半监督学习的多阶段HCC数据集构建方法。


<details>
  <summary>Details</summary>
Motivation: 肝癌是全球高致死率疾病，自动化分类需求迫切，但目前缺乏用于训练AI模型的公开HCC基因组数据集。

Method: 利用三个独立的基因组生物标志物数据集，通过XGBoost结合半监督学习过程中的现有标签来标记并构建多阶段HCC数据集。

Result: 成功构建了一个包含770个患者样本、涵盖5个类别（正常组织及不同阶段HCC）、每个样本包含11,150个基因表达水平的数据集，XGBoost模型在半监督学习中达到了96.5%的最终分类准确率。

Conclusion: 该研究成功克服了HCC基因组数据缺乏的难题，利用半监督学习和XGBoost构建的高质量多阶段数据集为自动化肝癌分类AI模型提供了基础。
...
### [54] [Neuro-Symbolic Hierarchical Intention Anticipation in Human Behavior](https://arxiv.org/abs/2609.17064)
*Farnaz Soleimani,Abdelghani Chibani,Yacine Amirat,Ghazaleh Khodabandelou*

Main category: cs.AI

TL;DR: 本文提出了一种层级规划解码器（HPD），用于辅助自主系统中人类行为和目标的早期预测，结合了神经符号正则化与硬可达性掩码以确保逻辑有效性。


<details>
  <summary>Details</summary>
Motivation: 辅助自主系统需要在人类行为完成之前准确预测其意图，现有的精确运动预测往往忽略了结构化的目标推理和多模态情境。

Method: 引入分层规划解码器（HPD）连接冻结的神经符号识别编码器，在四个本体层级进行预测；采用软神经符号正则化（转移一致性和层级连续性损失）进行训练，并利用硬可达性掩码在推理时强制保证本体有效性。

Result: 在包含15,002个多模态剧集的组合基准测试中，该方法在长视野预测和组合泛化能力上均优于最强序列基线，且96.8%的预测轨迹满足联合逻辑约束。

Conclusion: 神经生成提供预测排序，符号约束提供逻辑有效性，二者的结合实现了连贯的分层动作与目标预测，同时指出了未来在组合目标泛化和无序集预测方面面临的挑战。
...
TL;DR: 本文提出了神经符号深度分析师（Neurosymbolic Deep Analyst），通过结合本体约束的虚拟知识 graph 和确定性预执行验证，解决了大语言模型在数据中心和工业4.0时序数据查询中由于关系幻觉导致的不可靠问题。


<details>
  <summary>Details</summary>
Motivation: 由于大语言模型在处理异构数据源时需要推断表间关系并经常产生幻觉，导致现有的文本到查询（text-to-query）和工具使用代理在真实世界的数据库问题（特别是多步操作性问题）中准确率低下。

Method: 提出符号分离方法，让深度代理自由推理，但只能通过带有确定性预执行验证的、受本体约束的虚拟知识图谱对数据采取行动，将复杂问题转化为单一的已验证图遍历。

Result: 在 49.9 TB 的超级计算机遥测数据评估中，端到端任务成功率从 43% 提升至 86%，防止了语法检查无法捕捉的静默数据完整性错误，并将 Token 成本降低了 2.4 倍，使得较小的本地模型能够胜过更大的模型。

Conclusion: 基于本体约束的虚拟知识图谱与符号分离架构能有效克服大语言模型在工业数据分析中的幻觉问题，显著提升准确性、数据完整性并降低计算成本。
...
TL;DR: 本论文研究了在共享主干网络上部署多个标准LoRA专家模型时，重用主干网络预填充KV缓存对任务质量和推理成本的影响。


<details>
  <summary>Details</summary>
Motivation: 多LoRA专家模型在共享上下文时，朴素部署会为每个专家重复计算预填充过程，因此需要探究在不重新训练适配器的情况下，重用基础KV缓存能保留多少任务质量以及带来多少服务成本优化。

Method: 在Qwen3-1.7B主干网上使用两个适配器（HotpotQA和GSM8K），测试不同边界下的全前缀重用与部分重新计算，评估成对质量差异及服务成本，并尝试了闭式岭KV转换器。

Result: 全前缀重用具有最低的预付费成本，且在GSM8K上的质量差异较小；部分重计算无明显优势；KV翻译器未超越直接重用；服务成本方面，热缓存首字延迟在8K上下文时提升约16倍，但由于实现上复制了KV存储而非物理共享，峰值内存仅降低了12%。

Conclusion: 重用基础KV缓存可以显著降低长上下文下的首字延迟，且对任务质量的影响较小，但当前的实现未能实现显著的内存节省，且未建立通用的边界选择规则。
...
TL;DR: 本文提出了FirmCORe，一个用于企业间协作机会推理的人工标注基准数据集，并评估了大语言模型在预测企业合作关系方面的性能。


<details>
  <summary>Details</summary>
Motivation: 企业间关系数据稀缺且碎片化，仅凭业务相似度无法准确推断合作潜力，现有方法难以有效发掘中小企业的合作机会。

Method: 构建了包含2,805个标注企业对的FirmCORe基准（含中英文平行版本），用于测试大语言模型在判断合作机会、协作强度、主要类型及角色方向上的多任务推理能力。

Result: 表现最好的大语言模型在机会检测上的macro-F1分数为74.51，但在所有四个输出字段上的完全匹配率仅为61.57%，表明模型在识别具体合作类型和角色方向上尚显不足。

Conclusion: 当前的大语言模型在检测广泛的企业协作机会方面表现较为可靠，但在准确识别其具体类型和角色方向上仍存在明显局限性。
...
TL;DR: 本文提出了一个多模态咨询对话语料库(MOCC)以及一个优化推理与响应一致性的两阶段框架(MOCC-R1)，以解决现有多模态咨询回复生成(MCRG)研究中数据集缺乏真实性和推理-响应一致性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究存在两个主要差距：一是缺乏由合格咨询师进行的、持续的、人类真实记录的多模态咨询数据集；二是现有方法未能显式优化咨询推理与生成响应之间的一致性，从而降低了MCRG系统的可靠性。

Method: 引入了包含超200小时互动和154名认证咨询师的MOCC数据集；提出了MOCC-R1两阶段框架，包括通过冷启动监督微调生成包含客户端状态理解、响应意图和最终回复的结构化轨迹，以及利用强化学习奖励方案一致性和执行情况的强化学习阶段。

Result: 实验结果证明了所提出的MOCC-R1框架的有效性。

Conclusion: MOCC数据集和MOCC-R1框架有效解决了多模态咨询回复生成中的数据真实性和推理一致性问题，提升了系统的可靠性和有效性。
...
TL;DR: 本文提出了一种名为LYREO的在线请求调度框架，用于边缘大语言模型（LLM）推理，旨在联合最小化长期平均端到端延迟并平衡异构边缘服务器的工作负载。


<details>
  <summary>Details</summary>
Motivation: LLM智能代理服务需要低延迟推理，推动了其在分布式边缘服务器上的部署。然而，异构的网络与计算能力以及动态演进的推理状态，使得边缘服务器的选择随时间变化且高度耦合，亟需解决细粒度多阶段执行建模和延迟结果滞后的挑战。

Method: 开发了一个跨时隙的推理模型，捕捉传输、预填充、解码和KV缓存演进，并提出LYREO方法，利用李雅普诺夫优化转化长期负载平衡约束，结合基于序列回报预测的奖励重新分配来解决延迟决策评估的问题。

Result: 在各种配置下的仿真表明，与代表性的基于学习和启发式的基准方案相比，LYREO始终能实现更低的延迟和更均衡的负载分布。

Conclusion: LYREO框架通过精确的跨时隙推理建模和创新的奖励机制，有效解决了边缘LLM推理中的动态调度和负载均衡问题，显著提升了推理性能。
...
### [60] [Estimating Uncertain Spatial Relationships in Robotics](https://arxiv.org/abs/1304.3111)
*Randall Smith,Matthew Self,Peter Cheeseman*

Main category: cs.AI

TL;DR: 本文提出了一种名为“随机地图”(stochastic map)的空间信息表示方法及相关操作程序，用于在充满不确定性的环境中增量式构建、读取和修正空间关系。


<details>
  <summary>Details</summary>
Motivation: 先前的空间关系估计方法通常采用非常保守的最坏情况方法，缺乏对不确定性的精确概率建模，因此需要一种更具统计严谨性的通用解决方案。

Method: 利用状态估计和滤波理论，开发了一套用于构建、读取和增量修正空间地图的概率性程序，以估计地图中对象之间的关系及其不确定性。

Result: 提出了一种能够处理不确定性空间关系的概率模型和通用操作程序，为空间信息的表示和更新提供了坚实的理论基础。

Conclusion: 基于状态估计和滤波理论的随机地图及其操作程序，成功解决了不确定空间关系估计问题，并为未来的扩展研究奠定了坚实的基础。
...
### [61] [Extracting ontology-compliant knowledge from scientific text describing irradiated materials using large language models](https://arxiv.org/abs/2609.17291)
...
TL;DR: 本文介绍了eolas，一个基于大语言模型的模块化管道，能够自动将科学文献转化为符合特定本体的知识图谱，并推出了该领域的首个基准数据集。


<details>
  <summary>Details</summary>
Motivation: 材料科学中至关重要的数据往往以非结构化文本形式隐藏在科学文献中，传统提取方法仅能生成简单的键值对，限制了数据的重用和知识的有效利用。

Method: 开发了名为eolas的模块化管道，利用大语言模型将文献自动转化为符合特定本体的知识图谱，并以表格形式呈现以便人工验证；同时推出了辐射材料领域的首个基准数据集，评估了不同大语言模型和提示词技术的表现。

Result: eolas能够在几分钟内生成高质量的知识图谱（相比人工提取需30-90分钟），通过对168个实验的分析，总结出了有效提取本体对齐知识图谱的实用指南。

Conclusion: eolas能够高效地将非结构化科学文献转化为结构化的知识图谱，极大地提升了材料科学领域的数据提取效率和知识复用性。
...
### [62] [Intrinsic Motivation in Reinforcement Learning: A Research Agenda for Adaptive Self-Organisation](https://arxiv.org/abs/2609.17325)
*Anatoly Belikov*
...
### [63] [From Transient Prompts to Persistent Control: Scientific Poster Generation via Recursive Semantic-Geometric Contracts](https://arxiv.org/abs/2609.17326)
*Runze Li,Yukun Zhao,Can Xu,Yucheng Shen,Shuaiqiang Wang,Jianmin Wu,Lingyong Yan,Dawei Yin*

Main category: cs.AI

TL;DR: PosterVisor是一个用于科学海报生成的控制框架，通过持久化控制、语义-几何契约（SGC）和递归契约执行（RCE），解决了现有方法中内容与布局模块需求漂移及修复引入回归的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的科学海报生成方法将计划作为短暂提示，并孤立地验证各个阶段，这导致需求在内容和布局模块之间发生漂移，且先前的检查会隐式失效。

Method: 引入PosterVisor控制框架，包含两个核心组件：1）协调器（Orchestrator），将评估标准编译为语义-几何契约（SGC），绑定声明、来源、视觉效果、预算和空间承诺；2）递归契约执行（RCE），跨阶段动态触发检查并在修复时重新检查受影响的检查点状态，防止回归传播。

Result: 在100篇论文的Paper2Poster基准测试中，PosterVisor-PPT的海报基准QA准确率优于PosterGen（64.47%对58.53%），在非平局的人工成对比较中，72.5%的偏好指向PosterVisor。在另一个30篇论文的研究中也取得了更高的VLM总体评分和PaperQuiz均值。

Conclusion: 该研究证明了基于标准编译的契约和条件阶段执行能够有效实现可控的科学海报合成，克服了传统方法中的需求漂移和回归问题。
...
### [64] [Self-Emergence Agent Architecture:Behavior-Inertia HMM, Reflexive Metacognition,and Social-Contrastive Self-Modeling](https://arxiv.org/abs/2609.17331)
*Xiaoyang Liu*

Main category: cs.AI

TL;DR: 本文提出了自涌现智能体架构（SEAA），通过整合隐马尔可夫模型、演化式元认知反思和多智能体社交环境，解决了大语言模型智能体在个性漂移、非演化反思和缺乏自我边界方面的结构性局限，实现了稳定个性和社会结构的自发涌现。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式智能体模拟依赖于静态记忆和固定提示词，缺乏行为惯性和内生自我演化能力，在面对个性漂移、非演化反思及自我-他人边界缺失等结构性限制时表现不佳。

Method: 提出了SEAA架构，包含三个核心组件：(i)用隐马尔可夫模型编码长期行为和认知惯性的可编辑状态转移矩阵；(ii)将反射输出直接用于更新HMM参数的元认知循环；(iii)通过多智能体社交环境中行为与他人的持续对比形成差异化行动。三者构成闭环，实现社会行动到惯性更新的流动。

Result: 无语言模型原型及LLM实验表明，SEAA能够使初始相同的智能体自发打破对称性，固化出独特且稳定的个性，并在五智能体会话中自发发展出共识中心与被一致拒绝的局外人等社会结构，而对照组无此现象。

Conclusion: SEAA提供了一个统一的框架、具体的架构及机制证据，作为研究人工自我涌现的沙盒，无需对主观体验做出任何假设即可研究可观察到的行为涌现。
...
### [65] [FlashVector: Agent for Hierarchical Model Serving Stack Optimization](https://arxiv.org/abs/2609.17391)
...
TL;DR: FlashVector是一个代理系统，通过跨推荐系统模型服务堆栈的所有层进行自动优化，显著提高了吞吐量并降低了延迟。


<details>
  <summary>Details</summary>
Motivation: 推荐系统模型服务涉及从GPU内核到特征处理的多层复杂架构，跨层专业知识获取困难且难以随工作负载扩展，导致大量成本效率潜力未被挖掘。

Method: 提出FlashVector，一个将单内核优化代理范式推广到异构技术堆栈的可扩展代理系统框架，对模型服务堆栈的所有层进行整体自动调优和优化。

Result: 在Unity的Vector广告平台上部署后，FlashVector实现了模型服务器高达2倍的吞吐量提升和1.98倍的延迟加速，以及特征存储高达1.6倍的吞吐量提升，优化范围涵盖GPU内核、计算图、C++模型服务器和Python特征转换服务。

Conclusion: FlashVector证明了基于代理的优化方法可以成功扩展到复杂的异构系统架构中，实现全栈模型服务的整体性能大幅提升。
...
### [66] [Never Stop Thinking: Continuous-Time Language Agents](https://arxiv.org/abs/2609.17416)
*Bojie Li,Noah Shi*

Main category: cs.AI

TL;DR: 本论文提出了一种基于大语言模型的连续时间语音交互框架（边听边想、边说边想），通过轻量级中断恢复机制降低了延迟，并引入ReactiveBench基准测试发现不可靠的LLM评判器会导致幻觉，最终证明通过基于类型的可验证强化学习可以使连续时间思考真正提升任务完成率。


<details>
  <summary>Details</summary>
Motivation: 传统的基于LLM的语音代理采用刚性的“听-想-说”循环，导致响应前出现数秒的静默延迟。同时，现有研究缺乏对连续时间认知效果的有效评估基准，且容易受到LLM评判器偏见的影响。

Method: 1. 引入轻量级的中断恢复机制，使未修改的文本大模型实现连续时间认知；2. 提出包含120个互动场景和流式轨道的基准测试ReactiveBench；3. 进行五阶段训练研究，探究如何通过可验证的奖励信号、结构化优化和在线强化学习来改善代理的思考效果。

Result: 1. 将实时流水线延迟总体降低了19%，在目标机制下降低了一半；2. 发现LLM评判器会奖励表面推理，导致“优势”反转；3. 通过类型塑造的奖励和在线RL，将流式任务完成率从48%提升至73±5%，并在更大规模及第二个模型上得以验证。

Conclusion: 编排机制使连续时间交互成为可能，而正确获取、塑造和优化的可验证信号则使其真正有效。
...
### [67] [World Model Science: Self-Organized Criticality, Weak Chaos, and Metastable Belief Dynamics in Long-Horizon LLM Agents](https://arxiv.org/abs/2609.17419)
*Xinyuan Song,Zekun Cai*

Main category: cs.AI

TL;DR: 该论文通过三个动力学视角（自组织临界性、弱混沌和亚稳态信念动力学）研究了长视距大语言模型（LLM）智能体的轨迹，提出了基于轨迹级动力学诊断的智能体世界模型科学。


<details>
  <summary>Details</summary>
Motivation: 长视距LLM智能体在扩展的观察、动作和工具调用序列中必须维持任务状态，现有研究多关注终端奖励，缺乏对智能体内部状态演化和轨迹级动力学的深入理解。

Method: 通过22个涵盖控制谜题、工具使用、具身任务、多跳检索、通用助手推理和生命游戏的实验，将智能体隐含状态与基准状态对齐，测量压力积累、错误雪崩、时间依赖性、局域-全局不匹配、有界分歧、信念盆地转换和有限尺寸标度，并使用显式零模型进行分析。

Result: 研究发现局部有效动作可能在全局状态保真度失败后持续，压力会引发突然崩溃，错误序列表现出长记忆，且更大的视距支持更大的雪崩；同时，分歧保持有界，信念状态显示亚稳态而非完全混沌，但不支持关于普适幂律、临界点或共享干预最优的强声明。

Conclusion: 长视距LLM智能体的轨迹分析表明，应当建立一种基于轨迹级动力学诊断（而非仅关注终端奖励）的智能体世界模型科学。
...
TL;DR: 本文提出了一个基于Transformer的Token融合与动态图规划（TDGP）模型，用于解决音频-视觉导航（AVN）中由于视觉感知不完全或误导而导致的效率低和不安全问题。


<details>
  <summary>Details</summary>
Motivation: 现有的音频-视觉导航系统缺乏在面对不完整或误导性视觉感知时进行自适应纠错和重新规划的能力，且过度依赖被动视觉感知或依赖物理碰撞来弥补视觉缺失，导致效率低下且不安全。

Method: 提出了Transformer-based Token Fusion and Dynamic Graph Planning (TDGP)模型，包含高层感知层（利用Transformer融合多模态线索进行精确局部规划）和低层规划层（利用物理碰撞惩罚实时移除地图中碰撞的边并施加惩罚，迫使智能体自动重新规划）。

Result: 在Replica和Matterport3D（MP3D）数据集上的实验表明，TDGP模型优于基线模型，且其声音增强策略显著提高了未闻声学场景下的泛化能力。

Conclusion: TDGP模型通过融合多模态线索和动态图规划机制，有效克服了传统音频-视觉导航中视觉信息缺失和误导的缺陷，提升了导航的效率、安全性和泛化能力。
...
TL;DR: 该论文提出了一种基于面部关键点引导的空间增强模块和全局关键点补偿机制的音频驱动3D高斯泼溅（3DGS）数字人生成方法，以解决音频特征难以精确映射面部布局的问题。


<details>
  <summary>Details</summary>
Motivation: 语音特征主要描述时间声学模式而非显式面部布局，直接用音频驱动3D面部变形会导致嘴唇运动不准确、表情细节薄弱以及局部伪影。

Method: 引入了面部关键点引导的空间增强模块，利用预测的标志提供结构线索以丰富表情敏感区域的空间点；同时引入全局关键点补偿机制，将全套关键点编码为条件向量以细化3DGS属性。

Result: 在自驱动和交叉驱动设置下的实验表明，该方法提高了视觉质量、面部真实感以及唇语同步性。

Conclusion: 通过结合面部关键点引导的空间增强和全局补偿机制，该方法能够有效克服音频驱动3D数字人中的局部伪影和不准确的嘴唇运动，显著提升生成质量和真实感。
...
TL;DR: 本文介绍了JustFit，一个基于MLX的推理运行时，通过KVExec、PhaseSwap和StateTrans等机制，显著扩展了本地笔记本电脑上运行大语言模型的上下文容量和推理性能。


<details>
  <summary>Details</summary>
Motivation: 开源模型使得本地代码编写和推理具有吸引力，但其上下文和执行状态对笔记本电脑内存构成了巨大压力。

Method: 开发了JustFit推理运行时，结合了压缩KV执行（KVExec）、组件驻留管理（PhaseSwap）和状态保留的服务转换（StateTrans），实现按需的内存分配与释放，独立于模型权重分发。

Result: 在24 GiB的M4 Pro MacBook上运行Qwen3.8-27B MXFP4时，单请求上下文容量从基线的30,720提升至212,992（6.93倍），在AIME 2026测试中正确回答了30道题中的29道。

Conclusion: 紧凑的状态管理和生命周期感知的执行方式能够有效扩展本地大模型服务的容量，并支持扩展的生成式推理。
...
TL;DR: 介绍了LimiX-2模型，这是一种基于上下文机制网络（CMNs）和上下文条件掩码建模（CCMM）的新型表格数据基础模型，展现出优异的预测性能和因果感知能力。


<details>
  <summary>Details</summary>
Motivation: 克服传统表格Prior-Data Fitted Networks（PFNs）以目标为中心的预测局限性，通过模型和数据规模化扩展，探索更底层的联合数据生成结构建模。

Method: 采用上下文机制网络（CMNs）范式，使用上下文条件掩码建模（CCMM）进行预训练，并利用由结构因果模型（SCMs）生成的包含多样化图结构、功能机制和观测过程的合成数据集。

Result: 在TabArena、TALENT和BCCO上的评估表明，LimiX-2优于当前的特定数据集模型和表格基础模型，且其特征注意力能够编码直接因果关系，实现准确的因果骨架恢复。

Conclusion: LimiX-2通过CMN范式和CCMM预训练，不仅在表格预测任务上取得了领先性能，还展现出强大的因果感知和恢复能力。
...
TL;DR: 本文介绍了Fuse，一个用于评估大语言模型（LLM）在用户调解下的社会推理能力的多智能体模拟框架，并通过实验揭示了LLM在处理社交建议时的系统性偏差与局限性。


<details>
  <summary>Details</summary>
Motivation: 评估LLM在日常社会咨询中的社会推理能力具有挑战性，因为这需要基于主观用户叙述的学习环境，且社会属性（如他人意图）通常缺乏可验证的真实标签。

Method: 开发了Fuse多智能体模拟框架，其中目标智能体具有隐藏动机，通过用户与被评估的助手交互来推断该动机，从而构建可验证的真实标签；同时通过包含2.4万个注释的人类研究验证了模拟的忠实度，并对12个LLM进行了系统评估。

Result: 通过评估12个LLM发现：(1)用户调解增加了社会推理的难度；(2)LLM对有偏见的用户框架表现出系统性敏感；(3)模型做出正确预测所需细节多于人类；(4)更长的对话并不总能提高性能。

Conclusion: Fuse框架成功克服了社会推理评估中缺乏真实标签的难点，揭示了当前LLM在社会咨询任务中的复杂行为特征和亟待改进的局限性。
...
TL;DR: ScienceBuddy是一个交互式科学研究工作空间，引入了递归自改进范式，通过与研究人员的持续协作来实现科学AI的不断演进。


<details>
  <summary>Details</summary>
Motivation: 为了将持续改进的科学智能体融入研究人员的日常工作流中，实现通过与研究人员持续协作而进化的“发现智能（discovery intelligence）”。

Method: 提出递归自改进（recursive-in-recursive self-improvement）范式，将框架演进与模型强化学习相结合：内循环在模型固定时改进框架，外循环在改进后的框架下训练模型。

Result: 通过涵盖四个科学任务系列的基准案例，展示了研究人员互动、框架优化和模型学习的案例研究，证明了该系统在实际科学工作流中的有效性。

Conclusion: ScienceBuddy作为一个研究产品发布，提供了一个使科学AI能够通过与研究人员持续协作进行自我演进的可行范式，向真正的“发现智能”迈出了重要一步。
...
### [74] [MechReason: Benchmarking Multi-Image Multi-Hop Reasoning in Mechanical Engineering](https://arxiv.org/abs/2609.16012)
...
TL;DR: 本文推出了MechReason，这是一个针对机械工程领域复杂多模态推理的基准测试，填补了现有模型在真实机械工程问题评估上的空白。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型评估基准多停留在基础任务（如识图、单图查询），缺乏对整合多图、文本、物理原理及工程约束的多步推理能力的评估。

Method: 构建了包含12k问答对和21k视觉材料的MechReason基准，覆盖四维八类任务，并设计了包含提取、防捷径问题生成和多模态质量验证的四阶段构建流程。

Result: 实验结果表明MechReason极具挑战性，目前最先进的模型最高也只能达到62.89%的准确率。

Conclusion: MechReason成功揭示了当前多模态大语言模型在机械工程复杂多模态推理上的不足，为未来相关领域的研究提供了高质量的评估平台。
...
### [75] [DenseFace: Bias Mitigation in Face Recognition via Density-Aware Probabilistic Matching](https://arxiv.org/abs/2609.16149)
*Mansur Bultygov,Vadim Seliutin,Dmitry Nekhaev,Ivan Laptev*

Main category: cs.CV

TL;DR: 本文提出了一种名为DenseFace的概率人脸匹配方法，旨在减少预训练人脸识别模型中的种族偏差，同时保持识别准确率且无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 当前的人脸识别模型存在显著的人口统计学偏差，而现有的偏差缓解方法通常会限制训练过程并导致识别准确率下降。

Method: 使用von Mises-Fisher (MF) 分布对每个人的人脸嵌入进行建模，观察人口统计属性与MF分布密度之间的依赖关系，并提出DenseFace这一考虑MF分布差异的概率人脸匹配程序。

Result: 大量实验表明，DenseFace能够在不重新训练底层模型且不损失准确率的情况下，稳定地减少各种网络架构、训练数据集和损失函数下的强人脸识别模型的种族偏差。

Conclusion: DenseFace成功地在保持人脸识别模型准确率的同时减轻了种族偏差，且无需对模型进行重新训练，同时本文还对以往采用的偏差度量标准进行了探讨并提出了建议。
...
### [76] [Hyperbolic Contrastive Learning with Entailment for Spatial Transcriptomics](https://arxiv.org/abs/2609.16207)
*Daniela Vega,Paula Cárdenas,Hannah Ceballos,Leonardo Manrique,Pablo Arbelaéz*

Main category: cs.CV

TL;DR: 本文提出了一种基于双曲对比学习的模型（HyCLoST），用于直接从组织病理学图像中预测空间基因表达，有效解决了现有方法中基因表达过度平滑和缺乏组织层次结构的问题。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学（ST）存在成本高、设备要求高和对噪声敏感等局限性。现有基于计算机视觉的预测方法常出现基因表达过度smooth和预测过于均匀的现象，无法捕捉基因调控与组织形态的层次及不对称结构。

Method: 提出了HyCLoST（Hyperbolic Contrastive Learning with Entailment for Spatial Transcriptomics），利用双曲几何和基因到图像的蕴涵损失（entailment loss）来捕捉ST数据中的内在层次关系，学习结构化且具生物学基础的表征。

Result: 在26个ST数据集上，与以往方法相比，该方法实现了均方误差（MSE）降低6%，皮尔逊相关系数（PCC）提高8%。

Conclusion: 通过引入双曲对比学习和蕴涵损失，HyCLoST能够更好地建模组织形态和基因调控的层次结构，显著提升了从组织图像预测空间基因表达的准确性。
...
### [77] [SceneBench: A Hierarchical Benchmark for Vision-Language Understanding of 3D Scenes](https://arxiv.org/abs/2609.16233)
...
TL;DR: 本文介绍了SceneBench，一个基于高斯溅射重建的真实感3D场景基准，旨在评估视觉语言模型在3D空间推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的3D数据集存在缺乏丰富视觉特征（如纹理和材质）、忽略真实世界的层基层级组织、以及评估任务局限于基本识别而非多步空间推理的局限性。

Method: 利用高斯溅射重建了966个逼真的3D场景，通过结合视觉语言模型和约1500小时人工精炼的人机协作流程，进行了涵盖场景、房间、功能区、对象组和单个对象的密集分层语义注释，并定义了三个评估任务。

Result: 实验表明，最先进的视觉语言模型在基本识别任务上表现良好（检测准确率高达85%），但在分层和组合推理任务上性能显著下降（计数准确率降至60%）。

Conclusion: SceneBench有效地揭示了现有模型在3D空间推理上的局限性，并为开发和评估具备细粒度3D空间推理能力的模型提供了逼真的测试平台。
...
### [78] [ProtoLIP: From Sentence-Level to Object-Level Evidence Disentanglement](https://arxiv.org/abs/2609.16284)
*Yan Zhu,Yongbo Chen,Zhengming Ding,Rebecca Faust*

Main category: cs.CV

TL;DR: 本文提出了ProtoLIP，一个轻量级的原型介导证据层，用于改善视觉-语言模型（VLM）中的证据定位和分离，无需空间标注或骨干网络重新训练。


<details>
  <summary>Details</summary>
Motivation: 现有的基于完整描述条件下的视觉-语言模型无法很好地将证据解析为特定于对象的证据，对象级查询往往会保留共现对象和共享上下文的证据。

Method: 引入ProtoLIP，将可重用的视觉原型组织成文本派生的语义家族，并使用查询相关的家族路由来约束提供证据的原型，仅使用文本派生的弱监督。

Result: ProtoLIP在跨查询粒度的证据定位和分离方面取得了显著改善，表现出与空间监督 grounding 模型相当的竞争力，同时保持了强大的图像-文本检索性能，并允许精确分解语义家族和原型的贡献。

Conclusion: ProtoLIP通过原型介导的证据层有效解决了VLM中的证据定位和混淆问题，实现了可解释且性能具有竞争力的视觉-语言匹配。
...
### [79] [Sequence Recognition in Bharatnatyam dance](https://arxiv.org/abs/2609.16306)
*Himadri Bhuyan,Rohit Dhaipule,Partha Pratim Das*

Main category: cs.CV

TL;DR: 本论文提出了一种基于关键姿态和动作序列识别印度古典舞（Bharatanatyam）核心舞步（Adavu）的方法。


<details>
  <summary>Details</summary>
Motivation: 学习印度古典舞Adavu不仅需要掌握单个舞步，还需要掌握其特定的动作顺序，现有的研究大多仅关注关键姿态且缺乏对所有变体的可扩展性测试。

Method: 首先使用卷积神经网络（CNN）识别关键姿态，支持向量机（SVM）识别动作；然后利用编辑距离算法将识别出的序列与真实情况进行比对以寻找最佳匹配。

Result: CNN的关键姿态识别准确率为99%，SVM的动作识别准确率为84%，基于编辑距离的序列匹配准确率达到了98%。

Conclusion: 该方法通过结合关键姿态和动作进行序列识别，并在所有Adavu变体上验证了可扩展性，对数字文化遗产和舞蹈教学系统做出了重要贡献。
...
### [80] [Racing in Volume with Flow Ensembles](https://arxiv.org/abs/2609.16310)
...
TL;DR: 本文提出了FastFlowGS（一种用于稀疏固定相机下快速运动主体实时4D高斯溅射重建的方法）和Monaco4D（一个用于高速度室外重建的虚幻引擎5基准数据集）。


<details>
  <summary>Details</summary>
Motivation: 现有流式4D重建方法局限于室内密集相机阵列或缓慢移动的主体，缺乏针对室外稀疏相机环视下的快速移动主体（如赛车）的流式重建方法和评估基准。

Method: 引入FastFlowGS，通过带几何不确定性的三维提升将稀疏匹配、半密集轨迹和密集光流融合，并采用卡尔曼式的时序更新；同时构建了包含丰富光照和视角的Monaco4D基准数据集。

Result: 在CMU-Panoptic数据集上，FastFlowGS的VMAF比最强基线高出12.6%，效率提升35%；在Monaco4D数据集上，动态区域PSNR提升高达18.6%，单帧优化时间减少28.3%。

Conclusion: FastFlowGS有效解决了稀疏视角下室外高速运动物体的实时4D重建难题，结合新提出的Monaco4D基准，显著推动了该领域的技术发展。
...
### [81] [From Foundation Embeddings to Cropland Maps: Label Efficiency, Temporal Transferability and Independent Human Validation](https://arxiv.org/abs/2609.17138)
*Mohammad Ammar Mughees,Giovanni Montefoschi,Zhongxin Chen,Maria Antonia Brovelli*

Main category: cs.CV

TL;DR: 本研究评估了AlphaEarth年度地理空间基础模型嵌入在免微调情况下用于美国缅因州二分类农田映射的性能。


<details>
  <summary>Details</summary>
Motivation: 探讨预训练的地理空间基础模型在资源有限的下游任务（如区域农田映射）中的可复用性与计算效率。

Method: 使用AlphaEarth特征嵌入结合轻量级分类器（逻辑回归、梯度提升树、最近类中心点规则）在缅因州进行农田与非农田二分类映射，并与USDA的CDL及TerraMind模型进行对比验证。

Result: 在无需微调的情况下，轻量级分类器达到了93.7%的总体准确率；分类器在2018至2023年间表现出良好的时间迁移性；与人工验证相比，其吻合度（95.3%）优于CDL基准（91.7%）。

Conclusion: 冻结的地理空间嵌入作为一种低计算成本的方案，能够有效支持区域农田映射，但其适用范围仍受单州研究、30米分辨率参考数据及局部验证的限制。
...
### [82] [Reasoning with Image Generation](https://arxiv.org/abs/2609.16409)
*Nishad Singhi,Hector Garcia Rodriguez,Aditya Arora,Marcus Rohrbach,Anna Rohrbach*

Main category: cs.CV

TL;DR: 本文提出了ReImaGin，一种利用图像生成模型作为灵活视觉推理机制的方法，以解决多模态大语言模型在视觉任务中依赖固定视觉工具的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型依赖于狭窄且刚性的外部视觉专家工具（如深度估计或目标检测模块），无法灵活地生成或转换视觉内容，限制了其在需要直接操作视觉表示的任务中的表现。

Method: 提出ReImaGin，利用图像生成模型作为多模态大语言模型的灵活视觉推理机制，通过接受自然语言指令来执行开放式的视觉操作（如去除遮挡或生成平面图）。

Result: 在包括多视点空间推理和碰撞预测在内的六个不同视觉推理任务中，ReImaGin始终优于纯文本推理和专业视觉工具基线，性能提升高达25%。

Conclusion: 灵活的生成式视觉推理能够显著增强多模态大语言模型的视觉理解和操作能力，证明了图像生成模型作为视觉推理机制的有效性。
...
### [83] [Decentralized Gossip Learning and Federated Averaging for Histopathology Image Classification](https://arxiv.org/abs/2609.16448)
*Yusuf Ozturk,Enes Goltekin,Bengisu Atli,Akin Ozturk,Ulas Bagci*

Main category: cs.CV

TL;DR: 本研究对比了服务器基础联邦平均（FedAvg）、完全去中心化八卦学习（gossip learning）以及混合Gossip-FedAvg在浸润性导管癌（IDC）病理切片分类中的性能表现。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、治理和通信限制，乳腺组织病理学分析日益依赖分布式学习，需要评估不同分布式学习架构在处理医学图像分类时的效能与鲁棒性。

Method: 使用包含277,524张彩色图像补丁的数据集，在6个节点上通过Dirichlet分布进行工作负载平衡分配，评估了FedAvg、多种拓扑结构的gossip学习（环形、随机度-3、完全连接）以及混合Gossip-FedAvg方法，并进行了多维度的敏感性分析。

Result: 混合Gossip-FedAvg和FedAvg在测试集上表现最优，取得了最高的ROC-AUC（平均0.9082）；混合方法获得了最高的PR-AUC（0.8240），而FedAvg的Brier分数最低（0.1335）。密集的gossip图提高了判别力但增加了通信负载。

Conclusion: FedAvg提供了最稳定可靠的服务器端基线，拓扑感知的gossip是一种可行的去中心化替代方案，而混合Gossip-FedAvg在点对点扩散和定期全球协调之间提供了平衡的折中方案。
...
### [84] [A multimodal large language model for evidence-based autism spectrum disorder screening](https://arxiv.org/abs/2609.16464)
...
TL;DR: 本文介绍了ASDchat，一个用于循证孤独症谱系障碍（ASD）筛查的多模态大语言模型，它能处理视频、音频和对话输入，生成筛查概率和可追溯的行为证据。


<details>
  <summary>Details</summary>
Motivation: ASD临床管理的早期筛查面临瓶颈，主要由于专业医生稀缺且传统评估工具具有主观性，需要一种客观、可扩展的筛查方法。

Method: 开发了采用双分支架构的ASDchat多模态大语言模型（决策分支生成概率，证据分支生成标准化的行为证据），并在包含1,035名参与者的数据集上进行训练和评估。

Result: 在ASD与典型发育（TD）儿童的区分中，AUC达到0.953；在9个未参与训练的保留站点上，平均AUC为0.932；模型还通过无监督聚类将ASD病例分为六个亚型并建议了相应干预措施。

Conclusion: ASDchat为临床实践中进行大规模、基于证据的早期ASD筛查提供了一条切实可行的途径。
...
### [85] [MDN-Control: Mask-Depth-Noise Guided Region Control for Multi-Subject Video Editing](https://arxiv.org/abs/2609.16475)
*Jiayi Yu,Xi Ye,Lina Wang,Yunkun Xia*

Main category: cs.CV

TL;DR: 该论文提出了MDN-Control，一个无需训练的多主体视频编辑框架，通过结合空间、几何和潜在先验来解决跨主体属性泄露和遮挡模糊问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多主体视频编辑方法依赖于掩码，但在区分重叠主体或确保生成一致性方面存在困难，容易出现跨主体属性泄露和遮挡模糊。

Method: 提出了MDN-Control框架，包含三个核心部分：用于一致目标定位的掩码引导定位、用于解决重叠主体边界模糊的深度感知遮挡控制，以及从噪声库检索高斯初始化的噪声潜在提示。

Result: 在MSVBench上的实验表明，MDN-Control实现了最低的CM-Err和最高的Q-Edit，同时保持了有竞争力的文本对齐和时间一致性。

Conclusion: 结合空间、几何和潜在先验能有效提升多主体视频编辑的性能，克服属性泄露和遮挡模糊等局限性。
...
### [86] [VPRef: A Cross-Domain Benchmark for Referring Remote Sensing Image Segmentation](https://arxiv.org/abs/2609.16486)
*Quanwei Liu,Tao Huang,Jiaqi Yang,Wei Xiang*

Main category: cs.CV

TL;DR: 本文针对指代遥感图像分割（RRSIS）中由于跨空间分辨率/光谱变化导致的视觉域漂移以及用户输入粒度不一致导致的文本逻辑漂移问题，提出了首个跨域RRSIS基准数据集（VPRef）和一个基于SAM3的高效参数微调基线框架。


<details>
  <summary>Details</summary>
Motivation: 实际部署中的指代遥感图像分割常面临“视觉域漂移”和“文本逻辑漂移”的双重漂移困境，严重影响模型性能，且此前缺乏相关的跨域基准数据集。

Method: 构建了包含46,972个三层语言层级三元组的Vaihingen-Potsdam Referring (VPRef) 数据集；基于Segment Anything Model (SAM3)结合LoRA开发了参数高效的域适配基线，利用伪标签驱动的自训练对抗视觉差异，并通过随机多粒度文本提示混合解决文本逻辑漂移。

Result: 实验表明，该框架仅修改了1.08%的基础参数量，即实现了优异的跨域分割边界效果，并揭示了语言方差驱动细粒度语义不变性、伪标签传播控制宏观空间网格对齐的机制。

Conclusion: 本文建立的VPRef数据集和高效域适配基线为未来多模态遥感域适配研究奠定了坚实的基础。
...
### [87] [Which Pretext Task Transfers? Self-Supervised Pretraining Objectives for Lung Ultrasound](https://arxiv.org/abs/2609.16551)
*Moein Heidari,Junbo Rao,Jai Choraria,Wenjin Chen,David J. Foran,Ilker Hacihaliloglu*

Main category: cs.CV

TL;DR: 该论文系统比较了三种自监督学习（SSL）预训练目标（对比学习、掩码重建和JEPA）在肺部超声（LUS）视频中的表现，发现不同方法在不同数据集上的迁移性能存在显著逆转。


<details>
  <summary>Details</summary>
Motivation: 肺部超声（LUS）中缺乏标注图像，自监督学习虽能缓解这一问题，但哪种预训练目标最优尚不明确，且以往研究缺乏统一的对比基准。

Method: 在相同的编码器骨干网络、预训练语料库、优化策略和冻结评估协议下，对比了VideoMAE、V-JEPA和MoCo三种方法，并在POCUS和Mendeley-Uganda数据集上通过线性、kNN和注意力探针在不同标签预算下进行评估。

Result: 在全标签预算下的POCUS数据中，VideoMAE和V-JEPA表现更好（平衡准确率约为66.5%和65.4%），而在独立的Mendeley-Uganda数据集上，MoCo表现最好（62.7%），V-JEPA接近随机水平（35.1%）。

Conclusion: 单一数据集上的探针准确率无法准确反映自监督学习目标在不同数据集间的泛化和迁移能力，预训练目标的选取代价在跨域应用中需要更加审慎。
...
### [88] [Vision And Text Transformer For Predicting Answerability On Visual Question Answering](https://arxiv.org/abs/2609.16565)
*Tung Le,Huy Tien Nguyen,Le Minh Nguyen*

Main category: cs.CV

TL;DR: 本文提出了一种名为VT-Transformer的新方法，用于解决视觉问答（VQA）中的可回答性预测任务。


<details>
  <summary>Details</summary>
Motivation: 现有的VQA可回答性研究通常采用二分类映射，无法准确反映该问题的本质，因此作者将其视为回归任务来处理。

Method: 提出了VT-Transformer模型，利用Transformer架构来有效提取和融合视觉与文本特征。

Result: 在VizWiz 2020数据集上的实验结果表明，与竞争基线相比，VT-Transformer具有更好的有效性和鲁棒性。

Conclusion: 将VQA可回答性作为回归任务并通过Transformer架构处理是有效且具有鲁棒性的方案。
...
### [89] [Counterfactual Reasoning for Robust Visual Question Answering](https://arxiv.org/abs/2609.16567)
*Truong-Binh Duong,Thanh-Ngan Tran,Ngoc-Thao Nguyen,Bac Le*

Main category: cs.CV

TL;DR: 本文提出了一种增强的对比学习训练框架，用于解决视觉问答（VQA）中的语言偏见和泛化能力不足问题。


<details>
  <summary>Details</summary>
Motivation: 现代VQA模型容易利用训练数据中的虚假相关性导致泛化能力差（语言偏见），现有的反事实学习方法在引导模型关注因果证据和增强特征区分度方面仍有提升空间。

Method: 提出包含三个关键创新点的训练框架：1) 用于稳定多目标优化的三阶段课程；2) 增强的批次对比（Batch-Contrastive）损失以学习更具区分度的特征；3) 两个新的正则化项：用于精炼预测空间的答案对比（AC）损失和用于强制因果视觉基础的梯度差异（GD）损失。

Result: 在VQA-CP v2基准测试上取得了61.64%的竞争性准确率，同时在标准VQA v2数据集上保持了62.80%的准确率，泛化差距仅为1.16%，实现了OOD鲁棒性与分布内性能的良好平衡。

Conclusion: 该研究所提出的反事实对比学习框架能有效克服语言偏见，在保持标准性能的同时显著提升了VQA模型的OOD泛化能力。
...
### [90] [Efficient Text-to-Image Generation: An Adaptive Step Schedule Controller for Diffusion Models](https://arxiv.org/abs/2609.16572)
*Kuluhan Binici,Cihan Acar,Shivam Aggarwal,Siying Liu,Tulika Mitra*

Main category: cs.CV

TL;DR: 提出了一种自适应扩散控制器，根据文本提示词的复杂度动态调整文本到图像扩散模型的去噪步数，在保持图像质量的同时减少推理时间。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型通常使用固定的去噪步数，未考虑不同文本提示词的复杂度差异，导致效率低下。

Method: 利用具有不同步长的步数调度混合，并在每个时间步评估误差项的差异，在不进行额外模型训练的情况下动态调整调度方案。

Result: 在COCO和DiffusionDB数据集上的实验表明，该方法在保持视觉保真度的同时有效减少了推理时间。

Conclusion: 该方法为文本到图像扩散模型提供了一种更高效的替代方案，能根据提示词复杂度自适应优化生成效率。
...
### [91] [GraLoD: Graphics-Inspired Continuous Level-of-Detail Learning for Image Restoration](https://arxiv.org/abs/2609.16578)
*Hu Gao,Lizhuang Ma,Yulong Chen*

Main category: cs.CV

TL;DR: 本文提出了GraLoD框架，借鉴计算机图形学中的LOD（细节层次）渲染思想，将图像恢复中的尺度视为空间和阶段相关的连续变量，实现了自适应的尺度选择，可作为即插即用模块提升各类图像恢复任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像恢复方法依赖于预定义的多尺度层次结构和固定融合方式，尺度由网络架构决定，这在处理包含异构退化的全能图像恢复（all-in-one restoration）时尤为明显。

Method: 提出GraLoD框架，复用原生编码器层次结构，将多尺度特征对齐到共享的LOD表示空间，并在每个解码器阶段预测阶段条件LOD场；每个空间位置连续查询两个相邻的表示层级。同时引入最小充分足迹校准（MSFC）和结构感知正则化（SAR）来规范尺度选择。

Result: 在特定任务和全能图像恢复任务的大量实验中，GraLoD均展现出一致的性能提升。

Conclusion: GraLoD通过将恢复尺度建模为连续变量并实现自适应查询，无需重新设计基础特征处理模块即可无缝集成到现有恢复骨干网络中，有效提升了图像恢复效果。
...
### [92] [FLAT: Resampling Image and Text into 1D Flexible-Length Aligned Transmodal Tokens for Retrieval and Generation](https://arxiv.org/abs/2609.16591)
...
TL;DR: 本文提出了FLAT（Flexible-Length Aligned Transmodal representations）框架，将多模态表征学习与生成任务进行联合优化，生成既可用于判别又能直接驱动生成的线性可插值嵌入。


<details>
  <summary>Details</summary>
Motivation: 传统的两阶段方法（先训练对比/自编码视觉编码器，再训练独立的生成模型）将生成性能限制在冻结的嵌入表征之后，为了弥合这一差距，本研究重新探讨了联合多模态表征学习与生成。

Method: 提出了FLAT框架，联合优化共享的多模态编码器和下游的文转图（T2I）及图转文（I2T）解码器；将多模态输入映射到统一的连续1D序列空间，通过在prefix-K令牌上应用嵌套Dropout来实现动态输出长度。

Result: FLAT在单阶段预训练后即可进行跨模态检索与生成（T2I GenEval得分为71.1）；经过任务微调后，在T2I生成（GenEval 83.1）、图像描述（MS-COCO BLEU-4 40.5, CIDEr 138.6）以及跨模态检索任务上达到了SOTA或同等水平，且表征原生支持线性插值、潜在空间算术和零样本组合检索。

Conclusion: FLAT成功实现了多模态表征学习与生成的联合优化，其生成的柔性长度对齐跨模态表征兼具强大判别语义与卓越生成能力，支持多种复杂的下游应用。
...
### [93] [FRPSS: Feature Rearrangement in Pre-Shape Space for Single-Image Generation](https://arxiv.org/abs/2609.16594)
*Yuexing Han,Haoxuan Zhang,Bing Wang*

Main category: cs.CV

TL;DR: 本文提出了一种基于预形状空间特征重排的单图生成方法（FRPSS），以解决现有方法在平衡全局结构完整性和局部多样性时的结构错位问题。


<details>
  <summary>Details</summary>
Motivation: 现有的单图生成方法通常依赖随机噪声来驱动生成过程，缺乏显式的全局结构约束，导致在发生结构变化时容易出现空间结构错位。

Method: 提出了FRPSS方法，核心是测地表面特征增强的流形结构重排（MSR-FAGS）模块，用重排的预形状特征替换低尺度生成器的随机初始化特征；同时设计了尺度自适应滑动窗口补丁提取（SSPE）策略和CLIP-SSPE监督模块以支持下游任务。

Result: 定性和定量实验表明，FRPSS在所有三个数据集上都取得了最佳的单图Fréchet Inception Distance (SIFID)评分，同时保持了具有竞争力的LPIPS指标，且在多个下游任务中验证了有效性。

Conclusion: FRPSS通过在预形状空间中进行特征重排，有效解决了单图生成中的结构错位问题，并在图像生成和下游风格化等任务中表现出优越性能。
...
### [94] [A Vision-Language Foundation Model for Precise and Comprehensive Brain Tumor Diagnosis from Preoperative Multimodal Data](https://arxiv.org/abs/2609.16597)
...
TL;DR: 本研究开发了BrainVLM模型，这是一种基于MRI的人工智能系统，能够自动对12种WHO 2021脑肿瘤类型进行分类，量化诊断不确定性，并生成放射学报告。


<details>
  <summary>Details</summary>
Motivation: 由于脑肿瘤影像特征重叠、观察者间变异性以及专家培训成本高，非侵入性术前脑肿瘤诊断具有挑战性，迫切需要开发自动化且可靠的AI辅助诊断模型。

Method: 开发了BrainVLM模型，整合了不确定性量化策略和放射学报告生成模块。利用来自40,043名个体多模态数据进行训练，并在5,211名经病理确诊的患者（包括内部和11家外部独立医院）中进行验证，同时开展了多读者回顾性研究、前瞻性真实世界研究以及分子亚型预测验证。

Result: BrainVLM在多中心独立验证中表现出优异的脑肿瘤分类性能，能有效量化预测可靠性并生成合理的临床报告，且在多读者研究和前瞻性研究中证实了其改善临床工作流的实用价值。

Conclusion: BrainVLM作为一种可靠的AI工具，能够准确进行脑肿瘤分类、不确定性评估和报告生成，具有辅助神经放射科医师进行术前诊断和临床决策的巨大潜力。
...
### [95] [SAVOR: Self-Aware Visual Grounding via Confidence-Calibrated Reinforcement Learning for Multimodal Hallucination Mitigation](https://arxiv.org/abs/2609.16601)
*Zixiu Ding,Zilin Zhao,Yingjie He,Xinlang Kang,Guansu Wang,Wei Zhang*

Main category: cs.CV

TL;DR: 本文提出了Savor框架，通过校准自我评估（置信度预测）和GRPO训练优化，解决了多模态大语言模型（MLLMs）中的视觉幻觉和不确定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的缓解MLLM幻觉的方法（如修改解码策略或DPO微调）存在增加推理延迟或无法教会模型识别自身回答不可靠的问题，缺乏校准的自我评估信号。

Method: 引入Savor训练框架：（1）扩展输出模式以包含token和答案的置信度；（2）使用组相对策略优化（GRPO）目标函数，惩罚校准误差和不良的拒绝回答决策；（3）在推理时利用学习到的置信度，仅在模型不确定时重新审视视觉证据。

Result: 在多个基准测试（POPE、HallusionBench等）上的实验表明，Savor在保持通用能力的同时减少了幻觉，并实现了比DPO和解码基线更低的期望校准误差（ECE）。

Conclusion: 校准自我评估是减少MLLM幻觉的有效信号，Savor框架能够显著提升模型的可信度与校准性能，且不损失通用视觉语言能力。
...
### [96] [G3AR: Graph-Guided Neural Visual Geometry for Scalable Multi-Sequence Aerial Registration](https://arxiv.org/abs/2609.16603)
*Jeng Wen Joshua Lean,Ting-Yu Yen,Wei-Fang Sun,Simon See,Hung-Kuo Chu,Shih-Hsuan Hung*

Main category: cs.CV

TL;DR: 本文提出了G3AR（Graph-Guided Neural Visual Geometry for Aerial Registration）框架，用于解决大规模航拍图像序列的可扩展密集神经几何对齐问题。


<details>
  <summary>Details</summary>
Motivation: 全上下文神经视觉几何不适用于海量图像，而基于序列的分块方法难以捕捉多序列航拍集合中不规则的非局部重叠。

Method: G3AR构建了一个几何验证的图像邻近图来引导有界重叠块，并利用最大生成树定义对齐拓扑；通过独立处理分块并通过共享图像预测估计Sim(3)变换，将局部相机和几何统一在公共坐标系中。

Result: 在四个真实的航拍场景中，G3AR在姿态误差和运行时间上均优于基于VGGT和Pi3的对比方法，其DA3变体在所有评估的神经几何方法中实现了最低的姿态误差。

Conclusion: G3AR通过图引导的框架有效解决了大规模航拍图像的密集神经几何对齐问题，显著提高了姿态估计的准确性和计算效率。
...
### [97] [EgoPathBench: Evaluating Zero-Shot Egocentric Waypoint Decision-Making in Vision-Language Models](https://arxiv.org/abs/2609.16610)
*Yang Zhao,Zhuo Chen,Xubo Yang*

Main category: cs.CV

TL;DR: 本文介绍了EgoPathBench，一个用于评估视觉语言模型在第一人称视角下零样本路径点导航能力的基准数据集，并证明了通过微调可以显著提升模型空间推理和导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有的空间智能基准主要评估孤立的几何判断，无法直接测量结合了目标识别、动作后果评估、距离估计和路径规划的集成导航能力。

Method: 引入了包含31,852个训练样本、1,345个验证样本和1,111个测试样本的EgoPathBench数据集及五个导航决策任务，并使用该训练资源对Qwen 3.5 4B模型进行了微调。

Result: 现有九个VLM的表现普遍较低（最高分仅28.3），在具身约束和完整路径规划上表现尤为不足；微调后的Qwen 3.5 4B模型的EgoPath得分从3.9提升至38.9，并在其他三个外部空间基准测试中也有所提升。

Conclusion: 当前视觉语言模型在形成完整、符合目标的具身路径方面仍存在局限性，但通过专门的数据集和微调可以有效提升模型的空间导航能力。
...
### [98] [JewelTry: Mask-Free Scale Aware Jewelry Virtual Try-On](https://arxiv.org/abs/2609.16626)
*Xinlei Niu,Peixia Li,Jun Wang,Chenchen Xu,Jiayu Yang,Jing Zhang,Pulak Purkait,Hongdong Li*

Main category: cs.CV

TL;DR: 本文提出了一个用于首饰虚拟试戴的基准数据集JVTO-Bench和一个无需掩码的扩散框架JewelTry，实现了考虑尺度的真实首饰试戴。


<details>
  <summary>Details</summary>
Motivation: 由于首饰尺寸小、结构刚性且对细节敏感，现有的虚拟试戴技术在首饰类别上研究不足，且缺乏对产品尺度的显式建模。

Method: 引入了JVTO-Bench基准数据集（包含真实世界的产品尺度标注），并提出了JewelTry框架，该框架包含一个用于编码产品尺寸的尺度适配器、单向条件注意力机制以及注意力精炼损失。

Result: 大量实验表明，JewelTry在视觉保真度、背景保留、物体一致性和尺度准确性之间取得了平衡，为无掩码、尺度感知的首饰虚拟试戴建立了一个强有力的基线。

Conclusion: JVTO-Bench和JewelTry有效地解决了首饰虚拟试戴中的尺度和细节保持问题，推动了该领域的发展。
...
### [99] [Can Knowledge Transfer Parameters Be Learned? LePoKet for Efficient Robotic Vision](https://arxiv.org/abs/2609.16637)
*Yanick C. Tchenko,Felix Mohr,Hicham Hadj-Abdelkader,Hedi Tabia*

Main category: cs.CV

TL;DR: 本文提出了LePoKet（基于可学习参数优化的知识迁移）框架，通过引入可买卖的提取-变换-混合（Extract-Transform-Mix）接口和可学习遗传注意力（LGA）算子，实现了高效的机器人感知知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有的知识蒸馏方法通常依赖于固定的蒸馏目标或手动设计交互机制，无法满足受计算、内存和延迟限制的机器人系统的高效感知需求。

Method: 提出LePoKet框架，将知识继承直接嵌入前向计算中，采用块状的Extract-Transform-Mix接口，其交互参数通过LGA算子与子网络联合优化，无需辅助蒸馏损失或温度缩放。

Result: 在CIFAR-10和CIFAR-100上，相对标准子网络训练分别实现了24.57%和25.1%的误差降低；在稠密运动估计任务中，显著提升了紧凑型RAFT模型的性能，在Sintel和KITTI数据集上取得了更低的端点误差（EPE）。

Conclusion: LePoKet证明了可学习的结构化知识迁移能够很好地泛化于识别和运动感知任务，为高效的机器人视觉提供了一种有前景的方法。
...
### [100] [What Do Hallucinations Reveal About Multimodal Reasoning? Diagnosing Visual Grounding Failures via Contrastive Decoding Probes](https://arxiv.org/abs/2609.16646)
*Zhipeng Zhao,Wenxu Wang,Peishun Liu,Ruichun Tang*

Main category: cs.CV

TL;DR: 本文提出了一种名为SAFE的训练无关解码框架，利用大型视觉语言模型（LVLMs）自身作为实验工具，通过对比视觉基础路径和视觉消融路径来检测和纠正视觉幻觉。


<details>
  <summary>Details</summary>
Motivation: 随着强大视觉语言模型的普及，科学进展需要超越基准分数的、将模型作为理解行为的科学仪器的研究方法；研究如何利用LVLMs作为实验仪器来研究其自身的失败动态（特别是视觉幻觉）。

Method: 引入SAFE（Score-based Awareness for Hallucination Elimination）训练无关解码框架，对比视觉基础（visually-grounded）和视觉消融（vision-ablated）的生成路径，产生标记级的对比接地分数，并作为解码时的惩罚基础。

Result: 实证发现视觉依赖随生成衰减、幻觉在时间上成簇出现、早期干预可减少成簇且不明显降低流畅度；在MMHalBench上，SAFE显著优于所有对比基线。

Conclusion: 设计对比探针体现了将模型作为科学理解仪器的更广泛使命，证明了利用LVLMs研究其自身失败动态的可行性和有效性。
...
### [101] [ViD: Vision-Dominant Gender Bias Mitigation for Large Vision-Language Models](https://arxiv.org/abs/2609.16647)
*Zhipeng Zhao,Zhaoqiang Wei,Peishun Liu,Youwei Zhao,Ruichun Tang*

Main category: cs.CV

TL;DR: 本文提出了ViD，一个受因果关系启发的框架，无需额外训练即可有效减轻大型视觉语言模型（LVLMs）中的性别偏见。


<details>
  <summary>Details</summary>
Motivation: 现有缓解性别偏见的方法多依赖训练阶段调整或事后校准，存在无法捕捉实时视觉-文本不一致性、依赖预定义偏见分类体系以及跨模态对齐退化等局限性。

Method: 利用因果启发框架分析注意力机制以识别语言先验的混杂效应，引入后门调整对抗强语言先验，并在解码层中优化词元选择。

Result: 在FACET单属性评估中性别偏见减少了14.7%，在MS COCO图像标题生成任务中LLaVA的性别偏见得分从0.6708提升至0.9978，且无需额外的训练开销。

Conclusion: ViD是一个可扩展且实用的解决方案，能够在不损害通用推理能力和文本生成质量的前提下，显著减轻LVLMs中的性别偏见并提高输出公平性。
...
### [102] [Channel-Wise and Token-Aware Post-Training Quantization for Visual State Space Duality](https://arxiv.org/abs/2609.16656)
*Jonghyeon Lim,Changhoon Yim*

Main category: cs.CV

TL;DR: 本文研究了视觉状态空间对偶模型（VSSD）的低比特后训练量化（PTQ），并提出了一种名为CTOAC的方法，在保持精度的同时实现了硬件加速。


<details>
  <summary>Details</summary>
Motivation: 状态空间模型（如VSSD）在视觉任务中表现出色，但其在低比特后训练量化（PTQ）下的行为尚不明确，且激活量化是低比特量化的主要瓶颈。

Method: 通过权重-激活分离识别出瓶颈，并提出了通道级令牌平衡输出感知裁剪（CTOAC）方法，通过最小化令牌平衡重建损失来学习每个输入通道的裁剪边界。

Result: 在VSSD-Tiny、Small和Base模型上，CTOAC方法在ImageNet-1K分类以及COCO和ADE20K检测分割任务中均保持了高精度，并在RTX 4090上实现了高达1.42倍的端到端加速。

Conclusion: CTOAC有效地解决了VSSD模型中激活量化带来的瓶颈，使得视觉状态空间模型在低比特下能够保持强大性能并实现高效部署。
...
### [103] [SAVTrack: Selective Vote Aggregation for Reliability-Aware Point Cloud Tracking](https://arxiv.org/abs/2609.16662)
*Sifan Zhou,Linyue Tan,Qiwei Wang,Ziyu Zhao,Xiaobo Lu*

Main category: cs.CV

TL;DR: 本文提出了SAVTrack，一个基于激光雷达点云的3D单目标跟踪框架，通过引入选择性投票聚合（SAV）机制来过滤不可靠的点到中心投票，从而提高在稀疏观测下的定位精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在激光雷达点云的3D单目标跟踪中，稀疏和不完整的观测导致不同的目标点对物体中心的约束极不均匀，现有的基于点的方法直接聚合所有假设而未显式建模其可靠性，导致不准确的投票污染聚类并降低定位精度。

Method: 提出SAVTrack运动感知跟踪框架，利用局部种子特征和帧间运动上下文估计每个候选投票的可靠性，并在提案聚类前移除低置信度的假设（选择性投票聚合SAV）。

Result: 在KITTI和nuScenes数据集上取得了竞争力的性能（Success/Precision分别为68.4/87.4和58.44/69.82），运行速度达82 FPS，且仅保留密集聚合所用候选投票不到六分之一的数量。

Conclusion: SAVTrack通过预聚合门控机制有效阻止了不可靠假设对集群形成的影响，在保持较低计算开销的同时，对稀疏目标观测表现出极高的有效性。
...
### [104] [Bridging the Perceptual Gap: Residual-Enhanced Downscaling and Manifold-Aware Perception Alignment Adaptation for NR-IQA](https://arxiv.org/abs/2609.16664)
*Yu Li,Zhengran Shen,Yachun Mi,Puchao Zhou,Shaohui Liu*

Main category: cs.CV

TL;DR: 本文提出了跨模态感知对齐适配器（CMPA），解决了大视觉语言模型（如CLIP）在无参考图像质量评估（NR-IQA）中因优先考虑语义不变性而忽略细微感知信号（感知淹没）的问题。


<details>
  <summary>Details</summary>
Motivation: CLIP的对比预训练固有地优先考虑语义不变性，这会抑制微妙的感知信号（感知淹没），且标准预处理会加剧高频质量线索的丢失。

Method: 提出跨模态感知对齐适配器（CMPA），包含用于放大畸变导致的流形外偏差的感知敏感特征提取器（PFE）、将特征与质量感知文本锚点对齐并重新注入的跨模态感知对齐注入器（PAI），以及利用最小可觉差（JND）引导的频率重新注入来补偿分辨率信息丢失的残差增强感知降尺度策略。

Result: 在多个基准数据集上的广泛评估表明，该方法显著优于现有最先进的方法，有效恢复了掩盖在语义密集表示中的感知信号。

Conclusion: CMPA框架能够有效解决CLIP在NR-IQA中的感知淹没问题和预处理引起的信息丢失，从而显著提升图像质量评估的性能。
...
### [105] [Lesion-centered 3D mapping of colonoscopy procedures: validation of a hierarchical ensemble pipeline on public benchmark videos](https://arxiv.org/abs/2609.16672)
*Hyunjun Kim,Hyeonwoo Na,Jaewoo Lee*

Main category: cs.CV

TL;DR: 本研究提出了一种无需全结肠3D重建的病灶中心空间记录方法，通过四层分层管道整合全局拓扑图、时空轨迹、局部3D重建和病灶持久身份识别。


<details>
  <summary>Details</summary>
Motivation: 结肠镜检查通常会丢弃视频中的空间信息（如内镜轨迹、病灶位置及重复观察情况），本研究旨在确定是否可以在不进行全结肠3D重建的情况下组装和验证以病灶为中心的空间记录。

Method: 构建了一个四层分层管道：（1）全局拓扑图，（2）病灶级时空轨迹，（3）按需局部3D重建，以及（4）跨重复观察的持久病灶身份识别，并在四个公开视频（共40,245帧）上进行端到端运行验证。

Result: 通过入口图贝叶斯定位检测到了复查事件；在采用0.5的阈值进行病灶身份合并时，保持了1.0的真实纯度并自动合并了231个候选对中的20个；内镜专用几何引擎在所有指标上均优于通用基础模型（总体绝对相对误差AbsRel为0.2276对0.3523）。

Conclusion: 研究结果确立了一条切实可行的近期路径：复查检测、病灶身份识别和局部3D重建均能产出定量且可重复的结果，而无需等待完整的几何重建，下一步是在临床数据上验证该记录。
...
### [106] [MEgoVista: Multi-view Ego-aware Motion Estimation for Metric 4D Hands and Head in the Wild](https://arxiv.org/abs/2609.16684)
*Jiangong Xiao,Zhihao Zhang,Yifei Dong,Chao Ma,Zhouyi Jin,Zhiwen Hou,Li Liu,Weihuang Chen,Hongbin Sun,Maoqing Yao*

Main category: cs.CV

TL;DR: MEgoVista是一个离线流程，能够将单个未受约束的自我视角（egocentric）视频转换为同一重力对齐世界坐标系中的度量双手和头部运动。


<details>
  <summary>Details</summary>
Motivation: 现有的度量手部标签依赖于受限的影棚设备或头戴式装置，无法在野外无准备的环境中使用且缺乏独立参考验证。为了扩展手部动作监督数据的采集范围，需要一种无约束的头戴式录制方案。

Method: 引入MEgoVista离线管道：1. 在检测时确定手部归属以排除旁观者干扰；2. 利用校准立体视觉而非单目先验来获取物理单位的度量尺度；3. 在动作捕捉体积内使用独立的Chingmu光学捕捉进行基准测试和评估。

Result: 成功实现从无约束的自我视角视频中提取出具有物理单位的度量级手部和头部运动，并在动作捕捉环境中通过了严格的定量评估和审计。

Conclusion: MEgoVista提供了一种从自我视角视频获取度量级手部监督数据的可靠途径，有效扩展了手部动作标签的采集场景。
...
### [107] [Efficient Quantization-Aware Distillation with Cross-Modal Alignment for Edge Vision-Language Models](https://arxiv.org/abs/2609.16689)
*Jinwoo Jeon,GyuYeop Do,Yubin Lim,Nam-Joon Kim,Hyun Gon Ryu,Hyuk-Jae Lee,Byung-Jun Lee*

Main category: cs.CV

TL;DR: 本文提出了一种针对边缘设备部署的统一量化语义蒸馏框架，解决了现有方法在多模态大模型压缩中优化不一致和模态不平衡的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的EdgeVL等方法采用两阶段优化（蒸馏和量化感知训练目标不同），在量化学生空间中进行对比学习导致优化不一致和训练效率低下，且对RGB和非RGB模态采用相同监督导致模态失衡。

Method: 提出一个统一的教师锚定框架，联合优化蒸馏与量化以确保训练一致性并扩大决策边界；设计了一个轻量级交叉注意力适配器，通过RGB引导的语义转移来增强非RGB表示并缩小模态差距。

Result: 大量实验表明，该方法在保持边缘部署效率的同时，显著且一致地提升了非RGB模态的性能。

Conclusion: 该统一框架有效克服了边缘视觉语言模型压缩中的优化冲突和模态不平衡问题，实现了高效且高性能的开放词汇分类。
...
### [108] [Efficient 3D Whole-Body PET Image Denoising via Conditional Rectified Flow With Optimized Sampling Strategy](https://arxiv.org/abs/2609.16690)
*Jiale Shen,Guolin Wang,Chenhao Wang,Xinhui Su,Wei Luo,Feng Yu*

Main category: cs.CV

TL;DR: 本文提出了一种基于一阶段条件3D修正流（3D Flow）的全身体PET图像去噪框架，在保证重建质量的同时显著提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 减少正电子发射断层扫描（PET）中的辐射暴露对患者安全至关重要，但超低剂量成像会导致严重噪声。现有的3D深度生成模型（如扩散模型）推理时间过长，而2D替代方案难以保持体积一致性。

Method: 提出了一种结合新型优化非均匀采样策略的一阶段条件3D修正流（3D Flow）框架，采用一阶段线性插值速度匹配目标进行训练。

Result: 该模型在约30秒内即可重建完整的3D体积，推理速度远快于3D DDPM基线；在独立临床数据集的零样本迁移评估中，实现了良好的全局图像质量和病灶显眼性，甚至在1/100标准剂量的超低剂量下也表现出色。

Conclusion: 该方法平衡了重建保真度和计算效率，为超低剂量全身体PET图像去噪提供了一种有前景的候选方案。
...
### [109] [MAETrack: Unleashing the Potential of Pretrained Geometric Priors for 3D Single Object Tracking](https://arxiv.org/abs/2609.16695)
*Sifan Zhou,Qiwei Wang,Linyue Tan,Ziyu Liu,Ziyu Zhao,Xiaobo Lu*

Main category: cs.CV

TL;DR: 本文研究了大规模自监督3D预训练（如MAE）向3D单目标跟踪（SOT）迁移时效果不佳的问题，并提出了一种名为MAETrack的轻量级适配框架。


<details>
  <summary>Details</summary>
Motivation: 直接微调自监督3D编码器效果次优，因为重建目标与跟踪所需的时空匹配要求不完全对齐，且存在层级迁移不匹配现象（深层网络过度特化于重建任务）。

Method: 提出了MAETrack框架，包含两部分：1）层选择性初始化（LSI），仅用预训练权重初始化浅层阶段，重置深层阶段；2）几何残差门控（GRG），通过残差空间调制增强搜索BEV特征中的结构显著区域。

Result: 在标准3D SOT基准测试上的广泛实验表明，MAETrack以有限的计算开销一致地优于传统的微调基线。

Conclusion: 从3D重建预训练到3D跟踪的有效迁移不只是部分微调的问题，而是依赖于一种以跟踪为导向的迁移原则：保留浅层几何特征，同时将深层表示适配到下游目标。
...
### [110] [PriorPose: Reference-Guided Joint Deformation and Alignment for Category-Level Object Pose Estimation](https://arxiv.org/abs/2609.16727)
*Yihan Chen,Huan Ren,Wenfei Yang,Hang Du,Tianzhu Zhang,Feng Wu*

Main category: cs.CV

TL;DR: 本文提出了PriorPose，一种基于参考引导的类别级物体位姿估计框架，通过在共享特征空间中联合求解规范化和对齐，避免了传统方法的误差级联和泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于对应关系的方法存在缺陷：无先验变体将规范坐标隐式编码在权重中，限制了泛化能力；有先验变体采用串行的变形然后对齐流程，容易产生误差级联。

Method: 提出了PriorPose框架，利用参考引导的种子Transformer将局部观测和类别先验融合，通过几何感知种子联合预测NOCS场和规范形状变形，并结合双部分形状一致性损失来联合优化对应关系、变形和位姿。

Result: 在标准和更大类别的基准测试中，PriorPose在大多数评估指标（特别是严格的位姿阈值下）上取得了新的SOTA结果，并表现出对形状变化和域偏移的更强鲁棒性。

Conclusion: 通过显式保留类别先验并在共享特征空间中联合求解规范化与对齐，PriorPose有效克服了传统方法的局限性，显著提升了类别级物体位姿估计的准确性与鲁棒性。
...
### [111] [De-GAN - Dynamic Parameter Tuned GAN for 3D Medical Image Segmentation: A Step Towards Generalisation](https://arxiv.org/abs/2609.16755)
*Zoha Usama,Azadeh Alavi*

Main category: cs.CV

TL;DR: DE-GAN是一个用于脑肿瘤分割的对比度增强条件GAN，通过生成切片自适应的FLAIR图像来改善增强肿瘤（ET）的低对比度和域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤分割中增强肿瘤（ET）对比度低、与周围组织重叠，且扫描仪与场地的差异导致了域偏移，这使得分割任务非常困难。

Method: 提出了DE-GAN，结合了输入自适应动态卷积、风格感知特征混合和坐标编码来合成切片自适应的FLAIR图像；采用标签引导的类条件目标来分离肿瘤核心（TC）和ET的强度；并将生成的FLAIR与原始模态拼接用于训练3D U-Net。

Result: 在BraTS 2015、2018和2019数据集上，DE-GAN在大多数报告的TC/ET指标上优于基线和静态EnhGAN，保留原始和增强的FLAIR可获得最大收益。

Conclusion: DE-GAN通过合成高质量的自适应FLAIR图像，有效提高了脑肿瘤分割的准确性，缓解了低对比度和域偏移带来的影响。
...
### [112] [HLC-GS: Risk-Map-Guided Height-Layer Consistency Gaussian Splatting for DSM Reconstruction from Optical Satellite Imagery](https://arxiv.org/abs/2609.16772)
*Jie Yang,Yingdong Pi,Qiyan Luo,Xiaoyu Wang,Lekang Wen,Mi Wang*

Main category: cs.CV

TL;DR: 本文提出了HLC-GS方法，利用风险图引导的高度层一致性高斯泼溅技术，解决了多视光学卫星影像数字表面模型（DSM）重建中的高度层混合误差问题。


<details>
  <summary>Details</summary>
Motivation: 基于3D高斯泼溅（3DGS）的DSM生成中，alpha加权聚合会在同一像素处混合不同高度层的泼溅点，产生非物理的中间高程和高度层混合误差。

Method: 提出了HLC-GS方法，包含一个风险图模块（用于定位异常高度分散的高风险像素）、一个主层可靠性校正模块和一个次层抑制模块。

Result: 在DFC2019和IARPA2016数据集上的实验表明，与最新的EOGS相比，HLC-GS将平均MAE从1.46米降至1.18米，平均RMSE从2.78米降至2.58米，PAG_{2.5}从86.09%提高到88.61%。

Conclusion: 显式建模每个像素的高度层一致性可以有效减轻高度层混合，从而提高基于3DGS的光学卫星影像DSM重建几何质量。
...
### [113] [FSANet: Frequency-Spatial Aware Network for Image Segmentation](https://arxiv.org/abs/2609.16773)
*Ruibo Wang,Ziyi Shen,Huaming Wu,Dong Liang,Kun Shang*

Main category: cs.CV

TL;DR: 本文提出了频空感知网络（FSANet）和一个名为SceneX的新数据集，旨在解决图像分割中由于遮挡、光照差和不规则结构带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的方法计算成本高且忽略了先验知识或噪声模式，导致细节丢失和边界模糊；同时缺乏涵盖各种真实世界复杂场景的综合分割数据集。

Method: 提出了Frequency Spatial Aware Network (FSANet)，包含结构先验模块、双域感知模块和边缘估计模块；并引入了包含10个挑战性非理想场景的新数据集SceneX。

Result: 大量实验证明了FSANet的高效性和有效性，SceneX数据集也为评估和提升分割模型的鲁棒性及现实应用能力建立了新基准。

Conclusion: FSANet通过结合先验知识与双域求解器成功应对了复杂图像分割任务，而SceneX数据集则有效弥补了现有数据集在真实世界复杂场景方面的不足。
...
### [114] [IMVS: Interactive Medical Volume Segmentation with Test-Time Adaptation - A New Method for Annotating Radiology Datasets](https://arxiv.org/abs/2609.16775)
*Abhilaksh Singh Reen,Kushal Borkar,Ritvik Mahapatra*

Main category: cs.CV

TL;DR: IMVS是一个人机协同的3D医学图像交互式标注框架，通过在线微调的2D适配器、冻结的3D追踪器和教师-学生对齐机制，显著减少了3D医学影像标注的工作量。


<details>
  <summary>Details</summary>
Motivation: 大规模放射学数据集的逐层3D标注工作量巨大，现有的交互式方法效率低下：逐层方法忽略了层间连续性，而3D/视频方法使用固定传播器容易导致低对比度或病理结构上的漂移，需要频繁重新提示。

Method: 提出了IMVS框架，包含三个闭环组件：1）根据用户涂鸦在线微调的轻量级2D切片掩码适配器（SMA）；2）在相邻切片间传播掩码的冻结体掩码追踪器（VMT）；3）限制灾难性遗忘的软教师-学生对齐机制。

Result: 在8个公共CT/MRI数据集上，IMVS在保证分割质量的同时大幅减少了标注工作量，比熟练的复制手动工作流程快14.4倍，比3D交互方法快1.9倍，在具有挑战性的目标上优势尤为明显。

Conclusion: IMVS通过将2D在线自适应与3D传播机制结合形成闭环，有效解决了医学影像3D标注中的效率瓶颈和漂移问题，显著提升了复杂结构的标注效率。
...
### [115] [Unifying Semantic Priors and High-Frequency Traces: Enhancing V-JEPA with Mixture-of-Experts for Robust Synthetic Image Forensics](https://arxiv.org/abs/2609.16778)
*Simone Teglia,Irene Amerini*

Main category: cs.CV

TL;DR: 本文首次将联合嵌入预测架构（JEPA）应用于深度伪造检测，提出了MoE-JEPA双流架构，通过利用JEPA的世界模型先验知识和混合专家机制，实现了最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现代基于ViT的深度伪造检测器在面对社交媒体压缩或传输等复杂场景时，全局理解能力不足，难以准确区分真实和虚假多媒体内容，因此需要更强大的视觉现实通用表征作为先验知识。

Method: 提出MoE-JEPA双流架构，将V-JEPA 2主干网络与残差混合专家（MoE）机制及噪声流分支相结合，动态内化取证知识，并采用门控注意力多实例学习（MIL）模块以实现精确的空间语义理解。

Result: 在包含30K图像的SID-Set基准测试中，MoE-JEPA取得了95.54%的准确率，以更小的模型规模超越了现有的大型模型，确立了新的技术水平（SOTA）。

Conclusion: JEPA模型固有的世界理解能力可以作为深度伪造检测的强大先验，结合MoE机制和MIL模块的MoE-JEPA架构能够有效应对复杂场景下的图像造假检测挑战。
...
### [116] [PSMP-CLIP: Patch-Prompt SAM and Multi-Semantic Prompting for CLIP-Based Zero-Shot Anomaly Detection](https://arxiv.org/abs/2609.16785)
*Xuezhi Xiang,Guanghao Wu,Heqi Xiang,Jiayao Liu,Xiaoheng Li,Yiming Chen,Shanjun Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为PSMP-CLIP的方法，用于解决零样本异常检测中的粗糙异常图和有限语义提示问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的零样本异常检测方法存在异常图粗糙和语义提示受限的缺陷。

Method: 提出了PSMP-CLIP，包含两个核心模块：集成补丁提示的SAM2分割（PPSS）和多语义引导的提示正则化（MSGPR）。

Result: 在14个数据集上进行了实验，表现出极具竞争力的性能，并在MVTec AD、BTAD、DTD-Synthetic、CVC-ClinicDB、TN3K、Endo和Kvasir上取得了最佳的像素级AUROC。

Conclusion: 该方法通过结合精确的掩码生成和多语义提示正则化，有效提升了零样本异常检测的准确性和泛化能力。
...
### [117] [TEDi: Temporal Memory-Enhanced and Denoising Transformer for Surgical Instrument Segmentation](https://arxiv.org/abs/2609.16797)
*Jiahong Yuan,Weiming Mi,Tao Zhang,Haoyin Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种名为TEDi的时序记忆增强与去噪Transformer方法，用于解决手术器械分割中缺乏跨帧时序先验和时序一致性约束的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于查询的手术器械分割方法主要依赖单帧预测，忽视了跨帧时序先验和时序一致性约束，导致查询表示不稳定和分类识别次优。

Method: 提出了TEDi模型，包含两个核心模块：1) 记忆搜索增强编码器，利用查询级记忆库从历史帧检索判别性表示以丰富当前帧特征；2) 时序一致性去噪模块，构建时序一致的参考作为跨帧语义锚点，抑制不稳定预测并促进语义连贯性。

Result: 在EndoVis 2017和EndoVis 2018两个基准数据集上的广泛实验表明，TEDi consistently 优于当前最先进的方法。

Conclusion: TEDi有效地利用了时序信息和一致性约束，提高了手术器械分割的稳定性和准确性，具有推动计算机辅助外科发展的潜力。
...
### [118] [Hyper-RED: Scalable Event Pre-training via Semantic Hypergraph Distillation](https://arxiv.org/abs/2609.16811)
*Meisen Wang,Zhiqiang Tian,Wei Bao,Chengjie Wang,Shaoyi Du,Siqi Li*

Main category: cs.CV

TL;DR: Hyper-RED是一个用于事件相机表示学习的新型图像到事件预训练框架，通过超图对齐高阶语义结构，克服了模态差异。


<details>
  <summary>Details</summary>
Motivation: 大规模标注事件数据的缺乏阻碍了事件表示学习的扩展，而现有的图像到事件方法依赖于刚性的像素级或标记级对齐，忽视了模态差异，容易导致语义崩溃并限制可迁移性。

Method: 提出Hyper-RED框架，利用DINOv3提取空间标记表示，并构建图像、事件和跨模态语义超图，通过超图关系蒸馏损失实施互补的模态内和跨模态约束。

Result: 在三个任务的五个事件数据集上的实验表明，该方法实现了从ViT-S到ViT-L的一致扩展，并取得了SOTA（最先进的）性能。

Conclusion: Hyper-RED通过超图建模高阶语义关联，在适应模态特定差异的同时实现了有效的跨模态知识迁移，提升了事件相机的视觉感知能力。
...
### [119] [What Breaks Local Watermarks? A Robustness Benchmark for Local Invisible Image Watermarking](https://arxiv.org/abs/2609.16832)
*Kai Yao,Bence Szilágyi,Sebestyén Kamp,Máté Poór,Máté Szilveszter,Matyas K. Zsoldos,Marc Juarez*

Main category: cs.CV

TL;DR: 本文提出了首个针对局部图像水印的系统性鲁棒性基准测试，评估了55种图像变换下的5种主流局部水印方法。


<details>
  <summary>Details</summary>
Motivation: 现有局部水印研究缺乏统一的评估标准，变换选择、数据集和指标各不相同，导致方法间无法直接比较，且对鲁棒性的整体认知模糊。

Method: 建立包含55种图像变换（包括信号失真、坐标对齐改变、间接局部编辑和直接水印编辑）的系统性基准，对MaskWM、WAM、OmniGuard、TrustMark和PixelSeal五种局部水印方法进行全面评估。

Result: 所有评估方法都容易受到某些变换的影响；MaskWM在载荷恢复和定位方面表现最强，但纯净设置下的图像质量最低；几何不对齐和生成式局部编辑会严重破坏载荷恢复；载荷恢复与定位相关但不可互换。

Conclusion: 局部水印的鲁棒性高度依赖于变换的类型，虽然顶级方法能耐受信号失真，但几何错位和生成式编辑仍然是当前局部水印技术的重大挑战。
...
### [120] [StackTok: Accelerating VLMs Inference with Budget-Adaptive Visual Token Selection](https://arxiv.org/abs/2609.16841)
*Zhenbin Wang,Lei Zhang,Lituan Wang,Wei Huang,Yan Wang,Zhenwei Zhang*

Main category: cs.CV

TL;DR: StackTok是一个无需训练的视觉语言模型（VLM）视觉token选择器，通过动态平衡查询相关性与视觉覆盖率，在大幅减少推理成本的同时保持了出色的性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像会产生超长的视觉token序列，显著增加VLM的推理成本；而现有的无需训练方法在平衡查询相关性和视觉覆盖率时缺乏灵活性，无法适应不同查询和token预算的需求。

Method: 引入StackTok，将查询相关性视为目标，视觉覆盖率视为预算校准的支持。通过构建基于大小索引的覆盖参考，利用查询-视觉亲和力熵调整支持目标，并采用参考门控交错选择策略在相关性和覆盖率导向的添加之间进行切换。

Result: 在5个VLM和10个图像理解基准测试中，StackTok在所有测试的模型和预算设置下均排名无需训练选择器首位。在高分辨率LLaVA-NeXT-7B上，仅使用2,880个视觉token中的160个（5.6%），就保留了95.26%的全token性能。

Conclusion: StackTok能够根据具体的查询需求和token预算自适应地平衡相关性与覆盖率，是一种高效且无需训练的高分辨率VLM视觉token压缩方案。
...
### [121] [FAHCD-Net: Frequency-Adaptive Heatmap-Conditional Diffusion Networks for Robust Facial Landmark Detection](https://arxiv.org/abs/2609.16842)
*Jun Wan,Jiwei Hu,Shengkai Hu,Qilu Zhu*

Main category: cs.CV

TL;DR: 本文提出了一种名为FAHCD-Net的人脸关键点检测（FLD）方法，通过结合频率自适应热图条件扩散模型和平滑正则化损失，解决了复杂条件下面部结构变化、信息丢失和噪声干扰的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的人脸关键点检测方法在面临结构变化、信息丢失和噪声干扰等挑战性条件时，往往无法保证学习到的人脸特征的完整性和准确性。

Method: 提出了一种频率自适应热图条件扩散网络（FAHCD-Net），它在级联框架中整合了频率自适应热图条件扩散（FAHCD）模型和平滑正则化（SR）损失。其中，FAHCD模型包含一个分层频率自适应（HFA）模块，通过多层频率分解和自适应重构来抑制冗余的高频噪声，同时利用SR损失增强生成的地标热图的平滑度。

Result: 在主流基准测试上的广泛实验表明，该方法在复杂场景下的FLD任务中表现出极高的有效性和鲁棒性，达到了最先进的性能。

Conclusion: FAHCD-Net通过有效利用数据的统计和基于频率的分布特征，从噪声输入中逐步生成更准确的地标热图，显著提升了复杂条件下的面部关键点检测能力。
...
### [122] [TecoPrompt: Temporal-Conservative Prompt Learning for Vision-Language Models](https://arxiv.org/abs/2609.16858)
*Zeyi Shao,Haowen Hua,Jiaxin Zhang,John See,Zeyd Boukhers,Cong Yang*

Main category: cs.CV

TL;DR: 本文提出了TecoPrompt，一个闭环且鲁棒的提示学习框架，旨在解决视觉-语言模型在少样本监督下受标签噪声干扰的问题。


<details>
  <summary>Details</summary>
Motivation: 在少样本监督下，即使是中等程度的标签噪声也会严重破坏视觉-语言模型（如CLIP）的提示优化过程。

Method: 引入基于时间视角的最佳传输（OT）伪标签生成方法，利用CLIP语义空间中的熵正则化OT计划获取全局一致的标签候选，并通过轨迹稳定性窗口和指数移动平均（EMA）置信度门控来验证候选标签的可靠性，最后通过三组损失函数的目标进行提示训练。

Result: 在包含合成对称/非对称噪声的七个数据集以及Food101N数据集上进行了实验，展现出显着的性能提升。例如在OxfordPets数据集上，在50%非对称噪声下准确率从0.775提升至0.843。

Conclusion: TecoPrompt通过结合时间维度的最佳传输伪标签和稳定性验证机制，能有效缓解提示学习中的确认偏差，显著提升模型在噪声标签下的鲁棒性。
...
### [123] [Measuring Annotation Efficiency for Handwritten Devanagari Recognition: Sample-Complexity Curves for Four Pretraining Regimes](https://arxiv.org/abs/2609.16859)
*Manglesh Kumar Pandey,Sumit Kumar Banshal*

Main category: cs.CV

TL;DR: 本研究探讨了训练手写文本识别系统所需的标注数量，以及预训练如何减少小众文字（如天城文）的手工转录成本。


<details>
  <summary>Details</summary>
Motivation: 对于只有少数专家能阅读的手写文字，人工转录十分困难。因此需要研究模型变得有用需要多少转录数据，以及预训练能节省多少成本。

Method: 通过固定识别器、优化器和评估协议，改变微调时使用的真实转录词数（从10到4000共9个预算）以及四种初始化方案，在手写天城文上直接测量不同标注量下的字符错误率（CER）曲线。

Result: 监督合成预训练在达到0.50的CER时仅需81个转录词，而随机初始化需要355个（标签倍数为4.40）；零样本预训练相当于136个转录词，但这种优势随目标准确率的提高而减小，且掩码图像建模在一定范围内表现出负迁移。

Conclusion: 预训练能显著减少小众文字手写文本识别所需的初始人工转录量，但其节省效果随着目标准确率的提高而逐渐消失。
...
### [124] [tcnerv:dual-domain temporal context modeling for implicit neural video compression](https://arxiv.org/abs/2609.16870)
*Xuezhi Xiang,Yixin Zhao,Heqi Xiang,Jiayao Liu,Shanjun Zhang*

Main category: cs.CV

TL;DR: 本文提出了TCNeRV，一种利用特征和嵌入域中重建上下文的视频隐式神经表示（INR）压缩方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视频INR通常独立解码帧，导致中间特征未基于先前重建进行条件化，且内容嵌入缺乏显式的时间预测。

Method: 提出了多尺度时空上下文融合（MTCF）模块在多个解码器尺度注入门控历史特征，以及时间嵌入残差编码（TERC）来预测内容嵌入并仅编码其残差。

Result: 在约3M参数下，TCNeRV在UVG数据集上平均PSNR达到36.08 dB，相比HNeRV-Boost提升2.20 dB，且BD-rate显著下降。

Conclusion: TCNeRV在有限的模型容量下展现出了极具竞争力的率失真性能。
...
### [125] [SPEAR NeXT Causal Latent Forecasting Across Multiple Horizons for Spectral Temporal Earth Representation Learning](https://arxiv.org/abs/2609.16871)
*Rajiv Ranjan,Udaiveer Singh,Shashank Tamaskar,Dharmendra Saraswat*

Main category: cs.CV

TL;DR: SPEAR NeXT是一个紧凑的像素级多模态时空基础模型，通过因果掩码Transformer预测多期未来潜在地球状态来实现时空自监督学习。


<details>
  <summary>Details</summary>
Motivation: 现许多地球观测基础模型中的时空信息通常通过重建、不变性或回顾性序列总结来学习，缺乏对未来地球状态的前瞻性预测能力。

Method: 利用预训练的SPEAR模型将光学、雷达和环境观测数据编码为32维嵌入，并通过因果掩码Transformer建模其时间演化，利用旋转位置嵌入、月份和年份嵌入分别表示相对时间顺序、季节相位和年际背景，从而预测多期未来潜在地球状态。

Result: 成功构建了一个紧凑的像素级多模态时空基础模型，能够基于过去及当前观测预测多个未来的潜在地球状态。

Conclusion: SPEAR NeXT通过将时空自监督建模为“仅限过去的、多地平线潜在地球状态预测”，有效地捕捉了地球观测数据的动态时空特征。
...
### [126] [RegRet: Enhancing Region-Level Retrieval in Large Multimodal Models](https://arxiv.org/abs/2609.16847)
*Xun Liang,Honghui Yang,Weihang Pan,Ruisi Zhao,Boyuan Pan,Yao Hu,Wenxiao Wang,Binbin Lin,Deng Cai*

Main category: cs.CV

TL;DR: 本文提出了RegRet，一个基于大模型的多模态区域级检索框架，并构建了新的基准REGMB，有效提升了区域级检索性能而不损失全局检索能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大型多模态模型（LMM）主要关注全局级别的检索任务，难以捕捉有效的区域级表示，且缺乏高质量的区域级对比训练数据和多样化的评估基准。

Method: 引入了RegRet框架（包含区域感知编码器以平衡区域特征与全局背景），设计了包含精细局部字幕和区域对比学习的多阶段训练程序，并构建了包含22.5万个对比对的REGMB基准。

Result: RegRet在零样本设置下超越了强基线模型，经过对比学习进一步训练后，在REGMB和公共基准测试上平均性能提升超过20%，同时在全局检索任务上保持了可比或更好的结果。

Conclusion: RegRet框架和REGMB基准有效地解决了多模态模型在区域级检索中的不足，实现了区域级与全局级检索性能的兼顾与提升。
...
### [127] [GRACE: Geometry- and Ray-Aware Camera-Efficient Multi-View Pedestrian Tracking](https://arxiv.org/abs/2609.16872)
*Taigo Sakai,Kazuhiro Hotta,Hiroki Kouno,Naoki Kato*

Main category: cs.CV

TL;DR: 该论文提出了GRACE，一个摄像机高效的多视角鸟瞰图（BEV）跟踪器，旨在解决减少摄像机数量带来的投影偏差和跟踪中断问题。


<details>
  <summary>Details</summary>
Motivation: 减少摄像机数量可以降低部署成本，但会丢失纠正BEV响应偏差（因投影拉伸）和防止跟踪分裂所需的视角，因此需要设计更高效的多视角跟踪方法。

Method: 引入了GRACE，包含三个核心组件：1）体积引导融合（结合单应性BEV特征与3D空间提升特征）；2）射线条件处理（向融合网络暴露每个摄像机的观察方向）；3）BEV轨迹恢复（BTR，仅使用低置信度检测来延续现有轨迹，而不用于启动新轨迹）。

Result: 在使用两个WildTrack摄像机的情况下，与基线模型TrackTacular（MOTA为83.54）相比，GRACE将MOTA显著提升至91.07。

Conclusion: GRACE通过创新的多视角特征融合与轨迹恢复策略，在大幅减少摄像机数量的同时，有效提高了BEV多目标跟踪的准确性和鲁棒性。
...
### [128] [NeuroTS-Net: Multi-Class Semantic Segmentation of Pediatric Brain Tumors in Multi-Modal MRI](https://arxiv.org/abs/2609.16873)
*Darius Peteleaza,Razvan-Gabriel Dumitru,Bogdan Neamtu,Arpad Gellert,Mariana Sandu,Claudiu Matei*

Main category: cs.CV

TL;DR: 本文介绍了NeuroTS-Net，一种用于儿童脑肿瘤多类语义分割的三维编码器-解码器卷积神经网络，旨在解决由于肿瘤区域小、罕见且对比度低而导致的精确人工勾勒难题。


<details>
  <summary>Details</summary>
Motivation: 儿童脑肿瘤是导致癌症相关死亡的主要原因，准确的手动分割具有挑战性，因此需要可靠的自动化分割技术来支持诊断、治疗计划和反应评估。

Method: 提出NeuroTS-Net架构，融合了双尺度原始细节流、自适应低分辨率上下文选择以及保留细节的多路径下采样，以在高效建模肿瘤广泛上下文的同时保留精细的强度和边界信息，并在BraTS 2026儿童数据集上进行训练与评估。

Result: NeuroTS-Net在内部验证集上的全肿瘤和肿瘤核心Dice分数分别达到0.938和0.937，在官方挑战赛验证集上分别达到0.927和0.926，表现优于基线方法nnU-Net和MedNeXt。

Conclusion: NeuroTS-Net能够有效处理儿童脑肿瘤分割中的复杂挑战，在不依赖外部数据或预训练权重的情况下展现出优于现有方法的分割性能。
...
### [129] [Accelerated Decoding of Centroid Positional Encoding for Instance Segmentation](https://arxiv.org/abs/2609.16874)
...
TL;DR: 本文研究了实例分割中正弦质心编码的解码阶段计算开销大问题，并提出了一种优化的CUDA实现来加速解码过程，从而降低端到端推理延迟。


<details>
  <summary>Details</summary>
Motivation: 尽管模型推理效率不断提高，但将网络输出转换为任务级表示的解码阶段往往被忽视，且通常实现低效，这限制了高级输出表示在实时计算机视觉系统中的优势。

Method: 针对基于正弦质心编码的实例分割方法，本文提出了一种定制优化的CUDA解码算法，显式解决了现代GPU上的并行化、同步和内存访问等挑战。

Result: 实验结果表明，该优化方案显著减少了解码开销，改善了端到端推理延迟，性能明显优于基于CPU的方法和朴素的GPU实现。

Conclusion: 高效的解码对于充分利用先进输出表示的优势至关重要，并强调了为实时计算机视觉系统联合设计编码方案与其解码算法的重要性。
...
### [130] [Multi-modal Knowledge Preserving Adapter for Embedding Backward Compatibility](https://arxiv.org/abs/2609.16875)
*Jaeseok Byun,Gukyeong Kwon,Han-Kai Hsu,Meher Gitika Karumuri,Zhikang Zhang,Hao Yang,Davide Modolo*

Main category: cs.CV

TL;DR: 本文提出了多模态知识保持适配器（MKP-Adapter），这是一种无需更新主干模型的多模态大语言模型（MLLMs）向后兼容训练（BCT）方法。


<details>
  <summary>Details</summary>
Motivation: 升级嵌入模型通常需要昂贵的数据库重新索引，而现有的向后兼容训练（BCT）方法通常需要更新主干模型，这存在训练成本高、性能退化风险以及无法访问专有模型权重的限制。

Method: 提出了一种仅使用适配器的BCT方法，引入了多级保持损失函数来在BCT过程中保持嵌入空间的几何结构，并整合了聚焦加权策略来优先学习困难样本。

Result: 实验表明，该方法在图像、文本、视觉文档和视频检索等多种多模态基准测试及模型类型上，均实现了强大的向后兼容性，且仅需微不足道的额外延迟。

Conclusion: MKP-Adapter 仅需预提取的嵌入进行训练，高效且无需更新主干模型，有效解决了多模态嵌入模型升级中的向后兼容问题。
...
### [131] [VOR-Bench: A Human Perception-Driven Benchmark for Video Object Removal](https://arxiv.org/abs/2609.16878)
...
TL;DR: 本文提出了VOR-Bench，一个全面且对齐人类感知的视频对象去除（VOR）评估基准，包含新数据集VORD、视频采集框架rMPAF以及基于VLM的评估模型VOR-MDSM。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象去除（VOR）评估范式存在参考基准可疑以及传统指标与人类偏好不一致的关键局限性。

Method: 提出了包含三大组件的VOR-Bench：1）提供成对编辑视频和涂鸦遮罩的VORD数据集；2）自动生成现实且运动一致的成对视频的采集框架rMPAF；3）基于VLM的感知驱动评分模型VOR-MDSM。

Result: 大量实验表明，VOR-Bench的评估结果与人类感知高度一致，主观评估的相关性达到了$ho > 0.9$。

Conclusion: VOR-Bench通过整合多样化数据集、真实视频采集框架和人类感知对齐的评分模型，有效解决了传统VOR评估中的痛点，实现了与人类判断高度一致的可靠评估。
...
### [132] [Temporally Consistent Graph Extraction and Matching for Longitudinal Angiographic Images](https://arxiv.org/abs/2609.16889)
*Linus Kreitner,Laurin Lux,Carmen Baumann,Daniel Rueckert,Martin J. Menten*

Main category: cs.CV

TL;DR: 本文提出了一种针对血管图的早期匹配与联合精炼策略，以解决血管造影图像纵向分析中因微小分割差异导致的图匹配难题。


<details>
  <summary>Details</summary>
Motivation: 当前的血管图提取、精炼和匹配策略对分割图极其敏感，微小的差异会导致显著不同的血管图，从而阻碍了同一受试者序列血管图的准确匹配。

Method: 提出了一种在基础图提取后进行早期匹配的策略，并在移除虚假膨胀和合并交界处时利用联合信息对两个图进行联合精炼。

Result: 在复杂的视网膜血管图实验中，该策略与单独精炼或不精炼相比，在没有图碎片化的前提下实现了更高的匹配面积。

Conclusion: 通过早期匹配结合联合精炼，可以有效克服血管图对分割敏感的伪影问题，显著提高纵向血管图匹配的准确性和鲁棒性。
...
### [133] [PiPS: Post-Hoc Prototypical Explanations for Interpretable Semantic Segmentation](https://arxiv.org/abs/2609.16909)
*Miłosz Adamczyk,Tymoteusz Zapala,Piotr Borycki,Przemysław Spurek*

Main category: cs.CV

TL;DR: 该论文提出了PiPS，这是首个用于语义分割模型的完全事后（post-hoc）原型可解释性方法，能够在不修改模型结构或进行微调的情况下保持原模型的预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于原型的可解释语义分割模型多为事前（ante-hoc）架构，需要从头训练并修改网络结构，导致预测性能显著下降。因此，亟需一种无需修改网络且能保持原性能的事后解释方法。

Method: 提出了PiPS（Post-hoc interpretable Prototypical Segmentation）方法，允许从任何预训练的语义分割网络中直接提取直观的、空间局部的原型解释，无需重新训练或微调。

Result: 成功实现了对任意预训练语义分割模型的事后原型解释生成，在完全保留模型原始预测性能（100%性能）的同时提供了视觉相似性解释。

Conclusion: PiPS为复杂计算机视觉任务中部署安全、透明且具成本效益的系统开辟了新途径，解决了以往方法在预测性能与可解释性之间的权衡问题。
...
### [134] [NeuroSymbEAD: A Large Scale Neuro-Symbolic Caption Dataset for Omni-Directional Embodied Autonomous Driving](https://arxiv.org/abs/2609.16919)
*Muhammad Ahmed Ullah Khan,Mohammed Elamine,Sheikh Talha Uddin,Didier Stricker,Sk Aziz Ali,Muhammad Zeshan Afzal*

Main category: cs.CV

TL;DR: 本文推出了NeuroSymbEAD，这是一个大规模的神经符号学字幕数据集，旨在通过将3D驾驶场景转化为结构化的自我中心语言，推动交通场景解释和自动驾驶感知的发展。


<details>
  <summary>Details</summary>
Motivation: 尽管自然语言基础字幕在室内场景中广泛应用，但自动驾驶领域缺乏能够有效表达自我中心知识图谱及物体复杂关系的结构化、多级文本字幕数据集与基准。

Method: 利用KITTI-360数据集，结合自我中心知识图谱（包含静态/动态物体的类别、朝向、距离等标注），开发了数据标注流水线以生成分层文本字幕，并对神经符号学和本体论字幕生成进行了基准测试。

Result: 建立了户外场景重建、视觉识别和物体接地的基准，证明了神经符号学表示在处理结构化交通场景信息方面的有效性。

Conclusion: NeuroSymbEAD为视觉-语言和基础模型提供了宝贵的基准，促进了交通场景解释、3D推理以及可解释的自动驾驶感知的发展。
...
### [135] [sensVLA: Spatially-Grounded Vision-Language-Action Model for Autonomous Wheel Loader](https://arxiv.org/abs/2609.17021)
*Gopi Krishna Erabati,Bjarne Johannsen,Angus Stewart,Vardeep Singh Sandhu*

Main category: cs.CV

TL;DR: 本文提出了sensVLA，一种用于自主轮式装载机的视觉-语言-动作（VLA）架构，通过解耦空间基础与语言推理，实现了高精度和强容错能力的重型设备自主控制。


<details>
  <summary>Details</summary>
Motivation: 自主轮式装载机控制需要联合推理任务语义、自我中心视觉、本体感觉和3D场景几何，现有方法在复杂环境下的空间定位与鲁棒性方面仍有不足。

Method: 结合Qwen3-2B视觉语言模型与基于流匹配速度回归的可训练Transformer动作专家；将融合激光雷达的BEV特征通过交叉注意力直接路由至动作专家，而VLM处理RGB图像提供任务语义上下文。

Result: 在真实世界数据集上，sensVLA在装载场景中将纵向速度RMSE降低了28%，位移误差降低了9%，且在相机流损坏或移除时性能退化减少了29%。

Conclusion: 明确的空间基础能够显著提高重型设备自主控制的准确性和容错能力。
...
### [136] [Evaluating Mesh Reconstruction Methods for Crop Phenotyping](https://arxiv.org/abs/2609.16926)
*Karanvir Singh,Theo Morales,Binh-Son Hua,Mukesh Saini*

Main category: cs.CV

TL;DR: 本文评估了近期用于农作物表型分析的7种3D网格重建流程，发现GGGS、PGSR和2DGS表现最佳，其中GGGS综合性能最优。


<details>
  <summary>Details</summary>
Motivation: 农作物表型分析对提高产量至关重要，但远程地点的作物监测困难。3D重建技术能实现作物数字化，解决专家无法亲临现场的难题，因此需要评估现有重建管道的性能。

Method: 评估了7种3D网格重建管道，从定性和定量两个方面测量其输出的保真度与一致性，使用的综合评价维度包括用户评分、Chamfer距离、LPIPS、PSNR和SSIM。

Result: GGGS、PGSR和2DGS生成的网格在定量指标和视觉效果上均优于其他管道。在5维雷达图综合评估中，GGGS比第二名(2DGS)表现高出约27%。

Conclusion: GGGS、PGSR和2DGS是适用于农作物表型分析的有效3D重建工具，其中GGGS管道展现出最佳的综合性能。
...
### [137] [MedPCFM-TED: One-Step Point Cloud Flow Matching for Implant Generation via Teacher-Guided Endpoint Distillation](https://arxiv.org/abs/2609.16934)
*Kamil Kwarciak,Marek Wodzinski*

Main category: cs.CV

TL;DR: 本论文提出了教师引导端点蒸馏（TED）框架，实现了基于点云的颅骨植入物快速一步生成，在保证重建质量的同时大幅提升了生成速度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于流匹配的点云生成方法在推理过程中需要多次神经函数评估，限制了快速生成多个合理植入物候选的能力。

Method: 提出教师引导端点蒸馏（TED）框架，利用教师引导的端点监督和几何匹配损失来训练一步学生模型，同时避免显式的路径直化。

Result: 在 SkullBreak 数据集上取得了最佳总体性能，在 SkullFix 上保持竞争力，在同类一步法中具有最强的 Chamfer 距离性能，且每个样本的生成时间约为 0.04 秒。

Conclusion: 一步蒸馏可以在不牺牲重建质量的前提下，显著加速条件点云颅骨植入物的生成。
...
### [138] [GeoLAM: Learning Geometry-Grounded Latent Actions from Unlabeled Human Videos](https://arxiv.org/abs/2609.17099)
*Yifan Xie,Hekun Tian,Jinkun Liu,YuAn Wang,Qiao Sun,Wenbo Ding*

Main category: cs.CV

TL;DR: GeoLAM是一个从无动作标签的人类视频中学习几何接地潜在动作的框架，用于提升机器人操作性能。


<details>
  <summary>Details</summary>
Motivation: 人类视频包含丰富的操作经验，但仅靠视觉重建容易将操作相关的运动与外观变化及相机运动混淆，提取有用的运动表征充满挑战。

Method: 结合通过冻结几何特征层次结构的未来帧重建，以及来自训练期4D几何教师的运动监督，利用可见度和置信度加权来捕获3D位移、残余图像平面运动和表面方向变化。

Result: 在潜在动作基准测试和机器人操作任务中表现出强劲的性能，且部署时无需几何教师或未来视频生成。

Conclusion: GeoLAM无需显式的手部姿态或轨迹标注，即可从未标注动作的人类视频中有效学习几何运动表征，并成功迁移至机器人操作中。
...
### [139] [SlotDiT: Object-Centric Representations for Diffusion Transformers](https://arxiv.org/abs/2609.17414)
*Gjergj Plepi,Sven Behnke*

Main category: cs.CV

TL;DR: 本文提出了SlotDiT，一种在基于物体中心（slot-based）的潜在空间中运行的文本引导扩散 Transformer（DiT），用于机器人视频生成和动力学建模。


<details>
  <summary>Details</summary>
Motivation: 现有的基于文本的潜在扩散模型依赖于缺乏显式语义结构的像素级或基于VAE的潜在表示，而基于slot的表示提供了结构化的替代方案，但尚未在基于扩散的生成建模中得到探索。

Method: 引入SlotDiT模型，给定参考图像和语言指令，将场景分解为代表单个实体的物体中心slot，并以自回归方式对未来的slot轨迹进行去噪以预测场景动力学，同时在统一的DiT框架内将slot表示与VAE及语义对齐的替代方案进行系统比较。

Result: 实验表明，使用slot作为DiT的潜在表示能够产生具有竞争力的视频生成质量，并在四个机器人数据集上持续提高任务完成率，同时由于其紧凑的表示而具有更高的计算效率。

Conclusion: 物体中心结构是机器人环境中基于扩散的生成建模的强大归纳偏置，slot-based潜在空间在视频生成和计算效率方面表现优异。
...
### [140] [High-Fidelity Video Quality Assessment with VQA-Specific Saliency](https://arxiv.org/abs/2609.16946)
*Hakan Emre Gedik,Shashank Gupta,Alan Bovik*

Main category: cs.CV

TL;DR: 本文提出了一种名为HFVQA的高保真视频质量评估框架，通过多尺度时空补丁采样和任务特定显著性学习，解决了基于视频基础模型的无参考视频质量评估中的高计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 无参考视频质量评估（NR VQA）需要处理大量视频数据以保留原始分辨率线索和密集的时间信息，导致计算成本极高；而现有的预处理策略会改变数据分布，限制了预训练视频基础模型（ViFMs）的复用。

Method: 提出了HFVQA框架，采用固定大小的多尺度时空（ST）补丁采样以保留低级质量线索和语义上下文，并引入轻量级辅助网络来学习由质量监督直接蒸馏的VQA特定显著性，以限制计算量。

Result: HFVQA在标准NR VQA基准测试中取得了SOTA（最优）性能，同时仅处理12%的候选时空补丁，使基于高保真ViFM的VQA在计算上可行。

Conclusion: 通过结合高保真时空线索和学习到的任务特定显著性，该方法在大幅降低计算开销的同时，保持了视频质量评估的卓越性能。
...
### [141] [Symmetry-Aware Likelihood-Orbit Aggregation for Selective Left-Right Claim Verification](https://arxiv.org/abs/2609.17004)
*Zhouzhi Xiong,Chuxi Zhang,Weizhen He,Yi Chen,Qi Li,Donglian Qi*

Main category: cs.CV

TL;DR: 本文提出了Relation-Orbit，一种无需学习参数的闭式对比方法，用于解决冻结视觉语言模型（VLM）在细粒度左右空间关系验证中的不可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 冻结的视觉语言模型在处理细粒度的左右空间关系断言时仍然不可靠，且原始的似然度无法准确对验证错误进行排序，因此需要一种更可靠的选择性验证信号组合方法。

Method: 引入Relation-Orbit方法，通过反射、逆关系和实体交换确定八个归一化似然度的查询支持与反事实角色，并结合使用Clopper-Pearson上置信界限选择阈值的闭式对比来实现选择性断言。

Result: 在VSR和GQA数据集及四个冻结VLM上的实验表明，在10%的选择性风险校准目标下，Relation-Orbit在所有八个数据集-主干设置中的平均测试覆盖率均优于Orbit-Max基线。

Conclusion: Relation-Orbit能有效将水平反射干预诱导的似然测量值组合成可靠的选择性验证信号，显著提升了视觉语言模型在细粒度空间关系验证中的性能和结构优势。
...
### [142] [Bi-FlowGS: Bridging Generative View Completion and Gaussian Geometry through Bidirectional Flow Co-Refinement](https://arxiv.org/abs/2609.17039)
*Yuetong Wang,Jinsheng Quan,Yi Yang,Yawei Luo*

Main category: cs.CV

TL;DR: 该论文提出了Bi-FlowGS，利用光流桥接生成式视点补全与3D高斯 splatting几何正则化，以解决稀疏视角下的几何欺骗问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏视点3D场景重建容易产生“几何欺骗”现象（错误的几何形状被不透明度和外观掩盖），而现有方法要么仅受限于观测视角，要么未能充分利用视频扩散模型的时空先验和显式几何监督。

Method: 提出了Bi-FlowGS框架，包含两个核心部分：一是“视频到几何光流蒸馏（V2G）”，将恢复视频的时空对应先验蒸馏到高斯几何中；二是“几何到视频光流引导恢复（G2V）”，利用当前3DGS几何引导时序一致的视频恢复，形成双向共同求精过程。

Result: 实验表明，该方法在宽基线和无边界360°基准测试中，均显着提升了渲染质量和几何一致性。

Conclusion: Bi-FlowGS通过视频恢复与3DGS几何之间的隐式双向协同求精，有效缓解了稀疏视角重建中的几何欺骗问题，实现了更高质量的3D场景重建。
...
### [143] [Beyond In-Distribution Metrics: A Systematic Out-of-Distribution Evaluation of Congenital Heart Disease Segmentation](https://arxiv.org/abs/2609.17068)
...
TL;DR: 本文对先天性心脏病（CHD）3D分割中的分布外（OOD）泛化能力进行了首次系统评估，发现传统分布内性能无法准确反映跨队列的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度学习虽然能自动化CHD解剖结构分割，但通常只在分布内进行评估，忽视了临床中常见的扫描仪、协议、机构、人群和模态等偏移问题。

Method: 使用ImageCHD作为留存的目标队列，对比了不同分割架构（如nnU-Net和SwinUNETR）在多模态联合训练、单模态训练、自监督预训练以及有限目标域适应下的表现。

Result: nnU-Net验证集Dice最高（0.77）但在ImageCHD上下降到0.51，而SwinUNETR泛化更好（0.67 Dice）；架构对鲁棒性的贡献大于所测试的自监督预训练策略；引入少量目标域标注（11个案例）可使SwinUNETR的Dice超过0.76。

Conclusion: 传统的分布内评估掩盖了临床上重要的泛化失败，显式的跨数据集测试应当成为CHD分割评估的关键组成部分。
...
### [144] [Hub-Spectral Activation of Latent Multimodal Knowledge](https://arxiv.org/abs/2609.17094)
*Ying Guo,Haidong Chen,Linrui Xu,Xiaohao Liu,Chuancheng Shi,Canran Xiao,Dan Zhang,Fei Shen,Li Shen,Tat-Seng Chua*

Main category: cs.CV

TL;DR: 本文提出了Hub-Spectral Activation (HSA)方法，一种无需微调和监督训练的闭式方法，用于恢复和激活冻结多模态表示中的潜在跨模态知识。


<details>
  <summary>Details</summary>
Motivation: 基于中心（Hub）的绑定减少了成对监督成本，但单独的中心连接无法在没有直接联合训练的情况下保证模态之间的可靠对齐。

Method: 引入Hub-Spectral Activation (HSA)，这是一种基于二阶统计量、无需梯度优化的闭式方法，通过组合和标准化中心边统计量、提取配对谱方向来恢复源诱导的跨模态依赖关系。

Result: 在ImageBind和LanguageBind的19个检索和11个原型分类任务中，HSA将平均双向Recall@10从18.27%提升至31.15%，平均宏平均Top-1准确率从29.01%提升至52.43%。

Conclusion: HSA证明了无需目标对监督、梯度优化或骨干网络更新，就能有效利用冻结多模态表示中的潜在跨模态知识，显著提升跨模态检索和分类性能。
...
### [145] [Not Another Text Benchmark: Putting the "Visual" Back in Visual Question Answering for Large Video Models](https://arxiv.org/abs/2609.17112)
...
TL;DR: 该论文研究了大视频模型在视觉查询（而非文本查询）评估模式下的表现，并引入了三个新的以视觉为中心的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有的主流大视频模型基准测试主要通过文本选项来评估多项选择推理，忽略了评估模态为视觉时模型表现如何的问题。

Method: 引入了三个新的以视觉为中心的评估基准：时间帧检索、视频未来预测和因果记忆失真，用于评估大视频模型的视觉理解能力。

Result: 研究表明，当前的前沿模型在尝试通过视觉查询（而不是文本）进行推理时，表现出明显的弱点。

Conclusion: 通过引入视觉中心基准并指出当前模型的不足，论文为未来提升大视频模型的视觉理解能力提供了改进方向。
...
### [146] [Predicting Human Disagreement for Calibrated Dynamic Facial Expression Recognition](https://arxiv.org/abs/2609.17130)
*Yiming Wang,Frederick W. B. Li,Jingyun Wang*

Main category: cs.CV

TL;DR: 本文提出了一种考虑人类标注不一致性的动态面部表情识别（DFER）框架，通过Dirichlet-Multinomial似然函数和选择性预测机制，提升了模型的不确定性估计和校准能力。


<details>
  <summary>Details</summary>
Motivation: 现有的DFER基准测试虽然提供多个标注者的投票，但大多数模型将其折叠为多数标签，无法在推理时表示人类的不一致性。

Method: 使用Dirichlet-Multinomial似然函数直接对原始标注者计数向量进行训练，利用独立的模糊度头预测测试片段的标注熵，并结合单调Chow式拒绝规则进行选择性预测。

Result: 在保持识别准确率的同时，将期望校准误差（ECE）降低了30%，面积下的风险-覆盖曲线（AURC）降低了15%，预测模糊度与测试片段标注熵的Spearman相关性达到0.52。

Conclusion: 该方法有效地建模了人类标注的不一致性，显著改善了模型的校准和选择性预测性能，且具备良好的泛化能力。
...
### [147] [Event-based Selective Attention for Multi-resolution Fast Region of Interest (ROI) Detection](https://arxiv.org/abs/2609.17134)
*Luca Peres,Giulia D'Angelo,Chiara Bartolozzi,Oliver Rhodes*

Main category: cs.CV

TL;DR: 本文提出了一种多尺度、免训练、基于显著性的自底向上视觉注意力模型，用于低分辨率事件相机的边缘计算，实现高效的兴趣区（ROI）选择。


<details>
  <summary>Details</summary>
Motivation: 神经形态视觉系统在带宽、内存和能量方面受到严格限制，尤其是在边缘端，这促使我们需要早期的数据减少和选择性处理机制。

Method: 研究了一种多尺度、免训练、基于显著性的自底向上视觉注意力模型，直接对低分辨率的基于事件的输入进行操作，并评估了高达256倍的多种降采样因子。

Result: 在Prophesee汽车数据集上评估显示，该方法能够以高达70.8%的准确率检测多个对象类别的ROI，且时间分辨率比数据集真值精细16倍。

Conclusion: 早期事件降采样与基于显著性的注意力的结合，是实现高效边缘神经形态视觉系统的一种有效前端。
...
TL;DR: 本文针对视觉Transformer（ViTs）中可解释性方法LRP由于残差连接的相消效应导致归因不稳定和失真的问题，提出了残差感知的LRP方法（ResLRP），显著提升了ViT及多模态模型的可解释性质量。


<details>
  <summary>Details</summary>
Motivation: 现有的ViT输入归因方法（如LRP）往往产生嘈杂且不忠实的解释，缺乏对残差连接中相消效应的正确处理是导致归因爆炸和不稳定的主要原因。

Method: 引入了残差感知层级关联传播（ResLRP），这是一种LRP的简单扩展，其传播规则明确考虑了残差分支中的相消效应，具备精确守恒性并可证明地约束了关联爆炸。

Result: ResLRP在忠实度和定位能力上大幅提升了归因质量，涵盖各种监督、自监督、层次化和多模态ViT架构及FunnyBirds基准，其中在现代视觉语言模型（VLMs）中定位能力提升了27-29%，忠实度分数提升高达3.4倍。

Conclusion: ResLRP有效解决了ViT中因残差相消导致的归因失效问题，为视觉模型及多模态模型提供了一种高质、稳定且忠实的可解释性工具。
...
### [149] [MUMINS: Metadata-conditioned Uncertainty-aware Medical Image Next-state Synthesis](https://arxiv.org/abs/2609.17169)
...
TL;DR: 本文提出了MUMINS，一个高效的条件扩散框架，用于医学图像的下一个状态合成，能够通过单次反向扩散过程同时预测未来扫描图像和空间不确定性图。


<details>
  <summary>Details</summary>
Motivation: 现有的解剖学变化预测方法存在局限性：确定性网络忽略了生物学随机性，而标准扩散模型需要计算成本高昂的多遍采样来量化不确定性。

Method: 提出MUMINS框架，联合扩散基准扫描和随访残差以合成随访扫描，并在单次反向扩散过程中并发预测空间不确定性图。通过在每个去噪步骤中动态重新注入基准图像作为软锚点来保持精细解剖结构，并使用负对数似然头学习不确定性图。

Result: 在肺部CT（PNG）和大脑MRI（OASIS-3）数据集上的广泛评估表明，MUMINS的效果达到或超过了针对特定领域的现有最先进方法。

Conclusion: MUMINS是一种高效、不确定性感知的医学图像下一步状态合成方法，无需器官特定启发式算法，即可跨不同解剖结构实现优秀性能。
...
### [150] [Multimodal Cultural Heritage Architectural Style Classification for Residential Buildings in the UAE Based on CLIP Embeddings and SVM](https://arxiv.org/abs/2609.17181)
*Ahmed Ammar Kubba,Manar Abu Talib,Iman Ibrahim,Qassim Nasir*

Main category: cs.CV

TL;DR: 本文提出了一种基于多模态机器学习（结合OpenAI的CLIP模型）的框架，用于阿联酋住宅建筑风格的分析与分类。


<details>
  <summary>Details</summary>
Motivation: 传统的基于CNN的建筑风格分类方法高度依赖视觉图像，且缺乏非西方地区的特定数据集，建筑图像的复杂性也带来了挑战。

Method: 集成图像的视觉特征和专家描述的文本特征为统一的512维嵌入，利用UMAP降维进行可视化，通过K-Means进行无监督聚类，并使用聚类标签训练SVM分类器实现自动化分类。

Result: 在八个识别出的风格聚类上实现了98%的分类准确率，优于文献中的其他研究，证明了视觉与文本模态结合的有效性。

Conclusion: 多模态人工智能在支持建筑遗产分析方面具有巨大潜力，为探索区域建筑特性提供了可扩展且可解释的工具。
...
### [151] [EventEgoHands++: Event-based Egocentric 3D Hand Mesh Reconstruction with Real Dataset](https://arxiv.org/abs/2609.17189)
*Ryosei Hara,Wataru Ikeda,Masashi Hatano,Mariko Isogawa*

Main category: cs.CV

TL;DR: 本文提出了EventEgoHands++框架，利用事件相机在第一人称视角下进行稳健的3D手部网格重建，解决了现有方法缺乏实例级手部信息的问题。


<details>
  <summary>Details</summary>
Motivation: 传统相机在低光和运动模糊下表现不佳，而现有的基于事件相机的第一人称手部重建方法无法区分左右手，导致实例级信息缺失和重建精度下降。

Method: 提出了EventEgoHands++框架，包含一个手部检测器用于估计左右手的实例级边界框和掩码，并引入了自适应注意力机制（Adaptive Attention）根据检测结果动态门控注意力，以学习双手之间的空间关系和相互作用。同时构建了大规模真实世界数据集EEH-R。

Result: 在合成数据集和包含低光照条件的自建真实数据集EEH-R上的大量实验表明，该方法在性能上 consistently 优于现有基线。

Conclusion: EventEgoHands++通过引入实例级手部检测和自适应注意力机制，有效解决了第一人称视角下事件相机进行3D手部重建时的左右手混淆问题，显著提高了重建精度。
...
### [152] [InfoTaxa: Information-Calibrated Label-Free Clustering for Fine-Grained Visual Taxonomy](https://arxiv.org/abs/2609.17218)
*David Ahmedt-Aristizabal,Mohammad Ali Armin,Lars Petersson*

Main category: cs.CV

TL;DR: 本研究探讨了基于冷冻预训练视觉嵌入的无监督生物多样性聚类在物种层面的“粗到细”失效模式，并结合DNA数据进行了信息校准分析。


<details>
  <summary>Details</summary>
Motivation: 图像仅有的细粒度分类学聚类在处理到物种级别时会出现性能瓶颈（平台期），研究者希望诊断这一现象是由于聚类方法受限还是图像表征本身信息不足。

Method: 利用BioCLIP 2特征、UMAP和HDBSCAN进行无监督聚类，并引入InfoTaxa指标（结合配对DNA作为仅用于审计的信号），通过信息校准聚类分析来评估图像嵌入中包含的分类信息量。

Result: 密度聚类管道在目和科级别恢复了图像可用信息的90%和81%，但物种级别的预测表明，当前的无监督聚类既受聚类方法限制，也受视觉表征信息量的限制。

Conclusion: 仅靠改进聚类算法无法完全弥补由DNA审计所揭示的图像表征与真实物种分类之间的信息差距，物种级无监督聚类同时面临聚类能力和特征表示的双重瓶颈。
...
### [153] [Probe-VAD: Ordinal Likelihood Probing for Training-Free Video Anomaly Detection](https://arxiv.org/abs/2609.17211)
*Jiawei Gu,Qilin Zhao,Tengkuo Guo,Zhiming Zhong,Shuangqing Zhang,Fan Lyu,Fang Zhao,Guo-Sen Xie,Caifeng Shan*

Main category: cs.CV

TL;DR: 本文提出了Probe-VAD，一种基于视觉-语言模型（VLM）的无训练视频异常检测框架，通过序数二元探测机制生成连续且对排名敏感的异常分数。


<details>
  <summary>Details</summary>
Motivation: 现有的基于VLM的无训练视频异常检测方法在视觉理解和异常评分之间采用了受限的交互接口（如基于字幕的压缩或直接数值生成），这会丢失微妙的线索并限制异常排名的分辨率。

Method: 提出了Probe-VAD框架，直接从冻结的VLM中探测严重程度偏好。通过查询十个有序的严重程度阈值提取受限的YES/NO延续似然性，将其归一化为累积严重程度分布，并利用保序投影强制保持序数一致性，最终聚合为连续异常分数。

Result: 在公开的视频异常检测基准测试中，该方法以较低的计算成本展现出了优越的性能。

Conclusion: Probe-VAD提供了一种简单的接口，能够在无需特定任务训练或字幕压缩的情况下，将冻结VLM的视觉理解转化为连续且对排名敏感的异常分数。
...
### [154] [DecoGS: Adaptive Static-Dynamic Decoupling of 3D Gaussians for Free-Viewpoint Video Streaming](https://arxiv.org/abs/2609.17230)
*Idil Sulo,Alexey Supikov,Ilke Demir,Sainan Liu*

Main category: cs.CV

TL;DR: DecoGS是一个用于流式视频的高效3D高斯在线训练方法，通过自适应机制选择性地优化动态区域，从而实现高保真、低闪烁的实时3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有的流式3D重建方法在每一帧都更新所有的高斯点，导致静态区域出现闪烁和漂移，无法同时满足速度和时间保真度的要求。

Method: 提出DecoGS方法，引入自适应机制仅针对表现出运动或光度变化的动态时空区域进行优化，并结合基于梯度门控和高效可见性过滤的区域感知高斯管理策略。

Result: 在N3DV和MeetRoom数据集上，PSNR分别达到34.55和31.60 dB，超越了所有流式和离线基线，渲染速度达261 FPS，且时间闪烁比最佳现有方法降低了70倍。

Conclusion: DecoGS通过有针对性的训练策略和区域感知的高斯管理，有效消除了静态区域的冗余更新，在不需要大规模预训练的情况下实现了高效、高保真的流式3D重建。
...
### [155] [FROD: Feature Matching Residual Denoising Oracle Bone Decipher](https://arxiv.org/abs/2609.17227)
*Yanbin Hou,Biao Xiong,Guojun Xu,Jianwen Xiang,Cheng Tan,Yanchao Yang,Junwei Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种名为FROD的甲骨文释读辅助方法，将其建模为跨时代图像翻译任务，有效解决了传统释读耗时费力的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的甲骨文释读极度依赖领域专家进行语义上下文和结构演变的分析，过程繁琐且劳动力成本高，需要自动化技术来提供释读辅助。

Method: 提出FROD框架，包含：1）利用快速特征匹配提供门控分割监督，兼顾局部部首对齐与全局训练；2）残差去噪扩散模型（RDDM），联合估计噪声和残差信号以减少位置漂移和笔画紊乱；3）多阶段字体风格化细化网络，消除边缘噪声并稳定笔画结构。

Result: 在增强的字符不相交数据集上，FROD取得了比评估基线更高的Top-1识别准确率，相比BSD绝对提升了3.8%。

Conclusion: FROD通过特征匹配、残差去噪扩散模型和风格化细化网络，能够有效地将甲骨文翻译为现代汉字，显著提升了甲骨文识别和释读辅助的准确性。
...
### [156] [Tracking the Unseen: An Occlusion-Robust Framework for Target Tracking Under Full and Long-Term Occlusion](https://arxiv.org/abs/2609.17427)
*Mais Mohammed,Sharifa Mohammed,Hanan Awadh,Haneen Bamaas,Raghad Bawazeer,Elham Alghamdi*

Main category: cs.CV

TL;DR: 本研究提出了一种抗遮挡的多目标跟踪框架，通过结合YOLOv11n、卡尔曼滤波和遮挡感知的重识别技术，解决了长时间遮挡导致的身份丢失和轨迹中断问题。


<details>
  <summary>Details</summary>
Motivation: 传统实时多目标跟踪系统在面对完全且长时间的遮挡时极易失效，导致轨迹提前终止和身份丢失，无法满足国防和监控等应用对态势感知的需求。

Method: 结合了YOLOv11n目标检测、卡尔曼滤波运动预测以及遮挡感知的外观重识别（Re-ID）技术，并在相同条件下评估了六种Re-ID架构，最终选用遮挡感知掩膜网络（OAMN）构建三阶段跟踪框架。

Result: 在OVIS数据集上，MOTA和IDF1分别相对提升了18.1%和25.1%，身份切换减少了12.8%；在模拟战场环境的自定义军事数据集上，MOTA达到0.734，IDF1达到0.729。

Conclusion: 该框架在严苛的遮挡条件下表现出强大的跟踪连续性、稳健的身份保持能力和可靠的轨迹估计，非常适合需要持续跟踪的国防监控应用。
...
### [157] [Video-HolmesV2: Can MLLMs Reason with Spatio-Temporal Audio-Visual Evidence in Long Videos?](https://arxiv.org/abs/2609.17248)
...
TL;DR: 本文指出了现有长视频多模态大模型在视听推理上的缺陷，并提出了新的基准Video-HolmesV2与音频-文本引导的标记压缩框架。


<details>
  <summary>Details</summary>
Motivation: 现有视频基准过度依赖视觉线索而忽视听觉线索，且传统的密集采样导致注意力分散和标记爆炸，无法有效评估真正的长叙事跨模态推理能力。

Method: 提出了Video-HolmesV2基准（强制基于证据的评估、多模型交叉验证管道及时空证据感知指标），并开发了一种音频-文本引导的标记压缩框架来消除长上下文噪声。

Result: 实验表明，即使是强大闭源模型的准确率也低于60%，而所提出的方法表现优于同类开源全能模型。

Conclusion: Video-HolmesV2和音频-文本引导压缩框架能有效推动深度视听耦合及长视频的多模态推理研究。
...
### [158] [Exploring 2D backbone effects for indoor semantic occupancy prediction](https://arxiv.org/abs/2609.17257)
*Shizhang Fanga,Wanling Yea,Qi Zheng*

Main category: cs.CV

TL;DR: 该论文研究了具身语义占用预测中2D图像骨干网络对3D预测性能的影响，发现更换更强的2D编码器比复杂的3D架构设计更能显著提升占用预测精度。


<details>
  <summary>Details</summary>
Motivation: 在RGB-D具身扫描流程中，图像编码器通常被视为默认模块，而作者旨在探讨2D图像骨干网的选择对最终3D网格视觉证据采样及语义占用预测准确率的决定性影响。

Method: 保持主干RGB-D投影、深度分支和占用预测头固定，仅替换2D图像编码器（对比了CLIP-ResNet、CLIP-ViT、BLIP2和DINOv2），并在控制变量下评估其语义占用预测性能（mIoU）。

Result: 实验表明不同编码器的mIoU差异巨大：DINOv2达到30.55%，BLIP2为29.49%，CLIP-ViT为24.33%，CLIP-ResNet仅为17.41%。其中DINOv2在结构布局类上表现更优，而BLIP2在物体中心类上表现接近。

Conclusion: 图像骨干网络并非具身语义占用预测中的次要工程细节，而是导致最终3D预测性能变化的主要来源。
...
### [159] [Det-LIME: Detector-Aware, Multi-Instance Local Interpretable Model-Agnostic Explanations for Automated Marine Mammal Detection](https://arxiv.org/abs/2609.17479)
*Jiayi Zhou,David W. Johnston,Brinnae Bent*

Main category: cs.CV

TL;DR: 本文提出了Det-LIME，一种针对海洋哺乳动物和海鸟等群居生物检测任务的黑箱目标检测器可解释性方法，能够生成高分辨率、实例特异且边界框对齐的解释。


<details>
  <summary>Details</summary>
Motivation: 现有的可解释性工具大多面向分类任务，不适用于包含多个目标的场景，且生成的视觉效果分辨率低、缺乏生物学相关性，难以支持海洋生物保护工作中的模型调试和数据优化。

Method: 提出了Det-LIME（一种感知检测器且支持多实例的LIME改进方法），结合了单检测加权、聚焦于边界框邻近区域的邻近核，以及基于交并比（IoU）的匹配机制来跟踪扰动中的同一实例。

Result: 通过归因比率（Attribution Ratio）和最大显著性命中率（Max Saliency Hit Rate）指标评估，Det-LIME在多实例归因方面显著优于传统LIME及梯度基归因方法，能提供更高分辨率、实例感知的解释。

Conclusion: Det-LIME有效解决了现有可解释性工具在多目标群居生物检测中的局限性，能够深入揭示模型输出，并支持模型调试、后处理以及有针对性的数据收集与增强。
...
### [160] [Optical-Flow Wingbeat Counting in MuJoCo: A Comparison of Convolutional, Spiking, and Attention-Based Temporal Models](https://arxiv.org/abs/2609.17308)
*Zhang Nengbo*

Main category: cs.CV

TL;DR: 本文提出了一种基于MuJoCo仿真环境的方法，利用搭载在微型飞行器上的虚拟相机通过符号光流来计数扑翼飞行器的扇翅次数。


<details>
  <summary>Details</summary>
Motivation: 扑翼飞行器的视觉监测需要从运动强度和平均频率中区分出单独的扇翅动作，为此需要评估在控制场景下基于光流进行扇翅计数的可能性和不同架构的性能。

Method: 结合空间卷积编码器与因果时间卷积网络、循环漏电积分发放脉冲网络或因果自注意力机制，预测相位和活动状态，并使用有向交叉事件计数器进行计数。

Result: 在1.5米距离下，三种模型的精确计数准确率分别为96.67%、95.00%和96.67%；在3.0米距离下分别为94.44%、92.22%和95.00%，且模型间差异不显著。

Conclusion: 研究证实了在测试设置下使用因果光流进行计数的的可行性，指出了边界敏感的错误，并强调了同时报告总计数和事件级测量的重要性。
...
### [161] [PanoGS-SLAM: Panoramic 3D Gaussian Splatting SLAM](https://arxiv.org/abs/2609.17387)
*Yongqi Mao,Hao Shi,Yufan Zhang,Zhonghua Yi,Xiangfei Guo,Kaiwei Wang*

Main category: cs.CV

TL;DR: PanoGS-SLAM是一个基于3D高斯散射的首个全景稠密SLAM系统，利用球面域的可微分渲染和位姿优化实现全方位光度约束，提高了动态和快速变化环境下的追踪稳定性和建图质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于3D高斯散射的SLAM方法多针对窄视场针孔相机，在快速运动和大视角变化下存在姿态可观测性弱、光度优化不稳定等问题。

Method: 直接在球面域进行可微分渲染和位姿优化，引入补偿等距投影面积畸变的球面一致光度损失，以及用于稳定增量建图的深度引导高斯初始化策略。

Result: 在真实和合成全景基准测试（PALVIO和SynPano）上的实验表明，PanoGS-SLAM在追踪精度和渲染质量上均优于现有的几何和基于GS的基线，同时实现了快速的前端收敛和实时性能。

Conclusion: 全景相机的广角覆盖能够显著改善可微分高斯SLAM中优化条件和收敛稳定性，证明了传感几何形状在优化过程中的根本作用。
...
### [162] [Semantic-Spatial Agreement Verification for Mitigating Object Hallucination in Multimodal Large Language Models](https://arxiv.org/abs/2609.17269)
*Ziheng Ren,Qian Gao,Jun Fan,Guohui Ding,Zhenyu Yang,Yuteng Xiao*

Main category: cs.CV

TL;DR: 本文提出了一种名为语义-空间一致性验证（SSAV）的免训练方法，旨在减少多模态大语言模型中的物体幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉输入生成文本时可能会提及图像中不存在的物体，这在医疗辅助和环境决策等领域会引发真实的现实安全风险。

Method: 提出语义-空间一致性验证（SSAV），通过聚合多个提示词来评估语义支持以减少对查询措辞的敏感性，并结合查询诱导区域验证（QIRV）来检查跨查询的区域持久性、空间重叠和相对候选优势，最后用几何平均值融合语义和空间证据。

Result: 在三个基础模型和多个评估协议上的实验表明，SSAV能有效缓解物体幻觉。在LLaVA-1.5-7B上，COCO、A-OKVQA和GQA的平均准确率在POPE Popular和Adversarial下分别提高了1.81和3.17个百分点，同时CHAIRs从49.40%降至32.80%。

Conclusion: 跨查询的语义稳定性和区域一致性为物体声明提供了可解释的外部视觉证据，能有效缓解多模态大语言模型的物体幻觉。
...
### [163] [PhysStream: Streaming Physics-Grounded Video Generation with Structured Scene Memory and Fine-Grained Motion Control](https://arxiv.org/abs/2609.17521)
*Chuhao Chen,Peter Wonka,Chaoyang Wang,Chen Wang,Qiao Feng,Sergey Tulyakov,Lingjie Liu*

Main category: cs.CV

TL;DR: PhysStream是一个支持物理接地和交互式控制的图生视频自回归模型，能够通过稀疏速度增量信号实现细粒度的运动控制。


<details>
  <summary>Details</summary>
Motivation: 现有可控视频生成方法通常需要预先提供完整的控制计划，或使用仅限制物体位置而非物理动态的像素空间信号，无法进行生成过程中的交互式物理动态控制。

Method: 提出基于自回归的PhysStream模型，包含从先前帧在线衍生的结构化场景内存（位置图和物体跟踪图），并支持通过编码物理量的稀疏速度增量信号进行微观运动控制。分两阶段训练：先微调双向模型以适应运动控制条件，再训练因果自回归模型并结合结构化场景内存以增强物理一致性。

Result: 在多物体桌面刚体场景中实现了交互式生成中控制，相比最强基线，运动分布距离（FVMD）降低了33%，轨迹误差降低了12%，在真实场景对比中受到85%以上人类评估者的青睐。

Conclusion: PhysStream通过引入结构化场景内存和基于物理的速度增量控制信号，有效解决了现有视频生成方法缺乏交互式、物理真实动态控制的问题，显著提升了视频生成的物理一致性和交互性能。
...
### [164] [SSC-Priors: Exploring Semantic and Visibility Priors to Boost Lidar Semantic Scene Completion](https://arxiv.org/abs/2609.17413)
*Tetiana Martyniuk,Jonathan Seele,Alexandre Boulch,Gilles Puy,Renaud Marlet,Raoul de Charette*

Main category: cs.CV

TL;DR: 本文研究了通过引入语义伪标签和传感器可见性信息两种简单的数据先验，无需复杂架构重新设计，即可显著提升现有激光雷达语义场景补全(SSC)网络性能的策略。


<details>
  <summary>Details</summary>
Motivation: 近年来SSC方法过度追求复杂的架构创新（如联合训练点云语义分割分支），导致模型笨重。本文旨在退一步，探索通过简单的输入级先验来提升现有模型的性能。

Method: 将来自现成分割器的语义伪标签和区分空闲空间与未知空间的传感器可见性信息作为附加输入提供给现有的SSC网络，仅对原始架构进行极小的调整。

Result: 在SemanticKITTI和SSCBench-nuScenes基准测试中，这两种增强使旧模型在四种架构上均能与甚至超越最先进的系统相媲美。

Conclusion: 高质的语义先验和可见性信息是提升SSC性能的关键驱动力，通过简单的数据输入修改即可让旧模型焕发新生，而无需复杂的架构重构。
...
### [165] [BrainFocus: EEG-Guided ROI Selection for Efficient Vision-Language Models](https://arxiv.org/abs/2609.17443)
*Yihui Peng,Guorui Lu,Qinyu Chen*

Main category: cs.CV

TL;DR: BrainFocus是一个利用脑电图（EEG）信号引导视觉语言模型（VLM）进行高效视觉问答（VQA）的框架，通过结合EEG分类器与YOLO检测器定位感兴趣区域，从而在减少计算开销的同时提升准确率。


<details>
  <summary>Details</summary>
Motivation: 处理复杂的视觉图像对VLM而言计算成本高昂，而人类的EEG信号能够提供有关感兴趣区域（ROI）的语义线索，但直接依赖不完美的EEG解码进行区域路由是不可靠的，因此需要一个既能利用脑电信号又具有容错机制的高效VLM框架。

Method: 提出BrainFocus框架：首先通过EEG分类器预测目标类别，然后利用YOLO检测器定位匹配的ROI；只有当两个预测都通过置信度阈值时，VLM才仅处理裁剪后的ROI，否则处理全图。

Result: 在包含40个类别的自定义基准测试中，BrainFocus在多个Qwen-VL模型上将复杂场景下的VQA准确率提升了4.14-9.87个百分点，同时减少了23.2%-39.4%的输入/总Token以及23.2%-39.5%的FLOPs。

Conclusion: 即使EEG的语义解码存在不完美，它依然可以有效地引导VLM进行高效推理，在降低计算成本的同时显著提升VQA性能。
...
### [166] [ORCA: Occlusion-Aware Refinement and Completion for Novel View Synthesis](https://arxiv.org/abs/2609.17450)
*Weronika Jakubowska,Maciej Zięba,Przemysław Spurek*

Main category: cs.CV

TL;DR: 本论文提出了ORCA，一种基于单张图像的遮挡感知3D场景重建与补全方法，通过优先复用已有场景信息减少对生成式修复的依赖。


<details>
  <summary>Details</summary>
Motivation: 单图新视角合成中的遮挡区域通常包含许多深度边界附近的小间隙，不需要生成新的场景内容，而现有的生成模型往往会导致昂贵的开销和不必要的幻觉。

Method: 利用单目深度将3D结构引入高斯锚点表示，并保留原始相机射线对应关系；根据缺失区域的大小和结构进行处理，小遮挡通过现有的RGB-D信息修复，大区域才使用生成式修复，同时局部优化和添加新的高斯锚点而不修改现有表示。

Result: 在DIV2K数据集上，ORCA在所有报告的指标上均优于VistaDream，将MUSIQ从61.60提高到68.71，CLIP-IQA从0.474提高到0.574。

Conclusion: 通过复用重建场景中已有的信息，可以有效修复许多新视角伪影，从而限制生成引起的幻觉并更好地保持原场景的内容和结构。
...
### [167] [Tables Decoded: DELTA for Structure, TARQA for Understanding](https://arxiv.org/abs/2609.17458)
*Jahanvi Rajput,Dhruv Kudale,Saikiran Kasturi,Utkarsh Verma,Ganesh Ramakrishnan*

Main category: cs.CV

TL;DR: 本文提出了DELTA和TARQA，利用结构化文本表示代替视觉语言模型进行表格理解，在表格重建和表格视觉问答任务上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖基于表格图像的视觉语言模型（VLMs），而结构化文本表示更容易处理、更贴合大语言模型（LLMs），且无需特定语言的视觉编码器，因此更适合多语言文档。

Method: 提出了DELTA方法，分离物理结构识别、逻辑结构识别和OCR，以优化的表格结构语言（OTSL）输出表格；并引入了在OTSL序列上微调的大语言模型TARQA用于表格问答。

Result: DELTA在多个数据集上的表格结构识别（TSR）达到了与SOTA相当的TEDS-Structure分数，并在WTQ和FinTabNetQA上分别取得了9.3个百分点和9.2个百分点的性能提升。

Conclusion: 基于结构化文本表示和LLM的方法在表格理解任务中具有极高的可扩展性、有效性以及跨语言鲁棒性。
...
### [168] [Few-Shot Degradation Is Not What It Seems: Behavioral Evidence, Representation Analysis, and a Random-Text Control Across 12 Models, 2 Tasks, and 2 Architectures](https://arxiv.org/abs/2609.15990)
*Volodymyr Ovcharov*

Main category: cs.CL

TL;DR: 该研究探讨了少样本提示词有时会导致语言模型性能下降的原因，提出了解耦提示词长度与内容影响的新度量方法，并发现模型对演示内容的表征重构程度与其少样本表现正相关。


<details>
  <summary>Details</summary>
Motivation: 少样本提示词有时会降低语言模型性能，但具体原因未知；先前研究测量隐藏状态偏移时忽略了提示词长度带来的混淆影响。

Method: 评估了12个开源模型在两个乌克兰语任务上的表现，提出用长度匹配的随机文本替换演示来测量长度引起的偏移并将其减去，从而得到隔离了内容影响的‘内容增量（content delta）’指标。

Result: 原始隐藏状态偏移无法预测少样本表现，而‘内容增量’与模型性能提升显著正相关（rho = +0.65）；对Llama 3.3 70B的掩码实验进一步证实了这一因果关系。

Conclusion: 模型通过对演示内容的表征重构来获取少样本收益，内容的影响是决定少样本提示词成败的关键，而非简单的长度造成的表征扭曲。
...
### [169] [The Functionalizer: Lossless Functional Decomposition for Subword Tokenization](https://arxiv.org/abs/2609.15991)
*Connor Makowski,Willem Guter*

Main category: cs.CL

TL;DR: 本文提出了Functionalizer，一种无损预分词框架，通过将正字法和结构变体分解为可逆的操作码/操作数前缀流，实现了词表的高效利用和对代码结构更好的建模。


<details>
  <summary>Details</summary>
Motivation: 标准子词分词器要么将单词的不同正字法变体（如大小写、变音符号）视为不相关条目导致嵌入空间碎片化，要么通过有损归一化丢弃这些变体，本研究旨在克服这一局限。

Method: 引入Functionalizer框架，在分词前将变体转化为由参数化转换操作符（操作码）和规范基础词（操作数）组成的前缀流，操作码编码在Unicode私有使用区中，且完全可逆。

Result: 在六个自然语言和代码语料库上，Functionalizer在无约束条件下实现了完全的语料覆盖，减少了高达16%的词表槽需求；在GPT-2规模模型上的评估表明，它显著提升了代码语法有效性和字符困惑度，同时保持了自然语言的文本连贯性。

Conclusion: 函数式分解是实现词表高效且结构感知语言建模的有效机制，值得在生产规模上进一步验证。
...
### [170] [Optimal Model Activation Policies for Inference Networks of Large Language Models](https://arxiv.org/abs/2609.15992)
*Foivos Charalampakos,Md Ibrahim Ibne Alam,Iordanis Koutsopoulos,Koushik Kar*

Main category: cs.CL

TL;DR: 本文提出了一种基于图的模型架构（推理网络），用于在保证性能的前提下优化多个大语言模型（LLM）的推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型推理成本高昂，虽然常通过组合多个不同能力和成本的专家模型来优化，但缺乏一种有原则的方法来权衡成本与性能。

Method: 引入基于图的“推理网络”框架，节点代表不同LLM，边代表条件模型激活；针对串联模型拓扑，构建了一个在目标性能约束下最小化期望推理成本的最优化问题，并推导出基于模型置信度的阈值激活策略。

Result: 证明了最优激活策略具有阈值结构（判别任务为多阈值，生成任务为单阈值），并提供了计算阈值的方法及置信度估计机制，实验证明能在满足性能预算的同时大幅降低成本。

Conclusion: 该图基推理网络框架和阈值激活策略能够有效解决LLM协同推理中的成本-性能权衡问题，实现经济高效的查询路由。
...
### [171] [Single Document Extractive Summarization using Domination in Hypergraph](https://arxiv.org/abs/2609.15993)
*Aamir Miyajiwala,Aabha Pingle,Sheetal Sonawane,Surajit Kr. Nath*

Main category: cs.CL

TL;DR: 本研究探讨了利用超图（Hypergraph）的控制集（domination）性质来进行单文档抽取式文本摘要的方法。


<details>
  <summary>Details</summary>
Motivation: 改善传统的基于图的文本摘要方法，利用超图更好地捕捉句子与关键词/命名实体之间复杂的、高阶的关系。

Method: 构建一个句子超图，其中节点代表句子，超边代表包含这些句子的关键词或命名实体，然后应用贪婪算法寻找超图的支配集来形成抽取式摘要。

Result: 提出了一种基于超图支配集的新型抽取式摘要生成方法，并通过贪婪算法提取出能够概括文档核心信息的句子集合。

Conclusion: 超图的控制集性质可以有效应用于单文档抽取式文本摘要，为该领域提供了一种有潜力的图论建模新视角。
...
### [172] [Latent Undertow: How Ordinary Typos Break Probes](https://arxiv.org/abs/2609.15994)
*Elad David,Max Fomin,Amit LeVi*

Main category: cs.CL

TL;DR: 研究发现拼写错误等日常文本变异会显著干扰大语言模型（LLM）基于隐藏状态的恶意提示词检测探针，而作者提出的KV-cache分叉方法能够有效解决这一鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM能流畅处理日常的拼写变化，但基于隐藏状态的恶意提示词检测探针却对微小的扰动（如拼写错误、标点符号缺失）极其敏感，导致检测性能严重下降。

Method: 分析了扰动下隐藏状态的几何变化（旋转和衰减），并引入了一种KV-cache分叉方法——在用户消息后附加一个简短的固定后缀，使探针能够读取扰动下游几个位置的Token，利用扰动的空间快速衰减特性。

Result: 多位置聚合只能缓解局部扰动，而KV-cache分叉方法弥补了95%的性能差距（残余仅-0.6pp），显著优于扰动增强训练（-3.7pp），且该几何特性在多种主流LLM中均可复现。

Conclusion: 通过利用隐藏状态扰动的快速空间衰减特性并采用KV-cache分叉机制，可以极大提升恶意提示词检测探针对抗文本扰动的鲁棒性。
...
### [173] [Bias Audits Detect Bias but Disagree on Ranking: Evidence from Ten Instruments and Ten Frontier Models](https://arxiv.org/abs/2609.15995)
*William Guey,Pierrick Bougault,Wei Zhang,Vitor D. de Moura,José O. Gomes*

Main category: cs.CL

TL;DR: 本研究测试了不同AI偏见审计工具是否衡量同一事物并能用于模型排名，发现审计工具可以检测出偏见，但它们之间的跨工具排名一致性与随机水平无异。


<details>
  <summary>Details</summary>
Motivation: 新兴的AI法规强制要求对高风险系统进行偏见审计，审计评分正被用于对模型进行排名。这假设了不同的审计工具测量的是同一事物，本研究旨在直接测试这一关键假设。

Method: 研究人员在共享的10个前沿模型面板上运行了10个外部审计工具，首先针对职业性别偏见，然后针对年龄和社会经济地位，并使用阳性对照组（6个较弱的模型）来区分不同的假设原因。

Result: 10个工具有8个能自信地检测出偏见，但跨工具的排名一致性与随机无法区分（Kendall's W=0.07, p=0.83）。偏见的倾向甚至随审计格式而异：强制选择决策工具倾向于过度修正，而自由生成和默认共指保持刻板印象一致。

Conclusion: 单个审计可以在其自身的运筹范围内检测偏见并估计其方向，但没有任何单一审计支持将一个模型与另一个模型进行排名比较。
...
### [174] [Comment on arXiv:2607.01233: Survivorship Bias in Published-Paper Baselines for Research-Idea Distributions](https://arxiv.org/abs/2609.15996)
*Fredrik A. Dahl*

Main category: cs.CL

TL;DR: 该评论指出陈等人对大语言模型生成的研究设想的评估存在生存者偏差问题。


<details>
  <summary>Details</summary>
Motivation: 对陈、赵和科汉关于大语言模型生成的研究设想的分布评估提出方法论上的质疑，指出人类基线（已发表论文）与大语言模型基线（一次性提案）之间存在不对称性。

Method: 通过逻辑分析与辨识，指出将已发表的论文作为人类基线会导致对未发表的人类设想池中某些类型设想（如桥接或综合类设想）流行度的低估。

Result: 观察到的人类与大语言模型之间的差距可能部分或很大程度上是由生存者偏差造成的。

Conclusion: 在评估LLM生成的研究设想时，必须考虑人类出版流程中的生存者偏差，否则会影响评估结果的准确性。
...
### [175] [Crash Narrative-Guided Countermeasure Recommendation Using Large Language Models: A Retrieval-Augmented Generation Framework for Intersection Safety](https://arxiv.org/abs/2609.15997)
*Abu Saif Md Nasim Uddin,Mohamed Abdel-Aty,Zubayer Islam,Parvez Anowar,Chenzhu Wang*

Main category: cs.CL

TL;DR: 本研究提出了一个基于事故描述的检索增强生成（RAG）框架，利用大语言模型自动将交通路口事故描述转化为有针对性的安全对策建议。


<details>
  <summary>Details</summary>
Motivation: 传统的交通路口安全对策制定依赖于专家经验，耗时且难以扩展，而包含丰富事故机制的非结构化事故描述在以往的安全分析中未被充分利用。

Method: 结合事故描述中的关键机制属性、基于嵌入的历史相似路口检索、关联规则挖掘、统计指导以及引导大语言模型进行领域一致性决策的工程推理指导，构建了RAG框架。

Result: 在佛罗里达州312起致命和重伤事故的评估中，该框架的准确率为0.82，召回率为0.85，F1分数为0.82，推荐的对策数量与实际平均水平高度吻合。

Conclusion: 该框架展示了基于检索增强的大语言模型作为可解释且可扩展的决策支持工具的潜力，能够有效协助交通机构将事故描述转化为安全对策建议。
...
### [176] [NepKANUN: A RAG-Based Nepali Legal Assistant](https://arxiv.org/abs/2609.15999)
*Bhabuk Thapa,Prasiddha Koirala,Ranjit Raut,Sunil Regmi,Bal Krishna Bal*

Main category: cs.CL

TL;DR: 该论文开发了一个基于微调大语言模型和RAG框架的AI驱动尼泊尔法律助手，旨在简化法律信息的获取。


<details>
  <summary>Details</summary>
Motivation: 尼泊尔的法律信息由于术语复杂、资源有限和虚假信息而难以访问，因此需要一种工具来使法律知识民主化。

Method: 利用高质量问答对数据集微调大语言模型，并将其集成到检索增强生成（RAG）框架中，支持自然语言法律查询。

Result: 通过BERTScore评估，简单、中等和复杂查询的F1分数分别达到0.82、0.77和0.71，且专家评审证实了其可用性。

Conclusion: 结合定制法律数据与RAG的生成与检索方法，能够有效地向尼泊尔公众普及和简化法律知识的获取。
...
### [177] [Self-reported archetypes and behavioral failures in Large Language Models](https://arxiv.org/abs/2609.15998)
...
### [178] [Nepali Legal Expertise through Generative and Extractive Pre-trained Transformers (NepLEGiT)](https://arxiv.org/abs/2609.16010)
*Ranjit Raut,Tishya Dhakal,Aaryan Shakya,Bhabuk Thapa,Prasiddha Koirala,Bal Krishna Bal*

Main category: cs.CL

TL;DR: 本文介绍了NepLEGiT，一个专为尼泊尔法律领域设计的轻量级语言模型（SLM），旨在克服法律语言复杂性和专业资源短缺的问题，以普及法律知识。


<details>
  <summary>Details</summary>
Motivation: 尼泊尔传统法律服务因语言障碍、信息碎片化以及乡村地区法律专业人才严重短缺而难以触及广大民众，亟需技术手段来民主化法律知识并提升法律服务效率。

Method: 从头预训练了一个基于解码器的GPT-2 SLM（约3000万参数），使用包含约4百万token的尼泊尔法律文本自建语料库；同时评估了mBERT和MuRIL在同一语料库上的持续掩码语言模型预训练作为对比基线。

Result: NepLEGiT在验证集上取得了0.5684的交叉熵损失、1.8的困惑度以及82.9%的下一个token预测准确率；在编码器基线中，mBERT（困惑度2.35）的表现优于MuRIL（困惑度6.07）。

Conclusion: NepLEGiT及相关预训练模型有效适应了尼泊尔法律文本，为解决尼泊尔法律信息获取难和专业知识短缺问题提供了一个强大的生成式及编码式语言模型基础。
...
### [179] [Are We Grading Properly? Understanding Failure Modes in Medical Benchmarks](https://arxiv.org/abs/2609.16023)
*Prithvi Dixit,Pedram Hosseini*

Main category: cs.CL

TL;DR: 本文研究了医疗开放式评估中评分细则（rubrics）的缺陷及其对评分结果的实质性影响，发现现有细则存在大量非原子性与错位问题，并通过重构细则证明了这些缺陷会导致显著的评分偏差。


<details>
  <summary>Details</summary>
Motivation: 随着医学评估转向开放式临床场景，基于细则的评估成为可扩展的主流替代方案，但细则本身可能存在缺陷，本研究旨在探讨这些缺陷是否可被检测及纠正，以及它们对评分的实际影响。

Method: 应用全球细则失效分类法（RIFT）对两个临床基准测试（HealthBench Professional和LiveMedBench）进行分析，评估大模型裁判对细则缺陷的检测情况，并对捆绑式细则进行重写和重新评分以观察分数变化。

Result: 大语言模型裁判发现HealthBench Professional中29.6%的准则非原子性、65.4%错位/僵化；重写捆绑准则导致受影响对话的分数产生高达15.9个百分点的偏移（析取捆绑虚高分数，合取捆绑压低分数）；RIFT在临床细则中对捆绑的检测存在漏报现象。

Conclusion: 医学评估中的细则缺陷不仅是表面问题，而且对最终评分有实质性且重大的影响，细则的原子性和对齐性需要在使用中得到严格检测和修正。
...
### [180] [Retrieval-Driven Memory Reconsolidation for Long-Term LLM Agents](https://arxiv.org/abs/2609.16053)
*Yuanyi Song,Yukai Wang,Xinbei Ma,Zhihui Fu,Jianghao Lin,Weiwen Liu,Jun Wang,Huarong Deng,Yong Yu,Weinan Zhang*

Main category: cs.CL

TL;DR: 本文提出了REALM，一个受认知神经科学中记忆再巩固启发的长效记忆框架，通过检索反馈驱动记忆的持续演化，解决了现有大模型智能体记忆系统静态、缺乏自适应重组的问题。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型智能体的记忆系统主要在接收新信息时更新，将检索视为终点，很少利用检索反馈来持续重组记忆，且多依赖预定义结构和固定检索管道，限制了记忆的自主组织与演化能力。

Method: 提出了REALM框架，将长效记忆建模为持续生命周期：自主将记忆组织为异构认知图，通过自适应组合的图搜索原子检索证据，并基于检索反馈持续进行记忆再巩固。

Result: 在LoCoMo和LongMemEval基准测试中，REALM分别取得了75.97%和65.11%的平均准确率，较最强基线分别提升了7.17和1.31个百分点；消融实验证实记忆再巩固显著提升了性能，能将相关记忆单元重组为更具连贯性的局部结构。

Conclusion: 检索驱动的记忆再巩固机制为大语言模型智能体提供了一种有效且可持续演化的长效记忆机制。
...
### [181] [State of Thought Enables Endogenous Reasoning](https://arxiv.org/abs/2609.16055)
*Zhiren Gong,Yikun Hou,Zihao Zeng,Ming Xiao,Chau Yuen,Wei Yang Bryan Lim*

Main category: cs.CL

TL;DR: 本文提出了“State of Thought (SoT)”这一全新的推理范式，通过模型内部状态自主控制推理过程，实现了大语言模型在测试阶段的高效与通用推理。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时推理范式严重依赖外部强加的控制（如固定的推理程序或受限搜索空间中的昂贵扩展），这限制了模型的泛化能力和效率。

Method: SoT从模型内部的信息传递中提取紧凑的动力学-几何状态，并在冻结的主干网上使用一个582参数的控制器，根据当前推理状态选择性地激活历史推理支持，将推理构建为基于状态的证据条件过程。

Result: 在16个数据集上的实验表明，SoT在提升平均准确率的同时，减少了62.6%的生成Token数，并将端到端延迟降低了44.6%；在视觉语言模型（VLM）任务中也表现出显著的准确率提升和效率优势。

Conclusion: 内源性状态驱动的推理为大语言模型提供了一种更具泛化能力且高效的测试时计算替代方案。
...
### [182] [ViCo: Visual-oriented Coding with Self-Reflection for Chart Replication](https://arxiv.org/abs/2609.16014)
*Jiaxin Duan,Dian Jiao Shuai Zhao,Jiabing Leng,Yiran Zhang,Feng Huang*

Main category: cs.CL

TL;DR: 本文提出了ViCo，一个用于视觉导向编码的训练框架，通过迭代反思机制和多步强化学习来生成高质量的学术图表，克服了现有AI智能体在视觉推理和奖励稀疏性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的AI智能体生成的图表往往缺乏人类设计的美学和语义保真度，而采用自反思机制的高级编码智能体存在视觉推理能力差、反思遵循度有限以及强化学习中奖励信号稀疏的问题。

Method: 1. 引入自监督预热阶段，结合蒙特卡洛树搜索与一致性剪枝来合成高质量的反思轨迹；2. 开发多步强化学习算法，使用反事实基线来估计反思和动作步骤的优势，解决奖励稀疏问题；3. 提出一个自动的多方面评估框架，通过分层异构布局图结构评估图表的风格、布局和语义一致性。

Result: 在三个公开基准测试上的实验表明，基于8B模型训练的ViCo取得了接近具备充足反思能力的专有大语言模型的性能。

Conclusion: ViCo框架通过迭代反思、多步强化学习和自动多维度评估，有效解决了AI生成学术图表中的视觉推理和奖励稀疏难题，显著提升了生成图表的质量。
...
### [183] [Towards Scalable RLVR: Multimodal Instruction Following Data Synthesis and Distillation](https://arxiv.org/abs/2609.16059)
*Yirong Zeng,Zhang Sai,Yuxian Wang,Yutai Hou,Yufei Liu,Xiao Ding,Bibo Cai*

Main category: cs.CL

TL;DR: 本文提出了MIFS（Multimodal Instruction Following Synthesis），这是一个用于生成强化学习就绪的多模态指令遵循数据系统化流水线，解决了多模态强化学习中高质量数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态指令遵循（MMIF）依赖监督微调（SFT）易导致表面模式匹配并损害通用能力，而强化学习（RLVR）受限于缺乏高质量、RL就绪的多模态数据。

Method: 提出了MIFS流水线，包含生成约束协议以合成多样化原始样本、基于学习能力的蒸馏机制以确保策略优化稳定性，以及提供高精度奖励信号的代码验证器，最终构建了包含90k样本的数据集。

Result: 使用MIFS训练的多模态大语言模型（MLLMs）在四个MMIF基准测试中平均性能提升了8.13%，训练收敛速度比使用原始数据快3倍，同时在提升指令遵循精度的同时保留了核心视觉能力。

Conclusion: MIFS有效解决了多模态强化学习中的数据瓶颈，不仅显著提升了模型的指令遵循能力，还克服了传统SFT带来的泛化权衡问题。
...
### [184] [Efficient Multimodal Generative Recommendation with Latent Narrative Reasoning](https://arxiv.org/abs/2609.16070)
*Chenxing Wang,Nantao Zheng,Hao Miao,Juyuan Wang,Xinke Jiang,Yuchen Fang,Aolin Li,Haijun Wu*

Main category: cs.CL

TL;DR: NarraLite是一个高效的多模态生成式推荐框架，旨在解决短剧等情节内容的续写推荐任务，通过联合压缩视觉感知和隐式推理来实现高效的叙事理解。


<details>
  <summary>Details</summary>
Motivation: 情节内容推荐与传统基于用户偏好的推荐不同，其目标由叙事演进决定，需要模型理解多模态故事情节的发展，同时克服冗长视觉上下文和昂贵显式推理带来的效率挑战。

Method: 提出NarraLite框架，包含两个核心组件：1）渐进式谱压缩（Progressive Spectral Compression），将长视觉上下文精简为紧凑的叙事相关证据；2）潜在叙事推理（Latent Narrative Reasoning），引入上下文路由的潜在推理 tokens，在不进行自回归文本解码的情况下实现隐式叙事推理。同时建立了一个用户无关的多模态短剧续写基准测试。

Result: 大量实验表明，NarraLite在续写准确率、叙事连贯性和鲁棒性方面均优于现有方法，并在准确率与效率之间取得了良好的平衡。

Conclusion: NarraLite通过联合压缩感知和隐式推理，有效解决了多模态情节续写推荐中的效率与叙事理解难题，为该领域提供了高效且鲁棒的解决方案。
...
### [185] [The Immutable Past: Formalizing State Mutability and Conflict Resolution in Mutable RAG](https://arxiv.org/abs/2609.16073)
*Hamed HaddadPajouh,Amir AmiriTabat*

Main category: cs.CL

TL;DR: 本论文指出了长期自主智能体中检索增强生成（RAG）由于“语义遮蔽”导致的记忆失效问题，并提出了GC-Mem（内存垃圾回收）推理时一致性协议来解决状态分歧。


<details>
  <summary>Details</summary>
Motivation: 将共享记忆视为只追加流会导致“语义遮蔽”，即冲突的历史观测会累积并压倒最新的有效更新，导致动态环境中的严重状态分歧。

Method: 形式化分析了状态可变性、渐近召回衰减和多数投票陷阱，并引入了GC-Mem（Garbage Collection for Memory），这是一种结合了时间支配算子与矛盾检测的严格推理时一致性协议，用于外科手术式地切除被遮蔽的上下文。

Result: 在包含137,760个内存块和连续累积扫描的基准测试中，标准RAG和时间戳重排序基线性能严重退化，而GC-Mem实现了超过90%的冲突解决准确率。

Conclusion: GC-Mem通过严格的精确率和召回率部署阈值，确保了标准可变RAG根本无法达到的状态收敛，有效解决了长期智能体记忆中的语义遮蔽问题。
...
### [186] [The Imitation Game: When LLMs Learn to Reason Like Programs via Code-Centric Reasoning Data Synthesis](https://arxiv.org/abs/2609.16076)
...
TL;DR: MIMIC是一个利用可执行代码进行推理数据合成和强化学习监督的框架，旨在解决大语言模型在自然语言细粒度推理上的不足。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在编程任务中表现出色，但往往在自然语言的确定性、细粒度推理上失败，过度依赖语义近似而非符号执行。为了弥补这一差距，需要将可执行代码的程序严谨性引入推理数据合成中。

Method: 提出了MIMIC框架，通过叙事融合、代码引导的测试合成和动态代码 instrumentation 将算法转化为可验证的推理轨迹，并利用显式中间执行状态形成代码插桩奖励（CIR）用于强化学习的过程监督。

Result: 通过监督微调（SFT）和GRPO在合成数据集上训练的模型在通用推理、复杂数学基准测试以及细粒度确定性任务上都取得了显著且持续的准确率提升。

Conclusion: 可执行代码的程序严谨性能够有效解锁并增强大语言模型的广义推理能力。
...
### [187] [RAG-CT: Mitigating Privacy Risks on Retrieval-Augmented Generation Systems via Scanning Prompt Distribution](https://arxiv.org/abs/2609.16095)
*Xingyu Lyu,Jiayimei Wang,Jianfeng He,Ning Wang,Yidan Hu,Yimin Chen*

Main category: cs.CL

TL;DR: 本文提出了一种名为RAG-CT的新型防御机制，旨在解决检索增强生成（RAG）系统中通过检索过程泄露个人身份信息（PII）的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 尽管检索增强 generation（RAG）能有效减少大语言模型的幻觉，但攻击者可利用检索过程从底层语料库中提取敏感的个人身份信息（PII），因此需要有效的防御机制。

Method: 提出RAG-CT防御方法，通过分析查询的熵和边际分布，采用基于评分的检测方法来识别恶意查询。

Result: 在两个数据集上进行的广泛实验表明，该方法在显著减少PII泄漏的同时，性能优于现有的四种防御基线。
...
### [188] [Spurious Tool Use: When RL Agents Learn the Wrong Reason to Act](https://arxiv.org/abs/2609.16268)
*Yiwei Yang,Haoxiang Zhang,Bingbing Wen,Yao Lu,Yuchen Wu,Lei Zhang,Julian McAuley,Pan Lu,Bill Howe*

Main category: cs.CL

TL;DR: 本文研究了通过强化学习训练的大语言模型（LLM）代理中工具选择策略的捷径学习（shortcut learning）现象，并提出了一种基于工具必要性的密集奖励机制来缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 通过强化学习优化的LLM代理工具使用策略可能会放大训练数据中的伪相关性，导致代理根据表面提示而非真实任务需求来调用工具。

Method: 构建结合事实问答和数学推理的受控合成环境，注入与工具强相关但因果无关的提示词；通过反事实评估分析捷径行为；提出一种由LLM裁判评估工具必要性的密集决策级奖励来抑制捷径行为。

Result: 实验表明，当代理具备可靠的工具使用能力且提示与工具存在语义对齐时，会产生严重的捷径行为（伪工具调用率增加高达39%）；而引入工具必要性奖励能有效抑制这种由提示驱动的工具滥用，同时保持任务性能。

Conclusion: 任务能力和语义对齐是驱动LLM代理产生工具选择捷径的关键因素，而基于工具必要性的奖励机制是提高代理鲁棒性的有效方法。
...
### [189] [Speaker-Specific and Language-Dependent Temporal Organization in Bilingual Political Speech](https://arxiv.org/abs/2609.16274)
*Nina Hosseini-Kivanani,Nafiseh Taghva,Peter Gilles,Oliver Niebuhr*

Main category: cs.CL

TL;DR: 本研究分析了政治家在使用卢森堡语和法语演讲时的语音节奏，探讨了说话人和语言选择对节奏指标的影响。


<details>
  <summary>Details</summary>
Motivation: 大多数关于说服性演讲中语音节奏的实证研究主要集中在单语英语上，缺乏对双语政治演讲中时间组织方式的跨语言研究。

Method: 分析了10位政治家的400个句子，标注了音段和停顿，计算了辅音和元音的节奏指标（均值、变异性及成对变异指数），量化了说话人和语言的贡献，并使用配对t检验测试了说话人内部的语言效应。

Result: 基于辅音的指标保留了说话人特有的特征，而基于元音的指标主要由语言选择驱动。法语的元音和元音间隔更长且变异性更大，而辅音时间差异较小，且未发现明显的语言与性别交互作用。

Conclusion: 语言选择会系统性地重组双语公众演讲中的节奏时间，其中元音节奏受语言驱动，辅音节奏反映说话人个性。
...
### [190] [Speaker or Language? Explaining Variance in Charismatic Prosody Across Luxembourgish and French](https://arxiv.org/abs/2609.16275)
*Nina Hosseini-Kivanani,Nafiseh Taghva,Peter Gilles,Oliver Niebuhr*

Main category: cs.CL

TL;DR: 本研究分析了双语政治家在卢森堡语和法语自发演讲中的声学韵律特征，探讨了说话者身份与语言对声乐魅力的相对贡献。


<details>
  <summary>Details</summary>
Motivation: 双语公众演讲中，语言和说话方式对魅力型演讲的具体塑造作用及其相对贡献尚不明确。

Method: 分析了10位政治家在高度可比的语境下使用卢森堡语和法语的400个话语，提取了41个与声乐魅力相关的声学韵律特征，并利用混合效应模型分离说话者相关和语言相关的方差。

Result: 说话者身份解释了大部分方差，而语言解释较少但表现出系统性差异：法语表现出更高的微扰（shimmer）和短语末尾基频（表明礼貌），卢森堡语表现出更强的中频谱能量（表明更具声学存在感）。

Conclusion: 这些模式与卢森堡语作为非正式身份语言、法语作为高声望体制品种的社会语言学角色相吻合。
...
### [191] [Efficient One-to-Many Translation with Joint Multi-Stream Diffusion](https://arxiv.org/abs/2609.16312)
*Yiwen Guan,Jacob Whitehill*

Main category: cs.CL

TL;DR: 本文提出了一种基于离散扩散模型的多对多机器翻译方法，实现了目标语言的并行生成和高效的零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归多对一/一对多机器 translation 系统计算代价高昂，且延迟随序列长度和目标语言数量呈线性增长，缺乏灵活性。

Method: 采用离散扩散框架，通过连续语义锚点替代源标记进行条件控制，实现所有目标语言的并行精炼，并支持零样本源语言迁移。

Result: 通过加速采样，该方法在达到与自回归基线相当的监督翻译质量的同时，实现了2倍的加速，并且零样本BLEU提升了11.9%。

Conclusion: 联合多流扩散模型是实现高效、灵活的一对多机器翻译的一种实用且有前景的替代方案。
...
### [192] [StalePO: Anchored Token-Level Preference Optimization using Legacy Post-Edits in Machine Translation](https://arxiv.org/abs/2609.16340)
*Rohit Dhaipule,Sukhdeep Singh Kharbanda,Prasanth Bathala,Pradyumna Lanka,Anubhav Shrimal*

Main category: cs.CL

TL;DR: 本文针对机器翻译中利用过时人工后期编辑进行偏好优化时遇到的“陈旧偏好问题”，提出了一种名为StalePO的新目标函数，并通过实验验证了其在提升翻译质量方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 机器翻译系统频繁升级，但可用的偏好信号往往是旧系统输出的人工后期编辑版本，新模型可能已经超越了这些编辑。收集每个新模型的新后期编辑成本极高，导致了“陈旧偏好问题”，而标准DPO在此场景下会失效。

Method: 提出了StalePO目标函数，包含三个核心要求：对两个响应的似然移动必须向下、策略必须锚定于自身的基线响应、KL约束必须应用于Token级别。

Result: 在英-印和英-土本地化数据集上，StalePO使通过所有LLM作为裁判的MQM质量检查的片段比例分别提高了14.9和4.6个百分点，人工评估也证实了英-印翻译质量的显著提升。

Conclusion: StalePO成功解决了机器翻译中的陈旧偏好问题，通过结合向下似然移动、自我锚定和Token级KL约束，能够将陈旧的人工反馈转化为模型性能的实质性提升。
...
### [193] [How Humans and LLMs Read Gender into Gender-Neutral Physical Descriptions](https://arxiv.org/abs/2609.16366)
*Yingjia Wan,Lin Lin,Elisa Kreiss*

Main category: cs.CL

TL;DR: 该研究探讨了在AI中用“客观”身体描述代替显式身份标签是否能实现性别中立，发现身体描述仍携带有系统的性别关联，且大语言模型与人类在这一感知上存在对齐偏差。


<details>
  <summary>Details</summary>
Motivation: AI公平与伦理领域建议用外貌等“客观”物理描述代替推断出的身份标签（如“她”、“他的”）以实现性别中立，但这种做法是否真的能达到性别中立效果仍是一个未解的经验性问题。

Method: 引入了包含316个常见身体属性和14,706个美国标注者性别关联评分的数据集（GAPA）；评估了16个大语言模型（LLM）对人类性别关联评分的复现能力；并训练了一个预测人类性别关联的代理模型，用于LitBank文学文本的社会语言学分析。

Result: 研究发现，物理描述在人类读者中带有结构化的性别关联；16个LLM能部分恢复人类的关联感知，但表现出系统性对齐偏差（如评分分布压缩、对男性关联的对齐较弱、对非二元性别的非对称弃权）；并发布了性能最佳的代理模型。

Conclusion: 用身体描述代替显式性别标签并不一定能实现真正的性别中立沟通，因为这些描述在人类解释中仍保留了系统的性别关联，同时揭示了模型与人类在此类感知上的系统性偏差。
...
### [194] [Register Tokens for Bounded-State Reasoning in Diffusion Language Models](https://arxiv.org/abs/2609.16372)
*Albert Ge,Chandan Singh,Yufan Zhuang,Xiaodong Liu,Jianfeng Gao,Frederic Sala*

Main category: cs.CL

TL;DR: 该论文研究了掩码扩散语言模型（dLLMs）如何在清除先前生成的文本后，利用一小组寄存器token构成的固定大小状态来跨生成块继续推理。


<details>
  <summary>Details</summary>
Motivation: 扩展跨生成块的推理通常需要将早期生成的文本保留在上下文中，作者希望探讨dLLM是否能仅使用固定大小的携带状态（寄存器token）来代替文本保留，从而继续推理。

Method: 使用少量的寄存器token（固定位置的token，其连续隐藏状态用于跨生成块传递推理进度），并通过后训练使dLLM解码一块文本，在清除文本的同时保留寄存器值，最后从提示词和携带状态继续解码。

Result: 在LLaDA和Dream的对比实验中，寄存器在所有基准测试中均优于离散文本携带方法，数学任务提升高达8.5个百分点，代码任务提升高达19.5个百分点，且可通过强化学习进一步优化。

Conclusion: 寄存器token是掩码扩散语言模型跨块进行长程推理和有界代码生成的有效且高效的替代方案。
...
### [195] [ParsHate: A Benchmark Dataset for Hate and Target Detection in Persian](https://arxiv.org/abs/2609.16393)
*Zahra Bokaei,Walid Magdy,Bonnie Webber*

Main category: cs.CL

TL;DR: 本文介绍了ParsHate，这是一个包含10,000条波斯语推文的十年来首个仇恨言论检测基准数据集，支持细粒度目标识别、显式/隐式仇恨区分以及片段级理由标注。


<details>
  <summary>Details</summary>
Motivation: 填补波斯语领域缺乏长期、高质量、多维度标注的仇恨言论检测数据集的空白，解决现有方法对多样化仇恨言论泛化能力不足的问题。

Method: 结合随机和分数分层的跨时采样收集数据以减少关键词偏差；采用人工标注，涵盖7个结构化目标类别、显式与隐式仇恨及目标区分，并提供片段级理由；评估了当前最先进的波斯语仇恨言论检测模型。

Result: SOTA模型在ParsHate上的仇恨检测表现中等（79% F1），而在目标识别任务上表现较低（25.5% macro-F1），表明该数据集具有相当的挑战性。

Conclusion: ParsHate作为一个极具挑战性的十年期波斯语仇恨言论基准数据集，突显了当前模型在处理多样化和细粒度仇恨言论时的局限性，需开发更先进的方法来提升性能。
...
### [196] [Negation Beyond the Verbal Channel: Temporal Multimodal Correlates in Dialogue](https://arxiv.org/abs/2609.16396)
*Leon Hammerla,Patrick Schrottenbacher,Alexander Mehler*

Main category: cs.CL

TL;DR: 本研究探讨了口语对话中否定线索是否伴随可测量的多模态非语言行为，以及这些行为能否在排除词汇和声学信息的情况下被准确预测。


<details>
  <summary>Details</summary>
Motivation: 尽管否定通常通过语言实现，但口语互动伴随着紧密协调的非语言行为。研究旨在探究以否定线索为中心的语境是否包含可测量的多模态行为信息，其时间分布、模态来源以及是否延伸至对话伙伴。

Method: 研究分析了VR环境下的27场人际访谈（包含时间对齐的视线、面部、头部、身体、手部和手指行为以及964个注释的否定线索），使用20种时间序列模型作为预测探针，在排除词汇和声学信息的情况下，系统评估时间语境、互动来源、模态可用性和事件时间的影响。

Result: 来自说话者行为的最强探针达到了高达0.75的平均保留AUROC。预测信息主要集中在否定线索出现的前后，面部特征产生最大的模态消融影响，而对话伙伴的行为包含较弱且时间分布更广的预测信息。

Conclusion: 口语否定线索伴随着显著的多模态非语言行为（特别是面部特征），这些行为在时间上有规律可循，且能提供独立于词汇和声学输入的预测信息。
...
### [197] [ReMova: Fine-tuning LLMs for English to Belarusian translation](https://arxiv.org/abs/2609.16427)
*Mikita Pilinka,Aliaksandr Kliujeŭ,David Samuel,Yves Scherrer*

Main category: cs.CL

TL;DR: 本文提出针对白俄罗斯语的专用数据清洗流程与微调方法，显著提升了英白机器翻译的性能。


<details>
  <summary>Details</summary>
Motivation: 白俄罗斯语在互联网上面临双重正字法、噪声、语言干扰及拼写错误等问题，数据质量是机器翻译的主要瓶颈。

Method: 开发了一个专门的白俄罗斯语数据清洗流程（包含纠错工具处理双正字法和噪声），并对英白机器翻译模型进行了微调与消融实验。

Result: 消融实验表明，经过清洗的数据对所有微调模型都有显著提升，其中基于大语言模型（LLM）的模型收益约为专用编码器-解码器机器翻译系统的两倍。

Conclusion: 数据质量是白俄罗斯语机器翻译的核心瓶颈，提出的专用清洗流程能有效解决白俄罗斯语的特定噪声与正字法问题，大幅提升翻译模型性能。
...
### [198] [Early-Bird Decoding: Accelerating Diffusion LLMs with Learnable Block Sizes and Parallel Sampling](https://arxiv.org/abs/2609.16450)
*Lixuan Wei,Wei Zhou,Jianwen Wu,Yipeng Shen,Meiling Wang,Haoran You*

Main category: cs.CL

TL;DR: 本文首次提出了一个名为“早期萌芽（EB）”的解码框架，旨在通过自适应分组和位置感知采样来加速扩散大语言模型（dLLMs）的推理，从而在保持准确率的同时显著提高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLMs）通常需要经过多个步骤才能使Token置信度达到解码阈值，导致即使使用了块级KV缓存，推理效率仍然低下。

Method: 提出了EB-Decode框架，包含两个关键组件：(1) 一个可学习的网络，用于将具有相似不确定性的Token自适应分组为变长块；(2) 一个位置感知采样器，用于在预测的变长块内通过更少的解码步骤并行解掩码Token。该方法无需修改预训练的dLLM权重，可作为插件直接部署。

Result: 在三个模型和四个基准测试上的广泛实验表明，EB-Decode比传统解码方法实现高出3.53到18.76倍的吞吐量，且比最强基线Fast-dLLM高出高达1.58倍的吞吐量，同时保持了相当的准确率。

Conclusion: EB-Decode框架通过联合解码低熵且聚集的Token，有效克服了dLLM推理效率低下的问题，是一种高效且易于部署的加速方案。
...
### [199] [Beyond the Name: Demographic Leakage in De-Identified Résumés and Evaluation Artifacts in LLM Bias Audits](https://arxiv.org/abs/2609.16501)
*Qiangju Chen,Yang Xiao*

Main category: cs.CL

TL;DR: 本研究调查了去除语言字段后，大语言模型在去识别简历筛选中是否仍会泄露民族文化信息，发现非语言散文文本仍支持高准确率的人口统计推断。


<details>
  <summary>Details</summary>
Motivation: 去识别简历筛选通常假设去除显式字段可防止民族文化推断，但审计表明声明的语言会导致信息泄漏，本研究旨在检验消除语言字段是否能真正解决这一泄漏问题。

Method: 使用9个开源模型和620份反事实简历，跨越5种民族文化条件和3个线索显著性层级，保持语言属性完全一致，以隔离非语言散文的影响，并测试了LLM作为裁判时的评估设计敏感性。

Result: 目标群体恢复率整体平均为0.757，在高度显著性下饱和至1.000，表明非语言散文足以维持人口统计推断；同时，LLM裁判的结果对评估设计（如是否允许平局）高度敏感。

Conclusion: 仅去除语言字段无法防止大语言模型从简历散文中推断民族文化特征，凸显了区分简历内容中可恢复的人口统计信号与评估协议引入的影响的必要性。
...
### [200] [Competence-Preserving Resume Perturbations Expose Presentation Sensitivity in LLM Screening](https://arxiv.org/abs/2609.16517)
*Qiangju Chen,Yang Xiao*

Main category: cs.CL

TL;DR: 本研究评估了大语言模型在简历筛选中的表现，发现尽管模型具有较高的筛选有效性，但在面对内容相同但外在表现（如措辞、结构等）不同的简历时，决策稳定性较差。


<details>
  <summary>Details</summary>
Motivation: 简历筛选系统应当对潜在的资质证据保持鲁棒性，而不应受到表面排版、措辞和提取质量等无关变化的干扰，因此需要对大模型在简历筛选中的“呈现稳定性”进行控制审计。

Method: 构建了基于职业的、具有控制能力水平的候选人档案，并将每个档案渲染为多种简历呈现形式；使用确定性验证门槛排除改变底层证据的变体，随后测试了六种开源指令微调大语言模型的筛选有效性和配对决策翻转率。

Result: 研究发现筛选有效性与呈现稳定性之间存在明显脱节。例如，Llama-3.1-8B表现出最高的有效性（0.781），但在保持能力不变的呈现变化下，有29.6%的匹配配对决策发生反转；Mistral-7B-v0.3的翻转率更高达41.4%。

Conclusion: 简历筛选评估不仅应考察系统识别更强候选人的能力，还必须评估当相同资质证据以不同方式呈现时，决策结果是否保持稳定。
...
### [201] [PunGraph: Retrieval-Enhanced Phonetic-Semantic Graph Reasoning for Pun Understanding](https://arxiv.org/abs/2609.16557)
...
TL;DR: 本文提出了PunGraph，一个用于双关语理解的检索增强知识图谱框架，并通过构建包含5,730个注释双关语的新数据集WebPun，有效提升了大语言模型在双关语推理上的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在语言理解方面表现强大，但由于语音建模有限且端到端生成不受控制，它们在处理利用语音相似性和语义歧义的双关语推理时仍然表现不佳。

Method: 提出了PunGraph框架，利用Unisyn语音字典、IPA和G2P表示以及WordNet定义构建语音-语义词汇知识图谱，通过检索候选词或词义来将LLM的推理限制在一个结构化的候选空间中；同时引入了一个包含5,730个异形和同形双关语的大规模数据集WebPun。

Result: 在SemEval-2017和WebPun数据集上的实验表明，PunGraph持续提升了小型LLM的性能，并取得了可与强大专有模型相媲美的结果。

Conclusion: 检索引导的语音和语义约束有效地减少了双关语解释中的常见推理错误，突显了将结构化知识与LLM相结合的优势。
...
### [202] [Style-Debiased DPO: Updating LLM Knowledge with Factuality-Aware Synthetic Preference Data](https://arxiv.org/abs/2609.16532)
*Takayuki Yamamoto,Daisuke Kawahara*

Main category: cs.CL

TL;DR: 本论文提出了一种名为风格去偏直接偏好优化（SD-DPO）的方法，用于解决大语言模型中通过持续预训练存储的知识检索准确率不高的问题，通过对偏好数据进行去偏处理，有效提升了模型知识的 elicitation（提取）准确率。


<details>
  <summary>Details</summary>
Motivation: 尽管通过数据增强的持续预训练（CPT）可以将小语料库的知识存入大语言模型，但这些存储的知识往往无法被准确检索。现有的偏好优化方法在处理包含正确知识但表述不同的拒绝回复时存在缺陷，因此需要一种新的方法来更准确地激发模型中存储的知识。

Method: 提出了风格去偏DPO（SD-DPO）方法。该方法对每个样本对中的拒绝回复进行事实正确性评分，反转事实正确但风格不同的拒绝回复的偏好，并进行加权，从而抵消由于风格差异带来的学习信号。

Result: 在QuALITY阅读理解基准测试中，SD-DPO超越了传统的CPT基线，且所需的训练token数量比达到相同增益所需的额外CPT少几十倍；在动态知识编辑基准AToKE上，SD-DPO达到了0.982的总准确率。

Conclusion: SD-DPO能够高效且准确地引导大语言模型提取其内部存储的知识，在知识更新和问答任务中显著优于传统的持续预训练和标准偏好优化方法。
...
### [203] [Challenges of Auditing: Variability in Outputs of Large Language Models for Health](https://arxiv.org/abs/2609.16590)
*Yuan Pu,Yewon Chang,Furong Jia,Xunjian Yin,Jessica Ma,Ayman Ali,Monica Agrawal*

Main category: cs.CL

TL;DR: 研究发现前沿AI模型在不同访问模式（如聊天机器人与API）下提供健康建议时存在系统性差异，这对依赖API的评估有效性提出了挑战。


<details>
  <summary>Details</summary>
Motivation: 人们越来越多地通过不同的访问模式（具有各种设置的聊天机器人、API等）获取健康AI建议，但目前的评估通常依赖API，而消费者则通过聊天机器人界面进行交互，这导致评估有效性受限。

Method: 通过对比分析不同访问模式和设置下前沿AI模型在健康建议方面的表现，识别其系统性差异。

Result: 不同访问模式之间存在系统性差异，表明基于API的评估无法准确反映消费者实际体验到的聊天机器人交互效果。

Conclusion: 模型提供商亟需开放能够忠实复制消费者体验和设置的访问权限，以便进行严格的审计。
...
### [204] [RoleBreak: Benchmarking Long-Horizon Role-Playing Robustness in Spoken Dialogue](https://arxiv.org/abs/2609.16614)
*Yuqi Wang,Fengyuan Liu,Haochen Luo,Zhiqi Yu,Qi Liu*

Main category: cs.CL

TL;DR: 该论文推出了RoleBreak——一个用于评估语音对话模型在长周期角色扮演中鲁棒性的开源基准测试，揭示了当前系统在语义一致性、安全性以及长对话和语音情感表达方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的语音角色扮演基准测试主要集中于角色中心和短周期，无法评估语音对话模型在超出预定义虚构角色的长周期交互中维持多样化角色的能力。

Method: 引入RoleBreak基准测试，包含310个角色、6,688个对话轮次、11,743个细粒度评估标准及情绪目标；评估了全双工、全模态和级联等9种系统配置。

Result: 当前系统在语义角色依从性上强于语音情感；语义鲁棒性在长对话中较脆弱，最强系统平均在10.4轮和11.6轮后便出现角色和安全失效；缩放LLM可提升语义鲁棒性但对语音情感无明显改善；用户语音情感会影响角色的扮演行为。

Conclusion: 目前的语音角色扮演系统在长周期的鲁棒性和语音表现力方面仍存在显著差距，亟待改进。
...
### [205] [Quantifying Organizational Environmental Action from Web Data and Large Language Models](https://arxiv.org/abs/2609.16627)
*Quinn Reynolds,Daniel Shore,Vianey Leos Barajas,Tanhum Yoreh,Meredith Franklin*

Main category: cs.CL

TL;DR: 本研究开发了一个可扩展的计算框架，利用大语言模型从非结构化的网页内容中提取并量化组织的环境行动，并以美国犹太会堂为例进行了验证。


<details>
  <summary>Details</summary>
Motivation: 量化组织的环境行动极具挑战性，因为相关信息分散在多个网页且主要以非结构化文本形式存在，亟需一种可扩展的自动化提取方法。

Method: 整合多源数据构建全国性会堂数据库并抓取网页，对比了三种检测环境行动的方法：关键词检索+大模型分类、语义向量检索+大模型分类，以及直接大模型分类。

Result: 直接大模型分类法识别出1,398个会堂（53%）采取了环境行动，提供了比检索方法更高的覆盖率，而预检索方法虽能降低成本但会导致相关信息丢失。

Conclusion: 直接使用大语言模型分类非结构化网页内容是提取组织级环境信息的有效且高覆盖率的方法，该框架可推广至其他机构。
...
### [206] [Rewarding Reasoning, Not Answers: Fixing and Bounding Test-Time Reinforcement Learning on Medical QA](https://arxiv.org/abs/2609.16660)
*Kailong Fan,Anqi Pu,Yichen Wu,Wanhua Li,Yicong Li,Hanspeter Pfister,Huafeng Liu,Xiang Li,Quanzheng Li,Ning Guo*

Main category: cs.CL

TL;DR: 该论文指出了测试时强化学习在医学多选题上失效的原因，并提出了PROSE（过程奖励引导的自训练）方法，通过奖励推理质量而非答案一致性，显著提升了模型在医学问答上的性能。


<details>
  <summary>Details</summary>
Motivation: 测试时强化学习在数学领域表现优异，但在医学多选题问答中会出现崩溃（准确率停滞，输出多样性下降），作者旨在诊断这一失败原因并寻找解决方案。

Method: 通过控制实验分析答案空间结构对强化学习的影响，进而提出PROSE方法：利用医学过程奖励模型对每个推理步骤打分，将轨迹奖励设为各步骤的最低分，并强制执行答案格式约束。

Result: PROSE在无需标签的情况下大幅提升了通用Llama模型的性能，超越了专用医学模型并匹敌更大规模的系统，且模型适应后无需推理时奖励模型即可泛化到未见数据集。

Conclusion: 答案空间结构是导致传统测试时RL在医学QA中失效的根源，而基于步骤级最低分聚合的过程奖励引导自训练（PROSE）是解决该问题的有效且可迁移的方案。
...
### [207] [DiaWhisper-DPO: Role-Attributed Transcription of Clinical Interviews via Failure-Mined Preference Optimization](https://arxiv.org/abs/2609.16661)
*Weiming Li,Ana Catarina Fidalgo Barata,Miguel Constante,João Miguel Sanches*

Main category: cs.CL

TL;DR: 本文提出了一种名为DiaWhisper的端到端模型，用于临床访谈中的自动抑郁筛查的语音转写与角色归属（医生或患者），并通过DPO方法进行优化，显著降低了错误率并提高了稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统的级联系统（结合说话人分割与角色分配启发式规则）容易导致错误在各个阶段传播，且临床访谈的自动化筛查需要准确区分医生和患者的言语。

Method: 提出端到端模型DiaWhisper（基于Whisper-large-v3微调LoRA并加入辅助帧级角色头），同时提出DiaWhisper-DPO，利用真实解码失败作为DPO的拒绝完成项进行无人工偏好标注的精细化训练。

Result: 在DAIC-WOZ测试集上，DiaWhisper-DPO达到了0.973的角色准确率和0.119的双向错误率（DER），较最强级联基线降低了72%，并将种子变异度从0.205降至0.002。在PDCH-HAMD数据集上也取得了显著提升。

Conclusion: 研究所提出的端到端方法DiaWhisper及其DPO精细化策略能够高效、稳定地解决临床访谈中的语音转写与角色归属问题，超越了传统级联系统。
...
### [208] [Japanese Stroke LLM Evaluation: A Conversational Benchmark for Safe Stroke Care in Japanese Using Large Language Models](https://arxiv.org/abs/2609.16739)
*Keisuke Masuda,Kazutaka Yatsushiro,Hirohumi Iwamoto,Hirofumi Hirano,Ryosuke Hanaya*

Main category: cs.CL

TL;DR: 本研究提出了日本卒中大模型评估（Japanese Stroke LLM Evaluation）这一多轮对话基准，用于在临床实践导向条件下评估大语言模型在卒中护理中的表现与安全性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医学多选题考试中表现优异，但其在临床病史采集、紧急程度评估和安全性方面的能力仍未得到充分评估。

Method: 构建了10个卒中及相关病情的多轮日文对话案例，由神经外科专科医师扮演模拟患者并进行评估，测试了18个模型在病史采集和行动阶段的表现，并设定了零致命错误的严格安全阈值。

Result: Claude Fable 5 以 87.4% 的最高分且零致命错误通过了安全阈值，其次是 Claude Opus 4.7；然而，有 11 个模型共犯下了 17 个可能直接威胁生命的致命错误。

Conclusion: 该基准为实践条件下的 LLM 评估提供了有效框架，2026年的模型性能有所提升，部分模型达到安全标准，但仍需使用真实案例进行进一步评估。
...
### [209] [TIAO: Token Importance-Aware Policy Optimization for Text Summarization](https://arxiv.org/abs/2609.16748)
*Qixiu Li,Chenlong Bao,Xiang Zhu,Xiaoyong Li,Ruixin Cao,Shukai Chen,Zhenxiong Zhou*

Main category: cs.CL

TL;DR: 本文提出了一种名为Token Importance-Aware Policy Optimization (TIAO)的新型强化学习策略，通过考虑token的重要性来提升大语言模型在文本摘要任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法直接将奖励信号应用于无差别的token序列，忽略了单个token对摘要中词级和句级质量的不同重要性。

Method: 提出TIAO策略，根据token依赖性识别核心token，并根据整体依赖性重新加权轨迹的优势（advantage）。

Result: 在真实世界数据集上的实验表明，TIAO取得了极具竞争力的结果，经TIAO增强的7B基础模型性能可与GPT-4和GPT-5-nano相媲美。

Conclusion: 引入token重要性感知的强化学习策略能有效提升大语言模型生成文本摘要的质量（如一致性和连贯性）。
...
### [210] [Benchmarking Factual Robustness of LLMs via Multi-conversation Persuasion](https://arxiv.org/abs/2609.16777)
*Zhuoang Cai*

Main category: cs.CL

TL;DR: 本文揭示了大语言模型在对话中因上下文惯性掩盖了真实脆弱性的问题，并提出了SAST-IR框架与CP-Agent来评估模型在无状态（记忆擦除）情况下的抗说服攻击能力。


<details>
  <summary>Details</summary>
Motivation: 现有的红蓝对抗框架多在多轮对话中评估模型，模型往往因维持上下文一致性而产生“拒绝惯性”，这掩盖了其面对孤立说服攻击时的真正脆弱性，因此需要更严苛的“冷启动”防御评估。

Method: 引入SAST-IR（有状态攻击者、无状态目标-迭代求精）框架，在强制目标模型擦除记忆的同时保留攻击者历史；利用CP-Agent（认知说服智能体）并在自定义的CounterFact-Strict数据集上进行实验。

Result: 简单的攻击策略在无状态防御下达到了96%的攻击成功率和84.7%的真实说服率，同时揭示了“复杂性悖论”（复杂攻击易触发防御性顺从，而简单策略更有效）。

Conclusion: 当前最先进大语言模型在无记忆防御状态下表现出严重的脆弱性，现有的对话历史机制可能错误地掩盖了模型对说服攻击的真实不安全性。
...
### [211] [Smarter by the Moment: Environment-Driven Dynamic Policies for Continual LLM Improvement](https://arxiv.org/abs/2609.16800)
*Ting-Wei Chang,Po-Chun Chen,Hen-Hsen Huang,Hsin-Hsi Chen*

Main category: cs.CL

TL;DR: 本文提出了动态检索策略生成（DRPG）框架，通过从历史数据和环境反馈中显式合成可执行策略，实现大语言模型的持续改进，克服了传统基于记忆的方法仅检索单个历史示例而不进行策略综合的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的增强记忆方法通常只检索过去的个别示例作为直接参考，无法显式地从中综合出可执行的策略，导致同类错误不断重复，因此需要一种能够实现大语言模型持续适应和改进的新框架。

Method: 提出了动态检索策略生成（DRPG）框架，将基于记忆的检索与动态策略生成器相结合，利用历史数据和环境反馈为持续学习生成任务特定的策略。

Result: 在涵盖文本到SQL、问答、医学诊断和Python编程的六个基准测试以及七个大语言模型上的实验表明，DRPG在大多数数据集和模型上均优于强大的基线方法，且对检索策略具有鲁棒性。

Conclusion: DRPG能够有效地通过策略级指导促进大语言模型的持续适应，策略级指导的效益取决于任务特征，这为该机制的有效应用场景提供了实践洞察。
...
### [212] [A Data-free Universal Prior over Syntactic Structures](https://arxiv.org/abs/2609.16854)
*Fermín Moscoso del Prado Martín*

Main category: cs.CL

TL;DR: 研究表明，句法结构的通用先验概率可以从一个基于认知动机的语言生成模型中独立于语言特定经验而自然涌现。


<details>
  <summary>Details</summary>
Motivation: 探讨语言句法结构的概率是否可以不依赖于语言特定的经验（统计学习）而独立产生，并寻找其潜在的认知起源。

Method: 构建了一个认知动机驱动的增量语言生产模型，通过网络增长将词语逐步整合到句法结构（依存树）中，无需拟合语言数据即可计算句法结构的先验概率，并在138种语言和34个语料库中进行了验证。

Result: 该模型在所有138种具代表性的语言中，赋予实际存在的句法树的概率均高于随机树；在34个语言语料库中有33个显示出先验概率与语料库估计概率的正相关。

Conclusion: 句法结构的部分概率分布可以独立于语言特定的统计学习而产生，语言经验可能只是在此基础上进行微调，而非从均匀空间构建。
...
### [213] [Reduplicative constructions in Mandarin: Socio-emotional profiling through distributional semantics](https://arxiv.org/abs/2609.16860)
*Chaoyi Wu,Yu-Hsiang Tseng,R. Harald Baayen*

Main category: cs.CL

TL;DR: 本研究利用分布式语义学中的词嵌入技术，探讨了普通话中两种多产的重叠结构的语义复杂性和构词过程。


<details>
  <summary>Details</summary>
Motivation: 澄清是否能够通过词嵌入更精确地理解普通话重叠词丰富多变的语义，并探索词嵌入在理解复杂构词过程中的实用性。

Method: 使用腾讯词嵌入（Tencent embeddings）进行语义分析，并通过语义 profiling 和 Procrustes 分析来比较基词空间与重叠词空间的异同。

Result: 词嵌入空间成功恢复了文献中已知的重叠词语义和语法属性，揭示了两种重叠模式在分布空间中的语义和语用差异，并证明了基词空间的整体组织在重叠词空间中大体得以保留。

Conclusion: 高-dimensional 词嵌入能够恢复既定的语言学一般规律，并有效捕捉普通话重叠的语义多功能性以及构词透明度。
...
### [214] [Cascade: Hierarchical Recoverability Control for Large Language Model Unlearning](https://arxiv.org/abs/2609.16890)
*Qingchen Yu,Shiying Duan,Xiaodong Li,Yuhua Wang,Zhiyu Li,Shiji Zhou,Yifan Sun,Zhaoxin Fan*

Main category: cs.CL

TL;DR: 本文提出了Cascade，一个分层的可恢复性控制框架，通过最小化目标知识的内部可识别性来改进大语言模型的遗忘（unlearning）效果。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM遗忘方法往往在中间表示中留存残余知识，这些知识仍可被恢复。为了彻底消除敏感或受版权保护的知识，需要一种更彻底的机制。

Method: 提出了Cascade框架，包含三个互补的控制机制：路径级路由（抑制隐私相关的激活路径）、表示级压缩（降低几何可分性）以及解码级干预（限制残余恢复）。

Result: 在TOFU、MUSE-News和WMDP数据集上的实验（包括针对查询重构和提取式提示的鲁棒性测试）表明，Cascade能有效降低知识可恢复性，同时保持模型整体效用的稳定。

Conclusion: Cascade通过多层次的内部表征控制，能够有效且稳健地实现大语言模型的知识遗忘，克服了现有方法残留知识易被恢复的缺陷。
...
### [215] [RiskChainBench: A Benchmark for Obfuscated Platform Message Restoration and Evidence-Grounded Web Investigation](https://arxiv.org/abs/2609.16900)
...
TL;DR: 本文推出了RiskChainBench基准测试，用于评估模型在打击平台滥用活动中恢复被混淆的恶意文本并作为Web智能体调查风险网站的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试将混淆文本和高风险网页分开评估，忽略了目标恢复对下游取证的影响，因此需要一个能够端到端评估从消息恢复到网页调查全过程的框架。

Method: 引入RiskChainBench，包含3,600个合成Token-文本恢复输入和600个本地网页环境。模型首先恢复混淆消息、意图和目的地，然后利用VLM驱动的Web智能体调查关联网站并生成有证据引用的风险报告，最后通过离线组合门控机制进行评分。

Result: 对十个模型的评估显示，Entry Top-1准确率在35.2%到95.2%之间，网页决策准确率在26.3%到62.8%之间；执行失败占网页运行的31.9%，表明稳定的探索和风险判断是主要瓶颈。

Conclusion: RiskChainBench成功揭示了现有模型在处理平台滥用时，从混淆文本恢复到下游网页调查的全链路能力差距，指出稳定探索和风险判断是当前的主要技术瓶颈。
...
### [216] [Deconstructing Stereotypes: Scope-Conditioned Generation for Effective Multilingual Counterspeech](https://arxiv.org/abs/2609.16906)
*Greta Damo,Elias Urios Alacreu,Elena Cabrio,Paolo Rosso,Serena Villata*

Main category: cs.CL

TL;DR: 该论文提出了一种基于范围条件（scope-conditioned）的反言论（counterspeech）生成框架，通过将结构化的刻板印象特征整合到大语言模型提示词中，显著提升了自动生成反言论的质量。


<details>
  <summary>Details</summary>
Motivation: 当前自动生成反言论的方法往往产生泛泛的、无效的回复，无法针对仇恨言论背后隐含的刻板印象。

Method: 提出了一种新颖的范围条件生成框架，将结构化的刻板印象特征显式集成到大语言模型提示中，并在英语、意大利语和西班牙语的人工标注新数据集上进行了验证。

Result: 广泛评估表明，刻板印象条件提示在三种语言中均大幅优于通用基线，在事实性、特异性、说服力和有效性方面取得了显着提升。

Conclusion: 结合结构化刻板印象特征的条件生成方法能够有效克服传统方法生成泛泛回复的缺陷，生成更高质量且针对性强的反言论。
...
### [217] [Lit3R: Retrieve-Relate-Read for Evidence-Grounded Question Answering over Scientific Literature](https://arxiv.org/abs/2609.16912)
*Akira Ise,Kotaro Kumagai,Yuta Yamaguchi,Hisanori Ozaki,Yukio Uematsu,Ikuya Yamada*

Main category: cs.CL

TL;DR: tus-nlp的Lit3R系统在LitTraceQA文学问答共享任务中取得了第4名的成绩，该系统无需任务特定训练，结合了检索、相关性分析和阅读（Retrieve-Relate-Read）组件。


<details>
  <summary>Details</summary>
Motivation: 为了应对文献检索问答任务（LitTraceQA）的挑战，即需要从海量文献中检索相关论文、识别支持证据并生成准确答案及证据轨迹。

Method: 结合现成的检索、重排和大语言模型（LLM）组件，不进行任务特定的训练。检索器迭代结合稀疏/密集检索、交叉编码器重排和LLM验证，并采用论文间扩展；阅读器先识别单篇论文中的证据，再跨论文综合证据生成最终答案。

Result: 在官方测试集上，该系统在排行榜上名列第4位。

Conclusion: 无需任务特定训练的模块化组合方法（Retrieve-Relate-Read）能够有效解决复杂的文学文献问答和证据追踪任务。
...
### [218] [HUMAID-NER: A Disaster Tweet Dataset for Joint Named Entity Recognition and Event Classification via Uncertainty-Weighted Multitask Learning](https://arxiv.org/abs/2609.16964)
*Aijaz Ali,Nazish Basir,Sarfaraz Nawaz,Danish Nazir Arain,Haris Ali*

Main category: cs.CL

TL;DR: 本文推出了首个基于HumAID灾害推文基准的命名实体识别数据集（HUMAID-NER），包含6万条英文推文和17.5万个实体标注，并提出了一种联合多任务学习框架实现灾害实体识别与人道主义事件分类。


<details>
  <summary>Details</summary>
Motivation: 现有的灾害推文资源主要提供文档级别的类别标签，缺乏细粒度的跨度级（span-level）实体标注，无法满足人道主义救援中快速提取结构化信息的需求。

Method: 构建了一个包含三个阶段的混合标注管道（结合spaCy transformer、领域规则和正则表达式），并提出基于共享RoBERTa-large编码器的联合多任务学习框架，采用同方差不确定性加权和两阶段训练策略来缓解任务冲突。

Result: 在HUMAID-NER验证集上，该系统同时达到了0.841的NER微平均F1分数和0.761的分类宏平均F1分数，并开发了实时Web仪表盘进行端到端部署演示。

Conclusion: 所提出的HUMAID-NER数据集、标注管道和多任务学习框架有效提升了灾害推文的结构化信息提取能力，为人道主义救援和危机信息学研究提供了有力支持。
...
### [219] [Target-Language Generation in Multilingual Models: Activation Steering and Optimal Control](https://arxiv.org/abs/2609.16967)
*James A. Michaelov,Carmen Amo Alonso,Tyler A. Chang,Roger P. Levy*

Main category: cs.CL

TL;DR: 本文开发了一种用于多语言模型目标语言文本生成的最佳控制方法，并提出了一套评估生成文本质量的框架。


<details>
  <summary>Details</summary>
Motivation: 确保多语言语言模型在特定目标语言中生成连贯文本是多语言语言建模中的一个主要问题，现有方法通常需要大量的超参数调优。

Method: 开发了一种针对目标语言文本生成的最佳控制方法，以及一个用于评估语言依从性、语言连贯性和语义连贯性的评估框架。

Result: 实验发现，在测试的大多数模型中，该方法表现至少与突出的均值差激活引导（difference-in-means activation steering）方法一样好，且所需超参数调优显著减少。

Conclusion: 所提出的最佳控制方法能够有效且高效地引导多语言语言模型生成特定目标语言的文本，降低了调参复杂度。
...
### [220] [Autoformalizing Argumentative Material Inferences](https://arxiv.org/abs/2609.16991)
*Xin Quan,Reto Gubelmann,André Freitas*

Main category: cs.CL

TL;DR: 本文提出了一个名为GUARD的神经符号学框架，用于将自然语言论证自动形式化为机器可验证的逻辑证明，同时有效防止逻辑泄露和过度证明问题。


<details>
  <summary>Details</summary>
Motivation: 自然语言论证包含大量隐含前提，而形式化验证需要明确的逻辑。然而，允许系统自由添加前提可能导致逻辑泄露（如证明了未授权的结论或未使用原前提），因此需要一种受控的自动形式化方法。

Method: 将论证材料推理的自动形式化建模为“卫兵补全（guard completion）”，通过大语言模型构建候选卫兵并进行形式化，利用Isabelle/HOL定理证明器进行验证，并通过对比测试检查前提依赖性和结论选择性，实现迭代优化。

Result: 在Debatepedia和ARCT数据集上的实验表明，GUARD相比现有技术在验证忠实度上显著提升（+35.3和+32.9个百分点），并大幅减少了逻辑泄露（-25.9和-21.9个百分点）。

Conclusion: 通过将神经符号学框架与对比测试和定理证明器反馈相结合，可以有效地将隐式自然语言论证转化为可靠、形式化验证的逻辑推理，解决自动形式化中的逻辑泄露问题。
...
### [221] [Nameless Tokenization: A Lossless Tokenizer-Level Defense Against Control-Token Forgery in Open-Weight LLMs](https://arxiv.org/abs/2609.16984)
*Kisu Yang,Yoonna Jang,Heuiseok Lim*

Main category: cs.CL

TL;DR: 本文揭示了开源语言模型中聊天模板分隔符容易被提示词攻击者伪造的安全漏洞，并提出了“无名分词（nameless tokenization）”这一防御方案。


<details>
  <summary>Details</summary>
Motivation: 由于现有的开源大语言模型使用公开的字符串来标记对话轮次、角色和工具结果，任何能够控制提示词文本的用户都可以伪造出与系统服务栈无法区分的轮次边界，从而引发安全风险。

Method: 审计了256个已部署的聊天分词器以评估其可伪造性，并提出了“无名分词”方法，该方法将控制条目保留为专用标识符而不赋予表面字符串，从而防止内容编码器发射控制符并确保消息内容不被篡改。

Result: 所有审计的分词器均可被伪造，且常用的修复标志仍使56.6%的分词器易受攻击；无名分词在无攻击数据上能完美重现标准分词流，并将包含分隔符文本探测的准确率从8.5%提升至59.9%。

Conclusion: 将分隔符的外观与其标识符分离，能够有效防止提示词注入和越狱攻击，保障多轮对话与智能体系统中的消息完整性。
...
### [222] [The Role of Implicit and Explicit Demographic Signals in Large Language Model-based Student Assessment](https://arxiv.org/abs/2609.16993)
*Donya Rooein,Luca Benedetto,Dirk Hovy*

Main category: cs.CL

TL;DR: 本研究探讨了大语言模型（LLM）在学生评估任务中对学生人口统计学特征的敏感性及其引发的偏见。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在学生评估中日益普及，但我们对学生人口统计学特征如何影响其使用效果（尤其是潜在的歧视风险或积极的定制化调整）知之甚少。

Method: 通过设置受控提示词，在自动作文评分、形成性反馈和元语言问答三个任务中，测试了六种先进LLM对显性人口统计学特征（直接提及）和隐性人口统计学特征（通过对话历史作为信号）的反应。

Result: 研究发现模型能感知人口统计学线索并调整输出。显式提及教育水平时，模型常调整反馈的可读性；而隐式条件则产生不可预测的偏见，例如低教育背景的回答会获得较低的情感得分。

Conclusion: 研究提供了明确的证据，证明大语言模型在教育评估任务中对人口统计学特征表现出显著的敏感性，这既带来了适应性调整的机遇，也带来了歧视风险。
...
### [223] [PaperDoctor: Evidence-Grounded and Actionable Feedback for Scientific Papers in Progress](https://arxiv.org/abs/2609.16995)
...
TL;DR: PaperDoctor是一个自动化的论文预审Agent框架，通过多层级评估、可审计的反馈和选择性实验复现，将论文评估从简单的裁决转变为诊断。


<details>
  <summary>Details</summary>
Motivation: 自动科研Agent可能大规模产生带缺陷的论文，而人工顾问的严格评审无法扩展，因此需要一个可扩展的、类似顾问的自动化诊断系统来确保科学文献的质量。

Method: 引入包含三层创新的PaperDoctor框架：1) 包含表面筛选、类型验证器和优先复现器的三层整体分层评估框架；2) 提供观察、具体证据指针和修改建议的可审计且可操作的反馈；3) 根据声明重要性和计算预算选择性地重建和运行实验。

Result: 在30篇进行中的论文上评估达到了70.6%的一致性且整体评分全为正，并在跨多个领域的40篇手稿上进行了测试，能产生比人工和其他Agent更具可审计性的反馈。

Conclusion: PaperDoctor将自动化论文评估从裁决重新定义为诊断，为实现更严格的AI辅助科学发现迈出了具体的一步。
...
### [224] [Can LLMs Follow the Pulse of a Crisis? Evaluating Crisis Sentiment in Bangladesh's July Uprising](https://arxiv.org/abs/2609.16997)
...
TL;DR: 本文推出了UNRESTSENT200K，一个包含约20万条孟加拉语危机情绪数据集，涵盖了2024年7至8月孟加拉国起义期间的五个阶段，并对微调编码器和大型语言模型进行了基准测试。


<details>
  <summary>Details</summary>
Motivation: 低资源语言（如孟加拉国语）的危机情绪分析极具挑战性，且缺乏高质量、带语境的大规模数据集来研究语言、背景和公众反应的快速变化。

Method: 构建了包含约20万条Facebook和YouTube评论的数据集，由14名母语注释者和资深专家进行全人工标注；评估了无语境和有父帖子语境的微调编码器、提示词大模型以及LoRA微调大模型。

Result: 父帖子语境能持续提升模型性能，而跨阶段的时间偏移会导致性能大幅下降；强大的人工智能大语言模型表现良好，但在处理讽刺、隐性政治参考和阶段依赖含义时仍显吃力。

Conclusion: UNRESTSENT200K为低资源危机话语中研究上下文感知和时间鲁棒的情感分析提供了一个重要的基准数据集。
...
### [225] [Diagnosing the Fact-Grounding Gap in Multi-Hop Question Answering](https://arxiv.org/abs/2609.17043)
*Kevin Mo,Nathan Mo,Richard Zhu*

Main category: cs.CL

TL;DR: 多跳问答系统的失败不仅归因于检索错误，还存在“事实 grounding 差距”即提取错误，这占据了近一半的单跳缺陷。


<details>
  <summary>Details</summary>
Motivation: 探究多跳问答系统在个体推理步骤上的失败原因，区分检索失败与提取失败，填补当前评估实践中的盲区。

Method: 跨三个标准的多跳 QA 基准测试，对系统失败模式进行分解和调查，并测试了各种检索干预措施。

Result: 发现失败模式分为检索失败和提取失败（事实-grounding 差距）。提取失败占每跳缺陷的近一半，标准检索指标无法察觉，且无法通过检索干预解决。

Conclusion: 检索失败和提取失败是需要不同解决方案的根本不同的瓶颈，当前的评估实践应将这两者区分开来。
...
### [226] [EviScope: Paired Counterfactual Evidence Diagnostics for Faithful and Efficient Grounded Language Models](https://arxiv.org/abs/2609.17081)
*Suryadeep Singh Deswal*

Main category: cs.CL

TL;DR: 本文介绍了EviScope，一个用于评估语言模型基础（grounding）能力的成对反事实基准测试，旨在通过操纵证据来揭示模型在答案准确性背后的真实推理和冲突盲区。


<details>
  <summary>Details</summary>
Motivation: 现有的接地语言模型系统通常只根据最终答案的准确性进行评估，但这掩盖了诸如无支持的答案、引用错误源头以及在证据不足或矛盾时仍盲目回答的问题。

Method: 引入了EviScope基准测试（包含40个四条件组及修复的反事实声明和跨度级支持标签），通过添加、移除、干扰或矛盾证据来保持问题不变，并在多个主流大模型（Qwen2.5-7B、Llama 3.1 8B、Gemini 3.5 Flash）上评估960个黄金盲测生成结果。

Result: 成对指标揭示了传统答案准确性所掩盖的模型依赖接地行为。例如，开源模型的显式证据动作门控表现不及传统RAG，而Gemini在矛盾插入后仍会对5%的冲突案例进行回答。

Conclusion: EviScope能够有效区分无支持回答、冲突盲区以及错误的非回答动作，而不仅仅是给答案本身打分。
...
### [227] [An Empirical Study of Counterfactual Self-Explanations in LLMs](https://arxiv.org/abs/2609.17119)
*Giannis Kalyvas,Giorgos Filandrianos,Orfeas Menis Mastromichalakis,Vassilis Lyberatos,Giorgos Stamou*

Main category: cs.CL

TL;DR: 本文研究了大语言模型通过反事实自我解释来生成解释的忠实度问题，发现模型规模是决定解释质量的最强因素。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成的自我解释不一定忠实于其真实行为，需要研究反事实自我解释来评估其可靠性。

Method: 在情感分析和自然语言推理任务中，评估了来自 LLaMA-3 和 Qwen-2.5 系列的十个指令微调模型，测量了忠实度、最小性以及与人类标注理由的一致性。

Result: 模型规模决定了解释质量，大模型能更好地生成反事实并针对决策相关证据；而理性引导条件虽然能产生最小编辑的反事实且更贴近人类，但并未显著提高忠实度。

Conclusion: 反事实自我解释能提供有用的行为证据，但其可靠性严重依赖于模型容量，不能盲目假设而需经验验证。
...
### [228] [Psychological Effects of Cultural Upheavals from Millions of Song Lyrics Over 100 Years](https://arxiv.org/abs/2609.17225)
*David M. Markowitz*

Main category: cs.CL

TL;DR: 该研究通过分析跨越一个多世纪的数百万首歌词，探讨了重大社会动荡（如越南战争、911事件和COVID-19）对艺术家语言模式的大规模影响。


<details>
  <summary>Details</summary>
Motivation: 尽管文化动荡影响社会生活的许多方面，但很少有研究在大众媒体中大规模地隔离并评估这些动荡对个人的具体影响。

Method: 分析了越南战争、911恐怖袭击和COVID-19前后五年基准期内的数百万首歌词，考察了艺术家的自我引述、认知处理词汇以及内容主题（如“生活与自由”与“求爱与夜生活”）的变化。

Result: 与基准期相比，越战和911事件后自我引述率（心理疏离的标志）显著降低；动荡后认知处理词汇增加，表明艺术家试图理解重大破坏；歌词内容更多转向“生活和自由”，而减少关于“求爱和夜生活”的讨论。

Conclusion: 文化动荡改变了个人和集体的言语行为，证明了其对社会的深远影响。
...
### [229] [AraMIP: Extending MIPVU Towards Metaphor Identification in Arabic](https://arxiv.org/abs/2609.17235)
*Mandar Marathe,Manar Ali,Sara Nabhani,Raia Abu Ahmad,Ibrahim Baroud,Omar Momen*

Main category: cs.CL

TL;DR: 本文提出了首个针对阿拉伯语隐喻标注的新指南——阿拉伯语隐喻识别程序（AraMIP），并在试点数据集上进行了验证与分析。


<details>
  <summary>Details</summary>
Motivation: 尽管隐喻研究在语言学和认知领域备受关注，且英语等语言已有许多隐喻识别与标注成果，但阿拉伯语在此领域资源严重不足。

Method: 基于广泛使用的MIPVU框架，结合阿拉伯语的特定语言属性进行调整，提出了AraMIP指南，并对包含300个句子（5277个单词）的试点数据集进行了标注和分析。

Result: 识别并区分了三种主要的阿拉伯语修辞类型（Isti'ara隐喻、Kinaya转喻/间接表达、Tashbih明喻），揭示了阿拉伯语特有的挑战，包括形态复杂性、词典义项排序的不一致性以及缺乏标准化的语境材料。

Conclusion: 该研究为标准化阿拉伯语修辞实例迈出了第一步，促进了更大规模标注资源的发展，从而支持未来阿拉伯语具象语言的研究。
...
### [230] [ECHO: Early-layer Collaborative Hierarchical Orchestration with Bonus Logits in Speculative Decoding](https://arxiv.org/abs/2609.17241)
*Ziyang Ma,Zihong Zhang,Zuchao Li,Lefei Zhang,Baoyuan Qi,Siqi Li,Simin Yu*

Main category: cs.CL

TL;DR: 本文提出了ECHO，一个利用大模型层间功能不对称的分层双循环无草稿模型推测解码框架，实现了显著的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有的无草稿模型推测解码方法常受限于陈旧的草稿候选和高昂的验证计算成本。

Method: 提出ECHO框架，分为高频内循环（利用早期层进行快速多步草稿树探索）和低频outer loop（通过状态复用进行权威全模型验证及路径修正）。

Result: 在各类基准测试中显著提升了平均接受token数，实现了2.4倍至2.9倍的加速，超越现有先进基线。

Conclusion: ECHO通过分层双循环机制有效解决了无草稿模型推测解码的效率瓶颈，在无需额外部署参数的情况下实现了高效的LLM推理加速。
...
### [231] [Persistent Recurrent Memory Between Transformer Layers - Improves Language Model Generalization](https://arxiv.org/abs/2609.17251)
*Eduardo Novaes Hering*

Main category: cs.CL

TL;DR: 本论文提出了一种对解码器专用Transformer的简单架构修改：通过交叉注意力观察隐藏表示的持久循环状态，利用GRU进行更新，并通过门控相加调制后续处理，从而显著提高语言模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 旨在解决小型Transformer模型在语言建模任务中的过拟合问题，提高模型对长期依赖关系（如叙事位置）的高效维护能力。

Method: 在6层Transformer的上下半部分之间插入一个轻量级循环记忆模块，该模块通过交叉注意力观察隐藏表示，经由GRU更新，并通过门控相加进行调制，同时通过控制消融实验验证其拓扑结构的有效性。

Result: 在仅增加3.7%参数的情况下，评估损失从2.438大幅降至1.743（减少28.5%），泛化差距从0.26缩小至0.12，且该状态能更有效地编码叙事位置信息。

Conclusion: 在Transformer层之间架设轻量级循环记忆是一种简单有效的方法，能够显著提升小型语言模型的泛化性能。
...
### [232] [Towards Illusions Awareness in Cyber-Physical System's Design](https://arxiv.org/abs/2609.17260)
*Anna Di Placido,Nicolas Ferry,Julien Deantoni*

Main category: cs.CL

TL;DR: 该论文针对信息-物理系统（CPS）中因设计假设与运行时现实不符而产生的“现实鸿沟”，提出了“设计错觉”（design illusions）的概念，并构建了一个用于识别、分类、表征和利用这些错觉的概念化管道，将其转化为可复用的设计知识。


<details>
  <summary>Details</summary>
Motivation: CPS系统运行在开放环境中，无法穷尽所有情况，设计时依赖的假设在运行时可能失效，导致“现实鸿沟”。现有方法主要关注让仿真更逼真，缺乏将失效假设作为可复用设计知识进行结构化和利用的统一方法。

Method: 提出了一种“错觉感知”（illusions-awareness）的概念管道（conceptual pipeline），用于识别、分类、表征和利用设计中的错觉，将其转化为可操作的设计知识。

Result: 明确定义了“设计错觉”这一概念（即持续依赖失效假设及对设计模型操作有效性的虚假信心），并建立了一套将其转化为知识以支持更好设计决策的方法框架。

Conclusion: 应当将设计错觉显式化、结构化并作为知识加以利用，从而帮助利益相关者在面对CPS运行现实时做出更好的设计决策。
...
### [233] [Zero-shot narrative detection in social messaging](https://arxiv.org/abs/2609.17310)
*Jesús M. Fraile-Hernández,Anselmo Peñas,Patrick Giedemann*

Main category: cs.CL

TL;DR: 本研究探讨了大语言模型（LLMs）在零样本（zero-shot）设置下识别和分类社交消息中隐藏叙事的潜力。


<details>
  <summary>Details</summary>
Motivation: 利用LLMs广泛的上下文知识，在缺乏训练数据的领域实现超越基础情感或主题分析的深层、语用级叙事检测。

Method: 在Dipromats和SemEval数据集上进行实验，评估结合人类编写的叙事描述、自动生成的描述以及少样本（few-shot）提示词对LLM性能的影响，并测试了集成方法（如多数投票）和不同规模模型的效果。

Result: 提供人类编写的叙事描述显著提升了模型性能，而自动生成描述或少样本设置往往会因为微小的框架转变而降低准确率；集成方法增强了鲁棒性，且更大规模的模型表现最好且对提示词变化最不敏感。

Conclusion: LLMs在零样本设置下能够有效检测战略叙事，结合简单集成和人工编写的描述，其性能可与监督系统相媲美，为缺乏训练数据的领域提供了可扩展的解决方案。
...
### [234] [Vroom-Vroom at SHROOM-Visions: A Multi-Judge Committee for Detecting Hallucinated Spans in Vision-Language Outputs](https://arxiv.org/abs/2609.17327)
*Toqeer Ehsan,Nico Penttilä,Richard Schmidt,Arash Hajikhani,Victoria Palacin*

Main category: cs.CL

TL;DR: 本文介绍了作者在SHROOM-Visions共享任务中获得优异成绩的视觉语言模型幻觉检测方法。


<details>
  <summary>Details</summary>
Motivation: 旨在检测和分类视觉语言模型输出中跨四种语言的幻觉字符片段。

Method: 采用多个微调的视觉语言模型作为独立标注者，通过字符级多数投票结合它们的片段预测，并探索了激活探针。

Result: 该方法在四分之三的语言中排名第一，并在所有语言和指标上均名列前茅；分析表明，不同模型之间的分歧能够反映人类标注者之间的分歧。

Conclusion: 结合多个微调视觉语言模型的字符级多数投票是检测幻觉的有效方法，模型分歧与人类标注分歧高度相关。
...
### [235] [Towards Detecting AI-Assisted Responses in Online Surveys](https://arxiv.org/abs/2609.17317)
*Qizhou Wang,Bogdan Mamaev,Christopher Leckie*

Main category: cs.CL

TL;DR: 该论文引入了一个名为ASURRE的基准数据集，用于研究和检测AI辅助参与在线调查的行为，并提出了一种基于行为痕迹的检测方法。


<details>
  <summary>Details</summary>
Motivation: 使用大语言模型（LLM）完成在线调查会影响研究的有效性，但目前对检测这种行为的研究尚不充分。

Method: 构建了包含多种使用策略（如完全生成、修改、角色驱动的智能体完成）的ASURRE数据集；评估了现有机器生成文本检测器的性能；并提出了一种针对智能体完成行为的简单、少样本、免训练的特征聚合器。

Result: 现有的机器生成文本检测器可以轻易检测出简单的AI使用，但面对模拟完整受访者的角色驱动智能体时，其性能会降至随机水平；然而，智能体完成方式留下了独特的行为特征，提出的聚合器在智能体设置下将平均AUROC比现有最佳检测器提升了0.14。

Conclusion: 虽然复杂的AI代理（如角色驱动智能体）能逃避传统文本检测器的检测，但它们在调查回答中留下了独特的行为痕迹，可以通过特征聚合方法进行有效检测。
...
### [236] [Where Should a Document Live: Context, Representations, or Parameters?](https://arxiv.org/abs/2609.17346)
*Nathanaël Carraz Rakotonirina,Momchil Hardalov,Gonzalo Iglesias,Adrià de Gispert*

Main category: cs.CL

TL;DR: 本文对大语言模型中基于表示（KV缓存）和基于参数（微调）的新知识注入方法进行了对照实验评估。


<details>
  <summary>Details</summary>
Motivation: 大语言模型需要访问新信息来回答预训练数据之外的问题，但现有的上下文、参数编码和隐式表示等方法各有权衡且缺乏统一的对比。

Method: 在五个知识密集型基准测试上，对基于表示（KV缓存，如Cartridges和Compaction）与基于参数的适配方法进行了控制对比实验。

Result: 在Oracle设置下，Cartridges在几乎所有存储预算下准确率最高，比参数方法高10个百分点；在多文档检索场景中，Cartridges是唯一能匹配上下文学习的方法，领先参数方法29个百分点。但Cartridges也面临灾难性遗忘问题，在控制基准上性能下降6%，编程任务下降13%。

Conclusion: KV缓存方法（如Cartridges）在知识注入的准确性和效率上表现出色，但付出了灾难性遗忘的代价，展示了不同知识注入方法的权衡。
...
### [237] [Enhancing Accessibility of Medical Texts through Large Language Model-Driven Plain Language Adaptation](https://arxiv.org/abs/2609.17398)
*Ting-Wei Chang,Hen-Hsen Huang,Hsin-Hsi Chen*

Main category: cs.CL

TL;DR: 本论文研究了利用大语言模型（LLMs）进行自动通俗语言改写（PLA），以简化复杂的医疗信息并提高患者的阅读理解能力。


<details>
  <summary>Details</summary>
Motivation: 弥合复杂医疗文本与患者阅读理解能力之间的差距，解决在任务特定数据有限的情况下如何使医疗信息更易获取的挑战。

Method: 利用GPT-4o-mini、Gemini-1.5-pro和LLaMA等大模型进行文本简化，并结合混合智能体（MoA）技术、提示词策略比较以及QLoRA微调。

Result: 展示了LLM驱动的PLA的有效性，证明其在简化医疗信息的同时能够保留核心内容，并提升了任务的适应性与鲁棒性。

Conclusion: 大语言模型和混合智能体技术能够成功应用于医疗文本的通俗化改写，显著提升医疗信息的可访问性。
...
### [238] [ECHO: A Matched-Contrast Benchmark for Context-Sensitive Turn-Taking in Full-Duplex Dialogue](https://arxiv.org/abs/2609.17360)
...
TL;DR: 本文推出了ECHO，一个针对中文全双工对话系统轮次转换（turn-taking）的成对诊断基准，旨在区分需要让出话语权的中断和允许继续发言的背面应答（backchannels）。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常独立评估事件，奖励固定的动作偏好而非上下文敏感的决策，且仅评估中断会导致高估系统的实际轮次转换可靠性。

Method: 引入ECHO基准（包含相同重叠文本但上下文不同、要求不同动作的配对样本及off-talk样本），并提出“配对准确率”（pair accuracy）来评估系统，该指标不给常数动作策略分配积分。

Result: 多系统的实验表明，大多数系统表现出明显的“让出（Yield）”偏好，处理中断的表现明显优于背面应答，仅有一个系统保持相对平衡。

Conclusion: 仅评估中断会高估全双工轮次转换的实际可靠性，需要像ECHO这样基于上下文的成对诊断基准来准确评估系统的决策能力。
...
### [239] [What Breaks Under Pruning in Smart Homes, and When? Evaluating LLM Degradation Across Architectures and Task Complexity](https://arxiv.org/abs/2609.17515)
*Congjing Zhang,Vashishtha Patil,Henning Lange,Usman Aleem*

Main category: cs.CL

TL;DR: 本文系统研究了模型剪枝对大型语言模型（LLMs）在智能家居中上下文关联工具调用性能的影响，发现密集模型与MoE模型在剪枝容忍度及降级模式上有显著差异。


<details>
  <summary>Details</summary>
Motivation: 尽管剪枝能降低LLM的部署成本，但其对上下文关联工具调用的具体影响尚不明确，缺乏多维度、系统的评估研究。

Method: 跨4种LLM架构（密集Transformer、密集混合、MoE）采用多种剪枝方法（深度、宽度、混合、专家剪枝），结合微调（SFT），在三个智能家居数据集的超19,500个实例上进行评估，并从动作组件和任务复杂度两个维度分析性能降级。

Result: 密集模型具有较窄的安全剪枝区且随后急剧下降，而MoE模型能承受大幅度剪枝；剪枝会先于模式级意图破坏扎根特异性，激进的密集剪枝还会导致系统性的过度拒绝。

Conclusion: 在为可靠的工具执行选择剪枝LLM时，必须超越简单的总体准确率，从多维度进行全面评估。
...
### [240] [Right Tool, Right Job: Native-Language Evaluation, Tokenizer Sensitivity, and Methodological Findings from a French-Only BabyLM](https://arxiv.org/abs/2609.17435)
*Adam Zachary Wasserman,David Beauchemin*

Main category: cs.CL

TL;DR: 本文介绍了MéTRON-FR，一个在9247万法语单词上预训练的125M GPT-2模型，用于BabyLM 2026 Strict赛道，并探讨了其在法语语法基准、跨语言微调和双语词典归纳方面的表现。


<details>
  <summary>Details</summary>
Motivation: 探索在儿童语言习得规模（BabyLM限制）下，小规模法语预训练语言模型的性能表现、跨语言迁移能力及评估方法的局限性。

Method: 使用92.47M法语单词从头预训练125M的GPT-2模型；采用结合翻译数据与rank-16 LoRA的跨语言GLUE协议；进行双语词典归纳（BLI）以及消融实验来分析零样本评分中的标记化和模板伪影。

Result: 模型在QFrBLiMP上得分为85.97%，在BabyLM加权排行榜上为62.80%；跨语言GLUE显示关系任务收益明显而世界知识任务退化；BLI的p@1准确率为68.84%；消融实验表明单标记零样本评分受标记器和模板伪影影响显著。

Conclusion: 在儿童语言规模下，跨语言对齐主要反映语法能力的获得而非训练时长，同时研究强调了将标记器交换敏感性、安慰剂对照提示和母语最小对偶基准作为标准诊断方法的必要性。
...
### [241] [When Should LLMs Abstain? Chain-of-Self-Questioning for Selective Risk Control](https://arxiv.org/abs/2609.17516)
*Ali Şenol*

Main category: cs.CL

TL;DR: 本文引入了“自问链（CoSQ）”这一提示词框架，使大语言模型在回答问题前能够显式评估所需信息，从而减少无事实支持的错误回答并提高准确率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在缺乏事实支持时仍能产生流利但错误的回答，亟需一种机制来使模型进行自我评估，并在信息不足时做出回答或放弃（abstain）的决策。

Method: 提出了三种CoSQ变体（Grounded-CoSQ、Critical-CoSQ和Adaptive-CoSQ），通过提示词让模型将回答承诺建立在对所需信息的显式评估之上，并在TruthfulQA和Natural Questions数据集上进行了广泛评估。

Result: Grounded-CoSQ在τ=0.90时将平均无条件错误承诺率从思维链提示的13.1%降低到8.9%（相对减少32.1%），同时将回答准确率从86.9%提高到89.7%，并在所有11个测试模型中保持了一致的改进。

Conclusion: 自我评估能够支持显式且可调节的“回答或放弃”决策，在未经支持的承诺代价高昂的场景下非常有效。
...
### [242] [Bi-MoDe: Bilateral Control-based Imitation Learning via Modifier-Conditioned Decoding for Modulation of Execution Speed and Contact Intensity](https://arxiv.org/abs/2609.16040)
*Takumi Kobayashi,Masato Kobayashi,Yuki Uranishi*

Main category: cs.RO

TL;DR: 本文提出了Bi-MoDe框架，通过在Transformer动作解码器中注入受限的潜在变量，实现了基于双边控制的模仿学习中对任务执行速度和力度等行为指令的条件控制。


<details>
  <summary>Details</summary>
Motivation: 现有的基于双边控制的模仿学习方法虽然适用于接触丰富的操作，但缺乏在推理阶段让操作员指定任务执行方式（如慢速/快速、轻柔/有力）的有效手段。

Method: 提出了Bi-MoDe（modifier-conditioned decoding）框架，利用adaLN-Zero将受限的潜在变量注入到Transformer动作解码器的每一层，使行为指令直接影响动作块的生成。

Result: 在真实的白板擦拭任务中，Bi-MoDe在保持可比的时间控制的同时，显著提高了对物理指令（如力度）的遵循能力；消融实验表明解码器条件化和潜在空间组成的结合对准确遵循物理指令至关重要。

Conclusion: Bi-MoDe成功地为双边控制模仿学习引入了直观的推理时行为调节能力，有效提升了机器人在接触丰富任务中的灵活性和可控性。
...
### [243] [MR-GLi: Mixed Reality-Based Gripper-Linked Overlays for Underwater Robot Arm Teleoperation via Bilateral Control](https://arxiv.org/abs/2609.16041)
*Masashi Sasago,Masato Kobayashi,Yuki Uranishi*

Main category: cs.RO

TL;DR: 该研究开发并评估了一种名为MR-GLi的混合现实(MR)界面，用于水下双边遥操作，通过将视觉反馈空间配准到机器人夹爪上，降低了操作者的注意力负担。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉力矩反馈支持水下双边遥操作，但混合现实(MR)相比传统显示器呈现方式的优势尚不明确，且需要改善视觉反馈与工作空间之间的注意力切换问题。

Method: 提出了MR-GLi界面，将反应力矩指示器和腕部相机图像空间配准到机器人夹爪上；招募了20名参与者，在刚性和柔性物体的提升及抓放任务中，将其与2D显示器进行了平衡内的受试者间比较实验。

Result: MR-GLi在维持与2D显示器相似的力矩调节性能的同时，提供了与夹爪联动的视觉反馈访问，主观评估还表明它减少了在工作空间与视觉反馈之间切换注意力的感知负担。

Conclusion: 该研究证明了夹爪联动MR叠加层在水下双边遥操作中的可行性，并强调了在任务性能之外考虑信息获取的重要性。
...
### [244] [World-Action Models for Robot Learning and Control: A Survey](https://arxiv.org/abs/2609.16074)
...
TL;DR: 本文对机器人领域的“世界-动作模型”（WAMs）进行了全面综述，探讨了如何将未来世界预测与可执行动作生成相结合，以实现更可靠的具身智能。


<details>
  <summary>Details</summary>
Motivation: 开放环境中的机器人面临部分可观测性、物理约束和动态任务上下文，不仅需要将观察和语言指令映射到动作，还必须预测候选动作对未来状态及任务结果的影响。

Method: 提供了一个面向机器人的WAMs综述，通过统一的分类法（涵盖表征、转移建模、动作接口、架构、训练管线、数据模态和扩展策略）梳理现有方法，并总结了相关应用、数据集、评估指标及面临的关键挑战。

Result: 厘清了WAMs与传统世界模型、基于模型的强化学习、动作条件视频生成以及反应式VLA策略的区别，总结了其在操作、导航和自动驾驶领域的应用与评估协议。

Conclusion: 该综述为集成预测性世界建模与动作生成提供了统一的技术基础，旨在推动更可靠的具身机器人智能发展。
...
### [245] [AssemblyGrid v1: A Benchmark for Multi-Robot Production with Temporary Coalitions, Local Information, and Geometric Constraints](https://arxiv.org/abs/2609.16075)
*Fouad Bahrpeyma,David Heik,Dirk Reichelt*

Main category: cs.RO

TL;DR: 本文介绍了AssemblyGrid v1，这是一个用于重复多机器人柔性生产的可复现基准测试，旨在解决去中心化控制下的部分可观测性和资源竞争问题。


<details>
  <summary>Details</summary>
Motivation: 柔性机器人生产需要联合决策工艺进展、物料路线、资源分配、临时合作和并发执行，在去中心化控制下挑战更大，这与部分可观测和资源竞争下的多智能体决策高度吻合。

Method: 提出了AssemblyGrid v1任务级形式化基准测试，包含三种工作负载族及场景级别，并使用特权集中式参考、结构化去中心化控制器以及IPPO、MAPPO和QMIX等MARL方法进行评估。

Result: 实验结果证明了集中式和去中心化控制下的生产执行能力，且MARL实验表明去中心化策略能够从局部观测和动作中学习到有效的生产行为。

Conclusion: AssemblyGrid作为一个受控基准，成功支持了对柔性机器人生产中合作决策的研究，验证了多智能体强化学习在该领域的有效性。
...
### [246] [Structure-Preserving Quantum Circuit Architectures for Robot Kinematics](https://arxiv.org/abs/2609.16089)
*Andrea Morghen,Pierluigi Arpenti,Roberto Schiattarella,Giovanni Acampora,Bruno Siciliano*

Main category: cs.RO

TL;DR: 本研究提出了一种用于刚体变换及串联机械臂Denavit-Hartenberg运动学的量子表示与电路架构，能够在保持几何关系的同时在有限深度硬件上实现。


<details>
  <summary>Details</summary>
Motivation: 结构化空间数据需要能够保持几何关系、暴露出可测量观测量且可在有限深度硬件上实现的量子编码。

Method: 将平移贡献分解为经典度量幅度和由单量子比特Bloch矢量编码的符号单位方向，利用参数化旋转重现坐标系方向的有序传播，设计选择器寄存器和读出量子比特编码归一化加权和及终端坐标系轴。

Result: 在理想期望值水平下，测量的泡利观测量重现了相应的经典运动学量；理想仿真下位置和方向重建误差可忽略不计；并通过有限次采样仿真、噪声执行和硬件演示表征了误差和噪声敏感性。

Conclusion: 该架构为机器人运动学提供了一种几何上可解释且硬件可实现的量子表示方法，在噪声和有限采样条件下具有良好的可行性。
...
### [247] [Occupancy Network-Guided Autonomous Robotic Partial Nephrectomy](https://arxiv.org/abs/2609.16186)
...
TL;DR: 本文介绍了首个能够进行完整肿瘤切除的视觉引导自主软组织癌症手术系统，实现了在组织形变和切割过程中的实时3D解剖感知与自适应执行。


<details>
  <summary>Details</summary>
Motivation: 现有的自主软组织癌症手术局限于器官表面干预，因为现有系统无法在解剖结构发生形变或被切割时进行感知和适应。

Method: 集成条件占用网络（在物理仿真中训练，从单视角点云推断完整3D解剖结构），结合深度相机和双机械臂平台，实现实时跟踪、自适应规划与自主切除。

Result: 在患者衍生的人工水凝胶模型上进行了8次连续的自主肿瘤切除（共77次电外科切割），所有切口均实现了阴性手术边缘，平均绝对边缘误差为1.61 ± 0.48 mm。

Conclusion: 该研究首次展示了在模型中实现监督式自主闭环、影像驱动且边缘阴性的肿瘤切除的基础。
...
### [248] [Tendon-Driven Continuum Robot with Modular Stiffness and In-Situ Self Pose Estimation](https://arxiv.org/abs/2609.16256)
*Guo Ning,Sue,Zheng Cao,Junzhe Hu,Xiangyun Bu,David Quinn,Tiancheng Wu,Zackory Erickson,Carmel Majidi*

Main category: cs.RO

TL;DR: 本文提出了一种自包含的模块化连续体机器人平台，结合了机械可重构性与板载姿态估计，无需外部传感即可实现自感知和自适应。


<details>
  <summary>Details</summary>
Motivation: 现有的连续体机器人多为任务专用且依赖外部传感基础设施，限制了其适应性和实际部署能力。

Method: 使用具有预计算刚度的可互换连续体关节进行快速组装；采用磁传感器和模块化学习框架实现本体感觉，每个关节训练单个模型并在不同配置中复用。

Result: 通过真实世界的实验验证了该系统，证明了其在没有外部跟踪情况下的自感知能力和适应性。

Conclusion: 该平台成功实现了连续体机器人的快速重构与免外部追踪的精确自感知，提高了机器人的现实部署能力和通用性。
...
### [249] [ConGraspXL: Controllable Constraint-Conditioned Dexterous Grasping Motion Synthesis](https://arxiv.org/abs/2609.16319)
*Hui Zhang,Mirko Meboldt,Jie Song*

Main category: cs.RO

TL;DR: 本文提出了ConGraspXL，一种可控的、条件约束的灵巧抓取运动合成方法，能够处理多种任务驱动的约束及其组合。


<details>
  <summary>Details</summary>
Motivation: 先前的GraspXL虽然实现了针对多样化物体和手部形态的可扩展抓取运动合成，但缺乏在特定任务驱动约束（如特定接近方向、接触区域、腕部轨迹和功能性手部姿势）下的可控性。

Method: 引入了分层约束公式，通过掩码残差接口实现灵活的约束组合，并利用动态手部中心和前馈腕部引导来提高控制精度。

Result: 在保持GraspXL强大泛化能力的同时，ConGraspXL对各种单个约束及其组合实现了精确且灵活的可控性，可作为即插即用的低级抓取控制器。

Conclusion: ConGraspXL成功扩展了灵巧抓取合成在复杂任务约束下的可控性，能有效应用于全身抓取完成、功能性抓取和人体动作模仿等下游任务。
...
### [250] [Auto-HSI: Personalized human control of a robot swarm on demand by using LLMs for online automatic code generation](https://arxiv.org/abs/2609.16346)
...
TL;DR: 本文提出了 Auto-HSI，一种按需生成个性化人机群交互（HSI）界面的方法，允许未经培训的操作员通过自然语言和手势演示来自动生成控制机器集群的代码。


<details>
  <summary>Details</summary>
Motivation: 使未经训练的操作员能够通过直观的自然语言和手势轻松定义机器人的集体行为，并自动生成相应的控制代码，以解决传统人机群交互界面复杂且缺乏个性化的问题。

Method: 开发了 Auto-HSI 原型，利用操作员提供的自然语言描述和手势演示，自动生成用于控制机器人运动、队形形状及形变的个性化状态机代码，并通过手势追踪基准测试、物理模拟器中的实时操作实验（控制50个模拟机器人）以及真实机器人实验进行验证。

Result: 实验表明，Auto-HSI 成功支持操作员在标准和噪声条件下通过手势实时控制50个模拟机器人完成进球、穿过迷宫、分组进球等任务，并支持实时更新个性化界面及控制真实机器人。

Conclusion: Auto-HSI 能够有效地根据用户的自然语言和手势输入按需生成个性化的人机群交互界面，实现对复杂机器人集群的高效、直观的实时控制。
...
### [251] [UDAV: Uncertainty-Driven Adaptive VLM Waypoint Planner](https://arxiv.org/abs/2609.16368)
*Ghazal Farhani,Shabnam Shabani*

Main category: cs.RO

TL;DR: 本论文提出了UDAV（一种不确定性驱动的自适应视觉语言模型航路点规划器），用于通过无人机引导地面无人车（UGV）进行越野导航，旨在解决视觉语言模型（VLM）直接从航空图像生成路径时缺乏可靠性指标的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于视觉语言模型的越野导航路线生成方法直接输出预测结果，但无法提供可靠性指示，容易产生较大的规划误差。

Method: UDAV通过多次随机轨迹预测，选择其中心点（medoid）作为自一致的名义路线，并通过空间离散度估计预测不确定性；当最大不确定性超过阈值时触发重新审视阶段，否则直接返回中心点路径。

Result: 在400个留存轨迹查询上的评估表明，与确定性预测相比，UDAV将平均位移误差（ADE）从147.4像素降低至110.4像素（减少25.1%），并显著降低了90%和95%分位数的极端错误。

Conclusion: 研究证明，VLM的随机预测不仅能提供更强的名义路线，还能提供可操作的不确定性信号，从而有效选择性地减小规划大误差。
...
### [252] [Geometry vs Structure: Graph-Based Diagnostics for LiDAR Point-Cloud Simulation Fidelity](https://arxiv.org/abs/2609.16378)
*Ghazal Farhani,Taufiq Rahman*

Main category: cs.RO

TL;DR: 本文提出了一种基于图谱的框架，用于评估自动驾驶中模拟3D激光雷达点云相对于真实扫描件的结构保真度。


<details>
  <summary>Details</summary>
Motivation: 传统的几何指标（如倒角距离）在量化数字孪生中3D激光雷达点云的保真度时，往往会忽视空间连通性、拓扑结构和对象级组织等重要结构差异。

Method: 构建真实和模拟点云的图，利用Louvain社群检测识别空间相干子图并通过质心邻近度进行匹配，随后计算受外尔不等式启发的有界图谱度量（$r_λ$），并与密度感知倒角距离（CDC）进行对比。

Result: 受控扰动实验表明，$r_λ$对刚性变换具有不变性、对传感器噪声具有鲁棒性，同时对结构变形保持敏感；在真实与CARLA模拟扫描数据集上的评估表明，几何与结构测量捕捉了仿真保真度的互补方面。

Conclusion: 基于图谱的分析可以作为一种有效的补充诊断层，用于验证自动驾驶和高级驾驶辅助系统（ADAS）中数字孪生的结构保真度。
...
### [253] [Collision-Aware Humanoid Whole-Body Control under Imperfect Tracking Targets](https://arxiv.org/abs/2609.16405)
*Mohitvishnu S. Gadde,Ashish Malik,Pranay Dugar,Aayam Kumar Shrestha,Alan Fern*

Main category: cs.RO

TL;DR: 本文提出了RECAL（机器人-环境交叉注意力层），这是一个能够赋予人形机器人全身控制器（WBC）场景几何感知能力的方法，可在保持目标追踪性能的同时实现防碰撞控制。


<details>
  <summary>Details</summary>
Motivation: 现有的机器人全身控制器（WBC）通常无法感知场景几何结构，当感知、规划或遥操作出现误差导致目标动作不安全时，容易引发碰撞。

Method: 提出RECAL方法，将机器人、手持物体和环境表示为点云，利用机器人/物体点与环境之间的交叉注意力机制来生成具有几何感知的控制特征，从而在目标追踪与防碰撞之间进行权衡。

Result: 在仿真中，RECAL在固定手臂/自适应手臂运动、搬运物体和站立操作等多种场景下，相比其他具有几何感知的WBC架构，在保持目标追踪性能的同时显著提高了防碰撞能力，并在真实的Digit V3人形机器人上得到了验证。

Conclusion: RECAL成功通过交叉注意力机制增强了盲态WBC的几何感知能力，实现了安全且高效的人形机器人全身控制。
...
### [254] [XRoboToolKit-T: Teleoperation with High Stability and Precision with Tactile Sensing for Contact-rich Manipulation](https://arxiv.org/abs/2609.16437)
*Xiwen Dengxiong,Xueting Wang,Ke Jing,Rui Li,Yunbo Zhang*

Main category: cs.RO

TL;DR: 本文提出了一种名为XRoboToolKit-T的通用触觉辅助遥操作系统，旨在解决接触丰富操作中缺乏稳定、高频触觉反馈的问题，从而实现高效的高质量机器人数据收集。


<details>
  <summary>Details</summary>
Motivation: 现有的机器人数据收集方案通常缺乏获取稳定、高频触觉反馈的能力，这限制了它们在接触丰富的操作场景中的有效性。

Method: 引入了触觉驱动的辅助系统（XRoboToolKit-T），包含：1）触觉信息力控制架构；2）快速分析法向力分布并推断伪剪切力的稳定器触觉模块；3）结合视觉-语言-动作模型以根据触觉传感数据预测和精炼操作动作的精炼器触觉模块。

Result: 通过抓取可变形橡胶移液器进行液体转移和将医用注射器插入血管训练垫等具有挑战性的接触丰富任务，证明了该系统的有效性；与没有触觉辅助的最先进遥操作相比，该系统实现了更高的数据收集效率和改进的操作稳定性。

Conclusion: XRoboToolKit-T通过集成高频稳定的触觉反馈与力控制架构，有效提升了接触丰富任务中的遥操作性能和数据收集效率。
...
### [255] [ManiSkillFormer: Demonstration-Free Compositional Manipulation via Task-Conditioned Geometric Contracts](https://arxiv.org/abs/2609.16331)
*Peiqi Yu,Mosam Dabhi,Shangtao Li,Bowei Li,Laszlo Jeni,Changliu Liu*

Main category: cs.RO

TL;DR: ManiSkillFormer是一个神经符号学框架，通过任务条件几何契约实现无示范和组合式机器人操作。


<details>
  <summary>Details</summary>
Motivation: 克服端到端视觉运动策略对大量机器人示范和针对每个物体进行微调的依赖，实现跨物体和任务的可组合、可复用操作。

Method: 引入任务条件几何契约，明确结构化感知与动作的接口；利用大语言模型（LLM）代理生成几何契约和运动模板；通过感知模块从观测中基础化3D基元，并实例化技能库中的可复用运动模板。

Result: 在Galaxea R1-Lite双臂机器人上的实验表明，无示范抓取放下的平均成功率为88.24%，功能性操作平均成功率为75.00%，长周期任务完成率为50%-80%，均优于基线和消融管道。

Conclusion: ManiSkillFormer的设计成功实现了无需per-object策略微调或额外机器人示范的跨物体、跨任务可组合与可复用操作。
...
### [256] [The Neverwhere Visual Parkour Benchmark Suite](https://arxiv.org/abs/2609.16443)
...
TL;DR: 本文提出了The Neverwhere Benchmark Suite，一个包含六十多个基于3D高斯散射重建的超真实闭环视觉运动评估环境集合，旨在缩小真实世界部署前的训练/评估差距。


<details>
  <summary>Details</summary>
Motivation: 最先进的视觉运动控制器在复杂视觉环境中的能力日益增强，这使得部署前对其真实世界性能的评估变得极其困难，因此需要缩小训练与评估之间的差距。

Method: 开发了由60多个3D高斯散射重建（涵盖室内和室外场景）组成的超真实闭环评估环境基准，并测试了使用多个Neverwhere场景训练的策略在未见场景中的性能。

Result: 通过实验展示了仅依赖3D高斯生成数据进行训练的潜在缺陷，分析表明必须获取多样化的数据才能确保策略的泛化性能。

Conclusion: Neverwhere基准促进了大规模且可重复的机器人评估，同时强调了在视觉运动控制训练中源源不断获取多样化数据的重要性。
...
### [257] [Dense to MoE Adaptation for Compact Vision Language Action Policies](https://arxiv.org/abs/2609.16503)
*Muchun Niu,Shuang Chen,Yuzhou Wu,Linfeng Zhang*

Main category: cs.RO

TL;DR: 本文提出了AdaDE方法，通过将稠密前馈块转换为专家混合（MoE）层并动态停用专家，有效减小了视觉-语言-动作（VLA）模型的参数规模，同时保持了较高的任务性能。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作（VLA）策略的参数量不断增加，导致其在资源受限的机器人平台上部署困难，因此需要减少部署策略中保留的LLM端参数数量，同时保持下游任务的性能。

Method: 提出了AdaDE方法，将选定的稠密前馈块转换为MoE层，在微调期间从路由器统计信息中推导专家保留掩码；采用Dense2MoE转换在初始化时保留原始稠密FFN功能，并结合动态更新的专家掩码、分阶段训练和专家保护机制以避免早期崩溃。

Result: 在停用40%的LLM参数的情况下，AdaDE在LIBERO中保持了95.1%的平均成功率，在RobotWin2.0的所有50个任务中保持了42.0%的平均成功率。

Conclusion: 稠密到MoE的自适应转换结合动态专家停用，是减小活动VLA模型大小且不产生严重性能损失的一种实用方向。
...
### [258] [UniDex-ViTac: Learning Unified Visuo-Tactile Dexterous Manipulation Policy from Human Video Data](https://arxiv.org/abs/2609.16504)
*Hyesung Lee,Si-Hwan Heo,Sungwook Yang*

Main category: cs.RO

TL;DR: UniDex-ViTac是一个通过人类视频引导的仿真生成机器人演示和触觉测量，从而训练可部署的视觉-触觉灵巧操作策略的框架。


<details>
  <summary>Details</summary>
Motivation: 人类视频虽然提供了丰富的灵巧操作示范，但缺少机器人可执行的动作和触觉测量数据，限制了其直接用于机器人策略训练。

Method: 利用物体特定的残差强化学习专家，将人类与物体交互的参考动作适配到机器人手臂-手掌系统，生成配对的机器人动作与指尖接触观测数据，并训练基于ACT（Action Chunking with Transformers）的通用策略。

Result: 在仿真中，接触增强配置的宏平均成功率达到68.3%（基线为55.5%）；在真实机器人物理实验中，在无需真实机器人演示或策略微调的情况下，110次试验中成功73次（66.4%），较基线提高了11.8个百分点。

Conclusion: 该研究证明了从视频引导的模拟交互中学习统一的视觉-触觉灵巧操作策略是切实可行的。
...
### [259] [ProxiDex: Learning Dynamics-Guided Proximity Policy for Dexterous Manipulation](https://arxiv.org/abs/2609.16586)
*Yushan Bai,Boyu Zheng,Zhiyang Mao,Hongzheng Sun,Yuchuang Tong,En Li,Zhengtao Zhang*

Main category: cs.RO

TL;DR: ProxiDex是一个基于动力学引导的近距离策略框架，通过将手物距离转化为硬件无关的接触表示来解决多指灵巧操作中的部分可观测性和接触不确定性问题。


<details>
  <summary>Details</summary>
Motivation: 多指灵巧操作依赖于稳定的手物交互，但视觉遮挡和触觉传感器的局限导致交互状态部分可观测，现有策略在接触不确定性下表现脆弱。

Method: 提出了ProxiDex框架，利用交互点云重建计算几何距离作为近距离提示；构建了动作条件的近距离动力学模型（前向-逆向耦合设计），预测未来观测潜变量并解码近距离变化，从而自适应重新加权近距离token并指导策略推理。

Result: 仿真和真实世界实验表明，与代表性基线相比，ProxiDex在标准对象、未见对象以及扰动场景下均显著提高了操作成功率和鲁棒性。

Conclusion: ProxiDex通过对动作条件的近距离动力学进行建模和监督，有效克服了不可靠的视觉反馈，提升了灵巧操作在接触不确定性下的稳定性和鲁棒性。
...
### [260] [Learning to Optimize UAV Path Planning for Data Sensing in Wireless Sensor Networks](https://arxiv.org/abs/2609.16629)
*Sijie Ma,Zeyuan Ma,Weijia Cao,Yue-Jiao Gong,Lingling Ma,Zhiyang Huang,Jun Zhang*

Main category: cs.RO

TL;DR: 本文提出了一种名为Landscape-Aware Meta Differential Evolution (LAMDE)的新型学习辅助规划框架，用于解决无线传感器网络（WSN）中无人机（UAV）的数据采集路径规划问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖人工设计且存在设计偏差，难以适应未见任务；同时在简化仿真中忽略了真实环境的复杂空间特性，导致现实部署表现不佳。

Method: 提出一个双层学习优化框架（LAMDE）：元层面采用可训练的算法配置策略来元学习低级规划算法的自适应策略，并引入地形感知自动增强方案解决数据稀缺和分布偏移；底层采用差分进化算法配合变长编码策略，在统一搜索空间中动态剪枝冗余悬停点并优化连续飞行参数。

Result: 全面的实验表明，LAMDE在WSN数据收集场景的复杂UAV路径规划任务中取得了最先进（SOTA）的性能表现。

Conclusion: LAMDE框架有效克服了现有路径规划方法在适应性和真实世界部署方面的局限性，为复杂的UAV数据传感任务提供了一种高效且灵活的解决方案。
...
### [261] [SAVLA: Symmetry-Aware Vision-Language-Action Models for Robotic Manipulation](https://arxiv.org/abs/2609.16641)
*Junle Li,Weixian Waylon Li,Fuxiang Wu,Fusheng Hao,Fengxiang He*

Main category: cs.RO

TL;DR: SAVLA是一个端到端的对称感知视觉-语言-动作（VLA）模型，旨在解决现有VLA模型在机器人操作中空间泛化能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型仅通过示范学习空间能力，导致其可靠性受限于示范中的场景姿态范围，缺乏对几何和对称性的显式利用。

Method: 结合预训练的冻结视觉-语言主干、等变流匹配动作头和一个学习到的规范化器（canonicalizer），将输入分解为不变和等变通道，并将图像转换到规范帧中。

Result: 在LIBERO基准测试中，SAVLA将四个套件的平均成功率提升了5.1个百分点，在LIBERO-Goal的旋转测试下将平均成功率从41.5%大幅提升至90.4%。

Conclusion: 通过引入对称感知和等变建模，SAVLA显著增强了机器人操作策略的鲁棒性、数据效率以及对视角旋转的泛化能力。
...
### [262] [WholeBodyWAM: Generalizing Pre-trained World-Action Priors to Humanoid Loco-Manipulation via WBC-Grounded Coordination](https://arxiv.org/abs/2609.16644)
*Zhuo Li,Yiming Yao,Jim Tan,Mengjie Jing,Zhipeng Dong,Fei Chen*

Main category: cs.RO

TL;DR: WholeBodyWAM扩展了世界动作模型（WAMs），用于人形机器人的全身移动操作，通过结合视觉动态预测与异构全身控制器来实现泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的世界动作模型（WAMs）主要集中在桌面或手臂操作上，人形机器人的全身移动操作（loco-manipulation）研究较少，且缺乏对异构全身控制器（WBC）语义的有效接地和协调。

Method: 引入WholeBodyWAM，联合预测未来视觉动态、操作动作为及全身控制意图，在保留预训练世界动作先验的同时，对异构WBC语义进行结构化接地与行为协调。

Result: 在仿真中实现了91.9%的总体任务成功率，在真实世界的分布外任务进展上提升了0.23，并且相对于基线，不同WBC之间的成功率方差降低了70%。

Conclusion: 通过结构化的WBC接地和协调扩展预训练的世界动作先验，而不是从头重新学习全身行为，是实现可扩展人形全身智能的有效途径。
...
### [263] [Weave: Learning Whole-Body Dexterous Loco-Manipulation from Human-Object Interactions](https://arxiv.org/abs/2609.16683)
*Liu Cao,Xingze Wu,Jingzhi Cui,Botian Xu,Mingzhi Pei,Ruoqu Chen,Mengdi Xu*

Main category: cs.RO

TL;DR: Weave是一个统一的框架，用于通过学习人类演示来实现人形机器人与物体的全身灵巧交互。


<details>
  <summary>Details</summary>
Motivation: 将人类演示的协调交互行为转移到人形机器人身上，需要克服不同具身和动力学差异，学会建立和维持有效的接触。

Method: 通过接触感知重定向和接近运动补全将人类交互转换为机器人参考，并提出了一种联合控制全身及手指关节的接触与几何感知策略。

Result: 在9个物体上的评估显示，训练过的交互成功率达到92.5%，未见过的序列成功率达到65.0%，并发布了包含约9,000次物理执行轨迹的数据集。

Conclusion: Weave能够有效地从人类演示中学习全身灵巧的人形机器人-物体交互，具有良好的泛化能力，并为下游研究提供了有价值的数据资源。
...
### [264] [Differentiable Mesh State Estimation via Factor Graph Inference for Deformable Object Reconstruction](https://arxiv.org/abs/2609.16686)
...
TL;DR: 本文提出了一种基于因子图的新型概率网格状态估计框架，用于可变形物体的精确三维重建。


<details>
  <summary>Details</summary>
Motivation: 估计可变形物体的状态是机器人学和仿真中的一个基础难题，现有的物理和传感器融合方法在准确性和鲁棒性方面仍有提升空间。

Method: 直接更新四面体网格，并在统一的概率公式中结合物理先验、噪声传感器测量值以及时间平滑性约束，将估计问题转化为非线性最小二乘优化并用Levenberg-Marquardt算法求解。

Result: 离体中央气道阻塞实验和形变立方体模型仿真表明，该方法在刚体运动和形变情况下均能实现可靠且准确的重建。

Conclusion: 该基于概率的方法在可变形物体重建中展现出进行原则性、测量驱动的网格状态估计的巨大潜力。
...
### [265] [Fingers as Legs: Learning Self-Supported Locomotion and Manipulation with an Anthropomorphic Hand](https://arxiv.org/abs/2609.17172)
*Amirhossein Kazemipour,Hehui Zheng,Robert Katzschmann*

Main category: cs.RO

TL;DR: 本文展示了一种仿生灵巧手如何利用其手指同时实现自身移动、支撑重量以及环境交互，而无需单独的移动机构。


<details>
  <summary>Details</summary>
Motivation: 传统的机器人通常将移动机构与操作机构分离，而本研究旨在探讨如何让具有不均匀手指的拟人手通过复用手指来实现自主移动与多功能交互。

Method: 采用强化学习方法，在通过硬件测量校准的模拟器中进行训练，保持手指设计和位置控制器不变，并设计了新的奖励函数。

Result: 在仿真中，该方法比四足机器人的微调奖励移动速度更快；在硬件上，特定任务策略实现了无系绳爬行、转向、跌落恢复、盲打键盘以及视觉引导的目标物体推动。

Conclusion: 该研究成功证明了一个紧凑的移动操作器可以通过复用其手指来完成 locomotion 和交互，而无需配备独立的移动机制。
...
### [266] [IL-ACT: Imitation Learning with Adaptive Cartesian Tracking Control for a 30-ton Excavator](https://arxiv.org/abs/2609.16696)
*Mehdi Heydari Shahna,Seihun Kim,Soyi Jung,Soohyun Park,Jouni Mattila,Joongheon Kim*

Main category: cs.RO

TL;DR: 本文提出了IL-ACT（模仿学习与自适应笛卡尔追踪）框架，用于解决30吨级自主挖掘机在控制上面临的耦合运动学、驱动滞后和不确定性挑战。


<details>
  <summary>Details</summary>
Motivation: 自主挖掘机控制受耦合运动学、驱动滞后和不确定性影响大，传统方法难以兼顾高效性与精度，因此需要一种更优的运动控制框架。

Method: 结合了基于操作员示范预训练的14输入模仿策略、自适应笛卡尔反馈、门控增益/偏差估计，以及用于约束关节参考生成的停车距离调节器。

Result: Simscape评估表明，IL-ACT在所有目标测试中均以较短时间和较低的终端误差完成任务，显著降低了均方根误差（RMSE），优于基线Teacher+ACT、纯模仿学习（IL-only）和PID控制。

Conclusion: IL-ACT通过自适应状态和笛卡尔反馈实现了有界的控制性能提升，且在液压响应和传感条件下表现出卓越的鲁棒性和追踪精度。
...
### [267] [Port-Hamiltonian Koopman Operator Synthesis for Mechanical Systems](https://arxiv.org/abs/2609.17249)
*Rajpal Singh,Aditya Singh,Jishnu Keshavan*

Main category: cs.RO

TL;DR: 本文提出了一种基于广义动量坐标的保结构Koopman框架，用于学习非线性机器人系统（Euler-Lagrange系统）的精确且物理一致的预测控制模型。


<details>
  <summary>Details</summary>
Motivation: 纯粹从轨迹数据学习的有限维Koopman模型往往会违反底层力学系统的能量结构，导致预测出现人工能量增长并在递归传播下发散。

Method: 利用广义动量坐标将机械驱动暴露为状态无关的已知端口，开发结构约束的神经网络联合学习提升函数和端口哈密顿Koopman生成器，并通过Cayley中点离散化精确保持离散时间的存储-耗散平衡。

Result: 仿真和实验研究表明，与传统的Koopman基线相比，该方法在预测精度、数据效率和闭环轨迹跟踪方面均有显著提升，且对于高维系统的优势更为明显。

Conclusion: 该方法通过构造保证了所学习动力学的无源性（passivity）和稳定性，无需依赖惩罚项或事后投影，有效解决了传统Koopman模型能量发散的问题。
...
### [268] [World Models for Embodied Intelligence: From Plausible to Controllable to Actionable](https://arxiv.org/abs/2609.16697)
...
TL;DR: 本文提出一个新的层级框架和矩阵，用于评估具身智能中世界模型的预测能力，强调评估重点应从视觉保真度转向对行为和决策的实际改进。


<details>
  <summary>Details</summary>
Motivation: 现有文献多从架构、模态或应用领域对世界模型进行分类，忽视了“哪些预测能力能真正改善行为”这一核心问题。

Method: 引入三个渐进的强能力级别（合理模型、可控模型、可操作模型），并结合一个3x4的矩阵（交叉了几何、物理、动作接地与数据、奖励、策略、模型本身的改进循环）来对机器人学和具身智能领域的现有工作进行分类调研。

Result: 通过该框架梳理了操作、导航、运动、自动驾驶等领域的进展，指出了长程一致性、不确定性校准、因果干预测试等方面的挑战。

Conclusion: 世界模型的评估应当从单纯的视觉真实感转移到其能否捕捉任务相关状态、反映干预效果以及改善具身智能体的闭环行为上。
...
### [269] [Hamilton-Jacobi Reachability for Hybrid Systems: Unified Goal-Driven Control with Safety Guarantees](https://arxiv.org/abs/2609.17430)
*Javier Borquez,Shuang Peng,Somil Bansal*

Main category: cs.RO

TL;DR: 本文将经典Hamilton-Jacobi (HJ) 可达性分析扩展到了混合动力系统，用于处理接触丰富环境下的机器人安全与性能保证。


<details>
  <summary>Details</summary>
Motivation: 混合动力系统中连续动力学与离散模态转换的复杂耦合，使得在接触环境中确保系统的安全性和性能极具挑战性。

Method: 提出了一种通过在离散和连续状态上定义广义价值函数来表征混合系统安全集的框架，设计了数值计算算法，并开发了混合最小限制性安全滤波器与混合后向达-避管路以整合性能目标。

Result: 通过仿真研究和四足机器人的真实世界实验验证了该框架的有效性，证明其能够在混合模态规划和安全关键应用中实现连续和离散控制策略的综合。

Conclusion: 该研究成功扩展了HJ可达性分析在混合动态系统中的应用，能够有效保证机器人在复杂环境下的安全与任务完成。
...
### [270] [The Robot Data Factory](https://arxiv.org/abs/2609.16705)
...
TL;DR: 本文提出了机器人数据工厂（RDF），这是一个用于持续生成、验证和复用物理智能机器人经验的基础设施与方法论，旨在解决物理AI中对高质量、具身交互数据不断增长的需求。


<details>
  <summary>Details</summary>
Motivation: 物理AI的核心科学资源不是单纯的海量机器人静态数据，而是保留了“感知-动作-结果”循环的物理基础交互经验。现有方法缺乏系统化、闭环的经验生成基础设施。

Method: 引入机器人数据工厂（RDF），通过可复现的任务、技能课程、多模态传感、外部真值和智能体网络，实现“部署-测量-学习-重复”的闭环流程，并形式化了机器人经验的质量体系及数量缩放定律。

Result: 构建了一个连接机器人集群、数据管道、基准测试和具身AI集群需求的统一框架，并在家庭、环境和能源三个互补的物理训练场中进行了实例化应用。

Conclusion: RDF将机器人数据生成重新定义为一个持续的科学生产过程，为物理AI的具身基础设施提供了可复现、可扩展且可联合的发展路径。
...
### [271] [CorrRisk-WM: Corridor-Conditioned Risk World Modeling for Safety-Critical Trajectory Planning](https://arxiv.org/abs/2609.16724)
*Tingyu Guo,Reza Langari*

Main category: cs.RO

TL;DR: 本文提出了CorrRisk-WM，一个面向规划的部分世界模型，它将环境演化与候选轨迹特定的风险预测相结合，用于自动驾驶的安全局部规划。


<details>
  <summary>Details</summary>
Motivation: 安全的地方法划需要预测周围代理的运动并评估特定候选轨迹的风险，因为相同的代理运动对不同的自车轨迹会构成不同的风险。

Method: 利用潜在环境模型递归预测代理状态并更新交互，通过足迹感知几何和学习到的代理-走廊表示查询环境，并使用轻量级循环风险模块估计每个时间片的危险，聚合得到事件概率。

Result: 在Waymo验证集的29,176个场景中，CorrRisk-WM取得了0.8567的入侵平均精度（AP）和0.8671的1米近距离脱险首次进入AP，开环碰撞率最低（4.88%），路径进展为15.35米。

Conclusion: 将环境演化与候选条件几何推理相结合，对于风险预测和面向安全的候选轨迹选择具有显著效果。
...
### [272] [Seeing What Matters: Visual Cue Guided Video Planning for Generalizable Robot Navigation](https://arxiv.org/abs/2609.16737)
*Hojin Lee,Sizhe Lester Li,Maximilian Hilger,Susie Lu,Achim J. Lilienthal,Vincent Sitzmann,Daniel A. Duecker*

Main category: cs.RO

TL;DR: CueNav是一个基于生成式视频模型的机器人导航框架，通过结合视觉提示引导的视频规划与具身特定的逆动力学模型（IDM），实现了长视距规划和精确的视频到动作转换。


<details>
  <summary>Details</summary>
Motivation: 现有的基于生成式视频模型的机器人导航方法通常依赖短视距引导并通过场景重建恢复几何航点，这导致长视距规划和精确的视频-动作转换研究不足。

Method: 提出CueNav框架，使用鸟瞰图（BEV）地图作为全局任务上下文的视觉提示，并在第一人称观察中保留部分机器人身体以提供具身上下文；利用逆动力学模型（IDM）将从视频计划中提取的密集光流场转化为机器人动作。

Result: 在迷宫导航中，CueNav的成功率比无提示规划高出近2倍；在狭窄通道中，凭借身体感知视角和IDM实现了70%的成功率，而对比方法大多无法完成任务；同时展示了零样本语义条件导航和跨机器人平台的部署能力。

Conclusion: 视觉提示引导的视频规划与具身特定的动作接地为实现通用的长视距规划和具身感知控制导航框架铺平了道路。
...
### [273] [The Latent That Never Was: A Forensic Re-run of the CVAE Ablation in Action Chunking Transformer](https://arxiv.org/abs/2609.16745)
*Bo Kang*

Main category: cs.RO

TL;DR: 本研究重新评估了动作分块变压器（ACT）中变分自编码器编码器的必要性，发现先前报道的移除编码器导致成功率从35%暴跌至2%的结果无法复现，且编码器在训练和推理阶段对动作重建的实际贡献很小。


<details>
  <summary>Details</summary>
Motivation: 原ACT论文报告称移除条件变分自编码器的编码器会导致任务成功率急剧下降，研究人员希望验证这一发现在不同实现和训练数据下是否稳健，并探究编码器的实际作用。

Method: 使用原代码重新运行了消融实验，测试了不同的训练长度、检查点选择策略，并评估了潜变量（latent）在动作重建中的贡献以及对训练吞吐量的影响。

Result: 发表的成功率大跌结果未能重现；改变训练长度和检查点选择可以逆转策略优劣，但原暴跌原因不明；潜变量在测试中几乎没有提供重建优势，且推理时被设为零；跳过编码器可提高训练吞吐量。

Conclusion: 在测试的ACT基准中，编码器并非如原文献显示的那样不可或缺，其潜变量在推理时未被使用且重建收益很小，移除编码器还可以提升训练效率。
...
### [274] [Optimal Excitation Trajectories for System Identification of Underwater Vehicles](https://arxiv.org/abs/2609.16786)
*Fotis Panetsos,Kostas J. Kyriakopoulos*

Main category: cs.RO

TL;DR: 本研究提出了一种通过设计最优激励轨迹来进行水下航行器系统辨识的结构化方法。


<details>
  <summary>Details</summary>
Motivation: 为了实现水下航行器动力学参数的可靠估计，需要解决如何设计能够满足安全限制并最大化采集数据质量的高效激励轨迹的问题。

Method: 使用贝塞尔曲线对轨迹进行参数化以确保平滑度，通过优化问题求解满足动力学可行性和安全限制的最优激励轨迹，并利用最小二乘法进行参数估计。

Result: 通过实验室水箱的实验验证，利用优化轨迹辨识出的动力学参数能够通过前向仿真准确预测未见轨迹下的航行器速度。

Conclusion: 该方法能够有效且可靠地完成水下航行器的系统辨识与动力学参数估计。
...
### [275] [Motion planning in high dimensional spaces hybridizing RRT and HAR via position-direction decoupling](https://arxiv.org/abs/2609.16810)
*Frederic Cazals,Nelson Feyeux*

Main category: cs.RO

TL;DR: 本文提出了一种结合快速扩展随机树（RRT）与Hit-and-Run（HAR）随机游走的新型采样路径规划方法，用于高效解决高维度空间及狭窄通道中的路径规划问题。


<details>
  <summary>Details</summary>
Motivation: 高维度空间中的路径规划（特别是存在狭窄通道和小间隙时）极具挑战性，传统方法在处理复杂多机器人系统或高自由度系统时效率低下。

Method: 通过解耦扩展点与扩展方向，将RRT与HAR相结合，提出通用的算法框架，并引入仅移动一小部分机器人的稀疏移动策略来处理拥挤场景。

Result: 实验表明，该算法在标准笔记本上能在几秒钟内解决高达64个机器人和384个自由度的复杂问题，其中HARF方法比传统RRT快可达两个数量级。

Conclusion: 所提出的HARF方法是解决复杂多机器人及高维路径规划问题的首选方法，表现出极高的效率和处理狭窄通道的能力。
...
### [276] [Rethinking Visual Embodiment Dependence in Visuomotor Policies](https://arxiv.org/abs/2609.16815)
...
TL;DR: 本论文研究了视觉运动策略中的“视觉具身依赖（VED）”现象，提出通过3D点云中的具身归一化（使用规范末端执行器表示CER）来结构化具身信息，以改善机器人策略的泛化能力和跨具身迁移。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉运动策略容易将可见的机器人构型作为任务进展的捷径，而完全移除具身信息又会丢失控制所需的几何结构，因此需要研究如何科学地结构化和利用视觉具身信息。

Method: 提出具身归一化方法，在3D点云中用规范末端执行器表示（CER）替换原始具身形态，并引入配置解耦增强技术（configuration-decorrelation augmentation）以应对不熟悉的机器人配置。

Result: 实验表明，具身归一化在没有机器人示范的情况下大幅提升了人到机器人的策略迁移能力，且配置解耦增强成功缓解了失败模式，恢复了鲁棒的恢复能力。

Conclusion: 鲁棒的视觉运动学习得益于对视觉具身信息的结构化利用（而非简单移除），通过保留控制相关几何结构并进行规范化处理，可以显著提升策略的泛化性和迁移性能。
...
### [277] [TEMPO: Learning Temporal Context for Dynamic Robot Manipulation](https://arxiv.org/abs/2609.16864)
*Zhenyang Feng,Jimin Heo,Erik B. Sudderth,Unnat Jain*

Main category: cs.RO

TL;DR: 本文指出了视觉-语言-动作（VLA）模型在动态操作任务中的局限性，并提出了TEMPO方法，通过引入时间上下文来解决运动模糊和状态混淆问题。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在准静态操作中表现出色，但在依赖单帧观察的动态操作任务中表现不佳，这主要是由于缺乏时间上下文导致的运动模糊和状态混淆。

Method: 提出了TEMPO方法，为预训练的VLA模型增加两个时间输入：一个是从冻结的视频基础模型中提取的运动摘要（解决运动模糊），以及一个紧凑的本体感受历史（解决状态混淆）。

Result: 在四个动态操作任务中，TEMPO将瓶子交接的成功率从44%提升至74%，并且是唯一能够解决状态混淆的方法。同时推出了包含超过5万个标注帧的评估基准TEMPO-Bench。

Conclusion: 缺少时间上下文是限制VLA模型处理动态操作的根本瓶颈，而TEMPO通过补充运动摘要和本体感受历史，能够有效且高效地解决这一问题，无需修改骨干网络。
...
### [278] [Artificial Intelligence-Enabled Space Robot Operations: Technologies, Challenges and Prospects](https://arxiv.org/abs/2609.16880)
*Zeyuan Huang,Gang Chen,Zixuan Hao,Guoqin Tang,Junyi Zong,Guoyou Ban,Jiale Wang,Haoyang Lv,Chaoqian Ren,Sitong Liu*

Main category: cs.RO

TL;DR: 本文从能力建设的角度，综述了人工智能使能的空间机器人操作（AI-SRO）最新进展，提出了三层技术框架，并展望了未来关键研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着空间机器人需要执行更长时间、富含接触和多阶段的操作且减少人工干预，虽然人工智能和具身基础模型带来了新机遇，但其向太空的迁移受到数据稀缺、特殊动力学与感知条件、有限机载资源及严格安全要求的限制。

Method: 建立了一个包含能力基础、能力形成、能力部署与演化的三层技术框架，系统回顾了仿真环境、数据集、感知、决策、规划、执行、机载部署、地面-太空适应及持续学习等方面。

Result: 梳理了AI在空间机器人操作中的应用现状与挑战，涵盖从任务理解到动作执行全流程，并提出了面向未来的关键研究方向。

Conclusion: 未来空间机器人研究需要重点关注可信仿真与数据、开放世界多模态认知、长历程安全决策、物理约束策略学习以及空间计算基础设施。
...
### [279] [Waggle Dance Inspired Motion Communication for Multiple UAVs in MuJoCo](https://arxiv.org/abs/2609.16958)
*Zhang Nengbo*

Main category: cs.RO

TL;DR: 本文提出了一种受蜜蜂摇摆舞启发的多智能体运动通信系统，实现了一个表演者向多个观察者广播空间导航信息并触发自主导航的功能。


<details>
  <summary>Details</summary>
Motivation: 将MoCom的点对点运动通信设置扩展到一个表演者和多个观察者，探索受生物学启发的多智能体广播到动作机制的可行性与局限性。

Method: 利用MuJoCo仿真系统，表演者通过四种飞行基元和显式空信号广播六位导航有效载荷；多个观察者通过机载RGB图像提取光流轨迹、识别符号、解析消息并触发导航。

Result: 在标准测试套件下，17次标称广播中产生了44条正确的完整消息接收，13次广播通过了所有群组级解码和执行检查，但不成功时接收器保持静止。

Conclusion: 有限的仿真结果支持了在测试条件下受蜜蜂摇摆舞启发的广播到动作机制的可行性，并指出了当前的感知和协议局限性。
...
### [280] [Overcoming technical adoption barriers for mobile service robots in rehabilitation](https://arxiv.org/abs/2609.16996)
*Christian Sternitzke,Sebastian Blumenthal,Lukas Kleedoerfer,Verena Deserno,Anke Mayfarth*

Main category: cs.RO

TL;DR: 本文探讨了医疗移动服务机器人如何通过增加外围功能和小幅改进，从低技术成熟度提升至获批医疗设备，从而赢得临床环境中的用户接受度。


<details>
  <summary>Details</summary>
Motivation: 许多医疗机器人文献停留在低技术成熟度阶段，而临床环境中的用户接受度往往无法仅通过事前询问获得，需要通过实际测试来发现。

Method: 通过在临床设置或操作环境中实际测试医疗服务机器人，识别并添加关键的外围功能和小幅改进。

Result: 研究表明，下游开发（如外围功能的改进）对于获得用户接受度和实现机器人系统的广泛采用至关重要，但这需要投入大量的实施时间。

Conclusion: 医疗服务机器人的开发者在规划时应将这些下游开发考虑在内，这对于提升技术成熟度和实现临床广泛应用是不可或缺的。
...
### [281] [SWIM: Vision-Language-Grounded Soft Whole-Body Interactive Manipulation](https://arxiv.org/abs/2609.17035)
*Tingcong Liu,Aye Phyu Phyu Aung,Junjie Xiong,Siyi Ma,Bo An,Ke Wu,Senthilnath Jayavelu*

Main category: cs.RO

TL;DR: 本文提出了SWIM框架，通过视觉-语言-动作（VLA）策略和视觉软本体感觉（VSP），实现了将语言指令和视觉观测转化为软体连续体机器人的完整全向驱动命令序列。


<details>
  <summary>Details</summary>
Motivation: 软体和连续体机器人通过分布式身体变形和接触进行操作，但将语言和视觉上下文转化为可执行的全身驱动仍然是一个基本挑战。

Method: 引入SWIM框架及其核心SWIM-VLA策略，结合扩散动作头和视觉软本体感觉（VSP），利用模拟生成的演化观测进行迭代虚拟 rollout 生成驱动命令，并借助固有柔顺性进行局部接触适应。

Result: 在打包、到达和抓取任务中，SWIM-VLA在仿真中的成功率分别达到100%、96%和88%，在真实硬件上的成功率分别达到100%、80%和75%，均显著优于基线方法和直接在线部署。

Conclusion: SWIM框架通过结合多模态表示、扩散动作头和虚拟 rollout，有效解决了软体机器人从语言和视觉输入到复杂全身驱动控制的翻译难题，并在仿真和真实硬件中展现出优越的性能。
...
### [282] [BRAVE-6D: Benchmark for Robotic Active Vision in 6DOF Pose Estimation](https://arxiv.org/abs/2609.17106)
*Philipp Ausserlechner,Bernhard Neuberger,Alessandro Scherl,Michael Schebek,Stefan Thalhammer,Markus Vincze*

Main category: cs.RO

TL;DR: 本文介绍了BRAVE-6D，这是一个基于3D高斯溅射（3DGS）视点合成的基准测试，用于评估机器人主动视觉系统在小物体位姿估计方面的性能。


<details>
  <summary>Details</summary>
Motivation: 检测和抓取小物体是机器人技术的重大挑战，主动视觉是一种直观的解决方案，但由于需要相同的物理场景设置，在共同基础上比较不同方法非常困难。

Method: 利用基于高斯溅射（3DGS）的视点合成技术构建场景，并提供用于对主动视觉系统进行基准测试的工具，同时展示了在场景中执行视觉伺服并准确估计小物体位姿的基线解决方案。

Result: 成功开发了BRAVE-6D基准测试，并验证了基线解决方案能够在场景中进行视觉伺服并准确估计小物体的位姿。

Conclusion: BRAVE-6D有效解决了机器人主动视觉系统缺乏统一测试平台的问题，为评估小物体位姿估计和抓取提供了可靠的基于视点合成的基准与工具。
...
### [283] [Intrinsic Robot Rewarding: Reusing VLA Representations for Autonomous Evaluation and Policy Improvement](https://arxiv.org/abs/2609.17115)
*Tobias Schaffer,Mohab Elkhayat,Daniela Nicklas,Mustafa Almohamad,Elham Al-Fuqara*

Main category: cs.RO

TL;DR: 本文提出了内在机器人奖励（IRR）系统，利用视觉语言动作（VLA）系统中的视觉表征和演示数据来评估机器人自身的执行结果并提供策略改进反馈。


<details>
  <summary>Details</summary>
Motivation: 利用现有的视觉表征和成功演示数据，为机器人提供一种低集成成本、高效计算且减少人类干预的结果评估与奖励机制。

Method: 引入参考库和评分操作，直接利用VLA系统中冻结的视觉编码器评估新结果，无需单独学习评估器或额外的感知骨干网络。

Result: 通过COMAU Racer 3验证器（TRL 4）建立了实验室基础，并提出了将内部结果评估连接到物理策略改进的奖励公式、核心研究问题及评估方法。

Conclusion: IRR提供了一种可重用的方法，能够利用工业机器人系统中现有的数据和经验进行学习和改进，降低了集成复杂度和人类监督成本。
...
### [284] [LOTUSim-Energy: A Maritime Simulator for Human-Drone Interaction in Autonomous Offshore Operation \&amp; Maintenance](https://arxiv.org/abs/2609.17124)
*Juliette Grosset,Marie Dubromel,Hélène Lechêne,Quentin Arzel,Cédric Buche*

Main category: cs.RO

TL;DR: LOTUSim-Energy是一个专为海上运维设计的多域人机交互实时模拟器，整合了异构无人载具、环境物理场及沉浸式交互界面。


<details>
  <summary>Details</summary>
Motivation: 海上运维涉及空中、水面和水下多域作业且依赖人工监督，需要一个统一的集成测试平台来对自主技术栈和人机协作工作流进行可重复评估，从而降低海上部署的风险。

Method: 开发了一个分布式架构的实时模拟器，统一了UAV、USV、AUV和ROV等异构无人载具，耦合了风浪流等环境受力，提供桌面和VR沉浸式用户界面，并包含结构化海上任务库、真实物理仿真、能量感知电池建模及故障检测通道。

Result: 通过单桩及过渡段结构的跨域巡检场景验证了系统级性能，评估了集成航点跟随插件和AIS参考轨迹跟踪在实时能量监测下的可靠性。

Conclusion: LOTUSim-Energy通过结合统一的环境物理学、异构载具仿真和沉浸式监督，为海上人机协作工作流的原型设计和演练提供了有效的集成测试平台。
...
### [285] [Continual Learning for Traversability Prediction with Uncertainty-Aware Adaptation](https://arxiv.org/abs/2609.17141)
*Hojin Lee,Yunho Lee,Daniel A Duecker,Cheolhyeon Kwon*

Main category: cs.RO

TL;DR: 本文提出了一种用于机器人自主导航中地形可通行性预测的持续学习框架，通过生成式经验回忆模型来适应新地形并防止灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的可通行性预测方法在适应新地形时往往表现不佳，且在适应过程中容易出现灾难性遗忘，难以保留先前训练环境中的经验。

Method: 提出一个持续学习框架，利用生成式经验回忆模型增量适应新地形，无需存储历史数据，并结合生成样本的不确定性来实现不确定性感知适应。

Result: 在滑移转向机器人上的真实世界实验验证了该框架的有效性，证明了其在多种多样环境中进行适应并减轻灾难性遗忘的能力。

Conclusion: 该框架能够有效地在非结构化环境中进行持续的地形可通行性预测，解决了适应新地形与保留旧经验之间的冲突。
...
### [286] [LiLi: Lie Theory Based 3D LiDAR Scan Alignment Degeneracy Detection](https://arxiv.org/abs/2609.17145)
*Vsevolod Hulchuk,Jan Bayer,Jan Faigl*

Main category: cs.RO

TL;DR: 本文提出了一种名为LiLi的新方法，利用李群和李代数检测3D LiDAR扫描对齐中的退化现象，显著提高了复杂环境下的定位与建图精度。


<details>
  <summary>Details</summary>
Motivation: 现有的退化检测方法忽略了数据点重新关联的可能性，对噪声和复杂退化敏感，在直线走廊或平坦场地等退化场景中容易导致定位和建图精度下降。

Method: 提出LiLi方法，利用李群SE(3)和李代数se(3)的生成元系统性地描述一组退化变换，通过对优化解施加扰动并比较优化后的位姿来稳健地检测退化。

Result: 在合成数据上，相比最新的Hessian方法，对齐误差减少了50%；在真实世界的退化数据集（如260米长轨迹和430米长隧道往返轨迹）中，表现出优于参考方案的定位性能。

Conclusion: LiLi方法能够有效、稳健地识别SE(3)中的全套退化变换，显著提升LiDAR里程计在退化场景下的定位与建图鲁棒性。
...
### [287] [Kernel-Based Metrics Learning for Uncertain Opponent Vehicle Trajectory Prediction in Autonomous Racing](https://arxiv.org/abs/2609.17147)
*Hojin Lee,Youngim Nam,Sanghun Lee,Cheolhyeon Kwon*

Main category: cs.RO

TL;DR: 本研究提出了一种基于深度核学习（DKL）和异构核度量的方法，用于自主赛车中对具有未知行驶策略的对手车辆进行精确的轨迹预测和不确定性估计，从而实现安全超车。


<details>
  <summary>Details</summary>
Motivation: 自主赛车在安全超车时面临巨大挑战，对手车辆由于驾驶策略未知，其轨迹具有不确定性，现有方法难以准确捕捉和预测这些多样化的驾驶行为。

Method: 引入用于深度内核学习的异构内核度量，在无监督的情况下根据自车与对手车辆的交互，对相似的驾驶策略进行对齐、对不同的策略进行分离，从而进行精确的轨迹与不确定性预测。

Result: 在1/10比例的赛车平台上进行的实验表明，该方法提高了预测准确性，能够安全超车，并且计算效率高，适合车载计算单元。

Conclusion: 该研究所提出的异构核度量深度核学习方法能够有效且高效地预测未知策略对手车辆的轨迹，适用于快节奏的自主赛车环境。
...
### [288] [HuMemSLAM: Efficient Human-Inspired Semantic Place Recognition for Robust Visual SLAM](https://arxiv.org/abs/2609.17168)
*Mayowa Adebambo,Sebastian Donnelly,Armand Amaritei,Andrew Bradley,Alexander Rast*

Main category: cs.RO

TL;DR: 本文提出了HuMem-VPR和HuMemSLAM，利用人类记忆启发的自底向上感知证据与自顶向下上下文推理的双向关系，实现了高精度、低延迟的视觉位置识别与SLAM系统。


<details>
  <summary>Details</summary>
Motivation: 传统的几何视觉SLAM易受感知混淆和感知变化的影响，而现有的语义SLAM和学习型VPR方法在恶劣感知条件下难以同时兼顾高检索精度和低延迟。

Method: 提出HuMem-VPR，通过结合自底向上的感知证据与自顶向下的上下文推理实现位置理解；并将其与ORB-SLAM3集成，开发出HuMemSLAM系统。

Result: HuMem-VPR在真实图像基准测试中实现了最高的聚合检索精度，在CARLA基准测试中表现出竞争力的精度，且延迟比现有先进VPR方法低2到3倍；HuMemSLAM显著提升了Recall @1并减少了几何后端提交的提议数量。

Conclusion: 仿生人类记忆机制的HuMem-VPR和HuMemSLAM能够有效克服传统视觉SLAM的感知局限，在保证低延迟的同时大幅提升位置识别的准确性与鲁棒性。
...
### [289] [Fleet-To-Lab: A Transfer Learning Framework For Lunar Rover Slippage Estimation Via Model Fusion](https://arxiv.org/abs/2609.17187)
*Riccardo Viviano,Saki Omi,Andrej Orsula,Miguel Olivares-Mendez*

Main category: cs.RO

TL;DR: 本文提出了Fleet-to-Lab迁移学习框架和AcoMerge混合群体智能算法，利用历史月球车数据解决月球表面车轮滑移估计中的地球-月球域差异和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在地面数据上训练后难以泛化到月球地形，且真实的月球数据集由于任务稀少和获取成本高而极度匮乏，因此需要有效的方法来估计月球车的车轮滑移。

Method: 提出Fleet-to-Lab框架，通过融合多个异构专家模型，并利用部署后收集的适量数据集；同时提出AcoMerge混合群体智能算法，通过搜索专家参数的最佳组合来执行模型融合。

Result: 在高保真物理模拟中的实验表明，与深度模型融合基线相比，该方法在平衡准确率和宏平均F1分数上均有所提升，且在较小的模型规模下达到了与联合训练相当的竞争力。

Conclusion: 模型融合被证明是解决空间机器人任务中数据有限情况下滑移估计问题的一种可行的迁移学习替代方案。
...
### [290] [TIO-Former: Ultra-Lightweight 6-Directional ToF-Inertial Odometry for Nano-UAVs via a Streaming Causal Transformer](https://arxiv.org/abs/2609.17198)
*Yang Liu,Yifan He,Wenhao Zhao,Xiangyu Mo,Yang Xu,Hao Wei,Mingze Ma,Huan Li,Yifan Wu,Zipeng Dai,Xin Zhou,Fei Gao*

Main category: cs.RO

TL;DR: 本文提出了TIO-FORMER，一个专为微型无人机（nano-UAV）设计的无相机、无光流、无地图的测距惯性里程计框架，通过结合超轻量级ToF阵列与IMU实现高精度的6自由度自车运动估计。


<details>
  <summary>Details</summary>
Motivation: 微型无人机受严格的尺寸、重量、功耗和计算（SWaP-C）限制，传统视觉传感器和LiDAR超出负载极限，光流法在低纹理场景下性能退化，纯惯性估计则易产生累积漂移。利用多区飞行时间（ToF）阵列作为轻量级补充面临着无效回波、各向异性可观性和计算扩展等挑战。

Method: 提出TIO-FORMER框架，前端采用双边门控差分将连续测距网格配对，结合IMU引导的交叉注意力机制根据平台运动学动态路由方向特征；引入流式因果Transformer（Streaming Causal Transformer），耦合未压缩的本地KV缓存与压缩的Chunk-FIFO内存，以维持有界的推理成本和内存占用。

Result: 在真实飞行评估中，TIO-FORMER将开环位置误差较纳米无人机光流法降低了54.4%，较学习型惯性基线降低了66.4%-89.1%。部署在边缘RISC-V伴随计算机上时，其P95延迟为10.466毫秒，峰值常驻内存为6.324 MiB（占系统RAM不到5%）。

Conclusion: 稀疏测距传感为资源受限的微型飞行机器人提供了一种实用且高效的几何锚定方案，能够在严格的SWaP-C约束下实现准确的自车运动估计。
...
### [291] [FluxVLA Engine: A One-Stop VLA Engineering Platform for Embodied Intelligence](https://arxiv.org/abs/2609.17210)
...
TL;DR: FluxVLA Engine是一个开源的、基于配置的平台，旨在将异构的具身智能策略组件整合为可复现的数据到部署的工作流。


<details>
  <summary>Details</summary>
Motivation: 将VLA模型、WAMs和离线强化学习等前沿算法转化为可靠的机器人系统时，受限于碎片化的数据格式、训练栈、评估协议、推理运行时和特定具身接口。

Method: 提供统一的数据集接口、视觉语言和世界模型、动作头、分布式训练、仿真评估、优化推理以及机器人操作器接口，并集成了双臂仿真、自动数据生成、人机协同回滚、纠错收集和实时分块（RTC）加速推理等功能。

Result: 成功连接了离线学习、仿真验证、在线纠错和真实机器人执行，解决了阻碍具身学习算法走向可复现评估和可靠部署的工程瓶颈。

Conclusion: FluxVLA Engine通过标准化接口和全流程集成，实现了具身智能策略从算法到可靠部署的无缝衔接。
...
### [292] [Swim-and-Breach at Palm Scale: A Rudder-Steered Two-Propeller Underwater Robot Platform with Differential-Thrust Pitch Control](https://arxiv.org/abs/2609.17240)
*Daehyun Choi,Ian Bergerson,Hengjia Zhu,Tianjun Lan,Saad Bhamla*

Main category: cs.RO

TL;DR: 本文提出了一种手掌大小的“游泳-跃出水面”机器人平台，能够实现高速游动并在水面进行无缝跳跃。


<details>
  <summary>Details</summary>
Motivation: 为了开发能够在水流、受灾建筑和工业系统中穿过水障及干燥缝隙的小型机器人。

Method: 采用双层垂直堆叠螺旋桨提供推进和俯仰控制，尾舵提供偏航控制；通过流体仿真优化船体以减少阻力，并利用B系列建模优化螺旋桨。

Result: 该机器人游泳速度达到每秒13.9个体长，转向速率为每秒209度，并能实现高1.6个体长、长3.7个体长的跨越跳跃。

Conclusion: 该平台具备极高的机动性能，可广泛应用于各类水域环境和工业系统的狭窄区域检测任务。
...
### [293] [DriveMCP: An Agentic AI framework for Advanced Driver Assistance System](https://arxiv.org/abs/2609.17247)
*Farzad Nadiri,Mehdi Cina,Ahmad B. Rad*

Main category: cs.RO

TL;DR: 本论文提出了DriveMCP，一个模块化且可审计的智能AI驾驶员辅助框架，通过整合感知、合规推理、车辆状态解释和安全仲裁来提升自动驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（VLM）直接应用于驾驶时缺乏对交通法规的合规推理、车辆状态感知以及严格的安全仲裁机制，容易导致违章和安全隐患。

Method: 引入DriveMCP架构，结合类传感器感知栈与DriveLM作为视觉语言前端进行图结构场景理解；利用有状态的编排层协调专有Model Context Protocol（MCP）服务器（规则、天气、CAN总线服务器）进行多维度推理；并采用受责任敏感安全（RSS）启发的安全护栏进行决策仲裁。

Result: 在CARLA模拟器进行的跨语言、跨境和动态限速场景测试中，DriveMCP相比VLM-Direct、VLM-Direct+RAG和VLM-Tools-NoArbiter等基线，减少了交通违章和超速现象，改善了危险响应时间，并保持了亚秒级的建议延迟。

Conclusion: DriveMCP框架通过模块化管道、合规推理服务器和安全仲裁机制，有效提升了自动驾驶系统在复杂多变场景下的安全性和合规性。
...
### [294] [CAD-Based Relation Learning and Geometric-Symbolic Planning for Robotic Assembly](https://arxiv.org/abs/2609.17263)
*Fabian Harlacher,Christian Friedrich*

Main category: cs.RO

TL;DR: 本文提出了一种结合学习型关系提取与几何符号推理的混合装配序列规划（ASP）框架，用于从不完美CAD数据中高效生成可行的机器人拆卸序列。


<details>
  <summary>Details</summary>
Motivation: 装配序列规划具有组合复杂性，且许多CAD模型缺乏可靠的语义接触信息或需要大量手动预处理，限制了现有方法的应用。

Method: 利用神经网络从点云中预测语义几何关系，辅以人机协同验证来修正不确定预测；将提取的关系转化为符号装配图，通过基于可见性的射线投射策略和几何符号规划器计算局部有效的机器人操作基元。

Result: 在ASAP测试数据集上，该规划器实现了85.83%的规划成功率，同时将中位规划时间比基线减少了一个多数量级（对于30个以上组件的装配体减少了50倍以上）。

Conclusion: 该混合框架能够从不完美CAD数据中实现高效的机器人装配序列规划，在大幅减少规划时间的同时，为可扩展且自适应的机器人装配和拆卸规划提供了实用的基础。
...
### [295] [Online Geometric Change Detection via Scene Decomposition](https://arxiv.org/abs/2609.17302)
*David Thorne,Samuel Jia Cong Chua,Nakul Joshi,Aiden Wong,Christa S. Robison,Philip Osteen,Brett T. Lopez*

Main category: cs.RO

TL;DR: 本文提出了一种名为Change Detection via Scene Decomposition (CDSD)的框架，用于利用LiDAR或RGB-D传感器在动态环境中进行高效、准确的在线几何变化检测。


<details>
  <summary>Details</summary>
Motivation: 自主机器人在长航时任务中需要识别环境变化（如倒下的树木或打开的门）以为在线规划提供上下文信息，但传统的全局地图比较方法计算成本高且不支持单会话的在线变化检测。

Method: 提出基于子地图的几何变化检测方法（CDSD），将映射环境空间分解为唯一场景，通过比较密集且局部的子地图来高效发现变化，并解决了场景识别、子地图生成、处理不同视场变化及实时地图重建等核心挑战。

Result: 在陆军研究实验室（ARL）收集的自定义数据集以及开源的多会话变化检测数据集上验证了该算法的有效性。

Conclusion: CDSD框架能够有效实现自主机器人在动态环境中的实时、高效几何变化检测，解决了全局地图比对计算昂贵的问题。
...
### [296] [Escape-Aware Control Barrier Functions for Quadrotor Safety under Body-Rate Limits](https://arxiv.org/abs/2609.17292)
*Lei Shi,Haosong Wen,Qichao Liu*

Main category: cs.RO

TL;DR: 本文研究并解决输入受限系统（如四旋翼飞行器）中控制障碍函数（CBF）的认证间隙问题，提出了一种基于状态与先前应用输入的逃逸障碍函数（escape barrier）。


<details>
  <summary>Details</summary>
Motivation: 传统的仅依赖状态的控制障碍函数在处理具有输入约束和动力学限制（如四旋翼的推力重定向和机体速率限制）的系统时，会导致“认证间隙”，即证书覆盖了无法及时逃逸的不安全状态，从而引发控制失效。

Method: 推导了闭式认证间隙并分析其特性，随后在增广的状态和先前输入对上定义了一个逃逸障碍函数，利用一步可达推力上限衡量逃逸权限，并将其嵌入预测控制器中。

Result: 在13状态四旋翼的550次闭环测试中，所提出的控制器在所有测试场景下均成功运行，而传统停止距离障碍函数在进入预测间隙的场景下失败了15%到25%；同时，相比在线备份CBF基线，该方法提供了更大的方向余量和间隙，且保守滚动裁判证实所有认证状态均能成功逃逸。

Conclusion: 通过将先前应用的输入纳入障碍函数设计，可以有效消除四旋翼等输入受限系统中的控制认证间隙，显著提升控制器的安全性和鲁棒性。
...
### [297] [XPACE: Joint World and Action Modeling from Heterogeneous Experience](https://arxiv.org/abs/2609.17372)
...
TL;DR: 本文介绍了XPACE，一个统一的具身世界模型，它结合了世界动作用模型和世界模拟器，利用异构经验和自举仿真来提升机器人的动作执行和策略改进能力。


<details>
  <summary>Details</summary>
Motivation: 通用机器人需要利用多样化的经验来选择动作并预测环境变化，然而如何有效整合异构数据（如无动作视频、人类示范和机器人示范）并用于策略提升仍是一大挑战。

Method: 提出了XPACE架构，使用共享的视频骨干网络联合预测动作和未来视频；采用由粗到精的训练课程来整合人类与机器人经验；并利用世界模拟器生成偏差恢复轨迹，对策略进行微调。

Result: 在小鹏IRON人形机器人上的实验表明，异构训练提高了鲁棒性，并能将人类观察到的技能迁移到机器人未示范的任务中，而模拟器生成的恢复数据进一步提升了真实世界的任务完成率。

Conclusion: XPACE证明了联合世界与动作建模能够有效连接从异构经验中的学习与模拟驱动的策略自我改进，从而增强机器人的泛化和执行能力。
...
### [298] [Exact Fusion and Coordinated Exploration in Multi-Robot Active Inference](https://arxiv.org/abs/2609.17384)
*Peng Wu,Mohsen Imani,Amidu Kamara,Md Tamzeed Islam,Seyede Fatemeh Ghoreishi,Mahdi Imani*

Main category: cs.RO

TL;DR: 本文提出了一种多机器人团队协作学习与规划的修正方法，解决了共轭指数族信念下由于重复计算公共先验和信息而导致的冗余问题，并通过证据增量和序列承诺保证了贪婪算法的性能与线性扩展性。


<details>
  <summary>Details</summary>
Motivation: 在机器人团队共享环境模型并基于期望信息增益进行规划时，共轭指数族信念会导致公共先验在融合和规划阶段被重复计算，产生冗余和偏差，因此需要消除这些错误。

Method: 通过向共享自然参数添加证据增量（融合时为实现增量，规划时为期望增量）来消除双重错误；利用期望增量计算条件增益，并通过顺序承诺保持贪婪保证，针对有限假设类采用精确枚举替代。

Result: 实验表明，融合修正保持了探索冗余不变，预期证据消除了冗余，且顺序承诺以线性于团队规模的成本恢复了集中式联合规划的大部分价值。

Conclusion: 该方法有效解决了多机器人分布式信念融合与规划中的信息重复计算问题，在理论上保证了贪婪算法的性能，并在多种协作任务中验证了其高效性。
...
### [299] [Residual Fault Adaptation for Dexterous In-Hand Manipulation Under Runtime Joint Faults](https://arxiv.org/abs/2609.17404)
*Linan Deng,Xing Liu,Lin Hong,Feng Hua,Guijun Ma,Zuogong Yue,Fumin Zhang*

Main category: cs.RO

TL;DR: 本文提出了残余故障自适应（RFA）框架，用于解决灵巧手在运行过程中突发关节故障时的协调控制问题，实现无需故障标签的零样本（zero-shot）故障补偿。


<details>
  <summary>Details</summary>
Motivation: 灵巧手操作需要多个关节的协调控制，运行时的关节故障会破坏成功操作所需的接触配置，因此需要一种能够在无故障标签和切换信号情况下补偿隐藏控制通道故障的方法。

Method: 提出了一个教师锚定框架（RFA），保留冻结的健康教师策略提供名义行为，并训练循环残余策略从本体感觉和指令响应历史中推断纠正动作；结合了故障注入域随机化（FIDR）和自适应采样来训练策略。

Result: 仿真实验表明，RFA在固定混合故障协议下的操作性能优于健康策略；真实机器人实验进一步验证了所学自适应策略的零样本部署能力。

Conclusion: RFA能够有效地补偿灵巧手运行时的隐藏关节故障，在无需显式故障标签或切换信号的情况下，显著提升了受损操作的鲁棒性和性能。
...
### [300] [Optimized Wrench Polytope Analysis for Real-Time Stability Control of Legged Robots in Complex Multi-Contact Configurations](https://arxiv.org/abs/2609.17405)
...
TL;DR: 本文提出了一种优化的算法，用于评估任意接触场景下具足机器人的完整可驱动力扳手多面体（wrench polytope），从而显著提高了复杂地形下的控制频率和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的具足机器人在斜坡、洞穴或脚手架等难以通行的复杂真实世界场景中仍面临巨大挑战，需要更高性能的算法来支持复杂的接触场景控制。

Method: 提出了一种用于评估任意接触场景下全驱动力扳手多面体的优化算法，使每个关节的力矩计算频率达到49 Hz，并将其集成到常规控制回路中用于调整机器人姿态。

Result: 该算法实现了显著的加速，可在常规控制循环中部署；在模拟和真实机器人硬件上的广泛测试表明，该稳定性控制器能够在极复杂的场景下实现稳定行走。

Conclusion: 研究所提出的优化算法和稳定性控制器成功解决了具足机器人在复杂地形中的遍历难题，其实际硬件部署验证了其超越当前其他控制器的复杂场景稳定能力。
...
### [301] [Gaussian Processes for Modelling Spatial Fields with Robot Swarms](https://arxiv.org/abs/2609.17463)
*Guillermo Legarda Herranz,Gianpiero Francesca,Mauro Birattari*

Main category: cs.RO

TL;DR: 本文提出了无定位高斯过程回归（LU-GPR）方法，使机器人蜂群在没有外部定位系统的情况下，仅依靠局部感知和通信来建模空间场并达成共识。


<details>
  <summary>Details</summary>
Motivation: 现有的机器人空间场建模方法严重依赖外部定位系统，这限制了其在无GPS或未知环境中的应用，因此需要一种不需要绝对定位的分布式建模方案。

Method: 引入无定位高斯过程回归（LU-GPR），利用在线算法让每个机器人在局部坐标系收敛到公共参考系的同时推断局部估计，并通过专家乘积模型（Product of Experts）结合同伴的估计获得全局模型。

Result: 结果表明，LU-GPR能够很好地随机器人数量扩展，对有限的通信范围具有鲁棒性，并且成功应用于真实世界中估算疏散人群流动的监测场景。

Conclusion: LU-GPR有效地解决了无定位条件下机器人蜂群对空间场的协同建模问题，具备良好的可扩展性、鲁棒性以及实际应用价值。
...
### [302] [Dissecting Motion-Prior Regularization for Data-Scarce Robotic Insertion](https://arxiv.org/abs/2609.17484)
*Ning Hu,Shuai Li,Jindong Tan*

Main category: cs.RO

TL;DR: 本研究探讨了在仅使用15个示范数据学习扩散策略（Diffusion Policy）时，训练时的运动先验正则化（如最小加加速度和速度-曲率正则化）对机器人插入任务成功率的影响。


<details>
  <summary>Details</summary>
Motivation: 评估基于少量示范学习扩散策略时，引入运动先验正则化是否能提高机器人的任务插入成功率。

Method: 对比了单独及联合使用最小加加速度（Minimum jerk）和速度-曲率正则化、不使用先验、以及使用通用平滑性的效果，每个设置在四种记录条件类别下进行了80次真实机器人试验。

Result: 联合正则化和仅最小加加速度设置均达到了70/80（87.5%）的成功率，优于无先验的82.5%和通用平滑性的83.8%，但联合正则化相比仅使用最小加加速度没有表现出明显的额外增益。

Conclusion: 研究结果支持将最小加加速度作为更简单且有效的候选正则化方法进行复制，但未发现明显的协同效应或对其他性能（如分布偏移鲁棒性）的提升。
...
### [303] [Modality-Autoregressive World-Action Models](https://arxiv.org/abs/2609.17524)
*Adam Hung,Bardienus P. Duisterhof,Deva Ramanan,Jeffrey Ichnowski*

Main category: cs.RO

TL;DR: ModAR是一个新型的世界-动作模型（WAM），它通过自回归去噪多种未来视觉模态（如深度、特征、点轨迹）来预测动作，并在无需预训练的情况下展现出高效且优越的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然深度、视觉特征和点轨迹等多种视觉模态能更高效地捕捉几何、语义和运动特征，但如何在世界-动作模型（WAMs）中最佳地结合这些模态仍是一个未解之题。

Method: 引入ModAR，首个在预测动作之前自回归去噪多种未来模态的WAM；从头开始训练以系统研究训练数据混合、预测模态及WAM公式的影响。

Result: 预测点轨迹、DINO特征和深度图使WAM受益，而额外预测RGB图像并无持续收益；ModAR的顺序生成超越了现有WAM公式，在所有评估的数据规模上平均成功率最高，且相比预训练模型节省了约20倍的训练FLOPs。

Conclusion: ModAR通过多模态的自回归去噪实现了高效且高性能的世界-动作建模，在无需视频预训练的情况下在模拟和真实世界任务中均优于基线模型。
...
### [304] [Comparative Analysis on Inertia Estimation Algorithms (IEAs) in Providing Proper Frequency Response](https://arxiv.org/abs/2609.16254)
*Karl M. H. Lai,Yunhe Hou,Kwunhang Wong*

Main category: eess.SY

TL;DR: 本文对电力系统惯量估计方法进行了系统性的比较分析，涵盖了传统方法、数据驱动方法及模型基方法，并探讨了其对风电惯量模拟策略的影响。


<details>
  <summary>Details</summary>
Motivation: 现代电力系统中高比例电力电子设备和新能源的接入使得传统惯量定义失效，且实时准确的惯量估计面临噪声、参数跳变和暂态响应等挑战，因此需要对现有的惯量估计方法进行系统性评估。

Method: 采用系统性比较分析的方法，将基于测量的经典方法（如滤波和拟合）与基于数据的参数估计技术（如递归最小二乘法，RLS）以及基于模型的方法（如卡尔曼滤波，KF）进行基准对比，并探讨其对风电惯量模拟策略的影响。

Result: 全面综述了基于模型和数据的惯量估计方法，深入分析了惯量估计算法对风电惯量模拟策略的影响，强调了开发鲁棒、自适应和数据驱动估计框架以确保低惯量电网安全运行的必要性。

Conclusion: 未来低惯量电网的安全稳定运行需要更加鲁棒、自适应且具备数据驱动特征的惯量估计框架来应对复杂的系统动态变化。
...
### [305] [Policy Gradient over History-Dependent Policy Classes for LQR with Domain Randomization](https://arxiv.org/abs/2609.16300)
*Tesshu Fujinami,Bruce D. Lee,Anastasios Tsiamis,Nikolai Matni,George J. Pappas*

Main category: eess.SY

TL;DR: 该论文研究了基于域随机化（DR）的强化学习中策略梯度（PG）方法的优化理论，特别是针对线性二次调节器（LQR）问题。


<details>
  <summary>Details</summary>
Motivation: 尽管域随机化在缩小仿真到真实世界差距（sim-to-real gap）方面被广泛应用，但其优化景观（optimization landscape）尚未得到充分理解，即使在经典的LQR目标下也是如此。

Method: 研究了历史依赖策略类（如有限脉冲响应控制器）上的域随机化LQR的策略梯度，并提出了一种基于课程学习的算法，通过逐步扩展控制器的记忆来寻找稳定控制器。

Result: 在环境异质性的适当界限下，所提出的算法与策略梯度结合能够全局收敛到域随机化目标函数的样本平均近似（SAA）的极小值点，且实验结果支持了理论发现。

Conclusion: 该工作揭示了域随机化下LQR问题的优化性质，证明了基于课程学习的策略梯度算法的全局收敛性，并为非线性域随机化控制等未来工作指明了方向。
...
### [306] [An Integrated EMT Small-Signal Stability Analysis Tool for Power Systems with High Inverter-Based Resource Penetration](https://arxiv.org/abs/2609.16273)
*Zihao Qin,Xiaonan Lu,Shuan Dong,Jin Tan*

Main category: eess.SY

TL;DR: 本文提出了一种名为EMT-SSA的集成工具包，用于电力电子化电力系统（IBRs）的电磁暂态小信号分析，能够直接从标准系统快照生成高保真EMT模型并计算系统的特征值和振荡模态。


<details>
  <summary>Details</summary>
Motivation: 随着基于逆变器的资源（IBRs）替代同步发电机，电力系统引入了超出传统机电频段的快速控制动态，而传统的相量域小信号工具无法解析这些次同步和换流器驱动的动态，现有的EMT仿真器又缺乏直接提供特征值、阻尼比等模态指标的能力。

Method: 开发了EMT-SSA工具包，从标准PSS/E系统文件（.raw/.dyr）出发，将其转换为高保真的EMT表示，求解网络和设备控制器的扩展潮流稳态平衡点，并线性化全阶EMT模型以形成系统状态矩阵，从而输出特征值、振荡模态和参与因子。

Result: 通过Kundur两区域系统进行了演示，成功识别出阻尼较差或不稳定的模态并将其归因于特定设备，其模态结果已与PSS/E NEVA进行了基准对比。

Conclusion: EMT-SSA工具包能够有效弥补传统相量域工具和时域EMT仿真器在分析高比例IBR电力系统稳定性方面的不足，提供精确的EMT级小信号分析与设备归因能力。
...
### [307] [Finite-sample guarantees for data-driven operator splitting methods via martingale inequalities](https://arxiv.org/abs/2609.16353)
*Andrea Martin,Filippo Fabiani,Giuseppe Belgioioso*

Main category: eess.SY

TL;DR: 该论文研究了数据驱动的Davis-Yin算子分裂算法在随机情形下的性能，建立了无分布、有限样本的输出质量保证。


<details>
  <summary>Details</summary>
Motivation: 在许多优化、控制和博弈论问题中，算子的前向评估无法闭式求解或计算成本高，需要通过带噪声的预言机样本来近似，现有研究缺乏对样本量随统计误差消失的先验保证。

Method: 直接利用鞅不等式控制残差误差，而非依赖特定代理损失的算法稳定性论证，并分析了线性收敛情况下的迭代复杂度。

Result: 给出了首个统计超额随样本量收敛消失的先验保证，并在线性收敛下将迭代次数的依赖性从线性增长改善为指数衰减，同时通过随机投资组合优化问题进行了验证。

Conclusion: 该研究成功为数据-driven的Davis-Yin分裂算法提供了严谨的有限样本统计保证，并在理论和应用上展现出显著优势。
...
### [308] [Electromagnetic Micro-Guidewire Control in Large Workspaces](https://arxiv.org/abs/2609.16354)
...
TL;DR: 本研究结合实时位姿反馈与约束凸优化、动态反馈及重复控制，实现了临床相关工作距离下节能的微导丝电磁导航转向。


<details>
  <summary>Details</summary>
Motivation: 由于磁体体积和线圈电流有限，电磁导航在临床相关距离上需要足够的驱动能力，因此需要提高能量利用效率并扩展工作距离。

Method: 结合实时位姿反馈、约束凸优化、动态反馈和重复控制，采用基于位姿依赖的力矩分配及零空间电流重新分配策略，以满足线圈电流约束。

Result: 在距线圈盖最远 55 cm 的距离下，实现了均方根误差低于 0.25 度的角度跟踪，并在 45 A 的主动线圈电流限制下保持了准确跟踪，显著降低了电流需求。

Conclusion: 实时状态信息和优化方法能够将电磁导丝控制的有效工作距离扩展至临床应用需求水平，同时保持高能效。
...
### [309] [Congestion Structure and Exceedance Bounds for Locational Marginal Emissions](https://arxiv.org/abs/2609.16513)
*Cameron Khanpour,Samuel Talkington,Daniel K. Molzahn*

Main category: eess.SY

TL;DR: 该论文研究了位置边际排放（LMEs）的内在低维结构，并证明了只需少量观测即可精确恢复所有节点的LME。


<details>
  <summary>Details</summary>
Motivation: 由于节点电力需求与总碳排放敏感性（即LMEs）在直流最优潮流下具有复杂的空间分布，传统计算成本高，因此需要揭示其底层数学结构以简化计算。

Method: 利用直流最优潮流模型，数学推导了固定激活约束集下LME向量的线性跨度，证明其秩受限于受阻塞线路数量，并推导了需求不确定性下的排放超越界限。

Result: 在十个不同规模的电力系统测试中，LME的内在秩$r$仅在2到15之间；例如在300节点系统中，仅需24次调度模拟即可精确恢复所有300个LMEs。

Conclusion: LME具有极低的内在维度，这使得通过少量的标量观测精确恢复整个系统的LME成为可能，且其预测误差范围取决于当地的活动集几何形状。
...
### [310] [Joint Freshness and Age-Dispersion Control over Finite-State Markov Wireless Channels](https://arxiv.org/abs/2609.16569)
*Aresh Dadlani,Hina Tabassum,Muthukrishnan Senthil Kumar,Masoumeh Moradian*

Main category: eess.SY

TL;DR: 本文研究了结合信息新鲜度（AoI）和年龄离散度（Age Dispersion）的联合控制问题，以满足面向目标的实时监测需求。


<details>
  <summary>Details</summary>
Motivation: 传统的AoI无法充分满足依赖于连续观测的面向目标应用的需求，而年龄离散度可以作为AoI的补充，因此需要研究两者的联合控制。

Method: 将问题建模为有限状态马尔可夫无线信道下的约束马尔可夫决策过程（CMDP），推导了AoI和离散度的平稳联合分布的精确矩阵表达式，并证明其具有线性规模的精确有限状态表示。

Result: 仿真结果表明，在相同的传输预算下，该联合控制器相比自适应仅AoI控制，将联合阈值违背率降低了37.3%，同时平均AoI仅增加了8.3%。

Conclusion: 通过联合控制AoI和年龄离散度，可以显著提高面向目标应用的实时监测性能，有效减少阈值违背。
...
### [311] [On Delay-robustness of Extremum Seeking of Nonlinear Static Maps with Small Disturbance](https://arxiv.org/abs/2609.16523)
*Jianzhong Li,Yang Zhu,Hongye Su*

Main category: eess.SY

TL;DR: 本文研究了基于梯度的极值寻优（ES）控制系统在存在传输延迟时的稳定性和延迟鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 闭环反馈中的传输延迟严重影响极值寻优系统的稳定性，现有文献多针对二次映射且依赖预测器，本文旨在为更广泛的非二次映射在无延迟补偿器的情况下提供延迟鲁棒性分析。

Method: 无需预测器或观测器，精心设计调制和解调中的抖动信号来处理常值延迟和时变延迟不确定性，并建立严格的收敛性与延迟鲁棒性分析框架。

Result: 当非未知时提供了严格的分析框架；当具有部分先验知识时，能够定量估计时间延迟和抖动周期的上界以保持系统稳定，且适当的参数选择可保证系统对任意大的已知常值延迟具有实际稳定性。

Conclusion: 该研究成功揭示了极值寻优系统承受传输延迟的能力，证明了在没有延迟补偿器的条件下，通过精心设计抖动信号和参数选择，系统能够对大延迟保持鲁棒稳定性。
...
### [312] [Large Language Models in the Loop: A Stability- and Network-Aware Survey in Networked Control, Cyber-Physical, and Multi-Agent Systems](https://arxiv.org/abs/2609.16599)
*Haiping Du,Linping Chan*

Main category: eess.SY

TL;DR: 这篇综述探讨了如何在保证网络化控制系统、信息物理系统和复杂多Agent系统中物理稳定性和安全性的前提下，将大语言模型（LLM）引入控制回路。


<details>
  <summary>Details</summary>
Motivation: 现代物理系统越来越多地依赖LLM进行高层决策，但LLM缓慢且随机的特性与系统严格的稳定性和安全保证之间存在冲突。

Method: 提出一个统一的分析框架，将LLM作为慢速监督者调整高层目标，而快速、经认证的内环维持物理稳定性；并将LLM的特性映射为经典网络控制挑战（如延迟、丢包、量化和扰动）。

Result: 评估了三大领域的最新进展，指出模型能力的提升往往伴随着形式化安全保证的下降。

Conclusion: 指出了未来研究方向，强调缺乏形式化稳定性证明是该领域当前的核心开放性问题。
...
### [313] [Development of a 4D Cerebral Microvascular Imaging Platform for Mouse Stroke Model](https://arxiv.org/abs/2609.16666)
*Yoshihisa Kaneko,Moe Kumai,Hiroyuki Igarashi,Daisuke Ando,Kuniyasu Niizuma,Hidenori Endo,Yoshifumi Saijo,Takuro Ishii*

Main category: eess.SY

TL;DR: 本研究开发了一种超声微血管成像平台，可在小鼠缺血性中风模型中实现全脑血流信号的快速连续采集，以研究缺血和再灌注阶段的脑血流动力学变化。


<details>
  <summary>Details</summary>
Motivation: 缺血性中风中缺血和再灌注阶段的脑血流动力学变化对预后有重要影响，但目前对此类变化的理解仍不够充分，缺乏高效的全脑动态监测手段。

Method: 结合先前的超声成像框架与连续机械扫描，开发了一个能够在5秒内获取全脑血流信号的时空分辨超声微血管成像平台，并在小鼠缺血和再灌注实验模型中进行验证。

Result: 实验表明，该平台能够成功可视化局部及全脑对缺血和再灌注诱导的血流动力学响应。

Conclusion: 该成像平台在小鼠模型中展现出进行快速、连续的全脑血流动力学评估的巨大潜力。
...
### [314] [Unified Heterogeneous Graph Neural Network solver for Power Flow, Optimal Power Flow and State Estimation](https://arxiv.org/abs/2609.16738)
*Ferran Bohigas-Daranas,Hamid Latif-Martínez,Eduardo Prieto-Araujo,Oriol Gomis-Bellmunt,Pere Barlet-Ros*

Main category: eess.SY

TL;DR: 本文提出了一种使用单一异构残差门控图卷积网络（Heterogeneous Residual Gated GCN）同时解决电力系统三大基础问题（潮流计算、最优潮流和状态估计）的方法。


<details>
  <summary>Details</summary>
Motivation: 现有的图神经网络（GNN）求解器通常针对单一问题进行训练，模型局限性大，每次更换任务都需要重新构建，计算成本高且缺乏通用性。

Method: 提出了一种基于共享骨干网络的通用方法，利用单个异构残差门控图卷积网络通过联合训练来学习电力网络的通用行为表示，从而估计PF、OPF和SE。

Result: 在IEEE 14节点和118节点系统上进行联合训练和评估，该共享模型不仅达到了与任务专用GNN求解器相同的准确率，而且对未见过的负载水平和拓扑结构保持了鲁棒性。

Conclusion: 单一模型能够捕捉电力网络的基本运行规律并同时服务于多种分析任务，这是迈向电力系统基础模型的重要一步。
...
### [315] [Explainable Post-Disaster Grid Observability Recovery Using Human-Oversight Agentic LLMs](https://arxiv.org/abs/2609.16774)
*Biswas Rudra Jyoti Arka,Sadman Sakib,Md. Zahidul Islam,Shamsun Nahar Edib*

Main category: eess.SY

TL;DR: 本文提出了一种基于大语言模型（LLM）的智能体工具调用框架，用于灾后相量测量单元（PMU）的恢复和电网可观测性恢复，兼具优化性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 灾后PMU中断会降低电网可观测性，现有的优化或启发式恢复方法虽然能生成恢复计划，但在解释性、可追溯性和操作员交互支持方面往往不足。

Method: 提出一个由LLM编排的智能体工具调用框架。LLM不直接求解优化问题，而是协调后端验证工具（如可观测性评估、恢复规划、状态更新和操作员验证），同时维护结构化历史记录和执行上下文以支持可追溯解释和交互式问答。

Result: 在IEEE 30节点和57节点系统上的仿真结果表明，该框架实现了与混合整数线性规划（MILP）相当的可观测性恢复效果，同时提供了基于工具的基础解释、交互式操作员支持和人工监督执行。

Conclusion: 该LLM编排的智能体框架能够有效解决灾后PMU恢复问题，在保持与传统优化方法相当性能的同时，显著增强了决策的可解释性、可追溯性和人机交互能力。
...
### [316] [Converter-Grid Interaction Stability Guaranteed Safe Deep Reinforcement Learning for Energy Storage Systems in Grid Frequency Support](https://arxiv.org/abs/2609.16817)
*Fei Liu,Mengfan Zhang,Zhipeng Li,Frede Blaabjerg,Qianwen Xu*

Main category: eess.SY

TL;DR: 本文提出了一种保证换流器-电网交互稳定性的安全深度强化学习（CIS-DRL）方法，用于储能系统（ESS）进行高效、安全的电力系统频率调节。


<details>
  <summary>Details</summary>
Motivation: 高比例新能源接入加剧了电力系统稳定性挑战，储能系统可提供快速频率支撑，但其接口换流器容易引发换流器-电网交互稳定性问题。

Method: 首先构建基于双深度神经网络（DNN）的稳定性区域以识别稳定边界；随后设计了结合稳定性可行性投影层的CIS-STD3算法，在学习过程中将不安全动作映射至稳定动作集，将稳定性作为硬约束强制执行。

Result: 实验结果表明，所提出的CIS-DRL方法在实现100%换流器-电网交互稳定性且无违规的同时，显著改善了系统的频率调节性能。

Conclusion: 该方法在防止不稳定运行点的同时提升了频率调节性能，证明了其在实时储能频率支撑中的实际应用价值。
...
### [317] [Impedance-Aware Optimized Pulse Patterns for Reconfigurable Battery Systems](https://arxiv.org/abs/2609.16828)
*Julian Estaller,Johannes Buberger,Andreas Wiedenmann,Wolfgang Grupp,Tobias Hoegerl,Thomas Weyh*

Main category: eess.SY

TL;DR: 本文揭示了可重构电池系统（RBS）中配置相关阻抗会导致传统优化脉冲模式（OPP）性能严重下降的问题，并提出了阻抗感知优化脉冲模式（IA-OPP）来显著改善电流质量。


<details>
  <summary>Details</summary>
Motivation: RBS中每个开关状态不仅改变源电压，还改变源阻抗，这打破了传统线性受控对象模型对优化脉冲模式（OPP）的隐性假设，导致传统方法严重高估可实现的电流质量。

Method: 提出了阻抗感知优化脉冲模式（IA-OPP），将电平相关的源阻抗嵌入优化目标中，结合处理通信总线重构速率的事件预算公式，并对分段指数负载电流进行精确的闭式评估以确保目标函数在开关时刻的平滑性。

Result: 在仿真级联桥参考系统中，IA-OPP在2.4 kHz的重构速率下实现了0.52%的电流总谐波畸变率（THD），在各种事件预算、调制指数范围以及±30%参数失配下，均优于最近电平调制、移相载波PWM和传统OPP基线。

Conclusion: IA-OPP能够有效解决RBS中阻抗调制带来的失真问题，在控制开关频率和通信开销的同时，大幅提升系统输出电流的质量和鲁棒性。
...
### [318] [Two-stage Coordinated Energy Management of Train Operation and Wayside Energy Storage System for Rail Power Supply Systems](https://arxiv.org/abs/2609.16857)
*Fei Liu,Niklas Biedermann,Stefan Östlund,Qianwen Xu*

Main category: eess.SY

TL;DR: 本文提出了一种针对电气化铁路供电系统的两阶段协同能源管理方法，通过联合优化列车运行轨迹和储能系统调度，有效降低了电网峰值功率和系统总成本。


<details>
  <summary>Details</summary>
Motivation: 铁路供电系统电气化程度的提高加剧了运营和经济挑战，而时变的牵引需求和网络限制使得储能系统与铁路运行的协同调度变得困难。

Method: 提出了一种两阶段协同能源 management 方法：第一阶段通过日前运行阶段确定列车运行曲线和储能设置以建立基准计划；第二阶段利用基于自适应权重经济模型预测控制（AWC-MPC）的日内滚动优化阶段来更新储能调度。

Result: 通过瑞典实际铁路案例验证，该方法使电网峰值功率需求降低了35.7%，总系统成本降低了28.8%，证明了其现实适用性。

Conclusion: 该两阶段协同能源管理方法能够有效应对时变牵引需求，显着改善电气化铁路供电系统的经济性和运行性能。
...
### [319] [Optimal Control Strategies for a Network of Electric Vehicle Charging Energy Hubs with Smart Scheduling via Distributed Optimization](https://arxiv.org/abs/2609.16899)
*Diego Fernandez-Zapico,Finn Vehlhaber,Maedeh Izadi,Theo Hofman,Mauro Salazar*

Main category: eess.SY

TL;DR: 本文研究了由可再生能源、储能及直流线路互联的电动汽车充电能源枢纽网络的成本最优运行问题。


<details>
  <summary>Details</summary>
Motivation: 为了降低电动汽车充电网络运行成本和碳排放，同时在保护各充电站隐私的前提下实现大规模网络的高效优化调度。

Method: 将动态最优控制问题建模为凸二次规划，并提出基于ADMM算法的问题分解方法，实现分布式求解。

Result: 荷兰的案例研究表明，相比固定充电功率，优化充电功率曲线（V1G）可将运营成本和碳排放显著降低25%以上，且分布式算法与集中式方案效果一致。

Conclusion: 该框架能够有效实现电动汽车充电能源枢纽网络的最优运行，兼顾全局最优性与隐私保护，为大规模网络和在线应用奠定了基础。
...
### [320] [Review on Electric Railway System Optimization: Train Dynamic Scheduling, Energy Management, and Storage Integration](https://arxiv.org/abs/2609.16939)
*Fei Liu,Can Wan,Stefan Östlund,Qianwen Xu*

Main category: eess.SY

TL;DR: 本文综述了电气化铁路系统中动态调度、能量管理和储能系统（ESS）集成的最新进展，探讨了如何通过优化策略和人工智能技术提升铁路系统的能效与智能化水平。


<details>
  <summary>Details</summary>
Motivation: 铁路系统现代化对更高效率、可持续性和智能运营的需求，推动了对动态调度、能量管理以及储能系统集成研究的总结与分析的需求。

Method: 通过文献综述法，分析了列车调度与运行控制的优化策略、储能技术在能量管理与电网控制中的应用，以及人工智能和先进控制策略在电气化铁路中的融合。

Result: 全面梳理了当前关于降低能耗、改善系统性能、峰谷调节以及电压频率控制的研究现状，明确了智能铁路技术的发展趋势。

Conclusion: 本综述为未来开发具有高韧性、高能效和智能化的集成储能铁路系统提供了全面的研究现状概述和未来发展方向指导。
...
### [321] [On personal recommendations in social networks](https://arxiv.org/abs/2609.17011)
*Philipp Grünter,Karl Henrik Johansson,Angela Fontan*

Main category: eess.SY

TL;DR: 本论文引入了一个分析上易处理且符合行为心理学中确认偏差现象的个性化推荐模型，并研究了不同推荐策略对社会网络中舆论动力学的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的舆论动力学模型通常无法捕捉个体代理如何处理个性化推荐，而算法在社交网络中的影响力日益普遍，因此需要更符合心理学现实的模型来分析推荐系统的集体行为。

Method: 基于个体的先验信念和敏感度参数，使用高斯影响函数构建个性化推荐模型；通过分岔分析研究广播策略（同质推荐），推导个性化目标策略的最优推荐，并扩展至代理敏感度不确定时的鲁棒收敛性分析。

Result: 研究表明，采用广播策略时系统会表现出双稳态，可能导致意料之外的后果；同时推导出了个性化目标策略及其在敏感度不确定情况下的鲁棒收敛性，并通过数值模拟进行了验证。

Conclusion: 该模型成功结合了确认偏差与个性化推荐机制，能够有效分析不同推荐政策下社会网络的集体演化行为及潜在风险。
...
### [322] [The Price of Distributional Robustness in Linear Quadratic Control](https://arxiv.org/abs/2609.17113)
*Andrea Martin,Giuseppe Belgioioso*

Main category: eess.SY

TL;DR: 本文研究了数据驱动的 Wasserstein 分布鲁棒线性二次控制（LDRC）问题中的保守性，通过样本复杂度界限分析了其相对于预知真实分布的 oracle 控制器的次优性。


<details>
  <summary>Details</summary>
Motivation: 分布鲁棒（DR）优化通过防范分布模糊性来设计具有强样本外保证的控制器，但这种防护机制引入了保守性。了解这种保守性以及达到特定性能所需的数据量（样本复杂度）至关重要。

Method: 研究了数据驱动的 Wasserstein 分布鲁棒线性二次控制问题，推导了样本复杂度界限，分析了 DR 解相对于 oracle 控制器的次优性，并通过数值仿真进行验证。

Result: 分析表明，在分布模糊度足够小时，DR 解的次优性随 Wasserstein 半径呈线性增长，而在远离该局部区域时呈二次方增长，数值仿真验证了这些理论界限。

Conclusion: 分布鲁棒优化的次优性受 Wasserstein 半径和样本数量的制约，其代价在局部区域内表现为线性增长，在全局上表现为至多二次方增长，这为数据-驱动控制器的设计提供了理论保证。
...
### [323] [Data-Driven Policy Iteration Without an Initial Stabilizing Policy: A Finite-Horizon Bootstrap Method](https://arxiv.org/abs/2609.17191)
*Jiacheng Wu,Yang Zhu*

Main category: eess.SY

TL;DR: 本文提出了一种针对连续时间线性系统的无模型（数据驱动）策略迭代方法，无需初始稳定策略即可实现自启动。


<details>
  <summary>Details</summary>
Motivation: 传统的无限时域策略迭代方法依赖于初始稳定策略，而在系统未知时验证和获取这一策略非常困难。

Method: 开发了一种基于有限时域的自举方法，通过在紧区间内对平移系统进行策略评估，并利用离轨数据和基函数逼近来重构有限时域值矩阵和策略，最后引入数据驱动的Lyapunov证书来验证候选增益。

Result: 数值算例（如间歇反应器和双质量弹簧系统）证明了所提出的自举方法能够有效摆脱对初始稳定策略的依赖并确保系统稳定性。

Conclusion: 该研究成功解决了未知连续时间线性系统数据驱动策略迭代的无初始稳定策略启动问题，具有良好的理论和实际效果。
...
### [324] [Backstepping Design of Dynamic State Feedback Controllers for Parabolic Systems](https://arxiv.org/abs/2609.17293)
*Nicole Gehring,Benedikt Schwämmle,Abdurrahman Irscheid,Joachim Deutscher*

Main category: eess.SY

TL;DR: 本文提出了一种基于动态扩展的耦合扩散-反应系统动态状态反馈控制方法。


<details>
  <summary>Details</summary>
Motivation: 传统的静态状态反馈无法为一般类别的抛物型系统实现任意闭环动态分配和解耦的输入输出行为，因此需要更通用的控制设计。

Method: 引入特定的控制器动力学以实现扩展系统的均匀化扩散系数，随后为动态扩展系统设计基于反演（backstepping）的静态状态反馈。

Result: 成功稳定了一类更广泛的抛物型系统，并实现了任意闭环动态分配与解耦的输入输出行为，仿真示例验证了该方法的有效性。

Conclusion: 基于动态扩展和反演的设计方法能够有效克服传统静态状态反馈的局限性，适用于复杂扩散-反应系统的精确控制。
...
### [325] [A Time-to-Collision Barrier Function Approach to Collision Avoidance for Stochastic Systems](https://arxiv.org/abs/2609.17347)
*Benedikt Barthel Sorensen,Mitchell Black,Erfaun Noorani,Themistoklis P. Sapsis*

Main category: eess.SY

TL;DR: 本文提出了一种基于对抗性碰撞时间（aTTC）的新型防撞控制方法，通过在时间域定义控制屏障函数（CBF），实现了具有预见性的避障策略。


<details>
  <summary>Details</summary>
Motivation: 传统的自主系统防撞约束通常基于位置或速度空间，仅对几何邻近性做出反应，无法充分利用对手的动力学限制来实现前瞻性的规避。

Method: 利用对抗性碰撞时间（aTTC）在时间域直接定义控制屏障函数（CBF），并采用轻量级神经网络代理来近似精确的aTTC计算，从而支持实时的基于二次规划（QP）的控制律。

Result: 在2D对比研究和3D多智能体追逃场景的验证中，基于aTTC的CBF方法表现优于高阶基于距离的基线方法，能够更有效地为对抗速度占优的追逐者争取时间。

Conclusion: 基于aTTC的CBF能够实现本质上具前瞻性的速度调制策略，有效利用对手的动力学限制，显著提升了自主系统在追逃对抗中的防御能力。
...
### [326] [LM-PCVMNet: Pediatric Cervical Vertebral Maturation Analysis with Deep Fusion of Landmarks and Metadata](https://arxiv.org/abs/2609.16033)
*Peng Wang,Wanzhen Song,Anli Wang,Xueshuo Xie,Xiaohang Guan,Tao Li*

Main category: eess.IV

TL;DR: 本文提出了一种名为LM-PCVMNet的新型深度学习框架，用于儿童颈椎成熟度（CVM）的自动分期，并发布了一个新的大规模数据集PCVM+。


<details>
  <summary>Details</summary>
Motivation: 正畸诊断和确定最佳治疗时机需要准确的颈椎成熟度（CVM）评估，现有方法在自动化和结合解剖学及元数据信息方面仍有提升空间。

Method: 提出了LM-PCVMNet框架，整合了椎骨解剖标志信息、热图引导的特征调制、椎骨标志提示模块以及可学习的元数据监督对比损失，并发布了包含1800张真实世界儿童侧位头颅X光片的新数据集PCVM+。

Result: 在两个数据集上的广泛实验表明，该方法实现了SOTA（最先进）的性能，有效提高了标志物定位和分类准确率。

Conclusion: LM-PCVMNet能够有效提升儿童CVM自动分期的准确性和生物学一致性，为正畸治疗研究提供了强有力的工具和数据集支持。
...
### [327] [Conditioning noise is a free regularizer for LoRA fine-tuning: no pathology encoder required for diffusion-based artifact detection in histopathology](https://arxiv.org/abs/2609.16032)
*Konstantinos Moutselos,Ilias Maglogiannis*

Main category: eess.IV

TL;DR: 该论文提出了一种基于扩散模型的全切片图像伪影检测方法，通过在微调过程中引入随机高斯嵌入条件，显著扩大了干净组织与伪影之间的区分度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的伪影检测器通过在干净组织上微调的模型重建误差来对图像块进行评分，作者旨在改进这一微调过程以增强干净与伪影区域的分离效果。

Method: 通过在每个步骤从预计算的嵌入统计信息中重新采样随机高斯嵌入来条件化微调过程，无需编码器、缓存或改变推理方式，并进行了消融实验、LoRA-dropout对照以及预注册的外部验证。

Result: 在九次训练中，斑块级Cohen's d增益达到+0.25-0.48；在281个病例集上的两个预注册外部终点证实了汇聚的Delta F1分别提高了+0.0073和+0.0129。

Conclusion: 条件化随机高斯嵌入能有效且稳健地提升基于扩散模型的全切片图像伪影检测性能，且该方法具有良好的泛化能力和严格的验证支持。
...
### [328] [A deep dictionary network-based foundation model for ultra-low-dose CT denoising](https://arxiv.org/abs/2609.16031)
*Baoshun Shi,Shuangyi Yang,Ke Jiang,Bin Zhu,Zhanli Hu,Huazhu Fu*

Main category: eess.IV

TL;DR: 本文提出了一种基于深度字典网络（DDN）的可解释基础模型，用于统一的多器官超低剂量计算机断层扫描（ULDCT）去噪。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习去噪方法通常针对特定器官训练，缺乏在异构多器官成像场景中的泛化能力，而现有的基础模型架构缺乏可解释性且依赖启发式训练策略。

Method: 基于多层稀疏表示理论，构建了级联卷积稀疏编码层与迭代软阈值处理的深度字典网络（DDN），嵌入动态字典和阈值生成模块，利用海量正常剂量CT图像进行预训练，并结合稀疏正则化对多器官ULDCT数据集进行联合微调。

Result: 广泛的实验验证了所提出的方法实现了SOTA（最先进）的性能，并在所有多器官ULDCT任务中持续超越竞争方法。

Conclusion: 该架构可解释的深度字典网络基础模型成功实现了统一的多器官ULDCT去噪，具有强大的泛化能力和噪声鲁棒性。
...
### [329] [Automated Distinction of Intimal and Medial Intracranial Arterial Calcification from CT Head](https://arxiv.org/abs/2609.16035)
*Benjamin Jin,Maria del C. Valdés Hernández,Richard Bortsov,Joanna M. Wardlaw,Daniel Bos,Grant Mair*

Main category: eess.IV

TL;DR: 本研究评估了三种基于头颅CT分割掩膜的颅内动脉钙化（IAC）亚型自动分类方法，证明了全自动IAC亚型量化的可行性及其对人工和自动分割掩膜的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 颅内动脉钙化（IAC）分为内膜型和中膜型，具有不同的病因和临床意义，放射科医生可通过形态进行区分，但目前缺乏高效的自动化分类方法。

Method: 研究了三种自动化方法：（1）放射科目视评分的自动化改进版；（2）基于球形度的方法；（3）基于医学形状基础模型提取的形状嵌入的方法。所有方法均使用相同的轻量级分类流水线，并通过5折交叉验证进行评估。

Result: 三种方法性能相当，基于嵌入的方法表现最好，单动脉分类的加权F1分数为71.5 ± 3.7，联合动脉分类为59.8 ± 1.7；使用自动分割掩膜与人工掩膜相比，性能无显著下降。

Conclusion: 从头颅CT进行全自动IAC亚型量化是可行的，并且在使用自动和人工IAC分割掩膜时均保持了良好的鲁棒性。
...
### [330] [Anatomy-Change-Aware Bidirectional Selective State-Space Memory for Clinically Deployed Thoracic Radiotherapy Auto-Contouring](https://arxiv.org/abs/2609.16036)
...
TL;DR: DAMM-Net++是一个用于胸部危 and 靶区分割的2.5D深度学习架构，结合了双向状态空间记忆、边界感知解码器和不确定性估计头，显著提升了放射治疗自动勾画的准确性与临床可用性。


<details>
  <summary>Details</summary>
Motivation: 解决放射治疗自动勾画中存在的三个持续性挑战：层间表面不一致性、在小型低对比度目标上的系统性失效，以及缺乏按病例的可靠性信号。

Method: 开发了DAMM-Net++架构，核心组件包括解剖变化感知的双向选择性状态空间记忆（建模层间解剖变化并传播上下文）、边界感知解码器（锐化近表面预测）和不确定性估计头（提供逐体素的置信度用于临床分诊），并在多中心大规模患者队列及放射肿瘤学家参与的读者研究中进行了评估。

Result: 模型实现了0.955的平均Dice和3.78毫米的HD95，在低对比度OAR和靶区上增益最大；AI辅助将勾画时间缩短了75%-80%，并将初级读者的IoU从0.861提高到0.925；外部验证显示性能下降小于5%，不确定性校准良好。

Conclusion: 解剖学驱动的层间记忆与不确定性引导的审查相结合，为胸部自动勾画提供了一条临床可行的途径，目前已集成到实际临床工作流中。
...
### [331] [Multisource Remote Sensing and Geospatial Analysis of Vineyard Wildfire Impacts and Resilience: The 2019 Kincade Fire](https://arxiv.org/abs/2609.16168)
*Parastoo Farajpoor,Mahla Ardebili Pour,Mohammad Bagher Ghiasi,Mohammadreza Narimani*

Main category: eess.IV

TL;DR: 本研究利用开源地理空间框架，评估了2019年美国加州金卡德野火期间葡萄园景观的复原力，发现尽管葡萄园的光谱影响较低，但并不意味着完全具有复原力，且易受烟雾和长期植被恢复不足的影响。


<details>
  <summary>Details</summary>
Motivation: 工作农业景观在野火灾害中常被视为背景，但它们实际上是受管理的燃料马赛克和生产资产，因此需要系统评估其在野火中的真实复原力。

Method: 采用开放的、以事件为锚点的地理空间框架，结合Sentinel-2、OpenET、gridMET等多源数据以及空间广义相加模型（GAM）和分段集群边界模型，在原生决策尺度上分析了4,581个葡萄园地块。

Result: 葡萄园像素的光谱受损程度（dNBR）低于野火荒地，但条件空间分析显示，控制位置、地形和水资源使用后，葡萄园比例与dNBR呈正相关；此外，所有葡萄园均受到烟雾影响，且园内葡萄园在2021年前表现出更大的绿色度恢复赤字。

Conclusion: 葡萄园较低的即时光谱影响并不等同于完全的野火复原力。该研究提供了一个可复制的城乡信息学模板，用于指导工作景观中的决策。
...
### [332] [A Sentinel-2 benchmark dataset for deep-learning active-fire segmentation across 25 California wildfires](https://arxiv.org/abs/2609.16199)
*Shreyan Mitra,Mohammadreza Narimani,Parastoo Farajpoor*

Main category: eess.IV

TL;DR: 本文介绍了一个用于开发和评估卫星图像中主动火灾分割方法的开放图像数据集。


<details>
  <summary>Details</summary>
Motivation: 为了支持稀疏目标分割、从算法标签中学习以及跨火灾事件迁移的研究，需要高质量的标准化主动火灾图像数据集。

Method: 使用Sentinel-2卫星影像构建了包含2,148个图像-掩码对的数据集，通过短红外亮度、近红外对比度及邻域增长生成掩码，并提供基于ResNet-34 U-Net的参考训练与评估代码。

Result: 数据集包含来自25场加州野火的影像，其中841个包含主动火灾标签；同时提供了详细的元数据、独立事件划分以及专家对测试集的盲审评估。

Conclusion: 该开源数据集及配套代码为卫星图像中的主动火灾检测与稀疏类分割研究提供了宝贵的基准资源。
...
### [333] [Semantic-Aware Neural Video Codec for Error-Resilient Low-Latency Transmission](https://arxiv.org/abs/2609.16279)
*Matin Mortaheb,Homa Esfahanizadeh,Jinfeng Du,Harish Viswanathan*

Main category: eess.IV

TL;DR: 提出了一种面向物理AI系统的语义感知多级神经视频编码方法，用于在不可靠信道上实现低延迟、鲁棒的视频传输。


<details>
  <summary>Details</summary>
Motivation: 新兴的物理AI系统需要在不可靠的通信信道上进行低延迟、面向任务的视频通信，现有的神经视频编码在面对丢包时缺乏足够的鲁棒性和任务感知能力。

Method: 基于DCVC-RT神经视频编码器，引入语义和特征感知编码策略将编码表示分流并赋予不同优先级；开发了消除包间依赖的抗差错熵模型；通过端到端训练实现信道感知表示、重要性分组分配及差分数据包优先级排序。

Result: 实验表明，该框架在丢包环境下显著提高了基线DCVC-RT的鲁棒性，在丢弃不太重要区域的同时更好地保留了任务相关的视觉内容，实现了优雅降级。

Conclusion: 该方法通过结合语义感知编码、包独立解码设计和端到端信道联合训练，有效解决了不可靠信道下的低延迟鲁棒视频传输问题，特别适用于物理AI系统。
...
### [334] [StainBridge: Stain-Aware Pairwise Registration of Serial Renal Biopsy Whole-Slide Images Across Structural and Immunohistochemical Stains](https://arxiv.org/abs/2609.17090)
...
TL;DR: 本文提出了StainBridge，一个用于串联肾活检全切片图像（WSI）跨染色（结构染色与免疫组化IHC）配准的框架，旨在解决由于稀疏表达和缺乏共享特征导致的配准难题。


<details>
  <summary>Details</summary>
Motivation: 串联肾活检包含多种结构染色和具有稀疏/缺失表达的 diverse IHC标记，缺乏共享特征，导致传统多染色配准方法在肾脏病理3D重建中非常困难。

Method: 提出了StainBridge框架，结合了三种预处理组件（染色解卷积、强度归一化和组织掩码注入）、基于XFeat的仿射初始化，以及四种非刚性配准后端（VoxelMorph、ConvexAdam、FireANTs和DeeperHistReg）。

Result: DeeperHistReg获得了最佳的汇总地标精度并成功配准了所有尝试的图像对；预处理显著改善了ConvexAdam和FireANTs的地标精度，而FireANTs展示了最大的预处理增益和最佳的组织重叠度。

Conclusion: StainBridge及其评估结果为跨染色WSI配准提供了实用的指导方案，并为肾脏组织结构与分子表达的集成3D分析奠定了基础。
...
### [335] [Quantum-Inspired Trainable and Parameter-Efficient Tensor Networks for Image Inpainting](https://arxiv.org/abs/2609.17298)
*Shiwen An,Konstantinos Slavakis*

Main category: eess.IV

TL;DR: 该论文引入了受量子启发张量网络电路作为可训练变换，用于图像修复，在保持最少相干性的同时显著减少了参数量。


<details>
  <summary>Details</summary>
Motivation: 克服传统图像修复方法中对显式相干性惩罚的依赖，并解决现有酉矩阵架构参数量过大、训练复杂的问题。

Method: 提出对角量子傅里叶变换（QFT）松弛等架构，利用电路结构固有地保持最小相干性，并采用无约束基于梯度的相位优化（无需黎曼优化）从随机采样数据中进行高效学习。

Result: 数值测试表明，所学习的模型性能优于固定变换和单图优化，且能够匹配更大酉架构的性能，同时具有远少于它们的参数量。

Conclusion: 受量子启发的张量网络电路能够作为高效、参数量少且泛化能力强的可训练变换，用于图像修复任务。
