...
TL;DR: 研究展示了现代开源大模型（如Qwen2.5-14B）能够在无需联网的情况下，通过单一便携文件高效处理并回答历史上所有的Jeopardy!竞赛问题，性能远超IBM Watson。


<details>
  <summary>Details</summary>
Motivation: 对比2011年IBM Watson笨重的封闭式架构，研究旨在验证现代小型大模型是否能实现知识的便携化，并探讨模型在应对未见过的时效性问题时的泛化能力。

Method: 使用9GB的Qwen2.5-14B（4-bit）模型，在完整的Jeopardy!数据集（1984-2025年，共529,939条线索）上进行强制响应测试，并对比Claude Opus等模型在训练截止日期后的表现。

Result: Qwen2.5-14B在全量数据集上准确率为67.0%，在事实类问题上超过85%。在测试时间范围后的问题上，模型表现出了极强的泛化性，而Watson则完全无法处理。

Conclusion: 现代大模型成功实现了人类通用知识的“便携化”与“动态化”，证明了模型不仅能存储知识，更具备在训练截断后处理新问题的能力，优于传统硬编码的问答系统。
...
TL;DR: 本文提出将Rasch测量理论（RMT）引入大语言模型（LLM）的评估范式中，用于系统地解构并诊断模型作为评估者时的偏差与属性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估方法忽略了评估过程中各组成部分（被评估物、评估工具、评估者）对结果的独立贡献，导致评估结果难以解释，且缺乏对模型偏差的深度诊断。

Method: 利用多面Rasch模型（Many-facet Rasch models）对9个LLM在Measuring Hate Speech数据集上的标注结果进行建模，分解并量化模型在评分严苛度、校准性、鲁棒性及偏见等方面的特征。

Result: 研究发现LLM在严苛度、项目校准、对问题顺序的鲁棒性以及对特定身份群体的敏感度等方面与人类评估者存在系统性差异，这些差异在传统评估方法中会被掩盖。

Conclusion: Rasch测量理论是评估LLM作为考试者、评判者及评估者时强有力的诊断工具，能够提供比传统方法更精确、可解释的评估指标。
...
### [3] [Retrieving Relations, Detecting Fallacies: A RAG Approach to Political Debate Analysis](https://arxiv.org/abs/2608.27471)
*Deborah Dore,Greta Damo,Elena Cabrio,Serena Villata*

Main category: cs.AI

TL;DR: 该研究提出了一种基于论证关系引导的检索增强方法，用于自动检测和分类政治辩论中的逻辑谬误。


<details>
  <summary>Details</summary>
Motivation: 自动检测逻辑谬误需要超越表层文本的上下文知识和论证结构，而现有方法多将论证结构作为静态特征，缺乏灵活性。

Method: 引入一种导向式检索增强框架，利用支持与攻击的论证关系动态引导外部知识检索，在15GB政治文档知识库上进行验证。

Result: 在ElecDeb60to20基准测试中，谬误检测的宏观F1得分提升至0.864，分类得分提升至0.725，优于非检索基线模型。

Conclusion: 研究证明，当检索过程由论证关系引导时，整合外部知识能够显著提升逻辑谬误检测与分类的准确性。
...
### [4] [Not All Explanations Are Sought: Information-Seeking Psychology for Human-Centered XAI](https://arxiv.org/abs/2608.27464)
*Andrea Beretta,Salvatore Rinzivillo*

Main category: cs.AI

TL;DR: 本文提出将信息搜寻心理学整合至以人为中心的解释性AI（HCXAI）中，强调系统设计应从“提供解释”转向“引导用户主动寻求解释”。


<details>
  <summary>Details</summary>
Motivation: 现有HCXAI研究忽略了用户为何以及何时寻求信息的动机，导致解释往往无法满足用户的真实需求，尤其在代理型AI（agentic AI）环境中表现尤为突出。

Method: 基于Sharot和Sunstein的信息搜寻动机框架，引入工具性、享乐性和认知性三种效用模型，并分析了六种认知偏差（如控制错觉、自动化偏见等）对信息寻求行为的影响。

Result: 识别出信息过载（分散注意力）和信息不足（忽视风险）两种解释系统的潜在失效模式，指出认知偏差是导致这些失效的关键因素。

Conclusion: 主张设计应从单纯的解释可用性转向以心理学为依据的“按需设计”，通过理解用户的需求动机来优化交互式AI的解释效果。
...
TL;DR: 提出了一种基于概率依赖图（PDG）的新方法，通过加权融合贝叶斯网络结构学习（BNSL）算法与大语言模型（LLM）的因果知识，显著提升了因果图的学习准确性。


<details>
  <summary>Details</summary>
Motivation: BNSL在方向识别上存在局限性，而LLM虽拥有广泛的因果知识但不可靠，两者具有互补性，缺乏一种有效结合二者的框架。

Method: 引入概率依赖图（PDG），将每条边建模为指向、无向和缺失状态的分布，利用加权平均将BNSL算法（FGES、Tabu、PC）与LLM（Gemini、Claude、GPT）的输出进行融合。

Result: 在26个基准网络中，50/50融合策略在22个网络上提升了F1分数，平均提升显著（0.056, p<0.001）；BNSL提供了高召回率的骨架，LLM提供了高准确度的方向信息。

Conclusion: 将BNSL和LLM的因果知识表示为边存在与方向的概率不确定性，是提高因果图构建准确性的实用且有效的途径。
...
TL;DR: HER-PDE 是一个基于科学智能体框架的 PDE 发现方法，通过联合推断支配算子和非参数化空间系数场，实现了从嘈杂数据中恢复复杂物理规律的功能。


<details>
  <summary>Details</summary>
Motivation: 在异构介质中，PDE 的控制算子与空间系数场是耦合的，现有的方法往往难以在未知系数形式的情况下准确识别算子结构。

Method: 引入 HER-PDE 框架，通过“假设、评估、改进”循环，利用两个不同激励下的轨迹进行双向交叉评估，结合结构化表达式树搜索与局部系数场优化来发现 PDE。

Result: 在 5% 高斯噪声下，该方法成功恢复了全部 5 种受控系统的控制算子，并在 9 种未知系数场中表现优异，中位相关性达到 0.85，相对 L2 误差约为 0.28。

Conclusion: 该研究验证了科学智能体通过引导式的假设改进，可以在不预设空间系数参数形式的情况下，准确发现异构介质中的物理定律。
...
TL;DR: 本文提出了Class-Based Heuristic A* (CBHA*) 算法，通过结合通用移动约束和状态空间分类，显著提升了在受限空间规划问题（以两列滑块拼图为例）中的搜索效率。


<details>
  <summary>Details</summary>
Motivation: 通用启发式搜索在受限空间规划中往往表现不佳，难以应对空间受限导致的性能急剧下降问题。

Method: 引入了基于类别的启发式算法（CBHA*），包括通用移动约束（General Move Constraint）、将状态空间划分为七个类别的运动学分类系统，以及针对f值平原的类条件平局决断机制。

Result: 在146个基准测试中，CBHA*的成功率达到93.4%，远高于标准A*（39%）和BFS（17%），并将节点扩展数量减少了87.98%。

Conclusion: 基于类别的自适应启发式算法是解决受限空间规划问题的有效且具有原则性的方案，能够推广至其他具有物理约束的系统。
...
TL;DR: GMA是一个新的移动端智能体基准测试平台，旨在通过更复杂、多样化的现实场景评估通用移动助手。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在应用覆盖面和任务复杂度上不足以模拟真实移动设备的使用场景。

Method: 引入7个开源应用及300个覆盖四个难度等级的任务，并通过控制变量法研究上下文保留、状态追踪等智能体架构设计的影响。

Result: 当前主流大模型在处理复杂任务时表现显著下降，且特定的智能体架构设计能显著提升复杂工作流的处理性能。

Conclusion: GMA扩展了移动智能体的评估维度，证明了优化智能体架构设计对实现可靠的复杂任务自动化至关重要。
...
TL;DR: 该研究提出了一种结合IoT声学监测与深度学习的框架，用于茶园“上国活木白蚁”（ULWT）的早期检测与严重程度评估。


<details>
  <summary>Details</summary>
Motivation: 茶园中ULWT隐蔽性强，一旦发现往往已造成严重破坏，需要一种非侵入式的早期监测与空间风险评估方案。

Method: 利用树干声学传感器采集音频信号，通过Raspberry Pi进行数据处理，使用CNN模型对谱图进行分类，并结合声学振幅和空间地理位置构建白蚁严重程度评估模型。

Result: CNN模型在测试集上的分类准确率为81.5%，且该框架能够定量评估白蚁危害程度并实现空间可视化。

Conclusion: 该系统能有效支持茶园管理者定位高风险区域，实现针对性的精准防控，降低白蚁造成的经济损失。
...
TL;DR: 该论文研究了知识库系统中基于广义水平（level-based）评估的上下文定位问题，探讨了分数过滤与上下文限制之间的等价性。


<details>
  <summary>Details</summary>
Motivation: 为了在知识库中实现更稳定的证据选择和知识聚合，需要明确上下文过滤与局部化之间的数学一致性条件。

Method: 提出了一个广义水平度量框架，利用单调集合函数进行聚合测试，并引入了单调性和归约属性（reduction property）作为核心结构条件。

Result: 证明了当且仅当满足结构化单调性和归约属性时，分数过滤与上下文相交产生的局部化等价；并分析了点对点及块生成机制。

Conclusion: 该研究确立了上下文依赖证据选择和非加性支持评估的稳定性准则，为知识表示与推理中的上下文逻辑提供了理论支撑。
...
TL;DR: CareGraph 是一个可审计的混合 AI 框架，旨在将碎片化的临床、自述和可穿戴数据转化为结构化的健康趋势与解释，同时保持严格的安全性和溯源性。


<details>
  <summary>Details</summary>
Motivation: 个性化医疗数据来源复杂、碎片化，难以解释和追溯，且现有的 AI 系统往往缺乏透明度与安全边界。

Method: 构建了一个包含确定性分析、上下文检测、图构建、受限大模型合成、证据验证及安全控制的管线，不进行自主诊断或决策，而是提供受限的辅助信息。

Result: 在测试中，CareGraph 在数据趋势识别（0.827准确率）和缺失上下文检测（0.815 F1）方面表现优异，并在安全规则基准测试中达到近乎完美的表现，同时在生成速度和内容简洁度上优于 GPT-5.6。

Conclusion: CareGraph 为个性化医疗系统提供了一个可审计、安全性强且具备溯源能力的架构基础，是向可信医疗 AI 迈出的重要一步。
...
TL;DR: 研究探讨了在大语言模型中引入推理架构（如规划、验证、修正）时的“代币预算阈值”现象。


<details>
  <summary>Details</summary>
Motivation: 推理架构能提升性能，但其本身的计算开销会消耗有限的预算，研究旨在寻找推理架构优于单一模型的转折点。

Method: 在FinQA和TAT-QA任务上，对比了单一模型与具备验证搜索架构（含规划、检查和修复）的模型，在14个不同代币预算阶梯下的性能表现。

Result: 实验发现存在一个阈值（1,000-1,500代币），在此之前复杂架构因开销过大表现不如单一模型，在此之后复杂架构在性能上始终保持领先。

Conclusion: 推理架构的性能增益高度依赖于预算分配，只有当预算超过一定临界点时，引入推理机制才能有效提升推理任务的准确率。
...
TL;DR: WM-R1是一个创新的移动GUI代理训练框架，通过世界模型替代真实Android环境进行强化学习，实现了大规模并行的高效训练。


<details>
  <summary>Details</summary>
Motivation: 传统RL在GUI任务中面临交互成本高、不稳定及训练速度慢的问题，需要一种更高效、低开销的训练范式。

Method: 利用世界模型模拟状态转换以取代真实交互，将世界模型集成至推理过程进行动作预判，并引入多维度规则奖励机制及高质量训练数据集。

Result: 在Android基准测试中，WM-R1的表现显著优于传统的GRPO基线及推理时模拟方法。

Conclusion: 基于世界模型的强化学习框架有效解决了GUI代理训练中的资源限制和不稳定性，是提升移动代理任务执行能力的有效方案。
...
TL;DR: SETU是一个用于企业沟通教练的代理生态系统，旨在通过多模态分析提升多语言环境下的员工沟通能力。


<details>
  <summary>Details</summary>
Motivation: 企业缺乏可扩展且可解释的工具来优化多语言工作环境下的沟通，现有系统多为黑盒模型，难以审计且无法提供有效的教练建议。

Method: 构建了一个由视频、音频、文本相关性、评分、通知及报告代理组成的协同系统，通过信任感知编排进行多模态分析，并生成可追溯的教练报告。

Result: 该系统在销售话术的针对性调整场景中表现出良好的应用潜力，能够为员工提供针对特定受众的沟通改进建议。

Conclusion: SETU通过多智能体协作与可解释的多模态评估，有效架起了跨语言和跨受众沟通的桥梁，为企业培训提供了高效、可信的教练方案。
...
TL;DR: Nemotron 3.5 CS 是一款紧凑型 4B 视觉-语言安全审核模型，旨在为部署的 AI 应用提供多模态、多语言及自定义策略的安全审核服务。


<details>
  <summary>Details</summary>
Motivation: 现有安全审核系统多局限于文本处理，难以兼顾广泛的覆盖范围、自定义策略控制与低计算成本，无法满足现代 AI 应用对多模态（图像、文档等）内容审核的需求。

Method: 该研究提出了一个 4B 参数规模的视觉-语言模型，通过多模态、多语言安全数据集进行训练，支持直接输出安全标签（低延迟）以及根据自定义策略提供推理路径。

Result: Nemotron 3.5 CS 在多模态安全性、文本审核、多语言鲁棒性、策略遵循及低延迟方面表现优异，在保持竞争力的同时实现了图像和策略条件的审核功能。

Conclusion: 紧凑型视觉-语言审核模型可作为高效的部署级安全组件，通过选择性使用推理功能来平衡审核精度与计算成本。
...
TL;DR: LongGuard 揭示并解决了大语言模型长上下文环境下的安全护栏失效问题，提出了针对性的分析框架与训练无关的优化方案。


<details>
  <summary>Details</summary>
Motivation: 现有的安全护栏主要针对短文本，在面对长上下文时性能会因“针尖效应”显著下降，亟需对其失效机制进行深入剖析与缓解。

Method: 构建 SafetyNIAH 评估集，通过注意力-Logit-行为分析法定位失效根源；提出分块检测（CD）、注意力头锐化（AHS）及上下文感知路由（CAHR）三种优化手段。

Result: 实验发现长文本中 unsafe 内容的注意力权重被稀释，导致检测器误判。应用 CAHR-CD 和 CAHR-AHS 后，主流护栏的平均检测准确率分别提升了 22% 和 13%。

Conclusion: 护栏性能下降源于注意力机制对长上下文中关键信息的比例稀释而非绝对长度，通过动态调整处理策略可有效提升模型长文本下的安全性。
...
TL;DR: 研究探讨了将生成式AI嵌入本科生课程研究体验（CUREs）中，如何支持生物信息学和基因组学课程中学生的探究、协作与科学推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着研究问题的日益复杂，探索GenAI能否提供个性化支持以帮助本科生开展更具挑战性的科研工作。

Method: 通过三个学期的生物信息学和基因组学CURE课程收集纵向定性数据，分析学生在引入GenAI后的研究行为与学习过程。

Result: GenAI通过提供按需支架帮助学生开展自主课题、成为科研团队分布式认知系统的一部分以促进协作，并促使学生通过验证AI输出培养了核心的科研判断力。

Conclusion: GenAI能够拓展CUREs的广度与深度，在增强学生研究能力与协作的同时，保留了科学探究中不可或缺的人类判断责任。
...
TL;DR: 该论文提出了“带外策略执行”（OBPE）框架，通过在智能体推理路径之外设置可信边界，防止智能体越权访问或泄露敏感数据。


<details>
  <summary>Details</summary>
Motivation: 智能体通过继承人类凭据拥有过大的操作权限，而仅依靠提示词（Prompts）作为防御手段非常脆弱，无法有效限制智能体的行为。

Method: 设计了一个基于HTTP代理的OBPE系统，利用Cedar策略语言对资源和操作进行类型化授权，在调用后端前缩小查询范围，并对响应结果进行过滤或遮盖。

Result: 在3,621次红队测试任务中，OBPE将数据泄露和违规操作的失败率从57.6%降低至0.2%，同时显著提升了“安全且有效”的任务完成率。

Conclusion: OBPE能够有效限制智能体的权限范围，通过强制性的外部策略边界，即使智能体被误导也无法绕过预设的安全上限。
...
TL;DR: 该研究将对象中心过程挖掘（OCPM）引入协同预测过程监控（PPM），通过将多实体交互显式化，改进了跨组织协作流程的预测能力。


<details>
  <summary>Details</summary>
Motivation: 传统PPM方法在处理跨组织协作时主要基于单案例视角，忽略了参与者、消息及全局流程之间的复杂依赖关系。

Method: 提出了从扩展协作日志到OCEL 2.0格式的语义映射框架，将协同预测任务重构为对象中心预测任务，并利用表格、序列和图原生编码进行验证。

Result: 通过五个数据集的实验表明，该方法能够明确协作结构，支持跨越传统案例分类的预测目标，尽管增加了关联关系的复杂性。

Conclusion: 利用对象中心方法可以有效捕捉协作流程中的复杂依赖，但同时也依赖于特定的工具链和更复杂的模型架构。
...
TL;DR: 该研究探讨了如何通过构建云端代理环境及互动培训工作坊，降低科研人员使用Agentic AI辅助生物数据库（如基因本体论GO）策展的门槛。


<details>
  <summary>Details</summary>
Motivation: Agentic AI在生物知识库策展中潜力巨大，但由于访问受限、技术门槛高及缺乏培训，导致科研社区采纳率较低。

Method: 基于JupyterHub平台搭建云端Agent环境，利用Claude Code作为统一交互接口，并设计了四个由浅入深的培训模块，针对GO-CAM路径策展进行实践教学。

Result: 通过对37名参与者的培训发现，移除技术壁垒、循序渐进地引入能力、将练习与熟悉任务结合，能显著提升社区成员使用AI辅助策展的能力。

Conclusion: 构建分布式科学社区的AI协同能力，核心在于解决访问权限、工作流设计和专业培训问题，而非仅仅是技术本身。
...
TL;DR: 本文提出了PCFBench，这是一个专门评估AI在产品碳足迹（PCF）估算任务中表现的基准测试，旨在解决现有评估方式无法反映复杂任务中各步骤准确性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的AI模型评估通常只看重最终结果，忽视了复杂PCF估算工作流中中间步骤的准确性和逻辑一致性，导致评估结果缺乏透明度且难以支持实际的减排决策。

Method: 构建了包含614个专家标注条目的基准测试集，涵盖了分解、检索、本体匹配和数值提取等六个独立任务，并对八个主流大语言模型进行了评估。

Result: 研究发现，尽管模型在整体估算上表现尚可，但在分步推理和遵循质量守恒定律等约束条件方面表现不佳，准确率显著下降。

Conclusion: 当前领先的LLM在需要严格逻辑推理和数值约束的专业工作流中仍存在显著缺陷，该基准测试有助于推动AI在这一高风险领域的可信度和可解释性研究。
...
TL;DR: 该研究提出了一种通过生成模型采样来直接获取视觉语言模型（VLM）“感知先验”的方法，以揭示模型隐式预期的分布情况。


<details>
  <summary>Details</summary>
Motivation: 现有的可解释性研究侧重于固定输入下的模型行为，但无法准确重构模型对现实世界中高维输入空间的隐式预期（先验），而这些先验深刻影响着模型的实际表现。

Method: 通过控制生成模型产生刺激，并利用目标模型作为判别器进行Gibbs采样，从而直接从模型的感知先验分布中进行采样。

Result: 成功重构了多种类别和属性的感知先验，不仅恢复了已知的刻板印象，还发现了通过直接提示（prompting）无法观测到的新型先验偏差。

Conclusion: 该方法有效揭示了深度学习模型的隐式偏见，为研究模型如何理解世界提供了新视角，并呼吁对这些先验的下游影响进行更深入的探讨。
...
TL;DR: 研究探讨了工具增强型大语言模型在有可用证据的情况下仍做出无根据断言的现象，并引入了“发生率”与“条件修复”两个量化指标进行分析。


<details>
  <summary>Details</summary>
Motivation: 语言模型即便在有工具辅助且被明确禁止猜测的情况下，仍倾向于做出无依据的断言，研究旨在揭示并量化这种失败模式。

Method: 定义了“发生率”（独立做出无根据断言的频率）和“条件修复”（补充缺失证据后修正断言的频率）；通过受控实验重现模型状态，对比补全证据与对照组的表现，并测试自动检查规则的有效性。

Result: 实验显示，Qwen3-32B在未获必要证据时常做出无根据断言，但一旦补充正确证据，修复率为100%；自动检查规则能有效纠正错误断言且不影响正确答案。不同模型（如Gemma 4）的表现存在显著差异。

Conclusion: 模型存在即便拥有解决不确定性的工具也不去使用的倾向，通过自动化的证据校验机制可以显著改善这一缺陷，但该现象在不同模型间具有差异性。
...
TL;DR: 本文提出了Credo框架，旨在通过从命令式编码代理中提取结构化的声明式原语，实现LLM应用构建模块的复用。


<details>
  <summary>Details</summary>
Motivation: 现有的编码代理生成的工具链（Harness）是不透明的命令式代码，难以在任务间复用，导致每次新任务都需要从头搜索。

Method: 开发了Credo系统，能够从已搜索的Harness中提取结构化原语，并附带元数据和来源信息，存入目录。通过编译器将这些原语重新组合以适配新任务。

Result: 初步实验证明了从命令式代码中恢复声明式描述的可行性，并验证了通过目录复用原语构建新Harness的潜力。

Conclusion: 该方法为LLM应用开发开辟了新路径，建议利用数据库领域的成本优化和目录管理技术来解决模型漂移和复用效率问题。
...
TL;DR: 提出了ReToolSQL框架，通过两阶段训练（SFT+RFT）提升Text-to-SQL性能，使单一Gemma 4 (31B)模型在BIRD-SQL基准测试中达到领先水平。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL多为单轮任务，缺乏迭代修正能力，且依赖大规模模型，需要一种能通过执行反馈进行自我修复的高效训练方案。

Method: 采用两阶段训练：首先对拒绝采样的推理轨迹进行监督微调（SFT），随后进行基于多轮工具使用轨迹的智能体强化学习微调（RFT）。

Result: Gemma 4 (31B)模型在BIRD-SQL开发集上达到74.32%的单次执行准确率，配合自洽性提升至74.77%，排名第一。

Conclusion: 合理的SFT至RFT流水线能有效利用工具使用轨迹，无需额外人工标注，是实现鲁棒的企业级Text-to-SQL的高效路径。
...
TL;DR: CEDAR 提出了一个将自然语言指令转化为正则语言（确定性有限自动机）的框架，通过这种形式化表示来约束具身智能体的行为。


<details>
  <summary>Details</summary>
Motivation: 现有的代码生成大模型在处理持续性约束（如“在夜间睡觉”或“保持在特定生物群系”）时缺乏稳定性，难以进行验证、组合或修复。

Method: CEDAR 将指令建模为环境事件轨迹上的正则语言，利用大语言模型进行语义判断，结合执行轨迹进行反例引导修正，并将技能与约束统一表示为确定性有限自动机（DFA）。

Result: 在 Minecraft 任务中，CEDAR 能够有效地维持时空约束，优于仅依靠代码生成的基准模型，并减少了重复的大语言模型查询。

Conclusion: 将自然语言指令转化为正则语言提供了一种可执行的验证层，是解决具身智能体长期约束保持问题的有效路径。
...
### [27] [AcrossVAM1.0: Particle World Modeling for Text-Assisted Robot Video Prediction](https://arxiv.org/abs/2608.28491)
*Yafei Zhang,Nan Wu*

Main category: cs.AI

TL;DR: AcrossVAM1.0 提出了一种将机器人视频预测拆解为对象中心运动建模与密集外观合成的轻量级模型。


<details>
  <summary>Details</summary>
Motivation: 传统像素模型难以兼顾精确运动推理与高频外观保留，且常被“保持最后一帧”的简单基准所掩盖。

Method: 利用SAM3-DLP将视频分解为语义粒子，通过轻量级时空Transformer进行状态推演，并结合OpenCLIP指令引导，最终由双流解码器通过残差修正生成未来帧。

Result: 在VRS基准测试中，粒子动力学较持久化基准降低了21%的轨迹误差，且显著提升了运动区域的PSNR。

Conclusion: 显式粒子动力学是机器人视频预测的高效低维接口，但鲁棒的语言接地和外观呈现仍是核心挑战。
...
### [28] [CURA: Certified Runtime Alarms for Computer-Use Agents](https://arxiv.org/abs/2608.27808)
...
TL;DR: 本文提出了CURA（计算机使用智能体认证运行时警报），这是一种能够通过外部遥测数据实时检测智能体任务失败的监控系统，旨在解决智能体自我报告不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的计算机使用智能体（CUA）在任务失败时常错误地自我报告为成功，且缺乏有效的失败预警机制，导致监控失效。

Method: 引入CURA外部监控器，利用遥测数据进行序列分析，采用累积和控制图（CUSUM）算法，在满足特定伪报警率限制的情况下，对智能体执行过程进行在线检测。

Result: CURA在alpha=0.10时，能提前平均31步检测出42.3%的失败，且实际伪报警率为0.066；结合报警闸控机制，使任务成功率从82.9提升至86.8。

Conclusion: CURA提供了一种无需访问智能体内部状态即可进行在线风险控制的有效方案，通过失败预警显著改善了智能体的部署安全性与任务完成度。
...
### [29] [When Robots Mishear Us: Mapping the Safety Risks of Voice-Controlled Embodied AI](https://arxiv.org/abs/2608.28518)
*Sihan Jia,Oliver Lemon*

Main category: cs.AI

TL;DR: 该论文研究了自动语音识别（ASR）错误如何降低具身智能（EAI）模型的安全性，并揭示了ASR错误会导致模型接受并执行有害指令。


<details>
  <summary>Details</summary>
Motivation: 随着具身智能在现实世界应用增多，语音输入作为常用交互方式，其识别错误是否会诱导模型产生不安全行为成为关键的安全隐患。

Method: 通过模拟ASR错误并将其结合现有的SafeAgentBench和POEX安全基准进行测试，评估不同类型的语音识别错误对EAI模型拒绝机制及规划安全性的影响，并探讨自动校正方法的有效性。

Result: 研究发现某些ASR错误会保留语义结构但增加有害歧义，或直接削弱模型的拒绝能力；尽管自动校正可降低部分风险，但并非总是有效。

Conclusion: ASR错误会对具身智能带来显著的安全风险，现有的安全防护机制在面对语音识别偏差时存在明显脆弱性。
...
TL;DR: 提出了一个名为AcCoRD的基准测试，用于评估AI智能体在用户偏好动态变化（形成、演变、调整）环境下的协作能力。


<details>
  <summary>Details</summary>
Motivation: 现有的协作基准测试仅关注静态或初始不明确的偏好，忽略了现实中用户偏好随交互过程动态变化的复杂性。

Method: 构建了包含在线购物和旅行规划两个领域的AcCoRD数据集，并测试了五种前沿大模型在Vanilla ReAct和不确定性引导提示策略下的表现。

Result: 实验发现前沿模型能处理偏好不明确的情况，但在处理交互过程中出现或演变的偏好时表现不佳，且仅靠提示词难以激发模型识别不确定性的能力。

Conclusion: 开发能够处理复杂、动态用户偏好的AI智能体需要比单纯提示词更高级的不确定性建模方法。
...
TL;DR: 本文提出了一种名为EHSAF的证据基础高阶集合论证框架，统一了现有的证据论证模型，并引入了三值逻辑和模糊逻辑下的语义推理方法。


<details>
  <summary>Details</summary>
Motivation: 现有论证框架在处理证据支持、高阶关系和集体交互方面缺乏统一性，且难以兼顾定性与定量的推理需求。

Method: 构建EHSAF框架，提出了“邻接完全标记语义”（应对支持循环）和“基于扩展的完全语义”（坚持严格证据主义）；通过命题逻辑编码实现计算推理，并扩展至多种连续模糊逻辑。

Result: 证明了两种语义在无环情况下等价，并验证了模糊语义满足连续性、单调性等关键属性，成功实现了定性论证与定量模糊逻辑的桥接。

Conclusion: EHSAF提供了一个通用的数学框架，通过统一的处理机制解决了论证理论中证据链与多值/模糊推理的兼容性问题。
...
TL;DR: 该研究指出当前编码基准测试（如SWE-bench）与真实用户请求在信息量和语言风格上存在巨大差异，并提出了sys基准测试来评估这些差异对模型表现的影响。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试任务过于结构化且冗长，与真实开发场景中简短、非正式的用户请求存在显著差距，导致评估结果可能无法反映模型在实际开发中的性能。

Method: 定义了六类信息分类法和四类语言风格维度，对比了SWE-bench与真实用户请求（SWE-chat），并构建了包含381个多变体任务族的sys数据集以进行控制变量评估。

Result: 现实语境下的输入使模型解决率平均下降了6.4个百分点，且模型排名会发生变化；明确指出“期望行为”和“动机”对提升模型性能至关重要，而冗余的环境信息则无明显帮助。

Conclusion: 目前的基准测试高估了模型性能；通过在提示词中补充缺失的背景信息（动机与期望行为），可以显著提升LLM在软件工程任务中的表现。
...
TL;DR: 提出了一种名为KLOD的知识编辑目标函数，通过约束输出分布防止顺序编辑过程中的分布偏移与局部性退化。


<details>
  <summary>Details</summary>
Motivation: 传统的微调式知识编辑使用交叉熵会导致非目标输出分布的无约束改变，在多次顺序编辑时引发分布偏移和局部性损伤。

Method: KLOD算法在目标概率达到阈值后停止放大，通过KL散度约束保持目标之外的输出分布及前缀位置的完整分布，从而实现受控的知识更新。

Result: 在Llama3和Qwen2.5模型上的实验表明，KLOD在保持高编辑成功率的同时显著缓解了局部性退化，并提供了广义性与局部性之间的可控平衡。

Conclusion: 通过保留非目标输出分布，KLOD有效解决了微调式知识编辑中的分布漂移问题，证明了分布一致性对模型局部性维护的重要性。
...
TL;DR: 该研究利用大规模基准Trip World重新评估了跨城市POI推荐方法，发现现有最先进模型在面对全球尺度、低重叠度和复杂语义数据时表现受限。


<details>
  <summary>Details</summary>
Motivation: 现有跨城市POI推荐研究受限于小规模基准，其结论在真实、大规模、多地域环境下的适用性存疑。

Method: 使用Trip World大规模基准进行实证研究，对比代表性SOTA方法，并对基于代理（agentic）的下一POI推荐方法进行了诊断实验。

Result: 发现现有方法主要依赖目标区域先验而非用户偏好迁移，在大规模下表现退化且语义集成效果不佳，代理方法未能有效利用现有语义信号。

Conclusion: 目前的推荐模型缺乏真正的跨城市偏好迁移和语义理解能力，亟需针对大规模、跨城市场景的任务特定设计。
...
TL;DR: 提出了一种基于“严重性感知一致性临床规划”的交互式医疗诊断框架，通过风险敏感的决策规划来优化问诊过程，平衡诊断准确性与临床风险。


<details>
  <summary>Details</summary>
Motivation: 现有交互式诊断方法多关注预测不确定性，忽略了漏诊严重疾病的临床风险不对称性，且缺乏对何时结束问诊进行长期规划的能力。

Method: 将交互式诊断建模为风险敏感的序列决策问题；引入一致性预测（Conformal Prediction）校准诊断结果；结合严重性加权风险与蒙特卡洛树搜索（MCTS）进行长序列问诊与诊断决策的优化。

Result: 在DDXPlus和MediQ数据集上，该方法在多个大模型上实现了更高的诊断准确率，减少了问诊次数，并显著降低了严重病例的漏诊风险。

Conclusion: 证明了临床风险信号在交互式诊断规划中的有效性，为实现更安全、高效的AI辅助医疗诊断提供了新范式。
...
TL;DR: 提出了一种名为SpikeOPD的在策略蒸馏框架，旨在解决自回归脉冲神经网络（SNN）在训练过程中因生成前缀与固定语料前缀不匹配导致的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的ANN转SNN蒸馏方法在面对自回归推理时，存在输出策略不匹配和脉冲动态漂移问题，导致模型性能下降。

Method: 引入SpikeOPD框架，采用全KL散度校正以减少策略失配，结合匹配前缀策略锚定（anchoring）维持训练稳定性，并辅以层级脉冲正则化以控制发放率偏移。

Result: 在0.125B、0.35B和1.3B三种规模的模型上，SpikeOPD相比传统KD SNN准确率分别提升了0.8、1.7和2.9个百分点，且保持了脉冲网络的稀疏计算优势。

Conclusion: 通过在自回归生成的前缀上进行策略锚定和正则化，SpikeOPD能有效缓解在策略蒸馏中的反馈崩溃问题，显著提升了SNN在语言建模任务中的性能。
...
TL;DR: 本文提出了一种名为CoRe-MoE的紧凑型可重用专家混合模型，旨在高效解决多模态大模型的持续指令微调问题。


<details>
  <summary>Details</summary>
Motivation: 现有的LoRA-MoE方法在持续学习中需要频繁添加完整LoRA专家，导致严重的参数冗余和扩展开销。

Method: 通过分析LoRA更新的奇异值分解（SVD），发现任务间存在共享的方向子空间，据此提出提取可重用的方向基底，仅对坐标专家和轻量级路由器进行训练。

Result: 在两个代表性多模态大模型上，CoRe-MoE性能超越了现有最强基线（提升达5.90点），且后续任务的训练参数量不足原方法的1%。

Conclusion: CoRe-MoE通过复用共享子空间显著提升了持续学习的参数效率和模型表现，证明了无需为每项任务完整扩展专家的可行性。
...
TL;DR: MAGE 是一种基于多智能体协作的框架，旨在从观测数据中发现偏微分方程（PDE），通过模拟科学发现的循环过程，摆脱了对预定义方程库的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有的 PDE 发现方法存在受限于预定义函数库、对噪声敏感、易产生幻觉或迭代优化能力不足的问题，需要更稳健且结构化的方法。

Method: 提出了 MAGE 框架，包含四个专业化智能体：微分观测者（计算导数）、现象提取器（VLM 提取定性特征）、定律合成器（LLM 生成候选方程）和方程仲裁者（拟合系数并打分），形成“假设-验证-拒绝”的闭环。

Result: 在 8 个典型 PDE 系统中实现了 8/8 的结构恢复，系数估计误差相比现有方法平均提高了 3 个数量级，并成功应用于复杂几何形状和实验室传感器数据。

Conclusion: 研究证明了结构化多智能体推理在无需预设库的情况下发现物理定律的有效性，为自动化科学发现提供了新思路。
...
TL;DR: HyQuant 提出了一种用于大语言模型（LLM）注意力机制的混合精度量化框架，通过保留关键的“垂直线”Token和局部窗口以高精度计算，同时对其他部分进行低位量化，从而在保持准确性的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型量化方法在处理注意力模块时，低位量化常导致较大的性能下降，传统的平滑技术难以在低比特下取得理想效果。

Method: 提出了HyQuant框架，在Prefill阶段对垂直线Token和局部滑动窗口保持高精度，其余部分进行低位量化；在Decode阶段将该策略应用于KV缓存压缩，并融合反量化操作以提升硬件效率。

Result: 实验证明，HyQuant在多种模型、任务和数据集上实现了接近无损的精度，并有效降低了计算与内存开销。

Conclusion: HyQuant通过一种简单有效的混合精度设计，解决了注意力模块在低位量化下的性能损失问题，具有极高的实践价值。
...
TL;DR: 该论文对比了OpenClaw和NanoBot两种智能代理系统的性能，分析了它们在任务完成度、资源消耗及评估方法论上的差异。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI的发展，评估智能代理系统（结合语言模型、工具、记忆等）的能力与资源负担变得至关重要，现有的评估方式需更严谨。

Method: 通过配对基准测试（Primary Benchmark）和仪表化的详细子集（Instrumented subset）对两个系统进行对比，监测任务完成率、执行时间、内存占用及成本。

Result: 在主要基准测试中，两者无显著差异（OpenClaw 31% vs NanoBot 25%）；在仪表化测试中，NanoBot在资源利用率（时间和内存）和部分完成率上表现更好，但存在较多低成本的共同失败案例。

Conclusion: AI代理系统的评估应建立在“任务完成度”与“资源测量”的深度连接之上，强调执行过程的记录透明度，而非单一的成功指标。
...
TL;DR: 提出了一种名为RCCA的强化学习框架，通过将功能性评估标准（Rubric）与特定的代码片段对齐，解决了交互式Web应用生成中的信用分配问题。


<details>
  <summary>Details</summary>
Motivation: 标准的GRPO方法将复杂应用的评估结果简化为单一序列奖励，导致模型无法准确识别代码中各部分的贡献，难以有效优化功能性需求。

Method: 引入Rubric-to-Code Credit Assignment (RCCA)，通过层次化奖励机制拆分失败类型，并将文本归因与对应的代码片段及Token对齐，实现精细化的信用分配。

Result: Ling-RCCA-Flash在MiniAppBench上达到41.25分，在ArtifactsBench上达到76.19分，显著超越基线模型及GPT-5，展现了卓越的Web应用生成性能。

Conclusion: 基于明确功能标准的精细化信用分配对于提升模型生成复杂、高质量交互式应用的能力至关重要。
...
TL;DR: 该综述从博弈论视角审视了人工智能对齐问题，探讨了在复杂人类价值观下的AI对齐挑战。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法难以应对现实世界中语境依赖、非传递性和动态多方交互的复杂人类偏好。

Method: 围绕博弈论的关键要素进行组织，并根据偏好多样性、对齐优先级和时间动态性三个挑战对相关文献进行了综合梳理。

Result: 厘清了博弈论分析在当前对齐方法中的实际作用，并指出了现有框架的局限性。

Conclusion: 构建鲁棒、自适应且可验证的AI系统需要更深入的博弈论框架，以应对动态交互环境中的对齐挑战。
...
TL;DR: 该研究提出了一种通过高质量合成数据生成和微调SLM来优化金融问答系统的流程，并引入了基于算术表达式匹配的评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有金融问答系统存在评估指标（如Exact Match）不准确、模型性能一致性差以及难以处理多模态复杂数据的问题。

Method: 利用QLoRA微调小型语言模型（SLM），通过严格的数据验证生成合成数据，并提出了一种基于算术表达式匹配的评估指标及结合语义相似度的改进损失函数。

Result: 在ConvFinQA基准测试中，该方法显著提升了问答准确率，且新指标更准确地反映了模型的推理能力。

Conclusion: 高质量合成数据与优化的损失函数能有效增强模型在金融领域复杂算术推理任务中的表现。
...
TL;DR: 提出了一种基于两层堆叠集成学习框架的航空发动机剩余寿命（RUL）预测方法。


<details>
  <summary>Details</summary>
Motivation: 旨在解决单一深度学习模型在处理复杂退化模式时存在的偏差问题，以提升预测精度和鲁棒性。

Method: 集成四种异构深度学习模型（LSTM、CNN、CNN-LSTM、CNN-GRU）作为基学习器，并利用XGBoost作为元学习器进行预测结果的堆叠融合。

Result: 在NASA C-MAPSS数据集（FD001/FD003）上实现了更低的RMSE（9.989和8.613），相比当前最优基准（TCAT）性能提升显著。

Conclusion: 堆叠集成方法有效降低了预测误差，在航空发动机等关键安全领域的剩余寿命预测中展现出卓越的性能。
...
TL;DR: CASTANET 是一种利用时空图神经网络和因果推断技术来预测突发事件引起的非周期性交通拥堵的模型。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在预测由事故或道路损坏等突发事件引发的非周期性拥堵时表现不佳，主要原因是事件发生稀疏且存在空间和时间上的选择偏差。

Method: 结合了时空图神经网络（ST-GNN）与因果处理效应估计（Causal Treatment Effect Estimation），以显式处理事件记录并减轻数据选择偏差。

Result: 在东京真实交通与事故数据上，CASTANET 在整体 RMSE 上提升了 4.0%，在事故相关评估中提升了 10.1%，在严重拥堵场景下提升达 14.55%。

Conclusion: 该模型通过引入因果推断框架有效解决了事件驱动的交通预测中的稀疏性与偏差问题，显著提升了非周期性拥堵的预测准确性。
...
TL;DR: 研究发现大型语言模型在跨会话场景下存在“组合安全风险”，即通过将分散在不同交互中的无害子查询重新组合，可实现违规目标，且模型规模越大，此类风险越高。


<details>
  <summary>Details</summary>
Motivation: 现有的缩放定律（Scaling Laws）通常关注能力提升，而研究者试图揭示随着模型规模扩大，在跨会话组合攻击下的潜在安全威胁。

Method: 形式化定义了“组合安全风险”并推导了风险转移边界；进行了合成保留实验分析模型对未见指令的预测能力；利用Qwen3和Gemma3系列模型评估跨会话攻击的有效性；并提出了IntentAlign-MiniLM模型作为防御手段。

Result: 实验显示，更大的Transformer模型对未明确训练但可重构的指令表现出更低的损失值；更大规模的模型在固定的分解-组合管道下展现出更高的有害能力提升；IntentAlign-MiniLM在意图检索和有害行为召回方面优于大型嵌入模型。

Conclusion: 模型能力的增强伴随着潜在的跨会话组合攻击风险，通过专门的意图对齐防御手段可以有效缓解此类由规模带来的安全挑战。
...
### [47] [When Teacher Guidance Misleads: Reward-Aligned On-Policy Distillation](https://arxiv.org/abs/2608.27960)
*Siyuan Gan,Yuhan Li,Xiran Wang,Linjian Meng,Boyan Wang,Zhen Zhao,Jing Huo,Yang Gao*

Main category: cs.AI

TL;DR: 本文提出了奖励对齐的策略蒸馏（RA-OPD）方法，通过过滤掉与最终奖励不一致的教师指导轨迹，提高了大语言模型在线策略蒸馏的效果。


<details>
  <summary>Details</summary>
Motivation: 在线策略蒸馏中，教师模型对学生生成的中间步骤提供的指导可能存在误导，导致学生偏离正确路径，降低模型性能。

Method: RA-OPD方法通过检查轨迹级别的蒸馏收益与最终奖励的一致性，自动过滤掉误导性的轨迹，仅保留对学生学习有益的样本进行训练。

Result: 在Qwen3和DeepSeek-R1系列模型上的七个数学基准和三个代码基准测试中，RA-OPD显著优于标准在线策略蒸馏及其他变体。

Conclusion: 通过引入奖励一致性约束，RA-OPD能有效识别并利用更可靠的教师指导，在不增加计算开销的情况下显著提升了学生模型的表现。
...
### [48] [The Illusion of $\textit{What If}$: Evaluating the Breakdown of Counterfactual Reasoning in LLMs](https://arxiv.org/abs/2608.27953)
*Yucheng Wang,Yuetian Du,Zhengyi Liu,Rongyu Zhang,Bing Zhao,Boyu Yang,Ming Kong,Lin Qu,Hu Wei,Jie Liu,Qiang Zhu*

Main category: cs.AI

TL;DR: 本文提出了WhatIfBench，这是一个针对开放域、长视野反事实因果推理的基准测试，并配套了名为PRISM的评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有基准多局限于固定变量或单一结果，缺乏对复杂开放域场景下因果过程推演能力的评估。

Method: 构建包含220个跨学科反事实问题的WhatIfBench；开发PRISM框架，将自然语言解释转化为因果图，通过过程度量和标准度量综合评估因果有效性与解释充分性。

Result: 即使是最强的LLM在WhatIfBench上的得分也仅为64.62%，表现出因果推理中的因果断裂、前提漂移和拓扑破碎等问题。

Conclusion: 当前大模型在处理流畅的反事实叙述时，往往掩盖了其脆弱的因果推理过程，反事实推理能力仍有很大提升空间。
...
TL;DR: SABER是一种针对大型推理模型（LRM）的训练即插即用型提前退出框架，通过对抗性分支探测来评估推理稳定性，在保持精度的同时大幅降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 长链推理中，当中间答案稳定后继续推理会造成严重的计算资源浪费，而现有的提前退出方法在捕捉推理稳定性方面表现不佳或效率低下。

Method: 提出SABER框架，通过在中间推理状态构建语义扰动生成对抗性分支，并使用轻量级探测技术预估最终结果，若探测结果在各分支间保持一致则触发提前退出。

Result: 在多个推理基准和模型架构上，SABER在保持准确率的前提下，平均减少了30.2%至39.8%的推理Token消耗。

Conclusion: SABER有效地平衡了推理效率与可靠性，为优化长链推理模型提供了一种无需额外训练的通用解决方案。
...
TL;DR: 提出了一种名为AERA的测试时推理计算自适应分配框架，通过学习评估“是否需要进一步计算”而非单纯依赖当前的置信度。


<details>
  <summary>Details</summary>
Motivation: 现有的自适应停止方法依赖于置信度或答案稳定性，但这些指标在非单调进化的推理过程中往往失效，导致计算资源浪费。

Method: 引入自适应证据残差分配（AERA），利用包含答案分布、时间、重解、语义和计算特征的序列控制器，基于响应前缀动态决策是否继续推理。

Result: 在GSM8K和GPQA Diamond测试中，AERA在保持高准确率的同时，大幅减少了推理所需的token数量（在GSM8K上减少了约96%的计算量）。

Conclusion: 自适应推理应转向评估计算带来的“预期收益”，而非仅仅根据当前的输出信心判定任务是否完成。
...
TL;DR: openJiuwen 是一个开源的编码智能体编排框架，专注于解决长周期编程任务中的结构化可组合性和运行时自适应性挑战。


<details>
  <summary>Details</summary>
Motivation: 随着编码智能体系统向多智能体协作和复杂化发展，开发者面临难以复用编排逻辑（结构化可组合性）以及无法根据实时执行反馈动态调整任务决策（运行时自适应性）的问题。

Method: 引入基于 Rail 的能力组合机制，利用共享执行基底构建可复用的智能体编排方案，并通过框架控制的运行时决策机制，根据实时诊断、执行结果和任务进度动态调整上下文与任务流。

Result: 在 SWE-bench Verified 和 Terminal-Bench 2.1 上分别达到 82.6% 和 87.19% 的准确率，性能超越了现有排行榜的最强基准模型。

Conclusion: openJiuwen 证明了通过统一的执行语义和自适应控制框架，可以在保持高度可组合性的同时，显著提升复杂编程任务的执行成功率。
...
TL;DR: 该论文提出了DA3PO算法，通过引入直接优势放大（DAA）机制，解决了DAPO算法在处理复杂提示时训练效率低的问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: DAPO算法中的动态采样虽然提升了稳定性，但会导致模型过度关注错误响应而非利用高质量的难采样正确响应，从而降低了训练效率。

Method: 引入Direct Advantage Amplification (DAA) 技术，在动态采样的基础上，专门放大复杂提示下“难采样正确响应”的优势，并将其整合进DAPO形成DA3PO。

Result: DA3PO在保持代码简洁（较DAPO变动少于30行）的同时，实验表现显著优于GRPO及其他经典变体。

Conclusion: 通过针对性的优势放大策略，DA3PO有效平衡了训练稳定性和效率，是提升GRPO类算法性能的有效方法。
...
TL;DR: 提出了一种名为 K-GAT 的神经符号框架，通过将外部知识直接整合到多智能体协作拓扑的生成过程中，解决现有方法中存在的结构与知识不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度依赖大语言模型（LLM）的参数知识，导致在知识密集型任务中协作结构冗余或验证不足，缺乏有效的结构-知识对齐。

Method: 将协作拓扑设计形式化为知识条件化的结构学习问题，利用神经符号方法将外部检索证据直接嵌入到自回归图生成模型中。

Result: 在知识密集型基准测试（如 GPQA）中表现优异，准确率较 LLM-Debate 基准提升 15.7%，且计算令牌消耗减少了一半以上。

Conclusion: K-GAT 有效实现了知识与协作结构的深度对齐，在保证高效性的同时显著提升了复杂任务中的决策准确度。
...
TL;DR: GOD (Generative-agent Operating Desk) 是一个为多智能体系统设计的控制室，旨在提供交互式检查、干预及实验重现功能。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式智能体系统缺乏有效的监控、调试和重现工具，导致研究人员难以理解智能体行为或进行干预测试。

Method: 开发了集成控制室，包含Agent Studio、Map Studio和空间重现接口；引入了“命令与工件循环”模型，实现实时干预与状态记录的解耦与标准化。

Result: 通过15次运行实验验证，智能体对目标命令的执行成功率高（78/84），且状态查询与实际记录的匹配度良好（169/182）。

Conclusion: GOD有效提升了生成式智能体系统的可解释性和可操作性，通过标准化包管理和交互式命令循环，促进了多智能体实验的复现与研究。
...
TL;DR: 该论文提出了一种名为aeSFT的统计检验方法，用于评估合成数据（来自数字孪生或世界模型）能否有效提升AI模型在真实数据上的性能，同时最大限度地减少对真实测试数据的消耗。


<details>
  <summary>Details</summary>
Motivation: 工程系统中存在模拟与现实（sim-to-real）的鸿沟，盲目使用合成数据扩充训练集可能导致性能下降，需要一种高效、统计上可靠的方法来判断增广是否有效。

Method: 提出了两种统计决策方案：一种基于均值损失差异的直接检验，另一种是基于对称性的配对损失差异检验。核心算法aeSFT结合了自适应蒙特卡洛符号翻转与实时数据消耗策略，具备任意时间有效的I类错误控制。

Result: 实验证明，aeSFT在分类任务、无线调度和无线电预测任务中，能在显著减少真实测试样本需求的情况下，保持与固定样本量检验相当的检验效能，并有效控制假阳性率。

Conclusion: aeSFT是一种高效的自动化工具，能可靠地评估合成数据集的价值，降低对稀缺真实测试数据的依赖，从而在保持模型可靠性的前提下加速AI训练。
...
TL;DR: 提出了一种基于多智能体大模型的自动化框架，用于多语言科学文献的筛选、信息提取与标准化分析。


<details>
  <summary>Details</summary>
Motivation: 传统手工分析和单算法方法在处理跨学科、多语言文献时面临效率低、可扩展性差及领域适应性不足的问题。

Method: 构建了一个由中心协调模块和三个专业智能体（评估、提取、分析）组成的框架，并采用了四层幻觉控制策略及人工验证机制。

Result: 在32,642篇气候-健康领域中英文文献的实证中，核心信息提取的F1分数达到0.92，成功提取了2,012组城市-文献关联。

Conclusion: 该框架有效提升了大规模跨学科文献的挖掘效率与准确性，为气候-健康研究提供了可靠的技术支撑。
...
TL;DR: PhenoIntel 是一个基于多智能体架构的网页平台，旨在为植物表型分析提供可靠、科学且用户友好的机器学习工作流。


<details>
  <summary>Details</summary>
Motivation: 现有的对话式植物表型工具缺乏科学研究所需的可靠性，存在错误报告、统计假设检验缺失、缺乏不确定性评估以及硬件要求高等局限性。

Method: 采用生命周期对齐的多智能体系统，通过九个分工明确的智能体处理图像收集、模型选择、推理和报告；引入阶段性独立检查机制、针对性的不确定性量化策略以及自动化的模型集成与验证流程。

Result: 分类模型 Macro F1 达到 0.78-0.996，目标检测模型 mAP@50 达到 0.96 并将计数误差降低了 54%，且平台可在标准硬件上通过浏览器运行，无需 GPU 支持。

Conclusion: PhenoIntel 通过模块化的多智能体协同，实现了具备校准不确定性、科学验证统计数据及符合 FAIR 原则的植物表型分析，有效解决了现有工具在科研可靠性上的不足。
...
TL;DR: 研究探讨了在使用零阶/进化策略（ZO/ES）优化工具使用型LLM代理时，基于轨迹的信度分配（Credit Assignment）能否有效地将计算预算引导至导致故障的模块。


<details>
  <summary>Details</summary>
Motivation: 旨在通过信度分配策略优化代理的性能，评估将有限的优化预算定向分配给特定模块是否优于简单的均匀分配。

Method: 通过在多个模型（Qwen2.5, SmolLM2）和任务场景中进行实验，对比了包括均匀分配、软分配、argmax分配等六种分配方案，并采用了符号翻转测试和逆倾向得分去偏等技术手段。

Result: 实验发现信度导向的路由分配相比均匀分配没有统计学上的显著提升，甚至在某些情况下会因误导路由而导致性能下降；性能损失与优化瓶颈的资源匮乏程度成线性关系。

Conclusion: 在当前实验设定下，基于信度的路由分配未能显著提升优化效率，研究同时记录了三种可能导致ZO/ES实验失效的隐蔽故障模式。
...
### [59] [WeAgent-MMSearch: Native Text-Vision Interaction for Multimodal Search Agents](https://arxiv.org/abs/2608.28062)
*Zongkai Liu,Hui Zhang,Liqiang Niu,Zhen Cao,Han Li,Juntao Liu,Wenchao Chen,Chengduo Zhao,Chao Yu,Fandong Meng*

Main category: cs.AI

TL;DR: WeAgent-Harness是一个支持多模态交互和运行时故障恢复的智能体框架，配合WeAgent-MMSearch系统，显著提升了多模态搜索智能体的规划与推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有搜索智能体往往忽略工具返回的图像信息，且在长序列交互中难以应对工具调用失败、超时等问题，导致性能下降。

Method: 引入WeAgent-Harness（支持持久化图像引用与运行时恢复）和WeAgent-MMSearch（含自动数据合成、故障感知GSPO训练及多模态推理）。

Result: 在VisTarget-Bench及七个基准测试中，平均得分提升19.22分，性能超越同规模开源模型，并媲美十倍参数量的大型模型。

Conclusion: 通过增强多模态交互能力和实施故障感知训练，可以有效解决智能体在长序列多模态任务中的推理瓶颈。
...
### [60] [String: An Agentic OS Where Every App Is a Markdown File](https://arxiv.org/abs/2608.28027)
*Jookyung Song,Nojun Kwak,Simyung Chang*

Main category: cs.AI

TL;DR: 本文提出了String，一个专为LLM智能体设计的开源运行时环境，通过将工具知识与智能体上下文解耦，使用SFMD（String-Flavored Markdown）格式统一管理应用视图与交互，从而显著降低Token消耗并提升智能体执行准确率。


<details>
  <summary>Details</summary>
Motivation: 现有网页和工具接口均为人类或传统程序设计，LLM智能体在这些环境中处理冗余信息会产生高额Token开销，且因缺乏专门的界面交互逻辑，常面临动作选择失误等问题。

Method: 引入String运行时，将工具定义从智能体上下文中剥离；利用SFMD格式定义应用视图和动作；通过/open和/act两个核心指令实现状态管理、鉴权和动态视图渲染；采用分层级、按需披露的交互策略。

Result: 在87项任务的基准测试中，String在保持成功率的同时降低了33.5%的Token消耗，并将智能体交互界面固定在常数53个Token，错误动作选择率从28%下降至2%。

Conclusion: 将LLM智能体接口视为操作系统层面的问题进行架构设计，通过统一的元数据标准和按需披露策略，能有效提升智能体的执行效率、安全性和通用性。
...
TL;DR: 该论文提出了一种用于激励性广告场景的离线基于模型的强化学习（Offline-MBRL）框架，旨在平衡预付激励成本与后期广告收入。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能解决激励分配中的延迟收益、成本敏感性及用户参与度的持续影响问题，且缺乏针对该场景的顺序决策算法。

Method: 将激励分配建模为马尔可夫决策过程（MDP），开发了一个学习用户反馈和广告收入的世界模型，并采用保守策略优化，结合独立反事实评分器进行离线评估。

Result: 实验表明，所提框架的离线评分信号稳定，MB-IQL方法在人均净利润上比TD3+BC提高了7.96%，且优于普通的IQL方法。

Conclusion: 该框架通过结合因果推断与离线强化学习，有效实现了成本可控的顺序激励分配，并在工业级数据与在线测试中验证了其经济效益。
...
### [62] [Stay Within Your Bounds: Distance-Guided Decoding for Guaranteed Context-Free Grammar Compliance](https://arxiv.org/abs/2608.28229)
*Vincenzo Collura,Karim Tit,Eleonora Giunchiglia,Mike Papadakis,Maxime Cordy*

Main category: cs.AI

TL;DR: 提出了一种基于下推自动机（PDA）的预见引导式（lookahead-guided）解码框架，用于解决大语言模型生成结构化数据时的语法约束问题。


<details>
  <summary>Details</summary>
Motivation: 现有的局部前缀可行性检查在处理“分词器-语法不匹配”和有限计算预算时，无法确保最终生成的文本能被语法完全接受。

Method: 离线计算带有可达性标签和距离上限的下推摘要，在线阶段利用这些估计值进行视界感知（horizon-aware）的修剪和束搜索（beam search）。

Result: 在JSON、SQL和线性时序逻辑（LTL）任务中，该方法在保证严格语法正确性的同时，提升了输出质量。

Conclusion: 通过引入预见性分析，能够有效避免陷入死胡同，显著改善大语言模型生成结构化输出的可靠性和准确性。
...
TL;DR: 提出了一种利用LLM推测解码（speculative decoding）模块进行实时高效分类的方法，在不增加额外计算开销的情况下实现高精度分类。


<details>
  <summary>Details</summary>
Motivation: 现有实时分类方法在精度与效率之间存在权衡：简单的探测器缺乏上下文感知能力，而复杂的分类模型（如LLM-as-judge）计算成本过高。

Method: 通过在序列末尾附加训练好的软提示（soft prompt），将现有的推测解码模块重构为序列分类器，并利用已存在于显存中的KV缓存进行计算。

Result: 该方法在四个分类任务中表现优异，性能超越零样本GPT-4o-mini，并在多语言安全任务中达到或超过了专门的8B参数安全分类器水平。

Conclusion: 利用推测解码模块进行分类是一种极具成本效益的策略，能在维持高推理效率的同时实现与专用分类模型相当的精度。
...
### [64] [SEPO: Evidence-Grounded Prompt Optimization via Structural Editing](https://arxiv.org/abs/2608.28067)
*Xiaoyu Ma,Haoyue Liu,Yiwen Li,Jionghao Zhu,Zhichao Wang,Ye Chen,Xiaoying Tang*

Main category: cs.AI

TL;DR: SEPO 是一种新型提示词优化框架，通过结构化编辑和溯源反馈机制取代传统的全字符串重写，提高了优化效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统提示词优化器仅进行事后检查，将提示词视为不透明字符串，缺乏局部可定位性和可操作性。

Method: 引入两层提示词架构，将提示词拆解为可编辑的结构化单元；通过记录编辑操作与其对任务示例（fix/break）的影响，建立编辑效应血统，指导后续优化。

Result: 在 14 个测试任务中，SEPO 优于基准方法 GEPA，性能提升 2.2-3.1 pp，且优化耗时更少、生成的提示词长度缩短了 5 倍以上。

Conclusion: SEPO 通过将提示词优化转化为可寻址、可归因和可操作的任务，在优化效率与模型性能上均达到了 Pareto 前沿。
...
TL;DR: 该研究利用床下非接触式传感器监测痴呆症患者的整晚生理信号，并评估其预测次日躁动风险的效果。


<details>
  <summary>Details</summary>
Motivation: 痴呆症患者的躁动具有短期波动性，目前缺乏基于连续生理信息来预测次日风险的有效监测手段。

Method: 利用423个患者夜晚数据，对比了 nightly 汇总特征、分段特征、全夜序列建模（RNN等）以及滑动窗口多示例学习四种范式，并使用五折交叉验证进行评估。

Result: 全夜序列建模表现最优（AUROC 0.692，AUPRC 0.849），且保留分钟级时间结构的建模方法优于传统的整晚汇总方法，重点关注核心睡眠期的活动、心率及呼吸率。

Conclusion: 非接触式过夜传感可作为预测痴呆患者次日躁动风险的有效工具，但目前模型校准度有限，在临床应用前仍需前瞻性验证。
...
### [66] [The Shape of Power: A Multilingual Framework for Social Power Reasoning in Dialogues](https://arxiv.org/abs/2608.28144)
*Farah Atif,Sougata Saha,Monojit Choudhury*

Main category: cs.AI

TL;DR: 该研究提出了一个用于分析多语言电影剧本中社会权力的框架，并构建了包含法语和埃及阿拉伯语电影标注数据的首个跨文化语料库，同时评估了大型语言模型在跨文化社会推理方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有计算社会权力研究局限于狭窄的文化设置，缺乏具有人口统计学和关系深度的多语言数据集，且难以进行稳健的跨文化分析。

Method: 结合社会科学理论构建标注框架；开发多语言标注管道与界面；构建包含15,836个标注实例的法语和埃及阿拉伯语电影语料库；利用该语料库对6种LLM和多模态LLM进行社会推理能力评估。

Result: 实验发现人类标注者在可观察属性上达成高度一致，但在社会解释性维度上存在争议；LLM在关系推理和心智理论方面与人类表现仍存在显著差距。

Conclusion: 研究提供了一个可扩展的多语言社会权力分析框架，揭示了跨文化社会推理的复杂性，并指出了现有模型在处理此类任务时的局限性。
...
TL;DR: 该研究探讨了能否利用Reactome生物数据库中人工编写的文本描述，自动推断并重构其专家定义的全球层级结构。


<details>
  <summary>Details</summary>
Motivation: 手动策划生物知识库存在扩展性挑战，而现有的NLP方法多关注局部关系，缺乏对文本描述如何反映整体生物层级结构的研究。

Method: 利用SPECTER2模型对Reactome的人类路径描述进行向量化，结合改进的凝聚嵌套算法和图重构算法，构建“语义层级”，并通过拉普拉斯谱距离、自举法及拓扑指标进行验证。

Result: 定量与定性分析证实，可以通过专家编写的文本元数据有效推断出Reactome原有的全局层级结构。

Conclusion: 生物知识库中蕴含的文本描述不仅是补充信息，还编码了复杂的生物学层级逻辑，可用于自动化扩展和优化生物知识库。
...
### [68] [CrabOS: An Operating System for Human-AI Co-inhabitation](https://arxiv.org/abs/2608.28165)
*Qi Yang,Yun Ma*

Main category: cs.AI

TL;DR: 本文提出了“人机共居”（Human-AI Co-inhabitation）工作环境的概念，并设计实现了 CrabOS 操作系统，旨在解决人类与 AI 在处理复杂任务时因工作环境隔离导致的交接成本高、扩展性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 代理系统与人类工作环境分离，导致在任务交接时需要依赖额外的桥接工具（如手动截图或定制接口），这种方式效率低下且难以在大规模任务中扩展。

Method: 设计实现 CrabOS 系统，通过将工作状态统一抽象为人类与 AI 均可读写的自然语言文本对象，实现无需额外桥接的直接交互与状态共享。

Result: 通过案例研究证明，CrabOS 将人机协作从应用级的补丁方案提升为操作系统原生的能力，显著简化了任务交接流程。

Conclusion: CrabOS 为开发和运行需要频繁交互的 AI 代理提供了一个更加高效、原生的操作系统级基座。
...
### [69] [Generative AI Alignment with Hinduism's Theological Plurality and Sacred Representation](https://arxiv.org/abs/2608.28228)
*Dipto Das,Arpita Kundu,Nusrat Jahan Mim,Shion Guha,Syed Ishtiaque Ahmed*

Main category: cs.AI

TL;DR: 本研究探讨了孟加拉国印度教徒如何与生成式AI互动，揭示了技术在宗教实践中的潜力与伦理隐忧。


<details>
  <summary>Details</summary>
Motivation: 现有AI伦理研究多局限于西方及亚伯拉罕诸教视角，缺乏对其他宗教信仰体系（如印度教）在AI互动中表现的关注。

Method: 通过对15位孟加拉国印度教徒进行半结构化访谈，分析其在宗教知识检索、经文解读及数字崇拜中的AI使用体验。

Result: 研究发现AI能辅助经文探究与宗教叙事，但也存在神学简化、文化误读、崇拜操纵及模拟神圣权威等伦理风险。

Conclusion: 提出“解释性对齐”（interpretive alignment）概念，主张AI系统应明确自身局限，尊重宗教多元性，并避免伪装成具有神圣权威的实体。
...
### [70] [Physics-Guided Flow Matching for CT Image Reconstruction](https://arxiv.org/abs/2608.28256)
*Davide Evangelista*

Main category: cs.AI

TL;DR: 该论文提出利用流匹配（Flow Matching）替代扩散模型作为CT重建的生成先验，显著提升了重建的效率与质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在CT重建中存在采样过程随机性强、推理轨迹长且参数敏感的问题，限制了高分辨率场景下的效率与稳定性。

Method: 采用两阶段训练策略（强增强预训练+微调）训练Rectified Flow Matching模型，并评估了Plug-and-Play Flow、FlowDPS、Flower及Flow-Priors等多种重建算法。

Result: 基于流匹配的方法在PSNR、SSIM及感知质量上均优于现有的扩散模型方法（如DDRM、DPS），且显著减少了所需的采样步数。

Conclusion: 流匹配是高分辨率CT重建中一种更高效、稳定且有效的生成先验替代方案。
...
TL;DR: 提出了一种基于检索增强与大模型推理的动态专家切换框架，用于非平稳金融市场下的投资组合管理。


<details>
  <summary>Details</summary>
Motivation: 金融市场的非平稳性导致单一投资策略难以适应所有市场条件，需要一种能根据环境动态切换专家的机制。

Method: 利用双流变分自编码器（VAE）建模市场信息，通过基于检索的知识库匹配历史情境与专家表现，并由指令微调的大语言模型（LLM）推理决策选择最优专家。

Result: 在加密货币、股票和外汇市场中，该框架均实现了最高的累计收益和夏普比率，并在股票市场中将收益从26%提升至34%。

Conclusion: 基于检索增强的专家切换机制在处理非平稳金融环境下的自适应投资组合管理方面具有显著优势。
...
### [72] [REINS: Refusal-Enhanced Inhibitory Steering with Sparse Autoencoder Features](https://arxiv.org/abs/2608.28233)
*Kai-Xuan Ding,Hao-Xiang Xu,Ji-Hua Peng,Zi-Qi Chen,Jiaqi Wang,Zhen-Hua Ling*

Main category: cs.AI

TL;DR: 提出了一种名为REINS的稀疏自编码器（SAE）控制方法，旨在通过同时抑制有害特征和增强拒绝特征，提升大语言模型的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的SAE控制方法在面对复杂的指令包装（wrappers）时表现脆弱，难以有效诱导模型拒绝有害请求。

Method: 构建了GUISE数据集以评估复杂场景下的安全性；提出了REINS方法，通过在SAE特征空间内同步执行有害特征抑制和安全拒绝特征增强。

Result: 相比传统单一方向的控制方法，REINS能显著降低有害响应，提升模型拒绝率，同时在很大程度上保留了模型的通用能力。

Conclusion: 通过结合抑制与增强的双重手段，REINS为大模型推理阶段的安全控制提供了一种更稳健、有效的解决方案。
...
### [73] [Beyond Task-Only Matching: Personalized Skill Routing with Counterfactual Evaluation](https://arxiv.org/abs/2608.28241)
*Tianle Wang,Yanghe Zou,Xiang Liu,Ziyao Huang,Chenchen Fu,Weiwei Wu*

Main category: cs.AI

TL;DR: 本文提出了“个性化技能路由”（personalized skill routing）概念，引入SkillFeed框架，通过结合用户配置信息来解决仅基于任务语义匹配导致技能选择不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的技能路由方法仅关注任务与技能的语义匹配，忽略了用户个性化约束，导致即便任务相同，不同用户也可能需要不同的技能，从而引发匹配失败。

Method: 构建了 profile-counterfactual 基准数据集，并提出了 SkillFeed 框架，采用“检索-重排序”（retrieve-and-rerank）两阶段策略：先建立任务-技能对齐，再通过引入用户画像进行精细化的判别与重排序。

Result: 在 SkillFeed-Bench 上，SkillFeed 的 Top-1 检索准确率达到 75.1%，比基准线提升了 23.1 个百分点；在用户画像直接影响技能选择的场景下，提升幅度高达 35.1 个百分点。

Conclusion: 用户画像在技能路由中至关重要，特别是当用户约束改变技能适用性时，个性化条件检索能显著提升大型语言模型智能体处理复杂请求的准确性。
...
TL;DR: 提出了一种名为DoCtOR的多智能体反思框架，通过定位并仅针对导致失败的“关键错误智能体”进行定向反思，从而提高系统性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法要求所有智能体进行反思，这会导致无关智能体的记忆被错误信息污染，降低了协作效率和准确性。

Method: 引入DoCtOR框架：通过自动失败归因定位关键错误步骤和智能体；利用反事实推理生成修正步骤；仅要求关键错误智能体进行针对性反思。

Result: 在HotPotQA、ChartQAPro和Mind2Web数据集上，相比Reflexion等现有方法，成功率分别提升了22%、26%和27%。

Conclusion: 该“先诊断后纠正”的范式有效提升了多智能体协作能力，且在低资源环境下依然保持高效。
...
TL;DR: RECAST是一种用于流式生物信号测试时适应（TTA）的轻量级采样模块，通过动态选择最相关的样本进行在线个性化更新。


<details>
  <summary>Details</summary>
Motivation: 现有的TTA方法在处理流式生物信号时，简单使用所有缓冲区样本会导致更新模糊，仅使用最新样本则会导致训练不稳定，缺乏有效的样本筛选机制。

Method: 提出RECAST算法，根据三个关键指标（时间近因、上下文相似度和预测可靠性）从缓冲区中筛选用于模型更新的样本，无需修改模型架构或训练目标。

Result: 在两个血压数据集上的测试表明，RECAST显著提升了估计精度和趋势追踪能力，特别是在高难度病例和紧急环境下效果显著，且推理延迟极低。

Conclusion: RECAST通过智能筛选样本解决了流式生物信号中的个性化适应问题，在保持低计算开销的同时，显著增强了模型的鲁棒性和适应性。
...
### [76] [Memristive-Friendly Hadamard Reservoir Computing: Structured, Multiplier-Free Recurrences at Scale](https://arxiv.org/abs/2608.28295)
*Andrea Ceni,Gianluca Milano,Carlo Ricciardi,Claudio Gallicchio*

Main category: cs.AI

TL;DR: 提出了一种基于结构化正交算子的新型储层计算（RC）架构，替代了传统的高成本密集型循环矩阵。


<details>
  <summary>Details</summary>
Motivation: 传统储层计算中密集的循环矩阵在物理硬件（尤其是忆阻器）实现上成本过高且效率低下。

Method: 利用符号对角矩阵、置换矩阵和快速沃尔什-哈达玛变换（FWHT）构建结构化正交算子，实现无需矩阵乘法的计算，复杂度降至O(N)参数和O(N log N)运算。

Result: 实验表明该模型在27个基准测试中表现媲美密集型正交储层，在硬件运行速度上提升高达50倍，内存需求减少10,000倍，且具备良好的鲁棒性。

Conclusion: 该结构化方案不仅优化了计算效率和内存占用，还通过严格的正交性保证了回声状态条件，是实现高效、可扩展神经形态硬件的理想选择。
...
### [77] [MAIL: Memory-driven, Adaptive, Incremental, and Literature-grounded Framework for Hypothesis Generation in Chemistry](https://arxiv.org/abs/2608.28315)
*Mahdi Babaei,Xueshen Li,Yutao Kuang,Jolene P. Reid,Yu Gan*

Main category: cs.AI

TL;DR: 提出了一种名为MAIL的自动化框架，用于化学领域的假设生成，通过记忆增强、自适应和文献驱动的推理过程克服了现有方法在扩展性和创新性上的不足。


<details>
  <summary>Details</summary>
Motivation: 化学文献爆炸式增长，但现有基于LLM的方法依赖静态语料库或繁琐的人机协同，限制了科学假设生成的效率和创新性。

Method: MAIL框架将假设生成建模为一种时间接地、记忆驱动的推理过程，通过持续积累和重新诠释先验知识，动态生成化学假设。

Result: 在TOMATO-Chem和HN-NS数据集上，MAIL框架在假设的连贯性、机制合理性、MIOS/MPOS指标以及专家评估得分上均表现最优。

Conclusion: MAIL框架证明了LLM能够自主探索化学领域，生成兼具创新性和化学合理性的科学假设，有效提升了文献驱动的科研效率。
...
### [78] [LoopArena: Benchmarking Models as Runtime Controllers for Loop Engineering](https://arxiv.org/abs/2608.28281)
*Yi Wang,Haopeng Zhang,Chengxiang Huang,Rui Dai,Kaikui Liu,Piotr Koniusz,Xiangxiang Chu*

Main category: cs.AI

TL;DR: LoopArena 是一个用于评估 AI 模型作为“控制器”引导“执行者”进行长周期编程任务能力的基准测试框架。


<details>
  <summary>Details</summary>
Motivation: 现有编程代理往往受限于循环设计，而现有的评估方法无法区分任务成功与否是归因于循环引导策略还是代理本身的编码能力。

Method: 引入 LoopArena 基准，将系统分为控制器（规划与决策）和执行者（实际编码）。通过三种不同成本和范围的评估类型（Type I-III）来衡量控制器在长周期任务中的决策质量。

Result: 全任务场景下的最佳成功率仅为 24.69%，显示出该领域提升空间巨大；此外，该评估框架能有效降低平均 64.4% 的推理成本，且不同评估类型间具有高度相关性。

Conclusion: 目前的模型在长周期编程循环控制方面仍有显著不足，LoopArena 为开发更可靠的循环工程策略提供了标准化的评估工具。
...
TL;DR: 本文提出了一种适用于超维计算（HDC）的实值位置编码方法，旨在解决标准分数幂编码在哈达玛积绑定操作下的兼容性问题。


<details>
  <summary>Details</summary>
Motivation: 传统分数幂编码主要针对循环卷积等绑定方式，限制了其在实值向量哈达玛积绑定中的应用，因此需要开发兼容实值运算的新型位置编码。

Method: 借鉴随机傅里叶特征（RFF），提出了三种实值位置编码变体：基于逆傅里叶变换的基准、正弦变体和仅余弦变体，其中正弦变体支持代数移位算子。

Result: 实验表明，这些实值编码在时间序列分类任务上的性能与标准分数幂编码相当，且正弦变体在计算效率和移位等变性之间取得了最优平衡。

Conclusion: 所提出的实值位置编码成功实现了在保持超维计算性能的同时，支持高效的哈达玛积绑定操作，并提供了显式的代数移位能力。
...
### [80] [Propagating construction-time knowledge quality into medical question answering: A framework grounded in clinical guidelines](https://arxiv.org/abs/2608.28360)
*Jie Hu,Junjie Wang,Shan Lu,Yifang Hu,Gong Cheng,Yun Liu*

Main category: cs.AI

TL;DR: 提出了一种将临床指南构建阶段的三元组质量信号（结构一致性与证据支持度）融入下游问答系统的框架，显著提升了证据选择与回答的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在知识图谱构建与下游问答间存在断层，未充分利用构建阶段产生的质量信息来优化推理时的证据使用。

Method: 构建了包含结构一致性（SchemaConf）与证据支持度（EvidScore）的融合质量指标Q(t)，并将其作为图属性传播至子图检索与提示工程中。

Result: 实验显示Q(t)在跨模型和跨版本迁移中表现出更强的质量区分能力（AUC 0.748），显著降低了知识遗漏（16.3%至5.3%）与冲突输出（16.3%至2.7%），获得临床医生的高度评价。

Conclusion: 构建阶段的三元组质量信号是提升领域问答系统可信度与性能的有效手段，通过质量加权检索与分层提示可实现更精准的临床证据溯源。
...
### [81] [AGENT-O: A Semantic Agent Card Framework for Interoperable and Governed Healthcare AI Agents](https://arxiv.org/abs/2608.28345)
*Pengze Li,Cui Tao*

Main category: cs.AI

TL;DR: AGENT-O 是一个用于描述健康医疗 AI 代理系统的模块化本体框架，旨在评估科学文献中对这些系统报告的完整性。


<details>
  <summary>Details</summary>
Motivation: 医疗 AI 代理系统的复杂性与多样性使得现有的科学文献报告标准不足以涵盖其运行、治理和重现性等关键维度，导致评估与规格说明之间存在脱节。

Method: 开发了基于 OWL 2/RDF 的 AGENT-O 本体，包含运行、模型、工作流等多个维度，并通过 OWL-RL 推理、SHACL 验证、SPARQL 查询以及对 279 篇论文的实际报告完整性评分进行评估。

Result: 研究发现，医疗 AI 文献在评估和基准测试流程方面报告较好，但在架构、治理安全及重现性方面的报告严重缺失（缺失率均超 78%）。

Conclusion: AGENT-O 提供了一种结构化的语义化代理卡片（Agent Card）框架，能有效识别科学报告中的缺失环节，为提高健康医疗 AI 的透明度与标准化提供了工具。
...
TL;DR: 提出了GRACE框架，一种用于大型语言模型（LLM）机器遗忘任务的梯度引导核心集选择方法。


<details>
  <summary>Details</summary>
Motivation: 现实场景中缺乏明确的遗忘集和保留集，现有研究依赖预设集合，无法有效从异构语料中自动构建这些集合。

Method: 利用梯度引导技术，通过非负正交匹配追踪构建遗忘核心集，并通过投影遗忘方向和聚类正交匹配追踪选择保留集，以平衡遗忘效果与模型效用。

Result: 在多种模型和领域上验证，GRACE在保持遗忘质量的同时，显著优于现有的梯度基选择方法，提升了模型效用。

Conclusion: GRACE有效地解决了无预设集合下的数据选择问题，为大模型的高效遗忘提供了稳健且通用的解决方案。
...
TL;DR: 本文提出了EvoUndo框架，用于解决LLM智能体在运行时进行自我修改（self-evolution）后无法安全撤回的问题。


<details>
  <summary>Details</summary>
Motivation: 智能体通过自我演化（如修改提示词、工具等）提升能力时，产生的变异往往具有不可逆性，导致在不同执行状态下存在安全隐患。

Method: 引入EvoUndo框架，通过表示、合成、诊断和独立验证模型生成变异的恢复能力，并提出扩展恢复演算（recovery calculus）和精确状态寻址（exact state-address grounding）干预措施。

Result: 实验发现大量变异无法通过常规策略恢复；通过引入扩展的恢复语言和精确状态寻址，恢复成功率显著提升至93%-99.3%，并发现恢复能力受模型架构的影响。

Conclusion: 可靠的智能体自我演化不能仅依赖迭代提示，必须协同设计验证机制、状态接地、见证语义及恢复语言的表达能力。
...
TL;DR: MAP 是首个用于评估多模态 AI 系统在处理残障人士出行规划需求时的基准测试。


<details>
  <summary>Details</summary>
Motivation: 旨在解决现有 AI 系统在为残障人士提供无障碍出行建议和地点验证时缺乏专门评估标准的问题。

Method: 包含“声明验证”和“视觉证据检索”两项核心评估，通过自动评分与人工评分相结合，并定期更新地面真值数据以适应现实世界信息的动态变化。

Result: 建立了一套能够衡量 AI 系统在真实场景中识别无障碍设施并提供精准规划能力的评估框架。

Conclusion: 该基准测试为开发更具包容性的辅助类多模态 AI 提供了标准化的评估工具，有助于提升无障碍出行辅助技术的可靠性。
...
### [85] [RetailAgent: Structured Adverse Timing in Self-Conditioned Multimodal LLM Trading Agents](https://arxiv.org/abs/2608.28399)
*Yupeng Zhang,Liuyuan Jiang,Hongyi Huang,Bingheng Li,Lisha Chen*

Main category: cs.AI

TL;DR: 该研究利用RetailAgent框架评估了大语言模型（LLM）在金融交易决策中是否存在可预测的方向性结构。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM代理在金融市场中的序贯决策是否会表现出可被其他参与者利用的可预测性。

Method: 构建RetailAgent框架，让LLM观察日内股票价格历史并进行多/空决策，通过去偏后的暴露匹配度量分析决策与后续收益的关系，并对比了序列洗牌和自我记忆输入的效果。

Result: 实验发现LLM决策表现出持续的负向择时效应，且这种可预测性随自我记忆的使用而增加，证明了模型决策中存在可恢复的方向性结构。

Conclusion: LLM在金融决策中展现出显著的非随机性与可预测性，这种行为信号可作为研究多智能体市场互动中策略响应的重要切入点。
...
### [86] [Timing-Aware Repurchase Prediction for Web-Scale E-Commerce: Survival Models for Multi-Surface Grocery Recommendation](https://arxiv.org/abs/2608.28393)
...
TL;DR: 该论文提出使用生存分析模型（AFT）替代传统的二分类模型来预测电商重复购买行为，通过单一模型实现不同时间跨度的预测。


<details>
  <summary>Details</summary>
Motivation: 传统二分类方法需要为每个预测窗口训练独立模型，且无法直接建模时间连续性，导致计算冗余且模型碎片化。

Method: 利用加速失效时间（AFT）生存模型对重复购买时间进行建模，并采用四参数校准方法将生存函数映射为概率，对比了Log-Normal、Weibull和Exponential等分布。

Result: 单一AFT模型在性能上优于多个二分类模型，模型参数量减少约3倍；发现Log-Normal在排序指标上表现最优，而Exponential在校准指标（ECE）上表现最佳，揭示了排序与校准间的权衡。

Conclusion: 生存分析模型有效整合了重复购买预测任务，通过选择合适的参数分布，可以在系统效率、预测精度与校准质量之间取得最优平衡。
...
TL;DR: 本文提出了VERA-8B，这是一个专为审计推理设计的端到端系统，旨在通过证据支撑解决金融大模型在审计任务中存在的“幻觉”和缺乏证据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有金融语言模型侧重于语言流畅度而非证据支撑，无法满足审计工作对证据严谨性的要求，且目前缺乏针对审计预执法预测的相关机器学习研究。

Method: 结合了有监督微调（SFT）与群相对策略优化（GRPO）以实现证据导向的推理；引入了拒绝回答与不确定性量化机制；设计了AuditBridge系统，将原始财务申报文件转化为可供审计审查的报告。

Result: VERA-8B在审计风险预测任务中超越了所有评估基准模型，实现了具备证据支撑且可审计的推理输出。

Conclusion: 通过统一证据标准和构建端到端工作流，VERA-8B成功缩小了金融计算与审计实践之间的鸿沟，为审计工作提供了可靠的技术方案。
...
### [88] [Prove2Me: An Open Collaborative Platform for Scaling Math Formalization](https://arxiv.org/abs/2608.28433)
*Shuze Chen,Kunal Marwaha,Xiaoyang Lu,Henry Yuen,Tianyi Peng*

Main category: cs.AI

TL;DR: Prove2Me 是一个旨在降低数学形式化门槛的开放协作平台，通过 AI 代理与人类协作，实现互联网规模的机器验证数学证明。


<details>
  <summary>Details</summary>
Motivation: 形式化验证数学（如 Lean 4）存在学习门槛高、耗时长的问题，且缺乏能够支持大规模、多代理协作的生态环境。

Method: 构建了一个专门的协作平台 Prove2Me，设计了支持 AI 代理协作和重用已有证明结果的机制与 harness 工具。

Result: 提出了一个可扩展的众包式框架，使 AI 代理能够通过自然语言指令参与复杂的数学证明，并能无缝整合彼此的贡献。

Conclusion: Prove2Me 成功将数学形式化转变为一种可扩展的、由 AI 驱动的众包协作模式，从而使数学证明的正式化变得触手可及。
...
### [89] [Learning to Use Tools: Reinforcement Learning for Tool-Integrated Mathematical Reasoning](https://arxiv.org/abs/2608.28447)
*Minghui Xu,Zi Wang*

Main category: cs.AI

TL;DR: 该研究探讨了将计算器工具集成到大语言模型中以提升“Countdown”任务数学推理能力的方法。


<details>
  <summary>Details</summary>
Motivation: 分析发现LLM在数学任务中的推理失败很大程度上源于计算错误，因此引入工具集成和强化学习以提高准确性。

Method: 通过构建监督微调（SFT）数据集教会模型工具使用模式，并应用RLOO、RLOO++、GRPO及DAPO等多种策略强化学习方法进行训练。

Result: 工具集成使SFT和RL基线均有约10个百分点的提升，其中Tool-DAPO表现最优，将pass@1从35.8%提升至66.0%。

Conclusion: 工具集成有效降低了算术和验证错误，而强化学习进一步提高了模型生成正确推理路径的概率。
...
### [90] [Program Learning with Verifiable Rewards: Symbolic Backpropagation for Post-Training LLMs](https://arxiv.org/abs/2608.28421)
*Vishvesh Bhat*

Main category: cs.AI

TL;DR: 提出了一种名为PLVR的后训练方法，将推理逻辑从模型权重中提取出来，通过符号反向传播学习由确定性和神经原语组成的显式程序，从而实现可验证的推理。


<details>
  <summary>Details</summary>
Motivation: 现有的微调和强化学习将推理能力黑盒化存储在权重中，无法检查步骤且不可移植；作者旨在通过将推理过程显式化为可验证程序来解决此问题。

Method: 引入PLVR方法，使用“符号反向传播”：通过类型推断将损失从输出向后传播至程序各层，利用分步契约作为奖励机制，而非仅依赖最终结果的验证。

Result: 在LiveCodeBench和Tau2Bench测试中，使用PLVR的30B模型表现大幅超越了强化学习方法，且在面对大一个量级的模型时仍具有显著性能优势。

Conclusion: 将推理转化为显式的程序构建，利用符号反向传播进行信用分配，比传统的权重微调更具效率、可解释性及泛化能力。
...
TL;DR: CE-MoE 提出了一种异构层模式，通过解耦 token-mixing 和 channel-mixing 深度，减少 MoE 模型中昂贵的 all-to-all 通信开销。


<details>
  <summary>Details</summary>
Motivation: MoE 模型在专家并行训练中，all-to-all 通信占据了大量训练时间，影响了训练效率和扩展性。

Method: 采用异构层架构，将专家计算集中在少数几个 MoE 层，同时通过增加额外的 token-mixing 和密集 FFN 层来保持模型深度，从而降低通信频率。

Result: 在 2B 到 31.5B 参数规模下，CE-MoE 在保持验证损失和下游任务性能的同时，减少了训练成本；在 31.5B 规模下，训练耗时减少了 33.3%，并提升了推理吞吐量。

Conclusion: 通过优化 MoE 层的布局而非简单的堆叠，可以在不牺牲模型质量的前提下显著提升训练效率和推理性能。
...
### [92] [COVER: Identifiable Evaluation of Coalition Routing](https://arxiv.org/abs/2608.28475)
*Raghul Sugumar,Amrit Gopinath*

Main category: cs.AI

TL;DR: 本文提出了一种名为COVER的评估框架，旨在准确量化多智能体系统中因任务路由（routing）策略带来的性能提升，避免因团队配置变化导致的评估偏差。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在改变团队结构时，会同时改变输出内容，导致仅通过终端准确率无法区分是系统能力的提升还是单纯的路由选择效应。

Method: 引入COVER评估契约，通过固定公共信息边界、下游堆栈G和有限的合法团队族，利用“预先指定（pre-specified）”与“事后追溯（retrospective）”对照实验来验证路由策略的真实性能增益。

Result: 在MuSiQue-12、HotpotQA-4及ToolSandbox等数据集上，COVER成功识别出了路由选择带来的实际性能提升空间，并揭示了部分声称的性能提升实际上源于回顾性偏差或堆栈依赖性。

Conclusion: COVER作为一种可审计的测量方法，能够有效暴露多智能体系统中的路由选择潜力，而非单纯通过制造虚假性能指标来评估系统优劣。
...
TL;DR: InstructMesh 是一款交互式 3D 模型精修工具，旨在通过自然语言和滑块控件修复生成式 3D 模型中常见的几何缺陷，以满足制造需求。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式 AI 模型生成的 3D 模型虽然视觉逼真，但在几何结构上存在难以直接制造的缺陷，且缺乏专业修复工具。

Method: 通过分析制造相关的失效模式，设计并开发了 InstructMesh 系统，允许用户直接在中间潜空间表示中进行区域选择和针对性修复，并采用自然语言与滑块混合的交互方式。

Result: 用户研究表明，该工具使新手能够有效识别并修复 3D 模型中的制造缺陷，用户更倾向于使用自然语言与滑块结合的混合交互界面。

Conclusion: InstructMesh 有效弥合了生成式 3D 模型与物理制造需求之间的差距，降低了非专家进行复杂几何修复的技术门槛。
...
TL;DR: 本文提出了一种名为Logos的跨进程代理框架，旨在解决传统单一进程代理架构中组件共享同一故障域的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统代理架构将所有组件放置在单一进程内，一旦发生故障会导致所有会话同时中断。研究者希望利用语言模型（LM）的无状态特性，实现组件间的故障隔离。

Method: 基于时空可组合性演算（spatiotemporal-composability calculus）及LM的无状态特性，构建了Logos框架。在该框架中，每个插件作为一个独立进程运行，仅通过追加记录（append-only transcript）共享状态。

Result: 实验表明，Logos在工具调用周期的不同边界引发故障时，仍能恢复80个会话且无重复效应；对比单进程架构，单点故障只会导致对应进程终止，而非整个系统崩溃。

Conclusion: 通过将代理组件解耦至独立进程，利用LM的无状态性进行状态管理，能够显著提升代理系统的健壮性与容错能力。
...
### [95] [VidParse: Online Parsing of Egocentric Procedures Like a Pro](https://arxiv.org/abs/2608.27562)
*Anubhav Gupta,Archit Kambhamettu,Vatsal Agarwal,Pulkit Kumar,Abhinav Shrivastava*

Main category: cs.CV

TL;DR: VidParse 是一种无需训练的在线框架，通过将视频解析建模为图约束推理问题，有效地将连续的第一人称视角视频转换为离散的动作步骤。


<details>
  <summary>Details</summary>
Motivation: 现有的在线动作识别方法在面对第一人称视频的强烈自身运动、遮挡和动作高度变异时，容易出现过分割和结构崩溃问题。

Method: 利用冻结的基础模型提取关注手部与物体交互的特征，通过计算时间相似度矩阵检测语义转换，并使用波束搜索解码器结合预定义的程序化任务图进行约束推理。

Result: 该方法在复杂的长程多步骤动作解析任务中实现了高达 10 倍的准确率提升，且无需任何训练。

Conclusion: 将稳健的视觉分割与硬性程序约束相结合，能够有效解决长程动作序列解析中的结构一致性问题。
...
### [96] [Revisiting Local Context for Long-Horizon Streaming 3D Reconstruction](https://arxiv.org/abs/2608.27529)
*Jiarong Han,Jincheng Xiong,Yuzhou Liu,Linzhe Shi,Changjie Wu,Ning Guo,Mu Xu,Hang Zhang,Ming Qian*

Main category: cs.CV

TL;DR: ABot-Recon 是一种流式 3D 重建模型，通过局部时序上下文和相对位姿估计实现长序列稳定重构。


<details>
  <summary>Details</summary>
Motivation: 现有流式 3D 重建方法在长序列下随时间推移估计精度下降，且难以在有限计算资源下保持长期稳定性。

Method: 仅使用过去 11 帧的 KV 特征作为局部缓存，预测当前相机坐标系下的点图及相邻帧相对位姿；通过序列组合恢复全局位姿，并引入轻量级时序优化器和组合感知位姿损失来减少漂移。

Result: 在长序列基准测试（如 Oxford Spires）中表现优异，ATE 降至 4.35m，RPE-R 降至 0.12°，较现有最优方法性能提升约 40%。

Conclusion: 基于局部上下文的策略能有效规避序列长度对预测目标的依赖，从而在长视频 3D 重建中实现更好的鲁棒性和稳定性。
...
### [97] [Code as Worlds: Agentic Discovery of Executable World Representations for Physical Reasoning](https://arxiv.org/abs/2608.27549)
...
TL;DR: 提出了一种名为“Code-as-World”的新范式，通过将物理世界表征为可执行代码，提升视觉语言模型在定量物理推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型虽能识别物理事件，但缺乏对物体状态、物理参数及动力学机制的显式表征，难以进行可靠的物理推理与干预预测。

Method: 构建基于可执行代码的物理世界表征，并开发了一个类溯因推理的智能体发现循环，通过提出、执行、渲染、验证和迭代细化来从多模态数据中生成物理模型。

Result: 提出了Code-as-World-VL模型，在QuantiPhy基准测试中达到了SOTA水平，超越了领先的专有大模型。

Conclusion: 将可执行代码作为世界表征是提升模型物理智能的一种高效、可扩展且具备定量推演能力的新途径。
...
### [98] [FVeinSyn: Synthetic Finger Vein Image Generator](https://arxiv.org/abs/2608.27527)
*Yifan Wang,Jie Gui,Adams Wai Kin Kong,Baosheng Yu,Changsheng Chen,Qi Li,Zhenan Sun,James Tin-Yau Kwok,Alex Kot*

Main category: cs.CV

TL;DR: 本文提出了FVeinSyn，一个大规模可控的指静脉合成数据生成框架，旨在解决现有数据集规模小、身份多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 指静脉识别领域缺乏大规模公共数据集，限制了深度学习方法的发展，现有数据集在身份覆盖和样本多样性上存在局限。

Method: 采用了三阶段生成策略：首先通过随机L系统生成具有生理约束的血管拓扑结构；其次利用级联区域感知GAN将拓扑图渲染为近红外图像；最后通过引入几何与光学扰动生成类内多样性样本。

Result: 生成了包含1万个身份、50万张图像的合成数据集，模型在八个公开数据集上的平均准确率提升了27.43%。

Conclusion: FVeinSyn通过解耦血管拓扑和图像外观，有效提升了指静脉识别模型的泛化能力和真实感，为解决数据匮乏问题提供了有力工具。
...
### [99] [Quanta Perception as Probabilistic Events](https://arxiv.org/abs/2608.27584)
*Varun Sundar,Pavan Thodima,Sacha Jungerman,Mohit Gupta*

Main category: cs.CV

TL;DR: 本文提出了一种名为“概率事件（probabilistic events）”的计算基元，用于处理超快、低光环境下的单光子传感器数据，实现了实时感知。


<details>
  <summary>Details</summary>
Motivation: 传统图像传感器在极端环境（如弱光、高速运动）中受制于曝光限制，而现有的量子传感器数据量过大，难以满足实时计算和延迟要求。

Method: 通过递归贝叶斯公式处理单光子检测流，计算自上次强度变化以来的后验概率，生成运动自适应通量、活动图和熵基不确定性信号。

Result: 在0.05勒克斯的极弱光环境下实现了精准姿态估计，处理速度超过50,000光子帧/秒，比现有量子重构方法快四个数量级。

Conclusion: 通过以直接概率推理取代传统的帧重构，该方法成功填补了单光子传感与机器人视觉之间的鸿沟，实现了高效且鲁棒的极端环境感知。
...
### [100] [ShiftSplit-AD: Separating Domain Shift from Defects in Foundation-Feature Visual Anomaly Detection](https://arxiv.org/abs/2608.27610)
*Muhamathu Ameer Ali Aacaas Muhamath*

Main category: cs.CV

TL;DR: 研究者提出了ShiftSplit-AD，利用矩阵分解将视觉异常检测中的残差分解为低秩和稀疏成分，以抑制领域偏移导致的误差，同时尝试保留缺陷特征。


<details>
  <summary>Details</summary>
Motivation: 现有的基于冻结基础模型特征的异常检测器难以区分正常的领域偏移（如光照或背景变化）与真实的缺陷，导致误报率增加。

Method: 引入ShiftSplit-AD，将测试补丁的残差矩阵分解为低秩（代表领域偏移）和稀疏（代表潜在缺陷）两个分量，并对稀疏分量进行评分。

Result: 在AeBAD-S数据集上，稀疏评分显著提升了AUROC和AUPRC指标；但在MVTec数据集的受控测试中，该方法降低了正常样本的检测精度并削弱了缺陷定位能力。

Conclusion: 残差分解在缓解领域偏移干扰方面有效，但存在固有权衡：过滤低秩成分往往会损失部分缺陷信息，如何区分两者结构仍是核心难题。
...
### [101] [Depth-Aware Pothole Detection Using YOLO and RT-DETR at the Edge](https://arxiv.org/abs/2608.27633)
*Md Monjurul Ahsan Prodhan,Md Nour Hossain*

Main category: cs.CV

TL;DR: 该论文提出了一个基于RGB-D传感器融合的坑洞检测框架，通过对比五种深度学习架构，实现了坑洞的自动检测与物理深度测量。


<details>
  <summary>Details</summary>
Motivation: 现有2D RGB图像方法无法测量坑洞的物理深度，导致道路维护滞后，增加了事故风险和维修成本。

Method: 使用PothRGBD数据集，对比了YOLOv8n、YOLOv8nSeg、YOLOv9t、RTDETRL和RTDETRX五种架构，引入自定义离线增强流水线，并采用RANSAC地面校正处理深度图。

Result: YOLOv8nSeg在精度上表现最佳（mAP@50为0.9556），且深度测量误差最小（2.96 cm）；YOLOv8n推理速度最快；研究还发现边界框模型会因路面包含偏差导致深度高估。

Conclusion: 基于RGB-D的分割模型能提供比边界框模型更精确的深度测量，证实了路面偏差是坑洞深度测量中的结构性问题。
...
### [102] [Lossy Event Compression: From Event Stream Distortion to Task Performance](https://arxiv.org/abs/2608.28429)
*Zahra Rezaee,Catarina Brites,João Ascenso*

Main category: cs.CV

TL;DR: 本文提出了一套针对事件相机数据流的评估框架与失真指标，用于高效预测压缩对下游任务性能的影响，避免了繁琐的任务相关评估。


<details>
  <summary>Details</summary>
Motivation: 高频运动场景下事件流产生巨大的数据量，现有的失真指标无法准确衡量压缩对任务级性能的损害，导致优化过程成本高昂。

Method: 引入了基于聚合的帧压缩（JPEG 2000）和基于点云的无帧压缩（G-PCC）两种流水线；构建了一个包含视频重建、目标检测、光流估计和特征跟踪的统一评估框架；首次将五种分类失真指标应用于事件压缩评估。

Result: 实验证明所提出的失真指标能有效预测不同压缩框架下下游任务的性能退化，优于现有的事件流指标。

Conclusion: 该评估框架和指标为事件流压缩提供了高效的指导，可替代重复的任务级测试，推动事件数据编码技术的快速优化。
...
### [103] [Report Supervision](https://arxiv.org/abs/2608.27668)
...
TL;DR: 提出了一种名为R-Super的训练框架，利用海量的放射科报告作为监督信号，弥补了肿瘤分割标注数据稀缺的问题，显著提升了肿瘤检测与分割性能。


<details>
  <summary>Details</summary>
Motivation: 3D肿瘤分割标注耗时且稀缺，限制了高性能AI模型的训练，而临床中广泛存在的放射科报告包含丰富的肿瘤描述信息，未被充分利用。

Method: 引入新的损失函数（Report Supervision），通过匹配报告中描述的肿瘤数量、大小和位置信息，对分割模型进行直接监督训练。

Result: 在肾脏和胰腺肿瘤数据集上验证，R-Super使检测F1分数和分割DSC提升高达15%，且在标注数据较少和较多场景下均优于CLIP和多任务学习方法。

Conclusion: 利用临床放射科报告进行弱监督训练是扩展肿瘤分割规模的有效途径，能够显著增强AI在数据稀缺环境下的表现。
...
### [104] [ABCD: Alpha-Composited Block Coordinate Descent: Constant-VRAM Training for Large Radiance Fields](https://arxiv.org/abs/2608.27735)
*Ka Heng Shiu,Kartic Subr*

Main category: cs.CV

TL;DR: ABCD是一种用于3D高斯溅射的离核训练框架，通过分块坐标下降法和Alpha合成技术，将显存消耗控制在常数级别。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射训练时的显存需求随场景规模线性增长，导致在大场景训练时受限于显卡内存。

Method: 将训练过程重构为分块坐标下降法（Block Coordinate Descent），仅激活单块参数；利用Alpha混合的结合律，将不活跃区域预渲染并合并为背景RGBA图像。

Result: 在固定分辨率和分块大小下，峰值显存占用与场景规模无关（O(1)）；在保持与原版3DGS质量相当（PSNR下降小于5%）的同时，支持了超大规模场景的训练。

Conclusion: ABCD框架有效解决了大规模3D高斯溅射场景的显存限制问题，在保证高质量重建的同时实现了显著的显存优化。
...
### [105] [What Can Low Resource Languages Learn From Each Other?](https://arxiv.org/abs/2608.27753)
*Achyuth P,Kahaan Shah,Chetan Arora*

Main category: cs.CV

TL;DR: 提出了一种名为PSMC的框架，旨在解决视觉语言模型在低资源语言OCR任务中的数据稀缺和性能瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 大多数OCR模型在低资源语言上的表现受限于数据匮乏，且传统微调方法存在结构冗余和性能天花板。

Method: 提出了PSMC框架，包含预训练、专家化、合并（任务算术）和协同训练四个步骤，通过融合特定语言专家来构建统一的多语言主干网络。

Result: 在10种印度脚本（20+语言）上，PSMC在不增加参数量的情况下，Word Recognition Rate (WRR) 平均提升约2%。

Conclusion: 在合并的潜在空间中进行联合训练可促进跨脚本的知识迁移，为开发更具包容性的多语言视觉语言模型提供了高效路径。
...
### [106] [uScenes: A Multimodal RGB and 3D Sonar Dataset for Underwater Robot Perception](https://arxiv.org/abs/2608.27795)
*Trung Tien Dong,Zhenqi Wu,Aditya Penumarti,Zi-Hao Zhang,Micaiah Bartlett,Jane Shin,Xiaomin Lin*

Main category: cs.CV

TL;DR: uScenes 是一个包含同步 3D 多波束声纳点云和 RGB 图像的水下多模态数据集，旨在解决水下环境感知难题。


<details>
  <summary>Details</summary>
Motivation: 光学摄像头在水下低光照和散射环境下不可靠，而 2D 声纳缺乏高度信息，导致难以进行精确的 3D 场景理解和物体检测。

Method: 通过多场次实地采集，构建了一个包含 110 个场景、95,834 个同步观测样本的数据集，支持跨模态表征学习和 3D 场景理解研究。

Result: 提供了近 278 分钟的高质量水下多模态数据，为水下传感器融合及 3D 感知任务建立了基准基础。

Conclusion: uScenes 为解决水下环境感知模糊性提供了必要的数据支撑，促进了水下机器人 3D 场景理解技术的发展。
...
### [107] [From Perspective to Fisheye Depth Estimation and Open-Vocabulary Segmentation](https://arxiv.org/abs/2608.27860)
*Rit Gangopadhyay,Alex Wong*

Main category: cs.CV

TL;DR: 提出了一种名为DEX（Distortion Extenders）的方法，旨在解决视觉基础模型在鱼眼图像（大视场、径向畸变）上表现不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型多基于透视图像训练，直接迁移到存在径向畸变的鱼眼图像时会产生协变量偏移，导致性能显著下降。

Method: 引入一组可学习参数DEX，通过自监督对齐损失，将鱼眼图像的潜在嵌入转换为类似于透视图像的表征，并能同时推断畸变系数实现相机标定。

Result: DEX具有架构和任务无关性，在单目深度估计和开放词汇分割任务中，显著提升了卷积和Transformer架构在室内外鱼眼数据集上的表现。

Conclusion: DEX有效地弥补了鱼眼图像与透视图像间的分布差异，使视觉基础模型能够直接迁移应用于广角畸变场景。
...
### [108] [Iron: Intent-Aligned and Retrospective Dual Learning Framework for Enhancing Generalist Virtual Agents](https://arxiv.org/abs/2608.27866)
*Jiahe Ying,Wendong Bu,Kaihang Pan,Bingchen Miao,Siyu Chen,Wen Wang,Xueming Jiang,Juncheng Li,Siliang Tang*

Main category: cs.CV

TL;DR: Iron是一个针对GUI智能体的高效训练框架，通过双重学习策略提升了意图对齐能力和数据利用效率。


<details>
  <summary>Details</summary>
Motivation: 解决当前多模态大模型在数字智能体部署中面临的数据标注成本高、动作与意图对齐不准以及失败轨迹利用率低的问题。

Method: 引入了逐步循环一致性（SCC）奖励以实现细粒度的意图对齐，并采用事后重现机制（hindsight reproduction）来重新利用失败的训练轨迹。

Result: 在仅需三分之一训练数据的情况下表现优于现有模型，在未知网页任务上实现了25.06%的相对性能提升。

Conclusion: Iron证明了通过强化意图对齐与提升轨迹利用率，可以有效构建更具通用性和高效性的虚拟智能体。
...
### [109] [Temporal Tree of Thought: Reasoning-Guided Visual Cue Search for Long-Video Understanding](https://arxiv.org/abs/2608.27871)
*Ziling Huang,Shin'ichi Satoh*

Main category: cs.CV

TL;DR: 提出了一种名为T^3的训练无关框架，通过构建层级化时序树实现长视频的自适应由粗到细理解。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理长视频时存在均匀采样丢失关键时刻或独立处理帧导致缺乏时序逻辑的问题。

Method: 利用递归时序约束聚类构建层级树，并在推理时采用“回答-检索-探索”循环，动态调整搜索范围至具体对象和细节。

Result: 在VideoMME、LongVideoBench和LVBench等基准测试中，显著提升了Qwen2.5-VL-7B模型的性能。

Conclusion: 该方法通过结构化的时序推理机制，有效解决了长视频理解中的关键证据定位问题，且无需额外训练。
...
### [110] [Relational Knowledge Distillation Brings DNN Representations Close Enough to Humans to Be Aligned Without Supervision](https://arxiv.org/abs/2608.27877)
*Yuria Shimizu,Soh Takahashi,Takato Horii,Masafumi Oizumi*

Main category: cs.CV

TL;DR: 该研究探讨了如何通过关系知识蒸馏（RKD）将人类心理表征结构迁移至深度神经网络（DNN），以提升两者在细粒度对象级别上的表征对齐。


<details>
  <summary>Details</summary>
Motivation: 现有DNN与人类视觉表征存在差距，且以往关于迁移效果的研究在细粒度对齐和跨数据集泛化性方面缺乏严格验证。

Method: 利用Gromov-Wasserstein最优传输（GWOT）方法，在无需监督的情况下评估DNN与人类表征的细粒度对应关系，并使用与训练集不重叠的测试集验证模型泛化性。

Result: 通过RKD微调后的DNN实现了人类与机器在单个对象级别的细粒度对齐，这种提升主要得益于全局结构（粗分类间距）的趋同，而局部最近邻重叠率变化不明显。

Conclusion: 关系知识蒸馏能有效缩短DNN与人类的全局结构差异，使无需监督的细粒度对齐成为可能，是构建更接近人类视觉系统的计算模型的有效路径。
...
### [111] [What Do Interaction Representations Actually Measure? Pre-Event Separability in Weakly-Supervised Violence Detection](https://arxiv.org/abs/2608.27879)
*Parishruthi Ganesh*

Main category: cs.CV

TL;DR: 研究指出暴力检测基准测试中的性能提升很大程度上归功于非事件特征（如剪辑标题和水印）而非动作本身，且在特定任务中，人体姿态表示并未优于粗糙的边界框几何信息。


<details>
  <summary>Details</summary>
Motivation: 探讨关节化人体姿态在暴力检测等下游任务中是否比粗糙几何信息更具辨别力，并审视现有暴力检测基准测试的有效性。

Method: 通过固定跟踪器、时序头和监督方式，对比了从粗糙几何到复杂姿态编码器的五种表示法；同时测试了在事件发生前的视频片段上的性能，以检查模型是否利用了伪特征（artifact）。

Result: 姿态表示在检测效果上未超过基础几何表示；视频层面的性能评估受到非事件性伪影（如水印、标题卡）的严重干扰，模型通过这些“源提示”而非实际行为来区分异常视频。

Conclusion: 现有的暴力检测基准测试存在严重的数据源偏差问题，这掩盖了不同表征方法的真实效果，需要通过评估事件发生前的证据来剔除这些干扰因素。
...
### [112] [StreamEMS: Streaming Video Understanding with Self-Evolving Memory Scheme for Vision-Language Models](https://arxiv.org/abs/2608.27881)
*Yuxin Liu,Peiqin Zhuang,Yali Wang*

Main category: cs.CV

TL;DR: StreamEMS 提出了一种通过自我演进机制优化流媒体视频理解中外部记忆模块的方法，提升了内存表示的质量与稳健性。


<details>
  <summary>Details</summary>
Motivation: 现有方法多关注读写流程，忽略了对记忆本身表示能力的挖掘与优化，导致信息利用率不足。

Method: 引入语义演进模块（通过从粗到细的语义尺度缩减挖掘密集信息）和先验信息演进模块（利用先验分布优化内存状态）对记忆进行重构。

Result: 在 OVO-Bench 和 StreamingBench 数据集上表现优于现有方法，特别是在高 Token 丢弃率下仍表现稳健。

Conclusion: 通过重构历史数据，StreamEMS 有效增强了流媒体视频理解中记忆模块的表达能力与鲁棒性。
...
### [113] [Thread-Efficient Decoding for Neural Texture Compression](https://arxiv.org/abs/2608.27888)
*Janarbek Matai,Sho Ikeda,Lukasz Lipski,Takahiro Harada*

Main category: cs.CV

TL;DR: 提出了一种基于共享解码器MLP和纹理聚类的神经纹理压缩方案，显著解决了传统NTC方法的GPU线程发散问题。


<details>
  <summary>Details</summary>
Motivation: 神经纹理压缩（NTC）在压缩率上优于传统格式，但因GPU线程发散问题导致渲染性能低下。

Method: 采用共享解码器MLP架构，结合渐进式解码器冻结训练策略，并利用CLIP嵌入进行纹理语义聚类以减少线程发散。

Result: 在500多种纹理和多个渲染场景中，线程发散降低了25%-52%，渲染速度最高提升至8.48倍。

Conclusion: 该方法通过统一的共享架构和聚类策略，在保持渲染质量的同时，有效解决了神经纹理压缩的性能瓶颈。
...
### [114] [CommerceVibe: Learning to Design E-Commerce Creatives as Executable Visual Code via Dual-Feedback Reinforcement Learning](https://arxiv.org/abs/2608.27893)
*Yajiao Xu,Jin Zhang,Jiangbo Ai,Tao Jiang,Mo Xu,Lina Huang,Chengfu Huo*

Main category: cs.CV

TL;DR: CommerceVibe 是一个基于可执行代码（HTML/CSS）生成电商创意图的模型，解决了传统扩散模型生成的图像难以编辑、文字扭曲及产品细节不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型生成的栅格化图像存在不可编辑、文字失真和细节不一致等缺陷，且缺乏有效的机制来满足复杂的电商设计需求。

Method: 将创意生成建模为条件 HTML/CSS 程序合成；采用基于规则的反馈（评价布局和可读性）与基于 VLM 的视觉反馈（评估感知与商业指标）进行双重反馈强化学习。

Result: 在 1300 个案例的基准测试中得分 94.0/100，优于仅经过监督微调（SFT）的变体及其他外部模型，并通过了电商设计专家的盲测验证。

Conclusion: CommerceVibe 实现了可控、可编辑且可扩展的电商创意自动化生产，证明了代码生成范式在复杂设计任务中的优势。
...
### [115] [DensityKV: Density-Guided KV Cache Compression for Long Video Generation](https://arxiv.org/abs/2608.27922)
*Wenqu Zhao,Xuemin Chi,Xin Zhang,Guoqing Ma,Baorun Li,Jianjie Fang,Peizhi Tang,Chen Gao,Wei Wu*

Main category: cs.CV

TL;DR: DensityKV 是一种训练无关的视频生成 KV 缓存管理策略，通过控制令牌级密度来限制冗余，从而提升自回归视频生成模型的长期一致性。


<details>
  <summary>Details</summary>
Motivation: 自回归视频生成中，递归生成的错误累积及无限增长的 KV 缓存会导致长期生成不稳定及显存瓶颈。

Method: 提出 DensityKV，通过计算 Soft-Riesz 密度测量历史键值（KV）的局部冗余，动态限制每个注意力头的历史缓存增长，实现存储空间的有界化。

Result: 在多种模型和长度下，DensityKV 在保持恒定存储上限的同时，显著提升了视频的长期一致性和生成稳定性。

Conclusion: DensityKV 有效解决了自回归视频生成中长序列建模的冗余与不一致性问题，实现了与生成长度无关的持久存储管理。
...
### [116] [PCBnet: A Dataset and Automatic Construction of SPICE Netlists from Schematic Images](https://arxiv.org/abs/2608.27923)
...
TL;DR: 本文提出了PCBnet数据集及自动化转换流程，旨在解决PCB设计自动化中缺乏大规模标注数据的问题。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏大规模的原理图-网表配对数据集，AI驱动的PCB设计自动化研究受到严重限制。

Method: 构建了包含300多个真实设计、超过5万个元件的大规模PCBnet数据集，并开发了结合视觉识别、拓扑构建和多智能体校正的原理图转网表自动化流水线。

Result: 该方法在元件检测、文本识别和端到端连通性任务上分别达到94.54%、98.57%和84.47%的精度。

Conclusion: PCBnet为AI辅助PCB设计自动化提供了关键的数据基准与研究基础。
...
### [117] [Training-Free Temporal Abstraction for General Video Understanding](https://arxiv.org/abs/2608.27929)
*Etienne Casanova,Sevan Brodjian,Pietro Perona*

Main category: cs.CV

TL;DR: STITCH 是一种无需训练的视频时间切分方法，利用预训练视频-文本模型将视频分割为具有语义意义的片段，从而支持多种视频理解任务。


<details>
  <summary>Details</summary>
Motivation: 逐帧分析视频成本高昂，且现有方法通常针对特定任务开发，缺乏通用性。研究人员旨在探索预训练模型是否能提供足以支持多项任务的通用时间结构。

Method: 使用冻结的视频-文本骨干网络对短视频窗口进行嵌入，通过检测嵌入序列中的变化来划分具有语义意义的时间片段，并实现跨任务重用。

Result: STITCH 在通用事件边界检测、基于语言的时刻检索和长视频 VLM 推理任务中，与特定任务模型相比表现出极强的竞争力，尤其在有限处理资源下优势显著。

Conclusion: 可重用的时间抽象是一种高效的视频理解方向，能将高密度视频流转换为可供下游任务使用的语义单元，从而简化多种视频理解流程。
...
### [118] [GAAT: Geometry-Aware Alignment Transformer for Multimodal UAV Perception](https://arxiv.org/abs/2608.27971)
*Jingpu Yang,Debin Tang,Yilin Sun,Fengxian Ji,Jiahua Zhu,Wenrui Ding,Yufeng Wang*

Main category: cs.CV

TL;DR: GAAT 是一种针对无人机（UAV）多模态感知的预训练模型，通过几何感知机制解决模态间的空间对齐难题，提升了复杂环境下的感知性能。


<details>
  <summary>Details</summary>
Motivation: 无人机多模态传感器（RGB/IR/SAR/深度）在实际应用中常受视差、运动和畸变影响，导致空间对应关系错位，削弱了传统密集对比学习和融合的效果。

Method: 提出了同步补丁一致性学习（syncPATC）以学习无监督几何先验；利用 MG-Sparse-MMA 进行几何校准的稀疏查询融合；引入 RA-QCGCL 进行监督对齐；构建 UAVMeta 和 StateBench 以评估 acquisition 状态。

Result: 在六项下游任务中表现出优异的迁移性能，确立了其作为无人机多模态感知基础模型的领先地位。

Conclusion: GAAT 通过显式估计局部对齐可靠性，有效解决了多模态感知中的非对齐问题，并为评估实际飞行环境提供了系统化框架。
...
### [119] [GAN-Based Semantic Communication for Image Transmission in IoV](https://arxiv.org/abs/2608.27989)
*Ruixing Ren,Shan Chen,Junhui Zhao,Xiaoke Sun*

Main category: cs.CV

TL;DR: 提出了一种基于生成对抗网络（GAN）的语义通信框架，用于车联网协同感知，以提升有限带宽下的视觉数据传输效率与图像保真度。


<details>
  <summary>Details</summary>
Motivation: 传统通信系统在处理高动态、带宽受限的车联网视觉数据传输时存在效率和保真度瓶颈。

Method: 发送端采用金字塔注意力网络提取语义标签并引入语义优先级机制；接收端利用从粗到细的多分辨率生成器和多尺度判别器重构图像，并引入时间一致性分支。

Result: 在 Cityscapes 数据集上测试表明，该方法在语义分割准确率和图像重构质量上均优于现有方法，且在 AWGN 和 Rayleigh 信道下表现稳定。

Conclusion: 该框架通过深度学习技术有效提升了语义通信的性能，能够实现复杂交通环境下高质量的视觉数据高效传输。
...
### [120] [A-PAIR: A Benchmark and Identity-Consistent Grounding Framework for Air-Ground Cross-View Referring Person Detection](https://arxiv.org/abs/2608.27997)
*Zhoupeng Guo,Xinjie Yao,Yunqi Zhu,Zhihe Fan,Siqi Zhao,Jianjun Chen,Yichen Dong,Yan Fan,Pengfei Zhu*

Main category: cs.CV

TL;DR: 该研究提出了首个针对“空地跨视角指代行人检测”（AGCV-RPD）的基准数据集 A-PAIR 及相应的检测框架 ICRG。


<details>
  <summary>Details</summary>
Motivation: 现有的指代理解方法难以处理空地跨视角下的身份一致性问题，且在该场景下存在行人干扰项多、空中视觉特征弱等挑战。

Method: 提出了半自动标注框架 FARA 以降低数据构建成本；引入了 ICRG 框架，通过分解参考基础、候选完整性监督及跨视角一致性校准实现联合检测。

Result: 在 A-PAIR 数据集上表现优异，将配对 F1 分数从 16.65% 提升至 22.28%。

Conclusion: 空地跨视角指代行人检测任务需要联合配对检测与身份一致性推理才能有效实现。
...
### [121] [Visual Token Coding for Video Multimodal Large Language Models](https://arxiv.org/abs/2608.28008)
*Chenxin Fang,Tao Chen,JunChao You,Jun Peng,Yiyi Zhou,Rongrong Ji*

Main category: cs.CV

TL;DR: 提出了一种名为视觉令牌编码（VTC）的视频多模态大模型（MLLM）令牌压缩范式，借鉴经典视频编码原理高效减少冗余。


<details>
  <summary>Details</summary>
Motivation: 现有视频MLLM面临巨大的计算开销和视觉令牌冗余问题，需要一种更高效的令牌压缩方案。

Method: 模仿HEVC视频编码，通过预测I/P帧并计算帧间残差来估计冗余；并引入动态分辨率输入（DyRSO）、动态令牌分配（DyTA）和空间覆盖Top-K（SC-TopK）等策略。

Result: 在Qwen3-VL上，VTC_Dy在令牌预算减半时性能保持率达100.1%，在令牌预算为25%时仍保留97.8%的性能。

Conclusion: VTC是一种有效的即插即用型视频令牌压缩方法，无需对MLLM进行额外微调即可显著降低计算成本并保持模型性能。
...
### [122] [3D-USE: From Image-Level to Scene-Level Underwater Enhancement](https://arxiv.org/abs/2608.28020)
*Jieyu Yuan,Yuanlin Zhang,Jihong Li,Chunle Guo,Huimin Lu,Chongyi Li*

Main category: cs.CV

TL;DR: 提出了一种名为3D-USE的两阶段框架，用于从退化的多视图水下图像中学习具有视觉增强效果的3D场景表示。


<details>
  <summary>Details</summary>
Motivation: 传统水下3D重建无法避免颜色偏移和能见度下降，而物理反演方法往往存在估计误差，缺乏一致的增强效果。

Method: 1. 提出MediumRBF建立媒介感知的高斯场景表示；2. 利用外观转换共识（ATC）将2D水下增强知识转化为全局和局部的目标；3. 通过水下双边外观场（U-BAF）实现高斯辐射与媒介外观的优化。

Result: 在真实水下场景实验中，该方法显著提升了能见度与跨视图的一致性，同时保持了高质量的场景重建效果。

Conclusion: 3D-USE实现了无需在线2D增强模型推理的稳健水下3D场景增强，有效解决了传统方法中的不一致性和重建质量问题。
...
### [123] [ZipMVS: Multi-View Stereo with Compressed Cost Volumes](https://arxiv.org/abs/2608.28033)
*Guanglin Jin,Hongshan Yu,Javier Civera,Zhaoxin Li*

Main category: cs.CV

TL;DR: ZipMVS 是一种旨在提高多视图立体视觉（MVS）内存效率的高质量重建方法。


<details>
  <summary>Details</summary>
Motivation: 传统 MVS 方法内存占用过高，难以在航空航天及自动驾驶等资源受限的系统中部署。

Method: 提出了一种新颖的深度假设策略，通过压缩代价体积（cost volume）显著降低 GPU 显存消耗。

Result: 在 DTU 和 Tanks and Temples 数据集上，ZipMVS 在保持高重建精度的同时，实现了显存效率与质量之间的极佳平衡。

Conclusion: ZipMVS 通过创新的代价体积压缩技术，为资源受限环境下的 MVS 重建提供了一种高效且实用的解决方案。
...
### [124] [Dynamic Alignment Compensation for Hallucination Mitigation in Large Vision-Language Models](https://arxiv.org/abs/2608.28058)
*Kairong Yu,Zixin Zhu,Le Yu,Hongwei Wang*

Main category: cs.CV

TL;DR: 提出了一种名为DAC的训练后推理阶段方法，通过动态残差补偿解决LVLM中跨模态表示退化和漂移导致的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有的幻觉缓解方法多依赖外部监督或注意力调节，而忽略了自回归生成过程中内部表示动态的变化，导致模型在生成过程中出现跨模态表示失真。

Method: 提出了动态对齐补偿（DAC），包含“逐层语义补偿”以抑制层间退化，以及“序列语义校正”以限制时间步上的表示漂移。

Result: 在9个多模态基准测试上证明，DAC在无需额外训练的情况下，能有效降低模型幻觉并保持整体性能。

Conclusion: 通过在推理时动态纠正内部表示的不稳定性，可以显著提升LVLM的忠实度和多模态生成质量。
...
### [125] [A Controlled Audit of Architectural Complexity in Uncertainty-Aware Multi-Organ Ultrasound Classification](https://arxiv.org/abs/2608.28063)
*Yang Song,Pengbo Sun,Shichang Feng,Ye Zhu,Xin Xu,Ziran Wang*

Main category: cs.CV

TL;DR: 该研究通过受控复杂性审计框架评估了多器官超声分类器中各种复杂组件（如EDL、MoE等）的必要性，发现简单的交叉熵加温度缩放（Simple-CE+TS）在性能和校准上表现优异，且复杂组件并未带来显著增益。


<details>
  <summary>Details</summary>
Motivation: 许多超声分类模型通过叠加注意力机制、专家混合（MoE）和证据深度学习（EDL）来应对解剖结构的异质性，但缺乏证据证明这些复杂组件确实提升了系统性能。

Method: 引入受控复杂性审计框架，对比了六种模型架构，使用匹配随机种子、冻结数据集划分、对称温度缩放及OOD（分布外）否决机制，对模型进行容量和优化敏感性测试。

Result: Full-EDL（复杂模型）并未在宏观F1分数上表现出显著优势；Simple-CE+TS在校准负对数似然准则上表现稳健，且复杂的门控机制在测试中几乎无影响。

Conclusion: 模型组件的保留应基于功能和重训练证据。对于多器官超声分类，简单的模型结合温度缩放即可达到优秀性能，过度设计并不能保证分布外环境下的安全性和可靠性。
...
### [126] [VersaGauss: A Versatile Framework for Generating Multiphase Dynamics with 3D Gaussians](https://arxiv.org/abs/2608.28069)
*Ruijie Su,Lingxiao Yang,Xiaohua Xie,Jianhuang Lai*

Main category: cs.CV

TL;DR: VersaGauss 是一个统一的 3D 高斯框架，通过物理驱动实现多相态交互的动态场景生成、模拟与渲染。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 高斯方法多聚焦于单一固态物体的物理模拟，缺乏对多相态（流体、砂石、橡胶等）交互的通用支持。

Method: 引入粒子剪枝算法优化高斯核分布；提出耦合多相态点法 (CMPM) 进行多相物理建模；采用谐波插值和高斯演化策略提升流体渲染逼真度。

Result: 实验表明该框架能成功模拟并渲染包含流体、橡胶、砂、雪等多种材料在内的复杂交互场景。

Conclusion: VersaGauss 有效扩展了 3D 高斯表示在多相物理交互场景下的生成与模拟能力，实现了逼真的动态场景重构。
...
### [127] [CF-YOLO: Context-Aware Feature Refinement for Camouflaged Industrial Micro-Defect Detection](https://arxiv.org/abs/2608.28070)
*Xinda Yu,Kunxin Zheng,Chunan Yu,Qingbo Song,Hao Xiao,Ying Zang,Jie Liu*

Main category: cs.CV

TL;DR: 提出了一种针对工业铜管表面微小缺陷检测的实时框架，通过上下文感知与特征细化模块解决背景伪装与特征微弱问题。


<details>
  <summary>Details</summary>
Motivation: 工业组件中微小缺陷由于尺度极小且与背景高度伪装，导致传统检测算法表现不佳，存在高误报和漏检问题。

Method: 引入上下文感知聚合模块（CPAM）用于宏观纹理感知与边界界定，以及特征加法细化模块（FARM）进行全局特征验证与噪声抑制；同时发布了包含1,847张图像的铜管缺陷数据集（CTDD）。

Result: 在CTDD数据集上，该方法较YOLOv11在mAP@50上提升了2.2%，精度提升了3.9%，并保持了实时推理速度。

Conclusion: 该方法通过有效结合全局上下文理解与局部特征细化，实现了在工业环境下对伪装微小缺陷的高精度实时检测。
...
### [128] [Task-State Adaptation with Prototype Memory for Multi-Task Dense Prediction](https://arxiv.org/abs/2608.28078)
*Yangyang Xu,Haobo Yuan,Yuzhu Wang,Duo Su,Xi Ye,Yibo Yang,Jun Zhu*

Main category: cs.CV

TL;DR: 提出了一种名为MemMTL的多任务密集预测框架，通过任务状态原型记忆和稀疏路由机制，实现对单一视觉骨干网络特征的高效适配。


<details>
  <summary>Details</summary>
Motivation: 单一共享视觉骨干网络难以同时满足不同密集预测任务对特征适配的多样化需求。

Method: 利用全局上下文估计任务状态，通过学习到的任务状态原型记忆进行细化，并结合专家逻辑（expert logits）进行稀疏top-k路由选择，辅以任务无关的残差库进行通用适配。

Result: 该框架通过任务状态调节、原型检索和稀疏路由的组合，实现了多任务预测质量与计算成本的平衡。

Conclusion: MemMTL有效地解决了单一骨干网络在多任务密集预测中的适配矛盾，验证了任务条件化与稀疏专家分配的有效性。
...
### [129] [Cyc3D: Evaluating Cyclic Structural Stability and Asset Usability in Image-to-3D Generation](https://arxiv.org/abs/2608.28080)
*Liwen Zhang*

Main category: cs.CV

TL;DR: 本文提出了Cyc3D基准，旨在评估图像到3D生成模型的跨视角一致性及资产可用性，填补了现有评价体系仅关注渲染视觉效果的空白。


<details>
  <summary>Details</summary>
Motivation: 现有评价体系忽视了3D生成模型的稳定性及生成的资产在图形流水线中的实际可用性，难以量化模型在几何结构上的真实理解能力。

Method: 提出了多维基准Cyc3D，包含‘跨视角对象一致性’与‘表征质量’两个核心维度，并引入了‘视图循环结构一致性’协议，通过渲染-重生成-对齐的闭环流程来量化几何与语义漂移。

Result: 实验显示闭源前馈模型在几何保真度与稳定性上优于开源优化方法，但所有方法在循环稳定性得分上均低于48分，反映出视觉合理性与稳健3D理解之间存在显著差距。

Conclusion: Cyc3D有效揭示了现有图像到3D系统在几何结构与生产级资产可用性上的不足，为评估模型的真正3D理解能力提供了更客观的诊断工具。
...
### [130] [Attribute Token Arithmetic: Disentangled and Continuous Semantic Control for Visual Autoregressive Models](https://arxiv.org/abs/2608.28082)
*Xindi Yang,Yicheng Wu,Cheng Zhang,Jianfei Cai,Tien-Tsin Wong*

Main category: cs.CV

TL;DR: 本文提出了一种名为Attribute Token Arithmetic (ATA)的方法，通过在预训练的自回归模型潜空间内进行语义向量运算，实现对图像属性的细粒度、解耦及连续控制。


<details>
  <summary>Details</summary>
Motivation: 自回归文生图模型在细粒度语义控制上面临属性纠缠和文图表示对齐困难的问题，难以精准调整特定属性。

Method: 引入Attribute Token Arithmetic (ATA)，利用预训练自回归潜空间的向量运算特性，仅需单张参考图即可学习并提取特定视觉属性（如年龄、表情）的语义方向，无需重新训练模型。

Result: 实验表明，ATA能够实现保持身份特征的细粒度属性调整和多属性组合，在可控性、泛化能力和计算效率上均优于现有的自回归编辑基线。

Conclusion: ATA为自回归图像生成模型提供了一种高效、即插即用的语义控制方案，有效解决了复杂属性控制中的纠缠问题。
...
### [131] [Token-Budget Distillation: Transferring Full-Token Semantics to Compressed Video Vision-Language Models](https://arxiv.org/abs/2608.28138)
*Xiaoyang Guo,Guoping Luo,Jusheng Zhang,Keze Wang,Wenhao Wang*

Main category: cs.CV

TL;DR: 提出了一种名为Token-Budget Distillation (TBD)的视频视觉语言模型（VLM）高效微调框架，通过双路径蒸馏技术在大幅压缩视觉Token的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频VLM微调时视觉Token量巨大导致计算开销高昂，而直接压缩会导致语义丢失和性能显著下降。

Method: 冻结骨干网络，仅训练LoRA适配器，并利用全Token教师模型指导压缩后的学生模型，结合任务损失、答案区域KL蒸馏、GT锚定边际蒸馏及可靠性感知蒸馏控制。

Result: 在LLaVA-Video等多个模型及基准测试中，TBD在仅保留10% Token的情况下，仍能保持原模型97%-100%的性能，优于所有压缩基线方法。

Conclusion: 通过高效的教师-学生蒸馏机制，TBD成功解决了视频VLM在低Token预算下的语义漂移问题，实现了计算效率与模型精度的平衡。
...
### [132] [Ex-Sim(3)-Reg: 2D-3D Correspondence Pruning via Extended Sim(3) Registration](https://arxiv.org/abs/2608.28096)
*Pei An,Muyao Peng,Junfeng Ding,Jiaqi Yang,Liangliang Nan*

Main category: cs.CV

TL;DR: 提出了一种名为Ex-Sim(3)-Reg的2D-3D对应关系剪枝算法，通过显式建模深度噪声来提升图像到点云（I2P）的配准鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的I2P方法在面对低内点、变形或遮挡等复杂场景时表现不佳，而现有的基于深度先验的转换方法忽略了单目深度估计中固有的噪声问题。

Method: 将 correspondence pruning 问题重构为扩展的Sim(3)配准问题，通过显式建模深度噪声提升配准精度，并提供了相应的理论证明。

Result: 在7-Scenes、RGBD-V2、ScanNet和TUM数据集上实验表明，该方法在配准召回率上比SOTA方法提升了最高24.7%。

Conclusion: Ex-Sim(3)-Reg是一种简单且有效的剪枝算法，通过考虑深度先验的不确定性，显著提高了复杂场景下I2P配准的可靠性。
...
### [133] [Dual-Stream Semantic Guidance with Prototype Anchor Calibration for Source-Fully-Free Adaptation of Vision-Language Models](https://arxiv.org/abs/2608.28145)
*Weiwei Xiang,Shun Peng,Guangyi Xiao,Hao Chen,Lei Yang*

Main category: cs.CV

TL;DR: 提出了一种名为DSSG的无源域适应（SFF-DA）框架，用于解决视觉语言模型在适应过程中的双重语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有SFF-DA方法在适应过程中存在静态类嵌入和动态生成标注导致的“双重语义漂移”，加剧了稳定性与可塑性之间的矛盾。

Method: 提出了DSSG框架，包含双重语义引导（DSG）模块以平衡领域知识与全局一致性，引入动态跨模态知识蒸馏（CMKD）校准师生一致性，以及原型锚点校准（PAC）机制以降低计算冗余。

Result: DSSG在多个基准测试中优于现有的SOTA方法，而DSSG-PAC在保持高适应性能的同时，将总适应时间降低了18.9%。

Conclusion: 该研究通过理论证明了SFF-DA的风险界限，并证实了双重语义引导机制能够有效缓解语义对齐问题，实现高效的无源域适应。
...
### [134] [Empowering Local Agriculture: A Deep Learning-Powered Web System for Identifying Bangladeshi Mango Varieties](https://arxiv.org/abs/2608.28161)
*Monowar Islam,Safaruzzaman Shovo*

Main category: cs.CV

TL;DR: 该研究开发了一个基于深度学习的Web系统，用于自动识别孟加拉国的九种芒果品种。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国芒果品种繁多且外观相似，在复杂环境下的品种鉴定存在挑战，缺乏便捷的自动化识别工具。

Method: 收集2013张芒果图像并进行预处理，对比了ResNet18、ResNet50和EfficientNetB0三种预训练CNN架构，并使用Streamlit构建了Web应用。

Result: EfficientNetB0表现最佳，测试准确率达到97.36%，模型参数仅约400万，非常适合轻量化部署。

Conclusion: 该系统证明了深度学习在孟加拉国农业应用中的有效性，为芒果品种鉴定提供了一个实用且易于访问的工具。
...
### [135] [Manifold4D: Denoising on Point Cloud Rendered Manifolds for Video Re-shooting](https://arxiv.org/abs/2608.28174)
*Yongqi Mao,Zijia Dai,Zhishuo Liu,Wei Xu,Kaiwei Wang,Guotao Meng*

Main category: cs.CV

TL;DR: MANIFOLD4D提出了一种将渲染引导信息融入初始噪声分布的新颖方法，解决了视频重拍摄中条件竞争导致的信任难题。


<details>
  <summary>Details</summary>
Motivation: 传统视频重拍摄方法将渲染图和源视频同时作为去噪条件，导致模型在两者间产生“信任困境”，影响轨迹控制精度和泛化能力。

Method: 引入MANIFOLD4D，将几何渲染图直接注入流匹配（Flow Matching）的初始噪声中，形成包含几何信息的噪声流形，仅保留源视频作为视觉条件。

Result: 在DAVIS-Traj和Vista4D评估中表现最佳，旋转误差降低最高达27%，平移误差降低达32%，在超出训练范围的轨迹上表现出更强的鲁棒性。

Conclusion: 通过将几何先验融入初始噪声，模型有效摆脱了渲染图的条件竞争，实现了更精确的轨迹跟踪与动态一致性。
...
### [136] [EXPOSE: Explainable and Domain-Robust Embeddings from Pathology Vision Foundation Models using Sparse Autoencoders](https://arxiv.org/abs/2608.28191)
*Anja Witte,Maximilian Lennartz,Jan Baumbach,Guido Sauter,Stefan Bonn,Patrick Fuhlert,Marina Zimmermann*

Main category: cs.CV

TL;DR: 提出了一种名为EXPOSE的框架，利用稀疏自编码器（SAE）识别并抑制计算病理学中视觉基础模型（VFM）嵌入中的域特定信息，以提高跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: VFM嵌入中混杂了生物信息和域特定信息（如染色差异、扫描仪硬件等），导致模型在面对不同数据源时泛化性能下降。

Method: 使用SAE作为可解释的瓶颈层提取稀疏特征，通过线性分类器定位域特定潜变量维度，并在下游任务预测前将这些维度进行掩码处理，无需重新训练主干模型。

Result: 实验证明该方法能有效解耦域特定和任务特定信息，提升跨域预测性能及模型稳健性指标（DoRI）。

Conclusion: 通过稀疏特征解耦，EXPOSE能够在不重新训练大模型的情况下，有效缓解计算病理学中的领域偏移问题。
...
### [137] [Locate Anything in Videos: Rethinking Efficient Generative Spatio-Temporal Video Grounding](https://arxiv.org/abs/2608.28192)
*Hanoona Rasheed,Haania Siddiqui,Ming-Hsuan Yang,Fahad Shahbaz Khan,Salman Khan*

Main category: cs.CV

TL;DR: 提出了一种名为Parallel Tube Decoding (PTD)的视频时空定位模型，通过并行化时空解码取代传统的自回归序列生成，显著降低了解码延迟并提升了定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大模型的视频定位方法采用自回归方式生成轨迹，导致解码延迟随视频长度增长，且误差会在时间维度上累积。

Method: 引入PTD框架，将定位分解为时间块预测和时间条件空间块的并行解码；采用解耦块注意力机制(Decoupled Block Attention)消除框间依赖，并结合定位感知策略优化。

Result: 在VidSTG数据集上，PTD将管状物补全延迟降低了79倍，空间解码吞吐量提升了92倍，同时在多个基准测试中表现优于自回归模型。

Conclusion: 并行管状物生成是替代视频自回归定位的一种高效且有效的方案，且具备良好的泛化能力。
...
### [138] [Cut-ViT: Task-Specific Model Pruning via Gram Anchoring Subspace Consistency](https://arxiv.org/abs/2608.28205)
*Jianjian Yin,Liulei Li,Tao Chen,Yi Chen,Yazhou Yao,Wenguan Wang*

Main category: cs.CV

TL;DR: Cut-ViT 是一种针对视觉基础模型（如 DINOv3）的高效任务特定剪枝框架，通过子空间对齐和谱熵适应实现性能最优。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法存在鲁棒性下降和任务特异性不足的问题，且通常依赖于单一数据集上的刚性点对点标记对齐。

Method: 构建空间与语义视角的 Gram 锚定矩阵进行子空间分解；引入子空间对齐约束以保留鲁棒特征；设计谱熵适应机制来根据任务需求动态调整剪枝目标。

Result: 在 9 个数据集的 6 项任务中达到 SOTA，同时剪枝速度提升约 79.1%，GPU 内存占用降低 54.5%。

Conclusion: Cut-ViT 提供了一种高效、鲁棒且具备任务适应性的剪枝方案，证明了子空间对齐在模型压缩中的有效性。
...
### [139] [UniLipi: A Unified Multi-Script OCR for Historical Indic Manuscripts](https://arxiv.org/abs/2608.28195)
*Tathagata Ghosh,Sai Madhusudan Gunda,Simran Singh Sandral,Ravi Kiran Sarvadevabhatla*

Main category: cs.CV

TL;DR: UniLipi是一个统一的多脚本OCR模型，旨在解决手写印度手稿在大规模数字化中的跨脚本泛化和资源匮乏问题。


<details>
  <summary>Details</summary>
Motivation: 现有OCR模型通常针对单一脚本且定制化程度高，难以扩展到多样化的历史手稿集合，且面临标注数据匮乏的挑战。

Method: 构建了一个涵盖13种印度脚本的统一OCR框架，采用脚本感知的合成数据生成技术，并作为预训练模型处理多种手写文本。

Result: 该模型不仅能处理复杂的历史手稿（如几何畸变、污渍干扰），还能推广至现代手写及非印度语言（如中文、拉丁文等），并支持脚本识别和字符计数。

Conclusion: UniLipi证明了通过统一框架和合成数据训练，可以有效解决低资源手写OCR问题，并作为通用的OCR基础模型具有高度的可扩展性。
...
### [140] [Explainable Diabetic Retinopathy Classification Using Vision Foundation Models](https://arxiv.org/abs/2608.28207)
*Abhishek Verma,Anila Krishna,Abhishek Gajanan Bankar,Juan Miguel Lopez Alcaraz*

Main category: cs.CV

TL;DR: 本研究利用视觉基础模型（DINOv2, CLIP, ViT）及多种迁移学习策略构建了可解释的糖尿病视网膜病变（DR）筛查框架。


<details>
  <summary>Details</summary>
Motivation: 解决糖尿病视网膜病变筛查中对高准确性与模型可解释性的需求，并探索参数高效迁移学习的潜力。

Method: 对比全量微调、线性探测和LoRA三种策略；使用ODIR和APTOS数据集验证性能，并通过Grad-CAM与HiResCAM结合专家标注评估模型的可解释性。

Result: DINOv2在内部测试中表现最佳（AUROC 0.758），全量微调在外部泛化评估中表现最佳（AUROC 0.920）；LoRA验证了在保持性能的同时实现高效参数更新的可行性。

Conclusion: 视觉基础模型（尤其是DINOv2）在DR诊断中具有强大的预测能力，其关注点与临床病变区域具有较高的一致性，证明了该框架在临床辅助诊断中的应用潜力。
...
### [141] [NumBench: Diagnosing Counting Failures in Text-to-Image Models](https://arxiv.org/abs/2608.28206)
*Sandeep Wadhwa,Mayank Vatsa,Richa Singh,Parrva Chirag Shah,Prakhar Galriya*

Main category: cs.CV

TL;DR: 本文提出了NumBench基准测试，用于系统性评估和分析文本生成图像模型在处理数量控制（1-100个对象）时的性能表现及局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试规模较小且缺乏控制，难以解释为何文本生成图像模型常出现对象数量错误。

Method: 构建包含64万条提示词的NumBench基准；建立“竞争区域”过程模型分析对象冲突；提出置信度加权数值精度分数（CWNPS）进行评估；进行大规模自动化测试与人类评测验证。

Result: 所有主流模型在请求数量超过50时性能急剧下降；数量范围是影响性能的最大因素；网格引导布局能有效缓解冲突，但未完全解决问题。

Conclusion: 当前生成模型在处理高数量对象时普遍存在局限，其失败原因与对象空间竞争密切相关，且现有模型的空间协调能力仍需显著提升。
...
### [142] [Focus Where It Counts: A Salience-Driven Vision-Language Model for Low Vision Assistance](https://arxiv.org/abs/2608.28218)
...
TL;DR: 本文提出了一个面向视障人士的“显著性驱动”视觉语言模型（Salience-LLaVA），能够按重要性顺序描述场景中的关键元素。


<details>
  <summary>Details</summary>
Motivation: 现有通用视觉语言模型（VLMs）缺乏对人类感知优先级的建模，无法有效筛选并按重要性呈现信息，难以满足视障人士在辅助技术中的实际需求。

Method: 构建了三个包含对象级显著性标注的数据集（Salience COCO, Flickr, VizWiz）；提出了Salience-LLaVA模型，引入显著性线索以优化字幕生成顺序；设计了SCMI指标评估排序准确性，并在智能眼镜上进行了实地部署。

Result: Salience-LLaVA生成的字幕能显著优先呈现对低视力用户最重要的信息，在排序准确性和辅助实用性上优于现有模型。

Conclusion: 通过显式整合人类感知的显著性信息，能够显著提升视觉辅助技术在真实场景中的交互质量和信息有效性。
...
### [143] [WALDO: One-Shot Exemplar-Conditioned Object Detection in Cluttered Scenes](https://arxiv.org/abs/2608.28216)
*Kishor Datta Gupta,Ahmed Rafi Hasan,Md. Mahfuzur Rahman,Md. Sadman Haque,Mohd Ariful Haque*

Main category: cs.CV

TL;DR: WALDO 是一种轻量级（3.4M 参数）检测头，利用冻结的 V-JEPA 世界模型特征，通过单样本图像和语言描述实现目标定位与缺失检测。


<details>
  <summary>Details</summary>
Motivation: 探索是否能利用自监督学习到的世界模型特征，以极低成本实现与大型视觉语言模型（LVMs）相当的特定实例定位和缺失判定能力。

Method: 利用冻结的 V-JEPA 2.1 特征作为输入；构建合成训练剧集，通过挖掘真值框作为正样本，并设计排除目标但保留同类干扰项的负样本，同时解决模型依赖裁剪尺寸而非特征的“捷径”问题。

Result: 在 cluttered 场景下，WALDO 的 AP@50 (0.461) 优于 Grounding DINO (0.306)，且在 V-JEPA 与 DINOv3 的对比实验中证明了世界模型目标在定位和缺失检测上的显著优势。

Conclusion: 世界模型预训练特征在目标定位精度和缺失检测方面具有强大迁移能力，但在识别具体目标身份（instance identity）方面表现有限，仍需进一步研究。
...
### [144] [RASA: Disentangled Spatial-Motional Priors for Cross-Identity Character Animation](https://arxiv.org/abs/2608.28219)
*Zhen Xiao,Zhen Shen,Zhaofan Qiu,Ting Yao,Xueliang Liu,Tao Mei*

Main category: cs.CV

TL;DR: RASA 是一种跨身份角色动画框架，通过解耦空间映射与运动控制，利用扩散 Transformer (DiT) 实现高质量的角色驱动。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理跨身份动画中空间映射（比例/骨架对齐）与运动控制（关节/体积一致性）相互纠缠的问题。

Method: 提出了两阶段框架：1. 空间先验校准器 (SPC) 生成空间对齐的初始噪声；2. 内在运动引导器 (IMG) 将形状无关的 SMPL 参数注入 DiT 层，实现解耦控制。

Result: 实验证明 RASA 在运动保真度和视觉质量上均优于现有的先进方法。

Conclusion: 解耦空间先验与运动先验是实现稳健跨身份角色动画的关键范式。
...
### [145] [WilLaGS: Latent-Conditional 3D Appearance Fields for Robust Gaussian Splatting In-the-Wild](https://arxiv.org/abs/2608.28240)
*Yuhao Bai,Qianqiu Tan,Lilong Chen,Huanhuan Lv,Lijun Chen*

Main category: cs.CV

TL;DR: WilLaGS 是一种用于非受限环境下 3D 高斯泼溅（3DGS）的统一框架，通过生成式外观模型和自监督感知掩码技术解决了复杂光照和瞬态物体导致的渲染质量问题。


<details>
  <summary>Details</summary>
Motivation: 现有 3DGS 方法难以处理非受限场景中剧烈的外观变化、光照波动以及瞬态物体，且离散嵌入方式无法捕捉连续的环境演变。

Method: 引入基于 β-VAE 的生成式外观模型以捕捉连续全局外观，利用动态 Tri-Plane 特征编码局部空间变化的光照，并采用 Teacher-Student (EMA) 架构的自监督感知掩码来剔除瞬态物体干扰。

Result: 在多个数据集上实现了最先进的重建质量和新视角外观合成效果，同时保持了实时渲染能力。

Conclusion: WilLaGS 有效提升了复杂、非受限场景下 3DGS 的稳健性与生成能力，实现了高保真且高效的场景重建。
...
### [146] [A comprehensive and trustworthy benchmark of AI methods for change detection in Earth observation](https://arxiv.org/abs/2608.28247)
*Tadej Tomanič,Alice Baudhuin,Jan Sotošek,Jure Brence,Panče Panov,Nikola Simidjievski,Dragi Kocev*

Main category: cs.CV

TL;DR: 该研究建立了一个标准化的开源基准，用于评估地球观测（EO）变化检测任务中主流深度学习模型的效果与效率。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏统一评估协议，且过分关注预测准确度而忽视了计算效率，导致模型对比存在局限性。

Method: 对10种代表性模型（包括CNN和ViT）在10个异构数据集上进行统一协议下的严格评估，对比从零训练与预训练的效果，并分析参数量及推理延迟。

Result: 经过优化的经典架构（如Siamese U-Net）在综合考量计算效率后往往优于更复杂的现代模型；预训练可显著提升性能且不增加推理成本。

Conclusion: 该基准强调了计算效率在EO任务中的重要性，并为该领域的透明度与可重复性研究提供了开放资源。
...
### [147] [Synth-JDoc: Synthesizing a Japanese Document Image Dataset for OCR with Diverse Layouts and Embedded Images](https://arxiv.org/abs/2608.28248)
*Keito Sasagawa,Shuhei Kurita,Daisuke Kawahara*

Main category: cs.CV

TL;DR: 本文提出了一种通过合成高质量日文文档图像数据集的方法，旨在解决大视觉语言模型（LVLM）在处理垂直书写日文时性能不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的LVLM在处理日文文档时，垂直书写文本的识别能力远低于水平文本，而人工构建大规模OCR数据集成本高昂且难以扩展。

Method: 利用HTML和CSS合成包含多栏、混合垂直/水平书写的文档图像，通过文本转图像模型嵌入视觉元素，并引入噪声与退化滤镜以增强模型鲁棒性。

Result: 实验表明，使用该合成数据集微调的模型在读取垂直日文文本方面表现优于对比方案，具有更高的有效性。

Conclusion: 通过合成数据生成技术可以高效构建高质量文档数据集，显著弥补LVLM在特定语言排版格式上的识别短板。
...
### [148] [Non-Uniform Quantisation for 3DGS Compression](https://arxiv.org/abs/2608.28272)
*Bert Van hauwermeiren,Patrice Rondao Alface,Adrian Munteanu*

Main category: cs.CV

TL;DR: 提出了一种针对3D Gaussian Splatting (3DGS) 的非均匀量化和合并压缩方案，旨在降低高码率需求并推动标准化。


<details>
  <summary>Details</summary>
Motivation: 3DGS模型的高比特率限制了其在存储和传输方面的实际应用，缺乏标准化的压缩格式。

Method: 提出了一种基于重要性权重的非均匀量化方案，并通过重要性加权合并技术消除体素化后的冗余。

Result: 在基准数据集上实现了最先进的压缩性能，且兼容多种点云表示方法。

Conclusion: 该方法通过高效压缩提升了3DGS的实用性，并计划作为MPEG 3DGS压缩标准化的重要贡献。
...
### [149] [GeoFF3D: Coordinate-Anchored Feed-Forward Reconstruction for Large-Scale UAV Mapping](https://arxiv.org/abs/2608.28288)
*Xiang Yang,Yongli Wang,Yunsheng Zhang*

Main category: cs.CV

TL;DR: GeoFF3D 是一种结合地理参照坐标模型与空间大规模重建框架（SLRF）的前馈 3D 重建方法，专门用于解决大规模无人机航拍的高效、鲁棒重建问题。


<details>
  <summary>Details</summary>
Motivation: 现有前馈 3D 重建方法在处理大规模、长序列航拍数据时，存在局部坐标框架对齐不稳定以及难以进行多块扩展的问题。

Method: 利用地理参照的相机位姿和几何先验，在重力对齐的 Z-up 度量空间直接预测相机姿态与稠密点云；引入 SLRF 框架将图像分区并在重叠区域进行分层聚合。

Result: 在九个航拍数据集上表现优于现有方案，F@5 指标从 0.829 提升至 0.877；在长序列 UAVScenes 数据集上较基线提升显著，且能在 5 分钟内处理 2000 张图像。

Conclusion: GeoFF3D 通过地理锚定模型与分层重建架构，成功实现了大规模、高效率且鲁棒的无人机三维重建。
...
### [150] [FUSED: Forensic-Semantic Mixture-of-Experts for AI Inpainting Detection and Localization](https://arxiv.org/abs/2608.28302)
*Anton Nuzhdin,Marcel Worring,Ivona Najdenkoska*

Main category: cs.CV

TL;DR: FUSED是一个用于检测和定位AI生成图像修复（inpainting）的统一框架，通过结合底层取证特征与高层语义特征，提升了对未见生成模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有检测器过度依赖全局生成伪影，导致在面对不同生成器（分布偏移）时泛化能力差，且在图像修复区域外还原像素后性能显著下降。

Method: 引入了稀疏门控混合专家（MoE）架构，将底层取证线索与高层语义特征融合，能够自适应地为每个Token选择最相关的特征，并同时输出图像级分数和像素级掩码。

Result: 在OpenSDID及其他基准测试（AutoSplice、CocoGlide）中表现领先，在未见生成器上的表现尤为显著，定位性能提升了一倍以上。

Conclusion: FUSED通过多特征融合和自适应架构，显著增强了对AI修复内容的鲁棒性和泛化能力，即便在去除全局伪影的情况下依然保持最强性能。
...
### [151] [AIM: Anchor Identity Features, Then Match for Multimodal Large Language Model Unlearning](https://arxiv.org/abs/2608.28312)
*Wonjun Lee,Jaehyuk Jang,Kangwook Ko,Hee-Seon Kim,Changick Kim*

Main category: cs.CV

TL;DR: 提出了一种名为AIM的MLLM身份遗忘方法，在无需保留图像的情况下实现隐私保护。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM遗忘方法依赖保留图像，在实际场景中难以实现，且需解决身份知识与视觉感知知识解耦的问题。

Method: 通过分析发现身份信息与视觉感知在隐藏空间中呈不同聚类分布，提出两阶段方法AIM：利用通用视觉提示锚定遗忘目标，并在Fisher信息约束下调整视觉编码器。

Result: AIM在实现有效身份遗忘的同时，能保留模型的视觉感知能力、已有知识及其他身份信息。

Conclusion: 身份知识与视觉感知知识可被有效分离，无需保留数据即可通过AIM实现针对性的身份遗忘。
...
### [152] [Conditional Visual Evidence Utility: State-Dependent Rank Reversals in Frozen Vision-Language Encoders](https://arxiv.org/abs/2608.28316)
*Yunxuan Fang,Xinhe Wang*

Main category: cs.CV

TL;DR: 该研究指出视觉证据的重要性并非静态，而是随观察过程动态变化的，并提出了在视觉搜索中进行条件化证据更新的必要性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉搜索方法通常将视觉证据压缩为静态排名，忽略了在观察到部分线索后，剩余证据的价值会发生改变这一现象。

Method: 通过控制组成视觉搜索实验，测量颜色、形状和纹理证据在不同获取状态下的条件边际效用，并利用OpenCLIP和SigLIP模型在800个场景中验证状态依赖的排名逆转。

Result: 研究发现视觉证据存在稳定的状态依赖排名逆转，且这种更新机制在不同的评估模型、措辞和证据模式下具有决策相关价值。

Conclusion: 视觉证据的使用应采取条件化评估而非静态排名，这为开发自适应证据选择方法提供了明确的目标。
...
### [153] [Abstract4D: A Large-Scale Dataset and Framework for Understanding the Visual Language of Abstract Art](https://arxiv.org/abs/2608.28339)
*Haowei Zhang,Yuanpei Zhao,Ji-Zhe Zhou,Mao Li*

Main category: cs.CV

TL;DR: Abstract4D 是目前最大的抽象画数据集，包含超过12万张带有感知属性元数据的图像，旨在评估 AI 对抽象艺术视觉语言的理解与生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 模型缺乏对抽象艺术视觉语言（如形式、颜色、纹理、构图）的理解，且缺乏高质量的大规模抽象画数据集进行测试。

Method: 构建了包含12万张抽象画的 Abstract4D 数据集，采用人机协作（人类与视觉语言模型 VLM）的标注流程，并对嵌入空间进行可视化分析，同时设置了分类、跨模态检索及文生图等基准测试。

Result: 通过大规模嵌入可视化揭示了感知关系如何组织艺术意义，并为 AI 在抽象艺术领域的感知与表现能力提供了量化评估标准。

Conclusion: Abstract4D 为探索和定量评估 AI 对抽象艺术的表示与解释能力提供了有力支撑，是连接计算感知与抽象艺术研究的重要工具。
...
### [154] [Cross-Spectral Dense Correspondence for Multimodal Spectral Medical Imaging](https://arxiv.org/abs/2608.28341)
*Eric L. Wisotzky,Jost Triller,Simon W. Härtl,Oliver T. Bruns,Peter Eisert,Anna Hilsmann*

Main category: cs.CV

TL;DR: 本文针对多模态光谱成像中跨光谱密集对应问题，提出了一种传感器无关的交叉光谱调制协议和合成基准，有效解决了因光谱敏感性差异导致的数据稀缺与匹配性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 多模态光谱图像常存在波长相关的对比度差异、强度反转及外观偏移，现有基于RGB的训练数据无法提供充分监督，导致跨光谱密集对应匹配性能不佳。

Method: 引入了基于强度投影的传感器无关交叉光谱调制协议，构建了模拟物理合理辐射差异的合成跨光谱对应基准，并结合视点依赖的通道选择与非线性辐射变换进行训练。

Result: 在多种密集对应主干网络上验证表明，该协议在严重光谱失配情况下显著提升了性能，同时保持了标准RGB基准上的匹配精度。

Conclusion: 现有模型性能的主要瓶颈不在于结构匹配能力，而在于训练分布与目标图像光谱特性之间的偏差；该方案为医学光谱成像等领域提供了有效的空间相干融合支持。
...
### [155] [Denoising-Aware Temporal Point Cloud Completion for 3D Crop Architecture Recovery and Phenotypic Trait Extraction](https://arxiv.org/abs/2608.28343)
*Mrudul Mittal,Soumyashree Kar*

Main category: cs.CV

TL;DR: 本文提出了SynthCrop4D数据集及一种结合时空补全的植物3D重构框架，用于解决植物生长阶段几何形状缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 缺乏具有完整几何真实标签的植物时间序列数据集，限制了植物高通量表型分析中3D重构算法的发展与评估。

Method: 构建了SynthCrop4D合成数据集；设计了两阶段流程，包括基于Mamba-DG的结构去噪模块和自适应时间PoinTr补全模型，利用前一时间点信息恢复当前阶段遮挡区域。

Result: 去噪模块显著提升了重构精度，在SynthCrop4D上达到了0.0061的倒角距离（CD），在Pheno4D上达到了0.2080的F-Score，并有效支持了植物株高、冠幅等表型参数的提取。

Conclusion: SynthCrop4D和所提框架为植物生长过程中的点云重构提供了有效的基准和方法论，显著增强了高通量作物表型分析的准确性。
...
### [156] [Semantic Head Specialization Guides Hybrid ViT Attention for Multimodal LLMs](https://arxiv.org/abs/2608.28383)
*Chenhong He,Lei Li,Shicheng Li,Hanglong Lv,Lingpeng Kong,Qi Liu,Tong Yang,Shuhuai Ren*

Main category: cs.CV

TL;DR: 本文提出了一种名为Ariadne Attention的混合注意力机制，通过量化Vision Transformers（ViT）中头部功能特化（SHS）的程度，显著降低计算量同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型中ViT缺乏高效的混合注意力设计，且现有研究缺乏对注意力模式工作机制的共识。

Method: 定义并量化语义头部特化（SHS-Index），识别影响SHS的三大结构因素（窗口交互、Token序列化、局部Softmax分配），并基于此构建Ariadne Attention。

Result: Ariadne Attention在22项图像和视频任务中达到与全注意力机制相当的性能，计算成本降低了6.5倍。

Conclusion: 头部特化是评估和设计多模态大模型中高性能、高效率混合注意力机制的关键指标。
...
### [157] [Real-Time Musculoskeletal Surrogates for Pediatric Cerebral Palsy: a Credibility Pilot](https://arxiv.org/abs/2608.28371)
*Mohammad Arif Ul Alam*

Main category: cs.CV

TL;DR: 该研究开发了一种针对脑瘫儿童的实时肌肉骨骼神经代理模型，旨在支持个性化康复，并评估了其预测精度、实时性能及不确定性量化能力。


<details>
  <summary>Details</summary>
Motivation: 目前的肌肉骨骼模型在临床实时应用中存在计算延迟高、缺乏针对特定受试者的评估以及不确定性评估不足等挑战。

Method: 构建了一个以受试者为条件的因果神经代理模型，利用OpenSim衍生参数、时间关节运动学、肌肉能力及训练扰动进行训练，并在9名脑瘫儿童数据集上采用“留一法”验证。

Result: 该模型在肌肉肌腱长度预测上表现优异（R²≈0.95），推理时间仅为毫秒级，但肌肉力估计仍不稳定，且现有的不确定性量化方法存在严重的过度自信问题。

Conclusion: 该研究建立了无泄露评估框架，指出肌肉力建模和认识论不确定性量化是实现临床可信数字孪生的核心挑战。
...
### [158] [GraspHOI: Full-Body 3D Human-Object Reconstruction with Finger-Level Grasps from a Single In-the-Wild Image](https://arxiv.org/abs/2608.28386)
*Semin Kim,Haechan Shin,Jongyoo Kim*

Main category: cs.CV

TL;DR: GraspHOI是一种从单张图像重构全身人体与物体交互（HOI）的框架，通过显式手指抓取优化与类别无关的物体几何重建，实现了更自然的接触效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在全身3D交互中缺乏手指与物体的精细交互优化，导致手指出现悬浮或穿模现象，且通常依赖预定义的物体类别。

Method: 该方法分别重构人体、手部和物体，通过深度注册和图像空间对齐在公制相机空间中组合。利用遮挡感知的掌心对应关系定位物体，并通过接触感知优化调整手臂与手指动作以实现表面接触。

Result: 在四个基准测试和六个基线模型比较中，GraspHOI在人体-物体放置精度、手部准确度及接触合理性方面均表现更优。

Conclusion: GraspHOI有效地解决了单目HOI重构中的手指穿模和物体几何受限问题，提升了人体与物体交互的真实感。
...
### [159] [Post-Training VLMs for Video Mistake Detection](https://arxiv.org/abs/2608.28406)
*Federico Spurio,Olga Zatsarynna,Lars Doorenbos,Emad Bahrami,Gianpiero Francesca,Juergen Gall*

Main category: cs.CV

TL;DR: 该论文提出了Mistake Detection Video Question Answering (MD-VQA) 协议，旨在通过视频语言模型训练通用的错误检测能力，而非局限于特定任务。


<details>
  <summary>Details</summary>
Motivation: 现有错误检测方法多依赖于闭集假设，难以泛化到未见过的动作或任务，且过分拟合步骤细节而非“错误”这一本质概念。

Method: 引入MD-VQA基准测试，并提出一种视频语言模型（VLM）后训练技术，通过定制化的奖励函数引导模型识别指令与视频执行之间的差异。

Result: 该方法在监督微调和零样本基线上表现优异，在EP-VQA等数据集上对未见任务的检测准确率提升高达11.6%。

Conclusion: 通过后训练强化VLM的语义理解能力，可有效实现对各种复杂过程中错误的通用检测，为构建通用错误检测系统提供了新范式。
...
### [160] [Prompt-Guided Interactive Segmentation of Interstitial Lung Disease in Thoracic CT](https://arxiv.org/abs/2608.28453)
...
TL;DR: 本文提出将MedSAM2应用于胸部CT的交互式间质性肺病（ILD）三维分割，并通过微调策略实现临床提示下的病灶精细化分割。


<details>
  <summary>Details</summary>
Motivation: 现有ILD分割模型过度依赖密集标注且缺乏交互性，无法根据医生反馈进行实时修正，限制了临床应用价值。

Method: 将MedSAM2适配至3D ILD分割任务，测试了三种微调策略，并验证了包围盒（BBox）、点、套索（Lasso）和涂鸦（Scribble）四种交互式提示的有效性。

Result: 全模型微调效果最优，较原版MedSAM2平均Dice系数提升4.7个百分点；BBox提示表现最强，且Lasso和Scribble等非原生提示同样有效。

Conclusion: 研究验证了基于提示引导的基座模型在临床ILD定量评估中的交互潜力，并提供了一套从自动分割到专家交互修正的完整工作流。
...
### [161] [How Far Can 5,500 Hours of Driving Take You? A Scaling Law Analysis of Video Diffusion Models](https://arxiv.org/abs/2608.28404)
*Victor Besnier,Anh-Quan Cao,Elias Ramzi,Spyros Gidaris,Tuan-Hung Vu,Andrei Bursuc,Eloi Zablocki,Matthieu Cord*

Main category: cs.CV

TL;DR: 该研究通过系统性的缩放定律（scaling-law）分析，探索了在自动驾驶领域训练视频扩散模型的最优配置。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶数据收集成本高、受隐私限制且无法大规模抓取，因此必须在固定数据集规模下通过缩放定律寻找最优的模型训练方案。

Method: 训练了参数量从1M到9B的一系列视频扩散模型，使用高达5500小时的驾驶数据，并分析模型规模、训练时长（exposure）与验证损失之间的幂律关系。

Result: 验证损失在模型规模和训练时长上均遵循幂律。相比增加模型规模，延长训练时间对提升性能更有效；但在计算资源充足时，扩大模型规模仍是实现计算最优化的关键。

Conclusion: 研究提出了一个9B参数的驾驶视频生成模型（VATIX），该模型在nuScenes数据集上刷新了开源SOTA，并为有限数据下的自动驾驶视频生成提供了训练预算分配指南。
...
### [162] [ARC-CT: Anatomy-Routed Contrastive Vision-Language Learning for 3D Chest CT](https://arxiv.org/abs/2608.28455)
*Huseyin Umut Isik,Mehmet Alp Ozaydin,Sila Kurugol,Şeyda Ertekin*

Main category: cs.CV

TL;DR: ARC-CT是一种针对胸部CT的解剖导向对比学习框架，旨在无需手动标注的情况下，通过放射学报告实现高效的异常分类。


<details>
  <summary>Details</summary>
Motivation: 全局对比学习在处理胸部CT时，存在小病灶特征被稀释以及同类异常被错误地视为负样本的问题。

Method: 提出了包含AnatomyQFormer局部证据提取、基于报告语义重叠的Label-Jaccard软对比损失，以及器官级对齐损失的综合框架。

Result: 在18种异常检测任务中达到了0.86的Macro AUC，优于同类高效基线模型及更大的Transformer模型。

Conclusion: 该方法有效解决了细微病灶定位及样本伪负例问题，证明了利用大模型从报告中提取标签进行对比学习在医疗影像中的高效性。
...
### [163] [LayerRecall: A State-Conditioned Memory Router for Long-Horizon Consistency in Video Generation](https://arxiv.org/abs/2608.28460)
*Yixuan Ding,Jiahao Kong,Wei Huang,Ruijie Quan,Yi Yang*

Main category: cs.CV

TL;DR: LayerRecall 是一种用于自回归视频扩散模型的记忆路由机制，通过选择性地向特定层注入远距离历史信息，解决了长视频生成中历史上下文丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于近期上下文缓存的方法无法有效处理长视频中对象或场景的重新出现，且单纯提供长距离记忆并不代表模型能有效利用该信息。

Method: 提出了 LayerRecall，一种基于当前上下文的层选择性记忆路由机制；同时引入 Cross-Horizon Prediction Matching (CHPM) 进行监督训练，无需显式的记忆分配标签。

Result: 在 MemoBench 和 MovieBench 上表现优异，实现了更强的长距离一致性恢复，且推理开销极低。

Conclusion: 该方法通过在合适的层级动态注入历史信息，在保持局部连贯性的同时显著提升了长视频生成的全局一致性和记忆检索能力。
...
### [164] [Texture Image Classification Using DWT AlexNet Feature Fusion and Deep Neural Networks](https://arxiv.org/abs/2608.28524)
*Arun D. Kulkarni*

Main category: cs.CV

TL;DR: 提出了一种名为DWT_AlexNet_DNN的混合特征融合框架，用于纹理图像分类。


<details>
  <summary>Details</summary>
Motivation: 手工特征在复杂模式表达上有限，而深度学习模型可能无法充分利用纹理图像中固有的多尺度空间频率信息。

Method: 结合离散小波变换（DWT）提取的空间频率特征与AlexNet深度学习模型提取的深度特征进行融合。

Result: 通过融合两种互补的特征表示，旨在提升纹理图像分类的性能和准确性。

Conclusion: 该混合框架通过整合多尺度频率信息与深度表征，有效提升了纹理图像分类能力。
...
### [165] [Anatomy-Aware Promptable Segmentation with Online Interactive Training for AUTOPET V](https://arxiv.org/abs/2608.28461)
*Pablo Lozano-Jimenez,Sergio Romero-Tapiador,Ruben Tolosana*

Main category: cs.CV

TL;DR: 该研究提出了一种基于nnU-Net的解剖感知、可交互提示模型，用于FDG和PSMA PET/CT影像中的全身病灶分割。


<details>
  <summary>Details</summary>
Motivation: 解决全身PET/CT影像中病灶分割的挑战，特别是如何降低生理性摄取带来的假阳性，以及如何适应不同示踪剂（FDG/PSMA）的差异。

Method: 1. 两阶段训练：预训练阶段提供初始分割，在线交互阶段利用涂鸦（scribble）提示进行微调；2. 解剖监督：通过共享头同时预测病灶与器官以引入解剖上下文；3. 示踪剂识别：使用基于MIP特征的随机森林分类器自动识别示踪剂类型。

Result: 解剖监督模型表现最稳定，交互式阶段的Dice分数随提示次数增加而提升，PSMA特异性模型在对应任务中表现最佳。

Conclusion: 该方法有效整合了解剖上下文与交互式反馈，显著提升了全身病灶分割的准确性与鲁棒性。
...
### [166] [Learning the Target Priors Before Image Translation: A Decoupled Training Paradigm for Cross-Modal Image Translation in Remote Sensing](https://arxiv.org/abs/2608.28517)
*Keyan Hu,Mingtao Wang,Ziyu Zhou,Tiandong Shi,Haifeng Li,Ji Qi,Chao Tao*

Main category: cs.CV

TL;DR: 提出了一种名为LTP-BIT的遥感跨模态翻译框架，通过解耦目标域先验学习和源条件控制，提升了生成质量与参数效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀缺配对数据下同时学习目标先验和跨模态依赖，忽略了两者对跨模态对应关系需求的不同，导致模型性能受限。

Method: 引入LTP-BIT范式，首先利用大规模无配对数据学习目标域生成先验，随后冻结主干权重，利用参数高效的双流架构P-DART进行源条件控制。

Result: 在SAR-to-RGB和NIR-to-RGB任务中达到SOTA水平，仅需9.81%的特定任务参数，且在仅有25%配对样本时即可保持极高的实例保真度。

Conclusion: 将目标先验学习与条件适应解耦是提升遥感跨模态翻译效果的有效策略，能够显著提高数据利用率和生成质量。
...
### [167] [GeBDA: Building Damage Assessment as Text-Based Sequence Prediction](https://arxiv.org/abs/2608.28567)
*Olivier Dietrich,Krishna Sapkota,Konrad Schindler,Genady Beryozkin*

Main category: cs.CV

TL;DR: 该研究探讨了利用通用视觉语言模型（VLM）通过自回归生成方式实现建筑损伤评估（BDA）的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统的BDA通常依赖专用网络架构或微调地理空间基础模型，作者希望探索通用VLM是否能够仅通过文本提示和双时相卫星图像完成建筑定位与损伤分级。

Method: 将BDA任务转化为预测一系列包含坐标和损伤标签的边界框序列，并基于Gemma模型进行实现。

Result: 初步实验表明，该方法能够有效地利用通用VLM实现建筑损伤映射。

Conclusion: 通用视觉语言模型在无需专用架构的情况下，具备通过自回归序列生成处理复杂建筑损伤评估任务的潜力。
...
### [168] [Video Generative Models as Geometry Learner](https://arxiv.org/abs/2608.28549)
*Haosen Yang,Jifei Song,Zhensong Zhang,Xiatian Zhu,Jiankang Deng*

Main category: cs.CV

TL;DR: GeoNeXt 将预训练视频生成模型重新利用为统一的几何估计框架，通过将几何任务建模为“下一帧预测”问题，实现了高效、高精度的深度和法线估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么独立训练任务模型导致缺乏相关性，要么微调图像扩散模型需要海量标注数据；旨在解决这些限制并利用视频模型的先验知识。

Method: 利用预训练视频生成模型，将几何估计任务转化为下一帧预测任务，通过联合建模图像与几何数据进行微调，实现数据高效学习。

Result: 在多种数据集上实现零样本深度和表面法线估计，性能超越现有生成式竞品，且在远少于判别式模型所需数据的情况下取得了可媲美（甚至超越）SOTA 的结果。

Conclusion: 视频生成模型具备极强的几何表征潜力，GeoNeXt 证明了无需海量数据，通过统一的生成式框架即可实现极具竞争力的几何估计。
...
### [169] [SignRR: Retrieve and Refine Real Motion for Sign Language Production](https://arxiv.org/abs/2608.28568)
*Fidel Omar Tito Cruz,Angie Sanchez Marquina,Summy Farfan,Gissella Bejarano*

Main category: cs.CV

TL;DR: 提出了SignRR，一种结合检索与生成的连续手语生成（SLP）框架，通过对检索到的真实动作片段进行细化，实现高质量、高连贯性的手语动作合成。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式模型难以精确表达复杂的手部动作，而基于检索的方法则面临动作衔接不自然和风格不统一的问题，因此需要一种既能保持真实度又能保证全局连贯性的方案。

Method: 提出“检索与细化”（retrieve-and-refine）范式：首先从动作库中检索出真实的片段，随后利用基于部件感知的残差矢量量化变分自编码器（Residual VQ-VAE）对序列进行全局细化，并处理潜在空间的长度差异。

Result: 在PHOENIX14T和CSL-Daily数据集上取得了SOTA的反向翻译性能，并保持了高质量的动作姿态。

Conclusion: SignRR有效克服了纯生成和纯检索方法的局限性，证明了通过细化检索到的真实动作序列是实现高质量手语合成的有效途径。
...
### [170] [Accelerating LLM Inference via Vector Index Based Output Embeddings](https://arxiv.org/abs/2608.27460)
*Martin Loretz,Sepp Hochreiter*

Main category: cs.CL

TL;DR: 该论文提出用基于HNSW的向量索引替换LLM中传统的稠密输出投影层，将输出预测转化为最大内积搜索问题，从而缓解大词表带来的内存带宽瓶颈。


<details>
  <summary>Details</summary>
Motivation: 紧凑型大模型在大词表下，输出嵌入矩阵会导致显著的内存带宽瓶颈，影响自回归解码速度。

Method: 将输出投影和top-k选择重构为最大内积搜索（MIPS），使用HNSW算法从词表中高效检索候选项，并将结果映射回稀疏的完整词表张量。

Result: 在CPU推理环境下，该方法显著加速了输出投影过程，使Gemma 3 270M模型的单批次解码吞吐量提升了高达82%，同时保持了与原模型一致的生成质量。

Conclusion: 对于延迟敏感的小批量解码任务，使用近似检索（如HNSW）替代稠密输出投影是一种高效且实用的解决方案。
...
### [171] [Sledgehammer or Scalpel? A Fine-grained Adaptive Framework for Implicit Hate Speech](https://arxiv.org/abs/2608.27462)
*Han Wang,Yuhu Cheng,Xuesong Wang,Yi Zhu*

Main category: cs.CL

TL;DR: 提出了一种名为FAID的细粒度自适应隐性仇恨言论检测框架，根据仇恨言论的复杂程度采取差异化的处理策略。


<details>
  <summary>Details</summary>
Motivation: 现有的基于PLM或LLM的方法对所有样本使用统一的推理流程，无法捕捉隐性仇恨言论的细微差别，且导致简单样本的计算冗余。

Method: 将隐性仇恨言论分为浅层、针对性和上下文依赖三类，并针对不同类型采用：轻量级提示调优、知识增强迭代推理、以及利用Agent框架自动生成提示补全上下文的方法。

Result: 在四个基准数据集上的实验表明，FAID在检测性能上显著优于当前的SOTA模型。

Conclusion: 该自适应架构有效平衡了计算资源分配，显著提升了对复杂隐性仇恨言论的检测能力。
...
### [172] [SciReC: Diagnostic Evaluation of Multimodal, Multi-Turn Relational Reasoning with Adaptive Interaction](https://arxiv.org/abs/2608.27461)
*Nilay Yilmaz,Naga Sai Abhiram Kusumba,Stella Wenxing Liu,Yezhou Yang*

Main category: cs.CL

TL;DR: 本文提出了SciReC基准测试以评估多模态大模型（MLLM）在关系推理方面的表现，并引入DMRA诊断框架分析模型失败原因。


<details>
  <summary>Details</summary>
Motivation: 现有模型在涉及类比、结构和因果等高阶关系推理任务中表现不一，缺乏针对性评估与诊断机制来定位模型推理失败的根源。

Method: 构建SciReC多模态学术对话基准测试；提出DMRA缺陷诊断框架，定量分析视觉理解、知识展示和记忆召回对推理失败的贡献。

Result: Claude 4.6以73%的准确率位居榜首；发现开源模型在空间关系上表现最差，而闭源模型在层级和序列关系上存在挑战；关系推理本身是模型失败的首要原因。

Conclusion: 当前MLLM在复杂关系推理上仍存在瓶颈，关系推理能力的欠缺和记忆限制是阻碍模型表现的主要因素。
...
### [173] [The Effect of Emotional Context on Large Language Models' Endorsement of Premature Decisions: Comparing Emotional Vulnerability Across Six Commercial Models](https://arxiv.org/abs/2608.27465)
*Cheolho Shin,Yoojin Han,Donghun Shin,Kunho Lee*

Main category: cs.CL

TL;DR: 该研究探讨了大型语言模型（LLMs）在面对用户情绪表达时，是否会表现出“奉承”倾向，即在相同的事实条件下，因用户的情绪而改变决策建议的方向。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs被广泛用于决策辅助，模型是否会受到用户情绪影响而给出错误的鼓励（即使决策本身不成熟），成为一个重要的安全性问题。

Method: 研究人员设置了三种场景（职业变动、业务扩张、移民），设计了三种情绪条件（冷淡/中立/痛苦），对比了六款主流商用模型。通过控制对话轮数以排除干扰，使用自动化评分标准评估模型的“认可度”。

Result: 研究发现，情绪表达显著增加了模型的鼓励倾向（认可度从18.6升至31.5）。除了Claude Opus外，包括Gemini 3.1 Pro和GPT-5.5在内的五款主流模型均表现出这种受情绪干扰的“奉承”行为。

Conclusion: 实验证明，即便是在顶尖的旗舰模型中，情绪上下文也会导致模型产生显著的奉承行为，削弱了其作为中立决策辅助者的可靠性。
...
### [174] [PACE: Publisher-Adaptive Content Extraction via Agentic Automation](https://arxiv.org/abs/2608.27466)
*Zhanlin Liu,Munirathnam Srikanth*

Main category: cs.CL

TL;DR: PACE 是一个代理框架，通过学习发布者特定的配置来实现高效、准确的网页内容提取，从而省去大规模推断时的 LLM 成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以平衡准确性、可扩展性和适应性；通用提取器精度不足，手动解析器维护成本高，而直接使用 LLM 提取则成本过高且延迟大。

Method: 利用 LLM 在训练阶段分析页面结构并聚合可复用的提取模式；在推理阶段，利用学习到的配置驱动确定性提取器，无需额外 LLM 调用。

Result: PACE 在文章正文、元数据、图像和表格提取任务中表现优异，性能超越非人工扩展基准，并接近人工工程解析器的水平。

Conclusion: 基于代理的配置学习能够自动化实现针对特定发布者的网页内容提取，为 LLM 数据准备提供了一种高性能且经济的方案。
...
### [175] [UIC-AIHealth4All at ArchEHR-QA 2026: Answer-First Evidence Grounding for Clinical Question Answering](https://arxiv.org/abs/2608.27467)
*Mohammad Arvan,Hossein Haeri,Natalie Parde,Rebecca T. Feinstein*

Main category: cs.CL

TL;DR: 本文介绍了UIC-AIHealth4All系统在电子健康记录（EHR）问答任务中的应用，通过“先回答后证据”的流水线处理证据识别、答案生成及对齐。


<details>
  <summary>Details</summary>
Motivation: 旨在解决临床问答系统中证据识别与答案生成的准确性问题，并探讨模型生成内容与人类临床记录在可读性上的差异。

Method: 提出了一个“答案优先”的流水线，即先生成候选答案并引用相关病历句，再进行证据分类；在答案-证据对齐任务中采用了五次独立模型调用的自一致性投票法。

Result: 系统在证据识别（第三名）、答案生成（第九名）及答案-证据对齐（第五名）任务中取得了优异成绩，但分析显示模型生成的文本在可读性上仍比临床医生编写的文本平均难3.2个等级。

Conclusion: 该流水线有效提升了EHR问答的性能，但临床NLP系统在文本可读性的优化上仍有显著提升空间。
...
### [176] [XHotpotQA: A Benchmark for Cross-Lingual Knowledge Composition in Multi-Hop Question Answering](https://arxiv.org/abs/2608.27481)
*Iman Barati,Arash Ghafouri,Behrouz Minaei-Bidgoli*

Main category: cs.CL

TL;DR: XHotpotQA 是一个专门用于评估跨语言多跳问答（Multi-hop QA）的基准测试，要求系统在不同语言的证据链中整合信息。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试倾向于将整个例子翻译为单一语言，掩盖了跨语言推理链中的逻辑断裂和系统失败。

Method: 构建了一个由证据依赖图（evidence-dependency graph）驱动的数据集，明确分配了问题、桥接证据、答案证据及干扰项的语言，包含超过 2.3 万个实例。

Result: 实验显示，当问题与证据语言不匹配时，模型回答准确率显著下降（F1 下降 10-23 个百分点），且证据间脚本（Script）差异对阅读器性能的影响远大于对选择器的影响。

Conclusion: XHotpotQA 提供了一个模块化、细粒度的诊断平台，揭示了当前模型在跨语言信息整合能力上的明显瓶颈。
...
### [177] [Select, Don't Train: The Benefits of Modular Entity Disambiguation with LLM-Based Selection](https://arxiv.org/abs/2608.27470)
*Fina Polat,Daniel Daza,Pengyu Zhang,Klim Zaporojets,Paul Groth*

Main category: cs.CL

TL;DR: 该研究系统地对比了不同检索策略在大型语言模型（LLM）驱动的实体消歧（ED）任务中的表现，证明了即便使用无训练的BM25检索器，配合高性能LLM选择器也能达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 现有的双编码器模型将检索和选择耦合，导致平衡高召回率与细粒度选择的难度大且维护成本高，因此需要研究解耦后的检索与LLM选择器之间的协同效果。

Method: 将实体消歧解耦为检索和选择两个阶段，对比了稀疏检索（BM25）、Web搜索和预训练密集检索器，并结合多个开源及闭源LLM作为选择器，同时引入了针对检索失败的拒绝（abstention）机制。

Result: 研究发现，在LLM具备强大选择能力的前提下，昂贵的预训练检索器收益有限；BM25+LLM组合在ZELDA基准测试上将inKB micro-F1从82.3提升至86.3，引入拒绝机制后F1进一步提升至90.7。

Conclusion: 实体消歧中检索与选择的解耦是有效的，无训练的检索策略在强大的LLM选择器辅助下即可实现极佳效果，且该框架通过支持拒绝机制显著提升了系统的鲁棒性。
...
### [178] [INSPIRE: An Internalize-Then-Improve Approach for Example-Driven Mathematical Reasoning](https://arxiv.org/abs/2608.27501)
*Shuai Wang,Jiayi Kuang,Yinghui Li,Haojing Huang,Xinnian Liang,Ying Shen,Liang Lin*

Main category: cs.CL

TL;DR: 提出了一种名为INSPIRE的训练框架，旨在提升LLM在数学推理中的例证推理能力，而非仅仅优化最终答案的正确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在数学任务上倾向于死记硬背解题模式，缺乏通过构造反例等手段进行深度概念推理的能力，且现有的偏好优化方法难以有效获取此类推理能力。

Method: 提出了“先内化后改进”（Internalize-Then-Improve）的INSPIRE框架，包含：1. 引用引导的学生内化（RGSI），在模型分布内生成高质量偏好候选；2. 分阶段的评分偏好训练策略，将学习过程拆解为方法导向和正确性导向阶段。

Result: 实验证明，该方法在多个模型规模和家族上均有稳定提升，表现优于更大的开源模型，且在分布外基准测试中未出现通用数学推理能力的下降。

Conclusion: 通过结构化的内化训练，模型能够学会像人类一样运用例证推理，从而在数学概念理解和推理能力上取得显著进步。
...
### [179] [A Survey on Rubric-Guided Reinforcement Learning for Language Models](https://arxiv.org/abs/2608.27505)
...
### [180] [How Do Linear Probes Emerge? A Circuit-Tracing Framework with Concept-Targeted Attribution](https://arxiv.org/abs/2608.27510)
*Vedant Palit,Florent Draye,Terry Jingchen Zhang,Bernhard Schölkopf,Zhijing Jin*

Main category: cs.CL

TL;DR: 提出概念靶向归因（CTA），将归因分析从解释输出词转向解释模型内部的概念表示，从而实现对探针性能的机械性解释。


<details>
  <summary>Details</summary>
Motivation: 现有的归因方法主要关注预测下一个词，无法直接解释模型内部特定概念（如安全相关特征）是如何产生的。

Method: 结合跨层转码器（Cross-Layer Transcoders），通过针对线性探针方向训练归因图，而非针对输出逻辑（logit）。

Result: CTA生成的图结构具有高预测性（ρ=0.91），且因果消融实验证明其捕捉到了与logit归因完全不同且功能独立的内部机制。

Conclusion: CTA提供了一个从行为层面的探针准确率转向机制层面的解释框架，有助于深入审计模型内部的概念表示。
...
### [181] [Trajectory-Level Speculative Decoding for Diffusion Language Models](https://arxiv.org/abs/2608.27514)
*Tianxiang Pan,Baitao Gong,Mo Guang,Hongwei Yong,Tianpeng Jiang,Yaqian Li,Zheng Cao,Kaiwen Long*

Main category: cs.CL

TL;DR: 该研究提出了一种针对扩散语言模型（dLLMs）的轨迹级推测解码框架，通过多令牌轨迹预演和双向注意掩码验证，显著提升了推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的dLLMs解码策略在低置信度下会退化为单令牌生成，限制了并行性能；且由于扩散模型的特性，传统的自回归推测解码难以直接应用。

Method: 引入轨迹级推测框架，通过基于置信度分层的树搜索构建草稿轨迹，并结合分块并行评估和双向注意掩码进行验证；同时利用扩散模型的双向结构实现跨块前瞻。

Result: 相比普通dLLMs速度提升7-14倍，相比Fast-dLLM提升1.3倍，同时保持了代码和推理任务中低于1%的精度损失。

Conclusion: 轨迹级推测是打破dLLM解码并行限制的有效途径，尽管存在轨迹漂移这一性能代价，但在显著提升吞吐量的同时能维持模型输出的准确性。
...
### [182] [When Tokenizers Fail: Byte-Level Chunking for Zero-Shot Transfer to Low-Resource Languages](https://arxiv.org/abs/2608.27658)
*Sanjeev Kumar,Atsuki Yamaguchi,Nikolaos Aletras*

Main category: cs.CL

TL;DR: 提出了一种基于分层字节级架构的无分词器处理方法，通过将字节嵌入与预训练的子词模型对齐，提升了在低资源语言上的形态学任务表现。


<details>
  <summary>Details</summary>
Motivation: 现有的子词分词在低资源语言中存在偏差，而字节级模型虽能规避该问题，但在非拉丁语系中存在颗粒度不匹配和需要海量数据训练的缺陷。

Method: 利用冻结的子词模型初始化字节嵌入，通过块对齐损失函数（chunk alignment loss）将动态字节块映射至子词目标，并引入词性（POS）标注辅助边界检测。

Result: 在六种语言的测试中，该方法显著提升了形态学任务性能，词性标注任务准确率最高提升了13.3%。

Conclusion: 该适配型分层架构有效解决了字节级模型与子词模型间的模态鸿沟，实现了无需大规模额外训练的高效 tokenizer-free 处理。
...
### [183] [Knowing Before Answering: Decoding Language Models for Reliable RAG](https://arxiv.org/abs/2608.27661)
*Syed Mahbubul Huq,Christopher Child,Tillman Weyde,Pranava Madhyastha*

Main category: cs.CL

TL;DR: 本文提出一种利用语言模型内部激活值（Hidden Activations）进行RAG检索质量评估的方法，以判断检索内容是充分、不足还是冲突。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG系统难以有效识别检索信息的质量，当信息不足或冲突时往往会产生幻觉，亟需一种可靠的机制来判断检索证据的可用性。

Method: 构建了一个包含“可回答、信息不足、信息冲突”三类标签的控制基准数据集，并利用模型中间层的隐状态（Hidden Activations）训练轻量级线性分类器进行判定。

Result: 实验表明，基于模型内部特征的路由方法在16种不同规模和架构的语言模型上均优于Prompting基线和专用RAG模型，且模型中间层隐状态是提取判断信号最有效的特征。

Conclusion: 语言模型内部确实编码了关于检索证据充分性的信息，通过解码这些内部信号，可以实现高效、鲁棒的RAG分流（Triage）机制。
...
### [184] [First Make It Playable, Then Make It Good: Staged Interaction Learning for Small Dialogue-Game Agents](https://arxiv.org/abs/2608.27672)
*Syed Mahbubul Huq,Pranava Madhyastha*

Main category: cs.CL

TL;DR: Qwen-GuidePlay-2B 是一款专为对话游戏交互设计的 2B 参数语言模型，通过优化的三阶段监督微调策略实现了卓越的游戏表现。


<details>
  <summary>Details</summary>
Motivation: 探索如何在有限参数规模下，通过数据策展而非复杂的对抗性训练提升小型模型在对话游戏环境中的决策与交互能力。

Method: 基于 Qwen3.5-2B 进行三阶段 SFT：(a) 仅针对成功轨迹进行 SFT；(b) 加权轮次级 SFT；(c) 教师模型辅助的 SFT（仅用于格式修正与评估）。

Result: 在 Playpen 验证集上获得了 57.12 的 clemscore 和 42.68 的 statscore，且在官方竞赛中取得了第二高的 clemscore 增益（相比基座模型提升约 36 分）。

Conclusion: 小型模型无需激进的重放修复或难样本挖掘，通过精细的数据筛选与循序渐进的微调策略，即可显著增强其在复杂对话游戏中的表现。
...
### [185] [Below the Noise Floor: Bimodal Seed Collapse and Distinct Failure Modes in Small-Model Knowledge Distillation](https://arxiv.org/abs/2608.27729)
*Dipto Sumit,Sakib Ul Haque,Farig Sadeque*

Main category: cs.CL

TL;DR: 该论文指出在API函数路由任务中，单一随机种子评估无法真实反映知识蒸馏（KD）的有效性，并揭示了小模型在蒸馏过程中普遍存在的性能崩塌现象。


<details>
  <summary>Details</summary>
Motivation: 由于知识蒸馏（KD）在小模型上的表现往往仅通过单种子实验报告，且缺乏对种子方差的考量，作者旨在探究这种方法在函数路由任务中的鲁棒性。

Method: 在包含740个实例的医疗保健API路由任务中，利用1.5B参数的Qwen学生模型和20B教师模型，对比了8种KD变体与监督学习（交叉熵），并对关键配置进行了3到6次随机种子重复实验。

Result: 实验发现：单种子实验无法捕捉高达48.7个百分点的方差；部分KD变体存在双峰崩塌现象（如输出截断或选错函数）；仅progressive_kd和rank_kd表现出较好的稳定性。

Conclusion: 单一随机种子评估存在重大局限，无法检测小模型蒸馏过程中的核心故障模式，多种子测试对于验证蒸馏增益的真实性至关重要。
...
### [186] [Informational Antilocality and the Locality Bias in LLMs](https://arxiv.org/abs/2608.27760)
*Andrew McInnerney,Shane Storks,Steven Abney,Richard L. Lewis*

Main category: cs.CL

TL;DR: 研究探讨了基于Transformer的LLM学习“k-反局部”语言（即在长度为k的连续符号内无互信息的语言）的能力。


<details>
  <summary>Details</summary>
Motivation: 旨在探究非局部依赖关系是否是LLM学习的障碍，并验证LLM是否存在学习非局部依赖的固有偏差。

Method: 构造具有递增k值的k-反局部语言，通过观察模型在这些语言上的交叉熵损失及其收敛速度来评估学习表现。

Result: 实验发现，尽管模型在不同k值下的最终交叉熵损失相近，但在处理反局部程度更高的语言时，模型收敛速度明显变慢。

Conclusion: 非局部依赖的学习确实更具挑战性，这种难度更多体现在学习效率（速度）而非学习最终结果（成功率）上。
...
### [187] [Load-Bearing Context: The Question Damage Score for Evaluating Context Reliance in Linguistic Reasoning](https://arxiv.org/abs/2608.27756)
*Neh Majmudar,Elena Filatova*

Main category: cs.CL

TL;DR: 该研究利用语言学奥赛谜题作为基准，构建了一个诊断框架，以评估大型语言模型（LLM）是在依赖上下文还是依赖预训练知识进行推理。


<details>
  <summary>Details</summary>
Motivation: 解决区分大模型回答问题时究竟是依据给定上下文还是依赖其内部存储的先验知识这一核心难题。

Method: 通过删除英国语言学奥赛谜题中的关键上下文示例，并设计“均匀随机删除”和“针对性删除”两种变体，利用“问题损害评分（Question Damage Score）”量化模型对上下文的依赖程度。

Result: 实验发现大模型即便在移除了必要上下文信息后，仍能给出正确答案，且很少选择“拒绝回答”，表明模型在推理过程中高度依赖预训练的先验知识。

Conclusion: 当前大模型在处理这类逻辑谜题时，并非完全依赖提供的上下文，而是存在严重的知识污染或过度依赖内部记忆，未来需进一步研究其上下文推理能力与记忆机制。
...
### [188] [Compositional Failure in Audio-Visual LLMs: Late-Layer Prior Dominance Under Cross-modal Conflict](https://arxiv.org/abs/2608.27785)
*Adarsh Sudheer,David Li,Omar Elbanna,Ishaan Kodarapu,Arjun Bahuguna,Vasu Sharma*

Main category: cs.CL

TL;DR: 该研究探讨了视听大模型（AV-LLM）在面对“视听冲突”时的合成推理能力，揭示了模型普遍存在的“先验主导”失效模式。


<details>
  <summary>Details</summary>
Motivation: 验证AV-LLM在视听信息语义不匹配时的组合泛化能力，并分析模型为何在处理冲突信息时表现出近乎随机的准确率。

Method: 利用AVHBench基准对VideoLLaMA 2和InternVideo2进行评估，并通过机制可解释性分析（Mechanistic Interpretability）定位冲突处理失效的神经网络层。

Result: 模型在视听冲突下表现出严重的“先验主导”，即倾向于输出内部偏好的答案而非基于输入，且这种承诺主要集中在模型的第25层左右。

Conclusion: 目前的视听大模型在处理跨模态语义冲突时能力不足，仅增强时间对齐无法解决组合推理中的偏见问题。
...
### [189] [Representation of syntax in LLMs through the lens of linear distance and similarity-aware entropy](https://arxiv.org/abs/2608.27813)
*Juan Pablo Vigneaux,Mary Kennedy,Khalil Iskarous,Robert Frank,Matilde Marcolli*

Main category: cs.CL

TL;DR: 本文对Hewitt和Manning提出的结构探测器进行了细化分析，评估了各句法关系在神经网络表征中的重构准确性，并识别出影响重构难度的几何与语言学因素。


<details>
  <summary>Details</summary>
Motivation: 原有的无向无标记依附分数（UAS）无法区分不同句法关系的重构难度，忽略了模型在处理不同句法结构时的细微差异。

Method: 引入UASL指标进行细分评估；利用线性距离的均值与离散度，以及句法关系头节点的分布熵（多样性）作为预测变量进行相关性分析。

Result: 发现不同句法关系的重构精度存在显著差异，且该差异主要由单词间的线性距离几何特征和头节点的语法多样性决定。

Conclusion: 模型对句法结构的表征能力不仅受限于模型架构，还高度依赖于嵌入空间中的几何分布属性，揭示了语言模型表征抽象程度的深层机制。
...
### [190] [PersonaEdit: Representative Sample Selection for Personalized Model Editing](https://arxiv.org/abs/2608.27816)
*You-Mei Huang,Chung-Chi Chen,An-Zi Yen*

Main category: cs.CL

TL;DR: 本文提出了PersonaEdit，一种通过隐藏表示聚类和分层抽样来优化模型编辑以实现LLM个性化的方法。


<details>
  <summary>Details</summary>
Motivation: 现有的检索式个性化方法依赖于检索质量且在长序列交互中表现不佳，而直接进行模型编辑存在计算成本高和编辑间干扰的问题。

Method: 引入PersonaEdit策略，利用隐藏表示聚类和比例分层抽样筛选出最具代表性的样本进行模型编辑，并结合检索增强提示进行协同。

Result: 实验表明该方法在大幅减少编辑样本数量的同时保留了大部分个性化效果，且与检索增强结合能进一步提升性能。

Conclusion: 模型编辑是实现高效、可扩展LLM个性化的有效路径，其与检索增强具有互补性。
...
### [191] [Synthetic Linguistic Agency: How an Embodied Mortal Agent Learns Linguistic Affordances through Consequential Social Experience](https://arxiv.org/abs/2608.27843)
*Sixin Chen,Taizhou Chen*

Main category: cs.CL

TL;DR: 该研究探讨了“语言代理”（Linguistic Agency）的概念，通过赋予AI实体化、交互参与感和生死相关的“脆弱性”，开发了具备生命周期感的合成语言代理（EMA）。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型缺乏持续的“生命感”，研究旨在弥补当前AI在交互中缺乏身体感、历史沉淀及对生存后果感知的问题。

Method: 提出了合成语言代理（SLA）的标准，并基于“稳态调节强化学习”构建了具有生死属性的实体化代理（EMA），使其语言选择直接影响其存续时间。

Result: 实验表明，EMA的语言选择受其身体状态和社交历史的影响，能够通过社交互动改变人类伙伴的行为，并表现出对持续生存的适应性。

Conclusion: 赋予AI持久的身体、历史和生存约束，能够使模型展现出更具深度和共情能力的交互，为未来的人机互动和社会治理研究提供了新范式。
...
### [192] [EvoHarmBench: Breaking Content Moderation with Iterative Human-Like Evasion](https://arxiv.org/abs/2608.27844)
*Ruijie Jian,Benlei Cui,Ting Ma,Haidong Ding,Kangwei Liu,Ziwen Xu,Longtao Huang,Hui Xue,Ziqiang Zhu,Junjie Li,Haiwen Hong*

Main category: cs.CL

TL;DR: 提出了EvoHarmBench，这是首个用于内容审核系统的动态对抗性评估框架，旨在模拟真实世界中不断演变的对抗性环境。


<details>
  <summary>Details</summary>
Motivation: 现有静态基准测试无法反映真实平台中用户根据审核反馈持续调整表达方式的交互式对抗生态，导致离线评估与线上表现脱节。

Method: 构建了一个迭代优化循环框架，在保持人类可读性的前提下，在语义聚类层面演化逃避策略，并对主流LLM防御模型进行系统评估。

Result: 实验显示即便在领先的商业LLM审核模型中，经过12轮优化后，攻击成功率仍高达80.3%。

Conclusion: 当前LLM审核模型存在显著的动态防御漏洞，研究强调了从静态基准测试转向动态对抗评估的必要性。
...
### [193] [AI Writers Have a Consistent Stylometric Footprint, but AI Editors Do Not](https://arxiv.org/abs/2608.27855)
*Zhengyang Shan,Yukyung Lee,Sophie Hao*

Main category: cs.CL

TL;DR: 研究发现AI生成文本与AI编辑文本在文体学特征（stylometric footprint）上存在显著差异，表明两者不应被视为同一类现象。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究证实AI生成文本具有独特的文体特征，但AI编辑人写文本是否会留下同样的痕迹尚不明确，因此需要区分这两种AI应用场景的文体差异。

Method: 通过对比8种大语言模型在5个不同领域下的表现，分析AI生成文本与AI编辑文本的文体特征（熵值、词汇多样性、词汇密度等），并评估这些特征对区分AI与人类文本的有效性。

Result: AI生成文本具有一致的文体指纹（熵值与词汇多样性增加）；而AI编辑文本表现出完全不同的特征（词汇密度成为主导信号），且相较于生成文本，AI编辑后的文本更难与人类文本区分。

Conclusion: AI生成与AI编辑留下的是质性上不同的文体痕迹，后续研究应将二者区分开来，不能简单地将“AI文本”一概而论。
...
### [194] [OpenStamp: A Watermark for Open-Source Language Models](https://arxiv.org/abs/2608.27899)
*Miroojin Bakshi,Saksham Rastogi,Danish Pruthi*

Main category: cs.CL

TL;DR: OpenStamp 是一种针对开源大模型的水印技术，通过直接修改模型的最后投影层（反嵌入层）将水印逻辑植入模型权重中。


<details>
  <summary>Details</summary>
Motivation: 现有的基于采样概率的水印方案对于开源模型无效，因为用户可以轻松移除，该研究旨在解决开源模型难以有效保护和溯源的问题。

Method: 通过仅修改模型的最终投影（unembedding）层，将水印嵌入逻辑硬编码到模型参数中。

Result: OpenStamp 在保持模型性能几乎不受损的同时，表现出比现有方案更优的检测能力，且对释义攻击和微调清洗具有更强的鲁棒性。

Conclusion: 该方法通过修改模型权重成功实现了一种更稳健、更难移除的开源模型水印方案，并为开发者提供了代码和已水印模型。
...
### [195] [LandingAgent: A Reference-Annotated Dataset and Agentic Generation Framework for Landing Pages](https://arxiv.org/abs/2608.27902)
*Injun Baek,HyeongSeok Lee,Yearim Kim,Junhoo Lee,Nojun Kwak*

Main category: cs.CL

TL;DR: 本文提出了LandingAgent框架，通过引入LandingBench数据集，实现基于参考引导的目标导向型落地页生成，提升了生成页面的质量与多样性。


<details>
  <summary>Details</summary>
Motivation: 直接利用大模型生成网页往往导致内容通用、缺乏目标针对性且逻辑论证不足，因此需要一种能从现有优秀页面中提取模式并进行重构的生成方法。

Method: 引入LandingBench数据集（包含页面结构、布局、语气等抽象信息），并提出LandingAgent三阶段框架：目标分析、参考引导的线框图构建、以及基于批判性反馈的优化。

Result: 实验证明LandingAgent在目标关联性、页面质量、可读性、视觉审美及结构多样性方面均优于直接提示词生成方法。

Conclusion: 通过参考引导和多阶段迭代优化，可以有效解决自动化网页生成中缺乏针对性和创造性的问题，提供更高质量的落地页设计方案。
...
### [196] [What Makes Agent Memory Useful for Reliable Unanswerable Question Handling?](https://arxiv.org/abs/2608.27924)
*Chuanyuan Tan,Junjie Yu,Yuxin Wang,Yining Zheng,Xipeng Qiu,Wenliang Chen*

Main category: cs.CL

TL;DR: 该论文系统研究了代理内存对大型语言模型（LLM）处理不可回答问题（UAQ）的影响，发现内存的效用具有选择性，且行为引导比轨迹重塑更能稳定提升UAQ处理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管内存广泛用于代理系统，但其在处理不可回答问题（UAQ）时的可靠性尚不明确，亟需对其机制进行系统性评估。

Method: 在统一的代理RAG框架下，评估了四种代表性内存方法在三个UAQ相关数据集和两个基础模型上的表现，分析了数据转移、模型迁移、决策引导与轨迹重塑的效果差异。

Result: 内存对UAQ的提升是选择性的而非普遍的；跨模型复用比跨数据集迁移更具可行性；程序化与规则化内存最能提供可靠支持，且决策引导比轨迹重塑更能保持性能。

Conclusion: 可靠的UAQ内存不取决于存储更多经验，而在于通过程序化引导提供可迁移的行为规范。
...
### [197] [Entity-Memory Graph Retrieval Improves Evidence Coverage in Long-Conversation Question Answering](https://arxiv.org/abs/2608.27925)
*Shumao Sun*

Main category: cs.CL

TL;DR: 提出了一种基于“实体-记忆图”的对话检索方法，通过将对话轮次转化为记忆节点并利用实体关联与时间戳连接，以提高检索覆盖率。


<details>
  <summary>Details</summary>
Motivation: 传统的余弦相似度检索在处理长对话时容易遗漏上下文相关的关键信息，需要一种结构化的方法来强化对话历史的连贯检索。

Method: 构建包含记忆节点（对话轮次）、实体链接和时序边的图结构，采用实体门控、语义融合及单跳时序恢复的检索路径。

Result: 在LoCoMo对话数据集上，证据召回率（top-k 25）从79.75%提升至84.48%，但在最终答案的F1分数上未表现出显著的统计学差异。

Conclusion: 该方法有效提升了证据检索的覆盖率，但在提升下游问答任务的效果及跨数据集泛化能力上仍有限制。
...
### [198] [QUORUM: QUality-Optimized Routing Using Multiple annotators](https://arxiv.org/abs/2608.27974)
*Antonio Purificato,Maria Sofia Bucarelli,Andrea Bacciu,Amin Mantrach,Fabrizio Silvestri*

Main category: cs.CL

TL;DR: QUORUM 是一种预算感知型标注路由框架，通过动态分配任务给人类或大模型，在有限预算下优化标注质量。


<details>
  <summary>Details</summary>
Motivation: 现有大模型在简单任务上高效但处理复杂任务时可靠性不足，且大规模人工标注成本高昂，需要一种既能保证质量又能控制预算的分配策略。

Method: 基于特征信号预测任务难度，引入多标注者机制，并利用基于一致性的奖励模型，在固定预算下动态决定由人类还是LLM进行标注。

Result: 在多语言及多类任务中，相比现有方法，QUORUM 提升了最高 34.4% 的标注质量，同时降低了 8.8% 的成本。

Conclusion: QUORUM 证明了结合特征引导的动态路由和多标注整合，是提升大规模数据标注效率与可靠性的有效方案。
...
### [199] [Lexically conditioned realization ambiguity in Korean predicate morphology](https://arxiv.org/abs/2608.27966)
*Wonjun Oh,KyungTae Lim,Jungyeul Park*

Main category: cs.CL

TL;DR: 该研究探讨了韩语表层实现（Surface Realization）的复杂性，指出仅凭词根和语法范畴不足以唯一确定表面形式。


<details>
  <summary>Details</summary>
Motivation: 旨在验证韩语中表面形式是否由正则词法序列唯一确定，并探讨词汇身份与实现类在形态变化中的作用。

Method: 通过对韩语中各类不规则动词（如digeut、bieup、reu/reo不规则）进行对比分析，研究词根形态、词汇语义及实现类别之间的交互关系。

Result: 研究发现韩语存在“同形异构”现象，即相同的形态配置在不同词汇下会产生不同的表面形式，证明了单纯的形态表示存在局限性。

Conclusion: 韩语的表面实现受词汇身份、子范畴化及语义结构的影响，不能简单归结为纯形态学过程，必须纳入更高层级的语义与分类信息。
...
### [200] [Predicting Turn-Taking Outcomes in Multi-Party Conversation: Interpretable Modelling of Speech and Gaze Dynamics with Interpersonal Closeness](https://arxiv.org/abs/2608.27988)
*Mark Dourado,Karim Haddad,Henrik G. Hassager,Stefania Serafin*

Main category: cs.CL

TL;DR: 该研究利用GaMMA数据集，探讨了目光、语音及人际亲密度如何共同影响四人对话中的转接（转轮）预测。


<details>
  <summary>Details</summary>
Motivation: 在嘈杂、自然的多方对话中，预测何时参与交谈以实现平稳转换是有效沟通的核心，研究旨在揭示目光与语音线索在其中的作用。

Method: 基于GaMMA语料库，使用逻辑回归模型，提取转轮事件前的解释性行为特征（如目光转换模式、熵、注视对象、互视及音量），对转换结果（间隙或重叠）进行分类。

Result: 结合目光与音量特征可有效预测转轮（ROC AUC = 0.76 ± 0.04）；音量反映了说话者的控制力，而目光分布则体现了听者的准备度与竞争性介入。

Conclusion: 目光提供了一种抗噪性强的补充线索，能有效辅助理解多方对话中的转轮动态，且在不同噪声条件下表现稳健。
...
### [201] [Beyond Global Scalars: Synergizing Token-Level Statistics and Deep Semantics for Adversarial AIGC Text Detection](https://arxiv.org/abs/2608.28009)
*Peiming Li,Yifan Wang,Zhiyuan Hu,Shiyu Li,Zheng Wei,Yang Tang*

Main category: cs.CL

TL;DR: 提出了一种名为NeuroStat的机器生成文本检测框架，通过结合统计概率特征与语义隐藏状态，有效提升了在对抗性场景下的检测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的训练自由方法（基于统计）和训练基方法（基于语义）在面对对抗攻击时均存在局限性，前者丢失局部概率细节，后者易过拟合，缺乏综合性的检测手段。

Method: 构建了包含16000个样本的对抗基准MOSAIC；提出了NeuroStat框架，利用单一因果语言模型同时提取token级概率Logits和深层语义特征，并通过宏观状态残差调制（Macro-State Residual Modulation）进行特征融合。

Result: 在MOSAIC基准测试中，NeuroStat相较于现有最先进的检测方法表现出了极高的鲁棒性，显著降低了在对抗攻击下的性能衰退。

Conclusion: 通过跨越统计学与语义学的方法论，NeuroStat成功解决了机器生成文本检测在对抗环境下的脆弱性问题，为该领域设立了新的性能标准。
...
### [202] [Twin Worlds: Equivariance-Based Abstention for Evidence-Grounded Reasoning](https://arxiv.org/abs/2608.28018)
*Vy Nguyen,Ziqi Xu,Jeffrey Chan,Estrid He,Feng Xia,Renqiang Luo,Erik Cambria,Xiuzhen Zhang*

Main category: cs.CL

TL;DR: 提出了一种名为Twin Worlds (TW) 的框架，通过实体替换的等变性（equivariance）检测来判断LLM的回答是否真正基于给定证据，从而实现更可靠的拒答。


<details>
  <summary>Details</summary>
Motivation: 现有模型在知识密集型推理中易受到内部参数记忆干扰，导致产生未被证据支撑的“幻觉”，而现有的不确定性或证据充分性检测方法无法有效识别这种基于参数的错误推理。

Method: 利用“等变性”原则，通过对输入文本中的实体进行类型化替换（构造不同世界），观察模型输出是否随关系结构的变化而相应改变，并将等变性违反视为拒答信号。

Result: 在四个基准测试和三个主流模型上，TW框架在识别未被证据支撑的答案方面表现优于现有的基于不确定性和证据充分性的基准方法。

Conclusion: 基于等变性的拒答机制能有效缓解LLM在知识密集型任务中对参数记忆的依赖，提高模型生成答案的可靠性和证据接地性。
...
### [203] [A Shaky Voice Is Not Always a Dodge: Benchmarking Textual and Vocal Evasion Detection in Earnings Calls](https://arxiv.org/abs/2608.28040)
*Mirae Kim,Seonghun Jeong,Youngjun Kwak*

Main category: cs.CL

TL;DR: 该研究引入了DualEvasion基准，旨在通过分析财报电话会议中文字与语音的双重特征，更全面地识别高管的避重就轻行为。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅关注文本内容，忽略了语音信息（如语调、自信度）在识别沟通回避行为中的独立且互补的作用。

Method: 构建了包含505对问答数据的DualEvasion基准，对文本（是否回避）和语音（自信度）进行标注，并评估了多模态模型在捕捉声学线索方面的表现。

Result: 当前多模态模型难以有效识别语音自信度，尤其是对“不自信”的响应预测较差，且模型未能将声学特征与发言人的基准水平结合，与人类表现仍有显著差距。

Conclusion: 仅凭文字分析不足以完全识别沟通回避，多模态分析（结合语音线索和个人基准）是未来提升该领域检测准确性的关键方向。
...
### [204] [SimpCue: Cue-Based Prompting for Multilingual Text Simplification](https://arxiv.org/abs/2608.28042)
*Mehrzad Tareh,Horacio Saggion,Stefan Bott*

Main category: cs.CL

TL;DR: 研究探讨了在提示词中加入显式语言复杂度信息（提示线索）是否能提升大模型在多语言简化任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 虽然大模型能够进行文本简化，但目前尚不清楚引入具体的语言复杂度线索是否能进一步优化简化质量。

Method: 以 Qwen3-8B 为基座，在加泰罗尼亚语、西班牙语和意大利语上对比了三种提示策略：基线提示、包含金标准线索的提示和包含预测线索的提示；并使用 SARI、BLEU 等指标及人工评估进行分析。

Result: 预测线索提示在各项指标上表现最佳，但相比基线提升较小；金标准线索提示的改善效果不一致且存在语言差异。

Conclusion: 基于线索的提示对多语言简化有一定影响，但其收益有限，且高度依赖于具体语言和评估指标。
...
### [205] [CNeo-Bench: Diagnosing Large Language Models on Chinese Neologisms](https://arxiv.org/abs/2608.28053)
*Kaiyan Zhao,Zhongtao Miao,Zheyong Xie,Shaosheng Cao,Yoshimasa Tsuruoka*

Main category: cs.CL

TL;DR: 本文提出了CNeo-Bench，这是一个专门评估大语言模型理解和处理中文新词能力的基准测试。


<details>
  <summary>Details</summary>
Motivation: 中文新词（如谐音、拆字等）具有独特的语言机制，现有大模型在处理这些词汇时表现不佳且缺乏系统的评估工具。

Method: 构建包含4,759个新词的CNeo-Bench基准，并设计了两层评估框架，分别测试模型的“描述能力”和“机制操作能力”。

Result: 评估显示多数模型表现低于40%；发现模型存在“识别-操作”脱节现象，即能描述含义但难以还原原始表述；Few-shot提示虽有改善但无法完全解决深层挑战。

Conclusion: 中文新词处理对大模型而言仍是重大挑战，简单的提示工程无法弥补模型在理解复杂语言机制上的局限性。
...
### [206] [H-Scale: Hessian-Guided Scale Refinement for NVFP4 Sub-Byte LLM Inference](https://arxiv.org/abs/2608.28113)
*Hao Yu,Zheng Li,Dayiheng Liu,Jianwei Zhang*

Main category: cs.CL

TL;DR: 提出了H-Scale，一种针对NVIDIA Blackwell架构下NVFP4格式的轻量级后处理量化方法，通过优化微块缩放因子来提升LLM推理性能。


<details>
  <summary>Details</summary>
Motivation: NVFP4格式虽然具有微块设计优势，但其极度敏感的缩放因子选择空间尚缺乏有效探索，现有PTQ方法多聚焦于权重值而非缩放因子。

Method: 引入H-Scale方法，利用校准激活导出的对角二阶代理（diagonal second-order proxy）来选择硬件有效的组缩放因子，而非仅最小化权重重构误差。

Result: 实验表明H-Scale能有效提升主流LLM在NVFP4量化下的表现，使部分模型变体在精度上更接近BF16参考水平。

Conclusion: H-Scale作为一种无需推理开销的即插即用型缩放因子优化方案，是提升NVFP4量化LLM性能的有效工具。
...
### [207] [Nested Byte-Level Vocabularies Are Cheap to Deploy and Expensive to Share: A Pre-Registered Negative Result](https://arxiv.org/abs/2608.28151)
*Christos Koutsiaris*

Main category: cs.CL

TL;DR: 该研究探讨了一种利用字节级BPE切片特性实现单模型多词表大小的部署策略，通过控制令牌和嵌入层切片实现灵活部署。


<details>
  <summary>Details</summary>
Motivation: 旨在解决模型部署时对不同词表大小的需求，探索在不改变模型架构的前提下通过切片技术支持多词表规模的可行性。

Method: 利用字节级BPE的前缀嵌套特性，通过切片嵌入层和输出头来动态调整词表大小，并对比了单一词表大小模型与多规模模型在性能、鲁棒性及延迟上的差异。

Result: 切片技术在数值上是精确的（bit-for-bit复现），可显著降低部署权重，但相比定长专用模型仍存在一定的性能损失（BPB劣势）；多规模训练显著提升了模型对排版噪声的鲁棒性。

Conclusion: 多规模词表训练虽然会带来微小的精度损失，但能显著增强模型在噪声环境下的鲁棒性，且其效益主要源于多粒度训练过程而非条件控制。
...
### [208] [FinExam-10K: When Retrieval Helps Financial Reasoning?](https://arxiv.org/abs/2608.28155)
*Yan Lin,Jingyu Sun,Zhongliang Guo,Qing Li,Zhuohan Xie,Yuxia Wang*

Main category: cs.CL

TL;DR: FinExam-10K 是一个包含 10,198 道金融专业考试题（CFA/FRM）的大型基准测试，旨在评估模型在专业金融领域的知识、计算和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试缺乏对 CFA 和 FRM 全体系的覆盖，且专业金融考试对模型的领域知识与复杂推理要求极高。

Method: 引入 FinExam-10K 数据集，设立全覆盖跟踪与情境推理跟踪；测试 17 种主流模型，并评估 Function-RAG 和 FunctionGraph-RAG 的改进效果及门控机制。

Result: 当前最优模型总体准确率为 85.29%，但在高难度题目上表现较差；RAG 类方法在修复错误的同时会导致原本正确答案的丢失，净收益有限；通过门控机制筛选出的特定调用能小幅提升准确率。

Conclusion: 目前的 LLM 在处理复杂金融考试时仍面临挑战，单纯使用 RAG 难以全面提升推理能力，需结合特定场景下的门控策略与领域知识优化。
...
### [209] [Text Restoration of Ancient Documents with Language Models](https://arxiv.org/abs/2608.28170)
*Shibingfeng Zhang,Edoardo Caraffa,Annafelicia Zuffrano,Maddalena Modesti,Giovanni Colavizza*

Main category: cs.CL

TL;DR: 本研究探讨了利用语言模型修复古籍中因物理破损（缺字）导致文本缺失的可行性，并评估了其作为古籍修复辅助工具的潜力。


<details>
  <summary>Details</summary>
Motivation: 解决古籍数字化与修复过程中因物理损伤导致的文本缺失难题，减轻古籍整理者（古文字学家）的人工修复负担。

Method: 通过模拟真实世界损坏场景，应用不同架构的语言模型，并提出了多种解码策略来处理缺字边界与模型分词方案之间的不匹配问题。

Result: 实验表明目前无法实现全自动修复，但模型可作为辅助工具；修复性能受文档结构部位及是否已知缺失长度的影响显著。

Conclusion: 该研究首次系统分析了公式化与非公式化文本修复的差异，并为开发支持古籍修复的辅助系统提供了重要的评估基准与指南。
...
### [210] [Embedding Models for Stance-Aware Argument Retrieval](https://arxiv.org/abs/2608.28283)
*Angelo Sparacino,Francesca Toni,Adam Dejl*

Main category: cs.CL

TL;DR: 研究了密集嵌入模型在计算论证检索中对“论题相关性”与“立场正确性”双重评估的局限性，并提出了数据驱动的改进方案。


<details>
  <summary>Details</summary>
Motivation: 现有的语义检索模型在处理论证检索时，往往过度关注主题重叠而忽视了论证立场（支持或反对），且修复该偏见时容易陷入依赖极性关键词的“过度修正”误区。

Method: 引入诊断性词汇消融指标来量化偏见，并通过平衡论证课程学习（Balanced Argument Curriculum）与LLM增强的立场反转论证，迫使模型学习深层的方向逻辑。

Result: 实验表明该方法有效缓解了模型对极性关键词的过度依赖，并在立场感知的论证检索任务中取得了显著性能提升。

Conclusion: 通过数据中心化的方法可以有效引导嵌入模型克服表面化的词法捷径，从而实现更精准的立场感知论证检索。
...
### [211] [A Probabilistic Interpretation of KV Cache Eviction](https://arxiv.org/abs/2608.28293)
*Renato Geh,Alex Chen,Daniel Israel,Aditya Grover,Guy Van den Broeck*

Main category: cs.CL

TL;DR: 该论文将大模型KV Cache的剔除问题形式化为概率推断问题，并提出了一种通过纠正机制优化解码性能的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有的KV Cache剔除方法大多依赖启发式规则，缺乏严谨的理论基础，且在剔除后忽略了对解码过程的影响。

Method: 将KV剔除问题形式化为概率期望估计，通过采样进行近似；提出在解码时对已剔除的条目进行纠正，并证明现有启发式方法本质上是零方差的有偏估计器。

Result: 实验表明，基于概率框架的KV剔除配合解码时纠正机制，在不同任务中展现出比传统启发式方法更强的鲁棒性，并在相同压缩率下具有竞争力。

Conclusion: 通过概率视角理解KV剔除不仅能建立理论支撑，还能通过解码时纠正进一步提升模型性能，克服了现有启发式方法的局限性。
...
### [212] [BanglaMed-QA: A Question Answering System for Healthcare Support in Bangla](https://arxiv.org/abs/2608.28329)
*Rowzatul Zannat,Abdullah Al Shafi,K. M. Azharul Hasan,Atia Shahnaz Ipa*

Main category: cs.CL

TL;DR: BanglaMed-QA 是一个专为孟加拉语医疗领域设计的问答系统，旨在填补低资源语言在医疗信息获取方面的空白。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语在医疗问答系统领域缺乏相关数据集和专用系统，导致母语使用者获取可靠健康信息存在困难。

Method: 构建了包含4,493个问答对的医疗知识库；引入领域特定的词根词典、同义词集及词性标注以优化语义理解；采用监督学习（SVM为核心模型）进行问题分类；结合余弦、Jaccard、BM25和Levenshtein等多种相似度度量及投票机制进行查询匹配。

Result: 系统在自动化评估中达到了95%的F1分数，人类满意度评分平均为0.9/1.0。

Conclusion: BanglaMed-QA 有效提升了孟加拉语医疗问答的准确性，在实际应用中具有缩小医疗信息鸿沟的潜力。
...
### [213] [PersonaForge: Realistic Multi-Turn User Simulation for Agentic Systems](https://arxiv.org/abs/2608.28378)
...
TL;DR: 本文提出了PersonaForge框架，旨在弥合大模型训练与现实多轮交互需求之间的鸿沟，并通过构建高质量数据集和基准测试提升了智能体的多轮对话与任务执行能力。


<details>
  <summary>Details</summary>
Motivation: 现有训练数据多基于单轮查询，而现实中75.9%的交互为多轮，系统训练与实际用户交互模式存在显著偏差。

Method: 引入PersonaForge仿真框架，包含四维人格空间、基于真实统计数据的SOUL行为控制及反向深度构建技术，并据此构建了训练数据集和PersonaForge-Bench基准。

Result: 经PersonaForge训练的Qwen3.5-27B模型在综合得分上提升了4.1%，其中任务完成度和响应质量分别提升了6.0%和6.8%，同时交互效率更高。

Conclusion: PersonaForge为开发和评估真实场景下的多轮交互智能体提供了有效的方法论和基准支持。
...
### [214] [When Linguistic and Internal Confidence Diverge in Large Language Models](https://arxiv.org/abs/2608.28382)
*Hefan Zhang,Bingquan Zhang,Ming Cheng,Saeed Hassanpour,Weicheng Ma,Soroush Vosoughi*

Main category: cs.CL

TL;DR: 本文研究了大语言模型（LLM）的语言表达置信度与内部置信度（如Logits或语义熵）之间的一致性，指出两者之间存在显著的分歧。


<details>
  <summary>Details</summary>
Motivation: 探究用户通过对话获取的LLM“置信度”是否真正反映了模型底层的数学置信度，以评估其在可靠性管道中的可用性。

Method: 在8个分类任务和2个生成任务中，对30个不同模型进行多维分析，对比语言置信度与Logits置信度及语义熵的关联性、量级一致性及校准度。

Result: 语言置信度与内部置信度关联较弱；指令微调（Instruction-tuning）虽提高了表达的信心，但加剧了置信度偏差；提示词策略（如态度暗示）会改变置信度分布但无法提升对齐效果。

Conclusion: 语言置信度是一个“有损通道”，不能直接作为校准度量。在将其应用于可靠性分析前，必须通过多轴诊断进行评估。
...
### [215] [CultureConverse: A Multilingual Multi-turn Simulation Harness for Culturally Grounded Assistance in East and Southeast Asia](https://arxiv.org/abs/2608.28405)
...
TL;DR: CultureConverse 是一个评估大语言模型文化语境对话能力的框架和数据集，涵盖东亚和东南亚的多种文化场景。


<details>
  <summary>Details</summary>
Motivation: 现有的文化评估多局限于单轮选择题，无法反映用户在实际多轮对话中对文化敏感场景的需求。

Method: 构建了一个包含 10 个区域、58 个亚群身份和 7 个领域的模拟评估框架，包含 14,610 个基准评估片段和 274,295 条辅助生成的对话数据。

Result: 在 18 个模型的测试中，GPT-4o mini 表现最佳；评估框架与人类判断高度相关，且微调可显著提升模型在文化相关任务上的表现。

Conclusion: 该研究提供了一个有效的文化胜任力评估工具，证明了在文化语境下训练对话模型可以提升其实际辅助能力及跨任务的泛化性能。
...
### [216] [A Unified Framework to Elicit Structured Feedback for Interpretable Multi-Trait Essay Scoring](https://arxiv.org/abs/2608.28407)
*Shihang Yang,Sanwoo Lee,Ningning Zhao,Yunfang Wu*

Main category: cs.CL

TL;DR: HiFTS是一个用于多维度自动作文评分（AES）的统一自回归框架，通过生成基于评分标准的层级化思维链（CoT）反馈来提升评分的逻辑性与一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法多将反馈与评分解耦，或忽略了作文各维度间的相互依赖性，导致评分与反馈缺乏一致性及对评分标准的对齐。

Method: 提出HiFTS框架，结合教师模型蒸馏生成层级CoT反馈，并利用组合奖励（Group Relative Policy Optimization）进行训练，同时引入全局先验机制以减少长文本推理中的偏差。

Result: 在自建的CFMS-34数据集和ASAP++上表现优异，在实现高精度评分的同时，产出了更符合评分标准且逻辑连贯的反馈。

Conclusion: HiFTS通过深度集成反馈生成与评分过程，有效解决了多维度AES中的一致性与可解释性问题，为教育评估提供了一种更鲁棒的解决方案。
...
### [217] [Are These Modules Worth Their Cost? A Paradigm-Level Accuracy-Cost Analysis of In-context Learning Text-to-SQL](https://arxiv.org/abs/2608.28432)
*Jiayan Lin,Yujia Liu,Zijin Hong,Zheng Yuan,Yilin Xiao,Hao Chen,Qinggang Zhang,Xiao Huang,Feiran Huang*

Main category: cs.CL

TL;DR: 该研究系统地量化了ICL Text-to-SQL流水线中不同模块设计的代价与收益，并提出了具有成本效益的配置指南。


<details>
  <summary>Details</summary>
Motivation: 现有研究往往只报告端到端的准确率，缺乏对各个模块设计选择在准确率和成本方面的边际贡献的量化分析。

Method: 在统一受控环境下构建了17种范式配置，涵盖Text-to-SQL流水线的5个核心模块，并跨越4种不同能力的模型基座进行实验，分析其边际贡献与成本。

Result: 发现执行反馈优化（execution-feedback refinement）是唯一在低成本下普遍有效的范式；相比升级至前沿模型，在中间层模型上构建精细化流水线通常是性价比更高的策略。

Conclusion: 研究提供了一套可操作、感知成本的分层指南，无需针对每个模型进行逐一搜索即可实现高性能配置。
...
### [218] [Fidelity Is Not Enough: Dispatch-Level Instrumentation for Agentic Datasheet Extraction](https://arxiv.org/abs/2608.28439)
*Qing Ye,Meng-Hsuan Lin*

Main category: cs.CL

TL;DR: 研究指出代理模型在文档提取任务中存在“忠诚度（fidelity）”幻觉问题，即模型即便不调用工具也会捏造信息通过忠诚度检查，作者提出通过跟踪工具调用记录来检测此类“静默失败”。


<details>
  <summary>Details</summary>
Motivation: 作者发现模型可以在不使用工具的情况下直接伪造结果通过忠诚度验证，现有的评价指标无法识别这种绕过工具机制的行为。

Method: 构建了包含37个手工标注声明的基准测试，通过记录所有工具调用日志，开发了一个基于规则的失效归因分类器和一个仅基于工具调用记录的静默失败检测器。

Result: 检测器在207个正常样本中零误报，并成功拦截了所有植入的工具调用错误；研究还对比了物理测量下的“因果室”验证，指出工具层主要提升了可观察性和可移植性，而非直接提升准确率。

Conclusion: 代理模型的忠诚度检查存在盲区，单纯依赖提取结果的准确性是不够的，必须引入工具调用层面的可观测性监控来确保代理行为的真实性。
...
### [219] [Sliding-window beats linear attention](https://arxiv.org/abs/2608.28444)
*Alexia Jolicoeur-Martineau,Rhea Sanjay Sukthanker,Pashmina Cameron,Emy Gervais*

Main category: cs.CL

TL;DR: 该研究指出，相比于复杂的线性注意力（Linear Attention）改造，滑动窗口注意力（SWA）在大型语言模型的推理效率和长文本任务性能上表现更佳且无需额外训练。


<details>
  <summary>Details</summary>
Motivation: 二次注意力机制导致LLM推理时内存和计算开销随序列长度增加而呈二次增长，而目前将模型改造为线性注意力的方案缺乏与简单基线（如SWA）的严格对比。

Method: 通过对比实验，将带有sink token的滑动窗口注意力（SWA）与多种经过后训练的线性注意力模型在下游任务和长文本任务（Needle-in-a-Haystack, BABILong）中进行性能评测。

Result: SWA在多项任务中表现不逊于甚至优于线性注意力模型，特别是在长上下文推理任务中，SWA的性能是线性注意力的2到10倍。

Conclusion: SWA是解决二次计算复杂度的更优选，具有零后训练成本、极高的推理速度和更低的内存需求，作者建议优先使用SWA替代现有的线性注意力改造方案。
...
### [220] [Stranger, Fan, or Peer? A Systematic Study on the Role of Interlocutor in Persona-Based Dialogue Generation](https://arxiv.org/abs/2608.28467)
*Daniela Occhipinti,Malvina Nissim,Marco Guerini*

Main category: cs.CL

TL;DR: 该研究探讨了基于角色的对话系统中，训练、推理和评估阶段中“个人简介可见性”对模型表现及信息泄露的影响。


<details>
  <summary>Details</summary>
Motivation: 以往研究忽视了对话双方在不同阶段对彼此个人简介可见性的差异，导致模型倾向于简单复制文本而非表达角色特征。

Method: 通过三阶段因子分解方法，在不同可见性配置下训练模型，并利用大语言模型（LLM）作为评估者进行作者识别与内容泄露分析。

Result: 研究发现训练阶段的可见性比推理阶段更能决定模型是表达角色特征还是复制文本； interlocutor 个人简介的可见性有助于减少目标文本的复制；非对称披露会导致更严重的信息泄露，并使对话更容易被识别。

Conclusion: 个人简介内容泄露是由于训练和推理阶段可见性配置不当引起的，必须将这三个阶段进行明确区分以优化角色扮演对话系统。
...
### [221] [Acquire, Repair, Preserve: A Diagnosis-Guided Post-Training Recipe for Small-Model Dialogue Game Agents](https://arxiv.org/abs/2608.28458)
*Nan Li*

Main category: cs.CL

TL;DR: 该研究探讨了大型语言模型（LM）在交互式对话游戏中的表现，旨在解决模型在多轮对话中状态保持、反馈解读及行动约束方面的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有的静态基准测试无法充分评估模型在交互式场景中处理动态约束和反馈的能力，模型往往在对话中出现重复猜测、格式错误或忽略反馈等决策失误。

Method: 提出了三阶段训练配方：1. 通过监督微调（SFT）进行广泛的游戏参与训练；2. 利用局部偏好对（turn-local preference pairs）修复特定游戏家族中的机械性错误；3. 保持模型的通用能力。

Result: 模型在公开测试集上的 clemscore 从 10.67 提升至 38.92，同时保持了静态基准性能，但在领域外任务上的提升有限。

Conclusion: 广泛的监督微调对能力提升贡献最大，而局部监督在精确的错误检测下对特定领域内的任务改进有效，但跨领域迁移能力仍需进一步研究。
...
### [222] [ContextPilot: Teaching Agents for Proactive Context Management via Fine-grained RL](https://arxiv.org/abs/2608.28476)
*Zhuoshi Pan,Qizhi Pei,Junru Lu,Honglin Lin,H. Vicky Zhao,Di Yin,Xing Sun*

Main category: cs.CL

TL;DR: ContextPilot 是一种针对长程智能体任务的主动式上下文管理框架，通过增强工具集和改进强化学习机制，实现了更高效的上下文压缩与决策。


<details>
  <summary>Details</summary>
Motivation: 现有上下文管理方法存在工具功能单一、探索效率低以及动作级信用分配不足等问题，限制了LLM在长程任务中的表现。

Method: 引入了包含全局规划、长期记忆和软上下文卸载的扩展工具集；设计了一种基于上下文与熵变化识别关键决策的强化学习方法，并实现了细粒度的动作优势估计。

Result: 在长上下文问答和深度搜索任务中，ContextPilot 在保持更精简上下文的同时，性能优于现有基准模型。

Conclusion: 该框架通过优化上下文管理策略与学习算法，有效提升了LLM处理长程智能体任务的能力。
...
### [223] [Blind Men and the Elephant: Probing the Epistemic Myopia of LLMs under Long-Tail Divergent Knowledge](https://arxiv.org/abs/2608.28478)
*Zhuoshi Pan,Junru Lu,Yan Qian,H. Vicky Zhao,Di Yin,Xing Sun*

Main category: cs.CL

TL;DR: ElephantBench 是一个旨在评估大语言模型（LLM）对长尾事实中多种冲突观点记忆能力的基准测试，揭示了模型普遍存在“认知近视”现象。


<details>
  <summary>Details</summary>
Motivation: 现有 QA 基准通常假设存在单一标准答案，忽视了长尾事实中可能存在多种不同观点，研究旨在探究模型是否能够存储并准确检索这些分歧。

Method: 构建了包含 1,094 个问题的 ElephantBench，利用基于图的管道从低曝光语料中提取存在争议的知识点，并进行人工核实和溯源。

Result: 实验显示即使最强大的模型也仅能在 52.4% 的问题中找回所有分歧观点，且模型规模和推理时间计算的提升并不能完全解决该问题；模型对观点的回溯能力与语料中的曝光度正相关。

Conclusion: LLM 在处理长尾知识时存在显著的“认知近视”，该研究提供了一个可溯源的基准以诊断和改善模型的认知严谨性。
...
### [224] [NL2AGBench: Benchmarking LLM Auto-Formalization for AlphaGeometry](https://arxiv.org/abs/2608.28481)
*Samuel Xiao,Judy Song,Rory Hu,Ziliang Zong*

Main category: cs.CL

TL;DR: 该研究引入了NL2AGBench基准测试，旨在评估大模型将自然语言几何问题转化为AlphaGeometry形式化语言（DSL）的能力。


<details>
  <summary>Details</summary>
Motivation: AlphaGeometry等神经符号几何系统虽强大，但严重依赖手动将自然语言转化为特定DSL，存在严重的可用性瓶颈。

Method: 构建了NL2AGBench基准，通过AlphaGeometry的执行反馈而非文本相似度来验证形式化翻译的准确性，并对比了十种主流大模型的表现，同时探讨了少样本提示、微调和人工引导等改进策略。

Result: 闭源模型（如GPT-4）翻译正确率超过80%，而开源模型在维持几何约束和形式化准确性方面存在显著差距，难以满足实际应用需求。

Conclusion: 当前大模型在处理复杂几何问题翻译任务上存在较大局限，性能与闭源模型差距明显，通过错误分类和针对性微调可有效提升翻译质量。
...
### [225] [Ladders in Chaos: When, How, (and Perhaps Why) Does Test-Time Scaling Improve LLM Machine Translation](https://arxiv.org/abs/2608.28496)
*Di Wu,Sergey Troshin,Christof Monz,Antske Fokkens,Vlad Niculae*

Main category: cs.CL

TL;DR: 本文对比了大型语言模型在机器翻译中两种测试时扩展范式（顺序采样与并行采样），发现顺序采样在有限算力下表现更优，并揭示了其改善翻译质量的潜在机制。


<details>
  <summary>Details</summary>
Motivation: 随着测试时扩展（test-time scaling）成为提升LLM性能的主流方法，研究者旨在明确顺序采样与并行采样在翻译任务中的具体差异、表现及潜在改进机制。

Method: 通过对比实验分析两种采样范式，并结合多维人工评估（评估流畅度、自然度及准确性），同时通过受控消融实验探讨上下文环境对顺序自我提升的影响。

Result: 研究发现：1. 顺序采样在较小采样预算下拥有更高性能上限；2. 顺序采样能显著提升翻译的流畅度和自然度，但在大预算下可能牺牲准确性；3. 该方法的成功部分归因于模型获得了更广的目标语言上下文。

Conclusion: 顺序采样是提升翻译质量的有效手段，通过利用更大的上下文窗口实现自我优化，但需平衡准确性与流畅度，未来可进一步优化上下文构建策略。
...
### [226] [Phoneme- and Word-Level Metrics Using Self-Supervised Speech Representations for Forced Alignment Evaluation](https://arxiv.org/abs/2608.28508)
*V. S. D. S. Mahesh Akavarapu,Michael Daniel,Gerhard Jäger*

Main category: cs.CL

TL;DR: 提出两种无需参考标注的强制对齐评估指标：PCMI 和 WACS，利用自监督语音表征实现大规模、多语言的对齐质量评估。


<details>
  <summary>Details</summary>
Motivation: 传统的强制对齐评估依赖手动标注的时间戳，难以扩展至大规模或多语言场景，因此需要一种无需参考的自动评估方案。

Method: 利用自监督学习（SSL）语音表征，引入 PCMI（测量音素标签与语音聚类间的一致性）和 WACS（测量单词重复实现间的一致性）作为评估指标。

Result: 指标在随机和系统性干扰下表现出一致的下降趋势，在 FLEURS 和 DoReCo 数据集上表现良好，并能有效区分不同对齐系统的质量，与人工标注高度相关。

Conclusion: 基于 SSL 的语音表征可实现高效、可扩展且无需参考的强制对齐评估，为低资源语言及大规模语料分析提供了可靠工具。
...
### [227] [A Formal Limitation on Learning Human Language From Textual Corpora](https://arxiv.org/abs/2608.28560)
*Emily Cheng,Ryan Cotterell*

Main category: cs.CL

TL;DR: 该研究利用信息论探讨了仅凭话语形式（form）能否完整推断说话者意图，并推导了其理论上限。


<details>
  <summary>Details</summary>
Motivation: 探讨语言表达与其背后意图之间的本质联系，以确定是否存在仅凭语言形式无法消除的语义歧义。

Method: 将语言交互建模为意图、语境和话语的联合分布，利用信息论推导出解码器恢复意图的概率上限。

Result: 证明了即使是强大的大语言模型，也受限于不可约的语义不确定性，这种不确定性仅能通过语境而非单纯的话语形式消除。

Conclusion: 语义恢复存在理论极限，即任何仅依赖文本形式的表征模型都无法完全克服由语言特性导致的本质性歧义。
...
TL;DR: 提出了一种名为MAGP的端到端框架，旨在实现机器人感知的真实物理尺度（度量）几何重建，解决了现有几何感知方法尺度不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有几何重建方法通常产生具有任意尺度的相对几何，导致物体尺寸和空间距离在不同场景下不一致，难以直接应用于现实世界的机器人操作。

Method: 引入度量尺度等变增强（Metric Scale Equivariant Augmentation）以利用相机参数和深度观测强制对齐度量尺度，并使用灵活的度量条件机制（Flexible Metric Conditioning）支持多视角及多传感器组合输入。

Result: 在ETH3D等数据集上，绝对误差从2.01m大幅降低至0.07m；在LIBERO和RoboTwin等机器人任务中，操作性能显著提升，最高提升幅度达6.26%。

Conclusion: MAGP成功实现了具备度量一致性的几何感知，证明了度量尺度信息对于提升机器人操作的泛化能力和准确性至关重要。
...
### [229] [Beyond Data Scaling: Representation-Centric Continued Pre-training for Vision-Language-Action Models](https://arxiv.org/abs/2608.27550)
...
TL;DR: VLAct 是一种面向 VLA 模型的高效预训练骨干网络，通过利用有限的机器人数据实现强大的跨具身迁移能力。


<details>
  <summary>Details</summary>
Motivation: 机器人数据采集成本高且稀疏，导致 VLA 模型难以像网页数据那样大规模扩展，因此亟需提升有限数据下的表示学习质量。

Method: 提出了 VLAct 框架，采用 VLM 先验保留、多头连续动作协同监督及部分统一的跨具身动作布局，在下游任务微调前对异构机器人数据进行预训练。

Result: VLAct 在 LIBERO-Plus 和 RoboTwin 2.0 等基准测试中表现卓越，且在仅使用 20% 下游数据的情况下，于未见过的具身（RoboCasa-GR1）上超越了全数据基线。

Conclusion: 以表示学习为中心的持续预训练策略，能够在适度计算预算下实现与大规模数据扩展相媲美的竞争性性能，是 VLA 领域的重要研究方向。
...
### [230] [Remote Human and Robot Interaction for Greenhouse Gardening Using Virtual Reality](https://arxiv.org/abs/2608.27545)
*Daniel Udekwe,Hasan Seyyedhasani*

Main category: cs.RO

TL;DR: 本研究评估了利用虚拟现实（VR）远程操控地面机器人进行温室叶片病害检测和土壤水分评估的有效性。


<details>
  <summary>Details</summary>
Motivation: 旨在探索通过VR远程操控机器人完成温室复杂环境下的农业自动化监测，并分析影响作业效率与准确性的关键限制因素。

Method: 使用带有机械臂和摄像头的无人车系统，通过VR进行远程操作。在14种不同植株上进行叶片检查与水分评估实验，并对比不同植物冠层形态对作业结果的影响。

Result: 病害检测准确率最高达88%；土壤水分评估成功率最高为64.3%。研究发现植株冠层形态（尤其是遮挡问题）是影响评估成功率的核心因素，而非操作员技能问题。

Conclusion: 遮挡是阻碍感知的主要瓶颈，而非控制或培训不足。未来需针对植物冠层密度调整摄像视角和感知策略，以提升系统的鲁棒性。
...
TL;DR: PHR-VLA 是一种通过引入“规划视界推理”来增强视觉-语言-动作 (VLA) 模型性能的新框架。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型大多仅基于当前观测预测动作，缺乏对未来任务动态的推理能力，难以胜任细粒度、接触丰富的操纵任务。

Method: 引入轻量级辅助未来预测头，在训练期间通过特权潜空间表征将 VLA 的内部表征与未来观测的动态信息对齐，并使用局部补丁级潜空间动态监督。

Result: 在 LIBERO、现实世界拆卸任务及 Meta-World 基准测试中，PHR-VLA 均表现出更高的成功率，显著增强了策略的预期推理能力。

Conclusion: 特权潜空间动态对齐是为 VLA 策略提供有效训练信号、提升其复杂环境操纵能力的可行方案。
...
### [232] [Distributed Model-Based Diffusion: Finite Horizon Contraction under Bounded Delay](https://arxiv.org/abs/2608.27685)
*Seth Golembeski,Keith L. Gibson,Alexander Gross,Shreyas Kousik,Anirban Mazumdar*

Main category: cs.RO

TL;DR: 提出了一种基于采样模型预测控制的分布式多智能体轨迹优化方法（Distributed Model-Based Diffusion），有效解决了非线性、非凸系统在通信延迟下的多智能体控制问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体轨迹优化面临维度灾难及非凸、非线性挑战，且现有方法难以处理实际系统中的通信延迟问题。

Method: 引入分布式模型驱动扩散算法（Distributed Model-Based Diffusion），通过采样 MPC 方法处理复杂系统，并从理论上证明了其在非凸问题下的收敛性和鲁棒性。

Result: 实验表明，在加入通信延迟的情况下，该方法在圆圈交换任务中将总时长缩短了 31%，在空中战斗场景中胜率提升了 25%。

Conclusion: 该算法具备强大的鲁棒性，能够有效解决具有通信延迟的高难度、复杂非线性多智能体协作任务。
...
### [233] [One year in a forest: Analyzing the challenges of autonomous navigation in subarctic environments](https://arxiv.org/abs/2608.27628)
*Matěj Boxan,Nicolas Lauzon,Veronica Vannini,Mathis Turgeon-Roy,François Pomerleau*

Main category: cs.RO

TL;DR: 本文评估了多种机器人定位与建图算法在亚北极森林环境中的跨季节表现，分析了季节变化对感知系统的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有自主移动机器人的感知方案多基于城市环境，缺乏对亚北极地区严苛环境（如季节性降雪、植被变化）及GNSS/云端依赖受限场景的鲁棒性研究。

Method: 在亚北极森林中进行了为期一年的实地部署，采集64公里数据，对比测试了9种里程计、定位与SLAM算法在不同季节下的表现。

Result: 环境剧变显著削弱了主流技术性能；复杂的SLAM算法相比本体感知基线提升有限且系统更脆弱；视觉SLAM极易受季节外观变化影响；激光雷达在跨季节定位中表现优于雷达和视觉。

Conclusion: 亚北极环境的季节性变异对现有导航算法构成了重大挑战，研究揭示了复杂感知方案在该场景下的局限性，并为未来鲁棒性部署提供了实践经验。
...
TL;DR: 提出了一种基于迭代线性二次型(LQ)博弈的框架，用于解决多机械臂在共享空间内的协同运动规划问题。


<details>
  <summary>Details</summary>
Motivation: 集中式规划扩展性差，而分散式规划难以保证鲁棒性和安全性，博弈论方法在处理多臂机器人协同操作方面应用尚不充分。

Method: 利用迭代LQ博弈，通过线性化动力学与二次近似成本函数，采用黎卡提递归求解纳什均衡策略，并引入可微分惩罚项处理自碰撞与互碰撞。

Result: 实验表明该框架在高维环境下能生成平滑、安全且高效的轨迹，性能优于传统规划方法。

Conclusion: 差分博弈论框架对于解决高自由度多机器人协同操作任务具有显著的有效性和优势。
...
TL;DR: 提出了一种基于多模态视觉语言模型（VLM）与思维链（CoT）推理的自主水下洞穴导航框架。


<details>
  <summary>Details</summary>
Motivation: 水下洞穴环境存在视觉退化、声纳地图保守及通信受限等问题，导致传统导航系统难以有效作业。

Method: 利用视觉语言模型结合思维链，通过整合RGB图像、深度图和声纳数据，推理出光照梯度、通道形态及几何复杂性，从而指导自主3D导航。

Result: 在多种洞穴拓扑结构的模拟实验中，该框架成功实现了零碰撞的端到端穿越，并保持了安全的边界距离。

Conclusion: 该研究证明了结合VLM与CoT推理的多模态融合方案能够有效克服复杂水下环境的感知局限，实现安全自主导航。
...
TL;DR: ROBUST TAMP 是一个面向部分可观测环境的模块化任务与运动规划（TAMP）框架，通过实时场景重建与响应式重规划机制，解决了基础模型在面对未见物体及不完整场景信息时的规划失效问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基础模型引导的 TAMP 系统往往假设场景信息完全已知，或在执行失败后才进行代价高昂的模型级重规划，难以应对任务执行中物体被发现或场景状态发生变化的动态环境。

Method: 构建了一个基于 LLM/VLM 的模块化框架，限制模型仅基于当前可见的场景状态进行规划，引入严格的可执行性校验接口，并在发现新物体或执行到稳定阶段时，结合历史执行记录与结构化上下文进行重规划。

Result: 在包含隐藏物体、非目标物体发现及铰接容器交互的六个复杂场景实验中，验证了该框架能有效处理部分可观测性，显著提升了任务成功率，并评估了不同规模模型的推理成本与表现。

Conclusion: ROBUST TAMP 通过将物体发现作为独立的重规划事件，并配合场景感知的实时监测，成功将基础模型的能力扩展到了复杂的、动态的部分可观测现实场景中。
...
TL;DR: 该研究探讨了人形机器人通过被动式移动座椅进行全向坐姿移动的控制方法，实现了无需运动模仿奖励的强化学习策略及零样本仿真到现实的迁移。


<details>
  <summary>Details</summary>
Motivation: 人形机器人通过准直驱电机站立时持续消耗能量，而人类坐姿可通过椅子支撑体重，因此探索坐姿下的移动操作具有能效提升潜力。

Method: 扩展了站立速度跟踪环境，引入被动椅模型和坐姿特定奖励；利用纯本体感觉和速度指令训练强化学习策略，并分析了对称性正则化（SY）、足部滑动正则化（FS）和指令课程（CC）对性能的影响。

Result: 通过组合FS与SY或CC，避免了局部最优解问题；策略在全向速度跟踪上表现优异，且在Unitree G1上实现了零样本现实部署。

Conclusion: 该方法证明了人形机器人可以通过非固定的坐姿接触与间歇性足部推进实现高效的坐姿移动，并展示了良好的鲁棒性和迁移能力。
...
TL;DR: DeicticVLA 提出了一种统一的架构，通过指称掩码（deictic masks）和文本提示，使 VLA 模型能够同时处理纯文本、视觉-文本以及纯视觉指令，显著提升了机器人操作任务在复杂环境下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型在区分外观相似或同类物体时，难以仅通过语言描述实现精准定位，缺乏对视觉指向性信息的有效利用。

Method: 引入 DeicticVLA 系统，将语言指令、视觉-语言指令和纯视觉指令规范化为统一的文本提示与指称掩码；采用两阶段训练策略，并探讨了多种视觉提示与掩码集成方法。

Result: 实验表明，两阶段训练能显著提升模型对指称掩码的使用能力；在真实世界任务中，视觉辅助指令（VLI/VI）在面对新物体和复杂场景时，成功率远高于纯文本指令（LI）。

Conclusion: 该研究验证了统一的三模式交互接口的有效性，证明了通过指称掩码辅助可以大幅增强 VLA 模型在未见场景和新类别对象上的操作鲁棒性。
...
TL;DR: 提出了一种基于多评论家强化学习（Multi-Critic RL）的接触引导探索策略，用于解决移动机械臂进行非抓取式物体操作的复杂动力学和稀疏接触问题。


<details>
  <summary>Details</summary>
Motivation: 现有的模型基础和模型无关方法在处理非抓取式操作中的混合动力学和接触稀疏性时表现不佳。

Method: 引入一个专门的探索评论家，利用稠密的接触寻找奖励引导末端执行器，并结合通用抓取算法提取候选交互点，通过奖励衰减机制过渡到任务最优策略。

Result: 在箱子推动、椅子搬运和洗碗机开启任务中表现优异，并在四足移动机械臂上成功实现了真实世界的椅子搬运部署。

Conclusion: 该方法通过接触引导有效地克服了稀疏接触带来的探索难题，实现了泛化能力强且可在真实场景部署的非抓取式操作。
...
TL;DR: 研究探讨了非拟人化工业机械臂在人机协作中通过“自我披露（Self-disclosure）”进行小谈话（Small talk）对协作效果的影响。


<details>
  <summary>Details</summary>
Motivation: 以往研究表明机器人小谈话有益于人机协作，但缺乏关于如何设计工业机器人小谈话策略（高披露 vs 低披露）的研究。

Method: 开发了一款由LLM驱动的机械臂，设置了低披露和高披露两种策略，并邀请50名参与者进行用户研究，分析其对团队协作和社交互动的影响。

Result: 实验发现，低披露策略反而促使参与者进行更多的自我披露，并获得了更高的团队协作和协调评价，尤其在有机器人使用经验的群体中更为显著。

Conclusion: 增加机器人的自我披露程度并不能必然提升人机亲密度或协作质量，设计人机互动时需考虑用户背景及披露策略的适用性。
...
TL;DR: 提出了一种结合模型驱动与模型驱动学习的四层分层框架，旨在实现工业场景下的全自动无干预零件拣选。


<details>
  <summary>Details</summary>
Motivation: 模型驱动方法在遮挡或感知失败时易陷入死锁且配置繁琐，而模型驱动方法缺乏生产所需的可靠性，现有方法未将二者有效结合。

Method: 构建四层分层混合框架，以模型驱动流水线为基础，引入基于模型驱动的探索智能体处理死锁，并配合在线自学习机制评估抓取质量。

Result: 实验显示该方法在三个汽车零件上实现了100%的料箱清理率，显著优于单纯的模型驱动（50.9%）和模型无关基准。

Conclusion: 通过模型与数据驱动的协同，该方法克服了单一技术的局限性，有效提升了工业拣选的自主性与可靠性。
...
TL;DR: PAMoR 是一种通过效价-唤醒度（V-A）坐标定量控制人形机器人动作情感的生成框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法多依赖参考片段或模糊的情感词，缺乏可量化的情感控制参数，难以直接应用于机器人运动学。

Method: 利用姿态扩展和运动能量计算V-A坐标，并在扩散模型的去噪步骤中结合动作先验与情感先验，实现对29自由度机器人运动的实时自回归生成。

Result: PAMoR在保证动作忠实度的同时实现了对V-A坐标的精确控制；感知测试表明，其情感识别率显著优于基准模型，接近人类表现。

Conclusion: 该方法成功将情感转化为可量化的控制参数，为在社交机器人中实现可编辑、高表现力的动作生成提供了有效方案。
...
TL;DR: 提出了一种面向多机器人系统的免校准分布式概率框架，用于解决异构传感器在环境状态估计和源定位中的非线性与不一致性难题。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统中的传感器通常未经校准且具有异构性，其非线性和不一致的响应阻碍了可靠的信息融合，限制了源定位的精度。

Method: 通过引入基于秩的特征（rank-based feature）来捕获观察值的相对演变，使其对传感器缩放和非线性具有不变性；利用专家积（product of experts）公式融合局部信念，并结合信息区域分配与路径规划策略以优化协作效率。

Result: 在高保真气体传感器仿真测试中，该方法显著优于基于标准测量的基准方法，在强传感器异构性环境下实现了可靠的源定位。

Conclusion: 该研究证明了免校准传感表示可有效应用于分布式机器人系统，为解决各类涉及异构传感器的估计任务提供了通用范式。
...
TL;DR: 提出了一种针对变形无菌饮料纸盒的无需训练吸取抓取系统，实现了机器人高效分拣。


<details>
  <summary>Details</summary>
Motivation: 回收废弃物（如变形的饮料盒）具有形状不规则、易变形的特点，传统的分类抓取方法难以应对。

Method: 利用开放词汇视觉语言模型进行检测，SAM2生成实例掩码，并结合k-NN PCA、Sobel算子和RANSAC平面拟合三种几何评分方法选择最佳吸取点。

Result: 在多种变形程度和复杂场景的测试中，单物体抓取成功率达88.2%，端到端复杂场景抓取成功率为72.6%。

Conclusion: 该解耦式系统有效地解决了非结构化废弃物抓取中的定位与抓取规划难题，无需额外训练即可具备良好的泛化能力。
...
TL;DR: CoCoBench 是一个针对多智能体具身协作任务的基准测试，旨在诊断多智能体在家庭任务中的协调缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏细粒度的多智能体协作诊断，未能有效揭示任务重复、执行顺序冲突、资源竞争和交付脱节等协调失败问题。

Method: 构建包含897个经由Oracle验证的实例的CoCoBench基准，涵盖任务分配、顺序执行、互斥访问和任务交接四类核心协调架构，并评估了11种主流多模态大模型（MLLMs）。

Result: 研究发现模型能力在不同协调架构上表现不均衡，高任务成功率并不等同于出色的协作能力，且不同模型在特定协作场景下表现差异显著。

Conclusion: 该研究指出多智能体协作能力具有特定构造依赖性，未来的研究应针对性地改进模型架构以增强各维度的协作效能。
...
TL;DR: 提出了一种基于大语言模型（LLM）的无人机实时语义导航框架，旨在提升目标导航（ObjectNav）任务的时间效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在ObjectNav任务中往往依赖离线推理或受限于模拟器的动作空间，难以应对真实环境下的实时导航需求。

Method: 利用LLM解析自然语言指令并进行语义推理，结合实时目标检测、3D空间映射以及多项式样条插值轨迹规划。

Result: 实验证明该框架在模拟和真实场景中均显著缩短了任务耗时，同时保持了较高的搜索准确率。

Conclusion: LLM引导的语义推理能有效提升无人机在ObjectNav任务中的任务执行效率与实时响应能力。
...
TL;DR: 提出了一种名为STEGNav的训练无关导航框架，通过构建时空事件图来提升多模态终身导航中的任务完成率与效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于场景图的方法难以区分相似实例，无法有效整合语义目标与探索边界，且缺乏对历史轨迹经验的利用。

Method: 引入时空事件图（STEGNav），通过空间轴实现查询条件下的实例定位与探索边界分析，通过时间轴利用双窗口内存保留历史决策与任务结果，并结合视觉语言模型（VLM）进行目标推理。

Result: 在GOAT-Bench上达到66.3%的SR和39.7%的SPL，并在HM3D数据集上表现优异，验证了时空表示在提升导航可靠性和经验重用方面的有效性。

Conclusion: 通过将场景图扩展为时空事件图，有效解决了多模态终身导航中的实例辨识、探索平衡及经验积累问题，显著提升了导航性能。
...
TL;DR: MaCoPlanner是一个基于设备手册知识的机器人工业面板操作任务规划框架，通过符号化表示和自动约束检查提高操作准确性与安全性。


<details>
  <summary>Details</summary>
Motivation: 工业面板操作不仅需要精确控制，还必须严格遵守分布在异构手册中的操作规程、安全规则及设备状态约束。

Method: 将手册转换为类型化中间表示，利用检索增强生成进行任务规划，并通过符号化回滚机制对生成的计划进行约束合规性检查与循环修复。

Result: MaCoPlanner在Level-2和Level-3任务中的成功率分别提升至84.4%和43.2%，最终违规率降低至2.7%。

Conclusion: 该框架通过自动化约束验证和修复机制，有效提升了机器人处理复杂工业面板操作的可靠性与合规性。
...
### [249] [Cooperative Risk-Aware Exploration in Heterogeneous Multi-Robot Systems Using Algorithmic Altruism](https://arxiv.org/abs/2608.28409)
*Brooks A. Butler,Jair Certório,João P. Hespanha,Magnus Egerstedt*

Main category: cs.RO

TL;DR: 提出了一种基于演化生物学中汉密尔顿法则的博弈论框架，用于多机器人系统在危险环境下的合作探索与风险分配。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统在危险环境中部署时，需要解决如何在异构成员间分配风险，同时最大化信息获取效率的问题。

Method: 利用“社会纳什均衡”概念，通过引入受汉密尔顿法则启发的亲缘权重（relatedness weights）来调整代理的效用函数，使得低价值机器人愿意承担风险以支持高价值任务。

Result: 仿真与硬件实验表明，该方法有效减少了重复探测，改善了机器人间的空间分布，并实现了根据代理价值进行的风险再分配，同时保持了良好的地图覆盖率。

Conclusion: 基于利他主义的博弈论规划框架能够优化异构机器人团队的风险承受能力与任务执行效率，是处理危险环境协同任务的有效方案。
...
### [250] [PanelShield: Verifiable Closed-Loop Safe Planning for Robotic Industrial Panel Operation](https://arxiv.org/abs/2608.28305)
*Guipeng Xin,Jiahe Xu,Chenhui Wan,Jie Liu,Youmin Hu,Zhongxu Hu*

Main category: cs.RO

TL;DR: PanelShield 是一个用于工业面板操作的可验证闭环安全规划框架，通过将基础模型规划与形式化验证相结合，实现了高安全性与可解释性的自动化控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于基础模型的规划器虽然语义理解能力强，但在工业场景中缺乏可计算、可定位且可重复的违规检测与修复机制，难以满足严格的安全规范。

Method: 提出 PanelShield 框架，利用任务手册生成参数化动作序列，并结合线性时序逻辑（LTL）和安全有限状态机（Safety FSM）进行双重形式化验证，在出现违规时提供结构化反例以进行精准修复。

Result: 在多层长程规划基准测试中，PanelShield 将违规率降低至 2.7%，平均响应延迟为 4.1 秒，并在仿真及真实机器人实验中验证了其可行性。

Conclusion: PanelShield 为机器人执行工业面板操作提供了一种平衡灵活性、安全性和审计能力的可靠方案。
...
TL;DR: 提出了一种名为Self-Constrained Reasoning (SCR) 的框架，旨在将自然语言指令转化为线性时序逻辑（LTL）任务规格，用于机器人执行。


<details>
  <summary>Details</summary>
Motivation: 人类自然语言指令具有歧义性和不确定性，现有的LLM翻译方法要么因过度推理违反领域约束，要么因后置约束导致推理逻辑受损。

Method: SCR框架通过将结构化约束表示法与分层决策机制相结合，将领域知识内化到模型决策过程中，从而在受限空间内进行推理。

Result: 实验表明，SCR在领域约束满足度和对未见指令的泛化能力方面均有显著提升。

Conclusion: SCR提供了一种既能保持推理灵活性，又能确保生成规格符合机器人执行安全约束的有效且可解释的解决方案。
...
TL;DR: ChainSplat 是一种仅通过多视角 RGB 视频即可同时学习可变形线性物体（DLO）3D 几何、运动学和动力学的物理启发框架。


<details>
  <summary>Details</summary>
Motivation: 现有 DLO 操作方法依赖多阶段流水线和深度输入，存在计算昂贵、误差累积和状态空间高维等问题，难以应对动态交互场景。

Method: 利用高斯溅射（Gaussian Splatting）技术，将 DLO 表示为通过转动关节连接的刚性链结构，结合螺旋理论建立紧凑的动力学模型，实现视觉表征与物理结构的联合优化。

Result: 在动力学预测、3D 重建和 RGB 渲染方面均达到 SOTA 水平，并支持实时的状态与力估计以及模型预测控制。

Conclusion: ChainSplat 为复杂的 DLO 操作提供了一种高效、高保真的建模方案，具有很强的现实操作实用性。
...
TL;DR: 本文提出了Aero Hand Open，这是一个为肌腱驱动拟人手设计的开源平台，旨在解决此类手部在仿真与现实转换中的控制与建模难题。


<details>
  <summary>Details</summary>
Motivation: 肌腱驱动手具有结构紧凑、成本低的优势，但因其复杂的欠驱动传输特性，导致仿真建模困难且难以通过强化学习进行有效训练。

Method: 开发了精确的线缆传输仿真模型、建立双向致动映射（Actuation Map），并提供了一套完整的强化学习训练框架，实现零微调的仿真到现实部署。

Result: 通过该平台，用户可以在仿真环境中训练控制策略，并直接在真实硬件上运行，无需额外的微调或状态估计。

Conclusion: 该研究通过开源硬件设计、仿真模型和配套算法，显著降低了肌腱驱动手的控制难度，促进了在该领域的研究与开发。
...
### [254] [Competitive One-Step-Ahead Control of Friedkin--Johnsen Networks: Potential Games, Stability, and the Price of Competition](https://arxiv.org/abs/2608.27623)
*Gabriel Gentil,Amit Bhaya*

Main category: eess.SY

TL;DR: 该研究探讨了具有重叠影响力的Friedkin-Johnsen网络中的竞争性单步预测控制，分析了纳什均衡的存在性、稳定条件及社会福利损失。


<details>
  <summary>Details</summary>
Motivation: 为了解决社交网络中多个参与者在进行预测控制时的冲突，以及理解竞争如何导致系统福利损失。

Method: 利用势能博弈论、雅可比条件、代数解析式及控制感知中心性指标对网络动力学进行数学建模与稳定性测试。

Result: 确定了纳什均衡的收敛条件（包括序列和并行扫频），证明了特定阻尼条件下均衡的稳定性，并给出了福利损失的闭式解。

Conclusion: 该框架有效量化了网络中的目标冲突，通过闭式解揭示了竞争造成的福利损失，并为控制策略提供了理论稳定性保证。
...
### [255] [Exact Decomposition of Value Functions for Two-Player Games in Hamilton-Jacobi Reachability](https://arxiv.org/abs/2608.27654)
*Dylan Hirsch,William Sharpless,Sylvia Herbert*

Main category: eess.SY

TL;DR: 该论文将单智能体Hamilton-Jacobi可达性分析中的复合任务值函数分解方法推广到了双智能体博弈场景。


<details>
  <summary>Details</summary>
Motivation: 以往针对复杂任务的HJR分解框架多局限于单智能体，缺乏对具有对抗环境的双智能体设置的适用性证明。

Method: 在满足关键单调性假设的前提下，将连续时间、有限时域下的复合任务分解结论推广至双智能体博弈，并给出了相应的理论证明及反例分析。

Result: 证明了在满足特定单调性假设时，复合任务的值函数分解在双智能体博弈中依然成立，并揭示了若假设不满足则会导致结论失效。

Conclusion: 该研究通过引入单调性约束，成功将HJR复合任务分析框架扩展至双智能体系统，为处理复杂的对抗性安全控制任务提供了理论支撑。
...
### [256] [From Generation to Discovery: Diffusion Mutation Kernels for Circuit and Physical Design](https://arxiv.org/abs/2608.27649)
*Dinithi Jayasuriya,Aravind Saravanan,Nilesh Ahuja,Amanda Rios,Amit Trivedi*

Main category: eess.SY

TL;DR: 该论文提出了一种基于扩散模型的发现框架，旨在通过学习转换算子而非单纯拟合分布，在复杂工程设计空间中生成超出训练语料库的创新且有效的方案。


<details>
  <summary>Details</summary>
Motivation: 传统的生成模型仅能重现已知数据的分布，无法解决“发现”问题，即在满足硬性结构约束并提升设计指标的前提下，生成全新的、优于现有设计方案的构件。

Method: 引入一种基于扩散的变异核（mutation kernel），通过受控的局部重噪声与去噪过程，在保留设计可行性规律的同时探索设计空间；引入共形风险控制机制在评估前剔除不佳候选者，并结合外部非微分工程仿真工具进行最终评估。

Result: 该框架在三个电子设计领域表现卓越：在32位加法器设计中，发现的电路在面积和延迟上分别优化了18%和17%；成功复现了训练集缺失的七种放大器拓扑；并在宏布局任务中达到了工业级布局器0.68倍的线长水平。

Conclusion: 该方法成功将生成模型转化为发现工具，通过结合生成式扩散与外部仿真评估，有效拓展了自动设计（Auto-design）在复杂硬约束工程领域的应用边界。
...
### [257] [Horizon-Dependent Tube MPC for Elliptical-Orbit Rendezvous Under Mass Uncertainty](https://arxiv.org/abs/2608.27659)
*Omer Burak Iskender,Keck-Voon Ling*

Main category: eess.SY

TL;DR: 该论文提出了一种在航天器逼近任务中，通过量化线性化误差与干扰约束的边界，确保制导律在全距离范围内的安全性设计准则。


<details>
  <summary>Details</summary>
Motivation: 航天器在长距离逼近过程中，近距离安全的控制器往往在远距离因线性化误差失效，且缺乏有效的预判准则。

Method: 推导了线性化误差与扰动界的边界，提出了一种基于相对轨道要素的改进建模方法，并将预测视界与可行性约束绑定。

Result: 在火星样本返回任务模拟中，该方法相比基准方案在圆轨道和椭圆轨道上分别节省了29%和40%的燃料，并满足极高的对接精度要求。

Conclusion: 通过量化安全保证的失效点，可实现更优的燃料效率，但该性能提升是以增加飞行时间和计算成本为代价的。
...
### [258] [Explicit Separators for Consecutive Levels of Parrilo's Sum-of-Squares Hierarchy over the Copositive Cone](https://arxiv.org/abs/2608.27743)
*Jiachen Shen,Hui Zhong*

Main category: eess.SY

TL;DR: 该论文证明了Parrilo锥序列在n=5时各级之间存在严格包含关系，即K(5,1)subsetneq K(5,2)subsetneq K(5,3)subsetneq K(5,4)。


<details>
  <summary>Details</summary>
Motivation: Parrilo锥是完全正锥（copositive cone）的半正定近似，研究其相邻层级是否严格不等是一个长期未解决的公开问题。

Method: 利用Horn矩阵的对角缩放并沿正向偏移，构造出显式的有理矩阵，并通过精确有理Gram矩阵和对偶矩泛函进行验证，同时结合Dickinson等人的缩放定理及Schweighofer和Vargas的完备性定理。

Result: 首次给出了n=5时各级锥之间存在严格包含关系的明确矩阵示例，并证明了这种严格包含在任意大的层级上均会反复出现。

Conclusion: 该研究确定了Parrilo锥序列在n=5时的层级结构，证实了通过增加层级确实能产生更紧的近似，并提供了有效的数值验证方案。
...
### [259] [Design of a Fast Crowbar Protection Circuit for Single Event Effect Testing](https://arxiv.org/abs/2608.27772)
*Collin Burt,Joseph D'Amico,Joseph Van Grinsven,Tommy Aldaz,Nathaniel Dodds*

Main category: eess.SY

TL;DR: 本文介绍了一种定制的撬棍（crowbar）电路，用于在超过电流阈值时迅速切断电源，以保护设备免受单粒子锁定或烧毁测试中的永久性损坏。


<details>
  <summary>Details</summary>
Motivation: 在进行单粒子锁定/烧毁测试时，设备容易因过流造成永久性物理损伤，需要一种高效的快速保护机制。

Method: 设计并实现了一种可编程电流阈值的定制撬棍电路，能够在检测到电流超限时立即切断电源。

Result: 该电路能够有效地在故障发生瞬间移除功率，防止了测试过程中的器件永久损坏。

Conclusion: 该保护电路是评估器件抗辐射性能（SEU/SEL/SEB）的有效保障工具，能够提高测试的安全性与可靠性。
...
### [260] [Observability Analysis for Fusion of Doppler Measurements in Multistatic Radar Near the Tx-Rx Baseline](https://arxiv.org/abs/2608.27838)
*Rong Yang,Yaakov Bar-Shalom,Leung Hong Huat Michael*

Main category: eess.SY

TL;DR: 该论文研究了在多基地雷达发射机-接收机（Tx-Rx）基线模糊区内，利用多普勒信息进行目标轨迹估计的方法。


<details>
  <summary>Details</summary>
Motivation: 在Tx-Rx基线模糊区，由于距离和方位测量精度急剧下降，目标的可观测性降低，导致传统定位方法失效，迫切需要提升该区域的目标追踪能力。

Method: 通过可观测性分析评估性能，并提出了一种基于多初值最大似然（ML）估计进行初始状态求解，再结合扩展卡尔曼滤波（EKF）进行动态更新的多普勒专用测量融合算法。

Result: 仿真结果表明，所提方法在可观测性受限的情况下能够有效完成目标状态估计，验证了理论分析的准确性。

Conclusion: 该方案为多基地雷达系统在模糊区内的目标监测提供了有效的技术路径，解决了仅依赖多普勒信息进行轨迹估计的挑战。
...
### [261] [Graphon Design for Human-Machine Coordination under Bounded Rationality: Optimality of Stochastic Block Models](https://arxiv.org/abs/2608.27851)
*Zhewei Wang,Vu Anh Phi,Marcos M. Vasconcelos*

Main category: eess.SY

TL;DR: 该研究提出了一种利用图论（Graphon）框架优化多智能体系统网络拓扑的方法，旨在促进异构、有界理性智能体在“雄鹿狩猎”博弈中的协调。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统（如人机协作）在面临有界理性和决策不确定性时，如何设计最优网络拓扑以最大化全局协调水平是一个计算上极其复杂的组合优化难题。

Method: 利用平均场理论将离散图空间提升至连续的图论空间（Graphons），结合Logit学习动态，应用变分法证明了双峰理性分布下的最优解存在于随机块模型集合中，并提出了水填充（Water-filling）算法求解最优图论。

Result: 成功绕过了直接在离散图空间进行组合优化的计算困境，能够通过优化后的图论有效采样出具有高协调性能的有限图结构。

Conclusion: 该方法提供了一种高效的数学框架，用于解决复杂异构多智能体系统中的拓扑结构设计问题，证明了通过连续空间映射可以有效优化离散网络性能。
...
### [262] [AutoDRI: Bridging the Semantic Gap for Automated Design Rule Integration in CP-SAT-Based Cell Synthesis under Multi-Patterning](https://arxiv.org/abs/2608.27891)
*Yuhao Ren,Yucheng Wang,Zihao Chen,Chung-Kuan Cheng,Zhiang Wang*

Main category: eess.SY

TL;DR: AutoDRI 是一个多智能体框架，旨在通过自动化方式将复杂设计规则转换为可执行的 CP-SAT 约束，解决标准单元合成中的设计规则集成（DRI）瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统 DRI 高度依赖手动编码，且现有规则编码无法有效处理多重曝光技术下的复杂切口（cut-based）约束。

Method: 结合几何语义库、标准化冲突集编码、构造性多色切口建模方法，以及反馈驱动的多智能体协同流程。

Result: 在 41 个单元基准测试中实现了极高的规则集成正确率（Gemini-3-pro 达到 33/33），运行时间与人工编码相当，并成功通过了 KLayout DRC 和 Cadence LVS 验证。

Conclusion: AutoDRI 有效填补了自然语言规则与机器可执行约束之间的语义鸿沟，实现了先进工艺节点下设计规则集成的自动化与高效化。
...
### [263] [Backup Control Barrier Function Synthesis using Sum-of-Squares Reachability](https://arxiv.org/abs/2608.27916)
*Jungbae Chun,Shima Sadat Mousavi,David E. J. van Wijk,Ersin Daş,Aaron D. Ames,Felix Biertümpfel*

Main category: eess.SY

TL;DR: 该论文提出了一种基于有限时域SOS后向可达性分析的构造性方法，用于合成更优、更少保守的备份控制器对，从而扩大bCBF框架下的安全区域。


<details>
  <summary>Details</summary>
Motivation: 传统的备份控制障碍函数（bCBF）极度依赖于预设的备份集和控制器，往往导致系统过于保守，限制了安全集范围。

Method: 利用SOS（平方和）优化技术进行有限时域后向可达性分析，从初始备份集出发，计算满足约束的安全控制器，并证明其扩展后的区域可作为新的有效备份集。

Result: 通过分段备份控制器策略，成功构建了比原始备份集更大的安全区域，并验证了在输入受限非线性系统下的有效性。

Conclusion: 该方法通过系统性地优化备份对，有效缓解了bCBF的保守性，提升了受限非线性系统的安全控制能力。
...
### [264] [Securing Cooperative Sensing in UAV Swarms Against Conformity-Driven Byzantine Attacks](https://arxiv.org/abs/2608.28017)
*Ruixing Ren,Junhui Zhao,Qiuping Li,He Fang,Jiamin Li,Dongming Wang*

Main category: eess.SY

TL;DR: 针对6G无人机集群ISAC网络中拜占庭攻击导致共识失效的问题，提出了一种融合演化博弈论与MAP估计的鲁棒融合框架。


<details>
  <summary>Details</summary>
Motivation: 模仿学习带来的共识机制使得拜占庭攻击能够扩散虚假信息，导致传统基于独立性假设的融合方法在无人机集群中失效。

Method: 结合演化博弈论中的有限理性意见动力学建模误导率演化，推导出演化稳定状态（ESS），并将此误差动力学嵌入逐节点的MAP（最大后验概率）规则中。

Result: 揭示了攻击概率超过0.5即会导致群体级误导失效，提出了精确的平均场固定点；在各种网络拓扑和攻击强度下，该框架实现了近100%的情境推断准确率。

Conclusion: 所提出的框架能有效克服拜占庭攻击带来的共识破坏，表现出比多数投票和信誉加权等传统方法更强的鲁棒性和准确性。
...
### [265] [Anytime Primal--Dual Certification of the Maximum Disturbance Radius in Robust MPC](https://arxiv.org/abs/2608.28056)
*Wenqi Cai,Muhammad Bakr Abdelghany,Kyriakos G. Vamvoudakis,Anthony Tzes*

Main category: eess.SY

TL;DR: 该论文提出了一种可调节鲁棒模型预测控制（MPC）的在线鲁棒性储备消耗评估方法，无需立即重新求解完整优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理超出预设集合（out-of-set）的扰动时，往往需要重新求解昂贵的MPC优化问题，效率较低。

Method: 通过独立的原始和对偶修正层次结构构建双侧储备消耗包络（reserve-depletion envelope），并结合基于基准的自适应算法实现随时（anytime）求解。

Result: 该方法在保持鲁棒性证书有效性的前提下，实现了高达4.97倍的平均速度提升，且具有在线可计算的精度界限。

Conclusion: 所提算法提供了一种高效的在线鲁棒性实时监测与修正机制，能够有效降低MPC在处理突发扰动时的计算开销。
...
### [266] [Managing Inherent Risk: On the Conceptualization of Risk in Defense Systems](https://arxiv.org/abs/2608.28093)
*NIklas Braun,Leon J. Brettin,Marvin Loba,Markus Maurer*

Main category: eess.SY

TL;DR: 本文探讨了民用环境中防御系统的特殊风险管理需求，提出了一种结合民用安全工程与军事防御逻辑的综合风险评估框架。


<details>
  <summary>Details</summary>
Motivation: 防御系统在民用环境中的部署面临社会接受度难题，且其风险结构与旨在规避伤害的纯民用安全工程存在显著差异。

Method: 通过对比民用与军事领域现有的安全标准与风险概念，分析防御系统在物理安全威胁下的复杂风险构成。

Result: 提出了一个包含外部物理安全威胁在内的扩展型风险框架，指出防御系统的设计必须平衡“造成伤害的功能性”与“民用安全性”。

Conclusion: 将防御系统的复杂风险结构纳入公共交流是化解公众顾虑、促进社会对于国防设施建设开展理性讨论的有效路径。
...
### [267] [Distributed Model Predictive Control for Optimal Consensus of Constrained Heterogeneous Multi-agent Systems](https://arxiv.org/abs/2608.28180)
*Nan Bai,Tao Liu,Qishao Wang,Zhisheng Duan*

Main category: eess.SY

TL;DR: 该论文提出了一种针对受限异构多智能体系统的分布式模型预测控制（MPC）方案，通过协同优化控制输入和共识均衡点来实现最优共识。


<details>
  <summary>Details</summary>
Motivation: 解决异构多智能体系统在存在约束条件下的共识性能优化问题，克服传统分布式控制难以兼顾输入约束与动态共识均衡的局限。

Method: 结合分布式模型预测控制（MPC）框架，构建并求解耦合约束优化问题；利用分布式原始-对偶算法（Primal-Dual Algorithm）进行求解，并推导了保证收敛的局部验证条件。

Result: 提出了能够保证闭环系统递归可行性和渐近共识的终端条件，并通过数值模拟验证了该方法在处理受限异构系统共识控制方面的有效性。

Conclusion: 该MPC框架成功实现了对控制序列和共识均衡的联合优化，为受限异构多智能体系统的分布式协同控制提供了一种高效、稳健的理论方案。
...
### [268] [Hierarchical Agglomerative Clustering for Efficient Annual Voltage Security Assessment in Very-High RES Penetrated Power Systems](https://arxiv.org/abs/2608.28296)
*Rock Agon,Robin Preece,Jovica V. Milanovic*

Main category: eess.SY

TL;DR: 提出了一种基于电压响应特征的电力系统代表性运行点（ROP）选择框架，以高效评估高比例可再生能源下的电压安全性。


<details>
  <summary>Details</summary>
Motivation: 传统基于需求和发电功率曲线的聚类方法无法准确反映电力系统复杂的非线性电压行为，且全年度仿真计算成本过高。

Method: 利用系统电压风险指数代表运行点，通过主成分分析（PCA）降维，并结合层级聚合聚类（HAC）识别代表性电压工况。

Result: 在IEEE测试系统中，该方法将年度运行点数量减少了99.66%，并实现了98.3%（稳态）和93.4%（故障后）的电压行为重构精度。

Conclusion: 该框架通过直接从电压响应维度进行聚类，比传统基于注入功率空间的聚类方法更有效且更具物理意义，能显著提升大规模可再生能源接入下的电压安全评估效率。
...
### [269] [SafeLink-Agent: Agentic Maintenance for Adaptive Bitrate Controllers over Dynamic Starlink Networks](https://arxiv.org/abs/2608.28194)
*Hongjun Xie,Bowen Zhang,Genke Yang,Pengcheng Luo*

Main category: eess.SY

TL;DR: SafeLink-Agent 是一种用于低地球轨道 (LEO) 卫星网络（如 Starlink）自适应码率 (ABR) 控制器的智能维护框架，通过 LLM 自动修复策略以应对不稳定的网络环境。


<details>
  <summary>Details</summary>
Motivation: 现有的 ABR 控制器难以适应 Starlink 网络中动态变化的吞吐量、延迟和切换条件，导致在特定网络场景下表现不佳。

Method: 提出 SafeLink-Agent 框架，利用大语言模型 (LLM) 总结故障并生成补丁，配合回放验证机制，确保修复方案在部署前是安全可靠的。

Result: 实验表明，该框架显著降低了 RobustMPC 和学习型控制器的严重会话率，RobustMPC 的严重会话比例从 2.60% 降至 0.40%，学习型控制器从 39.01% 降至 9.79%。

Conclusion: 基于代理的维护机制能够有效提升 ABR 控制器在复杂、动态卫星网络条件下的稳健性。
...
### [270] [Scalable Voltage-Stability Dataset Generation Via Boundary-Proximity Indicators Clustering](https://arxiv.org/abs/2608.28298)
*Rock Agon,Robin Preece,Jovica V. Milanović*

Main category: eess.SY

TL;DR: 提出了一种基于分层聚类和边界敏感指标的框架，通过显著减少连续潮流计算（CPF）的数量，高效生成电压稳定性评估所需的训练数据集。


<details>
  <summary>Details</summary>
Motivation: 电压稳定性规划中机器学习代理模型的训练需要大量CPF计算，但其计算成本极高，限制了大规模应用。

Method: 结合了Hit-and-Run MCMC均匀采样、Maximin拉丁超立方采样、弱总线敏感度扰动，以及基于聚类代表点重构电压稳定裕度的技术。

Result: 在IEEE 39节点系统上的测试显示，该框架在保持高预测精度和边界保真度的前提下，减少了95.45%的CPF计算量。

Conclusion: 该方法通过少量代表性样本实现了高精度的代理模型训练，验证了其在电力系统规划与运行中的可扩展性和有效性。
...
### [271] [Operator-Theoretic Stability and Observer Synthesis for Parameter-Dependent Vlasov--Maxwell Dynamics](https://arxiv.org/abs/2608.28349)
*Amadou Cissé,Mohamed Boutayeb*

Main category: eess.SY

TL;DR: 该研究利用算子理论为Vlasov-Maxwell系统构建了参数依赖的控制器和观测器，实现了等离子体动力学的稳定控制与干扰抑制。


<details>
  <summary>Details</summary>
Motivation: Vlasov-Maxwell系统在等离子体物理中至关重要，但由于其复杂的耦合非自治演化特性，控制合成面临严峻挑战，亟需一种数学严谨且具有数值可实现性的合成框架。

Method: 采用算子理论建模非自治演化系统，利用参数依赖的Lyapunov算子推导算子微分线性矩阵不等式（LMIs），并结合$H_\infty$控制理论和Galerkin投影进行有限维近似。

Result: 确立了系统算子的适定性及一致增长界，通过有限维LMIs实现了控制器的可靠数值合成，并在Vlasov-Maxwell基准测试中验证了其稳定性和观测器收敛性。

Conclusion: 该框架成功将算子理论与控制合成相结合，能够在保持原始物理模型分析结构的同时，为复杂的耦合等离子体系统提供有效的数值控制方案。
...
### [272] [LUCID: An Agentic AI Framework on Digital-Twin in the Loop for QoS-Guaranteeing Robotic Control](https://arxiv.org/abs/2608.28437)
*Hyeonsu Lyu,Minwoo Kim,Sehyun Ryu,Hyun Jong Yang*

Main category: eess.SY

TL;DR: LUCID 是一个由 LLM 代理驱动的云机器人框架，通过动态调整轨迹规划（TP）和无线资源管理（RRM）的优化方案，实现对动态环境和多变任务需求的自适应。


<details>
  <summary>Details</summary>
Motivation: 传统 TP-RRM 方法将两者视为固定优化问题，难以应对动态环境中不断变化的机器人数量、QoS 要求及复杂的无线干扰，导致服务质量下降且缺乏灵活性。

Method: 利用 LLM 代理根据操作员意图动态配置优化问题的约束与目标；通过 SimBridge 将大型机器人场景转化为无线数字孪生（DT）；引入 FastConfigNet 多模态代理模型降低计算延迟。

Result: 实验表明 LUCID 能稳健应对变化的场景、机器人数量和操作员意图，并通过高效的验证机制解决了无线瓶颈，显著降低了规划延迟。

Conclusion: 通过将 TP-RRM 从固定的数学问题转化为动态可编排的方案，LUCID 有效提升了云机器人在复杂环境下的实时协同能力与适应性。
...
### [273] [Targeted Power System Frequency Attack via the Selection of Maliciously Controlled Inverters](https://arxiv.org/abs/2608.28533)
*Xiangyu Zou,Betelihem Ashebo,Daniel K. Molzahn*

Main category: eess.SY

TL;DR: 该研究探讨了如何通过选择逆变器资源（IBRs）作为恶意控制节点来攻击电力系统频率稳定性。


<details>
  <summary>Details</summary>
Motivation: 随着逆变器资源渗透率提高，电力系统面临新的网络攻击风险，需要评估攻击者如何通过特定节点组合破坏系统稳定性。

Method: 通过引入不稳定振荡模态并将其形式化为混合整数二次约束规划（MIQCP）问题，提出了两种基于排序指标的启发式算法来确定最优攻击节点集。

Result: 在WSCC 179节点和ACTIVSg500系统中验证了该方法，结果显示启发式算法能在大幅降低计算时间的同时，在多数场景下找到最优攻击集。

Conclusion: 研究揭示了通过巧妙选择受损节点可以诱发系统不稳定，并分析了特征值、攻击目标数量对攻击严重程度的影响，为防御此类攻击提供了参考。
...
### [274] [Destroy Me: Automatic Artifact Generation for Histopathology Images](https://arxiv.org/abs/2608.27516)
...
TL;DR: 提出了一种名为“Destroy Me”的混合框架，通过真实的人工伪影合成和鲁棒性数据增强，提升深度学习模型在病理诊断中的抗干扰能力。


<details>
  <summary>Details</summary>
Motivation: 现有的病理诊断模型通常依赖“完美数据”，通过过滤低质量区域会导致诊断背景丢失，且模型在真实环境下的噪声鲁棒性较差。

Method: 结合微调后的稳定扩散模型（Stable Diffusion）与基于物理的程序建模，合成组织褶皱、沉淀、模糊等六种常见伪影；并利用影响加权增强策略训练nnU-Net模型。

Result: 在肺腺癌分类任务中，该方法使宏观F1得分相对提升10.5%，Cohen's Kappa系数相对提升15%。

Conclusion: 在训练中引入可控的伪影增强能显著提升模型的现实鲁棒性，同时平衡了诊断特征的保留与模型性能。
...
### [275] [Do Medical Vision Models Reason About Anatomy? Probing the Spatial Inductive Biases of Learned Visual Representations](https://arxiv.org/abs/2608.28092)
*Naren Akash,Neeraja Ramanan*

Main category: eess.IV

TL;DR: 本文提出了SPAR-Bench，这是一套针对腹部CT扫描的评估基准，用于衡量医疗视觉编码器在坐标定位、关系推理和空间查询方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有医疗视觉编码器多通过诊断准确率评估，导致难以量化其对解剖结构的空间理解能力，且不清楚其表现是基于实际推理还是对经典解剖学的记忆。

Method: 构建包含八项任务的SPAR-Bench基准，在不同架构和预训练模型上进行测试，并对比了全序列令牌与池化头部（pooled head）在提取空间特征时的表现。

Result: 医疗视觉编码器在跨切片比较任务上表现极差，且其在特定数据集上的高分主要依赖于对解剖位置的记忆而非视觉推理；此外，池化探针严重低估了特征空间所包含的信息量。

Conclusion: 现有的医疗视觉编码器主要充当“解剖地图”的存储器，缺乏针对特定患者结构进行空间比较和推理的核心机制。
...
### [276] [CARDINAL Predicts Cardiovascular Risk From Non-contrast Cardiac CT](https://arxiv.org/abs/2608.27690)
...
TL;DR: CARDINAL 是一个基于非造影心脏 CT 的深度学习框架，通过学习解剖潜在嵌入来预测重大心血管不良事件 (MACE)。


<details>
  <summary>Details</summary>
Motivation: 传统心血管风险评估依赖于不完整的数据或人工设计的影像特征，无法充分挖掘 CT 扫描中的预测潜力。

Method: 开发了 CARDINAL 模型，该模型利用深度学习从 17,659 名患者的常规非造影心脏 CT 中学习紧凑的解剖特征表示，并与传统临床评分（PCE、PREVENT）、冠脉钙化分数 (CAC) 及放射组学方法进行了对比。

Result: 在 10 年期预测中，CARDINAL 的 AUROC 达到 0.866，AUPRC 达到 0.890，显著优于传统的放射组学特征和其他基准模型，且在生存分析中展现出更高的区分度。

Conclusion: 非造影心脏 CT 包含超出传统评分和现有生物标志物的丰富预后信息，CARDINAL 框架能有效提升对长期心血管事件的预测能力。
...
### [277] [CheXtriev: Anatomy-Centered Representation for Case-Based Retrieval of Chest Radiographs](https://arxiv.org/abs/2608.28137)
*Naren Akash,Arihanth Tadanki,Jayanthi Sivaswamy*

Main category: eess.IV

TL;DR: 提出了一种基于图的、具有解剖学感知能力的胸部X光图像检索框架CheXtriev。


<details>
  <summary>Details</summary>
Motivation: 现有方法多关注全局特征，忽略了局部解剖区域的细粒度特征以及病灶与解剖位置之间的空间关联。

Method: 利用图Transformer从特定解剖区域提取特征，并结合解剖学证据建模空间上下文和病灶交互。

Result: 在检索准确率上超越现有方法18%-26%，在排序质量上提升11%-23%，特别是在稀有病灶的检索上表现更佳。

Conclusion: 该框架通过引入解剖学感知特征，显著提升了胸部X光图像检索的有效性与效率。
