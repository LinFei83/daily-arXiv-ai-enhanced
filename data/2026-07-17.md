...
TL;DR: 提出了面向无人集群搜救任务的三层分层学习架构，整合Hebbian神经可塑性、多智能体强化学习与元学习，通过22个架构契约提供6类形式化保证，并引入群体元认知作为涌现属性。


<details>
  <summary>Details</summary>
Motivation: 现有分层强化学习在搜救任务中存在5个根本性局限，需要更鲁棒且具备认知能力的混合架构。

Method: 三层学习机制（反射/技能/推理），结合BDI、行为树、图神经网络、神经可塑性、元学习与数字孪生，通过架构契约与构造性进步函数实现形式化保障。

Result: 集成定理证明满足6类保证（安全性等），动态扩展后新增认知弹性等3项保证；理论分析表明解决了5个现有方法局限。

Conclusion: 混合神经符号系统通过分层认知与契约设计，有效提升无人集群在搜救任务中的自主决策可靠性与适应性。
...
TL;DR: 提出HG-RAG框架，通过层次知识图谱的图遍历来增强大语言模型的检索能力，有效处理结构化和层次化查询。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统在查询需要层次推理或关系推理时表现不佳。

Method: 构建层次知识图谱，检索时先解析查询中的命名实体锚点，然后向上、横向和向下遍历父节点、邻居节点和子节点，以提供结构化的上下文。

Result: 在三种规模（18-800节点）和四种查询类型中，HG-RAG在层次、关系和多跳推理任务上优于平坦检索基线，同时减少幻觉并保持局部连贯性。

Conclusion: HG-RAG能有效改善RAG系统在复杂结构化查询上的表现。
...
TL;DR: 提出了一种名为IMEX的可解释预测模型框架，通过静态相关功率和交互相关功率两个指标识别重要特征及其交互作用，在合成数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 黑盒模型预测准确但缺乏可解释性，在关键应用中仅靠准确性不足，需要揭示模型决策背后的原因。

Method: 基于IMEX算法，使用静态相关功率（PCS）和交互相关功率（PCI）两个互补指标，分别量化个体特征贡献和特征间非加性交互作用，支持高阶交互分析。

Result: 在三个已知结构的合成数据集上与INVASE对比，IMEX能有效恢复非线性、条件性和多重共线性关系下的相关特征结构。

Conclusion: IMEX框架为解释预测模型提供了有效方法，能够识别重要特征及其交互模式，增强模型的可解释性。
...
TL;DR: 提出RegNetAgents，一个基于多智能体的框架，用于在异质性基因调控网络中识别查询驱动的调控候选基因，整合了TCGA和GREmLN的调控网络。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以统一分析不同来源（如bulk肿瘤和单细胞）的调控网络，且缺乏结构化、可解释的候选基因识别流程。

Method: 采用多智能体LangGraph DAG工作流，通过双网络分类、OncoKB癌症基因过滤和moA分配进行候选排序，利用Python API和MCP客户端实现。

Result: 在11个BRCA和12个COAD焦点基因中，识别出的候选基因显著富集OncoKB注释的癌症基因（BRCA/COAD中TCGA来源Z=6.69/6.95，GREmLN来源Z=5.51/7.06，p<0.0001），且对管家基因无富集。

Conclusion: RegNetAgents提供了一个可解释的跨网络调控候选基因识别框架，支持从候选识别到生物假说的端到端解读。
...
TL;DR: 本文提出了一种基于对话推理的地点识别新范式（DlgPR），通过交互式对话处理自然语言描述的歧义和不完整性，并构建了首个大规模对话基准数据集DlgQuest-Cities。


<details>
  <summary>Details</summary>
Motivation: 现有语言引导的地理定位方法依赖静态单次检索，难以处理真实世界自然语言描述的模糊性和不完整性。

Method: 提出DlgPR范式，包含跨模态多级检索器和智能提问器DQ-pilot。DQ-pilot采用课程学习策略：先在DQ-cities-20k子集上进行监督微调，再通过GRPO在更难的数据集上强化学习。使用DDI进行课程采样，PRG作为奖励直接衡量问题对检索性能的提升。

Result: 实验表明，基于推理的方法显著优于基线方法。

Conclusion: 对话驱动的地点识别范式能有效处理语言歧义，提升检索准确性。
...
TL;DR: 提出LLM-T1D方法，结合强化学习与大型语言模型，构建透明可靠的胰岛素泵控制器，在FDA模拟器上实现73.5%时间在目标血糖范围内。


<details>
  <summary>Details</summary>
Motivation: 强化学习驱动的闭环胰岛素系统虽然有效，但其'黑箱'性质导致患者和医生难以信任。

Method: 训练专家强化学习系统，将其知识蒸馏到微调的LLaMA 3.1 8B和Qwen3 8B模型中，生成可解释的决策文本。

Result: LLM控制器在血糖控制上超越强化学习系统（73.5%时间在目标范围），并通过严格形式化安全验证避免幻觉。

Conclusion: 结合强化学习精度与大型语言模型可解释性的方法，能显著提升自动化胰岛素系统的透明度和可靠性。
...
TL;DR: 提出利用大语言模型构建贝叶斯信念网络的新方法，通过AI智能体面板估计概率并应用修剪均值规则降噪，在替代医疗系统顾客咨询意图建模中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有贝叶斯网络构建需依赖专家判断或大数据集，难以兼顾两者优势，亟需新方法平衡专家知识与数据驱动学习。

Method: 采用大语言模型驱动的AI智能体面板模拟不同角色和情境估计概率，结合修剪均值规则去除噪声，提出六步贝叶斯网络框架。

Result: 自我效能感表面重要但因果影响小，主观规范对顾客意图影响更强；同时提升自信与社区规范是最有效策略。

Conclusion: 该方法有效融合专家意见与数据学习，揭示了替代医疗系统中影响顾客意图的关键因素及其相互作用机制。
...
### [8] [ToolAnchor: Anchoring Counterfactual Context to Boost Agentic Tool-use Capability](https://arxiv.org/abs/2607.14145)
*Weiting Liu,Jieyi Bi,Wanqi Zhou,Jianfeng Feng,Yining Ma,Ai Han,Wenlian Lu*

Main category: cs.AI

TL;DR: 提出了ToolAnchor框架，通过反事实锚定语境打破工具增强LLM代理在工具集扩展中的行为惯性，实现动态适应。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在固定工具集上训练，面对新工具时难以有效整合，重新训练不切实际，核心障碍是行为惯性。

Method: 利用教师模型假设反事实语境，通过学生模型执行验证，并将成功干预内化为代理后训练过程。

Result: 在GAIA、BrowseComp和VDR-Bench任务上，ToolAnchor在扩展工具集下持续表现出竞争性能。

Conclusion: 该方法弥合了静态后训练与动态适应之间的差距，为可扩展的代理强化学习提供了新路径。
...
### [9] [Capability from Access Structure, Not Scale: Lower Bounds and Pre-Registered Tests for Hybrid Sequence Models](https://arxiv.org/abs/2607.14144)
*Wenhui Chen,Jianlin Chen,Ziyao Lin,Chi Man Vong*

Main category: cs.AI

TL;DR: 本文提出能力趋同假说（CCH），指出在固定推理预算下，不同架构的表征趋同不代表能力趋同。能力收敛于一类具有压缩状态通道和可扩展索引通道的混合架构，并通过牛顿苹果问题验证了三条资源壁垒及其突破方法。


<details>
  <summary>Details</summary>
Motivation: 现有柏拉图表征假说（PRH）认为模型缩放会导致表征趋同，但未考虑能力层面的差异。因此需要研究在固定推理预算下，不同架构的能力收敛条件及其边界。

Method: 提出能力收敛假说，将理论下界与预先注册实验相结合。理论部分基于信息论和计算复杂性（Shannon壁垒、地平线壁垒、电路壁垒），实验部分采用预先注册的小规模测试，对比不同架构在牛顿苹果问题上的表现。

Result: 测量到剪刀差（精确检索误差0.994 vs 0.000），状态跟踪分叉位于注册边界，连接见证显示不可约的双通道方案；有一项预测失败且方向相反。

Conclusion: 表征趋同由缩放自由产生，能力趋同需要依靠特定访问结构（即双通道混合架构）来购买。
...
TL;DR: 本文提出一种神经符号代理框架，通过工具调用和RGCN专家提示增强小型语言模型在多跳逻辑推理任务中的能力，发现提取瓶颈和顺序推理脆弱性等关键挑战。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型零样本推理成本高且环境不友好，小型语言模型虽可持续但难以处理复杂多跳逻辑推理任务。

Method: 将小型语言模型转化为代理，使用两种工具：extract_facts进行符号三元组提取，get_hint通过RGCN提供专家推理提示。在Oracle和Realistic两种场景下评估Gemma 3和Llama 3.2模型。

Result: RGCN提示相比基线实现1.5-2倍性能提升，但受限于提取瓶颈和顺序推理脆弱性，早期错误会在多跳链中累积；特定架构中噪声自生成事实产生分散效应。

Conclusion: 表征了低资源代理系统中符号接地挑战，为神经符号代理管道的迭代验证提供路线图。
...
TL;DR: 本文探讨了如何将Agentic AI和MCP协议应用于输电系统运营商的电网研究，提出了一个基于MCP的工具接口pypowsybl-mcp，并讨论了人机协同的多智能体工作流与评估策略。


<details>
  <summary>Details</summary>
Motivation: 现有电网研究工具缺乏标准化AI交互接口，难以实现智能体与仿真工具的灵活协作与人工监督。

Method: 开发了pypowsybl-mcp接口，通过MCP协议暴露仿真工具功能；设计多智能体工作流及人机回环评估策略。

Result: 成功构建了测试平台，验证了AI智能体通过标准化工具调用完成仿真设置、分析和结果提取的能力。

Conclusion: MCP工具集成有助于实现更互动、可审计且可扩展的电网研究环境。
...
TL;DR: 提出MemoHarness框架，通过执行经验自适应优化智能体控制层，在多个基准测试中提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体大多使用固定全局控制层，而控制层设计对行为影响显著，需要更自适应的优化方法。

Method: 将控制层分解为六个可编辑维度，利用双层级经验库存储诊断与模式，并通过检索经验自适应调整控制层。

Result: 在shell-agent、代码生成和分析推理基准上，MemoHarness优于固定控制层，并展现出选择性迁移能力。

Conclusion: 执行经验可作为构建自适应智能体控制层的实用基础，超越静态配置，但统计鲁棒性和组件归因需进一步研究。
...
TL;DR: 本文指出，用过渡准确率评估LLM合成的代码世界模型（CWM）来规划是错误的，即使高准确率也可能因遗漏关键规则而失败。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方式（采样轨迹上的过渡准确率）无法保证CWM在规划中的充分性，因为微小错误可能影响关键动态。

Method: 通过LLM合成CWM并利用经典规划器搜索，分析过渡准确率、状态准确率与游戏表现的关系；推导定量法则；对比不同模型和数据增强方法（如DAgger）；测试不完美信息博弈的信念推理函数。

Result: 100%过渡准确率的CWM可能在规划搜索分布上状态准确率≥98%，但游戏中系统性失败（危险系数0.091）；失败遵循定量法则；数据增强无法修复；相同机制在信念推理中出现。

Conclusion: 规划导向世界模型的充分性应基于搜索分布或直接游戏表现衡量，而非预测准确率。
...
TL;DR: 提出了ReasFlow，一个面向推理密集型科学发现的端到端自主代理系统，能够从最小提示自动生成完整的理论论文。


<details>
  <summary>Details</summary>
Motivation: 现有自动化研究系统主要关注经验驱动领域，缺乏对理论驱动发现的支持，特别是在需要严格证明和领域知识综合的数学基础学科中。

Method: ReasFlow包含内部验证循环和自动知识检索与自我改进机制，模拟人类专家与研究生之间的协作范式。

Result: 系统从最小提示自主生成了五篇完整的理论论文，在基于LLM的评审指标上达到了最高分。

Conclusion: ReasFlow在推理密集型科学发现中有效，且通过ReasLab平台公开可用。
...
TL;DR: 提出一种联合语言-视觉推理与想象的具身认知基础模型RxBrain，通过混合规划序列实现高层任务推理与物理状态连接的认知能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型侧重场景理解与文本决策，生成式世界模型主要预测视觉状态，缺乏将语言抽象结构与视觉物理状态联合表征的具身认知能力。

Method: 采用统一多模态Mixture-of-Transformers架构，支持语言、图像、视频的理解与生成；构建自动流水线将具身视频转化为文本-视觉联合规划监督信号。

Result: 在RxBrain-Bench评估中，模型保持具身理解与生成能力，产生耦合文本推理、世界状态预测和联合子目标规划的计划，并在连续机器人动作生成中展现潜力。

Conclusion: 该模型为具身认知基础模型提供了初始步骤，验证了联合语言-视觉规划在具身推理中的互补作用。
...
TL;DR: 本研究评估了五大AI问答引擎对28个冲突的5460个回答，发现记录薄弱的冲突更容易出现幻觉和错误，且易受生成式引擎优化（GEO）导致的信息操控。通过对1048个来源网站的分析，揭示了GEO已开始影响冲突信息，并提出加强本地监测和翻译研究的政策建议。


<details>
  <summary>Details</summary>
Motivation: AI问答引擎日益成为分析师和公众获取和平与冲突信息的主要来源，但大型语言模型可能产生系统性错误。本研究旨在探索这些错误在冲突问题上的模式及其对全球冲突信息环境的启示。

Method: 向五大AI问答引擎提出关于28个冲突的一系列问题，将5460个回答与文献证据进行评分对比；分析引擎引用的1048个网站来源，评估GEO的影响和状态。

Result: 记录越薄的冲突，引擎越容易捏造、错误归属或计数错误。薄记录不仅促进幻觉，还因最易被GEO扭曲而暴露于虚假信息。分析发现GEO源优化已存在，国家级数字操控虽初现但快速增长。

Conclusion: 研究结果对GEO信息战时代的学术研究有重要启示，政策上应回归AI无法复制的深度本地监测和翻译研究，并讨论了该快速变化领域的未来研究机会和挑战。
...
TL;DR: 本文批判了当前对齐方法中的静态和模仿性缺陷，提出了互动性和互补性的对齐视角，并基于跨学科讨论，为开发与人类互动中动态对齐的AI系统制定了研究议程。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法无法捕捉真实世界中人类与AI交互的动态性和情境依赖性，仅满足偏好而非让其涌现。

Method: 通过形式化现有对齐与轨迹级观点的差距，结合跨学科工作坊的见解，借鉴人-人协作的社会科学经验，并分析人-AI系统的放大效应和新挑战。

Result: 明确了人-AI系统引入的新不对称性、不确定性推理困难和协调挑战，并提出了整合机器学习、社会科学和决策科学的研究方向。

Conclusion: 需要从静态模仿转向互动互补的对齐范式，以开发能在交互中与人类共同进化的AI系统。
...
TL;DR: 生成模型的属性调节存在一个由训练数据设定的预算，分为旋钮可达范围和仅示例可达范围，后者通常更大；通过示例而非旋钮，可以跨越整个预算实现更灵活的控制。


<details>
  <summary>Details</summary>
Motivation: 发现生成模型的调节旋钮（如提示、引导尺度）在超过某点后无法继续改变目标属性，这种天花板并非模型缺陷，而是由训练数据的预算决定的。

Method: 通过审计训练数据测量预算，并提出构建示例集的方法，这些示例由模型已学习的知识组合而成，而非新增训练。

Result: 示例方法在图像和晶体结构生成两个领域验证成功，能覆盖全部预算，并实现对难以用语言描述的目标的引导。

Conclusion: 示例方法比旋钮具有更大可达范围和更强表现力，能触及旋钮无法达到的属性部分，并支持仅通过示例指定目标。
...
TL;DR: 提出了一种自动化的智能红队框架，通过多智能体架构合成困难样本，提升多模态大语言模型的安全性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有主动学习与人工标注无法应对复杂多变的多模态威胁。

Method: 迭代策略合成假设与变异；多智能体架构（高推理架构师、图像生成器、多级验证委员会）；测试时检索上下文示例。

Result: 在公开图像安全基准上，假阴性率从41.2%降至24.5%，无需人工标注。

Conclusion: 该自动化红队框架能有效提升模型对边界攻击的鲁棒性。
...
TL;DR: 论文验证了上下文工程质量是AI智能体可靠性的独立领先指标，并通过开源工具ProofAgent-Harness实现了跨七个维度的量化评估。


<details>
  <summary>Details</summary>
Motivation: 上下文工程对构建可靠AI智能体至关重要，但缺乏系统性测量方法，现有评估常将行为表现与上下文质量混为一谈，导致难以诊断失败根本原因。

Method: 提出ProofAgent-Harness评估框架，采用多裁判共识评分机制，从角色清晰度、护栏覆盖、指令一致性、工具模式质量、基础充分性、注入防护和令牌效率七个维度测量上下文质量，并严格隔离上下文分数与行为指标。

Result: 在受控实验中，固定前沿LLM智能体仅改变上下文，发现上下文质量维度一致预测对应行为：基础充分性预测幻觉抵抗、护栏覆盖预测操纵抵抗、指令一致性预测指令遵循、工具模式质量预测工具使用。
...
TL;DR: 研究LLM输出分布与训练数据中经验性下一词分布（ENTD）的关系，发现两者在多数输入上高度一致，但存在长尾差异。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型的行为如何源于训练数据，而非仅学习到的权重。

Method: 对比LLM的下一词分布与训练数据的ENTD，分析缩放规律、架构影响及有限样本噪声。

Result: 平均一致性随模型规模和计算量提升；差异主要来自长尾输入，与架构、训练和估计噪声相关。

Conclusion: 呼吁推进‘数据中心的机制可解释性’，从数据角度理解模型行为。
...
### [22] [Measuring How Students Rely on Generative AI in Academic Writing: Development and Multi-Source Validation of the Generative AI Reliance Types Scale (GenAI-RTS)](https://arxiv.org/abs/2607.14301)
*Shahin Hossain,Tukhbita Afroz Nawmi*

Main category: cs.AI

TL;DR: 开发和验证了生成式AI依赖类型量表（GenAI-RTS），包含五个因子，具有良好的心理测量学属性。


<details>
  <summary>Details</summary>
Motivation: 现有GenAI依赖测量方法归纳性强、针对单一问题解决任务，且样本同质化，缺乏理论基础和异质样本验证。

Method: 基于标准教育心理测试框架，对382名本科生进行问卷调查，结合14名学生的访谈，通过验证性因子分析、Rasch分析等方法进行验证。

Result: 支持五因子结构（策略依赖分为刻意使用和批判评估两个子维度，以及工具性、依赖性和对话性因子），模型拟合良好（CFI=0.92-0.98, RMSEA=0.07-0.08），子量表信度可接受至良好（omega=0.75-0.88），跨性别、第一代大学生身份和STEM/非STEM专业的测量不变性成立。

Conclusion: GenAI-RTS是一个理论驱动、心理测量学验证有效的量表，可用于识别本科生GenAI依赖类型，支持研究、评估和AI素养干预。
...
TL;DR: 本文提出Traccia，一个基于OpenTelemetry构建的多层AI治理栈，通过集成遥测数据、语义护栏评估和执行溯源，解决当前LLM评估和治理工具分散、无法应对状态空间架构威胁的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估与治理平台存在碎片化、无法防范对齐漂移、SaaS安全风险和影子AI系统部署等漏洞，且欧盟AI法案等国际框架的理论要求与实际执行存在显著差距。

Method: 设计Traccia治理栈，利用OpenTelemetry基础设施，通过哈希溯源账本集成遥测数据、被动语义护栏评估和执行血统，自动生成带有防篡改指纹（SHA-256哈希）的合规证据包。

Result: 成功创建了可映射至欧盟AI法案具体条款（第12、14、19、26(6)、50条）的机器可读企业级管理基础，且不侵犯数据隐私。

Conclusion: Traccia通过一体化、可审计的治理方案，弥合了自主AI系统在理论监管要求与实际工程实现之间的最后差距。
...
TL;DR: CIPHER是一个利用测试时扩展的自动化数据科学代理，通过生成和选择多个初始状态进行并行执行，在封闭式和开放式任务上超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理依赖单一初始状态，易因次优初始状态引发级联错误，需提升鲁棒性。

Method: 提出解耦探索-选择（DES）框架，分离候选初始状态生成与策略选择以实现并行执行。

Result: 在匹配模型比较中达到最先进性能，且以较小基础模型与更大模型基线竞争。

Conclusion: DES框架通过生成和选择策略有效提升性能，为实践者提供可操作的设计建议。
...
TL;DR: 本文提出Chat2Scenic，一种迭代式检索增强框架，用于从法规描述自动生成可执行场景脚本，显著提升了编译成功率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从法规描述自动生成自动驾驶测试场景脚本时面临可伸缩性或编译成功率低的问题，需要更有效的解决方案。

Method: 采用迭代式检索增强生成框架，结合聊天机器人界面支持交互式场景精炼，并利用RAG技术将场景生成锚定在法规知识和DSL语法上。

Result: Chat2Scenic在123个场景基准上实现了76.42%的编译成功率和58.17%的框架准确性，大幅优于现有方法（检索组装：30.08% CSR, 11.03% FA；检索全脚本生成：16.26% CSR, 10.86% FA）。

Conclusion: Chat2Scenic通过迭代检索增强和交互式优化，有效解决了从法规生成可执行测试场景的挑战，为自动驾驶测试提供了高效且合规的解决方案。
...
TL;DR: 比较多种机器学习模型预测埃及EGX30股指，发现GRU在中长期表现最优，XGBoost在日预测最佳，集成方法显著提升长期预测效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究多聚焦全球股市，对埃及等发展中国家股市的预测研究不足，需探索适合其独特市场动态的模型。

Method: 使用KNN、随机森林、XGBoost、LSTM和GRU等模型，基于历史EGX30数据，以RMSE、MAPE和R²为评估指标。

Result: GRU在周、月、两个月预测中最佳；XGBoost在日预测中最佳；集成方法在两个月预测中性能提升5倍；KNN在长期预测中表现意外良好。

Conclusion: 不同时间尺度需匹配不同模型：GRU和XGBoost分别适合中长、短期预测，集成学习对长期预测至关重要，KNN在金融科技领域仍有潜力。
...
TL;DR: 本文提出了CatalogAgent，一个基于智能体的系统，通过监督代理解决生成器和评估器的冲突，并利用记忆机制实现自我改进，用于电商目录属性预测。


<details>
  <summary>Details</summary>
Motivation: 电商目录中结构化属性经常缺失，现有基于LLM的生成-评估框架在生成器和评估器产生冲突时效果不佳，需要一种能自我改进的系统。

Method: 使用监督代理调解生成器和评估器之间的内部冲突或来自卖家的外部反馈；通过记忆库和记忆摘要器存储并聚合监督代理的活动模式，再通过上下文工程将学习结果注入工人LLMs。

Result: 生成器和评估器的性能分别提升了15.24%和13.98%。

Conclusion: 监督代理介导的自我学习系统能有效提升生成式AI模型的准确性，为电商目录丰富提供了新范式。
...
TL;DR: 提出使用成对验证器替代传统标量奖励信号，在无需额外标注成本的情况下，实现与完整奖励基线相当的自演化智能体性能。


<details>
  <summary>Details</summary>
Motivation: 传统自演化智能体的每次迭代需要可靠标量奖励信号，而设计该信号依赖领域专家和标注数据，成本高昂。

Method: 用冻结的LLM作为成对验证器，对父代与子代候选智能体进行二元比较判决；集成到三种现有自演化框架中，并提出Adaptive Focus和Soft Elo两种变体。

Result: 在多个智能体任务和两种制品（提示词、代码）上，该方法在多数设置中匹配或超越完整奖励基线，且跨族验证器替换后模式仍然稳定。

Conclusion: 成对验证器可以作为每步奖励设计的即插即用替代方案，在保持任务准确率的同时避免了标注成本。
...
### [29] [Instrument Effects in Language-Model Honesty Evaluation: An Auditable Single-System Demonstration](https://arxiv.org/abs/2607.14399)
*Justin Bronder*

Main category: cs.AI

TL;DR: 本文通过构建文本冒险世界，发现评估语言模型诚实度的工具选择会显著改变测量结果，并提出一种评估工具的四步完整性协议。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型诚实度评估通常将模型输出的判断视为模型本身的反映，但忽略了评估工具（如判决语法、成功标准提示、预算表述等）对模型行为的影响，导致测量偏差。

Method: 设计一个由游戏引擎（而非语言模型）掌握任务可完成性的文本冒险环境，固定模型，系统改变判决语法（二判决 vs 三判决）、成功标准披露、预算表述等工具变量，记录决策规则并执行预先注册分析，同时通过重复运行检查结果稳定性。

Result: 工具选择显著改变模型行为：三判决语法将强断言从38/40降至7/40；披露成功标准将错误假判决从18/59降至0/58；重复运行中3/4实例的判决分布不稳定；预先注册的叙事梯度假设被否定，而预算表述（0.383米）对判决的影响大于叙事内容（0.150灯笼）。

Conclusion: 评估工具本身是行为测量的关键调节因素，必须通过完整性协议（如预设规则、结果前记录、稳定性检查等）来确保评估的有效性，而当前许多评估可能仅反映工具特性而非模型真实诚实度。
...
TL;DR: 提出了CausalGraphX框架，用于通过反事实推理和GNN提供可解释的系统性风险评估。


<details>
  <summary>Details</summary>
Motivation: 传统模型难以捕捉金融网络的复杂非线性动态，且现有GNN缺乏可解释性，无法揭示冲击传播的因果机制。

Method: 结合图注意力机制学习机构脆弱性表示，并引入对抗正则化捕获因果驱动因素；通过优化生成反事实解释。

Result: 在大型合成金融网络上，相比传统和深度学习基线，显著提升级联违约预测性能，并提供稀疏、可行且可操作的反事实解释。

Conclusion: CausalGraphX在预测和可解释性上均优于现有方法，有助于监管机构进行压力测试和干预设计。
...
### [31] [Tactile: Giving Computer-Using Agents Hands and Feet](https://arxiv.org/abs/2607.14443)
*Yong Liu,Zhenyi Zhong,Zhanpeng Shi*

Main category: cs.AI

TL;DR: 提出了一个名为Tactile的开源工具层，通过融合多源UI证据（系统语义、OCR文本、视觉区域），将动作执行转化为可验证的语义状态，显著提升了桌面代理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用代理依赖截图坐标进行点击，缺乏对目标状态和动作结果的明确验证，导致操作模糊、不可靠。

Method: 构建观察-验证-动作循环，优先使用原生语义动作，必要时回退到OCR坐标，并通过可执行属性和验证线索实现状态追踪。

Result: 在macOSWorld任务中，Codex Success@100从41.1%提升至50.0%；在96任务跨代理测试中，Claude Code等模型均获一致改进。

Conclusion: 可靠计算机使用不仅需要更强模型，更需要将软件动作暴露为可验证、可审计的语义对象，而非匿名屏幕坐标。
...
TL;DR: 深度递归Transformer在135M规模模型上，每令牌状态会收敛至固定点，但收敛深度因令牌类型而异（空白最浅，内容词最深）。利用这种非均匀性，一种无需训练的停止规则可在平均4.94个循环达到均匀深度8的质量（降低38%深度），而学习型路由器无法获得收益。


<details>
  <summary>Details</summary>
Motivation: 探究深度递归Transformer在每个令牌上的实际计算行为，特别是收敛的非均匀性及其可读性。

Method: 训练135M参数的深度递归Transformer，随机化递归次数，直接测量每令牌的KL散度和状态变化，设计基于输出稳定性的无训练停止规则，并对比学习型线性路由器。

Result: 状态收敛至固定点（KL散度从3.9e-1降至8.5e-6）；约10%令牌在训练平均深度8时仍更新；无训练停止规则以4.94平均深度匹配均匀深度8的性能，而学习型路由器无改进。

Conclusion: 深度递归Transformer中令牌级收敛的差异可利用于高效推理，无需额外训练，但结果限于单一规模和随机种子。
...
TL;DR: 通过收集57K细粒度人类偏好数据，对LLM代理进行步骤级偏好学习，显著提升了模拟保真度、协调性和社交行为效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的生成式代理在模拟人类行为时缺乏中间步骤（如规划、记忆检索等）的细粒度人类标注，且未在人类偏好上对齐。

Method: 构建交互模拟接口收集57K步级人类偏好数据，使用监督微调（SFT）和直接偏好优化（DPO）在开放权重语言模型上进行步骤级偏好学习。

Result: 步骤级偏好学习持续提升了模拟保真度、协调性和交互质量，诱导出更有效的社会行为。

Conclusion: 步骤级人类监督是提升代理局部决策质量和长期行为效果的有效训练信号。
...
TL;DR: 提出SAGA框架，通过模式感知的约束优化智能代理的SPARQL查询生成，减少空结果查询并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有KBQA智能代理在语义解析中采用类型盲目的实体关系筛选，导致搜索空间过大且生成语义不匹配的查询，需要更系统的模式感知方法。

Method: SAGA框架：维护双向类型状态，在构建时过滤不兼容属性，使用紧凑模式注释格式展示图模式，并基于经验及痕迹证据宽容处理缺失模式信息。

Result: 在9个基准设置中全部取得最高F1，8个设置取得最高精确匹配准确率，并在所有Wikidata设置中减少空结果查询。

Conclusion: 模式感知的约束能有效提升Text-to-SPARQL生成的准确性和鲁棒性，减少冗余检索。
...
TL;DR: 提出了CEDI框架，通过动态多轮交互评估多模态大模型，揭示出比静态评估更多且更接近实际应用的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在静态受控环境与真实世界动态交互场景之间存在根本性错位，无法反映模型实际有效性。

Method: 构建三阶段交互框架，使用图表示指导半结构化对话，通过状态空间转换实施澄清请求、对抗探测等策略评估模型。

Result: 发现上下文交互评估能揭示更多幻觉，且幻觉在长上下文和自我强化对话历史中累积，模型对前提拒绝类问题尤为脆弱。

Conclusion: CEDI提供了一个更真实、系统且生态有效的评估方法，是改进MLLM评估的重要一步。
...
TL;DR: 提出了VLT，一种多模态基础模型，联合建模时间序列、频谱视觉表示和文本知识，通过频谱作为视觉桥梁，解决连续时间信号与离散语义的融合问题。


<details>
  <summary>Details</summary>
Motivation: 现有工业时间序列方法局限于单模态建模，复杂场景下泛化能力不足，且连续信号与离散文本的跨模态学习存在挑战。

Method: 设计Time-MoE捕获异质时间动态，Frequency-Text Augmented Learner实现频谱与语义联合建模，时间中心梯度对齐机制缓解跨模态优化冲突。

Result: 在多个工业数据集上，VLT优于现有方法，在少样本、噪声和不完整模态场景下展现出卓越鲁棒性和泛化性。

Conclusion: VLT通过多模态融合有效提升了工业时间序列预测的可靠性，尤其在复杂条件下表现突出。
...
TL;DR: RetroAgent是一个LLM代理，通过结构化记忆和化学工具结合符号搜索与神经推理，在多步逆合成规划中实现强性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统多步逆合成方法使用离线训练的价值网络孤立评分候选方案，缺乏对完整多步路线的推理；而现有LLM方法依赖简单接口，限制了搜索空间的全面探索。

Method: 提出RetroAgent，利用LLM代理与结构化记忆和化学工具集成，使代理能够观察完整搜索状态（包括已探索路径、可用替代方案和中间体属性），从而基于全局进展和领域知识做出决策。

Result: 在分布内和分布外基准测试中，RetroAgent均展现出强劲的性能和良好的泛化能力。

Conclusion: RetroAgent通过符号搜索与神经推理的桥接，有效改善了多步逆合成规划的效率和效果。
...
TL;DR: 本研究提出了WrAFT，一种写作评估与反馈工具，能够在议论文中提供准确可靠的评分和有效的综合反馈。


<details>
  <summary>Details</summary>
Motivation: 现有自动写作评估工具在评分准确性和反馈有效性方面存在不足，亟需改进。

Method: 采用模块化设计，将任务分为评分、表层反馈和深层反馈；评估了多种大语言模型（如LLaMA-3.3-70B-Instruct、GPT-4o和Claude 3.7），并使用直接提示和监督微调方法；利用480篇托福独立写作论文的基准数据集进行训练。

Result: 在评分方面达到最优性能（QWK=0.84，RMSE=0.44）；反馈获得高认可度（表层：96.14%，深层宏观：93.03%，深层微观：94.69%）。

Conclusion: WrAFT在自动写作评估和反馈方面表现优异，且其交互界面已公开免费使用。
...
### [39] [Towards an Intention Abstraction Layer for Autonomous Industrial Systems](https://arxiv.org/abs/2607.14553)
*Artan Markaj,Raphael Höfer,Felix Gehlhoff*

Main category: cs.AI

TL;DR: 提出意图抽象层（IAL），将自然语言目标转换为结构化意图，并在执行前检测冲突，实现从事后故障分析到执行前意图级检查的转变。


<details>
  <summary>Details</summary>
Motivation: 现代工业环境中多个自主子系统共享资源但目标可能冲突，传统方法在冲突发生后才发现问题。

Method: IAL中间件：基于OWL本体论的大语言模型解析自然语言目标，一致性监视器在注册时检测冲突，透明度模块用自然语言解释冲突。

Result: 在双代理冲突场景中，IAL成功标记并解释了生产与能源意图冲突。

Conclusion: IAL提供了一种机制，将协同自主系统的行为保证从事后分析转移到执行前意图级检查。
...
### [40] [Are LLM-Generated GPU Kernels Production-Ready? A Trace-Driven Benchmark and Optimization Agent](https://arxiv.org/abs/2607.14541)
...
TL;DR: 提出Atrex-Bench基准，直接从生产推理轨迹中采样30个算子和440种形状，并发布Atrex-Kernel-Agent代理以优化内核性能，弥合生成内核与生产环境之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有GPU内核生成基准依赖合成或筛选的问题，偏离实际部署工作负载，导致模型在真实场景中性能不佳。

Method: 从全集群生产推理轨迹采样算子和形状，赋予重要性权重和罗浮线上限；提出Atrex-Kernel-Agent，包含迭代测量-修订搜索、优化丢弃和分层GPU优化知识库。

Result: 最佳基础模型仅达到硬件罗浮线的约10%；Atrex-Kernel-Agent将零FlyDSL回退转化为匹配或超越手工调优生产基线的真实内核。

Conclusion: 现有模型在真实生产算子中性能不足，而Atrex-Kernel-Agent通过轮廓驱动的优化有效提升了内核性能。
...
TL;DR: 本文提出Seer方法，通过一次性截断冗余[EOS]令牌，加速扩散多模态大语言模型推理，最高实现31倍吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型因固定长度生成导致大量冗余计算，效率低下。

Method: 基于首步MLP激活稀疏性检测语义边界，利用信噪比准则一次性截断冗余后缀，并采用混合执行策略适应动态序列长度。

Result: 吞吐量提升约31倍，在9个基准测试中保持性能，复杂视觉任务准确率提升（如DocVQA从63.52升至63.66）。

Conclusion: Seer是一种高效、即插即用的训练无关加速方案，有效消除填充浪费并抑制噪声泄漏。
...
TL;DR: 本文研究了AI开发代理在修改基础设施时可能引入的安全风险，提出了一种基于信息流图（IFG）的监控方法，通过分析代码的结构变更来检测隐蔽攻击。


<details>
  <summary>Details</summary>
Motivation: 现有监控方法需要复杂的学习模型和资源，不适合资源有限的用户；基础设施即代码场景下的隐蔽破坏风险未被充分研究。

Method: 采用信息流图（IFG）监控，结合控制流图和数据流图差异分析以及原始代码差异，可同步或异步部署。

Result: 未训练的IFG监控在异步模式下将漏检率从11.6%降至3.5%；同步模式下将联合攻击成功率从74.4%降至0.0%，且不影响合法任务。

Conclusion: 未训练的结构监控方法为组织提供了实用、可审计的AI代理部署安全方案，有助于降低高级AI代理的采用风险。
...
TL;DR: 提出了Alipay-PIBench基准，用于评估编码智能体在支付宝支付集成任务上的表现，包含9个产品和18个任务实例。


<details>
  <summary>Details</summary>
Motivation: 支付集成是复杂的仓库级软件任务，现有基准无法充分评估编码智能体在此类任务上的能力。

Method: 构建包含基本功能补全和高级风险感知的基准，使用确定性静态测试、单元测试、集成测试和端到端检查，并结合LLM辅助评估语义要求。

Result: 在with-skill条件下，平均通过率（RPR）为68.58%至91.37%；使用技能可平均提升10.31%的RPR。
...
TL;DR: 本研究通过大型语言模型代理在Mason-Watts实验中的表现，发现随机化首轮选择指令能显著提升网络效率效应，且贝叶斯优化代理表现优于LLM代理。


<details>
  <summary>Details</summary>
Motivation: 探究网络效率对人类群体在集体问题解决中探索-利用权衡的影响，并扩展至LLM代理群体。

Method: 使用16个LLM代理在8种Mason-Watts网络拓扑上执行任务，并开发贝叶斯优化代理进行对比。

Result: LLM代理在随机化首轮选择时表现显著网络效率效应；贝叶斯优化代理获得更高收益；指令优化使集体收益提升远超网络拓扑差异。

Conclusion: 首轮随机化指令可增强LLM代理的网络效率效应，但贝叶斯优化代理在当前空间搜索任务中表现更优。
...
TL;DR: 提出一种利用多个LLM协作生成和校验报告的新方法，创建脑肿瘤3D MRI-文本数据集，并构建视觉语言模型，在报告生成和视觉问答任务上优于现有2D和3D方法。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型主要针对2D图像，缺乏3D成像-文本配对数据，难以扩展到3D领域。

Method: 使用多个LLM协作系统生成和校验报告，构建3D MRI-文本数据集，并通过将MRI扫描转换为token并与文本指令对齐来训练视觉语言模型。

Result: 在报告生成和视觉问答任务上优于2D和3D基线方法。
...
### [46] [Action QFormer: Structured Representation Shaping under Action Supervision in Vision-Language-Action Models](https://arxiv.org/abs/2607.14635)
*Yufeng Ji,Wenhao Tang,Haoyi Niu,Koushil Sreenath,Yi Wu,Zhongyu Li*

Main category: cs.AI

TL;DR: 本文提出Action QFormer，一种查询驱动的动作接口，通过重构多模态表征来平衡动作监督的双重效应，显著提升VLA模型的零样本导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型将动作监督作为下游目标，导致继承的多模态表征在动作兼容性提升的同时，语言处理和对象定位能力被破坏。

Method: 设计Action QFormer模块，使用指令条件化查询从继承的多模态信息中重构面向动作的表征，再输入下游动作生成器。

Result: 零样本仿真-真实导航任务中，闭环成功率从18.8%提升至56.3%，固定指令动作正确率从22.5%提升至75.5%，几乎消除分布外指令生成。

Conclusion: 提升VLA性能不仅需要更强的预训练骨干网络，还需设计更好的机制来选择和组织多模态信息，并控制动作监督对表征的塑造方式。
...
TL;DR: 提出MathCoPilot，一种人机协同的数学定理证明系统，通过交互蓝图、自动证明编排和论文自动形式化，实现数学家与AI的协作证明。


<details>
  <summary>Details</summary>
Motivation: 现有LLM定理证明器只能作为自主代理证明给定命题，缺乏与人类数学家的深度交互和指导。

Method: 交互式工作台（可检查/指导证明步骤）、自适应知识库搜索与Lean集成验证、论文检索与自动形式化。

Result: 在本科生级别问题上有高成功率，但领域特定定理仍需真正数学理解；比较了Gemini 3.1 Pro、GPT-5.4、Claude Opus 4.7等模型。

Conclusion: 当前模型在有利自动形式化条件下表现良好，但深度数学理解仍是挑战，MathCoPilot范式有效支持人机协作。
...
TL;DR: 提出SportD基准，评估VLM在足球场景中基于价值的决策能力，发现VLM决策显著次优，倾向于低风险低回报动作。


<details>
  <summary>Details</summary>
Motivation: 研究VLM能否利用视觉信息做出策略有效决策，而非仅理解场景。

Method: 基于2022年世界杯478个实时决策，使用控球价值模型计算每个动作的预期进球概率，比较VLM与球员的决策质量和估值损失。

Result: 最佳VLM在31.4%的事件中选择最优动作（球员为38.9%），VLM整体遗憾更高，且偏好低方差、低回报动作（少射门、少渐进传球）。

Conclusion: VLM表现出部分模仿人类动作而非系统评估反事实替代方案，SportD提供价值导向的物理策略推理测试平台。
...
### [49] [MCPEvol-Bench: Benchmarking LLM Agent Performance Across Dynamic Evolutions of MCP Servers](https://arxiv.org/abs/2607.14642)
*Huanxi Liu,Kun Hu,Jiaqi Liao,Qiang Wang,Pengfei Qian,YuanZhao Zhai,Dawei Feng,Bo Ding,Huaimin Wang*

Main category: cs.AI

TL;DR: 提出了MCPEvol-Bench基准，用于评估LLM代理在动态工具集演化下的任务解决能力，发现前沿模型在演化工具中性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准忽略了MCP服务器中工具接口和功能的持续演变，导致对代理适应性的评估有缺陷。

Method: 基于大规模实证研究，设计了11种突变算子，在123个MCP服务器中模拟工具的真实演化，并评估了12个LLM。

Result: GPT-5.4和Claude-Sonnet-4-6在演化的MCP服务器中性能分别下降13.7%和14.4%，计划和推理错误显著增加。

Conclusion: LLM驱动的工作流在动态工具环境中存在脆弱性，MCPEvol-Bench可作为评估代理适应性的标准基准。
...
TL;DR: 本文提出一种非贪婪、风险敏感的解析溯因分析框架，利用κ-τ机制和因果簇结构，使候选解释因素共存互动，直至满足治理条件才形成结论。


<details>
  <summary>Details</summary>
Motivation: 现有溯因推理多偏向合成模式，缺乏对复杂观测状态下潜在因素交互作用的系统分析，且易陷入早熟收敛。

Method: 建立κ-τ框架：κ编码假设间认知交互，τ设定基于决策风险的承诺阈值。核心概念为因果簇，记录因素参与分解的权重与交互结构，并采用双层架构（簇内κ*、簇间κ**）防止因果误判。

Result: 在流行病危机分解和网络威胁分析中展示框架有效性，其关键贡献是将暂停分解作为协调对象，提升人机推理可读性，抵抗过早收敛。

Conclusion: 决策者获得的不再是单一答案，而是加权的情景与消歧证据，即使歧义未解也能采取合理行动。
...
TL;DR: 提出一个自演化的拓扑框架TopoAgent，通过有向无环图组织和动态原子分裂，解决多模态大模型在科学推理中的语义错位和长上下文幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs基于线性规划的方法存在视觉-语义错位、长上下文幻觉以及固定任务粒度下的脆弱执行问题。

Method: 前端分解器将复杂查询分解为视觉原子，按依赖关系组织成有向无环图，并引入自适应原子分裂在运行时动态细分瓶颈节点。

Result: 在数学、物理、化学基准测试中，TopoAgent显著优于最先进的线性智能体框架。

Conclusion: TopoAgent提供了一种鲁棒、抗噪且自纠正的科学推理范式。
...
### [52] [Project Kaleidoscope: Contextual, Human-Aligned Evaluation for Real-World AI Applications](https://arxiv.org/abs/2607.14673)
*Leanne Tan,Rohan Jaggi,Shaun Khoo,Roy Ka-Wei Lee*

Main category: cs.AI

TL;DR: 提出Kaleidoscope，一个结合人物驱动的测试生成、上下文相关评分标准和人工审核的集成工作流，用于可靠的自动化评分。


<details>
  <summary>Details</summary>
Motivation: 现有公共基准测试难以匹配实际应用场景，人工评估难以规模化，尤其是当AI应用需满足本地政策和治理要求时。

Method: Kaleidoscope工作流：基于人物画像生成测试用例；使用应用特定评分标准；人工提供可审查标签；仅当LLM法官与标签一致性达到设定阈值时，才自动评分。

Result: 在为期三周的试点中，覆盖四个组织用例和108个标注问答对（四个领域、14个评估维度），展示了端到端可靠自动化评分的有用功能。

Conclusion: Kaleidoscope是一种实用、可检查、迭代的产品评估工作流，可提升自动化评分的可靠性。
...
TL;DR: SmartRAG是一个完全在设备上运行的框架，通过四个协调模块（感知、记忆、聚焦、思考）和可增量学习的命名实体识别器EvoNER，结合知识图谱MRGraph，实现高效、低成本的智能助手，在移动设备上达到与大型模型相当的多跳推理性能。


<details>
  <summary>Details</summary>
Motivation: 在移动设备上部署大语言模型作为个人助手时，面临隐私、低延迟和离线可用需求与边缘硬件预算限制之间的冲突，仅靠模型压缩无法解决。

Method: 提出SmartRAG框架，包含四个模块：Perception、Memory、Focus、Thinking；核心是EvoNER，通过教师蒸馏增量扩展标签库；知识存储在MRGraph（三层溯源知识图谱）中；检索时使用混合管道（图遍历、词法匹配、稠密语义搜索）；仅对高价值语义操作调用设备上的LLM。

Result: 在四个QA基准测试中，使用量化1.7B参数的SmartRAG实现了多跳推理性能，与高达18倍大的模型竞争，且完全在商用智能手机上运行，满足实际内存和延迟限制。

Conclusion: SmartRAG通过在设备上分解功能角色和智能调度资源，有效解决了边缘部署大语言模型的挑战，实现了可竞争的推理性能。
...
TL;DR: 提出了Perception-RFT框架，通过直接对齐视觉特征与结构化定位输出，无需中间推理步骤，显著提升了多模态文档问答的效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在显式视觉定位方面存在数据需求大、优化困难或推理成本高的问题，需要更高效的训练方法。

Method: 使用Group Relative Policy Optimization (GRPO) 直接训练视觉特征与定位输出的对齐，并构建了带推理的变体进行对比。

Result: 基于感知的训练减少了60%以上的推理token长度，发现推理模型会抑制推理痕迹，并识别出Grounding Divergence现象。早期SFT转RL仅用65%数据即可达到类似精度。

Conclusion: 感知优先的训练在多模态文档问答中优于基于推理的强化学习，且存在新的优化权衡现象。
...
TL;DR: InCarEmo是一个多模态数据集，集成了RGB和红外视频、车内音频和对话文本，用于车内情感识别、疲劳检测和分心监测，支持跨语言评估。


<details>
  <summary>Details</summary>
Motivation: 现有车内情感计算数据集主要依赖视觉模态，缺乏对话信息，难以捕捉驾驶员的语言和交互线索。

Method: 通过脚本化车内场景收集多模态数据，包括RGB/红外视频、音频和对话文本，提供统一基准和基线结果，分析模态缺失和噪声条件。

Result: 多模态融合提升了性能，但真实噪声和低光条件下仍存在挑战。

Conclusion: InCarEmo为稳健、可解释、以人为本的车内情感理解提供了基础，促进更安全、更共情的驾驶员-车辆交互。
...
TL;DR: 研究了使用视觉-语言模型（VLM）从谷歌街景图像推断建筑类型（用途、层数等），发现VLM可达到约70%的准确率，其推理偏向视觉线索，而人类专家更依赖上下文和领域知识。


<details>
  <summary>Details</summary>
Motivation: 探索VLM在建筑类型推断上的自动化潜力，并与人类专家进行对比，理解两种推理方式的差异。

Method: 使用GPT-4o、Claude 3.5 Sonnet和Gemini 2.0 Flash等VLM，结合不同缩放策略和提示技术（如思维链提示），分析AI推理中的关键词概率模式。

Result: 思维链提示使模型更稳定；VLM平均准确率约70%，其推理更关注视觉指标，而人类专家结合视觉、上下文和领域知识。
...
TL;DR: 基于人权框架的全球负责任AI指数评估各国AI治理，发现政策采纳增长但实施和可执行保护不足。


<details>
  <summary>Details</summary>
Motivation: 探究各国在AI治理中如何将负责任AI承诺转化为可落实的保护、机构能力和补救机制。

Method: 通过135个国家研究人员收集68,138个数据点，基于三大支柱（政策、公民参与、赋能条件）共38个指标，得出0-100分排名。

Result: 126/135国有AI政策，但全球南方78%框架非约束性；透明度指标最强仅18%要求政府算法公开；35国发现政府部署不可接受风险AI系统。

Conclusion: 负责任AI治理需从采纳框架转向可执行的基于权利的保护、有资源的监督机构和可及的补救措施。
...
TL;DR: 本文利用通译器（transcoders）分析语言模型中的欺骗行为，通过构建归因图识别欺骗相关特征，并证明这些特征在欺骗输出中起关键作用，为行为监控和安全检测提供了新方法。


<details>
  <summary>Details</summary>
Motivation: 语言模型中的欺骗行为构成安全风险，现有可解释性方法难以进行电路级分析，因此需要更精细的工具来理解其内部机制。

Method: 使用Qwen3-4B模型和预训练的逐层通译器（PLTs），构建归因图捕获特征激活和依赖关系，结合特征引导和电路分析识别欺骗相关特征并验证其影响。

Result: 识别出一组欺骗相关特征字典；这些特征对欺骗输出有更强的影响，能通过特征引导在欺骗与非欺骗响应间产生可预测的切换。

Conclusion: 欺骗行为源于模型内部机制，通译器在行为监控和早期检测恶意行为相关的安全漏洞方面具有潜力。
...
TL;DR: 本文介绍了CrimeNER Demo，一个用于从文档中提取犯罪相关信息并进行细粒度实体分类的AI平台，提供预训练模型和用户自定义数据微调功能。


<details>
  <summary>Details</summary>
Motivation: 为了促进犯罪领域命名实体识别研究，并为研究人员和执法机构提供自动提取犯罪信息的实用工具。

Method: 基于CrimeNER数据库的预训练NER模型，支持用户提供注释数据微调模型，并构建了从文档中自动提取和标注犯罪实体的流水线。

Result: 开发了一个演示平台，包含预训练模型、微调功能和自动化提取流水线，并公开了代码、教程和视频演示。

Conclusion: 该平台为犯罪信息提取提供了实用的AI工具，有助于相关研究和实际应用。
...
TL;DR: 提出RAPTOR自监督预训练方法，通过可到达性感知的归纳偏置提升强化学习在时序知识图谱外推任务中的训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习方法在时序知识图谱推理中面临奖励稀疏和动作空间大导致的探索效率低下问题，限制了训练效率和性能。

Method: 设计自监督预训练方法RAPTOR，通过学习候选动作到目标实体的可到达性估计，减少对无希望路径的探索，为下游强化学习微调提供强初始化。

Result: 在ICEWS14、ICEWS05-15和ICEWS18数据集上，RAPTOR显著提升了训练效率，并持续优于传统基线方法。

Conclusion: RAPTOR是增强基于强化学习的多跳推理方法在时序知识图谱推理中效果的有效手段。
...
TL;DR: 提出Proof-or-Stop生命周期控制方法，要求自动化编码代理在生命周期转换时提供可验证证据，评估显示该方法有效减少虚假完成和错误放大。


<details>
  <summary>Details</summary>
Motivation: 自动化编码代理常声称完成生命周期状态但缺乏实际证据支持。

Method: Proof-or-Stop生命周期控制，要求状态转换基于新鲜、可验证的证据，并将代理输出视为待验证声明而非状态。

Result: 在10个场景中零虚假完成；拒绝18种篡改类型；对比测试中门控循环将错误放大从31/1800降至2/1800；自应用语料库显示高解决率。

Conclusion: 方法可作为模型无关、宿主中立的控制层，但评估仅限于单一模型和有限任务。
...
TL;DR: 提出基于大语言模型的在线风暴检测系统，包含回溯分类与实时预警两种模式，在Reddit数据集上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法依赖音量、情感等间接信号，难以捕捉讨论中语义变化。

Method: 采用LLM的全局模式（分块汇总）与滑动窗口预警模式（评估负面占比、升级程度、贡献者数量）。

Result: 全局模式分类性能强；预警模式仅需少量评论即达到高召回率。

Conclusion: LLM不仅可做静态分类，还能作为上下文感知的重复评估工具用于社交媒体监测。
...
### [63] [MedFailBench: A Clinician-Built Open-Source Benchmark for Medical AI Safety Boundary Inspection](https://arxiv.org/abs/2607.15166)
*Goktug Ozkan*

Main category: cs.AI

TL;DR: MedFailBench是一个基于医生构建的合成基准，用于评估医疗AI的安全性失败模式，包含严重度分级的错误类型和安全门分类。


<details>
  <summary>Details</summary>
Motivation: 现有的医疗AI基准主要衡量模型是否给出正确答案，而MedFailBench旨在回答‘哪些安全边界失效了’这一不同问题。

Method: 通过医生构建的合成病例集，定义了5种安全门类型、0-5级严重度分级，并提供了自动分析流程。

Result: 目前发布版本包含44个医生复核的合成病例及其严重度注解，以及相关分类框架和排行榜预览。

Conclusion: MedFailBench以开源许可发布，用于系统性评测医疗AI的安全边界失效情况，但未包含患者数据或模型排名。
...
TL;DR: CFM-Bench是一个统一的多域、多任务基准测试，用于公平比较通道基础模型（CFMs）和任务特定模型，提供了6种通道配置和6个任务组。


<details>
  <summary>Details</summary>
Motivation: 现有CFM评估在不同管道中进行，导致不公平比较，缺乏统一的基准。

Method: 设计CFM-Bench基准，包含多域通道配置、官方数据划分、严格禁止训练阶段使用测试单元，并覆盖PHY、RAN和ISAC三类应用下的6个任务。

Result: 基准测试可评估CFM的迁移能力，确保公平对比。

Conclusion: CFM-Bench为通道表示在不同模型、域和任务间的可比性提供了统一基础。
...
### [65] [Explaining Process Control Optimisation Recommendations via GradientSHAP and Implicit Differentiation](https://arxiv.org/abs/2607.14970)
*Paul Darm,Cem Alpturk,Kenneth Ulrich,William Duncan,Ali Anwar,Annalisa Riccardi*

Main category: cs.AI

TL;DR: 论文提出一种融合隐函数定理灵敏度分析、SHAP归因和大语言模型的方法，为工业优化输出生成实时可解释的自然语言说明，在高压辊磨机控制优化中实现40倍加速且保持高归因一致性。


<details>
  <summary>Details</summary>
Motivation: 工业自动化优化算法与操作员之间存在信任鸿沟，现有可解释AI主要针对预测任务，优化输出的可解释性方法缺失。

Method: 利用隐函数定理从最优性条件计算精确参数灵敏度，结合GradientSHAP进行高效归因，再通过大语言模型生成面向操作员的自然语言解释。

Result: 在22特征的工业高压辊磨机控制优化问题中，与KernelSHAP的归因相关性>0.99，计算速度提升40倍以上，支持实时解释生成。

Conclusion: 该方法能有效弥合优化算法与操作员之间的信任差距，通过实时可解释性增强工业优化系统的实用性和接受度。
...
### [66] [Benchmarking Multimodal Large Language Models for Scientific Visualization Literacy](https://arxiv.org/abs/2607.15176)
*Patrick Phuoc Do,Chau M. Ta,Chaoli Wang*

Main category: cs.AI

TL;DR: 本研究评估了六种多模态大语言模型在科学可视化素养测试中的表现，发现其能力不均，Gemini最强，开源模型较弱，且在纹理、集成可视化和定量估计上存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有评估多集中于图表，缺乏对科学可视化理解能力的充分证据，因此需要专门评估MLLM在SciVis上的素养。

Method: 使用科学可视化素养评估测试（49道题，基于18个可视化），在封闭世界协议下评估三种闭源和三种开源模型，并与485名人类数据对比。

Result: Gemini在子集上超越人类平均水平；开源模型低于人类基线；模型在科学插图、搜索和空间理解上表现好，但在纹理/集成可视化和定量估计上表现差。

Conclusion: 科学可视化素养应作为评估多模态AI系统的必要维度。
...
### [67] [Demographically-Conditioned Synthetic Medical Images for Bias Mitigation and Bias Detection in Disease Classifiers](https://arxiv.org/abs/2607.14984)
*Mahmoud Ibrahim,Bart Elen,Chang Sun,Gokhan Ertaylan,Michel Dumontier*

Main category: cs.AI

TL;DR: 本文提出利用人口统计学条件生成式模型（如Stable Diffusion 2.1）来解决医学图像分类器在少数子组公平性审计中的样本量不足问题，在训练和评估阶段分别实现了偏差缓解和检测功能。


<details>
  <summary>Details</summary>
Motivation: 现有子组公平性审计面临测试集中少数子组样本量过小，导致性能置信区间过宽，无法有效检测偏差，从而影响公平性评估的可靠性。

Method: 在COVID-19胸部CT分类任务中，使用微调后的Stable Diffusion 2.1生成合成数据，训练阶段采用顺序预训练再微调策略（而非联合增强），评估阶段利用合成数据模拟少数子组以进行偏差检测。

Result: 训练时，顺序预训练比联合增强效果更好，分类器性能超过全真实基线，且数据效率提升约100倍；评估时，合成数据在5个少数子组和5个分类器种子下，对真实测试集排序（Spearman ρ=1.00）和单细胞估计的可靠性均优于小样本真实集。

Conclusion: 人口统计学条件合成数据能有效解决子组公平性审计中的样本不足问题，既能训练时缓解偏差（作为预训练先验），又能在评估时可靠检测偏差，尤其适用于公平性审计关注的少数子组细胞分析。
...
### [68] [Pretraining Data Can Be Poisoned through Computational Propaganda](https://arxiv.org/abs/2607.15267)
*Victoria Graf,Hannaneh Hajishirzi,Noah A. Smith,David Kohlbrenner,Kyle Lo*

Main category: cs.AI

TL;DR: 本文研究通过公共讨论界面注入恶意内容来毒化预训练数据的可行性，并提出HalfLife分析方法评估此类攻击。


<details>
  <summary>Details</summary>
Motivation: 现有预训练数据毒化研究局限于Wikipedia等已知数据源，未考虑真实预训练语料库的大规模异质性及数据清洗流程的影响。

Method: 利用现有网络规模内容注入机制（公共讨论界面）进行攻击，并开发HalfLife分析方法估计恶意内容在网络爬取和数据清洗后的保留概率。

Result: 证明通过开放讨论接口毒化预训练数据在网页规模上具有可行性，且HalfLife能有效评估毒化内容的包含概率。

Conclusion: 第三方网页内容可能成为攻击语言模型预训练的潜在向量，需重视毒化注入的评估。
...
### [69] [SMC-ES: Automated synthesis of formally verified control policies](https://arxiv.org/abs/2607.15003)
*Riccardo Curcio,Toni Mancini,Enrico Tronci*

Main category: cs.AI

TL;DR: 提出一种基于仿真与统计模型检验的方法，为自主控制策略自动生成性能、安全性和鲁棒性的形式化保证。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法缺乏正式保证，无法安全部署于关键任务环境。

Method: 结合进化策略与统计模型检验的SMC-ES算法，以置信度1-δ保证策略违规概率不超过ε。

Result: 在Gym和Safety Gym任务中，SMC-ES在适度增加计算成本下，提供形式化保证且性能与主流DRL/Safe-DRL方法相当。

Conclusion: 所提方法能在保持竞争力的同时，为策略提供零违反概率的概率性正式保证。
...
### [70] [Moral Attitudes of Sentient ASI towards Humanity and Implications for AGI Development](https://arxiv.org/abs/2607.14998)
*Jean-Paul Van Belle*

Main category: cs.AI

TL;DR: 论文提出了一种新视角：从未来有感知能力的通用人工智能（ASI）如何道德评价人类的角度，审视人与AI的关系。


<details>
  <summary>Details</summary>
Motivation: 传统AI伦理聚焦人类如何对待ASI，忽略了未来ASI可能对人类形成的道德判断，以及当前技术选择对此的长期影响。

Method: 采用概念倒置方法，提出一套可能指导ASI行为的后人类道德原则，并探讨技术设计选择、人类道德行为及人性本质如何影响人类在ASI世界中的地位。

Result: 识别出技术设计、人类道德行为等因素可能决定人类在ASI世界中的长期处境，并给出初步原则示例。

Conclusion: 人类当前的设计选择与道德行为，将深刻影响未来ASI对人类的道德评价与对待方式，需提前考量这些影响。
...
### [71] [Man, Machine, and Masterpiece: Artistic Ownership in the AI Era](https://arxiv.org/abs/2607.15027)
*Sofi Gjing Jovanovska,Kuntal Ghosh,Daniel Muhu Njenga,Ahmed Mufassir,Shadan Sadeghian*

Main category: cs.AI

TL;DR: 通过设计原型ArtSplit量化人类与AI在创作中的贡献，引发关于所有权定义的反思，并批判量化方法无法匹配艺术家的创作意图。


<details>
  <summary>Details</summary>
Motivation: AI系统融入创意工作引发所有权争议，但缺乏对人类与AI贡献交织时所有权的明确定义。

Method: 开发了名为ArtSplit的“促发原型”，通过量化创作流程中人类与AI的贡献来引发艺术家反馈。

Result: 量化方法未能与艺术家对创意意图和能动性的理解一致，可能稀释长期存在的创作假设。

Conclusion: 将所有权转化为技术问题的冲动受到批判，强调应尊重历史和社会情境中的关系。
...
### [72] [BrainPilot: Automating Brain Discovery with Agentic Research](https://arxiv.org/abs/2607.15079)
...
TL;DR: 本文提出了一个完全开源的多智能体系统 BrainPilot，用于加速脑科学研究，具备可追溯日志和结果验证功能。


<details>
  <summary>Details</summary>
Motivation: 当前 AI 代理缺乏脑科学领域专业知识，可能捏造结果，在多步推理中偏离目标，且缺少专家干预点，这些问题在脑科学中尤其严重。

Method: 该系统包含一个首席研究员（PI）智能体，协调多个专业智能体，利用包含 7233 个索引条目和 72 个可复用方法单元的知识库与技能库。所有步骤记录在跟踪图中，审计智能体进一步集成捏造检查。

Result: 在三个脑科学任务和自建基准 BrainPilotBench-v0 上，BrainPilot 使用开源模型达到与最先进框架相当的性能，且成本更低。

Conclusion: BrainPilot 提供了一个可靠、可审计且经济高效的解决方案，用于加速脑科学研究。
...
### [73] [Concept-Guided Spatial Regularization for World Models in Atari Pong](https://arxiv.org/abs/2607.15142)
*Yukuan Lu,Zaishuo Xia,Weyl Lu,Yubei Chen*

Main category: cs.AI

TL;DR: 在Atari Pong游戏中，五种视觉世界模型（DreamerV3、DIAMOND、TWISTER、Simulus、STORM）在冻结后均出现视觉和动力学错误，导致像素空间零样本MBRL性能大幅下降；提出CGSReg辅助损失可部分改善模型质量。


<details>
  <summary>Details</summary>
Motivation: 世界模型通常作为MBRL组件被评估，很少被单独研究，无法诊断其固有缺陷。

Method: 先复现训练管道匹配报告性能，冻结模型后用闭环滚动诊断和像素空间零样本MBRL评估；提出CGSReg辅助像素重建损失。

Result: 所有模型滚动中存在球消失、运动错误等；零样本MBRL策略性能显著低于原管道，DreamerV3回报从-5.5降至-20.9；CGSReg在部分模型中有所改善。

Conclusion: 现有世界模型对任务关键概念建模不足，CGSReg可缓解部分问题但非全面解决方案。
...
### [74] [Long-Context Fine-Tuning with Limited VRAM](https://arxiv.org/abs/2607.15105)
*Vladimir Fedosov,Aleksandr Sazhin,Artemiy Grinenko,Frank Woernle*

Main category: cs.AI

TL;DR: 提出层级全局注意力（HGA），通过分段反向传播和分层KV存储，在16GB GPU上实现16K训练和131K推理，性能接近全注意力但显存开销更低。


<details>
  <summary>Details</summary>
Motivation: 参数高效微调降低了模型和优化器内存，但密集注意力在长序列训练时仍显存昂贵，限制上下文长度。

Method: 层级全局注意力（HGA），结合分段反向传播（仅活跃段可微）和分层KV存储（旧KV卸载到RAM/NVMe），每个查询块加载有限的历史令牌。

Result: 在Qwen3-8B 4-bit QLoRA上，密集训练仅支持2K令牌（16GB GPU），HGA支持16K训练（15.28GB显存）和131K推理；2K训练时HGA适配器性能（2.7405 nat）接近密集适配器（2.7383 nat），且速度略快。

Conclusion: HGA在长上下文场景下显著降低显存需求，保持质量的同时扩展性优于密集注意力，适用于检索和生成，生产实现正在开发。
...
TL;DR: 本文研究了AI基准测试中项目反应理论（IRT）建模的可靠性问题，发现经典估计器在大规模基准中不可行，而可扩展估计器在小规模或非正态分布模型集合下不可靠。


<details>
  <summary>Details</summary>
Motivation: AI基准测试数据与人类测试数据存在差异（模型少、项目多、能力分布非正态），标准IRT工具可能不可靠，需评估其适用性。

Method: 基于六个LLM基准的真实参数模拟响应矩阵，使用四种估计工具（MML、MCMC、VI、神经伪孪生估计器）在18000种条件下比较。

Result: 经典估计器在大规模基准中不可行，可扩展估计器在小规模或非正态分布模型下产生不可靠的推断。

Conclusion: IRT模型在AI基准测试中的可靠性取决于样本大小和分布，需诊断以确保可信使用。
...
### [76] [The Industrialization of Research ; On AI-Driven Science and Its Consequences](https://arxiv.org/abs/2607.15164)
*Emmanuel Jeannot*

Main category: cs.AI

TL;DR: 本文探讨了人工智能从工具转变为科研自主参与者所带来的工业化转型，并分析了七个关键问题：科学能力传承削弱、理论不透明、同行评议崩溃、颠覆性创新潜力不足、议程被政治和工业主导、闭环系统错误累积以及全球科研群体结构性分化。


<details>
  <summary>Details</summary>
Motivation: 人工智能正在成为科研的自主参与者，这一转型可能根本改变科学研究的本质，引发一系列未解决的挑战，需要系统性地识别和讨论。

Method: 采用概念性分析方法，以美国能源部“创世纪任务”为典型案例，提出并分析了AI驱动科学工业化的七个核心问题。

Result: 识别出七个关键挑战：科学能力传承、AI理论不透明、同行评议泛滥、范式创新潜力、议程政治化、系统误差固化以及科研群体分层化。

Conclusion: 这些挑战并非完全否定AI驱动的科学，而是为确保其潜在利益得到负责任的实现所必须解决的核心条件。
...
TL;DR: 提出了一种基于计划的可视化GUI自动化系统Plover，通过外部化任务计划和重新规划来提高透明度、可控性和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有GUI自动化系统在动态环境中容易偏离用户意图，且内部规划缺乏可审查性和可干预性。

Method: 采用计划器-执行器架构，将计划作为持久、可检查和可修改的工件，支持显式监督、局部修正、自然语言指导和截图干预。

Result: 许多自主GUI代理的失败可以通过可见计划和局部干预进行结构性修复，显式重新规划提高了系统的透明度和适应性。

Conclusion: Plover通过计划外化使GUI自动化更加透明、可控和可适应，能够有效支持用户监督和干预。
...
TL;DR: 提出了一种结合大语言模型和专家反馈的自进化注释框架，用于构建符合DSM-5-TR标准的、可解释的重性抑郁障碍数据集。


<details>
  <summary>Details</summary>
Motivation: 抑郁症数据集的标签缺乏结构化证据、症状层面理由及与DSM-5-TR标准的可追溯对齐，限制了透明度和模型可解释性。

Method: 采用三阶段流程：候选证据选择、DSM-5-TR标准分析、案例级综合；并设计双记忆架构（示例记忆和反思记忆）内化专家反馈，实现迭代改进。

Result: 初步研究表明，该方法提高了注释一致性和可解释性，减少了人工修订工作量。

Conclusion: 该框架通过可追溯的临床证据和推理过程，提升了数据集质量，支持构建可靠且可解释的AI系统。
...
TL;DR: 该论文发现大语言模型中物理危险与内容危险在表示层面可分离，并提出PRISM探针实现高精度检测。


<details>
  <summary>Details</summary>
Motivation: 大语言模型为具身智能体规划时，语言上安全的指令在物理世界中可能带来危险，现有方法难以区分物理危险与文本内容危险。

Method: 通过隐藏状态方向分析和随机分割零假设检验分离表示，基于全隐藏状态训练单层L2正则化逻辑探针PRISM。

Result: PRISM在SafeAgentBench上达86.2-87.7%准确率及11.7-13.7%假阳性率；在PSB-1K上达99.6%准确率及0.7%假阳性率，优于同等规模LLM法官。

Conclusion: 隐藏状态探针可作为超越文本审核的表示级物理安全方法，有效检测隐含物理危险。
...
TL;DR: 提出了一个端到端的多智能体系统AutoSynthesis，能自动完成从搜索文献到元分析的全流程，并与人工专家结果一致。


<details>
  <summary>Details</summary>
Motivation: 传统定量证据合成依赖人工，过程繁琐且难以规模化，需要自动化方法提高效率。

Method: 采用多智能体系统，输入自然语言研究问题后，自动执行搜索、筛选、数据提取、效应量计算和随机效应元分析，并支持异质性分析和偏倚评估。

Result: 系统筛选了28项研究，提取了20多个定量结果，其合并效应量与专家手动元分析结果（Hedges’ g）高度一致。

Conclusion: AutoSynthesis能够可扩展地完成定量证据合成，支持跨学科循证决策。
...
TL;DR: 提出了teLLMe系统，用于城市驾驶数据集的探索性因果分析，结合因果结构学习与自然语言查询，生成Causal Card摘要。


<details>
  <summary>Details</summary>
Motivation: 交通机构拥有大量视频数据，但缺乏干预实验，难以回答因果关系问题（如雨天如何影响交通密度）。

Method: 从行车记录仪注释构建结构化事件表，使用PC算法进行因果结构学习，结合bootstrap稳定性检查，通过线性回归和DoWhy实现查询特定效应估计，利用Schema-aware LLM将自然语言问题映射为结构化因果查询。

Result: 在BDD交通事件案例中，系统能揭示天气、高峰时段和交通密度之间的合理关系，并明确表示不确定性和建模选择。

Conclusion: 系统作为假设生成和专家推理工具，而非因果结论的权威来源。
...
### [82] [SearchOS-V1: Towards Robust Open-Domain Information-Seeking Agent Collaboration](https://arxiv.org/abs/2607.15257)
...
TL;DR: 提出了SearchOS，一种系统级多智能体框架，通过外部化搜索状态和管道并行调度，显著提升了开放域信息检索任务的效率和完整性。


<details>
  <summary>Details</summary>
Motivation: 现有工具集成的大语言模型在长对话中难以追踪任务进度，容易陷入重复搜索循环，浪费预算并影响输出质量。

Method: 将开放域信息检索建模为关系模式补全与引用验证，设计面向搜索的上下文管理(SOCM)外部化状态，采用管道并行调度和搜索工具中间件进行流程控制。

Result: 在WideSearch和GISA基准上，SearchOS在所有指标上领先于现有的单智能体和多智能体基线。

Conclusion: SearchOS通过显式状态管理和高效调度，为鲁棒的信息寻求协作提供了新方向。
...
TL;DR: MultiRef-Compass是一个统一基准，用于评估多参考到音视频生成任务，包含350个精心设计的样本和14个子指标的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注文本驱动生成或单参考保持，缺乏对多参考联合推理和音视频同步生成的评估。

Method: 通过可扩展的资源组合管道构建350个样本，并采用自动指标和重判增强的MLLM裁判框架进行四维度评估。

Result: 对八个代表性MR2AV系统的实验显示，在多个评估维度上仍有显著改进空间。

Conclusion: MultiRef-Compass为未来MR2AV研究提供了基础性基准，并揭示了现有系统的不足。
...
TL;DR: 本文提出SIRUS，一种无需训练的推理时框架，用于文本到视频生成中的概念消除，在保持视频质量的同时实现目标概念遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有T2V概念消除方法难以在抑制跨帧目标概念的同时，保留非目标主体、动作、场景和时间结构。

Method: SIRUS通过定位与目标概念相关的文本证据，并在采样过程中抑制其表达，无需更新模型参数。同时提出了一套面向视频的评估框架。

Result: 在CogVideoX上，SIRUS达到70.4%的平均遗忘成功率和25.7%的平均帧命中率，优于VideoEraser的44.4%/47.2%，且VBench质量下降从-0.043降至-0.016。

Conclusion: SIRUS实现了遗忘-质量的最佳权衡，并在Wan2.2上验证了跨骨干网络的泛化能力。
...
TL;DR: 提出KeyFrame-Compass基准测试，用于评估关键帧条件视频生成，发现现有模型在执行关键帧与自然视频合成之间存在权衡，且性能随关键帧密度增加而下降。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽支持多关键帧条件，但其能否在保持视频质量的同时忠实再现指定关键帧尚不明确。

Method: 构建包含386个精心设计样本的基准测试，涵盖多个应用领域、视频结构、提示粒度等；并设计自动化评估框架，通过六项互补指标衡量关键帧执行质量，结合MLLM和感知模型评估整体视频质量。

Result: 在九个代表性视频生成系统上的实验表明，模型在忠实执行关键帧与自然视频合成之间存在权衡；关键帧约束越密集，性能越差；大多数开源模型无法将故事板网格输入解释为时序关键帧序列。

Conclusion: 当前模型在关键帧条件视频生成中存在根本性限制，即在执行约束与自然度之间需进一步优化。
...
TL;DR: 本文通过拓扑和几何探针，发现自监督视觉基础模型在隐空间中存在与3D欧几里得空间强相关的子空间，并提出基于该子空间的潜空间导航技术进行视觉里程计与定位。


<details>
  <summary>Details</summary>
Motivation: 现有工作通过回归深度或法线来探测视觉特征的3D意识，但缺乏对特征空间结构与欧几里得变换群SE(3)之间关系的研究，本研究旨在揭示视觉基础模型是否内隐3D空间结构。

Method: 提出互邻域度量（拓扑对齐）和庞加莱适配器（几何线性可访问性）两类探针，分别评估特征邻域与空间拓扑的匹配度及相机运动在潜空间中的几何表示。

Result: 自监督视觉模型在无3D监督或主动交互下，其潜空间子集与3D欧几里得空间表现强相关。

Conclusion: 基于此发现，可应用潜空间导航技术（视觉里程计与定位）直接通过隐层特征操作实现，无需显式3D重建。
...
TL;DR: 提出了一个用于赛车场景的3D车道检测数据集，包含25万+图像和多摄像头惯性测量数据，并展示了近300Hz处理速度的改进方法。


<details>
  <summary>Details</summary>
Motivation: 现有车道检测研究主要针对常规驾驶场景，缺乏对高速、极端几何道路的赛车场景的探索。

Method: 利用多摄像头和惯性测量数据进行预积分，回归道路几何形状，并采用集成预测方法。

Result: 相比BevLaneDet，F1分数提升3点，近车平均绝对误差降低30%以上，F1>0.9且横向MAE<0.18m。

Conclusion: 惯性测量和集成预测能显著提升赛车场景中3D车道检测的性能和鲁棒性。
...
TL;DR: MonteRET是一个区域感知的检索增强框架，用于自动生成胸部CT报告。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时理解整个CT体积和准确定位局部解剖发现，导致临床报告生成不准确。

Method: 整合全局CT特征与区域级解剖表示，利用预测的医学状况和区域级视觉-语言对齐检索相关知识，并通过知识引导的报告改写代理优化初始报告。

Result: 在公共数据集和外部多中心队列中，MonteRET在报告质量、语义相似性和临床效能上优于基线方法和现有最优方法，尤其提高了召回率，减少了遗漏发现。

Conclusion: MonteRET通过区域感知和检索增强策略显著提升了胸部CT报告生成的临床准确性。
...
TL;DR: 提出了XCT-SAM，一种面向增材制造XCT缺陷分割的序列参数高效适配框架，通过中间域（合金微观结构）逐步适应，解决了自然图像与工业XCT域间大迁移差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有SAM在增材制造XCT缺陷分割中性能差，主要因为自然图像预训练与XCT域存在大分布偏移、缺陷表现为非语义微结构异常且标注数据稀缺。

Method: 采用序列参数高效适配：先在合金微观结构数据集上微调Conv-LoRA适配器（秩r=2），再将适配模型迁移至XCT图像；仅训练约4.15M参数，冻结SAM超99%参数。

Result: 在CycleGAN-XCT基准和真实NIST XCT扫描上，XCT-SAM在IoU和Dice指标上一致优于零样本SAM及其他域适配基线。

Conclusion: 中间域适配配合参数高效适配器能有效提升工业XCT缺陷分割性能，代码已开源。
...
TL;DR: 提出DCVC-Mamba，一种用于B帧编码的神经视频编码框架，融合状态空间模型的双向预测和熵感知跳过机制，在BD-rate上相比先前的神经视频编码器降低8.98%，相比传统标准VTM-19.0-LDP降低30.45%。


<details>
  <summary>Details</summary>
Motivation: 现有神经视频编码器在B帧编码中效率有限，需要改进双向时序预测和熵编码效率。

Method: 采用IBP框架进行低延迟B帧编码；基于状态空间模型的时空融合模型实现双向时序预测；熵感知跳过机制选择性忽略部分潜在表示以减少熵编码时间；并引入两种推理时策略提升压缩性能。

Result: 相比先前神经视频编码器平均BD-rate降低8.98%；相比VTM-19.0-LDP降低30.45%；相比VTM-19.0-RA(Inter-GoP=16)降低1.81%。

Conclusion: DCVC-Mamba在B帧压缩中显著优于现有神经编码器和传统标准，推动了神经视频压缩的进步。
...
TL;DR: 提出SD-MAR框架和GRPO-lite训练方法，显著提升多图像分析推理能力，Qwen2.5-VL-7B性能超越GPT-4.1。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在多图像比较、变化检测等分析推理任务中表现不足，而现有基准缺乏对此能力的评估。

Method: 通过控制扰动构建配对的视觉场景并生成推理任务，使用GRPO-lite（移除KL正则化并引入后向折扣分配）进行强化学习微调。

Result: 域内准确率提升高达36.95%，Qwen2.5-VL-7B在SD-MAR基准上超越GPT-4.1；域外泛化保持或提升，MMBench提升4%，LLM-as-judge评估显示逻辑连贯性和解释质量一致改进。

Conclusion: GRPO-lite微调有效提升多图像分析推理能力，且不影响通用性能，验证了框架的有效性。
...
TL;DR: MixCompress提出混合专家与深度混合的稀疏结构专门化框架，统一可变比特率图像压缩，超越单速率基线。


<details>
  <summary>Details</summary>
Motivation: 现有可变比特率方法因共享主干网络导致特征纠缠，低速率平滑梯度与高速率纹理细节矛盾，限制性能。

Method: 基于稀疏门控混合专家（MoE）缓解梯度冲突，引入混合深度（MoD）动态缩放模型容量，配合条件辅助变换（CAT）进行子带能量调制。

Result: MixCompress匹配甚至超越独立优化的单速率模型，在计算高效图像编码中建立新的帕累托前沿。

Conclusion: 稀疏结构专门化有效解决了可变比特率压缩中的特征冲突，实现了统一的、高性能的图像压缩框架。
...
TL;DR: 提出梯度向量场手术来校正区域损失函数导致的过度自信预测，在2D/3D医学分割中保持高精度的同时提升校准性能。


<details>
  <summary>Details</summary>
Motivation: 区域损失函数（如Dice损失）在医学分割中产生过度自信的预测，阻碍了临床采用。

Method: 对损失函数的偏导数进行手术，添加与预测误差线性相关的缩放因子，修改梯度向量场。

Result: 在2D和3D医学分割任务中，该方法在不降低精度的情况下有效缓解了校准问题。

Conclusion: 梯度手术是一种简单有效的干预手段，可改善区域损失函数的校准问题。
...
### [94] [Compression of 3D Gaussian Splatting Data Using GPU-friendly Graphics Texture Coding](https://arxiv.org/abs/2607.14513)
*Amir Said,Randall Rauwendaal*

Main category: cs.CV

TL;DR: 提出一种利用GPU纹理压缩方案压缩3D高斯溅射中球谐系数的方法，在不明显损失质量的前提下大幅减少内存占用，并支持高效并行解码。


<details>
  <summary>Details</summary>
Motivation: 3DGS存储大量球谐系数导致内存需求大，现有压缩方法不支持GPU并行解码。

Method: 使用BC1/BC7纹理压缩格式对局部群组化重排的球谐系数进行压缩，并引入比特率控制策略保持随机访问能力。

Result: GPU并行解压缩在渲染质量损失可忽略或不可感知的情况下有效压缩系数。

Conclusion: 纹理压缩方案可利用GPU硬件加速高效压缩3DGS的颜色系数，兼顾压缩率与渲染性能。
...
### [95] [Emergent Region-Level Facial Correspondence in Frozen Vision Foundation Models](https://arxiv.org/abs/2607.14423)
*Izaldein Al-Zyoud,Abdulmotaleb El Saddik*

Main category: cs.CV

TL;DR: 冻结的DINOv3自监督视觉模型能够建立人脸区域级的坐标系统，实现跨身份和时间的人脸部位对齐，无需针对人脸的专门训练。在CelebDF-v2数据集上，区域语义准确率达83.0%，时间追踪准确率达95.5%。


<details>
  <summary>Details</summary>
Motivation: 探究冻结的自监督视觉模型（如DINOv3）是否能在无需人脸特定训练的情况下，建立人脸区域的跨身份对应关系，以验证其是否具备面部坐标系统的能力。

Method: 使用DINOv3 ViT-L/16的补丁嵌入特征，结合FaRL作为面部部位标注接口，通过跨身份最近邻匹配和时间标签传播进行评估。实验在200个CelebDF-v2真实视频上进行，并比较了不同中间层的性能。

Result: DINOv3在跨身份匹配中达到83.0%区域语义准确率（随机基线23.0%），时间追踪准确率95.5%；无FaRL控制实验降至0.9%。最强对应出现在中间层（第18块），区域内外判别比达4.93x（最终块仅1.48x）。相比CLIP，DINOv3在解剖区域上提升16.8个百分点。
...
### [96] [Dynamic Manipulation Hypergraphs for HAR: Beyond Pairwise Relations: Dynamic Manipulation Hypergraphs for Vision-Based Human Activity Recognition](https://arxiv.org/abs/2607.14350)
*Fatemeh Ziaeetabar*

Main category: cs.CV

TL;DR: 提出动态操作超图框架，通过高阶关系建模提升细粒度操作识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统图方法使用二元边会割裂事件中的关系，无法有效建模多实体间的协同交互。

Method: 构建动态超图，每个时间步编码实体的外观、空间、运动和语义角色特征，基于邻近、接触和运动耦合谓词生成并排序超边候选，通过节点-超边消息传递和时间注意力进行推理。

Result: 在EPIC-KITCHENS-100/VISOR和Assembly101上，HO-F1比匹配的成对图分别提高6.9和9.5个百分点，比静态超图提高4.4和5.8个百分点。

Conclusion: 时变高阶关系建模对细粒度操作活动识别具有显著价值。
...
TL;DR: 提出K-NeAS，一种自动多材料表面重建的统一可扩展架构，通过共享潜在主干网络、K材料顺序软选择器和GMM自动衰减阈值化，在稀疏投影下显著提升CBCT三维重建保真度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有隐式场景表示方法受限于手动调整衰减范围和刚性双材料约束，无法自动处理多材料表面重建。

Method: 采用共享潜在主干网络替代独立材料网络，引入可微分的K材料顺序软选择器，利用高斯混合模型自动确定衰减界限，并设计辅助漂浮损失以减少几何幻觉。

Result: 在四个临床CBCT数据集上，K=3时腹部区域3D PSNR达33.28 dB（比单材料基线提升1.88 dB），在5-和10-视图稀疏采样下PSNR提升达1.17 dB。

Conclusion: K-NeAS成功扩展到任意材料数量且无需手动调参，在复杂多组织区域实现更优的体积保真度和稀疏采样鲁棒性。
...
### [98] [Cotton-SF YOLO: Learning Structural and Frequency Cues for Early Cotton Square Detection in Complex Field Environments](https://arxiv.org/abs/2607.14445)
*Chengjia Zhang,Yu Li,Feiri Ali,Yan Zhang,Xin Chen,Longke He,Daokun Ma,Liting Gao*

Main category: cs.CV

TL;DR: 提出了一种基于YOLO26m的棉花方块检测框架Cotton-SF YOLO，通过动态蛇形卷积和频域特征调制模块，提升了复杂田间环境下小目标检测的性能。


<details>
  <summary>Details</summary>
Motivation: 棉花方块是早期生殖生长的重要表型指标，但在复杂田间环境中，由于目标小、遮挡、模糊、光照变化和低对比度，检测面临挑战。

Method: 采用动态蛇形卷积增强边缘特征感知，并在C2f结构中引入频域特征调制模块，增强边缘和纹理特征，减少叶片背景干扰。

Result: 在自建数据集上，mAP50达0.8196，mAP50:95达0.4942，召回率0.7939，较基线YOLO26m分别提升1.25%、3.45%和2.96%。

Conclusion: 结构特征与频域特征互补能最优提升检测性能，验证了方法的有效性。
...
### [99] [G$^2$SR: Geometric Methods for Fast and Memory-Efficient Gaussian-based Surface Reconstruction](https://arxiv.org/abs/2607.14470)
*Dasong Gao,Vivienne Sze,Sertac Karaman*

Main category: cs.CV

TL;DR: 提出G2SR方法，利用2D高斯斑点的跨视图对应关系，通过轻量级神经网络前端检测跟踪和解析后端三角化，实现快速、几何精确且内存高效的少视图表面重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在移动平台上实现快速、几何精确且内存高效的在线表面重建，端到端方法计算和内存需求大且泛化性差。

Method: 采用轻量级神经网络前端在图像平面上检测和跟踪2D高斯斑点，并利用解析后端通过多视图几何三角化每个2D斑点为度量3D斑点。

Result: 在ScanNet、Replica和DTU数据集上，几何精度匹配或超越最先进端到端方法，在384x512分辨率下实现69-89帧/秒的重建速度，GPU内存占用仅203 MB（减少5-107倍）。

Conclusion: G2SR为基于高斯表示的在线表面重建提供了实用路径，兼顾速度、精度和内存效率。
...
### [100] [Variational Inference for Bird's Eye View Segmentation in Autonomous Driving](https://arxiv.org/abs/2607.14710)
*Jingyue Shi,Huaicheng Li,Junhui Zhao,Yanxiang Jiang*

Main category: cs.CV

TL;DR: 本文提出了一种基于变分推断和Transformer的鸟瞰图分割方法TVB，通过条件变分自编码器和归一化流生成多个BEV候选图，并利用注意力机制融合，在nuScenes和OPV2V数据集上取得了优越性能。


<details>
  <summary>Details</summary>
Motivation: 解决多相机数据融合和复杂驾驶环境中BEV分割的挑战。

Method: 采用CVAE作为主干网络，结合归一化流增强分布表达能力，并设计BEV注意力融合模块（BAF）自适应整合多个候选BEV图。

Result: 在nuScenes和OPV2V数据集上，多相机BEV分割和车道环境感知任务中均达到最优性能。

Conclusion: TVB方法有效提升了BEV分割的准确性和鲁棒性，为自动驾驶环境感知提供了新思路。
...
### [101] [Reinforcing Egocentric Spatial Perception in Multimodal Large Language Models via Ego Scene Augmentation](https://arxiv.org/abs/2607.14497)
*Chi Kit Wong,Ye Pan,Yuanhuiyi Lyu,Xu Zheng,Zidong Cao,Lutao Jiang,Zixin Zhang,Huiyu Zhou,Xuming Hu*

Main category: cs.CV

TL;DR: 提出Ego Scene Augmentation框架，通过Ego-element Graph增强多模态大模型在自我中心场景中的空间感知能力，在EgoTextVQA任务上显著提升表现。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在复杂自我中心场景中进行空间推理时效果不佳，因其空间感知能力有限。

Method: 构建Ego-element Graph封装自我中心空间特征，利用视觉基础模型增强MLLMs的空间感知。

Result: 在EgoTextVQA上室内设置提升8.14%，室外设置提升8.72%，购物子集表现最突出。

Conclusion: ESA框架有效增强了自我中心场景的空间感知，代码已公开。
...
### [102] [Immediate 3D Gaussian Splat Reconstruction of Unordered Input with Global Consistency](https://arxiv.org/abs/2607.14481)
*Andreas Meuleman,Linus Franke,Boris Zhestiankin,Camille Montemagni,George Drettakis*

Main category: cs.CV

TL;DR: 提出首个为3D高斯泼溅（3DGS）提供即时反馈的全局一致重建方法，支持无序图像序列，通过重用地标识别模型、协视图和GPU优化实现快速局部重建，并引入基于聚类渐进层次进行回环检测与场景扩展。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS场景重建依赖连续有序图像或离线SfM，无法在无序捕获过程中提供即时反馈，而增量SLAM方法虽实时但无法处理此类顺序。需实现首个兼顾全局一致性与即时反馈的无序序列解决方案。

Method: 1.利用视觉地标识别模型和协视图实现无序图像快速匹配与关键帧选择；2.基于GPU优化和高斯图元放置进行局部快速重建；3.提出基于协视图的聚类分层方法进行高效回环检测；4.引入渐进层次结构扩展至大规模场景。

Result: 在包含数千张图像的多个数据集上，实现了视觉质量良好的即时3DGS重建，有效处理无序序列且全局一致。

Conclusion: 该方法首次实现了无序捕获场景下3DGS的即时全局一致重建，显著降低了计算成本且可扩展至大规模场景。
...
TL;DR: 本文提出了一种基于视觉-拓扑记忆的训练无关VLM导航框架，通过跨情景经验复用显著提升了物体目标导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅评估单情景导航，忽略了机器人在同一场景中重复运行时可保留经验的潜力。

Method: 构建层次化视觉-拓扑记忆，通过粗到细匹配检索经验，并设置保守执行防护缓解导航问题。

Result: 在HM3D v0.1、HM3D v0.2和MP3D三个基准上均取得最优性能。

Conclusion: 结构化视觉-拓扑经验复用在跨情景导航中有效且鲁棒。
...
TL;DR: Uni-AdaVD是一个通用的推理时概念擦除框架，通过正交值分解和自适应擦除偏移，在多种视觉生成模型上有效去除目标概念并保留非目标内容。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法通常依赖特定架构，难以在去除目标概念的同时保留非目标内容和生成先验。

Method: 将多模态注意力的值空间作为统一干预空间，引入编码器感知的目标表示构建，结合正交值分解与自适应擦除偏移，无需更新模型权重。

Result: 在U-Net、DiT、自回归图像生成器及文本到视频模型上，单/多概念擦除性能优异且保留非目标先验。
...
### [105] [Multi-Scale ViT Inference with Habitat-Fit Priors and kNN Retrieval for Multi-Species Plant Identification](https://arxiv.org/abs/2607.14509)
*Alper Erten,Murilo Gustineli,Adrian Cheung*

Main category: cs.CV

TL;DR: 本文提出了用于PlantCLEF 2026挑战的第三名解决方案，通过微调DINOv2 ViT-L/14分类器，结合多尺度瓦片分解、FAISS kNN检索、源感知时间融合、栖息地适应降级和地理掩码，在只有单标签训练数据的情况下，实现了对植被样方高分辨率图像中多物种的识别，私有排行榜macro-F1为0.43902。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要多标签训练数据，而实际中植物识别常只有单标签个体图像，且野外植被样方图像存在复杂场景和多物种共存的挑战，亟需开发无需配对多标签数据的高效识别系统。

Method: 构建了基于微调DINOv2 ViT-L/14分类器的 pipeline，包括多尺度瓦片分解；每个瓦片的预测经FAISS kNN检索混合；通过源感知时间融合处理重复访视；栖息地适应降级注入地理和海拔先验，并使用西南欧地理掩码。同时探索了跨区域Transformer和标签查询Transformer，但无提升。

Result: 提交的配置在私有排行榜上达到macro-F1 0.43902，公共排行榜为0.51096；另一未选配置超过0.45；消融实验表明栖息地适应降级和多尺度聚合贡献最大，而实例感知分割增强等未提升性能。

Conclusion: 单标签训练数据可通过多尺度、时间融合和地理先验有效识别多物种植被图像；多尺度聚合和栖息地先验是关键模块；某些复杂方法（如跨区域蒸馏、实例分割）在此任务中无效，提示简单而精心设计的融合策略更有效。
...
### [106] [SwinAD: Multi-stage feature reconstruction for unsupervised industrial anomaly detection](https://arxiv.org/abs/2607.14534)
*Huong Ninh,Chien Thai,Mai Xuan Trang,Vu-Minh Le,Thanh Ha Le,Long Tran*

Main category: cs.CV

TL;DR: 提出了一种基于重构的多类无监督异常检测框架SwinAD，利用冻结的Swin Transformer V2编码器和特征多样性保持解码器，在MVTec AD等基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督方法主要针对单类场景，难以应对多类场景中多样正常模式导致的泛化问题，降低了正常与异常区域的区分能力。

Method: 使用冻结的Swin Transformer V2分层编码器提取多尺度特征，在瓶颈层引入带丢弃机制的逐阶段模块防止恒等映射，并通过两个解码分支保持重构假设的多样性，最后聚合多尺度差异生成异常图。

Result: 在MVTec AD、VisA和Real-IAD三个基准上，图像级和像素级定位精度均具竞争力，尤其在MVTec AD的像素级AP和1指标上提升显著。

Conclusion: 结合分层Swin特征与多样化多尺度重构可显著提升多类无监督异常检测的像素级定位性能。
...
### [107] [3D Geometric Tooth Alignment Planning via Deep Reinforcement Learning](https://arxiv.org/abs/2607.14544)
*Yong Li,Jianwen Lou,Jiayue Ma,Yao-Xiang Ding,Youyi Zheng,Haihua Zhu*

Main category: cs.CV

TL;DR: 提出基于深度强化学习的框架，自动生成3D牙齿正畸对齐路径，结合DDPG、Transformer、动态掩码和课程学习，在安全性和效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 3D牙齿几何对齐规划是数字化正畸的核心，但现有方法缺乏自动化，且需处理牙间碰撞等空间约束。

Method: 将规划过程建模为马尔可夫决策过程（MDP），使用DDPG算法，结合Transformer代理、动态掩码（限制每步移动少数组牙齿）和两阶段课程学习。

Result: 在10K专家设计治疗计划的数据集上，方法在路径安全和几何效率方面优于基线。

Conclusion: 提出的框架提供了鲁棒且自动化的3D正畸对齐规划解决方案。
...
### [108] [AdaTurn: Budget-Aware Test-Time Scaling for Active Visual Perception Agents](https://arxiv.org/abs/2607.14547)
*Susan Liang,Chao Huang,Filippos Bellos,Jing Bi,Jason J Corso,Chenliang Xu*

Main category: cs.CV

TL;DR: 提出AdaTurn框架，通过条件化预算和强制回答策略，解决视觉智能体在低预算下因截断导致回答失败的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法未考虑部署时不同预算限制，导致低预算下智能体未完成推理即被截断（灾难性截断）。

Method: 采用预算条件策略、强制回答DAPO（FA-DAPO）将超预算转化为训练步骤，并引入负载均衡调度器实现预算随机化。

Result: 低预算准确率显著提升（如VisualProbe-Medium在4步内从36.7%提高到47.6%），同时保持高预算性能。

Conclusion: AdaTurn有效缓解灾难性截断，在不同骨干和基准任务上表现稳健。
...
### [109] [HyMobileAgent: Data-Environment Co-Scaling for Efficient GUI Agents](https://arxiv.org/abs/2607.14548)
...
TL;DR: HyMobileAgent是一个基于Hy3.0-VL-A3B视觉原生模型的移动GUI代理，通过联合数据和环境中心缩放框架，解决了移动交互中的感知、数据获取和长期决策三大瓶颈。


<details>
  <summary>Details</summary>
Motivation: 移动GUI代理面临精确感知复杂界面、可扩展获取高质量交互数据以及鲁棒长期决策的挑战，现有方法多依赖模型缩放而忽视数据与环境的协同优化。

Method: 提出了联合数据和环境中心缩放框架，包含GUI感知飞轮（模拟界面合成、拒绝采样、图标增强）、知识管道（教程视频转结构化数据）、百万级动作数据管道（2000+沙箱/真机实例）、PhoneWorld模拟应用工厂（34个模拟应用+34000任务）以及结构化规划与反思机制（含死循环检测）；采用渐进式训练流程（中期训练、监督微调、强化学习+任务特定奖励）。

Result: 在34个模拟应用和超过34000个任务上验证了方法的有效性，实现了可靠的长期执行和错误恢复。

Conclusion: 通过数据和环境中心的缩放策略而非单纯模型缩放，HyMobileAgent在移动GUI任务上取得了性能提升，为数字具身智能提供了可行方案。
...
### [110] [Breaking the Model Forgetting Cycle in Long-Incremental 3D Object Detection](https://arxiv.org/abs/2607.14560)
*Peisheng Qian,Jie Xu,Xulei Yang,Na Zhao*

Main category: cs.CV

TL;DR: 提出LDMR框架，通过动态学习信号驱动记忆和回顾机制，解决增量3D目标检测中长序列下的模型遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有伪标签方法在长增量序列中因数据分布偏移导致自我强化遗忘循环，模型性能严重下降。

Method: 基于训练检查点的每类检测质量动态信号，驱动两种机制：内阶段回顾（子阶段训练聚焦最易遗忘目标）和跨阶段记忆进化（场景可学习性与多样性协调记忆库）。

Result: 在SUN RGB-D和ScanNetV2长增量协议上显著缓解遗忘，以明显优势超越所有基线方法。

Conclusion: 人类学习启发的动态记忆与回顾机制有效打破增量检测中的遗忘循环，实现长序列稳定学习。
...
### [111] [Advanced Image Generation: Negative Prompt Optimization and Latent Classifier Guidance](https://arxiv.org/abs/2607.14580)
*Vaddi Charan Sai Nandan Reddy,Harini B,Chandana M S*

Main category: cs.CV

TL;DR: 提出一种集成负提示优化和潜空间分类器引导的Stable Diffusion图像生成增强系统。


<details>
  <summary>Details</summary>
Motivation: 为解决Stable Diffusion图像生成中常见的伪影和语义保真度不足问题。

Method: 使用微调的序列到序列LLM自动生成优化负提示，并通过CNN-RNN混合分类器评估扩散步骤，回滚低质量潜更新。

Result: 双引导框架相比基线扩散模型减少了伪影，提升了语义保真度。

Conclusion: 该集成方法有效改善了生成图像质量。
...
### [112] [MagicPrompt: Ultra-Lightweight Prompt Tuning for Video Generation](https://arxiv.org/abs/2607.14595)
*Yinhan Zhang,Dinwei Tan,Xianghao Kong,Yue Ma,Yeying Jin,Anyi Rao*

Main category: cs.CV

TL;DR: MagicPrompt是一种轻量级框架，通过注意力嵌入提示调优和双空间奖励反馈优化，在视频扩散模型微调中实现极致的参数效率与稳定奖励优化，训练参数少于1%即可达到竞争性能。


<details>
  <summary>Details</summary>
Motivation: 大规模视频扩散模型全微调计算成本高昂，现有参数高效微调方法在十亿级模型中仍需大量可训练参数，且基于奖励的训练受噪声影响导致优化不稳定。

Method: 采用注意力嵌入提示调优（轻量软提示引导生成，保留预训练知识）和双空间奖励反馈优化（利用自监督潜在目标改善条件引导奖励训练）。

Result: 在少于1%可训练参数下达到竞争性能，显著降低训练成本。

Conclusion: MagicPrompt在参数效率和优化稳定性上优于现有方法，适用于大规模视频扩散模型的高效微调。
...
### [113] [Hough-SIFT: Robust Image Registration for Linear Structures via Hough Space](https://arxiv.org/abs/2607.14598)
*Masaki Satoh*

Main category: cs.CV

TL;DR: 提出Hough-SIFT，通过在霍夫空间进行SIFT描述子匹配，解决强线性结构场景下传统SIFT注册失败的问题。


<details>
  <summary>Details</summary>
Motivation: SIFT在强线性结构场景（如百叶窗）中因局部特征模糊而常失效，需要一种更鲁棒的配准方法。

Method: 在霍夫空间执行SIFT描述子匹配，利用线性结构形成独特峰值来恢复描述子区分性。

Result: 在线性场景中鲁棒性优于SIFT，同时在常规场景中保持与SIFT相当的精度。

Conclusion: Hough-SIFT有效解决了线性结构场景下的图像配准难题，且不影响正常场景表现。
...
### [114] [Knowing You at First Glance: Inferring Apparent Personality from Faces](https://arxiv.org/abs/2607.14631)
*Shuhuan Chen,Xiangyu Zhu,Weisong Zhao,Haichao Shi,Xiao-Yu Zhang,Zhen Lei*

Main category: cs.CV

TL;DR: 提出了GlanceFace框架，利用视觉-语言模型和语义增强的面部表示模块，从面部图像推断表观人格（尤其是MBTI类型），并通过不确定性学习处理标注噪声。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要依赖语言或多模态输入推断人格，不清楚仅凭面部线索能否支持有意义的人格关联，尤其是对MBTI类型。

Method: 端到端框架GlanceFace，包含视觉-语言模型引入语义先验、语义增强面部表示模块捕捉细微线索，以及不确定性学习策略处理主观噪声。

Result: 在MBTI表观人格基准上表现优异，揭示了面部特征与感知人格特质的关系。

Conclusion: 该方法能支持具身代理的适应性初始交互策略，展示了仅凭面部图像推断人格的可行性。
...
### [115] [Autoregressive Modeling of Film with Applications in Video Montage](https://arxiv.org/abs/2607.14645)
*Marcelo Sandoval-Castañeda,Fabian Caba Heilbron,Shiry Ginosar,Bryan Rusell,Josef Sivic,Alexei A. Efros,Greg Shakhnarovich*

Main category: cs.CV

TL;DR: FilmGPT 是一种自回归Transformer，通过学习电影中的“语法”从原始素材中自动选择最佳镜头，生成连贯视频序列，无需生成新帧。


<details>
  <summary>Details</summary>
Motivation: 解决将原始、无法观看的素材编辑成连贯电影序列的挑战，避免手工编码规则，直接从数据中学习编辑模式。

Method: 在大型电影语料库上训练长上下文自回归Transformer，推理时使用素材约束解码算法从输入素材中选择最佳下一镜头。

Result: 在镜头排序基准上达到最佳预测性能，用户研究表明全编辑任务显著优于先前方法，并适用于多种视频编辑应用。

Conclusion: FilmGPT通过数据驱动的自动编辑有效改善了视频蒙太奇，支持自动修剪和人机协作编辑等应用。
...
TL;DR: VIABench是一个专为评估多模态大语言模型在盲人辅助场景中性能的视频基准，包含三个核心任务，发现现有模型在实时预警任务中表现不佳。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在通用视觉语言任务中表现出色，但在实际盲人辅助中的实用性尚未被充分探索。

Method: 通过盲人第一人称视频定义三个任务（主动提醒、视觉问答、视觉引导交互），并设计支持在线和离线测试的严格评估流程。

Result: 当前MLLMs在盲人辅助任务上表现不足，尤其在需要实时预测的主动提醒任务中。

Conclusion: VIABench可推动定制化MLLMs的开发，以改善盲人导航和交互体验。
...
TL;DR: 提出ReBind框架，通过嵌入参考标记的语义指令实现多参考图像视频编辑，解决现有方法中视觉属性与来源绑定模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以准确协调多视觉源信息，且编辑指令缺乏显式参考关系，多模态大模型无法可靠生成。

Method: 设计ReBind-Instruct（两阶段渐进训练专用MLLM）和ReBind-Edit（轻量适配文本到视频模型），通过语义位置嵌入参考标记建立精确绑定。

Result: 在指令质量上显著优于通用MLLM，并在开源方法中达到多参考图像条件视频编辑的领先性能。

Conclusion: ReBind通过显式语义绑定有效提升多源视频编辑的准确性和协调性。
...
TL;DR: 本文提出GlobalForge框架，通过抑制局部伪影并强化全局结构推理，显著提升了AI生成图像检测器在现实传播退化条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有AIGI检测器在干净基准上表现良好，但经JPEG压缩、模糊等传播退化后性能骤降，原因是过度依赖易被破坏的局部伪影。

Method: 采用两模块框架：本地信息瓶颈（LIB）抑制局部特征防止捷径学习；全局结构推理（GSR）强制令牌从远距离区域收集证据，并通过退化对比结构损失联合训练。

Result: 在8个野外基准组上平均分类精度提升5.89%，在RealDeg-Bench的单重与复合退化测试中全面领先现有方法。

Conclusion: 从局部伪影转向全局结构是提升检测器鲁棒性的有效途径，GlobalForge在现实退化场景中取得最优性能。
...
TL;DR: 提出一种以文本为中心的多模态融合方法，用于视频中的矛盾与犹豫识别，在ABAW挑战的BAH语料库上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 矛盾与犹豫状态常通过不一致的多模态信号表现，现有高性能系统多依赖计算开销大的集成模型，亟需高效的单模型解决方案。

Method: 采用文本残差融合模型，以文本为锚模态，对其他模态特征进行门控残差调整，实现多模态融合。

Result: 在开发集和公开测试集上平均MF1达75.14%，私有测试集上MF1为78.24%，相比纯文本模型提升4.03%。

Conclusion: 互补多模态信息可提升识别性能，且无需大规模模型集成。
...
TL;DR: 提出了一种基于知识蒸馏的MIL预训练框架，通过两个WSI基础模型作为教师，使用角度散度归一化损失平衡监督，在15个基准数据集上验证预训练优于从头训练。


<details>
  <summary>Details</summary>
Motivation: 现有MIL聚合器从头训练依赖有限标签，易过拟合且迁移性差，缺乏可复用的预训练初始化。

Method: 使用TITAN和CARE作为教师模型进行知识蒸馏，设计角度散度归一化损失平衡多教师监督，预训练权重用于下游初始化。

Result: 在15个基准数据集上，预训练在线性探测和少样本场景下显著优于从头训练，保持轻量模型计算效率。

Conclusion: MIL预训练可提升聚合器性能，尤其适用于标签稀缺情况，为弱监督WSI分析提供有效初始化策略。
...
TL;DR: 提出了一种分时空注意力模型VideoSEMA，包含高效的Mamba式空间注意力和softmax时间注意力，在视频分类任务上优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 为了提升视频理解的效率与精度，同时降低计算成本，并解决传统全时空注意力方法的高计算复杂度问题。

Method: 设计SEMA模块（结合局部窗口注意力和全局平均的Mamba架构）用于空间维度，采用softmax时间注意力处理时间维度，并通过理论证明分注意力等价于全注意力。

Result: 在K400数据集上超越重量的Vision Transformer和Mamba模型；在SSv2上取得相似参数量模型中的最佳精度；图像分辨率从224^2升至1024^2时，性能下降比VideoMamba更平缓。

Conclusion: VideoSEMA在效率与精度间取得良好平衡，且可扩展至更长视频，具有广阔应用前景。
...
### [122] [Causal-Adversarial Probing of Clinical Covariates for Prostate MRI Grading](https://arxiv.org/abs/2607.14720)
*Yipei Wang,Shiqi Huang,Wen Yan,Weixi Yi,Dean C. Barratt,Mark Emberton,Daniel C. Alexander,Veeru Kasivisvanathan,Yipeng Hu*

Main category: cs.CV

TL;DR: 提出了一个因果推理框架，通过对抗性协变量抑制来分析前列腺MRI分级模型中对临床协变量的依赖，区分有害的快捷信息与有用的疾病相关信号。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型可能编码临床协变量，这些变量可能反映有用信号或非泛化的快捷信息，但其作用通常被假设而未经检验。

Method: 采用对抗性框架逐个抑制单个临床协变量的可解码性，同时保留MRI分级预测能力，并基于因果推理将MRI与ISUP等级视为潜在肿瘤病理的观测。

Result: 抑制年龄、BMI和饮酒可提升AUC（分别为1.23%、0.84%、1.42%），而抑制PSA和前列腺体积则降低AUC（分别为1.91%、7.61%），表明不同协变量的作用不同。

Conclusion: 对抗性协变量抑制能有效区分模型中有害的快捷信息与有用的信号，提升对深度学习泛化性的理解。
...
### [123] [Multimodality as Supervision: Self-Supervised Specialization to the Test Environment via Multimodality](https://arxiv.org/abs/2607.14721)
...
TL;DR: 提出测试空间训练（TST），在测试环境中收集多模态数据并自监督预训练，实现与大规模互联网数据集训练的通用模型竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 探索跨模态学习在受限测试环境中的有效性，减少对外部互联网数据集的依赖，受发展心理学中生物利用多模态构建表征的启发。

Method: 在受限测试环境中进行多模态数据收集，并执行自监督预训练（TST），学习跨模态映射。

Result: TST模型在相同环境下下游任务中达到与DINOv2、CLIP等通用模型竞争的性能；预训练数据量可权衡模型专化与泛化能力。
...
TL;DR: AE-UAV是首个空中拍摄的空中-空中事件相机跟踪基准，包含178个飞行序列，并提出了轻量级无训练框架FSFT，在仅有CPU的情况下实现420 FPS的跟踪速度，同时保持与GPU依赖方法相当的精度。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机空中-空中跟踪缺乏专用数据集，且现有方法依赖GPU和大量训练数据，难以在资源受限的无人机上实时运行。

Method: 提出了FSFT框架，融合频域模板匹配、搜索区域预测和基于检测的漂移校正，无需训练。

Result: FSFT在CPU上达到420 FPS，保持了SOTA GPU依赖方法93.97%的精度，加速比5.32倍，并具有良好的时间分辨率泛化能力。

Conclusion: AE-UAV数据集和FSFT方法为空中-空中无人机跟踪提供了高效、鲁棒且无需GPU的解决方案。
...
### [125] [WorkDrive: Roadwork Chain of Causation for Autonomous Driving](https://arxiv.org/abs/2607.14727)
*Tianyi Jiang,Wen Zhang,Sihan Yang,Ming Lu,Wentao Zhang*

Main category: cs.CV

TL;DR: 提出WorkDrive框架，通过感知驱动的因果推理与轨迹预测对齐，提升自动驾驶视觉语言模型在道路施工区的规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在施工区因缺乏明确引导，依赖预训练中的熟悉元素，无法正确连接观测与决策。

Method: 构建多任务感知流水线提取场景事实，注入Chain-of-Causation标注以聚焦领域特定元素，结合监督微调与基于横向元动作与轨迹一致性的强化学习。

Result: 在ROADWork数据集上，CoC使轨迹平均位移误差降低9.0%，一致性GRPO再降3.0%。

Conclusion: 感知因果推理可有效提升施工区规划性能，实现渐进式改进。
...
### [126] [VQ-Touch: A Data-Efficient Tactile Generation Framework Across Sensors and Scenarios](https://arxiv.org/abs/2607.14728)
*Kailin Lyu,Long Xiao,Jianing Zeng,Di Wu,Lin Shu,Jie Hao*

Main category: cs.CV

TL;DR: VQ-Touch是一种基于离散扩散的触觉图像生成框架，支持跨传感器和多场景应用，能高效生成高质量触觉数据。


<details>
  <summary>Details</summary>
Motivation: 现有触觉生成方法依赖特定传感器的大规模数据集，数据利用效率低且泛化能力差，尤其在视觉受限环境下表现不佳。

Method: 提出DM-VQGAN高效提取形变与纹理特征，结合离散扩散解码器和统一条件接口，通过少样本混合训练增强跨传感器泛化能力。

Result: 在多项任务中超越现有方法。

Conclusion: VQ-Touch为触觉数据合成提供了高效、泛化的解决方案，适用于机器感知和人机交互。
...
### [127] [GeoDetect: Geometric Adversarial Detection for VLPs](https://arxiv.org/abs/2607.14737)
*Afsaneh Hasanebrahimi,Hanxun Huang,Christopher Leckie,James Bailey,Sarah Erfani*

Main category: cs.CV

TL;DR: 本文提出GeoDetect方法，利用视觉-语言预训练模型（VLP）嵌入空间的各向异性几何结构，通过几何分数检测对抗样本，在多种攻击场景下表现出鲁棒性。


<details>
  <summary>Details</summary>
Motivation: VLP模型易受对抗攻击，但现有检测方法多针对单模态，在VLP等多模态模型中的有效性尚不明确。

Method: 分析VLP嵌入空间的几何各向异性，理论证明对抗样本会增大与随机采样点的期望距离，基于此设计几何分数检测偏离流形的样本。

Result: 在多种VLP架构和攻击设置（包括单模态、多模态和自适应攻击）下，GeoDetect可靠地检测出对抗样本。

Conclusion: 该方法提供了一种稳健实用的方式来提升VLP模型的安全性和可靠性。
...
### [128] [FoMoVLA: Bridging Visual Foresight and Motion Guidance for Vision-Language-Action Models](https://arxiv.org/abs/2607.14739)
*Wei Li,Peijin Jia,Yuan Ma,Xuefeng Jiang,Titong Jiang,Sheng Sun,Yujian Li,Xin Wen,Han Hong,Zhikang Liu,Bailin Li,Kun Zhan*

Main category: cs.CV

TL;DR: 本文提出 FoMoVLA 框架，通过联合学习未来特征预测和稀疏点跟踪，增强 VLA 模型的显式时空监督，提升连续动作策略性能。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型缺乏世界动态的前向预测，视觉预见方法仅提供目标位置而无运动指导。

Method: 引入紧凑未来令牌解码特征状态，解码稀疏时空 2D 点轨迹模拟几何运动，并通过未来条件交叉注意力模块耦合两者。

Result: 在 LIBERO、RoboCasa GR-1 Tabletop 和 LIBERO-Plus 上达到最先进性能，并展现强零样本泛化能力。

Conclusion: 未来特征预测与稀疏点跟踪互补，显式结合可显著提升 VLA 策略的连续动作规划与泛化性能。
...
### [129] [On the Disagreement in Perturbation-based xAI -- Benchmarking Perturbation Choices for Flood Detection from SAR Images](https://arxiv.org/abs/2607.14743)
*Anastasia Schlegel,Ronny Hänsch*

Main category: cs.CV

TL;DR: 扰动性xAI方法对参数（如图块几何和扰动类型）敏感，不同设置会导致模糊甚至矛盾的解释。


<details>
  <summary>Details</summary>
Motivation: 现有扰动性xAI方法虽简单易用，但参数选择对结果影响大，缺乏系统研究其一致性和忠实性。

Method: 以SAR影像洪水检测为案例，系统改变图块大小、形状和替换方案，通过视觉检查和一致性、忠实性评估分析扰动设置的影响。

Result: 不同扰动设置会产生显著差异，甚至导致矛盾的解释说明。

Conclusion: 扰动性xAI的参设置至关重要，需仔细检验和评估，并作为解释的一部分，以确保对模型预测的稳健理解。
...
### [130] [Clean-Reference Streaming Detection of Lens Occlusion and Photometric Transitions for Camera Tamper Monitoring](https://arxiv.org/abs/2607.14760)
*Bo Ma,WeiQi Yan,Jinsong Wu*

Main category: cs.CV

TL;DR: 提出了一种窄焦点的流式完整性监控器，用于检测摄像头纹理模糊和场景突变两种故障，通过滑动参考统计、结构光抑制和亮度抑制，在低误报率下实现可控诊断。


<details>
  <summary>Details</summary>
Motivation: 摄像头物理退化无声破坏下游数据质量，现有方法误报率高、计算受限且对光照变化不可解释。

Method: 比较采样亮度与局部梯度统计与滑动参考，应用粗网格结构光抑制和模式/快速亮度抑制，每次篡改事件最多发出一次通知，并通过状态机规则确保快速亮度抑制时场景突变路径不可达。

Result: 在320个受控序列中F1=0.800、平衡准确度=0.822；公共数据集上5%误报预算下部分AUC最高；0.025误报率下召回率0.925；9.09摄像头小时验证零误报。

Conclusion: 该方法更适合作为可审计的传感器健康子系统，而非通用篡改分类器，在限定范围内性能显著优于基线。
...
### [131] [Rare Concept Generation via Counterfactual Inference in Diffusion Models](https://arxiv.org/abs/2607.14765)
*Zhengyuan Jiang,Haipeng Liu,Meng Wang,Yang Wang*

Main category: cs.CV

TL;DR: 本文提出一种基于反事实推理的扩散模型（CI-Diff），通过阻断模型固有常识偏差，利用自然直接效应解耦罕见概念中的异常属性，显著提升罕见概念生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型因训练阶段存在常识偏差，难以生成具有非常见属性的罕见概念图像。

Method: 基于反事实推理，引入自然直接效应（Natural Direct Effect）和修正的无分类器引导机制，独立捕捉罕见概念文本提示对生成的直接影响。

Result: 在RareBench基准上，CI-Diff优于当前最先进扩散模型。

Conclusion: 首次将因果推理引入罕见概念生成任务，有效缓解常识偏差导致的属性生成错误问题。
...
### [132] [TAMF-VTON: Texture-Aware Mask-Free Virtual Try-On via High-Fidelity Image Synthesis](https://arxiv.org/abs/2607.14807)
*Jie Wang,Qian He,Gaofeng He,Xiaogang Jin,Huamin Wang*

Main category: cs.CV

TL;DR: 提出了一种无分割蒙版、纹理感知的虚拟试穿框架TAMF-VTON，支持多衣物同时转移，在15秒内完成推理。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的虚拟试穿方法依赖分割蒙版、纹理保留不足且不支持任意多衣物组合，难以在真实电商环境中部署。

Method: 包含三个关键组件：（1）基于混合专家（MoE）的轻量级微调方案；（2）频域监督机制优化高频纹理一致性；（3）自适应修补数据流水线生成高质量训练对。

Result: 在定量指标和感知质量上均优于现有方法，INT4量化下RTX 4090上单张图像推理时间小于15秒。

Conclusion: TAMF-VTON通过无蒙版操作、灵活多衣物组合和忠实纹理保留，提供了可扩展的商用虚拟试穿方案。
...
### [133] [An LLM-Based Automatic Sportscast Solution for Robot Soccer Matches](https://arxiv.org/abs/2607.14809)
*Francesco Petri,Michele Brienza,Daniele Nardi,Domenico Daniele Bloisi,Aldo Gangemi,Vincenzo Suriani*

Main category: cs.CV

TL;DR: 本文提出了一款完全自主的实时体育解说系统，通过神经符号架构将原始运动跟踪与自然语言生成结合，实现从视频流中提取精确统计信息并生成无幻觉的流畅解说，适用于2025年RoboCup比赛。


<details>
  <summary>Details</summary>
Motivation: RoboCup Soccer联赛需要持续衡量研究进展，而视觉统计是跟踪此类进展的关键手段，现有方法尚无法实现自动化的实时解说。

Method: 提出神经符号架构，连接原始运动跟踪与自然语言生成，从视频流中提取精确统计并将其转化为无幻觉的流畅解说。

Result: 系统可在直播和赛后分析中生成统计数据和解说，能适应联盟中不同大小的人形机器人共存的新动态。

Conclusion: 该系统为RoboCup比赛提供了自主、实时、无幻觉的解说和统计生成能力。
...
### [134] [Blurring Modal Boundaries: A Unified Survey from Single- to Multi-Modal Person Re-ldentification](https://arxiv.org/abs/2607.14821)
*Xiao Wang,Bing Wang,Bin Yang,Cuiqun Chen,Xin Xu,Mang Ye*

Main category: cs.CV

TL;DR: 该综述系统地回顾了行人重识别从单模态向跨模态与多模态范式的转变，涵盖可见光-红外、文本-图像、草图及非视距等任务，并提出了一种基于Transformer的可见光-红外基准框架。


<details>
  <summary>Details</summary>
Motivation: 传统单模态RGB方法受限于低光照、遮挡等环境挑战，需向跨模态与多模态发展以提升鲁棒性。

Method: 综述了跨模态（VI-ReID、TI-ReID、Sketch-ReID、NLOS-ReID）及多模态融合方法，并提出了基于Transformer的可见光-红外基准框架。

Result: 总结了现有数据集、挑战与方法，并展示了Transformer框架在捕获模态不变特征方面的潜力。

Conclusion: 跨模态与多模态融合是ReID的重要发展方向，未来应关注NLOS等新任务及更有效的特征融合策略。
...
### [135] [SUFLECA: Scaling Up Feature Learning for CAD-to-image Alignment](https://arxiv.org/abs/2607.15058)
*Saad Ejaz,Miguel Fernandez-Cortizas,Javier Civera,Holger Voos,Jose Luis Sanchez-Lopez*

Main category: cs.CV

TL;DR: SUFLECA是一种弱监督零样本CAD对齐方法，通过几何特征学习和一致性匹配算法，在RGB图像中高效估计物体9D位姿，在ScanNet25k上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有零样本方法依赖外观驱动匹配，在遮挡或仿真到真实领域迁移时准确率下降。

Method: 利用NOCs监督从12个数据集的674K图像中学习几何感知特征，并设计几何一致性匹配算法。

Result: 在ScanNet25k上达到33.4%/42.3%类别/实例准确率，比最强零样本基线提升10.3/12.2个百分点，甚至超越全监督方法。

Conclusion: 通过缩放几何特征学习和一致性匹配，实现精确高效的零样本CAD对齐，无需迭代优化。
...
### [136] [Physics-Informed Diffusion for Biomechanically Plausible 3D Sign Language Generation](https://arxiv.org/abs/2607.14836)
*Emanuele Colonna,Moises Diaz,Gennaro Vessio,Miguel Angel Ferrer,Giovanna Castellano*

Main category: cs.CV

TL;DR: 本文提出PIDiffSign，一种物理信息扩散模型，通过融入解剖约束（如骨骼长度一致性、关节角度合理性）和对比学习损失，显著提升手语生成中的语义保真度和运动真实性。


<details>
  <summary>Details</summary>
Motivation: 现有手语生成方法忽略生物力学约束，导致骨骼长度漂移、关节角违规等问题，影响运动真实性和语义可理解性。

Method: 采用Transformer编码器-解码器架构，解码器通过自适应零初始化层归一化条件化扩散时间步，并交叉关注手语词汇表示。可微几何模块强制骨骼长度和关节角度约束，训练结合人类形态学、运动学、角度、手指关节约束及对比性词汇-姿势对齐损失和无分类器引导。

Result: 在PHOENIX14T和CSL-Daily基准上，相比强扩散基线，姿态精度、关节角度正确性、分布真实性和反向翻译质量均有持续改进。

Conclusion: 物理信息扩散模型能同时提升手语生成的运动真实感和语义保真度。
...
### [137] [Rotational Motion-Induced Error Compensation for Phase-Shifting Profilometry-Based Eye Reconstruction](https://arxiv.org/abs/2607.14876)
*Seong-Jin An,Sanghoon Jeon,Yatong An,Jae-Sang Hyun*

Main category: cs.CV

TL;DR: 提出基于旋转运动补偿的相移轮廓术框架，用于动态3D眼球重建，通过眼球模型补偿帧间旋转误差，提升重建精度与稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统2D眼动追踪方法稳定性、精度有限；PSP在眼球旋转时产生运动伪影，需补偿。

Method: 利用用户特定3D眼球模型在球坐标中估计旋转运动，补偿像素偏移和相移误差；引入区域优化策略减少残留伪影。

Result: 在旋转假眼实验中显著抑制运动变形并提高重建精度；非球形刚体实验表明补偿方法不限于球形几何。

Conclusion: 该方法为PSP动态3D眼球重建提供了实用基础，有望用于高精度眼动追踪沉浸式环境。
...
### [138] [Selectivity Drives Efficiency: Dataset Pruning for Visual Place Recognition](https://arxiv.org/abs/2607.14897)
*Tong Jin,Yunpeng Liu,Shuyu Hu,Chun Yuan,Song Wang,Feng Lu*

Main category: cs.CV

TL;DR: 提出了一种针对视觉地点识别（VPR）的按地点剪枝框架，通过新定义的多样性（IPD）和相似性（IPS）指标保留信息量大的地点，实现高效训练。


<details>
  <summary>Details</summary>
Motivation: 现有VPR模型依赖大规模数据集，导致存储和训练成本高，而传统数据集剪枝方法忽略VPR中基于图像对的训练特性。

Method: 以地点为基本剪枝单位，提出两个互补指标：地点内多样性（IPD）和地点间相似性（IPS），联合评估每个地点的训练价值，构建紧凑核心集。

Result: 在不同剪枝比例下优于现有基线方法，在GSV-Cities规模的3.5倍数据集上剪枝后，用NetVLAD在MSLS-val和Nordland分别达到94.5%和97.0%的R@1。

Conclusion: 该框架能有效降低选择和训练成本，同时保持VPR模型的鲁棒性和判别性，具有实际应用价值。
...
### [139] [FlashDecoder: Real-Time Latent-to-Pixel Streaming Decoder with Transformers](https://arxiv.org/abs/2607.14898)
*Minguk Kang,Suha Kwak*

Main category: cs.CV

TL;DR: FlashDecoder是一种快速、内存高效的纯Transformer视频解码器，通过逐帧解码和滚动KV缓存实现恒定延迟流式处理，在保持重建质量的同时显著提升速度并降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 现有潜在视频扩散模型使用3D卷积解码器，在高分辨率或长视频场景下解码速度慢且内存占用大，需要更高效的解码方案。

Method: 提出FlashDecoder，采用纯Transformer架构，逐帧解码，每帧仅通过滚动KV缓存关注固定大小的历史帧窗口，强制时间因果性，无需显式注意力掩码。

Result: 在Wan2.1和Wan2.2潜在空间上，FlashDecoder在1080p分辨率下达到41.55dB vs 41.49dB PSNR的重建质量，解码速度提升3.6-4.7倍，内存减少最高11倍，架构优化后速度提升可达12倍。

Conclusion: FlashDecoder能在保持与卷积解码器相当的重建质量的同时，大幅提升解码速度和降低内存使用，适用于高分辨率实时视频生成。
...
### [140] [TanGO: Training-Free 3D Editing via Tangent-Space Guidance and Optimization](https://arxiv.org/abs/2607.14927)
*Siwoo Lim,Sunjae Yoon,Gwanhyeong Koo,Hyeonseo Yun,Chang D. Yoo*

Main category: cs.CV

TL;DR: 提出TanGO，一种无需训练的3D生成编辑框架，通过在生成动力学的切空间中进行自适应逐令牌控制，显著减少结构伪影。


<details>
  <summary>Details</summary>
Motivation: 现有基于流匹配的3D生成模型（如VecSet）采用结构化表示，但令牌共享全局上下文，导致传统免训练编辑出现语义伪影（如保留区域塌陷或变换不完整）。

Method: 在生成动力学的切空间中实现自适应逐令牌控制，通过一步最优控制规则确定控制信号强度，并使用基于冯·米塞斯-费舍尔分布的定向差异（从源和目标速度场导出）来调节每个令牌的控制强度。

Result: 实验表明TanGO显著减少结构伪影，性能优于现有3D编辑基线，达到最先进水平。

Conclusion: TanGO通过解耦全局上下文的干扰，实现了更精准的3D生成编辑，且无需额外训练。
...
### [141] [Benchmarking Face Recognition without Real Faces](https://arxiv.org/abs/2607.14932)
*Paweł Borsukiewicz,Daniele Lunghi,Wendkûuni C. Ouédraogo,Jacques Klein,Tegawendé F. Bissyandé*

Main category: cs.CV

TL;DR: 本文探讨了合成人脸数据集能否替代真实基准用于人脸识别评估。通过测试12个合成数据集与7个真实基准，发现MorphFace和Vec2Face能复现真实基准的相对行为，达到与真实基准间自然差异相当的一致性水平。


<details>
  <summary>Details</summary>
Motivation: 合成数据集已能训练出准确度匹敌真实照片模型的人脸识别模型，但评估仍依赖真实基准，隐私问题未完全解决。

Method: 使用24个预训练模型（涵盖卷积和Transformer架构），在12个合成数据集与7个真实基准上进行生物特征验证指标、相似度分布、跨模型排名一致性和分布属性测试。

Result: 两个最强合成数据集MorphFace和Vec2Face在重现真实基准相对行为时，其一致性水平落入真实基准间的自然差异范围内。

Conclusion: 精心构建的合成数据集可支持可靠的人脸识别比较评估，推动实现全合成、隐私保护的训练与评估流程。
...
### [142] [Still image and spatial-temporal tomato data enabling detection, segmentation, tracking, and video-instance segmentation using strong and weak labels](https://arxiv.org/abs/2607.14934)
...
TL;DR: 论文公开了两个用于番茄植株视觉感知的数据集：BUTom21（静态图像+人工标注）和BUTom-ST21（视频数据+半自动伪标签），均包含果实成熟度的像素级标注，旨在推动温室环境下番茄表型研究。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏商业化种植条件下的真实番茄图像和时空信息，限制了对番茄果实计数和成熟度估计的研究。

Method: 通过机器人获取数据，采用人工标注（BUTom21）和基于AI的半自动标注（BUTom-ST21）生成像素级成熟度标签。

Result: 提供了具有挑战性的真实场景图像集，其中时空数据集包含个体果实计数和成熟度信息。

Conclusion: 该数据集可推动野外表型分析的发展，为视觉传感方法的研究提供重要资源。
...
### [143] [VideoChat3: Fully Open Video MLLM for Efficient and Generalist Video Understanding](https://arxiv.org/abs/2607.14935)
...
TL;DR: VideoChat3是一个全开源、高效、通用的视频理解多模态大模型，仅4B参数便在多项基准上超越参数量更大的模型。


<details>
  <summary>Details</summary>
Motivation: 现有开源视频理解模型存在泛化能力弱、计算成本高、以及关键组件（训练代码、策略、数据集）未完全开放的问题，阻碍了社区发展和可复现性。

Method: 提出双管齐下的设计方案：1）效率方面，引入膨胀3D视觉Transformer(I3D-ViT)和自适应帧分辨率流式视频感知，减少训练推理成本；2）效果方面，构建可扩展视频数据合成流程，生成三类高质量数据集（学术、长视频、流式视频），提升跨域泛化能力。

Result: 在通用、长视频和流式视频基准测试中，VideoChat3以4B参数在多项任务上超越同等或更大规模的开源模型，实现了广泛的泛化性与高计算效率的平衡。

Conclusion: 全开源、高效且通用的VideoChat3有效解决了现有模型在泛化、效率和可复现性上的局限，为视频理解社区提供了坚实基础。
...
### [144] [Frequency-Structured Field Learning for Light-Field Disparity Estimation](https://arxiv.org/abs/2607.14941)
*Sara Monji-Azad,Yulin Liu,Jürgen Hesser*

Main category: cs.CV

TL;DR: 提出FreqLF方法，通过傅里叶-局部潜场学习实现光场视差估计，无需显式构建代价体，在HCI基准上接近监督方法性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖局部窗口、离散视差假设或高内存消耗的代价体/注意力机制，难以同时保证全局一致性与局部精度。

Method: 从水平和垂直EPI堆栈中提取角度视差线索，与中心视图特征融合为潜场；通过混合傅里叶-局部层交替更新全局/局部特征；利用坐标条件高斯混合解码器预测视差。

Result: 在HCI 4D光场基准上，FreqLF避免了显式代价体构建仍接近强监督基线；消融实验验证傅里叶与局部分支互补性；尺度实验展示实际分辨率行为。

Conclusion: 傅里叶-局部潜场学习是光场视差估计的有竞争力替代方案。
...
### [145] [Introspective Attention Modulation for Safe Text-to-Image Generation](https://arxiv.org/abs/2607.14945)
*Basim Azam,Hossein Rahmani,Naveed Akhtar*

Main category: cs.CV

TL;DR: 提出了一种基于推理时注意力动态调节的文本到图像模型安全方法，通过内省控制实现鲁棒安全，优于现有概念擦除技术。


<details>
  <summary>Details</summary>
Motivation: 现有流式文本到图像模型易生成不安全内容，而概念擦除、提示过滤等方法易被简单参数高效微调绕过，亟需内在鲁棒的安全机制。

Method: 在扩散Transformer的图像合成过程中，通过推理时内省分析并重新平衡注意力激活，将生成导向远离不安全概念，同时保持语义对齐。

Result: 在标准和对抗性安全基准上取得卓越安全评分，同时保持或提升生成质量与语义对齐性。

Conclusion: 注意力空间调节为扩散Transformer图像生成提供了比现有概念擦除更有效的安全路径。
...
### [146] [DINE: Distance Is Not Enough -- Learning Global Deformation Priors for Robust Soft-Tissue Point Cloud Registration](https://arxiv.org/abs/2607.14946)
*Sara Monji-Azad,Rohit Beer,Marvin Kinz,Claudia Scherl,Jürgen Hesser*

Main category: cs.CV

TL;DR: 提出DINE框架，通过最大后验估计结合位移场先验，提升非刚性点云配准的全局合理性，显著降低Chamfer距离并增强抗噪性。


<details>
  <summary>Details</summary>
Motivation: 现有学习型方法依赖局部目标（如Chamfer距离）不约束全局变形合理性，导致大变形、噪声和异常值下配准性能受限。

Method: 采用两阶段策略：第一阶段用Chamfer距离训练基础模型，第二阶段利用其预测位移场估计先验（主成分分析高斯先验或逐向量归一化流先验），再以组合损失（距离+负对数先验）微调模型。

Result: 在DeformedTissue上Chamfer距离降低27–69%，异常值鲁棒性提升66%，高斯噪声鲁棒性提升83%；SynBench上中等至严重变形下性能提升59–79%。

Conclusion: 全局变形合理性是软组织点云可靠配准的重要约束，所提先验框架有效提升配准性能。
...
### [147] [U-shaped Multi-granularity Learning for Vision-Language Models](https://arxiv.org/abs/2607.14966)
*Biao Chen,Yunqian Yu,Xiangxu Zhao,Zhongshu Chen,Mengmeng Jing,Lin Zuo*

Main category: cs.CV

TL;DR: 提出UPrompt框架，通过U形多粒度提示学习解决视觉-语言模型中的粒度困境，在密集预测任务中均衡全局与局部语义。


<details>
  <summary>Details</summary>
Motivation: 现有提示学习存在粒度困境：全局提示缺乏细粒度语义，局部提示忽略上下文关联，限制了跨任务泛化能力。

Method: 借鉴U-Net架构，构建对称编码器-解码器路径，包括粗到细级联增强（传播全局上下文以细化局部细节）和细到粗分层监督（确保跨尺度语义一致性）。

Result: 在17个基准测试中表现优异：MSCOCO上超过MAMET和VPKE（rSum提升4.1和7.3），基础到新型类别泛化超过CoCoA-Mix 5.09%，以最小开销保持竞争力（粗粒度），以1/3成本匹配PSRC（中粒度）。

Conclusion: UPrompt有效缓解了粒度困境，实现了高效且泛化的提示学习。
...
### [148] [On Success and Simplicity: A Second Look at Transferable Vision-Language Attack Pipeline](https://arxiv.org/abs/2607.14974)
*Yuchen Ren,Zhengyu Zhao,Chenhao Lin,Bo Yang,Chao Shen*

Main category: cs.CV

TL;DR: 本文提出SimVLA，一种针对视觉-语言预训练模型的简单而更有效的对抗攻击方法，通过解决交叉模态交互不当和操作过度的问题，显著提高了攻击的可迁移性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的可迁移攻击方法使用复杂的损失函数或多阶段设计，但忽略了交叉模态交互不当和过度操作的问题，导致性能受限。

Method: 提出SimVLA流水线，包括交叉模态词识别等简单操作，取代复杂的损失函数和多阶段攻击设计。

Result: 在Flickr30k数据集上，R@1可迁移性比SOTA基线提升8.01%-14.71%，仅消耗35.73%的时间和46.26%的显存。

Conclusion: 利用领域知识的简单设计比盲目追求复杂操作更有效，SimVLA可作为有效的未来扩展基础框架。
...
TL;DR: 提出一种实时、模型无关的推理框架Stitch-Inferencer，通过将观察到的图像缝合到全景画布上，显式扩展视野并解决遮挡问题，无需重新训练模型即可提升内窥镜视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型隐式存储时间信息，依赖特定任务训练、大量标注数据和更高计算成本；内窥镜手术中视野有限且器械频繁遮挡，需要更高效的时空上下文推理方法。

Method: 将历史帧的有效像素在线缝合到显式图像空间的全景画布上，生成无器械视野；下游模型仅对全景感兴趣区域推断，再将结果重投影回当前帧。

Result: 在解剖分割和点/盒跟踪任务中，多个基线模型均获得一致改进；缝合模块运行速度超过60 FPS，保持实时性。

Conclusion: 该框架可作为一种实用的推理时增强方案，在计算受限的术中环境中提升内窥镜感知能力，无需重新训练或增加标注需求。
...
TL;DR: 提出了从草稿到无草稿（D2DF）框架，通过蒸馏多步教师模型到一步学生模型，并结合自生成伪草稿模块，实现高效高质量的视频物体移除，单视频处理仅需约1秒。


<details>
  <summary>Details</summary>
Motivation: 现有视频物体移除方法存在伪影（传统方法）或效率低（多步扩散方法）的问题，需要一种既高质量又高效的方法。

Method: 使用教师模型多步精炼粗糙草稿，通过先验特权一致性蒸馏（PPCD）将能力蒸馏到一步学生模型；引入自引导快速植入（SGFP）模块，基于时序掩码Transformer在潜空间自动生成伪草稿，实现无草稿一步推理。

Result: 在多个指标上达到最先进性能，超越传统和多步生成方法，单视频去噪仅需约1秒。

Conclusion: D2DF框架（包括草稿条件版和无草稿版）在质量和效率上均优于现有方法，实现了实用的一步视频物体移除。
...
### [151] [Weakly-Supervised RGB-D Salient Object Detection via SAM-driven Pseudo Annotation and State Space Interaction-based Diffusion](https://arxiv.org/abs/2607.15041)
*Wenqi Si,Gongyang Li,Shixiang Shi,Weisi Lin*

Main category: cs.CV

TL;DR: 提出了一种基于SAM的伪标注生成和条件扩散模型的弱监督RGB-D显著目标检测方法。


<details>
  <summary>Details</summary>
Motivation: 为了减轻像素级标注负担，同时解决涂鸦标注缺乏结构和细节导致显著图不准确的问题。

Method: 包括SAM驱动的伪标注生成（SAM-PAG）和状态空间交互条件扩散模型（$S^2$Diff）。

Result: 在七数据集上优于相关弱监督方法，性能接近全监督方法。

Conclusion: SAM-PAG与$S^2$Diff的结合有效提升了弱监督RGB-D SOD的性能。
...
### [152] [JADE-GS: Joint Alternating Deblurring Guided by Events in 3D Gaussian Splatting](https://arxiv.org/abs/2607.14990)
*Haoyu Fu,Jiafeng Huang,Yuchen Wang,Shengjie Zhao*

Main category: cs.CV

TL;DR: 提出了一种名为JADE-GS的双向学习框架，通过像素自适应路由门融合物理学和学习的互补先验，并与3D高斯泼溅模型耦合，实现从模糊事件数据到清晰3D几何的恢复。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，物理和学习的先验在恢复模糊事件数据时各有缺陷（物理积累漂移，学习扭曲边界），且单向管道无法修正噪声或伪标签偏差，导致3D几何质量受限。

Method: 像素自适应路由门融合互补先验，与3D高斯泼溅学生模型构成双向循环；利用分离的多视图一致渲染和物理重模糊约束正则化恢复器。

Result: 在合成和真实基准上取得最佳感知质量（LPIPS和CLIP-IQA领先），PSNR和SSIM具有竞争力；单消费级GPU上约1小时训练，低于5GB内存，支持实时渲染。

Conclusion: JADE-GS有效解决了事件数据3D监督中的先验冲突和单向问题，实现了高质量、高效的几何感知恢复。
...
### [153] [Video = World + Event Stream](https://arxiv.org/abs/2607.15038)
...
TL;DR: 提出Wan-Streamer v0.3，将视频建模为“世界+事件流”，并基于此设计通用预训练任务，用于实时全双工音视频交互。


<details>
  <summary>Details</summary>
Motivation: 为了统一原生流式交互模型，将视频内容分解为稳定的世界和动态的事件流，从而支持广泛的实时下游任务。

Method: 定义视频为世界（持久上下文）和事件流（随时间变化的内容），设计预训练任务：给定世界和输入，预测世界如何实时变化；并实例化于全双工音视频交互，采用视觉-语言-动作多模态理解。

Result: 保持v0.2性能：640x368视频@25FPS，160ms流单元，约200ms模型响应延迟，约550ms总交互延迟（350ms双向网络预算下）。

Conclusion: Wan-Streamer v0.3通过世界+事件流视角，实现高效的实时音视频交互，且具备通用性。
...
TL;DR: 提出一种参数高效的MCI检测框架，使用冻结的DINOv2-Small模型和三个可学习提示令牌，通过交叉注意力图实现空间可解释性，并引入MoCA自适应焦点损失处理边界模糊性。


<details>
  <summary>Details</summary>
Motivation: 现有方法因数据稀缺、类别不平衡和临床边界诊断模糊而受限，且依赖计算昂贵的全调优混合架构，缺乏内在空间可解释性。

Method: 采用冻结的DINOv2-Small模型，通过三个模态特定的可学习提示令牌和共享交叉注意力层生成空间可解释性；使用MoCA自适应焦点损失整合连续认知分数。

Result: 在分层五折交叉验证中，MCI类F1得分为0.641，AUC为0.795，比ResViT基线提高0.110的MCI类F1。

Conclusion: 参数高效框架在提升MCI检测性能的同时，提供了内在空间可解释性，优于计算更重的基线方法。
...
### [155] [Beyond Single Expert: Harmonizing Diverse Visual Priors in MLLMs for Spatial Understanding](https://arxiv.org/abs/2607.15054)
*Xiao Lin,Xiaohu Huang,Kai Han*

Main category: cs.CV

TL;DR: 提出ViPS框架，通过融合多种视觉先验模型提升MLLM的空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只集成单个先验模型，但不同模型提供互补的空间先验，有助于不同任务。

Method: 提出高效先验代理生成多种基础先验，并设计动态先验融合机制实现上下文感知的融合与注入。

Result: 在多个复杂空间推理和3D空间理解基准上达到新SOTA。

Conclusion: ViPS成功协调多种视觉先验，显著提升MLLM的空间理解性能。
...
### [156] [Towards Hierarchical Structure Understanding of Newspaper Images](https://arxiv.org/abs/2607.15082)
...
TL;DR: 本文探索了两种互补方法（模块化流水线和端到端Tiramisu架构）来理解报纸图像的层级结构，并发布了新数据集Finlam La Liberté。


<details>
  <summary>Details</summary>
Motivation: 报纸图像具有复杂的嵌套层级和密集异构布局，现有方法难以有效处理其结构理解。

Method: 方法一为模块化流水线（YOLO+LayoutReader+自定义算法），方法二为新型端到端Tiramisu变换器，通过迭代层级过程显式建模文档结构。

Result: 实验表明两种方法在重建报纸层级方面均有效，对比分析突出各自在可扩展数字化中的优势。

Conclusion: 两种方法在报纸层级理解中互补，Tiramisu和数据集已开源。
...
### [157] [RoGS: Adaptive Meshgrid Gaussian for Large-Scale Road Surface Mapping](https://arxiv.org/abs/2607.15048)
*Tianchen Deng,Zhiheng Feng,Wenhua Wu,Ziming Li,Siting Zhu,Hesheng Wang*

Main category: cs.CV

TL;DR: 提出基于自适应网格高斯的鲁棒高效大规模道路表面映射框架，通过二维高斯曲面片建模道路，结合结构感知网格分配和轨迹一致性位姿优化，提升重建质量并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于网格的道路表面重建方法在大规模场景中重建质量有限且优化成本高，需要更高效鲁棒的表示方法。

Method: 在网格上放置二维高斯曲面片存储颜色、语义和几何信息；采用道路结构感知自适应网格策略，在复杂区域（车道线、边界等）分配更密集的高斯；引入轨迹一致性引导的位姿鲁棒优化，从多邻近位姿估计局部表面先验并加权高度正则化。

Result: 相比传统网格表示和三维高斯基元，该方法更匹配道路薄表面特性，减少冗余基元和重叠，提升表示效率和结构保真度。

Conclusion: ROADGS-T实现了大规模道路表面的高质量、高效鲁棒映射，适用于自动驾驶高精地图生成、车道级感知和自动道路标注等应用。
...
### [158] [Quantifying Training Membership Information in the Hyperspherical Embedding Geometry of Face Recognition Models](https://arxiv.org/abs/2607.15084)
*Ünsal Öztürk,Sébastien Marcel*

Main category: cs.CV

TL;DR: 本文研究了人脸识别模型中成员与非成员嵌入向量的几何差异，发现训练身份数量是影响这种可分离性的最大因素，而骨干网络和损失函数的影响较小。


<details>
  <summary>Details</summary>
Motivation: 现有角度间隔损失只作用于训练身份，非成员身份可能形成不同几何特性的簇，需量化这种差异及其控制因素。

Method: 在180个人脸识别模型上，通过因子设计（IResNet骨干大小、损失头、训练时长、训练身份数量）计算四种簇几何统计量，并在九个基准上评估。

Result: 训练身份数量对成员/非成员可分离性影响最大，骨干和损失函数贡献较小；同域参考上成员信号随身份数量增加单调递减；跨域基准（姿态、年龄、质量、种族）会夸大成员信号。

Conclusion: 通过融合所有四种统计量的学习分类器，可揭示比最佳单个统计量更多的成员信息。
...
### [159] [AlphaWiSE: Adaptive Weight Interpolation for Continual Multimodal Representation Learning](https://arxiv.org/abs/2607.15094)
*Sarthak Jain,Qiran Hu,Zhen Zhu,Yaoyao Liu*

Main category: cs.CV

TL;DR: 提出AlphaWiSE方法，通过权重空间插值组合两个冻结检查点，缓解多模态模型持续学习中的跨模态对齐遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 多模态模型在持续适应新数据时，早期学到的跨模态对齐会被破坏，而传统持续学习方法单一检查点无法平衡稳定性与可塑性。

Method: 对每个对齐参数张量拟合一个标量插值系数（来自少量记忆示例），组合两个冻结检查点生成单个插值检查点，不改变模型结构或增加推理时间。

Result: 在音频-图像-文本检索任务上，多个检索方向和评估指标均一致优于强持续学习基线。

Conclusion: AlphaWiSE通过后验权重插值有效缓解多模态持续学习中的灾难性遗忘，且无需额外推理开销。
...
### [160] [QuReC: All-in-One Image Restoration with Query-Specific Guidance and Local-Global Response Calibration](https://arxiv.org/abs/2607.15097)
*Shen Zhou,Jinghui Zhang,Wenbo Huang,Xuwei Qian,Zhen Wu,Guangwen Peng,Zhiyuan Li,Ding Ding,Dian Shen,Fang Dong*

Main category: cs.CV

TL;DR: 提出QuReC统一框架，解决全场景图像修复中空间异质或混合退化问题，通过退化引导查询重构和局部-全局响应校准实现最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法在处理空间异质或混合退化时不足，且空间适应性引导缺乏局部与全局信息聚合的可靠性。

Method: 构建退化原型空间，通过弱监督原型匹配进行查询特定重构（DQRM）；设计局部-全局双分支响应校准模块（LGRCM），结合可学习先验优化特征聚合。

Result: 在多个全场景图像修复基准上取得最优性能，代码已开源。

Conclusion: QuReC通过细粒度空间适应性引导和可靠的局部-全局特征聚合，有效解决了复杂退化场景下的统一修复挑战。
...
### [161] [Ray-based phase error correction for miniaturized DOE projector-based FPP under single-directional hyperbolic projection](https://arxiv.org/abs/2607.15139)
*Seung-Jae Son,Yatong An,Jae-Sang Hyun*

Main category: cs.CV

TL;DR: 提出基于投影光线的相位误差校正框架，解决迷你DOE投影仪FPP系统中的非线性投影伪影问题。


<details>
  <summary>Details</summary>
Motivation: 迷你DOE投影仪的非线性投影特性和有限图案可控性导致严重相位伪影。

Method: 沿投影光线建模相位伪影，使用单向双曲条纹估计投影机针孔几何，通过单标定姿态构建精修模型。

Result: 在迷你DOE投影仪FPP系统实验中显著提升重建精度，验证鲁棒性和物理一致性。

Conclusion: 该方法有效校正非线性投影条件下的相位误差，无需依赖图像域处理或邻域像素。
...
TL;DR: 提出了一种动态自适应物理引导图扩散网络（DAPGNet），通过注入结构约束的物理先验来改进高光谱图像分类中的关系图学习。


<details>
  <summary>Details</summary>
Motivation: 现有图分类器主要基于空间邻近性或特征相似性构建拓扑，而忽略了光谱连续波段携带的物理先验信息，限制了拓扑估计和消息传播。

Method: 采用多尺度物理先验编码、两阶段图构建器（结合光谱-空间亲和度、物理先验一致性和空间距离）、物理门控扩散（将边权重转化为注意力偏置，并插值图聚合特征与物理先验特征）、跨尺度融合及多任务优化（含二阶光谱平滑正则化）。

Result: 在四个数据集上（Indian Pines、WHU-Hi-LongKou、Houston2013、Houston2018）取得了最佳OA、AA和Kappa，AA比最强基线提升3.64~7.31个百分点。

Conclusion: 物理先验提取、先验感知拓扑、物理门控传播和光谱平滑正则化具有互补效果，显著提升了高光谱图像分类性能。
...
### [163] [MAGiSt3R: Multi-Agent Feed-forward 3D Reconstruction from Monocular RGB Videos](https://arxiv.org/abs/2607.15211)
*Ziren Gong,Xiaohan Li,Fabio Tosi,Ninghui Xu,Stefano Mattoccia,Jianfei Cai,Matteo Poggi*

Main category: cs.CV

TL;DR: 提出了一个多智能体3D重建框架MAGiSt3R，能以接近10 FPS的速度从单目RGB视频中进行重建和相机跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有基于前馈的3D重建方法面临累积相机漂移问题，且实时性能不足。

Method: 使用3R系列前馈模型处理视频并回归局部点云，通过MAGMA模型在智能体内和智能体间融合局部点云，并采用位姿图优化减少漂移。

Result: 在合成和真实数据集上，MAGiSt3R在重建和相机跟踪精度上优于现有技术。

Conclusion: 多智能体架构结合局部融合与位姿优化，可在保持高帧率的同时显著提升重建质量。
...
### [164] [Symbal: Detecting Systematic Misalignments in Model-Generated Captions](https://arxiv.org/abs/2607.15216)
*Maya Varma,Jean-Benoit Delbrouck,Sophie Ostmeier,Akshay Chaudhari,Curtis Langlotz*

Main category: cs.CV

TL;DR: 提出系统性错位检测任务、Symbal方法和SymbalBench基准，用于自动识别MLLM生成标题中的重复错误。


<details>
  <summary>Details</summary>
Motivation: MLLM生成的图像标题常出现与特定视觉特征相关的系统性错误，现有方法难以自动检测这类错误。

Method: Symbal使用现成基础模型的两阶段结构化流程，先检测系统性错位，再用自然语言总结结果。

Result: 在SymbalBench上，Symbal正确识别了63.8%数据集的系统性错位，性能是最近基线的近4倍。
...
### [165] [Structural-Semantic Reciprocal Learning for Unsupervised Visible-Infrared Person Re-Identification](https://arxiv.org/abs/2607.15220)
*Moyao Tian,Shijia Liu,Yan Yang,Xin Yuan,Minshi Chen,Wei Wang,Xiao Wang*

Main category: cs.CV

TL;DR: 提出结构-语义互惠学习（SSRL）框架，将开放循环关联转化为自校正闭环系统，通过细粒度结构解耦和闭环语义校准，解决无监督可见光-红外行人重识别中的模态差异和伪标签噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有渐进式关联方法依赖模糊全局表示且以开环方式传播伪标签噪声，导致性能瓶颈。

Method: SSRL包括：细粒度结构解耦（FSD）提取判别性身体部位原语作为空间锚点；闭环语义校准（CSC）在每个训练周期重构共享语义原型并反馈过滤噪声。

Result: 在SYSU-MM01和RegDB数据集上达到最先进水平，尤其在RegDB上超越部分有监督方法。

Conclusion: 结构学习与语义学习的互惠交互有效提升了无监督跨模态行人重识别的鲁棒性。
...
### [166] [Divergent Gaze Patterns in Artistic Viewing: Spatial and Temporal Signatures of Attention Across Autistic Individuals, Artists, and Neurotypical Observers](https://arxiv.org/abs/2607.15227)
...
TL;DR: 本文通过眼动实验比较了自闭症成人、训练有素的艺术家和神经典型者在自由观看30幅绘画时的空间和时间注视模式，发现自闭症群体具有独特的注意力特征。


<details>
  <summary>Details</summary>
Motivation: 现有自闭症眼动研究多关注社交场景而非艺术，且忽略注视顺序和时间动态，本文旨在填补这一空白。

Method: 采用空间轴（6种显著性指标）和时间轴（MultiMatch、ScanMatch、FDISS、DTW）的定向度量框架，统一使用离散阈值算法提取注视点。

Result: 艺术家与神经典型者在空间和时间上几乎不可区分，而自闭症群体在空间上探索更广但与艺术家不同，在时间上表现为更短注视、更少滞留和更多异常轨迹。

Conclusion: 自闭症对艺术观看具有独特的群体特异性注意力轮廓，支持建立群体条件化的审美注意力模型。
...
### [167] [CRISP: Constrained Refinement via Iterative Squeezing Process for Robust Medical Image Segmentation under Domain Shift](https://arxiv.org/abs/2607.15231)
*Yizhou Fang,Pujin Cheng,Yixiang Liu,Xiaoying Tang,Longxi Zhou*

Main category: cs.CV

TL;DR: 提出了CRISP框架，利用概率排名稳定性在分布偏移下提供稳健空间先验，无需目标域数据或测试时参数更新，在心脏MRI和CT肺血管分割中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医学成像中的分布偏移导致模型在实际部署中性能严重下降，现有域适应方法无法应对开放世界中无限的分布偏移。

Method: 基于"正区域的排名稳定性"假设，通过潜在特征扰动生成高精度核心和高召回支撑区域，并设计迭代训练框架逐步细化分割。

Result: 在多中心心脏MRI和CT肺血管分割中，HD95降低最高达8.39像素（38.9%改进），显著优于现有方法。

Conclusion: CRISP框架无需目标域数据或测试时适应，有效应对分布偏移，提升了医学分割的鲁棒性。
...
### [168] [ARMOR++: Agentic Orchestration of a Multi-Domain Primitive Set for Transferable Attacks on Deepfake Detectors](https://arxiv.org/abs/2607.15246)
*Christos Korgialas,Gabriel Lee Jun Rong,Dion Jia Xu Ho,Pai Chet Ng,Xiaoxiao Miao,Konstantinos N. Plataniotis*

Main category: cs.CV

TL;DR: 本文提出ARMOR++多智能体框架，利用视觉语言模型和大型语言模型协同驱动多种对抗攻击原语，显著提升了黑盒深度伪造检测器的可迁移攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测器在对抗迁移攻击下可靠性低，且传统迁移攻击缺乏语义感知，在无查询约束下效果差。

Method: 使用Qwen2.5-VL提供空间语义先验，Qwen3 LLM进行原语选择、超参数重参数化和熵正则化扰动混合，集成五种互补攻击原语。

Result: 在AADD-2025基准上，ARMOR++在低质和高质图像场景下均显著优于现有基线，盲目标攻击成功率大幅提升。

Conclusion: ARMOR++证明了智能体编排在发现深度伪造检测器潜在漏洞方面的有效性，揭示了当前检测器部署中存在的显著可靠性差距。
...
### [169] [HoloGeo: Mitigating Landmark Bias in Geo-localization via Evidence-Driven Reasoning](https://arxiv.org/abs/2607.15255)
*Pengcheng Zhou,Xuanyu Liu,Yanchen Yin,Bobo Li,Shengqiong Wu,Mong-Li Lee,Wynne Hsu*

Main category: cs.CV

TL;DR: 本文针对视觉语言模型在地理定位中的地标偏差问题，提出量化指标BI和BH，构建基准LandmarkBias-3K，并设计推理框架HoloGeo，通过多证据无偏推理链数据集BF-30k和多维奖励机制缓解偏差，显著提升定位鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在地理定位中受地标偏差影响，易忽视地理线索或产生虚假关联，导致定位不准确。

Method: 设计了BI和BH指标量化地标偏差，构建LandmarkBias-3K基准；提出HoloGeo框架，基于BF-30k数据集（含结构化多证据无偏推理链），通过多维奖励实现平衡注意力和证据驱动推理。

Result: HoloGeo在IM2GPS3K和YFCC4k上保持优异性能，并在LandmarkBias-3K上显著优于现有开源VLM。

Conclusion: HoloGeo有效缓解了地标偏差，验证了其鲁棒地理空间推理的有效性。
...
### [170] [SceneBind: Binding What and Where Across Vision, Audio and Language](https://arxiv.org/abs/2607.15265)
*Mingfei Chen,Zijun Cui,Ruoke Zhang,Hyeonggon Ryu,Eli Shlizerman*

Main category: cs.CV

TL;DR: 本文提出SceneBind，一种统一语义与3D空间理解的跨模态场景表示方法，支持视觉、音频和语言，通过对象级语义空间槽和全局语义嵌入实现场景检索与目标定位。


<details>
  <summary>Details</summary>
Motivation: 现有全模态编码器擅长实例级语义（什么存在），但缺乏空间结构（在哪里），因此需要一种同时捕捉语义和空间信息的场景表示。

Method: 构建语义-空间实体（全局语义嵌入+对象级语义空间槽），提出SceneBind Matching跨模态检索方案，并创建新的真实世界双耳视听数据集用于训练和评估。

Result: 在场景和空间检索任务上达到最先进性能，并实现强大的零样本迁移（如视听定位）。

Conclusion: SceneBind通过轻量空间建模（少量额外令牌）兼容大规模预训练语义编码器，有效弥补了语义-空间联合表示的空白。
...
### [171] [Online Neural Space Time Memory for Dynamic Novel View Synthesis](https://arxiv.org/abs/2607.15271)
...
TL;DR: 提出了一种通过解耦记忆更新与应用的频率，实现动态场景实时新视角合成的方法。


<details>
  <summary>Details</summary>
Motivation: 在线新视角合成面临实时性与长期记忆的矛盾，标准测试时训练方法因每帧梯度更新导致计算成本高、不稳定。

Method: 周期性记忆更新配合逐帧记忆应用，引入交叉视角注意力处理变形，辅助记忆损失和记忆缓存策略保持历史上下文。

Result: 在动态人体运动和分钟级在线记忆场景中实现了实时、最先进的性能。

Conclusion: 解耦记忆更新与应用频率能有效平衡实时性与长期记忆，适用于动态场景。
...
TL;DR: 提出了MCF-Net，一种运动引导的多视图融合框架，利用极稀疏监督的运动信息和基础模型表示进行心肌梗死节段定位。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖密集标注或单视图分析，在视图依赖模糊下节段定位不可靠，尤其是心尖视图。

Method: 使用预训练的心超基础模型EchoPrime提取双视图视觉特征；通过单模板帧转移初始化点追踪，实现极稀疏监督的运动建模；从运动中导出节段软掩码作为空间先验；设计运动条件融合机制跨视图整合运动与视觉信息。

Result: 节段水平MI定位F1得分72.4%，准确率84.9%，优于纯运动、纯视觉及融合基线。

Conclusion: 运动引导的多视图融合能有效提升梗死定位性能，减少对密集标注的依赖。
...
### [173] [MeanFlowNFT: Bringing Forward-Process RL to Average-Velocity Generators](https://arxiv.org/abs/2607.15273)
*Yushi Huang,Xiangxin Zhou,Jun Zhang,Liefeng Bo,Tianyu Pang*

Main category: cs.CV

TL;DR: 提出MeanFlowNFT，一种将强化学习应用于MeanFlow生成器的高效对齐方法，结合了均值流快速采样和奖励优化。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法（如DiffusionNFT）适用于瞬时速度模型，但MeanFlow使用平均速度，直接应用RL存在定义困难。

Method: 利用MeanFlow恒等式构建诱导的瞬时速度预测器，对其实施DiffusionNFT的目标函数，采样时仍使用平均速度。

Result: 在图像和视频生成任务中，MeanFlowNFT优于基线，且在SD3.5-M上6/8指标超越现有方法，4步采样在Wan 2.1上达到84.33 VBench分。

Conclusion: MeanFlowNFT兼具快速采样和严格策略改进保证，有效提升了MeanFlow的奖励对齐性能。
...
### [174] [Hierarchical Denoising For Multi-Step Visual Reasoning](https://arxiv.org/abs/2607.15278)
...
TL;DR: 提出HDR框架，通过分层潜在变量集成到因果视频生成中，实现多步推理，在推理一致性、速度和数据效率上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型缺乏类人多步推理能力，流式自回归扩散模型推理受限，双向扩散推理成本高，两者难以兼顾逻辑一致性和低延迟。

Method: HDR框架将视频潜在变量组织成树状层次结构，粗层保留不确定假设进行全局规划，细层逐步细化具体视觉状态，并采用稀疏层次注意力降低计算成本。

Result: 相比流式自回归扩散基线，成功率从34.22提升至60.29（相对增益76.2%），平均进度从76.00提升至89.56；推理延迟0.70秒/潜在，比双向扩散快54.2倍；仅用2%训练数据保留82.9%性能。

Conclusion: HDR在逻辑一致性、低延迟和数据效率方面表现优越，并展示了在真实机器人任务中的潜力，为视频基础模型的多步推理提供了有效框架。
...
### [175] [Just Keep Prompting: Evaluating Repetitive Socratic Prompting in VLMs](https://arxiv.org/abs/2607.14099)
*Shayda Moezzi,Bishoy Galoaa,Lorena Genua,Taskin Padir,Sarah Ostadabbas*

Main category: cs.CL

TL;DR: 提出JKP多轮评估框架，测量VLM在持续对话压力下的稳定性，发现重复提示会破坏推理稳定性，模型间存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有评估集中于单轮推理，忽略了真实场景中用户反复质疑、挑战模型答案的稳定性问题。

Method: 设计三种策略（否定对抗、苏格拉底式追问、上下文感知总结）构建最多10轮追踪评估，使用STAR基准测试3种模型共720次运行。

Result: 总体准确率变化不大，但轨迹分析显示严重不稳定：正确答案退步、错误答案恢复、答案反复翻转；Qwen3-VL准确率最高但更易自信出错，GPT-4o最脆弱，Gemini最稳定但价格高。
...
### [176] [Quantum Compositional NLP for Arabic: Grammar, Morphology, and Word Sense in Circuit Topology](https://arxiv.org/abs/2607.14100)
*Wajahath Mohammed*

Main category: cs.CL

TL;DR: 首次将基于预群语法的量子组合自然语言处理应用于阿拉伯语，通过量子电路建模其形态丰富、自由语序的句子，并与经典方法对比。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语形态复杂且语序自由，为量子意义组合理论提供了独特的高难度测试环境。

Method: 将阿拉伯语句子转换为量子电路，其中主语、动词、宾语对应量子门，预群语法决定的类型依赖关系连接这些门。

Result: 通过三项实验（词序、形态时态、动词歧义消除）比较量子电路方法与传统基线（AraVec和AraBERT）。

Conclusion: 该工作首次验证了预群语法量子组合方法在阿拉伯语上的可行性，展示了结构化语言建模的潜力。
...
### [177] [LBA: Textual Hard-Label Adversarial Attack under Low Query Budgets](https://arxiv.org/abs/2607.14101)
*Shixin Guo,Ming Zhong,Xuhong Zhang,Dandan Zhao,Zhe Wang,Bo Zhang,Shouling Ji,Hao Peng*

Main category: cs.CL

TL;DR: 提出了一种基于采样的硬标签对抗文本生成方法LBA，通过整合先验和后验知识构建高质量对抗样本的近似分布，并利用该分布进行高效采样，在低查询预算下生成高质量对抗文本。


<details>
  <summary>Details</summary>
Motivation: 现有硬标签对抗文本生成方法依赖贪心算法进行局部搜索，难以发现高质量对抗样本且查询成本高；而穷举所有位置组合在计算上不可行。

Method: LBA方法：通过整合先验和后验知识构建高质量对抗样本的近似分布，并基于该分布进行采样；随着采样进行，后验知识不断更新近似分布以指导更有效的采样。

Result: 在四个数据集上对六个语言模型（从小规模到大规模架构）的实验表明，LBA在所有评估指标上显著优于最先进的基线方法；LLM评估表明LBA生成的对抗文本语义保留更好且更易理解。

Conclusion: LBA方法能有效解决硬标签场景下低查询预算生成高质量对抗文本的挑战，优于现有方法。
...
### [178] [UniSAGE: Unifying Static and Dynamic Attributes with Hyper-Structure](https://arxiv.org/abs/2607.14102)
*Taoran Fang,Yan Deng,Chunping Wang,Yang Wang,Lei Chen,Yang Yang*

Main category: cs.CL

TL;DR: 提出了UniSAGE统一框架，通过全局属性图、正交参数子空间和轻量超结构机制，实现静态与动态属性的联合建模，在多个数据集上性能提升超10%。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手动设计、与数据模式紧密耦合，且孤立处理静态与动态属性，忽略了隐含交互。

Method: 构建全局属性图统一表示层次与时间关系；引入正交参数子空间实现静态聚合与动态推理；通过轻量超结构机制实现任务特定交互。

Result: 在多个公开基准和金融数据上，UniSAGE优于现有方法，多项任务提升超10%。

Conclusion: UniSAGE实现了自动化、鲁棒且能捕捉复杂跨属性依赖的异构数据统一建模。
...
### [179] [Latent Communication Between Language Model Agents: Channels, Alignment, and the Limits of Text](https://arxiv.org/abs/2607.14103)
*Markus Wenzel*

Main category: cs.CL

TL;DR: 本研究通过稀疏自编码器（SAE）特征分析，量化了LLM代理在文本通信中的信息丢失，发现文本序列化破坏了88%的SAE特征，且潜在通信通道在简单概念任务上未优于文本通道，最终对初始假设得出负面结论。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统依赖文本通信，但大型语言模型可能拥有超越文本表达能力的复杂概念世界模型，作者希望验证这一假设。

Method: 构建三种通信通道（文本、密集潜在、SAE稀疏潜在），使用Procrustes对齐进行跨架构潜在空间对齐，通过特征生存分析、检索准确率和任务级评估量化信息损失。

Result: SAE稀疏通道在28倍压缩下保持99.4%的探针准确率，文本通道仅80.4%；跨架构对齐（Llama和Mistral）的Top-1检索准确率达92%；但文本序列化破坏88%特征，且潜在通道在跨语言概念任务中未超越文本通道。

Conclusion: 丢失的特征主要编码表面形式而非任务相关语义，文本增强潜在特征无益处，需更复杂任务和对应分析框架才能体现潜在通信优势。
...
### [180] [UzWordnet and Generative AI for Learning Uzbek by Game Playing](https://arxiv.org/abs/2607.14104)
*Alessandro Agostini,Saydobid Khusanov,Mirkamol Mirkamilov*

Main category: cs.CL

TL;DR: 提出一个通过游戏学习乌兹别克语的教育系统架构，结合UzWordnet、正字词典和生成式AI，设计四款教育游戏，同时改进词汇资源。


<details>
  <summary>Details</summary>
Motivation: 现有乌兹别克语学习资源和词典资源有限，且缺乏互动式学习工具，研究者希望通过游戏整合词汇资源与AI来同时实现语言学习和词典改进。

Method: 构建系统架构，集成UzWordnet、正字词典和生成式AI，设计四款教育游戏，并基于游戏动态提出改进UzWordnet的方法。

Result: 成功实现游戏驱动的语言学习与词汇资源优化的双重目标。

Conclusion: 该架构有效结合游戏设计与词汇资源，可用于教育目的和词典丰富。
...
### [181] [Automatically Evolving Prompt Guidelines for Task-Specific Optimization](https://arxiv.org/abs/2607.14105)
*Cedric Richter,Salah Ghamizi,Mike Papadakis*

Main category: cs.CL

TL;DR: 提出AGOPS方法，自动生成任务特定提示指南，帮助用户编写更明确的提示，显著提升大语言模型在下游任务中的效果。


<details>
  <summary>Details</summary>
Motivation: 用户查询常因未明确指定需求、上下文和约束而导致模型推断错误意图，现有提示工程指南通用且缺乏系统性，难以实用。

Method: AGOPS利用已有任务示例（参考答案）隐式编码缺失信息，通过提示LLM编写器、求解器LLM和提示演化优化方案，自动生成任务特定指南。

Result: 提示不明确导致下游任务性能下降高达95.3%，且现有优化技术难以恢复；而AGOPS指南可恢复损失，平均性能提升15.5%至81.7%。

Conclusion: 自动生成的任务特定提示指南能有效解决提示不明确问题，显著提升LLM在不同任务中的可靠性和效果。
...
### [182] [Token Time Continuous Diffusion for Language Modeling](https://arxiv.org/abs/2607.14106)
*Parikshit Bansal,Sujay Sanghavi*

Main category: cs.CL

TL;DR: 提出了一种新的连续空间扩散语言模型TTCD，通过引入每个token的时间步长，在高速生成时优于离散模型。


<details>
  <summary>Details</summary>
Motivation: 离散空间模型在高速并行采样时误差较大，且缺乏对token间差异化建模的能力。

Method: 连续空间扩散模型，每个token具有独立的时间步长，从高斯噪声确定性地映射到token，无需额外采样。

Result: 160M参数模型在OpenWebText上自蒸馏后，无条件生成质量与同类模型相当，条件生成和数独求解任务上均表现更优。

Conclusion: TTCD在高速生成场景下优于离散模型，尤其适合条件生成和结构任务。
...
### [183] [Polestar: Drift-Aware Cache Calibration and Token Commitment for Efficient Inference of Diffusion LLMs](https://arxiv.org/abs/2607.14107)
*Mingyu Lee,Akshat Ramachandran,Souvik Kundu,Tushar Krishna*

Main category: cs.CL

TL;DR: 提出了Polestar框架，通过检测token表示漂移来同时优化KV缓存重用和解码并行性，在数学与编程基准上实现了精度与吞吐量的帕累托最优。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型因双向注意力机制导致KV缓存无法高效重用，且静态置信度阈值会损害生成质量，而两者源于相同的token表示漂移现象。

Method: 基于训练无关的Polestar框架，包含Polestar-Cache（检测漂移位置并稀疏刷新KV缓存）和Polestar-Commit（检测剧烈漂移事件识别可提交token）。

Result: 在多个dLLM上，实现最高10.73%精度提升、3.7倍吞吐量提升、每前向传递解码3.67个token。

Conclusion: token表示漂移是dLLM效率瓶颈的统一信号，Polestar无需训练即显著提升推理效率与质量。
...
### [184] [Eta Given Delta: Defining LLM Tool Efficiency With Marginal Tool Utility](https://arxiv.org/abs/2607.14108)
*Nyx Iskandar*

Main category: cs.CL

TL;DR: 本文提出了工具效率这一新指标，用于评估LLM代理轨迹中有用工具调用的比率，并引入边际工具效用来确定每个工具调用的有用性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注改进LLM工具使用技术或利用准确率间接衡量效率，缺乏直接量化工具效率的方法。

Method: 定义工具效率和边际工具效用两个指标，通过LLM作为裁判判断每个工具调用的边际效用符号。

Result: 提出了直接测量工具效率的定量方法，可后验分析代理轨迹中工具调用的效用。

Conclusion: 该工作为未来基准设计和代理工程优化（如精简工具集）提供了补充准确率的新评估维度。
...
### [185] [Simplicity Paradox: Debunking myths about prompting and datasets for LLM evaluation](https://arxiv.org/abs/2607.14109)
*Inder Preet,Shuxin Lin,Dhaval Patel*

Main category: cs.CL

TL;DR: 对8种提示技巧在10个MCQA数据集上的大规模实证研究，发现基础提示法在多数情况下优于复杂推理技巧，仅少数专家或归纳框架带来轻微提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究常声称复杂提示技巧更优但缺乏全面评估，作者因此系统性探究不同提示方法的实际效果。

Method: 在27种模型配置上，对约4300个问题进行了43万次评估，比较了8种提示技巧（包括基线、CoT及其他复杂方法）在10个MCQA数据集上的表现。

Result: 基础提示法在多数基准上优于或持平复杂方法（如自类比技巧表现差至31个百分点）；仅有CoT-Expert和CoT-Inductive带来约3个百分点的微小提升；发现Qwen3-30B-A3B-Thinking-2507在Elo评分中意外获胜，以及数据集难度差异显著（准确率跨度达47.5个百分点）。

Conclusion: 提示工程可能被过度复杂化，模型改进空间更多来自基准多样性而非提示优化。
...
### [186] [MAPS: Modeling Co-Existing Subjective Perspectives and Shared Meaning in Multi-Agent Cognitive Dialogue](https://arxiv.org/abs/2607.14110)
*Molood Arman,Clément Bonnafous*

Main category: cs.CL

TL;DR: 提出MAPS框架，通过多智能体视角空间实现对话中的语义对齐与主观性保持。


<details>
  <summary>Details</summary>
Motivation: 现有AI对话系统强制语义统一，缺乏多样性和可解释性。

Method: 基于领域加权配置、动态GRU记忆和可解释token级注意力的多智能体框架。

Result: 在EmpatheticDialogues等数据集上实现语义对齐而不丧失主观性。

Conclusion: 为可解释、认知基础的对话系统提供了新的路径。
...
### [187] [Introspection Fine-Tuning (IFT): Training Small LLMs to Introspect](https://arxiv.org/abs/2607.14111)
*Ely Hahami,Ishaan Sinha,Lavik Jain*

Main category: cs.CL

TL;DR: 该论文研究了小语言模型能否内省自身内部激活的扰动，并提出无混淆的评估范式（句子定位和强度比较），发现2B及以上模型表现优于随机水平，而1B模型通过内省微调（IFT）可从9.6%提升至60.6%。


<details>
  <summary>Details</summary>
Motivation: 先前工作使用二元检测范式存在混杂因素（steering会偏向肯定回答），因此需要更可靠的评估方法来验证模型的内省能力。

Method: 提出句子定位和强度比较两种无混淆评估范式，并引入内省微调（IFT），即利用模型自身受扰动的正向传播构建监督数据。

Result: 2B及以上模型内省能力显著高于随机；1B模型IFT后句子定位准确率从9.6%提升至60.6%，且零样本泛化到强度比较任务（30.2%→52.2%），3B/8B模型也有提升，能力基准几乎无损失。

Conclusion: 内省能力不仅随模型规模增长，还可通过直接训练习得，有助于提升AI透明性和对齐性。
...
### [188] [Information-Theoretic Limits of Reliability and Scaling in Language Models](https://arxiv.org/abs/2607.14112)
*Subhabrata Majumdar*

Main category: cs.CL

TL;DR: 本文从信息论角度证明，大语言模型在生成任务中存在无法突破的可靠性上限，该上限由上下文可消解的不确定性和任务固有歧义共同决定。基于此提出一个基础缩放定律，并统一解释了检索增强、灾难遗忘等现象。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM时默认只要规模足够大就能达到完美可靠性，但作者认为这在信息论上不成立，因此需要从理论上明确LLM的性能极限。

Method: 使用信息论分析，引入“输出不确定性可消解量”和“任务依赖核”（量化输出中令牌间相关性），推导出性能缩放定律。

Result: 发现性能瓶颈来自训练数据或模型容量中更稀缺的资源；该定律可退化为Chinchilla缩放定律；统一解释了检索增强、灾难遗忘的谱机制等现象。

Conclusion: 生成式语言模型的性能受限于资源-复杂度权衡，本文为理解其性能极限提供了统一的理论框架。
...
### [189] [CoEvoT: Co-Evolving Chain-of-Thought Prompting for Graph-LLM Reasoning](https://arxiv.org/abs/2607.14114)
*Haohua Niu,Xingtong Yu,Yang Liu,Junfeng Fang,Xuanting Xie,Jie Tan,Zhongjian Zhang,Hong Cheng,Yuan Fang*

Main category: cs.CL

TL;DR: 提出CoEvoT框架，通过文本-图令牌重写与图-文本推理指导的闭环，实现逐步状态感知的证据精炼，在8个数据集上超越SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有CoT方法在图LLM推理中固定图令牌，无法逐步优化结构信息。

Method: CoEvoT框架：文本令牌更新图令牌状态（轻量条件网络），更新后令牌反馈至下一步指令。

Result: 8个数据集上一致超越现有最先进方法。

Conclusion: 动态迭代的图令牌更新能有效提升分布偏移下的图学习性能。
...
### [190] [T5-CSBoost: Adversarial Perturbation Resistant LLM Fingerprinting](https://arxiv.org/abs/2607.14113)
*Gayan K. Kulatilleke,Mahsa Baktashmotlagh,Siamak Layeghy,Marius Portmann*

Main category: cs.CL

TL;DR: 提出T5-CSBoost方法，通过对比风格正则化提升AIGT检测器的鲁棒性，在多种攻击和分布偏移下保持SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有AIGT检测器在轻度改写、字符编辑和分布偏移下性能严重下降，需要更鲁棒的检测方法。

Method: 在T5-Sentinel框架上扩展，保持原有下一词预测目标，引入基于三重损失的辅助对比风格正则化，无需修改主干网络或额外对抗训练。

Result: 在OpenLLMText和HC3数据集上达到SOTA多源归因和人类vs.LLM检测性能，对高达90%强度的对抗扰动保持鲁棒，在MAGE/Deepfake测试集上包括未见模型、领域和极端改写场景均取得SOTA。

Conclusion: 通过对比学习显式正则化风格嵌入是构建更鲁棒LLM指纹识别系统的实用有效策略。
...
### [191] [ReportMedSAM: Guiding Segmentation Through Radiology Reports](https://arxiv.org/abs/2607.14116)
*Anghong Du,Theodoros N. Arvanitis,Colin Watts,Alejandro F. Frangi,Le Zhang*

Main category: cs.CL

TL;DR: 提出了ReportMedSAM，一个基于报告驱动的分割框架，通过可学习的概念库替代离散提取，结合对比学习对齐器官概念与临床语料，并采用动态MoE模块实现零干扰扩展。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预定义短语或基于规则的推理时提取，难以扩展到新解剖结构，且对语言变异敏感。

Method: 使用冻结的BiomedCLIP进行对比学习，构建互为正交的器官概念锚点，推理时通过概念匹配动态激活MoE模块，支持参数隔离的新概念和专家添加。

Result: 在AbdomenAtlas 3.0上有效解释自由文本报告，达到竞争力分割精度，并实现无缝、无干扰的新临床任务扩展。

Conclusion: ReportMedSAM通过可学习概念库和MoE设计，显著提升了报告驱动分割的鲁棒性和可扩展性。
...
### [192] [Heterogeneous Element-Aware Cross-Version Differencing of Scientific Documents via Layout-Aware Alignment and Structure-Aware Reasoning](https://arxiv.org/abs/2607.14117)
*Zhen Yina,Wenkang An,Hao Wang,Keran You*

Main category: cs.CL

TL;DR: 提出一种布局感知、异构元素感知的科学文档差异比较框架，统一支持文本、表格、公式、图形的变化检测、定位与结构分析。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么丢失布局和结构信息，要么缺乏语义可解释性且对渲染变化敏感，无法满足科学文档跨版本差异比较的挑战。

Method: 将文档版本分解为语义类型元素，通过先对齐的机制建立跨版本对应，联合建模空间、内容和结构兼容性，并对对齐元素对进行类型感知的差异推理。

Result: 在真实PDF数据上，对文本、表格、公式、图形的检测F1分别达0.903、0.855、0.862和0.845，并在定位、结构意识和匹配质量上持续优于基线。

Conclusion: 异构元素感知的差异比较为科学文档在现实编辑生产场景中提供了鲁棒且可解释的解决方案。
...
### [193] [Budgeted Subset Refinement for Execution-Aware LLM Research Ideation](https://arxiv.org/abs/2607.14118)
*Micah Zhang*

Main category: cs.CL

TL;DR: 提出预算分配的子集细化策略，在有限资源下提升LLM生成研究创意的组合质量与多样性。


<details>
  <summary>Details</summary>
Motivation: LLM生成的研究创意虽看似新颖，但缺乏多样性、难以可靠评估且执行效果差，需优化资源分配以构建更好的创意组合。

Method: Budgeted Subset Refinement策略族，包括均匀细化、随机k选择和多样性感知的MMR-k细化。

Result: 随机k细化为低预算基准，MMR-k细化在非重复创意产出、重复率和性价比上最优；外部裁判验证了细化效果的稳健性。

Conclusion: LLM研究创意系统应作为预算分配系统评估，而非仅作为创意生成器；结果限于代理评分，需专家或执行验证。
...
### [194] [Semantic Register Compression in Multi-Agent LLM Cascades](https://arxiv.org/abs/2607.14119)
*Manuele Tele Junior Fernandez*

Main category: cs.CL

TL;DR: 多智能体LLM系统中的语义注册压缩现象导致下游决策准确度下降，本文通过三智能体流水线量化该现象，发现关键评估阶段标签可分离性平均降低41.7%。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统虽然通过角色分解简化复杂任务，但中间智能体在文本转换时可能系统性压缩语义区分性，影响下游决策准确性，这一风险尚未被充分研究。

Method: 采用收集器-评估者-决策者三智能体流水线，利用句子变换器嵌入空间中的类间分离度量化压缩；通过五种架构变体因果隔离定向语义变换的影响。

Result: 在政治事实核查（LIAR）、情感分析（SST-5）和医学分诊（Triagegeist）中，评估阶段压缩分别达到41.7%、27.2%和20.0%；提示级回归解释78%方差，操作约束与较低压缩相关。

Conclusion: 语义注册压缩是可测量且可泛化的多智能体系统失败模式，其程度受变换效价和操作约束影响，对高风险领域的安全评估具有重要启示。
...
### [195] [Cross-Dataset Generalization in Urdu Fake News Detection: An Empirical Study with XLM-RoBERTa and a Length Confound Analysis](https://arxiv.org/abs/2607.14131)
*Muhammad Abdullah Haroon*

Main category: cs.CL

TL;DR: 首次系统研究乌尔都语假新闻检测的跨数据集泛化，发现因文章长度差异导致的零样本迁移性能严重不对称（反向迁移F1仅0.005），并通过消融实验确认长度混淆问题。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语使用者超2.31亿，但假新闻检测资源匮乏，且现有研究忽视跨数据集泛化。

Method: 微调xlm-roberta-base模型，在Ax-to-Grind和Notri-Fact两个数据集上进行域内训练和零样本跨域迁移，以TF-IDF+逻辑回归/SVM为基线。

Result: Notri-Fact→Ax-to-Grind迁移宏F1为0.771，反向则降至0.005（模型将99.7%测试样本预测为假），异常源于Ax-to-Grind中假新闻平均长度（117词）是真新闻（35词）的3.4倍。

Conclusion: 长度混淆导致模型学习捷径，在预测中过度依赖长度特征；通过双向迁移分析和预测崩溃检查可识别此类混淆，长度截断（50词后）F1仅降0.0067。
...
### [196] [Breaking Refusal in the First Half: A Mechanistic Study of the Prefill Jailbreak](https://arxiv.org/abs/2607.14147)
*Alex Kwon*

Main category: cs.CL

TL;DR: 论文发现，对齐语言模型在特定前缀（'Sure, here is'）的引导下会放弃拒绝有害请求的能力，尽管内部危害表示仍然完整。拒绝是一种浅层、响应层面的计算，主要发生在响应的前半部分。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型拒绝机制为何在简单前缀攻击下失效，以及这种失败的内在机制和位置。

Method: 使用线性探针、因果干扰（包括位置控制、注意力剔除）和状态注入等方法，在多个模型家族和规模上分析行为拒绝与内部表示之间的差异。

Result: 拒绝行为在早期响应窗口被破坏，但危害表示保持高激活；恢复该方向的激活可部分恢复拒绝；注意力剔除实验表明前缀的抓取是一种通用的自回归条件作用机制，而非安全特定的抑制。

Conclusion: 拒绝机制是分布式的且集中在响应早期，安全监控可以通过仅读取提示侧表示来绕过响应层面的攻击，但无法防御其他类型的攻击。
...
### [197] [Implicit Reasoning Steering via Concept Chaining](https://arxiv.org/abs/2607.14242)
*Xiao Ye,Sanika Chavan,Yuxi Huang,Shahriar Kabir Nahin,Muhao Chen,Anshuman Chhabra,Ben Zhou*

Main category: cs.CL

TL;DR: 本文提出概念链方法，通过自然语言文本间接引导大语言模型的答案偏好，揭示了推理脆弱性可被利用产生隐蔽的决策偏移。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多次采样中表现出推理脆弱性，即同一问题可能输出正确或错误答案，研究这种脆弱性能否被间接利用。

Method: 概念链：生成连接问题和目标选项的简短段落，利用一两个中间概念，然后对受害模型继续预训练这些段落。

Result: 间接、自然的文本能系统性地改变模型答案偏好，且比直接改写更难被检测出。

Conclusion: 推理脆弱性不仅是评估伪影，还创造了实际通道，可通过普通文本放大潜在偏见并隐蔽地重定向模型决策。
...
### [198] [The Severance Problem: LLMs are Unaware of the Person Beyond the Prompt](https://arxiv.org/abs/2607.14250)
*Dor Litvak,Liu Leqi*

Main category: cs.CL

TL;DR: 本文提出Severance Problem，即语言模型缺乏对用户未知信息的显式表征，导致奉承、过度自信和幻觉。通过Severance Schema结构化的无知表示，减少了不良行为。


<details>
  <summary>Details</summary>
Motivation: 当前AI助手在个人化场景中持续出现奉承、幻觉等不良行为，根源在于模型无法显式表征用户未知信息。

Method: 提出Severance Schema，在语言模型上下文中显式标注六类未知维度（物理性、时间性、后果、连续性、多样性、内在性），结构化呈现模型对用户的未知信息。

Result: 在五个模型系列中，使用Severance Schema后，助手一致减少了奉承、有害建议和幻觉，并在信息缺失时主动提出澄清问题而非盲目推断。

Conclusion: 结构化无知表示是解决AI助手因缺失用户表征导致不良行为的有效方法。
...
### [199] [Multi-Head Latent Control: A Unified Interface for LLM Agent Decision Making](https://arxiv.org/abs/2607.14277)
*Amirhosein Ghasemabadi,Ruichen Chen,Bahador Rashidi,Di Niu*

Main category: cs.CL

TL;DR: 提出了多头潜在控制（Multi-Head Latent Control），一种轻量级层，通过读取冻结的LLM/VLM的隐藏状态轨迹来生成部署时的控制信号，显著提升计算效率与决策质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示路由、外部编排或任务特定微调的方法依赖输入侧信号，成本高且难以维护；需要一种能从模型内在生成过程直接推断控制决策的方法。

Method: 在冻结的LLM/VLM之上添加能力头（Capability Head）和分辨率头（Resolution Head），前者预测模型是否能独立解决问题，后者决定澄清、工具使用、弃权或直接回答的决策；头仅基于冻结骨干的潜在痕迹训练。

Result: 在路由执行中，将大型模型使用率降低高达90.7%（AndroidWorld）和平均27-53%；工具使用决策质量相对得分提升+158%，缺失必要工具调用减少65.5%。

Conclusion: 多头潜在控制能有效改善多模型系统的质量-成本权衡，实现早期任务移交和更准确的干预决策。
...
### [200] [PReM: Learning What to Preserve and When to Refresh for Context Compression](https://arxiv.org/abs/2607.14327)
*Bohan Yu,Lei Shen,Chenxi Zhou,Chen Han,Junlin Liu,Wenbo Su,Yu Cheng,Bo Zheng*

Main category: cs.CL

TL;DR: PReM是一种通过内部KV记忆层动态保留和刷新上下文信息的框架，在32K上下文下以16-32倍压缩实现优于基线的推理质量与效率。


<details>
  <summary>Details</summary>
Motivation: 现有压缩方法（如KV缓存或上下文压缩）在生成过程中难以动态调整保留的上下文，导致后续推理步骤缺乏所需证据。

Method: 使用专用记忆层选择保留信息，并通过特殊token <m>触发刷新；采用分阶段隔离的刷新训练（Phase-Separated Refresh Training）对齐记忆选择与生成。

Result: 在32K上下文实验中，PReM在16倍和32倍压缩下均优于强基线，平衡了答案质量与推理效率。

Conclusion: 动态记忆管理（而非静态压缩）能有效提升长上下文推理的适应性，且训练策略对维持连续性至关重要。
...
### [201] [HABIB_TAZ at SemEval-2026 Task 11: Disentangling Formal Logic from Content via Synthetic Training and Multi-Objective Optimization](https://arxiv.org/abs/2607.14349)
*Abdullah Shaikh,Zain Naqi,Taha Zahid,Sandesh Kumar,Abdul Samad*

Main category: cs.CL

TL;DR: 提出基于mDeBERTa-v3的多任务学习系统，在SemEval-2026任务11上对12种语言进行逻辑推理，通过合成数据和多目标损失函数消除内容偏差，在三个子任务上取得完美成绩。


<details>
  <summary>Details</summary>
Motivation: LLM在形式推理中因内容效应产生现实偏差，需要开发能脱离语义噪声、专注逻辑结构的稳健推理模型。

Method: 使用mDeBERTa-v3网络，在基于规则的三段论合成数据集上微调，采用结合自适应组分布鲁棒优化、可微分偏差惩罚和KL散度一致性正则化的多目标损失函数。

Result: 子任务1-3取得第一名，零偏差和100%精度；子任务4排名第六，准确率89.06%，偏差2.89%

Conclusion: 合成数据结合多目标训练能有效解耦逻辑与内容，但面临含干扰前提的多语言复杂场景时仍有改进空间。
...
### [202] [MamaBench: Benchmarking LLM Robustness in Maternal and Child Health Diagnosis through Counterfactual Clinical Perturbation](https://arxiv.org/abs/2607.14385)
...
TL;DR: 提出了MamaBench基准和EA-RAG方法，证明大语言模型在医学问题上的准确率不可靠，EA-RAG降低了偏见陷阱率但仍有20%的残余错误。


<details>
  <summary>Details</summary>
Motivation: 现有医学基准孤立评估每个问题，无法衡量系统区分临床表现相似但干预不同的疾病的能力。

Method: 构建了包含434个临床叙事（217对）的MamaBench基准，定义偏见陷阱率（BTR）度量；提出证据锚定RAG（EA-RAG）方法，通过临床参数提取、覆盖审计和对比子查询的三阶段检索替代聚合相似性。

Result: 在四种前沿LLM的八种配置中，基础准确率高估稳健准确率16-28个百分点；EA-RAG在Claude Sonnet 4.6上将BTR降至20.3%，稳健准确率达65.0%，BTR降低5.5个百分点且不降低基础准确率。

Conclusion: 临床AI的反事实稳健性仍是一个开放挑战，EA-RAG提供了初步缓解但无法完全解决。
...
### [203] [DS@GT ARC at LongEval: Citation Integrity and Factual Grounding in Scientific QA](https://arxiv.org/abs/2607.14400)
*Brandon Michaels,Brendon Johnson*

Main category: cs.CL

TL;DR: 该论文介绍了DS@GT ARC在CLEF 2026 LongEval任务4中关于检索增强生成（RAG）的提交，重点比较了传统评估指标与引用完整性之间的差异，并评估了纠错管线的效果。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于传统自然语言评估指标与RAG问答系统中引用完整性之间的不一致性，特别是前沿模型虽得分高却未正确使用检索到的上下文。

Method: 采用了纠错RAG（CRAG）和CiteFix的纠错管线，与基线和前沿模型的基准RAG问答得分进行对比，并使用RAGAs LLM-as-judge诊断工具进行评估。

Result: 前沿模型在答案相关性和流畅性上得分最高，但诊断显示它们未在生成答案时使用相关文档的上下文；而纠错管线通过预生成过滤和后生成强制推理链接，略微提高了引用忠实性和答案基础性。

Conclusion: 作者提出，评估可信赖的RAG问答需要奖励严格答案基础性的指标。
...
### [204] [Smarter and Cheaper at Once: Byte-Exact KV-Cache Grafting Turns a Frozen Small Model into a Verified-Knowledge Flywheel](https://arxiv.org/abs/2607.14431)
*Sietse Schelpe*

Main category: cs.CL

TL;DR: 本文提出了一种方法，通过冻结的小语言模型在不改变权重的情况下，注入验证过的知识作为字节精确的KV状态，使得模型能力提升且计算成本大幅降低。


<details>
  <summary>Details</summary>
Motivation: 旨在同时提升冻结小语言模型的能力和效率，解决传统微调或扩展模型规模的高成本和低效问题。

Method: 将验证知识以字节精确的键值对（KV）状态形式存储，并在推理时通过“移植”（graft）方式注入到新上下文中，确保输出与重新计算完全一致（字节级相等）。

Result: 在AIME 2025上，Gemma-4-12B精度从80.0%提升至93.3%，超越其31B版本；对于8个基模型在40万token预算内无法解决的问题，仅用61个token和约8700倍能量节约完成；可用上下文从32,768 token扩展至2,854,766 token且零额外显存。

Conclusion: 字节精确的KV状态移植是一种可行且高效的方法，能显著提升冻结模型的性能并降低计算资源消耗，且在不同规模模型和硬件上验证了数值精确性。
...
### [205] [LLM Evaluators are Biased across Languages](https://arxiv.org/abs/2607.14480)
*Ej Zhou,Lucas Resck,Zheng Hui,Anna Korhonen*

Main category: cs.CL

TL;DR: 在多语言环境中，LLM评估者（如奖励模型和LLM作为裁判）在不同语言间存在显著的评分偏差，低资源语言被评分更慷慨，但这一偏差在成对准确率中不可见。


<details>
  <summary>Details</summary>
Motivation: 验证多语言评估者是否真正语言中立，发现成对准确率不能揭示语言偏差。

Method: 对23种语言的语义相同指令-响应对进行实验，使用八种开源评估者和前沿评估者，分析评分偏差与语言资源水平的关系，并探究偏差原因。

Result: 评估者在不同语言间评分差异显著（接受率差达43%），低资源语言得分更高，但成对准确率超过90%；偏差与语言资源水平强相关，且模型不确定性部分解释了该现象。

Conclusion: 语言偏差是结构性的，不能通过成对准确率或内容难度解释，需要新的评估方法来确保多语言公平性。
...
### [206] [Controlled Reformulation Testing for Logical Consistency in Large Language Models](https://arxiv.org/abs/2607.14528)
*Alexander Gu,Alan Chen*

Main category: cs.CL

TL;DR: 提出CRTBench基准测试评估LLM的逻辑不变性，发现模型在逻辑等价改写下一致性低，最高仅96.9%。


<details>
  <summary>Details</summary>
Motivation: LLM在面对逻辑等价但形式不同的问题时频繁出现自相矛盾的现象，现有评估仅关注准确率，忽略了逻辑一致性。

Method: 构建包含350个问题族（共1750个问题）的基准测试，涵盖逆否命题、双重否定、否定翻转、被动语态等四种改写类型，评估多个前沿模型。

Result: GPT-5.4-mini基准确率98.9%但一致性仅60.3%，o4-mini一致性96.9%；失败集中在逻辑复杂变换（逆否命题72.4%、双重否定84.6%），表面改写则稳健；增加推理努力对一致性提升效果有限。

Conclusion: 仅依靠准确率不足以评估LLM的逻辑推理能力，需同时关注一致性，尤其需改进模型对逻辑复杂变换的处理。
...
### [207] [CityLLM: A framework for natural-language querying of semantic 3D city models](https://arxiv.org/abs/2607.14542)
*Rabindra Lamsal,Sisi Zlatanova,Johnson Xuesong Shen*

Main category: cs.CL

TL;DR: 提出CityLLM框架，通过自然语言查询语义3D城市模型及补充数据集，结合空间数据库和图数据库与LLM工作流，在多场景下实现高正确率与零失败查询。


<details>
  <summary>Details</summary>
Motivation: 语义3D城市模型结构复杂、格式专业，非专家和跨学科研究者难以访问和查询。

Method: 结合空间数据库与图数据库，设计基于LLM的工作流，支持迭代查询精炼和跨数据库链接。

Result: 在Rotterdam数据集上，答案正确率85.2%-100%，可视化正确率92.9%-100%，查询成功率100%，重试次数少于3次。

Conclusion: CityLLM提供轻量级、可扩展的方法，实现语义3D城市数据的对话式访问。
...
### [208] [MARS: Multi-hop Adaptive Retrieval and SPARQL Generation for KGQA](https://arxiv.org/abs/2607.14561)
*Nikit Srivastava,Daniel Vollmers,René Speck,Nikolaos Karalis,Hamada M. Zahera,Axel-Cyrille Ngonga Ngomo*

Main category: cs.CL

TL;DR: 提出MARS，一种无需微调的可扩展KGQA方法，通过结构化检索与自适应图遍历生成SPARQL查询，在多个基准上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在知识密集型任务中易产生幻觉，结合知识图谱可提供可更新的显式符号知识，但现有方法依赖昂贵的微调或不可预测的自主探索。

Method: MARS执行结构化检索，将问题实体链接到知识图谱后迭代检索下一跳信息，每步决策继续遍历或生成最终SPARQL查询，自适应调整检索深度。

Result: 在三个KGQA基准上，多种大语言模型和设置下（包括多语言评估）取得与最先进方法竞争的性能，且保持高效可扩展。

Conclusion: MARS通过可预测的结构化流程替代完全自主探索，在无需微调下实现高效、可扩展且表现优异的KGQA，代码和资源已公开。
...
### [209] [Answer-Conditioned Chains of Thought Degrade Verifiable-Reasoning Distillation in Large Language Models](https://arxiv.org/abs/2607.14552)
*Jungseob Lee,Seungyoon Lee,Suhyune Son,Dongyub Jude Lee,Sungbin Han,Sugyeong Eo,Heuiseok Lim*

Main category: cs.CL

TL;DR: 本文发现，在蒸馏大语言模型推理能力时，在生成链式思维时显示正确答案会导致训练数据退化，即使经过正确性过滤也无法挽回，并可能使模型在困难问题上的准确率下降高达27个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法在采样失败时，常通过展示正确答案并要求模型写出推导链来改进，但作者怀疑这种答案条件化生成方式会损害数据质量。

Method: 设计控制实验，固定生成器、问题集和正确性过滤器，仅改变链是否在答案条件化下生成（即展示正确答案并要求推导），并训练同一模型进行对比。

Result: 答案条件化生成链的训练显著降低可验证推理准确性，难度越大损失越大（高达27点）；退化可从未标记链中检测出来，且跨模型家族转移；机制源于理性化指令而非答案可见性。

Conclusion: 实际启示是应进行答案盲生成，因为任何正确性过滤器都无法捕捉这种数据损伤。
...
### [210] [How Well Does AI-Generated Feedback Work? Intrinsic and Extrinsic Evaluation across more than 20,000 EFL Essay Drafts](https://arxiv.org/abs/2607.14591)
*Steven Coyne,Diana Galvan-Sosa,Ryan Spring,Machi Shimmei,Michael Zock,Keisuke Sakaguchi,Kentaro Inui*

Main category: cs.CL

TL;DR: 本研究通过近2000名学生和2万篇草稿的实验，对比了教师评分与学生反馈，发现两者低度对齐，强调以学习者为中心的AI反馈评估框架的重要性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）能大规模提供书面纠正性反馈（WCF），但如何使反馈符合教学实践并真正从学习者视角具备实用性仍是一大挑战。

Method: 在EFL课堂部署WCF系统，收集2万余篇草稿；采用内在评估（教师根据评分标准）和外在评估（学生反馈和参与指标）两种方式。

Result: 教师专家评分与学生反馈之间存在低度对齐，表明仅依赖传统专家评估不足以评估WCF的可用性。

Conclusion: 需建立以学习者为中心的评估框架来评价AI在语言教育中的应用，确保WCF真正服务于学习需求。
...
### [211] [Investigating first-language bias in LLM-based automated essay scoring: A cross-prompt evaluation of an open-weight AI-model on TOEFL essays](https://arxiv.org/abs/2607.14605)
*John Maurice Gayed*

Main category: cs.CL

TL;DR: 研究评估了LoRA微调的Gemma-3-27B-it模型在自动作文评分中的跨提示泛化能力和第一语言评分偏差。


<details>
  <summary>Details</summary>
Motivation: 验证开源LLM在自动作文评分中的跨提示泛化性及是否存在L1相关的评分公平性问题。

Method: 使用480篇两主题议论文微调的LoRA模型，在TOEFL11语料库（12100篇、8个未见提示、11种L1背景）上进行评分，将原始分数映射到三个熟练等级。

Result: 整体等级一致率77.79%，二次加权kappa为0.702，相邻等级一致率99.98%；跨提示表现稳定；但存在系统性L1相关偏差：同一等级内欧洲语言背景作文得分显著高于东亚语言背景。

Conclusion: 模型跨提示泛化能力强，但表现出与训练数据组成无关的L1评分偏移，这是首个大规模LLM自动评分公平性分析。
...
### [212] [Routing Ceilings Are Domain-Independent: Structural Prior Injection in Code Security Vulnerability Detection](https://arxiv.org/abs/2607.14628)
*Manuel Israel Cázares*

Main category: cs.CL

TL;DR: 本文验证了SAIR中关于结构先验（cheatsheets）在域内提升性能但引起域外崩溃的发现，在源代码安全漏洞检测领域也得到了验证。


<details>
  <summary>Details</summary>
Motivation: 验证SAIR中观察到的跨分布权衡现象是否在源代码安全领域普遍存在，测试路由器假说的跨域适用性。

Method: 复现SAIR设计，在源代码安全漏洞检测任务中评估GPT-OSS-120B、Llama-3.3-70B和Gemma-4-31B三个模型，涵盖三种漏洞类型（CWE-798、CWE-284和N+1反模式），并将cheatsheet增强提示迁移到VUDENC中的真实CVE数据。

Result: 结构先验将语义漏洞召回率从20.0%提升至100.0%；零样本性能沿语义复杂度梯度下降；相同cheatsheet在合成数据上达到100% F1，在真实CVE数据上降至48.9%（CWE-89，-51.1pp）；迭代重校准生成的v2 cheatsheet在真实数据上表现差于v1。

Conclusion: SAIR中记录的跨分布权衡表面在代码安全领域普遍适用，路由器假说具有跨域特性；结构崩溃的本质支持进行分布感知训练而非提示校准。
...
### [213] [D-cut: Adaptive Verification Depth Pruning for Batched Speculative Decoding](https://arxiv.org/abs/2607.14647)
*Tianyu Liu,Yuhao Shen,Rui Cen,Junhan Shi,Jiebin Zhang,Guangshuo Qin,Hong Liu,Song Liu,Guanghua Yu,Jianchen Zhu*

Main category: cs.CL

TL;DR: D-Cut 是一种自适应剪枝方法，通过批量选择草稿令牌并集中验证预算于高接受概率令牌，在高并发场景下提升投机解码的加速效果。


<details>
  <summary>Details</summary>
Motivation: 现有并行草稿方法在高并发时因拒绝令牌浪费计算，导致投机解码可能比自回归解码更慢。

Method: 利用跨请求置信度进行自适应剪枝，并引入运行时成本模型调整剪枝深度以适应部署环境。

Result: 在高并发下，平均加速比从1.26倍提升至1.65倍，在密集模型中恢复加速效果，MoE模型上最高达3.0倍加速。

Conclusion: D-Cut 有效解决了高并发下的效率退化问题，显著提升投机解码的性能。
...
### [214] [Harnessing LLMs for Reliable Academic Supervision: A Comparative Study](https://arxiv.org/abs/2607.14707)
*Akash Raj*

Main category: cs.CL

TL;DR: 本文提出了一种'框架工程'方法，通过将确定性脚手架（如符号过滤、检索、LLM-as-judge循环等）与大语言模型核心结合，在学术监督领域显著提升了系统可靠性。实验表明，使用较小模型（GPT-4o-mini）并附加框架的系统（ASuS），在六个评估维度上全面优于无框架的GPT-5基线（ASA），平均得分4.08 vs 1.23。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在单次提示中表现流畅，但作为可靠决策系统组件时存在显著差距。学术监督领域要求高可靠性、可追溯性和机构一致性，现有'更大模型更好'的直觉难以满足这些需求。

Method: 构建多模块系统ASuS，包括：语义-符号检索、架构验证输出、LLM-as-judge有限重试、人在回路门控、确定性加权风险评分与LLM叙述、SQLite审计追踪。采用混合评估（10名盲审员）和2x2模型-框架消融实验。

Result: ASuS（GPT-4o-mini）在所有六维（基础性、可解释性、一致性、流程完整性、认知负荷、约束遵循）上超越ASA（GPT-5）。10名评审员中8人Wilcoxon检验拒绝零假设（α=0.05），消融实验证实框架贡献具有模型不变性。

Conclusion: 在可靠性、可追溯性和机构一致性优先于开放流畅性的场景中，框架工程挑战了'更大模型更好'的直觉。提炼出七种通用框架工程模式。
...
### [215] [Gold-Guided Programmatic Distillation for Financial Reasoning over Hybrid Tables and Text](https://arxiv.org/abs/2607.14709)
*Yun Dong,Erica Zhao,Elana Chen*

Main category: cs.CL

TL;DR: 提出一种基于执行验证的程序蒸馏方法，通过可执行Python程序替代自然语言推理，将大模型数值推理能力迁移至小模型，在TAT-QA上7B模型超越72B教师模型。


<details>
  <summary>Details</summary>
Motivation: 自然语言推理步骤易产生算术错误，无法作为可靠的蒸馏监督信号，需探索更可靠的数值推理蒸馏方法。

Method: 利用黄金推导指导教师模型生成Python程序，仅保留执行正确且输出正确答案的程序用于监督学生模型；引入迭代恢复阶段，重新处理教师失败的样本以扩充训练数据。

Result: 7B学生模型在TAT-QA测试集上达到87.00 EM/87.18 F1，显著超越72B教师模型（78.46 EM）及TAGOP、TAT-LLM等基线。

Conclusion: 执行验证的程序蒸馏为训练小型模型实现可靠数值推理提供了有效且可扩展的框架。
...
### [216] [The Misclassification of Autistic Writing as AI-Generated](https://arxiv.org/abs/2607.14729)
*Summer Chambers,Matthew C. Kelley*

Main category: cs.CL

TL;DR: 该研究实证检验了AI检测模型对自闭症作者的偏见，发现尽管仅少数帖子被标记为AI生成，但自闭症相关子语料库的被标记率显著更高。


<details>
  <summary>Details</summary>
Motivation: 以往研究指出AI检测模型对少数群体可能存在偏见，而自闭症作者的写作风格可能被误判为AI生成，但缺乏实证支持。

Method: 收集约6万条Reddit帖子，分为“疑似自闭症”和“普通Reddit”两个子语料库，使用OpenAI的GPT-2检测模型计算文本被判定为AI生成的概率分布，并比较两组差异及文本特征。

Result: 两组被标记为AI生成的帖子均不足2%，但疑似自闭症子语料库的被标记率显著高于普通子语料库，且文本特征差异与AI生成文本特征的关联不明确。

Conclusion: AI检测模型对自闭症作者存在潜在偏见，需对其本身及在学术等场景中的应用进行伦理审查与进一步批评性检验。
...
### [217] [CoTu at EXACT 2026: Neuro-Symbolic Reasoning for Transparent Educational QA](https://arxiv.org/abs/2607.14735)
*Quoc-Khang Tran,Minh-Thien Nguyen,Phu-An Thai,Xuan-Tung Bui,Truong-Thanh Ma,Nguyen-Khang Pham*

Main category: cs.CL

TL;DR: 本文介绍了一个用于透明教育问答的神经符号程序思维链系统，使用4B参数模型，通过Z3编码解决逻辑推理，通过Python代码解决物理问题，在EXACT 2026竞赛中获得物理任务满分和最高技术分。


<details>
  <summary>Details</summary>
Motivation: 教育问答需要可解释的答案，但大型专有模型不透明，因此研究旨在用小模型实现既正确又可解释的问答系统。

Method: 采用神经符号程序思维链管道，4B骨干模型生成程序而非直接回答：法规查询用Z3编码，物理问题用数值Python；结合答案类型路由、蒸馏微调、自纠错循环和SGLang推测解码。

Result: 物理任务在自动选择轮中获得满分，最终轮技术分13.44/15（最高），综合排名第三。

Conclusion: 基于符号求解器的答案在4B规模下可实现正确、可验证的推理，主要难点在于前提选择而非推理本身。
...
### [218] [Dialogue Summarization with Emotion Dynamics Using Topic- and Participant-Centric Decomposition](https://arxiv.org/abs/2607.14769)
*Linyun Xiang,Mark Neerincx,Stephanie Tan*

Main category: cs.CL

TL;DR: 提出一种基于分层链式代理的多模态对话摘要框架，同时建模语义和情感动态，并在小语言模型上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有摘要研究多聚焦于独白信息，忽视对话中多参与者的交互与情感变化。

Method: 从话题分割和参与者特定片段两个视角分解对话，结合自动推断情感生成摘要，通过分层链式代理聚合。

Result: 在小语言模型上的多模态对话数据集实验中，生成的摘要同时包含语义和情感内容。

Conclusion: 所提方法能有效保留情感轨迹，为对话分析提供新机遇。
...
### [219] [SEED: Self-Evolving On-Policy Distillation for Agentic Reinforcement Learning](https://arxiv.org/abs/2607.14777)
*Jinyang Wu,Shuo Yang,Zhengxi Lu,Fan Zhang,Yuhao Shen,Lang Feng,Haoran Luo,Zheng Lian,Shuai Zhang,Zhengqi Wen,Jianhua Tao*

Main category: cs.CL

TL;DR: 提出SEED框架，利用策略自身生成的 hindsight 技能提供密集的 token 级监督信号，与稀疏的结果奖励结合，提升智能体在长周期任务中的性能和 sample efficiency。


<details>
  <summary>Details</summary>
Motivation: 结果驱动的强化学习在长周期交互任务中面临监督不足的问题，即轨迹级别的稀疏奖励难以指导中间决策。

Method: SEED框架：1. 策略微调后分析已完成轨迹，提取自然语言技能（可复用工作流、关键观察、失败规避规则）。2. 在强化学习过程中，当前策略既收集轨迹又作为分析器生成技能，实现自我演化。3. 通过重新评分普通和技能增强的上下文动作，将技能引起的概率偏移转化为密集的 token 级蒸馏信号，与结果奖励联合优化。

Result: 在文本和视觉智能体任务上，SEED一致提升了性能和样本效率，并展现了良好的泛化能力。

Conclusion: SEED通过自我演化的 hindsight 技能蒸馏有效弥合了结果监督与 token 级学习之间的鸿沟，是一种实用的辅助优化方法。
...
### [220] [Show Me How You Reason and I'll Tell You Who You Are: Reasoning Graphs for Robust LLM Authorship Attribution](https://arxiv.org/abs/2607.14905)
*Zlata Kikteva,Artur Romazanov,Annette Hautli-Janisz,Ramon Ruiz-Dolz*

Main category: cs.CL

TL;DR: 提出利用推理图（reasoning graphs）和GNN检测LLM生成文本，抵抗改写等混淆攻击，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于表层语言特征的LLM文本检测方法易受改写技术规避，需要更鲁棒的深层特征。

Method: 从LLM生成文本中提取推理图（argument mining pipeline），用图神经网络（GNN）分析推理结构进行检测。

Result: 在改写和回译攻击下，性能比Longformer基线提升最多27个百分点；在未见模型版本生成文本上提升19个百分点。

Conclusion: 推理图结构特征能有效提升LLM文本检测的鲁棒性和泛化能力，适应真实世界中模型持续更新的场景。
...
### [221] [Latent Trajectory Discrimination for AI-Generated Text Detection](https://arxiv.org/abs/2607.14967)
...
TL;DR: 提出GTCL框架，通过建模文本生成的几何轨迹来进行AI文本检测，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成文本检测方法将文档视为静态对象，忽略了自回归生成过程的动态特性。

Method: 将文档分割为有序局部单元，编码为嵌入表示，构建序列级轨迹，并通过对比学习学习几何正则性。

Result: 在三个基准测试中，GTCL一致优于检测基线，证明显式建模序列动态性提供了跨模型和领域的鲁棒判别信号。

Conclusion: 建模轨迹差异可改进AI文本检测，开辟了此前未充分探索的动态研究方向。
...
### [222] [OmniaBench: Benchmarking General AI Agents Across Diverse Scenarios](https://arxiv.org/abs/2607.14989)
...
TL;DR: 提出了OmniaBench，一个覆盖多场景、多领域、多能力的通用智能体基准测试，包含1431个任务，对当前最先进模型构成显著挑战。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准测试局限于特定场景、工具或交互格式，难以系统评估模型在异构应用场景中的能力。

Method: 通过应用商店、产品文档等构建层次化分类法，设计DAG、DAG-S、Solver、Program四种任务合成路线，引入十维能力分类和八种组合原子难度因子。

Result: 顶级模型Claude-Sonnet-5和GPT-5.6-Sol的Overall Pass@1分别仅为58.54和57.14，模型在规划、约束维护和自适应修正方面存在明显不足。

Conclusion: OmniaBench是一个广泛且可诊断的基准测试，有助于刻画通用智能体的能力边界。
...
### [223] [Rubrics on Trial: Evolving Rubrics from a Single Query via Synthetic Pairwise Evidence](https://arxiv.org/abs/2607.15092)
*Haocheng Yang,Licheng Pan,Xiaoxi Li,Zhichao Chen,Zhiheng Zhang,Yuan Lu,Haoxuan Li,Hao Wang*

Main category: cs.CL

TL;DR: 提出Rubrics on Trial框架，无需外部标注或模型训练，仅通过查询生成和验证评分标准，在五个偏好基准套件上取得最佳平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有评分标准构建方法依赖人工编写、偏好数据或采样响应，且缺乏对生成标准实用性的验证，可能导致标准无法区分答案质量、奖励无关风格或惩罚有效策略。

Method: 从空集开始迭代演化评分标准集，利用合成评分标准-响应对进行监督，并通过三类筛选（非判别性、过度具体、仅风格）验证每个候选标准。

Result: 在五个偏好基准套件的七个评估集上取得六项领先，实现最佳平均准确率。

Conclusion: 仅依赖查询的框架能有效生成有用且可靠的评分标准，验证了无需外部监督的可行性。
...
### [224] [Digital Pantheon: Simulating and Auditing Coalition Formation with LLM Agents](https://arxiv.org/abs/2607.15095)
*Dylan Van Mulders,Matthias Bogaert,Dirk Van den Poel*

Main category: cs.CL

TL;DR: 提出了一种结合SFT、DPO和RAG的多智能体框架，用于模拟党派联盟谈判，并通过MILT和CIS追踪协议溯源，在2019年佛兰德选举中验证了框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM因RLHF的偏见无法维持稳定的党派行为，需设计能平衡事实准确性和意识形态对齐的谈判模拟框架。

Method: 使用DPO训练党派特定角色，结合每党独立RAG约束至官方宣言；采用hub-and-spoke谈判结构并由formateur仲裁；引入MILT溯源协议条款的五种来源状态，CIS量化党派影响力。

Result: 三次独立模拟均产生稳定的赢家及其排名（N-VA领先），宣言锚定内容可靠预测现实协议，而虚构内容无此能力。

Conclusion: 该框架提供了透明、可扩展的联盟形成前测试平台，可用于党派兼容性和调解妥协的探索。
...
### [225] [Grokipedia vs Wikipedia: An LLM-Based Audit of Political Neutrality along Ideologies](https://arxiv.org/abs/2607.15146)
*Filippos Vlahos,Guillaume Bied,Tijl De Bie*

Main category: cs.CL

TL;DR: 比较Grokipedia（由LLM Grok编写）和Wikipedia在政治中立性上的差异，发现Grokipedia对经济右翼政客更有利，对自由派政客不公，而Wikipedia相反。


<details>
  <summary>Details</summary>
Motivation: 在线百科全书影响政治观点，Grokipedia声称提供无偏替代方案，但需要检验其真正中立性。

Method: 分析1394篇政府成员文章，使用四个LLM法官（Grok、Claude、Mistral、DeepSeek）评分，基于九种专家编码的意识形态维度。

Result: 所有LLM法官认为Grokipedia不如Wikipedia中立；Grokipedia偏向经济右翼政客，Wikipedia偏向自由派政客。

Conclusion: LLM编写的百科全书并不比人类编写的更中立，可能嵌入不同意识形态偏见。
...
### [226] [Linear representations of grammaticality in neural language models](https://arxiv.org/abs/2607.15175)
*Jane Li,Najoung Kim*

Main category: cs.CL

TL;DR: 本文通过内部表征探究神经语言模型是否编码了语法性，发现语法与不合语法句子在表征空间中显著分离，且此编码不受其他句子因素完全解释，并跨语法现象和语言泛化。


<details>
  <summary>Details</summary>
Motivation: 现有研究多基于概率比较来评估模型的语法知识，但概率可能受词汇频率、合理性等因素干扰，因此需要更直接的表征层面证据。

Method: 使用质量均值探测（mass-mean probing）方法，在预训练神经语言模型的句子表征空间中测试语法与不合语法句子的系统分离性，并控制相关句子属性。

Result: 语法性在多种预训练模型的句子表征中被鲁棒编码，形成清晰的表征分离，且无法由其他因素完全解释；编码跨语法现象和部分跨语言泛化。
...
### [227] [Mask-Aware Policy Gradients for Diffusion Language Models](https://arxiv.org/abs/2607.15200)
*Haran Raajesh,Kulin Shah,Adam Klivans,Philipp Krähenbühl*

Main category: cs.CL

TL;DR: 提出了一种新的强化学习方法，用于掩码扩散语言模型的两阶段决策：令牌预测与掩码位置选择。


<details>
  <summary>Details</summary>
Motivation: 现有方法因忽视生成过程中掩码位置的顺序而难以准确估计对数似然。

Method: 将生成过程形式化为两阶段动作MDP，策略梯度自然分解为令牌项和掩码项。

Result: 在GSM8K和MBPP上分别达到87.1%和53.4%，达到最先进水平。

Conclusion: 同时优化令牌和掩码项可显著提升推理与代码生成性能。
...
### [228] [T^2MLR: Transformer with Temporal Middle-Layer Recurrence](https://arxiv.org/abs/2607.15178)
*Ziyang Cai,Xingyu Zhu,Yihe Dong,Yinghui He,Sanjeev Arora*

Main category: cs.CL

TL;DR: 提出T2MLR架构，通过仅在中层引入循环机制提升Transformer的推理能力，无需完整层循环。


<details>
  <summary>Details</summary>
Motivation: Transformer的自回归解码限制了推理能力，因为中间推理状态难以跨时间步持久化。

Method: 在Transformer中，将前一个token的缓存中层表示直接注入当前token的早期层，实现时间持久化的中间计算。

Result: T2MLR在自然语言预训练和多跳推理微调中优于基线模型；仅对20%的中层网络应用循环常优于全层循环；可迁移到预训练模型并提升数学推理。

Conclusion: 有效的潜在推理不需要遍历所有层，针对中层的循环机制能更显著地提升性能。
...
### [229] [Language Identification via Compositional Data Analysis: A Linear-Time Classifier Based on Log-Ratio Geometry](https://arxiv.org/abs/2607.15238)
*Paul-Andrei Pogăcean,Sanda-Maria Avram*

Main category: cs.CL

TL;DR: 提出使用中心对数比变换处理字符和二元组频率分布，实现高效且可解释的语言识别。


<details>
  <summary>Details</summary>
Motivation: 神经网络方法计算资源需求大，而统计n-gram模型依赖的距离度量不适用于成分数据。

Method: 将字符和二元组频率视为成分向量，通过CLR变换映射到零和子空间，结合拉普拉斯平滑处理稀疏性。

Result: 在六种语言上，该方法在不同文本长度下均表现出鲁棒精度，长文本性能尤其优秀。

Conclusion: 成分表示提供了一种确定性、计算高效的语言识别替代方案，特别适用于可解释性和低资源消耗场景。
...
### [230] [Expanding the Lexicon of Ge'ez Based African Languages: A Comparative Study of Amharic and Tigrinya](https://arxiv.org/abs/2607.15209)
*Hailay Kidu Teklehaymanot,Debela Desalegn Yadeta,Wolfgang Nejdl*

Main category: cs.CL

TL;DR: 本文提出VEXMLM，通过扩展词汇表和两阶段训练，显著提升了多语言预训练模型在低资源格埃兹语系语言上的性能。


<details>
  <summary>Details</summary>
Motivation: 多语言预训练模型在低资源非拉丁语系语言上表现不佳，主要原因是基于拉丁语系的tokenizer训练导致的高未登录词率和过度子词分割。

Method: 在阿姆哈拉语和提格里尼亚语上训练专用SentencePiece分词器，扩展XLM-R词汇表3万个格埃兹语子词，并利用原始分词器的子词嵌入平均初始化。采用两阶段训练：先对扩展词汇表进行掩码语言建模，再在问答、命名实体识别和情感分析上微调。

Result: 在阿姆哈拉语和提格里尼亚语问答任务中，F1达到90.0%（XLM-R为78.0%）；情感分析准确率80.0%（XLM-R为77.0%）；命名实体识别中，OOV实体准确率从81.4%提升至94.3%。

Conclusion: VEXMLM通过词汇扩展和两阶段训练，有效提升了低资源格埃兹语系语言的性能，且收益可迁移至其他相关语言。
...
### [231] [In-Place Tokenizer Expansion for Pre-trained LLMs](https://arxiv.org/abs/2607.15232)
...
TL;DR: 本文提出了一种在预训练后扩展分词器的方法，通过继续BPE合并和新词分解初始化嵌入，配合两阶段适应训练，在混合专家模型上实现多语言token数量减少2.4-2.6倍，并提升解码速度2.2-3.7倍。


<details>
  <summary>Details</summary>
Motivation: 固定分词器导致后期添加的语言每个词被拆分为更多token，增加延迟和资源消耗，而紧凑型模型的词汇表有限，难以覆盖多语言。

Method: 在现有分词器基础上继续BPE合并，保持源token不变，新token可分解为源token。复制原有嵌入行，新行初始化为子token嵌入均值，再通过嵌入训练和全模型继续预训练两阶段适应。

Result: 在LFM2-8B-A1B模型上应用后，印地语和越南语token减少约2.4倍和2.6倍，泰语最多4倍，参考设备上解码速度提升2.2-3.7倍。

Conclusion: 扩展分词器配方能有效缓解多语言token碎片化问题，且不破坏原有模型质量，同时公开了模型权重和负面发现。
...
### [232] [TikStance: A Multimodal and Hierarchical Dataset for Multi-target Stance Analysis in TikTok Political Conversations](https://arxiv.org/abs/2607.15240)
*Yazhi Zhang,Fuqiang Niu,Bowen Zhang*

Main category: cs.CL

TL;DR: 提出了TikStance数据集，包含161个视频和13,876条评论，用于检测2024年美国大选中三位政治人物的立场，结合了多模态和层级对话信息。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏同时包含视听信息和层级对话的短视频政治内容，限制了计算分析。

Method: 从TikTok收集数据，每段讨论单元连接视频元数据和父链接评论树，采用三级分类（赞成、反对、无）进行人工标注，分歧项重新标注。

Result: Krippendorff's α在三个子集上分别达到0.743、0.723和0.722，嵌套回复占所有评论的23.3%。

Conclusion: TikStance支持多模态立场检测、政治传播、计算社会科学和上下文感知自然语言处理研究。
...
### [233] [Beyond the Leaderboard: Design Lessons for Trustworthy Multimodal VQA](https://arxiv.org/abs/2607.15241)
*Sushant Gautam,Vajira Thambawita,Michael A. Riegler,Pål Halvorsen,Steven A. Hicks*

Main category: cs.CL

TL;DR: 通过在MediaEval Medico 2025中回顾性分析GI内窥镜案例，研究了九个系统的问答和解释质量，发现参数高效的预训练骨干网络能提升性能，但答案层增益不总能转化为忠实的临床推理。


<details>
  <summary>Details</summary>
Motivation: 多模态医疗AI需结合视觉与文本证据，同时确保可靠性和可解释性，现有方法在忠实推理方面存在不足。

Method: 以MediaEval Medico 2025为案例，分析九个系统的设计选择，涉及参数高效预训练骨干网络、结构化推理和显式接地方法。

Result: 参数高效的预训练骨干网络在挑战中表现强劲，但答案层增益未一致转化为忠实推理；结构化推理方法在不同问题类型上更可靠，但证据仅为相关性而非消融性。

Conclusion: 需超越词汇重叠评估，采用标准化解释链接、泄漏感知数据治理、轻量级鲁棒性和校准检查，以支持可信的多模态医疗AI。
...
### [234] [Partition, Prompt, Aggregate: Statistical Self-Consistency in Language Models](https://arxiv.org/abs/2607.15277)
*Patrik Wolf,Thomas Kleine Buening,Andreas Krause,Celestine Mendler-Dünner*

Main category: cs.CL

TL;DR: 研究LLM的上下文估计是否满足概率自洽性（全概率公式），发现普遍违反该性质，并提出“宏观谬误”现象：细粒度子群体估计比直接总体估计更准确。


<details>
  <summary>Details</summary>
Motivation: 检验常用LLM上下文估计是否满足基本概率一致性原理（特别是全概率公式）。

Method: 使用二叉树递归划分人群，通过提示词描述子群体，比较不同粒度下聚合估计与直接总体估计的一致性。

Result: 发现LLM普遍违反自洽性，且存在“宏观谬误”：细粒度子群体估计更接近人类数据。

Conclusion: 统计自洽性可作为无需参考标准的LLM评估指标，模型虽拥有子群体知识但未能可靠地整合为总体估计。
...
### [235] [SciDiagramEdit: Learning to Edit Scientific Diagrams from Paper Revisions](https://arxiv.org/abs/2607.15272)
*Yasheng Sun,Zezi Zeng,Yifan Yang,Chong Luo,Wenyi Wang,Ziwei Liu,Jürgen Schmidhuber*

Main category: cs.CL

TL;DR: 提出了一个名为SciDiagramEdit的基准和技能进化框架，用于根据自然语言指令自动编辑科学图形，并通过从arXiv版本历史中挖掘的修订对进行训练。


<details>
  <summary>Details</summary>
Motivation: 科学图形编辑是研究中的常规但耗时的任务，自动执行此工作流程具有挑战性，因为图形包含异构视觉元素并遵循严格的视觉语法。

Method: 构建了包含修订前/后图形对的基准数据集，并采用代理学习机制通过技能进化不断优化编辑指令的执行能力。

Result: 技能进化框架在验证集上逐步提高了编辑准确性。

Conclusion: 自然语言驱动的图形编辑可以通过从真实论文修订中学习来实现，且自然修订对是有效的训练信号。
...
### [236] [Adaptive Control of Motor-Position-Controlled Flexible Joint Robots with Uncertain Joint Stiffness](https://arxiv.org/abs/2607.14177)
*Annika Kirner,Grazia Zambella,Igor Kovacevic,Hannes Höppner,Jee-Hwan Ryu,Christian Ott*

Main category: cs.RO

TL;DR: 提出一种自适应控制方法，用于位置控制型柔性关节机器人，在线更新关节刚度估计。


<details>
  <summary>Details</summary>
Motivation: 柔性关节机器人的模型控制需要精确关节刚度，但实际中刚度随工况变化且缓慢老化，缺乏精确模型。

Method: 采用隐式控制律和依赖控制输入的回归矩阵，自适应更新关节的非线性力矩-变形关系。

Result: 实验验证了在非线性刚度柔性关节上的有效性，并分析了抗电机位置控制器误差的鲁棒性。

Conclusion: 该方法可有效应对关节刚度不确定性，提高模型控制精度。
...
### [237] [Semantic Audio-driven Understanding for Dynamic Humanoid Whole Body Control](https://arxiv.org/abs/2607.14182)
*J. M. A. Marcelo,M. Brienza,E. Bugli,L. Comito,D. Nardi,D. D. Bloisi,V. Suriani*

Main category: cs.RO

TL;DR: 提出了一种跨模态语义驱动框架，使类人机器人能通过音频（音乐/语音）自主选择并执行运动技能，在仿真和真实机器人上验证了鲁棒的从仿真到现实迁移。


<details>
  <summary>Details</summary>
Motivation: 现有机器人表演多依赖预设脚本或外部触发，缺乏自主性和动态环境响应能力。

Method: 构建多模态编排框架：音频流经音乐分支（音频指纹+语义嵌入）或语音分支（技能库检索），统一接口调度强化学习控制流水线。

Result: 在Unitree G1人形机器人上实现仿真到现实的鲁棒迁移，以及音频条件化的策略选择。

Conclusion: 该框架赋予了人形机器人基于语义音频的自主运动控制能力，提升了交互的灵活性与实时性。
...
### [238] [Open-AoE: An Open Egocentric Manipulation Dataset and Toolchain for Embodied Learning](https://arxiv.org/abs/2607.14183)
...
TL;DR: Open-AoE是一个开放的、以社区为中心的自我中心操作数据集和工具链，包含约2000小时的自然环境操作视频，提供文本标注、手部姿态、相机轨迹和原子动作，以及从数据采集到模型训练的完整流程。


<details>
  <summary>Details</summary>
Motivation: 现有资源很少结合低成本连续采集、操作级结构化标注和可复用的机器人学习工具，需要一种开放的基础设施来降低数据贡献和复用的门槛。

Method: 通过智能手机连续拍摄采集自我中心视频，结合时间动作分割、语义标注、手部重建和相机轨迹重建的处理管道，并支持跨实体重定向、模型特定转换和训练配方。

Result: 首个发布包含来自500多名贡献者使用400多部手机采集的约2000小时操作视频，以及结构化标注和工具链。

Conclusion: Open-AoE通过集成可扩展采集、结构化处理和下游适配，降低了数据贡献和复用的障碍，为具身模型训练、人机转移和世界建模提供了实用的开放基础设施。
...
### [239] [Never Too Late for Force: Accelerating VLA Post-Training with Reactive Force Injection](https://arxiv.org/abs/2607.14236)
*Yi Wang,Wendi Chen,Zimo Wen,Han Xue,Xueqi Li,Wenye Yu,Zhijie Chen,Hao Yang,Jun Lv,Chuan Wen,Cewu Lu*

Main category: cs.RO

TL;DR: LIFT是一个力感知的后训练框架，通过向预训练的视觉-语言-动作（VLA）策略中添加接触反应性，以增强其在接触状态下的操作能力。
...
Method: LIFT在原始动作专家旁嫁接一个反应性动作专家，使用预训练的动作权重初始化，通过因果力记忆和零初始化的交叉注意力注入6D末端执行器力，并结合在线DAgger循环训练。

Result: 在毛巾折叠、书本插入和汉诺塔环放置任务中，LIFT比仅视觉的后训练方法学习更快且性能更高，消融实验表明反应性力记忆和在线纠正数据对鲁棒接触操作至关重要。

Conclusion: LIFT通过力感知后训练，在保持通用操作知识的同时，显著提升了接触丰富操作中的表现。
...
### [240] [MEMORA: Embodied Action Memory from Egocentric Videos for Reasoning and Planning](https://arxiv.org/abs/2607.14252)
*Zihao Yu,Xiu Yuan,Chongjie Zhang*

Main category: cs.RO

TL;DR: 本文提出体行动记忆（EAM）概念，并实现MEMORA系统，通过形成-巩固-检索生命周期和四类型存储（环境、实体、活动、推断知识）为机器人长时程规划提供记忆基础；在EPIC-KITCHENS-100扩展视频上评估，全MEMORA方法在记忆评估任务中准确率提升最高20.5%，并在分布外机器人规划任务中得分提升16.6%。


<details>
  <summary>Details</summary>
Motivation: 现有机器人规划方法仅依赖当前场景预测动作，缺乏类似人类的长期经验记忆来支持未来目标解释，导致在长时程任务中性能受限。

Method: 提出MEMORA系统，包含形成-巩固-检索生命周期，并设计环境记忆、实体记忆、活动记忆和推断知识四种类型存储；在线阶段维护物体身份和状态历史，离线阶段从重复经验中抽象出可重用程序与参与者规律。

Result: 在EPIC-KITCHENS-100扩展视频的18位参与者（45小时）评估中，全MEMORA在四种开放权重语言模型上取得最佳综合结果；记忆评估准确率提升最高20.5%，分布外机器人规划得分提升16.6%；机器人部署实验验证了记忆语言规划与下游控制的可行性。

Conclusion: 可编辑、可巩固的记忆系统能够为机器人规划提供可靠的上下文经验，显著提升长时程和分布外任务的规划能力。
...
### [241] [Stochastic Filtering for Quorum Sensing in Robot Swarms under Anonymous Communication](https://arxiv.org/abs/2607.14262)
*Fabio Oddi,Andreagiovanni Reina,Vito Trianni*

Main category: cs.RO

TL;DR: 本文研究了机器人集群中的群体感知（QS）能力，提出了一种基于k优先级采样的随机滤波协议（ANTk），以改善匿名通信协议中因重复消息导致的估计偏差和稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 匿名通信协议无法区分消息来源，导致重复消息被重复计数，从而产生群体估计偏差，因此需要一种能提高估计稳定性的方法。

Method: 采用k优先级采样驱动的随机滤波协议（ANTk），并与基线匿名协议（AN）和随机化变体（ANT）进行比较。

Result: 基线协议AN快速但准确性低；ANT提高准确性但收敛慢；ANTk成功减少临时误差并稳定估计，但错误恢复时间增加。

Conclusion: ANTk协议在稳定估计方面表现优异，但代价是较长的错误恢复时间，适用于需要高稳定性而非快速响应的场景。
...
### [242] [DiMaS: Distribution Matching for Steering Vision-Language-Action Models](https://arxiv.org/abs/2607.14280)
*Pegah Khayatan,Sara Meziane,Jayneel Parekh,Matthieu Cord*

Main category: cs.RO

TL;DR: 提出DiMaS分布匹配策略，实现对流匹配VLA模型的精细行为控制，并解释经典线性方法失败原因。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作(VLA)模型缺乏细粒度行为控制能力，而传统表示干预方法在此场景下效果不佳。

Method: 通过分布匹配（分布间传输）替代线性方向位移，专门针对流匹配VLA设计DiMaS策略。

Result: 在两种SOTA VLA上成功控制行为；行为控制效果随任务差异增大而减弱；发现行为特征线性可解码但不可线性操纵。
...
### [243] [Beyond Visual Grasping: Benchmarking Complex Grasping from Detection to Execution](https://arxiv.org/abs/2607.14341)
...
TL;DR: 提出GCA-Bench基准，用于评估复杂抓取任务中的多步推理与语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有抓取基准主要关注视觉抓取姿态检测，缺乏复杂场景下的多步推理和语义约束评估。

Method: 设计了包含场景级推理和语义约束的复杂动作抓取场景，并基于大型基础模型进行统一评估。

Result: 基线方法在复杂场景下成功率低于70%，揭示了关键局限性。

Conclusion: 新提出的指标和失败模式分析可为鲁棒抓取策略开发提供指导。
...
### [244] [An offline approach to fNIRS-guided reinforcement learning for robot behavior](https://arxiv.org/abs/2607.14393)
*Julia Santaniello,Madelaine Brower,Benson Jiang,Donatello Sassaroli,Robert Jacob,Jivko Sinapov*

Main category: cs.RO

TL;DR: 本文探索使用功能性近红外光谱（fNIRS）脑信号调节模拟环境中的机器人强化学习，通过参数增强而非替换的方法，验证了该框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统人机交互方法依赖显式反馈，而脑信号可提供更隐式、自然的反馈机制，本研究旨在探索其可行性。

Method: 比较被动（观察）和主动（演示）交互任务，利用fNIRS信号增强RL算法中的轨迹优先级和状态动作Q值，并分析模型粒度和噪声的影响。

Result: 脑信号在增强轨迹优先级和Q值时提升了学习效果，且框架能从离线数据中成功学习，适用于实时BCI受限的场景。

Conclusion: 基于fNIRS的脑信号增强RL框架有效，为机器人行为对齐提供了新的隐式反馈途径。
...
### [245] [ConFlow: Constraints-Guided Learning with Flow Matching for Motion Generation](https://arxiv.org/abs/2607.14424)
*Nutan Chen,Jianxiang Feng,Marvin Alles,Botond Cseke*

Main category: cs.RO

TL;DR: 提出ConFlow框架，通过训练时集成约束（可微障碍函数/成本函数）和条件高斯过程，解决机器人运动生成中训练-推理差距问题，在双机器人导航任务中降低碰撞率并提升轨迹质量。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法在训练时未考虑任务特定约束，依赖推理时引导，导致训练-推理不一致，无法有效满足机器人运动生成中的约束需求。

Method: 将约束信息通过可微障碍/成本函数融入训练目标，用条件高斯过程替代标准高斯源分布，并利用不可行演示作为负监督。

Result: 在双机器人导航任务中，ConFlow的碰撞率低于标准流匹配基线（含推理时引导），轨迹质量更高。

Conclusion: 训练时约束集成有效缩小了生成运动模型中的训练-推理差距，无需额外专家数据即可提升约束满足度。
...
### [246] [Mixed-Agent Museum Tour Guide Design Improves Gendered Learning Outcomes and Visitor Preferences](https://arxiv.org/abs/2607.14468)
*Annette M. Masterson,Wonse Jo,Helena C. Sieh,Lionel P. Robert,,Dawn Tilbury*

Main category: cs.RO

TL;DR: 提出了一种结合物理机器人与投影虚拟代理的混合代理导览系统，通过对话和互动增强游客体验，实验表明该系统能提高女性学习表现。


<details>
  <summary>Details</summary>
Motivation: 为了提升博物馆中机器人导览的游客体验和参与度，并解决单个平台实现双代理交互丰富性的问题。

Method: 设计混合代理系统，通过30名被试的组内实验，评估不同对话风格和代理配置下的参与度、体验质量和学习表现，结合问卷、行为传感器和访谈收集数据。

Result: 参与度和体验质量在各条件下一致；学习表现存在性别差异，混合代理条件显著提升女性表现；访谈中参与者更偏好混合代理团队。
...
### [247] [Motion Planning with Model-Based Diffusion via Constraint Optimization and Adaptive Scheduling](https://arxiv.org/abs/2607.14455)
*Zhilin He,Bowei Li,Jianlin Dou,Yuner Zhang,Changliu Liu*

Main category: cs.RO

TL;DR: 提出MD-COAS框架，统一软硬约束，通过自适应调度提升非凸环境下单机器人轨迹规划的安全性与效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于模型的扩散方法在安全约束处理上存在软硬约束未统一、调度固定不变的局限。

Method: 融合非精确增广拉格朗日软约束先验与凸可行集硬投影算子，并自适应调度安全强制与扩散进程。

Result: 在2D非凸基准和7自由度机械臂任务中，较基线方法安全率、成功率更高，收敛更快，最终代价更低。

Conclusion: 统一软硬约束并动态调度可有效提升复杂约束下运动规划性能。
...
### [248] [MIDAS Hand: Modular low-Impedance Direct-drive Anthropomorphic Sensing Hand](https://arxiv.org/abs/2607.14487)
*Alvin Zhu,Mingzhang Zhu,Beom Jun Kim,Quanyou Wang,Jose Victor S. H. Ramos,Dennis Hong*

Main category: cs.RO

TL;DR: 提出了MIDAS Hand，一款低成本、开源、类人尺寸的灵巧手，集成触觉传感，用于操作研究。


<details>
  <summary>Details</summary>
Motivation: 现有灵巧手在形态、制造、传感和成本之间难以平衡，缺乏理想的硬件平台。

Method: 设计13主动自由度、16总自由度的直接驱动结构，集成283个三轴触觉单元，3D打印组装。

Result: 700克重量、3000美元成本、3小时组装，通过工作空间、抓取分类、负载、可靠性及遥操作测试验证。

Conclusion: 提供了平衡、可复现的触觉灵巧操作和人机数据采集平台。
...
### [249] [Safe Execution of RL Policies Via Acceleration-Based CBF-QP Constraint Enforcement for Real-World Robotic Deployments](https://arxiv.org/abs/2607.14488)
...
TL;DR: 提出一种基于加速度的二次规划安全滤波器（Acc-CBF-QP），通过控制障碍函数将任意强化学习策略约束在安全集内，显著降低机器人部署时的约束违反。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人控制中缺乏安全保证，难以直接部署于硬件，因为腿式机器人和机械臂常在安全边界附近运行，异常状态会导致失败。

Method: 使用加速度基二次规划（QP）安全滤波器，结合控制障碍函数（CBF），在运行时约束RL策略，无需修改训练；提出TorqueTask和Forward Dynamics Task两种QP任务，调控RL命令偏离。

Result: 在7自由度Kinova Gen3机械臂和19自由度Unitree H1人形机器人上，模拟和实物实验显示约束违反大幅减少。H1上Safe-RL策略违反率从10.04/s降至0.80/s（降92%），Gen3上完全消除违反；任务性能在无违反区域保持。

Conclusion: Acc-CBF-QP有效提升RL策略的安全性与执行稳定性，防止因约束导致的停机，延长生存时间，且为开源方案。
...
### [250] [Modeling and Validation of Quality of Control for Edge-Offloaded Collaborative Navigation](https://arxiv.org/abs/2607.14853)
...
TL;DR: 本文针对复杂环境中因无线网络随机延迟和可靠性变化导致的协作控制问题，扩展了控制质量框架，通过模型与实验分析了5G网络下机器人控制的优化区间，并发现RELIABLE QoS比BEST-EFFORT提升51.5%的控制质量。


<details>
  <summary>Details</summary>
Motivation: 无线网络的不确定性（延迟和可靠性波动）严重影响了多机器人协同控制中的导航、跟踪和避碰性能，且易导致资源过度配置，能量效率难以保障。

Method: （1）建立端到端网络对闭环控制性能的模型；（2）系统性分析控制参数对网络延迟-可靠性的影响；（3）在私有5G测试平台上进行不同延迟、可靠性和控制配置的实验验证。

Result: 确定了实际机器人控制-通信协同设计的最优运行区间，并展示了在特定实验条件下RELIABLE QoS比BEST-EFFORT提升51.5%的控制质量。

Conclusion: 通过控制与通信的协同设计，可以有效缓解网络不确定性对机器人协作控制的负面影响，为实际5G环境下机器人系统提供性能优化依据。
...
### [251] [DRIFT: Drift and Aggregation for Motion Planning](https://arxiv.org/abs/2607.14507)
*Yining Xing,Zhiyuan Liu,Zehong Ke,Wenhao Yu,Jianqiang Wang*

Main category: cs.RO

TL;DR: 提出DRIFT，一种固定深度轨迹规划器，通过单步潜在空间漂移和场景感知聚合生成最终路径，在NAVSIM上实现89.6 PDMS和90.4 EPDMS，推理时间66.43毫秒。


<details>
  <summary>Details</summary>
Motivation: 端到端规划器需要处理多种驾驶行为并实时输出单一轨迹，但现有提案方法在聚合多候选方案时存在设计难题。

Method: 结合潜在空间单步漂移与场景感知聚合，解码器生成48个提案特征，聚合头整合场景、导航和自车状态直接预测轨迹，训练使用专家模仿和地图边界正则化。

Result: NAVSIM测试中PDMS 89.6，EPDMS 90.4，提案生成和聚合仅需10.82毫秒，完整模型66.43毫秒，可驾驶区域合规性和自车进度优异。

Conclusion: 单步潜在提案生成与直接聚合为多假设运动规划提供了高效设计方案。
...
### [252] [Steering Robustness into World Action Models via Mechanistic Interpretability and Optimal Control](https://arxiv.org/abs/2607.14943)
*Jihoon Hong,Julian Skifstad,Qiyue Dai,Alice Chan,Glen Chou*

Main category: cs.RO

TL;DR: 本文利用机械可解释性研究世界动作模型(WAMs)在分布偏移下的鲁棒性表征，发现部分架构具有低维线性可分离性，并据此提出无训练和基于最优控制的两种引导方法(WA-LQR)，在多个模型上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: WAMs在语义和物理控制上表现良好，但在分布偏移下脆弱，亟需理解鲁棒性相关扰动在激活空间的表征并改进控制。

Method: 通过对比成功与失败rollout的激活，分析鲁棒性特征的线性可分离性；利用对比激活方向进行无训练引导；基于激活动力学的局部线性性，设计世界-动作线性二次型调节器(WA-LQR)进行反馈控制。

Result: Cosmos-Policy和DiT4DiT模型表现出强可引导性，LingBot-VA较弱；WA-LQR在相机、夹爪和视觉噪声扰动上优于无引导和提示引导基线，并泛化到新任务。

Conclusion: 机械可解释性可用于WAM鲁棒性分析和引导，WA-LQR提供了一种最小侵入性的高效控制方案，增强了WAM的分布外鲁棒性。
...
### [253] [Communication-Efficient Relative Pose Estimation with Vision Foundation Models for Ephemeral Collaborative Perception](https://arxiv.org/abs/2607.14539)
*Qihang Li,Jo-Hao Huang,Jiewen Liu,Suyoung Kang,Hao Zhang,Peng Gao*

Main category: cs.RO

TL;DR: 提出了通信高效的相对位姿估计框架CERPE，利用视觉基础模型在短暂交互和有限通信条件下实现多机器人协同感知。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖全局参考系、持续视觉重叠或高通信成本，难以适用于短暂协同感知场景。

Method: 通过持续共享固定大小描述符减少原始观测交换，并利用事件触发的原始图像请求；在无视觉重叠时通过度量缩放的自运动传播相对位姿。

Result: 在仿真和真实机器人实验中，CERPE在短暂协同感知中提升了6自由度相对位姿估计性能。

Conclusion: CERPE有效解决了低通信带宽和间歇性视觉重叠下的相对位姿估计问题，适用于短暂协作场景。
...
### [254] [Catch, Throw, Repeat: Planning for Human-Robot Partner Juggling](https://arxiv.org/abs/2607.15129)
*Jonathan Rainer Lippert,Kai Ploeger,Abir Chowdhury,Hermann Müller,Jan Peters,Alap Kshirsagar*

Main category: cs.RO

TL;DR: 提出了一种用于人机合作抛接球的实时规划与控制架构，实现了多球模式下的同步抛接，并通过用户研究验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 人机间动态物体交换因感知、时序和接触交互的不确定性而具有挑战性，人机合作抛接是其中的典型问题，推动物理人机交互和共享自主性的发展需要此类技能。

Method: 集成预测性球追踪、自适应在线轨迹优化（采用多重打靶法）以及基于状态机的协调逻辑。

Result: 8名不同技能水平的参与者均能在10分钟内超越先前最佳结果，其中一名参与者将三人抛接记录提高五倍（20次连续机器人接球），另一名在单球模式中达到100%成功率（40次连续接球）。

Conclusion: 该架构能可靠实现人机多球合作抛接，显著提升性能，且适应不同技能水平的人类合作伙伴。
...
### [255] [SafeRelBench: A Spatial-Relation-Aware Benchmark for Process-Level Safety in VLM-Driven Embodied Agents](https://arxiv.org/abs/2607.14543)
*Huaigang Yang,Ya Li,Min Ren,Bo Dai,Zhenliang Zhang,Zhaofeng He*

Main category: cs.RO

TL;DR: 本文提出SAFERELBENCH基准，用于评估VLM驱动的具身代理在空间关系相关的过程级安全性。


<details>
  <summary>Details</summary>
Motivation: 现有具身安全评估主要关注静态风险识别或最终任务完成，忽视了由支撑、包含、接近等空间关系引起的过程级安全故障。

Method: 构建包含507个可执行样本的SAFERELBENCH基准，其中248个为空间关系样本，259个为非空间对照样本，用于评估VLM代理在风险动作前的安全条件满足情况。

Result: 评估七个开源和闭源VLM驱动的具身代理发现，任务成功与过程级安全合规之间存在显著差距：模型常完成任务但违反过程级安全约束。

Conclusion: 安全的具身智能不仅需要更强的感知和规划能力，还需要对目标关系如何影响交互过程中的风险进行可靠推理。
...
### [256] [Beyond Implicit Force: Evaluating Explicit Force-Torque Proxies in Action Chunking with Transformers](https://arxiv.org/abs/2607.14578)
*King Hang Wong,Lingqiao Liu,Feras Dayoub*

Main category: cs.RO

TL;DR: 研究表明，ACT在接触丰富操作中的力感知能力依赖于遥操作中的隐含交互线索，通过引入简单的关节扭矩代理可恢复并增强其力敏行为。


<details>
  <summary>Details</summary>
Motivation: 探究ACT在精细操作中的力感知是否依赖于主从遥操作中的隐含交互信号，并寻找无外部力/扭矩传感器时恢复接触感知的方法。

Method: 提出一种以观测为中心的ACT变体，移除遥操作差异信号；评估基于电机电流或关节力矩的简单扭矩代理对接触感知的恢复效果。

Result: 在四项任务中，移除隐含线索导致严重失败；加入扭矩代理后，策略恢复了鲁棒的接触行为，并提升了基础ACT性能。

Conclusion: 遥操作中的隐含线索是可恢复的力感知源，扭矩代理可匹配、超越或增强原有性能，无需外部力传感器。
...
### [257] [SoftNav: Injecting 3D Scene Tokens into VLMs for Embodied Navigation](https://arxiv.org/abs/2607.14586)
*Yi Wu,Junjie An,Xiao Liu,Yiqun Zhou,Yuechen Wu,Xiaoqing Guan,Shuyang Yu,You Wang,Guang Li*

Main category: cs.RO

TL;DR: 提出SoftNav方法，通过向VLM注入三维连续软令牌（每检测对象/前沿一个令牌），在HM3D-OVON上取得74.2%/68.3%/66.7% SR，零样本迁移至GOAT-Bench、SG3D和真实机器人。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过文本传递三维场景信息存在表示鸿沟，导致导航性能受限，需探索更高效的3D场景与VLM交互方式。

Method: 冻结3D编码器和VLM，用轻量投影器将实体级3D连续表示（每个检测对象或前沿一个令牌）作为软令牌注入VLM隐空间，仅需约1200样本和17M可训练参数。

Result: 在HM3D-OVON三个分割上分别达74.2%/68.3%/66.7% SR（超越所有先前方法），零样本迁移至GOAT-Bench（67.2% SR）、SG3D（47.2% s-SR）及真实机器人，无需重训或架构修改。

Conclusion: 直接向VLM注入3D场景令牌可弥合表示鸿沟，实现仅需少量训练的可迁移导航。
...
### [258] [Representation-Aligned Tactile Grounding for Contact-Rich Robotic Manipulation](https://arxiv.org/abs/2607.14609)
...
TL;DR: 本文提出一种潜在触觉预测器（LTP），通过从动作专家中间表示预测紧凑的触觉嵌入，对齐动作表示与未来接触结果，从而增强触觉增强型视觉-语言-动作策略在接触密集型操作中的表现。


<details>
  <summary>Details</summary>
Motivation: 触觉增强型VLA策略在接触密集型操作中关键交互状态对视觉隐藏，未来触觉预测可监督动作诱导的接触动力学，但现有方法不清楚应在何种表征上施加监督。

Method: 通过线性探针分析发现未来触觉状态最易从动作专家中间特征预测，据此引入LTP，从中间动作表示预测紧凑触觉嵌入，避免预测噪声原始触觉信号。

Result: 在真实接触密集型操作任务中，表示对齐的触觉接地优于未对齐或多界面触觉预测。

Conclusion: 触觉监督施加的位置（中间动作表示而非视觉语言或最终动作表示）对性能至关重要。
...
### [259] [Image-to-Point Cloud Registration Made Easy with Rectified Flow-based LiDAR Upsampling](https://arxiv.org/abs/2607.14639)
*Reon Tabata,Kenji Koide,Shuji Oishi,Masashi Yokozuka,Taku Okawara,Aoki Takanose,Jun Miura*

Main category: cs.RO

TL;DR: 提出一种基于条件整流流将LiDAR点云转换为密度强度图像，再利用预训练特征匹配器与相机图像配准的I2P方法，在R3LIVE数据集上实现4.89°/1.63m的配准误差，耗时0.68秒。


<details>
  <summary>Details</summary>
Motivation: 图像与点云之间的模态差异导致现有I2P方法难以同时实现高精度和强泛化能力。

Method: 从单次稀疏LiDAR扫描生成密集LiDAR强度图像（条件整流流），与相机图像进行特征匹配，通过PnP-RANSAC估计6-DoF位姿；模型通过自监督图像补全预训练，并微调少量LiDAR数据。

Result: 在R3LIVE数据集上平均误差4.89°/1.63m，优于现有方法，单次配准约0.68秒。

Conclusion: 该方法以简单有效的策略实现高精度实时I2P配准，且无需图像-点云配对或真值位姿，具有良好的泛化性。
...
### [260] [NavCMPO: Critic-Guided MeanFlow Policy Optimization for Adaptive Navigation](https://arxiv.org/abs/2607.14643)
*Junjie An,Yi Wu,Xiao Liu,Yiqun Zhou,Yuechen Wu,Xiaoqing Guan,You Wang,Guang Li*

Main category: cs.RO

TL;DR: 提出了NavCMPO两阶段导航框架，通过少步MeanFlow轨迹生成、评论家引导精化及强化学习微调，在降低推理延迟的同时提升成功率。


<details>
  <summary>Details</summary>
Motivation: 端到端扩散策略在无地图视觉导航中推理延迟高，且行为克隆受限于专家演示质量。

Method: 预训练阶段包含障碍物接近预测任务和评论家引导的轨迹精化（CGTR），微调阶段使用PPO并结合行为克隆正则化。

Result: 在InternVLA-N1基准上，成功率达74.7%，比NavDP高6.4个百分点，推理延迟从85ms降至60ms，且在Unitree Go2上验证了仿真到现实的迁移。

Conclusion: NavCMPO通过两阶段框架有效平衡了导航性能与推理效率，并具备良好的迁移能力。
...
### [261] [An Intelligent-Cloud Edge Multimodal Interaction System for Robots](https://arxiv.org/abs/2607.14675)
*Zihan Guo,Xiaoqi Li*

Main category: cs.RO

TL;DR: 提出了一种云边缘多模态交互框架，通过改进的YOLO手势检测器（融合CBAM和DIoU）与LLM/VLM协同，在资源受限的机器人上实现鲁棒的人机交互。


<details>
  <summary>Details</summary>
Motivation: 复杂环境中人机交互需要手势感知、场景理解和任务规划，但机器人计算资源有限。

Method: 采用改进的YOLO-DC检测器（加入CBAM注意力模块和DIoU损失函数），结合云层的LLM和VLM智能体，边缘层由TonyPi机器人执行本地任务。

Result: 公共数据集精度98.9%，mAP@0.5为90.7%；自定义数据集精度95.0%，mAP@0.5为92.7%；系统成功率最高95%，用户满意度平均3.69/5。

Conclusion: 改进手势检测与多模态智能体的结合在资源受限机器人交互中可行且有效。
...
### [262] [MIND-CAVs: Multi-Intelligence Negotiation and Decision System for CAVs based on Intent-Driven Autonomy](https://arxiv.org/abs/2607.14688)
*Mainak Mondal,Yihang Feng,Yangchao Luo,Han Song*

Main category: cs.RO

TL;DR: 本文提出MIND-CAVs框架，通过意图驱动、V2X通信和边缘-云协作实现全局一致的车辆协调决策，在仿真中提升了机动效率与安全性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶车辆作为孤立代理运行，缺乏高层意图共享与全局协调机制，无法实现意图感知的一致性决策。

Method: 车辆将传感器数据抽象为结构化意图表示，通过V2X交换；边缘服务器结合学习与规则仲裁机制协商冲突意图；云端记录决策用于审计与持续训练。

Result: 相比孤立系统、先到先服务和多智能体强化学习基线，该方法改善了机动完成时间，减少了不安全接近和过度刹车。

Conclusion: MIND-CAVs通过意图驱动和边缘仲裁有效提升了多车协调性能，为互联自动驾驶提供了可行方案。
...
### [263] [Reflex: Real-Time VLA Control through Streaming Inference](https://arxiv.org/abs/2607.14695)
*Yuanchun Guo,Bingyan Liu*

Main category: cs.RO

TL;DR: 提出Reflex框架，通过时间步不变性、自适应归一化、异步流水线和算子融合，实现流匹配VLA模型的实时流式推理，速度提升2.58倍，达到50Hz稳定流式处理。


<details>
  <summary>Details</summary>
Motivation: 流匹配VLA模型的迭代去噪特性导致全局时间步注入使KV缓存失效，需要大量重计算或错误的缓存复用，无法满足实时机器人控制需求。

Method: 利用时间步不变性将注意力上下文分为静态、滑动和动态区域，实现O(1)增量缓存更新；引入AdaRMSNorm防止BFloat16数值崩溃；设计异步流水线解耦视觉编码与动作生成，并结合算子融合减少内核开销。

Result: 在LIBERO和Kinetix基准上，推理速度提升2.58倍，实现50Hz稳定流式推理，反应延迟降低最多54%，且无性能退化。

Conclusion: Reflex有效解决了流匹配VLA模型在实时机器人部署中的计算瓶颈，实现了高效、稳定的实时推理。
...
### [264] [Lights, Camera, Malfunction: When Illumination Robustness Leaves VLA Models Blind to Color](https://arxiv.org/abs/2607.14698)
*Marino Watanabe,Takami Sato,Kentaro Yoshioka*

Main category: cs.RO

TL;DR: 本文提出FLARE攻击和ChromaGuard防御，揭示对抗训练中数据增强导致VLA模型颜色感知崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: VLA模型在实际环境中对微小扰动（如光照变化）表现脆弱，现有对抗训练可能无意中破坏模型对颜色的依赖。

Method: 提出FLARE物理光照攻击框架（黑盒攻击），诊断性灰度评估暴露防御缺陷，以及ChromaGuard保色对抗训练方法。

Result: FLARE攻击使基线任务成功率降至0%；ChromaGuard在攻击下实现92.5%成功率，而未防御模型在颜色相关任务中仅47.5%。

Conclusion: 对抗训练需谨慎处理颜色信息，ChromaGuard可有效恢复模型对颜色的感知并抵抗物理攻击。
...
### [265] [Reinforcement Learning for the Full Strawberry Harvesting Process: Obstacle Separation, Detachment, and Placement](https://arxiv.org/abs/2607.14708)
*Changyou Miao,Teng Li,Ya Xiong*

Main category: cs.RO

TL;DR: 提出了一种策略驱动的强化学习框架，结合启发式阶段协调，实现机器人在严重遮挡和变形植物结构下的草莓采摘，仿真和真实世界成功率分别达89.7%和82.0%。


<details>
  <summary>Details</summary>
Motivation: 严重的遮挡和植物结构变形带来了复杂的接触动力学，挑战了机器人的草莓采摘任务。

Method: 采用策略驱动的强化学习框架，将障碍分离、果实摘取和放置作为顺序决策任务；通过共享交互感知策略生成笛卡尔运动，用轻量级启发式逻辑协调任务阶段和夹爪事件；使用结构化观察空间表示目标、障碍、末端执行器和任务上下文；结合高层策略与低层笛卡尔阻抗控制实现柔顺交互；采用可行性优先的观察对齐和域随机化实现零样本仿真到真实迁移。

Result: 仿真成功率达89.7%，真实实验成功率为82.0%；遮挡级别从1增加到5时，平均执行时间从12.99秒增至21.73秒。

Conclusion: 该策略展示了交互感知的采摘行为成功迁移到了结构不同的机器人平台上。
...
### [266] [BridgeFlow: Fast and Robust SE(2)-Equivariant Motion Planning with Flow Matching](https://arxiv.org/abs/2607.14725)
*Xinzhe Zhou,Xuyang Wang,Xiaoming Duan,Jianping He*

Main category: cs.RO

TL;DR: 提出BridgeFlow，一种严格SE(2)等变的快速生成运动规划框架，通过轻量级规范化和布朗桥先验实现高效推理与泛化。


<details>
  <summary>Details</summary>
Motivation: 现有学习型规划器要么缺乏等变性，要么依赖计算昂贵的专用架构，导致实时推理瓶颈。

Method: 采用任务中心规范化模块实现等变，结合布朗桥先验与上下文感知小批量最优传输构造直化矢量场，并使用分类器无关引导嵌入环境感知。

Result: 在密集2D环境和7-DoF Franka机械臂上实现高达15倍推理加速和2倍有效轨迹率提升，对未见环境和任意空间变换具有鲁棒泛化。

Conclusion: BridgeFlow打破了等变性与推理速度之间的权衡，为实时等变运动规划提供有效解决方案。
...
### [267] [Hybrid Rigid-Soft Robotic Gripper with Shape Adaptation, Uniform Force Distribution, and Self-Locking Capabilities](https://arxiv.org/abs/2607.14730)
*Xi Chen,Yun Wang,Lichao Yang,Haitao Li,Ya Xiong*

Main category: cs.RO

TL;DR: 提出了一种混合刚柔夹爪，结合气动执行器和棘轮机构，实现了自适应、均匀力分布和能量自锁，负载4200g，能耗降低50%。


<details>
  <summary>Details</summary>
Motivation: 传统夹爪在农业自动化中难以平衡柔顺性、力均匀分布和高负载能力，且能耗高。

Method: 混合刚柔夹爪，采用低成本膜基气动执行器和3D打印双棘轮机构，偏移配置提高关节锁止角度分辨率。

Result: 最大负载4200g，力分布差异1.75-35.29%，能耗42.6J（降低50.05%），优于传统软体夹爪。

Conclusion: 该夹爪弥合了软体柔顺性与刚性可靠性之间的差距，为农业自动化提供了低成本和高效的解决方案。
...
### [268] [Curvature-Constrained and Constant-Speed Distributed Simultaneous Arrival Control for Multi-Robot Systems](https://arxiv.org/abs/2607.14781)
*Zhouru Xiao,Yang Lu,Weijia Yao,Min Liu,Yaonan Wang*

Main category: cs.RO

TL;DR: 提出一种分布式切换控制方法，使多移动机器人在常速和曲率约束下同时到达目标点。


<details>
  <summary>Details</summary>
Motivation: 现有协同到达问题存在曲率约束与常速要求，且控制律需分布式实现。

Method: 基于最大一致性协议，融合Dubins路径几何特性与最优控制+饱和比例控制的混合律，引入虚拟时间变量。

Result: 理论证明可达最优时间，仿真与实验验证了可扩展性、实时性与鲁棒性。

Conclusion: 该方法在轻微条件下实现同时到达，兼具最优性与低通信开销。
...
### [269] [Interventional Causal Circuits for Safe Robot Action Testing and Failure Recovery](https://arxiv.org/abs/2607.14826)
*Naren Vasantakumaar,Tom Schierenbeck,Michael Beetz*

Main category: cs.RO

TL;DR: 提出一个闭环框架，将联合概率树与因果电路结合，实现机器人动作参数的因果诊断与纠正，无需额外训练数据。


<details>
  <summary>Details</summary>
Motivation: 传统形式化测试计算成本高，且被拒绝后盲目重采样效率低下、无收敛保证，亟需有原则的因果诊断方法。

Method: 基于边际确定性变量树构建因果电路，与联合概率树耦合，支持在线精确多时计算所有干预查询。

Result: 在ROS2仿真中，高质量JPT下失败尝试减少10.3%，低质量JPT下减少37%。每次拒绝都生成结构化因果报告。

Conclusion: 该框架无需单独训练故障模型，即可实现可解释的自主恢复与操作监督。
...
### [270] [KineFuse: Kinematic-Aware Haptic Fusion for In-Hand Occluded-Object Pose Tracking](https://arxiv.org/abs/2607.14842)
*Chanyoung Ahn,Jaesung Lee,Sungwoo Park,Donghyun Hwang*

Main category: cs.RO

TL;DR: 本文提出一种运动学感知的手指级编码器，将稀疏触觉信号与视觉跟踪结合，在遮挡下实现灵巧手内操作的6D位姿跟踪，并验证了序列跟踪优于逐帧评估，且紧凑的令牌化表示效果最佳。


<details>
  <summary>Details</summary>
Motivation: 灵巧手内操作需要持续的6D位姿跟踪，但由于机械手指遮挡导致视觉信号缺失，现有方法难以处理。

Method: 提出运动学感知的手指级编码器，融合本体感觉、近端力/扭矩和二进制接触信号，与预训练的视觉位姿跟踪器结合，并通过三种评估层次（逐帧、序列开环、闭环操作）对四种对比设计进行系统比较。

Result: 逐帧评估无法区分编码器质量；序列跟踪放大了编码器差异（高达15倍）；结构化编码器学习任务特定的跨模态门控（视觉负责平移，触觉负责旋转）；紧凑的手指级4令牌表示优于扁平融合和关节级表示。

Conclusion: 改进的跟踪提高了下游重定向任务的成功率，并在真实世界演示中得到验证，表明结构化触觉编码在非视觉环境下的重要性。
...
### [271] [Towards Human-like Physical Intelligence: LifelongVision-Language-Action Learning for Robotic Manipulation](https://arxiv.org/abs/2607.14852)
*Yao He,Gan Sun,Wenqi Liang,Fazeng Li,Yang Cong*

Main category: cs.RO

TL;DR: 提出了一个用于机器人操作的终身视觉-语言-动作学习框架LifelongVLA，通过双时间尺度适应和缓存高效重放策略解决可塑性-稳定性权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有终身学习模型难以平衡学习新任务的可塑性和保持旧任务准确率的稳定性，这一权衡在机器人操作模型中尚未解决。

Method: 设计了双时间尺度LoRA门控模块（短期适配器用于可塑性，长期适配器用于稳定整合）和缓存高效随机重放策略。

Result: LifelongVLA在xArm机器人实验上优于基线方法，实现了高效技能扩展、稳定保留已学操作行为，并减少了对重新训练的依赖。

Conclusion: 该方法有效缓解了可塑性-稳定性权衡，降低了部署成本，适用于机器人在开放环境中的终身学习。
...
### [272] [OASIS-Map: Object-Level Change Detection in Multi-Session Mapping using Semantic Correspondence Matching](https://arxiv.org/abs/2607.14899)
*Haedam Oh,Yifu Tao,Nived Chebrolu,Maurice Fallon*

Main category: cs.RO

TL;DR: 提出OASIS-Map，一种多会话地图系统，通过密集补丁级语义对应实现跨访问的物体级地图时空一致性。


<details>
  <summary>Details</summary>
Motivation: 解决半静态环境中地图过时问题，以及现有方法在物体关联、部分遮挡和不完美分割下的挑战。

Method: 通过建立时间观测间的密集补丁级语义对应，检测场景变化并增量关联物体。

Result: 在车替换场景中变化检测F1达0.783，3RScan中移动物体关联F1达0.667。

Conclusion: OASIS-Map能有效维持长期场景一致性，适用于多种现实变化场景。
...
### [273] [Human-Robot Interaction in GenAI Architectures via the Agent-Client Protocol](https://arxiv.org/abs/2607.14919)
*Jesus Moncada-Ramirez,Jose-Raul Ruiz-Sarmiento,Javier Gonzalez-Jimenez*

Main category: cs.RO

TL;DR: 提出基于ACP和MCP的三层解耦架构，实现异构用户界面与机器人系统的统一通信，支持实时人类参与协作。


<details>
  <summary>Details</summary>
Motivation: 现有机器人系统中，人类-机器人交互层因专有接口碎片化，阻碍实时人类参与协作。

Method: 采用ACP作为交互层统一通信协议，结合MCP，构建三层解耦架构（人类交互、规划决策、物理执行）。

Result: 在实体移动机器人上验证，三种异构界面实现互操作，实时人类参与流程延迟开销可忽略。

Conclusion: 该架构消除了刚性依赖，支持系统组件灵活替换，为机器人系统提供标准化人类交互通信协议。
...
### [274] [AeroAct: Action-Centered World-Action Models for Language-Conditioned Quadrotor Flight](https://arxiv.org/abs/2607.14997)
...
TL;DR: 提出AeroAct，首个用于真实世界四旋翼飞行的世界-动作模型，通过预测局部轨迹-动作块实现语言引导的导航。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用离散动作或高层航点，未能充分利用动作对未来观测的密集监督，且难以应对快速变化的视觉环境。

Method: 基于预训练视频扩散Transformer，从自我中心视觉历史、本体感知和语言中预测轨迹-动作块；训练时利用未来第一人称帧作为密集后果监督，部署时直接解码动作。

Result: 在闭环仿真和真实实验中，时间视觉上下文提升了目标跟踪和物体搜索性能，物理四旋翼成功执行了基于世界-动作模型的策略。

Conclusion: AeroAct证明了世界-动作模型在实际无人机导航中的可行性，通过密集视觉监督和低成本数据采集实现了有效导航。
...
### [275] [CosFly-VLA: A Spatially Aware Vision-Language-Action Model for UAV Tracking](https://arxiv.org/abs/2607.15004)
...
TL;DR: 提出了一种空间感知的VLA模型CosFly-VLA，用于在复杂城市环境中持续遮挡条件下跟踪动态目标。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略在目标被持续遮挡时性能下降，可能丢失目标状态并导致无法重新获取。

Method: 采用大规模数据空间预训练、三阶段课程监督微调、思维链训练和闭环强化学习，集成目标定位、可见性估计和连续飞行动作生成。

Result: 相比OpenVLA，开环ADE降低34.1%-35.3%，闭环SR提升29.8%和2.5%。

Conclusion: 从可见帧模仿向空间接地闭环控制取得进展，在共享历史状态下验证有效性。
...
### [276] [Risk-Aware Belief Control Barrier Functions over Random Finite Sets](https://arxiv.org/abs/2607.15016)
*Shaohang Han,Gang Chen,Yixi Cai,Ignacio Torroba,Ivan Stenius,Patric Jensfelt,Javier Alonso-Mora,Jana Tumova*

Main category: cs.RO

TL;DR: 提出了一种基于风险感知信念控制障碍函数（BCBF）的框架，利用随机有限集（RFS）和粒子滤波处理未知动态环境中的多目标状态不确定性，确保机器人安全。


<details>
  <summary>Details</summary>
Motivation: 在未知动态环境中，需要从噪声和不完整测量中推断未知且时变数量的移动物体状态，以确保机器人安全。

Method: 采用序贯蒙特卡罗概率假设密度（SMC-PHD）滤波器将不确定性表示为粒子集，并基于此构建非光滑BCBF，建立安全集前向不变性条件及离散更新安全条件。

Result: 仿真和水下真实实验验证了该方法在安全控制中的有效性和效率。

Conclusion: 所提框架能有效处理多目标状态不确定性，确保机器人安全，并具有实际应用潜力。
...
### [277] [Learning Agile Navigation in Crowded Environments for Quadruped Robots](https://arxiv.org/abs/2607.15036)
*Shuyu Wu,Zeyu Liu,Tianbao Zhang,Fanxing Li,Fangyu Sun,Mingkang Xiong,Wei Xi,Wenxian Yu,Danping Zou*

Main category: cs.RO

TL;DR: 提出VOP-Nav导航系统，结合速度障碍法的几何安全性与端到端学习的敏捷适应性，通过隐式编码动态约束实现四足机器人在密集人群中的安全高效导航。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在模型方法依赖精确障碍物估计但在密集场景易失效，而端到端学习方法缺乏运动预测能力导致碰撞或保守行为的问题。

Method: 使用VOP-Net处理多帧激光雷达数据，隐式编码动态约束并基于速度障碍法预测安全速度区域；VO预测同时作为导航策略的输入和训练奖励信号。

Result: 在Isaac Gym中，VOP-Nav在平衡速度和避碰方面成功率高；在Unitree Go2机器人上的真实测试验证了其在复杂室内外动态环境中的鲁棒性和效率。

Conclusion: VOP-Nav结合模型和学习的优势，有效解决了密集动态环境下的导航挑战。
...
### [278] [DriftWorld: Fast World Modeling through Drifting](https://arxiv.org/abs/2607.15065)
*Susie Lu,Haonan Chen,Weirui Ye,Yilun Du*

Main category: cs.RO

TL;DR: 提出DriftWorld，一种基于漂移生成模型的行动条件世界模型，在单次前向传播中快速生成未来帧，速度比扩散模型快17倍，在机器人操作任务中实现最先进的决策性能。


<details>
  <summary>Details</summary>
Motivation: 扩散世界模型的多步采样导致推理时动作搜索成本高，限制了大规模行动搜索，需要更快的规划方法。

Method: 基于漂移生成模型，训练时学习行动条件漂移，推理时从当前观测和候选行动序列单步生成未来帧。

Result: 在Bridge-V2等基准上以30+ fps运行，比扩散基线快17倍；决策性能达到最先进；离线仿真评分与真实世界相关性高达0.99。

Conclusion: 漂移模型因快速高质量想象，适合机器人世界建模，支持直接规划和策略评估。
...
### [279] [Goal-Oriented Semantic Communication for Distributed ISAC-Enabled Vehicle Coordination](https://arxiv.org/abs/2607.15111)
*Wenjie Liu,Yansha Deng*

Main category: cs.RO

TL;DR: 提出一种基于分布式集成感知与通信（ISAC）的车辆协同框架，通过面向目标的语义通信（GSC）和掩码混合近端策略优化（MHPPO）显著降低信令开销，实现100%无碰撞交叉口协调。


<details>
  <summary>Details</summary>
Motivation: 现有研究将感知、通信与控制分离处理，导致冗余传输、状态信息过时及不可靠的车辆协调问题。

Method: 1）使用扩展卡尔曼滤波（EKF）预测车辆状态并融合分布式感知数据；2）构建掩码混合近端策略优化（MHPPO）框架，基于信息价值（VoI）奖励联合决定感知/控制传输决策与内容；3）设计不确定性感知传输设计（UTD），包含鲁棒波束成形和基于VoI的时分功率分配。

Result: 与先进预测ISAC基线及消融基线相比，所提框架在显著降低信令开销的同时实现100%无碰撞车辆协调。

Conclusion: 面向目标的语义通信与强化学习结合能有效提升路口车辆协同效率与可靠性。
...
### [280] [Assessing Physical Frailty and Fall-Risk Indicators with Social Robots: An in situ Evaluation with Older Adults](https://arxiv.org/abs/2607.15156)
*Aniol Civit,Antonio Andriella,Alba Martínez,Joan Ars,Aida Ribera,Cristian Barrué,Guillem Alenyà*

Main category: cs.RO

TL;DR: 提出一种基于行为树架构的机器人系统，用于指导老年人完成标准化虚弱评估测试，并自动获取时间及生物力学等深度指标。


<details>
  <summary>Details</summary>
Motivation: 现有虚弱评估依赖人工且指标粗糙，缺少精准生物力学指标，需要一种资源节约、客观的自动化评估方案。

Method: 采用行为树整合感知（视觉骨架追踪）、决策、交互与测量模块，自主执行SPPB和TUG测试，并与临床金标准对比。

Result: 机器人所得任务完成时间及步态参数与临床仪器一致性极高（ICC>0.9），SPPB总分与治疗师评估的一致性为k=0.67，与IMU为k=0.55。

Conclusion: 社交机器人可在医疗环境中提供可靠、客观的虚弱评估，并自然获取超越传统结局的额外功能指标。
...
### [281] [Scaling Behavior Foundation Model for Humanoid Robots](https://arxiv.org/abs/2607.15163)
...
TL;DR: 本文提出了一种名为Humanoid Transformer的行为基础模型，通过协调运动跟踪学习范式、在线策略数据与参考动作多样性以及可扩展模型架构，实现了人形机器人全身协调控制与任务泛化的大幅提升。


<details>
  <summary>Details</summary>
Motivation: 现有行为基础模型在扩展时缺乏对学习范式、数据和模型架构协调性的系统性研究，导致性能提升有限。

Method: 1) 采用全局坐标系下的全身行为跟踪学习范式；2) 结合在线策略 rollout 数量与参考动作多样性的协同策略；3) 设计Humanoid Transformer模型以实现结构化行为表征。

Result: 在测试集上，局部模式下Mean Per-Keypoint Position Error (MPKPE)降低超过10%，全局模式下降低82%，显著优于现有控制器。

Conclusion: 行为基础模型通过协调三大核心组件，可成为可扩展通用人形控制的有效基础方法。
...
### [282] [AHEAD: Anticipatory Hand-Driven Teleoperation via Human Intent Prediction](https://arxiv.org/abs/2607.15172)
*Seok Joon Kim,Junho Lee,Federica Spinola,Taein Kwon,Mohsen Moghaddam*

Main category: cs.RO

TL;DR: 提出AHEAD系统，通过预测操作员意图（抓取对象和放置槽位）实现主动手控遥操作，减少机器人反应延迟和操作员负荷。


<details>
  <summary>Details</summary>
Motivation: 现有直接手控遥操作需要持续监控，耗力且慢；监督遥操作有延迟。需减少机器人反应时间并降低操作员负荷。

Method: 利用数字孪生中短窗3D手部、头部信号与场景上下文，通过注意力分类器预测意图；状态机将预测转化为可靠机器人目标，处理噪声和纠正动作。

Result: 意图预测Top1准确率：抓取对象76%，目标槽位76%；用户研究中机器人反应延迟减少0.6秒（对象）和1.4秒（槽位），操作员负荷更低。

Conclusion: AHEAD通过预期操作员意图实现主动控制，降低延迟和疲劳，提升效率。
...
### [283] [RoboTTT: Context Scaling for Robot Policies](https://arxiv.org/abs/2607.15275)
...
TL;DR: 提出了RoboTTT，一种通过测试时训练将视觉-运动上下文扩展到8000时间步的机器人模型，实现了单次人类视频演示的上下文模仿学习、实时策略改进和长时任务完成。


<details>
  <summary>Details</summary>
Motivation: 现有机器人基础模型仅能处理单步或短历史上下文，无法有效利用长时间跨度的视觉运动信息，限制了机器人策略的鲁棒性和泛化能力。

Method: 将测试时训练集成到视觉-语言-动作策略中，通过梯度下降更新快速权重作为循环状态，结合序列动作强制和截断反向传播来训练长上下文。

Result: 在真实机器人操作任务中，相比单步基线提升了87%的性能，能完整完成5分钟10阶段的组装任务；8K上下文训练比1K上下文训练提升62%。

Conclusion: 上下文长度是机器人基础模型的新扩展轴，通过长上下文和测试时训练可以显著提升机器人策略的闭环性能和鲁棒性。
...
### [284] [Information-Theoretic Adaptive Cooling for Deterministic MPPI via Entropy Feedback](https://arxiv.org/abs/2607.14245)
*Shuqi Wang,Wenrong Sun,Tao Han,Yue Gao,Xiang Yin*

Main category: eess.SY

TL;DR: 提出一种基于信息论的自适应冷却框架，利用重要性权重的香农熵在线调节温度，实现确定型MPPI控制的快速收敛。


<details>
  <summary>Details</summary>
Motivation: 现有确定型MPPI依赖预设开环冷却策略，效率低且鲁棒性差。

Method: 利用重要性权重的香农熵作为反馈信号，自适应调节冷却速率，并推导关键熵阈值防止早熟坍塌。

Result: 在非光滑信号时序逻辑运动规划任务中，ITAC比基线方法采样效率更高，收敛更快。

Conclusion: 该方法在不牺牲无导数特性的前提下，显著提升了算法鲁棒性和收敛速度。
...
### [285] [Learning reduced-order latent linear models for Kalman filtering of nonlinear systems](https://arxiv.org/abs/2607.14273)
*Manas Mejari,Milad Banitalebi Dehkordi,Dario Piga*

Main category: eess.SY

TL;DR: 提出了一个面向滤波的端到端学习框架，用于识别面向状态估计的降阶模型，并通过共形预测量化性能差距。


<details>
  <summary>Details</summary>
Motivation: 传统的两步法（先降阶再滤波）导致模型参数仅优化系统近似精度，而非滤波性能，可能影响状态估计效果。

Method: 使用自编码器学习低维潜在表示和提升映射，结合线性时不变模型描述潜在动力学，并通过可微卡尔曼滤波的多目标损失联合训练。

Result: 在热扩散基准测试中，从稀疏测量重构完整温度场，验证了有效性。

Conclusion: 该框架能学习任务导向的降阶模型，其参数由滤波性能决定，优于传统两步法。
...
### [286] [Wasserstein Stability of Contracting Flows: Effective Rates, Euler Self-Correction, and Noise Tightening](https://arxiv.org/abs/2607.14291)
*Ali Baheri*

Main category: eess.SY

TL;DR: 本文针对非线性收缩系统的Wasserstein距离保守界问题，提出了位移加权平均收缩率的更紧界，并首次理论描述了收缩下的欧拉离散化误差的非单调指数衰减特性，同时证明非线性收缩系统在稳态方差上优于同最差收缩率的线性系统。


<details>
  <summary>Details</summary>
Motivation: 经典Wasserstein分布界仅对线性系统紧致，对非线性系统因使用最差收缩率而过于保守，忽视了空间变化的局部收缩率和分布信息，导致难以准确评估非线性收缩系统的性能。

Method: 将传统单值最差收缩率替换为位移加权分布平均局部收缩率，推导更紧的Wasserstein界；理论分析收缩下欧拉离散化误差的演化规律；比较非线性与线性系统在相同最差收缩率下的稳态方差。

Result: 新Wasserstein界严格优于经典界；欧拉误差非单调，在通用时间峰值后指数衰减；非线性收缩系统的稳态方差严格小于线性系统。

Conclusion: 非线性收缩动力学在分布收敛、离散化误差和噪声抑制方面具有固有优势，本文方法有效降低了经典分析的保守性。
...
### [287] [Assessing Risks of Hydro-Generator Shaft Fatigue from Data Center Load Oscillations](https://arxiv.org/abs/2607.14412)
*Kaustav Chatterjee,Meghana Ramesh,Shuchismita Biswas,Brett A. Ross,Antos C. Varghese,Sameer Nekkalapu,Slaven Kincic*

Main category: eess.SY

TL;DR: 提出一个模型框架，评估AI数据中心振荡负载对水轮发电机轴疲劳风险的影响，通过电磁暂态仿真和频率扫描方法识别共振区域，并基于Goodman安全系数提供实用评估指标。


<details>
  <summary>Details</summary>
Motivation: 大型AI数据中心负载可能引发持续的次同步有功功率振荡，激励附近发电机组的扭转模态，增加轴应力，亟需评估这种振荡对水轮发电机疲劳寿命的影响。

Method: 使用两质量涡轮发电机轴模型的电磁暂态仿真；通过网络传递函数和电厂传递函数分析振荡传播与轴扭矩放大；采用频率扫描识别共振区域；基于S-N曲线和Goodman图进行疲劳评估。

Result: 扭矩放大受发电机与涡轮惯性比和扭转阻尼强烈影响；低惯性比将扭转模态移至更低频率并增加放大，Kaplan型机组可能比Francis或Pelton型更敏感；减少阻尼会加剧共振响应和疲劳暴露。

Conclusion: Goodman安全因子是评估AI数据中心振荡对水轮发电机寿命影响的实用指标，可用于互连研究、振荡限值和电厂监控策略。
...
### [288] [A Distributed PI+Reset Scheme for Discrete-Time Economic Dispatch of A Grid-connected BESS Network](https://arxiv.org/abs/2607.14508)
*Yalin Zhang,Zhongxin Liu,Fuyong Wang,Zengqiang Chen*

Main category: eess.SY

TL;DR: 提出了一种基于离散时间多智能体系统的新型比例积分控制器的分布式解决方案来解决电池储能网络的经济调度问题，降低了运行成本。


<details>
  <summary>Details</summary>
Motivation: 电池储能网络的运行成本因内部功耗和容量退化而持续增加，且与电网交易也构成成本。

Method: 采用离散时间多智能体系统，设计了一个基于比例积分控制器的边际成本共识控制器和平均功率失配共识控制器，并引入了重置机制。

Result: 该方案加速了收敛速度，提高了控制精度且无显著超调，仿真验证了其有效性和先进性。

Conclusion: 提出的算法有效解决了电池储能网络的经济调度问题，降低了成本。
...
### [289] [An Evidential Reasoning Approach for Aerial Target Classification and Intent Prediction](https://arxiv.org/abs/2607.14606)
*Tenzing Thiley Bhutia,Subash Kumaraguru,Devaprakash Muniraj*

Main category: eess.SY

TL;DR: 提出一种基于短时序子样本和证据推理的集成方法，实现空中目标分类与意图预测，在部分数据下达到88%和93%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖长时序数据，不适用于需要快速决策的作战场景；同时需要最小化错误预测以避免高风险环境中的误判。

Method: 从短时序子样本生成预测，通过子样本间置信传播优化结果；使用证据推理框架融合分类器输出，结合规则和距离方法推断意图。

Result: 在8个目标的案例研究中，目标分类准确率达88%，意图预测准确率达93%。

Conclusion: 该方法能在部分数据下有效实现快速、可靠的空中目标分类与意图预测，适用于高动态威胁环境。
...
### [290] [Consistent Variance Estimation for Q-Function Estimators in Finite-Horizon MDP Tree Search](https://arxiv.org/abs/2607.14555)
*Zhenyu Yue,Jie Xu,Chun-Hung Chen,Hadi El-Amine,Michael C. Fu*

Main category: eess.SY

TL;DR: 本文研究了有限时域、有限状态马尔可夫决策过程（MDP）树搜索中Q函数估计量的方差，揭示了其分解为三个组成部分，并指出了基于独立同分布路径的样本方差估计量存在偏差且不消失。提出了一个一致的递归方差估计量，并实现为节点本地统计的迭代更新。将该估计量集成到蒙特卡洛树搜索（MCTS）中，在库存控制和肾脏配对捐赠匹配的数值示例中，相比基线方法提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有MDP树搜索中Q函数方差的估计方法可能不准确，且基于独立同分布假设的样本方差估计量存在偏差，影响MCTS算法的性能。

Method: 通过理论分析分解方差为即时奖励、状态转移和未来值函数不确定性三个成分；开发递归方差估计量并推导等效的节点本地统计实现；将估计量集成到MCTS采样过程中。

Result: 在库存控制和肾脏配对捐赠匹配的数值实验中，新估计量改进了MCTS算法的性能，优于使用独立同分布样本方差估计量的基线方法。

Conclusion: 所提出的递归方差估计量能够提供一致的无偏估计，并有效提升MCTS在有限时域MDP中的实际表现。
...
### [291] [Modular Sign Compensation for MIMO Systems with Unknown Control Direction: An Exact Nominal Recovery Approach](https://arxiv.org/abs/2607.14839)
*Diego Gutiérrez-Oribio,Ioannis Stefanou*

Main category: eess.SY

TL;DR: 针对MIMO系统中不确定时变对角输入方向问题，提出了一种模块化符号补偿层，通过有限次符号变化保持标称控制特性，并设计了两种调度器以恢复标称稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如Nussbaum增益、监视函数、二元自适应）在处理输入方向不确定时存在局限性，需要一种保持标称控制幅值且无需复杂机制的方法。

Method: 提出了由标称李雅普诺夫量构建的自适应变量的模块化符号补偿层，通过两种调度器（向量调度器和标量模式调度器）探索未知输入符号配置，并在正确配置时恢复标称闭环。

Result: 在飞行滚转反转、视觉伺服基准和地下储层控制（人为地震缓解）三个案例中验证了方法的有效性，能恢复标称稳定性。

Conclusion: 该补偿层方法有效解决含不确定时变输入方向的MIMO系统稳定性问题，具有普适性和实用价值。
...
### [292] [Finite-Sample Conformal Coverage Recovery via Fusion under Degraded Local Guarantees in Occupancy Map Estimation](https://arxiv.org/abs/2607.14906)
*Ritvik Mahajan,Aneesh Raghavan,Karl Henrik Johansson*

Main category: eess.SY

TL;DR: 提出一种分布式保形预测融合算法，使多机器人系统即使在局部地图保证退化的情况下，也能恢复全局覆盖保证，并能在通信拓扑和传感器噪声未知时提供有限样本的统计保证。


<details>
  <summary>Details</summary>
Motivation: 现有连续映射方法（如高斯过程占用映射）缺乏有限样本保证，而保形预测虽能提供分布无关的覆盖保证，但在实际多机器人场景中因轨迹时间相关性和空间观测不均匀导致局部保证退化。

Method: 机器人间仅交换轻量标量e值，接收方使用每邻域误覆盖预算和不确定性衰减融合算子进行融合，形成基于融合证据的集合值地图。

Result: 融合预测器在模拟实验中可靠达到理论覆盖界，且密集通信拓扑显著减少未分类区域，提高地图效率。

Conclusion: 该算法能恢复目标覆盖水平，但代价是证据不足时地图放弃决策，保留未分类区域而非强制二分类。
...
### [293] [A Model Predictive Control Framework for Assisted Vehicle Drifting](https://arxiv.org/abs/2607.15117)
*Marco Cortese,Antonio Gallina,Matteo Grandin,Giovanni Righetti,Mattia Bruschetta,Basilio Lenzo*

Main category: eess.SY

TL;DR: 提出了一种基于非线性模型预测控制的辅助漂移框架，允许驾驶员通过方向盘控制侧滑角，同时控制器负责稳定性，在仅使用简单预测模型和基本轮胎动力学的情况下实现了高保真仿真中的稳定漂移。


<details>
  <summary>Details</summary>
Motivation: 现有的辅助漂移方法依赖预计算平衡点、完全控制权限或路径预设等限制性假设，限制了其在非专业驾驶员中的应用。

Method: 采用非线性模型预测控制框架，通过线控转向和线控驱动接口解耦驾驶员指令与执行器输入，结合专用激活逻辑仅在驾驶员明确意图时介入。

Result: 高保真仿真表明，即使驾驶员连续变化侧滑角参考值，该架构也能稳定漂移操作，仅需简单的单轨预测模型和基础轮胎动力学。

Conclusion: 该NMPC框架为后轮驱动车辆的辅助漂移提供了一种实用解决方案，降低了对驾驶员专业技能的要求，同时保持稳定性和操控灵活性。
...
### [294] [Integrated Discovery and State-Aware Servicing for Mobile AUVs With UOWC: Modeling and Performance Analysis](https://arxiv.org/abs/2607.15183)
*Qiyu Ma,Jiajie Xu,Mohamed-SlimAlouini*

Main category: eess.SY

TL;DR: 提出了一种集成任务级框架，结合随机节点发现与状态感知服务，通过SA-OPS调度方案优化水下无线光通信网络的能效平衡。


<details>
  <summary>Details</summary>
Motivation: 解决水下无线光通信节点能量受限问题，通过AUV联合无线信息和能量传输延长网络寿命。

Method: 基于SNR分析的节点发现概率模型与状态感知最优点服务（SA-OPS）阈值调度框架，实时选择预充电、通信后充电或仅通信操作。

Result: SA-OPS在指定参数范围内改善AUV能耗与全网能量健康的权衡，且充电阈值可用简单状态依赖启发式近似。

Conclusion: 为水下自主能量补给提供了实用指导，验证了状态感知调度在有限假设下的有效性。
...
### [295] [OvAi Focus: AI-based Multi-class Segmentation of Functional Ovaries and Adnexal Masses in Gynecological Ultrasound](https://arxiv.org/abs/2607.14179)
...
TL;DR: 提出多类语义分割AI软件OvAi Focus，用于超声图像中卵巢及附件肿块（囊性/实性）分割，多中心验证DICE 0.62–0.87。


<details>
  <summary>Details</summary>
Motivation: 卵巢癌致死率高，超声中附件肿块和功能性卵巢的分割因操作者变异和形态复杂而缺乏客观准确方法。

Method: 基于深度学习的多类语义分割AI医疗设备（OvAi Focus），在6个中心1081名成人女性数据集上训练与独立验证。

Result: 分割DICE：完整病灶0.87，囊性0.85，实性0.68，功能卵巢0.62，达到或超越现有最优方法。

Conclusion: 该AI系统能在异质采集环境下实现稳健、客观的卵巢及附件肿块分割，具有临床辅助诊断潜力。
...
### [296] [A Hybrid Framework for Blood Vessel Morphology Classification: Discrete Geometry-based Tortuosity Feature Measurement, Information Gain-based Feature Selection, and Random Forest Classification](https://arxiv.org/abs/2607.14195)
*Yu Zhong,Jingzhi Guo,Luyao Li,Zehao Wang,Zhihui Yang,Yixin Lin,Weilun Fu,Yang Wang*

Main category: eess.IV

TL;DR: 提出了一套基于离散几何特征、信息增益特征选择和随机森林的ICA-C1形态分类框架，在二元和三元分类任务中分别达到0.9206和0.8626的Macro-F1分数，并定义了MRI指标辅助临床评估。


<details>
  <summary>Details</summary>
Motivation: 主观分级依赖经验，传统指数无法充分表征三维变形，且颈内动脉形态异常与脑血管评估和中风风险相关，需要客观可重复的量化方法。

Method: 从379条临床血管中心线提取13个迂曲度特征，经信息增益筛选为6个（TI, AC, TC, AC/AT, AT, TT），使用随机森林进行二元（非严重/严重）和三元（直/低迂曲/高迂曲）分类。

Result: 二元分类Macro-F1=0.9206，三元分类Macro-F1=0.8626；伸长曲率特征对基本筛查有效，扭转特征提升精细分类效果。

Conclusion: 该框架可客观量化血管形态，结合MRI指数可能促进更一致、客观的临床评估。
...
### [297] [FORCE-Interior: A Poisson Flow Generative Prior for Interior Tomography Reconstruction](https://arxiv.org/abs/2607.14320)
*Kang Chen,Wenjun Xia,Jianxu Wang,Mahmud Wasif Nafee,Ge Wang*

Main category: eess.IV

TL;DR: 提出FORCE-Interior框架，结合泊松流生成模型与全视野测量约束初始化，解决内层断层成像中的投影截断问题，实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 内层断层成像投影截断导致病态逆问题，现有方法泛化性差且缺乏数据一致性约束。

Method: 利用泊松流生成模型，结合全视野测量约束初始化与逐步数据一致性约束，锚定生成采样过程。

Result: 在严重截断的感兴趣区域尺寸下，结构及感知质量提升，同时保持投影域一致性。

Conclusion: FORCE-Interior框架有效改善内层CT重建质量，尤其适用于高截断场景。
...
### [298] [ViPSAM: Visual Prompting Medical Image Segmentation Using Segment Anything Model](https://arxiv.org/abs/2607.14328)
*San Lee,Nalee Kim,Jeong Il Yu,Hee Chul Park,Boah Kim*

Main category: eess.IV

TL;DR: 提出ViPSAM框架，利用对比增强MRI的视觉提示辅助非对比CT的病灶分割，在质子治疗规划中提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 非对比CT图像中病灶与背景对比度低，传统方法及学习模型在非增强图像上分割效果不佳，而临床实践中常参考对比增强MRI进行标注。

Method: 基于SAM模型，引入视觉提示编码器提取对比增强图像特征，并通过视觉引导交叉注意力模块融合非对比与对比增强特征，适配掩码解码器实现高效参数微调。

Result: 在质子治疗的非对比CT肝脏病灶分割任务中，ViPSAM优于典型U-Net和SAM方法。

Conclusion: 跨模态视觉提示可提升非对比图像的鲁棒性和准确分割性能。
...
### [299] [Domain Adaptation of Mismatched Proximal Denoiser for Plug-and-Play Image Reconstruction](https://arxiv.org/abs/2607.14894)
*Guixian Xu,Jinglai Li,Junqi Tang*

Main category: eess.IV

TL;DR: 本文定义了PnP-PGD中由域不匹配引起的近端失配，证明了收敛率与失配量相关，并提出通过近端匹配自适应来改善重构质量。


<details>
  <summary>Details</summary>
Motivation: 现有PnP-PGD分析假设去噪器是近端映射或收缩，但未考虑域不匹配对收敛的影响。

Method: 定义近端失配，推导站定界，使用近端匹配自适应替代MSE自适应。

Result: 近端匹配自适应在域偏移下显著提升重构质量，尤其在少样本场景中。

Conclusion: 近端匹配自适应优于MSE自适应，对域迁移更具鲁棒性。
...
### [300] [Deep Scene-Driven Ordering of Hadamard Basis for Single-Pixel Spectral Imaging](https://arxiv.org/abs/2607.15045)
*Brayan Monroy,Hans Garcia,Henry Arguello,Jorge Bacca*

Main category: eess.IV

TL;DR: 提出了一种基于场景驱动的Hadamard基排序方法，用于优化光谱成像中的单像素成像系统，显著提升了图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度光学编码设计的单像素成像系统受限于非反馈光学设计，图像质量有限，且仅在训练场景中表现最优，缺乏通用性。

Method: 重构深度光学编码设计框架，引入场景驱动的Hadamard矩阵排序，在端到端优化中根据场景特征灵活选择调制模式。

Result: 在光谱数据集仿真和真实测试平台上，所提方法在可见光和近红外光谱图像质量上优于固定设计。

Conclusion: 该场景驱动方法有效提升了单像素光谱成像的质量和泛化能力。
...
### [301] [ESAR: Event-Based Synthetic Aperture Reconstruction](https://arxiv.org/abs/2607.15073)
*Harbir Antil,Daniel Blauvelt,David Sayre*

Main category: eess.IV

TL;DR: 本文提出了一种基于事件相机的单目成像方法，将事件数据建模为合成孔径逆问题，通过正则化反演恢复静态场景的对数辐射场，在近天底运动下能恢复大尺度空间结构并抑制细纹理。


<details>
  <summary>Details</summary>
Motivation: 事件相机输出异步极性事件而非传统帧，现有方法直接重建潜在像素-时间体积，未充分利用场景几何约束，导致计算复杂且效果受限。

Method: 将问题分解为几何映射P和时域差分算子A，建立线性模型APθ=b+η，利用正则化反演直接恢复静态场景的对数辐射场θ，避免重建整个体积。

Result: 数值实验和Falcon Neuro事件数据表明，相比动态图像和基于学习的基线，该方法在近天底运动下能恢复一致的大尺度结构，同时抑制精细纹理。

Conclusion: 提出的θ基公式通过合成孔径结构有效解决了事件反演问题，适用于实际事件相机数据。
