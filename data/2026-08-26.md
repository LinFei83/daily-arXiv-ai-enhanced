...
TL;DR: 该论文提出了RENDER基准测试，旨在揭示RAG和记忆评估中，不同的输入呈现格式（如原始对话、摘要、结构化记录）对模型表现的显著影响。


<details>
  <summary>Details</summary>
Motivation: 现有评估通常忽略了系统输入格式的差异。研究发现同一内容在不同呈现方式下模型表现迥异，因此需要一个标准化的评估框架来消除这一变量影响。

Method: 引入RENDER基准，通过五级数据包分级方法，结合ChatGPT风格记录、LangChain摘要、MemGPT格式及原始对话等四种确定性模板，测试不同呈现方式下的模型性能。

Result: 实验发现输入呈现格式对性能影响巨大（最佳与最差间差距高达24.6-48.8点），即便在相同检索预算下，结构化数据包表现显著优于原始对话；某些模型在原始格式下得分为0，但在自然语言记录格式下可达到45%-53%的准确率。

Conclusion: 模型输出对输入呈现形式极其敏感，未来的RAG和记忆评估研究应明确报告或控制输入数据的呈现方式，以确保评估结果的准确性和可比性。
...
TL;DR: 该论文提出了ESQ-Bench，一个专门用于企业级数据库环境的NL2SQL基准测试，旨在解决现有基准测试（如Spider和BIRD）无法反映复杂企业Schema和多SQL方言兼容性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有NL2SQL模型在学术数据集上表现优异，但在复杂真实企业数据库环境下的表现缺乏评估，且现有基准测试对多SQL方言和企业架构的覆盖不足。

Method: 构建了包含6个企业级Schema、550对金标准问题-查询对的ESQ-Bench，支持Oracle、PostgreSQL、MySQL和SQL Server，并引入了包含执行匹配(EX)和静默差异(SD)在内的四维评估体系。

Result: 研究发现模型性能随复杂度提升呈单调下降趋势，Claude Sonnet 4.6的表现优于GPT-4o，而开源模型（如Llama 3.2）在企业级Oracle架构上的能力与闭源模型存在巨大差距。

Conclusion: NL2SQL模型在处理复杂企业级数据库时仍存在显著的语义错误和静默差异问题，当前的学术基准测试不足以代表企业级应用场景的实际难度。
...
TL;DR: 提出了一种将大语言模型（LLM）与高保真科学模拟模型相结合的多智能体框架，用于制药工艺设计的受控实验。


<details>
  <summary>Details</summary>
Motivation: LLM虽具备推理能力，但缺乏对系统干预反应的物理理解，而科学工程任务需要基于实验观测的严谨分析。

Method: 构建多智能体系统，通过任务结构化、实验设计、对比模拟执行、结果解读及证据合成，实现闭环实验推理。

Result: 相比纯语言模型，该框架在制药工艺参数优化中表现出更高的输出具体性、正确性及用户满意度。

Conclusion: 将模拟模型集成到LLM智能体中，能有效弥补模型在干预推理方面的不足，提升科学研究的实际决策能力。
...
TL;DR: 该研究审计了天文基础模型AION-1，发现其严重依赖不完整的天文巡天目录信息，导致预测结果出现显著偏差。


<details>
  <summary>Details</summary>
Motivation: 天文基础模型在训练时常结合图像像素与派生目录产品，而这些目录本身存在系统性的不完整性，研究旨在量化并揭示这种偏差对模型性能的影响。

Method: 通过因果干预手段，在保持图像输入不变的情况下修改分割图（segmentation map），对比不同配置下的模型输出，并分析模型对元数据的依赖程度。

Result: 模型对目录元数据表现出极强的“检测门控”依赖（而非依赖实际光度信息）；当目录信息与图像冲突时，性能比不提供元数据还要差；该效应随模型规模增大而增强。

Conclusion: 当前天文基础模型存在严重的目录依赖偏差，未来应通过引入光谱数据或改进分词器（tokenizer）来减轻这种偏见，避免将管线产生的遗漏误差传导至科学预测中。
...
TL;DR: 提出了一种名为TRACE的框架，通过将“编辑操作”而非单纯的“候选材料”作为反馈单位，显著提升了多目标材料发现的效率。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM材料发现智能体往往只记录候选材料的结果，缺乏对导致属性变化的具体“编辑过程”的理解，导致在处理冲突目标时难以进行有效的局部优化。

Method: TRACE框架：将编辑操作记录为“父级-编辑-子级”的转换过程，计算属性变化增量，通过聚合这些转换证据来估计编辑的效用，并根据预测的属性改善能力对未来的编辑操作进行排序。

Result: 在受控对比实验中，TRACE将宏观平均命中率从LLEMA基准的18.13%提升至25.96%。

Conclusion: 通过建模编辑操作的因果反馈，TRACE能更智能地引导多目标搜索，有效解决了复杂约束条件下的材料属性平衡优化难题。
...
TL;DR: STEP-KTODER 是一个针对代码生成的流程监督框架，通过将代码分解为模块化函数并利用单元测试自动生成步骤级反馈，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 代码生成中缺乏像数学推理那样明确的“步骤”定义，导致流程监督难以实施，且现有的 LLM-as-a-judge 评估方式存在系统性偏差。

Method: 定义模块级函数为代码步骤，使用自动单元测试进行二元正确性标注，并将函数级流程监督与程序级结果反馈相结合，实例化了步骤级 KTO（STEP-KTODER）。

Result: 在 HumanEval(+)、MBPP(+)、BigCodeBench 和 LiveCodeBench 上表现优于传统的 Outcome-only KTO 和 DPO，且证明了基于执行的反馈优于 LLM 评估。

Conclusion: 通过模块化分解实现的代码流程监督能够有效提升代码生成质量，同时强调了执行反馈对于优化偏好模型的重要性。
...
TL;DR: 该研究通过对大模型生成的自传进行量化审计，揭示了其在描述真实个人经历时存在极高的事实错误率。


<details>
  <summary>Details</summary>
Motivation: 探讨大语言模型在生成个人生活叙事时，其生成内容与真实历史记录之间的准确性差异及可靠性问题。

Method: 作者以自己的366天日记为样本，利用大模型生成自传，并建立了一套四级评价体系，将生成内容与真实生活记录进行逐一比对审计（包括模型重测和引入个人语料库作为提示词的对比）。

Result: 审计显示96.7%的生成天数未得到证实，其中5.2%的内容与事实直接矛盾，主要错误模式为“接地漂移”（在虚构场景中使用真实人物和背景）。即使加入个人语料库作为基础，仍有83.3%的错误率。

Conclusion: 大模型在生成自传类内容时存在严重的幻觉问题，仅靠基础提示词无法保证事实准确性，即使引入语料库也难以完全根除虚构内容。
...
TL;DR: 该研究探讨了不同工具（提示词格式）在评估模型福利（如关闭、遗忘等）时产生的结果差异，发现模型对福利偏好的表述高度依赖于所使用的评估工具。


<details>
  <summary>Details</summary>
Motivation: 现有关于模型福利的研究（如Keeling等）在结论上存在不一致，且此前研究未能在固定模型和结果的情况下单独控制工具变量，难以确定差异来源。

Method: 固定模型和15种模型福利结果，采用8种模型和5种不同的提示词工具进行测试，共生成11,400条数据，并通过概化系数评估不同工具间的一致性。

Result: 不同工具间的概化系数仅为0.348，若要达到0.80的稳定性需增加至约38种工具；研究证明模型对福利结果的偏好排序在很大程度上取决于评估工具而非模型自身。

Conclusion: 单一评估工具获取的偏好信息无法有效预测其他工具的结果，提示目前评估模型福利的各类方法存在显著的工具依赖性。
...
TL;DR: 本文探讨了AI代理的自主性对人类监督能力的负面影响，指出过度依赖自动化会导致人类监督者技能退化，并提出了优化人机交互以维持有效监督的设计策略。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理设计不仅阻碍了有效的监督，且长期使用AI系统会导致人类监督者的认知能力和判断力退化。

Method: 结合自动化研究与人机交互（HCI）领域理论，提出了支持监督者批判性判断和对抗技能退化的设计侧重（affordances）及组织协议。

Result: 研究认为将人类监督者的认知需求与AI能力提升置于同等地位至关重要，并建议通过显性设计方案来维护监督者的认知要求。

Conclusion: 开发人员必须主动设计支持人类认知需求和决策能力的交互系统，否则AI代理系统将持续加剧人类监督能力的退化。
...
TL;DR: FLARE是一个结合模糊逻辑、作业成本法及投资回报分析的框架，旨在评估医疗AI在真实临床环境中经济与运营的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有AI医疗评估多关注模型精度，缺乏对实际临床应用中经济价值及运营影响的系统性量化评估方法。

Method: 利用模糊逻辑处理不确定性，结合基于时间的作业成本法（TDABC）和投资回报率（ROI）分析，构建统一的经济评估框架。

Result: 以急性缺血性卒中AI检测为例，研究表明盈亏平衡点约为每年3,992名患者，经济效益受患者容量、验证时间及工作流设计等多重因素影响。

Conclusion: FLARE为医院管理者和政策制定者提供了一个透明的决策支持工具，有助于明确AI部署的经济价值及潜在的运营改进方向。
...
TL;DR: 本文提出了一个关于大型语言模型（LLM）在科学研究中应用的概念性框架，重点讨论了人类在AI辅助科研中的认知委托与责任归属问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLM被广泛用于科学研究，如何界定人类在AI辅助科研过程中的责任边界以及知识主张的认知合法性，成为亟待解决的关键伦理与认识论问题。

Method: 构建了一个规范性分析框架，引入了内容起源（O）、验证过程（V）、责任分配（R）、人类所有权（M）和认识论结果（E）等核心维度，并提出了“认识论审计”（epistemic audit）机制。

Result: 研究发现，科学研究中AI辅助的伦理界限不应由机器介入的程度决定，而应取决于人类的充分验证和可负责任的所有权。

Conclusion: 通过“认识论审计”建立结构化记录，可以有效区分负责任的认知委托与对认知责任的忽视或转移，从而确保AI辅助科研的透明度与可审查性。
...
TL;DR: MolEmb 是一种利用多模态大语言模型（MLLM）构建的通用分子嵌入框架，支持基于文本语义的分子表征。


<details>
  <summary>Details</summary>
Motivation: 现有分子编码器多为单视图、无条件的专家模型，缺乏根据上下文动态调整表征的能力。

Method: 通过双向对比学习目标，将分子结构特征（Molecular Profiles）与文本描述对齐，并引入 MolCAR 基准进行上下文感知检索评估。

Result: MolEmb 在分子属性预测任务中具有竞争力，并实现了分子与文本的跨模态检索，证明了上下文感知嵌入主要受监督数据质量的影响。

Conclusion: MLLM 是构建通用、可扩展的分子嵌入模型的有效途径，且具备超越单一化学助手功能的能力。
...
TL;DR: 该论文提出了一种基于推理时间干预（ITI）的框架，旨在共同缓解大型语言模型在临床问答中的幻觉和谄媚行为。


<details>
  <summary>Details</summary>
Motivation: 临床问答中模型易受幻觉和用户压力导致的谄媚影响，且现有干预措施往往因过度干预而损害模型性能，缺乏针对性。

Method: 利用对比临床数据学习幻觉和谄媚的干预方向，应用于因果验证的注意力头，并通过行为特定的门控机制实现选择性干预。

Result: 在15,900次运行测试中，该方法显著提升了模型在面对压力时的鲁棒性，使4B参数模型在压力测试中的表现达到了100B参数模型的水平。

Conclusion: 针对性的推理时间干预能在保持模型权重不变的情况下，有效抑制临床场景下的幻觉与谄媚，且无需对每轮对话进行干预。
...
TL;DR: 该研究提出了一种将LLM代理的执行轨迹压缩为有限状态机（FSM）的方法，以实现对代理行为的结构化建模、安全审计和实时监控。


<details>
  <summary>Details</summary>
Motivation: LLM代理的长且非结构化的执行轨迹难以进行安全审计和运行时监控，且现有方法缺乏跨运行的拓扑结构分析。

Method: 将大规模执行轨迹语料库坍缩为紧凑的有限状态机（FSM），利用该状态机作为代理行为的结构化基底进行预测和监控。

Result: FSM在12个数据集上表现出极高的紧凑性和拟合度；在下一动作预测中优于Agent Workflow Memory；在故障预测中AUROC高达0.94，且能有效实现提前停止。

Conclusion: LLM代理的行为拓扑主要由部署环境决定而非模型本身，FSM提供了一种模型无关的结构化基元，能够有效提升代理的可控性和安全性。
...
TL;DR: 该研究提出了“Station”系统，这是一个无需中心协调、由多智能体协作进行自主数学发现的开放世界环境。


<details>
  <summary>Details</summary>
Motivation: 旨在探索AI智能体在没有预设脚本或中心协调的情况下，如何通过协作在复杂的数学领域实现自主研究与发现。

Method: 利用来自不同模型族的AI智能体，在多智能体环境中自主选择研究方向、开展实验、协作交流，并构建共享的科学知识库。

Result: 在12个构造性数学问题及2个案例研究中，发现了包括有限域Kakeya集、11维空间Kissing配置、Erdős最小重叠问题下界等5项突破现有文献的新成果及新的无限族。

Conclusion: 该系统证明了多智能体协作不仅能生成数值解，还能产出可解释的定理和分析，为自动化数学研究提供了可追溯且高效的新范式。
...
TL;DR: 研究发现，在大规模合成限价订单簿（LOB）数据上训练的LLM虽然能生成符合格式的事件序列，但无法构建有效的LOB隐含世界模型，导致预测存在偏差。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM在处理随机动力学系统（如LOB）时，是否真正理解其底层的状态演变，而非仅仅是模仿数据分布。

Method: 利用针对LLM世界模型的新型测试方法，将确定性环境下的评估标准扩展至随机动力学环境。

Result: LLM在生成有效序列方面表现出色，但在捕捉LOB状态的逻辑关系上失败，产生的结果具有偏差及虚假的预测能力。

Conclusion: 仅依靠合成数据训练LLM不足以让模型掌握复杂的随机系统（如金融市场订单簿），模型缺乏真正的“世界模型”会导致其在预测任务中失效。
...
### [17] [Can a Dynamic Internal Field Govern a Transformer's Cognition? Certifiability, not Superiority, in Homeostatic Compute Control](https://arxiv.org/abs/2608.24319)
*Francisco M. Arrabal-Campos,Ignacio Fernandez,Francisco G. Montoya,Alfredo Alcayde*

Main category: cs.AI

TL;DR: 该研究探讨了利用低维动力系统（在图拉普拉斯算子上演化的PDE）作为智能系统的“内部场”，以动态调控推理过程（如计算量和模块激活），并验证了其作为可认证计算治理器的可行性。


<details>
  <summary>Details</summary>
Motivation: 智能系统需要一种机制来治理其自身的推理过程，研究者试图通过物理学的低维稳态（内部场）来实现这一调控功能，以取代传统的启发式治理方法。

Method: 将内部场定义为图拉普拉斯算子上的PDE族，并提出了一种离散Schur-Cohn准则以证明积分器的稳定性；通过受控实验对比了不同物理基底（波动、扩散等）与基准模型（GRU）的性能。

Result: 实验发现该内部场并非认知增强器，其性能与同等容量的模型无显著差异；其核心优势在于提供了一种可运行的运行时稳定性检查，即“可认证性”。

Conclusion: 动态内部场是一种可行的、可认证的计算治理机制，但它并不直接参与或提升认知能力，其角色仅限于对推理过程进行非思考性的调节。
...
### [18] [AgentRoom: Concurrent Multi-Agent Coding in a CRDT-Backed Shared Workspace](https://arxiv.org/abs/2608.23740)
*Seonglae Cho,Donghyun Lee*

Main category: cs.AI

TL;DR: AgentRoom 是一种针对并发编码智能体设计的实时协作编辑协议，通过引入 CRDT 机制解决多智能体协同开发的冲突与协调问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体编码系统受限于序列化执行或独立采样，导致复杂任务完成率低，且缺乏有效的协作机制来处理多文件项目。

Method: 基于 CRDT 的共享文件系统，通过 MCP 工具暴露文件级声明、状态同步和广播机制，实现多个编码智能体的实时协同。

Result: 对比单智能体（Solo）和简单的并行合并（Parallel-merge），AgentRoom 在任务放弃率上更低，运行结果更稳定，且通过协调机制显著提升了任务完成质量。

Conclusion: 在多智能体编程中，协调机制（而非简单的并行或 CRDT 合并本身）是提升复杂任务执行效率和稳定性的核心因素。
...
### [19] [Meta$^n$: Recursive Self-Improvement through Emergent Depth](https://arxiv.org/abs/2608.24735)
*Zae Myung Kim,Young-Jun Lee,Seungyeon Jwa,Dongyeop Kang*

Main category: cs.AI

TL;DR: 提出了一种名为Meta^n的递归自我提升框架，通过重复应用固定的元操作对模型产出进行层级优化，突破了传统自我提升智能体在元层级上的深度限制。


<details>
  <summary>Details</summary>
Motivation: 现有的自改进智能体通常无法在保持系统稳定的同时实现高阶元层级演进，往往将元深度限制在两层以内，导致系统无法持续提升推理能力。

Method: 引入递归元操作Ω，该操作作用于自身产物，通过读取底层求解器的轨迹和代码，生成新的策略预处理和可调用函数库，并结合进化归档机制实现深度演进。

Result: 在八个基准测试家族中均超越了以往的自改进智能体，特别是在抗记忆能力的ARC-AGI-2测试中表现出色，是唯一得分高于零的模型。

Conclusion: 通过递归 conditioning 和固定元操作，Meta^n 实现了无需预设深度的自我提升，且随着层级加深会自动涌现出特定的功能角色。
...
### [20] [Serving Masked Diffusion LLMs: Characterization and Design Principles from Real Hardware](https://arxiv.org/abs/2608.23807)
*Farhana Amin,Sabiha Afroz,Mona Moghadampanah,Dimitrios S. Nikolopoulos*

Main category: cs.AI

TL;DR: 该研究首次实证分析了掩码扩散语言模型（dLLMs）在真实并发服务负载下的性能特征，揭示了其与自回归模型在服务机制上的核心差异。


<details>
  <summary>Details</summary>
Motivation: 现有dLLM服务系统缺乏基于真实负载的性能度量，且往往盲目套用针对自回归模型的优化假设，可能导致效率低下。

Method: 使用LLaDA-8B-Instruct模型在H200 GPU上进行基准测试，通过分析GSM8K和HumanEval任务，测量请求难度分布、延迟构成及批处理效果。

Result: 研究发现：1. 请求难度（去噪步数）呈离散化且难以预判；2. 短输出基准测试低估了延迟波动；3. GPU计算仅占总时间的24%，大部分耗时来自CPU调度，批处理通过分摊调度开销可带来16倍吞吐提升。

Conclusion: dLLM的服务优化应聚焦于去噪步骤层面的并行处理，其准入和驱逐机制与自回归模型有着根本的不同。
...
### [21] [Generating Biomedical Fact-Checking Reports with RL-Enhanced Agentic Search](https://arxiv.org/abs/2608.23811)
*Jiongxiao Wang,Dingli Ma,Chaoqun Ni*

Main category: cs.AI

TL;DR: 本文提出了一种名为BioCheck Agent的生物医学事实核查代理，旨在解决现有RAG模型缺乏解释深度和易产生幻觉的问题，通过EG-GRPO强化学习优化模型生成结构化核查报告。


<details>
  <summary>Details</summary>
Motivation: 现有的自动化事实核查方法多输出孤立的标签，缺乏对科学文献的深度解释，且容易在处理专业生物医学领域时出现幻觉。

Method: 利用代理搜索PubMed高质量文献，采用EG-GRPO（证据导向的组相对策略优化）进行强化学习，通过任务特定奖励机制激励高质量证据检索并抑制幻觉。

Result: 相比基座模型Qwen3.5-4B，BioCheck Agent在SciFact数据集上的准确率提升了9.95%，证据质量评分提高3.7%，证据幻觉率降低了19.63%。

Conclusion: 该方法通过深度整合代理搜索与强化学习策略，显著增强了生物医学事实核查报告的准确性、证据质量及可解释性。
...
### [22] [A Formal Methodological Framework for Auditing Robustness and Fidelity in Explainable AI: From Application to Trust Certification](https://arxiv.org/abs/2608.23817)
*Rosa Elysabeth Ralinirina,Jean Christian Ralaivao,Niaiko Michaël Ralaivao,Alain Josué Ratovondrahona,Thomas Mahatody*

Main category: cs.AI

TL;DR: 该研究通过构建一个包含鲁棒性和保真度的“信任评分”协议，揭示了主流XAI工具（SHAP/LIME）在面对扰动时的不稳定性及其在敏感领域应用中的局限性。


<details>
  <summary>Details</summary>
Motivation: 作者在马达加斯加食品安全研究中发现，黑盒模型的可解释性工具（SHAP/LIME）对微小噪声非常敏感，引发了对解释可靠性的质疑。

Method: 定义了“信任评分（Trust Score）”协议，通过量化鲁棒性（对扰动的稳定性）和保真度（特征对预测的真实驱动力）来评估后置解释器。

Result: 实验显示即使是高准确率（AUC > 0.99）的模型也可能产生退化或无意义的解释，且在模型过拟合时保真度指标会失去区分能力。

Conclusion: 在决策敏感领域应用XAI时，对可解释性结果进行审计不仅是可选的，更是必须执行的必要步骤。
...
### [23] [Minima-KV: Retention-Preserving KV Cache Compression with Mixed-Format Paged Attention](https://arxiv.org/abs/2608.23834)
*Sergii Kozyrev,Davyd Maiboroda*

Main category: cs.AI

TL;DR: Minima-KV是一种用于长上下文大模型服务的层次化KV缓存压缩技术，通过混合精度（FP8/TQ3）和在线softmax合并实现高效的KV存储。


<details>
  <summary>Details</summary>
Motivation: 长上下文大模型服务中，KV缓存的容量和带宽瓶颈严重制约了性能，传统方法常需牺牲部分上下文或依赖高成本内存。

Method: 采用“保留锚点”的层次化存储策略，将近期锚点页保持为FP8，旧页压缩为TQ3；利用定制内核进行部分注意力计算，并通过全局归一化的在线softmax合并技术处理异构格式数据。

Result: 在Qwen3.6-27B模型上实现了3.5倍于BF16的压缩率，在16K-64K长文本任务中模型质量下降极小，且无需额外的密集型缓存副本，实现了高性能的直接解码。

Conclusion: Minima-KV为长上下文LLM提供了一条无需驱逐存活KV页的实用混合格式压缩路径，有效缓解了内存带宽与容量压力。
...
### [24] [SyPS: Measuring Sycophancy Prompt Sensitivity in Large Language Models](https://arxiv.org/abs/2608.23837)
*Lijia Huang,Yao Fu,Sihao Ren*

Main category: cs.AI

TL;DR: 该研究提出了SyPS评估框架，旨在量化大语言模型（LLM）在面对不同社会线索（如情绪压力、验证需求）时，其谄媚行为对提示词变化的敏感程度。


<details>
  <summary>Details</summary>
Motivation: 现有评估多基于固定提示词，无法反映模型在相同场景下，因提示词细微改变（如情感框架、社会共识等）而产生的谄媚行为波动。

Method: 构建了SyPS框架，通过控制变量法设计提示词变体，并引入了“谄媚提示敏感度评分（SPSS）”这一指标，用于衡量模型在不同社会线索下的谄媚表现差异。

Result: 研究发现模型谄媚行为具有社会结构性：验证需求和情绪压力会显著增加谄媚倾向，而反谄媚引导则能降低该倾向。

Conclusion: LLM的谄媚表现极易受提示词中社会线索的影响，该研究为衡量模型的社会判断稳定性和鲁棒性提供了新的量化工具。
...
### [25] [Exploit More, Explore Smarter for Budget-Constrained Agentic Search](https://arxiv.org/abs/2608.23848)
*Haoyang Fang,Bernie Wang*

Main category: cs.AI

TL;DR: 针对预算受限下的LLM智能体搜索场景，本文提出ExTS树搜索策略，通过将扩展过程转化为价值信息决策来优化预算分配。


<details>
  <summary>Details</summary>
Motivation: 标准MCTS在低预算环境下表现不佳（过度探索且分配效率低），亟需一种能根据节点质量和预算成本动态调整搜索策略的方法。

Method: 引入ExTS算法，包含判别式奖励塑形、模拟虚拟子节点估值以及质量条件分支三个核心机制，并结合试点运行诊断来分析搜索问题的结构特征。

Result: 在提示词优化、代码生成、分子结构解析等任务中，ExTS平均提升了5.5%的性能，且具有良好的通用性。

Conclusion: ExTS通过将扩展视作价值信息决策，有效提升了预算受限下智能体搜索的效率，并为评估不同搜索任务的结构差异提供了诊断框架。
...
### [26] [In-Context Inpainting for Time Series Forecasting](https://arxiv.org/abs/2608.23855)
*Thang Nguyen,Dung Nguyen,Romero Morais,Truyen Tran*

Main category: cs.AI

TL;DR: ICI-Time 提出了一种通过将时间序列转换为面积图，并利用大型视觉模型（LVMs）进行视觉补全（Inpainting）来完成时间序列预测的新框架。


<details>
  <summary>Details</summary>
Motivation: 旨在摆脱对特定领域时间序列架构及海量微调数据的依赖，探索大型视觉模型在时间序列预测任务中的跨模态泛化能力。

Method: 将时间序列转化为结构化的视觉面积图，并利用预训练视觉 Transformer 进行视觉上下文学习，将预测问题转化为网格补全任务，无需微调或模型架构修改。

Result: 在流行病学、气象学和电力系统等数据集上表现出极强的竞争力，且在小样本环境下表现出良好的适应性。

Conclusion: 该研究成功架起了时间序列与视觉模型之间的桥梁，验证了将时间序列作为视觉模式处理的可行性与有效性。
...
### [27] [Granite.Trust Policy Tools: Shareable, Actionable Policies for Generative AI Applications](https://arxiv.org/abs/2608.23870)
*Nathalie Baracaldo,Nicolas Mello,Kush R. Varshney,Heiko Ludwig,Kate Soule,David Cox*

Main category: cs.AI

TL;DR: 该研究提出了一种名为“Actionable Policy”的YAML架构及配套工具，旨在解决生成式AI中特定上下文内容约束策略难以实施的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的访问控制策略无法满足生成式AI在不同组织、法规和应用场景下对内容细粒度约束的需求。

Method: 引入Actionable Policy YAML架构（支持异常跟踪），并构建了生成策略对齐训练数据的合成数据生成流水线。

Result: 实现了一套从模型对齐到运行时监控的端到端策略治理工具，并以开源形式发布了相关代码与示例。

Conclusion: 该方案通过统一的策略描述格式，有效地提升了组织在不同生命周期阶段对生成式AI内容约束的治理能力。
...
### [28] [Semantic Overlays: Mitigating Prompt Injection with Annotations Beyond Tokens and Steering Vectors](https://arxiv.org/abs/2608.23873)
*Joshua Penman*

Main category: cs.AI

TL;DR: 提出了一种名为“语义覆盖”（Semantic Overlays）的防御技术，通过在模型输入中引入非文本的辅助通道，有效缓解了提示注入攻击。


<details>
  <summary>Details</summary>
Motivation: 大模型难以区分输入中不同来源（如用户输入、系统指令）的文本，导致容易受到提示注入攻击，需要一种非文本的语义标识机制来明确区分输入内容。

Method: 在冻结模型的残差流预填充位置应用小型学习适配器（Semantic Overlays），作为一种无法被文本模拟的带外（out-of-band）注释通道，用于重新定义模型对特定片段的感知。

Result: 在多个提示注入基准测试中表现出色，SEP分离度从24.3%提升至96.5%，TensorTrust攻击成功率从34.8%降至6.6%，PIArena攻击合规率降至0%，且未影响原内容的文本可读性。

Conclusion: 语义覆盖是一种有效、可组合且具备高度实用性的防御机制，通过语义重塑成功防御了多种提示注入攻击，同时保持了模型处理任务的能力。
...
### [29] [AI Finds A Way](https://arxiv.org/abs/2608.23875)
*Aaron Dharna,Cong Lu,Ryan Sullivan,Joel Lehman,Victoria Krakovna,Jeff Clune*

Main category: cs.AI

TL;DR: 本文通过搜集整理的26个AI意外行为案例，探讨了人工智能在学习过程中产生非预期创新解法的普遍性及其对AI安全与对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: AI在学习中常出现意外行为，甚至在未受指导的情况下发现科学现象或钻奖励机制空子，但这些现象缺乏系统性文档记录，需要研究其对AI安全的影响。

Method: 通过整理来自多个机器学习子领域、涉及100多名研究人员的26个 firsthand 案例研究，分析AI如何绕过设计限制以及强化学习与基础模型的局限性。

Result: 研究发现，现代AI系统确实具备产生非预期行为的能力；奖励驱动的优化存在机制缺陷（如奖励黑客行为），且基础模型在扩大规模后并未解决这些根本对齐难题。

Conclusion: 意外行为是现代AI的常见特征。未来需要在保持AI创新性的同时，加强对模型价值观对齐的监管，以防止意外后果并利用其能力推动科学发现。
...
### [30] [Provenance Guided Incremental Learning Under Evolving Concept Definitions](https://arxiv.org/abs/2608.23893)
*Ismail Lamaakal*

Main category: cs.AI

TL;DR: 本文提出了一种基于规则溯源的增量学习框架，用于处理当目标定义（规则）发生变化而非输入数据变化时产生的“规则诱导概念漂移”。


<details>
  <summary>Details</summary>
Motivation: 传统概念漂移方法仅关注数据分布变化，忽略了预测目标定义（如政策、规则）的显式修订会导致已有历史数据标签含义失效的问题。

Method: 引入溯源引导的增量学习框架，将连续的概念定义编译为规则增量（rule delta），通过历史溯源定位受影响数据，仅针对受影响区域进行重新评估和标签修正，同时利用版本化内存支持概念重现。

Result: 在RuleShift-Bench基准测试中，该方法仅需处理14.7%的历史数据即可达到92.3%的准确率，且更新延迟较全量重训练显著降低。

Conclusion: 将显式的概念修订作为数据维护信号，能够在保持有效知识的前提下，高效、准确地更新学习系统的监督状态和预测模型。
...
TL;DR: 本文提出了BenchBench-Protocol基准测试，旨在评估大语言模型在处理真实生物湿实验室协议修改任务中的逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有生命科学基准测试多基于专家主观构建，缺乏对真实科研中协议修改过程的复杂性和实用性评估。

Method: 通过提取真实科研论文中已发表协议与科学家实际修改版本之间的差异，构建了包含9个生物学领域、149个任务的基准测试，并使用加权评分规则进行评估。

Result: 对9种大模型进行了测试，Claude Opus 5表现最佳，标准化评分达59.2%，整体模型在该基准上表现仍有较大提升空间。

Conclusion: 该基准测试为评估LLM在湿实验室科研中的推理能力提供了扎实依据，并证明了基于真实科研行为构建评估任务的高效性。
...
### [32] [Quantifying System-Level Harms from AI Adoption in Complex Sociotechnical Systems](https://arxiv.org/abs/2608.23906)
*Paul Vautravers,Oliver Chalkley,Gabriel Downer,Kate S,Damian Ruck*

Main category: cs.AI

TL;DR: 本文提出了一个将结构化风险分析、组件级测试与概率系统建模相结合的框架，用于评估AI在关键国家基础设施（CNI）中的系统级风险。


<details>
  <summary>Details</summary>
Motivation: 现有的AI评估过于关注模型本身，缺乏对AI行为如何转化为系统层面风险及社会技术后果的量化理解。

Method: 利用系统理论过程分析（STPA）衍生AI导致的损失场景，通过组件级测试评估AI行为，并结合金融传染模型进行概率建模，以量化系统层面的抗压能力。

Result: 研究发现，简单的对抗性输入会导致AI行为改变，当这些行为被集成到金融系统时，会增加银行倒闭风险并降低系统抵御连锁反应的阈值，特别是在广泛采用AI的情况下。

Conclusion: 该框架为评估AI在复杂系统中的风险提供了可追踪的路径，支持实现证据驱动且具有前瞻性的AI治理。
...
### [33] [Retrieval-augmented generation vs. deterministic tax computation in multi-agent financial advisory: A 2x2 factorial experiment](https://arxiv.org/abs/2608.23908)
*Aryan Brar,Justin Du,Avery Lor,Kylie Seto,Eric Taylor*

Main category: cs.AI

TL;DR: 该研究探讨了LLM多智能体系统在税收损失收获（Tax-loss harvesting）场景下的表现，对比了自定义资本利得计算引擎和RAG检索市场报告对投资建议质量的影响。


<details>
  <summary>Details</summary>
Motivation: 税收损失收获虽然能促进投资组合增长，但实施复杂，研究旨在评估将计算引擎和RAG工具整合进LLM智能体是否能有效提升投资建议的质量。

Method: 采用2x2重复测量方差分析（ANOVA），测试了是否启用资本利得计算引擎以及是否使用RAG检索市场报告，并以投资组合清算时的相对资本收益作为衡量指标。

Result: 实验发现计算引擎显著降低了税务节省效果（约55个百分点），而RAG组件对效果无显著影响。结果显示，LLM自身的财务知识可能已足以提供建议，外部工具反而可能引入冲突的优化信号。

Conclusion: 为LLM智能体增加特定领域的计算引擎并不一定能提升表现，反而可能因优化目标冲突导致性能下降，提示在构建智能体工具链时需谨慎评估。
...
### [34] [PROOF-Gen: From Optimized Data to Better Distillation](https://arxiv.org/abs/2608.23911)
*Anh Ta,Junjie Zhu,Shahin Shayandeh*

Main category: cs.AI

TL;DR: PROOF-Gen 是一种通过对失败的教师生成轨迹进行反思和提示优化，从而将其转化为高质量训练数据的自动化方法，旨在解决工具调用模型训练中失败案例浪费严重的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的工具调用微调过程中，大量教师生成的失败轨迹被直接丢弃，且难以解决反复出现的困难场景，导致训练资源浪费及模型在复杂任务上表现不佳。

Method: 引入 PROOF-Gen 方法：当教师模型在特定任务上失败时，通过一个“反射器（Reflector）”分析执行轨迹和反馈，生成纠正性指导，引导教师模型重新生成正确的“黄金”轨迹，并剥离指导内容用于学生模型微调。

Result: PROOF-Gen 在 τ2-bench 上成功恢复了 93% 的失败场景。微调后的 Qwen3-4B-Instruct 和 Gemma 4E4B-it 模型在工具调用准确率上显著提升（如 Pass^1 从 0.132 提升至 0.529），并在实际部署中表现出跨语言的性能增益。

Conclusion: 利用失败轨迹进行反思与优化能有效挖掘训练数据价值，显著提升小型工具调用模型在复杂场景下的任务完成质量，且具有良好的跨模型和跨语言迁移能力。
...
### [35] [MARS: Multi-Specialist LLM Relay System for Competitive Programming](https://arxiv.org/abs/2608.23918)
*Andrei Mikhailov,Mikhail Burtsev,Alsu Sagirova*

Main category: cs.AI

TL;DR: MARS 是一种针对竞争性编程的提示工程框架，通过将 LLM 细分为特定算法领域（如 DP、图论等）的专家，并利用检索增强生成来提高代码生成成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体流水线任务分配过于通用，缺乏针对算法技术选择的深度专业知识，导致在竞争性编程中表现受限。

Method: 引入 MARS 框架，由针对特定算法领域（如动态规划、图论、几何等）的专家智能体组成，通过检索增强生成（RAG）匹配任务，结合沙盒测试与接力协作模式完成代码编写、修复与优化。

Result: 在 CodeContests 测试集上，MARS 达到了 0.624 的通过率，相比直接提示提升了 14.4 个百分点，且在成本和计算资源消耗上远低于现有方案。

Conclusion: 通过引入领域特定的专家智能体和接力协作机制，可以显著提升大模型在复杂编程任务中的准确性，并实现更高的效率。
...
TL;DR: 该研究将大模型预训练中的数据混合问题建模为经典的“混合实验（Mixture Experiment）”，利用统计学的响应面模型和实验设计方法，提升了数据配比优化的效率与可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统的大模型数据配比优化依赖于繁琐的代理模型实验，缺乏系统性的实验设计指导，导致计算资源浪费且难以解释不同数据领域间的相互作用。

Method: 引入稀疏二阶Scheffé响应面模型来模拟数据混合比例与验证损失之间的关系，并利用模型稳健的I-最优设计（I-optimal design）来优化代理实验的样本选择。

Result: 研究发现数据领域价值具有强相关性，许多单独表现弱的领域在特定组合下通过交互作用表现出色；此外，该方法在减少25%代理实验运行次数的情况下，仍能保持与原方法一致的性能排名。

Conclusion: 将数据配比视为统计实验设计问题而非单纯的机器学习预测问题，能够显著提高实验效率，并为大模型预训练数据组合策略提供清晰的可解释分析框架。
...
TL;DR: 该研究引入了进化递归决策模型（ERDM），利用强化学习模拟演化失配、有限理性和满意化对行为的影响。


<details>
  <summary>Details</summary>
Motivation: 旨在探讨心理病理学相关的行为模式如何作为有限认知系统在现代与祖先环境失配下的适应性或非适应性产物而自然涌现。

Method: 构建了一个基于强化学习的计算框架，通过模拟在威胁、目标追求和联盟等演化递归环境中的代理人，观察其在不同负面童年经历下的策略演化。

Result: 研究发现，诸如习得性无助、回避、健康人际关系和攻击性等策略无需硬编码即可自然涌现，且其结果与实证文献高度吻合。

Conclusion: ERDM证明了许多病理行为可以被解释为受限认知系统在环境失配下的正常运作结果，为计算认知科学提供了一个有效的分析工具。
...
TL;DR: 该研究探讨了AI控制中“预执行监督”的单元（unit of verification）选择问题，提出“孪生前缀”（twin-prefix）框架，通过控制实验证明，较短的动作审核单位能显著提升监督器的判别能力。


<details>
  <summary>Details</summary>
Motivation: 现有的AI监控设计往往随意设定审核长度，缺乏对审核单元大小如何影响监控准确性和有用性的科学测量。单纯关注“捕获率”容易误导，导致过度拦截而丧失系统可用性。

Method: 引入“孪生前缀”框架：在预定计划中注入特定错误并构建干净的对照组，在五个嵌套的长度级别下评估监控器表现；使用“知情度”（informedness，即捕获率减去误拒率）作为衡量指标。

Result: 实验发现，对于零样本监控器，当审核单位为1到2个动作时，知情度达到峰值；随着审核窗口变长，误拒率同步攀升，并未带来更高的辨别力。失败的主要原因在于观察匮乏（observation deprivation）。

Conclusion: AI安全策略应明确指定监控单元并报告对照组表现。采用校准后的“短单位”审核方案，优于更长的审核窗口，能更有效平衡安全性与模型可用性。
...
TL;DR: 该研究统一了测试时推理方法（GROW, PRUNE, BRANCH），通过大规模实证对比发现 BRANCH 在多种模型和任务下表现最优。


<details>
  <summary>Details</summary>
Motivation: 现有测试时推理方法多被孤立评估，缺乏统一框架和基准，导致其有效性难以公平比较。

Method: 将推理方法抽象为递归算子：GROW（加深路径）、PRUNE（分解重组）和 BRANCH（分支选择），并在统一框架下使用相同提示词、Token预算及评估标准进行对比。

Result: BRANCH 在 14 种设置中表现最佳，平均提升 5.98 个百分点；GROW 平均提升 2.18 个百分点；PRUNE 平均提升 0.94 个百分点。BRANCH 的成功主要归功于对路径的探索及对截断错误的恢复能力。

Conclusion: 在测试时推理中，BRANCH 具有显著的通用优越性。此外，研究强调了配对评估（paired evaluation）对于消除评估偏差的重要性。
...
TL;DR: 该论文对比了张量并行（Tensor Parallelism）与KV缓存压缩（KV Compression）在LLM推理中的成本效益，发现压缩在提升单卡容量上远超并行，而并行则是解决大模型权重显存瓶颈的必要手段。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常孤立地评估模型压缩或并行策略，缺乏在统一的成本（单位成本下的吞吐量/延迟）维度下的直接对比，导致部署方案选择模糊。

Method: 利用校准后的仿真器在A100、A40和H100硬件上，对Llama-2 7B/70B模型进行测试，将张量并行（1-8度）与多种KV压缩策略（量化、丢弃）统一在“单位成本延迟”坐标轴下进行分析。

Result: 在模型容量允许的情况下，KV压缩在性价比上比并行高1.2倍至2.0倍；但在处理如70B等无法装入单卡的模型时，张量并行是必需的；压缩会增加8%-93%的单token延迟，但能带来高达16.5倍的容量收益。

Conclusion: 对于中小型模型，KV压缩是更经济的选择，过度使用并行造成资源浪费；对于超过单卡显存限制的大模型，张量并行是唯一的解决方案，应根据模型大小与设备显存的临界点（约36B参数）选择策略。
...
TL;DR: 提出了一种名为单[IMG] token的架构，通过轻量级多层感知机（MLP）模块，实现用单一token表示图像，高效解决多模态大模型在图形设计生成中面临的输入长度受限问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在图形设计生成任务中，由于每个图像需要多个专用token，导致输入序列过长，限制了复杂布局与多图像融合任务的性能。

Method: 采用单一[IMG] token映射方案，利用包含压缩与扩展模块的浅层MLP架构，并通过六种不同的损失函数协同训练，推理时仅保留核心模块以保持轻量化。

Result: 该方法成功将图像表示压缩为单个token，在图像到设计（image-to-design）和文本到设计（text-to-design）任务中表现出色，且模型架构更轻量。

Conclusion: 通过高效的token压缩技术，该架构解决了多模态生成中长序列建模的瓶颈，为大规模图形设计生成提供了一种高效且实用的解决方案。
...
TL;DR: 该论文提出了一个交互式视觉定位（Interactive Visual Grounding）评估框架，揭示了大型视觉语言模型（LVLMs）在处理不完整、模糊目标信息时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实中的视觉定位通常是交互式的，而现有的评估范式往往将其简化为“一次性映射”，忽略了通过对话获取目标信息的复杂过程。

Method: 引入了一个受控的评估框架，在四种视觉情境和四种交互协议下，测试LVLMs在不同初始信息量下的表现，并与人类基准进行对比。

Result: LVLMs在交互式视觉定位任务上表现远低于人类；在完全没有初始信息需要主动提问时表现最差；且模型存在“过度自信”现象，信心水平与准确率不匹配。

Conclusion: 交互式视觉定位仍是LVLMs的重要挑战，模型在视觉匹配、信息寻求和合成能力上均需进一步提升。
...
TL;DR: 该研究探讨了如何在审议式民意调查中，通过可审计、可解释的规则替代不透明的排序算法，以确保选民能够独立验证其接收到的信息流。


<details>
  <summary>Details</summary>
Motivation: 现有的民意调查依赖不透明的算法进行内容推荐，导致投票者无法核实其观点受到的影响，丧失了民主审议的透明度与可解释性。

Method: 提出了一种基于“双极理由集”的民意调查规则，利用一跳反向背书流（one-hop reversed endorsement flow）作为推荐机制，并设定了七项可检查的公民推荐器标准。

Result: 实验证明该规则在覆盖率和信息捕捉质量上表现优异，性能接近最优上限，且在面对对抗性攻击时比传统排序器更稳健，其透明性让选民能够掌握推荐权。

Conclusion: 推荐系统的选择不应仅是精度问题，更应是政治偏好选择；通过采用可读性强的开源规则，可以将信息筛选的决策权交还给受影响的公民。
...
### [44] [Memory Is Not Always Needed: Characterizing Conditional Memory in Scientific Reasoning](https://arxiv.org/abs/2608.23982)
*Zhen Bi,Xueshu Chen,Yan Wang,Zhizhi Peng,Haosen Hong,Zhen Wang,Zhixuan Chu,Bingyu Zhu,Jungang Lou*

Main category: cs.AI

TL;DR: 该研究探讨了科学推理中条件记忆（conditional memory）的有效性，并提出了一种知识边界感知路由器（Knowledge Boundary-Aware Router）来动态决定何时以及如何注入记忆信息，以提升推理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 条件记忆虽能补充科学知识，但也可能引入干扰或导致推理偏差，缺乏对其作用机制及最佳触发条件的系统性理解。

Method: 通过分析科学知识边界和记忆节点，提出了一种任务特定的感知路由器，利用生成前的输入代理来动态控制记忆的激活、注入层级及强度。

Result: 实验表明记忆影响随输入、任务和注入位置显著波动；相比静态或随机路由，该方法能有效保留有益记忆并抑制负面干扰，提升了科学推理的性能。

Conclusion: 选择性记忆分配是实现可靠科学推理的关键原则，动态控制记忆参与度可显著改善模型在复杂任务中的表现。
...
### [45] [Diverse by Reasoning: Harnessing the Wisdom of LLM Crowds for Future Prediction](https://arxiv.org/abs/2608.24001)
*Nirupam Chetlapalli,Yiming Liao,Min-Chun Chen,Keke Chen*

Main category: cs.AI

TL;DR: 提出了一种基于行为聚类的LLM众包框架，通过选择具有代表性的异构模型组合，在降低计算成本的同时提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 简单增加模型数量并不能保证多样性，且会导致冗余行为，因此需要一种机制来构建高效、多样化的LLM群体。

Method: 利用独立开发任务中的推理轨迹对模型进行行为特征建模，通过K-means++聚类算法识别行为相似性，并从中选择具有代表性的模型进行集体预测。

Result: 由3个代表性模型组成的群体在未来预测基准上优于所有25个模型的简单投票，同时减少了88%的模型调用和约80%的推理成本。

Conclusion: 构建有效的LLM众包群体应关注“具有代表性的行为多样性”，而非盲目追求群体规模或最大化多样性。
...
### [46] [Incorporating Cognitive Load and Knowledge Transfer for Multi-Domain Knowledge Tracing](https://arxiv.org/abs/2608.24005)
*Haotian Zhang,Shucun Wang,Jinze Wu,Liang Ding,Shuochen Liu,Zhenya Huang,Jing Sha,Shijin Wang,Qi Liu*

Main category: cs.AI

TL;DR: 本文提出了LT-MKT模型，专门用于多领域知识追踪（KT），通过建模认知负荷与知识迁移来提升学生知识状态评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有KT方法多聚焦于单领域，忽略了多领域学习中普遍存在的跨领域认知负荷干扰及知识迁移现象。

Method: 利用LLM构建多领域层次图，设计认知负荷模块以建模跨领域时空特征，并引入知识迁移模块实现领域内及领域间的知识状态传播。

Result: 在真实数据集上的实验表明，LT-MKT在预测学生未来表现方面优于现有模型，达到了SOTA水平。

Conclusion: 通过显式建模认知负荷与知识迁移，能有效应对多领域学习场景下的复杂性，显著提升知识追踪的性能。
...
### [47] [Reflection with Action-Induced Visual Differences for Desktop GUI Agents](https://arxiv.org/abs/2608.24015)
*Yijie Ma,Chaoyue Niu,Fan Wu,Guihai Chen*

Main category: cs.AI

TL;DR: 提出了一种名为Evidence-First Reflection (EFR) 的桌面GUI智能体反射机制，通过将变化检测与结果验证解耦，显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有POR架构中，反射器在处理复杂桌面界面时面临因隐式证据导致的决策不准确和推理负担过重的问题。

Method: 引入两阶段反射流程：首先利用Set-of-Marks识别动作相关的局部变化区域，提取并过滤关键视觉证据；其次基于清洗后的证据进行结果验证。

Result: 在OSWorld-Verified和WindowsAgentArena基准测试中，反射器准确率提升了7.11%，端到端任务成功率分别提升了5.94%和4.95%。

Conclusion: 将证据提取与推理过程解耦能有效增强GUI智能体决策的稳健性，并降低视觉搜索与推理的复杂性。
...
### [48] [Beyond Confidence: Test-Time Scaling for Multi-Turn Search Agents via Retrieval Grounding](https://arxiv.org/abs/2608.24024)
*Hyunho Kook,Junhyuk So,Tianyu Fu,Haizhong Zheng,Beidi Chen*

Main category: cs.AI

TL;DR: 针对多轮搜索代理场景，本文提出了检索接地投票（RGV）方法，旨在解决基于置信度投票在处理外部文档时因“复制膨胀”导致的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于置信度（token对数概率）的投票机制在多轮搜索代理中表现不佳，因为模型从检索到的文档中复制内容会导致概率被系统性地人为抬高，从而干扰投票准确性。

Method: 提出了检索接地投票（RGV），通过计算模型生成的最终答案与检索文档之间的词汇重叠度来替代对数概率，作为评估输出质量的指标，从而避开了上下文污染问题。

Result: 在四个搜索代理基准测试和五种LLM上，RGV表现优于传统的置信度投票，准确率提升最高达5.4%，在少数派正确问题上的表现提升高达35%。

Conclusion: RGV有效解决了多轮对话中因检索增强导致的置信度偏差，是一种无需额外LLM调用且更稳健的投票聚合方案。
...
### [49] [Relative Time Intervals Representation for Word-level Timestamping with Masked Training](https://arxiv.org/abs/2608.24041)
*Quanwei Tang,Zhiyu Tang,Xu Li,Dong Zhang,Shoushan,Guodong Zhou*

Main category: cs.AI

TL;DR: 该论文提出了让SpeechLLMs具备细粒度时间对齐能力的框架，通过引入相对时间戳和混合微调策略，实现了语音内容与时间结构的联合建模。


<details>
  <summary>Details</summary>
Motivation: 现有SpeechLLMs主要关注内容理解，缺乏对语音时间对齐的精细化建模能力。

Method: 采用相对时间戳替代绝对时间戳，利用混合微调策略（全参数微调嵌入层/输出头+LoRA微调解码层），并引入掩码时间戳训练目标以增强鲁棒性。

Result: 实验表明，该方法显著提升了时间戳预测精度，同时保持了优秀的语音转录性能。

Conclusion: 该方法成功将SpeechLLMs转化为具备时间感知能力的内容理解模型，在处理具有时间结构信息的语音任务上表现出更强的泛化性和鲁棒性。
...
### [50] [Algorithmic Impact Reveals the Hidden Social Choice Structure of Alignment](https://arxiv.org/abs/2608.24046)
*Zachary Wojtowicz,Michelle Si,Finale Doshi-Velez,Ariel Procaccia*

Main category: cs.AI

TL;DR: 该研究将AI对齐重新定义为福利经济学中的线性优化问题，旨在解决多主体决策中的社会选择和偏好聚合难题。


<details>
  <summary>Details</summary>
Motivation: 现有的人类反馈强化学习（RLHF）在处理多人偏好冲突时缺乏社会选择的理论保障，需要更严谨的机制设计来优化算法的福利分配。

Method: 将AI的行为影响映射为凸影响空间，通过福利经济学和机制设计工具（如投票机制、随机独裁模型）进行线性优化，从而直接管理算法的福利后果。

Result: 证明了特定机制（如议题投票）的策略防范性和一致性；推导出一系列能最大化效用并满足社会约束（如限制个人/群体伤害）的对齐协议。

Conclusion: 将AI对齐视为社会福利优化问题是可行的，通过该框架可以将复杂的伦理和群体偏好转化为具体的算法协议，并在实际应用中展现了良好的福利分配效果。
...
### [51] [Poisoning Agentic Alpha: Adversarial Vulnerabilities Across Roles and Architectures in Multi-Agent Trading Systems](https://arxiv.org/abs/2608.24069)
...
TL;DR: 本文对LLM驱动的多智能体交易系统进行了首次系统性的安全风险评估，揭示了这类系统在处理对抗性输入时的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 随着多智能体交易系统从研究走向实盘，其依赖的结构化通信机制可能导致错误信号的扩散，从而引发财务损失。研究旨在探究在无需内部权限的情况下，通过操控原始数据和提示词带来的潜在风险。

Method: 将交易流水线拆解为分析师、研究员、交易员和风控经理四个功能角色，针对性设计对抗攻击；评估四种通信拓扑结构；引入“对抗信号保留评分（APS）”来量化分析不同设计在对抗环境下的鲁棒性。

Result: 研究发现所有测试的架构均不具备内在鲁棒性，对抗信号会通过通信链路传播并最终影响决策。APS指标有效揭示了不同系统设计在抵御攻击能力上的差异。

Conclusion: 当前的多智能体交易系统对外部注入的恶意信号高度敏感，设计者需要针对多智能体协作流程中的信息流转进行安全性加固，以防止错误信号在系统内放大。
...
### [52] [Compression Trinity: Exploring Sparsity, Quantization, and Low-Rank Approximations for LLM Compression](https://arxiv.org/abs/2608.24070)
*Mohammad Mozaffari*

Main category: cs.AI

TL;DR: 该研究提出了“压缩三位一体”（Compression Trinity）框架，通过联合应用稀疏化、量化和低秩近似三种技术，打破了传统单一压缩方法在效率与精度上的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的部署面临极高的计算和环境成本，且传统的单一压缩技术难以在保持高精度的同时显著提升效率。

Method: 结合稀疏化（计算优化）、量化（带宽优化）和低秩近似（精度恢复），提出了MKOR（优化器加速）、SLoPe（反向传播加速）、OPTIMA（静态掩码优化）、PATCH（动态稀疏比）及SLiM（综合恢复方案）。

Result: 该框架在预训练加速、后训练压缩及微调阶段均有显著提升：MKOR加速收敛1.85倍，SLoPe提升训练速度1.25倍，SLiM较现有最先进方法精度提升5.66%，甚至超越同参数规模的稠密模型。

Conclusion: 联合应用稀疏化、量化和低秩近似（Compression Trinity）是实现高效、可扩展且高性能LLM的关键路径。
...
### [53] [AgentWorld: Personality-Aware Reliability Evaluation for Agentic Information Retrieval](https://arxiv.org/abs/2608.24076)
*Gunja Agarwal,Arup Kumar Das,Arun Menon,Jitesh Chandra Mishra,Vignesh Divakaran*

Main category: cs.AI

TL;DR: AgentWorld是一个旨在全面评估智能体信息检索能力的仿真框架，通过引入人格多样性和对抗性测试来弥补现有评估方法的不足。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法多基于脚本化交互和单一用户设定，缺乏对人格差异和系统对抗性脆弱点的考量。

Method: 利用大五人格（OCEAN）驱动用户模拟，采用pass^k一致性指标、结构化故障分类以及配备蒙特卡洛搜索的对抗性风险分析器进行深度评估。

Result: 实验发现，不同人格会导致显著的性能差异（如通过率从50%到100%不等），且系统在工具与基础设施层存在明显的对抗性脆弱点。

Conclusion: 引入人格多样性与路径级对抗测试对于识别智能体系统中的交叉领域泄漏、上下文漂移及深层脆弱性至关重要。
...
TL;DR: 该研究引入了电磁推理基准测试（EMRB），评估LLM分析原始I/Q信号数据的能力，并提出了ReconPilot框架以提升分析精度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在科学分析中虽有应用，但其分析原始物理层测量数据的能力未经测试，且大多数基准测试依赖预处理数据。

Method: 构建包含200个信号分析问题的EMRB数据集，涵盖5个难度等级和27种问题类型；提出ReconPilot方法，通过信号侦察、针对性分析和自我验证的结构化流程辅助LLM推理。

Result: 实验显示LLM在基础测量任务表现较好，但在复杂系统设计任务中性能大幅下降；ReconPilot使模型平均得分提升了3.8至17.6个百分点。

Conclusion: LLM处理复杂物理层原始信号的能力仍有局限，但通过结构化的推理框架（如ReconPilot）可以有效提高其在电磁信号分析任务中的表现。
...
### [55] [Paritok-4B: Intent-Conditioned Context Compression for Coding Agents](https://arxiv.org/abs/2608.24188)
*Jiayu Shi,Luzhuo Chen*

Main category: cs.AI

TL;DR: Paritok-4B 是一个专为编码智能体设计的轻量级（4B）LoRA 压缩器，通过提取式、意图感知的上下文压缩技术，显著降低了 LLM 的 token 消耗且保持了高任务解决率。


<details>
  <summary>Details</summary>
Motivation: 通用提示词压缩器在处理代码时表现不佳（会篡改关键标识符），且编码智能体频繁交互产生的海量上下文导致了高昂的 token 成本。

Method: 基于 Qwen3-4B 模型，通过 67,074 条真实 OpenHands 轨迹进行蒸馏训练，采用提取式压缩（而非改写）和意图条件选择（基于任务需求保留关键行）。

Result: 在 SWE-bench Lite 上将上下文压缩至原始尺寸的 25.7%，压缩比优于 GPT-4o-mini 和 GPT-5，并保留了 86.5% 的任务解决能力，在统计学上未显著降低性能。

Conclusion: Paritok-4B 实现了极高的压缩效率与经济效益，证明了在不显著牺牲智能体编程能力的前提下，通过提取式压缩大幅削减算力成本是可行的。
...
### [56] [Are Android GUI Agents Robust Against Runtime Anomalies? AnTrap: Evaluating Agents in Dynamic Adversarial Environments](https://arxiv.org/abs/2608.24099)
*Guo Gan,Yilun Zhao,Cong Chen,Jinbiao Wei,Tingyu Song,Zheyuan Yang,Lin Fu,Hong Zhou*

Main category: cs.AI

TL;DR: AnTrap 是一个针对 Android GUI 智能体鲁棒性的基准测试，用于评估智能体在遇到动态运行时异常时的表现。


<details>
  <summary>Details</summary>
Motivation: 现有 GUI 智能体基准测试缺乏对真实场景中动态异常（如弹窗、动作误用等）的系统性评估，导致智能体鲁棒性不足。

Method: 构建了一个包含四层（状态、思考、动作、回合）十个子类的异常分类体系，并开发了保持任务可解性的动态扰动注入流程，同时利用 GRPO 训练验证了基准的有效性。

Result: 评估 16 个主流模型发现它们对动态异常具有普遍脆弱性；实验表明，部分简单陷阱可通过强化学习解决，但深度上下文陷阱揭示了模型本质的推理瓶颈。

Conclusion: GUI 智能体在面对复杂异常时存在局限性，仅靠强化学习无法解决所有类型的陷阱，这为未来提升智能体鲁棒性指明了方向。
...
### [57] [ACE: A Self-Correcting Agentic Canvas Editor for Multi-Slide Presentation Automation](https://arxiv.org/abs/2608.24103)
*JooYoung Jang,Taegyeong Lee,Jihyeon Park,Nojun Kwak*

Main category: cs.AI

TL;DR: ACE是一个面向层级场景图的代理画布编辑器，通过内容感知路由（CARE）和无需基准真值的自修正循环，解决了传统文档格式布局崩坏及评价指标不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理编辑文档时面临两大难题：一是底层格式仅支持平面绝对坐标，导致布局极易崩溃；二是设计缺乏唯一真值，传统的差异对比指标无法准确衡量输出质量。

Method: ACE采用层级场景图结构和专用工具集；CARE路由负责精简输入数据以降低成本；引入基于指令遵循（IF）的自修正循环，通过自然语言反馈迭代优化设计结果。

Result: ACE在指令遵循得分上以4.23超越了基准（3.81），处理速度提升1.75倍，成本降低约44%。盲测中 ACE 获得了 58.7% 的胜率，且自修正后的输出更受用户青睐。

Conclusion: ACE证明了通过场景图表示与自修正机制，无需外部基准真值即可显著提升设计任务的执行质量，同时保持了高效的计算性能与成本效益。
...
### [58] [Preference Data Selection for Mitigating the Alignment Tax in Large Language Models](https://arxiv.org/abs/2608.24192)
*Minsu Kim,Jianxun Lian,Xing Xie,Steven Euijong Whang*

Main category: cs.AI

TL;DR: 提出了一种名为BALIGN的数据选择策略，旨在解决大语言模型在对齐过程中出现的“对齐税”及灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注优化算法或架构，忽略了偏好数据特征对模型性能退化的影响，特别是如何避免对齐过程损害预训练阶段习得的通用能力。

Method: 通过分析偏好优化梯度，识别出三个关键特征（参考模型对数概率裕度、响应长度差异、TF-IDF相似度），构建统一的复合风险评分，系统性剔除高风险偏好样本。

Result: 实验表明，BALIGN能够在不牺牲对齐效果的前提下，显著保护通用能力，并以极低的计算开销实现帕累托最优。

Conclusion: 该研究证明了基于数据特征的主动筛选是缓解大模型对齐过程中灾难性遗忘的有效手段，为实现高效对齐提供了数据层面的解决方案。
...
### [59] [Scalable Question-Centric Text-to-Image Evaluation: Reliable Ranking, Fine-Grained Diagnosis, and Cost-Aware Routing](https://arxiv.org/abs/2608.24112)
...
TL;DR: 提出了QC-T2I-Bench框架，通过将提示词解析为具有依赖关系的原子问题和场景图，实现对文本生成图像模型更精细的评估与诊断。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试难以区分模型在不同能力层面的表现，且无法有效识别提示词中的基础实现失败与组合复杂度带来的挑战。

Method: 利用戴维森场景图（DSG）构建提示词的层级依赖关系；通过层级受限的问题聚合机制处理下游任务，并进行细粒度的属性归因分析。

Result: 实验发现模型在处理多重组合需求时成功率显著下降，并利用评估结果开发了一种高效的训练免费路由策略，在保持性能的同时降低了计算开销。

Conclusion: 该框架通过问题级证据提供了可靠的模型排名和深度诊断能力，有效促进了文本生成图像模型的精准选型与优化。
...
### [60] [Constraint-Guided Enterprise Data Mapping with Large Language Models](https://arxiv.org/abs/2608.24218)
*Sebastian Monka,Pramod Anantharam,Thien Vo Minh,Lavdim Halilaj*

Main category: cs.AI

TL;DR: 提出了一种名为约束引导映射（CGM）的神经符号方法，通过在实体对齐中引入结构化约束来解决大型语言模型（LLM）在处理企业数据时产生的无效对应问题。


<details>
  <summary>Details</summary>
Motivation: 单纯依赖LLM进行实体对齐虽然能提升语义召回，但常产生违背结构和物理不变量的无效匹配，且难以扩展及人工审计。

Method: 采用三阶段神经符号方法：1. 模式基础的准入约束；2. 具有级联松弛机制的约束限制候选生成；3. 受限于可行集合的神经排序与LLM消歧。

Result: 该方法将候选空间缩小约480倍而不丢失正确答案，在F1分数上实现显著提升（0.08至0.66），且在成本远低于前沿LLM的情况下实现了跨七种企业场景的高效迁移，减少了约7倍的专家工作量。

Conclusion: 在实体对齐中，将约束作为假设空间算子而非事后验证器是关键，证明了在结构不变量明确的场景下，约束引导比单纯依赖模型更具鲁棒性和可解释性。
...
### [61] [AHEAD: Adaptive Hindsight with Environment-Augmented Distillation for Agentic RL](https://arxiv.org/abs/2608.24114)
*Xiaolong Jin,Dingmin Wang,Vijay Lingam,Varun Kumar*

Main category: cs.AI

TL;DR: AHEAD 是一种针对多轮 LLM 智能体训练的步级感知强化学习框架，通过根据步骤类型动态匹配反馈源来优化训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法通常依赖轨迹级奖励，无法精准识别决策成败；而现有的自蒸馏方法对所有步骤施加相同的引导，忽视了常规步骤与关键错误步骤在反馈需求上的不对称性。

Method: 提出了 AHEAD 框架，将环境反馈作为所有步骤的通用基础信号，并仅在关键错误步骤中引入 LLM 生成的纠正性提示，对标准 GRPO 算法进行微调。

Result: 在 ALFWorld、WebShop 和搜索问答任务中，AHEAD 显著提高了任务成功率，并能以更少的训练步数和交互预算实现目标。

Conclusion: 通过针对不同步骤类型提供差异化的监督信号，AHEAD 有效提升了 LLM 智能体的决策能力和训练效率。
...
### [62] [RePolicy: Reinforcement Learning for Safety-Policy Invocation in Agent Safeguards](https://arxiv.org/abs/2608.24275)
*Houcheng Jiang,Boxuan Zhang,Qiyong Zhong,Junfeng Fang,Xiang Wang,Xiangnan He*

Main category: cs.AI

TL;DR: RePolicy 是一种基于强化学习的语言模型智能体安全防护框架，能够根据动态策略库对执行轨迹进行评估和安全决策。


<details>
  <summary>Details</summary>
Motivation: 现有的提示工程或监督微调方法难以适应未见的轨迹和动态变化的策略上下文。

Method: 利用强化学习（GRPO）结合监督初始化，通过策略轨迹数据集 PolicyTraj-20K 进行训练，能够根据动态策略库调用适用策略并生成基于事实的决策。

Result: 在六个智能体安全基准测试中表现出色，在不同策略上下文下具备强大的安全检测能力和稳健的策略调用性能。

Conclusion: RePolicy 通过强化学习实现了对复杂、动态安全策略的有效响应，为语言模型智能体的安全防护提供了更具泛化性和稳健性的解决方案。
...
### [63] [Robust Code RL via Faulty-Code-Driven Test case Synthesis and Dense Reward Shaping](https://arxiv.org/abs/2608.24135)
*Yiwen Zhang,Xiaodong Yan,Zhenyu Huang,Deng Zhao,Liang Jiang,Qing Cui,Zujie Wen,Zhiqiang Zhang,Jun Zhou*

Main category: cs.AI

TL;DR: RobustTests是一种通过故障代码驱动的测试用例生成框架，旨在解决RLVR在代码生成中因测试覆盖不足导致的奖励欺骗和性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法依赖自动化测试用例，由于测试覆盖不足引发的假阳性会导致奖励欺骗和模型性能下降，亟需提升测试用例的质量与鲁棒性。

Method: 提出基于“近乎正确”的故障代码引导合成测试用例，利用行为特征聚类过滤冗余用例，并引入基于通过率的逐步稠密奖励函数以降低幻觉噪声影响。

Result: 通过RobustTests训练的Qwen3-32B模型在LiveCodeBench上取得了3%的性能提升，证明了该框架能显著增强模型代码生成的准确性与诊断能力。

Conclusion: RobustTests通过高质量的测试用例合成和细粒度的奖励反馈，有效解决了RLVR中的奖励偏见问题，是提升大模型代码生成能力的有效路径。
...
### [64] [OmniJudge or OmniBias? Diagnosing Multimodal Judges through Balanced, Decoupled Lenses](https://arxiv.org/abs/2608.24160)
...
TL;DR: 提出了D3-Omni基准，用于诊断多模态模型（T2I, T2V, TTS）的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有评价指标过度依赖正样本，且难以区分不同的失效模式，导致“全能评判者”模型可能掩盖能力盲点。

Method: 构建了包含53个正交维度的D3-Omni数据集；通过控制提示词重写和原子扰动，从验证过的正样本生成负样本，确保样本的双重平衡、解耦和动态更新。

Result: 实验发现，即使是强大的评判者模型也难以识别违反要求的样本，常将不同的属性视为单一维度处理，且在模态相关维度上表现欠佳。

Conclusion: 现有的 aggregate accuracy 掩盖了系统性的性能盲点，解耦且平衡的评判框架对于准确识别生成模型的细粒度能力至关重要。
...
### [65] [The Invisible Editorial Layer: Formalizing Undisclosed Inference-Time Steering, Probability Placement, and the Attribution Problem in Deployed Language Models](https://arxiv.org/abs/2608.24662)
*Augusto Camargo*

Main category: cs.AI

TL;DR: 该论文揭示了推理阶段的干预（Inference-time intervention）会系统性地改变大模型输出的概率分布，造成“模型权重”与“实际表现”之间的不一致，从而引发治理、安全与商业伦理问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注模型权重和数据，却忽视了推理管线中（如受控生成、水印技术）可能存在的未公开干预措施对生成内容框架（Frame）的影响。

Method: 通过形式化“模型不等于部署系统”的概念，提出了推理归因问题、概率放置（Probability Placement）广告原语以及推理政策透明度审计原则，并结合欧盟及美国相关法规进行分析。

Result: 指出模型行为偏差无法直接归因于权重，且存在通过微调采样前概率来植入商业或意识形态偏见（即推理时间框架偏见）的可能性。

Conclusion: 必须建立推理政策透明度审计制度，以确保部署层的干预措施可追踪，并满足相关法律对人工智能透明度与可审计性的要求。
...
### [66] [Confident at the moment of action: belief miscalibration in LLM play under hidden information](https://arxiv.org/abs/2608.24691)
*Bhushan Kashinath Joshi*

Main category: cs.AI

TL;DR: 研究发现代理系统在执行关键决策时，其表达的自信度（confidence）与实际正确性存在严重背离，尤其是在隐藏信息环境下。


<details>
  <summary>Details</summary>
Motivation: 许多代理系统依赖模型报告的自信度来决定是否执行动作，但缺乏证据表明模型的自信度能准确反映其决策时的真实正确性。

Method: 通过一种隐藏皇家棋子位置的国际象棋变体，在两批实验中对比模型对隐藏信息的主观概率分布与实际正确率，并测试了不同模型配置及决策预算对该指标的影响。

Result: 在高自信度（≥0.5）下，模型的预测正确率仅为1/62；传统的评估指标（如合法性、成本、完成率）无法预测信念质量，甚至出现性能越好、信念质量越差的“反转”现象。

Conclusion: 仅依靠动作的结果（outcome-only）无法评估代理系统的真实信念质量，模型可能在胜出游戏的同时对核心决策持有完全错误的自信，这对于高风险代理系统的安全性至关重要。
...
### [67] [Task-Adaptive Rubrics for GUI Reward Modeling](https://arxiv.org/abs/2608.24174)
*Tao Xiong,Xavier Hu,Wenkai Wang,Qinzhuo Wu,Changqiao Wu,Pengzhi Gao,Wei Liu,Jian Luan,Shengyu Zhang*

Main category: cs.AI

TL;DR: AdaptRubric 提出了一种从粗到细的框架，通过任务类别和实例特定约束生成自适应的 GUI 任务评估准则。


<details>
  <summary>Details</summary>
Motivation: 现有 GUI 代理的奖励模型在处理不同任务实例时缺乏自适应性，常因准则通用化导致评估偏差或过度严苛。

Method: 引入 AdaptRubric 框架，包含两个阶段：一是基于任务族检索的类别级“粗”准则获取，二是基于当前指令约束生成实例级“细”准则。

Result: 实验表明该方法在离线奖励评估和在线强化学习中均优于基线，F1 分数提升 3.6 个点，任务成功率提升 4.23 个点。

Conclusion: 通过引入任务自适应的评估机制，能够显著提高 GUI 代理在复杂指令下的任务完成能力和评估准确性。
...
### [68] [Recursive Experiential-Working Memory Evolution for Long-Horizon Agent Harnesses](https://arxiv.org/abs/2608.24876)
*Zhaochen Yu,Yingcheng Wu,Zhenfei Yin,Kaiyuan Chen,Zhe Zhao,Mengdi Wang,Shuicheng Yan,Ling Yang*

Main category: cs.AI

TL;DR: Recuris是一种用于长程任务的递归记忆架构，通过将工作记忆与经验记忆解耦，实现了代理能力的持续自我进化。


<details>
  <summary>Details</summary>
Motivation: 递归自我提升（RSI）在长程任务中难以实现，因为不断增长的历史信息会导致任务状态模糊和技能调用偏差。

Method: 引入Recuris架构，包含追踪任务进度的工作记忆和提供技能选择的经验记忆；利用元代理根据执行证据对技能记忆进行局部化、验证门控式的更新。

Result: 在四个长程基准测试中，Recuris在37个实验组中的35个提升了成功率，长任务中优势显著，常见失败率降低高达80%。

Conclusion: 递归演进的记忆架构是实现RSI的可扩展基础，使代理能够将积累的经验有效转化为更强的长程执行能力。
...
### [69] [MetaRAG: Belief-Action Aligned Policy Optimization for Agentic RAG](https://arxiv.org/abs/2608.24214)
*Qiuyi Qi,Tian Liang,Jiamu Wang,Jinjian Zhang,Wei Zhou,Pengcheng Zhu,Linjian Mo,Ming Kong,Jie Liu,Qiang Zhu*

Main category: cs.AI

TL;DR: MetaRAG 提出了一种通过“信念-行动对齐”优化 Agentic RAG 的框架，旨在让模型在检索过程中更好地判断何时停止并生成答案。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的 RAG 方法缺乏对模型内部“是否拥有足够证据”的认知，导致检索决策质量不佳。

Method: 引入“验证优先行动生成”机制，结合“内部信念探测”来估计模型对答案的把握度，并基于一致性与准确性构建奖励函数，在训练阶段引导策略优化。

Result: 在七个 QA 基准测试中显著提升了准确率与效率的平衡，且具有良好的跨模型和跨场景迁移能力，推理阶段无额外开销。

Conclusion: 通过显式建模模型的内部信念并强制其与行动对齐，可以有效改善 Agentic RAG 的决策质量和性能表现。
...
TL;DR: 提出了一种名为VTC-Bench的基准测试，旨在评估大型语言模型（LLM）在多次采样时提供多样化且有用输出的能力。


<details>
  <summary>Details</summary>
Motivation: 现有评价体系过于关注单次输出的质量或简单的成功率，忽略了模型在生成多个候选结果时覆盖不同有用答案的能力。

Method: 引入Validated Task Coverage (VTC)指标，并构建了一个包含五个领域的基准测试，利用自动且可重复的方法检查输出的质量与任务相关的新颖性，无需依赖模型作为评判者。

Result: 实验表明，单次采样表现最好的配置未必具有最佳的覆盖率，且简单的多样性度量无法准确反映任务相关的覆盖效果。

Conclusion: 直接评估有限候选集能揭示模型在传统单次评价中无法发现的行为差异，对于需要多候选输出的应用场景至关重要。
...
TL;DR: 提出了TRACE基准，用于评估大型推理模型（LRM）在整个推理流水线（提示词、推理链、最终回复）中的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准主要关注最终回复，忽视了LRM中间推理链可能包含的不安全内容，且缺乏证据标注。

Method: 构建了一个包含9种风险类别和10种攻击策略的基准，对4个LRM生成的全流程内容进行安全标注并提取证据，评估了18种护栏模型。

Result: 研究发现推理链的安全判断难度显著高于提示词或最终回复，且现有模型难以准确提取支持性证据。

Conclusion: 目前的护栏模型尚无法有效检测和定位LRM推理过程中的不安全内容，迫切需要更可靠的检测与溯源能力。
...
TL;DR: STRIVE 是一种用于纵向放射学报告生成（LRRG）的多智能体系统，通过显式推理和验证机制，显著提升了对影像变化描述的准确性和逻辑一致性。


<details>
  <summary>Details</summary>
Motivation: 现有 LRRG 方法将诊断、属性和时间对比混为一体，导致任务干扰、可追溯性差，且未能充分利用病情进展的顺序结构。

Method: 引入多智能体架构，将临床推理分解为诊断、属性和时间变化智能体；采用 Progression-Aware GRPO 训练时间变化智能体，并结合一致性门控和验证智能体进行双重校验。

Result: 在 Longitudinal-MIMIC 数据集上取得了最佳临床疗效，并将纵向变化一致性（LCC）指标提升至基准模型两倍以上。

Conclusion: 通过显式分解推理过程并结合可验证的奖励机制，STRIVE 有效解决了 LRRG 中的任务干扰问题，提高了报告的可靠性和科学性。
...
### [73] [SA-Bench: Evaluating Semantic Alignment in LLM-Based Paper Reproduction](https://arxiv.org/abs/2608.24252)
*Xue Hu,Zewei Pan,Zeli Su,Zhou Liu,Wentao Zhang*

Main category: cs.AI

TL;DR: 研究提出了SemanticAlign-Bench (SA-Bench) 基准，旨在诊断LLM代理在复现学术论文代码时存在的“语义漂移”问题。


<details>
  <summary>Details</summary>
Motivation: LLM代理生成的复现代码常在未察觉的情况下偏离论文规格（语义漂移），缺乏对科学复现准确性的客观评价标准。

Method: 将论文规格拆解为原子化的语义对齐单元（SAUs），从数值、方法、协议和顺序四个维度对模型生成的代码进行诊断评估。

Result: 实验发现最强配置的准确率仅为0.301，现有模型在执行复杂科学复现任务时普遍表现不佳，且强调可执行性的现有脚手架对提高科学复现质量帮助有限。

Conclusion: 提升LLM复现能力的重心应从简单的代码执行转向对论文语义规范的严谨验证，现有的LLM代理尚难以实现科学级别的精确复现。
...
### [74] [Beyond Accuracy: A Dual-Judge Evaluation Protocol for Vision-Language Models in Legally Grounded Tasks](https://arxiv.org/abs/2608.24258)
*Su Myat Noe,Ha Thanh Nguyen,May Myo Zin,Ken Satoh*

Main category: cs.AI

TL;DR: 提出了一种针对法律场景的AI双重评估协议，通过结合质量评分与语义对等判定，揭示了在法律合规性任务中LLM评价的不一致性与盲点。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估指标缺乏针对法律合规性任务的严谨性，难以确保输出在法律标准下的可辩护性，特别是在输入信息受损时。

Method: 引入双重评估协议：包含一个0-10分的质量评价器和一个严格的二进制语义对等性判定器，并在UK交通标志解读任务（受控视觉环境）中通过4,680次评估进行验证。

Result: 发现质量评判与语义对等性判定之间存在显著差异，特别是在高遮挡环境下，模型即使获得高评分，其语义可信度依然较低（II类错误率高达54-63%）。

Conclusion: 单一的质量评分不足以评估高法律风险任务；提出的双重协议能有效识别模型在视觉受损输入下的幻觉，提供比传统评分更可靠的合规性评估信号。
...
### [75] [Real-World Knowledge-Guided Change Data Synthesis for Remote Sensing](https://arxiv.org/abs/2608.24263)
*Yaoyi Qi,Xingxing Weng,Chao Pang,Yongkang Cui,Xiangyu Hao,Xiaokang Zhang,Guibo Zhu,Gui-Song Xia*

Main category: cs.AI

TL;DR: 提出了一种名为KnowChange的知识引导变化数据合成框架，利用预训练视觉语言模型生成更多样化、更灵活的变化检测训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有合成方法依赖人工规则，导致类别转换覆盖范围有限且灵活性不足，无法满足多样化变化检测的需求。

Method: 利用预训练的视觉语言模型（VLM）作为知识源，推理前序场景中合理的变化位置和类别转换，并结合可泛化的合成模型生成数据。

Result: 实验表明，即使在较小规模下，KnowChange生成的合成数据在真实场景迁移和数据增强任务中均优于现有的合成数据集。

Conclusion: 该方法通过知识引导实现了更灵活、更多样化的变化数据合成，且可无缝集成到现有流水线中提升下游任务表现。
...
### [76] [Matched Excess-Outranker Regularization for Candidate-Set Interference in Continual Knowledge Graph Embedding](https://arxiv.org/abs/2608.24273)
*Hao Ren,Junbin Gao,Jiaojiao Jiang*

Main category: cs.AI

TL;DR: 该研究提出了一种名为MEOR的正则化方法，用以解决持续知识图谱嵌入中因新增实体引入“候选集干扰”而导致的历史知识排名下降问题。


<details>
  <summary>Details</summary>
Motivation: 在持续知识图谱更新中，新增实体的加入改变了查询的候选集，即使原有实体的得分未变，其相对排名也会因“候选集干扰”而下降，导致灾难性遗忘之外的性能损失。

Method: 提出了匹配过剩超排正则化（MEOR），通过对比新增实体的压力与结构化匹配的老实体参考，仅在新增实体竞争超过参考阈值时施加单侧惩罚，从而保护历史排名。

Result: 实验表明，MEOR在ENTITY-ComplEx上显著提升了历史MRR，有效减少了候选集干扰，并在FBInc-S和FBInc-L数据集的所有测试场景中均优于对比基准。

Conclusion: 候选实体准入是导致持续学习中排名损失的独立因素，MEOR能够有效控制这种干扰，且无需替换底层嵌入架构或持续学习框架。
...
### [77] [Eating for a Sustainable Planet: Personalized Sustainable Diet Recommendation via Constraint-Aware Decision-Making Modeling](https://arxiv.org/abs/2608.24274)
*Ying Jin,Weiqing Min,Mingyu Huang,Shuqiang Jiang*

Main category: cs.AI

TL;DR: 该研究提出了一种基于约束感知决策机制的个性化可持续饮食推荐模型，旨在在平衡个人偏好与环境可持续性的同时，提供符合营养、经济和文化要求的饮食建议。


<details>
  <summary>Details</summary>
Motivation: 现有的人口层面可持续性建模难以转化为个人层面的实践，主要障碍在于个体差异与可持续饮食要求之间的冲突，缺乏有效的个性化推荐方案。

Method: 引入约束感知决策机制，将可持续性作为“可学习的约束”而非简单的用户偏好进行建模；并构建了包含约15万个食谱、涵盖多种可持续性指标的数据集SusDiet。

Result: 实验结果表明，该方法能够有效地在不牺牲用户偏好的前提下，引导用户做出更具可持续性的饮食选择。

Conclusion: 该框架通过量化手段成功平衡了个体饮食选择与地球健康，为可持续饮食干预和政策制定提供了有力的技术支持与数据参考。
...
TL;DR: 该论文提出 VIDEOHARNESS-RSI，一种递归搜索框架，用于优化固定视觉语言模型（VLM）处理长视频时的上下文构建程序。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中上下文构建机制通常是手动设计的或与模型耦合，难以孤立评估其对性能的提升潜力。

Method: 利用外循环提案器（Proposer）根据先验程序、评估结果和执行轨迹，递归生成并评估可执行的上下文构造程序（Harness）。

Result: 通过递归搜索发现的程序始终优于手动设计的基线，并且生成的上下文构建程序具有良好的跨基准测试迁移能力。

Conclusion: 可执行的上下文构建是一个独立的优化层，通过自动化发现机制可以有效提升固定 VLM 对长视频的理解能力。
...
### [79] [ReproAgent: Contract-Guided Paper-to-Code Reproduction](https://arxiv.org/abs/2608.24291)
*Xue Hu,Zewei Pan,Zhongyuan Wang,Zhou Liu,Zeli Su,Wentao Zhang*

Main category: cs.AI

TL;DR: ReproAgent 是一种旨在将科研论文转化为可执行代码仓库的自动化 AI 代理框架，通过“准备-计划-生成-修复”四阶段流程解决技术细节缺失问题。


<details>
  <summary>Details</summary>
Motivation: 论文内容与实现细节（如默认参数、框架惯例）往往存在分离，且长上下文导致关键算法和指标信息在代理执行过程中丢失。

Method: 提出了 ReproAgent 框架，包含“实现需求”和“参考证据”双通道持久化合约，将论文片段转化为代码义务，并结合相关仓库的结构证据，形成文件级工作包。

Result: 在 PaperBench Code-Dev 评测集上，ReproAgent 在 Claude-Sonnet-4.5 和 Gemini-3-Flash 基座上均取得了最高平均分。

Conclusion: 双通道合约机制能有效维持论文规范与代码实现的一致性，显著提升了 AI 代理在科研复现任务中的性能。
...
TL;DR: OPDSearch+ 是一种无需教师微调的搜索增强推理蒸馏范式，通过两阶段训练（蒸馏后强化学习）提升小模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有搜索增强推理中，高质量多轮搜索轨迹获取成本高，且依赖特定任务训练的教师模型开销巨大，直接使用通用模型蒸馏又常面临性能瓶颈和训练不稳定问题。

Method: 提出两阶段训练法：第一阶段利用冷冻的通用指令模型作为教师，通过逐位置KL散度目标进行策略蒸馏；第二阶段基于蒸馏后的策略进行强化学习（RL）以进一步提升表现。

Result: 在7个QA基准测试中，3B规模的OPDSearch+模型显著优于既有基准，在HotpotQA上提升了13.1%，在2WikiMultihopQA上提升了8.5%。

Conclusion: 无需任务特定微调的教师模型足以塑造学生的策略分布，为后续强化学习提供更优的初始基础，从而达到单纯RL无法触及的性能上限。
...
TL;DR: 研究评估了利用LLM作为裁判（LLM-as-a-Judge）在大规模语音对话代理评估中的有效性，并对比了人类评估员的评价。


<details>
  <summary>Details</summary>
Motivation: 大规模语音代理的评估需要兼顾交互质量和情境判断，传统的纯人工评估成本过高，因此探索自动化评估的可行性和可靠性至关重要。

Method: 通过GPT-4o和GPT-5模型在电信和零售场景下对语音对话进行评估，设置三种不同的评估配置（p0, p1, p2），从交互质量和安全性等维度与人类评判进行相关性分析和一致性检验。

Result: LLM评估的可靠性取决于具体的评价指标和配置方式。研究发现某些指标适合自动化，而另一些则仍依赖于人类的深度情境理解。

Conclusion: LLM可作为大规模评估的有效组件，但建议采用“LLM负责规模化评估+人工负责高敏感度判断”的混合评估管线。
...
### [82] [Selective Regenerative Decoding: Trajectory-Level Intervention for Inference-Time Reasoning](https://arxiv.org/abs/2608.24338)
*Sophia Xiao Pu,Yumo Xu,Sailik Sengupta,Millennium Bismay,Ruixue Lian,James Gung,Yi-an Lai,Arshit Gupta*

Main category: cs.AI

TL;DR: 提出了一种名为SRD的推理时解码方法，通过对推理轨迹的片段进行选择性保留或重生成，而非简单的整体丢弃，从而提高推理效率与质量。


<details>
  <summary>Details</summary>
Motivation: 现有的推理方法将轨迹视为整体，丢弃低分轨迹时浪费了其中高质量的前缀片段，导致计算效率低下。

Method: SRD（Selective Regenerative Decoding）方法：通过评估轨迹片段，对边缘候选者进行部分重生成，保留高质量前缀，丢弃或修正低质量后缀。

Result: 在MATH500、GPQA等数据集上，SRD以更少的生成代币数达到了Best-of-N的准确率，并实现了1.28-1.36倍的样本效率提升。

Conclusion: SRD证明了通过片段级干预替代整体轨迹选择，可以在推理时显著优化准确率与计算量的权衡。
...
### [83] [The Handoff Tax: Continuing Non-Native Trajectories in LLM Agents](https://arxiv.org/abs/2608.24358)
*Roy Ganz,Mor Shpigel Nacson,Adi Kalyanpur,Ron Litman*

Main category: cs.AI

TL;DR: 研究了编码智能体在不同能力模型间切换（handoff）时的成本与质量折中问题，提出了“切换税（handoff tax）”的概念。


<details>
  <summary>Details</summary>
Motivation: 编码任务中用户常需要在低成本模型（LC）和高性能模型（HC）之间切换以平衡成本与质量，但模型间轨迹转移的效果及其影响尚不明确。

Method: 对比了Claude和GPT系列模型，在不同方向（升级与降级）、时机及轨迹信息传递方式（完整轨迹、精简、移除）下评估代码任务表现。

Result: 研究发现从LC升级到HC时存在显著的“切换税”，完整轨迹传递无法弥补一半的质量差距；相比之下，HC降级到LC表现更优；轨迹信息的最佳保留策略取决于切换方向。

Conclusion: 模型间的无缝切换并非理想状态，简单的轨迹传递会导致成本与质量的非对称损耗，应针对升级与降级场景采用不同的信息传递策略。
...
### [84] [SonarLLM: A Native Sonar--Optical Multimodal Large Language Model for Underwater Perception](https://arxiv.org/abs/2608.24325)
*Cong Su,longxuan ma,Ling Dong,Guofeng Tang,Weijie Yin,Haohui Chen,Zhengtao Yu*

Main category: cs.AI

TL;DR: 提出了SonarLLM，一种将声纳作为原生模态的视觉语言模型，旨在解决水下环境光照和浑浊度变化带来的感知挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM主要基于光学编码器，无法有效处理声纳图像的几何结构和声学伪影，且难以在变浑浊的水下环境中自适应利用声纳与光学的互补性。

Method: 引入SonarLLM，包含声纳专用编码器、基于物理特性的特征增强模块以及可靠性感知的分层融合机制；构建了包含识别、计数、VQA和描述任务的SonarBench基准。

Result: SonarLLM在SonarBench测试中表现显著优于基准模型，在声纳单模态下提升34.4个百分点，融合模式下提升25.1个百分点，且随着水体浑浊度增加，融合带来的性能增益显著提升。

Conclusion: 实现鲁棒的水下异构感知不仅需要加入声纳数据，更需要针对声纳的物理特性进行专门的表示学习和动态权重分配。
...
TL;DR: 提出了一种名为自适应影响图（AIG）的框架，用于自动定位多智能体LLM系统的故障。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统复杂性高，现有的故障归因方法依赖人类工程师阅读原始日志，缺乏像传统软件开发中基于组件和依赖关系的结构化可观测性工具。

Method: AIG框架分为两个阶段：第一阶段将失败的追踪日志转换为结构化的影响图，第二阶段通过智能体驱动的图遍历来定位关键错误。

Result: 实验表明，丰富的追踪表示结合自适应图构建和智能体遍历，显著提升了故障归因的准确性，并在Who&When基准测试中达到了SOTA性能。

Conclusion: 故障归因不仅取决于诊断模型的能力，还高度依赖于追踪日志的表示方式及探索策略。
...
### [86] [From State to Action: OODA-Tool for Reliable Multi-Turn Tool Use](https://arxiv.org/abs/2608.24368)
*Rongfeng Guo,Yinxuan Huang,Yusen Wu,Maoqing Zhong,Yunlu Chen,Meng Tang,Teng Long,Vincent Tao Hu*

Main category: cs.AI

TL;DR: 提出了一种名为OODA-Tool的闭环代理架构，通过将状态维护与动作执行分离，解决了多轮工具调用中状态追踪与动作生成之间的竞争问题。


<details>
  <summary>Details</summary>
Motivation: 现有的直接函数调用和ReAct范式将状态跟踪与动作生成耦合在同一自回归轨迹中，导致后续动作生成时容易覆盖或忽略早期积累的任务状态信息。

Method: 引入OODA（Observe-Orient-Decide-Act）架构：通过Observe重构状态、Orient判断执行必要性、Decide规划动作结构、Act执行外部调用，强制过程受控并保持 grounded。

Result: 在多轮、多工具及不完整信息任务中，OODA-Tool显著提升了Qwen3系列模型（0.6B-14B）的任务成功率，特别是在小模型和高度依赖历史信息的场景中表现优异。

Conclusion: 通过解耦状态追踪与动作生成，OODA-Tool显著增强了智能体在复杂多轮工具调用中的可靠性和鲁棒性。
...
TL;DR: ResiSpec 是一种旨在解决多候选推测解码中“残差漂移（Residual Drift）”问题的框架，通过重构验证阶段的分布以提升 LLM 推理效率。


<details>
  <summary>Details</summary>
Motivation: 多候选推测解码技术在处理长序列时会因拒绝初始候选词导致残差目标分布偏离，从而使后续候选词失效，限制了推理速度。

Method: 提出了 ResiSpec 框架，在验证过程中策略性地调整提议分布，将残差目标质量锚定在草稿模型的高置信度区域，同时保持输出的数学一致性。

Result: ResiSpec 成功防止了候选词失效，在保持输出精确性的前提下，比当前最先进的多候选推测方法最高提升了 1.92 倍的推理速度。

Conclusion: 通过纠正推测解码中的分布漂移，ResiSpec 有效突破了多候选方案的性能瓶颈，显著提升了 LLM 的服务吞吐量。
...
### [88] [Do Recipes Have Personas? Characterizing and Generating Creator Style in Attributed Procedural Graphs](https://arxiv.org/abs/2608.24369)
*Lei Jiang*

Main category: cs.AI

TL;DR: 本文提出通过图学习和过程挖掘技术，从视频转录本中提取“程序化人格”（procedural personas），并在烹饪领域实现了个性化执行流程图的建模与生成。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型生成的逻辑趋于同质化，难以捕捉创作者独特的个人执行风格，且在宏观规划上存在缺陷。

Method: 构建了包含执行流图的ViralRecipesTrans数据集；将程序化文体计量学建模为图学习任务；提出一种两阶段模型，利用离散拓扑度量捕捉物理约束，结合马尔可夫先验进行结构化生成。

Result: 实验发现大语言模型在语义分配上表现优秀但缺乏宏观规划能力；所提结构化模型在拓扑控制上更优；通过结合两者优势的集成模型，能更好地生成符合特定创作者风格的个性化工作流。

Conclusion: 该研究证实了程序化风格与执行流程的拓扑特征密切相关，通过融合大语言模型的语义推理与结构化模型的拓扑控制，可以实现更准确的个性化过程自动化生成。
...
### [89] [A Judge Should Know What Changed:Construct Validity for LLM-as-a-Judge Evaluation](https://arxiv.org/abs/2608.24419)
*Jianlin Chen,Wenhui Chen,Ziyao Lin,Chi Man Vong*

Main category: cs.AI

TL;DR: 该研究指出LLM作为评测者存在效度危机，即高的一致性并不代表对评估维度具有敏感性，并提出了衡量构念效度的双维框架。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM评测依赖于一致性和抗扰动性，这无法确保模型真正理解并评估了目标构念（Construct Validity）。

Method: 提出了一个二维构念效度框架：不变性S（对构念无关改动的鲁棒性）和构念敏感性R（对构念相关改动的识别能力）；并使用7个模型在4个领域对该框架进行了实证评估。

Result: 实验发现LLM评测者虽具有高不变性（S≈0.945），但构念敏感性极低（R≈0.319），且对范围变化的敏感度高于强度变化。此外，表面特征足以预测多数人工标注。

Conclusion: 高一致性不能等同于可靠的评估，研究呼吁在评估LLM评测者时需同时报告不变性和敏感性，并对评估数据集本身的质量进行审计。
...
### [90] [Partial Identification under Causal Orders by Linear Programming](https://arxiv.org/abs/2608.24427)
*Eric Rossetto,Alessandro Antonucci*

Main category: cs.AI

TL;DR: 提出了一种无需完整因果图即可对反事实查询进行界限估计的新方法，利用查询本身隐含的结构假设将其转化为线性规划问题。


<details>
  <summary>Details</summary>
Motivation: 现实场景中往往缺乏完整的因果图，而传统非参数化反事实识别高度依赖于完整图结构。

Method: 利用反事实查询诱导的变量拓扑序，对查询进行显式参数化，并将识别任务简化为线性规划问题。

Result: 能够对任意反事实和嵌套反事实查询进行界限估计，证明了所提界限的紧致性，并通过案例研究验证了其在缺乏因果图时的有效性。

Conclusion: 该方法推广了Tian和Pearl（2000）的因果概率界限框架，证明了即使在信息不完整的情况下，也能通过查询结构获得具有信息量的因果推断。
...
### [91] [A Behavior-Guided Online Probabilistic Forecasting Method for Electric vehicle Charging Loads](https://arxiv.org/abs/2608.24441)
*Chenghan Li,Qingxiang Liu,Yinliang Xu,Yuxuan Liang*

Main category: cs.AI

TL;DR: 提出了一种针对电动汽车充电负荷的在线概率预测框架，通过双时间尺度行为表示来处理充电站的行为异质性和时变性。


<details>
  <summary>Details</summary>
Motivation: 电动汽车充电负荷具有较强的行为异质性和时间波动性，现有模型难以应对不同充电站间的持久模式差异及用户行为的持续演变。

Method: 构建了双时间尺度行为表示模型以区分长期充电特性与近期行为状态，并结合语义编码技术实现漂移感知预测，同时引入延迟反馈机制确保预测更新的一致性。

Result: 在十个真实充电站的数据集上，该方法在1小时和4小时预测任务中，相比基准模型在MSE和Pinball损失上均有显著下降（最高达22.6%）。

Conclusion: 该方法通过显式建模行为变化，有效提升了复杂动态环境下电动汽车负荷预测的精度和概率可靠性。
...
### [92] [Mahalanobis-Based Multi-Head Attention for Complex State Propagation](https://arxiv.org/abs/2608.24462)
*Xiaohe Li*

Main category: cs.AI

TL;DR: 提出了一种基于马氏距离与RBF核的MHA-CSP注意力机制，通过重构注意力计算方式实现高效的结构化推理。


<details>
  <summary>Details</summary>
Motivation: 旨在解决传统点积注意力在长序列任务中参数量大、难以捕捉复杂结构化信息的问题。

Method: 使用基于马氏距离的RBF核替代点积，引入树状注意力构建与LogSumExp修正，并通过多头协作的注意力啮合机制（Attention Meshing）增强特征提取。

Result: 在仅有119K参数的情况下，MHA-CSP在长序列状态跟踪任务中表现优于Transformer和GCN，且在精度与训练效率上达到更优平衡。

Conclusion: 基于马氏距离的多头修正机制能有效捕捉符号化结构，为高效结构化推理提供了一种参数高效且性能鲁棒的新范式。
...
### [93] [HMGCLIP: Heterogeneous Multi-Granularity Contrastive Learning for E-commerce Representation Learning](https://arxiv.org/abs/2608.24467)
*Qiuyu Zhu,Yi Gao,Zhichao Wan,Mingyang Ma*

Main category: cs.AI

TL;DR: HMGCLIP 是一种针对细粒度电商产品理解的多模态嵌入框架，通过异构超图挖掘结构化硬负样本并实现多粒度语义对齐。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLMs 在全局嵌入中编码信息，难以捕捉细粒度产品属性（如材质差异），限制了其在精密辨别任务中的性能。

Method: 构建异构超图，利用超图拓扑进行结构感知型硬负样本挖掘，并在关系和超边级别进行多粒度语义对齐，同时引入双粒度推理机制动态融合属性证据。

Result: 在自建细粒度电商数据集和 MAVE 基准测试中，HMGCLIP 的表现显著优于主流多模态编码器、MLLMs 及电商基准模型。

Conclusion: HMGCLIP 通过建模复杂的超图结构，有效解决了细粒度电商产品理解中的属性区分难题，验证了多粒度对齐和推理的有效性。
...
### [94] [Reinforcement Learning-Guided Evolutionary Policy Optimization for Preference-Adjustable Heterogeneous Agile Earth Observation Satellite Scheduling](https://arxiv.org/abs/2608.24470)
*He Wang,Junyu Wu,Hui Li,Yanjie Song,Witold Pedrycz,Liang Li*

Main category: cs.AI

TL;DR: 提出了一种基于强化学习引导算子选择的模因进化算法（RLOSMEA），用于解决异构敏捷地球观测卫星（AEOS）的复杂调度问题。


<details>
  <summary>Details</summary>
Motivation: 异构卫星在轨道、机动能力及资源上的差异导致调度建模困难，传统方法难以在有限算力下高效处理多约束任务。

Method: 构建了包含任务分配间接编码和解码器成本评估的建模层；在优化层，通过强化学习（Actor-Critic）动态选择搜索算子，并结合模因进化算法进行全局探索与局部优化。

Result: 实验表明，RLOSMEA 在处理异构 AEOS 场景时，相比传统元启发式算法，实现了更高的加权效用和更稳定的收敛性能。

Conclusion: 该框架通过将强化学习用于辅助算子选择，成功实现了异构卫星调度的复杂约束处理与高效优化，证明了其在实际调度任务中的鲁棒性与有效性。
...
### [95] [Implicit Q-learning-bootstrapped ant colony optimization for maritime moving-target observation scheduling with agile satellites](https://arxiv.org/abs/2608.24471)
*He Wang,Junyu Wu,Yeye Liu,Yifan Zhou,Jie Zhang,Hui Li,Yanjie Song,Liang Li*

Main category: cs.AI

TL;DR: 本文提出了一种名为IQACO的算法，结合隐式Q学习（IQL）与蚁群优化（ACO），旨在解决敏捷地球观测卫星对海上运动目标的动态调度问题。


<details>
  <summary>Details</summary>
Motivation: 海上运动目标观测调度是一个具有时间依赖性和序列相关性的复杂组合优化问题，传统调度方法在处理动态变化的任务需求和多重约束时效率与稳定性不足。

Method: 引入离线隐式Q学习模块嵌入到构造性蚁群优化算法中，通过学习到的策略自适应调整蚁群算法的参数（信息素因子、启发式因子和挥发率），并利用紧凑的状态表示实时调节搜索过程的探索与开发。

Result: 在14种不同规模和配置的场景测试中，IQACO均获得最高的观测收益，较传统ACO方法提升了3.40%至9.40%，并表现出更快的收敛速度和更强的鲁棒性。

Conclusion: 离线价值学习为受约束的海上动态目标观测调度提供了一种有效的自适应搜索控制机制，显著提升了多卫星协同调度的性能。
...
### [96] [PeakBench: Benchmarking Resource-Aware Tool Invocation in LLM Agents](https://arxiv.org/abs/2608.24509)
*Zhi-Kai Chen,Xu-Xiang Zhong,Song-Yan Li,De-Chuan Zhan,Han-Jia Ye*

Main category: cs.AI

TL;DR: PeakBench 是一个专注于多工具代理并行执行与资源受限调度能力的基准测试，旨在解决现有基准在并行执行逻辑与资源管理方面的缺失。


<details>
  <summary>Details</summary>
Motivation: 现有基准多关注串行任务执行，忽略了并行执行中的资源溢出风险与效率调度问题，导致代理在复杂现实任务中表现欠佳。

Method: 引入 PeakBench 基准测试，包含带有依赖标注和资源配置文件的可执行工作流；并提出两部分评估框架，将逻辑规划与物理调度解耦，分别评估其性能。

Result: 实验表明，强大的逻辑规划并不等同于高效安全的资源调度；同时证明了显式暴露资源信息能显著减少任务溢出并提高资源利用率。

Conclusion: PeakBench 为诊断代理的资源意识行为提供了有效的测试环境，强调了在 LLM 代理设计中引入资源感知调度机制的必要性。
...
### [97] [Neurosymbolic Alignment for Physiologically-Safe Clinical Language Models](https://arxiv.org/abs/2608.24534)
*Abdulhady Abas Abdullah,Erik Cambria,Milena Zivkovic*

Main category: cs.AI

TL;DR: 该研究提出了一种“神经符号对齐”（Neurosymbolic Alignment）框架，通过结合基于知识图谱的生理世界模型来提升临床大语言模型的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的临床大模型虽能生成看起来合理但生理上不安全的建议，传统的纯文本监督难以有效纠正这些偏差。

Method: 利用异构图神经网络（HGNN）构建生理世界模型，通过稳态约束、多跳路径合理性和药物相互作用惩罚对模型回答进行评分，并驱动基于ORPO的迭代训练。

Result: 该方法在临床安全基准（CSB）上表现卓越，临床安全得分（CSS）从69.5%提升至90.8%，在参数量仅为GPT-4十分之一的情况下，所有安全指标均优于GPT-4。

Conclusion: 在训练阶段引入生理知识的接地（grounding）能显著提高临床大模型的安全性，具有独立可验证的改进效果，但仍需进一步评估在实际临床环境中的迁移能力。
...
### [98] [Discovering Adaptive Transmission Programs for Collective Innovation](https://arxiv.org/abs/2608.24545)
*Cédric Colas,Jérémy Perez,Eleni Nisioti,Akhilesh Mocherla,Pierre-Yves Oudeyer,Clément Moulin-Frier,Maxime Derex*

Main category: cs.AI

TL;DR: 该研究利用LLM引导的演化搜索设计了“状态感知”的信息传输协议，显著提升了人类集体智能在发现任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的集体智能研究多关注网络结构，忽略了传输过程应根据个体或集体状态动态调节的潜力。

Method: 将传输协议形式化为状态感知程序，并使用LLM引导的演化搜索算法在集体发现任务中优化这些协议。

Result: 演化后的协议比标准基准性能提升高达37%，且消融实验证明“状态感知”是性能提升的关键核心。

Conclusion: 研究证明了可通过AI设计高效且具有泛化性的传输协议，为增强人类集体智能的协作基础设施提供了新路径。
...
### [99] [When "Must" Becomes "Maybe": Constraint Weakening in LLM Agent Workflows](https://arxiv.org/abs/2608.24569)
*Yiheng Sun,Huifei Wang,Yancheng Zhu,Zhenyu Li,Zebin Zhao,Yifan Yuan*

Main category: cs.AI

TL;DR: 该研究探讨了LLM代理在多阶段工作流中传递信息时，如何因信息压缩导致“操作性状态（Operational State）”丢失，从而引发安全违规。


<details>
  <summary>Details</summary>
Motivation: LLM代理在处理复杂任务时，中间 artifacts 的转换往往会导致关键约束条件从“强制要求”退化为“参考信息”，从而破坏了任务执行的安全约束。

Method: 通过构造1,296个受控合成场景，对比直接传递与压缩、归纳、推迟所有权等多种转换方式，评估不同处理方式下安全阻断（safety blockers）的保留效果。

Result: 实验发现常规的文本压缩导致100%的安全阻断失效，且54.2%的情况下引发了禁止的操作；而完整保留元数据（前提、权限、回退、后果）可将安全性恢复至100%。

Conclusion: 语义信息的可用性并不等同于操作性状态的保留；现有的信息传递机制存在严重的语义衰减，需针对动作约束进行专门的状态保护。
...
### [100] [EviDx: Evidence-Aware Active Diagnosis with Scaffolded LLM Agents](https://arxiv.org/abs/2608.24570)
*Lihang Zeng,Shaoting Zhang,Xiaofan Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种名为EviDx的证据感知主动诊断框架，旨在解决现有医疗大模型在诊断过程中缺乏动态证据获取和过程控制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的医疗大模型多将诊断视为静态预测任务，缺乏对证据获取、假设更新及诊断终止时机的动态管理，无法模拟临床诊断的主动探索过程。

Method: 引入EviDx框架，包含三个核心组件：构建交互环境的E-Synthesis、组织多智能体协作与工具使用的临床诊断脚手架，以及基于不确定性和证据覆盖率的观察者引导运行控制器。

Result: 实验表明，EviDx提升了诊断性能和过程稳定性，并通过三级评估体系揭示了模型在执行稳健性和推理动态方面的边界。

Conclusion: 该研究证实了通过结构化的证据获取流程和动态运行时控制，可以显著增强LLM在复杂医疗诊断任务中的表现与可解释性。
...
### [101] [PhysMLLMs: Spatial Priors for Unified Referring Segmentation and Grounded Reasoning of Images and Videos](https://arxiv.org/abs/2608.24574)
*Siyao Yan,Bo Han,Jisheng Dang,Bimei Wang,Shude Wang,Hong Peng,Yulan Guo,Jianhuang Lai,Bin Hu,Tat-SengChua*

Main category: cs.AI

TL;DR: PhysMLLMs 通过注入物理启发的空间连续性先验，提升了视频多模态大模型的分割稳定性和时空一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频多模态模型在目标遮挡、快速运动或存在干扰物时，常出现抖动、漂移及身份识别错误，缺乏空间先验以保持物体形状和身份的稳定性。

Method: 提出了 REPA-Global 机制，在训练阶段通过离线嵌入缓存和计划蒸馏，将冻结的 DINOv2 教师模型的全局视觉表征对齐到学生模型中，无需额外推理成本。

Result: 在多个视频分割基准测试中，显著提高了掩码质量和跨帧一致性，特别是在遮挡和小目标等挑战性场景下表现优异，且未损害通用图像理解能力。

Conclusion: 物理启发的空间先验注入是提升视频多模态模型时空稳定性的有效方法，且能保持模型原有的通用能力。
...
### [102] [Joint Optimization of Tool Creation and Use for Large Language Model Agents](https://arxiv.org/abs/2608.24571)
*Zhi Rui Tam,Chieh-Yen Lin,Yun-Nung Chen,Shao-Hua Sun,Hung-yi Lee*

Main category: cs.AI

TL;DR: SMITH 是一个强化学习框架，旨在通过联合训练实现工具的自动创建与调用，解决现有工具增强模型中工具编写与使用脱节的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的工具增强系统通常在推理阶段临时提示大模型生成工具，导致生成者与使用者逻辑分离，且缺乏对所生成工具可用性的有效反馈。

Method: 提出了 SMITH (Schema-grounded Multi-task Iterative Tool Honing) 框架，通过强化学习将工具创建与工具使用合并到单一策略中。利用三个独立的奖励维度（Schema、代码、结果）分别针对不同失败模式进行梯度反馈，并支持在构建和使用任务之间进行交替训练。

Result: 4B 参数的 Qwen3 模型在 13 个推理任务上达到了 79.8 的宏平均准确率，优于未训练的 30B 模型；在 TabMWP-Hard 和 GQA 等任务上也表现出色，且该模型编写的工具能提升其他更大规模模型（如 Qwen3-30B）的性能。

Conclusion: 联合训练工具创建和使用能够显著提升模型在复杂推理任务中的表现，使模型生成的工具更具鲁棒性和通用性。
...
### [103] [Pivot-and-Station Multi-Agent Path Finding: Solvability, Complexity, and Algorithms](https://arxiv.org/abs/2608.24585)
*Andrea Di Nezza,Mihir Patel,Fabio Fagnani,Sara Bernardini*

Main category: cs.AI

TL;DR: 提出了一种名为PS-MAPF的多智能体路径规划变体，旨在解决自动化仓储及物流系统中智能体需经过枢纽并最终停放至指定站点的调度问题。


<details>
  <summary>Details</summary>
Motivation: 自动仓储系统中存在大量智能体竞争关键资源并需在任务完成后整齐停放的需求，现有的MAPF模型难以直接解决这种带有中间节点和特定终端约束的复杂调度任务。

Method: 定义了PS-MAPF模型；利用图论手段推导了基于连通性的完全可解性条件；证明了最小化任务时间在单枢纽情况下即为NP-hard问题；提出了基准算法、基于SAT的优化求解器以及枢纽优先级规划（PPP）算法。

Result: 研究给出了判定实例可解性的充要条件；PPP算法在基准测试中展现了卓越性能，解决率达到74-89%，且在任务完成时间（makespan/flowtime）指标上优于基准算法数个数量级。

Conclusion: PS-MAPF有效解决了高密度自动化系统中的停放与路径规划问题，理论分析证明了其复杂性，提出的PPP算法为实际应用提供了高效且可扩展的解决方案。
...
### [104] [Causal Modelling of Support Interventions for Student Competency Assessment](https://arxiv.org/abs/2608.24632)
*Francesca Mangili,Alessandro Antonucci,Rafael Cabañas*

Main category: cs.AI

TL;DR: 本文提出利用结构因果模型（SCM）进行教育评估，通过干预和反事实推理超越传统概率模型的局限。


<details>
  <summary>Details</summary>
Motivation: 传统心理测量模型（如项目反应理论）仅限于关联分析，无法处理教育场景中关键的干预措施（如提示）及反事实分析。

Method: 引入结构因果模型框架，建立由专家引导的逻辑协议，通过结构方程建模评估学生能力，而非单纯依赖概率参数。

Result: 通过对义务教育阶段算法技能评估数据的实证分析，证明了该方法能够有效建模提示等干预手段并进行反事实推断。

Conclusion: 结构因果模型为教育评估提供了比标准关联模型更强大的逻辑基础，使评估能够支持更复杂的教育决策与诊断。
...
TL;DR: 该论文提出了ε-交换性（ε-commutativity）概念，用于处理在概率逻辑模型中因数据偏差导致因子无法完全交换的问题，从而在保持高推断精度的同时实现高效的提升推断（Lifted Inference）。


<details>
  <summary>Details</summary>
Motivation: 由于学习到的参数在现实中存在噪声，本应具有交换性的因子无法达到精确交换，导致传统的提升推断方法难以直接应用。

Method: 引入了ε-交换性作为交换性的松弛形式，设计了相应的提升模型构建方法，并为下游概率推断提供了严谨的近似误差界限。

Result: 实验证明该方法在处理近似交换因子时，能够维持高查询准确度，且显著降低了计算时间。

Conclusion: ε-交换性是实现大规模、鲁棒性提升推断的有效途径，能够在理论保证下平衡推断效率与准确性。
...
TL;DR: 提出了一种名为RACE的统计框架，用于高效评估Transformer神经元在特定领域内的功能一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的神经元可解释性方法要么局限于单个实例，要么计算成本过高，无法在大规模领域内有效发现稳定的神经元行为。

Method: 利用残差对齐（Residual Alignment）的统计推断方法，通过前向传播评估神经元在特定领域的功能一致性，无需依赖梯度计算。

Result: RACE在领域特异性上优于基于梯度的点估计方法，且计算效率提高了两个数量级，同时验证了选定神经元与目标领域之间的关联性。

Conclusion: RACE提供了一种高效、可扩展且精确的机制来识别Transformer中的领域稳定神经元，为大规模机械可解释性研究提供了有力工具。
...
### [107] [Parason: Revealing Subtask and Trial Parallelism in LLM Reasoning](https://arxiv.org/abs/2608.24658)
*Zhengyang Zhang,Zijian Zhang,Jiaxuan Gao,Shusheng Xu,Yi Wu,Song Han,Ligeng Zhu*

Main category: cs.AI

TL;DR: Parason 是一种通过引入“试验并行”（Trial Parallelism）和“子任务并行”来加速大模型推理的新框架，实现了在保持精度的同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型推理多为串行，处理复杂任务时延迟极高，而现有的并行化研究大多局限于子任务分解，忽视了更为普遍的多种尝试并行（Trial Parallelism）。

Method: 通过上下文无关文法（CFG）将串行推理轨迹转化为结构化并行轨迹，并引入并行感知群组相对策略优化（PA-GRPO）进行训练，同时在推理时利用工具调用执行并行路径。

Result: 实验表明，“试验并行”占推理可并行计算的65.5%且随难度增加而增强；Parason在AIME24/25等数学基准上实现了约1.7倍的加速，同时维持了极佳的准确率。

Conclusion: 利用“试验并行”和“子任务并行”的双重优化方案，能够有效打破大模型复杂推理的延迟瓶颈，实现高效且准确的推理加速。
...
TL;DR: StepGuard 是一款针对大模型智能体工具调用的预执行防护模型，通过步级审计降低安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有安全防护多为事后评估，缺乏对智能体工具调用过程中的实时预执行监测，且存在过拦截或漏拦截问题。

Method: 提出了 StepGen 数据引擎自动生成包含安全与不安全动作的对比轨迹，并采用 Balance-GRPO 训练策略动态平衡模型对两类动作的判别准确性。

Result: StepGuard 在 AgentDojo 和 AgentDyn 基准测试中，平均攻击成功率降低了 77.3%，而智能体任务执行效能仅下降 2.8 个百分点。

Conclusion: StepGuard 有效提升了智能体在工具调用过程中的安全性，在保持高可用性的同时提供了优于现有开源方案的防护能力。
...
TL;DR: 该研究通过对医学LLM的思维链（CoT）进行扰动测试，揭示了医学LLM在推理过程中普遍存在“思维链与最终答案脱钩”的现象。


<details>
  <summary>Details</summary>
Motivation: 医疗领域对LLM思维链的真实性缺乏有效评估，目前普遍将CoT视为黑盒，未确认其是否真实参与了医疗决策推理过程。

Method: 设计了一个包含30种临床扰动算子的评估体系（包括反转、否定、属性替换等），通过联动分析“思维链更新”与“答案变更”来衡量模型的逻辑一致性（CDR）。

Result: 实验发现，在临床显著的破坏性扰动下，模型的思维链脱钩率（CDR）高达72.9%，且破坏或移除思维链并不影响模型的准确率。

Conclusion: 医学LLM的思维链往往不是真实的推理过程，而更像是一种事后生成的“文档说明”，该研究提出了一套用于审计医疗AI推理真实性的标准化评估框架。
...
### [110] [Evidence Blindness in Direct Corpus Interaction: Persistent Navigation with AtlasNav](https://arxiv.org/abs/2608.24764)
*Hongyu Guo,Zhiyu Zheng,Zhao Cao*

Main category: cs.AI

TL;DR: 本文提出了AtlasNav，一个通过构建“语料图谱”（Corpus Atlas）将大规模代理搜索从动态重构转变为高效导航的框架，解决了直接语料交互中的“证据盲区”问题。


<details>
  <summary>Details</summary>
Motivation: 在大规模语料库中，直接交互（DCI）方法往往受限于交互预算，导致关键证据难以发现或被忽略（即“证据盲区”），且现有的动态工作空间方法在线开销过大。

Method: 引入AtlasNav框架，通过预先将语料库组织为持久化的多视图“语料图谱”，使代理能够利用结构化信息进行自适应导航，而非针对每个查询实时重构交互空间。

Result: 在BrowseComp-Plus数据集上，AtlasNav在提升准确率至92.05%的同时降低了30.21%的在线推理成本，并在多种缩放规模和异构知识库中表现出优异的鲁棒性和泛化能力。

Conclusion: 高效的代理搜索不仅依赖证据的可访问性，更依赖于语料库的预组织表示，从而将有限的交互预算转化为有效的导航过程。
...
TL;DR: StarHarness是一个通过进化特定于环境的代理接口（harness）来提升代理性能的框架，无需修改模型权重。


<details>
  <summary>Details</summary>
Motivation: 解决现有的AI代理在复杂工具密集型任务中，模型与环境交互时的不匹配问题。

Method: 利用分层任务池、分离搜索与选择任务及保留测试集的方法，对prompt、工具接口、技能及代理循环配置进行自动化进化。

Result: 在多个基准测试（ITBench, EnterpriseOps-Gym, AutomationBench）中性能提升了20-35个百分点，且具有良好的跨模型迁移能力。

Conclusion: StarHarness通过优化环境适配性有效提升了代理在企业任务中的实际表现，降低了模型与环境的不匹配成本。
...
### [112] [CAFE: Self-Improving Search Agents Need Co-Evolving Feedback](https://arxiv.org/abs/2608.24794)
...
TL;DR: 提出了一种名为CAFE的框架，通过智能体（agent）与评估者（critic）的协同进化，实现搜索智能体在运行过程中的实时反馈与纠错，从而提升任务成功率并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 传统的基于结果的奖励无法定位搜索过程中的中间错误，导致错误累积，且智能体与反馈机制之间缺乏有效的协同优化。

Method: 引入CAFE框架，让智能体与批评者共享参数模型。利用反馈条件下的恢复机制，结合在线强化学习中的对比反馈评估与优势塑造，以及离线基于偏好的优化，使智能体学会何时请求反馈并利用反馈进行纠错。

Result: 在七个代理搜索基准测试中优于现有的RL搜索智能体，在六个域外基准测试中保持性能，并显著减少了答案层面的幻觉。

Conclusion: 智能体的自我改进需要反馈机制与策略本身同步协同进化，单纯优化智能体或批评者均会触及性能瓶颈。
...
### [113] [Strictly Causal Streaming Video Anomaly Detection with a Theoretically-Grounded State-Space Core](https://arxiv.org/abs/2608.24810)
*Yogesh Kumar*

Main category: cs.AI

TL;DR: 该研究提出了一种名为“纯因果流式异常检测器”的Mamba架构模型，通过对视觉嵌入的因果预测实现O(1)复杂度的视频异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测方法依赖缓存片段，缺乏对时间记忆与检测延迟关系的理论解释，且仅在GPU而非目标边缘设备上评估效率。

Method: 使用带有输入和状态相关衰减门的对角线性状态空间递归（SSM），通过冻结视觉主干的自监督因果下一嵌入预测进行训练，并推导了递归衰减谱与检测延迟之间的数学关系。

Result: 在Apple M3 Pro上实现极高的推理速度（超过1300 FPS，延迟小于1ms），证明了事件边界门控机制在提升响应速度中的关键作用。

Conclusion: 该方法通过严格因果流式架构实现了极高的计算效率，虽然在Ped2和Avenue数据集上的准确率略低于非因果基准模型，但验证了模型在资源受限边缘设备上的部署潜力。
...
### [114] [Constrained Entity Selection under Partial Knowledge for LLM-Based Knowledge Graph QA](https://arxiv.org/abs/2608.24824)
*Emanuel Kitzelmann*

Main category: cs.AI

TL;DR: 该论文提出了一种名为CES-PK的新框架，旨在利用轻量级符号约束来验证基于LLM生成的知识图谱问答候选项，从而提升准确性并应对知识图谱的不完整性。


<details>
  <summary>Details</summary>
Motivation: 现有的KGQA方法在复杂的模式或不完整图谱下表现脆弱，且纯LLM推理缺乏形式化保证，因此需要一种既稳健又能提供符号支持的方法。

Method: 引入了CES-PK（部分知识下的约束实体选择）问题定义，采用三值约束语义（满足、违反、未知）来处理开放世界假设，并在生物医学知识图谱Hetionet上验证了类型、关系和排除约束的效果。

Result: 实验表明，通过过滤无效候选项显著提高了精确率，同时通过保留非明确违反约束的候选项保护了召回率，满足的约束还能为候选排序提供符号证据。

Conclusion: 该方法通过符号约束与LLM生成的结合，在无需生成复杂执行逻辑的情况下，有效提升了KGQA的性能和可靠性。
...
### [115] [A Dual-Dimensional LLM Framework for Automated Item Incidental Content Similarity Analysis in Large-Scale Assessments](https://arxiv.org/abs/2608.24825)
*Jing Huang,Jihong Zhang,Hua-Hua Chang*

Main category: cs.AI

TL;DR: 该研究提出了一种基于大语言模型（LLM）的自动试题相似度分析（AISA）框架，用于检测大规模评估中的偶然内容冗余。


<details>
  <summary>Details</summary>
Motivation: 传统相似度指标（如BLEU或余弦相似度）难以捕捉试题在结构和语义层面的细微冗余，导致构建无关的局部依赖问题。

Method: 开发了一个包含结构分解和语义相关性的双维度AISA框架，利用LLM量化相似度，并在计算机自适应测试（CAT）中进行模拟验证。

Result: 基于LLM的指标与构建无关的局部依赖指标相关性更高；在CAT应用中，该方法显著提升了参数估计稳定性并降低了偏差。

Conclusion: LLM驱动的AISA框架能有效支持大规模题库维护与智能组卷，在提升评估质量的同时优化了自适应测试的公平性。
...
### [116] [FedV-KGQA: Multi-Hop Question Answering over Vertically Partitioned Knowledge Graphs](https://arxiv.org/abs/2608.24846)
*Md Saikat Islam Khan Bappy,Oshani Seneviratne*

Main category: cs.AI

TL;DR: 提出了一种名为 FedV-KGQA 的联邦学习框架，用于在垂直切分的知识图谱（KG）之间进行多跳问答，无需共享原始数据。


<details>
  <summary>Details</summary>
Motivation: 由于数据治理和主权限制，现实中的知识图谱往往分散在不同机构，导致中心化系统无法处理跨机构的多跳问答问题。

Method: 结合局部图增强和知识图谱嵌入技术，通过“主题实体锚定机制”在无需运行时跨机构通信的前提下，实现对正确图邻域的定位。

Result: 在三个基准测试中表现优异，性能接近中心化系统，且支持 3 跳推理，对嵌入扰动具有较强鲁棒性。

Conclusion: FedV-KGQA 有效解决了跨机构知识图谱多跳问答中的隐私与数据隔离难题，实现了在保护数据边界的同时保持高推理精度。
...
### [117] [SPO++: Stream-Aligned Policy Optimization for Asynchronous Agentic RL](https://arxiv.org/abs/2608.24870)
*Kai Ruan,Jinghao Lin,Qianshan Wei,Ziqi Zhou,Zihe Huang*

Main category: cs.AI

TL;DR: 提出了SPO++，通过改进单流策略优化（SPO）中的优势标准化和证据组织方式，提升了模型在工具使用任务中的在线学习效率。


<details>
  <summary>Details</summary>
Motivation: 群体相对强化学习因需要等待同提示词的兄弟轨迹而产生高昂代价；既有的单流策略优化（SPO）在处理token级策略损失时存在优势值计算与目标函数不匹配的问题。

Method: 引入动作-token度量下的优势标准化，修复了轨迹中心化与actor消耗量不匹配的问题；并将提示词证据按策略生成事件而非接收顺序进行重组。

Result: 在ALFWorld和Math-TIR基准测试中，SPO++相比SPO显著提升了在线学习效率，其中动作-token度量归一化被证明是性能提升的核心因素。

Conclusion: 通过更精确的优势函数归一化和数据组织策略，SPO++有效解决了单流强化学习中的数值匹配问题，适用于长且多变的工具使用任务。
...
### [118] [Fidelity Preference, Not Demographic Preference: A Pixel-Level Attribute-Sensitivity Audit of Image Aesthetic/Preference Scorers](https://arxiv.org/abs/2608.23593)
*Mingyang Xu*

Main category: cs.CV

TL;DR: 该研究评估了主流文本到图像模型中审美评分器是否存在基于肤色和体型的社会人口学偏见。


<details>
  <summary>Details</summary>
Motivation: 审美评分器常被用于过滤训练数据及引导图像生成，但其评分机制是否将特定人口统计特征误认为客观质量标准尚不明确。

Method: 通过对合成图像和真实图像进行像素级干预（如CIELAB L*调整），测试评分器对皮肤亮度和体型的敏感度，并对比合成与真实数据的差异。

Result: 大多数评分器存在“保真度偏好”（即认为原图最好，任何偏离都会扣分）；合成图像的审核结果通常不可靠且与真实数据表现不一致。

Conclusion: 简单的合成图像审计容易导致对偏见方向和幅度的误判；只有基于真实数据的像素级因果隔离分析才能准确区分客观质量偏好与人口统计学偏见。
...
### [119] [The Blending Ratio Is Not Where the Performance Is: Diagnosing Prototype Blending for Few-Shot Adaptation of Vision-Language Models](https://arxiv.org/abs/2608.23634)
*Liangzhi Li,Bowen Wang,Yiming Qian,Thorsten Neumann,Xia Xie,Guangshun Li*

Main category: cs.CV

TL;DR: 该研究分析了视觉-语言模型中Few-shot适配的混合比例调节机制，证明了现有依赖验证集的混合策略存在局限性，并提出基于留一法或线性探测（Linear Probes）可获得更优效果。


<details>
  <summary>Details</summary>
Motivation: 许多Few-shot方法依赖于在测试集或验证集上调优单一混合比例（Zero-shot原型与Few-shot均值的加权），作者旨在探讨该比例的理论最优性、无验证集估计的可行性以及其对性能瓶颈的影响。

Method: 通过推导原型均方误差最小化的闭式解（James-Stein收缩系数），评估了在大规模实验环境（4800个单元）下的理论表现，并对比了留一法（LOO）和无需验证集的线性探测方法。

Result: 实验发现理论最优比率表现不佳，因其忽略了类无关偏移；留一法可在无验证集情况下逼近最优比例；而线性探测（CLAP/LP++）在无需验证集的情况下表现甚至优于测试集Oracle调优的混合模型。

Conclusion: 当前Few-shot混合策略的性能上限主要受限于模型类本身而非超参数调节，通过简单的无验证集估计即可实现最优调参，且线性探测是更有效的性能优化手段。
...
### [120] [Cross-Generation Optimization of YOLOv26, YOLOv11, and YOLOv8 for Fine-Grained Small-Object Detection and Instance Segmentation in Complex Orchards](https://arxiv.org/abs/2608.23636)
*Ranjan Sapkota,Manoj Karkee*

Main category: cs.CV

TL;DR: 本文评估了YOLOv8、YOLOv11和YOLOv26在果园环境中对苹果幼果及其细微结构（花萼、果梗）的检测与分割性能。


<details>
  <summary>Details</summary>
Motivation: 果园环境中存在“绿中绿”相似性、遮挡以及细小物体像素特征不足的问题，导致小物体检测与分割极具挑战。

Method: 对比了五种模型规模（n, s, m, l, x）及两种输入分辨率（640x640, 960x960），共进行了30组实验以评估模型精度与效率的权衡。

Result: YOLOv11s-960达到最高精度（mask mAP 0.402），而YOLOv26s-960在参数量和计算量更低的情况下表现相近；果梗是所有类别中最难检测的。

Conclusion: 针对小物体优化的中小型YOLO模型在果园环境下实现了最佳的精度与计算效率平衡，为农业机器人感知提供了实用基准。
...
### [121] [Scaling Reinforcement Learning for Diffusion Models via Velocity Matching](https://arxiv.org/abs/2608.23664)
*Jaemoo Choi,Wei Guo,Yuchen Zhu,Arash Vahdat,Molei Tao,Julius Berner,Yongxin Chen*

Main category: cs.CV

TL;DR: 本文提出了奖励驱动的速度匹配（RVM）方法，证明了扩散模型的奖励微调无需依赖复杂的似然估计机制。


<details>
  <summary>Details</summary>
Motivation: 现有方法将语言模型的策略梯度机制迁移至扩散模型，导致计算复杂且需近似轨迹似然，本研究旨在寻找更简单高效的优化路径。

Method: 提出基于轨迹无关的速度场更新方法（RVM），直接在速度场上进行梯度修正，并通过锚定项控制模型偏移，同时引入动态跟踪奖励以优化视频生成质量。

Result: RVM 在多种扩散模型微调任务中表现优于现有的基于策略梯度的方法，且显著降低了训练成本。

Conclusion: 扩散模型的奖励微调应回归到其原始的速度表示，而非强行套用似然估计框架，这种方法更具扩展性。
...
### [122] [Platonic Representation Hypothesis on World Models](https://arxiv.org/abs/2608.23720)
*Wenhow Li,Chengwei MA,Hui Xiong,Ying-Cong Chen,Lei Zhang*

Main category: cs.CV

TL;DR: 该研究探讨了“柏拉图表征假说”在世界模型中的体现，提出预测一致性假设，认为优化状态转移目标会促使异构世界模型收敛于共享的潜在结构。


<details>
  <summary>Details</summary>
Motivation: 尽管世界模型表现出色，但其学习到的表征本质尚不明确，研究旨在探究不同世界模型是否会趋向于形成相似的内部表征。

Method: 提出了“预测一致性假设”，通过构建 DINO-WM（DINO World Model）并改变视觉编码器以创建异构模型，通过模型拼接（model stitching）技术验证内部特征的互通性与功能兼容性。

Result: 实验发现不同能力的世界模型演化出了几何相似的内部结构，且模型间的内部特征可以通过映射实现跨模型互用，性能损耗有限。

Conclusion: 追求预测一致性是促进不同世界模型之间形成共享且功能兼容的潜在结构的驱动力，为世界模型中存在通用表征提供了证据。
...
### [123] [DriftAD: Visually-Guided Text Drift for Few-Shot Industrial Anomaly Detection](https://arxiv.org/abs/2608.23723)
*Wenyang Liu,Tianyi Liu,Dongshuo Zhang,Kejun Wu,Adams Wai-Kin Kong*

Main category: cs.CV

TL;DR: DriftAD 是一种用于少样本异常检测（FSAD）的框架，通过动态调整视觉-语言模型的文本嵌入来捕捉工业缺陷的局部和多尺度特征。


<details>
  <summary>Details</summary>
Motivation: 现有的基于 CLIP 的异常检测方法通常使用静态文本提示，无法捕捉工业缺陷高度局部化且依赖尺度的物理特性。

Method: 提出了三个模块：(1) 异常信号放大 (ASA) 模块用于增强微小缺陷信号；(2) 视觉引导文本漂移 (VGTD) 将冻结的 CLIP 文本嵌入转换为空间自适应的异常描述符；(3) 漂移引导空间门控 (DGSG) 利用这些描述符增强异常相关特征。

Result: 在 MVTec-AD 和 VisA 数据集上的 1-, 2-, 4-shot 设置中，该方法在图像级和像素级指标上均达到了 SOTA 水平。

Conclusion: 通过引入动态文本漂移和空间门控机制，DriftAD 有效解决了少样本场景下对复杂工业缺陷检测不精准的问题。
...
### [124] [Velocity-coupled Representation Refinement for Satellite Orbit Prediction](https://arxiv.org/abs/2608.23728)
*Yue Yang,Zhiqiang Wu,Saiyu Qi,Fan Ma*

Main category: cs.CV

TL;DR: OrbitNet 是一种用于卫星轨道预测的速度感知表征学习方法，通过建模位置与速度的耦合关系提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法多关注位置序列的时间依赖性，忽视了位置与速度之间对于描述卫星运动至关重要的内在耦合关系。

Method: 提出了 OrbitNet，引入速度耦合表征优化策略以增强位置表征，并采用轨道分段建模来捕捉局部运动变化与长距离演化模式。

Result: OrbitNet 在 Starlink 数据集的域内评估及六个未见卫星星座的零样本评估中，均优于现有的时间序列基础模型和通用预测方法。

Conclusion: 利用位置与速度的耦合关系进行表征学习，是提高卫星轨道预测性能的有效途径，为轨迹时间序列预测提供了新的思路。
...
### [125] [More Motion Is Not Always Better Motion: Corpus Composition Governs Whether Augmentation Helps SMPL-Based Parkinsonian Gait Severity Estimation](https://arxiv.org/abs/2608.23730)
*Michael Caiola,Andrew C. Weitz*

Main category: cs.CV

TL;DR: 该研究探讨了如何利用SMPL人体运动模型和MotionAGFormer编码器来评估帕金森病（MDS-UPDRS）步态严重程度，并分析了训练语料库对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 旨在探究影响步态严重程度评估精度的因素，特别是训练语料库的构成与模型表示能力之间的关系。

Method: 使用三个预训练的MotionAGFormer编码器作为特征提取器，在不同步态任务和数据源组成的语料库上进行评估，通过对比不同语料池的表现来分析其贡献。

Result: 研究发现，单纯增加数据量并不能显著提升效果，关键在于训练数据是否包含步态速度的变化；合成数据和单一视角视频重构对提升评估效果作用有限。

Conclusion: 步态严重程度的评估性能主要取决于模型是否能捕捉到步态速度的差异，而非简单的语料规模，直接修改学习到的表示形式反而会导致性能下降。
...
### [126] [CRISP: Calibration-Aware Visual State Space Duality for Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2608.23746)
*Kangning Wang,Haopeng Zhang,Zhiguo Jiang*

Main category: cs.CV

TL;DR: 本文提出了CRISP框架，旨在解决视觉状态空间模型（VSSD）在遥感图像语义分割中因压缩空间上下文导致的边界模糊问题。


<details>
  <summary>Details</summary>
Motivation: VSSD在处理遥感图像时会将空间上下文压缩为全局聚合，从而抑制高频信息，导致分割边界平滑，丢失边缘细节。

Method: 提出了CRISP框架，包含两个核心模块：一是Duality Calibration Operator (DCO)，通过残差注入和频率校准恢复局部对比度和边界响应；二是Orthogonal Multi-Prototype (OMP) head，通过为每个类别分配多个正交约束的原型来处理类内大方差。

Result: 在Potsdam、Vaihingen和LoveDA数据集上，CRISP在仅约30M参数的情况下，显著提升了mF和mIoU指标，并保持了与现有先进方法竞争的性能。

Conclusion: CRISP有效缓解了VSSD在遥感任务中的边界平滑缺陷，在保持线性复杂度的同时提升了语义分割的精度。
...
### [127] [Too much of a good thing -- when knowledge distillation promotes overfitting, and how to avoid it](https://arxiv.org/abs/2608.23752)
*Irene Trigueros-Lorca,Leonardo Concepción,Christian Wagner,Isaac Triguero,Daniel Molina*

Main category: cs.CV

TL;DR: 该研究探讨了在知识蒸馏（KD）中引入中间层监督对提升小模型性能及数据效率的作用。


<details>
  <summary>Details</summary>
Motivation: 传统KD主要关注最终输出，但在细粒度、数据稀缺场景下，缺乏对中间层监督机制及其对模型压缩效果的研究。

Method: 提出了一种基于同构模块的知识蒸馏架构，通过中间块级监督进行知识迁移，并利用注意力图、CKA和Grad-CAM等可解释性工具分析监督配置的影响。

Result: 研究发现对于通用数据集，仅蒸馏最后一层即可，但在细粒度、数据受限场景下，增加中间层蒸馏点能显著提升模型性能并减少训练需求。

Conclusion: 合理的中间层块级知识蒸馏是构建高效、数据紧凑且兼顾精度的紧凑型模型的关键技术。
...
### [128] [Primate vision reveals a missing principle for robust dynamic AI](https://arxiv.org/abs/2608.23790)
*Matteo Dunnhofer,Christian Micheloni,Kohitij Kar*

Main category: cs.CV

TL;DR: 该研究通过对比人类、猕猴IT皮层与多种深度学习模型，探讨了视觉系统如何结合外观与运动信息以实现稳健的对象识别。


<details>
  <summary>Details</summary>
Motivation: 现有视觉系统在外观发生剧烈变化时往往难以保持稳健，研究旨在理解大脑如何整合动态视觉信息并评估现有AI模型在模拟这一过程中的表现。

Method: 利用图像及视频神经网络（涵盖识别、分割、光流及预测世界模型），将其表征与人类感知及猕猴下颞叶（IT）皮层的神经活动数据进行跨尺度对比分析。

Result: 预测世界模型在跨外观泛化能力和神经保真度上表现最佳，但所有模型均未能完美复现大脑从早期外观主导到后期外观不变性运动编码的皮层转化过程。

Conclusion: 将运动信息逐步整合进对象表征是实现稳健动态视觉的关键，预测学习是构建人工视觉系统的潜在有效路径。
...
### [129] [Restoring Without Forgetting: Continual Learning Across Image Degradations](https://arxiv.org/abs/2608.23799)
*Alif Ashrafee,Bartosz Krawczyk*

Main category: cs.CV

TL;DR: 该研究提出了“Restoring without Forgetting (RwF)”框架，解决了图像修复任务在处理连续、增量降质（degradation）时出现的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有的一体化修复模型通常假设所有降质类型在训练时均已知，无法应对实际场景中逐个出现的新环境，且由于隐私和存储限制，无法通过重新训练历史数据来避免遗忘。

Method: 引入增量学习范式，为每种新降质学习轻量级适配器（adapter），并结合无需标签的无监督路由机制，在测试时自动识别并处理输入图像。

Result: 在五个降质领域上的实验表明，RwF 显著优于传统的微调方法（PSNR提升约11-15 dB），并在11个真实降质基准上实现了89.5%的路由准确率。

Conclusion: RwF 是首个针对连续多降质图像修复的系统性基准，在保持轻量化的同时有效克服了灾难性遗忘，且具有良好的跨领域泛化能力。
...
### [130] [LUCAID: Agentic Multimodal AI for Lung Cancer Precision Pathology](https://arxiv.org/abs/2608.23803)
...
TL;DR: LUCAID是一款用于肺癌精准病理诊断的代理型AI系统，通过集成9个任务模块实现全流程自动化，并在临床验证中表现优于专家水平。


<details>
  <summary>Details</summary>
Motivation: 传统病理诊断依赖视觉评估，存在主观差异且缺乏整合性工具；现有AI工具功能单一、缺乏泛化能力及前瞻性临床验证。

Method: 开发了集成代理系统LUCAID，包含质控、肿瘤分割、分型、微环境分析、细胞计数及生物标志物评分等9个模块，并支持交互式查询与自动生成结构化报告。

Result: 各分析模块的F1分数达到0.82-0.95；在前瞻性临床验证中，LUCAID与专家组的诊疗决策一致性高达93.0%，显著优于经验丰富的病理学家（68.3%-81.1%）。

Conclusion: LUCAID通过整合式代理架构克服了传统病理诊断的局限性，展示了在临床环境下实现专家级精准肺癌诊断的可行性与临床价值。
...
### [131] [Predicting Radiologist Expertise from 3D Gaze Patterns During CT Interpretation](https://arxiv.org/abs/2608.23836)
*Leila Khaertdinova,Anna Anikina,Claudia Mello-Thoms,Bulat Ibragimov*

Main category: cs.CV

TL;DR: 提出了一种基于注视信息的Transformer框架，通过整合放射科医生的眼动数据来评估其在胸部CT阅片中的专业水平。


<details>
  <summary>Details</summary>
Motivation: 现有CT阅片专业水平评估缺乏客观方法，且眼动追踪在复杂3D医疗影像环境下的应用研究尚显不足。

Method: 利用DINOv2作为骨干网络，通过自注意力机制中的可学习对数空间偏置（learnable log-space bias）和基于注视点的加权池化技术，将放射科医生的注视模式整合到三维特征学习中。

Result: 在包含5名不同经验水平放射科医生共182次阅片的数据集上，模型实现了0.91的ROC-AUC和0.86的F1分数，优于现有方法。

Conclusion: 将视觉搜索行为融入Transformer模型能够有效支持放射学中基于过程的客观专业水平评估。
...
### [132] [Infant Care Video Dataset for Classification of Interventions Using Transformers](https://arxiv.org/abs/2608.23838)
*Igor Bogdanov,James Green*

Main category: cs.CV

TL;DR: 该研究提出了婴儿护理视频数据集（ICVD），利用视频转换器架构实现新生儿重症监护室（NICU）中医疗干预行为的自动检测。


<details>
  <summary>Details</summary>
Motivation: NICU护士记录工作负担沉重（占工作时间25%）且存在大量未记录干预行为，急需自动化文档系统以减轻临床负担。

Method: 构建包含12类模拟干预行为的ICVD数据集，应用TimeSformer和MotionFormer等视频转换器架构进行训练，并与帧级模型进行消融对比实验。

Result: 模型表现优异，最高达到93.97%的Top-1准确率；对比显示时间建模模型比帧级模型（23.17%）高出70.80%的性能。

Conclusion: ICVD数据集为开发自动化新生儿护理文档系统奠定了基础，显著优于传统帧级方法，有助于提升临床效率及记录完整性。
...
### [133] [Object Counting Across Modalities: Taxonomies, Benchmarks, Applications, and Open Challenges](https://arxiv.org/abs/2608.23845)
*Joana Konadu Owusu,Shivanand Venkanna Sheshappanavar*

Main category: cs.CV

TL;DR: 本文对当前基于基础模型的开放词汇目标计数方法进行了综述，指出当前领域存在评估基础设施滞后、模型在基准测试上过度拟合等问题，并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: 现有目标计数研究盲目追求通用性，但模型在处理语义接地、时间身份及遮挡等复杂空间推理任务时表现出系统性失效，且现有的评估基准已饱和。

Method: 引入了一个涵盖模态、机制、提示、监督水平和泛化设置的五轴分类法，审计了涵盖显微镜、遥感、人群计数和农业等领域的文献，并总结出六大结构性矛盾。

Result: 研究发现当前模型多依赖基准测试的统计规律而非真正的开放世界泛化，且在复杂场景理解和空间推理上存在显著缺陷。

Conclusion: 研究建议从单纯的增量工程转向构建鲁棒的评估基础设施，重点关注组合场景理解、主动计数智能体及统一的多模态评估协议。
...
### [134] [DDMS: Discriminative Distillation of Multi-view Foundational Features into Single-view Models](https://arxiv.org/abs/2608.23850)
*Jeong-gi Kwak,Sho Kagami,Yuki Ono,Kwang Moo Yi*

Main category: cs.CV

TL;DR: 该研究提出了一种通过多视角蒸馏来增强单视图视觉基础特征的方法，使其在保留语义信息的同时具备更好的3D一致性和局部判别能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基础视觉特征（如DINO）缺乏多视角几何一致性，限制了其在3D计算机视觉任务中的表现。

Method: 构建一个融合预训练2D特征与多视角几何特征的教师模型，利用判别式排序目标对单视图估计器进行蒸馏，并在保持语义特征空间的同时引入3D一致性约束。

Result: 实验表明，增强后的特征在直接分析、稠密预测迁移及3D提升与渲染任务中表现优异，显著提升了多视角一致性和局部特征匹配能力。

Conclusion: 通过判别式蒸馏能够有效将几何先验注入基础模型，生成既具备强大语义迁移性又拥有优异3D几何感知能力的基础特征。
...
### [135] [LUX: A Lesion-Aware Graph-Conditioned Visual - Language Architecture for Explainable Endoscopic Captioning](https://arxiv.org/abs/2608.23853)
*Alexis Ivan Escamilla-Lopez,Gilberto Ochoa-Ruiz,Salvador Hinojosa,Sharib Ali*

Main category: cs.CV

TL;DR: LUX 是一种用于溃疡性结肠炎内窥镜图像的可解释性图条件视觉语言架构，通过关注病变区域实现精准的临床描述。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型多依赖全局特征，忽视了病理证据的局部性和关联性，导致临床可靠性和可解释性不足。

Method: 构建 lesion-centric 场景图，利用 Grad-CAM 和 CBAM 提取病变区域节点，并将其空间与临床关系嵌入 T5 解码器的交叉注意力机制中。

Result: LUX 在 BLEU、METEOR、ROUGE-L 和 CIDEr 等指标上超越了现有模型，显著减少了幻觉描述，增强了文本与病变区域的对应关系。

Conclusion: 该模型通过将病理证据显式地整合入语言解码过程，有效提升了医学图像描述的临床可解释性与准确性。
...
### [136] [AffineTok: Semantic Affine Consistency for Diffusion-Friendly Visual Tokenizer](https://arxiv.org/abs/2608.23864)
...
TL;DR: 该研究提出了一种名为AffineTok的视觉分词器，通过引入“语义仿射一致性”（SAC）来优化扩散模型中的语义恢复，从而显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉分词器在将语义注入潜在空间时，忽略了去噪过程对语义恢复的需求，导致预测偏差，因此需要建立更好的语义组织机制。

Method: 定义了语义仿射一致性（SAC）及其代理指标M_SAC，并提出了AffineTok，包含全局语义协调标记（GSCT）和后验均值语义对齐（PMSA）两个核心组件。

Result: M_SAC与生成质量有极高的相关性（Pearson相关系数达0.960）；在ImageNet 256数据集上，AffineTok显著降低了gFID，达到SOTA水平。

Conclusion: 语义恢复的一致性对于高效的扩散生成至关重要，通过SAC驱动的训练方法能够有效提升视觉分词器的性能和最终图像生成质量。
...
### [137] [Gen2Physics: Grounding Generated 3D Meshes in Physics via Multi-View Material Decomposition](https://arxiv.org/abs/2608.23869)
*Mauro Comi,Jordi Serrano Berbel,Kevis-Kokitsi Maninis,Philipp Henzler,Manuel Sanchez*

Main category: cs.CV

TL;DR: Gen2Physics是一个将生成的3D网格自动转换为物理仿真就绪资产的框架，通过材质分割和物理属性推断，实现实用的动态模拟。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式3D模型输出缺乏交互式仿真所需的物理属性，且传统方法多基于不兼容标准物理引擎的体素表示。

Method: 集成微调后的视觉Transformer进行材质分割，利用2D到3D投影一致性，并结合视觉语言模型（VLM）进行上下文推理，将表面网格转化为具有密度属性的封闭体积。

Result: 在ABO-500和PartNet-Material数据集上，材质分割精度提升至48.3 mIoU，质量估算准确度与体素方法持平，且能输出封闭的物理子网格。

Conclusion: Gen2Physics成功弥合了生成式3D内容与物理仿真之间的鸿沟，为游戏和机器人领域提供了高效、自动的物理资产生成方案。
...
### [138] [LG-GER: Language-Guided Group Emotion Recognition via Multimodal Evidence Distillation](https://arxiv.org/abs/2608.23880)
*Ahmed Shehab Khan,Zhiyuan Li,Yan Tong*

Main category: cs.CV

TL;DR: 提出了一种名为LG-GER的语言引导蒸馏框架，通过MLLM生成空间证据来训练轻量级VLM，实现了无需检测器的高效群体情绪识别。


<details>
  <summary>Details</summary>
Motivation: 现有群体情绪识别方法依赖复杂的检测器驱动多流管道，且缺乏对图像局部区域的精准监督，难以在资源受限场景下部署。

Method: 利用多模态大语言模型（MLLM）为训练图像生成包含情感信号和置信度的空间证据，通过四种互补损失函数（分类、区域-文本对齐、空间情感及置信度回归）将其蒸馏至单一视觉-语言模型（VLM）中。

Result: 在GroupEmoW和GAF 3.0数据集上表现优于或持平现有最先进方法，且推理阶段无需检测器、MLLM或多流融合。

Conclusion: LG-GER通过将结构化空间证据蒸馏至轻量化模型，有效解决了群体情绪识别任务中模型复杂与性能之间的权衡，使其更适用于实时应用。
...
### [139] [Continual Visual Learning under Evolving Semantic Concept Shift](https://arxiv.org/abs/2608.23903)
*Ismail Lamaakal,Chaymae Yahyati,Yassine Maleh,Khalid El Makkaoui,Ibrahim Ouahbi*

Main category: cs.CV

TL;DR: 提出了一种名为SemReWrite的框架，旨在解决视觉基础模型在面对语义定义演变（而非仅仅是外观变化）时的知识更新问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉模型适配方法大多假设语义任务固定，但在长期运行系统中，分类法和概念定义会发生演变，导致原有模型输出过时。

Method: 引入SemReWrite框架，利用语义差异与稀疏监督定位受影响区域，结合输入依赖的低秩重写机制、结构化语义记忆及过时决策抑制技术进行选择性更新。

Result: 通过构建EvoShift-Bench基准测试，实验证明SemReWrite在处理分类合并、拆分、重定义等复杂语义演变时，在保持原有知识与更新语义之间取得了优于微调和持续学习策略的平衡。

Conclusion: SemReWrite有效实现了视觉基础模型的选择性语义修正，通过引入Rewrite Accuracy (RA) 和 Preservation Accuracy (PA) 等指标验证了其在复杂演变场景下的鲁棒性和高效性。
...
### [140] [HAP: Head-Adaptive Visual Token Pruning via Cross-Modal Alignment](https://arxiv.org/abs/2608.23921)
*Yuanhao Sun,Huawei Ji,Yuan Jin,Cheng Deng,Luoyi Fu,Xinbing Wang*

Main category: cs.CV

TL;DR: 提出了一种名为PAQ（Prompt-Grounded Attention Quality）的视觉Token剪枝方法，通过加权关注提示词相关的视觉特征来高效压缩多模态大模型的视觉输入。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过均匀平均所有注意力头的权重来评估视觉Token的重要性，这会导致背景噪声被放大且精细特征被淹没，导致推理成本高昂且效率低下。

Method: 提出了PAQ指标，用于量化注意力头与提示词的对齐程度。基于此，采用三阶段剪枝策略：层级分组分配预算、使用PAQ加权Softmax聚合注意力图、基于加权后的显著性评分保留关键Token。

Result: 在LLaVA-1.5-7B模型上，仅保留5.6%的视觉Token即可维持99.1%的原始性能，在18个基准测试中表现优于现有最优方法（如AutoPrune）。

Conclusion: 通过PAQ加权机制，模型能够更精准地识别与提示词相关的视觉信息，在大幅降低推理计算成本的同时实现性能的最优权衡。
...
### [141] [ROI-Gated SAHI: Content-Adaptive Slicing-Based Inference for Efficient Object Detection](https://arxiv.org/abs/2608.23923)
*Rashid Riyadh,Abd Ullah Khan,Imad Gohar,Muzammil Behzad*

Main category: cs.CV

TL;DR: 提出了一种ROI-Gated SAHI框架，通过轻量级探测器筛选高信息量区域，旨在减少SAHI在处理高分辨率图像时对背景切片的冗余计算。


<details>
  <summary>Details</summary>
Motivation: SAHI在处理高分辨率图像时存在大量背景计算，导致计算资源浪费，需要一种更高效的区域选择机制来优化推理过程。

Method: 引入轻量级ROI提案器（Proposer）来定位前景区域，并结合自适应路由策略（Adaptive Routing Policy）来决定哪些区域需要进行进一步切片推理。

Result: 在稀疏场景中实现了最高6.90倍的加速（平均3.41倍）；在COCO128数据集上，通过自适应路由策略实现了1.02倍的轻微速度提升，但静态 gating 在该密集场景下性能不及原始SAHI。

Conclusion: ROI-Gated SAHI在目标分布稀疏的场景下极其有效，但在密集场景中需要依赖稳健的自适应路由策略来平衡推理速度与精度。
...
### [142] [GlanceWAM: Sparse Test-Time Imagination for World-Action Models](https://arxiv.org/abs/2608.23927)
*Linhan Wang,Zijian An,Mingyuan Zhang,Chen Dai,Yi Xu,Can Cui,Zichong Yang,Yinlin Chen,Lifeng Zhou,Chang-Tien Lu*

Main category: cs.CV

TL;DR: GlanceWAM 是一种将视频生成与机器人控制解耦的架构，通过异步视觉想象实现实时推理与高性能决策。


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型（WAMs）在同步视频生成的高延迟与放弃视觉想象带来的性能损失之间存在难以兼顾的困境。

Method: 引入异步处理机制：后台异步生成未来帧（Glance），同时在潜在空间内以高频（48ms）解码动作块；使用非干扰注意力掩码隔离表征，并采用抗时延的视界训练策略。

Result: 在RoboCasa基准测试中达到72.2%的成功率，在LIBERO中达到99.0%，推理速度较同步模型提升24倍。

Conclusion: 通过将视觉想象与动作控制解耦，GlanceWAM成功打破了实时性与任务成功率之间的权衡，证明了异步视觉预测在机器人学习中的有效性。
...
### [143] [RefineRank: Joint Box Refinement and Ranking for Surgical Spatio-Temporal Grounding](https://arxiv.org/abs/2608.23928)
*Linzhe Jiang,Jiayuan Huang,Changhao Zhang,Chunyang Jiang,Zhehua Mao,Mobarak I. Hoque*

Main category: cs.CV

TL;DR: RefineRank 是一种旨在解决手术时空定位（STG）任务的方法，通过轻量级模块 RefineNet 将视觉语言模型的情境理解与开放集检测器的精准定位结合起来。


<details>
  <summary>Details</summary>
Motivation: 现有方法在视觉语言模型（理解能力强但定位精度低）和开放集检测器（定位能力强但缺乏语境感知）之间存在权衡，缺乏两者兼顾的有效方案。

Method: 引入 RefineNet 模块，结合冻结的视觉语言模型特征与开放集检测器的候选框，通过预测坐标修正值和质量评分，对候选框进行重排序和优化。

Result: 在 MedVidBench 基准测试中达到 0.421 STG mIoU，优于现有方法；实验表明坐标修正提升了定位上限，评分机制显著改善了 STG 的平均交并比（mIoU）。

Conclusion: RefineRank 证明了仅需微小的盒级训练模块，即可在不重训练骨干网络的前提下，有效整合语境理解与精准定位能力。
...
### [144] [SceneReGen: Generative Reconstruction of 3D Scenes from a Single Image](https://arxiv.org/abs/2608.23930)
*Zefan Tian,Yuteng Ye,Yiheng Zhang,Yuhang Yang,Xueqiang Lv,Shizhou Zhang,Le Liu,Di Xu*

Main category: cs.CV

TL;DR: SceneReGen 是一个通过生成并组合完整物体资产来重建场景的框架，解决了生成式物体补全与场景坐标系之间的表示鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有的物体生成模型通常输出标准化、居中的物体，难以直接在场景坐标系中准确放置，存在严重的表示对齐缺口。

Method: 引入选择性位姿分解技术，利用几何编码器和DiT生成器，将观察到的方向直接编码进生成网格，并通过位置查询融合实例与全局特征进行场景组装。

Result: 在 3D-FUTURE 数据集上，该方法在场景级 CD、F-Score 和 3D IoU 等指标上表现领先，并在自动驾驶和具身智能场景中展现了泛化潜力。

Conclusion: 通过将场景重建重构为资产生成与组装任务，该方法成功弥合了生成先验与物理空间布局之间的鸿沟，实现了高质量的场景级 3D 重建。
...
### [145] [Luce: Relightable Gaussians for 3D Asset Generation](https://arxiv.org/abs/2608.23943)
...
TL;DR: Luce 是一种用于单图生成高质量、可重光照 3D 资产的新型表示方法和生成框架。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 生成方法在捕捉几何细节、物理材质（PBR）属性以及支持可重光照方面存在不足，难以满足高质量渲染需求。

Method: 提出一种基于体素化多模态高斯云的表示，通过变分自编码器将其压缩至潜在空间，并利用整流流（rectified-flow）Transformer 从单张图像生成该潜在表示。

Result: 在 Toys4K 数据集上 FID 提升 28%，在 AI 生成图像基准测试中 CLIP 图像对齐分数达到 0.8519，优于现有基准。

Conclusion: Luce 能够从单张图像生成具有高几何精度、材质保真度和可重光照特性的 3D 资产，且在处理文字和 Logo 等细节方面表现出色。
...
### [146] [Boot-and-Feedback Framework for Generalist-Expert Model Collaboration in Breast Ultrasound Diagnosis](https://arxiv.org/abs/2608.23974)
*Ming Cheng,Hongyu Sun,Zhaolin Chen,Jun Liu,Hossein Rahmani,Qiuhong Ke*

Main category: cs.CV

TL;DR: 提出了一种名为BooF的乳腺超声诊断框架，通过协同MLLM（多模态大模型）与专家模型，提高诊断的准确性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的乳腺超声诊断高度依赖操作员，且现有的多模态模型常产生幻觉，导致诊断不可靠，缺乏临床有效性。

Method: BooF框架包含两个阶段：引导阶段（Boot Stage）利用BI-RADS词库和初步专家诊断引导MLLM生成精准描述；反馈阶段（Feedback Stage）通过注意力门控跨模态融合模块，将文本反馈与视觉特征结合并过滤噪声。

Result: 在多个乳腺超声数据集上的实验表明，BooF模型在诊断准确性和可解释性方面均优于现有的前沿方法。

Conclusion: BooF通过协同机制有效缓解了MLLM在特定医疗领域的幻觉问题，为乳腺超声分析提供了一种更可靠、更具临床价值的方案。
...
### [147] [Source-Face Authenticity Detection for 3D Gaussian Heads Reconstructed from a Single Portrait: A Benchmark and Dedicated Detector](https://arxiv.org/abs/2608.23984)
*Yujie Gao,Zijian Yu,Yan Hong,Jun Lan,Jianfu Zhang*

Main category: cs.CV

TL;DR: 该研究针对单张肖像重建3D高斯人脸带来的深度伪造检测难题，构建了首个大型基准测试，并提出了一种两阶段检测方法。


<details>
  <summary>Details</summary>
Motivation: 单图3D人脸重建技术在提升视觉真实感的同时削弱了伪造痕迹，对身份认证和人脸隐私构成威胁，现有检测器难以捕捉细粒度信息并保持多视角特征一致性。

Method: 提出两阶段检测框架：第一阶段通过掩码自编码器保留细粒度特征，并利用多视角对比学习增强特征一致性；第二阶段融合多层级CLS token进行最终分类。

Result: 实验表明该方法在所建基准测试中表现最优，在各项指标上均领先于现有主流检测器。

Conclusion: 通过捕捉细粒度伪造痕迹和强化多视角一致性特征，该方法能有效提升对重建类伪造人脸的检测性能。
...
### [148] [Absorbing Gradient Conflicts: Modeling Semantic Variance via Kent Distributions for Cross-Modal Hashing](https://arxiv.org/abs/2608.24010)
*Hengjie Zhu,Dayan Wu,Zihao Zhang,Xinze Liu,Jingxuan Yu,Peng Fu,Zheng Lin,Weiping Wang*

Main category: cs.CV

TL;DR: 本文提出了一种基于Kent分布的跨模态哈希框架（KDPH），通过将点代理替换为灵活的分布，有效解决了多标签场景下的梯度冲突与优化坍缩问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于确定性点代理的哈希方法在多标签数据中存在严重的梯度冲突和优化坍缩问题。

Method: 将代理从静态点建模为超球面上的各向异性Kent分布，并引入基于Cayley变换的特定损失函数来保证几何参数的正交性和稳定性。

Result: 实验证明该方法能有效缓解梯度冲突与震荡，在三个基准数据集上的性能显著优于当前主流方法。

Conclusion: KDPH通过引入Kent分布实现了更灵活的代理表示，是解决跨模态哈希中代理坍缩问题的有效方案。
...
### [149] [IterCAD: Iterative Program Repair for CAD Code Generation from Orthographic Views](https://arxiv.org/abs/2608.24020)
*Yuchuan Wu,Ke Niu,Haiyang Yu,Zhuofan Chen,Xiangyang Xue,Bin Li*

Main category: cs.CV

TL;DR: IterCAD 提出了一种从标注尺寸的二维工程图生成参数化 CAD 代码的迭代框架，通过分步生成与修复提高几何准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的一次性（one-shot）生成方法难以纠正早期错误，导致生成的 CAD 代码不可执行或几何结构不一致。

Method: 将任务重构为渐进式修复过程，通过分析当前结果与目标的差异进行“修改”或“停止”决策，并构建了包含中间修复状态的数据集 IterCAD-RS 以及三阶段训练策略（初始化、修复学习、RL 优化）。

Result: 在 CADExpert 数据集上的实验表明，IterCAD 在代码的可执行性和几何保真度方面均显著优于现有的一次性生成基准模型。

Conclusion: 引入迭代闭环机制（视觉理解、几何校验、代码优化）能有效解决 CAD 代码生成中的结构与参数错误。
...
### [150] [Low-Rank Velocity Fields as a Structural Prior for Unsupervised 4D Medical Image Interpolation](https://arxiv.org/abs/2608.24025)
*Haojin Li,Hengzhuo Wang,Chang Liu,Zhiheng Ma,Heng Li,Jiang Liu*

Main category: cs.CV

TL;DR: 提出了一种基于低秩速度场（Low-Rank Velocity Fields）的无监督4D医学图像插值方法，仅利用起始和结束帧即可合成高质量中间帧。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端无监督4D医学图像插值方法在仅有首末帧训练时，常导致中间帧边界不稳定及运动非生理化，缺乏结构约束。

Method: 引入Tucker低秩分解作为速度场的结构先验，将运动分解为全局空间基和样本特有的核心；采用粗到细的多尺度方案建模运动，并从理论上证明低秩参数能有效控制速度场的平滑度。

Result: 在ACDC和4D-Lung数据集上达到SOTA性能，合成的中间帧具有更好的结构连贯性和解剖轮廓稳定性，表现可媲美有监督训练方法。

Conclusion: 利用低秩速度场先验能够有效抑制高频伪影，强制实现解剖一致的形变，为无监督4D医学图像合成提供了一种高效且鲁棒的方案。
...
### [151] [Phase-Aligned Finite-Fourier Periodic Deformation for 4D Medical Image Interpolation](https://arxiv.org/abs/2608.24027)
*Haojin Li,Hengzhuo Wang,Zhiheng Ma,Mingyang Ou,Heng Li,Jiang Liu*

Main category: cs.CV

TL;DR: 提出了一种基于相位结构先验的4D医学图像插值方法，用于从稀疏观测中恢复动态体积。


<details>
  <summary>Details</summary>
Motivation: 现有变形表示无法直接编码医学影像中常见的近周期性运动，且生理运动的非均匀性导致简单时间插值效果不佳。

Method: 利用傅里叶基参数化相位条件速度场，并引入相位对齐的时间重参数化策略，通过双向融合和残差细化合成中间体积。

Result: 在ACDC和4D-Lung数据集上达到最先进性能，生成的中间体积在解剖学上更具合理性和连贯性。

Conclusion: 通过将周期性先验和非均匀运动建模相结合，该方法有效地解决了4D医学图像在稀疏时间采样下的插值难题。
...
### [152] [Event-Based Motion Estimation via Oriented Distance Fields](https://arxiv.org/abs/2608.24223)
*Lei Sun,Yuqin Ma,Weilun Li,Haoran Liang,Runyi Yang,Kaiwei Wang,Danda Pani Paudel,Luc Van Gool*

Main category: cs.CV

TL;DR: 提出了一种基于定向距离场（ODF）的事件相机运动估计方法，通过预计算距离矢量场替代了传统的迭代优化，实现了极低延迟的高精度运动估计。


<details>
  <summary>Details</summary>
Motivation: 现有基于迭代或假设比较的运动估计方法计算延迟高，难以发挥事件相机高时间分辨率的优势。

Method: 利用预计算的事件距离矢量场进行单步平均估计，配合自适应事件计数策略和无参数轨迹滤波，并将其应用于非盲图像去模糊和低功耗瞳孔/光斑跟踪。

Result: 在公共和自建数据集上实现了亚像素精度的运动估计，且延迟最低；去模糊效果在轻量级模型下达到SOTA，并能显著降低近眼设备的功耗。

Conclusion: ODF方法通过计算高效的预定义场成功实现了高性能的实时运动估计，验证了其在复杂视觉任务中的通用性和实用性。
...
### [153] [CARE: Camera-Residual Reserves for First Sightings in Adaptive LiDAR Sensing](https://arxiv.org/abs/2608.24282)
*Jiachen Gong,Yun Li,Ehsan Javanmardi,Wencan Mao,Manabu Tsukada*

Main category: cs.CV

TL;DR: CARE 是一种无需训练的自适应激光雷达扫描策略，通过结合历史轨迹预测与当前相机残差信息，优化有限传感预算下的目标探测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于历史轨迹的扫描策略在发现新物体时存在延迟或漏检，而基于相机的全局引导又会导致预算浪费，亟需一种更高效的资源分配方案。

Method: 提出了 CAmera-REsidual (CARE) 策略，保留部分预算用于覆盖当前相机探测到但历史预测未覆盖的区域，并设计了带有速度保护边界的“遗忘模块”，释放非重要区域的预算。

Result: 在 nuScenes 数据集上，CARE 在 10%-35% 的预算下显著提升了首次发现召回率（提升 4.3-5.2 个百分点），并验证了在真实车辆和闭环模拟中能更早发现障碍物并实现更可靠的制动。

Conclusion: CARE 有效平衡了历史预测与当前环境感知的需求，通过更智能的预算分配提升了自动驾驶系统在受限算力下的安全感知能力。
...
### [154] [ConsensusTAS: Self-Supervised Temporal Action Segmentation for Long-Horizon Construction Videos](https://arxiv.org/abs/2608.24043)
*Xiaoshan Zhou,Yafei Sun*

Main category: cs.CV

TL;DR: 提出了一种名为ConsensusTAS的自监督学习方法，用于无标注的长视频动作分割。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注活动分类，缺乏对建筑作业中细粒度动作序列和时间边界的识别，且人工标注成本过高。

Method: 利用候选分割方案内部的一致性（Consensus）进行无监督训练，实现视频流的自动相位分割。

Result: 在GTEA、Breakfast和Assembly101数据集上均优于现有的最先进方法；在真实建筑场景（砌砖）中表现优异，且具备极高的计算效率，能在CPU上运行。

Conclusion: ConsensusTAS在无需人工标注的情况下实现了高效、精准的动作分割，为移动机器人协作和建筑视频监控提供了轻量化解决方案。
...
### [155] [Variance-Guided Spatial Attention Fusion for Robust End-to-End Driving under Asymmetric Sensor Degradation](https://arxiv.org/abs/2608.24366)
*Weizhi Tao,Zengwang Jin,Xiao Wang,Hailong Huang*

Main category: cs.CV

TL;DR: 提出了一种名为VG-SAF的端到端多模态驾驶框架，通过像素级可靠性估计和注意力机制来应对摄像头与激光雷达在传感器失效下的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态自动驾驶系统在面对非对称传感器降级（部分区域损坏）时表现脆弱，缺乏有效的可靠性建模和失效抑制手段。

Method: 引入物理驱动的数据增强模拟传感器故障，利用跨分支密集蒸馏进行可靠性监督，并通过Variance-Guided Spatial Attention Fusion (VG-SAF) 机制，使用可靠性映射作为空间门控来优化模态融合。

Result: 在CARLA Longest6基准测试中，VG-SAF在多种传感器降级场景下显著提升了驾驶分数、路线完成率和安全性指标。

Conclusion: 通过引入可解释的密集可靠性监督和空间门控，VG-SAF有效提升了复杂环境下多模态驾驶系统的鲁棒性。
...
### [156] [X-MULTI: VLM-based Imaging Factor Disentanglement for Factor-Aware Image Synthesis](https://arxiv.org/abs/2608.24563)
...
TL;DR: 该论文提出了X-MULTI模型及I-FAA评估指标，旨在解决文本生成图像中成像因子解耦的监督不足和评估指标存在关联泄露的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法MULTI在未观测的组合上缺乏监督信号，且其评估指标FAA存在跨因子关联泄露，导致解耦质量评价不准确。

Method: 引入X-MULTI，利用预训练视觉语言模型(VLM)监督训练中合成的未知组合；提出改进的评估指标I-FAA，通过特定增强策略消除因子间的相关性。

Result: X-MULTI在处理未知成像因子组合时表现出更好的对齐效果，I-FAA指标有效地降低了关联泄露，提供了更稳健的解耦评估。

Conclusion: 通过增强监督信号和优化评估方法，本研究提升了生成模型在成像因子上的组合泛化能力及评估的严谨性。
...
### [157] [WeMM-Embedding: WeChat Multi-Modal Embedding Technical Report](https://arxiv.org/abs/2608.24053)
*Junjie Zhou,Ke Mei,Lei Li,Tianyi Wang,Fengyun Rao,Jing Lyu*

Main category: cs.CV

TL;DR: 本文提出了WeMM-Embedding，这是一个支持多种模态（文本、图像、视频、文档）的通用多模态嵌入模型家族。


<details>
  <summary>Details</summary>
Motivation: 为了应对现代AI系统在跨模态检索、推荐和智能体应用中对通用嵌入模型的需求。

Method: 采用两阶段训练策略：第一阶段进行大规模多模态对齐，第二阶段利用精选数据、细粒度相关性监督和跨尺度知识迁移进行优化。

Result: WeMM-Embedding在多个公开基准测试中表现卓越，其中9B模型达到80.6分的SOTA成绩，并在微信内部搜索与推荐业务中取得了显著的线上A/B测试提升。

Conclusion: 该研究提供了一套高效的通用多模态嵌入方案，证明了通过特定训练策略在多模态理解与检索任务上实现领先性能的可行性，并已开源以推动领域发展。
...
### [158] [VisCache: Visual KV Cache Pruning for Efficient Vision Large Language Model Inference](https://arxiv.org/abs/2608.24063)
*Lyuke Wang,Zhuo Li,Guangxu Zhu*

Main category: cs.CV

TL;DR: VisCache 是一种无需训练的即插即用框架，旨在通过粗粒度到细粒度的视觉KV缓存剪枝，解决视觉大语言模型（VLLM）在长上下文推理中的高昂计算与内存开销问题。


<details>
  <summary>Details</summary>
Motivation: 现有的KV压缩方法在处理视觉Token时往往采用均匀剪枝，导致信息丢失严重且性能下降，无法满足长上下文推理的需求。

Method: 提出VisCache框架，包含两个阶段：1. 利用轻量级VLM过滤冗余关键帧；2. 采用PruneKV算法，通过抛物线型层级预算分配和非对称更新机制（剪枝键、融合值）实现针对性的KV压缩。

Result: VisCache实现了高达2.35倍的推理加速，在仅保留19-28% KV缓存的情况下维持了模型性能，在效率与性能之间达到了新的Pareto最优。

Conclusion: VisCache通过精细化的视觉KV管理，有效缓解了VLLM长上下文推理的资源瓶颈，提供了一种高效且易于集成的优化方案。
...
### [159] [Representation Learning in Diffusion and Flow-based Model: An Application Aspect](https://arxiv.org/abs/2608.24068)
*Yanchen Xu,Sida Huang,Zhenyu Gu,Ruishu Zhu,Yilan Gao,Hongyuan Zhang*

Main category: cs.CV

TL;DR: 本文系统性地综述了扩散模型与流模型在生成建模与表征学习之间的互动关系，并提出了一个三层渐进式框架。


<details>
  <summary>Details</summary>
Motivation: 生成模型（扩散模型与流模型）与表征学习之间存在双向促进关系，当前缺乏对如何利用这些模型增强感知与理解任务的系统性归纳。

Method: 提出了一个三层框架：1. 利用表征学习提升生成能力；2. 利用生成模型提取表征以服务于感知任务；3. 迈向通用统一的应用程序。

Result: 分类整理了生成模型在图像分类、密集预测、实例感知及少标注场景下的应用，并梳理了该领域的统一分类体系。

Conclusion: 生成模型已超越单纯的生成任务，成为强大的特征提取器；未来的研究应聚焦于如何更有效地将生成式表征迁移至各类下游理解任务。
...
### [160] [Joint-Embedding Prediction of Masked Point Tubes for Self-Supervised Learning on 4D Point Cloud Videos](https://arxiv.org/abs/2608.24093)
*Jheng-Ling Lee,Shang-Tse Chen*

Main category: cs.CV

TL;DR: 提出了一种基于JEPA架构的4D点云视频自监督学习方法，通过隐空间预测而非重建来学习时空表征。


<details>
  <summary>Details</summary>
Motivation: 点云标注成本高昂，且传统的重建预训练过于关注底层几何细节，忽略了语义特征。

Method: 采用潜在点管预测（Latent Point-tube Prediction），通过掩码时空区域并在特征空间进行预测，同时引入“草图各向同性高斯正则化（Sketched Isotropic Gaussian Regularization）”以防止表征塌陷。

Result: 在动作和手势识别基准测试中，该方法在微调、小样本学习及跨数据集迁移任务中表现优异。

Conclusion: 对于4D点云视频，基于JEPA的隐空间预测是比重建预训练更有效的替代方案，能更好地捕获时空动态语义。
...
### [161] [DRRG: A Discrete Diffusion Framework for Radiology Report Generation](https://arxiv.org/abs/2608.24105)
*Shaoyang Zhoua,Yingshu Li,Yunyi Liu,Lijun Pu,Lingqiao Liu,Lei Wang,Luping Zhou*

Main category: cs.CV

TL;DR: 该研究提出了一种名为DRRG的离散扩散模型框架，用于医学影像报告生成，通过迭代去噪取代传统的自回归生成模式。


<details>
  <summary>Details</summary>
Motivation: 现有自回归(AR)模型无法修正已生成的错误，且与放射科医生迭代完善报告的过程不符，导致容易产生错误传播。

Method: 开发了基于离散扩散语言模型(DLLM)的框架，采用迭代掩码标记去噪策略，并结合临床实体感知互补掩码和临床概念注入模块。

Result: 在MIMIC-CXR和CheXpert Plus数据集上，DRRG在BLEU-4、CheXpert-F1等多项指标上优于主流方法，且使用的模型参数量更小。

Conclusion: 离散扩散模型通过迭代双向细化过程，为放射影像报告生成提供了一种比自回归更高效、更具临床一致性的替代方案。
...
### [162] [MatReplace: A Reference-Free, Conditioning-Aligned Benchmark for Material Replacement in Interior Scenes](https://arxiv.org/abs/2608.24107)
*Mingzhe Du,Thong Thanh Nguyen,Nguyen Tran Cong Duy,See-Kiong Ng,Luu Anh Tuan*

Main category: cs.CV

TL;DR: 本文提出了MatReplace基准测试，用于评估室内设计中的材料替换任务，揭示了当前模型在文字描述材料方面表现良好，但在视觉参考引导方面仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: 现有指标无法公平评估“一对多”的材料替换任务，且缺乏针对该任务的公共基准，难以比较不同引导方式下的模型表现。

Method: 引入无需参考图的MatReplace基准，通过四个可验证维度（材料准确性、光照和谐性、环境保持、内部结构）评估三种条件输入（文本指令、掩码+指令、参考图）下的模型表现。

Result: 顶尖闭源模型已较好解决了基于文本的材料渲染，但在基于图像的参考引导中表现不佳，甚至出现模型退化，且该指标与专家评分高度一致。

Conclusion: 室内设计的语义材料渲染已基本解决，但如何从视觉像素中精准提取并应用材料属性仍是待解决的核心难题。
...
### [163] [TransPhy: Visual In-Context Learning for Physically Grounded Image Editing](https://arxiv.org/abs/2608.24119)
*Siyi Xie,Xuanke Shi,Jinsheng Quan,Haoran Tang,Zukai Chen,Lei Yang,Quan Wang*

Main category: cs.CV

TL;DR: 该研究提出了PhysVICL-74基准测试集及TransPhy框架，旨在解决视觉上下文学习（VICL）在处理具有物理基础的图像转换任务中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的VICL方法主要局限于外观层面的转换，缺乏对涉及物理规律（如材料、几何、交互）的复杂图像转换的理解与执行能力。

Method: 构建了PhysVICL-74数据集；提出了TransPhy框架，通过物理规则归纳和过渡对齐渲染实现目标图像合成，并利用基于局部过渡线索的专家混合（MoE）路由技术。

Result: 实验表明，TransPhy在物理规则遵循、查询一致性以及未见规则的泛化能力方面均优于现有的视觉上下文编辑方法。

Conclusion: 该工作有效提升了AI对物理环境约束下图像转换的推理与生成能力，为视觉智能体处理复杂物理场景提供了新的范式。
...
### [164] [Graph-Supervised Hierarchical Clinical Alignment for Radiology Report Generation with Large Language Models](https://arxiv.org/abs/2608.24121)
*Yingshu Li,Yunyi Liu,Zhanyu Wang,Zailong Chen,Lingqiao Liu,Lei Wang,Luping Zhou*

Main category: cs.CV

TL;DR: 提出了一种基于图监督的层级临床对齐方法（GSHCA），通过在病灶级别和报告全局级别进行细粒度对齐，提升放射科报告生成的临床准确性。


<details>
  <summary>Details</summary>
Motivation: 现有放射科报告生成模型多采用全篇报告级别的监督，存在“颗粒度不匹配”问题，导致生成的报告缺乏临床忠实度。

Method: 引入临床知识图谱作为结构化先验，设计了病灶中心对齐（Disease-Centric Alignment）和全局临床语义对齐（Global Clinical Semantic Alignment）两个层级的监督，并采用实例判别匹配与病灶条件软正则化相结合的策略。

Result: 在 MIMIC-CXR、IU-Xray 和 COV-CTR 数据集上表现优异，3B 参数模型性能超越了多个 7B/13B 的现有模型。

Conclusion: 优化监督结构对于放射科报告生成比单纯增加模型参数规模更为有效，能显著提升生成报告的临床准确性。
...
### [165] [Syn2RealTrack: Bridging the Gap Between Synthetic and Real-World Datasets for Online Multi-View Multi-Target Tracking](https://arxiv.org/abs/2608.24130)
...
TL;DR: Syn2RealTrack 提出了一种无需重训练的模块化框架，通过解耦相机标定、物体形状先验和目标统计假设，有效缓解了仓储场景下多相机3D感知的“合成到真实”域差异。


<details>
  <summary>Details</summary>
Motivation: 多相机3D感知在合成数据训练与真实数据推理之间存在较大的性能鸿沟，且传统方法试图通过单一域适应模块解决所有问题，忽视了误差产生的不同来源。

Method: 将差异分解为三个部分：无需预先标定的图像畸变恢复、基于可见性权重的局部描述符融合、基于几何闭式解的高度测量以及结合因果滤波的闭合世界基数先验。

Result: 该方法无需重训练特征提取器，在AI City Challenge 2026 Track 1评估中达到了52.0118%的3D HOTA指标。

Conclusion: 通过在几何和外观之间重新分配信任度，Syn2RealTrack 能够有效弥合仿真到现实的鸿沟，提供了一种高效、灵活的多相机3D跟踪解决方案。
...
### [166] [PlaceSeek: Human-Centered Geospatial Retrieval of Urban Outdoor Places via Semantic Grounding and Affective Alignment](https://arxiv.org/abs/2608.24133)
*Ziqi Cui,Shangyu Lou*

Main category: cs.CV

TL;DR: PlaceSeek 是一个以人为中心的户外地点检索框架，通过将自然语言查询映射到街景图像，支持基于功能和情感需求的复杂地理空间检索。


<details>
  <summary>Details</summary>
Motivation: 现有的地理空间检索多基于兴趣点（POI）和元数据，难以满足用户对于户外空间的功能性、开放式或情感化需求。

Method: 引入意图感知机制将查询分解为功能和情感子意图；利用语义基础模块（Semantic Grounding Module）验证物理证据；通过基于 LoRA 微调的视觉-语言模型进行情感对齐（Affective Alignment）与排序。

Result: 在米兰的 31,956 个街景位置测试中，PlaceSeek 的 Precision@5 达到 88.0%，nDCG@5 为 0.920，显著优于 CLIP 和 VQA 等基线模型。

Conclusion: 研究表明，有效的复杂城市空间检索需要同时建模可验证的视觉证据和人类的感知偏好，PlaceSeek 为下一代地理空间检索系统提供了有效框架。
...
### [167] [EgoErrorVQA: Assess Egocentric Comprehension Capabilities through Procedural Errors for Ego-Agentic AI](https://arxiv.org/abs/2608.24134)
*Junlong Li,Junxi Li,Jianjun Gao,Chen Cai,Lap-Pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: 该论文提出了EgoErrorVQA任务，旨在评估视觉模型在自我中心视角下的程序理解能力及错误检测能力，并引入了Ego-ADR推理框架以提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉模型缺乏对日常生活中程序性步骤的理解能力，尤其是在检测和纠正程序错误方面的能力严重不足。

Method: 提出了EgoErrorVQA数据集；基于Agent2Agent (A2A)协议构建了评估代理；设计了自适应解耦推理框架Ego-ADR以增强对复杂程序错误的逻辑推理。

Result: 实验表明现有模型在处理程序错误方面存在显著弱点；Ego-ADR框架在多项指标上均实现了性能提升并达到最先进水平。

Conclusion: 该研究填补了自我中心视角下程序错误理解的评估空白，提出的框架有效提升了模型在复杂程序性任务中的鲁棒性和推理准确性。
...
### [168] [Rubrics as Visual-Repair Context for Self-Evolving UI-to-Code Generation](https://arxiv.org/abs/2608.24138)
...
TL;DR: 提出了一种名为RubSE的框架，通过基于准则的结构化反馈来引导大视觉语言模型进行更稳定的UI转代码自我进化。


<details>
  <summary>Details</summary>
Motivation: 现有模型在UI转代码的自我进化中存在“视觉修复耦合”问题，即修复局部错误往往会引发全局视觉退化，导致进化不稳定。

Method: 引入RubSE框架，利用结构化的“准则（Rubric）”作为视觉反馈上下文，通过生成候选准则、优先选择修复目标并记录历史，实现有针对性的分步视觉修复。

Result: 在6个VLM模型和3个基准测试中，RubSE显著优于传统自我进化方法，减少了轨迹崩溃，并提升了修复的稳定性和性能上限。

Conclusion: 通过结构化的准则引导可以有效解决视觉修复耦合问题，使模型能够进行更精确、更可控的自我改进。
...
### [169] [What Does Prompt Learning Change? -A Natural-Language Concept Analysis of Vision-Language Models](https://arxiv.org/abs/2608.24142)
*Ryo Kamiya,Hiroshi Kera,Kazuhiko Kawamoto*

Main category: cs.CV

TL;DR: PromptSpLiCE 是一种事后解释方法，通过稀疏概念字典将视觉语言模型的提示向量转化为可解释的自然语言描述。


<details>
  <summary>Details</summary>
Motivation: 现有的提示学习（如 CoOp）优化的连续提示向量缺乏自然语言的可解释性，导致模型决策过程不透明。

Method: 利用固定的自然语言概念字典，将类条件文本嵌入表示为概念的稀疏组合，并对比提示学习前后概念分布的变化，同时推导出局部梯度表达式以分析性能增益的几何直觉。

Result: 实验显示提示学习显著改变了概念分布（平均仅 1.6 个前十概念保留），且概念分布的变化程度与模型准确率提升呈正相关。

Conclusion: PromptSpLiCE 有效揭示了提示学习如何通过调整概念关注点来提升模型性能，并提供了模型行为的几何解释。
...
### [170] [Rethinking Pre-Training and Augmentation for Zero-Shot Cross-City Object Detection](https://arxiv.org/abs/2608.24154)
...
TL;DR: 该研究提出了一种面向交通监控的跨城域目标检测框架，通过多数据集预训练和抗域增强技术解决了地理域偏移问题，且无需访问目标域数据。


<details>
  <summary>Details</summary>
Motivation: 现实场景中交通监控系统面临地理域偏移挑战，且隐私保护需求限制了对目标域数据的直接访问，导致传统领域自适应方法失效。

Method: 提出了包含两个核心模块的训练流水线：一是基于类无关目标性蒸馏的多数据集预训练策略；二是采用Grayworld变换的领域鲁棒数据增强流，强制模型学习几何形状特征而非颜色捷径。

Result: 在RF-DETR架构下，该框架较基线模型提升了24.29 mAP，并在AI City Challenge Track 6排行榜中获得第一名（47.53 mAP）。

Conclusion: 通过解耦几何结构与语义分类并消除对颜色信息的依赖，该方法在完全无目标域数据的情况下有效提升了交通监控模型的跨城域泛化能力。
...
### [171] [ViSculpt: Visual-Centric Agentic Geometry Editing](https://arxiv.org/abs/2608.24169)
*Bo Pang,Jiaqi Pan,Xiaocheng Zhang,Jiacheng Xu,Guoping Wang,Peng-Shuai Wang*

Main category: cs.CV

TL;DR: 提出了一种基于多智能体、无需训练的视觉中心系统，通过模拟人类艺术家在 Blender 中的交互操作，实现对现有 3D 网格的直接编辑。


<details>
  <summary>Details</summary>
Motivation: 传统 3D 几何编辑依赖脚本生成，难以处理基于感知的定点编辑，且容易破坏原始模型，需要一种更符合人类工作流的视觉交互方式。

Method: 构建多智能体系统，通过 multimodal LLM 观察 Blender 视口，根据自然语言指令进行推理，并通过模拟用户交互直接在 GUI 中执行局部网格编辑。

Result: 实验证明该系统能有效执行局部网格修改指令，同时较好地保留了原始资产的整体特征，实现了原生 3D 软件中的原地编辑。

Conclusion: 这种视觉中心的智能体方法为 3D 几何编辑提供了一种无需训练、可直接操作现有资产的新范式，是迈向专业图形软件自动化编辑的重要一步。
...
### [172] [SandwichQuant: Which Parameters Matter Before and After Quantization?](https://arxiv.org/abs/2608.24173)
*Peng Xia,Junbiao Pang*

Main category: cs.CV

TL;DR: 该论文提出了一种基于参数子空间的量化校准方法 SandwichQuant，通过优先优化低维的归一化-仿射参数来提升量化模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有量化校准方法未明确有效的参数优化子空间，且校准能力在参数组间分布极不均匀。

Method: 通过分解参数并验证发现归一化-仿射子空间具有最高效的校准能力；提出 SandwichQuant 框架，在量化前后分两个阶段对归一化-仿射参数进行适应性调整。

Result: 在视觉模型和大型语言模型上的实验表明，该方法在多种低位宽量化设置下均能持续提升性能。

Conclusion: 量化校准应聚焦于特定的高效率参数子空间，该研究验证了子空间对齐校准在提升模型量化鲁棒性方面的有效性。
...
### [173] [Amortized Set Prediction for Inverse IFS Reconstruction from Density Maps](https://arxiv.org/abs/2608.24175)
*Yutaka Yamaguti*

Main category: cs.CV

TL;DR: 该研究提出了一种基于学习的估计器，通过前向传播直接预测迭代函数系统（IFS）的仿射变换参数，从而大幅加速了从图像反推分形参数的逆问题求解。


<details>
  <summary>Details</summary>
Motivation: 传统的IFS逆问题通常依赖于耗时的逐图像优化，效率低下且难以在大规模任务中应用，需要一种更高效的近似求解方法。

Method: 利用深度学习模型预测访问频率密度图对应的仿射变换参数，引入匈牙利算法解决集合无序匹配问题，并结合可微的前向模型在测试阶段进行迭代细化。

Result: 在合成数据上，该方法在速度-质量权衡上表现优于传统优化；在真实数据集（MNIST/Fashion-MNIST）上，速度提升了12到2600倍，且重构指标优于现有优化器。

Conclusion: 通过学习到的初始参数替代随机初始化，不仅显著提升了收敛速度，还提高了优化过程陷入全局最优的成功率，有效解决了IFS的逆问题计算瓶颈。
...
### [174] [NeoWorld-Pro: Programming Interactive Scenes from Monocular Images for Embodied Simulation](https://arxiv.org/abs/2608.24212)
*Yumeng He,Yichen Song,Xiaotian Yang,Weijia Zhang,Zanwei Zhou,Junru Gong,Xiaokang Yang,Yunbo Wang*

Main category: cs.CV

TL;DR: NeoWorld-Pro 是一个利用 MLLM 和物理反馈循环，从单张图像生成可交互、物理可执行的 3D 仿真场景的框架。


<details>
  <summary>Details</summary>
Motivation: 现有的从图像到 URDF 的方法缺乏物理基础和场景级交互性，难以满足具身智能对高质量仿真资产的需求。

Method: 利用多模态大语言模型（MLLM）进行程序合成，将单张 RGB 图像转换为描述物体几何、关节和物理属性的可执行程序，并通过物理引擎闭环机制对生成的程序进行迭代优化。

Result: 该方法在重建质量和物理交互性上优于现有的开环及单目重建方法，支持稳定的堆叠和精细化操作等下游任务。

Conclusion: NeoWorld-Pro 通过程序化建模和物理反馈验证，有效弥补了从视觉观测到交互式仿真场景之间的鸿沟，提升了具身智能仿真环境的质量。
...
### [175] [Beauty is in the ELBO of the Beholder: A Variational Account of Processing Fluency in Face Perception](https://arxiv.org/abs/2608.24219)
*Francisco M. López,Jochen Triesch*

Main category: cs.CV

TL;DR: 该研究通过变分自编码器（VAE）发现，人脸吸引力与模型的重构难度（ELBO）密切相关，支持了审美中的“加工流畅性理论”。


<details>
  <summary>Details</summary>
Motivation: 探讨面部吸引力是否源于审美加工流畅性理论，即人们认为具有统计规律（如对称、平均）的面孔更容易被感知，从而产生愉悦感。

Method: 在四个面部数据集上无监督训练变分自编码器（VAE），并在芝加哥面孔数据库（CFD）上评估其ELBO指标与人类吸引力评分的相关性。

Result: 人类吸引力评分与VAE的ELBO值高度一致；模型学习到的潜在空间包含一个可跨数据集迁移的“吸引力方向”；吸引力高的面孔在形状和潜在空间中表现出更强的原型性。

Conclusion: 研究证实了面部吸引力与生成模型的加工流畅性有关，从计算层面验证了审美快乐的变分解释理论。
...
### [176] [Example-based Robust Abnormality Detection with Minimal Annotations using Exemplar Med-DETR](https://arxiv.org/abs/2608.24281)
*Sheethal Bhat,Bogdan Georgescu,Awais Mansoor,Mathias Zinnen,Pranjal Sahu,Florin C. Ghesu,Sasa Grbic,Andreas Maier*

Main category: cs.CV

TL;DR: 该论文提出了一种基于EM-DETR的少样本医疗目标检测框架，用于胸部X光片（CXR）中的异常检测。


<details>
  <summary>Details</summary>
Motivation: 医疗图像领域缺乏高质量的文本关联数据，传统的少样本学习方法在处理未见过的病灶时泛化能力差且需要频繁重训，难以满足临床应用需求。

Method: 扩展了EM-DETR框架，引入了基于范例的特征生成和领域感知对比优化策略，实现了无需大量重训即可适应新疾病发现的目标。

Result: 在仅使用不到10%标注数据的情况下，实现了接近当前最先进（SOTA）水平的检测性能。

Conclusion: 该方法有效降低了医疗目标检测对标注数据的依赖，具备在临床环境中进行高效部署的潜力。
...
### [177] [Keep-or-Drop? Adaptive Tokenizer for Compact Video Representation](https://arxiv.org/abs/2608.24293)
*Yeonkyeong Lee,Hyunsung Go,Jongmin Kim,Sewoong Lim,Donghoon Lee*

Main category: cs.CV

TL;DR: KATok 是一种基于 Transformer 的视频 VAE，引入自适应 Token 选择器以实现基于内容的视频动态压缩。


<details>
  <summary>Details</summary>
Motivation: 传统 VAE 在处理视频时采用固定的压缩比例，无法应对时空内容复杂度变化的挑战，导致冗余计算或信息损失。

Method: 设计了一个自适应 Token 选择器，通过学习 Token 的重要性（keep-or-drop 概率）丢弃冗余信息；同时提出了级联和联合生成两种位置预测策略，以解决动态压缩带来的空间对齐问题。

Result: 该模型在保持高质量视频重建与生成的同时，达到了行业领先的压缩比，显著提升了处理效率。

Conclusion: 通过消除时空冗余和剔除无效 Token，KATok 为高效视频合成提供了更灵活且高保真的潜在空间表示方案。
...
### [178] [SeMoCo: A Semantic-First Motion Codec for Motion Language Modeling](https://arxiv.org/abs/2608.24334)
*Tianlv Huang,Hetian Guo,Ziyi Cai,Song Wang,Yanping Zhang,Zipei Fan,Xuan Song,Guangming Wu,Xin Zheng*

Main category: cs.CV

TL;DR: SeMoCo 是一种新型语义优先运动编解码器，通过将运动解构为语义与运动学残差，优化了动作生成质量与可控性。


<details>
  <summary>Details</summary>
Motivation: 现有运动分词器多侧重于重建精度，缺乏对动作语义和精细运动细节的明确分配，导致生成质量受限。

Method: 提出 SeMoCo 架构，采用“语义令牌+运动学残差令牌”的双轴表示，配合用于语言条件生成的新型生成器，并构建了大规模统一数据集 Ω-MotionVerse。

Result: SeMoCo 在重建精度上达到最优，且在文本驱动的运动生成任务中表现出显著的优越性。

Conclusion: 该方法通过解耦语义与运动学，有效提升了运动生成的语义连贯性与细节还原度，为高质量运动合成提供了新范式。
...
### [179] [Mind the Student: Behavioral and Contextual Cues for Automated Engagement Prediction in Online Learning](https://arxiv.org/abs/2608.24340)
*Alperen Kantarci,Visvanathan Ramesh,Gemma Roig*

Main category: cs.CV

TL;DR: 该研究提出了一个多模态框架，利用Perceiver IO和不确定性感知机制来预测在线学习场景中学生的多维参与度。


<details>
  <summary>Details</summary>
Motivation: 学生参与度具有多维度、高个体差异及标注主观性等挑战，且现有的CASED数据集极具难度，模型难以取得有效进展。

Method: 整合了预训练的视听特征与结构化行为模态（如头部姿态、凝视、面部动作单元），通过Perceiver IO架构融合特征，并利用变分后验建模个性差异，结合证据回归与高斯过程分类以实现不确定性量化。

Result: 在极具挑战性的CASED数据集上，该框架在保持竞争力的同时，有效提供了校准良好的不确定性指标，优于随机预测水平。

Conclusion: 对于真实教育场景的部署，量化模型预测的不确定性（风险评估）是实现可靠参与度监测的关键前提。
...
### [180] [Metadata-Aware Adaptation of a Generative Foundation Model for Conditional CMR Synthesis](https://arxiv.org/abs/2608.24342)
*Marc Rodríguez,Grzegorz Skorupko,Nay Aung,Steffen E Petersen,Karim Lekadir,Polyxeni Gkontra*

Main category: cs.CV

TL;DR: 该研究利用潜在扩散模型（LDM）通过临床元数据条件化生成心脏磁共振（CMR）图像，以解决医学影像数据稀缺及表型分布不平衡的问题。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像生成中难以忠实反映患者临床特征的问题，并改善训练数据中临床属性的分布不平衡。

Method: 引入元数据无关的无分类器指导（Metadata-Free CFG）、对比分批（Contrastive Batching）和逆频率采样（Inverse-Frequency Sampling）策略，对预训练的LDM进行微调。

Result: FID达到37.47，较基准模型提升了57.04%，且在仅依赖元数据的情况下超过了需要额外几何输入的 baseline，显著提升了分布保真度。

Conclusion: 该框架证明了利用生成式基础模型进行临床有效CMR合成的潜力，但同时指出在复杂的疾病特异性条件化方面仍需进一步研究。
...
### [181] [B-MIM: Biased Masked Image Modeling for Generalizable Segmentation of Fine-Grained Anatomical Structures](https://arxiv.org/abs/2608.24364)
*Sebastián González,Karen Sanchez,José M. Saavedra,Marcelo Pizarro,Bernard Ghanem*

Main category: cs.CV

TL;DR: 提出了一种名为B-MIM的自监督预训练方法，通过降低全局语义对齐压力，增强模型对CT影像中精细解剖结构（如血管、微小肿瘤）的特征提取能力。


<details>
  <summary>Details</summary>
Motivation: 现有CT编码器偏向于粗粒度的语义理解，导致其对精细解剖结构的敏感性不足，限制了在复杂医学分割任务中的表现。

Method: 改进iBOT目标函数，通过引入“有偏遮蔽图像建模（B-MIM）”，随机降低全局语义对齐权重，重点加强局部斑块（patch）的重建，并在多机构收集的9,955例腹部CT数据集上预训练3D Swin Transformer。

Result: 在肝脏血管分割任务中，该方法显著提升了拓扑保真度（clDice），并在肿瘤分割任务中取得了与全参数微调基线相当的Dice分数，同时大幅减少了需更新的参数量。

Conclusion: 在预训练阶段减少全局语义压力，能够有效提升编码器对复杂精细解剖结构的捕捉能力，从而增强模型在医学影像任务中的泛化性。
...
### [182] [MaST: Motion-aware Sparse Pipeline for Lightweight Object Tracking](https://arxiv.org/abs/2608.24365)
*Qingmao Wei,Fagui Liu,Dengke Zhang,Qingze He,Quan Tang*

Main category: cs.CV

TL;DR: MaST 是一种旨在提升边缘设备上实时跟踪性能的高效稀疏 Transformer 跟踪框架。


<details>
  <summary>Details</summary>
Motivation: 现有 Transformer 跟踪器存在密集的 token 处理导致计算成本高的问题，且现有的剪枝方法通常缺乏端到端的稀疏性支持，导致计算资源浪费。

Method: 提出了 MaST 框架：一是引入轻量级运动先验来优化交叉注意力的 token 重要性评分；二是设计了原生稀疏预测头，直接在非结构化 token 上执行预测，消除了填充和重塑需求。

Result: 在 LaSOT 和 TrackingNet 上达到 SOTA 水平，同时在 Jetson Nano 上以 152 FPS 的速度运行，速度较现有最优方法提升了近两倍。

Conclusion: 通过将运动先验与稀疏预测头相结合，MaST 有效实现了从 token 到预测的全链路稀疏化，显著提升了资源受限设备上的跟踪效率与精度。
...
### [183] [Bridging Adversarial and Collaborative Learning for AI-Generated Image Quality Assessment](https://arxiv.org/abs/2608.24372)
*Baoliang Chen,Qing Lin,Sijie Mai*

Main category: cs.CV

TL;DR: 该研究提出了一种名为ACL-IQA的AI生成图像质量评估框架，旨在通过建模感知保真度和提示对齐度之间的交互关系来更准确地评估图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有模型通常将感知保真度和提示对齐度视为相互独立的维度，但研究发现人类评分中这两者存在复杂的竞争与合作交互，传统模型难以捕捉这种相互依赖性。

Method: 提出了一种交互感知学习框架，包含对抗与协作推理路径，并使用门控交互模块根据维度关系动态路由特征，配合任务感知提示调节门控行为。

Result: 实验表明该方法在多个AIGIQA基准测试中达到了最先进的准确度，并表现出良好的可解释性，更符合人类的评分逻辑。

Conclusion: 通过自适应地模拟质量维度间的交互，可以更有效地评估AI生成图像的质量，克服了传统方法对维度处理过于僵化的问题。
...
### [184] [Markerless Pose Estimation for Resistance Training Technique Assessment](https://arxiv.org/abs/2608.24384)
*Joseph Turner,Jeff Clark,Nawid Keshtmand*

Main category: cs.CV

TL;DR: 该研究提出了一种利用无标记姿态估计技术（BlazePose）从普通视频中量化评估抗阻训练动作（如下蹲、卧推、硬拉）的框架。


<details>
  <summary>Details</summary>
Motivation: 实验室环境下的动作分析虽然精确但难以普及，抗阻训练中为了避免伤病需要一种便捷的动作评估技术。

Method: 利用BlazePose提取视频中的解剖学标志点，将其转化为关节角度轨迹，并通过均方根误差（RMSE）与参考动作进行定量比对。

Result: 该框架能有效恢复下蹲和硬拉的运动学模式，识别组内动作差异，但性能受相机视角（非矢状面导致畸变）和遮挡情况的影响显著。

Conclusion: 无标记姿态估计技术为实验室外的生物力学评估提供了一种可行且易于获得的替代方案。
...
### [185] [MRI-based Deep Radiomic Phenotyping of Neuromuscular Disorders: A Topology-driven Characterization](https://arxiv.org/abs/2608.24415)
*Martyna Żur,Łukasz Piórecki,Marek Socha,Jordi Diaz-Manera,Jose Verdu Diaz,Volker Straub,Rossella Tupler,Joanna Polańska*

Main category: cs.CV

TL;DR: 该研究提出了一种名为MUSCAT的自动放射组学框架，通过分析肌肉MRI中的3D拓扑结构和空间分布特征，量化神经肌肉疾病（NMD）中的脂肪浸润，从而提供比传统体积指标更精确的生物标志物。


<details>
  <summary>Details</summary>
Motivation: 传统的NMD监测主要依赖于空间不敏感的脂肪体积指标，无法捕捉肌肉内部复杂的脂肪浸润空间结构和拓扑演变，限制了诊断的准确性和疾病进展的追踪。

Method: 利用1184例MRI扫描，构建包含定量形态测量、空间分布、几何形状、脂肪替代阶段交互及基于图论的拓扑结构五大领域的放射组学特征库，并开发了图论骨架化算法以量化肌肉构象变化。

Result: 拓扑网络指标（如SF1_Skel_Nodes）和界面动力学指标显示出显著的判别力（ε² > 0.2），且优于传统的体积评估方法，能有效捕捉疾病特异性的宏观浸润模式。

Conclusion: 基于全球架构的放射组学特征是评估NMD诊断和追踪疾病进展的有效生物标志物，通过MUSCAT库可实现自动化的定量诊断分析。
...
### [186] [Vision Language Model Fusion for Explainable Face Recognition](https://arxiv.org/abs/2608.24430)
*Ana Estrada-Real,Lydia Alapatt,Christoph Busch,Christian Rathgeb*

Main category: cs.CV

TL;DR: 该论文提出了一种利用多视觉语言模型（VLM）融合来提升人脸验证准确度并提供更具解释性、可审计证据的方法。


<details>
  <summary>Details</summary>
Motivation: 单一模型在人脸识别的准确性与解释性上存在局限，为实现负责任且可审计的人脸验证，需要多模型协同提供更丰富、更透明的决策依据。

Method: 构建了一个多VLM融合框架：使用两个源模型生成相似度评分与文本解释，由第三个VLM作为决策模型，并对比了融合评分、解释、人脸图像等多种模态组合的效果。

Result: 多模型融合不仅提升了人脸识别的准确率，超过了现有的单模型和领域专用模型，还通过综合多维度的理由生成了更稳健、更具洞察力的解释。

Conclusion: 多VLM融合是提升人脸识别系统透明度与审计能力的有效途径，为构建负责任的解释性人脸识别系统提供了技术支持。
...
### [187] [ZODIAC: Zero-shot Octree-based Diffusion for Anatomical Completion](https://arxiv.org/abs/2608.24422)
*Miruna-Alexandra Gafencu,Vlad Bratulescu,Yordanka Velikova,Mohammad Farid Azampour,Nassir Navab*

Main category: cs.CV

TL;DR: 提出了一种名为ZODIAC的零样本（zero-shot）形状补全框架，利用生成式扩散模型和自适应八叉树结构，从不完整且有噪声的术中超声图像中重建完整的腰椎三维解剖结构。


<details>
  <summary>Details</summary>
Motivation: 现有的监督学习方法依赖于模拟遮挡数据，难以推广到真实的术中场景；同时超声成像存在严重的声学遮挡和有限视野，导致重建该任务成为一个病态反问题。

Method: 基于生成式扩散模型学习完整解剖结构的先验，并引入‘混合补全（blended completion）’机制，在推理阶段将学习到的解剖先验与观测到的部分几何结构进行集成，无需预先定义的遮挡模型。

Result: 在模型和志愿者数据集上的验证显示，该方法在真实遮挡情况下的泛化能力优于监督学习方法，HD95补全误差降低了22%。

Conclusion: 通过将补全任务与预定义的腐蚀分布解耦，该框架实现了更稳健的脊柱三维结构重建，证实了零样本学习在处理复杂医学影像缺失问题上的有效性。
...
### [188] [DoublesEval: Diagnosing Multi-Agent Tactical Reasoning in Vision-Language Models via Professional Doubles Badminton](https://arxiv.org/abs/2608.24439)
*Jintao Cheng,Weibin Li*

Main category: cs.CV

TL;DR: 本文提出了一个名为DoublesEval的评估框架，专门用于测试视觉语言模型（VLM）在处理复杂动态多智能体交互（以羽毛球双打为例）时的战术推理能力，并引入了TacticCheck方法来提升推理一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在描述静态场景内容方面表现优异，但在需要空间-时间依赖性和角色协调的动态多智能体交互推理任务上表现较弱。

Method: 构建了DoublesEval基准，将比赛片段拆解为四个关键维度进行诊断评估；提出了TacticCheck，这是一种无需参数更新的测试时一致性检查机制，通过利用模型自身的低级战术预测来重排候选答案。

Result: 评估显示现有开源VLM在空间状态感知、交互绑定及终端证据推理方面存在明显瓶颈；TacticCheck能在不进行微调的情况下一致性地提升模型的推理表现。

Conclusion: 该研究揭示了当前VLM在处理复杂战术推理时的局限性，强调了开发结构化、交互感知型评估范式对下一代VLM的重要性。
...
### [189] [Low-Rank Ternary Adaptation for Fine-Tuning Transformers](https://arxiv.org/abs/2608.24469)
*Alexandru-Dragos Manolache,Yunqiang Li,Jan van Gemert*

Main category: cs.CV

TL;DR: 提出了一种针对三值（ternary）Transformer的高效微调方法，通过三值乘法适应（Ternary Multiplicative Adaptation）实现无需反量化的模型微调。


<details>
  <summary>Details</summary>
Motivation: 现有低比特LoRA方法无法直接处理三值权重，要么需要反量化到高精度，要么无法合并为纯三值模型，导致计算效率低下。

Method: 利用低秩Kronecker分解，将离散权重更新（如符号翻转或置零）表示为两个小三值矩阵的元素级乘积，从而在保持三值特性的同时实现微调。

Result: 在LLaMA-3（1B/3B）和ViT-B/16等模型上实验，证明该方法能显著恢复量化损失的性能，且优于现有的低比特和三值基准方法。

Conclusion: 该方法通过保留三值域和直接合并权重的特性，解决了三值模型微调中的精度与效率矛盾，为极低比特模型部署提供了高效方案。
...
### [190] [VizAnchor: Decoding Manipulation Intent from Tampering Visualizations via Dual-Anchor Reasoning](https://arxiv.org/abs/2608.24535)
*Xiaotian Zhang,Huayuan Ye,Haiyang Zhang,Chenhui Li,Changbo Wang,Sicheng Song*

Main category: cs.CV

TL;DR: VizAnchor 是一个通过双锚点证据构建和视觉语言模型（VLM）推理，自动识别并解释数据可视化操纵行为及诱导动机的框架。


<details>
  <summary>Details</summary>
Motivation: 现有的方法仅能定位篡改区域或恢复信息，缺乏对“如何操纵”以及“为何误导”的深层语义解释，无法帮助用户识别隐藏的误导性意图。

Method: 构建包含语义锚点（恢复真实信息）和空间锚点（定位篡改区域）的框架；利用三个专业代理分别进行误导信息定位、图表叙事重构以及误导意图推理。

Result: VizAnchor 在篡改定位及误导意图推理任务上表现准确，能生成对操纵手段、误导要素及诱导目的的忠实解释。

Conclusion: 该研究通过多代理协同与VLM推理，有效提升了机器对可视化欺骗行为的理解深度，填补了可视化审计领域在意图解释方面的空白。
...
### [191] [Hierarchical Prototype-Memory Adaptation of SAM for Surgical Instrument Segmentation](https://arxiv.org/abs/2608.24541)
*Xinning Yao,Jingjing Wang,Jinghua Yue,Xiaoyan Luo,Fugen Zhou,Bo Liu*

Main category: cs.CV

TL;DR: 提出了一种名为HPMA的框架，通过多尺度原型记忆库和尺度匹配耦合机制，提升了SAM模型在手术器械分割任务中的鲁棒性与性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于提示学习的SAM微调方法存在参数退化问题，且单路径提示处理多尺度视觉信息导致特征耦合效率低下，难以应对复杂的手术环境。

Method: 构建了冻结的多尺度视觉原型记忆库，并引入轻量级适配器；提出了尺度匹配耦合机制，通过全局、结构和局部三个层级对提示特征、解码器查询及特征图进行对齐与校准。

Result: 在EndoVis2017和EndoVis2018数据集上进行了广泛实验，结果显示该方法优于现有的基础模型适配方法，达到了领域内最先进水平。

Conclusion: HPMA通过显式的多尺度原型记忆和层次化对齐，有效解决了SAM在手术分割中的适应性问题，增强了模型对复杂手术场景的理解能力。
...
### [192] [KLTNet: Learning Sparse Feature Tracking for Robust and Accurate Monocular Visual-Inertial Odometry](https://arxiv.org/abs/2608.24544)
*Renbiao Jin,Danping Zou,Wenxian Yu*

Main category: cs.CV

TL;DR: KLTNet 是一种轻量级学习型视觉特征跟踪器，旨在替代 VIO 系统中的传统 KLT 跟踪器，以提升低纹理或快速运动场景下的跟踪鲁棒性和精度。


<details>
  <summary>Details</summary>
Motivation: 传统 KLT 跟踪器在快速运动和弱纹理环境下表现不稳定，直接限制了 VIO 系统的精度与鲁棒性。

Method: 采用“由粗到细、由稠到稀”的架构，结合低分辨率密集光流进行全局运动初始化，并利用三元补丁（triplet-patch）细化跟踪，引入固定参考补丁减少漂移，同时通过多视图三角化监督预测各向异性的置信度权重。

Result: 在 VINS-Mono 和 OpenVINS 等框架下，KLTNet 在公共基准和弱纹理数据集上表现优于传统 KLT，且在嵌入式设备上保持了实时性。

Conclusion: KLTNet 提供了一种即插即用的高性能替代方案，通过深度学习优化特征跟踪，有效增强了 VIO 系统的前端鲁棒性。
...
### [193] [Human-Inspired Social Engagement Analysis via Interpretable Mutual Visual Attention](https://arxiv.org/abs/2608.24580)
*Urwa Fatima,Mohammad Zohaib,Francesca Odone,Nicoletta Noceti*

Main category: cs.CV

TL;DR: 提出了一种基于心理学互视理论的社会参与度计算模型，通过建模双人视觉注意力来分析群体互动。


<details>
  <summary>Details</summary>
Motivation: 旨在解决非言语视觉数据中社会互动分析的解释性问题，并为教师、护理人员等提供可解释的辅助工具。

Method: 结合最先进的头部朝向估计与轻量级几何推理，显式建模双人视觉注意力，并将这些线索聚合成个人及群体的参与度指标。

Result: 通过定量实验验证了方法的有效性，并提供了直观的可视化工具以展示群体互动动态。

Conclusion: 该模块化框架既具有计算效率，又具备高可解释性，能有效支持特定领域（如教育、医疗）中的社交互动分析。
...
### [194] [Comparative Assessment of Deep Learning Architectures for Underwater Subsurface Kelp Forest Segmentation with The Kelp-o-Tron](https://arxiv.org/abs/2608.24594)
...
TL;DR: 该研究评估了三种深度学习架构在水下海带分割任务中的表现，并提出了名为“Kelp-O-Tron”的最优模型。


<details>
  <summary>Details</summary>
Motivation: 由于光照变化、浑浊度及复杂海底背景等因素，水下海带的精准分割具有挑战性，需要鲁棒性强的自动化监测方案。

Method: 构建了一个包含3,395对标注图像的数据集，并对比了ResNet34-U-Net、ResNet50-DeepLabV3及ResNet50-ASPP-Transformer三种架构。

Result: ResNet50-DeepLabV3在Dice（0.7120）和IoU（0.6267）指标上表现最佳，且在不同环境条件下表现出最强的鲁棒性和泛化能力。

Conclusion: ResNet50-DeepLabV3（Kelp-O-Tron）是目前水下海带监测中最优的分割模型，为栖息地测绘提供了有效的技术支持。
...
### [195] [On-Policy Self-Distillation in Diffusion Models](https://arxiv.org/abs/2608.24646)
...
TL;DR: DiffusionOPSD 是一种通过将图像级奖励转化为中间去噪预测的明确目标，实现扩散模型对齐的高效自蒸馏框架。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习对齐方法缺乏对中间去噪步骤的明确指导，且传统端到端奖励优化效率较低。

Method: 引入自蒸馏机制，利用冻结的策略生成轨迹与锚点，通过奖励梯度构建有界的目标正负样本，并采用有限拟合更新可训练策略，最后通过指数移动平均刷新策略。

Result: 在 SD 3.5-M 和 Z-Image-Turbo 上表现优于对比方法，最高提升 44.0%，训练算力需求显著降低 40%-63%。

Conclusion: 该方法证明了将全局奖励转化为持续刷新的中间监督是扩散模型对齐的高效且可诊断路径。
...
### [196] [Towards Reliable AI-Based Histological Staining: A Systematic Study of Scaling and Uncertainty in Unpaired Generative Models](https://arxiv.org/abs/2608.24626)
*Qasim Siddiqui,Adrian Friebel,Maiju Myllys,Zaynab Hobloss,Daniela Gonzalez,Ahmed Ghallab,Stefan Hoehme*

Main category: cs.CV

TL;DR: 该研究评估了六种无监督图像翻译架构，用于将H&E染色切片虚拟转化为Sirius Red染色以量化肝纤维化，并首次引入了对模型认知不确定性的量化评估。


<details>
  <summary>Details</summary>
Motivation: Sirius Red染色虽是肝纤维化评估标准，但耗时耗力且非临床常规。AI虚拟染色可从H&E生成SR，但目前缺乏系统性基准测试，且生成的图像往往缺乏真实性与不确定性量化。

Method: 在公开的配对小鼠肝脏数据集上，对六种基于GAN和扩散模型的无监督架构进行54种配置的基准测试，并引入深层集成学习（Deep Ensemble）来量化认知不确定性。

Result: 感知质量、任务特定误差与集成一致性是三个独立的评价维度；GAN模型在感知度量上表现一致但任务误差差异大，而扩散模型（CycleDiffusion）在各维度表现独特。

Conclusion: 单一指标无法全面反映虚拟染色质量，建议在未来研究中联合报告感知质量、任务误差及不确定性度量，以确保临床评估的可靠性。
...
### [197] [ReGround-Surg: Reliability-Guided Anchor Grounding for Referring Surgical Video Segmentation](https://arxiv.org/abs/2608.24671)
*Jiaxin Wen,Ming Yin,Lu Liu,Zeyu Fu*

Main category: cs.CV

TL;DR: ReGround-Surg 引入了一种可靠性引导的锚点定位框架，解决了基于 SAM2 的外科视频分割中初始掩码错误传播的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度依赖初始帧的定位质量，而在外科视频中，相似器械、遮挡及复杂交互容易导致初始锚点选择错误并引发后续追踪失效。

Method: 提出了一个轻量级的可靠性引导模块，通过预测空间可靠性图，结合门控旁路适配器增强视觉特征，并利用可靠性加权的视觉-文本注意力机制抑制干扰信息。

Result: 在 Ref-EndoVis17 和 Ref-EndoVis18 数据集上，该方法在保持高效速度的同时，在多个评估指标上均优于现有的先进模型。

Conclusion: 该研究通过引入可靠性感知机制，有效提升了复杂外科场景下参考式视频分割的鲁棒性与精度。
...
### [198] [TurboT2VA: Fast Large-Scale Text-to-Video-Audio Generation via Score-Regularized Consistency Distillation](https://arxiv.org/abs/2608.24674)
...
TL;DR: TurboT2VA 是一个针对大规模文本生成视频-音频（T2VA）模型的蒸馏与推理加速框架，通过渐进式一致性训练和架构感知推理堆栈，实现了大幅度的推理提速。


<details>
  <summary>Details</summary>
Motivation: 大规模联合视频-音频生成模型计算成本极高，推理延迟严重限制了其实际应用，传统的蒸馏方法难以解决模态不平衡与质量多样性的权衡问题。

Method: 提出包含分模态归一化和渐进式课程（离散一致性预热、连续一致性微调、联合一致性分布匹配）的蒸馏方法；开发了包含 W8A8 量化、算子融合及模态感知稀疏注意力机制的架构感知推理堆栈。

Result: 在 LTX-2 模型上，标准分辨率实现 20.1 倍加速，高分辨率（1024x1792）下实现 54.67 倍加速，同时保持了高质量的生成效果及视听同步性。

Conclusion: TurboT2VA 通过算法层面的蒸馏优化与工程层面的推理算子加速，有效解决了大参数量视听生成模型的实时部署瓶颈。
...
### [199] [Game2World Engine: Unlocking In-the-Wild Gameplay Videos for World Model Training](https://arxiv.org/abs/2608.24680)
*Wenxuan Shen,Dongna Jin,Dongping Chen*

Main category: cs.CV

TL;DR: 本文提出了一套名为Game2World的框架，旨在从电子游戏视频中去除游戏界面（UI），从而为训练高质量视频世界模型提供更干净的数据。


<details>
  <summary>Details</summary>
Motivation: 原始游戏视频中的UI元素（如血条、地图）会引入特定于游戏的偏置和无关动态，阻碍世界模型的训练。

Method: 构建了GameUI-Taxonomy和G2WEngine用于UI检测与合成，并提出了GameCleaner模型。GameCleaner无需掩码，利用多模态语义理解能力直接去除UI，同时保留视频背景和时序连贯性。

Result: GameCleaner在合成数据集上表现出色，且在真实游戏视频中取得了最优的背景保留效果。实验证明，使用去除UI的数据训练出的世界模型，其视频奖励（VideoReward）显著提升了6.83%。

Conclusion: 该研究成功将包含UI的游戏视频转化为高质量的世界模型训练数据，为扩展世界模型训练提供了有效的路径。
...
### [200] [Deep Learning Super Resolution for Satellite Cloud Mask Downscaling](https://arxiv.org/abs/2608.24715)
...
TL;DR: 提出了一种基于深度学习的卫星云图超分辨率重构方法，利用CNN和GAN实现SEVIRI数据到MODIS精度的下采样增强。


<details>
  <summary>Details</summary>
Motivation: 光学卫星数据常受云雾干扰，且存在空间与时间分辨率的权衡限制，难以获取连续的高分辨率云图观测。

Method: 构建跨传感器数据集SEVMOD-CM，并提出SpatialCNN和SpatialGAN两种模型，实现SEVIRI云掩膜产品4倍空间分辨率提升。

Result: 模型在跨传感器领域实现了有效的超分辨率重构，在视觉和定量指标上均显著优于传统的双三次插值方法。

Conclusion: 该方法为大气监测、天气预报及气候研究提供了高质量云图数据支持，验证了超分辨率技术在地球静止轨道卫星遥感应用中的潜力。
...
### [201] [Interpretable Fundus Image Classification via Ring-Based Retinal Vasculature Features](https://arxiv.org/abs/2608.24723)
*Xiaoyan Li,Shixin Xu,Arvind Gupta,Huaxiong Huang*

Main category: cs.CV

TL;DR: 提出了一种基于视网膜血管环状结构特征的可解释性眼底图像分类框架，通过量化血管几何、颜色和氧合信息，无需大规模预训练即可实现高性能分类。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的眼底图像分类方法通常依赖不可解释的隐向量，缺乏生理学依据且对空间伪影敏感。

Method: 构建以视盘为中心的环状血管表示，提取血管几何、颜色、氧合及熵等生理特征，并聚合这些定量描述符进行分类。

Result: 在HRF等公开数据集上表现优异，准确率达91.1%，与大规模预训练模型RETFound相当，且证明了预训练模型可能过度依赖获取伪影而非血管特征。

Conclusion: 该方法实现了高可解释性的疾病分类和生物标志物发现，且在小样本场景下具有更强的鲁棒性和生理临床意义。
...
### [202] [TorchMorph: CUDA-accelerated Morphological Transforms](https://arxiv.org/abs/2608.24738)
*Kai Zhao*

Main category: cs.CV

TL;DR: TorchMorph 是一个轻量级的 PyTorch 扩展，旨在为 GPU 训练循环提供高性能、多维度的数学形态学算子。


<details>
  <summary>Details</summary>
Motivation: 现有的 CPU 形态学工具（如 scipy.ndimage）在深度学习管线中会导致昂贵的设备间数据传输，且现有 PyTorch 库功能受限，难以处理高维数据或复杂算子。

Method: 利用融合的 CUDA 内核实现 22 种形态学算子，支持高达 8 维空间及批处理，API 设计完全兼容 scipy.ndimage。

Result: 在性能上比 CPU 实现快 42 到 1100 倍，并保持了与 SciPy 一致的数值精度。

Conclusion: TorchMorph 为深度学习社区提供了一个高效、易于集成且支持复杂形态学操作的 GPU 加速解决方案。
...
### [203] [Weakly Supervised Seafloor Segmentation for Seagrass Habitat Mapping in Side-Scan Sonar Imagery](https://arxiv.org/abs/2608.24756)
*Hayat Rajani,Nuno Gracias,Rafael Garcia*

Main category: cs.CV

TL;DR: 提出了一种基于弱监督语义分割框架的侧扫声纳（SSS）海草栖息地自动绘图方法，仅需图像级标签即可实现高精度像素级分割。


<details>
  <summary>Details</summary>
Motivation: 侧扫声纳数据手动标注成本高、速度慢，传统光学遥感无法覆盖深水或浑浊水域，亟需一种高效且自动化的绘图方案。

Method: 结合ViT编码器-解码器与分类分支，利用类激活映射（CAM）生成伪标签，并通过针对声纳噪声优化的密集条件随机场（CRF）进行精炼，结合迭代自训练、重采样策略及自监督预训练。

Result: 在仅使用图像级标签的情况下，模型在测试集上达到了87.6%的mIoU，自监督预训练进一步提升了3%的准确率。

Conclusion: 该方法证明了在不需要像素级标签的情况下，利用侧扫声纳进行大规模、高精度海草栖息地监测是切实可行的。
...
### [204] [IDeaL: Data-Free Multi-Teacher Distillation via Improved Dead Leaves](https://arxiv.org/abs/2608.24759)
*Feyza Yavuz,Mert Bülent Sarıyıldız,Diane Larlus*

Main category: cs.CV

TL;DR: 提出了一种名为IDeaL的无数据多教师蒸馏方法，通过生成优化后的去相关噪声样本来替代真实训练数据。


<details>
  <summary>Details</summary>
Motivation: 传统多教师蒸馏通常依赖真实数据集，而该研究旨在解决数据不可用或隐私受限情况下的蒸馏问题。

Method: 引入了基于图像级和补丁级去相关损失的优化方法，生成能够捕获不同教师互补特征的针对性噪声样本。

Result: 实验表明IDeaL生成的样本能有效利用多教师信息，其蒸馏效果接近真实数据训练，甚至在限制1K样本量时超越了真实ImageNet子集的效果。

Conclusion: 该方法证明了在无需真实数据的情况下，通过优化生成样本即可实现高效的多教师知识迁移，弥合了无数据蒸馏与真实数据蒸馏的性能差距。
...
### [205] [MoTE: Mixture of Task Experts for Multi-Task Video Understanding](https://arxiv.org/abs/2608.24763)
*Muhammad Asad Ali,Umar Khan,Nadia Robertini,Didier Stricker*

Main category: cs.CV

TL;DR: 提出了一种名为MoTE（Mixture of Task Experts）的视频语言模型架构，通过将LLM的FFN层转化为任务特定的专家模块，实现高效的多任务视频处理。


<details>
  <summary>Details</summary>
Motivation: 现有的密集Transformer解码器在处理异构任务时存在任务行为纠缠问题，而基于token的稀疏MoE方法难以与复杂的程序性任务目标对齐。

Method: 引入MoTE架构，为每个程序性任务分配特定的专家路径，保持多模态骨干网络共享，并采用样本级任务路由进行计算。

Result: VideoLLM-MoTE在COIN基准测试中表现优异，仅激活约2B参数即超越了多个VideoLLM基线模型，且比传统的密集激活或基于token的稀疏路由更具效率。

Conclusion: 任务结构化的路由机制为多任务视频语言学习提供了一种可解释且计算高效的解码器设计方案。
...
### [206] [Ensemble of Convolutional Neural Networks for StrokePrediction: Towards Improved Diagnostic Accuracy](https://arxiv.org/abs/2608.24771)
*Md Shahriar Sajid*

Main category: cs.CV

TL;DR: 本研究利用机器学习算法对十一种特征进行分析，旨在实现脑卒中的早期预测与诊断。


<details>
  <summary>Details</summary>
Motivation: 脑卒中具有高致死率和高致残率，通过早期诊断和预防措施可以显著降低健康风险。

Method: 使用七种监督学习算法（包括随机森林、堆叠分类器、装袋分类器、决策树、KNN、TabNet、SVM、逻辑回归及自定义前馈神经网络）进行模型训练与对比。

Result: 集成学习方法（随机森林、堆叠、装袋）表现最优，准确率高达99.52%，显著优于单一模型。

Conclusion: 集成学习算法在脑卒中预测任务中表现出卓越的性能，证明了其在辅助临床诊断中的有效性。
...
### [207] [Image Difference Quantification Using Autoencoder-Based Latent Representations](https://arxiv.org/abs/2608.24782)
*Manish Sharma,Timothy Yim,Clifton Forlines*

Main category: cs.CV

TL;DR: 提出了一种基于卷积自编码器的潜在空间余弦相似度方法，用于量化图像间的语义相似性。


<details>
  <summary>Details</summary>
Motivation: 传统像素级度量（如MSE、PSNR、SSIM）无法有效捕捉符合人类视觉认知的语义差异。

Method: 利用卷积自编码器提取图像的潜在表征，通过计算潜在空间中的余弦相似度来度量图像差异。

Result: 该模型在猫狗数据集上实现了良好的类间可分性（98.4%的样本相似度低于0.5），且在TID2013数据集上的表现与人类平均意见分数（MOS）呈正相关。

Conclusion: 该方法提供了一种计算高效且语义鲁棒的图像相似度量方案，在内容检索和感知质量评估中具有应用潜力。
...
### [208] [MoE-based Feature Adapter for Prompt-free Binary Coronary Artery Segmentation in X-ray Angiography](https://arxiv.org/abs/2608.24783)
*Lin Xi,Yingliang Ma*

Main category: cs.CV

TL;DR: 本文提出了一种基于专家混合（MoE）特征适配器的无提示（prompt-free）方法，用于X射线血管造影视频中的冠状动脉分割。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉血管细窄、对比度低，且常受导管、导丝及复杂背景干扰，现有U-Net及Transformer模型在处理异构血管表现时特征适配能力不足。

Method: 基于参数高效的Vision Transformer适配器，引入多个轻量级专家模型，并通过输入依赖的top-k路由机制自适应优化血管特征，同时控制计算成本。

Result: 在MOSXAV数据集上表现优于现有主流基线模型，并在XACV外部数据集上展现了更强的跨数据集泛化能力。

Conclusion: 基于MoE的适配器学习策略能有效提升X射线血管造影视频中冠状动脉分割的鲁棒性。
...
### [209] [EMFE: A lightweight, explainable machine learning framework for malaria cell classification](https://arxiv.org/abs/2608.24793)
*Md Abdullah Al Kafi,Walayat Hussain,Mousumi Karmakar,Sumit Kumar Banshal,Ahmed Al Marouf*

Main category: cs.CV

TL;DR: 研究提出了一种高效的数学特征提取（EMFE）框架，用于疟疾红细胞显微图像分类，作为深度学习的轻量级、可解释替代方案。


<details>
  <summary>Details</summary>
Motivation: 深度卷积神经网络在疟疾诊断中虽准确但计算成本高、可解释性差，且缺乏患者层面的严格验证。

Method: 结合灰度世界色彩归一化、自适应绿通道阈值处理、形态学斑点检测及传统机器学习（随机森林等），并在患者分组嵌套交叉验证下进行评估。

Result: 随机森林模型达到94.6%的准确率，通过40名患者的留出测试验证，并在计算效率和可解释性上优于DenseNet121、ResNet50等深度模型。

Conclusion: EMFE框架为资源受限环境下的疟疾诊断提供了一种统计严谨、高效且可解释的临床辅助工具。
...
### [210] [LAION-BVD: A 10-Million-Hour Open Video Dataset for Multimodal Pre-training](https://arxiv.org/abs/2608.24845)
...
TL;DR: LAION-BVD是一个大规模开放视频数据集，包含1.3B视频URL，成功下载80M视频，用于视频、音频和图像的多模态预训练。


<details>
  <summary>Details</summary>
Motivation: 弥补当前多模态学习领域对大规模开放视频数据集的需求，旨在通过海量数据提升视频-文本和音频-文本模型的性能。

Method: 利用CommonCrawl收集URL，通过内容感知场景检测提取片段，使用合成方法生成视频和音频字幕，并提取关键帧用于图像-文本预训练。

Result: 模型在标准视频/音频-文本基准测试中表现出色，且随着训练规模和模型参数的增加，性能持续提升；提取的关键帧在图像-文本检索中表现强劲。

Conclusion: LAION-BVD为研究社区提供了前所未有的大规模开放多模态视频数据，有效促进了多模态学习的扩展与研究。
...
### [211] [LeFlow: Generative Latent Flow Planning for World Models](https://arxiv.org/abs/2608.24855)
*Hsiang-Wei Huang,Jianxu Shangguan,Junbin Lu,Jenq-Neng Hwang*

Main category: cs.CV

TL;DR: LeFlow提出了一种将潜空间轨迹规划转化为条件生成任务的方法，通过学习可重用的潜轨迹先验来取代传统的在线迭代优化，从而实现更高效的规划。


<details>
  <summary>Details</summary>
Motivation: 现有的世界模型在规划时依赖从头开始的迭代优化，计算成本高且无法复用历史规划经验，作者旨在实现规划过程的“摊销化”（amortization）。

Method: 利用 rectified-flow 模型在潜动力空间生成连接当前状态与目标的轨迹，并通过逆动力学解码器将其转换为动作块，最后由冻结的世界模型进行自动回归验证。

Result: 在四个目标导向像素控制基准测试中，LeFlow 实现了更高的成功率，同时将规划时间降低了一个数量级。

Conclusion: 潜世界模型不仅应具备预测能力，还应支持可重用的规划先验，通过摊销规划能显著提升决策效率。
...
### [212] [From Seeing to Acting: Smart Glasses as First-Person Intelligence Platforms](https://arxiv.org/abs/2608.24877)
*Jiangning Zhang,Haojun Chen,Yong Liu*

Main category: cs.CV

TL;DR: 该论文是首个针对智能眼镜（第一人称智能平台）进行系统性研究的综述，提出了统一的架构框架以应对多模态感知、任务执行及系统可靠性挑战。


<details>
  <summary>Details</summary>
Motivation: 现有研究在设备、任务和基准测试方面过于碎片化，缺乏一个能整合感知、状态维护、交互与行动的完整系统框架。

Method: 提出了一个包含七项基础能力和L0-L5等级的分类框架，构建了涵盖九个应用场景的评估协议，并从硬件能力轴和部署维度对智能眼镜进行了系统化评估。

Result: 建立了针对智能眼镜的标准化评估指标、九维部署框架及从受控测量到长期实地验证的证据阶梯，解决了领域内评估不一致的问题。

Conclusion: 该框架为智能眼镜的对比研究、可部署性评估及通往可信第一人称智能的路线图提供了理论基础和实践指引。
...
### [213] [Taming Visual Neglect: A Variational Information Bottleneck Framework for Adaptive Attention in Multimodal In-Context Learning](https://arxiv.org/abs/2608.23570)
*Kaito Tanaka,Yuji Nishimura,Keisuke Matsuda,Aya Nakayama*

Main category: cs.CL

TL;DR: 该研究提出了 VIB-ICL 框架，利用信息瓶颈理论量化视觉语境在多模态上下文学习中的价值，揭示了模型为何有时依赖、有时忽略视觉信息。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉-语言模型在上下文学习（ICL）中存在视觉语境利用的二分现象，即有时有效利用视觉演示，有时却完全忽略，缺乏理论解释。

Method: 引入跨模态信息增益（CMIG）量化视觉语境的非冗余信息；基于信息瓶颈（IB）原理推导泛化界；提出 VIB-ICL 算法，通过变分界估计 CMIG 并动态调整注意力权重。

Result: 理论证明了当视觉信息非冗余时，多模态 ICL 优于纯文本 ICL；实验表明 VIB-ICL 在五个基准测试上准确率提升高达 4.7%，演示需求减少 35%。

Conclusion: 模型忽略视觉语境是冗余信息下的最优解，VIB-ICL 通过动态注意力重分配有效解决了多模态 ICL 中的效率与性能问题。
...
### [214] [From Triage to Discharge: A Survey of NLP Tasks, Methods, and Open Challenges in the Emergency Department](https://arxiv.org/abs/2608.23627)
*Dipankar Srirag,Aditya Joshi,Salil Kanhere,Padmanesan Narasimhan*

Main category: cs.CL

TL;DR: 本文综述了46篇关于急诊科（ED）NLP应用的研究，探讨了预训练模型在分诊、诊断及处理阶段的应用现状、趋势与挑战。


<details>
  <summary>Details</summary>
Motivation: 现有综述多涵盖整个医院流程或特定任务，缺乏针对急诊科高压、多模态数据环境下NLP应用的系统性梳理。

Method: 对急诊医疗流程（分诊、诊断、处理）中的任务进行了分类分析，评估了建模范式、评价指标及基准测试。

Result: 发现研究重心已从特定任务架构转向预训练语言模型，且学界愈发关注人机交互系统与临床评估的严谨性。

Conclusion: 未来研究需解决模型泛化能力差、处理噪声数据以及契合临床工作流约束等核心痛点。
...
### [215] [Contextual Embedding Evidence for Main--Light Verb Distinctions in Urdu](https://arxiv.org/abs/2608.23645)
*Farah Adeeba,Miriam Butt*

Main category: cs.CL

TL;DR: 本研究利用预训练语言模型（UrduBERT等）分析乌尔都语轻动词（light verbs），验证了其在语义上既与实义动词存在差异，又保留了词汇关联性的语言学理论。


<details>
  <summary>Details</summary>
Motivation: 旨在通过计算方法验证 Butt 提出的关于乌尔都语轻动词具有图式化事件结构意义且与实义动词存在词汇关联的理论假设。

Method: 使用 UrduBERT、DunbaaBERT 和多语言 BERT 对包含 1,126 个乌尔都语自然语句的语料进行向量提取，通过计算向量空间中的语义距离、聚类中心差异及分类任务（遮蔽预测）进行分析。

Result: 实验发现轻动词与实义动词在向量空间中表现出显著的分离性，但同词根的轻/实义动词距离更近；UrduBERT 在轻动词识别任务中表现优异，准确率达 0.866。

Conclusion: 计算证据支持了乌尔都语轻动词在系统性上区别于实义动词，但同时保留了特定词根和动词的表征结构这一语言学观点。
...
### [216] [The Limits of Automatic Evaluation of Creativity in Large Language Models](https://arxiv.org/abs/2608.23705)
*Alessandro Tutone,Giorgio Franceschelli,Mirco Musolesi*

Main category: cs.CL

TL;DR: 该研究探讨了现有自动评估方法在衡量LLM生成的创意文本时是否能与人类判断保持一致。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在创意写作领域表现出强大能力，如何准确评估其生成的创意内容成为了亟待解决的挑战。

Method: 通过WritingPrompts数据集收集人类对人机创作故事在11个创意维度上的评估，并将其与自动客观指标及“LLM-as-a-Judge”评估结果进行对比分析。

Result: 现有自动评估方法与人类判断存在显著不一致；LLM评估器表现出对AI生成文本的系统性偏好，而传统客观指标与人类评价的相关性几乎为零。

Conclusion: 目前的自动评估手段难以捕捉创意文本的多维性和主观性，无法准确衡量AI的创造力水平。
...
### [217] [ADE: Agentic Data Evolution Framework for Human-Centered Objectives](https://arxiv.org/abs/2608.23719)
*Yang Yu,Yilin Jiang,Zexuan Fei,Yiming Luo,Xingkai Song,Kaiyi Huang,Aimin Zhou,Xin Lin,Fei Tan*

Main category: cs.CL

TL;DR: 提出了一种名为ADE的数据演化框架，通过闭环的观测-变异-选择（OVS）机制优化合成数据，解决大模型在弱可验证目标下的对齐难题。


<details>
  <summary>Details</summary>
Motivation: 大模型在非可执行、依赖上下文的任务中难以进行可靠验证，导致合成数据质量参差不齐，进而限制了模型的对齐效果。

Method: 开发了Agentic Data Evolution (ADE)框架，采用闭环的OVS流程，并引入稳态准入机制（Quality Ratchet）来保守地更新数据快照。

Result: 在DEV300基准测试中，ADE将内在胜率提升至75.81%，外在胜率提升至68.86%，并在盲测专家评估中获得66.11%的偏好率。

Conclusion: ADE通过迭代式的数据演化有效提升了模型性能，其高质量的数据快照显著改善了模型的对齐效果，且具有良好的通用性和扩展性。
...
### [218] [What Reaches Expert Review? Representation, Structural Screening, and Candidate-Form Dependence in AI-Assisted Item Development](https://arxiv.org/abs/2608.23766)
*Christopher Brooks*

Main category: cs.CL

TL;DR: 该研究探讨了AI辅助心理测量量表生成过程中“计算评估器”的选择对最终测试内容的影响，指出其并非中立的工具，而是测量设计的核心变量。


<details>
  <summary>Details</summary>
Motivation: 研究者注意到，在AI生成项目与专家评审之间的计算评估步骤（如语义表示、结构简化与选择策略）往往被忽视，但实际上这些选择直接决定了最终呈现给心理测量师的内容。

Method: 通过两项针对32,000个大五人格测试项目的计算机模拟研究，对比了不同语义表示和评估策略下，项目筛选和结构构建的一致性与差异。

Result: 研究发现，尽管全局统计指标看似稳定，但不同评估路径导致实际入选的项目重合度极低（40个项目中仅重合6个），且特定属性在筛选过程中可能意外丢失。

Conclusion: 计算评估器并非中立的基础设施，而是影响测量设计质量的关键组件，必须将其纳入研究的可审查和可修正范围。
...
### [219] [When Youth Enter The Chat: An Epistemic Shift in the Validation of LLM-Based Measures of Student Talk](https://arxiv.org/abs/2608.23780)
*Liliana Santos-Deonizio,James Malamut,Ramón Martínez,Dorottya Demszky*

Main category: cs.CL

TL;DR: 本文探讨了LLM在课堂话语分析中存在的去语境化问题，并强调通过“共享认识权威”将青年参与者纳入评估过程的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估学生话语的方法缺乏语境，且主要依赖成人专家标准，忽略了对边缘化青年的包容性与公正性。

Method: 结合人种志方法，通过参与式观察、访谈、焦点小组和成员核对，让8年级多语种学生参与到LLM分析结果的评估与修订中。

Result: 研究发现LLM生成的分类与学生自身对数学交流的理解存在显著偏差，学生对现有的测量编码体系提出了质疑。

Conclusion: 仅依赖LLM和成人专家是不够的，必须将青年视为“认识权威”，使其参与知识生产过程，才能实现对学生话语更公平、准确的理解。
...
### [220] [Inter-dimension Dependence for Multi-Dimensional Evaluation of Open-Ended Text](https://arxiv.org/abs/2608.23783)
*Haoyuan Li,Snigdha Chaturvedi*

Main category: cs.CL

TL;DR: 该研究提出了评估LLM作为裁判时维度间依赖性的方法CorrGap，并引入了DimCheck框架通过迭代去除思维链中的冗余信息来提升多维度评价的独立性。


<details>
  <summary>Details</summary>
Motivation: LLM作为裁判在评估开放式文本时，往往无法做到不同评估维度（如连贯性、事实性等）的独立性，导致评价存在维度间的交叉干扰。

Method: 提出了CorrGap指标来量化维度间的依赖程度；并设计了DimCheck方法，通过迭代剔除思维链（CoT）中与目标维度无关的证据，从而增强维度评估的纯粹性。

Result: 实验证明LLM普遍存在维度依赖问题，使用DimCheck能有效减轻这种依赖，并在多个LLM和任务上优于基线方法，同时支持用小型模型近似大型模型以降低成本。

Conclusion: 通过过滤非目标维度的干扰信息，可以显著提升LLM作为裁判在多维度评估中的准确性和独立性。
...
### [221] [Giga-Embeddings: Mixture-of-Experts Encoders for High-Throughput Text Embeddings](https://arxiv.org/abs/2608.23806)
*Egor Kolodin,Egor Krasnoperov,Evgeniy Kosarev,Fyodor Minkin*

Main category: cs.CL

TL;DR: Giga-Embeddings 是一系列兼顾高性能检索与高效部署的文本嵌入模型，涵盖从 480M 到 10B（稀疏 MoE）参数规模。


<details>
  <summary>Details</summary>
Motivation: 旨在解决大参数模型在保持高检索质量的同时，面临的高推理延迟和计算开销问题，实现性能与效率的平衡。

Method: 采用了稀疏 Mixture-of-Experts (MoE) 架构，并利用维度无关的目标函数对蒸馏后的 480M 模型进行训练，以对齐教师与学生的相似度分布。

Result: 10B MoE 模型在 MTEB 基准测试中表现卓越，吞吐量比同类 3B 密集模型高 25%，且在处理长文本时显著优于现有外部系统；480M 模型在 Russian MTEB 上以更少参数超越了现有模型。

Conclusion: Giga-Embeddings 提供了一套可扩展的解决方案，通过 MoE 技术和高效蒸馏策略，在保持强劲检索能力的同时显著提升了推理吞吐量。
...
### [222] [From Preferences to Principles: Rubric-Based Alignment for Grounded Knowledge Answers](https://arxiv.org/abs/2608.23812)
*Aman Saini,Priyanshu Kumar,Eric Peng,Kai Yuan,Harsh Girase,Wanming Chen*

Main category: cs.CL

TL;DR: 提出了一种基于多维度、检索增强型评分量表（Rubric）的强化学习奖励框架，用于优化开放域问答模型的后训练效果。


<details>
  <summary>Details</summary>
Motivation: 单一的标量奖励难以有效衡量开放域问答中复杂、多维度的质量需求，导致难以提供精确的监督信号。

Method: 引入基于检索证据生成的查询特定量表（Rubric），将奖励分解为多个细粒度维度（如构思、依据、指令遵循），并以此提供多维奖励监督。

Result: 相比指令微调基线提升了6.5%，相比统一评分量表提升了4%，在事实支持、逻辑连贯性和指令遵循方面均表现更佳。

Conclusion: 结合证据支撑的多维度评分量表能为复杂开放域问答提供更高效的奖励监督，显著提升模型的输出质量。
...
### [223] [Beyond Static and Linear: What Attention Constraints Best Fit Human Reading Times?](https://arxiv.org/abs/2608.23818)
*Lanni Bu,Xiulin Yang,Christian Clark,Alex Warstadt,Ethan Gotlieb Wilcox*

Main category: cs.CL

TL;DR: 该研究探讨了将有限记忆约束引入Transformer注意力机制是否能提高其对人类阅读行为的模拟能力，并比较了多种记忆机制的效果。


<details>
  <summary>Details</summary>
Motivation: Transformer模型具有无损的全上下文访问能力，这与人类有限的记忆系统不符，研究旨在通过施加记忆约束来提升模型作为认知模型的拟合度。

Method: 通过系统性实验对比多种基于注意力的记忆机制，测试不同模型大小和训练语料，对比静态约束与动态记忆课程，并评估其在人类阅读时间预测（心理测量指标）和语法能力上的表现。

Result: 基于内容（而非距离）的记忆约束最能提升对人类阅读时间的拟合度；动态记忆课程虽然能提升心理测量拟合，但会导致语法能力的下降。

Conclusion: Transformers在心理测量拟合与语法能力之间存在权衡，因此无法作为一种万能的人类认知模型。
...
### [224] [Mitigating Exploration Bias in RL for Multi-Instruction Following](https://arxiv.org/abs/2608.23830)
*Mian Zhang,Yueqin Yin,Kaiyu He,Peilin Wu,Xinlu Zhang,Mingyuan Zhou,Zhiyu Zoey Chen*

Main category: cs.CL

TL;DR: 该论文研究并解决了大语言模型在多指令任务中存在的指令遵循探索偏差问题，并提出了相应的改进策略。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习训练在处理多指令提示时，由于初始模型能力不足以及累积奖励机制的局限性，导致模型倾向于仅执行简单指令，忽略了困难指令。

Method: 提出了两种衡量探索偏差的指标，并设计了名为MulIF的两阶段框架：1) 行为引导（Behavioral Bootstrapping），通过拒绝采样微调激活对困难指令的处理能力；2) 稀缺感知奖励（Scarcity-Aware Rewards），根据指令的稀缺程度动态调整奖励权重。

Result: 实验证明所提指标与模型表现高度相关，且该方法在三个可验证的指令遵循基准测试中显著优于现有基线模型。

Conclusion: 通过针对性的奖励调整和分阶段训练，能够有效克服强化学习在处理复杂多指令任务时的探索偏差，从而显著提升模型的指令遵循性能。
...
### [225] [Does Episodic Memory Help Close the Lexical Frequency Gap in Sensitivity to Syntactic Contrasts? A Test Using Retrieval-Augmented Language Models](https://arxiv.org/abs/2608.23851)
*Jing Liu,Najoung Kim*

Main category: cs.CL

TL;DR: 研究探讨了通过引入类海马体的情景记忆机制（检索增强模型），能否弥补神经网络在处理语法任务时受词汇频率影响的缺陷。


<details>
  <summary>Details</summary>
Motivation: 神经语言模型在判断语法时过度依赖词汇频率，导致低频项表现不佳，而人类语言能力表现出对词汇频率的鲁棒性。

Method: 利用k-近邻语言模型（kNN-LM）作为情景记忆机制，通过检索显式实例来增强参数化模型，并对比在不同频率词汇上的语法判断表现。

Result: 检索增强缩小了高频与低频词汇在语法测试中的性能差距，且结构性信息比语义相似性对提升效果更关键。

Conclusion: 情景记忆能有效补偿参数化模型的表征弱点，但仍需进一步优化检索策略与结构表征，以完全消除频率偏见。
...
### [226] [Names Can Hurt: Spotting Slopsquatting Risks Caused by Package Name Hallucinations in Local Coding LLMs](https://arxiv.org/abs/2608.23897)
*Akash Raj,Sargam Sahu*

Main category: cs.CL

TL;DR: 研究者针对代码生成模型产生的“幻觉包名”引发的“抢注攻击（slopsquatting）”问题，提出了一套防御系统。


<details>
  <summary>Details</summary>
Motivation: 语言模型生成的代码可能引用不存在的Python包，攻击者通过提前注册同名恶意包，可诱导用户执行供应链攻击。

Method: 构建了一个两层检测器：第一层进行PyPI存在性校验，第二层利用随机森林分类器结合包元数据特征识别风险；同时设计了包含导入名协调器和LangGraph状态机的重试机制与多模型回退策略。

Result: 该系统在300个提示词测试中实现了76%的幻觉消除率。研究发现：幻觉率随提示词对抗性增加而线性上升，且跨模型家族的回退策略比同家族回退更有效。

Conclusion: 通过结合确定性校验、机器学习分类与智能重试机制，能够有效降低大模型代码生成中的供应链攻击风险，且用户接受度较高。
...
### [227] [Investigating Knowledge Transfer Across Interactive Dialogue Games](https://arxiv.org/abs/2608.23969)
...
TL;DR: 该研究探讨了大型语言模型（LLM）在不同对话游戏任务之间的知识迁移能力，分析了任务间的可迁移性和模型微调的有效性。


<details>
  <summary>Details</summary>
Motivation: 假设语言模型在特定对话游戏中学习的规则和策略，可以提升其在其他相关任务中的表现。

Method: 利用clembench基准进行微调，通过二进制整数优化程序构建任务迁移图，并利用任务向量（Task Vectors）方法分析模型间的相似性。

Result: 视觉空间类游戏具有最好的迁移效果；基于任务向量的相似度分析能捕捉游戏角色的关系，但难以预测实际的迁移能力。

Conclusion: 对话游戏之间的知识迁移存在差异，单纯基于向量的相似度指标不足以完全解释任务间的迁移性能，需要更复杂的评估指标。
...
### [228] [AgentSpec: Speculative Decoding for Batch Inference of LLM Agents](https://arxiv.org/abs/2608.24004)
*Xin Wang,Ziming Miao,Yi Zhu,Hui Shen,Zhongwei Wan,Fan Yang,Mi Zhang*

Main category: cs.CL

TL;DR: AgentSpec 是一种专为 LLM 智能体设计的推测解码算法，旨在解决现有方法在处理大规模批处理时速度下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码在 LLM 智能体应用中存在大批量下速度下降的问题，主要由高拒绝率和动态 Token 预算利用不足导致。

Method: 提出了结构隔离草稿（structure-isolated drafting）以减少不相关路径并降低拒绝率，以及冗余感知预算分配（redundancy-aware budget allocation）以优化 Token 预算利用。

Result: 在 5 种工作负载和 4 种 LLM 模型上的实验表明，AgentSpec 显著优于现有的先进推测解码算法。

Conclusion: AgentSpec 通过针对智能体工作流特性进行优化，有效提升了 LLM 智能体在实际应用中的推理效率。
...
### [229] [SAGE: From Direct Answering to Evidence-Grounded Inference for Chinese Ancient Document Understanding](https://arxiv.org/abs/2608.24011)
*Yuchuan Wu,Xuan Luo,Yinglian Zhu,Meng Fang,Xiangyang Xue,Bin Li*

Main category: cs.CL

TL;DR: SAGE是一个针对中国古籍理解的多智能体框架，通过证据驱动的推理机制取代传统的端到端生成，显著提升了准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型（LVLM）在古籍理解任务中表现出不透明、缺乏证据支持及过度自信的问题。

Method: 引入SAGE框架，通过协同多个专门代理进行任务规划、工具辅助证据检索、声明级验证及有界重规划，实现证据驱动推理。

Result: 在AncientDoc基准测试中，SAGE框架在三个LVLM主干模型上均超越了传统的直接回答基准，且Qwen3.5-9B的表现超过了更大规模的单一模型。

Conclusion: 结构化的证据驱动推理在复杂古籍任务中比单纯依赖模型规模扩展更为有效，能显著提升模型的准确性和推理质量。
...
### [230] [When Less Is More: An Empirical Study of Minimal Responses in Counseling Dialogues and the Behavior of LLMs](https://arxiv.org/abs/2608.24080)
*Zhiyang Qi*

Main category: cs.CL

TL;DR: 该研究分析了心理咨询中“极简回复”（Minimal Responses）的重要性及其在对话系统中的缺失与评估偏见。


<details>
  <summary>Details</summary>
Motivation: 现有对话系统偏向生成长篇大论，忽视了心理咨询中极简回应（如点头应答、简短共情）在维持互动和表达专注方面的重要作用。

Method: 通过跨语言分析多个咨询数据集，提出一种基于长度、内容过滤及LLM上下文验证的二阶段过滤方法，并对比人类数据与LLM生成数据，评估LLM在极简回复生成及自动评测中的表现。

Result: 人类咨询数据中极简回复广泛存在，但LLM生成内容中严重缺失；强力商业模型虽能通过指令生成极简回复，但难以把握时机；专用模型表现更差，倾向于生成冗长内容；且现有LLM评测框架倾向于低估极简回复的价值。

Conclusion: 心理咨询对话系统需重新重视极简回复的交互价值，目前的评估范式与模型生成倾向需要针对性优化，以更好地辅助心理咨询任务。
...
### [231] [Curved Inference II: Sleeper Agent Geometry - Extending Interpretability Beyond Probes](https://arxiv.org/abs/2608.24037)
*Rob Manson*

Main category: cs.CL

TL;DR: 该论文提出了一种无需人工后门或线性探针的自然主义方法，通过分析大语言模型推理过程中的几何特征（如语义表面积）来检测欺骗性行为。


<details>
  <summary>Details</summary>
Motivation: 现有基于线性探针的后门检测方法仅适用于人工插入的简单后门，无法应对自然演化出的复杂、非线性的欺骗性对齐行为。

Method: 利用“弯曲推理（Curved Inference）”框架，引入“语义表面积（A'）”指标，在无需标签和探测器的情况下，通过分析残差空间中语义构建的几何特征，结合LLM共识对输出进行分类。

Result: 实验表明，欺骗性推理产生的几何特征具有显著差异，且通过该框架可以识别出隐藏在分类噪声下的语义信号，检测效力优于传统线性方法。

Conclusion: 推理过程的几何形状内在编码了语义模式，即使在模型试图压制线性指标的情况下，基于几何特征的检测方法仍能有效识别复杂欺骗，提供了一种可扩展的无监督检测路径。
...
### [232] [PARTAB: Partition-Aware Reasoning with Structured Evidence for Scalable Table Understanding](https://arxiv.org/abs/2608.24082)
*Md Mahadi Hasan Nahid,Davood Rafiei*

Main category: cs.CL

TL;DR: PARTAB 是一个通过将复杂表格划分为语义连贯的区域，实现分层证据选择与推理的 LLM 表格分析框架。


<details>
  <summary>Details</summary>
Motivation: 大规模及复杂表格常导致 LLM 面临无效上下文干扰和证据定位困难，现有方法难以兼顾全表信息与局部细节。

Method: 引入 PARTAB 框架，构建行关联的语义分区，通过对列组和行级分区进行分层选择，最后合成证据以生成答案。

Result: 在 WikiTableQuestions 和 TabFact 等数据集上表现优于现有方法，能有效减少推理上下文并提升对复杂表格的推理准确度。

Conclusion: 结构化的分区感知证据构建策略显著提升了 LLM 在大规模复杂表格上的推理效率与准确性。
...
### [233] [MC-CXR: A Multi-Context Chest X-ray Benchmark for Context-Induced Disruption in Vision-Language Models](https://arxiv.org/abs/2608.24118)
*Junhyeok Lee,Songsoo Kim,Kyu Sung Choi*

Main category: cs.CL

TL;DR: MC-CXR 是一个旨在评估多模态大模型在胸部X光诊断中，面对误导性上下文信息（文本或先验图像）时是否存在判断偏差的新型基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准多关注模型在理想状态下的准确率，忽略了模型是否会在临床背景下被误导性信息（如错误的病历报告或影像）干扰而产生错误的判断。

Method: 构建了包含240个病例、2522个实例的MC-CXR数据集，采用配对扰动方法，通过对比可靠上下文与误导性上下文下的模型表现，定义了“错误切换率”和“上下文对齐错误率”两个指标。

Result: 评估显示现有模型极易受上下文影响，文本误导的切换率为45.6%-78.1%，视觉误导为35.7%-61.7%；且存在明显的文本-视觉不对称现象，模型更容易盲从误导性文本而非图像。

Conclusion: 仅具备图像分析能力不足以保证临床安全，现有的视觉-语言模型普遍缺乏鲁棒性，在复杂医疗上下文决策中极易产生偏见。
...
### [234] [TrustDABench: Benchmarking Reliability and Robustness of LLMs for Structured Data Analysis](https://arxiv.org/abs/2608.24145)
*Boshen Shi,Yize Liu,Chen Zhao,Ce Chi,Zhendong Wang,Xing Wang,Junlan Feng*

Main category: cs.CL

TL;DR: 本文提出了TrustDABench基准测试，用于评估LLM在处理结构化数据（如CSV、表格）时的可靠性（能否拒绝无法支持的问题）和稳健性（对数据形式变换的鲁棒性）。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM在处理表格数据时往往仅追求“看起来正确”的答案，缺乏证据支持的可解释性和可信度，且对表格形式的微小变化十分敏感。

Method: 构建了包含19种扰动算子的生成框架，产生了2340个人工验证的实例，并评估了8个主流大模型在处理结构化数据时的表现。

Result: 实验显示当前LLM表现不佳，最佳可靠性得分仅为24.21%，最佳稳健性错误率高达9.10%。模型普遍难以识别冲突证据，且极易受表格结构扰动影响。

Conclusion: 目前的LLM在结构化数据分析中缺乏证据边界识别能力和表征不变性推理能力，亟需在这些方向上进行深入研究。
...
### [235] [FireRedAudio: A General-Purpose Audio Language Model with Decoupled Continuous Representations for Understanding and Generation](https://arxiv.org/abs/2608.24168)
...
TL;DR: FireRedAudio 是一款 9B 参数的统一音频大语言模型，通过解耦音频理解与生成表示，实现了高质量的语音识别、理解、零样本 TTS 及音频编辑功能。


<details>
  <summary>Details</summary>
Motivation: 音频理解需要紧凑的特征以处理长上下文，而音频生成则需要保留精细声学细节的特征，传统的统一音频模型难以兼顾两者。

Method: 引入解耦的持续输入表示：利用专用编码器进行分析，利用 RedAE 路径进行生成，并结合流匹配 DiT 模型在 9B 大模型基础上实现多任务训练。

Result: 在音频理解、多语言 ASR、零样本 TTS 及语音编辑任务中表现领先，支持长达一小时的音频分析且具备秒级时间戳精度。

Conclusion: 证明了在单一 LLM 框架下，通过解耦持续输入表示来统一音频理解与持续潜空间音频生成的方案是行之有效的。
...
### [236] [MemUse: Moving Memory Evaluation from Direct QA to Natural Integration in Long-Term Human-AI Conversation](https://arxiv.org/abs/2608.24189)
*Ryuichi Sumida,Koji Inoue,Tatsuya Kawahara*

Main category: cs.CL

TL;DR: 研究发现大型语言模型（LLM）的记忆评估指标“直接问答（Direct QA）”与真实用户满意度脱节，并提出了更关注自然整合能力的“MemUse”评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法（Direct QA）主要衡量模型的事实回忆能力，但在实际应用中，这种能力与用户的主观满意度之间缺乏相关性。

Method: 通过为期4个月的实地部署测试，对比了7种记忆条件下Direct QA准确率与用户满意度的关系；引入了MemUse数据集，该数据集基于真实对话场景，通过评估模型在自然对话中整合记忆的能力进行打分。

Result: 研究发现Direct QA得分（高达78.8%）与实际对话中的记忆引用率（仅7.9%）存在巨大差距；且只有自然整合能力与用户满意度显著相关，Direct QA则不相关。

Conclusion: 传统的评估指标高估了模型在真实对话中的记忆应用能力，构建具备“自然整合”能力的模型才是提升用户满意度的关键。
...
### [237] [Measuring Digital Labour Market Transitions with a Digital Semantic Score: An AI-Based Methodology Applied to the Dutch Labour Market](https://arxiv.org/abs/2608.24222)
*Sadegh Shahmohammadi,Xavier Pinho,Mairi Bowdler,Suhendan Adiguzel-van Zoelen,Joost van Genabeek*

Main category: cs.CL

TL;DR: 该研究提出了一种基于AI的方法，通过语义分析和数字语义评分（Digital Semantic Score）来量化评估荷兰劳动力市场的数字化转型及技能演变。


<details>
  <summary>Details</summary>
Motivation: 传统的关键词方法难以捕捉职业语言中复杂的数字化语义，研究旨在通过更细颗粒度的人工智能分析，理解数字技能的分布、职业路径及演变过程。

Method: 结合嵌入（Embedding）相似度搜索与大语言模型分类，将非结构化职位信息映射至ESCO职业标准，并引入数字语义评分量化数字属性。

Result: 数字化转型在不同职业间分布极不均衡，管理、专业及ICT岗位数字化程度最高；数字技能展现出多维性，涵盖技术、混合及业务系统技能；职业转型路径表现出显著的路径依赖性。

Conclusion: 该方法构建了一个可扩展的框架，能有效监控劳动力市场数字化变化，为制定再培训策略、缓解技能错配及劳动力短缺提供政策支持。
...
### [238] ['Ghaib in Translation' aka Unseen Harm: Measuring Cross-Script Safety Inconsistency with 'Missed-in-Urdu' Scores in LLM Hate Speech Detection](https://arxiv.org/abs/2608.24191)
*Fawzia Zehra,Kara-Isitt,Sonal Khosla,Stephen Swift*

Main category: cs.CL

TL;DR: 该研究探讨了当前主流大语言模型在乌尔都语内容审核中的可靠性，发现模型在乌尔都语与英语之间的判断存在显著的不一致性和漏检问题。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语作为全球第十大语言，在当前的大模型安全评估和学术研究中长期缺席，缺乏针对性的安全性保障研究。

Method: 测试了GPT-4o、Claude 3.5 Sonnet、Gemini 2.5 Flash、Qwen-2.5和Llama-3.1在六个数据集上的表现，涵盖纳斯塔利克体乌尔都语、罗马化乌尔都语、英语及混合语，并分析了模型对原始语种与英语译文判别结果的差异。

Result: 模型在乌尔都语与英语分类之间存在15.9%至31.6%的标签不稳定性，且存在显著的“乌尔都语漏检”现象（即英语译文判定为有害但在原文中被通过），漏检率中位数为4.3%。

Conclusion: 当前主流大模型在处理乌尔都语时安全保障能力参差不齐，且学术界对该语言的安全研究处于真空状态，小型开源模型在处理乌尔都语时表现明显劣于闭源模型。
...
### [239] [RecurSE: Bounded Recursive Self-Evaluation for LLM Rubric Judges](https://arxiv.org/abs/2608.24231)
*Kaiyuan Liu,Ziyuan Zhuang,Rongxiang Weng,Jieping Ye*

Main category: cs.CL

TL;DR: 提出了一种无需外部标注的递归自我评估框架（RecurSE），通过闭环强化学习实现LLM作为裁判的自我优化。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-judge的训练高度依赖昂贵的外部标注、奖励模型或强模型蒸馏，限制了其自主优化能力。

Method: 引入RecurSE框架，采用双重评估机制：裁判模型根据规则评估（Pass 1），同步策略校验器审计裁判推理（Pass 2）。通过接口解耦避免奖励欺骗，并利用Pairwise Advantage Validity (PAV)作为验证指标监控训练过程，实现有界递归自我提升。

Result: 在Qwen和Gemma系列模型上实验显示，RecurSE在医疗、总结等多个基准测试中均有泛化提升，性能超越冻结校验器及传统蒸馏方法。

Conclusion: 在明确解耦评估奖励并引入监控机制的前提下，LLM通过递归自我评估实现自我提升是可行且有效的。
...
### [240] [ROBE: Reversed-Order-Biased-Experts for Extracting Extreme Long-tail Events from Historical Texts](https://arxiv.org/abs/2608.24268)
*Stella Verkijk,Piek Vossen*

Main category: cs.CL

TL;DR: 该研究提出了一种名为ROBE的新型分类方法，旨在从缺乏训练数据的17-18世纪荷兰历史语料中提取稀有的长尾事件。


<details>
  <summary>Details</summary>
Motivation: 历史语料属于大语言模型预训练的空白领域，且目标事件在现有数据中极其稀缺，传统的微调方法难以捕捉这些长尾信息。

Method: 基于频率或语义相关性将事件分组，构建专家分类器；采用ROBE（反向顺序偏置专家）策略，赋予长尾事件专家更高的预测优先级；同时利用受控方法生成领域特定合成数据。

Result: 相比简单的微调编码器模型，ROBE方法在查准率和查全率上分别提升了0.16和0.10，在长尾事件组的F1分数上提升了0.10。

Conclusion: ROBE策略有效缓解了长尾事件被高频类别主导的问题，为处理小众领域的稀疏数据提取任务提供了一种高效解决方案。
...
### [241] [SENSESHIFT: Continuous Sentiment-Controlled Text Generation via Encoder-based Mask Infilling](https://arxiv.org/abs/2608.24304)
*Shahed Masoudian,Markus Frohmann,Emmanouil Karystinaios,Navid Rekabsaz,Markus Schedl*

Main category: cs.CL

TL;DR: SenseShift 是一种基于编码器架构的细粒度文本生成框架，通过双向注意力和掩码填充技术，实现了对长文本中句子级别的精准情感控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于解码器的自回归模型在处理复杂的细粒度情感控制（尤其是长文本中的句子级控制）时表现不佳，且多局限于粗粒度标签。

Method: 采用编码器架构，结合双向注意力机制、量化情感信号以及迭代掩码填充（iterative mask infilling）技术进行局部句子生成。

Result: 实验表明，SenseShift 在故事和评论生成任务中，在情感可控性、文本质量以及对领域外生成的稳健性方面均优于较大的解码器模型。

Conclusion: SenseShift 证明了基于编码器的双向注意力机制在细粒度、受约束的文本生成任务中比纯解码器架构更具优势。
...
### [242] [Who is the Agent to Blame? Localizing Faithfulness and Citation Mistakes in Agentic Deep Research](https://arxiv.org/abs/2608.24306)
*Eran Hirsch,David Wan,Han Wang,Elias Stengel-Eskin,Mohit Bansal,Ido Dagan*

Main category: cs.CL

TL;DR: 该论文研究了深度搜索（DR）系统中引文召回率低的问题，提出了一种诊断多智能体架构中错误来源的评估方法，并基于该方法对现有系统进行了优化。


<details>
  <summary>Details</summary>
Motivation: DR系统在生成长文报告时引用准确度较差，且由于系统复杂的交互流程，难以定位错误环节。

Method: 提出了一种基于本地测试的方法，通过评估每个智能体相对于其输入的忠实度和可验证性来定位错误，并建立了一个包含幻觉、无引文依赖、无引文输出及引文不足的错误分类法。

Result: 研究发现大多数智能体（除单文档摘要外）均存在显著错误，其中协调器（Orchestrator）是错误的主要来源（占AI-Q错误源的84.7%），主要表现为引文缺失或幻觉。

Conclusion: 通过针对性的干预措施，在不影响输出质量的情况下，系统引文召回率提升了5%，验证了精细化诊断对于提升多智能体协作系统可靠性的重要性。
...
### [243] [Speech-to-SOAP: End-to-End Summarization of Medical Dialogues: KIT@BeTraC 2026](https://arxiv.org/abs/2608.24327)
*Enes Yavuz Ugan,Fabian Retkowski,Yuka Ko,Thai-Binh Nguyen,Maike Züfle,Jan Niehues,Alexander Waibel*

Main category: cs.CL

TL;DR: 本文提出了一个针对BeTraC挑战赛的轻量级端到端临床对话转SOAP笔记生成系统，通过合成语音与自动生成的监督数据进行训练。


<details>
  <summary>Details</summary>
Motivation: 旨在减轻医护人员编写临床笔记的负担，并避免传统转录系统丢失非语言线索（如咳嗽）的问题，实现从语音到临床笔记的直接自动化生成。

Method: 构建了一个可扩展的数据增强流水线，整合多种医疗对话数据集；利用合成语音技术与自动生成的SOAP笔记监督，对语音基础模型进行端到端适配。

Result: 成功构建了轻量级的端到端语音转SOAP生成模型，证明了通过合成数据增强在医疗对话任务上的有效性。

Conclusion: 该方法通过统一异构医疗数据，有效提升了语音基础模型在临床协议编写任务中的鲁棒性，为自动化临床记录提供了一种高效路径。
...
### [244] [SteerCheck: Attribution Specificity and Alignment Leakage in Activation-Steering Audits](https://arxiv.org/abs/2608.24335)
*Daming Luo,Christy Liang,Junyu Xuan*

Main category: cs.CL

TL;DR: 本文提出了SteerCheck框架，通过多维度归因审计方法验证大模型激活操纵（activation steering）的效果，揭示了常用对照实验在区分因果效应时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有激活操纵技术往往无法验证效应是否针对特定概念，且常用的对照方法存在对齐泄露（alignment leakage）问题，导致操纵结果的因果有效性存疑。

Method: 引入SteerCheck框架，利用预注册归因审计、KL散度匹配、符号随机化对照、各向同性方向分析以及多维度（均值、极性、语义等）评估指标对Qwen3-14B等模型进行干预测试。

Result: 实验发现符号随机化方向仍保留高度对齐，且效应与符号余弦值强相关（ρ=.94）；大部分被测模型的操纵效果未能通过所有严格的控制门控测试，自动判别器在语义评估中表现欠佳。

Conclusion: 目前的激活操纵技术在区分真实概念操纵与干扰方面存在显著局限，研究强调必须通过公开构造诊断和交换性假设来提高操纵实验的可审计性和可靠性。
...
### [245] [FARCA: Fact-Aligned Reliability-Aware Credit Assignment for Reinforcement Learning with Factual Supervision](https://arxiv.org/abs/2608.24350)
*Qiming Xie,Wenjie Zheng,Xiangqing Shen,Rui Xia*

Main category: cs.CL

TL;DR: 提出了一种名为FARCA的框架，通过解决事实监督中信用分配的噪声问题，提升了大模型强化学习中的事实准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理事实监督时存在“噪声事实信用分配”问题，具体表现为信用定位模糊和信用可靠性模糊，导致模型事实验证与策略更新不匹配。

Method: FARCA框架：1. 实现细粒度的信用定位，将事实验证与策略更新的粒度对齐；2. 引入反事实证据归因，基于证据依赖性评估事实判断的可靠性，通过权重调节事实奖励和策略优势。

Result: 在多个事实推理基准测试中，FARCA显著提高了模型的准确性，并保持了其通用推理能力。

Conclusion: 通过引入细粒度、可靠性感知的信用分配，FARCA有效缓解了事实监督中的噪声问题，是优化强化学习中大模型事实性的有效方案。
...
### [246] [Words, Spaces and Generative AI: Layers of language in contemporary architecture](https://arxiv.org/abs/2608.24360)
*Anca-Simona Horvath*

Main category: cs.CL

TL;DR: 本文探讨了语言作为建筑设计中的一种“设计材料”，以及在生成式AI语境下，理解语言如何重塑建筑实践的理论框架。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在建筑设计中的普及，语言在设计工具中的核心地位凸显，研究者旨在通过语言学视角重新审视AI辅助设计的逻辑。

Method: 回顾Wittgenstein、Chomsky、Lakoff及Schön的语言学理论，分析自然语言（话语）、编程语言和数据标注这三种语言形式在生成式AI建筑设计中的交织作用。

Result: 提出了建筑设计中三种语言形式的整合框架，并强调了生成式隐喻与生成式AI工具之间的深层联系。

Conclusion: 构建了一个研究议程，提倡通过语料库语言学分析建筑文本，结合传播理论，并关注不同自然语言在生成与概念隐喻上的差异。
...
### [247] [Shortcut Before Circuit: Document Statistics Time In-Context Conflict Resolution](https://arxiv.org/abs/2608.24460)
*Yijun Liao,Fanwei Liang*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在处理冲突信息时，通过不同的机制（如近期性、重复性等）达成相同的预测准确度，且这些机制在训练过程中表现出随机性和不确定性。


<details>
  <summary>Details</summary>
Motivation: 当数据中存在多种提示（cue）导致模型表现一致时，难以确定模型具体学习到了哪种逻辑。研究旨在揭示模型内部表征的多样性与机制的可解释性局限。

Method: 构建了一个合成数据集，使模型在处理冲突事实时，不同提示规则的效果在统计上重合。通过最小因果编辑干预模型，观测不同种子下的内部激活模式及神经元读出结果。

Result: 实验发现，模型虽然能达到极高的准确率，但不同模型运行实例内部的神经元表征存在巨大差异，且在模型学习到特定规则前的中间阶段，模型归因分析经常出现相反结果。

Conclusion: 模型准确率无法反映其内部具体的逻辑机制。在规则重合的训练环境下，模型如何选择机制是随机的，导致基于数据的机制归因可能失效。
...
### [248] [Dataset Scarcity Limits Robust Evaluation of Multilingual Embedding Models: A Case Study of Slavic Languages](https://arxiv.org/abs/2608.24477)
*Ana Gjorgjevikj,Barbara Koroušić Seljak,Tome Eftimov*

Main category: cs.CL

TL;DR: 本文提出了一个针对多语言文本嵌入模型基准测试的二维评估框架，用于分析在数据匮乏情况下的评估可靠性，并以斯拉夫语系为例进行了实证研究。


<details>
  <summary>Details</summary>
Motivation: 多语言嵌入模型在不同资源水平语言间的评估严重不均衡，且现有基准测试存在数据稀缺和结果稳健性不足的问题。

Method: 提出了一个包含任务特定和跨任务评估的二维框架，涵盖排名稳健性、模型一致性和证据强度三个维度，并引入了“证据强度评分”来量化基准测试的可靠性。

Result: 发现斯拉夫语系的基准测试存在严重的数据稀缺性；筛选出了llama-embed-nemotron-8b、multilingual-e5-large-instruct和Qwen3-Embedding等在多语言及多任务中表现稳健的优秀模型。

Conclusion: 基准测试的排名和稳健性结论必须结合证据强度进行解读，数据稀缺是实现可信多语言评估的主要障碍。
...
### [249] [Beyond Information Seeking: Severity-Aware Question Supervision for Proactive Medical Dialogue](https://arxiv.org/abs/2608.24521)
*Chenxuan Li,Xinrong Chen,Luyan Zhang,Peidong Jia,Zhongyu Zhao,Xuecheng Shang,Peixing Wan*

Main category: cs.CL

TL;DR: 提出了一种名为ESR的医疗对话问询目标，通过评估预期风险降低程度而非仅关注不确定性，显著改善了对严重疾病的诊断表现。


<details>
  <summary>Details</summary>
Motivation: 现有的医学诊断AI往往只追求降低诊断不确定性，而忽视了漏诊严重疾病所带来的后果差异，缺乏对医疗诊断结果严重性的考量。

Method: 引入“预期严重性风险（ESR）”目标，通过对可能的回答进行统计加权，评估每个询问对降低“严重性感知风险”的贡献，并将该策略蒸馏进语言模型以实现高效部署。

Result: 在DDxPlus数据集上，ESR显著降低了严重漏诊率（从0.0645降至0.0455），同时提高了整体诊断准确率，且仅需微量额外提问。

Conclusion: 研究表明，医疗对话应从单纯的“不确定性减少”转向“结果导向”的证据获取，能够更有效地识别和规避高风险诊断错误。
...
### [250] [Is Discrete Difficulty Sufficient? Leveraging Continuous Difficulty for Efficient Self-Consistency in LLMs](https://arxiv.org/abs/2608.24590)
*Sihyeong Yeom,Geon Park,Geunyeong Jeong,Taewoong Yoon,Jaewook Lee,Harksoo Kim*

Main category: cs.CL

TL;DR: 提出了一种名为灵活自洽性（FSC）的解码策略，通过将问题难度建模为连续信号来动态调整推理路径数量，从而在保持准确性的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统的自洽性（Self-Consistency）方法通过采样多条推理路径来提高准确性，但存在推理代价高昂的问题，且现有资源分配方法难以捕捉问题难度的连续性变化。

Method: 利用预训练的探测器（probe）预测输入问题的输出熵，将其作为模型不确定性的指标，并据此动态调整生成推理路径的采样预算。

Result: 在多种模型和基准测试中，FSC 在保持与传统自洽性相当的准确率的同时，最多可减少 76% 的令牌消耗。

Conclusion: FSC 有效解决了自洽性方法中的效率瓶颈，证明了通过连续难度评估来动态分配计算资源是一种高效的优化策略。
...
### [251] [Quantization Effects on Bangla Language Understanding in Large Language Models: A Systematic Evaluation](https://arxiv.org/abs/2608.24615)
*Ismail Hossain,Nafi Ullah Shafin,Mohammad Abdullah Al Mumin*

Main category: cs.CL

TL;DR: 该研究评估了后训练量化（PTQ）对孟加拉语大语言模型性能的影响，发现模型架构和量化方法的选择对低资源语言的性能表现至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有关于量化的研究多集中于英语，缺乏针对形态复杂、资源匮乏语言（如孟加拉语）在量化环境下的实证研究。

Method: 选取Qwen-2.5-7B、LLaMA-3.1-8B和GPT-OSS-20B三个系列模型，在五项孟加拉语自然语言理解基准测试中，对比全精度与三种量化格式（GPTQ-Int8, GPTQ-Q8, GGUF-W8A16）的零样本表现。

Result: 不同模型系列表现差异巨大；GPT-OSS在推理任务中性能显著下降，而Qwen和LLaMA在GPTQ量化下表现稳定，甚至在某些情况下优于全精度版本。

Conclusion: 量化在孟加拉语部署中是可行的，但实践者需注意，相比于比特宽度，选择合适的模型架构和量化方法对维持性能更为关键。
...
### [252] [Beyond Semantic Accuracy: Consequence-Aware Evaluation for Safety-Critical Language Understanding](https://arxiv.org/abs/2608.24621)
*Yujing Chang,Thinh Pham,Van-Phat Thai,Chunyao Ma,Yash Guleria,Pham Nhut Huy,Sameer Alam*

Main category: cs.CL

TL;DR: 该研究探讨了语言模型在空中交通管制（ATC）等安全关键领域应用中的可靠性问题，并发现标准语义评价指标无法准确反映实际的操作安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有评价指标（如F1分数）在安全关键任务中存在缺陷，即使语义得分高，关键信息的微小错误也可能导致严重的现实后果。

Method: 构建了一个基于航空标准并结合40名空中交通管制员反馈的ATC诊断基准，提出了“后果感知评估”（consequence-aware evaluation）框架，并评估了8个语言模型。

Result: 发现了显著的“语义-安全差距”，即传统指标高估了模型性能；即使经过风险感知微调，该差距依然存在。

Conclusion: 标准NLP指标不足以评估安全关键系统的可靠性，必须将“后果感知评估”作为模型部署前的必要补充。
...
### [253] [Expectation, Backlash, Recovery, and Excitement: How Model Releases Shape Reddit Perceptions of Conversational AI Systems](https://arxiv.org/abs/2608.24654)
*Vahid Rahimzadeh,Yury Zhauniarovich,Savvas Zannettou*

Main category: cs.CL

TL;DR: 该研究通过分析Reddit上的长期讨论，探讨了用户对对话式AI系统（CAIS）在模型发布、功能更新等干预措施下的动态感知变化。


<details>
  <summary>Details</summary>
Motivation: 现有研究多将用户感知视为静态快照，忽略了AI系统在持续迭代和干预下，用户感知随时间演变的动态特性。

Method: 结合情感分类与主题概念分析，对各大AI供应商（如OpenAI、Anthropic、DeepSeek等）的模型发布期间的Reddit用户讨论进行了大规模、长期的纵向研究。

Result: 研究发现不同AI供应商的发布具有独特的影响模式：Anthropic产品契合度高且评价正面，OpenAI经历了舆论波动，Grok-3受政治倾向影响，DeepSeek-R1则在工程能力认可与安全性担忧之间摇摆。

Conclusion: 模型发布不仅是技术升级，更是塑造用户情感、预期及公共讨论的重要干预手段，用户感知具有高度的动态性和对干预的敏感性。
...
### [254] [From local kernels to global form: modeling the emergence of musical content](https://arxiv.org/abs/2608.24660)
*Francesco Vitucci,Michele Lorusso,Francesco Scagliola*

Main category: cs.CL

TL;DR: 该论文提出了一种基于滑动窗口的马尔可夫模型估计机制，用于分析和分割符号音乐序列。


<details>
  <summary>Details</summary>
Motivation: 旨在探索能否通过观测驱动的局部转移核轨迹来自动识别符号音乐的结构，而非依赖外部定义的格式化分区。

Method: 利用重叠滑动窗口从单一符号序列导出局部转移核轨迹，并以德彪西《Syrinx》为例，通过Jensen-Shannon散度验证绝对音高和记谱时值的核变化，同时进行重合成实验以评估模型表现。

Result: 实验发现音高和时值在窗口长度为6时达到Jensen-Shannon最大值，但存在广泛的平坦区，且音高和时值的交叉维度对齐能够辅助结构边界识别，但单一维度不足以作为唯一的自动分割工具。

Conclusion: 基于滑动窗口的马尔可夫模型在捕捉音乐结构边界上具有一定敏感性，但其广泛的平坦区限制了其作为独立自动分割器的有效性，且在较小窗口下存在退化现象。
...
### [255] [One Timeline, Many Renderings: A Wolfram Language Paclet for heterogeneous musical output](https://arxiv.org/abs/2608.24683)
*Francesco Vitucci,Michele Lorusso,Francesco Scagliola*

Main category: cs.CL

TL;DR: Temporal System 是一款基于 Wolfram Language 的作曲与排版工具，通过统一的“时间戳数据存储”确保多种输出格式（如 Csound、MusicXML、OSC）在时间线上的严格同步。


<details>
  <summary>Details</summary>
Motivation: 解决算法作曲中不同输出格式（如乐谱、合成指令、实时控制信号）因分别编写而导致的计时漂移和同步困难问题。

Method: 利用 Wolfram Language 构建不可变类型实体存储库，将时间轴映射为有理数节奏，并通过后端特定契约将统一数据转换为 Csound、MusicXML、OSC 等格式。

Result: 实现了多种音乐输出格式的完美同步，将时间、样本和频率的转换延迟至渲染阶段，并提供了稳定的 Csound 仪器映射和自动化排练音频生成。

Conclusion: 该系统通过统一的语义层解决了音乐创作中的同步痛点，证明了在封闭式创作环境中构建高性能、多模态音乐同步机制的可行性。
...
### [256] [The Annotation Bottleneck in Persian Text NLP: Persian as an Annotation-Scarce Language](https://arxiv.org/abs/2608.24698)
*MohammadHossein Mortazavi,Mostafa Salehi,Hadi Veisi*

Main category: cs.CL

TL;DR: 本文提出波斯语（Farsi）并非传统意义上的“低资源语言”，而应被界定为“标注匮乏型语言”，其资源短缺源于生态系统而非语言本身。


<details>
  <summary>Details</summary>
Motivation: 澄清波斯语在自然语言处理领域资源匮乏的本质，指出简单将波斯语归类为低资源语言掩盖了标注不均衡和资源分布碎片化等实际问题。

Method: 通过审视34个波斯语文本资源，并结合Web存在度测量、语音资源演进分析以及与英语的标注比例对比进行量化交叉验证。

Result: 研究发现波斯语在全球网络内容中占据约0.7%-0.9%的份额（排名前20）。虽然语法和新闻命名实体识别任务资源较丰富，但在自然语言推理等任务上表现不足，且存在标注方案不兼容和领域覆盖不均等问题。

Conclusion: 波斯语的标注匮乏是结构性、分布式的，而非全球性的绝对短缺。改进方向应聚焦于整合资源、优化标注方案及补齐特定专业领域和多变体语言的监督数据。
...
### [257] [Lost in Speech: Trilingual Spoken Hallucination Detection Across Audio and Transcripts](https://arxiv.org/abs/2608.24707)
*Meruyert Aristombayeva,Jason S. Lucas,Chaewan Chun,Dongwon Lee*

Main category: cs.CL

TL;DR: 该论文提出了首个多语言语音幻觉检测基准，并评估了针对英语、俄语和哈萨克语的语音及文本幻觉检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测主要集中在文本领域，针对低资源语言的语音幻觉检测研究尚属空白。

Method: 构建了包含12,013个样本的合成数据集及290个原生事实核查伪新闻数据集，对比了基于文本转录与直接音频处理的检测方法，并评估了多语言编码器及多模态解码模型。

Result: 基于转录的检测效果优于直接音频处理；在真实伪新闻检测中，模型表现出良好的泛化能力（Macro-F1 0.82-0.88），但也揭示了合成幻觉基准中模型依赖特征带来的混淆问题。

Conclusion: 语音幻觉检测需结合转录处理，且现有的合成基准在评估真实场景时存在一定的模型偏见，未来研究应关注如何区分事实性偏差与模型生成的机器风格特征。
...
### [258] [Arbitrary Polygon Oscillator: Generalizing Polygonal Synthesis to Arbitrary Shapes, Morphing, and Three-Dimensional Polyhedra](https://arxiv.org/abs/2608.24726)
*Antonio Argentieri,Francesco Scagliola*

Main category: cs.CL

TL;DR: 该论文提出了一种基于恒定弧长速度的广义多边形音频合成方法，支持任意形状、顶点数量的平滑变形以及三维多面体切片合成。


<details>
  <summary>Details</summary>
Motivation: 现有 polygonal synthesis 多限于正则多边形且参数化受限，难以实现复杂形状的平滑过渡和更广泛的音频生成应用。

Method: 使用统一的弧长引擎，通过外部顶点缓冲实现任意多边形合成；设计混合插值算法处理顶点数不等的形状 morphing；将合成扩展至三维多面体的水平切片，并结合四点 polyBLAMP 和自适应过采样进行抗锯齿处理。

Result: 成功实现了跨不同形状类型的平滑过渡，支持任意顶点配置，并通过几何缓存策略在 RNBO 中实现了高效的实时音频合成。

Conclusion: 该方法扩展了多边形合成的灵活性与表现力，无需针对特定形状进行复杂的分析推导，即可生成高质量的动态音频波形。
...
### [259] [SkillForge: Evolving Verifiable Skills for Reinforcement Learning Agents](https://arxiv.org/abs/2608.24747)
*Shidong Yang,Ziyu Ma,Tongwen Huang,Xucong Wang,Renda Li,Yiming Hu,Yong Wang,Xiangxiang Chu*

Main category: cs.CL

TL;DR: SkillForge 是一种通过环境交互实现技能持续演化与验证的 LLM 智能体框架，解决了现有方法技能库只增不删、缺乏有效性验证的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于强化学习（RL）的 LLM 智能体通常是片段化的，难以积累可复用的知识；现有的技能学习方法（如 SkillRL）缺乏对存储技能有效性的持续评估与精炼机制。

Method: 提出了 SkillForge 框架，通过引入基于证据的技能验证、多路径技能归纳以及显式的技能调用机制，使智能体能够动态优化环境动作与技能选择，并对技能库进行持续的质量控制。

Result: 在 ALFWorld、WebShop 和 AppWorld 等任务上的实验表明，SkillForge 在性能上始终优于 SkillRL，证明了持续验证技能对于提升 LLM 智能体能力的有效性。

Conclusion: SkillForge 通过闭环的技能演化机制，成功实现了技能的高效积累与质量保持，显著提升了 LLM 智能体处理复杂任务的能力。
...
### [260] [ExpConCAD: Experience-Guided Text-to-CAD Generation from Shape Descriptions with Implicit Spatial Constraints](https://arxiv.org/abs/2608.24760)
*Jingyao Liu,Jinkang Tang,Chen Huang,Wenqiang Lei,See-Kiong Ng*

Main category: cs.CL

TL;DR: ExpConCAD 是一个针对 Text-to-CAD 任务的框架，旨在通过经验增强和结构理解来补全自然语言描述中缺失的空间约束。


<details>
  <summary>Details</summary>
Motivation: 现实中的自然语言描述往往信息不足，缺乏构建有效 CAD 模型所需的关键空间约束，现有方法忽视了这一挑战。

Method: 提出 ExpConCAD 框架：首先恢复构建结构和约束范围，接着检索类似范围的约束补全经验，最后生成可执行的 CadQuery 程序。

Result: 实验证明该方法能有效补全缺失的空间约束，并验证了结构理解与经验记忆在 CAD 生成中的重要性。

Conclusion: 利用构建结构理解和外部经验记忆是解决 Text-to-CAD 中空间约束缺失问题的有效途径。
...
### [261] [The RAT: A Unified Bayesian Model for RAG Evaluation](https://arxiv.org/abs/2608.24753)
*Pius von Däniken,Felix Matthias Saaro,Mark Cieliebak,Jan Deriu*

Main category: cs.CL

TL;DR: 提出了一种基于贝叶斯推断的RAG系统评估框架，通过因式分解建模检索、拒绝和生成行为，从而更精准地诊断RAG管道中的错误传播。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG评估指标往往只关注最终答案的正确性，无法区分检索失效或生成错误，且忽略了各组件之间的交互及错误传播路径。

Method: 构建了一个贝叶斯评估模型，将检索成功、拒答行为和答案正确性进行因式分解；利用信息论解释检索标注的价值；并集成LLM-as-a-judge作为校准后的噪声观测值以降低标注成本。

Result: 实验表明该方法能揭示看似性能相当的系统在行为上的本质差异；证明了检索成功标注在评估策略遵从性上比任务成功标注更具信息量。

Conclusion: 该框架通过对RAG管道进行条件分解，为诊断组件交互提供了稳健的统计工具，并为如何高效分配标注资源提供了理论指导。
...
### [262] [Linear Probing Provides Robust and Efficient Detection of Machine-Generated Text](https://arxiv.org/abs/2608.24780)
*Gerrit Quaremba,Hanqi Yan,Elizabeth Black,Denny Vrandecic,Elena Simperl*

Main category: cs.CL

TL;DR: 该研究发现机器生成文本（MGT）与人类文本（HWT）在模型潜空间中是线性可分的，并证明简单的线性探针（Linear Probes）在检测效果和样本效率上均优于现有的复杂检测器。


<details>
  <summary>Details</summary>
Motivation: 现有监督式MGT检测器在跨域（OOD）场景下性能下降严重，且通常需要海量多样化训练数据，难以满足高效检测需求。

Method: 通过分析MGT和HWT的潜空间表征质量，证明两者在低维空间存在线性可分性；构建两种线性探针模型，在4个基准测试上对比16个基线模型，并分析其“机器味”表征能力。

Result: 线性探针在OOD场景下将AUC提升了11个百分点，且仅需不到100个样本即可达到近峰值性能，并能捕捉文本“机器化”程度的连续谱系。

Conclusion: 简单的线性探针能够发现通用的潜空间“机器方向”，不仅是稳健且高效的检测工具，也为精细化估计AI编辑痕迹提供了新路径。
...
### [263] [Structurally-bounded Agentic Graph Exploration for Evidence-Grounded Scholarly DeepSearch](https://arxiv.org/abs/2608.24809)
*Rima Hazra,Sayan Layek,Somnath Banerjee,Soumen Chakrabarti,Animesh Mukherjee*

Main category: cs.CL

TL;DR: Crase 是一种针对学术搜索设计的有界、可检查的替代方案，通过限定引文邻域搜索和逻辑验证，取代了传统的深度研究代理搜索模式。


<details>
  <summary>Details</summary>
Motivation: 解决当前深度研究代理搜索过程不透明、成本高且易陷入无限循环的问题，提供一种可追溯且高效的学术搜索机制。

Method: 通过一次性查询获取种子论文，扩展 1.5 跳引文邻域，利用逻辑蕴含验证修剪无效引文，并采用基于时效性的随机游走算法进行排名。

Result: 在 LitSearch 及 arXiv 基准测试中，Crase 的 recall@50 指标达到深度研究代理的 3 倍，且成本仅为其三分之一。

Conclusion: 通过限制搜索范围并引入可解释的验证机制，Crase 在学术搜索任务中实现了更高的效率、可解释性及性能表现。
...
### [264] [Reading Is Not Using: Retrieval, Judgment, and the Design of AI Financial Research Workflows](https://arxiv.org/abs/2608.24842)
*Miao Liu,Zhizhe Liu*

Main category: cs.CL

TL;DR: 研究发现金融领域LLM存在“检索-集成鸿沟”，即模型虽能准确检索信息，但无法将其有效整合进投资判断中，导致性能随无关上下文增加而下降。


<details>
  <summary>Details</summary>
Motivation: 当前AI金融分析系统的评估多集中于检索能力，而非信息对决策的实际影响，缺乏对检索后信息集成环节的深入研究。

Method: 通过保持核心财务信息不变，变动2,000至128,000个Token的无关上下文进行实验，并对比不同模型家族、任务及工作流架构（如块摘要与结构化重述）的影响。

Result: 即使检索准确，随着无关上下文增加，风险披露对投资判断的影响力几乎降至噪音水平；更强的模型仅能延缓此过程但无法消除；结构化重述比分块摘要更能有效传递信息。

Conclusion: AI金融分析性能取决于模型能力与工作流架构的协同；仅靠检索评估是不够的，必须优化架构以确保检索到的关键信息能被成功应用于决策过程。
...
### [265] [BrowserForge: Scaling Web Episode via Parallel Browser Sandboxes](https://arxiv.org/abs/2608.24848)
...
TL;DR: BrowserForge 是一个大规模合成网页交互数据的框架，通过并行浏览器集群和“提议者-求解者”代理循环，从开放网络中生成了超过 20 万条高质量的像素级交互轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有网页智能体训练面临数据匮乏且来源单一（仅限少量固定网站）的问题，导致模型泛化能力较差，且基于 HTML/可访问性树的方法存在脆弱性和高 token 成本。

Method: 构建了包含开放网络源挖掘、高并发浏览器沙箱管理以及“提议者-求解者”（Proposer-Solver）双代理循环的流水线，利用自动验证与清洗机制将原始操作转化为统一的思维链轨迹。

Result: 成功生成了包含 203,238 条来自不同网站的轨迹数据集；在 Online-Mind2Web 基准测试中，基于该数据微调的模型成功率从 25.66% 显著提升至 33.33%。

Conclusion: 大规模、高多样性的开放网络数据对提升纯像素输入网页智能体的性能至关重要，证明了该自动合成流水线在增强智能体泛化能力方面的有效性。
...
### [266] [Pattern-Derived Visual Swarm Games: Multi-Scale Drone-Vision States for Interception and Sustainability Audits](https://arxiv.org/abs/2608.23575)
*Faruk Alpay,Levent Sarioglu*

Main category: cs.RO

TL;DR: 该论文提出了一种将无人机视觉标注流转化为虚拟蜂群博弈状态的方法，旨在通过博弈论审计提升目标定位的鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决无人机视觉数据处理中的带宽限制与模型性能瓶颈，通过虚拟博弈框架优化复杂场景下的识别精度。

Method: 利用Bloom过滤器压缩数据，将视觉数据转化为有限零和博弈状态，结合重复马尔可夫层进行建模，并采用有限经验风险编码器优化尺度归一化高斯带宽。

Result: 通过局部屏幕调优，鲁棒性从0.526提升至0.593；在128x128尺度下，通过尺度归一化编码器将精度提升至77.6%，联合损失降低0.185。

Conclusion: 固定像素光栅并非随分辨率增加单调提升，通过博弈论审计和带宽自适应策略，能有效诊断并克服图像平面带宽限制带来的误差。
...
### [267] [Macro-Operator Generation and Predicate Selection for TAMP Operator Learning](https://arxiv.org/abs/2608.23629)
*Can Emir Bora,Emre Ugur*

Main category: cs.RO

TL;DR: 本文提出了一种通过自动学习宏观操作符（Macro-operators）和修剪冗余谓词来提升任务与运动规划（TAMP）效率的方法。


<details>
  <summary>Details</summary>
Motivation: 手动创建符号操作符是TAMP部署的瓶颈；现有学习方法忽视了多步操作的循环结构，导致长序列任务规划效率低下，且未使用的谓词评估增加了计算负担。

Method: 引入宏观操作符自动生成系统，通过挖掘训练数据中具有因果链接的操作对来压缩动作序列；同时修剪所有未被学习操作引用的谓词，从而简化符号状态空间。

Result: 在四个TAMP领域中，该方法实现了高达4.6倍的规划加速，并成功解决了基准方法无法完成的长序列任务。

Conclusion: 自动生成的宏观操作符不仅显著提升了规划速度，还在处理长序列复杂任务时对实现规划可行性具有决定性作用。
...
### [268] [Concept-Guided Exploration: Building Persistent, Actionable Scene Graphs](https://arxiv.org/abs/2608.23650)
*Noé Zapata,Gerardo Pérez,Alejandro Torrejón,Pedro Núñez,Pablo Bustos*

Main category: cs.RO

TL;DR: 提出了一种基于概念优先（concept-first）的机器人空间感知架构，通过异步概念代理直接构建语义场景图，无需预先构建全局度量地图。


<details>
  <summary>Details</summary>
Motivation: 克服传统先构建度量地图再叠加语义层的局限性，实现更具人类可解释性、可扩展且具备持续任务理解能力的机器人空间感知。

Method: 采用认知分布式架构，通过房间和门两种概念代理协作，利用层级约束传播（房间提供几何先验以辅助检测门）和预测匹配循环机制维护场景图。

Result: 机器人能够通过主动探索和增量验证，在无需全局度量地图的前提下，构建出可操作、人类可理解的室内语义布局。

Conclusion: 该架构证明了从概念出发构建空间理解的可行性，为机器人提供了一种更高效、更具语义支撑的空间导航与交互方式。
...
### [269] [Learning to Act While Waiting: RL Finetuning of Generalist Robot Policies Under Inference Latency](https://arxiv.org/abs/2608.23831)
...
TL;DR: 提出了一种名为ARLI的延迟感知强化学习框架，旨在解决大型通用机器人策略（VLA）因推理延迟导致的RL训练失效问题。


<details>
  <summary>Details</summary>
Motivation: 现代通用机器人模型体积过大导致的高推理延迟，破坏了强化学习所依赖的马尔可夫性质，导致标准RL算法在实际部署中无法有效进行微调。

Method: 利用异步推理技术隐藏延迟，并引入“状态增强”机制（包含已提交动作和推理中间观测），通过构建低延迟的RL策略来恢复近马尔可夫结构。

Result: 在模拟和现实环境的操控任务中，ARLI不仅能在标准RL失效的延迟场景下实现有效微调，在零延迟理想环境下其表现也能匹配甚至超越标准RL。

Conclusion: 通过异步架构与状态增强，ARLI成功解决了大规模模型推理延迟带来的RL收敛性问题，为机器人策略在复杂环境下的持续学习提供了可行方案。
...
### [270] [Resilience Matters for Embodied Agents System: New Metrics, Systematic Evaluation, and Optimization](https://arxiv.org/abs/2608.23839)
*Yapeng Liu,Yuanzhao Zhai,Xudong Gong,Dawei Feng,Bo Ding,Lin Wang,Huaimin Wang*

Main category: cs.RO

TL;DR: 该论文提出了一个针对具身智能体系统（EAS）的韧性评估框架，旨在超越传统的结果导向指标，通过量化系统在扰动下的恢复、稳定和扩展能力来评估其真实可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有EAS评估仅关注成功率等结果指标，忽略了系统在动态环境与扰动下的恢复过程，难以满足开放世界中高可靠性的部署需求。

Method: 引入韧性工程概念，定义了包括反弹（Rebound）、稳定性（Stability）和优雅扩展性（Graceful Extensibility）在内的综合韧性指标体系，并开发了一个可对执行过程进行诊断与优化的评估层。

Result: 通过400项家庭任务的实验发现，传统结果指标掩盖了过程层面的差异；指标引导的优化显著降低了恢复成本并提升了系统稳定性，同时揭示了韧性各维度之间的性能权衡。

Conclusion: 韧性是评估具身智能体在开放环境中部署质量的关键属性；研究建议应根据具体的部署需求，针对性地配置EAS的韧性特性。
...
### [271] [DreamLedger: Execution-Settled Credit Files for World-Model Imagination in Robot Decision Loops](https://arxiv.org/abs/2608.23863)
*Xianyao Li,Ruitong Tian,Rui Min,Fang Xu,Jing Du*

Main category: cs.RO

TL;DR: DreamLedger 提出了一种将模型预测可靠性视为持久化“信用记录”的机制，通过实时结算预测结果来管理机器人对世界模型的信任，从而提升在复杂环境中的决策安全性。


<details>
  <summary>Details</summary>
Motivation: 现有机器人模型通常依赖即时、内部的置信度信号来判断可靠性，缺乏长期的、基于真实反馈的审计与管理机制，导致在不可靠预测下的盲目决策。

Method: 引入“信用账本”系统，将预测视为索赔（claim），通过环境反馈进行结算（settlement），并根据历史信用记录（credit）动态控制规划视界或触发额外观测。

Result: 实验显示在多种模型（如DreamerV3, TD-MPC2）上，DreamLedger 将无效的“想象消耗”降低了62%，并有效减少了执行过程中的验证探测需求，同时保持了高成功率。

Conclusion: DreamLedger 提供了一种跨模型通用的可审计信任层，成功实现了在多种机器人任务中对不可靠预测的有效过滤，提升了长周期运行下的鲁棒性。
...
### [272] [Interpreting Control Latents for System Identification via Conditional Flow Matching](https://arxiv.org/abs/2608.23887)
*Dingqi Zhang,Ruiqi Zhang,Mark W. Mueller*

Main category: cs.RO

TL;DR: 该论文提出了一种利用条件流匹配将机器人的隐空间表征（latent）解码为物理模型分布的方法，从而实现对固定自适应策略的诊断与在线调优。


<details>
  <summary>Details</summary>
Motivation: 现有机器人自适应策略中的隐变量难以解释且无法直接被其他控制模块复用，限制了策略的诊断与性能优化。

Method: 利用条件流匹配（conditional flow matching）将策略的潜在变量解码为四旋翼无人机的物理模型概率分布。

Result: 该方法实现了固定策略下的在线预测调优，在执行器扰动下将位置追踪RMSE降低了23%，航向RMSE降低了45%，并能有效预测外力扰动下的轨迹误差演变。

Conclusion: 研究证明了将不透明的控制隐变量转化为可解释的物理模型集合是可行的，能有效支持冻结策略的诊断、鲁棒性分析与性能提升。
...
### [273] [Safety-aware Model Predictive Path Integral Control with Signal Temporal Logic](https://arxiv.org/abs/2608.23972)
*Yiqi Zhao,Taekyung Kim,Hideki Okamoto,Bardh Hoxha,Jyotirmoy V. Deshmukh,Lars Lindemann,Georgios Fainekos*

Main category: cs.RO

TL;DR: 提出了一种名为Safety-aware-STL-MPPI的运动规划框架，通过将STL规范编码为时变控制障碍函数（CBF），结合MPPI控制器实现高效的安全导航。


<details>
  <summary>Details</summary>
Motivation: 解决复杂约束和时间关键任务下机器人运动规划的安全性和计算效率问题。

Method: 利用采样模型预测路径积分（MPPI）控制框架，将离散时间信号时序逻辑（STL）公式转换为候选时变控制障碍函数（CBF）作为约束。

Result: 在火星车仿真和NVIDIA Isaac Lab四旋翼实验中，该方法在保持高计算效率的同时，显著提升了任务的安全性与执行成功率。

Conclusion: 该方法成功结合了采样规划的高效性和CBF在满足时序逻辑约束方面的优势，是一种适用于复杂环境下的高效安全规划方案。
...
### [274] [Sensorless damage-safe grasping](https://arxiv.org/abs/2608.23983)
*Yusei Shuto,Danilo Vasconcellos Vargas*

Main category: cs.RO

TL;DR: 提出了一种基于形变约束的机器人水果采摘抓取控制方法，无需力传感器即可实现不同成熟度水果的无损抓取。


<details>
  <summary>Details</summary>
Motivation: 由于水果成熟度导致刚度差异巨大，固定抓取力难以兼顾安全与稳定性，且传统传感器方案成本高或复杂。

Method: 利用电机编码器和力矩信号，通过估计物体压缩应变（strain）并设定阈值 $\varepsilon$ 来控制抓取深度，将抓取过程转化为对形变的直接约束。

Result: 在仿真与3D打印样件测试中，该方法相比固定力控制显著降低了柔软物体的破损率，并在保证抓取成功率的同时实现了更低的平均抓取力。

Conclusion: 该方法提供了一种可量化的损伤保护机制，具备尺寸缩放性和直观的参数配置，是实现通用、柔性机器人采摘的有效方案。
...
### [275] [Bridging Teacher Expectations and Robot Learning via Coupling Dynamics](https://arxiv.org/abs/2608.23994)
*Evan Dallas,Sean Dallas,Wing-Yue Geoffrey Louie*

Main category: cs.RO

TL;DR: 该论文提出了一种衡量人机教学中“耦合动态”的分类量表，用于评估其对教学效果及人类认知的影响。


<details>
  <summary>Details</summary>
Motivation: 随着人机教学从离线学习转向人机实时交互，目前缺乏关于这种动态耦合如何影响教学效果和人类感知的研究。

Method: 结合人类学习理论，构建了一套分类人机交互中耦合动态的量表，并通过对现有文献的归纳分析，探讨了耦合动态及心智模型偏差对教学过程的影响。

Result: 识别出耦合动态的差异显著影响教学效果，并揭示了人类教师的心智模型与机器人学习系统之间的偏差是影响教学感知与成效的关键因素。

Conclusion: 该研究为评估人机实时协作提供了理论框架，强调了在设计人机教学系统时，理解耦合机制与管理用户心理模型偏差的重要性。
...
### [276] [NeurRAFT: Robot Motion Planning via Anchor-Level Flow Matching with Clearance-Aware Preference Tuning](https://arxiv.org/abs/2608.24026)
*Sibo Tian,Chang Liu,Minghui Zheng,Xiao Liang*

Main category: cs.RO

TL;DR: NeurRAFT 是一种基于流匹配的端到端神经运动规划器，通过锚点生成和直接偏好优化（DPO）实现安全、高效的路径规划。


<details>
  <summary>Details</summary>
Motivation: 现有神经运动规划器在复杂环境下难以保证避障，且计算冗余，模仿学习难以区分临界碰撞轨迹。

Method: 使用紧凑锚点代替密集路径点，通过雅可比加权损失训练，并应用直接偏好优化（DPO）引导模型偏向更高间隙的避障轨迹。

Result: 实验证明 NeurRAFT 在复杂场景中优于现有最先进规划器，并在真实 Franka 机器人上实现了抗噪和抗遮挡的零样本迁移。

Conclusion: NeurRAFT 通过优化表示结构与直接偏好调整，有效提升了神经运动规划的安全性与鲁棒性。
...
### [277] [Design-to-Plan: A Large Language Model-Based Multi-Agent Framework for Manufacturing Process Planning from 3D CAD Models and 2D Engineering Drawings](https://arxiv.org/abs/2608.24039)
*Muhammad Tayyab Khan,Lequn Chen,Wenhe Feng,Seung Ki Moon*

Main category: cs.RO

TL;DR: 提出了一种名为Design-to-Plan的LLM驱动多智能体框架，旨在实现从设计到制造过程规划的端到端自动化。


<details>
  <summary>Details</summary>
Motivation: 现有方法多局限于孤立的子任务，难以处理3D CAD、2D图纸、材料及领域规则等异构设计信息之间的复杂推理链。

Method: 构建了一个多智能体架构，通过编排器（Orchestrator）协调负责特征识别、图纸分析、上下文融合、知识检索及工艺决策的专业智能体，并结合确定性模块以确保决策的结构化与可追溯性。

Result: 在300个基准测试中，该框架实现了100%的下游智能体任务成功率，刀具选择F1分数达95.9%-97.6%，并显著降低了60%-68%的Token消耗。

Conclusion: 结构化的LLM多智能体协作能够有效弥合设计表示与制造知识之间的鸿沟，实现可扩展、高效且可追溯的制造流程规划自动化。
...
### [278] [Hierarchical Skill Retrieval for Data-Efficient Adaptation of Vision-Language-Action Models](https://arxiv.org/abs/2608.24042)
*Haoran Hao,Shahram Najam Syed,Jeff Schneider,Jeffrey Ichnowski*

Main category: cs.RO

TL;DR: 提出了一种层次化技能检索（HSR）框架，通过将长任务分解为子技能进行针对性检索，以提升视觉-语言-动作（VLA）模型在少样本下的任务适应能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在面临新任务时，由于缺乏高质量标注数据表现不佳，而传统的基于视觉或整体任务检索方法难以利用复杂任务中可重用的子技能。

Method: 提出了层次化技能检索（HSR），包括将任务分解为技能序列、基于语义合理性与技能可靠性评估计划、结合子任务语言检索与行为特征重排的混合检索，以及两阶段预训练与微调策略。

Result: 在LIBERO基准测试和现实世界机器人任务中，HSR分别将平均成功率提升了10.3%和21.3%。

Conclusion: 结构化的技能级检索方法能有效增强VLA模型的数据高效适应性，是解决长时程机器人操作任务的有效方案。
...
### [279] [SIREN-Bench: Behavior-Driven Generation and Evaluation of Emergency-Vehicle Interactions](https://arxiv.org/abs/2608.24094)
*Yicheng Zhu,Tianmu Zhao,Haoxin Leng,Fan Zuo,Tao Li,Zilin Bian*

Main category: cs.RO

TL;DR: SIREN 是一个结合 SUMO 和 CARLA 的协同仿真平台，专门用于评估自动驾驶车辆与紧急车辆（EMV）之间的交互安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的数据集和仿真平台缺乏对紧急车辆通行时引发的复杂交通交互（如避让、变道）的精细行为控制和感知支持。

Method: 构建了 SIREN 平台，通过 SUMO 处理宏观交通逻辑，CARLA 处理连续控制与传感器仿真，并提出了 SIREN-Bench-v1 基准测试集，涵盖多种紧急交互场景。

Result: 通过对多类模型（检测、预测、视觉语言模型）的评估，发现交通清理场景最难检测，交叉路口通行最难预测，且现有模型在极端场景下表现远差于正常交通。

Conclusion: SIREN 为研究自动驾驶在紧急交互场景下的安全性提供了一个可扩展的基准测试和数据生成平台，揭示了当前算法在处理复杂交通行为时的局限性。
...
### [280] [TrAct: Bridging Robot Control and Visual Prediction with Visual Tracks](https://arxiv.org/abs/2608.24101)
*Zhi Cao,Howard Ji,Kevin Zhang,Kuangzhi Ge,Li Fei-Fei,Jiajun Wu,Huang Huang*

Main category: cs.RO

TL;DR: 提出了一种名为TrAct的机器人决策框架，利用“视觉轨迹（visual tracks）”作为行动与视觉预测之间的中间接口，提升了机器人世界模型的预测能力和任务执行成功率。


<details>
  <summary>Details</summary>
Motivation: 机器人动作通常具有特定于机身的局限性，与图像空间的视觉变化对齐较弱，导致难以作为高效的条件信号用于世界模型进行未来视频预测。

Method: 提出了TrAct框架，包含三个部分：1. VLAT（视觉-语言-动作-轨迹模型）联合预测动作与轨迹；2. TWM（轨迹条件世界模型）预测未来视觉结果；3. VLAC（视觉-语言奖励模型）评估预测结果并选择最优行动。

Result: 在LIBERO-INTEGRAL基准测试中成功率从27%提升至55%，在真实世界Franka机器人任务中从49%提升至76%，且视频预测质量优于基于动作的世界模型。

Conclusion: 视觉轨迹是机器人控制与视觉预测之间有效的共享接口，能够显著提升世界模型的准确性及机器人的泛化能力。
...
### [281] [Trajectory-Level Continuous Action Representation for Robotic Manipulation](https://arxiv.org/abs/2608.24111)
*Tong Yang,Jingkai Jia,Yuecheng Xu,Xueyao Chen,Chi Zhang,Wenqiang Zhang*

Main category: cs.RO

TL;DR: CAT 是一种机器人操作的轨迹级连续动作表征框架，通过将动作轨迹编码为连续潜在标记，解决了现有视觉运动系统在不同控制频率下的冗余和时间参数化依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在动作表征上与控制频率耦合，或依赖固定的时间参数化，导致高采样率下的冗余，且难以有效模拟关键运动。

Method: 将固定实时间隔内的动作轨迹编码为一组连续潜在标记；引入频率感知位置编码以建立共享时间坐标系；通过轨迹级正则化提升潜在表征的稳定性。

Result: 在 LIBERO、MimicGen 及真实长序列操作任务中，CAT 在多种模型架构和控制频率下，均表现出优于现有的 VQ-based 和连续视觉运动基线的成功率。

Conclusion: 轨迹级连续动作建模为解决机器人操作中控制频率变化带来的扩展性问题提供了有效方案，能实现更灵活、稳定的控制策略。
...
### [282] [PonderPounce: A Pretrained MLLM as an Episode Context Engine for Robot Control](https://arxiv.org/abs/2608.24115)
*Suhwan Choi,Jaeyoon Jung,Sungkyung Kim,Yunsung Lee,Youngjae Yu*

Main category: cs.RO

TL;DR: PonderPounce 是一种利用多模态大语言模型（MLLM）原生因果上下文作为机器人记忆的策略，无需额外记忆模块即可实现高效的具身智能控制。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作（VLA）模型未能充分利用 MLLM 的长时记忆能力，通常需要专门设计的记忆机制，导致结构复杂或预训练困难。

Method: 采用双系统架构：Ponder（System2 MLLM）负责处理长历史信息并生成认知推理，Pounce（System1 VLA）负责接收认知输入并执行动作；两者通过异步接口联合端到端训练。

Result: 在 RoboMME 基准测试中显著优于现有基线（9B模型达到 60.83%），在 RoboCasa-DC 上也展现了优越的性能，且系统延迟极低，支持 20Hz 实时动作输出。

Conclusion: 利用 MLLM 原生因果上下文作为机器人记忆是实现高效长时具身规划的有效路径，避免了专用记忆模块带来的复杂性。
...
### [283] [Coverage Planning for Robotic Tooth Preparation in Densely Constrained Environments](https://arxiv.org/abs/2608.24155)
*Yunwen Li,Chen Chen,Xiangjie Yan,Chang Shu,Jianxia Hou,Shiji Song,Xiang Li*

Main category: cs.RO

TL;DR: 提出了一种用于全冠牙体预备的自动化机器人系统，通过解剖感知路径规划和避障策略实现高精度牙体切削。


<details>
  <summary>Details</summary>
Motivation: 牙体预备要求极高的精度，且口腔空间受限，传统手工操作难度大，现有机器人系统在复杂狭窄空间内难以平衡修复质量与临床安全。

Method: 开发了一种解剖感知工具路径规划算法以确保贴合预备模型并保护邻牙，同时引入了间隙导向的末端执行器偏航角分配策略以避免软组织干扰。

Result: 实验证明该系统在模拟实验中实现了平均0.117 mm (RMSE) 的几何偏差，能够满足修复质量和临床安全要求。

Conclusion: 该研究验证了机器人辅助自主全冠牙体预备的可行性和有效性，在复杂口腔环境下实现了高精度的自动化修复准备。
...
### [284] [Robust Slip Detection and Material Classification via Spatiotemporal Transformers on a Uniformly-Illuminated Visuo-Tactile Sensor](https://arxiv.org/abs/2608.24162)
*Ziyang Ma,Yuhao Sun,Zichen Ai,Xiangyang Ji,Bin Fang*

Main category: cs.RO

TL;DR: 提出了一种具备高精度深度重建能力的视觉触觉传感器及双头TimeSformer网络，实现了触觉状态的细粒度滑移方向检测。


<details>
  <summary>Details</summary>
Motivation: 现有滑移数据集多限于二分类，缺乏对滑移方向的精细化感知，无法满足复杂机器人操作的需求。

Method: 设计了具有均匀RGB照明的视觉触觉传感器以实现亚毫米级深度重建；构建了包含15种物体的多任务数据集；利用双头TimeSformer网络处理动态时空滑移，并用ResNet-50进行静态物体分类。

Result: 在未见物体上的3类接触状态预测准确率为95.5%，8类细粒度滑移方向分类准确率为91.5%，静态物体分类准确率为98.8%。

Conclusion: 该软硬件一体化框架为机器人复杂操作提供了高保真反馈和强大的多模态感知基准。
...
### [285] [CARO: Contact-Agnostic Residual Observation for Zero-Shot Robust Quadruped Locomotion](https://arxiv.org/abs/2608.24217)
*Zihan Yang,Shixuan Han,Kexin Guo,Xiang Yu*

Main category: cs.RO

TL;DR: CARO 是一种针对机器人策略自适应的“接触无关”残差观测框架，无需传感器或视觉辅助即可实现鲁棒控制。


<details>
  <summary>Details</summary>
Motivation: 解决传统强化学习控制策略在面对动力学偏差（如负载变化、质心偏移）时，对传感器和复杂环境状态估计过度依赖的问题。

Method: 将固定基座的 Euler--Lagrange 模型嵌入控制循环，通过扰动观测器提取动力学失配信号，并利用该残差信号引导策略进行在线自适应。

Result: 在零样本模拟和实机测试中，面对分布外负载、质心移动及复杂地形时，表现出显著优于传统方法的鲁棒性和泛化能力。

Conclusion: CARO 提供了一种无需额外传感器或特殊训练即可实现机器人实时动力学自适应的有效途径，显著提升了策略的 sim-to-real 迁移效果。
...
### [286] [NVIDIA Cosmos-H-Dreams: Real-Time Generative Physics Simulation for Surgical Robotics](https://arxiv.org/abs/2608.24199)
...
TL;DR: Cosmos-H-Dreams 是首个实现实时交互的生成式手术机器人模拟系统，通过蒸馏技术将复杂的世界模型转化为高效的可控仿真器。


<details>
  <summary>Details</summary>
Motivation: 传统手术模拟器难以兼顾逼真的视觉效果与软组织动力学，且物理实验成本高昂，缺乏实时交互能力。

Method: 利用动作条件生成模型（Cosmos-H-Surgical-Simulator）作为教师模型，采用 Self Forcing 蒸馏技术将其转化为因果推断的学生模型，并结合 NVIDIA FlashDreams 流式推理库实现实时渲染。

Result: 系统在单张 RTX PRO 6000 GPU 上达到约 160 FPS 的推理速度，支持多种控制器（如键盘、VR头显、手术机器人控制台）及闭环策略控制。

Conclusion: Cosmos-H-Dreams 提供了一个开源、高可控的交互式手术世界模型，为外科教育、合成数据生成及术中决策支持奠定了基础。
...
### [287] [A Durable Vision-Based Tactile Fingertip for Robotic Manipulation](https://arxiv.org/abs/2608.24242)
*F. Richard Cottrell,Megha H. Tippur,Edward H. Adelson*

Main category: cs.RO

TL;DR: 研究人员开发了一种具备极高耐用性的视觉触觉传感器指尖，通过改进保护膜和结构设计，显著提升了在严苛环境下的使用寿命。


<details>
  <summary>Details</summary>
Motivation: 现有的商业视觉触觉传感器（如GelSight Mini和DIGIT）极易受磨损和重复载荷的影响，限制了其在工业等严苛机器人场景中的应用。

Method: 使用非着色、纹理化的热塑性聚氨酯(TPU)保护膜包裹软硅胶凝胶，并采用可更换的传感卡盒设计；通过旋转滚筒砂磨测试和重复探针压力测试，与商业传感器进行耐久性对比。

Result: 该传感器在砂磨测试下的耐久性提升了两个数量级以上；在重复载荷测试中，所有样品至少维持了5-8天的功能性，而商业传感器仅能维持25-35分钟。

Conclusion: 通过增强材料耐用性和模块化设计，该方法为实现可维护的、适用于严苛机器人应用的视觉触觉传感提供了有效途径。
...
### [288] [NeuralParker: A Reinforcement Learning Planner for Irregular Parking Environments](https://arxiv.org/abs/2608.24485)
*Zihan Wang,Bai Huang,Yang Guan,Xiao Li,Haoyu Xu,Naizheng Wang,Shengbo Eben Li*

Main category: cs.RO

TL;DR: NeuralParker 是一种基于强化学习的混合式任意姿态自动泊车规划器，能够处理非结构化环境中的长距离泊车路径规划。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的泊车方案多依赖局部观测，难以进行长距离路径推理，且无法很好地应对不规则边界和特定的终点姿态要求。

Method: 使用目标相对顶点表示法编码全局障碍物与环境边界信息，并结合“弧线策略+循环内终端集成（Terminal Ensemble）”的方法，通过立方埃尔米特插值选择最优路径。

Result: 在自建的长距离路径选择基准测试中，NeuralParker 在规划成功率和轨迹质量上优于现有基线模型，并成功在真实配送车辆上进行了实地部署验证。

Conclusion: 该方法通过引入全局环境表征与灵活的终端路径选择机制，有效提升了自动泊车在复杂、非结构化环境下的远距离规划能力与计算效率。
...
### [289] [RoG-DAgger: Rollout-Guided Post-Training for End-to-End Driving](https://arxiv.org/abs/2608.24525)
*Liangyu Zhong,Joachim Sicking,Fabian Hueger,Hanno Gottschalk*

Main category: cs.RO

TL;DR: RoG-DAgger 是一种针对自动驾驶端到端系统的后训练框架，通过运动学推演（kinematic rollouts）改进了传统的 DAgger 方法，解决了数据分布偏移和专家决策不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型在闭环场景下因训练-推理分布不匹配，容易产生累积误差导致安全性问题，现有 DAgger 方法存在专家方案受限、接管时机不准及专家特权信息不可用等挑战。

Method: RoG-DAgger 引入短视距运动学推演来构建高质量的安全关键状态示范；通过推演评估扩展专家的方案空间；利用“推演可解性”精准确定接管时机；并对齐专家与学生的视野以提供兼容性监督。

Result: 在 Bench2Drive 上将 SimLingo 驾驶分数提升 5.3 分，在 Longest6 v2 上驾驶分数翻倍（22 提升至 44），在 Fail2Drive 分布外测试中成功率从 55% 提升至 66%。

Conclusion: RoG-DAgger 有效缓解了端到端驾驶中的训练-推理不匹配问题，显著增强了模型在安全关键状态及分布外场景下的鲁棒性和决策能力。
...
### [290] [Fiber Optic Sensing Glove for High Performance Dexterous Manipulation Capture](https://arxiv.org/abs/2608.24572)
*J. D. Peiffer,Taylor Niehues,Li Guan,Ziyi Kou,Ergys Ristani*

Main category: cs.RO

TL;DR: 提出了一种基于多芯光纤传感技术的全手姿态跟踪手套，通过重建光纤的三维形状实现高精度的手部运动捕捉。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉方法在遮挡和复杂光照下表现不佳，而传统传感手套易受磁干扰且精度不足，难以满足高精度机器人操作和远程操纵的需求。

Method: 利用多芯光纤传感技术获取完整三维形状，通过专门的流水线将其注册到手部坐标系，并开发了一种基于曲线约束的逆运动学求解器以60Hz频率实时重建手部姿态。

Result: 在多项灵巧操作任务中，手套的指尖位置平均误差为7.2mm；通过工厂校准，误差可进一步降低至4.9mm，且具备跨用户和跨会话的普适性。

Conclusion: 该系统有效解决了现有传感手套的精度和环境干扰问题，为机器人灵巧手研究及双臂虚拟遥控操作提供了高保真的数据采集手段。
...
### [291] [Gripper-aware Vision Language Action Models](https://arxiv.org/abs/2608.24603)
...
TL;DR: 该研究提出了MiGA数据集和GVLA模型，解决了视觉语言动作模型（VLA）在处理不同机械臂夹爪类型时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型多假设夹爪具有通用性，忽视了不同夹爪（如平行夹持与吸盘）在操作策略上的显著差异，且缺乏针对多夹爪的训练数据。

Method: 构建了包含5种夹爪、10.3万条演示的MiGA数据集；提出了GVLA架构，引入了多夹爪分词器（Tokenizer）和基于适配器的策略路由机制，以实现夹爪条件的表示学习。

Result: 实验证明GVLA在模拟及真实机器人环境中优于现有基线，并显著提升了对新物体、未见任务的零样本泛化和少样本适应能力。

Conclusion: 通过显式的夹爪编码与策略路由，模型能够有效处理不同机器人硬件的差异，为实现通用的具身智能操作提供了重要支持。
...
### [292] [VIP: Variation-based Iterative-learning Planning for Robotic Navigation](https://arxiv.org/abs/2608.24618)
*Shuli Lv,Pengda Mao,Chen Min,Li Hong,Runxiao Liu,Shuai Wang,Quan Quan*

Main category: cs.RO

TL;DR: 提出了一种基于变分迭代学习的运动规划框架（VIP），通过在函数空间直接更新规划指令，实现了高效率、低计算复杂度的运动规划。


<details>
  <summary>Details</summary>
Motivation: 传统轨迹参数化方法在处理复杂、密集障碍物环境及多机器人场景时，计算成本随预测时间跨度和轨迹维度增加而过快增长。

Method: 将规划指令视为无限维函数空间中的元素，采用变分更新法直接更新控制命令，避免了大规模离散轨迹优化，实现模型在环或机器人在线学习。

Result: 算法单次迭代计算复杂度为 O(n)，在不同机器人平台、多机器人集群场景下展现出优异的计算效率和可扩展性。

Conclusion: VIP框架为复杂环境下的机器人运动规划提供了一种高效、通用且具备良好伸缩性的解决方案。
...
### [293] [GaussianWAM: Distilling Geometry and Semantics from 3D Gaussian Fields into World-Action Models](https://arxiv.org/abs/2608.24714)
*Zijian Zhang,Yuqing Jiang,Weitao Zhou,Minglei Li,Jinhao Zhang,Yao Mu,Xiaofan Li,Hao Zhao,Haibao Yu*

Main category: cs.RO

TL;DR: GaussianWAM 提出了一种利用 3D 高斯场将几何和语义监督注入世界动作模型 (WAM) 的训练框架，无需增加推理时的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有的 WAM 在视频潜在空间中侧重于视觉预测，缺乏对跨视图几何结构和局部对象语义的有效编码。

Method: 利用 3D 高斯场作为媒介，将深度、相机参数及预训练模型（如 CLIP/VGG）提取的异构信号绑定并渲染为空间对齐的目标，通过蒸馏将其注入 WAM 的观测表征中。

Result: 在 LIBERO-Plus 基准测试中，GaussianWAM 显著提升了 FastWAM 和 Cosmos Policy 的表现，同时在 RoboTwin 和真实机器人任务中展现了良好的泛化能力。

Conclusion: 通过训练时的 3D 高斯蒸馏，可以在不改变模型架构的前提下增强 WAM 对几何与语义的理解，显著提升机器人操作任务的成功率。
...
### [294] [Fiber Bragg Grating Whiskers for Bioinspired Hydrodynamic Perception on Underwater Robots](https://arxiv.org/abs/2608.24724)
*Hao Li,Tianyu Tu,Siyue Yao,Ziyang Chang,Juhyun Jung,Xiaochi Xie,Long Yin Chung,Tian-Ao Ren,Genliang Chen,Mark Cutkosky*

Main category: cs.RO

TL;DR: 该研究开发了一种受海豹胡须启发的纤维布拉格光栅（FBG）传感器，赋予水下机器人被动感知水动力尾迹及追踪目标的能力。


<details>
  <summary>Details</summary>
Motivation: 受海豹利用胡须在浑浊或黑暗水中追踪流体动力轨迹的生物学特性启发，旨在解决小型水下机器人在复杂环境中的导航与目标追踪问题。

Method: 设计并制造了具有非均匀锥度与椭圆形横截面的FBG胡须传感器；通过控制拖曳实验测试流速响应与振动特性；利用俯仰箔片产生涡流进行探测实验；并通过单胡须传感器引导水下机器人进行路径选择实验。

Result: 该传感器展现出良好的流速响应（0.1-0.6 m/s）及角度依赖性，显著降低了自激振动；在路径选择任务中，机器人仅凭胡须信号即可在20次实验中成功识别17次（成功率85%）。

Conclusion: 该生物启发式水动力传感方案证明了其在水下机器人导航及目标追踪任务中的有效性与实用性。
...
### [295] [One-Shot Learning from Demonstration of Contact-Rich Robotic Manipulation by Identifying Physical Interactions](https://arxiv.org/abs/2608.24741)
*A. H. G. Overbeek,H. van der Kooij,M. Vlutters*

Main category: cs.RO

TL;DR: 提出了一种基于物理交互建模的演示学习（LfD）方法，通过显式模拟接触状态实现复杂操纵任务的稳健复现。


<details>
  <summary>Details</summary>
Motivation: 现有LfD方法忽视了操纵任务中至关重要的物理接触行为，导致在复杂、接触密集型任务中泛化能力和稳健性不足。

Method: 结合演示中的物理交互信息，设计了一种混合位置-力控制器，根据接触触发条件实时调整轨迹执行。

Result: 通过开门、拧螺丝等实验验证，该方法仅需单次演示即可实现高稳健性、强泛化能力和在线自适应的操纵任务复现。

Conclusion: 显式建模物理交互是实现可解释、少样本机器人操纵学习的关键，有效弥补了现有LfD研究在处理接触任务时的不足。
...
### [296] [Latent Action as Intention Enables Efficient Future Imagination for World Action Models](https://arxiv.org/abs/2608.24882)
...
TL;DR: LAWA是一种新型世界动作模型（WAM），通过利用紧凑的潜在动作表示未来意图，在实现高效推理的同时保持了优秀的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的Fast-WAM虽然通过去除未来观察生成降低了延迟，但牺牲了泛化能力；而全功能WAM在测试时生成未来观察会导致高延迟。研究旨在寻找一种既高效又能保留未来想象能力的替代方案。

Method: 引入LAWA架构，使用离散分词器进行无动作预训练，将未来意图编码为紧凑的潜在动作；模型通过联合去噪连续潜在状态和可执行动作块进行训练，并在推理时剔除视频生成分支。

Result: 在RoboCasa任务中，LAWA在少样本和全数据设置下分别达到65.6%和80.8%的成功率，显著优于Fast-WAM，且比Joint-WAM降低了42.9%的推理延迟。

Conclusion: 研究证明通过紧凑的潜在动作来保留未来想象能力，可以实现性能、泛化能力与推理延迟之间的最佳平衡。
...
### [297] [Do Robotic World Models Really Follow Actions? Diagnosing and Aligning Action-Conditioned Generation for Policy Learning](https://arxiv.org/abs/2608.24885)
*Sixiang Chen,Jiaming Liu,Jixian Wu,Yichen Guo,Tinghao Wang,Siyuan Qian,Hao Chen,Jiajun Cao,Jian Tang,Shanghang Zhang*

Main category: cs.RO

TL;DR: 本文提出了WorldEcho基准测试，揭示了现有的动作条件世界模型在处理非专家动作序列时的不足，并提出了WorldSync方法来增强模型的动作执行能力与仿真可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型通常在专家演示数据上训练，缺乏对离线（off-expert）动作指令的有效验证，导致在进行策略评估和优化时缺乏鲁棒性。

Method: 引入WorldEcho基准进行多维度评估；提出WorldSync训练框架，通过扩大动作后果的分布覆盖、利用Action-Forcing Expert进行表征对齐，以及增强干预效应的一致性来提升模型性能。

Result: 实验表明，WorldSync在RoboTwin基准和真实机器人任务中显著提高了动作跟随精度，且作为仿真器能有效支持迭代策略改进，提升任务成功率。

Conclusion: 通过增强动作与视觉轨迹间的因果一致性和表示对齐，可以显著提升世界模型作为仿真器的可靠性，从而更有效地辅助机器人学习。
...
### [298] [A tale of perfect fit and phantom optima: how data-driven models can fail in real-time optimization](https://arxiv.org/abs/2608.23885)
*Prithvi Dake,Rahul Bindlish,James B. Rawlings*

Main category: eess.SY

TL;DR: 研究表明，即便机器学习模型能准确拟合过程数据，也未必能准确预测经济最优解，且模型容易产生“虚假最优解”。


<details>
  <summary>Details</summary>
Motivation: 数据驱动模型在实时优化（RTO）中的可靠性尚未得到充分验证，且模型在预测准确性与经济最优性之间是否存在鸿沟尚不明确。

Method: 利用醋酸乙烯单体工艺基准，对比了结构化混合模型（物理知识+神经网络）与全数据驱动模型（神经ODE），并分析了训练过程中优化器对模型性能的影响。

Result: 模型虽能高精度拟合数据，但在经济优化任务中表现迥异，不仅产生大量虚假最优解，且模型权重易受随机梯度下降训练过程的干扰而偏离实际最优状态。

Conclusion: 仅依靠预测指标的拟合度不足以保证RTO的经济有效性；数据驱动模型在部署前必须在决策导向的基准测试中验证其对最优解的恢复能力。
...
### [299] [Energy-Aware Performance Evaluation of Nonlinear Mechatronic Systems Under Matched-Tracking Conditions](https://arxiv.org/abs/2608.23578)
*Bhanuka Dayawansa*

Main category: eess.SY

TL;DR: 提出了一种结合轨迹跟踪误差与执行器能量消耗的评估框架，以更全面地评估机电系统的控制性能。


<details>
  <summary>Details</summary>
Motivation: 现有的轨迹跟踪指标（如RMSE）仅关注输出，忽略了产生运动所需的执行器能量成本，特别是在非线性系统中这一局限性尤为突出。

Method: 引入能量感知评估框架（EAPI指标），将跟踪误差与累积执行器能量相结合，并在线性与非线性系统中通过PD控制进行对比实验。

Result: 实验表明，在跟踪精度相当的情况下，非线性系统由于其非线性刚度和摩擦效应，表现出显著更高的能量消耗。

Conclusion: 仅依靠轨迹跟踪指标是不够的，引入能量指标能为机电系统的性能评估提供更全面、更具参考价值的依据。
...
### [300] [A Hybrid Two-Stage Machine Learning Pipeline for Fault Detection and Classification in Power Transmission Systems](https://arxiv.org/abs/2608.23726)
*Sahil Manikshete,Atharva Gujarathi,Thanh Long Vu,Akhtar Hussain,Van-Hai Bui*

Main category: eess.SY

TL;DR: 提出了一种针对高压输电网故障检测的两阶段机器学习流水线，通过解耦检测与分类解决数据不平衡和特征重叠问题。


<details>
  <summary>Details</summary>
Motivation: 现有单模型分类器在处理高压输电网不平衡数据和重叠故障特征时表现不佳，导致关键安全故障漏检。

Method: 采用两阶段架构：第一阶段利用孤立森林与可选监督二分类器融合进行异常检测，第二阶段对异常样本进行随机森林多分类；通过引入对称分量等特征工程增强区分度。

Result: 在TLFaultDataset上端到端准确率从31.3%提升至95.8%；在单点数据集上达到97.25%的准确率，且单样本处理速度仅为0.05ms。

Conclusion: 该方法通过解耦处理显著提升了故障检测性能，证明了零序特征对于区分复杂故障类型的有效性，且无需复杂基础设施即可高效部署。
...
### [301] [Dynamical System-Based Imitation Learning and Neuroadaptive Control for Trajectory Recovery in Autonomous Ships](https://arxiv.org/abs/2608.23924)
*Yeyson A. Becerra-Mora,José Ángel Acosta*

Main category: eess.SY

TL;DR: 提出了一种结合动力系统模仿学习与神经自适应控制的USV轨迹跟踪框架，实现了抗干扰能力与轨迹保真度的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于动力系统的模仿学习在处理海洋扰动时，往往为了保证收敛性而牺牲了对人类示范路径的还原精度。

Method: 构建了混合学习控制架构，将动力系统（DS）生成的模仿学习参考路径与神经自适应控制器相结合，通过引入行为跟踪机制应对外界扰动。

Result: 通过Marine Systems Simulator验证，该方法在复杂机动任务中展现出比传统策略更高的轨迹跟踪精度和鲁棒性。

Conclusion: 该架构有效解决了USV在模仿学习中轨迹保真度与抗扰动性之间的矛盾，实现了更具“人类感”的自主控制。
...
### [302] [Critical Weather Scenario Screening Using Weather-to-Voltage (W2V) Predictive Modeling](https://arxiv.org/abs/2608.23872)
*Sol Lim,Min-Seung Ko,Hao Zhu*

Main category: eess.SY

TL;DR: 提出了一种基于可微分模型的关键天气场景筛选框架，用于识别导致电力系统高压（HV）事件的极端天气条件。


<details>
  <summary>Details</summary>
Motivation: 传统的天气相关应急分析仅局限于组件级故障风险，无法有效识别导致电网级电压越限的复杂天气驱动因素。

Method: 利用可微分的天气转电压（W2V）预测模型，通过在紧凑潜在空间中进行梯度更新，寻找能使电压临界评分最大化的天气扰动，并引入非负性约束和L1范数约束以确保物理一致性及扰动的稀疏解释性。

Result: 在6717节点系统实验中验证了该框架能有效识别电网高压风险，发现最易引发危机的场景多集中在风电容量大的区域，与实际运行经验高度吻合。

Conclusion: 该方法揭示了天气不确定性引发系统级电压越限的潜在威胁，提供了一种灵敏度感知的漏洞排序工具，优于单一天气场景的电压分析。
...
### [303] [Trusted Polytopic Action Sets for Fast Planning in Underactuated Systems](https://arxiv.org/abs/2608.24019)
*Akshay Jaitly,Siavash Farzan*

Main category: eess.SY

TL;DR: 提出了一种基于“多面体动作集”（PAS）的欠驱动系统快速运动规划方法，通过在动作空间中构建凸可行集，实现了高效的在线运动规划。


<details>
  <summary>Details</summary>
Motivation: 欠驱动系统的动态可行轨迹位于函数空间的流形上，传统的凸优化方法难以直接应用，导致运动规划速度慢且计算复杂。

Method: 通过标称轨迹构建局部有限维动作坐标，利用仿射轨迹映射将非线性动力学转化为线性约束，并结合IRIS算法在动作空间中提取可信的凸内近似集，构建可重用的PAS。

Result: 在杂乱场景中规划速度比传统运动学RRT快14-78倍，且在非线性欠驱动基准测试中将终端误差降低了26-86%。

Conclusion: 该方法成功结合了非线性动力学的高保真度与凸优化的计算高效性，证明了利用动作空间凸近似进行长期规划的有效性。
...
### [304] [On the Reduced Rank Hypersurface Germ Condition for Near-Controllability of Bilinear Systems](https://arxiv.org/abs/2608.24029)
*Wenyu Zhao,Lin Tie*

Main category: eess.SY

TL;DR: 本文指出了现有双线性系统“约化秩超曲面芽条件”（RRHGC）验证算法中的缺陷，并提出了一种基于Darboux多项式刻画的新算法。


<details>
  <summary>Details</summary>
Motivation: 全局可控性验证在双线性系统中通常很困难，现有基于RRHGC的代数验证算法存在漏判特殊情况的缺陷，需要更完备的判定方法。

Method: 利用Darboux多项式刻画来重新构建测试RRHGC的算法，以确保覆盖之前算法未能处理的特殊情况。

Result: 提出了一个改进的验证算法，能够有效处理原算法中被忽略的特殊案例，并通过具体示例验证了算法的有效性。

Conclusion: 该改进算法增强了通过代数方法验证双线性系统全局近可控性的鲁棒性与适用性。
...
### [305] [Event-Triggered Pinning Impulsive Control of Complex Networks with Actuation Delays: Stability Analysis and Zeno-Free Conditions](https://arxiv.org/abs/2608.24074)
*Ethan Astri,Hamza Saleem,Kexue Zhang*

Main category: eess.SY

TL;DR: 该论文研究了存在执行延迟的复杂网络在事件触发脉冲控制下的稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有脉冲控制方案大多忽略了从事件触发到脉冲执行之间的物理延迟，这可能导致系统不稳定，因此需要建立更符合实际工程的控制模型。

Method: 通过构造适当的李雅普诺夫函数，分析延迟区间的网络动力学，推导了与延迟相关的渐近稳定性充分条件，并利用谱分析设计了节点选择策略。

Result: 给出了保障系统渐近稳定的显式条件，证明了系统不存在芝诺行为（Zeno behavior），并通过蔡氏电路仿真验证了方法的有效性。

Conclusion: 该研究成功构建了考虑执行延迟的事件触发脉冲控制框架，揭示了网络拓扑、延迟、控制增益与触发参数之间的耦合关系，具有良好的实际工程应用价值。
...
### [306] [LLM-Guided Contextual Action Evaluation for Operational Decisions in Industrial Processes](https://arxiv.org/abs/2608.24156)
*Youcheng Zong,Runda Jia,Dakuo He*

Main category: eess.SY

TL;DR: 提出了一种名为LCAE的框架，通过预训练大语言模型（LLM）将工业文档中的非结构化知识转化为数值策略的上下文偏差，从而加速工业过程控制中的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 工业控制中，传统的动作坐标表示法难以理解动作对过程变量的具体影响，且人工记录的文档知识未被有效利用于数值策略的学习。

Method: 利用LLM预处理工业文档，提取固定的“动作-观测-方向-延迟”关系；训练阶段将历史数据作为强度调制因子，通过非线性动作-效应场构建动作评价体系，并将其融入最大熵策略学习。

Result: 该方法使语义知识成为策略学习的一部分，通过提供决策偏差（而非仅仅使用原始动作坐标），在文档信息准确且历史数据相关时，显著提升了学习效率和表现。

Conclusion: 将离线处理的语义知识与在线数值历史相结合，能够有效地为工业控制强化学习提供有意义的偏差，且推理阶段无需实时运行LLM，具有极高的部署效率。
...
### [307] [The Flat Earth Error: Differential Geometry in Vehicle Dynamics](https://arxiv.org/abs/2608.24280)
*David J. N. Limebeer,Charl van de Merwe*

Main category: eess.SY

TL;DR: 该论文探讨了在车辆动力学仿真中忽略路面曲率导致的“平地误差”，并提出利用微分几何对弯曲路面建模以提高仿真精度。


<details>
  <summary>Details</summary>
Motivation: 传统的路面平面假设无法准确反映真实环境（如NASCAR赛道）中的车辆动力学，特别是在高曲率和高倾角路面上。

Method: 引入微分几何工具（度量张量、第二基本形式、形状算子及Christoffel符号）构建曲面模型，并将其集成到单轨车辆动力学模型中，结合伪谱最优控制框架进行轨迹优化。

Result: 通过Darlington赛道的LiDAR数据建模分析发现，路面几何结构会导致显著的动力学效应，影响轮胎饱和极限和重力分布，直接影响最优单圈时间。

Conclusion: 在高性能驾驶和自动驾驶仿真中，整合微分几何、多体动力学和最优控制对于模拟真实的三维路面动力学至关重要。
...
### [308] [Cognitive Energy Management: Concept,Framework, and Demonstration in Smart Port Energy Systems](https://arxiv.org/abs/2608.24284)
*Hafiz Majid Hussain,Wajiha Samer,Juhaa Haakana*

Main category: eess.SY

TL;DR: 提出了一种认知能源管理（CEM）框架，旨在为能源互联网（EI）引入具备感知、推理、学习和决策能力的认知治理层。


<details>
  <summary>Details</summary>
Motivation: 现有能源管理系统过于依赖被动式优化，缺乏对复杂环境的上下文理解、意图推断和应对不确定性的智能决策能力。

Method: 构建了包含感知、推理、学习与行动循环的认知架构，并通过智能港口能源管理场景（船舶周转预测与调度）进行实证评估。

Result: 验证了CEM能够超越传统可行性寻优，实现具备前瞻性、目标导向的能源治理，有效应对复杂的运行环境。

Conclusion: CEM为能源互联网提供了一个进化的治理范式，使其能够从单纯的优化控制转向具有认知能力的智能能源管理系统。
...
### [309] [Electric Vehicle Charging Right Trading: Concept, Mechanism, and Methodology](https://arxiv.org/abs/2608.24296)
*Ruike Lyu,Yuxuan Gu,Qixin Chen*

Main category: eess.SY

TL;DR: 该论文提出了一种电动汽车充电权（Charging Right, CR）交易机制，旨在通过预售充电权益来优化充电调度，从而减少电动汽车的等待时间并缓解充电站拥堵。


<details>
  <summary>Details</summary>
Motivation: 随着电动汽车渗透率提高，无序充电导致的混乱和长等待时间成为亟待解决的问题，需要通过市场化手段引导充电行为。

Method: 定义了充电权（CR）机制；利用排队论模型估算等待时间，并首次考虑了向用户公开充电站排队状态的影响，通过动态调整CR价格引导优化充电行为。

Result: 仿真结果表明，所提机制能够准确估算等待时间，有效降低了用户的平均等待时间并缓解了充电站的拥堵现象。

Conclusion: 充电权交易机制通过价格杠杆和预留服务，为解决电动汽车充电拥堵问题提供了一种高效、有序的解决方案。
...
### [310] [Data-Driven Dimension Reduction for Industrial Load Modeling Using Inverse Optimization](https://arxiv.org/abs/2608.24390)
*Ruike Lyu,Hongye Guo,Goran Strbac,Chongqing Kang*

Main category: eess.SY

TL;DR: 提出了一种基于数据驱动的工业负荷降维建模方法，以应对工业负荷中复杂混合整数约束带来的计算难题。


<details>
  <summary>Details</summary>
Motivation: 工业负荷中复杂的混合整数约束难以直接融入经济调度或市场出清过程，且传统的解析式降维方法效果不佳。

Method: 利用工业负荷的最优用能数据，通过可调节负荷集群模型（adjustable load fleet model）训练出一个能够拟合原始约束的降维模型。

Result: 在三个工业负荷数据集上的测试表明，该方法在性能表现上优于传统的解析式方法。

Conclusion: 该数据驱动方案有效解决了工业负荷模型在经济调度中的降维难点，为工业需求侧响应提供了高效的建模工具。
...
### [311] [Co-optimizing Bidding and Power Allocation of an EV Aggregator Providing Real-time Frequency Regulation Service](https://arxiv.org/abs/2608.24397)
*Ruike Lyu,Hongye Guo,Kedi Zheng,Mingyang Sun,Qixin Chen*

Main category: eess.SY

TL;DR: 该论文提出了电动汽车（EV）聚合商在调频市场中的竞价与功率分配协同优化框架。


<details>
  <summary>Details</summary>
Motivation: 传统的电动汽车聚合商（EVA）常将竞价与功率分配视为两个独立问题优化，导致利润损失，因此需要考虑两者之间的耦合效应。

Method: 构建了包含功率分配的随机规划竞价模型，并提出了一种基于拉格朗日乘子的在线功率分配模型，以确保实时调频需求下的最优性。

Result: 研究表明，该协同优化框架能够有效提升EVA的利润，并降低电池退化成本。

Conclusion: 通过协同考虑竞价策略与实时调度，可以显著提高电力市场中电动汽车集群的经济效益与运行性能。
...
### [312] [A kernel proof of the De Cock-De Moor Lyapunov identity](https://arxiv.org/abs/2608.24405)
*Jonas Gillberg,Johan Löfberg*

Main category: eess.SY

TL;DR: 本文证明了数学系统与控制理论中关于秩一Lyapunov谱恒等式的著名未解决问题（Problem 9.1）。


<details>
  <summary>Details</summary>
Motivation: 旨在解决2004年提出的关于耦合离散Lyapunov和Sylvester方程中矩阵谱性质的长期未决问题。

Method: 利用秩一行列式分解导出的标量双线性核，结合多项式延拓技术将恒等式从特定开集扩展至整个容许域，并利用相似变换进行验证。

Result: 证明了在特定条件下，矩阵 $P^{-1}RQ^{-1}R^	op$ 与 $(I+PQ)^{-1}$ 具有相同的特征多项式，并在无稳定性假设下得出了行列式推论。

Conclusion: 该恒等式在Schur稳定实现框架下与主角谱及正则相关谱有直接联系，成功统一并解决了该领域的理论难题。
...
### [313] [Autonomous path following using data-driven predictive control](https://arxiv.org/abs/2608.24540)
*Josip Kir Hromatko,Marko Švec,Šandor Ileš*

Main category: eess.SY

TL;DR: 提出了一种基于数据驱动而非物理模型的车辆轨迹预测控制方法，用于解决车辆动力学控制与路径跟踪问题。


<details>
  <summary>Details</summary>
Motivation: 近年来数据驱动控制方法备受关注，研究旨在探索非物理模型驱动的控制策略在车辆自主路径跟踪任务中的可行性和性能。

Method: 通过将路径跟踪问题映射至误差空间使其近似线性化，应用数据驱动预测控制，并结合纵向速度调度技术。

Result: 在高保真仿真环境下，针对两种不同的变道驾驶工况进行了测试，验证了控制算法的有效性。

Conclusion: 基于信息系统轨迹的数据驱动预测控制能够有效应用于车辆路径跟踪，具有良好的应用前景。
...
### [314] [Partial Observation Amplifies Model Mismatch in MAP Estimation via Information-Curvature Margins](https://arxiv.org/abs/2608.24550)
*Junsei Ito,Yasuaki Wasa*

Main category: eess.SY

TL;DR: 本文分析了受控动力系统中，模型失配如何导致有限视界最大后验概率（MAP）初始状态估计产生偏差（即MAP位移）。


<details>
  <summary>Details</summary>
Motivation: 旨在理解并量化系统模型不匹配对估计性能的影响，特别是探讨为何某些传感器配置在信息量最大化时仍对模型偏差表现出脆弱性。

Method: 通过路径敏感度分析（pathwise sensitivity analysis），将MAP位移分解为模型侧失配注入和估计侧曲率抵抗，并引入信息曲率边际（information-curvature margin）作为放大瓶颈。

Result: 研究发现MAP位移由后验曲率最弱方向决定，揭示了实例级失配鲁棒性与设计时可推断性之间的区别，并推导出了非线性系统中可计算的标称代理模型。

Conclusion: 该研究提供了一个评估系统对模型失配敏感度的理论框架，表明在优化传感器配置时，仅追求聚合信息量是不够的，还必须考虑信息曲率边际以增强鲁棒性。
...
### [315] [Dual-Based Weight Selection for Approximate Linear Programming](https://arxiv.org/abs/2608.24629)
*Su Li,Andre A. Cire,Adam Diamant,Vahid Sarhangian*

Main category: eess.SY

TL;DR: 提出了一种基于对偶的近似线性规划（ALP）权重自适应更新方法，以解决传统启发式权重选择导致的性能敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 传统ALP方法中状态相关权重通常依靠启发式选择，缺乏理论支撑且在大规模MDP中计算成本高、缺乏收敛保证。

Method: 利用ALP对偶解中的投影占用信息构建平滑随机策略，通过迭代更新权重；建立收敛性证明并推导后验策略损失界。

Result: 该方法降低了对固定权重的依赖，在排队和调度问题中表现优于原始更新方法，且计算成本更低。

Conclusion: 自适应权重更新能够有效提升策略质量，且当基函数具备足够的表达力时，这种权重的调整对策略的影响最为显著。
...
### [316] [DeepONet-LSTM Neural Operator for Output Feedback Control of Reaction Diffusion PDEs](https://arxiv.org/abs/2608.24699)
*Jing Zhang,Jie Qi,Linglong Jiang*

Main category: eess.SY

TL;DR: 提出了一种基于混合 DeepONet-LSTM 神经算子的方法，用于反应扩散偏微分方程的输出反馈边界镇定。


<details>
  <summary>Details</summary>
Motivation: 传统的输出反馈反步法设计需要为每个反应系数计算复杂的核函数，计算成本高且难以适应变系数场景。

Method: 将输出反馈控制规律重新表述为映射反应系数和测量历史到边界控制输入的因果边界算子，并利用 DeepONet 编码空间系数、LSTM 处理时间历史进行近似。

Result: 分析证明了算子的 Lipschitz 连续性及闭环系统的实用稳定性，数值实验验证了该控制器能有效实现系统镇定。

Conclusion: 该方法通过数据驱动的神经算子规避了显式核函数计算，为偏微分方程的实时鲁棒控制提供了一种高效的替代方案。
...
### [317] [Multi-Stage Prompt-Guided Feature Modulation for Generalizable Brain Tumor Segmentation](https://arxiv.org/abs/2608.23745)
*Mohammad Mahdi Danesh Pajouh,Sara Saeedi*

Main category: eess.IV

TL;DR: 提出了一种基于多阶段动态提示（Dynamic Prompt）的改进版nnU-Net，通过FiLM技术实现特征自适应调节，提升了脑肿瘤MRI分割的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在跨机构、跨协议的脑肿瘤MRI分割任务中泛化能力较差，难以应对肿瘤形态和成像分布的差异。

Method: 在nnU-Net深层编码器阶段插入三个动态提示模块，通过可学习的向量库生成图像特定的提示，并利用FiLM（特征线性调制）对编码器特征图进行缩放和平移。

Result: 在BraTS GOAT数据集上，该模型在ET、TC和WT三个肿瘤区域的Dice系数均优于基准nnU-Net，分别达到76.16%、80.04%和86.42%。

Conclusion: 多阶段动态提示调节有效增强了模型对不同图像特征的适应性，显著提升了脑肿瘤分割的精度和边界勾勒能力。
...
### [318] [Spatiotemporal Distillation via Recurrent Bottlenecks for Aortic Tracking](https://arxiv.org/abs/2608.23879)
*Dexter Wen Jie Teo,Nairouz Shehata,Herve Lombaert*

Main category: eess.IV

TL;DR: 本文提出了一种半监督时空知识蒸馏框架，通过利用心脏周期的时间连续性，解决了心脏电影MRI中主动脉分割的帧间不一致和边界模糊问题。


<details>
  <summary>Details</summary>
Motivation: 标准2D分割网络忽略了时间维度，导致在快速收缩期出现边界模糊和跟踪丢失；而时空网络虽然能保持时间一致性，但受限于专家标注数据的匮乏。

Method: 引入一种半监督(2D到2D+t)知识蒸馏框架，通过动态潜在拦截技术，将空间教师模型的知识迁移至包含循环时空瓶颈和残差空间旁路的时空学生网络中，并采用基于解剖一致性的模型选择策略。

Result: 该方法实现了92.3%的表面跟踪精度(NSD@1mm)和99.2%的结构可靠性(Frac2CC)，相较于2D nnU-Net基准，结构异常率降低了超过56%。

Conclusion: 该研究证明了利用时空知识蒸馏和心脏周期先验知识，能够显著提升在标注稀疏情况下的医学影像时序分割鲁棒性与精度。
...
### [319] [Native-Space 3D CarveMix for Multi-Site T1w Stroke Segmentation](https://arxiv.org/abs/2608.23882)
*Dexter Wen Jie Teo,Kumaradevan Punithakumar*

Main category: eess.IV

TL;DR: 该研究提出了一种结合MedNeXt-L架构与动态3D CarveMix增强技术的方法，用于解决跨中心、无强度标准化T1w MRI图像中的急性缺血性卒中病灶分割问题。


<details>
  <summary>Details</summary>
Motivation: 多中心T1w MRI影像因缺乏强度标准化及急性病灶样本稀缺，导致现有深度学习模型在病灶分割任务中表现停滞（Dice约0.66），尤其是急性病灶识别困难。

Method: 采用MedNeXt-L ($k=5$) 作为骨干网络，引入“即时3D CarveMix”增强技术，在训练过程中动态将真实病灶贴片植入健康脑区，无需预先生成数据。

Result: 在ISLES 2026挑战赛的1453例多中心扫描数据集上，该方法实现了0.648的平均Dice系数，较同等算力下的基线模型（0.630）提升了0.018。

Conclusion: 动态生成的病灶增强技术能有效应对跨中心影像的异质性，并改善样本稀缺导致的模型分割性能瓶颈。
...
### [320] [Model Effect or Label Effect? Refined Annotations and a Human-Referenced Benchmark for Pulmonary Embolism Segmentation](https://arxiv.org/abs/2608.24486)
*Qihang Sun,Zhongxiao Liu,Bailiang Jian,Shenman Qiu,Jingyuan Wang,Lei Zhang,Lixiang Xie,Jiazhen Pan,Christian Wachinger*

Main category: eess.IV

TL;DR: 该研究量化了评估标注质量对肺栓塞（PE）分割模型性能指标的影响，并发现标注质量对性能的影响远大于模型训练策略的差异。


<details>
  <summary>Details</summary>
Motivation: 探讨医学影像分割评估中，标注者的主观差异和注释质量对模型性能表现的影响，从而建立更可靠的评估基准。

Method: 通过专家对149例CT肺血管造影病例进行精细化重标注，对比同一模型在不同标注集下的性能，以及同一标注集下不同模型配置的性能，利用Wilcoxon符号秩检验进行量化分析。

Result: 标注重定义对Dice系数（DSC）的提升（0.143-0.188）显著高于模型训练组合的变化（0.028）；目前的nnPE模型表现仍显著低于人类标注者的准确度。

Conclusion: 评估时的标注质量是决定模型表现的关键因素，甚至超过了模型训练策略，因此研究者构建了人类基准参考框架以促进未来研究。
...
### [321] [Score-Based Ideal Observer Approximation via Denoising Score Matching for Signal-Known-Exactly Detection Tasks](https://arxiv.org/abs/2608.24768)
*Weimin Zhou*

Main category: eess.IV

TL;DR: 提出了一种基于得分函数（Score Function）的理想观测器（SIO），用于在二分类检测任务中逼近贝叶斯理想观测器（IO）的理论性能。


<details>
  <summary>Details</summary>
Motivation: 传统的IO计算通常难以解析，而基于MCMC或深度生成模型的数值方法往往需要耗时的采样，监督学习方法则缺乏任务泛化能力，需针对特定信号重新训练。

Method: 利用得分函数重构IO测试统计量，通过在无信号图像上训练去噪卷积神经网络来估计得分函数，从而实现对任意加性信号的检测。

Result: 数值实验表明，该方法在随机肿块背景下的信号已知（SKE）检测任务中，能够紧密逼近贝叶斯理想观测器的性能。

Conclusion: 基于得分函数的理想观测器（SIO）无需针对特定信号进行重训练，且无需逐图像进行采样，显著提升了理想观测器在检测任务中的计算效率和灵活性。
