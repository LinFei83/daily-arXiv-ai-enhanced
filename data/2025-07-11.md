...
Motivation: 现代化学过程日益复杂，劳动力短缺和故障场景多变，需要新的自动化范式。

Method: 采用有限状态机（FSMs）作为可解释的操作框架，结合LLM驱动的规划代理、模拟代理和验证-重新提示循环。

Result: 在案例1中，GPT-4o和GPT-4o-mini在180个随机生成的FSMs中实现100%有效路径成功率；案例2中，LLM控制器在双加热器控制中表现与PID相当。
...
<details>
  <summary>Details</summary>
Motivation: AI模型在艺术分类中因数据集不平衡导致的偏见问题日益严重，影响了公平性和准确性，而现有研究忽视了这一关键问题。

Method: 提出BOOST方法，动态调整温度缩放和采样概率，并引入新指标SODC评估类别分离和偏见减少。

Result: BOOST在KaoKore和PACS数据集上有效减少了类别偏见，同时保持了高性能。

Conclusion: BOOST是一种兼顾性能和公平性的解决方案，适用于艺术领域的AI模型去偏见。
...
Result: 通过系统评估，发现人工与机器分类新闻文章在识别强迫劳动方面的差异。

Conclusion: 神经符号方法在稀疏数据中检测非法活动具有潜力，问题树方法可有效量化文章相关性。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的探索效率问题因算法复杂性而加剧，需要新的解决方案。

Method: 采用大型语言模型作为专家规划器，用于基于规划任务的多智能体探索。

Result: 未明确提及具体实验结果，但提出了方法的可行性。

Conclusion: 大型语言模型在多智能体任务中作为专家规划器具有潜力。
...
Method: ViDove模拟人类翻译流程，整合多模态记忆系统和长短时记忆模块，并利用领域知识。

Result: 在字幕生成和通用翻译任务中，ViDove的BLEU分数提升28%，SubER提升15%。

Conclusion: ViDove在多模态翻译任务中表现优异，并推出了新的评测基准DoveBench。
...
Method: 通过理论分析，研究输入提示和输出内容的过滤效率，并基于密码学假设证明其计算困难性。

Result: 发现高效过滤输入提示和输出内容在计算上不可行，且外部过滤器不足以确保安全。

Conclusion: 安全需依赖LLMs内部设计，而非外部过滤器，智能与判断不可分离。
...
<details>
  <summary>Details</summary>
Motivation: 供应链运输中的高响应性和经济效率是关键目标，而运输模式的选择直接影响这些目标。需要一个集成框架来设计低风险、可观察的运输策略。

Method: Sim-to-Dec框架包括生成模拟模块（利用自回归建模模拟连续状态变化）和双感知决策模型（结合历史与未来预测），通过端到端优化迭代优化。

Result: 在三个真实数据集上的实验表明，Sim-to-Dec显著提高了及时交付率和利润。

Conclusion: Sim-to-Dec框架满足跨场景通用性、细粒度动态模拟、历史与预测结合以及模拟反馈与策略优化的紧密集成需求，有效提升供应链运输效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在科学领域推理能力受限的问题，避免传统方法的高计算成本或数据利用不足。

Method: 提出DrugMCTS框架，整合RAG、多智能体和蒙特卡洛树搜索，通过五个专业智能体实现结构化推理。

Result: 在DrugBank和KIBA数据集上，DrugMCTS的召回率和鲁棒性显著优于通用LLM和深度学习基线。

Conclusion: 结构化推理、智能体协作和反馈驱动搜索对药物发现中的LLM应用至关重要。
...
Method: StarDojo提供1000个任务和100个代表性子任务，支持多环境并行执行，并采用统一用户界面。

Result: 当前最先进的MLLM代理表现有限，GPT-4.1的成功率仅为12.7%，主要因视觉理解和多模态推理能力不足。

Conclusion: StarDojo为复杂生产生活环境中鲁棒开放式代理的研究提供了便利工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前研究多关注性能提升，缺乏对LLM学习算法的理论理解，AlgEval填补这一空白。

Method: 提出AlgEval框架，结合案例研究，通过注意力模式和隐藏状态分析算法。

Result: 案例展示如何通过假设和电路级分析揭示LLM的搜索算法。

Conclusion: AlgEval为理解LLM底层计算提供新视角，有望推动高效训练和新架构设计。
...
<details>
  <summary>Details</summary>
Motivation: 在高风险领域，错误的解释可能误导决策者，因此需要严谨的解释方法。规则模型虽广泛使用，但其存在负面影响。

Method: 开发算法分析规则模型中的负面问题，如重叠和冗余。

Result: 研究发现，常用规则模型学习工具会引入负面问题。

Conclusion: 规则模型在高风险应用中需谨慎使用，以避免负面影响。
...
Method: 提出Context Pooling方法，通过设计邻域精度和邻域召回率两个指标，筛选逻辑相关的邻居节点，生成查询特定图。
...
Result: 微调的Llama 3B参数模型在提取疫苗名称的准确性上优于其他模型，模型量化实现了资源受限环境下的高效部署。
...
Method: 提出了一种基于信念和可能性函数的框架，用于在链式信用网络中传播不确定性，并生成保守区间。

Result: 数值结果表明，该方法在计算速度和鲁棒性方面具有优势，但也存在局限性。

Conclusion: 该框架为链式信用网络及一般信用网络中的信念推理提供了实用见解。
...
<details>
  <summary>Details</summary>
Motivation: 揭示当前LLM在真实世界布局推理中的盲点，推动语言模型在空间和几何属性推理方面的改进。

Method: 使用符号化格式（如JSON、XML）编码室内场景，设计多样化问题类型测试度量、拓扑推理及设计约束。

Result: LLM在浅层查询中表现良好，但在模拟物理约束、保持空间一致性及布局扰动泛化方面表现不佳。

Conclusion: PlanQA揭示了LLM在空间推理上的不足，为未来研究提供了改进方向。
...
<details>
  <summary>Details</summary>
Motivation: DPO虽然在实践中表现良好，但其理论性质和内在局限性尚未充分研究，尤其是在初始化和概率分配方面的问题可能影响模型对齐的稳定性。

Method: 提出了一种双层优化框架，结合监督微调和改进的DPO目标，引入正则化机制以优化概率分配。

Result: 实验表明，该方法在推理和摘要任务中表现优于标准DPO，提升了推理准确性和输出分布的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于定量分析小提琴轮廓线的差异，尤其是缩减与非缩减小提琴之间的几何特征差异，填补专家观察但未定量研究的空白。

Method: 方法包括对25把小提琴的3D几何网格进行摄影测量，提取10-20条轮廓线，拟合抛物线曲线（参数alpha和beta），并通过回归和阈值计算特征，最终生成数值化轮廓。

Result: 结果表明，仅凭几何特征可以在一定程度上区分缩减与非缩减小提琴，其中参数beta最具预测性，但存在部分难以量化的中间状态。

Conclusion: 结论指出，几何方法可用于小提琴缩减分类，但需考虑中间状态的复杂性，未来研究可进一步优化模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统AI评估多关注技术能力或避免危害，而FAI Benchmark旨在衡量AI对人类全面福祉的贡献，填补了现有评估的空白。

Method: 采用1229个主客观问题，结合专业LLM评委和跨维度评估，使用几何平均分确保各维度平衡。

Result: 测试28个模型，最高得分72/100，但无模型在所有维度（尤其是信仰与灵性、品格与美德、意义与目的）上完全对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有混合专家模型（MoE）需要大量数据和复杂优化，而MoSE通过模仿人类驾驶员的技能学习和推理过程，减少计算需求并提升性能。

Method: MoSE采用技能导向的路由机制，定义并标注特定技能，构建分层技能数据集，并通过单次前向过程整合辅助任务。

Result: MoSE在CODA AD任务中优于多个8B+参数模型，激活参数量减少至少62.5%，性能达到SOTA。

Conclusion: MoSE通过技能分步学习和推理，显著提升了自动驾驶模型的效率和性能，同时减少了计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI依赖大规模模型和数据集，带来环境、经济和伦理问题，而生物感官系统的动态适应性启发了自适应感知的研究。

Method: 提出自适应感知方法，动态调整传感器参数（如曝光、灵敏度），减少协变量偏移并提高效率。

Result: 实证显示自适应感知让小模型（如EfficientNet-B0）超越更大模型（如OpenCLIP-H）。
...
<details>
  <summary>Details</summary>
Motivation: 传统可解释AI和因果性研究未满足非专家用户对实际原因的需求，且识别实际原因是NP完全问题。

Method: 提出一组多项式复杂度的算法，可调整精度和完备性。

Result: 算法能识别现有方法无法处理的系统（非布尔、黑盒、随机），且计算时间与精度可调。

Conclusion: 算法有效填补实际原因识别的空白，提供灵活性和实用性。
...
Result: 实验结果显示法律纠纷分析性能显著提升，能准确分析复杂案例并理解司法决策逻辑。
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI模型性能不平等问题，并提出计算资源边际效益递减将导致性能收敛的观点。

Method: 开发模型分析固定分布下计算资源投入的边际效益，并结合基准数据和理论模型验证代理指标的有效性。

Result: 实证数据显示，随着时间推移，资源有限模型的性能差距缩小，支持边际效益递减的假设。
...
<details>
  <summary>Details</summary>
Motivation: 理解AI对经济的潜在影响是社会的关键问题。

Method: 分析20万条匿名用户与Bing Copilot的对话数据，结合职业活动分类和任务成功度量。
...
Method: MMoE包含四个模块：描述感知提及增强模块、多模态特征提取模块、层级内和层级间专家混合模块，动态选择信息。
...
Method: 通过LLM生成域描述，结合CLIP模型生成文本嵌入，设计CoPT损失学习域不变特征。

Result: 在四个基准测试中达到UDA分割的最新性能。

Conclusion: CoPT通过文本嵌入有效提升了UDA分割的域适应能力。
...
<details>
  <summary>Details</summary>
Motivation: 现代导航系统需要精确的位置估计，而VSLAM依赖于可靠的特征点提取。资源受限的平台（如移动或嵌入式系统）需要高效解决方案。
...
Conclusion: 量化技术对模型精度与性能有显著影响，该方案在视觉里程计任务中表现出色，适合资源受限平台。
...
<details>
  <summary>Details</summary>
Motivation: 尽管图像生成模型（如Stable Diffusion）在生成内容方面取得了显著进展，但其可能产生有害或侵权内容，引发伦理和法律问题。机器未学习（MU）被提出作为解决方案，但其鲁棒性尚未充分研究。

Method: Recall是一种对抗性框架，通过优化对抗性图像提示（基于单张参考图像）来测试未学习模型的鲁棒性，不同于仅依赖文本提示的现有方法。

Result: 实验表明，Recall在对抗效果、计算效率和语义保真度上均优于现有基线，揭示了当前未学习机制的关键漏洞。

Conclusion: 研究强调了开发更鲁棒的未学习解决方案的必要性，以确保生成模型的安全性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估协议依赖粗粒度指标，掩盖模型差异，缺乏神经科学基础，无法捕捉细粒度视觉区别。

Method: 引入BASIC框架，包括基于分割的层次化结构指标和多模态大语言模型的语义场景表示。

Result: 在统一框架下评估多种视觉解码方法，提供了更具区分性和解释性的评估基础。

Conclusion: BASIC为大脑视觉解码方法提供了更全面、可解释的评估标准。
...
Result: 提供了针对生物医学图像分析的XAI方法综合分类，突出了不同模态的解释性挑战，并探讨了未来方向。
...
Method: 论文正式定义了模态冲突，构建了MMMC数据集，并提出了基于提示工程、监督微调和强化学习的三种方法来缓解幻觉。
...
Result: 在MBZIRC2023竞赛的实际实验中验证了方法的可行性，并评估了视角对检测和定位精度的影响。
...
Conclusion: 通过结构化语义中介，实现了与认知对齐的EEG视觉解码，为神经科学和AI提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法在生成长视频时存在角色一致性和场景布局问题，多主题长视频尤其难以保持角色一致性和运动连贯性。

Method: 研究了32篇视频生成论文，识别关键架构组件和训练策略，构建了新的分类法并比较了不同方法的设计和性能。

Result: 提出了能够生成长视频、多角色、叙事连贯且高保真细节的方法。

Conclusion: 通过系统研究，为长视频生成提供了关键见解和分类框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统身份识别在多样化活动中面临身份线索与运动动态和外观变化纠缠的挑战，现有视觉数据（如姿态）提取不准确。

Method: 提出DisenQ，一种统一查询转换器，利用结构化语言指导解耦特征，避免误识别。

Result: 在三个活动视频基准测试中达到最先进性能，并在传统视频识别基准中表现优异。

Conclusion: DisenQ框架有效解决了活动生物特征识别的复杂性，具有强泛化能力。
...
Result: 闭源模型整体表现最佳，开源模型Qwen2.5在多语言上表现优异。

Conclusion: 发布基准测试和评估代码以促进可复现性和进一步研究。
...
Conclusion: MagiC为评估和改进视觉语言模型的推理能力提供了新工具和见解。
...
Result: 评分模型在多个基准测试中优于所有开源和专有模型，显著提升了与人类评分的相关性及比较准确性。

Conclusion: ADIEE的评分模型可作为奖励模型，提升图像编辑模型的性能，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决虚拟试妆应用中粉底与肤色准确混合的技术挑战，同时保持方法的可扩展性。

Method: 近似Kubelka-Munk理论以加速图像合成，构建仅依赖电商产品信息的端到端框架。

Result: 在真实化妆图像上验证，框架性能优于其他技术。

Conclusion: 该方法在保持逼真度的同时提高了效率，适用于多样化的产品范围。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉叙事系统在跨帧识别相同实体时表现不佳，导致引用不一致和幻觉问题。

Method: 采用对比强化学习框架，扩展数据集并引入双组分奖励函数，优化Qwen Storyteller模型。

Result: 模型在多项指标上显著提升，如接地mAP提高14.8%，F1提高17.1%，跨帧实体持续性增强。

Conclusion: 该方法有效提升了视觉叙事系统中实体识别的连贯性和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 训练泛化性深度补全模型通常需要大量带标注的深度数据，收集成本高。PacGDC旨在通过最小标注努力提升数据多样性。

Method: 利用2D到3D投影中的形状和位置一致性，合成多尺度伪几何数据；结合深度基础模型、插值和重定位策略，以及未标注图像扩展数据覆盖。

Result: PacGDC在多个基准测试中表现出卓越的泛化能力，适应不同场景语义、尺度和深度稀疏性/模式。

Conclusion: PacGDC通过高效数据合成显著提升深度补全的泛化性，为下游任务提供鲁棒感知能力。
...
Conclusion: 提出的机制显著提高了PSM的自适应性，为复杂解剖结构的表征提供了更准确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有精确事件检测模型在时间感受野和空间适应性上存在局限，需要改进。

Method: 提出MSAGSM模块，结合多尺度时间扩张和多头空间注意力，提升短期和长期依赖建模能力。

Result: 在五个基准测试中，MSAGSM显著提升性能，达到新最优结果。

Conclusion: MSAGSM是一种轻量级即插即用模块，适用于多种2D主干网络，推动了精确事件检测的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有运动转移方法主要针对人类运动，忽视了动物特有的习惯行为，因此需要一种能够保留这些行为的跨类别运动转移方法。

Method: 基于生成框架，引入习惯保留模块和类别特定习惯编码器，结合LLM支持未观察物种的运动转移。

Result: 在DeformingThings4D-skl数据集上的实验和定量分析验证了模型的优越性。

Conclusion: 该框架成功保留了动物特有的习惯行为，为跨类别运动转移提供了有效解决方案。
...
TL;DR: 论文提出了SURPRISE3D数据集，用于评估复杂3D场景中语言引导的空间推理分割能力，旨在解决现有数据集中语义与空间信息混杂的问题。
...
Method: 通过构建包含200k+视觉语言对和89k+人工标注空间查询的SURPRISE3D数据集，避免对象名称干扰，专注于空间关系。

Result: 初步测试显示当前先进3D视觉定位方法和3D-LLMs在该数据集上表现不佳，凸显其挑战性。

Conclusion: SURPRISE3D和3D-SRS基准套件为空间感知AI的发展提供了重要工具，推动具身交互和机器人规划的进步。
...
### [53] [EscherNet++: Simultaneous Amodal Completion and Scalable View Synthesis through Masked Fine-Tuning and Enhanced Feed-Forward 3D Reconstruction](https://arxiv.org/abs/2507.07410)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要多阶段和复杂流程来补全图像缺失部分并合成新视角，忽视了跨视角依赖且计算冗余。

Method: 采用输入级和特征级掩码微调，实现端到端模型，提升新视角合成和模态补全能力。

Result: 在遮挡任务中，PSNR提升3.9，Volume IoU提升0.28，重建时间减少95%。

Conclusion: EscherNet++在较小数据集和批量下实现最佳性能，并支持快速3D重建。
...
### [54] [EPIC: Efficient Prompt Interaction for Text-Image Classification](https://arxiv.org/abs/2507.07415)
...
### [55] [Corvid: Improving Multimodal Large Language Models Towards Chain-of-Thought Reasoning](https://arxiv.org/abs/2507.07424)
*Jingjing Jiang,Chao Ma,Xurui Song,Hanwang Zhang,Jun Luo*

Main category: cs.CV

TL;DR: Corvid是一个增强链式思维推理能力的多模态大语言模型，通过混合视觉编码器和精心设计的连接器提升性能，并在数学推理和科学问题解决中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前开源多模态大语言模型在复杂结构化推理任务中存在显著局限性，特别是在需要深度推理的决策和问题解决中。

Method: Corvid采用混合视觉编码器和GateMixer连接器，结合高质量多模态CoT指令数据集MCoT-Instruct-287K，通过两阶段CoT格式训练和推理时自验证策略提升推理能力。

Result: Corvid在数学推理和科学问题解决中优于现有类似参数规模的MLLMs。

Conclusion: Corvid通过增强的链式思维推理能力和高质量数据集，显著提升了多模态大语言模型在复杂推理任务中的表现。
...
### [56] [Towards High-Resolution 3D Anomaly Detection: A Scalable Dataset and Real-Time Framework for Subtle Industrial Defects](https://arxiv.org/abs/2507.07435)
...
Method: 提出生成高分辨率3D异常数据的流程，创建MiniShift数据集；设计Simple3D框架，结合MSND和LFSA方法。

Result: Simple3D在MiniShift和现有基准测试中表现优异，精度和速度均超越现有方法，实时推理速度超过20 fps。

Conclusion: 高分辨率数据和有效特征聚合对提升3D异常检测实用性至关重要，Simple3D为此提供了高效解决方案。
...
### [57] [A Comprehensive Survey on Deep Learning Solutions for 3D Flood Mapping](https://arxiv.org/abs/2506.13201)
*Wenfeng Jia,Bin Liang,Yuxi Liu,Muhammad Arif Khan,Lihong Zheng*

Main category: cs.CV

TL;DR: 本文综述了基于深度学习的3D洪水制图技术，比较了其与传统2D方法的优势，并探讨了数据来源、应用场景及当前挑战。


<details>
  <summary>Details</summary>
Motivation: 洪涝灾害因气候变化和城市化加剧，传统2D洪水制图方法局限性明显，需更先进的3D技术以提升灾害管理效果。

Method: 将深度学习技术分为任务分解和端到端方法，分析其在静态和动态洪水特征中的应用，并比较不同架构的预测精度和计算效率。

Result: 3D洪水制图通过整合洪水范围和深度数据，显著提升了预测准确性和应用范围，但仍面临数据稀缺和模型解释性等挑战。

Conclusion: 未来需改进数据集、优化模型并关注政策影响，以推动3D洪水制图技术在实际灾害管理中的应用。
...
### [58] [Dual Semantic-Aware Network for Noise Suppressed Ultrasound Video Segmentation](https://arxiv.org/abs/2507.07443)
*Ling Zhou,Runtian Yuan,Yi Liu,Yuejie Zhang,Rui Feng,Shang Gao*

Main category: cs.CV

TL;DR: 提出了一种名为DSANet的双语义感知网络，通过增强局部和全局特征的语义感知能力，提高了超声视频分割的噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 超声成像因其简单和非侵入性被广泛使用，但其固有噪声对自动分割任务提出了挑战。

Method: 设计了AFSA模块和LGSA模块，分别通过相邻帧语义感知和局部-全局特征融合来减少噪声影响。

Result: 在四个基准数据集上表现优于现有方法，且推理速度更快。

Conclusion: DSANet在噪声鲁棒性和分割准确性上取得了显著提升，同时保持了高效的计算性能。
...
### [59] [Bluish Veil Detection and Lesion Classification using Custom Deep Learnable Layers with Explainable Artificial Intelligence (XAI)](https://arxiv.org/abs/2507.07453)
...
<details>
  <summary>Details</summary>
Motivation: BWV是黑色素瘤诊断的关键特征，但现有研究有限。本研究旨在开发一种高效、准确的BWV检测方法，以辅助早期黑色素瘤诊断。
...
### [60] [Objectomaly: Objectness-Aware Refinement for OoD Segmentation with Structural Consistency and Boundary Precision](https://arxiv.org/abs/2507.07460)
*Jeonghoon Song,Sunghun Kim,Jaegyun Im,Byeongjoon Noh*

Main category: cs.CV

TL;DR: Objectomaly是一种基于对象感知的OoD分割框架，通过三阶段方法解决边界模糊和异常评分不一致问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有OoD分割方法存在边界不精确、对象内异常评分不一致及背景噪声导致的误报问题。

Method: 三阶段框架：1) 粗粒度异常评分（CAS）；2) 对象感知评分校准（OASC）；3) 精细边界优化（MBP）。

Result: 在多个基准测试中表现优异，像素级和组件级指标均有显著提升。

Conclusion: Objectomaly在OoD分割任务中表现出色，具有鲁棒性和泛化能力。
...
### [61] [Degradation-Agnostic Statistical Facial Feature Transformation for Blind Face Restoration in Adverse Weather Conditions](https://arxiv.org/abs/2507.07464)
*Chang-Hwan Son*
...
<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件下的人脸图像质量下降显著影响识别精度，现有方法缺乏专门针对天气退化的模块，导致纹理和结构失真。

Method: 提出了一种GAN框架，包含局部统计特征变换（SFFT）和退化无关特征嵌入（DAFE）模块，分别用于增强局部统计分布对齐和鲁棒特征提取。
...
### [62] [NexViTAD: Few-shot Unsupervised Cross-Domain Defect Detection via Vision Foundation Models and Multi-Task Learning](https://arxiv.org/abs/2507.07579)
...
Method: 采用分层适配器模块融合Hiera和DINO-v2特征，共享子空间投影策略，多任务解码器架构，以及基于Sinkhorn-K-means的异常评分方法。
...
### [63] [Temporal Unlearnable Examples: Preventing Personal Video Data from Unauthorized Exploitation by Object Tracking](https://arxiv.org/abs/2507.07483)
...
<details>
  <summary>Details</summary>
Motivation: 随着社交媒体的兴起，大量用户上传的视频被用于视觉目标跟踪（VOT）训练，但数据隐私问题被忽视。

Method: 提出了一种生成TUEs的框架，并引入时间对比损失以增强效果。

Result: 实验表明，该方法在视频数据隐私保护方面达到最先进性能，且具有强迁移性。
...
### [64] [Rationale-Enhanced Decoding for Multi-modal Chain-of-Thought](https://arxiv.org/abs/2507.07685)
...
<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在CoT推理中常忽视生成的中间理由内容，导致推理效果不佳，需改进。

Method: 重新定义多模态CoT推理为KL约束的奖励最大化问题，提出RED解码策略，结合视觉和理由条件分布。

Result: RED在多个基准测试和LVLMs中显著优于标准CoT及其他解码方法。

Conclusion: RED为提升LVLMs的CoT推理忠实性和准确性提供了实用有效的方法，推动了多模态系统的发展。
...
### [65] [Driving by Hybrid Navigation: An Online HD-SD Map Association Framework and Benchmark for Autonomous Vehicles](https://arxiv.org/abs/2507.07487)
*Jiaxu Wan,Xu Wang,Mengwei Xie,Xinyuan Chang,Xinran Liu,Zheng Pan,Mu Xu,Ding Yuan*

Main category: cs.CV

TL;DR: 论文提出了首个用于混合导航在线地图关联的基准OMA，并提出了基于注意力机制的新框架MAT，以提升自动驾驶车辆的规划能力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆依赖全球SD地图和在线HD地图进行导航，但现有研究忽视了二者的关联，导致实际应用中存在挑战。

Method: 提出了OMA基准，包含大量道路和车道路径数据，并设计了基于路径感知和空间注意力机制的MAT框架。

Result: OMA包含480k道路和260k车道路径，MAT框架能有效理解几何和拓扑对应关系。

Conclusion: OMA和MAT为自动驾驶车辆的混合导航提供了实用工具，代码和数据集已开源。
...
<details>
  <summary>Details</summary>
Motivation: 理解并改进在数据集偏移下的模型校准性能，为实践者提供稳健校准的方法。

Method: 比较多种后处理校准方法和训练中校准策略（如标签平滑），在多种自然偏移和不同分类任务中测试。

Result: 发现熵正则化和标签平滑结合效果最佳；后处理校准方法在少量OOD数据下更稳健；新校准方法未显著优于简单方法；校准改进常以牺牲分布内校准为代价。

Conclusion: 结合熵正则化、标签平滑和预训练模型微调，以及校准前置的集成方法，能获得最佳校准效果。
...
### [67] [Divergence Minimization Preference Optimization for Diffusion Model Alignment](https://arxiv.org/abs/2507.07510)
...
Result: DMPO在PickScore上比现有基线至少提升64.6%，表现优异。
...
Conclusion: ViaPT为视觉提示调优提供了新范式，平衡了数据集级和实例级知识，同时减少了参数。
...
### [69] [GGMotion: Group Graph Dynamics-Kinematics Networks for Human Motion Prediction](https://arxiv.org/abs/2507.07515)
*Shuaijin Wan,Huaijiang Sun*

Main category: cs.CV

TL;DR: GGMotion提出了一种基于分组图网络的动力学-运动学模型，通过捕捉关节的多尺度依赖关系提升运动预测的物理合理性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将人体姿态表示为抽象图结构，忽略了关节间的物理依赖，导致学习困难和不真实的运动生成。

Method: 提出分组图网络，利用径向场捕捉时空依赖，结合组内和组间交互模块及等变MLP更新关节特征，并通过辅助损失监督运动先验。

Result: 在Human3.6M、CMU-Mocap和3DPW基准测试中表现优异，显著提升了短期运动预测性能。

Conclusion: GGMotion通过建模多尺度物理依赖，显著提升了运动预测的物理合理性和性能。
...
### [70] [MUVOD: A Novel Multi-view Video Object Segmentation Dataset and A Benchmark for 3D Segmentation](https://arxiv.org/abs/2507.07519)
...
<details>
  <summary>Details</summary>
Motivation: 动态场景中的4D对象分割研究不足，主要原因是缺乏高质量的多视角视频数据集。
...
TL;DR: 研究了内容拼图求解器对缺失、边缘腐蚀和内容腐蚀的鲁棒性，发现传统方法性能下降明显，但深度学习模型通过数据增强可显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有拼图求解器在真实场景（如文物或文件碎片重组）中缺乏对实际挑战的评估。

Method: 引入三种拼图损坏类型，评估启发式和深度学习求解器的表现。

Result: 传统方法性能随损坏增加快速下降，深度学习模型（如Positional Diffusion）通过微调表现更优。

Conclusion: 研究为提升真实世界文物自动重建提供了方向。
...
### [72] [Spline Deformation Field](https://arxiv.org/abs/2507.07521)
...
<details>
  <summary>Details</summary>
Motivation: 当前密集点轨迹建模方法存在空间一致性不足、模型不透明或依赖启发式初始化的问题，且隐式表示在稀疏时间信号插值中的潜力未充分挖掘。

Method: 采用基于样条的轨迹表示，引入低秩时变空间编码来建模节点特性，取代传统的耦合时空技术。

Result: 在稀疏输入下拟合连续场时表现出优越的时间插值性能，同时在动态场景重建中达到与先进方法竞争的质量。

Conclusion: 该方法在保持运动一致性和高效性方面优于现有技术，无需依赖线性混合蒙皮或尽可能刚性的约束。
...
### [73] [MAPEX: Modality-Aware Pruning of Experts for Remote Sensing Foundation Models](https://arxiv.org/abs/2507.07527)
...
Method: MAPEX通过多模态数据预训练，采用模态条件令牌路由机制激活模态特定专家，并通过模态感知剪枝技术保留任务相关专家，简化微调和部署。

Result: 实验表明，MAPEX在多种遥感数据集上表现优于全监督训练和现有遥感基础模型。

Conclusion: MAPEX有效解决了多模态任务中的模态不匹配问题，提升了模型效率和性能。
...
### [74] [Beyond the Linear Separability Ceiling](https://arxiv.org/abs/2507.07574)
*Enrico Vompa,Tanel Tammet,Mohit Vaishnav*

Main category: cs.CV

TL;DR: 论文研究了视觉语言模型（VLMs）在抽象推理任务中的线性推理瓶颈，提出线性可分性天花板（LSC）概念，发现该瓶颈源于语言模型的推理路径问题而非感知能力不足，并通过任务依赖性干预展示了可解决的方案。


<details>
  <summary>Details</summary>
Motivation: 探索VLMs在抽象推理任务中的性能限制，特别是线性可分性对模型推理能力的制约。
...
Conclusion: 研究表明，VLMs的稳健推理依赖于针对性对齐而非单纯的表征学习改进。
...
### [75] [Towards Continuous Home Cage Monitoring: An Evaluation of Tracking and Identification Strategies for Laboratory Mice](https://arxiv.org/abs/2507.07929)
...
Method: 结合多目标跟踪器（MouseTracks）、基于Transformer的ID分类器（Mouseformer）和轨迹关联线性程序（MouseMap）实现实时识别。

Result: 算法以30帧/秒的速度准确识别小鼠，相比现有方法提高了跟踪效率并减少了ID切换。
...
### [76] [Diffusion-Guided Knowledge Distillation for Weakly-Supervised Low-Light Semantic Segmentation](https://arxiv.org/abs/2507.07578)
...
Method: 通过DGKD对齐正常光和低光特征，利用扩散去噪和知识蒸馏；DGF2则整合深度图作为光照不变的几何先验，增强结构特征学习。
...
### [77] [HOTA: Hierarchical Overlap-Tiling Aggregation for Large-Area 3D Flood Mapping](https://arxiv.org/abs/2507.07585)
*Wenfeng Jia,Bin Liang,Yuxi Lu,Attavit Wilaiwongsakul,Muhammad Arif Khan,Lihong Zheng*

Main category: cs.CV

TL;DR: HOTA是一种多尺度推理策略，结合SegFormer和深度估计模块，实现3D洪水测绘，显著提高了洪水边界和深度的准确性。


<details>
  <summary>Details</summary>
Motivation: 洪水是频繁的自然灾害，现有产品在空间细节和覆盖范围之间存在权衡，且常忽略洪水深度。HOTA旨在填补这一空白，提供快速灾害响应所需的大范围3D洪水信息。

Method: HOTA采用分层重叠平铺聚合策略，结合SegFormer模型和DEM差异法，通过多尺度推理捕获局部和公里级洪水特征，同时估计洪水深度。

Result: 在澳大利亚Kempsey洪水案例中，HOTA将IoU从73%提升至84%，边界误差小于0.5米，证明了其在大范围3D洪水测绘中的准确性。

Conclusion: HOTA能够生成适合快速灾害响应的高精度大范围3D洪水地图。
...
### [78] [Scaling RL to Long Videos](https://arxiv.org/abs/2507.07966)
*Yukang Chen,Wei Huang,Baifeng Shi,Qinghao Hu,Hanrong Ye,Ligeng Zhu,Zhijian Liu,Pavlo Molchanov,Jan Kautz,Xiaojuan Qi,Sifei Liu,Hongxu Yin,Yao Lu,Song Han*

Main category: cs.CV

TL;DR: 论文提出了一种全栈框架，通过强化学习扩展视觉语言模型（VLMs）在长视频中的推理能力，并引入了大规模数据集LongVideo-Reason、两阶段训练流程和高效训练基础设施MR-SP。
...
Method: 1. 构建包含52K长视频QA对的数据集LongVideo-Reason；2. 采用两阶段训练流程（CoT-SFT和RL）；3. 开发MR-SP训练基础设施，支持高效长视频RL训练。

Result: LongVILA-R1-7B在长视频QA基准测试中表现优异，超越Video-R1-7B，甚至与Gemini-1.5-Pro相当；MR-SP系统实现2.1倍加速。
...
### [79] [Stable-Hair v2: Real-World Hair Transfer via Multiple-View Diffusion Model](https://arxiv.org/abs/2507.07591)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散方法在多视角生成一致且高质量的头发转移方面研究不足，而这对数字人类和虚拟化身等应用至关重要。

Method: 提出多视角训练数据生成流程，包括扩散模型、数据增强修复模型和面部微调的多视角扩散模型，并设计多阶段训练策略。

Result: 实验表明，该方法能准确转移逼真发型，并在多视角下实现无缝一致的效果，显著优于现有方法。
...
### [80] [Geometry Forcing: Marrying Video Diffusion and 3D Representation for Consistent World Modeling](https://arxiv.org/abs/2507.07982)
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型通常无法捕捉几何感知结构，需要弥补其与真实3D世界之间的差距。

Method: 提出Angular Alignment和Scale Alignment两种对齐目标，利用预训练几何模型引导扩散模型学习3D表示。

Result: 实验表明，该方法显著提升了生成视频的视觉质量和3D一致性。

Conclusion: Geometry Forcing有效增强了视频扩散模型的几何感知能力。
...
### [81] [HiM2SAM: Enhancing SAM2 with Hierarchical Motion Estimation and Memory Optimization towards Long-term Tracking](https://arxiv.org/abs/2507.07603)
...
Method: 引入分层运动估计策略（轻量级线性预测与选择性非线性优化）和优化内存库（区分长短期记忆帧）。
...
### [82] [Multi-Granular Spatio-Temporal Token Merging for Training-Free Acceleration of Video LLMs](https://arxiv.org/abs/2507.07990)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（LLMs）因时空令牌数量庞大导致计算复杂度呈二次方增长，亟需一种高效且无需训练的方法来减少令牌数量。

Method: STTM通过多粒度空间令牌转换和时空维度上的定向合并，利用视频数据的局部冗余性，实现了令牌的高效合并。

Result: 在六个视频问答基准测试中，STTM表现优于现有令牌减少方法，实现了2倍和3倍的速度提升，仅伴随0.5%和2%的准确率下降。

Conclusion: STTM是一种高效、无需训练的视频令牌合并方法，显著提升了计算效率，同时保持了较高的准确性，适用于多查询场景。
...
### [83] [LOSC: LiDAR Open-voc Segmentation Consolidator](https://arxiv.org/abs/2507.07605)
...
Result: 在nuScenes和SemanticKITTI数据集上显著优于现有零样本开放词汇分割技术。
...
### [84] [Single-pass Adaptive Image Tokenization for Minimum Program Search](https://arxiv.org/abs/2507.07995)
*Shivam Duggal,Sanghyun Byun,William T. Freeman,Antonio Torralba,Phillip Isola*

Main category: cs.CV

TL;DR: KARL是一种基于Kolmogorov复杂性原理的单次自适应图像标记器，通过预测图像的最小描述长度，实现高效表示学习。
...
Method: KARL采用单次前向传播预测图像的适当标记数量，并在达到近似Kolmogorov复杂性时停止。训练过程类似于倒置强化学习范式。

Result: KARL在单次操作中匹配了现有自适应标记器的性能，并分析了编码器/解码器大小、连续与离散标记化等因素的影响。
...
### [85] [SpatialViz-Bench: Automatically Generated Spatial Visualization Reasoning Tasks for MLLMs](https://arxiv.org/abs/2507.07610)
...
Result: 模型表现差异大，显示出与人类直觉不符的困难感知、2D到3D性能骤降，以及过度依赖公式推导而非可视化。
...
### [86] [Traceable Evidence Enhanced Visual Grounded Reasoning: Evaluation and Methodology](https://arxiv.org/abs/2507.07999)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型如OpenAI-o3在视觉推理方面表现不足，缺乏全面评估其能力的基准测试。

Method: 提出TreeBench基准测试，基于三个原则设计，并通过专家标注和严格质量控制生成405个挑战性问题。同时提出TreeVGR训练范式，结合强化学习提升定位和推理能力。

Result: TreeBench对现有模型极具挑战性，最高准确率不足60%。TreeVGR显著提升了多个基准测试的性能。

Conclusion: TreeBench填补了视觉推理评估的空白，TreeVGR证明了可追溯性对视觉推理的重要性。
...
### [87] [ViLU: Learning Vision-Language Uncertainties for Failure Prediction](https://arxiv.org/abs/2507.07620)
...
Conclusion: ViLU通过其架构和训练方法实现了有效的不确定性量化，适用于大规模数据集。
...
### [88] [T-GVC: Trajectory-Guided Generative Video Coding at Ultra-Low Bitrates](https://arxiv.org/abs/2507.07633)
*Zhitao Wang,Hengyu Man,Wenrui Li,Xingtao Wang,Xiaopeng Fan,Debin Zhao*

Main category: cs.CV

TL;DR: 提出了一种轨迹引导的生成视频编码框架（T-GVC），通过语义感知稀疏运动采样和轨迹对齐损失约束，在超低码率下实现更真实的视频重建。
...
Method: 采用语义感知稀疏运动采样管道，结合轨迹对齐损失约束，实现低码率下关键时间语义信息的保留。

Result: 在超低码率条件下优于传统编解码器和端到端视频压缩方法，且比现有文本引导方法实现更精确的运动控制。
...
### [89] [Bridging the gap in FER: addressing age bias in deep learning](https://arxiv.org/abs/2507.07638)
*F. Xavier Gaya-Morey,Julia Sanchez-Perez,Cristina Manresa-Yee,Jose M. Buades-Rubio*

Main category: cs.CV

TL;DR: 该论文研究了基于深度学习的面部表情识别（FER）系统中的年龄偏见问题，并提出三种缓解策略，显著提高了老年人群体的识别准确性。


<details>
  <summary>Details</summary>
Motivation: 现有FER系统在年龄群体间存在性能差异，尤其是对老年人的识别存在偏见，影响公平性和可靠性。

Method: 通过可解释AI技术分析年龄偏见，提出并评估三种策略：多任务学习、多模态输入和年龄加权损失。

Result: 实验表明，这些策略显著提高了老年人群体的识别准确性，尤其是对易错表情。
...
### [90] [MolCLIP: A Molecular-Auxiliary CLIP Framework for Identifying Drug Mechanism of Action Based on Time-Lapsed Mitochondrial Images](https://arxiv.org/abs/2507.07663)
...
Method: 设计分子辅助CLIP框架，结合度量学习优化视频特征。

Result: 在MitoDataset上，药物识别和MoA分类mAP分别提升51.2%和20.5%。
...
### [91] [Attend-and-Refine: Interactive keypoint estimation and quantitative cervical vertebrae analysis for bone age assessment](https://arxiv.org/abs/2507.07670)
*Jinhee Kim,Taesung Kim,Taewoo Kim,Dong-Wook Kim,Byungduk Ahn,Yoon-Ji Kim,In-Seok Song,Jaegul Choo*

Main category: cs.CV

TL;DR: 研究提出了一种基于深度学习的ARNet模型，用于通过侧位头颅X光片预测儿童正畸中的生长潜力，显著减少了关键点标注的人工工作量。
...
### [92] [Action Unit Enhance Dynamic Facial Expression Recognition](https://arxiv.org/abs/2507.07678)
*Feng Liu,Lingna Gu,Chen Shi,Xiaolan Fu*

Main category: cs.CV

TL;DR: 提出了一种结合AU-expression知识的动态面部表情识别架构（AU-DFER），通过量化AU对表情的贡献并设计权重矩阵，结合深度学习网络提升识别效果。实验表明该方法优于现有技术，并探讨了解决数据标签不平衡的策略。


<details>
  <summary>Details</summary>
Motivation: 现有动态面部表情识别（DFER）研究主要从深度学习角度进行特征学习，缺乏对AU-expression知识的利用。本文旨在通过结合AU知识提升DFER的建模效果。

Method: 提出AU-DFER架构，量化AU对表情的贡献并设计权重矩阵，引入AU损失函数将先验知识与深度学习网络结合。在主流数据集上验证，并探讨解决数据标签不平衡的策略。

Result: 实验表明AU-DFER优于现有技术，无需额外计算即可提升效果。同时，AU损失函数设计有助于解决数据标签不平衡问题。

Conclusion: 结合AU-expression知识能有效提升DFER性能，解决数据不平衡问题是关键。该方法为首次尝试，代码已开源。
...
### [93] [Tree-Mamba: A Tree-Aware Mamba for Underwater Monocular Depth Estimation](https://arxiv.org/abs/2507.07687)
...
<details>
  <summary>Details</summary>
Motivation: 现有Mamba方法在水下单目深度估计（UMDE）任务中表现不佳，因其状态扫描策略无法有效建模水下图像的结构特征，且现有数据集标签不可靠。

Method: 提出了一种树感知扫描策略，基于特征相似性自适应构建最小生成树，并通过上下遍历灵活聚合空间拓扑特征。

Result: 实验表明，Tree-Mamba在定性和定量评估中均优于现有方法，且计算效率高。

Conclusion: Tree-Mamba方法有效解决了UMDE任务中的挑战，并提供了高质量的数据集支持未来研究。
...
### [94] [Motion-Aware Adaptive Pixel Pruning for Efficient Local Motion Deblurring](https://arxiv.org/abs/2507.07708)
*Wei Shang,Dongwei Ren,Wanying Zhang,Pengfei Zhu,Qinghua Hu,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 论文提出了一种新的局部运动模糊去除方法，通过可训练的模糊区域预测器和帧内运动分析器，优化计算资源分配并提升模糊恢复效果。
...
Method: 1. 提出可训练模糊区域预测器，排除清晰区域；2. 开发帧内运动分析器，将像素位移转换为运动轨迹；3. 结合重构损失、再模糊损失和掩码损失进行端到端训练。
...
### [95] [One Object, Multiple Lies: A Benchmark for Cross-task Adversarial Attack on Unified Vision-Language Models](https://arxiv.org/abs/2507.07709)
*Jiale Zhao,Xinyang Jiang,Junyao Gao,Yuhao Xue,Cairong Zhao*

Main category: cs.CV

TL;DR: 论文提出CrossVLAD数据集和CRAFT方法，用于评估和攻击统一视觉语言模型（VLMs）在多任务指令下的对抗性表现。


<details>
  <summary>Details</summary>
Motivation: 统一视觉语言模型（VLMs）在多任务指令下存在安全挑战，需要评估其对抗性攻击的跨任务有效性。

Method: 提出CrossVLAD数据集和CRAFT方法，通过区域中心攻击和令牌对齐技术实现跨任务对抗攻击。

Result: CRAFT在Florence-2等VLMs上表现优于现有方法，成功率高。
...
### [96] [Understanding Dataset Bias in Medical Imaging: A Case Study on Chest X-rays](https://arxiv.org/abs/2507.07722)
*Ethan Dack,Chengliang Dai*

Main category: cs.CV

TL;DR: 研究探讨了开源胸部X光数据集中是否存在数据集偏见，通过数据集变换增加任务难度，并测试了多种网络架构。


<details>
  <summary>Details</summary>
Motivation: 医疗图像因敏感性难以开源，导致某些数据集被广泛使用，需验证其中是否存在偏见，以确保AI方法关注病理而非捷径。

Method: 对NIH、CheXpert、MIMIC-CXR和PadChest数据集进行变换，应用多种网络架构进行分析。

Result: 研究发现医疗数据集中可能存在偏见，需进一步验证。

Conclusion: 研究鼓励医疗影像领域进行更多可解释性研究，并推动开源数据集的发展。
...
### [97] [RAPS-3D: Efficient interactive segmentation for 3D radiological imaging](https://arxiv.org/abs/2507.07730)
...
<details>
  <summary>Details</summary>
Motivation: 现有2D模型（如SAM）难以直接应用于3D医学影像（如CT或MRI），且现有3D方法通常需要复杂的策略（如滑动窗口）来管理计算资源，导致推理时间长和实现复杂。

Method: 受SegVol启发，提出了一种简化的3D可提示分割方法，避免使用滑动窗口等复杂策略。

Result: 该方法在减少推理时间的同时，实现了最先进的性能。

Conclusion: 该方法简化了3D分割的实现，提高了效率，适用于医学影像分析。
...
### [98] [Energy-Guided Decoding for Object Hallucination Mitigation](https://arxiv.org/abs/2507.07731)
*Xixi Liu,Ailin Deng,Christopher Zach*

Main category: cs.CV

TL;DR: 提出了一种基于能量的解码方法，减少视觉语言模型中的对象幻觉，显著提升了三个基准数据集的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在限制，如依赖特定解码方法或复杂视觉输入修改，需更简单有效的解决方案。

Method: 动态选择能量分数最低的隐藏状态层，以减少对“是”回答的偏见。

Result: 在三个VQA数据集上平均准确率提升4.82%，“是”比例差距减少8.81%。
...
### [99] [EEvAct: Early Event-Based Action Recognition with High-Rate Two-Stream Spiking Neural Networks](https://arxiv.org/abs/2507.07734)
*Michael Neumeier,Jules Lecomte,Nils Kazinski,Soubarna Banik,Bing Li,Axel von Arnim*

Main category: cs.CV

TL;DR: 提出了一种基于事件的高速率双流脉冲神经网络（SNN），在THU EACT-50数据集上准确率提升2%，并展示了其在体育动作捕捉中的实际应用。


<details>
  <summary>Details</summary>
Motivation: 早期识别人类活动对安全性和响应性至关重要，事件视觉传感器因其高时间分辨率和低延迟成为理想选择，但现有方法在早期预测能力上受限。

Method: 采用高速率双流SNN处理事件数据，避免传统方法累积事件导致的低速率问题，提升早期预测能力。

Result: 在THU EACT-50数据集上，准确率比之前工作提升2%，并通过早期识别框架验证了其性能。

Conclusion: 该方法在准确率和早期预测能力上均优于现有技术，并在实际应用中展示了潜力。
...
### [100] [Sparse-Dense Side-Tuner for efficient Video Temporal Grounding](https://arxiv.org/abs/2507.07744)
*David Pujol-Perich,Sergio Escalera,Albert Clapés*

Main category: cs.CV

TL;DR: 论文提出了一种稀疏-密集侧调谐器（SDST），用于视频时间定位（VTG），通过引入基于参考的可变形自注意力机制，显著提升了性能，同时减少了参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖预训练模型的最终层特征，限制了其在新领域的适应性。参数高效微调（如侧调谐）虽有效，但忽略了MR的稀疏性。

Method: 提出SDST架构和基于参考的可变形自注意力机制，首次将InternVideo2骨干网络集成到侧调谐框架中。
...
### [101] [X-RAFT: Cross-Modal Non-Rigid Registration of Blue and White Light Neurosurgical Hyperspectral Images](https://arxiv.org/abs/2507.07747)
...
<details>
  <summary>Details</summary>
Motivation: 将高光谱成像整合到荧光引导神经外科手术中，以提供实时定量荧光测量，从而改善手术决策。

Method: 引入X-RAFT模型，改进RAFT光流模型以适应跨模态输入，采用自监督方式微调图像编码器。

Result: 与基线方法相比，误差减少36.6%，与现有跨模态光流方法（CrossRAFT）相比减少27.83%。
...
### [102] [Deep Learning based 3D Volume Correlation for Additive Manufacturing Using High-Resolution Industrial X-ray Computed Tomography](https://arxiv.org/abs/2507.07757)
...
<details>
  <summary>Details</summary>
Motivation: 增材制造中的几何不准确性会影响部件性能，传统DVC方法配准困难且计算量大。
...
### [103] [SCOOTER: A Human Evaluation Framework for Unrestricted Adversarial Examples](https://arxiv.org/abs/2507.07776)
...
Result: 发现现有攻击方法无法生成不可感知图像，且自动化系统与人类感知不一致。

Conclusion: SCOOTER框架填补了评估无限制对抗攻击的空白，强调人类感知的重要性。
...
### [104] [Robust and Generalizable Heart Rate Estimation via Deep Learning for Remote Photoplethysmography in Complex Scenarios](https://arxiv.org/abs/2507.07795)
...
Conclusion: 该方法显著提升了rPPG信号的提取精度和泛化能力，适用于复杂场景。
...
### [105] [Synergistic Prompting for Robust Visual Recognition with Missing Modalities](https://arxiv.org/abs/2507.07802)
...
### [106] [Patient-specific vs Multi-Patient Vision Transformer for Markerless Tumor Motion Forecasting](https://arxiv.org/abs/2507.07811)
*Gauthier Rotsart de Hertaing,Dani Manjah,Benoit Macq*

Main category: cs.CV

TL;DR: 该研究首次将Vision Transformers（ViT）应用于无标记肺肿瘤运动预测，比较了患者特异性（PS）和多患者（MP）模型的性能。PS模型在训练数据充足时表现更优，而MP模型在无需重新训练的情况下对解剖变异更具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管基于深度学习的无标记方法在肺肿瘤运动预测中已有应用，但基于Transformer的架构尚未探索。本研究旨在填补这一空白，并评估两种训练策略在临床限制下的表现。

Method: 使用31名患者的4DCT扫描生成的DRR训练MP模型，保留第32名患者用于评估。PS模型仅使用目标患者的规划数据。两种模型均预测1秒内的肿瘤运动，并通过ADE和FDE指标评估性能。

Result: PS模型在规划数据（T1）上表现更优，但MP模型在治疗数据（T2）上无需重新训练即可达到可比性能，且对解剖变异更具鲁棒性。
...
### [107] [Rethinking Query-based Transformer for Continual Image Segmentation](https://arxiv.org/abs/2507.07831)
*Yuchen Zhu,Cheng Shi,Dingyou Wang,Jiajin Tang,Zhengxuan Wei,Yu Wu,Guanbin Li,Sibei Yang*

Main category: cs.CV

TL;DR: SimCIS提出了一种简单而强大的基线方法，通过直接选择图像特征进行查询分配，解决了类增量图像分割中的可塑性和输入顺序依赖问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有解耦框架在类增量图像分割中存在的可塑性丧失和对输入数据顺序的严重依赖问题。

Method: 基于高度聚合的图像特征，直接选择特征进行查询分配，确保完美对齐以保留对象性，同时引入跨阶段一致性选择和视觉查询重放机制。

Result: SimCIS在各种分割任务、设置、分割和输入数据顺序下均优于现有最先进方法。

Conclusion: SimCIS通过简单而有效的方法解决了类增量图像分割中的关键问题，并在实验中表现出色。
...
### [108] [3D-ADAM: A Dataset for 3D Anomaly Detection in Advanced Manufacturing](https://arxiv.org/abs/2507.07838)
...
<details>
  <summary>Details</summary>
Motivation: 表面缺陷是制造业低产出的主要原因之一，但现有数据集无法反映真实工业场景，限制了异常检测方法的改进。

Method: 通过4种工业深度成像传感器采集14,120个高分辨率扫描，涵盖217个独特零件，并标注27,346个缺陷实例和8,110个机械设计特征。

Result: 评估表明，3D-ADAM对现有SOTA模型提出了显著挑战，并通过专家调查验证了其工业相关性。
...
### [109] [THUNDER: Tile-level Histopathology image UNDERstanding benchmark](https://arxiv.org/abs/2507.07860)
...
<details>
  <summary>Details</summary>
Motivation: 数字病理学领域近期涌现大量基础模型，但缺乏统一的评估标准，尤其在医疗领域需关注模型的稳健性和不确定性。

Method: 开发THUNDER基准，支持高效比较多种模型在多样化数据集和任务中的表现，分析特征空间并评估预测的稳健性和不确定性。

Result: 对23种基础模型在16个数据集上进行了全面比较，涵盖多样化任务、特征分析和稳健性评估。

Conclusion: THUNDER为数字病理学领域提供了可靠的模型评估工具，代码已公开。
...
### [110] [Single-Step Latent Diffusion for Underwater Image Restoration](https://arxiv.org/abs/2507.07878)
*Jiayi Wu,Tianfu Wang,Md Abu Bakr Siddique,Md Jahidul Islam,Cornelia Fermuller,Yiannis Aloimonos,Christopher A. Metzler*

Main category: cs.CV

TL;DR: 论文提出了一种名为SLURPP的新型网络架构，结合了预训练的潜在扩散模型和显式场景分解，用于水下图像恢复，显著提升了速度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素域扩散的水下图像恢复方法在复杂场景中效果不佳且计算量大，需要更高效的解决方案。

Method: 结合预训练的潜在扩散模型和显式场景分解，设计物理基础的水下图像合成管道生成多样化训练数据。

Result: SLURPP比现有方法快200倍，PSNR提升约3 dB，在合成和真实数据上均表现优异。

Conclusion: SLURPP通过创新的架构和数据生成方法，显著提升了水下图像恢复的效果和效率。
...
### [111] [MIRA: A Novel Framework for Fusing Modalities in Medical RAG](https://arxiv.org/abs/2507.07902)
...
Conclusion: MIRA通过优化检索和推理，有效提升了医学诊断中多模态大语言模型的事实准确性。
...
### [112] [Not Only Consistency: Enhance Test-Time Adaptation with Spatio-temporal Inconsistency for Remote Physiological Measurement](https://arxiv.org/abs/2507.07908)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度rPPG模型在未见部署环境中的适应性受限于隐私问题和实时调整需求，因此需要一种无需源数据访问的实时自监督自适应方法。

Method: 基于生理学先验知识，提出CiCi框架，利用频率域的一致性和时间域的不一致性，并结合梯度动态控制机制以避免冲突。

Result: 在五个不同数据集上的实验表明，该方法在TTA协议下优于现有技术，实现了最先进的实时自监督自适应性能。

Conclusion: CiCi框架为rPPG任务提供了一种高效、稳定的测试时间自适应解决方案，无需源数据即可实现实时性能提升。
...
### [113] [TinierHAR: Towards Ultra-Lightweight Deep Learning Models for Efficient Human Activity Recognition on Edge Devices](https://arxiv.org/abs/2507.07949)
...
Method: TinierHAR采用残差深度可分离卷积、GRU和时间聚合技术，通过系统消融研究分析了空间-时间组件的贡献。

Result: 在14个公共HAR数据集上评估，TinierHAR参数减少2.7倍（相比TinyHAR）和43.3倍（相比DeepConvLSTM），计算量减少6.4倍和58.6倍，同时保持平均F1分数。

Conclusion: 研究提供了高效HAR系统的设计指南，并开源所有材料以促进边缘HAR研究。
...
### [114] [Martian World Models: Controllable Video Synthesis with Physically Accurate 3D Reconstructions](https://arxiv.org/abs/2507.07978)
...
TL;DR: 提出了一种合成火星景观视频的方法，包括数据重建和视频生成两部分，解决了火星数据稀缺和与地球图像的领域差距问题。
...
Method: 1) M3arsSynth：从NASA的立体导航图像重建3D火星环境并渲染视频；2) MarsGen：基于初始帧和可选条件生成逼真视频。
...
### [115] [OST-Bench: Evaluating the Capabilities of MLLMs in Online Spatio-temporal Scene Understanding](https://arxiv.org/abs/2507.07984)
...
Method: 通过高效的数据收集流程，构建了包含1.4k场景和10k问答对的OST-Bench数据集，并评估了多个领先MLLMs的表现。

Result: 实验显示，现有MLLMs在复杂时空推理任务中表现不佳，尤其是在在线探索和长期记忆需求下准确性显著下降。
...
### [116] [CLIP Won't Learn Object-Attribute Binding from Natural Data and Here is Why](https://arxiv.org/abs/2507.07985)
...
Result: 发现数据属性（如高属性密度和完整标注）是CLIP学习可靠绑定的关键，而非批量大小或硬负样本。

Conclusion: 通过优化数据属性，可以显著提升CLIP模型的绑定能力。
...
### [117] [Doodle Your Keypoints: Sketch-Based Few-Shot Keypoint Detection](https://arxiv.org/abs/2507.07994)
...
### [118] [MGVQ: Could VQ-VAE Beat VAE? A Generalizable Tokenizer with Multi-group Quantization](https://arxiv.org/abs/2507.07997)
...
<details>
  <summary>Details</summary>
Motivation: 现有VQ-VAEs在量化策略上仍有改进空间，与VAEs存在较大差距，目标是缩小这一差距并提升重建质量。

Method: 保留潜在维度以保留编码特征，并引入一组子码本进行量化。

Result: 在ImageNet和8个零样本基准测试中达到最先进性能，显著优于SD-VAE（rFID 0.49 vs. 0.91）。
...
### [119] [Impact of Pretraining Word Co-occurrence on Compositional Generalization in Multimodal Models](https://arxiv.org/abs/2507.08000)
...
<details>
  <summary>Details</summary>
Motivation: 理解训练数据中概念组合对模型性能的影响，特别是常见对象在不常见配对中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs认知偏差的来源，区分预训练、微调和训练随机性的影响。

Method: 采用两步因果实验方法：1）多次微调模型以研究训练随机性对30多种认知偏差的影响；2）通过交叉调换指令数据集（cross-tuning）隔离偏差来源。

Result: 训练随机性虽引入变异性，但认知偏差主要由预训练决定。共享预训练主干的模型表现出更相似的偏差模式。

Conclusion: 理解微调模型的偏差需考虑其预训练起源，这为未来评估和减少LLMs偏差提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs作为人类受试者替代品在调查中的可靠性及其对已知响应偏差的敏感性。

Method: 测试9种不同LLMs对世界价值观调查（WVS）问题的响应，应用11种扰动（问题表述和答案结构），模拟超过167,000次访谈。

Result: 所有模型均表现出不同程度的近因偏差，且对语义变化和组合扰动敏感；大模型更稳健但仍不完美。
...
TL;DR: SynthTextEval是一个用于全面评估合成文本的工具包，支持多维度评估，旨在提升合成文本的可用性及隐私保护。


<details>
  <summary>Details</summary>
Motivation: 合成文本在隐私保护和高风险领域AI开发中具有潜力，但需要系统化评估以确保其效用、公平性和隐私安全性。

Method: 提供多维度评估功能，包括下游系统效用、公平性、隐私泄漏风险、分布差异及专家反馈，支持用户上传或生成数据。

Result: 工具包在医疗和法律领域的数据上展示了功能和有效性，标准化了评估指标。

Conclusion: SynthTextEval通过标准化评估，提升了合成文本的可行性，促进了AI开发中的隐私保护。
...
TL;DR: 本文提出了一种针对医疗领域大语言模型（LLMs）的安全评估协议，填补了现有文献的空白，并通过构建PatientSafetyBench和案例研究MediPhi模型，为医疗LLMs的安全部署奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在医疗领域的广泛应用，其安全性问题日益突出，尤其是对患者和临床医生等不同角色的潜在影响。现有评估多集中于通用安全基准，缺乏针对医疗领域的专门评估。

Method: 提出了一种医疗领域的安全评估协议，包括患者和临床医生视角的评估，并构建了包含466个样本的PatientSafetyBench。通过案例研究MediPhi模型，验证了协议的有效性。

Result: 首次定义了医疗LLMs的安全评估标准，并通过多视角（患者、临床医生和普通用户）的红队测试，为医疗领域的安全部署提供了基础。

Conclusion: 本文填补了医疗LLMs安全评估的空白，为未来更安全的医疗应用提供了方法论和工具支持。
...
TL;DR: 论文研究了协作学习中的中断现象，提出了一种在多组对话环境中识别中断的先进方法，适用于课堂场景。


<details>
  <summary>Details</summary>
Motivation: 中断在协作学习中至关重要，但现有研究多在单对话环境中进行，无法适应多组对话的复杂场景。

Method: 分析了单对话和多组对话中的中断检测，开发了一种对重叠语音鲁棒的中断识别方法。

Result: 提出了一种适用于课堂环境的先进中断识别方法，并揭示了中断在协作互动中的语言和韵律特征。

Conclusion: 研究为未来在多组对话中跟踪中断提供了基础，并强调了重叠语音的影响。
...
Result: 方法在礼貌性、相关性、信息量和事实准确性上优于基线，并通过消融研究和人类评估验证了有效性。

Conclusion: 该框架能生成高质量反驳言论，且细化步骤显著提升质量和人类偏好。
...
Result: 模型在文本分类任务中表现出高效和竞争力，生成图具有语义组织特性。

Conclusion: 新模型在效率和性能上优于传统方法，适用于长文本处理。
...
TL;DR: MedReadCtrl是一个可控制可读性的指令调优框架，显著降低了LLMs在医疗领域的可读性错误，优于GPT-4，并受到专家青睐。
...
Conclusion: MedReadCtrl为患者教育和AI医疗服务的公平访问提供了可扩展的解决方案。
...
Method: 结合LLMs、人工标注和自动提示优化（APO）的管道，提取临床记录中的驱逐状态。

Result: 创建的SDoH数据集包含14个细粒度类别，微调LLMs在驱逐和其他SDoH上的Macro-F1分别达到88.8%和90.3%，优于其他模型。
...
<details>
  <summary>Details</summary>
Motivation: 开发轻量级、可解释的模型，用于自然语言解释时间序列模式，适合隐私敏感或设备端部署。
...
TL;DR: 论文提出SAND框架，通过自我一致性动作采样和执行引导的动作评估，让LLM代理在行动前进行显式思考，从而提升性能。
...
Method: 采用自我一致性动作采样和执行引导的动作评估，合成逐步动作思考，并用这些轨迹微调LLM代理。

Result: 在两个代表性任务中，SAND比初始监督微调平均提升20%，且优于现有最佳方法。

Conclusion: SAND框架通过显式动作思考有效提升了LLM代理的性能。
...
Result: 在GSM8k和MATH基准测试中平均提升7%，在域外数据集OlympiadBench和AIME 2024上分别提升10%和12%。
...
<details>
  <summary>Details</summary>
Motivation: 增强LLM的推理能力是研究重点，但现有方法依赖复杂代码结构，容易过拟合。
...
Method: 采用DyGIE++和多BERT变体（如MatSciBERT、PubMedBERT）进行NER和RE，构建FC-CoMIcs数据集，并通过数据标注和模型微调提升性能。

Result: PubMedBERT在NER任务中F1得分为82.19%，MatSciBERT在RE任务中F1得分为66.10%，优于通用科学模型（如BlueBERT）。

Conclusion: 领域特定BERT模型在ORR催化剂信息提取中优于通用模型，展示了自动化文献分析的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在AI中的广泛应用，人们对其能力边界（如幻觉问题）和代理任务（如自主执行任务）的兴趣日益增长，需要明确LLMs的能力范围。

Method: 从计算复杂性的角度分析LLM推理，展示其在计算和代理任务中的局限性，并提供具体示例。
...
<details>
  <summary>Details</summary>
Motivation: 解决非英语语言心理支持数据集稀缺的问题，提升心理支持对话的质量和规模。

Method: 通过Dialog Generator生成大规模心理对话，Dialog Modifier优化对话质量，结合自动和人工审核构建CPsDD数据集，并开发CADSS系统。

Result: CPsDD包含68K对话，覆盖13组、16种心理问题等；CADSS在CPsDD和ESConv数据集上达到最优性能。

Conclusion: 该框架有效解决了心理支持数据稀缺问题，CADSS系统在对话支持任务中表现出色。
...
TL;DR: 该研究将语音活动预测（VAP）应用于三元对话场景，预测即将发生的轮流发言，发现三元对话训练的VAP模型优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 传统研究多关注二元对话，本研究旨在探索三元多参与者对话中的轮流发言预测。

Method: 利用声学数据训练多个VAP模型，应用于日本三元对话数据集。

Result: 三元对话训练的VAP模型在所有模型中表现最佳，但对话类型影响准确性。

Conclusion: VAP可用于三元对话轮流发言预测，未来将整合到口语对话系统中。
...
<details>
  <summary>Details</summary>
Motivation: 研究目的是探索LLMs在多语言主观性检测任务中的潜力，尤其是在数据质量较差或标注不一致的情况下。

Method: 使用精心设计的少样本提示，结合LLMs，并尝试了高级提示工程技术（如辩论LLMs和示例选择策略）。

Result: 系统在CheckThat! 2025任务中表现优异，尤其在阿拉伯语和波兰语中排名第一，其他语言中也名列前茅。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨语言RAG研究多基于开放领域数据（如维基百科），掩盖了检索问题。本研究旨在探索特定领域（企业数据）中的跨语言检索行为。

Method: 使用阿拉伯语-英语的企业数据集构建基准，系统研究多语言检索行为，并提出一种强制均衡检索的策略。

Result: 发现检索是跨语言领域场景的主要瓶颈，尤其是查询与文档语言不同时。提出的策略显著提升了性能。

Conclusion: 跨语言检索在特定领域中存在挑战，改进检索策略可显著提升RAG应用的实际效果。
...
TL;DR: 研究探讨了长链思维监督微调（SFT）和强化学习（RL）在多模态推理模型中的协同作用，发现两者结合未能产生叠加效果，反而导致性能权衡。


<details>
  <summary>Details</summary>
Motivation: 探索SFT和RL在多模态推理模型中的联合效果，以提升复杂推理能力。

Method: 通过多种训练策略（如两阶段、交替或渐进训练）和数据混合、模型合并，评估SFT和RL的作用。

Result: SFT提升复杂问题性能但导致冗长，RL促进泛化和简洁但效果有限；两者结合未实现叠加优势。

Conclusion: 需开发更自适应的方法以实现SFT和RL的无缝结合，充分发挥多模态推理模型的潜力。
...
Method: 引入M4Doc框架，将仅图像编码器与多模态大语言模型的多模态表示对齐，利用预训练知识。

Result: 实验表明翻译质量显著提升，尤其在跨域泛化和复杂文档图像场景中。

Conclusion: M4Doc通过轻量级模型高效利用多模态知识，提升了文档图像翻译的性能。
...
Method: 通过蒙特卡洛边缘化独立破坏样本，以O(1/sqrt(K))的速率收敛到后验分布，提出推理时集成方法。
...
TL;DR: 论文探讨了通过知识蒸馏压缩大语言模型（LLMs）以在资源受限环境中保持问答任务性能的方法，结果显示学生模型在参数减少57.1%的情况下仍能保留90%以上的性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在NLP任务中表现优异，但计算需求高，限制了其在资源受限环境中的部署。

Method: 使用知识蒸馏（KD）压缩LLMs，并在问答任务（SQuAD和MLQA）上评估学生模型（基于Pythia和Qwen2.5家族）在零样本和单样本提示条件下的表现。

Result: 学生模型在参数减少57.1%的情况下仍能保留90%以上的性能，且单样本提示比零样本提示表现更优。
...
TL;DR: 论文提出了一种改进检索增强生成（RAG）的方法，强调检索效率的重要性，并证明大规模微调并非必要，通过改进提示和少量训练即可达到或超越现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 解决复杂问题时，现有方法主要关注RAG的准确性和召回率，而忽略了检索效率的重要性。本文旨在证明高效检索的可行性及其对RAG性能的提升。

Method: 采用标准ReAct流程并改进提示，结合少量监督和基于强化学习的微调，以减少检索次数并提升性能。

Result: 在HotPotQA等基准测试中，改进后的方法在检索效率（减少近半检索次数）和性能上均优于现有方法，且仅需少量训练数据（1000例）。

Conclusion: 研究表明，高效检索是RAG的关键改进方向，大规模微调并非必需，通过优化提示和少量训练即可显著提升性能。
...
TL;DR: NOIR是一种评估文本摘要质量的量化指标，结合语义保留和摘要长度压缩，提供自动化评估工具。


<details>
  <summary>Details</summary>
Motivation: 传统摘要评估依赖人工参考摘要，耗时且主观，需要自动化、客观的评估方法。

Method: 利用语言模型嵌入测量语义相似性，结合摘要压缩率，量化语义保留与压缩的权衡。

Result: 实验表明NOIR能有效捕捉语义保留与压缩的权衡，并与人类评估质量相关。
...
TL;DR: 论文提出了一种名为Simulated Attention Score (SAS)的方法，通过模拟更多注意力头和更大的隐藏维度来提升性能，同时保持模型紧凑。
...
Method: 引入SAS，通过将低维头表示投影到高维空间，模拟更多头和更大隐藏维度；同时提出Parameter-Efficient Attention Aggregation (PEAA)控制参数成本。

Result: 在多个数据集和任务上的实验表明，SAS显著优于其他注意力变体。
...
<details>
  <summary>Details</summary>
Motivation: 针对大型语言模型（LLMs）微调的资源密集问题和传统RAG实现的可扩展性与准确性不足，提出K2RAG框架以改进这些问题。

Method: K2RAG整合了密集和稀疏向量搜索、知识图谱和文本摘要，并引入预处理步骤以减少训练时间。

Result: 在MultiHopRAG数据集上，K2RAG实现了0.57的平均答案相似度得分和0.82的第三四分位数相似度，训练时间减少93%，执行速度提高40%，VRAM需求减少三倍。

Conclusion: K2RAG在准确性、效率和可扩展性方面均优于传统RAG实现，为知识扩展提供了更优解决方案。
...
TL;DR: 该论文验证了Vec2Text方法能从文本嵌入中重构原始文本，并探讨了其隐私风险与防御措施。


<details>
  <summary>Details</summary>
Motivation: 传统认为传输文本嵌入而非原始文本是隐私保护的，但Vec2Text方法挑战了这一假设，因此需要进一步验证其效果与风险。
...
### [152] [When Large Language Models Meet Law: Dual-Lens Taxonomy, Technical Advances, and Ethical Governance](https://arxiv.org/abs/2507.07748)
...
Conclusion: 提出新的分类法，系统化法律推理、检索、预测和争议解决，为法律人工智能的下一阶段奠定基础。
...
### [153] [Code-Switching in End-to-End Automatic Speech Recognition: A Systematic Literature Review](https://arxiv.org/abs/2507.07741)
*Maha Tufail Agro,Atharva Kulkarni,Karima Kadaoui,Zeerak Talat,Hanan Aldarmaki*

Main category: cs.CL

TL;DR: 本文对端到端自动语音识别（ASR）模型中的代码转换（CS）进行了系统文献综述，分析了语言、数据集、模型选择和性能，并讨论了挑战。
...
Method: 收集并手动标注了同行评审论文，分析了语言、数据集、指标、模型选择和性能。

Result: 提供了当前研究资源和挑战的见解，并指出了未来研究的机会和空白。

Conclusion: 本文为端到端ASR中代码转换的研究提供了系统综述，指导未来研究方向。
...
TL;DR: StreamUni提出了一种统一的语音-语言大模型（LSLM），通过语音思维链（CoT）实现流式语音翻译（StreamST），无需依赖分段模型，同时完成语音分段、策略决策和翻译生成。


<details>
  <summary>Details</summary>
Motivation: 现有流式语音翻译方法依赖句子级分段模型，限制了上下文信息，且策略学习复杂。StreamUni旨在解决这些问题。

Method: StreamUni利用语音CoT引导LSLM生成多阶段输出，实现语音分段、策略决策和翻译生成的一体化。提出流式CoT训练方法，优化低延迟策略。
...
Method: 训练一个仅解码器的Transformer模型，通过STL语法构建的小词汇表，反转STL公式的语义嵌入。
...
### [156] [On the Effect of Instruction Tuning Loss on Generalization](https://arxiv.org/abs/2507.07817)
*Anwoy Chatterjee,H S V N S Kowndinya Renduchintala,Sumit Bhatia,Tanmoy Chakraborty*

Main category: cs.CL

TL;DR: 论文提出了一种名为Weighted Instruction Tuning（WIT）的方法，通过差异化加权提示和响应标记来优化指令微调的损失函数，实验表明其优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统指令微调中损失函数仅计算响应标记，忽略提示标记，可能导致性能不佳。本文探讨了这种损失函数是否最优。

Method: 提出WIT方法，差异化加权提示和响应标记的损失，并通过实验验证其效果。

Result: 实验表明，传统指令微调损失函数性能较差，而WIT方法在多个模型和数据集上表现更优，且对后续偏好对齐训练更有利。
...
### [157] [Understanding and Controlling Repetition Neurons and Induction Heads in In-Context Learning](https://arxiv.org/abs/2507.07810)
*Nhi Hoai Doan,Tatsuya Hiraoka,Kentaro Inui*

Main category: cs.CL

TL;DR: 研究探讨了大语言模型（LLMs）识别重复输入模式的能力与其上下文学习（ICL）性能的关系，从技能神经元（重复神经元）视角分析，发现其影响因层深度而异，并提出减少重复输出同时保持ICL性能的策略。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注注意力头，本文从技能神经元（重复神经元）的角度探讨LLMs的重复模式识别与ICL性能的关系。

Method: 通过实验分析重复神经元对ICL性能的影响，并与归纳头进行比较。

Result: 重复神经元的影响因所在层深度而异，研究发现减少重复输出同时保持ICL性能的策略。

Conclusion: 从技能神经元视角揭示了LLMs的重复模式识别与ICL性能的关系，为优化模型提供了新思路。
...
TL;DR: 提出了一种基于条件单字标记化的新方法，通过并行数据中的源语言标记来调整目标标记概率，以优化跨语言语义对齐。


<details>
  <summary>Details</summary>
Motivation: 扩展单字标记化方法，通过引入源语言标记的条件概率，提升跨语言语义对齐能力。

Method: 在固定源标记器的基础上，学习目标标记器，最大化跨语言语义对齐。

Result: 在四种语言对上评估，机器翻译质量无提升，但语言建模困惑度一致降低。

Conclusion: 条件概率估计的二次方扩展导致数据效率瓶颈，可能需要其他参数化方法。
...
TL;DR: 研究探讨了检索增强生成（RAG）中实体共指对检索和生成性能的影响，发现共指消解能提升检索效果和问答性能，且小模型从中获益更多。


<details>
  <summary>Details</summary>
Motivation: RAG框架在自然语言处理中表现优异，但共指复杂性会引入歧义，影响性能。本研究旨在系统分析共指对RAG的影响。

Method: 通过比较不同池化策略在检索任务中的表现，并分析共指消解对问答任务的影响。

Result: 共指消解提升了检索效果和问答性能，均值池化在共指消解后表现最佳，小模型受益更明显。
...
Method: 引入复合算子ϕ(⋅,γ(⋅))，其中ϕ驱动主语义收敛，γ解决局部子博弈，结合不动点迭代和博弈论。

Result: 证明了语义均衡的存在唯一性，提出了基于Kozlov-Maz'ya-Rossmann公式的ϕ-拓扑，并通过Yoneda引理验证一致性。

Conclusion: 该框架为AI嵌入空间提供了实用的语义模式传播工具，兼具数学严谨性和实际应用价值。
...
### [161] [DTECT: Dynamic Topic Explorer & Context Tracker](https://arxiv.org/abs/2507.07910)
...
<details>
  <summary>Details</summary>
Motivation: 现有动态主题建模技术缺乏统一的解释和用户友好探索支持，DTECT旨在填补这一空白。

Method: DTECT提供统一工作流，支持数据预处理、多模型架构、评估指标，并引入LLM驱动的自动主题标注和自然语言聊天界面。

Result: DTECT显著提升了主题模型的解释性和用户友好性，支持动态主题的跟踪和理解。

Conclusion: DTECT是一个开源平台，有效帮助用户从文本数据中提取时间洞察。
...
### [162] [DocCHA: Towards LLM-Augmented Interactive Online diagnosis System](https://arxiv.org/abs/2507.07870)
...
<details>
  <summary>Details</summary>
Motivation: 现有对话健康代理（CHAs）缺乏适应性和透明性，无法满足临床诊断中多轮推理和结构化对话的需求。
...
### [163] [Automating MD simulations for Proteins using Large language Models: NAMD-Agent](https://arxiv.org/abs/2507.07887)
*Achuth Chandrasekhar,Amir Barati Farimani*

Main category: cs.CL

TL;DR: 论文提出了一种自动化流程，利用大型语言模型（Gemini 2.0 Flash）和Python脚本，结合Selenium网页自动化，简化分子动力学模拟输入文件的生成。
...
Method: 通过整合Gemini的代码生成与迭代优化能力，自动编写、执行和修订脚本，利用CHARMM GUI生成NAMD输入文件，并通过后处理软件进一步优化输出。
...
### [164] [MIRIX: Multi-Agent Memory System for LLM-Based Agents](https://arxiv.org/abs/2507.07957)
*Yu Wang,Xi Chen*

Main category: cs.CL

TL;DR: MIRIX是一种模块化、多代理的记忆系统，解决了AI记忆的关键挑战，支持多模态体验，并在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有AI记忆解决方案能力有限，无法实现个性化、抽象化和可靠回忆用户信息。

Method: MIRIX包含六种结构化记忆类型和多代理框架，支持动态更新与检索。
...
### [165] [SAGE: A Visual Language Model for Anomaly Detection via Fact Enhancement and Entropy-aware Alignment](https://arxiv.org/abs/2507.07939)
...
Method: SAGE结合SFE和E-DPO，利用AD-PL数据集和MLE评估框架优化模型。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何在资源有限的医疗环境中利用语言模型支持临床决策，特别是在风湿病学等复杂领域。

Method: 结合检索增强生成（RAG）的小语言模型（SLMs）进行诊断和治疗评估。

Result: SLMs+RAG在诊断和治疗性能上优于LLMs，且更节能、适合本地部署，但未达到专家级准确度。
...
TL;DR: MedThink-Bench是一个用于评估大型语言模型（LLMs）医学推理能力的基准测试，包含500个医学领域问题，并提出了LLM-w-Ref评估框架。实验表明，该框架与专家判断高度相关，且较小模型可能优于大型专有模型。
...
Method: 提出MedThink-Bench基准测试和LLM-w-Ref评估框架，结合专家标注的逐步推理和LLM-as-a-Judge机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法受限于预定义的工作流和静态工具集，限制了模型的灵活性和问题解决能力。

Method: 开发PyVision框架，支持多轮交互和动态生成Python工具，并对其工具使用进行分类分析。

Result: PyVision显著提升了性能，GPT-4.1在V*上提升7.8%，Claude-4.0-Sonnet在VLMsAreBlind-mini上提升31.1%。

Conclusion: 动态工具生成使模型不仅能使用工具，还能发明工具，推动了更具代理性的视觉推理。
...
### [170] [g2o vs. Ceres: Optimizing Scan Matching in Cartographer SLAM](https://arxiv.org/abs/2507.07142)
...
### [171] [Self-Wearing Adaptive Garments via Soft Robotic Unfurling](https://arxiv.org/abs/2507.07221)
...
Method: 采用基于展开的部署方法，使SWAG能贴合人体，减少皮肤与衣物的摩擦，实现更安全高效的穿衣过程。

Result: SWAG系统在各种衣物配置下均表现出有效的穿衣能力。
...
### [172] [3D Steering and Localization in Pipes and Burrows using an Externally Steered Soft Growing Robot](https://arxiv.org/abs/2507.07225)
*Yimeng Qin,Jared Grinberg,William Heap,Allison M. Okamura*

Main category: cs.RO

TL;DR: 提出了一种可转向的藤蔓机器人，专为管道和洞穴环境设计，具有主动分支选择、小半径导航、适应急转弯和实时定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有藤蔓机器人在人造和自然通道中的导航能力有限，尤其是在分支和急转弯环境中。

Method: 采用外部尖端安装的转向机制，通过改变生长方向并在必要时支撑管道壁，实现三维自由度的转向。

Result: 机器人可实现51.7度的最大转向角，导航半径小至2.5厘米的管道，适应急转弯，并支持实时3D定位。
...
TL;DR: LangNav是一个专注于语言理解的语义导航数据集，用于测试代理在不同详细程度的语言指令下定位目标对象的能力。LangNavBench是基于LangNav的基准测试，评估模型对属性、空间和关系线索的处理能力。提出的MLFM方法在LangNav数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个专注于语言理解的语义导航基准测试，以评估代理如何基于语言指令定位目标对象。
...
Conclusion: LangNav和LangNavBench为语言驱动的语义导航提供了系统评估工具，MLFM方法在处理复杂语言指令时表现优异。
...
### [174] [Classifying Emergence in Robot Swarms: An Observer-Dependent Approach](https://arxiv.org/abs/2507.07315)
*Ricardo Vega,Cameron Nowzari*

Main category: cs.RO

TL;DR: 论文探讨了“涌现”和“群体”缺乏共识定义的问题，提出了一个框架，强调这些概念的主观性，并区分了可观察和不可观察的状态。


<details>
  <summary>Details</summary>
Motivation: 由于“涌现”和“群体”缺乏统一的定义，新研究者和专家在理解和应用这些概念时存在困难。

Method: 提出一个框架，将可观察状态与潜在不可观察状态分离，以比较现有定义。

Result: 认为这些概念是主观的，取决于观察者的视角和隐性知识，而非系统本身。

Conclusion: 强调群体行为不仅由行为本身定义，还由生成行为的过程定义，为机器人群体系统的设计提供支持。
...
### [175] [Effects of Wrist-Worn Haptic Feedback on Force Accuracy and Task Speed during a Teleoperated Robotic Surgery Task](https://arxiv.org/abs/2507.07327)
*Brian B. Vuong,Josie Davidson,Sangheui Cheon,Kyujin Cho,Allison M. Okamura*

Main category: cs.RO

TL;DR: 研究提出将触觉反馈从手部移至腕部，以解决手部触觉反馈对手术机器人操作的影响，并通过实验验证腕部触觉反馈在力控制任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 手部触觉反馈会干扰手术机器人操作，因此探索腕部触觉反馈的可行性。

Method: 使用软气动手腕触觉设备，测试参与者在有无触觉反馈下完成力控制任务的准确性。
...
### [176] [UniTracker: Learning Universal Whole-Body Motion Tracker for Humanoid Robots](https://arxiv.org/abs/2507.07356)
*Kangning Yin,Weishuai Zeng,Ke Fan,Zirui Wang,Qiang Zhang,Zheng Tian,Jingbo Wang,Jiangmiao Pang,Weinan Zhang*

Main category: cs.RO

TL;DR: UniTracker框架通过集成CVAE显式建模人类运动多样性，提升人形机器人全身控制的鲁棒性和适应性。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于师生框架方法在策略蒸馏中运动多样性损失及对新行为泛化能力有限的问题。

Method: 提出UniTracker框架，将CVAE整合到学生策略中，利用学习到的CVAE先验显式建模运动多样性。

Result: 实验表明，UniTracker在运动质量、对新参考的泛化及部署鲁棒性上显著优于MLP-based DAgger基线。
...
### [177] [Data-driven Kinematic Modeling in Soft Robots: System Identification and Uncertainty Quantification](https://arxiv.org/abs/2507.07370)
*Zhanhong Jiang,Dylan Shah,Hsin-Jung Yang,Soumik Sarkar*

Main category: cs.RO

TL;DR: 本文提出了一种基于共形预测的软体机器人运动学建模框架，用于量化预测不确定性，并通过非线性集成方法提升建模精度。


<details>
  <summary>Details</summary>
Motivation: 软体机器人运动学建模因其高度非线性和复杂性而具有挑战性，现有数据驱动方法存在预测不确定性问题。
...
### [178] [PILOC: A Pheromone Inverse Guidance Mechanism and Local-Communication Framework for Dynamic Target Search of Multi-Agent in Unknown Environments](https://arxiv.org/abs/2507.07376)
*Hengrui Liu,Yi Feng,Qilong Zhang*

Main category: cs.RO

TL;DR: PILOC框架通过局部感知和通信解决多智能体搜索救援中的动态和未知环境问题，结合信息素机制和深度强化学习提升效率和鲁棒性。
...
### [179] [Towards Safe Autonomous Driving: A Real-Time Safeguarding Concept for Motion Planning Algorithms](https://arxiv.org/abs/2507.07444)
*Korbinian Moller,Rafael Neher,Marvin Seegert,Johannes Betz*

Main category: cs.RO

TL;DR: 提出了一种用于自动驾驶运动规划模块的安全保障概念，通过引入时间监控器扩展了现有方法，确保系统响应的及时性。


<details>
  <summary>Details</summary>
Motivation: 解决复杂或基于学习的软件中运动规划功能安全性的挑战，尤其是在嵌入式实时环境中的在线验证问题。

Method: 结合约束可行性检查和基于成本的合理性指标，在实时操作系统上实现原型，监控规划输出的时间一致性。

Result: 初步结果显示安全保障模块在实时范围内运行，并能有效检测不安全轨迹。

Conclusion: 研究提供了一个模块化、可扩展的运行时轨迹验证框架，未来工作包括完善安全保障逻辑并通过硬件在环仿真和车辆测试验证其有效性。
...
### [180] [SCREP: Scene Coordinate Regression and Evidential Learning-based Perception-Aware Trajectory Generation](https://arxiv.org/abs/2507.07467)
*Juyeop Han,Lukas Lao Beyer,Guilherme V. Cavalheiro,Sertac Karaman*

Main category: cs.RO

TL;DR: 提出了一种结合场景坐标回归（SCR）和轨迹优化的感知感知框架，用于GPS拒绝环境下的自主飞行，显著降低了定位误差。


<details>
  <summary>Details</summary>
Motivation: 在GPS拒绝的室内环境中，视觉惯性里程计（VIO）会随时间累积漂移，而SCR提供了无漂移的高精度绝对位姿估计，因此需要一种方法结合两者的优势。

Method: 框架结合了基于证据学习的SCR位姿估计器和后退水平轨迹优化器，优化器引导相机朝向预测可靠的场景坐标的像素，同时使用固定滞后平滑器融合SCR和IMU数据。

Result: 仿真中，该规划器将平移（旋转）平均误差分别降低了54%/15%（40%/31%），硬件在环实验验证了框架的可行性。

Conclusion: 提出的框架在GPS拒绝环境中显著提高了自主飞行的定位精度，并通过实验验证了其可行性。
...
### [181] [FiDTouch: A 3D Wearable Haptic Display for the Finger Pad](https://arxiv.org/abs/2507.07661)
...
<details>
  <summary>Details</summary>
Motivation: 探索指尖触觉设备在虚拟现实、医疗培训和远程机器人操作等领域的应用潜力，提升用户体验和交互效果。

Method: 设计FiDTouch设备，利用微型倒置Delta机器人提供接触、压力、皮肤拉伸和振动反馈，并通过用户研究评估其性能。

Result: 用户研究表明，FiDTouch能有效提供静态空间接触和皮肤拉伸刺激，提升触觉反馈的精确性和动态变化速度。

Conclusion: FiDTouch通过精确的触觉反馈，显著提升了人机交互和人机协作的沉浸感和效率。
...
### [182] [Adaptive Gaussian Mixture Models-based Anomaly Detection for under-constrained Cable-Driven Parallel Robots](https://arxiv.org/abs/2507.07714)
...
<details>
  <summary>Details</summary>
Motivation: CDPRs在负载操作任务中需要检测异常（如风或电缆碰撞）以确保安全，但传统方法依赖额外传感器。本文探索仅用电机扭矩数据实现异常检测的可能性。

Method: 采用自适应GMM算法，通过校准期拟合无异常数据，实时计算马氏距离并设置统计阈值触发异常标志，定期更新模型参数以适应环境变化。

Result: 在14次长时间测试中，方法实现了100%的真阳性率和95.4%的平均真阴性率，检测延迟为1秒，优于功率阈值和非自适应GMM方法。

Conclusion: 该方法仅需扭矩数据即可高效检测异常，适应性强，适用于动态环境中的CDPRs。
...
### [183] [Implementation and Assessment of an Augmented Training Curriculum for Surgical Robotics](https://arxiv.org/abs/2507.07718)
...
<details>
  <summary>Details</summary>
Motivation: 通过整合高级辅助算法，提升手术机器人培训课程的全面性和鲁棒性，从而改善临床表现。

Method: 开发了一种触觉增强的虚拟现实模拟器，包含8个手术任务，通过物理引擎和高级触觉接口实现运动引导和性能评分。

Result: 实验研究表明，增强的机器人辅助显著提升了训练效果，并促进了技能向无辅助临床场景的转移。

Conclusion: 该模拟器为手术机器人培训提供了高效工具，能够显著提升学员的技能水平和临床适应能力。
...
### [184] [Distributed Surface Inspection via Operational Modal Analysis by a Swarm of Miniaturized Vibration-Sensing Robots](https://arxiv.org/abs/2507.07724)
*Thiemen Siemensma,Niels de Boer,Bahar Haghighat*

Main category: cs.RO

TL;DR: 本文研究了利用微型振动传感机器人群体在仿真环境中检测和定位结构损伤的方法，验证了其有效性。
...
Method: 结合有限元分析（Abaqus）和物理仿真（Webots），使用高斯过程估计器引导机器人探索，并通过模态分析检测损伤。

Result: 在10种随机场景中验证了方法的有效性，分析了探索半径对估计不确定性的影响。

Conclusion: 微型机器人群体在振动基结构检测中具有高效性。
...
### [185] [On the capabilities of LLMs for classifying and segmenting time series of fruit picking motions into primitive actions](https://arxiv.org/abs/2507.07745)
*Eleni Konstantinidou,Nikolaos Kounalakis,Nikolaos Efstathopoulos,Dimitrios Papageorgiou*

Main category: cs.RO

TL;DR: 该论文研究了大型语言模型（LLMs）在学习和演示（LbD）中分类和分割复杂动作为基本动作的能力，特别是在水果采摘场景中的应用。


<details>
  <summary>Details</summary>
Motivation: 通过利用LLMs替代传统的监督学习或分析方法，旨在使方法更易于在实际场景中应用和部署。

Method: 研究了三种不同的微调方法，并在使用UR10e机器人进行水果采摘时捕获的数据集上进行了比较。

Result: 论文展示了LLMs在分类和分割复杂动作方面的潜力。

Conclusion: LLMs在学习和演示任务中具有实际应用价值，特别是在需要将复杂动作分解为基本动作的场景中。
...
### [186] [IRAF-SLAM: An Illumination-Robust and Adaptive Feature-Culling Front-End for Visual SLAM in Challenging Environments](https://arxiv.org/abs/2507.07752)
*Thanh Nguyen Canh,Bao Nguyen Quoc,Haolan Zhang,Bupesh Rethinam Veeraiah,Xiem HoangVan,Nak Young Chong*

Main category: cs.RO

TL;DR: IRAF-SLAM提出了一种光照鲁棒且自适应的特征筛选前端，通过图像增强、动态特征提取和特征筛选策略，显著提升了vSLAM在复杂光照条件下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有SLAM系统因固定前端参数在光照变化下性能下降，需一种自适应方法提升鲁棒性。

Method: 1. 图像增强预处理；2. 基于图像熵、像素强度和梯度的动态特征提取；3. 密度分布和光照影响因子的特征筛选。

Result: 在TUM-VI和EuRoC数据集上，IRAF-SLAM显著减少跟踪失败并提高轨迹精度。

Conclusion: 自适应前端策略能有效提升vSLAM鲁棒性且计算开销低。
...
### [187] [Collaborative Human-Robot Surgery for Mandibular Angle Split Osteotomy: Optical Tracking based Approach](https://arxiv.org/abs/2507.07794)
*Zhe Han,Huanyu Tian,Tom Vercauteren,Da Liu,Changsheng Li,Xingguang Duan*

Main category: cs.RO

TL;DR: 提出了一种人机协作系统，用于在术前计划和外科医生指导下进行下颌角劈开截骨术（MASO），通过任务分解和光学跟踪系统提高手术精度和安全性。


<details>
  <summary>Details</summary>
Motivation: 尽管技术和器械有所进步，MASO手术的成功仍高度依赖外科医生的经验，因此需要一种更精确和安全的方法。

Method: 将手术分解为机器人主导的位置和方向控制，以及外科医生主导的力控制，并使用光学跟踪系统实现患者追踪。

Result: 在真实模型上的钻孔实验显示，计划与实际钻孔点的平均误差为1.85mm。

Conclusion: 该系统通过人机协作和光学跟踪技术，显著提高了MASO手术的精度和安全性。
...
### [188] [Beyond Robustness: Learning Unknown Dynamic Load Adaptation for Quadruped Locomotion on Rough Terrain](https://arxiv.org/abs/2507.07825)
...
Method: 提出负载特性建模方法，结合强化学习控制策略，实现负载动态推断和稳定交互。

Result: 仿真实验表明，该方法在突发负载抵抗、负载稳定和复杂地形负重运动方面优于其他方法。

Conclusion: 该方法有效解决了四足机器人在未知负载下的控制问题，具有实际应用潜力。
...
### [189] [Perceptual Distortions and Autonomous Representation Learning in a Minimal Robotic System](https://arxiv.org/abs/2507.07845)
...
<details>
  <summary>Details</summary>
Motivation: 探索感知失真如何影响自主机器人的内部世界表示，以及机器人如何在不依赖显式空间信息的情况下学习导航策略。

Method: 使用模拟的两轮机器人，配备距离传感器和指南针，在简单方形环境中随机探索，分析其传感器数据。
...
### [190] [ROS Help Desk: GenAI Powered, User-Centric Framework for ROS Error Diagnosis and Debugging](https://arxiv.org/abs/2507.07846)
*Kavindie Katuwandeniya,Samith Rajapaksha Jayasekara Widhanapathirana*

Main category: cs.RO

TL;DR: ROS Help Desk通过直观的错误解释和调试支持，简化了机器人操作系统的诊断过程，减少了维护时间。
...
Result: 测试表明，系统能主动准确诊断问题，减少维护时间。
...
### [191] [Improving AEBS Validation Through Objective Intervention Classification Leveraging the Prediction Divergence Principle](https://arxiv.org/abs/2507.07872)
...
<details>
  <summary>Details</summary>
Motivation: 解决开放循环重模拟中AEBS激活分析的复杂性，避免人工标注的主观性和局限性。

Method: 采用基于预测分歧原则（PDP）的规则分类方法，应用于简化AEBS系统。

Result: 揭示了该方法的优势、局限性和系统要求，表明结合人工标注可提升分类透明度和一致性。

Conclusion: 该方法有望补充现有实践，为更可靠和可重复的AEBS验证框架铺平道路。
...
### [192] [UniTac: Whole-Robot Touch Sensing Without Tactile Sensors](https://arxiv.org/abs/2507.07980)
...
<details>
  <summary>Details</summary>
Motivation: 大多数商用机器人缺乏触觉皮肤，难以实现基本触觉功能，如接触定位。UniTac旨在普及触觉感知，为HRI研究人员提供现成工具。
...
TL;DR: 该研究提出了一种多层图神经网络框架，用于提升电网故障预测和基于韧性的变电站聚类，显著优于传统方法。
...
Result: 模型在30天F1分数上达到0.8935，优于XGBoost和随机森林3.2%和2.7%，因果层移除后性能显著下降。聚类分析识别出8个风险组，最高风险组每年388.4次故障。

Conclusion: 该框架通过改进故障预测和风险聚类，支持主动电网管理。
...
### [194] [Probability-Raising Causality for Uncertain Parametric Markov Decision Processes with PAC Guarantees](https://arxiv.org/abs/2507.07319)
*Ryohei Oura,yuji Ito*

Main category: eess.SY

TL;DR: 提出了一种在不确定参数MDP中识别不良行为潜在原因的方法，通过参数采样、模型检查和样本覆盖集实现。


<details>
  <summary>Details</summary>
Motivation: 复杂决策系统需要可靠解释不良行为，但现有方法未解决MDP转移概率不确定性问题。

Method: 使用参数采样、模型检查和样本覆盖集识别潜在原因，定义基于概率提升原则的状态子集为原因。

Result: 证明识别子集作为原因的概率超过指定阈值，并最大化访问子集的概率下界。

Conclusion: 通过采样获得概率近似正确下界，验证了方法在路径规划场景中的有效性。
...
### [195] [Distributed and adaptive model predictive control for vehicle platoon systems under non-ideal communication](https://arxiv.org/abs/2507.07429)
*Qiaoni Han,Chengfei Xu,Zhiqiang Zuo*

Main category: eess.SY

TL;DR: 提出了一种分布式自适应模型预测控制方法，以应对无线通信不确定性对车队控制的影响，通过定制补偿数据包和优化预测时间域策略，显著减少了计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 无线通信的不确定性对车队控制性能提出了挑战，需要一种方法在非理想通信条件下优化系统响应速度和跟踪精度。

Method: 设计了补偿数据包以应对通信不确定性，提出自适应MPC方法平衡响应与精度，并引入预测时间域更新策略以减少计算需求。

Result: 仿真结果表明，该方法在保证系统性能的同时显著降低了优化问题的计算资源需求。

Conclusion: 所提方法有效解决了非理想通信对车队控制的影响，同时优化了计算资源的使用。
...
### [196] [Optimization of Probabilistic Constellation Shaping for Optical OFDM Systems with Clipping Distortion](https://arxiv.org/abs/2507.07507)
*Thanh V. Pham,Susumu Ishihara*

Main category: eess.SY

TL;DR: 该论文研究了概率星座成形（PCS）与光OFDM结合时对信号峰均比（PAPR）的影响，并提出了一种优化PCS的方法以最大化信道容量。


<details>
  <summary>Details</summary>
Motivation: PCS虽然提高了频谱效率和适应性，但与光OFDM结合时会增加PAPR，导致信号削波失真。本文旨在解决这一问题。

Method: 通过投影梯度下降法优化PCS，以最大化信道容量并考虑削波失真。

Result: 仿真结果表明，所提方法在严重削波失真条件下优于传统均匀信号传输。

Conclusion: 优化后的PCS方法能有效降低PAPR并提升系统性能。
...
### [197] [Perspective Chapter: Insights from Kalman Filtering with Correlated Noises Recursive Least-Square Algorithm for State and Parameter Estimation](https://arxiv.org/abs/2507.07588)
*Abd El Mageed Hag Elamin Khalid*

Main category: eess.SY

TL;DR: 提出了一种新的卡尔曼滤波算法KF-CN-RGELS，利用过程噪声与测量噪声的互相关性联合估计参数和系统状态，并通过理论和数值研究验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 提升线性随机系统中参数和状态的估计精度，特别是在存在确定性控制输入的情况下。

Method: 引入KF-CN-RGELS算法，利用噪声互相关性进行联合估计，并通过性能分析研究相关系数对估计精度的影响。

Result: 发现估计精度与正相关系数成正比，并通过对比实验验证了算法的优越性。

Conclusion: 该算法显著提升了估计精度，为控制系统设计和状态空间建模提供了新思路。
...
### [198] [PhysioEdge: Multimodal Compressive Sensing Platform for Wearable Health Monitoring](https://arxiv.org/abs/2507.07645)
*Rens Baeyens,Dennis Laurijssen,Jan Steckel,Walter Daems*

Main category: eess.SY

TL;DR: 论文提出了一种基于RP2350微控制器的硬件平台，用于多模态生物医学信号的高效同步采集，结合压缩感知技术降低功耗，并展示了其可扩展性和低成本优势。


<details>
  <summary>Details</summary>
Motivation: 探索压缩感知与实时嵌入式系统的结合，以实现高效、低功耗的生物医学信号采集，满足远程医疗和长期监测的需求。

Method: 开发了基于RP2350微控制器的硬件平台，支持多模态信号（如PCG、ECG、EMG、PPG和IMU）同步采集，利用Sub-1GHz无线电系统实现精确同步，并通过Wi-Fi和蓝牙集中数据。

Result: 实验表明，该系统在压缩感知下显著降低功耗，实现高效的多节点同步，并具备无线生物医学监测的可扩展性。

Conclusion: 该平台紧凑、低成本，适用于远程医疗和长期监测等多种医疗应用。
...
### [199] [Remote Renewable Energy Hubs: a Taxonomy](https://arxiv.org/abs/2507.07659)
*Victor Dachet,Antoine Dubois,Bardhyl Miftari,Raphaël Fonteneau,Damien Ernst*

Main category: eess.SY

TL;DR: 提出了一种用于远程可再生能源中心（RREH）的分类法，以解决可再生能源分布不均的问题，帮助设计和比较不同中心配置。


<details>
  <summary>Details</summary>
Motivation: 可再生能源在需求中心附近的有限可用性限制了其供应，RREH作为一种解决方案，需要系统化的分类方法以指导设计和政策制定。

Method: 通过分析RREH中可能采用的技术及其相互作用，提出了一种分类法，用于定义和比较不同中心的设计。

Result: 分类法能够更准确地描述和比较RREH的设计，并有助于发现新的配置方案。

Conclusion: 该分类法为政策制定者和工程师提供了设计RREH的工具，有助于提高成本效率和本地整合能力。
...
### [200] [Ammonia, Methane, Hydrogen and Methanol Produced in Remote Renewable Energy Hubs: a Comparative Quantitative Analysis](https://arxiv.org/abs/2507.07681)
*Antoine Larbanois,Victor Dachet,Antoine Dubois,Raphaël Fonteneau,Damien Ernst*

Main category: eess.SY

TL;DR: 研究评估了四种远程可再生能源中心（RREHs）生产合成燃料的成本，发现氨（NH3）、氢（H2）和甲醇（CH3OH）均比甲烷（CH4）更具成本效益，其中氨的成本效益最高。


<details>
  <summary>Details</summary>
Motivation: 评估不同能源载体的生产成本，并比较其技术性能的优缺点，以优化远程可再生能源中心的设计。

Method: 扩展了Berger等人的研究，引入氨、氢和甲醇三种新载体，使用GBOML建模语言对系统进行建模和优化。

Result: 三种新载体均比甲烷更具成本效益，氨的成本效益最高。

Conclusion: 氨是最具成本效益的能源载体，为远程可再生能源中心的优化提供了重要参考。
...
### [201] [Set-Based Control Barrier Functions and Safety Filters](https://arxiv.org/abs/2507.07805)
*Kim P. Wabersich,Felix Berkel,Felix Gruber,Sven Reimann*

Main category: eess.SY

TL;DR: 提出了一种基于集合的控制屏障函数（CBF）方法，用于线性系统与凸约束，结合控制不变集与预测控制，实现隐式、数据驱动的高维CBF表示。


<details>
  <summary>Details</summary>
Motivation: 工业控制应用中高性能与形式化安全保证的需求，传统CBF设计复杂，难以适用于大规模或数据驱动系统。

Method: 利用可达性分析和预测控制中的控制不变集，通过最小缩放定义隐式集合CBF，支持实时实现与学习近似。

Result: 在高维质量-弹簧-阻尼系统和运动控制任务中验证有效性，并在短采样时间的电动驱动应用中实验验证。

Conclusion: 集合CBF方法为安全关键控制提供了实用解决方案，适用于实时与数据驱动场景。
...
### [202] [Identifying the Smallest Adversarial Load Perturbations that Render DC-OPF Infeasible](https://arxiv.org/abs/2507.07850)
*Samuel Chevalier,William A. Wheeler*

Main category: eess.SY

TL;DR: 研究如何找到使直流最优潮流（DC-OPF）不可行的最小负载扰动，并提出了非凸问题的上下界求解方法。


<details>
  <summary>Details</summary>
Motivation: 在电网相关领域（如机器学习验证、网络安全、可再生能源主导的电网运行鲁棒性）中，识别这种扰动有重要应用。

Method: 应用参数化的Farkas引理处理扰动后的DC-OPF方程，并提出参数化发电控制策略以保证可解性。

Result: 通过结合非凸问题的上下界，高效逼近全局解，并在PGLib测试案例中验证了方法的有效性。

Conclusion: 提出的方法能够可靠地识别最小扰动，为电网安全提供了理论支持。
...
### [203] [DpDNet: An Dual-Prompt-Driven Network for Universal PET-CT Segmentation](https://arxiv.org/abs/2507.07126)
*Xinglong Liang,Jiaju Huang,Luyi Han,Tianyu Zhang,Xin Wang,Yuan Gao,Chunyao Lu,Lishan Cai,Tao Tan,Ritse Mann*

Main category: eess.IV

TL;DR: 论文提出DpDNet网络，通过双提示机制解决PET-CT病灶分割中不同癌症类型的特异性问题，实验表明其优于现有方法，并可用于个性化风险分层。


<details>
  <summary>Details</summary>
Motivation: PET-CT病灶分割面临噪声敏感、病灶形态多变及生理高代谢信号干扰等问题，现有方法忽略不同癌症类型的独特性。

Method: 提出DpDNet网络，结合特定提示和通用提示捕捉癌症特异性特征与共享知识，并采用提示感知头避免信息遗忘。

Result: 在四种癌症类型的PET-CT数据集上，DpDNet表现优于现有模型，并成功用于乳腺癌生存分析。

Conclusion: DpDNet可作为个性化风险分层的有效工具，辅助临床优化治疗策略。
...
### [204] [Wrist bone segmentation in X-ray images using CT-based simulations](https://arxiv.org/abs/2507.07131)
...
<details>
  <summary>Details</summary>
Motivation: 由于手腕骨X射线图像分割需要大量高质量标注数据，而标注过程耗时且依赖专家知识，因此作者希望通过模拟X射线图像来减少对真实标注数据的依赖。

Method: 作者使用CT体积生成的模拟X射线图像及其对应的10个骨标签来训练深度学习模型，并在模拟和真实X射线图像上评估模型性能。

Result: 模型在模拟数据集上的Dice分数为0.80至0.92，且在真实X射线图像上的定性分析也表现出色。
...
### [205] [Label-Efficient Chest X-ray Diagnosis via Partial CLIP Adaptation](https://arxiv.org/abs/2507.07254)
...
Method: 使用NIH Chest X-ray14数据集，通过零样本和少样本学习（每类1-16标注样本）评估模型。

Result: 在少样本任务中，平均AUC分数比零样本基线提升超过20%。

Conclusion: 该策略为常见和罕见疾病诊断提供了实用且可扩展的解决方案。
...
### [206] [Computation-resource-efficient Task-oriented Communications](https://arxiv.org/abs/2507.07422)
*Jingwen Fu,Ming Xiao,Chao Ren,Mikael Skoglund*

Main category: eess.IV

TL;DR: 论文提出了一种任务导向通信（TOC）的新方法，包含静态和动态模型，分别适用于有无计算资源限制的场景，动态模型通过动态神经网络和多出口机制优化资源分配。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习驱动的任务导向通信在资源受限系统（如手机和无人机）中计算需求高的问题。

Method: 静态模型使用神经网络作为任务导向编码器（TOE），动态模型采用动态神经网络和多出口机制，根据输入数据复杂度分配计算资源。

Result: 静态模型在传输维度、浮点运算和准确性上优于基线模型，动态模型进一步提升了准确性和计算效率。

Conclusion: 提出的方法为资源受限系统提供了高效的任务导向通信解决方案，动态模型尤其适用于计算资源有限的环境。
...
### [207] [Computationally Efficient Information-Driven Optical Design with Interchanging Optimization](https://arxiv.org/abs/2507.07789)
...
<details>
  <summary>Details</summary>
Motivation: 解决IDEAL方法在内存、运行时间和目标函数匹配性上的不足。

Method: 提出IDEAL-IO，通过交替优化密度估计和光学参数，减少计算资源需求。

Result: 在多种成像系统中验证，内存和运行时间减少6倍，设计更优。
...
### [208] [Semi-supervised learning and integration of multi-sequence MR-images for carotid vessel wall and plaque segmentation](https://arxiv.org/abs/2507.07496)
...
Result: 在52名患者数据上验证，实验表明方法有效，融合策略和半监督学习显著提升分割性能。

Conclusion: 融合策略和半监督学习在数据有限的多序列MRI应用中具有潜力，可提升颈动脉分割准确性。
...
### [209] [D-CNN and VQ-VAE Autoencoders for Compression and Denoising of Industrial X-ray Computed Tomography Images](https://arxiv.org/abs/2507.07704)
*Bardia Hejazi,Keerthana Chand,Tobias Fritsch,Giovanni Bruno*

Main category: eess.IV

TL;DR: 研究探讨了使用深度学习自动编码器压缩工业X射线计算机断层扫描（XCT）数据的方法，比较了不同压缩率和架构对数据恢复质量的影响。
...
Method: 使用深度卷积神经网络（D-CNN）和向量量化变分自动编码器（VQ-VAE）两种架构，对砂岩样本的XCT数据进行压缩和解码。
...
### [210] [Compressive Imaging Reconstruction via Tensor Decomposed Multi-Resolution Grid Encoding](https://arxiv.org/abs/2507.07707)
*Zhenyu Jin,Yisi Luo,Xile Zhao,Deyu Meng*

Main category: eess.IV

TL;DR: GridTD是一种基于张量分解和多分辨率网格编码的无监督连续表示框架，用于高效高维图像重建。
...
Method: 结合轻量级神经网络和输入张量分解模型，通过多分辨率哈希网格编码学习参数。

Result: 在多种压缩成像任务中表现优异，优于现有方法。
...
### [211] [Breast Ultrasound Tumor Generation via Mask Generator and Text-Guided Network:A Clinically Controllable Framework with Downstream Evaluation](https://arxiv.org/abs/2507.07721)
*Haoyu Pan,Hongxin Lin,Zetian Feng,Chuxuan Lin,Junyang Mo,Chu Zhang,Zijian Wu,Yi Wang,Qingqing Zheng*

Main category: eess.IV

TL;DR: 提出一种临床可控的生成框架，用于合成乳腺超声图像，解决专家标注数据稀缺问题，提升乳腺癌诊断效果。
...
Method: 结合临床描述和结构掩码生成肿瘤，通过语义-曲率掩码生成器控制肿瘤形态、回声和形状等特性。
...
### [212] [MeD-3D: A Multimodal Deep Learning Framework for Precise Recurrence Prediction in Clear Cell Renal Cell Carcinoma (ccRCC)](https://arxiv.org/abs/2507.07839)
*Hasaan Maqsood,Saif Ur Rehman Khan*

Main category: eess.IV

TL;DR: 提出一种多模态深度学习框架，整合CT、MRI、病理图像、临床数据和基因组数据，以提高透明细胞肾细胞癌（ccRCC）复发预测的准确性。
...
Result: 框架能够处理不完整数据，并整合多源互补信息，提高预测准确性。
...
### [213] [ArteryX: Advancing Brain Artery Feature Extraction with Vessel-Fused Networks and a Robust Validation Framework](https://arxiv.org/abs/2507.07920)
*Abrar Faiyaz,Nhat Hoang,Giovanni Schifitto,Md Nasir Uddin*

Main category: eess.IV

TL;DR: ArteryX是一种新型半监督动脉评估框架，用于从3D TOF MRA中高效、准确地量化血管特征，解决了现有方法的局限性，并展示了在临床中的潜力。
...
Method: ArteryX采用基于血管融合网络的标志点方法，结合半监督学习，快速处理数据并解决血管追踪问题。
