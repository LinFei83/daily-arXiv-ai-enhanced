...
TL;DR: 论文介绍了 PUBG Ally，一个可用于《PUBG: BATTLEGROUNDS》的具身智能体，能作为语音队友与玩家实时互动、自主推理和行动。它结合大模型智能体工具调用与实时游戏控制，并通过真实对局数据迭代训练，最终在线上服务中获得玩家正面反馈。


<details>
  <summary>Details</summary>
Motivation: 研究动机是构建一个能与玩家并肩作战的语音智能队友，需要同时解决两个难题：在严格延迟约束下感知和响应不断变化的游戏世界，以及与玩家自然交互并让语音与行动保持同步。

Method: 方法上，Ally 将智能体工具使用与实时游戏控制结合：语言模型智能体通过受控接口查看游戏信息、理解玩家语音、维护上下文、决定发言并给出高层动作选择，再驱动更快的控制层完成移动、战斗和恢复。训练依赖近 3.9 万场真实玩家与 Ally 共同游玩的会话数据，记录游戏过程、玩家语音、智能体决策、工具使用、动作和玩家反馈，用于迭代训练。评估使用玩家反馈和偏好比较，并针对线上部署采用模型压缩、上下文压缩、针对性安全训练、运行时防护和记忆脱敏等低延迟与安全措施。

Result: 在线上服务中，团队调查了 141 个国家的玩家。在游戏记录确认与 Ally 玩过的受访者中，当被问及是否会推荐 Ally 时，正面回应比负面回应高出 25.1 个百分点；玩家不仅将 Ally 描述为工具，也将其视为队友或伙伴。

Conclusion: PUBG Ally 表明，大模型智能体结合实时控制、真实对局数据迭代训练和线上安全部署，可以构建出被玩家认可甚至视为队友或伙伴的游戏内语音智能体。
...
### [2] [Clinical Knowledge Graphs for Chest X-Ray Device Reasoning](https://arxiv.org/abs/2609.29536)
*Harshil Lodhiya*

Main category: cs.AI

TL;DR: 该研究提出一种不确定性感知的临床知识图谱，用于胸片中的导管、管线和支撑设备评估，将设备实例、尖端估计、放置判断、来源、报告事件和时间关系表示为可连接证据，并在RANZCR CLiP测试集上评估图谱构建层。


<details>
  <summary>Details</summary>
Motivation: 现有图像模型多只输出标签或分割结果，而报告处理系统缺乏图像几何信息；同时设备评估中的不确定性、来源和时序证据难以被保留和关联。

Method: 构建视觉图谱层，将设备实例、尖端估计、放置评估、碎片来源、报告事件和时间链接等作为独立但相连的证据节点；使用RANZCR CLiP完整测试档案的保存预测进行评估，并定义类型化数据契约、不确定性表示、弃权规则、报告-图像 grounding 和纵向查询机制。

Result: 在30,083项研究、3,255名患者、五个非重叠外层折上，图谱构建器生成914,632个B7证据节点和884,549条类型化关系；118,647个B7预测设备节点保留尖端协方差、放置概率、碎片来源和碎片计数，而直接B2基线不保留这些字段。

Conclusion: 该工作是事后描述性分析，未证明报告 grounding、纵向性能或临床效用；但为胸片设备评估中保留证据的AI推理提供了可复现基础。
...
### [3] [Qwen-Planner-Agent: A Closed-Loop AI-for-AI Framework for Real-World Mobile Planner Agents](https://arxiv.org/abs/2609.29892)
...
TL;DR: 本文提出Qwen-Planner-Agent，在闭环AI-for-AI框架中实现可扩展的智能体开发与迭代优化，并在移动规划任务上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 探索AI能否既作为开发对象，又作为构建下一代AI系统的主动参与者；移动规划因长时程任务和真实设备交互成本高，成为检验该问题的理想场景。

Method: 构建闭环AI-for-AI框架，通过共享的动作-反馈-验证契约连接数据生产、模型训练与部署；包括：AI for Data的人机门控数据飞轮、AI for Training的监督冷启动与混合环境在线强化学习（提出CARE奖励机制）、以及AI驱动的模型-框架协同进化循环。

Result: Qwen-Planner-Agent在MobilePA-Bench上综合性能最佳，在工具使用、记忆、技能和子智能体协调上超越基座模型；在非移动智能体基准上也有提升，并基本保持通用能力。

Conclusion: 该闭环AI-for-AI框架能有效支持可扩展的智能体开发与迭代改进，表明AI可主动参与构建下一代AI系统。
...
TL;DR: Jev-Mobile 提出一种低频 VLM 规划与高频轻量执行解耦的移动 GUI 智能体框架，用 VLM 设定局部目标，再由快速类型化决策模型 Jev 在无障碍树定义的动作空间中反复选择动作，从而减少 VLM 调用并提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有移动 GUI 智能体大多在几乎每个交互步骤都依赖 VLM 完成规划与动作定位，导致显著延迟和模型服务成本，因此需要降低 VLM 推理频率同时保持自适应交互能力。

Method: 采用低频 VLM 规划与高频轻量执行架构：VLM 仅指定局部目标，无障碍树提供结构化可执行动作空间，快速类型化决策模型 Jev 在该空间内重复选择动作，使单次 VLM 决策可支撑多个 GUI 动作执行。

Result: 在完整 AndroidWorld 任务集上，Jev-Mobile 任务成功率达 79%，对比 SeeAct-V 的 78% 与逐步 VLM 基线的 84%；在成功轨迹中，平均端到端执行时间较逐步 VLM 降低 32.7%，平均模型 API 成本降低 73.4%。

Conclusion: 将高层 VLM 推理与低层动作执行解耦，可在保持有竞争力任务性能的同时，大幅提升移动 GUI 智能体的执行效率并降低成本。
...
### [5] [ExplorationBench: Measuring AI Systems' Exploration in Verifiable Alien Worlds](https://arxiv.org/abs/2609.30199)
...
TL;DR: 论文提出 ExplorationBench，一个用于评估 AI 系统在可验证的“异世界”环境中进行科学探索能力的基准，包含 AlienCode 和 AlienLogic 两个沙盒，共 141 个任务。


<details>
  <summary>Details</summary>
Motivation: 科学发现始于已知问题之外，AI 需要能提出假设、设计实验并迭代结果；但评估这种能力很难，因为既要验证新假设是否成立，又要区分系统是通过探索发现还是仅从预训练数据中回忆。

Method: 构建基于可执行规则且与常识冲突的 Alien Worlds 基准，包含 AlienCode（31 个发现目标、70 个任务）和 AlienLogic（24 个发现目标、70 个任务）；每个沙盒提供有缺陷的手册、任务特定环境反馈和专用工具调用模式，系统需探索后完成留出任务。

Result: 评估 10 个 AI 系统后发现，最强系统能够习得并应用不熟悉的规则，但不同轨迹间性能差异显著，持续探索可能停滞甚至逆转先前收益。
...
TL;DR: 提出 AD-WM，一种面向反事实模型预测控制（MPC）的动作判别式联合嵌入世界模型。它通过动作恢复正则提升候选动作区分能力，在仿真和真实零样本迁移任务上显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有潜在世界模型通常只优化事实预测误差，但 MPC 需要从同一状态比较不同候选动作。因此模型可能事实预测误差低，却难以区分候选动作，导致规划性能不佳。

Method: AD-WM 结合残差潜在动力学与预测器级动作恢复正则，包括逆动力学和基于条件互信息动机的归一化恢复目标，使规划转移保留动作信息；测试时丢弃辅助头，MPC 流程不变。

Result: 在 OGBench-Cube 上，AD-WM 将 hard-start 成功率从匹配 LeWM 基线的 3.7% 提升至 52.0%，并在五个仿真环境中的四个提升平均成功率。诊断显示事实预测误差和整体动作排序与闭环成功率顺序不一致，而 CEM 对齐的 elite regret 更相关。使用冻结 V-JEPA 2 编码器与匹配 DROID 后训练，零样本迁移到 Franka 平台时，基础抓放成功率从 42.2% 提升至 71.1%，无需实验室特定适配。

Conclusion: 面向规划的世界模型应保留反事实选择所需的动作依赖差异，而不仅优化事实预测精度。
...
TL;DR: 提出DeltaWAM，联合预测视觉增量与动作，并开发流式增量记忆（SDM），提升机器人控制成功率与效率。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAMs）在训练时预测密集未来帧，重复建模几乎不变的内容，并将动作条件动态与无关外观变化耦合；推理时处理完整观测导致少步动作生成瓶颈。

Method: DeltaWAM使用密集锚点、稀疏增量和动作流联合预测视觉增量与动作，并设计三种不同表示与计算共享的架构；进一步提出流式增量记忆（SDM），用紧凑的观测增量更新缓存的锚点上下文。

Result: 在RoboTwin上，DeltaWAM+SDM将平均成功率从Fast-WAM的81.3%提升至85.4%（干净设置），视觉随机化下从75.8%提升至83.9%；三种架构减少训练FLOPs 17.78-23.77%，SDM降低单步推理延迟36.57%和FLOPs 31.55%；真实世界评估中总体成功率和归一化进度均最高。

Conclusion: DeltaWAM通过增量建模和SDM有效提升机器人控制性能与计算效率，并在真实场景中表现最佳。
...
### [8] [AV-GRPO: Modality-Anchored Decoupling Diffusion Reinforcement Learning for Joint Audio-Video Generation](https://arxiv.org/abs/2609.29816)
*Zhiyu Xu,Weilong Yan,Yufei Shi,Shiyang Li,Yihao Liu,Kin-Man Lam,Yuewen Cao*

Main category: cs.CV

TL;DR: 本文提出AV-GRPO，一种面向联合音视频生成的模态锚定在线扩散强化学习框架，并构建5DAV解耦且难度可控的训练数据集，以提升生成质量、文本对齐和跨模态同步性。


<details>
  <summary>Details</summary>
Motivation: 现有联合音视频生成模型存在单模态保真度有限、文本-模态对齐不足、跨模态同步较弱等问题；直接套用强化学习后训练又面临多模态奖励纠缠、双塔联合优化成本高、同步性评估依赖配对样本导致奖励比较不公平等挑战。

Method: 提出AV-GRPO框架，包含三个模块：模态锚定采样以解耦学习信号并稳定难度；轨迹锁定的冻结塔优化以降低成本并重新分配信用；针对不同模态动态的自适应目标与扰动强度。同时构建5DAV数据集，在五个维度上解耦样本，实现系统化训练与难度控制，将耦合的多模态偏好学习转化为单模态子问题。

Result: 在JavisBench和VABench上的实验表明，AV-GRPO在LoRA和全量微调设置下，在生成质量、语义对齐和跨模态同步方面优于LTX-2.3；消融实验验证了各设计模块的有效性。

Conclusion: AV-GRPO通过模态锚定在线扩散强化学习和5DAV数据集，有效缓解了联合音视频生成中的奖励纠缠、优化成本与同步评估问题，实现了更精确的奖励归因和更好的跨模态同步，为多模态生成后训练提供了可行方案。
...
TL;DR: 本文提出了RGBD20K数据集，包含160个细粒度类别和2万对RGB-D图像，并提供高质量标注；同时提出Score-Purified Fusion（SPF）方法，在多个基准上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D语义分割基准存在类别少、规模有限和标注噪声等问题，限制了更鲁棒、更通用分割模型的发展。

Method: 构建RGBD20K数据集：扩展语义空间至160类、扩大规模至20,000对RGB-D图像，并对现有标签进行严格重评估和校正；提出新颖的Score-Purified Fusion（SPF）融合方法。

Result: RGBD20K在类别多样性和数据规模上显著超过NYUv2、SUN RGB-D等基准，并提供更干净可靠的标注；SPF在所有评估基准上均达到state-of-the-art性能。

Conclusion: RGBD20K和SPF能有效利用高质量多模态信息，推动更通用、更鲁棒的RGB-D语义分割研究。
...
### [10] [WanPE: Towards Cinematic Prompt Enhancement for Modern Text-to-Video Generation](https://arxiv.org/abs/2609.30221)
*Yubo Zhu,Yawen Shao,Ziyun Dai,Zixun Fang,Kai Zhu,Siyang Sun,Haolan Xue,Chuxin Wang,Tingyu Weng,Jingming Luo,Chen Shi,Lianghua Huang,Yufeng Ai,Yuzheng Wang,Wenyuan Zhang,Yu Shang,Yuxiang Bao,Zoubin Bi,Jie Xiao,Jinbo Xing,Jiaxing Zhao,Chongyang Zhong,Hengjian Chen,Chenwei Xie,Akide Liu,Zhehan Kan,Yu Liu,Wei Zhai,Sheng Zhong,Wei Tong*

Main category: cs.CV

TL;DR: 论文提出 WanPE，一个 397B 参数、基于 105 万真实视频训练的提示增强模型，用于将用户文本提示提升为导演级、分镜级电影化计划，并配套构建 WanPEval 基准。


<details>
  <summary>Details</summary>
Motivation: 视频生成正从简单文本提示走向 30 秒级、多镜头、复杂条件控制，文本提示需要承担导演级规划职责，如动作、镜头轨迹、灯光和声音安排，因此需要专门的提示增强能力。

Method: WanPE 通过视频接地的反向构建生成分镜级电影计划，并采用 Semantic-Consistency GRPO（SC-GRPO）在跨镜头和时间维度上保持用户需求语义一致；同时构建 WanPEval 人工标注测试集，覆盖 5 至 30 秒、不同意图粒度，并包含约 11K 盲配对评估。

Result: 在驱动 Wan3.0 视频生成器时，WanPE-397B 相比原始用户提示将人类偏好提升 10.66-18.84 分（5-15 秒），在 30 秒场景中提升 50.86 分。消融表明反向构建优于前向改写，SC-GRPO 在不同模型规模下均能保持语义保真。WanPE 在 5-15 秒领先所有已评估商业方案，在 30 秒与 Seedance 2.5 保持竞争力。

Conclusion: WanPE 证明导演级、分镜级提示增强能显著提升长时视频生成的人类偏好，反向构建与 SC-GRPO 是关键有效设计，并使该模型在短时长上达到领先商业水平，在 30 秒场景中具备竞争力。
...
### [11] [JevOut: Natural Context Can Flip Decision Models](https://arxiv.org/abs/2609.30243)
*Zixiang Xu*

Main category: cs.CL

TL;DR: 论文发现，能够将语言映射为有限选择概率分布的决策模型，容易因自然、简短的上下文添加而被误导，从而把原本正确的决策转向高置信度的错误选项。


<details>
  <summary>Details</summary>
Motivation: 现实输入通常不是孤立的，而是带有背景和上下文。作者想研究这些自然融入语境的简短补充是否会在正确答案不变的情况下，仍改变模型的决策。

Method: 对每个初始正确样本固定一个错误目标选项，利用模型输出的选项概率优化生成流畅的上下文补充，同时保持原始来源、问题、选项和标准答案不变，并评估模型是否被重定向到固定错误选项。

Result: 在64个被接受的目标评估中，优化器使Jev在508个初始正确决策中的312个发生重定向，占61.4%；其中229个案例中错误选项概率至少为0.7。在七个数据集上，另外三个决策系统对初始正确决策的目标翻转率为64.9%至73.2%。

Conclusion: 当前决策模型存在明显脆弱性：简短、普通的上下文就能把正确选择变成高置信度错误选择。由于这些模型直接将语言转化为下游选择，其概率输出作为可靠决策接口的做法值得担忧。
...
### [12] [SemMSA: Latent Semantic-Aided Robust Multimodal Sentiment Analysis with Incomplete Data](https://arxiv.org/abs/2609.30238)
*Wenhao Li,Zhibin Wu,Chong Xiao,Qiangchang Wang*

Main category: cs.CL

TL;DR: 提出SemMSA，一种利用LLM构建潜在语义并通过对齐所有模态的无锚谱对齐框架，用于不完整多模态情感分析，在SIMS、MOSI、MOSEI上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有不完整多模态情感分析方法多通过重建模态特征或复杂融合机制补偿缺失信息，但由于缺乏对部分观测多模态证据的高层语义 grounding，仍存在虚假生成和噪声引导问题。

Method: SemMSA包含跨模态语义精炼（CSR）和跨模态谱对齐（CSA）。CSR用适配器提取视觉与声学表示，在冻结LLM嵌入空间与语言形成统一多模态前缀，并通过token高效的潜在精炼过程迭代产生连续判别性语义状态，无需解码文本。CSA通过增强核Gram矩阵的主导谱成分，将精炼语义与所有模态同时对齐，不依赖预定义锚模态；实例级谱分离约束保持跨样本判别性并缓解表示坍缩。

Result: 在SIMS、MOSI和MOSEI基准上的大量实验表明，SemMSA取得了最先进性能。

Conclusion: SemMSA通过LLM辅助的潜在语义构建和无锚谱对齐，有效缓解了不完整多模态情感分析中的虚假生成与噪声引导问题，并提升了多模态情感分析性能。
...
TL;DR: 该论文研究社交媒体上的阴谋论话语检测，主张应将其视为一种社会情境中的意图推断任务，并提出一个可主动查询社会语境的智能体框架。


<details>
  <summary>Details</summary>
Motivation: 阴谋论话语并不总是通过明确主张或固定词汇表达；相同表层内容可能表达支持、合理关切、批评、讽刺或嘲弄。因此核心挑战不仅在于识别阴谋论相关主张，还在于推断说话者的意图，即话语的言外之力。

Method: 作者提出一个智能体框架，配备一组支持社会查询的工具，利用相关社会语境进行判断。在覆盖2018年末至2023年初、约占希伯来语公共推文80%–90%的独特数据集上进行验证，涵盖多个选举周期、新冠疫情及疫苗接种运动。并在人工标注的对抗性数据集上评估该框架，比较上下文感知流程、纯文本分类、非智能体但可访问相同上下文的模型等设置。

Result: 上下文感知的工作流程始终优于纯文本分类；智能体框架显著优于其他框架和设置，包括可访问相同上下文但没有主动工具使用的非智能体模型。论文还分析了结果、错误以及效率（token经济性）权衡。

Conclusion: 这些发现支持将阴谋论检测视为一种社会嵌入的解释任务：有效分类不仅取决于能否获得上下文，还取决于自适应推理，即智能体能否按个案使用工具，并只请求与当前推理步骤相关的证据。
...
TL;DR: 本文研究编码智能体能否自动合成可在实例间泛化的任务与运动规划（TAMP）程序，并在多个仿真环境上验证其效果。


<details>
  <summary>Details</summary>
Motivation: 广义TAMP虽能利用问题实例间的规律降低新实例的规划开销，但现有方法依赖大量TAMP专用工程，因此作者希望探究编码智能体能否自动化这一程序合成过程。

Method: 给定任务描述和模拟器访问权限，每个智能体在固定合成预算内自行决定如何与环境交互来开发程序；程序随后被冻结，并在未见实例上评估。实验使用 Claude Code (Opus 5) 与 Codex (GPT-5.6 Sol、GPT-6 Astra)，在 KinDER 和 PDDLStream 的28个仿真环境中测试，对象数量超出原基准；共评估980个程序，每个程序100个留出实例，合计98,000次评估回合。

Result: 三种智能体配置在16个有可用规划器的环境中平均成功率为56%–95%，均优于手工规划器（47%）、单次生成和基于LLM的广义规划基线；当对象数量增加时，智能体程序的成功率仍高于规划器，且平均每实例计算量低一个数量级。日志显示智能体利用交互来校准物理模型、测试边界情况并改进策略。

Conclusion: 编码智能体是广义TAMP的强基线，作者发布了全部代码及给智能体的完整提示词。
...
### [15] [RAPID: Robot Agentic Programming from Demonstrations](https://arxiv.org/abs/2609.30249)
*Yuyao Liu,Jiayuan Mao,David Hsu,Leslie Pack Kaelbling,Tomás Lozano-Pérez*

Main category: cs.RO

TL;DR: 提出RAPID，从单次视觉人类演示自动生成、验证并精炼机器人程序，在仿真与真实Franka机械臂上均表现出强泛化性能。


<details>
  <summary>Details</summary>
Motivation: 代码智能体在复杂编程问题上已取得巨大成功，希望将其潜力用于机器人系统，但需要从单次演示中自动生成可复用的机器人程序。

Method: 采用迭代式智能体代码精炼循环，自动从演示中推断可测试任务规范、动作原语和交互执行环境；使用以对象为中心的关系程序表示，将动作原语表达为轨迹优化程序以实现对象级运动效果，并通过关系约束在运行时组合，捕捉场景特定几何。

Result: 在仿真中评估了八项接触丰富的非抓取操作任务及LIBERO-Pro基准中的通用抓取任务，并在真实Franka机械臂上部署评估了全部八项非抓取任务；所有实验中均表现强劲，对物体位姿、形状、材质和环境具有泛化能力。

Conclusion: RAPID能够有效自动生成、验证和精炼机器人程序，实现从单次视觉演示到可泛化机器人操作的迁移。
