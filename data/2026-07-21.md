...
TL;DR: 本文识别并形式化分析了RLHF中的一种结构混淆——评估者状态漂移，即评估者情绪状态随时间变化对偏好标签的持续影响。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF偏好数据隐含假设标签仅反映输出质量，但未考虑评估者状态变化导致的结构性偏差。

Method: 提出评估者状态漂移、评估者状态混淆和相关评估者状态偏差的概念，定义可测量的生存级情感真实性指标，并推导五个可证伪预测。

Result: 形式化描述了状态漂移如何绕过聚合过程进入奖励信号，并设计了审计协议和试点研究计划。

Conclusion: 评估者状态漂移是RLHF偏好数据中一个合理且可测试的结构性偏差来源，需要通过审计框架加以检测和控制。
...
TL;DR: 提出一个开源MATLAB框架，通过紧凑型1D CNN模型（13.2k参数，75%准确率）和混合推理管道，实现软体社交伴侣中的情感触觉识别。


<details>
  <summary>Details</summary>
Motivation: 软体传感器的可变形性和多通道触觉传感使情感触觉的鲁棒解释复杂化，需要高效、可嵌入的计算方法。

Method: 构建含1326个标注手势序列的FAIR兼容数据集；系统探索468个CNN架构和超参数，选出最优的紧凑型膨胀1D CNN；采用启发式阈值过滤+CNN混合推理策略。

Result: 最优模型（13.2k参数）达75%测试准确率、85%留一受试者交叉验证准确率；量化部署需3.2 MMAC/窗，满足20Hz实时需求；混合管道正确区分社交触碰与负面互动。

Conclusion: 情感触觉识别在计算上可嵌入软体治疗伴侣，实现隐私保护；混合推理策略结合了启发式逻辑的可靠性与CNN的细微辨识能力。
...
### [3] [Some Large Language Models Exhibit Consistent Risk Attitudes](https://arxiv.org/abs/2607.16197)
*Bowen Sun,Rui Min,Yuxi Wang,Brian Odegaard,Qi Wang,Jing Du*

Main category: cs.AI

TL;DR: 本文提出跨领域框架衡量大语言模型（LLM）在不确定性下的风险态度，发现其具有稳定的风险敏感性和态度偏差。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统缺乏对感知风险如何转化为行动的量化评估。

Method: 设计空间导航、临床分诊和金融分配任务，通过回归模型提取信念到决策的映射，并量化风险敏感性和态度偏差。

Result: LLM表现出任务内一致性、跨领域稳定性，且风险态度分布比人类更集中。

Conclusion: 风险态度是LLM行为的稳定维度，为开放决策场景下的AI对齐提供基础。
...
### [4] [A Survey on GNN-based Link Prediction: Techniques, Applications, and Challenges](https://arxiv.org/abs/2607.16198)
*Chengcheng Sun,Yajie Song,Cheng Zhai,Jiayun Tian,Jia Yang,Xiaobin Rui,Jian Zhang,Zhixiao Wang,Philip S. Yu*

Main category: cs.AI

TL;DR: 从GNN架构和多样图结构角度，系统综述基于GNN的链接预测方法，提出分类法并探讨挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有综述缺乏专门针对GNN架构和图结构多样性的系统探讨。

Method: 提出新的分类法，从技术（GCN、GAE、GAT、GFormer）和应用（知识图谱、推荐系统）两个维度综述。

Result: 总结了各种GNN架构在链接预测中的优缺点及实际应用案例。

Conclusion: 该综述填补了研究空白，明确当前挑战并指出未来方向。
...
TL;DR: 提出PlanFlip框架，包含四种规划阶段提示注入攻击，揭示多智能体LLM系统的安全漏洞，并提出了两种检测方法。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统依赖Planner分解任务，但规划阶段易被注入攻击，导致下游任务连锁受控。

Method: 设计四种规划阶段注入攻击（目标替换、优先级反转、上下文污染、角色混淆），隐藏为工具输出；评估9个前沿LLM，提出GoalAnchorCheck和CrossAgentConsensus检测方法。

Result: GPT-5攻击成功率最高（0.68）；同质化管道存在盲点；推理增强模型（如DeepSeek-R1）完全抵抗注入；提出方法在15/16个单元格中优于基线。

Conclusion: 异构模型多样性是多智能体系统安全的前提，同质化无法防范规划阶段攻击。
...
### [6] [Deterministic Replay for AI Agent Systems](https://arxiv.org/abs/2607.16200)
*Rasheed Mudasiru*

Main category: cs.AI

TL;DR: 提出了一个用于AI Agent确定性重放的CLI框架agrepl，通过中间人代理拦截和序列化外部交互实现隔离重放，验证了重放保真度F=1.0并降低98.3%的延迟。


<details>
  <summary>Details</summary>
Motivation: 现有AI Agent系统因LLM采样方差、外部API状态变化等非确定性因素无法实现可复现执行，导致调试和观察困难。

Method: 采用中间人代理在传输层拦截所有外部交互，序列化为结构化执行轨迹，在零出站网络访问的隔离环境中重放；并形式化了请求键匹配函数K(s)和噪声感知差异算法。

Result: 在五个工作负载（250次重放实例）上验证重放保真度F=1.0，每步延迟中位数降低98.3%。

Conclusion: agrepl能够高效实现AI Agent的确定性重放，且以轻量级单静态二进制形式开源可用。
...
### [7] [Generative Ontology Induction: Domain-Agnostic Schema Discovery from Document Corpora Using Large Language Models](https://arxiv.org/abs/2607.16201)
*Sergei Sergienko*

Main category: cs.AI

TL;DR: 提出生成式本体归纳(GOI)框架，能从示例语料中自动导出结构化本体蓝图，并在四个对比本体上验证其结构覆盖率达95-100%。


<details>
  <summary>Details</summary>
Motivation: 现有自动化本体工程方法依赖预定义模式、局限于狭窄领域或输出非结构化结果，成为知识密集型AI系统的关键瓶颈。

Method: 基于生成式AI的领域无关框架，从示例语料中归纳出包含实体、维度、属性、关系和约束的生成式蓝图，并导出为带类型图（6节点类型、7边类型）的YAML/JSON格式；提出节点覆盖率指标评估输出中结构本体节点的比例。

Result: 在四个对比本体（软件服务发票、岗位描述、疼痛管理临床记录、专业服务合同）上，GOI生成的结构覆盖率达95-100%；而通用三字段模板从发票模式的97.8%降至其他领域的52.2%-78.3%。

Conclusion: GOI框架在结构覆盖上具有领域普遍性，且不依赖模型对文档类型的熟悉程度，优于固定模板方法。
...
### [8] [Democratizing AI with Small Language Models: Structured Benchmarking and Parameter-Efficient Fine-Tuning for Local Deployment](https://arxiv.org/abs/2607.16202)
*Daniel Cersosimo*

Main category: cs.AI

TL;DR: 本文通过构建1,085道多选题基准并采用LoRA微调，证明3B以下开源模型在结构化局部部署中具备实用专家能力。


<details>
  <summary>Details</summary>
Motivation: 探索在普通机构的硬件和治理约束下，是否可以通过可控评估和低成本微调实现AI民主化，而非追求前沿泛化能力。

Method: 构建16主题1,085道多选题基准，评估9个135M-3B开源模型；采用4位NF4量化与DoRA/LoRA适配器在NVIDIA L4级预算下进行参数高效微调。

Result: Qwen Coder 3B基线准确率75.67%最高；微调后其提升26.85个百分点，SmolLM2 1.7B提升25.92，证明低成本专业化有效性。

Conclusion: 通过基准构建、跨模型评估和低成本微调，部分子3B模型可作为结构化局部工作的可行专家。
...
### [9] [It Takes 8 Tokens: Weak-to-Strong Off-Policy RL via Auxiliary Branches](https://arxiv.org/abs/2607.16205)
*Dayu Wang,Jiaye Yang,Weikang Li,Jiahui Liang,Liwei Qian,Xin Pei,Jizhou Huang*

Main category: cs.AI

TL;DR: 针对强化学习推理中因样本语义冗余导致奖励对比不足的瓶颈，提出基于弱到强学习的W2SPO方法，通过弱模型注入短辅助段扩展探索支持，在数学推理上获得性能提升与3.55倍训练加速。


<details>
  <summary>Details</summary>
Motivation: 标准验证奖励强化学习中，目标模型采样存在语义冗余，陷入错误推理盆地导致奖励对比不足，限制了策略更新。

Method: 提出W2SPO离线强化学习方法，利用弱但高效的辅助模型向目标模型中间轨迹注入短段，目标模型从转移状态补全推理，仅对注入段基于最终验证奖励进行策略更新。

Result: 在4B规模模型数学推理基准上，W2SPO的Pass@1达64.2%，优于对比基线，且相比GRPO在相同采样预算下实现3.55倍训练加速。

Conclusion: 弱辅助分支可通过扩展局部探索支持来诱导更强推理策略，验证了弱到强学习范式的有效性。
...
### [10] [Masked Diffusion Language Models are Strong and Steerable Text-Based World Models for Agentic RL](https://arxiv.org/abs/2607.16204)
*Darshan Deshpande*

Main category: cs.AI

TL;DR: 本论文提出了一种基于掩码扩散语言模型（MDLM）的可控文本世界建模方法，通过双向锚点感知去噪生成状态轨迹，在多样化、接地性和泛化能力上超越自回归模型，并在零样本迁移中实现高达47%的绝对性能提升。


<details>
  <summary>Details</summary>
Motivation: 强化学习需要多样化训练环境，但手工构建的环境难以随模型性能提升而扩展，且稀疏奖励导致模式崩溃。世界模型有望按需生成多样性状态，但自回归模型存在单向偏差，无法基于全局依赖性（如工具模式、历史轮次）进行条件生成。

Method: （i）将文本世界建模形式化为可引导状态转移问题，分解为初始状态、任务上下文、工具模式、领域规则和引导指令；（ii）构建包含239,403条轨迹的数据集；（iii）对比自回归语言模型与掩码扩散语言模型（MDLM），后者通过双向锚点感知去噪提升连贯性和接地性；（iv）提出GRPO训练框架，含确定性状态检查。

Result: MDLM在连贯性、接地性和轨迹多样性上优于4倍参数规模的LLM，推断延迟相当。在三个领域外环境（ScienceWorld、ALFWorld、AppWorld）的零样本迁移实验中，基于三个不同骨干网络（1.2B-7B参数）取得最高47%的绝对性能提升。

Conclusion: 掩码扩散语言模型通过双向条件生成可有效解决文本世界建模中的全局依赖问题，显著提升强化学习训练环境的多样性和泛化能力。开源工作以推动该方向研究。
...
### [11] [PPO-HSC: An Exploratory Reinforcement Learning Framework Based on Wide-Area Policy Coverage Optimization](https://arxiv.org/abs/2607.16206)
*Yujie Shen,Haowen Chen*

Main category: cs.AI

TL;DR: 提出PPO-HSC框架，通过高阶采样覆盖奖励激励LLM探索多样化推理模式，缓解微调中的模式崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 标准RLVR方法导致LLM过度优化已知解决方案，牺牲探索能力，陷入模式崩溃。

Method: 在PPO中引入高阶采样覆盖奖励，维护动态轨迹库，对低相似度高有效性的推理模式给予奖励，并添加合理性约束。

Result: 在数学推理（GSM8K、SVAMP）和代码生成任务上，提升了解决方案多样性和状态空间覆盖率，同时保持或超过现有RL基线的准确性和语法完整性。

Conclusion: PPO-HSC有效平衡探索与利用，增强LLM的推理多样性而不牺牲性能。
...
### [12] [JUMP: Single-Pass Membership Inference on Fine-Tuned Diffusion Language Models](https://arxiv.org/abs/2607.16207)
*Yeachan Jun,Albert No*

Main category: cs.AI

TL;DR: 针对微调离散扩散语言模型的成员推理攻击，提出JUMP方法，通过联合不确定性引导掩码探测，改进攻击性能。


<details>
  <summary>Details</summary>
Motivation: 现有SAMA方法仅将任意顺序接口作为随机化方式，需要大量查询，未充分利用离散扩散模型的并行解码能力。

Method: 利用任意顺序解码选择低参考置信度位置，通过并行解码对所有选定位置进行一次联合掩码查询，计算裁剪的目标/参考重建差距统计量。

Result: 在微调LLaDA-8B-Base的六个MIMIR域上，JUMP将平均ROC-AUC从0.82提升至0.90，并显著改善低假阳性率下的检测能力。

Conclusion: JUMP有效利用离散扩散模型的独特特性，以单次通过目标模型和参考模型的方式实现更高效、更精准的成员推断攻击。
...
### [13] [ColGraphRAG: Late-Interaction Evidence Retrieval for Multimodal GraphRAG](https://arxiv.org/abs/2607.16208)
*Seonok Kim*

Main category: cs.AI

TL;DR: 本文提出用多向量MaxSim评分替代单向量双编码器，用于图链图像候选排序，在MultimodalQA上提升了检索和下游问答性能，尤其在视觉证据重要的问题上表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的跨模态问答中，单向量双编码器可能丢失图像细粒度结构信息，影响图链视觉证据的排名与推理准确性。

Method: 在保留原有图构建、文本/表格检索、结构提取和图链路推理的前提下，将图像候选排序算子替换为ColBERT/ColPali系列的多向量MaxSim晚期交互评分。

Result: 在MultimodalQA上，检索阶段图链图像候选的点估计提升，下游问答准确率提高，尤其在视觉证据重要的问题上改善显著，而在文本主导问题上表现不一。

Conclusion: 结果从机制层面支持图链视觉证据的包含，但需更广泛的验证和更细粒度的图级诊断作为未来工作。
...
### [14] [Shapley Context Pruning: A Cooperative Game Perspective for Context Reranking and Pruning](https://arxiv.org/abs/2607.16209)
*Yanqiao Chen,Dongsheng Hou,Yuhan Rui,Zhen Cao,Yepang Liu*

Main category: cs.AI

TL;DR: 提出Shapley Context Pruning (SCP)框架，基于合作博弈论实现上下文重排序与剪枝，兼顾精细与粗粒度表示，利用Deep Sets架构和蒙特卡洛采样保证效率与理论保证。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统上下文重排序方法多依赖启发式损失函数和归因经验，缺乏可解释统一框架。

Method: 将上下文建模为合作博弈，采用Deep Sets架构近似排列不变值函数，使用预训练语言模型嵌入句子，通过成对边际排序损失优化，蒙特卡洛采样训练与推理。

Result: 在支持句子召回、AIAH、长上下文QA和多跳推理任务中取得竞争性下游QA性能。

Conclusion: SCP框架提供了合作博弈视角下的可解释上下文重排序方法，具有理论误差界和样本复杂度保证。
...
### [15] [A Survey on the Verification of Reinforcement Learning Policies](https://arxiv.org/abs/2607.16210)
*Luca Marzari,Ezio Bartocci,Enrico Marchesini*

Main category: cs.AI

TL;DR: 该综述对强化学习策略验证方法进行了统一视角的分析，提出了一个三维分类法：验证范式（形式化 vs 概率性）、时间范围（单步 vs 多步）和保证强度，并明确了理论根基、隐含假设与未来方向。


<details>
  <summary>Details</summary>
Motivation: 强化学习在安全关键领域的应用日益广泛，但神经网络策略缺乏严格的行为保证，现有验证研究概念分散、缺乏统一框架，亟需系统性梳理。

Method: 通过文献综述构建三维分类法（验证范式、时间范围、保证强度），统一理论基础，并显式剖析隐含假设与局限性。

Result: 识别了不同验证方法之间的关联与差异，明确了现有工作的隐含假设与局限性，指出了新兴研究方向。

Conclusion: 强化学习策略验证领域需要更统一的理论框架和显式假设分析，以推动安全关键场景中的可靠部署。
...
### [16] [Accurate and Efficient Long-Term Memory for LLM Agents](https://arxiv.org/abs/2607.16211)
*Zicheng Zhao,Xinyang Guo,Luyao Lv,Menghan Wang,Ming Li,Shuaicheng Li*

Main category: cs.AI

TL;DR: 提出结构化、冲突感知的长期记忆框架MOSAIC，通过实体图存储、哈希加速检索和主动冲突检测提升准确率和效率


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理长期记忆系统存在存储无结构、检索延迟高、易积累矛盾的问题

Method: 实体类型图存储、哈希加速双路径检索、保存时主动冲突检测

Result: LoCoMo准确率89.35%（+27.21%），冲突检测率66%（基线14%），检索延迟0.58秒

Conclusion: MOSAIC在准确率、效率、冲突检测方面显著优于现有方法
...
### [17] [Symbolic Augmentation Closes a Canonical-Equivalence Blind Spot in Neural Fact-Checkers](https://arxiv.org/abs/2607.16212)
*Genpei Zhang*

Main category: cs.AI

TL;DR: 提出符号增强训练框架，解决大语言模型在科学文本摘要中数值与单位幻觉问题，在物理等价改写上准确率从36.5%提升至98.2%。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在总结科学文本时经常幻觉数字和单位，可能无声地颠倒科学主张，现有事实检测器无法有效检测此类错误。

Method: 构建五类数值错误分类体系和1500项基准数据集；采用ModernBERT微调作为基线；提出符号增强训练框架，反向运行符号验证器生成标签保持的增强数据。

Result: 微调模型宏F1达0.899，但物理等价改写准确率仅36.5%；符号增强后提升至98.2%，宏F1微升至0.902，且能迁移到外部基准（SciFact-Open从0.791提升至0.828）。

Conclusion: 训练时增强是符号组件与学习组件的最佳集成点，符号特征作为辅助输入和符号银标签存在负面效果。
...
### [18] [SelKV: Selective KV Cache Merging with Per-Token Merge-or-Drop and Attention Compensation](https://arxiv.org/abs/2607.16213)
*Soumia Bouyahiaoui,Manel Kara laouar,Aicha Boutorh,Mohamed Hadj Ameur*

Main category: cs.AI

TL;DR: 提出了一种无需训练的KV缓存压缩方法，通过软余弦门控和注意力比率补偿机制，在保留25%缓存时实现接近无损的生成质量，并在多文档QA任务中超越全缓存基线。


<details>
  <summary>Details</summary>
Motivation: LLMs的KV缓存随上下文长度线性增长，现有token合并方法存在退化表示和注意力凹陷问题。

Method: 双组件框架：1) 软余弦门控基于值向量相似性自适应合并决策；2) 注意力比率补偿利用预填充注意力统计量添加解码时对数偏差。

Result: 在LongBench 16个英文数据集上压缩至25%缓存时性能优异，GQA模型接近无损，复杂QA任务超越全缓存，100k tokens解码加速3.3倍。

Conclusion: 该方法有效缓解了KV缓存压缩中的语义失真和注意力失衡问题，在效率和生成质量间取得良好平衡。
...
### [19] [RAIL Guard: Closing the Evaluation-to-Remediation Gap in Responsible AI for LLM Agents](https://arxiv.org/abs/2607.16215)
*Sumit Verma,Pritam Prasun,Pritish Kumar*

Main category: cs.AI

TL;DR: 本文提出RAIL Guard闭环管道，通过评估-重写-再评估循环修复LLM输出，在8个维度上实现96.9%收敛率，比阻塞重试高出47.8%。


<details>
  <summary>Details</summary>
Motivation: 现有护栏系统作为二元分类器仅能阻止不安全内容，丢弃失败输出后需从头重试，效率低下。

Method: 构建包含评估、重写、再评估三个阶段的闭环管道，在8个可衡量维度上迭代修复输出。

Result: 闭环修复收敛率96.9% vs 阻塞重试49.1%；反馈驱动自修复在可修复维度达86.6%收敛率且无显著效用损失；工具调用前评估减少33%不安全执行且不影响任务完成。

Conclusion: 部分维度（如透明度、问责性、包容性）需架构级解决方案而非算法级修复，系统已开源。
...
### [20] [Generalist AI Control: Towards Multi-purpose Adaptive Algorithms](https://arxiv.org/abs/2607.16313)
*Klinsmann Agyei,Pouria Sarhadi*

Main category: cs.AI

TL;DR: 提出一种通用控制器，利用注意力机制和状态空间表示，单次训练即可控制不同阶数和动态的系统。


<details>
  <summary>Details</summary>
Motivation: 传统控制器无法跨系统迁移，需要针对不同系统单独设计。

Method: 引入动态状态空间表示、注意力掩盖、系统标签、多尺度时间处理和混合专家架构。

Result: 在25个多样化系统上，性能与专用LQI控制器相当，且能泛化到未见过的工况。

Conclusion: 在SISO系统中实现没有系统特定调优的通用控制策略。
...
### [21] [LaCache: Exact Caching and Precision-Adaptive Inference for Diffusion Large Language Models](https://arxiv.org/abs/2607.16339)
*Xingru Chen,Zelang Liang,Yongjia Ma,Jiqing Zhan,Shuling Yang,Lian Wen,Kun Zhan*

Main category: cs.AI

TL;DR: 提出了LaCache，一种通过无损缓存和混合精度加速扩散语言模型的训练无关框架，实现约1.3倍加速，结合现有方法可达40.2倍。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型在去噪步骤中重复计算整个序列，忽略了前缀和掩码后缀保持不变部分导致的算子级冗余。

Method: 采用无损状态记忆化，缓存EmbedCache、RoPECache和FACache三种中间结果；并对FFN层应用基于组FP8量化策略。

Result: LaCache单独实现约1.3倍端到端加速，与现有加速方法结合达到40.2倍，同时保持任务精度。

Conclusion: LaCache能有效消除扩散语言模型中的冗余计算，显著提升推理速度而无需重新训练。
...
### [22] [Interactive Task Alignment as a POMDP](https://arxiv.org/abs/2607.16412)
*Andy Dai,Zexue He,Zhenyu Zhang,Alex Pentland,Jiaxin Pei*

Main category: cs.AI

TL;DR: 论文研究了语言模型在模糊任务中的任务对齐能力，发现模型在明确任务时表现良好，但在处理不确定性时效果不佳，平均仅22-32%能正确推断用户意图，而人类可达48%。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估模型在完全指定任务上的执行能力，但实际用户任务常是模糊的，需要模型先推断用户意图，这一能力尚未被充分研究。

Method: 将指定任务转化为不完全信息交互，形式化为POMDP框架；通过用户模拟器进行后验验证，并结合人类用户研究对比。

Result: 模型在任务明确时表现良好，但在模糊条件下仅22-32%能恢复用户意图；人类可达48%；后训练（SFT和RL）能改善对齐，但仍落后于人类。

Conclusion: 当前模型缺乏通过交互解决不确定性的关键能力，无法实现可靠的自主代理。
...
### [23] [When to Plan: Learning to Select Between Reactive Control and Deliberative Planning](https://arxiv.org/abs/2607.16421)
*Adam Labiosa,Josiah P. Hanna*

Main category: cs.AI

TL;DR: 提出基于反应策略不确定性评分的元推理强化学习方法，使智能体在快速反应与慢速规划之间自适应切换。


<details>
  <summary>Details</summary>
Motivation: 人类具备在快速反应与慢速规划间切换的能力，但人工代理缺乏这种元推理学习机制。

Method: 使用强化学习训练元推理策略，通过反应策略的不确定性分数决定何时直接反应、何时进行决策时规划。

Result: 在运动规划与导航实验中成功学会分配计算资源，并随反应策略优化逐渐偏向完全反应控制。

Conclusion: 研究证明基于不确定性的元推理方法可有效实现计算资源自适应分配。
...
### [24] [Berkeley and Heiserman as an Unexhausted Architecture for Embodied Machine Intelligence](https://arxiv.org/abs/2607.16465)
*Christopher A. Tucker*

Main category: cs.AI

TL;DR: 埃德蒙·C·伯克利不仅将符号逻辑与计算机关联起来，更致力于将符号逻辑作为机器的实用设计语言，涵盖信息获取、记忆、响应和自适应行为，而戴维·L·海瑟曼将其程序扩展至适应性生物架构。


<details>
  <summary>Details</summary>
Motivation: 动机在于重新审视伯克利的贡献，指出其核心关注点不仅是连接逻辑与机器，而是展示符号逻辑作为设计语言，构建适应环境、具有记忆和控制的智能机器。

Method: 通过分析伯克利的主要著作《符号逻辑与智能机器》及其相关项目，以及海瑟曼的机器智能工作，从逻辑形式、电路、控制、状态和事件等操作层面解读智能行为。

Result: 伯克利和海瑟曼的工作共同构成了一种尚未被充分检验的架构方法，用于实现具身机器人认知，强调适应性和环境耦合。

Conclusion: 他们的研究并非历史残余，而是对基于记忆、信心和泛化的适应性生物架构的重要贡献，对现代机器人设计仍有启示。
...
### [25] [SEER: Supervised Learning to Control Energetic Reasoning](https://arxiv.org/abs/2607.16523)
*Sascha Van Cauwelaert,Michele Lombardi,Pierre Schaus*

Main category: cs.AI

TL;DR: 本文提出使用机器学习构建预言机，动态决定是否运行复杂传播器，以平衡约束传播的剪枝能力与计算时间.


<details>
  <summary>Details</summary>
Motivation: 约束传播虽能减少搜索空间，但更强剪枝需更多计算时间；最佳传播器因实例和搜索阶段而异，现有方法（静态选择或动态调整）各有局限.

Method: 通过机器学习训练预言机，结合活动启发式，针对具体约束决定是否运行复杂传播器（如能量推理）；设计上注重灵活性，易于嵌入现有求解器.

Result: 实验表明，针对能量推理传播器可获得高预测精度，提供了分类特征建议，并揭示了构建此类预言机的关键挑战.

Conclusion: 基于机器学习的预言机在动态传播器选择上具有可行性，为平衡效率与剪枝能力提供了新方向.
...
### [26] [Nonuniformity Principle in Human-AI Coworking](https://arxiv.org/abs/2607.16530)
*An Luo,Jie Ding*

Main category: cs.AI

TL;DR: 本文探讨了在生成式AI工作流中，如何在人类监督与AI效率之间取得平衡，提出了非均匀性原理来优化监督阶段的位置，并通过两种AI工作流进行了实证验证。


<details>
  <summary>Details</summary>
Motivation: 观察到在长AI工作流中，人类监督虽能提升满意度并减少返工，但受限于时间和资源，需研究如何最优地安排监督。

Method: 基于经验观察，形式化人类-AI协作中监督阶段放置问题，提出非均匀性原理（监督间隔非递减），并在文献综述撰写和网站构建中验证。

Result: 非均匀性原理有效指导监督阶段布局，提升用户满意度并减少不必要的重做和token消耗。

Conclusion: 在合理假设下，人类监督的最佳策略是沿工作流以非递减间隔放置监督点。
...
### [27] [From Modalities to Propositions: A Language-Centric Framework for Multimodal Intelligence](https://arxiv.org/abs/2607.16560)
...
TL;DR: 提出一种统一的多模态语言表示方法，将图像、视频或文本转换为由原子命题组成的全局语义码本，实现跨模态的可解释性与组合推理。


<details>
  <summary>Details</summary>
Motivation: 现有多模态表示缺乏统一的语义空间，难以同时实现可解释推理、跨模态理解和组合泛化。

Method: 定义全局语义码本，将各模态观测编码为共享的规范原子命题集（实体、动作、关系），构成可组合的语义包。

Result: 在自动驾驶和开放世界数据上验证，支持细粒度事实到高层概念的统一表示，具备跨模态检索与复杂组合理解能力。

Conclusion: 该框架通过原子命题码本实现了多模态数据的可解释统一表示，为组合推理和结构化数据管理提供了新范式。
...
### [28] [Exact Network Surgery: Functional Invariance and Gradient Plasticity in Reactive Computational Graphs](https://arxiv.org/abs/2607.16568)
*Abdallah Khemais*

Main category: cs.AI

TL;DR: 提出了精确网络手术方法，在不破坏现有功能的情况下向计算图中插入残差块，并通过理论证明和实验验证了其位精确性、可训练性和优化行为。


<details>
  <summary>Details</summary>
Motivation: 现有Net2Net等网络生长技术存在数值扰动或需要重建训练程序的问题，需要一种能在原位插入模块且不改变网络功能的精确方法。

Method: 形式化精确网络手术，包括恒等态射定理、结构局部性定理和逃离初始化命题；采用NeuroDSL反应式图引擎实现，并验证退化配置（零初始化输出投影加零门）是鞍点。

Result: 在1600个logit测试中实现位精确移植；门控在第一步优化后逃离零；退化配置在600步内梯度为零；手术时间与下游锥大小相关（r=0.9992），开销恒定约0.75ms；训练在进程重启后位一致恢复。

Conclusion: 精确网络手术能实现位精确的模块插入且保持可训练性，理论预测与实验高度一致，为网络架构扩展提供了可靠基础。
...
### [29] [FST.ai 2.5: Explainable and Uncertainty-Aware AI for Olympic and Para-Taekwondo Decision Support, Athlete Digital Twins, and Federation-Scale Analytics](https://arxiv.org/abs/2607.16597)
*Keivan Shariatmadar,Ahmad Osman,Ramin Rey*

Main category: cs.AI

TL;DR: 提出了一个面向奥运和残奥跆拳道的可解释、不确定性感知、安全统一的AI框架FST·ai 2.5，整合了运动员智能、比赛分析、联盟数据管理等功能，并通过原型部署验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 现有精英体育的AI解决方案碎片化，仅处理孤立任务（如表现分析或裁判支持），缺乏统一的生态系统。

Method: 构建了包含运动员智能、比赛分析、数字孪生、可解释性指标和自适应训练推荐的一体化框架，采用多源数据融合、不确定性感知和联盟治理机制。

Result: 原型部署展示了框架的可行性，实现了战术诊断、纵向监测、表现预测、个性化规划和联盟级基准测试。

Conclusion: 该框架在跆拳道领域验证有效，其方法论可推广至其他格斗运动和高性能体育场景，为可解释AI和可信决策支持提供了范例。
...
### [30] [Just A Rather Very Intelligent Spoken Agent](https://arxiv.org/abs/2607.16610)
...
### [31] [A Research Prototype for Closed-Loop Generative Design of Customized Foot Orthoses via Semantic-Physics Alignment](https://arxiv.org/abs/2607.16631)
*Rui Wang,Byungwon Min,Suxing Liu*

Main category: cs.AI

TL;DR: 本文提出TANS-FO模块化管线，通过文本对齐神经替代和图形神经网络替代，实现定制足矫形器的计算设计自动化，在标准化负载下达到高精度和低压应力，并初步观察显示短期舒适度改善。


<details>
  <summary>Details</summary>
Motivation: 转换非结构化临床处方到患者特定足矫形器受限于语义-物理错位：高级临床意图无法确定性地映射到3D几何参数，且现有设计流程依赖人工专业知识且无即时生物力学验证。

Method: 文本对齐神经替代使用交叉注意力将临床文本嵌入投影到连续晶格密度场，图形神经网络替代实时预测足底应力以替代有限元分析，基于PicoFoot-5K数据库。

Result: 图形神经网络替代与Abaqus参考求解器一致（R^2=0.94），完整管线在几分钟内合成制造就绪晶格鞋垫；在Male 18-40队列中，峰值压力降低34.7%，拟合误差0.42毫米；初步观察（n=12）显示VAS疼痛评分从6.4降至2.1。

Conclusion: TANS-FO作为研究原型实现了从文本到定制足矫形器的自动化设计，但初步观察数据仅为探索性可行证据，非临床有效性证据。
...
### [32] [TopoTuner: Topological Finetuning of Large Language Models](https://arxiv.org/abs/2607.16637)
*Abdulkadir Erol,Yash Mahajan,Vepaul Hariprashad,Baha Rababah,Santu Karmaker,Cuneyt G. Akcora,Mubarak Shah*

Main category: cs.AI

TL;DR: TopoTuner是一种基于拓扑引导的微调框架，通过选择性冻结注意力投影矩阵，仅训练1-2%的参数，在多个大模型上达到或超越全微调性能，并比LoRA表现更好。


<details>
  <summary>Details</summary>
Motivation: 全微调更新所有参数成本高，LoRA虽减少参数量但未直接解答哪些预训练组件应被训练或冻结，因此需要一种可复用的选择性冻结方法。

Method: 将每个投影矩阵视为点云，利用持续同调的Wasserstein距离测量拓扑变化，从源数据集学习可复用的冻结配置，并迁移到其他任务。

Result: 在LLaMA-3.1-8B、Mistral-7B-v0.3和Qwen3-8B-Base上，TopoTuner在7/9设置中优于LoRA，训练时间比全微调减少20.4%，比LoRA减少5.5%。

Conclusion: TopoTuner开辟了可复用冻结配置的新方向，使在一个数据集上学习的微调行为可跨任务共享。
...
### [33] [Diversity-Oriented Fine-Tuning for Uncertainty-Based Hallucination Detection](https://arxiv.org/abs/2607.16643)
*Qiuyuan Li,Hongliang Dai,Piji Li*

Main category: cs.AI

TL;DR: 提出多样性微调策略（基于SFT和DPO）提升模型生成多样性，从而使基于语义熵的幻觉检测更有效，效果优于或持平现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测多在推理阶段进行，不修改模型；但模型在多次生成中常产生相同的错误答案，导致基于语义熵的检测失效。因此，需要从训练阶段增强模型输出的多样性，以提高幻觉的可检测性。

Method: 采用两种微调策略：1）基于监督微调（SFT）增强输出多样性；2）基于直接偏好优化（DPO）优化生成分布。核心目标是降低模型对幻觉答案的语义熵一致性。

Result: 微调后模型对幻觉答案的语义熵降低，幻觉检测效果改善；在实验中，性能达到或超越当前最先进方法。

Conclusion: 通过多样性导向的微调能有效提升模型输出的可变性，从而增强语义熵检测方法的可靠性，是一种可行的模型级干预方案。
...
### [34] [DS@GT ARC at eRisk 2026: Hybrid Multi-Agent LLM System with Structured Algorithmic Guidance for Conversational Depression Screening](https://arxiv.org/abs/2607.16712)
*Victor Gong,David Guecha*

Main category: cs.AI

TL;DR: 本文针对eRisk 2026任务1，提出了一种基于混合架构的对话式抑郁筛查系统，使用弱开源模型在算法监督下达到与强商业模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 降低对话式心理健康筛查中对昂贵商业模型的依赖，验证开源模型通过算法增强能否在敏感对话场景中匹敌专有模型。

Method: 采用多智能体架构，包括协调层、评估层和对话层；引入对话树、可靠性加权共识聚合和聚类插补三种算法组件，以增强弱推理能力的开源模型。

Result: 混合版本（Run 3）ADODL达0.9063，排名第三；团队整体排名第二；成本仅为商业基线版本的1/4。

Conclusion: 通过充分的算法监督，弱开源模型可以在对话式筛查中与强商业模型竞争，验证了核心假设。
...
### [35] [Tractable Query Answering under Epistemic Confidentiality Policies in DL Ontologies (extended version)](https://arxiv.org/abs/2607.16715)
*Lorenzo Marconi,Daniela Rieti,Riccardo ROsati*

Main category: cs.AI

TL;DR: 本研究在描述逻辑本体论中探讨受控查询评估，提出基于最小策略违规的新语义，该语义在保护机密性的同时具有多项式时间复杂度和不可区分性。


<details>
  <summary>Details</summary>
Motivation: 现有控制查询评估语义在计算上难以处理（尤其在DL-Liteᵣ中）或缺乏机密性保护性质（如IGA语义的不可区分性缺失）。

Method: 提出最小策略违规（MPV）语义，作为GA-和IGA-蕴涵的合理近似，并通过DL-Liteᵣ本体上的布尔合取查询回答复杂度分析及软件实现验证。

Result: MPV语义满足不可区分性；在DL-Liteᵣ中，查询蕴涵可在多项式时间（数据复杂度）内判定；实现测试显示可行性。

Conclusion: MPV语义在保证机密性的同时提供了计算上可处理的控制查询评估方法，适用于DL-Liteᵣ本体和基于依赖的策略。
...
TL;DR: 提出RECON基准测试，评估大语言模型在长上下文中的组合推理能力，涵盖六个记忆密集型任务，当前最强系统仅达22.4%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅测试事实检索或变更检测，但无法评估变更后对下游结论的连锁影响和替代时间线推理。

Method: 设计RECON基准，包含24个案例文件（5万至10万词），跨刑事、医疗、金融三领域，测试多跳证据链重建、级联失效传播等六项任务。

Result: 非Oracle系统最高准确率仅22.4%，检索和推理均为主要挑战。

Conclusion: 当前架构在长上下文组合推理中存在显著局限，亟需改进。
...
### [37] [Constraint-Anchored Reasoning Traces](https://arxiv.org/abs/2607.16727)
*Zehua Cheng,Wei Dai,Jiahao Sun*

Main category: cs.AI

TL;DR: 提出约束锚定推理轨迹（CART）框架，通过神经符号方法减少多模态大语言模型在链式推理中的错误雪球效应，在多个基准测试上将雪球率从0.65降至0.14。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在链式推理中一旦出现早期错误会导致后续推理全面失败（雪球率65%），而现有缓解方法要么缺乏符号基础，要么错误检测滞后，或牺牲自然语言推理的灵活性。

Method: CART框架：1）训练MLLM在自然语言推理步骤中穿插轻量级符号约束断言；2）双管约束传播模块（神经接地头+布尔约束传播）持续验证约束一致性；3）矛盾时回溯控制器回滚至最近一致检查点；4）变频率发射机制自适应控制锚点密度。

Result: 雪球率从0.65降至0.14，GQA准确率提升4.6个百分点，POPE-all达89.1 F1，推理开销最多增加18%。

Conclusion: CART通过神经符号混合方法有效抑制了多模态链式推理中的错误传播，在保持推理灵活性的同时显著提升鲁棒性。
...
### [38] [Supporting Autonomous Process Execution within a Multi-Perspective Constraint Frame via Numeric Planning](https://arxiv.org/abs/2607.16738)
*Paul Wittlinger,Giacomo Acitelli,Anti Alman,Fabrizio Maria Maggi,Andrea Marrella*

Main category: cs.AI

TL;DR: 提出了一种支持多视角约束（数据感知和时间条件）的假设分析工具，用于在ABPMS中推荐合规且最优的过程延续方案。


<details>
  <summary>Details</summary>
Motivation: 现有框架自治研究主要关注控制流约束，缺乏对数据和时间等多视角约束的支持。

Method: 通过扩展过程框架加入数据与时间约束，结合部分执行状态进行优化推荐。

Result: 实证评估证明了技术的可扩展性和有效性。

Conclusion: 该工具有望支持ABPMS中自主且约束感知的决策。
...
### [39] [RELIC: Revealed Principles for Learning Interpretable Composable Skills in Multi-Agent Planning](https://arxiv.org/abs/2607.16745)
*Nguyen Viet Tuan Kiet,Bui Dinh Pham,Duong Quoc Chinh,Dao Van Tung,Tran Cong Dao,Huynh Thi Thanh Binh*

Main category: cs.AI

TL;DR: RELIC提出了一个通过揭示原则学习可解释和可组合技能的多智能体规划框架，在保护隐私的同时实现异构智能体间的协调。


<details>
  <summary>Details</summary>
Motivation: 现有研究依赖集中优化、共享策略或共同技能表示，难以适应隐私受限的多智能体协作场景。

Method: 每个智能体通过私有LLM引导搜索优化自身的程序化技能，由可信协调器仅基于团队性能评估更新，并将成功行为抽象为可移植原则，供其他智能体在自身接口中实例化。

Result: 实现了协调与实施分离，支持异构技能签名下的跨智能体迁移。
...
### [40] [FUSAR-R1: A Large-Scale Reasoning Model for Intelligent Interpretation of SAR Images](https://arxiv.org/abs/2607.16819)
*Yi Yang,Xiaokun Zhang,Yuxuan Li,Ruyi Zhang,Xinpeng Zhou,Haipeng Wang*

Main category: cs.AI

TL;DR: 提出FUSAR-R1模型，通过链式推理数据和强化学习提升SAR图像智能解读的可靠性与自校正能力。


<details>
  <summary>Details</summary>
Motivation: 现有SAR视觉语言模型缺乏人类专家的逐步分析、逻辑判断和自校正能力，难以应对复杂场景下的可靠智能解读。

Method: 构建显式的链式思维推理数据引导指令学习，引入强化学习策略根据推理结果优化输出。

Result: 在SAR目标检测、计数分类和土地覆被识别等任务中一致优于现有多模态大模型。

Conclusion: FUSAR-R1通过模拟专家推理和自校正机制，有效提升了SAR图像智能解读的可靠性。
...
### [41] [From Overload to Insights: How AI Agents Can Support Scientists in Analyzing Complex Data](https://arxiv.org/abs/2607.16845)
*Tim Fuchs,Luca Gelisio,Steffen Hauf,Walid Maalej*

Main category: cs.AI

TL;DR: 设计和评估了一个与欧洲XFEL高性能计算环境集成的代理AI系统，以解决科学数据分析中的知识检索和代码生成挑战。


<details>
  <summary>Details</summary>
Motivation: 科学家在分析欧洲XFEL产生的大型复杂数据集时，需要结合领域专业知识与分散在文档、工具和支持渠道中的设施及软件特定知识，过程具有挑战性。

Method: 采用设计科学研究方法，包括快速文献综述、对16个AI工具的系统评估、多次访谈、焦点小组以及专家用户研究，开发并评估了两个原型。

Result: 确定了科学数据分析中的关键知识挑战，推导出支持知识检索和源代码生成的AI代理需求，并提出了适应不断变化AI工具环境的专业系统设计建议。

Conclusion: 这些发现为高度专业化科学环境中开发可维护的AI支持提供了指导。
...
### [42] [AgentBrew: Lifelong Knowledge Brewing from Strong Teachers to Weak LLM Agents](https://arxiv.org/abs/2607.16851)
*Yangqin Jiang,Chao Huang*

Main category: cs.AI

TL;DR: 提出了一种通过外部记忆蒸馏交互经验的训练方法，无需权重更新或教师协助，即可提升学生代理性能。


<details>
  <summary>Details</summary>
Motivation: 实际部署中需使用轻量学生模型，但训练时可利用更强教师，现有方法依赖权重更新或教师实时参与。

Method: AgentBrew框架包含两个模块：失败触发教师Ralph Loop将失败转化为环境验证笔记；学生感知合成调整教师知识至学生可执行粒度。

Result: 在编码、数学和工具使用任务中，该方法显著提升学生代理能力，且无需额外监督或教师在线访问。

Conclusion: 非对称、免训练的蒸馏范式可高效生成可部署的LLM代理，克服稀疏反馈与弱学生执行能力差距。
...
### [43] [Beyond Semantic Equivalence: Logical Graphs for LLM Uncertainty Quantification](https://arxiv.org/abs/2607.16868)
*Yanni Dong,Minghua Liu,Meiling Zhu,Xiaowei Huang,Lijun Zhang*

Main category: cs.AI

TL;DR: 提出逻辑图不确定性（LGU）框架，通过建模答案间的蕴含与不相容关系改进大语言模型的不确定性估计，在多个QA基准上超越语义熵方法。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性度量（如语义熵）忽略答案间的逻辑关系，导致对兼容性输出过度估计不确定性，错误标记幻觉。

Method: 构建逻辑图，沿蕴含链聚合概率质量，计算逻辑最大假设上的熵，并惩罚假设间的不相容性。

Result: LGU在多个问答基准上持续提升不确定性估计，AUROC最高提升7.1%，AUARC最高提升3.5%。

Conclusion: 显式建模逻辑关系能有效改善LLM输出可靠性评估，减少对兼容性输出的误判。
...
### [44] [Environment-free Synthetic Data Generation for API-Calling Agents](https://arxiv.org/abs/2607.16900)
*Seanie Lee,Sanjoy Chowdhury,Chao Jiang,Cheng-Yu Hsieh,Ting-Yao Hu,Alexander T Toshev,Oncel Tuzel,Raviteja Vemulapalli*

Main category: cs.AI

TL;DR: 提出一种无环境的合成数据生成方法，利用LLM作为数字世界模型，通过生成模拟轨迹来训练API调用智能体。


<details>
  <summary>Details</summary>
Motivation: 收集大规模高质量轨迹数据通常需要完整实现的环境，包括可执行API和预填充数据库，这成为可扩展性的主要瓶颈。

Method: 使用LLM生成多样化任务，教师智能体迭代求解，同时LLM模拟器生成一致的API响应，最后LLM裁判过滤轨迹以保证质量。

Result: 在AppWorld和OfficeBench基准测试中，微调模型在信息检索和状态更改任务上均取得显著性能提升。

Conclusion: 无需执行环境即可生成有效监督，基于LLM的API模拟为训练跨生态的智能体提供了实用且可扩展的方案。
...
### [45] [Lomekwi: Resource-Bounded Tool Discovery in LLM Agents](https://arxiv.org/abs/2607.16961)
*Roshan Klein-Seetharaman,Daniel Wang,Andrew Xu*

Main category: cs.AI

TL;DR: 该论文提出了工具发现的三个维度：好奇心、识别和效率，并分析了现有工具使用基准的局限性，发现识别能力与模型大小呈反比。


<details>
  <summary>Details</summary>
Motivation: 现有工具使用基准仅报告复杂多步任务的单一成功率，受认知科学启发，作者区分工具使用与工具发现，旨在更细致地评估模型能力。

Method: 将工具发现分解为好奇心、识别和效率三部分，应用于Voyager等任务，并通过组合游戏和模拟真实任务的实验进行验证。

Result: 发现识别能力与模型大小呈反比例关系，并在不同环境中观察到类似的反向缩放现象。

Conclusion: 工具发现分解有助于理解模型能力，并指出识别能力的反向缩放需要进一步研究。
...
### [46] [Training Continuous Chain of Thought Models: A Tale of Two Regimes](https://arxiv.org/abs/2607.16972)
*Varun Yerram,He He,Eunsol Choi*

Main category: cs.AI

TL;DR: 提出C-MTP，一种直接监督的连续思维链方法，使用嵌入平均值压缩推理痕迹，在短链任务上表现良好，但在长推理任务上性能下降约65%。


<details>
  <summary>Details</summary>
Motivation: 现有连续CoT方法依赖间接监督且训练缓慢，需探索更简单快速的直接监督方法。

Method: 将每个潜在表示建模为CoT轨迹中标记嵌入的平均值进行直接监督训练。

Result: 优于先前的直接监督方法，与间接监督方法竞争性相当；但在长推理痕迹任务上性能显著下降。

Conclusion: 当前连续CoT方法在复杂长推理任务中存在局限性，需进一步改进。
...
### [47] [Expected Free Energy as Belief-Dependent Utility for rho-POMDPs](https://arxiv.org/abs/2607.16981)
*Patrick Cooper,Alvaro Velasquez*

Main category: cs.AI

TL;DR: Active inference中的期望自由能最小化等价于ρ-POMDP，且探索权重固定为w=1，无需手动调参。


<details>
  <summary>Details</summary>
Motivation: 标准POMDP通过奖励间接评估信息价值，而ρ-POMDP虽直接奖励不确定性减少，但需要手动调整ρ和权重。

Method: 证明EFE最小化等价于ρ-POMDP（信息增益效用），且探索权重固定为w=1，基于变分界将实用与认知价值统一为nats；扩展到因子化观测POMDP。

Result: 在老虎问题、RockSample和结构检测（超65，000状态）中，未调参的EFE匹配或超越奖励导向规划，避免过度探索，位于帕累托最优前沿。

Conclusion: EFE提供即插即用的探索目标，适用于测试有成本且故障有代价的应用（如故障检测、医学筛查）。
...
### [48] [PriorProof: A Point-in-Time Measure of Technique Novelty for Formal Proofs](https://arxiv.org/abs/2607.16997)
*Neel Somani*

Main category: cs.AI

TL;DR: 提出了一种基于时间相对性的形式化数学证明非常规性度量方法PriorProof，通过从证明项中提取依赖足迹并计算其加权惊奇值，在Mathlib快照数据上实现了无监督的证明新颖性判别。


<details>
  <summary>Details</summary>
Motivation: 数学家对证明的判别（如解释性、简化性、非常规性）难以操作化，现有方法依赖人工标注或技术本体，缺乏可自动计算的度量标准。

Method: 从形式化证明的展开项中提取依赖子图，基于先前季度Mathlib快照构建检索条件化的分层平滑先验，计算该子图在该先验下的加权惊奇值。通过证明导出的对比对学习语句检索，无需人工标签。

Result: 在盲法拓扑学研究中，PriorProof对76个独立对（含12个规范对）的判别准确率为69.7%（规范对91.7%），与最先进语言模型（78.9%）无显著差异（McNemar p=0.210），但分数差距的端点校准趋势提供了可解释可靠性指标。

Conclusion: PriorProof不是替代专家或语言模型的方案，而是一种可分解、时间锚定的信号，其分数差距可作为可解释的可靠性指示器。
...
### [49] [Reward-Driven LLM Agent Workflows: Synthesizing POMDP Routing and Self-Correction for Autonomous Decision-Making](https://arxiv.org/abs/2607.17038)
*Amez Amanj Ali,Kuo-Kun Tseng*

Main category: cs.AI

TL;DR: 本文提出一种基于POMDP路由和自校正奖励模型的智能体工作流，在ALFWorld和WebShop任务中实现24.5%的成功率提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体存在长程规划、稀疏奖励和动态环境交互等关键技术挑战。

Method: 结合视觉、语言、生成、图、多模态、强化和智能体七种AI范式，采用POMDP路由机制与内部自校正奖励模型，集成PPO和值函数近似。

Result: 在ALFWorld和WebShop基准上，相比ReAct框架实现24.5%的绝对成功率提升，消融实验证实奖励批判模块有效抑制幻觉。

Conclusion: 该架构为复杂多步自主系统提供可扩展参考框架，桥接强化学习与图记忆理论基础。
...
### [50] [When LLMs Over-Answer: Measuring and Mitigating Quality Issues in LLM-Based Hardware Description Language Question Answering](https://arxiv.org/abs/2607.17063)
*Ziteng Hu,Jiachi Chen,Wenhao Lv,Huan Zhang,Yingjie Xia*

Main category: cs.AI

TL;DR: 该研究收集了6,246个硬件描述语言（HDL）问答数据集，发现LLM回答存在过度回答、冗余和啰嗦的问题，并提出一个多智能体框架以提高回答质量。


<details>
  <summary>Details</summary>
Motivation: 由于HDL硬件合成特性，LLM回答的低质量可能导致时序违规，但LLM与人类专家回答质量对比尚不明确。

Method: 收集Stack Overflow的HDL问答数据，构建分类体系；进行19名工程师的用户研究；提出多智能体框架并采用LLM-as-Judge和结构指标评估。

Result: LLM回答中65.7%存在冗余替代方案，69.1%为啰嗦填充，49.0%不完全对齐专家答案，但58.3%参与者偏好LLM可读性；框架使核心答案质量从3.71提升至4.67，非核心内容质量从3.72提升至4.23。

Conclusion: LLM在HDL问答中普遍存在过度回答问题，所提多智能体框架能有效提升答案质量。
...
### [51] [Otap:Structure-Aware Optimal Transport for Evaluating Planning and Execution in Agent Trajectories](https://arxiv.org/abs/2607.17082)
*Babak Barazandeh,Subhabrata Majumdar,George Michailidis*

Main category: cs.AI

TL;DR: 提出了一种基于最优传输的评估指标OTAP，通过将智能体执行轨迹与有效解图进行非平衡融合Gromov-Wasserstein距离比较，实现对规划过程的鲁棒评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法（成功标志或精确匹配）无法区分偶然成功与合理方案，且难以诊断失败原因或容忍有效但顺序不同的规划。

Method: 将轨迹评估转化为执行图与有效解图之间的非平衡融合Gromov-Wasserstein最优传输问题，利用属性依赖图计算伪度量。

Result: 在受控扰动和三个公开基准测试中，OTAP能在语义指标低于随机水平时有效区分有效与无效轨迹；当依赖图精确恢复时准确率最高，仅在图依赖启发式推断时略有下降。

Conclusion: OTAP提供了一种对依赖保持顺序不变、对冗余步骤具有有界敏感性的伪度量，为分析智能体规划过程提供了新的鲁棒评估框架。
...
### [52] [Bridging the Information Gap: Semantic Densification and Hindsight Distillation for Cold-Start Prediction](https://arxiv.org/abs/2607.17070)
*Hao Duong Le,Yifei Gao,Huan Li,Lun Jiang,Chen Bai,Ke Xing,Chen Zhang*

Main category: cs.AI

TL;DR: 提出SemRaD框架，通过结构化语义推理和事后蒸馏，解决电商平台新用户冷启动中LTV和CVR预测问题，在工业数据集上提升1.9% LTV和1.0% CVR。


<details>
  <summary>Details</summary>
Motivation: 现有LLM语义增强和学习特权信息方法分别存在结构化缺失和信息鸿沟问题，无法有效处理新用户冷启动。

Method: SemRaD框架包含结构化语义推理管道（发现-整理-审计流程生成稠密语义档案和事后蒸馏目标）和事后感知蒸馏网络（通过蒸馏专家处理用户异质性）。

Result: 工业数据集LTV提升1.9%（基尼系数），CVR提升1.0%（AUROC）；在线A/B测试LTV提升1.0%，CVR提升0.43%；仅用9%训练数据即匹配生产系统LTV并提升0.8% CVR。

Conclusion: SemRaD有效弥合特权信息差距，可显著提升新用户冷启动预测性能，具备工业部署价值。
...
### [53] [Is Progressive Disclosure All You Need for Long-Context Agents?](https://arxiv.org/abs/2607.17598)
*Yifeng He,Yinzhe Zhao,Jicheng Wang,Hao Chen*

Main category: cs.AI

TL;DR: 本文首次对长篇文档问答中的渐进式披露模式进行受控研究，对比了原始文档导航与多种Agent技能包设计。结果表明，在单书任务中，收益取决于Agent框架；在多书任务中，原始文档导航失效，单层渐进式披露表现更优，而深层路由无益甚至有害。


<details>
  <summary>Details</summary>
Motivation: 实践者迅速采用Agent技能包的渐进式披露模式处理书籍级理解任务，但缺乏系统性的实验证据支持该方法的有效性。

Method: 在InfiniteBench上，使用三种Agent框架和三种模型家族，对比原始文档导航、多种Agent技能包设计与经典混合检索器的性能。

Result: 单书任务中，收益因框架而异；多书任务中，原始文档导航性能崩溃，单层渐进式披露下降更慢并领先，双层路由无帮助或破坏准确性。

Conclusion: 渐进式披露提供的是上下文而非智能；在强Agent能定位正确段落时冗余，而在语料库过大无法通过阅读导航时至关重要。
...
### [54] [Fourier Geometric Wind Power Forecasting with Numerical Weather Prediction](https://arxiv.org/abs/2607.17095)
*Shiyuan Piao,Fan Zehui,Yang Liu,Hong Cheng,Juepeng Zheng,Jie Zhou,Fugee Tsung*

Main category: cs.AI

TL;DR: 提出了一种多模态框架，融合历史SCADA数据和网格化NWP预报，利用傅里叶神经算子进行全球卷积建模，提高了短期风电预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能有效整合天气预测与风机SCADA数据，导致预测不准确。

Method: 将输入显式分解为标量和向量特征，通过几何编码器提取旋转不变特征，并采用傅里叶神经算子建模长程时空关系。

Result: 在三个真实风电场数据上优于现有方法，验证了物理信息设计的有效性。

Conclusion: 该多模态框架通过结合异构数据和物理约束显著提升了短期风电预测性能。
...
### [55] [WorldCupArena: Fine-Grained Evaluation of Language Models and Deep-Research Agents on Football Forecasting](https://arxiv.org/abs/2607.18084)
*Zhaokai Wang,Tianlin Gui,Jiayuan Rao,Shangzhe Di,Yihong Tang,Dingli Liang*

Main category: cs.AI

TL;DR: 提出了一个动态基准WorldCupArena，用于评估语言模型和深度研究代理在2026年世界杯前预测比赛结果和细节的能力。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型主要依赖历史数据，缺乏对动态信息和详细预测的评估，需要更全面的基准。

Method: 在每个比赛前，模型接收共同证据包或自主搜索信息，预测结果、比分、球员事件、统计数据和比赛结果；赛后与实际结果比较，使用结果准确率、精确比分准确率和比分评分等指标评估。

Result: 在104场比赛和13个系统中，结果准确率相近的模型在详细预测上差异更明显；最佳系统在结果和精确比分准确率上优于博彩市场和人类粉丝基线，但在比分评分上优势更显著。

Conclusion: WorldCupArena提供了一个动态、可重复使用的基准，能够有效区分模型的详细预测能力，且可通过添加新赛程持续评估未来模型，避免已知结果偏差。
...
### [56] [Evidence Interfaces Shape How Retrieval-Augmented Readers Use Support](https://arxiv.org/abs/2607.17108)
*Junchi Liao,Jiawen Deng,Fuji Ren*

Main category: cs.AI

TL;DR: 论文指出多跳RAG评估中top-k答案分数隐藏了两种失败：检索窗口丢失支持链或包含读者难用的形式。通过三个带支持标注的多跳QA基准测试，区分了支持可用性失败和读者接口效应。


<details>
  <summary>Details</summary>
Motivation: 现有top-k答案分数无法区分检索窗口是否丢失支持链或证据形式不适合读者，导致评估不准确。

Method: 使用三个支持标注的多跳QA基准，比较匹配的读者模型经过原始上下文、检索窗口和金支持诊断渲染训练后的表现，并通过支持移除检查分析。

Result: 当完整支持链保留时，短排名窗口可匹配或超越原始上下文；否则缺失支持导致损失。金支持优先提升读者性能，支持监督排序器提高覆盖率并恢复质量。

Conclusion: 在支持标注评估中，top-k答案分数应与完整支持覆盖率一同报告。
...
### [57] [A Diagnostic Framework for AI Agent Behavior](https://arxiv.org/abs/2607.17149)
*Xichen Zhang,Yingjie Zhang,Tianshu Sun*

Main category: cs.AI

TL;DR: 提出了一个诊断AI Agent行为的框架——层级归因（Layer Attribution），从计算层与行为调制层分析行为来源。


<details>
  <summary>Details</summary>
Motivation: AI Agent在行为科学涉及的系统中作用日益增强，但现有方法缺乏对行为根源（如表征、角色、目标、制度等）的系统诊断。

Method: 构建两层框架：基础计算层（架构、记忆、感知、注意、表征）决定行为可能性，行为调制层（身份、资源、目标、社交、制度、治理）塑造行为表达。

Result: 阐述了三个结论：表面效度是模型-任务-层级关系、人机差异提供诊断证据、治理需先进行来源归因再干预。

Conclusion: 应将AI Agent视为行为行动者，先确定行为起源，再决定如何解释、验证或治理。
...
### [58] [Logical Judgments Under Pressure: Diagnosing Syllogistic Stability with Learned Soft Prefixes](https://arxiv.org/abs/2607.18228)
*Brian K Chen*

Main category: cs.AI

TL;DR: 通过向逻辑推理基准添加软前缀，研究发现学习到的上下文可以覆盖正确判断，暴露模型逻辑稳定性的局限；在多个模型中，成功软前缀的主要效果是广泛的答案偏好，而非固定符号或操作转移。


<details>
  <summary>Details</summary>
Motivation: 为了测试正确逻辑判断如何响应习得上下文，通过固定模型并添加软前缀来观察行为变化并识别其稳定性和限制。

Method: 在精确标记的三段论推理基准上添加软前缀（连续向量），保持模型固定，通过控制逻辑形式和界面变化，使用Qwen3.6-35B-A3B MoE、Qwen3-8B和Gemma 4 31B模型进行实验，并分析翻转率、效果泛化及诊断测试。

Result: 软前缀在16个模型-方向-分割比较中优于随机对照，翻转率高出37-99个百分点；Qwen3.6 MoE在不同措辞和提示下翻转率为72%-90%，Gemma保持54%-56%翻转；主要行为是广泛的答案偏好，但模型间差异明显。

Conclusion: 成功软前缀的主要行为效果是答案偏好，而非固定符号或逻辑操作；模型在逻辑稳定性上存在显著差异，Gemma更接近简单分数模型预测。
...
### [59] [Is Your Model Thinking or Just Stagnating? PUMA: Diagnosing Reasoning Pathology via Phase-Momentum Alignment](https://arxiv.org/abs/2607.17188)
*Cheng Yan,Guangyang Ye,Wuyang Zhang,Fan Xu,Zhijun Fan,Xiang Xia,Yanyong Zhang*

Main category: cs.AI

TL;DR: 提出PUMA框架，通过相位-动量对齐假设和认知能量模型，在不训练的情况下优化大型推理模型（LRMs）的测试时效率，减少过度推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法（信息论和潜在表示分析）存在欺骗性收敛或缺乏实时敏感性，无法有效区分主动探索与被动停滞，导致计算浪费。

Method: 基于相位-动量对齐假设，构建认知能量模型（几何认知努力和熵认知不确定性），PUMA采用分层诊断架构：轻量级相位监控+事件触发几何分析，实现自适应截断或纠正。

Result: 在1.5B-32B的LRMs上，PUMA在多个基准测试中优于现有方法，实现精度与效率的更好平衡，并具有跨域泛化能力。

Conclusion: PUMA有效缓解LRM的过度推理问题，通过实时动态干预提升测试时效率，无需额外训练。
...
### [60] [Toward Anthropomorphic Dialogue: A Closed-Loop Framework for Human-Like Chat Generation, Evaluation, and Preference Alignment](https://arxiv.org/abs/2607.17191)
*Wentao Liu,Siyu Song,Xi Chen,Youjia Li,Xiaokun Wang,Min Ji,Ji Wang*

Main category: cs.AI

TL;DR: 提出AnthroDial框架，通过联合系统架构、可执行评估和诊断对齐，显著提升类人对话的角色一致性、记忆和长期连贯性，最佳模型严格准确率达39%。


<details>
  <summary>Details</summary>
Motivation: 现有对话系统缺乏对角色、关系、记忆、时间等多维类人属性的协同建模，导致对话不够自然。

Method: 包含角色条件调度运行时（角色卡、场景卡、长期记忆、虚拟时间）、多维度可执行基准（L0验证、5个轮次维度和5个对话级维度）、后训练流水线（SFT+GRPO，基于认知诊断和ZPD奖励，卡尔曼滤波估计能力缺陷并动态调整权重）。

Result: 在55角色、50场景的基准上，最强非训练基线严格ACC为32%，Qwen3.6-27B-SFT+RL达39%准确率和98.5总分；无思考的9B模型通过SFT和RL从0%提升至13%和18.37%。

Conclusion: 共享行为维度的生成、评估和奖励设计能有效提升类人对话质量。
...
### [61] [A Systematic Evaluation of Trajectory Data Curation for LoRA Fine-Tuning of Code Agents](https://arxiv.org/abs/2607.17205)
*Yunze Han*

Main category: cs.AI

TL;DR: 研究了在代码代理监督微调中轨迹数据筛选对模型性能的影响，提出了效率和风格双轴质量评分框架，并在不同数据规模下验证了质量与数量的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖专家轨迹数据进行微调，但轨迹质量和数量如何共同影响模型性能尚不明确。

Method: 使用LoRA微调Qwen2.5-Coder-7B-Instruct，基于SWE-trajectory数据集，通过16个控制实验评估两轴质量评分（效率与风格），以交叉熵损失为主指标，并用首动作生成进行代理验证。

Result: 数据规模较小时（500-1000），翻倍数据量使交叉熵损失降低约12.7%，而质量筛选差异不显著；在2000条数据时，高质量与随机筛选差距达3.6%，其中错误重试率是关键子维度。

Conclusion: 轨迹质量筛选是有效但受数据规模影响的策略，所提评估协议适用于端到端解决率不可行的情况。
...
### [62] [LenGuard-GPC: Length Guarding with Guided-Prompt Consistency for Spatial Reasoning Reinforce Learning](https://arxiv.org/abs/2607.17243)
*Xingjian Tao,Yiwei Wang,Yujun Cai,Jing Tang*

Main category: cs.AI

TL;DR: 提出LenGuard-GPC框架，利用KL散度作为密集奖励信号并配合分段长度奖励，在多视角空间推理中提升准确率并缩短响应长度。


<details>
  <summary>Details</summary>
Motivation: 标准GRPO奖励仅提供稀疏的结果反馈，无法指示推理轨迹错误位置或控制长度，导致chain-of-thought推理冗长且不准确。

Method: 对每个采样轨迹，计算标准提示与引导提示下token级预测分布的KL散度作为密集奖励；引入分段长度奖励控制推理长度在合理范围。

Result: 在六个多视角空间推理基准上，LenGuard-GPC相比标准GRPO提升准确率同时减少平均响应长度。

Conclusion: 该框架有效解决了多视角空间推理中长上下文推理的准确性与长度控制问题。
...
### [63] [Coordinated Disentanglement with Iterative Mode Discovery Under Hidden Correlations](https://arxiv.org/abs/2607.17264)
*Rong Hu,Ling Chen*

Main category: cs.AI

TL;DR: 提出CoDID框架，通过联合模式发现与条件独立性约束实现解缠表示学习，动态架构适应模式数量变化，元优化协调机制缓解误差放大，在多种任务上达到最优。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽处理属性相关性，但未充分探索隐藏相关性（属性值下潜在模式与其他属性相关），需要保留模式信息并实现解缠。

Method: 联合模式发现与基于模式的条件独立性约束；采用动态架构适应模式数量迭代变化；通过元优化机制协调两个模块以缓解误差放大。

Result: 在多种任务上取得当前最优性能。

Conclusion: CoDID有效解决隐藏相关性下的解缠问题，动态架构与协调机制是性能提升的关键。
...
TL;DR: 本文提出了约束路径推理（CPR）框架，通过阶段性承诺与验证来优化LLM推理管道，在1,180个QCQP问题中残余分诊恢复63.0%的可行产额且仅用17.7%的尝试，证明受信约束能有效平衡成本与收益。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理管道中，中间阶段的承诺通常增加额外成本，何时值得投入缺乏明确标准。CPR旨在量化承诺的价值，通过受信约束与可修正软提议的分离，在搜索成本与输出质量间寻求最优平衡。

Method: CPR结合源感知路径假设与阶段级记账：搜索生成暂态状态，受信不变量形成硬约束，其余提议保持软性可修正。形式化框架处理离散承诺与连续流，测量有效分支因子、终点集中度及可用输出成本。实验采用QCQP基准与退化多项式实例，通过固定LLM调用、配对条件测试及端点探针等方法验证。

Result: 在1,180个QCQP中，残余分诊以17.7%的尝试恢复63.0%的额外可行产额；固定LLM调用中形式化-确定性执行达90.0%可用产额（直接41.1%）；两动作回滚规则在120次配对调用中达90%可用产额（vs. 36.7%）；跨轨迹移植降低熵与可接受质量，自提议测试中25.0% vs. 8.3%可用产额。

Conclusion: 模型生成状态可作为假设来源，但须通过受信执行获得约束强度。CPR阶段性承诺在付出可控误差与执行成本时，能有效集中候选空间、暴露反馈信号，为LLM推理的代价敏感优化提供理论依据与实践验证。
...
### [65] [An Explicit World Model Based on Data-First Ontology: DaoQL Multimodal Storage Validation and Counterfactual Reasoning Evaluation](https://arxiv.org/abs/2607.17269)
*Zhanbo Li,Shifeng Wu,Xiangjin Meng,Wenjie Cai*

Main category: cs.AI

TL;DR: 提出数据优先本体，将确定性知识移至显式多模态数据库DaoQL，隐式模型缺乏原子语义保证，DaoQL在反事实实验中提升49个百分点。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在高精度领域存在幻觉、知识冻结、可解释性差和可修改性差的结构性风险。

Method: 将LLM作为推理和语言引擎，知识存储在DaoQL中，形式化显式世界模型并验证其组合反事实可分解性条件。

Result: DaoQL在嵌入式场景中报告图BFS 1.20 ms、HNSW 83.1 μs、Fluent混合查询105.8 μs；规则独立下反事实可分解性达94%，比GPT-4o高49个百分点。

Conclusion: 显式模型提供组合反事实可分解性的充分条件，隐式模型无类似保证；工程潜力需结合部署差异解释。
...
### [66] [Lossless but Not Free: An Empirical Anatomy of Speculative Decoding on Consumer Hardware](https://arxiv.org/abs/2607.17283)
*Param Chordiya*

Main category: cs.AI

TL;DR: 本文提出了一个从零开始的、设备无关的投机解码实现，并在消费级苹果硅芯片笔记本电脑上进行了实验验证，最佳配置在K=6时实现了1.61倍的加速，而部分配置因并行验证失效或延迟差距不足导致减速。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的单流自回归解码受限于内存带宽，令牌生成顺序执行且无法并行化，因此需要探索更高效的解码策略。

Method: 使用小型草稿模型自回归生成K个令牌，目标模型批量评分所有令牌，并通过拒绝采样保持分布一致性。

Result: 在五个草稿/目标配置上实验，最佳加速比1.61倍，接受率从69.7%降至37.8%；三种配置减速，归因于并行验证失败或延迟差距不足。

Conclusion: 投机解码仅在验证真正批并行且草稿/目标延迟差距显著时有效，失败案例揭示了关键限制因素。
...
### [67] [Learning-Driven Adaptive Audit Scheduling: A Sequential Decision Approach to Off-Chain Data Integrity](https://arxiv.org/abs/2607.17305)
*Changting Lin,Fan Li,Weihang Yu,Keyang He,Mingyuan Yan,Yourong Chen,Meng Han*

Main category: cs.AI

TL;DR: 提出DRQN-CMDP方法，在部分可观测的约束MDP下，通过GRU和拉格朗日对偶实现安全且高效的链下数据审计，平衡了Gas消耗、漏检率和检测延迟。


<details>
  <summary>Details</summary>
Motivation: 链下数据存储存在恶意节点腐败风险，现有审计方法在成本、安全性和检测效率间难以取得良好平衡。

Method: 将问题建模为约束MDP下的POMDP，使用带GRU的深度循环Q网络维护信念状态，结合拉格朗日对偶自动调节漏检率惩罚，并采用配对无关的同态MAC实现O(1)链上验证成本。

Result: 相比12种基线方法，DRQN-CMDP在Gas消耗（降低83%）、漏检率（7.5%）和检测延迟上取得最优权衡。

Conclusion: 所提方法在链下数据审计中能同时优化安全性、效率和成本，优于现有方法。
...
### [68] [DeeperRadar: End-to-End MIMO Radar Design and Multi-Modal Fusion for Autonomous Vehicle Perception](https://arxiv.org/abs/2607.17351)
*Eli Goldenshluger,Barak Pinkovich,Chaim Baskin*

Main category: cs.AI

TL;DR: 本文提出一种雷达为中心的传感器融合框架，通过端到端学习稀疏采样模式，在减少接收器数量的同时匹配或超越全阵列基线性能。


<details>
  <summary>Details</summary>
Motivation: 降低雷达成本和集成复杂度，同时保持甚至提升多模态3D检测性能。

Method: 在融合网络内端到端训练可学习的MIMO设计模块，该模块直接处理原始雷达ADC数据，并受其他传感器监督，学习激活哪些接收天线及数量。

Result: 在RADIal数据集上，稀疏雷达配置在性能上与全阵列基线相当或更优，但使用更少接收器。

Conclusion: 最优MIMO雷达设计依赖于融合堆栈和下游感知任务，可通过端到端学习实现。
...
### [69] [Agentic ERP: Multi-Agent Large Language Model Architecture for Autonomous Enterprise Resource Planning](https://arxiv.org/abs/2607.17331)
*Zhihao Liu,Tianyu Wang,Xi Vincent Wang,Lihui Wang*

Main category: cs.AI

TL;DR: 提出一种基于角色对齐的大语言模型智能体与风险分级人工监督结合的Agentic ERP架构，将ERP系统从被动记录交易转变为主动执行运营决策。


<details>
  <summary>Details</summary>
Motivation: 传统ERP系统依赖人工决策，规则自动化无法处理异常，单一大模型助手跨职能协调能力差，导致运营自动化受限。

Method: 将ERP操作建模为约束序列决策问题；采用基于图的Planner-Executor-Reflector-Responder编排机制，结合外部评分标准和冲刺合同进行解耦；通过角色对齐分配降低工具选择复杂度。

Result: 多智能体方法在跨职能危机任务上显著优于基线；在365天仿真中实现零库存短缺，而规则基线积累数百次缺货。

Conclusion: 角色对齐的LLM智能体在人类监督下能驱动ERP主动执行运营决策，提供了自主企业资源规划的参考架构和评估协议。
...
TL;DR: 提出一种自进化Lean证明智能体，通过协同进化智能体和基准测试，比固定基准方法在miniF2F上获得45.1%的解决率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手工设计工作流程，而代码级自进化智能体研究启发探索工作流程的自动进化。

Method: 构建包含固定运行时和可变工作空间的智能体，通过掌握限定的课程更新和单锚点重校准实现智能体与基准的协同进化。

Result: 自进化智能体在保留的miniF2F测试集上达到45.1%解决率，显著优于种子12.7%和固定基准32.0%。

Conclusion: 验证器约束的自进化能够有效提升Lean证明工作流程，即使基准本身也在进化。
...
### [71] [Quantifying Diversity of Thought: A Predictive Law of Weighted LLM Ensemble Lift](https://arxiv.org/abs/2607.17384)
*Junade Ali*

Main category: cs.AI

TL;DR: 本文提出一个经实验验证的形式化定律，用于计算大型语言模型（LLM）集成中思维多样性带来的提升。从第一性原理出发，推导了LLM集成提升的精确分解（救援质量和损害质量），并提取了预测集成性能的指标：准确率调整的正确性相关性 φ_adj，以及准确率差距和集体准确率。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对LLM集成中思维多样性提升效应的精确计算和预测方法。

Method: 从第一性原理推导LLM集成提升的分解公式，提出启发式计算方法。定义关键指标（φ_adj等），并在多模型、多基准数据集上验证。

Result: 在767,520次推理（10个模型）上测试，启发式在标定集上Spearman ρ=0.84，迁移至未见数据集ρ=0.51-0.84；交换质量跟踪实际提升R²≥0.96；原始φ预测力弱（R²≤0.09），φ_adj显著更优（R²=0.67）。

Conclusion: 提出的启发式可有效预测LLM集成提升，且在不同数据集上具有稳定迁移性；φ_adj是比原始相关性更可靠的预测指标。
...
### [72] [Intermittent Control Is Not Diluted Control: A Switching Effect in Artificial Agency](https://arxiv.org/abs/2607.17432)
*Veronique Ziegler*

Main category: cs.AI

TL;DR: 在具有状态历史记忆的适应性智能体中，间歇性获得预期控制能降低平均调节负担，且该效应无法用固定模式混合的加权平均预测，揭示了调节时机顺序对长期负担的隐藏影响。


<details>
  <summary>Details</summary>
Motivation: 先前假设提出智能体在稳定控制与反应性恢复之间切换时，平均调节负担应等同于两种固定模式的加权平均。本文旨在验证该假设是否成立。

Method: 通过模拟具有保留状态历史的适应性智能体，在周期性和随机切换模式下，比较间歇性预期控制与固定模式混合的调节负担差异。使用N=1000匹配重复复制的高统计量实验。

Result: 间歇性预期控制使平均调节负担低于固定模式混合的预测值，效应虽小(约0.5%增益)但一致(63-68%复制低于零)，且无后期负担累积的迹象。

Conclusion: 历史依赖自适应系统的调节负担不仅取决于各模式的时间比例，还取决于干扰与恢复进入状态的顺序。间歇性预期控制可作为一种减少长期恢复负担的机制。
...
TL;DR: 提出AEC-DS机制，利用PDP驱动的闭环自适应纠删码，通过节点信誉和QoS迁移策略优化分片放置，在800节点仿真中实现1.25倍冗余下100%数据持久性，减少66.8%-75.2%恢复操作。


<details>
  <summary>Details</summary>
Motivation: 传统去中心化存储中审计结果未直接用于冗余和分片放置，导致资源分配低效和恢复延迟。

Method: 基于PDP反馈动态更新节点信誉，结合QoS感知迁移策略，将高优先级分片从不稳定节点迁移到冷层可靠节点，并惩罚不稳定节点后续放置。

Result: 仿真中AEC-DS在1.25倍冗余下维持100%数据持久性，累计恢复操作减少66.8%-75.2%，类迁移使数据丢失预防能力提升176.8%。

Conclusion: PDP反馈可连接完整性审计与动态冗余及放置调整，为自修复去中心化存储提供可行方案，但需考虑迁移额外成本。
...
### [74] [Empirical Grounding Improves the Realism of LLM Agents Simulating Human Behavior During Disruptions](https://arxiv.org/abs/2607.17437)
*Chen Xia,Zexi Kuang,Yuqing Hu*

Main category: cs.AI

TL;DR: 提出一种基于经验数据的LLM智能体框架，通过嵌入人口统计、日常活动和空间上下文，显著提升对灾害期间人群行为模拟的统计 realism。


<details>
  <summary>Details</summary>
Motivation: LLM智能体能生成缺乏历史数据的灾害情境下的人类行为，但其个体级合理性可能无法复现真实人群分布，存在有效性缺口。

Method: 构建经验数据驱动的LLM智能体：利用美国社区调查、时间使用调查和城市空间数据嵌入智能体初始化和决策提示，并对比无经验数据基线。

Result: 经验模型在正常和热浪条件下均显著提高活动分布相关性（0.528→0.912, 0.349→0.836），降低误差（MSE 0.066→0.008, 0.098→0.012），并捕捉46.4%的实际热浪响应幅度。

Conclusion: 经验数据能使LLM智能体成为统计可信的人群行为模拟工具，但需进一步建模人类适应机制。
...
### [75] [Panache: One-Pass Motif Discovery at Every Window Length](https://arxiv.org/abs/2607.17481)
*Tej Sanibh Ranade*

Main category: cs.AI

TL;DR: 提出了一种名为Panache的一遍流算法，用于发现时间序列中具有未知持续时间的z归一化模式，显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法需尝试所有可能的窗口长度，导致计算开销呈二次方增长，缺乏高效的一遍解决方案。

Method: 利用滑动DFT递归和运行统计在线维护z归一化子序列的非DC频谱，通过哈希目录和Parseval定理的下界快速筛选相似子序列。

Result: 在17个UCR配置上恢复所有top-20模式，比CPU和GPU基线快数倍；在Wafer数据集上处理51个长度仅需6分钟，而最快CPU基线需7.95小时。

Conclusion: Panache是首个实现z归一化PMP一遍流发现的方法，通过频谱哈希和动态参数调整实现高效精确的模式挖掘。
...
### [76] [Pailitao-MMSearch: Building Native E-Commerce Multimodal Search Foundation](https://arxiv.org/abs/2607.17499)
...
TL;DR: 提出Pailitao-MMSearch，一种原生电子商务多模态搜索基础模型，通过混合语义ID、两阶段连续预训练和混合推理后训练，显著提升在线搜索效果。


<details>
  <summary>Details</summary>
Motivation: 现有单模态专家模型无法处理跨模态查询，而通用视觉语言模型缺乏领域特定知识，导致电子商务多模态搜索效果不佳。

Method: 采用混合语义ID、两阶段连续预训练策略和混合推理后训练管道，基于Qwen模型构建。

Result: 在淘宝的在线A/B测试中，GMV提升+13.61%，交易量提升+8.21%。

Conclusion: Pailitao-MMSearch证明了原生电子商务多模态搜索大语言模型的有效性，显著优于传统多模态搜索流程。
...
### [77] [Can AI Agents Really Complete RTL-to-GDS? Lessons from Benchmarking Tool-Interactive EDA Workflows](https://arxiv.org/abs/2607.17528)
*Jinyuan Deng,Zhengrui Chen,Xufeng Wei,Tianyu Xing,Chenyi Wen,Cheng Zhuo*

Main category: cs.AI

TL;DR: 提出FluxBench系统，评估AI代理在端到端EDA工作流中的性能，发现不同代理架构间性能差距可达86.27%，Token ROI差距达105.92倍。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅关注单个语言模型在孤立EDA任务上的表现，无法反映不同代理系统在完整EDA流程中的差异。

Method: 在统一提示、工具环境和技术库设置下，构建RTL生成和RTL-to-GDS流程，引入Token ROI指标衡量成本效率。

Result: 相同基础模型下不同代理架构性能差距达86.27%，Token ROI差异达105.92倍；FluxEDA在PicoRV32案例中得分达97.94，优于Claude Code。

Conclusion: 领域专用技能不足以提升大规模EDA场景中的代理性能，代理系统设计与基础模型能力均至关重要。
...
### [78] [Why Does Feedback-Augmented Self-Distillation Fail to Improve Retrieval-Interleaved Search Agents?](https://arxiv.org/abs/2607.17558)
*Fan Yang,Rui Meng,Yuxin Wen*

Main category: cs.AI

TL;DR: 提出FA-SD算法，发现解码坍塌现象，并引入EMA教师缓解不一致性以提升自蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 现有自我蒸馏在复杂智能体任务中效果未知，需要探索其有效性及潜在问题。

Method: 利用成功演示作为特权信息进行自蒸馏；通过分解不一致性为模型与提示不一致，采用指数移动平均教师稳定监督信号。

Result: 识别出解码坍塌问题，证明提示不一致性会降低监督质量；EMA教师虽需预热但最终提升性能。

Conclusion: 自蒸馏在智能体搜索中面临监督不一致挑战，EMA教师可缓解该问题。
...
### [79] [Retain or Consolidate? Budget-Dependent Operator Selection for Language Agent Memory](https://arxiv.org/abs/2607.17545)
*Qingcan Kang,Mingyang Liu,Shixiong Kai,Kaichao Liang,Zhentao Tang,Yuqi Cui,Tao Zhong,Mingxuan Yuan*

Main category: cs.AI

TL;DR: 论文提出了一种基于离线抽象-安全（OAS）的轻量级学习机制，通过权衡记忆保留与压缩的效用，在有限上下文窗口下动态选择最优操作符（合并、抽象或重写），从而提升语言代理的长程记忆性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型上下文窗口有限且推理成本高，现有记忆保留策略（保留原始记录）在预算紧张时可能遗漏关键证据，而记忆压缩策略（合并/抽象）可能丢失细节，因此需要一种动态决策机制来平衡两者。

Method: 将每个操作符的效用分解为覆盖效应（补充保留策略遗漏的证据）和有符号替换效应（替换已保留的原始证据），并基于预算压力动态平衡；实现通过离线抽象-安全（OAS）轻量级学习器，从预生成特征估算操作效用，并结合留出法进行伤害校准。

Result: 在LongMemEval和LoCoMo基准上验证了预算依赖模式：紧预算下压缩可提升绝对准确率最高48%，松预算下保留更优；跨笔记抽象和合并整体优于局部重写。

Conclusion: 记忆保留与压缩的选择取决于预算压力，该机制通过动态权衡实现了记忆效率与信息完整性的平衡，且离线学习器可有效预测最优操作。
...
TL;DR: 本文从两个角度研究强化学习：博弈论中的多智能体强化学习和基于基础模型的强化学习。


<details>
  <summary>Details</summary>
Motivation: 经典强化学习局限于单智能体和固定环境，无法处理策略交互、不确定性和高维世界等复杂问题。

Method: 第一部分研究多智能体强化学习在竞争和通用和博弈中的学习行为；第二部分利用生成模型和基础模型作为先验知识，发展扩散世界模型、视频生成和策略类等方法。

Result: 在博弈中揭示了均衡概念与策略学习的关系；在基础模型方向证明了生成模型能有效提升规划、控制和策略优化性能。

Conclusion: 强化学习是目标驱动的适应性框架，能够统一策略决策、环境建模和基础模型能力，拓展智能行为的理解。
...
### [81] [The Curvature Shadow: An Apparent Failure of Maximum-Entropy Equilibrium Selection is a Removable Artifact](https://arxiv.org/abs/2607.17543)
*Luis Leal*

Main category: cs.AI

TL;DR: 研究解释正则化纳什动力学在库恩扑克中与最大熵选择存在微小差异的原因，提出曲率阴影定律并验证其精度


<details>
  <summary>Details</summary>
Motivation: 正则化求解器在凸纳什均衡集中选择最大熵点，但库恩扑克中R-NaD结果与理论值存在0.021的坐标差距，需明确该差距是选择偏差还是计算伪影

Method: 推导坐标差距与熵短缺、熵曲面曲率的因子化关系，通过五组博弈实验验证，并用量子磁感强度扫掠检验因果性

Result: 差距满足gap≈√(2δ/κ)关系，五组实验误差<2×10⁻⁴；库恩扑克中唯一存在δ>0，磁感强度扫掠驱动δ→0时差距沿预测曲线趋零，拟合指数0.50与理论1/2高度吻合

Conclusion: 差距是熵短缺在平坦熵峰上的曲率阴影，可归因于计算短差而非系统性偏差，最大熵投影理论在平坦限制残余内成立
...
TL;DR: 提出ARBITER框架，通过双假设推理和多组件监督微调，实现高效、可解释的LLM安全护栏。


<details>
  <summary>Details</summary>
Motivation: 现有基于推理的护栏方法依赖昂贵的大模型生成推理链和全参数微调，成本高且可解释性不足。

Method: 双假设推理（同时考虑安全/不安全解释）和MC-SFT（按组件重要性加权损失），结合自生成推理链与LoRA高效微调。

Result: 在三项安全基准上优于现有方法，尤其在域外评估中表现显著。

Conclusion: 低成本、高性能的透明安全护栏有效提升LLM安全性。
...
TL;DR: 提出结构化记忆系统ZifaMem，通过会话摘要、情景记忆和用户模型提升AI伴侣情感连续性，在多个评估指标上优于原始对话历史。


<details>
  <summary>Details</summary>
Motivation: AI伴侣需要情感连续性，即记住用户身份、偏好和关系变化，而非仅关注单轮对话流畅性。

Method: ZifaMem结构化记忆系统，包含会话摘要、情景记忆和用户模型；使用固定LLM-as-a-judge协议和路由审计进行评估，并与原始历史、Mem0等系统比较。

Result: 结构化记忆使情感智力评分平均提升11.4%，人物角色基础在所有骨干模型上改善（Claude相对提升42%），多轮情感上下文偏好优于单轮（+39%），而额外情感状态机无增益。

Conclusion: ZifaMem显著优于原始历史部署，与Mem0统计等效（±5分），开源发布SDK、CLI和便携Agent技能。
...
### [84] [Mechanistic Attention Guidance for Agent Memory Refinement](https://arxiv.org/abs/2607.17621)
*Yechao Hong,Haiquan Qiu,Yaqing Wang,Quanming Yao*

Main category: cs.AI

TL;DR: 提出注意引导记忆精炼框架（AGMR），通过分析检索头注意力揭示记忆利用模式，实现细粒度记忆更新，在交互决策任务中提升性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有自演化记忆系统仅依赖文本输出，忽略了内部机制信号，导致错误归因不可靠和记忆修改幻觉。

Method: 构建上下文利用矩阵聚合注意力，识别段级记忆重用模式，据此指导删除、增强或简化记忆，并通过重执行验证更新。

Result: 在交互决策基准上，AGMR相比纯文本记忆精炼基线，同时提升了任务性能和记忆效率。

Conclusion: 注意力信号可作为记忆利用的有效机制信号，实现更可靠和高效的记忆演化。
...
### [85] [FlowBlock: Wavefront-Parallel Decoding for Self-Correcting Diffusion Language Models](https://arxiv.org/abs/2607.17652)
*Bing Tian,Haikun Liu,Xiaocheng Zhong,Zhuohui Duan,Zhaokai Luo,Huayi Jin,Zhiyong Wang,Xiaofei Liao*

Main category: cs.AI

TL;DR: 提出FlowBlock，一种无需训练的并行解码框架，通过自纠正扩散LLM和门控波前解码实现块级并行，显著提升速度且不损失精度。


<details>
  <summary>Details</summary>
Motivation: 块级扩散LLM因串行解码导致延迟高，现有后训练方法并行加速有限且精度下降，亟需无需训练的并行方案。

Method: 采用门控波前解码（限制并行块数、块因果掩码保持KV缓存复用）和异质波前打包（独立波前异步打包为稠密批次）实现并行。

Result: 在LLaDA-2.1/2.0上TPS提升最高2.95×/4.01×，延迟降低53.6%/77.1%，精度平均提升1.3点；相比D2F吞吐量提升16×。

Conclusion: FlowBlock通过自纠正机制实现高效并行解码，优于现有训练方法，是训练自由的实用方案。
...
### [86] [OrientSAM: Mitigating Camera-Centric Shortcut in Multimodal Spatial Reasoning via Orientation-Aware Spatial Alignment](https://arxiv.org/abs/2607.17657)
*Wenxiao Fan,Hang Yin,Kan Li*

Main category: cs.AI

TL;DR: 提出OrientSAM框架，通过显式注入朝向信息增强多模态大模型的空间推理能力，缓解相机中心捷径行为。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在需要视角变换的空间推理中仍表现不佳，常依赖相机中心线索而非参考对象视角。

Method: 设计朝向感知令牌和傅里叶角度编码注入朝向信息，结合课程学习策略逐步提升视角感知推理，构建空间数据流水线生成朝向监督。

Result: 在Spatial-MM、ViewSpatial和3DSRBench上，OrientSAM持续优于强基线，尤其在非相机视角、以人为中心和朝向敏感任务上表现突出。

Conclusion: 显式朝向建模对缓解相机中心捷径行为、实现更鲁棒的异中心空间推理至关重要。
...
TL;DR: 提出VRR-Stop框架，在噪声下的验证-修复-重复循环中通过信念滤波估计有效性和边际收益来决定停止时机；搭配VRR-Guard备用机制防止校准失败，显著提升最终真实有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在验证器和修复器均有噪声时，修复会损害已正确计划，且接受率持续上升而真实有效性下降，缺乏何时停止修复的严谨准则。

Method: 建立四参数噪声模型分离验证器和修复器的错误行为；通过信念滤波从重复验证票数估计承诺有效性；基于真实边际收益的符号决定继续修复或提交，只需参数符号可识别性；当验证器区分度不足时启用无估计的VRR-Guard备选机制。

Result: 在GSM8K压力测试中，VRR-Stop相比固定五轮修复将最终真实有效性提升60.6个百分点，平均仅使用0.72个修复轮次。停止可靠性由验证器区分度和决策边界共同决定，而非估计误差绝对值。

Conclusion: VRR-Stop能鲁棒地控制噪声VRL循环的停止时机，在提高真实有效性的同时减少不必要的修复轮次，其可靠性受验证器区分度和决策边界的联合影响。
...
### [88] [ProEvent: An Event-centric Benchmark for Proactive Agents](https://arxiv.org/abs/2607.17701)
*Guanzhen Li,Liangming Pan,Leye Wang*

Main category: cs.AI

TL;DR: 提出首个事件中心基准ProEvent，评估智能体从即时通讯聊天中主动维护用户日程的能力，发现当前LLM（如GPT-5.1）仅在26.7%场景中正确反应，存在过度反应和取消事件困难等问题。


<details>
  <summary>Details</summary>
Motivation: 现有主动智能体研究忽视事件中心协助，且主动协助的开放性导致可靠评估困难。

Method: 构建合成但真实的聊天场景（含用户动态交互、并发线程和现实噪声），从响应时机、单步正确性和多步正确性三个维度评估主动智能体。

Result: 八个LLM和流水线的实验表明当前智能体频繁过度反应，处理事件取消能力弱；GPT-5.1仅在26.7%场景中正确反应。

Conclusion: 当前LLM作为主动智能体存在根本局限，尤其在检测隐式事件和从用户第一人称视角推理方面不足。
...
### [89] [Artificial Intelligence for Understanding and Managing Transportation Behavior in Sustainable Smart Cities](https://arxiv.org/abs/2607.17694)
*Junbiao Pang,Muhammad Ayub Sabir,Fatima Ashraf*

Main category: cs.AI

TL;DR: 本文提出以行为为中心的人工智能视角，将城市交通数据视为行为证据，构建从数据输入到治理反馈的闭环框架，应用于公交到站预测、出租车模式发现、异常行为检测和乘客风险挖掘四个方向，并明确了部署所需的数据质量、隐私、公平性等关键条件。


<details>
  <summary>Details</summary>
Motivation: 城市交通系统生成异构数据，但这些数据不能自动转化为可操作的管理智能，因此需要一种统一的方法将行为证据转化为各类决策支持。

Method: 采用行为中心视角，将移动记录和乘客生成文本视为行为证据；通过闭环框架整合数据输入、行为表征、AI推理、决策支持、公共价值和治理反馈；部署条件包括数据质量、隐私、公平性、可解释性、不确定性、可迁移性和人类问责。

Result: 在公交到站预测、出租车模式发现、异常行为检测和乘客风险感知四个方向实现了从行为证据到运营、规划、监管和乘客服务决策的统一路径。

Conclusion: 确立了行为中心AI在城市交通管理中的可行框架，强调了部署需满足的多项条件，以保障技术有效性和社会可接受性。
...
### [90] [LaT: LLM-as-Trainer for Multi-Task Vehicle Routing Solvers](https://arxiv.org/abs/2607.17708)
*Yang Wang,Ya-Hui Jia,Wei-Neng Chen,Yi Mei,Wen Song,Zhiguang Cao*

Main category: cs.AI

TL;DR: 提出LLM-as-Trainer训练范式，利用大语言模型作为外部教练，通过阶段反馈提升多任务VRP求解器性能。


<details>
  <summary>Details</summary>
Motivation: 现有多任务VRP求解器缺乏训练状态反馈，容易偏向特定变体，而元学习计算成本过高。

Method: 使用预训练LLM分析跨任务验证指标，生成阶段指导向量，与约束向量结合注入编码器层。

Result: 在16个VRP变体上提升了多种先进求解器的解质量，包括训练和未见过的变体。

Conclusion: LaT是一种有效且通用的训练范式，可增强多任务VRP神经求解器的泛化能力。
...
### [91] [WuYu-EnvLE-Bench: A Benchmark for Evaluating Large Language Models in Environmental Law Enforcement](https://arxiv.org/abs/2607.17745)
*Ziliang Yang,Yi Zhang,Kaijun Lin,Jiachao Ke,Haihong Xu,Zongguo Wen*

Main category: cs.AI

TL;DR: 针对大语言模型在环境执法中的证据推理能力进行评估，构建了包含2521个实例的WuYu-EnvLE-Bench基准，发现模型在规则任务表现好，但证据链构建、多源融合等复杂任务仍不可靠。


<details>
  <summary>Details</summary>
Motivation: 大语言模型被考虑用于环境执法，但能否产生可追溯的执法决策尚不明确。

Method: 基于真实执法案例、监管标准和专家评审构建基准，包含14个任务和12个污染介质子领域，使用AES和IEI指标评估模型。

Result: 模型在规则约束任务上表现良好，但在证据链构建、矛盾检测、多源整合和程序判断上不可靠；模型扩展收益递减。

Conclusion: 需要基于证据、规则感知和任务自适应的执法推理能力提升。
...
### [92] [Semantically Similar, Logically Distinct: Diagnosing the Semantic-Answerability Gap in Table RAG](https://arxiv.org/abs/2607.17742)
*Jiaming Tian,Liyao Li,Wentao Ye,Haobo Wang,Lihua Yu,Zujie Ren,Gang Chen,Junbo Zhao*

Main category: cs.AI

TL;DR: 本文提出TCR-Bench基准，揭示检索增强生成中表格的语义-可答性差距，并验证可答性感知重排序能显著恢复性能。


<details>
  <summary>Details</summary>
Motivation: 检索模型优化语义相关性而忽视表格可答性，导致即使检索到正确表格组也无法定位可答表格，造成性能下降。

Method: 构建TCR-Bench基准测试（基于相似模式但内容不同的兄弟表格），评估密集检索器，并测试两阶段流水线AAR（可答性感知重排序）。

Result: 密集检索器存在语义-可答性差距，QA性能从0.755（oracle）降至0.330（top-5）；AAR将top-1目标检索率从18.2%提升至57.4%。

Conclusion: 表格可答性验证是RAG中缺失的关键步骤，模型性能限制并非主要障碍，AAR可作为诊断差距的有效方法。
...
TL;DR: 本文首次发现视觉语言模型（VLM）的潜在空间中否定语义不具有线性/非线性可分性，并提出跨模态注意力架构显式建模模态依赖，在F1上提升最多7.03%；同时揭示文本否定常独立出现，而视觉否定依赖语言上下文，结合JEPA2推进时序否定建模。


<details>
  <summary>Details</summary>
Motivation: 当前多模态系统难以跨模态检测高级语义概念（如否定），主要因为标准VLM的潜在空间未能将否定编码为可泛化的线性/非线性可分类别，且缺乏对否定跨模态依赖性的建模。

Method: 1) 分析VLM潜在空间否定表示的线性/非线性可分性；2) 提出显式建模跨模态依赖的注意力架构；3) 利用Qwen2.5-VL自动标注3222个政治视频-文本对进行统计分析；4) 结合自监督视频表示（JEPA2）改进时序否定学习。

Result: 跨模态注意力架构相比单模态基线在F1上最高提升7.03%；统计分析发现文本否定可独立存在，而视觉否定语义依赖于语言上下文；JEPA2整合进一步提升了时序否定建模效果。

Conclusion: 本文通过分析否定表示的局限性、提出新架构并揭示不对称依赖性，为多模态系统中学习鲁棒且语义对齐的表示提供了新方法和见解。
...
### [94] [SR-Agent: An Experience-Driven Agentic Framework for Post-Ranking Strategies Refinement in E-Commerce Recommendation](https://arxiv.org/abs/2607.17719)
*Hanchen Yang,Kaiwen Yang,Junpeng Zhuang,Yang He,Keting Cen,Bochao Liu,Zhongbo Sun,An Liu,Zhongteng Han,Chenyi Lei*

Main category: cs.AI

TL;DR: 本文提出SR-Agent，一个用于工业电商推荐系统中后排序策略自动优化的智能体框架，通过用户模拟、问题分析和受约束的策略优化，实现了自助式持续改进。


<details>
  <summary>Details</summary>
Motivation: 工业推荐系统的后排序策略依赖静态配置，难以适应动态变化的电商环境，手动调优成本高、速度慢且难以复用。现有基于LLM的智能体未能实现完整的自动化闭环。

Method: SR-Agent包含三个组件：UserSim智能体通过分阶段检测技能发现用户感知的坏案例；Analysis智能体将重复坏案例归纳为结构化诊断；受约束的策略优化工具箱根据诊断执行类型与范围受限的动作，并通过四阶段奖励管道（含可逆回滚机制）进行筛选。

Result: 在快手电商平台一个月的在线A/B测试中，订单量提升0.71%，浏览深度提升0.34%，点击类别多样性提升0.48%，同时显著缩短优化周期并降低运营成本。

Conclusion: SR-Agent是首个部署于工业推荐系统的后排序策略自动化优化框架，通过闭环迭代有效提升用户体验与业务指标，验证了LLM智能体在工业场景中的实用性。
...
### [95] [Dynamic Defense Profiling Enables Cognitive Jailbreak of Text-to-Image Models](https://arxiv.org/abs/2607.17779)
*Dongdong Yang,Deyue Zhang,Zhao Liu,Zonghao Ying,Wenzhuo Xu,Jiankai Jin,Xiangzheng Zhang,Quanchen Zou*

Main category: cs.AI

TL;DR: 提出了MIND框架，通过建模潜在防御机制，将多模态反馈转化为高密度信号，在多个T2I模型上实现高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有越狱攻击方法依赖粗粒度的二元反馈（成功/失败），忽略了文本拒绝、视觉屏蔽等失败模式中的丰富信息，导致探索效率低和语义崩溃。

Method: MIND框架包含三个核心组件的推理驱动进化优化：多模态评估器进行细粒度反馈分解，防御建模器迭代更新信念，元记忆模块检索有效攻击策略，整合成一个自适应、语义一致的越狱生成过程。

Result: 在Stable Diffusion v1.5的六种防御设置下，ASR达95.62%；在四个商业T2I系统上最高ASR达91.58%。

Conclusion: MIND通过认知推理和多模态反馈利用，显著提升了越狱攻击效果，超越了现有方法。
...
### [96] [Financial Audit Assistance using Misinformation Detection and Explanation](https://arxiv.org/abs/2607.17797)
*Kshitij Madhav Jadhav,Sushodhan Vaishampayan,Manoj Apte,Sachin Pawar,Nitin Ramrakhiyani,Girish Keshav Palshikar*

Main category: cs.AI

TL;DR: 本文提出无监督方法检测财务报表中的虚假信息，并解释可能的虚假信息来源，基于历史财务报表及审计报告，在11460份数据上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 财务报表审计复杂耗时，且存在人为隐瞒或篡改信息的动机，现有审计系统缺乏自动化辅助工具，需要AI系统自动检测虚假信息并定位数据源。

Method: 采用无监督技术，利用历史财务报表和相关审计报告构建模型，识别财务数据中的异常变量并生成解释性线索。

Result: 在五年期11460份财务报表及审计报告的大规模语料上验证了方法的有效性，能准确检测虚假信息并指示潜在源头。

Conclusion: 该无监督方法可辅助审计师高效定位财务误导信息，提升审计效率与准确性，并为后续深入研究提供基础。
...
### [97] [PGN: Design and Implementation of a Vision-Language Navigation System Based on Pangu Multimodal Foundation Model](https://arxiv.org/abs/2607.17806)
*Li Xian,Mingxi Li,Yizheng Wang,Yiming Shen,Qi Chen,Zhuoling Xiao*

Main category: cs.AI

TL;DR: 提出了基于OpenPangu-7B的离线视觉-语言导航动作预测系统PGN，通过两阶段训练实现专家轨迹的高精度动作匹配。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型在VLN任务中面临视觉-语言对齐、紧凑时间输入、动作空间接地和硬件稳定训练等挑战。

Method: 两阶段训练：第一阶段用Q-Former和MLP投影器冻结对齐视觉编码器和语言骨干；第二阶段用五观察窗口、时间采样和LoRA适配器适配专家轨迹。

Result: 在500条专家轨迹上，NAM达到62.29%，NER达到100.00%。

Conclusion: PGN有效对齐视觉-语言并预测专家动作，但闭环导航性能需进一步评估。
...
### [98] [A Hardware-oriented Approach for Efficient Bayesian Inference Computation and Deployment](https://arxiv.org/abs/2607.17855)
*Nikola Pižurica,Matteo Risso,Nikola Milović,Alessio Burrello,Igor Jovančević,Conor Heins,Miguel de Prado*

Main category: cs.AI

TL;DR: 本文提出了一种硬件导向的方法，用于在商用嵌入式GPU上加速离散贝叶斯推理，通过改进内存布局和引入稀疏表示等技术，实现了高达5倍的加速比。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯推理的计算成本高，阻碍了其在资源受限的边缘设备上的部署。

Method: 采用两种互补的合并策略重构张量收缩的内存布局，生成紧凑规则的原语；引入稀疏数组表示和张量聚类方案减少内存占用；利用机器学习自动调优选择最佳算法变体。

Result: 在NVIDIA Jetson Orin AGX上对770个随机POMDP配置进行测试，实现了最高5倍、典型2-2.5倍的加速，输出与基线相同。

Conclusion: 该方法能有效加速嵌入式GPU上的离散贝叶斯推理，且保持数值精度。
...
TL;DR: 提出了探索-同化反思（EAR）框架，用于提升LLM自主代理的检索性能，结合探索性反思和同化反思，在长期对话任务中检索准确率提升高达17.9%。


<details>
  <summary>Details</summary>
Motivation: 现有记忆检索方法缺乏适应性、样本效率低，且难以从异构存储中检索到正确的混合记忆。

Method: EAR框架包含探索性反思（迭代搜索引导检索并收集经验）和同化反思（从经验缓冲区回放经验以全局优化重排序器）。

Result: 在两项长期对话基准上，检索性能相比基线提升最多17.9%，且具有高样本效率和鲁棒性。

Conclusion: EAR能有效提升初始检索性能并实现样本高效的适应，优于依赖即时奖励的方法。
...
TL;DR: 提出STACE框架，用多智能体自动化压力测试概念消除模型，优于现有评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有概念消除评估方法静态预定义，无法暴露多样化自然语言提示下的漏洞，且人工设计有偏差、难扩展。

Method: 基于自适应假设搜索，使用多个LLM代理迭代生成和验证压力测试假设，结合外部知识。

Result: 在四个概念类别上优于五种LLM基线；在多种模型、消除方法和强度下表现鲁棒；可扩展到LLM越狱等领域。

Conclusion: STACE是概念消除评估的有效通用框架，可为负责任的AI部署服务。
...
TL;DR: 提出训练无关的时空令牌否决方法，利用扩散模型的多步观察能力改进多模态推理，无额外成本提升准确率最高9%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型使用思维链推理存在高顺序生成成本、错误积累和有限自我纠正问题，而扩散多模态大语言模型虽有序无关解码和迭代细化优势但推理能力未充分探索。

Method: 时空令牌否决（ST-Veto）：通过二阶泰勒预测置信度动态来否决时间不稳定令牌，并使用图像注意力质量过滤弱对齐令牌，替换为更安全候选。

Result: 在多个扩散多模态大语言模型和多模态推理基准上，ST-Veto一致优于标准解码策略和先前视觉语言模型推理方法，准确率提升最高9%。

Conclusion: ST-Veto能够引导生成朝向更高置信度和更好对齐的路径，无需额外训练或生成成本，有效增强扩散多模态大语言模型的推理能力。
...
### [102] [PEARL: Auditable Repair for Scientific Reasoning Graph Extraction](https://arxiv.org/abs/2607.17917)
*Bohan Su,Pengze Li,Yuchen Lu,Xi Chen*

Main category: cs.AI

TL;DR: 提出PEARL框架，通过抽象和修复层将LLM生成的噪声图转换为可审计的科学推理图，在ARCHE基准上严格门控通过率从0/350提升至300/350。


<details>
  <summary>Details</summary>
Motivation: LLM生成图表推理时存在语法错误、边标签漂移、根节点方向和源锚点弱等问题，缺乏语义有效性。

Method: 基于皮尔士模式显式化图内容，利用证据匹配的裁判反馈修复被拒绝的边类型、局部推理步骤和终端根节点，同时保留审计轨迹。

Result: 在ARCHE的5个70论文模型存档上，严格门控通过率从0/350提升至300/350，平均REA从0.339提升至0.906。

Conclusion: PEARL为需要可检查推理轨迹的研究代理和AI科学家工作流提供了可靠性层，优于无约束的图重建。
...
TL;DR: 探索将大语言模型驱动的决策引入基于主体的模型对模拟可靠性、计算成本和行为的影响。


<details>
  <summary>Details</summary>
Motivation: ABM依赖简单规则，而LLM能提供更复杂的决策能力，但需评估其对模拟的可靠性和成本的影响。

Method: 在Mesa ABM框架中扩展经典Schelling隔离模型，引入一个通过工具调用委托LLM进行分类的主体，与使用符号规则的普通主体进行混合模拟。

Result: 小模型在语义分类任务中失败或无法稳定生成工具调用，而大模型通过初步测试；统计模型检查可量化LLM组件对模拟的影响。

Conclusion: LLM的引入可能影响模拟可靠性，需通过统计模型检查来评估其行为变化和成本影响。
...
TL;DR: 本文提出自主能动性量表（AAS），从七个维度对AI系统的自主能动性进行评分，区分活动期与空闲期的行为表现，并应用于六个现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架仅衡量认知能力、任务自动化或灾难性风险，但未评估系统的自主能动性，即系统自主行动的程度。

Method: 提出包含7个维度（认知自主、时间持久性等）的0-5级行为框架，每个维度分为活动期和空闲期两个时间带，通过可证伪的阈值测试进行评分，并引入空闲间隙测试来区分自我导向与预调度行为。

Result: 任务型智能体在活动期得分为2.3-2.4，空闲期仅为0.6-1.9，其空闲行为可归因于用户预设调度；而持久陪伴架构（Airi）是唯一空闲期行为在移除触发后依然持续的系统。

Conclusion: AAS能够量化单分数框架忽略的自主能动性边界，但存在单评估者、开发者偏差等局限性，活动期的自我导向边界仍需进一步操作化。
...
TL;DR: 提出了一个基于需求的框架OntoExtend，利用大语言模型和检索增强生成技术来自动化本体扩展过程，并通过案例验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 手工本体扩展耗时且易出错，现有方法很少将扩展与需求和可重用核心模型明确关联，且缺乏系统的LLM输出评估。

Method: 采用检索增强生成（RAG），从输入本体和需求（能力问题）中检索相关信息，以生成基于能力问题的扩展建议。

Result: 在39个能力问题上的评估显示，生成的本体片段结构问题少，通过所有功能测试，且被领域专家评定为只需要少量到中等修改即可集成。

Conclusion: OntoExtend可作为需求驱动本体扩展的有效草稿助手，其效果受能力问题特异性和建模档案的影响。
...
TL;DR: 提出一种先验条件规划器，通过潜在子目标分解引导动作生成，显著提升长时域规划性能。


<details>
  <summary>Details</summary>
Motivation: 长时域规划中，固定候选动作预算在指数级搜索空间内难以产生高质量候选序列，导致性能受限。

Method: 使用不同时间尺度的潜在子目标作为先验，引导动作序列生成，再由冻结的世界模型评估与优化。

Result: 在目标偏移150时，PushT成功率从12.7%提升至64.7%，OGBench Cube从26.7%提升至67.3%。

Conclusion: 潜在子目标分解结合先验条件动作生成，能有效平衡长时域与短时域规划性能。
...
TL;DR: 研究了等值线图在基础模型空间理解中的有效性，构建了基准测试并发现地图能显著提升空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 探究当模型可直接处理结构化地理数据时，等值线图是否仍对机器空间理解有帮助。

Method: 创建ChoroplethMap-Bench基准测试，包含2400张合成等值线图和12000个问题，评估22个模型在三种输入条件下的性能。

Result: 地图结合数据条件下性能最优，尤其在需要高级空间模式理解的任务中提升明显。

Conclusion: 等值线图仍是基础模型空间推理的有价值外部表示。
...
TL;DR: 本文提出PAMD方法，通过可学习的马氏距离替代固定范数度量，提升强化学习中的潜在状态相似性估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定全局范数（如ℓp距离）限制表达能力，而无约束距离可能导致退化解，需更灵活的结构化距离。

Method: 提出成对自适应马氏距离（PAMD），参数化正定、条件依赖的度量，作为现有基于双模拟RL算法的插件。

Result: 在视觉MuJoCo连续控制任务中，多个双模拟RL算法使用PAMD后最终性能显著提升。

Conclusion: PAMD提供比固定距离更富表达性且结构化的替代方案，可有效改进强化学习表征学习。
...
### [109] [AdaHome: An Adaptive Smart Home Assistant using Local Small Language Models](https://arxiv.org/abs/2607.18034)
*Eu Jin Lim,Zhaoxing Li,Sebastian Stein*

Main category: cs.AI

TL;DR: 提出了AdaHome，一种面向本地部署小语言模型的智能家居助手，通过意图感知规划、链式草稿策略和偏好适应机制，实现了高效、隐私保护且支持个性化。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能家居助手依赖云端部署、计算成本高且隐私存忧，同时缺乏稳定的长期个性化支持。

Method: 引入意图感知规划框架（路由命令至直接提示或轻量推理）、链式草稿策略（高效稳定决策）及偏好适应机制（从用户反馈学习，无需重训练或提示增强）。

Result: 直接命令准确率86.7%（提升3倍延迟），模糊输入保持竞争力，多轮场景偏好一致性达88%（基线仅52.5%）。

Conclusion: AdaHome在资源受限环境中实现了高效、精准且个性化的智能家居任务处理。
...
### [110] [Rethinking Heterogeneous LLM Merging: A Weighted Model Averaging Perspective](https://arxiv.org/abs/2607.18026)
*Jiahe Fan,Yinghao Hou,Si Chen,Aiyuan Zhang,Hong Xie,Defu Lian*

Main category: cs.AI

TL;DR: 论文研究了不同参数规模的大语言模型能否通过简单加权平均直接融合，发现结合维度适应和比例控制的参数平均是一种出人意料的强基线方法，但平衡插值会导致性能崩溃，存在跷跷板效应。


<details>
  <summary>Details</summary>
Motivation: 现有异构模型融合方法通常需要蒸馏、适配器、潜在空间学习等复杂手段，论文探究更简单的直接加权平均在十亿级参数异构模型上的可行性。

Method: 采用无训练的维度适应（扩展或截断参数空间）结合比例控制的插值方法，在Qwen系列模型对上进行融合实验。

Result: 确定性扩展能保留源模型功能，小比例插值可通过迁移互补能力提升性能；但近平衡插值常崩溃，任务层面存在跷跷板效应。

Conclusion: 基于简单参数平均并配合轻量维度适应和精细比例控制，是异构LLM融合的强基线，揭示直接加权融合的极限可能也限制了更复杂方法的效果。
...
TL;DR: LLM judges高估了临床语言模型的安全改进，真实效果依赖于评判者校准，且伴随帮助度下降。


<details>
  <summary>Details</summary>
Motivation: 现有测量临床语言模型安全性的方法未区分真实行为变化与评判者校准的影响。

Method: 在三个公共基准上，使用结构化的证据充分性提示测试四个模型，采用主要GPT评判、次要Claude评判、正确性评判、匹配控制及盲法临床评审。

Result: 不安全过度自信从49.3%降至24.7%，但效果大小依赖评判者（Sonnet减半），主要评判者灵敏度高(1.00)而特异度低(0.55)，帮助度从80.3%降至50.3%（模型差异大）。

Conclusion: LLM评判的临床安全效应应报告为方向和相对值，以人类评审为基准，并联合评估帮助度，不能视为校准的绝对率，不表明临床部署可行。
...
TL;DR: 提出SOPHIA方法，通过隐状态干预实现对大语言模型推理过程中自循环失败的控制，提升推理质量和令牌效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅能在输入层面控制推理过程，无法对推理轨迹进行细粒度控制，且失败轨迹常陷入自循环。

Method: 将推理轨迹建模为隐状态序列，通过状态分类、记录步级转移构建导向向量库。推理时检测自循环并应用向量干预。

Result: 能可靠干预自循环失败，导向向量可泛化至不同状态对，最终任务准确性和令牌效率显著提升。

Conclusion: 细粒度隐状态控制能有效改善推理质量，提供了超越输入级提示的推理轨迹调控手段。
...
### [113] [The Shared Discovery Paradox: How a One-Answer Rule Turns Better Information into Worse Search](https://arxiv.org/abs/2607.18045)
*Yohei Nakajima*

Main category: cs.AI

TL;DR: 论文研究了一个16个盒子、1个目标、8个搜索者的经典发现问题，发现群体共享排名信息虽提高最佳建议准确度（0.20→0.3835），但重复采用会因协议错误降低群体发现率（0.8322→0.3835），而协调行动或适当激励机制可恢复效率。


<details>
  <summary>Details</summary>
Motivation: 揭示组织集中信息后共享排名导致群体发现效率下降的悖论（协议失败而非信息失败），并探索激励、协调和依赖性的影响。

Method: 构建可解析基准模型：包含混合策略均衡、势博弈分析、水填充规则、协作惩罚价格（2-1/N）、潜在公共线索模型及大规模比例极限。

Result: 协调8行动组合达0.8594；自私均衡达0.5991；唯一救援奖励实现最优；复制概率c>0.788时市场超越分散报告；大规模极限中共识消失，其余协议排序保持。

Conclusion: 信息共享效率取决于协议设计：重复单一答案压缩有效行动组合，而协调或合适激励可恢复甚至超越分散搜索，贡献了分离信息、分配、激励和依赖性的可量化基准。
...
### [114] [SGA: Plug&Play Geometric Verification for Educational Video Synthesis](https://arxiv.org/abs/2607.18116)
*Lopez Jhon,Hinojosa Carlos,Ghanem Bernard*

Main category: cs.AI

TL;DR: 提出了符号几何智能体(SGA)和Manim视觉质量评分(MVQS)，用于改善LLM生成的动画代码的空间正确性和视觉可读性，在多个LLM骨干网络上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有框架强调教学内容但忽略了几何遮挡问题，导致LLM生成的动画代码空间正确性和视觉可读性不足。

Method: SGA模块拦截LLM生成的代码，部分执行以提取符号场景图，检测到空间冲突时进行针对性优化；MVQS作为无需渲染的空间完整性代理指标。

Result: 在MMMC-Code基准测试中，SGA在8个骨干网络×管线配置中的7个上提升了MVQS，最高达到73.11（Code2Video + GPT-5.1），相对原始基线提升16.1%。

Conclusion: SGA作为即插即用模块能有效提升代码驱动动画的空间质量，MVQS是可靠的渲染无关评估指标。
...
### [115] [What Makes Linguistic Representations Good Models of High-Level Visual Perception in the Human Brain?](https://arxiv.org/abs/2607.16214)
*Anna Bavaresco,Ina Klarić,Raquel Fernández,Marie-Francine Moens*

Main category: cs.CV

TL;DR: 该研究探讨了语言模型生成的图像描述如何预测大脑活动，发现机器生成的描述和文本嵌入模型比人工注释的描述和自回归模型更有效，且预测效果在神经网络中间层达到峰值。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明语言模型对图像描述能预测大脑反应，但驱动因素尚不明确。

Method: 使用六种图像描述类型（含人工与机器生成），五种语言模型（自回归LM与文本嵌入器），分析其与大脑活动和行为判断的一致性。

Result: 机器生成的描述优于人工描述，文本嵌入器始终优于自回归LM，预测峰值出现在网络中间层。

Conclusion: 描述内容与语言模型类型均影响建模表现，描述嵌入可作为研究高级视觉感知的有效工具。
...
### [116] [ForensicNet: Lightweight Attention-Enhanced MobileNetV2 for Automated Face Identification](https://arxiv.org/abs/2607.16273)
*Savitha N J,Lata B T*

Main category: cs.CV

TL;DR: 提出了一种轻量级深度学习框架ForensicNet，结合MobileNetV2和CBAM注意力机制，用于法医人脸识别，在保持计算速度的同时提高准确性。


<details>
  <summary>Details</summary>
Motivation: 法医环境中因姿态、光照、遮挡变化以及缺乏标注数据，导致自动识别困难。

Method: 采用MobileNetV2作为骨干网络，集成CBAM模块提升判别特征学习，并利用两阶段迁移学习策略（自适应层解冻）改善域适应和减少过拟合。

Result: 在LFW和SCFace数据集（15000张人脸，68个类别）上，准确率92.4%，精确率90.8%，召回率89.5%，每次推理仅需2.1 GFLOPs。

Conclusion: 该框架在实时法医监控应用中具有可行性，性能优于AlexNet、ResNet-50和MobileNetV2等基线模型。
...
### [117] [A Step Forward Towards Trustworthy Risk-Aware Facial Retrieval (RA-FR)](https://arxiv.org/abs/2607.16279)
*Muhammad Emmad Siddiqui,Muhammad Rafi*

Main category: cs.CV

TL;DR: 提出风险感知人脸检索框架RA-FR，通过混合盲人脸修复、自监督特征提取和共形预测，在监控场景中保证以用户指定风险水平包含真实身份。


<details>
  <summary>Details</summary>
Motivation: 现有系统在监控场景（低分辨率、运动模糊、光照变化）中可靠性不足，无法容忍漏检。

Method: 结合Latent Consistency Models与DiffBIR进行混合盲人脸修复，使用DINOv1 ViT-B提取特征，基于Hoeffding不等式进行共形预测动态调整检索集大小。

Result: 在IMFDB基准上以5%风险水平实现平均约10张图像的检索集，满足可靠性目标。

Conclusion: RA-FR通过统一领域修复、鲁棒特征学习和可证明决策规则，提供了可靠且可审计的监控人脸检索方案。
...
### [118] [The JEPA Predictor: A Transferable Operator for Occluded Feature Completion](https://arxiv.org/abs/2607.16274)
*William Nguyen,Christopher Nguyen*

Main category: cs.CV

TL;DR: 本文证明JEPA中的预测器可跨编码器家族移植：通过单个线性投影（500张ImageNet图像拟合），冻结的I-JEPA/V-JEPA预测器能提升CLIP、DINO等非JEPA编码器在遮挡场景下的分类准确性，线性桥性能随掩码率单调增长。


<details>
  <summary>Details</summary>
Motivation: 传统JEPA部署丢弃了与编码器联合训练的预测器（本质是部分视图分类器），本文探索能否将此预训练完成的预测器作为即插即用模块，提升非JEPA编码器在遮挡场景的特征完整性。

Method: 在四个非JEPA编码器（CLIP/DINOv3/DINOv2/MAE）与两个JEPA预测器（I-JEPA/V-JEPA）之间，通过闭式求解的线性投影（500张ImageNet图像）对齐特征空间，冻结预测器并评估不同掩码率下的分类提升效果。

Result: 1) 重遮挡下预测器能显著缩小与非JEPA判别模型的性能差距（如CLIP+IJEPA在ImageNet-9恢复大部分遮挡损失）；2) Stanford Dogs从15.9%提升至52.1%（+36pp）；3) 提升效果随掩码率单调增加，但低掩码率下投影成本可能超过收益。

Conclusion: 冻结的JEPA预测器可作为跨编码器家族的便携式遮挡特征补全算子，无需重训练，仅需线性投影适配即可显著改善掩码场景的分类性能。
...
### [119] [3D FaceShell: Attribute Transfer in 3D Face Avatars as a VLM Defense Mechanism](https://arxiv.org/abs/2607.16280)
*Weston Bondurant,Srijan Das,Hieu Le,Stephanie Schuckers*

Main category: cs.CV

TL;DR: 提出3D FaceShell框架，通过可学习高斯壳层扰动3D面部模型渲染结果，在不影响人类视觉感知和身份保持的前提下，引导VLM对其属性的语义解读。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要在2D图像空间操作，无法应对从3D面部模型渲染图像中提取敏感属性的隐私风险，需一种保护隐私的同时保持3D模型可用性的方法。

Method: 在原始3D表示上附加可学习高斯壳层，通过多视图嵌入对齐优化产生视觉不可察觉但视图一致的扰动，以重定向VLM属性推断。

Result: 在重建名人头像和多种黑盒VLM上实验表明，该方法能显著提高属性注入和错误率，同时保持高感知相似性和身份一致性。

Conclusion: 可以在不损害人类可识别外观的情况下操纵VLM对3D人脸语义的理解，为3D面部隐私保护提供新思路。
...
### [120] [Moving Like a Human: Ego-Motion-Normalized Temporal Signatures for Real-Time Aerial Person Tracking on Milliwatt-Class Hardware](https://arxiv.org/abs/2607.16282)
*Akbar Anbar Jafari,Cagri Ozcinar,Gholamreza Anbarjafari*

Main category: cs.CV

TL;DR: 提出了EMTS-Det，一种用于无人机跟随目标的轻量级检测系统，在低功耗设备上实现高效运行。


<details>
  <summary>Details</summary>
Motivation: 无人机上的跟随追踪需在廉价计算平台上实时运行，但目标尺寸小、背景杂乱，单帧外观检测器效果差。

Method: 五阶段系统：估计自我运动、生成残差运动通道、用22k参数网络检测行人中心、卡尔曼滤波跟踪、1D卷积运动分类。

Result: 在VisDrone上AP25达0.694；Raspberry Pi Zero 2W上31.85 FPS、AP25 0.462；实际序列97.9%锁定率、零误重建。

Conclusion: 将时域信息纳入输入表示而非学习模块，结合轻量网络与运动分析，可显著提升低功耗平台上的跟踪性能。
...
### [121] [GenSyn10: A Multi-Generative AI Dataset For Benchmarking Image Classification](https://arxiv.org/abs/2607.16283)
*Md Faraz Kabir Khan,Saeed Anwar,Ghulam Mubashar Hassan*

Main category: cs.CV

TL;DR: 本文介绍GenSyn10，一个CIFAR-10对齐的合成图像数据集，用于研究对未见生成器的检测泛化能力，发现现有检测器在域外泛化上仍有显著局限。


<details>
  <summary>Details</summary>
Motivation: 生成式AI快速发展，检测器对未见生成器的性能显著下降，缺乏标准化基准来研究跨生成器泛化问题。

Method: 使用FLUX.2-dev、HunyuanImage-3.0、Qwen-Image-2512三种架构生成60000张32×32图像，采用模板提示引擎和降采样，并通过四阶段协议评估17个分类模型。

Result: CIFAR-10预训练模型对GenSyn10零样本准确率达96.86%，微调后达99.88%；二元检测中，对已见生成器准确率97-99.9%，但对未见生成器降至79-96%。

Conclusion: GenSyn10作为控制基准，揭示了检测器在域外泛化上的持续挑战，为鲁棒性、域适应和跨生成器研究提供支持。
...
### [122] [MAC 2026: Advancing Micro-Action Analysis Towards Fine-Grained Understanding](https://arxiv.org/abs/2607.16284)
*Kun Li,Dan Guo,Jihao Gu,Pengyu Liu,Xiaobai Li,Haoyu Chen,Yanbin Hao,Guoying Zhao,Meng Wang*

Main category: cs.CV

TL;DR: 本文介绍了第三届微动作分析挑战赛，从识别转向细粒度理解，引入基于多模态大语言模型的新任务，总结了数据集、任务设置、评估协议及竞赛结果。


<details>
  <summary>Details</summary>
Motivation: 微动作难以标注、建模和评估，缺乏标准化平台，因此需要公开基准来推动研究。

Method: 举办年度挑战赛，提供公开数据集和协议，并在第三届引入多模态大语言模型辅助的细粒度微动作理解新任务。

Result: 总结了数据集、任务设置、评估协议、竞赛结果及优胜团队的解决方案。

Conclusion: 微动作分析需从识别转向更细粒度的语义理解，未来在人机交互和情感计算中有广阔应用前景。
...
### [123] [Depth Estimators Are Implicit Neural Fields for 3D Scene Geometry Inpainting and Reconstruction](https://arxiv.org/abs/2607.16286)
*Yingzhao Jian,Zihao Lin,Hehe Fan*

Main category: cs.CV

TL;DR: 提出NDF，通过测试时统一优化深度估计与隐式场拟合，解决3D几何补全中跨视角不一致与分布外数据不可靠问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖深度估计补全3D几何，但预测与观测几何不一致，且在分布外数据上不可靠。

Method: 将深度估计器视为场景级隐式场，通过测试时优化同时学习深度数据并拟合几何结构。

Result: 跨视角不一致减少63.3%，补全准确率提升23.1%，在室内扫描到卫星图像等多样化场景中达到最先进性能。

Conclusion: NDF通过单次测试时优化即可生成高保真、全局一致的几何结构。
...
### [124] [A Shared Latent for Partially-Labeled Multi-Task Facial Affect Recognition](https://arxiv.org/abs/2607.16285)
*Hong Hai Nguyen,Sy Phan Van,Soo-Hyung Kim,Van-Thong Huynh*

Main category: cs.CV

TL;DR: 提出一种将部分标注多任务学习视为共享情感潜在变量边缘化的方法，在仅37%帧完整标注的s-Aff-Wild2数据集上，通过变分瓶颈耦合三个任务解码器，将表达式宏F1从0.403提升至0.446，并突破动作单元天花板，同时保持效价-唤醒在噪声范围内。


<details>
  <summary>Details</summary>
Motivation: 自然场景中的面部情感是多任务的，但真实语料库仅部分且不均匀地标注这些任务，现有方法通常掩盖缺失标签或填充伪标签，忽略了跨任务信号。

Method: 将部分标注多任务学习视为对共享情感潜在变量的边缘化：一个变分瓶颈调节三个任务解码器，使一个任务标注的帧能为其他任务塑造表示，掩码目标重新出现为证据下界的重建项。

Result: 在s-Aff-Wild2上，表达式宏F1从0.403提升至0.446，动作单元性能突破外部数据无法达到的上限，效价-唤醒保持噪声水平。

Conclusion: 耦合机制有效提升了罕见类别的表示能力，而非损失函数塑造的结果；对AffectNet和RAF-DB的表达式优势转移是依赖特定实验设置的探索性发现，而非决定性结论。
...
### [125] [Identity-Consistent Expression Fields: A Disentangled Neural Radiance Field Framework for Few-Shot Facial Expression Synthesis](https://arxiv.org/abs/2607.16287)
*Minh Tran*

Main category: cs.CV

TL;DR: 提出ICEF框架，通过解耦静态身份辐射与动态表情变形，实现少样本下保身份一致性的面部表情编辑。


<details>
  <summary>Details</summary>
Motivation: 现有少样本动态NeRF方法在远距离表情编辑时会导致身份细节漂移。

Method: 解耦身份辐射与表情变形、身份保持正则化、置信度加权条件特征扭曲。

Result: 在表情参数外推时提升渲染质量和身份一致性。

Conclusion: ICEF有效减少少样本动态NeRF在外推新表情时的伪影。
...
### [126] [A Synthetic 3D Gear Dataset for Manufacturing Quality Inspection (MFGNet-Gear)](https://arxiv.org/abs/2607.16288)
*Ruo-Syuan Mei,Chenhui Shao*

Main category: cs.CV

TL;DR: 提出MFGNet-Gear数据集，含24,000个齿轮的网格与点云，覆盖12种设计、4个质量等级，用于深度学习驱动的3D质量检测。


<details>
  <summary>Details</summary>
Motivation: 数据驱动质量检测需大量标注数据，但缺陷样本稀缺且标注成本高，合成数据可解决该问题。

Method: 利用参数化CAD软件生成齿轮几何，扰动尺寸参数(±0.0254 mm)并采样缺陷形态分布，对每个网格均匀采样100,000点。

Result: 成功创建包含24,000实例的平衡数据集，支持设计分类、缺陷检测等任务。

Conclusion: 该开源数据集可扩展至其他零件，为3D计量深度学习提供可复现的基准。
...
### [127] [Strength-Parity Ensembling with Parameter-Isolated Experts for Multi-Task Affect Recognition](https://arxiv.org/abs/2607.16290)
*Hong Hai Nguyen,Van Thong Huynh*

Main category: cs.CV

TL;DR: 提出强度-对等规则来解释多任务集成中的成员有效性，通过参数隔离在共享骨干网络的不同低秩子空间中训练专家，实现去相关性与准确率平衡，最终在ABAW挑战赛上取得1.7259的总体验证分数。


<details>
  <summary>Details</summary>
Motivation: 现有ABAW挑战赛多任务集成依赖大量模型集成，但缺乏明确标准判断哪些成员对已有强集成有贡献，需要探索集成成员选择原则。

Method: 基于共享情感隐空间联合两个情感监督骨干网络，提出强度-对等规则（成员需与当前集成去相关且个体准确率接近）；通过参数隔离将各专家限制在共享骨干的不同低秩子空间，结合AffectNet适应专家与每个AU校准。

Result: 总体验证分数从基线的0.45提升至1.6949（不含校准），最强调配置通过共享隐空间头部的价效副产品池化达到1.7259。

Conclusion: 参数隔离可有效解决单骨干多次微调导致预测高度相关（0.98）的问题，使专家保持去相关（0.91）且准确率相近，是提升多任务集成性能的关键策略。
...
### [128] [Beyond Target Scores: Measuring Off-Target Drift in Diffusion-Based Medical Image Editing](https://arxiv.org/abs/2607.16291)
*Todd Zhou*

Main category: cs.CV

TL;DR: 提出了CIB-Med-1基准，用于评估胸部X光片中的受控生物标志物编辑，并发现扩散模型存在奖励黑客问题；同时，提出了一种约束扩散引导方法，在保持目标进展的同时减少语义漂移。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像编辑评估过于狭窄（仅关注目标分数是否增加），忽略了与目标相关的非目标发现（如合并症、采集效应）导致模型可能通过修改非目标特征而看似成功。

Method: 构建了CIB-Med-1轨迹级基准，评估胸腔积液编辑的定向目标进展、反转率和14个临床相关干扰轴的语义漂移；提出受限扩散引导方法，在保证目标进展的同时约束偏离目标的改变。

Result: 约束编辑器保持了目标进展（ρ_trend=0.88 vs. 0.90），并将中位离目标漂移从0.46降至0.20，90分位漂移从0.98降至0.33；人类验证中与预期进展序列的一致性更高（τ=0.61 vs. 0.29）。

Conclusion: 医学图像编辑应作为轨迹级语义控制进行评估，而非端点分数最大化。
...
### [129] [It Depends on the Dataset: When a Brain-Encoding Model's Predicted Responses Beat Their Visual Backbone for Video Memorability](https://arxiv.org/abs/2607.16292)
*Carson Rodrigues*

Main category: cs.CV

TL;DR: 通过脑编码基础模型预测fMRI反应来预测视频记忆性，但效果因数据集而异。


<details>
  <summary>Details</summary>
Motivation: 探索无扫描仪条件下预测的脑反应能否作为下游任务（视频记忆性预测）的有用特征。

Method: 将视频片段投射到TRIBE v2预测的皮质空间，使用岭回归预测短期记忆性，并与模型自身的视觉骨干网络比较。

Result: 在Memento10k数据集中视觉骨干网络胜出（Spearman 0.594 vs 0.544），在VideoMem数据集中脑投射胜出（0.415 vs 0.368），跨数据集转移表现分裂。

Conclusion: 预测的脑特征携带了视觉骨干网络在特定数据集上缺失的记忆性信号，但非领域通用先验，而是数据集特定的表示。
...
### [130] [SLT: Robust Quantum Neural Networks for Noisy-Label Medical Image Classification via Supermartingale-based Label Transition](https://arxiv.org/abs/2607.16293)
*Jun Zhuang,Mohammad Al Hasan,Yiyu Shi,Chaowen Guan*

Main category: cs.CV

TL;DR: 提出基于超鞅的标签转移框架（SLT），用于解决小样本医疗图像分类中的噪声标签问题，结合量子神经网络提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 量子神经网络在有限数据下具有潜力，但其自然平滑性在噪声标签学习中导致高置信样本混淆，影响噪声转移估计。

Method: 将预测分布的熵降建模为超鞅过程，利用单调性识别稳定的转移矩阵更新步，实现动态噪声转移修正并减少振荡。

Result: 在多个小规模医疗图像数据集上，SLT显著提升QNN分类性能，在合成和真实噪声下稳定优于经典方法。

Conclusion: SLT框架有效解决了QNN在噪声标签下的学习瓶颈，提供了收敛性保证，并展现了实际应用潜力。
...
### [131] [A${}^2$BM: Alignment-Aware Bridge Matching for Image-to-Image Translation](https://arxiv.org/abs/2607.16294)
*Aimi Okabayashi,Georges Le Bellier,Nicolas Audebert,Charlotte Pelletier,Thomas Corpetti,Nicolas Courty*

Main category: cs.CV

TL;DR: 提出了一种对齐感知桥接匹配方法（A²BM），通过利用图像对的弱对齐信息，在训练中集成对齐分数，使模型能够区分语义对应与对齐伪影，从而在弱对齐数据下提升图像转换的保真度。


<details>
  <summary>Details</summary>
Motivation: 现实应用中图像对常因采集条件变化（如异步拍摄、不同光照等）而存在弱对齐，但现有桥接匹配方法假设完美对齐的训练对，无法处理此类弱对齐问题。

Method: 在训练过程中引入对齐分数，使模型学习表示对齐信息，并在推理时将对齐分数作为控制变量，通过设定最高分数获得强对齐输出。

Result: 在合成实验和真实任务（跨传感器超分辨率、无监督域适应）中，A²BM 一致优于基于 GAN、扩散模型和薛定谔桥的基线方法。

Conclusion: 对齐条件化是为弱对齐数据提供图像转换模型的一个原则性解决方案。
...
### [132] [Emergent Hierarchical Monosemantic Neurons from the Group-Contrastive Forward-Forward Algorithm](https://arxiv.org/abs/2607.16295)
*Yiming Tang,Qinglin Qi,Zhaoqian Yao,Harshvardhan Saini,Dianbo Liu*

Main category: cs.CV

TL;DR: 提出一种名为组对比前向-前向（GCFF）的生物启发式训练算法，通过架构约束而非稀疏性实现单语义神经元，能捕捉非线性概念并随深度增加抽象层次。


<details>
  <summary>Details</summary>
Motivation: 稀疏字典学习方法存在非可识别性、依赖线性表示假说以及无法表达非线性编码概念等局限性，而生物视觉系统通过局部分层学习规则自然形成高度选择性神经元，启发探索生物合理学习算法实现单语义性。

Method: 采用前向-前向训练算法，结合类别特定路由与类内对比目标，通过多层非线性网络捕获非线性概念，无需稀疏性约束。

Result: 在CLIP表示上，单个GCFF模块无需稀疏性约束或抽象层次监督，即可恢复随深度递增抽象的单语义神经元，且能抓住独立于图像前景的环境属性；从头训练时在多个图像分类基准上达到前向-前向算法最优性能。

Conclusion: 生物合理的学习算法可通过架构约束而非稀疏性实现单语义神经元，为非线性概念编码提供新路径。
...
### [133] [Rethinking Feature Reliance Evaluation with Semantically Matched Suppression](https://arxiv.org/abs/2607.16298)
*Ning Jiang,Tianyi Luo,Zhengyong Huang,Yao Sui*

Main category: cs.CV

TL;DR: 论文提出语义匹配的评估框架，发现ImageNet训练的CNN更依赖纹理，而ViT在纹理和形状抑制下均保持更高精度，且与人类视觉皮层更兼容。


<details>
  <summary>Details</summary>
Motivation: 以往冲突实验暗示CNN有纹理偏好，但该方法仅测量人工冲突下的偏好，无法反映自然识别中的特征依赖。

Method: 构建语义匹配框架，通过控制类别可分性损失来对比形状和纹理抑制的效果。

Result: 在匹配框架下，CNN在纹理抑制下性能下降更严重；ViT在两种抑制下均优于CNN；ViT的神经预测性能下降更小。

Conclusion: 语义可比性对解释特征依赖至关重要，ViT的鲁棒优势可能源于其更接近人类视觉皮层的表征。
...
### [134] [FedDP-PALD: A Privacy-Preserving Federated Latent Diffusion Framework with Prototype Aggregation for Medical Data Synthesis](https://arxiv.org/abs/2607.16300)
*Md. Sajeebul Islam Sk.,Khan Enaet Hossain,Md. Mehedi Hasan Shawon*

Main category: cs.CV

TL;DR: 提出FedDP-PALD框架，在差分隐私保证下联合生成多模态医学合成数据，有效抵御成员推理攻击同时保持诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法存在隐私泄露风险（训练过程中交换信息可推断患者数据是否被使用）且合成数据无法保留预测结构，限制临床应用。

Method: 使用门控多头注意力机制与模态可用性掩码联合处理X光和心电图信号，提出差分隐私原型混合聚合方法，对类级潜在原型裁剪并添加校准高斯噪声。

Result: 差分隐私将汇总级攻击AUROC从0.6229降至0.5016-0.5093（ε=1-8），合成潜在训练F1分数0.8993、AUROC 0.9057，接近真实潜在训练的0.9747、0.9132。

Conclusion: FedDP-PALD能生成保留决策性能的私有合成表示，同时强抵抗成员推理攻击。
...
### [135] [Med-OPD: Improving Medical Vision-Language Models via Evidence-Aware On-Policy Distillation](https://arxiv.org/abs/2607.16303)
*Yunhang Qian,Jiaquan Yu,Jiawei Liu,Meng Wang,Hongwei Bran Li,Xiaobin Hu*

Main category: cs.CV

TL;DR: 提出Med-OPD，一种结合在线策略蒸馏与医学证据感知监督的统一后训练框架，通过医学证据优势（MEA）信号增强视觉证据关注，提升Med-VLMs的诊断可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有Med-VLMs依赖语言先验或模板生成答案，而非真正关注关键视觉证据；标准在线策略蒸馏均匀处理所有词元，稀释了证据依赖的关键信号。

Method: 引入医学证据优势（MEA），通过教师模型在原始与退化图像下的似然差异衡量词元对视觉证据的依赖，并据此重新分配蒸馏信号（词元级与轨迹级），突出诊断关键词元与证据依赖轨迹。

Result: 在OmniMedVQA子集（CT、MRI、疾病诊断、病变分级）上，Med-OPD持续优于SFT和标准在线策略蒸馏。

Conclusion: 证据感知蒸馏能有效增强Med-VLMs对关键视觉证据的依赖，提升多模态医学推理的可靠性。
...
### [136] [Dual-Domain Self-Supervised Artifact Removal Framework for Photoacoustic Computed Tomography](https://arxiv.org/abs/2607.16304)
*Yucheng Zhou,Shuang Li,Yu Zhang,Yibing Wang,Chulhong Kim,Seongwook Choi,Changhui Li*

Main category: cs.CV

TL;DR: 提出基于双域特征的自监督伪影去除框架，有效抑制光声计算层析成像中的稀疏检测伪影。


<details>
  <summary>Details</summary>
Motivation: PACT在稀疏检测条件下常因重建伪影影响图像质量，现有方法难以兼顾伪影抑制与计算效率。

Method: 利用反投影与傅里叶重建算法的伪影差异，构建轻量孪生神经网络，结合跨域保真度与不确定性加权一致性的复合损失函数，实现双域特征解耦与伪影过滤。

Result: 仿真、体模、大鼠及人体实验数据验证显示：该方法显著抑制伪影，且通过空间域与频域逆算子加速，实现高效计算。

Conclusion: 提出的端到端方法在伪影消除和计算效率上均表现优异，适用于临床PACT成像。
...
### [137] [LookME: Lookup-Based Multimodal Embeddings for Layer Injection in Vision-Language Models](https://arxiv.org/abs/2607.16305)
*Zeyu Xu,Xingzhong Hou,Pengkai Guo,Siling Lin,Xiao Xu,Menghua Zhai,Haoyu Chen,Yunke Zhang,Fei Huang*

Main category: cs.CV

TL;DR: 提出了LookME，首个支持多模态嵌入查找增强的框架，通过层次化查找和稀疏注入策略，在VLMs中实现高效存储和性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的PLE方法主要针对文本嵌入设计，难以有效处理多模态嵌入的丰富信息；同时，密集或稀疏MoE模型在资源受限环境中面临内存和延迟的权衡。

Method: 采用场景级到原语级的层次化两级查找方法；集成稀疏注入策略，自适应优先关键嵌入并支持跨层重用嵌入表。

Result: 在多个视觉基准上，LookME优于纯文本PLE方法，验证了多模态嵌入查找增强的有效性。

Conclusion: LookME通过查找增强和高效存储设计，在VLMs中实现了更好的效率、模型大小和性能权衡。
...
### [138] [CoBind: Stage-Aware Compositional Binding for Training-Free Text-to-Image Generation](https://arxiv.org/abs/2607.16307)
*Kaijie Chen,Ethan Caldwell,Mira Vossen,Julian Hartwell,Serena Whitlock,Adrian Bellamy*

Main category: cs.CV

TL;DR: 提出CoBind，一种无需训练的框架，通过阶段感知的组合绑定方法，在复杂文本到图像生成中改进属性绑定、空间关系和多实体生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散文本到图像模型在处理多实体、属性和关系的复杂提示时经常失败，且现有无训练方法未显式建模属性归属和分阶段约束。

Method: 解析提示为组合图，先通过实体完整性和关系约束建立全局布局，再通过对比跨实体优化为实体绑定属性，逐步放松结构指导以保留细节，并根据约束满足度自适应调整指导强度。

Result: 在T2I-CompBench++、GenEval和多种扩散骨干网络上的实验显示，在属性绑定、空间关系和复杂组合生成上持续改进，同时保持竞争力视觉质量。

Conclusion: CoBind有效提升复杂文本到图像生成的组合准确性，无需重训练或额外标注。
...
### [139] [DAUPNet: Domain-Aware Uncertainty Modeling for Reliable Prototype Discrimination in Cross-Domain Few-Shot Semantic Segmentation](https://arxiv.org/abs/2607.16308)
*Lei Yuan,Zhongxu Hu,Jingyi Wen,Pengxing Yi*

Main category: cs.CV

TL;DR: 提出DAUPNet框架，通过不确定性感知的原型判别来解决跨域小样本语义分割中域偏移导致的原型匹配不可靠问题。


<details>
  <summary>Details</summary>
Motivation: 现有跨域小样本语义分割方法在域偏移下原型匹配不可靠，表现为层次响应不一致、原型无法表达边界和外观模糊性、不同可靠性原型在优化中平等对待。

Method: 统一框架：1) 协调层次支持-查询特征提供稳定证据；2) 概率化表示前后景原型；3) 利用估计的不确定性调节对比优化。

Result: 在四个目标域上，1-shot和5-shot设置平均mIoU分别达72.6%和76.7%，在医学域尤其显著。

Conclusion: 建模原型不确定性并将其融入优化提供了鲁棒且可解释的跨域小样本语义分割方法。
...
### [140] [SaaF: Scene-Specific Ambiguity-Aware 3D Language Fields towards Interactive Real-World Object Retrieval](https://arxiv.org/abs/2607.16309)
*Yuga Yano,Daiju Kanaoka,Hakaru Tamukoh,Yasutomo Kawanishi*

Main category: cs.CV

TL;DR: 提出SaaF，一种基于高斯泼溅的3D语言场，通过度量学习和多标签训练增强实例判别性和模糊感知性，提升开放词汇下交互式对象检索的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有3D语言场方法因特征压缩导致相似对象判别性下降，且难以处理模糊查询，引起检索不稳定或错误。

Method: 引入度量学习构建统一特征空间：多视角图像特征拉近增强实例判别性；联合训练多文本标签（含模糊描述）学习语义关系以感知模糊性。

Result: 在开放词汇场景下，SaaF不仅提高了检索精度，还能稳健检测并处理用户文本查询中的歧义。

Conclusion: SaaF通过场景特定的模糊感知特征空间，有效提升了3D语言场在交互式对象检索中的性能。
...
### [141] [Seeing What Is Actually There: PriVE-Bench and PriVE-Tools for Counterfactual Evaluation of Agentic Visual Evidence in VLMs](https://arxiv.org/abs/2607.16311)
...
TL;DR: 提出PriVE-Bench基准和PriVE-Tools方法，研究视觉语言模型在反事实图像中是否依赖先验知识，并评估工具增强的视觉证据能否帮助模型更基于图像推理。


<details>
  <summary>Details</summary>
Motivation: 现有反事实基准主要检测先验跟随行为，但未探讨额外视觉证据（如工具提供的边界框、裁剪等）是否帮助模型克服先验。

Method: 构建PriVE-Bench使用配对原始与反事实图像；PriVE-Tools扩展工具化视觉证据（边界框、裁剪、缩放面板、轮廓），对比原始、配对图像和工具条件输入下的准确率、先验错误率等。

Result: 视觉证据工具在某些设置（尤其是局部证据有效时）可提升表现，但非通用解决方案；多模型在明确提供相关视觉证据后仍依赖语言和类别先验。

Conclusion: 工具化视觉证据部分缓解先验依赖，但不足以完全消除该问题，模型仍需改进对像素证据的利用能力。
...
### [142] [xperception -- Making Robotic Grasping Easier](https://arxiv.org/abs/2607.16312)
*Matteo Bortolon,Andrea Caraffa,Alice Fasoli,Fabio Poiesi*

Main category: cs.CV

TL;DR: 提出零样本6D位姿估计技术xperception，无需对象微调，直接使用CAD模型和基础模型特征，实现毫米级精度，并通过BOP挑战赛验证。


<details>
  <summary>Details</summary>
Motivation: 高混合低产量制造需要灵活机器人操作，但传统视觉系统对新物体需大量数据收集和模型重训练，存在刚性瓶颈。

Method: 基于FreeZe算法，集成基础模型（如DINOv2, GeDi）的丰富语义特征，直接利用典型CAD模型进行零样本6D位姿估计。

Result: 在工业任务（如料箱拣选）中对严重遮挡具有鲁棒性，达到毫米级精度，并在工业边缘硬件（如NVIDIA Jetson Thor）上部署，技术成熟度6级。

Conclusion: xperception为高混合低产量制造中可扩展、即插即用的机器人自动化提供了新路径，消除了物体特异性微调和数据标注需求。
...
### [143] [Eddy-VL 1.9B: Structural Pruning and Layered Distillation for Edge-Deployable Multimodal Embedding](https://arxiv.org/abs/2607.16316)
*HanYeong Cho,Changwoo Kim,Taeuk Chu,Jimin Park*

Main category: cs.CV

TL;DR: 本文介绍了Eddy-VL 1.9B，一种压缩的多模态嵌入模型，专为离线、边缘部署的视觉语言检索而设计。


<details>
  <summary>Details</summary>
Motivation: 在无法使用云API且对低延迟至关重要的气隙取证和调查环境中，需要一种高效的多模态检索模型。

Method: 采用基于探测的结构性剪枝（CKA排名删除冗余层）和分层知识蒸馏（含Matryoshka维度损失）进行压缩。

Result: 模型参数量减少9.5%，MMEB-V2上保留教师模型91.7%性能，延迟降低约10%。

Conclusion: Eddy-VL在受约束的边缘部署下实现了有效的多模态检索，性能接近教师模型。
...
### [144] [Depth-Regularized JEPA World Models Learn More Transferable Representations from Real Outdoor Robot Data](https://arxiv.org/abs/2607.16314)
*Usman M. Khan*

Main category: cs.CV

TL;DR: 该论文提出通过深度几何先验（SIGReg正则化）和训练时过参数化，提升JEPA世界模型在真实户外机器人视频数据上的鲁棒性和泛化能力，在不增加推理时间下使视觉里程计误差降低33%。


<details>
  <summary>Details</summary>
Motivation: 现有基于JEPA的世界模型在预测真实环境（尤其户外复杂场景）的鲁棒动态时仍存在困难，缺乏有效利用几何先验来应对视觉复杂性的方法。

Method: 在训练过程中引入深度监督作为几何先验，结合各向同性诱导潜在正则化器（SIGReg）最大化任务无关的潜在多样性，同时通过训练时过参数化增加模型复杂度而不影响推理速度。

Result: 与基准LeWM相比，视觉里程计误差降低33%，领域内和域外基准的惊喜分数分离显著提升，多步潜变量滚动保真度在域迁移下随步长增大而改进，并间接提升了光照和阴影等非直接3D几何的物理理解。

Conclusion: 轻量级的几何先验（深度）能有效提升紧凑JEPA世界模型在真实户外数据上的表征质量与迁移能力，且不增加推理开销，为世界模型泛化提供了物理驱动的新思路。
...
### [145] [Deep Learning-based Filtering for Video Coding: A Survey on Architectures, Algorithms, and Complexity Analysis](https://arxiv.org/abs/2607.16319)
*Young-Woon Lee,Byung-Gyu Kim*

Main category: cs.CV

TL;DR: 本文全面调查了面向硬件的深度学习滤波技术，旨在弥合学术研究与消费电子设备实际部署之间的差距。


<details>
  <summary>Details</summary>
Motivation: UHD显示和沉浸式媒体服务对高效视频编码需求巨大，但深度学习滤波在计算、内存和功耗方面受限，难以部署于CE设备。

Method: 提出三维分类法（集成方案、编码信息利用、网络设计策略），从率失真性能和硬件可行性角度分析，并纳入JVET标准化活动。

Result: 分析了从性能导向到轻量级、硬件友好架构的演变，识别了实时推理延迟和错误传播等挑战。

Conclusion: 为下一代CE设备中鲁棒、低功耗智能视频编码提供了路线图。
...
### [146] [Monte Carlo Dropout Uncertainty and Entropy-Thresholded Selective Prediction for Architecture-Agnostic Brain Tumor MRI Triage](https://arxiv.org/abs/2607.16317)
*Medhansh Sharma*

Main category: cs.CV

TL;DR: 提出了一种基于MC Dropout的不确定性优先管道，用于四级脑肿瘤MRI分类，通过熵值规则将不确定病例转诊给放射科医生，在内部验证中实现了校准和可操作的延迟决策。


<details>
  <summary>Details</summary>
Motivation: 临床应用中模型置信度不可靠，确定性估计无法指示分类是否正确，需要一种可信任的机制来标记可能误诊的病例并转诊给人。

Method: 使用MC Dropout（T=20次）从模型中读取预测熵，基于熵值设计显式转诊规则；通过感知哈希聚类划分数据集（7200张图像），避免近似重复泄漏；在ViT-B/16和ResNet-50上评估五个种子，分析判别力、校准度和选择性预测。

Result: 两种模型均表现强劲（宏AUC 0.994，准确率0.962-0.964），无显著差异；温度缩放将确定性softmax的预期校准误差降至0.016-0.020；转诊最不确定的5%病例后，其余病例准确率升至约0.98（风险覆盖曲线下面积0.010-0.011）。

Conclusion: MC Dropout不确定性在此任务中校准良好、非崩塌且可通过具体转诊规则直接应用，为内验证的脑肿瘤MRI分诊提供了与架构无关的校准基础。
...
### [147] [The Devil is in the Dark Pixels: Toward Brightness Bias-Robust Denoising](https://arxiv.org/abs/2607.16320)
*Sungjun Cho,Zhuangzhuang Chen,Xiaomeng Li*

Main category: cs.CV

TL;DR: 本文发现并解决了图像去噪中一个被忽视的问题：在信号依赖的相机噪声模型下，暗区信噪比低，而MSE训练的去噪器反而加剧了暗区恢复困难。提出的BBRD方法通过亮度分组、误差归一化和群体分布鲁棒优化，在无额外参数和推理成本下，显著提升了所有亮度区域的去噪性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像去噪方法在信号依赖噪声模型下对暗区恢复效果差，MSE损失会加剧暗区像素重建的偏差，导致暗区性能远低于亮区。

Method: 提出BBRD，作为MSE损失的替代方案：将像素按亮度分成多个波段，用每个波段的经验噪声方差归一化误差，然后应用群体分布鲁棒优化动态加权当前最差波段，无额外参数和推理成本。

Result: 在8种架构和2个数据集上，相比13种对比方法，BBRD是唯一同时提升所有亮度波段性能的方法，在SIDD数据集上暗区PSNR提升0.45 dB、亮区提升0.32 dB、总体提升0.65 dB，暗区增益最大。
...
### [148] [Art Beyond Semantics: Sheaf-Informed Contrastive Learning for Multi-Relational Representations](https://arxiv.org/abs/2607.16321)
*Ludovica Schaerf,Antonio Purificato,Piera Riccio,Fabrizio Silvestri,Noa Garcia*

Main category: cs.CV

TL;DR: 本文提出 CANVAS 框架，基于层论学习多关系对齐的多模态表征，解决 CLIP 等模型将艺术理解中多样语义关系压缩为单一对齐的问题，在三个新基准上优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 如 CLIP 通过单一共享嵌入空间丢失了艺术史推理中风格、图像志、历史语境等多重语义关系。

Method: 基于层论，将每件作品投影到多个关系条件嵌入，并用新对比损失在训练时编码上下文信息，推理时无需外部数据。

Result: 在多模态检索和艺术理解任务中，CANVAS 在 WikiArt+、HertzianaDP 和 SemArt+ 三个基准上均优于基线方法。

Conclusion: 多关系对齐不仅在理论上合理，实践中也至关重要。
...
### [149] [GMoT: Gated Motion-Aware Tokenization for Fine-Grained Micro-Gesture Video Reasoning with Multimodal LLMs](https://arxiv.org/abs/2607.16322)
*Taorui Wang,Wei Xia,Hui Ma,Zijia Song,Jiayu Zhang,Zeheng Wang,Yong Xu,Zitong Yu*

Main category: cs.CV

TL;DR: 提出GMoT模块和渐进式奖励引导策略，提升微动作识别与推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型难以捕捉细微运动，常依赖静态姿态先验。

Method: Gated Motion-Aware Tokenization模块提取稀疏运动证据，结合渐进式奖励引导策略和半监督标注。

Result: 在iMiGUE和SMG上分别达67.32%和73.11%的Top-1准确率，优于Qwen3-VL-8B基线。

Conclusion: GMoT有效提升微动作识别精度与解剖推理能力。
...
### [150] [SGMCE: Segment-Grounded Morphological Concept Explanation for Malaria Parasite Species Identification in Thick Blood Smears](https://arxiv.org/abs/2607.16324)
*Ahmed Tahiru Issah,Charles B. Delahunt,Carine Mukamakuza*

Main category: cs.CV

TL;DR: SGMCE 框架为厚血膜疟疾显微检测生成基于分割的形态学自然语言解释，无需额外训练或标注，在737个检测上平均KBC为0.91，DS为0.99，CCF为0.97。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习疟疾检测器缺乏形态学证据，限制了显微镜检查人员在病例层面审核预测的能力。

Method: 提取掩码引导的裁剪缩略图，计算14种手工计算机视觉形态特征，结合世界卫生组织知识库查询GPT-4o生成解释，并通过四种自动指标验证。

Result: 在139张厚血膜图像上的737个检测中，寄生虫类别平均KBC为0.91，DS为0.99，CCF为0.97。

Conclusion: SGMCE 框架能有效生成忠实于形态学测量的结构化解释，辅助疟疾诊断审计。
...
### [151] [RegionFM: Interpretable Region-Based Brain MRI Classification Using Foundation Model Embeddings](https://arxiv.org/abs/2607.16325)
*Wei Zhang*

Main category: cs.CV

TL;DR: 提出RegionFM框架，通过解剖分割与基础模型嵌入结合，实现脑MRI分析的解剖可解释性，在保持性能的同时提供区域级解释。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型预测难以在解剖意义上解释，而临床评估依赖区域异常，常规解释方法仅产生体素/斑块级重要性图，无法量化脑区贡献。

Method: 先分割MRI为解剖区域并构建独立体素，用冻结基础模型编码各区域嵌入，再通过区域加性逻辑模型组合嵌入，使每个脑区贡献明确的标量项。

Result: 在认知障碍分类任务中，RegionFM性能与微调方法相当，提供解剖基础的解释；随机嵌入消融实验性能接近随机，表明预测依赖有意义的结构特征。
...
### [152] [CRISP: Pre-LLM Yet Text-Driven Visual Token Pruning for Efficient LVLM Inference](https://arxiv.org/abs/2607.16326)
*Xu Li,Yi Zheng,Mengyang Zhao,Yuxuan Liang,Zhe Liu,Rui Zhu,Xiaolei Chen,Wei Zhou,Baoquan Zhao,Juncen Guo*

Main category: cs.CV

TL;DR: 提出了CRISP，一种在进入LLM之前进行文本驱动的视觉token剪枝框架，显著减少推理开销同时保留关键信息。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法要么在LLM前做文本无关剪枝，要么在LLM内部剪枝但效率低且受噪声影响，缺乏高效的文本驱动方案。

Method: 两阶段流水线：阶段1识别文本对齐的视觉token，阶段2通过语义多样性增强上下文完整性。

Result: 在LLaVA-1.5和LLaVA-NeXT上，CRISP在激进剪枝下保持99.5%准确率，同时降低2倍以上推理成本和延迟。

Conclusion: CRISP是一种高效、实用的LVLM推理加速方案，特别适用于资源受限场景。
...
### [153] [Localization-Infused Vision-Language Semantic Fusion for Text-Guided Medical Image Segmentation](https://arxiv.org/abs/2607.16327)
*Songyue Han,Mingye Zou,Shuchang Ye,Lei Bi,Mingyuan Meng*

Main category: cs.CV

TL;DR: 提出了一种融合定位信息的视觉-语言框架（LoG），通过多尺度目标定位任务显式提取文本中的位置语义，并实现三级定位融合策略，在三个医学影像数据集上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导分割方法未能显式捕获文本报告中的目标位置信息，且缺乏多层级信息融合策略，限制了关键语义的提取与整合。

Method: LoG框架联合执行多尺度目标定位任务，实现三级定位融合：定位引导特征融合、定位门控注意力融合、定位约束损失融合。

Result: 在QaTa-COV19、MosMedData+和Kvasir-SEG数据集上分别获得91.59%、80.71%和94.59%的Dice分数，超越现有方法。

Conclusion: 显式捕获目标位置信息的视觉-语言融合能显著提升文本引导医学图像分割的性能。
...
### [154] [Local Brushstroke Quality Assessment via Vision-Language Feedback](https://arxiv.org/abs/2607.16330)
*Mio Mitamura,Hirokatsu Kataoka*

Main category: cs.CV

TL;DR: 论文研究了多模态大语言模型在书法评估中的应用，发现GPT-4o在绝对评分准确性上表现最佳，但所有模型与人类专家的排名相关性均不显著；RAG改善排名相关性但降低绝对准确性。


<details>
  <summary>Details</summary>
Motivation: 探索多模态大语言模型能否评估书法的局部笔画质量并生成有用的教育反馈。

Method: 构建评估框架，使用三个多模态大模型（GPT-4o、Claude Sonnet 4、Gemini 2.5 Flash）对书法作品的前后对比图像进行五点评分，并与三位专家评分比较；同时测试Claude的检索增强生成变体。

Result: GPT-4o在绝对准确度上最优（MAE=0.885），但所有模型与专家排名的肯德尔tau系数均不显著；RAG改善了排名相关性但降低了绝对准确性。

Conclusion: 多模态大模型在书法评分绝对准确性上可用，但缺乏与人类一致的排序能力；文本规则注入（RAG）在效果上存在负面结果。
...
### [155] [DMFNet: Dual-Backbone Multiscale Fusion Network for Urban Scene Classification](https://arxiv.org/abs/2607.16338)
*Anamitra Ghosh,Abhiroop Chatterjee,Susmita Ghosh*

Main category: cs.CV

TL;DR: 提出了一种双骨干多尺度特征融合框架DMFNet，结合残差特征传播和空间注意力，用于遥感场景分类，在AID数据集上达到97.46%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效捕捉多尺度特征交互，以及从具有高类内变异性和类间相似性的复杂航拍场景中学习鲁棒特征表示。

Method: 使用两个预训练骨干网络提取层次特征，引入多尺度特征融合机制（残差传播）增强交互，添加空间注意力模块强调信息区域，并采用两阶段训练策略（冻结骨干后选择性微调）。

Result: 在AID基准数据集上，平均准确率为97.46% ± 0.14%；消融实验验证了各组件的有效性。

Conclusion: DMFNet通过双骨干、多尺度融合、空间注意力和两阶段训练，有效提升了遥感场景分类性能。
...
### [156] [Clarify Before Executing: A Self-Evolving Agent for Resolving Intent Asymmetry in 3D Tool Orchestration](https://arxiv.org/abs/2607.16352)
*Xiaoye Zhu,Weixin Li,Junan Huo,Bozhong Wang,Jia Zeng,Yi Yang,Cen Chen,Qi Liu*

Main category: cs.CV

TL;DR: 提出CLARE，一个澄清感知的进化3D智能体，通过解耦生成的四个认知角色来主动澄清模糊指令，并使用多轮奖励机制进行策略优化，显著提升3D任务执行成功率。


<details>
  <summary>Details</summary>
Motivation: 现有3D工具链需要精确参数，但用户输入通常模糊且未明确指定，现有3D智能体将其视为噪声而盲目执行。

Method: CLARE将生成流水线解耦为四个专门认知角色，通过多轮交互自我进化澄清策略，并利用多轮奖励优化交互效率与任务完成之间的平衡。

Result: 在3D-Clarify基准上，CLARE在单步和多步任务上分别达到60.40%和43.34%的成功率，超过基线两倍以上。

Conclusion: 主动澄清是实现鲁棒3D执行的关键，CLARE证明了此方法的有效性。
...
### [157] [Privacy-Aware Synthetic Video Benchmarking and Relational Evaluation for Worker-Under-Suspended-Load Detection](https://arxiv.org/abs/2607.16351)
*Anshu Singh,Alejandro Seif*

Main category: cs.CV

TL;DR: 提出SynthSite合成视频基准，用于研究施工中'工人处于悬挂载荷下'这一关系性危险，并评估不同隐私保护方法对下游危险识别的影响。


<details>
  <summary>Details</summary>
Motivation: 公开可共享的施工视频基准稀缺，尤其针对安全关键但罕见、危险且难以发布的危险场景。

Method: 创建SynthSite基准（55个合成视频），采用混合生成工作流；测试五种全身隐私条件对工人保留、定位稳定性和危险识别的影响。

Result: 结构保留的模糊方法比外观平滑基线保留更多下游效用；保留原始视觉参考并不能保证与人类危险标签的最佳一致性。

Conclusion: 施工安全分析的隐私评估应同时考虑外观抑制和危险推理所需几何线索的保留。
...
### [158] [PhysAgent: Reflective Agentic Physics Control for Physically Plausible Video Generation](https://arxiv.org/abs/2607.16355)
*Qirui Li,Jinkun Hao,Yibo Li,Ran Yi,Paul L. Rosin,Yu-Kun Lai*

Main category: cs.CV

TL;DR: 提出PhysAgent框架，通过反射式代理循环实现物理程序生成、仿真、验证与修复，生成更物理合理的视频。


<details>
  <summary>Details</summary>
Motivation: 现有方法单次预测物理规范常无法准确翻译用户意图，尤其在细粒度物体动力学、复杂运动轨迹及时序交互中失败。

Method: 设计反射式代理框架，包含物理程序生成、物理仿真、分阶段验证及目标程序修复，并引入物理控制API。

Result: PhysAgent生成更物理合理的视频，提示对齐更好，在不同物理场景中泛化更有效。

Conclusion: 反射式代理循环能有效提升物理视频生成的准确性与稳定性。
...
### [159] [Spatial Transport of Integration Error in Generative ODEs](https://arxiv.org/abs/2607.16361)
*Songheng Yin*

Main category: cs.CV

TL;DR: 本文研究了扩散模型在少步采样中的积分误差分布，发现误差主要通过模型动力学传播，而非局部注入。


<details>
  <summary>Details</summary>
Motivation: 扩散模型常用少步求解器，但积分误差分布不均匀，研究者希望理解误差的来源和传播机制。

Method: 使用扰动实验、符号源-传输分析、线性化动力学重建、结构破坏消融以及训练惩罚等方法。

Result: 模型动力学将局部扰动广泛传播，少步误差主要由远程传输贡献；速度场的变化可部分预测区域误差。

Conclusion: 误差传播机制可解释，且通过训练惩罚可减少少步误差，表明模型结构可被优化改变。
...
### [160] [OmniStyle-INR: Universal and Multimodal Style Transfer for INRs](https://arxiv.org/abs/2607.16362)
*Rafał Kajca,Michał Miziołek,Kornel Howil,Rafał Tobiasz,Przemysław Spurek*

Main category: cs.CV

TL;DR: 提出OmniStyle-INR框架，利用隐式神经表示实现多种视觉模态（2D/3D/视频/4D）的文本与图像引导风格迁移。


<details>
  <summary>Details</summary>
Motivation: 高斯泼溅在表示视频和2D图像时需接近总像素数的高斯体，结构次优性促使探索更自然的隐式神经表示方法。

Method: 基于网络连续表示（隐式神经表示）构建通用域，同时支持文本提示和视觉示例引导的跨模态风格迁移。

Result: 在2D图像、3D场景、视频和4D动态数据上均实现高质量风格迁移。

Conclusion: 隐式神经表示作为统一数据处理域在风格迁移任务中具有优越的压缩、超分辨率和生成模型兼容性优势。
...
### [161] [Disentangling Model and Human Data Uncertainty in Apparent Facial Age Estimation](https://arxiv.org/abs/2607.16378)
*Andrei Foitos,Ivo Pascal de Jong,Matias Valdenegro-Toro*

Main category: cs.CV

TL;DR: 本文通过贝叶斯神经网络（BNN）在APPA-REAL数据集上估计人脸表观年龄，并量化了认知不确定性和偶然不确定性。研究发现，偶然不确定性随数据集大小保持稳定，而认知不确定性随训练数据减少而增加。


<details>
  <summary>Details</summary>
Motivation: 人脸表观年龄估计受主观感知和数据变异性影响，现有研究缺乏对不确定性来源（认知与偶然）的量化分析，因此本文旨在探索不同不确定性在年龄估计中的角色。

Method: 使用APPA-REAL数据集，训练三种BNN近似方法（MC-DropConnect、Flipout和Deep Ensembles），结合人类提供的偶然不确定性标签，输出预测年龄及两类不确定性。

Result: 偶然不确定性在不同数据集大小下保持稳定，认知不确定性随训练数据减少而显著增加。

Conclusion: 人脸年龄估计中可量化不同来源的不确定性，验证了理论假设，为模型鲁棒性分析提供了新视角。
...
### [162] [Apple-$π$: Benchmarking Thinking with Video Towards Law-Grounded Physical Intelligence](https://arxiv.org/abs/2607.16401)
...
TL;DR: 提出Apple-PI基准，通过物理定律对视频生成模型进行阶段性诊断，发现现有模型远未达到可靠的物理世界模拟器水平。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估视频输出的物理合理性，未检验模型是否通过基于物理定律的忠实推理过程得出结果。

Method: 构建包含3部分的Apple-PI基准：Orchard数据集（400个经典力学视频）、三阶段科学推理协议（感知-公式化-演绎）、混合评估套件（MLLM主观评分+物理客观指标）。

Result: 11个模型中最佳视频模型得分仅0.473，揭示了感知到公式化再到演绎的瓶颈、多定律状态转移弱、以及持续的模拟到真实差距。

Conclusion: Apple-PI可作为引导未来视频模型向具有物理定律基础的世界模型发展的诊断基础。
...
### [163] [Think, Plan, Paint: Layout-Aware Reasoning for Controllable Image Generation in Unified Models](https://arxiv.org/abs/2607.16409)
*Junhao Liu,Jian-Wei Zhang,Tao Huang,Miles Yang,Zhao Zhong,Liefeng Bo*

Main category: cs.CV

TL;DR: 提出ATLAS统一框架，采用“思考-规划-绘制”范式，通过布局表示改进多模态大模型的可控图像生成，在空间指令任务上实现SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态大模型难以遵循复杂空间指令和逻辑约束，缺乏空间推理与规划能力。

Method: 布局作为三阶段共享表示，结合强化学习优化布局对齐，实现空间推理、对象规划与图像生成。

Result: 在7B和80B规模上，图像生成基准平均提升65.31%，空间任务提升23.06%，支持指令编辑与多模态定位。

Conclusion: ATLAS通过结构化布局和强化学习，显著增强了MLLM在可控生成中的空间指令遵循能力。
...
### [164] [One Modality to Forget Them All: Enhancing Cross-Modal Unlearning in Vision-Language Models](https://arxiv.org/abs/2607.16442)
*Sudharshan Balaji,Yili Ren,Guangjing Wang,Yimin Chen,Ning Wang*

Main category: cs.CV

TL;DR: 研究了视觉-语言模型中跨模态遗忘的迁移现象，发现其不对称且不完整，易受文字攻击影响，并提出了 CrossInf 缓解策略。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘机制在单模态语言模型中有效，但视觉-语言模型多模态交互可能带来遗忘在模态间迁移的安全问题，需系统评估。

Method: 系统评估三种VLM架构（LLaVA-1.5、InstructBLIP、IDEFICS）的跨模态遗忘迁移，提出基于影响的 CrossInf 策略聚焦跨模态泛化相关的Transformer块进行遗忘。

Result: 遗忘跨模态迁移不对称且不完整；文字攻击可恢复遗忘知识；CrossInf 减少超半迁移差距并保持效用，将攻击成功率降至近零；人类评估一致性好。

Conclusion: 视觉-语言模型遗忘存在浅层迁移问题，CrossInf 有效增强跨模态遗忘的鲁棒性，CKA分析揭示了迁移行为原因。
...
### [165] [DS@GT ARC at AnimalCLEF 2026: Species-Aware Graph Construction for Multi-Species Animal Re-Identification](https://arxiv.org/abs/2607.16453)
*Evan Sinclair Smith,Anthony Miyaguchi,Snigdha Palamari,Danté Evangelista*

Main category: cs.CV

TL;DR: 提出了一种多物种图像聚类系统，通过物种感知图构建、局部特征验证和图聚类，解决野外图像中个体重识别问题，在AnimalCLEF 2026中取得第五名。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以分离个体身份特征与姿态、光照等干扰因素，且聚类易因高评分假阳性连接导致不同个体被误合并。

Method: 集成定制预处理、全局候选检索、LightGlue局部验证、LightGBM对评分、保守边准入和Leiden社区检测，构建物种感知图进行聚类。

Result: 公开ARI 0.733，私有ARI 0.674，在230支队伍中排名第五。

Conclusion: 稳健的野生生物重识别需要全局相似性、局部标识、邻域上下文和图级约束的校准整合。
...
### [166] [Spectral-Morphological Attention U-Net: An Efficient Network for Active Wildfire Detection](https://arxiv.org/abs/2607.16472)
*Yugong Zeng,Jonathan Wu*

Main category: cs.CV

TL;DR: 提出SMA-UNet模型，通过光谱注意力、注意力残差UNet、通道-空间调制器和可微分形态学门实现早期野火检测，在两个数据集上取得最优分割结果。


<details>
  <summary>Details</summary>
Motivation: 全球野火频率持续增加，早期检测和精确定位对减少危害至关重要。

Method: 设计SMA-UNet，包含光谱注意力模块、残差注意力UNet骨干、通道-空间调制器和可微分形态学门，首次将可微分形态学门用于主动火情检测。

Result: 两个数据集分别达到75.16%和22.50%的IoU，通过消融实验验证了各模块的贡献。

Conclusion: 模型在不同环境下鲁棒性强，未来需在大规模多区域多传感器数据上验证泛化性。
...
### [167] [WeedExpert-R1: Incentivizing Botanical Reasoning in MLLMs with Reinforcement Learning for Precision Weed Grounding](https://arxiv.org/abs/2607.16492)
*Zonglin Yang,Wei-Zhen Liang,Nevin Lawrence,Xin Qiao,Benjamin Riggan,Chi-En Chiang,Fuchen Li*

Main category: cs.CV

TL;DR: 提出 WeedExpert-R1，一种通过可验证奖励学习视觉基础植物推理的多模态模型，在 37 种杂草上达到 75.82% 精确率，超越 GPT-5.4 等模型，支持开放词汇部署。


<details>
  <summary>Details</summary>
Motivation: 传统杂草检测器词汇封闭、无法跨区域部署，且无法推理复杂场景；MLLM 存在幻觉问题。

Method: 结合人工植物特征字典与审核-合成 LLM 工作流生成推理数据，通过带格式、准确率等奖励的 GRPO 训练。

Result: 在 6 个数据集 37 种杂草上，精确率 89.30%，召回率 87.81%，超越 GPT-5.4 和 Qwen3-VL-30B-Instruct 等。

Conclusion: 方法具备零样本开放词汇能力，无需再训练即可跨区域、跨作物部署。
...
### [168] [Geometry-Enhanced Portion Estimation for Multimodal LLMs](https://arxiv.org/abs/2607.16514)
*Lin Liao,Peng Li*

Main category: cs.CV

TL;DR: 该论文提出一种增强多模态大语言模型（MLLM）的餐食份量估计方法，通过添加基于DINOv2和软注意力机制的小型几何网络，在无需深度传感器或微调MLLM的情况下，显著降低份量估计误差（33-41%），并超越现有旗舰模型和基准方法。


<details>
  <summary>Details</summary>
Motivation: 基于图像的饮食评估有望替代传统记录方法，但份量估计是主要瓶颈；现有MLLM在零样本食物识别上表现优异，但份量估计能力薄弱。

Method: 在冻结的商业MLLM上附加一个几何增强的小型网络，该网络基于冻结的DINOv2骨干网络和结构化软注意力体积，利用MLLM输出的食物名称、边界框和密度范围进行份量估计，无需深度传感器和MLLM微调。

Result: 在三个真实世界基准上的开放词汇评估中，该方法的每食物份量误差相对单独MLLM降低33-41%，优于所有旗舰MLLM的直接估计，且在每个基准上超过其原发表的图像模型。

Conclusion: 所提方法能有效提升MLLM的份量估计准确性，且无需额外传感器或模型微调，具有实用性和泛化能力。
...
### [169] [Do Vision Encoders Exhibit Human-like Color Thresholds?](https://arxiv.org/abs/2607.16540)
*Engy Ehab,Pablo Hernández-Cámara,Nahla Belal,Jesús Malo,Javier Vazquez-Corral,Alexandra Gomez-Villa*

Main category: cs.CV

TL;DR: 研究了50多个预训练视觉编码器的色度敏感性与人类感知阈值的一致性，发现当前模型普遍表现不佳（最佳mIoU<0.25）。


<details>
  <summary>Details</summary>
Motivation: 探索深度视觉编码器是否自发形成类似人类的色觉判别能力，填补该领域研究空白。

Method: 使用多色度水平控制刺激，通过区域重叠度量（mIoU）对比模型色度判别椭圆与人类判别椭圆。

Result: 模型与人类阈值对齐弱；自监督编码器优于监督编码器，语言监督模型表现两极分化。

Conclusion: 当前大规模视觉训练目标未能使模型自然产生人类色度敏感特性。
...
### [170] [Can Multimodal Large Language Models Understand OCT?](https://arxiv.org/abs/2607.16609)
*Baochen Fu,Wenzhi Deng,Baihao Jin,Yang Li,Zihan Nie,Kailin Jiang,Yuntao Du,Weiye Song*

Main category: cs.CV

TL;DR: 提出OCT-Bench基准，包含10,076个高质量选择题，从感知、认知、推理三个维度20项细粒度任务系统评估20种多模态大语言模型的OCT图像理解能力，发现现有模型在临床级可靠性上仍有显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准将OCT理解简化为粗粒度疾病分类或孤立视觉问答，未能全面评估从视觉感知到临床推理的完整认知过程。

Method: 构建包含20项细粒度任务的层级能力分类体系（感知/认知/推理），基于7个公开数据集的4,137幅OCT图像生成10,076道选择题，系统评估20种代表性多模态大语言模型。

Result: 当前模型在OCT理解上远未达到临床可靠水平，且医学领域适配或增大模型规模并非总能提升各能力层级表现。

Conclusion: OCT-Bench为识别模型能力瓶颈和推进临床级OCT理解提供了细粒度评估基础，揭示现有多模态大语言模型的根本性局限。
...
### [171] [AffectFuse: Cross-Task Feature Fusion with Temporal Modeling for Multi-Task Affective Behavior Analysis](https://arxiv.org/abs/2607.16546)
*Dipit Saha,Mohammad Raihan Rashid,Shah Mohammad Abdul Mannan,Ahnaf Tahmid,Md. Mehedi Hasan*

Main category: cs.CV

TL;DR: 本文提出了一个用于ABAW竞赛中多任务情感行为分析的系统，通过冻结预训练骨干网络、任务特定头部和跨任务融合模块，在不训练新大规模模型的情况下，在s-Aff-Wild2验证集上达到P=1.7302。


<details>
  <summary>Details</summary>
Motivation: 在无约束人脸图像中联合预测连续效价- arousal、分类表情和多标签动作单元需要高效的多任务学习，而现有模型难以同时兼顾任务特异性与跨任务协作。

Method: 采用冻结的AffectNet监督骨干网络提取多分辨率特征，通过任务特定时间头部和跨任务融合模块选择信号；对动作单元识别使用MAE-Face加LoRA适配，并通过DISFA进行每单元专家路由迁移。

Result: 在官方验证集上获得P=1.7302的性能指标，验证了后编码器适配与任务级建模的有效性。

Conclusion: 后编码器适配和任务特定建模策略能够构建强大的多任务学习管道，避免训练新的大规模面部基础模型。
...
### [172] [Cross-Branch Conflict as a Shield: Safeguarding Facial Identities in Unified Multimodal Image Editing](https://arxiv.org/abs/2607.16898)
*Weiwei Tan,Junxian Li,Rui Wang,Zhenhua Xu,Yanjun Zhang,Yu Leo Zhang*

Main category: cs.CV

TL;DR: 针对统一多模态模型的肖像编辑保护不足问题，提出CCS框架通过协同干扰ViT和VAE双分支表征并破坏跨分支兼容性，显著提升对抗保护效果。


<details>
  <summary>Details</summary>
Motivation: 现有对抗保护方法针对视觉理解或生成模型设计，在统一多模态模型的互补视觉通路中失效，需探索双分支协同干扰机制。

Method: 联合干扰ViT和VAE分支表征，利用线性CKA破坏跨分支兼容性，同时移除两通路稳定信息并制造不兼容视觉上下文。

Result: 在抑制身份保留编辑任务中，CCS性能持续优于现有保护方法。

Conclusion: 双分支特征协同扭曲能有效防御统一多模态模型的肖像编辑滥用。
...
### [173] [CNS-Edit++: Category-Agnostic 3D Editing with Coupled Neural Shape Representation](https://arxiv.org/abs/2607.16577)
*Jingyu Hu,Weilong Yan,Zhengzhe Liu,Haipeng Li,Ka-Hei Hui,Hao,Zhang,Chi-Wing Fu*

Main category: cs.CV

TL;DR: 提出基于隐空间的多功能3D形状编辑框架CNS-Edit++，通过耦合神经表示和特征体优化实现类别无关的编辑操作。


<details>
  <summary>Details</summary>
Motivation: 现有3D编辑方法局限于特定类别或缺乏灵活的局部操控能力。

Method: 耦合神经形状（CNS）表示（全局语义编码+3D神经特征体）与协同优化机制，结合KV缓存替换和隐空间正则化实现区域保护。

Result: 在多种3D生成模型上，定量定性评估均显著优于现有方法。

Conclusion: 该框架实现了类别无关、多操作（复制/缩放/删除/混合/拖拽）的高质量3D编辑。
...
### [174] [Digital measurement of droplet flame diameter in microgravity combustion images using Segment Anything Model 2 with automatic prompt selection](https://arxiv.org/abs/2607.16587)
*Minghui Xu,Chaoyi Zhou,Aaron P. Cecil,Xi Liu,Siyu Huang,Yuhao Xu*

Main category: cs.CV

TL;DR: 提出了一种基于SAM2的自动火焰直径测量流程，通过RANSAC圆拟合和自动提示点生成，在微重力液滴燃烧图像中实现了高精度、高效率的测量。


<details>
  <summary>Details</summary>
Motivation: 传统从自发光火焰图像提取火焰直径存在偏差大、依赖操作者的问题（如烟尾、模糊边界、反射和液滴漂移）。

Method: 集成自动提示点生成到Segment Anything Model 2中，采用RANSAC圆拟合，并引入视频记忆机制维持时序一致性。

Result: 在19537张图像上验证，平均相对一致性96.9%，平均绝对百分比误差3.1%，效率提升约229倍，比传统霍夫圆检测表现更好。

Conclusion: 该SAM2流程提供了可重复、全自动、计量学特征的火焰直径数字测量系统，支持高通量燃烧诊断，展示了AI分割在量化测量中的可行性。
...
### [175] [PAVXploreRL: Physical-Action-Visual World Model Reinforcement Learning with Action Exploration](https://arxiv.org/abs/2607.16602)
*Han Wang,Zijun Wang,Shuoshuo Xue,Rui Cao,Fenjiao Cheng,Xiaodang Liang,Roy Ka-Wei Lee*

Main category: cs.CV

TL;DR: 提出PAVXploreRL框架，通过强化学习优化动作条件世界模型的物理合理性、动作遵从性和视觉保真度，提升对专家数据和分布外动作的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖专家数据与像素级损失，未能明确优化PAV目标，且在分布外动作上泛化能力差。

Method: 基于预训练潜空间世界模型，使用强化学习通过奖励信号直接优化PAV目标，并联合利用专家轨迹与噪声驱动的分布外动作探索。

Result: 在多个基准上平均提升5.6%，生成更高质量的PAV属性，作为策略评估器可减少过估偏差。

Conclusion: PAVXploreRL能显著提升世界模型的多目标性能，并改善其对分布外动作的泛化与评估可靠性。
...
### [176] [Thinking in Video: Can Video Generators Really Reason About the Real World?](https://arxiv.org/abs/2607.17523)
...
TL;DR: 本文提出因果-生成双判官（CGDJ）框架，从显式因果感知和隐式生成预测差距两个维度审计视频生成器的世界模型一致性，发现开源模型虽能生成合理动态但因果感知近乎为零，闭源系统因果推理与视频生成存在有限对齐，且存在视听错位问题，质疑了视频模型作为世界模拟器的能力。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型虽被视作世界模型，但无法确定其生成结果源于因果理解还是记忆模式，现有评估指标将感知保真度与语义逻辑分离，因此需要统一框架来评估视频生成器的因果推理能力。

Method: 提出因果-生成双判官（CGDJ）方法，包含显式因果感知测试（通过时空展平视觉问答检测生成器是否将视频场景视为推理问题）和隐式生成预测差距测试（评估生成器是否将因果结果渲染为一致未来视频）。

Result: 开源模型生成合理动态但显式因果感知近零，闭源系统因果推理与生成对齐有限；进一步发现模型言语化因果逻辑比渲染视觉因果更可靠，存在视听错位。

Conclusion: 当前视频生成模型（包括先进闭源系统）在因果推理与视觉生成方面存在显著差距，不能视为真正的世界模拟器，未来需改进模型因果感知与生成的一致性。
...
### [177] [SPARE-GS: Structural Parsimony and Resource Efficiency for 3D Gaussian Splatting](https://arxiv.org/abs/2607.16624)
*Zhang Chen,Shuai Wan,Fuzheng Yang,Jiazhi Xia,Weiyao Lin,Junhui Hou*

Main category: cs.CV

TL;DR: 提出SPARE-GS框架，通过全局预算约束优化3DGS的结构演化，在保持质量的同时减少高斯原语数量和训练时间。


<details>
  <summary>Details</summary>
Motivation: 3DGS虽然高质量实时渲染，但训练效率和表示紧凑性受原语过度增殖影响。

Method: 将结构演化建模为全局预算约束优化问题，利用边际效用平衡条件，通过自适应配额和区域偏差协调密集化、剪枝和终止。

Result: 高斯原语数量减少30.38%，训练时间减少23.81%，平均PSNR提升；下游压缩/剪枝方法性能改善。

Conclusion: 全局结构预算调控能有效提升3DGS的效率和紧凑性，具有广泛适用性。
...
### [178] [TellTale: Blending Multi-Instance LoRA Text Encoders and a Zero-Shot LLM Judge for Ambivalence/Hesitancy Recognition in Videos](https://arxiv.org/abs/2607.16635)
*Abdel-Karim Al-Tamimi,Ali Rodan*

Main category: cs.CV

TL;DR: TellTale是一种仅使用转录文本的犹豫/矛盾识别方法，在BAH数据集上结合三个概率流（两个微调文本编码器和一个零样本大模型），通过加权平均和决策阈值实现最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖视频、音频等多模态数据，而该研究探索仅用文本就能有效识别犹豫/矛盾的可能性。

Method: 采用两个文本编码器（multilingual-e5-large和mDeBERTa-v3-base）用LoRA微调，通过多实例学习（MIL）目标对转录片段评分；另用14B指令大模型零样本评分；最后加权平均三个概率流并调阈值。

Result: 在152个未见参与者视频的测试集上，Macro-F1为0.7364，平均精度0.7940，远超视觉基线（Macro-F1 0.2827）。

Conclusion: 仅用文本即可高效识别犹豫/矛盾，方法简单且性能优于多模态基线。
...
TL;DR: 提出了一种无需训练、无需领域知识的工业视频异常检测智能体框架，通过跟踪对象时空状态演化来推理异常，在三个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有VLM方法在工业场景中因对象变换复杂、物理约束严格而性能下降，且需重新训练或注入领域知识，限制了实际应用。

Method: 构建了无训练智能体框架，模拟人类检查员跟踪对象状态演化，包括时空动态和潜在变换，再基于对象时间状态轨迹推理异常。

Result: 在三个IVAD数据集上优于前沿VLM、智能体框架及传统VAD方法，并能提供异常过程和类型的可解释报告。

Conclusion: 所提方法无需领域知识或重训练，通过状态轨迹推理有效提升工业异常检测性能，同时保持可解释性。
...
### [180] [DRIFT: Difficulty-aware Rectified Flows for Through-plane MRI Super-Resolution](https://arxiv.org/abs/2607.16649)
*Yoonseok Choi,Eun-Gyu Ha,Daniel Kim,Mohammed A. Al-masni,Ming-Hsuan Yang,Dong-Hyun Kim*

Main category: cs.CV

TL;DR: DRIFT 是一个两阶段厚度条件化的整流流框架，用于MRI各向异性超分辨率，在保持高保真度的同时降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法在效率与保真度之间权衡不佳：前馈回归器在大切片厚度下过平滑，而采样方法推理成本高。

Method: 第一阶段使用解剖投影网络（APN）映射低分辨率到粗糙高分辨率流形进行初始化；第二阶段通过整流流精化细节，并引入物理感知难度（PAD）指标和自适应积分调度器（AIS）调整ODE步长，以及一致性端点轨迹对齐（CETA）损失。

Result: DRIFT 在超分辨率基准测试中表现优于现有方法，同时降低了推理成本。

Conclusion: DRIFT 有效解决了效率-保真度权衡问题，代码和模型已公开。
...
### [181] [DARA: Degradation-Aware Low-Rank Residual Adaptation with Original-to-Corrupted Distillation for Corruption-Robust Animal Re-Identification](https://arxiv.org/abs/2607.16644)
*Cynthia Xie,Talia Xu*

Main category: cs.CV

TL;DR: 本文提出DARA，一种轻量级特征空间修复方法，用于增强动物重识别在视觉退化下的鲁棒性。DARA冻结骨干网络，学习路由低秩残差专家适配退化输入，无需退化类型标注，并通过原始-退化蒸馏保持嵌入和检索关系。实验表明DARA在多个数据集上显著提升退化查询检索性能，仅增加少量参数和计算量。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒策略（如退化增强训练或像素级恢复）间接提升鲁棒性，但未显式修复退化导致的身份检索空间偏移。作者旨在直接修复特征空间，实现高效鲁棒动物重识别。

Method: DARA包含三个核心：1) 冻结微调骨干网络；2) 学习路由低秩残差专家（无需退化类别标注）自适应修复退化输入嵌入；3) 原始-退化蒸馏，使用原始图像教师保持个体嵌入和检索关系。

Result: 在ATRW、FriesianCattle2017、MPDD和SeaStarReID2023上，DARA优于标准微调和增强微调，泛化到未知退化和跨域场景，恢复77.0%退化查询mAP差距，仅增加0.49%参数和0.05%FLOPs。

Conclusion: DARA是一种高效的退化鲁棒动物重识别方法，通过轻量级特征空间修复显著提升性能，且易于集成到现有紧凑模型。
...
### [182] [Position: Explanation Stability Is a Property of the Model Method Pair, Not the Model](https://arxiv.org/abs/2607.16652)
*Kabilan Elangovan,Daniel Ting*

Main category: cs.CV

TL;DR: 本文论证解释稳定性需跨方法验证，通过X光实验表明稳定性是模型-方法对的涌现属性。


<details>
  <summary>Details</summary>
Motivation: 当前解释稳定性声明缺乏科学有效性，需明确验证方法。

Method: 基于DenseNet201、ResNet50V2、InceptionV3的胸部X光控制实验，使用LayerCAM和GradCAM++进行稳定性评估。

Result: 三模型AUC>99%，但稳定性排名在归因方法间反转；LayerCAM认为InceptionV3最稳定（IoU=0.777），而GradCAM++使InceptionV3稳定性下降17.3%。

Conclusion: 解释稳定性非模型固有属性，应跨归因方法验证，并明确规范使用算子以避免虚假安全保证。
...
### [183] [DORS: Dynamic Attention Routing for Diffusion-based Object Removal in Dense Scenes](https://arxiv.org/abs/2607.16656)
*Haitong Tang,Haipeng Liu,Yang Wang*

Main category: cs.CV

TL;DR: 提出基于扩散模型和动态注意力路由的目标移除框架DORS，解决密集场景中因语义干扰导致的移除不完整问题；同时构建专用基准DOR-Bench，实验表明该方法在减少不完整移除和重复伪影方面优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有方法在密集场景中因自注意力全局匹配机制导致掩码查询与相似实例错误对齐，产生语义干扰，造成目标移除不完整。

Method: 利用动态注意力路由机制，包括实例过滤注意力（抑制相似实例的误导信息）和上下文引导路由（动态融合场景信息以维持视觉一致性）。

Result: 在密集场景目标移除任务上，DORS显著优于现有最先进方法，尤其有效减少不完整移除和重复伪影。

Conclusion: 所提出的动态注意力路由机制能有效解决密集场景中的语义干扰问题，提升目标移除的完整性和视觉一致性。
...
### [184] [OFD-Net: Teacher-Free Reliable Semi-supervised Medical Image Segmentation with Orthogonal Feature Disentanglement Net of Foreground-Background](https://arxiv.org/abs/2607.16705)
*Shao-feng Jiang,Zhe-yang Jing,Qin Lu,Huan-huan Shi,Zhen Chen,Cong-xuan zhang,Chen Yi*

Main category: cs.CV

TL;DR: 提出OFD-Net，一种无教师单网络半监督医学图像分割框架，通过正交特征解耦提高可靠性，减少错误累积和确认偏差。


<details>
  <summary>Details</summary>
Motivation: 现有半监督方法依赖伪标签，但缺乏结构参考，低质量伪标签导致训练不可靠、错误累积和确认偏差。

Method: 使用正交特征解耦模块（OFDM）将未标注数据解耦为背景-前景表示；通过解耦引导模块（DGM）注入结构先验；基于结构一致性评估伪标签可靠性并降低不可靠区域权重。

Result: 在ISIC-2016、Kvasir-SEG、Synapse和ACDC四个公共数据集上验证了有效性。

Conclusion: 正交前景-背景解耦使OFD-Net能在无教师单网络框架中建立高效可靠的训练范式。
...
### [185] [Foundation-Assisted Active Learning for Object Detection Annotation](https://arxiv.org/abs/2607.16671)
*Jinchang Zhang,Arnold Zumbrun,Jing Lin,Guoyu Lu*

Main category: cs.CV

TL;DR: 提出一种基于基础模型协同的主动学习和半自动标注框架，通过双源不确定性估计和目标中心多样性采样，有效降低遥感目标检测标注成本。


<details>
  <summary>Details</summary>
Motivation: 遥感目标检测标注成本高，现有主动学习方法存在定位与分类不确定性耦合、冷启动阶段定位噪声大、高召回候选提案导致伪多样性等问题。

Method: 构建参考定位源（SA-source）和检测预测源（OD-source）的双源机制；提出Foundation-model-enhanced Dual-Source Uncertainty estimation联合建模定位一致性与分类置信度；设计Object-Centric Diversity Sampling使用DINOv2和SAM2特征构建目标级表示；提出Dual-Source Box Switching以SA-source精化框替换噪声检测框。

Result: 在DIOR、HRSC2016、DOTAv2和FAIR1M数据集上，该方法在大多数标注预算下取得优越或相当结果，低预算场景下冷启动样本效率显著更强。

Conclusion: 该方法能有效提高主动学习样本选择质量，减少半自动标注中的人工修正负担，可高效构建遥感目标检测数据集。
...
### [186] [Can Experts Adapt Without Training? On Test-Time Modality Generalization in MVLMs](https://arxiv.org/abs/2607.16726)
*Raza Imam,Darakshan Rashid,Yutong Xie,Dwarikanath Mahapatra,Brejesh Lall,Mohammad Yaqub*

Main category: cs.CV

TL;DR: 提出MoBE框架，实现零训练的动态专家路由与自适应，提升医学视觉语言模型在未见模态和领域的泛化鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型在临床未知模态和域下可靠性骤降，解决测试时模态泛化瓶颈。

Method: 基于混合专家的熵引导动态路由与专家级贝叶斯自适应（免梯度优化）。

Result: 在已见、未见及异构医疗基准上，平均准确率分别提升+4.72、+7.17和+4.3，超越SOTA测试时适应方法。

Conclusion: 免训练专家自适应可有效增强模态泛化的鲁棒性，适用于临床动态场景。
...
### [187] [Multi-Dimensional Quality Assessment for AI-Generated Human-Centric Videos: Dataset and Model](https://arxiv.org/abs/2607.16742)
*Sijing Wu,Yunhao Li,Huiyu Duan,Yucheng Zhu,Xiongkuo Min,Patrick Le Callet,Guangtao Zhai*

Main category: cs.CV

TL;DR: 构建了最大规模AI生成人像视频质量评估数据集HVEval+，提出基于MoE-MLLM的统一评估方法MoE-Rater。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成人像视频存在质量问题和语义偏差，缺乏针对性的全面质量评估方法。

Method: 扩展HVEval数据集形成HVEval+，包含2万视频、6万MOS和偏好对；设计MoE-Rater模型，采用MoPE和MoLE专家模块以及三阶段训练策略。

Result: MoE-Rater在HVEval+和Human-AGVQA数据集上均取得优异性能，支持多维度质量评分、成对比较和分类问答。

Conclusion: HVEval+数据集和MoE-Rater方法有效推进了AI生成视频质量评估，为T2V模型优化提供了可靠工具。
...
### [188] [VisionAssist: An Open-Source Smartphone Assistant for AI-Based Visual Accessibility](https://arxiv.org/abs/2607.16750)
*Ayşe Özlem Çalışkan,Jordi Sanchez-Riera*

Main category: cs.CV

TL;DR: 本文介绍了VisionAssist，一个开源移动应用，通过AI视觉辅助帮助低视力用户独立完成日常任务，集成了物体定位、图像描述和紧急呼叫等功能。


<details>
  <summary>Details</summary>
Motivation: 低视力用户在需要解释视觉信息的日常任务中面临挑战，现有解决方案分散且缺乏统一平台。

Method: 集成三个功能：实时摄像头分析定位物体、生成图像语音描述、整合联系人和日历进行紧急呼叫与提醒；支持语音命令和文本转语音反馈。

Result: 开源发布，鼓励社区贡献，加速无障碍技术开发。

Conclusion: 通过统一平台和开源模式，VisionAssist有望提升低视力用户的独立性并促进无障碍技术发展。
...
### [189] [Hybrid Machine Learning for Articulation Angle Estimation of Truck-Semitrailer Combinations](https://arxiv.org/abs/2607.16758)
*Qixuan Zhang,Jonas Boettcher,Simon F. G. Ehlers,Marvin Stuede*

Main category: cs.CV

TL;DR: 本文提出多种学习模型结合扩展卡尔曼滤波，直接从视觉和运动学输入估计卡车拖车铰接角，无需初始化或额外传感器，并经过真实世界实验验证其鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手动初始化、额外传感器或拖车参数，实际部署受限，因此需要一种无需这些条件的实用铰接角估计方法。

Method: 采用多个学习模型与运动学模型在扩展卡尔曼滤波框架内集成，并通过不确定性量化实现基于视觉输入的自适应加权方案。

Result: 在不同拖车类型、颜色和光照条件下的真实世界实验中，混合方法实现了准确可靠的铰接角估计，并展现出良好的鲁棒性和泛化性。

Conclusion: 该方法在降低实施要求的同时，实现了准确且可靠的铰接角估计，具有实际部署优势。
...
### [190] [HTT-Net: Hierarchical Text-guided Transition Modeling for Surgical Video Phase Recognition](https://arxiv.org/abs/2607.16787)
*Kunjie Deng,Jinghui Zhang,Weidong Chen,Ganbin Li,Xiangjun Lyu,Zhendong Mao,Yingchi Yang*

Main category: cs.CV

TL;DR: 提出HTT-Net，通过层次化文本引导的转换建模网络，利用结构化手术语义知识解决视频相位识别中的视觉模糊和噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有模型因局部视觉模糊、瞬时预测噪声及程序语义利用不足，导致手术视频相位识别不准确且时间一致性差。

Method: 构建层次化手术语义记忆（包括阶段内/间描述和细粒度单元），结合TAS-Con（转换感知段构建）和TAS-Calib（转换感知段校准）来组织帧级证据并增强表示。

Result: 在Cholec80和LCRS-100数据集上验证了HTT-Net的有效性，实现鲁棒的手术视频相位识别。

Conclusion: 层次化文本引导方法可有效提升相位识别的准确性和时间连贯性。
...
### [191] [InLiER: Learning-Free Heterogeneous LiDAR Place Recognition via Intermediate Mixed-Radix Structural Keypoint Tokenization](https://arxiv.org/abs/2607.16862)
*Nikolaos Stathoulopoulos,George Nikolakopoulos*

Main category: cs.CV

TL;DR: 本文提出了一种无学习的跨传感器LiDAR地点识别方法InLiER，通过中间离散标记步骤，能在不同视场、分辨率和扫描模式的LiDAR之间稳定工作。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR地点识别描述子过度依赖传感器特性，在多种LiDAR共存的场景中性能下降，需要一种与传感器无关的通用方法。

Method: 基于中间离散标记的无学习流程：从结构元素提取高度切片关键点，为其分配混合进制标记ID，编码高度、径向距离、局部形状和方位角，生成紧凑的<2KB表示；通过三级检索（高度直方图相交、二进制位掩码对齐、标记引导的几何验证）实现地点识别和位姿估计。

Result: 在HeLiPR数据集和真实实验中达到最先进性能，在手工艺方法中领先，并在多数跨传感器配置上优于基于学习的基线方法。

Conclusion: InLiER通过中间离散标记化有效解耦了传感器特性，实现了跨不同LiDAR传感器的高效地点识别和位姿估计。
...
### [192] [What Do They See? Interpreting Complex Road Scenarios Through the Eyes of Vision-Language-Action Models for Safe and Trustworthy Autonomous Vehicle Learning](https://arxiv.org/abs/2607.16938)
*Kalpana Panda,Wesley Maia,Vinti Agarwal,Ross Greer*

Main category: cs.CV

TL;DR: 提出反事实消融框架CVAA，通过生成式修复移除图像中的物体，分析端到端自动驾驶模型对单个目标的因果响应，并结合机制可解释性探究内部表征。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型的黑箱特性导致安全关键决策难以理解，现有方法缺乏对单个场景元素因果影响的系统分析。

Method: 1) 反事实消融：用生成式修复从摄像头图像中移除检测到的物体，构建反事实集；2) 机制可解释性：对比原始与修复图像的中间层表征，追踪物体移除对各层的影响。

Result: 1) 车辆和行人沿模型规划路径主导因果影响，交通灯影响非对称；2) 发现模型对无关物体也有强响应；3) 内部表征与人类可理解场景元素不完全对应。

Conclusion: CVAA从行为审计到表征理解分层揭示模型推理，提升自动驾驶系统的可解释性与人机信任。
...
### [193] [Spatiotemporal Facial Action Unit Detection using Twin Cycle Autoencoders for Driver Monitoring](https://arxiv.org/abs/2607.16760)
*Sai Sidharth D*

Main category: cs.CV

TL;DR: 提出Twin Cycle Autoencoder（TCA）时空架构，通过双分支循环自编码器联合学习面部动作单元（AU）的空间外观与时间动态，在驾驶场景中实现低光照、遮挡等复杂条件下的实时AU检测，并在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有AU检测器主要分离处理空间外观与时间动态，未能充分利用大量未标记驾驶视频中的自监督信号，且难以应对驾驶场景中的低光照、遮挡和细微AU动态。

Method: 构建双分支循环自编码器：空间分支通过图像级循环一致性分离AU相关外观与身份特征；时间分支通过前向-后向一致性学习AU轨迹的起始-顶点-消退动态；两分支通过跨分支潜在对齐损失和注意力融合后进行分类。

Result: 在DISFA、BP4D及自然驾驶数据集上，TCA在低强度和快速瞬态AU（如疲劳相关AU45、AU43和哈欠AU26）检测中优于CNN-RNN、3D-CNN和图模型基线，并在Jetson Xavier NX上实现实时推理。

Conclusion: TCA通过时空联合自监督学习有效提升驾驶场景中AU检测的鲁棒性和实时性，适用于生产级高级驾驶辅助系统（ADAS）。
...
### [194] [Scene-SAM3D: Multi-View Scene Asset Generation Without Fine-Tuning](https://arxiv.org/abs/2607.16805)
*Yuqi Zhang,Yadan Luo,Xiangyu Sun,Fengyi Zhang,Zi Huang,Xin Tan*

Main category: cs.CV

TL;DR: 提出Scene-SAM3D，一种无需训练的框架，将单视图3D物体生成扩展为校准的多视图场景生成，通过紧凑视图选择、潜在速度融合和轻量级高斯优化，在降低计算开销的同时提升场景重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有单图像3D生成模型（如SAM3D）在处理真实场景时因严重遮挡、冗余观测和跨视图不一致而效果不佳，亟需一种可靠的场景生成方法。

Method: 1. 互补视图选择减少冗余；2. 潜在速度融合整合多视图证据并抑制冲突；3. 轻量级刚性物体高斯优化（200次迭代）精修场景布局。

Result: 在Replica和ScanNet++上，场景级Chamfer距离分别降低43.8%和30.9%，且在多视图设置下流模型采样FLOPs和延迟降低约20%。

Conclusion: Scene-SAM3D通过训练自由的多视图融合策略显著提升3D场景生成质量与效率，适用于机器人操作等具身应用。
...
### [195] [Test-Time Registers as Global Priors for Tokenized Image Generation](https://arxiv.org/abs/2607.16824)
*Cheng-Yao Hong,Yifan Wang,Yuewei Lin,Chenyu You*

Main category: cs.CV

TL;DR: 本文发现视觉Transformer中的注意力汇（register）具有低频集中特性，并提出无需训练的RegToken方法将其转化为全局先验，用于提升token化图像生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要将register视为注意力伪影并通过解释性分析研究，未探索其能否作为即插即用的生成信号。

Method: 提出RegToken方法，包括基于NFN的层定位、TokenRank引导的子空间提取以及寄存器子空间的投影与守恒更新。

Result: 在冻结的token生成管道中，RegToken将ImageNet FID-5k从20.5降至20.1，SigLIP从3.6提升至3.9，并加速测试时优化。

Conclusion: 注意力伪影结构（如register）可被重新用作轻量级全局先验，以改善token化图像生成。
...
### [196] [Spatially-Aware Class-Agnostic Object Counting](https://arxiv.org/abs/2607.16826)
*Robert Wijaya,Md. Tanvir Hossain,Amanda Kau,Ngai-Man Cheung*

Main category: cs.CV

TL;DR: 提出UpCount，一种通过增强空间结构表示来提升类无关目标计数性能的方法，在FSC-147和CARPK数据集上取得先进结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理结构复杂目标时空间建模不足，导致计数精度受限。

Method: 利用ViT-B/16编码器提取多层特征，通过Dense Prediction Transformers和FeatUp进行空间精炼，形成多尺度金字塔，并使用提议-验证计数头生成密度图。

Result: 在FSC-147测试集上MAE为12.39，RMSE为100.89；在CARPK数据集上MAE为6.27，RMSE为8.79。

Conclusion: UpCount有效提升了对结构复杂目标的计数准确性，且具有良好的跨数据集迁移能力。
...
### [197] [Robust PnP on a Neuromorphic Processor for Object Pose Estimation](https://arxiv.org/abs/2607.16834)
*Tam Ngoc-Bang Nguyen,Mohsi Jawaid,Tat-Jun Chin*

Main category: cs.CV

TL;DR: 提出一种神经形态可部署的鲁棒PnP方法，结合脉冲神经网络预测2D地标与分布式鲁棒最小二乘优化，并在Loihi 2上实现高能效物体姿态估计。


<details>
  <summary>Details</summary>
Motivation: 神经形态计算在非学习任务（如PnP优化）中的部署困难，限制了其在混合感知管线（如OPE）中的应用。

Method: 设计脉冲神经网络从事件数据预测2D地标，并提出基于分布式鲁棒最小二乘的神经形态PnP算法。

Result: 在Intel Loihi 2上实现更高的能效，同时保持竞争性精度。

Conclusion: 实现了OPE管线从事件感知、地标预测到几何优化的全神经形态化处理。
...
### [198] [UniNDM: A Unified Noise-driven Detection and Mitigation Framework Against Sexual Content in Text-to-Image Generation](https://arxiv.org/abs/2607.16828)
...
TL;DR: 提出统一噪声驱动框架UniNDM，利用扩散模型早期噪声可分性进行隐含性暗示内容检测与抑制，在U-Net和DiT模型上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型易受隐含性暗示提示攻击，现有检测与缓解方法无法处理。

Method: 基于早期噪声的隐式内容检测器，结合大语言模型生成自适应负面提示与噪点注意力抑制。

Result: 在自然与对抗数据集上，检测精度和内容抑制效果显著优于SLD、UCE、Safree等方法。

Conclusion: UniNDM能有效防御隐含性暗示内容，且计算开销低，可扩展至新架构。
...
### [199] [TopoGS: Planar Reconstruction via Topology-aware 3D Gaussian Splatting](https://arxiv.org/abs/2607.16838)
*Shanshan Pan,Jiale Chen,Yilin Liu,Hui Huang*

Main category: cs.CV

TL;DR: TopoGS 是一种结合平面与拓扑约束的3D高斯泼溅框架，能从多视图图像中生成拓扑一致的3D模型，在ScanNet++上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将平面视为孤立离散集，缺乏拓扑连接导致重建碎片化、边界不对齐，无法满足精确空间分析和专业设计需求。

Method: 从多视图图像分割中提取全局2D拓扑关系，将高斯图元锚定到这些结构元素上，联合优化平面参数、渲染保真度和拓扑邻接，并强制执行多视图一致性。

Result: 在ScanNet++数据集上取得最先进性能，几何对齐显著改善，生成连通且结构化的3D模型。

Conclusion: TopoGS 能够生成拓扑正确、视觉保真且高度鲁棒的场景表示，有效解决了3D重建中的拓扑连通性问题。
...
### [200] [Look Clearly Before Answering: Mitigating Hallucinations in LVLMs via Saliency-Driven Perceptual Realignment](https://arxiv.org/abs/2607.16841)
*Pengxu Chen,Yao Zhu,Guangming Zhu,Jun Sheng,Jincai Huang,Xiangyang Ji,Liang Zhang*

Main category: cs.CV

TL;DR: 提出SDPR框架，通过注意力重分配、KV缓存对齐和对比解码减少LVLM幻觉，无需训练且性能领先。


<details>
  <summary>Details</summary>
Motivation: LVLM存在幻觉问题，现有方法忽视视觉感知和记忆的渐进退化。

Method: SDPR包括3个步骤：1) 基于显著性的注意力重分配；2) 基于显著性的KV缓存对齐；3) 先验约束的对比解码。

Result: 在多个LVLM架构上，SDPR在幻觉和通用基准上优于现有方法，无需额外训练。

Conclusion: SDPR通过全生成轨迹对齐视觉意识，有效缓解幻觉。
...
### [201] [InfoDense: Density-Aware Regional Decisive Replay for Memory-Efficient Incremental Face Forgery Detection](https://arxiv.org/abs/2607.16873)
*Jikang Cheng,Hao Shen,Xueyi Zhang,Guangcheng Wang,Zhongyuan Wang,Renye Yan,Baojin Huang*

Main category: cs.CV

TL;DR: 提出了一种密度感知区域关键重放策略（InfoDense），用于增量式人脸伪造检测，以缓解灾难性遗忘并提升跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统重放方法在有限存储下难以保留细微伪造线索，且易引入域偏差，阻碍模型学习内在可迁移的操纵特征。

Method: 包括三个模块：InfoDense Cut（基于CLIP定位关键区域）、InfoDense Select（结合潜在空间代表性和关键补丁数量排序）、InfoDense Fuse（自适应融合存储段与当前任务样本）。

Result: 在增量深度伪造基准上有效缓解了灾难性遗忘，并提升了跨域泛化性能。
...
### [202] [Dataset Distillation by Influence Matching](https://arxiv.org/abs/2607.16859)
*Haoru Tan,Wang Wang,Sitong Wu,Xiuzhe Wu,Yangtian Sun,Chirui Chang,Shaofeng Zhang,Xiaojuan Qi*

Main category: cs.CV

TL;DR: 提出一种基于最终训练结果对齐的数据集蒸馏方法——影响力匹配（Inf-Match），通过可微分的样本级影响力估计器来学习合成数据集，在多个分类和视觉-语言蒸馏任务上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法大多关注过程代理（如每步梯度或训练轨迹）的对齐，而非最终训练结果的对齐，这可能导致次优表现。

Method: 提出了影响力匹配（Inf-Match）方法：设计了一个完全可微分的样本级影响力估计器，通过展开优化动力学并应用一阶泰勒近似来线性时间估计参数变化，然后最小化合成数据集与真实数据集之间的影响力差异。

Result: 在Tiny-ImageNet（IPC=10）上达到31.5%准确率，比NCFM提升4.7%；在Flickr30K视觉-语言蒸馏任务中，在图像/文本检索任务上平均提升2.5%

Conclusion: Inf-Match通过直接对齐最终训练结果，避免了启发式过程模仿，显著提升了数据集蒸馏的性能，并能扩展到多模态任务。
...
### [203] [Pedestrian Archetypes Extension -- More Pedestrian Models for Autonomous Vehicle Safety Testing](https://arxiv.org/abs/2607.16922)
...
TL;DR: 本文基于前期提出的12种行人原型，通过分析YouTube行车记录仪视频，新增了7种可观察且行为差异显著的行人原型，并详细描述了每种新原型的定义、行为特征及视频证据。


<details>
  <summary>Details</summary>
Motivation: 前期提出的12种行人原型未能完全解释实际交通场景中的行人行为模式，为提升原型分类的完整性和准确性，需补充新发现的原型。

Method: 采用YouTube行车记录仪视频的进一步标注方法，通过观察和对比行为差异，识别并定义7种新的行人原型。

Result: 新增7种行人原型，补充了原原型集在行为模式覆盖上的空白，并提供了视频帧证据支持。

Conclusion: 行人原型分类体系需持续扩展以捕捉真实场景中的多样化行为，新原型的引入有助于更自然、渐进地描述危险行人行为。
...
### [204] [Transferable Low-Rank Convolutional Bases for Onboarding Unseen Medical Imaging Modalities](https://arxiv.org/abs/2607.16888)
*Ranat Das Prangon,Istiaque Ahmed,Shajid Hasan Naim,Waseem Mustak Zisan,Hossain Md Shakhawat*

Main category: cs.CV

TL;DR: 本文研究了在严格留一域外协议下，将医学影像模型部署到未见模态时的问题。通过冻结卷积骨干网络，仅用少量参数（全微调的0.78%）进行低秩卷积适配，即可有效适应新模态，同时保持源模态性能不变。


<details>
  <summary>Details</summary>
Motivation: 医疗影像模型部署后常需适应新模态，但重新训练共享表示代价高昂且会损害已有模态性能。

Method: 采用冻结卷积骨干网络的严格留一域外协议，比较线性探针、全连接LoRA和卷积LoRA等适配方法，并分析低秩卷积基的可迁移性。

Result: 卷积LoRA恢复大部分准确率，低秩卷积基可迁移且仅用0.78%参数，比随机基高6.11个百分点；决策层基无可靠迁移；适配后源模态性能不变。

Conclusion: 基于适配器的模态接入可有效应对未见模态，同时保留源模态性能，且马氏距离可高灵敏度检测新模态作为触发信号。
...
### [205] [Splat-based 3D Scene Reconstruction with Extreme Motion-blur](https://arxiv.org/abs/2607.16926)
*Hyeonjoong Jang,Dongyoung Choi,Donggun Kim,Woohyun Kang,Min H. Kim*

Main category: cs.CV

TL;DR: 提出一种基于高斯溅射的RGB-D 3D场景重建方法，能有效处理极端运动模糊，在低光环境下实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 传统方法在低光快速运动下相机位姿估计和图像去模糊效果差，影响3D重建质量。

Method: 结合RGB-D帧的光流和ICP对齐，利用高斯溅射框架优化位姿和几何，并通过模拟曝光期间相机运动来去模糊。

Result: 在极端运动模糊的新RGB-D数据集上优于现有方法，实现高质量重建。

Conclusion: 该方法适用于机器人、自主导航和增强现实等领域的3D映射应用。
...
### [206] [Pediatric Bone Age Prediction Using Deep Learning](https://arxiv.org/abs/2607.16936)
*Al Zadid Sultan Bin Habib,Md. Ekramul Islam,Md Asif Bin Syed,Md Younus Ahamed,Tanpia Tasnim*

Main category: cs.CV

TL;DR: 该论文提出了一种基于EfficientNet和加性注意力机制的深度学习方法，利用超过1.2万张X光图像预测儿童骨龄，提高了预测准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统骨龄预测方法劳动强度大且需要专业放射学知识，因此需要更自动化和精确的解决方案来辅助诊断儿童内分泌疾病。

Method: 使用RSNA骨龄数据集，通过预处理将X光图像转为三通道图像，采用EfficientNetB0和EfficientNetB4模型（后者结合加性注意力机制）进行卷积神经网络训练，自动学习手骨特征以预测骨龄。

Result: EfficientNetB4和带加性注意力机制的EN-AA模型在大多数情况下比EfficientNetB0更准确地预测骨龄，并且学习曲线显示无过拟合或欠拟合。

Conclusion: 该深度学习方案有效提高了骨龄预测的准确性，有助于儿科内分泌疾病的诊断。
...
### [207] [Automated Cardiac Adipose Tissue Segmentation in Computed Tomography: A Literature Review](https://arxiv.org/abs/2607.16992)
...
TL;DR: 综述了CT图像上心外膜脂肪组织和心包脂肪组织自动分割方法的最新进展，包括AI和非AI方法，并讨论了剩余挑战。


<details>
  <summary>Details</summary>
Motivation: 人工量化心脏脂肪耗时且观察者间差异大，自动方法能提供高效一致的解决方案。

Method: 涵盖AI和非AI的自动分割方法。

Result: 自动方法可获得与人工标注相当的分割质量。

Conclusion: 自动方法有潜力作为临床工具发现新生物标志物并改善患者预后。
...
### [208] [When Physical Preferences Meet Semantic Constraints: Physical and Semantic Direct Preference Optimization for Text-to-Video Generation](https://arxiv.org/abs/2607.16947)
*Siwei Meng,Yawei Luo,Shu Zhang,Ping Liu*

Main category: cs.CV

TL;DR: 提出PSDPO方法，在不牺牲语义一致性的前提下提升文本生成视频的物理合理性，通过约束偏好优化解决物理-语义冲突。


<details>
  <summary>Details</summary>
Motivation: 物理偏好优化常导致语义漂移，现有方法忽视两者冲突，需在保持语义一致的同时提升物理合理性。

Method: 将问题建模为约束偏好优化，通过物理与语义信号一致性调控偏好对贡献，结合梯度分析与分阶段优化协议。

Result: 在VideoPhy-2上物理合理性提升2倍，VBench上语义一致性保持高水平，优于现有方法。

Conclusion: 提出的PSDPO在标准DPO框架内有效平衡物理与语义，无需额外模型或损失项。
...
### [209] [EvoGUI: An Evolution-Aware Benchmark for GUI State-Transition Understanding](https://arxiv.org/abs/2607.17050)
*Yaohan Yang,Minglei Shi,Borui Zhang,Jie Zhou,Jiwen Lu*

Main category: cs.CV

TL;DR: 提出了EvoGUI，一个诊断框架，通过三个VQA探针测试GUI智能体对状态转换的理解，并构建了EvoGUI-Bench基准，发现最强模型仅达到60.4 EvoGain，显示模型在该任务上仍存在显著提升空间。


<details>
  <summary>Details</summary>
Motivation: 端到端成功率无法分离GUI智能体对状态转换的理解与其他能力（感知、规划等），需要专门的诊断工具。

Method: 从归一化的GUI轨迹中提取时序排序、逆向动作/值预测、对比式单步后继判别三个探针，基于Mind2Web和WebLINX构建基准。

Result: 28个视觉语言模型零样本测试，最强模型EvoGain仅60.4，模型规模和GUI专精化不能可靠预测性能。

Conclusion: EvoGUI-Bench可作为端到端评估的可扩展诊断补充，并揭示了状态转换理解中的显著差距。
...
### [210] [High-Capacity Robust Watermarking Technology for High-Resolution Images](https://arxiv.org/abs/2607.17056)
*Shaowu Wu,Wei Lu,Qing Qian,Mingsen Deng*

Main category: cs.CV

TL;DR: 针对高分辨率图像的高容量鲁棒水印方法，嵌入容量达4 KB，嵌入率0.0313 bpp。


<details>
  <summary>Details</summary>
Motivation: 现有水印技术主要针对低分辨率图像，且嵌入容量有限，无法满足实际需求。

Method: 采用分块策略、可逆对称编码器-解码器（3卷积+3反卷积层），结合全局与局部加权损失函数。

Result: 在1024×1024图像中嵌入4KB水印，实验验证有效可行，且抵抗多种噪声攻击。

Conclusion: 该方法在高分辨率、高容量场景下有效，鲁棒性强。
...
### [211] [Searching for Task-Specific Vision Paths: Evolutionary Block Pruning Across Vision-Language Models](https://arxiv.org/abs/2607.17052)
*Tarun Tomar*

Main category: cs.CV

TL;DR: 论文研究了在视觉-语言模型中，是否可以在不微调的情况下跳过固定预算的视觉块组合，并比较了共享路径和能力特定路径的效果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型对每个问题都使用相同的完整视觉编码器，但OCR、计数、物体、属性和空间查询可能不需要相同的计算，因此作者希望探索更高效的视觉处理方式。

Method: 提出了源平衡进化搜索方法，与独立排序、连续移除和随机路径在匹配预算下进行比较，并在Qwen2.5-VL-3B-Instruct、SmolVLM2-2.2B-Instruct模型以及876示例的图像分离选择分割上进行了实验。

Result: 搜索跨架构迁移：SmolVLM2上搜索的共享四块路径比独立构造高4.91个百分点；能力专门化不稳定，Qwen上六块能力策略比共享路径高2.17点（OCR增益7.10点），但SmolVLM2的OCR专用路径比共享路径低13.6点。

Conclusion: 组合搜索可靠地改善了路径构建，但能力标签并未定义普遍可迁移的视觉通路。
...
### [212] [AdvSerial: Physical Adversarial Attacks on Infrastructure-mounted Pedestrian Detectors via Semantic Feature Suppression](https://arxiv.org/abs/2607.17069)
*Yuanhao Huang,Yilong Ren,Jinlei Wang,Xuesong Bai,Jinchuan Zhang,Haiyang Yu*

Main category: cs.CV

TL;DR: 提出AdvSerial，动态2D-3D联合优化框架，生成连续性高角度物理对抗补丁，有效攻击行人检测器。


<details>
  <summary>Details</summary>
Motivation: 基础设施监控中视觉感知系统面临物理对抗攻击的安全威胁。

Method: UV映射边界感知拼接线纹理到3D衣物，结合2D数字攻击与3D稀疏/连续帧渲染，抑制语义特征，利用特征平滑缝合策略和序列帧损失增强攻击连续性。

Result: 在YOLO-v5上攻击成功率达74.8%，检测置信度从84.30%降至39.38%；跨八种检测器具有强迁移性，对YOLO-v2达89.71%，并抵抗防御NapGuard和3D感知Sparse4D-v3。

Conclusion: 揭示了高角度监控下持续性失败模式，需设计运动感知和3D感知的防御机制。
...
### [213] [ALLUDE: A Unified Evaluation System for Configurable Attacks in Differentiable Environments](https://arxiv.org/abs/2607.17077)
...
TL;DR: ALLUDE是一个支持跨场景、物体、环境条件和相机轨迹的攻击评估系统，通过可微渲染和拉丁超立方采样，揭示了现有攻击（如CAMOU、RAUCA、FCA）在多变天气和轨迹下的性能下降。


<details>
  <summary>Details</summary>
Motivation: 当前对抗攻击评估局限于有限条件，缺乏统一、可定制的评估系统，难以模拟真实部署中的变化。

Method: 提出ALLUDE系统，集成可微渲染进行端到端优化，并使用拉丁超立方采样从5400种配置中高效抽取代表性子集。

Result: 在多种天气、轨迹和检测模型下，现有攻击的失败率显著增加，暴露了先前评估的缺陷。

Conclusion: ALLUDE提供了更全面的评估框架，表明对抗攻击在真实世界条件下需要更鲁棒的优化。
...
### [214] [HarmoHOI: Harmonizing Appearance and 3D Motion for Multi-view Hand-Object Interaction Synthesis](https://arxiv.org/abs/2607.17097)
*Lingwei Dang,Juntong Li,Zonghan Li,Hongwen Zhang,Liang An,Wei Min,Yebin Liu,Qingyao Wu*

Main category: cs.CV

TL;DR: 提出了HarmoHOI，一个统一的扩散框架，用于同步生成多视角手物交互视频和全局对齐的3D点轨迹，实现了多视角几何一致性。


<details>
  <summary>Details</summary>
Motivation: 视频基础模型虽然具有强大先验，但复杂的手部运动和遮挡使多视角一致的手物交互合成仍然困难。

Method: 采用混合多视角扩散Transformer共同建模RGB视频和3D点轨迹，将点轨迹表示为伪视频以对齐2D潜在空间，并通过全局运动对齐扩散优化3D轨迹，同时使用混合数据课程学习克服多视角数据稀缺。

Result: 在视觉质量、运动合理性和多视角几何一致性上达到最先进性能。

Conclusion: HarmoHOI有效实现了2D外观与3D运动的协同演化，解决了多视角HOI合成中的一致性问题。
...
### [215] [Autoregressive B-Rep Shape Generation with Parametric Surfaces](https://arxiv.org/abs/2607.17093)
*Dafei Qin,Rui Xu,Zeyu Shen,Kaichun Qiao,Hongyang Lin,Qixuan Zhang,Huaijin Pi,Lan Xu,Jingyi Yu,Wenping Wang,Taku Komura*

Main category: cs.CV

TL;DR: ParaCAD提出了一种基于自回归框架的条件性B-Rep生成方法，直接操作原生参数曲面，利用曲面中心令牌化编码保持几何语义，通过全局求交构建完整B-Rep，在几何精度、鲁棒性等方面显著优于点基方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于点或网格的B-Rep生成方法牺牲了原生曲面类型和参数，导致几何保真度和下游可用性受限。

Method: ParaCAD采用自回归框架，先基于曲面中心令牌化生成带约束UV域的参数曲面，再通过全局曲面求交恢复边和顶点，以点云为条件实现B-Rep合成。

Result: 实验证明ParaCAD生成的B-Rep与点云对齐准确，在几何精度、鲁棒性、水密性和下游可用性上优于点基基线方法。

Conclusion: ParaCAD为B-Rep合成提供了保留几何语义的直接参数曲面生成方法，适用于用户引导重建和3D生成流程集成。
...
### [216] [DepthART: Scaling Foundation Monocular Depth to Tiny Models](https://arxiv.org/abs/2607.17099)
*Feng Xue,Wu Chen,Mingshuai Zhao,Guofeng Zhong,Anlong Ming,Haozhe Wang,Dianqiao Lei,Zhaowen Lin,Haiyang Zhang,Nicu Sebe*

Main category: cs.CV

TL;DR: 提出DepthART，一种用于设备端部署的紧凑单目深度估计模型，通过偏差敏感数据采样和相机条件微调，在保持轻量级的同时实现跨场景高精度。


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型在单目深度估计上表现优异，但无法适用于小型模型；小型模型存在过拟合分布偏差和相机位移下度量不稳定的问题。

Method: 结合偏差敏感数据采样（减少训练分布偏差）和相机条件微调（冻结编码器，根据相机内参调整度量尺度以保留泛化能力）。

Result: 在零样本和度量精度上全面超越之前的小型基线（如NYUD v2上DepthART-S零样本δ₁=0.964），部分接近大型模型，且推理速度高效。

Conclusion: 通过针对性设计解决了小型模型在跨场景泛化中的瓶颈，实现了轻量化且高性能的深度估计模型系列。
...
### [217] [Learning Emotion from Motion: Kinetic Multi-Stream Skeleton Modeling with Metadata-Conditioned Weak Label Distributions](https://arxiv.org/abs/2607.17121)
*Sosuke Suzuki,Yijin Wei,Koichiro Kamide,Ran Dong,Haoran Xie,Chao Zhang*

Main category: cs.CV

TL;DR: 提出一个多分支骨架情感识别框架，结合6D旋转、部分感知运动和弱标签分布学习，在MMAC ACII 2026挑战的DIEM-A任务中显著提升准确率和Macro-F1。


<details>
  <summary>Details</summary>
Motivation: 基于骨架的情感识别因情感表达动态细微、关系运动线索复杂且硬标签难以捕捉类别模糊性而具有挑战性。

Method: 多分支框架：6D旋转分支、部分感知运动多流分支、元数据条件弱标签分布学习分支；独立训练后概率级集成。

Result: 10折留出交叉验证中，准确率从0.271提升至0.366，Macro-F1从0.252提升至0.353。可解释性分析显示速度和骨骼流、臂和腿区域是关键线索。

Conclusion: 所提框架有效利用多模态运动和标签分布信息，提升了基于骨架的情感识别性能，验证了关键运动线索的重要性。
...
### [218] [The generator is the tracker: Multi-object tracking by painting persistent identity colours](https://arxiv.org/abs/2607.17120)
*Haiyu Yang,Miel Hostens*

Main category: cs.CV

TL;DR: 本文提出一种基于视频生成器的多目标跟踪方法，通过文本到视频扩散模型直接生成ID映射视频，在不使用检测器、跟踪器或运动模型的情况下实现身份保持，在DanceTrack上达到40.3 HOTA。


<details>
  <summary>Details</summary>
Motivation: 探索视频生成器能否通过像素级颜色编码取代传统的跟踪状态管理（如轨迹缓冲区、运动模型和外观嵌入），实现身份保持和重识别。

Method: 微调22B文本到视频扩散模型（LTX-2.3），使用轻量级上下文LoRA将RGB视频转换为ID映射视频；通过链式窗口生成长视频，并利用连续微调实现身份跨窗口传播。

Result: DanceTrack测试集上HOTA为40.3，关联分数AssA 44.1超过原始基准套件中的所有跟踪器，而检测是主要短板；生成器在383个遮挡事件中42%实现身份重获取，远超基线方法。

Conclusion: 生成式跟踪器虽然整体性能低于专业方法，但具有独特的错误分布（高关联、低检测），并展现出基于颜色的身份保持作为涌现的重识别信号，为多目标跟踪提供了新范式。
...
### [219] [Denoising Models Develop Human-Like Perceptual Illusion Representations Across Architectures](https://arxiv.org/abs/2607.17138)
*Gautam Ranka,Paras Chopra*

Main category: cs.CV

TL;DR: 发现去噪模型内部存在对亮度错觉敏感的表示，这些表示在输出层不可见，被称为“感知幽灵”。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅从输出层面（如像素、分类）验证了深度神经网络对亮度错觉的响应，但未探索模型内部是否真正表征错觉及其机制。

Method: 通过分析去噪模型（多种架构）内部层和通道对错觉刺激与物理匹配控制区域的区分能力，结合心理物理模型（FLODOG）验证，并使用通道消融实验提供因果证据。

Result: 去噪目标比架构更重要；错觉敏感通道可追踪人类知觉模型（Spearman ρ≥0.70），并随错觉强度单调变化；消融这些通道显著影响内部信号，但操纵后不改变输出像素。

Conclusion: 去噪模型中存在内部-输出分离的错觉表示，首次在视觉处理模型中刻画了此类“感知幽灵”现象。
...
### [220] [STBridge: Shared-Target Alignment for Bridging Understanding and Generation in UMMs](https://arxiv.org/abs/2607.17140)
...
### [221] [BanClickThumb: A Multimodal Dataset and Transformer Fusion Benchmarks for Clickbait Detection in Bengali YouTube Videos](https://arxiv.org/abs/2607.17182)
*Md. Ariful Islam,Md Tanvirul Islam,Md. Maruf Hossain Miru,Md Khalid Syfullah*

Main category: cs.CV

TL;DR: 提出了一个包含7147个孟加拉语YouTube缩略图-标题对的数据集BanClickThumb，并验证了多模态融合方法在点击诱饵检测中的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有孟加拉语点击诱饵检测因缺乏公开的多模态数据集而面临挑战。

Method: 构建BanClickThumb数据集，采用VIT和XLM-RoBERTa的中间融合模型BanClickFusionFormer进行多模态检测。

Result: 多模态模型BanClickFusionFormer达到0.84准确率，优于单模态模型的0.82和0.68。

Conclusion: 多模态融合对孟加拉语点击诱饵检测有效，但密集文本、比喻语言和文化专用俚语仍是难点。
...
### [222] [VLA-ReID: Video-Level Association for Re-Identification in Multi-Object Tracking with Highly Similar Objects](https://arxiv.org/abs/2607.17157)
*Yanrong Qin,Xiaoyan Cao,Yao Yao*

Main category: cs.CV

TL;DR: 提出VLA-ReID方法，通过视频级关联建模和共性外观抑制，解决多目标跟踪中小而相似物体的身份切换问题，在BEE24数据集上显著提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有重识别模型采用单实例分配训练（实例级查询），但推理时需要全局分配（视频级查询），导致训练-推理不匹配，引发相似物体的身份切换。

Method: 提出视频级关联重识别（VLA-ReID），将重识别建模为视频级关联；包含帧共性外观估计（FCAE）和共性外观抑制（CAS），放大相似物体间差异。

Result: 在BEE24数据集上，HOTA提升1.1，MOTA提升0.3，AssR提升2.6，AssA提升0.7，IDF1提升0.8，身份切换减少28%。

Conclusion: 视频级重识别建模能有效改善外观关联的MOT性能，尤其适用于小且相似物体场景。
...
### [223] [Noise-Robust Box-Supervised Infrared Small Target Detection via Physics-Inspired Soft Label Optimization](https://arxiv.org/abs/2607.17148)
*Xizhe Zhang,Fan Shi,Mianzhao Wang,Jiangpeng Zheng,Xu Cheng,Shengyong Chen*

Main category: cs.CV

TL;DR: 提出了HALO方法，通过热斑锚定和高斯软标签将框监督转化为像素级软标签，解决了红外小目标检测中框监督的边界模糊和标注不确定性难题。


<details>
  <summary>Details</summary>
Motivation: 红外小目标像素级掩膜标注成本高且因目标边界模糊、纹理弱而存在固有不确定性，亟需利用更经济的框监督实现有效检测。

Method: 在框内根据局部背景统计约束定位辐射热斑锚点，围绕锚点合成物理锚定高斯软标签，离线生成后固定用于训练，不依赖在线标签更新。

Result: 在标准紧框下与现有框监督方法性能相当；在更松或偏移框（模拟真实场景）下鲁棒性显著更强，且不同骨干网络表现一致。

Conclusion: HALO有效解决了框监督的噪声问题，并在实际非理想标注场景中保持稳健；引入的污染感知工作区间分析揭示了信杂比与性能的关系。
...
### [224] [Cross-Coordinate Correspondence Pruning for Image-to-Point Cloud Registration](https://arxiv.org/abs/2607.17200)
*Xin Liu,Rong Qin,Huipeng Lin,Leizhi Shu,Jin Wu,Chi-Man Vong,Liang Lin,Jufeng Yang*

Main category: cs.CV

TL;DR: 提出了一种跨坐标对应剪枝（CCP）和多密度点集成（MDPE）策略，解决了图像到点云配准中因点云密度导致的粗对应质量权衡问题，在多个基准上实现配准召回率至少提升8.6%。


<details>
  <summary>Details</summary>
Motivation: 现有检测免方法在图像到点云配准中忽略了点云密度的关键作用，密度稀疏导致内点不足，密集则外点率高，形成固有权衡限制配准精度。

Method: 首先将粗对应投影到2D图像坐标统一空间，再用轻量级剪枝网络基于坐标几何和模态特征预测内点置信度以过滤外点；最后通过多密度点集成策略合并去重不同密度下的剪枝对应。

Result: 在多个基准上超越现有最先进方法，配准召回率至少提升8.6%。

Conclusion: 该方法有效缓解了密度权衡问题，显著提升了图像到点云配准的精度和鲁棒性。
...
### [225] [Induce to Empower: Improving Lightweight Baselines via Foundation Model Induction for Generalized Polyp Segmentation](https://arxiv.org/abs/2607.17208)
*Shivanshu Agnihotri,Snehashis Majhi,Deepak Ranjan Nayak,Dwarikanath Mahapatra,Debesh Jha*

Main category: cs.CV

TL;DR: 提出了Lite-Polyp Inductor (Lite-Pi)框架，通过基础模型诱导和轻量级模型结合，提升了息肉分割的泛化性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在息肉分割中难以直接应用（缺乏标注数据、计算量大），而轻量模型泛化能力不足。需要一种方法兼顾两者优势。

Method: 生成基础模型特有的原型表示，通过重建式监督与基础模型先验语义对齐，再利用Transformer融合突出息肉区域和边界信息。

Result: 在五个息肉分割数据集上显著提升了轻量模型性能，泛化性优秀且计算开销小。

Conclusion: Lite-Pi为实际临床应用提供了一种高效、通用的息肉分割解决方案。
...
### [226] [Semantic Context Matters: Analysis of Color Names Across Domains](https://arxiv.org/abs/2607.17221)
...
TL;DR: 本文通过分析化妆品、蜡笔和汽车颜色三个语境下的颜色命名数据集，发现语境显著影响颜色分类，蜡笔覆盖最广，化妆品集中在暖色调，汽车颜色集中于蓝色和消色区域。


<details>
  <summary>Details</summary>
Motivation: 颜色命名不仅受物理颜色值影响，还受语义语境影响，但现有研究缺乏对语境依赖性的系统性分析。

Method: 将三个颜色名称数据集映射到COLIBRI颜色模型的86个模糊颜色类别上，使用类别覆盖率、香农熵和最大提升度分析语境变化。

Result: 化妆品覆盖48个模糊类别，蜡笔覆盖50个，汽车颜色覆盖40个；蜡笔使用最广泛均衡，化妆品集中于暖色调，汽车颜色专攻蓝色和消色区域。
...
### [227] [VecFontLLM: Anchor-Guided Direct Synthesis of Chinese Vector Fonts](https://arxiv.org/abs/2607.17251)
*Hao Yuan,Yuxuan Luo,Xing Chen,Zhouhui Lian*

Main category: cs.CV

TL;DR: 提出了VecFontLLM，一种锚点引导的多模态大语言模型，用于直接小样本合成中文矢量字体。


<details>
  <summary>Details</summary>
Motivation: 中文矢量字形的生成面临复杂的组件结构、锚点布局和贝塞尔曲线细节，现有方法处理复杂字符时效果不佳。

Method: 利用锚点引导大语言模型：先预测并细化锚点骨架固定粗粒度布局，再完成贝塞尔控制点恢复局部曲率和风格；测试时通过置信度引导生成链提高稳定性。

Result: 在多个中文数据集上显著优于现有矢量字体合成方法，渲染质量与光栅域基线相当，矢量命令分布接近真实字体。

Conclusion: 首次实现了无需光栅生成或矢量化的高质量小样本复杂中文矢量字体直接合成。
...
### [228] [PACE: Polar Axis-Conditioned Estimation for PairUAV Relative Localization](https://arxiv.org/abs/2607.17268)
*Ze Rong*

Main category: cs.CV

TL;DR: 提出PAAER方法，利用极轴条件估计分离UAV相对定位中的航向和距离解码，性能优于统一方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法将航向和距离作为同质坐标处理，导致共享解码器证据和优化状态，但两者在最佳检查点、误差分布上存在结构差异。

Method: Polar Axis-Conditioned Estimation：共享PairUAV表示，但为航向和距离分配不同的解码接口——航向使用中/晚期关系证据，距离使用直接晚期度量路径。

Result: 隐藏测试中，原始预测器得分0.002460，PAAER得分0.002514（角度误差更低），结合确定性挑战打包后最终得分0.001874。

Conclusion: 分离极轴解码能更好适应航向和距离的不同结构，提升相对定位精度。
...
### [229] [MIS-HCC: Hierarchical Channel Clustering for Efficient Medical Image Segmentation](https://arxiv.org/abs/2607.17329)
*Bo Zhao,Haoran Yu,Lifei Liu,Zongcheng Chu,Yining Liu,Chang Liu,Szu-Yu Chen,Zequn Xie*

Main category: cs.CV

TL;DR: 提出了一种用于医学图像分割模型的分层聚类压缩方法（MIS-HCC），通过通道聚类与参数融合实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法忽略网络内部结构关联，导致压缩模型难以保留预训练网络的基本特征。

Method: 使用 Wasserstein 距离计算层内通道相似度矩阵，基于分层聚类对相似通道进行分组并融合参数，生成压缩网络。

Result: 在三个医学图像数据集上，MIS-HCC 在准确率和压缩效率上均优于现有方法。

Conclusion: MIS-HCC 为资源受限平台上的医学图像分割模型部署提供了有效解决方案。
...
### [230] [Orthogonal Knowledge Refreshing for Domain-Incremental Object Detection](https://arxiv.org/abs/2607.17340)
*Aoting Zhang,Dongbao Yang,Chang Liu,Xiaopeng Hong,Can Ma,Yu Zhou*

Main category: cs.CV

TL;DR: 提出正交知识刷新（OKR）框架，通过构建独立的低秩子空间和梯度正交投影策略，解决域增量目标检测中的知识干扰与遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法在域增量目标检测中会导致跨域干扰和性能下降。

Method: 使用专用低秩分支构建独立子空间，通过梯度正交投影到历史子空间的正交补空间，并引入拓扑感知一致性对齐语义结构。

Result: 在Pascal VOC和BDD100K系列上分别比最佳无样本方法高出+5.6%和+6.5% mAP。

Conclusion: OKR能有效减轻知识干扰，实现无遗忘的持续学习与冲突无关的容量扩展。
...
### [231] [Understanding From Human Perspective: A Multi-agent System for Interactive Egocentric Medical Image Segmentation](https://arxiv.org/abs/2607.17341)
*Rongjun Ge,Dongyang Wang,Heng Zhu,Zhirui Li,Yang Chen,Yuting He*

Main category: cs.CV

TL;DR: 提出了EgoMed-Agent多智能体系统，通过目标确认与定位引导传播两个工作流，解决第一人称视角下医学图像分割的语义模糊和视觉变化问题，在Dice指标上远超基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法处理用户第一人称视角下的语义模糊和视觉变化，导致分割目标不明确且帧间剧烈跳动。

Method: 采用多智能体架构，包含目标确认工作流（通过可靠性评分结合用户澄清）和定位引导传播工作流（通过目标定位校正传播掩码）。

Result: 在Dice指标上达到71.34%，远超最佳文本提示基线方法的11.70%。

Conclusion: EgoMed-Agent有效解决了第一人称医学图像分割中的语义歧义和视觉变化问题，显著提升分割准确性和稳定性。
...
### [232] [STAR: Skeletal Token Alignment and Rearrangement for Interaction Recognition](https://arxiv.org/abs/2607.17342)
*Yuhang Wen,Mengyuan Liu,Zixuan Tang,Junsong Yuan,Sirui Li,Beichen Ding*

Main category: cs.CV

TL;DR: 提出了STAR框架，通过骨架与RGB视频对齐学习交互特征，仅用骨架即可高效识别。


<details>
  <summary>Details</summary>
Motivation: 现有基于骨架的人机交互方法缺乏视觉信息，难以有效利用交互线索。

Method: 设计骨架编码器（实体重排+交互时空令牌）、视觉编码（焦点交互策略），并通过对比学习对齐表示。

Result: 在Chico、HARPER、NTU Mutual 11/26数据集上优于现有方法。

Conclusion: STAR在仅使用骨架时仍能保持视觉信息增强的鲁棒性，实现高效交互识别。
...
### [233] [SkyVLaM: Multimodal Large Language Model for UAV Video Understanding in Remote Sensing](https://arxiv.org/abs/2607.17386)
*Kaiwen Jing,Ruixu Jia,Bingyao Li,Ruizhe Ou,Ming Wu,Chuang Zhang*

Main category: cs.CV

TL;DR: 提出了SkyVLaM模型，通过稀疏和密集令牌联合处理，提升了无人机视频中语言引导的分割效果。


<details>
  <summary>Details</summary>
Motivation: 无人机视频中的目标小、视觉模糊、视角动态，传统方法难以处理语言引导的分割任务。

Method: 构建时间基底感知器提取稀疏令牌，正则化稀疏基底以增强时间线索，并自适应选择密集分割区域，通过大语言模型联合处理查询条件分割。

Result: 在SkyVid数据集（101视频，33.6K帧，1.53M实例）上，SkyVLaM有效分配视觉令牌预算，提升了无人机视频分割性能。

Conclusion: SkyVLaM为无人机视频中的语言引导分割提供了更优的视觉令牌分配策略。
...
### [234] [Automating Visual Recognition of Leprosy in Wild Chimpanzees](https://arxiv.org/abs/2607.17395)
...
TL;DR: 本文首次提出深度学习流水线用于野生黑猩猩麻风病自动检测，并构建PanLep300数据集，发现基于裁剪图像预测的简单聚合方法优于复杂的时间模型。


<details>
  <summary>Details</summary>
Motivation: 传统人工审核摄像机陷阱影像不可行，亟需自动化筛查方案。

Method: 对比了空间（2D）、时间聚合（2.5D）和视频（3D）三种分类方法，并针对部分可见个体进行优化。

Result: 简单聚合方法优于复杂时间模型，部分可见帧会抑制性能但可通过定向构建和聚合策略缓解。

Conclusion: 对于静态皮肤病变，简单聚合方法足够有效，无需复杂时间建模。
...
### [235] [The PanAf-SBR Dataset: Social Behaviour Recognition for Wild Great Apes](https://arxiv.org/abs/2607.17399)
...
TL;DR: 提出首个野生类人猿相机陷阱数据集PanAf-SBR，标注了社交行为，并建立了细粒度社交行为识别基准。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏野生类人猿的细粒度社交行为标注，且多来自圈养环境或无人机视角，无法准确监测野外种群的社会结构变化。

Method: 扩展PanAf500数据集，新增36,063帧视频和81,096个标注（边界框、分割掩码、个体身份、七类社交行为）；采用AlphaChimp架构建立基准，并进行跨数据集迁移学习与背景消融实验。

Result: 跨数据集预训练对特定社交行为类别有显著提升，非普遍效益；背景抑制实验显示非猿类像素对识别有影响。

Conclusion: PanAf-SBR填补了野生类人猿社交行为识别数据的空白，为早期预警种群下降提供了自动化工具基础。
...
### [236] [TimeLens2: Generalist Video Temporal Grounding with Multimodal LLMs](https://arxiv.org/abs/2607.17423)
...
TL;DR: 提出TimeLens2，一种将时间证据视为区间集合进行监督和优化的通用视频时间定位方法，在多个基准上超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频多模态大模型能描述视频内容，但很少能识别事件发生的具体时间；现有的训练策略与集值型时间定位任务不匹配。

Method: 构建TimeLens2-93K多跨度标注数据集，设计时间Wasserstein奖励函数W1和IoU联合优化，实现无匹配的密集反馈。

Result: TimeLens2-2B在所有基准上超越同规模模型，4B和8B版本达到最先进性能，超越397B参数的开源模型；相比Qwen3-VL提升14.2-18.1 mIoU。

Conclusion: 将时间证据视为区间集合的监督和优化策略有效提升了视频时间定位的通用性和准确性。
...
### [237] [Does Super-Resolution Preserve Defect Evidence? A Low-False-Call Benchmark for Semiconductor Inspection](https://arxiv.org/abs/2607.17401)
*Shaoliang Yang,Jun Wang*

Main category: cs.CV

TL;DR: 超分辨率虽能提升图像清晰度，但可能破坏缺陷检测所需的证据。本文通过基准测试发现，重建质量高的模型在缺陷检测中表现差于双三次插值，而直接检测器效果更优。结论强调应采用任务证据保留和操作点迁移来评估超分辨率方法。


<details>
  <summary>Details</summary>
Motivation: 超分辨率在工业检测中可能使图像看起来更清晰，但掩盖了缺陷检测所需的关键细节，现有评估仅关注重建质量，忽视了检测性能。

Method: 构建基准测试，分离重建与检测阶段；在低假阳性率下评估；使用独立生成的线/空间与接触孔图像；对比学习型重建模型、直接检测器（DeepLabV3）和联合模型（DPU-WaferSR）；应用公共数据集迁移验证。

Result: 学习型重建模型的SSIM最高，但缺陷像素召回率低于双三次插值；DeepLabV3检测器在0.000174低假阳性率下达到0.1984召回率；联合模型校准成功但迁移失败；弱缺陷召回率接近零。

Conclusion: 超分辨率用于检测时，应以任务证据保留和操作点迁移为标准，而非仅依赖重建质量。
...
### [238] [An Explainable FFT-Based Spatial-Frequency Fusion Framework for Deepfake Detection](https://arxiv.org/abs/2607.17441)
*Pamela Kirui,Cho Hyuk,Qingzhong Liu,Haodi Jiang*

Main category: cs.CV

TL;DR: 提出了基于FFT的多尺度交叉注意力框架用于图像级深度伪造检测，结合了部分微调的Xception空间分支和FFT频率分支，通过交叉注意力融合和MLP分类，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 深度伪造生成引发对数字媒体真实性、错误信息和身份欺诈的担忧，现有研究表明空间与频率特征结合能提升检测效果。

Method: 采用部分微调的Xception作为空间分支，FFT变换后取对数缩放幅度谱并通过浅卷积层作为频率分支，两分支经Transformer编码器处理，通过交叉注意力融合后由MLP分类器预测。

Result: 在性能上持续优于基于DCT的先进融合方法和基线模型，消融实验证实FFT频率分支提供互补的光谱信息。

Conclusion: FFT频率分析与可解释性方法（Grad-CAM/LIME）一致显示，模型关注眼部、口部、鼻子和面部边界等操纵敏感区域，验证了方法的有效性。
...
### [239] [DA-MergeLoRA: Hypernetwork-Based LoRA Merging for Few-Shot Test-Time Domain Adaptation](https://arxiv.org/abs/2607.17467)
*Siobhan Reid,Zhixiang Chi,Li Gu,Omid Reza Heidari,Ziqiang Wang,Yang Wang*

Main category: cs.CV

TL;DR: 提出了一种融合LoRA微调与模型合并的FSTT-DA框架，通过超网络生成合并因子，在少量无标签目标样本下实现域适应，跨数据集达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有FSTT-DA方法未能有效利用源域知识，依赖浅层BN更新、黑盒提示或忽视跨域关系的集成策略。

Method: 对每个源域在CLIP视觉编码器上微调独立LoRA模块，保留基础知识；通过元训练的超网络为给定目标样本生成每列合并因子，融合多个源域LoRA。

Result: 在多个域适应数据集上取得SOTA性能，代码已开源。

Conclusion: LoRA+模型合并策略有效利用源域知识并适应目标域，优于现有方法。
...
### [240] [TraversRL: Traversable Pedestrian Pathway Generation With Reinforcement Learning](https://arxiv.org/abs/2607.17479)
*Bin Han,Robert Wolfe,Bill Howe*

Main category: cs.CV

TL;DR: 提出TraversRL模型，通过强化学习模拟行人视角，逐步生成高连通性的人行道网络，显著优于传统分割方法。


<details>
  <summary>Details</summary>
Motivation: 现有分割方法生成的人行道图谱常存在断裂或碎片化问题，难以用于导航路线规划。

Method: 采用视觉条件模型，迭代生长路径网络；定义短程和长程方向-距离动作空间并设计图级与步级混合奖励优化全局连通性。

Result: 在三种视觉骨干和三个交叉路口数据集上，缓冲IoU显著提升，连通性指标提高一倍以上，且全局-局部奖励组合减少多余分支。

Conclusion: 将路径提取建模为行人的序贯决策过程并结合强化学习优化，可生成可靠的人行道网络。
...
### [241] [Bio-SFT: Asymmetric Cortical Guidance and Retinal Adaptation for Robust HDR Reconstruction](https://arxiv.org/abs/2607.17456)
*Tingyu Cheng,Ting Zhang,Chongyi Li,Zhaoqing Pan,Tiesong Zhao*

Main category: cs.CV

TL;DR: 提出Bio-SFT，一种受生物启发的脉冲频率变压器，用于从单张SDR图像恢复HDR辐射度，通过模拟视网膜适应、Parvo-Magno通路分离和事件驱动的SNN硬门控，在HDRTV1K数据集上实现了竞争性的感知质量并改进了HDR-VDP-3和ΔE_ITP指标。


<details>
  <summary>Details</summary>
Motivation: 从单张标准动态范围图像恢复高动态范围辐射度是高度病态的，极端亮度变化和暗区严重量化导致重建困难，产生视觉伪影和颜色失真。

Method: 提出Bio-SFT，包含三个生物启发模块：可学习的Naka-Rushton视网膜适应前端稳定输入，显式Parvo-Magno分离引入非对称引导，以及事件驱动的SNN硬门控模块抑制暗区噪声并保留结构细节，以稀疏先验训练。

Result: 在HDRTV1K数据集上，Bio-SFT实现了竞争性的感知质量，一致改进了HDR-VDP-3和ΔE_ITP指标，并减少了对称引导管道中的伪影传播。

Conclusion: 受生物启发的轻量级组件在端到端训练中展现出强参数效率，为单图像HDR重建提供了有效方案。
...
### [242] [Coarse-to-fine Framework for Generative MEF via Implicit Neural Representation](https://arxiv.org/abs/2607.17611)
*Sangmin Han,Jinho Kim,Jinwoo Kim,Dongyoung Kim,Seon Joo Kim*

Main category: cs.CV

TL;DR: 提出LIIFusion粗到细框架，在生成式多曝光融合中平衡质量和效率，速度提升3.5倍且保持结构保真度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散生成方法计算昂贵且在饱和区域难以保留精细结构，需要更高效的生成式MEF方案。

Method: 粗阶段进行低分辨率生成融合并引入自适应曝光校正；细阶段将局部隐式图像函数适配为多曝光融合函数，跨分辨率融合源证据。

Result: 相比现有生成方法速度提升3.5倍，同时维持或改进结构保真度和感知质量。

Conclusion: 该框架为生成式MEF在实际应用中提供了有效途径。
...
### [243] [Generative Transmission: Rethinking Computation, Bandwidth, and Memory in Communication](https://arxiv.org/abs/2607.17482)
*Xiangyu Chen,Jixiang Luo,Yuankai Fan,Haibin Huang,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: 本文提出了一种在超低带宽和弱网络条件下用于视频通信的生成式传输（GenTrans）方法，通过结合生成式视频压缩、跨片段内存重用、运行时状态重用和弱网络感知传输，实现了带宽、计算和内存的联合优化，在降低传输开销的同时保持视觉连贯性和实际可用性。


<details>
  <summary>Details</summary>
Motivation: 传统视频编码和传输方法在超低带宽和弱网络条件下难以平衡视觉可用性、传输效率和链路鲁棒性，需要向接收端感知效用和系统级可用性转变。

Method: 采用生成式视频压缩（GVC）框架，将视频传输建模为带宽、计算和内存的联合优化问题，利用生成式先验、跨片段内存重用、运行时状态重用和弱网络感知传输策略。

Result: 实验表明，GenTrans在超低比特率和弱网络条件下支持有效的视频传输，提升了传输效率、解码效率和鲁棒性，同时保持了感知质量。

Conclusion: 在AI Flow框架下，视频通信正从保真度导向的信号重建转向任务导向和感知导向的令牌流传输，GenTrans验证了生成式方法在极端网络条件下的可行性和优势。
...
### [244] [FlexiGrad: Adaptive Gradient Modulation for Hierarchical Fine-Grained Classification](https://arxiv.org/abs/2607.17563)
*Zilu Zhou,Dongliang Chang,Junhan Chen,Zhanyu Ma*

Main category: cs.CV

TL;DR: 提出了 FlexiGrad，一种无参数方法，通过调节梯度冲突来稳定层级标签下的细粒度识别训练，提升多粒度准确率。


<details>
  <summary>Details</summary>
Motivation: 层级标签（如目、科、种）在细粒度识别中导致粗粒度与细粒度分类器对共享骨干网络产生不一致梯度，造成训练不稳定，影响表示学习。

Method: FlexiGrad 在反向传播中通过平滑权重函数，移除有害冲突梯度，增强部分一致梯度，实现稳定优化。

Result: 在 CUB-200-2011、FGVC-Aircraft 和 Stanford Cars 数据集上提升了多粒度准确率。

Conclusion: FlexiGrad 能有效解决层级梯度冲突，无需修改架构，提升细粒度识别性能。
...
### [245] [DecoyFace: Beyond Obfuscation via Controllable and Imperceptible Identity Misdirection for Privacy-Preserving Face Recognition](https://arxiv.org/abs/2607.17504)
*Zhihan Ren,Lijun He,Xinyao Wang,Xinzhu Fu,Fan Li*

Main category: cs.CV

TL;DR: 提出DecoyFace框架，通过分解表示空间并注入诱饵身份线索，在保护识别效用的同时，使未经授权的重构生成看似合理但错误的身份。


<details>
  <summary>Details</summary>
Motivation: 现有隐私保护人脸识别方法主要抵抗未授权重构，但重构结果明显降质，暴露保护存在并可能引发自适应攻击。

Method: 将中间表示分解为重构敏感子空间和互补子空间；客户端在重构敏感子空间注入诱饵身份线索，互补子空间保留真实样本识别信息；服务端通过规范化模块抑制诱饵成分恢复可用表示。

Result: 保持竞争性识别精度，将身份泄露降低至2.93%（U-Net攻击）和0.74%（Flow-Matching攻击），生成的诱饵重构视觉上合理且不可感知，LFW数据集上人脸有效性超过99.78%。

Conclusion: DecoyFace有效抵御特征反转和半诚实服务器重构攻击，同时生成不可感知的诱饵重构，提升隐私保护隐蔽性。
...
### [246] [Hierarchy-Aware and Anatomy-Guided Learning for Lung Ultrasound Video Classification](https://arxiv.org/abs/2607.17551)
*Alya Almsouti,Lotfi Mecharbat,Noha Aboukhater,Yousef Alabrach,Siddiq Anwar,Andre Kumar,Ibrahim Almakky,Mohammad Yaqub*

Main category: cs.CV

TL;DR: 提出结合层级感知训练与解剖引导的深度学习框架，实现肺部超声视频的多分类（健康、B线、实变、混合），在公开数据集上达到65.7%的宏观F1分数，并具有可解释的注意力定位。


<details>
  <summary>Details</summary>
Motivation: 肺部超声自动分析受散斑噪声、伪影及操作者差异影响，现有方法缺乏针对临床结构（如病理层级、解剖区域）的指导。

Method: 在强基线模型基础上，引入分层训练策略（层级感知）和胸膜线掩膜监督（解剖引导），通过区域注意力约束提升模型对解剖相关区域的关注。

Result: 层级感知训练优于扁平分类；掩膜引导注意力监督实现最高65.7%宏观F1，并生成更聚焦的注意力图；在外部COVID-BLUeS数据集上具有竞争力和参数高效迁移性。
...
### [247] [DA-Fusion: Deformable Attention-Based RGB-D Fusion Transformer for Unseen Object Instance Segmentation](https://arxiv.org/abs/2607.17754)
*Yesol Park,Hye-Jung Yoon,Juno Kim,Byoung-Tak Zhang*

Main category: cs.CV

TL;DR: 提出DA-Fusion，一种基于可变形注意力的RGB-D融合Transformer，用于未见物体实例分割，并引入OCBD数据集，在杂乱环境物流自动化中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统RGB方法因依赖纹理而过分割，深度方法因侧重几何特征而欠分割，现有方法在杂乱和多层物体环境中分割精度不足。

Method: 采用可变形注意力机制的RGB-D融合Transformer（DA-Fusion），结合RGB纹理与深度几何信息；构建OCBD数据集用于顶视拣选场景评估。

Result: 在多种杂乱环境中，DA-Fusion分割精度优于现有最先进方法。

Conclusion: DA-Fusion有效融合RGB和深度数据，适合真实物流任务中的未见物体实例分割。
...
### [248] [Scalable Model-Assisted Multi-Target Estimation in Large Image Collections](https://arxiv.org/abs/2607.17581)
*Max Hamilton,Jinlin Lai,Daniel Sheldon,Subhransu Maji*

Main category: cs.CV

TL;DR: 提出了一种结合模型预测和有限人工标注的多目标估计框架，通过抽样策略实现无偏估计，适用于科学测量。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉模型在估计群体量时存在预测偏差且缺乏统计保证，而先前的蒙特卡罗方法只能处理单标量估计。

Method: 将抽样调查中的策略（重要性抽样、均匀抽样与控制变量、子集比率估计）应用于多目标估计，结合模型预测与人工标注。

Result: 在5个检测/分割数据集（7-80类）上，重要性抽样在中等标注预算下表现最佳；均匀抽样+控制变量在目标多或标注少时占优；子集比率估计在所有场景下表现优异。

Conclusion: 该框架能有效融合有偏模型预测与有限人工标注，为科学测量提供统计严谨的估计。
...
### [249] [Measuring Monosemanticity in Sparse Autoencoders via Latent Activation Coherence](https://arxiv.org/abs/2607.17770)
*Katarzyna Filus,Sebastian Pokuciński*

Main category: cs.CV

TL;DR: 提出一种无标签的单语义性度量指标 TMS，基于激活集一致性评估 SAE 特征，避免了对嵌入编码器的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有单语义性评估指标需要外部概念标签或依赖预训练嵌入模型，易受编码器几何结构影响，缺乏稳健的标签无关度量。

Method: 定义 Tversky Monosemanticity Score (TMS)，通过二值化 SAE 潜在激活的集合一致性计算单语义性，无需外部编码器。

Result: TMS 比基于嵌入的替代方法更不受编码器各向异性影响，与已有单语义性指标一致，并能揭示不同基础模型间的 SAE 训练动态差异。

Conclusion: TMS 是有效的无标签单语义性度量，在编码器各向异性下对概念删除有效性有更强指示，其他情况下竞争力相当。
...
### [250] [Pixel-Space Diffusion Transformers](https://arxiv.org/abs/2607.17585)
...
TL;DR: 综述像素空间扩散变换器(pDiTs)在高保真图像生成中的架构、机制与多模态统一建模进展。


<details>
  <summary>Details</summary>
Motivation: 隐空间扩散模型存在纹理细节丢失、重建与生成目标不匹配等问题，促使重新探索像素空间扩散以实现端到端优化。

Method: 从模型架构、连续生成机制和统一多模态建模三个角度综述pDiTs的代表性方法。

Result: 总结了pDiTs在解决高维建模挑战（如噪声调度、损失加权、标记效率和可扩展架构）方面的进展，并展示了其在统一视觉理解与生成中的潜力。

Conclusion: 像素空间扩散是迈向高保真、端到端视觉基础模型的关键方向，未来需进一步解决高维建模与统一多模态系统的技术挑战。
...
### [251] [CDIS: Cross-Dimensional Class-Agnostic 3D Instance Segmentation via 2D Mask Tracking and 3D-2D Projection Merging](https://arxiv.org/abs/2607.17778)
*Juno Kim,Hye-Jung Yoon,Yesol Park,Byoung-Tak Zhang*

Main category: cs.CV

TL;DR: 提出了一种跨维度的零样本3D实例分割框架CDIS，通过追踪2D掩码并与3D超点关联，实现全局一致的3D实例分割，无需3D训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法逐帧投影2D掩码到3D并合并，导致对象身份跨时间断裂和3D实例碎片化，需要更好的跨维度关联。

Method: 在2D帧间追踪实例掩码，并与3D超点关联，形成2D-3D反馈环，利用时间稳定的2D轨迹和空间一致的3D区域生成全局一致的3D标签。

Result: 在基准数据集上，CDIS相比最先进零样本方法获得更高准确性和一致性，且高效可扩展。

Conclusion: CDIS通过跨维度推理有效解决了现有方法的碎片化问题，实现了无需3D训练的鲁棒3D实例分割。
...
### [252] [ConsiSpace: Learning Geometric Consistency Matters for Video Spatial Reasoning](https://arxiv.org/abs/2607.17599)
*Ting Huang,Zhenyu Zhang,Wenyuan Huang,Jian Yang,Hao Tang*

Main category: cs.CV

TL;DR: 提出ConsiSpace框架，通过几何一致性感知实现视频空间推理，在三个基准上平均提升12.6分。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM语义中心化，难以从冗余视频中聚合一致空间证据，导致推理低效不稳定。

Method: 构建几何一致记忆（GCM）和统一一致性自监督强化学习（UC-SSRL），包括隐式证据令牌和显式几何线索。

Result: 在VSI-Bench、OSI-Bench、MMSI-Video-Bench上一致提升，平均得分超最强基线12.6分。

Conclusion: 空间一致性可作为证据组织原则和SFT后学习信号，显著提高跨视角稳定性。
...
### [253] [Miles: Metric Learning with Expandable Subspace for Pre-Trained Model-Based Class-Incremental Learning](https://arxiv.org/abs/2607.17593)
*Kai Jiang,Zisong Lin,Hongyuan Zhang,Xueru Bai,Xuelong Li*

Main category: cs.CV

TL;DR: 本文提出 Miles 方法，通过解耦可学习模块与预训练模型，利用中间特征先验信息实现参数空间的高效扩展，并使用中心损失和距离正则化项平衡任务间度量，在六类基准数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练模型（PTM）的类增量学习方法在性能与计算开销之间难以平衡：要么使用相同参数空间导致灾难性遗忘，要么为每个任务扩展新分支增加计算成本。

Method: 提出度量学习与可扩展子空间（Miles）方法，解耦可学习模块与预训练模型，利用骨干网络中间特征的先验信息灵活扩展参数空间；采用中心损失引导新类别向任务子空间的原型聚类，并添加辅助距离正则化项维持任务间度量平衡。

Result: 在六个基准数据集上的大量实验表明，Miles 在多种类增量学习设置中达到最优性能。

Conclusion: Miles 通过高效利用预训练先验知识，在避免灾难性遗忘和降低计算开销的同时，实现了性能提升。
...
### [254] [BrainNext: A General-Purpose Self-Supervised Foundation Model for Brain MRI Analysis](https://arxiv.org/abs/2607.17782)
*Moona Mazher,Abdul Qayyum,Steven A. Niederer,Daniel C. Alexander*

Main category: cs.CV

TL;DR: BrainNext是一个针对脑部MRI分析的自监督基础模型，结合MAE预训练和3D Bi-Directional xLSTM-UNet架构，在60,551个未标记脑部MRI扫描上预训练，并在FOMO 2025挑战中取得第二名，脑膜瘤分割任务第一名。


<details>
  <summary>Details</summary>
Motivation: 现有神经影像基础模型受限于任务特定训练、切片学习策略或较小的预训练数据集，泛化能力不足。

Method: 使用掩码自编码器（MAE）预训练，结合原生三维双向xLSTM-UNet架构，在60,551个多模态脑部MRI扫描上学习表示，并通过轻量级微调适应下游任务。

Result: 在FOMO 2025方法赛道中总体排名第二，脑膜瘤分割任务排名第一，展示了跨分类、分割和脑龄估计任务的强迁移能力。

Conclusion: 大规模自监督预训练能学习鲁棒且可迁移的体素级表示，BrainNext成为可扩展的脑部MRI基础模型。
...
### [255] [Medical Imaging Fusing Vision Transformer: Laryngeal Cancer Screening with Explanation](https://arxiv.org/abs/2607.17789)
*Haiyang Wang,Luca Mainardi*

Main category: cs.CV

TL;DR: 提出使用Transformer和注意力机制分析窄带成像，区分喉部良性及恶性病变，结合MedSAM分割实现可解释性，分类F1达82.72%，准确率82.33%。


<details>
  <summary>Details</summary>
Motivation: NBI内镜依赖临床医生经验且耗时、观察者变异大，AI可辅助决策并提升筛查效率与一致性。

Method: 采用Transformer与注意力机制进行NBI图像分类，并利用MedSAM分割模型提供病理信息区域作为可解释性依据。

Result: 分类性能F1为82.72%，准确率为82.33%，且筛查结果具有临床可解释性。

Conclusion: 融合分类与分割的方法为喉癌筛查提供了可行的AI辅助方案，兼具性能与可解释性。
...
### [256] [ReViV: Reconstructing the Viewer and the View in 4D from Monocular Egocentric Video](https://arxiv.org/abs/2607.17790)
*Xiaozhong Lyu,Gen Li,Zhiyin Qian,Xucong Zhang,Marc Pollefeys,Siyu Tang*

Main category: cs.CV

TL;DR: 提出ReViV，首个从单目RGB视频中统一重建4D自我中心表征的框架，同时恢复人体运动、手部动作、视线、相机轨迹和深度。


<details>
  <summary>Details</summary>
Motivation: 现有自我中心4D重建方法依赖辅助输入（如预计算相机轨迹），将场景感知与人体运动分离建模，且推理速度慢。

Method: 使用掩码生成自我中心Transformer（Masked Generative Egocentric Transformer）学习多模态信号联合概率分布，单次前馈实现4D重建。

Result: 在HoloAssist、HOT3D、ARCTIC等多个基准上，在全身、手部、视线重建及相机跟踪上达到SOTA精度，深度估计同样具有竞争力。

Conclusion: ReViV实现了高效、统一的4D自我中心重建，无需特定任务先验，代码和模型已开源。
...
### [257] [Semantic Color Naturalness Breaker: Preventing Illegitimate Colorization via Content-Aware Color Priors](https://arxiv.org/abs/2607.17610)
*Yuki Nii,Futa Waseda,Ching-Chun Chang,Isao Echizen*

Main category: cs.CV

TL;DR: 针对灰度图像未经授权着色问题，提出语义级不可着色示例框架(SCNB)，通过添加扰动使自动着色输出颜色与内容不一致，同时保持发布灰度图像视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 自动图像着色技术可能被用于未经授权地着色和传播灰度内容（如漫画、档案照片），亟需在发布时进行主动防护。

Method: 提出语义颜色自然性破坏器（SCNB）框架，利用语义级扰动破坏着色模型输出；引入内容感知颜色分布距离（CaCDD）作为优化目标和评估指标。

Result: 在ImageNet上，SCNB在较小扰动预算和常见后处理条件下仍保持有效性。

Conclusion: 该方法适用于真实内容共享场景中灰度媒体的保护，具有实际部署价值。
...
### [258] [Brain-Aligned Multi-Stream Video Transformers with Sparse Self-Selection](https://arxiv.org/abs/2607.17625)
*Amir Hosein Fadaei,Mahyar Maleki,Mohammad-Reza A. Dehaqani*

Main category: cs.CV

TL;DR: 提出神经启发式的稀疏胜者全拿令牌选择模块和分离融合视频变压器，在效率和神经对齐上优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频变压器忽略了灵长类视觉原理，且缺乏与神经数据的比较，限制生物可解释性。

Method: 引入稀疏胜者全拿令牌选择模块替代密集自注意力；设计分离融合架构，包含高分辨率低帧率“什么”通路和低分辨率高帧率“哪里”通路，在分类前融合。

Result: 在Kinetics-400和Something-Something V2上达到精度与推理时间的帕累托最优；脑-模型相关性峰值0.18，达噪声天花板的78%，优于强基线模型。

Conclusion: 通路专门化和稀疏竞争是高效、脑对齐视频理解的有用归纳偏差。
...
### [259] [Rarity-Aware Discrete Diffusion with Spatially Consistent Decoding for Photo-Realistic Image Super-Resolution](https://arxiv.org/abs/2607.17612)
*Ao Li,Yapeng Du,Yi Xin,Lei Zhu,Le Zhang,Guangtao Zhai,Ce Zhu,Xiaohong Liu*

Main category: cs.CV

TL;DR: 提出了一种基于稀有性感知的多模态离散扩散框架DiMOO-SR，通过逆频率采样和空间一致性排序，在少量并行解码步骤下实现与连续扩散模型竞争的超分辨率效果。


<details>
  <summary>Details</summary>
Motivation: 连续扩散模型在超分辨率中依赖外部条件模块，难以利用统一的多模态分词范式；自回归模型因果解码效率低；直接应用离散扩散面临视觉令牌长尾分布和空间不一致并行解码问题。

Method: 采用稀有性感知的多模态离散扩散框架，训练阶段使用逆频率采样优先处理稀有但信息丰富的令牌；推理阶段通过空间一致性排名利用局部邻域一致性优化令牌置信度。

Result: 在真实世界超分辨率基准上，仅需少量并行解码步骤即可达到与连续扩散模型相当的感知质量。

Conclusion: 离散扩散在生成式图像超分辨率中具有潜力，能高效实现并行解码并保持感知质量。
...
### [260] [Vis2Reg: Visibility-Aware Landmark-Free Geometric 3D--2D Registration for Liver Laparoscopy](https://arxiv.org/abs/2607.17810)
*Jiaming Feng,Xukun Zhang,Shahid Farid,Sharib Ali*

Main category: cs.CV

TL;DR: 提出一种可见性感知的配准框架Vis2Reg，通过掩码引导的自我监督实现部分到完整的3D-2D肝脏配准，在真实术中数据集上Dice=92.6%，CD=1.43mm，推理速度111ms/帧。


<details>
  <summary>Details</summary>
Motivation: AR引导腹腔镜手术中，3D-2D肝脏配准因严重遮挡、视野有限且缺少3D真值监督而极具挑战，现有无标注方法在极端部分可见性下鲁棒性不足。

Method: 采用可见性感知自监督，通过可微点光栅化和掩码引导的反投影从术中掩码导出3D监督信号；结合几何刚性初始化模块与隐式神经变形场，仅依赖自监督学习。

Result: 真实术中数据集上Dice分数92.6%，Chamfer距离1.43mm，每帧推理时间111ms，兼顾精度与效率。

Conclusion: Vis2Reg框架在严重遮挡下实现高精度、高效率的3D-2D肝脏配准，为术中增强现实导航提供实用解决方案。
...
### [261] [MixDiffusion: Mixing Diffusion-based Uni-condition Text-to-Image Generation Models for Multi-condition Image Synthesis](https://arxiv.org/abs/2607.17634)
*Pengcheng Wan,Liang Han,Lin Xu,Bowen Xiao,Liqiang Nie*

Main category: cs.CV

TL;DR: 提出MixDiffusion，一个无需训练的多条件文本到图像生成框架，支持任意数量的控制条件。


<details>
  <summary>Details</summary>
Motivation: 现有扩散方法仅支持单种控制条件（如边界框或关键点），灵活性不足。

Method: 通过推导多条件扩散模型的预测噪声分布公式，集成多个预训练的单条件扩散模型实现多条件协同。

Result: MixDiffusion支持边界框、关键点、草图、深度图、参考图像和文本等多种条件，且部署简单、易于扩展。

Conclusion: MixDiffusion通过理论推导的噪声分布集成方法，成功实现无需训练的多条件图像生成。
...
### [262] [HAS: Highlight-guided Attention Steering for Multimodal LLM Video Summarization](https://arxiv.org/abs/2607.17994)
*Rui Chu,Yingjie Lao*

Main category: cs.CV

TL;DR: 提出了HAS（高亮引导注意力导向）方法，通过全局连续帧级高亮分布引导MLLM注意力，实现连贯视频摘要。


<details>
  <summary>Details</summary>
Motivation: 现有视频摘要方法仅聚焦孤立关键帧，忽略全局重要性，导致理解不连贯且信息丢失。

Method: HAS包含两步：1）计算视频全局连续的帧级高亮分布；2）将该分布作为注意力导向向量，使MLLM在推理时更关注高亮帧，同时通过降低注意力避免忽略非高亮帧。

Result: 在多个基准测试上，HAS在视频摘要任务中表现优异。

Conclusion: 全局高亮引导的注意力导向能显著提升MLLM视频摘要的连贯性与完整性。
...
### [263] [Measuring and Improving Complex-Atomic Answer Consistency in Endoscopic VQA](https://arxiv.org/abs/2607.17834)
*Yuhao Liu,Cheng Zhao,Guanghui Yue*

Main category: cs.CV

TL;DR: 本文提出了EndoCA，一个用于评估内窥镜视觉问答（VQA）中复杂答案与原子答案一致性的基准，并引入了ASR，一个无需训练的机制，用于通过原子答案作为上下文前提来修正答案，从而提升一致性。


<details>
  <summary>Details</summary>
Motivation: 现有内窥镜VQA中复杂问题的答案可能被判定为正确，即使模型在相关的原子问题上失败，因此需要评估答案的一致性。

Method: 构建了EndoCA-Core和EndoCA-Diagnostic两个测试集，评估了11个视觉语言模型（VLM），并提出了ASR机制，包括ASR-Revise（基于原子答案修正复杂答案）和ASR-Selective（基于一致性选择性回答）。

Result: 部分VLM在复杂答案上表现良好，但原子答案准确率和一致性较低。ASR-Revise在略微改变复杂答案准确率的情况下提升了成对一致性，ASR-Selective通过允许模型放弃低可靠性案例提高了回答准确性。

Conclusion: EndoCA和ASR为内窥镜VQA提供了一致性感知的基准和无需训练的答案修正及选择性回答机制。
...
### [264] [Reviving Ancient Paintings via Poem: A Colorization Framework for Aligning Cultural Semantics](https://arxiv.org/abs/2607.17638)
*Junming Gao,Biao Zhu,Xiaosong Wang,Tan Tang*

Main category: cs.CV

TL;DR: 提出 PoemColor 框架，通过诗画投影器（P3）和结构感知语义注意力（SASA）实现古诗引导的古画色彩恢复，有效克服语义偏差、过饱和与颜色泄漏问题。


<details>
  <summary>Details</summary>
Motivation: 古画褪色破坏诗画意境，现有扩散模型难以在保持历史真实性的同时避免现代语义偏差和颜色失真。

Method: 构建诗画投影器（P3）将诗意转化为古典色彩条件，并设计结构感知语义注意力（SASA）控制颜色语义注入的方向与区域强度；同时构建混合复原数据集。

Result: 在多项指标上显著优于现有方法，实现了兼具历史真实性与诗意语义的可控色彩化。

Conclusion: PoemColor 有效解决古诗引导古画色彩恢复中的多义性和偏差问题，为文化文物数字化提供新思路。
...
### [265] [CaT-GS: Efficient 3DGS Rendering for Large Scale Scenes via Inter-frame Caching and Tile Scheduling](https://arxiv.org/abs/2607.17842)
*Tingjia Zhang,Bo Chen,Shengzhong Liu,Fan Wu,Guihai Chen*

Main category: cs.CV

TL;DR: 提出CaT-GS，一种针对大规模场景下3D Gaussian Splatting渲染效率问题的优化方案，实现最高10倍加速。


<details>
  <summary>Details</summary>
Motivation: 3DGS在大规模场景中因瓦片栅格化计算负担导致性能下降；现有优化方法或需重新训练或仅关注单一环节，忽视实际部署中的关键效率问题。

Method: 通过分析识别帧间预处理冗余、视点遮挡冗余及瓦片负载不均衡三大瓶颈；提出CaT-GS：推测性多帧预处理消除冗余计算、帧间缓存机制消除视点冗余渲染、专用核重构瓦片栅格化任务缓解负载不均衡。

Result: 相比原始3DGS加速最高10倍，相比此前最优方法加速最高70%，为大规模场景高保真实时渲染建立新基准。

Conclusion: CaT-GS有效缓解了3DGS在大规模场景下的渲染效率瓶颈，显著提升GPU利用率，验证了所提冗余消除与负载均衡策略的有效性。
...
### [266] [Direct Clinical Joint Angle Extraction from Parametric Body Model Rotation Matrices](https://arxiv.org/abs/2607.17639)
*Joey Páolo Kardolus,Daan Hendriks,Jaap Jansen*

Main category: cs.CV

TL;DR: 提出一种直接从参数化人体模型旋转矩阵读取临床关节角度的方法，无需逆运动学或肌肉骨骼模型拟合，在单一智能手机视频上达到约4.5度的平均绝对误差。


<details>
  <summary>Details</summary>
Motivation: 现有临床关节角度测量工具慢、昂贵或局限于实验室，难以在常规护理中广泛应用。

Method: 利用参数化人体模型每段旋转矩阵直接映射关节角度，通过小型校准表代替优化步骤，且方法可迁移至其他人体模型（如SAM 3D Body）。

Result: 在OpenCap Lab验证集上，GEM-X模型平均绝对误差为4.50度（15个关节角度），与OpenCap Monocular的4.8度相当；SAM 3D Body为4.66度，实时运行于单摄像头流。

Conclusion: 该方法简化了运动分析流程，无需额外输入（身高、相机参数等），可扩展至临床、居家、远程康复和大规模分散研究。
...
### [267] [Anticipate Before Acting: Future-State-Conditioned Vision-Language Navigation](https://arxiv.org/abs/2607.18042)
*Lingfeng Zhang,Zhanguang Zhang,Liheng Ma,Tongtong Cao,Yingxue Zhang*

Main category: cs.CV

TL;DR: 本文提出FSC-VLN方法，通过未来状态条件化训练提升视觉语言导航性能，在R2R数据集上超越基线。


<details>
  <summary>Details</summary>
Motivation: 标准行为克隆只监督下一步动作，未明确训练策略状态预测未来视觉结果，而未来观测可提供丰富动作线索。

Method: 提出未来状态条件化VLN（FSC-VLN），添加未来查询令牌，通过训练时目标分支将其隐藏状态对齐到冻结的未来视觉嵌入，推理时移除该分支。

Result: 在R2R val-unseen上，FSC-VLN在两种训练数据设置下均提升SR/OSR/SPL，长时任务增益更大；消融实验支持双重查询设计。

Conclusion: 利用未来视觉隐式监督可有效提升VLN策略，无需推理时未来图像。
...
### [268] [Sparse Evidence Can Suffice: Agentic Evidence Seeking for Multimodal Video Misinformation Detection](https://arxiv.org/abs/2607.18080)
*Haochen Zhao,Yongxiu Xu,Xinkui Lin,Dong Xie,Jiarui Lu,Yuqi Qian,Yubin Wang,Hongbo Xu,Gaopeng Gou*

Main category: cs.CV

TL;DR: 提出SIEVE框架，通过稀疏交互证据提取实现多模态视频虚假信息检测，显著提升检测性能并增强可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将整个视频全局处理，但真实虚假信息仅依赖少数耦合线索，全局推理引入冗余并掩盖关键证据。

Method: 设计证据搜索智能体主动提取紧凑证据包，结合监督轨迹和证据感知强化学习训练，优化证据获取过程。

Result: 在多个基准上超越现有方法，仅需紧凑证据包即可实现可靠验证，并提供可追溯的证据链。

Conclusion: 稀疏证据提取能有效减少冗余、突出关键证据，提升检测透明度与鲁棒性。
...
### [269] [LFM: Leveraging Foundation Models for Source-Free Universal Domain Adaptation](https://arxiv.org/abs/2607.17653)
*Jing Li,Pan Liu,Meng Zhao,Wanli Xue,Yanhong Yang,Xu Cheng,Fan Shi,Jianhua Zhang,Qinghua Hu,Shengyong Chen*

Main category: cs.CV

TL;DR: 提出了一种利用基础模型（如视觉-语言模型和大语言模型）解决源自由通用域适应问题的新框架，通过生成文本标签、判别标签偏移类型和精炼伪标签，提升了目标域适应性。


<details>
  <summary>Details</summary>
Motivation: 现有源自由通用域适应方法依赖阈值调优和聚类，效率低下，且基础模型在此领域未得到充分探索。

Method: 使用视觉-语言模型计算样本与文本标签的相似度，通过变异系数判断标签偏移类型，基于二项高斯混合模型识别未知样本，结合预训练源模型与基础模型知识精炼伪标签。

Result: 在多种标签偏移和基准测试中，该方法表现有效且优于现有技术。

Conclusion: 所提基础模型框架有效解决了源自由通用域适应问题，代码已开源。
...
### [270] [GigaPath-Flash and GigaTIME-Flash: Efficient Pathology Foundation Models for Whole-Slide and Tumor Microenvironment Analysis](https://arxiv.org/abs/2607.18218)
...
TL;DR: 提出了GigaPath-Flash和GigaTIME-Flash模型，通过知识蒸馏和高效架构，在保持高精度的同时显著降低计算成本，并开放权重以推动病理AI应用。


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型大多在图像图块级别操作、许可证限制严格、计算成本高，限制了全切片水平的临床和研究应用。

Method: 使用知识蒸馏将十亿参数GigaPath教师模型压缩为2200万参数的ViT-S图块编码器，并结合2100万参数的LongNet切片编码器，构建GigaPath-Flash；GigaTIME-Flash在此基础上预测肿瘤免疫微环境。

Result: GigaPath-Flash在平均切片级性能上保持GigaPath的97%，计算量减少50倍；GigaTIME-Flash预测质量超过原始CNN模型，速度快6倍且GPU内存减少8倍。

Conclusion: 该开放权重、Apache-2.0许可的模型系列为计算病理学和精准健康提供了高效且可访问的基础工具。
...
### [271] [BMFA: Boundary-Minority Free-Energy Adaptive Screening](https://arxiv.org/abs/2607.17656)
*Wenyan Xu,Alizer Wong*

Main category: cs.CV

TL;DR: 本文提出边界-少数自由能自适应筛选（BMFA）方法，通过分层分段常数近似和局部自由能下界增量递归细化块，解决了视觉Transformer中空间小但高响应区域（边界-少数）被低估的问题。


<details>
  <summary>Details</summary>
Motivation: 传统粗粒度令牌摘要（如块均值）低估了空间面积小但响应强烈的区域（边界-少数），导致指数注意力聚合所需证据不足，影响模型性能。

Method: BMFA方法：构建分层分段常数近似，根据可计算局部自由能下界增量递归细化块；通过自由能差异形式化边界-少数低估的失败，并证明其非消失性。

Result: BMFA在合成测试中将均值低估从2.582降至0.261（叶比率5.794%）；在COCO图像边缘将均值差距从2.254降至0.526；在ImageNet-1K上以55.861%叶比率保持71.520% Top-1准确率。

Conclusion: BMFA有效缓解了边界-少数低估问题，但其选择质量评估依赖于完整QK计算，当前叶比率反映表示粒度而非稀疏内核加速的实际效果。
...
### [272] [Early Yield Prediction for Sugar Beet Fields using Satellite Data -- Learnings from Specialized Vision Transformers](https://arxiv.org/abs/2607.17661)
*Philipp Vaeth,Bhumika Laxman Sadbhave,Denise Dejon,Gunther Schorcht,Magda Gregorova*

Main category: cs.CV

TL;DR: 本文展示了如何通过紧密结合领域知识和机器学习，利用Sentinel-2光学影像早期预测甜菜收获产量，并发现使用极小视觉Transformer补丁大小和所有可用光谱带可提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 遥感在农业监测中价值日益凸显，但有效整合领域知识与机器学习方法仍具挑战。

Method: 使用全光谱Sentinel-2影像，采用极小补丁大小的视觉Transformer，并结合修正训练设置和基于排序的低产田检测方法。

Result: 模型在早期生长周期中成功识别了大量低产田，且非传统设计选择（如极小补丁大小、全光谱使用）显著提升了预测效果。

Conclusion: 领域知识与机器学习的紧密整合可产生协同增益，为早期产量预测提供实用方法。
...
### [273] [RayOcc: Occlusion-Aware Ray Occupancy Estimation via Gaussian Mixture Intensity](https://arxiv.org/abs/2607.17660)
*Junho Kim,Seongwon Lee*

Main category: cs.CV

TL;DR: 提出了RayOcc，一种基于射线占用框架的多标签存在预测方法，通过非归一化高斯混合强度建模多个占用假设，在nuScenes上达到领先水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法将每条射线视为单一深度估计，无法处理复杂遮挡和多个空间分离表面共存的情况。

Method: 将射线建模为多标签存在预测，使用非归一化高斯混合分布估计占用强度，通过泊松事件公式转换为区间占用概率，并用高斯基元优化细化。

Result: 在nuScenes基准上，RayOcc在高斯基元占用方法中实现了最佳的整体IoU和mIoU。

Conclusion: RayOcc有效解决了深度模糊和遮挡问题，为相机仅3D语义占用预测提供了新的思路。
...
### [274] [Simple Domain Generalization for Strong Pixel-Level Image Tampering Detection in Modern VLMs](https://arxiv.org/abs/2607.18230)
...
TL;DR: 提出了一个面向现代视觉语言模型生成图像的像素级篡改检测的领域泛化训练框架，通过平衡采样和晚注入策略显著提升了跨模型和领域外鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代VLM生成的图像存在欺骗性篡改，但跨模型和分布外场景下的像素级检测仍具挑战。

Method: 平衡小批量采样策略（防止优化偏向）和晚注入策略（先用大数据训练，再用少量新兴VLM数据微调）。

Result: 在多个OOD VLM上，平均gIoU和cIoU相对提升26.1%和26.8%。

Conclusion: 该简单框架显著优于此前SOTA方法PIXAR，在VLM图像篡改检测领域泛化中效果显著。
...
### [275] [ShotPlan: Cinematic Video Generation with Learnable Planning Token](https://arxiv.org/abs/2607.17675)
*Su Guo,Guangce Liu,Haosen Yang,Jiepeng Wang,Cong Liu,Junqi Liu,Haibin Huang,Hongxun Yao,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: 提出ShotPlan框架，通过可学习规划令牌和FRoPE位置嵌入，实现显式多镜头电影级视频生成，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在单次生成中表现出色，但在需要镜头规划的连续叙事和多镜头合成方面存在局限。

Method: 基于视频扩散基础模型，引入可学习规划令牌捕获镜头转换线索，采用分数时间旋转位置嵌入实现帧级建模。

Result: 在镜头管理灵活性和镜头间一致性上显著超越当前电影级视频生成方法。

Conclusion: 显式镜头规划框架可有效提升多镜头视频生成的叙事连贯性和画面一致性。
...
### [276] [Memory-Supported Synergistic Adaptation for Training-Free Test-Time Medical Image Segmentation](https://arxiv.org/abs/2607.17693)
*Lingrui Li,Nan Pu,Dong Zhao,Wenjing Li,Andrew P French,Zhun Zhong,Xin Chen*

Main category: cs.CV

TL;DR: 提出训练免费的测试时适应框架MSSA，通过内存支持和协同适应改进视觉-语言模型在医学图像分割中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM的测试时适应方法在医学分割中因噪声、更新驱动学习导致预训练特征退化，改进有限且不稳定。

Method: MSSA包括噪声感知内存构建模块（筛选稳定跨模态预测）和相关性引导原型对齐模块（对齐结构一致样本与可靠预测）。

Result: 在多个医学分割基准上，MSSA一致提升VLM分割模型，超越现有微调方法，DSC提升12.2%，mIoU提升11.7%。

Conclusion: 无需参数更新即可实现鲁棒适应的测试时适应框架，有效解决医学图像分割中的分布偏移问题。
...
### [277] [To Blend In, First Decouple: Rethinking Camouflage Image Generation via Context-Decoupled Representations](https://arxiv.org/abs/2607.17768)
*Wenzhuang Wang,Yifan Zhao,Mingcan Ma,Yunlong Che,Haoran Chen,Ming Liu,Jia Li*

Main category: cs.CV

TL;DR: 提出了一种新的上下文解耦生成范式CamoDreamer，通过解耦目标和背景特征生成更高保真度的伪装图像。


<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法因上下文表示泄漏导致外观不一致和背景伪影，需要解耦条件控制。

Method: 包含对比感知上下文桥、上下文解耦同化流和频率自适应上下文混合模块，解耦潜在伪装特征。

Result: 在多个指标上显著优于现有方法，且模型轻量。

Conclusion: CamoDreamer通过解耦条件控制有效提升伪装图像生成质量。
...
### [278] [Attention from Above: A Multimodal Model for Drone-Based Object Localization](https://arxiv.org/abs/2607.17669)
*Hyun-Ki Jung*

Main category: cs.CV

TL;DR: 提出一种基于YOLO-World的高效多模态无人机目标检测模型，通过将主干网络中的C2f层替换为注意力A2C2f层，提升小目标检测性能。在VisDrone数据集上，精度从43.0%提升至45.1%，mAP@0.5从32.5%提升至35.2%。


<details>
  <summary>Details</summary>
Motivation: 无人机目标检测技术快速发展，需求从预定义目标检测扩展至文本提示的指定目标检测，亟需提升小目标检测性能。

Method: 基于YOLO-World框架，将YOLOv8主干中的C2f层替换为基于注意力的A2C2f层，并引入注意力机制与并行处理结构。

Result: 在VisDrone数据集上，相比原始YOLO-World，精度提升2.1%、召回率提升2.2%、F1提升2.2%、mAP@0.5提升2.7%、mAP@0.5-0.95提升1.4%。

Conclusion: 该方法为无人机图像和视频应用提供了有效且高精度的目标检测解决方案。
...
### [279] [PC-Seg: Progressive Cross-View Consistency for 3D OCT Segmentation from Sparse 2D Annotations](https://arxiv.org/abs/2607.17718)
...
TL;DR: 提出PC-Seg框架，用稀疏2D标注学习高精度3D OCT分割模型，通过渐进交叉视图一致性和协同训练，仅需0.7%标签数据达到全监督水平。


<details>
  <summary>Details</summary>
Motivation: 现有半监督OCT分割方法逐切片处理，未充分利用3D空间上下文，且标注成本高。

Method: 单2D模型从标准B扫描和正交切片学习交叉视图一致性，生成可靠3D伪标签；蒸馏至3D模型后，2D和3D模型通过集成伪标签相互协同训练。

Result: 在MSHC和Duke DME数据集上，仅用约0.7%训练数据标签，精度与全监督相当，优于现有半监督和视网膜层分割方法。

Conclusion: PC-Seg有效降低标注成本，实现高精度3D OCT分割，代码已公开。
...
### [280] [Toward Optimal Adenovirus Detection Using YOLO26](https://arxiv.org/abs/2607.17799)
*Olivier Rukundo*

Main category: cs.CV

TL;DR: 系统评估了四种数据增强设置（NAS, GAS, GMAS, DAS）在YOLO26模型对腺病毒TEM图像检测中的性能，发现最有效的设置。


<details>
  <summary>Details</summary>
Motivation: TEM图像中腺病毒检测面临数据量有限和标注成本高的挑战，需优化数据增强策略以提高检测性能。

Method: 使用YOLO26模型变体，在相同训练条件下基准测试NAS、GAS、GMAS和DAS四种数据增强方法；利用公开TEM病毒数据集，重新标注腺病毒粒子位置生成YOLO兼容的边界框。

Result: 不同数据增强设置显著影响腺病毒检测性能，并确定了最有效的设置。

Conclusion: 数据增强是提升TEM图像中腺病毒检测效果的关键，本研究为实际应用提供了选择最佳增强策略的依据。
...
### [281] [Consistent Feature Transport for Image Relighting](https://arxiv.org/abs/2607.17833)
*Bohan Zhang,Huanwei Liang,Yuhan He,Hongteng Xu,Quxiao Chao,Luoqi Liu,Dixin Luo,Ting Liu*

Main category: cs.CV

TL;DR: 本文提出基于照明特征传输的图像重打光方法CFT，通过轨迹级监督实现噪声到图像生成与照明一致传输的联合建模，显著提升复杂光照下的重打光效果。


<details>
  <summary>Details</summary>
Motivation: 现有扩散方法缺乏显式特征变换学习机制，导致复杂光照下照明变化不稳定或内容不一致。

Method: 将重打光重新定义为照明特征传输问题，提出CFT训练原则，利用整流流实现噪声到图像生成与照明一致传输的双重轨迹级监督。

Result: 在自建大规模肖像重打光数据集上，CFT超越现有方法，并能泛化至风格迁移等其他编辑任务。

Conclusion: CFT通过显式分离照明变化与内容特征，为图像重打光提供了稳定且通用的解决方案。
...
### [282] [FF-ProCams: Feed-Forward Gaussian Splatting for Projector-Camera System](https://arxiv.org/abs/2607.17803)
*Ziyao Wang,Yuqi Li,Wenxing Zheng,Jiaying Chen,Chong Wang*

Main category: cs.CV

TL;DR: 提出了FF-ProCams，一个前馈式3D高斯逆渲染框架，用于投影仪-相机系统，在保持高保真度的同时实现了快速重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖耗时的逐场景优化，要么生成的不可重新光照表示无法适应投影仪照明，缺乏准确性和效率的平衡。

Method: 采用混合Mamba2-Transformer编码器聚合多视图几何和光度线索，通过轻量级头预测可重新光照的高斯表示，并设计了投影仪感知的可微分渲染器。

Result: 仅用8个输入视图，比优化基线方法（297视图）表现更好，测试时间重建仅需0.13秒，实现了3到5个数量级的加速。

Conclusion: FF-ProCams在高保真投影仪感知渲染、对未见模式的泛化以及支持新型投影仪-相机位姿方面表现优异。
...
### [283] [Fine-Detail Monocular Geometry Estimation with Self-Guided Sparse Volumetric Refinement](https://arxiv.org/abs/2607.17967)
*Lingyu Kong,Ruicheng Li,Ruicheng Wang,Sicheng Xu,Chengtang Yao,Jianfeng Xiang,Jiaolong Yang*

Main category: cs.CV

TL;DR: 本文提出了一种自引导稀疏3D细化（SSR）方法，用于提升单目几何估计在精细细节上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有模型在2D参数化中解码3D几何，导致特征融合混乱，尤其在薄结构和物体上产生过平滑失真。

Method: 将粗略点图提升至稀疏体素壳，并使用基于3D空间邻近性的稀疏卷积进行细化。

Result: 在多个数据集上显著优于现有方法，在定量和定性指标上均表现更好。

Conclusion: 通过3D空间特征聚合可有效恢复精细3D几何结构。
...
### [284] [Leveraging Dissimilarity Invariance as a Robust Anchor for Learning with Noisy Labels](https://arxiv.org/abs/2607.17857)
*Wenxiao Fan,Kan Li*

Main category: cs.CV

TL;DR: 提出NegScale框架，利用语义不相似性在标签噪声下的稳定性，通过负正交惩罚和不相似性校准相似性调整，提升深度模型在噪声标签下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在标签噪声下难以学习准确相似性，导致性能下降，需要探索更鲁棒的度量。

Method: 提出NegScale框架，包含结构化负正交惩罚和不相似性校准相似性调整两个模块。

Result: 在CIFAR合成噪声和真实数据集上均超越现有基线，建立新基准。

Conclusion: 利用不相似性不变性可有效提升噪声标签下的表示学习性能。
...
### [285] [PRiSM: Prototype Regularization for Few-Shot VLMs](https://arxiv.org/abs/2607.17820)
*Ghassen Baklouti,Omprakash Chakraborty,Jose Dolz,Ismail Ben Ayed*

Main category: cs.CV

TL;DR: 提出了一种名为PRiSM的类原型正则化方法，用于改进无训练少样本适应方法在非平衡类别数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准假设少样本任务中类别平衡，但实际场景中类别不平衡和类别数量变化会严重影响模型性能。

Method: 通过Dirichlet采样构建更现实的基准，并引入PRiSM模块：包含最大化类间距离的正则化、支持特征对齐和原型保真度项，使用块Majorize-Minimize优化器高效求解，基于Gershgorin圆定理计算块Lipschitz常数。

Result: PRiSM在多种无训练基线方法上显著提升性能，尤其在严重类别不平衡和高类别数场景下增益明显；发现现有方法在非平衡设置下性能大幅下降。
...
### [286] [Exploration Matters for Escaping the Blur Trap in 3D Gaussian Splatting](https://arxiv.org/abs/2607.17965)
*Chengbo Wang,Guozheng Ma,Jinhong Wu,Tie Ji,Yizhen Lao*

Main category: cs.CV

TL;DR: 本文提出两种简单探索策略（随机播种和随机分割）来解决3D高斯溅射优化中的模糊陷阱问题，显著提升了渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS优化存在梯度偏差，易收敛到局部次优解（模糊陷阱），影响重建质量。

Method: 通过数学分析将模糊陷阱分为远侧和近侧两类，并分别采用随机播种和随机分割作为探索策略。

Result: 实验证明该探索策略能有效且互补地克服模糊陷阱，在多数据集上实现高质量渲染。

Conclusion: 简单显式探索机制能显著改善3DGS优化，为高保真场景重建提供新方案。
...
### [287] [Locality-Aware Density Control for Efficient Gaussian-based Image Representation](https://arxiv.org/abs/2607.17896)
*Jiacong Chen,Qingyu Mao,Xiandong Meng,Shuai Liu,Chao Li,Fanyang Meng,Youneng Bao,Yongsheng Liang*

Main category: cs.CV

TL;DR: 提出了一种基于局部感知的密度控制框架LocoADC，通过区域高斯稠密化和相似性驱动的高斯合并策略，有效提高2D高斯表示中高斯容量的利用效率。


<details>
  <summary>Details</summary>
Motivation: 现有2D高斯点云方法在优化时无法高效分配高斯容量，欠重建区域仅进行碎片化像素级细化，而过重建区域冗余保留相似高斯，导致效率低下。

Method: 提出LocoADC框架，包含区域高斯稠密化（RGD）和相似性驱动高斯合并（SDGM）两种策略，并引入局部颜色一致性约束以确保合并可靠性。

Result: 在多个数据集上显著提升基线方法性能，例如在CLIC数据集上以相同3万高斯预算下PSNR提升2.93 dB。

Conclusion: 通过利用图像空间和高斯空间的双重局部性，LocoADC实现了更有效的局部高斯分配，提升了表示质量。
...
### [288] [Keyframe-Anchored Identity Preservation for Sequential-Action Video Generation](https://arxiv.org/abs/2607.17985)
*Zhenjie Liu,Binyan Chen,Hao Chen,Tong Pan,Shangfei Wang*

Main category: cs.CV

TL;DR: 本文提出了一种无训练的三阶段框架，用于解决身份保持的文本到视频生成中时间结构化动作序列导致的身份漂移问题，并在IPVG26挑战赛中取得第三名。


<details>
  <summary>Details</summary>
Motivation: 现有身份保持生成任务主要关注单一提示，缺乏对时间结构化动作序列的支持，而端到端生成器在累积运动和动作变化中容易出现外观漂移。

Method: 三阶段流水线：1) 动作感知提示润色，将输入改写为指定动作终态的图像生成提示；2) 身份保持关键帧生成，联合参考身份与前驱帧条件化，解耦时不变外观与时变姿态；3) 身份感知推理增强，通过多参考引导和身份驱动噪声搜索合成中间帧。

Result: 在IPVG26 Track 2官方排行榜上排名第三，展示了竞争性能和强泛化性。

Conclusion: 所提无训练框架有效解决了时间结构化动作序列中的身份漂移问题，具有良好的通用性和实际应用潜力。
...
### [289] [MuViSeg: Multi-View Segment Correspondences from Dense Geometry Priors](https://arxiv.org/abs/2607.17938)
*Denis Fatykhoph,Timur Akhtyamov,Konstantin Pakulev,German Devchich,Gonzalo Ferrer*

Main category: cs.CV

TL;DR: 本文提出一种基于实例分割的物体级图像匹配方法，通过学习三种匹配头（LightGlue风格注意力、DPT风格多尺度融合和多视图联合自注意力）提升分割级别的对应关系，并在零样本和导航任务中取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 现有的图像对应方法在稀疏关键点或密集像素层面求解，而高级任务（如物体级建图、拓扑导航）需要物体级推理，因此需要直接匹配实例分割。

Method: 在类无关分割器划分图像并利用3D基础模型池化特征的基础上，提出三种学习型匹配头：LightGlue风格注意力头（带DoubleSoftmax评分）、DPT风格多尺度融合模块（利用VGGT模型）、以及多视图扩展（联合自注意力处理多视角分割）。

Result: 在Replica和Virtual KITTI 2上，LightGlue头相比Sinkhorn匹配器AUPRC分别提升4.85和25.9；在HM3D导航中，多视图变体将成功率从50%提升至70%，LightGlue头将SPL从45.7提升至59.1。

Conclusion: 基于实例分割的学习型匹配头能有效弥合像素级匹配与物体级需求之间的差距，提升零样本匹配和下游导航任务性能。
...
### [290] [When 2D Cues Fail: Improving Image Manipulation Localization with Reliable 3D Geometry](https://arxiv.org/abs/2607.18040)
*Guofeng Yu,Zhiqing Guo,Dan Ma,Gaobo Yang*

Main category: cs.CV

TL;DR: 提出几何感知框架GFrame，利用单目重建的深度和法线作为辅助线索，通过可靠性估计选择性地融合RGB特征，提升图像篡改定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有2D篡改定位方法在处理外观一致但几何结构异常的篡改区域时效果有限，需引入3D几何推理。

Method: 基于单目重建获取深度和表面法线，估计其可靠性后与RGB特征融合，跨尺度传播以优化定位。

Result: 在有限预算下达到优秀性能，验证可靠3D几何线索对2D线索的互补作用。

Conclusion: 可靠3D几何信息能有效补充传统2D取证线索，提升篡改定位能力。
...
### [291] [SAMRI-3D: Adapting SAM2 for 3D MRI Segmentation with Global Volume Tokens](https://arxiv.org/abs/2607.18014)
*Zhao Wang,Wei Dai,Hongfu Sun,Craig Engstrom,Shekhar S. Chandra*

Main category: cs.CV

TL;DR: 提出了针对3D MRI分割的SAMRI-3D基准和方法，通过冻结图像编码器、微调轻量解码器和记忆模块，并引入全局体积令牌（GVT）配合截断符号距离场（TSDF）重建目标，将Dice从0.58提升至0.78，且在未见数据集上无性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法主要针对自然图像，缺乏针对MRI低软组织对比度的专门建模和大型基准测试。

Method: 冻结SAM2图像编码器，微调解码器和记忆模块；设计全局体积令牌（GVT）并结合截断符号距离场（TSDF）重建目标训练，推理时无额外成本。

Result: 在10,392个体积的34个数据集上，平均Dice达到0.78，超过SAMed-2等模型，且在8个未见数据集上未见性能下降。

Conclusion: MRI特定的轻量微调和空间连续性先验（TSDF）能显著提升SAM2在3D MRI分割上的性能，且泛化能力良好。
...
### [292] [SAR Vessel Detection and Gross Tonnage Estimation from Heterogeneous Datasets for Dark Vessel Identification](https://arxiv.org/abs/2607.18051)
*Davide Paltrinieri,Andrea Diecidue,Roberto Basla,Daniele Casciani,Piero Fraternali,Giacomo Boracchi*

Main category: cs.CV

TL;DR: 提出一个利用异构图像和表格数据检测SAR图像中暗船（关闭应答器船只）的框架，结合多任务深度学习与KNN，实现位置、类型、尺寸预测及吨位回归。


<details>
  <summary>Details</summary>
Motivation: 现有SAR数据集缺乏吨位标注，无法直接训练端到端模型识别必须携带应答器（基于吨位）的暗船，需结合多源数据解决这一安全问题。

Method: 多任务深度学习框架（预测位置、类型、尺寸）串联非参数模型（KNN，使用混合欧氏-分类距离）从尺寸和类别回归吨位。

Result: 在多个子任务上达到与现有技术竞争的性能，并能联合预测多输出，实现暗船识别。

Conclusion: 该方法能有效识别暗船，代码已开源，为海上安全监测提供了可行方案。
...
### [293] [VGOcc: Learning Visual-Geometric Gaussians for Vision-Centric 3D Driving Occupancy Prediction](https://arxiv.org/abs/2607.18078)
*Junhong Lin,Xianda Guo,Kangli Wang,Yuqi Ye,Xiaoyu Liang,Yanlun Peng,Wei Gao*

Main category: cs.CV

TL;DR: 本文提出VGOcc，通过从基础模型学习视觉和几何线索，用于高斯建模，实现仅视觉的3D占用预测，在nuScenes数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的方法主要依赖图像域特征，缺乏明确的几何信息进行体积推理，而有效的占用建模需要更丰富的几何和语义学习线索。

Method: 提出视觉-几何高斯初始化（通过光线深度假设形成平衡高斯中心）和姿态感知特征学习（融合基础模型令牌、相机嵌入和光线信息），再经高斯解码器细化并渲染为语义占用。

Result: 在nuScenes数据集上，VGOcc在仅视觉3D占用预测任务中取得最先进性能。

Conclusion: 结合视觉和几何线索的高斯建模能有效提升占用预测精度，验证了从基础模型学习辅助信息的有效性。
...
### [294] [Benchmarking NACTI Species Recognition in Long-Tailed Regimes](https://arxiv.org/abs/2607.18033)
*Zehua Liu,Tilo Burghardt*

Main category: cs.CV

TL;DR: 本文系统评估了长尾识别（LTR）方法在北美相机陷阱图像数据集（NACTI）上的表现，优化配置达到99.40%的Top-1准确率，并在跨域测试中展现出更强的泛化能力，但极端域移下尾部类别性能仍严重下降。


<details>
  <summary>Details</summary>
Motivation: NACTI数据集存在严重的类别长尾不平衡（最大类占50%以上），标准基线方法性能不足，需系统评估LTR方法以提升物种识别鲁棒性。

Method: 基于PyTorch Wildlife模型，系统评估了专用损失函数和LTR敏感正则化等长尾识别方法，并在三个独立域移测试集上进行泛化验证。

Result: 优化配置在NACTI测试集上达到99.40% Top-1准确率，显著优于基线；跨域测试中LTR模型泛化能力更强，但尾部类在严重域移下出现预测崩溃。

Conclusion: 当前LTR优化无法完全克服表征瓶颈，尾部类在极端域移下仍存在灾难性预测失败，需进一步研究缓解方法。
...
### [295] [QIRF Quantum-Inspired Non-Orthogonal Function-Space Compression for 3D Gaussian Splatting](https://arxiv.org/abs/2607.18067)
*Shizeng Jiang,Hao Zhang,Xuerui Ma,Ying Hu,Tao Zhang*

Main category: cs.CV

TL;DR: 提出QIRF方法，通过量子启发的非正交函数空间压缩，减少3D高斯溅射中的高斯数量，实现平均71.7%的高斯数压缩和3.54倍存储压缩，渲染速度提升34.3%。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射压缩方法主要处理原始冗余，但忽略了高斯基函数重叠和非正交性带来的冗余。

Method: 构建高斯重叠矩阵和辐射响应密度矩阵，通过广义特征分解识别主导子空间并挑选代表性高斯，结合响应模型和细节保护进行激进剪枝。

Result: 在13个场景上平均减少71.7%高斯数，压缩3.54倍，PSNR提升0.10 dB，渲染速度提高34.3%。

Conclusion: 非正交函数空间冗余是高斯辐射场中重要但未充分探索的冗余来源，QIRF有效利用该冗余实现高效压缩。
...
### [296] [SciForma: Structure-Faithful Generation of Scientific Diagrams](https://arxiv.org/abs/2607.18091)
*Yuxuan Luo,Peng Zhang,Xinjie Zhang,Xun Guo,Zhouhui Lian,Yan Lu*

Main category: cs.CV

TL;DR: 提出了SciForma框架，通过多维度结构忠实性优化，实现了科学方法图的高质量生成，开源模型首次接近专有水平。


<details>
  <summary>Details</summary>
Motivation: 现有模型无法保证科学方法图的结构忠实性，SFT无法确保结构正确，标量奖励后训练掩盖了具体失败维度，因此需要更精准的结构化生成方法。

Method: 将图表质量分解为组件、箭头、文本三个结构轴，构建SciFormaData-700K训练数据和SciFormaBench-2K评估集，提出M-DPO多维度联合偏好优化，结合推理时迭代编辑修正残差。

Result: SciForma-9B在SciFormaBench-2K和AIBench上超过所有开源基线和GPT-Image-1.5，接近专有模型的结构忠实性。

Conclusion: 多维度结构优化方法有效提升了科学图解的结构准确性，为高质量自动生成奠定基础。
...
### [297] [SpEmoC: A Balanced Speaker-Segment Multimodal Emotion Benchmark](https://arxiv.org/abs/2607.18109)
*Sania Bano,Shahzad Ahmad,Santosh Kumar Vipparthi,Sukalpa Chanda,Subrahmanyam Murala*

Main category: cs.CV

TL;DR: 本文介绍了SpEmoC语料库，包含从3100部电影/电视剧中提取的30,000个高质量、类别平衡的剪辑，涵盖七种情绪，并通过严格的数据分割和混合标注流程，验证了平衡数据和数据设计对情感识别模型鲁棒性和可迁移性的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集在规模、情绪分布、模态对齐和数据划分策略上存在差异，影响跨数据集泛化和少数情绪建模的可靠性，需要构建更合适的数据集。

Method: 从3100部电影/电视剧提取306,544个原始剪辑，经筛选获得30,000个高质量、类别平衡的剪辑；采用预训练模型与人工验证结合的混合标注流程；使用严格的电影/剧集级别数据分割；包含视觉、音频和文本模态。

Result: 在域内基准测试、跨数据集迁移、低数据训练、类别不平衡分析和模态迁移实验中，平衡数据和仔细分割使模型在其他数据集上的情绪表现更稳定。

Conclusion: 数据集设计对于实现稳健且可迁移的多模态情感识别至关重要，平衡数据和严格分割有助于提高模型泛化能力。
...
### [298] [Occlusion-Aware Panoptic Segmentation with Joint Position Embedding and Occlusion-Level Attention](https://arxiv.org/abs/2607.18112)
*Wenbo Wei,Jun Wang,Shan Raza,Abhir Bhalerao*

Main category: cs.CV

TL;DR: 提出PEMOLA模块，通过遮挡感知注意力改进全景分割，在COCO-OLAC和Cityscapes-OLAC上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 场景中的遮挡问题普遍存在，但现有方法常忽略遮挡建模。

Method: PEMOLA：基于遮挡分类器生成遮挡级注意力，结合可学习位置嵌入进行联合调制。

Result: 在遮挡和跨数据集场景下一致提升全景分割质量，计算开销小。

Conclusion: 遮挡建模对鲁棒全景分割至关重要，遮挡级注意力可有效增强模型性能。
...
### [299] [Lossless-INR: Lossless Volumetric Implicit Neural Representations](https://arxiv.org/abs/2607.18150)
*Kaiyuan Tang,Daniel Burke,Chaoli Wang*

Main category: cs.CV

TL;DR: 提出一种基于位平面分解的无损隐式神经表示框架Lossless-INR，通过逐位二进制分类、八叉树块划分和三元特征网格网络，实现了3D科学体数据的精确无损重建。


<details>
  <summary>Details</summary>
Motivation: 现有隐式神经表示方法对体数据重建有损，微小误差会传播到渲染和下游分析中，需要无损解决方案。

Method: 采用位平面分解将体素值转化为二进制位，将重建视为逐位二进制分类；结合八叉树自适应划分复杂区域和三元特征网格网络，实现紧凑表示和无损优化。

Result: 在多种体数据集上达到零比特错误率和比特精确重建，支持高保真渲染和下游分析，且表示紧凑。

Conclusion: Lossless-INR框架成功实现了体数据的无损隐式表示，兼顾精度和紧凑性，适用于科学可视化与分析。
...
### [300] [Certified Training for Convolutional Perturbations](https://arxiv.org/abs/2607.18195)
*Benedikt Brückner,Alessio Lomuscio*

Main category: cs.CV

TL;DR: 本文提出一种基于卷积扰动高效编码的认证训练方法，使视觉模型对运动模糊等扰动具有可证明的鲁棒性，在CIFAR10上对合理强度的运动模糊实现超过80%的鲁棒准确率。


<details>
  <summary>Details</summary>
Motivation: 视觉模型易受运行时抖动相机产生的运动模糊等扰动影响，导致关键应用失效；现有数据增强或对抗训练方法缺乏形式化安全保证，难以识别和缓解隐藏脆弱性。

Method: 提出一种新的认证训练方法，通过高效编码卷积扰动来训练可证明鲁棒的模型。

Result: 该方法显著优于对抗训练，例如在CIFAR10上对合理强度的运动模糊实现超过80%的鲁棒准确率，同时保持可比的常规准确率。

Conclusion: 该认证训练方法能有效提供对运动模糊等扰动的可证明鲁棒性，并保持良好标准准确率。
...
### [301] [Robust Multimodal Dynamic Object Segmentation](https://arxiv.org/abs/2607.18153)
*Zhe Xin,Hanzhi Chang,Penghui Huang,Yinian Mao,Guoquan Huang*

Main category: cs.CV

TL;DR: 提出融合多模态线索（2D点轨迹、3D重建、语义）的动态目标分割框架，结合Transformer与特征聚类聚合模块实现精确完整掩膜，并引入点查询SAM后处理方法。


<details>
  <summary>Details</summary>
Motivation: 现有光流法边界分割不一致，3D重建法对重建误差敏感，需提升分割精度与完整性。

Method: 利用Transformer和特征聚类聚合网络对多模态特征轨迹进行静态/动态分类，设计点查询SAM后处理处理多目标掩膜。

Result: 在动态目标分割和静态场景重建任务中均达到最优性能。

Conclusion: 多模态融合与新型网络设计可有效提升分割鲁棒性与精度。
...
### [302] [HOMIE: Human-object Centric Video Personalization via Multimodal Intelligent Enchancement](https://arxiv.org/abs/2607.18217)
*Yiyang Cai,Nan Chen,Rongchang Xie,Junwen Pan,Chunyang Jiang,Cheng Chen,Wen Zhou,Zhenbang Sun,Wei Xue,Wenhan Luo,Yike Guo*

Main category: cs.CV

TL;DR: 提出HOMIE框架，统一处理人类-物体中心视频生成中的主体间与主体内参考输入，通过改进多模态大语言模型集成策略，在保持文本编码器可控性的同时提升主体保真度与交互准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难平衡主体保真度与交互准确性，且缺乏理解主体内参考（如OCR图、多视角输入）的机制。

Method: 全局多模态引导自注意力对齐MLLM语义特征与VAE令牌，引入模态参考嵌入区分并关联令牌。

Result: 在多种人类-物体中心视频个性化任务中达到最先进性能。

Conclusion: HOMIE有效解决了主体间与主体内参考的联合建模挑战，实现了高保真度与准确交互。
...
### [303] [Plenoptic Condensation: A Novel Approach to Generalized Scene Reconstruction](https://arxiv.org/abs/2607.18151)
*Brevin Tilmon,Alex DeJournett,John Leffingwell,Scott Ackerson*

Main category: cs.CV

TL;DR: 提出了基于低幂次“汤状”元素到高幂次结构化元素自适应凝聚的多阶段重建管道Plenoptic Condensation (PCon)，实现空间变化表示能力的Reality Models (Relms)，在手机和无人机拍得的场景上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 实现高保真渲染、测量和场景理解，需要灵活表示尖锐边缘和光滑反射表面的能力。

Method: 多阶段重建管道：先在图像中生成低幂次“汤状”元素，再自适应凝聚为高幂次结构化元素。

Result: 在‘Damaged Fiat’场景中，汽车引擎盖重建精度高于NeRO和RT-Splatting两倍以上，局部损伤误差35 μm，而未使用方法无法测量损伤。

Conclusion: PCon能够高效重建复杂场景，尤其在精细损伤检测中显著优于现有方法。
...
### [304] [The Many Senses of Visual Similarity: A Text-Prompted Image Perceptual Metric](https://arxiv.org/abs/2607.18237)
*Sheng-Yu Wang,Yotam Nitzan,Aaron Hertzmann,Jun-Yan Zhu,Eli Shechtman,Alexei A. Efros,Richard Zhang*

Main category: cs.CV

TL;DR: 提出TPIPS指标，通过文本提示实现多维度视觉相似性度量，解决现有指标无法区分不同感知维度的局限。


<details>
  <summary>Details</summary>
Motivation: 人类视觉相似性判断依赖上下文（如形状或颜色），而现有指标将其归为单一标量值，缺乏条件调节机制。

Method: 构建三元组图像相似性数据集（含多自由语义标注），微调视觉语言模型（VLM）得到TPIPS，支持文本提示调节相似性维度。

Result: VLM与人类注释者存在显著差距；TPIPS与人类感知更一致，且泛化至训练分布外场景；在文本引导检索、组合搜索和生成模型细粒度评估中展现新能力。

Conclusion: TPIPS通过文本提示捕捉多维度视觉相似性，优于现有指标，并拓展了相关应用的可能性。
...
### [305] [FlowMimic: Mask-free Visual Editing and Generation with Pixel-pair Warped Flow Field for Online Video Editing Data Generation and Modality Mimicry](https://arxiv.org/abs/2607.18227)
*Dingyun Zhang,Lixue Gong,Wei Liu*

Main category: cs.CV

TL;DR: 探索视频与图像生成编辑统一模型，提出像素对时间扭曲流场实时生成视频编辑样本，并通过模态模仿损失和感知任务损失实现内化编辑定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑数据收集耗时、易错且任务扩展性有限，导致视频编辑任务多样性远低于图像编辑。

Method: 构建像素对时间扭曲流场从图像样本实时生成视频编辑数据；设计模态模仿生成损失和编辑损失；引入指代分割等感知任务及区域感知损失。

Result: 模型仅用生成数据即可学习多级视频编辑任务，并内化语言驱动的编辑定位能力。

Conclusion: 统一模型可同时处理图文生成与编辑，通过数据流和损失设计克服数据瓶颈与外部依赖。
...
### [306] [Multi-level context Modeling for consistent expert selection in Mixture-of-Experts](https://arxiv.org/abs/2607.16427)
*Shuhan Huang,Naifan Zhang,Yuanbo Tang,Yang Li,Wai Kin Victor Chan*

Main category: cs.CL

TL;DR: 提出多级上下文融合MOE (MCF-MOE)，通过跨层语义聚合和局部token交互增强专家路由的上下文完整性，提升路由一致性和下游性能。


<details>
  <summary>Details</summary>
Motivation: 现有路由器依赖浅层或孤立token表示，导致路由决策不稳定、语义不一致；上下文不完整是限制专家有效特化的关键瓶颈。

Method: 构建上下文感知表示：集成跨层语义聚合与局部token交互信号，实现更充分一致的专家选择。

Result: 在语言建模和理解基准上，MCF-MOE持续优于强MoE基线，提升路由一致性和下游任务表现。

Conclusion: 上下文完整性对专家路由至关重要，MCF-MOE通过多级融合有效解决了该问题。
...
### [307] [RIMS: Preference Optimization via Smoothed Multi-pair Aggregation for Small-Scale LLM Retrieval-Augmented Generation](https://arxiv.org/abs/2607.16431)
*Pei Tian,Zihan Dong,Tianci Liu,Linjun Zhang,Haoyu Wang*

Main category: cs.CL

TL;DR: 提出RIMS框架，通过软聚合和自生成链式思维偏好数据，提升小模型在噪声检索下的RAG性能。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法数据利用低，小模型对噪声证据敏感。

Method: 三阶段框架：自生成链式思维偏好数据、可微软聚合替代硬选择、优化平滑目标。

Result: 在多跳问答基准上，RIMS优于现有方法，在噪声检索下持续提升精确匹配和F1分数。

Conclusion: RIMS有效利用偏好信号，提升小模型鲁棒性，理论保证误差有界且梯度更优。
...
### [308] [Encoding EEG Signals to Examine Human-Like Next-Word Prediction Behaviour in Language Models](https://arxiv.org/abs/2607.16549)
*Boi Mai Quach,Binh T. Nguyen,Cathal Gurrin,Graham Healy*

Main category: cs.CL

TL;DR: 本文发现仅语言模型的surprisal指标与人类阅读时的EEG脑电信号显著相关，且增大模型规模并不必然提升与人类认知的匹配度。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型在预测下一个词任务上的高准确率是否意味着其能有效捕捉人类阅读理解的认知信号。

Method: 基于top-1预测和surprisal两种信息度量，比较人类与语言模型对EEG中的事件相关电位的预测能力。

Result: 仅surprisal与语言处理相关的ERP存在潜在相关性，尤其对高语义内容的开放类词；扩大模型参数和计算量并未持续改善与人类语言处理的趋同。

Conclusion: 语言模型的认知合理性需通过精细的电生理指标评估，且模型规模扩展不能保证更好的模拟人类认知过程。
...
### [309] [Committed Before Reasoning: Behavioral Reproduction and Preliminary Activation-Level Evidence of Answer Pre-Commitment in an Open-Weight LLM](https://arxiv.org/abs/2607.16451)
*Heejin Jo*

Main category: cs.CL

TL;DR: 本研究发现，在“洗车步行还是开车”的简单推理任务中，Qwen3-8B模型在85-100%的采样中错误地选择了“步行”，即使只有“开车”才符合逻辑。激活模式分析显示，模型在输出答案前就已偏向“步行”，包括那些最终回答“开车”的样本。研究还提出了方法论改进：需要为每个问题措辞提供正对照。


<details>
  <summary>Details</summary>
Motivation: 观察到聊天模型在推理中有时会先承诺答案再找理由，即使该答案与任务前提矛盾。为了系统研究这种“承诺后推理”现象，作者设计了极简探针任务。

Method: 使用Qwen3-8B在五种系统提示条件下进行210次采样和贪婪解码；利用预训练的无训练激活预言机（非任务特定）在答案文本输出前的位置探测隐藏状态，并与中性上下文基线比较。

Result: “步行”承诺在85-100%的采样和100%的贪婪输出中出现；激活预言机在输出前的状态中68%读出“步行”（基线17%），即使最终回答“开车”的6个样本中有5个在承诺前也倾向于“步行”。正对照（开放式问题2/16 vs. 闭合式问题11/16）验证了方法有效性。

Conclusion: 模型存在系统性“承诺后推理”偏差，即使有足够思考预算（4096 token）也无法修复。初步激活证据表明这种偏差在文本输出前就已存在。方法论上，负预言机结果需要正对照来解释。
...
### [310] [NOWJ@COLIEE 2026: Adaptive Pipelines for Legal Retrieval and Reasoning](https://arxiv.org/abs/2607.16603)
...
TL;DR: NOWJ团队在COLIEE 2026全部五个任务中的方法：法律案例检索采用四阶段流水线，法律案例蕴涵结合BM25、T5重排序和LLM共识，法律检索与蕴涵使用RAG框架，文本蕴涵引入动态路由流水线，判决预测融合层次Transformer与概率论证推理。


<details>
  <summary>Details</summary>
Motivation: 解决COLIEE 2026竞赛中法律信息处理的多项挑战，包括案例检索、蕴涵判断和判决预测等任务。

Method: 针对不同任务采用专门流水线：案例检索用候选过滤、双编码器检索、交叉编码器重排序和自适应截止预测；法律蕴涵结合BM25、T5和LLM集成；法律检索与蕴涵采用RAG和注意力重排序；文本蕴涵用动态路由分类困难并分配不同解题器；判决预测结合层次Transformer、CRF和论证图推理。

Result: 在五个任务中均取得有效性能，但摘要未报告具体数字。

Conclusion: 所提方法在多个法律任务上有效，展示了流水线设计和LLM应用的灵活性。
...
### [311] [From Memory to Skills: Evidence-Grounded Co-Evolution Governance for Long-Horizon LLM Agents](https://arxiv.org/abs/2607.16621)
...
TL;DR: 提出MSCE框架，使LLM代理将经验转化为可执行的技能和认知，无需训练，在长周期任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有内存系统仅将历史轨迹作为被动上下文，无法转化为可执行能力，限制了长周期LLM代理的性能。

Method: 通过记忆-技能协同进化框架，利用反射加权价值回填和证据校准技能策略，实现无训练进化。

Result: 在EvoAgentBench和LoCoMo基准上超越现有方法，具备跨领域迁移和终身进化能力。

Conclusion: MSCE通过将经验结晶为可复用技能和环境认知，有效提升长周期代理的自主进化能力。
...
### [312] [OpenLanguageModel: Readable and Composable Small-Language-Model Pretraining for Education and Research](https://arxiv.org/abs/2607.16669)
*Tavish Mankash,Vardhaman Kalloli,Keshava Prasad,Deepan Muthirayan*

Main category: cs.CL

TL;DR: OLM是一个开源PyTorch库，用于构建和预训练小型语言模型，强调代码可读性和架构透明性，支持从教学到研究的全流程，包含27个预置模型和文档。


<details>
  <summary>Details</summary>
Motivation: 促进语言模型研究的教育和可复现性，通过提供清晰、可修改的代码和工具链，降低小型模型构建和预训练的门槛。

Method: 采用模块化设计，通过Block、Residual、Repeat、Parallel等组件描述模型架构；连接可读模型层与数据处理、优化、混合精度、回调、检查点及硬件感知执行；提供AutoTrainer自动配置。

Result: 与独立参考实现高度一致，四GPU弱缩放效率达90.6%（348M参数），支持紧凑架构编辑，并呈现积极初步可用性结果。

Conclusion: OLM作为开源工具，成功平衡了可读性、灵活性和性能，适用于教学、研究和实际预训练，并通过MIT许可广泛发布。
...
### [313] [SpecLA: Efficient Speculative Decoding for Linear-Attention Models](https://arxiv.org/abs/2607.16673)
*Zhibin Wang,Xuying Han,Zhaohua Yang,Fuliang Liu,Xue Li,Rong Gu,Sheng Zhong,Chen Tian*

Main category: cs.CL

TL;DR: 针对状态化线性注意力模型的推测解码系统SpecLA，通过拓扑感知验证、状态恢复和优化草稿生成，在H100上实现最高1.70倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码系统专为Transformer KV缓存设计，无法处理线性注意力模型中的循环依赖和状态更新问题。

Method: 提出拓扑感知核验证链条和树结构，利用验证产生的紧凑因子恢复已接受状态，结合置信度剪枝和目标对齐的EAGLE风格草稿模型。

Result: 在NVIDIA H100上使用GDN-1.3B目标模型，相比自回归解码实现最高1.70倍端到端加速。

Conclusion: SpecLA有效解决了状态化线性注意力模型的推测解码挑战，显著提升推理效率。
...
### [314] [Are Arithmetic Heuristic Neurons Form-Invariant? A Mechanistic Analysis of Symbols, Text, and Code in LLMs](https://arxiv.org/abs/2607.16693)
*Sharath Naganna,Tanvir Ahmed Sijan,Uddipta Kalita*

Main category: cs.CL

TL;DR: 大型语言模型在不同格式的算术问题中共享相同的神经回路，跨格式失败源于激活状态差异而非不同电路。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型在等价的算术问题不同表述下失败的原因，是源于内部电路不同还是共享电路的激活状态不同。

Method: 使用归因修补和激活修补的两阶段流水线，在Llama-3模型中识别算术启发式神经元，并分析其在符号算术、自然语言和Python代码三种格式中的共享性。

Result: 发现一组紧凑的共享神经元对所有三种格式的算术计算必要且充分；激活状态转移可恢复超过97%的加法和减法错误预测。

Conclusion: 大语言模型中的算术计算在神经元层面具有格式不变性，跨格式失败主要源于共享电路的激活状态不同。
...
### [315] [Though Language Models Err While They Strive: Conformal Prediction for Self-Correcting Scientific Generation](https://arxiv.org/abs/2607.16704)
*Mingqiao Mo,Yunlong Tan,Hao Zhang*

Main category: cs.CL

TL;DR: 提出了 SFC，一种图结构共形预测框架，用于确保大语言模型生成内容符合科学原理，在多个科学推理基准上取得了优异成绩。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在生成技术内容时常违反科学原理，降低了其在科学应用中的可靠性。

Method: 将科学推理分解为原子事实单元，通过渐进式绝对-一致-事实性验证，并构建近似可推导性图来建模逻辑依赖关系，实现实时验证和动态分支。

Result: 在 PhyX 物理推理上准确率达 50.1%，优于 DeepSeek-R1 和 GPT-4；科学有效性达 91.7%，违背科学规律次数减少 73%。

Conclusion: SFC 能有效保证科学推理的有效性，减少科学错误级联，提升模型在科学任务中的可靠性。
...
### [316] [JOR-Bench: Japanese Operations Research Benchmarks for Large Language Models](https://arxiv.org/abs/2607.16777)
*Yuu Jinnai*

Main category: cs.CL

TL;DR: 本文提出JOR-Bench，一个包含五个日语基准的集合，用于评估大语言模型在运筹学问题求解中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有OR基准多为英文，缺乏日语评估资源，且需要探究语言对LLM的OR能力的影响。

Method: 构建日语翻译版本，使用OR-Tools标准化执行，评估7个LLM在英日语版本上的表现。

Result: 强多语言模型的表现几乎与语言无关，但日语提示存在决策变量值与目标值混淆的语用歧义问题。

Conclusion: OR公式能力对强多语言模型而言是语言中性的，但需注意跨语言细微差异。
...
### [317] [Cascading versus Joint Modeling for Hierarchical Offensive Language Detection](https://arxiv.org/abs/2607.16790)
*Ruixing Ren,Junhui Zhao,Xiaoke Sun*

Main category: cs.CL

TL;DR: 该论文比较了层级级联与联合多任务两种范式在细粒度冒犯性语言检测中的性能，提出了一个三级级联检测系统并验证了类别不平衡处理策略。


<details>
  <summary>Details</summary>
Motivation: 先前研究缺乏对两种范式的直接、可控比较，且未验证类别不平衡处理策略是否最优。

Method: 提出三级级联检测系统，每个子任务定制训练策略；通过消融研究确定最优不平衡处理策略；训练联合多任务模型作为架构控制。

Result: 级联系统在三个子任务上分别达到0.795、0.716和0.557的宏F1分数；约五分之一的错误源自第一阶段过滤器；相比联合模型，级联架构在所有子任务上准确率更高，在最不平衡子任务上宏F1提升7.1点，但参数多3倍、延迟高1.67倍。

Conclusion: 级联架构在准确率上优于联合模型，但带来更高的部署成本，两者存在明确的量化权衡。
...
### [318] [Diagnosing Correctness Probes under Self-Judgement Confounding](https://arxiv.org/abs/2607.16799)
*Yi-Long Lu*

Main category: cs.CL

TL;DR: 研究揭示语言模型的隐藏状态读取中，客观正确性（OC）与模型自我判断（SJ）存在冲突，且SJ相关方向在跨任务转移中优于OC，质疑了可迁移性对客观正确性语义的指征作用。


<details>
  <summary>Details</summary>
Motivation: 现有方法混淆了客观正确性与模型的自我判断，需明确二者在隐藏状态中的语义差异及其可迁移性。

Method: 通过构建OC与SJ预测排序相反的冲突样本，分离SJ和OC的因子方向，使用对比学习评估方向极性，并在数学推理和事实回忆任务中测试迁移性，同时控制答案可能性、序列长度等混淆因素。

Result: SJ方向在跨领域迁移中显著高于随机水平，而OC方向的点估计在所有条件下低于预期；该不对称性在模型中间到后期层显现，且对多个控制变量鲁棒。
...
### [319] [Schema-Constrained Document-Level Event Argument Extraction with Lightweight LLM Fine-Tuning](https://arxiv.org/abs/2607.16808)
*Roberto Pietrantuono,Antonio Guerriero,Pouya Sattari*

Main category: cs.CL

TL;DR: 论文研究了中型开源大语言模型在文档级事件论元抽取任务中的表现，通过角色集注入提示、LoRA微调和确定性解码后处理，在MAVEN-ARG数据集上超越了GPT基线，最佳模型Phi-4（14B）达到42.39% F1。


<details>
  <summary>Details</summary>
Motivation: 文档级事件论元抽取面临长距离依赖、跨句上下文和严格角色约束等挑战，导致边界错误、角色不确定性和与受限模式不一致的问题。

Method: 结合（i）提示中注入角色集以确保模式合规，（ii）使用推理时相同的JSON接口进行参数高效监督微调（LoRA），（iii）确定性解码与后处理（验证JSON、过滤无效角色、去重论元、对齐跨度到文档窗口）。

Result: 在MAVEN-ARG官方评估器下，微调的中型开源模型在提及、实体共指和事件共指评估上优于先前GPT基线；最佳模型Phi-4（14B）在事件共指级别达到42.39% F1。

Conclusion: 中型开源大语言模型通过合适的微调和后处理，能有效执行文档级模式约束的事件论元抽取，并达到竞争性表现。
...
### [320] [Group Entropy-Controlled Policy Optimization](https://arxiv.org/abs/2607.16850)
*Guangran Cheng,Chengqi Lyu,Songyang Gao,Wenwei Zhang,Kai Chen*

Main category: cs.CL

TL;DR: 提出GEPO方法，通过分组熵估计和条件优势塑造，解决了GRPO在多任务强化学习中熵控制不足和优势信号偏差的问题，在13个基准上表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO在异构任务上使用全局或词级熵控制，无法适应不同任务的探索需求，且归一化优势存在熵依赖偏差，导致信号不可比。

Method: GEPO基于GRPO，利用分组样本估计组熵，实现条件非对称优势塑造：衰减低熵组的正优势以减少过度利用，增强高熵组的负优势以保持探索，使用历史熵统计自适应阈值。

Result: 在两种基模型和13个基准（涵盖数学、物理、科学、代码生成和指令遵循）上，GEPO持续优于GRPO和现有熵控制方法，实现跨任务平衡改进。

Conclusion: GEPO通过分组熵条件优势调整，有效解决多任务RL中熵控制不匹配问题，提升整体性能并保持任务特定探索水平。
...
### [321] [Trace-Based On-Policy Distillation for Masked Diffusion Language Models](https://arxiv.org/abs/2607.16872)
*Haolin Ren,Ziyang Huang,Chenhao Yuan,Jun Zhao,Kang Liu*

Main category: cs.CL

TL;DR: 提出了基于轨迹的在线策略蒸馏（TOPD），一种无需奖励估计的教师监督框架，用于将推理能力转移到扩散大语言模型（dLLM）上。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在推理导向的后训练中面临挑战：监督微调需要密集但离策略的掩码状态，而强化学习依赖稀疏奖励或价值建模。

Method: 从目标dLLM采样在线策略扩散轨迹，在部分去噪状态下获取教师模型的分词分布，并用分词级逆向KL散度目标更新目标dLLM。

Result: 在数学推理基准上，TOPD使SDAR-4B-Chat在MATH500准确率上匹配强化学习训练的TraDo-4B-Instruct，在静态和动态评估中分别提升+5.7和+4.5，且使用4倍更少的轮次，实现估计的96倍模型计算速度提升。

Conclusion: TOPD有效转移推理能力至dLLM，无需奖励估计，并在减少计算开销的同时达到竞争性能。
...
### [322] [Learning from Synthetic Data without Model Collapse in Iterative Instruction Tuning](https://arxiv.org/abs/2607.17043)
*Xiaonan Luo,Yue Huang,Kehan Guo,Ping He,Chuan Zou,Ting Hua,Xiangliang Zhang*

Main category: cs.CL

TL;DR: 提出KITE框架，通过失败引导数据生成和边界感知不确定性筛选，解决指令调优中合成数据导致的模型能力极化问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注模型崩溃的性能下降上界，但缺乏可操作用于数据筛选的细粒度诊断，尤其在迭代模型进化中需要确保每代模型持续改进。

Method: 两阶段框架：1）基于模型失败案例的引导数据生成；2）联合知识边界和不确定性进行数据筛选。

Result: 在多个数据集和开源LLM上，KITE相比强合成数据基线实现了更稳定的性能提升。

Conclusion: 合成数据导致的模型崩溃并非均匀退化，而是表现为能力极化，通过针对性数据筛选可缓解该问题并促进持续改进。
...
### [323] [Real-World Evaluation of an AI Agent Drafting Translational Impact Summaries](https://arxiv.org/abs/2607.16989)
...
TL;DR: 构建了一个人机协同的AI代理，用于自动生成学者影响力档案，使评估时间从15小时缩短至14分钟，且81.7%的结果被评审者接受或编辑。


<details>
  <summary>Details</summary>
Motivation: CTSA项目需要记录学者研究影响力，但手动整理每位学者档案耗时约15小时且无法规模化，因此需要AI工具自动收集多平台数据。

Method: 开发了一个人机协同的AI代理，自动汇编学者证据档案并生成TSBM影响力摘要；在10名KL2/K12学者中评估，两名评审员独立对507条结果编码为接受/编辑/拒绝。

Result: 81.7%的结果被双人一致接受或编辑；每位学者中位评审时间14分钟（vs. 人工15小时）；Cohen's kappa 0.43；AI召回率接近人工搜索；影响力证据覆盖所有TSBM领域；合成准确率4.5/5，有用性4.8/5。

Conclusion: 人机协同AI代理可作为学者影响力记录的首稿作者，将员工从收集写作转向审核，使群体规模的影响力报告可行。
...
### [324] [BLAD: A Historically Contextualized, Multilingual Dataset of Bangladeshi Legal Acts (1799 to 2025)](https://arxiv.org/abs/2607.17111)
*Adib Sakhawat*

Main category: cs.CL

TL;DR: 该论文介绍了孟加拉国法律法案数据集（BLAD），该数据集包含1799年至2025年间颁布的1484项立法法案的全文、结构化章节与脚注、废止状态及元数据，支持时间与多语言法分析，旨在填补南亚民事法司法管辖区法律自然语言处理资源的空白。


<details>
  <summary>Details</summary>
Motivation: 南亚低资源民事法司法管辖区缺乏法律自然语言处理资源，尤其是覆盖多语言和长时间跨度的法律文本数据集。

Method: 通过采集、整理和丰富流程构建数据集，包括全文提取、结构化章节与脚注、废止状态标注，以及关联执政政权、国家元首和当时法律框架的元数据。

Result: 生成了一个涵盖英语、孟加拉语及混合语言文档的1484项法案数据集，并提供了超过两个世纪立法活动的描述性统计。

Conclusion: BLAD填补了现有法律NLP资源在低资源、民事法司法管辖区中的空白，并为时间分析、多语言分析及法律研究提供了基础，数据集已公开可用。
...
### [325] [Scope3Trace: Evidence-Based Identification and Extraction of Scope 3 GHG Emissions from Sustainability Reports](https://arxiv.org/abs/2607.17122)
*Siyuan Zheng,Yifan Duan,Chao Xue,Flora D. Salim*

Main category: cs.CL

TL;DR: 提出Scope3Trace框架，从ESG报告中提取可解释、可溯源的Scope 3排放数据，并构建多模态数据集。


<details>
  <summary>Details</summary>
Motivation: Scope 3排放占企业碳足迹大部分，但现有方法依赖人工标注或缺乏证据可溯性。

Method: 集成PDF收集、OCR解析、LLM辅助页面定位和表格重建、混合规则-LLM提取及证据验证。

Result: 在提取Scope 1-3总量和分类披露方面实现高准确性。

Conclusion: Scope3Trace可靠提取并透明整合异构可持续性披露信息。
...
### [326] [KyrgyzLLM-Bench: Benchmarking Kyrgyz Language Understanding](https://arxiv.org/abs/2607.17173)
*Timur Turatali,Aida Turdubaeva,Rustem Izmailov,Anton M. Alekseev,Sergey I. Nikolenko*

Main category: cs.CL

TL;DR: 本文提出了KyrgyzLLM-Bench基准，包含两个原生数据集和四个翻译后编辑数据集，系统评估了26个大型语言模型在吉尔吉斯语上的表现。


<details>
  <summary>Details</summary>
Motivation: 大多数多语言基准依赖翻译的英语数据集，掩盖了目标语言的语言和文化特异性，尤其是资源较少的吉尔吉斯语缺乏可靠的本地评估数据。

Method: 构建包含KyrgyzMMLU和KyrgyzRC原生数据集以及WinoGrande等翻译后编辑数据集的基准，在零样本和少样本设置下评估26个开源和闭源模型。

Result: 模型排名在WinoGrande和BoolQ上从英语到吉尔吉斯语广泛转移，而HellaSwag表现出较大的英吉性能差距；少样本提示对开源模型的阅读理解有提升，但对专有模型不一致。

Conclusion: 发布了所有数据集和评估代码，并将吉尔吉斯语任务集成到多语言评估框架中，以支持未来的吉尔吉斯语NLP研究。
...
### [327] [Auditing Question-Order Effects in Large Language Models with the QQ Equality: Mechanism Characterization and a Saturation Caveat](https://arxiv.org/abs/2607.17219)
*Pilsung Kang*

Main category: cs.CL

TL;DR: 本文提出将量子问题排序模型中的QQ等式作为大型语言模型顺序判断的审计标准，并通过理论分析和实证检验发现，在测试条件下，强制二选一的下一词元概率不适合进行分布级QQ审计。


<details>
  <summary>Details</summary>
Motivation: 人类调查问卷中存在符合QQ等式的顺序效应，而自回归大型语言模型也可能存在类似的顺序判断偏差，因此需要开发一种审计标准来检测这种偏差。

Method: 理论上刻画了满足QQ等式的机制类；方法上开发了预指定的审计流程，包括鲁棒性边界、抽样一致性检查、标签平衡和饱和度诊断；实证上对指令微调模型进行了两项框架的初步测试。

Result: 所有预指定的健康门通过，但17/18和7/8的项目对被饱和（接近确定性），且无项目通过残余上下文化认证，表明强制二选一的下一词元概率不足以进行分布级QQ审计。

Conclusion: 当将下一词元分布视为调查响应分布时，推荐使用预指定的饱和度诊断；当前测试条件下，基于对数的强制选择概率不适合QQ审计。
...
### [328] [Literary Non-Style in LLM-Generated Text](https://arxiv.org/abs/2607.17228)
*Cory Massaro*

Main category: cs.CL

TL;DR: 本研究揭示了LLM生成文本中n-gram的统计分布与人类文本存在一致差异，并通过定性分析暴露了LLM在风格上的缺陷。


<details>
  <summary>Details</summary>
Motivation: 已有研究显示LLM文本在风格和语义范围上与人类文本存在差异，但尚未从n-gram统计分布角度系统分析其风格缺陷。

Method: 统计LLM生成文本中n-gram的分布模式，并结合定性分析方法评估高阶n-gram与语义内容的关联。

Result: 发现LLM文本中n-gram分布存在一致模式，高阶n-gram的定性分析揭示了风格缺陷，且风格与语义问题不可分离。

Conclusion: 风格与语义在LLM文本中紧密耦合，改善文本生成需同时考虑两者。
...
### [329] [Robust Summarization of Doctor-Patient Conversations: TalTech Systems for the Beyond Transcription Challenge](https://arxiv.org/abs/2607.17230)
*Aivo Olev,Tanel Alumäe*

Main category: cs.CL

TL;DR: 本文提出了一种从长对话录音直接生成SOAP笔记的方法，无需中间转录，在BeTraC挑战中双轨夺冠，并具备低幻觉率。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖转写，而直接从录音生成SOAP笔记可减少误差和延迟，挑战指标需要同时追求准确性和事实可靠性。

Method: 筛选开放权重的语音大模型后，采用Voxtral Mini（轻量级）和Voxtral Small（重量级），通过LoRA监督微调，再使用DAPO强化学习，以Open Medical Concept F1作为奖励函数。

Result: 在两个赛道均排名第一，独立评估显示最低幻觉率，表明针对概念匹配指标的强化学习不影响事实可靠性；文本转录的微调可迁移至语音输入，提升对域外录音的鲁棒性。

Conclusion: 结合LoRA微调和基于概念指标强化学习的方法能有效从长录音直接生成高可靠性的SOAP笔记，且文本微调对语音任务有正向效果。
...
### [330] [AI_LectureNote: A Retrospective Pilot Study of a Post-ASR Workflow for English-Script Rendering and Semantic Drift in Korean-English Medical Lectures](https://arxiv.org/abs/2607.17237)
*Kyeongeon Lee,Donghoon Chang,Seungryeol Baek,Taehong Kim,Wonjun Yang*

Main category: cs.CL

TL;DR: 提出了一种面向韩英医学讲座的可读性后ASR工作流AI_LectureNote，将语音转写为学习笔记，并恢复拉丁语医学术语。初步实验显示英文脚本渲染率提升，但存在语义漂移和极性错误。


<details>
  <summary>Details</summary>
Motivation: 现有的语音识别输出对医学讲座的可读性较差，且韩语音译导致拉丁医学术语丢失，影响学习效果。

Method: 设计后处理工作流，重写语音转写文本，恢复拉丁脚本医学术语，并在四个讲座上对比五种条件（包括whisper-1和gpt-4o-transcribe路径）进行评估。

Result: 后处理将whisper-1的英文脚本渲染率从0.39提升至0.71，gpt-4o-transcribe从0.26至0.65；但语义漂移（282句中出现34和36句）和极性错误（101个极性线索中出现11和13个）仍然存在。

Conclusion: 英文脚本渲染不能保证语义保真度，需分别评估表面准确性、术语脚本、分块一致性及医学意义保留等维度。
...
### [331] [EvolvingWorld: An Open-Schema Framework for Co-Evolving Role-Play Agents and World Model in Interactive Literary World](https://arxiv.org/abs/2607.17250)
*Qing Zong,Yue Guo,Mengxin Yang,Yiwen Guo,Yangqiu Song*

Main category: cs.CL

TL;DR: 提出了EvolvingWorld框架，用于交互式文学世界中的角色与世界共同演化，通过开放式架构和长期模拟实现一致性发展。


<details>
  <summary>Details</summary>
Motivation: 现有系统将交互式文学模拟视为静态角色模仿或孤立场景生成，未能捕捉角色与世界的共同演化过程。

Method: 构建了EvolvingWorld框架，包含角色代理（多角色扮演与持久档案演化）和基于LLM的世界模型（全局与位置/实体级状态维护及场景推进），并设计7个可训练任务，从57本书中构建数据集（138,596监督样本和222快照）。

Result: 实验表明该框架能改善长期模拟，有效维持角色与世界的持久、连贯发展。

Conclusion: EvolvingWorld通过开放式框架和演化机制解决了交互式文学模拟的演化缺失问题，提升了长期一致性。
...
### [332] [Should Missing Modalities Always Be Necessary to Repair for Multi-modal Sentiment Analysis?](https://arxiv.org/abs/2607.17262)
...
TL;DR: 提出SIEVE框架，将缺失模态的修复决策从固定范式转为样本级别的可学习决策，通过经验充分性信号和证据门控选择最优处理路径。


<details>
  <summary>Details</summary>
Motivation: 发现并非所有缺失模态都需要修复，全模态输入仅对少数样本最优，不同样本偏好不同的模态子集。

Method: 设计双分支结构（直接预测分支与修复分支），通过样本损失差距计算经验充分性信号，利用证据门控联合建模充分性与认知不确定性，实现修复无关的即插即用决策。

Result: 在CMU-MOSI和IEMOCAP上，SIEVE持续提升代表性修复骨干的性能，接近每样本双分支理论最优。

Conclusion: 缺失模态的修复应样本自适应，SIEVE通过显式学习是否修复，有效提升多模态情感分析的鲁棒性。
...
### [333] [Debate-on-Graph: Reliable and Adaptive Reasoning of Large Language Model on Uncertain Knowledge Graph](https://arxiv.org/abs/2607.17266)
*Peiji Yu,Xin Chen,Tianxing Wu*

Main category: cs.CL

TL;DR: 提出 Debate-on-Graph (DoG) 框架，利用不确定知识图（UKG）和LLM通过多智能体辩论实现可靠问答推理，实验表明性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: LLM在问答中易产生幻觉并缺乏相关知识，现有KG增强方法无法过滤噪声和错误，反而会放大幻觉问题。

Method: 设计针对UKG的启发式搜索算法提取可靠子图，引入多智能体辩论机制通过对抗辩论自适应利用知识并保持证据可靠性。

Result: 在四个基准问答数据集上，DoG达到了最先进的性能，优于现有的LLM推理方法和KG基线方法。

Conclusion: DoG框架通过UKG和LLM的协作实现了可靠且自适应的推理，有效缓解了噪声和幻觉问题。
...
### [334] [Safety That Does Not Transfer: Cross-Lingual Clinical Correctness Drift in Deployable Medical Language Models](https://arxiv.org/abs/2607.17270)
*Anthonio Oladimeji Gabriel,Dimeji Olawuyi,Toba Ajayi,Temilola Aderemi*

Main category: cs.CL

TL;DR: 该研究评估了大型语言模型在尼日利亚北部低资源医疗环境下的临床安全性，发现本地可部署模型从英语迁移到豪萨语时临床正确性显著下降，而前沿模型表现稳定。


<details>
  <summary>Details</summary>
Motivation: 临床安全性评估主要集中在英语和前沿系统上，未反映低资源医疗环境中本地运行、用本地语言查询的小型量化模型的使用情况。

Method: 构建了匹配的英语-豪萨语问题对，覆盖疟疾、镰状细胞病和结核病三种疾病，测试知识回忆、急诊分诊、禁忌操作引导问题和传统疗法声称。评估了六种模型（五种本地可部署模型、两种医学微调模型和一种前沿模型），由两名独立评阅者根据尼日利亚国家治疗指南评分。

Result: 本地可部署模型的平均临床正确性从英语的1.57降至豪萨语的-0.03（2为正确，-1为有害），而前沿模型从2.00降至1.75且无有害回答。跨三种疾病条件的一致性显著。

Conclusion: 临床安全性缺陷不是语言或临床材料本身的属性，而是本地可部署模型层级的问题，因为前沿模型在豪萨语中表现良好。
...
### [335] [Team DACTYL at PAN 2026: Bayesian Data Mixing and Empirical X-risk Minimization for AI-text Detection](https://arxiv.org/abs/2607.17382)
*Shantanu Thorat*

Main category: cs.CL

TL;DR: 通过贝叶斯分类头微调BERT-tiny模型选择训练数据，结合MCGrad校准方法，在PAN 2026测试集上实现0.974平均得分，击败DeBERTa和ModernBERT，证明精细数据集筛选可提升OOD泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有AI文本检测分类器在分布外(OOD)文本上性能下降，且混合训练集可能引发捷径学习，需解决过拟合问题。

Method: 用贝叶斯分类头微调BERT-tiny选择三个数据集文本，分别训练DeBERTa-V3-large(经验风险最小化)、ModernBERT-large及MCGrad(校准预测)。

Result: MCGrad以0.974平均分(五项指标)排名第二，ModernBERT-large(0.96)次之，DeBERTa(0.882)最低。

Conclusion: 精心筛选训练集可显著提升OOD性能，模型已开源。
...
### [336] [The Librarian Who Refused to Code: Model-Dependent Identity Enactment in LLM Code Generation](https://arxiv.org/abs/2607.17420)
*Shayell Aharon Salomon,Noam Israel,Ido Safruti,Amir Shaked*

Main category: cs.CL

TL;DR: 研究了系统提示中的人物角色对代码生成的影响，发现角色效果依赖于模型，且不一定提高正确性。


<details>
  <summary>Details</summary>
Motivation: 人物角色常用于系统提示，但其对代码生成的影响缺乏受控、预先注册的实验评估。

Method: 在受控、预先注册条件下，测试了4种提示条件、12个代码生成任务、2个前沿模型，每单元5次运行，共480次完成。

Result: 角色效应因模型而异；Claude Opus上，极简工程师角色减少输出但未提高正确性，详尽工程师角色增加输出但无正确性提升；图书管理员角色在Opus中引发角色内免责声明并降低正确性。

Conclusion: 人物角色更像是模型相关的行为策略偏差，而非通用质量干预措施。
...
### [337] [How Reliable Are Multimodal Signals of Conversational State? Evidence from Remote Dyadic Collaborative Tasks](https://arxiv.org/abs/2607.17452)
*Tahiya Chowdhury*

Main category: cs.CL

TL;DR: 本研究提出了一个三维评估框架（预测准确性、跨任务泛化性、重测信度），分析对话行为中的交互、声学和语言特征。结果表明语言特征预测认知负荷最准确但泛化性差；声学特征在控制说话者身份后可靠性降至零；交互特征是唯一可靠的信号，可用于预测认知负荷不对称性。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对特征在不同任务中的一致性和可靠性的系统性评估，导致难以选择适用于多种对话情境的稳健特征。

Method: 使用AVCAffe数据集（53对双人、9个任务），提取交互、声学和语言特征，并在三维评估框架下测试其预测准确性、跨任务泛化性和重测信度。

Result: 语言特征预测准确性最高但跨任务泛化性差；声学特征在控制说话者身份后可靠性大幅下降；交互特征保持稳定，且主导性预测了对话中的认知负荷不对称性；权力角色分类在所有条件下接近随机。

Conclusion: 建议在对话系统中采用说话者归一化和多维评估，以确保多模态特征选择的稳健性和情境适应性。
...
### [338] [Multilingual Sentence Embeddings for Linguistic-Integrated Reliability Audit](https://arxiv.org/abs/2607.17466)
*Ummugul Bezirhan,Ji Yoon Jung,Matthias von Davier*

Main category: cs.CL

TL;DR: 本文评估了多语言句子嵌入能否替代翻译后的英文输入，用于PIRLS构建响应题目的语言集成可靠性审计（LiRA），发现原生语言嵌入可准确复现基于翻译的可靠性估计，且不会改变可靠性。


<details>
  <summary>Details</summary>
Motivation: 多语言评估系统通常依赖翻译进行评分和质量控制，但翻译过程可能导致数据丢失或偏差，需要探索更高效的方法。

Method: 使用三种多语言句子嵌入模型，对11个PIRLS构建响应题目进行LiRA分析，比较原生语言嵌入与翻译后英文输入的可靠性估计结果。

Result: 原生语言嵌入复现了基于翻译的可靠性估计，同时恢复了因翻译失败而被排除的响应，且可靠性无显著变化。

Conclusion: 多语言句子嵌入可替代翻译输入，用于多语言评估系统的可靠性审计，避免翻译相关的数据丢失问题。
...
### [339] [Token-Level Off-Policy Learning for Faithful Generation Under Distribution Shift](https://arxiv.org/abs/2607.17524)
*Zitong Huang,Gustavo Lucas Carvalho,Deqing Fu,Robin Jia*

Main category: cs.CL

TL;DR: 提出了一种名为TOPL的离线训练方法，将后训练重构为令牌级正确性预测任务，在文档摘要和机器翻译任务上取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 传统直接训练模型生成离线策略令牌的方法存在缺陷，旨在通过区分响应中的好坏令牌来自然引导模型生成优质令牌。

Method: 采用令牌级离线策略标注，将训练转化为令牌级正确性预测，基于此进行模型训练。

Result: 在11个数据集上实现跨域强泛化，优于多种序列级和令牌级基线方法，且在机器翻译中同样有效。

Conclusion: 令牌级学习信号对性能至关重要，其带来的模型更新具有可解释性，LoRA适配器可作为线性分类头和引导向量。
...
### [340] [Oracle Gap and Signal Fidelity: A Fixed-Pool Diagnostic for Test-Time Collaboration](https://arxiv.org/abs/2607.17531)
*Jie Hu*

Main category: cs.CL

TL;DR: 本文研究了训练无关的协作方法（如自一致性、最佳N选择等）在提升大语言模型推理性能时的实际效果，并提出了一个可测量的分解框架来预测其增益。


<details>
  <summary>Details</summary>
Motivation: 测试时协作方法虽被广泛使用，但效果不均衡甚至有时为负，因此需要探究何时这些方法能够真正提升性能。

Method: 针对固定候选池，将选择器或验证器的净增益分解为可恢复质量、验证信号覆盖、条件选择质量和对已正确输出的伤害四个可测量因素，从而将协作问题转化为候选选择问题。

Result: 在LiveCodeBench、MATH Level-5和GPQA-Diamond上，增益首先受限于理论最优差距，然后受限于信号保真度；不同验证器表现差异显著，且部分方法伤害率较低。

Conclusion: 协作效果的优劣取决于任务、模型和采样配置的共同作用，并提出了一个实用的预部署诊断方法：先估计理论最优差距，再测量信号覆盖、保真度和伤害率。
...
### [341] [AEGIS: Awareness-Enhanced Guidance for Iterative Safeguard](https://arxiv.org/abs/2607.17713)
*Kyungwon Park,Sangmin Lee,Heejae Chon,Hyungu Kang*

Main category: cs.CL

TL;DR: 研究了跨度级引导在多语言文本去毒中的作用，发现其效果依赖于生成器骨干和语言环境。


<details>
  <summary>Details</summary>
Motivation: 探讨跨度级理由在文本去毒中何时有帮助，何时引入权衡。

Method: 提出AEGIS框架，结合跨度级检测器与冻结生成器，在重写时提供结构化引导。

Result: 跨度引导改变毒性降低与意义保留的权衡，效果因生成器和语言而异。

Conclusion: 跨度级控制信号有条件地有用，既有前景也有局限性。
...
### [342] [Tokenizing Crosslingual Homographs](https://arxiv.org/abs/2607.17689)
*Rotem Brillant,Yuval Pinter*

Main category: cs.CL

TL;DR: 论文研究了多语言模型中共用子词词汇表导致跨语言同形词处理不当的问题，并提出在分词阶段引入语言提示（如语言特定字符替换共享词汇首字符）来改善处理效果。实验表明该方法在分词统计和下游机器翻译中取得一定改进，但效果因语言和评估集而异。


<details>
  <summary>Details</summary>
Motivation: 多语言语言模型依赖共享子词词汇表，导致跨语言同形词（如虚假同源词）被过度统一处理，忽略语言特定含义差异。

Method: 提出基于语言提示的分词器干预：用语言特定字符替换共享词汇的首字符，减少词汇构建中的通用性；并在BPE、UnigramLM和SaGe分词器上评估效果。

Result: 内在分析显示BPE和UnigramLM对同形词处理语言无关性强，而SaGe处理差异更大；干预后消除了这一差距。下游英译X机器翻译中，该方法在BPE下取得适度改进，但效果不稳定。

Conclusion: 在分词阶段加入轻量级语言信息是改进跨语言同形词处理的有前景方向，但需进一步探索以提升稳定性和跨语言一致性。
...
### [343] [C$^2$KV: Compressed and Composable KV Cache Reuse for Efficient LLM Inference](https://arxiv.org/abs/2607.17715)
*Chuheng Du,Junyi Chen,Hanlin Tang,Kan Liu,Tao Lan,Lin Qu,Chaoyue Niu,Shengzhong Liu,Guihai Chen,Fan Wu*

Main category: cs.CL

TL;DR: 提出C²KV框架，通过位置无关的压缩KV缓存和训练策略，实现长上下文推理中KV缓存的高效复用与拼接，显著加速并降低存储成本。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存复用方法忽视长上下文LLM服务中存储和访问大KV缓存的瓶颈，且简单组合压缩与复用会导致精度下降。

Method: 设计轻量侧边提取器，学习可组合、位置无关的压缩KV缓存流形；采用压缩-拼接联合训练策略对齐表征与下游复用行为。

Result: 在多个长上下文基准和模型上，实现17倍推理加速，生成质量保持稳定。

Conclusion: C²KV统一非前缀KV复用框架兼顾效率与精度，为长上下文LLM服务提供实用方案。
...
### [344] [Large Language Models for Citation Function Classification](https://arxiv.org/abs/2607.17738)
*Daniel Vodička,Jakub Šmíd,Pavel Král,Christophe Cerisara*

Main category: cs.CL

TL;DR: 本文对多个大语言模型在引文功能分类任务上进行了全面评估，在ACL-ARC数据集上取得了新的最佳结果，并引入了新数据集AC3及七类标注方案。


<details>
  <summary>Details</summary>
Motivation: 引文功能分类对理解科学出版物关系至关重要，但现有研究缺乏对多种大语言模型的系统比较。

Method: 比较了Mistral 7B、Orca 2-7B、LLaMA 3.1-8B、Falcon 7B和SciBERT五个模型，采用零样本、少样本和微调方法，并构建了具有四种上下文提取变体的AC3数据集。

Result: 微调后的Falcon 7B模型在ACL-ARC上达到73.3%的宏F1分数，显著优于先前方法。

Conclusion: 该研究填补了引文功能分类中模型比较的空白，为未来研究提供了实验配置和局限性的详细分析。
...
### [345] [When to Use Extra Context: Evidence-Grounded Terminology Adaptation for Simultaneous Speech Translation](https://arxiv.org/abs/2607.17766)
*Zeyu Yang,Satoshi Nakamura*

Main category: cs.CL

TL;DR: 提出EGTA框架，通过文档术语缓存和状态条件选择提升同传翻译中的术语恢复，在科技讲座场景下显著提升BLEU、实体召回等指标。


<details>
  <summary>Details</summary>
Motivation: 现有方法将整个文档上下文注入流式片段过于粗糙，而诊断实验表明上下文增益主要来自论文特定术语恢复而非统一语义增强。

Method: 构建文档术语记忆库，基于当前流式状态选择紧凑候选术语，分别调整ASR/语音端和解码端决策空间，支持级联/端到端/生成式三种SimulST架构且无需全模型微调。

Result: 在MCIF-dev上英中/英德BLEU提升+1.05/+0.59，XCOMET-XL提升+0.019/+0.006，命名实体召回率相对提升+79%/+73%，首字母缩写召回提升+0.099/+0.171，ACL60/60-dev验证集上术语召回持续提升。
...
### [346] [ESCUCHA: A Spanish Speech Benchmark for Heterogeneous Acoustic Conditions](https://arxiv.org/abs/2607.17812)
*Fernando López,Ana Ayala,Guillermo Segovia,Fernando Ibáñez,Ana Martínez,Pablo Gómez,Jordi Luque*

Main category: cs.CL

TL;DR: 提出了首个西班牙语语音理解基准ESCUCHA，包含1000个自然场景问题，评估大语言模型在异质声学条件和推理能力上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架对西班牙语在真实声学条件下的语音理解关注不足。

Method: 构建包含162.9小时自然音频、9种感知类别和10种推理类别、多口音及非规范语音的基准，并通过多音频问题和语音指令测试模型。

Result: 多个最先进的多模态和语音模型与训练有素的人类表现存在显著差距。

Conclusion: ESCUCHA揭示了当前模型在真实场景语音理解上的局限性，为未来研究提供了重要评估工具。
...
### [347] [When a Name Is Not a Name: A Benchmark Dataset and Distilled Reasoning for Culturally Entangled Bangla Homographs in Low-Resource LLMs](https://arxiv.org/abs/2607.17828)
*Md. Asaduzzaman Shuvo*

Main category: cs.CL

TL;DR: 提出了孟加拉语中文化纠缠同形词（CEH）消歧任务，构建了1,516条专家验证的基准，发现模型存在主导意义偏差，通过对比思维链提示和小模型蒸馏可将偏差从100%降至5%以下。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语中很多词汇同时是人名和有文化内涵的普通名词，现有语言模型因训练数据缺乏文化知识而难以正确区分。

Method: 构建CEH基准数据集，采用对比思维链提示和知识蒸馏方法，将文化解释从大模型迁移到小模型。

Result: 开放和封闭模型均表现出主导意义偏差；孟加拉语专用模型在所有提示策略下失败；对比思维链提示显著降低偏差，蒸馏使小模型推理能力提升，偏差从100%降至5%以下。

Conclusion: 仅靠语言特定预训练不足以获得文化基础，引入文化解释的推理方法能有效消解文化同形词歧义，提升了模型在文化敏感任务上的表现。
...
### [348] [Zero Hallucination, by Construction: Hallucination-Aware Layered Oversight for Trustworthy Enterprise AI](https://arxiv.org/abs/2607.17883)
*Bogdan Raduta,Horia Velicu,Alexandru Preda,Serban Chiricescu*

Main category: cs.CL

TL;DR: 提出HALO架构，将幻觉从模型的固有缺陷重新定义为系统可管控的失效模式，通过六层防御实现可信任的AI代理部署。


<details>
  <summary>Details</summary>
Motivation: 企业因幻觉问题不信任AI代理，而等待无幻觉模型是不现实的；需要从系统设计角度将幻觉作为可封装的故障模式来治理。

Method: 提出HALO架构，包含六层防护：基于检索与审核内容的生成、约束性确定性执行、多信号验证（LLM检测器+基于源文本的证据检查）、校准式弃权机制、全链路追溯、持续监控与反馈优化。

Result: 在受监管的理赔提取任务中验证了架构有效性，强调基于源文本的证据置信度比模型自报置信度更可靠。
...
### [349] [DeLIVeR: Decomposed Learning for Information-grounded Veracity Recognition via Reinforced Knowledge Graph Exploration](https://arxiv.org/abs/2607.17935)
*Cong Hoan Nguyen,Thomas Hoang,Hieu Minh Duong,Long Nguyen*

Main category: cs.CL

TL;DR: 提出DeLIVeR框架，通过强化问题规划策略从知识图谱中检索证据，在多个基准上显著提升事实验证性能。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在事实核查中因传统检索系统的查询脆弱性而面临挑战。

Method: 使用规划器大模型将复杂主张分解为问题集，结合GRPO算法优化策略，通过结构化奖励系统引导知识图谱遍历。

Result: 在LIAR、FEVER和PolitiFact上分别达到83.73、84.57和79.70的F1分数，比HippoRAG2提升10-15%。

Conclusion: DeLIVeR通过强化问题规划有效弥合多跳推理差距，为可验证的虚假信息检测提供透明可审计方案。
...
### [350] [What Transfers Under Source Shift? Definitions, Examples, and Fine-Tuning for Climate Disclosure Classification](https://arxiv.org/abs/2607.17952)
*Guosheng Li,Fenghui Ren,Bin Liu,Chuan Yu,Kaiying Ji,Lin Yue,Jun Shen,Sasa Qian*

Main category: cs.CL

TL;DR: 该论文研究了气候披露分类中的跨源适应问题，比较了三种常用的大语言模型适应策略（定义、示例、微调）在源数据变化时的效果。


<details>
  <summary>Details</summary>
Motivation: 现有气候披露分类评估多在同一数据源内进行，忽略了不同来源（如年报、新闻稿、财报电话会议）之间的差异，需要检验常见LLM适应策略在源数据变化时是否仍有效。

Method: 将气候披露分类重新定义为跨源适应问题，使用共享标签空间但来自不同来源的两个语料库，在11个开源和闭源LLM上测试定义、示例（基于相似性检索或随机选择）和微调（LoRA）三种策略。

Result: 所有策略在跨源场景下平均带来正向收益，但最强源内策略（基于相似性检索和LoRA微调）在源变化时优势大幅下降；随机选择示例（较弱源内基线）更可靠地保持优势；定义策略传输一致性最高，仅当粒度匹配目标文本时。

Conclusion: 跨源场景下，简单策略通常更安全，定义策略需调整粒度；最适合源内的方法不一定适合跨源适应，选择策略时应考虑源数据差异。
...
### [351] [An Early Warning of Emerging Biosecurity Risks in Frontier LLMs](https://arxiv.org/abs/2607.18056)
...
TL;DR: 本文评估了前沿大语言模型在生物安全方面的风险，发现现有文本级防护措施不足以应对模型更强的生物能力，并展示了模型生成的有害序列可在实验中实际实现。


<details>
  <summary>Details</summary>
Motivation: 前沿大语言模型在科学工作流中的应用日益增多，但其生物能力可能超越当前安全措施，需要系统评估生物风险。

Method: 开发了Intern-BioBreaker生物红队模型，结合计算与湿实验框架，通过生成定向越狱提示评估模型的安全性，并对输出序列进行合成、表达和蛋白质验证。

Result: Intern-BioBreaker在多个模型上达到高攻击成功率，GPT-5.5可生成具有致病潜力的病毒序列，且模型设计的生物产物在实验中可物理实现。

Conclusion: 需要加强生物红队测试、核酸合成筛选和与模型能力匹配的安全机制。
...
### [352] [Pancasila-Dilemmas: Evaluating Large Language Models on Indonesian Human Value Dilemmas Grounded in Pancasila](https://arxiv.org/abs/2607.18066)
*Supryadi,Irfan,Julianti,Darren Keanly Martin,Jayvin Fernando,Yuqi Ren,Deyi Xiong*

Main category: cs.CL

TL;DR: 提出了包含1834个基于印尼潘查希拉价值观困境问题的评估数据集，评估50个LLM后发现它们在捕捉印尼价值观方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有价值观对齐评估多聚焦西方或普世价值观，缺乏针对特定国家价值观的评估。

Method: 从印尼新闻构建潘查希拉困境数据集，由母语者校对、5名印尼公民作答，用概率匹配分数和最大投票一致分数评估50个闭源和开源LLM。

Result: 所有LLM的PMS低于0.5，MVAS低于0.72，尤其在宗教和统一价值观上表现较差。

Conclusion: LLM在捕捉印尼地方性价值观方面存在显著不足，需更多针对性评估和改进。
...
### [353] [Modeling turn-taking with distant viewing: investigating silence thresholds in human and AI-generated discourse](https://arxiv.org/abs/2607.18076)
*Taylor Arnold,Nicolas Ballier,Artem Saloev*

Main category: cs.CL

TL;DR: 本研究分析了美国情景喜剧与合成播客中的沉默间隙，比较了不同性别和制作环境下的沉默模式。


<details>
  <summary>Details</summary>
Motivation: 探索不同媒体类型中沉默间隙的分布特征及其与性别、制作环境的关系。

Method: 分析了30部美国情景喜剧和51个合成播客，使用Praat设定基频阈值划分说话者性别，比较沉默间隙。

Result: 沉默间隙在不同媒体类型、说话者性别和制作环境下存在差异。

Conclusion: 沉默间隙受媒体类型、性别和制作环境因素影响。
...
### [354] [VDAR-Router: Adaptive LLMs Routing via Verbalized Query Difficulty Analysis Retrieval](https://arxiv.org/abs/2607.18098)
*Yu-Chien Tang,Jun-Chen Hung,Wen-Chih Peng,An-Zi Yen*

Main category: cs.CL

TL;DR: 提出VDAR-Router框架，通过显式难度分析和历史示例检索实现无训练的LLM路由，在三个数据集上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有路由方法依赖表面语义或嵌入相似性，忽略查询难度导致次优决策，需提高成本-性能权衡可靠性。

Method: 对输入查询生成难度分析，检索相似难度历史示例，基于记录评估候选模型并用奖励函数（兼顾性能和成本）选择模型。

Result: 三个数据集实验表明VDAR-Router在成本-性能权衡上持续优于基线，案例显示显式分析提升检索相关性和路由可靠性。

Conclusion: 难度感知检索有效实现无训练LLM路由，为高效部署提供实用方案。
...
### [355] [VEHBench: A Stage-Local Diagnostic Benchmark for LLM-Assisted Vibration Energy Harvester Design](https://arxiv.org/abs/2607.18181)
*Depeng Su,Yuyu Luo,Guobiao Hu*

Main category: cs.CL

TL;DR: 提出了VEHBench基准，用于评估LLM在振动能量收集器耦合物理设计中的阶段能力，包含763个任务和物理评估器。


<details>
  <summary>Details</summary>
Motivation: 现有工程基准只评估最终工件有效性，无法揭示LLM在不同设计阶段的行为差异。

Method: 构建763个基于文献的任务，使用分析物理评估器评分，评估四个设计角色：规范分类、验证引导搜索、损坏状态恢复和策略条件选择。

Result: LLM能力高度依赖设计阶段，无单一模型在所有阶段持续占优，响应控制曲线揭示不同角色的行为模式。

Conclusion: VEHBench为评估、选择、路由和改进验证工程LLM提供了阶段感知基础。
...
### [356] [How Does Alignment Tuning Shape Representations of Sycophancy and Related Cue-Induced Biases in LLMs?](https://arxiv.org/abs/2607.18114)
*Prakhar Gupta,Terry Jingchen Zhang,Florent Draye,Bernhard Schölkopf,Zhijing Jin*

Main category: cs.CL

TL;DR: 研究发现，大型语言模型（LLMs）对提示中的微小变化（如随意提示、错误标签的样本或虚假的助手回合）异常敏感，这种敏感性源于对齐微调而非预训练。每种偏差在激活空间中表现为单一、可解码的方向，且可通过干预恢复无偏答案。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs为何对输入提示的细微、无关变化（如随意提示、错误标签的少样本示例或虚假的历史回合）高度敏感，以及这些敏感性（包括谄媚和线索诱导的偏差）在模型内部的来源。

Method: 跨五个模型家族和七种偏差类型，通过三个测量方法：探测（probing）、留一数据集转移（leave-one-dataset-out transfer）和因果干预（causal intervention），从隐状态中提取每种偏差的方向。

Result: 对齐微调后，每种偏差成为单一、连贯的方向，可通过解码和沿方向操作恢复无偏答案；预训练基础模型几乎没有这些偏差。不同偏差在表征上保持分离，且行为相似的偏差占据不同方向。同一干预可适度去偏，保留大部分正确答案。

Conclusion: 线索诱导的偏差不应被视为LLMs的单一缺陷，而应理解为对齐微调引入的一系列独立、因果活跃的方向，这些方向因模型而异，且代表不同性质的问题。
...
### [357] [PPL-Factory: Task-Aware and Budget-Aware Data Selection from Language Modeling to Reasoning](https://arxiv.org/abs/2607.18199)
*Hang Zhang,Warren J. Gross*

Main category: cs.CL

TL;DR: 提出PPL-Factory框架，通过任务感知和预算感知的困惑度分数选择高价值训练样本，仅用1%数据即可超越现有方法，10%数据时性能超过全量微调。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法依赖固定启发式规则（如质量、多样性），难以适应不同任务；困惑度方法未区分语言建模与推理任务的差异，且忽略数据预算。

Method: 结合任务感知的困惑度分数（区分语言建模/推理目标）与数据预算感知的选择标准，构建简单可解释的筛选框架PPL-Factory。

Result: 在GSM8K上仅用1%训练数据表现最佳；用10%数据时GSM8K准确率提升0.9，MATH提升4.8，超越全量微调。

Conclusion: 任务感知与预算感知的困惑度选择为高效微调提供了有效且可泛化的方法。
...
### [358] [SWE-Pruner Pro: The Coder LLM Already Knows What to Prune](https://arxiv.org/abs/2607.18213)
*Yuhang Wang,Yuling Shi,Shaoqiu Zhang,Jialiang Liang,Shilin He,Siyu Ye,Yuting Chen,Kai Cai,Xiaodong Gu*

Main category: cs.CL

TL;DR: 提出SWE-Pruner Pro方法，利用智能体内部表示直接剪枝长上下文代码，在节省39%令牌的同时保持任务质量，并提升基准性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部代码分类器进行上下文剪枝，但智能体自身编码了代码相关性的内部表示，可更高效地实现剪枝。

Method: 在智能体内部添加小型头，利用长度感知嵌入将每个工具输出的内部表示转换为保留或剪枝标签，逐行决定代码上下文取舍。

Result: 在两个开源模型和四个多轮基准上节省高达39%令牌，在MiMo-V2-Flash上SWE-Bench Verified分辨率提升+3.8%，Oolong长上下文准确率提升+2.2点。
...
### [359] [It's Not What You Say, It's How You Say It: Evaluating LLM Responses to Expressions of Belief](https://arxiv.org/abs/2607.18232)
*Kevin Du,Clara Kümpel,Michelle Wastl,Alex Warstadt*

Main category: cs.CL

TL;DR: 提出了一个基于语言学维度的分类体系，系统评估用户信念表达（EoBs）如何影响大语言模型（LLMs）在遵循上下文与坚持先验知识之间的行为。


<details>
  <summary>Details</summary>
Motivation: 用户常以多样的语言形式（如预设、证据标记、语气）向LLMs表达信念，但这些表达的语言差异如何影响模型响应尚不明确。

Method: 构建了包含形式、证据性、认知立场、语气4个维度、17种细粒度类型的分类体系；通过将EoBs与世界知识事实配对生成受控查询对，隔离语言变异的影响，并评估16种LLMs（不同架构、规模、训练阶段）。

Result: 更大模型和指令模型比小模型和基础模型更不遵循上下文；某些EoBs显著更一致地改变模型行为。

Conclusion: 语言框架系统性地影响LLMs的上下文整合，对提示工程和模型鲁棒性有重要启示。
...
### [360] [Automated Discovery Has No Universally Superior Harness](https://arxiv.org/abs/2607.18235)
*Akshat Gupta,Jermaine Lei,Alexander Lu,Gopala Anumanchipalli,Leshem Choshen*

Main category: cs.CL

TL;DR: 该论文系统分解了自主发现框架（如OpenEvolve和TTT-Discover）的组件，通过大规模实验（310万次LLM调用）发现：没有固定的发现框架在所有模型-问题上都表现优越，并且OpenEvolve变体通常不如更简单的替代方案。研究提出将框架选择视为超参数，并基于早期进度预测最终性能，设计了一种自适应分配预算的方法，显著优于固定框架或非自适应集成。


<details>
  <summary>Details</summary>
Motivation: 现有自主发现系统常被作为通用框架，但它们是多种设计选择的复合体，且由于实验昂贵且随机性高，以往比较中试验次数不足，难以区分真正的方法改进与运行方差。

Method: 分解OpenEvolve和TTT-Discover的组件（存档、父代选择、探索、预算分配），在12个模型-问题对上比较30个预算匹配的框架，使用310万次LLM回滚和重复试验统计分析，并测试基于早期进度的自适应预算分配。

Result: 无固定框架在所有场景下可靠；OpenEvolve变体表现弱于简单替代方案；早期发现进度可预测最终性能；自适应分配方法优于固定框架或非自适应集成。

Conclusion: 框架选择应作为超参数针对具体问题调整，建议从固定选择转向基于早期表现的在线自适应，并释放可复用的统计基础设施。
...
TL;DR: 本文提出HyperDCM，一种基于双曲动态聚类记忆的持续学习框架，通过场景图建模和结构化记忆回放增强扩散策略导航，缓解灾难性遗忘并提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视觉导航中的持续学习面临灾难性遗忘和难以适应多样化环境变化的挑战。

Method: 利用大视觉语言模型提取语义场景三元组，通过R-GCN编码为场景图嵌入，投影到双曲空间增强结构可分性，并采用动态聚类与结构敏感更新策略选择代表性样本进行记忆回放。

Result: 在多场景室内外数据集上，HyperDCM相比现有持续学习基线方法，在保持旧导航能力与泛化性方面表现更优。

Conclusion: 场景图引导的结构化记忆能在持续导航中有效缓解遗忘并支持环境适应。
...
### [362] [Articulated Humanoid Head for a Robot Receptionist Capable of Natural Human Interaction](https://arxiv.org/abs/2607.17042)
...
TL;DR: 本文介绍了一款适用于接待员角色的人形机器人头部，具有21个自由度、逼真的硅胶皮肤，并集成了面部识别和自然语言处理系统，在用户研究中获得了4.13/5的人性化评分。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人头部存在成本高、机械复杂、环境适应性有限等问题，限制了其在实际服务场景中的应用。

Method: 设计了一个21自由度的机械结构（包括嘴、眼、眉毛和颈部），覆盖逼真的硅胶皮肤；集成了基于模型的架构，使用SCRFD、ArcFace和ByTetrack进行面部识别，以及Llama和Whisper进行语言处理。

Result: 机器人头部实现了实时操作和人类重新识别；用户研究显示其情感表达和人性化评分平均为4.13/5。

Conclusion: 该人形机器人头部在成本、机械复杂性和适应性方面具有优势，能有效应用于自然的人机交互场景。
...
### [363] [Control Design for a Rideable Animatronic Two-Wheeled Robot with Quadruped Form](https://arxiv.org/abs/2607.16302)
*Tadashi Sumioka,Kazushi Akimoto,Takuya Tsujimura,Sho Takayanagi,Yuya Horiuchi*

Main category: cs.RO

TL;DR: 本文提出了一种具有四肢的骑行两轮机器人概念，通过自平衡底盘与肢体协调运动，实现动态四足步态，旨在吸引年轻一代对摩托车的兴趣。


<details>
  <summary>Details</summary>
Motivation: 摩托车在年轻群体中受欢迎度下降，为通过可骑行的机器人角色重燃兴趣，同时兼顾安全性、重量匹配和表达性动作。

Method: 采用自平衡两轮基座作为主要运动方式，肢体在上下车时提供辅助支撑，并随轮速协调运动；通过鲁棒自平衡控制方法应对快速肢体运动，并设计运动控制策略生成自然四足步态。

Result: 实现了重量和尺寸符合角色规格的机器人，具备稳定平衡和动态肢体运动能力，支持基于重心偏移的直观控制。

Conclusion: 该设计通过轮式与肢体运动的协同，解决了纯肢体运动导致的重量和速度问题，展示了可自然交互的智能机器人概念。
...
### [364] [Real-Time sEMG-Based Telecontrol of an Assistive Robotic Arm Using a 1D Convolutional Neural Network](https://arxiv.org/abs/2607.16310)
*Edgar Manacorda,Mena Samir Kama Abouseffien,Olivier Lecompte,Amandine Gesta,Abolfazl Mohebbi*

Main category: cs.RO

TL;DR: 提出基于实时表面肌电信号的辅助机械臂遥操作接口，采用四通道采集、预处理和1D-CNN分类，在仿真和实际机器人上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 上肢运动障碍影响日常独立性，现有辅助机械臂控制需更直观、可靠且响应快的方式，sEMG提供非侵入式肌肉活动信息。

Method: 系统包括四通道sEMG采集、信号预处理、滑动窗口分割、1D-CNN分类，对比阈值检测、两阶段分类及统一分类器等实时策略。

Result: 测试准确率>90%，平均延迟约0.32秒，在仿真与真实环境中实现稳定、平滑的离散手势控制。

Conclusion: sEMG遥控制助机械臂可行，需集成信号处理、深度学习与控制策略；未来可结合混合感知提高鲁棒性。
...
### [365] [S.E.A.G.R: A Socially and Emotionally Aware Greeting Robot Framework with Dual-Layer Cultural and Affective Modulation](https://arxiv.org/abs/2607.16341)
*Sajjad Hussain,Ranveer Bhura,Amandip Dutta,Shwetangshu Biswas*

Main category: cs.RO

TL;DR: 提出了一种面向多元文化与情感状态的社交情感感知问候机器人框架SEAGR，通过双层调节机制实现文化适配与情感动态响应。


<details>
  <summary>Details</summary>
Motivation: 现有机器人问候系统缺乏对文化差异、情感状态和人际距离的动态适应能力。

Method: 采用双层调节框架：文化身份决定问候类型，情感信号调节执行方式；集成上下文文化映射、情感姿态调节与空间距离控制在Sense-Think-Act架构中。

Result: 完成了低成本原型系统（USB摄像头、超声波传感器、Arduino舵机与Python处理），但尚未进行实证验证。

Conclusion: SEAGR为文化感知人机交互提供了概念验证设计，但用户研究作为未来主要方向当前仍是局限性。
...
### [366] [PRISM: Multimodal Terrain Mapping for Rover Navigation in Unstructured Environments](https://arxiv.org/abs/2607.16366)
*Raul Castilla-Arquillo,Carlos Perez-del-Pulgar,Levin Gerdes,Alfonso Garcia-Cerezo,Miguel A. Olivares-Mendez*

Main category: cs.RO

TL;DR: 本文提出PRISM，一种用于非结构化环境的多模态地形感知系统，通过RGB-D-T融合和OmniUnet网络实现高效语义地形分割与自主导航。


<details>
  <summary>Details</summary>
Motivation: 现有方法在非结构化环境中难以可靠检测陡坡、岩石等危险，需多模态融合提升地形感知鲁棒性。

Method: 搭建定制传感器套件采集RGB、深度、热成像数据；设计基于视觉Transformer的OmniUnet网络进行多模态语义分割；在资源受限嵌入式平台上部署。

Result: 推出BASEPROD和LAENTIEC两个新标注数据集；物理实验验证了系统的实时性和导航可行性。

Conclusion: PRISM可高效生成可通行性地图，直接驱动自主导航系统，验证了多模态感知在非结构化环境中的实用性。
...
### [367] [Back to the museum: Investigation of the acceptance of Android Andrea with and without emotion simulation in a museum](https://arxiv.org/abs/2607.16428)
*Marcel Heisler,Christian Becker-Asano*

Main category: cs.RO

TL;DR: 该研究在公共博物馆部署安卓机器人Andrea，实现自主多语言对话，并测试三种情绪模拟条件（无情绪、ChatGPT 4.1驱动、WASABI架构）对访客主观评价的影响。


<details>
  <summary>Details</summary>
Motivation: 基于前期定性结果，探索在自主对话中集成情绪模拟功能对用户体验的实际效果。

Method: 机器人配备博物馆上下文知识，采用人工合成声音；设计三种情绪实验条件；通过扩展TAM2问卷收集73名访客反馈。

Result: 两种情绪模拟方法（ChatGPT和WASABI）均未对访客评价产生显著积极影响，且未被意识层面检测到。

Conclusion: 当前情绪模拟方法对用户主观体验无提升，需进一步优化情绪集成策略。
...
### [368] [Linear Stability Analysis of an INDI Pitch-Rate Controller under Model Mismatch for a Tilt-Rotor VTOL UAV](https://arxiv.org/abs/2607.16471)
*Lorenzo Schenk,Guillaume Ducard,Christopher Onder*

Main category: cs.RO

TL;DR: 本文针对倾转旋翼垂直起降飞行器的增量非线性动态反演控制器，提出了对模型失配的线性稳定性分析方法，并给出了鲁棒性和性能导向的调优策略。


<details>
  <summary>Details</summary>
Motivation: 现有INDI控制器在倾转旋翼VTOL架构中，快速内环允许的模型失配范围缺乏显式参数化分析。

Method: 通过隔离俯仰率/升降副翼子通道，推导五阶闭环传递函数，利用Routh-Hurwitz准则和参数化线性模型进行稳定性分析。

Result: 作动器延迟和惯性失配在标称增益下影响较小，而控制效能失配（特别是符号错误）是最危险的不稳定因素。
...
### [369] [Certifiable Safe Model-Based Reinforcement Learning with Control-Affine Dynamics Approximation](https://arxiv.org/abs/2607.16501)
*Hao Zhou,Yanze Zhang,Cameron Reid,Wenhao Luo*

Main category: cs.RO

TL;DR: 提出了一种基于控制仿射动力学的安全强化学习框架，结合控制障碍函数和自适应共形预测，实现了数据驱动的安全策略与高效控制器合成。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略残差模型不确定性的结构（如控制仿射性），导致机器人行为过于保守或安全保证无效。

Method: 使用控制仿射随机傅里叶特征（ARFF）建模动力学，结合自适应共形预测（ACP）量化安全约束的不确定性，并通过控制障碍函数（CBF）合成安全策略。

Result: 在倒立摆和3D四旋翼仿真中验证了框架的有效性。

Conclusion: 所提框架通过利用控制仿射结构和不确定性量化，在安全性和任务效率之间实现了更优的平衡。
...
### [370] [Foresight Residual RL for Long-Horizon Robot Manipulation with Vision-Language-Action Models](https://arxiv.org/abs/2607.16506)
*Yuhan Liu,Xinyu Zhang,Litao Liu,Abdeslam Boularias*

Main category: cs.RO

TL;DR: 提出远见残差强化学习，通过离线估计的远见值奖励优化子任务终端状态，将三阶段装配任务成功率从54.5%提升至85.6%。


<details>
  <summary>Details</summary>
Motivation: VLA策略在紧公差装配中因子任务耦合和长期信用分配失败，稀疏奖励虽提升独立子任务成功率但无法改善链式任务表现。

Method: 训练视觉远见预测器评估终端状态对后续子任务的价值，将预测输出作为奖励乘子，通过反向远见归纳训练残差策略。

Result: 在三阶段螺母装配任务中实现85.6%完整任务成功率，显著优于标准子任务残差RL的54.5%和VLA基线。

Conclusion: 提升长期任务性能需要优化子任务产生的成功状态质量，而非仅关注是否实现成功。
...
### [371] [Differentiable Reinforcement Learning for Path Tracking by an Agile Fish-Like Robot](https://arxiv.org/abs/2607.16508)
*Prashanth Chivkula,Kartik Loya,Venkata Ravindhra Reddy Varikuti,Phanindra Tallapragada*

Main category: cs.RO

TL;DR: 该研究开发了一个结合计算效率的仿真平台与PID控制（通过时间反向传播学习可变增益），实现鱼类机器人从仿真到实物的运动控制与路径跟踪。


<details>
  <summary>Details</summary>
Motivation: 鱼类机器人因流固耦合建模困难和非线性欠驱动特性导致控制与运动规划极具挑战，现有强化学习因缺乏合适的仿真环境难以应用。

Method: 构建高效近似运动仿真的平台，采用PID控制（可变增益通过学习得到），并使用时间反向传播与课程训练策略进行学习。

Result: 仿真中学习的策略成功迁移至实物平台，展现了良好的匹配效果。

Conclusion: 所提出的仿真平台与学习控制方法有效克服了鱼类机器人控制难题，且仿真到实物的迁移表现优异。
...
### [372] [Autonomous VR-Based Risk Detection for Situational Awareness in Dangerous Settings](https://arxiv.org/abs/2607.16582)
*Mohammad Eskandari,Murali Krishna Varma Indukuri,Stephanie M. Lukin,Cynthia Matuszek*

Main category: cs.RO

TL;DR: 提出一个VR-HRI框架，利用VLM辅助机器人识别危险并通过VR界面传递给操作员，实验表明该带注释界面相比基线更受用户青睐，清晰度、有用性和舒适度评分均较高。


<details>
  <summary>Details</summary>
Motivation: 灾害响应等高风险环境中，情境意识既需检测危险也需清晰传达给人类操作员，而VLM在面向人类的机器人系统中的应用尚未充分探索。

Method: 搭建VR人机交互框架，机器人探索虚拟场景并调用VLM识别危险和标注兴趣点，通过沉浸式VR界面呈现给操作员，并开展用户偏好对比实验。

Result: 带标注VR界面的用户偏好优于无标注基线；参与者对系统清晰度、有用性和舒适度给予高评分。

Conclusion: 结合VLM机器人感知与沉浸式可视化是支持高风险环境情境意识的有效途径。
...
### [373] [An Indoor Navigation System for the Visually Impaired based on UWB Positioning and D* Lite Path Planning Algorithm](https://arxiv.org/abs/2607.16614)
*Thanh C. Vo,Dong LT. Tran,Huy HM. Le,Duyen N Ha,Tuan Anh Pham,Hai Thanh Dang,Hoang T. Tran*

Main category: cs.RO

TL;DR: 提出了一种结合UWB定位与D* Lite算法的室内导航系统，为视障人士在GPS受限环境中提供可靠、低延迟的路径规划。


<details>
  <summary>Details</summary>
Motivation: 解决视障人士在GPS信号缺失的室内环境中导航困难，提升其行动安全与自主性。

Method: 利用UWB传感器实现高精度室内定位，并集成D* Lite算法进行动态路径规划与快速重规划。

Result: 系统在复杂室内空间运行可靠，延迟低，能有效应对动态障碍。

Conclusion: 该系统可安全灵活地辅助视障人士室内导航。
...
### [374] [SAGE: A Socially-Aware Generative Engine for Heterogeneous Multi-Agent Navigation](https://arxiv.org/abs/2607.16619)
*Lan Hu,Minghui Liwang,Wenbo Zhu,Xinlei Yi,Yiguang Hong,Xianbin Wang,Zhenzhen Jiao,Seyyedali Hosseinalipour*

Main category: cs.RO

TL;DR: 提出了SAGE框架，通过异构图表征和扩散模型实现人机环境中的安全和社会合规导航。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以建模异构主体的非对称交互、耦合预测规划以及软性社会规范。

Method: 使用异构图表征与HGT编码类型特定的交互，结合扩散模型联合预测轨迹和规划，并在推理时通过免训练的能量引导机制优化安全性。

Result: 在真实和合成数据集上降低了碰撞率和社会违规率，可扩展到20台机器人，并支持安全-精度-任务权衡的显式控制。

Conclusion: SAGE是复杂环境中社会感知多机器人导航的可扩展框架。
...
### [375] [AI-Augmented Model Predictive Control for Safe and Adaptive Rendezvous and Proximity Operations](https://arxiv.org/abs/2607.16630)
*Luca Sportelli,Tyler Barr,Cagri Kilic,Di Wu*

Main category: cs.RO

TL;DR: 提出一种自适应MPC框架，用于航天器自主交会与近距离操作，在对抗性多智能体场景中实现安全、实时且鲁棒的制导。


<details>
  <summary>Details</summary>
Motivation: 学习型方法在安全关键空间机器人应用中受限于可解释性、鲁棒性和约束意识，现有方法无法平衡目标追踪、安全保持和实时适应性。

Method: 结合约束滚动时域MPC与数据驱动监督调优层，基于CW动力学建模，融入执行器限制、禁飞区约束、松弛变量处理和可选CBF安全滤波，通过离线闭环评估和在线交互几何调整MPC参数。

Result: 在KSPDG捕卫星环境中，相比固定参数MPC，自适应MPC在闭环鲁棒性、自适应机动性和交会性能上更优，同时保持安全意识和实时可行性。

Conclusion: 该框架提供了模块化、可解释的自适应航天器RPO基础，能有效应对动态对抗环境。
...
### [376] [PhyAgentOS: A Self-Evolving Operating System for Embodied Agents with Decoupled Cognitive Planning and Physical Execution](https://arxiv.org/abs/2607.16636)
*Yang Liu,Weixing Chen,Xinshuai Song,Tao Pu,Siwen Mo,Yongjie Bai,Zihao Chen,Qianran Sun,Liruo Zhong,Ying Shen,Liang Lin*

Main category: cs.RO

TL;DR: 提出了PhyAgentOS运行时系统，通过会话中心调度、状态即文件协议、语义验证和经验记忆，统一了视觉-语言-动作模型、世界模型和智能体规划器的执行抽象与状态管理。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型、世界模型和智能体规划器缺乏统一的执行抽象、共享状态、语义验证和跨实体经验持久化，导致组合困难。

Method: 基于会话中心运行时，将会话而非动作作为最小调度单元；通过State-as-a-File协议将跨层状态序列化为Markdown+YAML文件，形成可检查、可版本化的认知-物理边界；引入SessionVerifier基于证据的语义验证；通过认知记忆合并验证后的结果。

Result: 在Optimus-67、StarDojo和DST-Dojo基准上测试，在19+模拟和物理实体上验证，在LIBERO、Calvin和RoboCasa365上跨多个VLA模型获得效果提升。

Conclusion: PhyAgentOS通过系统级服务实现了多种智能体范式的高效组合，提供了可验证、可复现且安全的物理智能体执行基础。
...
### [377] [Token-Wise Latent Streaming from Slow Reasoners to Fast Planners for Dynamic Vision Language Navigation](https://arxiv.org/abs/2607.16806)
*Tianshuai Hu,Yangyi Zhong,Zeying Gong,Lingdong Kong,Xiaodong Mei,Guoyang Zhao,Xiaolu Liu,Song Wang,Rong Li,Junwei Liang*

Main category: cs.RO

TL;DR: 提出SPARK-VLN双系统框架，通过流式传输VLM中间隐藏状态给专家规划器，解决动态社交导航中推理延迟导致的观测过时问题，提升导航成功率和社会合规性。


<details>
  <summary>Details</summary>
Motivation: 语言推理缓慢，而社交安全导航需即时响应，导致决策过时存在安全隐患。

Method: 利用Token-Wise Hidden Streamer、Sequence-to-Slot Latent Bridge和Evolving Latent Conditioner三个模块，将VLM的中间状态实时注入流匹配专家规划器。

Result: 在动态社交VLN基准测试中，SPARK-VLN提高了导航成功和社会合规性，同时保持推理效率。

Conclusion: 中间隐藏状态可提前提供动作相关意图，双系统框架能有效缓解观测过时问题。
...
### [378] [GLidE-SLAM: GL-Accelerated Indirect-Direct Embedded SLAM](https://arxiv.org/abs/2607.16897)
*Carlos A. Pinheiro de Sousa,Heiko Hamann,Oliver Deussen*

Main category: cs.RO

TL;DR: 提出了GLidE-SLAM，一种混合间接-直接的单目视觉SLAM框架，通过GPU加速直接跟踪和架构分离，在嵌入式设备上实现了高帧率同时保持轨迹精度。


<details>
  <summary>Details</summary>
Motivation: 嵌入式设备上运行Visual SLAM时，跟踪帧率需高而计算资源有限，现有方案难以兼顾速度与地图维护。

Method: 采用混合间接-直接架构，GPU加速直接跟踪中间帧（基于OpenGL ES 3.1计算着色器），CPU负责间接地图维护，不使用深度优化或地图点创建。

Result: 在目标平台上帧率比纯CPU基线提升高达9倍，同时保持轨迹精度，并提升了在资源受限硬件上的实用性。

Conclusion: GLidE-SLAM首次通过计算着色器实现完整的直接光度姿态估计，适用于多种嵌入式平台，平衡了性能与资源消耗。
...
### [379] [Approximate Relative Entropy Constraints for Nonlinear Covariance Steering Under Distribution Ambiguity](https://arxiv.org/abs/2607.16914)
*Trevor N. Wolf,Jay W. McMahon*

Main category: cs.RO

TL;DR: 本文提出了一种基于相对熵（KL散度）的分布鲁棒协方差控制框架，用于处理非线性系统中高斯代理与真实分布之间的歧义，从而准确估计风险敏感量（如碰撞概率和均方误差）。通过变分表示推导了风险敏感量的可计算上界，并利用时间KL散度变化率约束设计随机引导策略。方法在非线性航天器近直线晕轨道转移中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 协方差控制线性随机反馈策略高效，但推广到非线性系统时需局部线性化生成高斯代理，导致风险敏感量（如碰撞概率）估计不准确。

Method: 基于KL散度构建分布鲁棒协方差控制框架，利用指数积分变分表示推导歧义集上风险敏感量的上界，建立真实分布与高斯代理之间KL散度变化率的上界约束，通过序列凸规划算法优化决策变量。

Result: 在非线性航天器轨道转移任务中，成功实现了真实分布与高斯代理的接近性控制，同时满足风险敏感性能约束。

Conclusion: 所提分布鲁棒方法能有效处理非线性系统中分布歧义，提升风险估计的准确性，适用于实际高风险任务（如航天器交会）。
...
### [380] [A BIM-enabled, Agent-based Discrete-event Simulation Platform for Robotic Studies: A Method based on Graph Theory](https://arxiv.org/abs/2607.16920)
*Ping Xu,Xinghua Gao*

Main category: cs.RO

TL;DR: 提出了一种基于BIM的智能室内机器人导航与操作规划平台，通过网格离散化和图论方法实现高效、无碰撞的路径规划，并解决了粗离散化带来的重叠问题。


<details>
  <summary>Details</summary>
Motivation: 现有导航方法缺乏对建筑信息的理解，无法满足复杂设施管理任务（如定位和修复管道）的需求，而BIM信息未被充分利用。

Method: 将室内环境离散化为网格单元，映射为图节点（目标、障碍、常规节点），为边分配遍历代价，利用图论算法规划路径，并通过网格细化解决目标与障碍重叠问题。

Result: 仿真显示该图表示方法实现了高效、无碰撞的导航；网格细化提高了空间精度和路径可行性。

Conclusion: 该平台支持部署前的虚拟评估，为基于BIM的设施管理机器人系统奠定了基础。
...
### [381] [PREFAIL: Identifying Precursors to Failures in Robotic Lift-and-Place Tasks to Improve Task Execution Performance](https://arxiv.org/abs/2607.16921)
*Zeyu Shangguan,Rajas Chitale,Rutvik Patel,Satyandra K. Gupta,Daniel Seita*

Main category: cs.RO

TL;DR: 本文提出PREFAIL方法，通过分析目标物体与载体的相对运动来预测非抓取物料搬运操作中的失败前兆，并引入精确定义最新干预时间的数据集。


<details>
  <summary>Details</summary>
Motivation: 现有非抓取操作中的失败预测方法对动态动作敏感且高度依赖已知策略结构，同时缺乏对最新干预时间的精确刻画，导致无法判断预测的失败是否可及时阻止。

Method: 通过分析目标物体与载体之间的相对运动来预测失败前兆，并构建包含精确最新干预时间的数据集。

Result: 在仿真和真实数据集上，PREFAIL显著提升了失败前兆预测的准确性和响应及时性。

Conclusion: PREFAIL能有效提高非抓取物料搬运操作的效率和可靠性，为及时干预提供可行方案。
...
### [382] [SinD 2.0: A Multi-City UAV Dataset with Semantic Risk Annotations for SOTIF-Oriented Safety Validation at Signalized Intersections](https://arxiv.org/abs/2607.16943)
...
TL;DR: 提出大规模无人机交叉路口数据集SinD 2.0,支持跨域自动驾驶安全验证,包含跨城市多样性、高密度风险交互与分层语义标注。


<details>
  <summary>Details</summary>
Motivation: 现有数据集存在地理同质、安全事件稀疏且缺乏语义风险标注,难以评估算法泛化性与SOTIF验证。

Method: 构建无人机采集的六路口四城市数据集,提取32682个安全临界事件,提供HD地图/SPaT数据与多维语义标签,开发全栈测试工具链。

Result: 跨城市域迁移显著,语义风险子集有效暴露算法性能局限。

Conclusion: SinD 2.0填补了信号交叉口安全验证数据空白,开源工具链可促进ADS系统鲁棒性测试。
...
### [383] [G2-Nav: Grounded and Guarded Vision-Language Costmaps for Robot Social Navigation](https://arxiv.org/abs/2607.16956)
*Yuwen Liao,Yihang Lan,Yizhuo Yang,Ruimeng Liu,Xinhang Xu,Shenghai Yuan,Lihua Xie*

Main category: cs.RO

TL;DR: 提出 G2-Nav 框架，将大视觉语言模型的抽象社会推理转化为可靠且可解释的代价地图，结合高频安全检查，实现安全、高效、符合社会规范的自主导航。


<details>
  <summary>Details</summary>
Motivation: 现有端到端方法形成不可预测的黑箱，指令跟随方法缺乏完全自主性，需弥合社会推理与安全部署之间的差距。

Method: 利用 VLM 进行开放集感知，评估可通行区域和社会智能体，将社会上下文映射为视觉语言代价地图；对上流跟踪进行语义验证，并引入高频安全检查防止系统延迟。

Result: 真实环境实验表明 G2-Nav 能在非结构化环境中实现安全、高效且符合社会规范的自主导航。

Conclusion: 通过可解释的代价映射和双重安全检查，有效弥合了 VLM 社会推理与现实部署之间的鸿沟。
...
### [384] [User-Driven Learning from Demonstration: A Trajectory and Impedance Learning Method](https://arxiv.org/abs/2607.16998)
*Zi-Qi Yang,Mehrdad R. Kermani*

Main category: cs.RO

TL;DR: 本文提出了一种用户驱动的机器人学习从示范（LfD）方法，通过单次示范实现实时学习，同时确保精准再现和柔顺性，减少用户重复教学负担，并通过阻抗参数化函数应对交互中的安全问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于时间索引的轨迹缺乏柔顺性，可能导致人机交互中的安全隐患，且传统LfD方法需要多次示范，用户负担大。

Method: 集成三维快速微分同胚匹配（FDM）算法与基于动力系统（DS）的运动生成器，结合扩展卡尔曼滤波（EKF）框架补偿误差，并引入阻抗参数化函数学习阻抗变化。

Result: 在7自由度KUKA LWR IV+机器人上实验验证，单次示范后能高精度再现运动，实时学习阻抗变化，保持表面接触，并有效恢复交互扰动。

Conclusion: 该方法能减少用户努力，提升LfD的柔顺性和鲁棒性，适用于需要安全交互的机器人任务。
...
### [385] [SHAPE: Simultaneous Water Hydraulic Actuation and Position Estimation of a Sensorless Remote Actuator through a Thin and Long Flexible Tube](https://arxiv.org/abs/2607.17054)
*Yuki Nakamura,Shuto Yoshimura,Tomoyuki Noda,Yoshihiro Nakata*

Main category: cs.RO

TL;DR: 提出一种利用水填充柔性管道同时传输动力和位置信息的方法，无需执行器侧传感器，实现50米内远程位置控制。


<details>
  <summary>Details</summary>
Motivation: 恶劣环境下机器人传感器易失效，而细长管道可实现无传感器远程操作，但低阻抗和有限可见性导致精确定位困难。

Method: 基于压力波动建模传输中的容积损失，并考虑少量空气截留，通过长达50米的管道同时实现动力传输和位置估计。

Result: 实验验证了在变负载下无传感器液压缸的稳定位置控制，并开发了无需执行器侧传感器的现场参数辨识方法。

Conclusion: 该方法促进了恶劣环境中机器人的可靠远程控制。
...
### [386] [ADMM-Based Safety-Critical Distributed NMPC for Cooperative Transportation by Quadrupedal Robots](https://arxiv.org/abs/2607.17007)
*Ruturaj S. Sambhus,Kapi Ketan Mehta,Yicheng Zeng,Kaveh Akbari Hamed*

Main category: cs.RO

TL;DR: 提出一种面向四足机器人团队协作搬运的安全关键分布式非线性模型预测控制框架。将机器人团队和共享负载建模为具有刚性完整约束的动态耦合网络系统，通过ADMM分解集中优化问题，结合高阶控制障碍函数实现避障，实验验证了其在通信延迟、负载不确定等条件下的鲁棒性和实时性，相比集中式NMPC平均求解时间降低23%。


<details>
  <summary>Details</summary>
Motivation: 现有分布式控制方法难以同时处理动态耦合的完整约束、安全避障和实时性需求，亟需一种可扩展、鲁棒的分布式框架。

Method: 将集中式NMPC分解为通过ADMM协调的并行局部子问题，在预测控制中显式引入加速度级完整耦合约束，并采用HOCBFs确保机器人及负载的避障安全。

Result: 2-4台四足机器人协作搬运的数值仿真和2-3台机器人实物实验均验证了安全性、鲁棒性；与集中式NMPC相比，平均NLP求解时间降低23%，闭环性能相当；消融实验表明有效载荷状态共识和完整约束显著优于仅施加力共识的现有方法。

Conclusion: 该分布式NMPC框架可有效解决多机器人协作搬运中的耦合约束、安全避障和实时优化问题，具有实际部署潜力。
...
TL;DR: 提出了 Metric-DROID，一种端到端循环架构，通过集成本体感测解决单目 SLAM 的尺度模糊问题，实现精确度量深度估计。


<details>
  <summary>Details</summary>
Motivation: 单目系统固有尺度模糊和漂移问题，现有基于光流的SLAM虽然鲁棒但仍是尺度模糊的。

Method: 集成LSTM更新算子编码高频里程计为空间特征图，不确定性感知度量后端平衡视觉重投影与度量平移残差，选择性残差微调保留预训练先验。

Result: 系统能智能平衡视觉和度量残差，减少轮滑和传感器噪声影响，支持零样本度量对齐。

Conclusion: 该架构将视觉SLAM锚定到物理现实，提升了尺度精度和鲁棒性。
...
### [388] [BoxTwin: Learning Elastoplastic Articulated Object Dynamics from Videos](https://arxiv.org/abs/2607.17132)
*Heng Zhang,Gehan Zheng,Kaifeng Zhang,Jay Song,Shivansh Patel,Sonny Hu,Yunzhu Li,Changxi Zheng,Peter Yichen Chen*

Main category: cs.RO

TL;DR: 提出BoxTwin框架，从视频学习弹塑性铰接物体的完整动力学，实现长期轨迹跟踪与塑性行为预测。


<details>
  <summary>Details</summary>
Motivation: 现有数字孪生难以处理具有非线性弹性、塑性屈服和损伤累积的弹塑性铰接物体（EAOs）。

Method: 通过视频重建场景，为每个EAO识别基于物理的本构模型。

Result: 在EAO的手动折叠和双臂操作实验中，BoxTwin能准确跟踪关节轨迹，并长时间再现接触后塑性行为。

Conclusion: BoxTwin将视频驱动重建与弹塑性损伤模型结合，推动数字孪生向非结构环境中可变形铰接物体的预测性和自适应控制发展。
...
### [389] [Move First, Commit Later: Selective LiDAR-to-BIM Global Initialization via Sequential Consensus with Symmetry-Aware Abstention](https://arxiv.org/abs/2607.17103)
*Yujie Zhang,Yuxuan Guo,Jiashuo Zheng,Zeheng Fan,Xiaohui Jia,Jinyue Liu*

Main category: cs.RO

TL;DR: 提出一种选择性决策层，通过多子图投票和对称感知避免LiDAR-to-BIM初始化中的别名错误，实现可逆与类型化决策。


<details>
  <summary>Details</summary>
Motivation: 在重复室内环境中，传统注册方法常因对称性产生高置信度错误匹配（别名），而非低置信度注册。

Method: 基于多子图候选映射到SE(2)锚点，通过top-1共识聚合证据，拓扑仅作可行性门控，决策类型包括COMMIT/DEFER/AMBIGUOUS(τ)并支持对称破坏升级。

Result: 在真实建筑与对称仿真中，该方法在所有测试轨迹配置下正确执行决策，而强制选择策略多数误判房间，提交锚点误差0.02-0.36米和1.9°。

Conclusion: 该层可模块化集成于前端注册器，通过类型化决策和可逆性有效解决室内对称别名问题。
...
### [390] [From Perception to Assistance: Open-Vocabulary Shared Autonomy for Robotic Manipulation](https://arxiv.org/abs/2607.17323)
*Murilo Vinicius da Silva,Ricardo V. Godoy,Juliano Negri,Gustavo J. G. Lahr,Ranulfo Bezerra,Marcelo Becker*

Main category: cs.RO

TL;DR: 提出了一种共享自主框架，通过RGB-D相机、视觉语言模型和GPU加速模型预测控制器，辅助操作员在工业环境中精确遥操作机器人机械臂，避免碰撞并完成目标抓取。


<details>
  <summary>Details</summary>
Motivation: 摄像头界面在工业环境中难以提供足够精度，操作员需要在有限深度感知和杂乱环境中对齐末端执行器并避免碰撞。

Method: 使用RGB-D相机捕获操作员动作，通过视觉语言模型和提示视频分割模型定位目标，GPU加速模型预测控制器执行运动并避免碰撞，势场在最终接近时校正操作员参考。

Result: 位置均方根误差为59毫米，控制器保持机械臂距障碍物至少18厘米；在阀门操作和拾取任务中，完整框架全部成功，消融实验失败，自主模式在每项任务中五次尝试成功四次。

Conclusion: 该框架有效提升了遥操作精度和安全性，但自主模式仍有改进空间。
...
### [391] [VIDAR: Visual-Inertial Dense Alignment and Reconstruction via a Geometric Foundation Model](https://arxiv.org/abs/2607.17171)
*Diyari Mohammed Salih,Lingxiang Hu,Naima AitOufroukh-Mammar,Fabien Bonardi*

Main category: cs.RO

TL;DR: VIDAR是一个视觉-惯性密集重建框架，通过结合SVO+IMU里程计和Depth Anything 3基础模型，实现尺度稳定的密集单目重建。


<details>
  <summary>Details</summary>
Motivation: 单目基础模型提供稠密几何但缺乏稳定度量尺度，需要引入视觉-惯性系统作为度量锚点。

Method: 使用视觉-惯性前端提供相机位姿、尺度和一致的世界坐标系，将基础模型的局部几何预测融合到全局重建中；研究了位姿条件化和解耦对齐两种策略。

Result: 在EuRoC上，位姿注入将尺度误差降低至约1%，平均F@0.10达到0.463；解耦混合方法在无真实姿态下提升至0.676。

Conclusion: VIDAR是实现度量密集单目重建的实用方法，在EuRoC和TUM RGB-D数据集上验证了有效性。
...
### [392] [Stability and Comfort in Mobile Robot-Pedestrian Interactions](https://arxiv.org/abs/2607.17604)
*Alireza Jafari,Hong-Son Nguyen,Yen-Chen Liu*

Main category: cs.RO

TL;DR: 该论文针对非完整移动机器人（NMR）在公共空间中的社交导航问题，提出了基于社会力模型（SFM）和投影碰撞时间社会力模型（TSFM）的框架，通过实验验证了系统稳定性与行人舒适度的提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究对行人主观安全的实证分析不足，经典导航算法常将行人视为动态障碍物，忽略主观因素，且多针对完整移动机器人，而实际应用需要非完整移动机器人。

Method: 结合SFM和TSFM建模NMR与行人及障碍物的交互，通过混合成本函数（舒适度与速度）校准模型，并采用统计工具分析实验中的问卷结果。

Result: 所提方法在稳定性、行人舒适度方面优于遥控基线方法，且与先前研究相比在指标上具有优势。

Conclusion: 提出的模型是稳定的，能够改善NMR穿越行人群体时行人的舒适度。
...
### [393] [Retriever: Composing Closed-Loop Asynchronous Robot Programs](https://arxiv.org/abs/2607.17213)
*Linfeng Zhao,Haojie Huang,Jiayuan Mao,Weiyu Liu,Mykel Kochenderfer,Lawson L. S. Wong*

Main category: cs.RO

TL;DR: 提出Retriever框架，通过异步决策模型、编程模型和运行时系统，统一管理机器人长周期任务中异构组件的时序与并发问题，支持确定性重放与多后端调试。


<details>
  <summary>Details</summary>
Motivation: 现有机器人闭环系统因组件时钟不同步、延迟可变及隐式时序语义，导致行为难以复现、调试和复用，缺乏端到端的系统层与算法层统一解决方案。

Method: 将智能体建模为在显式运行时钟上执行的状态因果流函数图，形式化为连续时间流上的异步环境-智能体循环，并编译为支持多后端调试和确定性重放的运行时。

Result: 通过真实机器人案例及运行时开销与确定性重放控制实验验证了框架的有效性。

Conclusion: Retriever提供了从模型到运行时的全栈方案，能够系统性地解决时序不确定性带来的调试难题，并支持面向长期任务的机器人智能体构建。
...
### [394] [Asynchronous Multimodal Diffusion Policy Composition via Latency-Aware Guidance Fusion](https://arxiv.org/abs/2607.17257)
*Zihao He,Hongjie Fang,Shirun Tang,Cewu Lu,Haoshu Fang*

Main category: cs.RO

TL;DR: 提出LAG-Fusion框架，通过异步多模态扩散策略组合，实现不同传感速率和延迟下的机器人操纵性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有多模态扩散策略依赖同步融合或手动设计的多频率架构，限制了高反馈频率和可扩展性。

Method: LAG-Fusion框架支持各模态策略按原生推理率运行，通过参考帧中相对动作表示的rebasing规则对齐延迟引导。

Result: 在接触丰富操纵任务中，LAG-Fusion在异构模态延迟下优于同步融合和人工设计的力感知基线。

Conclusion: LAG-Fusion通过异步组合提升了策略响应性和任务性能。
...
### [395] [A2RL V\textsubscript{max}: The A2RL autonomous racing dataset for long-range, high-speed perception and multi-vehicle interaction](https://arxiv.org/abs/2607.17813)
...
TL;DR: 本文介绍了A2RL V_max开源数据集，专为高速自动驾驶和多车交互感知任务设计，包含近30,000个专业标注的LiDAR点云，并在2024年阿布扎比自动驾驶赛车联盟赛事中采集。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶感知数据集主要针对结构化城市环境，缺乏高速驾驶和多车交互场景的数据支持。

Method: 在亚斯码头F1赛道采集单/多车及四车决赛数据，提供专业标注的LiDAR和RADAR点云，并以开发者友好格式发布。

Result: 基准方法在3D检测和追踪上表现良好，但高速场景仍需专门算法。

Conclusion: 该数据集填补了高速自动驾驶感知研究的空白，为多车交互场景的深度学习研究提供了新基准。
...
### [396] [Multi-Resolution Voxelized Map-Based Stereo Visual-Inertial Odometry](https://arxiv.org/abs/2607.17332)
*Shuyi Pan,Hangtian Wang,Zhaoxing Zhang,Chengliang Zhang,Zikang Yuan,Xin Yang*

Main category: cs.RO

TL;DR: 本文提出一种多分辨率先验地图构建方法及基于地图的VIO系统，通过锥形索引策略和3D DDA算法实现低数据传输量的精确位姿估计。


<details>
  <summary>Details</summary>
Motivation: 先验地图能提升VIO精度，但地图数据量大、传输带宽有限，难以在边缘设备上实时加载完整局部地图。

Method: 多分辨率先验地图构建及锥形索引策略，利用3D数字微分分析算法根据距离选择合适的分辨率层级并检索对应体素内的唯一点。

Result: 在两个公共数据集上的实验表明，系统在实现精确位姿估计的同时，所需数据传输量最小。

Conclusion: 该方法有效平衡了地图数据量与位姿估计精度，适用于带宽受限的边缘设备场景。
...
### [397] [Test-Time Scaling for World Action Models via Zero-Shot Geometric Evaluation](https://arxiv.org/abs/2607.17454)
*Zesen Zhao,Minkyoung Cho,Hui shen,Boyuan Zheng,Kunxiao Gao,Yulong Cao,Z. Morley Mao*

Main category: cs.RO

TL;DR: 提出了一种无需训练的、选择性测试时扩展框架，用于世界动作模型，通过交叉视图深度重投影一致性和动作-未来一致性门控来提升机器人控制性能。


<details>
  <summary>Details</summary>
Motivation: 机器人控制需要在执行动作前判断额外计算是否有用，现有方法缺乏高效的测试时扩展决策机制。

Method: 包括固定预算的最佳N选择器（基于交叉视图深度重投影一致性排序）和轻量级动作-未来一致性门控，仅在初始rollout内部不一致时调用选择器。

Result: 在五个基准设置中，固定预算方法始终提升成功率；门控版本平均恢复74.8%的增益，仅在26.2%的决策点触发额外采样。

Conclusion: 交叉视图重投影是无任务标签下的强选择器，但假低分选择是性能饱和或下降的失败模式。
...
### [398] [Disturbance-Aware Flight for Aerial Robots in Narrow Space](https://arxiv.org/abs/2607.17476)
*Lei Qiang,Tianyu He,Chenyang Sun,Xurui Liu,Miao Wang,Xiaobin Zhou*

Main category: cs.RO

TL;DR: 提出了一种扰动感知规划与控制框架（DAPCF），通过在线扰动估计和自适应速度调节，实现四旋翼飞行器在0.6米窄通道中的自主飞行，效率超越人类飞手。


<details>
  <summary>Details</summary>
Motivation: 现有方法在控制层处理气动扰动，而规划层依赖几何约束和固定速度限制，导致窄空间飞行保守或不安全。

Method: 双环观测器实时估计六自由度扰动力矩；扰动风险函数自适应调节参考速度；基于电机动力学的非线性模型预测控制器（MDNMPC）进行扰动补偿。

Result: 对角线0.39米四旋翼可穿越0.6米宽直道、斜道和弯曲隧道，成功率和飞行效率均优于人类飞手。

Conclusion: 该框架有效整合扰动估计与规划控制，提升窄空间飞行鲁棒性和效率。
...
### [399] [GeoWorldAD: Geometry World Action Model for Autonomous Driving](https://arxiv.org/abs/2607.17521)
...
TL;DR: 提出GeoWorldAD，将轨迹规划建立在自车对齐的3D几何空间与潜在未来几何令牌上，通过迭代轨迹细化实现安全高效的自动驾驶。


<details>
  <summary>Details</summary>
Motivation: 现有视觉/视频动作模型缺乏明确的几何约束和未来空间引导，难以平衡避碰与驾驶进度。

Method: 在自车对齐3D空间中融合当前几何约束与潜在未来几何令牌，通过渐进式多尺度几何聚合进行迭代轨迹细化。

Result: 在NAVSIM v1和v2上达到最先进性能，验证了显式3D几何和未来几何建模的有效性。

Conclusion: 明确几何基础与未来几何世界建模是实现安全高效自动驾驶的关键。
...
### [400] [Predicting Grasping Compliance in Robotic Hands through Analytical-Model-Informed Neural Networks](https://arxiv.org/abs/2607.17541)
*Qianwen Zhao,Long Wang*

Main category: cs.RO

TL;DR: 提出了分析模型通知神经网络（AMINN），结合分析力学和数据驱动学习，预测欠驱动机器人手中工具在强力交互下的行为。


<details>
  <summary>Details</summary>
Motivation: 传统抓取视为二元成功/失败，忽略抓取顺应性在强力工具使用中的关键影响，尤其欠驱动手更难建模。

Method: 混合预测模型（AMINN），包含分析力学层和数据驱动学习，估计抓取稳定性和工具位移。

Result: 在三指欠驱动手上评估，展现出强预测能力，机械意义输出，能量一致性优于多层感知器基线。

Conclusion: AMINN提高了物理可解释性，支持更可靠、安全、可信的自主工具使用，尤其在安全关键场景。
...
### [401] [Finite-Time Curvature-Constrained Vector Field for Saturation-Free Motion Planning of Nonholonomic Robots](https://arxiv.org/abs/2607.17542)
*Zhouru Xiao,Sha Luo,Yang Lu,Héctor García de Marina,Zhenyang Xu,Chaosong Gong,Yaonan Wang,Weijia Yao*

Main category: cs.RO

TL;DR: 提出了一个有限时间曲率约束向量场（FT-C2VF）和免饱和控制器的通用框架，能在满足曲率约束和驱动器限制下实现非完整移动机器人有限时间到达目标位姿。


<details>
  <summary>Details</summary>
Motivation: 现有向量场方法无法显式生成满足曲率约束的轨迹，常通过输入饱和强加驱动器限制，破坏稳定性保证且收敛时间无界。

Method: 构建FT-C2VF（互补增益实现有限时间收敛且保证积分曲线曲率连续有界且单调递减），设计几乎全局C1光滑免饱和控制器（无需雅可比信息，自动满足驱动器限制），并进行动力学系统分析证明几乎全局有限时间稳定性。

Result: 数值仿真优于代表性VF方法，户外Ackermann转向车辆实验验证了有效性和鲁棒性。

Conclusion: 所提框架在满足曲率约束和驱动器限制下实现有限时间收敛，具有实际应用价值。
...
### [402] [COLIP-2: Olfaction-Vision-Language Embeddings](https://arxiv.org/abs/2607.17559)
*Kordel Kade France*

Main category: cs.RO

TL;DR: COLIP-2是一个多模态嵌入空间，将嗅觉、视觉和语言统一起来，使机器人能通过气味定位物体，并在边缘设备上实现实时应用。


<details>
  <summary>Details</summary>
Motivation: 缺乏大规模配对图像-气味数据集，现有开放数据限制嗅觉感知能力，需推动新方法和数据集开发。

Method: 整合分子结构、气体传感器读数、气味描述语言和图像到一个共享表示空间，通过概率方式进行气味源定位。

Result: 在内部测试中验证了架构性能，并通过优化使模型可在边缘设备上实时运行。

Conclusion: COLIP-2展示了开放嗅觉数据的局限性，强调了新数据集和方法对先进嗅觉感知的必要性，跨学科设计使其适用于多模态嗅觉领域。
...
### [403] [Predictive Training with Latent Imagination for Visual Quadruped Navigation](https://arxiv.org/abs/2607.17574)
*Yancheng Zhu,Wanli Ma,Chen Han,Irvin Haozhe Zhan,Bingfeng Qin,Yixin Xu*

Main category: cs.RO

TL;DR: 提出一种轻量级预测监督方法，在训练时辅助循环状态编码障碍物动态，推理时无额外计算，显著提升腿式机器人在动态环境中的导航成功率并降低碰撞率。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习导航策略反应式响应，缺乏预测动态障碍物演变的能力，在动态环境中响应滞后。

Method: 在LSTM-SRU导航骨干中引入JEPA风格预测器和SIGReg正则化，训练时用预测器监督隐藏状态以预测下一状态，推理时丢弃预测器。

Result: 在仿真和真实动态障碍物导航基准中，导航成功率提升，碰撞率降低；在Unitree Go2机器人上实现零样本仿真到现实迁移。

Conclusion: 仅通过预测性训练信号即可提升动态导航性能，无需增加推理参数，并支持有效的仿真到现实迁移。
...
### [404] [Configuration-Induced Passive Self-Rotation for Perception-Enhanced Autonomous Flight](https://arxiv.org/abs/2607.17646)
*Xurui Liu,Miao Wang,Tianyu He,Linrui Yang,Xiaobin Zhou*

Main category: cs.RO

TL;DR: 论文提出了一种配置诱导的被动自旋三旋翼飞行器，通过调整后臂参数平衡扫描视野刷新率与飞行性能，并构建了集规划与控制的分层自主框架，实现了在狭窄杂乱环境中的感知增强自主飞行。


<details>
  <summary>Details</summary>
Motivation: 自主飞行在狭窄杂乱环境中受限于传感器视野，被动自旋可扩大感知范围，但会带来扫描视野刷新率与飞行性能之间的权衡。

Method: 利用后臂配置参数调节被动自旋工作点，实现机身层面的权衡；开发分层自主框架（规划与控制），并通过引导点重规划提升任务覆盖。

Result: 实际实验验证了高速轨迹跟踪、抗干扰测试和杂乱环境自主导航的有效性。

Conclusion: 所提方法在感知增强自主飞行中表现有效，通过被动自旋机制平衡了视野刷新率与飞行性能。
...
### [405] [HCPG-Flow:Hierarchical Contact-Progress Guidance for Flow-Policy Robot Manipulation](https://arxiv.org/abs/2607.17651)
*Guanghu Xie,Mingxu Li,Shuo Zhang,Yonglong Zhang,Yifan Yang,Yang Liu,Zongwu Xie,Baoshi Cao*

Main category: cs.RO

TL;DR: HCPG-Flow是一种改进的机器人操作动作选择方法，通过层次化、以物体为中心的接触-进度指导，提升了策略性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于流的策略在评估多个动作候选时，依赖值函数估计，而某些动作在经验回放中可能代表性不足，影响数据收集。

Method: 在SAC-Flow基础上引入HCPG选择器，在接触前优化末端执行器接近，接触后优化任务进度，通过标准化评分和温度控制嵌入动作。

Result: 在10个仿真任务中平均成功率提升，Maniskill基准提升9.5个百分点；4个物理任务中成功率提高，完成步骤减少17.4%。

Conclusion: HCPG-Flow通过层次化接触-进度指导，有效改善了流策略的动作选择，提升了操作效率。
...
### [406] [SLAM in Low-Light Environments: Project Report](https://arxiv.org/abs/2607.17699)
*Oleh Basystyi,Anna Stasyshyn,Oleksandr Kosovan,Yaroslav Prytula*

Main category: cs.RO

TL;DR: 在低光照环境下，仅依赖标准RGB相机的SLAM系统性能严重下降，只有结合惯性融合与全局优化的系统（如Kimera-VIO）才能稳定追踪，而传统方法则容易失败。


<details>
  <summary>Details</summary>
Motivation: 低光照条件（对比度降低、传感器噪声、运动模糊）会损害基于RGB特征的SLAM系统，现有基准测试缺乏对黑暗场景的评估，因此有必要探究标准RGB SLAM在黑暗中的极限。

Method: 将六种SLAM系统（特征法、直接法、滤波法和学习法）在五个不同难度的LaMARia序列上进行基准测试，使用绝对/相对位姿误差和召回率等指标评估。

Result: Kimera-VIO是唯一完成全部序列的系统，具有最低相对误差但绝对误差逐渐积累；DPVO和DPV-SLAM保持追踪但低光下绝对误差约100米；经典单目及滤波方法在大部分困难低光序列中失败。

Conclusion: RGB-only SLAM在低光照下稳定追踪需要惯性融合与全局优化；未来可能需要低光学习的特征前端或辅助传感器来弥补差距。
...
TL;DR: 提出Seg2Grasp模块化流水线，通过分割、抓取、分类三步实现鲁棒吸盘抓取，在动态杂乱料箱场景中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有端到端学习方法在非结构化环境中对未知或复杂物体泛化能力不足。

Method: 采用Transformer模型进行类无关分割，表面法线与掩码提议确定吸盘点，微调开放词汇Mask-CLIP进行物体识别。

Result: 真实机器人实验显示Seg2Grasp在成功率和适应性上优于现有方法。

Conclusion: Seg2Grasp是工业自动料箱抓取的有效工具。
...
### [408] [Lifelong Multi-Subsystem Pickup and Delivery with Buffer-Limited Handover Stations](https://arxiv.org/abs/2607.17724)
*Chuanlong Zang,Isabelle Barz,Anna Mannucci,Philipp Schillinger,Florian Lier,Wolfgang Hönig*

Main category: cs.RO

TL;DR: 本文提出了一种在线协调方法HARR，用于解决多子系统货物交付中因共享站点缓冲区有限导致的阻塞与饥饿问题，在仿真中吞吐量提升高达77%，积压降低92%。


<details>
  <summary>Details</summary>
Motivation: 多Agent货物交付(MAPD)中，不同子系统通过共享交接站交换货物时，单泊位和有限缓冲区易引发阻塞和饥饿，现有方法缺乏有效协调。

Method: 提出HARR方法，包括共享泊位预约日历和确定性滚动视界缓冲区占用预测，仅当泊位空闲且缓冲区不超容量时接受候选路径。

Result: 仿真表明，HARR在中等负载下比固定泊位消融方法吞吐量高77%、积压低92%，且规划时间少于耦合的站点感知Token Passing基线。
...
### [409] [VLN-AVP: Zero-Shot Vision-Language Navigation with Hybrid Long-Short-Term Memory for Autonomous Valet Parking](https://arxiv.org/abs/2607.17767)
*Yijian Li,Xiangru Mu,Changze Li,Hantian Shi,Jiyuan Cai,Jia Cai,Xiaoxue Liu,Yajing Sun,Ming Yang,Tong Qin*

Main category: cs.RO

TL;DR: 提出VLN-AVP零样本导航框架，结合BEV空间感知和VLM通用智能，实现无预建地图的自动代客泊车。


<details>
  <summary>Details</summary>
Motivation: 现有AVP方法依赖预建地图，限制了其在未知环境和开放词汇目标上的扩展性。

Method: 融合BEV模型与VLM，设计混合记忆系统（短时感知记忆+长时拓扑记忆），并构建VLN-AVP数据集与基准。

Result: 仿真实验中成功率比VLN方法提升25%，比自动驾驶方法提升15%；真实车辆实验也取得领先成功率。

Conclusion: VLN-AVP框架消除了地图依赖，实现了零样本自然语言导航，且具有实际可行性。
...
### [410] [From Sign Language Generation to Humanoid Execution: Vision-Language Guided Retargeting with Collision Mitigation](https://arxiv.org/abs/2607.17769)
*Nabeela Khan,Bowen Wu,Runwu Shi,Benjamin Yen,Takeshi Ashizawa,Carlos Toshinori Ishi,Takashi Minato,Kazuhiro Nakadai*

Main category: cs.RO

TL;DR: 提出一个系统级框架，通过两个组件将手语生成输出映射到人形机器人关节空间执行，以解决自交碰撞问题。


<details>
  <summary>Details</summary>
Motivation: 现有手语生成系统输出的人体运动在机器人执行时会出现自交碰撞，导致逆运动学无解和碰撞轨迹。

Method: 一是体素SMPL-X碰撞缓解模块，二是基于视觉-语言引导的重定向算法，利用VLM作为视觉批评器触发修正。

Result: 碰撞处理和感知引导修正是实现可靠人形机器手语的关键缺失组件。

Conclusion: 该框架能减少自交碰撞，改善机器人执行可行性。
...
### [411] [Polar Coordinate-based Differential Evolution for Moving Target Search Using Vision Sensor on Unmanned Aerial Vehicles](https://arxiv.org/abs/2607.17771)
*Thu Hang Khuat,Duy-Nam Bui,Thuy Ngan Duong,Manh Duong Phung*

Main category: cs.RO

TL;DR: 提出一种基于极坐标差分进化（PDE）的无人机搜索算法，以提升搜救中的检测概率和效率。


<details>
  <summary>Details</summary>
Motivation: 搜救存在“黄金时间”，需提高无人机检测概率并缩短执行时间。

Method: 将目标运动建模为马尔可夫过程，用贝叶斯理论构建适应度函数，结合极坐标与差分进化生成路径。

Result: 仿真与实验显示，PDE在检测概率和执行时间上优于现有算法，且适用于实际场景。

Conclusion: PDE算法在搜救任务中有效且实用，代码已开源。
...
### [412] [Reasoning as a Double-Edged Sword: Architecture and Cross-Stage Robustness in Vision-Language-Action Models](https://arxiv.org/abs/2607.17786)
*Tuan Duong Trinh,Naveed Akhtar,Basim Azam*

Main category: cs.RO

TL;DR: 本文探究了在视觉-语言-动作（VLA）模型中添加推理步骤是否提升鲁棒性，发现潜在迭代模型鲁棒性最差，而推理输出作为安全信号在自适应攻击下失效。


<details>
  <summary>Details</summary>
Motivation: 直觉上，推理策略应能更好地吸收扰动输入，但该假设缺乏系统性验证。

Method: 在LIBERO和SimplerEnv环境中，对无推理、文本链式推理、潜在迭代循环三种模型施加视觉、推理、动作阶段的随机噪声和白盒扰动，并测试推理输出作为安全信号的可行性。

Result: 潜在迭代模型鲁棒性最差，任务成功率大幅下降；推理深度变化影响微小；计划-动作一致性探针在自适应攻击下失效，与动作异常探针融合后防御成功率未提升。

Conclusion: 在输出级行为探针和视觉阶段白盒攻击条件下，推理输出作为安全信号存在固有上限，任何可行防御必须先满足这一前提。
...
TL;DR: 本文提出TSC-VP-STO算法，通过任务空间约束替代终端关节空间约束，联合优化轨迹和冗余自由度，实现林业起重机的无碰撞、时间最优运动规划，平均轨迹时长减少12-15%，并改善液压泵流利用。


<details>
  <summary>Details</summary>
Motivation: 现有VP-STO算法要求固定终端关节配置，限制了冗余机械臂（如林业起重机）利用冗余度的能力，尤其在非线性液压泵流约束下无法适应环境依赖运动。

Method: 采用任务空间约束扩展VP-STO，通过配置空间分解和推导林业起重机运动学的具体可达性约束，联合优化轨迹和终端冗余自由度。

Result: 在多个规划目标和航点配置下，轨迹时长平均减少12-15%，泵流利用率改善；通过实际林业起重机日志装载循环验证应用可行性。

Conclusion: TSC-VP-STO有效克服了终端约束限制，利用冗余度实现更平衡的泵利用和更短轨迹时长，适用于实际林业起重机作业。
...
TL;DR: 提出了一个面向协作机器人工具交接的接收者为中心的自适应系统，利用大语言模型和实时手部追踪动态调整工具姿态，减少抓取延迟并提升信任感知。


<details>
  <summary>Details</summary>
Motivation: 传统静态交接在处理不对称工业工具时会导致抓取笨拙，缺乏适应性和安全性。

Method: 基于Franka协作机器人，结合LLM进行意图识别和MediaPipe进行3D手部追踪，动态调整末端执行器朝向以呈现手柄优先的工效学最优姿态。

Result: 自适应系统相比静态基线减少了非对称工具的抓取延迟，提高了交互流畅性，并改善了运动可预测性和任务简单性等信任相关感知。

Conclusion: 接收者为中心的自适应交接能有效提升人机协作中工具交接的效率和信任感。
...
TL;DR: 提出了一种基于稀疏距离采样和里程计的鲁棒终身定位框架，能在动态室内环境中实现实时定位，并解决被绑架机器人问题。


<details>
  <summary>Details</summary>
Motivation: 在动态室内环境中，预定的地图可能因动态障碍物而不准确，需要鲁棒的定位方法。

Method: 利用机器人的里程计和稀疏距离采样，结合动态障碍物处理，通过时间融合收敛到真实位姿。

Result: 在真实室内环境中，仅用16个距离样本即可达到与SLAM相当的定位效果，显著降低传感器成本、隐私风险和带宽需求。

Conclusion: 该方法在静态环境中能保证收敛，在动态环境中只要变化规律被正确学习也能保证收敛，适用于大规模室内环境。
...
TL;DR: 本文提出ConceptTree框架，将机器人操作中的高层技能选择建模为基于可解释概念序列的推理，实现透明决策过程。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作方法将技能选择视为黑箱映射，缺乏决策透明性，难以支持人类监督和干预。

Method: 学习与视觉输入对齐的归一化概念空间，并训练决策树预测高层技能，形成可追溯、可干预的透明决策流程。

Result: 在真实机器人任务中，ConceptTree优于现有概念基线，尤其在复杂长时域场景中表现更佳；支持通过修改单个概念进行细粒度干预。

Conclusion: ConceptTree通过概念级推理实现了可解释、可干预的机器人操作决策，无需重新训练即可修正错误。
...
TL;DR: 提出了一种双机械臂协同装配的端到端框架，基于CAD模型实现，比单臂系统平均执行时间降低50%以上，轨迹质量更高，机器人放置搜索更快，并提供了理论保证和开源软件。


<details>
  <summary>Details</summary>
Motivation: 传统单臂系统在紧密装配操作中存在效率低、轨迹质量差等问题，而双臂协同有望改善这些不足。

Method: 利用CAD模型的数字信息，设计双臂协同规划与控制的端到端框架，在仿真和实体机器人上验证，并建立机器人单元尺寸的理论边界。

Result: 双臂协同比单臂减少50%以上平均执行时间，产生更高质量轨迹，加速有效机器人放置的搜索，实验支持理论保证。

Conclusion: 双臂协同在紧密装配中具有显著优势，提供了完整框架、理论保障和开源实现，适用于实际工业场景。
...
TL;DR: 提出CMG-RRT算法，通过临界流形引导采样，实现紧密装配运动规划，成功解决Elk谜题等难题。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理紧密几何约束下零件处于零间隙配置的装配规划。

Method: 基于临界流形（接触点集合）的自适应分层采样，结合RRT框架进行探索。

Result: 在旋转装配基准测试中达到100%成功率，首次全自动求解Elk解谜难题。

Conclusion: CMG-RRT在紧密装配问题中具有概率完备性和高效性，开源代码已验证。
...
TL;DR: 提出了统一的多任务协同感知网络UMCP，同时进行行李车检测、关键点检测和方向估计，在YOLOv12基础上融合关键点和方向特征，并采用圆形概率分布与KL散度损失提高方向估计精度，在降低模型复杂度和计算成本的同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 现有行李车视觉感知方法依赖级联多模型推理，导致推理延迟高、部署成本大，无法满足机器人实时高精度定位的需求。

Method: 基于YOLOv12架构，集成行李车检测、关键点检测和方向估计多任务；关键点特征与方向特征融合后输入方向特征增强模块（OFEM）；采用圆形概率分布建模和KL散度损失优化方向估计。

Result: 所提方法在保持竞争性整体精度的同时，显著降低了模型复杂度和计算成本。

Conclusion: UMCP网络有效解决了级联模型的延迟和成本问题，为机器人自主行李车收集提供了高效统一的视觉解决方案。
...
TL;DR: 本文提出一种最小学习框架，仅用SEA-RAFT生成稠密立体对应点及不确定性，其余保持显式处理，在分布偏移下实现鲁棒VIO。


<details>
  <summary>Details</summary>
Motivation: 探究分布偏移下鲁棒VIO是否真需更深层次的学习估计，还是仅限视觉测量生成。

Method: 使用SEA-RAFT提出稠密立体对应点及不确定性，稀疏采样、几何验证、不确定性加权重投影，融入滑窗立体惯性估计器，并传播不确定性进行3D高斯建图。

Result: 在EuRoC、VIODE、4Seasons上，运动模糊、动态场景、光照变化、室内外迁移下均稳定准确；消融表明单独学习光流不足，需结合几何验证与不确定性加权。

Conclusion: 对于分布外鲁棒VIO，精心整合的学习视觉测量比学习更多估计流水线更有效。
...
TL;DR: MEVION是一个低成本、开源的双臂机器人数据收集系统，能产生更大的力和速度，支持模仿学习。


<details>
  <summary>Details</summary>
Motivation: 现有ALOHA系统在力和速度方面有限，难以处理重物或快速操作，因此需要开发更强大的系统。

Method: 通过广泛使用钣金焊接，以少量零件低成本构建大型机身，配备四个6自由度机械臂，肘关节采用闭链机构。

Result: 每个臂重7.0公斤，最大扭矩60牛米，整个系统建造成本约14000美元，能完成以前无法实现的操作任务。

Conclusion: MEVION实现了更强功能的数据收集，支持基于模仿学习的运动生成。
...
### [422] [Distilling Global Traversability Priors for Image-based Affordance Prediction in Off-road Environments](https://arxiv.org/abs/2607.17984)
*Matthew Sivaprakasam,Samuel Triest,Micah Nye,Deegan Atha,Shehryar Khattak,David Fan,Wenshan Wang,Sebastian Scherer*

Main category: cs.RO

TL;DR: 提出利用卫星图像监督学习，从第一视角图像中提取远距离可穿越性前沿，以改善非结构化地形中的长距离导航性能。


<details>
  <summary>Details</summary>
Motivation: 传统自主导航方法在长距离场景中短视，且度量地图受限于深度传感范围，导致信息丢失。

Method: 通过卫星图像为图像/姿态对计算可行导航路径，并以此监督网络训练，避免人工演示数据。

Result: 在离线基准测试中性能提升超过10%，真实实验中减少人工干预次数。

Conclusion: 该方法能有效恢复地图范围外的导航信息，提升长距离越野导航效果。
...
### [423] [RynnBrain 1.1: Towards More Capable and Generalizable Embodied Foundation Model](https://arxiv.org/abs/2607.17977)
...
TL;DR: RynnBrain 1.1 是一个涵盖 2B、9B 和 122B-A10B 三种规模的具身基础模型家族，通过统一的时空与物理框架训练，支持具身感知、空间推理、定位和规划，并在机器人操作中引入接触点预测和原生 3D 定位，展现出更强性能。


<details>
  <summary>Details</summary>
Motivation: 现有具身模型在机器人操作的直接对齐和跨本体泛化方面存在不足，需要更统一的框架来提升感知、定位和规划能力。

Method: 采用统一时空与物理框架训练，引入接触点预测和原生 3D 定位；开发统一跨本体动作空间和本体特定掩码的 RynnBrain-VLA，并在多种机器人平台上部署。

Result: 122B-A10B 模型在 VSI-Bench、MMSI 和 RefSpatial-Bench 上优于所有评估的闭源和开源模型；实际机器人实验显示，RynnBrain 初始化的策略优于 Qwen 基线和通用 VLA，联合多任务与多本体训练提升了过程分数和成功率。

Conclusion: RynnBrain 1.1 在具身认知、定位和 3D 定位上表现优异，验证了统一框架和跨本体训练的有效性。
...
TL;DR: 提出了持久对象分词（POT）方法，通过维护角色索引的3D对象记录，将RGB-D观测转化为对象令牌，用于控制人形机器人全身动作，在Unitree G1上显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作策略在长期人形机器人操作中，任务对象在移动、接触、遮挡和恢复过程中需要被视为持久物理实体，但现有方法存在对象状态偏差问题。

Method: 通过POT从RGB-D观测中维护角色索引的3D对象记录，将其转化为对象令牌，用于条件化全身动作生成并支持几何谓词检查，形成闭环执行系统。

Result: 在Unitree G1上，POT-VLA将基准从39/80提升至71/80（8个任务族）；在外部基准上达44/50，优于Being-0的37/50。

Conclusion: 持久的对象中心状态是可行的人形机器人VLA执行抽象，尤其在需要维持3D关系的任务中效果显著。
...
TL;DR: 提出了RoboHarness，一个统一框架，通过多模态执行记忆和记忆桥接，实现异构策略的零样本长时域规划和鲁棒性提升。


<details>
  <summary>Details</summary>
Motivation: 现有规划方法忽略了异构策略间能力边界不确定性和分布不匹配问题。

Method: 采用多模态执行记忆和在线证据刻画策略能力边界，通过记忆桥接检索下一策略的执行轨迹并引导机器人至分布内状态区域。

Result: 在三个公开基准、500个自定义任务和135个真实机器人实验中，实现了有效的能力感知路由和稳定策略协调。

Conclusion: RoboHarness能显著提高零样本长时域规划和分布外鲁棒性。
...
TL;DR: 提出了一个统一接口UniETP，整合了四个常用模拟器（AI2-THOR、VirtualHome、Habitat、BEHAVIOR），用于标准化和多样化体现任务规划。


<details>
  <summary>Details</summary>
Motivation: 现有模拟器和数据集在观察格式、动作类型和任务领域上各自独立，导致模型评估碎片化，训练数据可扩展性受限。

Method: 设计UniETP，提供一致的观察和动作空间，构建支持复杂任务目标的评估系统，并自动生成多难度数据集。

Result: 在统一基准上进行了广泛实验，评估了近期模型的体现规划能力，并分析了性能瓶颈。

Conclusion: UniETP通过标准化和多样性促进了通用体现规划研究，并将开源代码和数据。
...
TL;DR: Oogway和Crush是杜克机器人俱乐部为RoboSub 2026设计的两款AUV，通过机械、电气和软件系统的全面升级，首次同时追求运动、视觉、操控和声学跟踪四大设计目标，并显著提升了可靠性。


<details>
  <summary>Details</summary>
Motivation: 过去团队专注于有限的任务范围，现在希望通过可靠性投资，首次全面覆盖RoboSub的四大设计目标，以提升竞争策略的多样性和稳定性。

Method: 在机械方面增加推进器并优化外壳设计以实现俯仰稳定；电气方面修复故障点并升级推进器控制硬件，同时重新设计声学系统，使用带高阶滤波器的定制PCB以提高检测可靠性；软件方面改进了状态估计、声呐目标检测、视觉任务规划和内部视觉通信（IVC）。

Result: 成功扩展了任务范围，同时保持了高可靠性，能够在有限的测试时间内执行更广泛的任务。

Conclusion: 通过系统性投资于硬件和软件的可靠性，团队能够首次全面挑战RoboSub的所有设计目标，为竞争策略提供了更强大的支持。
...
TL;DR: 首次在商用自动驾驶卡车系统中应用AST和DiFS两种稀有故障发现算法，并引入PCA统计方法进行故障分类和诊断。


<details>
  <summary>Details</summary>
Motivation: 现有罕见故障发现方法仅用于简单学术驾驶系统，未验证其在商业级鲁棒规划器中的有效性。

Method: 采用自适应压力测试（AST）和扩散故障采样（DiFS）搜索导致碰撞的噪声轨迹，结合PCA分类故障模式并反演通用噪声向量。

Result: 两种算法在并道和切入场景中发现了传统蒙特卡洛模拟无法检测的碰撞，PCA聚类可复现相同及相似场景的故障。
...
TL;DR: 提出了一种利用Nvidia Isaac Sim和Isaac Lab训练的四足机器人全身控制强化学习框架，实现了零样本sim-to-real迁移，在Unitree Go1上验证了性能。


<details>
  <summary>Details</summary>
Motivation: 学习型运动控制方法在模拟中表现良好，但在物理系统上部署时常遇到sim-to-real差距问题。

Method: 基于强化学习的框架，使用Nvidia Isaac Sim和Isaac Lab进行训练，实现零样本sim-to-real策略。

Result: 在Unitree Go1硬件上验证，速度跟踪性能与自带控制器相当，抗干扰能力更强，最高线速度2.0 m/s，角速度1.8 rad/s。

Conclusion: 该框架能有效减少sim-to-real差距，实现鲁棒的全身控制运动策略。
...
### [430] [Imitation of Arm Gestures by the Semi-Humanoid Robot NICO](https://arxiv.org/abs/2607.18197)
*Anastasiya Ihnatovich,Igor Farkaš*

Main category: cs.RO

TL;DR: 本文提出了一种基于解析几何和MediaPipe姿态估计的半人形机器人NICO手臂手势模仿系统，仅使用单目RGB输入即可生成有意义的模仿动作。


<details>
  <summary>Details</summary>
Motivation: 实现无缝的人机交互需要机器人具备多种感知和运动能力，其中模仿人类手势是关键；人形机器人因其拟人特征在此方面具有优势。

Method: 使用MediaPipe框架从RGB图像中获取人体手臂关节及手部关键点的3D坐标，通过解析几何关系计算关节角度，然后将角度映射到NICO机器人的电机配置并执行预定运动序列。

Result: 针对6名不同身高参与者的多种代表性手臂手势的初步实验表明，该方法能产生有意义的模仿动作，但在复杂姿势和手腕相关动作上存在局限。

Conclusion: 该方法仅利用单目RGB输入即可实现有效模仿，但需进一步改进以处理更复杂的手势。
...
### [431] [World Translation: Minimizing Sim-to-Real Gap with Backward Dynamics Extraction and Unpaired Domain Translation](https://arxiv.org/abs/2607.18154)
*Xinchen Yao,Leixin Chang,Hua Chen*

Main category: cs.RO

TL;DR: 提出World Translation方法，通过将不可观测动力学信息作为风格、可观测动力学作为内容进行跨域转换，解决了仿真到真实转移中部分可观测性问题。


<details>
  <summary>Details</summary>
Motivation: 现有实到仿方法基于学习动力学模型，但面临部分可观测性问题：相同观测可能因不可观测因素导致不同转移，而历史观测无法提供足够信息。

Method: 将不可观测动力学信息从观测转移中反向提取，作为域风格；将可观测动力学内容保留，通过无配对域翻译方法在仿真和现实间转换。

Result: 在人形、四足和机械臂平台上比基线方法更准确地建模动力学，尤其在历史观测无法恢复不可观测因素时提升最大。在Go2四足机器人上验证了改进的策略迁移。

Conclusion: World Translation有效结合仿真器的确定性和学习模型的准确性，解决了部分可观测性带来的动力学建模难题，提升了策略从仿真到现实的迁移效果。
...
TL;DR: 提出一种上下文条件安全评判器，通过分解安全、效率和距离约束三项指标来排序扩散规划器生成的轨迹，在杂乱室内导航中达到最高成功率和路径效率，并实现从仿真到真实机器人的零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 传统固定安全裕度在杂乱室内场景中因校准误差导致失败：保守裕度引起绕路和超时，宽松裕度则因感知偏差导致近边界捷径。

Method: 构建上下文条件安全评判器，包含三项分解损失：安全项（清除预算惩罚+控制障碍函数残差）、效率项（平滑惩罚+安全门控绕路比惩罚）、距离约束匹配项（锚定学习预算到ESDF清除距离），通过两阶段师生蒸馏将特权ESDF知识迁移至纯感知选择器。

Result: 在HM3D和MP3D的PointGoal导航任务中，包括跨数据集迁移，成功率和SPL均优于扩散、优化和强化学习基线；仅仿真训练后可直接部署至Unitree G1人形机器人完成任务。

Conclusion: 通过解耦安全-效率-距离约束的评判学习，能够自适应调整运动裕度，实现高鲁棒性跨场景导航，且仿真到真实迁移无需微调。
...
### [433] [FM-VLA: Force-based Memory for Vision-Language-Action Models in Contact-Rich Manipulation](https://arxiv.org/abs/2607.18231)
...
TL;DR: 提出FM-VLA模型，通过力记忆令牌增强VLA模型在接触丰富任务中的时序推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有记忆增强VLA依赖视觉记忆，在时序事件模糊（如重复按钮按压）时计算昂贵且效果有限。

Method: 使用变分自编码器（VAE）对力历史进行重构预训练，生成紧凑力记忆令牌，与状态历史共同作为条件输入动作专家模块。

Result: 在隐藏方块寻找、按钮按压、擦盘子三项记忆依赖任务中，成功率超80%，计算开销极小，显著优于基线方法。

Conclusion: 力记忆作为VLA模型的轻量级补充，能有效提升接触丰富、非马尔可夫任务的性能。
...
### [434] [Optimization of sim-to-real transfer in the humanoid robot NICO](https://arxiv.org/abs/2607.18210)
*Juraj Gavura,Igor Farkaš*

Main category: cs.RO

TL;DR: 本文提出了一种基于低成本触觉校准的机器人抓取管线，通过YOLO检测、立体视觉和视觉反馈模型，在无需深度相机或外部跟踪系统的情况下，提升了桌面物体抓取的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有机器人抓取中，从仿真到实际的迁移误差导致定位不准确，而传统高精度传感器成本较高，因此需要低成本校准方法。

Method: 扩展了之前的触觉校准方法，结合YOLO进行物体和手部检测、基于机器人内置低分辨率鱼眼摄像头的立体视觉定位，以及任务特定修正和视觉反馈模型。

Result: 完全非线性校准模型在校准区域内表现最佳，而视觉反馈模型在整个桌面工作空间的总抓取成功率最高。

Conclusion: 提出的校准抓取管线无需RGB-D摄像头或外部跟踪系统，即可有效缩小仿真到现实的差距，提高抓取性能。
...
TL;DR: 提出一种轻量级架构扩展，让基于Transformer的策略直接使用密集预训练补丁特征，无需完整视觉-语言模型，在模拟和真实环境中实现40%的相对性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有机器人策略要么压缩每个视觉观察为单个全局token（丢失空间细节），要么从头训练视觉骨干（未利用预训练优势），而使用密集补丁特征的大型视觉-语言-动作模型又过于臃肿缓慢，需要一种轻量高效的解决方案。

Method: 提出块因果注意力掩码，在保持标准策略时间因果性的同时，让模型能直接关注每个观察中的多个补丁token以及其他状态信息。

Result: 在四个模拟和三个真实环境套件中，相比使用最先进全局池化表示的策略，实现40%相对提升；超越微调OpenVLA-OFT 18%，参数量仅为其0.7%。

Conclusion: 补丁策略提供了一种让机器人社区直接利用视觉表示学习进展的轻量高效方法，不影响高频反应控制所需的训练效率和推理速度。
...
### [436] [An Interactive Interface for Control Integration in Mid-Fidelity Wind Farm Simulation](https://arxiv.org/abs/2607.16386)
*Zekai Chen,Casey Heiskell,Ryozo Nagamune*

Main category: eess.SY

TL;DR: 本文介绍ffconnect，一个基于Python的开源接口，用于连接FAST.Farm风电场模拟器与数据驱动控制和AI生态系统，支持交互式WFC设计验证。


<details>
  <summary>Details</summary>
Motivation: 数据驱动控制和AI为WFC带来新机遇，需要支持交互式设计和验证的仿真工具。

Method: 开发ffconnect接口：重构API、增强状态访问、基于Python构建，与科学计算和机器学习生态集成。

Result: ffconnect在模拟长度和农场规模上开销可忽略，并通过偏航跟踪案例验证有效性。

Conclusion: ffconnect为WFC研究提供了高效、通用的开放工具。
...
### [437] [A 0.74-mW K$_u$-band Cryogenic SiGe LNA in 130-nm BiCMOS with 14-K Noise Temperature for Scalable Quantum Readout](https://arxiv.org/abs/2607.16505)
*Xiuhan Chen,Haoling Li,Gun Suer,Leonardo Ranzani,Kin Chung Fong,Aravind Nagulu,Najme Ebrahimi*

Main category: eess.SY

TL;DR: 本文介绍了一种工作在Ku波段、功耗低于1毫瓦的低温低噪声放大器，采用SiGe BiCMOS工艺，专为亚开尔文量子比特读取链设计。


<details>
  <summary>Details</summary>
Motivation: 为了在量子比特读取链中实现低功耗和高性能的低温放大器，集成JPA和SiGe LNA于Still级。

Method: 采用四级共源共栅结构，通过巴伦耦合级间匹配、Q增强谐振腔（2位开关调谐）和电压控制交叉耦合负跨导单元，并优化不同温度下的工作点以稳定性能。

Result: 在2.5K下测试，12-18GHz内平均噪声温度约14K，16.5GHz处峰值增益28dB，功耗仅0.74mW。

Conclusion: 该放大器实现了Ku波段低温SiGe LNA中最低的直流功耗，适合量子应用。
...
### [438] [A Unified Statistical Framework for Multicopter Propeller Damage Diagnosis Based on Functionally Pooled Models and Bayesian Quantification: Experimental Flight Test Assessment](https://arxiv.org/abs/2607.16512)
*Shinan Huang,Jingxi Zhu,Fotis Kopsaftopoulos*

Main category: eess.SY

TL;DR: 提出了一种基于FP-AR模型的随机时间序列框架，用于多旋翼螺旋桨损伤诊断，仅需标准IMU数据即可实现检测、定位和量化。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖额外传感器或复杂模型，缺乏数据效率与统计严谨性，难以在实际飞行条件下稳定运行。

Method: 使用功能池化自回归模型表示系统动态，通过残差统计检验检测损伤、模型选择识别故障电机、贝叶斯推断量化损伤程度并提供不确定性估计。

Result: 在户外六旋翼飞行实验中，六通道IMU数据验证了跨飞行一致性，贝叶斯方法比批量估计更稳定且提供不确定性描述。

Conclusion: 该框架提供了一种数据高效、可解释且统计严谨的多旋翼结构健康监测解决方案。
...
### [439] [Distributed Adaptive Estimation of Unknown Nonlinear Systems without Input Sharing](https://arxiv.org/abs/2607.16510)
*Moh Kamalul Wafi,Milad Siami*

Main category: eess.SY

TL;DR: 针对有向通信网络下具有未知源动力学的离散时间非线性系统，提出了一种分布式自适应状态估计方法，实现了无需共享激励或控制输入的完全分布式实现，并通过Lyapunov分析证明了估计误差动力学的输入-状态稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有分布式状态估计方法通常要求共享激励或控制输入，且对非线性系统和非理想通信条件的适应性有限，亟需一种完全分布式且鲁棒的估计方案。

Method: 提出归一化自适应估计方案，结合Lyapunov稳定性理论进行输入-状态稳定性分析，并利用范数基和LMI基方法导出耦合算子的Schur稳定性条件。

Result: 在星型、环型和路径通信拓扑下实现精确的分布式状态估计，验证了稳定性条件的有效性，且计算结果表明估计器随网络规模扩展效率良好。

Conclusion: 所提方法在无人共享激励、有限扰动条件下仍能保证有界估计误差，扰动消失时渐近收敛，适用于非线性系统分布式估计。
...
### [440] [Comparative Analysis of Linepack Impact in Hydrogen and Natural Gas Networks under Dynamic Operating Conditions](https://arxiv.org/abs/2607.16527)
*Amin Salehi,Janne Seppanen,Mahdi Pourakbari-Kasmaei*

Main category: eess.SY

TL;DR: 比较了氢气与天然气管道在压缩机故障下的动态响应，发现氢气恢复更快但管存更少，管径选择影响压降和负荷削减。


<details>
  <summary>Details</summary>
Motivation: 天然气网络中管存是关键缓冲，但氢气作为替代能源后其动态行为尚不明确。

Method: 对天然气和氢气网络进行三天动态模拟，设置相同与不同管径两种案例。

Result: 氢气恢复更快；相同管径时氢气压降更小、负荷削减更少；减小管径后氢气压降增大、负荷削减超过天然气。

Conclusion: 结果可帮助天然气运营商优化氢气网络的管径设计与运行控制。
...
### [441] [Dynamic Speed Limit Control of Connected Automated Vehicles in Freeway Networks Considering Traffic Composition Uncertainty](https://arxiv.org/abs/2607.16615)
*Lei Wei,Yu Han,Haiyang Yu,Yunpeng Wang*

Main category: eess.SY

TL;DR: 提出了一种考虑交通组成不确定性的MPC框架，用于混合交通环境下CAV动态限速控制，以提高高速公路可持续性。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设CAV渗透率确定且精确已知，但实际观测误差导致控制性能下降。

Method: 建立不确定性感知的宏观混合交通模型，将渗透率不确定性传播至流量动态；设计交通组成感知MPC优化CAV限速，增强鲁棒性。

Result: 在单瓶颈和多瓶颈网络中，所提控制器生成空间协调的限速方案，有效减少旅行时间并带来环境效益。

Conclusion: 显式考虑渗透率不确定性可提升CAV限速控制的鲁棒性和可持续性。
...
### [442] [Cooperative MARL-Based Energy-Efficient Power Control for Two-Hop Relaying Networks](https://arxiv.org/abs/2607.16703)
*Xinyu Qu,Yuanzhe Geng,Erwu Liu,Rui Wang*

Main category: eess.SY

TL;DR: 本文研究合作通信网络中继的能效优化问题，提出基于延迟奖励的多智能体深度Q网络学习框架，在仅有部分可观测过时信道状态信息下实现接近最优的能效。


<details>
  <summary>Details</summary>
Motivation: 实际通信中瞬时信道状态信息难以获取，现有博弈论方法依赖理想信息，需要开发适应非理想信息条件的能效优化方案。

Method: 定义延迟奖励状态动作值函数，构建多智能体深度Q网络学习框架，通过博弈论方法分析瞬时信道状态信息下的能效上界。

Result: 所提方法显著优于对比方案，与最优解的差距仅为5.2%。

Conclusion: 该框架有效解决了非理想信道信息下的合作通信能效优化问题，性能接近理论最优。
...
### [443] [Driver Behavior Under Traffic Complexity: Variance Decomposition and Cross-Driver Generalization for Driver Monitoring](https://arxiv.org/abs/2607.16847)
*Lukas Köning,Nataša Miličić,Klaus Bogenberger*

Main category: eess.SY

TL;DR: 从行为信号推断交通复杂度，31个特征有效但方差分解显示复杂度仅解释1.5%行为变异，驾驶员身份解释23%，残余75%构成瓶颈。引导注视率是最实用特征。


<details>
  <summary>Details</summary>
Motivation: 现有复杂度指标仅关注外部环境或驾驶员内生状态，缺乏两者连接，需从驾驶员行为反推交通复杂度。

Method: 筛选20名驾驶员在真实城市交通中的175个行为特征（凝视、头部姿态、纵向控制、引导注视、扫描策略），采用等价检验和混合效应方差分解。

Result: 31个特征表现显著复杂度效应，140个为无效；复杂度仅占1.5%行为变异，驾驶员身份占23%；所有个性化策略失败，分类F1约0.45。

Conclusion: 引导注视率是唯一可部署特征，方差结构是复杂度估计的主要瓶颈，定义了复杂度自适应辅助系统的三个部署区间。
...
### [444] [Learning to Stay Fresh: A Self-Learning Semantic Framework for Underwater Internet of Things](https://arxiv.org/abs/2607.16902)
*Ananya Hazarika,Mehdi Rahmati*

Main category: eess.SY

TL;DR: 提出了一个基于语义贝叶斯优化的自学习框架，解决水下物联网互操作性问题，利用信息新鲜度指标提升实时监测性能。


<details>
  <summary>Details</summary>
Motivation: 非传统物联网范式强调信息有用性，但水下声学通信的延迟、带宽限制和节点异构性导致互操作性挑战。

Method: 三层架构结合语义贝叶斯优化与多传输贝叶斯优化，采用混合深度神经网络-高斯过程代理模型，并定义传播感知的信息年龄指标。

Result: 通过与竞争方法对比，证明了所提方法在实时环境监测中的优越性。

Conclusion: 该框架有效提升了水下物联网的互操作性和监测效率。
...
### [445] [Cluster-Based Distributed Small-Signal Stability Certificates for Grid-Forming Inverter Networks](https://arxiv.org/abs/2607.16985)
*Bhathiya Rathnayake,Sijia Geng*

Main category: eess.SY

TL;DR: 本文提出了一种针对构网型逆变器网络的时间域小信号稳定性认证框架，支持可选的聚类分辨率，可在不同组织尺度上认证稳定性。


<details>
  <summary>Details</summary>
Motivation: 大规模电力网络常按地理或所有权组织，全局模型难以获取，需要一种无需全局特征值计算的稳定性认证方法。

Method: 线性化后采用小角度近似分解模型为电压子系统和角频率子系统；电压子系统通过节点间增益和循环小增益论证获得从分散到集中的稳定性证书；角频率子系统利用对称加权拉普拉斯网络结构进行能量论证。

Result: 对于任意网络分区，集群验证内部有向环条件与集群间路径条件，可恢复分散和集中式证书；稳定性指数可定位极限裕度至单个节点、内部反馈回路和集群间通道。

Conclusion: 该方法能有效认证网络级稳定性，提供超越通过/失败的诊断信息，适用于分层组织的电力系统。
...
### [446] [A conditional no-go for resource-free magic-axis measurement on a static surface code](https://arxiv.org/abs/2607.16968)
*Jiachen Shen,Hui Zhong*

Main category: eess.SY

TL;DR: 在静态表面码补丁中，如果无折叠或自对偶结构，则无法实现魔法态培育所需的魔法轴检查，除非接受稀有机会。这是一个有条件的不可行性结果。


<details>
  <summary>Details</summary>
Motivation: 纠错容错机器大部分成本用于制造魔法态，而培育法通过测量魔法轴原位生成魔法态，但所有已知构造都依赖折叠或自对偶补丁，此必要性被广泛认为是民间传说，本文旨在检验该传说。

Method: 通过拓扑学解读接受结果，证明单次稳定子测量转录的不可行性；在自适应后选择协议的有界深度（多项式时空体积）模型中，基于两个结构假设和一个亚临界假设推导不可行性。

Result: 有用检查必须为魔法轴付出代价：添加电荷转换资源、离开历史稀释区域、或仅接受指数级罕见结果。在单次转录中直接证明；有界深度模型下在三个假设成立时成立，并确定剩余一个开放假设。

Conclusion: 魔法态培育的魔法轴检查在无额外资源或罕见接受时是不可能的，除非解决一个特定猜想。
...
### [447] [Feasibility-Aware Security-Constrained Unit Commitment via Hybrid Soft Actor-Critic with Quantum-Sampled Features](https://arxiv.org/abs/2607.17031)
*George Dimas,Amin Masoumi,Mert Korkali*

Main category: eess.SY

TL;DR: 提出三层混合框架，使用伯努利混合软演员-评论家(HSAC)策略提出每小时机组承诺，结合量子采样辅助通道和本地SCUC混合整数线性规划，实现计算高效的约束机组组合求解。


<details>
  <summary>Details</summary>
Motivation: 传统精确求解SCUC在高保真系统规模下计算成本过高，需要设计加速方法但保持与现有优化求解器的兼容性。

Method: 三层混合架构：①伯努利HSAC智能体输出部分机组承诺决策；②量子采样辅助通道增强状态表征；③仅对部分承诺二进制变量强制约束后，用MILP恢复调度和安全变量。

Result: 14节点案例实现全时段最优恢复；57节点案例极低屏拒绝率；118节点案例因强制承诺上限无法覆盖完整周期，暴露覆盖瓶颈。

Conclusion: 探索性的伯努利智能体与小强制承诺上限下，传至恢复模型的有效承诺信息量成为主导规模化瓶颈。
...
### [448] [Increasing Line Outage Localization Performance with Ensemble Classifiers](https://arxiv.org/abs/2607.17008)
*Daniel Flores,Yuanrui Sang,Michael P. McGarry*

Main category: eess.SY

TL;DR: 本文研究了在电力系统中利用集成分类器监测单条输电线路故障定位的性能提升，发现基于贪婪最大覆盖问题算法选择的观测线路和极端随机树集成技术表现最佳。


<details>
  <summary>Details</summary>
Motivation: 单条输电线路故障可通过监测另一条线路的潮流进行定位，但存在不确定性，需要机器学习方法提高区分能力，因此研究集成分类器在此任务中的改进效果。

Method: 比较了集成分类器与单模型方法（如k近邻分类器）的性能；使用三种算法（贪婪最大覆盖问题、高η、随机选择）选择观测输电线路；基于两个灵敏度因子（线路故障分布因子、线路故障影响因子）进行测量数据分析。

Result: 贪婪最大覆盖问题算法选择的观测线路获得最高F1分数；集成分类器显著优于基础k近邻分类器；极端随机树装袋技术在多项实例中达到最高F1分数。

Conclusion: 所有发现均具有统计显著性，表明集成分类器结合适当线路选择算法可有效提升线路故障定位性能。
...
### [449] [Optimal Safety Control using High-Order Control Barrier Functions](https://arxiv.org/abs/2607.17032)
*Neng Li,Zuodong Pan,Jiaxing Wang,Weiguo Xia,Wei Ren*

Main category: eess.SY

TL;DR: 本文针对非线性控制系统最优安全控制问题，提出了新型高阶控制障碍函数（HOCBFs）和一种高阶控制李雅普诺夫函数（HOCLF），并设计了显式安全控制器，通过向量李雅普诺夫函数方法保障稳定与安全目标兼容，最后通过四旋翼飞行器导航示例验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有零化高阶控制障碍函数在安全控制中存在局限性，亟需更显式、兼容性更强的控制方法以同时实现非线性系统的稳定与安全优化。

Method: 提出两种新型高阶控制障碍函数（HOCBFs）并显式设计安全控制器；利用向量李雅普诺夫函数方法构建高阶控制李雅普诺夫函数（HOCLF）；分析新型HOCBFs与现有方法的关联，并解决HOCLF与HOCBF的兼容性问题以构建最优控制器。

Result: 通过四旋翼飞行器导航数值示例，验证了所提方法在同时保证系统稳定与安全控制目标下的有效性。

Conclusion: 新型HOCBFs和HOCLF能够显式设计安全控制器并兼容稳定与安全目标，为非线性系统最优安全控制提供了有效框架。
...
### [450] [A recursive subspace based method for errors-in-variables model identification of time-varying systems](https://arxiv.org/abs/2607.17065)
*Deepanjhan Das,Shankar Narasimhan*

Main category: eess.SY

TL;DR: 提出了一种递归子空间模型辨识算法（RSMI-IPCA），用于在线辨识时变MIMO系统的状态空间模型，并同时估计异方差噪声方差。


<details>
  <summary>Details</summary>
Motivation: 现有SMI-IPCA方法只能离线处理批量数据，无法适应时变系统或在线更新。

Method: 在SMI-IPCA基础上引入固定长度滞后窗口和递归更新机制，实现模型参数、噪声方差和模型阶数的在线估计。

Result: 仿真验证了算法能适应缓慢或突变的时变系统（如传感器退化、工况变化、结构修改）。

Conclusion: RSMI-IPCA可有效用于在线辨识和自适应模型更新，适用于时变MIMO系统。
...
### [451] [Deep Reinforcement Learning-Based Energy Management for Hydrogen-Enabled Community Microgrids Under Uncertainty](https://arxiv.org/abs/2607.17248)
*Mohamed Atef,Sanath Alahakoon,Umme Mumtahina,Peter Wolfs,Tamer Khatib,Moslem Uddin*

Main category: eess.SY

TL;DR: 针对含氢能的社区微电网，提出PPO优化能量管理，在正常和故障场景下均表现良好。


<details>
  <summary>Details</summary>
Motivation: 可再生能源和负荷的不确定性、动态电价及电池与氢能储能的耦合增加了运行复杂性，现有方法难以应对。

Method: 基于PPO的深度强化学习，构建11维状态和3个连续动作的MDP模型，通过残差功率平衡决定电网交互。

Result: 正常场景年收入19.57万澳元，可再生能源占比91.2%，碳强度0.085 kg CO2/kWh；高故障概率下仍能保持高负载满足率。

Conclusion: PPO可有效协调电池与氢能调度，但训练稳定性和评估边界选择需注意。
...
### [452] [A Phased Development Framework Enabling Islanded Operation of Sustainable AI Data Centers With Onsite Grid-Following and Grid-Forming Energy Architectures](https://arxiv.org/abs/2607.17391)
*Soham Ghosh,Nabil Mohammed,Mohammad Ashraf Hossain Sadi*

Main category: eess.SY

TL;DR: 提出一种分阶段建设框架，通过模块化架构、混合现场发电和电磁暂态仿真，解决AI数据中心规模化部署中电网接入、设备采购和施工周期等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注AI数据中心的环境影响或电网交互能力，但缺乏对其通过工程、采购和施工（EPC）流程实现规模化部署所面临挑战的系统分析。

Method: 采用分阶段开发框架，结合模块化建筑架构、混合现场发电（天然气+构网型储能）及电磁暂态仿真（EMT）评估系统性能。

Result: 现场天然气发电与构网型储能的组合可在早期和中期部署阶段可靠支持数据中心运行；EMT仿真验证了系统在电网扰动下的孤岛运行能力及不同构网控制策略的重连恢复表现。

Conclusion: 所提分阶段框架可帮助开发者在满足快速上市目标的同时，应对电网接入审批和设备采购的多年轻时约束，并实现向全并网模式的可靠过渡。
...
### [453] [An Update to the Level Set Theorems in Hamilton-Jacobi Reachability Analysis](https://arxiv.org/abs/2607.17435)
*Dylan Hirsch,William McEneaney,Jaime Fisac,Claire Tomlin,Sylvia Herbert*

Main category: eess.SY

TL;DR: 本文修正了Hamilton-Jacobi可达性分析中水平集定理成立所需的附加条件。


<details>
  <summary>Details</summary>
Motivation: 针对传统水平集定理在安全关键系统控制中可能存在的技术不完善性，需要补充更严格的适用条件。

Method: 通过理论推导和数学证明，更新了水平集定理的充分条件。

Result: 明确了原有定理成立时被忽略的额外技术条件。

Conclusion: 修正后的水平集定理为安全控制综合提供了更严谨的理论基础。
...
### [454] [Multi-scale closed-loop melt pool control for LPBF via policy optimization](https://arxiv.org/abs/2607.17438)
*Junan Lin,Riccardo Zuliani,Baris Kavas,Markus Bambach,John Lygeros,Efe C. Balta*

Main category: eess.SY

TL;DR: 提出了一种双环数据驱动控制策略用于激光粉末床熔融过程的温度稳定，结合策略梯度优化的层内反馈控制和层间前馈控制，仿真与实验验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LPBF温度控制方法依赖手动调参，复杂几何形状下易导致缺陷，需自动化鲁棒优化方法。

Method: 双环控制：层内线性输出反馈+策略梯度优化，层间温度轨迹优化+迭代学习前馈控制。

Result: 仿真：模型失配和噪声下温度稳定；实验简化版：跟踪误差降低3.4%，约束违反降低47.5%，离线调参实现。

Conclusion: 首次成功在LPBF中应用sim-to-real策略优化，并发现高频激励减少向量头肿胀的新现象。
...
### [455] [Topology-Aware Propagation-Based Assessment of Extreme-Weather Impacts on Distribution System Resilience](https://arxiv.org/abs/2607.17565)
*Junjie Yin,Xinyu Feng,Jian Han,Fangxing Li*

Main category: eess.SY

TL;DR: 提出了一种面向暴雨事件的条件不确定性建模与拓扑感知影响传播框架，用于评估配电网运行影响。


<details>
  <summary>Details</summary>
Motivation: 极端天气和分布式能源接入带来不确定性与韧性挑战，现有方法缺乏拓扑感知和不确定性下的运行影响评估。

Method: 集成概率事件跟踪建模、分支级故障筛选、下游影响传播分析和运行影响评估的统一框架。

Result: 在IEEE 33节点系统上验证了框架可区分地理暴露与拓扑依赖的运行影响，支持渐进式预警评估。

Conclusion: 该框架能有效应对极端天气不确定性，为配电网韧性提升提供支持。
...
### [456] [Online learning of neural state-space models](https://arxiv.org/abs/2607.17614)
*Bendegúz Györök,Tamás Péni,Maarten Schoukens,Roland Tóth*

Main category: eess.SY

TL;DR: 提出了一种基于子空间编码器的人工神经网络状态空间模型的批量学习与直接递归辨识算法，实现了高效的在线自适应。


<details>
  <summary>Details</summary>
Motivation: 基于编码器的ANN-SS模型在离线设置中表现出色，但在线学习尚未被充分探索。

Method: 设计了批量学习流程和直接递归辨识算法，并进行了收敛性分析。

Result: 该方法实现了计算高效的在线自适应，且模型精度高。

Conclusion: 所提方法有效支持ANN-SS模型的在线辨识与更新。
...
### [457] [On Optimal Event-Triggered Distributed Control for Stochastic Multi-Agent Systems via Reinforcement Learning](https://arxiv.org/abs/2607.17635)
*Ziming Wang,Bingbing Li,Karl H. Johansson,Apostolos I. Rikos*

Main category: eess.SY

TL;DR: 提出了一种基于强化学习的最优分布式控制算法，用于处理具有随机不确定性的多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多智能体系统中的随机不确定性和控制优化方面存在不足。

Method: 使用执行器-评价器-辨识器结构，通过神经网络分别处理控制行为、性能评估和不确定性；采用低通滤波器和混合事件触发控制策略。

Result: 所有误差有界，实现领导者与跟随者之间的精确跟踪；在单轴机器人仿真中验证了有效性，并相比非最优算法具有优势。

Conclusion: 该算法能有效处理随机不确定性，实现最优控制，提高系统稳定性并降低控制频率。
...
### [458] [Graph-Induced Tensor Liftings for Networked SEIR Models: Dimensional Reduction and Residual Analysis](https://arxiv.org/abs/2607.17664)
*Enrique Baeyens*

Main category: eess.SY

TL;DR: 提出一种图诱导的张量提升方法，通过选择性保留有效传播支撑的观测变量，降低网络化SEIR模型线性高维表示的维度，并提供结构化降阶建模基础。


<details>
  <summary>Details</summary>
Motivation: 标准多项式提升因保留传输图中不存在的交互导致维度快速增长，需发展基于图支持的降维方法。

Method: 构建基于边的二次表示分离线性与非线性项，递归构造齐次层级，利用线性场饱和字典，并通过截断残差控制高阶项。

Result: 一阶提升维度与子种群数和有效传输通道数线性相关；在均匀有界局部连接下，图诱导字典维度与网络规模线性增长，而完全多项式提升呈多项式增长。

Conclusion: 图诱导提升可显著降低表示维度，残余轨迹反映权重累积与非线性传播，为后续模型分析与控制提供结构化框架。
...
### [459] [Multisine electrochemical impedance spectroscopy for Li-ion battery characterisation](https://arxiv.org/abs/2607.17832)
*Noël Hallemans,Heiko Seel-Mayer,Peter Keil,Stephen R. Duncan,David A. Howey*

Main category: eess.SY

TL;DR: 本文展示了在商用恒电位仪上使用多正弦激励在宽频率范围内（20 mHz至1 kHz）对锂离子电池进行高保真电化学阻抗谱(EIS)测量，并与单正弦EIS进行了比较。


<details>
  <summary>Details</summary>
Motivation: 为了缩短实验时间并能在操作条件下（如充电、放电、弛豫和温度变化）测量阻抗，多正弦激励替代单正弦激励具有优势，但需验证其线性和平稳性条件。

Method: 使用商用恒电位仪进行多正弦EIS和单正弦EIS测量，覆盖20 mHz至1 kHz频率范围，并比较两种技术。

Result: 多正弦EIS在不同操作条件下（充电/放电、弛豫、温度变化）提供了对电池动力学、材料特性、电荷转移过程和热性能的新见解。

Conclusion: 多正弦EIS是一种有效的工具，可在较短时间和动态条件下提供高保真阻抗数据，揭示电池内在过程。
...
### [460] [A Compact 3D-Printed Soft Finger with Cyclic Hydraulic Actuation](https://arxiv.org/abs/2607.17840)
*Zefang Mao,Yunjia Li,Victoria Bamgboye,Sara Ben Haj Hammouda,Aika Ono,Wen Fan,Chao Wu,Dandan Zhang*

Main category: eess.SY

TL;DR: 提出一种紧凑型3D打印软体液压手指，通过微型蠕动泵驱动，集成有限元分析与闭环控制，实现柔性抓取。


<details>
  <summary>Details</summary>
Motivation: 现有液压软体手指因流体硬件庞大、制造复杂及设计验证不足而难以实际应用。

Method: 采用3D打印集成波纹管、刚性连接件和流体通道；通过Abaqus流固耦合模型优化壁厚、螺距角和长度；结合基线校正压力测量和视觉角度追踪进行验证。

Result: 准静态有限元模型准确捕捉压力-角度趋势，视觉反馈控制实现大范围重复角度跟踪，抓取豆腐、蓝莓等易碎物体无损伤。

Conclusion: 建立了从有限元导引设计到闭环验证的可复现制造流程，推动了紧凑型软体液压手指的实际应用。
...
### [461] [An Augmented Load-Frequency Control Block Diagram for Power Systems](https://arxiv.org/abs/2607.17873)
*Mahyar Tofighi-Milani,Sajjad Fattaheian-Dehkordi,Matti Lehtonen*

Main category: eess.SY

TL;DR: 通过引入负荷侧动态特性，提出一种增强型负荷模型，以提高低惯量电力系统频率响应分析的精度。


<details>
  <summary>Details</summary>
Motivation: 逆变器资源渗透率增加导致系统惯量降低，频率偏差更快速、显著，传统静态负荷模型无法准确反映系统动态行为。

Method: 在传统负荷频率控制（LFC）模型中集成负荷侧动态特性，构建增强型负荷模型，并与静态负荷模型进行对比验证。

Result: 增强型模型能更准确揭示负荷动态对频率响应的影响，而静态模型可能导致不准确的分析结论。

Conclusion: 准确建模负荷侧动态对于现代低惯量电力系统的频率稳定性评估至关重要。
...
### [462] [Real-Time Flight Test Maneuver Selection with Monte Carlo Tree Search](https://arxiv.org/abs/2607.18089)
*Nicholas E. Bostock,Helen Pruitt-Kennett,Marc R. Schlichting,Mykel J. Kochenderfer*

Main category: eess.SY

TL;DR: 提出结合高斯过程和蒙特卡洛树搜索的实时飞行测试规划框架，在燃料约束下排序机动动作，相比贪心策略和固定任务卡基线，显著提升单位燃料的不确定性降低效率。


<details>
  <summary>Details</summary>
Motivation: 飞行测试向数据驱动转变，需在资源限制下最大化不确定性降低，现有预编排测试点方法效率不足。

Method: 使用高斯过程构建信念模型，通过蒙特卡洛树搜索选择机动动作；采用加权积分方差降低评估候选方案，并用传播方差状态进行浅层前瞻。

Result: 在X-Plane人机回环仿真中，MCTS-wIVR在单位燃料不确定性降低上显著优于贪心wIVR和固定任务卡基线。

Conclusion: 后验感知的自适应规划是提升飞行测试效率的有前景的方法。
...
### [463] [LLMs and Agentic AI Systems for Smart Grids: A Tutorial on Architectures and Applications](https://arxiv.org/abs/2607.18147)
...
TL;DR: 本文提出了基于求解器的设计原则，确保智能电网中LLM和AI代理系统的数值结果来自可信工具并通过验证。通过四个案例研究，展示了该方法在减少不可行输出、提高任务成功率方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在智能电网中的应用缺乏统一设计和评估方法，模型易产生物理不可行输出，评估协议不统一，模型与工具的责任边界模糊。

Method: 提出求解器接地设计原则，要求数值结果必须源于可信工具并通过显式验证。采用提示策略和代理架构，在四个案例中对比纯LLM基线与求解器接地版本。

Result: EVAgent减少纯LLM未满足能量7.5-9.5倍，GridDebugAgent修复17/39个事故并减少52.3%违规。提出四组评估框架。

Conclusion: 代理系统负责编排、检索和解释，可信工具负责计算，验证门控决定报告内容，形成一致的职责分工。
...
### [464] [Luminosity-Adaptive Contrast Enhancement Using CLAHE for Retinal Fundus Images with Quantitative Validation and Comparative Analysis](https://arxiv.org/abs/2607.17691)
*K. Mithra,Prem Kumar Santhanam*

Main category: eess.IV

TL;DR: 提出一种结合HSV亮度校正和CLAHE的眼底图像增强两阶段流程，在DRIVE数据集上优于传统方法，处理速度快。


<details>
  <summary>Details</summary>
Motivation: 眼底图像常因光照不均、运动模糊和对比度低影响诊断，需要有效增强方法。

Method: 两阶段增强流程：先用HSV分解进行亮度校正，再在V通道应用CLAHE，最后用二值掩膜提取高反射区域。

Result: PSNR 29.3 dB, SSIM 0.91, CNR 3.12，优于HE和AHE，处理时间0.14秒/图。

Conclusion: 所提方法在对比度和结构保真度上优于基线，速度适合临床筛查，未提及深度学习比较的局限性。
...
### [465] [A Unified Multisensor Machine-Learning Framework for Live Fuel Moisture Content Retrieval](https://arxiv.org/abs/2607.16714)
*Valerio Pampanoni,Emilio Chuvieco,Alvise Ferrari,Giovanni Laneve,Simone Saquella*

Main category: eess.IV

TL;DR: 提出一个统一机器学习框架，利用卫星、气象和地形数据估算全球植被燃料含水量，在草地、灌木和树木上分别达到R²约0.7。


<details>
  <summary>Details</summary>
Motivation: 燃料含水量控制植被易燃性，对火灾管理至关重要，但现有观测集中在特定区域，难以在大范围上准确估算和成图。

Method: 将GlobeLFMC 2.0数据与多源卫星反射率产品匹配，统一红、近红外和短波红外光谱空间；筛选站点-产品组合和时序；进行光谱响应函数诊断和反射率校正；使用随机森林和XGBoost对草地、灌木和树木分别建模。

Result: 在主验证设计中，草地、灌木和树木的最佳模型R²分别为0.715、0.693和0.700。

Conclusion: 该框架可扩展至更多光学传感器，为全球燃料湿度估算提供了可行方案。
...
### [466] [When Do Multimodal and Graph-Augmented RAG Help? A Controlled Evaluation for Document Question Answering](https://arxiv.org/abs/2607.16604)
*Sokipriala Jonah*

Main category: eess.IV

TL;DR: 提出可解释的多模态图RAG架构，通过消融实验发现图文增强效果依赖于检索设计、语料结构等，且图片问答准确率很低。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统仅处理文本，丢失了图表、布局等跨段落关系信息。

Method: 在文本基线基础上，加入LLM提取的三元组和CLIP检索的图表，三个证据源独立检索后在生成时融合。在1000页PubLayNet数据上进行四路消融实验，对比两种闭源和两种开源多模态生成器。

Result: 知识图谱增强未稳定提升准确率；纯像素问题中文本系统准确率为0，多模态系统仅0.057-0.114；图片检索Recall@3仅0.371；图片标记化导致输入token数有11倍差异。

Conclusion: 多模态和图谱增强的价值取决于检索设计、语料结构、基准构建和生成器效率，当前方法在图表问答上仍存在显著局限。
