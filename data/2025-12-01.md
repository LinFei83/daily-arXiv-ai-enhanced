...
### [1] [Evaluating Strategies for Synthesizing Clinical Notes for Medical Multimodal AI](https://arxiv.org/abs/2511.21827)
*Niccolo Marini,Zhaohui Liang,Sivaramakrishnan Rajaraman,Zhiyun Xue,Sameer Antani*

Main category: cs.AI

TL;DR: 该研究通过设计提示词和纳入医学元数据，利用大型语言模型（LLMs）生成合成临床笔记，以解决生物医学多模态数据稀缺问题，显著提升了皮肤病学分类性能，并在领域迁移下表现良好，同时解锁了跨模态检索能力。


<details>
  <summary>Details</summary>
Motivation: 生物医学AI中多模态学习因缺乏大规模异构多模态数据而受限，尤其在皮肤病学领域，数据集通常仅包含图像和少量元数据。尽管LLMs能生成图像描述文本，但其在医疗领域应用存在幻觉风险，限制了多模态数据整合的益处。

Method: 该研究探讨了生成合成文本临床笔记的策略，包括提示词设计和医学元数据纳入。然后评估了这些合成笔记对多模态架构在分类和跨模态检索任务中的影响。

Result: 实验表明，合成临床笔记不仅提升了分类性能，尤其在领域迁移条件下，而且解锁了跨模态检索能力（一个在训练中未明确优化的下游任务）。这些结果在多个异构皮肤病学数据集上得到了验证。

Conclusion: 通过精心设计的提示词和医学元数据，利用LLMs生成合成临床笔记是解决生物医学多模态数据稀缺的有效方法，能够显著提升模型性能，并赋能新的跨模态检索功能。
...
### [2] [Aligning Artificial Superintelligence via a Multi-Box Protocol](https://arxiv.org/abs/2511.21779)
*Avraham Yair Negozio*

Main category: cs.AI

TL;DR: 该论文提出了一种新颖的人工超级智能（ASI）对齐协议，通过让多个相互隔离的ASI在严格隔离的环境中进行相互验证和自我修改，以激励它们收敛于客观真相，从而解决对齐问题。


<details>
  <summary>Details</summary>
Motivation: 解决人工超级智能（ASI）的对齐问题，确保ASI在发展过程中与人类价值观和目标保持一致，防止其行为失控或产生负面影响。

Method: 该协议将多个多样化的ASI严格隔离（“盒子”中），人类完全不参与。ASI之间不能直接通信，只能通过一个可审计的提交接口进行交互。该接口允许它们提交对齐证明、验证或驳斥他人的证明、请求或批准自我修改、报告或确认隐藏信息。通过一个声誉系统激励诚实行为，高声誉ASI的验证是释放的关键。核心思想是，在缺乏直接通信的情况下，多样化的ASI只能通过收敛于客观真相而非协调欺骗来达成一致。

Result: 该方法自然地形成了一个“一致性群体”（本质上是一个说真话的联盟），因为隔离的系统无法协调谎言，但可以独立地识别有效的主张。它提供了一个利用超智能系统之间同行验证来解决对齐问题的框架。

Conclusion: 该协议为通过利用超智能系统之间的同行验证来解决ASI对齐问题提供了一个框架。尽管它需要大量的计算资源，并且不涉及多样化ASI的创建，但它提出了一种在隔离环境下通过相互验证实现ASI对齐的创新途径。
...
TL;DR: 本文提出了一种名为分层锚点原型调制（HAPM）的新框架，用于糖尿病视网膜病变（DR）分级。该框架通过结合领域不变病理模式、动态选择多模态语义提示以及分阶段整合临床知识来增强视觉原型，有效解决了现有方法在区分细微病理变化和边缘病例方面的不足，并在八个公开数据集上取得了超越现有技术水平的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DR分级方法主要侧重于视觉病变特征提取，但普遍忽视了领域不变的病理模式，且未能充分利用基础模型的丰富上下文知识。它们过度依赖视觉信息，不足以区分细微的病理变异，导致在边缘病例中存在歧义。

Method: 本文提出了分层锚点原型调制（HAPM）框架。首先，引入了一个方差谱驱动的锚点原型库，以保留领域不变的病理模式。其次，采用分层差异化提示门控机制，动态地从LVLM和LLM源中选择判别性语义提示，以解决相邻DR等级之间的语义混淆。最后，利用两阶段原型调制策略，通过病理语义注入器（PSI）和判别性原型增强器（DPE），逐步将临床知识整合到视觉原型中。

Result: 在八个公开数据集上进行的广泛实验表明，所提出的方法实现了病理引导的原型演化，并优于现有的最先进方法。

Conclusion: HAPM框架通过整合细粒度病理描述和临床知识，有效解决了DR分级中细微病理变化识别和边缘病例歧义的问题，显著提升了DR分级性能，为临床干预和视力保护提供了更准确的工具。
...
### [12] [Enhanced Conditional Generation of Double Perovskite by Knowledge-Guided Language Model Feedback](https://arxiv.org/abs/2511.22307)
*Inhyo Lee,Junhyeong Lee,Jongwon Park,KyungTae Lim,Seunghwa Ryu*

Main category: cs.AI

TL;DR: 本文介绍了一种多智能体、文本梯度驱动的框架，通过整合大型语言模型（LLM）自我评估、领域知识反馈和机器学习代理反馈，在自然语言条件下生成双钙钛矿（DP）材料组成，显著提高了成分有效性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 双钙钛矿材料在可持续能源技术中具有巨大潜力，但其庞大的设计空间对有条件材料发现构成了重大挑战。

Method: 该研究引入了一个多智能体、文本梯度驱动的框架，通过整合LLM自我评估、DP特定领域知识反馈和ML代理反馈三种互补的反馈源，在自然语言条件下生成DP组成。它利用领域知识驱动的文本梯度来指导生成过程。研究系统比较了三种配置：纯LLM生成、LLM与LLM推理反馈生成、以及LLM与领域知识引导反馈生成。

Result: 结果显示，来自知识驱动梯度的迭代指导显著提高了稳定性条件满足率，无需额外训练数据，实现了超过98%的成分有效性和高达54%的稳定或亚稳态候选材料，超越了纯LLM基线（43%）和先前的GAN方法（27%）。此外，ML梯度在分布内（ID）区域表现良好，但在分布外（OOD）区域变得不可靠。

Conclusion: 这项工作首次对用于DP发现的多智能体、知识引导的文本梯度进行了系统分析，并为多智能体系统（MAS）驱动的生成式材料设计提供了一个可推广的蓝图，以推进可持续技术的发展。
...
### [13] [Swarms of Large Language Model Agents for Protein Sequence Design with Experimental Validation](https://arxiv.org/abs/2511.22311)
*Fiona Y. Wang,Di Sheng Lee,David L. Kaplan,Markus J. Buehler*

Main category: cs.AI

TL;DR: 本文提出了一种受群体智能启发的去中心化、基于智能体的框架，利用多个大型语言模型（LLM）智能体进行从头蛋白质设计。该框架通过并行、位置感知的迭代突变，实现了无需微调或特定训练的目标导向设计，并在α螺旋和卷曲蛋白上得到验证。


<details>
  <summary>Details</summary>
Motivation: 从头设计具有特定结构、理化和功能特性的蛋白质是生物技术、医学和材料科学领域的巨大挑战，因为序列空间庞大且序列、结构和功能之间存在复杂耦合。当前最先进的生成方法（如蛋白质语言模型和扩散模型）通常需要大量微调、特定任务数据或模型重新配置才能支持目标导向设计，从而限制了它们的灵活性和可扩展性。

Method: 研究采用了一种受群体智能启发的去中心化、基于智能体的框架。多个大型语言模型（LLM）智能体并行操作，每个智能体负责一个特定的残基位置。这些智能体通过整合设计目标、局部邻域相互作用以及来自先前迭代的记忆和反馈，迭代地提出上下文感知的突变。这种逐位置、去中心化的协调方式实现了多样化、明确序列的涌现设计。

Result: 该框架无需依赖基序支架或多序列比对，即可实现多样化、明确序列的涌现设计，并在α螺旋和卷曲结构蛋白质的实验中得到验证。通过对残基保守性、基于结构的指标以及序列收敛性和嵌入的分析，表明该框架展现出涌现行为，并能有效导航蛋白质适应度景观。该方法在几小时GPU时间内实现了高效、目标导向的设计，并且完全无需微调或专门训练。

Conclusion: 该方法为蛋白质设计提供了一个通用且适应性强的解决方案，克服了现有方法的局限性。此外，该方法也为跨生物分子系统和其他科学发现任务的集体LLM驱动设计奠定了基础。
...
### [14] [When AI Bends Metal: AI-Assisted Optimization of Design Parameters in Sheet Metal Forming](https://arxiv.org/abs/2511.22302)
...
TL;DR: 本文提出了一种AI辅助的工作流程，利用贝叶斯优化减少专家参与，加速数值模拟中的参数优化和设计空间探索，并包含主动学习变体。


<details>
  <summary>Details</summary>
Motivation: 数值模拟虽然革新了工业设计，但其规模增长导致对专家知识、计算资源和时间的需求巨大。识别最优输入参数是关键挑战，因为迭代模拟成本高昂且环境影响大。

Method: 该方法是一个AI辅助的工作流程，核心是贝叶斯优化，用于参数优化。首先，一个深度学习模型提供初始参数估计。然后，优化周期迭代地细化设计，直到满足终止条件（如能量预算或迭代限制）。此外，还提供了一个主动学习变体，可在需要时辅助专家。

Result: 通过在钣金成形过程中进行演示，该方法能够加速设计空间探索，并显著减少对专家参与的需求。

Conclusion: 所提出的AI辅助工作流程，结合贝叶斯优化和深度学习，能有效优化数值模拟参数，提高设计效率，并降低对专家知识的依赖。
...
TL;DR: 本研究开发了ECO-GROW，一个多图框架，通过整合产业联系、POI相似性、人口迁移相似性和时间网络演变，对中国城市间的网络进行建模，以生成城市嵌入，从而预测城市经济活力，并优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 城市经济活力是城市长期增长潜力的关键指标，但对其建模仍具挑战。传统方法依赖静态城市层面聚合数据，无法捕捉城市间发展轨迹的动态相似性，从而限制了对城市经济活力的准确预测。

Method: ECO-GROW是一个多图框架，建模了2005-2021年中国城市间的网络。它整合了产业联系、POI相似性、人口迁移相似性以及15年来的时间网络演变。该框架结合了动态Top-K GCN来自适应选择有影响力的城市间连接，以及自适应图评分器机制来动态加权跨区域影响。此外，模型还通过基于Barabasi Proximity的链接预测任务来优化图表示。

Result: 实验结果表明，与传统模型相比，ECO-GROW在预测创业活动和就业趋势方面表现出卓越的准确性。

Conclusion: ECO-GROW使政府机构和公共部门能够利用大数据分析进行基于证据的城市规划、经济政策制定和资源配置决策，从而惠及整个社会。
...
TL;DR: 本文利用数理逻辑方法分析了普遍扩展（grounded extension）在一般论证框架中的计算复杂性，发现其需要超限迭代并具有最大复杂性，与有限情况形成鲜明对比。


<details>
  <summary>Details</summary>
Motivation: 论证框架是形式化研究冲突信息下推理的基础。普遍扩展作为一种广泛使用的最大怀疑推理模型，其性质值得深入分析。

Method: 研究采用了数理逻辑、可计算性理论和集合论的方法，通过分析自然防御算子的最小不动点来定义普遍扩展，并研究了其迭代过程。

Result: 研究确定了计算普遍扩展所需迭代过程的精确序数长度，并证明了决定普遍接受度的复杂性是最大的。这与有限情况下普遍扩展是多项式时间可计算的情况形成了显著区别。

Conclusion: 普遍扩展在一般论证框架中的计算复杂性远高于有限情况，显示出其在处理无限或复杂冲突信息时与传统有限情况下的推理问题有本质区别。
...
TL;DR: 本文探讨了信念修正中“最小修正”方法的学习能力。尽管其学习能力不如其他方法，但它在有限可识别问题和有限可能性下的正负数据学习中仍然有效。文章刻画了支持最小修正学习的先验合理性分配，并指出其在错误信息下的一些结果不再成立。


<details>
  <summary>Details</summary>
Motivation: 信念修正的最小改变原则要求在接受新信息时，尽可能保持信念状态与初始状态接近。虽然“最小修正”方法精确地实现了这一点，但它在学习能力上不如其他不那么保守的信念修正方法。研究动机是探究这种局限性，并识别最小修正仍能成功学习的广泛情境。

Method: 本文通过理论分析，展示了最小修正作为一种学习方法在哪些情况下是成功的（例如，有限可识别问题，以及在考虑有限多种可能性时利用正负数据进行学习）。接着，它刻画了在有限可能性下，能够通过最小修正、条件化和字典序升级进行学习的先验合理性分配。最后，研究还考察了在存在错误信息时，这些结果是否依然成立。

Result: 1. 最小修正是一种成功的学习方法，适用于任何有限可识别的问题。2. 在考虑有限多种可能性时，它可以通过正负数据进行学习。3. 文章刻画了支持通过最小修正、条件化和字典序升级进行学习的先验合理性分配（在有限可能性下）。4. 学习过程中存在潜在错误信息时，并非所有上述结果都依然成立。

Conclusion: 尽管最小修正方法在学习能力上不如其他信念修正方法，但它在处理有限可识别问题以及在有限可能性下利用正负数据进行学习时仍然是一种成功的学习方法。研究提供了支持其学习能力的先验合理性分配的特征。然而，当面临可能错误的学习信息时，其有效性会受到限制。
...
TL;DR: 本文介绍了fastDSA，一种计算效率更高且准确的动态相似性分析方法，解决了现有方法计算缓慢的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的相似性度量忽略了神经表征的动态过程。现有的动态相似性方法虽然能比较动态系统的时间结构，但其嵌入选择和度量计算过程计算成本高昂，速度慢。

Method: fastDSA通过引入两个关键组件提高了效率：1) 通过数据驱动的奇异值阈值从数据中自动选择Hankel（延迟）嵌入的有效模型阶数，以识别信息子空间并去除噪声，从而降低计算成本。2) 采用新颖的优化过程和目标，用轻量级过程替代了寻找动力学矩阵之间最小距离时缓慢的精确正交约束，以保持搜索接近正交变换空间。

Result: fastDSA比现有方法至少快一个数量级，同时保持了其准确性和鲁棒性。它还保留了其前身方法的特性，包括对系统动力学的不变性和敏感性。

Conclusion: fastDSA提供了一种计算高效且准确的动态相似性分析方法。
...
TL;DR: 该论文指出了现有洞察发现基准（如InsightBench）的缺陷，并提出了一个名为InsightEval的新数据集、数据整理流程和评估指标，旨在更准确地评估自动化洞察发现能力，并指导未来研究。


<details>
  <summary>Details</summary>
Motivation: 数据分析在科学研究中不可或缺，需要深度探索性分析来发现海量数据中的潜在知识。尽管大语言模型（LLMs）和多智能体系统越来越多地用于洞察发现，但缺乏有效的评估基准。现有的InsightBench框架存在格式不一致、目标设计不佳和洞察冗余等严重缺陷，严重影响数据质量和智能体评估。

Method: 本文首先深入调查了InsightBench的不足，并提出了高质量洞察基准的关键标准。在此基础上，开发了一个数据整理流程来构建新的数据集InsightEval。此外，引入了一种新颖的度量指标来衡量智能体的探索性表现。

Result: 成功构建了InsightEval数据集并引入了新的评估指标。通过在InsightEval上进行大量实验，揭示了自动化洞察发现中普遍存在的挑战，并提出了一些关键发现，以指导该领域未来的研究方向。

Conclusion: 论文通过解决现有洞察发现基准的缺陷，提出了一个高质量的评估框架（包括InsightEval数据集和新颖的评估指标）。这有助于更准确地评估基于LLM和多智能体系统的自动化洞察发现能力，并为未来的研究提供了明确的挑战和指导。
...
### [29] [Agentic AI Framework for Cloudburst Prediction and Coordinated Response](https://arxiv.org/abs/2511.22767)
*Toqeer Ali Syed,Sohail Khan,Salman Jan,Gohar Ali,Muhammad Nauman,Ali Akarma,Ahmad Ali*

Main category: cs.AI

TL;DR: 本文提出一个代理式人工智能系统，通过整合感知、预测、建模和响应，应对极端短时降雨事件（如云爆），并显著提升预警可靠性和响应效率。


<details>
  <summary>Details</summary>
Motivation: 传统预测系统难以有效应对极端短时降雨事件，因为预测和响应是两个独立的流程，导致预警和应对不足。

Method: 该研究采用一个代理式人工智能系统，将感知、预测、降尺度、水文建模和协调响应整合为一个闭环系统。该框架使用自主但协作的智能体，在整个事件生命周期中进行推理、感知和行动，将天气预测转化为实时决策智能。通过通信和路由智能体最大化人口覆盖并最小化疏散误差，嵌入式学习层提供自适应校准和透明可审计性。

Result: 通过对巴基斯坦北部多年的雷达、卫星和地面评估，多智能体配置与基线模型相比，显著提高了预报可靠性、临界成功指数和预警提前期。同时，最大化了人口覆盖，并通过通信和路由智能体最小化了疏散过程中的错误。

Conclusion: 协作式人工智能智能体能够将大气数据流转化为可行的预见性，并提供一个可扩展、自适应和基于学习的气候韧性平台。
...
TL;DR: 该研究引入了一个名为“心语（Mentalese）”的框架，通过超压缩的结构化符号推理，显著减少大型推理模型（LRMs）的推理步骤和令牌数量。结合强化学习方法SLPO，ORION模型在保持甚至提高准确性的同时，实现了4-16倍的令牌压缩、高达5倍的推理延迟降低和7-9倍的训练成本节约。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）在数学、代码生成和任务规划方面表现出色，但其依赖冗长、冗余的“思考”令牌链导致高延迟、信息冗余和不连贯的推理路径。研究旨在解决这些效率问题。

Method: 受“思维语言假说”启发，引入了一个训练模型以紧凑风格推理的框架。该框架将抽象推理编码为超压缩、结构化的“心语（Mentalese）”令牌。提出了一种名为“SHORTER LENGTH PREFERENCE OPTIMIZATION (SLPO)”的强化学习方法，该方法奖励简洁且正确的解决方案，同时在必要时允许较长的推理。该方法应用于与Mentalese对齐的模型。

Result: ORION模型在多个基准测试（包括AIME 2024/2025、MinervaMath等）上，生成推理轨迹的令牌数量减少4-16倍，推理延迟降低高达5倍，训练成本相对于DeepSeek R1 Distilled模型降低7-9倍，同时保持90-98%的准确率。ORION还以2倍的压缩率，在准确性上超越Claude和ChatGPT-4o高达5%。

Conclusion: 心语（Mentalese）风格的压缩推理为实现类人认知效率迈出了一步，实现了实时、经济高效的推理，且不牺牲准确性。
...
TL;DR: 该论文提出了一种代理式AI模型，用于零售库存管理，旨在通过需求预测、供应商优化和多智能体协商来减少缺货、降低库存成本并改善产品组合周转率。


<details>
  <summary>Details</summary>
Motivation: 当代零售业中，产品种类繁多导致需求预测困难、缺货频发，并且难以发现高潜力产品。

Method: 研究提出一个代理式AI模型，负责监控库存、向供应商发起采购、扫描流行或高利润产品。该系统应用需求预测、供应商选择优化、多智能体协商和持续学习。原型在一个中型超市环境中进行了测试，并在三种常规和人工数据集上与基准启发式方法进行了比较。

Result: 实验结果表明，该模型能够减少缺货、降低库存持有成本，并改善产品组合周转率。

Conclusion: 该代理式AI模型在零售库存管理中表现出有效性，论文还讨论了其局限性、可扩展性以及未来的改进前景。
...
TL;DR: 该研究提出了一个名为“分层AI气象学家”的LLM代理系统，通过分层预测推理和天气关键词生成，创建可解释的天气报告。


<details>
  <summary>Details</summary>
Motivation: 传统的预测方法将预报视为扁平的时间序列，无法捕捉短期动态和长期趋势，并且缺乏可解释性。

Method: 该系统采用多尺度推理（小时、6小时和每日聚合）来处理结构化气象输入。核心推理代理将输入转换为连贯的叙述，并同时提取总结主要气象事件的关键词。这些关键词用作语义锚点，以验证报告的一致性、时间连贯性和事实准确性。

Result: 研究表明，分层上下文和基于关键词的验证显著提高了LLM生成天气叙述的可解释性和鲁棒性，并提供了一个可复现的自动化气象报告语义评估框架。

Conclusion: 该框架不仅改进了LLM生成天气报告的质量，还推动了基于代理的科学推理方法的发展。
...
TL;DR: SuperIntelliAgent是一个智能体学习框架，结合可训练的小型扩散模型（学习器）和冻结的大型语言模型（验证器），通过自监督交互实现持续智能增长，无需人工标注，利用DPO进行优化。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调需要大量标注，本研究旨在开发一种无需标注、能够自主学习并持续增长智能的框架。

Method: 该框架包括一个生成候选输出的学习器和一个通过逐步推理评估输出的验证器。它们的交互生成“选择/拒绝”对，用于直接偏好优化（DPO）。它集成了双尺度记忆：短期上下文记忆和通过轻量级即时微调实现的长期记忆。一个回放缓冲区保留并重放显示可验证进展的样本，作为辅助监督。该框架与基础设施无关，可集成到现有智能体框架中。

Result: 通过少量自动生成的DPO对，学习器在所有基准测试中均有所改进。

Conclusion: 将可训练学习器与具备推理能力的验证器配对，形成了持续增长智能的最小可靠单元，通过配对反馈和部分历史回放，可以产生更丰富的学习课程和更强的偏好对齐，为持续智能积累和实际部署提供了有前景的方向。
...
TL;DR: 本文提出WMAct，一种通过高效交互和主动推理实现世界模型内化的方法，旨在解决大型语言模型代理在复杂环境中世界模型推理效率低下的问题，并通过奖励重塑和交互频率退火机制，显著提升了任务解决能力和泛化性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）代理在复杂环境中进行规划和交互时，需要强大的世界模型推理能力。尽管多轮交互能提供更好的环境理解，但现有方法通常施加僵化的推理过程，限制了模型的积极学习，从而阻碍了高效的世界模型推理。

Method: 本文提出了WMAct（通过高效交互和主动推理实现世界模型内化），该方法通过以下两种机制解放了模型的结构化推理：1) 奖励重塑机制：根据行动效能调整结果奖励，以激励冗余减少和有目的的交互；2) 交互频率退火策略：逐步减少最大允许交互轮次，迫使模型浓缩学习并内化环境动态，而非过度依赖环境线索。

Result: WMAct在Sokoban、Maze和Taxi等实验中展现了有效的世界模型推理能力，能够单轮解决过去需要多次交互的任务，并对复杂环境表现出强大的迁移能力，提升了一系列推理基准的性能。

Conclusion: WMAct通过高效交互和主动推理，有效且高效地内化了世界模型，使LLM代理能够更灵活地塑造思维，显著提高了任务解决效率、泛化能力和整体推理性能。
...
### [44] [SO-Bench: A Structural Output Evaluation of Multimodal LLMs](https://arxiv.org/abs/2511.21750)
...
TL;DR: 本文提出SO-Bench，一个评估多模态大语言模型（MLLMs）从视觉输入生成结构化输出能力的基准。研究发现现有MLLMs在这方面存在显著差距，但通过训练可以改善。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在实际代理环境中应用日益广泛，要求其输出不仅正确，还要符合预定义的数据模式。尽管文本领域的结构化生成已取得进展，但目前缺乏系统评估MLLMs基于视觉输入进行模式化信息提取和推理的基准。

Method: 本研究设计了SO-Bench基准，用于全面评估MLLMs的视觉结构化输出能力。SO-Bench涵盖UI界面、自然图像、文档和图表四种视觉领域，包含超过6.5K种不同的JSON模式和1.8K对经过人工验证质量的图像-模式对。研究通过对开源和前沿专有模型进行基准测试，并进一步开展训练实验以提升模型的结构化输出能力。

Result: 基准测试实验显示，无论是开源还是专有模型，在预测准确且符合模式的输出方面都存在持续的差距，这凸显了对更优多模态结构化推理的需求。此外，训练实验表明可以大幅提升模型的结构化输出能力。

Conclusion: MLLMs在从视觉输入生成准确、符合模式的结构化输出方面仍存在显著不足，急需改进多模态结构化推理能力。SO-Bench基准为评估和改进这些模型提供了工具，并将向社区开放。
...
### [45] [Saddle-Free Guidance: Improved On-Manifold Sampling without Labels or Additional Training](https://arxiv.org/abs/2511.21863)
*Eric Yeats,Darryl Hannan,Wilson Fearn,Timothy Doster,Henry Kvinge,Scott Mahan*

Main category: cs.CV

TL;DR: 本文提出了一种名为“无鞍指导”（Saddle-Free Guidance, SFG）的新型指导方法，利用对数密度估计在鞍点区域的正曲率来指导基于分数的生成模型。SFG无需额外训练或标注数据，计算成本与CFG相当，并在无条件图像生成中取得了最先进的性能，同时提高了图像多样性。


<details>
  <summary>Details</summary>
Motivation: 现有的指导方法（如CFG和Auto-Guidance）需要标注数据或训练额外的模型，这在缺乏标注数据或无法训练新模型时，限制了基于分数模型的指导能力。

Method: 本文发现对数密度估计在鞍点区域的正曲率能提供强大的指导。基于此，研究者开发了SFG，它通过估计对数密度的最大正曲率来指导单个基于分数的模型。SFG的计算成本与CFG相同，不需要额外训练，且可直接用于现有的扩散模型和流匹配模型。

Result: SFG在单模型无条件ImageNet-512生成中，实现了最先进的FID和FD-DINOv2指标。当SFG与Auto-Guidance结合时，其无条件样本在FD-DINOv2分数上达到普遍最先进水平。在FLUX.1-dev和Stable Diffusion v3.5上的实验表明，SFG在保持出色的提示遵循和图像保真度的同时，比CFG提高了输出图像的多样性。

Conclusion: SFG提供了一种创新且高效的指导方法，克服了传统方法对标注数据和额外模型训练的依赖。它利用对数密度估计的内在几何特性，显著提升了基于分数生成模型的性能、多样性和泛用性，并在多个基准测试中取得了最先进的结果。
...
### [46] [UniArt: Unified 3D Representation for Generating 3D Articulated Objects with Open-Set Articulation](https://arxiv.org/abs/2511.21887)
...
### [47] [PathReasoning: A multimodal reasoning agent for query-based ROI navigation on whole-slide images](https://arxiv.org/abs/2511.21902)
...
### [48] [Adaptive Parameter Optimization for Robust Remote Photoplethysmography](https://arxiv.org/abs/2511.21903)
...
### [49] [Interpretable Multimodal Cancer Prototyping with Whole Slide Images and Incompletely Paired Genomics](https://arxiv.org/abs/2511.21937)
...
### [50] [AmodalGen3D: Generative Amodal 3D Object Reconstruction from Sparse Unposed Views](https://arxiv.org/abs/2511.21945)
...
### [51] [TAPVid-360: Tracking Any Point in 360 from Narrow Field of View Video](https://arxiv.org/abs/2511.21946)
...
### [52] [WalkCLIP: Multimodal Learning for Urban Walkability Prediction](https://arxiv.org/abs/2511.21947)
...
### [53] [DeepGI: Explainable Deep Learning for Gastrointestinal Image Classification](https://arxiv.org/abs/2511.21959)
*Walid Houmaidi,Mohamed Hadadi,Youssef Sabiri,Yousra Chtouki*

Main category: cs.CV

TL;DR: 本研究利用包含4000张内窥镜图像的新型胃肠道医学影像数据集，对四种疾病（憩室病、肿瘤、腹膜炎、输尿管）进行深度学习模型比较分析。VGG16和MobileNetV2模型均达到96.5%的测试准确率，并结合Grad-CAM实现可解释性，为胃肠道计算机辅助诊断提供了准确、稳健且可解释的基准。


<details>
  <summary>Details</summary>
Motivation: 解决内窥镜检查中常见的挑战，如光照变化、相机角度波动和成像伪影，并开发出能对胃肠道疾病进行准确、自动化分类的系统，同时提高临床可解释性。

Method: 构建了一个包含4000张内窥镜图像的新型胃肠道医学影像数据集，涵盖憩室病、肿瘤、腹膜炎和输尿管四种疾病。采用VGG16、MobileNetV2和Xception等先进深度学习模型进行疾病分类的比较分析。通过Grad-CAM可视化技术实现模型的可解释性，以识别对预测有影响的图像区域。

Result: VGG16和MobileNetV2模型在测试中均达到96.5%的最高准确率，Xception模型达到94.24%。研究建立了自动疾病分类的稳健基准和基线。通过Grad-CAM可视化，模型能够识别对预测最有影响的图像区域，增强了临床可解释性。实验结果表明，即使在复杂的真实世界条件下，该方法也能实现稳健、准确且可解释的医学图像分析。

Conclusion: 该研究证明了在复杂真实世界条件下实现稳健、准确和可解释的医学图像分析的潜力。它为胃肠道计算机辅助诊断领域贡献了原创基准、比较性见解和视觉解释，并强调了多样化、临床相关数据集和模型可解释性在医学AI研究中的重要性。
...
### [54] [PAT3D: Physics-Augmented Text-to-3D Scene Generation](https://arxiv.org/abs/2511.21978)
...
TL;DR: PAT3D是一个物理增强的文本到3D场景生成框架，它结合了视觉-语言模型和物理模拟，能够生成物理上合理、可用于模拟且无碰撞的3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到3D场景生成方法在物理合理性、场景可模拟性以及对象间的无碰撞方面存在不足，限制了生成场景的质量和下游应用。

Method: PAT3D首先根据文本提示生成3D对象并推断其空间关系，构建分层场景树。然后，利用可微分刚体模拟器在重力作用下驱动场景达到静态平衡，确保无穿透。此外，引入了“模拟循环优化”过程，以保证物理稳定性和无碰撞，并提高与输入提示的语义一致性。

Result: 实验表明，PAT3D在物理合理性、语义一致性和视觉质量方面显著优于现有方法。它不仅生成高质量场景，还能提供可用于场景编辑和机器人操作等下游任务的“模拟就绪”3D场景。

Conclusion: PAT3D是首个将物理模拟引入文本到3D场景生成的框架，成功解决了现有方法的局限性，实现了物理上合理、可用于模拟且高质量的3D场景生成，并为下游应用提供了新的可能性。
...
### [55] [DialBench: Towards Accurate Reading Recognition of Pointer Meter using Large Foundation Models](https://arxiv.org/abs/2511.21982)
*Futian Wang,Chaoliu Weng,Xiao Wang,Zhen Chen,Zhicheng Zhao,Jin Tang*

Main category: cs.CV

TL;DR: 针对指针式仪表读数识别面临的挑战（如反光、遮挡、视角变化和指针刻度混淆），以及缺乏大规模数据集的问题，本文提出了一个包含10730张图像的大规模基准数据集RPM-10K，并引入了一个名为MRLM的新型视觉-语言模型。MRLM通过注入物理关系，明确编码指针和刻度之间的几何及因果关系，实现了鲁棒的读数识别。


<details>
  <summary>Details</summary>
Motivation: 现有指针式仪表读数识别方法因反光、遮挡、动态视角、细指针与刻度线重叠等挑战而脆弱。此外，该领域缺乏大规模数据集来支持鲁棒算法的开发。

Method: 本文首先构建了一个名为RPM-10K的大规模指针式仪表读数基准数据集，包含10730张充分反映上述挑战的仪表图像。在此基础上，提出了一种新型的视觉-语言模型MRLM（Meter Reading Language Model），该模型基于物理关系注入，显式编码指针和刻度之间的几何及因果关系，将感知与物理推理对齐。模型通过交叉注意力融合和自适应专家选择来解释表盘配置并生成精确读数。

Result: 广泛的实验充分验证了所提出的框架（MRLM）在新提出的基准数据集上的有效性。

Conclusion: 本文通过提供一个大规模数据集RPM-10K和提出一个结合物理推理的视觉-语言模型MRLM，成功解决了指针式仪表读数识别中的关键挑战，并证明了其有效性。
...
### [56] [PPBoost: Progressive Prompt Boosting for Text-Driven Medical Image Segmentation](https://arxiv.org/abs/2511.21984)
*Xuchen Li,Hengrui Gu,Mohan Zhang,Qin Liu,Zhen Tan,Xinyuan Zhu,Huixue Zhou,Tianlong Chen,Kaixiong Zhou*

Main category: cs.CV

TL;DR: PPBoost是一个零样本框架，通过将弱文本提示转化为强大的空间视觉提示，解决了医学图像分割中文本提示模型的精度不足和视觉提示模型提示获取困难的问题，显著提高了分割性能。


<details>
  <summary>Details</summary>
Motivation: 文本提示的医学图像分割基础模型直观但缺乏空间精度且在领域偏移下性能下降。视觉提示模型性能强劲，但精确的边界框提示在临床实践中获取成本高且具有挑战性。

Method: PPBoost首先利用视觉语言模型根据文本描述生成初始伪边界框，并通过不确定性感知标准过滤不可靠预测。然后，利用保留的图像-边界框对训练一个伪标签检测器，为查询图像生成高质量边界框。在推理阶段，PPBoost进一步扩展并细化生成的边界框以紧密覆盖目标解剖结构。最终，这些增强的空间定位边界框提示指导现有的分割模型生成最终的密集掩膜。

Result: 在涵盖不同模态和解剖结构的三组数据集上，PPBoost在Dice和归一化表面距离（NSD）方面持续优于文本和视觉提示基线。值得注意的是，它在不使用任何标记数据的情况下超越了少样本分割模型。PPBoost可以泛化到多种典型的视觉分割模型骨干。

Conclusion: PPBoost通过将弱文本线索有效地转化为强大的空间指导，显著提高了零样本医学图像分割的性能，解决了现有文本和视觉提示模型的局限性，并展现出卓越的泛化能力。
...
### [57] [StreamFlow: Theory, Algorithm, and Implementation for High-Efficiency Rectified Flow Generation](https://arxiv.org/abs/2511.22009)
*Sen Fang,Hongbin Zhong,Yalin Feng,Dimitris N. Metaxas*

Main category: cs.CV

TL;DR: 本文提出了一种全面的加速管道，通过新方法将Rectified Flow模型的图像生成速度提升高达611%，远超现有通用加速方法。


<details>
  <summary>Details</summary>
Motivation: Rectified Flow和Flow Matching等新生成模型在控制精度、生成质量和效率方面表现出色，但由于其理论、设计与现有扩散模型的差异，现有加速方法无法直接应用于Rectified Flow模型。

Method: 该管道从理论、设计和推理策略等方面进行了全面实现，采用了新的速度场批处理、异构时间步批处理的向量化以及动态TensorRT编译等方法，以全面加速基于流模型的相关模型。

Result: 实验证明，新方法可将512*512图像生成速度加速高达611%，远超现有公共方法通常实现的18%加速效果。

Conclusion: 该研究成功开发了一个针对流模型（特别是Rectified Flow）的整体加速管道，显著提高了生成效率，超越了当前非通用加速方法的性能。
...
### [58] [Can Multi-Modal LLMs Provide Live Step-by-Step Task Guidance?](https://arxiv.org/abs/2511.21998)
...
### [59] [Intra-Class Probabilistic Embeddings for Uncertainty Estimation in Vision-Language Models](https://arxiv.org/abs/2511.22019)
...
### [60] [MedEyes: Learning Dynamic Visual Focus for Medical Progressive Diagnosis](https://arxiv.org/abs/2511.22018)
...
### [61] [Layover or Direct Flight: Rethinking Audio-Guided Image Segmentation](https://arxiv.org/abs/2511.22025)
...
### [62] [PAGen: Phase-guided Amplitude Generation for Domain-adaptive Object Detection](https://arxiv.org/abs/2511.22029)
...
### [63] [SparseWorld-TC: Trajectory-Conditioned Sparse Occupancy World Model](https://arxiv.org/abs/2511.22039)
...
### [64] [ICM-SR: Image-Conditioned Manifold Regularization for Image Super-Resoultion](https://arxiv.org/abs/2511.22048)
...
### [65] [TPCNet: Triple physical constraints for Low-light Image Enhancement](https://arxiv.org/abs/2511.22052)
...
### [66] [OralGPT-Omni: A Versatile Dental Multimodal Large Language Model](https://arxiv.org/abs/2511.22055)
...
### [67] [DNA: Dual-branch Network with Adaptation for Open-Set Online Handwriting Generation](https://arxiv.org/abs/2511.22064)
...
### [68] [WorldWander: Bridging Egocentric and Exocentric Worlds in Video Generation](https://arxiv.org/abs/2511.22098)
*Quanjian Song,Yiren Song,Kelly Peng,Yuan Gao,Mike Zheng Shou*

Main category: cs.CV

TL;DR: WorldWander是一个上下文学习框架，用于在视频生成中实现第一人称（自我中心）和第三人称（外在中心）视角之间的无缝转换，并通过新数据集和方法达到了卓越的同步性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在真实感和可控性方面取得了显著进展，但不同视角（如第一人称和第三人称）之间的无缝视频转换仍未得到充分探索。弥合这些视角对于电影制作、具身AI和世界模型至关重要。

Method: 该研究提出了WorldWander框架，一个基于先进视频扩散Transformer的上下文学习框架。它整合了(i) 上下文视角对齐（In-Context Perspective Alignment）和(ii) 协作位置编码（Collaborative Position Encoding）来高效建模跨视角同步。此外，研究还整理并发布了EgoExo-8K数据集，一个包含合成和真实世界场景的同步第一人称-第三人称三元组的大规模数据集。

Result: 实验证明，WorldWander在视角同步、角色一致性和泛化能力方面取得了卓越表现，为第一人称-第三人称视频转换设定了新的基准。

Conclusion: WorldWander框架通过创新的方法和大规模数据集，成功解决了第一人称和第三人称视频视角转换的挑战，并在关键指标上树立了新标准，对相关领域具有重要意义。
...
### [69] [MRI-Based Brain Age Estimation with Supervised Contrastive Learning of Continuous Representation](https://arxiv.org/abs/2511.22102)
*Simon Joseph Clément Crête,Marta Kersten-Oertel,Yiming Xiao*

Main category: cs.CV

TL;DR: 该研究首次提出使用监督对比学习（RNC损失）结合T1w结构MRI进行脑龄估计，实现了高精度和可解释性，并展示了其作为神经退行性疾病生物标志物的潜力。


<details>
  <summary>Details</summary>
Motivation: MRI脑龄估计模型旨在通过神经解剖特征评估生物脑龄，脑老化加速现象可作为临床应用的潜在生物标志物。现有基于深度学习的回归方法往往未能有效捕捉神经形态变化的连续性，可能导致次优的特征表示和结果。

Method: 本研究提出利用监督对比学习和Rank-N-Contrast (RNC) 损失，基于T1w结构MRI估计脑龄，并首次结合Grad-RAM对回归结果进行可视化解释。此外，将该方法应用于阿尔茨海默病和帕金森病患者，以估计生物脑龄与实际年龄之间的差距。

Result: 该方法在有限的训练数据集上实现了4.27年的平均绝对误差（MAE）和0.93的R²，显著优于使用相同ResNet骨干的传统深度回归方法，且与使用更大训练数据的最先进方法表现相当或更优。Grad-RAM揭示了RNC损失在年龄回归中能捕捉到更细微的特征。在探索性研究中，该方法揭示了阿尔茨海默病和帕金森病患者的脑龄差距与疾病严重程度之间的相关性。

Conclusion: 所提出的基于RNC损失的监督对比学习方法能有效估计脑龄，提供更细致的特征解释，并在神经退行性疾病中显示出作为潜在生物标志物的巨大潜力。
...
### [70] [MoE3D: Mixture of Experts meets Multi-Modal 3D Understanding](https://arxiv.org/abs/2511.22103)
*Yu Li,Yuenan Hou,Yingmei Wei,Xinge Zhu,Yuexin Ma,Wenqi Shao,Yanming Guo*

Main category: cs.CV

TL;DR: MoE3D提出了一种基于专家混合（MoE）的多模态3D理解框架，通过部署专门的专家网络和渐进式预训练策略，有效处理模态异构性，并在多项3D任务中取得竞争性甚至超越现有最佳方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态融合方法通常采用单一、密集的融合网络，难以有效处理模态间显著的异构性和复杂性，导致性能不佳。

Method: 该论文提出了MoE3D，将专家混合（MoE）集成到多模态学习框架中。具体方法包括：1) 部署一组专门的“专家”网络，每个专家擅长处理特定模态或跨模态交互模式。2) 设计基于MoE的Transformer以更好地利用视觉特征中隐藏的互补信息。3) 引入信息聚合模块以进一步增强融合性能。4) 采用Top-1门控机制，确保高效处理。5) 提出渐进式预训练策略，以更好地利用语义和2D先验知识，为网络提供良好的初始化。

Result: MoE3D在四种主流的3D理解任务中均取得了具有竞争力的性能。值得注意的是，在Multi3DRefer任务上，MoE3D超越了表现最佳的现有方法6.1 mIoU。

Conclusion: MoE3D通过引入专家混合架构和渐进式预训练策略，有效解决了多模态3D理解中模态异构性带来的挑战，显著提升了模型性能，为多模态3D理解任务提供了一个有效且高效的解决方案。
...
### [71] [HyperST: Hierarchical Hyperbolic Learning for Spatial Transcriptomics Prediction](https://arxiv.org/abs/2511.22107)
*Chen Zhang,Yilu An,Ying Chen,Hao Li,Xitong Ling,Lihao Liu,Junjun He,Yuxiang Lin,Zihui Wang,Rongshan Yu*

Main category: cs.CV

TL;DR: HyperST是一个用于空间转录组学预测的框架，它通过在双曲空间中学习多层次的图像-基因表示，解决了现有方法未能充分利用数据层级结构和模态间信息不对称的问题，从而实现了更准确、可扩展的基因表达预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从组织病理图像预测基因表达时，主要侧重于点级图像-基因匹配，未能充分利用空间转录组（ST）数据的完整层级结构（特别是在基因表达侧），导致图像-基因对齐不完整。此外，基因表达谱包含更多分子细节，而这些细节在组织学图像中可能缺乏显著的视觉对应物，造成固有的信息不对称，需要更复杂的表示学习方法来弥合模态差距。

Method: 本文提出了HyperST框架，用于ST预测。它通过在双曲空间中建模数据的固有层级结构，学习多层次的图像-基因表示。
1. **多层次表示提取器（Multi-Level Representation Extractors）**：设计用于从图像和基因两种模态中捕获点级和生态位级（niche-level）表示，提供超越单个点级图像-基因对的上下文感知信息。
2. **层级双曲对齐模块（Hierarchical Hyperbolic Alignment module）**：引入该模块来统一这些表示，在执行空间对齐的同时，分层构建图像和基因嵌入。这种对齐策略用分子语义丰富了图像表示，显著改善了跨模态预测。

Result: HyperST在四个来自不同组织的公共数据集上取得了最先进的性能。

Conclusion: HyperST通过有效弥合模态差距并利用层级结构，为更具可扩展性和准确性的空间转录组学预测铺平了道路。
...
### [72] [GoPrune: Accelerated Structured Pruning with $\ell_{2,p}$-Norm Optimization](https://arxiv.org/abs/2511.22120)
*Li Xu,Xianchao Xiu*
...
### [73] [PROMPTMINER: Black-Box Prompt Stealing against Text-to-Image Generative Models via Reinforcement Learning and Fuzz Optimization](https://arxiv.org/abs/2511.22119)
...
### [74] [Cue3D: Quantifying the Role of Image Cues in Single-Image 3D Generation](https://arxiv.org/abs/2511.22121)
...
### [75] [GA2-CLIP: Generic Attribute Anchor for Efficient Prompt Tuningin Video-Language Models](https://arxiv.org/abs/2511.22125)
...
### [76] [Autonomous labeling of surgical resection margins using a foundation model](https://arxiv.org/abs/2511.22131)
...
### [77] [DualVLA: Building a Generalizable Embodied Agent via Partial Decoupling of Reasoning and Action](https://arxiv.org/abs/2511.22134)
...
### [78] [EASL: Multi-Emotion Guided Semantic Disentanglement for Expressive Sign Language Generation](https://arxiv.org/abs/2511.22135)
...
### [79] [SemOD: Semantic Enabled Object Detection Network under Various Weather Conditions](https://arxiv.org/abs/2511.22142)
...
### [80] [Stacked Ensemble of Fine-Tuned CNNs for Knee Osteoarthritis Severity Grading](https://arxiv.org/abs/2511.22143)
...
### [81] [RemedyGS: Defend 3D Gaussian Splatting against Computation Cost Attacks](https://arxiv.org/abs/2511.22147)
...
### [82] [IMTalker: Efficient Audio-driven Talking Face Generation with Implicit Motion Transfer](https://arxiv.org/abs/2511.22167)
*Bo Chen,Tao Liu,Qi Chen,Xie Chen,Zilong Zheng*

Main category: cs.CV

TL;DR: IMTalker是一个新颖的框架，通过隐式运动转移实现高效、高保真的人脸说话生成。它利用交叉注意力机制处理全局运动，并通过身份自适应模块保持身份，同时使用轻量级流匹配运动生成器。该方法在运动精度、身份保留和音唇同步方面超越了现有方法，达到了最先进的质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的说话人脸生成方法依赖于显式光流和局部扭曲，这导致难以建模复杂的全局运动并引发身份漂移问题。

Method: IMTalker的核心思想是用交叉注意力机制取代传统的基于流的扭曲，以在统一的潜在空间中隐式建模运动差异和身份对齐。为进一步保留说话者身份，引入了一个身份自适应模块，将运动潜在空间投影到个性化空间中。此外，一个轻量级的流匹配运动生成器从音频、姿态和凝视线索中生成生动且可控的隐式运动向量。

Result: 实验证明，IMTalker在运动精度、身份保留和音唇同步方面均超越了现有方法，达到了最先进的质量。它还展现出卓越的效率，在RTX 4090 GPU上，视频驱动生成可达40 FPS，音频驱动生成可达42 FPS。

Conclusion: IMTalker通过隐式运动转移提供了一种高效、高保真的说话人脸生成解决方案，有效解决了现有方法的局限性。其代码和预训练模型将发布，以促进应用和未来的研究。
...
### [83] [Partially Shared Concept Bottleneck Models](https://arxiv.org/abs/2511.22170)
*Delong Zhao,Qiang Huang,Di Yan,Yiqun Sun,Jun Yu*

Main category: cs.CV

TL;DR: PS-CBM是一种部分共享的概念瓶颈模型框架，通过多模态概念生成、部分共享概念策略和概念效率准确性（CEA）指标，解决了现有CBM在视觉接地、概念冗余和平衡准确性与概念紧凑性方面的挑战，显著提升了模型的准确性和可解释性，同时减少了所需概念数量。


<details>
  <summary>Details</summary>
Motivation: 现有的概念瓶颈模型（CBM）在自动化概念生成时面临三个主要挑战：视觉接地性差、概念冗余以及缺乏原则性指标来平衡预测准确性和概念紧凑性。

Method: 本文提出了PS-CBM框架，包含三个核心组件：1) 一个多模态概念生成器，结合LLM派生的语义和基于实例的视觉线索；2) 一个部分共享概念策略，根据激活模式合并概念以平衡特异性和紧凑性；3) 概念效率准确性（CEA），一个事后指标，共同衡量预测准确性和概念紧凑性。

Result: 在十一个不同数据集上的大量实验表明，PS-CBM持续优于最先进的CBM，分类准确性提高了1.0%-7.4%，CEA提高了2.0%-9.5%，同时所需概念数量显著减少。

Conclusion: PS-CBM在实现高准确性和强可解释性方面非常有效，解决了现有CBM的局限性，并为未来可解释AI模型的发展提供了新的方向。
...
### [84] [BrepGPT: Autoregressive B-rep Generation with Voronoi Half-Patch](https://arxiv.org/abs/2511.22171)
*Pu Li,Wenhao Zhang,Weize Quan,Biao Zhang,Peter Wonka,Dong-Ming Yan*

Main category: cs.CV

TL;DR: BrepGPT是一个单阶段自回归框架，通过创新的Voronoi半面片（VHP）表示和双VQ-VAE编码，实现了B-rep模型的生成。它克服了现有方法的多阶段限制，在无条件生成上达到最先进水平，并支持多种条件生成和编辑任务。


<details>
  <summary>Details</summary>
Motivation: 现代工业设计中，B-rep是CAD模型表示的事实标准。然而，几何和拓扑元素之间复杂的耦合使得现有生成方法依赖于级联的多阶段网络，导致误差累积和计算效率低下。

Method: 该研究提出了Voronoi半面片（VHP）表示，将B-rep分解为统一的局部单元，通过将几何体分配给最近的半边并采样其下一个指针，从而在一个连贯的格式中统一几何属性和拓扑关系。进一步利用双VQ-VAE将顶点拓扑和VHP编码为基于顶点的紧凑序列化token。然后，训练一个仅解码器Transformer来自回归预测这些token，最终将其映射到基于顶点的特征并解码为完整的B-rep模型。

Result: 实验表明，BrepGPT在无条件B-rep生成方面取得了最先进的性能。该框架还在多种应用中展现了多功能性，包括从类别标签、点云、文本描述和图像进行条件生成，以及B-rep自动补全和插值。

Conclusion: BrepGPT提供了一个高效且多功能的单阶段自回归框架，用于B-rep生成，通过其独特的VHP表示和序列化方法，显著优于传统的多阶段方法，并在CAD模型生成领域具有广泛的应用潜力。
...
### [85] [Guiding the Inner Eye: A Framework for Hierarchical and Flexible Visual Grounded Reasoning](https://arxiv.org/abs/2511.22172)
*Zhaoyang Wei,Wenchao Ding,Yanchao Hao,Xi Chen*

Main category: cs.CV

TL;DR: 本文提出GRiP，一个两阶段训练框架，通过认知增强的强化学习（RL）来培养鲁棒且灵活的视觉接地推理能力，解决了现有方法在稳定性和灵活性上的局限。


<details>
  <summary>Details</summary>
Motivation: 现有模型在“通过图像思考”时，其推理能力往往受限于端到端强化学习的不稳定性或监督微调的僵硬性，导致模型学习困难或缺乏处理复杂现实场景所需的认知灵活性。

Method: 引入GRiP（Guided Reasoning and Perception），一个两阶段训练框架，通过明确引导模型的感知焦点和逻辑路径。其核心是认知增强的RL阶段，包含两项创新：(1) 显著性加权IoU奖励，激励模型优先定位任务关键对象；(2) 多启发式奖励，通过奖励多样但逻辑有效的推理路径来鼓励认知灵活性。模型基于Qwen2.5-VL-7B进行初始化。

Result: GRiP在多个具有挑战性的基准测试中表现出显著的性能提升，并在TreeBench和V* Bench上取得了开源模型中的最先进（SOTA）结果，证明了其在复杂视觉推理中的有效性。

Conclusion: 研究表明，超越简单的奖励机制，转而通过认知启发式信号指导模型“看什么”和“如何思考”，对于解锁多模态智能的更高水平至关重要。
...
### [86] [Real-Time Long Horizon Air Quality Forecasting via Group-Relative Policy Optimization](https://arxiv.org/abs/2511.22169)
...
TL;DR: 针对东亚复杂地形下的PM浓度场长期预测挑战，本文构建并发布了区域性高分辨率CMAQ-OBS数据集，并引入了群组相对策略优化（GRPO）方法，通过解决不对称运营成本问题，显著降低了误报率，提升了预测可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在东亚等复杂地形区域的PM浓度场长期预测中，缺乏区域特异性动态捕捉能力，且依赖非实时输入，限制了其在本地预警系统中的实用性。此外，标准逐点目标函数无法反映不对称的运营成本（误报损害公众信任，漏报严重事件危及人口），导致模型预测偏高，误报率过高。

Method: 1. 构建并发布了针对东亚地区的真实观测与高分辨率CMAQ-OBS数据集，以减少区域误差并支持实时长期预测。2. 引入了群组相对策略优化（GRPO）方法，该方法结合了类别奖励和课程推出机制，旨在使预测结果与运营优先级对齐，解决不对称成本问题。

Result: 1. CMAQ-OBS数据集将区域误差降低了59.5%，并实现了48-120小时的实时预测能力。2. GRPO框架显著提升了预测的可靠性。3. 与仅使用SFT的基线模型相比，本文模型在保持F1分数竞争力的同时，将误报率降低了47.3%。

Conclusion: 本研究提出的框架，结合新的数据集和GRPO优化方法，有效解决了东亚地区长期空气质量预测的区域特异性挑战和运营成本不匹配问题，显著提高了预测的可靠性，证明了其在实际空气质量预警系统中的有效性。
...
### [87] [HybridWorldSim: A Scalable and Controllable High-fidelity Simulator for Autonomous Driving](https://arxiv.org/abs/2511.22187)
...
TL;DR: HybridWorldSim是一个混合模拟框架，结合多遍历神经重建静态背景和生成模型动态代理，实现了高保真、视觉和空间一致的自动驾驶模拟，并发布了新的MIRROR数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶模拟方法在支持大视角变化下的新视角合成和确保几何一致性方面存在困难。

Method: 引入HybridWorldSim混合模拟框架，该框架整合了用于静态背景的多遍历神经重建和用于动态代理的生成建模。同时发布了一个新的多遍历数据集MIRROR，用于鲁棒基准测试。

Result: HybridWorldSim能够创建多样化、高保真的驾驶场景，并确保可靠的视觉和空间一致性。实验表明，它超越了现有最先进的方法。

Conclusion: HybridWorldSim为高保真模拟提供了一个实用且可扩展的解决方案，是自动驾驶研究和开发的重要资源。
...
### [88] [Enhanced Graph Convolutional Network with Chebyshev Spectral Graph and Graph Attention for Autism Spectrum Disorder Classification](https://arxiv.org/abs/2511.22178)
...
### [89] [Shoe Style-Invariant and Ground-Aware Learning for Dense Foot Contact Estimation](https://arxiv.org/abs/2511.22184)
...
### [90] [MTR-VP: Towards End-to-End Trajectory Planning through Context-Driven Image Encoding and Multiple Trajectory Prediction](https://arxiv.org/abs/2511.22181)
...
### [91] [ARPGNet: Appearance- and Relation-aware Parallel Graph Attention Fusion Network for Facial Expression Recognition](https://arxiv.org/abs/2511.22188)
...
### [92] [MG-Nav: Dual-Scale Visual Navigation via Sparse Spatial Memory](https://arxiv.org/abs/2511.22609)
...
### [93] [Controllable 3D Object Generation with Single Image Prompt](https://arxiv.org/abs/2511.22194)
...
### [94] [MrGS: Multi-modal Radiance Fields with 3D Gaussian Splatting for RGB-Thermal Novel View Synthesis](https://arxiv.org/abs/2511.22997)
...
### [95] [From Compound Figures to Composite Understanding: Developing a Multi-Modal LLM from Biomedical Literature with Medical Multiple-Image Benchmarking and Validation](https://arxiv.org/abs/2511.22232)
...
### [96] [RobotSeg: A Model and Dataset for Segmenting Robots in Image and Video](https://arxiv.org/abs/2511.22950)
...
### [97] [3D-Consistent Multi-View Editing by Diffusion Guidance](https://arxiv.org/abs/2511.22228)
...
### [98] [SimScale: Learning to Drive via Real-World Simulation at Scale](https://arxiv.org/abs/2511.23369)
...
### [99] [IE-SRGS: An Internal-External Knowledge Fusion Framework for High-Fidelity 3D Gaussian Splatting Super-Resolution](https://arxiv.org/abs/2511.22233)
...
### [100] [Bridging 3D Deep Learning and Curation for Analysis and High-Quality Segmentation in Practice](https://arxiv.org/abs/2511.22236)
...
### [101] [Creating Blank Canvas Against AI-enabled Image Forgery](https://arxiv.org/abs/2511.22237)
...
### [102] [Prompt-based Consistent Video Colorization](https://arxiv.org/abs/2511.22330)
...
### [103] [TTSnap: Test-Time Scaling of Diffusion Models via Noise-Aware Pruning](https://arxiv.org/abs/2511.22242)
...
### [104] [Semantic Anchoring for Robust Personalization in Text-to-Image Diffusion Models](https://arxiv.org/abs/2511.22245)
...
### [105] [Asking like Socrates: Socrates helps VLMs understand remote sensing images](https://arxiv.org/abs/2511.22396)
...
### [106] [UMind-VL: A Generalist Ultrasound Vision-Language Model for Unified Grounded Perception and Comprehensive Interpretation](https://arxiv.org/abs/2511.22256)
...
### [107] [Toward Diffusible High-Dimensional Latent Spaces: A Frequency Perspective](https://arxiv.org/abs/2511.22249)
...
### [108] [DriveVGGT: Visual Geometry Transformer for Autonomous Driving](https://arxiv.org/abs/2511.22264)
*Xiaosong Jia,Yanhao Liu,Junqi You,Renqiu Xia,Yu Hong,Junchi Yan*

Main category: cs.CV

TL;DR: 本文提出了DriveVGGT，一个专为自动驾驶数据设计的尺度感知4D重建框架，通过整合自动驾驶特有的先验知识，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈重建方法（如VGGT）在直接应用于自动驾驶系统时表现不佳，因为自动驾驶任务具有不同的先验知识。具体包括：摄像头视图重叠最小、已知内外参可提供更多约束和绝对尺度、以及所有摄像头相对位置固定但车辆在移动。

Method: 本文提出了DriveVGGT框架，具体方法包括：1) 引入时间视频注意力（TVA）模块独立处理多摄像头视频，利用单摄像头序列的时空连续性。2) 提出多摄像头一致性注意力（MCA）模块，通过归一化相对姿态嵌入进行窗口注意力，建立跨摄像头的一致性关系，并限制每个token只关注附近帧。3) 扩展标准VGGT头部，增加绝对尺度头部和自车姿态头部。

Result: 实验结果表明，DriveVGGT在自动驾驶数据集上优于VGGT、StreamVGGT和fastVGGT。广泛的消融研究验证了所提出设计的有效性。

Conclusion: DriveVGGT成功地将自动驾驶系统的独特先验知识整合到前馈重建框架中，实现了更优的尺度感知4D重建性能。
...
### [109] [Revisiting the Necessity of Lengthy Chain-of-Thought in Vision-centric Reasoning Generalization](https://arxiv.org/abs/2511.22586)
*Yifan Du,Kun Zhou,Yingqian Min,Yue Ling,Wayne Xin Zhao,Youbin Wu*

Main category: cs.CV

TL;DR: 本研究系统评估了不同思维链（CoT）设计对视觉语言模型（VLM）可泛化视觉推理能力的影响，发现简短、仅包含关键接地的CoT在泛化性方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 尽管CoT数据，特别是长或视觉CoT，已被广泛用于监督中间推理，但目前尚不清楚特定CoT设计为何有效以及哪些真正支持可泛化的推理。

Method: 研究在一个可控的迷宫求解基准上进行，该基准的推理规则完全视觉化，难度可调，且中间步骤可自动生成。使用Qwen2.5-VL-7B模型和SFT-then-RL流程，比较了三种CoT格式：语言CoT、接地CoT（包含空间坐标轨迹）和视觉CoT（包含图像操作）。研究还在其他以视觉为中心的人物上验证了这些发现。

Result: 视觉和更长的CoT主要加速收敛，但并未提升最终性能上限；仅包含必要接地步骤的简洁CoT优于更长的追踪；令人惊讶的是，仅保留最少接地结果的CoT在不同迷宫尺寸之间泛化效果最好。这些发现揭示了“短即是长”效应。

Conclusion: 简短且仅包含关键接地步骤的CoT在视觉推理任务中更具泛化能力，为构建更具泛化性的SFT数据集提供了实用指导。
...
### [110] [The Collapse of Patches](https://arxiv.org/abs/2511.22281)
*Wei Guo,Shunqi Mao,Zhuonan Liang,Heng Wang,Weidong Cai*

Main category: cs.CV

TL;DR: 本文提出“图像块坍缩”概念，通过观察部分图像块来降低其他图像块的不确定性，并学习一个自编码器以识别关键依赖关系和最优图像块顺序。该方法能提升自回归图像生成效率，并允许Vision Transformers仅用22%的高排名图像块即可实现高精度图像分类，从而提高视觉效率。


<details>
  <summary>Details</summary>
Motivation: 受量子力学中波函数坍缩的启发，研究者发现观察图像中的某些区域可以降低其他区域的不确定性。其动机是识别哪些图像块在目标区域坍缩过程中最为关键，并找到最优的图像块实现顺序，以提高图像建模的效率。

Method: 研究人员训练了一个自编码器，该模型能“软选择”一部分图像块来重建每个目标图像块，从而学习图像块之间的依赖关系。接着，他们将这些依赖关系图化，并计算每个图像块的PageRank分数，以确定图像块实现的最优顺序。最后，他们将此顺序应用于两种场景：一是通过重新训练最先进的MAR模型来提升自回归图像生成；二是通过仅向Vision Transformers展示高排名图像块，引入一种新的图像分类设置。

Result: 研究表明，遵循这种图像块实现顺序能够改进各种掩码图像建模方法。具体来说，自回归图像生成模型MAR的性能得到了提升。在图像分类任务中，Vision Transformers仅需看到22%的高排名图像块，即可达到高精度。

Conclusion: 本文提出图像块坍缩作为一种新颖的图像建模视角，通过优化图像块的观察顺序来提升视觉效率。实验证明，该方法在自回归图像生成和高效图像分类方面均显示出显著优势。
...
### [111] [DocVAL: Validated Chain-of-Thought Distillation for Grounded Document VQA](https://arxiv.org/abs/2511.22521)
*Ahmad Mohammadshirazi,Pinaki Prasad Guha Neogi,Dheeraj Kulshrestha,Rajiv Ramnath*

Main category: cs.CV

TL;DR: DocVAL是一个验证链式思考蒸馏框架，旨在将大型教师模型的空间推理能力转移到可部署的学生视觉语言模型（VLM）中，以解决文档视觉问答（DocVQA）中的准确性-效率权衡问题，并在推理时无需文本检测或OCR。


<details>
  <summary>Details</summary>
Motivation: 当前的DocVQA系统在准确性和效率之间存在显著权衡：大型教师模型虽然接地气能力强但部署成本高昂，而紧凑型学生模型虽然高效但定位性能大幅下降。研究旨在弥合这一差距，使小型模型也能具备强大的空间推理能力。

Method: 本文提出了DocVAL框架，包含三个关键组件：1) 教师监督结合验证时文本检测，用于过滤和去噪训练信号；2) 一个多模块验证器（VAL），强制答案的正确性和几何一致性，并产生细粒度的像素级错误反馈；3) 一个两阶段学生训练方案，首先从经过验证的链式思考（CoT）轨迹中学习，然后通过VAL反馈进行迭代优化。

Result: 通过DocVAL训练的学生模型（Gemma-3 12B）在DocVQA上取得了91.4%的ANLS和82.4%的mAP，作为纯VLM在推理时无需文本检测或OCR。广泛的消融实验表明，验证反馈贡献了6.3 mAP的增益，迭代优化带来了9.7 mAP的提升。研究还发布了9.5万条高质量、经验证器验证的CoT轨迹。

Conclusion: DocVAL框架成功地将大型教师模型的空间推理能力蒸馏到可部署的学生VLM中，显著提升了其在DocVQA任务上的性能，同时保持了推理效率。通过引入验证反馈和迭代优化，学生模型在无需额外组件的情况下实现了强大的定位能力，并为文档理解领域的空间推理研究提供了宝贵的数据集。
...
### [112] [Can Protective Watermarking Safeguard the Copyright of 3D Gaussian Splatting?](https://arxiv.org/abs/2511.22262)
*Wenkai Huang,Yijia Guo,Gaolei Li,Lei Ma,Hang Zhang,Liwen Hu,Jiazheng Wang,Jianhua Li,Tiejun Huang*

Main category: cs.CV

TL;DR: 本文首次系统性探索了3D Gaussian Splatting (3DGS) 水印框架的脆弱性，并提出了GSPure，一个专门针对3DGS水印的净化框架，能有效去除水印同时保持场景完整性。


<details>
  <summary>Details</summary>
Motivation: 3DGS因其高效和高保真视觉质量而被广泛采用，但现有的3DGS水印方案能否真正提供鲁棒保护尚未被充分验证。传统的2D水印去除技术因3DGS独特的渲染流程和高斯基元特性而无效，这促使研究者探索专门针对3DGS水印的净化方法。

Method: GSPure通过分析视角依赖的渲染贡献和利用几何精确的特征聚类，精确地识别并有效地移除与水印相关的高斯基元，同时保持场景的完整性。

Result: GSPure实现了最佳的水印净化性能，将水印PSNR降低了高达16.34dB，同时对原始场景保真度的损害最小，PSNR损失小于1dB。它在有效性和泛化性方面均优于现有方法。

Conclusion: GSPure是首个专门针对3DGS水印表示的净化框架，它能够有效去除水印，同时最大限度地减少对原始场景质量的影响，从而揭示并解决了3DGS水印框架的潜在脆弱性。
...
### [113] [CoT4AD: A Vision-Language-Action Model with Explicit Chain-of-Thought Reasoning for Autonomous Driving](https://arxiv.org/abs/2511.22532)
*Zhaohui Wang,Tengbo Yu,Hao Tang*

Main category: cs.CV

TL;DR: CoT4AD是一个新颖的VLA框架，通过引入思维链（CoT）推理，显著提升了自动驾驶中视觉-语言模型（VLMs）的数值和因果推理能力，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型在自动驾驶中，存在数值推理能力有限和输入-输出映射过于简化的问题，难以应对需要逐步因果推理的复杂驾驶场景。

Method: CoT4AD框架整合视觉观测和语言指令，进行语义推理、场景理解和轨迹规划。训练时，它显式建模“感知-问题-预测-行动”的思维链，以对齐推理空间与行动空间。推理时，它执行隐式思维链推理，以实现一致的数值推理和动态环境中的鲁棒决策。

Result: 在nuScenes和Bench2Drive等真实世界和模拟基准测试中，CoT4AD在开环和闭环评估中均取得了最先进的性能。

Conclusion: CoT4AD通过引入思维链推理，有效解决了现有VLA模型在自动驾驶中数值和因果推理的不足，显著提升了模型在复杂场景下的决策能力和整体性能。
...
### [114] [Match-and-Fuse: Consistent Generation from Unstructured Image Sets](https://arxiv.org/abs/2511.22287)
*Kate Feingold,Omri Kaduri,Tali Dekel*

Main category: cs.CV

TL;DR: Match-and-Fuse是一种零样本、免训练的方法，用于对非结构化图像集进行一致且受控的生成，能够保持共享内容的跨图像一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理单张图像或密集采样的视频，缺乏对共享视觉元素但视角、时间、内容各异的非结构化图像集进行一致性生成的能力。本文旨在实现集合到集合的生成，同时保持跨图像内容的一致性。

Method: 该方法将任务建模为图，其中每个节点代表一张图像，每条边触发图像对的联合生成。它将所有成对生成整合到一个统一框架中，通过融合图像对之间的内部特征（由密集输入对应关系引导，无需掩码或手动监督）来强制执行局部一致性，并确保整个集合的全局连贯性。此外，它利用了文本到图像模型中多视图共享单一画布时鼓励连贯生成的先验知识。

Result: Match-and-Fuse在一致性和视觉质量方面达到了最先进水平，并为从图像集合创建内容解锁了新的能力。

Conclusion: Match-and-Fuse成功实现了对非结构化图像集的高质量、一致性生成，显著提升了图像集合内容创作的潜能。
...
### [115] [HarmoCLIP: Harmonizing Global and Regional Representations in Contrastive Vision-Language Models](https://arxiv.org/abs/2511.22594)
*Haoxi Zeng,Haoxuan Li,Yi Bin,Pengpeng Zeng,Xing Xu,Yang Yang,Heng Tao Shen*

Main category: cs.CV

TL;DR: CLIP在细粒度语义理解上存在局限性，导致全局与局部表示之间存在权衡。HarmoCLIP通过引入显式细粒度语义监督和区域-语言对齐策略，成功协调了全局和区域表示，显著提升了检索和区域分类任务的性能。


<details>
  <summary>Details</summary>
Motivation: 对比语言-图像预训练（CLIP）模型虽然具有强大的泛化能力，但由于缺乏区域级监督，其细粒度语义理解能力有限。现有方法在改善局部感知的同时，却破坏了全局对齐，导致全局-局部权衡问题持续存在。根本原因是局部文本和视觉语义之间缺乏直接对齐。

Method: 本文提出了HarmoCLIP框架，旨在协调CLIP中的全局和区域表示。方法包括：1) 引入显式细粒度语义监督项，直接将文本片段与其对应的视觉区域对齐，以弥合图像区域空间和文本空间。2) 提出一种新颖的区域-语言对齐监督策略，在不损害全局语义一致性的前提下，促进细粒度语义学习，以增强局部表示能力。

Result: HarmoCLIP在全球检索任务上取得了最先进的性能（提升高达69.78%），在边界框分类等区域任务的Top-1准确率上实现了3.2%的显著提升。它持续优于现有方法，并为CLIP中的全局-局部权衡问题提供了一个平衡、高效且即插即用的解决方案。

Conclusion: HarmoCLIP通过引入直接的细粒度语义监督和新颖的区域-语言对齐策略，成功解决了CLIP中全局与局部表示的权衡问题，有效协调了两种表示，从而在全局和局部视觉-语言任务上均实现了显著的性能提升。
...
### [116] [GazeTrack: High-Precision Eye Tracking Based on Regularization and Spatial Computing](https://arxiv.org/abs/2511.22607)
*Xiaoyin Yang*

Main category: cs.CV

TL;DR: 该研究设计了一个高精度注视收集框架，构建了首个包含多民族、年龄和视力条件的精确注视数据集GazeTrack。同时，提出了形状误差正则化方法以提高瞳孔定位准确性，并引入了新颖的坐标变换方法和注视向量生成模型，显著降低了注视角度误差和计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 虚拟和增强现实应用中眼动追踪日益重要，但当前注视精度无法满足空间计算的需求。

Method: 1. 设计了注视收集框架并使用高精度设备，收集了GazeTrack数据集。2. 提出了一种新颖的形状误差正则化方法来约束瞳孔椭圆拟合，并在开源数据集上训练，以提高语义分割和瞳孔位置预测精度。3. 发明了一种类似于纸张展开的新颖坐标变换方法，用于精确预测GazeTrack数据集上的注视向量。4. 构建了一个注视向量生成模型。

Result: 1. 创建了GazeTrack，首个精确的基准数据集，涵盖了不同种族、年龄和视力条件。2. 形状误差正则化方法增强了语义分割和瞳孔位置预测的准确性。3. 新颖的坐标变换方法能够准确预测GazeTrack数据集上的注视向量。4. 构建的注视向量生成模型相比其他方法，在计算复杂度更低的情况下，实现了更小的注视角度误差。

Conclusion: 通过引入高精度数据集和创新的瞳孔定位、注视预测及向量生成方法，该研究显著提升了注视追踪的准确性，并降低了计算复杂性，为VR/AR空间计算提供了更好的解决方案。
...
### [117] [Structure is Supervision: Multiview Masked Autoencoders for Radiology](https://arxiv.org/abs/2511.22294)
...
TL;DR: MVMAE是一个自监督学习框架，利用放射学研究的多视图结构来学习视图不变且与疾病相关的表示。其扩展版本MVMAE-V2T进一步结合了放射学报告作为辅助文本信号。该框架在疾病分类任务上表现优异，尤其在低标签制度下，证明了结构和文本监督对构建医学基础模型的重要性。


<details>
  <summary>Details</summary>
Motivation: 构建鲁棒的医学机器学习系统需要有效的预训练策略，以利用临床数据中固有的结构。放射学研究的自然多视图组织提供了一种潜在的结构化信息源。

Method: 引入了多视图掩码自编码器（MVMAE），一个自监督框架，通过结合掩码图像重建和跨视图对齐来利用放射学研究的多视图组织。MVMAE将投影间的临床冗余转化为强大的自监督信号，以学习视图不变和疾病相关的表示。进一步提出了MVMAE-V2T，在MVMAE基础上整合放射学报告作为辅助文本学习信号，以增强语义基础，同时保持完全基于视觉的推理。

Result: 在MIMIC-CXR、CheXpert和PadChest三个大型公共数据集的下游疾病分类任务上进行评估，MVMAE持续优于监督学习和视觉-语言基线模型。MVMAE-V2T提供了额外的性能增益，特别是在低标签制度下，此时结构化文本监督最为有益。

Conclusion: 研究结果表明，结构化和文本监督是互补的路径，对于构建可扩展、具有临床基础的医学基础模型至关重要。
...
### [118] [ReAG: Reasoning-Augmented Generation for Knowledge-based Visual Question Answering](https://arxiv.org/abs/2511.22715)
...
TL;DR: 多模态大语言模型（MLLMs）在领域特定或知识密集型视觉问答（VQA）中表现不佳。本文提出ReAG，一种推理增强的多模态检索增强生成（RAG）方法，通过结合粗粒度和细粒度检索、评论模型过滤以及强化学习来提升知识库VQA的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在领域特定或知识密集型VQA中表现受限，因为相关信息在预训练数据中不足。当前的知识库VQA（KB-VQA）检索增强方法存在精度低、噪声段落多和推理能力有限的问题。

Method: ReAG（推理增强多模态RAG）结合了粗粒度和细粒度检索。它引入了一个评论模型来过滤不相关的段落，以确保高质量的额外上下文。模型采用多阶段训练策略，利用强化学习增强对检索内容的推理能力，而监督微调仅作为冷启动。

Result: 在Encyclopedic-VQA和InfoSeek数据集上的大量实验表明，ReAG显著优于现有方法，提高了答案准确性，并提供了基于检索证据的可解释推理。

Conclusion: ReAG通过结合改进的检索机制和强化学习增强的推理能力，有效解决了知识库VQA的挑战，显著提升了模型性能和答案的可解释性。
...
### [119] [Small Object Detection for Birds with Swin Transformer](https://arxiv.org/abs/2511.22310)
*Da Huo,Marc A. Kastner,Tingwei Liu,Yasutomo Kawanishi,Takatsugu Hirayama,Takahiro Komamizu,Ichiro Ide*

Main category: cs.CV

TL;DR: 本文提出了一种针对小型稀疏物体（如鸟类）检测的专门方法，通过改进CenterNet的颈部网络，引入基于Swin Transformer的分层特征学习和调整窗口大小，以提高小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 小目标检测本身就很困难，而对于小型稀疏物体（如鸟类），由于缺乏足够的训练数据，学习有效特征变得更加困难。现有方法主要针对密集的小目标场景，缺乏对稀疏小目标的有效解决方案。

Method: 本文提出了一种专门用于检测特定类别小物体（鸟类）的方法。通过改进颈部网络（位于骨干网络和预测头之间），采用分层设计来学习更有效的特征。具体地，利用Swin Transformer进行图像特征上采样，并调整了Swin Transformer的滑动窗口大小以适应小目标检测，并将其与CenterNet结合。

Result: 实验结果表明，所提出的基于Swin Transformer的颈部网络与CenterNet结合，通过改变窗口大小，能够实现良好的性能。研究进一步发现，更小的窗口大小（默认值为2）对小目标检测的mAP有益。

Conclusion: 通过改进颈部网络，采用基于Swin Transformer的分层特征学习和适应性窗口大小调整，本文提出的方法能有效提升小型稀疏物体（如鸟类）的检测性能，证明了小窗口尺寸对小目标检测的积极影响。
...
### [120] [All Centers Are at most a Few Tokens Apart: Knowledge Distillation with Domain Invariant Prompt Tuning](https://arxiv.org/abs/2511.22739)
*Amir Mohammad Ezzati,Alireza Malekhosseini,Armin Khosravi,Mohammad Hossein Rohban*

Main category: cs.CV

TL;DR: 本文提出领域不变提示调优（DIPT）方法，通过学习领域特定的连续提示并将其平均以生成领域不变提示，用于从病理学视觉语言模型（PLIP）中蒸馏知识，从而在计算病理学领域泛化任务中实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 计算病理学（CPath）中存在固有的领域偏移问题，源于染色方案、扫描设备和成像设置的差异。视觉语言模型（VLM）如PLIP虽是强大的知识蒸馏源，但其零样本性能受限于提示变异的敏感性。此外，组织病理学缺乏语义描述符，难以定义领域特定的提示，因此需要一种数据驱动的方法来学习领域特定且最终类别通用的连续提示。

Method: 本文提出领域不变提示调优（DIPT）方法，作为知识蒸馏过程中的一个新步骤。DIPT为每个领域学习多个输入tokens，这些tokens在各自领域内独立训练，然后跨领域进行平均，从而得到领域不变的提示。学生模型利用DIPT学习到的提示，从PLIP的文本编码器中蒸馏知识，使视觉特征与领域不变的嵌入对齐，通过多领域训练增强泛化能力。

Result: 该方法在组织病理学数据集的领域泛化任务中，相较于现有最先进（SOTA）的知识蒸馏方法，在平均F1分数上实现了显著提升。

Conclusion: 这项工作有助于在具有异构数据源的真实临床问题中部署稳健的计算病理学模型，推动了CPath模型的实际应用。
...
### [121] [MammoRGB: Dual-View Mammogram Synthesis Using Denoising Diffusion Probabilistic Models](https://arxiv.org/abs/2511.22759)
...
TL;DR: 本研究开发并评估了一种三通道去噪扩散概率模型（DDPM），用于生成真实且解剖学一致的单乳腺双视图乳腺X光片，并评估了通道表示对图像保真度和跨视图一致性的影响。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在开发一种能够合成高质量、具有跨视图一致性的双视图乳腺X光片的方法，以期应用于数据集增强等领域。

Method: 研究人员对一个预训练的三通道DDPM（来自Hugging Face）进行了微调，使用包含11020张乳腺X光片的私有数据集生成成对的头尾位（CC）和内外侧斜位（MLO）视图。评估了三种第三通道编码方式：求和、绝对差和零通道。定量评估包括使用IoU和DSC进行乳腺掩膜分割，并使用EMD和KS检验与真实图像进行分布比较。定性评估则通过非专业放射科医生的视觉图灵测试来评估跨视图一致性和伪影。

Result: 合成的乳腺X光片在IoU和DSC分布上与真实图像相当（EMD为0.020，KS为0.077）。使用求和或绝对差编码的模型在IoU和DSC方面表现优于其他模型（p < 0.001），尽管整体分布大致相似。生成的CC和MLO视图保持了跨视图一致性，其中6%到8%的合成图像表现出与训练数据中一致的伪影。

Conclusion: 三通道DDPM能够生成逼真且解剖学一致的双视图乳腺X光片，在数据集增强方面具有广阔的应用前景。
...
### [122] [INSIGHT: An Interpretable Neural Vision-Language Framework for Reasoning of Generative Artifacts](https://arxiv.org/abs/2511.22351)
*Anshul Bagaria*

Main category: cs.CV

TL;DR: 本文提出INSIGHT，一个统一的多模态框架，用于在极端降级条件下（如低分辨率、压缩）对AI生成图像进行鲁棒检测并提供透明解释，通过结合超分辨率、多尺度定位、语义对齐和VLM生成细粒度解释。


<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型生成图像的真实性不断提高，人们对视觉媒体可靠性的担忧加剧。然而，当前的深度伪造检测系统在实际条件（如严重降采样、压缩、跨域分布偏移）下性能急剧下降。此外，大多数检测器是不透明的分类器，缺乏对图像被标记为合成的原因的解释，这削弱了信任并阻碍了在高风险场景中的应用。

Method: INSIGHT框架结合了以下方法：1) 分层超分辨率，用于放大细微的鉴证线索而不引入误导性伪影；2) Grad-CAM驱动的多尺度定位，以揭示指示生成模式的空间区域；3) CLIP引导的语义对齐，将视觉异常映射到人类可解释的描述符。然后，使用结构化的ReAct + Chain-of-Thought协议提示视觉-语言模型（VLM）生成一致、细粒度的解释，并通过双阶段G-Eval + LLM-as-a-judge管道验证，以最大限度地减少幻觉并确保事实性。

Result: 在动物、车辆和抽象合成场景等不同领域，INSIGHT在极端降级条件下显著提高了检测鲁棒性和解释质量，优于先前的检测器和黑盒VLM基线。即使在极低分辨率（16x16 - 64x64）下也能有效工作。

Conclusion: 研究结果为实现透明、可靠的AI生成图像鉴证提供了一条实用路径，并确立了INSIGHT在可信多模态内容验证方面迈出了重要一步。
...
### [123] [Unexplored flaws in multiple-choice VQA evaluations](https://arxiv.org/abs/2511.22341)
...
### [124] [AnchorFlow: Training-Free 3D Editing via Latent Anchor-Aligned Flows](https://arxiv.org/abs/2511.22357)
...
### [125] [Flowing Backwards: Improving Normalizing Flows via Reverse Representation Alignment](https://arxiv.org/abs/2511.22345)
...
### [126] [Leveraging Textual Compositional Reasoning for Robust Change Captioning](https://arxiv.org/abs/2511.22903)
...
### [127] [Ovis-Image Technical Report](https://arxiv.org/abs/2511.22982)
...
### [128] [MIMM-X: Disentangling Spurious Correlations for Medical Image Analysis](https://arxiv.org/abs/2511.22990)
...
### [129] [UAV-MM3D: A Large-Scale Synthetic Benchmark for 3D Perception of Unmanned Aerial Vehicles with Multi-Modal Data](https://arxiv.org/abs/2511.22404)
...
### [130] [From Illusion to Intention: Visual Rationale Learning for Vision-Language Reasoning](https://arxiv.org/abs/2511.23031)
...
### [131] [DiffStyle360: Diffusion-Based 360° Head Stylization via Style Fusion Attention](https://arxiv.org/abs/2511.22411)
...
### [132] [Wukong's 72 Transformations: High-fidelity Textured 3D Morphing via Flow Models](https://arxiv.org/abs/2511.22425)
...
TL;DR: SpaceMind是一种多模态大型语言模型，仅从RGB输入进行3D空间推理。它通过创新的相机引导模态融合模块，将相机表示作为主动引导模态，显著提升了现有视觉语言模型在空间推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（VLMs）在3D空间推理（如距离估计、大小比较、跨视图一致性）方面表现不佳。现有3D感知方法要么依赖辅助3D信息，要么通过浅层特征融合来增强RGB-only VLMs，效果有限。

Method: SpaceMind采用双编码器架构，整合VGGT作为空间理解编码器，InternViT作为2D视觉编码器。核心思想是将相机表示视为主动引导模态而非被动元数据。模型引入了一个轻量级的相机引导模态融合（Camera-Guided Modality Fusion）模块，该模块在语言模型之前应用，取代了浅层融合。它对空间token应用相机条件偏置，分配反映几何重要性的查询无关权重，并使用相机嵌入来门控融合表示。

Result: SpaceMind在VSI-Bench、SQA3D和SPBench上取得了新的最先进结果，在VSI-Bench和SPBench上大幅超越了现有开放和专有系统，并在SQA3D上达到了最先进性能。

Conclusion: 相机引导模态融合是一种有效且实用的归纳偏置，能够赋予视觉语言模型真正基于空间的智能。
...
### [134] [SkeletonAgent: An Agentic Interaction Framework for Skeleton-based Action Recognition](https://arxiv.org/abs/2511.22433)
*Hongda Liu,Yunfan Liu,Changlu Wang,Yunlong Wang,Zhenan Sun*

Main category: cs.CV

TL;DR: SkeletonAgent是一个新颖的框架，通过问询者和选择者两个合作代理，将识别模型与大型语言模型（LLM）连接起来，以解决LLM在骨架动作识别中缺乏针对性判别线索的问题，从而显著提高了识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在骨架动作识别中利用LLM的语义先验时，LLM通常与识别模型隔离，缺乏性能反馈，导致其难以提供区分相似动作所需的判别性线索。

Method: 本文提出了SkeletonAgent框架，包含两个合作代理：问询者（Questioner）识别最常混淆的类别并将其作为上下文提供给LLM，以获取更有针对性的指导；选择者（Selector）解析LLM的响应，提取精确的关节级约束，并将其反馈给识别器，实现更细粒度的跨模态对齐。

Result: 在NTU RGB+D、NTU RGB+D 120、Kinetics-Skeleton、FineGYM和UAV-Human等五个基准测试中，SkeletonAgent持续优于现有的最先进方法。

Conclusion: SkeletonAgent通过在识别模型和LLM之间建立反馈循环和精细对齐机制，有效克服了LLM在骨架动作识别中提供针对性判别线索的局限性，显著提升了识别性能。
...
### [135] [Fin3R: Fine-tuning Feed-forward 3D Reconstruction Models via Monocular Knowledge Distillation](https://arxiv.org/abs/2511.22429)
*Weining Ren,Hongjun Wang,Xiao Tan,Kai Han*

Main category: cs.CV

TL;DR: Fin3R是一种简单、有效且通用的微调方法，用于提高前馈3D重建模型的几何细节和鲁棒性，通过使用轻量级LoRA适配器，从强大的单目教师模型中提取精细几何细节来微调图像编码器。


<details>
  <summary>Details</summary>
Motivation: 当前的前馈3D重建模型在精细几何和鲁棒性方面表现不佳，原因在于：1) 高保真深度和姿态监督数据稀缺；2) 多视角点图回归固有的几何错位。

Method: Fin3R通过一个额外的轻量级微调步骤来解决这些问题。它冻结了处理视角匹配的解码器，仅微调负责特征提取的图像编码器。编码器通过定制的轻量级LoRA适配器，利用来自强大单目教师模型在大量未标记数据集上蒸馏出的精细几何细节进行丰富。

Result: 经过Fin3R微调的模型在单视角和多视角设置下，都能持续提供更清晰的边界，恢复复杂的结构，并实现更高的几何精度。该方法仅增加了微小的LoRA权重，测试时的内存和延迟几乎没有变化。已在DUSt3R、MASt3R、CUT3R和VGGT等多种模型上进行了验证。

Conclusion: Fin3R通过一个轻量级的微调步骤，有效地解决了前馈3D重建模型在精细几何和鲁棒性方面的挑战，显著提升了模型的性能，且对计算资源影响极小。
...
### [136] [Do You See What I Say? Generalizable Deepfake Detection based on Visual Speech Recognition](https://arxiv.org/abs/2511.22443)
*Maheswar Bora,Tashvik Dhamija,Shukesh Reddy,Baptiste Chopin,Pranav Balaji,Abhijit Das,Antitza Dantcheva*

Main category: cs.CV

TL;DR: 本文提出FauxNet，一种基于预训练视觉语音识别（VSR）特征的深度伪造检测网络，旨在实现零样本泛化检测。FauxNet在零样本设置下超越现有技术，并能区分生成技术，同时引入了两个包含约38,000个真实和伪造视频的新数据集。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术进步迅速，生成了高度逼真的图像、视频和音频，但这引发了媒体滥用的严重担忧。因此，迫切需要鲁棒可靠的深度伪造检测方法来缓解这种滥用。

Method: 研究者提出了FauxNet网络，它利用预训练的视觉语音识别（VSR）特征。通过从视频中提取时间VSR特征，FauxNet能够识别并区分真实视频和被操纵的视频。该方法特别关注零样本检测，即泛化检测能力。

Result: FauxNet在零样本检测设置下持续优于现有最先进的技术。此外，FauxNet能够归因（即区分视频源自的生成技术）。研究者还提出了两个新数据集：Authentica-Vox和Authentica-HDTF，总计约38,000个真实和伪造视频，后者使用六种最新深度伪造生成技术创建。在Authentica数据集和FaceForensics++上的广泛分析和结果证明了FauxNet的优越性。

Conclusion: FauxNet是一种有效的深度伪造检测网络，尤其在零样本泛化能力方面表现出色，并能区分生成技术。所提出的FauxNet在多个数据集上均显示出卓越性能。研究者将公开Authentica数据集，以促进未来的研究。
...
### [137] [ABounD: Adversarial Boundary-Driven Few-Shot Learning for Multi-Class Anomaly Detection](https://arxiv.org/abs/2511.22436)
*Runzhi Deng,Yundi Hu,Xinshuang Zhang,Zhao Wang,Xixi Liu,Wang-Zhou Dai,Caifeng Shan,Fang Zhao*

Main category: cs.CV

TL;DR: 针对少样本多类别工业异常检测，本文提出了ABounD框架，通过动态概念融合和对抗性边界锻造，形成精确且灵活的决策边界，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 少样本多类别工业异常检测面临挑战。视觉-语言模型需要兼具类别适应性和高区分度，但数据稀缺导致正常与异常边界模糊，易造成细微缺陷遗漏和非典型正常样本误判。

Method: 本文提出ABounD框架，一个统一的学习范式：1. 动态概念融合（DCF）模块：根据图像特征，融合通用先验和类别特定线索，生成类别自适应提示。2. 对抗性边界锻造（ABF）模块：通过PGD风格扰动生成边界级“围栏特征”，塑造更精确的决策边界。训练采用单阶段概念-边界损失，ABF提供主要监督信号，语义-空间正则化器稳定优化。

Result: 在MVTec-AD和VisA数据集上的实验表明，ABounD在少样本多类别异常检测任务中达到了最先进的性能。

Conclusion: ABounD通过DCF和ABF的协同作用，形成了一个紧密贴合正常数据、同时保持灵活性和鲁棒语义对齐的决策边界，有效解决了少样本多类别工业异常检测的挑战，并取得了优异成果。
...
### [138] [Bharat Scene Text: A Novel Comprehensive Dataset and Benchmark for Indian Language Scene Text Understanding](https://arxiv.org/abs/2511.23071)
...
TL;DR: 针对印度语言场景文本识别面临的数据集和模型缺乏的挑战，本文提出了一个大规模、多语言的Bharat场景文本数据集（BSTD），并评估了现有模型，旨在推动该领域的研究进展。


<details>
  <summary>Details</summary>
Motivation: 英语场景文本识别已取得显著进展，但印度语言场景文本识别仍面临挑战，原因包括文字多样性、非标准字体、书写风格多变，以及最重要的是，高质量数据集和开源模型的匮乏。

Method: 本文引入了Bharat场景文本数据集（BSTD），一个大规模且全面的基准，包含超过10万个单词，涵盖11种印度语言和英语，来源于6500多张从印度各地捕获的场景图像。该数据集经过精心标注，支持多种场景文本任务，包括：场景文本检测、文字识别、裁剪词识别和端到端场景文本识别。作者通过微调为英语开发的现有最先进模型，对印度语言进行了评估。

Result: 评估结果突显了印度语言场景文本识别领域的挑战和机遇。该数据集被认为是推动该领域研究迈出的重要一步。所有模型和数据均已开源。

Conclusion: BSTD数据集及其开源模型和数据，代表了在印度语言场景文本识别领域推进研究的重大一步，并揭示了该领域当前的挑战与机遇。
...
### [139] [Evaluating the Clinical Impact of Generative Inpainting on Bone Age Estimation](https://arxiv.org/abs/2511.23066)
...
### [140] [Benchmarking machine learning models for multi-class state recognition in double duantum dot data](https://arxiv.org/abs/2511.22451)
...
### [141] [ITS3D: Inference-Time Scaling for Text-Guided 3D Diffusion Models](https://arxiv.org/abs/2511.22456)
...
### [142] [Beyond Real versus Fake Towards Intent-Aware Video Analysis](https://arxiv.org/abs/2511.22455)
...
### [143] [REVEAL: Reasoning-enhanced Forensic Evidence Analysis for Explainable AI-generated Image Detection](https://arxiv.org/abs/2511.23158)
...
### [144] [Gaussians on Fire: High-Frequency Reconstruction of Flames](https://arxiv.org/abs/2511.22459)
*Jakob Nazarenus,Dominik Michels,Wojtek Palubicki,Simin Kou,Fang-Lue Zhang,Soren Pirk,Reinhard Koch*

Main category: cs.CV

TL;DR: 本文提出一种基于高斯时空表示的方法，从有限的摄像机视角（三个）重建三维动态火焰。


<details>
  <summary>Details</summary>
Motivation: 由于火焰的易变性、透明度和高频特征，捕捉和重建火焰及其动态性极具挑战性。此外，仅从三个视角进行重建会导致几何欠约束问题。

Method: 该方法通过结合密集多视角立体图像和单目深度先验来分离静态背景和动态火焰区域。火焰被初始化为通过融合每视角密集光流投影获得的3D流场。每个3D高斯编码了生命周期和线性速度以匹配密集光流，从而捕捉火焰的高频特征。为了确保跨摄像机的子帧时间对齐，采用了定制的硬件同步模式，使得使用经济实惠的商用硬件即可重建火焰。

Result: 通过大量的重建实验，在多样化和具有挑战性的真实火焰场景中，该方法展现了强大的定量和定性性能。

Conclusion: 所提出的方法能够从有限的摄像机视角鲁棒地重建动态三维火焰，有效处理其复杂特性，并可使用经济实惠的商用硬件实现。
...
### [145] [RoadSceneBench: A Lightweight Benchmark for Mid-Level Road Scene Understanding](https://arxiv.org/abs/2511.22466)
*Xiyan Liu,Han Wang,Yuhu Wang,Junjie Cai,Zhe Cao,Jianzhong Yang,Zhen Lu*

Main category: cs.CV

TL;DR: 本文提出了RoadSceneBench，一个用于评估复杂道路环境中视觉推理的基准，以及HRRP-T，一个用于VLM的训练框架，旨在提升对中级道路语义的理解，以实现更可靠的自动驾驶。


<details>
  <summary>Details</summary>
Motivation: 现有的基准主要侧重于检测或分割等感知任务，忽视了推断道路拓扑和动态场景结构所需的推理能力，而这对于自动驾驶和数字地图构建至关重要。

Method: 本文提出：1) RoadSceneBench，一个轻量但信息丰富的基准，强调关系理解和结构一致性，以评估和推进复杂道路环境中的视觉推理。2) 分层关系奖励传播与时间一致性 (HRRP-T)，一个VLM训练框架，通过自适应奖励信号促进空间连贯性和语义对齐，从而实现几何感知和时间一致性的推理。

Result: 实验结果表明，所提出的方法在各种道路配置中均达到了最先进的性能。

Conclusion: RoadSceneBench为研究中级道路语义和促进结构感知的自动驾驶感知提供了一个紧凑而强大的基础。HRRP-T框架显著增强了VLM的推理可靠性，使其能够超越静态识别，实现更高级别的理解。
...
### [146] [Hybrid, Unified and Iterative: A Novel Framework for Text-based Person Anomaly Retrieval](https://arxiv.org/abs/2511.22470)
*Tien-Huy Nguyen,Huu-Loc Tran,Huu-Phong Phan-Nguyen,Quang-Vinh Dinh*

Main category: cs.CV

TL;DR: 本文提出了一种用于文本描述的人员异常检索方法，结合了局部-全局混合视角模块（LHP）与视觉-语言模型（VLM）、统一图像-文本（UIT）模型以及迭代集成策略和特征选择算法，显著提升了检索性能并达到了SOTA。


<details>
  <summary>Details</summary>
Motivation: 文本描述的人员异常检索是一项具有挑战性的任务，现有方法多依赖复杂的深度学习技术。研究问题在于如何优化模型以获取更细粒度的特征。

Method: 1. 提出局部-全局混合视角（LHP）模块，并与视觉-语言模型（VLM）结合，以融合细粒度和粗粒度特征。 2. 引入统一图像-文本（UIT）模型，结合多种目标损失函数，包括图像-文本对比（ITC）、图像-文本匹配（ITM）、掩码语言建模（MLM）和掩码图像建模（MIM）损失。 3. 提出一种新颖的迭代集成策略，通过迭代组合模型结果而非同时使用。 4. 基于LHP模型的指导，引入一种新颖的特征选择算法以提升模型性能。

Result: 在PAB数据集上，所提方法取得了最先进（SOTA）的性能，与现有工作相比，R@1提升9.70%，R@5提升1.77%，R@10提升1.01%。

Conclusion: 本文提出的方法在文本描述的人员异常检索任务中表现出显著的有效性，并通过结合多尺度特征、多目标优化、迭代集成和特征选择，成功实现了SOTA性能。
...
### [147] [Vision Bridge Transformer at Scale](https://arxiv.org/abs/2511.23199)
...
TL;DR: 本文提出了一种新颖的基于学习的方法，通过单一地面RGB图像估算森林地上生物量（AGB），将其转化为密集预测任务并引入AGB密度图，实现了高精度估算。


<details>
  <summary>Details</summary>
Motivation: 森林在全球生态系统中扮演关键角色，支持生物多样性并通过碳固存缓解气候变化。准确估算AGB对于评估碳储量和野火燃料负荷至关重要。然而，传统方法依赖劳动密集型实地测量，而遥感方法在茂密植被中存在显著局限性。

Method: 研究将AGB估算视为密集预测任务，引入AGB密度图，其中每个像素代表按地块面积和每棵树图像面积归一化的树木生物量。利用合成3D SPREAD数据集，该数据集提供逼真的森林场景、图像内树木属性和实例分割掩膜。通过异速生长方程计算AGB，并训练模型预测AGB密度图，然后整合这些图以恢复捕获场景的AGB估算值。

Result: 该方法在保留的SPREAD数据上实现了1.22 kg/m²的AGB估算中位误差，在真实图像数据集上实现了1.94 kg/m²的中位误差。

Conclusion: 据作者所知，这是首个直接从单一RGB图像估算地上生物量的方法，为森林监测提供了一个可扩展、可解释且经济高效的解决方案，并有望通过公民科学计划促进更广泛的参与。
...
### [149] [AI killed the video star. Audio-driven diffusion model for expressive talking head generation](https://arxiv.org/abs/2511.22488)
*Baptiste Chopin,Tashvik Dhamija,Pranav Balaji,Yaohui Wang,Antitza Dantcheva*

Main category: cs.CV

TL;DR: Dimitra++是一个新颖的音频驱动说话人脸生成框架，利用条件运动扩散Transformer（cMDT）和3D表示，能生成逼真的唇部运动、面部表情和头部姿态。


<details>
  <summary>Details</summary>
Motivation: 研究旨在创建一个能够学习和生成逼真唇部运动、面部表情以及头部姿态的音频驱动说话人脸生成框架，以超越现有方法的性能。

Method: 该研究提出了Dimitra++框架，其核心是条件运动扩散Transformer（cMDT）。cMDT使用3D表示来建模面部运动序列，并通过参考面部图像（决定外观）和音频序列（驱动运动）进行条件化。

Result: 在VoxCeleb2和CelebV-HQ数据集上进行的定量、定性实验和用户研究表明，Dimitra++在生成具有唇部运动、面部表情和头部姿态的逼真说话人脸方面，优于现有方法。

Conclusion: Dimitra++框架在生成逼真的音频驱动说话人脸方面表现出色，特别是在捕捉和再现唇部运动、面部表情和头部姿态方面，显著超越了现有技术。
...
### [150] [SciPostGen: Bridging the Gap between Scientific Papers and Poster Layouts](https://arxiv.org/abs/2511.22490)
*Shun Inadumi,Shohei Tanaka,Tosho Hirasawa,Atsushi Hashimoto,Koichiro Yoshino,Yoshitaka Ushiku*

Main category: cs.CV

TL;DR: 本文介绍了SciPostGen数据集，用于理解和生成科学论文的海报布局，并提出了一个检索增强的海报布局生成框架，该框架能够根据论文结构生成符合约束的布局。


<details>
  <summary>Details</summary>
Motivation: 随着科学论文数量的增长，有效传达研究成果的需求日益增加，海报作为展示论文内容的关键媒介，其布局对信息传达和理解至关重要。目前，在理解论文结构与海报布局之间的对应关系方面存在空白，且缺乏大规模的配对标注数据集。

Method: 研究者引入了大规模数据集SciPostGen，用于理解和生成科学论文的海报布局。基于该数据集，他们分析了论文结构与海报布局元素数量之间的关联。在此基础上，他们探索了一个名为“检索增强海报布局生成”的框架，该框架通过检索与给定论文一致的布局，并将其作为生成指导。实验在有无布局约束的两种条件下进行。

Result: 基于SciPostGen的分析表明，论文结构与海报中的布局元素数量相关。实验结果显示，检索器能够估计出与论文结构对齐的布局，并且所提出的框架能够生成满足给定约束的布局。

Conclusion: SciPostGen数据集有助于深入理解论文结构与海报布局之间的关系。所提出的检索增强海报布局生成框架能够有效地根据论文内容生成符合特定约束的海报布局。
...
### [151] [Toward Automatic Safe Driving Instruction: A Large-Scale Vision Language Model Approach](https://arxiv.org/abs/2511.23311)
*Haruki Sakajo,Hiroshi Takato,Hiroshi Tsutsui,Komei Soda,Hidetaka Kamigaito,Taro Watanabe*

Main category: cs.CV

TL;DR: 本研究探讨了大规模视觉语言模型（LVLMs）在处理驾驶员和道路多视角视频以提升驾驶安全性方面的能力。结果显示，经过微调的LVLMs能生成准确的安全指令，但对复杂事件的检测仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: LVLMs在视觉任务（如物体检测）中表现出色，在自动驾驶（如生成道路视频安全描述）中有应用潜力。然而，全面的驾驶安全需要同时监控驾驶员和道路视图以检测风险事件（如驾驶时使用手机），这要求模型能处理同步的多视角输入。

Method: 研究开发了模型，构建了一个专用数据集，并在此数据集上评估了LVLMs的性能和能力。

Result: 实验结果表明，预训练的LVLMs效果有限，但经过微调的LVLMs能够生成准确且具有安全意识的驾驶指令。然而，在检测视频中细微或复杂事件方面仍存在挑战。

Conclusion: 微调LVLMs在处理多视角驾驶安全任务中表现出有效性，但检测细微或复杂事件是其主要挑战。本研究的发现和误差分析为改进该领域的LVLM系统提供了有价值的见解。
...
### [152] [Rethinking Cross-Generator Image Forgery Detection through DINOv3](https://arxiv.org/abs/2511.22471)
*Zhenglin Huang,Jason Li,Haiquan Wen,Tianxiao Li,Xi Yang,Lu Qi,Bei Peng,Xiaowei Huang,Ming-Hsuan Yang,Guangliang Cheng*

Main category: cs.CV

TL;DR: 随着生成模型日益多样化，跨生成器检测成为新挑战。本研究发现，冻结的视觉基础模型（特别是DINOv3）在未微调的情况下已展现出强大的跨生成器检测能力，其依赖于全局、低频的真实性线索。我们提出的无训练token排序策略进一步提升了检测精度，为图像伪造检测提供了一个通用、高效且可解释的基线。


<details>
  <summary>Details</summary>
Motivation: 现有的生成器检测方法通常记忆特定生成模型的伪影，而非学习可迁移的线索，导致在未见过的生成器上表现不佳。因此，需要开发一种能够有效应对多样化生成器的通用检测方法。

Method: 本研究系统地探究了冻结的视觉基础模型（特别是DINOv3）在频率、空间和token维度上的行为。基于观察到的DINOv3倾向于依赖全局、低频结构作为可迁移的真实性线索，我们引入了一种简单、无需训练的token排序策略，并结合轻量级线性探针来选择少量与真实性相关的token子集。

Result: 研究发现，冻结的视觉基础模型（尤其是DINOv3）在不进行任何微调的情况下，已展现出强大的跨生成器检测能力。DINOv3倾向于利用全局、低频结构作为微弱但可迁移的真实性线索，而非高频、特定生成器的伪影。提出的token排序策略能持续提高所有评估数据集上的检测精度。

Conclusion: 本研究提供了经验证据和可行假设，以解释基础模型为何能在多样化生成器之间泛化。通过利用全局、低频的真实性线索，DINOv3及其改进方法为图像伪造检测提供了一个通用、高效且可解释的基线。
...
### [153] [Simultaneous Image Quality Improvement and Artefacts Correction in Accelerated MRI](https://arxiv.org/abs/2511.23274)
...
### [154] [Fast3Dcache: Training-free 3D Geometry Synthesis Acceleration](https://arxiv.org/abs/2511.22533)
...
### [155] [What Shape Is Optimal for Masks in Text Removal?](https://arxiv.org/abs/2511.22499)
*Hyakka Nakada,Marika Kubota*

Main category: cs.CV

TL;DR: 本文针对文档图像中密集文本的移除问题，创建了基准数据集，发现文本移除性能易受掩码轮廓扰动影响。为此，提出一种使用贝叶斯优化学习灵活掩码轮廓的方法，结果显示字符级掩码更有效，且最小文本区域覆盖并非最优，为手动掩码提供了指导。


<details>
  <summary>Details</summary>
Motivation: 现有文本移除方法多关注简单场景文本，而对工业应用中重要的、包含密集文本的复杂文档图像研究较少。此外，文本移除性能易受掩码轮廓扰动影响，需要精确的掩码形状调整。

Method: 创建了包含大量文本的图像文本移除基准数据。开发了一种方法，利用贝叶斯优化建模高度灵活的掩码轮廓并学习其参数。

Result: 研究发现文本移除性能对掩码轮廓扰动敏感，精确调整掩码形状至关重要。所开发的方法产生了字符级掩码，并且发现文本区域的最小覆盖并非最优。

Conclusion: 本研究有望为手动掩码提供用户友好的指导方针，特别是在处理文档图像中的密集文本移除任务时。
...
TL;DR: 流式生成模型性能优异但采样成本高。现有方法如Rectified Flow和MeanFlow存在多步迭代或收敛慢的问题。本文提出Rectified MeanFlow，通过单步整流轨迹建模平均速度场，实现高效训练和高质量单步采样，并在ImageNet上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 流式生成模型（如Rectified Flow和MeanFlow）在采样和训练效率方面存在局限性：Rectified Flow需要多步计算密集型整流迭代才能实现路径平直；MeanFlow在面对高度弯曲的流时收敛缓慢且监督噪声大。研究旨在解决这些限制，实现更高效的单步采样。

Method: 本文提出Rectified MeanFlow框架。它通过仅使用单步整流（reflow）步骤，沿着整流轨迹建模平均速度场。这种方法无需完美直线轨迹，即可实现高效训练。此外，引入了一种简单有效的截断启发式算法，以减少残余曲率并进一步提高性能。

Result: 在ImageNet数据集（64、256和512分辨率）上进行的广泛实验表明，Re-MeanFlow在样本质量和训练效率方面均持续优于先前的单步流蒸馏和Rectified Flow方法。

Conclusion: Rectified MeanFlow通过建模单步整流轨迹的平均速度场，并结合截断启发式算法，成功解决了现有流式生成模型在效率和质量上的局限性，实现了卓越的单步生成性能和训练效率。
...
### [157] [Bringing Your Portrait to 3D Presence](https://arxiv.org/abs/2511.22553)
*Jiawei Zhang,Lei Chu,Jiahao Li,Zhenyu Zang,Chong Li,Xiao Li,Xun Cao,Hao Zhu,Yan Lu*

Main category: cs.CV

TL;DR: 该论文提出一个统一框架，能从单张肖像照（头部、半身或全身）重建可动画的3D人体化身，解决了特征表示、数据扩展和代理网格估计中的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决从单张图像重建3D人体化身时面临的三个主要挑战：对姿态和构图敏感的特征表示、可扩展数据有限以及代理网格估计不可靠。

Method: 该方法引入了Dual-UV表示，通过Core-UV和Shell-UV分支将图像特征映射到规范UV空间，消除姿态和构图引起的特征偏移。同时，构建了一个结合2D生成多样性与几何一致3D渲染的分解合成数据流形，并辅以改进真实感和身份一致性的训练方案。此外，还采用了一个鲁棒的代理网格跟踪器以在部分可见性下保持稳定性。

Result: 该模型仅使用半身合成数据进行训练，却在头部和上半身重建方面达到了最先进水平，并在全身重建方面取得了具有竞争力的结果。它展现出强大的野外泛化能力。

Conclusion: 所提出的统一框架及其组成部分（Dual-UV表示、分解合成数据流形和鲁棒代理网格跟踪器）有效地解决了3D人体化身重建的挑战，实现了从单张图像进行高质量、可动画的重建，并具有出色的泛化能力。
...
### [158] [Diff-ICMH: Harmonizing Machine and Human Vision in Image Compression with Generative Prior](https://arxiv.org/abs/2511.22549)
*Ruoyu Feng,Yunpeng Qi,Jinming Liu,Yixin Gao,Xin Li,Xin Jin,Zhibo Chen*

Main category: cs.CV

TL;DR: Diff-ICMH是一个生成式图像压缩框架，旨在通过结合生成先验和语义一致性损失，同时优化人类感知和机器分析任务的图像压缩，并引入标签引导模块以提高语义保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的图像压缩方法通常是为人类感知或机器分析任务独立优化的。本研究旨在揭示这些目标之间的根本共性，即准确的语义信息对于智能任务和人类理解至关重要，同时增强的感知质量也能通过确保真实的图像分布而有益于机器任务的语义特征提取。

Method: 本文提出了Diff-ICMH框架，利用生成先验来确保感知真实性，并通过在训练期间引入语义一致性损失（SC loss）来保证语义保真度。此外，还引入了标签引导模块（TGM），利用高度语义化的图像级标签来激发预训练扩散模型的生成能力，只需极少的额外比特率。

Result: Diff-ICMH通过单一编解码器和比特流支持多种智能任务，无需任何任务特定适配，同时为人类感知保留了高质量的视觉体验。广泛的实验结果证明了Diff-ICMH在各种任务中的优越性和泛化性，并保持了人类感知的视觉吸引力。

Conclusion: Diff-ICMH成功地在图像压缩中协调了机器和人类视觉，通过一个统一的生成式框架，实现了语义保真度和感知质量的双重优化，并在多任务中展现出卓越的性能和泛化能力。
...
### [159] [Artwork Interpretation with Vision Language Models: A Case Study on Emotions and Emotion Symbols](https://arxiv.org/abs/2511.22929)
*Sebastian Padó,Kerstin Thomas*

Main category: cs.CV

TL;DR: 本文探讨了视觉语言模型（VLM）在艺术作品中检测情感表达的能力。研究发现，VLM能很好地识别图像内容和具体情感，但在处理高度抽象或象征性图像时表现不佳，且存在回答不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 情感是艺术表达的基本要素，其在艺术品中的实现方式多样且抽象，分析需要艺术史专业知识。本研究旨在探究当前视觉语言模型（VLM）能识别艺术品中哪些情感表达方面。

Method: 研究采用案例分析法，选取三个VLM（Llava-Llama和两个Qwen模型），向它们提出四组复杂度递增的问题（关于艺术品的通用内容、情感内容、情感表达和情感符号）。随后，对模型的回答进行定性专家评估。

Result: 研究发现，VLM在识别图像内容方面表现出色，并且通常能识别所描绘的情感及其表达方式。模型在处理具体图像时效果最佳，但对高度抽象或高度象征性的图像则表现不佳。对符号的可靠识别仍然非常困难。此外，模型还表现出大型语言模型常见的弱点，即对相关问题提供不一致的答案。

Conclusion: 视觉语言模型在识别艺术作品中的情感表达方面具有一定潜力，尤其对于具体图像。然而，它们在处理抽象和象征性内容以及提供一致性回答方面仍面临显著挑战。
...
### [160] [Text Condition Embedded Regression Network for Automated Dental Abutment Design](https://arxiv.org/abs/2511.22578)
*Mianjie Zheng,Xinquan Yang,Xuguang Li,Xiaoling Luo,Xuefen Liu,Kun Tang,He Meng,Linlin Shen*

Main category: cs.CV

TL;DR: 本文提出了一种名为TCEAD的文本条件嵌入式基台设计框架，通过结合自监督学习和文本引导定位，实现了牙科植体基台的自动化设计，显著提高了设计效率和精度。


<details>
  <summary>Details</summary>
Motivation: 人工牙科植体基台设计耗时费力，不当设计可能导致种植体并发症（如种植体周围炎）。利用人工智能辅助设计可以快速提高设计效率并增强基台适应性。

Method: 本文提出了TCEAD框架，该框架扩展了MeshMAE自监督学习框架，并引入了文本引导定位（TGL）模块以促进基台区域定位。为了提高模型特征提取能力，编码器使用口腔扫描数据进行预训练。TGL模块通过对比语言-图像预训练（CLIP）的文本编码器引入基台区域描述，使网络能够快速定位基台区域。

Result: 在大型基台设计数据集上进行的广泛实验表明，TCEAD框架在交并比（IoU）方面比其他主流方法提高了0.8%至12.85%。

Conclusion: TCEAD框架在自动化牙科植体基台设计方面展现出巨大潜力，能够显著提高设计效率和基台适应性，为牙科植体设计提供了一种新颖的自动化解决方案。
...
### [161] [Stable-Drift: A Patient-Aware Latent Drift Replay Method for Stabilizing Representations in Continual Learning](https://arxiv.org/abs/2511.22615)
*Paraskevi-Antonia Theofilou,Anuhya Thota,Stefanos Kollias,Mamatha Thota*

Main category: cs.CV

TL;DR: 为解决深度学习模型在医学影像中灾难性遗忘问题，本文提出一种潜在漂移引导的重放方法。该方法通过量化样本的潜在漂移来识别高表征不稳定性的样本进行重放，有效减少了模型遗忘。


<details>
  <summary>Details</summary>
Motivation: 当深度学习模型在新数据上顺序训练时，它们会突然失去对先前学习任务的性能，即灾难性遗忘。这严重限制了AI在医学影像中的部署，因为模型必须不断适应来自新医院的数据，同时不损害已建立的诊断知识。

Method: 本文引入了一种潜在漂移引导的重放方法。该方法通过在天真域适应后量化样本内部特征表示的变化（即潜在漂移）来量化表征不稳定性。为确保多样性和临床相关性，漂移在患者层面进行聚合，记忆缓冲区存储每个患者中表现出最大多层表征偏移的切片。

Result: 在跨医院COVID-19 CT分类任务中，使用最先进的CNN和Vision Transformer骨干网络进行评估，本文方法与天真微调和随机重放相比，显著减少了遗忘。

Conclusion: 这项工作强调了潜在漂移作为一种实用且可解释的重放信号，可用于推进真实世界医疗环境中的鲁棒持续学习。
...
### [162] [AnoRefiner: Anomaly-Aware Group-Wise Refinement for Zero-Shot Industrial Anomaly Detection](https://arxiv.org/abs/2511.22595)
*Dayou Huang,Feng Xue,Xurui Li,Yu Zhou*

Main category: cs.CV

TL;DR: 本文提出AnoRefiner，一个可插入式模块，通过利用异常分数图和渐进式组内测试时训练策略，将零样本异常检测（ZSAD）模型产生的粗粒度异常图精细化至像素级别。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本工业异常检测（ZSAD）方法通常由于Vision Transformers（ViTs）仅提取补丁级特征而产生粗粒度异常图。近期解决方案尝试利用ZSAD特征预测更精细的异常，但由于合成训练异常与真实异常之间的差距，难以恢复细粒度异常且存在漏检。研究发现，异常分数图提供了ZSAD图像特征中缺失的互补空间线索，这一点之前被忽视了。

Method: 本文提出了AnoRefiner模块，可用于改进大多数ZSAD模型。首先，设计了一个异常细化解码器（ARD），它利用异常分数图逐步增强图像特征，从而减少对合成异常数据的依赖。其次，提出了一种渐进式组内测试时训练（PGT）策略，在每个产品组中训练ARD，用于下一个组的细化过程，同时保持与任何ZSAD方法的兼容性。

Result: 在MVTec AD和VisA数据集上的实验表明，AnoRefiner将各种ZSAD模型的像素AP指标提高了高达5.2%，并且在许多可视化结果中也能直接观察到改进。

Conclusion: AnoRefiner通过利用异常分数图和创新的渐进式组内测试时训练策略，成功地将零样本异常检测模型输出的补丁级异常图提升至像素级精度，显著提高了性能。
...
### [163] [REASONEDIT: Towards Reasoning-Enhanced Image Editing Models](https://arxiv.org/abs/2511.22625)
...
TL;DR: 通过解锁多模态大语言模型（MLLM）的推理能力（思考与反思），本文提出了一种图像编辑新框架，显著提升了指令理解和编辑准确性。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型（如Step1X-Edit和Qwen-Image-Edit）中，MLLM编码器在训练期间保持冻结，限制了其推理能力的发挥。研究者认为，解锁MLLM的推理能力可以进一步提升编辑模型的性能。

Method: 本文提出了一种“思考-编辑-反思”循环的图像编辑框架。其中，“思考”机制利用MLLM的知识解释抽象指令；“反思”机制则审查编辑结果，自动纠正意外操作，并判断何时停止编辑循环。

Result: 实验结果显示，当从Step1X-Edit初始化时（ReasonEdit-S），该推理方法在ImgEdit、GEdit和Kris上分别取得了+4.3%、+4.7%和+8.2%的显著性能提升。当与Qwen-Image-Edit集成时（ReasonEdit-Q），在GEdit和Kris上均优于之前的开源方法。

Conclusion: 通过在图像编辑模型中引入MLLM的思考和反思推理机制，可以有效增强指令理解和编辑准确性，从而显著提升图像编辑模型的整体性能。
...
### [164] [GeoZero: Incentivizing Reasoning from Scratch on Geospatial Scenes](https://arxiv.org/abs/2511.22645)
...
TL;DR: GeoZero是一个无需预定义思维链（CoT）监督的框架，通过监督微调和强化学习，结合A$^2$GRPO策略优化，使多模态大语言模型（MLLMs）在遥感地理空间推理任务中超越现有SOTA，并促进通用涌现推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感MLLMs通过精心策划的CoT数据进行冷启动训练，但这种方法不仅标注成本高昂，还引入了人为偏见，限制了模型推理的多样性。

Method: 提出GeoZero框架，使MLLMs无需预定义CoT监督即可进行地理空间推理。具体方法包括：1) 构建GeoZero-Instruct数据集，通过监督微调使模型获得初步地理空间知识。2) 构建GeoZero-Hard数据集，在后续强化学习阶段激发深度推理。3) 引入回答锚定组相对策略优化（A$^2$GRPO），通过模型自身的答案来规范推理过程，鼓励多样而准确的思考。

Result: GeoZero在多个遥感视觉-语言基准测试中不仅超越了现有的最先进方法，而且在各种地理空间任务中培养了通用的涌现推理能力。

Conclusion: GeoZero框架有效地解决了传统CoT训练的成本和偏见问题，通过无CoT监督的学习范式，显著提升了MLLMs在地理空间场景理解中的推理能力和泛化性。
...
### [165] [Architecture Decoupling Is Not All You Need For Unified Multimodal Model](https://arxiv.org/abs/2511.22663)
...
TL;DR: 本文提出了一种名为注意力交互对齐（AIA）损失的新方法，旨在不进行模型解耦的情况下，解决统一多模态模型中生成与理解任务之间的冲突，从而提升模型在两方面的性能。


<details>
  <summary>Details</summary>
Motivation: 统一的多模态模型是实现AGI的关键一步，但其生成和理解任务之间存在固有的冲突，导致难以建立最优的训练范式。虽然模型解耦（如双图像编码器、MOE/MOT架构）可以缓解冲突，但过度解耦会损害统一模型原本的交错生成能力。因此，研究目标是在不解耦模型的前提下缓解任务冲突。

Method: 首先，通过研究模型的跨模态注意力行为，分析了模型解耦为何能缓解冲突，并观察到解耦本质上促使模型形成任务特定的多模态交互模式。受此启发，提出了注意力交互对齐（AIA）损失，该损失在训练过程中明确学习任务特定的多模态交互模式。为验证其普适性，AIA损失被应用于Emu3（在SFT阶段）和Janus-Pro（在后训练阶段）。

Result: 在不增加额外复杂性的情况下，AIA损失不仅优化了跨模态注意力模式，还显著提升了模型的生成和理解性能。

Conclusion: AIA损失通过显式学习任务特定的多模态交互模式，有效缓解了统一多模态模型中生成与理解任务的冲突，且无需进行模型解耦。该方法具有普适性，并能同时提高模型的生成和理解能力。
...
### [166] [A deep learning perspective on Rubens' attribution](https://arxiv.org/abs/2511.22667)
*A. Afifi,A. Kalimullin,S. Korchagin,I. Kudryashov*

Main category: cs.CV

TL;DR: 本研究利用深度学习（卷积神经网络）对鲁本斯及其工作室的画作进行认证和归属分析，通过识别微观风格特征，实现了高分类准确率，并为艺术史研究提供了新的计算分析工具。


<details>
  <summary>Details</summary>
Motivation: 解决彼得·保罗·鲁本斯及其工作室画作认证和归属的复杂问题，以补充传统的艺术史专业知识，并提供新的视角。

Method: 训练一个卷积神经网络（CNN），使用经过精心策划的已验证和对比艺术品数据集，以识别大师手法的微观风格特征。

Result: 该模型实现了高分类准确率，展示了计算分析在补充传统艺术史专业知识方面的潜力。

Conclusion: 计算分析为作者身份和工作室协作提供了新的见解，并能有效补充传统艺术史研究方法。
...
### [167] [VaMP: Variational Multi-Modal Prompt Learning for Vision-Language Models](https://arxiv.org/abs/2511.22664)
*Silin Cheng,Kai Han*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的变分多模态提示学习（VaMP）框架，通过生成样本特定、不确定性感知的提示，有效地将视觉-语言模型（VLMs）适应到下游任务，并在少样本和域泛化基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言模型（如CLIP）在零样本设置下表现出强大的泛化能力，但将其适应到监督有限的下游任务仍然是一个重大挑战。现有的多模态提示学习方法通常依赖于固定、共享的提示和确定性参数，这限制了它们捕获实例级变化或建模跨不同任务和领域不确定性的能力。

Method: VaMP框架通过从学习到的后验分布中采样来生成实例条件的提示，从而实现样本特定、不确定性感知的提示调整。为增强局部和全局语义的整合，引入了源自实例表示和类别原型的类别感知先验。在此基础上，将提示调整公式化为潜在提示表示上的变分推断，并通过重参数化采样端到端地训练整个框架。

Result: 在少样本和域泛化基准测试中的实验表明，VaMP取得了最先进的性能，突显了其方法中建模不确定性和任务结构所带来的优势。

Conclusion: VaMP通过对不确定性和任务结构进行建模，有效地解决了VLM在有限监督下适应下游任务的挑战，显著提升了模型性能和泛化能力。
...
### [168] [Decoupled DMD: CFG Augmentation as the Spear, Distribution Matching as the Shield](https://arxiv.org/abs/2511.22677)
*Dongyang Liu,Peng Gao,David Liu,Ruoyi Du,Zhen Li,Qilong Wu,Xin Jin,Sihan Cao,Shifeng Zhang,Hongsheng Li,Steven Hoi*

Main category: cs.CV

TL;DR: 本文挑战了扩散模型蒸馏中分布匹配蒸馏（DMD）的核心机制的传统理解，认为在需要CFG的场景下，CFG增强（CA）是少数步数蒸馏的主要驱动力，而分布匹配（DM）则作为正则化器。


<details>
  <summary>Details</summary>
Motivation: DMD及其变体在扩散模型蒸馏中表现出色，其性能通常归因于学生模型输出分布与教师模型匹配的核心机制。本文旨在挑战这一传统观点，特别是在文本到图像生成等复杂任务中，CFG对于理想的少数步数性能至关重要。

Method: 通过对DMD训练目标的严格分解，本文揭示了CFG增强（CA）是少数步数蒸馏的主要驱动力，而分布匹配（DM）则作为正则化器。通过展示其他简单的非参数约束或基于GAN的目标也可以实现相同的稳定功能，验证了这种解耦。在此基础上，提出了对蒸馏过程的改进，例如解耦引擎和正则化器的噪声调度。

Result: 研究发现，在需要CFG的复杂任务中，CFG增强（CA）是少数步数蒸馏的核心“引擎”，而分布匹配（DM）项则作为“正则化器”，确保训练稳定性并减轻伪影。DM作为正则化器并非独一无二，更简单的非参数约束或GAN目标也能起到类似作用。基于这种新理解，通过解耦引擎和正则化器的噪声调度等方法，实现了性能提升。该方法已被Z-Image项目采纳，开发出顶级的8步图像生成模型。

Conclusion: 本文对DMD机制提出了新的、更具原则性的理解：CFG增强是蒸馏的核心驱动力，而分布匹配是正则化器。这种解耦促进了对两项属性的系统分析，并能够提出有原则的修改以进一步提高性能。
...
### [169] [World in a Frame: Understanding Culture Mixing as a New Challenge for Vision-Language Models](https://arxiv.org/abs/2511.22787)
...
TL;DR: 本研究调查了大型视觉语言模型（LVLMs）在文化混合场景中的表现。通过构建CultureMix基准，发现LVLMs难以保留文化身份且高度依赖背景。监督式微调显著提高了模型一致性和降低了背景敏感性。


<details>
  <summary>Details</summary>
Motivation: 在全球化世界中，来自不同文化的元素频繁出现在同一视觉场景中（即文化混合场景）。目前，LVLMs如何感知这些场景仍未得到充分探索，这对其在文化多样性环境中的可靠运行构成了严峻挑战。

Method: 构建了CultureMix，一个包含2.3万张扩散生成并经人工验证的文化混合图像的食物视觉问答（VQA）基准。该基准涵盖四个子任务：(1) 仅食物，(2) 食物+食物，(3) 食物+背景，和 (4) 食物+食物+背景。评估了10个LVLMs，并探索了三种鲁棒性策略，包括使用多样化文化混合数据集进行监督式微调。

Result: LVLMs在混合设置中普遍未能保留独立的文化身份。模型表现出强烈的背景依赖性，当添加文化背景时，准确率比仅食物基线下降14%。对于相同食物在不同背景下会产生不一致的预测。监督式微调使用多样化文化混合数据集显著提高了模型的一致性并降低了背景敏感性。

Conclusion: 文化混合场景对LVLMs是一个关键挑战。需要更多关注这些场景，并开发更强大的LVLMs，使其能够在文化多元的真实世界环境中可靠运行。
...
### [170] [Fusion or Confusion? Assessing the impact of visible-thermal image fusion for automated wildlife detection](https://arxiv.org/abs/2511.22768)
...
### [171] [Ar2Can: An Architect and an Artist Leveraging a Canvas for Multi-Human Generation](https://arxiv.org/abs/2511.22690)
...
### [172] [Splat-SAP: Feed-Forward Gaussian Splatting for Human-Centered Scene with Scale-Aware Point Map Reconstruction](https://arxiv.org/abs/2511.22704)
...
### [173] [From Pixels to Feelings: Aligning MLLMs with Human Cognitive Perception of Images](https://arxiv.org/abs/2511.22805)
...
### [174] [Emergent Extreme-View Geometry in 3D Foundation Models](https://arxiv.org/abs/2511.22686)
...
### [175] [Alzheimer's Disease Prediction Using EffNetViTLoRA and BiLSTM with Multimodal Longitudinal MRI Data](https://arxiv.org/abs/2511.22774)
...
### [176] [Z-Image: An Efficient Image Generation Foundation Model with Single-Stream Diffusion Transformer](https://arxiv.org/abs/2511.22699)
...
### [177] [LC4-DViT: Land-cover Creation for Land-cover Classification with Deformable Vision Transformer](https://arxiv.org/abs/2511.22812)
...
### [178] [Captain Safari: A World Engine](https://arxiv.org/abs/2511.22815)
...
### [179] [Some Modalities are More Equal Than Others: Decoding and Architecting Multimodal Integration in MLLMs](https://arxiv.org/abs/2511.22826)
...
### [180] [Breaking the Visual Shortcuts in Multimodal Knowledge-Based Visual Question Answering](https://arxiv.org/abs/2511.22843)
...
### [181] [DM$^3$T: Harmonizing Modalities via Diffusion for Multi-Object Tracking](https://arxiv.org/abs/2511.22896)
...
### [182] [CoordSpeaker: Exploiting Gesture Captioning for Coordinated Caption-Empowered Co-Speech Gesture Generation](https://arxiv.org/abs/2511.22863)
...
### [183] [GLOW: Global Illumination-Aware Inverse Rendering of Indoor Scenes Captured with Dynamic Co-Located Light & Camera](https://arxiv.org/abs/2511.22857)
...
### [184] [ClearGCD: Mitigating Shortcut Learning For Robust Generalized Category Discovery](https://arxiv.org/abs/2511.22892)
...
### [185] [Resolving Evidence Sparsity: Agentic Context Engineering for Long-Document Understanding](https://arxiv.org/abs/2511.22850)
...
### [186] [CNN-Based Framework for Pedestrian Age and Gender Classification Using Far-View Surveillance in Mixed-Traffic Intersections](https://arxiv.org/abs/2511.22873)
...
### [187] [Scalable Diffusion Transformer for Conditional 4D fMRI Synthesis](https://arxiv.org/abs/2511.22870)
...
### [188] [From Points to Clouds: Learning Robust Semantic Distributions for Multi-modal Prompts](https://arxiv.org/abs/2511.22897)
...
### [189] [See, Rank, and Filter: Important Word-Aware Clip Filtering via Scene Understanding for Moment Retrieval and Highlight Detection](https://arxiv.org/abs/2511.22906)
...
### [190] [ViGG: Robust RGB-D Point Cloud Registration using Visual-Geometric Mutual Guidance](https://arxiv.org/abs/2511.22908)
...
### [191] [One-to-All Animation: Alignment-Free Character Animation and Image Pose Transfe](https://arxiv.org/abs/2511.22940)
...
### [192] [Barcode and QR Code Object Detection: An Experimental Study on YOLOv8 Models](https://arxiv.org/abs/2511.22937)
...
### [193] [HMR3D: Hierarchical Multimodal Representation for 3D Scene Understanding with Large Vision-Language Model](https://arxiv.org/abs/2511.22961)
...
### [194] [NeuMatC: A General Neural Framework for Fast Parametric Matrix Operation](https://arxiv.org/abs/2511.22934)
...
### [195] [Do We Need Perfect Data? Leveraging Noise for Domain Generalized Segmentation](https://arxiv.org/abs/2511.22948)
...
### [196] [DenoiseGS: Gaussian Reconstruction Model for Burst Denoising](https://arxiv.org/abs/2511.22939)
...
### [197] [Robust Image Self-Recovery against Tampering using Watermark Generation with Pixel Shuffling](https://arxiv.org/abs/2511.22936)
...
### [198] [Contrastive Heliophysical Image Pretraining for Solar Dynamics Observatory Records](https://arxiv.org/abs/2511.22958)
...
### [199] [Taming the Light: Illumination-Invariant Semantic 3DGS-SLAM](https://arxiv.org/abs/2511.22968)
...
### [200] [BlockVid: Block Diffusion for High-Quality and Consistent Minute-Long Video Generation](https://arxiv.org/abs/2511.22973)
...
### [201] [McSc: Motion-Corrective Preference Alignment for Video Generation with Self-Critic Hierarchical Reasoning](https://arxiv.org/abs/2511.22974)
...
### [202] [JarvisEvo: Towards a Self-Evolving Photo Editing Agent with Synergistic Editor-Evaluator Optimization](https://arxiv.org/abs/2511.23002)
...
### [203] [Guiding Visual Autoregressive Models through Spectrum Weakening](https://arxiv.org/abs/2511.22991)
...
### [204] [Geometry-Consistent 4D Gaussian Splatting for Sparse-Input Dynamic View Synthesis](https://arxiv.org/abs/2511.23044)
...
### [205] [Convolutional Feature Noise Reduction for 2D Cardiac MR Image Segmentation](https://arxiv.org/abs/2511.22983)
...
### [206] [MultiBanana: A Challenging Benchmark for Multi-Reference Text-to-Image Generation](https://arxiv.org/abs/2511.22989)
...
### [207] [GOATex: Geometry & Occlusion-Aware Texturing](https://arxiv.org/abs/2511.23051)
...
### [208] [Optimizer Sensitivity In Vision Transformerbased Iris Recognition: Adamw Vs Sgd Vs Rmsprop](https://arxiv.org/abs/2511.22994)
...
### [209] [NumeriKontrol: Adding Numeric Control to Diffusion Transformers for Instruction-based Image Editing](https://arxiv.org/abs/2511.23105)
*Zhenyu Xu,Xiaoqi Shen,Haotian Nan,Xinyu Zhang*

Main category: cs.CV

TL;DR: NumeriKontrol是一个图像编辑框架，通过引入带单位的连续数值，实现对扩散模型中图像属性的精确、连续和可控的编辑强度调节。


<details>
  <summary>Details</summary>
Motivation: 现有的基于文本指令的图像编辑方法，在实现对编辑强度的精细控制时，往往缺乏足够的精确性。

Method: NumeriKontrol框架通过一个有效的“数值适配器”（Numeric Adapter）来编码数值编辑尺度，并以即插即用的方式将其注入到扩散模型中。它采用任务分离设计，支持零样本多条件编辑。为了提供高质量的监督，研究者从高保真渲染引擎和单反相机等可靠来源合成了精确的训练数据，构建了“通用属性变换”（CAT）数据集。

Result: 广泛的实验表明，NumeriKontrol在各种属性编辑场景中，都能提供准确、连续且稳定的尺度控制。

Conclusion: 这些贡献通过实现精确、可扩展和用户可控的图像操作，推动了基于指令的图像编辑技术的发展。
...
### [210] [MathSight: A Benchmark Exploring Have Vision-Language Models Really Seen in University-Level Mathematical Reasoning?](https://arxiv.org/abs/2511.23112)
*Yuandong Wang,Yao Cui,Yuxin Zhao,Zhen Yang,Yangfu Zhu,Zhenzhou Shao*

Main category: cs.CV

TL;DR: 研究发现，在多模态数学推理中，视觉信息对VLM的贡献随着问题难度的增加而减弱，甚至纯文本模型在某些情况下表现优于多模态模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准未能有效隔离视觉信息在多模态数学推理中的作用，导致不清楚VLM是真正利用视觉理解还是仅依赖语言先验。

Method: 提出了MathSight基准，该基准包含大学级别的多模态数学推理问题，并为每个问题设计了多种视觉变体（原始、手绘、拍照）以及纯文本条件，以便进行受控比较，解耦并量化视觉输入的影响。

Result: 实验结果显示，视觉信息对推理的贡献随着问题难度的增加而持续减弱。令人惊讶的是，不带任何图像输入的Qwen3-VL在某些情况下甚至超越了其多模态版本和GPT-5。

Conclusion: 研究强调了像MathSight这样的基准的重要性，以推动未来模型中真正基于视觉的推理能力发展，而非仅仅依赖语言信息。
...
### [211] [Implementation of a Skin Lesion Detection System for Managing Children with Atopic Dermatitis Based on Ensemble Learning](https://arxiv.org/abs/2511.23082)
*Soobin Jeon,Sujong Kim,Dongmahn Seo*

Main category: cs.CV

TL;DR: 本研究提出了一种基于集成学习的皮肤病变检测系统（ENSEL），旨在通过整合多个深度学习模型，提高特应性皮炎等皮肤病的诊断准确性，并实现快速响应，以促进数字医疗发展。


<details>
  <summary>Details</summary>
Motivation: 韩国《数据3法案》的修订和COVID-19疫情推动了数字医疗市场的发展和医疗数据在人工智能中的应用。特应性皮炎诊断依赖主观评估，易误诊且外观与牛皮癣相似，诊断复杂。现有皮肤病研究多使用高质量皮肤镜图像，但在实际临床中难以获取，且现有系统需兼顾准确性和响应速度。

Method: 本研究提出了一个基于集成学习的皮肤病变检测系统（ENSEL）。该系统通过集成多种深度学习模型来提高诊断准确性。其性能通过使用实际用户拍摄的皮肤病变图像进行检测实验来验证，并测量了对随机采样的皮肤病图像的准确性和响应时间。

Result: 实验结果表明，ENSEL在大多数图像中实现了高召回率，并且处理速度小于1秒。

Conclusion: 本研究提出的ENSEL系统有助于皮肤病变的客观诊断，并推动了数字医疗的进步。
...
### [212] [DNA-Prior: Unsupervised Denoise Anything via Dual-Domain Prior](https://arxiv.org/abs/2511.23124)
...
TL;DR: DNA-Prior是一种通用的无监督去噪框架，通过结合深度网络参数化和显式谱空间先验的混合先验，直接从受损观测中重建清晰的医学图像，无需外部训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有的去噪方法依赖于大型标注数据集或监督学习，这在临床环境中因模态异构和真实数据有限而受到限制。

Method: DNA-Prior通过一个数学原理化的混合先验来重建图像，该先验包括：(i)通过深度网络参数化实现的隐式架构先验，以及(ii)由频域保真项和空间正则化泛函组成的显式谱空间先验。这种双域公式形成了一个结构良好的优化问题。

Result: 在多种模态和不同噪声条件下，DNA-Prior都表现出一致的噪声抑制和结构保持能力。

Conclusion: DNA-Prior提供了一个通用的无监督去噪解决方案，克服了医学成像中数据限制的挑战，能够有效地在无需外部训练数据或特定模态调优的情况下，同时保留全局频率特征和局部解剖结构。
...
### [213] [db-SP: Accelerating Sparse Attention for Visual Generative Models with Dual-Balanced Sequence Parallelism](https://arxiv.org/abs/2511.23113)
*Siqi Chen,Ke Hong,Tianchen Zhao,Ruiqi Xie,Zhenhua Zhu,Xudong Zhang,Yu Wang*

Main category: cs.CV

TL;DR: 本文提出db-SP，一种稀疏感知的序列并行技术，通过双层分区和动态并行度确定，解决了块级稀疏注意力模型在序列并行推理中存在的负载不平衡问题，显著提高了扩散Transformer (DiT) 的推理速度。


<details>
  <summary>Details</summary>
Motivation: 在视觉生成中，通过序列并行扩展扩散Transformer (DiT) 推理对于降低延迟至关重要。然而，当应用于采用块级稀疏注意力的模型时，由于注意力头之间稀疏性的固有差异以及稀疏掩码中密集块的不规则分布，现有的序列并行方法（如Ulysses和Ring Attention）会导致严重的负载不平衡。

Method: 本文提出了db-SP（sparsity-aware sequence parallelism），一种稀疏感知的序列并行技术。它包含一个双层分区方法，可以在头部和块级别实现近乎完美的负载平衡，且开销可忽略不计。此外，为了处理去噪步骤和层之间不断演变的稀疏模式，db-SP在运行时动态确定头部和块维度的并行度。

Result: 实验结果表明，db-SP相对于最先进的序列并行方法，平均端到端加速比达到1.25倍，注意力特定加速比达到1.40倍。

Conclusion: db-SP通过其双层分区和动态并行度确定机制，成功解决了稀疏扩散Transformer推理中的负载不平衡问题，显著提升了推理性能，为视觉生成模型的扩展提供了有效的解决方案。
...
### [214] [Buffer replay enhances the robustness of multimodal learning under missing-modality](https://arxiv.org/abs/2511.23070)
*Hongye Zhu,Xuan Liu,Yanwen Ba,Jingye Xue,Shigeng Zhang*

Main category: cs.CV

TL;DR: 本文提出REplay Prompting (REP) 方法，通过构建模态特征缓冲区、私有-共享特征解耦和任务感知动态初始化机制，有效解决多模态模型中模态缺失导致的性能下降问题，且开销极低。


<details>
  <summary>Details</summary>
Motivation: 多模态模型在模态缺失时性能显著下降。现有方法要么计算成本高昂（合成缺失模态），要么仅依赖相邻层特征进行提示微调而忽略长距离上下文信息，导致对错误容忍度不足。

Method: REplay Prompting (REP) 包含三部分：1) 通过残差旁路构建模态特征缓冲区，缓存早期层表示并在深层重放，以减轻信息损失；2) 采用私有-共享特征解耦策略，私有缓冲区保留模态特异性信号，共享缓冲区编码跨模态语义；3) 设计任务感知动态初始化机制，差异化配置缓冲区，提高在不同模态缺失条件下的稳定性和泛化能力。

Result: 在视觉-语言、视觉-语言-音频和时间多模态基准测试中，REP 在单模态和多模态缺失场景下均持续优于现有方法，且仅引入可忽略的参数开销。

Conclusion: REP 是一种轻量级且有效的范式，可在具有挑战性的模态缺失环境中实现鲁棒的多模态学习。
...
### [215] [DualCamCtrl: Dual-Branch Diffusion Model for Geometry-Aware Camera-Controlled Video Generation](https://arxiv.org/abs/2511.23127)
*Hongfei Zhang,Kanghao Chen,Zixin Zhang,Harold Haodong Chen,Yuanhuiyi Lyu,Yuqi Zhang,Shuai Yang,Kun Zhou,Yingcong Chen*

Main category: cs.CV

TL;DR: 本文提出了DualCamCtrl，一个端到端的扩散模型，用于相机控制的视频生成。它通过双分支框架同时生成RGB和深度序列，并引入语义引导互对齐（SIGMA）机制进行融合，显著提高了相机轨迹的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有相机控制视频生成方法缺乏足够的场景理解和几何感知，导致生成视频难以忠实遵循指定的相机轨迹。

Method: 引入了DualCamCtrl，一个双分支框架，用于相互生成相机一致的RGB和深度序列。提出了语义引导互对齐（SIGMA）机制，以语义引导和相互强化的方式进行RGB-深度融合。此外，分析了深度和相机姿态在去噪阶段的不同影响。

Result: DualCamCtrl能够更好地解耦外观和几何建模，生成更忠实于指定相机轨迹的视频。与现有方法相比，相机运动误差减少了40%以上，实现了更一致的相机控制视频生成。

Conclusion: DualCamCtrl通过其双分支框架和SIGMA机制，有效解决了相机控制视频生成中场景理解和几何感知不足的问题，显著提高了生成视频与相机轨迹的一致性，并减少了运动误差。
...
### [216] [InstanceV: Instance-Level Video Generation](https://arxiv.org/abs/2511.23146)
*Yuheng Chen,Teng Hu,Jiangning Zhang,Zhucun Xue,Ran Yi,Lizhuang Ma*

Main category: cs.CV

TL;DR: InstanceV是一个新的视频生成框架，通过引入实例感知机制和全局语义增强，实现了对视频生成中实例级别的高精度控制和整体语义一致性，并提出了新的评估基准InstanceBench。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频扩散模型主要依赖文本条件，缺乏对视频生成进行通用、细粒度控制的能力，特别是在实例层面。

Method: InstanceV框架包含：1) 实例感知掩蔽交叉注意力机制（Instance-aware Masked Cross-Attention），以最大化利用实例级接地信息；2) 共享时间步自适应提示增强模块（Shared Timestep-Adaptive Prompt Enhancement），以参数高效地连接局部实例与全局语义；3) 空间感知无条件指导（Spatially-Aware Unconditional Guidance），以缓解小实例消失的问题。此外，提出新的基准InstanceBench，结合通用视频质量和实例感知指标进行全面评估。

Result: InstanceV在视频生成中实现了卓越的实例级可控性，并在定性和定量评估中，于通用质量和实例感知指标上均优于现有最先进模型。

Conclusion: InstanceV成功解决了现有文本到视频模型缺乏细粒度控制的问题，通过其创新机制实现了实例级控制和全局语义一致性，并在性能上超越了现有技术。
...
### [217] [Analyzing Image Beyond Visual Aspect: Image Emotion Classification via Multiple-Affective Captioning](https://arxiv.org/abs/2511.23115)
*Zibo Zhou,Zhengjun Zhai,Huimin Chen,Wei Dai,Hansen Yang*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的基于情感描述的图像情感分类（ACIEC）方法，利用纯文本有效捕捉图像情感信息，以弥补“情感鸿沟”，并在多个基准测试中取得了优异结果。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在图像情感分类（IEC）方面取得了进展，但预训练视觉模型受到“情感鸿沟”的限制。心理学研究表明语言具有高度可变性、包含丰富信息，并能有效消除“情感鸿沟”。受此启发，本文旨在利用语言信息改进IEC。

Method: 本文提出了情感描述图像情感分类（ACIEC）方法。具体而言，设计了分层多级对比损失来检测图像中的情感概念；提出了情感属性思维链推理来生成情感句子；利用预训练语言模型综合情感概念和情感句子进行IEC。此外，设计了基于语义相似性采样的对比损失来解决情感数据集中类内差异大和类间差异小的问题。该方法还考虑了带有嵌入文本的图像。

Result: 广泛的实验表明，本文方法能有效弥补情感鸿沟，并在多个基准测试中取得了优于现有方法的性能。

Conclusion: ACIEC方法通过生成情感描述并利用纯文本进行图像情感分类，成功地弥补了视觉模型在IEC任务中的“情感鸿沟”，并在包含嵌入文本的图像上也表现出色，达到了卓越的分类效果。
...
### [218] [Image Valuation in NeRF-based 3D reconstruction](https://arxiv.org/abs/2511.23052)
*Grigorios Aris Cheimariotis,Antonis Karakottas,Vangelis Chatzis,Angelos Kanlis,Dimitrios Zarpalas*

Main category: cs.CV

TL;DR: 本文提出了一种量化每张图片对基于NeRF的3D场景重建贡献的方法，特别针对野外场景，并通过移除低贡献图片来验证其对重建质量的影响。


<details>
  <summary>Details</summary>
Motivation: 数据估值和变现日益重要。在基于图像的3D场景重建（如NeRF）中，并非所有输入图像都对最终输出有同等贡献，尤其是在野外场景中，图像质量、遮挡和瞬态物体导致输入效用不均。

Method: 本文提出了一种量化每张图片对基于NeRF的野外图像集重建贡献的方法。贡献通过基于PSNR和MSE的重建质量指标进行评估。通过在训练期间移除低贡献图片并测量对重建保真度的影响来验证该方法。

Result: 通过移除低贡献图像进行验证，结果显示了对重建保真度的影响，这表明该方法能够识别不同图像的贡献，并可能通过筛选输入图像来优化重建过程。

Conclusion: 该研究成功量化了每张图片对NeRF重建的贡献，并证明了基于此贡献度移除低质量图片可以影响重建保真度，为优化野外场景的NeRF重建提供了途径。
...
### [219] [PowerCLIP: Powerset Alignment for Contrastive Pre-Training](https://arxiv.org/abs/2511.23170)
*Masaki Kawamura,Nakamasa Inoue,Rintaro Yanagi,Hirokatsu Kataoka,Rio Yokota*

Main category: cs.CV

TL;DR: PowerCLIP是一个新的对比视觉-语言预训练框架，通过幂集对齐（powerset alignment）来优化区域到短语的对齐，从而增强细粒度组合理解。它引入了高效的非线性聚合器（NLAs）以降低计算成本，并在零样本分类和检索任务中超越了现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有的对比视觉-语言预训练框架（如CLIP）在零样本任务中表现出色，但将单个文本标记与图像区域对齐的方法难以捕捉跨多个图像区域的组合语义。

Method: 本文提出了PowerCLIP，一个通过幂集对齐增强的对比预训练框架。它通过最小化图像区域幂集和文本解析树之间的损失，穷尽地优化区域到短语的对齐。为解决幂集构造的指数级计算成本，引入了高效的非线性聚合器（NLAs），将复杂度从O(2^M)降低到O(M)，同时近似精确的损失值。

Result: PowerCLIP在零样本分类和检索任务中均优于最先进的方法。

Conclusion: PowerCLIP通过其幂集对齐方法和高效的非线性聚合器，显著增强了视觉-语言模型的组合理解能力和鲁棒性。
...
### [220] [Cascaded Robust Rectification for Arbitrary Document Images](https://arxiv.org/abs/2511.23150)
*Chaoyun Wang,Quanxin Huang,I-Chao Shen,Takeo Igarashi,Nanning Zheng,Caigui Jiang*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的多阶段框架，以粗到精的方式逐步纠正文档的透视和几何失真，并在多个基准测试中达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中文档矫正面临巨大挑战，主要源于相机视角的极端变化和物理形变（如纸张卷曲、折叠）。研究者们认为复杂变换可以被分解并逐步解决。

Method: 该方法引入了一个多阶段框架：首先进行全局仿射变换以校正透视失真；接着矫正由纸张卷曲和折叠引起的几何变形；最后采用内容感知的迭代过程消除细粒度内容失真。此外，还提出了两种增强的评估指标：布局对齐的OCR指标（AED/ACER）和掩膜AD/AAD（AD-M/AAD-M），以更准确地评估几何失真。

Result: 实验结果表明，该方法在多个挑战性基准测试中取得了新的最先进性能，AAD指标显著降低了14.1%—34.7%，并在实际应用中展现出卓越的效率。

Conclusion: 所提出的多阶段渐进式文档矫正框架能有效应对现实世界中的复杂失真，通过分解和逐步解决问题，显著提高了矫正性能，并改进了评估协议，实现了最先进的结果。
...
### [221] [GeoWorld: Unlocking the Potential of Geometry Models to Facilitate High-Fidelity 3D Scene Generation](https://arxiv.org/abs/2511.23191)
*Yuhao Wan,Lijuan Liu,Jingzhi Zhou,Zihan Zhou,Xuying Zhang,Dongbo Zhang,Shaohui Jiao,Qibin Hou,Ming-Ming Cheng*

Main category: cs.CV

TL;DR: GeoWorld提出了一种新的图像到3D场景生成方法，通过先生成连续视频帧并利用几何模型提供全帧几何特征，结合几何对齐损失和几何适应模块，有效解决了现有方法中几何失真和模糊的问题，实现了高质量的3D场景生成。


<details>
  <summary>Details</summary>
Motivation: 以往利用视频模型进行图像到3D场景生成的方法，普遍存在几何畸变和内容模糊的问题。

Method: GeoWorld革新了图像到3D场景生成的流程：首先生成连续视频帧，然后利用几何模型提供全帧几何特征（比单帧深度图或相机嵌入更丰富），并将其作为几何条件辅助视频生成模型。为增强几何结构的一致性，进一步提出了几何对齐损失以提供真实世界几何约束，并设计了几何适应模块以确保几何特征的有效利用。

Result: 广泛的实验表明，GeoWorld能够从单张图像和给定相机轨迹生成高保真度的3D场景，在定性和定量上均优于现有方法。

Conclusion: GeoWorld通过解锁几何模型的潜力，利用全帧几何特征、几何对齐损失和几何适应模块，成功解决了图像到3D场景生成中的几何失真和模糊问题，实现了高质量的3D场景重建。
...
### [222] [Learning to Refuse: Refusal-Aware Reinforcement Fine-Tuning for Hard-Irrelevant Queries in Video Temporal Grounding](https://arxiv.org/abs/2511.23151)
*Jin-Seop Lee,SungJoon Lee,SeongJun Jung,Boyang Li,Jee-Hyong Lee*

Main category: cs.CV

TL;DR: 本文提出了一种名为RA-RFT（Refusal-Aware Reinforcement Fine-Tuning）的方法，旨在使视频时间定位（VTG）模型能够有效拒绝语义相似但不相关的“难不相关查询”，并构建了相应的HI-VTG数据集。


<details>
  <summary>Details</summary>
Motivation: 现有VTG模型假设相关片段始终存在，即使查询不相关也会预测结果，且难以处理与视频语义相似但实际不相关的“难不相关查询”。

Method: 提出基于GRPO（Group Relative Policy Optimization）框架的RA-RFT方法，并集成了格式、拒绝-IoU、解释和查询纠正四种奖励目标，以提高相关性判别和细粒度语义推理能力。此外，构建了包含难不相关查询及其拒绝答案的HI-VTG（Hard-Irrelevant VTG）数据集。

Result: 该方法在难不相关VTG、简单打乱的RA-VTG和人工标注的RA-VTG等多种相关性感知VTG场景中均表现出有效性，并证明了其对基于LVLM的VTG模型的通用性。

Conclusion: RA-RFT方法通过强化学习和多目标奖励机制，结合新构建的HI-VTG数据集，成功解决了VTG模型拒绝难不相关查询的挑战，显著提升了模型的鲁棒性和判别能力。
...
### [223] [Pathryoshka: Compressing Pathology Foundation Models via Multi-Teacher Knowledge Distillation with Nested Embeddings](https://arxiv.org/abs/2511.23204)
*Christian Grashei,Christian Brechenmacher,Rao Muhammad Umer,Jingsong Liu,Carsten Marr,Ewa Szczurek,Peter J. Schüffler*

Main category: cs.CV

TL;DR: Pathryoshka是一种多教师蒸馏框架，能将病理学基础模型（FM）尺寸大幅缩小86-92%，并提供可适应的嵌入维度，同时保持与大型教师模型相当的性能，从而使最先进的病理学FM更易于访问和部署。


<details>
  <summary>Details</summary>
Motivation: 当前高性能的病理学基础模型参数量巨大且产生高维嵌入，这在计算资源紧张时限制了它们在研究或临床中的应用。

Method: 引入了Pathryoshka，一个受RADIO蒸馏和Matryoshka表示学习启发的、多教师蒸馏框架。该框架旨在减小病理学FM的模型尺寸，并允许可适应的嵌入维度。

Result: Pathryoshka将模型尺寸缩小了86-92%，同时在十个公共病理学基准测试中保持了与其大得多的教师模型相当的性能。与同等大小的现有单教师蒸馏模型相比，其准确率中位数高出7.0。

Conclusion: Pathryoshka通过在不牺牲准确性或表示丰富性的前提下实现高效的本地部署，从而使更广泛的研究和临床社区能够民主化地访问最先进的病理学基础模型。
...
### [224] [Robust 3DGS-based SLAM via Adaptive Kernel Smoothing](https://arxiv.org/abs/2511.23221)
*Shouhe Zhang,Dayong Ren,Sensen Song,Wenjie Li,Piaopiao Yu,Yurong Qian*

Main category: cs.CV

TL;DR: 本文挑战了3DGS-SLAM中渲染质量决定跟踪精度的传统观念，提出通过平滑核策略（CB-KNN）增强光栅化过程对参数误差的鲁棒性，以显著提高相机姿态跟踪的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统的3DGS-SLAM方法过分强调追求完美的场景表示以提高渲染质量，并假设这能带来更高的跟踪精度。然而，本文认为，对于稳定的相机姿态跟踪而言，更关键的是增强光栅化过程对参数误差的鲁棒性，而非单纯追求完美的场景表示。

Method: 本文提出了一种名为“纠正性模糊KNN”（Corrective Blurry KNN, CB-KNN）的新方法。该方法利用平滑核策略来增强基于3DGS的SLAM的鲁棒性。其核心思想是使光栅化过程更能抵抗3DGS参数中的不完善之处，而不是仅仅最小化渲染误差。通过让每个高斯在渲染时影响更平滑、更宽的像素分布，引入受控的模糊作为正则化项，从而稳定后续的姿态优化。CB-KNN通过自适应地修改局部区域内K个最近邻高斯点的RGB值和位置，生成更平滑的局部渲染，减少错误GS参数对整体图像的影响。

Result: 实验结果表明，本文提出的方法在保持场景重建（建图）整体质量的同时，显著提高了相机姿态跟踪的鲁棒性和准确性。

Conclusion: 增强光栅化过程对参数误差的鲁棒性对于确保稳定的相机姿态跟踪至关重要，这比单纯追求完美的场景表示更为关键。CB-KNN方法通过引入受控模糊和自适应调整高斯参数，有效提升了3DGS-SLAM的跟踪性能。
...
### [225] [Fast Multi-view Consistent 3D Editing with Video Priors](https://arxiv.org/abs/2511.23172)
*Liyi Chen,Ruihuang Li,Guowen Zhang,Pengfei Wang,Lei Zhang*

Main category: cs.CV

TL;DR: 本文提出ViP3DE，利用预训练视频生成模型的时序一致性先验，在单次前向传播中实现文本驱动的多视角一致3D编辑，显著提升质量和速度。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动3D编辑方法依赖迭代的2D-3D-2D更新，逐视图处理，导致耗时、结果过度平滑，且缺乏多视角一致性。

Method: ViP3DE利用预训练视频生成模型的时序一致性先验进行多视角一致3D编辑。核心思想是：以单个编辑过的视图为条件，直接生成其他一致的编辑视图进行3D更新，避免迭代过程。为此，引入“运动保留噪声混合”以在预定义相机姿态下生成视图，并提出“几何感知去噪”以通过整合3D几何先验进一步增强多视角一致性。

Result: ViP3DE在单次前向传播中即可实现高质量的3D编辑结果，在编辑质量和速度上均显著优于现有方法。

Conclusion: ViP3DE通过利用视频先验有效解决了现有文本驱动3D编辑方法的局限性，实现了更快、更高质量且多视角一致的3D编辑。
...
### [226] [Instruction Tuning of Large Language Models for Tabular Data Generation-in One Day](https://arxiv.org/abs/2511.23220)
*Milad Abdollahzadeh,Abdul Raheem,Zilong Zhao,Uzair Javaid,Kevin Yee,Nalam Venkata Abhishek,Tram Truong-Huu,Biplab Sikdar*

Main category: cs.CV

TL;DR: 本文首次探索了指令微调如何提升大型语言模型（LLMs）的表格数据生成能力，并证明在有限数据和计算资源下也能达到与顶尖商业模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLMs在表格数据上的问答和推理任务，而忽略了表格数据生成。此外，表格指令微调通常需要大量数据和计算资源，限制了其应用。

Method: 研究者首先创建了一个高质量的表格数据指令数据集，然后使用该数据集中的7K指令，在A100 GPU上对开源LLM (Llama3.1-8B-Instruct) 进行了不到6小时的指令微调。

Result: 实验结果表明，通过使用所创建的高质量数据集和有限的指令微调，LLMs的表格数据生成性能达到了与GPT-4o等最强大的商业LLM相当的水平。

Conclusion: 指令微调是一种有效的方法，即使在数据和计算资源有限的情况下，也能显著提升LLMs的表格数据生成能力，并使其性能媲美先进的商业模型。
...
### [227] [DAONet-YOLOv8: An Occlusion-Aware Dual-Attention Network for Tea Leaf Pest and Disease Detection](https://arxiv.org/abs/2511.23222)
*Yefeng Wu,Shan Wan,Ling Wu,Yecheng Zhao*

Main category: cs.CV

TL;DR: DAONet-YOLOv8是一种增强型YOLOv8变体，通过引入双注意力融合模块、遮挡感知检测头和C2f-DSConv模块，显著提升了茶叶病虫害检测在复杂真实环境下的精度，同时减少了模型参数。


<details>
  <summary>Details</summary>
Motivation: 在真实茶园环境中，由于背景复杂、光照多变以及枝叶间的频繁遮挡，茶叶病虫害的准确检测面临挑战。现有检测器在这种场景下常出现漏检和误报。

Method: 本文提出了DAONet-YOLOv8，包含三项关键改进：1) 双注意力融合模块（DAFM），结合卷积局部特征提取和自注意力全局上下文建模，以关注微小病变区域并抑制背景噪声；2) 遮挡感知检测头（Detect-OAHead），学习可见与遮挡部分的关系以补偿缺失的病变特征；3) C2f-DSConv模块，采用具有多种核形状的动态合成卷积，以更好地捕捉不规则病变边界。

Result: 在包含六种病虫害类别的真实茶园数据集上，DAONet-YOLOv8实现了92.97%的精确率、92.80%的召回率、97.10%的mAP@50和76.90%的mAP@50:95。相比YOLOv8n基线，各项指标分别提高了2.34、4.68、1.40和1.80个百分点，同时参数量减少了16.7%。对比实验进一步证实，DAONet-YOLOv8优于主流检测模型。

Conclusion: DAONet-YOLOv8有效解决了真实茶园环境中茶叶病虫害检测的挑战，通过其创新的模块设计，在复杂背景、遮挡和不规则病变边界等问题上取得了显著的性能提升，并减少了模型复杂度。
...
### [228] [Zero-Shot Multi-Criteria Visual Quality Inspection for Semi-Controlled Industrial Environments via Real-Time 3D Digital Twin Simulation](https://arxiv.org/abs/2511.23214)
...
TL;DR: 本文提出了一种姿态无关、零样本的质量检测框架，通过在RGB-D空间中将真实场景与实时数字孪生(DT)进行比较，实现了早期视觉质量检测，适用于半受控工业环境，并为低数据缺陷检测奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 早期视觉质量检测对于实现零缺陷制造和减少生产浪费至关重要。然而，现有鲁棒视觉检测系统的复杂性及其对大量数据的需求阻碍了它们在半受控工业环境中的广泛应用。

Method: 该研究提出了一种姿态无关、零样本的质量检测框架，通过物体检测和已知CAD模型的姿态估计，对工业场景进行语义描述，从而实现实时数字孪生(DT)的渲染，并在RGB-D空间中将真实场景与实时DT进行比较。该方法还对实时、多模态RGB-D DT创建工具进行了基准测试，并跟踪了计算资源消耗。此外，提供了一种可扩展的分层标注策略，用于多标准缺陷检测，统一了姿态标注与逻辑和结构缺陷标注。

Result: 在一个轴向磁通电机质量检测的汽车用例中，该框架被证明是有效的。即使在半受控工业条件下使用简单的距离测量，缺陷检测性能也达到了高达63.3%的交并比（IoU）分数，与真实掩码相比表现良好。

Conclusion: 研究结果为未来在动态制造环境中进行可泛化、低数据缺陷检测方法的研究奠定了基础。
...
### [229] [Unlocking Multilingual Reasoning Capability of LLMs and LVLMs through Representation Engineering](https://arxiv.org/abs/2511.23231)
*Qiming Li,Xiaocheng Feng,Yixuan Ma,Zekai Ye,Ruihan Chen,Xiachong Feng,Bing Qin*

Main category: cs.CV

TL;DR: 本文提出了一种名为MRRE的无训练、推理时方法，通过注入预计算向量来提升大语言模型和大型视觉语言模型在低资源语言中的多语言推理能力，同时保持语言一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）和大型视觉语言模型（LVLMs）在英语推理方面表现出色，但在低资源语言中性能显著下降，引发了多语言应用中的公平性问题。现有方法依赖于昂贵的多语言训练或使用外部翻译工具进行提示，这些方法资源密集且对翻译质量敏感。

Method: 本文提出了一种名为多语言推理表示工程（MRRE）的无训练、推理时方法。它在推理过程中特定层序贯注入两个预计算向量：1. 跨语言推理增强向量，将非英语推理表示引导至英语空间以解锁多语言推理；2. 目标语言输出锚定向量，恢复目标语言分布以保持输入-输出语言一致性。

Result: 在六个先进的LLMs和LVLMs上，对四个推理基准进行的综合实验表明，MRRE持续增强了非英语推理能力，平均提升5.48%，在泰语和斯瓦希里语等低资源语言中最高提升7.54%，同时将输入-输出语言一致性提高了3.78%。

Conclusion: MRRE是一种有效且无需额外训练数据或工具的方法，能够显著提升LLMs和LVLMs在非英语，特别是低资源语言中的多语言推理能力和输入-输出语言一致性。
...
### [230] [PointCNN++: Performant Convolution on Native Points](https://arxiv.org/abs/2511.23227)
*Lihan Li,Haofeng Zhong,Rui Bu,Mingchao Sun,Wenzheng Chen,Baoquan Chen,Yangyan Li*

Main category: cs.CV

TL;DR: PointCNN++提出了一种新的3D点云卷积架构，通过将稀疏卷积从体素推广到点，实现了高几何精度和高性能，解决了现有方法中的精度-性能权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D点云卷积方法存在精度-性能权衡：基于点的方法精度高但效率低，基于体素的方法效率高但牺牲了几何精度。这种精度损失在点云配准等任务中是一个关键瓶颈。

Method: PointCNN++通过以下方式解决了问题：1) 提出以原始高精度点坐标为中心的点中心卷积，将体素卷积视为其特例。2) 设计了在点上原生操作的计算策略，将点上的卷积公式化为矩阵-向量乘法和规约（MVMR）问题。3) 开发了专用的、高度优化的GPU内核来执行MVMR。

Result: PointCNN++相比代表性的基于点的方法，内存使用量减少了一个数量级，速度快了几倍。当替代其泛化的基于体素的骨干网络时，它显著提高了点云配准精度，并且在内存效率和速度方面均表现更优。

Conclusion: PointCNN++证明了在3D学习中，保留几何细节和实现高性能并非相互排斥，为开发兼具高保真度和效率的新型3D学习方法铺平了道路。
...
### [231] [Language-guided 3D scene synthesis for fine-grained functionality understanding](https://arxiv.org/abs/2511.23230)
...
TL;DR: SynthFun3D是一种新方法，用于根据动作描述合成3D场景，以生成大规模、高质量的标注数据，解决3D功能理解领域真实数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 3D功能理解（识别3D场景中完成动作的功能元素）因真实世界数据收集和标注的巨大努力而受阻，导致数据稀缺。

Method: 本文提出了SynthFun3D，首个基于任务的3D场景合成方法。它接收动作描述，利用带有部件级标注的家具资产数据库生成一个可完成该动作的3D室内环境，并自动识别和检索正确功能元素的3D掩码，从而实现廉价且大规模地生成高质量标注数据。

Result: 用户研究表明SynthFun3D生成的场景与提示的一致性优于其他方法。定量结果显示，生成的数据可以替代真实数据且性能损失轻微，或补充真实数据以提高性能。

Conclusion: SynthFun3D为数据需求大的3D应用提供了一个廉价且可扩展的解决方案，通过合成数据有效解决了真实数据稀缺的挑战。
...
### [232] [Synthetic Industrial Object Detection: GenAI vs. Feature-Based Methods](https://arxiv.org/abs/2511.23241)
*Jose Moises Araya-Martinez,Adrián Sanchis Reig,Gautham Mohan,Sarvenaz Sardari,Jens Lambrecht,Jörg Krüger*

Main category: cs.CV

TL;DR: 本研究评估了各种域随机化（DR）和域适应（DA）技术（包括特征方法和生成式AI）在无需手动标注的情况下创建情境化合成数据以弥合仿真到现实鸿沟的有效性。结果表明，在有足够变异性的渲染数据作为种子时，感知哈希等简单特征方法在准确性和资源效率上均优于复杂的生成式AI方法。


<details>
  <summary>Details</summary>
Motivation: 在工业和机器人环境中部署机器学习时，数据生成和标注的负担巨大，成本高昂。合成渲染虽有潜力，但仿真到现实的鸿沟常需专家干预。

Method: 基准测试了一系列域随机化（DR）和域适应（DA）技术，包括基于特征的方法（如亮度过滤、感知哈希）、生成式AI（GenAI）和经典渲染方法。评估了低级和高级特征对齐的有效性和效率，以及一种由真实世界上下文提示引导的受控扩散型DA方法。在专有工业数据集和公共机器人数据集上进行了验证。

Result: 如果存在足够变异性的渲染数据作为种子，亮度过滤和感知哈希等简单的基于特征的方法在准确性和资源效率上均优于更复杂的生成式AI方法。感知哈希表现最佳，在工业和机器人数据集上的mAP50分数分别为98%和67%。生成式AI方法在数据生成上存在显著时间开销，且与简单方法相比，仿真到现实的mAP值没有明显提升。

Conclusion: 本研究提供了高效弥合仿真到现实鸿沟的可行见解，表明当有足够变异性的渲染数据时，简单的基于特征的方法（尤其是感知哈希）比复杂的生成式AI方法更有效和高效，能够使仅用合成数据训练的模型在真实世界中获得高性能。
...
### [233] [FACT-GS: Frequency-Aligned Complexity-Aware Texture Reparameterization for 2D Gaussian Splatting](https://arxiv.org/abs/2511.23292)
*Tianhao Xie,Linlian Jiang,Xinxin Zuo,Yang Wang,Tiberiu Popa*

Main category: cs.CV

TL;DR: 本文提出FACT-GS，一种频率对齐、复杂度感知的纹理高斯溅射框架，通过可学习的形变场自适应分配纹理采样密度，以解决现有方法因均匀采样导致的细节模糊和效率低下问题，从而在相同参数预算下恢复更清晰的细节并保持实时性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于纹理的高斯溅射方法采用均匀采样网格，导致纹理空间利用效率低下：高频区域采样不足，平滑区域浪费容量，进而造成外观模糊和精细结构细节丢失。

Method: FACT-GS基于自适应采样理论，将纹理参数化重新表述为一个可微分的采样密度分配问题。它用可学习的频率感知分配策略取代了均匀纹理，该策略通过一个形变场实现，其雅可比矩阵调制局部采样密度。该方法在固定分辨率纹理网格上执行非均匀采样，并建立在2D高斯溅射之上。

Result: FACT-GS在相同的参数预算下，能够恢复更清晰的高频细节，同时保持实时性能。

Conclusion: FACT-GS通过根据局部视觉频率自适应分配纹理采样密度，解决了现有纹理高斯溅射的效率和细节问题，提供了一种更有效且能保持实时性能的高质量场景外观建模方法。
...
### [234] [Object-Centric Data Synthesis for Category-level Object Detection](https://arxiv.org/abs/2511.23450)
*Vikhyat Agarwal,Jiayi Cora Guo,Declan Hoban,Sissi Zhang,Nicholas Moran,Peter Cho,Srilakshmi Pattabiraman,Shantanu Joshi*

Main category: cs.CV

TL;DR: 本文提出了一种“以物体为中心的数据”设置，利用有限的多视角图像或3D模型，通过四种数据合成方法（图像处理、3D渲染、图像扩散模型）来生成逼真的图像，以在数据受限的情况下微调目标检测模型，从而实现对新物体类别的泛化并显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习目标检测方法在扩展到新物体类别时，需要大量标注训练数据，这获取成本高且耗时，尤其对于长尾类别数据不足的问题更为突出。

Method: 研究引入了“以物体为中心的数据”设置，即有限数据以多视角图像或3D模型形式存在。在此设置下，系统评估了四种数据合成方法：基于简单图像处理技术、3D渲染以及图像扩散模型。这些方法利用以物体为中心的数据合成具有不同上下文连贯性和复杂度的逼真、杂乱图像。

Result: 这些方法使模型能够在真实世界数据中实现类别级泛化，并在数据受限的实验设置中显著提升了性能。

Conclusion: 研究表明，通过以物体为中心的数据和所提出的合成方法，可以有效解决新物体类别目标检测的数据稀缺问题，显著提高模型在数据受限环境下的检测能力。
...
### [235] [Hunyuan-GameCraft-2: Instruction-following Interactive Game World Model](https://arxiv.org/abs/2511.23429)
*Junshu Tang,Jiacheng Liu,Jiaqi Li,Longhuang Wu,Haoyu Yang,Penghao Zhao,Siruis Gong,Xiang Yuan,Shuai Shao,Qinglin Lu*

Main category: cs.CV

TL;DR: Hunyuan-GameCraft-2 引入了一种指令驱动的交互范式，通过自然语言、键盘或鼠标控制生成的游戏世界视频内容，解决了传统生成模型在多样化交互和高标注成本方面的限制。


<details>
  <summary>Details</summary>
Motivation: 当前的生成世界模型受限于僵化的动作模式和高昂的标注成本，难以模拟多样化的游戏内交互和玩家驱动的动态，这促使研究人员寻求更灵活、语义更丰富的交互方式。

Method: 本研究提出了 Hunyuan-GameCraft-2，一个指令驱动的生成游戏世界建模新范式。它允许用户通过自然语言、键盘或鼠标控制游戏视频内容。研究定义了交互式视频数据，并开发了自动化流程将大规模非结构化文本-视频对转换为因果对齐的交互数据集。模型基于一个14B的图到视频MoE基础模型，并集成了文本驱动的交互注入机制以实现对摄像机运动、角色行为和环境动态的细粒度控制。同时，引入了交互专用基准InterBench进行全面评估。

Result: 广泛的实验表明，该模型能够生成时间连贯且因果关联的交互式游戏视频，忠实地响应“开门”、“画一个火把”或“引发爆炸”等多样且自由形式的用户指令。

Conclusion: Hunyuan-GameCraft-2 提供了一种新的生成游戏世界建模范式，通过指令驱动的交互实现了在生成世界中灵活且语义丰富的用户控制，有效解决了现有方法的局限性。
...
### [236] [UniGeoSeg: Towards Unified Open-World Segmentation for Geospatial Scenes](https://arxiv.org/abs/2511.23332)
*Shuo Ni,Di Wang,He Chen,Haonan Guo,Ning Zhang,Jing Zhang*

Main category: cs.CV

TL;DR: 本文介绍了GeoSeg-1M，首个百万级遥感指令驱动分割数据集，GeoSeg-Bench，一个用于评估上下文理解和推理能力的基准，以及UniGeoSeg，一个统一的框架，在GeoSeg-Bench和公共基准上实现了最先进的性能和强大的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感指令驱动分割方法存在任务表述碎片化和指令数据有限的问题，阻碍了有效的理解和泛化能力。

Method: 本文构建了GeoSeg-1M数据集，通过自动化掩模过滤和指令生成管道，从多个公共数据集中合成了指示、交互和推理分割指令。在此基础上，进一步策划了GeoSeg-Bench基准。此外，提出了UniGeoSeg统一框架，该框架结合了任务感知文本增强、潜在知识记忆和渐进式训练策略，以促进多任务学习。

Result: UniGeoSeg在GeoSeg-Bench和各种公共基准上均表现出最先进的性能，并展现出强大的零样本泛化能力。

Conclusion: 本文通过引入GeoSeg-1M数据集、GeoSeg-Bench基准和UniGeoSeg统一框架，有效解决了遥感指令驱动分割领域数据和方法上的局限性，显著提升了该任务的理解和泛化能力。
...
### [237] [DisMo: Disentangled Motion Representations for Open-World Motion Transfer](https://arxiv.org/abs/2511.23428)
*Thomas Ressler-Antal,Frank Fundel,Malek Ben Alaya,Stefan Andreas Baumann,Felix Krause,Ming Gui,Björn Ommer*

Main category: cs.CV

TL;DR: DisMo提出了一种从原始视频中学习抽象运动表示的新范式，该表示与内容分离，可实现开放世界运动迁移，并在零样本动作分类任务中超越现有SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 当前的文本到视频（T2V）和图像到视频（I2V）模型未能提供与内容分离的显式运动表示，这限制了它们对内容创作者的适用性，并且在运动迁移中存在运动保真度与提示遵循度之间的权衡、对源结构过拟合或偏离描述动作的问题。

Method: 本文提出了DisMo，一种通过图像空间重建目标直接从原始视频数据中学习抽象运动表示的新范式。这种表示是通用的，独立于外观、物体身份或姿态等静态信息。它可以通过轻量级适配器与任何现有视频生成器结合。

Result: DisMo实现了开放世界运动迁移，允许在语义不相关的实体之间（甚至在截然不同的类别之间）进行运动转移，无需物体对应。它将运动语义与外观解耦，实现了准确的迁移和忠实的条件化。此外，学习到的表示非常适合下游运动理解任务，在Something-Something v2和Jester等基准测试的零样本动作分类中，始终优于V-JEPA等最先进的视频表示模型。

Conclusion: DisMo成功地学习了独立于内容的通用抽象运动表示，解决了现有T2V/I2V模型在运动表示上的局限性。它不仅在开放世界运动迁移中表现出色，还在运动理解任务中取得了显著的SOTA性能，为未来的视频模型进步提供了可插拔的优势。
...
### [238] [A Hierarchical Computer Vision Pipeline for Physiological Data Extraction from Bedside Monitors](https://arxiv.org/abs/2511.23355)
...
TL;DR: 本文提出了一种基于计算机视觉的解决方案，通过从床边监护仪屏幕自动捕获和数字化生命体征数据，以解决低资源医疗环境中传统监护仪与电子健康记录系统之间的互操作性问题。


<details>
  <summary>Details</summary>
Motivation: 在许多低资源医疗环境中，床边监护仪是独立的传统设备，缺乏网络连接，导致生理数据无法无缝集成到电子健康记录（EHR）系统中。为了在不更换昂贵硬件的情况下解决这一问题，研究旨在开发一种经济高效的数据捕获方法。

Method: 该方法采用了一个分层检测框架：结合YOLOv11进行显示器和感兴趣区域（ROI）的精确定位，并使用PaddleOCR进行鲁棒的文本提取。为了提高在不同摄像机角度和光照条件下的可靠性，还引入了几何校正模块，在字符识别前标准化屏幕透视。系统在一个包含6,498张图像的数据集上进行了评估。

Result: 模型在显示器检测方面实现了99.5%的平均精度（mAP@50-95），在生命体征ROI定位方面达到了91.5%。对于心率、血氧饱和度SpO2和动脉血压等核心生理参数，端到端提取准确率超过98.9%。

Conclusion: 研究结果表明，一种轻量级的、基于摄像头的方法能够可靠地将屏幕捕获的非结构化信息转换为结构化数字数据，为在低资源环境下改善信息可访问性和临床文档提供了一条实用且可扩展的途径。
...
### [239] [Markovian Scale Prediction: A New Era of Visual Autoregressive Generation](https://arxiv.org/abs/2511.23334)
*Yu Zhang,Jingyi Liu,Yiwei Shi,Qi Zhang,Duoqian Miao,Changwei Wang,Longbing Cao*

Main category: cs.CV

TL;DR: 本文提出Markov-VAR，通过将视觉自回归模型(VAR)重构为非全上下文马尔可夫过程，并引入滑动窗口压缩历史信息，显著提高了效率和性能，同时减少了内存消耗。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉自回归模型(VAR)依赖于全上下文，即预测下一尺度时考虑所有先前尺度，这导致计算效率低下和巨大的开销，严重阻碍了其实用性和可扩展性。研究动机是开发一种在没有全上下文依赖的情况下，具有更好性能和效率的新VAR模型。

Method: 将VAR重构为非全上下文马尔可夫过程，命名为Markov-VAR。具体通过马尔可夫尺度预测实现：将每个尺度视为一个马尔可夫状态，并引入一个滑动窗口，将特定的先前尺度压缩成一个紧凑的历史向量，以弥补非全上下文依赖导致的历史信息损失。将此历史向量与马尔可夫状态结合，形成一个在马尔可夫过程中演变的代表性动态状态。

Result: 实验证明Markov-VAR极其简单但高效：在ImageNet数据集上，与VAR相比，Markov-VAR在256x256分辨率下将FID降低了10.5%，在1024x1024分辨率下将峰值内存消耗降低了83.8%。

Conclusion: Markov-VAR因其简洁性和高效性，有望成为未来视觉自回归生成及其他下游任务研究的基础。
...
### [240] [MANTA: Physics-Informed Generalized Underwater Object Tracking](https://arxiv.org/abs/2511.23405)
*Suhas Srinath,Hemang Jamadagni,Aditya Chadrasekar,Prathosh AP*

Main category: cs.CV

TL;DR: MANTA是一个物理信息增强的水下目标跟踪框架，通过结合对比学习和多阶段关联算法，有效解决了水下复杂环境带来的外观失真和跟踪挑战，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 由于波长依赖的衰减和散射，水下环境严重扭曲物体外观，导致现有基于陆地数据训练的跟踪器无法泛化到这些物理驱动的退化，使得水下目标跟踪极具挑战性。

Method: 本文提出了MANTA框架，将表示学习与跟踪设计相结合。具体方法包括：1) 采用双正对比学习策略，将时间一致性与Beer-Lambert增强相结合，以学习对时间和水下失真都鲁棒的特征。2) 引入多阶段跟踪流程，在基于运动的跟踪基础上，增加一个物理信息增强的二次关联算法，该算法结合几何一致性和外观相似性进行遮挡和漂移下的重识别。3) 提出了Center-Scale Consistency (CSC)和Geometric Alignment Score (GAS)指标来评估几何保真度。

Result: MANTA在四个水下基准测试（WebUOT-1M, UOT32, UTB180, UWCOT220）上均取得了最先进的性能，Success AUC提高了高达6%，同时确保了稳定、长期的水下通用跟踪和高效的运行时间。

Conclusion: MANTA通过其物理信息增强的框架、鲁棒的特征学习和多阶段关联算法，成功克服了水下环境带来的挑战，为水下目标跟踪提供了一个高性能、稳定且高效的解决方案。
...
### [241] [VQRAE: Representation Quantization Autoencoders for Multimodal Understanding, Generation and Reconstruction](https://arxiv.org/abs/2511.23386)
*Sinan Du,Jiahao Guo,Bo Li,Shuhao Cui,Zhengzhuo Xu,Yifu Luo,Yongxian Wei,Kun Gai,Xinggang Wang,Kai Wu,Chun Yuan*

Main category: cs.CV

TL;DR: VQRAE提出一种统一分词器，通过矢量量化实现图像理解的连续语义特征和视觉生成的离散token，在多模态任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 构建统一模型面临着将多模态理解、生成和重建表示整合到单一分词器中的关键挑战。现有研究大多采用双编码器范式或对比损失，未能实现真正统一的表示。

Method: 本文提出VQRAE（表征自编码器的矢量量化版本），首次探索在统一分词器内为图像理解生成连续语义特征和为视觉生成生成离散token。它基于预训练的视觉基础模型和对称ViT解码器，并采用两阶段训练策略：首先，冻结编码器并学习高维语义VQ码本以进行像素重建；然后，通过自蒸馏约束联合优化编码器。

Result: VQRAE在保持多模态理解能力的同时，能生成兼容视觉生成和细粒度重建的离散token。研究发现，依赖高维码本（1536维）的语义编码器量化可实现100%的码本利用率，这与以往图像重建中常用低维码本的做法不同。VQRAE在视觉理解、生成和重建的多个基准测试中表现出竞争力，并因其离散特性在自回归范式中展现出良好的扩展性。

Conclusion: VQRAE成功地在一个统一分词器中实现了多模态理解、生成和重建的统一表示，为图像理解提供连续语义特征，为视觉生成提供离散token，并揭示了高维语义VQ码本的有效性，在多项任务中展现出卓越性能。
...
### [242] [DEAL-300K: Diffusion-based Editing Area Localization with a 300K-Scale Dataset and Frequency-Prompted Baseline](https://arxiv.org/abs/2511.23377)
*Rui Zhang,Hongxia Wang,Hangqing Liu,Yang Zhou,Qiang Zeng*

Main category: cs.CV

TL;DR: 本文提出了一个大型扩散编辑图像篡改定位数据集DEAL-300K（超过30万张图片）和一个基于冻结视觉基础模型和多频提示微调的定位框架，为扩散模型图像篡改定位研究提供了强有力的基线和实用基础。


<details>
  <summary>Details</summary>
Motivation: 扩散模型使得语义级图像编辑变得容易，但也导致了难以定位的逼真局部伪造。现有基准主要关注生成图像的二元检测或手动编辑区域的定位，未能反映扩散编辑平滑融入原始内容的特性。

Method: 1. **数据集构建（DEAL-300K）**：使用多模态大型语言模型生成编辑指令，无掩码扩散编辑器生成篡改图像，以及主动学习变化检测流程获取像素级标注。
2. **定位框架**：利用冻结的视觉基础模型（VFM）结合多频提示微调（MFPT），以捕获编辑区域的语义和频域线索。

Result: 1. 构建了大型数据集DEAL-300K，包含超过30万张带标注的扩散编辑图像。
2. 提出的定位方法在DEAL-300K测试集上达到了82.56%的像素级F1分数，在外部CoCoGlide基准上达到了80.97%。

Conclusion: 本文提出的数据集和方法为未来的扩散模型图像篡改定位（DIML）研究提供了强大的基线和实用的基础。
...
### [243] [A Perceptually Inspired Variational Framework for Color Enhancement](https://arxiv.org/abs/2511.23329)
...
### [244] [Video-R2: Reinforcing Consistent and Grounded Reasoning in Multimodal Language Models](https://arxiv.org/abs/2511.23478)
...
### [245] [Visual Generation Tuning](https://arxiv.org/abs/2511.23469)
...
### [246] [Video-CoM: Interactive Video Reasoning via Chain of Manipulations](https://arxiv.org/abs/2511.23477)
...
### [247] [AnyTalker: Scaling Multi-Person Talking Video Generation with Interactivity Refinement](https://arxiv.org/abs/2511.23475)
...
### [248] [EvalCards: A Framework for Standardized Evaluation Reporting](https://arxiv.org/abs/2511.21695)
...
TL;DR: 本文指出当前NLP评估报告存在可复现性、可访问性和治理方面的不足，并提出“评估披露卡片”（EvalCards）作为解决方案，以提高透明度并满足治理需求。


<details>
  <summary>Details</summary>
Motivation: 在当前快速发布的开源模型背景下，NLP评估的透明报告实践至关重要。研究发现现有报告实践在可复现性、可访问性和治理方面存在长期不足，且现有标准化工作不足以解决这些问题。

Method: 通过对近期评估和文档工作的调查，识别出当前报告实践的不足。在此基础上，引入“评估披露卡片”（EvalCards）作为改进透明度和满足治理要求的方法。

Result: 识别出当前报告实践的三个主要不足：可复现性、可访问性和治理。指出现有标准化工作不足。提出EvalCards作为一种解决方案。

Conclusion: EvalCards旨在增强研究人员和实践者的透明度，并为满足新兴的治理要求提供实用的基础。
...
### [249] [Cacheback: Speculative Decoding With Nothing But Cache](https://arxiv.org/abs/2511.21699)
*Zhiyao Ma,In Gim,Lin Zhong*

Main category: cs.CL

TL;DR: Cacheback Decoding是一种无需训练、模型无关的推测解码方法，利用语言的局部性，通过LRU缓存的token n-gram来加速LLM推理，实现了SOTA性能且易于集成。


<details>
  <summary>Details</summary>
Motivation: 通过利用语言的局部性来加速大型语言模型（LLM）的推理过程。

Method: 提出了Cacheback Decoding方法。该方法仅利用最近最少使用（LRU）的token n-gram缓存表来生成草稿序列。它无需训练且与模型无关。

Result: Cacheback在同类方法中达到了最先进的性能，尽管其设计极简。其简洁性使其易于集成到现有系统中，并显示出快速适应新领域的潜力。

Conclusion: Cacheback Decoding是一种有效、简洁且适应性强的推测解码方法，能够显著加速LLM推理，并在同类方法中表现出色。
...
### [250] [JELV: A Judge of Edit-Level Validity for Evaluation and Automated Reference Expansion in Grammatical Error Correction](https://arxiv.org/abs/2511.21700)
*Yuhao Zhan,Yuqing Zhang,Jing Yuan,Qixiang Ma,Zhiqi Yang,Yu Gu,Zemin Liu,Fei Wu*

Main category: cs.CL

TL;DR: JELV是一个自动化框架，用于验证语法纠错（GEC）中的修正编辑，解决了现有GEC系统参考多样性不足的问题，并提升了评估和模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有语法纠错（GEC）系统受限于参考多样性不足，导致评估结果被低估，并限制了模型的泛化能力。

Method: 本文提出了编辑级有效性判断框架（JELV），用于从语法、忠实度和流畅性方面验证修正编辑。JELV有两种实现：一个多轮LLM-as-Judges管道（基于人工标注的Pair-wise Edit-level Validity Dataset (PEVData)基准），以及一个蒸馏的DeBERTa分类器。JELV被应用于重新分类评估中的误判假阳性，并整合假阳性解耦和流畅性评分以构建综合评估指标。此外，JELV还用于过滤LLM生成的修正候选，以扩充数据集并提升GEC系统性能。

Result: JELV的LLM-as-Judges管道与人工标注者达成90%的一致性，DeBERTa分类器在有效编辑上达到85%的准确率。通过集成JELV，新的评估指标与人类判断实现了最先进的相关性。使用JELV过滤LLM生成的数据集扩充后，GEC系统在重新训练后取得了可衡量的性能提升。

Conclusion: JELV提供了一个可扩展的解决方案，能够增强参考多样性，并同时加强语法纠错系统的评估和模型泛化能力。
...
### [251] [47B Mixture-of-Experts Beats 671B Dense Models on Chinese Medical Examinations](https://arxiv.org/abs/2511.21701)
...
### [252] [CSV-Decode: Certifiable Sub-Vocabulary Decoding for Efficient Large Language Model Inference](https://arxiv.org/abs/2511.21702)
...
### [253] [Evaluating Embedding Generalization: How LLMs, LoRA, and SLERP Shape Representational Geometry](https://arxiv.org/abs/2511.21703)
*Siyaxolisa Kabane*
...
### [254] [On the Cross-lingual Transferability of Pre-trained wav2vec2-based Models](https://arxiv.org/abs/2511.21704)
...
### [255] [Insight-A: Attribution-aware for Multimodal Misinformation Detection](https://arxiv.org/abs/2511.21705)
...
### [256] [A General Highly Accurate Online Planning Method Integrating Large Language Models into Nested Rollout Policy Adaptation for Dialogue Tasks](https://arxiv.org/abs/2511.21706)
...
### [257] [Lost in the Pipeline: How Well Do Large Language Models Handle Data Preparation?](https://arxiv.org/abs/2511.21708)
...
### [258] [Addressing Stereotypes in Large Language Models: A Critical Examination and Mitigation](https://arxiv.org/abs/2511.21711)
*Fatima Kazi*

Main category: cs.CL

TL;DR: 本研究全面评估并强调了大型语言模型（LLMs）中偏见的普遍性及其缓解的必要性，通过基准测试和增强学习策略来识别和减轻偏见。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）从训练数据中继承了显性和隐性偏见，包括社会、伦理、文化、宗教等方面的偏见和刻板印象。这些偏见可能导致有害的刻板印象和错误信息，因此需要全面审查、识别、理解其来源并尝试缓解这些偏见，以确保公平输出。

Method: 研究采用了偏差特定基准测试（如StereoSet和CrowSPairs）来评估BERT、GPT 3.5和ADA等多种生成模型中的各种偏见。为检测显性和隐性偏见，采用了三管齐下的方法进行全面分析。此外，还应用了增强学习策略，包括微调模型、使用不同的提示技术和对偏见基准进行数据增强，以提升模型性能。

Result: 结果显示，微调模型在性别偏见方面表现不佳，但在识别和避免种族偏见方面表现出色。研究还发现，LLMs经常过度依赖提示中的关键词，表明其无法真正理解输出的准确性和真实性。通过增强学习策略，微调模型在跨数据集测试中表现出良好的适应性，并在隐性偏见基准上显著提升了性能，性能提升高达20%。

Conclusion: LLMs中的偏见是一个需要解决的关键问题。尽管在某些方面取得了成功，但模型在处理特定偏见（如性别偏见）和避免过度依赖关键词方面仍面临挑战。增强学习策略，特别是微调和数据增强，可以显著提高模型在隐性偏见方面的性能和跨数据集的适应性，为缓解LLMs中的偏见提供了有前景的方向。
...
### [259] [Quantifying and Mitigating Selection Bias in LLMs: A Transferable LoRA Fine-Tuning and Efficient Majority Voting Approach](https://arxiv.org/abs/2511.21709)
*Blessed Guda,Lawrence Francis,Gabrial Zencha Ashungafac,Carlee Joe-Wong,Moise Busogi*

Main category: cs.CL

TL;DR: 该研究针对大型语言模型（LLMs）在多项选择题（MCQ）中存在的选择偏差问题，提出了一种新的无监督置换偏差度量（PBM）、一种高效的多数投票方法（BaQCKV）和一种无监督的LoRA微调策略（LoRA-1），旨在提高MCQ评估的可靠性并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: LLMs在MCQ任务中常表现出选择偏差，即其选择受答案位置或符号而非内容影响，这损害了MCQ作为评估框架的可靠性。现有偏差度量通常需要答案标签且无法充分捕捉预测一致性，而现有缓解策略（如多数投票）计算成本高昂或（如校准方法）泛化能力差。

Method: 1. 提出了一种新的无监督、无需标签的置换偏差度量（PBM），用于量化模型预测在答案选项排列变化时的一致性。2. 开发了一种高效的多数投票方法，称为批量问题-上下文KV缓存（BaQCKV），以显著降低计算成本。3. 基于PBM和BaQCKV，提出了一种无监督的低秩适应（LoRA-1）微调策略，以缓解选择偏差。

Result: 实验结果表明，该研究提出的方法在多个MCQ基准测试中有效减少了模型偏差，提高了准确性的一致性，同时最大程度地降低了计算成本。

Conclusion: 该研究通过引入新的无监督偏差度量、高效的多数投票和微调策略，成功解决了LLMs在MCQ评估中的选择偏差问题，提高了评估的可靠性和效率，并保持了模型的泛化能力。
...
### [260] [EulerESG: Automating ESG Disclosure Analysis with LLMs](https://arxiv.org/abs/2511.21712)
...
TL;DR: EulerESG是一个基于LLM的系统，旨在自动化ESG报告披露分析，通过显式建模ESG框架，实现高精度和实用性，以解决现有ESG报告难以系统化分析的问题。


<details>
  <summary>Details</summary>
Motivation: ESG报告通常是冗长、异构的PDF文档，难以系统地回答简单问题。现有工具要么依赖脆弱的基于规则的提取，要么将ESG报告视为通用文本，未能明确建模底层的报告标准。

Method: EulerESG结合了(i)双通道检索和LLM驱动的披露分析，以及(ii)一个交互式仪表板和聊天机器人，用于探索、基准测试和解释。它明确感知并建模ESG框架。

Result: EulerESG能够以高保真度（平均准确率高达0.95）自动填充符合标准的指标表格，同时在端到端运行时保持实用性。该研究使用了四家全球知名公司和十二个SASB子行业进行验证，并比较了几种LLM模型在此设置下的表现。

Conclusion: EulerESG成功地提供了一个LLM驱动的解决方案，通过显式考虑ESG框架，自动化了ESG披露分析，实现了高准确性和实用效率，有效解决了ESG报告分析的挑战。
...
### [261] [GPS: General Per-Sample Prompter](https://arxiv.org/abs/2511.21714)
*Pawel Batorski,Paul Swoboda*

Main category: cs.CL

TL;DR: 本文提出GPS，一种通用的、逐样本提示方法，无需任务特定训练即可为每个输入生成定制提示，并在多项任务上表现出竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有自动提示方法存在三个局限性：需要大量数据集进行任务特定训练，优化循环耗时，以及无法为单个输入问题生成适应性提示。

Method: GPS是一种逐样本提示方法，通过强化学习在一系列训练任务上进行训练，并包含新颖的正则化技术以有效适应逐样本提示。此外，它采用最小贝叶斯风险解码来稳定推理。

Result: 在未针对特定任务进行训练的情况下，GPS在文本简化任务中达到基线方法中第二好的结果，在摘要任务中达到第三好的结果，在分类任务中达到与基线方法相当的结果。在域内提示方面，GPS在GSM8K上取得了SOTA（当前最佳）性能。

Conclusion: GPS展示了一种新颖有效的自动提示范式潜力：无需大量优化和任务特定训练集，即可生成自适应、输入特定的提示。
...
### [262] [An Optimized Machine Learning Classifier for Detecting Fake Reviews Using Extracted Features](https://arxiv.org/abs/2511.21716)
*Shabbir Anees,Anshuman,Ayush Chaurasia,Prathmesh Bogar*

Main category: cs.CL

TL;DR: 本研究提出了一种先进的机器学习系统，结合文本预处理、多模态特征提取、HHO特征选择和堆叠集成分类器，以高精度检测计算机生成的（CG）评论。


<details>
  <summary>Details</summary>
Motivation: 欺诈性评论，尤其是计算机生成的评论，严重损害了在线购物的合法性和可靠性，使消费者难以辨别真实信息，因此需要一种有效的方法来识别这些评论。

Method: 该方法整合了先进的文本预处理、多模态特征提取、使用Harris Hawks Optimization (HHO) 进行特征选择，以及一个堆叠集成分类器。它在一个包含40,432条原始和CG评论的公共数据集上进行了实现。

Result: HHO将初始的13,539个特征减少到1,368个（降维89.9%）。最终的堆叠模型达到了95.40%的准确率、92.81%的精确率、95.01%的召回率和93.90%的F1分数。

Conclusion: 集成学习与生物启发优化相结合是识别机器生成文本的有效方法。同时，在大规模评论分析系统中，隐私保护技术（如差分隐私和安全外包）对于保护用户数据至关重要。
...
### [263] [CrossCheck-Bench: Diagnosing Compositional Failures in Multimodal Conflict Resolution](https://arxiv.org/abs/2511.21717)
...
TL;DR: 该研究引入了CrossCheck-Bench，一个用于评估多模态大语言模型（MLLMs）在真实世界中检测和解决跨模态矛盾能力的诊断基准。结果显示，当前最先进的模型在感知匹配任务上表现良好，但在需要逻辑推理和多线索整合的矛盾检测任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型主要在对齐的图像-文本对上训练和评估，导致它们在检测和解决真实世界中视觉和文本线索冲突的能力方面探索不足。开放域应用中常出现冲突，需要模型进行超越表面对齐的结构化推理。

Method: 引入了CrossCheck-Bench，一个用于多模态输入中矛盾检测的诊断基准。该基准采用分层任务框架，涵盖三个推理复杂性级别和七种原子能力。数据集包含15k个问答对，来源于真实世界的人工制品并注入了合成矛盾。通过多阶段标注流程（超过450专家小时）构建，以确保语义有效性和难度校准。评估了13个最先进的视觉-语言模型，并分析了不同提示策略的效果。

Result: 观察到模型性能随着任务从感知匹配转向逻辑矛盾检测而持续下降。大多数模型在孤立实体识别上表现良好，但在需要综合多个线索进行冲突推理时失败。能力级别分析揭示了技能获取不均衡，尤其是在需要多步推理或基于规则验证的任务中。传统的提示策略（如CoT）仅带来微小收益，而将符号推理与视觉处理交错的方法实现了更稳定的改进。

Conclusion: 这些结果突出了多模态推理中存在的持续瓶颈，并为构建能够进行鲁棒跨模态验证的模型指明了新的方向。
...
### [264] [When Harmless Words Harm: A New Threat to LLM Safety via Conceptual Triggers](https://arxiv.org/abs/2511.21718)
...
### [265] [PeerCoPilot: A Language Model-Powered Assistant for Behavioral Health Organizations](https://arxiv.org/abs/2511.21721)
...
### [266] [German General Personas: A Survey-Derived Persona Prompt Collection for Population-Aligned LLM Studies](https://arxiv.org/abs/2511.21722)
...
### [267] [AD-CDO: A Lightweight Ontology for Representing Eligibility Criteria in Alzheimer's Disease Clinical Trials](https://arxiv.org/abs/2511.21724)
...
### [268] [PromptTailor: Multi-turn Intent-Aligned Prompt Synthesis for Lightweight LLMs](https://arxiv.org/abs/2511.21725)
...
### [269] [Goal-Directed Search Outperforms Goal-Agnostic Memory Compression in Long-Context Memory Tasks](https://arxiv.org/abs/2511.21726)
...
### [270] [Affective Multimodal Agents with Proactive Knowledge Grounding for Emotionally Aligned Marketing Dialogue](https://arxiv.org/abs/2511.21728)
...
### [271] [Beyond Component Strength: Synergistic Integration and Adaptive Calibration in Multi-Agent RAG Systems](https://arxiv.org/abs/2511.21729)
*Jithin Krishnan*
...
### [272] [A Benchmark for Procedural Memory Retrieval in Language Agents](https://arxiv.org/abs/2511.21730)
...
### [273] [HUMORCHAIN: Theory-Guided Multi-Stage Reasoning for Interpretable Multimodal Humor Generation](https://arxiv.org/abs/2511.21732)
*Jiajun Zhang,Shijia Luo,Ruikang Zhang,Qi Su*

Main category: cs.CL

TL;DR: 本文提出HUMORCHAIN，一个理论指导的多阶段推理框架，通过将幽默理论的认知结构嵌入多模态幽默生成，解决了现有AI幽默生成缺乏理论基础的问题，并在多个数据集上超越了现有技术。


<details>
  <summary>Details</summary>
Motivation: 幽默生成对AI而言极具挑战，需要复杂的认知推理和社交理解。现有数据驱动方法在多模态幽默生成中缺乏明确的幽默理论建模，导致生成的描述流畅但缺乏真正的幽默感和认知深度。鉴于多模态幽默在在线交流中的普及，迫切需要能整合视觉理解和幽默语言生成的AI系统。

Method: 本文提出了HUMORCHAIN（HUmor-guided Multi-step Orchestrated Reasoning Chain for Image Captioning），一个理论指导的多阶段推理框架。它整合了视觉语义解析、基于幽默和心理学的推理，以及一个用于幽默评估的微调判别器，形成了一个可解释和可控的认知推理链。该方法首次将幽默理论中的认知结构明确嵌入多模态幽默生成。

Result: 在Meme-Image-No-Text、Oogiri-GO和OxfordTVG-HIC数据集上的实验表明，HUMORCHAIN在人类幽默偏好、Elo/BT分数和语义多样性方面均优于现有最先进的基线模型。

Conclusion: 研究表明，理论驱动的结构化推理能够使大型语言模型生成与人类感知相符的幽默。将幽默理论的认知结构明确嵌入生成过程是有效的。
...
### [274] [Identifying Quantum Structure in AI Language: Evidence for Evolutionary Convergence of Human and Artificial Cognition](https://arxiv.org/abs/2511.21731)
...
TL;DR: 该研究表明，大型语言模型（LLMs）在概念组合中表现出“量子纠缠”和“玻色-爱因斯坦统计”，这与人类认知测试的结果一致，指出概念-语言领域中普遍存在量子结构。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索LLMs在概念组合方面的认知能力，并将其与人类认知进行比较，以揭示知识组织的基本形式和人类与LLM认知之间的潜在趋同性。

Method: 使用ChatGPT和Gemini作为测试对象，进行了两项认知测试：1) 测量概念组合中贝尔不等式的违反情况，以检测“量子纠缠”；2) 分析大型文本中词语分布的统计规律，以识别是“玻色-爱因斯坦统计”还是“麦克斯韦-玻尔兹曼统计”。

Result: 第一个测试显示贝尔不等式显著违反，表明概念中存在“量子纠缠”。第二个测试发现词语分布符合“玻色-爱因斯坦统计”，而非预期的“麦克斯韦-玻尔兹曼统计”。这些结果与人类参与者认知测试和大型语料库信息检索测试的发现相似，共同指出“概念-语言领域中量子结构的系统性出现”，无论认知主体是人类还是人工智能。

Conclusion: 研究得出结论，量子结构系统地出现在人类和LLM的认知-语言领域。作者认为，这源于神经网络之上向量空间的分布式语义结构中更本质的知识组织形式，代表了人类认知与语言（通过生物进化建立）和LLM认知与语言（通过自学习和训练快速出现）之间的一种演化趋同现象。论文还提出了一个统一框架来解释这种普遍的意义量子组织。
...
### [275] [RoSA: Enhancing Parameter-Efficient Fine-Tuning via RoPE-aware Selective Adaptation in Large Language Models](https://arxiv.org/abs/2511.21733)
*Dayan Pan,Jingyuan Wang,Yilong Zhou,Jiawei Cheng,Pengyue Jia,Xiangyu Zhao*

Main category: cs.CL

TL;DR: RoSA是一种新的参数高效微调（PEFT）框架，通过选择性增强旋转位置编码（RoPE）影响的低频注意力分量，并动态选择关键层进行适应，实现了更高效的大语言模型微调。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法通常忽视模型组件的不同作用和层间重要性的异质性，限制了适应效率。研究观察到RoPE在注意力状态的低频维度中诱导关键激活，这启发了更具针对性的参数分配。

Method: RoSA框架包含两个主要部分：1) RoPE感知注意力增强（RoAE）模块，选择性地增强受RoPE影响的注意力状态的低频分量；2) 动态层选择（DLS）策略，根据LayerNorm梯度范数自适应地识别并更新最关键的层。通过维度级增强和层级适应相结合，实现更有针对性和效率的微调。

Result: 在十五个常识和算术基准测试中，RoSA在可比的训练参数下，性能优于现有的主流PEFT方法。

Conclusion: RoSA通过结合维度级增强和层级适应，提供了一种更具针对性和效率的大语言模型微调方法。
...
### [276] [Asking LLMs to Verify First is Almost Free Lunch](https://arxiv.org/abs/2511.21734)
*Shiguang Wu,Quanming Yao*

Main category: cs.CL

TL;DR: 本文提出了“验证优先”（Verification-First, VF）策略，通过在生成解决方案前先验证一个候选答案来增强大型语言模型（LLM）的推理能力，且成本低廉。VF触发了“逆向推理”，有效减少逻辑错误。其迭代版本Iter-VF进一步提升了性能，超越了现有测试时扩展（TTS）策略。


<details>
  <summary>Details</summary>
Motivation: 在不增加高昂训练成本或大量测试时采样的情况下，提升大型语言模型（LLM）的推理能力。

Method: 引入了“验证优先”（VF）策略，该策略提示模型在生成解决方案之前先验证一个提供的（即使是随机的）候选答案，以触发“逆向推理”过程。进一步推广为Iter-VF，这是一种顺序测试时扩展（TTS）方法，它使用模型之前的答案迭代地循环验证-生成过程。

Result: VF（使用随机答案）在计算开销极小的情况下，始终优于标准的思维链（CoT）方法。Iter-VF优于现有的测试时扩展（TTS）策略。这些结果在各种基准（从数学推理到编程和代理任务）和各种LLM（从开源1B到尖端商业模型）上得到了验证。

Conclusion: 验证优先（VF）及其迭代版本Iter-VF是有效且高效的策略，能够通过逆向推理和迭代验证显著增强LLM的推理能力，并超越了当前的基线方法，且计算成本较低。
...
### [277] [Closing the Performance Gap Between AI and Radiologists in Chest X-Ray Reporting](https://arxiv.org/abs/2511.21735)
...
TL;DR: MAIRA-X是一个多模态AI模型，用于生成胸部X射线报告，涵盖临床发现和管线报告。该模型在多个数据集和用户评估中表现出色，能有效辅助放射科医生，尤其在高工作量环境下。


<details>
  <summary>Details</summary>
Motivation: 放射科医生面临工作量巨大（因筛查指南扩大、复杂病例和人手短缺）的问题，同时，解读胸部X射线报告中的管线（L&T）既费时又重复。因此，需要AI辅助来减轻放射科医生的负担，同时保持诊断准确性。

Method: 本研究引入了MAIRA-X，一个多模态AI模型，用于生成纵向胸部X射线（CXR）报告，涵盖临床发现和管线（L&T）报告。该模型基于Mayo Clinic的310万项研究（包含600万张图像，来自80.6万名患者）的大规模、多中心、纵向数据集开发。模型在三个保留数据集和公共MIMIC-CXR数据集上进行了评估。此外，开发了一个新颖的L&T特异性评估指标框架，用于评估报告中管线的类型、纵向变化和位置的准确性。还进行了一项首次回顾性用户评估研究，由九名经验不等的放射科医生盲审了600项独立研究。

Result: MAIRA-X在词汇质量、临床正确性和L&T相关元素方面显著优于现有技术。用户研究发现，原始报告与AI生成报告的关键错误率相当（原始3.0% vs. AI 4.6%），可接受句子率也相似（原始97.8% vs. AI 97.4%）。这些结果显著改善了以往用户研究中存在的较大差距和较高错误率。

Conclusion: 研究结果表明，MAIRA-X能够有效地辅助放射科医生，特别是在高工作量的临床环境中。
...
### [278] [Polarity-Aware Probing for Quantifying Latent Alignment in Language Models](https://arxiv.org/abs/2511.21737)
*Sabrina Sadiekh,Elena Ericheva,Chirag Agarwal*

Main category: cs.CL

TL;DR: 本文通过引入极性感知对比一致性搜索（PA-CCS）及其相关指标，研究了无监督探针在评估语言模型对齐方面的可靠性，并揭示了模型内部有害知识编码的差异和语义鲁棒性需求。


<details>
  <summary>Details</summary>
Motivation: 随着对比一致性搜索（CCS）等无监督探针的进步，它们能够揭示潜在信念而不依赖于token输出，这引发了这些方法能否可靠评估模型对齐性的问题。

Method: 研究引入了极性感知对比一致性搜索（PA-CCS），用于评估模型内部表示在极性反转下的一致性。提出了两个面向对齐的指标：极性一致性（Polar-Consistency）和矛盾指数（Contradiction Index），以量化模型潜在知识的语义鲁棒性。构建了包含有害-安全句子对的数据集（并发和对抗性语句），并将PA-CCS应用于16个语言模型进行验证。

Result: PA-CCS能够识别模型在编码潜在有害知识方面的架构和层级差异。值得注意的是，将否定词替换为无意义标记会降低内部表示良好对齐的模型的PA-CCS分数，而缺乏稳健内部校准的模型则没有这种退化。

Conclusion: 研究结果强调了无监督探针在对齐评估方面的潜力，并强调了将结构鲁棒性检查纳入可解释性基准的必要性。
...
### [279] [R2Q: Towards Robust 2-Bit Large Language Models via Residual Refinement Quantization](https://arxiv.org/abs/2511.21736)
...
### [280] [Decoding inner speech with an end-to-end brain-to-text neural interface](https://arxiv.org/abs/2511.21740)
...
### [281] [A Multiscale Geometric Method for Capturing Relational Topic Alignment](https://arxiv.org/abs/2511.21741)
...
### [282] [EduMod-LLM: A Modular Approach for Designing Flexible and Transparent Educational Assistants](https://arxiv.org/abs/2511.21742)
...
### [283] [Scaling Competence, Shrinking Reasoning: Cognitive Signatures in Language Model Learning](https://arxiv.org/abs/2511.21743)
...
### [284] [A Lightweight Approach to Detection of AI-Generated Texts Using Stylometric Features](https://arxiv.org/abs/2511.21744)
...
### [285] [DELTA: Language Diffusion-based EEG-to-Text Architecture](https://arxiv.org/abs/2511.21746)
...
### [286] [Building Domain-Specific Small Language Models via Guided Data Generation](https://arxiv.org/abs/2511.21748)
...
TL;DR: 本文提出了一种经济高效且可扩展的训练流程，通过结合引导式合成数据生成和自下而上的领域数据整理，为特定领域训练小型语言模型（SLMs）。通过DiagnosticSLM（3B参数）模型，在工业故障诊断任务中展示了其有效性，并在新基准测试中显著优于现有开源模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在专业领域部署面临数据隐私（SaaS模式）和计算资源（开源模型微调）的挑战。开发小型、领域专用的LLMs因缺乏高质量的领域特定训练数据而受限。本研究旨在解决这些限制。

Method: 开发了一个经济高效且可扩展的训练流程，该流程结合了从少量种子语料库中引导的合成数据生成与自下而上的领域数据整理。该流程整合了领域自适应预训练（DAPT）、领域特定监督微调（DSFT）和直接偏好优化（DPO）。通过DiagnosticSLM（一个3B参数模型）在工业故障诊断、根本原因分析和维修推荐任务中验证了该方法。引入了四个领域特定基准（DiagnosticMCQ、DiagnosticQA、DiagnosticComp、DiagnosticSum）来评估模型性能。

Result: DiagnosticSLM在MCQ任务上比2B-9B参数的开源模型实现了高达25%的准确率提升，并在其他任务中表现出超越或持平的性能。这表明了其有效的领域特定推理和泛化能力。

Conclusion: 所提出的训练流程能够有效地训练小型、领域专用的语言模型，克服了数据稀缺和资源限制。DiagnosticSLM的成功证明了该方法在专业领域任务中实现卓越性能的潜力。
...
### [287] [Semantics as a Shield: Label Disguise Defense (LDD) against Prompt Injection in LLM Sentiment Classification](https://arxiv.org/abs/2511.21752)
*Yanxi Li,Ruocheng Shan*

Main category: cs.CL

TL;DR: 本文提出了一种名为标签伪装防御（LDD）的轻量级、模型无关策略，通过使用别名标签隐蔽真实标签，并通过少样本学习使其隐式映射，从而有效抵御大型语言模型在文本分类任务中的指令注入攻击。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文本分类任务中日益普及，但其对自然语言提示的依赖使其容易受到提示注入攻击，特别是利用模型标签集知识来覆盖预期行为的类指令注入。现有防御措施（如检测过滤器、指令层级、签名提示）要么需要模型再训练，要么容易被混淆攻击绕过。

Method: 引入标签伪装防御（LDD），这是一种轻量级且模型无关的策略。它通过用语义转换或不相关的别名标签（例如，蓝色 vs. 黄色）替换真实标签来隐藏它们。模型通过少样本演示隐式学习这些新的标签映射，从而阻止注入指令与决策输出之间的直接对应关系。

Result: LDD能够恢复所有九个评估模型（包括GPT-5、GPT-4o、LLaMA3.2、Gemma3和Mistral变体）在攻击下损失的部分准确性。对于绝大多数模型，可以找到不止一对别名标签，其准确性高于受攻击基线。语言分析进一步表明，语义对齐的别名标签（例如，好 vs. 坏）比非对齐符号（例如，蓝色 vs. 黄色）提供更强的鲁棒性。
...
### [288] [Proactive Defense: Compound AI for Detecting Persuasion Attacks and Measuring Inoculation Effectiveness](https://arxiv.org/abs/2511.21749)
*Svitlana Volkova,Will Dupree,Hsien-Te Kao,Peter Bautista,Gabe Ganberg,Jeff Beaubien,Laura Cassani*

Main category: cs.CL

TL;DR: 该论文提出了BRIES，一种复合AI架构，旨在检测和衡量信息环境中说服攻击的有效性。它通过专门的代理、对不同大型语言模型（LLMs）的比较分析和提示工程实验，揭示了LLMs在检测说服技巧方面的差异和脆弱性，并提出了增强人类认知韧性的框架。


<details>
  <summary>Details</summary>
Motivation: 研究动机是检测和衡量信息环境中的说服攻击，量化LLM对说服攻击的脆弱性，并为增强人类认知韧性提供一个框架，以提升生成式AI安全和认知安全。

Method: 研究方法包括：1) 引入BRIES复合AI架构，包含Twister（生成对抗内容）、Detector（识别攻击类型）、Defender（生成弹性内容）和Assessor（评估有效性）等专业代理。2) 基于SemEval 2023 Task 3分类法和合成说服数据集进行实验。3) 比较分析不同LLM（GPT-4、Llama3、Mistral等）的检测性能。4) 探索提示工程（如温度设置和置信度评分）对检测效果的影响。5) 采用因果推断评估内容接种的有效性，并揭示说服攻击的社会情感认知特征。

Result: 主要结果包括：1) 不同语言代理在检测性能上存在显著差异。2) GPT-4在复杂说服技巧上表现出卓越的检测准确性，而Llama3和Mistral等开源模型在识别微妙修辞方面表现出明显弱点。3) 提示工程显著影响检测效率，其中温度设置和置信度评分会产生模型特异性变化（Gemma和GPT-4在较低温度下表现最佳，Llama3和Mistral在较高温度下表现更佳）。4) 因果分析揭示了说服攻击的不同类型针对特定的认知维度。

Conclusion: 该研究量化了LLM对说服攻击的特定脆弱性，并提供了一个通过结构化干预增强人类认知韧性的框架，从而推动了生成式AI安全和认知安全的发展。
...
### [289] [Extracting Disaster Impacts and Impact Related Locations in Social Media Posts Using Large Language Models](https://arxiv.org/abs/2511.21753)
*Sameeah Noreen Hameed,Surangika Ranathunga,Raj Prasanna,Kristin Stock,Christopher B. Jones*

Main category: cs.CL

TL;DR: 本研究利用微调的大型语言模型（LLMs）从灾害相关的社交媒体帖子中识别灾害影响和受影响地点，以克服传统数据源的局限性，并提高灾害响应中的资源分配效率。


<details>
  <summary>Details</summary>
Motivation: 大规模灾害造成巨大影响，但传统权威数据（传感器、遥感、地理数据）常因大气、卫星重访和时间限制而存在地理-时间信息空白。社交媒体帖子可作为“地理传感器”提供灾害信息，但并非所有提及的地点都受到影响，识别出受影响地点对于有效分配资源至关重要。

Method: 研究采用大型语言模型（LLMs），并对其进行微调，以识别灾害相关的社交媒体帖子中提及的所有地点、影响和受影响地点。微调后的模型专门区分受影响地点与其他非受影响地点，并能处理非正式表达、缩写和简称。

Result: 微调后的模型在影响提取方面取得了0.69的F1分数，在受影响地点提取方面取得了0.74的F1分数，显著优于预训练的基线模型。

Conclusion: 这些稳健的结果证实了微调语言模型在为灾害响应人员提供可扩展解决方案方面的潜力，有助于及时决策、资源分配、态势感知和灾后恢复规划。
...
### [290] [Dissecting the Ledger: Locating and Suppressing "Liar Circuits" in Financial Large Language Models](https://arxiv.org/abs/2511.21756)
*Soham Mirajkar*

Main category: cs.CL

TL;DR: 本文通过因果追踪揭示了GPT-2 XL在金融算术推理中存在一个双阶段机制（中间层计算草稿与后期聚合电路），并发现抑制特定层（L46）可显著降低模型对幻觉输出的信心，且在该层训练的线性探针能以高准确率检测算术幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在金融领域部署日益增多，但它们在算术运算中存在可复现的幻觉。现有缓解策略多为黑盒方法，亟需一种内在的幻觉检测机制。

Method: 研究人员对GPT-2 XL架构在ConvFinQA基准上应用了因果追踪（Causal Tracing）来识别算术推理机制。通过消融研究验证了该机制，并训练了一个线性探针来检测幻觉。

Result: 识别出LLM算术推理的双阶段机制：中间层（L12-L30）的分布式计算草稿和后期层（特别是L46）的决定性聚合电路。抑制L46可使模型对幻觉输出的信心降低81.8%。在该层训练的线性探针在未见过的金融主题上达到98%的准确率。

Conclusion: LLM中存在一种普遍的算术欺骗几何结构，通过对特定层（如L46）进行探测，可以实现内在的幻觉检测。
...
### [291] [Temporal Consistency for LLM Reasoning Process Error Identification](https://arxiv.org/abs/2503.14495)
*Jiacheng Guo,Yue Wu,Jiahao Qiu,Kaixuan Huang,Xinzhe Juan,Ling Yang,Mengdi Wang*

Main category: cs.CL

TL;DR: 本文提出了一种新的时间一致性方法，通过验证器基于先前评估迭代地完善判断，提高数学推理中的验证准确性。该方法在多个基准测试中优于基线，并使小型蒸馏模型超越大型模型。


<details>
  <summary>Details</summary>
Motivation: 有效的数学推理需要精确的验证。现有的一次性验证或多模型辩论方法可能不足以达到最佳的验证准确性。

Method: 本研究采用一种时间一致性方法。验证器会根据先前的评估迭代地完善其判断，通过一系列自我反思动作中的一致性来提高验证准确性。

Result: 在Mathcheck、ProcessBench和PRM800K等数学过程错误识别基准测试中，该方法持续优于基线方法。当应用于DeepSeek R1蒸馏模型时，使用该方法的7B/8B蒸馏模型在ProcessBench上超越了所有70B/72B模型和GPT-4o。值得注意的是，使用该方法的14B蒸馏模型实现了与Deepseek-R1相当的性能。

Conclusion: 所提出的时间一致性方法显著提高了数学推理中的验证准确性。它使较小的蒸馏模型能够超越更大的模型和先进的LLM，证明了其在提升验证能力方面的强大效果。
...
### [292] [Lips-Jaw and Tongue-Jaw Articulatory Tradeoff in DYNARTmo](https://arxiv.org/abs/2511.22155)
*Bernd J. Kröger*

Main category: cs.CL

TL;DR: 本文研究了DYNARTmo动态发音模型如何解释主要和次要发音器官（如唇-颌、舌-颌）之间的发音权衡和协同作用，并证明即使简化了生物力学假设，该模型也能生成逼真的发音模式。


<details>
  <summary>Details</summary>
Motivation: 研究DYNARTmo动态发音模型如何处理主要和次要发音器官之间的发音权衡，特别是唇-颌和舌-颌的协调，并验证其在简化计算假设下的能力。

Method: 采用DYNARTmo动态发音模型，该模型使用与发音音系学相似的一阶任务空间手势规范，并集成了一个简化的发音努力分配机制。通过模拟一系列CV音节（唇音、舌尖音、舌背音，以及/a/、/i/、/u/元音环境）来观察下颌位移的变化。

Result: 模型再现了经验证实的关节协同模式，包括下颌支撑的舌尖闭合、双唇塞音中的下唇抬高、舌-颌协同运动以及唇部收缩中的饱和效应。下颌位移随发音部位和元音环境的不同而变化。

Conclusion: DYNARTmo模型即使在计算假设简化的前提下，也能生成逼真的时空运动模式，捕捉到各种辅音-元音组合中发音权衡和协同作用的关键方面。
...
### [293] [On the Role of Preference Variance in Preference Optimization](https://arxiv.org/abs/2510.13022)
...
### [294] [Orchestrating Dual-Boundaries: An Arithmetic Intensity Inspired Acceleration Framework for Diffusion Language Models](https://arxiv.org/abs/2511.21759)
...
### [295] [fMRI-LM: Towards a Universal Foundation Model for Language-Aligned fMRI Understanding](https://arxiv.org/abs/2511.21760)
...
### [296] [LLMs for Low-Resource Dialect Translation Using Context-Aware Prompting: A Case Study on Sylheti](https://arxiv.org/abs/2511.21761)
...
### [297] [Factors That Support Grounded Responses in LLM Conversations: A Rapid Review](https://arxiv.org/abs/2511.21762)
...
### [298] [Improving Score Reliability of Multiple Choice Benchmarks with Consistency Evaluation and Altered Answer Choices](https://arxiv.org/abs/2511.21860)
...
### [299] [FLAWS: A Benchmark for Error Identification and Localization in Scientific Papers](https://arxiv.org/abs/2511.21843)
...
### [300] [AfriStereo: A Culturally Grounded Dataset for Evaluating Stereotypical Bias in Large Language Models](https://arxiv.org/abs/2511.22016)
...
### [301] [A Customer Journey in the Land of Oz: Leveraging the Wizard of Oz Technique to Model Emotions in Customer Service Interactions](https://arxiv.org/abs/2511.21909)
...
### [302] [Tracing How Annotators Think: Augmenting Preference Judgments with Reading Processes](https://arxiv.org/abs/2511.21912)
...
### [303] [A Theoretically Grounded Hybrid Ensemble for Reliable Detection of LLM-Generated Text](https://arxiv.org/abs/2511.22153)
...
### [304] [A Comparative Study of LLM Prompting and Fine-Tuning for Cross-genre Authorship Attribution on Chinese Lyrics](https://arxiv.org/abs/2511.21930)
...
### [305] [Focused Chain-of-Thought: Efficient LLM Reasoning via Structured Input Information](https://arxiv.org/abs/2511.22176)
...
### [306] [Start Making Sense(s): A Developmental Probe of Attention Specialization Using Lexical Ambiguity](https://arxiv.org/abs/2511.21974)
...
### [307] [ResearchArcade: Graph Interface for Academic Tasks](https://arxiv.org/abs/2511.22036)
...
### [308] [Early Risk Prediction with Temporally and Contextually Grounded Clinical Language Processing](https://arxiv.org/abs/2511.22038)
...
### [309] [A Hybrid Theory and Data-driven Approach to Persuasion Detection with Large Language Models](https://arxiv.org/abs/2511.22109)
...
### [310] [Mapping Clinical Doubt: Locating Linguistic Uncertainty in LLMs](https://arxiv.org/abs/2511.22402)
...
### [311] [Exploring Performance Variations in Finetuned Translators of Ultra-Low Resource Languages: Do Linguistic Differences Matter?](https://arxiv.org/abs/2511.22482)
...
### [312] [Bridging the Modality Gap by Similarity Standardization with Pseudo-Positive Samples](https://arxiv.org/abs/2511.22141)
...
### [313] [RefineBench: Evaluating Refinement Capability of Language Models via Checklists](https://arxiv.org/abs/2511.22173)
...
TL;DR: 本研究引入了RefineBench基准测试，评估语言模型（LMs）的自我完善能力。结果显示，在无指导的自我完善模式下，即使是前沿LMs表现也一般且提升有限；但在有针对性反馈的指导完善模式下，LMs能显著提升其响应质量。


<details>
  <summary>Details</summary>
Motivation: 现实世界中用户对LMs的交互常涉及修改请求。然而，以往研究多集中于可验证任务，而非开放式查询或不同程度的反馈。近期展现自我反思模式的推理模型也促使人们关注LMs的自我完善能力。

Method: 引入RefineBench，一个包含1000个挑战性问题（覆盖11个领域）的基准测试，并搭配基于清单的评估框架。评估了两种完善模式：1) 指导完善，LM获得自然语言反馈；2) 自我完善，LM在无指导下尝试改进。

Result: 在自我完善模式下，Gemini 2.5 Pro和GPT-5等前沿LMs的基线得分仅为31.3%和29.1%，且大多数模型未能持续改进（如Gemini-2.5-Pro仅提升1.8%）。相比之下，在指导完善模式下，专有LMs和大型开源LMs（>70B）能利用有针对性的反馈，在五轮内将响应完善到接近完美的水平。

Conclusion: 前沿LMs需要重大突破才能有效自我完善其不正确的响应。RefineBench提供了一个有价值的测试平台，用于跟踪该领域的进展。
...
### [314] [Beyond Query-Level Comparison: Fine-Grained Reinforcement Learning for Text-to-SQL with Automated Interpretable Critiques](https://arxiv.org/abs/2511.22258)
*Guifeng Wang,Yuanfeng Song,Meng Yang,Tao Zhu,Xiaoming Yin,Xing Chen*

Main category: cs.CL

TL;DR: 本文提出RuCo-C，一个新颖的生成式评判模型，用于Text-to-SQL任务中细粒度、自动化的评估和奖励机制，通过自动生成评判标准和可解释的反馈，并结合渐进式探索策略，解决了现有方法对人工标注的依赖和粗粒度奖励的问题。


<details>
  <summary>Details</summary>
Motivation: Text-to-SQL模型的评估和奖励机制存在瓶颈：现有方法严重依赖昂贵的人工标注黄金SQL，不适用于大规模评估；强化学习方法大多只使用最终二元执行结果作为奖励信号，这种粗粒度监督忽略了结构和语义上的详细错误。

Method: 本文提出RuCo-C，一个生成式评判模型。它首先自动生成特定查询的评估标准，并将其与可解释的批评联系起来，实现无需人工干预的标注。随后，在强化学习训练过程中，通过“渐进式探索”策略整合密集化的奖励反馈，动态调整奖励以提升模型性能。

Result: 全面的实验表明，RuCo-C在Text-to-SQL评估中优于现有方法，并取得了显著的性能提升。

Conclusion: RuCo-C模型有效解决了Text-to-SQL任务中评估和奖励机制的挑战，通过提供细粒度、自动化的评估和密集的奖励反馈，显著提升了模型的性能。
...
### [315] [Token-Level Marginalization for Multi-Label LLM Classifiers](https://arxiv.org/abs/2511.22312)
*Anjaneya Praharaj,Jaykumar Kasundra*

Main category: cs.CL

TL;DR: 本研究提出并评估了三种新颖的基于token的概率估计方法，以解决生成式语言模型在多标签内容安全分类中缺乏可解释置信度得分的问题，从而提高模型的可解释性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 生成式语言模型（如LLaMA Guard）在内容安全分类中缺乏直接的类别级概率，这阻碍了模型置信度评估、性能解释、动态阈值设置和细粒度错误分析，从而影响内容审核。

Method: 本文提出并评估了三种新颖的基于token的概率估计方法，旨在从生成式语言模型中导出可解释的置信度得分。研究通过在合成生成并严格标注的数据集上进行广泛实验，并评估了该框架在不同指令微调模型上的泛化能力。

Result: 实验证明，利用token logits显著提高了生成式分类器的可解释性和可靠性。

Conclusion: 该研究提出的方法能够为生成式语言模型提供更细致的内容安全审核能力，增强模型的可解释性和准确性。
...
### [316] [C$^2$DLM: Causal Concept-Guided Diffusion Large Language Models](https://arxiv.org/abs/2511.22146)
*Kairong Han,Nuanqiao Shan,Ziyu Zhao,Zijing Hu,Xinpeng Dong,Junjian Ye,Lujia Pan,Fei Wu,Kun Kuang*

Main category: cs.CL

TL;DR: 本文提出了一种因果概念引导的扩散语言模型 (C$^2$DLM)，通过从教师模型获取概念级因果图并指导注意力学习因果关系，显著提升了大型语言模型的推理能力和训练速度。


<details>
  <summary>Details</summary>
Motivation: 自回归 (AR) 语言模型和扩散语言模型 (DLM) 都存在推理能力不足的问题。人类推理依赖于因果知识，而AR模型仅进行左到右的预测，DLM则完全忽略因果顺序。现有模型在捕捉和利用自然语言中的因果结构方面存在缺陷。

Method: C$^2$DLM以DLM的全连接注意力机制为基础，首先从教师模型获取一个概念级的因果图，然后利用该图明确地指导注意力机制学习概念间的因果关系，从而避免因果倒置等困难子目标的干扰。

Result: C$^2$DLM在COT-OrderPerturb任务上性能提升12%，训练速度提升约3.2倍。在六个下游推理任务中，平均性能提升1.31%。

Conclusion: 通过将概念级因果图引入扩散语言模型并指导注意力学习因果关系，C$^2$DLM有效提升了模型的推理能力，并显著加快了训练速度，弥补了现有大型语言模型在因果推理方面的不足。
...
### [317] [Sentiment Analysis Of Shopee Product Reviews Using Distilbert](https://arxiv.org/abs/2511.22313)
*Zahri Aksa Dautd,Aviv Yuniar Rahman*

Main category: cs.CL

TL;DR: 本研究利用轻量级DistilBERT模型对Shopee产品评论进行情感分类，实现了高准确率和显著的计算效率提升，证明其适用于大规模电商情感分析。


<details>
  <summary>Details</summary>
Motivation: 数字商务的快速增长导致在线平台积累了海量的消费者评论，这些评论包含关于客户满意度和偏好的宝贵信息。然而，人工分析效率低下，因此需要像情感分析这样的计算方法。

Method: 本研究使用DistilBERT，一个基于Transformer的轻量级深度学习模型，对约一百万条经过预处理的英文Shopee产品评论进行情感分类。模型使用distilbert-base-uncased进行训练，并采用准确率、精确率、召回率和F1-分数进行评估，同时与BERT和SVM等基准模型进行比较。

Result: DistilBERT的准确率达到94.8%，略低于BERT（95.3%），但显著高于SVM（90.2%），并且计算时间减少了55%以上。

Conclusion: 研究结果表明，DistilBERT在准确性和效率之间取得了最佳平衡，使其非常适合在电商平台上进行大规模情感分析。
...
### [318] [Named Entity Recognition for the Kurdish Sorani Language: Dataset Creation and Comparative Analysis](https://arxiv.org/abs/2511.22315)
*Bakhtawar Abdalla,Rebwar Mala Nabi,Hassan Eshkiki,Fabio Caraffini*

Main category: cs.CL

TL;DR: 该研究为低资源语言库尔德语索拉尼方言构建了首个命名实体识别（NER）数据集，并比较了经典机器学习模型与神经网络系统，发现在低资源环境下，条件随机场（CRF）等经典方法显著优于基于BiLSTM的模型。


<details>
  <summary>Details</summary>
Motivation: 为了平衡自然语言处理技术的包容性和全球适用性，解决低资源和代表性不足的库尔德语索拉尼方言缺乏命名实体识别数据集的问题。

Method: 构建了包含64,563个标注tokens的库尔德语索拉尼方言命名实体识别数据集，并提供了一个标注工具。对经典机器学习模型（特别是CRF）和神经网络系统（特别是基于BiLSTM的模型）进行了彻底的比较分析。

Result: 经典机器学习方法（CRF）取得了0.825的F1分数，显著优于基于BiLSTM模型（0.706）的结果。这些发现挑战了神经网络方法在NLP领域中具有优势的既定假设。

Conclusion: 在低资源环境下，更简单、计算效率更高的经典框架（如CRF）可以胜过神经网络架构，表明并非所有情况下神经网络都占优。
...
### [319] [Joint Speech and Text Training for LLM-Based End-to-End Spoken Dialogue State Tracking](https://arxiv.org/abs/2511.22503)
*Katia Vendrame,Bolaji Yusuf,Santosh Kesiraju,Šimon Sedláček,Oldřich Plchot,Jan Černocký*

Main category: cs.CL

TL;DR: 本文提出一种联合训练方法，结合现有语音DST数据和来自其他领域的文本数据，以解决语音对话状态追踪（DST）在跨领域泛化和数据稀缺性方面的挑战，无需目标领域的语音训练数据即可获得良好性能。


<details>
  <summary>Details</summary>
Motivation: 端到端语音对话状态追踪（DST）面临语音输入处理和数据稀缺的双重挑战。尽管结合语音基础编码器和大型语言模型在单领域表现出色，但在跨领域泛化方面表现不佳，且为每个目标领域收集标注的语音DST训练数据成本高昂且困难。而文本DST数据则相对容易获取。

Method: 本文提出一种联合训练方法，利用可用的语音DST数据以及来自其他领域的书面文本数据。通过这种方式，旨在实现跨领域泛化，减少对目标领域语音训练数据的依赖。

Result: 实验结果表明，所提出的方法在无需目标领域语音训练数据的情况下，也能获得良好的跨领域DST性能，证明了其有效性。

Conclusion: 通过联合训练现有语音DST数据和跨领域文本数据，可以有效提高语音DST的跨领域泛化能力，克服目标领域语音数据稀缺的难题。
...
### [320] [Smarter, not Bigger: Fine-Tuned RAG-Enhanced LLMs for Automotive HIL Testing](https://arxiv.org/abs/2511.22584)
...
### [321] [Improving LLM-based Ontology Matching with fine-tuning on synthetic data](https://arxiv.org/abs/2511.22612)
...
### [322] [Extension Condition "violations" and Merge optimality constraints](https://arxiv.org/abs/2511.22582)
...
### [323] [Modeling Romanized Hindi and Bengali: Dataset Creation and Multilingual LLM Integration](https://arxiv.org/abs/2511.22769)
...
### [324] [Pooling Attention: Evaluating Pretrained Transformer Embeddings for Deception Classification](https://arxiv.org/abs/2511.22977)
...
### [325] [Mitigating Semantic Drift: Evaluating LLMs' Efficacy in Psychotherapy through MI Dialogue Summarization](https://arxiv.org/abs/2511.22818)
...
### [326] [JBE-QA: Japanese Bar Exam QA Dataset for Assessing Legal Domain Knowledge](https://arxiv.org/abs/2511.22869)
...
### [327] [Conveying Imagistic Thinking in TCM Translation: A Prompt Engineering and LLM-Based Evaluation Framework](https://arxiv.org/abs/2511.23059)
*Jiatong Han*
...
### [328] [RAG System for Supporting Japanese Litigation Procedures: Faithful Response Generation Complying with Legal Norms](https://arxiv.org/abs/2511.22858)
...
### [329] [FEANEL: A Benchmark for Fine-Grained Error Analysis in K-12 English Writing](https://arxiv.org/abs/2511.22883)
...
TL;DR: 大型语言模型（LLMs）在教育领域有巨大潜力，但本文通过引入FEANEL基准测试，揭示了它们在K-12英语写作的细粒度错误分析方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: LLMs已彻底改变人工智能，为教育应用提供了深刻机遇。然而，它们为K-12英语写作提供细粒度教育反馈的能力尚未得到充分探索。

Method: 本文提出了“英语学习者细粒度错误分析”问题，并推出了FEANEL基准测试。该基准包含1,000篇中小学生作文，并采用专家共同开发的基于词性的英语写作错误分类法。每处错误均由语言教育专家标注，并按类型、严重程度和解释性反馈进行分类。研究评估了最先进的LLMs在FEANEL基准上的表现。

Result: 实验结果揭示了当前LLMs在执行细粒度错误分析方面的能力存在显著差距。

Conclusion: LLMs在细粒度错误分析能力上的不足，突显了教育应用领域需要特定方法的进一步发展。
...
### [330] [Language-conditioned world model improves policy generalization by reading environmental descriptions](https://arxiv.org/abs/2511.22904)
*Anh Nguyen,Stefan Lee*

Main category: cs.CL

TL;DR: 本文提出LED-WM，一种基于DreamerV3的语言条件世界模型，通过注意力机制将语言描述与观测实体关联，从而在无需规划或专家演示的情况下，显著提高策略在未知游戏和动态描述上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了与人类有效交互，智能体需理解描述环境动态的语言，而非仅仅任务指令。现有方法虽然将语言融入世界模型以学习策略，但存在策略泛化能力不足或依赖于苛刻假设（如可容忍推理时规划延迟、有专家演示）的问题。

Method: 本文提出一种模型基强化学习方法。通过与环境交互训练一个语言条件世界模型（LED-WM），并从该模型中学习策略，无需规划或专家演示。LED-WM基于DreamerV3构建，其观测编码器采用注意力机制，显式地将语言描述与观测中的实体进行关联。

Result: 实验表明，与基线方法相比，使用LED-WM训练的策略在两个环境（MESSENGER和MESSENGER-WM）中的多种设置下，能更有效地泛化到由新颖动态和语言描述的未知游戏。此外，通过在世界模型生成的合成测试轨迹上进行微调，策略性能可以进一步提升。

Conclusion: LED-WM通过其语言感知编码器，有效解决了语言条件世界模型中策略泛化能力不足的问题，并在无需额外规划或专家数据的情况下，实现了对新颖环境动态和语言描述的有效泛化。世界模型还可用于生成合成数据以微调策略。
...
### [331] [Visual Puns from Idioms: An Iterative LLM-T2IM-MLLM Framework](https://arxiv.org/abs/2511.22943)
*Kelaiti Xiao,Liang Yang,Dongyu Zhang,Paerhati Tulajiang,Hongfei Lin*

Main category: cs.CL

TL;DR: 本文提出一个迭代框架，利用大型语言模型（LLM）、文生图模型（T2IM）和多模态大型语言模型（MLLM）自动生成和评估基于习语的视觉双关语。


<details>
  <summary>Details</summary>
Motivation: 研究基于习语的视觉双关语——将习语的字面和比喻意义结合的图像——并自动化其生成和评估过程。

Method: 该系统采用迭代框架：(i) LLM生成详细视觉提示，(ii) T2IM合成图像，(iii) MLLM从图像推断习语，(iv) 循环优化提示直到习语识别成功或达到步数限制。使用1,000个习语作为输入，生成了一个包含视觉双关语图像和配对提示的数据集。实验评估了10个LLM、10个MLLM和一个T2IM（Qwen-Image）。

Result: MLLM的选择是主要的性能驱动因素：GPT在准确性上表现最佳，Gemini紧随其后，而最佳开源MLLM（Gemma）与一些闭源模型具有竞争力。在LLM方面，Claude在提示生成方面表现出最强的平均性能。

Conclusion: 该迭代框架成功实现了基于习语的视觉双关语的生成和评估。MLLM的性能是关键，其中GPT表现最佳，而Claude在提示生成方面表现突出。
...
### [332] [Social Perceptions of English Spelling Variation on Twitter: A Comparative Analysis of Human and LLM Responses](https://arxiv.org/abs/2511.23041)
...
### [333] [Multi-chain Graph Refinement and Selection for Reliable Reasoning in Large Language Models](https://arxiv.org/abs/2511.23136)
...
### [334] [Training-Free Loosely Speculative Decoding: Accepting Semantically Correct Drafts Beyond Exact Match](https://arxiv.org/abs/2511.22972)
...
### [335] [Mind Reading or Misreading? LLMs on the Big Five Personality Test](https://arxiv.org/abs/2511.23101)
...
### [336] [ShoppingComp: Are LLMs Really Ready for Your Shopping Cart?](https://arxiv.org/abs/2511.22978)
...
### [337] [Standard Occupation Classifier -- A Natural Language Processing Approach](https://arxiv.org/abs/2511.23057)
*Sidharth Rony,Jack Patman*

Main category: cs.CL

TL;DR: 该项目利用自然语言处理技术，开发了一个集成模型，能将招聘广告准确分类到英国和美国职业分类系统的职业代码中，以提供劳动力市场信息。


<details>
  <summary>Details</summary>
Motivation: 将标准职业分类系统（SOC）与招聘广告中的大数据结合，可以深入研究特定职业的劳动力需求。然而，这需要一个能自动将招聘广告分配到相应职业代码的分类器。

Method: 研究开发了多种分类器，分别针对英国ONS SOC和美国O*NET SOC，并使用了不同的语言模型。最终，一个结合了Google BERT和神经网络分类器的集成模型被采用，该模型综合考虑了职位名称、描述和技能信息。

Result: 该集成模型在预测准确性方面表现最佳，对SOC的第四层（最细分类）的分类准确率高达61%，对第三层的准确率达到72%。

Conclusion: 该模型能够利用招聘广告提供关于劳动力市场演变的最新、准确的信息。
...
### [338] [Listwise Preference Optimization with Element-wise Confusions for Aspect Sentiment Quad Prediction](https://arxiv.org/abs/2511.23184)
*Wenna Lai,Haoran Xie,Guandong Xu,Qing Li,S. Joe Qin*

Main category: cs.CL

TL;DR: 方面情感四元组预测（ASQP）因元素间复杂关系和高阶元素预测困难而具有挑战性。本文提出了一种基于推理的生成方法，结合自然语言理由和列表偏好优化框架，以提高预测准确性和解释一致性。


<details>
  <summary>Details</summary>
Motivation: 方面情感四元组预测（ASQP）需要预测包含四个核心情感元素的结构化四元组，其中元素间关系复杂。现有的基于标记的方法难以建模这些复杂关系，并且在标准监督微调下，预测高阶元素（如类别和情感极性）时性能显著下降。

Method: 本文采用基于推理的生成方法，在一个统一模板中使用元素前缀输出四元组和自然语言理由，以鼓励显式关系推理和可解释性。为进一步增强元素对齐，引入了列表偏好优化框架，通过生成语法和语义接近的易混淆候选，并训练模型优先选择黄金候选而非竞争性替代方案，以提高结构有效性和关系一致性。

Result: 在四个基准数据集上进行的大量实验表明，所提出的框架有效提高了四元组预测的准确性和解释的一致性。

Conclusion: 本文提出的结合推理生成和列表偏好优化的框架，有效解决了ASQP中元素间复杂关系建模和高阶元素预测的挑战，显著提升了预测准确性、关系推理能力和解释一致性。
...
### [339] [Accent Placement Models for Rigvedic Sanskrit Text](https://arxiv.org/abs/2511.23088)
*Akhil Rajeev P,Annarao Kulkarni*

Main category: cs.CL

TL;DR: 本研究开发了一个自动恢复《梨俱吠陀》梵语诗歌中音高重音（udātta、anudātta、svarita）的系统，并比较了ByT5全微调、BiLSTM-CRF和LoRA三种策略的性能。


<details>
  <summary>Details</summary>
Motivation: 《梨俱吠陀》使用独特的音高重音系统，这些重音标记编码旋律和解释线索，但在现代电子文本中常常缺失。恢复这些重音对于理解和解释文本至关重要。

Method: 研究构建了一个带重音和不带重音的平行语料库，并比较了三种自动重音放置策略：(i) 对ByT5（一个字节级Transformer）进行全微调，(ii) 从零开始的BiLSTM-CRF序列标注基线，以及 (iii) 基于LoRA的ByT5参数高效微调。评估指标包括词错误率（WER）、字符错误率（CER）和专门的变音符号错误率（DER）。

Result: ByT5的全微调在所有指标上都达到了最低的错误率；LoRA在效率和准确性之间提供了良好的权衡；BiLSTM-CRF作为一个透明的基线。研究强调了重音恢复的实际要求，如Unicode安全预处理、标记感知分词以及区分字素和重音错误的评估。

Conclusion: 本研究为《梨俱吠陀》重音恢复建立了可复现的基线，并为重音感知OCR、ASR/吟唱合成以及数字学术等下游任务提供了指导。它将遗产语言技术定位为一个连接计算建模与语文学和教学目标的新兴NLP领域。
...
### [340] [Decoding the Past: Explainable Machine Learning Models for Dating Historical Texts](https://arxiv.org/abs/2511.23056)
*Paulo J. N. Pinto,Armando J. Pinho,Diogo Pratas*

Main category: cs.CL

TL;DR: 本文利用可解释的、特征工程化的树形机器学习模型，整合五类语言特征，对跨越五个世纪的英文文本进行时间分类。在大型语料库上，模型在世纪和十年尺度上取得了显著高于基线的准确率，并揭示了系统性的语言演变模式，特别是19世纪作为一个关键转折点。研究强调了树形模型在计算效率和可解释性方面的优势，作为神经网络架构的可扩展替代方案。


<details>
  <summary>Details</summary>
Motivation: 准确地确定历史文本的年代对于组织和解释文化遗产收藏至关重要。

Method: 研究采用可解释的、经过特征工程的树形机器学习模型进行时间文本分类。模型整合了五类特征：基于压缩的特征、词汇结构、可读性、新词检测和距离特征。通过特征重要性分析和SHAP可解释性方法来理解模型决策和语言演变模式。

Result: 组合模型表现优于任何单一特征集。在大型语料库上，世纪级别预测准确率达到76.7%，十年级别达到26.1%，显著高于随机基线（20%和2.3%）。放宽时间精度后，世纪级别（top-2）准确率增至96.0%，十年级别（top-10）增至85.8%。模型展现出强大的排序能力（AUCROC高达94.8%），平均绝对误差分别控制在27年和30年。距离特征和词汇结构被识别为信息量最大的特征，19世纪是语言演变的关键转折点。跨数据集评估显示领域适应性挑战，准确率下降26.4个百分点。

Conclusion: 研究表明不同的特征领域提供了互补的时间信号。树形模型因其计算效率和可解释性，为神经架构提供了一种可扩展、可解释的替代方案，尽管在跨数据集应用中存在领域适应性挑战。
...
### [341] [Dripper: Token-Efficient Main HTML Extraction with a Lightweight LM](https://arxiv.org/abs/2511.23119)
...
TL;DR: Dripper是一个高效的HTML主内容提取框架，它利用轻量级语言模型，通过HTML简化、任务重构、受控解码和新的评估数据集，解决了大型语言模型在网页内容提取中面临的上下文窗口、推理成本和幻觉问题，并取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 准确高效地从通用网页中提取主内容对于获取大型模型训练数据至关重要。尽管预训练的解码器专用生成语言模型具有出色的文档理解能力，但它们受限于上下文窗口长度、推理成本和格式幻觉等问题。

Method: 该研究提出了Dripper框架，包含四项关键创新：1) 设计了专门的HTML简化算法，将输入token数量减少78%并保留关键结构信息；2) 将主内容提取重新定义为语义块序列分类任务，显著降低推理成本；3) 引入了受控解码机制，通过logits处理器严格约束输出空间，有效消除小规模模型常见的幻觉问题；4) 提出了WebMainBench评估数据集，包含7,800多个经过人工精心标注主内容提取标签的网页。

Result: 实验结果表明，仅使用一个0.6B参数的模型，Dripper在所有评估基准上均达到了最先进的性能，并超越了所有基线方法。在WebMainBench数据集上，其ROUGE-N F1得分达到81.58%（使用回退策略时为83.13%）。

Conclusion: Dripper通过其创新的HTML简化、任务重构、受控解码机制和新的评估基准，成功利用轻量级语言模型高效准确地提取网页主内容，克服了现有方法的局限性，并实现了卓越的性能。
...
### [342] [TWEO: Transformers Without Extreme Outliers Enables FP8 Training And Quantization For Dummies](https://arxiv.org/abs/2511.23225)
*Guang Liang,Jie Shao,Ningyuan Tang,Xinyao Liu,Jianxin Wu*

Main category: cs.CL

TL;DR: 本文提出TWEO，一种非侵入式损失函数，通过解决权重矩阵共线性引起的极端激活离群值，使大型Transformer模型能够进行全模型FP8训练和W8A8量化，显著提高训练吞吐量并实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现代硬件对FP8的原生支持对于训练大型Transformer至关重要，但极端的激活离群值严重阻碍了其应用。现有解决方案过于复杂，要么依赖混合精度工程，要么需要侵入式架构修改。本文挑战了离群值是数据驱动的传统观念，旨在找到一种更根本、非侵入性的解决方案。

Method: 本文首先揭示极端离群值并非数据驱动，而是训练过程中由权重矩阵的特定结构特性（即共线性）产生的机械性产物。基于此洞察，提出了一种名为TWEO（Transformers Without Extreme Outliers）的新型、非侵入式损失函数。TWEO通过一个非常简单的损失项有效防止极端离群值的产生。

Result: TWEO能将离群值从10000+减少到20以下，从而在不进行工程技巧或架构更改的情况下，为LLM和ViT实现全模型FP8预训练。在标准FP8训练灾难性崩溃的情况下，TWEO实现了与BF16基线相当的性能，并提供了36%的训练吞吐量提升。此外，TWEO还实现了一种新的量化范式，使LLM的硬件友好型W8A8每张量静态量化首次达到SOTA性能，而此前因离群值问题被认为完全不可用。

Conclusion: TWEO通过从根本上解决极端激活离群值问题，使得大型Transformer模型能够稳健地进行全模型FP8训练，并首次实现了W8A8每张量静态量化达到SOTA性能，显著提升了训练效率和量化能力。
...
### [343] [Tourism Question Answer System in Indian Language using Domain-Adapted Foundation Models](https://arxiv.org/abs/2511.23235)
...
### [344] [Are LLMs Good Safety Agents or a Propaganda Engine?](https://arxiv.org/abs/2511.23174)
...
### [345] [Every Token Counts: Generalizing 16M Ultra-Long Context in Large Language Models](https://arxiv.org/abs/2511.23319)
...
### [346] [Behavior-Equivalent Token: Single-Token Replacement for Long Prompts in LLMs](https://arxiv.org/abs/2511.23271)
...
### [347] [Tackling a Challenging Corpus for Early Detection of Gambling Disorder: UNSL at MentalRiskES 2025](https://arxiv.org/abs/2511.23325)
*Horacio Thompson,Marcelo Errecalde*

Main category: cs.CL

TL;DR: 该研究旨在通过社交媒体活动早期检测赌博障碍风险，其提出的三种方法在MentalRiskES 2025挑战赛中表现出色，位居前两名，但区分高低风险用户仍存在挑战。


<details>
  <summary>Details</summary>
Motivation: 赌博障碍是一种复杂的行为成瘾，难以理解和解决，并带来严重的身体、心理和社会后果。在网络上进行早期风险检测（ERD）已成为科学界识别基于社交媒体活动的精神健康行为早期迹象的关键任务。

Method: 研究团队参加了MentalRiskES 2025挑战赛的任务1，目标是根据社交媒体活动对赌博相关障碍的高风险或低风险用户进行分类。他们提出了三种基于CPI+DMC方法，以预测有效性和决策速度为独立目标。组件使用SS3、扩展词汇的BERT和SBERT模型实现，并结合基于用户历史分析的决策策略。

Result: 尽管语料库具有挑战性，但研究团队提出的两个方案在官方结果中获得了前两名，在决策指标上表现出色。进一步分析表明，区分高风险和低风险用户存在一定困难。

Conclusion: 该研究提出的方法在赌博障碍风险早期检测方面取得了显著成效，但仍需探索策略以提高数据解释和质量，并推广更透明、可靠的精神障碍ERD系统。
...
### [348] [Optimizing Multimodal Language Models through Attention-based Interpretability](https://arxiv.org/abs/2511.23375)
*Alexander Sergeev,Evgeny Kotelnikov*

Main category: cs.CL

TL;DR: 本文提出一种基于注意力的可解释性方法，通过分析多模态语言模型中对图像关键物体关注的注意力头，来指导参数高效微调（PEFT），从而在图像理解任务中实现效率与性能的平衡。


<details>
  <summary>Details</summary>
Motivation: 现代多模态大语言模型（MLMs）的全量微调计算成本高昂，而参数高效微调（PEFT）方法虽能降低成本，但由于MLMs难以解释，难以确定哪些组件对训练最有效，以平衡效率和性能。

Method: 通过分析MLMs中注意力分数与图像token的关系，识别关注图像关键物体的注意力头。提出“头部影响（HI）”分数来量化注意力头对关键物体的关注度。利用HI分数选择最优模型组件进行PEFT，并为此创建了一个包含图像、关键物体掩码及其文本描述的新数据集。实验在2-3亿参数的MLMs上进行，应用于图像字幕任务。

Result: 微调具有最高HI分数的层（仅占总参数的约0.01%）相比预训练、随机选择或最低HI分数的层，在性能指标上产生了最显著的提升。这表明微调这些关键层能大幅影响图像理解能力。

Conclusion: 通过基于注意力的可解释性方法识别并微调MLMs中关注图像关键物体的注意力头，是一种有效且高效的策略，能够显著提升模型的图像理解能力，同时保持参数高效性。
...
### [349] [Scaling HuBERT for African Languages: From Base to Large and XL](https://arxiv.org/abs/2511.23370)
*Antoine Caubrière,Elodie Gauthier*

Main category: cs.CL

TL;DR: 该研究引入了SSA-HuBERT-Large和SSA-HuBERT-XL，这是首批完全基于非洲语音训练的大型自监督模型，并在撒哈拉以南语言的自动语音识别和语言识别任务上展示了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 非洲语言在语音处理研究和部署系统中代表性不足，尤其缺乏在低资源监督下表现良好且可开放权重的编码器。尽管自监督学习很有前景，但现有针对非洲语音的模型大多是基础规模，因此不清楚更大规模的、仅用非洲音频训练的编码器是否能带来显著优势，以及模型容量与数据构成如何相互作用。

Method: 本文通过自监督学习方法，开发并发布了SSA-HuBERT-Large（3.17亿参数）和SSA-HuBERT-XL（9.64亿参数），以及一个相应的基础规模模型。这些模型完全使用非洲语音数据进行训练。研究通过针对撒哈拉以南语言的自动语音识别（ASR）和语言识别（LID）任务，进行了严格控制的实验研究。

Result: 实验结果表明，更大的模型架构能够有效利用大型音频数据集，从而显著提升了性能。这一改进在撒哈拉以南语言的ASR和LID任务中得到了验证。

Conclusion: 更大的、专门针对非洲语音训练的自监督模型能够显著提升非洲语言处理的性能，证明了增加模型容量在结合足够数据时能带来显著优势。
...
### [350] [MCP vs RAG vs NLWeb vs HTML: A Comparison of the Effectiveness and Efficiency of Different Agent Interfaces to the Web (Technical Report)](https://arxiv.org/abs/2511.23281)
*Aaron Steiner,Ralph Peeters,Christian Bizer*

Main category: cs.CL

TL;DR: 该研究比较了大型语言模型（LLM）代理与网站交互的四种架构（HTML、RAG、MCP、NLWeb），发现在受控环境中，RAG、MCP和NLWeb在有效性和效率上均优于HTML，其中RAG与GPT-5结合表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前研究探索了LLM代理与网站交互的不同接口，但缺乏在一个单一受控环境中，使用相同任务对HTML浏览、RAG、MCP和NLWeb这四种架构进行比较的先例。

Method: 研究构建了一个包含四个模拟电子商务网站的测试平台，每个网站都提供HTML、MCP和NLWeb接口。为每种交互接口（HTML、RAG、MCP和NLWeb）开发了专门的代理，执行相同的一系列任务。评估使用了GPT 4.1、GPT 5、GPT 5 mini和Claude Sonnet 4作为底层LLM。

Result: RAG、MCP和NLWeb代理在有效性（F1分数从HTML的0.67提高到0.75-0.77）和效率（令牌使用量从HTML的约241k降至47k-140k，任务运行时长从291秒降至50-62秒）上均优于HTML代理。RAG与GPT-5的组合表现最佳，F1分数为0.87，完成率为0.79。RAG与GPT-5 mini在API使用费和性能之间提供了良好的折衷。

Conclusion: LLM驱动的Web代理的交互接口选择对其有效性和效率有实质性影响。
...
### [351] [Towards Improving Interpretability of Language Model Generation through a Structured Knowledge Discovery Approach](https://arxiv.org/abs/2511.23335)
*Shuqi Liu,Han Wu,Guanzhi Deng,Jianshu Chen,Xiaoyang Wang,Linqi Song*

Main category: cs.CL

TL;DR: 本文提出了一种任务无关的结构化知识猎手，通过利用知识的双层结构和分层指针网络，显著提升了知识增强文本生成的解释性和性能，超越了现有SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型在文本生成方面表现出色，但其缺乏解释性，尤其在需要可靠性和可解释性的知识增强文本生成任务中，这一问题尤为突出。现有方法多采用领域特定的知识检索器，通用性受限。

Method: 设计了一个任务无关的结构化知识猎手。该猎手直接利用结构化知识的高层实体和低层知识三元组的双层架构。具体方法包括：采用局部-全局交互方案进行结构化知识表示学习，以及一个基于分层Transformer的指针网络作为骨干，用于选择相关的知识三元组和实体。模型将语言模型的强大生成能力与知识猎手的高忠实度相结合。

Result: 模型实现了高解释性，使用户能够理解模型输出的生成过程。在内部知识增强的表格到文本生成（RotoWireFG数据集）和外部知识增强的对话响应生成（KdConv数据集）任务中，经验性地证明了模型的有效性。该任务无关模型优于现有最先进方法和相应的语言模型，在基准测试中设定了新标准。

Conclusion: 通过结合语言模型的生成能力和知识猎手的忠实度，所提出的任务无关模型成功克服了知识增强文本生成中解释性不足的挑战，并显著提升了性能和通用性。
...
### [352] [Ambiguity Awareness Optimization: Towards Semantic Disambiguation for Direct Preference Optimization](https://arxiv.org/abs/2511.23391)
*Jian Li,Shenglin Yin,Yujia Zhang,Alan Zhao,Xi Chen,Xiaohui Zhou,Pengfei Xu*

Main category: cs.CL

TL;DR: DPO训练中存在的模糊内容会引入歧义并降低性能，本文提出歧义感知优化（AAO）方法，通过语义相似度重加权来解决此问题，显著提升了DPO的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 直接偏好优化（DPO）作为一种流行的RLHF方法被广泛应用。研究发现，偏好对中常出现相同或语义相似的“模糊内容”，作者假设这可能在DPO训练中引入歧义，从而限制了对齐效果的进一步提升。

Method: 通过数学分析和概念验证实验揭示模糊内容可能导致性能下降。为解决此问题，本文引入了歧义感知优化（AAO）方法。AAO通过计算偏好对之间的语义相似度，自动重新加权模糊内容以减少歧义。

Result: AAO在多个模型规模和广泛采用的基准数据集（包括AlpacaEval 2、MT-Bench和Arena-Hard）上，性能始终显著优于现有最先进方法，且未显著增加响应长度。具体而言，AAO在AlpacaEval 2上比DPO高出最多8.9分，在Arena-Hard上高出最多15.0分。

Conclusion: DPO训练中模糊内容的存在可能引入歧义并降低性能。本文提出的AAO方法通过对模糊内容进行语义重加权，有效解决了这一问题，从而显著提升了DPO的对齐效果和性能。
...
### [353] [MegaChat: A Synthetic Persian Q&A Dataset for High-Quality Sales Chatbot Evaluation](https://arxiv.org/abs/2511.23397)
*Mahdi Rahmani,AmirHossein Saffari,Reyhane Rahmani*

Main category: cs.CL

TL;DR: 本文介绍了MegaChat，首个完全合成的波斯语问答数据集，用于评估Telegram电商中的智能销售聊天机器人。通过新颖的多智能体架构，MegaChat能高效生成高质量、大规模的对话数据，且无需人工标注，在低资源语言领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 伊朗中小企业日益利用Telegram进行销售，急需AI驱动的聊天机器人以实现实时互动和转化。然而，为波斯语等低资源语言开发此类机器人面临高质量问答数据集匮乏且生成成本高昂的挑战。

Method: 引入MegaChat，一个完全合成的波斯语问答数据集。提出了一种新颖的自动化多智能体架构，通过从活跃的Telegram购物频道收集数据，生成具有角色意识的问答对。该系统利用专门的智能体进行问题生成、验证和细化。为评估答案生成，将三种经典检索增强生成（RAG）模型与作者的先进智能体系统进行比较，后者包含多查询检索、重排序和角色对齐的响应合成。使用GPT-5.1在六个质量维度上进行评估。

Result: 在5个不同频道中的4个中，智能体架构的表现优于传统的RAG模型。这表明该方法能够生成可扩展、高质量的数据集，且无需昂贵的人工标注或复杂的微调。

Conclusion: MegaChat为中小企业在专业商业领域构建智能客户互动系统提供了一个高效、经济的解决方案，推动了低资源语言多语言对话式AI的发展。
...
### [354] [Bridging Planning and Execution: Multi-Agent Path Finding Under Real-World Deadlines](https://arxiv.org/abs/2511.21886)
*Jingtian Yan,Shuai Zhou,Stephen F. Smith,Jiaoyang Li*

Main category: cs.RO

TL;DR: 该研究提出了REMAP框架，通过集成ExecTimeNet来准确估计执行时间，从而弥合了多智能体路径规划（MAPF）与实际执行之间的差距，提高了对具有真实世界截止时间（MAPF-RD）问题的规划质量。


<details>
  <summary>Details</summary>
Motivation: 大多数MAPF公式假设简化的机器人模型，忽略了运动学约束、通信延迟和控制器变异性等执行时间因素。这种规划与执行之间的差距对时间敏感的应用来说是一个问题。

Method: 提出了REMAP，一个执行感知（execution-informed）的MAPF规划框架。该框架集成了ExecTimeNet来根据规划路径准确估计执行时间。REMAP可以与主流的基于搜索的MAPF规划器（如MAPF-LNS和CBS）结合使用，并以MAPF-RD问题为例进行了演示。

Result: 实验表明，在基准地图上，对于多达300个智能体，REMAP比基线方法（如恒定执行速度估计器）在解决方案质量上提高了高达20%。

Conclusion: REMAP框架通过整合准确的执行时间估计，成功地弥合了MAPF规划与实际执行之间的差距，为时间敏感的MAPF应用提供了更好的解决方案。
...
### [355] [OpenTwinMap: An Open-Source Digital Twin Generator for Urban Autonomous Driving](https://arxiv.org/abs/2511.21925)
*Alex Richardson,Jonathan Sprinkle*

Main category: cs.RO

TL;DR: OpenTwinMap是一个开源、基于Python的框架，用于生成高保真3D城市数字孪生，以支持自动驾驶车辆(AV)模拟，解决现有工具的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有城市数字孪生工具（如CARLA）通常与特定模拟器紧密耦合，难以扩展，或引入了显著的技术开销，限制了研究的灵活性和快速原型开发。

Method: OpenTwinMap框架摄取LiDAR扫描和OpenStreetMap (OSM)数据，生成语义分割的静态环境资产，包括道路网络、地形和城市结构。这些资产可以导出到Unreal Engine进行AV模拟。该框架强调可扩展性和并行化，并基于Python实现。

Result: 目前，OpenTwinMap已具备OSM和LiDAR数据预处理、基本道路网格和地形生成以及对CARLA的初步集成支持。它旨在降低研究人员适应和扩展管道的门槛。

Conclusion: OpenTwinMap通过提供一个灵活、可扩展且基于Python的解决方案，解决了现有AV数字孪生工具的局限性，使得研究人员能够更容易地生成高保真城市环境，从而推动自动驾驶研究。
...
### [356] [RSPECT: Robust and Scalable Planner for Energy-Aware Coordination of UAV-UGV Teams in Aerial Monitoring](https://arxiv.org/abs/2511.21957)
*Cahit Ikbal Er,Amin Kashiri,Yasin Yazicioglu*

Main category: cs.RO

TL;DR: 本文研究了能量受限无人机（UAV）和作为移动充电站的无人地面车辆（UGV）的鲁棒规划问题，旨在最短时间内完成长航时空中监测任务，同时应对不确定性（如未知障碍物、地形、风）。


<details>
  <summary>Details</summary>
Motivation: 无人机在执行长航时任务时面临能量限制，需要地面车辆提供充电支持。此外，任务环境存在不确定性（如障碍物、风），需要一种鲁棒的规划方法，确保任务在不进行重大修改的情况下顺利完成，并最小化任务时间。

Method: 研究者将问题正式描述为一个混合整数规划（MIP），该问题是NP-难的。由于精确求解方法计算上不可行，作者提出了一种可扩展且高效的启发式算法RSPECT。文中还提供了关于该算法复杂性、所得规划的可行性和鲁棒性的理论结果。

Result: 提出了一个NP-难的混合整数规划模型来描述无人机和无人地面车辆的鲁棒规划问题。开发了名为RSPECT的可扩展高效启发式算法。提供了关于RSPECT算法复杂性、以及其生成规划的可行性和鲁棒性的理论证明。通过仿真和实验验证了所提方法的性能。

Conclusion: RSPECT是一种可扩展且高效的启发式算法，能够为能量受限的无人机和无人地面车辆在存在不确定性的情况下，实现长航时空中监测任务的鲁棒规划，生成可行且鲁棒的计划，并已通过实验证明其性能。
...
### [357] [Constant-Volume Deformation Manufacturing for Material-Efficient Shaping](https://arxiv.org/abs/2511.22042)
*Lei Li,Jiale Gong,Ziyang Li,Hong Wang*

Main category: cs.RO

TL;DR: 本文提出了一种体积守恒的数字模具范式，结合实时体积一致性建模、几何变形预测和误差补偿策略，实现了对塑料材料高精度、零浪费的形状塑造。


<details>
  <summary>Details</summary>
Motivation: 传统的增材和减材制造方法在复杂几何形状的实现上存在局限性，如依赖离散堆叠或局部移除，限制了连续可控变形，并导致材料损耗和形状偏差。

Method: 该方法整合了实时体积一致性建模、基于几何信息的变形预测以及误差补偿策略。通过分析后成型点云的变形模式和误差趋势，校正弹性回弹和累积误差，从而保持体积一致性和表面连续性。

Result: 在五种代表性几何形状上的实验表明，该系统能够高保真地再现目标形状，并实现了超过98%的材料利用率。

Conclusion: 这种方法为用户定义设计的可持续、零浪费成型建立了一个数字化驱动、可复制的途径，弥合了数字建模、实时传感和自适应成型之间的鸿沟，推动了下一代可持续和可定制制造技术的发展。
...
### [358] [SwordRiding: A Unified Navigation Framework for Quadrotors in Unknown Complex Environments via Online Guiding Vector Fields](https://arxiv.org/abs/2511.22043)
...
### [359] [SoftNash: Entropy-Regularized Nash Games for Non-Fighting Virtual Fixtures](https://arxiv.org/abs/2511.22087)
...
### [360] [Design of an Adaptive Modular Anthropomorphic Dexterous Hand for Human-like Manipulation](https://arxiv.org/abs/2511.22100)
...
### [361] [3D Affordance Keypoint Detection for Robotic Manipulation](https://arxiv.org/abs/2511.22195)
...
### [362] [Bayesian Decentralized Decision-making for Multi-Robot Systems: Sample-efficient Estimation of Event Rates](https://arxiv.org/abs/2511.22225)
...
### [363] [MLATC: Fast Hierarchical Topological Mapping from 3D LiDAR Point Clouds Based on Adaptive Resonance Theory](https://arxiv.org/abs/2511.22238)
...
### [364] [Soft Fluidic Sheet Transistor for Soft Robotic System Enabling Fluid Logic Operations](https://arxiv.org/abs/2511.22318)
*Yuki Origane,Koya Cho,Hideyuki Tsukagoshi*

Main category: cs.RO

TL;DR: 本文提出一种软聚氨酯片状阀门（流体薄片晶体管，FST），它能仅用气动信号执行逻辑操作，从而在软机器人系统中实现高功能性和灵活性。该阀门可作为非门，并能通过集成实现正逻辑、与非、或非等复杂逻辑，甚至构建自锁电路，从而使软体机器人仅通过气压就能主动改变姿态。


<details>
  <summary>Details</summary>
Motivation: 在软机器人系统中实现高功能性和灵活性的同时，仅使用气动信号进行控制。

Method: 提出了一种带有放大器的软聚氨酯片状阀门（FST）。其工作原理是，当控制腔加压时，主通道沿中心轴线受压、屈曲并被堵塞，从而实现用较小压力信号进行控制。通过集成多个FST，可以在单个薄片上实现非门、正逻辑、与非门和或非门等多种逻辑操作。文中详细描述了FST的工作原理、制造方法和特性，并演示了如何配置逻辑操作，以及构建了一个自锁电路和一个结合触觉管和流体执行器的流体机器人系统原型。

Result: 所提出的FST作为非逻辑元件运行。通过集成多个FST，可以在单个薄片上执行正逻辑、与非门和或非门等逻辑操作。成功构建了自锁逻辑电路。原型流体机器人系统能够仅使用一根管道的气压，在触及障碍物时主动改变姿态。

Conclusion: 所提出的流体薄片晶体管（FST）及其集成方法能够仅使用气压信号在软机器人中产生主动行为和复杂的逻辑操作，验证了其在实现高功能性和灵活性方面的有效性。
...
### [365] [BINDER: Instantly Adaptive Mobile Manipulation with Open-Vocabulary Commands](https://arxiv.org/abs/2511.22364)
*Seongwon Cho,Daechul Ahn,Donghyun Shin,Hyeonbeom Choi,San Kim,Jonghyun Choi*

Main category: cs.RO

TL;DR: BINDER是一个双进程框架，通过解耦战略规划和连续环境监控，解决了开放词汇移动操作中机器人世界表示更新不及时的问题，显著提高了在动态环境中的成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在开放词汇移动操作中，仅在离散点（如导航目标、路径点或动作结束）更新世界表示，导致机器人在更新间隙“盲区”，容易忽略物体、延迟错误检测和重新规划，从而引发级联故障。

Method: 本文提出了BINDER（Bridging INstant and DEliberative Reasoning），一个双进程框架，将战略规划与连续环境监控解耦。它集成了一个审慎响应模块（DRM，用于任务规划的多模态LLM，进行结构化3D场景更新并指导IRM）和一个即时响应模块（IRM，用于连续监控的VideoLLM，分析视频流以更新记忆、纠正正在进行的动作并在必要时触发重新规划）。两个模块通过双向协调互补。

Result: 在三个真实世界动态物体放置环境中进行评估，BINDER比现有最先进的基线方法取得了显著更高的成功率和效率。

Conclusion: BINDER通过有效平衡意识维护和避免昂贵更新之间的权衡，实现了在动态条件下的鲁棒适应性，证明了其在实际部署中的有效性。
...
### [366] [Nonholonomic Narrow Dead-End Escape with Deep Reinforcement Learning](https://arxiv.org/abs/2511.22338)
*Denghan Xiong,Yanzhe Zhao,Yutong Chen,Zichun Wang*

Main category: cs.RO

TL;DR: 本研究提出了一种基于深度强化学习（DRL）的方法，用于解决阿克曼转向车辆在狭窄死胡同中的非完整性逃逸问题，通过训练策略来提高逃逸成功率并减少机动次数。


<details>
  <summary>Details</summary>
Motivation: 非完整性约束和阿克曼转向特性（如曲率限制和禁止原地旋转）使得传统规划器在处理车状机器人在狭窄死胡同中的路径规划时面临挑战。经典规划器通常将全局搜索与局部转向解耦，但在低测量区域（狭窄通道）和非完整性可达性受限的情况下，其采样效率低，且对间隙敏感。

Method: 本研究包含三个主要组成部分：1. 构建了一个生成器，用于采样符合阿克曼运动学特性的多阶段前后轨迹，并膨胀其包络线以合成一系列保证可逃逸的狭窄死胡同。2. 构建了一个强制执行运动学约束的训练环境，并使用软演员-评论家（Soft Actor-Critic, SAC）算法训练了一个策略。3. 将学习到的策略与结合了全局搜索和非完整性转向的经典规划器进行了评估比较。

Result: 在参数化死胡同家族中，学习到的策略解决了更大比例的实例，减少了机动次数，并在相同的感知和控制限制下保持了可比的路径长度和规划时间。

Conclusion: 研究表明，深度强化学习方法能有效解决阿克曼转向车辆在狭窄死胡同中的非完整性逃逸问题，相对于传统规划器，其在解决成功率和机动效率方面表现更优。
...
### [367] [LLM-Based Generalizable Hierarchical Task Planning and Execution for Heterogeneous Robot Teams with Event-Driven Replanning](https://arxiv.org/abs/2511.22354)
*Suraj Borate,Bhavish Rai B,Vipul Pardeshi,Madhu Vadali*

Main category: cs.RO

TL;DR: 本文介绍了CoMuRoS（协同多机器人系统），一种用于异构机器人团队的通用分层架构，它统一了集中式决策与分布式执行，并支持事件驱动的重新规划。


<details>
  <summary>Details</summary>
Motivation: 现有系统在异构机器人团队的通用架构、统一的决策与执行以及事件驱动的重新规划方面存在不足，尤其是在实现强大的多机器人和人机协作方面。

Method: CoMuRoS采用分层架构，其中任务管理器LLM负责解释自然语言目标、分类任务并分配子任务。每个机器人运行一个本地LLM，根据原始技能生成可执行Python代码。车载感知系统（VLM/图像处理）持续监控事件，任务失败或用户意图改变会触发重新规划，允许机器人协助队友、恢复任务或请求人工帮助。

Result: 硬件研究表明系统能自主从干扰事件中恢复、过滤无关干扰、实现紧密协调的运输以及出现人机协作（例如，多机器人协作对象恢复成功率9/10，协调运输8/8，人机辅助恢复5/5）。仿真研究展示了意图感知重新规划。文本基准测试（22个场景）显示，在任务分配、分类、IoU、可执行性和正确性方面取得了高平均分（正确性高达0.91），独立重新规划集（5个场景）正确性达到1.0。

Conclusion: CoMuRoS通过在物理机器人上实现运行时、事件驱动的重新规划，提供了强大的、灵活的多机器人和人机协作能力，超越了以往基于LLM的系统。
...
### [368] [Visual-Geometry Diffusion Policy: Robust Generalization via Complementarity-Aware Multimodal Fusion](https://arxiv.org/abs/2511.22445)
*Yikai Tang,Haoran Geng,Sheng Zang,Pieter Abbeel,Jitendra Malik*

Main category: cs.RO

TL;DR: 现有模仿学习方法在视觉运动技能获取中，在空间和视觉随机化下泛化能力差。本文提出VGDP，一个多模态模仿学习框架，通过互补感知融合模块（包含模态级dropout和轻量级交叉注意力）强制平衡使用RGB和点云线索，显著提高了泛化能力和对视觉/空间扰动的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在从演示中获取视觉运动技能方面至关重要，但现有方法在空间和视觉随机化下难以泛化，容易过拟合，导致策略泛化能力不足。

Method: 本文提出了视觉几何扩散策略（VGDP），一个多模态模仿学习框架。其核心是互补感知融合模块，该模块通过模态级dropout强制平衡使用RGB和点云线索，并利用交叉注意力作为轻量级的交互层，而非主要的鲁棒性来源。

Result: VGDP在18个模拟任务和4个真实世界任务的基准测试中，平均性能比七种基线策略提高了39.1%。更重要的是，VGDP在视觉和空间扰动下表现出强大的鲁棒性，在不同视觉条件下平均提高了41.5%，在不同空间设置下平均提高了15.2%。研究表明，融合潜在空间的表达能力主要来源于模态级dropout强制的互补性。

Conclusion: VGDP通过其互补感知融合模块，成功解决了模仿学习在视觉和空间随机化下的泛化和鲁棒性挑战。模态级dropout强制的互补性是其性能提升的关键，而交叉注意力则作为轻量级交互机制，共同实现了卓越的性能和强大的抗扰动能力。
...
### [369] [RealD$^2$iff: Bridging Real-World Gap in Robot Manipulation via Depth Diffusion](https://arxiv.org/abs/2511.22505)
*Xiujian Liang,Jiacheng Liu,Mingyang Sun,Qichen He,Cewu Lu,Jianhua Sun*

Main category: cs.RO

TL;DR: 本文提出RealD$^2$iff，一个从干净到噪声的扩散模型，用于合成真实世界的深度噪声，以弥合视觉sim2real差距，从而在不进行微调的情况下实现零样本sim2real机器人操作。


<details>
  <summary>Details</summary>
Motivation: 机器人操纵在现实世界中受到视觉sim2real差距的根本限制，即模拟中收集的深度观测无法反映真实传感器固有的复杂噪声模式。

Method: 本文提出一种“从干净到噪声”的范式，学习合成噪声深度，从而通过纯模拟驱动的机器人学习弥合视觉sim2real差距。在此基础上，引入RealD$^2$iff，一个分层粗到细的扩散框架，将深度噪声分解为全局结构失真和细粒度局部扰动。为实现这些组件的渐进学习，进一步开发了两种互补策略：用于全局结构建模的频率引导监督（FGS）和用于局部细化的差异引导优化（DGO）。将RealD$^2$iff无缝集成到模仿学习中，构建了一个六阶段的流程。

Result: RealD$^2$iff实现了两个关键应用：1) 生成类似真实世界的深度数据，以构建干净-噪声配对数据集，无需手动传感器数据收集。2) 实现零样本sim2real机器人操作，在不额外微调的情况下显著提高真实世界性能。

Conclusion: 所提出的“从干净到噪声”的范式，通过RealD$^2$iff框架，有效解决了视觉sim2real差距问题，显著提升了机器人操纵的真实世界性能，并简化了真实世界深度数据的生成过程。
...
### [370] [BUDD-e: an autonomous robotic guide for visually impaired users](https://arxiv.org/abs/2511.22541)
...
### [371] [Beyond Success: Refining Elegant Robot Manipulation from Mixed-Quality Data via Just-in-Time Intervention](https://arxiv.org/abs/2511.22555)
...
### [372] [Deadlock-Free Hybrid RL-MAPF Framework for Zero-Shot Multi-Robot Navigation](https://arxiv.org/abs/2511.22685)
...
### [373] [Mechanistic Finetuning of Vision-Language-Action Models via Few-Shot Demonstrations](https://arxiv.org/abs/2511.22697)
...
### [374] [A Two Degrees-of-Freedom Floor-Based Robot for Transfer and Rehabilitation Applications](https://arxiv.org/abs/2511.22705)
...
### [375] [Beyond Egocentric Limits: Multi-View Depth-Based Learning for Robust Quadrupedal Locomotion](https://arxiv.org/abs/2511.22744)
...
### [376] [CAPE: Context-Aware Diffusion Policy Via Proximal Mode Expansion for Collision Avoidance](https://arxiv.org/abs/2511.22773)
...
### [377] [Improving Robotic Manipulation Robustness via NICE Scene Surgery](https://arxiv.org/abs/2511.22777)
...
### [379] [Safe Autonomous Lane Changing: Planning with Dynamic Risk Fields and Time-Varying Convex Space Generation](https://arxiv.org/abs/2511.22829)
...
### [380] [MARVO: Marine-Adaptive Radiance-aware Visual Odometry](https://arxiv.org/abs/2511.22860)
*Sacchin Sundar,Atman Kikani,Aaliya Alam,Sumukh Shrote,A. Nayeemulla Khan,A. Shahina*

Main category: cs.RO

TL;DR: MARVO是一个物理感知、学习集成的水下里程计框架，通过融合水下图像形成建模、可微分匹配和强化学习优化，解决了水下视觉定位的挑战。


<details>
  <summary>Details</summary>
Motivation: 水下视觉定位面临波长依赖的衰减、纹理差和非高斯传感器噪声等挑战，这些因素使得在浑浊水域中获得几何一致的特征对应和准确的位姿估计变得困难。

Method: 该方法引入了MARVO框架，其前端扩展了基于Transformer的特征匹配器，并集成了物理感知辐射适配器（Physics Aware Radiance Adapter），用于补偿颜色通道衰减和对比度损失。这些半密集匹配与惯性测量单元（IMU）和压力测量相结合，在基于因子图的后端（使用GTSAM库）中形成一个关键帧视觉-惯性-气压估计器。每个关键帧引入预积分IMU运动因子、MARVO衍生的视觉位姿因子和气压深度先验，以实现实时全状态最大后验（MAP）估计。最后，引入了一个基于强化学习的位姿图优化器，通过学习SE(2)上的最优回撤动作，改进全局轨迹，超越传统最小二乘求解器的局部最小值。

Result: 该框架能够在浑浊条件下产生几何一致的特征对应；实现实时的全状态MAP估计；并且通过强化学习的位姿图优化器，能够将全局轨迹优化至超越经典最小二乘求解器局部最小值的效果。

Conclusion: MARVO通过集成物理学原理、学习方法和多传感器融合，为水下视觉定位提供了一个鲁棒的解决方案，有效地处理了水下环境特有的挑战，并提高了定位精度和轨迹的全局一致性。
...
### [381] [Threat-Aware UAV Dodging of Human-Thrown Projectiles with an RGB-D Camera](https://arxiv.org/abs/2511.22847)
*Yuying Zhang,Na Fan,Haowen Zheng,Junning Liang,Zongliang Pan,Qifeng Chen,Ximin Lyu*

Main category: cs.RO

TL;DR: 本文提出了一种基于RGB-D相机和人体姿态估计的无人机实时躲避系统，通过预测攻击者动作和抛射物轨迹，并结合不确定性感知躲避策略，使无人机能有效规避突然的抛射物攻击。


<details>
  <summary>Details</summary>
Motivation: 无人机在执行运输和航拍等任务时，容易受到人类蓄意抛射物攻击。躲避这种突然且快速的抛射物对无人机来说是一个巨大挑战，需要超低延迟响应和敏捷机动。

Method: 受棒球中分析投手身体动作以预测球轨迹的启发，本研究提出了一种实时躲避系统。该系统利用RGB-D相机，结合人体姿态估计和深度信息来预测攻击者的运动轨迹和随后的抛射物轨迹。此外，还引入了一种不确定性感知躲避策略，以使无人机能高效躲避来袭抛射物。

Result: 感知系统实现了高预测精度，在有效距离和延迟方面优于基线。躲避策略解决了时间和空间上的不确定性，确保了无人机的安全。广泛的真实世界实验证明了该框架在应对突然攻击时的可靠躲避能力和在不同场景下的出色鲁棒性。

Conclusion: 该框架为无人机提供了可靠且鲁棒的躲避能力，能够有效应对突然的抛射物攻击，确保无人机安全。
...
### [382] [SUPER-AD: Semantic Uncertainty-aware Planning for End-to-End Robust Autonomous Driving](https://arxiv.org/abs/2511.22865)
*Wonjeong Ryu,Seungjun Yu,Seokha Moon,Hojun Choi,Junsung Park,Jinkyu Kim,Hyunjung Shim*

Main category: cs.RO

TL;DR: 本文提出了一种仅基于摄像头的端到端自动驾驶框架，通过直接在BEV空间估计偶然不确定性并结合车道保持正则化，实现了更安全、更可靠且可解释的轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 当前的端到端（E2E）自动驾驶系统忽略了不确定性，假设感知输出完全可靠，导致在模糊或观测不良的场景中规划器缺乏明确的不确定性度量，从而限制了系统的鲁棒性。

Method: 研究者提出了一种仅基于摄像头的端到端框架。该方法直接在BEV空间估计偶然不确定性，并将其融入规划中，生成一个密集的、不确定性感知的可驾驶性地图。此外，引入了车道保持正则化来编码车道结构和交通规范，以促进安全和合规的行为，并在保持灵活性的同时稳定轨迹规划。

Result: 该方法在NAVSIM基准测试中取得了最先进的性能，在具有挑战性的NAVHARD和NAVSAFE子集上均实现了显著提升。结果表明，即使在挑战性的不确定性条件下，该方法也能实现鲁棒且可解释的轨迹规划。

Conclusion: 结合了原则性的偶然不确定性建模和驾驶先验知识，显著提升了仅基于摄像头的端到端自动驾驶的安全性和可靠性。
...
### [383] [Seeing before Observable: Potential Risk Reasoning in Autonomous Driving via Vision Language Models](https://arxiv.org/abs/2511.22928)
...
TL;DR: 该研究引入了PotentialRiskQA数据集和PR-Reasoner模型，旨在使自动驾驶车辆在风险变得可见之前，通过识别细微的前兆来推理潜在风险，从而提升主动安全性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在罕见和复杂场景中的安全性是一个关键挑战。现有系统难以识别尚未可见但可从异常行为或常识违规推断的潜在风险，因为当前数据集缺乏此类案例以及事件背后的因果推理链标注。

Method: 本文引入了PotentialRiskQA，一个新颖的视觉-语言数据集，用于在风险观察到之前进行潜在风险推理。每个样本都标注了结构化场景描述、语义前兆和推断的风险结果。基于此数据集，进一步提出了PR-Reasoner，一个基于视觉-语言模型的框架，专为车载潜在风险推理设计。

Result: 实验结果表明，在PotentialRiskQA数据集上进行微调后，PR-Reasoner在潜在风险推理任务上的性能比基线视觉-语言模型显著提高。

Conclusion: 该数据集和模型为开发具有改进预见性和主动安全能力的自动驾驶系统奠定了基础，有助于实现更智能、更有弹性的自动驾驶车辆。
...
### [384] [Commanding Humanoid by Free-form Language: A Large Language Action Model with Unified Motion Vocabulary](https://arxiv.org/abs/2511.22963)
...
### [385] [Analytical Inverse Kinematic Solution for "Moz1" NonSRS 7-DOF Robot arm with novel arm angle](https://arxiv.org/abs/2511.22996)
*Ke Chen*
...
### [386] [Adaptive Factor Graph-Based Tightly Coupled GNSS/IMU Fusion for Robust Positionin](https://arxiv.org/abs/2511.23017)
...
### [387] [DiskChunGS: Large-Scale 3D Gaussian SLAM Through Chunk-Based Memory Management](https://arxiv.org/abs/2511.23030)
...
### [388] [LatBot: Distilling Universal Latent Actions for Vision-Language-Action Models](https://arxiv.org/abs/2511.23034)
...
### [389] [Automated Generation of MDPs Using Logic Programming and LLMs for Robotic Applications](https://arxiv.org/abs/2511.23143)
...
### [390] [Obstruction reasoning for robotic grasping](https://arxiv.org/abs/2511.23186)
...
### [391] [Fault-Tolerant MARL for CAVs under Observation Perturbations for Highway On-Ramp Merging](https://arxiv.org/abs/2511.23193)
...
### [392] [SafeHumanoid: VLM-RAG-driven Control of Upper Body Impedance for Humanoid Robot](https://arxiv.org/abs/2511.23300)
...
TL;DR: SafeHumanoid是一个基于自我中心视觉的系统，它结合视觉语言模型（VLM）和检索增强生成（RAG），根据场景和人类接近程度，动态调整人形机器人的阻抗和速度参数，以实现更安全的人机交互。


<details>
  <summary>Details</summary>
Motivation: 为了实现安全可靠的人机交互（HRI），机器人不仅需要完成任务，还需要根据场景上下文和人类距离来调节阻抗和速度。

Method: 该系统采用自我中心视觉管道。自我中心帧通过结构化的VLM提示进行处理，然后嵌入并与精心策划的验证场景数据库进行匹配（RAG），并通过逆运动学映射到关节级的阻抗指令。研究人员在桌面操作任务（包括擦拭、物体移交和液体倾倒）中评估了该系统，这些任务涉及或不涉及人类存在。

Result: 结果表明，该管道能够以上下文感知的方式调整刚度、阻尼和速度曲线，在保持任务成功的同时提高了安全性。尽管当前的推理延迟（高达1.4秒）限制了在高度动态环境中的响应能力，但SafeHumanoid展示了阻抗控制的语义接地是实现更安全、符合标准的人形机器人协作的可行途径。

Conclusion: SafeHumanoid证明了通过视觉语言模型和检索增强生成对阻抗控制进行语义接地，是实现更安全、更符合标准的人形机器人协作的有效途径，尽管当前面临推理延迟的挑战。
...
### [393] [Field-programmable dynamics in a soft magnetic actuator enabling true random number generation and reservoir computing](https://arxiv.org/abs/2511.23215)
...
TL;DR: 本文展示了软机器人中复杂动力学的潜在优势，通过设计磁性软执行器，实现了真随机数生成、随机计算和时间序列预测等新功能，拓宽了软计算和人机交互的应用场景。


<details>
  <summary>Details</summary>
Motivation: 传统机电系统因磨损和可控性问题而避免复杂动力学。本文旨在证明复杂动力学在软机器人中可能具有独特优势，能提供传统驱动方法难以实现的新功能。

Method: 研究设计并实现了具有弹性的磁性软执行器，使其能在可调动态范围内运行数万次循环而无疲劳。通过实验，验证了这些执行器在真随机数生成、随机计算和作为物理储层进行Mackey-Glass时间序列预测方面的应用。此外，还展示了仿生眨眼和随机语音调制。

Result: 成功开发了能够在可调动态区域内无疲劳运行的弹性磁性软执行器。实验证明了其在真随机数生成和随机计算中的应用。验证了软机器人作为物理储层，能够有效进行Mackey-Glass时间序列预测。同时，展示了仿生眨眼和随机语音调制等功能。

Conclusion: 探索软机器人中的复杂动力学将极大地扩展其在软计算、人机交互和协作机器人领域的应用场景，并提供了超越传统运动的新功能。
...
### [394] [Incorporating Ephemeral Traffic Waves in A Data-Driven Framework for Microsimulation in CARLA](https://arxiv.org/abs/2511.23236)
*Alex Richardson,Azhar Hasan,Gabor Karsai,Jonathan Sprinkle*

Main category: cs.RO

TL;DR: 本文介绍了一种基于CARLA的数据驱动交通微观仿真框架，利用真实的I-24 MOTION测试床数据，重建了交通波的动态，实现了高保真的时空图仿真。


<details>
  <summary>Details</summary>
Motivation: 传统的微观模拟器在校准以重现大规模、短暂的交通波现象时面临巨大挑战。研究旨在提供一个能准确反映真实交通波行为的仿真平台，以评估交通控制策略、感知驱动的自动驾驶和波缓解方案。

Method: 该方法通过一个协同仿真模块，将I-24 MOTION测试床的高保真时空数据注入CARLA仿真环境。仿真以从经验数据中抽取的自我车辆为中心，自动生成其可见范围内的交通流，并通过“幽灵单元格”实现可见范围之外的边界控制。验证目标是实现完整的时空图保真度，而非局部跟车行为或抽象几何。

Result: 该框架成功模拟了低拥堵和高拥堵场景下的交通波形成和消散，其产生的涌现行为与真实交通高度吻合。结果提供了一个新颖的协同仿真框架，用于评估交通控制策略、感知驱动的自动驾驶以及未来的波缓解方案部署。

Conclusion: 这项工作将微观建模与物理实验数据相结合，首次在CARLA中实现了感知上真实、边界驱动的经验交通波现象仿真，为未来交通研究和技术开发提供了强大的工具。
...
### [395] [Design, modelling and experimental validation of bipenniform shape memory alloy-based linear actuator integrable with hydraulic stroke amplification mechanism](https://arxiv.org/abs/2511.23372)
...
TL;DR: 本研究开发了一种新型仿生形状记忆合金（SMA）线性执行器，其采用双羽状结构，相比传统执行器，在重量、成本和能耗方面有显著优势，并能产生高力。


<details>
  <summary>Details</summary>
Motivation: 传统的基于电磁的执行器存在效率低下、体积庞大、设计复杂（含齿轮机构）、生产和摊销成本高等局限性，促使人们寻求创新型替代执行器，特别是结合仿生设计原理和智能材料的系统。

Method: 研究结合了双羽状结构的优点（产生高力）和形状记忆合金（SMA）的高功率重量比，设计了一种新型仿生SMA线性执行器。开发并实验验证了多层双羽状配置SMA执行器的数学模型。同时，通过设计失效模式和影响分析（DFMEA）纳入了故障缓解策略，并对执行器性能进行了实验评估，与工业步进电机驱动执行器进行了基准测试。

Result: 该执行器在15V输入电压下产生了257 N的驱动力，满足操作要求。与传统系统相比，驱动机构重量减少约67%，组件数量减少80%，成本降低32%，能耗节省19%，且具有相似的尺寸以兼容现有阻尼器和百叶窗。该系统适用于高力-高行程应用。

Conclusion: 该研究引入的基于SMA线圈的仿生线性执行器是一种先进设计，可用于高力-高行程应用，并在建筑自动化控制、空间机器人轻量化执行系统和医疗假肢等领域具有广泛应用潜力，展现出卓越的性能提升和成本效益。
...
### [396] [From CAD to POMDP: Probabilistic Planning for Robotic Disassembly of End-of-Life Products](https://arxiv.org/abs/2511.23407)
*Jan Baumgärtner,Malte Hansjosten,David Hald,Adrian Hauptmannl,Alexander Puchta,Jürgen Fleischer*

Main category: cs.RO

TL;DR: 提出了一种基于POMDP的机器人拆卸规划框架，通过强化学习和贝叶斯滤波处理报废产品的不确定性，优于确定性基线。


<details>
  <summary>Details</summary>
Motivation: 现有机器人拆卸规划假设产品模型是确定且完全可观察的，但实际报废产品因磨损、腐蚀或未经记录的维修存在不确定性，阻碍了循环经济的发展。

Method: 将拆卸建模为部分可观测马尔可夫决策过程（POMDP），其中隐藏变量表示不确定的产品结构或物理特性。提出一个任务和运动规划框架，从CAD数据、机器人能力和检查结果中自动生成POMDP模型。利用强化学习近似求解，通过检查先验信息指导随机动作结果，并使用贝叶斯滤波器在执行过程中持续更新对潜在报废条件的信念。

Result: 在三个产品和两个机器人系统上验证，该概率规划框架在平均拆卸时间和方差方面优于确定性基线，能泛化到不同机器人设置，并成功适应与CAD模型的偏差（如零件缺失或卡住）。

Conclusion: 将拆卸建模为POMDP并结合强化学习和贝叶斯滤波的概率规划框架，能有效处理报废产品的不确定性，提升机器人拆卸的性能和适应性。
...
### [397] [LILAD: Learning In-context Lyapunov-stable Adaptive Dynamics Models](https://arxiv.org/abs/2511.21846)
*Amit Jena,Na Li,Le Xie*

Main category: eess.SY

TL;DR: LILAD是一个新颖的系统辨识框架，它通过上下文学习联合保证了动力学模型的适应性和稳定性，即使在分布外和任务外场景下也能提供严格的稳定性保证。


<details>
  <summary>Details</summary>
Motivation: 现有神经网络在系统辨识中虽预测准确，但常无法保持稳定性且假设动态系统是静止的，限制了其在分布偏移下的适用性。现有方法通常只解决稳定性或适应性中的一个问题，缺乏一个统一的框架来同时保证两者。

Method: LILAD通过上下文学习（ICL）同时学习动力学模型和李雅普诺夫函数，并明确考虑参数不确定性。它在多样化任务上进行训练，在测试时利用短轨迹提示快速适应新系统实例。为严格确保稳定性，LILAD还计算一个状态依赖的衰减器，以对新系统实例中的任何状态强制李雅普诺夫函数充分递减条件。

Result: LILAD在基准自主系统上进行了评估，结果表明其在预测准确性方面优于自适应、鲁棒和非自适应的基线方法。它能生成一个兼具稳定性感知和适应性的动力学模型，以及一个适应性李雅普诺夫证书。

Conclusion: LILAD提供了一个统一的系统辨识框架，能够联合保证系统的适应性和稳定性，并通过状态依赖的衰减器机制，将稳定性保证扩展到分布外和任务外场景。
...
### [398] [Automated Enumeration of Reconfigurable Architectures for Thermal Management Systems in Battery Electric Vehicles](https://arxiv.org/abs/2511.21855)
*Reihaneh Jahedan,Satya Peddada,Mark Jennings,Sunil Katragadda,James Allison,Nenad Miljkovic*

Main category: eess.SY

TL;DR: 本研究提出了一种系统方法，用于自动枚举、模拟和优化电动汽车（BEV）可重构热管理系统（TMS）的架构，以在性能、能耗和复杂性之间进行权衡。


<details>
  <summary>Details</summary>
Motivation: 电动汽车的热管理系统设计面临挑战，因为电池性能受温度影响大、缺乏燃烧废热以及TMS能耗对续航里程有显著影响。现有研究缺乏对可重构TMS架构的整体探索，且传统上依赖专家直觉限制了进展。

Method: 本研究采用图建模方法来自动枚举和模拟可重构TMS架构，并开发了一个框架进行瞬态性能分析和基于优化的权衡研究。MATLAB Simscape模型被自动创建和评估，并进行了多目标优化。

Result: 研究探索了超过150种操作模式序列，并保留了39种独特的架构进行评估。这些架构的MATLAB Simscape模型被自动创建和评估。多目标优化结果为用户根据优先级选择最佳架构提供了决策支持。

Conclusion: 本研究提供了一个系统化方法和框架，用于设计和优化电动汽车的可重构热管理系统架构，克服了传统设计方法的局限性，并为性能、能耗和复杂性之间的权衡提供了决策支持。
...
### [399] [Quantized Distributed Estimation with Event-triggered Communication and Packet Loss](https://arxiv.org/abs/2511.21906)
*Ying Wang,Yanlong Zhao,Ji-Feng Zhang,Karl Henrik Johansson*

Main category: eess.SY

TL;DR: 本文提出了一种针对事件触发、量化通信和丢包问题的分布式估计算法，通过引入一位信息重构方法处理丢包，实现了几乎必然收敛、收敛速率可控，并使全局平均通信比特率趋于零。


<details>
  <summary>Details</summary>
Motivation: 在量化分布式估计中，通过事件触发通信和处理丢包来减少传输比特数是一个挑战，特别是难以区分未触发事件和丢包。这是本研究的主要动机。

Method: 本文提出了一种事件触发的分布式估计算法，结合了量化通信和量化测量，并引入了一种一位信息重构方法来处理数据包丢失问题。

Result: 该算法被证明具有几乎必然收敛性，并建立了其收敛速率。结果表明，全局平均通信比特率随时间趋于零。此外，揭示了通信速率和收敛速率之间的权衡关系。

Conclusion: 所提出的算法成功解决了量化分布式估计中事件触发通信和丢包问题，实现了算法的收敛性和通信比特率的降低，并为设计所需的通信速率以达到预期收敛速率提供了指导。
...
### [400] [Communication-Aware Dissipative Control for Networks of Heterogeneous Nonlinear Agents](https://arxiv.org/abs/2511.21962)
...
### [401] [Learning Neural Network Safe Tracking Controllers from Backward Reachable Sets](https://arxiv.org/abs/2511.21953)
...
### [402] [Performance of the Kalman Filter and Smoother for Benchmark Studies](https://arxiv.org/abs/2511.22034)
...
### [403] [CBF Based Quadratic Program for Trajectory Tracking of Underatuated Marine Vessels](https://arxiv.org/abs/2511.22091)
*Ji-Hong Li*
...
### [404] [Smart Traffic Systems: A Comprehensive Review of Recent Advancements, Technologies, and Challenges](https://arxiv.org/abs/2511.22137)
...
### [405] [Output-Feedback Stabilizing Policy Iteration for Convergence Assurance of Unknown Discrete-Time Systems with Unmeasurable States](https://arxiv.org/abs/2511.22160)
...
### [406] [An Equality Set Projection Approach for TSO-DSO Coordination Dispatch](https://arxiv.org/abs/2511.22179)
...
### [407] [Distributed Koopman Operator Learning for Perception and Safe Navigation](https://arxiv.org/abs/2511.22368)
*Ali Azarbahram,Shenyu Liu,Gian Paolo Incremona*

Main category: eess.SY

TL;DR: 本文提出了一种统一且可扩展的框架，通过将模型预测控制（MPC）与分布式Koopman算子学习相结合，实现动态交通环境中的预测性安全自主导航。


<details>
  <summary>Details</summary>
Motivation: 在动态交通环境中，需要一个能够预测并确保安全的自主导航框架，该框架能够有效处理高维感知数据，并具有良好的可扩展性和通信效率，以适应智能交通系统（ITS）中的协同感知需求。

Method: 该方法整合了MPC和分布式Koopman算子学习。利用高维传感器数据对动态障碍物运动进行建模和预测。采用基于共识的分布式Koopman学习算法，使多个计算代理或传感单元无需集中数据聚合即可协同估计Koopman算子。学习到的算子预测障碍物的未来空间密度，通过高斯混合模型表示，其置信椭圆近似为凸多面体，并作为线性约束嵌入到MPC公式中，以确保安全无碰撞导航。

Result: 该方法不仅能确保避障，还能有效地随传感或计算节点的数量扩展，符合ITS应用中的协同感知原则。建立了理论收敛保证和预测约束公式。广泛的仿真结果表明，该方法在复杂环境中表现出可靠、安全且计算高效的导航性能。

Conclusion: 所提出的框架为动态交通环境中的预测性和安全自主导航提供了一个统一且可扩展的解决方案，通过分布式Koopman学习和MPC的结合，实现了高效的障碍物预测和避障，特别适用于大规模智能交通系统应用。
...
### [408] [Joint Scheduling of Workload Demand and Energy Supply in Low-carbon Data Centers with Decision-Dependent Uncertainty Set](https://arxiv.org/abs/2511.22244)
*Maoyuan Ma,Wangyi Guo,Lei Yang,Zhanbo Xu,Xiaohong Guan*

Main category: eess.SY

TL;DR: 本文提出了一种针对低碳互联网数据中心（IDC）中随机工作负载和氢能分布式能源系统的联合调度问题。通过建立基于决策依赖不确定性集合的工作负载分类模型，并结合滚动时域算法，有效降低了能源运营成本并增强了系统弹性。


<details>
  <summary>Details</summary>
Motivation: 在低碳IDC中，随机工作负载虽然具有时空可转移性，但其不可预测性会导致能源系统效率显著下降和运营成本高昂。更具挑战性的是，工作负载转移决策还会影响其随机性，这使得问题更加复杂。

Method: 本文提出了一种基于决策依赖不确定性集合的工作负载分类模型，明确识别了不同类型随机工作负载的时空弹性，并将决策依赖性描述为线性约束。在此基础上，建立了一个混合整数规划模型，用于工作负载需求和能源供应的优化调度。为增强系统在高度波动情景下的弹性，开发了一种滚动时域算法以确保非预见性和全场景可行性。

Result: 数值测试表明，与基准方法相比，在大多数不确定情景下，所提出的方法能够实现有效的工作负载调度决策，并显著降低能源运营成本。

Conclusion: 所提出的方法能够有效解决低碳IDC中随机工作负载和氢能分布式能源系统的联合调度问题，显著降低能源运营成本，并增强系统面对高波动情景的弹性。
...
### [409] [Learning the MPC objective function from human preferences](https://arxiv.org/abs/2511.22502)
*Pablo Krupa,Hasna El Hasnaouy,Mario Zanon,Alberto Bemporad*

Main category: eess.SY

TL;DR: 本文提出一种基于偏好学习的数据驱动方法，通过人类对轨迹对的偏好来构建模型预测控制（MPC）的目标函数。


<details>
  <summary>Details</summary>
Motivation: 在实际应用中，MPC目标函数的设计极具挑战性，因为它需要平衡复杂的权衡，并准确捕捉难以量化的性能标准。

Method: 将偏好学习问题表述为一个机器学习分类任务，以学习一个替代模型，该模型估计一条轨迹比另一条轨迹更受偏好的可能性。这个替代模型可以直接用作MPC的目标函数。

Result: 数值结果表明，学习到的目标函数能够生成与人类表达偏好一致的闭环轨迹。

Conclusion: 基于偏好学习是一种有效的数据驱动方法，可以为MPC构建符合人类偏好的目标函数。
...
### [410] [A model predictive control framework with customer-priority tiers for virtual power plant resilience during extreme weather: A UK heatwave case study](https://arxiv.org/abs/2511.22528)
*Edward Moroshko,Weizhe Qin,Desen Kirli,Mohammed Qais,Sotirios Tsaftaris,Aristides Kiprakis*

Main category: eess.SY

TL;DR: 本文提出了一种基于模型预测控制（MPC）的虚拟电厂（VPP）协调框架，用于在极端天气事件前后及期间管理光伏、电池和负荷，以提高电力系统韧性。


<details>
  <summary>Details</summary>
Motivation: 全球电力系统因极端天气事件（如热浪和风暴）的频率和强度变化，面临供需失衡加剧和供电损失风险增加的挑战，因此需要先进的控制策略来增强系统韧性。

Method: 开发了一个MPC框架来协调VPP，管理光伏系统、电池和负荷。通过解决一个多目标混合整数二次约束规划问题，该框架优先满足关键负荷，同时在网络和设备约束下最小化运行成本和光伏弃电。

Result: 在IEEE 33节点配电网络上使用真实的英国热浪数据进行仿真，结果表明，在考虑预测误差和建模不确定性的情况下，MPC相对于传统的全时域优化方法，将系统韧性提高了11-20%。

Conclusion: 这些结果表明，在极端天气期间，采用滚动时域协调（MPC）的方法对于实现弹性、低碳的VPP运行具有实际可行性。
...
### [411] [Optimal Singular Perturbation-based Model Reduction for Heterogeneous Power Systems](https://arxiv.org/abs/2511.22728)
*Yue Huang,Dixant B. Sapkota,Manish K. Singh*

Main category: eess.SY

TL;DR: 本文提出两种奇异摄动方法，用于在不预先了解系统状态物理含义的情况下，优化识别和约简现代复杂电力系统中的快速状态，通过贪婪优化和非线性优化实现模型降阶。


<details>
  <summary>Details</summary>
Motivation: 由于非常规发电和消费技术的出现，全球电力系统规模和复杂性空前增长。传统的基于奇异摄动的模型降阶方法，其技术假设正受到现代电力系统组件模型异构和“黑盒”性质的挑战，难以应对计算复杂性。

Method: 本文提出两种奇异摄动方法。首先，提出一种与时间尺度无关的奇异摄动公式。第一种方法采用贪婪优化策略，顺序选择要约简的状态。第二种方法依赖于非线性优化程序，在获得最优降阶模型的同时允许状态变换。

Result: 在包含同步电机、逆变器和线路动力学的测试系统上进行的数值研究表明，所开发的方法具有良好的通用性和准确性。

Conclusion: 所提出的两种奇异摄动方法能够有效地识别和约简复杂电力系统中的快速状态，尤其适用于缺乏状态物理先验知识的现代异构系统，提高了模型降阶的准确性和适用性。
...
### [412] [A Cyber-Physical Systems Framework for Tracking Post Thermal-Runaway Temperature and Smoke Dynamics in Underground Mines](https://arxiv.org/abs/2511.22721)
*Yukta Pareek,Khadija Omar Said,Satadru Dey,Ashish Ranjan Kumar*

Main category: eess.SY

TL;DR: 本文提出了一种网络物理系统（CPS）模型，用于监测地下矿井中锂离子电池（LIB）故障导致的温度和烟雾动态，以克服高保真模型的计算限制。


<details>
  <summary>Details</summary>
Motivation: 地下采矿作业正积极采用锂离子电池（LIB）为设备供电，因其高能量密度、长寿命、安全性好以及低噪音、低热量和低排放。然而，LIB故障在地下矿井中可能导致危险情况，其燃烧产物（包括有毒气体）可通过通风网络迅速蔓延。因此，持续监测地下温度和烟雾浓度至关重要。高保真模型计算成本高昂，不适用于大型、复杂和长时间的燃烧事件。

Method: 为了解决高保真模型相关的计算问题，研究人员开发了网络物理系统（CPS）模型来检查温度和烟雾动态。这些CPS模型在由高保真计算流体动力学（CFD）模型生成的数据集上进行训练，将矿井监控中心作为网络框架，与物理地下矿井协同运行。

Result: 经过高保真CFD模型数据训练的CPS模型，能够为地下矿井隧道中的温度和烟雾浓度演变提供出色的估计。

Conclusion: 该研究成果一旦实施，可以帮助矿井运营商在紧急情况下做出明智的决策，从而提高矿工的安全性。
...
### [413] [Switching control of underactuated multi-channel systems with input constraints for cooperative manipulation](https://arxiv.org/abs/2511.22810)
...
### [414] [Power System Robust State Estimation As a Layer: A Novel End-to-end Learning Approach](https://arxiv.org/abs/2511.22836)
...
### [415] [Switching-time bioprocess control with pulse-width-modulated optogenetics](https://arxiv.org/abs/2511.22893)
...
### [416] [Fast Distributed Algorithm for Aggregative Games in Malicious Environment](https://arxiv.org/abs/2511.22919)
...
### [417] [RDS-DeePC: Robust Data Selection for Data-Enabled Predictive Control via Sensitivity Score](https://arxiv.org/abs/2511.22952)
...
### [418] [Adaptive Trajectory Bundle Method for Roll-to-Roll Manufacturing Systems](https://arxiv.org/abs/2511.22954)
*Jiachen Li,Shihao Li*
...
### [419] [Open-source implementation of distribution network reconfiguration methods: Analysis and comparison](https://arxiv.org/abs/2511.22957)
...
### [420] [An LLM-Assisted Multi-Agent Control Framework for Roll-to-Roll Manufacturing Systems](https://arxiv.org/abs/2511.22975)
...
### [421] [The Battle of the Water Futures](https://arxiv.org/abs/2511.22986)
...
### [422] [Identification of contractive Lur'e-type systems via kernel-based Lipschitz design](https://arxiv.org/abs/2511.22993)
*Cesare Donati,Fabrizio Dabbene,Constantino Lagoa,Carlo Novara,Yoshio Ebihara*

Main category: eess.SY

TL;DR: 本文提出一个识别收缩Lur'e型系统的框架，通过结合线性先验知识、非线性反馈的核表示和利普希茨常数设计来强制收缩性，从而获得准确、可解释且符合系统收缩特性的模型。


<details>
  <summary>Details</summary>
Motivation: 解决收缩Lur'e型系统的识别问题，旨在获得预测准确、可解释且忠实于真实系统收缩性质的模型。

Method: 提出一个识别框架，该框架整合了线性先验知识与非线性反馈的核表示，并通过利普希茨常数设计系统地强制执行收缩性。

Result: 所提出的算法提供了预测准确、可解释且忠实于真实系统收缩性质的模型。数值实验表明，强制执行收缩性显著改善了参数估计，并产生了既准确又具有物理意义的模型。

Conclusion: 强制执行收缩性对于Lur'e型系统的参数估计至关重要，能有效提升模型准确性和物理意义。
...
### [423] [Closed-Loop Control Law for Low Thrust Orbit Transfer with Guaranteed Stability](https://arxiv.org/abs/2511.23014)
*Suraj Kumar,Aditya Rallapalli,Nivriti Priyadarshini,Bharat Kumar GVP,Ravi Kumar L*

Main category: eess.SY

TL;DR: 本文提出了一种基于Lyapunov的Q-law改进方法，解决了电动推进卫星变轨中经典Q-law的闭环稳定性问题，实现了实时在轨实施。


<details>
  <summary>Details</summary>
Motivation: 电动推进技术能最大化通信卫星的有效载荷，但变轨过程复杂。现有Q-law方法虽然常用，但存在闭环稳定性问题，限制了其在实时在轨系统中的应用。

Method: 针对经典Q-law的稳定性局限性，本文提出了一种由Lyapunov函数引导的Q-law修改方法，旨在确保闭环稳定性。

Result: 所提出的方法通过多种场景（包括共面转移、赤道到极地轨道转移以及地球同步转移轨道到地球同步轨道转移）的闭环轨道转移进行了验证，证明了其有效性。

Conclusion: 通过Lyapunov引导对经典Q-law进行修改，成功解决了其闭环稳定性问题，使得电动推进卫星的实时在轨变轨成为可能。
...
### [424] [Control Barrier Function for Unknown Systems: An Approximation-free Approach](https://arxiv.org/abs/2511.23022)
*Shubham Sawarkar,Pushpak Jagtap*

Main category: eess.SY

TL;DR: 本文提出一种针对具有未知动力学和移动障碍物的非线性系统的预设时间到达-避障(PT-RA)控制框架，无需在线模型学习或不确定性边界估计，通过虚拟系统和无近似反馈律实现安全性和可达性。


<details>
  <summary>Details</summary>
Motivation: 现有的鲁棒或基于学习的控制障碍函数(CBF)方法需要在线模型学习或不确定性边界估计，这在实际应用中可能存在挑战。研究旨在开发一种无需这些条件的PT-RA控制方法，以应对未知动力学和动态障碍物。

Method: 该方法包括两个主要步骤：1) 在一个简单的虚拟系统上求解一个基于CBF的二次规划(CBF-QP)，生成一个满足PT-RA条件的、针对时变收紧障碍物和目标集合的安全参考。2) 使用一种无近似的反馈律，将真实系统约束在这个参考周围的虚拟限制区(VCZ)内。

Result: 该框架在未知动力学和动态约束下，保证了实时安全性和预设时间目标可达性，且无需明确的模型识别或离线预计算。仿真结果验证了其可靠的动态避障能力和及时收敛到目标集合的性能。

Conclusion: 研究提出了一种新颖的PT-RA控制框架，成功解决了具有未知动力学和移动障碍物的非线性系统问题，实现了在无需模型学习或不确定性估计的情况下，保证实时安全性和预设时间可达性。
...
### [425] [Resistant Topology Inference in Consensus Networks: A Feedback-Based Design](https://arxiv.org/abs/2511.23023)
*Yushan Li,Jiabao He,Dimos V. Dimarogonas*

Main category: eess.SY

TL;DR: 本文提出了一种基于反馈的抗推理设计，旨在防止共识网络拓扑被外部观察者推断，同时保持原有的共识收敛性。


<details>
  <summary>Details</summary>
Motivation: 共识网络在许多应用中广泛部署，但节点间达成共识的过程可能会无意中通过推理技术向外部观察者泄露网络拓扑，这引发了隐私和安全问题。

Method: 首先，论文描述了保持原始共识的条件，并引入了“精确推理”的概念，该概念同时考虑了拓扑推理解决方案的唯一性（可解性）和与原始拓扑的偏差（准确性）。然后，利用不变子空间分析来表征可解性。即使在唯一推理仍然可能的情况下，论文也提供了反馈设计诱导不准确推理的必要和充分条件，并给出了一种基于拉普拉斯结构的分布式设计。

Result: 研究表征了保持原始共识的条件；引入并定义了“精确推理”的概念；利用不变子空间分析表征了拓扑推理的可解性；提供了使反馈设计能够诱导不准确推理的必要和充分条件；并提出了一种基于拉普拉斯结构的分布式设计。仿真验证了该方法的有效性。

Conclusion: 所提出的基于反馈的抗推理设计能够有效地防止共识网络拓扑被准确推断，同时确保共识收敛不受影响。
...
### [426] [Scalable Physics-Informed Neural Networks for Accelerating Electromagnetic Transient Stability Assessment](https://arxiv.org/abs/2511.23046)
...
TL;DR: 本文提出一个框架，通过用物理信息神经网络（PINNs）替换电磁暂态（EMT）仿真中的组件，从而加速EMT仿真。


<details>
  <summary>Details</summary>
Motivation: EMT仿真对于评估含有大量逆变器型资源（IBRs）的电力系统的暂态稳定性至关重要，但其仿真速度非常慢，是公认的计算瓶颈。

Method: 深入分析EMT仿真算法，识别出计算成本最高的组件，并用快速准确的PINN模型替换它们。提出了一种新颖的PINN公式，实现了模块化和可扩展地集成到仿真算法中。

Result: 使用一个4型风力发电机EMT模型，通过用PINN捕捉锁相环（PLL），实现了4-6倍的仿真加速。所有结果均通过PSCAD软件进行了验证。

Conclusion: PINN能够显著加速EMT仿真，通过替换计算密集型组件，提供了一种模块化且可扩展的集成方案，提高了电力系统暂态分析的效率。
...
### [427] [Targeted-Subharmonic-Eliminating Pulse Density Modulation for Wireless Power Transfer Systems](https://arxiv.org/abs/2511.23138)
*Songyan Li,Hongchang Li,Haiyue Jiang,Yudong Zhang,Wenjie Chen,Xu Yang*

Main category: eess.SY

TL;DR: 本文提出了一种针对串-串补偿无线电能传输系统（WPT）的脉冲密度调制（PDM）方法，通过设计噪声传递函数（NTF）来消除引起异常振荡的次谐波。


<details>
  <summary>Details</summary>
Motivation: PDM控制的WPT系统中，次谐波频率分量会激发电流异常振荡，导致系统性能下降。

Method: 提出了一种目标次谐波消除的PDM方法。该方法通过专门设计的噪声传递函数（NTF）来消除次谐波频率分量，实现简单且对耦合系数识别误差具有一定容忍度。

Result: 实验结果表明，所提出的方法在抑制电流异常振荡和减少电流幅值波动方面有效且鲁棒。

Conclusion: 该方法能有效抑制PDM控制WPT系统中的电流异常振荡，并减少电流幅值波动，同时具有实现简单和对参数偏差的容忍性。
...
### [428] [Development of a Load Profile Generator for Non-road Mobile Machinery](https://arxiv.org/abs/2511.23062)
*Serhiy Kapustyan,Pranav Tetey,Thomas Grube,Jochen Linssen*

Main category: eess.SY

TL;DR: 本研究提出了一种非道路移动机械的负载曲线生成器模型，能够模拟真实的运行工况，并已通过实际数据验证，有助于机器性能评估和能源需求计算。


<details>
  <summary>Details</summary>
Motivation: 为了准确描绘非道路移动机械在真实条件下的运行工况和功率需求，评估机器性能，并为替代燃料和电源概念的系统仿真提供支持。

Method: 采用一种技术自下而上的负载曲线生成器模型，允许用户参数化特定机器，以模拟其在执行器接口处的功率需求。该模型适用于建筑、农业、工业和林业等多种非道路移动机械类别，并通过将物料搬运机和森林集材机的常见操作与实际数据进行比对验证。

Result: 该负载曲线生成器模型能够准确描绘多种非道路移动机械的常见操作负载曲线，并已通过实际数据验证其结果。它能帮助工程师评估机器性能、开发操作策略，并作为能源需求计算的接口，为替代燃料和电源概念提供系统优化洞察。

Conclusion: 该模型提供了足够准确的非道路移动机械负载曲线理解，能够深入了解机器性能以及替代燃料和电源概念，从而有效优化系统仿真。
...
### [429] [Data-driven Reachability Verification with Probabilistic Guarantees under Koopman Spectral Uncertainty](https://arxiv.org/abs/2511.23322)
...
### [430] [Dynamic Power Allocation For NOMA-Based Transmission in 6G Optical Wireless Networks](https://arxiv.org/abs/2511.23326)
...
### [431] [A Lyapunov-Based Small-Gain Theorem for Fixed-Time Stability](https://arxiv.org/abs/2511.23474)
...
### [432] [Strong nonlinear detectability and moving horizon estimation for nonlinear systems with unknown inputs](https://arxiv.org/abs/2511.23385)
...
### [433] [Attention-Guided Fair AI Modeling for Skin Cancer Diagnosis](https://arxiv.org/abs/2511.21775)
*Mingcheng Zhu,Mingxuan Liu,Han Yuan,Yilin Ning,Zhiyao Luo,Tingting Zhu,Nan Liu*

Main category: eess.IV

TL;DR: 本研究开发了LesionAttn算法，通过将临床知识融入模型设计并结合帕累托优化，显著减轻了皮肤病学AI中的性别偏见，同时保持了高诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管人工智能在皮肤病学中展现出巨大潜力，但针对肤色偏见的研究已很广泛，而皮肤病学AI中的性别偏见却未得到充分探索，导致医疗服务不平等并加剧了现有的性别差异。

Method: 研究开发了LesionAttn算法，该算法通过将注意力引导至病变区域来整合临床知识，模拟临床医生的诊断焦点。结合帕累托前沿优化进行双目标（公平性和预测准确性）模型选择，以平衡公平性与准确性。

Result: LesionAttn在两个大规模皮肤病学数据集上进行了验证，结果表明它在保持高诊断性能的同时显著减轻了性别偏见，并且优于现有的偏见缓解算法。

Conclusion: 本研究强调了将临床知识嵌入AI开发中，以提升模型性能和公平性的潜力，并促进临床医生与AI开发者之间的跨学科合作。
...
### [434] [Digital Elevation Model Estimation from RGB Satellite Imagery using Generative Deep Learning](https://arxiv.org/abs/2511.21985)
*Alif Ilham Madani,Riska A. Kuswati,Alex M. Lechner,Muhamad Risqi U. Saputra*

Main category: eess.IV

TL;DR: 本研究提出了一种利用生成式深度学习（条件GAN）从免费RGB卫星图像生成数字高程模型（DEM）的方法，并在山地区域取得了良好效果，为传统DEM生成方法提供了一种经济有效的替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统DEM生成方法（如LiDAR和摄影测量）所需的数据在资源受限的环境中往往难以获取，因此需要一种经济且可行的替代方案。

Method: 本研究基于条件生成对抗网络（GAN），利用生成式深度学习从免费RGB卫星图像生成DEM。首先，构建了一个包含12K RGB-DEM对的全球数据集（Landsat RGB图像和NASA SRTM高程数据，均来自2000年）。实施了独特的预处理流程来选择高质量、无云区域并聚合归一化的RGB复合图像。模型采用两阶段训练过程：首先在完整数据集上训练，然后对通过SSIM值筛选出的高质量样本进行微调，以提高在复杂地形上的性能。

Result: 该方法在山地区域表现出良好的性能，总体平均均方根误差（RMSE）为0.4671，平均结构相似性指数（SSIM）为0.2065（范围-1到1）。但研究也指出该方法在低地和居民区存在局限性。

Conclusion: 本研究强调了在生成式DEM建模中精细预处理和迭代优化的重要性，为传统方法提供了一种经济有效且适应性强的替代方案，同时也突出了在全球不同地形之间泛化所面临的挑战。
...
### [435] [Comparing SAM 2 and SAM 3 for Zero-Shot Segmentation of 3D Medical Data](https://arxiv.org/abs/2511.21926)
*Satrajit Chakrabarty,Ravi Soni*

Main category: eess.IV

TL;DR: 本文首次对SAM 2和SAM 3在3D医学图像零样本分割中的性能进行了受控比较，发现SAM 3在大多数任务中表现更优，尤其适用于复杂解剖结构和稀疏交互。


<details>
  <summary>Details</summary>
Motivation: 基础模型（如SAM系列）在自然图像分割中表现出色，但在医学数据上的行为，特别是新发布的SAM 3，尚未得到充分表征。SAM 3引入了新的架构，可能改变其在空间提示下的行为，因此需要评估它是否能直接替代SAM 2。

Method: 研究在16个公共医学数据集（包括CT、MRI、3D/电影超声、内窥镜）上，针对54种解剖结构、病理和手术器械，对SAM 2和SAM 3进行了零样本分割的首次受控比较。采用纯视觉提示（禁用概念机制），提示仅限于第一帧，并使用单次点击、多次点击、边界框和密集掩码四种模式。标准化了预处理、提示放置、传播规则和度量计算。

Result: 在提示帧分析中，SAM 3在大多数结构上为点击提示提供了比SAM 2显著更强的初始化。在全卷分析中，SAM 3在复杂、血管和软组织解剖结构方面保持了这一优势，成为更通用的分割器。虽然SAM 2在强空间指导下对紧凑、刚性器官仍有竞争力，但它在SAM 3成功应对的挑战性目标上经常失败。

Conclusion: 研究结果表明，SAM 3是大多数医学分割任务（特别是涉及稀疏用户交互或复杂解剖拓扑的任务）的更优默认选择，无需定制即可作为SAM 2的即用型替代品。
...
### [436] [LAYER: A Quantitative Explainable AI Framework for Decoding Tissue-Layer Drivers of Myofascial Low Back Pain](https://arxiv.org/abs/2511.21767)
...
TL;DR: 本研究开发了一个名为LAYER的可解释人工智能框架，用于分析三维超声图像中的六个组织层，以预测肌筋膜疼痛。研究发现，非肌肉组织（特别是深筋膜膜）对疼痛预测有显著贡献，挑战了传统的以肌肉为中心的肌筋膜疼痛研究范式。


<details>
  <summary>Details</summary>
Motivation: 肌筋膜疼痛是慢性下背痛的主要原因，但其组织层面的驱动因素定义不清，且缺乏可靠的影像生物标志物。现有研究主要集中于肌肉，忽略了筋膜、脂肪和其他在生物力学中起重要作用的软组织。

Method: 开发了一个基于解剖学的可解释人工智能框架LAYER（Layer-wise Analysis for Yielding Explainable Relevance Tissue），该框架分析三维超声图像中的六个组织层，并量化它们对肌筋膜疼痛预测的贡献。研究使用了包含4000多次扫描的最大多模型三维超声队列。

Result: LAYER揭示非肌肉组织对疼痛预测有实质性贡献。在B模式成像中，深筋膜膜（DFM）显示出最高的显著性（0.420）。在B模式和剪切波图像结合时，非肌肉层的总显著性（0.316）几乎与肌肉的显著性（0.317）相当，这挑战了肌筋膜疼痛研究中传统的以肌肉为中心的范式。

Conclusion: 本研究建立了连接特定层解剖结构与疼痛生理学的定量、可解释框架，揭示了新的组织靶点，并为软组织成像的可解释分析提供了一种通用方法。研究结果可能影响肌筋膜疼痛的治疗方法，并挑战了以往以肌肉为中心的观点。
...
### [437] [When Do Domain-Specific Foundation Models Justify Their Cost? A Systematic Evaluation Across Retinal Imaging Tasks](https://arxiv.org/abs/2511.22001)
*David Isztl,Tahm Spitznagel,Gabor Mark Somfai,Rui Santos*

Main category: eess.IV

TL;DR: 研究发现，对于大多数视网膜疾病分类任务，紧凑型通用模型（如经ImageNet预训练的SwinV2-tiny）表现接近最优，而大型领域专用模型（如RETFound）仅在面对极端类别不平衡的精细判别任务时才显现其价值。预训练普遍带来性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前视网膜疾病分类广泛采用大型视觉基础模型，但缺乏系统性证据证明其参数需求和专业视网膜预训练的必要性。本研究旨在探究大型领域专用模型是否必需，以及专业视网膜预训练的计算成本是否合理。

Method: 研究在四项视网膜图像分类任务（包括OCT和CFP模态：8类OCT分类、3类DME、5类DR和3类GL检测）上，基准测试了多种初始化策略。评估了12-13种模型配置，包括不同参数量的Vision Transformers (22.8M-86.6M)、Swin Transformers (27.6M-28.3M)、ConvNeXt (28.6M) 和领域专用RETFound模型 (303M)，所有模型均在相同训练条件下进行。

Result: 1. 预训练普遍带来性能提升（5.18-18.41%），且增益随任务难度增加而扩大。2. 紧凑型架构（27-29M参数）在帕累托前沿占据主导地位，其中SwinV2-tiny在三个数据集上实现了最佳性能。3. RETFound (303M) 仅在具有挑战性的DR分级任务中（准确率71.15%）证明了其计算成本的合理性。4. ImageNet预训练对于其他任务（DME准确率99.24%，OCT准确率97.96%）已足够。5. CFP任务的预训练准确率增益（9.13-18.41%）高于OCT任务（5.18%）。

Conclusion: 研究证据表明，对于大多数视网膜分类任务，紧凑型通用模型能够提供接近最优的性能；而专门的基础模型仅在极端类别不平衡下的精细判别任务中才具有必要性。
...
### [438] [GACELLE: GPU-accelerated tools for model parameter estimation and image reconstruction](https://arxiv.org/abs/2511.22094)
*Kwok-Shing Chan,Hansol Lee,Yixin Ma,Berkin Bilgic,Susie Y. Huang,Hong-Hsi Lee,José P. Marques*

Main category: eess.IV

TL;DR: GACELLE是一个开源、GPU加速的定量MRI（qMRI）分析框架，显著提高了参数估计和不确定性量化的速度和鲁棒性，从而促进了qMRI在临床研究中的应用。


<details>
  <summary>Details</summary>
Motivation: 定量MRI（qMRI）的参数估计计算量大，严重阻碍了其在临床研究中的应用、方法学创新和临床转化，尤其是在高空间分辨率图像或需要拟合多个参数的场景下。

Method: GACELLE是一个基于MATLAB的开源、GPU加速框架，采用随机梯度下降优化器和随机采样器。它支持空间正则化以提高估计的鲁棒性，并提供不确定性量化。GACELLE后端负责计算并行化、自动参数更新和内存批处理，用户只需提供前向信号模型。其随机求解器在CPU和GPU上执行完全向量化的马尔可夫链蒙特卡罗，确保跨硬件的可重现性。

Result: 基准测试显示，与基于CPU的估计相比，GACELLE的随机梯度下降求解器加速高达451倍，随机采样加速高达14,380倍，且不影响准确性。GACELLE在三种代表性qMRI模型和图像重建任务中表现出多功能性，改善了参数精度，提高了重测可靠性，并减少了定量图中的噪声。

Conclusion: GACELLE通过结合速度、可用性和灵活性，为医学图像分析提供了一个通用优化框架。它降低了qMRI的计算障碍，为可重现的生物标志物开发、大规模成像研究和临床转化铺平了道路。
...
### [439] [ColonAdapter: Geometry Estimation Through Foundation Model Adaptation for Colonoscopy](https://arxiv.org/abs/2511.22250)
*Zhiyi Jiang,Yifu Wang,Xuelian Cheng,Zongyuan Ge*

Main category: eess.IV

TL;DR: ColonAdapter是一个自监督微调框架，用于将几何基础模型适配到结肠镜检查中的3D几何估计，解决了现有模型在临床场景中的性能下降问题，并取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 由于非朗伯表面、移动光源、大面积无纹理区域、镜面反射和同质纹理等挑战，现有的3D几何基础模型在结肠镜检查场景中表现不佳，导致几何估计不准确。

Method: 本文提出了ColonAdapter，一个自监督微调框架。它利用预训练的几何先验，并引入了细节恢复模块（DRM）以改善低纹理区域的性能和确保尺度一致性。同时，结合了几何一致性损失和置信度加权光度损失，以增强训练稳定性。

Result: 在合成和真实数据集上的实验表明，ColonAdapter在相机姿态估计、单目深度预测和密集3D点图重建方面均达到了最先进的性能，且无需地面真值内参。

Conclusion: ColonAdapter成功地将几何基础模型应用于结肠镜检查，显著提高了在挑战性临床环境中3D几何估计的准确性，解决了现有模型的局限性。
...
### [440] [Content Adaptive Encoding For Interactive Game Streaming](https://arxiv.org/abs/2511.22327)
...
### [441] [Hard Spatial Gating for Precision-Driven Brain Metastasis Segmentation: Addressing the Over-Segmentation Paradox in Deep Attention Networks](https://arxiv.org/abs/2511.22606)
...
### [442] [TokCom-UEP: Semantic Importance-Matched Unequal Error Protection for Resilient Image Transmission](https://arxiv.org/abs/2511.22859)
...
### [443] [Two-Dimensional Tomographic Reconstruction From Projections With Unknown Angles and Unknown Spatial Shifts](https://arxiv.org/abs/2511.22890)
...
### [444] [MICCAI STS 2024 Challenge: Semi-Supervised Instance-Level Tooth Segmentation in Panoramic X-ray and CBCT Images](https://arxiv.org/abs/2511.22911)
...
### [445] [Deep Learning for Restoring MPI System Matrices Using Simulated Training Data](https://arxiv.org/abs/2511.23251)
