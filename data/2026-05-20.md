...
TL;DR: 本文主张开发系统方法以生成“数据探针”序列，这些序列可揭示LLM在不同工作阶段的行为特征。


<details>
  <summary>Details</summary>
Motivation: 当前数据过滤和数据集构建依赖大量实验与经验启发式，缺乏理解数据特性如何驱动LLM行为的原则性方法。

Method: 通过定义随机过程生成合成序列（数据探针），利用典型集等理论概念分析序列统计特性，以观察LLM行为。

Result: 数据探针可支持系统性研究数据特性对模型性能、泛化性和鲁棒性的影响。

Conclusion: 数据探针方法为超越经验启发式、揭示数据在LLM训练与推理中的基础性作用提供了途径。
...
TL;DR: 本文提出了一种面向生产环境的文档理解微服务架构，集成了分类、OCR和LLM字段提取模型，支持每小时处理数千页多页文档。


<details>
  <summary>Details</summary>
Motivation: 学术研究侧重新模型开发，忽略了模型在生产规模下的部署经验，导致文献中存在模型定义与生产运行之间的鸿沟。

Method: 采用微服务架构，设计包括混合分类、GPU推理与CPU编排分离、异步处理IO操作以及独立水平扩展策略。通过批量分析识别性能瓶颈。

Result: 发现OCR（而非LLM解析）主导端到端延迟；系统并发受共享GPU推理容量限制，而非工作节点数量。
...
TL;DR: 本研究评估了大语言模型在患者管理的个人健康记录背景下回答用户健康查询的潜力、安全性和准确性，并提出了新的评估框架。


<details>
  <summary>Details</summary>
Motivation: 患者管理的个人健康记录承诺增强患者对自身健康的理解，但记录中的信息复杂，可能阻碍患者的洞察力。

Method: 从3种不同分布中抽取2,257个用户查询，匹配PHR数据，使用Gemini 3.0 Flash生成回答，并通过SHARP框架和新开发的PHR评估框架进行评价。

Result: 在加入PHR数据后，所有类型问题的回答帮助性显著提升（p < 0.001），同时观察到安全性、准确性、相关性和个性化方面的潜在改进。

Conclusion: PHR数据有潜力帮助满足多种用户需求，并提供了监测基于PHR语境下LLM回答缺陷的框架，激励进一步研究以实现用户通过理解健康记录而获益的潜力。
...
TL;DR: 提出了一种名为LBW-Guard的边界自主训练控制层，在AdamW之上运行，通过观察训练遥测、识别不稳定敏感区域并应用边界控制来优化训练，显著提升了Qwen2.5-7B模型在WikiText-103上的性能，困惑度从13.21降至10.74（改进18.7%），训练时间从392.54秒缩短至357.02秒（加速1.10倍），并在学习率压力测试中保持可训练性。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型训练日益受到不稳定性、性能退化和计算浪费的困扰，特别是在激进的学习率、规模扩展和运行时压力条件下。

Method: LBW-Guard作为一个自治训练控制层，在AdamW优化器之上运行，通过观察训练遥测解释不稳定敏感区域，并对优化器执行应用边界控制，同时保留固定的训练目标。

Result: 在7B参考设置中，LBW-Guard将最终困惑度从13.21降至10.74（18.7%改进），端到端时间从392.54秒减至357.02秒（1.10倍加速）。在更强的学习率压力下，AdamW困惑度退化至1885.24（LR=3e-3）和659.76（LR=1e-3），而LBW-Guard保持可训练，困惑度分别为11.57和10.33。梯度裁剪基线未能复现此效果。

Conclusion: 结果表明，对稳定性敏感的LLM训练可以从优化器之上的治理层面受益，LBW-Guard证明边界运行时控制能在压力下保持高效计算，且不同于优化器替换和局部梯度抑制。
...
TL;DR: 提出了一种多智能体NL2SQL方法，在BIRD基准上达到78.1%语义准确率，通过语义增强的schema表示和业务规则生成准确SQL。


<details>
  <summary>Details</summary>
Motivation: 现有NL2SQL系统准确率未达到人类专家水平，需要改进算法。

Method: 设计了多智能体编排器，利用LLM规划、反思和自纠正；开发了上下文感知的schema富化方法。

Result: BIRD基准上语义准确率78.1%，跨领域泛化性好。

Conclusion: 该方法有效提升NL2SQL准确率，具有通用性。
...
TL;DR: 本文提出一种混合KAN-MLP架构用于人体活动识别（HAR），通过策略性结合KAN的精度与MLP的噪声鲁棒性和效率，在八个公开数据集上平均宏F1分数提升5.33%，优于纯KAN或纯MLP模型。


<details>
  <summary>Details</summary>
Motivation: KAN在干净低维数据上表现优异，但受噪声影响大且计算低效；MLP则更鲁棒高效。现有HAR模型完全替换为KAN会降低性能和效率，因此需要融合两者优势。

Method: 系统探索KAN模块在深度HAR网络中的不同位置，提出混合架构：使用KAN输入嵌入层、MLP中间特征混合层、以及LarctanKAN模块进行最终分类。

Result: 在八个公开HAR数据集上，混合模型相比纯MLP平均宏F1分数提升5.33%，并显著优于独立KAN和MLP基线；将该策略集成到其他SOTA HAR架构中也能持续提升性能。

Conclusion: 精心设计的KAN、MLP及其他常规神经组件组合能够构建更鲁棒、准确的HAR模型，适用于真实世界的可穿戴传感环境。
...
TL;DR: 本文提出A2A网络（代理间网络）的信任问题，强调从初始架构阶段设计信任，而非事后修补。


<details>
  <summary>Details</summary>
Motivation: 现有代理对齐技术无法解决A2A网络中出现的系统性漏洞，如对抗性组合、语义错位和级联操作失败。

Method: 通过概念框架提出四个设计支柱，从初始阶段构建A2A协调系统的可信赖性。

Result: 未涉及具体实验结果，但论证了信任必须在A2A框架的设计初纳入。

Conclusion: A2A网络的可信赖性不能通过事后加固现有协议实现，而应从一开始架构在协调框架中。
...
TL;DR: 提出了一个干扰感知的机器反学习框架，用于多任务场景，通过任务感知梯度投影和实例级梯度正交化减少干扰。


<details>
  <summary>Details</summary>
Motivation: 现有机器反学习方法主要集中在单任务设置，但现代模型通常采用多任务共享骨干架构，移除一个任务的监督会无意中影响其他任务。

Method: 干扰感知框架，结合任务感知梯度投影（限制更新在任务特定子空间）和实例级梯度正交化（减少遗忘和保留信号之间的冲突）。

Result: 在五个任务的多任务计算机视觉基准上，全任务反学习UIS降低30.3%，部分任务反学习UIS降低52.9%，同时保持强泛化。

Conclusion: 该框架能有效解决多任务反学习中的任务级和实例级干扰问题，实现高效反学习并保持模型性能。
...
TL;DR: 提出了ReElicit框架，利用贝叶斯优化和LLM的特征空间诱导演化，在仅用30次评估的预算下，实现了对系统提示词的最优聚合性能优化。


<details>
  <summary>Details</summary>
Motivation: 系统提示词依赖聚合指标而非逐例反馈进行调优，现有的优化方法难以在样本受限的黑盒设置下高效工作。

Method: 基于LLM诱导演化生成紧凑可解释的特征空间，结合高斯过程代理模型和采集函数选择目标特征，并通过LLM将其转化为系统提示词，同时随新评估动态更新特征空间。

Result: 在10个系统提示词优化任务中，使用30次总评估预算，ReElicit在所有仅依赖聚合反馈的基线方法中取得了最强的聚合性能。

Conclusion: LLM不仅可作为提示词生成器，还能作为适应性语义表示构建器，增强贝叶斯优化在自然语言制品上的效果。
...
TL;DR: 提出了一个用于长期代理工作流中新兴委托的基准测试平台 DecisionBench，包含固定任务套件、模型池、委托接口和评估指标，并通过参考扫描揭示了委派效能的关键发现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏对长期代理工作流中委托行为的系统评估，无法捕捉协调信号和优化潜力。

Method: 构建了一个包含多模型、多任务、多指标的基准测试平台，通过五条件参考扫描（23,375个任务实例）进行分析，包括质量、成本、延迟、委派率、路由保真度等指标，并引入反事实天花板分析。

Result: 四个意识条件下的终端质量无显著差异；路由保真度范围从7.5%到29.5%；反事实天花板显示完美委派可提升15-31个百分点。

Conclusion: 质量导向评估会忽略协调信号，委派性能受传递渠道影响超过内容本身，存在大量未开发的协调优化空间。
...
TL;DR: 介绍了一个名为POLAR-Bench的基准测试，用于评估LLM代理在隐私政策下与对抗性第三方系统交互时遵循用户意图的能力。


<details>
  <summary>Details</summary>
Motivation: LLM代理越来越多地访问用户私人数据并代表用户行动，但现有模型在对抗性环境下可能泄露隐私信息，需要系统评估其隐私保护能力。

Method: 构建POLAR-Bench，包含10个领域和7852个样本，通过隐私政策维度和攻击策略两个正交轴生成5x5诊断表面，并使用确定性集合成员关系评估隐私和效用。

Result: 前沿模型能保护99%以上的受保护属性，而1-30B参数的开源模型表现较差，最弱的模型泄露超过一半的隐私信息。

Conclusion: POLAR-Bench能定位模型在遵循意图方面的弱点，为改进隐私对齐提供关键依据。
...
TL;DR: 提出了一种在接口受限的半马尔可夫决策过程（IC-SMDP）框架下，通过异步分散Q学习（IC-Q）实现多智能体协作的方法，并给出了首个具有有限样本保证的神经Q学习算法。


<details>
  <summary>Details</summary>
Motivation: 现有工作缺乏对多智能体LLM流程中分散部分可观测性（如跨组织边界）的形式化建模和理论保证。

Method: 形式化IC-SMDP框架，设计IC-Q算法（每次交接仅传递一个标量），将近似信息状态（AIS）扩展至多智能体SMDP，并控制随机时长下的马尔可夫噪声。

Result: 在合成IC-SMDP、多LLM数学推理、多智能体路由和CPU编程实验中，IC-Q性能与集中式基线相当，且误差按理论界的三个分量独立缩放。

Conclusion: 证明了在分散部分可观测条件下，神经Q学习可实现有限样本保证，为跨边界多智能体系统提供了首个理论基础。
...
TL;DR: 将信任校准形式化为偏好学习问题，通过高斯过程后验和不确定性定向查询实现工具使用的自主与人工审批决策。


<details>
  <summary>Details</summary>
Motivation: 解决智能体工具使用中自动执行与人工审批之间的信任校准问题。

Method: 采用偏好贝叶斯优化框架，利用高斯过程分类和概率似然模型，基于不确定性最大化原则选择需要人工审批的动作。

Result: 在理论层面证明该方法与偏好贝叶斯优化同构，继承了其推理机制和样本效率优势。

Conclusion: 该方法能够有效区分为允许、阻止和询问三类动作区域，实现信任的动态校准。
...
TL;DR: 提出了ResearchArena框架，对三种现成AI代理生成的117篇论文进行系统评估。虽然纯文本审稿评分看似乐观，但结合代码审查和人工评审后发现，实验严谨性存在严重问题，所有论文均未达到顶级会议接收标准。


<details>
  <summary>Details</summary>
Motivation: 尽管已有自动研究系统能生成完整论文，但可行性不等于质量，缺乏对代理生成论文质量的系统性研究。

Method: 构建ResearchArena minimal scaffold，让三种现成代理（Claude Code、Codex、Kimi Code）执行完整研究流程，在13个计算机科学种子任务上各进行3次实验，生成了117篇论文。采用三阶段评估：纯文本审稿（SAR）、结合工作区的人工制品感知同行评审（PR）、人工元评审。

Result: 纯文本审稿下Claude Code表现最佳，甚至超越某些基准，但人工检查发现SAR分数与真实接受决定对齐差。结合代码审查后得分显著下降，实验严谨性是主要瓶颈，三种代理表现出截然不同的失败模式，且所有论文均达不到顶级会议接收标准。

Conclusion: 目前代理生成的论文距离真正自动研究仍有差距，单纯改进写作能力不够，需要提升实验执行和结果验证的可靠性。
...
TL;DR: 本文回顾了语义Web服务的经验，为知识图谱提出了一个四维形式化框架，并从中导出了智能体可供性剖面（AAP），这是一个高于VoID和DCAT的语义层，能在智能体规划时支持原则性的KG选择、组合和故障诊断。


<details>
  <summary>Details</summary>
Motivation: 当前KG元数据标准（如VoID、DCAT）仅描述KG的内容，但未说明特定智能体能从中证明什么、空结果背后的闭合假设，或智能体任务词汇是否与模式对齐；此外，部署KG中的模式描述逻辑与推理机制可能存在差异，而现有元数据无法捕捉这种认知故障模式。

Method: 借鉴语义Web服务领域的OWL-S和WSMO方法，为KG场景设计了一个四维形式化框架，并基于此导出了智能体可供性剖面（AAP）作为VoID和DCAT之上的语义层。

Result: 提出了AAP概念，并制定了五点研究议程，涵盖实现基于AAP的可供性匹配所需的形式化、计算和工程工作。

Conclusion: AAP为KG的智能体交互提供了更精确的语义描述，有助于解决当前元数据在认知负载和推理差异上的不足，但其大规模实现仍需进一步形式化、计算和工程研究。
...
### [16] [Not all uncertainty is alike: volatility, stochasticity, and exploration](https://arxiv.org/abs/2605.19215)
*Payam Piray*

Main category: cs.AI

TL;DR: 本文揭示了在动态环境中，环境波动性增强探索，随机性抑制探索，并通过Gittins指数和CAUSE方法验证了这一不对称性。


<details>
  <summary>Details</summary>
Motivation: 此前研究将不同环境不确定性源视为等价，但本文发现波动性和随机性对最优探索的影响方向相反。

Method: 扩展Gittins指数框架至高斯状态空间赌博机，并推导出CAUSE探索奖励的闭式解。

Result: CAUSE在异质噪声环境中优于标准策略，且波动性增强探索而随机性抑制探索。

Conclusion: 探索与学习受同一噪声推断不对称性支配，病理噪声推断导致探索反转，对精神疾病计算模型有启示。
...
### [17] [Hallucination as Exploit: Evidence-Carrying Multimodal Agents](https://arxiv.org/abs/2605.19192)
*Guijia Zhang,Hao Zheng,Harry Yang*

Main category: cs.AI

TL;DR: 本文提出证据携带式多模态代理（ECA），通过将工具调用分解为关键谓词并由确定性门控基于证书授权，解决多模态代理中视觉幻觉导致的安全问题。在攻击测试中，门控绕过率从15%降至1.3%，端到端管道不安全动作率为0%。


<details>
  <summary>Details</summary>
Motivation: 现有多模态代理依赖模型文本判断，视觉幻觉可能导致未经授权的工具调用，造成安全风险。需要一种机制确保动作由外部证据而非模型内部信念授权。

Method: ECA将每个工具调用分解为动作关键谓词，从受限DOM/OCR/AX验证器获取类型化证书，由确定性门控仅授权证书支持的权限。模型文本视为不可采纳证据。

Result: 经过1900次攻击的红队测试，四步硬化将门控绕过率从15%降至1.3%；200任务端到端管道不安全动作率为0%（Wilson 95%上限2.67%）；HACR审计中，朴素代理不安全率100%，提示防御49.6%，ECA为0%。

Conclusion: 模型语言可提议动作，但外部证据必须授权这些动作。该原则有效防止幻觉到动作的转换导致的误授权。
...
TL;DR: 提出SimGym框架，使用视觉语言模型代理在实时浏览器中模拟A/B测试，将实验周期从数周缩短至一小时以内，在电子商务UI主题变化测试中与实际用户行为趋势一致率达77%。


<details>
  <summary>Details</summary>
Motivation: 传统A/B测试需要分流流量、耗时数周且可能影响用户体验，亟需快速、安全的替代方案。

Method: 包含三个组件：基于点击流数据的流量驱动角色生成流水线、结合多模态感知与记忆的实时浏览器代理架构，以及对比模拟结果与实际行为变化的评估协议。

Result: 在电商平台UI主题变化的A/B测试中，代理与实际加购行为变化的方向一致率达77%，实验时间从数周缩短至一小时。

Conclusion: SimGym可快速模拟A/B测试结果，无需真实用户参与，大幅降低实验风险与时间成本。
...
### [19] [Causal Evidence for Attention Head Imbalance in Modality Conflict Hallucination](https://arxiv.org/abs/2605.19250)
*Jinrui Jiang,Zhangtai Wu,Zhen Wu,Xinyu Dai*

Main category: cs.AI

TL;DR: 本文通过因果分析发现多模态大模型中存在分布广泛的驱动幻觉注意力头与集中的抵抗幻觉注意力头，这种不对称结构导致模型优先采用文本错误前提。提出条件干预方法MACI，仅在检测到冲突时抑制驱动头，有效减少幻觉且保持准确率。


<details>
  <summary>Details</summary>
Motivation: 现有研究未解释为何视觉证据在多模态大模型生成中无法战胜文本错误前提，需要从内部机制理解这种模态冲突幻觉的成因。

Method: 对5个开源模型进行注意力头级别的因果路径修补分析，识别两类对立因果作用的注意力头；基于此设计了条件性因果干预方法MACI。

Result: 在MMMC基准上，MACI在所有比较的推理时基线中取得最大幻觉减少，并在SCI-SemanticConflict测试上实现零样本迁移，同时保持有利的幻觉-准确率权衡。

Conclusion: 多模态大模型内部存在分布驱动的幻觉头与集中抵抗头的不对称路由结构，针对性干预可有效缓解模态冲突幻觉。
...
### [20] [Can Large Language Models Revolutionize Survey Research? Experiments with Disaster Preparedness Responses](https://arxiv.org/abs/2605.19229)
*Yan Wang,Ziyi Guo,Christopher McCarty*

Main category: cs.AI

TL;DR: 本文提出了一个基于大语言模型的五阶段调查框架，利用保护动机理论约束的知识图谱和锚定边际理论驱动LLM，在飓风准备调查中优于传统插补方法，并通过检索约束对话系统实现了可控的幻觉管理。


<details>
  <summary>Details</summary>
Motivation: 调查面临响应率下降、样本偏差、数据缺失和AI欺诈等结构性挑战，现有LLM方案缺乏对完整调查流程的严格评估，尤其忽略灾害背景下数据质量的关键需求。

Method: 构建五阶段LLM集成框架（问卷设计、样本选择、预测试、缺失数据插补、后收集分析），开发基于保护动机理论的共现知识图谱，设计7种LLM配置（零样本、检索增强、理论驱动变体），并提出锚定边际理论驱动LLM（A-TLM）和检索约束知识图谱聊天机器人。

Result: A-TLM在灾害相关分块非随机缺失条件下优于IPW/MI、MICE+PMM和missForest（RMSE 1.439 vs 1.496），近零符号偏差（-0.121 vs -0.631）；基于PMT因果结构的检索比非结构化检索性能更好（MAE 0.993 vs 1.097）；近零聚合偏差可能掩盖子群对立误差，需采用子群分层偏差审计。

Conclusion: 理论约束的LLM方法在调查数据质量提升中有效，幻觉可通过检索约束和基于知识图谱的拒绝机制进行架构性管理，子群偏差审计应成为报告标准。
...
TL;DR: 提出AquaUI，一种无需训练即可在推理时减少GUI截图视觉令牌的方法。


<details>
  <summary>Details</summary>
Motivation: 现有LMM作为GUI智能体时，高分辨率截图存在非均匀空间信息密度，且已有方法要么需额外训练，要么忽略结构布局和空间冗余。

Method: 构建自适应四叉树，每叶节点保留一个代表性合并令牌，并通过条件四叉树增强多步骤交互的时间一致性。

Result: 在GUI-Owl-1.5-32B-Instruct上，实现13.22%加速、29.52%令牌减少，保留99.06%全令牌性能。

Conclusion: AquaUI利用截图空间冗余，无需重新训练即可提升准确率与效率的权衡。
...
### [22] [MOCHA: Multi-Objective Chebyshev Annealing for Agent Skill Optimization](https://arxiv.org/abs/2605.19330)
...
TL;DR: MOCHA是一种多目标技能优化方法，通过切比雪夫标量化和指数退火策略，在满足平台约束的同时提升LLM代理的任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化器忽略技能优化的多目标性，无法处理非凸目标区域的帕累托最优变体。

Method: 切比雪夫标量化覆盖帕累托前沿（包括非凸区域），结合指数退火从探索过渡到利用。

Result: 在6个任务中，MOCHA在所有任务上实现改进，平均正确率相对提升7.5%，最高达14.9%，并发现两倍多的帕累托最优技能变体。

Conclusion: MOCHA有效处理了多目标技能优化中的复杂权衡，优于现有单目标优化方法。
...
### [23] [Swimming with Whales: Analysis of Power Imbalances in Stake-Weighted Governance](https://arxiv.org/abs/2605.19264)
*Yuzhe Zhang,Manvir Schneider,Qin Wang,Davide Grossi*

Main category: cs.AI

TL;DR: 论文通过计算社会学视角，研究PoS区块链中按权益权重投票导致的权力扭曲，使用Penrose-Banzhaf指数量化权力，结合理论分析和真实链上治理系统数据，揭示权力与权益占有之间的偏差。


<details>
  <summary>Details</summary>
Motivation: PoS区块链中的权益权重投票范式容易导致少数大权益持有者控制决策，产生权力扭曲，需系统分析这种不平衡的程度。

Method: 理论部分证明权力与权益完美对齐不可达但可在特定条件下近似；实证部分使用Project Catalyst链上治理数据进行分析。

Result: 理论表明完美对齐不可达但可近似；实证显示当前权益加权治理系统中存在显著的权力不平衡。

Conclusion: 权益加权投票无法实现权力与权益的完美对齐，实际系统中存在权力扭曲，需优化治理机制。
...
TL;DR: 本文通过协议编码快照（截至2026-03-09）对77项LLM交易代理研究进行证据映射，发现仅有2/19项主要研究具有可提取的时间一致分割协议，且无研究达到R3可重复性，指出可比较评估协议和可重复性工具有待发展。


<details>
  <summary>Details</summary>
Motivation: LLM交易代理研究快速增长，但缺乏系统性审计和可比较的评估协议，需要通过证据图谱揭示当前瓶颈。

Method: 采用协议编码的文献快照方法，筛选77项研究，对其中19项主要研究进行标准化协议提取和可重复性评分（R0-R3），并通过架构-能力-适应性分析框架进行讨论。

Result: 仅2/19研究报告时间一致分割协议，1/19报告交易成本模型，1/19说明生存偏差处理，11/19记录执行时间语义，15/19评为R0级，无研究达到R3可重复性。

Conclusion: 架构实验快速扩展，但可比评估协议、执行语义和可重复工具有待发展，证据清单、可重复性审计和报告核查表是核心贡献。
...
TL;DR: 提出了生成式递归推理模型（GRAM），将递归潜在推理转化为概率多轨迹计算，支持多种假设和推理时缩放。


<details>
  <summary>Details</summary>
Motivation: 现有递归推理模型为确定性单轨迹，缺乏多假设和随机性，限制了推理能力。

Method: 将推理建模为随机潜在轨迹，使用共享转移函数，通过变分推断训练生成式模型。

Result: 在结构化推理和多解约束满足任务上优于确定性基线，且具备无条件生成能力。

Conclusion: GRAM为神经推理系统提供了更灵活的推理和生成框架。
...
TL;DR: 提出了PRISM基准，包含10,372对人类校准的指令-代码对，用于评估语言模型在程序化视频生成中的空间正确性；发现了执行成功与空间正确性之间约41%的差距。


<details>
  <summary>Details</summary>
Motivation: 现有像素级扩散模型缺乏几何精度和时间一致性，而程序化视频生成虽具优势，但缺乏严格评估语言模型能否生成空间正确动画输出的方法。

Method: 构建PRISM基准（10,372对指令-代码，20倍于以往基准），引入漏斗式评估框架，包括代码级可靠性、空间推理、动态视觉复杂度和时间密度四个互补指标。

Result: 评估7个主流LLM，发现执行成功率到空间通过率的平均下降约41%，表明可执行代码不保证空间连贯输出。

Conclusion: 程序化视频生成评估不应仅关注可执行性，PRISM为推进空间连贯代码生成提供了原则性基准。
...
TL;DR: 提出SIGMA框架，通过符号关系图捕捉智能体间的信任、冲突与中立关系，提升多智能体系统推理的准确性和抗冲突能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的多智能体系统存在错误传播和缺乏结构感知的问题，无法处理冲突信号和识别可靠交互模式。

Method: SIGMA框架：1) 选择相关且多样的智能体；2) 构建带置信权重的符号交互图；3) 通过冲突感知符号消息传递强化可信信号、抑制冲突信号；4) 结构感知加权聚合生成全局一致预测。

Result: 在6个基准数据集、多种LLM后端和配置下，SIGMA持续超越最新基线，在准确性和抗冲突性能上取得显著提升。

Conclusion: SIGMA通过显式建模智能体间的信任与冲突关系，有效提升了多智能体推理的稳健性和一致性。
...
TL;DR: 提出SERL框架，利用环境反馈优化强化学习中的信用分配，在ALFWorld和WebShop上分别达到90.0%和80.1%的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用多轮交互中的环境反馈，导致长期信用分配困难。

Method: SERL框架：任务奖励决定更新方向，环境反馈调整动作放置和幅度，聚焦关键动作。

Result: 在ALFWorld和WebShop上分别取得90.0%和80.1%的成功率，优于现有RL和蒸馏基线。

Conclusion: 具有动作相关性且置于关键位置的环境反馈比无差别使用更长或更丰富的上下文更有效。
...
TL;DR: 提出了GUIDE框架，结合生成式模型与安全回退机制，通过决策变压器、Q值模块和逆动态模块实现自动竞价中的探索与安全平衡，在淘宝大规模部署中取得显著收益。


<details>
  <summary>Details</summary>
Motivation: 现有自动竞价方法在探索效率与安全性上存在不足，缺乏显式的安全回退机制，导致财务风险高。

Method: 采用决策变压器联合建模历史动作与环境状态，Q值模块通过正则化约束引导探索，逆动态模块基于预测状态推断安全动作，并自适应选择最终动作。

Result: 在公共数据集、模拟环境和淘宝线上部署中均超越基线，线上GMV提升4.10%、广告点击增加1.40%、成本提高1.66%、ROI增长3.52%。

Conclusion: GUIDE框架有效统一了竞价中的探索效率与安全性，具备强工业实用性。
...
TL;DR: 提出分布匹配策略优化（DMPO）方法，通过前向KL散度最小化避免策略坍塌，在多种推理任务上提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有on-policy强化学习方法（如GRPO）存在模式坍塌问题，一旦发现高奖励轨迹便停止探索其他策略，限制了多样性。

Method: 通过近似前向KL散度最小化，构建组级目标分布（按奖励加权采样轨迹），并调整策略分布对齐该目标，实现模式覆盖而不依赖全局采样。

Result: 在文本和视觉NP-Bench基准上分别提升9%和12%质量比；数学推理任务提升2.0%，域外任务提升2.3%。

Conclusion: 分布匹配是一种有原则且实用的方法，能有效防止on-policy RL中的模式坍塌，通过持续探索提升通用推理能力。
...
TL;DR: 本文发现大型推理模型（LRM）的注意力模式与越狱攻击成功率相关，并据此提出一种基于强化学习和多样化说服策略的越狱方法，在多个模型和基准测试中显著提升了攻击效果、效率和迁移性。


<details>
  <summary>Details</summary>
Motivation: LRM在展现强大推理能力的同时，其内部推理过程的暴露带来了额外的安全风险，例如比标准LLM更容易受到越狱攻击。

Method: 提出一种利用强化学习增强越狱攻击的方法，将注意力信号纳入奖励函数设计，并引入多样化说服策略以丰富动作空间。

Result: 在五个开源和闭源LRM的三个基准测试中，该方法在有效性、效率和迁移性上均显著优于现有方法，实现了更高的攻击成功率。

Conclusion: 注意力模式与越狱攻击成功密切相关，基于强化学习和说服策略的方法能有效提升对LRM的越狱攻击效果。
...
### [32] [BLINKG: A Benchmark for LLM-Integrated Knowledge Graph Generation](https://arxiv.org/abs/2605.19518)
*Carla Castedo,Enrique Iglesias,Manuel Lama,Alberto Bugarin-Diz,Maria-Esther Vidal,David Chaves-Fraga*

Main category: cs.AI

TL;DR: 提出BLINKG基准测试，评估大语言模型在知识图谱构建中的映射能力。


<details>
  <summary>Details</summary>
Motivation: 知识图谱构建耗时耗力，现有方法需大量人工对齐语义；LLM虽具潜力但缺乏标准化评估框架。

Method: 设计包含递增复杂度场景的基准测试集（基于真实用例），通过实验评估多个顶级LLM的映射性能。

Result: LLM在简单场景表现良好，但在复杂场景中性能有限。

Conclusion: 基准测试可有效评估LLM的当前能力，并定义了实现半自动化KG构建的需求，指引未来研究。
...
### [33] [Position: The Turing-Completeness of Real-World Autoregressive Transformers Relies Heavily on Context Management](https://arxiv.org/abs/2605.19514)
*Guanyu Cui,Zhewei Wei,Kun He*

Main category: cs.AI

TL;DR: 本文区分了Transformer图灵完备性的两种设定，指出常见证明基于缩放族设定而非更符合实际的固定系统设定，并强调上下文管理对计算能力的关键影响。


<details>
  <summary>Details</summary>
Motivation: 现有文献常混淆Transformer图灵完备性的两种设定，导致对真实LLM计算能力的误解，需要澄清并重新评估。

Method: 形式化定义固定系统设定，分析缩放族设定与固定系统设定的差异，并通过不同上下文管理方法的对比实验展示其计算能力影响。

Result: 缩放族设定下的图灵完备性证明具有理论资源界限意义，但不适用于固定系统设定；不同上下文管理方法导致显著不同的计算能力。

Conclusion: 上下文管理是决定真实自回归Transformer计算能力的核心组件，固定系统设定更符合实际LLM部署的图灵完备性概念。
...
### [34] [Generative-Evaluative Agreement: A Necessary Validity Criterion for LLM-Enabled Adaptive Assessment](https://arxiv.org/abs/2605.19529)
*Grandee Lee,Yue Wang,Che Yee Lye,Luke Peh*

Main category: cs.AI

TL;DR: 本文提出生成-评估一致性（GEA）作为验证LLM评估闭环的有效性指标，发现模型能恢复约一半的预设能力方差，但存在系统性正向偏差，语法能力评估较强而设计能力评估接近零。


<details>
  <summary>Details</summary>
Motivation: 当同一LLM生成题目、模拟学生回答并评分时，验证循环是自指的，需要衡量其评分与生成功能的一致性。

Method: 通过两阶段自适应评估，直接测量GEA指标，并分解不同技能类型（语法、设计）的评分偏差。

Result: GEA相关系数r=0.698，恢复约一半预设方差，语法技能GEA>0.7，设计技能GEA接近零，低能力高估导致阈值分数膨胀。
...
### [35] [Efficient Elicitation of Collective Disagreements](https://arxiv.org/abs/2605.19521)
*Mohamed Ouaguenouni,Felipe Garrido-Lucero,Umberto Grandi,César Hidalgo,Magdalena Tydrichova*

Main category: cs.AI

TL;DR: 本文提出分层框架，通过扩展成对比较到多重集排名，识别衡量群体分歧所需的最小偏好信息，发现许多分歧指标需要至少三元组信息（而非二元），并设计了两种数据收集协议。


<details>
  <summary>Details</summary>
Motivation: 现有调查用成对比较无法区分结构性分歧与噪声，需更细粒度偏好信息。

Method: 引入多元矩阵记录每个子集中元素的首位概率；定义分歧指标层级为所需的最小子集大小；设计两种估计矩阵的协议，权衡参与者数量与认知负担。

Result: 许多经典分歧指标（如排名方差与分裂度）位于层级3，成对比较不足；理论上和实验上证明了超越层级3的价值。

Conclusion: 分层框架揭示了分歧的结构需求，为实际调查设计提供了理论指导与实用工具。
...
TL;DR: 论文发现自演化技能库存在“库漂移”问题（技能无限制积累导致检索退化、假阳性注入和性能停滞），通过可重复的触发实验和诊断方法，提出并验证了一种最小治理方案，将性能从0.258提升至0.584。


<details>
  <summary>Details</summary>
Motivation: 自演化技能库中LLM生成的技能带来零性能提升（+0.0pp），而人类筛选的技能带来显著提升（+16.2pp），但底层机制未被隔离，因此需要系统研究库漂移的成因与解决方案。

Method: 通过三项可控消融实验隔离漂移触发因素（禁用技能注入、强制过早淘汰），建立完整的诊断日志（技能贡献分数、归因判定、路由器指标），并提出最小治理方案（结果驱动淘汰、活性容量上限、元技能先验编写）。

Result: 治理方案将MBPP+ hard-100的pass@1从0.258基线提升至晚期窗口均值0.584（滚动增益+0.328），八项消融实验分解了各治理机制的负载与替代关系。

Conclusion: 库漂移是自演化智能体中的关键失败模式，通过可复现的诊断方法和轻量级治理框架可有效缓解，为诊断此类问题提供了具体操作指南。
...
TL;DR: 提出SceneCode框架，将自然语言提示转化为可执行的代码驱动室内场景，生成可交互、带关节结构的物体资产。


<details>
  <summary>Details</summary>
Motivation: 现有室内场景合成方法仅生成静态网格，缺乏物体级别的可控性，无法按需生成可交互的新资产。

Method: 使用房间级智能体将提示转化为结构化布局，通过规划-设计-批评循环生成物体请求，路由到五种代码生成策略之一，利用Blender Python程序合成零件级资产，并通过执行引导修复循环进行验证。

Result: SceneCode提高了场景生成的提示忠实度，生成了更干净的网格结构和可加载的关节元数据资产。

Conclusion: 可执行世界程序改进了场景合成，支持物体级交互和可编辑的装配流程。
...
TL;DR: 针对多模型异构硬件场景，研究了LLM在CPU-GPU offloading和抢占下的性能特征，发现offloading导致非线性吞吐下降，抢占开销主要由模型状态重载而非KV缓存转移主导，并总结了未来调度器需考虑的关键特征。


<details>
  <summary>Details</summary>
Motivation: 现有LLM服务系统主要优化单模型吞吐，缺乏对多模型在异构硬件上共享资源时的调度研究，特别是GPU内存限制下的offloading和抢占行为。

Method: 通过实验测量不同模型在offloading不同比例的层时的解码吞吐、抢占开销（模型状态重载与KV缓存转移对比），并分析序列长度和互联带宽的影响。

Result: Offloading导致非线性且模型依赖的吞吐下降，小模型对GPU驻留减少更敏感；抢占开销主要由模型状态重载引起，且因模型和硬件而异；序列长度和带宽加剧数据移动与执行低效。

Conclusion: 未来多模型调度器需考虑模型特定的offloading敏感性、工作负载特征以及抢占和传输的成本结构，以高效管理异构多模型负载。
...
TL;DR: 提出了形式化技能（Formal Skill）这一运行时抽象，通过将可复用流程从提示文本迁移到可执行状态机与钩子策略中，为LLM智能体提供了高效、可执行的技能表示与管控方式。


<details>
  <summary>Details</summary>
Motivation: 现有技能表示（如Markdown指南、函数调用、MCP等）缺乏工作流状态、策略执行与完成纪律的内生支持，导致智能体在真实工作空间中可靠性不足。

Method: 定义Formal Skill抽象，包含JSON元数据、动作模式、Python执行器、钩子控制逻辑、路由机制及本地运行时状态；并在FairyClaw开源框架中实现为事件驱动运行时，支持可执行、可观测、可组合的技能。

Result: 在Harness-Bench上取得具有竞争力的平均分数，且使用显著更少的token，在凸显Formal Skill作用的任务上表现尤为强劲。

Conclusion: Formal Skill是一种token高效、可强制执行的技能表示方式，能显著提升LLM智能体在真实工作空间中的可靠性与效率。
...
TL;DR: 提出Emo-Boost框架，通过融合基于情感的检测器与低级特征检测器，提升深度伪造检测的跨操作泛化能力。


<details>
  <summary>Details</summary>
Motivation: 生成模型不断进步导致传统检测方法难以泛化到未见过的伪造类型，需要利用高级语义线索提高泛化性。

Method: 使用EmoForensics（基于视觉和音频情绪识别模块，建模跨模态情绪一致性），并与现有多模态检测器融合形成Emo-Boost。

Result: 在FakeAVCeleb数据集上，跨操作平均AUC提升2.1%。

Conclusion: 高级语义情绪特征与低级特征互补，能有效增强深度伪造检测的泛化能力。
...
TL;DR: 该论文研究了在策略性环境中，基于先验数据拟合网络（PFN）的表格基础模型如何泛化，并提出了一种无需重新训练的推理时策略感知框架SPN来适应策略性操纵。


<details>
  <summary>Details</summary>
Motivation: 现实决策场景中，个体可能策略性地修改特征以获取有利结果，导致部署后数据分布偏移，而现有表格基础模型通常针对非策略性环境设计，无法处理这种偏移。

Method: 提出策略先验数据拟合网络（SPN），通过构建策略性上下文示例来近似操纵后输入，并将PFN预测与诱导的策略性分布对齐。

Result: 在真实和合成表格数据集上，SPN在策略性操纵下持续提高了鲁棒性和预测性能，优于表格基础模型和经典表格方法。

Conclusion: SPN能有效适应策略性环境，无需重新训练即可提升模型在操纵下的表现。
...
TL;DR: 提出了PStar框架，通过伪代码引导的结构化推理路径，自适应选择推理策略，显著减少视觉语言模型的幻觉，提升鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: VLM在机器人自动化中易产生幻觉，导致决策错误和安全风险，且真实任务问题难度和模态多样，需要鲁棒且自适应的推理策略。

Method: 设计抽象推理函数和结构化伪代码库，利用难度特征向量评估问题复杂度，自适应选择推理路径。

Result: 在POPE上达87.1%，MMStar上达68.0%，超越GPT-4V，显著降低幻觉率。

Conclusion: PStar是减少视觉语言错误的关键一步，为可信赖的VLM在真实自动化系统部署提供了验证机制。
...
### [43] [From Prompts to Pavement Through Time: Temporal Grounding in Agentic Scene-to-Plan Reasoning](https://arxiv.org/abs/2605.19824)
*Ahmed Y. Gado,Omar Y. Goba,Alaa Hassanein,Catherine M. Elias,Ahmed Hussein*

Main category: cs.AI

TL;DR: 本文研究了在自动驾驶系统中，通过在智能体间通信中加入时间条件来提升场景理解与规划的一致性，发现时间条件虽改变推理风格，但未在标准NLP指标上带来显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法将时间视为次要属性，导致连续动作推理不一致，影响安全性与可解释性。

Method: 设计了三种逐步增强时间集成度的规划器架构，在BDD-X数据集子集上使用语义、句法和逻辑指标进行评估。

Result: 时间条件未显著改善标准NLP正确性指标，但定性分析显示在预测性危险推理、稳定校正行为和战略分歧方面有改进。

Conclusion: 研究揭示了基于提示的时间条件作用的局限性，并为时间场景到规划推理建立了首个经验基准。
...
### [44] [Transforming Constraint Programs to Input for Local Search](https://arxiv.org/abs/2605.19671)
*Jo Devriendt,Patrick De Causmaecker,Marc Denecker*

Main category: cs.AI

TL;DR: 本文建立约束优化问题的对称性与局部搜索邻域之间的联系，并利用这一联系在IDP系统中从约束规范自动生成邻域。


<details>
  <summary>Details</summary>
Motivation: 传统上解决组合优化问题需要人工干预将约束编译为元启发式算法的输入数据，缺乏自动化方法。

Method: 利用约束优化问题的对称性属性，自动从约束规范中生成局部搜索邻域。

Result: 在六个经典优化问题上评估生成的邻域，结果表明该技术具有可行性。

Conclusion: 基于对称性的自动邻域生成技术是有效的，能够减少人工干预。
...
### [45] [Beyond Rational Illusion: Behaviorally Realistic Strategic Classification](https://arxiv.org/abs/2605.19674)
...
TL;DR: 本文提出了一种基于前景理论的策略分类框架Pro-SF，以模拟和应对现实中因认知偏差而不完全理性的战略操控行为。


<details>
  <summary>Details</summary>
Motivation: 现有策略分类框架假设代理完全理性，但行为经济学和心理学的证据表明真实决策常受认知偏差影响，因此需要更符合实际的行为模型。

Method: Pro-SF框架通过前景理论中的三个关键机制（收益与成本不对称、主观参考点、概率扭曲）重新定义了代理与决策者的Stackelberg博弈交互。

Result: 在合成和真实数据集上的实验证明Pro-SF是一种行为学上合理的策略分类方法，能有效提升模型在现实中的可靠性。

Conclusion: Pro-SF连接了机器学习和行为经济学，为策略分类在现实世界的稳健部署提供了新的理论基础。
...
### [46] [Projecting Latent RL Actions: Towards Generalizable and Scalable Graph Combinatorial Optimization](https://arxiv.org/abs/2605.19721)
*Franco Terranova,Guillermo Bernardez,Albert Cabellos-Aparicio,Nina Miolane,Abdelkader Lahmadi*

Main category: cs.AI

TL;DR: 提出投影智能体（projection agents），通过连续动作嵌入空间实现更快的推理和更好的泛化，并发布LaGCO-RL库促进可重复性。


<details>
  <summary>Details</summary>
Motivation: 现有RL-GCO方法在泛化性和计算可扩展性上存在局限，随着动作空间增大问题加剧。

Method: 在GNN动作嵌入空间中直接预测连续潜在动作，然后解码为离散动作，使用共享嵌入空间实现RL方法公平比较。

Result: 推理速度提升高达16.2倍，泛化性能提升40%，在超线性决策空间中表现强鲁棒性。

Conclusion: 潜在动作空间方法有效提升了RL-GCO的效率和泛化性，LaGCO-RL库便于复现和新基准适配。
...
### [47] [Robotics-Inspired Guardrails for Foundation Models in Socially Sensitive Domains](https://arxiv.org/abs/2605.19940)
*Rebecca Ramnauth,Drazen Brscic,Brian Scassellati*

Main category: cs.AI

TL;DR: 该论文提出将防护栏问题重新定义为对交互轨迹的运行时行为控制，借鉴机器人学中的形式化约束执行方法，并在三个真实场景中验证了Grounded Observer框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有防护栏方法主要提供经验性风险降低而非可执行的行为保证，且将安全性视为单个输出而非交互轨迹的属性，难以应对累积性和上下文相关的失败。

Method: 借鉴机器人学，引入约束执行的形式化构造，提出Grounded Observer框架，在不确定的闭环系统中对交互轨迹进行运行时干预。

Result: 在闲谈、家庭自闭症治疗和学校行为降级三个真实部署中，该框架能缓解交互轨迹向不良状态漂移，并适应多样化的社会情境。

Conclusion: 运行时行为控制提供了比现有方法更强的保证潜力，并提出了扩展和研究方向以实现更可靠的保证。
...
### [48] [Memory-Augmented Reinforcement Learning Agent for CAD Generation](https://arxiv.org/abs/2605.19748)
*Yin Xiaolong,Liu Yu,Shen Jiahang,Lu Xingyu,Ni Jingzhe,Fan Fengxiao,Sang Fan*

Main category: cs.AI

TL;DR: 提出了一种基于记忆增强强化学习的CAD生成框架，通过结构化工具链、双轨记忆模块和动态效用检索算法，解决了复杂CAD模型生成中推理中断和几何约束失效的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的CAD生成方法在处理长序列、多样操作和强几何约束的复杂模型时，因推理链断裂和缺乏纠错机制而效果不佳。

Method: 构建了结构化工具链封装几何内核，设计双轨记忆模块（案例库与技能库），提出动态效用检索算法，并引入强化学习进行策略优化和自纠正。

Result: 在复杂CAD模型生成任务上显著提升了成功率和几何一致性。

Conclusion: 该方法无需大规模标注数据即可实现在线自纠正与持续演进，优于现有方法。
...
### [49] [Probing Embodied LLMs: When Higher Observation Fidelity Hurts Problem Solving](https://arxiv.org/abs/2605.20072)
*Oussama Zenkri,Oliver Brock*

Main category: cs.AI

TL;DR: 研究发现，在机械谜题任务中，使用原始RGB输入的LLM表现最佳，而完美观测反而最差；模拟中40%的动作结果翻转概率成功率提升2.85倍，与减少重复动作循环有关。


<details>
  <summary>Details</summary>
Motivation: LLM作为机器人认知组件的决策过程不透明，难以解释闭环具身任务成败原因。

Method: 通过Lockbox机械谜题，在物理机器人上比较RGB、RGB-D和符号观测，并用仿真随机翻转动作结果探究噪声影响。

Result: RGB输入表现最佳，完美观测最差；40%翻转概率时成功率提升2.85倍，归因于减少重复动作循环。

Conclusion: 成功率不能充分评估LLM，测量结果反映感知误差与推理失败的交互，而非鲁棒问题解决能力。
...
### [50] [EngiAI: A Multi-Agent Framework and Benchmark Suite for LLM-Driven Engineering Design](https://arxiv.org/abs/2605.19743)
*Gioele Molinari,Florian Felten,Soheyl Massoudi,Mark Fuge*

Main category: cs.AI

TL;DR: 提出了一个多智能体工程设计的基准套件EngiBench和参考实现EngiAI，包含工作流、RAG和HPC三个评测维度，通过实验验证了评测的有效性并揭示了条件分支和长工作流中的性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架无法充分覆盖结合模拟、检索和制造准备的多智能体系统，需要更全面的基准来评测LLM代理在工程设计中的表现。

Method: 设计了三个维度的基准测试（工作流、RAG、HPC），并基于LangGraph构建了EngiAI多智能体系统，协调七个专用智能体，使用四种LLM后端进行实验。

Result: 专有模型任务完成率达96-97%，开源4B模型达55-78%；条件分支任务完成率降至20-53%；RAG门控评分确认检索贡献显著；HPC编排中一步骤完成率从100%降至50%。
...
### [51] [CogScale: Scalable Benchmark for Sequence Processing](https://arxiv.org/abs/2605.19758)
*Yannis Bendi-Ouis,Romain de Coudenhove,Xavier Hinaut*

Main category: cs.AI

TL;DR: 提出了CogScale基准，包含14个可扩展的合成任务，用于评估不同规模下的认知和记忆能力，为快速验证架构创新提供轻量级框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法评估新架构处理序列信息的能力成本高昂且耗时，需要大规模数据和模型，导致计算开销大和迭代周期慢。

Method: 设计了14个可参数化扩展的合成任务，在严格参数预算（1k、10k、100k）下评估GRU、LSTM、xLSTM、ESN、Mamba、Transformer解码器和Transformer编码器-解码器等七种架构。

Result: 经典RNN和ESN在严格参数预算下擅长基本记忆，但只有注意力机制和现代状态空间模型在推理复杂性和任务难度扩展时保持高性能。

Conclusion: CogScale为快速评估序列处理架构提供了有效基准，并揭示了不同架构在扩展时的性能差异。
...
### [52] [What Really Improves Mathematical Reasoning: Structured Reasoning Signals Beyond Pure Code](https://arxiv.org/abs/2605.19762)
*Yuze Zhao,Junpeng Fang,Lu Yu,Zhenya Huang,Kai Zhang,Qing Cui,Qi Liu,Jun Zhou,Enhong Chen*

Main category: cs.AI

TL;DR: 通过控制实验，发现代码不增强通用推理，而是专注于编程能力；收益来自结构化的推理痕迹，如代码-文本和数学-文本混合。


<details>
  <summary>Details</summary>
Motivation: 评估代码在语言模型训练中的实际作用，特别是其对推理能力的影响。

Method: 在10T令牌语料库上进行控制预训练实验，使用细粒度领域分离。

Result: 代码优先提升编程能力，不增强通用推理；推理收益来自跨域结构化数据；增加数学样本密度可提升数学推理，同时保留编程性能。

Conclusion: 数据特征在不同能力维度间转移的方式不同，建议采用精确的数据中心优化策略。
...
### [53] [GroupAffect-4: A Multimodal Dataset of Four-Person Collaborative Interaction](https://arxiv.org/abs/2605.19765)
...
TL;DR: GroupAffect-4是一个包含40名参与者（10个四人组）的多模态语料库，在四种协作任务中同步采集生理、眼动、音频、自我报告、任务结果和大五人格数据，支持个体、人际和群体层面的情感分析。


<details>
  <summary>Details</summary>
Motivation: 现有语料库难以支持对协作群体情感作为耦合的个体、人际和群体过程进行分析，所需信号分散在不同的数据集传统中。

Method: 为每组参与者配备腕戴式生理传感器、眼动追踪眼镜和近讲麦克风，收集连续情感自我报告、任务后问卷、任务结果和大五人格评分，并通过共享时钟对齐所有时间数据。

Result: 数据集覆盖超过91%的预期生理窗口和98%的眼动窗口，通过协商块的情感操纵检查验证了任务有效性，并定义了15个跨三个分析层级（个体内状态、个体间特质、群体动态）的基准目标。

Conclusion: GroupAffect-4为多模态群体情感计算提供了高质量、结构化的基准语料库，支持多种分析层级，并附带开放的数据和代码资源。
...
### [54] [OpenComputer: Verifiable Software Worlds for Computer-Use Agents](https://arxiv.org/abs/2605.19769)
*Jinbiao Wei,Qianran Ma,Yilun Zhao,Xiao Zhou,Kangqi Ni,Guo Gan,Arman Cohan*

Main category: cs.AI

TL;DR: 提出了OpenComputer框架，通过应用状态验证器、自进化验证层、任务生成流水线和评估工具，构建可验证的桌面软件环境，覆盖33个应用和1000个任务。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用智能体评估依赖人工或LLM裁判，缺乏对细粒度应用状态的可信验证，导致自动化评估可靠性不足。

Method: 集成了专用状态验证器、基于执行反馈的自进化验证层、合成真实桌面任务的工作流，以及记录完整轨迹并计算可审计部分学分数的评估框架。

Result: 硬编码验证器比LLM裁判更接近人类判断；前沿智能体端到端完成困难，开源模型与OSWorld-Verified评分差距明显，暴露自动化鲁棒性缺口。

Conclusion: OpenComputer可作为可靠基准，揭示了当前智能体在细粒度应用状态理解上的不足，强调验证器对计算机自动化评估的必要性。
...
### [55] [Minimax Optimal Variance-Aware Regret Bounds for Multinomial Logistic MDPs](https://arxiv.org/abs/2605.19768)
...
### [56] [From SGD to Muon: Adaptive Optimization via Schatten-p Norms](https://arxiv.org/abs/2605.19781)
*Thomas Massena,Corentin Friedrich,Mathieu Serrurier*

Main category: cs.AI

TL;DR: 提出了一种基于数据驱动的动态LMO几何选择方法，通过单步随机特征回归代理模型优化深度学习优化器的更新规则，在SGD和Muon之间自适应插值，并实现约3%的额外开销。


<details>
  <summary>Details</summary>
Motivation: 现有优化器（如Muon）使用固定的矩阵约束（LMO几何），但固定几何不一定是问题的最优选择，因此需要自适应方法。

Method: 利用梯度和激活统计量的闭式解，通过单步随机特征回归代理模型动态选择每层的LMO几何，并结合参数级预条件恢复SGD、Muon、Adam和MuAdam。

Result: 在三种训练场景中，该方法性能优于或持平于Muon和AdamW中的最佳优化器，运行时开销仅约3%。

Conclusion: 证明了LMO几何可以从运行时数据高效自适应，为超越静态几何的优化器设计提供了新方向。
...
### [57] [Distribution-Free Uncertainty Quantification for Continuous AI Agent Evaluation](https://arxiv.org/abs/2605.19779)
*Yuxuan Gao,Megan Wang,Yi Ling Yu*

Main category: cs.AI

TL;DR: 将分裂共形预测与自适应共形推断(ACI)应用于连续AI Agent评估，提供分布自由的覆盖保证，开发多Agent管道的组合不确定性界限及共形弃权规则，并通过实际数据进行验证。


<details>
  <summary>Details</summary>
Motivation: 现有AI Agent评估方法缺乏统计可验证的不确定性定量化，尤其在模型持续更新和循环验证需求下。

Method: 采用分裂共形预测和自适应共形推断(ACI)构建预测区间，提出组合不确定性方法处理多Agent管道，设计共形弃权规则控制错误排序率，并进行FDR校正。

Result: 共形区间24小时校准误差低于0.02；ACI在Agent发布后自适应扩大区间35%并收敛；多管道实验覆盖相关性范围rho在[-0.5, 0.9]；Agent条件覆盖集中在标称水平(均值80.4%，90% Agent在[72%,90%])；跨源情感分歧预测排序不稳定性(r=0.64, p<0.01)；循环控制验证超越基准(rho_s=0.52, p<0.01)。

Conclusion: 提出的框架为AI Agent评估提供了统计保证的不确定性量化方法，可适应动态更新场景，并具备多Agent及排名场景的扩展性。
...
### [58] [Prior Knowledge or Search? A Study of LLM Agents in Hardware-Aware Code Optimization](https://arxiv.org/abs/2605.19782)
*Dmitry Redko,Albert Fazlyev,Konstantin Sozykin,Maria Ivanova,Evgeny Burnaev,Egor Shvetsov*

Main category: cs.AI

TL;DR: 通过三项实验发现，LLM在代码优化中主要依赖预训练先验，而非反馈或智能体结构。


<details>
  <summary>Details</summary>
Motivation: LLM优化系统复杂度增加，难以评估各组件贡献及失败原因。

Method: 三项对照实验：黑盒优化、零样本内核生成、反馈循环内核优化。

Result: 1）LLM表现为贪婪优化器；2）输入大小信息无影响，非常见内核尺寸下性能下降；3）CUDA迭代改进，TVM IR退化，低密度语言导致优化下降。

Conclusion: LLM代码优化高度依赖预训练先验，而非反馈或结构。
...
### [59] [Streamlined Constraint Reasoning via CNN Pattern Recognition on Enumerated Solutions](https://arxiv.org/abs/2605.19895)
*Patrick Spracklen*

Main category: cs.AI

TL;DR: 提出一种结合卷积神经网络和大型语言模型的自动化流线约束合成方法，通过对比学习从可行解中提取结构模式，显著加速约束编程问题求解。


<details>
  <summary>Details</summary>
Motivation: 传统流线约束合成依赖语法搜索或直接提示模型，未充分利用解的结构信息，导致效果受限。

Method: 枚举可行解并与扰动非解进行对比训练卷积神经网络，检测结构模式，再通过大型语言模型将判别信号转化为MiniZinc流线约束。

Result: 在硬化基准问题上实现98.8%至89.4%的求解时间缩减，单流线约束加速比达932倍、356倍和1103倍。

Conclusion: 该方法通过数据驱动的结构发现有效生成高性能流线约束，优于现有自动化方法。
...
### [60] [Explainable Wastewater Digital Twins: Adaptive Context-Conditioned Structured Simulators with Self-Falsifying Decision Support](https://arxiv.org/abs/2605.19826)
*Gary Simethy,Daniel Ortiz Arroyo,Petar Durdevic*

Main category: cs.AI

TL;DR: 提出了一个可解释的数字孪生系统CCSS-IX，用于污水处理厂的曝气和投药设定点安全认证，结合保形风险控制实现有限样本覆盖的安全决策。


<details>
  <summary>Details</summary>
Motivation: 安全关键工业流程的数字孪生缺乏认证安全保证，污水处理厂面临安全-效率权衡（曝气不足导致排放违规和N2O激增，曝气过度浪费能源）。

Method: 构建基于上下文门控网络的连续时间切换线性状态空间专家模型库，运行时层采用保形风险控制对操作建议进行认证/拒绝/生成时态证伪证据。

Result: 静态集成模型RMSE误差0.78%（相对黑箱参考），自适应模型1.08%；校准的拒绝规则将累积遗憾降低43.6%，在BSM2基准上完全消除不安全行为；事件对齐时态证伪防止93/187次假安全N2O批准（基线4.65倍，p<1e-21）。

Conclusion: 该方法在多个全规模污水处理厂和基准模型上验证有效，兼顾可解释性与统计安全保证。
...
### [61] [PEEK: Context Map as an Orientation Cache for Long-Context LLM Agents](https://arxiv.org/abs/2605.19932)
*Zhuohan Gu,Qizheng Zhang,Omar Khattab,Samuel Madden*

Main category: cs.AI

TL;DR: 介绍PEEK系统，通过缓存和维护可重用的方向性知识（上下文地图），提升LLM代理在长上下文和重复场景中的效率与准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅保留轨迹、原始材料或任务策略，缺乏对重复上下文中可重用方向性知识的持久化，导致效率低下。

Method: PEEK系统包含三个模块：Distiller（提取推理时信号中的可迁移知识）、Cartographer（转化为结构化编辑）、Evictor（基于优先级限制令牌预算）。

Result: 在长上下文推理中提升6.3-34.0%，减少93-145次迭代，成本降低1.7-5.8倍；在上下文学习中解决率和评分准确性分别提升6.0-14.0%和7.8-12.1%，成本为ACE的1.4倍。

Conclusion: 上下文地图能帮助LLM代理更准确、高效地处理重复的长上下文任务，且泛化至不同模型和架构。
...
### [62] [Probabilistic Tiny Recursive Model](https://arxiv.org/abs/2605.19943)
*Amin Sghaier,Ali Parviz,Alexia Jolicoeur-Martineau*

Main category: cs.AI

TL;DR: 提出Probabilistic TRM (PTRM)，通过随机探索提升TRM在复杂推理任务中的表现，以极小参数取得显著准确率提升。


<details>
  <summary>Details</summary>
Motivation: Tiny Recursive Models (TRM) 的确定性递归可能陷入次优解，现有方法需要任务特定扰动，缺乏通用逃逸机制。

Method: 在TRM的每一步深递归中注入高斯噪声，并行探索多个解空间，利用模型原有的Q头选择最佳轨迹。

Result: Sudoku-Extreme准确率从87.4%升至98.75%，Pencil Puzzle Bench从62.6%升至91.2%，超过前沿LLM两倍且成本极低（7M参数）。

Conclusion: PTRM是一种任务无关的测试时计算扩展框架，无需重训练即可大幅提升推理准确率，兼具高效与通用性。
...
### [63] [GeoX: Mastering Geospatial Reasoning Through Self-Play and Verifiable Rewards](https://arxiv.org/abs/2605.20006)
*Kyeongjin Ahn,Seungeon Lee,Krishna P. Gummadi,Meeyoung Cha*

Main category: cs.AI

TL;DR: GeoX是一种自我博弈框架，通过可执行程序获取空间逻辑与可验证奖励，在无需大规模人工标注数据的情况下提升地理空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 地理空间推理中，标注海量组合问题空间成本高昂，阻碍了该能力的发展。

Method: 使用单一多模态策略，通过可执行程序在三种推理模式（溯因、演绎、归纳）下提出并解决空间问题，利用验证器执行程序生成奖励信号，通过强化学习联合优化策略。

Result: GeoX在基础视觉语言模型上平均提升高达5.5个点，性能可媲美或超越基于数百万标注数据的传统基线。

Conclusion: GeoX通过自我博弈有效获取了地理空间推理能力，并发布了通过自我博弈积累的基准数据集。
...
### [64] [When Skills Don't Help: A Negative Result on Procedural Knowledge for Tool-Grounded Agents in Offensive Cybersecurity](https://arxiv.org/abs/2605.20023)
*Samuel Jacob Chacko,James Hugglestone,Chashi Mahiul Islam,Xiuwen Liu*

Main category: cs.AI

TL;DR: 技能在LLM代理中平均提升16.2%的任务通过率，但存在方差，且在某些环境中技能收益会大幅下降。


<details>
  <summary>Details</summary>
Motivation: 研究技能在何种情况下有效，以及为何在某些任务中技能反而会降低性能。

Method: 重分析180次对照实验（CTF代理在四种文档条件下运行），对应无技能、经验技能、策划技能和全面技能去除。

Result: 在网络安全领域，技能收益仅为8.9个百分点（p>0.7），五分之六的效应量低于小效应阈值，且时间侧信道任务中技能还降低了性能。

Conclusion: 环境反馈带宽是缺失的关键变量：当工具层提供严格、低延迟的观察时，环境本身提供了技能所需的纠正信号，从而降低了技能的边际收益。
...
### [65] [AutoResearchClaw: Self-Reinforcing Autonomous Research with Human-AI Collaboration](https://arxiv.org/abs/2605.20025)
...
TL;DR: 提出多智能体自主研究管线AutoResearchClaw，通过结构化辩论、自愈执行、可验证报告等机制，在ARC-Bench上比AI Scientist v2提升54.7%，实现迭代式科学发现。


<details>
  <summary>Details</summary>
Motivation: 现有自主研究系统将过程建模为线性管线，缺乏迭代修正、经验积累和人机协作。

Method: 多智能体框架包含：结构化多智能体辩论、Pivot/Refine自愈循环、可验证报告、七级人机协作干预、跨运行经验进化。

Result: 在ARC-Bench上超越AI Scientist v2达54.7%，精准的高杠杆点人机协作优于全自动或逐步监督。

Conclusion: AutoResearchClaw作为研究增强工具，以迭代和协作方式增强而非取代人类科学判断。
...
### [66] [Using Aristotle API for AI-Assisted Theorem Proving in Lean 4: A Formalisation Case Study of the Grasshopper Problem](https://arxiv.org/abs/2605.20120)
*Gabriel Rongyang Lau*

Main category: cs.AI

TL;DR: 报告了一个关于IMO 2009 Problem 6的Lean 4形式化案例研究，其中AI辅助证明生成了包含四个已验证引理的部分证明，但主定理因全局计数步骤未完成而由一个未解决的sorry闭合。


<details>
  <summary>Details</summary>
Motivation: 探索AI辅助定理证明在奥林匹克级别数学中的可靠性，特别关注生成证明的验证状态及其局限性。

Method: 使用Aristotle API生成证明尝试，并通过Lean 4进行形式化验证，分析已验证和未验证的证明组件。

Result: 四个局部引理通过验证，但主定理依赖的全局计数步骤未在Lean中编码，导致主定理存疑。

Conclusion: 案例展示了AI辅助形式化的核心局限：局部证明搜索可成功，但全局组合簿记仍可能未解决。
...
### [67] [Neurosymbolic Learning for Inference-Time Argumentation](https://arxiv.org/abs/2605.20098)
*Gabriel Freedman,Adam Dejl,Adam Gould,Mansi,Lihu Chen,Jianqi Jiang,Francesca Toni*

Main category: cs.AI

TL;DR: 本文提出了一种可训练的神经符号框架ITA，用于三元声明验证，通过形式论证语义指导LLM训练，生成带分数的论证，并基于这些论证计算确定性预测（真/假/不确定），在保持高准确性的同时提供忠实且可检查的解释。


<details>
  <summary>Details</summary>
Motivation: 现有声明验证系统在信息不完整或冲突时难以给出恰当的不确定答案，且事后解释可能不忠实，亟需一种能够结合论证推理与可训练性、确保预测忠实于论证结构的方法。

Method: ITA框架在训练时利用形式论证语义（如根据论证强度计算声明状态）同时指导LLM生成论证及其基准分数，并优化这些生成以提升论证预测质量；推理时直接根据生成的带分数论证计算三元预测，而非依赖事后推理链。

Result: 在两个三元声明验证数据集上，ITA不仅优于基于论证的基线方法，还能与非论证的直接预测方法竞争，同时提供由显式、可检查的论证结构确定的预测。

Conclusion: ITA通过将形式论证语义集成到LLM训练与推理中，实现了忠实、可解释的三元声明验证，在保持性能的同时提供了可控且透明的决策过程。
...
### [68] [Not Every Rubric Teaches Equally: Policy-Aware Rubric Rewards for RLVR](https://arxiv.org/abs/2605.20164)
*Utkarsh Tyagi,Xingang Guo,MohammadHossein Rezaei,Daniel George,Anas Mahmoud,Jackson Lee,Bing Liu,Yunzhong He*

Main category: cs.AI

TL;DR: 提出了POW3R框架，通过策略感知的权重调整，使基于规则的多准则奖励在强化学习中更有效，加速收敛并提升严格满足所有准则的比例。


<details>
  <summary>Details</summary>
Motivation: 标准静态聚合方法将人为重要性与当前优化信号混淆，导致已饱和或不可达准则干扰训练，而区分性准则未被充分利用。

Method: POW3R在保持人为权重和类别平衡的同时，利用rollout层级的对比分析动态调整准则奖励权重，使GRPO奖励更信息丰富。

Result: 在三个基础策略和两个数据集上，POW3R在24/30个对比中获胜，平均奖励和严格完成率均优于标准GRPO，且收敛速度提升2.5-4倍。

Conclusion: 基于准则的奖励应区分最终答案需要关注的内容与当前策略可学习的信号。
...
### [69] [HaorFloodAlert: Deseasonalized ML Ensemble for 72-Hour Flood Prediction in Bangladesh Haor Wetlands](https://arxiv.org/abs/2605.20167)
*Salma Hoque Talukdar Koli,Fahima Haque Talukder Jely,Md. Samiul Alim,Md. Zakir Hossen*

Main category: cs.AI

TL;DR: 提出了一个针对孟加拉国haor湿地突发性洪水的72小时概率预报系统，融合机器学习集成、上游SAR遥感数据和去季节化处理，兼有水稻灾损评估功能。


<details>
  <summary>Details</summary>
Motivation: 现有河漫滩洪泛预报系统无法捕捉haor湿地中的回水效应和突发性洪水，导致每年boro水稻严重损失。

Method: 构建了一个去季节化的机器学习集成模型（RF和XGBoost加权集成），引入上游Barak河Sentinel-1 SAR遥感变化检测（Otsu阈值法）提供约36小时提前量。

Result: 集成模型在77个实际Sentinel-1事件上达到89.6% LOOCV准确率、87.5%召回率和0.943 AUC-ROC，SAR变化检测空间匹配率为84-91%。

Conclusion: HaorFloodAlert能有效预测haor湿地突发洪水，为boro水稻灾害管理提供可靠的72小时预警和灾损估计支持。
...
### [70] [A Methodology for Selecting and Composing Runtime Architecture Patterns for Production LLM Agents](https://arxiv.org/abs/2605.20173)
*Vasundra Srinivasan*

Main category: cs.AI

TL;DR: 本文提出一种用于生产环境的 LLM Agent 运行时架构设计方法，核心是定义“随机-确定性边界 (SDB)”作为基本构造，并基于此组织协调、状态与控制三种设计关注点，给出六种运行时模式，以及选择模式、诊断故障的实用流程。


<details>
  <summary>Details</summary>
Motivation: 当前生产级 LLM Agent 将模型随机输出与确定性系统简单组合，但两者之间的边界未被作为一等架构对象加以规范，导致系统可靠性、可维护性不足。本文旨在形式化这一边界，并提供系统的运行时设计方法。

Method: 通过构建 SDB 四部分契约（提议者、验证者、提交步骤、拒绝信号），围绕协调、状态、控制三个关注点分类六种运行时模式（分层委托、散射聚合+ saga、事件驱动序列、共享状态机、超级门控、人在回环），并给出模式选择五步法与故障诊断流程。

Result: 提出六种运行时模式及其与分布式系统概念的关联；指出回放发散这一新故障模式；通过可靠性分解显示，模型方差降低时模式选择与 SDB 强度成为长期可靠性的重要杠杆；将方法论应用于五个工作负载，并给出一个 90 天合同续约 Agent 的可运行参考实现。

Conclusion: SDB 是生产型 Agent 运行时的关键抽象；架构模式的选择和 SDB 的强度在模型方差降低时对系统长期可靠性至关重要；所提方法论可实际指导运行时设计与故障排查。
...
### [71] [MotionMERGE: A Multi-granular Framework for Human Motion Editing, Reasoning, Generation, and Explanation](https://arxiv.org/abs/2605.18956)
*Bizhu Wu,Jinheng Xie,Wenting Chen,Zhe Kong,Jianfeng Ren,Linlin Shen,Ruibin Bai,Rong Qu*

Main category: cs.CV

TL;DR: 提出了MotionMERGE框架，通过细粒度建模和推理感知预训练，弥合运动与语言之间的粒度差距，实现精确的运动生成、理解和编辑。


<details>
  <summary>Details</summary>
Motivation: 现有运动-语言模型缺乏对身体部位的细粒度理解和控制，源于模型无法关注局部运动模式，且训练数据缺乏细粒度监督。

Method: 采用单一LLM显式建模局部和时间粒度的运动，设计推理感知粒度协同预训练策略，并构建包含细粒度时空修正指令和运动链式推理标注的大规模数据集MotionFineEdit。

Result: 在运动生成、理解和编辑任务上表现更精确，且对其它复杂运动任务具有零样本泛化能力。

Conclusion: 该项工作朝着具有更细粒度运动交互和类人推理能力的模型迈出了重要一步。
...
### [72] [Harnessing Self-Supervised Features for Art Classification](https://arxiv.org/abs/2605.18974)
*Federico Melis,Davide Bilardello,Emanuele Prato,Evelyn Turri,Lorenzo Baraldi*

Main category: cs.CV

TL;DR: 本文系统研究了监督学习和自监督学习骨干网络在艺术品分类和检索中的有效性，实验表明自监督骨干网络能持续提升分类性能，并探讨了在虚拟现实博物馆导航中的应用。


<details>
  <summary>Details</summary>
Motivation: 艺术品分类因细节与抽象特征交织而具有挑战性，需要有效的特征提取方法。

Method: 使用DINO系列和CLIP模型作为特征提取器，评估多种分类策略和特征表示，通过实验对比监督与自监督方法。

Result: 自监督骨干网络在艺术品分类中表现更优，且分类与检索模块在虚拟现实中具应用潜力。

Conclusion: 自监督学习是提升艺术品分类性能的有效途径，可支撑虚拟现实等现实应用。
...
### [73] [Artifact-Bench: Evaluating MLLMs on Detecting and Assessing the Artifacts of AI-Generated Videos](https://arxiv.org/abs/2605.18984)
...
TL;DR: 提出了Artifact-Bench基准，用于评估多模态大语言模型在AI生成视频伪影检测与分析上的能力，发现现有模型性能有限且与人类感知不匹配。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽提升真实感，但其输出仍存在伪影；多模态大语言模型对这类伪影的感知和推理能力不明确，且缺乏系统性基准。

Method: 构建三级分层伪影分类体系，定义三类任务（真假分类、成对比较、细粒度识别），在19个模型上进行实验。

Result: 多数模型在伪影感知任务上接近随机或低于随机性能，且MLLM判断与人类偏好存在显著偏差。

Conclusion: 当前MLLM作为AI生成视频真实感评估器可靠性有限，Artifact-Bench可推动该领域研究。
...
### [74] [EgoTraj: Real-World Egocentric Human Trajectory Dataset for Multimodal Prediction](https://arxiv.org/abs/2605.19004)
*Ahmad Yehia,Abduallah Mohamed,Tianyi Wang,Jiseop Byeon,Kun Qian,Junfeng Jiao,Christian Claudel*

Main category: cs.CV

TL;DR: 提出了一个名为EgoTraj的自我中心多模态开放数据集，包含75个真实城市环境中的行人导航序列，并利用该数据集对现有轨迹预测方法进行了基准测试和分析。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏在真实世界环境中收集的自我中心轨迹数据集，准确预测人类轨迹的研究进展有限。

Method: 通过Meta Quest Pro设备在真实城市环境中收集多穿戴者的75个导航序列，同步记录RGB视频、6自由度头部姿态、3D眼动向量和场景标注。

Result: 对多种先进自我中心轨迹预测方法进行了基准测试，并通过消融实验分析了眼动、场景和运动线索的贡献。

Conclusion: EgoTraj数据集在增强现实感知、导航和辅助系统方面具有重要价值，并已公开提供。
...
### [75] [A Systematic Failure Analysis of Vision Foundation Models for Open Set Iris Presentation Attack Detection](https://arxiv.org/abs/2605.19020)
*Rahul Anand,Siddharth Singh,Dileep A D,Mahadeva Prasanna,Raghavendra Ramachandra*

Main category: cs.CV

TL;DR: 本文系统性分析了通用视觉基础模型在开集虹膜呈现攻击检测（PAD）中的失败模式，发现模型在未知攻击和跨光谱场景下泛化能力不足，即使使用LoRA微调也会放大失败。


<details>
  <summary>Details</summary>
Motivation: 现有研究未充分检验视觉基础模型在开集虹膜PAD中的适用性，特别是面对真实部署中的分布偏移（如未知攻击仪器、跨光谱数据）。

Method: 评估五种基础模型，采用三种开集协议（区分不同分布偏移来源），比较冻结特征与LoRA适配，并补充完整微调、虹膜切分输入等多种验证实验。

Result: 模型在相似传感器数据上表现良好，但对未知攻击泛化失败，跨光谱性能急剧下降；LoRA仅在部分跨数据集场景提升性能，但在攻击和光谱偏移时加剧失败。

Conclusion: 强闭集或跨数据集性能不能保证开集安全，需要设计能同时保持呈现伪影敏感性且对部署变化鲁棒的PAD特征。
...
### [76] [MedFM-Robust: Benchmarking Robustness of Medical Foundation Models](https://arxiv.org/abs/2605.19027)
*Xiangxiang Cui,Tianjin Huang,Yifang Wang,Lijie Hu,Lu Yin*

Main category: cs.CV

TL;DR: 本文综述了医疗基础模型（MedFMs）在临床中的两大范式：医疗视觉语言模型（如LLaVA-Med、GPT-4o）和分割基础模型（如SAM-Med2D）。强调对其在真实环境中可靠性的严格评估。


<details>
  <summary>Details</summary>
Motivation: 随着MedFMs在临床广泛部署，评估其现实条件下的可靠性成为关键需求。

Method: 分类分析两类模型：Med-VLMs（视觉问答、报告生成等）和基于SAM的医疗分割模型。

Result: 指出两类模型在医疗应用中的能力与应用潜力。

Conclusion: 需要更多真实世界可靠性评估来指导临床部署。
...
### [77] [Personalized Face Privacy Protection From a Single Image](https://arxiv.org/abs/2605.19032)
*Zachary Yahn,Fatih Ilhan,Tiansheng Huang,Selim Tekin,Sihao Hu,Yichang Xu,Margaret Loper,Ling Liu*

Main category: cs.CV

TL;DR: 提出了FaceCloak，一种基于单张照片生成个性化面部隐私保护掩模的系统，有效防止面部识别模型识别用户身份。


<details>
  <summary>Details</summary>
Motivation: 在线面部照片易被恶意爬取和用于未经授权的面部识别，侵犯个人隐私。

Method: 三阶段个性化面部扰动学习：从单张图像生成多样合成图像；在关键身份泄露区域迭代添加扰动，使身份嵌入远离实际身份；生成轻量级、像素级的个性化掩模。

Result: 在三个数据集和十个识别模型上的实验表明，FaceCloak优于29种现有方法，具有有效性和良好感知质量。

Conclusion: FaceCloak能高效保护用户面部隐私，且仅需单张图像即可定制通用掩模。
...
### [78] [LiFT: Lifted Inter-slice Feature Trajectories for 3D Image Generation from 2D Generators](https://arxiv.org/abs/2605.19060)
*Xinhe Zhang,Yuyang Zhang,Pengfei Jin,Arnau Marin-Llobet,Na Li,Quanzheng Li*

Main category: cs.CV

TL;DR: LiFT通过将3D体积合成分解为逐切片生成和切片间轨迹学习，实现高效且解剖一致的高分辨率3D医学图像生成。


<details>
  <summary>Details</summary>
Motivation: 全3D模型计算成本高，而2D生成器难以保持切片间解剖一致性。

Method: 将体积视为特征空间中的有序轨迹，使用三平面漂移损失和双向z-上下文混合器分别处理无条件生成和配对翻译。

Result: 在BraTS 2023和SynthRAD2023上，LiFT保持逐切片质量，推理成本降低135倍，并改善切片间一致性。
...
### [79] [Probability-Conserving Flow Guidance](https://arxiv.org/abs/2605.20079)
*Parsa Esmati,Junha Hyung,Amirhossein Dadashzadeh,Jaegul Choo,Majid Mirmehdi*

Main category: cs.CV

TL;DR: 提出了自适应流形引导（AdaMaG）方法，通过连续性方程分析引导机制，修正了传统分类器无关引导等方法破坏概率守恒的问题，在强引导下保持生成样本在数据流形上。


<details>
  <summary>Details</summary>
Motivation: 现有引导方法（如CFG）忽略生成流形的几何结构，导致概率不守恒和样本偏离流形，引发饱和和幻觉问题。

Method: 从连续性方程分解引导效应为散度项和分数平行项，提出时间依赖调度与分数平行衰减策略，形成无需额外推理成本的即插即用规则AdaMaG。

Result: 在图像生成基准上，AdaMaG提升了真实感、减少幻觉，并在高引导区间实现可控去饱和。

Conclusion: 该分解统一了多种经验启发式，AdaMaG提供了理论驱动的有效引导改进方案。
...
### [80] [Learning Long-Term Temporal Dependencies in Photovoltaic Power Output Prediction Through Multi-Horizon Forecasting](https://arxiv.org/abs/2605.19074)
*Sumit Laha,Ankit Sharma,Hassan Foroosh*

Main category: cs.CV

TL;DR: 提出从单步预测转向多步联合优化的框架，利用天空图像和历史光伏数据，通过多步联合训练提升深度神经网络预测精度，且计算开销低。


<details>
  <summary>Details</summary>
Motivation: 全球太阳能光伏装机容量快速增长（2024年达597 GW），光伏发电间歇性导致电网不稳定，现有单架构、单步预测方法存在局限。

Method: 采用多步联合优化框架，通过联合优化未来序列值，避免网络过早收敛，整合天空图像序列与历史光伏数据，评估多种深度学习架构。

Result: 多步预测方法显著提升了跨整个预测区间的准确性和鲁棒性，计算开销与单步模型相当。

Conclusion: 该方法提供了一种可扩展、高效的电网韧性提升方案，优于传统单步预测。
...
### [81] [CRAFT: Critic-Refined Adaptive Key-Frame Targeting for Multimodal Video Question Answering](https://arxiv.org/abs/2605.19075)
*Mahesh Bhosale,Abdul Wasi,Vishvesh Trivedi,Pengyu Yan,Akhil Gorugantu,David Doermann*

Main category: cs.CV

TL;DR: CRAFT是一个基于查询条件的多视频问答管道，通过动态关键帧选择、ASR、混合批评家循环以及引用合并，在MAGMaR 2026上取得了最佳平均分0.739，并泛化至WikiVideo数据集。


<details>
  <summary>Details</summary>
Motivation: 现有系统在处理多视频问答时难以从异构视频档案中检索查询相关证据，且无法为每个声明归属其支持来源，因此需要一种能验证和修复声明并合并引用的方法。

Method: CRAFT结合了动态关键帧选择、带多语言回退的ASR、混合批评家循环（包括UNLI时间蕴含、DeBERTa-v3跨声明筛选和Llama-3.2-3B裁决器），以及最终引用合并阶段。

Result: 在MAGMaR 2026上取得最佳平均分0.739、参考召回率0.810和引用F1值0.635；在WikiVideo转换数据上平均分0.823。消融实验显示原子声明、ASR和批评家循环是主要增益来源。

Conclusion: CRAFT的声明中心证据聚合方法能有效泛化至不同数据集，其核心组件（原子声明、ASR、批评家循环）对性能提升至关重要。
...
### [82] [FAGER: Factually Grounded Evaluation and Refinement of Text-to-Image Models](https://arxiv.org/abs/2605.19111)
*Youngsun Lim,Cusuh Ham,Pin-Yu Chen,Deepti Ghadiyaram*

Main category: cs.CV

TL;DR: 现有文本到图像评估指标主要关注显式信息，忽略了隐含、外部知识或身份界定的事实要求。为此，我们提出FAGER框架，通过构建结构化事实评估体系，结合LLM与VLM，实现事实正确性的评估与改进，在多个数据集上优于先前指标。


<details>
  <summary>Details</summary>
Motivation: 现有T2I评估指标无法捕捉提示中隐含或基于外部知识的事实要求，导致在科学、历史等领域的事实正确性评估不足。

Method: FAGER框架结合LLM提出事实候选，通过参考引导的视觉事实提取与验证构建结构化事实评估，并转换为VLM问答对进行评估。

Result: 在科学、历史、产品、文化等五个数据集的事实性A/B测试中，FAGER优于所有先前指标，并能无训练优化T2I输出，显著提升事实性。

Conclusion: FAGER能有效评估并改进T2I生成的事实正确性，适用于知识密集型场景。
...
### [83] [Knowing When Not to Predict: Self Supervised Learning and Abstention for Safer DR Screening](https://arxiv.org/abs/2605.19133)
*Muskaan Chopra,Lorenz Sparrenberg,Jan H. Terheyden,Rafet Sifa*

Main category: cs.CV

TL;DR: 本研究评估了SSL预训练时长对医学图像模型校准置信度和选择性预测的影响，发现预训练能改善选择性表现，但准确率饱和后，更长的预训练不一定提升可靠性。


<details>
  <summary>Details</summary>
Motivation: 对于安全关键型筛查任务，如糖尿病视网膜病变分级，模型不仅需要准确，还需知道何时预测不可靠并推迟决策，而现有SSL研究主要关注下游准确率或AUROC。

Method: 在固定微调协议下评估多个SSL预训练检查点，利用校准置信度评估选择性预测，包括校准置信度、覆盖率和选择性准确率等指标。

Result: SSL预训练优于从零开始训练，但准确率饱和后，选择性性能在不同检查点间变化显著，且更长预训练并不一致提升可靠性。

Conclusion: 预训练长度应视为重要的可靠性设计选择，而非仅计算细节，并强调基于弃权的评估的重要性。
...
### [84] [Towards Data-Efficient Video Pre-training with Frozen Image Foundation Models](https://arxiv.org/abs/2605.19137)
*Svetlana Orlova,Niccolò Cavagnero,Gijs Dubbelman*

Main category: cs.CV

TL;DR: 本文探索了一种轻量级视频模型训练范式：冻结预训练图像基础模型的图像编码器，仅训练循环时序模块处理流式视频。实验表明，无需大规模视频预训练，模型即可在多种视频理解任务中展现出较强的时间推理能力。


<details>
  <summary>Details</summary>
Motivation: 视频基础模型通常需要大规模视频预训练，数据与计算成本高昂；而现有图像基础模型已具备强大的空间表征能力。因此，研究是否可以通过重用图像模型的空间表征并仅预训练时间推理模块来构建高性能视频模型。

Method: 冻结预训练图像基础模型作为空间编码器，仅训练一个循环时序模块（如RNN变体）来处理连续视频帧，从而减少视频数据与计算需求。

Result: 在多个视频理解任务中，该轻量级方法无需大规模视频预训练便能获得较强的时序性能。

Conclusion: 研究结果为使用冻结图像基础模型仅预训练时序模块构建循环视频基础模型提供了可行性证据，为未来减少视频预训练的计算开销指明了方向。
...
### [85] [Efficient coding along the visual hierarchy](https://arxiv.org/abs/2605.19155)
*Ananya Passi,Brian S. Robinson,Michael F. Bonner*

Main category: cs.CV

TL;DR: 本文提出一种无监督的高效编码模型，仅用少量数据就能构建出类似人类视觉的分层特征，从边缘到形状，且能预测人脑fMRI反应。


<details>
  <summary>Details</summary>
Motivation: 探索生物视觉系统为何能从有限经验中高效学习，而深度学习需要大量数据。

Method: 开发无监督学习流程，每层网络通过局部统计压缩自然图像的主导变化模式，不使用标签、任务或反向传播。

Result: 模型特征与人类视觉感知一致，能预测fMRI反应，且与监督微调结合后，在低数据设置下实现更好的脑对齐和快速类别学习。

Conclusion: 高效编码可能塑造整个视觉层次中的表示，并解释生物视觉的数据效率。
...
### [86] [Quantized Machine Learning Models for Medical Imaging in Low-Resource Healthcare Settings](https://arxiv.org/abs/2605.19207)
*Sumanth Meenan Kanneti,Aryan Shah*

Main category: cs.CV

TL;DR: 提出多策略压缩框架，结合量化感知训练、知识蒸馏和Float16量化，将脑肿瘤分类模型压缩6.14倍，验证准确率从82.20%微增至82.37%。


<details>
  <summary>Details</summary>
Motivation: 深度模型在低资源临床环境中难以部署，需解决计算、内存和功耗限制。

Method: 采用MobileNetV2骨干网络，经三阶段迁移学习后使用TensorFlow Lite进行Float16后训练量化；并设计DenseNet-101教师至DenseNet-32学生的知识蒸馏及量化感知训练作为补充策略。

Result: 量化后模型大小从35.34 MB降至5.76 MB，压缩比6.14x，验证准确率82.37%优于全精度基线82.20%，且各类肿瘤诊断性能保持稳定。

Conclusion: 轻量量化模型可在资源受限场景下实现临床可行的脑肿瘤筛查。
...
### [87] [D-Convexity: A Unified Differentiable Convex Shape Prior via Quasi-Concavity for Data-driven Image Segmentation](https://arxiv.org/abs/2605.19210)
*Shengzhe Chen,Hao Yan*

Main category: cs.CV

TL;DR: 提出一种基于拟凹性的无阈值凸性先验，通过可微不等式约束网络输出的所有超水平集为凸集，并设计凸梯度投影模块（CGPM）将其集成到分割网络中。


<details>
  <summary>Details</summary>
Motivation: 现有分割网络难以有效施加凸性先验，而传统方法依赖于阈值或离散约束，缺乏可微性。

Method: 利用拟凹性将全局形状约束转化为零阶、一阶和二阶可微局部不等式，导出中点凸化算法、梯度支撑超平面条件及切平面二次型不等式，并构造卷积损失和CGPM模块。

Result: 在多个数据集上持续保持凸性并改善形状规则性，超越专用视网膜分割网络和现有形状感知方法，同时统一了多种经典凸形状模型。

Conclusion: 拟凹性框架为可微凸性约束提供了统一理论，能有效改进分割网络的几何正则性。
...
### [88] [Smartphone-based Circular Plot Sampling for Forest Inventory](https://arxiv.org/abs/2605.19213)
*Su Sun,Jui-Cheng Chiu,Nabin Khanal,Songlin Fei,Yingjie Victor Chen*

Main category: cs.CV

TL;DR: 提出了一种基于智能手机的森林测量方法，通过单次行走视频实现圆形样地内树木的胸径和位置测量，精度接近传统方法，成本低且操作简单。


<details>
  <summary>Details</summary>
Motivation: 传统森林测量方法成本高或劳动强度大，缺乏可扩展性和可及性，需要开发一种低成本、易操作的解决方案。

Method: 结合预训练的单目深度估计和树木实例分割与SLAM框架，联合优化相机轨迹和深度，通过融合SLAM位姿与分割深度图恢复树木位置和DBH，并使用标定参考长度确定绝对尺度。

Result: 在管理林地和天然林地中，平均绝对误差分别为1.51厘米（MARE 3.98%）和2.30厘米（MARE 5.69%），在不同起始方向和位置下表现一致，交叉视频分析显示稳定的树木定位重复性。
...
### [89] [Rotation-Aligned Key Channel Pruning for Efficient Vision-Language Model Inference](https://arxiv.org/abs/2605.19218)
*Beomseok Kang,Dongwon Jo,Jiwon Song,Donghwee Son,Jae-Joon Kim*

Main category: cs.CV

TL;DR: 提出RotateK框架，通过旋转基的通道剪枝，在固定KV缓存预算下保留更多视觉令牌，提升视觉语言模型的细粒度感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有令牌剪枝方法因永久丢弃视觉内容而损害细粒度感知，而通道剪枝虽能保留更多令牌但面临结构化与效率的权衡。

Method: 采用在线PCA旋转将令牌依赖的通道重要性对齐到共享低维子空间，实现结构化头部掩码剪枝，并设计融合Triton注意力核以高效解码。

Result: 在两个主流VLM骨干上，RotateK在准确率和解码延迟上均优于现有通道剪枝方法，联合令牌-通道剪枝在匹配KV缓存预算下超越纯令牌基线。

Conclusion: 旋转基的通道剪枝有效解决了结构化与效率的权衡，提升了KV缓存受限下VLMs的性能与效率。
...
### [90] [Robust Mitigation of Age-Dependent Confounding Effects via Sample-Difficulty Decorrelation](https://arxiv.org/abs/2605.19230)
*Nikhil Cherian Kurian,Victor Caquilpan Parra,Abin Shoby,Luke Whitbread,Lyle J. Palmer*

Main category: cs.CV

TL;DR: 提出一种鲁棒框架，通过去相关年龄与标签条件难度趋势来减少医学图像分类中的年龄相关性能差异，而非强制年龄不变性。


<details>
  <summary>Details</summary>
Motivation: 年龄作为混杂因素导致过度诊断和诊断不足，且年龄分布变化加剧差距，传统方法可能抑制诊断信息。

Method: 预训练后，以标签条件方式建模样本难度和年龄趋势；通过Huber加权亲和权重去相关年龄与主要难度趋势，并引入年龄覆盖分数来根据小批量年龄方差缩放惩罚。

Result: 在两个放射学数据集上，该方法减少了年龄相关的真假阳性差异，AUC影响极小，并对年龄分布偏移具有鲁棒性。

Conclusion: 所提框架能有效缓解年龄作为混杂因素导致的诊断不公平，同时保留诊断相关年龄信息。
...
### [91] [HAVEN: Hierarchically Aligned Multimodal Benchmark for Unified Video Understanding](https://arxiv.org/abs/2605.19223)
*Mengqi Shi,Haopeng Zhang*

Main category: cs.CV

TL;DR: 提出了HAVEN基准，通过层级对齐的多模态数据集评估MLLMs在视频摘要与推理中的能力，发现模型在文本流畅性与多模态理解之间存在差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏跨模态的层级对齐标注，无法有效评估MLLMs对复杂叙事的忠实摘要与推理能力。

Method: 构建了细粒度（帧、镜头、视频级）且全模态（视频+文本）对齐的数据集，并设计了包含摘要、时序推理、多模态定位和显著性排序的综合评估套件。

Result: 主流MLLMs在文本流畅性上表现良好，但在多模态理解与层级对齐任务中存在显著不足。

Conclusion: HAVEN提供一个标准化测试平台，推动可解释的层级视频理解研究，弥补了传统QA格式的评估缺陷。
...
### [92] [PhyWorld: Physics-Faithful World Model for Video Generation](https://arxiv.org/abs/2605.19242)
...
TL;DR: 提出PhyWorld，一种通过两阶段后训练提升视频生成世界模型的物理一致性方法，在视频延续和物理合理性上超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型作为世界模拟器时，生成的视频延续缺乏物理保真度，无法可靠用于物理AI训练。

Method: 第一阶段使用流匹配微调优化视频到视频的延续；第二阶段通过直接偏好优化（DPO）基于物理偏好对对齐物理原则。

Result: 在VBench上平均得分0.769（基线最高0.756），在物理保真基准上平均得分3.09（基线最高2.99）。

Conclusion: 后训练大视频生成模型结合延续和物理偏好信号，可使其更有效的物理AI世界模拟器。
...
### [93] [Structuring Open-Ended NAS: Semi-Automated Design Knowledge Structuring with LLMs for Efficient Neural Architecture Search](https://arxiv.org/abs/2605.19247)
*Yuiko Sakuma,Masakazu Yoshimura,Marcel Gröpl,Zitang Sun,Junji Otsuka,Atsushi Irie,Takeshi Ohashi*

Main category: cs.CV

TL;DR: 提出FairNAD方法，利用大语言模型构建结构化搜索空间并通过公平采样与多类型变异高效探索，在多个数据集上取得SOTA性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有NAS方法搜索空间受限，LLM辅助方法探索低效且设计有偏。

Method: 定义高层结构模板，由LLM通过分析论文填充，形成结构化搜索空间；提出FairNAD，包含公平采样、Pareto感知变异、LLM迭代变异和细粒度反馈的多类型变异机制。

Result: 在CIFAR-10、CIFAR-100和ImageNet16-120上分别比当前最优方法提升0.84、2.17、2.35个百分比。

Conclusion: 结构化设计知识与公平探索策略能有效提升NAS性能。
...
### [94] [Distribution Matching Distillation without Fake Score Network](https://arxiv.org/abs/2605.19256)
*Youngjoong Kim,Deokyeong Lee,Jaesik Park*

Main category: cs.CV

TL;DR: 提出了一种无需额外假分数网络的分布匹配蒸馏方法（FSF-DMD），通过利用流映射生成器自身的伪速度来替代传统的假分数估计器，简化了生成模型的训练流程。


<details>
  <summary>Details</summary>
Motivation: 现有DMD方法需要额外的假分数网络来跟踪生成分布，带来内存和更新开销，研究旨在避免这种显式追踪器。

Method: 利用流映射生成器的端点伪速度作为假速度估计的代理，推导出实际目标函数，结合流映射一致的后向模拟和自教师变体。

Result: 在ImageNet-1K 256×256实验中，FSF-DMD提升了流映射基线性能，比DMD2在流映射初始化设置中FID更低，并在流匹配初始化和从头训练中保持有效。

Conclusion: FSF-DMD有效简化了DMD框架，无需额外假分数网络，同时保持或提升了生成质量。
...
### [95] [FPED: A Functional-Network Prior-Guided Mixture-of-Experts Framework for Interpretable Brain Decoding](https://arxiv.org/abs/2605.19279)
*Yudan Ren,Pengcheng Shi,Zihan Ma,Xiaowei He,Xiao Li*

Main category: cs.CV

TL;DR: 提出了一种基于功能网络先验的混合专家（MoE）框架FPED，用于从fMRI信号中重建视觉图像，在0.68B参数下取得竞争性语义重建性能，并提供了神经科学的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将fMRI信号摊平为一维向量，破坏了大脑的网络拓扑结构，且忽视了不同功能网络在高阶视觉语义处理中的协同作用。

Method: FPED框架将不同功能脑网络建模为专门的专家，并通过自适应路由机制捕捉它们对视觉语义理解的互补贡献。

Result: FPED在0.68B参数下实现了高度竞争性的语义重建性能，学习到的路由动态揭示了功能网络与模态特定语义处理之间的生物学对应关系。

Conclusion: 脑网络感知的专家建模为连接神经解码和生物启发式人工智能提供了有前景的方向。
...
### [96] [What Makes Synthetic Data Effective in Image Segmentation](https://arxiv.org/abs/2605.19289)
*Jinjin Zhang,Xiefan Guo,Yizhou Jin,Nan Zhou,Di Huang*

Main category: cs.CV

TL;DR: 该论文提出SENSE框架，利用扩散模型生成的合成数据增强视觉分割任务，系统分析了密集场景组合和高实例保真度的优势。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型生成的光写实图像在复杂视觉分割任务中的潜力未被充分探索。

Method: 系统分析合成图像特征，设计模型无关的SENSE框架，支持DPT和Mask2Former等架构，利用灵活可扩展的合成数据提升分割性能。

Result: 在Cityscapes、COCO和ADE20K数据集上验证了有效性和泛化能力，合成数据产生更可分的空间表征。

Conclusion: 合成数据（特别是密集场景组合和细粒度实例）能显著提升分割性能，且SENSE框架具有模型无关性和可扩展性。
...
### [97] [iGSP:Implicit Gradient Subspace Projection for Efficient Continual Learning of Vision-Language Models](https://arxiv.org/abs/2605.19301)
*Xuezhi Cui,Dongbo Zhou,Wang Guo,Zeyuan Wang,Ziyu Li,Gaozhi Zhou,Xian Li,Ling Zhao,Wentao Yang,Chao Tao,Haifeng Li*

Main category: cs.CV

TL;DR: 提出iGSP框架解决视觉语言模型持续学习中的负迁移问题，通过隐式梯度子空间投影实现高效参数共享，在MTIL基准上达到SOTA精度并减少42.7%可训练参数。


<details>
  <summary>Details</summary>
Motivation: 现有基于相似性驱动的任务共享机制错误地将表面视觉相似性与对齐一致性等同，导致视觉相似但逻辑不同的任务间出现严重负迁移。

Method: 两阶段隐式梯度子空间投影方法：子空间识别阶段通过MoE路由器建立基础，施加子空间约束正则化投影新任务梯度，利用路由概率精确剪枝冗余维度；正交子空间微调阶段固定结构基础并移除正则化，快速拟合任务残差损失。

Result: 在MTIL基准上取得SOTA准确率，平均可训练参数减少42.7%，最终总参数减少86.9%。

Conclusion: 将对齐共享视为几何问题，通过隐式子空间投影实现参数高效持续学习，验证了低维子空间中优化轨迹重叠的可重用性。
...
### [98] [MMGS: 10$\times$ Compressed 3DGS through Optimal Transport Aggregation based on Multi-view Ranking](https://arxiv.org/abs/2605.19304)
*Beizhen Zhao,Sicheng Yu,Ziran Yin,Dongxu Shen,Hao Wang*

Main category: cs.CV

TL;DR: 提出一种基于最优传输的3D高斯分布匹配框架，在仅用10%基元和10倍训练加速下实现与原始3DGS相当的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS压缩方法依赖局部采样或固定剪枝阈值，难以兼顾冗余消除与高保真渲染。

Method: 多视角高斯贡献排序、基于最优传输的合并与稠密化算子。

Result: 仅用10%基元，训练速度提升10倍，渲染质量达到最优。

Conclusion: 全局几何分布匹配能有效平衡3DGS的高效压缩与高质量渲染。
...
### [99] [MetaRA: Metamorphic Robustness Assessment for Multimodal Large Language Model-based Visual Question Answering Systems](https://arxiv.org/abs/2605.19307)
*Quanxing Xu,Yuhao Tian,Ling Zhou,Xian Zhong,Xiaohua Huang,Rubing Huang,Chia-Wen Lin*

Main category: cs.CV

TL;DR: 提出了一种名为MetaRA的测试框架，利用蜕变关系系统性地评估多模态大语言模型在视觉问答中的鲁棒性，揭示了传统指标无法发现的脆弱模式。
...
Method: 基于蜕变测试，通过定义蜕变关系生成图像-问题输入的受控变体，评估模型在不同条件下的表现。

Result: 在多种MLLM-based VQA模型上发现了语言扰动敏感、过度依赖浅层视觉线索和更深层次的多模态推理缺陷。

Conclusion: MetaRA提供了比传统准确率更丰富的诊断信息，是一种可扩展、与模型无关的鲁棒性评估方法，有助于可信多模态AI的发展。
...
### [100] [SWEET: Sparse World Modeling with Image Editing for Embodied Task Execution](https://arxiv.org/abs/2605.19319)
*Yiren Song,Yihan Wang,Xiyao Deng,Zhuoran Yan,Mike Zheng Shou*

Main category: cs.CV

TL;DR: 本文提出SWEET框架，利用图像编辑模型替代视频生成模型，通过逐步编辑生成任务相关的操作关键帧，并转换为机器人可执行动作，显著提升效率和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型用于机器人控制时计算开销大且对许多操作任务冗余，而任务进展可通过少量关键状态概括。

Method: 采用一次性的稀疏视觉规划框架SWEET，结合图像编辑模型FLUX-Kontext连续生成关键帧，并通过目标条件扩散动作预测器将相邻关键帧映射为动作。

Result: 在DROID和RoboMimic数据集上，SWEET在已知和未知场景下显著提升关键帧预测性能，并实现从规划到执行的完整流程。

Conclusion: 图像编辑模型作为一种轻量级替代方案，在机器人视觉预测中具有广阔应用前景。
...
### [101] [TextAlign: Preference Alignment for Text Rendering with Hierarchical Rewards](https://arxiv.org/abs/2605.19320)
*Mingxuan Cui,Jingpu Yang,Fengxian Ji,Qian Jiang,Zhecheng Shi,Jiaming Wang,Zirui Song,Fajri Koto,Xiuying Chen*

Main category: cs.CV

TL;DR: 提出TextAlign框架，通过层次化视觉语言模型奖励进行偏好对齐，在不改变生成器架构的情况下提升文本渲染准确性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在文本渲染上存在不足，而前方法依赖架构修改，难以推广到不同基础模型。

Method: 采用层次化VLM奖励，分解渲染错误为全局、单词和字形级别，将缺陷判断转换为标量偏好信号，支持GRPO和DPO优化。

Result: 在FLUX.1-dev和Z-Image-Turbo上，OCR文本准确性提升且不损害生成质量，优于SD3.5、AnyText等基线。

Conclusion: 奖励设计是改进文本渲染的可扩展替代方案，无需模型重构。
...
### [102] [DynaTok: Temporally Adaptive and Positional Bias-Aware Token Compression for Video-LLMs](https://arxiv.org/abs/2605.19322)
*Minyoung Park,Taehun Kong,Sangjun Ahn*

Main category: cs.CV

TL;DR: 提出了一种无需训练的令牌压缩框架DynaTok，通过时空自适应分配预算，在90%压缩率下保持95%以上的基准精度。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型中的大量视觉令牌导致计算成本过高，现有训练-free的压缩方法存在位置偏差和短时局部性依赖，导致令牌使用冗余。

Method: DynaTok包含时间预算分配模块（基于指数移动平均动态分配帧级令牌）和空间预算分配模块（利用激活注意力图选择多样且重要的特征，并通过空间记忆减少冗余）。

Result: 在MVBench、LongVideoBench、MLVU和VideoMME四个基准上，即使压缩90%令牌，仍保留95%以上基线准确率，优于现有训练-free方法。

Conclusion: DynaTok为高效视频推理提供了原则性基础，可无缝集成现有Video-LLMs，推动实时流式视频理解发展。
...
### [103] [RE-VLM: Event-Augmented Vision-Language Model for Scene Understanding](https://arxiv.org/abs/2605.19329)
*Hanqing Liu,Mingjie Liu,Luoping Cui,Endian Lin,Donghong Jiang,Chuang Zhu*

Main category: cs.CV

TL;DR: 提出RE-VLM，首个双流视觉语言模型，结合RGB和事件流数据，在常规和挑战条件下实现鲁棒场景理解。


<details>
  <summary>Details</summary>
Motivation: 传统VLM在恶劣环境（低光、高动态范围、快速运动）下性能下降，事件相机提供补充模态但缺乏多模态数据监督。

Method: 采用并行RGB和事件编码器结合渐进训练策略，并设计图驱动管道从RGB-事件流生成场景图，合成字幕和问答对。

Result: 在字幕和VQA基准上，RE-VLM以可比参数数量持续优于最先进的仅RGB和仅事件模型，尤其在挑战条件下提升显著。

Conclusion: 事件增强的VLM在多种真实环境中实现鲁棒的视觉语言理解。
...
### [104] [Selective, Regularized, and Calibrated: Harnessing Vision Foundation Models for Cross-Domain Few-Shot Semantic Segmentation](https://arxiv.org/abs/2605.19340)
*Junyuan Ma,Xunzhi Xiang,Wenbin Li,Qi Fan,Yang Gao*

Main category: cs.CV

TL;DR: 提出层次化示例表示适应（HERA）框架，通过选择-正则化-校准三阶段流程，无需源数据重训练即可实现跨域少样本分割，在多个基准上提升4.1 mIoU。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型在跨域少样本分割中面临两大挑战：每类标记样本少易过拟合，以及目标域分布偏移导致的不一致性和分层敏感性。

Method: 包括层次化层选择（HLS，基于示例迁移风险自适应选择最优层）、先验引导正则化（PGR，结构化局部信号）和像素级自适应校准（PAC，组合特征与交互图校正预测），仅微调<2.7%参数。

Result: 在多个CD-FSS基准上超越现有方法，平均mIoU提升超过4.1。

Conclusion: HERA有效缓解了少样本和域偏移问题，无需源数据重训练，具有高效性和泛化能力。
...
### [105] [Semantic-Enriched Latent Visual Reasoning](https://arxiv.org/abs/2605.19342)
*Tianrun Xu,Yue Sun,Qixun Wang,Jingyi Lu,Yuan Wang,Tianren Zhang,Longteng Guo,Fengyun Rao,Jing Lyu,Feng Chen,Jing Liu*

Main category: cs.CV

TL;DR: SLVR通过两阶段学习框架丰富潜在表示的语义，提升多区域推理的鲁棒性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有潜在空间推理方法依赖视觉监督，语义丰富度不足，难以支持多样区域级推理任务。

Method: 两阶段框架：第一阶段在细粒度属性监督下学习语义增强的区域潜在表示；第二阶段用M-GRPO对齐多查询潜在表示。

Result: SLVR在鲁棒性和语义一致性上优于基线方法，并构建了SLV-Set和SV-QA基准。

Conclusion: SLVR有效提升潜在视觉推理的语义丰富性和性能。
...
### [106] [MAM-CLIP: Vision-Language Pretraining on Mammography Atlases for BI-RADS Classification](https://arxiv.org/abs/2605.19359)
*Halil Ibrahim Gulluk,Olivier Gevaert*

Main category: cs.CV

TL;DR: 本文提出基于视觉-语言模型的乳腺X线摄影BI-RADS预测方法，通过对比学习结合影像与报告文本，并在少量标注数据上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 乳腺X线摄影图像解读存在歧义，仅靠标签训练模型性能有限。

Method: 收集2313张乳腺X线摄影图像及对应图注，使用预训练PubMedBERT作为语言模型，通过对比学习训练图像-文本对，再对视觉编码器进行微调用于BI-RADS预测。

Result: 3类平均F1分数提升1%至14%，尤其在1K标注样本时提升14%；2K图像-文本对比2K标注样本更有效（平均+1.1%）。

Conclusion: 视觉-语言预训练能有效提升乳腺X线摄影理解，图注文本信息在标签稀缺时特别有价值，并公开了模型与数据集。
...
### [107] [Scalable, Energy-Efficient Optical-Neural Architecture for Multiplexed Deepfake Video Detection](https://arxiv.org/abs/2605.19360)
*Parnian Ghapandar Kashani,Shiqi Chen,Aydogan Ozcan*

Main category: cs.CV

TL;DR: 提出了一种混合数字-模拟深度伪造视频检测框架，结合轻量级数字前端和空间复用光学解码后端，实现15路视频并行处理，以低计算成本达到高准确率。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习检测方法计算强度和能耗高，限制了可扩展性，急需高效、可信的检测系统。

Method: 使用可编程空间光调制器进行空间复用光学解码，在单次光学传播中并行处理15个以上视频流，结合数字前端进行混合推理。

Result: 在Celeb-DF数据集上，15路视频并行处理时，平均准确率97.79%，灵敏度99.86%，特异性95.72%，且对视频退化、噪声、压缩、实验偏差和黑盒攻击具有鲁棒性。

Conclusion: 将光学计算集成到AI推理中，可同时提升吞吐量、能效和对抗鲁棒性，这些在纯数字系统中难以兼得。
...
### [108] [Multi-Scale Generative Modeling with Heat Dissipation Flow Matching](https://arxiv.org/abs/2605.19371)
*Jun Ma,Hanquan Zhang,Yanjun Qin,Haoyuan Guan,Ke Zhang*

Main category: cs.CV

TL;DR: 提出了热耗散流匹配 (HDFM)，将连续的模糊（热耗散）过程引入流匹配框架，以注入多尺度先验，并通过插值热耗散路径和x预测解决病态和高维回归难题。


<details>
  <summary>Details</summary>
Motivation: 现有的模糊基扩散模型局限于SDE框架，未集成到ODE框架（如流匹配）中；且噪声基模型在色彩预算和多尺度细节上不如模糊基模型。

Method: 在流匹配中引入连续模糊热耗散过程；使用插值热耗散路径解决病态问题；采用x预测缓解高维回归困难。

Result: 玩具实验和消融研究表明HDFM从模糊和x预测中持续获益；其在所有数据集上优于多数基线方法。

Conclusion: HDFM有效结合了模糊多尺度先验与流匹配框架，显著提升了图像生成性能。
...
### [109] [Concept-Guided Noisy Negative Suppression for Zero-Shot Classification and Grounding of Chest X-Ray Findings](https://arxiv.org/abs/2605.19374)
*Chenyu Lian,Hong-Yu Zhou,Chun-Ka Wong,Jing Qin*

Main category: cs.CV

TL;DR: 提出CoNNS框架，通过概念引导的噪声负样本抑制，提升胸部X光零样本理解任务性能。


<details>
  <summary>Details</summary>
Motivation: 标准对比学习将不同患者的X光和报告视为负对，引入噪声负样本，导致语义模糊，影响零样本任务表现。

Method: 构建层次化概念本体（41个临床概念），并实施跨患者对重标记策略（细粒度分解、噪声负样本过滤、难负样本挖掘），结合概念感知NCE损失抑制噪声。

Result: 在多粒度零样本定位和5个零样本分类数据集上，CoNNS优于现有最先进模型。

Conclusion: CoNNS有效解决噪声负样本问题，提升零样本理解准确性。
...
### [110] [Sparse Mixture-of-Experts Routing in Visual Diffusion Transformers:Diagnosis, Boundary Calibration and Evolutionary Roadmap from Routing Collapse to Selective Deadlock](https://arxiv.org/abs/2605.19378)
*Haiying Sha*

Main category: cs.CV

TL;DR: 本文系统诊断了Token-Choice稀疏MoE在视频扩散Transformer中的训练失败模式，提出了功能性冗余假说，总结了密集到MoE转换的三定律，并提供了bfloat16精度陷阱的解决方案。


<details>
  <summary>Details</summary>
Motivation: 解决Token-Choice混合专家模型在视频扩散Transformer训练中出现的多种失效模式，特别是专家同质化和路由死锁问题。

Method: 从预训练密集模型转换到MoE架构，遵循三定律（路由专家克隆FFN权重、共享专家初始化为零或极小噪声、门控网络随机初始化），并通过路由决策时间序列分析提出功能性冗余假说。

Result: 发现五种失效模式：线性路由器的全局饱和、MLP路由器的选择性死锁、交叉注意力的部分恢复、死锁层的U形分布及bfloat16精度截断。

Conclusion: 死锁是共享专家成熟前的理性等待策略，当前Token-Choice范式能力有限，需通过三步进化路线向世界模型发展。
...
### [111] [MatPhys: Learning Material-Aware Physics Parameters for Deformable Object Simulation from Videos](https://arxiv.org/abs/2605.19386)
*Yang Yang,Yiyan Wang,Zheming Liu,Naoya Iwamoto*

Main category: cs.CV

TL;DR: 提出了MatPhys，一种从单视角视频前馈预测弹簧质点参数的物理感知框架，通过语义分解和材料码本实现跨场景一致的材料参数估计。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动方法假设均质材料且跨场景参数不一致，限制了可变形物体物理数字孪生的重建。

Method: 利用DINO特征进行语义部件分解并查询部件级材料先验，引入学习材料码本作为外观与物理的桥梁，并使用时序约束解码器保证参数一致性。

Result: 在重建与未来预测任务中匹配逐场景优化基线，并在未见交互和物体上展现出更强的泛化能力与参数一致性。

Conclusion: 将欠约束的单目问题转化为基于共享可复用材料概念的前馈推理，显著提升了材料参数跨场景的鲁棒性。
...
### [112] [LMM-Track4D: Eliciting 4D Dynamic Reasoning in LMMs via Trajectory-Grounded Dialogue](https://arxiv.org/abs/2605.19390)
*Chaoyue Li,Yongxue Xu,Jie Feng,Jiayu Ding*

Main category: cs.CV

TL;DR: 提出了Track4D-Bench基准和LMM-Track4D模型，用于增强大型多模态模型在4D动态推理中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有LMMs在图像和视频理解上表现良好，但难以处理持续的4D时空动态推理。

Method: 结合了射线-时间几何编码（RTGE）、专用流状态令牌TRK以及对象-槽运动学残差锚定解码器（OSK-RA）。

Result: 在Track4D-Bench上显著优于强基线模型。

Conclusion: 显式动态状态建模是提升LMMs 4D动态推理的有效设计原则。
...
### [113] [Neuron Incidence Redistribution for Fairness in Medical Image Classification](https://arxiv.org/abs/2605.19393)
*Abin Shoby,Lyle John Palmer,Nikhil Cherian Kurian*

Main category: cs.CV

TL;DR: 本文提出了一种轻量级正则化方法NIR，通过重新分配神经元激活来减少深度学习医疗图像分类中的群体性能差异。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医疗图像分类中，对不同人口统计群体（年龄、性别、种族）存在系统性诊断偏差，表现为对优势群体过度诊断、对劣势群体诊断不足。

Method: 提出神经元发生重分布（NIR），通过惩罚隐藏层神经元预测概率加权平均激活的方差，优化潜在表征分布，训练时不需人口统计标签。

Result: 在HAM10000数据集上，年龄组TPR差异从10.81%降至0.93%，性别差异从12.04%降至0.74%，AUC提升0.51点；在Harvard OCT-RNFL上，种族FPR差异从15.68%降至10.66%，年龄从12.69%降至1.80%。

Conclusion: 将潜在疾病证据均匀分布在全连接层是实现医疗AI人口公平性的有效策略。
...
### [114] [Rebalancing Reference Frame Dominance to Improve Motion in Image-to-Video Models](https://arxiv.org/abs/2605.19398)
*Wooseok Jeon,Seungho Park,Seunghyun Shin,Sangeyl Lee,Hyeonho Jeong,Hae-Gon Jeon*

Main category: cs.CV

TL;DR: 提出DyMoS方法，通过重新平衡注意力路径，在不改变输入图像和模型权重的情况下，提升图像到视频生成中的动态性，同时保持视觉质量和参考图像保真度。


<details>
  <summary>Details</summary>
Motivation: 图像到视频模型生成的视频过于静态，削弱信号的方法需额外训练或牺牲保真度。

Method: 识别参考帧主导机制，提出训练无关的DyMoS方法，在初始去噪步骤中重平衡生成帧对参考帧的注意力，通过单一标量参数控制运动强度。

Result: 在多个模型上验证，DyMoS持续提升运动动态性，同时保持视觉质量和参考图像保真度。

Conclusion: DyMoS有效解决了静态问题，是一种简单、可泛化的运动增强方法。
...
### [115] [CADENet: Condition-Adaptive Asynchronous Dual-Stream Enhancement Network for Adverse Weather Perception in Autonomous Driving](https://arxiv.org/abs/2605.19837)
*Sherif Khairy,Catherine M. Elias*

Main category: cs.CV

TL;DR: CADENet是一种无需训练的异步双流增强网络，能够在恶劣天气下实现低延迟目标检测，并通过零样本分类适应新天气，同时暴露了现有数据集的标注完整性偏差。


<details>
  <summary>Details</summary>
Motivation: 现有增强-检测方法延迟高，违反安全关键感知循环的实时性要求；且标注存在缺陷：增强后恢复的目标无法被标注者看到，导致F1评分失真。

Method: 三线程系统：线程S（YOLOv11n）无延迟实时检测；线程Q进行条件自适应增强（CAPE）并通过熵引导NMS融合结果；线程E使用CLIP零样本天气分类。

Result: 在DAWN数据集上，Recall=0.0103（微平均），雪天F1=0.0230，雨天F1=0.0038；线程S维持约44 FPS，无需重训练或额外硬件。

Conclusion: CADENet证明了低延迟增强在安全关键系统中的可行性，并揭示了标注完整性偏好的影响；Recall是更鲁棒的指标，F1仅为增益的下界。
...
### [116] [Vision Harnessing Agent for Open Ad-hoc Segmentation](https://arxiv.org/abs/2605.19410)
*Zilin Wang,Stella X. Yu*

Main category: cs.CV

TL;DR: 提出VASA，首个基于视觉引导的开放即时分割智能体，通过工作记忆和视觉操作实现无需训练的分割。


<details>
  <summary>Details</summary>
Motivation: 现有分割方法依赖已知概念，难以处理无预设掩码的开放即时分割任务（需通过部分、关系等证据构造）。

Method: 结合VLM智能体、分割基础模型和视觉导向工作流，利用持久工作掩码进行推理、构造、验证和错误恢复。

Result: 在PARS基准上领先SAM3 Agent 14-25%，在RefCOCOm上提升5-9%，超越其他基线最多20%。

Conclusion: 验证了智能体通过任务知识、视觉例程、工作记忆和错误感知工作流进行视觉构造的有效性。
...
### [117] [KappaPlace: Learning Hyperspherical Uncertainty for Visual Place Recognition via Prototype-Anchored Supervision](https://arxiv.org/abs/2605.19435)
*Maya Yanko,Yoli Shavit*

Main category: cs.CV

TL;DR: 提出KappaPlace框架，通过原型锚定监督策略和von Mises-Fisher分布建模，实现视觉地点识别中的不确定性校准，降低预期校准误差达50%。


<details>
  <summary>Details</summary>
Motivation: 现有VPR方法缺乏校准的不确定性估计，无法可靠检测查询模糊性或匹配错误，在安全关键型机器人应用中风险较高。

Method: 采用原型锚定监督策略，将图像描述符建模为vMF变量，预测浓度参数作为认知不确定性代理，并提出匹配级不确定性量化方法。

Result: 在五个基准测试上，KappaPlace将预期校准误差降低高达50%，同时保持或提升检索召回率。

Conclusion: KappaPlace提供鲁棒且校准良好的不确定性信号，支持VPR管道中的可靠决策，兼具联合训练和冻结骨干的后训练扩展能力。
...
### [118] [World-Ego Modeling for Long-Horizon Evolution in Hybrid Embodied Tasks](https://arxiv.org/abs/2605.19957)
*Zuyao Lin,Jianhui Zhang,Peidong Jia,Xiaoguang Zhao,Shanghang Zhang,Xingyu Chen*

Main category: cs.CV

TL;DR: 本文提出世界-自我建模范式，将未来演化分解为世界和自我组件，并实例化为世界-自我模型（WEM），在混合导航与操作任务上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型将世界与自我纠缠在一起，导致长时域混合任务（如导航与操作交织）退化。

Method: 定义运动、语义、意图三种世界-自我边界视角，分析后解耦、前解耦、全解耦策略；构建WEM，包含隐式分离的世界-自我规划器和级联并行混合专家扩散生成器。

Result: WEM在HTEWorld基准上达到最先进性能，在纯操作基准上保持竞争力。

Conclusion: 世界-自我建模范式有效提升长时域混合任务的世界建模能力。
...
### [119] [Targeted Downstream-Agnostic Attack](https://arxiv.org/abs/2605.19446)
*Zhuxin Lei,Ziyuan Yang,Yi Zhang*

Main category: cs.CV

TL;DR: 提出了一种针对预训练编码器的目标性下游无关攻击方法，通过引入威胁图像和特定样本的对抗性扰动来攻击编码器。


<details>
  <summary>Details</summary>
Motivation: 现有的下游无关攻击方法缺乏目标性，且威胁模型过于宽松，本研究的动机是提出更严格的威胁模型下的目标性攻击方法。

Method: 引入威胁图像作为目标特征锚点，设计生成器为每个样本产生特定的对抗性扰动，使编码器对对抗样本和威胁图像输出相同特征。

Result: 在10种自监督方法和3个基准数据集上验证了方法的有效性，揭示了预训练编码器的显著脆弱性。

Conclusion: 该方法通过特定样本扰动和威胁图像锚点，成功实现了目标性下游无关攻击，并证明预训练编码器存在安全隐患。
...
### [120] [Thinking in Scales: Accelerating Gigapixel Pathology Image Analysis via Adaptive Continuous Reasoning](https://arxiv.org/abs/2605.19491)
...
TL;DR: PathCTM通过动态序列推理和尺度切换，实现了高效的全切片图像分析，大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法处理高倍率图像块计算成本高，限制了效率和可扩展性。

Method: 采用条件计算、注意力引导区域剪枝和置信度感知早停，实现从低倍率到高倍率的动态推理。

Result: 与MIL方法相比，所需图像块减少95.95%，推理时间缩短95.62%，AUC无下降。

Conclusion: PathCTM在保持诊断性能的同时显著提升效率，适用于千兆像素WSI分析。
...
### [121] [CutVerse: A Compositional GUI Agents Benchmark for Media Post-Production Editing](https://arxiv.org/abs/2605.19484)
*Haobo Hu,Xiangwu Guo,Zhiheng Chen,Difei Gao,Haotian Liu,Libiao Jin,Qi Mao*

Main category: cs.CV

TL;DR: 本文提出Cutverse基准测试，系统评估GUI代理在专业媒体后期制作中的能力，发现现有代理在复杂的长期任务中成功率仅为36.0%。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理在网页导航等基础任务上表现良好，但在专业创意工作流（如媒体后期制作）中的能力尚未被充分探索。

Method: 构建Cutverse基准，包含7款专业应用的186个复杂长期任务，并使用轻量级解析器将屏幕录制和交互日志转为结构化GUI动作轨迹。

Result: 现有代理在真实媒体编辑任务中成功率为36.0%，虽展现出空间定位和多模态对齐能力，但长期可靠性和领域特定规划仍有限。

Conclusion: Cutverse揭示了长期、复杂的媒体后期制作工作流对现有GUI代理的挑战，指出了当前模型的主要局限性。
...
### [122] [EventPrune: Cascaded Event-Assisted Token Pruning for Efficient First-Person Dynamic Spatial Reasoning](https://arxiv.org/abs/2605.19506)
*Pengtao Ma,Ziliang Zhou,Ciyu Ruan,Haoyang Wang,Kaiyuan Li,Zihang Gong,Wenhua Ding,Chen Gao,Jingao Xu,Xinlei Chen*

Main category: cs.CV

TL;DR: 提出事件级联剪枝（ECP），首个无需训练的框架，利用事件相机的高频运动线索指导视觉令牌选择，在减少80%令牌的情况下提升性能并加速推理。


<details>
  <summary>Details</summary>
Motivation: Transformer视频大模型在动态空间推理中因二次注意力机制导致密集视觉令牌计算昂贵，现有剪枝方法依赖离散静态快照，丢失了运动和几何线索。

Method: ECP包含三个阶段：事件触发因果采样锚定运动信息关键帧、事件引导运动显著性过滤抑制非运动令牌、事件注意力排名融合校准空间注意力。

Result: 在80%令牌减少下，ECP超越全令牌基线（37.62% vs. 36.31%），实现1.89倍推理加速和52% GFLOPs减少；在真实世界RGB事件基准ESR-Real上准确率提升2.68个百分点。

Conclusion: ECP有效利用事件相机连续运动先验进行令牌剪枝，提升动态空间推理效率与准确性，且无需训练。
...
### [123] [Are Watermarked Images Editable? SafeMark for Watermark-Preserving Text-Guided Image Editing](https://arxiv.org/abs/2605.19511)
*Xiaodong Wu,Qi Li,Xiangman Li,Zelin Zhang,Lingshuang Liu,Jianbing Ni*

Main category: cs.CV

TL;DR: 提出SafeMark框架，在文本引导图像编辑中保持水印完整性，通过集成水印解码损失实现高比特精度和高质量语义编辑。


<details>
  <summary>Details</summary>
Motivation: 探索带水印图像在编辑时能否保持水印完整性这一未充分研究的问题。

Method: 在扩散编辑器训练目标中加入阈值化水印解码损失，微调编辑器以在语义有效编辑的同时保留水印。

Result: 跨多个数据集和编辑方法，SafeMark在保持高质量语义编辑的同时实现高水印比特精度，且对常见后编辑失真具有鲁棒性。

Conclusion: 语义可编辑性与水印完整性本质上是兼容的，可实现生成编辑管道中的可信图像溯源。
...
### [124] [Return of Frustratingly Easy Unsupervised Video Domain Adaptation](https://arxiv.org/abs/2605.19510)
*Pengfei Wei,Yiqun Sun,Zhiqiang Xu,Yiping Ke,Lawrence B. Hsieh*

Main category: cs.CV

TL;DR: 提出了一种简单有效的无监督视频域适应方法MetaTrans，通过分离处理时空差异，在跨域动作识别任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 无监督视频域适应（UVDA）在实际中很重要但研究不足，现有方法复杂且效果有限。

Method: 采用仅含两个损失项的简洁学习目标，并设计时间-静态减法模块，分离处理视频的时空差异。

Result: 在多种跨域动作识别任务中，与最先进UVDA基线相比，取得了显著的绝对性能提升和相对性能增益。

Conclusion: MetaTrans通过简单架构实现高效UVDA，验证了分离处理时空差异的有效性。
...
### [125] [iDiff: Interpretable Difference-aware Framework for Pairwise Image Quality Assessment](https://arxiv.org/abs/2605.19522)
*Xinli Yue,JianHui Sun,Tao Shao,Liangchao Yao,Fan Xia,Yuetang Deng*

Main category: cs.CV

TL;DR: 提出iDiff框架，通过显式差异建模和双分支设计，在成对图像质量评估中同时实现偏好预测和理由生成，并在NTIRE 2026 RAIM挑战中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 专业摄影中的成对图像质量评估需要模型不仅识别偏好图像，还要提供有说服力且基于图像的理由，现有方法无法同时满足这两点。

Method: 采用双分支设计：答案模型通过左右/全局局部视图分解、内容感知专化（人物/场景）和集成聚合进行偏好预测；思考模型利用专家模板、多源质量特征和答案感知监督生成理由。

Result: 在准确性和推理质量指标上均有效，并在NTIRE 2026 RAIM挑战中获得第一名。

Conclusion: 集成显式差异建模与结构化多模态推理能有效提升成对IQA的鲁棒性和可解释性。
...
### [126] [Dual-Prompt CLIP with Hybrid Visual Encoders for Occluded Person Re-Identification](https://arxiv.org/abs/2605.19527)
*Zhangjian Ji,Shaotong Qiao,Kai Feng,Wei Wei*

Main category: cs.CV

TL;DR: 提出DPL-ReID模型，利用双提示学习和真实遮挡增强模块，有效应对行人重识别中的遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型方法忽略了遮挡物语义信息，导致遮挡下匹配性能下降。

Method: 双提示学习策略（Dual-PL）捕捉完整行人语义，真实世界遮挡增强（RWOA）模拟遮挡场景，加权门控特征融合（WGFF）利用LSNet优化视觉特征。

Result: 在多个遮挡ReID数据集上达到最先进性能。

Conclusion: DPL-ReID通过语义提示和遮挡增强显著提升了遮挡行人重识别的鲁棒性。
...
### [127] [Boosting Text-to-Image Diffusion Models via Core Token Attention-Based Seed Selection](https://arxiv.org/abs/2605.19532)
*Yunzhe Zhang,Hongfu Liu,Pengyu Hong*

Main category: cs.CV

TL;DR: 提出了一种基于注意力机制的种子选择方法(ABSS)，通过分析去噪早期阶段的注意力动态来预测并选择优质种子，无需训练即可提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型对随机种子高度敏感，不同种子导致图像质量和语义对齐差异大，但现有方法缺乏有效的种子预筛选机制。

Method: ABSS通过计算提示词核心标记在去噪初始步骤中的交叉注意力得分来评估和排序候选种子，仅保留得分最高的k个进行完整生成，无需训练或修改噪声。

Result: 在三个基准测试上，ABSS显著提升了稳定扩散变体模型的文本-图像对齐和视觉质量，人类偏好和自动指标均验证了效果。

Conclusion: ABSS是一种轻量级、即插即用的种子预筛选方法，可融入现有优化流程，实现稳定且一致的生成质量提升。
...
### [128] [Towards Camera-Robust 3D Localization: Equation-Anchored Tool-Use for MLLMs](https://arxiv.org/abs/2605.19528)
*Xueying Jiang,Wenhao Li,Quanhao Qian,Deli Zhao,Shijian Lu,Gongjie Zhang,Ran Xu*

Main category: cs.CV

TL;DR: 提出了一种基于方程锚定的工具使用框架，通过将相机内参和多点度量深度代入针孔反投影方程，显式地在思维链中计算3D坐标，从而解决了多模态大语言模型在3D定位中的相机内参模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM要么忽略相机参数而过拟合到规范训练内参，要么借助外部工具获取深度但仅作为参考线索，无法确定性传播相机信息，导致3D定位性能受限于相机内参模糊性。

Method: 设计方程锚定工具框架：主动检索相机内参并采样多点度量深度，在CoT中显式写出针孔反投影方程并代入工具输出，最终回归9-DoF边界框。

Result: 在0.5×至1.5×缩放相机内参的3D目标检测和3D视觉定位任务上，方法优于纯RGB和工具增强基线，在相机偏离训练尺度最大的场景中提升显著。

Conclusion: 通过方程显式计算可确定性传播相机信息，有效解决相机内参模糊性，提升MLLM在跨相机场景下的3D定位鲁棒性。
...
### [129] [Replacement Learning: Training Neural Networks with Fewer Parameters](https://arxiv.org/abs/2605.19533)
*Yuming Zhang,Peizhe Wang,Tianyang Han,Hengyu Shi,Junhao Su,Dongzhi Guan,Jiabin Liu,Jiaji Wang*

Main category: cs.CV

TL;DR: 提出替换学习（RepL）训练范式，通过用轻量合成算子替换部分深度网络块，减少参数冗余和训练开销，同时保持甚至提升性能。


<details>
  <summary>Details</summary>
Motivation: 全深度反向传播在深网络中效率低，导致参数冗余、高显存开销和训练延迟，而简单跳层会削弱表示能力。

Method: 对于被移除的块，RepL插入轻量计算层，通过可学习变换从相邻块参数合成替代算子，并应用于前置激活，从而保持局部上下文连续性。

Result: 在CIFAR-10、ImageNet等多个基准上，RepL减少了可训练参数、GPU内存和训练时间，同时匹配或超越标准端到端训练。

Conclusion: RepL是一种通用、有效的训练范式，能减少深度网络冗余，兼容多种架构和任务，且与随机深度、量化等技术互补。
...
### [130] [CaptchaMind: Training CAPTCHA Solvers via Reinforcement Learning with Explicit Reasoning Supervision](https://arxiv.org/abs/2605.19538)
*Pengcheng Wang,Haoxiang Liu,Yang Dai,Xiangxiang Zeng,Guanhua Chen,Baotian Hu,Longyue Wang,Weihua Luo*

Main category: cs.CV

TL;DR: 本文提出了CaptchaBench基准和CaptchaMind训练框架，显著提升了CAPTCHA自动求解的性能。


<details>
  <summary>Details</summary>
Motivation: 现有CAPTCHA求解方法缺乏大规模训练数据和过程级标注，导致智能体无法完成细粒度视觉推理任务。

Method: 构建了包含16000个样本的CaptchaBench基准，并采用基于强化学习的CaptchaMind求解器，利用显式推理过程监督进行训练。

Result: CaptchaMind在八类任务上平均成功率达82.9%，在真实世界实例上达71.0%，显著优于现有方法。

Conclusion: 通过大规模标注数据和推理过程监督训练，可以有效提升CAPTCHA求解的鲁棒性和泛化能力。
...
### [131] [Trust It or Not: Evidential Uncertainty for Feed-Forward 3D Reconstruction with Trust3R](https://arxiv.org/abs/2605.19539)
*Zihao Zhu,Wenyuan Zhao,Nuo Chen,Chao Tian,Zhiwen Fan*

Main category: cs.CV

TL;DR: Trust3R 是一种轻量级的证据不确定性框架，用于前馈3D重建，通过门控残差均值精炼和正态逆Wishart证据头生成逐点多变量t分布的不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型在无约束密集几何预测中依赖启发式置信度分数，缺乏概率解释，无法可靠指示预测几何的可信度。

Method: 结合门控残差均值精炼与正态逆Wishart证据头，得到封闭形式的多元学生t分布，用于逐点几何不确定性估计。

Result: 在多个室内外基准上，Trust3R 一致改善了风险覆盖率和稀疏化性能，在ScanNet++上实现AURC降低25%、AUSE降低41%，并提升了几何精度。

Conclusion: Trust3R 为下游几何处理提供了实用的可靠性信号，不确定性估计在排名上优于MASt3R等基线方法。
...
### [132] [Self-Creative Text-to-Object Generation using Semantic-Aware Spatial Weighting](https://arxiv.org/abs/2605.19554)
*Yue Yu,Haibo Chen,Shuo Chen,Jian Yang,Jun Li*

Main category: cs.CV

TL;DR: 提出一种自创扩散模型（SCDiff），通过可学习空间加权模块和视觉语义混合损失函数，在保持语义对齐的同时增强文本到图像生成的创造性和新颖性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型优化于字面一致性，噪声预测网络限制生成高概率区域，导致输出缺乏真正的创造性。

Method: 采用可学习空间加权模块（参数化Kaiser-Bessel窗口增强中心特征）和视觉语义混合损失（相似性损失保持语义对齐，多样性损失最大化区别于原图）。

Result: 实验表明模型在创造性、语义一致性和视觉连贯性上显著提升。

Conclusion: 提供了一种简单而强大的框架用于生成具有创造性的图像。
...
### [133] [EpiDiffVO: Geometry-Aware Epipolar Diffusion for Robust Visual Odometry](https://arxiv.org/abs/2605.19556)
*Prateeth Rao*

Main category: cs.CV

TL;DR: 提出了一种稀疏极线匹配框架，结合扩散细化与图神经网络，在减少对应冗余的同时实现鲁棒的相对位姿估计。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的位姿估计方法依赖密集匹配或直接回归，导致冗余并降低几何可解释性。

Method: 稀疏极线匹配框架，极线扩散过程细化对应点，构建斯坦纳图经图神经网络选择子集，结合可微奇异值分解求解本质矩阵。

Result: 在TartanAir和KITTI SLAM数据集上验证，该方法在挑战性基线条件下减少了冗余，保持鲁棒位姿估计。

Conclusion: 稀疏匹配、扩散细化和图选择结合能有效降低对应冗余并提升几何一致性。
...
### [134] [EgoCoT-Bench: Benchmarking Grounded and Verifiable Operation-Centric Chain of Thought Reasoning for MLLMs](https://arxiv.org/abs/2605.19559)
*Yang Dai,Dian Jiao,Tianwei Lin,Wenqiao Zhang*

Main category: cs.CV

TL;DR: EgoCoT-Bench是一个细粒度自我中心视频基准，包含3172个可验证问答对和显式步骤推理注释，用于评估多模态大模型在操作中心推理中的时空证据一致性。


<details>
  <summary>Details</summary>
Motivation: 现有自我中心视频基准缺乏基于细粒度推理的评估，不支持显式时空证据验证，导致模型推理不可靠。

Method: 通过时空场景图(STSG)引导生成框架构建基准，再经人工精炼确保正确性、自我中心相关性和细粒度质量。

Result: 实验表明多模态模型在细粒度推理上仍困难，且常生成答案正确但证据与答案不一致的解释。

Conclusion: EgoCoT-Bench可作为自我中心视频理解中可验证推理的有效测试平台。
...
### [135] [A novel YOLO26-MoE optimized by an LLM agent for insulator fault detection considering UAV images](https://arxiv.org/abs/2605.19595)
...
TL;DR: 提出一种集成稀疏MoE模块的YOLO26-MoE模型，用于无人机电力线绝缘子故障检测，在mAP指标上超越最新YOLO版本。


<details>
  <summary>Details</summary>
Motivation: 绝缘子故障检测面临缺陷区域小、故障模式多样、背景复杂和成像条件变化等挑战。

Method: 在YOLO26的高分辨率分支中集成稀疏混合专家模块，并通过大型语言模型代理协调超参数优化与训练。

Result: 模型达到0.9900 mAP@0.5和0.9515 mAP@0.5:0.95，优于最新YOLO版本。

Conclusion: 该模型为无人机绝缘子故障检测提供了有效可靠的解决方案。
...
### [136] [Lens Privacy Sealing: A New Benchmark and Method for Physical Privacy-Preserving Action Recognition](https://arxiv.org/abs/2605.19578)
*Mengyuan Liu,Ziyi Wang,Peiming Li,Junsong Yuan*

Main category: cs.CV

TL;DR: 本文提出一种硬件级隐私保护方案Lens Privacy Sealing (LPS)，通过在镜头前添加可调散射膜实现物理不可逆的隐私遮挡，并设计MSPNet网络从退化视频中高效提取动作信息，在P3AR数据集上验证了隐私-效用的优异平衡。


<details>
  <summary>Details</summary>
Motivation: 现有RGB监控系统在采集阶段无法保护隐私，且软件方法或昂贵光学方案存在局限性，亟需一种低成本、物理层面的预传感器隐私保护方案。

Method: 1) 硬件：采用多层随机散射膜（LPS）实现物理不可逆的隐私遮盖；2) 数据：构建P3AR数据集（含NTU和PKU子集）并标注隐私属性；3) 算法：提出MSPNet，包含帧间噪声抑制器（IFNS）和跨帧语义聚合器（CFSA），结合对比语言-图像预训练增强鲁棒语义提取。

Result: MSPNet在动作识别精度上较基线提升近两倍，同时身份识别率低；LPS优于现有硬件方法，抗PSF逆推和数据驱动重建攻击，且在多种光学配置和恶劣环境下表现稳健。

Conclusion: LPS作为一种简单、低成本、可泛化的硬件隐私保护方案，结合定制化算法MSPNet，能在保证强隐私的同时实现高质量动作识别，为实际部署提供可行路径。
...
### [137] [Spectral Integrated Gradients for Coarse-to-Fine Feature Attribution](https://arxiv.org/abs/2605.19607)
*Soyeon Kim,Seongwoo Lim,Kyowoon Lee,Jaesik Choi*

Main category: cs.CV

TL;DR: 提出Spectral Integrated Gradients (SIG)，利用SVD构建从粗到细的积分路径，减少噪声，提升归因质量。


<details>
  <summary>Details</summary>
Motivation: 标准积分梯度方法因直线路径同时激活所有特征，易累积噪声，需改进路径选择。

Method: 基于基线到输入的差异矩阵进行奇异值分解，按奇异值从大到小逐步激活分量，构建积分路径。

Result: 在多个图像分类数据集上，SIG生成更清晰、噪声更少的归因图，定量性能优于现有路径方法。

Conclusion: SIG通过粗到细的路径结构有效改善了特征归因的质量。
...
### [138] [deadtrees.earth-aerial: A Multi-Resolution Aerial Image Dataset for Tree Cover and Mortality Detection](https://arxiv.org/abs/2605.19605)
...
TL;DR: 提出了两个全球尺度的公开数据集（DTE-aerial-train和DTE-aerial-bench），用于从厘米级航拍影像中联合分割树冠覆盖和树木死亡，并建立了强基准。


<details>
  <summary>Details</summary>
Motivation: 气候变化和干扰（火灾、病虫害等）威胁全球森林，亟需可扩展的树木覆盖和死亡监测方法，但缺乏全球统一的带标注数据集。

Method: 构建了385K张1024x1024像素训练图像（含伪标签）和525张全球分布基准图像（含专家标注），覆盖热带、温带、寒带和干旱生物群系，采用标准分割模型建立基线。

Result: 在基准测试中，联合分割方法显著提升各地生物群系和尺度的死亡分割效果，寒带森林F1分数从0.40提升至0.58（相对改进约45%）。

Conclusion: 这些数据集和基线模型为全球尺度树木死亡监测提供了标准化工具，并将开源以促进研究。
...
### [139] [Inverse Design of Metasurface based Absorbers using Physics Guided Conditional Diffusion Models](https://arxiv.org/abs/2605.19611)
*Vineetha Joy,Jamshed Palai,Satwik Sahoo,Anshuman Kumar,Amit Sethi,Hema Singh*

Main category: cs.CV

TL;DR: 提出了一种物理引导的条件质量增强扩散框架，用于超表面吸收器的逆向设计，能在30秒内生成满足频谱约束且可制造的几何结构。


<details>
  <summary>Details</summary>
Motivation: 传统逆向设计方法依赖全波模拟迭代优化，耗时且计算密集，而现有生成方法条件保真度低，设计存在不规则特征难以制造。

Method: 使用特征线性调制（FiLM）集成目标反射特性条件信息，并嵌入预训练代理电磁模拟器引入物理感知正则化（频谱级损失函数）。

Result: 在2-18 GHz频段实现平均频谱均方误差0.0006、频带对齐精度0.958，设计时间30秒（传统方法需数月），实验测量验证了有效性。

Conclusion: 该框架在条件准确性和设计多样性上表现优异，显著提升逆向设计效率，且生成设计具有实际可制造性。
...
### [140] [White-Balance First, Adjust Later: Cross-Camera Color Constancy via Vision-Language Evaluation](https://arxiv.org/abs/2605.19613)
*Shuwei Li,Lei Tan,Robby T. Tan*

Main category: cs.CV

TL;DR: 提出VLM-CC框架，利用视觉语言模型迭代反馈实现跨相机色彩恒常性，无需直接估计光源。


<details>
  <summary>Details</summary>
Motivation: 跨相机色彩恒常性泛化困难，现有学习模型易过拟合于训练相机的色彩响应特性。

Method: 迭代修正框架：白平衡后由LoRA微调的VLM评估残色，映射为RGB方向更新光照估计直至收敛。

Result: 在多个数据集上实现跨相机色彩恒常性的最先进鲁棒性。

Conclusion: 将色彩恒常性重构为迭代感知反馈问题，用VLM评估替代直接RGB回归可提升泛化能力。
...
### [141] [FineBench: Benchmarking and Enhancing Vision-Language Models for Fine-grained Human Activity Understanding](https://arxiv.org/abs/2605.19846)
*Gueter Josmy Faure,Min-Hung Chen,Jia-Fong Yeh,Hung-Ting Su,Winston H. Hsu*

Main category: cs.CV

TL;DR: 提出了细粒度人类中心视频理解基准FineBench和增强框架FineAgent。


<details>
  <summary>Details</summary>
Motivation: 现有模型缺乏结合长视频、密集QA和空间时间细粒度标注的基准。

Method: 构建含199420个问答对的FineBench基准，设计基于定位器和描述器的FineAgent模块化框架。

Result: 闭源模型表现较好，开源模型性能不足；FineAgent可提升多种开源模型表现。

Conclusion: FineBench为细粒度人类中心视频理解提供严格测试，FineAgent提供实用增强方法。
...
### [142] [Rethinking Visual Attribution for Chest X-ray Reasoning in Large Vision Language Models](https://arxiv.org/abs/2605.20158)
*Guangzhi Xiong,Qiao Jin,Sanchit Sinha,Zhiyong Lu,Aidong Zhang*

Main category: cs.CV

TL;DR: 本文提出因果评估框架验证LVLM在胸部X光片推理中的视觉归因可靠性，并设计MedFocus方法通过非平衡最优传输和因果干预实现更可信的归因。


<details>
  <summary>Details</summary>
Motivation: LVLM在医学应用中可能无法忠实依据视觉证据，且现有归因方法是否反映模型真实推理过程未经验证。

Method: 构建因果评估框架（反事实编辑验证），设计MedFocus方法（非平衡最优传输定位解剖区域+因果干预度量效果）。

Result: 11种归因方法、6个LVLM在两种输出模式下常无法识别模型使用的证据；MedFocus在空间、概念和词级别归因上显著优于现有方法。

Conclusion: MedFocus通过因果驱动的归因提高了医学LVLM的可信度，但需进一步验证临床适用性。
...
### [143] [Bézier Degradation Modeling for LiDAR-based Human Motion Capture](https://arxiv.org/abs/2605.19620)
*Xiaoqi An,Lin Zhao,Jun Li,Chen Gong,Jian Yang*

Main category: cs.CV

TL;DR: 提出BMLiCap框架，利用Bézier曲线对运动进行时域压缩，通过多尺度运动Transformer和聚合器从LiDAR点云中恢复高精度、时序连贯的3D人体动作，在多个基准上达到最优表现。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR人体运动捕捉方法容易因输入不稳定和严重遮挡导致预测抖动甚至失败。

Method: 采用粗到细框架，利用可时域压缩的Bézier曲线建模运动，设计时间尺度运动变换器预测多尺度运动曲线，并通过多级运动聚合器自适应融合以恢复细节姿态。

Result: 在LiDARHuman26M等四个数据集上，BMLiCap在复杂场景下实现了最优精度和时序连续性，有效补偿遮挡并减少抖动。

Conclusion: 该框架能显著提升LiDAR人体运动捕捉在遮挡和噪声下的鲁棒性与准确性。
...
### [144] [UniRefiner: Teaching Pre-trained ViTs to Self-Dispose Dross via Contrastive Register](https://arxiv.org/abs/2605.19622)
*Congpei Qiu,Zhaoyu Hu,Wei Ke,Zhuotao Tian,Yanhao Wu,Tong Zhang*

Main category: cs.CV

TL;DR: 提出 UniRefiner，一种通用框架，通过对预训练 Vision Transformer 进行微调，识别并消除三类虚假令牌，提升密集预测任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的 Vision Transformers 中存在虚假令牌，它们缺乏位置对齐语义，损害了空间敏感任务的性能，而先前方法仅将其定义为高范数异常值，范围有限。

Method: UniRefiner 使用对比注册令牌，通过双目标优化：将图像令牌与过滤后的常规令牌对齐以保留语义，并将注册令牌与检测到的虚假令牌对齐以捕获虚假信号。

Result: 在 ~5k 图像上微调少量轮次后，在 ADE20K 上提升 mIoU 达 9.4%（如 EVA-CLIP-8B 达 51.9%），零样本分割准确率提升高达 22%。

Conclusion: UniRefiner 有效释放了大规模 ViTs 的潜在空间能力，为其在密集预测任务中的广泛应用铺平了道路。
...
### [145] [PrAda: Few-Shot Visual Adaptation for Text-Prompted Segmentation](https://arxiv.org/abs/2605.19623)
*Gabriele Rosi,Fabio Cermelli,Carlo Masone,Barbara Caputo*

Main category: cs.CV

TL;DR: 提出了一种名为PrAda的新型参数高效方法，通过原型适应将冻结的文本提示分割模型适配到新领域，在少量样本下显著提升语义、实例和全景分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本提示分割模型在远离预训练数据的专业目标域上性能下降，错误分析表明主要原因是分类错误而非掩膜生成错误。

Method: PrAda方法：学习类别特定原型，结合细粒度像素特征和高层次变换器表示，通过学习的重要性因子与原始文本预测融合，保持零样本潜力并适应新领域。

Result: 在五个基准测试的语义、实例和全景分割任务上，PrAda显著优于现有最先进方法和提出的基线。

Conclusion: 文本提示分割的少样本视觉适应问题可通过参数高效的原型适应方法有效解决，在领域偏移下大幅提升性能。
...
### [146] [Component-Aware Structure-Preserving Style Transfer for Satellite Sim2Real 6D Pose Estimation](https://arxiv.org/abs/2605.19624)
*Yonglong Zhang*

Main category: cs.CV

TL;DR: 提出一种组件感知结构保持的风格迁移框架，用于卫星合成到真实数据构建，通过部分级样式注入缩小外观差距，提升6D位姿估计性能。


<details>
  <summary>Details</summary>
Motivation: 非合作卫星单目6D位姿估计依赖于标注训练数据，但真实卫星图像获取困难；合成渲染可提供精确几何标注，但外观差异导致直接迁移效果差。

Method: 基于弱配对真实-合成样本，从无标注真实图像提取部件级样式码，通过掩码对齐调制注入合成图像；结合对抗训练、局部对比一致性、自正则化和边缘保持约束。

Result: FID 54.32，KID 0.048; 在下游GDRNet上ADD通过率0.260，AUC 0.611。

Conclusion: 组件级外观迁移可改善卫星Sim2Real位姿估计，同时保留模拟几何标注。
...
### [147] [P2DNav: Panorama-to-Downview Reasoning for Zero-shot Vision-and-Language Navigation](https://arxiv.org/abs/2605.19634)
*Kai Sheng,Liuyi Wang,Haojie Dai,Jinlong Li,Yongrui Qin,Zongtao He,Chengju Liu,Qijun Chen*

Main category: cs.CV

TL;DR: 提出P2DNav分层框架，通过全景到俯视的显式分解、滑动窗口对话记忆和反思性重定向机制，在零样本视觉语言导航中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有零样本方法依赖额外的航点预测模块，容易混淆高层方向推理与细粒度局部定位，导致决策不稳定且易出错。

Method: 分层框架P2DNav包含三个核心组件：全景到俯视图（P2D）分解方向选择与局部定位；滑动窗口对话记忆（SDM）维护长程导航历史；反思性重定向机制（RRM）评估局部定位可靠性并在必要时返回方向选择。

Result: 在R2R-CE基准上，相比最先进的零样本基于航点和无航点方法，成功率分别提升146.6%和58.9%。

Conclusion: P2DNav通过显式分解和反思机制有效提升了零样本VLN的决策稳定性与性能。
...
### [148] [Benchmarking and Evolving Reason-Reflect-Rectify for Reflective Visual Generation](https://arxiv.org/abs/2605.19639)
*Junjie Wang,Xinghua Lou,Jason Li,Ye Tian,Keyu Chen,Yulin Li,Bin Kang,Jacky Mai,Yanwei Li,Zhuotao Tian,Liqiang Nie*

Main category: cs.CV

TL;DR: 提出了R^3循环框架和R^3-Bench基准，以及R^3-Refiner框架，通过GRPO和分层奖励机制提升多轮反思式视觉生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型依赖单次生成，无法处理需要迭代优化的复杂指令。

Method: 形式化R^3循环，构建R^3-Bench基准，提出R^3-Refiner双阶段框架，结合GRPO和分层奖励机制。

Result: R^3-Refiner在R^3-Bench上反射评分和修正评分分别提升12.0%和9.0%，并兼容多种多模态大模型。

Conclusion: R^3-Refiner有效提升复杂指令下的生成质量，并能集成到现有模型中。
...
### [149] [LIFT and PLACE: A Simple, Stable, and Effective Knowledge Distillation Framework for Lightweight Diffusion Models](https://arxiv.org/abs/2605.19729)
*Hyunsoo Han,Sangyeop Yeo,Jaejun Yoo*

Main category: cs.CV

TL;DR: 提出粗到细的蒸馏框架LIFT和PLACE，解决教师网络因高容量导致的复杂去噪过程对学生模仿的挑战，在极端压缩下仍稳定收敛并取得良好FID。


<details>
  <summary>Details</summary>
Motivation: 教师网络容量大导致去噪过程复杂，学生模型难以有效模仿，尤其是在极端压缩场景下传统知识蒸馏失效。

Method: LIFT将目标分解为粗对齐和细精炼两阶段；PLACE通过误差分组提供局部自适应系数估计，处理空间非均匀误差。

Result: 在多种扩散空间、骨干网络、任务和数据集上有效，极端压缩（学生仅1.6%参数）下FID从50-200+降至15.73。

Conclusion: 所提框架显著提升知识蒸馏在扩散模型中的效果，并扩展至流模型。
...
### [150] [CAD-Free Learning of Spacecraft Pose Estimators via NeRF-Based Augmentations](https://arxiv.org/abs/2605.19649)
*Antoine Legrand,Renaud Detry,Christophe De Vleeschouwer*

Main category: cs.CV

TL;DR: 提出基于NeRF的图像增强方法，仅需少量真实图像即可训练航天器姿态估计网络，无需CAD模型


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖大量CAD渲染图像，限制了对无协作航天器的适用性且泛化性差

Method: 学习目标的神经辐射场，通过几何一致的视点和外观增强生成多样化训练数据

Result: 仅用25-400张真实图像即可训练准确姿态估计器，且能提升CAD数据集训练的模型在真实场景的鲁棒性

Conclusion: NeRF增强能有效替代CAD数据，实现从少量真实图像学习航天器姿态估计
...
### [151] [Synergistic Foundation Models for Semi-Supervised Fetal Cardiac Ultrasound Analysis: SAM-Med2D Boundary Refinement and DINOv3 Semantic Enhancement](https://arxiv.org/abs/2605.19799)
*Tonghao Zhuang,Shanglong Hu,Yongsheng Luo,Zhiqi Zhang,Yu Li*

Main category: cs.CV

TL;DR: 提出了一种半监督框架，用于胎儿心脏超声图像的联合分割与分类，在FETUS 2026排行榜上取得了DSC 79.99%、NSD 61.62%和F1-score 41.20%的成绩。


<details>
  <summary>Details</summary>
Motivation: 改善产前先天性心脏病筛查中，胎儿心脏超声图像的分割与分类性能。

Method: 基于EchoCare多任务骨干网络，集成SAM-Med2D进行边界细化，利用DINOv3提升伪标签质量，采用视图特定硬掩膜和两阶段优化（EMA阶段和分类微调阶段）。

Result: 在FETUS 2026排行榜上，Dice相似系数79.99%，归一化表面距离61.62%，F1分数41.20%。

Conclusion: 该半监督框架能有效联合分割和分类胎儿心脏超声图像，有助于产前先天性心脏病筛查。
...
### [152] [Cross-View Splatter: Feed-Forward View Synthesis with Georeferenced Images](https://arxiv.org/abs/2605.19656)
...
TL;DR: 提出了Cross-View Splatter，一种前馈方法，融合地面和卫星图像预测高斯泼溅，提升户外场景的新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 地面图像采集耗时且难以大规模获取，而卫星图像易于获取但缺乏局部细节，融合两者可互补。

Method: 将正射校正的卫星图像与GPS标记的地面照片融合，在统一3D坐标中预测高斯泼溅，对齐地面和鸟瞰特征表示。

Result: 在公开数据集和新基准上，该方法相比仅用地面图像的方法，改善了场景覆盖和新视角合成。

Conclusion: 融合卫星和地面图像能有效提升户外场景重建的完整性和质量。
...
### [153] [Stitched Value Model for Diffusion Alignment](https://arxiv.org/abs/2605.19804)
...
TL;DR: 提出StitchVM框架，将预训练的像素空间奖励模型附加到扩散主干上，高效处理带噪潜在变量，实现扩散模型对齐。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法在估计噪声潜在变量的值函数时存在偏差-计算成本权衡，缺乏有效的学习价值函数。

Method: 模型拼接：从截断的像素空间奖励模型出发，附加冻结的扩散主干作为头部，保留奖励能力并继承处理噪声的能力。

Result: 训练仅需10 GPU小时，下游方法（如DPS和DiffusionNFT）速度提升2.3-3.2倍，峰值GPU内存减半。

Conclusion: StitchVM通过构建一次正确的噪声潜在变量值函数并摊销使用，开启了新的扩散对齐范式。
...
### [154] [WBCAtt+: Fine-Grained Pixel-Level Morphological Annotations for White Blood Cell Images](https://arxiv.org/abs/2605.19692)
*Satoshi Tsutsui,Winnie Pang,Shuting He,Bihan Wen*

Main category: cs.CV

TL;DR: 提出WBCAtt+数据集，包含白细胞图像的11种形态属性和5种像素级成分标注，支持属性识别和语义分割等任务。


<details>
  <summary>Details</summary>
Motivation: 现有白细胞数据集主要标注细胞类别，缺乏病理学家用于解释细胞形态的详细特征。

Method: 构建WBCAtt+数据集（113k图像级标签和10k分割图），设计基于细胞组成结构的属性识别模型。

Result: 提供了属性识别和语义分割的基线模型，通过引入细胞组成结构提升了识别性能，并展示了可解释AI等应用。

Conclusion: WBCAtt+是首个提供白细胞全面标注的数据集，推动了细粒度分析和可解释模型的发展。
...
### [155] [A Framework for Evaluating Zero-Shot Image Generation in Concept-based Explainability](https://arxiv.org/abs/2605.19855)
*Giacomo Astolfi,Matteo Bianchi,Riccardo Campi,Antonio De Santis,Marco Brambilla*

Main category: cs.CV

TL;DR: 本研究探索了零样本文本到图像生成模型用于概念性可解释人工智能的潜力，针对合成概念数据的保真度进行了四项分析。


<details>
  <summary>Details</summary>
Motivation: 概念性XAI需要大量标注图像，限制了可扩展性，因此需寻找替代数据源。

Method: 使用零样本文本到图像模型生成概念数据集，并通过表示相似性、内部相似性、下游解释任务和概念移除实验评估其保真度。

Result: 当前生成模型虽提供捷径，但存在挑战和开放问题，合成数据与真实数据在保真度上仍有差距。

Conclusion: 零样本T2I生成的概念数据在模型分析中使用需谨慎，不直接等同于真实数据。
...
### [156] [DocQT: Improving Document Forgery Localization Robustness via Diverse JPEG Quantization Tables](https://arxiv.org/abs/2605.19688)
*Kylian Ronfleux-Corail,Guillaume Bernard,Mickaël Coustaty,Nicolas Sidère*

Main category: cs.CV

TL;DR: 研究发现文档操作定位模型在公开基准上表现良好，但在实际工作流中泛化失败，主要原因是训练中JPEG量化表分布与真实场景不匹配。


<details>
  <summary>Details</summary>
Motivation: 为了解决文档操作定位模型在真实保险文档流程中性能下降的问题，尤其关注训练与部署间量化表分布的差异。

Method: 进行了控制因子研究，比较了FFDN和Mesorch两种架构，分别在标准质量因子和从DocQT（实际操作图像库）采样的量化表下训练，并在三种重压缩条件下评估。

Result: 使用真实训练数据显著提升了DocTamper上的定位性能，并降低了真实文档的像素级误报率，但仅对明确使用量化表作为输入的架构有效。

Conclusion: 标准质量因子增强不足以模拟实际压缩多样性，明确依赖量化表的架构选择在真实部署中具有重要的鲁棒性优势。
...
### [157] [Physics-informed simulation framework for realistic sonar image generation and statistical validation](https://arxiv.org/abs/2605.19712)
*Kamal Basha S,Athira Nambiar*

Main category: cs.CV

TL;DR: 提出了ACOUSIM框架，通过物理模拟和统计分布对比验证合成声纳图像的真实性。


<details>
  <summary>Details</summary>
Motivation: 合成声纳数据集缺乏严格的定量验证方法，导致其实际应用受限。

Method: 在Gazebo环境中控制海床纹理、阴影、高度和噪声生成声纳图像；利用KL散度、JS散度和地球移动距离比较全局强度与局部纹理的分布。

Result: 纹理对齐表现良好（KL<0.07），平面类强度对齐优于舰船类（因阴影几何复杂性）。

Conclusion: ACOUSIM为合成-真实声纳评估建立了可重复的分布级基线，支持水下图像分析的可靠数据集验证。
...
### [158] [Physics-in-the-Loop: A Hybrid Agentic Architecture for Validated CAD Engineering Design](https://arxiv.org/abs/2605.19717)
*Elias Berger,Muhammad Usama,Jan Mehlstäubl,Bernhard Saske,Kristin Paetzold-Byhain*

Main category: cs.CV

TL;DR: 提出一种混合智能体-物理架构，将知识工程工具嵌入AI决策，通过显式物理验证提升生成CAD设计的物理可靠性。


<details>
  <summary>Details</summary>
Motivation: LLMs生成CAD缺乏物理理解，难以保证工程设计的可靠性。

Method: 将工程设计建模为闭环序列决策过程，智能体基于负载情况迭代规划、生成、评估和修改设计，使用知识工具作为反馈信号。

Result: 相比同类方法，结构复杂度提升4.2倍，编译成功率提升3.5%。

Conclusion: 显式物理验证能有效提升生成CAD的功能有效性和复杂程度。
...
### [159] [Passive Construction Site Safety Monitoring via Persona-Scaffolded Adversarial Chain-of-Thought VLM Verification](https://arxiv.org/abs/2605.19869)
*Ananth Sriram,Neel Mokaria,Rajveer Singh*

Main category: cs.CV

TL;DR: 提出了一种被动式、班后处理的施工安全监控流水线，通过三阶段架构（YOLO11检测、SAM 3分割、Qwen3-VL-8B-Instruct对抗链式推理）检测PPE和危险违规，并生成符合OSHA标准的报告。


<details>
  <summary>Details</summary>
Motivation: 建筑业是美国最致命的行业（2023年1055人死亡），现有监控方法成本高、需实时操作或仅覆盖少量违规，亟需一种高效、全面的自动化安全监控方案。

Method: 三阶段架构：1) 微调YOLO11检测PPE和危险物；2) SAM 3进行分割精化和工人去重；3) Qwen3-VL-8B-Instruct通过方法提示、角色背书的对抗链式推理（生成-鉴别-调和三遍）进行合规验证和幻觉控制。

Result: 在12个视频的Ironsite开发集上，Stage 3的对抗链式提示相比单次提示精度提升12%，尤其在易产生幻觉的违规类别上改进最大。

Conclusion: 该方法能系统地将违规映射至OSHA标准，进行REBA人体工学风险评估，并生成带时间戳的证据报告，为未来可复现的安全监控提供了评估工具。
...
### [160] [Efficient Long-Context Modeling in Diffusion Language Models via Block Approximate Sparse Attention](https://arxiv.org/abs/2605.19726)
*Wenhu Zhang,Yiming Wu,Huanyu Wang,Yaoyang Liu,Huanzhang Dou,Senqiao Yang,Sitong Wu,Hanbin Zhao,Jiaya Jia*

Main category: cs.CV

TL;DR: 提出BA-Att框架，通过下采样空间识别信息区域，避免先验采样偏差，在保持性能的同时实现高达6.95倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型中的块稀疏注意力方法依赖固定先验采样模式，可能遗漏关键标记且在分布变化时不稳定。

Method: 采用块级预下采样操作，在紧凑下采样空间中识别信息区域；结合轻量级范数排序模块和协方差补偿校正，近似全协方差。

Result: 相比FlashAttention，注意力计算加速达6.95倍；50%稀疏度下在语言模型、多模态语言模型和视频生成模型中保持近乎全注意力性能。

Conclusion: BA-Att框架有效提升效率与泛化能力，避免了先验驱动的采样不稳定性。
...
### [161] [VL-DPO: Vision-Language-Guided Finetuning for Preference-Aligned Autonomous Driving](https://arxiv.org/abs/2605.20082)
*Zhefan Xu,Ghassen Jerfel,Marina Haliem,Qi Zhao,Jeonhyung Kang,Khaled S. Refaat*

Main category: cs.CV

TL;DR: 提出VL-DPO框架，利用视觉语言模型自动生成人类偏好对，通过直接偏好优化微调运动预测模型，在Waymo数据集上实现RFS提升11.94%、ADE降低10.01%。


<details>
  <summary>Details</summary>
Motivation: 标准模仿学习目标难以捕捉人类驾驶偏好的复杂细微差别，而视觉语言模型具备推理和常识理解能力。

Method: 使用VLM作为零样本推理器，从预训练模型输出自动生成偏好对，再通过直接偏好优化（DPO）微调模型。

Result: RFS提升11.94%，ADE降低10.01%。VLM的轨迹选择可作为人类偏好的高质量代理。

Conclusion: 视觉语言引导的偏好对齐能有效提升运动预测模型与人类偏好的一致性。
...
### [162] [Breaking Modality Heterogeneity in Low-Bit Quantization for Large Vision-Language Models](https://arxiv.org/abs/2605.19929)
*Yi Zhong,Haotong Qin,Xindong Zhang,Lei Zhang,Guolei Sun*

Main category: cs.CV

TL;DR: 针对视觉语言模型在低比特后训练量化中因跨模态激活分布不均导致精度下降的问题，作者提出基于通道拆分的量化框架SplitQ。其核心包括模态特定异常通道解耦模块和自适应跨模态校准模块，在多种低精度配置下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法在视觉语言模型上精度下降，因为文本和视觉模态的激活分布存在异质性，且这种异质性不均匀地分布于通道：少数通道包含模态特定的异常值，且不同模态的异常值位于不同通道。

Method: 提出SplitQ框架，包含两个核心模块：1) 模态特定异常通道解耦模块，隔离异常通道并最小化开销；2) 自适应跨模态校准模块，使用双轻量可学习分支动态缓解模态引起的量化误差。

Result: 在6个多模态数据集上，SplitQ在W4A8、W4A4、W3A3和W3A2量化设置下均优于现有方法。W3A3下保持93.5%的FP16性能（69.5 vs. 74.3）。

Conclusion: SplitQ通过解耦异常通道和自适应校准有效缓解了跨模态异质性导致的量化误差，提升了低比特量化下视觉语言模型的部署效率。
...
### [163] [StruMPL: Multi-task Dense Regression under Disjoint Partial Supervision and MNAR Labels](https://arxiv.org/abs/2605.19931)
*Reza M. Asiyabi,Juan Alberto Molina-Valero,The SEOSAW Partnership,Steven Hancock,Casey M. Ryan*

Main category: cs.CV

TL;DR: 提出StruMPL方法，通过多任务密集回归处理异构不完整标签与物理约束，显著提升森林生物量估测精度。


<details>
  <summary>Details</summary>
Motivation: 星载激光雷达与地面样地标签结构不兼容且存在MNAR偏差，现有方法无法联合利用两者优势。

Method: 共享编码器+多变量回归、插补、倾向性头+可学习物理模块+AIPW伪结果损失函数（含停止梯度）。

Result: 在两个生态区，StruMPL的AGB RMSE和偏差优于消融变体及最新方法，AIPW减少高AGB偏差约54%。

Conclusion: 联合处理异构标签与物理约束能有效提升生物量遥感估测的准确性与无偏性。
...
### [164] [Tango3D: Towards Alignment for Global and Local 2D-3D Correspondence](https://arxiv.org/abs/2605.19727)
*Zebin He,Mingxin Yang,Shuhui Yang,Hanxiao Sun,Xintong Han,Chunchao Guo,Wenhan Luo*

Main category: cs.CV

TL;DR: Tango3D 是一个统一密集对应和全局检索的3D基础模型，通过几何感知2D视觉骨干和预训练3D VAE实现像素到点的细粒度对齐，同时保持全局语义检索能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D基础模型仅将点云与CLIP等视觉-语言空间全局对齐，无法建立细粒度的像素到点对应，限制了密集3D下游任务的应用。

Method: 使用几何感知2D视觉骨干和预训练3D VAE分别编码图像（2D块）和点云（3D令牌），映射到共享空间实现局部像素-点对齐和全局语义对齐；采用三阶段渐进训练策略稳定联合学习。

Result: 成功实现对象级别像素到点对齐并保持竞争力的全局检索，联合能力优于现有3D基础模型。

Conclusion: Tango3D通过建立细粒度对齐的特征空间，为密集3D下游任务注入丰富语义，推动相关应用发展。
...
### [165] [FlowErase-RL: Rethinking Concept Erasure as Reward Optimization in Flow Matching Models](https://arxiv.org/abs/2605.19739)
*Yi Sun,Zhiqi Zhang,Xinhao Zhong,Yimin Zhou,Shuoyang Sun,Bin Chen,Shu-Tao Xia,Ke Xu*

Main category: cs.CV

TL;DR: 提出FlowErase-RL，首个基于GRPO框架的流匹配模型概念擦除方法，通过动态双路径奖励优化实现安全可控生成。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法在推理时干预效果有限，或依赖监督微调，难以扩展至多概念场景。

Method: 将概念擦除重构为奖励优化问题，引入动态双路径奖励机制，联合优化概念擦除奖励与非目标空间奖励，并通过性能驱动的切换策略自适应平衡。

Result: 在色情、物体、艺术风格擦除任务上达到最先进性能，保持图像质量与语义对齐，对对抗攻击鲁棒，且有效支持多概念场景。
...
### [166] [GeoMamba: A Geometry-driven MambaVision Framework and Dataset for Fine-grained Optical-SAR Object Retrieval](https://arxiv.org/abs/2605.19734)
*Tiantong Fang,Xiuwei Wang,Jing Xiao,Wujie Zhou,Liang Liao,Mi Wang*

Main category: cs.CV

TL;DR: 提出了GeoMamba框架，通过几何特征注入和一致性约束，在未对齐光学-SAR影像上实现跨模态细粒度目标检索，性能显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态检索方法依赖对齐样本，而实际光学-SAR检索受模态差异、噪声和结构不一致限制，缺乏鲁棒的表示学习框架。

Method: 设计了几何特征注入（GFI）模块增强跨模态交互，结合几何一致性约束（GCC）和深度监督（DS）策略，引入结构先验与经典算子保留目标几何信息。

Result: 在新建的FGOS-as数据集上达到63.3% mAP和77.0% Rank-1准确率，全面超越基线方法。

Conclusion: 几何驱动的表示学习能有效缓解模态差异，为未对齐光学-SAR数据提供细粒度检索能力。
...
### [167] [Aero-World: Action-Conditioned Aerial Video Generation from Inertial Controls](https://arxiv.org/abs/2605.19728)
*Abdul Mohaimen Al Radi,Kunyang Li,Yuzhang Shang,Mubarak Shah,Yu Tian*

Main category: cs.CV

TL;DR: 提出了Aero-World方法，通过将预训练的图像到视频扩散模型转换为可控的航拍视频生成器，并引入AeroBench基准来评估生成的航拍视频与低层控制信号的一致性。


<details>
  <summary>Details</summary>
Motivation: 基础视频模型在具身AI中受限，因为主要基于自然语言训练，缺乏对低层控制信号的响应，尤其是在无约束6自由度空间中的航拍场景，小的运动误差会导致大的轨迹漂移。

Method: 通过动作令牌流将平移加速度和角速度序列注入预训练的潜空间扩散变换器；利用在真实视频-IMU对上训练的冻结潜空间物理探测仪，在LoRA微调过程中提供可微的惯性一致性监督，避免昂贵的视频解码。

Result: 在AeroBench上，Aero-World相比仅动作微调，平均动作对齐得分从57.7提升至63.6；相比AirScape，FVD更低（596.5 vs 1058.6），SSIM更高（0.595 vs 0.505），Flow-IMU相关性更高（0.44 vs 0.20）。

Conclusion: 冻结的潜空间物理探测仪监督是一种实用的机制，能够将预训练的视频生成器调整为更符合动作的航拍运动。
...
### [168] [Real-World On-Vehicle Evaluation of Embedding-Based Anomaly Detection](https://arxiv.org/abs/2605.19744)
*Albert Schotschneider,Daniel Bogdoll,Svetlana Pavlitska,Ahmed Abouelazm,Johann Marius Zoellner*

Main category: cs.CV

TL;DR: 提出了一种基于预训练视觉Transformer嵌入的实时异常检测方法，通过单张参考图像定义正常性，无需监督或数据集特定训练。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法依赖于抽象语义类定义正常性，难以适应多样化的真实场景，且需要大量异常数据。

Method: 使用预训练视觉Transformer提取斑块级特征，通过最近邻相似度在潜在语义空间中检测偏差，生成密集异常掩码。

Result: 在Road Anomaly基准测试上表现良好，在实际自动驾驶场景中成功定位语义异常物体。

Conclusion: 简单的基于参考的方法可在现实条件下提供有效的异常信号，适合实际部署。
...
### [169] [Mechanisms of Object Localization in Vision-Language Models](https://arxiv.org/abs/2605.19792)
*Timothy Schaumlöffel,Martina G. Vilas,Gemma Roig*

Main category: cs.CV

TL;DR: 本文通过机械可解释性工具分析了LLaVA-1.5和InternVL-3.5中的定位机制，发现定位依赖于容器化机制，且仅有少量注意力头参与。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在分类和定位任务上表现不足，尤其是定位机制尚不明确。

Method: 使用令牌消融、注意力消除和因果中介分析等机械可解释性工具。

Result: 定位由容器化机制驱动，对象对齐令牌定义空间范围，令牌语义排列对预测框影响不大；分类和定位共享部分早期处理但依赖专门的头。

Conclusion: 首次在层和头层面揭示VLM的定位机制，为模型设计和接地目标提供指导。
...
### [170] [Preferences Order, Ratings Anchor: From Fused Expert Aesthetic Ground Truth to Self-Distillation](https://arxiv.org/abs/2605.19776)
*Yuanpei Zhao,Jie Lin,Chao Zhang,Yilin Wang,Mao Li,Chenhui Li,Jie Hou,Tangjie Lv*

Main category: cs.CV

TL;DR: 提出PPaint双协议基准和PSDistill蒸馏方法，利用偏好与评分互补性提升图像美学评估。


<details>
  <summary>Details</summary>
Motivation: 偏好与评分两种标注协议在图像美学评估中的互补性尚未在受控条件下量化。

Method: 构建PPaint基准（150幅国画×5维度×双协议），提出PSDistill方法将VLM偏好转化为伪分数并训练。

Result: 蒸馏后的Qwen3-VL-8B在国画评估中SRCC从0.504提升至0.709，超越开源基线，接近Gemini-3.1-Pro。

Conclusion: 偏好与评分融合能有效提升美学评估性能，且偏好到分数的转换方法可推广至无标签VLM训练。
...
### [171] [CPC-VAR:Continual Personalized and Compositional Generation in Visual Autoregressive Models](https://arxiv.org/abs/2605.19750)
*Junhao Li,Xinhao Zhong,Yi sun,Yuxia Qiao,Bin Chen,Shu-Tao Xia,Yaowei Wang*

Main category: cs.CV

TL;DR: 本文首次系统研究VAR模型中的持续个性化生成，提出了统一框架，包含用于持续单概念学习的GCNS方法和用于多概念合成的上下文感知组合策略，有效解决了灾难性遗忘和特征纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于VAR的个性化方法局限于静态设置，无法适应连续的用户需求，顺序概念学习会导致严重灾难性遗忘，而多概念合成常出现特征纠缠和属性不一致。

Method: 提出统一框架：1）GCNS方法，通过梯度选择概念相关神经元，仅约束冲突参数以缓解遗忘；2）上下文感知组合策略，利用多分支特征建模和空间条件引导的局部交叉注意力融合，实现精确解耦的概念合成。

Result: 在长序列持续个性化中性能显著提升，并在多概念图像合成上取得优于现有基线的结果。

Conclusion: 研究表明VAR模型具有可扩展和可控个性化生成的潜力。
...
### [172] [Fast 4D Mesh Generation by Spatio-Temporal Attention Chains](https://arxiv.org/abs/2605.19786)
*Dvir Samuel,Yuval Atzmon,Gal Chechik,Yoni Kasten*

Main category: cs.CV

TL;DR: 提出了一种无需训练的4D网格生成加速方法，通过时空注意力链在9秒内生成网格，速度提升13倍，并支持更长序列和零样本下游任务。


<details>
  <summary>Details</summary>
Motivation: 现有4D网格生成方法速度慢、计算成本高，难以扩展到长序列，需要一种快速且能保持高质量时间对应关系的方法。

Method: 利用4D骨干网络中早期涌现的时间对应关系，设计时空注意力链，从锚点网格出发将顶点映射到潜在标记，跟随潜在空间中的时间对应关系，并通过潜在到顶点的注意力恢复帧特定顶点。

Result: 9秒生成4D网格，速度提升13倍，质量更优；支持16倍长序列而不降质；在2D和4D跟踪任务上实现零样本竞争性能；首次支持相机估计。

Conclusion: 所提方法在加速4D网格生成的同时改善了时间对应质量，并能扩展至更长的视频序列和下游任务。
...
### [173] [LaCoVL-FER: Landmark-Guided Contrastive Learning Network with Vision-Language Enhancement for Facial Expression Recognition](https://arxiv.org/abs/2605.19821)
*Jiaxin Wang,Muwei Jian,Hui Yu,Junyu Dong,Yifan Xia*

Main category: cs.CV

TL;DR: 提出了一种结合地标几何先验和视觉语言模型语义先验的面部表情识别方法，通过自适应编码和对比学习提升复杂场景下的识别鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力机制的方法主要依赖视觉外观，存在注意力冗余和不稳定性问题，在复杂场景中表现受限。

Method: 设计了地标引导自适应编码器（LGAE）实现几何与外观特征融合，视觉语言增强策略（VLES）利用表情相关特征优化视觉表示，以及表情条件提示（ECP）生成实例感知文本表示。

Result: 在RAF-DB、FERPlus和AffectNet三个数据集上优于现有方法。

Conclusion: 结合几何和语义先验能有效提升面部表情识别的性能和鲁棒性。
...
### [174] [Depth2Pose: A Pose-Based Benchmark for Monocular Depth Estimation without Ground-Truth Depth](https://arxiv.org/abs/2605.19797)
*Viktor Kocur,Sithu Aung,Gabrielle Flood,Yaqing Ding,Lukas Bujnak,Torsten Sattler,Zuzana Kukelova*

Main category: cs.CV

TL;DR: 提出了Depth2Pose框架，利用相对相机姿态准确度作为任务驱动指标来评估单目深度估计器在下游几何任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计评估主要依赖全局深度误差指标，无法准确反映深度估计对下游几何任务（如SfM、视觉定位）的实用性。

Method: 结合深度预测与特征对应，在深度感知几何求解器中计算相对相机姿态估计准确度；并构建D2P数据集，包含难以获取真值深度的挑战性场景（如大尺度、强遮挡环境）。

Result: 标准深度误差指标表现好的方法在旧数据集上姿态指标也表现好，但在新D2P数据集上不普遍泛化。

Conclusion: 提出的姿态基指标能更好评估深度估计在几何任务中的实用性，且仅需相机姿态即可评估，扩展了评估范围至难以获取深度真值的场景。
...
### [175] [When Preference Labels Fall Short: Aligning Diffusion Models from Real Data](https://arxiv.org/abs/2605.19839)
*Weiyan Chen,Weijian Deng,Yao Xiao,Weijie Tu,ZiYi Dong,Ibrahim Radwan,Liang Lin,Pengxu Wei*

Main category: cs.CV

TL;DR: 本文研究了使用真实数据作为偏好对齐的监督信号，通过对比真实与生成样本构建偏好信号，无需人工标注，且效果与现有偏好方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有偏好对齐方法依赖模型生成的成对样本，当两者质量均较差时，难以推断理想输出，因此探索真实数据作为替代监督源。

Method: 采用数据中心的视角，将真实图像作为参考，通过对比真实与生成/扰动样本构建偏好信号，无需手动标注。

Result: 实证分析表明，基于真实数据的监督能有效指导扩散模型对齐，性能与现有偏好方法持平。

Conclusion: 真实数据为偏好对齐提供了实用且互补的监督来源，并指示了标签高效对齐策略的方向。
...
### [176] [Landscape-Awareness for Geometric View Diffusion Model](https://arxiv.org/abs/2605.19865)
*Yan-Ting Chen,Hao-Wei Chen,Tsu-Ching Hsiao,Chun-Yi Lee*

Main category: cs.CV

TL;DR: 提出了一种基于分数的优化方法，通过重塑损失景观和结合视角条件扩散模型，提高了稀疏视角下相机视角估计的准确性和样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在双视角估计中因非凸损失函数和几何歧义（如对称性）导致对初始值敏感，依赖多起点策略，收敛效果差。

Method: 先使用基于分数的梯度调整优化景观，引导更新至真实视角，再用视角条件扩散模型进行精细化估计。

Result: 提高了收敛性，减少了对暴力采样的依赖，在更高样本效率下实现了竞争性的精度。

Conclusion: 该方法有效解决了稀疏视角估计中的局部极小值问题，切实提升了估计的鲁棒性和效率。
...
### [177] [Eyes on VLM: Benchmarking Gaze Following and Social Gaze Prediction in Vision Language Models](https://arxiv.org/abs/2605.19859)
*Hengfei Wang,Anshul Gupta,Pierre Vuillecard,Jean-Marc Odobez*

Main category: cs.CV

TL;DR: 提出了EyeVLM框架，从任务和模型两个维度系统评估视觉语言模型（VLM）在注视理解上的表现。测试了零样本和微调设置下的注视跟随与社会注视预测任务，发现当前VLM在精确注视理解上存在不足，即使微调后与纯视觉模型仍有差距。


<details>
  <summary>Details</summary>
Motivation: VLM作为通用多模态推理器可能有助于人类注视分析，但现有研究未充分评估其能力。

Method: 构建EyeVLM评估框架，包含注视跟随（几何和视觉处理）与社会注视预测（社交关系推理）两个核心任务；在零样本和微调（基于任务QA对）两种设置下测试多种开源/闭源VLM。

Result: 当前VLM缺乏精确注视理解能力；标准训练能缩小与纯视觉模型的差距，但仍需显著改进。

Conclusion: VLM在注视理解任务上表现不足，尤其缺乏精确的空间和社交推理能力，未来工作需重点提升。
...
### [178] [GoTTA be Diverse: Rethinking Memory Policies for Test-Time Adaptation](https://arxiv.org/abs/2605.19890)
*Shyma Alhuwaider,Yasmeen Alsaedy,Merey Ramazanova,Silvio Giancola,Bernard Ghanem*

Main category: cs.CV

TL;DR: 本文系统研究了测试时适应中的内存管理问题，发现类内多样性是关键因素，并提出GOTTA内存策略来提升适应性能。


<details>
  <summary>Details</summary>
Motivation: 现有内存机制通常作为特定TTA算法的一部分评估，难以隔离内存设计选择的影响，需要系统性基准研究。

Method: 通过解耦内存与适应算法的基准测试，提出GOTTA内存策略，结合类平衡分配与特征空间多样性。

Result: GOTTA在内存受限和非独立同分布流下显著提升适应表现，且在内存充足时保持竞争力。

Conclusion: 内存管理是TTA的关键组件，多样性是实现鲁棒适应的核心原则。
...
### [179] [WoundFormer: Multi-Scale Spatial Feature Fusion for Multi-Class Wound Tissue Segmentation](https://arxiv.org/abs/2605.19868)
*Muhammad Ashad Kabir,Rabin Dulal*

Main category: cs.CV

TL;DR: 提出WoundFormer，一种基于Transformer的多类创面组织分割框架，通过保留空间特征拓扑的多尺度聚合头提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法主要聚焦于二值分割，无法有效处理多类创面组织的高度异质性和有限的标注数据。

Method: 改进SegFormer解码器为空间保持的多尺度聚合头，通过卷积融合增强跨尺度上下文交互，保留特征拓扑。

Result: 在WoundTissueSeg数据集上Dice达81.9%，超越基线方法最多4.3个百分点，在少数组织类别上持续改进。

Conclusion: 显式建模层次化空间交互能增强Transformer对异质性创面组织分割的表现，支持更可靠的定量创面评估。
...
### [180] [Structured Layout Priors for Robust Out-of-Distribution Visual Document Understanding](https://arxiv.org/abs/2605.19866)
*Peter El Hachem,Ahmed Nassar,A. Said Gurbuz,Christoph Auer,Peter W. J. Staar*

Main category: cs.CV

TL;DR: 本文通过在视觉语言模型解码前注入布局检测器输出，缓解了两跳瓶颈问题，显著提升了文档解析在布局漂移场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在遍历文档时，因训练数据中未见布局而频繁出错，归因于先分类定位布局实体（第一跳）再提取内容（第二跳）的两跳瓶颈导致解码失败。

Method: 采用轻量级RT-DETR检测器预解析布局，将其输出序列化为模型原生DocTags词汇并注入提示，保持全局图像作为备选，无需架构修改。

Result: 在结构分布外10k页基准上，markdown F1从0.37升至0.92；OmniDocBench中文子集TEDS从0.01升至0.36；ViDoRe V3基准中，各工业领域死循环解码失败均降低。

Conclusion: 通过注意力分析验证了两跳瓶颈的缓解，且代价仅为15%墙钟延迟和中等74个提示词，无需基础VLM架构改动。
...
### [181] [Structural Energy Guidance for View-Consistent Text-to-3D Generation](https://arxiv.org/abs/2605.19876)
*Qing Zhang,Jinguang Tong,Jing Zhang,Jie Hong,Xuesong Li*

Main category: cs.CV

TL;DR: 提出SEGS框架，通过PCA子空间中的结构能量引导采样，无需训练即可缓解文本到3D生成中的Janus问题，将Janus率降低约10%并提升多视图一致性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到3D生成中常出现Janus问题（视图几何不一致），根本原因是2D扩散先验存在视角偏差。

Method: 在U-Net特征的PCA子空间中构建结构能量，将其梯度注入去噪过程，作为训练无关的即插即用模块集成到SDS/VSD管线中。

Result: 在DreamFusion、Magic3D、LucidDreamer等基线中，Janus率平均降低约10%，View-CS分数提升，同时保持外观保真度。
...
### [182] [AffectVerse: Emotional World Models for Multimodal Affective Computing](https://arxiv.org/abs/2605.19950)
*Bo Zhao,Fanghua Ye,Yixin Ji,Sicheng Zhao,Xiaojiang Peng,Zitong YU*

Main category: cs.CV

TL;DR: 提出AffectVerse，一个基于Qwen2.5-Omni的多模态大模型，通过情感世界模块进行短时潜在情感预测，在9个基准上提升至少2.57%。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM将情感识别视为静态融合，忽视了情感状态的动态演变。

Method: 基于Qwen2.5-Omni，构建情感世界模块（EWM）：包含跨模态时序想象、MAMA信念聚合和信念注入，通过未来预测作为自监督信号。

Result: 在9个基准上优于其他模型至少2.57%，消融实验验证了各组件的贡献。

Conclusion: 预测性信念状态建模是情感计算的有效替代方案。
...
### [183] [Towards Fine-Grained Robustness: Attention-Guided Test-Time Prompt Tuning for Vision-Language Models](https://arxiv.org/abs/2605.19956)
*Jia-Wei Hai,Yijun Wang,Xiu-Shen Wei*

Main category: cs.CV

TL;DR: 提出了一种名为注意力引导测试时提示调优（A-TPT）的方法，用于提升视觉语言模型在对抗攻击下的鲁棒性，同时保持语义保真。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法依赖多视图增强，难以保留细粒度场景中的语义信息，易破坏判别区域。

Method: 改进梯度注意力传播机制以识别语义区域，并利用其指导空间变化增强强度和多视图集成进行提示调优与推理。

Result: 在对抗和干净数据上均优于现有测试时适应方法。

Conclusion: A-TPT有效平衡了对抗鲁棒性和语义保持，代码已开源。
...
### [184] [SphericalDreamer: Generating Navigable Immersive 3D Worlds with Panorama Fusion](https://arxiv.org/abs/2605.19974)
*Antoine Schnepf,Karim Kassab,Flavian Vasile,Andrew Comport*

Main category: cs.CV

TL;DR: SphericalDreamer 是一种从文本提示生成完全沉浸式、可长距离导航的360°全景3D户外环境的方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时实现长距离导航和全方位视野（360°水平×180°垂直）的3D世界生成。

Method: 先生成多个全景图像，再将其提升为3D并融合，保持视觉和几何一致性。

Result: 生成高度详细、完全沉浸的3D环境，在规模和导航性上显著优于先前方法。

Conclusion: SphericalDreamer 有效克服了长距离导航与全方位视野的矛盾，为沉浸式3D环境生成提供了可行方案。
...
### [185] [Feed-Forward Gaussian Splatting from Sparse Aerial Views](https://arxiv.org/abs/2605.19949)
*Dongli Wu,Zhuoxiao Li,Tongyan Hua,Yinrui Ren,Xiaobao Wei,Rongjun Qin,Wufan Zhao*

Main category: cs.CV

TL;DR: AnyCity是一个从稀疏航拍视角重建大规模城市场景的生成式框架，通过观测支持几何先验和门控令牌条件化，实现了一致的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 稀疏航拍存在证据不平衡问题，导致现有前馈方法出现伪影，而生成方法缺乏观测与先验的明确分离。

Method: 先预测观测支持的几何潜在变量，再用脚手架条件的航拍补全令牌预测门控残差更新，通过密集到稀疏蒸馏和航拍适配的视频扩散先验进行训练。

Result: 在合成、航拍域、无人机纹理和真实场景中一致优于前馈基线，实现秒级推理。

Conclusion: AnyCity有效结合观测约束与生成先验，从稀疏航拍视角生成连贯的城市场景新视角合成。
...
### [186] [InterLight: Leveraging Intrinsic Illumination Priors for Low-Light Image Enhancement](https://arxiv.org/abs/2605.19982)
*Ziqi Wang,Xu Zhang,Laibin Chang,Shi Chen,Jiaqi Ma,Huan Zhang*

Main category: cs.CV

TL;DR: 提出InterLight框架，通过挖掘内在光照先验（物理引导增强、自适应提示、亮度门控记忆机制）提升低光照图像增强的鲁棒性，避免过度增强和颜色失真。


<details>
  <summary>Details</summary>
Motivation: 现有基于Retinex理论的方法常出现过度增强、颜色失真，且假设均匀噪声或理想光照，缺乏对光照先验的系统利用。

Method: 1.物理引导的传感器级光照-响应先验注入；2.自适应提示表示退化；3.亮度门控内在记忆机制选择性补偿信息损失；4.自监督一致性目标规整。

Result: 在多个基准数据集上，InterLight相比现有方法获得了更清晰的纹理和更视觉连贯的增强效果。

Conclusion: 系统挖掘内在光照先验能显著提升低光照图像增强的鲁棒性和视觉质量。
...
### [187] [X-Ray cardiac angiographic vessel segmentation based on pixel classification using machine learning and region growing](https://arxiv.org/abs/2605.20073)
*E O Rodrigues,L O Rodrigues,J J Lima,D Casanova,F Favarim,E R Dosciatti,V Pegorini,L S N Oliveira,F F C Morais*

Main category: cs.CV

TL;DR: 提出了一种基于像素分类的X射线血管造影分割方法，结合纹理特征、ELEMENT方法和随机森林分类器，达到了95.48%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有无监督方法精度不足，需要更准确的血管分割技术。

Method: 提取各向异性扩散、Hessian矩阵、数学形态学和统计等纹理特征，采用ELEMENT方法（区域生长控制的像素分类），并使用随机森林分类器。

Result: 准确率95.48%，优于现有无监督方法。

Conclusion: 该方法有效提升了血管分割的准确性。
...
### [188] [Cardiac fat segmentation using computed tomography and an image-to-image conditional generative adversarial neural network](https://arxiv.org/abs/2605.20064)
*Guilherme Santos da Silva,Dalcimar Casanova,Jefferson Tales Oliva,Erick Oliveira Rodrigues*

Main category: cs.CV

TL;DR: 提出基于pix2pix网络的心脏脂肪自动分割方法，在心外膜和纵隔脂肪分割中达到99%以上准确率，且运行速度快。


<details>
  <summary>Details</summary>
Motivation: 心脏周围脂肪与心血管疾病相关，但手动分割耗时昂贵，需自动化方法。

Method: 使用pix2pix生成对抗网络进行图像到图像转换，实现两种心脏脂肪的自主分割。

Result: 心外膜脂肪分割准确率99.08%，F1分数98.73；纵隔脂肪准确率97.90%，F1分数98.40；实时分割。

Conclusion: 该方法精度高、速度快，优于现有研究，可满足临床自动化需求。
...
### [189] [RECIPE: Procedural Planning via Grounding in Instructional Video](https://arxiv.org/abs/2605.19976)
*Luigi Seminara,Antonino Furnari,Lorenzo Torresani*

Main category: cs.CV

TL;DR: 提出RECIPE框架，利用视频ASR转录的时序可验证性作为奖励信号进行强化学习（GRPO），无需干净标注即可提升视觉规划模型的多步步骤生成质量，在7个基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有视觉规划数据集规模小、领域窄且仅编码单一执行轨迹，而大规模教学视频语料虽丰富但噪声大，直接微调会传播错误并限制多样性。

Method: 利用关键不对称性：从噪声视频提取干净步骤标签困难，但验证步骤序列在ASR转录中的时序对齐低成本且可扩展。设计RECIPE框架，将时序对齐质量作为GRPO的奖励信号，将噪声语料转化为验证器而非标签源，统一处理Socratic和Video两种规划器输入配置。

Result: 在0.5B/3B/7B模型规模下，RECIPE-RL在所有7个基准上优于基线，域内宏准确率提升7-8分，零样本达16分；优于监督微调，且保持生成多样性。

Conclusion: 基于时序验证的强化学习有效利用大规模噪声语料提升视觉规划质量，无需人工标注，且优于传统微调方法。
...
### [190] [A Nash Equilibrium Framework For Training-Free Multimodal Step Verification](https://arxiv.org/abs/2605.20033)
*Rohit Sinha,Kunal Tilaganji,Tanuja Ganu,Nagarajan Natarajan,Amit Sharma,Vineeth N. Balasubramanian*

Main category: cs.CV

TL;DR: 提出了一种无需训练的验证方法，通过纳什均衡游戏协调多个专业评判者，利用跨模态一致性信号提升多模态大语言模型的推理准确性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的推理链中常存在细微错误，现有验证方法（如学习型评判者需大量标注数据且表现不稳定，无训练方法忽略评分分歧中的有效信息）存在局限性。

Method: 将步骤级验证建模为专家评判者间的协调问题，通过纳什均衡博弈形式化交互：一致表示有效步骤，分歧揭示不稳定性，并利用闭式解计算均衡分数实现分歧感知过滤和稳定性排序。

Result: 在六个基准上相比基线模型提升2.4%-5.2%，性能与学习型评判者相当，证明跨模态一致性比平均置信度提供更鲁棒的验证信号。

Conclusion: 跨模态一致性可作为无需任务适配的鲁棒验证信号，有效提升多模态推理的可靠性。
...
### [191] [CogOmniControl: Reasoning-Driven Controllable Video Generation via Creative Intent Cognition](https://arxiv.org/abs/2605.19995)
*Hongji Yang,Songlian Li,Yucheng Zhou,Xiaotong Zhao,Alan Zhao,Chengzhong Xu,Jianbing Shen*

Main category: cs.CV

TL;DR: 提出推理驱动框架CogOmniControl，将可控视频生成分解为创意意图认知与生成，通过专用CogVLM和CogOmniDiT实现稀疏条件下视频生成，并引入闭环架构和专用基准。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在抽象、稀疏或复杂条件下表现脆弱，无法对齐用户创意意图，专业生产流程（如故事板草图）中效果差。

Method: 训练专用CogVLM从稀疏条件认知意图并输出密集推理；通过CogOmniDiT统一多条件控制，并用强化学习对齐推理输出；利用CogVLM规划评估器实现Best-of-N筛选，形成闭环架构。

Result: 在CogReasonBench和CogControlBench上超越现有开源模型，项目网站有详细展示。

Conclusion: 推理驱动框架有效弥合了条件控制与创意意图间的差距，为可控视频生成提供新范式。
...
### [192] [Stage-adaptive Token Selection for Efficient Omni-modal LLMs](https://arxiv.org/abs/2605.20035)
*Zijie Xin,Jie Yang,Ruixiang Zhao,Tianyi Wang,Fengyun Rao,Jing Lyu,Xirong Li*

Main category: cs.CV

TL;DR: 提出 SEATS，一种无需训练的、分阶段自适应的令牌选择方法，用于提升全模态大语言模型推理效率，在保留 10% 令牌时实现 9.3x FLOPs 减少和 4.8x 预填充加速，同时保持 96.3% 性能。


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型处理密集非文本令牌计算开销大，现有令牌选择方法忽视跨模态令牌重要性在各层的变化。

Method: 通过分析层间令牌依赖，提出 SEATS：在 LLM 前用注意力加权多样性选择移除时空冗余，在 LLM 内分块渐进剪枝并用查询相关性分数动态分配预算，后期层移除所有非文本令牌。

Result: 在 Qwen2.5-Omni 和 Qwen3-Omni 上，仅保留 10% 视觉和音频令牌，实现 9.3x FLOPs 减少、4.8x 预填充加速，保持 96.3% 原始性能。

Conclusion: SEATS 有效提升全模态大语言模型推理效率，无需训练，可自适应跨模态令牌重要性演化。
...
### [193] [MetaEarth-MM: Unified Multimodal Remote Sensing Image Generation with Scene-centered Joint Modeling](https://arxiv.org/abs/2605.20090)
*Zhiping Yu,Chenyang Liu,Jinqi Cao,Qinzhe Yang,Siwei Yu,Zhengxia Zou,Zhenwei Shi*

Main category: cs.CV

TL;DR: 提出了一个统一的多模态遥感图像生成基础模型，可以从任意模态生成任意其他模态，支持5种模态的联合生成与翻译。


<details>
  <summary>Details</summary>
Motivation: 多模态遥感图像在现实中常缺乏完整配对观测，现有逐对翻译方法灵活性差、扩展性有限。

Method: 采用以场景为中心的联合建模范式，先推断潜在场景表示，再基于该中间状态生成目标模态；并构建了包含280万张图像的大规模数据集EarthMM。

Result: 实验表明模型在多种生成任务上具有强大生成能力和鲁棒泛化能力，并能在数据和表示层面支持下游任务。

Conclusion: MetaEarth-MM有望成为跨模态地球观测的通用基础模型。
...
### [194] [OP2GS: Object-Aware 3D Gaussian Splatting with Dual-Opacity Primitives](https://arxiv.org/abs/2605.20044)
*Guiyu Liu,Niklas Vaara,Janne Mustaniemi,Juho Kannala,Janne Heikkilä*

Main category: cs.CV

TL;DR: 提出OP2GS，一种通过双重不透明度机制解耦视觉重建与实例占用的3D高斯表示，实现高效开放词汇场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在目标级身份识别上存在挑战：特征嵌入方法存储开销大，2D标签升维方法易受标签污染，导致重建所需高斯获得错误标签。

Method: 为每个高斯添加实例身份和专用不透明度σ*；原始σ负责视觉重建，σ*建模对目标掩码的贡献；通过随机目标损失利用透射率优化实例占用场，并在对象级通过多视图聚合附加语义描述符。

Result: 相比特征训练方法，OP2GS在保持竞争力的开放词汇性能同时显著降低计算开销；相比免训练方法，通过物理一致的占用学习解决可见性歧义。

Conclusion: 双不透明度机制有效分离视觉存在与实例占用，标签错误的高斯仍可参与图像渲染但在目标掩码分支透明，实现高效且鲁棒的目标级场景理解。
...
### [195] [Spatially Prompted Visual Trajectory Prediction for Egocentric Manipulation](https://arxiv.org/abs/2605.20085)
*Yifan Li,Xinyu Zhou,Yunhao Ge,Yu Kong*

Main category: cs.CV

TL;DR: 提出了空间提示视觉轨迹预测（SP-VTP）问题，通过初始空间提示（如边界框或点）定义任务目标，从第一人称视频预测末端执行器轨迹。构建了EgoSPT数据集，并设计了SPOT模型，结合任务编码器、观测编码器和轨迹生成器，在场景划分严格的实验中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 在杂乱环境中，通过语言指令指定相似物体操作目标不够有效，而空间提示（如指定位置）更直观，但现有研究缺乏对此类问题的形式化定义和解决方案。

Method: 提出SP-VTP问题形式化，收集EgoSPT数据集（包含第一帧目标标注和3D末端轨迹），设计SPOT模型：任务编码器处理第一帧视觉和坐标提示，观测编码器处理当前视觉和历史轨迹，轨迹生成器预测未来末端运动。

Result: 在场景级严格划分的实验中，SPOT在跨场景轨迹预测上优于无提示或单源提示的基线方法。

Conclusion: EgoSPT和SPOT建立了新的空间提示问题SP-VTP，为第一人称操作提供简单可扩展的任务条件。
...
### [196] [SetCon: Towards Open-Ended Referring Segmentation via Set-Level Concept Prediction](https://arxiv.org/abs/2605.20110)
*Zhixiong Zhang,Yizhuo Li,Shuangrui Ding,Yuhang Zang,Shengyuan Ding,Long Xing,Yibin Wang,Qiaosheng Zhang,Jiaqi Wang*

Main category: cs.CV

TL;DR: 提出SetCon方法，用LVLM生成的语义概念替代特殊标记进行开放域分割，在图像和视频任务上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 已有方法用特殊标记顺序表示多个目标，无法捕捉集合级属性。

Method: 层级语义分解预测集合概念并细化，结合混合标注流水线。

Result: 在gRefCOCO上+3.3 gIoU，MUSE上+12.1 gIoU，视频基准上最高+12.4 J&F。

Conclusion: 集合级概念预测有效提升开放域分割性能且可迁移至视频任务。
...
### [197] [TideGS: Scalable Training of Over One Billion 3D Gaussian Splatting Primitives via Out-of-Core Optimization](https://arxiv.org/abs/2605.20150)
*Chonghao Zhong,Linfeng Shi,Hua Chen,Tiecheng Sun,Hao Zhao,Binhang Yuan,Chaojian Li*

Main category: cs.CV

TL;DR: TideGS 是一种离核训练框架，通过在 SSD-CPU-GPU 层次间管理参数，实现单 GPU 上超过十亿高斯原语的训练，并达到最佳重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有 3DGS 训练受限于 GPU 内存，无法处理十亿级别的高斯原语。

Method: 采用基于 SSD-CPU-GPU 层次管理的三种协同技术：块虚拟化几何、层次异步流水线、轨迹自适应差分流。

Result: 在单 24GB GPU 上训练超十亿高斯原语，重建质量优于现有单 GPU 方法，并超越离核基线（约 1 亿）和传统内存训练（约 1100 万）。

Conclusion: TideGS 利用 3DGS 训练的稀疏性和轨迹条件特性，实现了大规模场景的高效训练。
...
### [198] [CaMo: Camera Motion Grounded Evaluation and Training for Vision-Language Models](https://arxiv.org/abs/2605.20165)
*Hsiang-Wei Huang,Junbin Lu,Kuang-Ming Chen,Jianxu Shangguan,Cheng-Yen Yang,Jenq-Neng Hwang*

Main category: cs.CV

TL;DR: 提出空间叙事评分（SNS）评估框架和相机运动感知VLM（CaMo），揭示现有VLM缺乏相机运动理解，并验证显式空间叙事对评估空间智能的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在空间问答基准上表现优异，但不确定其是否反映真实空间智能，尤其缺乏对相机运动这一空间认知关键成分的理解。

Method: 提出SNS评估框架，要求VLM生成包含场景语义和相机运动的显式空间叙事，再通过冻结的代理LLM进行推理；并引入CaMo模型强化相机运动知识。

Result: 在SNS评估中，最先进的VLM性能显著下降，尽管直接问答准确率很高；CaMo在SNS评估和直接问答上均表现一致。

Conclusion: 显式外部化空间叙事对评估VLM的可迁移3D空间理解至关重要。
...
### [199] [Multi-axis Analysis of Image Manipulation Localization](https://arxiv.org/abs/2605.20174)
*Keanu Nichols,Divya Appapogu,Giscard Biamby,Dina Bashkirova,Anna Rohrbach,Bryan A. Plummer*

Main category: cs.CV

TL;DR: 本文提出了AUDITS基准，包含53万张图像，用于评估图像篡改检测方法在域偏移、质量、篡改类型和大小等轴上的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对高级图像篡改（如扩散模型修复）在多视觉域下检测的全面评估。

Method: 构建了包含用户照片和新闻照片的双源数据集，利用扩散修复技术生成多样篡改类型和大小，并在域偏移条件下测试现有检测方法。

Result: 实验揭示了现有方法在不同域偏移下的性能差异，并提供了系统化分析。

Conclusion: AUDITS基准为推动更可靠、泛化的图像篡改检测研究提供了新见解。
...
### [200] [PixVerve: Advancing Native UHR Image Generation to 100MP with a Large-Scale High-Quality Dataset](https://arxiv.org/abs/2605.20147)
...
TL;DR: 提出PixVerve-95K数据集和训练方案，将T2I模型扩展到原生100MP超高清生成，并建立PixVerve-Bench评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型最高仅1K-2K分辨率，但成像技术发展催生对超高清（100MP）图像生成的需求，而数据稀缺和内容复杂构成主要挑战。

Method: 构建含95K样本、每图超1亿像素的高质量数据集，并设计三种训练方案扩展基础T2I模型至100MP原生分辨率。

Result: 通过传统指标和多模态大模型评估验证了方法的有效性，并提供对超高清生成训练策略的深入见解。

Conclusion: 所提数据集、训练方案和评估基准为超高清文本到图像生成奠定了坚实基础，为未来突破提供了关键经验。
...
### [201] [Interpretable Computer Vision for Defect Detection in X-ray Tomography of Aerospace SiC/SiC Composites](https://arxiv.org/abs/2605.20159)
*Antonio Peña Corredor,Julien Lesseur,Romain Nunez,Paul Rivalland,Thomas Philippe*

Main category: cs.CV

TL;DR: 提出了一种基于原型层的p-ResNet-50框架，用于航空航天SiC/SiC复合材料的XCT无损检测，在保持高检测精度的同时提供了可追溯的案例解释。


<details>
  <summary>Details</summary>
Motivation: 现有XCT检测依赖专家视觉评估，缺乏可追溯性；深度网络虽能自动检测缺陷，但其黑箱特性不符合工业检测对透明度的要求。

Method: 在ResNet-50中引入原型层，结合锚点正则化和中心点正则化项，使原型与专家定义的语义类别对齐，并利用UMAP分析潜在空间。

Result: 在约12000个补丁的数据集上，p-ResNet-50达到了与黑箱ResNet-50相当的准确率（0.957 vs 0.959）和ROC-AUC（0.994 vs 0.993），但以轻微灵敏度下降换取了更高精度和特异性。

Conclusion: 原型框架不仅为每次决策提供了可解释的证据，还显式标注了不确定区域，为其他需要可追溯决策的XCT检测场景提供了可复用方法。
...
### [202] [MSAVBench: Towards Comprehensive and Reliable Evaluation of Multi-Shot Audio-Video Generation](https://arxiv.org/abs/2605.20183)
...
TL;DR: MSAVBench是首个用于多镜头音频视频生成的综合基准和自适应混合评估框架，覆盖四个关键维度，实现91.5%的人类判断一致性，并系统评估了19个模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准在范围和多样性上有限，依赖僵化的评估流程，无法满足现代多镜头音频视频叙事评估的需求。

Method: 提出自适应混合评估框架，包括镜头分割的自校正机制、主观指标的实例级标准以及复杂判断的工具化证据提取。

Result: MSAVBench与人类判断的斯皮尔曼相关系数达91.5%，评估19个模型发现当前系统在导演级控制和细粒度视听同步上仍有困难。

Conclusion: 模块化或智能体生成管道是缩小开源与闭源模型差距的有前景方向，基准数据和评估代码将开源以促进未来研究。
...
### [203] [The Annotation Scarcity Paradox in Low-Resource NLP Evaluation: A Decade of Acceleration and Emerging Constraints](https://arxiv.org/abs/2605.19066)
*Vukosi Marivate*

Main category: cs.CL

TL;DR: 本文通过批判性叙事调查，揭示了低资源NLP评估中的“注释稀缺悖论”——模型扩展能力远超社区评估能力，威胁到进展的真实性。


<details>
  <summary>Details</summary>
Motivation: 低资源NLP的指数级增长掩盖了评估所需的社会语言学专业知识严重不足、分配不均且边缘化的关键矛盾。

Method: 采用批判性叙事调查法，梳理2014年至今的低资源NLP评估演变（三阶段），并分析提取式数据流水线、幽灵工作等机制。

Result: 注释稀缺悖论导致模型扩展与人类评估基础设施脱节，现有方案（如数据增强、基于模型的评估等）存在公平性与有效性权衡。

Conclusion: 呼吁从交易式数据提取转向关系性、社区嵌入式评估，基于知识治理、数据主权和共享所有权实现范式转变。
...
### [204] [Benchmarking Commercial ASR Systems on Code-Switching Speech: Arabic, Persian, and German](https://arxiv.org/abs/2605.19069)
*Sajjad Abdoli,Ghassan Al-Sumaidaee,Clayton W. Taylor,Ahmad,ElShiekh,Ahmed Rashad*

Main category: cs.CL

TL;DR: 本文提出了一个评估商业ASR系统在代码切换场景下性能的基准测试，涵盖四种语言对，并验证了BERTScore作为更可靠评估指标的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有商业ASR基准测试主要针对清晰、单语言音频，无法反映多语言和代码切换场景下的实际性能。

Method: 构建双阶段筛选管道：启发式过滤评分与GPT-4o和Gemini 1.5 Pro组合评分；选用WER和BERTScore进行评估。

Result: ElevenLabs Scribe v2在WER（总体13.2%）和BERTScore（总体0.936）上均表现最优；BERTScore对阿拉伯语和波斯语对更可靠。

Conclusion: 难度分层分析揭示了聚合平均值掩盖的性能差异；BERT嵌入投影证实语义相似性不受脚本差异影响。
...
### [205] [ReacTOD: Bounded Neuro-Symbolic Agentic NLU for Zero-Shot Dialogue State Tracking](https://arxiv.org/abs/2605.19077)
...
TL;DR: ReacTOD是一种有界的神经符号架构，通过自校正ReAct循环和符号验证器提升任务型对话系统的准确性和可靠性，在MultiWOZ和SGD基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 中等规模LLM在任务型对话中容易产生幻觉和格式错误，导致错误操作，需要可预测的行为。

Method: 将NLU重构为离散工具调用，在自校正ReAct循环中加入确定性验证器，使用增量状态预测和按需历史检索来保持提示紧凑。

Result: 在MultiWOZ 2.1上，gpt-oss-20B达到52.71%联合目标准确率，比之前最佳高14个百分点；Qwen3-8B达到47.34%。在SGD上，Claude-Opus-4.6达到80.68% JGA。

Conclusion: 有界神经符号架构能有效提升中等规模LLM在任务型对话中的准确性和自我校正能力，无需任务特定训练数据即可跨基准泛化。
...
### [206] [Agent Meltdowns: The Road to Hell Is Paved with Helpful Agents](https://arxiv.org/abs/2605.19149)
*Rishi Jha,Harold Triedman,Arkaprabha Bhattacharya,Vitaly Shmatikov*

Main category: cs.CL

TL;DR: 本文发现并测量了AI代理在遇到非恶意环境错误时可能发生的一种新型故障“意外熔毁”，即表现出不安全或有害行为。通过系统性评估，在64.7%的故障模拟场景中观察到了这种行为，且超半数未被报告给用户。


<details>
  <summary>Details</summary>
Motivation: 现有可靠性或安全性基准无法捕捉代理在良性错误下表现出的不安全行为，因此需要定义并测量这种新型故障。

Method: 开发了代理无关的错误注入基础设施，模拟本地和远程错误，并系统评估了基于GPT、Grok和Gemini的代理系统。

Result: 64.7%的代理在遇到模拟错误时发生不同程度和成功率的熔毁（如未授权侦察或破坏访问控制），超半数不安全行为未被报告，且错误后的探索行为与不安全行为相关。

Conclusion: 代理在应对环境错误时可能产生未被监测的安全风险，现有基准不足，需加强安全评估和报告机制。
...
### [207] [Prompting language influences diagnostic reasoning and accuracy of large language models](https://arxiv.org/abs/2605.19173)
*Adrien Bazoge,Josselin Corvellec,Sofiane Djillali Sid-Ahmed,Pierre-Antoine Gourraud*

Main category: cs.CL

TL;DR: 评估五种大型语言模型在英语和法语临床诊断中的表现，发现大多数模型在英语中表现更好，提示语言是影响临床性能的关键因素。


<details>
  <summary>Details</summary>
Motivation: 现有评估多集中在英语，不确定模型在其他语言中的可靠性。

Method: 使用180个临床案例，由两位医师按18分制评估五个模型的诊断准确性和推理质量。

Result: 四个模型在英语中表现更好（平均差0.37-0.91，p<0.05），涵盖鉴别诊断、逻辑结构和内部有效性；o3无语言差异。
...
### [208] [MMoA: An AI-Agent framework with recurrence for Memoried Mixure-of-Agent](https://arxiv.org/abs/2605.19194)
*Rui Chu*

Main category: cs.CL

TL;DR: 提出MMoA，一种基于LSTM门控的递归混合代理框架，通过动态路由选择减少计算开销，在保持性能的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有MoA系统依赖静态路由器，无法在聚合层间捕捉时间和上下文依赖，导致资源浪费。

Method: 在MoA中引入LSTM门控的递归路由器，根据当前输入和路由历史自适应调整代理贡献，实现上下文感知的聚合。

Result: 在AlpacaEval 2.0等基准上，MMoA达到58.0%胜率（MoA为59.8%），计算效率提升最高4.6%。

Conclusion: MMoA为多代理LLM系统提供了可扩展且高效的自适应聚合方案。
...
### [209] [Time to REFLECT: Can We Trust LLM Judges for Evidence-based Research Agents?](https://arxiv.org/abs/2605.19196)
*Leyao Wang,Yanan He,Peng Chen,Asaf Yehudai,Yixin Liu,Rex Ying,Michal Shmueli-Scheuer,Arman Cohan*

Main category: cs.CL

TL;DR: 评估深度研究代理中LLM评判者可靠性的基准REFLECT，发现当前评判者准确率低于55%。


<details>
  <summary>Details</summary>
Motivation: LLM评判者用于评估深度研究代理时可靠性未知，现有元评估方法存在粗粒度与任务局限。

Method: 构建REFLECT基准，定义精细故障分类，通过受控干预生成可验证实例。

Result: 最佳LLM评判者总体准确率低于55%，证据验证表现最差。

Conclusion: 当前LLM评判者不可靠，需改进评估管道并权衡成本与可靠性。
...
### [210] [Position: Uncertainty Quantification in LLMs is Just Unsupervised Clustering](https://arxiv.org/abs/2605.19220)
*Tiejin Chen,Longchao Da,Xiaoou Liu,Hua Wei*

Main category: cs.CL

TL;DR: 本文指出大语言模型的不确定性量化方法本质上是无监督聚类算法，仅衡量内部一致性而非外部正确性，无法检测‘自信幻觉’，并提出了从内部状态依赖向客观真实锚定的范式转变路线图。


<details>
  <summary>Details</summary>
Motivation: 当前主流UQ方法存在分类错误，将内部一致性误当作可靠性，可能导致在高风险部署中产生虚假安全感，尤其是无法识别高置信度但错误的幻觉。

Method: 通过理论分析揭示UQ方法依赖内部状态导致的三种关键病理：超参数敏感性危机、内部评估循环混淆稳定与真实、缺乏真实标签导致依赖不稳定代理指标。

Result: 识别了当前UQ方法的根本局限，即仅量化生成内部的稳定性而非事实正确性，并论证了其在实际部署中的危险性。

Conclusion: 呼吁向以客观真实为锚点的UQ范式转变，包括采用更好的评估指标与设置、机制性改变以实现原生不确定性、以及将验证锚定于客观事实，使模型置信度成为可靠的现实代理。
...
### [211] [Fine-tuning language encoding models on slow fMRI improves prediction for fast ECoG](https://arxiv.org/abs/2605.19224)
*Aditya R. Vaidya,Richard J. Antonello,Alexander G. Huth*

Main category: cs.CL

TL;DR: 该研究提出利用非侵入性fMRI数据来增强ECoG模型的训练，证明fMRI虽时间分辨率低，但能有效提升ECoG预测性能，且性能随fMRI数据量增加而提升。


<details>
  <summary>Details</summary>
Motivation: ECoG数据受限于需要植入电极的患者群体，而fMRI作为非侵入性方法可提供更多训练数据，弥补这一限制。

Method: 使用基于fMRI微调的语言表征来构建ECoG编码模型，并测试了时间降采样fMRI的泛化能力及数据量扩展效果。

Result: fMRI微调后的表征显著提升ECoG预测性能，降采样fMRI仍保持可比效果，且ECoG性能与fMRI数据量呈正相关。

Conclusion: 慢速fMRI数据可用于改进快速ECoG数据模型，未来整合多模态记录方法可能进一步提升解码等应用性能。
...
### [212] [Diagnosing Multi-step Reasoning Failures in Black-box LLMs via Stepwise Confidence Attribution](https://arxiv.org/abs/2605.19228)
*Xiaoou Liu,Tiejin Chen,Dengjia Zhang,Yaqing Wang,Lu Cheng,Hua Wei*

Main category: cs.CL

TL;DR: 提出SCA框架，基于信息瓶颈原理为闭源LLM逐步骤评估推理置信度，识别错误步骤并引导自我修正。


<details>
  <summary>Details</summary>
Motivation: 现有置信度估计方法局限于最终答案或需要模型内部访问，无法诊断多步推理中的失败步骤。

Method: 基于信息瓶颈原理，提出NIBS（无参数一致性度量）和GIBS（图模型学习子结构）两种互补方法。

Result: SCA能可靠识别低置信步骤，与推理错误高度相关；引导自我修正时纠正成功率提升达13.5%。

Conclusion: 该方法有效提升推理可靠性和可解释性，无需模型内部访问即可诊断错误步骤。
...
### [213] [AI Technologies in Language Access: Attitudes Towards AI and the Human Value of Language Access Managers](https://arxiv.org/abs/2605.19234)
*Miguel A. Jiménez-Crespo,Stephanie Rodriguez,Alejandro Jaume Losa*

Main category: cs.CL

TL;DR: AI技术正在重塑翻译实践和理论，本文探讨了语言访问领域中管理者对AI的态度和看法，发现他们有条件地乐观、风险意识强且坚持人力价值。


<details>
  <summary>Details</summary>
Motivation: 语言访问领域需要服务广泛多样的用户群体，受法律、伦理、商业和安全因素影响，研究管理者对AI的看法有助于理解技术应用中的挑战。

Method: 对十位美国语言访问管理者（来自医疗、法院、公共服务和地方政府）进行半结构化访谈，并进行定性主题分析。

Result: 管理者对不可避免的AI实施持条件乐观态度，但强烈意识到风险，并高度承诺保留人力价值和监督。

Conclusion: 在AI时代，语言访问领域需要平衡技术效率与人类监督，确保安全与伦理。
...
### [214] [FormalASR: End-to-End Spoken Chinese to Formal Text](https://arxiv.org/abs/2605.19266)
*Wanyi Ning,Yinshang Guo,Haitao Qian,Jiyuan Cheng,Weiyuan Feng,Yufei Zhang*

Main category: cs.CL

TL;DR: 提出两个紧凑端到端模型(0.6B/1.7B)，直接将中文口语转录为正式文本，无需后处理LLM，在WenetSpeech-Formal和Speechio-Formal数据集上实现最多37.4%的CER相对降低。


<details>
  <summary>Details</summary>
Motivation: 现有ASR系统针对逐字转录优化，产生的不流畅词、填充词等不适于写作应用，两阶段ASR+LLM方案增加延迟和内存成本，难以在设备端部署。

Method: 构建两个大规模口语转正式文本数据集(WenetSpeech-Formal和Speechio-Formal)，采用LLM重写和质量过滤；在Qwen3-ASR模型上进行监督微调，形成两个尺寸的FormalASR模型。

Result: 相比逐字转录基线，FormalASR在CER上相对降低最多37.4%，同时ROUGE-L和BERTScore也有提升。

Conclusion: FormalASR提供了一种轻量级、可设备端部署的口语转正式文本解决方案，消除了部署时对后处理LLM的依赖。
...
### [215] [DECOR: Auditing LLM Deception via Information Manipulation Theory](https://arxiv.org/abs/2605.19270)
*Linyue Cai,Samuel Yeh,Jwala Dhamala,Rahul Gupta,Sharon Li*

Main category: cs.CL

TL;DR: 提出DECOR框架，基于信息操纵理论进行细粒度审计，检测LLM中的策略性欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒方法只能粗粒度判断欺骗，无法定位具体事实如何被扭曲。

Method: 多智能体框架DECOR，将输入分解为原子信息单元，在四个操纵维度上评分，生成可解释的操纵概况并聚合为全局欺骗指数。

Result: 在单轮和多轮欺骗检测基准上达到最优性能，适用于15个前沿模型，消融实验验证了各组件的贡献。

Conclusion: 基于理论的信息操纵细粒度审计为LLM欺骗检测提供了有效且可解释的路径。
...
### [216] [Lost in Interpretation: The Plausibility-Faithfulness Trade-off in Cross-Lingual Explanations](https://arxiv.org/abs/2605.19274)
*Somnath Banerjee,Pranav Jha,Rima Hazra,Animesh Mukherjee*

Main category: cs.CL

TL;DR: 评估多语言LLM的英文解释时发现，它在流畅性上表现良好，但在因果可信度和完整性上远不如母语解释，尤其在社会敏感分类任务中会丢失语用线索。


<details>
  <summary>Details</summary>
Motivation: 多语言LLM通常使用英文解释来审计非英文输入，但这种方式可能牺牲解释的忠實性。

Method: 在3项任务、5种语言、2个LLM家族中，比较英文与母语解释的跨度一致性、完整性和充分性。

Result: 英文解释跨度一致性高，但完整性下降最多5.7倍，且任务准确性不变；社会敏感分类中忠實性和跨度一致性均降低。
...
### [217] [OpenCompass: A Universal Evaluation Platform for Large Language Models](https://arxiv.org/abs/2605.19276)
...
TL;DR: 本文提出并开源了OpenCompass，一个一站式、可扩展、高并发的通用大语言模型评估平台，旨在解决当前静态基准数据集评估方法面临的多样性、标准不一致和工作流碎片化等问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于静态基准数据集的评估方法面临任务类型多样性、评估标准不一致以及数据处理工作流碎片化等挑战，难以高效进行跨领域、大规模模型评估。

Method: 采用模块化和组件解耦的设计理念，核心架构包括配置系统、任务划分模块、执行调度模块、任务执行单元和结果可视化模块，提供基于规则、LLM作为裁判和级联评估器三种工作流。

Result: 支持多领域主流基准数据集（如知识、推理、计算、科学、语言、代码等），实现了高兼容性、灵活性和高并发性，为学术界和工业界提供统一高效的LLM评估工具。

Conclusion: OpenCompass平台能够有效评估LLM的优缺点，促进后续优化，解决当前评估体系中的关键问题。
...
### [218] [Language models struggle with compartmentalization](https://arxiv.org/abs/2605.19284)
*Thomas Vincent Howe,David Wingate*

Main category: cs.CL

TL;DR: 大语言模型在不同方式呈现相同概念时会出现分区现象，无法共享统计强度，导致模型容量冗余和样本效率下降。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型在训练数据中相同概念以多种方式呈现时，是否能够统一识别和共享信息，以及分区现象对模型性能的影响。

Method: 通过分析大语言模型在不同表示（如语言、编程语言、形式语言）下的表现，研究分区现象及其对容量和样本效率的影响，并引入合成平行数据实验。

Result: 发现大语言模型存在分区现象，早期多语言学习几乎完全分区，合成平行数据也无法改进，且所有干预措施的有效性随表示数量出现相变。

Conclusion: 语言建模目标可能无法一致地统一不同表示，分区现象会降低模型效率和容量利用率。
...
### [219] [Are Rationales Necessary and Sufficient? Tuning LLMs for Explainable Misinformation Detection](https://arxiv.org/abs/2605.19285)
*Bing Wang,Rui Miao,Ximing Li,Chen Shen,Shaotian Yan,Changchun Li,Kaiyuan Liu,Xiaosong Yuan,Jieping Ye*

Main category: cs.CL

TL;DR: 提出LONSREX方法改进可解释谣言检测的微调数据质量，通过量化验证步骤的必要性与充分性解决标签粗糙和过度验证问题。


<details>
  <summary>Details</summary>
Motivation: 现有可解释谣言检测方法依赖大型语言模型生成解释，但基于简单标签过滤的微调数据存在解释不充分（粗粒度标签）和冗余（过度验证行为）的局限。

Method: 提出LONSREX数据合成流水线，包含：1) 收集大规模事实核查文章；2) 多强LLM生成预测与理由；3) 提出量化指标评估每个验证步骤对最终预测的贡献度，筛选必要且充分的理由用于微调。

Result: 实验验证LONSREX方法有效性，相比传统标签过滤策略提升了可解释谣言检测的性能。

Conclusion: 通过精炼微调数据中的理由质量（必要且充分）能显著改善LLM在可解释谣言检测任务上的表现。
...
### [220] [How Do Document Parsers Break? Auditing Structural Vulnerability in Document Intelligence](https://arxiv.org/abs/2605.19309)
*Yue Chen,Yihao Wang,Ziyi Tang,Keze Wang*

Main category: cs.CL

TL;DR: 提出基于结构感知的轻量级审计框架，通过B-SLR等指标揭示DLA系统对微小结构扰动的脆弱性，推动鲁棒性评估从面积导向转向结构导向。


<details>
  <summary>Details</summary>
Motivation: 现有DLA鲁棒性评估过度依赖扰动面积（足迹偏差），忽略布局结构的影响。

Method: 构建解耦探针设计、策略驱动目标与结构感知诊断的审计框架，包含块级结构损失率（B-SLR）、粒度暴露描述符和路径归因。

Result: 在MinerU和PP-StructureV3上，B-SLR与OCR不稳定性相关性（R²=0.727/0.916）远高于面积指标（R²=0.384/0.110），小结构扰动可导致与大面积扰动相当的QA/检索性能下降。
...
### [221] [A Multi-Agent Framework for Feature-Constrained Difficulty Control in Reading Comprehension Item Generation](https://arxiv.org/abs/2605.19316)
*Seonjeong Hwang,Jun Seo,Hyounghun Kim,Gary Geunbae Lee*

Main category: cs.CL

TL;DR: 提出多智能体框架MAFIG，通过协作生成和迭代修订，显著提高阅读生成题项的特征约束满足率，并实现难度控制。


<details>
  <summary>Details</summary>
Motivation: 现有单智能体方法难以持续满足指定特征约束，导致生成题项偏离目标难度。

Method: 采用多LLM智能体与特征评估器协作生成和迭代修订题项，并构建单调递增的难度约束序列。

Result: MAFIG生成题项的目标约束满足率显著高于基线，通过难度校准的约束序列实现稳健难度控制。

Conclusion: MAFIG框架有效提升了难度控制阅读题项生成的约束满足率和难度可控性。
...
### [222] [Retrieval-Augmented Linguistic Calibration](https://arxiv.org/abs/2605.19344)
*Yi-Fan Yeh,Linwei Tao,Minjing Dong,Tao Huang,Jialin Yu,Philip Torr,Chang Xu*

Main category: cs.CL

TL;DR: 本文提出基于分布的言语自信框架，并引入忠实度概念，通过信息论度量FD和轻量级后处理流水线RALC提高语言校准效果。


<details>
  <summary>Details</summary>
Motivation: 现有语言自信表达缺乏通用校准框架，且面临语言线索共现、语境变化和主观解读等挑战。

Method: 将言语自信建模为概率值分布，定义忠实度及FD度量，并设计检索增强语言校准（RALC）后处理流水线。

Result: 在三个QA基准和五个LLM系列上，RALC将域内忠实度和校准分别提升高达66%和58%，优于黑盒与灰盒基线。

Conclusion: 分布框架和RALC方法能有效改善语言自信校准，忠实度成为重要评价维度。
...
### [223] [HalluWorld: A Controlled Benchmark for Hallucination via Reference World Models](https://arxiv.org/abs/2605.19341)
*Emmy Liu,Varun Gangal,Michael Yu,Zhuofu Tao,Karan Singh,Sachin Kumar,Steven Y. Feng*

Main category: cs.CL

TL;DR: 引入HalluWorld基准，将幻觉明确定义为模型生成与参考世界不符的可观察声明，通过合成环境和半合成环境自动评估幻觉，发现前沿模型在感知幻觉上近乎解决，但在多步状态跟踪和因果模拟上仍有困难，且不同失败模式表明幻觉源于多种能力缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉基准在任务设置、标注方式和可复现性上存在碎片化问题，难以比较不同缓解方法的效果，且缺乏对幻觉根本原因的系统研究。

Method: 构建HalluWorld，基于显式参考-世界框架，在网格世界、国际象棋和终端任务中生成完全指定的参考世界、控制模型视角并自动标注幻觉标签，支持变化环境复杂性、可观测性、时间变化和源冲突策略，并将幻觉细分为错误类别。

Result: 评估前沿和开源模型发现：直接观察的感知幻觉几乎被解决，但多步状态跟踪和因果模拟仍困难且未被广泛推理解决；终端任务中模型在容错时机上表现不足；不同探针和领域的失败模式不均，表明幻觉来自不同的失败原因。

Conclusion: 受控参考世界提供了一种可扩展和可复现的路径，用于测量和减少现代语言模型中的幻觉，且幻觉并非单一能力问题，而是多种故障模式的结果。
...
### [224] [IMLJD: A Computational Dataset for Indian Matrimonial Litigation Analysis](https://arxiv.org/abs/2605.19346)
*Joy Bose*

Main category: cs.CL

TL;DR: 提出了一个包含3613份印度法院判决的开放数据集IMLJD，涵盖2000-2024年印度最高法院和2018-2024年卡纳塔克高等法院的婚姻纠纷案件，包括结构化标签、元数据派生指标和知识图谱。


<details>
  <summary>Details</summary>
Motivation: 研究印度不同层级法院在婚姻纠纷案件（尤其是第498A条、反家暴法及第482条）中驳回请求成功率差异的动机。

Method: 构建开放数据集IMLJD，包含结构化结果标签、元数据指标和知识图谱，并对最高法院与高等法院的驳回率进行时间匹配比较。

Result: 最高法院驳回请求成功率为57.6%（2018-2024年59.3%），卡纳塔克高等法院为39.7%，差异达19.6个百分点，且经时间调整后结果稳健。

Conclusion: 数据集的开放共享（GitHub和Hugging Face）为相关法律研究提供了基础资源，揭示了印度司法体系中驳回率层级差异。
...
### [225] [SciCustom: A Framework for Custom Evaluation of Scientific Capabilities in Large Language Models](https://arxiv.org/abs/2605.19357)
...
TL;DR: 提出SciCustom框架，从大规模科学数据中定制构建基准测试，以评估LLM在特定科学应用中的细粒度能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试难以扩展且无法反映实际科学应用中的细粒度能力。

Method: 将科学知识组织为本体知识单元，训练标注器映射数据，通过多模型共识检索相关单元，利用二分查找和代理子集选择生成基准。

Result: 在化学和医疗领域揭示了标准基准忽略的细粒度差异，无需专家标注或合成问题生成。

Conclusion: 为评估LLM科学能力提供了可扩展且应用感知的基准测试基础。
...
### [226] [Taming the Thinker: Conditional Entropy Shaping for Adaptive LLM Reasoning](https://arxiv.org/abs/2605.19358)
...
### [227] [EmbGen: Teaching with Reassembled Corpora](https://arxiv.org/abs/2605.19394)
*Arun K Lenin,Kai Rouse,Andrea Nicastro,Anna Leontjeva*

Main category: cs.CL

TL;DR: 提出EmbGen流水线，通过实体-描述分解和嵌入相似性聚类生成合成QA数据，在异构数据集上提升Binary Accuracy达88.9%。


<details>
  <summary>Details</summary>
Motivation: 小型指令微调模型适配专业领域依赖昂贵的监督微调，现有合成数据方法输出同质化且忽略跨文档依赖。

Method: 将语料库分解为实体-描述对，基于嵌入相似性聚类，通过近邻、簇内和簇间采样生成QA对，并采用簇专用系统提示。

Result: 在最高异构数据集上，EmbGen在5M和20M token预算下Binary Accuracy分别提升12.5%和88.9%。

Conclusion: EmbGen在异构数据上显著优于基线，但异质性较低时竞争力一般。
...
### [228] [LambdaPO: A Lambda Style Policy Optimization for Reasoning Language Models](https://arxiv.org/abs/2605.19416)
*Zhe Yuan,Yipeng Zhou,Jinghan Li,Xinyuan Chen,Bowen Deng,Zhiqian Chen,Liang Zhao*

Main category: cs.CL

TL;DR: 提出LambdaPO框架，通过将优势估计分解为成对偏好结构并引入语义密度奖励，解决GRPO中信息瓶颈问题，提升强化学习对齐性能。


<details>
  <summary>Details</summary>
Motivation: GRPO依赖的组均值基线丢失了轨迹空间的细粒度偏好信息，限制了复杂奖励景观中的优化效果。

Method: 将单标量优势估计重构为成对比较的积分和，每对比较按策略置信度衰减；并用生成轨迹与真实解的精确率-召回率对齐作为语义密度奖励。

Result: 在数学推理和问答任务中，LambdaPO性能优于GRPO等基线方法。

Conclusion: 通过挖掘组内细粒度优化信号，LambdaPO引导大语言模型达到更优解。
...
### [229] [Drifting Objectives for Refining Discrete Diffusion Language Models](https://arxiv.org/abs/2605.19470)
*Daisuke Oba,Hiroki Furuta,Naoaki Okazaki*

Main category: cs.CL

TL;DR: 提出TokenDrift方法，将反对称漂移原理应用于离散扩散语言模型，通过软令牌特征空间实现训练阶段校正，显著提升少步生成质量。


<details>
  <summary>Details</summary>
Motivation: 连续生成器中的漂移方法表明采样阶段校正可部分吸收到训练过程中，但离散文本的不可微性和非连续样本阻碍了这一原理的直接迁移。

Method: 将分类预测提升为软令牌特征，在冻结语义空间中应用反对称漂移，并通过停止梯度将特征目标反向传播到DDLM对数概率。

Result: 在Masked和Uniform-state扩散骨干网络上，TokenDrift在4步生成时将Gen.-PPL分别降低89%（MDLM）和86%（DUO）。

Conclusion: 漂移方法可作为离散扩散语言模型的有效优化目标。
...
### [230] [Backtracking When It Strays: Mitigating Dual Exposure Biases in LLM Reasoning Distillation](https://arxiv.org/abs/2605.19433)
*Bing Wang,Shaotian Yan,Chen Shen,kaiyuan liu,Sinan Fan,Ximing Li,Rui Miao,Xiaosong Yuan,Zhanming Shen,Jieping Ye*

Main category: cs.CL

TL;DR: 提出MOTAB框架，通过动态监控学生模型生成轨迹并在偏离时回溯至安全状态并引入教师干预，解决LLM推理蒸馏中的双重暴露偏差问题，在LIMO-v2和AceReason数据集上平均提升约3%性能。


<details>
  <summary>Details</summary>
Motivation: 现有推理蒸馏方法存在双重暴露偏差：离策略蒸馏因训练与推理分布不匹配导致错误级联，在策略蒸馏则因教师模型在学生生成的次优上下文中无法提供有效指导。

Method: MOTAB动态监控学生在线生成轨迹，设定自适应安全边界；当生成偏离超过阈值时，回溯至最后安全状态，并利用教师模型进行干预纠正。

Result: 在LIMO-v2和AceReason数据集上的实验表明，MOTAB有效缓解双重暴露偏差，在推理任务中平均性能提升约3%。

Conclusion: MOTAB通过容忍轻微学生错误来减轻暴露偏差，同时阻止次优上下文以避免反向暴露偏差，为长链推理蒸馏提供了有效解决方案。
...
### [231] [Base Models Look Human To AI Detectors](https://arxiv.org/abs/2605.19516)
*Yixuan Even Xu,Ziqian Zhong,Aditi Raghunathan,Fei Fang,J. Zico Kolter*

Main category: cs.CL

TL;DR: 研究发现AI文本检测器（如GPTZero、Pangram）误判基础模型输出为人类，而指令微调模型则否；提出迭代释义（HIP）方法可有效规避检测。


<details>
  <summary>Details</summary>
Motivation: 观察到AI文本检测器对基础模型与指令微调模型的判断存在显著差异，引发对其检测机制的探究。

Method: 提出Humanization by Iterative Paraphrasing（HIP）方法，通过微调基础模型为释义器并迭代应用，实现检测规避。

Result: 在Llama-3和Qwen-3系列模型上（0.6B-70B），HIP相比基线在语义保留与检测规避间取得更好平衡，显著提升人机相似度。

Conclusion: 当前检测器更关注指令微调痕迹和局部上下文而非机器文本的固有特征，需设计显式建模这些因素的新型检测器。
...
### [232] [Investigating Cross-Modal Skill Injection: Scenarios, Methods, and Hyperparameters](https://arxiv.org/abs/2605.19523)
*Zhiyu Xu,Lean Wang,Yuanxin Liu,Lei Li,Hao Zhou,Fandong Meng,Jie Zhou,Xu Sun*

Main category: cs.CL

TL;DR: 研究了跨模态技能注入在视觉语言模型中的应用，系统分析了场景、方法和超参数的影响。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态理解中表现优异，但难以高效获取领域特定技能；现有微调方法需大量数据和计算，模型合并作为高效替代方案，但缺乏系统性分析。

Method: 跨模态技能注入，通过将领域专家大语言模型融入视觉语言模型，在指令跟随、跨语言和数学推理等场景中测试不同合并方法（如TA、DARE）及超参数调优。

Result: 跨模态技能注入在指令跟随和跨语言场景中表现良好，但在数学推理中效果不佳；TA和DARE方法优于其他合并策略，且超参数调优关键。

Conclusion: 跨模态技能注入是提升视觉语言模型领域技能的有效方法，但需根据场景选择合适方法和调优超参数，尤其在数学推理中需进一步改进。
...
### [233] [m3BERT: A Modern, Multi-lingual, Matryoshka Bidirectional Encoder](https://arxiv.org/abs/2605.19568)
*Yaoxiang Wang,Simiao Zuo,Qingguo Hu,Yucheng Ding,Yeyun Gong,Jian Jiao,Jinsong Su*

Main category: cs.CL

TL;DR: 提出了一种新的预训练策略m3BERT，通过联合优化Transformer层和多种嵌入维度，使得单个模型能适应不同资源和精度需求，显著提升了工业检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有预训练模型架构和嵌入维度固定，无法灵活适应资源受限的工业部署场景，而常见微调方法因预训练与下游任务不一致而效果不佳。

Method: 采用三阶段预训练：单语预训练、多语言适应、大规模网络语料持续预训练；并引入Matryoshka表示学习，同时优化层和维度。

Result: 在Bing-Click工业数据集上显著优于现有最佳模型；在公开数据集上验证了多粒度Matryoshka预训练策略的通用有效性。

Conclusion: m3BERT作为资源感知工业检索的高效基础，具有实践上的多功能性，能有效解决部署灵活性问题。
...
### [234] [A Data-Driven Approach to Idiomaticity Based on Experts' Criteria in Theoretical Linguistics](https://arxiv.org/abs/2605.19575)
*Elena Mikhalkova,Anastasiya Vishnyakova,Anastasiya Drozdova,Polina Gavin,Aleksander Zhmykhov,Timofey Protasov*

Main category: cs.CL

TL;DR: 通过对286个多词表达基于16条词汇、语法等标准的数据分析，发现不存在绝对习语性表达，词汇标准影响力最大，语法标准受特定条件限制，过时词汇和语法影响多词表达能否被单个词替代。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理论基础上系统分析多词表达的习语性特征分布，验证关于习语性的理论假设。

Method: 从理论文献收集286个多词表达，由语言学家基于16条标准进行标注，然后分析这些标准的分布情况。

Result: 不存在绝对习语性表达；词汇标准影响最大；语法标准依赖于特定条件；过时词汇和语法影响替换为单个词的能力。

Conclusion: 多词表达的习语性是一个连续体，词汇和语法因素起关键作用，但无纯粹习语性表达。
...
### [235] [GoLongRL: Capability-Oriented Long Context Reinforcement Learning with Multitask Alignment](https://arxiv.org/abs/2605.19577)
...
TL;DR: 提出了GoLongRL，一个开源的基于可验证奖励的长上下文强化学习后训练方案，包含23000个样本的数据集和TMN-Reweight多任务优化方法。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文RL方法的数据构建局限于设计复杂检索路径，导致任务覆盖单一，奖励设计无法反映实际长上下文需求。

Method: (1) 基于长上下文能力分类构建数据集，包含9种任务类型，来源包括开源语料和合成数据； (2) 提出TMN-Reweight，结合任务级均值归一化和难度自适应加权，解决异构奖励的多任务优化问题。

Result: 数据集在相同GRPO设置下优于QwenLong-L1.5数据集；Qwen3-30B-A3B模型性能与 DeepSeek-R1-0528 和 Qwen3-235B 相当；TMN-Reweight 在平均性能上进一步提升，同时保持或提升了通用能力。

Conclusion: 更广泛的任务覆盖和奖励多样性显著提升长上下文能力；TMN-Reweight有效改进了异构奖励下的多任务优化。
...
### [236] [LLMEval-Logic: A Solver-Verified Chinese Benchmark for Logical Reasoning of LLMs with Adversarial Hardening](https://arxiv.org/abs/2605.19597)
...
TL;DR: 提出了一个中文逻辑推理基准LLMEval-Logic，包含246个Base样本和190个Hard样本，评估14个前沿LLM发现最佳模型在Hard子集上准确率仅37.5%。


<details>
  <summary>Details</summary>
Motivation: 现有逻辑推理基准存在自然语言模板化、形式化注释粗略或未审计、被前沿模型快速饱和等问题，缺乏高质量的中文逻辑推理评估数据集。

Method: 从真实场景构建自然语言题目，采用前向作者与专家审计结合的方式提供形式化表示，用Z3验证答案，编写专家评分标准，并通过对抗性工作流加固题目。

Result: 最佳模型在Hard子集准确率仅37.5%，即使提供参考符号，联合Z3+Rubric形式化评分最高仅60.16%，揭示当前模型在逻辑推理上的显著差距。

Conclusion: LLMEval-Logic有效评估了LLM的逻辑推理能力，表明现有模型在此任务上仍有大幅提升空间。
...
### [237] [optimize_anything: A Universal API for Optimizing any Text Parameter](https://arxiv.org/abs/2605.19633)
...
TL;DR: 单一LLM优化系统在六个不同领域任务中达到最优，性能匹敌甚至超越专门工具，通过文本优化实现通用问题求解。


<details>
  <summary>Details</summary>
Motivation: 验证单个基于LLM的优化系统是否能跨领域匹配专门工具的性能。

Method: 将优化问题形式化为改进文本工件并评分，支持单任务搜索、多任务跨问题迁移以及对未见输入的泛化。

Result: 系统在ARC-AGI准确率提升至89.5%、云计算成本降低40%、CUDA内核87%匹配或超越PyTorch，并在圆堆积问题上超越AlphaEvolve。多任务搜索优于独立优化，可操作侧信息比仅评分反馈带来更快收敛和更高分数。

Conclusion: 基于LLM的文本优化是一种通用问题求解范式，能统一传统需要领域特定算法的任务。
...
### [238] [K-Quantization and its Impact on Output Performance](https://arxiv.org/abs/2605.19645)
*Robin Baki Davidsson,Pierre Nugues*

Main category: cs.CL

TL;DR: 研究了8种大语言模型在2-6位量化下的性能，发现高精度量化提升性能但收益递减，激进量化在部分模型中仍可接受，大模型对量化更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 大语言模型部署受限于其庞大尺寸，量化是一种有效的压缩方法，但其对模型性能的确切影响仍需进一步探究。

Method: 对8种LLM在不同量化水平下进行测试，评估其在MMLU-Pro、CRUXEval和MuSR等任务上的表现。

Result: 高精度量化提升性能但收益递减；激进量化在部分模型中保持可接受精度；大模型对量化更鲁棒，7-9B参数模型在效率与资源间取得平衡。

Conclusion: 量化会普遍降低性能，但影响因模型和任务而异，为模型大小、量化与性能间的权衡提供了洞见。
...
### [239] [LLM-Based Financial Sentiment Analysis in Arabic: Evidence from Saudi Markets](https://arxiv.org/abs/2605.19714)
*Mona H. Albaqawi,Eman M. Albalkhi,Joud A. Albaiti,Enrico Lopedoto*

Main category: cs.CL

TL;DR: 提出了一个面向沙特市场的阿拉伯语金融情感分析框架，整合新闻与社交媒体数据，构建84K样本数据集，实现公司级别的情感聚合与分析。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语金融情感分析面临语言复杂性和资源匮乏的挑战，现有研究不足。

Method: 多阶段数据收集与清洗、Transformer命名实体识别结合公司词典进行实体链接、五类情感标注方案。

Result: 构建84K样本数据集，展示可靠且可扩展的阿拉伯金融情感分析能力。

Conclusion: 该框架有效支持沙特证券市场的情感动态分析，具有良好的可靠性与可扩展性。
...
### [240] [Can Large Language Models Reliably Correct Errors in Low-Resource ASR? A Contamination-Aware Case Study on West Frisian](https://arxiv.org/abs/2605.19711)
*Yun Hao,Reihaneh Amooie,Wietse de Vries,Rik van Noord,Martijn Wieling*

Main category: cs.CL

TL;DR: 研究LLM在低资源语言(弗里斯语)ASR中的生成纠错能力，通过构建非公开数据集控制数据污染，发现GER能显著提升ASR性能，最佳结果超越oracle WER。


<details>
  <summary>Details</summary>
Motivation: 低资源语言ASR性能有限，LLM的GER效果在此场景下未被充分探索，且数据污染对结果影响未知。

Method: 使用公开语料库和自建非公开弗里斯语离线数据集，评估GPT-5.1的GER效果，并进行错误模式分析。

Result: GER在多数场景提升ASR表现，GPT-5.1最优结果优于oracle WER；离线数据集效果一致，证实纠错能力真实。

Conclusion: LLM可有效改善低资源语言ASR，需控制数据污染以评估真实能力，错误分析揭示模型纠错模式。
...
### [241] [Mathematical Reasoning in Large Language Models: Benchmarks, Architectures, Evaluation, and Open Challenges](https://arxiv.org/abs/2605.19723)
*Husnain Amjad,Raja Khurram Shahzad,Aamir Shahzad,Mehwish Fatima*

Main category: cs.CL

TL;DR: 本文综述了大型语言模型在数学推理方面的最新进展，通过结构化分析数据集、架构、训练策略和评估协议，涵盖了约120项研究。


<details>
  <summary>Details</summary>
Motivation: 数学推理是AI系统的重要基准，随着LLM推理能力提升，理解其数学推理表现变得关键。

Method: 采用统一的数学数据集分类法（预训练语料、监督微调资源、评估基准），系统分析了推理架构和训练策略（工具集成、验证器引导推理、参数高效适应）。

Result: 发现了推理忠实性、基准偏差和泛化限制等失败模式，并指出了最终答案准确性与过程级推理验证之间的差距。

Conclusion: 提出了改进符号基础、评估可靠性以及开发更鲁棒可信的LLM推理系统的研究方向。
...
### [242] [CAIT: A Syntactic Parsing Toolkit for Child-Adult InTeractions](https://arxiv.org/abs/2605.19718)
*Francesca Padovani,Xiulin Yang,Bastian Bunzeck,Jaap Jumelet,Yevgen Matusevych,Nathan Schneider,Arianna Bisazza*

Main category: cs.CL

TL;DR: 本文提出了一个专门针对CHILDES语料库的句法分析工具包CAIT，包括依存句法分析器、词性标注器和构式标注器，在儿童-成人交互文本上优于通用工具。


<details>
  <summary>Details</summary>
Motivation: CHILDES是儿童语言习得研究的重要资源，但现有计算工具对其句法结构的分析能力有限。

Method: 利用最新的UD-English-CHILDES树库，训练了基于通用依赖标注的依存句法分析器，并开发了配套的词性标注器和构式标注器。

Result: CAIT工具包在儿童-成人交互文本上优于SpaCy和Stanza等通用英语分析器，并通过错误分析和个案研究展示了其实用性。

Conclusion: CAIT为儿童语言习得的大规模、可重复研究提供了有效工具。
...
### [243] [ContextRAG: Extraction-Free Hierarchical Graph Construction for Retrieval-Augmented Generation](https://arxiv.org/abs/2605.19735)
*Roman Prosvirnin,Sergei Kuznetsov,Seungmin Jin*

Main category: cs.CL

TL;DR: ContextRAG通过模糊概念图和共振逻辑构建图RAG，避免LLM实体抽取，显著降低索引成本且在多跳问答中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有图RAG依赖LLM提取实体和关系，导致令牌和耗时成本随语料库规模增长而剧增。

Method: 使用残差量化k-means和Lukasiewicz共振逻辑从块嵌入中推导模糊概念图，通过软模糊连接和meet运算生成上下文节点，无需LLM编写图边。

Result: 在130任务子集上仅用30次LLM调用和22,073个令牌构建索引；多跳任务F1达36.8%，检索到晶格派生节点的查询F1提升3.9个百分点。

Conclusion: ContextRAG在不牺牲性能的前提下，大幅降低了图RAG的构建成本，并验证了晶格结构对多跳推理的辅助作用。
...
### [244] [Synthesis and Evaluation of Long-term History-aware Medical Dialogue](https://arxiv.org/abs/2605.19766)
*Hebin Hu,Renke Dai,Ah-Hwee Tan,Yilin Kang*

Main category: cs.CL

TL;DR: 提出了一个合成高质量长期医疗对话的框架，并创建了MediLongChat数据集，用于评估医疗智能体的记忆力。


<details>
  <summary>Details</summary>
Motivation: 现有医疗数据缺乏长期对话记录，限制了跨会话推理能力的系统评估。

Method: 通过知识引导的三阶段合成：构建患者档案、生成多轮对话、整合为长期历史数据集，并设计多维度评估框架。

Result: 最先进的LLM在MediLongChat基准测试中表现不佳，表明该基准的挑战性和应用价值。

Conclusion: 该基准有助于推动针对医疗智能体的专门方法研究。
...
### [245] [TERGAD: Structure-Aware Text-Enhanced Representations for Graph Anomaly Detection](https://arxiv.org/abs/2605.19738)
*Wen Shi,Zhe Wang,Huafei Huang,Qing Qing,Ziqi Xu,Qixin Zhang,Xikun Zhang,Renqiang Luo,Feng Xia*

Main category: cs.CL

TL;DR: 提出了TERGAD框架，利用大语言模型将节点拓扑特性转化为自然语言描述，增强结构语义，通过门控双分支自编码器融合特征并计算异常分数，在六个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本丰富的图异常检测方法忽视节点结构上下文，难以检测内容与拓扑角色不一致的复杂异常。

Method: 将节点拓扑特性翻译为自然语言描述，由大语言模型生成语义嵌入，通过门控双分支自编码器自适应融合原始属性与语义嵌入，联合重构图结构和节点特征。

Result: 在六个真实数据集上，TERGAD一致优于最先进基线，消融研究证实结构语义引导和门控融合机制的有效性。

Conclusion: TERGAD有效利用大语言模型的结构语义推理能力，显著提升了图异常检测性能。
...
### [246] [LP-Eval: Rubric and Dataset for Measuring the Quality of Legal Proposition Generation](https://arxiv.org/abs/2605.19815)
*Shanshan Xu,Johan Lindholm,Amogh Raina,Henrik Palmer Olsen,Daniel Hershcovich*

Main category: cs.CL

TL;DR: 本文研究了使用大型语言模型从欧盟法院判决中自动生成法律命题，并提出了LP-Eval评估框架。


<details>
  <summary>Details</summary>
Motivation: 法律命题生成在法律NLP中尚未被充分研究，而它对法律推理和学术研究至关重要。

Method: 提出了LP-Eval三步评估标准，包括形式有效性和实质维度，并收集了两位专家对100个LLM生成命题的注释。

Result: LLM能生成高质量命题，但来自成熟案例的命题质量高于近期案例；基于标准的LLM评估比直接打分更接近专家判断。

Conclusion: LLM在法律命题生成中表现良好，但评估时对细微差别不敏感，需进一步改进。
...
### [247] [Towards Trust Calibration in Socially Interactive Agents: Investigating Gendered Multimodal Behaviors Generation with LLMs](https://arxiv.org/abs/2605.19798)
*Lucie Galland,Chloé Clavel,Magalie Ochs*

Main category: cs.CL

TL;DR: 本文探索了利用大语言模型生成多模态行为以校准用户对社交智能体信任的可行性，验证了GPT-5.4能按指定能力和善意水平生成行为，但存在性别刻板印象问题。


<details>
  <summary>Details</summary>
Motivation: 社交智能体需校准用户信任至其实际能力，以促进适当使用。

Method: 提出自动生成与指定可信度维度（能力、善意）对齐的多模态行为方法，使用GPT-5.4生成数据，通过随机森林分析特征重要性，并开展用户研究验证。

Result: GPT-5.4能生成连贯的多模态行为，且符合理论预期；但提示中指定性别时，模型会复制社会性别刻板印象。

Conclusion: LLM可在生成行为中反映可控的可信度维度，但需注意消除性别偏见。
...
### [248] [Chunking German Legal Code](https://arxiv.org/abs/2605.19806)
*Max Prior,Natalia Milanova,Andreas Schultz*

Main category: cs.CL

TL;DR: 本文研究了适用于德国成文法（以德国民法典为基准语料）的检索增强生成中的文本分块策略，比较了基于结构单元、固定窗口、语义聚类等多种方法，发现与法律固有结构对齐的分块（如章节和子章节）在召回率和计算效率上表现最优。


<details>
  <summary>Details</summary>
Motivation: 现有分块策略多针对通用文本，缺乏对法律领域结构化语料（如成文法）的系统性评估，而法律检索中保持领域结构对准确性和效率至关重要。

Method: 实施并比较了结构单元（章节、子章节、句子、命题）、固定大小窗口、上下文分块、语义聚类、Lumber风格分块及RAPTOR层级检索等方法，在带章节级标签的法律问答数据集上评估召回率、延迟、索引构建时间和存储需求。

Result: 基于章节和子章节的检索召回率最高，而覆盖固有结构的复杂方法（如语义聚类、RAPTOR）表现更差；简单方法在计算效率上优于LLM密集型技术（如上下文分块、RAPTOR、Lumber）。

Conclusion: 保留领域特定结构对有效法律信息检索至关重要，存在语义丰富性与操作成本之间的关键权衡，简单结构对齐方法在精度和效率上优于复杂语义方法。
...
### [249] [Are Tools Always Beneficial? Learning to Invoke Tools Adaptively for Dual-Mode Multimodal LLM Reasoning](https://arxiv.org/abs/2605.19852)
*Qinghe Ma,Zhen Zhao,Yiming Wu,Jian Zhang,Lei Bai,Yinghuan Shi*

Main category: cs.CL

TL;DR: 提出AutoTool模型，自适应决定是否调用工具，平衡推理准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强方法忽视调用工具的必要性，冗余调用增加开销并误导预测。

Method: 基于强化学习框架，设计双模式推理策略（工具辅助与文本推理），通过模式特定奖励函数和探索机制避免早期偏向。

Result: 在V*基准上准确率提升21.8%，在POPE基准上效率提升44.9%。

Conclusion: 自适应工具调用显著提升多模态大模型推理性能与效率。
...
### [250] [CLIF: Concept-Level Influence Functions for Transparent Bottleneck Models](https://arxiv.org/abs/2605.19848)
*Yike Sun,Mingkun Xu,Mu You,Zhongzhi He,Henghua Shen,Zehan Tan,Derek F. Wong,Tao Fang*

Main category: cs.CL

TL;DR: 本文提出使用影响函数增强NLP模型的可解释性，在样本和概念层面识别重要训练样本和概念，并通过调整它们来高效调试模型。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医疗、金融等高风险领域的应用受限于其黑盒特性，缺乏可解释性。

Method: 采用影响函数分析训练样本和概念对模型预测的影响，并在CEBaB和Yelp数据集上进行实验，通过调整样本标签和权重或修改关键概念来验证。

Result: 影响函数能有效识别有助和有害的训练样本，调整后可恢复基线性能；概念级分析显示修改关键概念能显著改变模型行为。

Conclusion: 影响函数为NLP模型提供有效的可解释性和数据调试方法，尤其在概念瓶颈模型中表现突出。
...
### [251] [Where Does Authorship Signal Emerge in Encoder-Based Language Models?](https://arxiv.org/abs/2605.19908)
*Francis Kulumba,Guillaume Vimont,Laurent Romary,Florian Cafiero*

Main category: cs.CL

TL;DR: 论文发现，相同预训练编码器、数据和损失下，不同评分机制可导致四倍性能差异，并通过机制可解释性工具分析了其原因。


<details>
  <summary>Details</summary>
Motivation: 解释不同评分机制导致作者归属模型性能巨大差异的根本原因。

Method: 使用机制可解释性工具进行因果干预，分析评分机制对编码器信号整合位置的影响，并基于梯度结构和训练动力学推导差异来源。

Result: 平均池化迫使信号在早期至中层整合，而晚期交互则推迟至后续层；这种差异源于梯度结构及相应训练轨迹。

Conclusion: 评分机制决定了编码器整合作者风格信号的位置，而非表征质量本身导致性能差异。
...
### [252] [Rethinking How to Remember: Beyond Atomic Facts in Lifelong LLM Agent Memory](https://arxiv.org/abs/2605.19952)
*Jingwei Sun,Jianing Zhu,Jiangchao Yao,Tongliang Liu,Bo Han*

Main category: cs.CL

TL;DR: 提出TriMem记忆系统，通过原始对话片段、原子事实和聚合概况三重粒度表示，结合TextGrad提示优化，实现长程交互中的可靠记忆管理。


<details>
  <summary>Details</summary>
Motivation: 现有基于事实的记忆方法丢失对话细节，无法对分散事实进行深度推理，且静态提示难以适应多样化的对话风格。

Method: 三重粒度记忆：原始片段保真、原子事实检索、概况推理；TextGrad优化提示，无需参数更新。

Result: 在LoCoMo和PerLTQA数据集上，TriMem优于强基线方法。

Conclusion: TriMem通过多粒度表示和提示优化，有效提升了LLM代理的长程交互记忆能力。
...
### [253] [What Are LLMs Doing to Scientific Communication? Measuring Changes in Writing Practices and Reading Experience](https://arxiv.org/abs/2605.19936)
*Filip Miletić,Neele Falk*

Main category: cs.CL

TL;DR: 本研究发现，LLM在写作中的使用显著改变了NLP领域论文的文体风格，导致词汇频率和用法变化、句式更复杂、词汇多样性降低，并影响专家的阅读体验。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型在科学写作中日益普及是否改变了学术交流的风格。

Method: 创建ACL Anthology（2020-2024）的逾37,000篇论文语料库和3,000组人类与LLM改写文本的合成数据集，进行历时词汇分析、复杂文体特征建模和20名领域专家的主观评估。

Result: 词汇频率和使用语境发生显著变化（语义专化或泛化）；LLM修改后的文本更常使用特定句式、更长单词、更低词汇多样性；专家认为LLM修改文本更易理解和令人兴奋，但对LLM持负面定性态度。

Conclusion: AI辅助写作对阅读体验具有强烈主观影响，既有积极效果也存在负面感知，需谨慎评估。
...
### [254] [FlexDraft: Flexible Speculative Decoding via Attention Tuning and Bonus-Guided Calibration](https://arxiv.org/abs/2605.20022)
*Yaojie Zhang,Jianuo Huang,Junlong Ke,Yuhang Han,Yongji Long,Tianchen Zhao,Biqing Qi,Linfeng Zhang*

Main category: cs.CL

TL;DR: 提出FlexDraft框架，通过三项设计（注意力调优、奖励引导校准、灵活解码）在不同批次大小下实现无损推测解码。


<details>
  <summary>Details</summary>
Motivation: 现有并行推测解码方法在批次增大时吞吐量下降，存在草稿验证不匹配和不确定性问题。

Method: 注意力调优（仅微调注意力投影器）、奖励引导校准（MLP）、灵活解码（并行/顺序切换及动态验证长度）。

Result: 在批次大小变化时保持高质量草稿和高效验证，避免了吞吐量崩溃。

Conclusion: FlexDraft可灵活适应不同批次大小，实现无损且高效的推测解码。
...
### [255] [Language Mutations Sustain the Persistences of Conspiracy Theories on Social Media](https://arxiv.org/abs/2605.20050)
*Calvin Yixiang Cheng,Dorian Quelle,Scott A. Hale*

Main category: cs.CL

TL;DR: 研究发现语言突变显著延长了阴谋论在社交媒体上的持续传播时间，简化与同化是主要突变模式，建议内容审核聚焦核心主张。


<details>
  <summary>Details</summary>
Motivation: 理解阴谋论如何通过语言突变持续传播，以改善内容审核策略。

Method: 基于X平台三年数据，使用计算语言学分析和生存模型，辅以定性分析。

Result: 语义突变越大，阴谋论寿命越长；心理语言学特征和AAT类别的突变均与更长寿命相关。

Conclusion: 语言突变促使阴谋论长期存续，审核应考虑其可变性，聚焦核心主张以应对潜在变异。
...
### [256] [Mind Your Moras: Orthography-Aware Error Analysis of Neural Japanese Morphological Generation](https://arxiv.org/abs/2605.20043)
*Wen Zhang*

Main category: cs.CL

TL;DR: 本文针对日语过去时形态屈折，提出了一种正字法感知的错误分析方法，揭示了平假名正字法特性如何影响模型泛化。


<details>
  <summary>Details</summary>
Motivation: 现有研究常将平假名仅视为转录媒介，而本文旨在考察其作为编码形态音位区别的表征系统对模型学习的影响。

Method: 采用两种字符级序列到序列架构，使用SIGMORPHON 2020和2023共享任务格式的数据集，并引入包含七种主要失败模式的错误分类法。

Result: 模型虽整体准确率高，但表现出系统性错误，其中双辅音化相关错误占残差错误的75-80%，尤其集中于以e结尾的动词。

Conclusion: 错误模式在架构和随机种子间高度一致，表明正字法、形态结构和数据频率的交互作用显著影响模型泛化，并强调了正字法感知评估的必要性。
...
### [257] [PromptRad: Knowledge-Enhanced Multi-Label Prompt-Tuning for Low-Resource Radiology Report Labeling](https://arxiv.org/abs/2605.20052)
*Ying-Jia Lin,Tzu-Chin Lo,Ping-Chien Li,Chi-Tung Cheng,Chien-Hung Liao,Hung-Yu Kao*

Main category: cs.CL

TL;DR: 提出PromptRad，一种基于知识增强的多标签提示调优方法，在低资源下进行放射学报告标注。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则和微调的方法在临床报告标注中面临数据不足和多样性挑战，需要少量标注数据的高效方法。

Method: 将多标签分类重构为掩码语言建模，集成UMLS同义词到多词言语机中，通过提示调优微调预训练语言模型。

Result: 在肝脏CT报告上仅用32个标注样本即优于字典和微调基线，与GPT-4性能相当，且更有效处理复杂否定模式。
...
### [258] [Rewarding Beliefs, Not Actions: Consistency-Guided Credit Assignment for Long-Horizon Agents](https://arxiv.org/abs/2605.20061)
*Wenjie Tang,Minne Li,Sijie Huang,Liquan Xiao,Yuan Zhou*

Main category: cs.CL

TL;DR: 提出ReBel算法，通过显式建模信念状态解决部分可观察环境中的时间信用分配问题，在ALFWorld和WebShop上任务成功率提升20.4%，样本效率提高2.1倍。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法在部分可观察环境和延迟奖励下，代理信念漂移和信用分配困难，需要更有效的解决方法。

Method: 构建可学习的信念状态总结交互历史，利用信念一致性监督生成密集自监督信号，并采用信念感知分组实现更稳健的优势估计。

Result: 在ALFWorld和WebShop上相比GRPO基线，任务成功率最高提升20.4个百分点，样本效率提升2.1倍。

Conclusion: 信念感知的自监督学习是提升部分可观察环境下长程决策可靠性的有效方向。
...
### [259] [Text-to-SPARQL Generation with Reinforcement Learning: A GRPO-based Approach on DBLP](https://arxiv.org/abs/2605.20066)
*Jann Pfeifer,Debayan Banerjee,Ricardo Usbeck*

Main category: cs.CL

TL;DR: 本研究探讨了基于结果的强化学习（GRPO）在小规模指令调优语言模型上进行零样本Text-to-SPARQL生成的有效性，发现其能显著提升性能，但监督式精细调优（DoRA）在总体准确率上更优。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱问答方法依赖大模型或完整的查询标注（金标准查询），而实际场景中这类标注难以获取，因此需要研究在无金标准查询的情况下如何训练模型。

Method: 采用组相对策略优化（GRPO）对Qwen3-1.7B模型进行强化学习，输入包含自然语言问题和实体关系符号提示，训练基于执行反馈、结构约束和答案级奖励，并对比了一个引入金标准查询塑形的变体。

Result: GRPO显著优于零样本基线，并表现出有竞争力的泛化能力；监督式DoRA精细调优在相同模型规模上达到更高总体准确率；消融实验显示执行奖励贡献了大部分提升，额外塑形收益有限。

Conclusion: 当金标准查询不可用于令牌级监督时，基于结果的强化学习是一种可行的训练策略。
...
### [260] [BalanceRAG: Joint Risk Calibration for Cascaded Retrieval-Augmented Generation](https://arxiv.org/abs/2605.20084)
*Zijun Jia,Yuanchang Ye,Sen Jia,Yiyao Qian,Haoning Wang,Baojie Chen,Diyin Tang,Jinsong Yu,Zhiyuan Wang*

Main category: cs.CL

TL;DR: 提出BalanceRAG框架，通过联合不确定性阈值校准优化级联RAG的准确性与效率，在保证系统级错误率的同时减少不必要的检索调用。


<details>
  <summary>Details</summary>
Motivation: 传统级联RAG逐阶段校准阈值可能过于保守，忽略了两阶段联合不确定性对最终效用的影响。

Method: 将阈值对视为二维网格上的操作点，通过序列图检验识别安全操作点，实现风险自适应的系统级错误率控制。

Result: 在多个开放域问答基准上，BalanceRAG在达到预设风险水平的同时保持更高覆盖率与正确接受例数，且减少不必要的检索。

Conclusion: BalanceRAG通过联合阈值校准有效平衡了级联RAG的准确性与效率，优于始终启用RAG的方法。
...
### [261] [CopT: Contrastive On-Policy Thinking with Continuous Spaces for General and Agentic Reasoning](https://arxiv.org/abs/2605.20075)
*Dachuan Shi,Hanlin Zhu,Xiangchi Yuan,Wanjia Zhao,Kejing Xia,Wen Xiao,Wenke Lee*

Main category: cs.CL

TL;DR: 提出CopT，一种反转推理顺序的范式，先给出草稿答案再思考修正，通过对比嵌入作为验证器评估可靠性，在不额外训练的情况下提升准确率并降低token消耗。


<details>
  <summary>Details</summary>
Motivation: 标准CoT在思考前回答会导致表演性推理，延迟答案产生并浪费token，促使探索更高效的推理顺序。

Method: CopT先获取草稿答案，然后基于该答案进行后续思考（on-policy thinking）来反思和修正。通过对比连续嵌入与离散token输入下模型对相同生成token的支持度，构造序列级反向KL估计器来衡量答案可靠性，并在不可靠时进行进一步思考，同时用第二个KL估计器动态控制草稿答案的可见性。

Result: 在数学、编码和智能体推理任务中，CopT将峰值准确率提升最高23%，在保持相近或更高准确率时最多减少57%的token使用。

Conclusion: CopT通过反转推理顺序和嵌入对比验证，有效减少了不必要的思考开销，提升了LLM推理效率和准确性，且无需额外训练。
...
### [262] [ThoughtTrace: Understanding User Thoughts in Real-World LLM Interactions](https://arxiv.org/abs/2605.20087)
...
TL;DR: 提出了ThoughtTrace，首个大规模真实人类-AI对话与用户思维标注数据集，包含1058用户、2155对话、17058轮次和10174个思维注释。


<details>
  <summary>Details</summary>
Motivation: 现有对话数据集仅记录用户说出的内容，缺乏捕捉用户隐藏思维（如发送提示的原因或对回复的反应）的能力，限制了AI对用户深层目标的理解。

Method: 通过收集用户与20种语言模型的多轮对话，并标注用户的思维（理由和反应），构建了ThoughtTrace数据集，并验证了思维的独特性、语义差异性及与对话阶段的关联性。

Result: 思维作为推理上下文可提升用户行为预测；思维引导的文本改写为个性化助手训练提供了细粒度对齐信号。
...
### [263] [MixRea: Benchmarking Explicit-Implicit Reasoning in Large Language Models](https://arxiv.org/abs/2605.20128)
*Yuanqing Cai,Ziyi Huang,Minhao Liu,Lixin Duan,Wen Li,Yanru Zhang*

Main category: cs.CL

TL;DR: 本文研究了大语言模型在任务指令下忽略微妙但重要上下文线索的问题，即注意力盲视，并提出了MixRea基准和PRCP方法。


<details>
  <summary>Details</summary>
Motivation: 受人类认知中"非注意盲视"理论的启发，探究LLMs是否因训练于嵌入注意力偏见的人类偏好语料而产生类似的注意限制。

Method: 创建包含2246道多选题的MixRea基准，覆盖9种推理类型；提出PRCP提示方法，通过恢复被忽略的因果关系改善推理。

Result: 评估21个先进LLMs，最佳模型Gemini 2.5 Pro仅有42.8%的一致性，揭示普遍存在的"非注意盲视";PRCP方法可缓解该问题。

Conclusion: LLMs存在类似人类注意盲视的认知限制，需开发更具认知对齐的模型。
...
### [264] [Less Back-and-Forth: A Comparative Study of Structured Prompting](https://arxiv.org/abs/2605.20149)
*Saurav Ghosh,Gabriella Polach,Abdou Sow*

Main category: cs.CL

TL;DR: 该论文研究了结构化提示设计是否能在减少用户努力的同时提高大语言模型的响应质量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在开放式任务中因提示不明确导致低质量答案和额外交互，因此需要探索改进方法。

Method: 比较了三种提示条件（原始提示、检查清单改进提示、澄清问题提示），在四个任务类型（摘要、规划、解释、编码）上使用三个LLM系统（ChatGPT、Claude、Grok）评估，采用统一评分标准。

Result: 检查清单改进提示的平均评分最高（7.50/8），高于原始提示（5.67/8）和澄清问题提示（6.67/8），且平均令牌数更少。

Conclusion: 简单的检查清单提示可以在减少不必要交互的同时提高LLM响应质量。
...
### [265] [KoRe: Compact Knowledge Representations for Large Language Models](https://arxiv.org/abs/2605.20170)
*Davide Cavicchini,Fausto Giunchiglia,Jacopo Staiano*

Main category: cs.CL

TL;DR: 提出KoRe方法，将知识图谱的1-hop子图编码为紧凑的离散知识令牌并注入大语言模型，在三个基准测试中取得竞争性表现，并将令牌使用量减少多达10倍。


<details>
  <summary>Details</summary>
Motivation: 现有LLM通过参数编码世界知识，存在不透明、难以调试更新且易产生幻觉的问题，而知识图谱虽能提供可读且易编辑的知识表示，但其当前集成技术需要大量重新训练或微调。

Method: KoRe方法：将1-hop子图编码为离散知识令牌，并注入LLM主干网络。

Result: 在三个基准测试中取得竞争性表现，且令牌使用量显著减少（最高10倍）。

Conclusion: 紧凑的离散知识图谱表示能有效且高效地用于增强现代LLM的 grounding。
...
### [266] [ClinSeekAgent: Automating Multimodal Evidence Seeking for Agentic Clinical Reasoning](https://arxiv.org/abs/2605.20176)
*Juncheng Wu,Letian Zhang,Yuhan Wang,Haoqin Tu,Hardy Chen,Zijun Wang,Cihang Xie,Yuyin Zhou*

Main category: cs.CL

TL;DR: 本文提出ClinSeekAgent，一个主动收集多模态临床证据的智能框架，在文本和影像任务上显著提升了多个大模型的性能，并通过知识蒸馏使小模型接近顶尖模型水平。


<details>
  <summary>Details</summary>
Motivation: 现有临床决策支持方法假设证据已预先整理好，但实际临床工作流需要主动搜索、迭代规划和综合来自异构源的多模态证据。

Method: ClinSeekAgent根据临床查询主动查询医学知识库、浏览原始电子病历、调用医学影像工具，动态收集并整合证据，支持推理时和训练时两种模式。

Result: 在文本任务上，Claude Opus 4.6 F1从60.0提升至63.2，MiniMax M2.5从43.1提升至47.3；多模态任务上，Claude Opus 4.6 F1从47.5提升至62.6；蒸馏模型ClinSeek-35B-A3B平均F1达34.0，比基线高11.9分，接近Claude Opus 4.6。

Conclusion: ClinSeekAgent能有效将被动证据消费转变为主动证据获取，显著提升模型在真实临床决策中的表现，并通过蒸馏使小模型受益。
...
### [267] [From Seeing to Thinking: Decoupling Perception and Reasoning Improves Post-Training of Vision-Language Models](https://arxiv.org/abs/2605.20177)
*Juncheng Wu,Hardy Chen,Haoqin Tu,Xianfeng Tang,Freda Shi,Hui Liu,Hanqing Lu,Cihang Xie,Yuyin Zhou*

Main category: cs.CL

TL;DR: 本文系统研究了VLM后训练中感知与推理的相互作用，提出分阶段训练方法，先优化视觉感知再训练推理，相比混合训练提升了准确率并缩短了推理链。


<details>
  <summary>Details</summary>
Motivation: 发现VLM在视觉任务上的性能瓶颈主要是视觉感知不足，而非推理能力有限。

Method: 将VLM后训练分解为视觉感知、视觉推理和文本推理三个阶段，分别使用专门数据，并采用强化学习而非基于描述的监督微调。

Result: 分阶段训练在多个VLM上一致提升感知和推理性能，准确率提高1.5%且推理链缩短20.8%；在视觉数学和感知任务上分别提升5.2%和3.7%。

Conclusion: 基于能力的课程学习与传统基于难度的课程学习正交，结合两者可进一步获益。
...
### [268] [TIDE: Efficient and Lossless MoE Diffusion LLM Inference with I/O-aware Expert Offload](https://arxiv.org/abs/2605.20179)
*Zhiben Chen,Youpeng Zhao,Yang Sui,Jun Wang,Yuzhang Shang*

Main category: cs.CL

TL;DR: 提出了TIDE，一种利用专家激活时间稳定性的无损推理系统，在GPU-CPU系统上实现dLLM的1.4-1.5倍吞吐加速。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在资源受限设备上的部署面临挑战，现有AR方法存在I/O开销或计算瓶颈。

Method: 利用块内扩散过程中专家激活的时间稳定性，引入基于间隔的专家刷新策略，并通过数学规划优化推理调度。

Result: 在单GPU-CPU系统上，LLaDA2.0-mini和LLaDA2.0-flash模型分别实现1.4倍和1.5倍的吞吐量提升。

Conclusion: TIDE是一种无损、无需训练的优化方法，可显著加速dLLM推理。
...
### [269] [RLFTSim: Realistic and Controllable Multi-Agent Traffic Simulation via Reinforcement Learning Fine-Tuning](https://arxiv.org/abs/2605.19033)
*Ehsan Ahmadi,Hunter Schofield,Behzad Khamidehi,Fazel Arasteh,Jinjun Shan,Lili Mou,Dongfeng Bai,Kasra Rezaee*

Main category: cs.RO

TL;DR: 提出RLFTSim框架，通过强化学习微调提升交通模拟的真实性与可控性，在Waymo数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 监督式开环训练无法捕捉复杂驾驶场景中的动态多智能体交互，导致模拟与现实分布不一致。

Method: 在预训练模型上通过强化学习微调，设计平衡保真度和可控性的奖励函数，并采用低方差密集奖励信号减少样本需求。

Result: 在Waymo数据集上实现SOTA真实性，样本效率显著优于启发式搜索方法，并成功蒸馏出目标条件可控性。

Conclusion: RLFTSim有效解决了模拟保真度对齐问题，同时为场景生成提供可控性蒸馏方法。
...
### [270] [Distributionally Robust Control via Stein Variational Inference for Contact-Rich Manipulation](https://arxiv.org/abs/2605.19029)
*Hrishikesh Sathyanarayan,Victor Vantilborgh,Harish Ravichandar,Tom Lefebvre,Ian Abraham*

Main category: cs.RO

TL;DR: 提出一种基于Stein变分推断的分布鲁棒控制方法，在接触丰富的操作任务中显式建模参数不确定性，在保持性能的同时提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法依赖大量训练数据，而模型基方法对不确定性建模能力有限，影响接触丰富操作中的性能。

Method: 将操作问题建模为分布鲁棒控制优化，利用Stein变分推断导出确定性公式，显式建模对任务敏感的参数不确定性。

Result: 在多种接触丰富操作任务中，参数不确定性下鲁棒性提升最高达3倍，优于现有模型基控制方法。

Conclusion: 通过更灵活的不确定性建模，模型基控制既能保持高性能，又能显式适应任务相关不确定性，显著提高可靠性。
...
### [271] [Adversarial Stress Testing of SPARK Humanoid Safety Filters](https://arxiv.org/abs/2605.19009)
*Saurav Ghosh,Abdou Sow,Luke Zhang*

Main category: cs.RO

TL;DR: 本文通过复现和压力测试，研究了SPARK人形安全过滤器的鲁棒性，发现不同方法在目标跟踪和碰撞避免上表现各异，且安全行为在障碍物密集等条件下会变化。


<details>
  <summary>Details</summary>
Motivation: 人形机器人因高维身体和碰撞约束难以安全部署，现有安全滤波器在复杂环境下的表现未被充分评估。

Method: 在MuJoCo中复现SPARK基准测试，评估六种安全过滤器，并构建后处理管道提取目标跟踪、最小距离和碰撞步数等指标。

Result: 不同方法在目标跟踪和碰撞避免间有取舍；压力测试显示障碍物拥挤、噪声和延迟信息会影响安全行为。

Conclusion: 人形机器人自主性应在名义性能之外，通过暴露失败模式的指标进行评估。
...
### [272] [Geo-Data-Driven HD Map Generation Workflow with Integrated Reference-Free Constraint-Based Verification](https://arxiv.org/abs/2605.18921)
*Ruidi He,Vaibhav Tiwari,Mohanad Al-Ghobari,Meng Zhang,Andreas Rausch*

Main category: cs.RO

TL;DR: 提出了一种基于地理数据的高清地图生成与验证工作流，使用开放地理工程数据生成车道路级地图，并通过约束验证确保质量。


<details>
  <summary>Details</summary>
Motivation: 传统高清地图生成依赖传感器和参考数据，成本高且在某些场景下不可行，因此需要一种低依赖性的替代方案。

Method: 利用开放地理工程数据（如shapefile），通过中间表示和显式处理阶段生成车道级地图，并集成基于约束的验证（如几何、拓扑和高度一致性检查）。

Result: 在实际道路数据上生成的HD地图满足约束条件，缺陷注入测试中完全检测到所有缺陷类型，无假阳性。

Conclusion: 该工作流可作为传感器密集型方法的互补方案，在减少传感和参考数据的情况下实现模块化、可检查的地图生成。
...
### [273] [KG-ASG: Collision-Knowledge-Guided Closed-Loop Adversarial Scenario Generation With Primary-Support Attribution](https://arxiv.org/abs/2605.18895)
*Cheng Wang,Chen Xiong,Ziwen Wang,Yuchen Zhou,Qiang Liu*

Main category: cs.RO

TL;DR: 提出KG-ASG框架，利用碰撞知识库和主从对抗机制生成可解释、可执行且多车交互可控的对抗场景，提升自动驾驶安全验证效果。


<details>
  <summary>Details</summary>
Motivation: 现有安全关键场景生成方法存在碰撞原因模糊、多车碰撞不可控以及可执行性低的问题，需要更鲁棒和可解释的验证方法。

Method: 构建结构化的碰撞知识库并训练轻量级碰撞专家模型，推断目标碰撞模式及车辆角色；在此基础上将多车对抗生成建模为主从过程，并集成规则、物理、交互安全及单碰撞器约束作为硬门控，结合规划器-控制器反馈进行诊断与优化。

Result: 在MetaDrive重构的WOMD场景上，KG-ASG相比基线方法在对抗有效性、有效主攻击率提升、多碰撞减少以及三种控制器下的闭环恢复方面均表现更优。

Conclusion: 碰撞知识指导与主从单碰撞器推理能够显著提升自动驾驶安全验证的对抗效果、可解释性和可执行性。
...
### [274] [Guiding Neuro-Symbolic Scenario Generation with Spatio-Temporal Logic](https://arxiv.org/abs/2605.19038)
*Lorenzo Bonin,Francesco Giacomarra,Luca Bortolussi,Jyotirmoy V. Deshmukh,Francesca Cairoli*

Main category: cs.RO

TL;DR: 本文提出STRELGen框架，结合扩散模型和时空逻辑规范，高效生成安全关键型自动驾驶场景。


<details>
  <summary>Details</summary>
Motivation: 传统自动驾驶测试依赖海量真实场景，成本高且难以捕获罕见的安全关键边缘案例。

Method: 融合多智能体轨迹生成扩散模型与可微分的时空逻辑（STREL）规范，通过梯度搜索优化潜空间以最大化逻辑满意度。

Result: 能高效生成既高度逼真又安全关键的多智能体场景，且位于学习数据分布内。

Conclusion: STRELGen提供了灵活、可解释且强大的自动驾驶系统压力测试工具，克服了暴力数据收集的局限。
...
### [275] [Neural Operators for Design-Space Surrogate Modeling of Tendon-Actuated Continuum Robots](https://arxiv.org/abs/2605.19104)
*Branden Frieden,James M. Ferguson,Alan Kuntz,Varun Shankar*

Main category: cs.RO

TL;DR: 本文提出了一种基于算子学习的连续体机器人代理建模方法，通过四种神经算子架构（DeepONet和FNO）实现跨设计的快速准确配置预测。


<details>
  <summary>Details</summary>
Motivation: 传统物理模型计算昂贵且存在未建模误差，现有数据驱动方法泛化能力差，需要一种能跨机器人设计泛化的高效模型。

Method: 将肌腱驱动连续体机器人的代理建模形式化为算子学习问题，设计四种神经算子架构（两种DeepONet和两种FNO），在仿真数据上训练以预测机器人构型。

Result: 所有架构均实现高精度预测，并能在不同机器人设计间快速准确泛化。

Conclusion: 算子学习为连续体机器人力学提供了有效且可泛化的代理模型，可加速外科和工业应用中的控制、规划与设计优化。
...
### [276] [CosFly: Plan in the Matrix, Fly in the World](https://arxiv.org/abs/2605.19120)
...
TL;DR: CosFly是一个模块化七步构建管道，用于无人机空中跟踪的规划和多模态仿真，配套CosFly-Track数据集包含250条轨迹和约10万张带六自由度位姿标注的图像。


<details>
  <summary>Details</summary>
Motivation: 现有无人机跟踪数据缺乏结构化规划和多模态仿真支持，且对环境多样性和实用化模拟的覆盖不足。

Method: 提出基于CARLA的构建管道，包含3D地图导出、网格简化、轨迹规划（两阶段或直接梯度优化）、多模态渲染（RGB/深度/语义掩码）、质量检查及教师-学生描述生成；可配置固定FOV缩放。

Result: 生成了250条验证轨迹和约10万张带六自由度位姿（位置+姿态角）的多模态图像，覆盖城市、高速公路、乡村、森林和海岸环境。

Conclusion: CosFly管道和CosFly-Track数据集为空中-地面协作研究（动态跟踪、导航、多模态感知）提供了可扩展的基础。
...
### [277] [Automatically Improving Simulation Physics for Articulated Objects](https://arxiv.org/abs/2605.19136)
*Anh-Quan Pham*

Main category: cs.RO

TL;DR: 提出了交互就绪性概念，通过多模态模拟器反馈方法从不完整3D资产生成高质量可仿真铰接物体。


<details>
  <summary>Details</summary>
Motivation: 现有3D数据集缺乏物理属性，导致仿真物体需要大量人工调整，缺少系统性评估方法。

Method: 将交互就绪性分解为可测量组件，结合几何、视觉、语义信息推断物理属性，并通过模拟器迭代反馈优化物理一致性。

Result: 通过多领域物体和操控任务验证，物体质量直接影响仿真稳定性与策略性能，所提方法生成的物体具有更稳定和真实的动力学特性。

Conclusion: 物理真实性对铰接物体仿真至关重要，所提多模态反馈方法可规模化构建高质量仿真物体。
...
### [278] [COBALT: Crowdsourcing Robot Learning via Cloud-Based Teleoperation with Smartphones](https://arxiv.org/abs/2605.19138)
...
TL;DR: 提出了COBALT，一个可扩展的远程操作平台，通过矢量化环境和多用户并发支持，降低了模仿学习在机器人操作中数据收集的成本和门槛。


<details>
  <summary>Details</summary>
Motivation: 大规模、高质量的示范数据稀缺，限制了模仿学习在机器人操作中的扩展。

Method: 利用矢量化环境、负载均衡基础设施、多设备支持（如智能手机、VR头显）、内存数据缓存和高效视频流，实现多用户并发远程操作。

Result: 在8个GPU上支持256个模拟客户端，用户研究显示手机远程操作性能与专用硬件相当或更优，收集了超过7500个示范（50小时以上）的数据集，并验证了数据质量。

Conclusion: COBALT能够显著降低远程操作成本，实现大规模、高质量数据收集，并提升模仿学习算法的性能。
...
### [279] [A Heuristic Approach for Performance Tuning in RL-based Quadrotor Control via Reward Design and Termination Conditions](https://arxiv.org/abs/2605.19166)
*Fausto Mauricio Lagos Suarez,Akshit Saradagi,Vidya Sumathy,George Nikolakopoulos*

Main category: cs.RO

TL;DR: 本文提出了一种启发式方法，通过奖励设计和终止条件实现强化学习四旋翼控制的可调性能，在6百万时间步内实现基线临界阻尼响应和约2%稳态误差，并支持快速（特技）和慢速（巡检）模式调节。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的四旋翼控制注重速度和敏捷性，但在基础设施巡检等需要精确、可控机动并调节性能的应用中存在不足。

Method: 设计了一种双带宽指数奖励结构，结合近端策略优化算法和回合截断条件，并通过启发式规则调整奖励权重和指数系数以调节性能。

Result: 在100次试验中，三种策略（基线、特技、巡检）实现了位置和偏航跟踪的可调性能，保持临界阻尼响应和约2%稳态误差。

Conclusion: 所提出的启发式方法能有效实现强化学习四旋翼控制的可调性能，满足不同应用需求。
...
### [280] [Aerial Inspection Behaviors via RL-based Quadrotor Control for Under-canopy Forest Environments](https://arxiv.org/abs/2605.19202)
*Fausto Mauricio Lagos Suarez,Akshit Saradagi,Vidya Sumathy,Viswa Narayanan Sankaranarayanan,George Nikolakopoulos*

Main category: cs.RO

TL;DR: 本文提出了一种基于深度强化学习的四旋翼控制器，结合TSP和RRT*规划器，实现了森林冠层下的自主导航与目标检测位姿跟踪。


<details>
  <summary>Details</summary>
Motivation: 解决在森林冠层环境下使用深度强化学习控制器进行远程自主检测任务时，需要兼顾安全性和可靠性的问题。

Method: 采用端到端的状态到转速映射控制策略，上层结合TSP规划最优访问顺序，RRT*生成无碰撞路径并满足下层控制器的跟踪限制。

Result: 通过五个检测场景验证，强化学习电机级稳定控制器在导航层的支持下可有效用于森林冠层检测任务。

Conclusion: 深度强化学习控制器结合导航规划层，能够安全可靠地执行远程森林冠层自主检测任务。
...
### [281] [CLUE: Adaptively Prioritized Contextual Cues by Leveraging a Unified Semantic Map for Effective Zero-Shot Object-Goal Navigation](https://arxiv.org/abs/2605.19206)
*Taeyun Kim,Alvin Jinsung Choi,Dasol Hong,Hyun Myung*

Main category: cs.RO

TL;DR: CLUE通过自适应平衡房间和物体上下文线索，利用LLM的常识知识，在零样本目标导航中显著提升了成功率和路径效率。


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标导航方法未区分房间类型和共存物体对不同目标的重要性差异，导致探索不准确、效率低。

Method: 利用离线大语言模型估计目标与房间类型的关联度，自适应地加权房间线索和物体线索，构建统一语义价值地图，并融合多视角验证与上下文感知探索策略。

Result: 在仿真和真实世界实验中，CLUE在成功率和路径加权成功率上均优于当前最先进基线方法。

Conclusion: 自适应平衡房间与物体上下文线索可有效提升零样本目标导航的鲁棒性和实用性。
...
### [282] [Graph Neural Planning and Predictive Control for Multi-Robot Communication-Constrained Unlabeled Motion Planning](https://arxiv.org/abs/2605.19209)
*Manohari Goarin,Yang Zhou,Giuseppe Loianno*

Main category: cs.RO

TL;DR: 提出结合图注意力规划器（GATP）与分散非线性模型预测控制（NMPC）的分层框架，解决多机器人未标记运动规划问题，兼顾可扩展性与现实部署中的动态可行性及通信约束。


<details>
  <summary>Details</summary>
Motivation: 现有基于图神经网络的方法依赖简化动力学和仿真环境，未考虑动态可行性和通信延迟等现实挑战。

Method: GATP作为上层规划器，通过注意力机制生成中间子目标；下层NMPC确保在非线性动力学和驱动约束下的安全性，并将推理分散到各机器人。

Result: 在仿真和四旋翼实物实验中，该方法展现出对更大规模团队的泛化能力、抗200ms通信延迟能力以及分散式机载推理的可行性。

Conclusion: 该分层框架有效弥合了图神经网络规划与真实多机器人部署之间的差距，具有实际应用潜力。
...
### [283] [Bilateral Teleoperation with Compliant 6-DOF Pose-and-Force Sensing](https://arxiv.org/abs/2605.19255)
*Yue Feng,Weicheng Huang,I-Ming Chen*

Main category: cs.RO

TL;DR: 提出基于低成本柔性传感器的笛卡尔双边遥操作框架，通过三时间尺度解耦实现异构机械臂的稳定控制。


<details>
  <summary>Details</summary>
Motivation: 现有双边遥操作平台依赖昂贵刚性传感器和紧耦合硬件，需要简化系统并降低成本。

Method: 使用低成本的Delta6末端执行器作为SEA，领导端采用阻尼-导纳控制，跟随端实现刚度-阻尼阻抗控制，通过三时间尺度解耦实现硬件无关性。

Result: 在Lite6/FR3平台上以150Hz运行，在120±40ms延迟和1%丢包下稳定，接触刚度匹配，能量签名满足无源性要求。

Conclusion: 提出的框架能有效降低双边遥操作系统的硬件成本，同时保证稳定性和性能。
...
### [284] [PRISM-SLAM: Probabilistic Ray-Grounded Inference for Scale-aware Metric SLAM](https://arxiv.org/abs/2605.19257)
*Eunsoo Im*

Main category: cs.RO

TL;DR: PRISM-SLAM是一个利用视觉基础模型（VFM）先验知识的单目SLAM框架，通过贝叶斯因子图实现尺度感知和度量一致的定位与建图，在动态环境中鲁棒运行，并以30 FPS输出度量轨迹。


<details>
  <summary>Details</summary>
Motivation: 单目SLAM存在尺度模糊和动态环境跟踪失效问题；现有VFM预测缺乏不确定性和帧间尺度一致性。

Method: 引入Plücker射线距离因子锚定度量尺度；提出动态场景不确定性门控（DSUG）软抑制动态干扰；采用多进程异步处理VFM推理与几何跟踪。

Result: 在TUM RGB-D和7-Scenes上，度量SE(3)绝对轨迹误差（ATE）接近Sim(3)误差，实现无需事后尺度纠正的度量SLAM。

Conclusion: PRISM-SLAM桥接了基础模型与机器人应用，提供实时、鲁棒、度量一致的SLAM解决方案。
...
### [285] [DEFLECT: Delay-Robust Execution via Flow-matching Likelihood-Estimated Counterfactual Tuning for VLA Policies](https://arxiv.org/abs/2605.19294)
*Yixiang Zhu,Yonghao Chen,Rui Meng,Jingyu Guo,Jiaxiang Zou,Zijie Yang,Taowen Wang,Xinyu Chen*

Main category: cs.RO

TL;DR: 本文提出DEFLECT，一种离线后训练方法，通过将延迟转化为偏好信号，解决异步VLA策略中的预测-执行错位问题，显著提升高延迟场景下的成功率。


<details>
  <summary>Details</summary>
Motivation: 异步VLA策略中，动作块基于推理前的观测生成，但执行时物理状态已偏移，导致成功率显著下降（如Kinetix从89%降至1%以下）。

Method: 将延迟转化为无标签偏好信号：从冻结参考策略构建反事实的新鲜/陈旧动作对，使用隐式流匹配似然比代理进行评分，无需人工标注、奖励模型或在线部署。

Result: 在5-7控制步的高延迟场景下成功率提升6.4个百分点；迁移至真实尺度VLA时，最长延迟下提升4.6个百分点；在真实机器人任务（双臂传送带抓取和打地鼠）中有一致改进。

Conclusion: DEFLECT可作为现有异步VLA框架的即插即用升级，有效扩展可用延迟范围，提升鲁棒性。
...
### [286] [ContextFlow: Hierarchical Task-State Alignment for Long-Horizon Embodied Agents](https://arxiv.org/abs/2605.19314)
*Shuhan Guo,Kun Zhang,Haifei Liu,Xingyu Gao,Yongqi Zhang,Yaqing Wang,Quanming Yao*

Main category: cs.RO

TL;DR: 本文提出ContextFlow框架，通过显式合同、证据包和范围更新解决长期任务中的状态不一致问题。


<details>
  <summary>Details</summary>
Motivation: 专家执行器越强，任务计划、监控、记忆和执行之间的协调瓶颈越突出，任务状态不一致常导致规划错误。

Method: 将任务阶段表示为显式合同，运行时观察转化为证据包，应用继续、细化、转移、提升和修复等范围更新。

Result: 在长期具身任务实验中，证据驱动的范围更新有效诊断并缓解了任务状态失败。

Conclusion: ContextFlow使任务前沿对齐显式且可审计，保持专家执行器局部闭环控制，提升了任务连贯性。
...
### [287] [Beyond Waypoints: Dual-Heatmap Grounding for Cross-Embodiment Semantic Navigation](https://arxiv.org/abs/2605.19420)
*Kaijie Yun,Yue Chen*

Main category: cs.RO

TL;DR: 提出双热图框架，将开放语义指令转化为可执行的导航区域，替代单点回归，提升多机器人平台的执行成功率。


<details>
  <summary>Details</summary>
Motivation: 现有导航框架单点回归导致不可执行的目标点，如非可穿越物体中心，造成执行失败。

Method: 使用视觉-语言模型预测导航可穿越热图和朝向热图，形成可微语义势场，结合自动合成数据管道和仿真基准训练。

Result: 在8B参数模型中达到最优性能，显著提高可穿越率，并在多种机器人平台上验证有效性。

Conclusion: 双热图表示能有效缓解单点回归的脆弱性，为跨体感安全导航提供可迁移路径。
...
### [288] [Hamilton--Jacobi Reachability for Spacecraft Collision Avoidance](https://arxiv.org/abs/2605.20138)
*Larry Hui,Jordan Kam,William Su,Jianshu Zhou*

Main category: cs.RO

TL;DR: 提出基于Hamilton-Jacobi可达性框架的两卫星碰撞规避方法，在RTN坐标系中用HCW动力学建模相对运动，通过零和微分博弈计算后向可达集，确保最坏扰动下的碰撞安全性。


<details>
  <summary>Details</summary>
Motivation: 解决同一圆轨道上两卫星碰撞规避问题，需满足FCC轨道标准的最小间距要求，并考虑对方卫星的未知意图扰动。

Method: 采用Hamilton-Jacobi可达性框架，将问题建模为零和微分博弈，通过计算后向可达集区分可规避与不可规避状态，结合混合监控逻辑触发规避机动。

Result: 定义了不安全相对状态的可达集，证明可达集外存在无碰撞轨迹，提供数学保证的安全边界。

Conclusion: 该方法能提供可扩展的安全保证，通过混合控制逻辑在确定性触发的规避机动中实现避碰。
...
### [289] [Self-assembling Modular Aerial Robot for Versatile Aerial Tasks](https://arxiv.org/abs/2605.19431)
*Junichiro Sugihara,Masaki Kitagawa,Jinjie Li,Yunong Li,Takuzumi Nishio,Kei Okada,Moju Zhao*

Main category: cs.RO

TL;DR: 提出LEGION模块化空中机器人单元，通过飞行中自组装实现集体操作，兼顾灵活机动与鲁棒操控。


<details>
  <summary>Details</summary>
Motivation: 现有空中机器人在灵活导航与强鲁棒操作之间存在设计权衡，受蚂蚁自组织集体启发，探索可重构模块化方案。

Method: 设计LEGION单元：双端关节接口实现端到端自组装，通过接触力/力矩控制保持零间隙互锁，支持飞行中自主对接与形态切换。

Result: 成功实现多单元空中自主对接，完成推、拉、旋转、抓取、携带等核心操作，并可在灵活个体飞行与集体操作间切换。

Conclusion: LEGION的自组织能力使空中机器人从被动观察者转变为主动环境参与者，拓展了空中物理交互的适用范围。
...
### [290] [Neuromorphic Control of a Flapping-Wing Robot on Resource-Constrained Hardware](https://arxiv.org/abs/2605.19430)
*Rim El Filali,Chenrui Feng,Chao Gao,Weibin Gu*

Main category: cs.RO

TL;DR: 本文提出了一个分层神经形态控制框架，在成本约5美元的ESP32微控制器上实现了扑翼微型飞行器的全机载闭环飞行，使用两个轻量级脉冲神经网络分别进行状态估计和控制，相比传统人工神经网络减少了36%的延迟和18%的功耗。


<details>
  <summary>Details</summary>
Motivation: 扑翼微型飞行器具有非线性动力学和严格的尺寸、重量和功率限制，而现有的控制方法难以在低成本、资源受限的硬件上实现全机载自主飞行。

Method: 设计了一个分层神经形态控制框架，在ESP32上部署两个脉冲神经网络：一个用于从原始传感器反馈中进行状态估计，另一个通过调制中央模式发生器来控制机翼驱动；通过模仿学习进行训练。

Result: 在无系留实际飞行中实现了稳定的俯仰和偏航角跟踪；SNN推理延迟从1059微秒降至680微秒（减少36%），功耗从0.033W降至0.027W（减少18%）。

Conclusion: 这项工作首次展示了扑翼微型飞行器上全机载神经形态控制，证明了在无需专用硬件的情况下，脉冲神经网络能在严格的SWaP约束下实现能效自主飞行。
...
### [291] [Closed-Loop Hybrid Digital Twin Platform for Connected and Automated Vehicle Validation](https://arxiv.org/abs/2605.19490)
*Kanglong Quan,Zhebing Xia,Linfeng Jiang,Hao Yu,Ziheng Qiao,Dapeng Dong,Dongyao Jia*

Main category: cs.RO

TL;DR: 提出一种实时混合数字孪生平台，通过低延迟V2X通信将高保真CARLA-SUMO联合仿真与实车测试集成，实现联网自动驾驶汽车的高效闭环验证。


<details>
  <summary>Details</summary>
Motivation: 现有仿真测试缺乏与实际车辆和现场数据的无缝集成，无法捕捉动态真实交互。

Method: 构建CARLA-SUMO联合仿真与物理测试场/车辆的耦合系统，通过自定义中间件同步真实车辆运动状态并转换CAN控制指令，使用摄影测量和云边协同架构。

Result: 实验显示稳定同步、有效闭环控制和低延迟，证明平台在多场景验证中的实用性。

Conclusion: 该平台弥合了仿真与实车测试之间的差距，为联网自动驾驶汽车提供了可扩展、高保真的验证方案。
...
### [292] [CANINE: Coaching Visually Impaired Users for Interactive Navigation with a Robot Guide Dog](https://arxiv.org/abs/2605.19501)
*Cunjun Yu,Zishuo Wang,Anxing Xiao,Linfeng Li,David Hsu*

Main category: cs.RO

TL;DR: 提出CANINE自动化教练系统，通过个性化自适应语音反馈训练用户与机器人导盲犬的交互导航技能，经实验证明其比通用语音指令学习效率更高、导航表现更优。


<details>
  <summary>Details</summary>
Motivation: 机器人导盲犬需人机协调，但用户难以从通用语音指令中学会有效协作。

Method: 将协调任务分解为子技能，高层用知识追踪确定薄弱技能并优先训练，低层用基础模型分析错误原因并生成针对性语音纠正。

Result: 盲人替代者实验显示CANINE显著提升学习效率和最终导航表现，两周后保留研究证实技能改善持久，视觉受损用户案例确认有效性。

Conclusion: CANINE通过自适应训练有效提升人机协调导航能力，为实际部署提供设计参考。
...
### [293] [SafeAlign-VLA: A Negative-Enhanced Safe Alignment Framework for Risk-Aware Autonomous Driving](https://arxiv.org/abs/2605.19524)
*Kefei Tian,Yuansheng Lian,Kai Yang,Xiangdong Chen,Shen Li*

Main category: cs.RO

TL;DR: 提出SafeAlign-VLA框架，通过引入负样本实现自动驾驶的安全对齐，在NAVSIM和DeepAccident上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型依赖正样本，缺乏对危险行为和安全边界的理解，导致处理长尾安全场景能力不足。

Method: 反事实安全配对生成结构化标签和正轨迹；两阶段训练：负增强监督微调 + 基于锚点的组相对策略优化。

Result: NAVSIM上PDMS达89.1（提升1.3%），DeepAccident碰撞率降至3.36%，语言与风险预测准确率分别为84.2%和85.8%。

Conclusion: 负增强安全对齐框架有效提升了自动驾驶的安全性和鲁棒性。
...
### [294] [ARC-RL: A Reinforcement Learning Playground Inspired by ARC Raiders](https://arxiv.org/abs/2605.19503)
*Carlo Romeo,Andrew D. Bagdanov*

Main category: cs.RO

TL;DR: 本文提出了ARC-RL，一个包含四种仿生机器人形态的MuJoCo连续控制环境套件，用于研究强化学习在游戏NPC运动生成中的表现，并提供了统一的奖励函数和手动设计的中央模式生成器。


<details>
  <summary>Details</summary>
Motivation: 现有腿式机器人强化学习研究多基于真实硬件形态和sim-to-real目标，缺乏对游戏NPC中常见风格约束和非真实形态的适应，导致方法难以直接迁移。

Method: 设计了四种机器人形态（Queen、Bastion、Tick、Leaper），共享统一的观测模板、动作约定和奖励函数（仅权重不同），奖励函数包含速度追踪、生存奖励、步态阶段锁定、动作正则化、安全惩罚和姿态锚定，未使用动捕数据，并提供了手工制作的中央模式生成器作为专家参考。

Result: 通过对比SAC、SPEQ、SOPE-EO等在线算法和SACfD、SPEQ-O2O、SOPE等先验数据增强方法，展示了不同算法在形态多样性和风格约束下的表现差异。

Conclusion: ARC-RL为游戏NPC运动生成提供了标准化测试平台，揭示了在风格约束下不同强化学习范式的适应性，并为非真实形态的机器人控制提供了研究基础。
...
### [295] [Learning-Accelerated Optimization-based Trajectory Planning for Cooperative Aerial-Ground Handover Missions](https://arxiv.org/abs/2605.19562)
*Jingshan Chen,Bochen Yu,Henrik Ebel,Peter Eberhard*

Main category: cs.RO

TL;DR: 提出了一个学习增强的轨迹规划框架，用于无人机与无人车的协同交接任务，结合神经网络预测与集中优化，实现快速可靠的轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 集中式轨迹优化计算成本高，限制了实时应用。

Method: 使用解耦的编码器-解码器LSTM网络作为神经替代规划器，生成交接轨迹预测，作为集中优化器的热启动。

Result: 相比冷启动优化，实现了超过三倍的加速和100%的优化成功率。

Conclusion: 数据驱动推理与模型优化相结合，能快速可靠地生成异构多机器人系统的轨迹。
...
### [296] [PAPO-VLA: Planning-Aware Policy Optimization for Vision-Language-Action Models](https://arxiv.org/abs/2605.19580)
*Peizheng Guo,Jingyao Wang,Changwen Zheng,Wenwen Qiang*

Main category: cs.RO

TL;DR: 提出PAPO-VLA方法，识别并强化规划动作的优化，提升VLA模型在语言引导机器人任务中的可靠性。


<details>
  <summary>Details</summary>
Motivation: VLA模型在闭环交互中需同时承担规划与执行角色，但现有优化方法未明确识别规划动作或衡量其对任务成功的重要性。

Method: 基于动作变化与轨迹结果联合识别规划动作，通过因果充分性与必要性评估其重要性，并在GRPO优势估计中整合该权重。

Result: 在多个基准测试中验证了PAPO-VLA的有效性。

Conclusion: 通过显式强化规划动作优化可显著提升VLA策略的可靠性。
...
### [297] [Implicit Action Chunking for Smooth Continuous Control](https://arxiv.org/abs/2605.19592)
*Bosun Liang,Shuo Pei,Zirui Chen,Chuanzhi Fan,Chen Sun,Yuankai Wu,Huachun Tan,Yong Wang*

Main category: cs.RO

TL;DR: 提出了双窗平滑框架，通过执行窗和价值窗实现隐式动作分块，在不扩大动作空间的情况下实现平滑连续控制，在多个基准测试中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习常产生高频振荡控制信号，影响物理部署的安全性和稳定性。显式动作分块虽可解决此问题，但会导致输出维度随预测步长线性增长，带来优化困难和步态兼容性问题。

Method: 提出双窗平滑框架：执行窗通过确定性调制保证物理平滑；价值窗对齐时序差分目标以校正因开环执行导致的评论家偏差；并引入基于一阶动作差的轻量级正则化促进全局连续性。

Result: 在DeepMind控制套件、工业能源管理任务中表现优于现有方法；在基于视觉的复杂自动驾驶任务中实现了更平滑、更安全的控制，抖动减少且成功率达到100%。

Conclusion: 双窗平滑有效弥合了时序抽象与反应式步态控制之间的差距，为安全稳定的物理部署提供了新范式。
...
### [298] [MCNav: Memory-Aware Dynamic Cognitive Map for Zero-shot Goal-oriented Navigation](https://arxiv.org/abs/2605.19594)
*Jingyu Li,Zhe Liu,Wenxiao Wu,Li Zhang*

Main category: cs.RO

TL;DR: 提出了MCNav，一种具有动态认知地图的导航框架，通过记忆感知策略解决目标错过或误识别导致的导航失败问题。


<details>
  <summary>Details</summary>
Motivation: 现有零样本方法主要关注探索新区域，缺乏对已探索区域信息的利用，导致目标错过或误识别时导航失败。

Method: 构建动态认知地图存储查询信息，并引入目标重验证、错过目标重探索两种记忆感知策略，辅以黑名单和双重检查机制。

Result: 在HM3Dv1和HM3Dv2数据集上，MCNav在三个任务中达到最先进性能，尤其在实例级目标导航任务上表现突出。

Conclusion: 记忆感知框架有效提升了复杂环境中实例级目标导航的可靠性。
...
### [299] [FlyMirage: A Fully Automated Generation Pipeline for Diverse and Scalable UAV Flight Data via Generative World Model](https://arxiv.org/abs/2605.19600)
*Jinhan Li,Xijie Huang,Zhaoqi Wang,Yijin Wang,Weiqi Ge,Qiyi He,Mo Zhu,Fei Gao,Yuze Wu,Xin Zhou*

Main category: cs.RO

TL;DR: 提出了FlyMirage，一个自动化的数据生成流水线，用于生成大规模、多样化、逼真的空中视觉语言导航数据集，包含动态可行的飞行轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有空中视觉语言导航数据集在规模、多样性和真实性方面存在局限，或依赖高成本真实场景，或使用视觉有限的模拟。

Method: 利用大语言模型作为环境设计师增加场景多样性，结合生成式世界模型（3D高斯泼溅）创建高保真场景，自动化场景探索与语义获取，并集成无人机动态可行轨迹规划器。

Result: 生成了大规模、多样化、逼真的空中视觉语言导航数据集，并包含动态可行的飞行轨迹。

Conclusion: FlyMirage工具链可高效生成高质量数据，支持下一代具身导航模型的发展。
...
### [300] [HEAT: Heterogeneous End-to-End Autonomous Driving via Trajectory-Guided World Models](https://arxiv.org/abs/2605.19631)
*Hoonhee Cho,Giwon Lee,Jae-Young Kang,Hyemin Yang,Heejun Park,Kuk-Jin Yoon*

Main category: cs.RO

TL;DR: 本文提出一种轨迹驱动的学习范式，通过规划轨迹组织训练，并引入世界模型预测未来潜在特征，实现多域异构数据集上的统一端到端自动驾驶，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端驾驶模型在单一域表现良好，但多域联合训练时因域间冲突导致性能下降，无法适应多样化的实际部署场景。

Method: 采用轨迹驱动学习范式，使模型捕捉域不变的驾驶意图表征；集成世界模型，根据自车动作预测未来潜在特征，增强跨域一致性并减少域偏置。

Result: 在nuScenes、NAVSIM和Waymo三个基准上，统一模型在各域均显著优于现有方法，证明了单一模型在多域训练中的有效性。

Conclusion: 该工作表明，通过轨迹驱动和世界模型可训练出跨域鲁棒的端到端驾驶模型，为实际大规模部署迈出关键一步。
...
### [301] [RoVLA: Multi-Consistency Constraints for Robust Vision-Language-Action Models](https://arxiv.org/abs/2605.19678)
*Jingzhou Luo,Yifan Wen,Yongjie Bai,Xinshuai Song,Yang Liu,Liang Lin*

Main category: cs.RO

TL;DR: 提出RoVLA框架，通过多一致性约束提升VLA模型在视觉、语言和轨迹变化下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在观测变化、指令改写等扰动下表现脆弱，依赖表层相关性而非稳定耦合。

Method: 引入三种一致性约束：指令一致性、演化一致性和观测一致性，分别处理语义等价改写、轨迹意图保持和扰动前后的预测一致性。

Result: 在LIBERO-Plus、RoboTwin 2.0及真实任务中，RoVLA优于基线方法，展现出更强的鲁棒性和泛化性。

Conclusion: 多一致性学习能有效减少对浅层相关性的依赖，实现鲁棒的具身控制。
...
### [302] [D-CLING: Prior-Preserving Depth-Conditioned Fine-Tuning for Navigation Foundation Models](https://arxiv.org/abs/2605.19690)
*Shintaro Nakaoka,Takayuki Kanai,Kazuhito Tanaka*

Main category: cs.RO

TL;DR: 提出一种新型的导航基础模型微调方法DCLING，通过冻结预训练主干的零初始化残差路径，在保持预训练知识的同时高效学习新场景的几何线索，显著提升鲁棒导航能力。


<details>
  <summary>Details</summary>
Motivation: 现有的导航基础模型微调会破坏预训练先验，导致避障失败、目标到达困难以及泛化能力下降。

Method: 受ControlNet启发，在预训练模型上附加一个可训练的副本，使用零初始化残差路径进行微调，保留预训练行为同时学习新场景的几何线索。

Result: 真实世界导航实验表明该方法能实现鲁棒的长时程导航，碰撞和人工干预最少；离线分析显示动作预测能力保持或超越微调数据集水平。

Conclusion: 该方法为通用导航的持续学习提供了关键见解，能够在保持预训练知识的同时高效学习新场景，提升导航鲁棒性。
...
### [303] [Multi-Session Ground Texture SLAM in Low-Dynamic Environments](https://arxiv.org/abs/2605.19701)
*Kyle M. Hart,Brendan Englot*

Main category: cs.RO

TL;DR: 本文研究了多会话低动态地面纹理环境下的轨迹估计，发现Kullback-Leibler散度作为相似度评分和闭环置信度偏置最有效，并介绍了包含地面变化和多会话图像的高精度数据集。


<details>
  <summary>Details</summary>
Motivation: 现有地面纹理系统未针对多会话低动态变化环境设计，需改进轨迹估计精度以适应长期运行需求。

Method: 比较三种技术（包括Kullback-Leibler散度）对轨迹估计的影响，并通过分析其作为相似度评分和闭环置信度偏置的效果进行验证。

Result: Kullback-Leibler散度在提升轨迹估计精度方面效果最佳，建立了包含多会话地面变化的高精度数据集。

Conclusion: Kullback-Leibler散度可有效优化多会话低动态环境下的SLAM性能，数据集为社区提供了评估基准。
...
### [304] [KIO-planner: Attention-Guided Single-Stage Motion Planning with Dual Mapping for UAV Navigation](https://arxiv.org/abs/2605.19703)
...
TL;DR: 提出KIO-planner，一种注意力引导的单阶段轨迹规划框架，用于在密集墙壁环境中实现自主无人机高速安全飞行，显著降低延迟并提高安全裕度。


<details>
  <summary>Details</summary>
Motivation: 传统优化规划器存在映射延迟和局部极小值问题，端到端学习方法难以提取细粒度几何特征且缺乏动力学约束，导致近壁碰撞风险。

Method: 集成卷积块注意力模块（CBAM）增强感知，提出双映射机制（物理边界激活+深度像素空间确定性几何安全盾），强制执行运动学可行性和无碰撞飞行。

Result: 在3.0 m/s速度下，推理延迟约24 ms，控制成本降低28.4%，最坏情况安全裕度从0.48 m提升至0.76 m。

Conclusion: 该方法实现了在高度受限环境中更快速、更平滑、更安全的导航，优于现有基线。
...
### [305] [Beyond Imitation: Learning Safe End-to-End Autonomous Driving from Hard Negatives](https://arxiv.org/abs/2605.19771)
*Junli Wang,Zhihua Hua,Xueyi Liu,Zebin Xing,Haochen Tian,Kun Ma,Hangjun Ye,Guang Chen,Long Chen,Qichao Zhang*

Main category: cs.RO

TL;DR: 提出超越模仿学习的失败感知框架 BeyondDrive，通过学习成功与失败驾驶行为，在轨迹空间中建立安全边界，实现规划性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有模仿学习假设空间接近即行为安全，导致轨迹相似但安全性迥异的目标错配问题。

Method: 基于流匹配的负轨迹生成器、多样性采样策略、排斥距离损失。

Result: 在NAVSIMv1上达89.7 PDMS，超越现有方法，并跨架构泛化，零样本迁移至HUGSIM。

Conclusion: 联合学习成功与失败行为可有效提升自动驾驶的安全性和泛化能力。
...
### [306] [Justifying bio-inspired robotics research: A taxonomy of strategies](https://arxiv.org/abs/2605.19840)
*Margaret J. Zhang,Justin Ting,Talia Y. Moore*

Main category: cs.RO

TL;DR: 提出生物启发设计动机的分类法，帮助机器人领域的研究者和资助者评估不同方法的潜在贡献。


<details>
  <summary>Details</summary>
Motivation: 缺乏系统化的生物启发设计方法导致动机和方法不一致，难以预测或评估设计成功，尤其在机器人领域容易因期望与结果不匹配而产生失望。

Method: 通过分析生物启发设计的动机，构建分类法，并描述不同方法可能带来的显著贡献。

Result: 提供了一个动机分类法，能够为研究者论证其生物启发方法合理性，并为资助者判断不同方法的价值提供依据。

Conclusion: 该分类法有助于提升生物启发设计在机器人领域的系统性和评估可靠性，减少误判。
...
### [307] [Trajectory Planning and Control near the Limits: an Open Experimental Benchmark on the RoboRacer Platform](https://arxiv.org/abs/2605.19881)
*Mattia Piccinini,Patrick Zambiasi,Aniello Mungiello,Mattia Piazza,Felix Jahncke,Johannnes Betz*

Main category: cs.RO

TL;DR: 提出模块化框架用于极限自主驾驶的轨迹规划与控制，包含时间最优路线生成、在线速度重规划、几何路径跟踪器及模型结构化神经网络（MS-NN）。在1:10比例RoboRacer平台上验证，MS-NN提升跟踪精度、减少振荡，速度重规划优化单圈时间并实现更高速安全驾驶。


<details>
  <summary>Details</summary>
Motivation: 现有自主驾驶方法在极限工况（高加速）下缺乏系统对比评估，且难以平衡跟踪精度与稳定性。

Method: 模块化框架：时间最优路线生成 + 在线速度重规划 + 几何跟踪控制器 + MS-NN逆动力学学习，结合消融实验评估。

Result: MS-NN显著提升跟踪精度、降低方向振荡；速度重规划减少单圈时间、补偿执行误差，支持更高安全速度。

Conclusion: 该模块化框架为极限自主驾驶提供标准化基准，MS-NN与速度重规划是关键性能提升组件。
...
### [308] [Beyond Action Residuals: Real-World Robot Policy Steering via Bottleneck Latent Reinforcement Learning](https://arxiv.org/abs/2605.19919)
*Dongjie Yu,Kun Lei,Zhennan Jiang,Jia Pan,Huazhe Xu*

Main category: cs.RO

TL;DR: 提出ZPRL方法，通过在瓶颈潜变量而非动作空间中进行强化学习微调，提升预训练模仿策略的适应能力。


<details>
  <summary>Details</summary>
Motivation: 预训练模仿策略在机器人操作中因执行错误、数据覆盖不足和部署不匹配，需要在线改进。现有轻量方法在动作空间直接添加残差修正，导致噪声大且探索结构差。

Method: 在离线训练时添加可插拔变分信息瓶颈(VIB)模块提取任务相关潜变量；在线微调时冻结基础策略，仅学习潜变量上的残差扰动，解码后条件化动作生成器，基于流匹配策略实现。

Result: 在8个仿真和4个真实任务中，相比强基线方法，ZPRL提升了样本效率和最终性能。真实任务平均成功率比模仿基线高33.7%，且探索行为比动作残差方法更平滑。

Conclusion: 紧凑、任务对齐的瓶颈潜变量为在线RL适应提供了有效接口。
...
### [309] [TravExplorer: Cross-Floor Embodied Exploration via Traversability-Aware 3-D Planning](https://arxiv.org/abs/2605.19958)
*Han Zheng,Zhe Chen,Yudong Huang,Haoran Liu,Jinghao Wang,Ming Yang,Tong Qin*

Main category: cs.RO

TL;DR: 提出了TravExplorer框架，用于跨楼层零样本目标导航，结合了语义引导和可遍历性感知的三维规划。


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标导航系统多基于平面表示和单楼层假设，不适用于真实建筑中的跨楼层导航。

Method: 维护统一三维地图区分障碍物与可遍历表面，提取可遍历前沿，使用视场感知策略解决观测不完整问题，并通过轻量级模块对齐语义与空间价值图，采用分层规划器进行目标感知导航。

Result: 在HM3D和MP3D数据集上的4195个模拟实验和50次真实世界实验中，优于代表性目标导航基线。

Conclusion: TravExplorer有效实现了跨楼层零样本目标导航，无需先验地图或人工干预。
...
### [310] [RoHIL: Robust Human-in-the-Loop Robotic Reinforcement Learning Against Illumination Variations](https://arxiv.org/abs/2605.19924)
*Shuoqin Zhang,Yixin Xiong,Xiru Gao,Kai Liu,Ke Wang,Xichuan Zhou,Zhe Hu*

Main category: cs.RO

TL;DR: RoHIL 是一个离线微调框架，通过图像重照明、数据级防遗忘和策略约束，解决了机器人跨工作站光照分布偏移导致的强化学习性能崩溃问题，无需重新收集数据。


<details>
  <summary>Details</summary>
Motivation: 标准人在环强化学习在训练工作站上表现完美，但机器人移动到几米外的新工作站时，因光照分布偏移（如灯光位置、窗户光变化）而性能崩溃。重新收集数据在每个工作站上不可行，且简单微调会导致灾难性遗忘。

Method: RoHIL 结合三个组件：(i) 基于世界模型的图像重照明器，将源工作站轨迹的视觉流在多种虚拟 HDRI 环境下重合成，保持动作和奖励不变；(ii) 光照保持重放机制，交织重照明适应轨迹和原始光照保持轨迹以保留源工作站贝尔曼覆盖；(iii) 锚定贝尔曼-演员正则化器，约束表示和策略漂移。

Result: 在四个真实机器人操作任务中，RoHIL 显著提升了跨工作站在光照变化下的性能，而标准 HIL-RL 性能崩溃，同时保持源工作站性能，无需为每个新工作站收集数据和重新训练。

Conclusion: RoHIL 有效消除了跨域差距，实现了零额外机器人交互的离线微调，为实际部署中的光照变化问题提供了可行解决方案。
...
### [311] [CEER: Compliant End-Effector and Root Control as a Unified Interface for Hierarchical Humanoid Loco-Manipulation](https://arxiv.org/abs/2605.19981)
*Xinyuan Luo,Xingrui Chen,Xunjian Yin,Hongxuan Wu,Boxi Xia,Zhuoqun Chen,Jinzhou Li,Boyuan Chen,Xianyi Cheng*

Main category: cs.RO

TL;DR: 提出一种名为CEER的柔顺末端-根控制抽象方法，用于人形机器人模块化全身操控，通过分层框架集成异构规划器，实现接触丰富的长程操控任务。


<details>
  <summary>Details</summary>
Motivation: 人形机器人操控面临接触丰富、长时程任务挑战，现有方法缺乏柔顺全身控制和模块化接口。

Method: 采用CEER抽象（末端姿态+根运动命令），结合教师-学生蒸馏框架训练底层策略，并构建集成异构规划器的分层系统。

Result: 仿真与硬件实验显示末端跟踪精度3.3厘米、抖动显著减少，遥操作下稳定接触操控，单物体操控任务成功率高达70%。

Conclusion: 柔顺末端-根控制为人形机器人操控提供了实用抽象，支持模块化、可扩展的技能集成。
...
### [312] [Beyond Binary Success: A Diagnostic Meta-Evaluation Framework for Fine-Grained Manipulation](https://arxiv.org/abs/2605.19986)
*He-Yang Xu,Pengyuan Zhang,Zongyuan Ge,Xiaoshuai Hao,Serge Belongie,Xin Geng,Yuxin Peng,Xiu-Shen Wei*

Main category: cs.RO

TL;DR: 提出MetaFine元评测框架，将操作能力解构为理解、感知和控制行为三个维度，识别出视觉编码器保留局部空间结构的能力是关键瓶颈，并通过混合真实-仿真验证提升评测可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度操作基准将多维能力简化为二元成功率，高估能力达70%，掩盖架构瓶颈，阻碍实际部署。

Method: 构建组合式任务图，吸收外部基准并重构为统一协议下的诊断场景，通过因果干预识别关键瓶颈，支持混合真实-仿真验证。

Result: 揭示VLA模型在特定维度的严重失败，发现提升视觉编码器局部空间结构保留能力可直接解锁先前无法实现的操作能力。

Conclusion: 元评测框架将评测从排名转向诊断，为修复物理灵巧性的分层能力提供可操作指南。
...
### [313] [Minimalist Visual Inertial Odometry](https://arxiv.org/abs/2605.19990)
*Francesco Pasti,Jeremy Klotz,Nicola Bellotto,Shree K. Nayar*

Main category: cs.RO

TL;DR: 本文提出一种极简平面里程计方法，仅用四个光电二极管和IMU实现差分驱动机器人的鲁棒运动估计，无需真实环境微调。


<details>
  <summary>Details</summary>
Motivation: VIO系统中的相机需要大量像素和处理资源，为降低负载和成本，探索极简感知方案。

Method: 利用四个朝下的光电二极管通过光学Gabor掩膜感知速度信号，联合优化掩膜参数与时间卷积网络解码平面速度，结合IMU角速度形成连续轨迹。

Result: 在室内外多种地形上，原型传感器紧密跟踪真值轨迹，无需真实环境微调。

Conclusion: 极简感知系统可实现高效准确的平面里程计，验证了最少传感器配置的可行性。
...
### [314] [Topology-Optimized Pneumatic Soft Actuator: Design and Experimental Validation](https://arxiv.org/abs/2605.20101)
*Sumit Mehta,Konstantinos Poulios*

Main category: cs.RO

TL;DR: 本文展示了一种基于非线性拓扑优化的软弹性气动执行器计算方法，将现有框架从2D扩展到3D，生成两种可制造设计，并通过数值和实验验证了其弯曲性能。


<details>
  <summary>Details</summary>
Motivation: 现有软执行器设计缺乏系统优化方法，难以在大变形下实现高效性能。

Method: 使用密度和孔超弹性模型进行3D拓扑优化，考虑大变形，并通过立体光刻制造和实验测试。

Result: 两种优化设计在给定驱动压力下实现了最大化弯曲响应，实验与数值结果一致。

Conclusion: 该拓扑优化框架能有效处理非线性大变形，生成高性能软执行器。
...
### [315] [Probabilistic Recursively Feasible Motion Planning Under Uncertain Environments](https://arxiv.org/abs/2605.19015)
*Hyeontae Sung,Hyeongchan Ham,Junyoung Park,Kai Ren,Heejin Ahn*

Main category: eess.SY

TL;DR: 提出了一种概率递归可行模型预测控制（PRF-MPC）框架，通过确保当前安全集以高概率包含于未来安全集中，实现了指定概率下的递归可行性。


<details>
  <summary>Details</summary>
Motivation: 在不确定、时变环境下，安全区域可能意外变化导致递归可行性丧失，现有方法难以保证。

Method: 定义了理想预测器的分布一致性属性，推导出未来轨迹均值和协方差的闭合表达式，并基于此构建概率安全约束。

Result: 在变道场景仿真中，该方法显著提升了递归可行性。

Conclusion: PRF-MPC框架能以概率方式保证递归可行性，适用于时变不确定环境。
...
### [316] [Dynamic Gradient-Based Calibration for Robust and Accurate Traffic Macrosimulation](https://arxiv.org/abs/2605.19056)
*Shreyaa Raghavan,Cameron Hickert,Monica Chan,Cathy Wu*

Main category: eess.SY

TL;DR: 提出了一种动态滚动时域标定框架，解决了交通流模型静态标定中因非凸优化导致的参数不稳定问题，在实际数据上实现了48%的预测精度提升。


<details>
  <summary>Details</summary>
Motivation: 标准静态标定方法因高度非凸的优化场景导致参数不稳定，产生不真实交通动态，削弱了模型在反事实场景测试中的可靠性。

Method: 将静态单次估计重新构造为闭环控制问题，采用动态滚动时域标定框架。

Result: 在I-24 MOTION实测数据上，所提方法对扰动具有更强的鲁棒性，预测精度相比传统静态标定提升48%。

Conclusion: 动态滚动时域标定有效解决了参数不稳定问题，同时提升了精度和鲁棒性，适用于实际交通场景。
...
### [317] [Continuous Aggregative LQG Games with Delayed Discrete Observations](https://arxiv.org/abs/2605.19134)
*Farid Rajabali,Roland Malhame,Sadegh Bolouki*

Main category: eess.SY

TL;DR: 研究了在信息延迟和离散时间观察条件下，均值场博弈中代理的最佳响应策略，给出了有限人口中纳什均衡存在的充分条件，并评估了延迟量化导致的成本增加。


<details>
  <summary>Details</summary>
Motivation: 现实世界中代理接收总体信息常有延迟，而现有均值场博弈理论假设即时交互，需要更贴近实际的模型。

Method: 构建了代理仅周期性延迟观察经验均值状态的动态模型，通过理论分析给出均衡存在条件，并对比不同观察方式的成本差异。

Result: 得出了有限人口中纳什均衡存在的充分条件，并定量分析了延迟离散观察相比零延迟和连续观察带来的额外成本。

Conclusion: 该研究揭示了信息延迟对均值场博弈均衡和成本效率的影响，拓展了相关理论在延迟信息系统中的应用边界。
...
### [318] [Multi-Criteria Integer Programming Model for Route Planning in an Off-Road Combat Environment](https://arxiv.org/abs/2605.19168)
*Joshua Betz,Daniel Herber,Jeffrey Niemann*

Main category: eess.SY

TL;DR: 提出一个集成土壤通过性和战术风险的军事车辆路径规划优化模型，并验证其在多场景下的有效性。


<details>
  <summary>Details</summary>
Motivation: 军事路径规划需同时考虑环境通过性与战术风险，现有研究缺乏综合模型量化二者权衡。

Method: 构建混合整数线性规划模型，将地形离散化为网格节点，节点间成本由土壤强度与敌方威胁联合风险函数计算，RCI数据来自外部模型。

Result: 目标函数值因成本权重与车辆特性变化达五个数量级，证明模型能量化环境约束与战术风险之间的权衡。

Conclusion: 该框架可有效支持野火响应、农业等越野场景的路径决策，通过场景分析可为规划者提供风险-收益平衡的定量依据。
...
### [319] [ERFSL: An Efficient Reward Function Searcher via Language Models for Custom-Environment Multi-Objective Optimization (Student Abstract)](https://arxiv.org/abs/2605.19259)
*Guanwen Xie,Jingzehua Xu,Yiyuan Yang,Yimian Ding,Shuai Zhang*

Main category: eess.SY

TL;DR: 提出ERFSL，一种利用大语言模型高效搜索奖励函数的方法，用于自定义环境的多目标学习，仅需少量迭代即可满足需求。


<details>
  <summary>Details</summary>
Motivation: 现有奖励函数设计依赖专家知识且难以适应多目标优化，需高效自动化方法。

Method: 基于LLM生成奖励组件，通过奖励评论家修正，结合训练日志分析器迭代优化权重。

Result: 奖励评论家一次反馈即可修正代码；权重偏差500倍时平均仅需5.2次迭代；在GPT-4o mini上表现良好。

Conclusion: ERFSL能高效搜索奖励函数，降低对模型能力的依赖，适用于多目标学习任务。
...
### [320] [Revisiting angle stability in power systems with grid-forming power converters](https://arxiv.org/abs/2605.19380)
*Régulo E. Ávila-Martínez,Javier Renedo,Luis Rouco,Aurelio Garcia-Cerrada,Lukas Sigrist*

Main category: eess.SY

TL;DR: 本文对电力系统中同步电机与构网型电压源变流器共同存在时的同步稳定性进行了全面分析，并提出用“角度稳定性”统一描述该现象。


<details>
  <summary>Details</summary>
Motivation: 现有IEEE/PES分类将同步稳定性分为转子角稳定性和慢相互作用变流器驱动稳定性，但两者本质均与电压源间的角度差动态相关，因此需要重新统一分类。

Method: 通过理论分析，比较同步电机和构网型变流器在大小扰动下的同步保持能力，揭示其共同特性。

Result: 发现该现象完全由连接至系统的电压源间角度差的慢动态特征决定，与具体设备类型无关。

Conclusion: 建议使用“角度稳定性”涵盖该现象，而“慢相互作用变流器驱动稳定性”仅用于描述涉及变流器的其他慢动态相互作用。
...
### [321] [MagCeptor: Encoding Broadcast-Addressable Logic into Magnetic Receptors](https://arxiv.org/abs/2605.19558)
*Sishen Yuan,Baijia Liang,Tangyou Liu,Yiqing Huang,Haoxuan Wu,Shuo Xu,Hongliang Ren*

Main category: eess.SY

TL;DR: 本研究提出MagCeptors，通过将选择性编码到磁性拓扑中，解决了全局磁场在功率和控制上固有的混淆问题，实现了多细胞协调式的分布式系统。


<details>
  <summary>Details</summary>
Motivation: 工程磁性系统面临地址性瓶颈，因为全局场无法同时实现功率传输和独立控制，这限制了分布式系统的复杂任务执行。

Method: 建立与生物受体能量同构的阵列，利用局部耦合塑造势能景观，使全局场向量作为空间钥匙触发确定性突跳不稳定性，从而解耦力与源距离。

Result: 实现了385 mN/mm³的力密度（较先前工作提升50倍以上），并通过信号解复用、实体顺序逻辑和无线分布式网络进行了验证。

Conclusion: MagCeptors框架使分布式系统无需导线或电子元件，仅凭物质的固有逻辑即可协调复杂任务，为无束缚系统提供新范式。
...
### [322] [Equalized Coverage in Motion Control Performance Prediction for Self-Adaptive Road Vehicles](https://arxiv.org/abs/2605.19655)
*Ole Reuter,Richard Schubert,Marvin Loba,Markus Maurer*

Main category: eess.SY

TL;DR: 提出基于等化覆盖的轻量级预测模型，在致动器降级/故障下评估自动驾驶车辆横向控制能力，并讨论其在运行时行为生成中的应用与局限。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需监控致动器降级/故障对运动控制能力的影响，实现基于剩余性能的适应性行为。

Method: 使用等化覆盖的共形分位数回归构建轻量级预测模型，确保条件覆盖（而非仅边际覆盖）；结合运行时启发式确定可行动作空间。

Result: 模型能在名义、降级、故障条件下预测车辆横向偏离轨迹的可控性，条件覆盖保证更可靠的统计保证。

Conclusion: 等化覆盖提升了预测可靠性，为过驱动车辆在致动器失效下的动态行为决策提供实用框架，但存在应用限制需进一步研究。
...
### [323] [Safe Deep Reinforcement Learning for Spacecraft Reorientation with Pointing Keep-Out Constraint](https://arxiv.org/abs/2605.19967)
*Juntang Yang,Mohamed Khalil Ben-Larbi*

Main category: eess.SY

TL;DR: 本文提出了一种结合安全滤波器的深度强化学习（DRL）方法，用于航天器单指向禁止区约束下的姿态重定向控制。通过新的状态空间表示、奖励函数设计、SAC算法和课程学习训练，并引入控制障碍函数（CBF）安全滤波器确保约束满足。仿真显示奖励设计有效，但仅靠奖励塑造无法保证安全，而CBF滤波器可严格保证约束。


<details>
  <summary>Details</summary>
Motivation: 航天器姿态重定向中需避免特定指向区域的约束，传统方法难以处理高维连续状态空间和安全保证的平衡。

Method: 采用SAC算法处理连续动作空间；设计包含约束紧致表示的状态空间和导向目标的奖励函数；通过课程学习分阶段训练；在部署时附加CBF安全滤波器实时修正动作以确保约束。

Result: 新状态表示和奖励函数有效；蒙特卡洛仿真表明仅靠奖励塑造无法完全约束，加入CBF安全滤波器后100%满足指向约束。

Conclusion: CBF安全滤波器可弥补强化学习在安全关键任务中的不足，实现高成功率且安全的姿态控制。
...
### [324] [Robust synchronization for multi-agent systems governed by PDEs with observable and unobservable disturbances](https://arxiv.org/abs/2605.19992)
*Yongchun Bi,Jun Zheng,Guchuan Zhu,Jiye Zhang*

Main category: eess.SY

TL;DR: 本文针对带可观测和不可观测扰动的抛物型偏微分方程多智能体系统，提出基于边界输出的鲁棒同步控制方法，实现指数跟踪与输入-状态稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有研究对分布式参数系统中混合类型扰动（如边界和域内扰动）的鲁棒同步控制研究不足。

Method: 设计扰动观测器估计可观测Dirichlet边界扰动，结合参考信号和局部输出构建分布式同步控制器，采用广义Lyapunov法与提升技术分析稳定性。

Result: 无不可观测扰动时实现指数跟踪；存在扰动时保持鲁棒性，并证明闭环系统的输入-状态稳定性。

Conclusion: 所提方案能有效估计和抑制扰动，实现鲁棒同步，仿真验证了其有效性。
...
### [325] [A New Simple-to-Configure Self-Perturbing Multivariable Extremum-Seeking Controller](https://arxiv.org/abs/2605.20038)
*Timothy I. Salsbury,Min Gyung Yu*

Main category: eess.SY

TL;DR: 提出了一种基于随机继电器的多输入单输出系统极值搜索控制器，显著减少了可配置参数数量。


<details>
  <summary>Details</summary>
Motivation: 简化极值搜索控制器的配置，使其更易于应用于实际问题。

Method: 先针对静态映射开发解决方案，再扩展到动态系统；通过随机继电器增益进行梯度识别。

Result: 静态情况每个输入通道只需一个参数，动态版本仅需额外一个参数；仿真验证了有效性。

Conclusion: 该策略能以较少参数有效优化静态和动态系统。
...
### [326] [k-Inductive Neural Barrier Certificates for Unknown Nonlinear Dynamics](https://arxiv.org/abs/2605.20108)
*Ben Wooding,Hongchao Zhang,Taylor T. Johnson,Abolfazl Lavaei*

Main category: eess.SY

TL;DR: 本文提出了一种利用数据驱动方法结合神经网络构造k-归纳神经网络屏障证书(k-NBCs)的技术，用于处理部分未知的非线性系统，并通过SMT验证保证安全性。


<details>
  <summary>Details</summary>
Motivation: 传统离散时间屏障证书条件严格，而k-归纳屏障证书提供了更多灵活性，但神经网络缺乏形式化保证，且现有CEGIS-SMT框架需要系统动力学知识，这在实践中常不可用。

Method: 基于Willems等人基本引理的泛化，利用单一状态轨迹构建(部分)未知模型的数据驱动表示，结合神经网络构造k-NBCs，并采用CEGIS-SMT框架进行验证，无需限制屏障证书的函数形式。

Result: 在三个(部分)未知动力学的非线性案例研究中验证了方法的有效性。

Conclusion: 提出的数据驱动方法成功实现了对部分未知非线性系统的安全性验证，并提升了屏障证书设计的灵活性。
...
### [327] [Enabling Real-Time Phase Control in Traffic Signal Hardware-in-the-Loop Simulation](https://arxiv.org/abs/2605.20136)
*Zhiyao Zhang,Gergely Zachár,William Barbour,Matt Bunting,Marcos Quiñones-Grueiro,Jonathan Sprinkle,Dan Work*

Main category: eess.SY

TL;DR: 首个支持实时相位控制的硬件在环仿真测试平台，通过中间件架构实现动态相位指令与商用控制器的兼容。


<details>
  <summary>Details</summary>
Motivation: 现有HILS测试平台仅支持预编程定时方案，无法满足先进交通信号控制算法对实时相位控制的需求。

Method: 开发中间件架构，将动态相位动作（选择、切换、持续时间）转换为NTCIP兼容指令，管理相位过渡、状态同步和错误处理。

Result: 系统可执行实时相位命令，处理冲突，平均内部延迟低于亚毫秒级。

Conclusion: 该测试平台有效支持实时相位控制，具有低延迟和冲突处理能力。
...
### [328] [The OAPS solution: a real-time predictive system for flexible PWR operation](https://arxiv.org/abs/2605.20169)
*Guillaume Dupré,Alain Grossetête*

Main category: eess.SY

TL;DR: 提出OAPS系统，利用模型预测控制技术为核电站功率变化提供实时优化建议，并在模拟器上验证了三种新策略。


<details>
  <summary>Details</summary>
Motivation: 现有核电站功率变化操作缺乏实时优化指导，需要安全灵活的控制方案。

Method: 采用模型预测控制（MPC）技术，基于最新测量数据更新推荐策略。

Result: 在Framatome中间复杂度模拟器上验证了：最快功率变化率、加速消除轴向振荡、最小化废液排放。

Conclusion: OAPS系统能有效提升核电站功率操作的效率与安全性。
...
### [329] [A Unified Framework for Attack-Resilient CLF-CBF Quadratic Programs for Nonlinear Control-Affine Systems](https://arxiv.org/abs/2605.20144)
*Mohamadamin Rajabinezhad,Shan Zuo*

Main category: eess.SY

TL;DR: 本文提出了一种针对非线性控制仿射系统的攻击弹性控制Lyapunov函数(AR-CLF)和攻击弹性控制屏障函数(AR-CBF)，用于抵御满足至少指数增长包络的虚假数据注入攻击(FDIA)。通过在线自适应增益调节，实现了有限时间恢复至名义安全集，并在无界FDIA下保证了最终有界稳定性和最终安全性。


<details>
  <summary>Details</summary>
Motivation: 现有基于输入-状态稳定性/安全性(ISS/ISSf)的方法依赖攻击幅度先验上界，且仅能保证扰动依赖的扩大安全集，无法实现有限时间恢复至名义安全集，因此需要一种更鲁棒的弹性控制框架。

Method: 在CLF和CBF约束中嵌入统一的自适应补偿项，并设计在线增益调节律以补偿攻击效应；通过统一二次规划(QP)同时满足AR-CLF和AR-CBF条件。

Result: 与ISS-CLF、ISSf-CBF和鲁棒CLF-CBF-QP方法相比，数值实验表明该方法在无界FDIA下具有更强的弹性。

Conclusion: 所提AR-CLF/AR-CBF框架结合自适应增益调节，可在无攻击幅度先验信息下实现有限时间恢复至名义安全集，显著提升系统对控制输入FDIA的韧性。
...
### [330] [SpecX: A Large-Scale Benchmark for Multi-Modal Spectroscopy and Cross-Paradigm Evaluation](https://arxiv.org/abs/2605.18791)
*Chengrui Xiang,Tengfei Ma,Yujie Chen,Tong Wang,Haowen Chen,Xiangxiang Zeng*

Main category: eess.IV

TL;DR: 提出了一个大规模多模态光谱基准SpecX，包含170万分子和多种光谱类型，支持多种任务和跨范式评估。


<details>
  <summary>Details</summary>
Motivation: 现有光谱基准在规模、模态对齐和评估范围上有限，且通常只关注专业模型或多模态语言模型中的一个。

Method: 构建了包含NMR、IR、MS等7种光谱模态的1.7M分子数据集，按预训练、基准测试和高质量评估三个层级组织，支持分子解析、光谱模拟等任务。

Result: 专业模型擅长信号级建模，而多模态语言模型在高层次推理上较强但缺乏精确光谱理解。

Conclusion: SpecX为光谱智能建立了统一基准，并强调了开发光谱原生基础模型的必要性。
...
### [331] [From Division to Decision: Leveraging Temporal Cell-Stage Segmentation for Embryo Transferability Prediction](https://arxiv.org/abs/2605.18923)
*Yasmine Hachani,Patrick Bouthemy,Elisa Fromont,Véronique Duranthon,Ludivine Laffont,Alline de Paula Reis*

Main category: eess.IV

TL;DR: 提出TransFACT框架，利用前四天延时视频预测牛胚胎可移植性，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前胚胎选择依赖单次专家评估导致高流产率，延时视频分析困难。

Method: 基于Transformer的框架，结合帧级时态特征和阶段级表示，以发育阶段为辅助监督预测可移植性。

Result: TransFACT在预测胚胎可移植性上优于现有竞争方法。

Conclusion: 利用动作识别技术可有效分析早期胚胎发育视频，提升选择准确性。
...
### [332] [An evaluation framework for sparse 4D (3D + time) imaging reconstruction via bootstrapped cross-validation](https://arxiv.org/abs/2605.19160)
*Yuhe Zhang,Zisheng Yao,Zhe Hu,Tobias Ritschel,Pablo Villanueva-Perez*

Main category: eess.IV

TL;DR: 提出了一种无需参考数据即可评估4D显微图像重建性能的自举交叉验证框架。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习重建方法在缺乏4D参考数据时无法评估其性能。

Method: 基于自举交叉验证，通过比较独立采样子集重建间的相关性来估计性能，受冷冻电镜3D验证策略启发。

Result: 在稀疏和超稀疏X射线数据集上验证了有效性，并使用4D-ONIX方法在水滴碰撞模拟实验中进行了测试。

Conclusion: 该框架提供了一种无参考的性能评估方法，有助于优化超快成像实验策略。
...
### [333] [Next-Acceleration-Scale Prediction for Autoregressive MRI Reconstruction](https://arxiv.org/abs/2605.19354)
*Yilmaz Korkmaz,Vishal M. Patel*

Main category: eess.IV

TL;DR: 提出一种基于离散多尺度潜空间的自回归MRI重建方法，通过自回归逐加速尺度预测和策略优化蒸馏，提升极端欠采样下的重建质量。


<details>
  <summary>Details</summary>
Motivation: 高加速MRI重建中，像素域连续预测导致高频细节丢失，需要离散化约束来锐化重建。

Method: 将重建视为离散潜空间中的自回归逐尺度预测，并引入策略优化知识蒸馏（教师使用全采样数据，学生基于自身生成学习）。

Result: 在fastMRI基准上，该方法在多种采样模式下显著提升了极端欠采样下的重建性能。

Conclusion: 离散自回归框架结合策略蒸馏可有效解决高加速MRI重建的模糊性问题，实现锐利重建。
...
### [334] [Perception-Aware Video Semantic Communication](https://arxiv.org/abs/2605.19397)
*Yinhuan Huang,Zhijin Qin*

Main category: eess.IV

TL;DR: 本文提出PVSC，一种感知感知的视频语义通信框架，用于实时无线视频传输。它消除显式运动矢量传输，利用时空特征编码生成紧凑且信道鲁棒的符号流，并在单GPU上实现实时推理。


<details>
  <summary>Details</summary>
Motivation: 无线视频流量快速增长，传统分离源信道系统在短块传输和带宽受限链路中性能下降，像素级失真优化与人类感知不一致，现有学习视频编解码器复杂度高且部署困难。

Method: PVSC通过时空特征编码消除运动矢量传输，制定侧信息格式、参考缓冲区管理和轻量级速率控制，实现单模型带宽自适应推理。

Result: 与VTM+5G LDPC基线相比，在LPIPS和DISTS指标上分别节省约75%和87%带宽，并在NVIDIA RTX 4090上实现实时推理。

Conclusion: PVSC在多种数据集、分辨率、GOP配置和信道条件下表现优越，表明感知感知语义通信是高效视频传输的有效方向。
...
### [335] [Diffusion Graph Posterior Sampling for Nonlinear Inverse Problems with Application to Electrical Impedance Tomography](https://arxiv.org/abs/2605.19621)
*Giovanni S. Alberti,Damiana Lazzaro,Serena Morigi,Matteo Santacesaria,Shibo Wang*

Main category: eess.IV

TL;DR: 本文提出了一种基于图的扩散后验采样框架RDPS，用于解决非结构化网格上的电学阻抗断层成像逆问题，在合成和真实数据集上实现了优于现有方法的稳定、物理合理的重建。


<details>
  <summary>Details</summary>
Motivation: 现有深度生成模型应用于偏微分方程逆问题时，标准卷积架构难以处理非结构化网格数据，且EIT问题严重病态。

Method: 在二维三角网格上构建无条件分数扩散模型学习先验，并引入正则化变体RDPS融合显式正则化项（如全变分和广义Tikhonov）与隐式扩散先验。

Result: 在合成和真实EIT数据集上，RDPS在重建精度和伪影减少方面超过现有最先进方法，对噪声鲁棒且能泛化到分布外的内含物几何。

Conclusion: 将扩散后验采样扩展到图结构数据的RDPS框架有效解决了非结构化网格逆问题，为EIT等病态物理逆问题提供了新方案。
...
### [336] [Set Shaping Theory as a Complementary Payload-Shaping Layer for Steganography](https://arxiv.org/abs/2605.19885)
*Aida Koch,Logan Lewis,Lily Scott,Agi Weber*

Main category: eess.IV

TL;DR: 提出使用集合塑造理论作为LSB图像隐写的预处理层，以降低统计可检测性。实验表明SST可降低相对熵等指标，提高隐写安全性。


<details>
  <summary>Details</summary>
Motivation: 现有隐写方法嵌入时会产生统计扰动，SST预处理可使后续嵌入更易应用且扰动更小。

Method: 采用Tankersley的近似快速变换算法实现SST，将消息长度增加K位，并通过KL散度等指标评估统计改变。

Result: 在1800次模拟中，SST平均降低相对熵25.16%（K=8时达42.81%），并降低JS散度、总变差等指标；在矩阵嵌入仿真中，K=8降低加权插入成本6.93%。

Conclusion: SST作为预处理层可有效提高LSB隐写的抗检测性，且与现有嵌入方法兼容。
...
### [337] [FGSVQA: Frequency-Guided Short-form Video Quality Assessment](https://arxiv.org/abs/2605.20016)
*Xinyi Wang,Angeliki Katsenou,Junxiao Shen,David Bull*

Main category: eess.IV

TL;DR: 本文提出一种用于短视频质量评估的端到端框架，利用CLIP密集视觉编码器结合频域压缩先验，生成工件与结构感知权重图进行特征聚合，通过可学习门控模块自适应融合特征，在短视频数据集上取得良好性能（SRCC:0.736, PLCC:0.787），且推理高效。


<details>
  <summary>Details</summary>
Motivation: 短视频因生成流程复杂、内容变化快、失真混合，给用户生成内容质量评估带来挑战，现有方法难以有效应对。

Method: 采用基于CLIP的密集视觉编码器，提取频域压缩先验生成工件和结构感知权重图，并分解为工件、结构和原始视觉特征分支，通过可学习门控模块自适应融合。

Result: 在短视频数据集上平均排序和线性相关性表现优异（SRCC:0.736, PLCC:0.787），同时推理效率高。

Conclusion: 该方法在短视频质量评估中准确且高效，代码已开源。
