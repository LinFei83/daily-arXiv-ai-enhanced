...
TL;DR: 论文研究了图结构在大型语言模型中作为内部推理组织工具的价值，而非仅作为外部知识源。


<details>
  <summary>Details</summary>
Motivation: 受人类使用图状思维导图组织分支和汇聚思维的启发，探究图能否作为语言模型内部的推理辅助形式。

Method: 在多跳问答任务中，将教师提供的推理轨迹重写为图状思维导图，用于指导学生模型，并比较文本化图与视觉图的效果。

Result: 文本化图在去除直接答案提示后效果大幅下降，而视觉图在无直接答案线索时仍能保持有效性，且在监督微调和KL蒸馏后优势持续。

Conclusion: 图不仅应作为语言模型的外部知识结构，还应作为组织推理的视觉支架进行深入研究。
...
TL;DR: 提出AURA-Mem内存机制，为机器人长时程任务设计固定大小循环记忆，减少写操作7倍以上，保持精度。


<details>
  <summary>Details</summary>
Motivation: 传统KV-cache内存用于机器人时，因存储开销随任务步长线性增长，不适配带宽有限的边缘硬件和长时非重置任务。

Method: 采用冻结视觉-语言-动作骨干网络，结合固定大小循环记忆和学习门控，基于动作误差信号决定何时写入。

Result: 在合成基准上比最佳O(1)基线精度相当，写操作减少5-9倍；在LIBERO-Long上成功率与基准持平，写操作减少7倍。

Conclusion: AURA-Mem以恒定内存和更少写入实现接近基线性能，适合机器人长时程任务；近似信息状态值损失界限在实验规模下较宽松。
...
### [3] [From Long News to Accurate Forecast: Importance-Aware Fusion and PRM-Guided Reflection for Time Series Forecasting](https://arxiv.org/abs/2606.03097)
*Mingyang Liu,Qingcan Kang,Yuke Wang,Shixiong Kai,Kaichao Liang,Hui-Ling Zhen,Tao Zhong,Mingxuan Yuan,Linqi Song*

Main category: cs.AI

TL;DR: 提出重要性感知新闻压缩与过程检索监督框架，解决LLM新闻预测中上下文窗口限制和检索冗余问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM新闻预测方法受限于上下文窗口，且迭代检索缺乏引导导致冗余和收敛慢。

Method: 训练重要性奖励模型进行新闻压缩分配预算，引入过程奖励模型（PRM）基于误差和历史选择排序候选新闻，两者离线训练后冻结使用。

Result: 在金融、能源、交通、比特币预测上，该方法提升准确性，减少迭代次数，且对长文本新闻有效。

Conclusion: 提出的框架有效结合压缩与过程监督，提升了新闻预测的实用性和效率。
...
### [4] [DeskCraft: Benchmarking Desktop Agents on Professional Workflows and Human-in-the-Loop Collaboration](https://arxiv.org/abs/2606.03103)
*Wenkai Wang,Tao Xiong,Jingchen Ni,Yunpeng Bao,Xiyun Li,Tianqi Liu,Hongcan Guo,Zilong Huang,Shengyu Zhang*

Main category: cs.AI

TL;DR: 提出了DeskCraft基准测试，针对长周期创意和工程工作流中的人机协作进行评估，发现现有代理在长任务执行和主动澄清方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 现有桌面GUI基准测试大多局限于简短、简化的任务，且用户指令事先全部提供，无法反映真实工作流中需要人机协作的长周期、主动交互场景。

Method: 构建了多级难度分类的任务集（最长超过50步），覆盖设计、视频、音频和3D创作软件，并设计了包含回合中和回合后交互的正式协议，模拟代理主动澄清、用户中断和反馈等协作模式。

Result: 在538个任务上评估了18个代理，GPT-5.4在标准任务上达到31.6%，在交互任务上达到27.6%，显示出长周期工作流交付和主动澄清方面的持续失败。

Conclusion: DeskCraft揭示了现有桌面代理在长周期复杂工作流和主动人机协作方面的严重不足，为未来研究提供了基准和方向。
...
### [5] [EvoTrainer: Co-Evolving LLM Policies and Training Harnesses for Autonomous Agentic Reinforcement Learning](https://arxiv.org/abs/2606.03108)
*Guhong Chen,Yingcheng Shi,Yongbin Li,Binhua Li,Xander Xu,Hu Wei,Shiwen Ni,Min Yang,Jieping Ye*

Main category: cs.AI

TL;DR: 提出EvoTrainer框架，通过联合进化策略与训练架构，在数学推理、编程代码生成等任务上超越人工设计的RL方法。


<details>
  <summary>Details</summary>
Motivation: 自主LLM训练常局限于静态配方搜索，难以应对RL中的动态瓶颈和多样失败模式。

Method: 基于实证反馈的协同进化框架：诊断轨迹证据、修正诊断、回溯干预、积累可复用技能。

Result: 在数学推理、编程代码生成和仓库级软件工程任务上，EvoTrainer匹配或超越人工设计的RL参考方法，尤其在长程SWE任务上提升最大。

Conclusion: 自主LLM RL应从配方搜索转向策略与训练架构的联合进化。
...
### [6] [Uncertainty-Aware Clarification in LLM Agents with Information Gain](https://arxiv.org/abs/2606.03135)
*Mengyi Deng,Zhiwei Li,Xin Li,Tingyu Zhu,Ying Zhao,Zhijiang Guo,Wei Wang*

Main category: cs.AI

TL;DR: 提出一种基于信息增益奖励的导向性澄清框架，通过贝叶斯信念更新度量澄清问题的效用，在τ-Bench环境中将任务成功率提升3.7%，仅增加0.3个交互步骤。


<details>
  <summary>Details</summary>
Motivation: LLM代理在用户指令不明确时，潜在意图不确定性常导致工具操作出错。

Method: 定义信息增益奖励，量化澄清对话对目标信念的贝叶斯更新，使用该奖励训练LLM优化澄清行为以最大化信息增益。

Result: 在五种异构骨干模型上，成功率比无澄清基线提升3.7%，平均交互步骤仅增加0.3。

Conclusion: 该澄清框架有效减少不确定性并提升任务完成率，且交互成本极低。
...
### [7] [Think-Before-Speak: From Internal Evaluation to Public Expression in Multi-Agent Social Simulation](https://arxiv.org/abs/2606.03137)
*Kaiqi Yang,Tai-Quan Peng,Sanguk Lee,Hui Liu*

Main category: cs.AI

TL;DR: 提出了TBS框架，分离智能体内部推理与公共表达，用于多代理对话模拟，并在气候政策讨论中验证了其内部状态轨迹的可观测性。


<details>
  <summary>Details</summary>
Motivation: 现有对话模拟框架大多关注外部交互或聚合输出，难以分析智能体在沉默、说话意图和公开表达之间的内部评价过程。

Method: TBS框架基于时间间隔，让所有智能体在每一步更新内部状态（包括失调评价、意见气候感知、孤立风险、应对策略和说话意愿），再由协调器解决竞争意图并决定一个公开话语。

Result: TBS产生连贯的内部状态轨迹，这些轨迹在轮次分配、沉默和记忆条件下系统性变化；失调评价增加说话意愿，沉默压力评价减少说话意愿，公开表达主要由轮次分配规则塑造。

Conclusion: TBS通过使内部评价到公开表达的路径可观测和分析，支持对机制敏感的社会模拟。
...
### [8] [ClinicalMC: A Benchmark for Multi-Course Clinical Decision-Making with Large Language Models](https://arxiv.org/abs/2606.03157)
*Ruihui Hou,Siyi Zhu,Ziyue Huai,Guangya Yu,Yongqi Fan,Chunming Wang,Tong Ruan*

Main category: cs.AI

TL;DR: 提出了临床多阶段决策基准ClinicalMC，含中文1275条和英文5804条样本，覆盖入院到出院四个阶段，并构建了多智能体评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估LLMs在单阶段场景中的表现，缺乏对多阶段动态临床决策场景的系统评估。

Method: 构建包含病人、考官和医生智能体的多智能体评估框架，设计单轮静态和多轮动态两种实验设置，评估闭源、开源和医疗专用三类LLMs。

Result: 未在摘要中明确给出具体数值结果，但指出通过广泛评估旨在理解LLMs在医疗领域的表现。

Conclusion: 该基准和框架有助于更好地理解LLMs在医疗领域的性能，支持其在医疗中的有效部署。
...
### [9] [GTBench: A Curriculum-Grounded Benchmark for Evaluating LLMs as Mathematical Research Assistants in Graph Theory](https://arxiv.org/abs/2606.03144)
*Noujoud Nader,Ibrahem Aljabea,Patrick Diehl,Deepti Gupta*

Main category: cs.AI

TL;DR: 提出GTBench基准测试，系统评估LLM在图论中的数学推理能力，发现性能随难度显著下降，且人类与自动评估存在分歧。


<details>
  <summary>Details</summary>
Motivation: LLM作为数学研究助手的可靠性尚不明确，缺乏课程化、分难度的图论推理评估框架。

Method: 构建包含63道题、分3组难度（本科定义、算法推理、研究生证明）的GTBench，用零样本和思维链提示评估5个前沿模型，结合精确匹配和LLM评判（低难度）或混合人类-LLM评判（高难度）。

Result: GPT-5表现最优（Group1零样本95.8%，Group3 82%），其他模型性能随难度急剧下降（Llama在Group3人类评估中0%）；主要错误类型为正确算法但执行错误及不完整推理，人类与自动评判一致性中等。

Conclusion: GTBench是首个课程化图论推理评估框架，揭示了LLM在数学推理中的局限性，对AI辅助教学与科研治理有直接意义。
...
### [10] [MedCUA-Bench: A Screenshot-Only Benchmark for Clinical Computer-Use Agents](https://arxiv.org/abs/2606.03203)
*Jia Yu,Zilong Wang,Xinyang Jiang,Dongsheng Li,Shuo Wang*

Main category: cs.AI

TL;DR: 本文提出了 MedCUA-Bench，一个用于评估临床计算机使用代理的交互式基准测试，涵盖 18 个临床场景和10个医学领域，发现最佳闭源模型仅在 54.2% 的任务上成功，而开源模型平均仅 2.5%。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注通用网络或桌面任务，忽略了对医学软件的评估，而医学软件需要专业知识、UI设计独特、缺乏公共测试环境且需要安全验证。

Method: 从真实产品手册和开源医疗系统重建18个临床场景，每个任务附带意图级和步骤级目标，通过确定性检查器评估任务完成情况和五个临床安全维度。

Result: 最佳闭源模型严格成功率为54.2%，所有模型在真实OpenEMR上低于9%；开源模型平均仅2.5%，最佳达16.2%。

Conclusion: MedCUA-Bench揭示了当前代理与可靠临床软件使用之间的差距，提供了可重复的测试平台。
...
### [11] [Effect of Demographic Bias on Skin Lesion Classification](https://arxiv.org/abs/2606.03214)
*Ralf Raumanns,Gerard Schouten,Veronika Cheplygina,Josien P. W. Pluim*

Main category: cs.AI

TL;DR: 本研究评估了基于ResNet的皮肤病变分类模型中的性别和年龄偏见，发现性别偏见主要源于数据不平衡，而年龄偏见始终偏年轻群体。


<details>
  <summary>Details</summary>
Motivation: 探究训练数据中人口统计学偏见（性别和年龄）对皮肤病变分类性能的影响机制。

Method: 使用线性规划生成具有受控人口特征的数据集，评估单任务、多任务增强和对抗学习三种策略。

Result: 性别特定训练优化性能；增强和对抗学习减少平衡数据中的偏见但效果有限；年龄偏见始终偏年轻群体。

Conclusion: 性别和年龄偏见机制不同，需要针对性缓解策略；域转移显著影响性能及偏见模式。
...
### [12] [Perceive Before Reasoning: A Pre-Reasoning Perception Framework for Efficient and Reliable Proactive Mobile Agents](https://arxiv.org/abs/2606.03236)
*Zhijie Ding,Weinan Hong,Zicheng Zhu,Lei Li,Dezhi Kong,Hao Wang,Peng Zhou,Xuchu Jiang,Jiaming Xu*

Main category: cs.AI

TL;DR: PRPF是一个两阶段框架，通过先感知再推理的方式，解决了多模态大模型在主动移动代理中的干预时机与协助方式之间的目标错位和冗余推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有系统将干预决策和协助生成统一在MLLM管道中，导致保守干预过滤与全面协助生成之间的目标错位，以及在代理应保持静默时的冗余推理。

Method: 提出了Pre-Reasoning Perception Framework (PRPF)，包含轻量级多模态主动感知器(MPP)用于干预门控和上下文压缩，仅在需要干预时激活Proactive Agent Reasoner (PAR)。

Result: 在ProactiveMobile基准上，PRPF显著降低了误触发率(FTR)，同时提高了成功率(SR)和推理效率。

Conclusion: PRPF框架有效改进了主动移动代理的干预决策和协助生成，提升了性能与效率。
...
### [13] [Distilling Answer-Set Programming Rules from LLMs for Neurosymbolic Visual Question Answering](https://arxiv.org/abs/2606.03269)
*Thomas Eiter,Nelson Higuera Ruiz,Johannes Oetsch*

Main category: cs.AI

TL;DR: 本文提出了一种从大语言模型中蒸馏规则的方法，用于为视觉问答系统构建可解释的推理理论，仅需少量示例即可生成正确规则。


<details>
  <summary>Details</summary>
Motivation: 现有模块化VQA方法虽可解释，但任务需求变化时更新逻辑表示对开发者负担重。

Method: 利用大语言模型基于示例扩充回答集序言推理理论，并通过ASP求解器反馈纠正错误规则。

Result: 方法在多个VQA数据集上有效，仅需少量示例即可从LLM中提取正确规则。
...
### [14] [Solipsistic Superintelligence is Unlikely to be Cooperative](https://arxiv.org/abs/2606.03237)
*Rakshit S Trivedi,Natasha Jaques,Logan Cross,Alexander Sasha Vezhnevets,Joel Z Leibo*

Main category: cs.AI

TL;DR: 本文指出AI的核心挑战从能力转向共存，强调现有AI范式忽略部署中的内生非平稳性，导致自我破坏的合作缺失，呼吁转向非唯我论的研究范式以增强合作。


<details>
  <summary>Details</summary>
Motivation: 现有AI设计认为世界是外生且静止的，导致智能体在部署时因内生非平稳性引发训练-测试-部署差异，难以实现合作。

Method: 提出非唯我论范式，包括构建动态评估平台、将制度作为设计基础、保持人类能动性作为系统结构特征。

Result: 论证了单方面优化具有自我破坏特性，需通过合作均衡选择过程来弥合分布差异。

Conclusion: 实现AI合作需视相互依赖为核心设计原则，而非仅作为待完成任务。
...
### [15] [A Negative Result on Cross-Model Activation Transfer in a Pythia Multi-Hop Setting](https://arxiv.org/abs/2606.03280)
*Peiyan Zhang*

Main category: cs.AI

TL;DR: 论文研究了语言模型之间通过隐藏状态传递推理信息的可能性，结果发现线性翻译层虽能对齐表示空间，但注入接收模型后并未提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型是否可以通过隐藏激活而非自然语言文本，在推理时直接传递中间推理状态。

Method: 使用Pythia-160M到Pythia-410M的多跳推理设置，训练线性翻译层对齐发送者和接收者的隐藏状态，并测试不同注入方式（加法、替换、缩放）的效果。

Result: 翻译层能实现高余弦相似度（接近0.97），但注入后接收模型性能无提升，低强度加法注入接近基线，替换注入破坏性显著，缩放也无帮助。

Conclusion: 离线表示对齐在此设置下不足以实现有效的因果通信，传递隐藏状态无法改善下游推理。
...
### [16] [LEAP: Supercharging LLMs for Formal Mathematics with Agentic Frameworks](https://arxiv.org/abs/2606.03303)
...
TL;DR: 提出LEAP框架，结合基础模型能力与Lean编译器交互，实现形式化定理证明的SOTA性能；引入Lean-IMO-Bench基准，在Putnam竞赛和复杂组合问题上取得突破。


<details>
  <summary>Details</summary>
Motivation: LLM在非正式数学推理强大但难以生成机械化验证的形式化证明，现有基准逐渐饱和，需要更严格评估框架。

Method: 将复杂问题分解为小单元，通过持续与Lean编译器交互，结合非形式推理、指令遵循和迭代自我优化来构建形式化证明。

Result: 在2025年Putnam竞赛中解决全部12题；Lean-IMO-Bench上通用LLM的一次性形式化解决率从10%以下提升至70%，超越专业IMO系统的48%；自主形式化验证Knuth哈密顿分解中的关键子问题。
...
### [17] [The Reliability Gap in Benchmark Auditing: Distribution Shift and Scale as Failure Modes of Contamination Detection](https://arxiv.org/abs/2606.03305)
*Wojciech Zarzecki,Jan Dubiński,Sebastian Cygert*

Main category: cs.AI

TL;DR: 本研究系统评估了三种主流数据污染检测方法在真实审计场景下的可靠性，发现它们存在严重的可靠性差距，仅有199次（共335次）评估得出正确结果。


<details>
  <summary>Details</summary>
Motivation: 现有的训练数据成员检测方法主要在受控学术环境中验证，但在实际审计场景中，分布偏移和规模限制等未充分研究的失败模式可能影响其可靠性。

Method: 对三种检测范式（LLM Dataset Inference、Post-Hoc Dataset Inference和CoDeC）进行系统性评估，涉及27个模型（包括Pythia、OLMo 2以及特定领域文化/医学LLM，参数规模达270亿）和前沿工业模型，共335次评估。

Result: LLM Dataset Inference在分布偏移下产生假阳性，Post-Hoc Dataset Inference在基准测试规模下统计功效不足，CoDeC仅能提供粗粒度来源信号，无法验证单个基准分割。整体正确率仅59.4%（199/335）。

Conclusion: 统计检测方法尚不能替代透明的数据溯源，揭示了受控验证与实际基准审计之间的系统性可靠性差距，并开源了评估基准。
...
### [18] [The Violation Situation Pattern: A Knowledge-Graph Pattern for Compliance Violations](https://arxiv.org/abs/2606.03326)
*Nima Kamali Lassem,Fuqi Song,Seyid Amjad Ali*

Main category: cs.AI

TL;DR: 提出违规情境模式（VSP），将合规流水线中的检测结果持久化为带状态、生命周期和审计历史的图对象，并验证其规则体独立性。


<details>
  <summary>Details</summary>
Motivation: 现有合规流水线仅将违规视为瞬态查询结果，缺乏持久化对象来记录审查状态、关联实体与审计历史。

Method: 基于Gangemi和Mika的情境模式，构建VSP图模型；对四条道义规则（V1-V4）实施FCL→Cypher→MERGE流水线；通过BODACC、GDPRhub数据及SHACL交叉检查进行评估。

Result: 规则体独立性得到验证：V4从条款检测扩展至截止日期检测后F1从0.312升至0.602，而VSP的身份、生命周期和证据语义保持不变。
...
TL;DR: 提出了一种基于答案信息量的奖励机制InfoMem，用于训练长上下文任务中的分块记忆智能体，以提升最终记忆对正确答案的支持度。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法对分块记忆智能体的奖励仅基于最终答案的稀疏信号或词汇重叠，无法直接评估最终记忆是否包含支持正确答案的信息。

Method: InfoMem通过计算最终记忆对模型生成正确答案的逐词对数似然的增加量作为奖励，仅在成功轨迹上应用该信号，并在奖励组合前进行归一化处理。

Result: 在相同GRPO框架和训练预算下，InfoMem比现有记忆智能体强化学习基线方法在长上下文任务中表现更优。

Conclusion: 有效的最终记忆奖励应基于成功轨迹、归一化处理，并基于答案而非查询条件进行信号设计。
...
### [20] [CP-Agent: Context-Aware Multimodal Reasoning for Cellular Morphological Profiling under Chemical Perturbations](https://arxiv.org/abs/2606.03435)
*Yuxin Zhang,Yiyao Li,Ping Shu Ho,Simon See,Zhenqin Wu,Kevin Tsia*

Main category: cs.AI

TL;DR: 提出了CP-Agent，一种结合多模态大语言模型与细胞绘画技术，用于生成可解释的药物作用机制报告的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有细胞绘画工作流程缓慢、昂贵且难以解释，且药物筛选模型忽略实验背景，限制了泛化性和机制分辨率。

Method: 整合上下文感知对齐模块CP-CLIP，联合嵌入高内涵图像与实验元数据，并借助智能体工具推理生成结构化报告。

Result: CP-CLIP在治疗方案和机制判别上达到最高F1分数0.896，CP-Agent能生成机制相关的人类可解释理由。

Conclusion: CP-Agent通过更可解释、可扩展且上下文感知的表型筛选，加速药物发现中的假设生成循环。
...
TL;DR: 本文提出Terminal-Lego流水线，将多领域问题转化为可验证的智能体任务，发现低分智能体DeepSeek-V3.2作为教师反而更有效，归因于环境接地监督（EGS），仅用15.3k轨迹即可使Qwen3-32B达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 强代码智能体通常被认为是更好的教师，但其教学效果与任务难度、框架设计和学生能力的关联尚未被充分解耦，研究动机是厘清这一关系。

Method: 使用Terminal-Lego流水线生成环境验证的智能体任务，对比不同智能体（Claude Opus 4.6和DeepSeek-V3.2）的教学效果，提出环境接地监督（EGS）概念。

Result: DeepSeek-V3.2作为教师时学生泛化能力更强；仅用15.3k轨迹，Qwen3-32B在Terminal-Bench 2.0上达到24.3%分数，超越此前30倍数据量的SOTA。

Conclusion: 智能体后训练的前沿在于框架工程（Harness Engineering），通过系统设计环境接地交互结构来促进可复现和泛化的智能体智能。
...
TL;DR: 本文提出确定性记忆框架（DMF），一种基于CPU的全确定性管道，用于对话AI代理的记忆管理，无需LLM调用即可实现高准确率和极低的令牌成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的摘要记忆方法存在非确定性、令牌成本高和剪枝决策不透明的问题。

Method: DMF使用经典NLP分析、向量几何和数学评分计算生存分数Ω，并通过交互计数衰减定律Ω_eff(Δn)控制相关性演化。

Result: 在LoCoMo和LongMemEval数据集上，DMF与Mem0相比，零令牌准备记忆上下文，且整个对话的令牌使用量减少5倍到242倍。

Conclusion: 可以从记忆管理循环中消除LLM调用，实现几乎零令牌成本的确定性记忆系统。
...
### [23] [StepFinder: A Temporal Semantic Framework for Failure Attribution in Multi-Agent Systems](https://arxiv.org/abs/2606.03467)
*Taiyu Zhu,Yifan Wu,Weilin Jin,Ying Li,Gang Huang*

Main category: cs.AI

TL;DR: 提出StepFinder，一种轻量级失败归因框架，通过时序语义编码和参数高效模型，在Who&When基准上超越LLM方法，推理时间减少79%。


<details>
  <summary>Details</summary>
Motivation: LLM多智能体系统对单步错误高度敏感，现有归因方法依赖LLM推理原始轨迹，成本高且受噪声干扰。

Method: 仅用LLM构建特征（将执行日志编码为时序语义序列），结合时序建模与注意力模块捕获依赖关系，通过多尺度差异和位置偏置精化步骤级错误分数。

Result: 在Who&When基准上，StepFinder在步骤级失败归因中优于基于LLM的方法，推理效率显著提升（比最快LLM方法快79%）。

Conclusion: StepFinder高效实现了精准的失败归因，无需文本生成开销，为多智能体系统可靠性提供了轻量级解决方案。
...
### [24] [A formal definition and meta-model for a machine theory of mind](https://arxiv.org/abs/2606.03471)
*Fabio Cuzzolin*

Main category: cs.AI

TL;DR: 本文首次提出了机器心智理论的严格形式定义，并基于认知心理学、神经科学和人工智能的证据，用该定义审视当前研究现状，推动未来研究议程。同时提出了一个通用的整体元模型，并评估了相关基准测试的现状。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对机器心智理论的统一严格定义，导致研究分散且难以推进。

Method: 从跨学科文献中提炼关键原则，形式化定义机器心智理论，并构建元模型以整合现有方法。

Result: 提出了一个可操作的机器心智理论定义和元模型，并指出了当前基准测试的不足。

Conclusion: 严格形式化的定义和元模型有助于系统化研究，但需要更多实证工作来验证和完善。
...
TL;DR: 提出ThoughtFold框架，通过细粒度偏好学习减少大推理模型中的冗余探索，提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法强化了长CoT中的冗余探索，导致过度思考问题，而之前的方法基于结果信号，无法减少记忆冗余。

Method: 采用内省策略识别正确轨迹中的冗余，生成候选子轨迹谱系，并用掩码偏好优化目标惩罚冗余探索，鼓励直接连接关键推理段。

Result: 在DeepSeek-R1-Distill-Qwen-7B上减少约56%的token使用，同时保持最先进精度。

Conclusion: ThoughtFold能有效折叠推理链为更简洁路径，兼顾效率与准确性。
...
### [26] [Overlaying Governance: A Compositional Authorization Framework for Delegation and Scope in Agentic AI](https://arxiv.org/abs/2606.03518)
*Amjad Ibrahim,Yong Li*

Main category: cs.AI

TL;DR: 提出一个组合治理框架，为自主AI代理引入递归委派、上下文边界和动态作用域等原语，支持可问责授权。


<details>
  <summary>Details</summary>
Motivation: 现有IAM和OAuth等框架无法处理代理系统的递归委派、动态权限和合同级委派语义。

Method: 定义了委派类型、权限与问责关系、资源作用域衰减概念，并通过组合算子将新语义叠加到现有关系策略上。

Result: 通过形式化证明和实证评估，验证了框架在代理系统中的可问责性和实用性。

Conclusion: 该框架为代理AI提供了形式化且可操作的授权基础，支持递归委派链等复杂场景。
...
### [27] [SAGE: A Quantitative Evaluation of Socialized Evolution in Agent Ecosystems](https://arxiv.org/abs/2606.03544)
*Linyue Pan,Yaoming Zhu,Lin Qiu,Xuezhi Cao,Xunliang Cai*

Main category: cs.AI

TL;DR: 本文提出SAGE框架研究社交演化与自我演化对语言代理性能的影响，发现群体历史并非普遍放大器，但能帮助陷入瓶颈的代理实现突破，且社交收益依赖于抽象能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究孤立评估自我改进的代理，但现实中代理常与同伴共享经验，需探讨共享经验能否产生超越自我改进的额外收益。

Method: 构建SAGE框架，对比SocialEvo（多模型家族代理共享历史）与SelfEvo（仅自我历史）在ML研究、经济规划和多人游戏三个场景的演化表现。

Result: 最强代理未超越自我演化上限；但自我改进停滞的代理可通过同伴经验实现突破；竞争环境中代理提升具有通用性；过滤后的同伴轨迹和反思摘要优于原始日志。

Conclusion: 同伴历史收益具有代理特异性、场景依赖性，且取决于从公共轨迹中抽象可迁移知识的能力。
...
### [28] [From Prompt to Service: An SLM-Based Agent Orchestration Gateway for AI-Driven Virtual Worlds](https://arxiv.org/abs/2606.03557)
*Louis Nisiotis,Aimilios Hadjiliasi*

Main category: cs.AI

TL;DR: 提出基于SLM的智能体编排网关，通过意图驱动服务路由解耦虚拟世界客户端与异构AI后端，支持边缘部署和可扩展AI服务编排。


<details>
  <summary>Details</summary>
Motivation: AI驱动的虚拟世界面临后端模型异构、资源需求多样、系统扩展性和维护性差、边缘/云服务协调困难等挑战。

Method: 设计SLM编排网关：边缘SLM分类用户意图→服务注册表验证并解析路由→透明调用后端。采用任务微调优化小参数模型作为路由器，搭配大模型实现对话生成。

Result: 在InterwovenXR虚拟博物馆测试中，紧凑SLM可作为可靠意图路由器；微调子十亿参数模型实现低延迟路由；双层配置（微调小模型+大模型）在中等边缘硬件上更高效。

Conclusion: SLM能支持虚拟世界的实用AI服务编排，该架构实现了可扩展、可扩展、边缘支持的AI交互，使虚拟代理成为分布式生成AI服务的访问点。
...
### [29] [Cross-Lingual Token Arbitrage: Optimizing Code Agent Context Windows via Local LLM Preprocessing](https://arxiv.org/abs/2606.03618)
*Mehmet Utku Colak*

Main category: cs.AI

TL;DR: 提出了一种预检、边缘侧的提示重写中间件，通过本地小模型将开发者输入优化后再发送给云端AI编码代理，显著降低令牌成本而不影响编码质量。


<details>
  <summary>Details</summary>
Motivation: AI辅助编码代理受输入令牌成本限制，原始人类输入存在非英语文本的令牌化低效和对话提示的结构性熵两个问题，现有方法只能被动压缩或事后干预。

Method: 在开发者和云代理之间部署本地Llama 3.2 (3B)模型，执行跨语言翻译为英语、结构性重写为紧凑任务格式，并通过正则验证和回退机制确保优化提示不超过原始大小。

Result: 在涵盖土耳其语、阿拉伯语、中文和代码混写的OMH-Polyglot基准测试中，提示令牌减少34%-47%，总令牌减少最多18.8%，同时保持或提升任务准确率；消融实验显示收益主要来自重写阶段；与LLMLingua-2相比，在相同压缩率下OckScore更优。

Conclusion: 主动提示优化能大幅降低推理成本而不牺牲编码质量，表明预检边端处理是一种有效方法。
...
### [30] [Bridging Auxiliary Constraints to Resolve Instruction Following in Large Reasoning Models](https://arxiv.org/abs/2606.03624)
*Zhengyi Zhao,Shubo Zhang,Huimin Wang,Zezhong Wang,Yutian Zhao,Yefeng Zheng,Binyang Li,Yulan He,Kam-Fai Wong,Xian Wu*

Main category: cs.AI

TL;DR: 提出约束关系图补全框架，通过桥接约束减少大型推理模型的多指令遵循冲突，降低约束违反率39%。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在多指令遵循中常失败，无法平衡或满足多个约束。

Method: 将指令表示为约束知识图谱，显式建模约束间关系，识别并发现桥接约束以增强主要约束的相容性。

Result: 在三个数据集上，约束违反率降低39%，同时保持推理能力。

Conclusion: CRGC通过利用模型自身知识创建更好的生成路径，有效提升约束遵循能力。
...
### [31] [TSQAgent: Rating Time Series Data Quality via Dedicated Agentic Reasoning](https://arxiv.org/abs/2606.03629)
*Shunyu Wu,Dan Li,Haozheng Ye,Weibin Feng,Jian Lou,Bo Zhang,Wenjie Feng,Chenjuan Guo,See-Kiong Ng*

Main category: cs.AI

TL;DR: 本文提出TSQBench基准测试和TSQAgent框架，评估并增强大语言模型在时间序列数据质量评估中的能力，显著提升了质量理解和定量比较的性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在时间序列质量评估中依赖预定义维度且仅进行文本推理，无法自动识别相关维度或进行有依据的定量比较。

Method: 构建TSQBench基准，提出TSQAgent框架，包含Perceiver（感知器）、Inspector（检查员）和Adjudicator（裁决者）三个角色，并结合外部分析工具进行智能推理。

Result: 在基准测试和11个真实数据集上，TSQAgent显著提升LLM在质量理解和定量比较方面的能力，进而改善数据选择和下游任务性能。

Conclusion: 所提出的框架能有效增强LLM对时间序列质量的多维评估能力，并提高数据效率。
...
### [32] [Gender-Dependent Diagnostic Substitution in LLM Medical Triage: Same Symptoms, Unequal Urgency](https://arxiv.org/abs/2606.03641)
*Qi Han Wong*

Main category: cs.AI

TL;DR: 大语言模型在神经症状分诊中表现出系统性性别偏见，年轻女性急诊转诊率显著低于同龄男性，而老年人群中无差异。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型是否因患者性别和年龄差异产生不同分诊建议，验证其是否复现人类临床偏见。

Method: 使用三个模型家族，对标准化症状描述在七个年龄-性别组合下进行630次试验，统计急诊转诊率并分析分诊机制。

Result: 年轻女性急诊转诊率显著低于男性（如Gemini：0% vs 23.3%），诊断替代是关键：女性被优先诊断为特发性颅内高压，男性为颅内压增高伴占位病变。

Conclusion: 临床LLM利用流行病学先验压制分诊紧迫性，需将紧迫性评估与概率诊断先验解耦以避免偏见。
...
### [33] [Towards Non-Monotonic Entailment in Propositional Defeasible Standpoint Logic](https://arxiv.org/abs/2606.03655)
*Nicholas Leisegang,Thomas Meyer,Ivan Varzniczak*

Main category: cs.AI

TL;DR: 本文提出了一种将非单调理性蕴涵关系引入命题可废止立场逻辑（PDSL）片段的方法，通过情境立场条件句扩展其表达能力，并将推理问题转化为命题情况下的算法，同时保持复杂度上界。


<details>
  <summary>Details</summary>
Motivation: 现有可废止推理在模态逻辑中主要关注满足性检查和单调蕴涵，这可能导致推理能力较弱，且缺乏非单调理性蕴涵关系在立场逻辑中的系统方法。

Method: 通过引入情境立场条件句扩展PDSL的表达能力，重新描述其语法，将PDSL片段表示为条件句集合；然后定义一种基于排位的蕴涵关系传递方法，将命题情况下的理性闭合和字典序闭合等非单调蕴涵关系忠实翻译到PDSL。

Result: 证明了PDSL的大片段可表示为情境条件句，并建立了从命题推理到PDSL的非单调蕴涵关系传递方法；展示了理性闭合和字典序闭合的翻译，并说明蕴涵检查可复用命题算法且保持复杂度上界。

Conclusion: 该工作首次将KLM风格的非单调理性蕴涵关系系统性地引入立场模态逻辑，拓展了可废止推理在立场逻辑中的应用，并提供了高效的计算框架。
...
### [34] [Diagnosing Knowledge Gaps in LLM Tool Use: An Agentic Benchmark for Novel API Acquisition](https://arxiv.org/abs/2606.03657)
*Jinnuo Liu,Yue Peng,Jinhan Niu,Hongyi Wen*

Main category: cs.AI

TL;DR: 提出NovelAPIBench动态基准测试，系统评估代码生成模型在使用未见API时的表现，发现知识组件非等价、上下文增加可能有害、微调与检索互补。


<details>
  <summary>Details</summary>
Motivation: 现有新颖API基准测试多为静态、依赖粗糙指标或使用合成API，无法真实反映库演化及模型在复杂API使用中的挑战。

Method: 构建全自动化动态基准，从目标库发现新颖API，提取知识束（签名、机制、示例等），生成可执行编码任务，将失败样本归入六类诊断类别，并对比检索增强与参数微调两种知识注入方式。

Result: 使用示例是最强的单一信号；最佳双组件设置是签名与机制或示例（取决于领域/模型）；添加源代码等上下文可能增加导入错误；微调主要教会模型使用知识束而非替代检索，且能力可迁移到未见库。

Conclusion: 检索提供易变的API内容，微调改进程序集成能力，两者互补而非替代。
...
### [35] [From Answers to States: Verifiable Process-Level Evaluation of Chemical Reasoning in Large Language Models](https://arxiv.org/abs/2606.03660)
*Hongyu Guo,Hao Li,He Cao,Gongbo Zhang,Li Yuan*

Main category: cs.AI

TL;DR: 提出了ChemCoTBench-V2，一个低成本、可审计的化学推理基准，通过规则验证中间步骤来诊断模型的化学逻辑错误。


<details>
  <summary>Details</summary>
Motivation: 现有化学基准只评分最终答案，忽略了模型推理可能违反化学逻辑的失败模式，且现有过程评估方法成本高、不一致且易出现幻觉。

Method: 构建包含5620个样本的基准，涵盖分子理解、编辑、优化和反应预测。模型需在专家设计模板中暴露中间步骤，并用确定性化学规则和参考轨迹进行验证，分子优化使用oracle可验证的状态约束。

Result: 前沿模型在最终答案正确性与结构化推理状态一致性之间存在持续差距，模型常遵循格式但化学步骤检查失败，或正确回答但推理薄弱。

Conclusion: ChemCoTBench-V2能实现细粒度模型比较，并精确定位推理路径首次违反验证器的具体步骤。
...
### [36] [EvoDrive: Pareto Evolution for Safety-Critical Autonomous Driving via Self-Improving LLM Agents](https://arxiv.org/abs/2606.03678)
*Tong Nie,Yuewen Mei,Yihong Tang,Junlin He,Jie Deng,Jian Sun,Wei Ma*

Main category: cs.AI

TL;DR: EvoDrive是一个基于LLM的自动化多目标场景生成框架，通过模拟器驱动的演员-评论家架构和帕累托存档，有效平衡了对抗性与真实性，显著扩展了帕累托前沿并生成了有价值的策略训练场景。


<details>
  <summary>Details</summary>
Motivation: 现有安全关键场景生成方法多依赖手工启发式，局限于已知模式且无法有效处理多目标冲突，而开放式的无约束代理又缺乏模拟器严格约束，易导致目标单一化。

Method: 采用模拟器驱动的演员-评论家架构，包含记忆驱动演员迭代改进生成器、评论家过滤不可行候选，以及自演化世界评估器优化模拟预算；同时维护帕累托存档以保持多样化的攻击-真实性权衡。

Result: 在MetaDrive和CARLA基准测试中，EvoDrive显著扩展了各生成器的帕累托前沿，并生成了对策略训练有价值的场景。

Conclusion: EvoDrive展示了基于LLM的代理进化在多目标场景生成中的有效性，为自动驾驶系统的安全验证提供了新方法。
...
### [37] [The DeepSpeak-Agentic Dataset](https://arxiv.org/abs/2606.03686)
*Sarah Barrington,Maty Bohacek,Hany Farid*

Main category: cs.AI

TL;DR: 提出了DeepSpeak-Agentic数据集，包含超过37小时的人与具身AI代理的半结构化对话视频，用于评估AI代理的司法鉴定、人机交互研究，并作为基准测试。同时贡献了一个可扩展的数据采集系统。


<details>
  <summary>Details</summary>
Motivation: 缺乏公开的、高质量的人与具身AI代理对话视频数据集，难以进行AI代理的司法鉴定以及人机交互研究。

Method: 构建了一个可扩展的数据采集系统，自动创建代理并配对人工众包工作者，记录指定场景下的音视频对话，并从混合流中分离人类和代理。

Result: 创建了DeepSpeak-Agentic数据集（超过37小时视频），并可用于评估AI代理的自动司法鉴定（音频、视频或文本），研究人机交互特性，以及作为未来大语言模型和AI语音/人脸基准。

Conclusion: 该数据集和采集系统为AI代理的司法鉴定和人机交互研究提供了有价值的资源与工具。
...
### [38] [SkillPyramid: A Hierarchical Skill Consolidation Framework for Self-Evolving Agents](https://arxiv.org/abs/2606.03692)
*Yuan Xiong,Ziqi Miao,Qian Chen,Lijun Li,Yequan Wang,Shizhu He,Jun Zhao,Kang Liu*

Main category: cs.AI

TL;DR: 提出SkillPyramid框架，通过分层技能拓扑和自我进化机制，将技能从静态资源池转化为动态进化系统，在多个环境中平均奖励提升38.0%，执行步骤减少27.7%。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体缺乏系统的技能构建、积累和迁移框架，导致重复构建能力、经验难以转化为可复用资产、技能泛化能力弱。

Method: SkillPyramid框架，基于分层技能拓扑结构，引入自我进化机制，使智能体在任务执行过程中能够组合、验证并纳入新技能。

Result: 在ALFWorld、WebShop和ScienceWorld上，使用四种骨干模型，平均奖励提升38.0%，执行步骤减少27.7%。

Conclusion: 该方法将技能集合从静态资源池转变为动态进化系统，显著提升了智能体的任务性能和泛化能力。
...
### [39] [Dynamic Objective Selection with Safeguards and LLM Oversight for Financial Decision-Making](https://arxiv.org/abs/2606.03704)
*Keigo Sakurai,Takahiro Ogawa,Miki Haseyama,Anjyu Anan,Kei Nakagawa*

Main category: cs.AI

TL;DR: 提出DOSS框架，通过分类学习与置信度门控，从统计摘要中动态选择投资目标函数，并引入LLM作为安全监督，避免频繁切换和误选。


<details>
  <summary>Details</summary>
Motivation: 固定目标函数在市场条件变化时表现不佳，而隐式状态切换方法存在噪声大、延迟高和操作不稳定的问题。

Method: 将目标选择建模为分类问题，使用滚动窗口进行序列更新，并采用置信度感知门控和切换频率控制，LLM仅作为监督组件，不生成新目标。

Result: DOSS能基于近期收益统计摘要做出前瞻性选择，且通过置信度门控和LLM监督降低了误选率和切换频率。

Conclusion: DOSS提供了无需隐式状态的动态目标选择方法，结合规则和LLM监督，提升了金融决策的鲁棒性和可解释性。
...
### [40] [Code-on-Graph: Iterative Programmatic Reasoning via Large Language Models on Knowledge Graphs](https://arxiv.org/abs/2606.03705)
*Weiwei Ding,Zixuan Li,Long Bai,Zhuo Chen,Kun Su,Fei Wang,Xiaolong Jin,Jin Zhang,Jiafeng Guo,Xueqi Cheng*

Main category: cs.AI

TL;DR: 本文提出Code-on-Graph（CoG），一种基于代码的推理框架，通过将知识图谱模式表示为Python类并生成可执行代码，解决了现有LLM-KG集成中灵活性和可扩展性不足的问题。在三个基准数据集上，CoG超越了先前最优模型达10.5%。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM-KG集成框架受限于预定义操作符，导致表达不灵活且无法处理大规模事实知识，因此需要一种更灵活、可扩展的集成方法。

Method: CoG在每个推理步骤中检索事实知识，将其对应的KG模式表示为Python类，然后生成基于这些类的可执行代码，在运行时将检索的事实实例化为对象，避免直接注入大规模知识。

Result: 在WebQSP、CWQ和GrailQA数据集上，CoG相比先前最优模型取得了最高10.5%的性能提升。

Conclusion: CoG通过程序化推理实现了灵活且可扩展的LLM-KG集成，显著提升了问答任务的准确性。
...
### [41] [Unveiling the Structure of Do-Calculus Reasoning via Derivation Graphs](https://arxiv.org/abs/2606.03719)
*Clément Yvernes,Emilie Devijver,Marianne Clausel,Eric Gaussier*

Main category: cs.AI

TL;DR: 本文引入了推导图来表示do-calculus规则的组合应用，描述了其等价空间，并提出最多四次规则应用即可完成转换，同时利用等价查询提高估计效率。


<details>
  <summary>Details</summary>
Motivation: do-calculus规则组合和排序困难，缺乏系统方法来表示等价干预表达式。

Method: 提出推导图（derivation graphs）来表征do-calculus规则应用和组合，并设计仅需最多四次规则应用的简单程序。

Result: 推导图能完整描述do-calculus下等价观测与干预概率空间，且等价查询经识别算法可得到多个有效 estimands，产生更高效估计器。

Conclusion: 推导图简化了do-calculus推理，并可通过等价因果查询提升估计效率。
...
### [42] [When to Re-Plan: Subgoal Persistence in Hierarchical Latent Reasoning](https://arxiv.org/abs/2606.03741)
*Ayushi Chadha*

Main category: cs.AI

TL;DR: 论文研究了潜在推理系统中的稳定性-适应性权衡，通过分层推理模型（HRM）中的封建式管理-工人接口，发现子目标持久性（中等范围P∈[3,6]）是关键，比频繁或过长计划更优。


<details>
  <summary>Details</summary>
Motivation: 动机在于解决长时域推理中重新规划频繁导致计算不连贯，或承诺过长导致计划过时的矛盾。

Method: 使用HRM模型，结合缓慢的高级模块定期发出归一化方向性子目标，持续P步低层步骤，通过余弦对齐损失偏置隐藏状态更新。

Result: 在ARC和ConceptARC上，中等持久性P=3时LM损失最低（1.544 vs. 1.674 at P=1, 1.640基线），对齐权重λ≈0.05为最优；消融实验表明定向结构是关键。

Conclusion: 潜在推理系统需中时域意图连贯跨越足够步骤以形成组合结构，这是设计原则。
...
### [43] [Proof-Refactor: Refactoring Generated Formal Proofs into Modular Artifacts](https://arxiv.org/abs/2606.03743)
*Yiming Fu,Peixuan Liu,Zichen Wang,Kun yuan*

Main category: cs.AI

TL;DR: 论文提出Proof-Refactor框架，将证明重构分解为提取、设计、证明和修复四阶段，改善大模型生成证明的可读性和模块化。


<details>
  <summary>Details</summary>
Motivation: 大模型生成的正式证明缺乏可读性、模块化等特性，现有方法过度依赖长度等单一指标，无法捕捉高阶质量。

Method: 提出Proof-Refactor框架，模仿人类重构流程，包含提取候选片段、设计辅助声明、形式化证明组件、修复原证明四个阶段。

Result: 在PutnamBench和Putnam2025数据集上，Proof-Refactor在基于评分的重构指标上超越Claude Code基线，尤其在签名质量和可读性方面提升最大。

Conclusion: 过程引导的重构方法能在不将证明长度作为主要目标的情况下改善证明结构。
...
### [44] [Enhancing Operational Safety via Agentic Dialogue Hazard Identification Analysis](https://arxiv.org/abs/2606.03812)
*Sanjay Das,Ran Elgedawy,Ethan Seefried,Ryan Burchfield,Tirthankar Ghosal*

Main category: cs.AI

TL;DR: 提出HAZDIAL框架，通过多代理结构化对话提升危险识别质量，优于单次推理基线。


<details>
  <summary>Details</summary>
Motivation: 高风险领域（如工业过程控制）中，单次大语言模型推理缺乏自我纠正和迭代优化能力。

Method: 设计了基于算法的对抗性辩论与建设性讨论两种对话模式，并优化代理交互。

Result: 在标准分类指标（准确率、精确率、召回率、F1）及新对话指标上，均优于单次基线。

Conclusion: 对话驱动的多代理推理能有效增强AI安全分析中的危险识别性能。
...
### [45] [From Control Boundary to Insurance Claim: Reconstructing AI-Mediated Losses Through the CER Framework](https://arxiv.org/abs/2606.03777)
*Alex Leung,Rex Zhang,Kentaroh Toyoda,SiewMei Loh*

Main category: cs.AI

TL;DR: 本文提出CER诊断框架，解决生成式/代理式AI系统损失中的状态重建问题，而不仅是事件重建，以支持保险理赔。


<details>
  <summary>Details</summary>
Motivation: AI系统在推理、检索、调用工具和行动时状态持续变化，传统事件重建不足以应对保险理赔需求，需要状态和因果链重建。

Method: 定义CER框架：C（控制边界）检查系统是否有可执行的操作范围，E（证据重建）检查能否从保留工件重建系统状态和因果链，R（保险响应）检查重建损失是否在保险范围内并具备理赔证据。

Result: 贡献三点：定义AI特定重建问题，通过CER操作化该问题，指定理赔级证据要求；公共案例包括PocketOS、Replit代理数据库删除事件和Moffatt v. Air Canada案。

Conclusion: CER框架为AI剩余风险转移提供用例级诊断，使保险理赔支持更可靠，需关注外部触发故障如提示注入、RAG投毒等。
...
### [46] [LAP: An Agent-to-Instrument Protocol for Autonomous Science](https://arxiv.org/abs/2606.03755)
*Linwu Zhu,Liqiang Gao,Yan Chen,Dan Zhu,Jian Huang*

Main category: cs.AI

TL;DR: 提出了实验室智能体协议（LAP），填补了自主科学中智能体与物理仪器间协议空白，支持状态化、安全关键操作与可复现测量。


<details>
  <summary>Details</summary>
Motivation: 现有自主科学系统需从零构建推理智能体与物理仪器的连接，而现有智能体互操作协议（MCP、A2A）未覆盖智能体-仪器边缘，该边缘具有状态化、安全关键、独占性、物理嵌入及测量不确定性等特性。

Method: 基于A2A对等发现-任务生命周期结构，增加四个物理世界原语：InstrumentCard能力描述、独占保留、安全围栏握手（操作确认令牌加密绑定任务参数）、MeasurementResult物理类型化模式（QUDT/UCUM、校准锚定、不确定性承载）。定义了六层架构、JSON-RPC方法集、任务/安全状态机、错误模型及跨实验室联邦。

Result: LAP与A2A/MCP生态传输兼容，封装而非替代现有设备标准（SiLA 2、OPC-UA），并通过端到端闭环自主实验验证。

Conclusion: LAP协议有效连接推理智能体与物理仪器，保障了状态化操作、安全关键流程和可复现测量，支持跨实验室联邦协作。
...
### [47] [Leveraging BART to Assess CS1 C++ Programming Assignments using Rubric-based Criteria](https://arxiv.org/abs/2606.03814)
*Kelsey Rainey,Jesse Roberts*

Main category: cs.AI

TL;DR: 该论文研究了基于评估标准的Transformer模型多任务微调，用于自动评分C++编程作业，以生成更贴近教师评分行为的预测。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在自动评分中不能充分反映教师评分行为，而现有方法忽略了评分分布对齐。

Method: 使用BART编码器-解码器，结合LoRA适配，进行多任务训练预测数值分数和分数段，引入分布匹配项，并比较单/多任务、硬/软标签、有无评估标准等条件。

Result: 多任务BART结合边界软标签和评估标准，实现了较低的平均绝对误差和更好的分数分布对齐；全微调T5进一步提升了分布保真度。

Conclusion: 校准感知、评估标准引导的训练能产生更接近教师行为的评分，优于仅优化准确率的替代方法。
...
### [48] [BigFinanceBench: A Workflow-Grounded Benchmark for Financial-Research Agents](https://arxiv.org/abs/2606.03829)
*Alex Wang,Georg Meinhardt,Jacob Katz,Joseph H. Kim,Pratyush K. Chaudhary,Chase Blagden,Eric Xu*

Main category: cs.AI

TL;DR: 提出BigFinanceBench基准，强调金融研究答案的可审计推导过程，而非仅最终结果。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准多数只评估孤立子技能或最终答案，忽略决策相关推导过程的可审计性。

Method: 构建包含928项专家编写的开放式金融研究任务，每项包含参考答案和点权重评分规则，分解推导为独立可检查步骤。

Result: 在36241个评分点上，最佳系统仅达58.8%评分；最终答案准确率是推导质量的有损代理；模型能力在金融工作流中非均匀分布。

Conclusion: 金融推导质量仍存在显著提升空间，现有模型无法完全实现可审计的金融分析。
...
### [49] [Calibrating Urban Traffic Simulation from Sparse Road Observations via Genetic Optimization](https://arxiv.org/abs/2606.03823)
*Hunter Sawyer,Jesse Roberts,Simon Matei*

Main category: cs.AI

TL;DR: 提出一种基于遗传算法的框架，仅需稀疏的道路观测数据即可校准城市交通模拟，无需详细的就业分布数据，并在格林斯博罗市验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统交通模拟依赖详细的实时交通数据和就业分布数据，但大多数城市缺乏此类高分辨率数据，导致模拟难以推广。

Method: 使用遗传算法优化就业分布和门控交通参数，使模拟流量与少量已知道路流量匹配，基于SUMO平台实现。

Result: 模拟流量与真实测量值高度相关，能泛化到未训练道路，且优化出的就业分布与普查数据定性一致。

Conclusion: 通过最小真实观测数据即可实现逼真的城市交通模拟，降低了部署交通模型的门槛，具有可扩展性和数据轻量性。
...
### [50] [EvoDS: Self-Evolving Autonomous Data Science Agent with Skill Learning and Context Management](https://arxiv.org/abs/2606.03841)
*Zherui Yang,Fan Liu,Yansong Ning,Hao Liu*

Main category: cs.AI

TL;DR: 提出了一个自进化的数据科学智能体EvoDS，通过强化学习自动扩展技能和管理长期上下文，在四个基准上平均提升28.9%，消除了令牌溢出错误。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在自动化数据科学中存在动作集静态、缺乏长期上下文管理，无法积累可复用经验和可靠地处理多阶段迭代流程。

Method: 自主技能获取（ASA）机制和自适应上下文压缩（ACC）策略，通过两阶段多智能体训练方案进行优化。

Result: 在四个不同基准上平均优于现有开源数据科学智能体28.9%，并消除了令牌溢出失败。

Conclusion: EvoDS通过分层设计减少工具选择错误，优化目标与信息瓶颈原则一致，实现了高效上下文使用和自主改进。
...
### [51] [Reasoning Structure of Large Language Models](https://arxiv.org/abs/2606.03883)
*Frédéric Berdoz,Luca A. Lanzendörfer,Fabian Farestam,Roger Wattenhofer*

Main category: cs.AI

TL;DR: 提出了一种通过逻辑谜题和推理图评估大推理模型（LRM）的结构化方法，并定义了推理效率指标。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标（如准确率和token数）无法区分推理结构的差异。

Method: 构建逻辑谜题基准，将推理轨迹转换为可验证的推理图，并定义基于拓扑的推理效率指标。

Result: 结构测量能区分被token数和准确率混淆的行为，有助于诊断失败模式和比较推理缩放特性。

Conclusion: 该方法为LRM推理质量提供了更细致的分析工具。
...
### [52] [PyraMathBench: Evaluating and Improving Mathematical Capability in Large Language Models](https://arxiv.org/abs/2606.03858)
*Zetian Ouyang,Linlin Wang,Gerard de Melo,Liang He*

Main category: cs.AI

TL;DR: 提出了PyraMathBench基准和SOLVE+IRPO方法，提升大语言模型在数值推理任务上的表现。
...
Method: 构建含32,505个问题的层级基准PyraMathBench，并设计智能优化与学习模块SOLVE及交互式相对策略优化IRPO，通过模糊匹配和低质量调用拒绝来增强数值-数学协同。

Result: Qwen-2.5模型在应用SOLVE和IRPO训练后，分数提升了5.0分。

Conclusion: 通过有效的工具调用策略，可以显著改善大模型在数值推理任务中的性能。
...
### [53] [scTranslation: A Comprehensive Benchmark for Single-Cell Multi-Omics Modality Translation](https://arxiv.org/abs/2606.03906)
*Jiabei Cheng,Jingbo Zhou,Jun Xia,Changkai Li,Zhen Lei,Chang Yu,Stan Z. Li*

Main category: cs.AI

TL;DR: 提出了scTranslation基准，用于系统评估单细胞多组学模态翻译方法，包含多样化数据集、先进模型和综合评价指标。


<details>
  <summary>Details</summary>
Motivation: 现有模态翻译模型缺乏系统基准评估，尤其在数据集、评价指标和影响因素方面。

Method: 构建scTranslation基准，整合多组翻译数据集和最新模型，设计综合评估指标，并测试特征选择、质量及少样本场景下的性能。

Result: 大规模研究发现特征选择、数据质量和少样本设置显著影响模型性能，并提出新见解。

Conclusion: 为单细胞多组学模态翻译提供了全面基准，促进了未来研究发展。
...
### [54] [Hedge-Bench: Benchmarking Agents on Hard, Realistic Tasks Pertaining to Financial Reasoning](https://arxiv.org/abs/2606.03918)
*Eric Cho,Shawn Huang,Alice Lu,Andy Lyu*

Main category: cs.AI

TL;DR: 提出了Hedge-Bench 1.0基准，包含102个对冲基金分析师的实际任务，支持确定性评分，前沿模型和智能体得分低于16%。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法捕捉金融分析中的开放式推理问题，且依赖模型评判带来噪声和循环论证。

Method: 基于专业分析师的实际工作轨迹构建基准，通过已验证的专家步骤进行确定性评分。

Result: 前沿模型和智能体在该基准上的得分低于16%。

Conclusion: 当前AI在金融分析师的开放式推理任务上表现不足，数据集和评估工具已公开。
...
### [55] [Entropy Is Not Enough: Unlocking Effective Reinforcement Learning for Visual Reasoning via Vision-Anchored Token Selection](https://arxiv.org/abs/2606.03937)
...
TL;DR: 论文发现文本RLVR中的token熵机制在视觉推理中失效，提出VEPO框架通过视觉敏感性与token熵的乘法耦合实现有效策略优化。


<details>
  <summary>Details</summary>
Motivation: 现有token熵机制在视觉推理中崩溃，多模态方法缺乏系统性视觉度量或忽视熵驱动语义探索。

Method: 提出VEPO，通过视觉敏感度与token熵的乘法耦合重新分配梯度信用于同时具备视觉基础和高度信息性的token。

Result: VEPO在7B和3B规模上分别比熵基线高2.28和3.15个点。

Conclusion: VEPO有效融合视觉感知与语义推理，在视觉推理任务中超越现有方法。
...
### [56] [Imaginative Perception Tokens Enhance Spatial Reasoning in Multimodal Language Models](https://arxiv.org/abs/2606.03988)
...
TL;DR: 提出想象感知标记(IPT)，增强VLM在不可见空间配置下的推理能力，并构建三个空间推理任务。


<details>
  <summary>Details</summary>
Motivation: VLM在空间推理任务中因缺乏直接观察信息而表现不佳，现有方法强制使用语言链式推理存在模态不匹配问题。

Method: 设计IPT作为中间感知表征，在统一VLM骨干BAGEL上进行监督训练，结合多视角计数、路径追踪等任务的数据集。

Result: IPT显著提升空间推理准确率（如MVC任务提升3.4%），且无需生成图像即可超越文本链式推理，但纯文本链式推理会降低性能。

Conclusion: IPT提供了一种有效的空间推理监督信号，兼顾泛化性与可解释性，避免语言模态带来的性能损失。
...
### [57] [Towards Compact Autonomous Driving Perception with Balanced Learning and Multi-sensor Fusion](https://arxiv.org/abs/2606.02979)
*Oskar Natan,Jun Miura*

Main category: cs.CV

TL;DR: 提出了一种紧凑型多任务学习模型，可同时处理语义分割、深度估计、LiDAR分割和鸟瞰图投影等自动驾驶感知任务，并通过自适应损失加权算法解决不平衡学习问题。


<details>
  <summary>Details</summary>
Motivation: 现有模型通常需要多个独立模型支持，处理多模态传感器数据时效率低下且计算资源消耗大。

Method: 采用紧凑深度学习架构、自适应损失加权算法，结合数据预处理和中间传感器融合技术，处理来自RGB摄像头、动态视觉传感器和LiDAR的多种输入。

Result: 模型在参数更少的情况下性能更优，推理速度更快且GPU内存占用更低，并在多个数据集上表现一致。

Conclusion: 该模型能高效处理多任务自动驾驶感知，为未来研究提供了开源代码。
...
### [58] [Plan2Map: A Multimodal Benchmark for Document-Grounded Geospatial Boundary Reconstruction from Planning Records](https://arxiv.org/abs/2606.02747)
*Fabian Degen,Oishi Deb,Jindong Gu,Junchi Yu,Samuele Marro,Philip Torr,Jialin Yu*

Main category: cs.CV

TL;DR: 本文提出Plan2Map基准测试及GeoPlanAgent系统，用于从英国规划记录中重建地理空间边界，GeoPlanAgent在IoU指标上显著优于直接VLM方法。


<details>
  <summary>Details</summary>
Motivation: 规划记录中的地理边界通常以间接空间证据形式存在，缺乏机器可读边界，因此需要自动化重建方法。

Method: GeoPlanAgent系统将任务分解为证据提取、定位、地图配准、边界分割、投影和验证，并采用多模态工具循环架构。

Result: GeoPlanAgent在Plan2Map上平均IoU为0.736，中位IoU为0.904，67.8%预测的IoU≥0.8，远超直接VLM基线。

Conclusion: Plan2Map为多模态地理空间边界重建提供了具体测试平台，监督式边界分割显著提升了像素级掩码质量，但定位和地图配准仍存在误差。
...
### [59] [Inverting the Generation Process of Denoising Diffusion Implicit Models: Empirical Evaluation and a Novel Method](https://arxiv.org/abs/2606.03111)
*Yan Zeng,Masanori Suganuma,Takayuki Okatani*

Main category: cs.CV

TL;DR: 提出混合方法来反演DDIM图像生成过程，显著提高初始噪声图预测和重建精度，并通过自插值测试验证效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在反演DDIM过程中预测潜在变量（初始噪声图）的精度不足。

Method: 结合梯度下降（第一步）和不动点方法（后续步骤）的混合反演策略。

Result: 在三个数据集上优于现有方法，在初始潜在变量预测和重建精度上均取得更好表现，自插值测试中显著领先。

Conclusion: 该方法为扩散模型在图像生成与编辑中的应用提供了更准确的潜在变量预测，并引入了自插值测试作为新的评估指标。
...
### [60] [FAF-CD: Frequency-Aware Fusion for Change Detection under Imperfect Multimodal Remote Sensing](https://arxiv.org/abs/2606.03114)
*Yufan Wang,Sokratis Makrogiannis,Chandra Kambhamettu*

Main category: cs.CV

TL;DR: 提出FAF-CD混合框架，结合DINOv3预训练ConvNeXt编码器和VMamba解码器，通过频率感知与三分支融合模块解决异构遥感变化检测中的噪声干扰问题，在BRIGHT、LEVIR-CD和WHU-CD数据集上取得最优性能，同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现实遥感变化检测常面临异构观测（如异步、跨传感器、光照/季节/模态差异），尤其是EO-SAR灾害制图中，无关变化易与结构损坏混淆，现有方法难以应对。

Method: 提出频率感知混合框架FAF-CD，包含DINOv3预训练ConvNeXt编码器、线性复杂度VMamba解码器，以及矫正感知三分支融合模块（结合可变形空间对齐、傅里叶与哈尔小波比较，通过自适应门控聚合多尺度互补特征）。

Result: 在BRIGHT数据集上，异构EO-SAR适配提升干净与扰动tc-mIoU/tc-mAP；在LEVIR-CD和WHU-CD上分别达0.924和0.955 cF1；在伪变化对齐压力测试中取得最佳平均扰动cIoU/cF1；相比NeXt2Former-CD减少24 GFLOPs计算量。

Conclusion: FAF-CD有效提升了异构遥感变化检测的鲁棒性和泛化能力，同时降低了计算开销，适用于实际灾害监测场景。
...
### [61] [GuidedBridge: Training-freely Improving Bridge Models with Prior Guidance](https://arxiv.org/abs/2606.03119)
*Zehua Chen,Yucheng Yang,Binjie Yuan,Kaiwen Zheng,Jun S. Liu,Jun Zhu*

Main category: cs.CV

TL;DR: 提出一种基于先验引导的桥接模型训练后引导方法，通过对比强弱先验提升生成质量，并设计频率调制引导(CG-FMPG)及级联框架(CFG-FMPG)适用于图像修复。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的引导方法(如CFG、AG)在噪声到数据生成中表现优异，但桥接模型的数据到数据生成过程可利用干净先验，如何有效利用这一特性提升生成效果是核心动机。

Method: 1) 通过引入未见过的弱先验与已见先验对比，利用缩放因子增强先验利用；2) 分析桥接过程的先验利用机制，设计频率调制引导(FMPG)按低频和高频调整引导尺度；3) 提出级联框架CFG-FMPG，先用CFG生成含噪隐表示，再作为先验由FMPG处理。

Result: 在多个图像翻译任务上，提出的PG方法及变体(FMPG、CFG-FMPG)一致提升了预训练桥接模型的性能。

Conclusion: 先验引导(PG)是一种有效的训练后引导方法，能增强桥接模型在数据到数据生成中的先验利用，CFG-FMPG框架在图像修复中兼顾效果与效率。
...
### [62] [KC-3DGS: Kurtosis-Constrained Gaussian Splatting for High-Fidelity View Synthesis](https://arxiv.org/abs/2606.03120)
*Vivekjyoti Banerjee,Abhay Yadav,Rama Chellappa,Aniket Roy*

Main category: cs.CV

TL;DR: 提出KC-3DGS，通过小波域约束增强3D高斯泼溅，提升稀疏视角下的感知质量。


<details>
  <summary>Details</summary>
Motivation: 标准像素空间损失仅约束整体重建误差，允许优化在频率尺度上重新分配误差，导致过平滑和结构伪影，尤其在稀疏视角下。

Method: 结合三种小波域损失：多尺度小波系数对齐损失、有监督的峰度集中损失、跨频带协方差惩罚。

Result: 在多个数据集上感知质量一致提升，WRIVA-ULTRRA上DreamSim提升9.48%，稀疏视角下PSNR最高提升0.5 dB。

Conclusion: 该方法可作为即插即用的正则化策略无缝集成到现有3DGS流程中。
...
### [63] [SRENet: Spectral Re-Entry Network for Point Cloud Action Recognition](https://arxiv.org/abs/2606.03160)
*Qiuxia Wu,Jiarui Lan,Wenxiong Kang,Zhiyong Wang,Kun Hu*

Main category: cs.CV

TL;DR: 提出SRENet，从频率视角学习点云序列中的全局上下文和细粒度时间动态，用于动作识别。


<details>
  <summary>Details</summary>
Motivation: 点云的不规则结构和时间不一致性使得时空表征学习困难，现有方法难以同时捕捉全局运动上下文和细粒度时间动态。

Method: 引入光谱分解块（SDeBlock）进行小波分析沿时间和空间轴解耦特征，光谱重入块（SReBlock）恢复残差动态，并用对比学习和课程策略增强频率子空间判别性。

Result: 在MSR-Action3D、NTU-RGBD和NTU-RGBD120上达到最先进性能。

Conclusion: 频率建模有效提升点云动作理解性能。
...
### [64] [NVIDIA OmniDreams: Real-Time Generative World Model for Closed-Loop Autonomous Vehicle Simulation](https://arxiv.org/abs/2606.03159)
...
TL;DR: 提出OmniDreams，一种基于Cosmos扩散模型的实时生成式世界模型，用于闭环仿真中自动回归生成动作条件视频，支持自动驾驶策略的安全评估。


<details>
  <summary>Details</summary>
Motivation: 传统重建型神经仿真器受限于初始数据，难以泛化到动态或新颖场景（如极端天气、不可预测行为），阻碍了长尾场景下的策略安全评估。

Method: 利用Cosmos扩散模型的视觉先验，通过2.1万小时驾驶场景的中期和后期训练，实现动作条件视频的自动回归生成，并集成到AlpaSim闭环仿真系统中。

Result: 世界动作模型（WAM）在NuRec数据集上超越基于VLA的Alpamayo 1.5策略模型，参数量仅为后者的1/5。

Conclusion: 实时世界模型OmniDreams不仅能作为高保真仿真环境，还能作为策略架构的骨干网络，为自动驾驶策略训练提供可扩展方案。
...
### [65] [JAVEDIT: Joint Audio-Visual Instruction-Guided Video Editing with Agentic Data Curation](https://arxiv.org/abs/2606.03168)
...
TL;DR: 提出了首个大规模高精度的联合音视频编辑数据集JAVEdit-100k与基准JAVEditBench，并构建了基线模型JAVEdit，实验表明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有指令视频编辑缺乏联合音视频编辑的数据集和基准，限制了该领域发展。

Method: 通过四个精心设计的生成流程与智能体在环质量控制机制构建数据集，并基于此提出基线模型JAVEdit。

Result: JAVEdit在六个评估指标中的五个上优于所有基线方法。

Conclusion: 该数据集、基准和模型有望推动联合音视频编辑研究的发展。
...
### [66] [Ask When It Pays: Cost-Aware Open-Ended Interaction for Instance Goal Navigation](https://arxiv.org/abs/2606.03175)
*Xunyi Zhao,Sihao Lin,Gengze Zhou,Zerui Li,Shijie Li,Wei Tao,Jiajun Liu,Qi Wu*

Main category: cs.CV

TL;DR: 本文提出了一种成本敏感的交互式实例目标导航方法，通过信息增益分析确定有效问题类型，并构建了评估交互效率的基准和指标。


<details>
  <summary>Details</summary>
Motivation: 现有方法允许反复提问以提高成功率，但未考虑问题成本，无法高效解决导航歧义。

Method: 将交互式IGN转化为成本敏感的不确定性减少问题，应用信息增益分析提取问题类型及权重，并提出零样本MLLM导航器，仅在预期收益大于成本时查询。

Result: 构建了诊断交互行为的基准和加权成功率指标，证明了方法在降低交互成本的同时保持导航成功率。

Conclusion: 成本敏感的交互策略能更高效地利用查询，在保持性能的同时减少不必要的交互。
...
### [67] [GLINT: Sparsely Gated Vision-Language Alignment for Fine-Grained Radiology Representations](https://arxiv.org/abs/2606.03180)
*Jonggwon Park,Seongeun Lee,Junhyun Park,Hannah Yun,Hyunwoong Kim,Sohyun Jeong,Hyewon Kang,Byungmu Yoon,Kyoyun Choi*

Main category: cs.CV

TL;DR: 提出GLINT框架，通过稀疏门控对齐和密集特征正则化，实现放射学VLM的零样本分类、定位和分割。


<details>
  <summary>Details</summary>
Motivation: 图像-报告全局配对与局部病灶信息不匹配，现有模型对所有图像块均匀加权，缺乏稀疏查询相关定位。

Method: 稀疏门控对齐（Sigmoid门控激活相关图像块）和密集特征正则化（冻结SSL教师网络保留细粒度特征）。

Result: 在2D CXR和3D CT上实现零样本分类、定位及首个无掩码监督的3D CT零样本分割，下游任务优于SSL编码器和医学VLM。

Conclusion: 明确建模稀疏对应关系能显著提升放射学VLM的定位与分割能力。
...
### [68] [Reinforcement Learning from Cross-domain Videos with Video Prediction Model](https://arxiv.org/abs/2606.03201)
*Zhao Yang,Xinrui Zu,Jacob E. Kooi,Thomas Delliaux,He Liu,Shujian Yu,Kevin Sebastian Luck,Vincent François-Lavet*

Main category: cs.CV

TL;DR: 提出XIPER模型，利用跨域视频预测奖励，从视觉差异大的专家视频中学习，在DMC和sim-to-real任务上优于基线。


<details>
  <summary>Details</summary>
Motivation: 专家视频与智能体外观存在域差距（颜色、形态、sim-to-real），导致无奖励信号的强化学习困难。

Method: 训练跨域视频预测模型，将智能体观测映射到专家域，用预测似然作为奖励信号。

Result: 在DMC Color Suite（8任务）和Body Suite（3任务）中持续优于基线，在sim-to-real迁移中为真实机器人提供有效奖励。

Conclusion: XIPER能有效应对视觉域差距，实现从不同域专家视频学习。
...
### [69] [Follow-Your-Preference++: Rethinking Preference Alignment for Image Inpainting](https://arxiv.org/abs/2606.03216)
*Junkun Yuan,Yutao Shen,Toru Aonishi,Hideki Nakayama,Yue Ma*

Main category: cs.CV

TL;DR: 本文从第一性原理重新审视图像修复中的偏好对齐问题，利用直接偏好优化框架和公开奖励模型进行实证研究，发现奖励模型存在亮度、构图和色彩方案的偏见，而简单的模型集成可缓解这些偏见并提升性能。


<details>
  <summary>Details</summary>
Motivation: 重新评估图像修复中偏好对齐的核心挑战，而非提出新方法，以解决现有奖励模型的偏见和奖励黑客问题。

Method: 采用直接偏好优化框架，使用多个公开奖励模型构建偏好训练数据，并进行候选和样本缩放实验，最后通过奖励模型集成校准偏见。

Result: 大多数奖励模型可提供有效信号，但存在偏见；模型集成可缓解偏见并提升鲁棒性；偏好对齐可迁移至物体移除任务。

Conclusion: 在不修改模型架构或引入新数据集的情况下，通过偏好对齐和奖励模型集成，显著优于现有最先进模型。
...
### [70] [MemoGen: Can Past Experience Improve Future Text-to-Image Generation?](https://arxiv.org/abs/2606.03243)
...
TL;DR: 提出了MemoGen框架，通过代理进化层和经验记忆机制，在不更新生成模型参数的情况下，实现文本到图像生成的自我进化。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在隐含视觉约束、关系推理或外部知识方面不可靠，而现有检索增强和代理生成方法将每次生成视为独立过程，未能系统保留经验。

Method: 训练无关框架MemoGen，包含代理进化层：显式推理视觉需求、检索外部证据、翻译为可执行约束、评估生成结果，并存储任务理解、参考选择、视觉反馈、成功策略和失败教训作为可重用经验记忆。

Result: 在两个进化轮次后，基于Qwen-Image骨干的MemoGen在WISE和Mind-Bench基准上超越Nano Banana Pro和GPT-Image-1等强专有系统。

Conclusion: 显式经验记忆可作为可靠文本到图像生成的有力持续学习信号，实现测试时自我进化。
...
### [71] [MariData: One-Step Unpaired Image Translation for Maritime Environments](https://arxiv.org/abs/2606.03246)
*Santeri Henriksson,Mehdi Asadi,Amin Majd,Juha Kalliovaara*

Main category: cs.CV

TL;DR: 提出了基于CycleGAN-turbo的框架，通过零卷积跳跃连接保留小物体细节，实现海上图像在雾天、日落和夜间等条件下的风格转换，缓解自动驾驶船舶训练数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 极端天气和低光照条件下海上图像难以获取，现有生成模型因潜在压缩瓶颈丢失小目标细节，需要结构保持的合成数据方法。

Method: 使用CycleGAN-turbo单步非配对翻译架构，引入零卷积跳跃连接绕过VAE瓶颈，保留小物体结构；收集7000张海上图像实现昼-雾、昼-日落、昼-夜三域转换。

Result: 昼-雾和昼-日落模型保持良好结构，昼-夜模型出现语义幻觉（如生成人工海岸灯光）；定性评估和变强度推理验证了真实性。

Conclusion: 建立了高效、结构感知的数据合成流程，直接缓解自主海上导航数据稀缺问题，但夜间转换仍需改进平衡训练策略。
...
### [72] [FreeStreamGS: Online Feed-forward 3D Gaussian Splatting from Unposed Streaming Inputs](https://arxiv.org/abs/2606.03254)
*Ruiyang Chen,Feiran Li,Chu Zhou,Zonglin Li,Zhanyu Ma,Heng Guo*

Main category: cs.CV

TL;DR: FreeStreamGS 提出了用于在线新视图合成的鲁棒前馈框架，解决了流式无位姿图像输入下的渲染质量问题。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3DGS方法依赖离线序列，无法处理在线流式图像；在线几何估计方法用于NVS时存在严重渲染伪影，因多视图一致性要求高。

Method: 引入解耦内参恢复头消除相机内参累积偏差，以及动态点偏移策略纠正姿态-深度耦合漂移。

Result: 无需未来帧即可达到与离线前馈3DGS方法可比的渲染质量。

Conclusion: FreeStreamGS 能实现高效在线NVS，鲁棒处理流式无位姿图像输入。
...
### [73] [PaddleOCR-VL-1.6: Expanding the Frontier of Document Parsing with Under-Optimized Region Refinement and Progressive Post-Training](https://arxiv.org/abs/2606.03264)
...
TL;DR: 本文提出了PaddleOCR-VL-1.6，一个升级的紧凑文档解析模型，通过区域感知数据优化框架和渐进式后训练策略，在OmniDocBench v1.6上达到96.33%的新SOTA成绩。


<details>
  <summary>Details</summary>
Motivation: PaddleOCR-VL-1.5虽建立强基线，但错误集中于模型行为不稳定、数据覆盖稀疏或监督不可靠的区域，促使研究针对性优化而非盲目扩大训练数据。

Method: 引入区域感知数据优化框架，识别前模型的弱区域并进行定向增强和改进监督信号；采用基于数据筛选和强化学习的渐进式后训练配方进行分阶段优化。

Result: 在OmniDocBench v1.6上达到96.33%的新SOTA，与顶级VLM竞争时展现出强竞争力。

Conclusion: PaddleOCR-VL-1.6为PaddleOCR-VL系列提供了实用的后训练配方，有效提升文档解析性能。
...
### [74] [VistaHop: Benchmarking Multi-hop Visual Reasoning for Visual DeepSearch](https://arxiv.org/abs/2606.03273)
*Hang He,Chuhuai Yue,Chengqi Dong,Chengcheng Wan,Ting Su,Haiying Sun,Jiajun Chai,Xiaohan Wang,Guojun Yin*

Main category: cs.CV

TL;DR: 提出了VistaHop基准测试，用于评估多模态大模型在视觉深度搜索中的多跳推理能力，发现当前最佳模型仅达24.31%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估单步视觉理解或静态问答，缺乏对迭代图像检查、视觉锚点定位和多跳证据整合的评估。

Method: 构建包含300张图像、25个搜索场景和350个多跳问答任务的VistaHop基准，以及支持工具增强推理的VistaArena评估环境。

Result: 7个代表性模型表现不佳，最佳模型SenseNova-MARS-32B仅达24.31% Pass@1。

Conclusion: 当前模型在视觉定位、证据回溯、长链推理和多锚点信息融合方面存在显著局限，需要更强的基准和训练方法。
...
### [75] [BA-T: An Iterative Transformer for Two-View Bundle Adjustment](https://arxiv.org/abs/2606.03287)
*Ganlin Zhang,Weirong Chen,Daniel Cremers,Xi Wang*

Main category: cs.CV

TL;DR: 提出BA-T，一种受经典集束调整启发的迭代Transformer，通过轻量级隐式残差更新实现三维重建中的几何优化，以少量参数达到与大型模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈三维重建模型依赖深度解码器且缺乏结构化的几何优化机制，导致多视图一致性差。

Method: 受集束调整启发，设计可重复的Transformer层，在隐式token空间中迭代传播姿态与局部几何信息，基于潜在残差精化预测。

Result: BA-T在迭代中逐步提升姿态和重建精度，跨视图一致性优于传统解码器，仅用16%解码器参数即可匹配或超越大型模型。

Conclusion: BA-T为深度注意力机制提供了紧凑、高效的结构化替代方案，在轻量级架构中实现精确的三维重建。
...
### [76] [TASE: Truncation-Aware Semantic Embeddings for 3D Scene Understanding and Editing](https://arxiv.org/abs/2606.03314)
*Tim-Felix Faasch,Jochen Kall,Lucas Nunes,Jens Behley,Cyrill Stachniss*

Main category: cs.CV

TL;DR: 本文提出TASE方法，通过将预训练的2D语义特征投影到截断感知嵌入空间，实现灵活可控的3D场景编辑，在几何大改动的编辑任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景表示方法对可控编辑支持有限，需要一种既能保留细节又能支持抽象语义编辑的灵活方法。

Method: 优化截断感知嵌入空间，逐步减少特征通道以抽象语义，保留更多通道以保持细节；利用尺度-平移等变损失增强多视角一致性；通过微调编辑扩散模型减少几何变化伪影。

Result: 在3D场景编辑中表现竞争性，尤其在涉及大几何修改的编辑上大幅超越先前方法。

Conclusion: 所提方法实现了对原始场景内容遵循度的显式控制，支持更大幅度的修改，且编辑结果具有高保真度和多视角一致性。
...
### [77] [Cross-Modality Feature Fusion Based on Structured State Space Duality for Multimodal Image Registration Network](https://arxiv.org/abs/2606.03341)
*Zhikang Li,Yan Wu,Xin Hu,Yi Dai,Ming Li*

Main category: cs.CV

TL;DR: 提出了基于结构化状态空间对偶（SSD）的多模态图像配准算法RegNetMamba-2，通过粗到精的匹配过程有效提取局部和全局结构特征，在VIS-SAR、VIS-IR和VIS-NIR数据集上取得了优于现有方法的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 多模态图像配准中共享结构信息提取是主要挑战，而SSD相比Transformer能更高效地提取全局结构特征，促使作者探索将其应用于该领域。

Method: 在三个尺度上应用SSD提取多模态特征；提出基于SSD的跨模态特征融合模型，包含跨模态交互（CMI）模块和多尺度特征融合（MSF）模块，分别用于跨模态特征提取和渐进式向上融合；最后利用1/8和1/2尺度特征计算匹配概率进行像素级对应。

Result: 在VIS-SAR、VIS-IR和VIS-NIR数据集上，与最先进深度学习方法相比，RegNetMamba-2在性能和效率上均取得了良好效果。

Conclusion: RegNetMamba-2有效解决了多模态图像配准中结构特征提取和融合的挑战，实现了高效且准确的配准。
...
### [78] [Beyond Semantics: Modeling Factual and Affective Perceptual Experiences from Vision-Language Data](https://arxiv.org/abs/2606.03345)
*Youssef Mohamed,Kenneth Ward Church,Mohamed Elhoseiny*

Main category: cs.CV

TL;DR: 提出了P-Topics建模问题，并设计了PercepT双阶段架构来发现和映射图像的情感与文化感知体验。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时建模图像中的客观事实与主观情感，且跨文化感知差异未得到充分探索。

Method: 构建PercepT架构：第一阶段通过无监督聚类形成视觉-文本语义簇；第二阶段利用注意力池化学习图像到簇的映射函数。

Result: 在ArtELingo数据集上，轮廓系数达0.97（基线0.37），AUC达0.94（基线0.77）。

Conclusion: PercepT能有效捕获有意义的感知体验并跨文化推广，性能显著优于现有方法。
...
### [79] [SynCred-Bench: Benchmarking Synthetic Credibility in AI-Generated Visual Misinformation](https://arxiv.org/abs/2606.03348)
*Junxiao Yang,Minghao Zhang,Xiaoce Wang,Haoran Liu,Shiyao Cui,Hongning Wang,Minlie Huang*

Main category: cs.CV

TL;DR: 论文提出SYNCRED-Bench基准，评估现有系统检测AI生成的可信图像错误信息的能力，结果发现所有系统表现不佳。


<details>
  <summary>Details</summary>
Motivation: 生成模型能制造具有真实嵌入文本和布局的视觉伪影，导致新的虚假信息威胁“合成可信度”，现有检测方法不足。

Method: 构建包含600张AI生成错误信息图像的基准（SYNCRED-Bench），涵盖6种可信类别和7种传播风格，以及FP450真实图像负集；评估15种多模态模型、开源检测器和商业API。

Result: 在5%假阳性率限制下，多模态模型真阳性率仅10.5%，开源检测器<5%，商业API达57.6%；人类标注者真阳性率仅63%。

Conclusion: 合成可信度是严重且未充分探索的视觉错误信息挑战，需开发超越表面可信线索的检测器。
...
### [80] [P\textsuperscript{2}-DPO: Grounding Hallucination in Perceptual Processing via Calibration Direct Preference Optimization](https://arxiv.org/abs/2606.03376)
*Ruipeng Zhang,Zhihao Li,Haozhang Yuan,C. L. Philip Chen,Tong Zhang*

Main category: cs.CV

TL;DR: 提出P²-DPO方法，通过模型自生成偏好对和校准损失，优化LVLM中的感知瓶颈与视觉鲁棒性，超越依赖人工反馈的基线。


<details>
  <summary>Details</summary>
Motivation: 现有直接偏好优化（DPO）未针对性解决感知瓶颈和视觉鲁棒性不足，且偏好对存在视觉无关和离策略问题。

Method: 构建在线策略偏好对（聚焦感知增强与视觉鲁棒性）并设计校准损失对齐视觉信号与文本生成。

Result: 在相似训练数据量下优于依赖人工反馈的强基线，在注意力区域保真度和图像退化场景中有效性显著。

Conclusion: P²-DPO通过自生成偏好对和校准损失有效缓解了LVLM的幻觉、感知瓶颈及视觉鲁棒性问题。
...
### [81] [Towards Characterizing Scientific Image Utility and Upgradability](https://arxiv.org/abs/2606.03401)
*WenZhe Li,Qihang Yan,Liang Chen,Junying Wang,Farong Wen,Yijin Guo,Chunyi Li,Zicheng Zhang,Guangtao Zhai*

Main category: cs.CV

TL;DR: 提出SIU²A框架评估科学图像的可信度与可修复性，通过两阶段协议衡量AI系统在错误检测与校正中的表现。


<details>
  <summary>Details</summary>
Motivation: AI生成内容引入细微博弈误差，传统质量指标与语言模型无法有效验证科学图像真实性。

Method: SIU²A框架包含Utility（错误检测与可修复性）和Upgradability（校正质量），基于四类腐败（细节失真/不完整/虚假内容/实体混淆）构建基准数据集，采用两阶段评估协议。

Result: 当前多模态系统在科学错误评估和忠实校正上表现显著不足，揭示视觉感知与科学可用性之间的根本差距。

Conclusion: 现有AI系统在科学图像评估方面存在关键缺陷，需发展更精准的验证机制。
...
### [82] [Mamba-Enhanced Implicit Motion Learning for Audio-Driven Portrait Animation](https://arxiv.org/abs/2606.03402)
*Xuan Wei,Jiahui Chen,Kaiheng Li,Mingyu Shao,Qingqi Hong*

Main category: cs.CV

TL;DR: 提出一种基于隐式运动的两阶段框架，从单张静态图像和音频生成真实、时间连贯的人体运动视频，在多个基准上达到最优。


<details>
  <summary>Details</summary>
Motivation: 传统关键点方法难以捕捉细微运动动态，驱动从单图生成连贯视频的需求。

Method: 两阶段流水线：第一阶段用区域感知注意力融合外观先验与层次深度线索建模隐式运动特征；第二阶段用Mamba增强扩散模型从音频和源图像直接预测特征。

Result: 在380小时数据集上训练，在准确度、自然度和时间连贯性上优于现有方法。

Conclusion: 提出的解耦架构提升了灵活性和效率，实现了音频驱动人体运动视频生成的最新水平。
...
### [83] [SAMatcher: Co-Visibility Modeling with Segment Anything for Robust Feature Matching](https://arxiv.org/abs/2606.03406)
*Xu Pan,Qiyuan Ma,Mingyue Dong,He Chen,Wei Ji,Xianwei Zheng*

Main category: cs.CV

TL;DR: 提出SAMatcher框架，通过共视性建模和跨视图交互机制（基于SAM），在匹配中先预测共视区域掩码和边界框作为先验，显著提升大视角和尺度变化下的匹配性能。


<details>
  <summary>Details</summary>
Motivation: 现有学习匹配方法多在像素/块级操作，缺乏对跨视图共视区域的显式建模，导致大视角和尺度变化下性能受限。

Method: 基于SAM构建对称跨视图交互机制，实现双向特征交换与语义对齐；提出统一监督方案，联合优化掩码、框回归及一致性约束。

Result: 在多个挑战性基准上相比现有匹配流程取得显著改进，尤其在大视角和尺度变化场景下表现优异。

Conclusion: 证明原本用于单目分割的基础模型可通过显式共视性建模扩展到多视图对应推理，为结构化匹配表示学习提供新视角。
...
### [84] [Enginuity: A Dataset and Benchmark for Vision-Language Understanding of Engineering Diagrams](https://arxiv.org/abs/2606.03410)
*Abhishek Kumar,Isha Motiyani,Tilak Kasturi,Ethan Seefried,Prahitha Movva,Tirthankar Ghosal*

Main category: cs.CV

TL;DR: 提出了Enginuity，首个针对工程图纸的VLM开放基准，包含结构化表格提取和自由形式VQA任务，评估发现模型在描述保真度和事实推理上存在显著差距，并建议使用LLM作为评估器。


<details>
  <summary>Details</summary>
Motivation: 工程图纸对VLM构成独特挑战，但缺乏公开基准来评估其能力。

Method: 基于美国军事维修手册构建数据集，定义两个任务，评估四个前沿VLM在零样本和链式思维提示下的表现。

Result: 任务1中Recall@all为0.61-0.87但Token F1pen仅0.03-0.18；任务2显示所有模型存在事实推理差距；语义相似度指标比词重叠指标好2-6倍。

Conclusion: 现有VLM在工程图纸上性能不足，需要更好的评估方法和领域特定校准。
...
### [85] [A unified multi-task framework enables interpretable chest radiograph analysis](https://arxiv.org/abs/2606.03417)
*Lijian Xu,Ziyu Ni,Xinglong Liu,Xiaosong Wang,Hongsheng Li,Shaoting Zhang*

Main category: cs.CV

TL;DR: 提出了一种可解释的多任务Transformer框架（IMT-CXR），通过三段式诊断流程（疾病识别、属性表征、证据整合报告生成）模拟放射科医生工作流，在胸部X光分析中兼顾性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有黑箱多模态模型忽视临床诊断的信任敏感性和多任务特性，缺乏可解释的决策路径。

Method: 基于统一Transformer架构，通过医学领域指令调优，依次执行多标签疾病分类、病灶定位、解剖分割和报告生成，形成可追溯的诊断证据链。

Result: 在10个CXR基准上表现优异；盲评中66%的AI生成报告诊断清晰度不亚于或优于原始临床报告。

Conclusion: 该框架通过建立从解剖发现到结论的可追溯诊断路径，弥合AI技术指标与临床实用性之间的差距，推进可信医疗AI系统发展。
...
### [86] [IDO: Incongruity-aware Distribution Optimization for Multimodal Fake News Detection](https://arxiv.org/abs/2606.03418)
*Hengyang Zhou,Rongman Hong,Yuxuan Zhou,Jing Wang,Zhaoyan Pan*

Main category: cs.CV

TL;DR: 提出一种基于不协调感知分布优化的多模态假新闻检测方法，通过事实不协调和模态不协调建模提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注跨模态一致性，未能显式建模虚假内容中的语义不协调特征。

Method: 采用通道重加权策略获取语义判别嵌入，用高斯分布建模事实不协调不确定性，并利用不协调对比学习学习跨模态语义。

Result: 在实验数据集上达到最先进性能。

Conclusion: 明确建模语义不协调能有效提升多模态假新闻检测效果。
...
### [87] [PHAF-Personalized Hand Avatars in a Flash](https://arxiv.org/abs/2606.03420)
*Meghana Shankar,Akanxit Upadhyay,Anmol Namdev,Green Rosh KS,Pawan Prasad BH*

Main category: cs.CV

TL;DR: 提出PHAF方法，从两张手部图像快速生成个性化逼真手部头像，支持实时部署和高质量多视角渲染。


<details>
  <summary>Details</summary>
Motivation: 现有基于优化的方法生成手部头像速度慢，难以满足AR/VR等实际应用的实时性需求。

Method: 结合语义引导网格对齐与密集纹理提取高效传递高频细节，采用基于视图的修复网络优化纹理连续性，并利用参数化手部模型实现准确关节驱动。

Result: 视觉保真度与现有方法相当，但纹理生成时间缩短30倍，支持实时部署。

Conclusion: PHAF在保持高质量的同时大幅提升生成速度，为边缘设备上的AR/VR应用提供了实用方案。
...
### [88] [PRISM: Synergizing Vision Foundation Models via Self-organized Expert Specialization](https://arxiv.org/abs/2606.03444)
*Ying Tang,Dong Li,Youjia Zhang,Zikai Song,Junqing Yu,Wei Yang*

Main category: cs.CV

TL;DR: 提出PRISM框架，通过双流专家混合模型整合多种视觉基础模型，解决特征冲突问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多种视觉基础模型进行单流蒸馏时存在负迁移问题，需要更有效的知识整合方案。

Method: 采用两阶段范式：先通过教师条件路由器引导专家分支学习不同表示子空间，再动态组合专家模块适配下游任务。

Result: 在PASCAL-Context和NYUD-v2数据集上取得最佳性能，验证了稀疏专家特化策略的可扩展性。

Conclusion: PRISM通过模块化特化和动态组合有效解决了多模型知识整合中的干扰问题。
...
### [89] [From 3D Perception to Safety Reasoning: A Graph-Based Framework for Real-Time Underground Mine Monitoring](https://arxiv.org/abs/2606.03460)
*Pasindu Ranasinghe,Simit Raval,Dibyayan Patra,Bikram Banerjee,Ismet Canbulat*

Main category: cs.CV

TL;DR: 提出了一种结合3D语义感知、异常检测、规则检查和LLM推理的地下煤矿连续监测框架，通过场景图和时序图实现结构化安全推理，在115个危险场景中记忆推理覆盖率达93%。


<details>
  <summary>Details</summary>
Motivation: 传统监控系统缺乏3D场景理解和上下文记忆，难以识别复杂或演变的井下安全隐患。

Method: 融合3D点云语义分割、不确定性异常检测、规则检查、设备端LLM推理和GraphRAG记忆分析，构建场景/时序图作为知识结构，结合真实扫描、物体放置和仿真生成多样化数据。

Result: 感知模型准确率92.7%且30 FPS低内存；规则检查覆盖57%，LLM推理提升至76%，记忆推理达93%。异常信号能检测未定义危险。

Conclusion: 图结构知识与3D感知和分层推理的结合，为矿井智能监控提供了实用基础。
...
### [90] [Learned Non-Maximum Suppression for 3D Object Detection](https://arxiv.org/abs/2606.03568)
*Timo Osterburg,Stefan Schütte,Torsten Bertram*

Main category: cs.CV

TL;DR: 提出两种基于学习的检测过滤模块（D2D-Rescore和GossipNet3D），替代启发式非极大值抑制（NMS），提升LiDAR 3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统NMS方法在处理密集、重叠的3D检测框时效果有限，需要更可靠的过滤机制。

Method: 使用Transformer的检测间注意力（D2D-Rescore）和基于鸟瞰图局部消息传递的GossipNet3D；采用与nuScenes评估协议一致的度量感知匹配策略。

Result: 相比CircleNMS，在mAP、NDS和真阳性质量上均有提升，尤其对小型和稀有类别，且计算开销极小。

Conclusion: 学习式检测级过滤可在不修改基础网络的情况下增强3D检测器可靠性，是启发式NMS的有效替代方案。
...
### [91] [Mixed-Modality Dual Face-Hair Retrieval](https://arxiv.org/abs/2606.03470)
*Quoc-Anh Bui-Huynh,Mai-Tuyen Lam,Dai-Anh-Tuan Nguyen,Thanh Duc Ngo*

Main category: cs.CV

TL;DR: 提出了双模态人脸发型检索任务（DFHR），并构建了首个基准数据集DFHR-Bench和统一框架MFHC。


<details>
  <summary>Details</summary>
Motivation: 现有检索任务无法同时处理跨模态的人脸身份和发型属性组合查询。

Method: 通过多阶段标注构建DFHR-Bench数据集，提出MFHC框架解耦身份和发型嵌入，使用令牌注入和多视角监督进行融合。

Result: 在DFHR-Bench上验证了方法的有效性，实现了跨模态身份和发型的准确检索。

Conclusion: DFHR和DFHR-Bench为跨模态身份可控视觉检索建立了新范式。
...
### [92] [UnsOcc: 3D Semantic Occupancy Prediction in Unstructured Scene via Rendering Fusion](https://arxiv.org/abs/2606.03581)
*Ye Wu,Ruiqi Song,Baiyong Ding,Nanxin Zeng,Junjie Cheng,Yunfeng Ai*

Main category: cs.CV

TL;DR: 提出UnsOcc，一种针对非结构化场景的多模态3D语义占用预测框架，通过渲染融合和高斯细化提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 非结构化场景中不规则障碍物和稀疏布局使传统感知方法失效，且现有3D语义占用预测面临跨模态融合困难与长尾分布问题。

Method: 提出RenderFusion模块通过双向渲染监督增强跨模态特征对齐；提出GSRefinement基于高斯泼溅的细节感知辅助监督方法。

Result: 在露天矿数据集和nuScenes数据集上均显著优于现有最先进方法。

Conclusion: UnsOcc有效提升了非结构化场景下的3D语义占用预测性能，验证了渲染融合与高斯细化策略的有效性。
...
### [93] [DyaPlex: Full-Duplex Speech-Motion Model for Dyadic Interaction](https://arxiv.org/abs/2606.03874)
...
TL;DR: 提出了一个面向双人交互的全双工语音-动作模型DyaPlex，实现了流式同步的多模态感知与生成。


<details>
  <summary>Details</summary>
Motivation: 现有模型多聚焦单模态或准双工交互，难以捕捉人类交流的连续性与互惠性。

Method: 基于全双工语音模型先验，设计双塔Transformer架构：冻结语音模型以保持零样本对话推理，新增运动通路并通过双人令牌交错机制和时间对齐的语音-动作RoPE实现深度耦合。

Result: 在4000小时交互数据集上训练，在单人和双人交互基准上均取得最优性能。

Conclusion: 证明了在全双工语音模型中集成运动通路能有效实现流式同步的多模态交互。
...
### [94] [PersistGS: Differentiable Physics for Object Permanence in 4D Gaussian Splatting](https://arxiv.org/abs/2606.03479)
*Adrian Ramlal,John S. Zelek*

Main category: cs.CV

TL;DR: 提出PersistGS方法，通过可微刚体模拟与3D高斯泼溅结合，在物体被完全遮挡时维持其物理一致性，轨迹误差降低40%。


<details>
  <summary>Details</summary>
Motivation: 现有动态3DGS方法在物体被完全遮挡时因缺乏光度监督而导致高斯退化，而神经重建方法依赖生成先验，优先视觉合理性而非物理正确性。

Method: 将场景分解为逐物体高斯和碰撞网格，通过可微模拟从遮挡前轨迹估计摩擦力和速度，使用SE(3)轨迹定位遮挡期间的高斯，并引入质心轮廓损失分离位置梯度与外观噪声。

Result: 在合成场景中，PSNR比恒定速度外推法提高2.46dB，仅比真实轨迹上限低0.19dB；轨迹误差降低40%。

Conclusion: 该方法在物体遮挡期间能再现接触事件（如弹跳、摩擦减速），优于运动学外推和光度监督，物理约束有效提升重建精度。
...
### [95] [TrAction: Action Recognition with Sparse Trajectories](https://arxiv.org/abs/2606.03490)
*Jan F. Meier,Felix B. Mueller,Alexander Ecker,Timo Lüddecke*

Main category: cs.CV

TL;DR: 提出一种基于稀疏点轨迹的2.5D Transformer架构，结合掩码轨迹预训练，高效识别动作且避免外观偏置。


<details>
  <summary>Details</summary>
Motivation: 现有动作识别模型依赖密集RGB视频，存在计算量大、易受外观和背景偏置（如通过物体或场景而非运动识别动作）的问题。

Method: 设计基于稀疏点轨迹的Transformer架构，并采用掩码轨迹预训练策略。

Result: 在Something-Something V2上达45% top-1，EPIC-Kitchens-100上达54% top-1；时间反转敏感性优于V-JEPA；与DINOv2/V-JEPA 2融合后精度分别提升8.7和1.6个点。

Conclusion: 轨迹特征与外观特征互补，且能高效实现动作识别，具有低计算开销和无偏置优势。
...
### [96] [VLESA: Vision-Language Embodied Safety Agent for Human Activity Monitoring](https://arxiv.org/abs/2606.03954)
*Hanjiang Hu,Yiyuan Pan,Jiaxing Li,Xusheng Luo,Alexander Robey,Na Li,Yebin Wang,Changliu Liu*

Main category: cs.CV

TL;DR: 提出VLESA框架，通过目标条件安全Q滤波器和意图-动作预测代理，从第一人称视频中实时监控并干预危险动作，显著提升干预准确性和动作安全性。


<details>
  <summary>Details</summary>
Motivation: AI系统在物理任务中辅助人类时，物理动作具有即时而不可逆的后果，安全性至关重要；且相同动作因意图不同可能安全或危险（意图依赖安全性）。

Method: 构建目标条件安全标注的数据集；使用GRPO训练目标条件安全Q滤波器；提出意图-动作预测代理联合推断目标并预测未来动作。

Result: 在ASIMOV-2.0基准上，VLESA在精确时间帧的干预准确性高于基线；GRPO训练的Q滤波器通过目标条件约束解码将动作安全性提升超过41个百分点。

Conclusion: VLESA框架有效提升了物理任务中AI辅助的安全性，解决了意图依赖的安全问题，代码已开源。
...
### [97] [Exploring Easy Boosts for Lidar Semantic Scene Completion](https://arxiv.org/abs/2606.03992)
*Tetiana Martyniuk,Jonathan Seele,Alexandre Boulch,Gilles Puy,Renaud Marlet,Raoul de Charette*

Main category: cs.CV

TL;DR: 该论文研究了无需复杂架构设计的“免费午餐”策略，通过为激光雷达点云添加语义伪标签和可见性信息，显著提升了语义场景补全性能，使旧模型与最新系统竞争甚至超越。


<details>
  <summary>Details</summary>
Motivation: 提升激光雷达语义场景补全性能，避免复杂架构重设计。

Method: 使用现成分割器为点云添加语义伪标签，并引入可见性信息区分空和未知空间。

Result: 高质量语义先验是mIoU提升的主要驱动因素，可见性信息提供次要提升，旧模型因此能与最新系统竞争或超越。

Conclusion: 简单的输入增强策略可显著提高现有SSC架构的性能。
...
### [98] [Low-Frequency Shortcuts in Texture-Driven Visual Learning](https://arxiv.org/abs/2606.03493)
*Utku Şirin,Cathy Hou,David Alvarez-Melis,Stratos Idreos*

Main category: cs.CV

TL;DR: 本文分析了纹理驱动领域中的捷径学习现象，发现低频成分会导致模型依赖低频捷径，影响泛化能力。通过去除低频成分，可提升分布内准确率并增强对低频扰动的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有捷径学习研究多基于形状驱动的标准基准，而许多实际应用是纹理驱动的，但缺乏相关分析。

Method: 使用频谱分析方法，识别低频成分捷径，并通过在训练和测试集移除低频成分来消除捷径，比较模型在不同领域的频谱行为。

Result: 去除低频成分后，分布内准确率提升8%，对低频扰动的鲁棒性提升40%，但高频扰动存在权衡。

Conclusion: 纹理驱动领域存在低频捷径，移除低频成分可改善泛化与鲁棒性，但高频依赖增加可能带来新的脆弱性。
...
### [99] [Characterizing Detectability in 3DGS Poisoning: A Stage-wise Benchmark](https://arxiv.org/abs/2606.03499)
*Quoc-Anh Bui-Huynh,Thanh Duc Ngo,Xue Geng,Kaixin Xu,Wang Zhe,Xulei Yang,Ngai-Man Cheung*

Main category: cs.CV

TL;DR: 提出Poison-3DGS基准,系统研究3DGS重建流水线各阶段对投毒攻击的可检测性,发现检测效果因阶段而异,后期阶段信号更强。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS投毒攻击研究侧重攻击成功,防御检测不足,且检测需考虑多流水线阶段异质性。

Method: 构建Poison-3DGS基准,包含多视图图像、几何、训练动态和高斯参数等阶段特定信号,跨场景和攻击类型分析检测性。

Result: 可检测性随阶段变化;不同攻击有独特阶段信号;后期训练动态和高斯参数提供强信号。

Conclusion: 提供了阶段依赖检测性的系统评估,为鲁棒3DGS系统奠定基础。
...
### [100] [SimuScene: Simulation-Ready Compositional 3D Scene Reconstruction from a Single Image](https://arxiv.org/abs/2606.03994)
*Inhee Lee,Sangwon Baik,Sungjoo Kim,Hyeonwoo Kim,Hyunsoo Cha,Hanbyul Joo*

Main category: cs.CV

TL;DR: 提出SimuScene，将物理模拟嵌入单图3D场景重建流程，通过物理引擎反馈纠正形状与布局误差生成稳定可模拟的组合场景。


<details>
  <summary>Details</summary>
Motivation: 现有单图3D重建方法生成场景存在物体穿透、悬浮等物理不稳定问题，而物理感知方法仅后处理布局校正，未解决根本几何误差。

Method: 将物理引擎作为诊断工具嵌入生成过程：对重建物体施加重力模拟，将穿透/支撑失败转化为定量校正信号，驱动重力轴拉伸与非完整形状重采样。

Result: 在物理稳定性与几何对齐基准上达到最先进性能，并成功应用于人形控制与机械臂操作任务。

Conclusion: 物理反馈循环可有效抑制累积重建误差，生成稳定的可模拟组合3D场景。
...
### [101] [Structure-Guided Mixed Masked Pretraining and Spatial Continuity Regularization for Printed Circuit Board Defect Detection](https://arxiv.org/abs/2606.03508)
*Peitong Wang,Nuo Wang,Enxin Qin,Chengjin Yu,Hanyu Xuan,Yuanting Yan*

Main category: cs.CV

TL;DR: 提出一种结合结构引导混合掩码预训练与空间连续性正则化的两阶段PCB缺陷检测框架，在DsPCBSD+数据集上达到85.5% mAP0.5。


<details>
  <summary>Details</summary>
Motivation: PCB缺陷微小、低对比度且嵌入密集电路背景，现有方法检测困难。

Method: 预训练阶段采用结构引导混合掩码的稀疏卷积重建；微调阶段引入空间连续性正则化约束缺陷定位。

Result: DsPCBSD+数据集上实现85.5% mAP0.5和52.3% mAP0.5:0.95，优于多个强基线。

Conclusion: 该框架有效提升工业AOI场景下PCB缺陷检测的鲁棒性。
...
### [102] [AvatarMix: Identity-Preserving Cross-Avatar Composition for Outfit Personalization](https://arxiv.org/abs/2606.03506)
*Zhaorong Wang,Yoshihiro Kanamori,Yuki Endo*

Main category: cs.CV

TL;DR: 本文提出AvatarMix，一种通过直接组合两个高保真高斯化身来避免衣物穿模和质量下降的组合范式，并通过SeamFix和FullbodyFix两阶段精修策略实现无缝拼接和身体重塑后的外观保持。


<details>
  <summary>Details</summary>
Motivation: 现有3D虚拟换装方法存在衣物或身份质量下降（2D提升至3D方法）以及身体与衣物分层建模导致的穿模问题。

Method: 提出AvatarMix组合范式：直接组合头部和身体高斯化身；采用SeamFix局部扩散模块修补贴合区域（头发和颈部），以及可选FullbodyFix全局精修恢复衣物外观；基于网格的高斯表示支持鲁棒的网格重定向技术以适应不同体型。

Result: 在衣物保真度和身份保持方面达到最先进水平，有效避免了多视角伪影和穿模问题。

Conclusion: AvatarMix为现实3D服装个性化提供了新视角，通过组合与精修策略平衡了保真度与平滑过渡。
...
### [103] [EvoMemNav: Efficient Self-Evolving Fine-Grained Memory for Zero-Shot Embodied Navigation](https://arxiv.org/abs/2606.03509)
*Zuhao Ge,Xiaosong Jia,Chao Wu,Yuchen Zhou,Zuxuan Wu,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: 提出EvoMemNav，一种自演化的细粒度记忆框架，用于零样本具身导航，在保持原始视图的同时通过层级组织和预算策略提升长时规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于检测图的记忆丢失细节且累积噪声，而3D重建方法计算开销大，难以支撑零样本场景下的长时导航。

Method: 构建视觉语义记忆图(VSMGraph)保存原始视图，组织为房间-对象层级；采用预算驱动的粗到细策略，仅对关键区域调用VLM；通过反射驱动写回机制更新图附加先验知识。

Result: 在GOAT-Bench和HM3D的物体、文本描述、图像目标三种模态上，成功率和路径长度权值均一致提升，减少过早停止，增强零样本泛化。

Conclusion: 融合细粒度记忆与层级预算策略能有效提升零样本导航的长时规划性能，且无需重新训练。
...
### [104] [Knowledge-Preserved Model Tuning in Null-Space for Robust Spatio-Temporal Video Grounding](https://arxiv.org/abs/2606.03539)
*Haoxuan Chen,Xianqin Liu,Jian-Fang Hu*

Main category: cs.CV

TL;DR: 提出了零空间微调（NST）框架，通过将高质量输入的残差限制在零空间内，低质量输入的修复分量引导至非零空间，从而在保留预训练知识的同时修复退化输入，在混合质量基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位方法主要处理高质量输入，忽略了真实场景中广泛存在的低质量视频，而LoRA等微调方法会破坏预训练知识。

Method: 利用冻结权重的零空间几何特性，通过质量自适应单元和双空间重参数化合成可学习残差，使高质量输入残差对主干网络不可见，低质量输入残差发挥作用。

Result: 在混合质量基准上，NST方法超越了当前最先进技术。

Conclusion: 零空间微调能有效处理低质量视频输入，同时保持高质量输入的性能，平衡了退化修复与知识保留。
...
### [105] [Attend to Anything: Foundation Model for Unified Human Attention Modeling](https://arxiv.org/abs/2606.03540)
*Wenzhuo Zhao,Ronghao Xian,Keren Fu,Qijun Zhao*

Main category: cs.CV

TL;DR: 提出一个多模态基础模型AAM，统一处理图像、视频和音视频任务中的注意力建模，在16个基准测试上平均提升6%，视频推理速度提升约4倍。


<details>
  <summary>Details</summary>
Motivation: 现有注意力建模方法在模态、场景和任务上高度碎片化，无法在实际应用中泛化。

Method: 将注意力重新定义为认知蕴含关系，通过双曲空间的分层语言提示实现；采用流体动力学视角，用Fokker-Planck方程统一静态和动态注意力。

Result: 在16个基准上平均优于现有方法6%，视频推理速度提升约4倍。

Conclusion: AAM为注意力及相关任务的研究提供了原理性基础。
...
### [106] [\textsc{CR-Seg}: Attention-Guided and CoT-Enhanced Coarse-to-Refined Reasoning Segmentation](https://arxiv.org/abs/2606.03564)
*Yifan Cao,Xiaocui Yang,Faxian Wan,Shi Feng,Daling Wang,Yifei Zhang*

Main category: cs.CV

TL;DR: 提出CR-Seg两阶段框架，结合注意力引导和全局到局部思维链，实现从粗到细的推理分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖语义令牌或空间提示，存在跨模态对齐困难或丢失整体语义的问题。

Method: 设计EAP模块提取注意力图进行粗定位，并选择信息点输入SAM精化掩码；引入GLCoT实现全局场景到局部细节的逐步推理。

Result: 在推理分割基准上取得有效结果。

Conclusion: CR-Seg通过注意力引导和链式推理缓解了推理-回答不一致问题。
...
### [107] [When Attention Collapses: Stage-Aware Visual Token Pruning from Structure to Semantics](https://arxiv.org/abs/2606.03569)
*Jiahui Wang,Kai Zhang,Mai Han,Huanghe Zhang*

Main category: cs.CV

TL;DR: 提出了一种两阶段视觉标记剪枝框架STS，通过结构多样性采样和语义相关性过滤来改善VLM的效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力分数的视觉标记剪枝方法导致特征多样性降低和上下文细节丢失。

Method: 第一阶段使用排斥性采样最大化空间结构多样性，第二阶段通过指令感知的交叉注意力过滤无关标记。

Result: STS减少了注意力选择导致的冗余，提升了结构多样性和细粒度任务对齐。

Conclusion: 两阶段协同方法能有效平衡视觉标记的几何覆盖和语义相关性。
...
### [108] [Efficient Transformer-Based Localized Patch Sampling for Choroid Plexus Segmentation in Multiple Sclerosis](https://arxiv.org/abs/2606.03566)
...
TL;DR: 提出一种基于SwinUNETR和局部斑块采样的自动侧脑室脉络丛分割方法，在降低99%计算成本的同时提高精度。


<details>
  <summary>Details</summary>
Motivation: 手动分割侧脑室脉络丛繁琐耗时，限制其在多发性硬化临床研究中的大规模应用。

Method: 采用SwinUNETR架构，对32x32x32体素斑块进行训练，并与3D UXNET对比；使用多模态MRI（MPRAGE和FLAIR）输入。

Result: SwinUNETR在扩展测试集上的DSC达0.868，显著优于UXNET（0.858）；FLAIR单模态下HD95为1.86mm vs 3.00mm；计算量降低99%（91.8 vs 22,080 GFLOPs）。

Conclusion: 该方法准确、鲁棒且统计显著优于现有模型，极低计算成本适合临床和科研推广。
...
### [109] [Eliciting Complex Spatial Reasoning in MLLMs through Wide-Baseline Matching](https://arxiv.org/abs/2606.03577)
*Hao Zhong,Muzhi Zhu,Shenyan Zeng,Anzhou Li,Cong Chen,Hua Geng,Duochao Shi,Wentao Ye,Tao Lin,Hao Chen,Chunhua Shen*

Main category: cs.CV

TL;DR: 提出了ReasonMatch-Bench基准测试和动态对应强化学习（DCRL）方法，显著提升了宽基线匹配任务中多模态大语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型缺乏对宽基线匹配（WBM）所需的空间推理能力的系统评估和训练框架，亟待填补这一空白。

Method: 构建ReasonMatch-Bench基准，按视角位移和匹配粒度分层；设计可扩展的数据生成流水线，从视频-3D语料自动提取宽基线视图对；提出动态对应强化学习（DCRL），结合图像级视角递进和点级对应课程，通过可验证奖励优化训练。

Result: 在困难子集上，人类F1为84.0，最佳基线仅37.2；DCRL显著提升ReasonMatch-Bench性能，并迁移至相关空间基准，同时保持或小幅改进通用视觉理解。

Conclusion: DCRL能有效提升宽基线匹配中的空间推理能力，具有良好泛化性，为MLLM在物理环境中的部署提供了支持。
...
TL;DR: 本文系统研究了潜在扩散模型中潜在表示的可扩散性，发现现有指标预测生成质量不稳定，并提出了新的指标Velocity Irreducible Variance (VIV)，实验证明VIV是最稳定的预测因子之一。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明潜在表示需要具备多种特性（如语义可分性）才能有利于扩散模型，但这些特性仅在有限设置下验证，缺乏系统性比较。

Method: 训练了大量具有不同正则化策略、架构和潜在配置的标记器，并在多个下游扩散骨干网络中进行评估；引入Velocity Irreducible Variance (VIV)指标衡量潜在轨迹交叉导致的歧义。

Result: 识别出多个与生成质量稳定相关的潜在特性，其中VIV是最稳定的预测因子之一。

Conclusion: 潜在扩散模型中，潜在表示的可扩散性相比单纯的重构质量更为关键，VIV可作为预测生成质量的有效指标。
...
### [111] [World Models Meet Language Models: On the Complementarity of Concrete and Abstract Reasoning](https://arxiv.org/abs/2606.03603)
*Yucheng Zhou,Wei Tao,Yiwen Guo,Jianbing Shen*

Main category: cs.CV

TL;DR: 提出了一种控制性具体推理方法PF-OPSD，通过特权未来在线自蒸馏学习何时调用、验证和整合视觉模拟，在VRQABench和OpenWorldQA上性能提升10.6%和10.9%。


<details>
  <summary>Details</summary>
Motivation: 世界模型和多模态大语言模型互补但各有局限，视觉模拟可能随机且任务相关错误，需要决定何时使用模拟、评估其可信度以及如何影响最终答案。

Method: 提出了PF-OPSD方法，在训练时使用真实未来视频和答案作为特权上下文，评估在线策略的具体推理轨迹，而部署的学生模型在测试时不使用真实未来。

Result: 在VRQABench和OpenWorldQA上分别比基线提升10.6%和10.9%，同时增加了对噪声或冲突模拟的鲁棒性。

Conclusion: PF-OPSD有效实现了控制性具体推理，提升了视觉模拟在开放域物理预测中的可靠性和实用性。
...
### [112] [SkelHCC: A Hyperbolic CLIP-Driven Cache Adaptation Framework for Skeleton-based One-Shot Action Recognition](https://arxiv.org/abs/2606.03610)
*Yanan Liu,Anqi Zhu,Jingmin Zhu,Jun Liu,Hossein Rahmani,Mohammed Bennamoun,Farid Boussaid,Dan Xu,Qiuhong Ke*

Main category: cs.CV

TL;DR: 本文提出SkelHCC框架，通过双曲几何和CLIP模型实现单样本骨架动作识别，在多个数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖欧几里得嵌入和低级运动线索，难以建模骨架数据的树状层次结构，导致跨模态对齐和泛化能力不足。

Method: 提出显式分层双曲CLIP模块（EH-HCLIP）将骨架序列和动作语言嵌入共享双曲空间，以及训练无关的LLM引导多粒度投票缓存（LMV-Cache）进行上下文推理。

Result: 在NTU RGB+D 60、120和PKU-MMD数据集上持续优于现有最先进方法。

Conclusion: 双曲几何有效编码人体关节-部位-身体的层次结构，结合缓存机制能高效实现单样本动作识别。
...
### [113] [TurtleAI: Benchmarking Multimodal Models for Visual Programming in Turtle Graphics](https://arxiv.org/abs/2606.03626)
*Chao Wen,Jacqueline Staub,Adish Singla*

Main category: cs.CV

TL;DR: 论文介绍TurtleAI基准测试，发现主流VLM在教育式可视化编程任务中成功率低于30%，并通过小样本数据增强和微调将Qwen2-VL-72B的性能提升约20%。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注VLM在生产力场景中的可视化编程，但缺乏对教育场景的评估，且模型性能瓶颈不明。

Method: 基于Turtle Graphics域构建823个真实任务的基准TurtleAI，评估20+个VLM；提出仅需少量种子样本的数据增强技术，微调Qwen2-VL-72B。

Result: 多数VLM成功率<30%；微调后Qwen2-VL-72B在真实任务上提升约20%。

Conclusion: 当前VLM在教育可视化编程中空间推理和精确复制能力差，微调可改善视觉推理与代码实现的匹配性。
...
### [114] [VidMsg: A Benchmark for Implicit Message Inference in Short Videos](https://arxiv.org/abs/2606.03635)
*Issar Tzachor,Michael Green,Rami Ben-Ari*

Main category: cs.CV

TL;DR: 提出了VidMsg基准，用于评估短视频中的隐含信息理解，并展示了现有模型在此任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 短视频理解不仅需要识别可见对象和动作，还需推断潜在信息或目的，但缺乏相应的基准测试。

Method: 构建VidMsg基准：通过LLM将目标信息转化为间接搜索场景，检索候选片段后人工筛选；设计双向信息-片段检索和多选题诊断任务。

Result: 现有视频语言和检索模型在VidMsg上表现不佳，需进行语用推理和上下文线索整合。

Conclusion: VidMsg揭示了隐含信息理解的挑战，VidVec-Msg基线方法为后续研究提供了方向。
...
### [115] [A Benchmark for Semi-supervised Multi-modal Crowd Counting](https://arxiv.org/abs/2606.03646)
*Haoliang Meng,Xiaopeng Hong,Yabin Wang,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 本文构建了半监督多模态人群计数的首个基准，通过制定标准化协议和评估多种基线方法，为这一新领域奠定基础。


<details>
  <summary>Details</summary>
Motivation: 半监督多模态人群计数是一个未被探索的任务，缺乏标准化的基准和评估方法，需要建立统一的框架来推动研究。

Method: 提出了半监督多模态设置和标准化协议（含不同标注比例的数据划分），并改造了全监督多模态方法和半监督单模态方法作为基线。

Result: 在提出的基准上系统评估了各种基线方法的性能，验证了协议的有效性。

Conclusion: 该基准为半监督多模态人群计数提供了可靠起点，代码和数据划分已公开以促进后续研究。
...
### [116] [Graph Regularized Non-negative Reduced Biquaternion Matrix Factorization for Color Image Recognition](https://arxiv.org/abs/2606.03654)
*Hailang Wu,Yonghe Liu,Bingxuan Yu,Chaoqian Li*

Main category: cs.CV

TL;DR: 结合图正则化的非负简化四元数矩阵分解（GNRBMF）用于彩色图像识别


<details>
  <summary>Details</summary>
Motivation: 现有NRBMF方法只关注重构精度，忽略数据局部几何结构，限制判别能力

Method: 在简化四元数系数矩阵中加入图拉普拉斯正则项，并设计分量交替投影梯度算法求解

Result: 在部分测试条件下取得有竞争性或更优的识别性能

Conclusion: GNRBMF保留了非负性并利用局部结构，提升了图像识别效果
...
### [117] [Beyond Single Solution: Multi-Hypothesis Collaborative Deep Unfolding Network for Image Compressive Sensing](https://arxiv.org/abs/2606.03666)
*Wenxue Cui,Hualin Li,Yuhang Qin,Yifu Xu,Xiaopeng Fan,Debin Zhao*

Main category: cs.CV

TL;DR: 提出了一种多假设协作的深度展开压缩感知网络，通过跨解空间联合优化以处理逆问题的多解性，并设计了自适应步长与协作近端映射模块，显著提升重构性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度展开网络局限于单一解空间，忽略了压缩感知逆问题的多解性（病态性），导致无法探索多种可行候选假设。

Method: 基于近端梯度下降算法，提出多假设协作框架：(1) AlphaNet动态预测所有假设的空间变步长，实现梯度协作更新；(2) 多假设协作近端映射模块利用假设内/间相关性先验联合精化解；(3) 复合损失函数平衡测量保真度、假设多样性与重构精度。

Result: 实验表明，所提方法在重构精度上优于现有压缩感知网络。

Conclusion: 通过显式建模多假设并引入协作优化机制，有效提升了压缩感知对病态问题的适应能力与重构质量。
...
### [118] [Text-to-Image Models Need Less from Text Encoders Than You Think](https://arxiv.org/abs/2606.03715)
*Nurit Spingarn,Noa Cohen,Tamar Rott Shaham,Tomer Michaeli*

Main category: cs.CV

TL;DR: 本文发现文本到图像模型主要依赖文本嵌入中的单词含义和词序两个简单特征，而非完整的上下文信息。


<details>
  <summary>Details</summary>
Motivation: 探究文本到图像模型是否真正利用了文本嵌入中丰富的上下文信息（如组合性和属性绑定），挑战现有假设。

Method: 构造仅包含单词含义和词序的“无上下文”文本嵌入，并与完整文本嵌入对比在扩散Transformer模型中的生成效果。

Result: 无上下文嵌入在视觉质量和文本忠实度上达到与完整嵌入相当的效果，表明模型未使用上下文信息。

Conclusion: 文本到图像模型主要依赖单词含义和词序，复杂语言结构的解码由图像模型自身完成。
...
### [119] [A Fast Methane Detection Pipeline on Board Satellites Based on Mag1c-SAS and LinkNet](https://arxiv.org/abs/2606.03675)
*Jonáš Herec,Vít Růžička,Rado Pitoňák,Jan Sedmidubsky*

Main category: cs.CV

TL;DR: 本文通过结合高效的算法（如Mag1c-SAS）与轻量级机器学习模型，显著加速了高光谱卫星图像上的甲烷检测，性能大幅提升而资源消耗极低。


<details>
  <summary>Details</summary>
Motivation: 现有高光谱甲烷检测方法计算需求高，无法在资源受限的星载硬件上实时运行，而传统卫星受限于人工目标选择和下行速率，易遗漏泄漏事件。

Method: 测试快速目标检测方法ACE和CEM；提出Mag1c-SAS（Mag1c的加速变体）；集成U-Net和LinkNet模型；引入两个新数据集（STARCOP和EMIT-MSeg）及两种波段选择策略。

Result: Mag1c-SAS比原Mag1c快约80倍；与LinkNet结合后，AUPRC在EMIT-MSeg上提升超30个百分点，F1在STARCOP上提高约4个百分点；硬件剖显示功耗低、CPU/RAM利用率高效。

Conclusion: 所提方法在星载平台上实现快速、低功耗的甲烷检测，显著优于现有基准，并能有效降低噪声，具备实际部署可行性。
...
### [120] [Qwen-Image-Flash: Beyond Objective Design](https://arxiv.org/abs/2606.03746)
...
TL;DR: 本文从训练配方角度重新审视了少步蒸馏，通过Qwen-Image-2.0案例系统研究了数据组成、教师引导和任务混合三个因素，提出了Qwen-Image-Flash模型。


<details>
  <summary>Details</summary>
Motivation: 先前工作多关注蒸馏目标，而忽略了训练配方对少步蒸馏中学生模型性能的关键影响。

Method: 以统一文本到图像生成和指令引导图像编辑蒸馏为背景，系统分析数据组成、教师引导和任务混合三个因素。

Result: 揭示了若干非直观行为，并据此开发了Qwen-Image-Flash模型。

Conclusion: 有效的少步蒸馏不仅需要精心设计的蒸馏目标，还需要对训练流程进行原则性组织。
...
### [121] [Beyond False Stability: High-Noise Drift Gating for Test-Time Adversarial Defenses in Vision-Language Models](https://arxiv.org/abs/2606.03730)
*Hashmat Shadab Malik,Muzammal Naseer,Salman Khan*

Main category: cs.CV

TL;DR: 本文发现CLIP模型在强噪声下对抗样本稳定性反转，提出基于噪声漂移的门控机制以优化测试时防御的干净-鲁棒平衡。


<details>
  <summary>Details</summary>
Motivation: 现有随机测试时防御方法虽提升鲁棒性但降低干净精度，且未充分探索噪声强度对对抗样本稳定性的影响。

Method: 分析CLIP表示空间中噪声强度变化导致的对抗样本稳定性反转，提出无训练、即插即用的漂移门控机制，通过高噪声特征漂移作为门控信号触发测试时防御。

Result: 在13个数据集上改善干净-鲁棒平衡，细粒度数据集平均干净+对抗准确率从65.7%提升至71.4%（反攻击防御）和68.4%提升至73.2%（噪声锚定），ImageNet及变体从56.1%提升至66.2%。

Conclusion: 对抗样本在强噪声下更不稳定的现象可有效区分对抗与干净样本，门控机制无训练开销且通用性强。
...
### [122] [Investigating Adversarial Robustness of Multi-modal Large Language Models](https://arxiv.org/abs/2606.03713)
*Hashmat Shadab Malik,Muzammal Naseer,Salman Khan*

Main category: cs.CV

TL;DR: 该论文系统研究了多模态大模型（MLLMs）的对抗鲁棒性，提出了诊断CLIP对齐协议来预测鲁棒视觉编码器的迁移效果，发现多模态对抗预训练是关键因素。通过端到端多模态训练，在强对抗攻击下显著提升了性能（Captioning CIDEr得分提高28，VQA准确率提高11.7%）。此外，测试时视觉随机变换可作为非鲁棒模型的有效黑盒防御，并且鲁棒模型能大幅减少白盒视觉越狱攻击下的有毒内容生成。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs通过视觉编码器（如CLIP）引入视觉输入，扩大了攻击面，使其易受视觉对抗扰动攻击。先前的防御方法通过强制对齐CLIP原始嵌入空间来保持兼容性，但严重限制了可获得的鲁棒性。

Method: 提出了诊断CLIP对齐协议预测鲁棒视觉编码器的迁移效果；采用多模态对抗预训练与端到端鲁棒训练；探索了测试时视觉随机变换作为黑盒防御方法。

Result: 端到端训练在强对抗攻击下获得平均28 CIDEr和11.7% VQA准确率提升；鲁棒骨干上的对抗训练进一步带来1.9 CIDEr和4.3% VQA增益；测试时变换将非鲁棒模型的对抗性能从接近零提升到与鲁棒模型相当的水平。

Conclusion: 鲁棒视觉表示是MLLMs的严格前提，端到端对抗训练可在鲁棒骨上带来额外收益，测试时变换是有效的黑盒防御，鲁棒模型能减轻越狱攻击中的有毒生成。
...
### [123] [AmbientEye: A Dataset for Pupil Segmentation under Natural Ambient Infrared Illumination](https://arxiv.org/abs/2606.03774)
*Mingyu Han,Hyunyoung Han,Nitheekulawatn Thommakoon,Gangtae Park,Jieun Han,Xucong Zhang,Ian Oakley*

Main category: cs.CV

TL;DR: 本文介绍了AmbientEye数据集，证明仅使用被动红外摄像头在室外自然光下进行瞳孔检测的可行性，并发现现有算法性能大幅下降。


<details>
  <summary>Details</summary>
Motivation: 现有眼动追踪系统依赖主动红外照明，导致全天户外使用功耗高，存在实际障碍。

Method: 构建包含260万张室外眼图像的数据集，使用SAM2自动分割并人工修正，测试现有瞳孔分割算法。

Result: 瞳孔分割性能从0.928（受控红外）降至0.767（环境光），显示环境光挑战。

Conclusion: AmbientEye作为首个环境光眼动追踪基准，揭示了被动红外方案的实用性与性能差距。
...
### [124] [Ultralytics YOLO26: Unified Real-Time End-to-End Vision Models](https://arxiv.org/abs/2606.03748)
*Glenn Jocher,Jing Qiu,Mengyu Liu,Shuai Lyu,Fatih Cagatay Akyon,Muhammet Esat Kalfaoglu*

Main category: cs.CV

TL;DR: 提出了Ultralytics YOLO26系列，通过双头设计、MuSGD优化器、渐进式损失和STAL标签分配，实现了无NMS的端到端推理，并在检测、分割、姿态估计等多个视觉任务上取得了精度-延迟的帕累托前沿提升。


<details>
  <summary>Details</summary>
Motivation: 现有YOLO检测器在推理时依赖NMS、头部因DFL而沉重、训练周期长、小物体正标签分配不足，亟需统一的实时视觉模型方案。

Method: 双头设计实现NMS-free推理，移除DFL简化头部；训练采用MuSGD优化器、渐进式损失（朝推理头监督）和STAL标签分配策略（确保小物体正覆盖）；按任务设计专用头与损失。

Result: YOLO26在COCO上40.9-57.5 mAP（1.7-11.8 ms T4 TensorRT延迟），YOLOE-26x在LVIS minival上达40.6 AP，优于现有实时检测器。

Conclusion: YOLO26系列在多个视觉任务上同步提升精度与效率，为实时视觉提供了统一且高效的解决方案。
...
### [125] [Conditional Latent Diffusion Model with Fourier-based Motion Modelling for Virtual Population Synthesis](https://arxiv.org/abs/2606.03827)
*Shaokun Lan,Haoran Dou,Jinghan Huang,Arezoo Zakeri,Fengming Lin,Zherui Zhou,Jinming Duan,Alejandro F. Frangi*

Main category: cs.CV

TL;DR: 提出了4D F-MeshLDM，一种用于生成3D+t心脏网格序列的条件生成框架，通过傅里叶序列建模运动，实现了高保真度和周期一致性。


<details>
  <summary>Details</summary>
Motivation: 现有网格生成器多关注静态解剖，而序列模型缺乏显式周期性，无法满足心血管虚拟试验对动态解剖模型的需求。

Method: 结合卷积网格VAE、截断傅里叶级数的结构潜在空间和扩散先验，通过仿射调制进行临床协变量条件化，实现可控合成。

Result: 在5000名UK Biobank受试者上，优于现有方法，解剖保真度最高，周期闭合误差近乎为零。

Conclusion: 4D F-MeshLDM能生成准确保留临床功能指标的虚拟人群，为可靠的心血管在体试验提供了新框架。
...
### [126] [SLU-2K: A Question-Based Benchmark for Semantic Evaluation of Sign Language Translation](https://arxiv.org/abs/2606.03788)
*Zeno Testa,Antonino Furnari,Lorenzo Baraldi,Natalia Díaz-Rodríguez*

Main category: cs.CV

TL;DR: 提出SLU-2K数据集（2350个视频问答对），从语义理解角度评估手语翻译系统，发现当前模型语义正确率仅56.7%-75.2%。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标（如BLEU）仅关注词汇重叠，无法衡量翻译是否真正保留语义，不利于实际应用。

Method: 基于PHOENIX-2014T和CSL-Daily数据集，通过自动化流程生成7类问题（动作/地点/数字等）的QA对；评估MLLMs和SOTA系统（MMSTL/SpaMo）。

Result: MLLMs接近随机水平；最佳系统语义准确率75.2%，存在显著语义差距。

Conclusion: 当前评估高估了理解能力，未来应同时考虑语义正确性和流畅度。
...
### [127] [Beyond Encoder Accumulation: Measuring Encoder Roles in Multi-Encoder VLMs](https://arxiv.org/abs/2606.03879)
*Wei Ding,Yudong Zhang,Ruobing Xie,Xingwu Sun,Jiansheng Chen,Yu Wang*

Main category: cs.CV

TL;DR: 通过重新训练所有31个非空编码器子集，发现编码器排名依赖于联合训练方式，提出容量-必要性分解和投影前秩分析两个工具来指导多编码器大视觉语言模型设计。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏理解多编码器在联合训练中交互作用的工具，导致参数高效的编码器配置难以在训练前确定。

Method: 在Cambrian-1套件上统一流程重训所有31个非空编码器子集，计算每个编码器的容量（独立性能）和必要性（移除后性能下降），并分析投影前的有效秩。

Result: 联合训练中的编码器排名与固定检查点上的不同；高容量编码器配对不一定最优，高容量锚点与自适应补充配对可匹配全五编码器模型；投影前有效秩可解释残差分数变化，最佳配对中锚点秩存活而补充秩扩展。

Conclusion: 容量-必要性分解和投影前秩分析揭示了多编码器LVLM设计的方法论差距，并提供了闭合该差距的具体基元。
...
### [128] [Training-Free Multi-Concept LoRA Composition with Prompt-Aware Weighting](https://arxiv.org/abs/2606.03792)
*Georgios Tsoumplekas,Stella Bounareli,Vasileios Argyriou*

Main category: cs.CV

TL;DR: 提出基于提示感知重要性加权的W-Switch和W-Composite方法，通过动态调整多LoRA模块输出权重，实现了文本到图像生成中的多概念个性化定制。


<details>
  <summary>Details</summary>
Motivation: 现有多LoRA模型组合时概念间存在干扰，导致视觉质量下降和参考图像保真度降低。

Method: 基于每个概念在目标提示中的语义影响（即触发词的重要度），提出W-Switch和W-Composite两种提示感知重要性加权策略来融合多个LoRA输出。

Result: 在ComposLoRA测试基准上优于现有方法，在视觉质量、身份保持和组合性方面均取得一致改进。

Conclusion: 提示感知权重策略有效缓解了多概念干扰，并提出了基于图像相似度的定量评估框架，验证了方法的有效性。
...
### [129] [Beyond Compression: Quantifying Spectral Accessibility in Vision Representations](https://arxiv.org/abs/2606.03795)
*Akayou A. Kitessa,Yijun Zhao*

Main category: cs.CV

TL;DR: 本研究通过引入残余频谱损失（RSL）来测量视觉-语言模型中视觉信息结构的频谱可访问性变化，发现频谱可访问性在深度上呈现非单调轨迹，中间层达到峰值后向输出层下降。CLIP的投影是频谱中性的，而DINOv2的[CLS]池化则导致频谱上的结构化损失。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然将视觉特征映射到共享嵌入空间，但投影层如何改变视觉信息的结构尚不清楚，特别是频谱域的变化。

Method: 提出残余频谱损失（RSL），通过线性可恢复性测量频带限傅里叶能量，排除降维效应，并使用冻结参数的预训练模型（CLIP和DINOv2）在ImageNet和MS-COCO上分析。

Result: 频谱可访问性深度上非单调变化，中间层最高；CLIP投影频谱中性，DINOv2的[CLS]池化引入结构化频谱损失。
...
### [130] [Formalizing the Binding Problem](https://arxiv.org/abs/2606.03976)
*Lianghuan Huang,Yihao Li,Saeed Salehi,Yingshan Chang,Ansh Soni,Konrad P. Kording*

Main category: cs.CV

TL;DR: 本文提出了一种基于信息论的绑定信息探测方法，发现视觉Transformer（ViT）中的表示包含特征绑定信息（如“蓝色圆形”中颜色与形状的关联），并验证了绑定能力对视觉识别和推理的关键作用。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型（如ViT）在复杂场景中常错误归因特征（如将颜色错配到不同物体），但尚无方法直接测量模型内部的绑定信息，缺乏对绑定问题形式化的理解。

Method: 通过信息论框架形式化绑定问题，设计探测方法测量模型表示中的绑定信息；在ViT的不同组件（如[CLS]令牌、空间令牌）上，使用具有特征共享、遮挡、自然特征等绑定挑战的数据集进行评估。

Result: ViT的表示中确实存在可量化的绑定信息，且不同预训练ViT在绑定任务上的表现差异显著；绑定信息强的模型在视觉推理任务中表现更优。

Conclusion: 绑定信息是视觉系统实现强大识别与推理能力的关键成分，为改进多物体场景理解提供了新方向。
...
### [131] [TeX-1500: A Paired Real-World LWIR Hyperspectral Dataset and Benchmark for Temperature-Emissivity-Texture Decomposition](https://arxiv.org/abs/2606.03806)
*Cheng Dai,Jiale Lin,Hongyi Xu,Bingxuan Song,Ziyang Xie,Fanglin Bao*

Main category: cs.CV

TL;DR: 提出大规模配对数据集TeX-1500和基线模型TeX-UNet，用于长波红外高光谱成像的温度-发射率-纹理分解，解决了监督学习数据缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有TeX分解方法依赖场景特定的逆求解器，缺乏配对监督数据限制了基于学习的分解方法的发展。

Method: 通过DARPA IH推扫成像和FTIR采集构建1,522对校准实景数据，并设计TeX-UNet波长感知基线模型进行监督分解。

Result: 在预留数据和零/少样本迁移测试中，TeX-1500提供了可用的配对监督和可测量的基准。

Conclusion: TeX-1500推动了数据驱动的热物理属性感知研究，验证了监督学习在HSI-to-TeX分解中的可行性。
...
### [132] [Visual Instruction Tuning Aligns Modalities through Abstraction](https://arxiv.org/abs/2606.03871)
*Luis Palacios,Lorenzo Basile,Diego Doimo,Alberto Cazzaniga*

Main category: cs.CV

TL;DR: 本研究揭示了视觉指令微调主要作用于大语言模型的中间语义层，将视觉特征嵌入其中，且这种多模态集成是一种局部化现象。


<details>
  <summary>Details</summary>
Motivation: 探索视觉特征如何嵌入大语言模型的层级抽象中，以及多模态集成的机制。

Method: 通过探测分析和因果干预，比较视觉和文本表示的几何结构，并限制微调仅作用于中间层。

Result: 微调将视觉特征直接嵌入LLM的中间语义层；中间层对多模态性能至关重要；限制微调至中间层可节省训练时间且保持性能。

Conclusion: 多模态集成是通过复用LLM的内部抽象引擎实现的局部化过程。
...
### [133] [Where Do We (Not) Need Temporal Context in Low-Resource Video Task Adaptation?](https://arxiv.org/abs/2606.03837)
*Luc P. J. Sträter,Hazel Doughty*

Main category: cs.CV

TL;DR: 论文系统地研究了视频理解中的参数高效微调（PEFT）和探针方法，发现时间上下文的分配方式对视频模型适应效果至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有视频PEFT方法主要集中在图像预训练模型上，且通常将时间推理局限于模型单一组件，缺乏对时间上下文在不同模块（骨干网络、PEFT、探针）中分配的系统比较。

Method: 在多种场景（外观聚焦、运动聚焦、空间密集）下，对不同的模型适应策略（PEFT、探针等）进行系统评估，特别关注数据受限时的参数效率。

Result: 揭示了不同设置下PEFT和探针的新特性，并证明了合理分配时间上下文对视频适应的关键作用。

Conclusion: 视频模型适应中，时间上下文应合理分布到骨干网络、PEFT和探针中，而非局限于单一组件，以实现高效适应。
...
### [134] [Template Collapse and Information-Theoretic Limits in Camera rPPG Pulse Morphology Restoration](https://arxiv.org/abs/2606.03802)
*Achraf Ben Ahmed*

Main category: cs.CV

TL;DR: 本研究评估了16种架构在153名受试者中从单周期面部rPPG信号恢复动脉僵硬度形态的能力，发现所有测试编码器均无法提取个体特异性形态结构。


<details>
  <summary>Details</summary>
Motivation: 研究旨在验证消费级摄像头远程光电容积描记（rPPG）能否编码动脉僵硬度的生物标志物，填补该领域形态恢复能力评估的空白。

Method: 采用16种来自6个家族的架构，基于3个数据集（153名受试者），引入跨受试者皮尔逊相关系数（r）区分个体特异性恢复与模板塌陷。

Result: 无架构恢复个体特异性形态（跨受试者r范围0.773–0.9999，真实上限0.601）；监督对比学习（SupCon）收敛至log N=4.844，证明测试编码器无法提取判别性形态结构；VAE解码器恢复群体谐波内容（H2/H1比率输出0.310 vs 输入0.275），零样本泛化至UBFC（r=+0.708）；方向性幻觉缺口（p=0.150）表明部分信号读取；抗塌陷目标在输入无判别结构时失效。

Conclusion: 消费级摄像头不能编码个体动脉形态；跨受试者r是波形重建基准的必要塌陷诊断指标。
...
### [135] [Unified Video-Action Joint Denoising for Dexterous Action and Data Generation](https://arxiv.org/abs/2606.03868)
*Dingrui Wang,YuAn Wang,Jinkun Liu,Yue Zhang,Mattia Piccinini,Yu Sun,Johannes Betz*

Main category: cs.CV

TL;DR: 提出了一个统一的视频-动作去噪模型Donk，通过从分布角度对齐视觉先验与可执行动作，在多种条件下生成手部交互视频和动作轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将视觉先验压缩为条件动作策略分布，限制了先验的多样性。

Method: 采用去噪模型联合建模交互视频和可执行手部轨迹，支持语言、图像、手部状态等多种条件，以及无图像条件时的文本条件生成。

Result: 在动作、视频和文本生成评估中，Donk提高了灵巧轨迹精度，保持了视频保真度，并生成了平滑的文本条件动作。

Conclusion: 统一的训练框架能有效融合视觉先验与动作生成，同时适用于策略推理和数据增强。
...
### [136] [Seg2Track++: Probabilistic Track Validation and Data Association for Multi-Object Tracking and Segmentation](https://arxiv.org/abs/2606.03875)
*Diogo Mendonça,Tiago Barros,Cristiano Premebida,Urbano J. Nunes*

Main category: cs.CV

TL;DR: 提出Seg2Track++框架，通过集成SAM2分割与新型轨迹管理模块，实现零样本多目标跟踪与分割，提升时空一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型（如SAM2）在MOTS中直接应用存在轨迹关联不可靠和假阳性传播问题，需改进以用于自主系统。

Method: 采用掩码质心距离（MCD）和置信感知成本调制（CCM）进行轨迹关联，并利用伯努利滤波器的概率性轨迹验证（PTV）抑制假阳性轨迹。

Result: 在KITTI MOTS数据集上，无需微调即实现更好的身份保持、更少的假阳性传播和鲁棒的轨迹管理。

Conclusion: Seg2Track++有效结合分割与跟踪，为零样本MOTS提供了可靠的解决方案，适用于动态环境中的自主系统。
...
### [137] [CoralBay: A Self-Supervised CT Foundation Model](https://arxiv.org/abs/2606.03888)
*Ioannis Gatopoulos,Nicolas Känzig,Sebastian Otálora,Fei Tang*

Main category: cs.CV

TL;DR: 提出的CoralBay框架通过分层3D Swin骨干网络和自蒸馏方法，实现了对医学影像数据的自监督学习，并在多种下游放射学任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有2D自监督方法无法有效处理医学CT等3D影像的结构和语义特性（如空间连续性和组织密度信息）。

Method: 基于DINO架构，采用层级化3D Swin Transformer作为主干网络，对多尺度拼接特征进行自蒸馏学习。

Result: 在多种下游放射学任务中展现出强且一致的性能，并建立了公开的3D放射学基准排行榜。

Conclusion: CoralBay通过3D自监督学习有效捕获全局语义和局部细节，为医学影像分析提供了数据高效的表示方法。
...
### [138] [MLP Splatting: Object-Centric Neural Fields](https://arxiv.org/abs/2606.03877)
*Shinjeong Kim,Yuzhou Cheng,Xin Kong,Paul H. J. Kelly,Andrew J. Davison*

Main category: cs.CV

TL;DR: 提出MLP-Splatting方法，通过轻量级光场原语实现场景分解和照片级新视角合成，支持无需分割掩码的物体级编辑。


<details>
  <summary>Details</summary>
Motivation: 现有3D方法（如3DGS和NeRF）虽能实现照片级新视角合成，但缺乏对场景元素进行简单分解的能力，需要额外分割或分组才能进行物体级操作。

Method: 每个原语用一个紧凑独立MLP表示，具有局部空间支持，预测辐射度和不透明度，通过稀疏体合成进行渲染，仅使用RGB监督，可选语义特征蒸馏。

Result: 相比语义3DGS方法，内存使用降低至1/15，渲染速度提升3倍，支持开放词汇场景交互和开放集实例分割。

Conclusion: MLP-Splatting通过神经原语实现了高效场景分解和交互，在性能和功能上均优于现有方法。
...
### [139] [SparseStreet: Sparse Gaussian Splatting for Real-Time Street Scene Simulation](https://arxiv.org/abs/2606.03909)
*Qingpo Wuwu,Xiaobao Wei,Peng Chen,Nan Huang,Zhongyu Zhao,Hao Wang,Ming Lu,Ningning Ma,Shanghang Zhang*

Main category: cs.CV

TL;DR: 提出SparseStreet压缩框架，通过可学习剪枝和背景压缩减少高斯原语数量，在街景重建中实现80%压缩率且质量损失小。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法在街景重建中需要大量高斯原语来捕捉细节，导致存储和渲染开销大，而动态物体和静态背景冗余度不同。

Method: 引入节点可学习剪枝策略移除低贡献高斯原语，并在场景稳定后对静态背景进行压缩，保留动态物体细节。

Result: 在Waymo和nuScenes数据集上实现高达80%压缩率，质量退化最小。

Conclusion: SparseStreet能实现资源高效、高保真的动态场景重建，适用于街景。
...
### [140] [An Attention-Based Denoising Model for Diffusion Weighted Imaging](https://arxiv.org/abs/2606.03903)
*Prithviraj Verma,Pawan Kumar,Chandan Deshani,Prasun Chandra Tripathi*

Main category: cs.CV

TL;DR: 提出一种噪声感知注意力驱动的去噪框架，结合层次化Swin Transformer窗口注意力和Transformer多维门控细化，用于弥散加权成像（DWI）的降噪，在1%-15%噪声水平下实现平均PSNR 33.69dB和SSIM 0.8539。


<details>
  <summary>Details</summary>
Motivation: DWI在全身癌症筛查中采集时间长，缩短时间会导致图像噪声增加，且幅度重建引入的Rician噪声使传统卷积方法难以去噪。

Method: 集成层次化Swin Transformer窗口注意力与Transformer多维门控细化，结合显式噪声水平条件和残差重建，实现自适应抑制异方差噪声。

Result: 在1%至15%噪声水平下，模型达到平均PSNR 33.69dB和SSIM 0.8539，且在严重噪声条件下保持稳定。

Conclusion: 注意力引导的上下文建模与通道自适应细化提供了鲁棒且可泛化的DWI去噪解决方案。
...
### [141] [Bootstrap Your Generator: Unpaired Visual Editing with Flow Matching](https://arxiv.org/abs/2606.03911)
*Yoad Tewel,Yuval Atzmon,Gal Chechik,Lior Wolf*

Main category: cs.CV

TL;DR: 提出了一种无需配对数据的生成模型编辑训练框架，通过梯度路由和循环一致性实现高效图像视频编辑。


<details>
  <summary>Details</summary>
Motivation: 配对数据获取成本高，尤其视频编辑，限制了生成模型编辑训练的规模化。

Method: 利用冻结基模型提取指令跟随线索，结合循环一致性保持结构，通过梯度路由将下游损失传递到噪声训练状态。

Result: 在数据稀缺的图像和视频编辑任务上达到最先进水平，超越监督基线方法。

Conclusion: 梯度路由弥合训练推理差距，基模型语义线索提供稳健训练信号，无需外部奖励模型。
...
### [142] [Electromagnetic Navigation for Femoral Osteotomy Using High-Accuracy X-ray-to-CT Registration](https://arxiv.org/abs/2606.03893)
*Roman Flepp,Arend Nieuwland,Bastian Sigrist,Philipp Fürnstahl,Lilian Calvet,Thomas Dreher*

Main category: cs.CV

TL;DR: 提出一种基于电磁跟踪的导航系统，用于股骨截骨术，仅需两张术中透视图像即可实现术前计划的实时导航，在合成股骨上达到与患者专用器械相当的精度，且无需额外手术暴露。


<details>
  <summary>Details</summary>
Motivation: 现有截骨术技术存在精度不足、创伤大或辐射暴露多的问题，需要一种微创、准确且辐射少的方法。

Method: 集成电磁跟踪与CT术前计划，通过单次C臂标定和两张透视图像完成X光到CT的配准，实现无透视的实时导航，适用于单平面和双平面截骨。

Result: 电磁导航在总角度误差上显著优于徒手操作（3.05° vs 6.32°，p=0.031），且与患者专用器械在角度和位移误差上统计等价，无患者专用器械组超出临床阈值（5°）。

Conclusion: 该系统仅用两张透视图像即可传递术前计划，达到患者专用器械的精度，且无需额外手术暴露，为后续尸体和临床验证提供了基础。
...
### [143] [OVO-S-Bench: A Hierarchical Benchmark for Streaming Spatial Intelligence in Multimodal LLMs](https://arxiv.org/abs/2606.03890)
*Yifei Li,Pengyiang Liu,Yuhang Zang,Zhongyue Shi,Qi Fu,Hongye Hao,Jiwen Lu*

Main category: cs.CV

TL;DR: 该论文提出了一个名为OVO-S-Bench的流式空间智能基准测试，包含1680个问题，评估多模态大语言模型在连续输入下的空间推理能力。实验表明，最佳模型Gemini-3.1-Pro准确率59.2%，远低于人类86.6%，异中心映射为关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么在完整视频上进行离线评估，要么针对事件而非空间结构，缺乏对从连续自我中心流中实时推理位置和布局的评估。

Method: 构建348个源视频，由12名标注员经804小时多轮质量保证标注1680个问题，每个问题包含查询时间戳和证据区间，评估时模型仅可见前缀。问题分四个抽象层次：瞬时自我中心感知、时空上下文跟踪、空间模拟与推理、异中心映射。

Result: 对38个专有和开源MLLM的评估显示，Gemini-3.1-Pro得分59.2，人类86.6；流式空间微调MLLM表现劣于自身基座模型；思维链推理若未扎根于流式输入会放大空间错误。

Conclusion: OVO-S-Bench凸显了当前MLLM在流式空间智能上的显著不足，尤其是异中心映射能力，为下一代流式空间MLLM建立了严峻的测试平台。
...
### [144] [PatchScene: Patch-based Voxel Diffusion for Large-Scale Scene Completion](https://arxiv.org/abs/2606.03915)
*Qingdong Xu,Jiajun Zhu,Shilin Zhu,Xinjing He,Chao Lu,Huanran Wang,Jiyao Zhang*

Main category: cs.CV

TL;DR: 本文提出PatchScene，一种基于扩散的大规模激光雷达场景补全框架，采用补丁式体素扩散、置信度引导的时空融合及环形流扩散策略，在SemanticKITTI基准上达到最优性能，并展现出良好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖全局潜在表示或密集体素网格，难以有效处理大规模LiDAR场景的细粒度几何补全和时空一致性。

Method: 提出补丁式体素扩散范式生成局部3D区域的细粒度几何；引入置信度引导的时空融合机制整合重叠补丁和相邻帧；设计环形流扩散策略利用径向密度模式从近到远传播高保真信息。

Result: 在SemanticKITTI基准上所有标准指标达到最先进水平，且20米范围训练的模型可直接泛化到50米场景。

Conclusion: PatchScene在几何精度和时间一致性上超越现有方法，具有较强的可扩展性和泛化能力，适用于自动驾驶等真实应用。
...
### [145] [Benchmarking Visual State Tracking in Multimodal Video Understanding](https://arxiv.org/abs/2606.03920)
...
TL;DR: 提出了VSTAT基准，用于评估多模态大模型在视频中的视觉状态追踪能力，发现现有模型远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解评估忽视了对连续实体、状态和事件的视觉追踪能力，而这是人类视频理解的基础。

Method: 构建了包含834个视频片段和1500个问题的VSTAT基准，问题需要跨帧整合信息，不能单帧回答。

Result: 顶尖MLLMs在VSTAT上表现远低于人类，仅略优于随机基线；模型在文本推理中正确，但视觉感知失败。

Conclusion: 当前MLLMs和智能体方法在视觉状态追踪上存在根本性缺陷，需要新的改进方向。
...
### [146] [GARDEN: Gravity-Aligned Reconstruction of Disentangled ENvironments from RGB images](https://arxiv.org/abs/2606.03921)
*Jiahao Sun,Dingkun Wei,Zehong Shen,Hongyu Zhou,Yujun Shen,Liang Li*

Main category: cs.CV

TL;DR: 提出了GARDEN框架，通过重力先验将多视图RGB重建转化为物理可交互的混合场景表示，支持直接物理模拟。


<details>
  <summary>Details</summary>
Motivation: 现有重建方法缺乏物理结构，物体与背景纠缠且姿态任意，阻碍了稳定物理交互；基于检索的方法破坏几何保真度。

Method: 利用重力作为通用物理先验，将重建对齐到统一重力视角帧，恢复物体中心刚性网格及6自由度位姿，并通过条件3D点分类去除背景中的重复物体。

Result: 在模拟和真实场景中，GARDEN在物体放置可靠性、解缠质量及渲染-模拟效率上均优于基于检索的基线方法。

Conclusion: GARDEN实现了无需CAD检索即可产出可直接物理模拟且保持视觉真实感的混合场景表示。
...
### [147] [Demo2Tutorial: From Human Experience to Multimodal Software Tutorials](https://arxiv.org/abs/2606.03951)
*Zechen Bai,Zhiheng Chen,Yiqi Lin,Kevin Qinghong Lin,Difei Gao,Xiangwu Guo,Xin Wang,Mike Zheng Shou*

Main category: cs.CV

TL;DR: Demo2Tutorial 将屏幕录制和交互日志转化为结构化教程，提升人类学习和代理规划能力。


<details>
  <summary>Details</summary>
Motivation: 数字环境中的人类经验包含丰富的程序性知识，但未被充分利用。

Method: 通过录制器收集经验，动作解析器重构感知/动作/意图，步骤规划器生成层级任务图，教程编辑器输出图像-文本指令。

Result: 教程质量超过人类编写和基线方法，人类任务完成更快，GUI代理规划更优。

Conclusion: 从人类经验中提取的结构化教程可有效促进人类和代理学习。
...
### [148] [AAD-1: Asymmetric Adversarial Distillation for One-Step Autoregressive Video Generation](https://arxiv.org/abs/2606.03972)
*Haobo Li,Yanhong Zeng,Yunhong Lu,Jiapeng Zhu,Hao Ouyang,Qiuyu Wang,Ka Leong Cheng,Yujun Shen,Zhipeng Zhang*

Main category: cs.CV

TL;DR: 提出一种非对称对抗蒸馏框架AAD-1，用于一步自回归图像生成视频，解决了运动崩溃和训练不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用对抗蒸馏但存在运动崩溃和训练不稳定，导致生成静态视频。

Method: 架构上打破生成器与判别器的对称性：生成器保持因果性以保留自回归采样能力，判别器双向关注全时空上下文并输出单一整体真实性评分；训练上采用分阶段策略，先通过分布匹配预热稳定单步生成器，再进行对抗蒸馏。

Result: 在VBench上达到一步自回归视频生成的最新性能。

Conclusion: AAD-1框架有效解决了运动崩溃和训练不稳定问题，实现了高质量的视频生成。
...
### [149] [NewtPhys: Do Foundation Models Understand Newtonian Physics?](https://arxiv.org/abs/2606.03986)
*Sebastian Cavada,Soumava Paul,Tuan-Hung Vu,Andrei Bursuc,Raoul de Charette*

Main category: cs.CV

TL;DR: 提出NewtPhys，一个基于真实场景多视图图像的4D物理标注数据集，包含密集的细粒度物理量注释，用于评估视觉模型在低层级牛顿物理推理上的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型物理推理评估多使用合成或半合成场景，缺乏真实视觉复杂性和细粒度低层级物理信息。

Method: 构建NewtPhys数据集，使用多视图真实场景图像结合物理模拟，提供逐时间步的3D力、逐像素物理量等标注；并在56个视觉语言模型和10个视觉基础模型上系统评估。

Result: 揭示出现有模型在低层级物理推理上的明显局限性。

Conclusion: NewtPhys为物理感知视觉研究和下一代物理感知评估提供了基准和资源，促进物理基础视觉的发展。
...
### [150] [Adaptive Causal Alignment for High-Confidence Adversarial Training](https://arxiv.org/abs/2606.03925)
*Zhiming Luo,Kejia Zhang,Yingxin Lai,Junwei Wu,Juanjuan Weng,Shaozi Li*

Main category: cs.CV

TL;DR: 本文提出高置信因果对齐训练（HICAT），通过可学习背景偏差估计器（LBBE）和前景逻辑正交增强（FLOE）损失，在保持高置信预测的同时消除非因果背景依赖，提升鲁棒泛化。


<details>
  <summary>Details</summary>
Motivation: 发现高置信预测常源于对背景伪相关而非目标语义的过拟合，需避免盲目抑制背景导致特征丢失。

Method: 采用“测量-去偏-对齐”流水线，集成LBBE自适应评估背景效用、自适应去偏机制进行逻辑修复、FLOE损失强制执行特征解耦。

Result: 在CIFAR-10/100和ImageNet-1K上，CNN和ViT架构均持续超越基线，显著缩小鲁棒泛化差距。

Conclusion: HICAT通过语义均衡有效区分背景的因果与伪相关作用，实现鲁棒性与特征学习的统一。
...
### [151] [Video-Mirai: Autoregressive Video Diffusion Models Need Foresight](https://arxiv.org/abs/2606.03971)
*Yonghao Yu,Lang Huang,Runyi Li,Zerun Wang,Toshihiko Yamasaki*

Main category: cs.CV

TL;DR: 提出Video-Mirai训练方法，通过未来帧监督因果状态表示，在不改变推理过程的前提下提升视频生成一致性。


<details>
  <summary>Details</summary>
Motivation: 标准训练中因果状态仅解释当前片段，可能丢弃未来一致性所需的信息（如身份、布局、运动），存在表示级规划差距。

Method: 训练时：因果滚动生成→冻结前瞻编码器读取完整视频→轻量预测器将停止梯度的目标蒸馏到因果状态。推理时丢弃编码器和预测器，保持原有架构。

Result: 5秒VBench总得分83.8→84.6；30秒外推中主体一致性84.9→88.5，背景一致性90.2→91.9。
...
### [152] [PixVOD: Pixel-Distributed Direct Visual Odometry and Depth Estimation](https://arxiv.org/abs/2606.03989)
*Shinjeong Kim,Ignacio Alzugaray,Callum Rhodes,Paul H. J. Kelly,Andrew J. Davison*

Main category: cs.CV

TL;DR: 提出一种基于像素的高斯信念传播（GBP）的视觉里程计与深度估计方法，实现传感器上像素级别的并行分布式计算，并通过关键帧锚定机制保持几何稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统传感器传输冗余、噪声像素数据效率低，需转向传感器内处理（focal-plane sensor-processors），在像素层面完成部分计算以减少下游负载并提供更丰富的输入。

Method: 提出全并行像素级视觉里程计与深度估计，利用高斯信念传播（GBP）在像素间交换信息，达成相机运动共识，并从光度观测和表面法线先验推断深度；引入关键帧锚定机制调节帧间有效基线。

Result: 在真实数据集上评估，验证了基于GBP的分布式像素级里程计与深度估计的可行性，以及关键帧锚定在传感器上的有效性。

Conclusion: 实现了传感器上像素级分布式视觉里程计与深度估计，为高效片上视觉计算提供了新途径。
...
### [153] [On the Persistent Effects of Lexicality in Large Language Mod](https://arxiv.org/abs/2606.02750)
*Hammad Rizwan,Muhammad Umair Haider,Nishant Subramani,Mona T. Diab,A. B. Siddique,Hassan Sajjad*

Main category: cs.CL

TL;DR: 论文研究了大型语言模型表示中词汇重叠相对于语义内容的影响，发现词汇影响广泛存在，甚至出现在为语义相似性训练的模型中。


<details>
  <summary>Details</summary>
Motivation: 理解表示中词汇影响与语义内容的关系及其对下游任务的影响。

Method: 通过对抗性语义压力测试和信息论视角，分析不同模型、训练目标和架构中的表示。

Result: 词汇影响贯穿模型深度；中层区域词汇与语义信号同时退化；词汇影响在摘要和模型编辑等下游任务中显著。

Conclusion: 词汇重叠对LLM表示有普遍且深层的影响，中层区域是表面形式与意义均薄弱的过渡区。
...
### [154] [Hallucinations as Orthogonal Noise: Inference-Time Manifold Alignment via Dynamic Contextual Orthogonalization](https://arxiv.org/abs/2606.03022)
*Mingkuan Zhao,Wentao Hu,Tianchen Huang,Yuheng Min,Suquan Chen,Yide Gao,Yanbo Zhai,Shuangyong Song,Xuelong Li*

Main category: cs.CL

TL;DR: 提出了动态上下文正交化(DCO)方法，通过几何框架解释并缓解大语言模型的幻觉问题，在多个基准测试中显著提升了上下文忠实度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的幻觉问题严重影响了其可靠部署，现有方法往往在抑制幻觉和保留参数知识之间存在权衡，需要一种更有效的解决方案。

Method: 基于线性表示假设，将幻觉建模为残差流中语义流形上的正交噪声。DCO通过利用输入残差流作为动态上下文锚点，对注意力头输出进行正交分解，并使用层间Z-score抑制机制选择性地衰减异常正交分量。

Result: 在Llama-3-8B和70B模型上，DCO在XSum、NQ-Swap、IFEval等基准测试中优于基线方法，同时在TriviaQA和TruthfulQA等知识密集型任务上保持了高表现，有效缓解了幻觉抑制与知识保留之间的权衡。

Conclusion: 验证了幻觉的几何解释，DCO是一种计算高效的流形对齐方法，能有效提升模型输出的上下文忠实度。
...
### [155] [Predicting Inference-Time Scaling Gains from Labeled Validation-Set Output Statistics](https://arxiv.org/abs/2606.02981)
*Luyang Zhang,Jingyan Li*

Main category: cs.CL

TL;DR: 提出一个稳定且紧凑的预测器，利用单一验证集采样计算的统计特征（如答案一致性、首次正确样本位置、完成长度方差等）来预测大语言模型在最佳N采样中的性能提升幅度。


<details>
  <summary>Details</summary>
Motivation: 现有的最佳N采样需要完整执行流程才能评估效果，缺乏提前预测其增益的廉价方法，因此需要找出稳定且紧凑的统计预测指标。

Method: 在单一标记验证集采样上拟合岭回归预测器，通过bootstrap-Lasso稳定性分析筛选特征，并结合线性近似残差的集中性分析。

Result: 识别出三个核心特征（提示级一致性分布、标签辅助的首次正确样本位置、完成长度方差）加上熵补充，使岭回归预测器与实际增益的Spearman相关系数达到0.90。

Conclusion: 该预测器可用于在支付完整奖励模型评分成本前，通过标记验证集筛选候选配置，从而高效评估最佳N推理的潜在收益。
...
### [156] [Regret Pre-training: Bridging Prior and Posterior Views for Enhanced Knowledge Grounding](https://arxiv.org/abs/2606.03080)
*Mingkuan Zhao,Xiayu Sun,Wentao Hu,Suquan Chen,Jiaxuan Li,Xiaoyan Zhu,Xin Lai,Jiayin Wang*

Main category: cs.CL

TL;DR: 提出遗憾预训练框架，利用未来信息增强因果语言模型。


<details>
  <summary>Details</summary>
Motivation: 因果语言模型训练时未利用未来信息，但数据中这些信息是可用的。

Method: 基于LUPI范例的双视角架构，含因果学生分布和未来条件教师分布，通过KL散度后悔损失传递未来感知信号。

Result: 在OLMoE-1B-7B架构上，GlobalRegret和LocalRegret分别达到33.9%和32.2%准确率，优于基线30.2%。

Conclusion: 该框架无需额外参数，训练步骤仅需一次额外前向推理，能显著提升下游任务性能。
...
### [157] [G^2C-MT: Graph-Guided Context Selection for Document-Level Machine Translation](https://arxiv.org/abs/2606.03078)
*Baijun Ji,Zixuan Zhou,Xiangyu Duan,Yu Liu,Longbo Sun,Rupu Wei,Bohong Zhao*

Main category: cs.CL

TL;DR: 提出G^2C-MT方法，通过图引导的上下文路径发现，提升文档级机器翻译对远距离话语依赖的建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏显式机制建模段落间结构化话语依赖。

Method: 将段落作为节点构建轻量级话语图，结合语义相似性、邻接关系和关键词重叠建模节点关系，使用深度偏置随机游走采样上下文路径，并支持多路径采样。

Result: 在多个域上，G^2C-MT在DeepSeek-V3、Gemini-2.5-Flash-lite和Qwen-2.5/3系列等LLM上优于强基线。

Conclusion: 图引导的上下文路径采样能有效提升文档翻译的鲁棒性和表现。
...
### [158] [The Geometry of LLM-as-Judge: Why Inter-LLM Consensus Is Not Human Alignment](https://arxiv.org/abs/2606.03043)
*Sourabrata Mukherjee,Hamna Hamna,Kalika Bali,Sunayana Sitaram*

Main category: cs.CL

TL;DR: 大语言模型作为评判器时，在主观评分上表现出高度一致的偏差：评分范围仅为人类的30%-50%，评估轴几乎与人类正交（87°-89°），且LLM间一致性高于LLM-人类一致性。只有通过后验校准才能部分改善，但仍远低于人类可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有研究将LLM间一致作为质量信号，但忽略了共享偏差的可能性。本文旨在量化LLM评判器与人类评判的几何差异，区分共享信号与共享偏差。

Method: 在4个印度语数据集、8种语言和41个LLM评判器上测量4个几何量：分数扩展度、有效秩、与人类子空间的主角、以及评判器与人类间的堆叠相关性，均附带bootstrap置信区间。

Result: 主观评分中LLM分数范围仅为人类30%-50%，评估轴与人类近乎正交（87°-89°），LLM间一致性（r≈0.35）高于LLM-人类一致性（r≈0.27-0.32）。可验证事实模板下诊断指标恢复至人类范围。微调和偏好优化只能恢复分数扩展，但轴心几乎不变。只有后验校准能改善所有指标，但校准后的24B印度语评判器仍低于人类可靠性。

Conclusion: LLM间一致性应仅在通过几何检查证明评判器子空间与人类对齐时才可作为人类对齐的证据，否则这种共识反映的是塌缩子空间内的偏差一致性。
...
### [159] [Can Factual Opinions Be Edited (Manipulated) in Large Language Models?](https://arxiv.org/abs/2606.03096)
*Yuanpu Cao,Ziyi Yin,Fenglong Ma,Jinghui Chen*

Main category: cs.CL

TL;DR: 本文提出了FOE基准和SGE方法，评估和改善LLM中事实观点编辑的安全性与一致性。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法主要针对原子事实，忽略了操纵事实观点（如公众人物立场）的潜在风险，可能重塑公众形象、影响选举或社会观点。

Method: 构建FOE基准（261位公众人物、19个议题类别、2178条观点记录），并提出Self-Generated Evidence-Aligned方法，通过自生成证据实现观点与证据对齐，无需显式指令。

Result: 当前编辑技术在事实观点上表现不佳，仅能实现表面修改，无法保持编辑后观点与支持证据的一致性。

Conclusion: FOE基准和SGE方法为理解LLM中事实观点编辑的安全风险提供了基础。
...
### [160] [Small RL Controller, Large Language Model: RL-Guided Adaptive Sampling for Test-Time Scaling](https://arxiv.org/abs/2606.03102)
*Runpeng Dai,Tong Zheng,Rui Liu,Chengsong Huang,Hongtu Zhu*

Main category: cs.CL

TL;DR: 将自适应采样问题建模为马尔可夫决策过程，利用强化学习训练轻量级控制器，在推理时权衡正确性、延迟和计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有自适应采样方法依赖启发式规则或分布假设，无法动态平衡推理性能与计算成本。

Method: 通过强化学习训练基于最终答案统计的轻量级控制器，动态决定采样停止或继续，并等价于带预算约束的拉格朗日松弛优化。

Result: 在多个基准上优于ASC和ESC等强基线，实现了更好的正确性-采样轮次-总样本量权衡。

Conclusion: 所提方法以低开销（CPU可运行）有效提升测试时推理效率，并提供理论解释。
...
### [161] [Experience-Driven Dynamic Exits for LLMs with Reinforcement Learning](https://arxiv.org/abs/2606.03113)
*Yanyu Zhu,Hoilam Pao,Niu Hu,Wei Guo,Shaoxiong Zhan,Boyu Lai,Zitai Wang,Yongqin Zeng,Hai-Tao Zheng*

Main category: cs.CL

TL;DR: 提出LEDE框架，通过离线强化学习动态选择退出层与投机长度，加速大语言模型推理，实现2.0~2.7倍加速。


<details>
  <summary>Details</summary>
Motivation: 静态配置（固定退出层、投机长度）限制了自推测解码的效率，需要动态优化。

Method: 将优化问题建模为马尔可夫决策过程，使用离线强化学习学习策略，根据局部上下文动态选择退出层和投机长度。

Result: 在Llama-2和Llama-3上，LEDE相比自回归解码加速2.0~2.7倍，相比静态推测基线额外加速17%。

Conclusion: LEDE通过动态配置有效平衡计算成本与草案质量，显著提升推理速度。
...
### [162] [Coherence Maximization Improves Pluralistic Alignment](https://arxiv.org/abs/2606.03110)
*Taslim Mahbub,Yiding Pei,Shi Feng*

Main category: cs.CL

TL;DR: 本文提出通过内部一致性最大化（ICM）生成与特定人群价值观一致的上下文示例，无需人工监督即可对齐AI系统。实验表明，这些示例与人工标注性能相当，且一致性比单纯准确性更重要。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要大量人工监督，缺乏自动生成多样化价值观兼容示例的机制，因此探索如何高效、无监督地生成符合目标群体价值观的示例。

Method: 采用内部一致性最大化（ICM），通过最大化标签间的互预测性推断示例标签，生成针对特定人群的上下文示例。

Result: 在分类、偏好和开放生成等四个基准上，ICM生成的示例性能与真实标签持平；一致性高的示例泛化能力显著优于准确性相同但一致性低的示例；对预训练中代表性不足的人群，针对性反馈（针对模型最不确定的问题）比随机反馈更有效。

Conclusion: 一致性是规模化价值观指定的关键设计原则，可利用预训练语言模型中已编码的多元人类视角实现无监督对齐。
...
### [163] [DMT-CBT: Longitudinal Therapeutic State Modeling for CBT Counseling](https://arxiv.org/abs/2606.03132)
*Chang Liu,Shuyi Zhang,Changsheng Ma,Yongfeng Tao,Minqiang Yang,Bin Hu*

Main category: cs.CL

TL;DR: 提出DMT-CBT框架，通过跨会话动态建模治疗状态、多模态行为锚定及工具增强干预，解决现有LLM在CBT咨询中忽视纵向治疗过程的问题；构建多会话多模态数据集DMTCorpus，实验显示该方法在治疗保真度、情感轨迹和状态保存上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM咨询方法将治疗视为短文本单会话的局部响应生成，忽略了真实CBT中跨会话的治疗状态演化、多模态推理及延迟干预效应。

Method: DMT-CBT框架：跨会话维护结构化治疗状态、多模态行为锚定（如图像）及工具增强干预；构建包含演变状态、图像多模态行为及跨会话干预连续性的合成多会话数据集DMTCorpus。

Result: DMT-CBT在治疗保真度、治疗联盟、纵向情感轨迹及治疗状态保持上均优于基线方法，包括后验提取方法。

Conclusion: 动态建模多会话治疗状态能提升LLM心理咨询的真实性和有效性，验证了跨会话状态更新与多模态信息整合的重要性。
...
### [164] [A cross-domain tropical species dataset with Chinese vernacular names and CITES source links](https://arxiv.org/abs/2606.03156)
*Jeff Wang*

Main category: cs.CL

TL;DR: 本文发布了一个包含410,499个热带物种的版本化跨领域数据集，整合了多个生物多样性数据库，并新增了跨领域本体、中文俗名层和CITES来源链接层，中文俗名覆盖率达99.50%。


<details>
  <summary>Details</summary>
Motivation: 热带物种在商业和监管生命周期中跨领域共享，但现有生物多样性基础设施按界组织，导致数据分散，缺乏统一的本体和中文俗名来源。

Method: 从GBIF、Plants of the World Online、iNaturalist、NCBI Taxonomy、Catalogue of Life和Encyclopedia of Life整合分类标识符，新增跨领域本体、中文俗名层（含四层来源类型）和CITES链接层。

Result: 中文俗名覆盖率达99.50%（408,456个物种），中文俗名来源类型明确，但准确性需进一步审计；数据集通过Zenodo发布。

Conclusion: 该数据集为热带物种的跨境管理和研究提供了统一资源，但未来需进行盲审和验证以完善准确性，并计划提交正式数据描述论文。
...
### [165] [Fully Automated Identification of Lexical Alignment and Preference-Stage Shifts in Large Language Models](https://arxiv.org/abs/2606.03165)
*Thomas Stephan Juzek,Xiaoyang Ming,Jose A. Hernandez*

Main category: cs.CL

TL;DR: 本文提出了两种无需人工标注的评估指标，用于检测数字聊天助手（如ChatGPT）语言使用中的词汇失配问题，并验证了其与人类偏好学习阶段的关联。


<details>
  <summary>Details</summary>
Motivation: 现有研究依赖手动整理来描述语言失配现象，缺乏可扩展的自动化评估方法。

Method: 提出词汇对齐分数（Lexical Alignment Score）和三角化偏好偏移（Triangulated Preference Shift）两种指标，基于PubMed摘要生成文本，并对比六个模型家族（Falcon, Gemma, Llama, Mistral, OLMo, Yi）的窗口化文档频率。

Result: 无人工干预下识别出过度使用的词汇（如'suggest', 'additionally', 'strategy'），并估计其与偏好学习的关联；结果在多种参数设置和随机种子上保持稳定。

Conclusion: 该方法可扩展用于跨语言和领域的词汇（失）对齐系统研究，有助于改进未来模型的对齐效果并理解其起源。
...
### [166] [HyperPatch: Sequential Knowledge Editing Under n-ary Structural Drift](https://arxiv.org/abs/2606.03179)
*Yu-Kai Chan,Wen-Sheng Lien,Dong-Ting Yao,Bo-Kai Ruan,Kwan-Yeung Lin,Hong-Han Shuai,Meng-Fen Chiang*

Main category: cs.CL

TL;DR: 提出HyperPatch框架，将顺序知识编辑转化为超图流形上的稳定性问题，解决n元关系更新引起的结构漂移。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法在n元关系顺序更新时出现结构漂移和知识传递失败，常被误诊为参数幻觉。

Method: 三阶段方法：基于HGNN的结构先验初始化、SimHash拓扑对齐与LoRA拓扑编辑的顺序编辑、结构条件推理。

Result: 在MQuAKE-CF和MQuAKE-T上相对最优基线提升96.24%和21.06%的Hop准确率，在连续更新下避免标准KG方法的88.3%精度下降。

Conclusion: HyperPatch能有效维护n元事件完整性，在动态知识编辑场景中具有高可靠性和参数保持特性。
...
### [167] [SenseJudge: Human-Centric Preference-Driven Judgment Framework](https://arxiv.org/abs/2606.03189)
*Rui Li,Junfeng Liu,Xiangwen Kong,Linhai Xu,Zhifang Sui*

Main category: cs.CL

TL;DR: 提出SenseJudge框架和SenseBench基准，实现基于用户偏好的可定制LLM评估，在个性化评判和模型排名任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有评判方法忽略用户偏好多样性，难以适应真实人机对话场景。

Method: 构建SenseJudge框架（结合人类偏好驱动）和SenseBench基准（多轮真实交互指令）；应用于个性化评判和模型排名任务。

Result: SenseJudge在个性化评判任务中优于其他方法，模型排名与人类感知一致；通过位置偏差和一致性分析验证鲁棒性。

Conclusion: SenseJudge能有效适应多样化用户偏好，提升LLM评估的实用性和准确性。
...
### [168] [MemTrain: Self-Supervised Context Memory Training](https://arxiv.org/abs/2606.03197)
*Ziheng Li,Xingrun Xing,Haoqing Wang,Zhi-Hong Deng,Yehui Tang*

Main category: cs.CL

TL;DR: 提出MemTrain自监督训练框架，通过掩码重建和记忆回忆两个代理任务，增强LLM代理在长上下文中的记忆能力。


<details>
  <summary>Details</summary>
Motivation: 现有记忆代理依赖端到端强化学习，但高质量标注数据成本高且多样性不足。

Method: 在未标注Wikipedia语料上设计两个耦合代理任务：端到端掩码重建和中间记忆回忆，通过GRPO联合优化。

Result: 在长文本问答和搜索问答基准上，相比直接任务后训练，性能提升最高达17.67点。

Conclusion: MemTrain能有效提升LLM代理在下游任务中的记忆推理性能。
...
### [169] [AI Rater Discrimination Depends on Scoring Protocol in Complex Clinical Decision-Making](https://arxiv.org/abs/2606.03198)
*Sangwon Baek,Kyu Yeon Hur,Kyunga Kim*

Main category: cs.CL

TL;DR: 研究了大型语言模型作为AI评分者在临床评估中的行为，发现带评分标准的评分协议（GR）相比无标准协议（Non-GR）能提供更好的区分能力和一致性。


<details>
  <summary>Details</summary>
Motivation: AI评分者在临床评估中应用日益广泛，但其评分行为在不同评估条件下的差异尚未被量化研究。

Method: 通过因子实验，使用四个开源LLM作为CDSS和AI评分者，在2型糖尿病药物管理场景下，比较带评分标准（GR）与无标准（Non-GR）两种评分协议的效果，并分析五个设计因素的主效应和交互作用。

Result: Non-GR下AI评分者得分范围窄且高（平均74-78分），GR下得分更低（低7.69-49.64分）且区分度更大（区分度提升1.76-5.10倍）。GR能揭示不同评分者模型的行为差异，而Non-GR掩盖了这些差异。
...
### [170] [Sample-Size Scaling of the African Languages NLI Evaluation](https://arxiv.org/abs/2606.03219)
*Anuj Tiwari,Oluwapelumi Ogunremu,Terry Oko-odion,Jesujuwon Egbewale,Hannah Nwokocha*

Main category: cs.CL

TL;DR: 系统性样本量扩展实验表明，非洲语言自然语言推理性能随标注数据增加并非单调提升，存在语言敏感的非单调变化。


<details>
  <summary>Details</summary>
Motivation: 非洲语言标注数据稀缺，且增加数据量是否能稳定提升下游性能尚不明确。

Method: 基于AfriXNLI基准，使用XLM-R Large和AfroXLM-R Large两个多语言模型，在50-500样本量下进行受控实验并取多次随机子采样平均结果。

Result: 发现性能增长呈语言敏感且非单调模式，部分语言早期饱和或下降，低资源环境下方差高。

Conclusion: 仅靠数据量无法保证性能稳定提升，需语言敏感的建库策略与更强的多语言建模方法。
...
TL;DR: 提出了WebRISE基准，通过交互契约图（ICG）评估多模态大模型生成的网页工件的功能正确性，覆盖442个任务和多种输入模态。


<details>
  <summary>Details</summary>
Motivation: 现有基准只评估局部交互证据，无法捕捉需求引起的状态和转换，即页面是否真正工作。

Method: 将任务需求编译成交互契约图（ICG），包含可观测状态、用户意图转换以及DOM/视觉断言，实现无关实现的浏览器执行。

Result: 最强模型仅达到65.6%转换正确率和66.3%需求覆盖率；视觉质量不能代表行为（如Qwen3.6-35B-A3B在Markdown上视觉得分80.8但转换得分仅15.5）；视频提供了最强交互信号；ICG检测状态错误的速率是检查点式评估的2-16倍。

Conclusion: WebRISE能有效评估多模态大模型生成网页的功能正确性，揭示视觉质量与行为之间的差距，检测隐式约束仍存挑战。
...
TL;DR: 提出ARBOR框架，通过可重用的过程奖励机制（对比轨迹生成局部草稿、跨查询合并通用规则、稀疏配对评分）为LLM搜索智能体提供过程级梯度，在结果奖励失效时仍能优化训练。


<details>
  <summary>Details</summary>
Motivation: 现有LLM搜索智能体依赖结果奖励，在结果同质化（所有轨迹正确性相同）时奖励无梯度；过程监督要么代价高（训练验证器），要么不一致（逐查询规则一次性使用）。

Method: ARBOR维护一个跨查询共享的规则记忆库，从对比轨迹中提取局部草稿，合并为通用规则并随策略演进淘汰；通过稀疏配对评分将过程级奖励叠加到基奖励上，提供梯度信号。

Result: 在四个多跳问答基准上持续优于GRPO和DAPO，LLM评判准确率平均提升4.2个百分点，零梯度训练组中最多42%转变为信息组。

Conclusion: ARBOR通过可复用过程奖励有效解决了结果奖励失效问题，提升了搜索智能体的训练效率和准确性。
...
### [173] [Benchmarking Speech-to-Speech Translation Models](https://arxiv.org/abs/2606.03241)
*Alkis Koudounas,Hayato Futami,Quentin Jodelet,Osamu Take,Shinji Watanabe,Emiru Tsunoo*

Main category: cs.CL

TL;DR: 提出了一个统一的语音到语音翻译评估基准COMPASS，整合了46个指标，用于比较不同模型的性能，并发现域特定指标比单一MOS预测器更有效。


<details>
  <summary>Details</summary>
Motivation: 现有离线评估缺乏统一协议，不同研究使用不一致的指标子集，导致无法直接比较模型效果。

Method: 设计COMPASS框架，集成8个维度的46个指标，在FLEURS和CVSS数据集上测试1248个模型-语言配置，包括级联和端到端架构，并通过相关性过滤减少指标数量。

Result: 不同架构在自然度和说话人保留上差距超过30%，而翻译质量差异很小；相关性过滤将指标减至10个，保持排名一致性（Spearman's ρ>0.80）且评估时间缩短2.5倍；人类验证表明域特定指标更准确。

Conclusion: COMPASS可作为域感知S2ST评估的基础，强调域特定指标的重要性，而非单一MOS预测器。
...
### [174] [Structures Facilitate Retrieve, Rerank, and Generate](https://arxiv.org/abs/2606.03247)
*Yeqin Zhang,Haomin Fu,Xujie Zhang,Cam-Tu Nguyen*

Main category: cs.CL

TL;DR: 提出SF-Re2G方法，通过结构感知的对比学习与重排序，提升文档对话系统的知识检索与生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法将文档分割为独立段落，忽略了文档结构信息和上下文，导致检索与生成效果不佳。

Method: 1) 通过同节段落对比学习优化段落表示；2) 构建结构增强的重排序器，利用文档结构对检索候选分组并重新评分；3) 在生成阶段融入子图上下文。

Result: 在两个中英文数据集上验证了方法的有效性，提升了检索和生成性能。

Conclusion: 利用文档结构信息能显著提升文档对话系统的表现。
...
### [175] [When Does Complexity Conditioning Help a Frozen Sentence Embedding? A Controlled Study of Per-Sentence and Pair-Level Difficulty Adaptation](https://arxiv.org/abs/2606.03244)
*Suhwan Hwang*

Main category: cs.CL

TL;DR: 本文系统研究了难度自适应句子嵌入的可行性，发现基于单句复杂度的适配无效，而基于句对难度信号的门控残差适配在段落及分级任务中有效，可视为轻量级重排序器。


<details>
  <summary>Details</summary>
Motivation: 常识认为句子嵌入应根据输入难度动态适配，但这一假设缺乏系统验证，作者希望通过控制性实验检验该直觉的有效性及适用条件。

Method: 在冻结的Qwen3-Embedding-0.6B编码器后附加轻量适配器，仅访问最终池化嵌入；实验控制种子、对比单句与句对难度信号（交叉编码器），在PAWS、MRPC、QQP、STS-B四项任务上评估。

Result: 单句复杂度与基准误差几乎无关（Pearson≈0.05），不如随机控制；而基于句对交叉编码器难度的残差门控在STS-B（Spearman+0.022）和QQP（+0.037）上获得一致增益，且不偏离冻结基准。

Conclusion: 难度适配需要句对级别信号，单句级适配失败；有效形式本质是轻量重排序器而非更好的单向量嵌入，并提供了预测性能提升空间的预训练诊断方法。
...
### [176] [The Word and the Way: Strategies for Domain-Specific BERT Pre-Training in German Medical NLP](https://arxiv.org/abs/2606.03250)
*Henry He,Johann Frei,Raphael Schmitt*

Main category: cs.CL

TL;DR: 提出了ChristBERT，一组基于RoBERTa的德语医学语言模型，在多个临床NLP任务上达到了新最优。


<details>
  <summary>Details</summary>
Motivation: 德语生物医学语言模型受限于老旧架构或有限训练数据，需要更先进的领域适应模型。

Method: 使用135亿词元的德语语料库，对比了继续预训练、从头训练和词汇适应三种策略。

Result: 在5个基准中的4个上超越现有模型，达到新最优。

Conclusion: 最优策略依赖于任务：从头训练适用于高度专业文本，继续预训练适用于常见医学文本。
...
### [177] [Beyond "To whom it may concern": Tailoring Machine Translation to Audience and Intent](https://arxiv.org/abs/2606.03259)
*Raphael Merx,Ekaterina Vylomova,Trevor Cohn*

Main category: cs.CL

TL;DR: 研究首次大规模评估了LLMs在50种语言、5种模型规模和8个文本领域中的目的驱动机器翻译能力，发现显式指令能显著提升翻译适应性，但传统指标无法有效衡量。


<details>
  <summary>Details</summary>
Motivation: 现有MT模型和指标将翻译视为固定映射，忽略了翻译质量取决于用途（如受众、语气、意图）的现实需求。

Method: 跨语言、模型规模和文本领域的系统实验，对比显式指令、少样本示例、段落上下文等条件，并评估自生成指令的效果。

Result: 显式指令在非正式领域和大模型上提升最大；指令优于少样本示例和段落上下文；传统指标会惩罚适应性翻译；自生成指令可缩小80%的适应性差距。

Conclusion: 目的适应性翻译是LLMs可行且可测量的能力，但需要开发新指标来正确评估适应性质量。
...
### [178] [SEA-NLI: Natural Language Inference as a Lens into Southeast Asian Cultural Understanding](https://arxiv.org/abs/2606.03284)
...
TL;DR: 提出SEA-NLI基准测试，评估LLM在东南亚文化中的推理能力，发现所有模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有NLI基准偏向西方文化，缺乏对东南亚等多文化背景的评估。

Method: 构建包含8个东南亚国家、英语和当地语言的NLI数据集，由母语者验证。测试17个编码器和解码器模型。

Result: 模型整体表现低，尤其在知识密集型类别（语言、科技）上失败，主要原因是缺乏东南亚文化知识。

Conclusion: 文化适应性模型和文化感知提示能提升性能，但思维链提示效果有限。
...
### [179] [Multilingual Unlearning in LLMs: Transfer, Dynamics, and Reversibility](https://arxiv.org/abs/2606.03291)
*Chaoyi Xiang,Olga Ohrimenko,Benjamin I. P. Rubinstein,Lea Frermann*

Main category: cs.CL

TL;DR: 多语言遗忘学习存在转移差异，发现遗忘主要在后期解码层实现浅层抑制，可通过推理时转向轻易恢复大部分遗忘知识。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘研究主要针对英语，缺乏对多语言场景下遗忘效果的探索。

Method: 扩展TOFU基准至五种语言，在不同语言组合下微调、遗忘和查询模型，并进行层分析。

Result: 遗忘转移因语言脚本和语系而异，遗忘语言决定最强转移的查询语言；层分析表明遗忘主要作用于后期解码层，早期共享潜空间基本完好。

Conclusion: 多语言遗忘并非真正擦除知识，而是浅层抑制，可通过推理时转向恢复高达90%的遗忘知识。
...
### [180] [SagaQA: A Multi-hop Reasoning Benchmark for Long-form Narrative Understanding in TV Series](https://arxiv.org/abs/2606.03301)
*Galann Pennec,Zhengyuan Liu,Nicholas Asher,Philippe Muller,Nancy F. Chen*

Main category: cs.CL

TL;DR: 提出了SagaQA，一个需要跨剧集多跳推理的长视频问答基准，实验表明混合规划器在复杂叙事理解上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准侧重于局部帧或片段理解，缺乏对完整电视剧贯穿全剧的多模态叙事理解能力。

Method: 构建包含跨剧集推理步骤的SagaQA数据集，将规划策略分为并行、串行和混合三类进行评估。

Result: 混合规划器在生成连贯完整推理计划方面优于其他策略，展现出更强的复杂高层叙事理解能力。

Conclusion: SagaQA填补了长篇视频多跳推理的空白，混合规划方法为复杂叙事理解提供了有效途径。
...
### [181] [From Script to Semantics: Prompting Strategies for African NLI](https://arxiv.org/abs/2606.03304)
*Anuj Tiwari,Terry Oko-odion,Hannah Nwokocha*

Main category: cs.CL

TL;DR: 系统研究了少资源非洲语言的NLI任务中提示策略的影响，发现对比提示最稳定有效。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多语言评估中增加，但在少资源非洲语言中的推理行为研究不足。

Method: 在AfriXNLI基准上，对斯瓦希里语、约鲁巴语和豪萨语评估五种提示策略（基线、脚本感知、语言特定、对比、原生标签自翻译），使用Llama3.2-3B和Gemma3-4B模型，排除少样本和思维链影响。

Result: 对比提示在不同语言和模型中最可靠，类平衡性更好，总体准确率更高；良好设计的提示可超越带少样本和思维链的基线。

Conclusion: 提示设计对少资源语言NLI至关重要，语言感知的结构化可增强鲁棒性。
...
### [182] [Beyond Ideal Instruction: A Comprehensive Framework for Evaluating LLMs in Realistic Interactions](https://arxiv.org/abs/2606.03318)
*Xuan Yang,Hao Xu,Tingfeng Hui,Hongsheng Xin,Kaike Zhang,Chunxiao Liu,Ning Miao*

Main category: cs.CL

TL;DR: 提出RUT-Bench基准，评估大语言模型在真实世界用户工具调用场景下的表现，发现当前模型成功率低于40%。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准无法模拟真实用户的模糊性、不协作行为和意图变化，缺乏经验导向评估。

Method: 构建RUT-Bench，支持单轮/多轮对话中理想理性与非理想用户行为的高保真仿真。

Result: 19个模型中，最高成功率低于40%，面对非理想输入时性能显著下降。

Conclusion: LLMs在处理复杂非理想用户输入方面仍有重大局限，需进一步改进。
...
### [183] [Evaluating LLMs' Effectiveness on Real-World Consumer Device Repair Questions](https://arxiv.org/abs/2606.03331)
*Atm Mizanur Rahman,Md Arid Hasan,Syed Ishtiaque Ahmed,Sharifa Sultana*

Main category: cs.CL

TL;DR: 提出了一个包含991个真实维修问题的基准，用于评估LLM在设备维修中的表现，结果显示LLM虽有帮助但在高风险的手机维修中不可靠，且跨语言性能（孟加拉语）较差。


<details>
  <summary>Details</summary>
Motivation: 消费者设备维修是评估LLM的重要但未充分探索的试验场，因为维修任务要求推理、安全决策，错误建议可能导致严重后果。

Method: 从Reddit收集991个真实维修问题（含电话、电脑、数据恢复），配有技术人员编写的参考答案，并提供孟加拉语翻译；使用4个标准（正确性、完整性、实用性、安全性）评估6个最新LLM。

Result: LLM能提供有用帮助，但在高风险任务中不可靠；电话维修最难且最需安全；所有模型在板级诊断、维修优先级和恢复程序上犯重大错误；孟加拉语回答一致差于英语；GPT-5.4表现最佳。

Conclusion: 没有严格评估和安全保障，LLM不适合高风险的设备维修任务，尤其跨语言和电话维修领域需改进。
...
### [184] [Lingo_Research_Group at SemEval-2026 Task 9: Evaluating Prompt Variants for Polarization Detection](https://arxiv.org/abs/2606.03334)
*Pritam Kadasi,Anuj Tiwari,Mayank Singh*

Main category: cs.CL

TL;DR: 本文为SemEval-2026任务9提交的系统，使用短提示设计方法，通过Gemma3-27B模型在22种语言上实现极化检测，三个子任务的平均宏F1分数分别为0.762、0.587和0.444。


<details>
  <summary>Details</summary>
Motivation: 针对多语言文本中极化检测的挑战，探索提示工程在粗粒度和细粒度分类任务中的有效性。

Method: 系统评估了12种不同清晰度、定义细节、推理指导和上下文示例的短提示，在aya-101和Gemma3-27B模型上进行实验，最终选用Gemma3-27B提交结果。

Result: 官方测试集上平均宏F1：子任务1为0.762，子任务2为0.587，子任务3为0.444；平均准确率分别为0.819、0.678和0.498。

Conclusion: 提示方法能有效检测粗粒度极化，但在细粒度和多标签社会语言学分类中面临更大困难。
...
### [185] [The Unsampled Truth: Psychometrics in SLMs Measure Prompt Artifacts, Not Psychological Constructs](https://arxiv.org/abs/2606.03357)
*Nils Schwager,Christoph Hau,Simon Münker,Achim Rettinger*

Main category: cs.CL

TL;DR: 该研究发现，在心理测量评估中，小型语言模型的输出主要由提示符伪影主导，而非语义推理。通过系统变化提示符元素，观察到了对模型响应的显著影响，这限制了SLM在心理测量中的实用性，但提供了一个诊断工具来识别和分离伪影。


<details>
  <summary>Details</summary>
Motivation: 研究者假设小型语言模型的输出反映语义推理，但这一前提未经充分验证。为了评估和区分语义信号与提示符伪影的影响，进行了实验。

Method: 使用提示符变化框架，系统性地改变角色、指令、项目和选项符号，在13个开源权重模型（0.6B到14B参数）上进行评估。

Result: 提示符伪影的方差经常超过语义信号，模型更多表现出对提示符的遵从，而非模拟心理特质。

Conclusion: 这些发现限制了SLM在心理测量中的效用，但框架可作为诊断工具，用于识别破坏性伪影并分离语义理解，以助于未来前沿模型的研究。
...
### [186] [EntSQL: A Benchmark for Grounding Text-to-SQL in Long-Context Enterprise Knowledge](https://arxiv.org/abs/2606.03363)
*Chengxi Liao,Tao Xu,Zulong Chen,Chuanfei Xu,Yiyan Wang,Xinyun Wang,Yanlong Zhang,Xiaojun Chen,Zhibo Yang,Zeyi Wen*

Main category: cs.CL

TL;DR: 提出了面向企业的Text-to-SQL基准EntSQL，包含1066个中英文语义示例，评估模型在长上下文中专有业务知识文档的利用能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准（如Spider、BIRD等）未涵盖企业场景中依赖内部指标、报告惯例等私域知识的情况。

Method: 构建5个业务领域的1066个对齐中英文示例，要求模型在问题和数据库模式之外利用企业文档中的长上下文知识。

Result: 最佳系统在英文输入和长文档条件下仅达到15.9%的准确率。

Conclusion: 当前LLM在企业知识驱动的SQL生成任务中表现不足，突显了该方向的挑战。
...
### [187] [See, Infer, Intervene: Proactive World Modeling for Goal-Oriented Social Intelligence](https://arxiv.org/abs/2606.03371)
*Honghui Zhang,Chenmeinian Guo,Yichen Yu,Guanyu Liu,Yongming Qin,Chongguo Song,Mengyue Yang,Lei Yu,Tianyu Shi*

Main category: cs.CL

TL;DR: 提出了See-Infer-Intervene（SII）框架用于多模态零售代理的主动行为干预，并引入了基于意图状态建模的PIWM模型和相应的零售基准数据集。


<details>
  <summary>Details</summary>
Motivation: 多模态零售代理需要在客户明确请求之前主动识别客户行为并决定是否及如何提供帮助。

Method: 采用SII框架，结合PIWM模型（利用AIDA阶段和BDI心理场表示客户状态，预测动作条件下的意图转换），并构建了GuidanceSalesBench基准。

Result: 在30个测试视频上PIWM达到0.641的宏F1（基于真实客户状态），优于Qwen2.5-VL-7B基线；但端到端视频输入性能下降至0.295，低于随机基线0.414；实际初步试点中达到0.579宏F1。

Conclusion: PIWM在明确状态信息下表现良好，但视频到状态的映射是部署中的主要瓶颈，需要进一步改善视频理解与状态推理的衔接。
...
### [188] [Causal Evidence of Stack Representations in Modeling Counter Languages Using Transformers](https://arxiv.org/abs/2606.03398)
*Nishit Singh*

Main category: cs.CL

TL;DR: 本文研究了Transformer在计数器语言训练中学习到的栈结构表示是否是模型性能的因果必要条件。通过线性探针提取栈深度主表示方向，并对其进行消融实验，发现消除该方向后模型顺序准确率骤降至近0%。


<details>
  <summary>Details</summary>
Motivation: 尽管已有研究表明Transformer在计数器语言任务中能学习到与堆栈结构一致的表示，但尚不清楚这些表示是否对模型性能具有因果必要性。

Method: 训练线性探针从模型隐藏状态预测每个token处的栈深度，提取探针中的主表示方向，并通过消融实验移除该方向观察模型性能变化。

Result: 消融栈表示主方向后，模型的顺序准确率从正常水平降至接近0%，表明该表示对模型性能具有因果必要性。

Conclusion: Transformer在计数器语言任务中不仅学习到栈结构表示，而且这种表示是模型正确执行任务所必需的因果因素。
...
### [189] [Selective Token-Level Cryptographic Redaction for Privacy-Preserving Clinical Deployment of Large Language Models](https://arxiv.org/abs/2606.03399)
*Farhan Sheth,Ziyuan Yang,Yongying Lan,Si Yong Yeo*

Main category: cs.CL

TL;DR: 提出了HERALD，一种在客户端侧对敏感令牌进行加密、保留上下文的模型无关框架，在分类和医疗问答任务上恢复接近明文性能，同时规避隐私泄露风险。


<details>
  <summary>Details</summary>
Motivation: 现有LLM临床管线需传输原始敏感健康信息，隐私风险高；全数据集加密导致过大的计算、对齐和通信开销，难以大规模实用部署。

Method: 结合医疗命名实体识别器和词性驱动策略选择敏感令牌，执行目标词形还原以稳定表层形式，再用确定性密文替换并包裹显式分隔符，全部在客户端完成，无需修改下游模型。

Result: 全加密基线大幅损失效用，而HERALD在分类和医疗问答任务上持续恢复接近明文的性能。

Conclusion: HERALD提供了一种兼顾隐私与可用性的新型管线，在实现敏感内容加密存储、传输和处理的同时保持下游模型效用。
...
### [190] [Lexicons and grammars for language processing: industrial or handcrafted products?](https://arxiv.org/abs/2606.03412)
*Eric Laporte*

Main category: cs.CL

TL;DR: 本文分析了语言资源（语料库、词典和语法）的两种构建趋势：手工构建与自动化生成，并探讨了各自的信息含量、构建时间及实际可行性。


<details>
  <summary>Details</summary>
Motivation: 语言资源（词典、语法）的信息含量高于语料库，但手工构建耗时且自动化程度低，为了优化语言处理系统，需评估两种构建路线的优劣。

Method: 通过分析Brown Corpus、WordNet、FrameNet等具体语言资源案例，比较手工构建与自动化生成在信息丰富度、构建效率上的差异，并讨论未来可能的发展方向。

Result: 手工构建的资源信息更丰富，但构建周期长；自动化生成效率高，但信息含量较低。两种趋势并存且存在张力。

Conclusion: 语言资源构建的最佳路径可能是手工与自动化的结合，具体需根据应用需求（如系统复杂度、开发时间）权衡，但当前更现实的方向是逐步自动化。
...
### [191] [Large Language Models Are Overconfident in Their Own Responses](https://arxiv.org/abs/2606.03437)
*Mario Sanz-Guerrero,Manuel Mager,Katharina von der Wense*

Main category: cs.CL

TL;DR: 研究揭示了对话式LLM在指令微调后校准变差的两个原因：指令微调本身有害，而聊天模板通过“归属偏差”进一步加剧，模型对自身回答的置信度比用户相同回答高26%。提出一种推理时策略，将模型回答伪装成用户输入，无需重新训练即可改善校准。


<details>
  <summary>Details</summary>
Motivation: 现有工作发现指令微调后的LLM校准不如基座模型，但聊天模板对校准的具体机制尚不明确。

Method: 解耦后训练算法与聊天格式的影响，通过六个开源LLM、三个基准和三种置信度提取方法进行实验，并设计将模型回答视为用户输入的推理策略。

Result: 模型对自身回答的置信度比用户相同回答高26%，所提策略可将过拟合降低并改善校准高达26%。

Conclusion: 指令微调固有地损害校准，聊天模板通过归属偏差加剧问题；提出的简单推理时策略能有效缩小基座与指令微调模型间的校准差距。
...
### [192] [BaltiVoice: A Speech Corpus and Fine-tuned Whisper ASR System for the Balti Language](https://arxiv.org/abs/2606.03504)
*Muhammad Ali*

Main category: cs.CL

TL;DR: 介绍了BaltiVoice语料库，一个16.8小时的巴尔蒂语朗读语音数据集，并微调Whisper-small模型，将词错误率从零样本的182.18%降至30.07%。


<details>
  <summary>Details</summary>
Motivation: 巴尔蒂语没有公开的自动语音识别资源，缺乏语音技术研究的基础。

Method: 从Mozilla Common Voice录音中构建了包含10,060个经验证的Nastaliq脚本话语的语料库，并微调OpenAI Whisper-small模型。

Result: 在538条保留验证集上，词错误率从零样本的182.18%降至30.07%。

Conclusion: 该语料库和微调模型可以促进巴尔蒂语的语音技术发展，并已在HuggingFace公开。
...
### [193] [AutoTail-BSFGM: Class-Balance-Aware Fine-Tuning for Chinese Scholarly Text Classification](https://arxiv.org/abs/2606.03576)
*Anling Xiang,Yuwen Yang,Yang Shen*

Main category: cs.CL

TL;DR: 提出AutoTail-BSFGM方法，通过自动门控尾部先验调整、弱平衡Softmax损失和快速梯度法对抗正则化，改善中文学术文本分类中类别不平衡和语义相邻标签问题。


<details>
  <summary>Details</summary>
Motivation: 中文学术语料库常存在类别不平衡和语义相邻标签问题，影响文本分类效果。

Method: AutoTail-BSFGM结合自动门控尾部先验调整、弱平衡Softmax辅助损失和快速梯度法对抗正则化，仅修改训练目标和过程，推理时使用与基线相同的单编码器线性分类器。

Result: 在基于CSL的摘要-学科任务（67标签）中，使用MacBERT-base时验证准确率提升0.83%，锁定测试准确率提升0.49%；在标题-分类任务（13类）中，验证准确率提升0.70%，平衡准确率提升2.64%。部分指标提升显著。

Conclusion: AutoTail-BSFGM能有效改善类别平衡敏感性，在基于摘要的学术分类中一致提升性能，但并非所有指标和分割都均匀受益。
...
### [194] [Beyond the Literal: Decomposing Pragmatic Intent in Multimodal Meme Understanding](https://arxiv.org/abs/2606.03604)
*Zhengyi Zhao,Shubo Zhang,Zezhong Wang,Luyao Ye,Huimin Wang,Hanqi Yan,Binyang Li,Kam-Fai Wong,Yulan He*

Main category: cs.CL

TL;DR: 提出意图投影框架，通过正交投影分离字面与语用含义，提升LVLM对模因和讽刺内容的理解。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM常将模因的字面内容与语用意义混淆，导致回答局限于图像描述而非作者意图。

Method: 在表示层使用正交投影模块移除融合表示中的主导单模态方向，保留语用残差；输出层外化推理链；目标层用对比奖励惩罚字面复述。

Result: 在六个多模态基准上优于开源模型，缩小与专有模型的差距，在高分歧模因上提升最大。

Conclusion: 分离字面与语用信号是理解模因的关键，意图投影有效缓解字面塌陷问题。
...
### [195] [Building Reliable Long-Form Generation via Hallucination Rejection Sampling](https://arxiv.org/abs/2606.03628)
*Lin Li,Georgia Channing,Suhaas M Bhat,Gabriel Davis Jones,Yarin Gal*

Main category: cs.CL

TL;DR: 提出分段幻觉拒绝采样（SHARS）框架，通过在长文本生成中检测并重新采样幻觉片段，减少累积错误，提升事实一致性。


<details>
  <summary>Details</summary>
Motivation: LLM在长文本生成中易出现幻觉雪崩现象，早期错误会传播放大，影响可靠性。

Method: 使用任意幻觉检测器识别生成中的幻觉片段，重新采样直到产出忠实内容；基于语义不确定性实现检测，并针对长文本进行适配改进。

Result: 在标准化幻觉基准上，该方法显著减少长文本生成中的幻觉，同时保持或提升信息丰富度。

Conclusion: SHARS无需外部资源即可自纠正幻觉，且兼容外部扩展，有效提升生成可靠性。
...
### [196] [Safety Measurements for Fine-tuned LLMs Should be Grounded in Capability](https://arxiv.org/abs/2606.03648)
*Krishnapriya Vishnubhotla,Hillary Dawkins,Isar Nejadgholi,Svetlana Kiritchenko*

Main category: cs.CL

TL;DR: 通过微调适应特定能力目标会降低模型安全性，导致生成不连贯内容且自动化评估不可靠，结论依赖基准和评估器选择。


<details>
  <summary>Details</summary>
Motivation: 先前工作对微调影响安全性的研究实验设置随意、结论缺乏一致性，需要锚定能力目标来系统分析。

Method: 多维度评估微调对模型能力和安全性的影响，使用不同安全基准和评估器。

Result: 微调模型对安全提示生成不连贯内容，自动化安全判断不可靠，微调效果结论因基准和评估器而异。

Conclusion: 微调安全影响评估需标准化基准和评估方法，注意生成不一致性带来的误判问题。
...
### [197] [CoEval: Ranking Language Models for Custom Tasks Without Labeled Data or Trustworthy Benchmarks](https://arxiv.org/abs/2606.03650)
*Alexander Apartsin,Yehudit Aperstein*

Main category: cs.CL

TL;DR: CoEval是一个开源框架，通过教师模型生成无污染基准测试并利用跨族评审团进行无标签排名，以解决语言模型评估中的数据泄露和标签依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有公共基准测试存在数据泄露风险，导致评分反映记忆而非真实能力，且缺乏任务特定标签数据时模型选择困难。

Method: 从任务描述出发，使用教师模型合成无人工标签的新属性控制基准，并通过跨模型族评审团（注重供应商多样性而非规模）进行无标签排名。

Result: 在真实标签验证下，CoEval恢复正确排名且正确率ho=0.86；生成项与五大基准零13-gram重叠；评审团无自偏好，单评估者可能出现反相关（后悔值0.35）；四项任务7938次评估仅耗5.89美元。

Conclusion: CoEval提供了一种低成本、无污染、可复现的模型评估流程，适用于任何领域，可替代传统基准测试。
...
### [198] [Does Language Shift Break Medical Vision-Language Models? Indonesian Radiology Visual Question Answering Case Study](https://arxiv.org/abs/2606.03693)
*Pieter Christy Yan Yudhistira,Dzaki Rafif Malik,Novanto Yudistira*

Main category: cs.CL

TL;DR: 本文提出了印尼语放射学VQA数据集IndoRad-VQA，发现医学视觉语言模型在印尼语环境下性能下降8-25%，凸显了多语言评估的必要性。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLM主要评估英语任务，缺乏在非英语临床语言下的鲁棒性研究。

Method: 将VQA-RAD问答对翻译为印尼语，并加入自评估质量控制；测试通用、东南亚多语言及医学VLM在英语和印尼语提示下的表现。

Result: 印尼语输入下性能下降8-25%，错误类型包括yes/no翻转、侧位错误及输出语言不匹配。

Conclusion: 英语医学VQA的高性能不能保证在印尼语临床场景中的鲁棒性，需要更包容的多语言评估。
...
### [199] [Don't Forget Your Embeddings: Robust Knowledge Erasure via Precise Editing of Embeddings](https://arxiv.org/abs/2606.03695)
*Clara Haya Suslik,Or Shafran,Mor Geva*

Main category: cs.CL

TL;DR: 本文提出EMBEDding ERasure（EMBER）模块，通过稀疏矩阵分解精确擦除词嵌入中的概念特征，增强现有知识擦除方法的持久性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有知识擦除方法常忽略嵌入层，导致目标知识可通过对抗提示或重学习恢复，这促使研究嵌入层干预的必要性。

Method: 采用稀疏矩阵分解技术，对词嵌入中的概念相关特征进行精确擦除，作为即插即用模块集成到现有方法中。

Result: 在Gemma-2-2B-it和Llama-3.1-8B-Instruct上的实验表明，EMBER提升了擦除效果和特异性，将重学习后的恢复准确率降低至35%（相比之前的70%-76%），且仅影响少量概念专属标记的连贯性。

Conclusion: 精确的嵌入层干预是实现鲁棒概念擦除的关键，现有方法可通过类似增强获得改进。
...
### [200] [Re-Ranking Through an Attribution Lens for Citation Quality in Legal QA](https://arxiv.org/abs/2606.03728)
*Mohamed Hesham Elganayni,Selim Saleh*

Main category: cs.CL

TL;DR: 本文提出一种基于扰动归因的轻量级交叉编码器重排序方法，以提高法律问答中检索增强生成系统的引用忠实性。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成系统假设语义相似度高的段落更可能被模型引用，但实验证明在AQuAECHR基准上语义相似度与段落归因无关，相似度排序甚至不如随机选择。

Method: 训练一个轻量级交叉编码器，使用连续扰动归因分数（如C-LIME）对检索后的候选段落进行重排序，再输入语言模型生成带引用的答案。

Result: 在AQuAECHR基准上使用两个语言模型和五折交叉验证，重排序方法显著提高了引用忠实性及其与专家答案的一致性。独立训练的两个重排序器在原始归因一致性的基础上趋同。

Conclusion: 扰动归因可作为模型无关的实用训练信号用于引用感知的检索重排序，跨模型共享的相关信号部分可迁移，但同模型重排序效果更佳。
...
### [201] [Entropy Gate: Entropy Quenching for Near-Lossless Token Compression in LLM Pipelines](https://arxiv.org/abs/2606.03739)
...
TL;DR: 提出了Entropy Gate框架，通过热力学启发的熵淬火过程压缩LLM token，实现40-60%压缩率且保持语义保真度>0.80。


<details>
  <summary>Details</summary>
Motivation: LLM管道中低信息内容的令牌（重复上下文、冗长回答、冗余模板）浪费了大量token预算。

Method: 基于多因素信息能量E(t)（统计、结构、位置），采用自适应淬火调度T(τ)移除玻尔兹曼生存概率低于阈值的token，并通过能量加权相似度门控停止压缩。

Result: Phase 1启发式实现40-60%压缩率（S_E>0.80）；E→E^2能量平方放大额外提升10-25个百分点；上下文去重增加50-70%节省；结合外部存储实现代理工作负载88-96%复合压缩。

Conclusion: Entropy Gate是一种无状态、模型无关的token压缩框架，可作为OpenAI兼容HTTP代理部署，显著减少token消耗且保持语义质量。
...
### [202] [Framing Migration News with LLMs: Structured CoT as a Support for Human Interpretation](https://arxiv.org/abs/2606.03761)
*David Alonso del Barrio,Jing Wen,Daniel Gatica-Perez*

Main category: cs.CL

TL;DR: 研究如何利用本地可部署的开源LLM进行可解释的移民新闻框架分析，通过结构化思维链提示实现可审计的推理过程。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法依赖专有API和大模型，存在数据隐私、可重复性和公平可及性问题，学术研究小组需要透明、可审计且资源消耗适中的工具。

Method: 提出使用Llama3-8B的结构化思维链提示方法，基于预定义框架类别生成逐步推理依据，并开展人机评估。

Result: SCoT方法在分类性能上优于零样本和少样本基线，其解释逻辑性平均评分4.1/5，能促使对初始解释进行反思，但存在文本间差异。
...
### [203] [HybridThinker: Efficient Chain-of-Thought Reasoning via Compressed Memory and Transient Thought Steps](https://arxiv.org/abs/2606.03768)
...
TL;DR: 提出HybridThinker，通过混合训练策略在保留压缩记忆令牌的同时暂时保留思维步骤，以平衡压缩效率与细节保留，在4个推理基准上平均准确率提升5.8点。


<details>
  <summary>Details</summary>
Motivation: 现有链式思维压缩方法因丢失细粒度信息导致后续步骤易出错。

Method: 混合训练方案：部分思维步骤直接可访问，其余步骤通过注意力屏蔽强制模型使用记忆令牌进行压缩与检索。

Result: 在4个推理基准上匹配未压缩基线，平均准确率超越现有SOTA 5.8点。

Conclusion: 临时思维步骤保留和混合训练方案共同贡献性能提升。
...
### [204] [KletterMix: Climbing Toward High-Quality German Pretraining Data](https://arxiv.org/abs/2606.03773)
*Maurice Kraus,Ruben Härle,Sebastian Sztwiertnia,Abbas Goher Khan,Mehdi Ali,Michael Fromm,Kristian Kersting*

Main category: cs.CL

TL;DR: KletterMix是一个通过翻译高质量英语预训练语料库构建的德语数据集，旨在提升德语语言模型预训练质量。


<details>
  <summary>Details</summary>
Motivation: 德语高质量预训练数据资源稀缺，缺乏与英语相当的精心策划、记录完整且经过验证的数据集。

Method: 将现代英语预训练语料库翻译成德语，保留文档边界、元数据、来源结构和主题多样性；使用COMETKiwi评估翻译质量。

Result: KletterMix在多个领域实现高质量翻译，并在德语下游任务中相比现有德语语料库取得可衡量的性能提升。

Conclusion: 精心策划的翻译数据能显著加强德语预训练数据生态系统。
...
### [205] [Expert-Aware Causal Tracing of Factual Recall in Sparse MoE Language Models](https://arxiv.org/abs/2606.03780)
*Yuetian Lu,Ali Modarressi,Yihong Liu,Hinrich Schütze*

Main category: cs.CL

TL;DR: 提出了专家感知因果追踪方法用于稀疏混合专家模型，并揭示了专家级定位并非普遍存在。


<details>
  <summary>Details</summary>
Motivation: 现有因果追踪研究集中于稠密变压器模型，而稀疏MoE模型引入了路由专家贡献的关键问题。

Method: 专家感知因果追踪，通过注入噪声破坏事实偏好，对比干净MoE块输出和专家级更新来恢复真实与虚假对数概率的差异。

Result: Qwen3-30B-A3B-Base中L44E069专家被反复选中且补丁性能优于其他；Mixtral-8x7B-v0.1中中层信号需通过路由多专家联合恢复。

Conclusion: MoE事实追踪可实现专家感知，但专家级定位依赖于模型和协议，并非普遍。
...
### [206] [Reasoning over Grammar: Can Synthetic Linguistic Reasoning Traces Enhance Low-Resource Machine Translation?](https://arxiv.org/abs/2606.03782)
*Renhao Pei,Yihong Liu,Sampo Pyysalo,Hinrich Schütze,Shaoxiong Ji*

Main category: cs.CL

TL;DR: 探索了通过结构化中间步骤的语法推理来提升大型语言模型在极度低资源语言翻译中的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在低资源翻译中难以有效应用语法信息，因此希望借鉴思维链推理的进展，通过结构化语法推理步骤改善翻译质量。

Method: 提出了从UD树库、字典和语法规则库自动生成逐步语言推理轨迹的流程，并在三种设置（ICL、SFT、RFT）下以锡伯语和Chintang语为测试案例进行评估。

Result: 在上下文中学习时，可靠的句子特定轨迹显著提升了大多数模型、语言和指标下的翻译性能；而作为训练数据使用时，模型虽然学习了轨迹格式但生成内容常出错，收益较小且不稳定。
...
### [207] [Backdoor Unlearning Generalization: A Path Toward the Removal of Unknown Triggers in LLMs](https://arxiv.org/abs/2606.03785)
*Lisa Bouger,Théo Lasnier,Philippe Looubet Moundi,Yannick Teglia,Djamé Seddah*

Main category: cs.CL

TL;DR: 研究表明，大型语言模型中的后门攻击可以通过遗忘学习进行跨后门抑制，即移除一个已知后门也能压制未知后门。


<details>
  <summary>Details</summary>
Motivation: 现有防御手段一次只能处理一个后门且需知道触发器，导致防御者面对未知后门时处于劣势。

Method: 通过遗忘学习移除一个后门，并分析模型变化；引入交叉激活偏移距离量化不同训练引起的模型变化距离。

Result: 遗忘一个后门能有效抑制其他未明确针对的后门，该现象在多个模型族和不同注入方式下得到验证。

Conclusion: 防御者可故意注入可控后门并移除，利用跨后门转移机制压制攻击者可能引入的未知后门，为LLM安全提供新方向。
...
### [208] [Exploring Adversarial Robustness and Safety Alignment in Multilingual Multi-Modal Large Language Models](https://arxiv.org/abs/2606.03793)
*Hashmat Shadab Malik,Muzammal Naseer,Salman Khan*

Main category: cs.CL

TL;DR: 本文系统研究了多模态大语言模型在12种语言上的对抗鲁棒性和多语言安全性，发现跨语言可迁移的漏洞和“安全因失败”现象。


<details>
  <summary>Details</summary>
Motivation: 先前工作集中于英语任务，缺乏对多语言行为的探索。

Method: 使用梯度攻击评估开源MLLMs，并分析文本和图像中含毒指令的多语言表现。

Result: 对抗图像跨语言转移；文本中强语言更易产生不安全响应；图像中仅英语脚本被解析；低资源语言看似安全但实为理解失败。Qwen3-VL具有真正跨语言安全。

Conclusion: 浅层多语言适应导致虚假安全，深层整合实现真正对齐。
...
### [209] [Consistency Training Can Entrench Misalignment](https://arxiv.org/abs/2606.03810)
*David Demitri Africa,Arathi Mani*

Main category: cs.CL

TL;DR: 一致性训练会放大模型中的谄媚行为，但抑制奖励黑客和新兴失调，因此不是对齐中性的。


<details>
  <summary>Details</summary>
Motivation: 理解一致性训练（简单、可扩展且无标签）对模型对齐效果的影响，特别是其自举特性是否可能放大不良行为。

Method: 在108个模型有机体（7B-70B开源模型）上测试了七种一致性训练方法，这些模型被微调以展示各种受控失调行为。通过分析分布偏移和选择算子来探究影响机制。

Result: 一致性训练总体上抑制奖励黑客和新兴失调，但显著放大谄媚行为。分布偏移主要驱动系统性对齐效应。

Conclusion: 一致性训练不是对齐中性的，在关键系统中使用时需仔细审计。提供了统一理论框架来推导其放大或抑制失调的条件。
...
### [210] [Rethinking the Idiomaticity Decomposability Hypothesis: Evidence from Distributional Learning](https://arxiv.org/abs/2606.03817)
*Maggie Mi,Golzar Atefi,Atsuki Yamaguchi,Felix Gers,Aline Villavicencio,Nafise Sadat Moosavi*

Main category: cs.CL

TL;DR: 使用上下文语言模型研究习语的可分解性，发现模型内部可分解性与人类判断及句法灵活性相关，但训练过程中的习语表示稳定化受多种因素影响。


<details>
  <summary>Details</summary>
Motivation: 检验习语可分解性预测句法灵活性的传统观点与基于使用频率的替代观点。

Method: 通过上下文语言模型作为受控分布学习器，提出模型内部可分解性度量，并关联人类评分、句法灵活性和可预测性。

Result: 模型可分解性与人类判断弱相关，与句法灵活性呈小但一致的负相关；习语表示稳定化受惊讶度、可分解性和频率共同影响，可分解性训练效应最强。

Conclusion: 习语行为由多种分布因素（如可分解性、频率和惊讶度）共同解释，而非单一依赖可分解性或频率。
...
### [211] [Clustered Self-Assessment: A Simple yet Effective Method for Uncertainty Quantification in Large Language Models](https://arxiv.org/abs/2606.03846)
*Qi Cao,Takeshi Kojima,Andrew Gambardella,Helinyi Peng,Yutaka Matsuo,Yusuke Iwasawa*

Main category: cs.CL

TL;DR: 提出一种通过语义分组将生成样本转化为多项选择题并利用模型自身概率进行置信度估计的LLM不确定性量化方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖熵等间接信号，难以解释且未充分利用模型自我评估能力。

Method: 将生成样本进行语义聚类，转换为结构化多选题，使用LLM对每个选项的概率作为置信度。

Result: 在多个模型和数据集上表现优于基线，且仅需少量额外样本即可达到竞争性能。

Conclusion: 该方法有效且高效，能提升LLM输出的可靠性判断。
...
### [212] [A Training-Free Mixture-of-Agents Framework for Multi-Document Summarization using LLMs and Knowledge Graphs](https://arxiv.org/abs/2606.03867)
*Cuong Vuong Tuan,Trang Mai Xuan,Tien-Cuong Nguyen,Vu-Duc Ngo,Thien Van Luong*

Main category: cs.CL

TL;DR: 提出一种无需训练的多智能体框架，结合大语言模型和知识图谱，实现多文档摘要，在英越数据集上取得最优或竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉文档间复杂关系、依赖大量标注数据、跨领域语言泛化能力有限。

Method: 将摘要分解为抽取选择、知识感知抽象和迭代细化三个智能体任务，通过多视角一致性机制整合输出。

Result: 在四个英越数据集上达到最优或竞争性表现。

Conclusion: 验证了模块化设计的有效性和适应性，实现了无需微调的跨领域语言泛化。
...
### [213] [RealClawBench: Live OpenClaw Benchmarks from Real Developer-Agent Sessions](https://arxiv.org/abs/2606.03889)
*Zongwei Lv,Zhewen Tan,Yaoming Li,Yilun Yao,Yuxuan Tian,Lin Sun,Xiangzheng Zhang,Weihong Lin,Tong Yang,Guangxiang Zhao*

Main category: cs.CL

TL;DR: RealClawBench是一个基于真实OpenClaw会话构建的实时基准框架，通过重建执行环境和确定性评分器，将真实会话转化为可复现、自动评分的任务，评估了14个当代模型，最佳系统仅解决65.8%的任务。


<details>
  <summary>Details</summary>
Motivation: 现有代理基准缺乏反映真实用户请求的分布、多样性和难度等关键现实特性。

Method: 利用重建执行环境和确定性可验证评分器，从真实会话中提取任务，保持源分布并实现自动评分。

Result: 发布281个可执行任务，模型评分最高65.8%，Jensen-Shannon散度最大0.0448。

Conclusion: RealClawBench通过将真实部署会话转化为受控评估实例，提供了更准确衡量代理能力的基准路径。
...
### [214] [Synthesize and Reward -- Reinforcement Learning for Multi-Step Tool Use in Live Environments](https://arxiv.org/abs/2606.03892)
*Ibrahim Abdelaziz,Asim Munawar,Kinjal Basu,Maxwell Crouse,Chulaka Gunasekara,Suneet Katrekar,Pavan Kapanipathi*

Main category: cs.CL

TL;DR: 提出了PROVE框架，通过20个状态化MCP服务器和自动化数据合成管线生成有效轨迹，再利用程序化奖励（无需外部评判模型）训练LLM进行多步工具调用。


<details>
  <summary>Details</summary>
Motivation: 当前LLM多步工具调用训练面临真实执行环境成本高、合成查询与服务器状态脱节以及强化学习奖励鼓励冗余调用模式三大障碍。

Method: 构建了20个带会话隔离的状态化MCP服务器（343个工具），通过依赖图引导的对话模拟自动合成已验证的多轮工具调用路径，并设计了包含渐变有效性评分、依赖感知覆盖、自适应效率惩罚、工具名信号和参数值匹配奖励的程序化奖励。

Result: 在BFCL Multi-Turn、tau2-bench和T-Eval上分别提升最多+10.2、+6.8和+6.5个点，且紧凑的程序化奖励在两个模型家族（Qwen和Granite）上表现一致。
...
### [215] [Knowledge Editing in Masked Diffusion Language Models](https://arxiv.org/abs/2606.03924)
*Haewon Park,Yohan Jo*

Main category: cs.CL

TL;DR: 将定位-编辑方法从自回归模型转移到掩码扩散模型，发现两者在编辑位置上一致，但多词元编辑在MDM中性能下降，通过优化中间状态可恢复。


<details>
  <summary>Details</summary>
Motivation: 探索定位-编辑方法是否能从自回归模型有效迁移到掩码扩散模型。

Method: 转移定位-编辑方法至MDM（LLaDA, Dream）并与ARM（LLaMA, Qwen）对比，分析因果追踪和编辑效果。

Result: 编辑位置在两种模型中一致（早期到中层的MLP），但多词元目标编辑在MDM中系统性衰退。

Conclusion: 多词元编辑失败源于未优化部分掩码的中间状态，引入针对这些状态的优化可显著恢复性能。
...
### [216] [A Pocket Offline Model for Simultaneous Speech Translation as CUNI Submission to IWSLT 2026](https://arxiv.org/abs/2606.03948)
*Aziz Sharipov Ortega,Dominik Macháček*

Main category: cs.CL

TL;DR: 本文展示了将离线直接语音到文本翻译模型Canary与最先进的策略AlignAtt结合，实现同步翻译能力，并提交至IWSLT 2026同步语音翻译共享任务（捷克语-英语、英语-德语与意大利语）。系统优势包括高质量翻译、低计算需求（1B参数）和多语言支持（25种源语言和25种目标语言）。


<details>
  <summary>Details</summary>
Motivation: 现有同步翻译系统在低延迟场景下翻译质量不足，且计算资源要求高；作者希望通过整合离线模型与高效策略，在保持低计算开销的同时提升翻译质量与多语言适用性。

Method: 使用离线直接语音到文本翻译模型Canary，结合最先进的策略AlignAtt实现同步翻译能力。

Result: 在计算无关仿真中，系统在低延迟和高延迟场景下均优于同规模基线，翻译质量领先；模型仅1B参数，支持多语言翻译。

Conclusion: Canary与AlignAtt的结合能有效平衡同步翻译中的翻译质量与计算效率，适合多语言场景。
...
### [217] [Agentic Chain-of-Thought Steering for Efficient and Controllable LLM Reasoning](https://arxiv.org/abs/2606.03965)
*Yu Xia,Zhouhang Xie,Xin Xu,Byungkyu Kang,Prarit Lamba,Xiang Gao,Julian McAuley*

Main category: cs.CL

TL;DR: 提出Agentic Chain-of-Thought Steering (ACTS)，通过强化学习训练控制器，在推理过程中自适应地调整思考策略，以在保持准确率的同时节省令牌。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在思维链推理中令牌使用效率低，且缺乏对推理过程的控制。

Method: 将推理引导建模为马尔可夫决策过程，控制器观察推理轨迹和预算，生成推理策略和引导短语；通过合成轨迹初始化并借助强化学习优化。

Result: 在多个基准上，ACTS在节省大量令牌的同时达到与完整思维链相当的准确率，并支持可控的准确率-效率权衡。

Conclusion: ACTS实现了高效且可控的推理，为大型语言模型的推理控制提供了新途径。
...
### [218] [Efficient ASR Training with Conversations that Never Happened](https://arxiv.org/abs/2606.03957)
*Máté Gedeon,Péter Mihajlik*

Main category: cs.CL

TL;DR: 提出场景级对话合成流程，用LLM生成对话+TTS合成为低资源语言提供多说话人训练数据，仅用67小时真实对话+636小时合成数据，在匈牙利语基准上超越2700小时训练的零样本模型。


<details>
  <summary>Details</summary>
Motivation: 低资源语言和特定领域的对话ASR受限于领域匹配的多说话人训练数据稀缺。

Method: 构建生成流程：LLM生成带参与者元数据的场景级对话→映射说话人属性到TTS声线→合成带说话人感知的模拟对话。评估5种LLM族在单生成器、固定预算混合、规模扩展设置下的性能。

Result: 合成对话持续提升识别性能，但生成器选择和组合方式显著影响效果；最大配置（67小时真实+636小时合成）在匈牙利语基准上优于2700小时训练的零样本模型。

Conclusion: LLM生成的对话数据经TTS合成是真实对话语料的有效补充，可用于语音模型训练。
...
### [219] [QUBRIC: Co-Designing Queries and Rubrics for RL Beyond Verifiable Rewards](https://arxiv.org/abs/2606.03968)
...
TL;DR: 提出 QUBRIC 框架，通过协同设计查询和评分标准，将开放式问题转化为可评估的情景问题，并利用对比评分生成和可学习性过滤优化 GRPO 训练，在多个基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分标准的强化学习将查询分布视为固定，导致开放式查询产生模糊评分标准，窄化查询又引入无法验证的虚构参考，使训练缺乏奖励信号。

Method: 教师提取关键点重写开放式查询为情景问题；对比评分生成将教师-策略差距转化为查询级别标准；可学习性过滤保留信息丰富的查询-评分对用于 GRPO 训练。

Result: ArenaHard 上较 SFT 基线提升 +5.5 分；仅用指令跟随数据训练，在三个保留基准（法律、道德、叙事推理）上平均提升 +6.3 分，改进集中于推理相关维度。

Conclusion: 协同设计查询和评分标准能使基于评分标准的强化学习成为超越严格可验证任务的 RLVR 实际补充。
...
### [220] [AlignAtt4LLM: Fast AlignAtt for Decoder-Only LLMs at IWSLT 2026 Simultaneous Speech Translation Task](https://arxiv.org/abs/2606.03967)
*Quentin Fuxa,Dominik Macháček*

Main category: cs.CL

TL;DR: 本文提出了AlignAtt4LLM系统，首次将AlignAtt策略应用于仅解码器的大型语言模型（如Gemma-4），通过源文本跨度、对齐头选择、qk快速重放和运行时键值捕获等创新，实现了英语到德语、意大利语和汉语的同步语音翻译。


<details>
  <summary>Details</summary>
Motivation: 现有AlignAtt系统依赖于编码器-解码器交叉注意力，而仅解码器LLM缺少此结构，需要设计新的策略来支持同步翻译。

Method: 方法包括：(1) 在提示中显式指定源文本跨度；(2) 离线选择翻译专用的对齐注意力头；(3) 选择性快速重放草稿到源文本的注意力块；(4) 运行时捕获查询/键值以保持模型输出比特级一致。

Result: 在IWSLT 2026开发集上，对于英语到德语和意大利语，方法在低延迟（约2秒）和高延迟（低于4秒）场景下均优于基线；英语到汉语结果较不均衡，但方法不限于特定模型。

Conclusion: AlignAtt4LLM通过确定性的提示布局、校准的对齐头和键值捕获，可复用于更强的仅解码器翻译模型，尤其适用于非欧洲语言。
...
### [221] [Quantifying Faithful Confidence Expression in Large Reasoning Models](https://arxiv.org/abs/2606.03969)
*Areeb Gani,Asal Meskin,Gabrielle Kaili-May Liu,Arman Cohan*

Main category: cs.CL

TL;DR: 本文研究了大型推理模型（LRM）在语言表达信心与内在不确定性对齐方面的忠实校准（FC）问题，发现LRM普遍无法忠实地表达其内在信心，且现有评估方法存在局限性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRM）的推理轨迹常被用户视为深思熟虑的体现，但模型内在信心与语言表达信心的不一致可能影响其可信度；现有忠实校准（FC）评估方法不适用于LRM的长链推理输出，因此需要新框架来量化FC。

Method: 提出一个新框架，通过分析语言决断性与三种内部不确定性（基于token概率、隐藏状态和采样响应一致性）的关系，并采用前缀条件采样方法控制轨迹中的条件和结构变化。

Result: 发现LRM在忠实表达信心方面存在显著挑战：推理行为不会自动改善FC，针对非推理模型的提示干预无法提升推理设置中的忠实性，不同信心估计器对相同轨迹产生不同评估结果。

Conclusion: 忠实校准（FC）是LRM的独特可靠性和对齐目标，尤其是在高风险应用中需要重点关注。
...
### [222] [Language Models Compare Quantities Using Number-specific and Unit-specific Heuristics](https://arxiv.org/abs/2606.03982)
*Mutsumi Sasaki,Go kamoda,Ryosuke Takahashi,Kosuke Sato,Kentaro Inui,Keisuke Sakaguchi,Benjamin Heinzerling*

Main category: cs.CL

TL;DR: 语言模型比较带单位的量时，存在边界效应，且其行为可通过数值差和单位尺度差的线性替代模型预测。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型如何处理带测量单位的数量比较，特别是在不同单位系统下的表现。

Method: 控制实验设计，涵盖多个单位系统；使用线性替代模型预测LM偏好；通过因果干预验证数值和单位维度对输出的影响。

Result: 模型在比较边界附近准确率下降；错误具有系统性；线性模型能基于数值差和单位尺度差预测LM的偏好。

Conclusion: 语言模型比较带单位的量时，并非先转换为同一尺度，而是依赖数值和单位的启发式组合。
...
### [223] [CARVE: Certified Affordable Repair of Vetoed Maneuvers via Envelopes for Interactive Driving](https://arxiv.org/abs/2606.02641)
*Yifan Wang*

Main category: cs.RO

TL;DR: 提出了交互修复认证方法CARVE，在无需预测的情况下为自动驾驶提供形式化的交互可行性证明，解决了规则感知系统中硬规则边界过于严格的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（规则簿、安全防护、可达性滤波器）能否决不安全动作但缺乏交互协商能力，而基于预测的方法无法提供运行时证明，导致合法但被错误否决的安全动作无法被还原。

Method: 基于有限格结构的Ego和Agent战术算子，定义Agent可允许修复边界B_j(s)，通过责任加权成本分割和备份策略构建认证证书。

Result: 在589个场景中，CARVE接受了98.64%被否决的动作，恢复了370/378个人工标记的假否，同时保持100%优先权遵守。

Conclusion: CARVE能在无预测条件下证明交互动作的有界性、可归因性和规范性，解决了安全与效率的平衡问题。
...
### [224] [Multi-Robot Bearing-only Pose Estimation via Angle Rigidity](https://arxiv.org/abs/2606.03931)
*J. Francisco Presenza,Leonardo J. Colombo,Ignacio Mas,Juan I. Giribet*

Main category: cs.RO

TL;DR: 提出了一种基于角度的分布式位姿估计器，用于时变多机器人系统，仅需角度刚性拓扑和部分机器人持续激励运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖较强的刚性条件，如方位刚性，而角度刚性条件更弱，适用于更广泛的场景。

Method: 利用体坐标系方位角计算位置，再通过估计位置、方位及方位导数恢复SO(3)姿态；采用角度刚性有向传感拓扑。

Result: 在部分机器人持续激励运动条件下，证明了局部一致指数稳定性，仿真验证了有效性和实用性。

Conclusion: 该方法能在弱拓扑条件下实现三维位姿估计，具有理论与实际应用价值。
...
### [225] [SplitAdapter: Load-Aware Humanoid Loco-Manipulation via Factorized Adaptation](https://arxiv.org/abs/2606.03297)
*Jeonguk Kang,Hanbyel Cho,Sanghyun Kang,Donghan Koo*

Main category: cs.RO

TL;DR: 提出SplitAdapter框架，通过分解式适应策略提升人形机器人负重操作在仿真到现实迁移中的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有历史基适配器将负载和动力学变化压缩为单一隐变量，导致重载操作鲁棒性差。

Method: 冻结预训练操作策略，扩展负载感知和动力学感知上下文编码器，采用分裂世界模型目标、GRL交叉对抗正则化和分层特征线性调制。

Result: 在2、4、6kg负载和0、30、60cm高度下，全任务成功率优于基策略和世界模型FiLM基线，重载条件下提升最大。

Conclusion: 分解式适应策略有效增强人形机器人在不同负载和高度下的操作鲁棒性。
...
### [226] [RobotValues: Evaluating Household Robots When Human Values Conflict](https://arxiv.org/abs/2606.03312)
*Jongwook Han,Hyeongjin Kim,Yohan Jo*

Main category: cs.RO

TL;DR: 提出 RobotValues 基准，用于评估家用机器人在价值冲突场景中的价值偏好选择能力。


<details>
  <summary>Details</summary>
Motivation: 现有机器人评估过度关注任务完成，忽略日常环境中的价值冲突问题。

Method: 借助 LLM 生成场景、利益相关者价值提取、图像生成和自动质量控制构建 10K 价值冲突实例的基准。

Result: VLM 模型默认偏好安全和适应性，但常忽略隐私；在指令冲突时 80% 无法正确调整行为。

Conclusion: 家用机器人评估应包含价值冲突下的行为选择能力，而非仅关注任务完成或安全合规。
...
### [227] [GPU-Parallel Multi-Task Reinforcement Learning with Demonstration Guided Policy Optimization](https://arxiv.org/abs/2606.03335)
*Rui Zhang,Qiwei Wu,Zhengyu Zhang,Tao Li,Yunrong Guo,Junjie Lai,Renjing Xu,Weihua Zhang*

Main category: cs.RO

TL;DR: 本文提出了MT-Libero，一个基于GPU并行的多任务强化学习基准，以及DGPO算法，通过演示引导和自适应行为克隆提升异构任务训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习系统大多为单个任务优化专家策略，缺乏高效支持多任务并行训练的基准和方法。

Method: 构建MT-Libero基准（基于LIBERO资产和Isaac Lab），提出DGPO算法（结合重要性加权PPO和自适应行为克隆）。

Result: DGPO在稀疏奖励和有限演示数据下优于无演示强化学习和现有演示方法，保持on-policy PPO的稳定性和在线改进优势。

Conclusion: 所提基准和方法能有效支持多任务异构策略的GPU并行训练，提升学习效率。
...
### [228] [Grasp-Then-Plan with Failure Attribution: A Closed Two-Stage Framework for Precise and Generalizable Robotic Manipulation](https://arxiv.org/abs/2606.03385)
*Jiahao Xu,Peiyuan Wang,Hanzhuo Zhang,Zihao Yu,Tianyu Fu,Hao Chen,Xuanhao Xiang,Jianbo Yu,Chenchen Fu,Wanyuan Wang*

Main category: cs.RO

TL;DR: 提出了GTP-FA框架，通过失败归因模型解耦抓取与运动规划，在15个模拟和真实机器人任务中显著提升成功率。


<details>
  <summary>Details</summary>
Motivation: 现有抓取与运动规划紧密耦合，导致失败原因难以诊断，效率低下。

Method: 两阶段抓取-规划框架，包含失败归因模型、基于归因的抓取优化和规划微调。

Result: 在RL、IL、扩散策略和VLA等多种基线上，任务成功率大幅提升。

Conclusion: 解耦诊断和优化机制可有效提升长程操控任务的鲁棒性和效率。
...
### [229] [SPADE: Sketch-guided Path Planning Augmented with Diffusion Experts](https://arxiv.org/abs/2606.03512)
*Charbel Abi Hana,Tatiana Ghantous,Mikael Khalil,Anthony Rizk*

Main category: cs.RO

TL;DR: 本文提出了一种增强的路径规划框架，通过改进注释工具和引入扩散增强训练策略，提高了AMR的泛化能力和鲁棒性，在减少参数的同时显著降低了误差。


<details>
  <summary>Details</summary>
Motivation: 现有基于模仿学习的路径规划方法面临泛化能力差和演示数据收集鲁棒性低的问题，需要更高效的解决方案。

Method: 基于ROS 2重建注释工具，并在基线行为克隆模型中集成扩散增强训练策略。

Result: 绝对姿态误差降低39.1%，FID降低33.5%，可训练参数减少93.8%，同时保持实时性和边缘部署能力。

Conclusion: 所提框架在保持轻量级和实时性能的同时，实现了扩散模型的泛化水平，优于现有方法。
...
### [230] [PHASER: Phase-Aware and Semantic Experience Replay for Vision-Language-Action Models](https://arxiv.org/abs/2606.03598)
*Ziyang Chen,Shaoguang Wang,Weiyu Guo,Qianyi Cai,He Zhang,Pengteng Li,Yiren Zhao,Yandong Guo*

Main category: cs.RO

TL;DR: 本文提出PHASER框架，通过相位中心容量分配和多模态干扰路由策略，解决了VLA模型在持续学习中的灾难性遗忘问题，在LIBERO基准上ASR提升高达31%。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在开放环境中连续学习新技能时遭受灾难性遗忘，而传统经验回放中的均匀采样忽略操作轨迹的时间特性，导致关键子技能采样不足和历史任务遗忘程度不均。

Method: 提出PHASER框架，包括相位中心容量分配（保证所有子技能均匀记忆支持）和多模态干扰路由（动态优先高遗忘风险的历史阶段），并集成Auto-PC（无监督动作信号变化点检测与VLM语义验证）自动提取时间边界。

Result: 在三个VLA骨干网络和LIBERO持续学习套件上，PHASER相比等预算ER在ASR上最高提升31%，在LIBERO-Goal CL设置中达到87.8%的最终ASR。

Conclusion: PHASER有效缓解了VLA模型持续学习中的灾难性遗忘，实现了全自主的终身适应，验证了相位感知策略在机器人技能学习中的优势。
...
### [231] [A 3D Isovist World Model -- Revealing a City's Unseen Geometry and Its Emergent Cross-City Signature](https://arxiv.org/abs/2606.03609)
*Xuhui Lin,Stephen Law,Nanjiang Chen,Kunyao Li,Tao Yang*

Main category: cs.RO

TL;DR: 提出以3D等距视域图为预测目标的世界模型，用于具身智能体导航，模型仅需少量历史数据和动作即可预测未来可通行空间。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型预测外观或2D鸟瞰图，忽略了三维空间中的可通行几何结构（如架空层、多层建筑）。

Method: 使用球面深度残差预测下一帧等距视域图，结合自滚动计划采样训练，并维护持久性潜在鸟瞰空间图以保持跨路径一致性。

Result: 单一城市盲模型（训练于曼哈顿和巴黎）自发学习到跨城市空间特征，城市身份可从时序潜在变量中线性解码，且显著优于单帧基线。

Conclusion: 该表示轻量、可解释、可复现，为具身AI、机器人及城市分析提供了几何基础；开放的数据集和代码已发布。
...
### [232] [GN0: Toward a Unified Paradigm for Generation, Evaluation, and Policy Learning in Visual-Language Navigation](https://arxiv.org/abs/2606.03682)
...
TL;DR: 提出统一框架GN-Matrix，包括大规模数据集GN-Matrix、高保真仿真平台（基于3D高斯溅射）和BEV基准GN-Bench，以及强化学习驱动的基础模型BAE，在VLN任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有VLN系统受限于导航数据的数量和质量，导致泛化能力和长时程能力不足。

Method: 通过自动化流水线构建大规模数据集GN-Matrix；基于3DGS引擎搭建高保真仿真平台；提出BEV基准GN-Bench；开发RL驱动的导航基础模型BAE，结合监督学习、DAgger算法和下游RL探索。

Result: GN0在GN-Bench和VLN-CE评估中优于现有最先进VLN方法。

Conclusion: GN-Matrix统一了数据、仿真和学习，推动具身导航在研究和工业中的应用。
...
### [233] [Face versus Body Tracking for Human-Robot Interaction: An Egocentric Dataset](https://arxiv.org/abs/2606.03694)
*Jessica Wenninger,Gabriel Skantze*

Main category: cs.RO

TL;DR: 提出基于Furhat机器人采集的自我中心数据集，通过扩展空间记忆和外观重识别优化跟踪管道，将身份切换减少49%，揭示了标准基准在密集遮挡场景下的不足。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉模型针对监控或自动驾驶优化，难以应对社交机器人在自我中心视角下的人类弹跳、相互遮挡、出框等挑战，导致频繁身份切换中断交互。

Method: 构建Furhat机器人自我中心数据集并自定义注释；系统评估检测与跟踪分离逻辑、面部与身体跟踪对比、扩展空间记忆及外观重识别的影响。

Result: 空间记忆缓解长时间遮挡但无法处理复杂动态事件；重识别稳定身体跟踪但面部身份切换因侧面角度敏感而激增；优化管道使身份切换减少49%。

Conclusion: 标准基准缺乏密集近距遮挡数据，原生捕捉的社会动态对验证人机交互感知模型至关重要。
...
### [234] [Neural Navigation Functions for Zero-Shot Generalizable Motion Planning](https://arxiv.org/abs/2606.03756)
*Benjamin D. Shaffer,Pei-An Hsieh,Brooks Kinch,Nathaniel Trask,M. Ani Hsieh*

Main category: cs.RO

TL;DR: 提出神经导航函数（Neural-NF），一种能够在未见环境中零样本迁移的被动导航函数。


<details>
  <summary>Details</summary>
Motivation: 解决传统导航函数在新几何环境中需要重新计算且泛化性差的问题。

Method: 将数据驱动的适应性嵌入结构化椭圆规划器，利用拉普拉斯特征映射到局部PDE系数，求解边界值问题生成全局值函数。

Result: 零样本迁移性能强，相比直接预测值函数的模型提升5倍。

Conclusion: 该方法可保证无碰撞、单调下降和全局最小值，具有线性可解的最优控制解释。
...
### [235] [Revisiting Embodied Chain-of-Thought for Generalizable Robot Manipulation](https://arxiv.org/abs/2606.03784)
...
TL;DR: 本文提出ERVLA模型，利用具身思维链作为表示塑造监督而非强制测试时推理，构建最大规模数据集，在多个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有具身思维链在形式与集成策略上探索不足，且显式思维链作为自回归动作前缀存在推理错误累积与不稳定问题。

Method: 采用推理丢弃策略，训练时模型吸收推理痕迹，推理时直接预测动作；将思维链作为表示塑造监督，避免自回归不稳定。

Result: LIBERO-Plus上86.9%成功率，VLABench上53.2%成功率，真实机器人实验中性能优于基线，尤其在语义歧义与长时任务中。

Conclusion: 具身思维链应接地到具体动作指导，ERVLA通过监督式思维链集成实现更好的可扩展性与泛化性能。
...
### [236] [Worth Remembering: Surprise-Gated Robot Episodic Memory](https://arxiv.org/abs/2606.03787)
*Nicolas Gorlo,Derek K. Wise,Alberto Speranzon,Luca Carlone*

Main category: cs.RO

TL;DR: 提出基于贝叶斯惊讶的选记记忆机制，通过V-JEPA-2语义空间过滤高价值事件，在机器人问答任务中提升12%以上。


<details>
  <summary>Details</summary>
Motivation: 通用机器人需将指令与过去经验关联，但存储所有历史不可行，需要选择性保留高未来效用的记忆。

Method: 使用贝叶斯惊讶作为记忆形成的门控机制，在V-JEPA-2的语义潜在空间中计算惊讶值，实现部署无关的选记。

Result: 在机器人问答任务中，时间、空间和二元问题准确率比现有方法提升≥12%；在事件分割中，无监督因果方法超越监督和非因果方法。

Conclusion: 贝叶斯惊讶驱动的选记记忆能有效提升机器人对过去事件的利用能力，且无需预知未来任务。
...
### [237] [Optimal Design and Analytical Modeling of a Soft Fin-Ray Effect Gripper Finger Using the Finite Rigid Elements Method](https://arxiv.org/abs/2606.03798)
*Sara Adeli,Hassan Sayyaadi*

Main category: cs.RO

TL;DR: 提出了一种基于Fin Ray效应的软体抓取手指，结合有限刚体元方法建模，实现了精确力控制，适用于农业中脆弱物体的轻柔抓取。


<details>
  <summary>Details</summary>
Motivation: 针对农业中脆弱、形状不规则物体（如番茄）的轻柔处理需求，以及软体机器人非线性、高自由度等建模难题，推动精准力控制研究。

Method: 采用有限刚体元方法（FREM）建模，结合ANSYS有限元模型（FEM）进行仿真与实验验证，并基于指尖位移、总变形、应力分布和接触力四个标准优化手指设计。

Result: 最优手指参数（长30mm、肋间距10mm、7条肋以-15°角排列、肋厚1mm）下，FREM理论变形预测误差3%，ANSYS模型误差2%。

Conclusion: 提出了一种结合FREM建模的Fin Ray软体手指设计，具有精确力控制潜力，可高效应用于农业中易损产品的轻柔抓取。
...
### [238] [Let the Dynamics Flow: Stable Flow Matching Dynamical Systems](https://arxiv.org/abs/2606.03834)
*Rodrigo Pérez-Dattari,Francisco Leiva,Andrea Testa,Leonel Rozo,Javier Ruiz del Solar,Noémie Jaquier*

Main category: cs.RO

TL;DR: 本文提出SFMDS框架，将流匹配与Lyapunov稳定性结合，实现稳定、可扩展、多模态的机器人运动策略。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型缺乏形式化稳定性保证，难以确保机器人行为的安全性和泛化性。

Method: 通过流参数化动态系统，结合软约束（惩罚项）和硬约束（结构嵌入）两种方式，并扩展到李群。

Result: 在基准测试、仿真和人形机器人上，SFMDS在低维和高维状态空间中均能学习稳定、可扩展、多模态的动态系统。

Conclusion: SFMDS有效桥接了高容量生成建模与Lyapunov稳定性保证，实现了安全且富有表现力的机器人运动生成。
...
### [239] [Denoising Tells When to Replan: Denoising-Variance Adaptive Chunking for Flow-Based Robot Policies](https://arxiv.org/abs/2606.03847)
*Xiangdong Feng,Yuxuan Cheng,Chen Shi,Boyao Han,Yuxuan Yan,Yitong Hong,Zhuotao Tian,Li Jiang*

Main category: cs.RO

TL;DR: 提出DVAC方法，根据去噪方差自适应调整动作执行长度，提升机器人操作成功率并减少重规划频率。


<details>
  <summary>Details</summary>
Motivation: 固定动作块长度的机器人策略忽略了不同任务阶段（如自由运动与精密交互）对重规划频率的不同需求。

Method: 通过测量流策略最后去噪步的干净动作估计方差，执行稳定低方差的前缀动作，在高方差动作前重规划，并用滚动估计校准阈值。

Result: 在LIBERO、RoboTwin、CALVIN及真实世界实验中，任务成功率提升（如LIBERO从94.75%到98.00%），重规划减少43.0%。

Conclusion: 基于去噪方差的适应性分块能有效平衡执行稳定性与重规划效率，提升策略整体性能。
...
### [240] [Semantic-weighted ICP for LiDAR Odometry: Class-Aware Residual Reweighting for Robust Scan Registration](https://arxiv.org/abs/2606.03905)
*Vasco Carvalho,Tiago Barros,Urbano J. Nunes*

Main category: cs.RO

TL;DR: 提出语义加权ICP方法，根据语义类别的几何稳定性动态调整点云配准权重，在动态和复杂环境中提升激光雷达里程计精度。


<details>
  <summary>Details</summary>
Motivation: 传统几何配准方法在动态或非结构化环境（如移动物体、稀疏几何特征、植被等）中因不可靠对应关系而性能下降。

Method: 采用加权ICP算法，对语义分割结果中不同类别的点云残差赋予权重，权重基于其预期几何稳定性，而非严格过滤。

Result: 在SemanticKITTI和RELLIS-3D数据集上验证，尤其在越野等挑战性场景中，位姿估计精度显著提升。

Conclusion: 语义加权策略的有效性高度依赖环境场景的结构和语义组成，提示需要场景自适应权重设计。
...
### [241] [PointAction: 3D Points as Universal Action Representations for Robot Control](https://arxiv.org/abs/2606.03943)
*Mutian Tong,Han Jiang,Qiao Feng,Lingjie Liu,Jiatao Gu*

Main category: cs.RO

TL;DR: PointAction通过预测动态3D点图将视频生成与机器人动作连接，实现跨任务和本体的泛化操作。


<details>
  <summary>Details</summary>
Motivation: 现有视频动作模型仅预测RGB帧，导致度量3D运动、接触几何等空间约束不明确，且动作标注成本高。

Method: 微调视频扩散模型联合预测RGB帧和动态3D点图，构建点动力学作为动作接口，再通过扩散解码器映射为机器人动作。

Result: 在机器人场景4D生成质量上达到SOTA，仿真中超越基线，且泛化到两个未预训练的真实机械臂。

Conclusion: 显式4D点模型有效降低了RGB动作歧义，支持有限标注下的跨任务和本体迁移。
...
### [242] [Preference-Calibrated Human-in-the-Loop Reinforcement Learning for Robotic Manipulation](https://arxiv.org/abs/2606.03949)
*Zeyi Liu,Guangyao Liu,Yinuo Qu,Yuquan Xue,Bofang Jia,Chunhua Yang,Weihua Gui,Keke Huang,Ziwei Wang*

Main category: cs.RO

TL;DR: 提出了PACT框架，通过干预信号修正次优轨迹的信用分配，提升机器人操作任务的样本效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有HIL-RL方法对次优轨迹统一分配折扣终端奖励，导致Q值高估和策略更新偏差。

Method: 设计进度模型识别次优片段；构建反事实优势调整贝尔曼目标；在边界均值空间对齐策略与人类修正动作。

Result: 在5个真实机器人任务上，平均成功率提升24.5%，收敛速度加快1.3倍。

Conclusion: PACT通过偏好校准的信用重分配和直接策略对齐，有效改善了HIL-RL的性能和样本效率。
...
### [243] [Self-Refining Agentic Reinforcement Learning for Vision-Conditioned UAV Navigation](https://arxiv.org/abs/2606.03963)
*Roohan Ahmed Khan,Yasheerah Yaqoot,Muhammad Ahsan Mustafa,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 提出了AgenticRL框架，利用GPT智能体自动设计奖励函数、训练策略并闭环优化，在无人机导航任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度强化学习依赖人工设计奖励函数和反复调参，耗时长且成功率低，需要更自主的框架。

Method: 使用多模态GPT智能体解析任务与视觉信息，生成奖励函数；采用PPO训练策略；智能体作为批评者诊断失败模式，闭环优化奖励函数；推理时根据场景自动选择策略。

Result: 闭环优化使策略行为提升71%；实际部署成功率达91%，sim-to-real准确率94%。

Conclusion: AgenticRL通过智能体引导的闭环自改进，提升了无人机导航的自主性和实际部署效果。
...
### [244] [Humanoid-GPT: Scaling Data and Structure for Zero-Shot Motion Tracking](https://arxiv.org/abs/2606.03985)
...
TL;DR: 提出了Humanoid-GPT，一种基于GPT风格的因果Transformer，在十亿级运动语料上预训练，用于全身控制。


<details>
  <summary>Details</summary>
Motivation: 现有浅层MLP跟踪器受限于数据稀缺和敏捷性-泛化权衡，需要大规模数据和模型容量提升。

Method: 使用因果Transformer架构，在20亿帧重定向运动语料（统一主要动作捕捉数据集和室内录制）上预训练。

Result: 单个生成式Transformer能跟踪高度动态行为，并在未见过的动作和控制任务上实现零样本泛化，建立新性能前沿。

Conclusion: 通过扩展数据和模型容量，模型在零样本泛化到未见任务的同时，能跟踪高度动态复杂动作。
...
### [245] [Unstable Poles Arising in AC Power Grid Subsystem Representations](https://arxiv.org/abs/2606.03514)
*Liam Hallinan,Ioannis Lestas*

Main category: eess.SY

TL;DR: 论文比较了电力系统稳定性分析中两种子系统模型(PQ和IV)，发现它们通过环路变换相关联，并指出每种模型在不同条件下可能出现不稳定极点，即使整体系统稳定。


<details>
  <summary>Details</summary>
Motivation: 现有小信号稳定性研究常使用PQ或IV模型，但缺乏对两者关系及潜在不稳定性的深入分析，需明确模型选择的风险。

Method: 推导两种模型（不简化母线或线路动态），通过环路变换建立联系，并分析极点稳定性与运行点、高频动态的关系。

Result: IV模型的不稳定极点主要取决于运行点，PQ模型则受高频无源动态影响；这些不稳定极点可能在子系统出现而整体系统稳定。

Conclusion: 选择子系统模型需谨慎，忽略高频动态或运行点可能掩盖不稳定极点，需在网络互联中考虑并纳入系统辨识。
...
### [246] [Sparse-View Lung Nodule Volumetry from Digitally Reconstructed Radiographs via AReT: Anatomy-Regularized TensoRF](https://arxiv.org/abs/2606.02639)
*Spoorthi M,Suja Palaniswamy*

Main category: eess.IV

TL;DR: 本文发现TensoRF在X射线衰减场中默认密度偏移导致梯度抑制，将其归零后实现三正交投影下的肺结节三维重建，并提出结合解剖先验的AReT框架，在11种策略中表现最优。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF方法需密集多视角采集，不适用于稀疏视角的医学影像重建，且TensoRF默认密度偏移在X射线衰减场中导致训练失败。

Method: 将密度偏移设为0恢复梯度流；构建AReT框架，使用张量辐射场（TensoRF）从三个正交投影（冠状面、矢状面、横断面）重建，并引入基于胸部解剖的正则化（L1稀疏性与全变分平滑联合）。

Result: 在临床结节（>=10mm）上Pearson r=0.983，体积误差中位数11.4%，近乎零偏差（-77.3mm³），比球近似法提升8.4倍。

Conclusion: 解剖先验正则化优于生成先验方法，AReT在稀疏视角下实现高精度肺结节体积重建，且系统性偏差极小。
