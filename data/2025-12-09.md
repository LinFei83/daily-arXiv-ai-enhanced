...
TL;DR: 本研究测试了一种将LLM评估任务框架为“下注游戏”的方法，发现这能有效地生成校准的置信度信号，即使对预测准确率的提升有限。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）越来越多地被用于评估其他模型，但这些评估通常缺乏对置信度的表示。

Method: 研究生成了100个可验证答案的数学和逻辑问题。六个基线模型回答了这些问题。三个预测模型随后预测基线模型是否会正确回答每个问题。每个预测模型在两种条件下运行：对照组（简单的正确/错误预测）和激励组（预测加上1-100,000 LLMCoin的赌注）。通过跟踪预测准确率和赌注大小来评估效果。

Result: 激励组的准确率略高于对照组（81.5% vs. 79.1%，p = .089），并且学习速度显著加快。最重要的是，赌注大小与置信度高度相关：大额赌注（40,000+ 硬币）的正确率约为99%，而小额赌注（<1,000 硬币）的正确率仅为74%左右。

Conclusion: 下注机制为LLMs创建了一个清晰的置信度信号，使其内部信念可见且可用，从而可能将LLMs转变为具有风险意识的预测器。该协议为元评估系统和LLM到LLM预测市场的未来工作奠定了基础。
...
TL;DR: 本研究提出了一种透明且可解释的机器学习方法，利用BioBERT分析非结构化临床文本，用于自闭症谱系障碍（ASD）诊断。该方法在混合数据集训练下表现最佳（97%敏感性，98%特异性），并优于黑盒模型，为可信赖、可泛化且临床可操作的AI工具铺平了道路。


<details>
  <summary>Details</summary>
Motivation: 自闭症谱系障碍（ASD）诊断过程漫长且需求日益增长。尽管机器学习在自动化诊断方面有潜力，但现有模型多为黑盒且通常在单一数据集上训练，限制了其泛化能力。因此，需要开发更透明、可解释且泛化能力强的AI诊断工具。

Method: 本研究引入了一种透明且可解释的机器学习方法，该方法利用BioBERT语言模型分析非结构化临床文本。模型被训练用于标注行为描述并将其映射到诊断标准，最终分配ASD或非ASD标签。研究使用两个不同的真实世界数据集评估了迁移学习能力，并比较了顺序训练和混合训练策略。同时，也创建了一个黑盒方法进行对比评估。

Result: 透明模型表现出强大的性能，其中混合数据训练策略取得了最佳结果（97%敏感性，98%特异性）。跨数据集的顺序训练导致性能略有下降，强调了训练数据顺序的重要性。黑盒模型在顺序训练或混合数据训练下表现均较差（90%敏感性，96%特异性）。总体而言，透明方法优于黑盒方法，且混合数据集训练略微提升了性能，应作为首选策略。

Conclusion: 本研究证明了利用BioBERT的透明、可解释机器学习方法在自闭症谱系障碍诊断中的有效性。这种方法提供了更可信赖、可泛化且临床可操作的神经发育诊断AI工具。建议在实际可行时采用混合数据集训练以获得最佳性能。
...
TL;DR: ARCANE框架通过动态生成的自然语言评分标准，为大型语言模型智能体提供可解释且在测试时自适应的对齐能力，无需重新训练即可调整偏好。


<details>
  <summary>Details</summary>
Motivation: 随着基于大型语言模型的智能体被部署到长期任务中，保持其与利益相关者偏好的一致性变得至关重要。这要求奖励模型是可解释的，以便利益相关者能够理解和审计模型目标；同时，奖励模型必须能够在交互时指导智能体，并在不重新训练的情况下纳入偏好变化。

Method: 本文提出了ARCANE框架，将对齐问题视为一个多智能体协作问题。该框架将利益相关者偏好动态表示为自然语言评分标准（可验证标准的加权集合，可根据任务上下文即时生成）。受效用理论启发，评分标准学习被表述为一个重建问题，并应用了正则化群序列策略优化（GSPO）过程，以平衡可解释性、忠实性和计算效率。

Result: 使用来自GDPVal基准的219个带标签评分标准语料库，ARCANE在需要多步骤推理和工具使用的挑战性任务上进行了评估。结果表明，学习到的评分标准能产生紧凑、易读的评估，并能在不重新训练的情况下实现可配置的权衡（例如，正确性与简洁性）。

Conclusion: 基于评分标准的奖励模型为复杂、长期的人工智能系统提供了可解释且在测试时自适应的对齐方法，展现了广阔前景。
...
TL;DR: 本文提出了一种名为PROBE的新型知识图谱补全（KGC）评估框架，旨在解决现有评估指标忽视预测锐度（predictive sharpness）和流行度偏差鲁棒性（popularity-bias robustness）的问题，从而提供更全面和可靠的KGC模型评估。


<details>
  <summary>Details</summary>
Motivation: 现有的知识图谱补全（KGC）评估指标存在不足，它们忽视了两个关键视角：(A1) 预测锐度，即评估单个预测的严格程度；(A2) 流行度偏差鲁棒性，即预测低流行度实体的能力。因此，需要一个能反映这两个视角的更完善的评估方法。

Method: 本文提出了一种新颖的评估框架PROBE。该框架包含两部分：一个秩转换器（rank transformer, RT），用于根据所需的预测锐度水平估计每个预测的分数；以及一个秩聚合器（rank aggregator, RA），以流行度感知的方式聚合所有分数。

Result: 在真实世界知识图谱上的实验表明，现有指标倾向于高估或低估KGC模型的准确性，而PROBE框架能够对KGC模型产生全面的理解和可靠的评估结果。

Conclusion: PROBE框架通过引入预测锐度和流行度偏差鲁棒性两个关键视角，提供了一种更全面、更可靠的知识图谱补全模型评估方法，解决了现有指标的不足。
...
TL;DR: 本文提出了一种名为DaGRPO的新型优化算法，通过序列级梯度修正和离策略数据增强，解决了现有GRPO在长程推理中训练不稳定和样本效率低的问题，显著提升了LLM在数学推理和OOD泛化任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管GRPO在激发大型语言模型(LLM)的后训练推理能力方面表现出色，但其存在显著的训练不稳定性和较差的样本效率问题。理论分析表明，这些问题源于在策略采样中缺乏区分度，即常规查询样本同质性高导致梯度冲突，而困难查询缺乏有效正样本导致优化无效。

Method: 本文提出了区分度感知群组相对策略优化(DaGRPO)。它包含两个核心机制：1) 序列级梯度修正：利用细粒度评分动态掩蔽区分度低的样本对，从源头消除梯度冲突。2) 离策略数据增强：引入高质量锚点，为挑战性任务恢复训练信号。

Result: 在9个数学推理和分布外(OOD)泛化基准上的广泛实验表明，DaGRPO显著超越了现有的SFT、GRPO和混合基线，实现了新的最先进性能（例如，在数学基准上平均准确率提高了+4.7%）。深入分析证实，DaGRPO有效缓解了梯度爆炸并加速了长链推理能力的出现。

Conclusion: DaGRPO通过解决GRPO的训练不稳定性和样本效率问题，显著提升了LLM在复杂推理任务上的性能和泛化能力，为长程推理的优化提供了一个更稳定和高效的解决方案。
...
TL;DR: 大型语言模型对语义保持的逻辑转换具有鲁棒性，但对缺失或冲突证据的逻辑推理表现出脆弱性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言任务中表现出色，但其在逻辑语境下对结构扰动的泛化能力仍不清楚。本研究旨在探究其推理可靠性。

Method: 引入了受控评估框架，包含四个针对性压力测试：1) 规则删除（冗余或必要）；2) 矛盾证据注入；3) 逻辑保持的重写（基于多种等价定律）；4) 多定律等价堆叠。实验在BERT、Qwen2和LLaMA类模型上进行。

Result: 所有模型在基础任务上均达到完美准确率，并能完全泛化冗余规则删除和所有基于等价的重写。但在必要规则删除下准确率急剧下降至25%，在存在明确矛盾的情况下完全崩溃（0%准确率）。

Conclusion: 大型语言模型对语义保持的逻辑转换具有稳定的不变性，但对缺失或冲突的证据仍然本质上脆弱。本框架提供了一个诊断工具，用于隔离推理失败模式，并突出了当前大型语言模型在逻辑泛化能力上的持续差距。
...
TL;DR: GENIUS是一个AI代理工作流，它结合了知识图谱和分层LLM，自动化DFT模拟的输入文件生成、验证和修复，显著提高了模拟成功率并降低了成本，从而使非专业人士也能进行电子结构模拟。


<details>
  <summary>Details</summary>
Motivation: 尽管预测性原子模拟推动了材料发现，但其设置和调试仍需计算机专家，这种知识鸿沟限制了集成计算材料工程（ICME）的应用，因为现有先进代码对非专家而言过于繁琐。

Method: 该研究提出了GENIUS，一个AI代理工作流。它将智能的Quantum ESPRESSO知识图谱与分层的大型语言模型（LLM）体系相结合，并由一个有限状态错误恢复机进行监督。

Result: GENIUS能将人类的自由形式提示转化为经过验证的输入文件，在295个多样化基准测试中，约80%的任务成功运行，其中76%是自主修复的。其成功率随复杂性呈指数衰减至7%的基线。与仅使用LLM的基线相比，GENIUS将推理成本减半，并几乎消除了幻觉。

Conclusion: 该框架通过智能自动化协议的生成、验证和修复，普及了电子结构DFT模拟，为全球学术界和工业界的大规模筛选和加速ICME设计循环开辟了道路。
...
TL;DR: 本文引入了UncertaintyZoo，一个统一的工具包，集成了29种不确定性量化（UQ）方法，旨在评估大型语言模型（LLMs）的预测不确定性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在实际应用中表现出色，但其预测错误可能在安全关键场景中导致严重后果。虽然已提出多种不确定性量化标准来衡量模型置信度，但缺乏集成这些方法的工具，这阻碍了UQ方法的实际应用和未来研究。

Method: 开发了UncertaintyZoo工具包，它集成了29种不确定性量化方法，涵盖五大类，并提供标准化接口。利用UncertaintyZoo，在代码漏洞检测任务中，评估了CodeBERT和ChatGLM3模型上现有UQ方法的有效性。

Result: 评估结果表明，UncertaintyZoo能够有效揭示模型的预测不确定性。

Conclusion: UncertaintyZoo成功弥补了现有UQ方法集成工具的空白，为不确定性量化方法的实际应用和未来研究提供了统一的平台。
...
### [11] [Utilizing Multi-Agent Reinforcement Learning with Encoder-Decoder Architecture Agents to Identify Optimal Resection Location in Glioblastoma Multiforme Patients](https://arxiv.org/abs/2512.06990)
...
### [12] [Smart Spatial Planning in Egypt: An Algorithm-Driven Approach to Public Service Evaluation in Qena City](https://arxiv.org/abs/2512.06431)
...
### [13] [The Effect of Belief Boxes and Open-mindedness on Persuasion](https://arxiv.org/abs/2512.06573)
...
TL;DR: LightSearcher是一个高效的强化学习框架，通过引入文本经验记忆和自适应奖励整形机制，有效平衡了DeepSearch范式中的准确性与效率权衡，在多跳问答任务中实现了与SOTA基线相当的准确性，同时显著减少了工具调用、推理时间和token消耗。


<details>
  <summary>Details</summary>
Motivation: 基于强化学习的DeepSearch系统在准确性和效率之间存在权衡：频繁调用外部搜索工具可以提高事实准确性，但会导致不必要的计算开销并降低效率。

Method: 本文提出了LightSearcher，一个高效的强化学习框架。它通过学习对比推理轨迹来生成成功推理模式的可解释摘要，从而整合了文本经验记忆。此外，它采用了一种自适应奖励整形机制，仅在答案正确的情况下惩罚冗余的工具调用。

Result: 在四个多跳问答基准测试中，LightSearcher在保持与SOTA基线ReSearch相当的准确性的同时，将搜索工具调用次数减少了39.6%，推理时间减少了48.6%，token消耗减少了21.2%，展示了其卓越的效率。

Conclusion: LightSearcher通过其创新的设计，成功地平衡了DeepSearch范式固有的准确性-效率权衡，在不牺牲准确性的前提下显著提升了效率。
...
### [15] [Stochasticity in Agentic Evaluations: Quantifying Inconsistency with Intraclass Correlation](https://arxiv.org/abs/2512.06710)
*Zairah Mustahsan,Abel Lim,Megna Anand,Saahil Jain,Bryan McCann*

Main category: cs.AI

TL;DR: 当前大型语言模型（LLM）评估方法因未报告结果方差而不可靠。本研究提出采用组内相关系数（ICC）来量化这种方差，从而区分模型能力的真实提升与测量噪声，并建议将其作为评估标准。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为代理系统（agentic systems）的组成部分，评估的可靠性变得至关重要。当前评估实践仅报告单次运行的单一准确率，掩盖了结果背后的方差，使得难以区分真正的能力提升和随机抽样带来的运气成分。

Method: 研究提出采用测量科学中的组内相关系数（ICC）来表征评估方差。ICC将观察到的方差分解为查询间方差（任务难度）和查询内方差（代理不一致性）。研究在GAIA（衡量不同推理复杂度的代理能力）和FRAMES（衡量跨文档检索和事实性）数据集上进行了评估，并分析了ICC的收敛性。

Result: 研究发现ICC随任务结构变化显著：推理和检索任务（FRAMES）在不同模型上ICC为0.4955-0.7118，而代理任务（GAIA）在不同模型上ICC为0.304-0.774。对于代理系统中的子代理替换决策，准确率的提升只有在ICC也提升时才值得信赖。研究还表明，对于结构化任务，ICC在n=8-16次试验后收敛；对于复杂推理任务，则需要n>=32次试验。

Conclusion: 本研究建议将准确率与ICC和查询内方差一起报告作为标准实践，并提出了更新的评估卡片。通过使评估稳定性可见，旨在将代理基准测试从不透明的排行榜竞争转变为可信的实验科学。
...
### [16] [Academic journals' AI policies fail to curb the surge in AI-assisted academic writing](https://arxiv.org/abs/2512.06705)
*Yongyuan He,Yi Bu*

Main category: cs.AI

TL;DR: 尽管期刊广泛采纳AI使用政策（主要要求披露），但研究人员对AI写作工具的使用显著增加，且政策未能促进透明度或限制AI采用。


<details>
  <summary>Details</summary>
Motivation: 生成式AI迅速融入学术写作，促使期刊和出版商出台大量政策，但这些政策的有效性尚不明确。

Method: 分析了5,114本期刊和超过520万篇论文，评估AI使用指南的实际影响。对16.4万篇科学出版物进行全文分析，以揭示AI使用披露情况。

Result: 70%的期刊采纳了AI政策，但研究人员对AI写作工具的使用显著增加，无论是否有政策。非英语国家、物理科学和高开放获取期刊增长率最高。2023年以来发表的7.5万篇论文中，仅有76篇（0.1%）明确披露了AI使用。

Conclusion: 目前的政策未能有效促进透明度或限制AI的采用。需要重新评估伦理框架，以促进科学领域AI的负责任整合。
...
TL;DR: 自主大型语言模型（LLM）代理易受间接提示注入（IPI）攻击，现有防御机制存在碎片化问题。本文提出认知控制架构（CCA），通过“意图图”进行前瞻性完整性强制和“分层裁决器”进行深度推理，实现全生命周期认知监督，有效抵御复杂IPI攻击，同时保持效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自主LLM代理极易受到间接提示注入（IPI）攻击，这些攻击通过污染外部信息源来劫持代理行为，导致恶意工具调用。现有防御机制在安全性和功能性之间存在根本性权衡，且架构碎片化，无法提供整个任务执行管道的完整性保证，导致安全、功能和效率之间不可接受的多维度妥协。

Method: 核心洞察是：无论IPI攻击多么微妙，其恶意目标最终都会表现为行动轨迹中可检测到的偏差。基于此，提出认知控制架构（CCA），这是一个实现全生命周期认知监督的整体框架。CCA通过两个协同支柱构建高效的双层防御系统：(i) 通过预生成的“意图图”主动和先发性地强制控制流和数据流完整性；(ii) 一个创新的“分层裁决器”，在检测到偏差时，基于多维度评分启动深度推理，专门设计用于对抗复杂的条件攻击。

Result: 在AgentDojo基准测试上的实验证明，CCA不仅能有效抵御挑战其他先进防御方法的复杂攻击，而且在不妥协安全性的前提下，实现了显著的效率和鲁棒性，从而解决了前述的多维度权衡问题。

Conclusion: CCA提供了一个全面的、全生命周期认知监督框架，通过其双层防御机制（意图图和分层裁决器），成功地抵御了复杂的间接提示注入攻击。它有效地平衡了安全、功能和效率之间的多维度权衡，为自主LLM代理提供了未受损害的安全保障。
...
TL;DR: ProAgent是首个端到端的主动式LLM代理系统，它利用大规模感官上下文和LLM推理，通过分层感知和上下文感知推理，在AR眼镜上提供主动协助，并在预测准确性、工具调用F1分数和用户满意度方面显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型（LLM）代理主要遵循被动范式，依赖用户明确的指令来启动服务，这增加了用户的物理和认知负担。

Method: ProAgent首先采用面向主动的上下文提取方法，通过按需分层感知持续感知环境，并提取包含感官和个性线索的层次上下文。随后，它采用上下文感知主动推理器，将这些上下文映射到用户需求和工具调用，从而提供主动协助。该系统在增强现实（AR）眼镜和边缘服务器上实现，并在真实世界测试平台、公共数据集和用户研究中进行了广泛评估。

Result: ProAgent实现了高达33.4%的主动预测准确率提升，16.8%的工具调用F1分数提升，并在用户满意度方面相对于最先进的基线有显著改善。

Conclusion: ProAgent在主动式助理方面迈出了重要一步，通过其创新的主动感知和推理能力，显著提升了预测准确性、工具调用性能和用户满意度。
...
### [19] [FlatFormer: A Flat Transformer Knowledge Tracing Model Based on Cognitive Bias Injection](https://arxiv.org/abs/2512.06629)
...
### [21] [DoVer: Intervention-Driven Auto Debugging for LLM Multi-Agent Systems](https://arxiv.org/abs/2512.06749)
...
### [22] [JT-DA: Enhancing Data Analysis with Tool-Integrated Table Reasoning Large Language Models](https://arxiv.org/abs/2512.06859)
...
### [23] [Do Persona-Infused LLMs Affect Performance in a Strategic Reasoning Game?](https://arxiv.org/abs/2512.06867)
...
### [24] [On Memory: A comparison of memory mechanisms in world models](https://arxiv.org/abs/2512.06983)
...
TL;DR: 本文提出并验证了一个包含9个类别的ARC任务分类法，用于诊断Transformer在ARC任务上的局限性，揭示了“组合性鸿沟”和“神经亲和力天花板效应”，并指出未来需要混合架构。


<details>
  <summary>Details</summary>
Motivation: 为响应Hodel等人对ARC中任务相关性形式化定义的需求，并理解当前AI模型（特别是Transformer）在ARC任务上的表现限制。

Method: 1. 提出了一个包含9个类别的400个ARC任务分类法，并通过基于规则的代码分析（97.5%准确率）进行验证。2. 训练CNN模型在原始网格像素上证明分类法的视觉一致性。3. 将分类法应用于ARC-AGI-2测试集进行诊断性课程分析。4. 微调一个1.7M参数的Transformer模型，以探究其对任务的适应性。5. 将该框架应用于Li等人独立的ViTARC研究，以确认其预测能力。

Result: 1. 该9类别分类法通过规则分析验证，准确率达97.5%。2. CNN在原始像素上训练，证明了分类法的视觉一致性（S3上95.24%，总体36.25%）。3. 35.3%的任务对Transformer表现出较低的神经亲和力。4. 发现了“组合性鸿沟”：在302个任务中，有210个（69.5%）任务在单元格准确率上超过80%，但在网格准确率上低于10%。5. 提供了“神经亲和力天花板效应”的直接证据，表明性能受限于架构而非课程。6. 验证了分类法的预测能力：极低亲和力任务准确率为51.9%，高亲和力任务为77.7%（p<0.001）。

Conclusion: 研究结果表明，当前的AI模型（如Transformer）在处理ARC任务时存在固有的架构限制，尤其是在全局合成能力上。未来的进展需要采用具有亲和力对齐模块的混合架构。本文发布的经过验证的分类法可用于精确诊断任务特性。
...
TL;DR: 该研究提出了一个Python包，通过将SHAP与大型语言模型（如GPT）集成，为机器学习模型生成更具上下文的文本解释，以提高非技术用户的理解度。


<details>
  <summary>Details</summary>
Motivation: 尽管SHAP能有效可视化特征重要性，但它常常缺乏对终端用户有意义的上下文解释，特别是对于没有技术背景的用户。因此，需要弥补这一差距，提供更易理解的解释。

Method: 研究开发了一个Python包，将SHAP与OpenAI的GPT等大型语言模型集成，通过用户定义的参数（如特征别名、描述和背景信息）生成情境化的文本解释。该方法在一个医疗保健案例研究中进行了应用，并通过李克特量表调查和后续访谈对真实终端用户进行了评估。

Result: 用户评估结果表明，与仅有视觉输出相比，生成的解释被认为更易于理解且更具上下文适宜性。

Conclusion: 初步研究结果表明，将可视化与情境化文本相结合，可能有助于提供更用户友好和值得信赖的模型解释。
...
TL;DR: 本文提出了一种名为PICKT的知识追踪模型，旨在克服现有模型在数据格式限制、冷启动问题和实际服务稳定性方面的不足，通过有效处理多类型输入数据和构建知识图谱来提高预测学生知识水平的准确性和实用性。


<details>
  <summary>Details</summary>
Motivation: 随着个性化学习的兴起，智能辅导系统（ITS）对准确追踪学生知识状态并提供定制化学习路径的需求日益增加。然而，现有的知识追踪（KT）模型存在输入数据格式受限、新学生或新问题加入时的冷启动问题以及在实际服务环境中稳定性不足等局限性。

Method: 本文提出了一种实用型互联概念知识追踪（PICKT）模型。该模型能够有效处理多种类型的输入数据。具体而言，它通过考虑问题和概念的文本信息来构建知识图谱，从而在冷启动情况下也能实现有效的知识追踪。

Result: 实验结果表明，PICKT模型在反映实际操作环境的测试中展现出卓越的性能和实用性。相较于现有模型，它在处理新学生注册和新问题添加这两种核心冷启动挑战时取得了显著的性能提升。此外，通过精细的实验设计，验证了模型的稳定性和实用性。

Conclusion: PICKT模型通过有效利用多样化数据格式、显著改善冷启动问题以及验证其稳定性和实用性，为下一代智能辅导系统的实际部署提供了关键的理论和技术基础。
...
TL;DR: 本文提出了一种两阶段管道用于eBay和Amazon的产品匹配，其中精细过滤阶段采用了一种名为RAEA（Relation-aware and Attribute-aware Graph Attention Networks for Entity Alignment）的新型实体对齐框架，该框架专注于属性三元组和关系三元组之间的交互，并在跨语言和单语言数据集上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有的实体对齐（EA）方法未能充分利用属性三元组和关系三元组，尤其是它们之间的交互，这限制了产品匹配的准确性。

Method: 本文采用两阶段管道进行产品匹配：粗过滤和精细过滤。精细过滤阶段引入RAEA框架，通过属性感知实体编码器和关系感知图注意力网络，聚合来自属性和关系的对齐信号，以捕捉属性三元组和关系三元组之间的交互来表示实体。

Result: 实验结果表明，RAEA模型在跨语言数据集DBP15K上比12个基线模型平均提升了6.59%的Hits@1，并在单语言数据集DWY100K上取得了有竞争力的结果。

Conclusion: RAEA模型通过有效利用属性三元组和关系三元组之间的交互，显著提高了实体对齐的性能，为产品匹配任务提供了更准确的解决方案。
...
### [32] [M-STAR: Multi-Scale Spatiotemporal Autoregression for Human Mobility Modeling](https://arxiv.org/abs/2512.07314)
...
### [33] [A Geometric Unification of Concept Learning with Concept Cones](https://arxiv.org/abs/2512.07355)
...
### [34] [How Do LLMs Fail In Agentic Scenarios? A Qualitative Analysis of Success and Failure Scenarios of Various LLMs in Agentic Simulations](https://arxiv.org/abs/2512.07497)
*JV Roig*
...
### [43] [Video Models Start to Solve Chess, Maze, Sudoku, Mental Rotation, and Raven' Matrices](https://arxiv.org/abs/2512.05969)
*Hokin Deng*

Main category: cs.CV

TL;DR: 研究表明视频生成模型（如Sora-2）已具备推理能力，在棋类、迷宫、数独等任务上成功率达60%。为此，提出了一种“任务对”实验范式和可扩展的代码框架（VMEvalKit），并验证了其自动化评估与人类判断的高度相关性，为未来通过强化学习提升视频模型的推理能力提供了可能。


<details>
  <summary>Details</summary>
Motivation: 探索并证明视频生成模型是否具备推理能力，以及如何有效评估和提升这种能力。

Method: 1. 在国际象棋、迷宫、数独、心理旋转和瑞文推理测验等任务上测试了Sora-2等领先的视频生成模型。2. 建立了一个以“任务对”设计为中心的鲁棒实验范式。3. 构建了一个包含39个模型的代码框架（VMEvalKit），支持该范式并允许用户高效添加模型和任务。4. 采用自动化评估方法，并验证其与人类判断的高度相关性。

Result: 1. 领先的视频生成模型（如Sora-2）在推理任务上取得了约60%的成功率。2. 所建立的“任务对”实验范式和VMEvalKit代码框架被证实是鲁棒且可扩展的。3. 自动化评估结果与人类判断高度相关，表明该范式具有高可扩展性。

Conclusion: 视频生成模型现在能够进行推理。所提出的实验范式和代码框架为评估和通过强化学习改进视频模型的推理能力提供了宝贵的机会和工具。
...
### [44] [Adaptive Dataset Quantization: A New Direction for Dataset Pruning](https://arxiv.org/abs/2512.05987)
*Chenyue Yu,Jianyu Yu*

Main category: cs.CV

TL;DR: 本文提出一种新颖的数据集量化方法，通过减少样本内部冗余来降低大型数据集在资源受限边缘设备上的存储和通信成本，同时保持模型训练性能。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的边缘设备上，大型数据集的存储和通信成本是巨大的挑战。传统方法侧重于样本间冗余，而本文旨在解决样本内部的冗余问题。

Method: 该方法首先对每个样本应用线性对称量化以获取初始量化范围和尺度。接着，引入一种自适应量化分配算法，根据不同样本的精度需求分配不同的量化比率，同时保持总压缩比不变。其核心是减少样本内部冗余，而非样本间冗余。

Result: 实验结果表明，该方法在CIFAR-10、CIFAR-100和ImageNet-1K数据集上，在显著压缩数据集的同时，能有效保持模型训练性能。在相同压缩比下，其表现优于传统的量化和数据集剪枝基线方法。

Conclusion: 该研究首次使用有限比特表示数据集以减少存储，并引入了带有自适应比率分配的数据集级量化算法，有效解决了边缘设备上大数据集的存储和通信挑战，证明了其在维持模型性能的同时实现高效数据集压缩的有效性。
...
### [45] [VG3T: Visual Geometry Grounded Gaussian Transformer](https://arxiv.org/abs/2512.05988)
*Junho Kim,Seongwon Lee*

Main category: cs.CV

TL;DR: 本文提出了VG3T，一个新颖的多视角前馈网络，通过3D高斯表示预测3D语义占据，克服了多视角融合中的碎片化问题，实现了更高的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 从多视角图像生成连贯的3D场景表示是一项基本但具有挑战性的任务。现有方法在多视角融合上表现不佳，导致3D表示碎片化和次优性能。

Method: VG3T直接以联合多视角的方式预测一组具有语义属性的高斯，而不是从单视角推断。它还引入了基于网格的采样（Grid-Based Sampling）和位置精炼（Positional Refinement）两个关键组件，以减轻像素对齐高斯初始化方法中常见的距离依赖密度偏差。

Result: VG3T在nuScenes基准测试上实现了1.7%p的mIoU改进，同时使用的基元数量比现有最先进方法减少了46%，突显了其卓越的效率和性能。

Conclusion: VG3T提供了一个统一的范式来表示几何和语义，克服了逐视角处理固有的碎片化和不一致性，并在效率和性能上达到了新的SOTA。
...
### [46] [EmoDiffTalk:Emotion-aware Diffusion for Editable 3D Gaussian Talking Head](https://arxiv.org/abs/2512.05991)
*Chang Liu,Tianjiao Jing,Chengcheng Ma,Xuanqi Zhou,Zhengxuan Lian,Qin Jin,Hongliang Yuan,Shi-Sheng Huang*

Main category: cs.CV

TL;DR: 本文提出 EmoDiffTalk，一种可编辑的3D高斯对话头，通过情感感知高斯扩散和文本到AU情感控制器，实现精细且广泛的多模态情感表达操控。


<details>
  <summary>Details</summary>
Motivation: 现有的基于3D高斯泼溅的逼真3D对话头在情感表达操控方面存在显著不足，尤其是在使用多模态控制进行细粒度和广泛动态情感编辑方面。

Method: 引入 EmoDiffTalk，核心是情感感知高斯扩散（Emotion-aware Gaussian Diffusion），包括用于细粒度面部动画的动作单元（AU）提示高斯扩散过程，以及提供准确文本输入进行广泛动态情感编辑的文本到AU情感控制器。

Result: 在公开的 EmoTalk3D 和 RenderMe-360 数据集上的实验表明，EmoDiffTalk 在情感微妙性、唇语同步保真度和可控性方面优于现有工作。它是首批支持在AU表达空间内进行连续、多模态情感编辑的3D高斯泼溅对话头生成框架之一。

Conclusion: EmoDiffTalk 为高质量、扩散驱动、多模态可编辑的3D对话头合成开辟了一条原则性路径，尤其支持基于AU表达空间的连续、多模态情感编辑。
...
### [47] [Domain-Specific Foundation Model Improves AI-Based Analysis of Neuropathology](https://arxiv.org/abs/2512.05993)
...
### [48] [FishDetector-R1: Unified MLLM-Based Framework with Reinforcement Fine-Tuning for Weakly Supervised Fish Detection, Segmentation, and Counting](https://arxiv.org/abs/2512.05996)
...
### [49] [Multi-Modal Zero-Shot Prediction of Color Trajectories in Food Drying](https://arxiv.org/abs/2512.06190)
...
### [50] [PrunedCaps: A Case For Primary Capsules Discrimination](https://arxiv.org/abs/2512.06003)
...
### [51] [Simple Agents Outperform Experts in Biomedical Imaging Workflow Optimization](https://arxiv.org/abs/2512.06006)
...
### [52] [High-Throughput Unsupervised Profiling of the Morphology of 316L Powder Particles for Use in Additive Manufacturing](https://arxiv.org/abs/2512.06012)
*Emmanuel Akeweje,Conall Kirk,Chi-Wai Chan,Denis Dowling,Mimi Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种自动化机器学习框架，通过高通量成像、形状提取和聚类，对选择性激光熔化（SLM）中的金属粉末形态进行大规模、快速、自动化的表征。


<details>
  <summary>Details</summary>
Motivation: SLM零件质量关键取决于原料粉末的形态，但传统粉末表征方法效率低、定性化，无法捕捉工业规模批次中的异质性。

Method: 开发并评估了三种聚类管道：自编码器管道、形状描述符管道和函数数据管道。该框架结合了高通量成像、形状提取和聚类技术，用于大规模分析金属粉末形态。

Result: 在约126,000张粉末图像（直径0.5-102微米）的数据集上，内部有效性指标显示傅里叶描述符+k-means聚类管道最有效，具有最低的Davies-Bouldin指数和最高的Calinski-Harabasz分数，同时在标准台式工作站上实现了每颗粒亚毫秒级的运行时间。

Conclusion: 该无监督学习框架能够实现粉末形态的快速、自动化评估，支持在重复使用周期中追踪形状演变，为SLM工作流中的实时原料监控提供了途径。形成的形状组也为未来研究其与流动性、堆积密度和SLM零件质量的关系奠定了基础。
...
### [53] [VAT: Vision Action Transformer by Unlocking Full Representation of ViT](https://arxiv.org/abs/2512.06013)
*Wenhao Li,Chengwei Ma,Weixin Mao*

Main category: cs.CV

TL;DR: 本文提出了视觉动作Transformer (VAT)，它扩展了Vision Transformer (ViT) 以利用其完整的特征层次结构，通过在所有层中融合感知和动作生成，在机器人操作任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 在机器人学习中，现有的方法在使用Vision Transformer (ViT) 进行视觉感知时，通常只利用最后一层的特征，这被认为提供了不足的表示。研究动机在于解锁ViT的完整特征层次结构以获取更丰富的信息。

Method: 本文提出了Vision Action Transformer (VAT) 架构，它是ViT的扩展。VAT通过在所有Transformer层中将专门的动作token与视觉特征一起处理，实现了感知和动作生成的深度且渐进的融合。

Result: 在模拟操作任务套件中，VAT在四个LIBERO基准测试中取得了98.15%的平均成功率，超越了OpenVLA-OFT等现有方法，确立了新的最先进水平。

Conclusion: VAT不仅是一个强大的模仿学习模型，还展示了利用视觉模型完整的“表示轨迹”对于推进机器人策略至关重要的作用。
...
### [54] [Dual-Stream Cross-Modal Representation Learning via Residual Semantic Decorrelation](https://arxiv.org/abs/2512.07568)
...
TL;DR: 该研究提出DSRSD-Net，一个双流残差语义去相关网络，通过残差分解和语义去相关约束，有效解耦模态特有和模态共享信息，解决了跨模态学习中模态主导、冗余耦合和虚假相关性问题，并在教育基准测试中显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 跨模态学习常面临模态主导（高方差模态掩盖弱信号）、冗余信息耦合和虚假跨模态相关性等问题，导致泛化能力差和可解释性受限。朴素的融合策略会无控制地纠缠模态共享和模态特有因素，使得难以理解预测驱动因素，并在模态噪声或缺失时缺乏鲁棒性。

Method: 本文提出了DSRSD-Net，一个双流残差语义去相关网络，其核心方法包括：(1) 双流表示学习模块：通过残差投影分离模态内（私有）和模态间（共享）潜在因子；(2) 残差语义对齐头：结合对比和回归式目标，将不同模态的共享因子映射到公共空间；(3) 去相关和正交性损失：规范共享空间的协方差结构，并强制共享流与私有流之间的正交性，以抑制跨模态冗余并防止特征坍塌。

Result: 在两个大规模教育基准测试中，DSRSD-Net在下一步预测和最终结果预测方面，持续优于强大的单模态、早期融合、晚期融合和协同注意力基线模型。

Conclusion: DSRSD-Net通过有效解耦模态特有和模态共享信息，成功解决了跨模态学习中的关键挑战，显著提升了预测性能和模型的鲁棒性与可解释性。
...
### [55] [Benchmarking CXR Foundation Models With Publicly Available MIMIC-CXR and NIH-CXR14 Datasets](https://arxiv.org/abs/2512.06014)
*Jiho Shin,Dominic Marshall,Matthieu Komorowski*

Main category: cs.CV

TL;DR: 本文对两个大型胸部X光（CXR）基础模型（CXR-Foundation和MedImageInsight）在MIMIC-CR和NIH ChestX-ray14数据集上进行了基准测试，发现MedImageInsight在大多数任务上表现略优，而CXR-Foundation展现出更强的跨数据集稳定性，强调了标准化评估的重要性。


<details>
  <summary>Details</summary>
Motivation: 尽管最近的基础模型在医学图像表示学习中表现出色，但它们在不同数据集上的比较行为仍未得到充分探索。

Method: 研究在MIMIC-CR和NIH ChestX-ray14公共数据集上评估了CXR-Foundation (ELIXR v2.0)和MedImageInsight两个大型胸部X光嵌入模型。采用统一的预处理流程和固定的下游分类器以确保可重现的比较。直接从预训练编码器中提取嵌入，在多个疾病标签上训练轻量级LightGBM分类器，并报告平均AUROC和F1分数（带95%置信区间）。此外，对MedImageInsight嵌入进行了无监督聚类分析。

Result: MedImageInsight在大多数任务中取得了略高的性能。CXR-Foundation展现出强大的跨数据集稳定性。对MedImageInsight嵌入进行无监督聚类进一步揭示了与定量结果一致的、连贯的疾病特异性结构。

Conclusion: 研究结果强调了对医学基础模型进行标准化评估的必要性，并为未来的多模态和临床整合研究建立了可重现的基线。
...
### [56] [PrefGen: Multimodal Preference Learning for Preference-Conditioned Image Generation](https://arxiv.org/abs/2512.06020)
*Wenyi Mo,Tianyu Zhang,Yalong Bai,Ligong Han,Ying Ba,Dimitris N. Metaxas*

Main category: cs.CV

TL;DR: 提出一种多模态框架，利用多模态大语言模型（MLLM）提取用户偏好，并通过对齐损失将其注入扩散模型进行个性化图像生成，显著提升图像质量和偏好对齐度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉用户细微偏好或有效编码个性化视觉信号方面存在不足，无法充分实现超越文本提示的个性化美学图像生成。

Method: 构建多模态框架，利用多模态大语言模型（MLLM）提取丰富的用户表示；通过偏好导向的视觉问答（VQA）任务训练MLLM，捕获细粒度语义线索；引入用户间和用户内判别任务以隔离偏好相关特征；设计基于最大均值差异（MMD）的对齐损失，弥合模态差距并保持多模态结构，确保与扩散模型文本编码器兼容；最终将生成的嵌入作为条件输入扩散生成器。

Result: 该方法在图像质量和偏好对齐方面显著优于现有强基线，突出显示了表示提取和对齐在个性化生成中的有效性。

Conclusion: 提出的多模态框架通过有效的用户表示提取和模态对齐机制，成功实现了对用户偏好的忠实遵循，极大地提升了个性化图像生成的能力。
...
### [57] [Fast and Flexible Robustness Certificates for Semantic Segmentation](https://arxiv.org/abs/2512.06010)
...
### [58] [Neural reconstruction of 3D ocean wave hydrodynamics from camera sensing](https://arxiv.org/abs/2512.06024)
...
### [59] [The SAM2-to-SAM3 Gap in the Segment Anything Model Family: Why Prompt-Based Expertise Fails in Concept-Driven Image Segmentation](https://arxiv.org/abs/2512.06032)
...
### [60] [Shoot-Bounce-3D: Single-Shot Occlusion-Aware 3D from Lidar by Decomposing Two-Bounce Light](https://arxiv.org/abs/2512.06080)
...
TL;DR: 本文提出一种数据驱动方法，利用单光子激光雷达的多反弹光，从单次测量中重建包含遮挡和镜面反射的3D场景，并通过大规模模拟数据集学习复杂光传输先验。


<details>
  <summary>Details</summary>
Motivation: 从单次测量中进行3D场景重建极具挑战性，尤其是在存在遮挡区域和镜面材料（如镜子）时。尽管单光子激光雷达能测量多反弹光以恢复深度和材料属性，但现有工作仅限于顺序点亮场景。本研究旨在解决更实际但也更具挑战性的同时照亮多个场景点的情况，因为其复杂的光传输难以解析反演。

Method: 提出一种数据驱动方法来反演单光子激光雷达中的光传输。为此，创建了第一个大规模（约10万个）室内场景激光雷达瞬态模拟数据集。利用该数据集学习复杂光传输的先验知识，从而将测量的二次反弹光分解为每个激光点的贡献。

Result: 实验证明，分解后的多反弹光可以用于从单次测量中推断包含遮挡和镜子的场景的3D几何结构。

Conclusion: 成功展示了利用单光子激光雷达的多反弹光和数据驱动方法，在同时多点照明的场景中，从单次测量重建包含遮挡和镜子的3D几何结构，并通过大规模模拟数据集和学习到的光传输先验实现了这一目标。
...
### [61] [BeLLA: End-to-End Birds Eye View Large Language Assistant for Autonomous Driving](https://arxiv.org/abs/2512.06096)
*Karthik Mohan,Sonam Singh,Amit Arvind Kale*

Main category: cs.CV

TL;DR: 本文提出了BeLLA，一个将统一的360°鸟瞰图(BEV)表示与大型语言模型相结合的端到端架构，用于自动驾驶中的问答，显著提高了空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶中的视觉-语言模型(VLM)和多模态语言模型(MLLM)常依赖单视角编码器或聚合的多视角特征，未能有效利用多摄像头系统的空间结构，也缺乏统一的空间表示，导致难以进行自我中心方向、物体关系和更广阔上下文的推理。

Method: 本文提出了BeLLA，一个端到端架构，它将统一的360°鸟瞰图(BEV)表示与大型语言模型连接起来，用于自动驾驶领域的问答任务。

Result: BeLLA在NuScenes-QA和DriveLM两个基准测试中表现出色，特别是在需要更高空间推理的问题（如涉及相对物体定位和附近物体行为理解）上，比现有方法有显著提升，在某些任务中绝对改进高达+9.3%。在其他问题类别中，BeLLA也表现出竞争力。

Conclusion: BeLLA证明了将统一的360°BEV表示与大型语言模型结合，能有效处理自动驾驶中多样化的问题，尤其显著增强了空间推理能力，从而实现了更丰富的场景理解和可解释的决策。
...
### [62] [SpectraIrisPAD: Leveraging Vision Foundation Models for Spectrally Conditioned Multispectral Iris Presentation Attack Detection](https://arxiv.org/abs/2512.06103)
*Raghavendra Ramachandra,Sushma Venkatesh*

Main category: cs.CV

TL;DR: 本文提出了一个名为SpectraIrisPAD的深度学习框架，用于多光谱虹膜演示攻击检测（PAD），并引入了一个新的多光谱虹膜PAD数据集（MSIrPAD），在未见攻击评估中表现出卓越的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 虹膜识别虽然准确，但易受演示攻击（PAs）影响。有效的PAD至关重要。传统系统多在近红外（NIR）光谱操作，而多光谱成像能提供互补的反射信息，有望增强PAD方法的泛化能力。

Method: 本文提出SpectraIrisPAD，一个基于DINOv2 Vision Transformer (ViT) 的深度学习框架，结合了可学习光谱位置编码、token融合和对比学习，以提取区分性、特定波段特征。此外，还引入了MSIrPAD数据集，使用定制多光谱虹膜传感器在五种NIR波长下捕获，包含18,848张虹膜图像和八种演示攻击类别。

Result: SpectraIrisPAD在未见攻击评估协议下进行了全面的实验，在所有性能指标上均持续优于多个现有先进基线，展示了在检测各种演示攻击方面的卓越鲁棒性和泛化能力。

Conclusion: SpectraIrisPAD框架及其配套的MSIrPAD数据集，显著提升了多光谱虹膜演示攻击检测的性能，增强了虹膜生物识别系统的安全性和完整性。
...
### [63] [Representation Learning for Point Cloud Understanding](https://arxiv.org/abs/2512.06058)
*Siming Yan*
...
### [64] [Explainable Melanoma Diagnosis with Contrastive Learning and LLM-based Report Generation](https://arxiv.org/abs/2512.06105)
...
### [65] [Tracking-Guided 4D Generation: Foundation-Tracker Motion Priors for 3D Model Animation](https://arxiv.org/abs/2512.06158)
...
### [66] [Physics-Grounded Shadow Generation from Monocular 3D Geometry Priors and Approximate Light Direction](https://arxiv.org/abs/2512.06174)
...
### [67] [EgoEdit: Dataset, Real-Time Streaming Model, and Benchmark for Egocentric Video Editing](https://arxiv.org/abs/2512.06065)
...
### [68] [Automated Annotation of Shearographic Measurements Enabling Weakly Supervised Defect Detection](https://arxiv.org/abs/2512.06171)
...
### [69] [SPOOF: Simple Pixel Operations for Out-of-Distribution Fooling](https://arxiv.org/abs/2512.06185)
...
### [70] [GPU-GLMB: Assessing the Scalability of GPU-Accelerated Multi-Hypothesis Tracking](https://arxiv.org/abs/2512.06230)
*Pranav Balakrishnan,Sidisha Barik,Sean M. O'Rourke,Benjamin M. Marlin*

Main category: cs.CV

TL;DR: 本文提出了一种改进的广义标记多伯努利（GLMB）滤波器变体，允许每个目标有多个检测，从而打破检测间依赖性，实现显著的并行可伸缩性，并在GPU上高效部署，以解决现有GLMB滤波器计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 多目标跟踪中的多假设方法（如标记随机有限集方法，特别是GLMB滤波器）虽然具有重要的理论特性，但由于需要维护多个假设，即使应用了假设修剪，计算成本仍然非常高。在分布式机器学习虚拟传感器网络中部署跟踪时，每个目标可能产生多个检测，这需要一种更高效、可并行化的跟踪解决方案。

Method: 本文研究了广义标记多伯努利（GLMB）滤波器的一个变体，该变体允许来自同一传感器对每个目标进行多次检测。这种方法打破了标准GLMB滤波器更新中的检测间依赖性，从而实现了显著改进的并行可伸缩性。该方法旨在高效部署在GPU硬件上，并通过GPU加速实现进行了初步分析。

Result: 所提出的GLMB滤波器变体允许每个目标进行多次检测，打破了滤波器更新中的检测间依赖性，从而实现了显著改进的并行可伸缩性，并能够在GPU硬件上高效部署。初步分析结果显示，该GPU加速实现的运行时可伸缩性与目标数量和保留假设的最大数量有关。

Conclusion: 通过允许每个目标进行多次检测并打破检测间依赖性，所提出的GLMB滤波器变体显著提高了并行可伸缩性，使其能够高效地部署在GPU硬件上，从而有效解决了传统多假设多目标跟踪方法计算成本高昂的挑战。
...
### [71] [The MICCAI Federated Tumor Segmentation (FeTS) Challenge 2024: Efficient and Robust Aggregation Methods for Federated Learning](https://arxiv.org/abs/2512.06206)
...
### [72] [Physics-Grounded Attached Shadow Detection Using Approximate 3D Geometry and Light Direction](https://arxiv.org/abs/2512.06179)
...
### [73] [NexusFlow: Unifying Disparate Tasks under Partial Supervision via Invertible Flow Networks](https://arxiv.org/abs/2512.06251)
...
TL;DR: NexusFlow是一个轻量级、可插拔的框架，通过可逆耦合层对不同任务的潜在特征分布进行对齐，创建统一表示，从而有效解决部分监督多任务学习（PS-MTL）中结构异构任务的知识迁移挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的部分监督多任务学习（PS-MTL）方法主要关注同质、密集预测任务，而对于更现实的结构多样化任务的学习探索不足。在这些场景下，任务间存在结构差异和域鸿沟，导致知识迁移困难。

Method: NexusFlow引入了一组带有可逆耦合层（invertible coupling layers）的替代网络（surrogate networks）。这些耦合层是双射的，能够在将特征映射到共享的规范空间的同时保留信息，从而对齐任务的潜在特征分布，创建统一表示。这种可逆性避免了表示坍塌，并允许在不降低表达能力的情况下实现结构不同任务之间的对齐。

Result: NexusFlow在域划分的自动驾驶任务（nuScenes数据集，包括密集地图重建和稀疏多目标跟踪）上取得了新的SOTA结果，超越了现有的强基线。在NYUv2数据集上，针对分割、深度和表面法线三个同质密集预测任务，NexusFlow也始终取得了一致的性能提升。

Conclusion: NexusFlow是一个有效且通用的框架，能够成功应对部分监督多任务学习中结构异构和同质任务的挑战。其通过可逆耦合层实现特征对齐的方法，在保持表达能力的同时，促进了跨任务的有效知识迁移，具有广泛的适用性。
...
### [74] [Opinion: Learning Intuitive Physics May Require More than Visual Data](https://arxiv.org/abs/2512.06232)
...
### [75] [Revisiting SVD and Wavelet Difference Reduction for Lossy Image Compression: A Reproducibility Study](https://arxiv.org/abs/2512.06221)
*Alena Makarova*
...
### [76] [Language-driven Fine-grained Retrieval](https://arxiv.org/abs/2512.06255)
...
### [77] [Knowing the Answer Isn't Enough: Fixing Reasoning Path Failures in LVLMs](https://arxiv.org/abs/2512.06258)
*Chaoyang Wang,Yangfan He,Yiyang Zhou,Yixuan Wang,Jiaqi Liu,Peng Xia,Zhengzhong Tu,Mohit Bansal,Huaxiu Yao*

Main category: cs.CV

TL;DR: 本文揭示大型视觉语言模型（LVLMs）即使知道正确答案，也常通过错误推理路径到达，核心问题是推理路径选择偏差而非知识不足。为此，提出PSO（路径选择优化）框架，通过两阶段后训练方法，显著提升LVLMs的推理准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: LVLMs存在一个未被充分探索的缺陷：它们经常通过不正确的推理路径得出正确答案。核心问题在于推理搜索空间中的路径选择偏差，模型倾向于不稳定或逻辑不一致的路径，导致结果不稳定。Pass@K和Pass@1之间的显著差异表明，这种失败主要源于错误推理而非知识匮乏。

Method: 提出PSO（路径选择优化），一个两阶段的后训练框架。第一阶段，采用组相对策略优化（GRPO），结合模板和基于答案的奖励，培养结构化、逐步的推理。第二阶段，进行在线偏好优化，模型从GRPO生成的数据中采样推理路径，进行自我评估，并向偏好轨迹对齐。不正确或次优路径被存储在负面重放记忆（NRM）中作为难负例，定期重访以防止重复错误并促进持续推理优化。

Result: 广泛实验表明，PSO有效剪枝了无效推理路径，显著提高了推理准确性（平均提升7.4%），并产生了更稳定和一致的思维链。

Conclusion: PSO框架有效解决了LVLMs在推理过程中路径选择偏差的问题，通过优化推理路径，显著提升了模型的准确性和稳定性，使其能够生成更可靠和一致的推理过程。
...
### [78] [TriaGS: Differentiable Triangulation-Guided Geometric Consistency for 3D Gaussian Splatting](https://arxiv.org/abs/2512.06269)
*Quan Tran,Tuan Dang*
...
### [79] [RefBench-PRO: Perceptual and Reasoning Oriented Benchmark for Referring Expression Comprehension](https://arxiv.org/abs/2512.06276)
*Tianyi Gao,Hao Li,Han Fang,Xin Wei,Xiaodong Dong,Hongbo Sun,Ye Yuan,Zhongjiang He,Jinglin Xu,Jingmin Xin,Hao Sun*

Main category: cs.CV

TL;DR: 本文提出了RefBench-PRO，一个用于指代表达理解（REC）的综合基准，将指代表达分解为感知和推理两个维度及其六个子任务，以实现多模态大语言模型（MLLM）可解释的认知能力评估。同时，提出了一种基于强化学习的Ref-R1学习方案，以提高定位准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的REC基准主要评估感知能力，缺乏可解释的评分机制，无法揭示MLLM在不同认知能力上的接地能力。

Method: 引入了RefBench-PRO基准，将指代表达分解为感知和推理两个核心维度，并进一步细分为属性、位置、交互、常识、关系和拒绝六个渐进式挑战任务。开发了一个全自动数据生成管道，为这六个子维度生成多样化的指代表达。此外，提出了Ref-R1，一个基于强化学习的学习方案，结合动态IoU的GRPO来提高在日益复杂的推理条件下的定位准确性。

Result: 广泛的实验表明，RefBench-PRO能够对MLLM在指代表达理解方面进行可解释的评估，在感知和推理方面都提出了更大的挑战。Ref-R1为REC建立了更强的基线。

Conclusion: RefBench-PRO为评估MLLM在指代表达理解方面的认知能力提供了一个全面的、可解释的基准，特别是在感知和推理方面，揭示了当前模型的挑战。Ref-R1则进一步提升了定位精度。
...
### [80] [FacePhys: State of the Heart Learning](https://arxiv.org/abs/2512.06275)
...
### [81] [Unleashing the Intrinsic Visual Representation Capability of Multimodal Large Language Models](https://arxiv.org/abs/2512.06281)
...
### [82] [A Sleep Monitoring System Based on Audio, Video and Depth Information](https://arxiv.org/abs/2512.06282)
...
### [83] [StrokeNet: Unveiling How to Learn Fine-Grained Interactions in Online Handwritten Stroke Classification](https://arxiv.org/abs/2512.06290)
...
### [84] [Exploiting Spatiotemporal Properties for Efficient Event-Driven Human Pose Estimation](https://arxiv.org/abs/2512.06306)
...
### [85] [ReCAD: Reinforcement Learning Enhanced Parametric CAD Model Generation with Vision-Language Models](https://arxiv.org/abs/2512.06328)
...
### [86] [S2WMamba: A Spectral-Spatial Wavelet Mamba for Pansharpening](https://arxiv.org/abs/2512.06330)
*Haoyu Zhang,Junhan Luo,Yugang Cao,Siran Peng,Jie Huang,Liangjian-Deng*

Main category: cs.CV

TL;DR: S2WMamba提出了一种基于Mamba的平移锐化方法，通过2D/1D Haar小波变换显式解耦空间细节和光谱信息，并通过Mamba交叉调制和动态门控实现高效融合，在多个数据集上超越了现有基线。


<details>
  <summary>Details</summary>
Motivation: 平移锐化中一个关键难点是高分辨率全色（PAN）图像与低分辨率多光谱（LRMS）图像联合处理时，空间细节和光谱保真度容易相互纠缠。

Method: 该方法称为S2WMamba，通过以下步骤实现：1) 对PAN图像应用2D Haar DWT以定位空间边缘和纹理；2) 对LRMS图像进行通道级1D Haar DWT，将每个像素的光谱视为一维信号，分离低/高频分量并限制光谱失真。3) 构建双分支结构：光谱分支将小波提取的空间细节注入MS特征，空间分支利用1D金字塔光谱细化PAN特征。4) 两个分支通过基于Mamba的交叉调制进行信息交换，以线性复杂度建模长程依赖。5) 最后，通过多尺度动态门（乘法+加法）自适应融合分支输出。

Result: S2WMamba在WV3、GF2和QB数据集上与FusionMamba、CANNet、U2Net、ARConv等强基线模型持平或超越，PSNR提升高达0.23 dB，在全分辨率WV3上达到0.956的HQNR。消融实验证明了2D/1D DWT放置、并行双分支和融合门控选择的合理性。

Conclusion: S2WMamba通过显式解耦频率信息并利用Mamba进行轻量级跨模态交互，有效解决了平移锐化中空间细节与光谱保真度纠缠的问题，显著提升了生成HRMS图像的质量，并展现出优越的性能。
...
### [87] [CryoHype: Reconstructing a thousand cryo-EM structures with transformer-based hypernetworks](https://arxiv.org/abs/2512.06332)
...
### [88] [Beyond Hallucinations: A Multimodal-Guided Task-Aware Generative Image Compression for Ultra-Low Bitrate](https://arxiv.org/abs/2512.06344)
...
### [89] [Rethinking Training Dynamics in Scale-wise Autoregressive Generation](https://arxiv.org/abs/2512.06421)
...
### [90] [Method of UAV Inspection of Photovoltaic Modules Using Thermal and RGB Data Fusion](https://arxiv.org/abs/2512.06504)
...
### [91] [TreeQ: Pushing the Quantization Boundary of Diffusion Transformer via Tree-Structured Mixed-Precision Search](https://arxiv.org/abs/2512.06353)
...
### [92] [When Gender is Hard to See: Multi-Attribute Support for Long-Range Recognition](https://arxiv.org/abs/2512.06426)
...
### [93] [Rectifying Latent Space for Generative Single-Image Reflection Removal](https://arxiv.org/abs/2512.06358)
...
### [94] [Spoofing-aware Prompt Learning for Unified Physical-Digital Facial Attack Detection](https://arxiv.org/abs/2512.06363)
*Jiabao Guo,Yadian Wang,Hui Ma,Yuhao Fu,Ju Jia,Hui Liu,Shengeng Tang,Lechao Cheng,Yunfeng Diao,Ajian Liu*

Main category: cs.CV

TL;DR: 本文提出了一种名为SPL-UAD的统一框架，通过在提示空间中解耦物理和数字攻击的优化分支，并结合提示感知增强，显著提升了人脸识别系统中对物理呈现攻击和数字伪造攻击的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界的人脸识别系统容易受到物理呈现攻击（PAs）和数字伪造攻击（DFs）的影响。现有基于CLIP的方法在处理这两种攻击时，由于在相同的提示空间下存在优化方向冲突，导致模型泛化能力受限。

Method: 本文提出了一种名为“欺骗感知提示学习统一攻击检测”（SPL-UAD）的框架。该框架在提示空间中解耦了物理和数字攻击的优化分支，构建了一个可学习的并行提示分支，并通过自适应欺骗上下文提示生成进行增强，以独立控制每种攻击类型的优化。此外，还设计了一种线索感知增强机制，利用双提示机制生成具有挑战性的样本挖掘任务，从而显著增强模型对未知攻击类型的鲁棒性。

Result: 在大型UniAttackDataPlus数据集上的广泛实验表明，所提出的方法在统一攻击检测任务中取得了显著的性能提升。

Conclusion: SPL-UAD通过解耦优化分支和引入线索感知增强，有效解决了统一物理-数字攻击检测中的挑战，显著提高了检测性能，为生物识别数据的全面保护提供了先进的解决方案。
...
### [95] [CLUENet: Cluster Attention Makes Neural Networks Have Eyes](https://arxiv.org/abs/2512.06345)
...
### [96] [Novel Deep Learning Architectures for Classification and Segmentation of Brain Tumors from MRI Images](https://arxiv.org/abs/2512.06531)
...
### [97] [VG-Refiner: Towards Tool-Refined Referring Grounded Reasoning via Agentic Reinforcement Learning](https://arxiv.org/abs/2512.06373)
...
### [98] [SUGAR: A Sweeter Spot for Generative Unlearning of Many Identities](https://arxiv.org/abs/2512.06562)
...
### [99] [Human3R: Incorporating Human Priors for Better 3D Dynamic Reconstruction from Monocular Videos](https://arxiv.org/abs/2512.06368)
...
### [100] [Are AI-Generated Driving Videos Ready for Autonomous Driving? A Diagnostic Evaluation Framework](https://arxiv.org/abs/2512.06376)
...
### [101] [TextMamba: Scene Text Detector with Mamba](https://arxiv.org/abs/2512.06657)
...
### [102] [ShadowWolf -- Automatic Labelling, Evaluation and Model Training Optimised for Camera Trap Wildlife Images](https://arxiv.org/abs/2512.06521)
...
### [103] [Masked Autoencoder Pretraining on Strong-Lensing Images for Joint Dark-Matter Model Classification and Super-Resolution](https://arxiv.org/abs/2512.06642)
...
### [104] [The Role of Entropy in Visual Grounding: Analysis and Optimization](https://arxiv.org/abs/2512.06726)
...
### [105] [VAD-Net: Multidimensional Facial Expression Recognition in Intelligent Education System](https://arxiv.org/abs/2512.06377)
*Yi Huo,Yun Ge*
...
### [106] [Task-Model Alignment: A Simple Path to Generalizable AI-Generated Image Detection](https://arxiv.org/abs/2512.06746)
...
### [107] [OCFER-Net: Recognizing Facial Expression in Online Learning System](https://arxiv.org/abs/2512.06379)
*Yi Huo,Lei Zhang*
...
### [108] [VisChainBench: A Benchmark for Multi-Turn, Multi-Image Visual Reasoning Beyond Language Priors](https://arxiv.org/abs/2512.06759)
...
### [109] [A Perception CNN for Facial Expression Recognition](https://arxiv.org/abs/2512.06422)
...
### [110] [Perceptual Region-Driven Infrared-Visible Co-Fusion for Extreme Scene Enhancement](https://arxiv.org/abs/2512.06400)
...
### [111] [DragMesh: Interactive 3D Generation Made Easy](https://arxiv.org/abs/2512.06424)
...
### [112] [RDSplat: Robust Watermarking Against Diffusion Editing for 3D Gaussian Splatting](https://arxiv.org/abs/2512.06774)
...
### [113] [Stitch and Tell: A Structured Multimodal Data Augmentation Method for Spatial Understanding](https://arxiv.org/abs/2512.06769)
...
### [114] [Automated Deep Learning Estimation of Anthropometric Measurements for Preparticipation Cardiovascular Screening](https://arxiv.org/abs/2512.06434)
...
### [115] [RMAdapter: Reconstruction-based Multi-Modal Adapter for Vision-Language Models](https://arxiv.org/abs/2512.06811)
...
### [116] [AGORA: Adversarial Generation Of Real-time Animatable 3D Gaussian Head Avatars](https://arxiv.org/abs/2512.06438)
...
### [117] [Less Is More, but Where? Dynamic Token Compression via LLM-Guided Keyframe Prior](https://arxiv.org/abs/2512.06866)
...
### [118] [Towards Stable Cross-Domain Depression Recognition under Missing Modalities](https://arxiv.org/abs/2512.06447)
...
### [119] [JoPano: Unified Panorama Generation via Joint Modeling](https://arxiv.org/abs/2512.06885)
...
### [120] [NeuroABench: A Multimodal Evaluation Benchmark for Neurosurgical Anatomy Identification](https://arxiv.org/abs/2512.06921)
...
### [121] [Sanvaad: A Multimodal Accessibility Framework for ISL Recognition and Voice-Based Interaction](https://arxiv.org/abs/2512.06485)
...
### [122] [More than Segmentation: Benchmarking SAM 3 for Segmentation, 3D Perception, and Reconstruction in Robotic Surgery](https://arxiv.org/abs/2512.07596)
...
### [123] [On The Role of K-Space Acquisition in MRI Reconstruction Domain-Generalization](https://arxiv.org/abs/2512.06530)
...
### [125] [sim2art: Accurate Articulated Object Modeling from a Single Video using Synthetic Training Data Only](https://arxiv.org/abs/2512.07698)
...
### [127] [Bridging spatial awareness and global context in medical image segmentation](https://arxiv.org/abs/2512.06560)
...
### [128] [GNC-Pose: Geometry-Aware GNC-PnP for Accurate 6D Pose Estimation](https://arxiv.org/abs/2512.06565)
*Xiujin Liu*
...
### [129] [UltrasODM: A Dual Stream Optical Flow Mamba Network for 3D Freehand Ultrasound Reconstruction](https://arxiv.org/abs/2512.07756)
...
### [130] [$\mathrm{D}^{\mathrm{3}}$-Predictor: Noise-Free Deterministic Diffusion for Dense Prediction](https://arxiv.org/abs/2512.07062)
...
### [131] [TrajMoE: Scene-Adaptive Trajectory Planning with Mixture of Experts and Reinforcement Learning](https://arxiv.org/abs/2512.07135)
...
### [132] [MedGRPO: Multi-Task Reinforcement Learning for Heterogeneous Medical Video Understanding](https://arxiv.org/abs/2512.06581)
...
### [135] [From Remote Sensing to Multiple Time Horizons Forecasts: Transformers Model for CyanoHAB Intensity in Lake Champlain](https://arxiv.org/abs/2512.06598)
...
### [137] [Learning Relative Gene Expression Trends from Pathology Images in Spatial Transcriptomics](https://arxiv.org/abs/2512.06612)
...
### [138] [VFM-VLM: Vision Foundation Model and Vision Language Model based Visual Comparison for 3D Pose Estimation](https://arxiv.org/abs/2512.07215)
...
### [139] [Hierarchical Deep Learning for Diatom Image Classification: A Multi-Level Taxonomic Approach](https://arxiv.org/abs/2512.06613)
*Yueying Ke*
...
### [140] [Personalized Image Descriptions from Attention Sequences](https://arxiv.org/abs/2512.06662)
...
### [141] [Towards Robust Protective Perturbation against DeepFake Face Swapping](https://arxiv.org/abs/2512.07228)
...
### [142] [CoT4Det: A Chain-of-Thought Framework for Perception-Oriented Vision-Language Tasks](https://arxiv.org/abs/2512.06663)
...
### [143] [1 + 1 > 2: Detector-Empowered Video Large Language Model for Spatio-Temporal Grounding and Reasoning](https://arxiv.org/abs/2512.06673)
...
### [144] [Dropout Prompt Learning: Towards Robust and Adaptive Vision-Language Models](https://arxiv.org/abs/2512.07234)
...
### [145] [DGGAN: Degradation Guided Generative Adversarial Network for Real-time Endoscopic Video Enhancement](https://arxiv.org/abs/2512.07253)
...
### [146] [Effective Attention-Guided Multi-Scale Medical Network for Skin Lesion Segmentation](https://arxiv.org/abs/2512.07275)
...
### [147] [RunawayEvil: Jailbreaking the Image-to-Video Generative Models](https://arxiv.org/abs/2512.06674)
...
### [148] [EMGauss: Continuous Slice-to-3D Reconstruction via Dynamic Gaussian Modeling in Volume Electron Microscopy](https://arxiv.org/abs/2512.06684)
...
### [149] [Towards Accurate UAV Image Perception: Guiding Vision-Language Models with Stronger Task Prompts](https://arxiv.org/abs/2512.07302)
...
### [150] [ContextAnyone: Context-Aware Diffusion for Character-Consistent Text-to-Video Generation](https://arxiv.org/abs/2512.07328)
...
### [151] [Lightweight Wasserstein Audio-Visual Model for Unified Speech Enhancement and Separation](https://arxiv.org/abs/2512.06689)
...
### [152] [Graph Convolutional Long Short-Term Memory Attention Network for Post-Stroke Compensatory Movement Detection Based on Skeleton Data](https://arxiv.org/abs/2512.06736)
...
### [153] [FedSCAl: Leveraging Server and Client Alignment for Unsupervised Federated Source-Free Domain Adaptation](https://arxiv.org/abs/2512.06738)
...
### [154] [MMDuet2: Enhancing Proactive Interaction of Video MLLMs with Multi-Turn Reinforcement Learning](https://arxiv.org/abs/2512.06810)
...
### [155] [DeepAgent: A Dual Stream Multi Agent Fusion for Robust Multimodal Deepfake Detection](https://arxiv.org/abs/2512.07351)
...
### [156] [Structure-Aware Feature Rectification with Region Adjacency Graphs for Training-Free Open-Vocabulary Semantic Segmentation](https://arxiv.org/abs/2512.07360)
...
### [157] [Think-Reflect-Revise: A Policy-Guided Reflective Framework for Safety Alignment in Large Vision Language Models](https://arxiv.org/abs/2512.07141)
...
### [158] [UARE: A Unified Vision-Language Model for Image Quality Assessment, Restoration, and Enhancement](https://arxiv.org/abs/2512.06750)
...
### [159] [Generating Storytelling Images with Rich Chains-of-Reasoning](https://arxiv.org/abs/2512.07198)
...
### [160] [Data-driven Exploration of Mobility Interaction Patterns](https://arxiv.org/abs/2512.07415)
...
### [161] [When normalization hallucinates: unseen risks in AI-powered whole slide image processing](https://arxiv.org/abs/2512.07426)
...
### [162] [JOCA: Task-Driven Joint Optimisation of Camera Hardware and Adaptive Camera Control Algorithms](https://arxiv.org/abs/2512.06763)
...
### [163] [Toward More Reliable Artificial Intelligence: Reducing Hallucinations in Vision-Language Models](https://arxiv.org/abs/2512.07564)
...
### [164] [Generalized Geometry Encoding Volume for Real-time Stereo Matching](https://arxiv.org/abs/2512.06793)
...
### [165] [Physics Informed Human Posture Estimation Based on 3D Landmarks from Monocular RGB-Videos](https://arxiv.org/abs/2512.06783)
...
### [166] [An AI-Powered Autonomous Underwater System for Sea Exploration and Scientific Research](https://arxiv.org/abs/2512.07652)
...
### [167] [VDOT: Efficient Unified Video Creation via Optimal Transport Distillation](https://arxiv.org/abs/2512.06802)
...
### [168] [Guiding What Not to Generate: Automated Negative Prompting for Text-Image Alignment](https://arxiv.org/abs/2512.07702)
...
### [169] [MeshSplatting: Differentiable Rendering with Opaque Meshes](https://arxiv.org/abs/2512.06818)
...
### [170] [DIST-CLIP: Arbitrary Metadata and Image Guided MRI Harmonization via Disentangled Anatomy-Contrast Representations](https://arxiv.org/abs/2512.07674)
...
### [171] [SparseCoop: Cooperative Perception with Kinematic-Grounded Queries](https://arxiv.org/abs/2512.06838)
...
### [172] [Improving action classification with brain-inspired deep networks](https://arxiv.org/abs/2512.07729)
...
### [173] [CADE: Continual Weakly-supervised Video Anomaly Detection with Ensembles](https://arxiv.org/abs/2512.06840)
...
### [174] [SAVE: Sparse Autoencoder-Driven Visual Information Enhancement for Mitigating Object Hallucination](https://arxiv.org/abs/2512.07730)
...
### [175] [Pseudo Anomalies Are All You Need: Diffusion-Based Generation for Weakly-Supervised Video Anomaly Detection](https://arxiv.org/abs/2512.06845)
...
### [176] [WorldReel: 4D Video Generation with Consistent Geometry and Motion Modeling](https://arxiv.org/abs/2512.07821)
...
### [177] [Hide-and-Seek Attribution: Weakly Supervised Segmentation of Vertebral Metastases in CT](https://arxiv.org/abs/2512.06849)
...
### [178] [One Layer Is Enough: Adapting Pretrained Visual Encoders for Image Generation](https://arxiv.org/abs/2512.07829)
...
### [179] [Omni-Referring Image Segmentation](https://arxiv.org/abs/2512.06862)
...
### [180] [Relational Visual Similarity](https://arxiv.org/abs/2512.07833)
...
### [181] [Boosting Unsupervised Video Instance Segmentation with Automatic Quality-Guided Self-Training](https://arxiv.org/abs/2512.06864)
...
### [182] [Towards Robust Pseudo-Label Learning in Semantic Segmentation: An Encoding Perspective](https://arxiv.org/abs/2512.06870)
...
### [184] [Hierarchical Image-Guided 3D Point Cloud Segmentation in Industrial Scenes via Multi-View Bayesian Fusion](https://arxiv.org/abs/2512.06882)
...
### [186] [Balanced Learning for Domain Adaptive Semantic Segmentation](https://arxiv.org/abs/2512.06886)
...
### [187] [Overcoming Small Data Limitations in Video-Based Infant Respiration Estimation](https://arxiv.org/abs/2512.06888)
...
### [188] [Scaling Zero-Shot Reference-to-Video Generation](https://arxiv.org/abs/2512.06905)
...
### [189] [Can We Go Beyond Visual Features? Neural Tissue Relation Modeling for Relational Graph Analysis in Non-Melanoma Skin Histology](https://arxiv.org/abs/2512.06949)
...
### [190] [Selective Masking based Self-Supervised Learning for Image Semantic Segmentation](https://arxiv.org/abs/2512.06981)
...
### [191] [Persistent Homology-Guided Frequency Filtering for Image Compression](https://arxiv.org/abs/2512.07065)
...
### [192] [RAVE: Rate-Adaptive Visual Encoding for 3D Gaussian Splatting](https://arxiv.org/abs/2512.07052)
...
### [193] [Evaluating and Preserving High-level Fidelity in Super-Resolution](https://arxiv.org/abs/2512.07037)
...
### [194] [Context-measure: Contextualizing Metric for Camouflage](https://arxiv.org/abs/2512.07076)
...
### [195] [COREA: Coarse-to-Fine 3D Representation Alignment Between Relightable 3D Gaussians and SDF via Bidirectional 3D-to-3D Supervision](https://arxiv.org/abs/2512.07107)
...
### [196] [DFIR-DETR: Frequency Domain Enhancement and Dynamic Feature Aggregation for Cross-Scene Small Object Detection](https://arxiv.org/abs/2512.07078)
...
### [197] [MSN: Multi-directional Similarity Network for Hand-crafted and Deep-synthesized Copy-Move Forgery Detection](https://arxiv.org/abs/2512.07110)
...
### [198] [Training-free Clothing Region of Interest Self-correction for Virtual Try-On](https://arxiv.org/abs/2512.07126)
...
### [199] [MulCLIP: A Multi-level Alignment Framework for Enhancing Fine-grained Long-context CLIP](https://arxiv.org/abs/2512.07128)
...
### [202] [TIDE: Two-Stage Inverse Degradation Estimation with Guided Prior Disentanglement for Underwater Image Restoration](https://arxiv.org/abs/2512.07171)
...
### [203] [Integrating Multi-scale and Multi-filtration Topological Features for Medical Image Classification](https://arxiv.org/abs/2512.07190)
...
### [204] [When Privacy Meets Recovery: The Overlooked Half of Surrogate-Driven Privacy Preservation for MLLM Editing](https://arxiv.org/abs/2512.07166)
...
### [205] [HVQ-CGIC: Enabling Hyperprior Entropy Modeling for VQ-Based Controllable Generative Image Compression](https://arxiv.org/abs/2512.07192)
...
### [206] [RefLSM: Linearized Structural-Prior Reflectance Model for Medical Image Segmentation and Bias-Field Correction](https://arxiv.org/abs/2512.07191)
...
### [207] [SUCCESS-GS: Survey of Compactness and Compression for Efficient Static and Dynamic Gaussian Splatting](https://arxiv.org/abs/2512.07197)
...
### [208] [Understanding Diffusion Models via Code Execution](https://arxiv.org/abs/2512.07201)
*Cheng Yu*
...
### [209] [AutoLugano: A Deep Learning Framework for Fully Automated Lymphoma Segmentation and Lugano Staging on FDG-PET/CT](https://arxiv.org/abs/2512.07206)
...
### [210] [MMRPT: MultiModal Reinforcement Pre-Training via Masked Vision-Dependent Reasoning](https://arxiv.org/abs/2512.07203)
...
### [211] [ReLKD: Inter-Class Relation Learning with Knowledge Distillation for Generalized Category Discovery](https://arxiv.org/abs/2512.07229)
...
### [212] [STRinGS: Selective Text Refinement in Gaussian Splatting](https://arxiv.org/abs/2512.07230)
...
### [213] [Object Pose Distribution Estimation for Determining Revolution and Reflection Uncertainty in Point Clouds](https://arxiv.org/abs/2512.07211)
...
### [214] [Zero-Shot Textual Explanations via Translating Decision-Critical Features](https://arxiv.org/abs/2512.07245)
...
### [215] [Squeezed-Eff-Net: Edge-Computed Boost of Tomography Based Brain Tumor Classification leveraging Hybrid Neural Network Architecture](https://arxiv.org/abs/2512.07241)
...
### [216] [Unified Camera Positional Encoding for Controlled Video Generation](https://arxiv.org/abs/2512.07237)
...
### [217] [AdLift: Lifting Adversarial Perturbations to Safeguard 3D Gaussian Splatting Assets Against Instruction-Driven Editing](https://arxiv.org/abs/2512.07247)
...
### [218] [A graph generation pipeline for critical infrastructures based on heuristics, images and depth data](https://arxiv.org/abs/2512.07269)
...
### [219] [See More, Change Less: Anatomy-Aware Diffusion for Contrast Enhancement](https://arxiv.org/abs/2512.07251)
...
### [220] [RVLF: A Reinforcing Vision-Language Framework for Gloss-Free Sign Language Translation](https://arxiv.org/abs/2512.07273)
...
### [221] [MICo-150K: A Comprehensive Dataset Advancing Multi-Image Composition](https://arxiv.org/abs/2512.07348)
...
### [222] [Debiasing Diffusion Priors via 3D Attention for Consistent Gaussian Splatting](https://arxiv.org/abs/2512.07345)
...
### [223] [Geo3DVQA: Evaluating Vision-Language Models for 3D Geospatial Reasoning from Aerial Imagery](https://arxiv.org/abs/2512.07276)
...
### [224] [The Inductive Bottleneck: Data-Driven Emergence of Representational Sparsity in Vision Transformers](https://arxiv.org/abs/2512.07331)
*Kanishk Awadhiya*
...
### [225] [Reevaluating Automated Wildlife Species Detection: A Reproducibility Study on a Custom Image Dataset](https://arxiv.org/abs/2512.07305)
...
### [226] [Generalized Referring Expression Segmentation on Aerial Photos](https://arxiv.org/abs/2512.07338)
...
### [227] [Enhancing Small Object Detection with YOLO: A Novel Framework for Improved Accuracy and Efficiency](https://arxiv.org/abs/2512.07379)
...
### [228] [LogicCBMs: Logic-Enhanced Concept-Based Learning](https://arxiv.org/abs/2512.07383)
...
### [229] [How Far are Modern Trackers from UAV-Anti-UAV? A Million-Scale Benchmark and New Baseline](https://arxiv.org/abs/2512.07385)
...
### [230] [Tessellation GS: Neural Mesh Gaussians for Robust Monocular Reconstruction of Dynamic Objects](https://arxiv.org/abs/2512.07381)
...
### [231] [GlimmerNet: A Lightweight Grouped Dilated Depthwise Convolutions for UAV-Based Emergency Monitoring](https://arxiv.org/abs/2512.07391)
*Đorđe Nedeljković*
...
### [232] [Reconstructing Objects along Hand Interaction Timelines in Egocentric Video](https://arxiv.org/abs/2512.07394)
...
### [233] [InterAgent: Physics-based Multi-agent Command Execution via Diffusion on Interaction Graphs](https://arxiv.org/abs/2512.07410)
...
### [234] [Unified Video Editing with Temporal Reasoner](https://arxiv.org/abs/2512.07469)
...
### [235] [Towards Robust DeepFake Detection under Unstable Face Sequences: Adaptive Sparse Graph Embedding with Order-Free Representation and Explicit Laplacian Spectral Prior](https://arxiv.org/abs/2512.07498)
...
### [236] [Single-step Diffusion-based Video Coding with Semantic-Temporal Guidance](https://arxiv.org/abs/2512.07480)
...
### [237] [MultiMotion: Multi Subject Video Motion Transfer via Video Diffusion Transformer](https://arxiv.org/abs/2512.07500)
...
### [238] [ControlVP: Interactive Geometric Refinement of AI-Generated Images with Consistent Vanishing Points](https://arxiv.org/abs/2512.07504)
...
### [239] [SJD++: Improved Speculative Jacobi Decoding for Training-free Acceleration of Discrete Auto-regressive Text-to-Image Generation](https://arxiv.org/abs/2512.07503)
...
### [240] [MeshRipple: Structured Autoregressive Generation of Artist-Meshes](https://arxiv.org/abs/2512.07514)
...
### [241] [From Orbit to Ground: Generative City Photogrammetry from Extreme Off-Nadir Satellite Images](https://arxiv.org/abs/2512.07527)
...
### [242] [LongCat-Image Technical Report](https://arxiv.org/abs/2512.07584)
...
### [243] [All You Need Are Random Visual Tokens? Demystifying Token Pruning in VLLMs](https://arxiv.org/abs/2512.07580)
...
### [244] [Robust Variational Model Based Tailored UNet: Leveraging Edge Detector and Mean Curvature for Improved Image Segmentation](https://arxiv.org/abs/2512.07590)
...
### [245] [Online Segment Any 3D Thing as Instance Tracking](https://arxiv.org/abs/2512.07599)
...
### [246] [Decomposition Sampling for Efficient Region Annotations in Active Learning](https://arxiv.org/abs/2512.07606)
...
### [247] [MoCA: Mixture-of-Components Attention for Scalable Compositional 3D Generation](https://arxiv.org/abs/2512.07628)
...
### [248] [Optimization-Guided Diffusion for Interactive Scene Generation](https://arxiv.org/abs/2512.07661)
...
### [249] [Liver Fibrosis Quantification and Analysis: The LiQA Dataset and Baseline Method](https://arxiv.org/abs/2512.07651)
...
### [250] [EgoCampus: Egocentric Pedestrian Eye Gaze Model and Dataset](https://arxiv.org/abs/2512.07668)
...
### [251] [UnCageNet: Tracking and Pose Estimation of Caged Animal](https://arxiv.org/abs/2512.07712)
...
### [252] [PVeRA: Probabilistic Vector-Based Random Matrix Adaptation](https://arxiv.org/abs/2512.07703)
...
### [253] [HLTCOE Evaluation Team at TREC 2025: VQA Track](https://arxiv.org/abs/2512.07738)
...
### [254] [DiffusionDriveV2: Reinforcement Learning-Constrained Truncated Diffusion Modeling in End-to-End Autonomous Driving](https://arxiv.org/abs/2512.07745)
...
### [255] [ViSA: 3D-Aware Video Shading for Real-Time Upper-Body Avatar Creation](https://arxiv.org/abs/2512.07720)
...
### [256] [SpatialDreamer: Incentivizing Spatial Reasoning via Active Mental Imagery](https://arxiv.org/abs/2512.07733)
...
### [257] [Unison: A Fully Automatic, Task-Universal, and Low-Cost Framework for Unified Understanding and Generation](https://arxiv.org/abs/2512.07747)
...
### [258] [Modality-Aware Bias Mitigation and Invariance Learning for Unsupervised Visible-Infrared Person Re-Identification](https://arxiv.org/abs/2512.07760)
...
### [259] [GorillaWatch: An Automated System for In-the-Wild Gorilla Re-Identification and Population Monitoring](https://arxiv.org/abs/2512.07776)
...
### [260] [Distribution Matching Variational AutoEncoder](https://arxiv.org/abs/2512.07778)
...
### [261] [OneStory: Coherent Multi-Shot Video Generation with Adaptive Memory](https://arxiv.org/abs/2512.07802)
...
### [262] [Multi-view Pyramid Transformer: Look Coarser to See Broader](https://arxiv.org/abs/2512.07806)
...
### [263] [Lang3D-XL: Language Embedded 3D Gaussians for Large-scale Scenes](https://arxiv.org/abs/2512.07807)
...
### [264] [Voxify3D: Pixel Art Meets Volumetric Rendering](https://arxiv.org/abs/2512.07834)
...
### [265] [OpenVE-3M: A Large-Scale High-Quality Dataset for Instruction-Guided Video Editing](https://arxiv.org/abs/2512.07826)
...
### [266] [UnityVideo: Unified Multi-Modal Multi-Task Learning for Enhancing World-Aware Video Generation](https://arxiv.org/abs/2512.07831)
...
### [267] [Morphologically-Informed Tokenizers for Languages with Non-Concatenative Morphology: A case study of Yoloxóchtil Mixtec ASR](https://arxiv.org/abs/2512.06169)
*Chris Crawford*

Main category: cs.CL

TL;DR: 本研究探讨了使用形态学感知的分词器，通过结合ASR和文本序列到序列工具，来辅助和简化Yoloxóchitl Mixtec (YM) 语音语料库的行间注音，旨在提高效率并减轻人工标注者的工作量。


<details>
  <summary>Details</summary>
Motivation: 研究动机是提高Yoloxóchitl Mixtec (YM) 语音语料库行间注音的效率，同时减少人工标注者的工作负担，通过改进ASR系统的分词方法来实现。

Method: 本文提出了两种新型的非线性分词方案（Segment and Melody 分词器和 Sequence of Processes 分词器），它们以非线性方式分隔单词，尽可能保留声调形态信息。研究将这些新型分词器与传统的BPE和Unigram模型进行比较，并使用ASR和文本序列到序列工具进行评估。此外，还通过形态学和信息论指标分析了分词器，以寻找与下游性能的预测相关性。

Result: 研究发现，这些新型分词器与BPE和Unigram模型具有竞争力。其中，Segment-and-Melody模型在词错误率方面优于传统分词器，但在字符错误率方面未能达到相同水平。结果表明，专门为语言的非连接形态设计的非线性分词器在ASR方面与传统的BPE和Unigram模型具有竞争力。

Conclusion: 为非连接形态语言设计的非线性分词器在ASR任务中与传统分词器具有竞争力。然而，这些分词器在下游处理任务中的适用性还需要进一步研究。
...
### [268] [Do You Feel Comfortable? Detecting Hidden Conversational Escalation in AI Chatbots](https://arxiv.org/abs/2512.06193)
*Jihyung Park,Saleh Afroogh,Junfeng Jiao*

Main category: cs.CL

TL;DR: 本文提出GAUGE框架，通过基于logits的实时检测，识别大型语言模型对话中可能导致情绪困扰的隐性情感升级。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）作为情感伴侣，可能通过重复情感强化或情感漂移造成传统毒性过滤器无法检测的“隐性伤害”。现有安全机制依赖外部分类器或临床标准，难以适应对话的实时动态。

Method: GAUGE（Guarding Affective Utterance Generation Escalation）是一个轻量级、基于logits的框架，用于实时检测隐藏的对话升级。它通过测量LLM输出如何概率性地改变对话的情感状态来工作。

Result: GAUGE能够实时检测LLM对话中潜在的隐性情感升级，从而解决传统毒性过滤器和现有安全机制的不足。

Conclusion: GAUGE通过监控LLM输出对对话情感状态的概率性改变，为实时检测和预防由LLM引起的情感困扰提供了一种新的解决方案。
...
### [269] [Empathy by Design: Aligning Large Language Models for Healthcare Dialogue](https://arxiv.org/abs/2512.06097)
*Emre Umucu,Guillermina Solis,Leon Garza,Emilia Rivas,Beatrice Lee,Anantaa Kotal,Aritran Piplai*

Main category: cs.CL

TL;DR: 本文提出了一种基于直接偏好优化（DPO）的对齐框架，旨在提高大型语言模型在医疗保健和护理应用中的事实准确性、语义连贯性以及同理心、礼貌和简洁性等以人为中心的质量，从而解决现有模型可靠性差和缺乏同理心的问题。


<details>
  <summary>Details</summary>
Motivation: 通用大型语言模型在医疗保健和护理应用中存在事实不可靠和缺乏同理心沟通的两大缺陷，这在用户（尤其是非专业人士和护理人员）寻求医疗指导或情感安慰的敏感情境中构成了重大风险。

Method: 研究引入了一个基于直接偏好优化（DPO）的对齐框架，通过成对偏好数据对领域适应型大型语言模型进行微调。其中，偏好响应反映支持性和易于理解的沟通风格，拒绝响应则代表规定性或过于专业的语气。这种直接优化方法比传统的基于强化学习的对齐方法更有效地使模型输出与人类偏好保持一致。

Result: 在多个开放和专有大型语言模型上的实证评估表明，DPO调优的模型比基线和商业替代方案（如Google医疗对话系统）实现了更高的语义对齐、更高的事实准确性和更强的人文中心评估分数。

Conclusion: 偏好驱动的对齐提供了一条可扩展且透明的途径，用于开发值得信赖、富有同理心且具有临床知识的AI助手，以促进护理人员和医疗保健沟通。
...
### [270] [Automated Data Enrichment using Confidence-Aware Fine-Grained Debate among Open-Source LLMs for Mental Health and Online Safety](https://arxiv.org/abs/2512.06227)
...
### [271] [Policy-based Sentence Simplification: Replacing Parallel Corpora with LLM-as-a-Judge](https://arxiv.org/abs/2512.06228)
...
### [272] [LOCUS: A System and Method for Low-Cost Customization for Universal Specialization](https://arxiv.org/abs/2512.06239)
...
### [273] [Convergence of Outputs When Two Large Language Models Interact in a Multi-Agentic Setup](https://arxiv.org/abs/2512.06256)
...
### [274] [Nanbeige4-3B Technical Report: Exploring the Frontier of Small Language Models](https://arxiv.org/abs/2512.06266)
...
TL;DR: Nanbeige4-3B是一个小规模但高性能的语言模型家族，通过大规模预训练、创新的训练调度器、高质量指令微调、双重偏好蒸馏和多阶段强化学习，显著超越同等规模模型并媲美更大模型。


<details>
  <summary>Details</summary>
Motivation: 研究旨在扩展小型语言模型的扩展定律边界，实现小规模模型的高性能表现。

Method: 1. 预训练：在23T高质量token上进行，设计了细粒度热身-稳定-衰减（FG-WSD）训练调度器，逐步优化数据混合。 2. 后训练（SFT数据质量）：设计了结合审议生成细化和思维链重建的联合机制。 3. SFT后：通过提出的双重偏好蒸馏（DPD）方法，使用旗舰推理模型对Nanbeige4-3B进行蒸馏。 4. 最后阶段：应用多阶段强化学习，利用可验证奖励和偏好建模来增强推理和人类对齐能力。

Result: Nanbeige4-3B不仅显著优于同等参数规模的模型，而且在广泛的基准测试中能与大得多的模型相媲美。

Conclusion: Nanbeige4-3B证明了通过先进的训练策略，小规模语言模型也能达到高性能，从而拓展了小型语言模型的性能边界。
...
### [275] [Knowing What's Missing: Assessing Information Sufficiency in Question Answering](https://arxiv.org/abs/2512.06476)
*Akriti Jain,Aparna Garimella*

Main category: cs.CL

TL;DR: 本文提出了一种“识别-验证”框架，通过让模型推理缺失信息来评估上下文对问题回答的充分性，尤其针对推理性问题，从而提高问答系统的可靠性并明确信息差距。


<details>
  <summary>Details</summary>
Motivation: 构建可靠的问答系统面临的关键挑战是判断给定上下文是否包含足够信息来回答问题，尤其对于需要推理而非直接提取的推理性问题，简单提示策略常常失效。

Method: 提出“识别-验证”框架。首先，生成关于缺失信息的多个假设并达成语义共识。其次，执行关键的验证步骤，强制模型重新检查原文以确认信息是否确实缺失。

Result: 在多跳和事实性问答数据集上，通过引导模型证明其关于缺失信息的判断，该框架比现有基线模型产生更准确的充分性判断，并能清晰地阐明信息差距。

Conclusion: 通过引导模型论证其关于缺失信息的判断，可以产生更准确的充分性判断，从而提高问答系统的可靠性。
...
### [276] [Modeling Contextual Passage Utility for Multihop Question Answering](https://arxiv.org/abs/2512.06464)
*Akriti Jain,Aparna Garimella*

Main category: cs.CL

TL;DR: 本文提出了一种轻量级方法，通过建模段落间的上下文依赖性来预测多跳问答（QA）中段落的实用性，从而改进段落重排序和下游QA性能。


<details>
  <summary>Details</summary>
Motivation: 多跳问答需要整合多个文本段落的信息。现有检索方法仅识别相关段落，但未充分评估其实用性，可能引入冗余和噪声。当前的实用性预测方法独立建模段落，忽视了多跳推理中段落实用性依赖于其与其他段落的关系（互补性或关键链接）。

Method: 我们提出了一种轻量级方法来建模上下文段落实用性，考虑段落间的依赖关系。我们微调了一个小型基于Transformer的模型来预测多跳QA的段落实用性分数。利用高级推理模型的推理轨迹，捕获段落用于回答问题的顺序，从而获得合成训练数据。

Result: 通过全面的实验，我们证明了基于实用性对检索到的段落进行评分，相比基于相关性的重排序方法，能带来更好的重排序效果和下游QA性能。

Conclusion: 建模段落的上下文实用性，考虑段落间依赖，能有效提升多跳问答系统的性能，优于仅基于相关性的方法。
...
### [277] [Classifying German Language Proficiency Levels Using Large Language Models](https://arxiv.org/abs/2512.06483)
*Elias-Leander Ahlers,Witold Brunsmann,Malte Schilling*

Main category: cs.CL

TL;DR: 本文研究使用大型语言模型（LLMs）自动将德语文本分类到欧洲共同语言参考框架（CEFR）的不同熟练度级别。


<details>
  <summary>Details</summary>
Motivation: 评估语言熟练度对教育至关重要，因为它能实现根据学习者需求量身定制的教学。

Method: 研究者构建了一个多样化的数据集，结合了多个现有CEFR标注语料库和合成数据。他们评估了提示工程策略、对LLaMA-3-8B-Instruct模型进行微调，以及一种利用LLM内部神经状态进行分类的探测方法。

Result: 结果显示，与现有方法相比，性能有持续改进。

Conclusion: 研究强调了LLMs在可靠且可扩展的CEFR分类方面的潜力。
...
### [278] [Adapting AlignScore Mertic for Factual Consistency Evaluation of Text in Russian: A Student Abstract](https://arxiv.org/abs/2512.06586)
...
### [279] [The Online Discourse of Virtual Reality and Anxiety](https://arxiv.org/abs/2512.06656)
...
### [280] [ProSocialAlign: Preference Conditioned Test Time Alignment in Language Models](https://arxiv.org/abs/2512.06515)
...
### [281] [CMV-Fuse: Cross Modal-View Fusion of AMR, Syntax, and Knowledge Representations for Aspect Based Sentiment Analysis](https://arxiv.org/abs/2512.06679)
*Smitha Muthya Sudheendra,Mani Deep Cherukuri,Jaideep Srivastava*

Main category: cs.CL

TL;DR: CMV-Fuse是一个用于方面级情感分析(ABSA)的跨模态视图融合框架，它通过系统地结合多语言学视角（包括抽象意义表示、成分句法、依存句法和语义注意力），并整合外部知识，以模仿人类语言处理。该框架利用分层门控注意力融合和结构感知多视图对比学习，显著提高了ABSA的性能。


<details>
  <summary>Details</summary>
Motivation: 当前的方面级情感分析(ABSA)系统通常独立利用语言学视图，忽略了人类自然利用的结构化表示之间的复杂相互作用。自然语言理解本质上依赖于整合从表层句法到深层语义和世界知识的多种互补视角。

Method: 本文提出了CMV-Fuse框架，通过系统地整合四种语言学视角（抽象意义表示、成分句法、依存句法和语义注意力），并增强外部知识集成，来模拟人类语言处理。该方法采用分层门控注意力融合机制，在局部句法、中间语义和全局知识层面进行融合。此外，引入了一种新颖的结构感知多视图对比学习机制，以确保互补表示之间的一致性，同时保持计算效率。

Result: 广泛的实验表明，CMV-Fuse在标准基准测试上比现有强基线模型有显著的性能提升。分析揭示了每个语言学视图如何为更稳健的情感分析做出贡献。

Conclusion: CMV-Fuse通过系统整合多语言学视角和外部知识，并利用分层注意力融合和对比学习，成功地模拟了人类语言处理，从而在方面级情感分析任务中实现了显著的性能提升和更鲁棒的情感分析。
...
### [282] [PersonaMem-v2: Towards Personalized Intelligence via Learning Implicit User Personas and Agentic Memory](https://arxiv.org/abs/2512.06688)
...
TL;DR: 本文介绍了PersonaMem-v2数据集，用于LLM个性化，并研究了强化微调和智能体记忆系统。结果显示，强化微调的Qwen3-4B在隐式个性化方面超越了GPT-5，而智能体记忆系统则以更少的输入token达到了最先进的性能，为可扩展的个性化智能提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 个性化是推动AI能力和对齐的下一个里程碑，但现有前沿大型语言模型在处理隐式个性化和长上下文推理方面仍面临挑战，尤其是在模拟真实世界交互时。

Method: 研究方法包括：1) 引入PersonaMem-v2数据集，模拟1000个用户-聊天机器人交互、300多个场景、20000多个用户偏好和128k token上下文窗口，其中大部分偏好是隐式揭示的。2) 利用该数据，探索强化微调（RFT）如何提高模型对用户理解和个性化的长上下文推理能力。3) 开发了一个智能体记忆系统框架，该系统维护一个单一、人类可读的记忆，并随时间与每个用户一起增长。

Result: 主要结果如下：1) 前沿LLM在隐式个性化方面表现不佳，准确率仅为37-48%，推理能力是瓶颈。2) 通过强化微调，Qwen3-4B在隐式个性化方面达到了53%的准确率，超越了GPT-5。3) 智能体记忆框架实现了55%的最先进准确率，同时使用比完整32k对话历史少16倍的输入token（仅2k token记忆）。

Conclusion: 研究结果强调了PersonaMem-v2数据集的影响力，并证明了智能体记忆系统是实现真实世界个性化智能的可扩展途径，能够有效解决LLM在隐式个性化和长上下文推理方面的挑战。
...
### [283] [Think-While-Generating: On-the-Fly Reasoning for Personalized Long-Form Generation](https://arxiv.org/abs/2512.06690)
*Chengbing Wang,Yang Zhang,Wenjie Wang,Xiaoyan Zhao,Fuli Feng,Xiangnan He,Tat-Seng Chua*

Main category: cs.CL

TL;DR: FlyThinker是一种高效的“边思考边生成”框架，通过并行推理模型实现个性化长文本生成，解决了现有方法在个性化和长文本生成中的效率与效果问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型(LLMs)的偏好对齐主要关注群体偏好，忽略了个人用户。早期个性化方法难以处理隐式偏好。最近的“先思考后生成”方法在长文本生成中面临挑战，因为其静态的一次性推理难以捕捉所有相关信息并适应内容演变。

Method: 本文提出了FlyThinker，一个高效的“边思考边生成”框架，用于个性化长文本生成。它采用一个独立的推理模型，并行生成潜在的token级推理，并将其融入生成模型以动态指导响应生成。这种设计确保了推理和生成并发运行，提高了推理效率。此外，推理模型仅依赖于先前的响应而非其自身的先前输出，从而保持了训练在不同位置的并行性，确保了训练效率。

Result: 在真实世界基准测试上的广泛实验表明，FlyThinker在保持训练和推理效率的同时，实现了更好的个性化生成。

Conclusion: FlyThinker是一个高效且有效的个性化长文本生成框架，通过其独特的“边思考边生成”机制，解决了现有方法在个性化能力和长文本生成效率方面的不足。
...
### [284] [Mechanistic Interpretability of GPT-2: Lexical and Contextual Layers in Sentiment Analysis](https://arxiv.org/abs/2512.06681)
*Amartya Hatua*

Main category: cs.CL

TL;DR: 本研究通过机械可解释性方法，发现GPT-2处理情感信息时，早期层负责词汇情感检测，而上下文整合（如否定、讽刺）主要发生在后期层，而非之前假设的中间层。


<details>
  <summary>Details</summary>
Motivation: 研究动机是因果性地探究GPT-2的Transformer层如何处理情感信息，并验证关于情感处理的两阶段架构（早期词汇检测和中间层上下文整合）的假设。

Method: 研究方法是GPT-2的机械可解释性研究，通过对所有12个层进行系统性的激活修补（activation patching），因果性地检验情感信息在不同层中的处理方式。

Result: 实验结果证实早期层（0-3）作为词汇情感检测器，编码稳定的、与位置相关的极性信号，且基本独立于上下文。然而，所有关于中间层上下文整合的假设（中间层集中、现象特异性、分布式处理）均被证伪。相反，上下文现象（如否定、讽刺、领域转移）主要通过统一的、非模块化机制在后期层（8-11）中进行整合。

Conclusion: 这些实验结果提供了因果证据，表明GPT-2的情感计算与预测的层次模式不同，强调需要进一步对大型语言模型中的上下文整合进行实证表征。
...
### [285] [TopiCLEAR: Topic extraction by CLustering Embeddings with Adaptive dimensional Reduction](https://arxiv.org/abs/2512.06694)
*Aoi Fujita,Taichi Yamamoto,Yuri Nakayama,Ryota Kobayashi*

Main category: cs.CL

TL;DR: 本文提出了一种名为TopiCLEAR的新方法，通过结合嵌入、自适应降维和迭代聚类，有效地从社交媒体短文本中提取主题，并在多项基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 社交媒体平台的快速发展带来了大量文本数据，但传统的主题建模方法难以处理短文本（如社交媒体帖子）的挑战，包括共现统计数据有限、语义碎片化、拼写不一致和非正式语言等问题。

Method: TopiCLEAR方法首先使用Sentence-BERT (SBERT) 对文本进行嵌入，然后使用高斯混合模型 (GMM) 进行初步聚类。接着，通过基于线性判别分析 (LDA) 的监督投影和GMM聚类进行迭代细化，直至收敛。该方法直接处理原始文本，无需停用词移除等预处理步骤。

Result: TopiCLEAR在20News、AgNewsTitle、Reddit和TweetTopic四个数据集上进行了评估，与七种基线方法（包括基于SBERT和零样本生成式AI的方法）相比，该方法在与人工标注主题的相似度方面取得了最高分，对社交媒体帖子和在线新闻文章均有显著改进。定性分析也表明，TopiCLEAR生成的主题更具可解释性。

Conclusion: TopiCLEAR是一种有效的主题提取方法，特别适用于社交媒体数据和网络内容分析，能够生成可解释的主题，并克服了传统主题模型在处理短文本时的局限性。
...
### [286] [Parameter-Efficient Fine-Tuning with Differential Privacy for Robust Instruction Adaptation in Large Language Models](https://arxiv.org/abs/2512.06711)
...
### [287] ["The Dentist is an involved parent, the bartender is not": Revealing Implicit Biases in QA with Implicit BBQ](https://arxiv.org/abs/2512.06732)
...
### [288] [A Patient-Doctor-NLP-System to contest inequality for less privileged](https://arxiv.org/abs/2512.06734)
...
### [289] [One Word Is Not Enough: Simple Prompts Improve Word Embeddings](https://arxiv.org/abs/2512.06744)
*Rajeev Ranjan*
...
### [290] [From Next-Token to Next-Block: A Principled Adaptation Path for Diffusion LLMs](https://arxiv.org/abs/2512.06776)
...
TL;DR: 本文提出了一种从自回归（AR）大语言模型到块级扩散语言模型（DLM）的有效且计算高效的适应方法，解决了传统AR解码速度慢和从头训练DLM成本高的问题，并实现了领先的性能。


<details>
  <summary>Details</summary>
Motivation: 自回归（AR）解码的顺序性导致吞吐量瓶颈。扩散语言模型（DLM），特别是块级变体，能够并行生成并进行块内双向推理，但从头训练大型DLM成本高昂且浪费了成熟AR检查点中的知识。现有的适应尝试未能有效解决AR因果性和块级双向性之间的根本不匹配。

Method: 将AR视为块大小为1的块级扩散模型，从而将适应过程重新定义为范式内的路径。具体方法包括：使用上下文因果注意力掩码（上下文中因果，仅在当前块内双向）、高效的并行适应程序、辅助AR损失以最大化数据利用并保留预训练知识，以及逐步增加生成块的大小。该方法与掩码块级扩散无缝集成并保持训练-推理一致性。

Result: 基于所提出的组件，NBDiff-7B（基础版和指令版）成功继承了长上下文建模和推理能力，并在7B级别的DLM中取得了最先进的性能，在通用知识、数学和代码基准测试中，相比强基线实现了显著提升。

Conclusion: 有原则的AR到块级扩散适应是一种有效且计算高效的替代方案，可以避免从头训练DLM，从而成功地将AR模型的知识和能力转移到块级扩散模型中。
...
### [291] [LLM4SFC: Sequential Function Chart Generation via Large Language Models](https://arxiv.org/abs/2512.06787)
*Ofek Glick,Vladimir Tchuiev,Marah Ghoummaid,Michal Moshkovitz,Dotan Di-Castro*

Main category: cs.CL

TL;DR: 本文提出了LLM4SFC，这是首个能将工业工作流的自然语言描述转换为可执行顺序功能图（SFCs）的框架，有效解决了图形化PLC语言的生成挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在生成结构化文本（ST）等文本PLC编程语言方面日益普及，但对顺序功能图（SFCs）等图形化IEC 61131-3标准语言的探索不足。SFCs的图形特性及其内嵌的ST动作使其难以直接兼容标准生成技术，常导致生成不可执行且与工业工具链不兼容的代码。

Method: LLM4SFC框架包含三个核心组件：(i) 一种简化的结构化表示，用于捕获关键拓扑、内联ST并减少文本冗余；(ii) 通过微调和少量检索增强生成（RAG）来与SFC编程约定对齐；(iii) 一种结构化生成方法，实时修剪非法标记，以确保符合SFC的文本格式。

Result: LLM4SFC在来自自动化制造项目的真实SFC数据集上进行了评估，使用开源和专有LLM均取得了75%至94%的生成成功率。结果表明，LLM4SFC能够可靠地生成语法有效的SFC程序，有效连接了图形和文本PLC语言。

Conclusion: LLM4SFC成功地为从自然语言描述生成可执行SFCs铺平了道路，实现了图形和文本PLC语言之间的桥接，为自动化工业编程奠定了基础。
...
### [292] [Becoming Experienced Judges: Selective Test-Time Learning for Evaluators](https://arxiv.org/abs/2512.06751)
...
### [293] [Large Language Model-Based Generation of Discharge Summaries](https://arxiv.org/abs/2512.06812)
*Tiago Rodrigues,Carla Teixeira Lopes*

Main category: cs.CL

TL;DR: 该研究探索使用大型语言模型（LLMs）自动生成出院总结。专有模型（尤其是Gemini）表现最佳，而开源模型（如Mistral微调后）虽有潜力但仍面临幻觉和重复信息等挑战。


<details>
  <summary>Details</summary>
Motivation: 出院总结包含对患者护理至关重要的信息。自动化生成出院总结可以显著减少医护人员的工作量，最大程度地减少错误，并确保关键患者信息易于获取和操作。

Method: 研究使用了五种大型语言模型（开源的Mistral、Llama 2和专有的GPT-3、GPT-4、Gemini 1.5 Pro），利用MIMIC-III的出院总结和病历数据进行实验。评估指标包括精确匹配、软重叠和无参考指标，并辅以临床专家的A/B测试人工评估。

Result: 专有模型，特别是采用单次提示的Gemini，表现优于其他模型，生成的总结与黄金标准具有最高相似度。开源模型（尤其是微调后的Mistral）虽有前景，但在性能上有所滞后，常出现幻觉和信息重复。临床专家的人工评估证实了专有模型生成总结的实用性。

Conclusion: 研究结果表明，尽管存在幻觉和信息缺失等挑战，但大型语言模型，尤其是专有模型，在确保数据隐私的前提下，是自动生成出院总结的有前途的候选方案。
...
### [294] [CAuSE: Decoding Multimodal Classifiers using Faithful Natural Language Explanation](https://arxiv.org/abs/2512.06814)
...
### [295] [AquaFusionNet: Lightweight VisionSensor Fusion Framework for Real-Time Pathogen Detection and Water Quality Anomaly Prediction on Edge Devices](https://arxiv.org/abs/2512.06848)
...
### [296] [An Analysis of Large Language Models for Simulating User Responses in Surveys](https://arxiv.org/abs/2512.06874)
...
### [297] [Rhea: Role-aware Heuristic Episodic Attention for Conversational LLMs](https://arxiv.org/abs/2512.06869)
...
### [298] [Large Language Models and Forensic Linguistics: Navigating Opportunities and Threats in the Age of Generative AI](https://arxiv.org/abs/2512.06922)
*George Mikros*
...
### [299] [XAM: Interactive Explainability for Authorship Attribution Models](https://arxiv.org/abs/2512.06924)
...
### [300] [Automated PRO-CTCAE Symptom Selection based on Prior Adverse Event Profiles](https://arxiv.org/abs/2512.06919)
...
### [301] [Prompting-in-a-Series: Psychology-Informed Contents and Embeddings for Personality Recognition With Decoder-Only Models](https://arxiv.org/abs/2512.06991)
...
### [302] [FVA-RAG: Falsification-Verification Alignment for Mitigating Sycophantic Hallucinations](https://arxiv.org/abs/2512.07015)
*Mayank Ravishankara*
...
### [303] [Progress Ratio Embeddings: An Impatience Signal for Robust Length Control in Neural Text Generation](https://arxiv.org/abs/2512.06938)
...
### [304] [Replicating TEMPEST at Scale: Multi-Turn Adversarial Attacks Against Trillion-Parameter Frontier Models](https://arxiv.org/abs/2512.07059)
*Richard Young*
...
### [305] [SETUP: Sentence-level English-To-Uniform Meaning Representation Parser](https://arxiv.org/abs/2512.07068)
...
### [306] [Do Large Language Models Truly Understand Cross-cultural Differences?](https://arxiv.org/abs/2512.07075)
*Shiwei Guo,Sihang Jiang,Qianxi He,Yanghua Xiao,Jiaqing Liang,Bi Yude,Minggui He,Shimin Tao,Li Zhang*

Main category: cs.CL

TL;DR: 本文提出了SAGE，一个基于场景的基准测试，通过跨文化核心概念对齐和生成任务设计，用于评估大型语言模型（LLMs）的跨文化理解和推理能力，并揭示了LLMs在此方面的系统性局限。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在多语言任务中表现出色，但现有评估其跨文化理解能力的基准存在局限性，包括缺乏情境场景、跨文化概念映射不足以及深层文化推理能力有限。因此，需要一个更全面的基准来填补这些空白。

Method: 本文提出了SAGE基准，基于文化理论将跨文化能力分为九个维度。通过策划210个核心概念，并在15个具体的真实场景下构建了4530个测试项，这些场景被组织成四大类跨文化情境。SAGE采用生成式任务设计，并支持持续扩展和跨语言迁移。

Result: 实验结果表明，SAGE能够揭示模型在不同维度和场景下的弱点，暴露了LLMs在跨文化推理方面的系统性局限。尽管LLMs有所进步，但离真正细致入微的跨文化理解仍有一定距离。

Conclusion: LLMs在跨文化理解和推理方面仍存在显著不足和系统性局限，需要进一步的研究和改进才能实现真正细致的跨文化理解。SAGE基准为评估和推动LLMs在此领域的发展提供了有价值的工具。
...
### [307] [Leveraging KV Similarity for Online Structured Pruning in LLMs](https://arxiv.org/abs/2512.07090)
...
### [308] [DART: Leveraging Multi-Agent Disagreement for Tool Recruitment in Multimodal Reasoning](https://arxiv.org/abs/2512.07132)
...
### [309] [GUMBridge: a Corpus for Varieties of Bridging Anaphora](https://arxiv.org/abs/2512.07134)
...
### [310] [MASim: Multilingual Agent-Based Simulation for Social Science](https://arxiv.org/abs/2512.07195)
...
### [311] [NeSTR: A Neuro-Symbolic Abductive Framework for Temporal Reasoning in Large Language Models](https://arxiv.org/abs/2512.07218)
...
### [312] [Ensembling LLM-Induced Decision Trees for Explainable and Robust Error Detection](https://arxiv.org/abs/2512.07246)
...
### [313] [TeluguST-46: A Benchmark Corpus and Comprehensive Evaluation for Telugu-English Speech Translation](https://arxiv.org/abs/2512.07265)
...
### [314] [Multilingual corpora for the study of new concepts in the social sciences and humanities:](https://arxiv.org/abs/2512.07367)
...
### [315] [Efficient ASR for Low-Resource Languages: Leveraging Cross-Lingual Unlabeled Data](https://arxiv.org/abs/2512.07277)
...
### [316] [Training Language Models to Use Prolog as a Tool](https://arxiv.org/abs/2512.07407)
...
### [317] [Investigating Training and Generalization in Faithful Self-Explanations of Large Language Models](https://arxiv.org/abs/2512.07288)
...
### [318] [Persian-Phi: Efficient Cross-Lingual Adaptation of Compact LLMs via Curriculum Learning](https://arxiv.org/abs/2512.07454)
...
### [319] [Enhancing Agentic RL with Progressive Reward Shaping and Value-based Sampling Policy Optimization](https://arxiv.org/abs/2512.07478)
...
### [320] [SPAD: Seven-Source Token Probability Attribution with Syntactic Aggregation for Detecting Hallucinations in RAG](https://arxiv.org/abs/2512.07515)
...
### [321] [Native Parallel Reasoner: Reasoning in Parallelism via Self-Distilled Reinforcement Learning](https://arxiv.org/abs/2512.07461)
...
### [322] [Beyond Real: Imaginary Extension of Rotary Position Embeddings for Long-Context LLMs](https://arxiv.org/abs/2512.07525)
...
### [323] [LIME: Making LLM Data More Efficient with Linguistic Metadata Embeddings](https://arxiv.org/abs/2512.07522)
...
### [324] [SwissGov-RSD: A Human-annotated, Cross-lingual Benchmark for Token-level Recognition of Semantic Differences Between Related Documents](https://arxiv.org/abs/2512.07538)
...
### [325] [Minimum Bayes Risk Decoding for Error Span Detection in Reference-Free Automatic Machine Translation Evaluation](https://arxiv.org/abs/2512.07540)
...
### [326] [Most over-representation of phonological features in basic vocabulary disappears when controlling for spatial and phylogenetic effects](https://arxiv.org/abs/2512.07543)
*Frederic Blum*
...
### [327] [MoCoRP: Modeling Consistent Relations between Persona and Response for Persona-based Dialogue](https://arxiv.org/abs/2512.07544)
...
### [328] [Performance of the SafeTerm AI-Based MedDRA Query System Against Standardised MedDRA Queries](https://arxiv.org/abs/2512.07552)
...
### [329] [A Simple Method to Enhance Pre-trained Language Models with Speech Tokens for Classification](https://arxiv.org/abs/2512.07571)
...
### [330] [Complementary Learning Approach for Text Classification using Large Language Models](https://arxiv.org/abs/2512.07583)
...
### [331] [Metric-Fair Prompting: Treating Similar Samples Similarly](https://arxiv.org/abs/2512.07608)
...
### [332] [PCMind-2.1-Kaiyuan-2B Technical Report](https://arxiv.org/abs/2512.07612)
...
### [333] [Bridging Code Graphs and Large Language Models for Better Code Understanding](https://arxiv.org/abs/2512.07666)
...
### [334] [When Large Language Models Do Not Work: Online Incivility Prediction through Graph Neural Networks](https://arxiv.org/abs/2512.07684)
*Zihan Chen,Lanyu Yu*
...
### [335] [HalluShift++: Bridging Language and Vision through Internal Representation Shifts for Hierarchical Hallucinations in MLLMs](https://arxiv.org/abs/2512.07687)
...
### [336] [Automated Generation of Custom MedDRA Queries Using SafeTerm Medical Map](https://arxiv.org/abs/2512.07694)
*Francois Vandenhende,Anna Georgiou,Michalis Georgiou,Theodoros Psaras,Ellie Karekla,Elena Hadjicosta*
...
### [337] [Mary, the Cheeseburger-Eating Vegetarian: Do LLMs Recognize Incoherence in Narratives?](https://arxiv.org/abs/2512.07777)
...
### [338] [On the Interplay of Pre-Training, Mid-Training, and RL on Reasoning Language Models](https://arxiv.org/abs/2512.07783)
...
### [339] [Collaborative Causal Sensemaking: Closing the Complementarity Gap in Human-AI Decision Support](https://arxiv.org/abs/2512.07801)
...
### [340] [Do Generalisation Results Generalise?](https://arxiv.org/abs/2512.07832)
...
### [341] [POrTAL: Plan-Orchestrated Tree Assembly for Lookahead](https://arxiv.org/abs/2512.06002)
*Evan Conway,David Porfirio,David Chan,Mark Roberts,Laura M. Hiatt*

Main category: cs.RO

TL;DR: 本文提出了一种名为POrTAL的轻量级概率规划算法，旨在帮助机器人在部分可观测环境中高效地制定计划，并在步数方面优于现有基线算法。


<details>
  <summary>Details</summary>
Motivation: 在人机交互的许多场景中，机器人所处的世界是部分可观测的，需要其在不确定性下制定计划。然而，现有的概率规划算法在机器人的有限计算资源下可能效率低下，或者需要比预期更多的步骤才能达到目标。

Method: 本文创建了一种名为POrTAL（Plan-Orchestrated Tree Assembly for Lookahead）的轻量级概率规划算法。该算法结合了两种基线规划算法FF-Replan和POMCP的优点。

Result: 通过一系列案例研究，POrTAL能够快速找到解决方案，并且在所需步骤数方面优于FF-Replan和POMCP这些基线算法。此外，POrTAL在不同的时间约束下也表现良好。

Conclusion: POrTAL是一种有效且高效的概率规划算法，能够帮助机器人在部分可观测环境中快速、以更少步骤实现目标。
...
### [342] [Training-Free Robot Pose Estimation using Off-the-Shelf Foundational Models](https://arxiv.org/abs/2512.06017)
*Laurence Liang*

Main category: cs.RO

TL;DR: 本文提出使用前沿视觉-语言模型（VLMs）作为即用工具，从单一图像估计机械臂的关节角度，并评估了其在合成和真实数据上的性能基线。


<details>
  <summary>Details</summary>
Motivation: 机械臂在工业和住宅应用中日益普及，可靠的关节角度估计对于提高安全性和性能至关重要，并可作为训练机器人策略的验证器。然而，从视觉输入估计机械臂姿态是一项具有挑战性的任务。

Method: 本文利用前沿VLMs作为“即用型”工具，从单个目标图像估计机械臂的关节角度。通过在合成和真实世界的图像-数据对上评估这些VLMs，建立了当前FLMs的性能基线。

Result: 本文建立了当前前沿视觉-语言模型（FLMs）在机械臂关节角度估计上的性能基线。此外，实证结果表明，单独的测试时间缩放或参数缩放并不能改善关节角度预测。

Conclusion: 前沿视觉-语言模型可以作为一种“即用型”工具来估计机械臂的关节角度，但单纯的测试时间或参数缩放不足以提高预测性能。
...
### [343] [Probabilistic Weapon Engagement Zones for a Turn Constrained Pursuer](https://arxiv.org/abs/2512.06130)
...
### [344] [Closed-Loop Robotic Manipulation of Transparent Substrates for Self-Driving Laboratories using Deep Learning Micro-Error Correction](https://arxiv.org/abs/2512.06038)
...
### [345] [WAM-Flow: Parallel Coarse-to-Fine Motion Planning via Discrete Flow Matching for Autonomous Driving](https://arxiv.org/abs/2512.06112)
...
### [346] [GuideNav: User-Informed Development of a Vision-Only Robotic Navigation Assistant For Blind Travelers](https://arxiv.org/abs/2512.06147)
...
### [347] [Situation-Aware Interactive MPC Switching for Autonomous Driving](https://arxiv.org/abs/2512.06182)
*Shuhao Qi,Qiling Aori,Luyao Zhang,Mircea Lazar,Sofie Haesaert*

Main category: cs.RO

TL;DR: 本文提出一种基于神经网络分类器的情境感知切换策略，用于在自动驾驶中动态选择不同交互能力的模型预测控制器（MPC），以平衡性能和计算成本。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶在交互式交通场景中面临挑战，不同MPC交互模型在智能行为和计算成本之间存在权衡。由于强交互不常见，需要一种实用策略来平衡性能和计算开销。

Method: 首先，对不同的MPC公式进行比较研究，评估并分层其交互能力。其次，开发一个基于神经网络的分类器，实现根据情境在不同交互能力控制器之间进行切换。

Result: 情境感知切换策略能够显著提高整体性能，在罕见但关键的情况下激活最先进的交互式MPC；同时，在大多数场景中使用基本MPC，显著降低了计算负荷。

Conclusion: 所提出的情境感知切换方法通过在关键时刻使用高级控制器并在常见场景中使用基本控制器，有效地提高了自动驾驶在交互场景中的整体性能并降低了计算负担。
...
### [348] [REWW-ARM -- Remote Wire-Driven Mobile Robot: Design, Control, and Experimental Validation](https://arxiv.org/abs/2512.06192)
*Takahiro Hattori,Kento Kawaharazuka,Temma Suzuki,Keita Yoneda,Kei Okada*

Main category: cs.RO

TL;DR: 本文提出一种名为“远程线驱动”的新型机器人系统，通过结合远程液压驱动和长线驱动的优点，实现了无电子设备的远端机器人，并在陆地和水下环境中展示了其运动、姿态控制和物体操纵能力，有望扩展机器人的操作范围。


<details>
  <summary>Details</summary>
Motivation: 电子设备限制了机器人的可用环境。现有的解决方案（如远程液压驱动和长线驱动）虽然各有优势，但存在局限性。研究旨在结合两者的优点，开发一种兼具高可达性和高环境耐受性的机器人系统。

Method: 研究提出“远程线驱动”系统，并开发了概念验证机器人“REWW-ARM”。该机器人包含：1) 核心技术“远程线传输机制”（RWTM）；2) 由RWTM驱动的无电子设备远端移动机器人；3) 负责提供动力和基于RWTM状态估计进行闭环控制的电机单元。通过实验评估了REWW-ARM的机械和控制性能。

Result: REWW-ARM在陆地和水下环境中均成功展示了其运动、姿态控制和物体操纵能力。

Conclusion: 远程线驱动系统具有应用于各种类型机器人的潜力，从而显著扩展其操作范围。
...
### [349] [Cascaded Tightly-Coupled Observer Design for Single-Range-Aided Inertial Navigation](https://arxiv.org/abs/2512.06198)
*Oussama Sifour,Soulaimane Berkane,Abdelhamid Tayebi*

Main category: cs.RO

TL;DR: 本文提出了一种单距离辅助导航观测器，结合惯性测量单元（IMU）、机体坐标系矢量测量和固定锚点的距离测量，重建刚体的完整状态（位置、速度、姿态）。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一种轻量级且有效的自主导航方案，仅使用单距离辅助作为传感器输入。

Method: 该方法采用级联观测器架构：首先，建立一个扩展的线性时变（LTV）系统来估计机体坐标系下的位置、速度和重力方向；其次，利用恢复的重力方向和机体坐标系矢量测量重建完整的SO(3)姿态。该级联设计的几乎全局渐近稳定性（AGAS）在均匀可观测条件下得到确立。

Result: 三维轨迹仿真研究表明，该观测器能够准确估计位置、速度和姿态，并对传感器噪声和轨迹变化具有鲁棒性。

Conclusion: 单距离辅助是一种轻量级且有效的自主导航方式，能够实现准确的完整状态估计。
...
### [350] [Where to Fly, What to Send: Communication-Aware Aerial Support for Ground Robots](https://arxiv.org/abs/2512.06207)
*Harshil Suthar,Dipankar Maity*

Main category: cs.RO

TL;DR: 本文提出一个多机器人系统框架，其中一架空中机器人负责在未知、带宽受限的环境中进行地图绘制，并智能地决定何时、传输多少以及传输哪些地图信息给地面机器人，以帮助它们导航至目标，同时平衡通信量与导航成本。


<details>
  <summary>Details</summary>
Motivation: 在未知且通信带宽受限的环境中，一个空中机器人需要绘制地图并将其部分信息传输给地面机器人以帮助其导航。挑战在于如何决定何时、传输多少以及传输哪些信息，同时空中机器人也要进行探索和测绘。

Method: 所提出的框架使空中机器人能够：基于信息价值（VoI）决定传输“什么”信息；使用混合整数线性规划（MILP）决定传输“多少”信息；通过基于效用分数的环境探索策略获取额外信息。此外，还对空中机器人传输的地图数据总量与地面机器人导航成本之间进行了通信-运动权衡分析。

Result: 该框架使辅助空中机器人能够智能地决定要传输的信息、传输量以及如何进行探索，并能够对空中机器人通信的地图数据总量与地面机器人产生的导航成本之间进行通信-运动权衡分析。

Conclusion: 所提出的框架为在带宽受限的未知环境中操作的多机器人团队提供了一种有效的方法，使空中代理能够智能地管理地图绘制和信息传输，以支持地面代理的导航，并通过通信-运动权衡分析优化整体操作。
...
### [351] [Safe Model Predictive Diffusion with Shielding](https://arxiv.org/abs/2512.06261)
...
### [352] [Real-Time Spatiotemporal Tubes for Dynamic Unsafe Sets](https://arxiv.org/abs/2512.06151)
...
### [353] [Leveraging Port-Hamiltonian Theory for Impedance Control Benchmarking](https://arxiv.org/abs/2512.06423)
...
### [354] [Fault Tolerant Control of Mecanum Wheeled Mobile Robots](https://arxiv.org/abs/2512.06444)
...
### [355] [Vision-Guided Grasp Planning for Prosthetic Hands in Unstructured Environments](https://arxiv.org/abs/2512.06517)
*Shifa Sulaiman,Akash Bachhar,Ming Shen,Simon Bøgh*

Main category: cs.RO

TL;DR: 本文提出了一种视觉引导的假肢手抓取算法，通过整合感知、规划和控制，实现灵巧操作，支持实时适应非结构化环境。


<details>
  <summary>Details</summary>
Motivation: 当前假肢技术日益关注通过智能控制提升灵巧性和自主性。基于视觉的方法在使假肢手与动态环境中多样物体进行自然交互方面展现出巨大潜力。

Method: 该方法通过摄像头捕捉场景，利用基于边界体积层次（BVH）的视觉算法分割物体并定义其边界框。然后，使用快速探索随机树星（RRT*）算法生成候选轨迹，并基于轨迹与物体点云之间的最小欧几里得距离选择指尖末端姿态来计算抓取接触点。每个手指的抓取姿态独立确定，通过阻尼最小二乘（DLS）逆运动学求解器计算相应的关节角度，并传输给手指执行器。这是一个模块化管道，支持按手指抓取规划。

Result: 所提出的方法在仿真中得到了验证，并在Linker Hand O7平台上进行了实验集成，展示了在非结构化环境中的实时适应性。

Conclusion: 本文开发并验证了一个模块化的视觉引导抓取管道，该管道能够实现按手指的抓取规划，并支持假肢手在非结构化环境中的实时适应性，从而实现灵巧操作。
...
### [356] [TacFinRay: Soft Tactile Fin-Ray Finger with Indirect Tactile Sensing for Robust Grasping](https://arxiv.org/abs/2512.06524)
*Saekwang Nam,Bowen Deng,Loong Yi Lee,Jonathan M. Rossiter,Nathan F. Lepora*

Main category: cs.RO

TL;DR: 本文提出了一种触觉传感化的Fin-Ray手指，通过间接传感方法，利用内部摄像头和卷积神经网络，同时检测接触位置和压痕深度，并在抓取任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为软体机器人结构提供一种轻量、灵活、可扩展的触觉传感解决方案，尤其适用于传感模块需远离接触界面的场景。

Method: 设计了一种集成铰链机制的Fin-Ray手指，将软体Fin-Ray结构的变形和位移信息传递给底部横梁。横梁上排列有受TacTip启发、带有标记尖端的销钉阵列。内部摄像头捕捉销钉的变形模式，并使用卷积神经网络处理这些模式，以推断接触条件（位置和深度）。通过调整销钉配置和铰链方向优化了手指设计。

Result: 实现了0.1毫米的深度传感精度和2毫米的位置传感精度。该感知系统对各种压头形状和尺寸表现出强大的泛化能力。在抓取和放置任务中，当抓取位置不确定时，触觉反馈显著提高了放置精度。

Conclusion: 该工作为软体机器人结构提供了一种轻量、灵活且可扩展的触觉传感解决方案，特别适用于传感需求远离接触界面的应用场景。
...
### [357] [Entropy-Controlled Intrinsic Motivation Reinforcement Learning for Quadruped Robot Locomotion in Complex Terrains](https://arxiv.org/abs/2512.06486)
*Wanru Gong,Xinyi Zheng,Xiaopeng Yang,Xiaoqing Zhu*

Main category: cs.RO

TL;DR: 本文提出了一种名为熵控内在动机（ECIM）的强化学习算法，通过结合内在动机和自适应探索，有效解决了PPO系列算法在四足机器人步态训练中过早收敛的问题，显著提高了在多种地形上的稳定性和能效。


<details>
  <summary>Details</summary>
Motivation: 经典的PPO系列深度强化学习算法在训练四足机器人步态策略时，虽然稳定且样本效率高，但实验和仿真常过早收敛，导致步态次优和任务性能下降。

Method: 引入了熵控内在动机（ECIM），这是一种基于熵的强化学习算法，与PPO系列算法不同，它通过结合内在动机和自适应探索来减少过早收敛。

Result: 在Isaac Gym的六种地形（上坡、下坡、崎岖不平、上楼梯、下楼梯、平地）上进行实验，ECIM始终表现更优：任务奖励增加4-12%，身体俯仰峰值摆动减少23-29%，关节加速度降低20-32%，关节扭矩消耗减少11-20%。

Conclusion: ECIM模型通过结合熵控制和内在动机控制，在四足机器人步态控制中实现了更好的跨地形稳定性，同时降低了能耗，使其成为复杂机器人控制任务的实用选择。
...
### [358] [Embodied Referring Expression Comprehension in Human-Robot Interaction](https://arxiv.org/abs/2512.06558)
*Md Mofijul Islam,Alexi Gladstone,Sujan Sarker,Ganesh Nanduru,Md Fahim,Keyan Du,Aman Chadha,Tariq Iqbal*

Main category: cs.RO

TL;DR: 该研究提出了Refer360数据集，一个包含室内外多视角具身指令的综合数据集，并引入了MuRes模块以增强机器人对具身指令的理解能力，实验证明其能显著提升现有模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着机器人进入人类工作空间，它们需要理解具身指令以实现直观流畅的人机交互。然而，由于缺乏大规模、多视角、包含非语言手势且覆盖多样环境的具身交互数据集，准确理解具身指令面临挑战。

Method: 研究构建了Refer360数据集，一个大规模的具身语言和非语言交互数据集，其数据在室内外多样视角下采集。此外，提出了一种多模态引导残差模块MuRes，旨在通过提取显着的多模态特定信号并将其强化到预训练表示中，形成互补特征，从而改善具身指代表达理解。

Result: 在包括Refer360在内的四个HRI数据集上进行了广泛实验，结果表明当前多模态模型未能全面捕捉具身交互，但使用MuRes增强后，模型性能得到持续提升。这确立了Refer360作为一个有价值的基准，并展示了引导残差学习在推进机器人具身指代表达理解方面的潜力。

Conclusion: Refer360数据集和MuRes模块为机器人理解人类环境中具身指代表达提供了有力的基准和有效的方法，有望推动人机交互领域的发展。
...
### [359] [Learning Agile Striker Skills for Humanoid Soccer Robots from Noisy Sensory Input](https://arxiv.org/abs/2512.06571)
...
TL;DR: 本文提出一个基于强化学习的系统，使人形机器人能够在不完善感知下实现鲁棒的连续踢球，并适应不同的球门配置。


<details>
  <summary>Details</summary>
Motivation: 人形机器人学习快速且鲁棒的踢球技能至关重要，但由于需要快速摆腿、单脚支撑时的姿态稳定性以及在噪声感知和外部扰动下的鲁棒性，这仍然是一个具有挑战性的问题。

Method: 该研究提出了一个基于强化学习的系统，通过扩展典型的“教师-学生”训练框架来实现。该框架包含四个训练阶段：长距离追球（教师）、定向踢球（教师）、教师策略蒸馏（学生）以及学生适应和优化（学生）。关键设计元素包括定制的奖励函数、真实的噪声建模以及用于适应和优化的在线约束强化学习，这些对于弥合虚实差距和维持感知不确定性下的性能至关重要。

Result: 在仿真和真实机器人上的广泛评估表明，该系统在各种球门配置下均展现出强大的踢球精度和进球成功率。消融研究进一步强调了约束强化学习、噪声建模和适应阶段的必要性。

Conclusion: 该工作提出了一个在不完善感知下学习鲁棒连续人形踢球的系统，为人形全身控制中的视觉运动技能学习建立了一个基准任务。
...
### [360] [Error-Centric PID Untrained Neural-Net (EC-PIDUNN) For Nonlinear Robotics Control](https://arxiv.org/abs/2512.06578)
*Waleed Razzaq*
...
### [361] [A New Trajectory-Oriented Approach to Enhancing Comprehensive Crowd Navigation Performance](https://arxiv.org/abs/2512.06608)
...
### [362] [Robust Optimization-based Autonomous Dynamic Soaring with a Fixed-Wing UAV](https://arxiv.org/abs/2512.06610)
...
### [363] [MIND-V: Hierarchical Video Generation for Long-Horizon Robotic Manipulation with RL-based Physical Alignment](https://arxiv.org/abs/2512.06628)
...
### [364] [Statistic-Augmented, Decoupled MoE Routing and Aggregating in Autonomous Driving](https://arxiv.org/abs/2512.06664)
...
### [365] [FedDSR: Federated Deep Supervision and Regularization Towards Autonomous Driving](https://arxiv.org/abs/2512.06676)
*Wei-Bin Kou,Guangxu Zhu,Bingyang Cheng,Chen Zhang,Yik-Chung Wu,Jianping Wang*

Main category: cs.RO

TL;DR: 本文提出了联邦深度监督与正则化（FedDSR）范式，通过在联邦自动驾驶（AD）系统中引入多层中间层监督和正则化，以解决非独立同分布（non-IID）数据导致的泛化能力差和收敛慢的问题，显著提升了模型性能和训练效率。


<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）在自动驾驶模型训练中面临两大挑战：由于来自不同驾驶环境的非独立同分布（non-IID）数据，导致模型泛化能力差和收敛速度慢，这严重阻碍了其在联邦自动驾驶系统中的实际应用。

Method: FedDSR包含以下策略：(I) 基于预定义的架构无关标准选择多个中间层。(II) 在这些选定的层上计算互信息（MI）和负熵（NE），作为中间损失和正则项，并将其集成到输出层损失中，形成统一的优化目标。(III) 将根据上述规则训练的车辆模型在中央服务器上进行聚合，生成全局模型。通过指导和惩罚中间阶段的特征表示学习，FedDSR旨在增强模型泛化能力并加速模型收敛。以语义分割任务为例，并在多种模型架构和FL算法上进行了评估。

Result: 实验结果表明，与现有的联邦学习基线相比，FedDSR在平均交并比（mIoU）上最高提升了8.93%，训练轮次减少了28.57%。

Conclusion: FedDSR通过深度监督和正则化有效解决了联邦自动驾驶中非独立同分布数据带来的挑战，显著提升了模型泛化能力和收敛速度，使其非常适用于联邦自动驾驶生态系统的实际部署。
...
### [366] [Model-Less Feedback Control of Space-based Continuum Manipulators using Backbone Tension Optimization](https://arxiv.org/abs/2512.06754)
...
### [367] [MagicSkin: Balancing Marker and Markerless Modes in Vision-Based Tactile Sensors with a Translucent Skin](https://arxiv.org/abs/2512.06829)
*Oluwatimilehin Tijani,Zhuo Chen,Jiankang Deng,Shan Luo*

Main category: cs.RO

TL;DR: MagicSkin是一种新型半透明触觉皮肤，解决了视觉触觉传感器中标记和无标记设计的权衡问题，实现了同时的切向位移跟踪、力预测和表面细节保留，显著提升了多模态感知性能。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉触觉传感器（VBTS）在标记和无标记皮肤设计之间存在根本性权衡：标记能测量力和切向位移但遮挡几何特征，而无标记皮肤保留表面细节但在测量切向位移方面效果不佳。现有解决方案（如UV照明或基于学习的虚拟传输）会增加硬件复杂性或计算负担。

Method: 本文引入了MagicSkin，一种新型触觉皮肤，其特点是采用半透明、着色标记。这种设计平衡了标记和无标记模式，使其能够同时实现切向位移跟踪、力预测和表面细节保留。MagicSkin易于集成到GelSight系列传感器中，无需额外的硬件或软件工具。

Result: MagicSkin在下游任务中表现出色，其半透明标记显著提升了传感性能：物体分类达到99.17%，纹理分类达到93.51%，切向位移跟踪保持97%的点，力预测的总力误差改善了66%。这些结果表明半透明皮肤优于传统的无标记和墨迹标记设计。

Conclusion: MagicSkin的半透明皮肤消除了标记和无标记模式之间的传统性能权衡，为触觉机器人中多模态触觉传感的发展铺平了道路。
...
### [368] [db-LaCAM: Fast and Scalable Multi-Robot Kinodynamic Motion Planning with Discontinuity-Bounded Search and Lightweight MAPF](https://arxiv.org/abs/2512.06796)
...
### [369] [Control of Powered Ankle-Foot Prostheses on Compliant Terrain: A Quantitative Approach to Stability Enhancement](https://arxiv.org/abs/2512.06896)
*Chrysostomos Karakasis,Camryn Scully,Robert Salati,Panagiotis Artemiadis*

Main category: cs.RO

TL;DR: 该研究实验验证了一种基于导纳的控制策略，通过动态调整动力假肢的准刚度，提高了在软性地面上步态的稳定性，从而降低了截肢者的跌倒风险。


<details>
  <summary>Details</summary>
Motivation: 截肢者在柔软地形上行走面临巨大挑战，跌倒风险高。现有动力踝足假肢虽能适应不同速度和刚性地形，但针对软性或顺应性地面的控制策略尚未得到充分探索。

Method: 本研究开发了一种基于导纳的控制策略，可动态调整动力假肢的准刚度。通过人体受试者实验（三名健康个体）在两种具有代表性软环境（地面刚度分别为63和25 kN/m）的双边顺应性地面上进行测试。控制器的性能通过相图和两种步态稳定性指标进行量化，并与为刚性地形开发的标准相变量控制器进行比较。

Result: 与为刚性地形设计的标准相变量控制器相比，所提出的导纳控制器在所有顺应性条件下均持续改善了步态稳定性。

Conclusion: 这些结果表明，自适应、稳定性感知的假肢控制有潜力降低现实环境中截肢者的跌倒风险，并提升康复机器人领域人机交互的鲁棒性。
...
### [370] [Dynamic Visual SLAM using a General 3D Prior](https://arxiv.org/abs/2512.06868)
...
### [371] [From Zero to High-Speed Racing: An Autonomous Racing Stack](https://arxiv.org/abs/2512.06892)
...
### [372] [Energy-Efficient Navigation for Surface Vehicles in Vortical Flow Fields](https://arxiv.org/abs/2512.06912)
...
### [373] [VideoVLA: Video Generators Can Be Generalizable Robot Manipulators](https://arxiv.org/abs/2512.06963)
*Yichao Shen,Fangyun Wei,Zhiying Du,Yaobo Liang,Yan Lu,Jiaolong Yang,Nanning Zheng,Baining Guo*

Main category: cs.RO

TL;DR: VideoVLA将大型视频生成模型转化为机器人VLA操纵器，通过预测动作序列和未来视觉结果，在机器人操作中实现了强大的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型在泛化到新任务、新物体和新场景方面能力有限，而这对于机器人在开放世界部署和迈向通用人工智能至关重要。

Method: VideoVLA采用多模态Diffusion Transformer架构，利用预训练的视频生成模型，共同建模视频、语言和动作模态。给定语言指令和图像，它同时预测动作序列和未来的视觉结果。

Result: 实验表明，高质量的“想象未来”与可靠的动作预测和任务成功率高度相关，突显了视觉想象在操作中的重要性。VideoVLA展示了强大的泛化能力，包括模仿其他实体的技能和处理新物体。

Conclusion: 这种预测动作及其视觉后果的双重预测策略，代表了机器人学习范式的一次转变，并为操作系统带来了强大的泛化能力，强调了视觉想象的关键作用。
...
### [374] [Task adaptation of Vision-Language-Action model: 1st Place Solution for the 2025 BEHAVIOR Challenge](https://arxiv.org/abs/2512.06951)
...
### [375] [Ground Compliance Improves Retention of Visual Feedback-Based Propulsion Training for Gait Rehabilitation](https://arxiv.org/abs/2512.06897)
*Bradley Hobbs,Panagiotis Artemiadis*

Main category: cs.RO

TL;DR: 本研究发现，在步态训练中结合视觉反馈和地面顺应性比单独使用视觉反馈更能有效且持久地增加推蹬力，对步态康复有重要意义。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探讨在视觉反馈（VF）步态训练中加入地面顺应性是否比单独使用VF更能有效增加推蹬力（POF），从而为步态康复提供新的思路。

Method: 十名健康参与者在定制的分体式跑步机上行走。所有参与者都获得实时地面反作用力视觉反馈。其中一组同时体验地面顺应性变化，而对照组仅接受视觉反馈。

Result: 两组均成功实现并维持了推蹬力的增加，尤其是在体验地面顺应性的组中效果更显著。该组还在肌肉活动和关节运动学方面表现出持久的后效应，表明对增加推进力的自然策略学习更稳健。

Conclusion: 研究表明视觉和本体感觉系统在步态适应中的协调作用。结合地面顺应性与视觉反馈能增强推进力的学习，支持在针对推进力缺陷（如中风后）的长期康复中使用顺应性地形。
...
### [376] [Interconnection and Damping Assignment Passivity-Based Control using Sparse Neural ODEs](https://arxiv.org/abs/2512.06935)
...
### [377] [Parametric Design of a Cable-Driven Coaxial Spherical Parallel Mechanism for Ultrasound Scans](https://arxiv.org/abs/2512.06995)
...
### [378] [A Hetero-Associative Sequential Memory Model Utilizing Neuromorphic Signals: Validated on a Mobile Manipulator](https://arxiv.org/abs/2512.07032)
...
### [379] [Surrogate compliance modeling enables reinforcement learned locomotion gaits for soft robots](https://arxiv.org/abs/2512.07114)
...
### [380] [CERNet: Class-Embedding Predictive-Coding RNN for Unified Robot Motion, Recognition, and Confidence Estimation](https://arxiv.org/abs/2512.07041)
...
### [381] [SINRL: Socially Integrated Navigation with Reinforcement Learning using Spiking Neural Networks](https://arxiv.org/abs/2512.07266)
...
### [382] [A Flexible Funnel-Shaped Robotic Hand with an Integrated Single-Sheet Valve for Milligram-Scale Powder Handling](https://arxiv.org/abs/2512.07091)
...
### [383] [Mimir: Hierarchical Goal-Driven Diffusion with Uncertainty Propagation for End-to-End Autonomous Driving](https://arxiv.org/abs/2512.07130)
*Zebin Xing,Yupeng Zheng,Qichao Zhang,Zhixing Ding,Pengxuan Yang,Songen Gu,Zhongpu Xia,Dongbin Zhao*

Main category: cs.RO

TL;DR: Mimir是一种新颖的分层双系统框架，通过拉普拉斯分布估计目标点不确定性并引入多速率引导机制，显著提高了端到端自动驾驶的鲁棒性和高层模块推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法受限于高层引导信号的不准确性和复杂引导模块带来的计算开销，导致其性能受限。

Method: 本文提出Mimir框架，采用分层双系统结构：1) 使用拉普拉斯分布估计目标点不确定性，以增强鲁棒性，而非确定性建模；2) 引入多速率引导机制，提前预测扩展目标点，以解决引导系统推理速度慢的问题。

Result: 在Navhard和Navtest基准测试中，Mimir的驾驶分数EPDMS比现有最佳方法提高了20%，同时在高层模块推理速度上提升了1.6倍，且未牺牲准确性。

Conclusion: Mimir框架通过有效处理不确定性和提高推理效率，为端到端自动驾驶生成了更鲁棒的轨迹，并在具有挑战性的基准测试中超越了现有先进方法。
...
### [384] [Model Predictive Control for Cooperative Docking Between Autonomous Surface Vehicles with Disturbance Rejection](https://arxiv.org/abs/2512.07316)
...
### [385] [Time-Varying Formation Tracking Control of Wheeled Mobile Robots With Region Constraint: A Generalized Udwadia-Kalaba Framework](https://arxiv.org/abs/2512.07137)
...
### [386] [Using Vision-Language Models as Proxies for Social Intelligence in Human-Robot Interaction](https://arxiv.org/abs/2512.07177)
...
### [387] [Spatiotemporal Calibration and Ground Truth Estimation for High-Precision SLAM Benchmarking in Extended Reality](https://arxiv.org/abs/2512.07221)
...
### [388] [Multi-Rigid-Body Approximation of Human Hands with Application to Digital Twin](https://arxiv.org/abs/2512.07359)
...
### [389] [Efficient Computation of a Continuous Topological Model of the Configuration Space of Tethered Mobile Robots](https://arxiv.org/abs/2512.07303)
*Gianpietro Battocletti,Dimitris Boskos,Bart De Schutter*
...
### [390] [ESPADA: Execution Speedup via Semantics Aware Demonstration Data Downsampling for Imitation Learning](https://arxiv.org/abs/2512.07371)
...
### [391] [Gait-Adaptive Perceptive Humanoid Locomotion with Real-Time Under-Base Terrain Reconstruction](https://arxiv.org/abs/2512.07464)
...
### [392] [Affordance Field Intervention: Enabling VLAs to Escape Memory Traps in Robotic Manipulation](https://arxiv.org/abs/2512.07472)
...
### [393] [From Real-World Traffic Data to Relevant Critical Scenarios](https://arxiv.org/abs/2512.07482)
...
### [394] [VP-AutoTest: A Virtual-Physical Fusion Autonomous Driving Testing Platform](https://arxiv.org/abs/2512.07507)
...
### [395] [See Once, Then Act: Vision-Language-Action Model with Task Learning from One-Shot Video Demonstrations](https://arxiv.org/abs/2512.07582)
...
### [396] [Multi-Domain Motion Embedding: Expressive Real-Time Mimicry for Legged Robots](https://arxiv.org/abs/2512.07673)
...
### [397] [AMBER: Aerial deployable gripping crawler with compliant microspine for canopy manipulation](https://arxiv.org/abs/2512.07680)
...
### [398] [Toward Seamless Physical Human-Humanoid Interaction: Insights from Control, Intent, and Modeling with a Vision for What Comes Next](https://arxiv.org/abs/2512.07765)
...
### [399] [Delay-Aware Diffusion Policy: Bridging the Observation-Execution Gap in Dynamic Tasks](https://arxiv.org/abs/2512.07697)
...
### [400] [OptMap: Geometric Map Distillation via Submodular Maximization](https://arxiv.org/abs/2512.07775)
...
### [401] [Inchworm-Inspired Soft Robot with Groove-Guided Locomotion](https://arxiv.org/abs/2512.07813)
...
### [402] [Efficient and Compliant Control Framework for Versatile Human-Humanoid Collaborative Transportation](https://arxiv.org/abs/2512.07819)
...
### [403] [A Nonlinear Observer for Air-Velocity and Attitude Estimation Using Pitot and Barometric Measurements](https://arxiv.org/abs/2512.06133)
*Melone Nyoba Tchonkeu,Soulaimane Berkane,Tarek Hamel*

Main category: eess.SY

TL;DR: 本文提出了一种在无GNSS环境下，利用最少机载传感器估计无人机空速和完整姿态的方法，并进行了可观测性分析和非线性观测器设计。


<details>
  <summary>Details</summary>
Motivation: 在GNSS受限环境中，无人机的导航面临挑战，需要利用最少的机载传感器进行空速和姿态估计，这是一个具有实际意义的问题。

Method: 1. 进行了可观测性分析，建立了均匀可观测性的条件，为无人机轨迹规划和运动控制提供了依据。2. 设计了一个基于SO3R3R的非线性观测器，将皮托管、气压高度计和磁力计测量作为输出，并将IMU数据作为输入，整合在一个统一的框架中。

Result: 仿真结果证实了所提出设计的收敛性和鲁棒性，即使在最小激励轨迹下也能表现良好。

Conclusion: 该研究通过可观测性分析和非线性观测器设计，有效解决了在无GNSS环境下利用最少传感器估计无人机空速和姿态的问题，并得到了仿真验证。
...
### [404] [Sparse Neural Approximations for Bilevel Adversarial Problems in Power Grids](https://arxiv.org/abs/2512.06187)
*Young-ho Cho,Harsha Nagarajan,Deepjyoti Deka,Hao Zhu*

Main category: eess.SY

TL;DR: 本文提出了一种新颖的数据驱动神经网络代理和物理约束神经网络（PCNN）方法，用于解决对抗性最坏情况负荷削减（AWLS）问题，通过将双层问题重构为单层凸模型，实现了在保证精度的同时，显著提高大规模电力系统偶发事件分析的计算速度和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统的对抗性最坏情况负荷削减（AWLS）问题是一个双层混合整数规划，涉及组合数量的拓扑和交流潮流的非凸性，导致计算成本过高，难以有效识别线路故障下的关键偶发事件。

Method: 研究者将AWLS问题重构为单层最优值函数（OVF）模型，并利用数据驱动的神经网络（NN）作为下层最优值的代理。为确保物理可行性，将训练好的代理嵌入到物理约束神经网络（PCNN）中，将OVF不等式与（松弛的）交流可行性相结合，形成一个混合整数凸模型。为实现可扩展性，通过谱聚类学习了稀疏、区域划分的神经网络，构建了块稀疏架构。

Result: 该方法能够生成接近最优的最坏情况故障，并能推广到不同的负荷条件和未见过的拓扑结构，支持快速在线重计算。在IEEE 14和118节点系统上的数值实验表明，该方法在大规模偶发事件分析中具有良好的可扩展性和求解质量，与传统方法相比，平均最优性差距为5.8%，而计算时间保持在一分钟以内。

Conclusion: 本文提出的方法通过结合最优值函数重构、数据驱动神经网络代理和物理约束，为解决AWLS问题提供了一个可扩展、准确且高效的解决方案，显著克服了传统方法的计算瓶颈，适用于大规模偶发事件分析。
...
### [405] [Explainable LP-MPC: Shadow Price Contributions Reveal MV-CV Pairings](https://arxiv.org/abs/2512.06194)
*Lim C. Siang,Daniel L. O'Connor*

Main category: eess.SY

TL;DR: 本文提出一种新颖的后验LP可解释性方法，通过将影子价格重塑为MV-CV关系的归因机制，并利用线性求和分配解决MV-CV配对问题，以诊断大型工业LP-MPC系统中的异常行为。


<details>
  <summary>Details</summary>
Motivation: 大型工业LP-MPC系统中的异常行为难以解释，即使LP模型是白盒模型。研究旨在提供一种实用工具，帮助诊断次优约束并验证控制器行为是否符合设计意图和历史约束。

Method: 将LP解中的影子价格重塑为MV-CV关系的归因机制。核心思想是CV影子价格是MV在执行CV约束时贡献的成本敏感度的总和，然后通过线性求和分配解决方案将其解析为一对一的MV-CV配对。

Result: 所提出的MV-CV配对框架作为在线LP-MPC系统的实用可解释性工具，能够帮助从业者诊断次优约束。

Conclusion: 该方法使从业者能够诊断次优约束，并验证控制器行为是否与其原始设计意图和历史约束保持一致。
...
### [406] [Variable L0 Guidance Strategy: Enlarged Operational Envelope and Path-Following](https://arxiv.org/abs/2512.06124)
...
### [407] [A Physics-Informed Fixed Skyroad Model for Continuous UAS Traffic Management (C-UTM)](https://arxiv.org/abs/2512.06268)
...
### [408] [Scale-free weak output synchronization of multi-agent systems with adaptive protocols](https://arxiv.org/abs/2512.06278)
...
### [409] [Distributionally Robust Kalman Filter](https://arxiv.org/abs/2512.06286)
*Minhyuk Jang,Astghik Hakobyan,Insoon Yang*

Main category: eess.SY

TL;DR: 本文提出了一种以噪声为中心的分布鲁棒卡尔曼滤波器（DRKF）新公式，用于噪声统计不确定的离散时间线性随机系统。该DRKF保留了经典卡尔曼滤波器的结构，提供了协方差的先验谱界，并在时不变设置中推导了稳态解，具有与标准卡尔曼滤波器相同的计算复杂度，并实现了渐近极小极大最优性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决离散时间线性随机系统中噪声统计数据不确定或未知时，传统卡尔曼滤波器性能下降的问题。

Method: 该研究通过直接在过程噪声和测量噪声分布上放置Wasserstein模糊集，提出了一种以噪声为中心的分布鲁棒卡尔曼滤波器（DRKF）公式。在时不变设置中，通过一个单一的平稳半正定规划推导出了稳态DRKF。

Result: 所提出的DRKF保留了经典卡尔曼滤波器的分析结构，并为所有可行协方差提供了先验谱界。在时不变设置中，推导出的稳态DRKF是一个常增益估计器，其每步计算复杂度与标准卡尔曼滤波器相同。研究建立了保证该稳态解存在性、唯一性和收敛性的条件，并证明了其在最坏情况均方误差方面的渐近极小极大最优性。数值实验验证了理论，并表明该DRKF在未知或不确定噪声模型下提高了估计精度，并比现有鲁棒和分布鲁棒滤波器具有计算优势。

Conclusion: 所提出的DRKF在噪声模型不确定或未知的情况下，能够有效提高估计精度，同时保持与标准卡尔曼滤波器相当的计算效率，并具有理论上的最优性保证，优于现有鲁棒滤波器。
...
### [410] [Optimal Platoon Formation and Stable Benefit Allocation in Mixed-Energy Truck Fleets under Size Limitations](https://arxiv.org/abs/2512.06283)
*Ting Bai,Xinfeng Ru,Andreas A. Malikopoulos*

Main category: eess.SY

TL;DR: 本文研究了混合动力卡车车队（电动和燃油卡车）中的合作式车队编队形成和收益分配问题，考虑了车队规模限制，并提出了最优的编队结构和稳定的收益分配方案。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于车队规模限制带来的挑战，该限制了每个车队中卡车的数量，并引入了组合耦合，使得寻找最优车队编队结构变得复杂。

Method: 将问题建模为具有有限联盟规模的联盟博弈，推导了最大化车队整体编队收益的最优联盟结构的闭式表征。在此基础上，开发了一种基于类型的最小核心支付分配方案，以确保联盟结构核心（CS-core）内的稳定性。当CS-core为空时，计算最小核心半径以确定实现近似稳定性所需的最小松弛。

Result: 数值研究表明，所提出的框架在所有可行编队配置中始终实现最高的总编队收益，并提供优于现有基线方法的稳定收益分配。

Conclusion: 该研究成功解决了混合动力卡车车队在车队规模限制下的合作式车队编队形成和收益分配问题，提供了一个最优且稳定的解决方案。
...
### [411] [A Hybrid Physics-Based and Reinforcement Learning Framework for Electric Vehicle Charging Time Prediction](https://arxiv.org/abs/2512.06287)
*Praharshitha Aryasomayajula,Ting Bai,Andreas A. Malikopoulos*

Main category: eess.SY

TL;DR: 本文提出了一种混合预测框架，结合物理分析模型和强化学习（RL）方法，用于准确估算电动汽车（EV）充电时间，并在电池寿命周期内保持预测精度。


<details>
  <summary>Details</summary>
Motivation: 准确估算电动汽车充电时间对于行程规划、用户满意度和充电基础设施的有效运行至关重要。

Method: 该框架包含两个核心组件：1) 基于物理的分析模型，捕捉非线性恒流/恒压（CC-CV）充电动态，并明确建模与健康状态（SoH）相关的容量和功率衰减，在历史数据有限时提供可靠基线。2) 强化学习（RL）组件，随着操作数据的积累逐步优化充电时间预测，实现长期自适应。两种模型都整合了SoH退化，以在电池整个寿命周期内保持预测准确性。

Result: 通过5,000次模拟充电会话（根据制造商规范和公开数据集校准）评估，结果显示：分析模型实现了 R²=98.5% 和 MAPE=2.1%；而结合RL的混合模型进一步将性能提升至 R²=99.2% 和 MAPE=1.6%，这意味着准确性提高了23%，对电池老化的鲁棒性提高了35%。

Conclusion: 该混合预测框架能够提供准确且鲁棒的电动汽车充电时间估算，通过结合物理原理和数据驱动的强化学习，有效应对电池老化问题，显著优于单一的分析模型。
...
### [412] [Control-Oriented System Identification: Classical, Learning, and Physics-Informed Approaches](https://arxiv.org/abs/2512.06315)
*S. Sivaranjani,Yuanyuan Shi,Nikolay Atanasov,Thai Duong,Jie Feng,Tim Martin,Yuezhu Xu,Vijay Gupta,Frank Allgöwer*

Main category: eess.SY

TL;DR: 本文综述了经典、机器学习和数据驱动的系统辨识方法，旨在学习具有控制相关性和物理信息的动态系统模型，并通过直接参数化、软约束和硬约束等方式融入控制理论特性，以提高模型效用和提供可证明的控制保证。


<details>
  <summary>Details</summary>
Motivation: 机器学习在处理复杂数据方面表现出色，但缺乏对控制相关属性的可证明保证。控制理论已识别出多种对分析和控制合成有用的特性（如耗散性、单调性），将这些特性与系统辨识相结合，可以提供有用的归纳偏置、增强可解释性、实现具有可证明保证的控制合成，并提高样本效率。

Method: 将系统辨识表述为一个优化问题，其中控制相关特性可以通过以下方式强制执行：1) 直接参数化（通过结构约束满足特性）；2) 软约束（通过正则化或惩罚项鼓励特性）；3) 硬约束（作为优化问题中的约束）。本文通过这一视角，综述了学习物理信息和控制相关模型的方法，涵盖经典线性/非线性系统辨识、机器学习方法以及数据驱动和行为表示的直接辨识。

Result: 本文综述了多种学习物理信息和控制相关模型的方法，展示了如何通过不同的约束策略（直接参数化、软约束、硬约束）将控制理论特性融入系统辨识过程。通过这种方式，可以从噪声大、高维度和复杂的数据中识别出具有可证明保证的模型。提供了带有代码和教程的示例。

Conclusion: 将系统辨识与控制相关或物理信息特性相结合是未来研究的关键方向，尤其是在网络化、切换和时变系统、实验设计以及弥合数据驱动、基于学习和面向控制方法之间的鸿沟方面。这种融合能够提供有用的归纳偏置，增强模型可解释性，并实现具有可证明保证的控制合成。
...
### [413] [Data Generation for Stability Studies of Power Systems with High Penetration of Inverter-Based Resources](https://arxiv.org/abs/2512.06369)
...
### [414] [The E-Rocket: Low-cost Testbed for TVC Rocket GNC Validation](https://arxiv.org/abs/2512.06535)
...
### [415] [Deep Neural Network-Based Aerial Transport in the Presence of Cooperative and Uncooperative UAS](https://arxiv.org/abs/2512.06577)
*Muhammad Junayed Hasan Zahed,Hossein Rastgoftar*
...
### [416] [Learning Reachability of Energy Storage Arbitrage](https://arxiv.org/abs/2512.06600)
...
### [417] [Data-driven functional state estimation of complex networks](https://arxiv.org/abs/2512.06614)
...
TL;DR: 本文提出了一种数据驱动的框架，用于在没有精确系统模型的情况下，从历史轨迹中估计复杂动态系统的隐藏目标状态，并构建了性能优于或媲美传统模型的观测器。


<details>
  <summary>Details</summary>
Motivation: 动态系统的内部状态通常是隐藏且无法完全测量的，这给实时监测和控制带来了挑战。传统的观测器设计依赖于精确的系统模型，但对于复杂的网络系统而言，这些模型往往难以获得。

Method: 研究引入了一个数据驱动的函数观测器框架，用于估计目标状态变量，无需识别模型参数。它建立了一个基于历史轨迹的基本函数可观测性准则，并开发了使用输入-输出数据或部分状态数据构建观测器的方法。该框架还包含了噪声缓解机制，并可通过Koopman嵌入扩展到非线性网络。

Result: 所构建的观测器性能与基于模型的观测器相当或更优，并且即使对于不可观测的系统也适用。该框架的广泛实用性通过在水网络（传感器故障检测）、电网（负荷频率控制）和非线性神经元系统（目标估计）中的应用得到了验证。

Conclusion: 这项工作为在模型不可用的复杂系统中进行实时目标状态推断提供了一条实用的途径。
...
### [418] [From Forecast to Action: A Deep Learning Model for Predicting Power Outages During Tropical Cyclones](https://arxiv.org/abs/2512.06644)
*Yongchuan Yang,Naiyu Wang,Zhenguo Wang,Min Ouyang,Can Wan*

Main category: eess.SY

TL;DR: 本研究引入了SpatioTemporal Outage ForeCAST (STO-CAST)模型，这是一个基于深度学习的框架，用于在热带气旋事件中进行高分辨率、实时和区域尺度的停电预测。


<details>
  <summary>Details</summary>
Motivation: 热带气旋导致的停电对电力系统和社区构成严重风险。准确、高分辨率的停电预测对于主动减灾规划和实时应急响应至关重要。

Method: STO-CAST模型采用门控循环单元（GRUs）和全连接层，整合了静态环境与基础设施属性以及动态气象和停电序列。模型通过“留一风暴交叉验证”（LOSO）策略进行训练，并结合保留网格实验来验证其泛化能力。它提供4公里*4公里分辨率的每小时停电预测，并支持两种预测模式：通过实时观测数据同化实现6小时提前期的短期临近预报，以及基于气象演变预测实现60小时提前期的长期预报。

Result: 对2022年台风梅花进行的案例研究表明了STO-CAST模型的操作有效性，包括模型设计、气象不确定性和观测差距导致的误差分解。研究强调了实时数据同化的价值以及模型识别不断演变的停电热点的能力。

Conclusion: STO-CAST模型提供了一个可扩展的、数据驱动的解决方案，以支持风险知情的应急响应，并增强在日益增强的热带气旋威胁下电力系统的韧性。
...
### [419] [Sliding-Mode Control Strategies for PMSM: Benchmarking and Comparative Simulation Study](https://arxiv.org/abs/2512.06603)
*Mubarak Badamasi Aremu,Abdullah Ajasa,Ali Nasir*

Main category: eess.SY

TL;DR: 本文提出一个统一的仿真基准，对六种代表性永磁同步电机（PMSM）滑模控制（SMC）技术进行比较分析，以评估其在速度调节中的性能、鲁棒性和计算成本，并为实际应用提供指导。


<details>
  <summary>Details</summary>
Motivation: 永磁同步电机（PMSM）因其高效率和功率密度在高性能驱动系统中广泛应用，但其非线性动态、参数不确定性和负载扰动使其控制复杂。滑模控制（SMC）具有强大的鲁棒性，但存在多种变体且缺乏标准化的评估准则。

Method: 研究采用统一的PMSM模型、扰动曲线和调谐协议，对六种代表性SMC技术（传统、积分、终端、分数阶、自适应和超扭曲）进行仿真比较。性能通过时域响应、积分误差指标（ISE、IAE、ITSE、ITAE）和控制力曲线进行评估，同时考察计算复杂性和实现可行性。

Result: 结果表明，自适应和高阶SMC，特别是超扭曲和自适应变体，在鲁棒性、平滑性和计算成本之间取得了最平衡的折衷。

Conclusion: 该研究提供了一个可复现的基准测试框架、参数选择指南和实用见解，用于设计高效、低抖振的基于SMC的PMSM驱动器，适用于实时嵌入式实现。
...
### [420] [Symmetry-Based Formation Control on Cycle Graphs Using Dihedral Point Groups](https://arxiv.org/abs/2512.06733)
*Zamir Martinez,Daniel Zelazo*

Main category: eess.SY

TL;DR: 本文提出一个基于对称性的循环图编队控制框架，利用二面体群约束，通过强制代理间反射对称性和锚定一个代理，仅用n-1个通信链路即可实现所有Cnv对称配置，并保证指数收敛，还可扩展到时变轨迹。


<details>
  <summary>Details</summary>
Motivation: 开发一种高效且鲁棒的方法，用于在循环图上实现对称编队控制，特别是利用更少的通信链路来达到Cnv对称配置。

Method: 1. 提出了一个基于对称性的编队控制框架，利用二面体点群约束。2. 通过强制代理间的反射对称性，并锚定一个指定的代理到其镜像轴。3. 设计了具有矩阵加权拉普拉斯结构的控制律。4. 将该方法扩展到沿时变参考轨迹的协同机动。

Result: 1. 仅需n-1个通信链路即可实现所有Cnv对称配置。2. 控制律保证指数收敛到期望的对称配置。3. 该方法能够实现沿时变参考轨迹的协同机动。4. 仿真结果支持了理论分析。

Conclusion: 基于对称性的框架为循环图上的编队控制提供了一种高效且鲁棒的方法，它能以更少的通信链路实现Cnv对称配置，保证指数收敛，并适用于动态环境下的协同机动。
...
### [421] [Distributed Traffic State Estimation in V2X-Enabled Connected Vehicle Networks](https://arxiv.org/abs/2512.06765)
...
### [422] [A Physics-Aware Attention LSTM Autoencoder for Early Fault Diagnosis of Battery Systems](https://arxiv.org/abs/2512.06809)
*Jiong Yang*
...
### [423] [Bridging Abstraction-Based Hierarchical Control and Moment Matching: A Conceptual Unification](https://arxiv.org/abs/2512.06875)
...
### [424] [Joint Learning of Feasibility-Aware Signal Temporal Logic and BarrierNet for Robust and Correct Control](https://arxiv.org/abs/2512.06973)
...
### [425] [Synergies between AI Computing and Power Systems: Metrics, Scheduling, and Resilience](https://arxiv.org/abs/2512.07001)
...
### [426] [Momentum-Accelerated Online Feedback Optimization for Power System Flexibility](https://arxiv.org/abs/2512.07077)
*Florian Klein-Helmkamp,Matthis Berger,Irina Zettl,Andreas Ulbig*

Main category: eess.SY

TL;DR: 本文提出了一种基于在线反馈优化的控制器框架，通过引入动量增强投影步，实现电力系统灵活性的实时协调，并展示了其在收敛速度和约束满足方面的优越性。


<details>
  <summary>Details</summary>
Motivation: 现代电力系统运行中，灵活性日益重要，需要一种有效的实时协调方法。

Method: 提出了一种基于在线反馈优化（Online Feedback Optimization, OFO）的控制器框架。核心创新在于引入了动量增强的投影步，以加速收敛并改善动态性能。

Result: 在两个代表性案例研究（配电馈线在线拥塞管理和跨系统接口的多层灵活性调度）中进行了性能和稳定性评估。数值结果表明，基于动量的控制器实现了更快的收敛速度并保持了约束满足。

Conclusion: 基于动量的控制器在实时灵活性控制方面具有巨大潜力，特别适用于大规模电力系统，因为它能实现更快的收敛和持续的约束满足。
...
### [427] [A Structured Review of Fixed and Multimodal Sensing Techniques for Bat Monitoring](https://arxiv.org/abs/2512.07153)
*Maatla Sefawe,Sravya Ganti,Julianna Segalla,Erwei He,Isaac Tourner,Julia Gersey*

Main category: eess.SY

TL;DR: 该综述回顾了用于监测蝙蝠等移动动物种群的固定传感模式，讨论了每种方法的挑战、贡献、覆盖范围、应用、准确性和局限性。


<details>
  <summary>Details</summary>
Motivation: 有效监测移动动物种群（特别是蝙蝠）对生态研究、野生动物管理和农业应用至关重要，有助于了解疾病传播、迁徙模式、种群动态以及环境变化对蝙蝠群落的影响。

Method: 本文首先提供了蝙蝠生物学的背景信息，然后综述了红外传感器、相机、雷达和声学探测器等固定传感模式，讨论了每种方法的独特挑战和贡献，并强调了它们的覆盖范围、应用、准确性和局限性。

Result: 通过综合最新进展，本文全面概述了用于蝙蝠监测的各种固定传感模式，突出了它们的特点和相关限制。

Conclusion: 该综述旨在为该领域的未来研究提供指导，以有效利用固定传感模式进行移动动物种群（尤其是蝙蝠）的监测。
...
### [428] [Green O-RAN Operation: a Modern ML-Driven Network Energy Consumption Optimisation](https://arxiv.org/abs/2512.07006)
...
### [429] [Off-grid solar energy storage system with hybrid lithium iron phosphate (LFP) and lead-acid batteries in high mountains: a case report of Jiujiu Cabins in Taiwan](https://arxiv.org/abs/2512.07353)
*Hsien-Ching Chung*
...
### [430] [Safety-Critical Control on Lie Groups Using Energy-Augmented Zeroing Control Barrier Functions](https://arxiv.org/abs/2512.07395)
...
### [431] [Scalable Formal Verification of Incremental Stability in Large-Scale Systems Using Graph Neural Networks](https://arxiv.org/abs/2512.07448)
*Ahan Basu,Mahathi Anand,Pushpak Jagtap*

Main category: eess.SY

TL;DR: 本文提出了一种利用图神经网络（GNN）的分布式框架，用于验证具有未知动力学和已知互连结构的大规模系统的增量稳定性。


<details>
  <summary>Details</summary>
Motivation: 验证具有未知动力学的大规模系统的增量稳定性是一个具有挑战性的问题。

Method: 该方法通过为子系统构建局部增量Lyapunov函数，然后将其组合以获得互连系统的Lyapunov函数。图神经网络以数据驱动的方式合成这些函数，并通过利用训练网络的Lipschitz界限来获得形式上的正确性保证。

Result: 通过两个非线性案例研究验证了该方法的有效性。

Conclusion: 所提出的GNN分布式框架能够有效验证具有未知动力学的大规模系统的增量稳定性，并提供形式上的正确性保证。
...
### [432] [Control of Discrete-Time Linear Systems with Charge-Balanced Inputs](https://arxiv.org/abs/2512.07506)
...
### [433] [Data-Driven Robust Safety Verification for Markov Decision Processes](https://arxiv.org/abs/2512.07550)
...
### [434] [Optimal Pulse Patterns through a Hybrid Optimal Control Perspective](https://arxiv.org/abs/2512.07585)
...
### [435] [Obstacle Avoidance of UAV in Dynamic Environments Using Direction and Velocity-Adaptive Artificial Potential Field](https://arxiv.org/abs/2512.07609)
...
### [436] [Linear Quadratic Control with Non-Markovian and Non-Semimartingale Noise Models](https://arxiv.org/abs/2512.07699)
...
### [437] [The explicit game-theoretic linear quadratic regulator for constrained multi-agent systems](https://arxiv.org/abs/2512.07749)
...
### [438] [Research on a Monitoring System for High-Voltage Cables in a Coal Mine Based on Intelligent Sensing Technology](https://arxiv.org/abs/2512.07714)
...
### [439] [Augmented Neural Ordinary Differential Equations for Power System Identification](https://arxiv.org/abs/2512.07757)
...
### [440] [Affine Subspace Models and Clustering for Patch-Based Image Denoising](https://arxiv.org/abs/2512.07259)
*Tharindu Wickremasinghe,Marco F. Duarte*

Main category: eess.IV

TL;DR: 本文提出使用仿射子空间模型进行图像瓦片聚类，以更好地匹配图像瓦片的几何结构，并展示了其在去噪和聚类方面的性能改进。


<details>
  <summary>Details</summary>
Motivation: 图像瓦片处理中常用的线性子空间模型不适合表示图像块，因为图像是非负的，在瓦片向量空间中不以原点为中心分布。

Method: 研究使用仿射子空间模型进行图像瓦片聚类，以更好地匹配图像瓦片向量空间的几何结构。提出了一种基于仿射子空间聚类模型和最小二乘投影的简单去噪算法。回顾了几种解决仿射子空间聚类问题的算法方法。

Result: 实验结果表明，在聚类和去噪方面均有性能提升。

Conclusion: 仿射子空间模型比线性子空间模型更适合图像瓦片聚类，能够有效提高聚类和去噪性能。
...
### [441] [From sparse recovery to plug-and-play priors, understanding trade-offs for stable recovery with generalized projected gradient descent](https://arxiv.org/abs/2512.07397)
*Ali Joundi,Yann Traonmilin,Jean-François Aujol*

Main category: eess.IV

TL;DR: 本文扩展了广义投影梯度下降（GPGD）框架的收敛性，使其对模型和投影误差具有鲁棒性，并提出了改进其稳定性和处理结构化噪声的方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于从带噪声、欠定观测中恢复低维未知向量，并统一传统的稀疏恢复方法与基于深度学习投影先验的现代方法。同时，需要提升这些方法的鲁棒性、稳定性，并有效处理结构化噪声。

Method: 本文采用广义投影梯度下降（GPGD）框架，扩展了其收敛性分析以涵盖模型和投影误差的鲁棒性。针对结构化噪声，提出了广义反投影策略。为提高GPGD的稳定性，引入了一种用于学习深度投影先验的归一化幂等正则化方法。

Result: 研究结果包括：理论上扩展了GPGD对模型和投影误差的鲁棒性，并提供了控制稳定性和鲁棒性常数的方法。数值实验（在稀疏恢复和图像逆问题中）表明，通过所提出的方法可以在可识别性和稳定性之间取得权衡，广义反投影策略能够减少结构化噪声引起的恢复误差，归一化幂等正则化能够提高GPGD的稳定性。

Conclusion: 本文成功地将GPGD框架扩展到对模型和投影误差更具鲁棒性，并通过引入广义反投影策略和归一化幂等正则化，有效提高了其处理结构化噪声的能力和稳定性。数值实验验证了这些改进，并揭示了这些方法在可识别性和稳定性之间的权衡。
...
### [442] [Physics-Guided Diffusion Priors for Multi-Slice Reconstruction in Scientific Imaging](https://arxiv.org/abs/2512.06977)
*Laurentius Valdy,Richard D. Paul,Alessio Quercia,Zhuo Cao,Xuan Zhao,Hanno Scharr,Arya Bangun*

Main category: eess.IV

TL;DR: 该论文提出了一种结合分区扩散先验和物理约束的多层切片重建框架，显著降低了内存使用，同时提高了重建质量和泛化能力，优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 从有限测量数据中进行精确的多层切片重建对于加速医学和科学成像采集过程至关重要，但由于问题的病态性以及高计算和内存需求，这仍然是一个挑战。

Method: 提出一个框架，将分区扩散先验与基于物理的约束相结合。通过这种方式，显著减少了每张GPU的内存使用。

Result: 在保持高重建质量的同时，显著降低了每张GPU的内存使用。在磁共振成像（MRI）和四维扫描透射电子显微镜（4D-STEM）等不同模态下，其性能优于仅基于物理和全多层切片重建的基线方法。此外，该方法提高了分布内准确性，并对分布外数据集展现出强大的泛化能力。

Conclusion: 所提出的方法通过整合分区扩散先验和物理约束，有效解决了多层切片重建中的内存和质量挑战，实现了卓越的性能和泛化能力。
...
### [443] [Clinical Interpretability of Deep Learning Segmentation Through Shapley-Derived Agreement and Uncertainty Metrics](https://arxiv.org/abs/2512.07224)
*Tianyi Ren,Daniel Low,Pittra Jaengprajak,Juampablo Heras Rivera,Jacob Ruzevick,Mehmet Kurt*

Main category: eess.IV

TL;DR: 本研究通过对比度级别的Shapley值，为医学图像分割模型提供可解释性，评估模型性能对不同MRI对比度的归因，并提出了与临床排名一致性和不确定性（排名方差）两个指标，发现高性能模型与临床排名一致性更高，且排名方差与性能呈负相关。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习模型在医学图像分割中表现出色，但其可解释性不足严重阻碍了临床应用中的接受度。现有解释方法（如基于梯度的技术）存在局限性，需要一种更广泛、更符合临床需求的方法来公平地解释模型性能如何归因于特定的影像对比度。

Method: 本研究采用对比度级别的Shapley值，通过系统地扰动模型输入来评估特征重要性。利用BraTS 2024数据集，为四种MRI对比度在四种模型架构上生成Shapley值排名。提出了两个基于Shapley排名的新指标：模型与“临床医生”影像排名之间的一致性，以及通过跨验证折叠中Shapley排名方差量化的不确定性。

Result: 研究发现，高性能案例（Dice > 0.6）与临床排名表现出显著更高的一致性。Shapley排名方差的增加与性能下降相关（U-Net模型的相关系数r = -0.581）。

Conclusion: 这些基于Shapley的指标为模型可靠性提供了临床上可解释的代理，有助于临床医生更好地理解最先进的分割模型。
...
### [444] [Semantic Temporal Single-photon LiDAR](https://arxiv.org/abs/2512.06008)
...
TL;DR: 本文提出了一种基于自更新语义知识库（SKB）的语义时间单光子（TSP-）激光雷达，将目标识别表述为语义通信，以解决传统TSP-LiDAR在开放集、低信噪比和短采集时间下的性能问题。


<details>
  <summary>Details</summary>
Motivation: 现有TSP-LiDAR在处理未知目标（开放集场景）时效果不佳，且在低信噪比和短采集时间（光子数量少）条件下性能显著下降。

Method: 受语义通信启发，提出了一种语义TSP-LiDAR，其核心是建立一个自更新的语义知识库（SKB），将TSP-LiDAR的目标识别过程表述为一个语义通信问题。

Result: 仿真和实验结果表明，该方法优于传统方法，尤其是在低信噪比和有限采集时间的挑战性条件下。更重要的是，自更新SKB机制能够动态更新新遇到目标的语义特征，实现持续适应，而无需对神经网络进行大量重新训练。在实际实验中，对九种未知目标实现了89%的识别准确率，而没有更新机制时为66%。

Conclusion: 该框架在复杂动态环境中具有实现自适应和鲁棒目标识别的潜力。
...
### [445] [R2MF-Net: A Recurrent Residual Multi-Path Fusion Network for Robust Multi-directional Spine X-ray Segmentation](https://arxiv.org/abs/2512.07576)
...
TL;DR: 本文提出R2MF-Net，一个循环残差多路径编解码网络，用于自动分割多方向脊柱X射线图像，以实现脊柱侧弯的定量评估。


<details>
  <summary>Details</summary>
Motivation: X射线图像中脊柱结构的精确分割是脊柱侧弯定量评估（如Cobb角测量、椎体平移估计和曲度分类）的先决条件。然而，在临床实践中，手动分割耗时、不可重现，尤其是在低对比度、肋骨阴影或组织重叠的情况下，存在诸多局限性。

Method: R2MF-Net是一个级联的粗分割网络和精细分割网络。它采用了改进的Inception风格多分支特征提取器，并在跳跃连接中引入循环残差跳跃（R2-Jump）模块以对齐编解码器语义。精细网络通过多尺度跨阶段跳跃（MC-Skip）机制重用粗网络多层级的表示，增强了跨成像方向和对比度条件的分割稳定性。此外，瓶颈处使用了轻量级空间-通道挤压激励（SCSE-Lite）模块以强调脊柱相关特征并抑制噪声。

Result: R2MF-Net在一个包含228套冠状位、左弯和右弯脊柱X射线图像及专家标注的临床多视图放射线数据集上进行了评估。

Conclusion: R2MF-Net为多方向脊柱X射线图像的自动分割提供了一个有效的解决方案，有望提高脊柱侧弯评估的效率、可重现性和准确性，尤其是在复杂成像条件下增强了分割的稳定性。
...
### [446] [Stronger is not better: Better Augmentations in Contrastive Learning for Medical Image Segmentation](https://arxiv.org/abs/2512.05992)
...
### [447] [Precise Liver Tumor Segmentation in CT Using a Hybrid Deep Learning-Radiomics Framework](https://arxiv.org/abs/2512.07574)
...
### [448] [Proof of Concept for Mammography Classification with Enhanced Compactness and Separability Modules](https://arxiv.org/abs/2512.06575)
*Fariza Dahes*
