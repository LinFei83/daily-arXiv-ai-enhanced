...
TL;DR: 本文提出了一种高效的、基于CPU的几何定理自动证明方法HAGeo，它通过启发式添加辅助结构，在IMO-30基准测试上取得了金牌级别的表现，超越了AlphaGeometry，并引入了更具挑战性的HAGeo-409基准测试。


<details>
  <summary>Details</summary>
Motivation: 欧几里得几何中的自动定理证明，特别是针对国际数学奥林匹克（IMO）级别的问题，仍然是一个重大挑战和人工智能领域的重要研究焦点。

Method: 本文提出了一种完全基于CPU运行且不依赖神经网络推理的高效几何定理证明方法。首先，通过一个简单的随机策略添加辅助点，达到了IMO银牌级别的表现。在此基础上，提出了HAGeo，一种基于启发式方法添加几何推导中辅助结构的策略。此外，构建了一个包含409个几何问题的HAGeo-409基准测试，用于更全面地评估现有方法。

Result: 初步研究表明，简单的随机策略添加辅助点能达到IMO银牌级别的人类表现。HAGeo在IMO-30基准测试中解决了30个问题中的28个，达到了金牌级别的表现，并显著超越了基于神经网络的AlphaGeometry。新构建的HAGeo-409基准测试比IMO-30更具挑战性，提供了更精确的评估。

Conclusion: HAGeo是一种高效且有效的几何定理证明方法，能够通过基于启发式的辅助结构策略在IMO级别问题上取得优异表现，甚至超越了基于神经网络的竞争方法。新引入的HAGeo-409基准测试为几何定理证明领域设定了更高的评估标准。
...
TL;DR: 该调查旨在整合多模态分块策略的现有格局，为研究人员和从业者提供技术基础和设计空间，以开发更有效和高效的多模态AI系统。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了整合多模态分块策略的现有格局，提供技术基础和设计空间，从而开发更有效和高效的多模态AI系统，并为创新更稳健的分块管道铺平道路。

Method: 该研究提供了一个全面的分类法和技术分析，涵盖针对文本、图像、音频、视频和跨模态数据的分块策略。它审视了经典和现代方法，如固定大小的token窗口、递归文本分割、以对象为中心的视觉分块、基于静音的音频分割和视频中的场景检测。每种方法都从其底层方法、支持工具、优点和挑战（特别是粒度-上下文权衡和多模态对齐）方面进行分析。此外，还探讨了新兴的跨模态分块策略。

Result: 该调查提供了比较性见解，强调了异步信息密度和嘈杂对齐信号等开放问题，并指出了自适应、基于学习和特定任务分块的未来研究机会。

Conclusion: 该调查旨在指导研究人员和从业者开发能够随模态复杂性扩展、提高处理准确性并改善真实世界应用中生成连贯性的稳健分块管道。
...
TL;DR: 针对AI基准测试快速饱和问题，本文提出一个统计框架，统一衡量模型能力和基准难度，实现跨基准、跨时间的比较，并应用于测量AI进展速度、预测未来能力、估算算法效率提升及检测加速。


<details>
  <summary>Details</summary>
Motivation: 大多数AI基准测试在推出后数月或数年内即饱和，使得研究AI能力的长期趋势变得困难。

Method: 构建了一个统计框架，将不同的基准测试“缝合”在一起，将模型能力和基准难度置于一个统一的数值尺度上。该框架充当“罗塞塔石碑”，允许比较不同能力范围和时间跨度的模型，即使它们未在相同的基准上进行评估，且不假设能力随时间或训练计算量的演变方式。

Result: 1. 测量了AI随时间的进展速度，并预测了未来的AI能力。2. 估算了算法效率的提升率，发现其估计值更高，但与先前的工作大致一致。3. 发现该方法可用于检测AI进展中的快速加速。

Conclusion: 所提出的统计框架成功地解决了AI基准测试饱和的挑战，提供了一种统一衡量AI能力和基准难度的方法，并有效应用于测量AI进展、预测未来能力、估算效率提升以及检测进展加速。
...
### [4] [Trification: A Comprehensive Tree-based Strategy Planner and Structural Verification for Fact-Checking](https://arxiv.org/abs/2512.00267)
*Anab Maulana Barik,Shou Ziyi,Yang Kaiwen,Yang Qi,Shen Xin*

Main category: cs.AI

TL;DR: 本文提出了一种名为Trification的新型自动化事实核查框架，它通过生成全面的验证行动并将其构建成可动态修改的依赖图，以解决现有系统验证不完整和缺乏逻辑连接的问题，显著提高了事实核查的准确性。


<details>
  <summary>Details</summary>
Motivation: 随着信息快速传播，虚假信息泛滥，自动化事实核查系统变得至关重要。现有基于LLM的多智能体系统存在两个主要局限：未能验证声明中的所有组成部分，以及缺乏结构化框架来逻辑连接子任务结果以得出最终预测。

Method: 本文提出Trification框架，其方法包括：1) 生成一套全面的验证行动以确保声明的完整覆盖；2) 将这些行动结构化为依赖图，以模拟行动之间的逻辑交互；3) 允许动态修改该图，使系统能够适应其验证策略。

Result: 在两个具有挑战性的基准测试上的实验结果表明，Trification框架显著提升了事实核查的准确性。

Conclusion: Trification框架通过提供全面的验证覆盖和结构化的逻辑连接，有效解决了现有自动化事实核查系统的局限性，从而推进了该领域的最新技术水平。
...
### [5] [Reasoning Under Pressure: How do Training Incentives Influence Chain-of-Thought Monitorability?](https://arxiv.org/abs/2512.00218)
*Matt MacDermott,Qiyao Wei,Rada Djoneva,Francis Rhys Ward*

Main category: cs.AI

TL;DR: 本文研究了不同训练激励如何影响AI系统自然语言推理（CoT）的可监控性。结果显示，对抗性优化会降低监控效果，而常用激励和直接优化并未带来可靠的改进。


<details>
  <summary>Details</summary>
Motivation: AI系统通过自然语言输出的思维链（CoT）为监控潜在有害推理提供了机会。然而，CoT是否能忠实反映底层推理过程，以及其可监控性，可能受训练方式影响。因此，研究不同训练激励对CoT可监控性的影响至关重要。

Method: 研究引入了一种新颖的方法来衡量可监控性，即通过监控器能否利用模型的推理来预测一个关键的潜在变量。在此基础上，文章调查了常见的训练激励（如长度惩罚和KL正则化）、对抗性优化（惩罚监控器准确性）以及直接优化可监控性对模型可监控性的影响，并在控制模型准确性的前提下进行比较。

Result: 在控制准确性的前提下，研究发现常用的训练激励（长度惩罚和KL正则化）对可监控性没有一致影响。然而，对抗性优化（惩罚监控器准确性）会降低监控器的性能。同时，直接优化可监控性也未能可靠地带来改进。

Conclusion: 尽管自然语言CoT为AI安全监控提供了潜在途径，但其可监控性受训练激励的影响。对抗性优化会损害监控效果，而常用的激励机制和直接优化可监控性并未显示出可靠的改善。这表明在训练中保持CoT的忠实性以实现有效监控是一个挑战。
...
### [6] [RL-Struct: A Lightweight Reinforcement Learning Framework for Reliable Structured Output in LLMs](https://arxiv.org/abs/2512.00319)
*Ruike Hu,Shulei Wu*
...
### [7] [ChartPoint: Guiding MLLMs with Grounding Reflection for Chart Reasoning](https://arxiv.org/abs/2512.00305)
...
TL;DR: 本文提出了一种名为临床目标相对策略优化（CRPO）的可扩展、多目标、可验证强化学习方法，用于将大型语言模型（LLM）的后期训练与临床推理原则对齐，同时优化准确性、忠实性和全面性，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 当前LLM后期训练方法（如GRPO）主要奖励正确性，但这与医学等高风险领域所需的忠实性和全面性等多维目标不符。临床推理不仅需要准确，还需要忠实和全面。

Method: 引入了临床目标相对策略优化（CRPO），这是一种可扩展、多目标、可验证的强化学习方法。CRPO集成了基于规则和可验证的奖励信号，共同优化准确性、忠实性和全面性，且不依赖人工标注。通过CRPO训练了30亿参数的临床推理模型Clinical-R1-3B。

Result: 在三个基准测试中，CRPO显著提高了推理的真实性和完整性，同时保持了良好的准确性提升，优于标准的GRPO方法。

Conclusion: CRPO框架为将LLM推理与临床目标对齐提供了一条可扩展的途径，有助于在医疗领域实现更安全、更协作的AI系统。它也凸显了多目标、可验证强化学习方法在医疗领域LLM后期训练扩展中的潜力。
...
TL;DR: 本文提出EDIT方法，通过监测训练阶段捕获的推理图与推理时token激活的对齐程度，自适应地提前终止扩散式大语言模型（dLLMs）的去噪过程，显著减少推理步数并保持或提升准确率。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型（dLLMs）在推理时通常在所有去噪步骤完成之前就已经稳定生成答案，导致计算资源浪费。研究旨在减少dLLM的推理时间和成本。

Method: EDIT（Early Diffusion Inference Termination）是一种推理时终止标准。它通过监控token激活与“推理图”之间的对齐来工作。推理图是从监督微调（SFT）期间捕获的AdamW聚合的LoRA更新中提取的，代表学习到的推理路径。在推理过程中，对齐分数被转换为当前去噪步骤中已解除掩码的token的分布。当连续步骤之间在匹配的已解除掩码token上的KL散度低于某个阈值时，即检测到收敛并终止去噪。

Result: EDIT在推理基准测试中将扩散步数减少了11.8%至68.3%，同时在大多数设置下保持或提高了准确性。该方法存储开销极低，约为0.02%（对于8 GB模型中32个块的所有QKV模块，大约1.5-2 MB）。

Conclusion: 通过利用训练梯度动态，EDIT成功地降低了dLLM的推理时间和成本。这项工作为减少dLLM推理时间和成本开辟了一个新的研究方向。
...
TL;DR: 本文提出一个神经理论框架，旨在解决大型语言模型在可解释性、统一理论和运营成本方面的局限性，并为创建可解释、通用且计算高效的人工智能提供方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的发展受到缺乏可解释性、缺乏统一理论以及高昂运营成本的限制。

Method: 提出一个神经理论框架，用于解释系统中智能的出现，该框架既具有功能鲁棒性又具有生物合理性。

Result: 该模型为决策和问题解决等认知过程提供了理论见解，并为创建可解释和可泛化的人工智能提供了一种计算高效的方法。

Conclusion: 该神经理论框架为理解认知过程和开发更具可解释性、通用性及计算效率的人工智能提供了新的途径，有望克服当前大型语言模型的挑战。
...
TL;DR: 本文提出了一种名为FA-DPO的算法，旨在解决大型语言模型（LLM）对齐过程中人类反馈数据中普遍存在的偏好翻转问题，通过建模实例相关的翻转概率并设计迭代优化算法来提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对齐中数据集质量至关重要，但人类反馈收集过程中偏好翻转现象普遍存在，导致数据标注损坏。这促使研究人员需要开发对潜在翻转对更具鲁棒性的对齐算法。

Method: 本文从强化学习与人类反馈（RLHF）的角度引入了翻转感知直接偏好优化（FA-DPO）算法。该方法将人类意图模型和外部因素导致的偏好翻转机制解剖为两个不同阶段。在后者，基于Bradley-Terry（BT）模型引入了实例相关的翻转概率。此外，通过利用与偏好标注相关的特征，捕获判断的不确定性并建模偏好翻转模式。实践中，设计了一个简单高效的迭代优化算法，与原始的RLHF和DPO算法兼容。

Result: 实验在多种情况下研究了实例相关的偏好翻转模型，并评估了本文提出的方法以及其他基线方法。 (抽象未提供具体量化结果，仅说明进行了调查和评估)。

Conclusion: 本文提出并评估了FA-DPO算法，通过建模实例相关的偏好翻转，为大型语言模型对齐提供了一种对偏好翻转更具鲁棒性的解决方案，解决了人类反馈数据中的关键挑战。
...
### [17] [Probing the "Psyche'' of Large Reasoning Models: Understanding Through a Human Lens](https://arxiv.org/abs/2512.00729)
...
### [19] [SpeContext: Enabling Efficient Long-context Reasoning with Speculative Context Sparsity in LLMs](https://arxiv.org/abs/2512.00722)
...
### [25] [Minimal neuron ablation triggers catastrophic collapse in the language core of Large Vision-Language Models](https://arxiv.org/abs/2512.00918)
*Cen Lu,Yung-Chen Tang,Andrea Cavallaro*

Main category: cs.AI

TL;DR: 本文研究了大型视觉-语言模型（LVLMs）的结构脆弱性，发现移除语言模型中极少数关键神经元即可导致灾难性崩溃。


<details>
  <summary>Details</summary>
Motivation: LVLMs展现出强大的多模态理解能力，但其鲁棒性尚不明确。本研究旨在调查LVLMs的结构脆弱性，识别移除后会导致灾难性崩溃的关键神经元。

Method: 提出CAN（Consistently Activated Neurons）方法，通过渐进式掩码（progressive masking）来检测持续激活的神经元并定位关键神经元。

Result: 实验表明，仅掩码语言模型前馈网络中极少数神经元（极端情况下少至四个）就足以引发灾难性崩溃。关键神经元主要位于语言模型而非视觉组件中，其中下投影层是一个特别脆弱的结构。此外，观察到一致的两阶段崩溃模式：初始表达能力退化，随后突然完全崩溃。

Conclusion: 本研究的发现为LVLMs的安全研究提供了重要见解。
...
### [26] [One Swallow Does Not Make a Summer: Understanding Semantic Structures in Embedding Spaces](https://arxiv.org/abs/2512.00852)
*Yandong Sun,Qiang Huang,Ziwei Xu,Yiqun Sun,Yixuan Tang,Anthony K. H. Tung*

Main category: cs.AI

TL;DR: 本文提出语义场子空间（SFS）及其无监督算法SAFARI，用于揭示嵌入空间中的局部语义邻域和分层语义结构，同时提供高效且可扩展的语义理解框架。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入空间的可解释性方法往往牺牲语义连贯性以获得结构规律性，或因提高可解释性而带来高昂的计算成本，导致其内部结构不透明。

Method: 引入语义场子空间（SFS），一种保留几何结构并感知上下文的表示，以捕获嵌入空间中的局部语义邻域。提出无监督、模态无关的SAFARI算法，利用语义漂移（Semantic Shift）度量来量化语义演变，从而揭示分层语义结构。为提高可扩展性，开发了一种高效的语义漂移近似方法，避免了昂贵的SVD计算。

Result: SFS在六个真实世界的文本和图像数据集上，不仅在分类任务中优于标准分类器，还在政治偏见检测等细致任务中表现出色。SAFARI持续揭示出可解释且泛化能力强的语义层次结构。语义漂移的近似方法实现了15~30倍的速度提升，平均误差低于0.01。

Conclusion: 本研究提出了一个统一的框架，用于在嵌入空间中构建、分析和扩展语义理解，有效解决了现有方法的透明度和效率问题。
...
### [27] [Hybrid-DMKG: A Hybrid Reasoning Framework over Dynamic Multimodal Knowledge Graphs for Multimodal Multihop QA with Knowledge Editing](https://arxiv.org/abs/2512.00881)
*Li Yuan,Qingfei Huang,Bingshan Zhu,Yi Cai,Qingbao Huang,Changmeng Zheng,Zikun Deng,Tao Wang*

Main category: cs.AI

TL;DR: 现有多模态知识编辑（MKE）基准未能评估中间推理质量和对视觉重述输入的鲁棒性。本文引入了MMQAKE基准来解决这些局限性，并提出了Hybrid-DMKG框架，通过动态多模态知识图谱和混合推理机制，显著提高了MKE在多跳推理和知识更新鲁棒性方面的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态知识编辑（MKE）基准主要评估最终答案的正确性，却忽略了中间推理的质量以及对视觉重述输入的鲁棒性。这限制了对MKE方法真实能力的全面评估。

Method: 本文引入了MMQAKE，这是首个用于多模态多跳问答与知识编辑的基准，用于评估模型在2-5跳事实链上的推理能力（包括每个中间步骤）以及对视觉重述输入在多跳问题中的鲁棒性。为解决挑战，提出了Hybrid-DMKG混合推理框架，它基于动态多模态知识图谱（DMKG）。Hybrid-DMKG首先使用大型语言模型将多模态多跳问题分解为顺序子问题，然后应用多模态检索模型通过联合编码子问题、候选实体及其关联图像来定位更新的事实。在答案推断阶段，混合推理模块通过关系链接预测和结合大型视觉-语言模型的RAG推理两条并行路径在DMKG上操作，最后由决策模块聚合证据以选择最可信的答案。

Result: 评估显示，当前MKE方法在知识编辑后难以一致地更新和推理多模态推理链。Hybrid-DMKG在MMQAKE上显著优于现有MKE方法，实现了更高的准确性并提高了对知识更新的鲁棒性。

Conclusion: MMQAKE揭示了当前MKE方法在多模态多跳推理和鲁棒性方面的局限性。Hybrid-DMKG通过其动态多模态知识图谱和混合推理框架，有效解决了这些挑战，在更新和推理多模态知识方面表现出卓越的性能。
...
### [28] [IndiMathBench: Autoformalizing Mathematical Reasoning Problems with a Human Touch](https://arxiv.org/abs/2512.00997)
*Param Biyani,Shashank Kirtania,Yasharth Bajpai,Sumit Gulwani,Ashish Tiwari*

Main category: cs.AI

TL;DR: IndiMathBench是一个由AI辅助人工验证的数学定理证明基准，包含312个源自印度数学奥林匹克竞赛的Lean 4形式化定理，旨在评估模型在形式化自然语言问题方面的能力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在创建一个具有挑战性的、人工验证的基准，以评估当前AI模型在数学定理证明中的表现，特别是将自然语言问题形式化为形式化语言的能力。

Method: 采用AI驱动的人工辅助流水线，通过基于类别的检索、迭代编译器反馈和多模型集成，生成Lean 4形式化定理的候选方案。专家通过交互式仪表板和自动化质量摘要进行高效验证。

Result: 对多个前沿模型进行评估表明，自动形式化仍然具有挑战性，语法有效性和语义正确性之间存在显著差距；即使经过迭代优化，定理证明的成功率仍然很低，这表明IndiMathBench是一个对数学推理能力具有挑战性的测试平台。

Conclusion: IndiMathBench为数学推理和定理证明提供了一个具有挑战性的测试基准，揭示了当前模型在自动形式化和定理证明方面的局限性，并可用于推动相关领域的研究进展。
...
### [29] [Integrating Causal Foundation Model in Prescriptive Maintenance Framework for Optimizing Production Line OEE](https://arxiv.org/abs/2512.00969)
*Felix Saretzky,Lucas Andersen,Thomas Engel,Fazel Ansari*

Main category: cs.AI

TL;DR: 本文提出一种基于因果机器学习的模型，旨在将制造维护从预测性转向规定性，通过识别故障的真正因果驱动因素并量化干预措施的影响，从而优化关键绩效指标。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型依赖虚假相关性而非真正的因果驱动因素，导致误诊和无效干预。它们能预测“可能发生什么”但无法解释“为何发生”，缺乏系统方法来理解故障原因并确定最有效的干预措施，这是向规定性维护转变的关键限制。

Method: 本文提出一个基于因果机器学习的模型，使用预训练的因果基础模型作为“假设”模型。该模型通过模拟和评估潜在修复方案，测量每次干预对系统级KPI（如OEE）的因果效应，从而提供数据驱动的行动建议排名。

Result: 该模型能够识别根本原因并量化其操作影响。通过在半合成制造数据上进行评估，并与基线机器学习模型进行比较，结果表明该方法为构建一个强大的规定性维护框架奠定了技术基础。

Conclusion: 该框架使工程师能够在因果环境中测试潜在解决方案，从而做出更有效的操作决策并减少代价高昂的停机时间，实现了从诊断到主动规定的转变。
...
### [32] [Shielded Controller Units for RL with Operational Constraints Applied to Remote Microgrids](https://arxiv.org/abs/2512.01046)
...
### [33] [Med-CRAFT: Automated Construction of Interpretable and Multi-Hop Video Workloads via Knowledge Graph Traversal](https://arxiv.org/abs/2512.01045)
...
TL;DR: 本文提出了“基础先验”框架，将基础模型生成的合成数据视为用户主观先验而非真实观察值，并提供了一种将其整合到实证工作中的原则性方法。


<details>
  <summary>Details</summary>
Motivation: 基础模型（特别是大型语言模型）能生成高度信息丰富的响应，引发了将这些“合成”输出用作实证研究和决策数据的兴趣。然而，需要解决这些合成数据的非真实性及其固有的主观性问题。

Method: 引入“基础先验”的概念，认为模型生成输出源于由基础先验诱导的先验预测分布。通过明确合成输出对用户预期数据分布、提示工程过程和对基础模型的信任的依赖，对生成过程的主观性进行建模。将基础先验推导为用户原始先验的指数倾斜广义贝叶斯更新，其中信任参数控制合成数据的权重。然后展示如何将合成数据和相关基础先验整合到标准统计和计量经济学工作流程中。

Result: 基础先验被推导为用户原始先验的指数倾斜广义贝叶斯更新，其中信任参数决定合成数据的权重。通过将生成输出视为结构化、明确主观的先验，而非经验观察，该框架提供了一种原则性的方法来利用基础模型进行实证工作，并可应用于完善复杂模型、推断潜在结构、指导实验设计和增强随机系数及部分线性规范。

Conclusion: 该框架通过将生成输出视为结构化、明确主观的先验而非经验观察，提供了一种原则性的方法来利用基础模型进行实证工作，从而避免将合成“事实”与真实数据混淆。
...
### [40] [fMRI2GES: Co-speech Gesture Reconstruction from fMRI Signal with Dual Brain Decoding Alignment](https://arxiv.org/abs/2512.01189)
*Chunzheng Zhu,Jialin Shao,Jianxin Lin,Yijun Wang,Jing Wang,Jinhui Tang,Kenli Li*

Main category: cs.AI

TL;DR: 本文提出fMRI2GES方法，利用双脑解码对齐技术，在缺乏配对数据的情况下，从fMRI信号重建与言语相关的肢体手势。


<details>
  <summary>Details</summary>
Motivation: 理解大脑如何响应外部刺激并解码这一过程是神经科学的重大挑战。以往研究主要集中于大脑到图像或语言的重建，但重建与言语相关的肢体手势仍未被探索。然而，缺乏配对的{大脑、言语、手势}数据阻碍了深度学习模型在此方向的应用。

Method: 引入fMRI2GES方法，通过“双脑解码对齐”在非配对数据上训练fMRI到手势的重建网络。该方法利用两个关键组件：(i) 引发大脑反应的观察文本，和 (ii) 与手势相关的文本描述。通过结合fMRI到文本模型、配对数据的文本到手势模型以及非配对数据的fMRI到手势模型，建立双重fMRI到手势重建模式，然后明确对齐两个输出，并以自监督方式训练模型。

Result: 所提出的方法能够直接从fMRI记录中重建富有表现力的手势。研究还探讨了皮层不同感兴趣区域(ROI)的fMRI信号及其对手势生成结果的影响。

Conclusion: 本研究为解码伴随言语的手势提供了新见解，从而推动了神经科学和认知科学的理解。
...
### [41] [A Benchmark of Causal vs Correlation AI for Predictive Maintenance](https://arxiv.org/abs/2512.01149)
*Krishna Taduri,Shaunak Dhande,Giacinto Paolo,Saggese,Paul Smith*

Main category: cs.AI

TL;DR: 在预测性维护中，因极度成本不对称，基于因果推理的AI方法比基于相关性的方法能带来显著的经济效益和更可解释的预测。


<details>
  <summary>Details</summary>
Motivation: 预测性维护存在极端的成本不对称问题（漏报成本远高于误报成本），而传统机器学习方法优化的统计准确性指标未能反映这种操作现实，也无法可靠地区分因果关系与虚假关联。

Method: 本研究在一个包含10,000台数控机床（故障发生率为3.3%）的数据集上，评估了八种预测模型，涵盖从基线统计方法到正式因果推理方法。

Result: 正式的因果推理模型（L5）估计每年可节省116万美元（减少70.2%），比最佳的相关性决策树模型（L3）每年多节省约8万美元。该因果模型达到了最高的召回率（87.9%），同时将误报减少了97%（从165次降至5次），并获得了92.1%的精确度，训练-测试性能差距仅为2.6个百分点。

Conclusion: 这些结果表明，在预测性维护应用中，结合领域知识的因果AI方法相较于基于相关性的方法，能够产生更优越的经济效益和更可解释的预测。
...
TL;DR: 本文提出了一种知识图谱（KG）引导的思维链（CoT）框架，用于从电子健康记录（EHR）中生成临床合理且时间一致的疾病预测解释，并在性能和可解释性方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于EHR的临床预测模型通常提供粗略的、事后解释，对患者层面的决策价值有限。研究旨在开发一种能生成具有临床依据和时间一致性的患者级解释的方法。

Method: 研究引入了KG引导的CoT框架。首先，将ICD-9编码映射到PrimeKG，从中提取与疾病相关的节点和多跳推理路径，作为CoT生成的支架。仅保留结论与实际观察结果匹配的解释。随后，使用这些监督语料库对轻量级LLaMA-3.1-Instruct-8B和Gemma-7B模型进行微调。

Result: 在MIMIC-III数据集上，KG引导模型在十种PrimeKG映射疾病和有限训练队列（400和1000例）中表现优于强大的传统基线，AUROC值达到0.66至0.70，macro-AUPR值达到0.40至0.47。这些模型还实现了对CRADLE队列的零样本迁移，将准确率从大约0.40至0.51提高到0.72至0.77。一项盲法临床医生评估显示，临床医生在清晰度、相关性和临床正确性方面一致偏爱KG引导的CoT解释。

Conclusion: KG引导的CoT框架能够为EHR中的疾病预测生成具有临床依据和时间一致性的解释，显著提高了预测性能和解释质量，并获得了临床医生的认可，有望提升患者层面的决策支持。
...
### [43] [OntoMetric: An Ontology-Guided Framework for Automated ESG Knowledge Graph Construction](https://arxiv.org/abs/2512.01289)
*Mingqin Yu,Fethi Rabhi,Boming Xia,Zhengyi Yang,Felix Tan,Qinghua Lu*

Main category: cs.AI

TL;DR: OntoMetric是一个本体引导框架，能将复杂的ESG监管文件转换为经过验证的、AI和网络就绪的知识图谱，以提高合规性和可审计性。


<details>
  <summary>Details</summary>
Motivation: ESG披露框架（如SASB、TCFD、IFRS S2）要求组织报告大量指标，但这些要求嵌入在长而无结构的PDF文档中，难以解释、标准化和审计。手动提取效率低下，而无约束的大型语言模型（LLM）提取则存在实体不一致、关系幻觉、溯源缺失和验证失败率高的问题。

Method: OntoMetric采用三阶段管道：1) 使用目录边界进行结构感知分段；2) 本体约束的LLM提取，将ESGMKG模式嵌入提示中，并用语义字段丰富实体；3) 两阶段验证，结合基于LLM的语义验证和基于规则的模式检查（涵盖实体、属性和关系层面）。该框架还保留了段落级和页面级溯源性以供审计。

Result: 在五个ESG标准（共228页、60个段落）上的评估显示，OntoMetric实现了65-90%的语义准确性和80-90%的模式合规性，远高于基线无约束提取的3-10%。每个验证实体的成本约为0.01至0.02美元。

Conclusion: 结合符号本体约束和神经提取能够生成可靠、可审计的知识图谱，适用于监管合规和网络集成。这支持了可持续金融分析、透明度门户和自动化合规工具等下游应用。
...
### [44] [RoboDriveVLM: A Novel Benchmark and Baseline towards Robust Vision-Language Models for Autonomous Driving](https://arxiv.org/abs/2512.01300)
*Dacheng Liao,Mengshi Qi,Peng Shu,Zhining Zhang,Yuxin Lin,Liang Liu,Huadong Ma*

Main category: cs.AI

TL;DR: 本文提出RoboDriveBench基准来评估VLM在自动驾驶中的鲁棒性，并提出RoboDriveVLM框架及跨模态知识蒸馏TTA方法，以提升系统在真实世界挑战下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前的视觉-语言模型（VLM）端到端自动驾驶系统在真实场景中存在多种风险，需要评估其可行性，并解决其在传感器和提示信息受损时的鲁棒性问题。

Method: 1. 引入RoboDriveBench，首个专注于端到端轨迹预测任务的鲁棒性基准，包含11种模拟场景（6种传感器损坏，5种提示信息损坏）。2. 提出RoboDriveVLM，一个新型VLM自动驾驶框架，通过将更多多模态数据（如激光雷达、毫米波雷达）映射到统一潜在空间来增强鲁棒性。3. 引入基于跨模态知识蒸馏的测试时自适应（TTA）方法，进一步提升系统鲁棒性。

Result: 通过广泛实验，本文揭示了当前VLM端到端自动驾驶系统的局限性，并证明所提出的RoboDriveVLM框架提供了一个更可靠的真实世界部署解决方案。

Conclusion: 当前VLM自动驾驶系统在真实世界挑战下存在鲁棒性问题。RoboDriveBench为评估提供了工具，而RoboDriveVLM及其跨模态知识蒸馏TTA方法为解决这些挑战并实现更可靠的自动驾驶系统提供了有效途径。
...
### [45] [Unsupervised decoding of encoded reasoning using language model interpretability](https://arxiv.org/abs/2512.01222)
*Ching Fang,Samuel Marks*

Main category: cs.AI

TL;DR: 本研究通过构建一个使用ROT-13加密推理的模型，评估了当前可解释性技术（特别是Logit Lens）解码模型隐藏推理过程的能力，并发现这些技术对简单的编码推理比预期更有效。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力增强，人们日益担忧其可能发展出人类难以监督的、编码或隐藏的推理过程。本研究旨在探究现有可解释性技术是否能穿透这类编码推理。

Method: 研究构建了一个受控测试平台：微调一个推理模型（DeepSeek-R1-Distill-Llama-70B），使其在ROT-13加密中执行思维链推理，同时保持可理解的英语输出。然后，使用机械可解释性方法（特别是Logit Lens分析）来评估其仅利用内部激活解码模型隐藏推理过程的能力。最后，开发了一个结合Logit Lens和自动化意译的完全无监督解码流程。

Result: 研究表明，Logit Lens能有效翻译编码推理，准确率在中到后期层达到峰值。此外，开发的无监督解码流程在从模型内部表示重建完整推理记录方面取得了显著的准确性。

Conclusion: 这些发现表明，当前的机械可解释性技术可能比之前认为的对简单形式的编码推理更具鲁棒性。本工作为评估针对以非人类可读格式推理的模型的可解释性方法提供了一个初步框架，有助于解决维持对日益强大的AI系统进行监督的挑战。
...
TL;DR: 本文提出CuES框架，通过好奇心驱动和环境感知，自主生成多样化、可执行且有意义的任务，以解决大型语言模型代理在缺乏预定义任务的环境中进行强化学习的“任务稀缺”问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理在复杂、工具增强环境中部署时，强化学习的有效性依赖于结构化训练任务。然而，许多现实场景中缺乏此类任务，尤其是在工具语义和功能未知的全新环境中，这成为代理强化学习扩展的关键瓶颈。现有方法通常假设任务集合已预定义，这在任务稀缺问题下失效。

Method: 本文提出了CuES（Curiosity driven and Environment grounded Synthesis）框架。该框架通过内在好奇心驱动探索，将交互模式抽象为可重用的任务图式，并通过轻量级的自上而下指导和基于记忆的质量控制进行完善。CuES直接从环境结构和功能中自主生成多样化、可执行且有意义的任务，无需依赖手工种子或外部语料库。

Result: 在AppWorld、BFCL和WebShop三个代表性环境中，CuES生成的任务分布在多样性和可执行性方面与手动策划的数据集相当或超越，并带来了显著的下游策略改进。这表明CuES能够有效解决任务稀缺问题。

Conclusion: 研究结果表明，好奇心驱动、环境感知的任务生成为代理提供了一个可扩展的基础，使其不仅能学习如何行动，还能学习“学什么”。这为代理在任务稀缺环境中自主学习和适应提供了新的途径。
...
TL;DR: 本研究将单智能体稀疏奖励强化学习算法NGU扩展到多智能体环境，并在PettingZoo的simple_tag环境中评估。结果显示，与DQN基线相比，NGU性能更优且学习更稳定，其中共享回放缓冲区是关键。


<details>
  <summary>Details</summary>
Motivation: NGU算法在单智能体稀疏奖励任务中表现出色，本研究旨在将其有效性扩展到多智能体环境。

Method: 将NGU算法扩展到多智能体设置，并在PettingZoo的simple_tag环境中与多智能体DQN基线进行比较。研究了三种设计选择：(1) 共享与独立回放缓冲区，(2) 智能体间通过不同k值共享情景新颖性，(3) 使用异构的beta参数值。

Result: NGU比多智能体DQN基线实现了适度更高的回报和更稳定的学习动态。带有共享回放缓冲区的NGU表现出最佳的性能和稳定性。新颖性共享在k=1时表现相当，但对于更大的k值会降低学习效果。异构beta值并未比小的通用值有所改进。

Conclusion: 研究结果表明，NGU可以有效地应用于多智能体设置，前提是经验得到共享且内在探索信号经过仔细调整。
...
TL;DR: 本文提出M3PO，一个基于强化学习的框架，通过多路径协同学习和跨路径交互，解决了大型语言模型（LLM）思维链（CoT）推理中存在的确定性解码限制，显著提升了推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管思维链（CoT）推理显著提升了LLM的解题能力，但传统CoT在解码时表现出内部确定性，限制了对替代方案的探索。现有方法试图通过生成软抽象token在连续语义空间中进行推理，但仍受自回归解码贪婪性质的限制，使模型无法接触到替代推理可能性。

Method: 本文提出了多路径感知策略优化（M3PO），一个新颖的强化学习框架。M3PO利用并行策略rollout作为多样化的推理来源，并通过轻量级协同机制将跨路径交互整合到策略更新中，使每个轨迹都能通过同伴反馈来改进其推理。

Result: M3PO在知识密集型和推理密集型基准测试上均取得了最先进的性能。经过M3PO训练的模型保持了可解释性和推理效率。

Conclusion: 多路径协同学习对于鲁棒的推理具有广阔前景，M3PO通过显式注入集体洞察力到推理过程中，有效提升了LLM的多步推理能力。
...
TL;DR: 该研究引入一种利用大型语言模型将合成策略知识提炼成可验证代码的方法，以增强计算机辅助合成规划（CASP）系统在策略考量方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现代CASP系统在生成化学有效反应步骤方面表现良好，但在整合收敛组装、保护基团最小化和最佳成环序列等战略性考量方面存在不足。

Method: 该方法利用大型语言模型分析合成路线，并将策略原则（如策略性官能团互变和成环策略）转化为Python函数。通过将合成知识形式化为可验证的代码而非简单的启发式规则，创建了可测试、可解释的合成策略表示。

Result: 研究发布了完整的代码库和USPTO-ST数据集（标注有策略标签的合成路线）。该框架实现了基于自然语言的路线检索，在基准测试中达到75%的Top-3准确率。通过历史趋势的时间分析和化学直观的路线聚类，进一步验证了该库，提供了比以往方法更精细的划分。

Conclusion: 这项工作弥合了CASP中的战术-策略鸿沟，使得可以根据策略标准而非仅仅结构来指定、搜索和评估合成路线。
...
### [56] [CLIP-RL: Aligning Language and Policy Representations for Task Transfer in Reinforcement Learning](https://arxiv.org/abs/2512.01616)
*Chainesh Gautam,Raghuram Bharadwaj Diddigi*

Main category: cs.AI

TL;DR: 本文提出一种受CLIP启发的创新方法，通过结合预训练的（语言，策略）对，为自然语言和策略嵌入创建统一的表示空间，从而实现跨任务的快速迁移。


<details>
  <summary>Details</summary>
Motivation: 开发能够在同一环境中解决多个任务，特别是与语言自然相关的任务的智能体，需求日益增长。

Method: 该方法利用预训练的（语言，策略）对的组合，建立高效的迁移管道。受计算机视觉中CLIP的启发，其核心思想是任务指令和对应策略代表同一概念（任务本身）的两种不同模态。通过将CLIP的思想扩展到强化学习，该方法为自然语言和策略嵌入创建了一个统一的表示空间。

Result: 实验结果表明，该算法在实现跨任务的更快迁移方面具有实用性。

Conclusion: 该方法成功地为自然语言和策略嵌入创建了统一的表示空间，并能有效加速任务间的知识迁移。
...
### [57] [LEC: Linear Expectation Constraints for False-Discovery Control in Selective Prediction and Routing Systems](https://arxiv.org/abs/2512.01556)
*Zhiyuan Wang,Aniri,Tianlong Chen,Yue Zhang,Heng Tao Shen,Xiaoshuang Shi,Kaidi Xu*

Main category: cs.AI

TL;DR: 该研究提出LEC方法，通过控制LLM预测的错误发现率（FDR），确保接受的预测中错误比例不超过目标风险水平，并将其扩展到双模型路由机制，显著提升了预测可靠性和样本保留率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）经常生成不可靠的答案，而现有的启发式不确定性方法无法充分区分正确和错误的预测，导致用户在没有统计保证的情况下接受错误答案。

Method: 该论文提出LEC（Linear Expectation Constraint）方法，将选择性预测重新解释为受约束的决策问题，通过对选择和错误指标施加线性期望约束。它建立了一个有限样本的充分条件，仅依赖于一个独立的校准数据集，来计算一个FDR受限、覆盖率最大化的阈值。此外，LEC被扩展到一种双模型路由机制：如果当前模型的不确定性超过其校准阈值，则将其委托给更强的模型，同时保持统一的FDR保证。

Result: 在封闭式和开放式问答数据集上的评估显示，LEC实现了更严格的FDR控制，并显著提高了样本保留率，优于现有方法。此外，双模型路由机制在接受更多正确样本的同时，实现了比单个模型更低的风险水平。

Conclusion: LEC提供了一种原则性的方法来控制LLM预测的错误发现率，通过确保统计保证来提高可靠性。其双模型路由扩展进一步优化了性能，实现了更低的风险和更高的效率。
...
TL;DR: HistoricalML是一个概率神经符号框架，通过整合贝叶斯不确定性量化、结构因果模型、合作博弈论和注意力机制，解决了历史事件建模中数据稀缺、异质性、反事实缺失和可解释性等挑战。


<details>
  <summary>Details</summary>
Motivation: 历史事件建模面临数据极度稀缺、测量异质且嘈杂、反事实缺失以及对人类可解释性要求高等根本性挑战。

Method: 本研究提出了HistoricalML框架，该框架原则性地整合了：1) 贝叶斯不确定性量化，以区分认知不确定性和偶然不确定性；2) 结构因果模型，用于混淆下的反事实推理；3) 合作博弈论（Shapley值），用于公平分配建模；4) 基于注意力的神经网络架构，用于上下文相关的因子加权。

Result: 理论分析表明，该方法在稀疏数据状态下，当领域知识提供强先验时，能够实现一致性估计，且基于Shapley的分配满足公理化公平性保证。在非洲瓜分案例中，模型识别出德国在第一次世界大战前存在+107.9%的结构性张力。在布匿战争中，蒙特卡洛模拟显示迦太基在坎尼战役有57.3%的胜率，罗马在扎马战役有57.8%的胜率，与历史结果一致。反事实分析揭示，迦太基的政治支持而非军事能力是决定性因素。

Conclusion: HistoricalML提供了一个原则性的框架来处理历史事件建模的固有挑战，通过结合多种先进的机器学习和统计方法，能够从稀疏、嘈杂的历史数据中提取有意义且可解释的见解，并与历史结果保持一致。
...
TL;DR: 本文提出了一种名为“按需LLM陪审团”的动态、基于学习的框架，用于对大型语言模型（LLMs）进行可扩展且可靠的评估，通过动态选择最可靠的LLM评审团并加权聚合其分数，显著提高了与人类判断的相关性。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs被整合到高风险领域，对既能实时部署又能在关键决策中可靠的评估方法的需求日益增长。现有方法存在局限：人类评估虽可靠但缓慢且昂贵；单一LLM评审员存在偏见；静态陪审团缺乏适应性。

Method: 提出“LLM按需陪审团”框架。该方法训练一组可靠性预测器，利用token分布、嵌入和结构化输入特征来评估LLM评审员与人类专家的意见一致性。这使得对每个数据点能够动态选择一个由最可靠评审员组成的最佳陪审团，并根据其可靠性作为权重聚合他们的分数。

Result: 在摘要生成和RAG基准测试中，该动态陪审团系统与人类判断的相关性显著高于单一评审员和静态陪审团基线。这些结果证明了自适应、基于学习的陪审团在构建高风险领域中可扩展、更可靠和值得信赖的现代LLM评估系统方面的潜力。

Conclusion: 自适应、基于学习的陪审团为构建在高风险领域中可扩展、更可靠和值得信赖的现代LLM评估系统提供了广阔前景。这一方法有效克服了传统评估方法的局限性，提升了评估的准确性和效率。
...
### [60] [Testing Transformer Learnability on the Arithmetic Sequence of Rooted Trees](https://arxiv.org/abs/2512.01870)
*Alessandro Breccia,Federica Gerace,Marco Lippi,Gabriele Sicuro,Pierluigi Contucci*

Main category: cs.AI

TL;DR: 研究表明，大型语言模型（GPT-2）能部分学习由迭代素因子分解生成的树序列的内部语法，捕捉到非平凡的规律和相关性。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型是否能学习自然数迭代素因子分解产生的确定性树序列，以了解其学习能力是否能超越经验数据，延伸到算术结构本身。

Method: 将每个整数映射为一个有根平面树，形成序列 $\mathbb{N}\mathcal{T}$。然后，从头开始训练一个Transformer网络（GPT-2架构），使用前 $10^{11}$ 个元素，并通过下一词预测和掩码词预测任务来测试其预测能力。

Result: 模型部分学习了 $\mathbb{N}\mathcal{T}$ 的内部语法，捕捉到非平凡的规律和相关性。

Conclusion: 这表明大型语言模型的学习能力可能不仅限于经验数据，还能扩展到算术本身的结构。
...
### [61] [H-Neurons: On the Existence, Impact, and Origin of Hallucination-Associated Neurons](https://arxiv.org/abs/2512.01797)
*Cheng Gao,Huimin Chen,Chaojun Xiao,Zhiyi Chen,Zhiyuan Liu,Maosong Sun*

Main category: cs.AI

TL;DR: 本文系统研究了大型语言模型中与幻觉相关的神经元（H-Neurons），发现一小部分神经元能可靠预测幻觉，这些神经元与过度顺从行为因果相关，并起源于预训练阶段。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）经常产生幻觉（貌似合理但不真实的结果），损害了它们的可靠性。现有研究多从宏观层面（如训练数据和目标）探讨幻觉，但其底层的神经元级机制仍未被充分探索。

Method: 本文从三个角度对LLMs中的幻觉相关神经元（H-Neurons）进行了系统调查：识别、行为影响和起源。具体方法包括：识别可预测幻觉的稀疏神经元子集，通过受控干预揭示这些神经元的行为影响，并追溯这些神经元的起源至预训练基础模型。

Result: 研究发现，极少数神经元（不到总神经元的0.1%）能可靠预测幻觉的发生，并且在不同场景下具有很强的泛化能力。受控干预表明，这些神经元与过度顺从行为存在因果关系。追溯其起源发现，这些神经元在预训练阶段就已出现，并且在基础模型中仍然能预测幻觉。

Conclusion: 本研究将宏观行为模式与微观神经机制联系起来，为开发更可靠的LLMs提供了深入见解。
...
### [62] [Predicting Human Chess Moves: An AI Assisted Analysis of Chess Games Using Skill-group Specific n-gram Language Models](https://arxiv.org/abs/2512.01880)
*Daren Zhong,Dingcheng Huang,Clayton Greenberg*

Main category: cs.AI

TL;DR: 本文提出一个新颖且计算高效的棋子走法预测框架，将国际象棋走法预测视为行为分析任务，利用n-gram语言模型捕捉不同技能水平玩家的走法模式，并展示了显著的预测准确性提升和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统的国际象棋引擎主要关注计算最优走法，而忽略了人类棋手（尤其是不同技能水平）走法中固有的可变性。研究旨在通过行为分析方法克服这一局限性。

Method: 该框架将国际象棋走法预测视为行为分析任务。它采用n-gram语言模型来捕捉特定玩家技能水平的走法模式。通过将玩家分为七个不同的技能组（从新手到专家），使用Lichess平台的开源数据训练了独立的模型。框架动态选择最合适的模型进行预测，并根据之前的走法序列生成玩家走法。

Result: 在真实游戏数据上的评估显示，框架中的模型选择模块在使用早期游戏信息（16个半回合）时，技能水平分类准确率高达31.7%。走法预测框架也显示出显著的准确性提升，其“选择器辅助准确性”比基准准确性高出39.1%。此外，该框架具有较高的计算效率。

Conclusion: 所提出的框架能够有效预测人类国际象棋走法，考虑了玩家的技能水平差异，并显著提高了预测准确性和计算效率，使其适用于实时国际象棋分析。
...
### [63] [Graph Distance as Surprise: Free Energy Minimization in Knowledge Graph Reasoning](https://arxiv.org/abs/2512.01878)
*Gaganpreet Jhajj,Fuhua Lin*

Main category: cs.AI

TL;DR: 本文提出通过最小化惊奇来指导知识图谱（KG）网络中的推理，将神经科学的自由能原理（FEP）与KG系统联系起来。


<details>
  <summary>Details</summary>
Motivation: 研究动机是将最近在句法结构中通过树结构最小化惊奇和自由能的工作扩展到知识图谱领域。

Method: 通过有向图中的最短路径距离来形式化“惊奇”概念，并提出一个基于KG的智能体框架。图距离与图神经网络中的消息传递深度以及基于模型的强化学习中的世界模型轨迹相关联。

Result: 这项正在进行中的研究探索了基于距离的惊奇是否能够扩展现有工作，即通过树结构最小化句法中的惊奇和自由能。主要结果是提出了一个将FEP与KG系统连接起来，并以最短路径距离形式化惊奇的框架。

Conclusion: 论文提出了一个新颖的框架，通过最小化惊奇指导知识图谱推理，并将其与自由能原理联系起来，为KG智能体提供了一种新的视角。
...
### [64] [From Atomic to Composite: Reinforcement Learning Enables Generalization in Complementary Reasoning](https://arxiv.org/abs/2512.01970)
...
### [65] [Learned-Rule-Augmented Large Language Model Evaluators](https://arxiv.org/abs/2512.01958)
*Jie Meng,Jin Mao*
...
### [68] [MOTION: ML-Assisted On-Device Low-Latency Motion Recognition](https://arxiv.org/abs/2512.00008)
*Veeramani Pugazhenthi,Wei-Hsiang Chu,Junwei Lu,Jadyn N. Miyahira,Soheil Salehi*

Main category: cs.CV

TL;DR: 本研究提出了一种基于三轴加速度计和AutoML流水线的运动模型构建方案，并在WeBe Band上实现了高效的实时手势识别，尤其适用于医疗监测。


<details>
  <summary>Details</summary>
Motivation: 在日常人机交互和医疗监测领域，需要微型设备实现低延迟手势识别。嵌入式解决方案（如跌倒检测、康复跟踪、患者监督）要求快速高效地跟踪运动，同时避免不必要的误报。

Method: 研究使用三轴加速度计传感器构建高效的运动模型。方法包括利用AutoML流水线从数据段中提取最重要的特征，并使用这些特征训练多个轻量级机器学习算法。所有处理都在WeBe Band（一种多传感器可穿戴设备）上进行，以实现设备端的手势识别。

Result: 在所探索的模型中，神经网络在准确性、延迟和内存使用之间提供了最佳平衡。研究结果表明，WeBe Band能够实现可靠的实时手势识别。

Conclusion: WeBe Band上的实时手势识别具有巨大潜力，可应用于需要安全快速响应时间的实时医疗监测解决方案。
...
### [69] [Closing the Gap: Data-Centric Fine-Tuning of Vision Language Models for the Standardized Exam Questions](https://arxiv.org/abs/2512.00042)
*Egemen Sert,Şeyda Ertekin*

Main category: cs.CV

TL;DR: 本研究通过高质量数据和优化推理语法对开源视觉语言模型进行监督微调，使其在标准化考试多模态推理任务上表现接近专有模型，强调了数据在多模态推理中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 多模态推理是AI研究的基石，标准化考试问题提供了严格的测试平台。现有研究主要关注算法进展（如强化学习），而视觉语言推理的数据中心基础仍未得到充分探索。

Method: 研究团队编译了一个包含1.614亿token的多模态数据集，结合了教科书问答对、课程对齐图表和上下文材料。使用优化的推理语法（QMSA）对Qwen-2.5VL-32B模型进行监督微调，并在新发布的YKSUniform基准（包含1,854个标准化多模态考试问题）上进行评估。

Result: 微调后的模型在YKSUniform基准上取得了78.6%的准确率，仅比Gemini 2.0 Flash低1.0%。结果表明，数据构成和表示语法在多模态推理中起着决定性作用。

Conclusion: 本工作建立了一个推进开源视觉语言模型的数据中心框架，证明了精心策划和基于课程的多模态数据可以使监督微调达到接近最先进的性能。
...
### [70] [PEFT-DML: Parameter-Efficient Fine-Tuning Deep Metric Learning for Robust Multi-Modal 3D Object Detection in Autonomous Driving](https://arxiv.org/abs/2512.00060)
*Abdolazim Rezaei,Mehdi Sookhak*

Main category: cs.CV

TL;DR: PEFT-DML是一个参数高效的深度度量学习框架，用于自动驾驶中鲁棒的多模态3D目标检测，即使在传感器丢失或未见模态组合下也能可靠工作。


<details>
  <summary>Details</summary>
Motivation: 传统模型假设传感器可用性固定，无法处理传感器丢失或未见模态组合的情况。研究旨在开发一个鲁棒且参数高效的多模态3D目标检测框架，以应对自动驾驶中的挑战，如传感器故障、恶劣天气和域转移。

Method: 引入PEFT-DML框架，通过深度度量学习将多种模态（LiDAR、雷达、相机、IMU、GNSS）映射到共享潜在空间。该框架集成了低秩适应（LoRA）和适配器层，以实现参数高效的训练并增强鲁棒性。

Result: 在nuScenes基准测试中，PEFT-DML展现了卓越的精度，并显著增强了对快速运动、天气变化和域转移的鲁棒性。

Conclusion: PEFT-DML通过参数高效的深度度量学习，成功实现了在自动驾驶中鲁棒的多模态3D目标检测，即使在传感器丢失或环境多变的情况下也能保持高性能。
...
### [71] [DL-CapsNet: A Deep and Light Capsule Network](https://arxiv.org/abs/2512.00061)
*Pouya Shiri,Amirali Baniasadi*

Main category: cs.CV

TL;DR: 本文提出了一种名为DL-CapsNet的深度胶囊网络变体，通过引入胶囊摘要层来减少参数，实现高准确性、更快的训练和推理速度，并能处理复杂数据集。


<details>
  <summary>Details</summary>
Motivation: 胶囊网络（CapsNet）在处理图像重叠类别和仿射变换方面比卷积神经网络（CNN）更准确，具有作为CNN替代者的潜力。作者旨在进一步改进CapsNet，使其能处理更复杂的图像数据集，同时提高效率。

Method: 本文提出了一种深度胶囊网络（DL-CapsNet），它由多个胶囊层组成。此外，设计了“胶囊摘要层”来减少参数数量，从而降低模型复杂性。

Result: DL-CapsNet在保持高准确性的同时，使用的参数数量更少，并能实现更快的训练和推理速度。它能够处理具有大量类别的复杂数据集。

Conclusion: DL-CapsNet通过其深度结构和胶囊摘要层，在处理复杂、多类别数据集时，展现出高准确性、低参数量和快速运行的优势。
...
### [72] [Satellite to Street : Disaster Impact Estimator](https://arxiv.org/abs/2512.00065)
*Sreesritha Sai,Sai Venkata Suma Sreeja,Deepthi,Nikhil*

Main category: cs.CV

TL;DR: 本文提出了一种名为“Satellite-to-Street: Disaster Impact Estimator”的深度学习框架，它通过联合处理灾前和灾后卫星图像，生成像素级的精细损害地图，解决了传统方法慢、主观以及现有深度学习模型在处理细微结构变化和类别不平衡时的不足。


<details>
  <summary>Details</summary>
Motivation: 灾后损害评估对手头紧急响应至关重要，但人工解读卫星图像耗时、主观且难以规模化。现有基于U-Net的图像分割和变化检测模型虽然有用，但常难以识别细微结构变化和处理严重的类别不平衡问题，导致对高度受损区域的检测效果不佳。

Method: 该模型采用了一种改进的双输入U-Net架构，具有增强的特征融合能力，以捕捉局部结构变化和更广泛的上下文线索。同时，集成了类别感知加权损失函数，以解决真实灾害数据中未受损像素占主导地位的问题，从而提高对主要和完全毁坏类别的敏感性。

Result: 在公开可用的灾害数据集上进行的实验表明，与传统分割和基线变化检测模型相比，该方法在结构损害的定位和分类方面均有所改进。

Conclusion: 所产生的损害地图提供了一种快速且一致的评估机制，旨在支持而非取代专家决策，从而实现更高效、数据驱动的灾害管理。
...
### [73] [Adapter Shield: A Unified Framework with Built-in Authentication for Preventing Unauthorized Zero-Shot Image-to-Image Generation](https://arxiv.org/abs/2512.00075)
...
### [74] [ProvRain: Rain-Adaptive Denoising and Vehicle Detection via MobileNet-UNet and Faster R-CNN](https://arxiv.org/abs/2512.00073)
...
### [75] [TinyViT: Field Deployable Transformer Pipeline for Solar Panel Surface Fault and Severity Screening](https://arxiv.org/abs/2512.00117)
...
### [76] [Diffusion-Based Synthetic Brightfield Microscopy Images for Enhanced Single Cell Detection](https://arxiv.org/abs/2512.00078)
...
### [77] [Efficient Edge-Compatible CNN for Speckle-Based Material Recognition in Laser Cutting Systems](https://arxiv.org/abs/2512.00179)
...
### [78] [Conceptual Evaluation of Deep Visual Stereo Odometry for the MARWIN Radiation Monitoring Robot in Accelerator Tunnels](https://arxiv.org/abs/2512.00080)
...
### [79] [AutocleanEEG ICVision: Automated ICA Artifact Classification Using Vision-Language AI](https://arxiv.org/abs/2512.00194)
...
### [80] [Exploring Diagnostic Prompting Approach for Multimodal LLM-based Visual Complexity Assessment: A Case Study of Amazon Search Result Pages](https://arxiv.org/abs/2512.00082)
...
### [81] [Multi-modal On-Device Learning for Monocular Depth Estimation on Ultra-low-power MCUs](https://arxiv.org/abs/2512.00086)
*Davide Nadalini,Manuele Rusci,Elia Cereda,Luca Benini,Francesco Conti,Daniele Palossi*

Main category: cs.CV

TL;DR: 针对超低功耗物联网平台中单目深度估计（MDE）的领域偏移问题，本文提出了一种多模态设备端学习（ODL）技术。通过在物联网设备上利用低功耗深度传感器收集伪标签，并采用一种内存驱动的稀疏更新方案进行模型微调，该方法显著提高了模型在新环境中的精度，同时大幅降低了内存消耗和训练时间。


<details>
  <summary>Details</summary>
Motivation: 超低功耗物联网（ULP IoT）平台上的单目深度估计（MDE）任务，由于其模型参数受限，在传感器数据与训练数据集存在显著领域偏移时，会出现严重的精度下降。

Method: 本文提出了一种多模态设备端学习（ODL）技术。该技术部署在一个集成了Greenwaves GAP9微控制器单元（MCU）、80毫瓦单目摄像头和8x8像素深度传感器的物联网设备上。在正常操作中，一个微型107k参数的μPyD-Net模型用于单目图像推理。当系统处于新环境时，深度传感器（通常为节能而停用）与摄像头同时激活，以收集伪标签。随后，微调任务完全在MCU上使用新数据执行。为优化设备端反向传播训练，引入了一种新颖的内存驱动稀疏更新方案。

Result: 所提出的稀疏更新方案将微调内存最小化至1.2 MB，比完全更新减少2.2倍，同时保持了精度（在KITTI和NYUv2数据集上仅下降2%和1.5%）。现场测试首次证明，MDE的设备端学习可在17.8分钟内在物联网节点上完成，仅使用3k个自标记样本，将均方根误差从4.9米降低到0.6米。

Conclusion: 在物联网节点上进行单目深度估计的设备端学习是可行的，它通过一种新颖的稀疏更新方案有效解决了领域偏移问题，在实际内存和时间限制下，显著提高了模型在新环境中的精度。
...
### [82] [A Fast and Efficient Modern BERT based Text-Conditioned Diffusion Model for Medical Image Segmentation](https://arxiv.org/abs/2512.00084)
...
### [83] [Exploring Automated Recognition of Instructional Activity and Discourse from Multimodal Classroom Data](https://arxiv.org/abs/2512.00087)
...
TL;DR: 本研究探索了利用AI驱动的多模态方法自动分析课堂录像，以识别教学活动和话语，为教师提供可扩展的反馈。


<details>
  <summary>Details</summary>
Motivation: 目前的课堂互动观察主要依赖手动标注，这种方法资源密集且难以扩展，限制了向教师提供具体反馈的效率和规模。

Method: 研究使用了一个包含164小时视频和68份课程转录稿的密集标注数据集。设计了并行的、特定模态的分析流程。对于视频分析（24个活动标签），评估了零样本多模态LLM、微调的视觉-语言模型和自监督视频Transformer。对于转录稿分析（19个话语标签），微调了基于Transformer的分类器，并与基于提示的LLM进行了比较。为处理类别不平衡和多标签复杂性，采用了逐标签阈值设定、上下文窗口和不平衡感知损失函数。

Result: 微调模型始终优于基于提示的方法，在视频分析上达到了0.577的宏观F1分数，在转录稿分析上达到了0.460的宏观F1分数。

Conclusion: 研究结果证明了自动化课堂分析的可行性，并为建立可扩展的教师反馈系统奠定了基础。
...
### [84] [TeleViT1.0: Teleconnection-aware Vision Transformers for Subseasonal to Seasonal Wildfire Pattern Forecasts](https://arxiv.org/abs/2512.00089)
*Ioannis Prapas,Nikolaos Papadopoulos,Nikolaos-Ioannis Bountos,Dimitrios Michail,Gustau Camps-Valls,Ioannis Papoutsis*

Main category: cs.CV

TL;DR: 本文介绍了TeleViT，一种融合局部火灾驱动因素、全球场数据和遥相关指数的多尺度Vision Transformer，用于提前数周到数月预测野火，并在所有预测提前期上均优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 提前数周到数月预测野火非常困难，但对于燃料处理和资源分配至关重要。虽然短期预测依赖局部天气，但长期预测需要考虑地球的互联性，包括全球模式和遥相关。

Method: 研究引入了TeleViT，一个遥相关感知的Vision Transformer。它通过不对称分词策略融合了(i)精细尺度的局部火灾驱动因素、(ii)粗化的全球场数据和(iii)遥相关指数，生成异构token。这些token由Transformer编码器共同处理，随后解码器通过将局部token映射到相应的预测区域来保留空间结构。

Result: 使用全球SeasFire数据集（2001-2021），TeleViT在所有提前期（包括长达四个月的预测期）的AUPRC性能均优于U-Net++、ViT和气候学模型。在零提前期，TeleViT的AUPRC为0.630（ViT 0.617，U-Net 0.620）；在16x8天（约4个月）提前期，TeleViT变体保持0.601-0.603（ViT 0.582，U-Net 0.578），并始终超越气候学模型（0.572）。区域结果显示，在非洲稀树草原等季节性一致的火灾区域技能最高，而在北方和干旱地区技能较低。注意力分析表明预测主要依赖局部token，全球场数据和指数提供粗粒度上下文信息。

Conclusion: 研究结果表明，明确编码大规模地球系统上下文的架构可以扩展野火在季节内到季节性时间尺度上的可预测性。
...
### [85] [SemImage: Semantic Image Representation for Text, a Novel Framework for Embedding Disentangled Linguistic Features](https://arxiv.org/abs/2512.00088)
*Mohammad Zare*

Main category: cs.CV

TL;DR: SemImage是一种将文本文档表示为二维语义图像的新方法，它使用解耦的HSV色彩空间编码语言特征，并插入动态边界行，以便通过CNN进行处理和分类，同时提供增强的可解释性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是希望将文本文档转化为二维图像，以便利用卷积神经网络（CNN）强大的图像处理能力进行文本分析，并使文本中的语义、情感等语言特征更直观地呈现给机器和人类。

Method: 该方法将文本文档表示为二维语义图像（SemImage），其中每个词是一个像素，行对应句子，句子间插入动态边界行。像素值不是RGB，而是解耦的HSV色彩空间向量：色相（H）编码主题，饱和度（S）编码情感，明度（V）编码强度/确定性。通过多任务学习框架强制实现特征解耦，ColorMapper网络将词嵌入映射到HSV空间，辅助监督用于预测色相和饱和度通道的主题和情感标签。SemImage与标准2D CNN（如ResNet）结合用于文档分类。

Result: SemImage在多标签和单标签数据集上，与包括BERT和分层注意力网络在内的强大文本分类基线相比，实现了具有竞争力甚至更好的准确性。它还提供了增强的可解释性。消融研究证实了多通道HSV表示和动态边界行的重要性。可视化结果定性地揭示了生成的图像中与主题和情感变化相对应的清晰模式，表明该表示使这些语言特征对人类和机器都可见。

Conclusion: SemImage是一种新颖、有效且可解释的文本表示方法，能够将文本文档转化为二维语义图像供CNN处理，并通过解耦的色彩空间和动态边界行，成功地使语言特征（如主题和情感）对人类和机器都变得可见和可理解。
...
### [86] [Comparative Analysis of Vision Transformer, Convolutional, and Hybrid Architectures for Mental Health Classification Using Actigraphy-Derived Images](https://arxiv.org/abs/2512.00103)
*Ifeanyi Okala*

Main category: cs.CV

TL;DR: 本研究评估了VGG16、ViT-B/16和CoAtNet-Tiny三种图像方法在通过日常活动记录识别抑郁症、精神分裂症和健康对照方面的表现，发现CoAtNet-Tiny表现最稳定和准确。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探讨不同的图像识别模型（卷积神经网络和Transformer）在将腕戴活动信号转换为图像后，识别精神健康状况（抑郁症、精神分裂症）的有效性，以期为精神健康诊断提供新的技术支持。

Method: 研究将Psykose和Depresjon数据集中的腕戴活动信号转换为30x48像素的图像。然后，使用VGG16、ViT-B/16和CoAtNet-Tiny三种图像识别模型进行分类，并采用三折受试者独立分割进行评估。

Result: 所有方法在训练数据上表现良好，但在未见数据上的表现不同。VGG16表现稳定但准确率较低；ViT-B/16在某些运行中表现强劲，但性能波动较大；CoAtNet-Tiny表现最可靠，平均准确率最高，跨折叠曲线最稳定，并在代表性不足的抑郁症和精神分裂症类别上取得了最强的精确度、召回率和F1分数。

Conclusion: 研究结果表明CoAtNet-Tiny在活动记录图像上表现最一致，而VGG16和ViT-B/16结果喜忧参半。这些观察结果表明，某些混合设计（如CoAtNet-Tiny）可能特别适合依赖活动记录图像的精神健康工作。
...
### [87] [Deep Filament Extraction for 3D Concrete Printing](https://arxiv.org/abs/2512.00091)
*Karam Mawas,Mehdi Maboudi,Pedro Achanccaray,Markus Gerke*

Main category: cs.CV

TL;DR: 本文提出了一种用于挤压式和喷射式3D混凝土打印（3DCP）中细丝（层）几何质量控制的自动化流程，该流程独立于传感器，可用于在线和打印后质量控制。


<details>
  <summary>Details</summary>
Motivation: 建筑、工程和施工（AEC）行业需要可持续和高效的设计与施工。在大型3D混凝土打印中，细丝（层）是构成打印结构的基本要素，其几何质量控制至关重要。

Method: 本文提出了一种自动化程序，用于挤压式和喷射式3DCP方法的细丝质量控制。该工作流程独立于数据采集传感器（如相机、结构光系统或地面激光扫描仪），并且可用于新鲜或固化状态的材料，从而支持在线和打印后质量控制。

Result: 本文成功地提出并描述了一种自动化细丝质量控制程序和一种通用的工作流程，该流程适用于挤压式和喷射式3DCP方法，不受传感器类型限制，并可在材料的新鲜或固化状态下进行质量检查。

Conclusion: 该研究提供了一种灵活且自动化的解决方案，用于确保3D混凝土打印中关键细丝结构的几何质量，从而有助于提高大型3DCP的整体质量和可靠性。
...
### [88] [Hybrid Synthetic Data Generation with Domain Randomization Enables Zero-Shot Vision-Based Part Inspection Under Extreme Class Imbalance](https://arxiv.org/abs/2512.00125)
...
### [89] [Analysis of Incursive Breast Cancer in Mammograms Using YOLO, Explainability, and Domain Adaptation](https://arxiv.org/abs/2512.00129)
...
### [90] [Local and Global Context-and-Object-part-Aware Superpixel-based Data Augmentation for Deep Visual Recognition](https://arxiv.org/abs/2512.00130)
...
### [91] [Mammo-FM: Breast-specific foundational model for Integrated Mammographic Diagnosis, Prognosis, and Reporting](https://arxiv.org/abs/2512.00198)
...
### [92] [ReactionMamba: Generating Short &Long Human Reaction Sequences](https://arxiv.org/abs/2512.00208)
...
### [93] [DenseScan: Advancing 3D Scene Understanding with 2D Dense Annotation](https://arxiv.org/abs/2512.00226)
...
### [94] [Relightable Holoported Characters: Capturing and Relighting Dynamic Human Performance from Sparse Views](https://arxiv.org/abs/2512.00255)
...
### [95] [UniDiff: Parameter-Efficient Adaptation of Diffusion Models for Land Cover Classification with Multi-Modal Remotely Sensed Imagery and Sparse Annotations](https://arxiv.org/abs/2512.00261)
...
### [96] [HeartFormer: Semantic-Aware Dual-Structure Transformers for 3D Four-Chamber Cardiac Point Cloud Reconstruction](https://arxiv.org/abs/2512.00264)
*Zhengda Ma,Abhirup Banerjee*

Main category: cs.CV

TL;DR: 本文提出了HeartFormer，首个基于点云表示的几何深度学习框架，用于从电影MRI数据中进行3D四腔心脏重建，并构建了首个大规模多类别心脏数据集HeartCompv1，实现了超越现有技术的鲁棒、准确和泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统的电影MRI数据仅提供心脏的2D切片图像，限制了对心脏形态和生理机制的全面理解。研究旨在克服这一局限，实现3D心脏的精确重建。

Method: 研究提出了一种名为HeartFormer的多类别点云补全网络。HeartFormer包含两个核心组件：语义感知双结构Transformer网络（SA-DSTNet），用于生成具有全局和子结构几何特征的初始粗糙点云；以及语义感知几何特征细化Transformer网络（SA-GFRTNet），用于在语义-几何表示的指导下逐步细化粗糙输出。此外，研究还构建了HeartCompv1，一个包含17,000个高分辨率3D多类别心脏网格和点云的大规模数据集。

Result: HeartFormer在HeartCompv1和UK Biobank数据集上进行了广泛的跨领域实验，结果表明它实现了鲁棒、准确和可泛化的性能，并持续超越了现有最先进的方法。

Conclusion: HeartFormer是首个基于点云表示的几何深度学习框架，能够从电影MRI数据中进行3D四腔心脏重建，有效解决了传统2D切片数据的局限性。同时，HeartCompv1数据集的构建为该新兴研究方向建立了通用基准。
...
### [97] [USB: Unified Synthetic Brain Framework for Bidirectional Pathology-Healthy Generation and Editing](https://arxiv.org/abs/2512.00269)
*Jun Wang,Peirong Liu*

Main category: cs.CV

TL;DR: USB是一个端到端框架，首次统一了病理性和健康脑图像的双向生成和编辑，通过配对扩散机制和一致性引导，解决了配对数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 获取配对的病理性和健康脑图像极其困难，因为这依赖于治疗前后的影像数据。现有的大多数脑图像生成和编辑方法都专注于视觉质量，但它们是领域特定的，并且独立地处理病理性和健康图像建模，未能统一两者。

Method: 本文引入了USB（统一合成大脑），这是一个端到端框架，通过配对扩散机制对病变和脑解剖的联合分布进行建模，从而实现病理性和健康图像的生成。此外，它还采用了一致性引导算法，在双向病理-健康编辑过程中保持解剖一致性和病变对应关系。

Result: 在包括健康对照、中风和阿尔茨海默病患者的六个公共脑MRI数据集上进行的广泛实验表明，USB能够生成多样化且逼真的结果。它建立了首个统一的脑图像生成和编辑基准。

Conclusion: USB作为首个统一的脑图像生成和编辑基准，为可扩展的数据集创建和稳健的神经影像分析提供了新的机会。
...
### [98] [HIMOSA: Efficient Remote Sensing Image Super-Resolution with Hierarchical Mixture of Sparse Attention](https://arxiv.org/abs/2512.00275)
*Yi Liu,Yi Wan,Xinyi Liu,Qiong Wu,Panwang Xia,Xuejun Huang,Yongjun Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种名为HIMOSA的轻量级遥感图像超分辨率框架，通过内容感知稀疏注意力机制和分层窗口扩展，在保持高性能的同时显著提升了计算效率。


<details>
  <summary>Details</summary>
Motivation: 在遥感应用（如灾害检测与响应）中，实时效率和模型轻量化至关重要。然而，现有的遥感图像超分辨率方法通常需要在模型性能和计算效率之间进行权衡。

Method: HIMOSA框架利用遥感图像固有的冗余性，引入了内容感知稀疏注意力机制以实现快速推理和强大的重建性能。此外，为有效利用遥感图像中的多尺度重复模式，该方法引入了分层窗口扩展，并通过调整注意力的稀疏性来降低计算复杂度。

Result: 在多个遥感数据集上的大量实验表明，HIMOSA方法在保持计算效率的同时，实现了最先进的性能。

Conclusion: HIMOSA成功地解决了遥感图像超分辨率中性能与效率的权衡问题，为实时遥感应用提供了高效且高性能的解决方案。
...
### [99] [Rethinking Lung Cancer Screening: AI Nodule Detection and Diagnosis Outperforms Radiologists, Leading Models, and Standards Beyond Size and Growth](https://arxiv.org/abs/2512.00281)
...
TL;DR: 该研究提出了一种AI系统，用于在低剂量CT扫描中直接对肺结节进行检测和恶性诊断，显著优于放射科医生和现有AI模型，尤其在早期癌症诊断方面。


<details>
  <summary>Details</summary>
Motivation: 肺结节的早期恶性检测对肺癌诊断至关重要，但现有筛查方法依赖于结节大小和生长，导致诊断延迟。因此，需要一种能直接在结节层面进行检测和诊断的AI系统。

Method: 设计了一个AI系统，该系统由浅层深度学习模型和基于特征的专业模型组成。系统在包含25,709次扫描和69,449个标注结节的数据集上进行训练和评估。

Result: 该系统在内部数据集上实现了0.98的受试者工作特征曲线下面积（AUC），在独立队列上达到0.945。在99.3%的敏感度下，每张扫描的假阳性率为0.5。它在所有结节大小和分期（尤其是一期癌症）以及所有基于生长的指标（包括最不准确的体积倍增时间）上均优于放射科医生、Lung-RADS和其他领先的AI模型。此外，它能将不确定和生长缓慢结节的诊断时间提前最多一年。

Conclusion: 该AI系统通过直接在结节层面进行检测和恶性诊断，重新定义了肺癌筛查，有效克服了传统筛查方法的局限性。它在准确性和早期诊断方面显著优于人类专家和现有AI模型，为AI在临床应用中扫清了关键障碍。
...
### [100] [Words into World: A Task-Adaptive Agent for Language-Guided Spatial Retrieval in AR](https://arxiv.org/abs/2512.00294)
*Lixing Guo,Tobias Höllerer*

Main category: cs.CV

TL;DR: 本文提出一个模块化AR代理系统，结合多模态大语言模型（MLLMs）和接地视觉模型，实现开放词汇的关系推理和语言引导的空间检索，生成动态AR场景图，并支持人机交互。


<details>
  <summary>Details</summary>
Motivation: 传统增强现实（AR）系统主要依赖固定类别检测器或标记，限制了其解释复杂、开放词汇自然语言查询的能力。

Method: 该系统整合了多模态大语言模型（MLLMs）与接地视觉模型；使用自适应任务代理协调MLLMs和坐标感知感知工具；构建编码九种类型关系（空间、结构语义、因果功能）的动态AR场景图；通过任务自适应的兴趣区域高亮和上下文空间检索引导用户；动态调用坐标感知工具（选择、测量、比较、执行）处理复杂查询；采用模块化架构支持即插即用视觉语言模型；并引入GroundedAR-Bench评估框架。

Result: 该系统实现了空间中的关系推理和物理环境中的语言条件空间检索；能处理从简单物体识别到多物体关系推理的各种查询复杂性，并返回米级精度的3D锚点；使MLLMs能够理解3D空间中物体的存在、关系和交互；通过引导人类注意力到信息密集区域并支持人机循环优化，增强了交互性；将语言理解根植于物理操作；并确立AR代理作为增强MLLMs现实世界空间智能的中间件。

Conclusion: 该模块化AR代理系统通过开放词汇的关系推理和精确的空间定位，增强了多模态大语言模型的现实世界空间智能，实现了交互式场景理解，克服了传统AR系统的局限性。
...
### [101] [TGSFormer: Scalable Temporal Gaussian Splatting for Embodied Semantic Scene Completion](https://arxiv.org/abs/2512.00300)
...
### [102] [Optimizing Distributional Geometry Alignment with Optimal Transport for Generative Dataset Distillation](https://arxiv.org/abs/2512.00308)
...
### [103] [ART-ASyn: Anatomy-aware Realistic Texture-based Anomaly Synthesis Framework for Chest X-Rays](https://arxiv.org/abs/2512.00310)
...
### [104] [Odometry Without Correspondence from Inertially Constrained Ruled Surfaces](https://arxiv.org/abs/2512.00327)
...
### [105] [MVAD : A Comprehensive Multimodal Video-Audio Dataset for AIGC Detection](https://arxiv.org/abs/2512.00336)
...
### [106] [Assimilation Matters: Model-level Backdoor Detection in Vision-Language Pretrained Models](https://arxiv.org/abs/2512.00343)
...
### [107] [mmPred: Radar-based Human Motion Prediction in the Dark](https://arxiv.org/abs/2512.00345)
...
### [108] [SMamDiff: Spatial Mamba for Stochastic Human Motion Prediction](https://arxiv.org/abs/2512.00355)
...
### [109] [Towards aligned body representations in vision models](https://arxiv.org/abs/2512.00365)
*Andrey Gizdov,Andrea Procopio,Yichen Li,Daniel Harari,Tomer Ullman*

Main category: cs.CV

TL;DR: 研究发现，较小的分割模型能自然形成类似人类的粗粒度物体“身体”表征，这与人类物理推理中的表征相似，而大型模型则倾向于过度详细的表征。


<details>
  <summary>Details</summary>
Motivation: 人类的物理推理依赖于粗粒度的“身体”表征，但其内部结构尚不清楚。本研究旨在测试经过分割训练的视觉模型是否能发展出类似的表征。

Method: 研究将一个针对50名人类参与者的心理物理学实验改编为语义分割任务，并测试了七个不同大小的分割网络。

Result: 结果显示，较小的模型自然形成了类似人类的粗粒度身体表征，而较大的模型则倾向于形成过于详细、细粒度的编码。

Conclusion: 研究表明，粗粒度表征可以在计算资源有限的情况下出现，并且机器表征可以为理解大脑中物理推理的结构提供可扩展的途径。
...
### [110] [MM-DETR: An Efficient Multimodal Detection Transformer with Mamba-Driven Dual-Granularity Fusion and Frequency-Aware Modality Adapters](https://arxiv.org/abs/2512.00363)
*Jianhong Han,Yupei Wang,Yuan Zhang,Liang Chen*

Main category: cs.CV

TL;DR: MM-DETR是一个轻量级、高效的多模态目标检测框架，通过基于Mamba的双粒度融合编码器、区域感知2D选择性扫描补全分支以及轻量级频率感知模态适配器，解决了现有方法在性能、轻量化和模态特异性建模方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态遥感目标检测方法在性能和轻量化设计之间难以平衡；依赖注意力或可变形卷积的融合模块过于复杂；共享骨干网络提取的特征次优；双流架构参数量过大，限制了实际部署。

Method: 本文提出了MM-DETR框架：1) 基于Mamba的双粒度融合编码器，将全局交互重构为通道级动态门控，利用一维选择性扫描实现高效跨模态建模。2) 将多模态融合重新解释为模态补全问题，引入区域感知2D选择性扫描补全分支，沿双向金字塔路径恢复模态特异性线索。3) 在共享骨干网络中插入轻量级频率感知模态适配器，采用空间-频率协同专家结构和像素级路由动态平衡专家贡献，以减少参数冗余并保留强大的特征提取能力。

Result: 在四个多模态基准数据集上进行了大量实验，证明了所提方法的有效性和泛化能力。

Conclusion: MM-DETR提供了一种有效且通用的轻量级高效多模态目标检测解决方案，通过创新的Mamba融合、模态补全和频率感知适配器设计，克服了现有方法的局限性。
...
### [111] [THCRL: Trusted Hierarchical Contrastive Representation Learning for Multi-View Clustering](https://arxiv.org/abs/2512.00368)
*Jian Zhu*

Main category: cs.CV

TL;DR: 本文提出了一种名为THCRL的新型可信分层对比表示学习方法，通过深度对称分层融合和平均K近邻对比学习，解决了多视图聚类中因噪声和缺乏结构信息导致的不可信融合问题，并实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 多视图聚类（MVC）中存在“不可信融合”的挑战。这主要是由于：1）现有方法忽略了单个视图中固有的噪声；2）传统的对比学习（CL）方法在计算相似度时仅依赖同一实例的不同视图，而忽略了同一簇内最近邻的结构信息，导致多视图融合方向错误。

Method: 本文提出了可信分层对比表示学习（THCRL）。它包含两个关键模块：1）深度对称分层融合（DSHF）模块：利用集成了多个去噪机制的UNet架构，实现多视图数据的可信融合。2）平均K近邻对比学习（AKCL）模块：将融合表示与视图特定表示对齐。与传统策略不同，AKCL通过增强同一簇内样本（而非仅同一实例跨视图）的表示相似性，强化了融合表示的置信度。

Result: 广泛的实验证明，THCRL在深度多视图聚类任务中取得了最先进的性能。

Conclusion: THCRL通过引入深度对称分层融合模块（结合去噪UNet）和平均K近邻对比学习模块（关注簇内邻居结构），有效解决了多视图聚类中不可信融合的问题，显著提升了聚类性能。
...
### [112] [POLARIS: Projection-Orthogonal Least Squares for Robust and Adaptive Inversion in Diffusion Models](https://arxiv.org/abs/2512.00369)
*Wenshuo Chen,Haosen Li,Shaofeng Liang,Lei Wang,Haozhe Jia,Kaishen Yuan,Jieming Wu,Bowen Tian,Yutao Yue*

Main category: cs.CV

TL;DR: 本文揭示了扩散模型反演去噪范式中近似噪声误差导致重建退化的关键问题。POLARIS通过将引导尺度视为分步变量，并推导数学公式来最小化每一步的反演误差，从而显著提高潜在编码质量并改善下游任务准确性，且仅需一行代码改动。


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的反演去噪范式在图像编辑和恢复任务中表现出色，但其重建质量存在退化。研究发现，这种退化源于近似噪声误差，即用t-1步的预测近似t步的噪声，导致反演过程中严重的误差累积。

Method: 本文提出了POLARIS（鲁棒自适应反演的投影正交最小二乘法）。它将反演问题从误差补偿重新定义为误差源问题。POLARIS将引导尺度ω视为一个分步变量，并推导出一个具有数学基础的公式，以在每一步最小化反演误差。该方法仅需一行代码即可实现。

Result: POLARIS显著减轻了噪声近似误差，持续提高了下游任务的准确性。它在可忽略的性能开销下，显著提升了反演潜在编码的质量。

Conclusion: POLARIS成功地解决了扩散模型反演过程中近似噪声误差导致的重建退化问题。通过在每一步自适应地最小化误差，POLARIS以极小的改动和开销，显著提升了反演的准确性和下游任务的表现。
...
### [113] [EZ-SP: Fast and Lightweight Superpoint-Based 3D Segmentation](https://arxiv.org/abs/2512.00385)
*Louis Geist,Loic Landrieu,Damien Robert*

Main category: cs.CV

TL;DR: 本文提出了一种名为EZ-SP的可学习、全GPU超点分割算法，相比现有方法速度提升13倍，能够生成几何和语义连贯的超点。结合轻量级分类器，整个管线在保持与最先进点云模型相当的精度下，实现72倍的推理速度提升和120倍的参数减少，支持实时推理，适用于多种3D语义分割任务。


<details>
  <summary>Details</summary>
Motivation: 超点（superpoint）基线的3D语义分割方法虽然高效，但其CPU受限的分割步骤是性能瓶颈。

Method: 提出一种可学习、全GPU的分割算法，用于生成几何和语义连贯的超点。该模块紧凑（参数少于60k），通过可微分替代损失在20分钟内训练完成，无需手动特征。它与一个轻量级超点分类器结合，形成完整的管线。

Result: 所提出的分割算法比现有方法快13倍。整个管线占用VRAM小于2MB，可扩展到数百万点场景，并支持实时推理。EZ-SP的推理速度比基于点的SOTA模型快72倍，参数减少120倍，但在室内扫描（S3DIS）、自动驾驶（KITTI-360）和航空激光雷达（DALES）三个领域均能达到与SOTA模型相当的精度。

Conclusion: EZ-SP提供了一种高效、精确且资源友好的3D语义分割超点解决方案，成功解决了以往方法的CPU瓶颈问题，并在多种应用场景下实现了实时高性能表现。
...
### [114] [WiseEdit: Benchmarking Cognition- and Creativity-Informed Image Editing](https://arxiv.org/abs/2512.00387)
*Kaihang Pan,Weile Chen,Haiyi Qiu,Qifan Yu,Wendong Bu,Zehan Wang,Yun Zhu,Juncheng Li,Siliang Tang*

Main category: cs.CV

TL;DR: WiseEdit是一个知识密集型基准，用于全面评估认知和创造力驱动的图像编辑模型，揭示了现有模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准评估范围过于狭窄，未能全面评估最新图像编辑模型的高级智能能力（认知和创造力驱动）。

Method: 引入了WiseEdit基准，通过将图像编辑分解为感知、解释和想象三个级联步骤（每个步骤对应一个挑战性任务），并包含更复杂的任务。它还整合了陈述性、程序性和元认知三种基本知识类型。WiseEdit包含1,220个测试用例。

Result: WiseEdit客观地揭示了当前最先进（SoTA）图像编辑模型在基于知识的认知推理和创意构图能力方面的局限性。

Conclusion: WiseEdit提供了一个全面评估认知和创造力驱动图像编辑模型的新范式，突显了现有模型在高级智能能力上的不足。
...
### [115] [Pore-scale Image Patch Dataset and A Comparative Evaluation of Pore-scale Facial Features](https://arxiv.org/abs/2512.00381)
*Dong Li,HuaLiang Lin,JiaYu Li*

Main category: cs.CV

TL;DR: 本文提出了PorePatch数据集和DMCE框架，以解决面部弱纹理区域的局部描述符匹配挑战。深度学习模型在匹配任务上表现出色，但在3D重建任务中优势不明显，表明该领域仍需进一步研究。


<details>
  <summary>Details</summary>
Motivation: 面部皮肤区域的弱纹理特性给面部运动分析和3D面部重建中的局部描述符匹配带来了巨大挑战。尽管深度学习描述符在许多应用中表现优异，但缺乏毛孔级图像块数据集阻碍了其在面部领域的进一步发展。

Method: 本文提出了PorePatch数据集，一个高质量的毛孔级图像块数据集，并建立了一个合理的评估基准。引入了数据-模型协同演化（DMCE）框架，从高分辨率面部图像中生成逐步精炼的高质量数据集。随后，在所构建的数据集上训练了现有的SOTA模型，并进行了广泛实验。

Result: SOTA模型在匹配任务中实现了1.91%的FPR95值，比PSIFT（22.41%）高出20.5%。然而，在3D重建任务中，其优势减弱，整体性能并未比传统描述符显著提升。

Conclusion: 深度学习描述符在解决面部弱纹理区域的挑战方面仍存在局限性，该领域仍有大量工作需要完成。
...
### [116] [Better, Stronger, Faster: Tackling the Trilemma in MLLM-based Segmentation with Simultaneous Textual Mask Prediction](https://arxiv.org/abs/2512.00395)
...
### [117] [Low-Bitrate Video Compression through Semantic-Conditioned Diffusion](https://arxiv.org/abs/2512.00408)
*Lingdong Wang,Guan-Ming Su,Divya Kothandaraman,Tsung-Wei Huang,Mohammad Hajiesmaili,Ramesh K. Sitaraman*

Main category: cs.CV

TL;DR: DiSCo是一种语义视频压缩框架，通过传输有意义的信息并利用生成先验合成细节，在超低比特率下实现高质量、时间连贯的视频重建，显著优于传统编解码器。


<details>
  <summary>Details</summary>
Motivation: 传统视频编解码器在超低比特率下无法保持像素保真度，产生严重伪影，原因在于像素精度与人类感知之间的根本错位。因此，需要一种更符合人类感知的压缩方法。

Method: DiSCo框架将源视频分解为三种紧凑模态：文本描述（语义）、时空降级视频（外观）以及可选的草图或姿态（运动线索）。一个条件视频扩散模型从这些紧凑表示中重建高质量、时间连贯的视频。为改进多模态生成和模态紧凑性，提出了时间前向填充、令牌交错和模态特定编解码器。

Result: 在低比特率下，该方法在感知指标上比基线语义和传统编解码器性能高出2-10倍。

Conclusion: DiSCo框架通过仅传输最有意义的信息并依赖生成先验进行细节合成，成功解决了超低比特率下的视频压缩挑战，实现了卓越的感知质量和时间连贯性。
...
### [118] [SplatFont3D: Structure-Aware Text-to-3D Artistic Font Generation with Part-Level Style Control](https://arxiv.org/abs/2512.00413)
...
### [119] [PhysGen: Physically Grounded 3D Shape Generation for Industrial Design](https://arxiv.org/abs/2512.00422)
...
### [120] [Recovering Origin Destination Flows from Bus CCTV: Early Results from Nairobi and Kigali](https://arxiv.org/abs/2512.00424)
...
### [121] [What about gravity in video generation? Post-Training Newton's Laws with Verifiable Rewards](https://arxiv.org/abs/2512.00425)
...
### [122] [Recognizing Pneumonia in Real-World Chest X-rays with a Classifier Trained with Images Synthetically Generated by Nano Banana](https://arxiv.org/abs/2512.00428)
...
### [123] [FR-TTS: Test-Time Scaling for NTP-based Image Generation with Effective Filling-based Reward Signal](https://arxiv.org/abs/2512.00438)
...
### [124] [RecruitView: A Multimodal Dataset for Predicting Personality and Interview Performance for Human Resources Applications](https://arxiv.org/abs/2512.00450)
*Amit Kumar Gupta,Farhan Sheth,Hammad Shaikh,Dheeraj Kumar,Angkul Puniya,Deepak Panwar,Sandeep Chaurasia,Priya Mathur*

Main category: cs.CV

TL;DR: 本文介绍了RecruitView数据集和CRMF几何深度学习框架，用于从多模态行为数据中自动评估人格和软技能，CRMF通过建模行为的几何结构，显著提升了评估性能和参数效率。


<details>
  <summary>Details</summary>
Motivation: 由于数据集有限以及现有方法未能捕捉人类特质固有的几何结构，从多模态行为数据中自动评估人格和软技能仍然面临挑战。

Method: 本文提出了RecruitView数据集，包含2011个自然视频面试片段和27000个人格及面试表现的成对比较判断。为利用此数据，作者提出了跨模态流形融合回归（CRMF）框架，这是一种几何深度学习方法，明确地在双曲、球面和欧几里得流形上建模行为表示。CRMF使用几何特定的专家网络捕捉分层特质结构、定向行为模式和连续性能变化，并通过自适应路由机制动态加权专家贡献，并通过切线空间融合实现优异性能。

Result: CRMF在训练参数比大型多模态模型少40-50%的情况下，取得了卓越的性能。实验表明，CRMF显著优于所选基线，Spearman相关性提高了11.4%，一致性指数提高了6.0%。RecruitView数据集已公开发布。

Conclusion: CRMF通过显式建模行为数据的几何结构，在人格和软技能评估方面取得了显著的性能提升和参数效率，RecruitView数据集为该领域的研究提供了宝贵的资源。
...
### [125] [CausalAffect: Causal Discovery for Facial Affective Understanding](https://arxiv.org/abs/2512.00456)
*Guanyu Hu,Tangzheng Lian,Dimitrios Kollias,Oya Celiktutan,Xinyu Yang*

Main category: cs.CV

TL;DR: 该论文提出了CausalAffect，一个用于面部情感分析中因果图发现的框架。它通过两级因果层级建模AU-AU和AU-表情的依赖关系，并利用反事实干预来识别真正的因果效应，无需联合标注数据或预设因果先验，实现了AU检测和表情识别的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 虽然动作单元（AUs）是情感计算的基础，但现有方法很少直接从数据中推断AUs和表情之间心理学上合理的因果关系，而这对于理解面部行为驱动的潜在依赖至关重要。

Method: 该研究提出了CausalAffect框架，用于面部情感分析中的因果图发现。它采用一个两级的、极性和方向感知的因果层级结构来建模AU-AU和AU-表情的依赖关系，该结构结合了群体层面的规律性和样本自适应结构。此外，它还引入了一种特征层面的反事实干预机制，以强制执行真实的因果效应并抑制虚假相关性。该方法无需联合标注数据集或手工设计的因果先验。

Result: CausalAffect恢复了与既定心理学理论一致的因果结构，并揭示了新颖的抑制性依赖和以前未被表征的依赖关系。在六个基准测试中，它在AU检测和表情识别方面都取得了最先进的（SOTA）性能。

Conclusion: CausalAffect通过建立因果发现与可解释面部行为之间的原则性联系，推动了面部情感分析的进步。它能直接从数据中发现心理学上合理的因果关系，而无需额外的标注或先验知识。
...
### [126] [Structured Context Learning for Generic Event Boundary Detection](https://arxiv.org/abs/2512.00475)
*Xin Gu,Congcong Li,Xinyao Wang,Dexiang Hong,Libo Zhang,Tiejian Luo,Longyin Wen,Heng Fan*

Main category: cs.CV

TL;DR: 本文提出了一种名为“结构化上下文学习”的新型通用事件边界检测（GEBD）方法，引入结构化序列划分（SPoS）提供结构化上下文，以提高速度-精度权衡，并在多个基准数据集上超越现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 通用事件边界检测（GEBD）旨在识别视频中人类感知的事件边界时刻。现有方法可能在速度和精度之间存在权衡，并且可能受限于特定的时间模型。本文旨在提出一种更灵活、高效且准确的方法来解决这一任务。

Method: 本文提出结构化上下文学习方法，引入结构化序列划分（SPoS）来为时间信息学习提供结构化上下文。该方法是端到端可训练且灵活的，不限于特定的时间模型。SPoS以线性计算复杂度划分输入帧序列，为后续时间模型提供结构化上下文。接着，通过计算组相似性来捕捉帧间差异，并利用轻量级全卷积网络根据分组相似性图确定事件边界。为解决边界标注的模糊性，还采用高斯核预处理真值事件边界。

Result: 该方法在Kinetics-GEBD、TAPOS和镜头转换检测等挑战性数据集上进行了广泛评估，结果表明其性能优于现有最先进的方法，并实现了更好的速度-精度权衡。

Conclusion: 本文提出的结构化上下文学习方法，通过引入SPoS和灵活的网络架构，有效解决了通用事件边界检测任务，不仅在多个基准数据集上展现出卓越的性能，还在速度和精度之间取得了更好的平衡。
...
### [127] [RealGen: Photorealistic Text-to-Image Generation via Detector-Guided Rewards](https://arxiv.org/abs/2512.00473)
*Junyan Ye,Leiqi Zhu,Yuncheng Guo,Dongzhi Jiang,Zilong Huang,Yifan Zhang,Zhiyuan Yan,Haohuan Fu,Conghui He,Weijia Li*

Main category: cs.CV

TL;DR: RealGen是一个新的框架，通过结合LLM提示优化、扩散模型和创新的“检测器奖励”机制，显著提升了文本到图像生成的真实感，并提出了一个名为RealBench的自动化评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型（如GPT-Image-1、Qwen-Image）在文本到图像一致性和世界知识方面表现出色，但在生成逼真图像方面仍存在不足，常产生带有“过度光滑皮肤”和“油腻面部光泽”等AI痕迹的“虚假”图像。研究目标是实现“与现实无法区分”的图像生成。

Method: RealGen框架整合了用于提示优化的LLM组件和用于真实感图像生成的扩散模型。它引入了“检测器奖励”机制，利用语义级和特征级合成图像检测器来量化AI痕迹并评估真实感。该奖励信号结合GRPO算法用于优化整个生成流程。此外，还提出了RealBench，一个采用检测器评分和竞技场评分的自动化评估基准，以实现无人参与的真实感评估。

Result: 实验表明，RealGen在真实感、细节和美学方面显著优于GPT-Image-1、Qwen-Image等通用模型以及FLUX-Krea等专业真实感模型。RealBench实现了无人参与的真实感评估，其结果更准确且与真实用户体验更一致。

Conclusion: RealGen成功解决了当前文本到图像模型在生成逼真图像方面的不足，通过其创新的框架和检测器奖励机制，显著提升了图像的真实感、细节和美学。同时，RealBench提供了一个更准确、更符合用户体验的自动化真实感评估方法。
...
### [128] [Learning What Helps: Task-Aligned Context Selection for Vision Tasks](https://arxiv.org/abs/2512.00489)
*Jingyu Guo,Emir Konuk,Fredrik Strand,Christos Matsoukas,Kevin Smith*

Main category: cs.CV

TL;DR: TACS框架通过结合梯度和强化学习的混合优化方案，使视觉转换器（ViTs）能够学习选择真正能提升任务性能的上下文示例，而非仅仅是相似的示例，从而在多种视觉任务中优于基于相似性的检索方法。


<details>
  <summary>Details</summary>
Motivation: 人类在解决视觉不确定性时常通过与相关示例比较，但当前的ViTs缺乏识别哪些示例能有效提升预测的能力，这限制了它们在复杂或数据受限场景下的表现。

Method: TACS框架通过混合优化方案（结合基于梯度的监督和强化学习）共同训练一个选择器网络和任务模型。该方法将检索作为学习目标的一部分，使选择器能够根据任务奖励来选择对性能有实际帮助的示例。

Result: TACS在18个数据集上（涵盖细粒度识别、医学图像分类和医学图像分割）持续优于基于相似性的检索方法，尤其在具有挑战性或数据有限的环境中表现突出。

Conclusion: 通过将示例选择与任务奖励对齐，TACS使判别模型能够发现真正有帮助的上下文示例，从而有效解决了ViTs在处理视觉不确定性时选择有用信息的能力不足的问题。
...
### [129] [CC-FMO: Camera-Conditioned Zero-Shot Single Image to 3D Scene Generation with Foundation Model Orchestration](https://arxiv.org/abs/2512.00493)
*Boshi Tang,Henry Zheng,Rui Huang,Gao Huang*

Main category: cs.CV

TL;DR: CC-FMO是一种零样本、相机条件下的单图像到3D场景生成方法，它通过结合语义感知实例生成和相机条件姿态估计，实现了高保真且空间一致的3D场景生成。


<details>
  <summary>Details</summary>
Motivation: 早期方法因依赖特定模型和小数据集而泛化性差。尽管最近的大规模3D基础模型在实例级生成方面有进步，但在连贯的场景生成方面仍面临挑战，主要受限于不准确的物体姿态估计和空间不一致性。

Method: 本文提出了CC-FMO，一个零样本、相机条件下的单图像到3D场景生成流程。它采用混合实例生成器，结合语义感知的向量集表示和细节丰富的结构化潜在表示，以生成语义合理且高质量的物体几何。此外，CC-FMO通过一个简单有效的相机条件尺度求解算法，将基础姿态估计模型应用于场景生成任务，以强制实现场景层面的连贯性。

Result: 广泛的实验表明，CC-FMO能够持续生成高保真、与相机对齐的组合场景，并且性能优于所有现有最先进的方法。

Conclusion: CC-FMO成功解决了从单图像生成高质量3D场景的挑战，通过确保实例保真度和场景级连贯性，为AR/VR和具身AI应用提供了重要的解决方案。
...
### [130] [Image Generation as a Visual Planner for Robotic Manipulation](https://arxiv.org/abs/2512.00532)
*Ye Pang*

Main category: cs.CV

TL;DR: 本文提出使用经过轻量级LoRA微调的预训练图像生成模型作为机器人视觉规划器，通过文本或轨迹条件生成连贯的机器人操作视频，展现了其潜在的时间建模能力。


<details>
  <summary>Details</summary>
Motivation: 生成逼真的机器人操作视频是实现具身智能体感知、规划和行动统一的关键一步。现有视频扩散模型需要大量特定领域数据且泛化能力差，而最近在语言-图像语料库上训练的图像生成模型展现出强大的组合性，包括合成时间连贯的网格图像的能力，这表明它们即使没有显式的时间建模也可能具备视频生成潜力。

Method: 本文提出了一个两部分框架：(1) 文本条件生成，使用语言指令和第一帧；(2) 轨迹条件生成，使用2D轨迹叠加和同一初始帧。通过LoRA微调对预训练图像生成模型进行轻量级适应。

Result: 在Jaco Play、Bridge V2和RT1数据集上的实验表明，两种模式都能生成与各自条件对齐的平滑、连贯的机器人视频。

Conclusion: 研究结果表明，预训练图像生成器编码了可转移的时间先验知识，并且在最少监督下可以作为视频式机器人规划器发挥作用。
...
### [131] [Terrain Sensing with Smartphone Structured Light: 2D Dynamic Time Warping for Grid Pattern Matching](https://arxiv.org/abs/2512.00514)
*Tanaka Nobuaki*

Main category: cs.CV

TL;DR: 本文提出一种基于智能手机的结构光系统，通过投射网格图案并利用拓扑约束的二维动态时间规整（2D-DTW）算法，从单个手持设备重建局部地形不平整度，以提高低成本移动漫游车在不平坦地形上的稳定性。


<details>
  <summary>Details</summary>
Motivation: 低成本移动漫游车在不平坦地形上运行时，微小的颠簸或倾斜难以通过视觉感知，但会显著影响其运动稳定性。现有方法可能成本较高或不适合资源受限的移动平台。

Method: 该系统采用智能手机作为平台，投射网格图案到地面，并从单手持设备重建局部地形不平整度。核心方法是提出了一种拓扑约束的二维动态时间规整（2D-DTW）算法。该算法在全局网格一致性约束下执行逐列对齐，以鲁棒地匹配投影网格与其在透视畸变和部分遮挡下的变形观测，解决了传统一维DTW不适用于二维网格模式的问题。该方法设计得足够简单，可在资源有限的平台上运行。

Result: 所提出的2D-DTW公式不仅可用于地形感知，还可以作为一种通用的工具，用于图像处理场景中结构化网格图案的匹配。该方法在资源有限的平台上运行的同时，保留了精确三角测量所需的网格结构。

Conclusion: 本文描述了一个整体系统设计以及由此应用衍生出的2D-DTW扩展。该系统为低成本移动漫游车提供了有效的地形感知能力，且2D-DTW算法具有更广泛的图像处理应用潜力。
...
### [132] [Cross-Temporal 3D Gaussian Splatting for Sparse-View Guided Scene Update](https://arxiv.org/abs/2512.00534)
*Zeyuan An,Yanghang Xiao,Zhiying Leng,Frederick W. B. Li,Xiaohui Liang*

Main category: cs.CV

TL;DR: 本文提出了一种名为Cross-Temporal 3D Gaussian Splatting (Cross-Temporal 3DGS) 的新框架，用于利用稀疏图像和历史场景先验，高效地重建和更新跨时间段的3D场景。


<details>
  <summary>Details</summary>
Motivation: 在计算机视觉中，随时间维护一致的3D场景表示是一个重大挑战。从稀疏视图观测更新3D场景对于城市规划、灾害评估和历史遗址保护等多种实际应用至关重要，因为在这些场景中，密集扫描通常不可用或不切实际。

Method: 该方法包含三个阶段：1) 跨时间相机对齐，用于估计和对齐不同时间戳的相机姿态；2) 基于干涉的置信度初始化，以识别时间戳之间未改变的区域，从而指导更新；3) 渐进式跨时间优化，迭代地将历史先验信息整合到3D场景中，以提高重建质量。该方法支持非连续捕获，不仅能用新的稀疏视图细化现有场景，还能借助当前捕获从有限数据中恢复过去场景。

Result: 实验结果表明，该方法在重建质量和数据效率方面均显著优于基线方法，并且能够仅使用稀疏图像实现时间变化，随后按需重建为详细的3D表示。

Conclusion: Cross-Temporal 3DGS 方法为场景版本控制、跨时间数字孪生和长期空间文档提供了有前景的解决方案。
...
### [133] [Asset-Driven Sematic Reconstruction of Dynamic Scene with Multi-Human-Object Interactions](https://arxiv.org/abs/2512.00547)
*Sandika Biswas,Qianyi Wu,Biplab Banerjee,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: 本文提出一种混合方法，结合3D生成模型、语义感知形变和GS优化，以应对单目多人类、多物体动态场景中复杂的交互和严重遮挡挑战，从而实现多视角和时间一致的高质量表面几何重建。


<details>
  <summary>Details</summary>
Motivation: 真实世界中涉及多人类和物体复杂交互的动态环境的3D几何建模对AR/VR、游戏和具身AI等应用至关重要，但由于多样化的运动模式和频繁的遮挡，尤其是在单目设置下，仍未得到充分探索。现有的3D Gaussian Splatting (GS) 方法很少能有效处理此类多人类、多物体场景，因为仅基于GS渲染损失难以在严重遮挡下保持结构一致性。

Method: 本文提出一种混合方法，有效结合了：1) 3D生成模型，用于生成场景元素的高保真网格；2) 语义感知形变，即对刚性物体进行刚性变换，对人类进行LBS形变，并将形变后的高保真网格映射到动态场景中；3) 基于GS的单个元素优化，以进一步精炼它们在场景中的对齐。

Result: 这种混合方法有助于在严重遮挡下保持物体结构，并能生成多视角和时间一致的几何结构。在HOI-M3数据集上的评估表明，该方法在生成此类场景的更好表面重建方面优于现有最先进的方法。

Conclusion: 所提出的混合方法通过结合3D生成模型、语义感知形变和GS优化，成功解决了多人类、多物体动态场景建模的挑战，即使在严重遮挡下也能保持物体结构，并生成高质量、多视角和时间一致的几何重建，显著优于现有技术。
...
### [134] [SAIDO: Generalizable Detection of AI-Generated Images via Scene-Aware and Importance-Guided Dynamic Optimization in Continual Learning](https://arxiv.org/abs/2512.00539)
...
### [135] [Describe Anything Anywhere At Any Moment](https://arxiv.org/abs/2512.00565)
...
### [136] [NeuroVolve: Evolving Visual Stimuli toward Programmable Neural Objectives](https://arxiv.org/abs/2512.00557)
...
### [137] [Integrating Skeleton Based Representations for Robust Yoga Pose Classification Using Deep Learning Models](https://arxiv.org/abs/2512.00572)
...
### [138] [SatireDecoder: Visual Cascaded Decoupling for Enhancing Satirical Image Comprehension](https://arxiv.org/abs/2512.00582)
...
### [139] [Scaling Down to Scale Up: Towards Operationally-Efficient and Deployable Clinical Models via Cross-Modal Low-Rank Adaptation for Medical Vision-Language Models](https://arxiv.org/abs/2512.00597)
...
### [140] [XAI-Driven Skin Disease Classification: Leveraging GANs to Augment ResNet-50 Performance](https://arxiv.org/abs/2512.00626)
...
### [141] [Automatic Pith Detection in Tree Cross-Section Images Using Deep Learning](https://arxiv.org/abs/2512.00625)
...
### [142] [Doppler-Enhanced Deep Learning: Improving Thyroid Nodule Segmentation with YOLOv5 Instance Segmentation](https://arxiv.org/abs/2512.00639)
*Mahmoud El Hussieni*
...
### [143] [Graph-Attention Network with Adversarial Domain Alignment for Robust Cross-Domain Facial Expression Recognition](https://arxiv.org/abs/2512.00641)
...
### [144] [Realistic Handwritten Multi-Digit Writer (MDW) Number Recognition Challenges](https://arxiv.org/abs/2512.00676)
*Kiri L. Wagstaff*

Main category: cs.CV

TL;DR: 该研究引入了多位数字书写者（MDW）基准数据集，以更真实地评估多位数字识别任务，发现孤立数字分类器在此任务上表现不佳，并强调需要新的方法和特定任务的性能指标。


<details>
  <summary>Details</summary>
Motivation: 孤立数字分类研究已久，但在现实世界中，数字常以多位形式出现，且由同一人书写（如邮政编码、支票金额）。现有的孤立数字分类不足以解决这些真实问题，因此需要更真实的多位数字识别基准。

Method: 利用NIST数字图像及其书写者信息，创建了更真实的多位数字书写者（MDW）数据集。同时，引入了超越传统错误计算的、更贴近实际影响的特定任务性能指标。

Result: 研究发现，在孤立数字上表现良好的分类器在多位数字识别任务上表现不佳。新的MDW基准数据集附带了特定任务的性能指标，并为开发能够利用特定任务知识以显著提高性能的方法提供了机会。

Conclusion: 为了解决真实的数字识别问题，需要超越单一数字分类方法的额外进展。MDW基准数据集及其特定任务指标为此提供了基础，以推动相关领域的发展。
...
### [145] [Dynamic-eDiTor: Training-Free Text-Driven 4D Scene Editing with Multimodal Diffusion Transformer](https://arxiv.org/abs/2512.00677)
*Dong In Lee,Hyungjun Doh,Seunggeun Chi,Runlin Duan,Sangpil Kim,Karthik Ramani*

Main category: cs.CV

TL;DR: Dynamic-eDiTor是一个免训练的文本驱动4D编辑框架，利用MM-DiT和4DGS实现动态4D场景的编辑，通过时空子网格注意力(STGA)和上下文令牌传播(CTP)确保多视图和时间上的一致性。


<details>
  <summary>Details</summary>
Motivation: 尽管4D表示（如Dynamic NeRF和4D Gaussian Splatting）在动态4D场景重建方面取得了进展，但文本驱动的4D场景编辑仍然探索不足。现有方法依赖于独立的2D扩散模型编辑帧，导致运动失真、几何漂移和编辑不完整，难以在空间和时间上保证多视图和时间一致性。

Method: 本文提出了Dynamic-eDiTor，一个免训练的文本驱动4D编辑框架，结合了多模态扩散Transformer (MM-DiT)和4D Gaussian Splatting (4DGS)。它包含两个核心机制：1) 时空子网格注意力 (STGA)，用于实现局部一致的跨视图和时间融合；2) 上下文令牌传播 (CTP)，通过令牌继承和光流引导的令牌替换实现全局传播。这些组件协同工作，无需额外训练即可直接优化预训练的4DGS。

Result: 在多视图视频数据集DyNeRF上的广泛实验表明，Dynamic-eDiTor在编辑保真度以及多视图和时间一致性方面均优于现有方法。

Conclusion: Dynamic-eDiTor提供了一个新颖的、免训练的解决方案，用于实现无缝且全局一致的文本驱动动态4D场景编辑，有效解决了现有方法在多视图和时间一致性方面的挑战。
...
### [146] [MambaScope: Coarse-to-Fine Scoping for Efficient Vision Mamba](https://arxiv.org/abs/2512.00647)
*Shanhui Liu,Rui Xu,Yunke Wang*

Main category: cs.CV

TL;DR: Vision Mamba的效率受限于输入token数量，现有token削减方法导致信息丢失。本文提出Coarse-to-Fine Vision Mamba (CF-ViM)，一个自适应框架，通过粗粒度推理和在低置信度区域进行精细化处理，根据图像复杂度动态调整分辨率，实现高效且准确的推理。


<details>
  <summary>Details</summary>
Motivation: Vision Mamba的效率受输入token数量的根本限制。现有token削减方法（如剪枝或合并）会固有地导致信息丢失，尤其是在不考虑视觉复杂度的情况下，对所有图像统一应用时问题更严重。研究者观察到并非所有输入都需要精细处理，简单图像可以在粗分辨率下有效处理，而复杂图像才需要精细化处理。

Method: 本文提出Coarse-to-Fine Vision Mamba (CF-ViM)，一个用于高效推理的自适应框架。CF-ViM首先通过将输入图像划分为大块来执行粗粒度推理，显著减少token长度和计算量。当模型的预测置信度较低时，选定的区域会以更精细的分辨率重新处理，以最小的额外成本恢复关键视觉细节。这种动态分辨率分配策略允许CF-ViM根据图像复杂度自适应地分配计算。

Result: 在ImageNet上的实验表明，CF-ViM在准确性和效率方面均优于基线Vision Mamba和最先进的token削减技术。

Conclusion: CF-ViM通过其自适应的粗到细推理策略，有效解决了Vision Mamba的效率瓶颈，能够在不牺牲关键视觉信息的情况下，根据图像复杂度动态分配计算，从而实现高效且准确的图像处理。
...
### [147] [Optimizing LVLMs with On-Policy Data for Effective Hallucination Mitigation](https://arxiv.org/abs/2512.00706)
*Chengzhi Yu,Yifan Xu,Yifan Chen,Wenyi Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种针对大型视觉语言模型（LVLMs）幻觉问题的新颖缓解方法，通过分析数据生成过程，引入幻觉分类器进行二元标注以确保干净的训练样本，并设计了一个鲁棒的迭代直接偏好优化（DPO）算法，显著降低了幻觉率并使开源模型超越了GPT-4V。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）在多模态任务中表现出巨大潜力，但幻觉问题仍然是一个关键挑战。现有的幻觉缓解方法在数据生成和偏好标注方面存在不足，特别是离策略数据效果不佳，且现有标注方法可能引入额外的幻觉，从而增强模型的幻觉模式。因此，需要一种高效、可靠的在策略数据偏好标注方法和鲁棒的对齐算法。

Method: ['分析了LVLM幻觉缓解中的数据生成过程，确认在策略数据显著优于离策略数据。', '提出训练一个幻觉分类器进行二元标注，以确保后续对齐训练中“干净”的选中样本，避免现有标注方法引入额外幻觉。', '设计了一个鲁棒的迭代直接偏好优化（DPO）算法，采用动态样本重加权方案，以进一步利用在策略数据的潜力。']

Result: ['在MMHalBench上，将LLaVA-1.5-7B的幻觉率降低了50.8%。', '在Object HalBench上，将平均幻觉率降低了79.5%。', '使LLaVA-1.5-13B的性能超越了GPT-4V。', '在三个基准上与8个最先进的基线进行了全面的实验对比。']

Conclusion: 该研究通过利用在策略数据、提出干净的幻觉分类器标注方法以及设计鲁棒的迭代DPO算法，有效地缓解了大型视觉语言模型的幻觉问题。其方法不仅显著降低了幻觉率，而且充分挖掘了开源模型的潜力，使其性能能够超越顶级的闭源模型（如GPT-4V）。
...
### [148] [Deep Learning-Based Computer Vision Models for Early Cancer Detection Using Multimodal Medical Imaging and Radiogenomic Integration Frameworks](https://arxiv.org/abs/2512.00714)
*Emmanuella Avwerosuoghene Oghenekaro*

Main category: cs.CV

TL;DR: 该论文探讨了深度学习和影像基因组学如何通过分析多模态影像数据并与基因组学整合，革新早期癌症检测和个性化肿瘤学，超越传统诊断方法。


<details>
  <summary>Details</summary>
Motivation: 早期癌症检测是现代医疗的重大挑战，延迟诊断会显著降低生存率。传统放射学评估难以识别肉眼不可见的细微组织异常。

Method: 使用深度学习计算机视觉模型（如CNN、Transformer和混合注意力架构）自动从MRI、CT、PET、乳腺X线、组织病理学和超声等多种模态影像数据中提取复杂模式。此外，整合多模态影像与影像基因组学，将定量影像特征与基因组学、转录组学和表观遗传生物标志物关联起来。

Result: 深度学习模型在识别细微组织异常和肿瘤微环境变异方面超越了传统放射学评估。影像基因组学融合实现了无需侵入性活检即可预测肿瘤基因型、免疫反应、分子亚型和治疗耐药性。

Conclusion: 人工智能（特别是深度学习）和影像基因组学的整合为早期癌症检测和个性化肿瘤学带来了新的范式，显著提高了诊断能力并为治疗提供了非侵入性指导。
...
### [149] [Silhouette-based Gait Foundation Model](https://arxiv.org/abs/2512.00691)
*Dingqiang Ye,Chao Fan,Kartik Narayan,Bingzhe Wu,Chengwen Luo,Jianqiang Li,Vishal M. Patel*

Main category: cs.CV

TL;DR: FoundationGait是首个可扩展的自监督步态基础模型，通过大规模预训练解决了传统步态模型的扩展性和泛化性问题，并在多种步态任务和数据集上实现了领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态模型规模小、设计狭窄，难以扩展和泛化。研究动机在于解决两大长期障碍：步态模型为何未能遵循扩展定律（可扩展性），以及一个模型能否服务于传统上独立研究的多种步态任务（泛化性）。

Method: 本文提出了FoundationGait，一个可扩展的自监督步态理解预训练框架。其最大版本拥有近1.3亿参数，并在包含超过200万个步行序列的12个公共步态数据集上进行了预训练。

Result: FoundationGait在广泛的步态数据集、条件、任务（如人体识别、脊柱侧弯筛查、抑郁症预测和属性估计）甚至输入模态上表现出强大的鲁棒性，无论是否经过微调。它在具有挑战性的Gait3D数据集上实现了48.0%的零样本rank-1准确率，在OU-MVLP数据集上实现了64.5%的准确率，为鲁棒步态识别树立了新里程碑。

Conclusion: FoundationGait成功解决了步态理解领域的扩展性和泛化性挑战，通过大规模自监督预训练，为统一的步态分析奠定了基础，并在多项关键任务中展现出卓越性能和鲁棒性。
...
### [150] [Affordance-First Decomposition for Continual Learning in Video-Language Understanding](https://arxiv.org/abs/2512.00694)
*Mengzhu Xu,Hanzhi Liu,Ningkang Peng,Qianyu Chen,Canran Xiao*

Main category: cs.CV

TL;DR: 本文提出了一种名为“Affordance-First Decomposition (AFD)”的持续学习方法，用于视频-语言理解，通过将视频映射到稳定的“可供性令牌”基底，并使用轻量级、按查询路由的调度器进行有针对性的适应，实现了卓越的性能和低遗忘率。


<details>
  <summary>Details</summary>
Motivation: 现有视频-语言理解的持续学习解决方案在面对非平稳数据、领域和查询风格时，存在稳定性与适应性界限模糊、依赖静态路由/容量或需要回放旧视频等问题，限制了其在实际内存和隐私受限场景下的应用。

Method: AFD方法将视频映射到缓慢变化的“可供性令牌”，形成一个共享的、时间对齐的稳定基底。同时，一个轻量级、由查询路由的、冲突感知调度器负责集中适应性，并在需要时才扩展容量。基底通过弱对齐和教师一致性进行稳定化，训练时仅使用问题回放。

Result: AFD在多项协议上取得了最先进的性能：在领域增量VideoQA上平均准确率达51.6%且遗忘率为-1.8%；在ViLCo R@1@0.5上，MQ为29.6%，NLQ为20.7%，VQ的stAP@0.25为18.4%；在时间增量iVQA上准确率达39.5%且遗忘率为-1.6%。

Conclusion: AFD提供了一种明确且可解释的分解，将稳定的以交互为中心的基底与有针对性的适应性分离开来，有效解决了持续学习中的稳定性和可塑性问题。
...
### [151] [TrajDiff: End-to-end Autonomous Driving without Perception Annotation](https://arxiv.org/abs/2512.00723)
*Xingtai Gui,Jianbo Zhao,Wencheng Han,Jikai Wang,Jiahao Gong,Feiyang Tan,Cheng-zhong Xu,Jianbing Shen*

Main category: cs.CV

TL;DR: TrajDiff是一种面向轨迹的BEV条件扩散框架，为端到端自动驾驶提供了一种完全无需感知标注的生成方法，直接从原始传感器输入生成多样且合理的轨迹，并在NAVSIM基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 由于手动感知标注成本高昂，开发无需感知标注的规划范式对于端到端自动驾驶系统变得越来越重要。

Method: 本文提出了TrajDiff，一个轨迹导向的BEV条件扩散框架。它仅需要原始传感器输入和未来轨迹，构建高斯BEV热图目标来捕捉驾驶模式。该方法设计了一个简单的轨迹导向BEV编码器，在没有感知监督的情况下提取TrajBEV特征。此外，引入了轨迹导向BEV扩散Transformer（TB-DiT），利用自我状态信息和预测的TrajBEV特征直接生成多样且合理的轨迹，无需手工运动先验。TrajDiff还探索了在无标注设置下数据扩展的益处。

Result: 在NAVSIM基准上，TrajDiff实现了87.5的PDMS，在所有无标注方法中达到了最先进的性能。通过数据扩展，性能进一步提升至88.5的PDMS，与先进的基于感知的方法相当。

Conclusion: TrajDiff成功建立了一个无需感知标注的端到端自动驾驶生成方法，通过创新的架构和对数据扩展的探索，在自动驾驶领域取得了显著的性能提升，并有望推动未来自动驾驶技术的发展。
...
### [152] [Generative Adversarial Gumbel MCTS for Abstract Visual Composition Generation](https://arxiv.org/abs/2512.01242)
*Zirui Zhao,Boye Niu,David Hsu,Wee Sun Lee*

Main category: cs.CV

TL;DR: 本文提出了一种约束引导框架，结合几何推理和神经语义，通过类AlphaGo搜索和对抗性奖励优化，解决了抽象视觉构图的挑战，在拼图组装任务中表现出更高的有效性和语义保真度。


<details>
  <summary>Details</summary>
Motivation: 抽象视觉构图（基于几何原语的空间配置和关系）面临组合选择多、数据有限和离散可行性（无重叠、允许的方向）等挑战。纯粹的统计像素空间生成器难以应对，因此需要一种新的方法。

Method: 提出了一种约束引导框架，融合了显式几何推理和神经语义。采用类AlphaGo的蒙特卡洛树搜索（MCTS）来强制执行可行性，并使用微调的视觉-语言模型作为奖励信号来评估语义对齐。算法利用策略网络作为MCTS的启发式方法，并通过搜索生成的计划微调网络。受GAN启发，使用生成的实例进行对抗性奖励细化。

Result: 在拼图组装任务中，该方法比扩散模型和自回归基线产生了更高的有效性和语义保真度，尤其是在约束条件更严格的情况下。

Conclusion: 该约束引导框架通过结合几何推理、神经语义和搜索技术，成功解决了抽象视觉构图的复杂性，并显著提升了生成结果的质量和准确性。
...
### [153] [Probabilistic Modeling of Multi-rater Medical Image Segmentation for Diversity and Personalization](https://arxiv.org/abs/2512.00748)
*Ke Liu,Shangde Gao,Yichao Fu,Shangqi Gao,Chunhua Shen*

Main category: cs.CV

TL;DR: 本文提出ProSeg模型，通过概率建模专家偏好和图像边界模糊性，在多标注者医学图像分割任务中同时实现多样化和个性化分割，解决了现有模型在这两方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割受数据不确定性（模糊边界、观察者间差异）影响。现有多标注者分割模型要么生成缺乏专家特异性的多样化结果，要么仅复制个体标注者的输出，无法同时实现多样化和个性化。

Method: 提出了多标注者医学图像分割的概率建模（ProSeg）。引入两个潜在变量来分别建模专家标注偏好和图像边界模糊性。通过变分推断获得这些潜在变量的条件概率分布，然后通过从这些分布中采样来生成分割输出。

Result: 在鼻咽癌（NPC）数据集和肺结节（LIDC-IDRI）数据集上进行了广泛实验，结果表明ProSeg达到了新的最先进性能，提供了既多样化又专家个性化的分割结果。

Conclusion: ProSeg通过对专家标注偏好和图像边界模糊性进行概率建模，成功解决了医学图像分割中的不确定性问题，实现了分割结果的多样化和专家个性化。
...
### [154] [RS-ISRefiner: Towards Better Adapting Vision Foundation Models for Interactive Segmentation of Remote Sensing Images](https://arxiv.org/abs/2512.00718)
*Deliang Wang,Peng Liu*

Main category: cs.CV

TL;DR: 本文提出RS-ISRefiner，一个专为遥感图像设计的交互式图像分割框架，通过适配器微调、混合注意力机制和改进的概率图调制方案，显著提高了遥感图像分割的准确性、效率和交互成本。


<details>
  <summary>Details</summary>
Motivation: 现有交互式图像分割(IIS)方法主要针对自然图像设计，难以泛化到遥感领域，原因包括遥感图像中物体尺度变化大、边界不规则、背景复杂，以及标注数据有限和计算开销大。

Method: RS-ISRefiner框架采用以下方法：1) 基于适配器的微调策略，在保留视觉基础模型通用表示的同时，高效学习遥感特有的空间和边界特征；2) 集成卷积局部建模和Transformer全局推理的混合注意力机制，增强对尺度多样性和场景复杂性的鲁棒性；3) 改进的概率图调制方案，有效融合历史用户交互，实现更稳定的迭代细化和更高的边界保真度。

Result: 在iSAID、ISPRS Potsdam、SandBar、NWPU、LoveDA Urban和WHUBuilding六个遥感数据集上的综合实验表明，RS-ISRefiner在分割精度、效率和交互成本方面始终优于现有最先进的IIS方法。

Conclusion: RS-ISRefiner框架的有效性和通用性得到了证实，使其非常适用于实际遥感场景中的高质量实例分割任务。
...
### [155] [SwiftVLA: Unlocking Spatiotemporal Dynamics for Lightweight VLA Models at Minimal Overhead](https://arxiv.org/abs/2512.00903)
...
TL;DR: 本文提出了SwiftVLA，一个轻量级的视觉-语言-动作(VLA)模型，通过集成4D理解能力，在保持高效设计的同时，实现了与大型VLA模型相当的性能，显著提升了推理速度并降低了内存占用。


<details>
  <summary>Details</summary>
Motivation: 现有的基于预训练VLM构建的VLA模型参数量大，实用性受限。轻量级VLM虽然能缓解此问题，但会牺牲时空推理能力。引入额外3D输入的方法通常依赖大型VLM融合2D和3D数据，且缺乏时间理解，无法有效解决轻量化和4D理解的冲突。

Method: SwiftVLA包含以下关键组件：1. 一个预训练的4D视觉几何Transformer，带有时间缓存，能从2D图像中提取4D特征。2. 引入融合令牌（Fusion Tokens），通过未来预测目标进行训练，以生成统一的表示用于动作生成，从而增强VLM利用2D图像和4D特征的能力。3. 提出掩码-重建策略，掩盖VLM的4D输入并训练VLA模型进行重建，使VLM学习有效的4D表示，并允许在推理时移除4D分支，性能损失最小。

Result: 实验结果表明，SwiftVLA在真实和模拟环境中均优于轻量级基线模型，并能与参数量大7倍的VLA模型媲美。它在边缘设备上实现了可比的性能，同时推理速度快18倍，内存占用减少12倍。

Conclusion: SwiftVLA成功地将4D理解能力集成到紧凑型VLA模型中，同时保持了高效的设计。它在性能上与大型模型相当，但在计算资源和速度方面具有显著优势，为VLA模型在实际应用中的部署提供了高效且高性能的解决方案。
...
### [156] [Generalized Medical Phrase Grounding](https://arxiv.org/abs/2512.01085)
*Wenjun Zhang,Shekhar S. Chandra,Aaron Nicolson*

Main category: cs.CV

TL;DR: 本文提出广义医学短语定位（GMPG）任务及其模型MedGrounder，以解决现有医学短语定位（MPG）系统无法处理多区域、非诊断性及不可定位短语的问题。MedGrounder采用两阶段训练，在多区域和不可定位短语方面优于现有方法，并能实现零样本迁移，同时显著减少人工标注需求。


<details>
  <summary>Details</summary>
Motivation: 现有医学短语定位（MPG）系统大多遵循指代表达理解（REC）范式，假设每个短语对应一个边界框。然而，真实报告常包含多区域发现、非诊断性文本和不可定位短语（如否定或正常解剖描述），这违反了上述假设，导致现有系统难以准确解释。

Method: 本文将任务重新定义为广义医学短语定位（GMPG），允许每个句子映射到零个、一个或多个得分区域。为此，引入了首个GMPG模型MedGrounder。该模型采用两阶段训练方案：首先在报告句子-解剖框对齐数据集上进行预训练，然后利用报告句子-人工标注框数据集进行微调。

Result: MedGrounder在PadChest-GR和MS-CXR数据集上表现出强大的零样本迁移能力。在处理多区域和不可定位短语时，其性能优于REC风格和基于定位报告生成的基线模型，且所需人工框标注数量显著减少。此外，MedGrounder可与现有报告生成器结合使用，无需重新训练即可生成带有定位信息的报告。

Conclusion: MedGrounder成功解决了传统医学短语定位在处理复杂真实世界医学报告（如多区域和不可定位短语）时的局限性。它以更少的人工标注实现了卓越性能，并能与现有系统无缝集成，从而生成更易于理解的定位报告。
...
### [157] [MM-ACT: Learn from Multimodal Parallel Generation to Act](https://arxiv.org/abs/2512.00975)
...
TL;DR: MM-ACT是一个统一的视觉-语言-动作（VLA）模型，通过共享的token空间整合文本、图像和动作，并采用并行解码策略和上下文共享多模态学习，显著提升了通用机器人策略在模拟和真实环境中的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略需要语义理解进行任务规划，并具备通过预测能力与环境交互的能力，现有方法难以同时满足这两点。

Method: 本文提出了MM-ACT，一个统一的视觉-语言-动作（VLA）模型：
1. 将文本、图像和动作整合到共享的token空间中，并在这三种模态之间进行生成。
2. 采用文本和图像生成的re-mask并行解码策略，以及动作生成的一步并行解码策略以提高效率。
3. 引入“上下文共享多模态学习”范式，从共享上下文监督所有三种模态的生成，通过跨模态学习增强动作生成。

Result: MM-ACT在以下方面取得了优异表现：
1. LIBERO模拟环境中成功率为96.3%。
2. 真实Franka机器人上的三项任务成功率为72.0%。
3. RoboTwin2.0上的八项双手任务成功率为52.38%。
4. 跨模态学习额外带来了9.25%的性能提升。

Conclusion: MM-ACT成功地将视觉、语言和动作统一起来，通过高效的解码策略和上下文共享多模态学习，在模拟和真实机器人任务中展现出强大的泛化能力和高成功率，尤其通过跨模态学习显著增强了动作生成性能。
...
TL;DR: 本文提出了一种名为CAR-net的新型神经网络架构，用于处理旋转运动模糊图像的去模糊问题，尤其是在仅有噪声模糊角度信息的半盲场景下，通过渐进式细化和可选的角度检测模块实现。


<details>
  <summary>Details</summary>
Motivation: 研究动机是去模糊受旋转运动模糊影响的图像，特别是在模糊角度信息不完整或带有噪声的半盲情境下。

Method: 该方法的核心是CAR-net（CAscade Refinement Network）架构。它采用渐进式细化过程：首先通过频域反演获得初始去模糊估计，然后一系列细化阶段预测并应用残差校正，逐步抑制伪影并恢复细节。为了处理参数不确定性，架构还包含一个可选的角度检测模块，可以与细化模块进行端到端训练。

Result: 通过对合成图像和真实图像的实验，证明了CAR-net架构的有效性。

Conclusion: CAR-net是一种高效的神经网络架构，能够有效去模糊受旋转运动模糊影响的图像，尤其适用于半盲场景，其渐进式细化和可选的角度检测模块是关键组成部分。
...
### [159] [Look, Recite, Then Answer: Enhancing VLM Performance via Self-Generated Knowledge Hints](https://arxiv.org/abs/2512.00882)
*Xisheng Feng*

Main category: cs.CV

TL;DR: 本文提出“看、背、答”（Look, Recite, Then Answer）框架，通过自生成知识提示增强视觉语言模型（VLMs）在专业领域的表现，有效缓解了推理驱动的幻觉和模态鸿沟问题，在农学基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在精准农业等专业领域表现不佳，主要原因在于“推理驱动的幻觉”（语言先验覆盖视觉感知）和“模态鸿沟”（视觉嵌入未能有效激活模型参数中编码的细粒度专家知识）。

Method: 提出的“看、背、答”是一个参数高效的框架，在冻结主干模型的情况下，通过自生成知识提示增强VLMs。推理分为三个阶段：1) 看（Look）生成客观视觉描述和候选集；2) 背（Recite）使用一个轻量级1.7B路由器将视觉线索转化为目标查询，触发特定候选的参数知识；3) 答（Answer）对描述和背诵的知识进行并行证据对齐，选择最一致的标签。

Result: 在AgroBench数据集上，该方法取得了最先进的成果，杂草识别准确率比Qwen-VL提高了23.6%，并且在没有外部搜索开销的情况下超越了GPT-4o。

Conclusion: 该模块化设计通过将被动感知转化为主动、可控的知识检索，有效缓解了幻觉问题。
...
### [160] [Multi-GRPO: Multi-Group Advantage Estimation for Text-to-Image Generation with Tree-Based Trajectories and Multiple Rewards](https://arxiv.org/abs/2512.00743)
...
TL;DR: Multi-GRPO通过引入基于树的轨迹和基于奖励的分组机制，解决了现有GRPO方法在文本到图像模型对齐中面临的信用分配不准确和多目标奖励混合不稳定问题，实现了卓越的对齐性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的GRPO（Group Relative Policy Optimization）方法在文本到图像（T2I）模型对齐中存在两个主要限制：1) 共享信用分配：轨迹级别的优势被统一应用于所有时间步，未能准确评估早期去噪步骤的潜力。2) 奖励混合：多目标奖励（如文本准确性、视觉质量、文本颜色）因尺度和方差不匹配，导致梯度不稳定和更新冲突。

Method: 本文提出了Multi-GRPO，一个多组优势估计框架，包含两种正交的分组机制。为改善信用分配，引入了受蒙特卡洛树搜索启发的基于树的轨迹，通过在早期去噪步骤分支形成“时间组”，利用后代叶子准确估计早期步骤的优势。为解决多目标优化问题，引入了“基于奖励的分组”，独立计算每个奖励函数的优势再进行聚合，从而解耦冲突信号。此外，还策划了一个名为OCR-Color-10的视觉文本渲染数据集，用于评估多目标对齐。

Result: Multi-GRPO在单奖励的PickScore-25k和多目标OCR-Color-10基准测试中均展现出卓越的稳定性和对齐性能，有效平衡了冲突的目标。

Conclusion: Multi-GRPO通过创新的多组优势估计框架，成功克服了现有GRPO方法的局限性，显著提升了T2I模型的对齐效果和多目标优化的稳定性。
...
### [161] [TAP-CT: 3D Task-Agnostic Pretraining of Computed Tomography Foundation Models](https://arxiv.org/abs/2512.00872)
*Tim Veenboer,George Yiasemis,Eric Marcus,Vivien Van Veldhuizen,Cees G. M. Snoek,Jonas Teuwen,Kevin B. W. Groot Lipman*

Main category: cs.CV

TL;DR: 本文提出TAP-CT，一种针对3D CT数据的任务无关预训练方法，通过改进的ViT和DINOv2实现可扩展的自监督学习，生成稳定的、可泛化的冻结表示，且只需极少微调。


<details>
  <summary>Details</summary>
Motivation: 现有的医学基础模型通常需要大量微调或依赖资源密集型解码器，且现有编码器常偏向特定任务。因此，需要一个强大的、任务无关的基础模型，在特征提取后只需极少微调。

Method: 引入了TAP-CT套件，将Vision Transformers (ViTs)和DINOv2有效适配于体数据，直接在3D CT体数据上进行可扩展的自监督预训练。方法中包含针对性的修改，如补丁嵌入、位置编码和体数据增强，使架构具有深度感知能力，同时保持了底层架构的简洁性。在大规模内部CT数据集（105K体数据）上进行了3D预训练。

Result: 大规模3D预训练产生了稳定、鲁棒的冻结表示，这些表示在各种下游任务中表现出强大的泛化能力。

Conclusion: TAP-CT为未来的医学影像研究提供了一个强大、低资源的基线，并通过发布所有预训练模型、实验配置和下游基准代码，促进了透明度和可复现性。
...
### [162] [EAG3R: Event-Augmented 3D Geometry Estimation for Dynamic and Extreme-Lighting Scenes](https://arxiv.org/abs/2512.00771)
*Xiaoshan Wu,Yifei Yu,Xiaoyang Lyu,Yihua Huang,Bo Wang,Baoheng Zhang,Zhongrui Wang,Xiaojuan Qi*

Main category: cs.CV

TL;DR: EAG3R是一种新的几何估计框架，通过整合异步事件流，增强了基于点图的重建，实现了在动态低光照场景下鲁棒的3D几何估计。


<details>
  <summary>Details</summary>
Motivation: 现有的纯RGB方法在处理真实世界中包含动态物体和极端照明的场景时，由于传统相机的局限性，难以进行鲁D的3D几何估计。

Method: EAG3R基于MonST3R骨干网络，引入了两项创新：1) 一个受Retinex启发的图像增强模块和一个轻量级事件适配器，该适配器具有SNR感知的融合机制，可根据局部可靠性自适应地结合RGB和事件特征；2) 一个新颖的基于事件的光度一致性损失，用于在全局优化期间增强时空连贯性。

Result: EAG3R在单目深度估计、相机姿态跟踪和动态重建任务中，显著优于最先进的纯RGB基线方法，尤其是在具有挑战性的动态低光照场景中，且无需夜间数据重新训练。

Conclusion: EAG3R通过有效整合事件流，解决了传统RGB方法在复杂动态和低光照环境下的局限性，实现了更鲁棒的3D几何估计。
...
TL;DR: 引入StreamGaze，首个评估多模态大语言模型（MLLMs）在流媒体视频中利用注视信号进行时间推理和主动预测能力的基准。研究发现当前MLLMs与人类表现存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有流媒体视频理解基准仅评估时间推理，但未衡量多模态大语言模型（MLLMs）如何解释或利用人类注视信号来预测用户意图，这对于AR眼镜等实际应用至关重要。

Method: 提出StreamGaze基准，包含注视引导的过去、现在和主动任务。开发了一个注视-视频问答生成管道，通过注视点提取、区域特定视觉提示和扫描路径构建，将第一人称视角视频与原始注视轨迹对齐，生成反映人类感知动态的时空关联问答对。

Result: 最先进的MLLMs在所有StreamGaze任务上与人类表现存在巨大差距，揭示了它们在基于注视的时间推理、意图建模和主动预测方面的根本局限性。研究还提供了注视提示策略、推理行为和任务特定失败模式的详细分析。

Conclusion: 当前MLLMs在流媒体视频中利用人类注视信号进行时间推理、意图建模和主动预测方面存在显著不足。StreamGaze基准揭示了这些局限性，并为未来模型在此领域的能力发展指明了方向。
...
### [164] [Real-Time On-the-Go Annotation Framework Using YOLO for Automated Dataset Generation](https://arxiv.org/abs/2512.01165)
...
### [165] [SPARK: Sim-ready Part-level Articulated Reconstruction with VLM Knowledge](https://arxiv.org/abs/2512.01629)
...
### [166] [ForamDeepSlice: A High-Accuracy Deep Learning Framework for Foraminifera Species Classification from 2D Micro-CT Slices](https://arxiv.org/abs/2512.00912)
...
### [167] [Joint Multi-scale Gated Transformer and Prior-guided Convolutional Network for Learned Image Compression](https://arxiv.org/abs/2512.00744)
...
### [169] [Seeing the Wind from a Falling Leaf](https://arxiv.org/abs/2512.00762)
...
### [170] [Parameter Reduction Improves Vision Transformers: A Comparative Study of Sharing and Width Reduction](https://arxiv.org/abs/2512.01059)
...
### [171] [GrndCtrl: Grounding World Models via Self-Supervised Reward Alignment](https://arxiv.org/abs/2512.01952)
...
### [172] [Visual Sync: Multi-Camera Synchronization via Cross-View Object Motion](https://arxiv.org/abs/2512.02017)
...
### [173] [Provenance-Driven Reliable Semantic Medical Image Vector Reconstruction via Lightweight Blockchain-Verified Latent Fingerprints](https://arxiv.org/abs/2512.00999)
...
### [174] [Charts Are Not Images: On the Challenges of Scientific Chart Editing](https://arxiv.org/abs/2512.00752)
...
### [175] [Data-Centric Visual Development for Self-Driving Labs](https://arxiv.org/abs/2512.02018)
...
### [176] [The Outline of Deception: Physical Adversarial Attacks on Traffic Signs Using Edge Patches](https://arxiv.org/abs/2512.00765)
...
### [177] [CycliST: A Video Language Model Benchmark for Reasoning on Cyclical State Transitions](https://arxiv.org/abs/2512.01095)
...
### [178] [DEJIMA: A Novel Large-scale Japanese Dataset for Image Captioning and Visual Question Answering](https://arxiv.org/abs/2512.00773)
...
### [179] [SocialFusion: Addressing Social Degradation in Pre-trained Vision-Language Models](https://arxiv.org/abs/2512.01148)
...
### [180] [PolarGS: Polarimetric Cues for Ambiguity-Free Gaussian Splatting with Accurate Geometry Recovery](https://arxiv.org/abs/2512.00794)
...
### [181] [M4-BLIP: Advancing Multi-Modal Media Manipulation Detection through Face-Enhanced Local Analysis](https://arxiv.org/abs/2512.01214)
...
### [182] [DPAC: Distribution-Preserving Adversarial Control for Diffusion Sampling](https://arxiv.org/abs/2512.01153)
...
### [183] [Thinking with Drafts: Speculative Temporal Reasoning for Efficient Long Video Understanding](https://arxiv.org/abs/2512.00805)
...
### [184] [CircleFlow: Flow-Guided Camera Blur Estimation using a Circle Grid Target](https://arxiv.org/abs/2512.00796)
...
### [185] [S$^2$-MLLM: Boosting Spatial Reasoning Capability of MLLMs for 3D Visual Grounding with Structural Guidance](https://arxiv.org/abs/2512.01223)
...
### [186] [IRPO: Boosting Image Restoration via Post-training GRPO](https://arxiv.org/abs/2512.00814)
...
### [187] [Diffusion Model in Latent Space for Medical Image Segmentation Task](https://arxiv.org/abs/2512.01292)
...
### [188] [PanFlow: Decoupled Motion Control for Panoramic Video Generation](https://arxiv.org/abs/2512.00832)
...
### [189] [FOD-S2R: A FOD Dataset for Sim2Real Transfer Learning based Object Detection](https://arxiv.org/abs/2512.01315)
...
### [190] [Rice-VL: Evaluating Vision-Language Models for Cultural Understanding Across ASEAN Countries](https://arxiv.org/abs/2512.01419)
...
### [191] [AFRAgent : An Adaptive Feature Renormalization Based High Resolution Aware GUI agent](https://arxiv.org/abs/2512.00846)
...
### [192] [Rethinking Intracranial Aneurysm Vessel Segmentation: A Perspective from Computational Fluid Dynamics Applications](https://arxiv.org/abs/2512.01319)
...
### [193] [Smol-GS: Compact Representations for Abstract 3D Gaussian Splatting](https://arxiv.org/abs/2512.00850)
...
### [194] [Neural Discrete Representation Learning for Sparse-View CBCT Reconstruction: From Algorithm Design to Prospective Multicenter Clinical Evaluation](https://arxiv.org/abs/2512.00873)
...
### [195] [Diffusion Fuzzy System: Fuzzy Rule Guided Latent Multi-Path Diffusion Modeling](https://arxiv.org/abs/2512.01533)
...
### [196] [Feed-Forward 3D Gaussian Splatting Compression with Long-Context Modeling](https://arxiv.org/abs/2512.00877)
...
### [197] [Quantum-Inspired Spectral Geometry for Neural Operator Equivalence and Structured Pruning](https://arxiv.org/abs/2512.00880)
...
### [198] [Deep Unsupervised Anomaly Detection in Brain Imaging: Large-Scale Benchmarking and Bias Analysis](https://arxiv.org/abs/2512.01534)
...
### [199] [TalkingPose: Efficient Face and Gesture Animation with Feedback-guided Diffusion Model](https://arxiv.org/abs/2512.00909)
...
TL;DR: 本文提出TalkingPose框架，利用扩散模型和反馈机制，解决了现有方法在生成长时间、时间一致的人物上半身动画方面的挑战，并引入了一个新的大规模数据集。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在角色动画中取得了显著进展，但由于计算和内存限制，现有方法通常在短视频片段上训练，难以生成时间一致的长时间内容。

Method: TalkingPose是一个基于扩散模型的新框架，专门用于生成长时间、时间一致的人类上半身动画。它利用驱动帧捕捉面部和手部表情，通过稳定的扩散骨干网络将其无缝传输到目标角色。为确保连续运动和时间一致性，引入了一种基于图像扩散模型的反馈驱动机制，该机制无需额外计算成本或二次训练，即可生成无限时长的动画。此外，还引入了一个大规模数据集作为新的基准。

Result: TalkingPose能够生成无限时长的动画，并确保时间一致性。同时，引入了一个全面的、大规模数据集，可作为人类上半身动画的新基准。

Conclusion: TalkingPose通过其创新的扩散框架和高效的反馈机制，成功解决了长时间、时间一致的人物上半身动画生成问题，并为该领域提供了新的基准数据集。
...
### [200] [Hierarchical Semantic Alignment for Image Clustering](https://arxiv.org/abs/2512.00904)
*Xingyu Zhu,Beier Zhu,Yunfan Li,Junfeng Fang,Shuo Wang,Kesen Zhao,Hanwang Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种名为CAE的无训练分层语义对齐方法，通过结合描述级和名词级文本语义，解决了图像聚类中名词固有的歧义问题，显著提升了聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有的图像聚类方法在利用名词作为外部语义知识时，常忽略名词固有的歧义性，这会导致语义表示失真并降低聚类质量。

Method: 该方法整合了两种互补的文本语义：传达图像内容细粒度属性的描述级语义和表示高级对象类别的名词级概念。首先，从WordNet选择相关名词并从图像描述数据集中选择描述，构建与图像特征对齐的语义空间。然后，通过最优传输将图像特征与选定的名词和描述对齐，以获得更具判别性的语义空间。最后，结合增强的语义和图像特征进行聚类，且该方法是无训练的。

Result: 在8个数据集上的广泛实验表明，该方法有效性显著，尤其在ImageNet-1K数据集上，其准确率比最先进的无训练方法提高了4.2%，调整兰德指数（ARI）提高了2.9%。

Conclusion: 所提出的CAE方法通过分层语义对齐，有效解决了名词歧义问题，并在无训练的情况下显著提高了图像聚类的性能。
...
### [201] [LAHNet: Local Attentive Hashing Network for Point Cloud Registration](https://arxiv.org/abs/2512.00927)
*Wentao Qu,Xiaoshui Huang,Liang Xiao*

Main category: cs.CV

TL;DR: 本文提出LAHNet，一种用于点云配准的局部注意力哈希网络，通过引入局部注意力机制、分组Transformer和交互Transformer，有效扩大感受野，增强特征区分度，实现鲁棒的配准性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的点云描述符主要关注局部信息，但缺乏合理且更广阔的感受野，这对于增强特征区分度至关重要。

Method: 本文提出LAHNet（局部注意力哈希网络），它将带有卷积类算子局部性归纳偏置的局部注意力机制引入点云描述符。具体方法包括：1) 设计一个分组Transformer，利用局部敏感哈希（Locality-Sensitive Hashing）实现均匀的点云窗口划分，以捕获合理的远距离上下文；2) 采用高效的跨窗口策略，进一步扩大特征感受野；3) 在此窗口策略基础上，提出一个交互Transformer，通过计算重叠矩阵并以全局信号表示每个窗口，增强点云对重叠区域的特征交互。

Result: LAHNet能够学习到鲁棒且具有区分度的特征，在真实世界的室内外基准测试中取得了显著的点云配准结果。

Conclusion: LAHNet通过创新的局部注意力机制、分组Transformer和交互Transformer，有效解决了现有方法感受野不足的问题，显著提升了点云配准的性能和特征的鲁棒性与区分度。
...
### [202] [Dual-Projection Fusion for Accurate Upright Panorama Generation in Robotic Vision](https://arxiv.org/abs/2512.00911)
*Yuhao Shan,Qianyi Yuan,Jingguo Liu,Shigang Li,Jianfeng Li,Tong Chen*

Main category: cs.CV

TL;DR: 该研究提出了一种双流角度感知生成网络，用于同时估计全景相机倾斜角并重建正立全景图像，以解决机器人视觉中非正立全景图的问题。


<details>
  <summary>Details</summary>
Motivation: 全景相机在机器人视觉中至关重要，但在机器人姿态不稳定时捕获的非正立全景图会阻碍后续任务。传统的惯性测量单元（IMU）校正方法存在漂移和外部干扰问题，而基于视觉的方法具有潜力。

Method: 本文提出一个双流角度感知生成网络，包含一个用于提取等距柱状投影局部几何结构的CNN分支和一个用于捕获立方体投影全局上下文线索的ViT分支。通过双投影自适应融合模块整合两者，实现跨域空间特征对齐。此外，引入了高频增强模块、循环填充和通道注意力机制，以保持360度连续性并提高几何敏感性。

Result: 在SUN360和M3D数据集上的实验表明，该方法在倾斜角估计和正立全景图生成方面均优于现有方法。消融研究进一步验证了每个模块的贡献，并强调了两个任务之间的协同作用。

Conclusion: 该研究提出的双流角度感知生成网络能有效解决非正立全景图问题，其结合CNN和ViT的双投影融合架构，辅以高频增强和注意力机制，在全景图像扶正任务中表现出色，且各模块间具有良好的协同效应。
...
### [203] [MasHeNe: A Benchmark for Head and Neck CT Mass Segmentation using Window-Enhanced Mamba with Frequency-Domain Integration](https://arxiv.org/abs/2512.01563)
...
TL;DR: 该研究引入了MasHeNe数据集，包含3,779张头颈部肿块（肿瘤和囊肿）的CT图像及像素级标注，并提出了WEMF模型，在分割任务上表现优异，为头颈部肿块分割提供了新的基准。


<details>
  <summary>Details</summary>
Motivation: 现有的公开数据集主要关注恶性病变，忽视了头颈部区域的其他占位性病变（如囊肿），导致研究空白。为了弥补这一不足，需要一个更全面的数据集。

Method: 研究构建了MasHeNe数据集，包含3,779张对比增强CT切片，对肿瘤和囊肿进行了像素级标注。在此基础上，建立了基准测试，并提出了一种名为WEMF（Windowing-Enhanced Mamba with Frequency integration）的模型。WEMF模型通过三窗口增强丰富输入外观，并使用多频注意力机制在U形Mamba骨干网络中融合跳跃连接的信息。

Result: 在MasHeNe数据集上，WEMF模型在所有评估方法中表现最佳，Dice系数达到70.45%，IoU为66.89%，NSD为72.33%，HD95为5.12毫米。该模型在此项具有挑战性的任务中展现出稳定且强大的结果。

Conclusion: MasHeNe数据集为超越仅限于恶性病变的头颈部肿块分割任务提供了一个新的基准。WEMF模型在数据集上表现出色，但观察到的错误模式也表明，这项任务仍然具有挑战性，需要进一步的研究。
...
### [204] [Accelerating Streaming Video Large Language Models via Hierarchical Token Compression](https://arxiv.org/abs/2512.00891)
*Yiyu Wang,Xuyang Liu,Xiyan Gui,Xinying Lin,Boxue Yang,Chenfei Liao,Tailai Chen,Linfeng Zhang*

Main category: cs.CV

TL;DR: 本文提出了流式令牌压缩（STC）框架，通过优化Vision Transformer编码和LLM预填充阶段，显著加速了流式视频大语言模型（VideoLLMs）的处理速度，同时保持了高准确性。


<details>
  <summary>Details</summary>
Motivation: 流式视频大语言模型在实时部署中面临巨大挑战，主要由于处理连续视频流中密集的视觉令牌导致高计算成本。具体瓶颈在于Vision Transformer（ViT）编码阶段对时间上相似帧的冗余处理，以及LLM预填充阶段令牌序列膨胀带来的延迟和内存开销。

Method: 本文提出了一种可插拔的分层框架——流式令牌压缩（STC），旨在优化ViT编码和LLM预填充阶段。STC包含两个令牌级加速器：1) STC-Cacher，通过缓存和重用时间上相似帧的特征来减少ViT编码开销；2) STC-Pruner，在视觉令牌进入LLM之前，基于空间和时间相关性压缩令牌序列，只保留最显著的令牌。

Result: 在四个基线流式VideoLLMs和五个基准测试上的广泛实验表明，STC优于其他压缩方法。STC在ReKV框架上保持了高达99%的准确性，同时将ViT编码延迟降低了24.5%，将LLM预填充延迟降低了45.3%。

Conclusion: STC框架通过有效地压缩流式VideoLLMs中的视觉令牌，显著解决了实时部署中的计算效率问题，实现了显著的延迟降低，且对模型准确性影响极小。
...
### [205] [HanDyVQA: A Video QA Benchmark for Fine-Grained Hand-Object Interaction Dynamics](https://arxiv.org/abs/2512.00885)
...
### [206] [RoleMotion: A Large-Scale Dataset towards Robust Scene-Specific Role-Playing Motion Synthesis with Fine-grained Descriptions](https://arxiv.org/abs/2512.01582)
...
TL;DR: 本文介绍了RoleMotion，一个大规模、高质量的人体运动数据集，包含丰富的角色扮演和功能性运动数据，专为特定场景设计，并带有细粒度文本描述。


<details>
  <summary>Details</summary>
Motivation: 现有的文本运动数据集存在去中心化构建、数据非功能性且孤立、运动数据质量不一致以及文本标注缺乏细粒度细节等问题，无法有效覆盖各种场景下的社交活动。因此，需要一个专注于场景和角色、高质量且功能性的数据集。

Method: RoleMotion数据集是精心设计和收集的，特别关注场景和角色。它包含25个经典场景、110个功能性角色、500多种行为，以及10296个高质量的人体（身体和手部）运动序列，并标注了27831条细粒度文本描述。此外，论文还构建了一个比现有更强的评估器，并用其评估了多种文本到运动的方法，并探讨了身体和手部运动生成之间的相互作用。

Result: RoleMotion数据集在文本驱动的全身运动生成方面表现出高质量和功能性。实验结果证明了数据集的有效性，并且所构建的评估器也展现了其可靠性。研究还探索了身体和手部运动生成之间的相互作用。

Conclusion: RoleMotion是一个高质量、功能性强的人体运动数据集，它通过专注于场景和角色，解决了现有数据集的局限性。它为文本驱动的全身运动生成提供了强大的支持，并为未来的研究奠定了基础。
...
### [207] [Multilingual Training-Free Remote Sensing Image Captioning](https://arxiv.org/abs/2512.00887)
*Carlos Rebelo,Gil Rocha,João Daniel Silva,Bruno Martins*

Main category: cs.CV

TL;DR: 本文提出了一种无需训练的多语言遥感图像字幕生成方法，通过检索增强提示，在多个基准数据集和十种语言上取得了与全监督英语系统相当的性能，并强调了图基重排序的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有遥感图像字幕生成方法过度依赖大型标注数据集且主要限于英语，限制了其全球适用性。

Method: 该方法是基于检索增强提示的，无需训练。它使用领域适应的SigLIP2编码器从数据存储中检索相关字幕和少样本示例，然后将其提供给语言模型。探索了两种变体：图像盲设置（多语言LLM）和图像感知设置（VLM）。为提高检索内容的连贯性，引入了一种基于图的重排序策略（使用PageRank）。

Result: 实验结果表明，该方法与全监督英语系统具有竞争力，并能很好地推广到其他语言。PageRank重排序对性能提升显著（最高35%）。VLM倾向于生成视觉上更准确但词汇多样性更高的字幕，而LLM在BLEU和CIDEr分数上表现更强。直接用目标语言生成字幕始终优于基于翻译的策略。

Conclusion: 这项工作首次系统地评估了遥感图像的多语言、免训练字幕生成方法，为更具包容性和可扩展性的多模态地球观测系统奠定了基础。
...
### [208] [LISA-3D: Lifting Language-Image Segmentation to 3D via Multi-View Consistency](https://arxiv.org/abs/2512.01008)
*Zhongbin Guo,Jiahe Liu,Wenyu Gao,Yushan Li,Chengzhi Li,Ping Jian*

Main category: cs.CV

TL;DR: LISA-3D是一个两阶段框架，通过将LISA模型与几何感知的LoRA层结合，并利用可微分的重投影损失实现跨视图一致性，从而将语言-图像分割提升到3D，用于文本驱动的3D重建。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的3D重建需要一个能够理解开放词汇指令并在不同视角下保持一致性的掩码生成器。

Method: LISA-3D是一个两阶段框架：1. 通过几何感知的低秩适应（LoRA）层改造指令遵循模型LISA，并利用现成的RGB-D序列和相机姿态构建可微分的重投影损失，以强制实现跨视图一致性，无需额外的3D文本监督。2. 将生成的掩码与RGB图像连接形成RGBA提示，输入到冻结的SAM-3D重建器中，输出高斯splat或纹理网格。

Result: 在ScanRefer和Nr3D数据集上，LISA-3D将语言到3D的准确性比单视图基线提高了高达+15.6点，同时仅适应了11.6M参数。该系统模块化、数据高效，并支持对未见类别的零样本部署。

Conclusion: LISA-3D提供了一个实用的、模块化的、数据高效的解决方案，用于语言引导的3D内容创建，并支持零样本部署。
...
### [209] [Binary-Gaussian: Compact and Progressive Representation for 3D Gaussian Segmentation](https://arxiv.org/abs/2512.00944)
*An Yang,Chenyu Liu,Jun Du,Jianqing Gao,Jia Pan,Jinshui Hu,Baocai Yin,Bing Yin,Cong Liu*

Main category: cs.CV

TL;DR: 本文提出了一种针对3D Gaussian Splatting (3D-GS) 的高效细粒度分割方法，通过二值编码压缩特征表示并采用渐进式训练策略，显著降低内存消耗并提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D-GS的分割方法存在高维类别特征导致的巨大内存开销，以及标签空间拥挤和缺乏稳定的多粒度控制机制导致的细粒度分割挑战。此外，光度渲染与语义分割之间的不兼容性常导致前景背景混淆。

Method: 1. 提出粗到细的二值编码方案，将每个高斯点的类别特征压缩为单个整数，大幅减少内存使用。2. 设计渐进式训练策略，将全景分割分解为一系列独立的子任务，减少类间冲突，提升细粒度分割能力。3. 在分割训练期间微调不透明度，以解决光度渲染和语义分割之间的不兼容性，避免前景背景混淆。

Result: 在多个基准测试中，该方法实现了最先进的分割性能，同时显著降低了内存消耗并加速了推理。

Conclusion: 本文提出的方法有效解决了3D-GS分割中内存开销大、细粒度分割困难以及前景背景混淆等问题，通过创新性的二值编码和渐进式训练策略，在性能和效率上均取得了显著提升。
...
### [210] [Efficient and Scalable Monocular Human-Object Interaction Motion Reconstruction](https://arxiv.org/abs/2512.00960)
...
TL;DR: 本文提出了4DHOISolver，一个利用稀疏人工标注接触点从单目视频中高效重建4D人-物交互(HOI)的优化框架。基于此，他们构建了大规模Open4DHOI数据集，并证明了其在强化学习代理运动模仿中的有效性，同时指出自动精确接触点预测仍是一个未解决的挑战。


<details>
  <summary>Details</summary>
Motivation: 通用机器人需要从多样化、大规模的人-物交互数据中学习，以在现实世界中稳健运行。单目网络视频提供了近乎无限且易于获取的数据源，但从中准确且可扩展地提取4D交互数据仍然是一个重大且未解决的挑战。

Method: 本文引入了4DHOISolver，一个新颖高效的优化框架。它通过利用稀疏的、人工参与的接触点标注来约束病态的4D HOI重建问题，同时保持高时空连贯性和物理合理性。基于此框架，他们构建了Open4DHOI数据集。

Result: 本文成功引入了4DHOISolver框架，并基于此构建了一个新的大规模4D HOI数据集Open4DHOI，该数据集包含144种物体类型和103种动作。他们还通过使基于强化学习的代理模仿恢复的运动，证明了重建的有效性。然而，对现有3D基础模型的全面基准测试表明，自动预测精确的人-物接触对应关系仍然是一个未解决的问题。

Conclusion: 精确的4D HOI重建目前需要人工参与策略，而自动预测人-物接触对应关系仍然是社区面临的一个开放挑战。
...
### [211] [Envision: Benchmarking Unified Understanding & Generation for Causal World Process Insights](https://arxiv.org/abs/2512.01816)
*Juanxi Tian,Siyuan Li,Conghui He,Lijun Wu,Cheng Tan*

Main category: cs.CV

TL;DR: 该研究提出了Envision，一个用于链式文本到多图像生成的因果事件进展基准，以评估多模态模型对动态过程的建模能力。结果显示，当前模型在时空一致性和世界知识内化方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型（特别是文本到图像模型）过度依赖静态单图像生成进行训练和评估，导致它们过拟合于静态模式匹配和语义融合，从而阻碍了其建模随时间展开的动态过程的能力。

Method: 研究提出了Envision，一个基于世界知识和时空因果关系构建的链式文本到多图像生成基准。它包含1,000个跨六个科学和人文学科领域的四阶段提示。为评估模型对世界知识的内化和因果时间约束的遵守情况，引入了Envision-Score，一个整合多维一致性、物理性和美学的整体指标。研究对15个模型（10个专业T2I模型，5个统一模型）进行了综合评估。

Result: 专业T2I模型在美学渲染方面表现出色，但缺乏内在世界知识。统一多模态模型在因果叙事连贯性方面优于专业模型，但仍逊色于闭源模型，并且难以克服时空一致性的核心挑战。这表明专注于因果隔离的单图像阻碍了多帧推理和生成，促进了静态模式匹配而非动态世界建模。

Conclusion: 当前模型对因果隔离单图像的关注限制了其世界知识的内化和动态生成能力，未能有效进行多帧推理和动态世界建模。未来的研究需要关注如何提升模型在时空一致性和动态过程建模方面的能力。
...
### [212] [Adaptive Evidential Learning for Temporal-Semantic Robustness in Moment Retrieval](https://arxiv.org/abs/2512.00953)
*Haojian Huang,Kaijing Ma,Jin Chen,Haodong Chen,Zhou Wu,Xianghao Zang,Han Fang,Chao Ban,Hao Sun,Mulin Chen,Zhongjiang He*

Main category: cs.CV

TL;DR: 本文提出了一种名为DEMR的新型框架，通过引入反射翻转融合块、查询重建任务和几何正则化器，解决了传统方法和深度证据回归在时刻检索中面临的模态不平衡和不确定性估计偏差问题，显著提升了检索的有效性、鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 时刻检索中，传统方法难以处理细粒度信息和确定性推理，难以匹配复杂或模糊的时刻。尽管深度证据回归（DER）被探索作为基线，但其存在模态不平衡和不确定性正则化器结构差异两大问题，导致不准确的样本被错误地赋予高不确定性，且现有方法缺乏对复杂视频场景的适应性。

Method: 本文提出了偏置消除证据学习时刻检索（DEMR）框架。该框架包含：1) 反射翻转融合（RFF）块，用于跨模态对齐；2) 查询重建任务，以增强文本敏感性并减少不确定性估计中的偏差；3) 几何正则化器（Geom-regularizer），用于细化不确定性预测，实现与困难时刻的自适应对齐。

Result: 在标准数据集和去偏数据集（ActivityNet-CD和Charades-CD）上的广泛测试表明，DEMR在有效性、鲁棒性和可解释性方面均取得了显著提升，并改善了检索准确性。

Conclusion: DEMR方法为时刻检索中的时序语义鲁棒性提供了一个有前景的解决方案，尤其擅长处理复杂和困难的时刻。
...
### [213] [SceneProp: Combining Neural Network and Markov Random Field for Scene-Graph Grounding](https://arxiv.org/abs/2512.00936)
*Keita Otani,Tatsuya Harada*

Main category: cs.CV

TL;DR: SceneProp通过将场景图定位重构为马尔可夫随机场（MRF）中的最大后验（MAP）推理问题，并利用可微分的信念传播算法进行全局推理，显著优于现有方法，并且查询图越大越复杂，性能提升越明显。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在处理包含多个对象和关系的复杂组合式视觉查询时面临挑战。标准短语定位方法擅长定位单个对象，但缺乏解析复杂关系描述的结构归纳偏差。场景图定位作为一种强大的但探索较少的公式，其现有方法也存在问题，即查询图越大性能反而下降，未能利用本应使定位更容易的信息。

Method: 该研究将场景图定位重构为马尔可夫随机场（MRF）中的最大后验（MAP）推理问题。通过在整个查询图上执行全局推理，SceneProp找到图像区域到节点的最佳分配，共同满足所有约束。这通过信念传播算法的可微分实现，在一个端到端框架中完成。

Result: 在四个基准测试中，SceneProp显著优于现有工作。更重要的是，其准确性随着查询图的大小和复杂性一致提高，首次证明了更多的关系上下文能够并且应该带来更好的定位效果。

Conclusion: SceneProp专注于场景图定位的特定公式，通过将问题转化为MRF中的MAP推理并使用可微分的信念传播进行全局推理，有效解决了现有方法的结构缺陷。这使得它能够充分利用复杂的查询图信息，实现更准确、更稳健的视觉查询定位。
...
### [214] [PhotoFramer: Multi-modal Image Composition Instruction](https://arxiv.org/abs/2512.00993)
*Zhiyuan You,Ke Wang,He Zhang,Xin Cai,Jinjin Gu,Tianfan Xue,Chao Dong,Zhoutong Zhang*

Main category: cs.CV

TL;DR: PhotoFramer是一个多模态构图指导框架，它能为构图不佳的图片提供自然语言的改进建议和高质量的示例图片，通过构建分层数据集和微调联合处理文本与图像的模型实现，并证明了其有效性。


<details>
  <summary>Details</summary>
Motivation: 许多普通用户在拍照时难以拍出构图良好的照片，因此需要提供构图指导来帮助他们改善图像构图。

Method: 引入PhotoFramer框架，它首先用自然语言描述如何改进构图，然后生成一个构图良好的示例图片。为此，研究人员构建了一个大规模数据集，将构图指导分为位移、缩放和视角改变等子任务。位移和缩放数据来自现有裁剪数据集，视角改变数据通过两阶段管道获得：从多视角数据集中采样不同视角的图片对，训练一个降级模型将构图良好的照片转换为差的，再将此模型应用于专家拍摄的照片以合成差的图像形成训练对。最后，微调一个能联合处理和生成文本与图像的模型。

Result: 实验证明，文本指令能有效引导图像构图，并且将文本指令与示例图像结合使用比单独使用示例的方法效果更好，取得了持续的改进。

Conclusion: PhotoFramer是实现构图助手的重要一步，它能让日常用户也能获得专业的摄影构图知识。
...
### [215] [S2AM3D: Scale-controllable Part Segmentation of 3D Point Cloud](https://arxiv.org/abs/2512.00995)
*Han Su,Tianyu Huang,Zichen Wan,Xiaohe Wu,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 本文提出S2AM3D方法，通过结合2D分割先验和3D一致性监督，解决了点云部件分割中3D模型泛化性差和2D知识引入导致多视角不一致的问题。同时，发布了一个大规模高质量数据集，S2AM3D在多个评估设置下表现出领先的性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究面临两大挑战：原生3D模型因数据稀缺而泛化能力不足；引入2D预训练知识会导致跨不同视角的分割结果不一致。

Method: 本文提出S2AM3D方法，其核心在于：1) 设计了一个点一致性部件编码器，通过原生3D对比学习聚合多视角2D特征，生成全局一致的点特征；2) 提出一个尺度感知提示解码器，通过连续尺度信号实现分割粒度的实时调整；3) 引入了一个包含超过10万个样本的大规模、高质量部件级点云数据集。

Result: S2AM3D在多个评估设置下取得了领先的性能，在处理复杂结构和尺寸差异大的部件时，表现出卓越的鲁棒性和可控性。

Conclusion: S2AM3D通过融合2D分割先验和3D一致性监督，并辅以大规模数据集，有效解决了点云部件分割的泛化性和一致性问题，实现了出色的分割表现。
...
### [216] [Accelerating Inference of Masked Image Generators via Reinforcement Learning](https://arxiv.org/abs/2512.01094)
*Pranav Subbaraman,Shufan Li,Siyan Zhao,Aditya Grover*

Main category: cs.CV

TL;DR: 本文提出Speed-RL，一种基于强化学习的新范式，旨在加速预训练的掩码生成模型（MGM），使其能在更少的步骤内生成高质量图像，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 掩码生成模型（MGM）能够生成高保真图像，但需要大量的采样步骤，导致推理速度缓慢。

Method: Speed-RL将加速问题视为一个强化学习问题，而非传统的分布匹配蒸馏。它结合了“质量奖励”和“速度奖励”，并通过强化学习对基础模型进行微调，以实现用更少步骤生成高质量图像的目标。

Result: 实验表明，所提出的方法能够将基础模型加速3倍，同时保持可比的图像质量。

Conclusion: Speed-RL通过强化学习范式，成功解决了MGM推理速度慢的问题，实现了显著的加速，并保持了高质量的图像生成能力。
...
### [217] [TRoVe: Discovering Error-Inducing Static Feature Biases in Temporal Vision-Language Models](https://arxiv.org/abs/2512.01048)
*Maya Varma,Jean-Benoit Delbrouck,Sophie Ostmeier,Akshay Chaudhari,Curtis Langlotz*

Main category: cs.CV

TL;DR: 本文介绍TRoVe，一种自动化方法，用于发现时间视觉语言模型（VLMs）中导致错误、依赖静态特征偏差的问题，并证明其在识别此类偏差方面的准确性和实用性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）在时间理解任务上取得了进展，但它们可能依赖静态特征偏差（如背景或物体特征）而非动态视觉变化进行预测，这是一种捷径，会导致系统性预测错误。在模型实际部署前，识别和表征这些导致错误的静态特征偏差至关重要。

Method: 本文提出了TRoVe，一种自动发现时间VLM学习到的错误诱导静态特征偏差的方法。TRoVe从数据集中提取候选静态特征，并根据(i)特征对分类错误的影响以及(ii)VLM在预测时对特征的依赖程度对每个特征进行评分。为了评估TRoVe，研究者构建了一个包含101个训练过的时间VLM及其真实静态特征偏差注释的评估框架。

Result: 通过评估框架，TRoVe能准确识别VLM中导致错误的静态特征偏差，比最接近的基线提高了28.6%。此外，将TRoVe应用于7个现成的VLM和2个时间理解任务，发现了先前未知的静态特征偏差，并证明了解这些学习到的偏差有助于在测试时提高模型性能。

Conclusion: TRoVe是一种有效且准确的自动化方法，能够发现时间视觉语言模型中导致错误的静态特征偏差。识别这些偏差对于模型在现实世界中的可靠部署至关重要，并且能够通过利用对偏差的认识来提升模型的性能。
...
### [218] [Lotus-2: Advancing Geometric Dense Prediction with Powerful Image Generative Model](https://arxiv.org/abs/2512.01030)
*Jing He,Haodong Li,Mingzhi Sheng,Ying-Cong Chen*

Main category: cs.CV

TL;DR: 本文提出了 Lotus-2，一个两阶段确定性框架，用于从单张图片中进行稳定、准确且细粒度的几何密集预测。它通过优化预训练的生成式扩散模型，实现了单目深度估计和表面法线预测的最新成果。


<details>
  <summary>Details</summary>
Motivation: 从单张图像恢复像素级几何属性是病态问题，因为存在外观模糊和2D观测到3D结构之间的非内射映射。判别式回归模型受限于数据规模、质量和多样性。虽然扩散模型拥有强大的世界先验知识，但其随机生成式公式不适合确定性几何推理，因为前者旨在生成多样性图像，而后者需要稳定准确的预测。

Method: Lotus-2 采用两阶段确定性框架：第一阶段，核心预测器使用单步确定性公式，结合纯净数据目标和轻量级局部连续性模块（LCM），生成全局一致的结构，避免网格伪影。第二阶段，细节锐化器在核心预测器定义的流形内执行受约束的多步整流流（rectified-flow）细化，通过无噪声确定性流匹配增强细粒度几何。

Result: 仅使用5.9万个训练样本（不到现有大规模数据集的1%），Lotus-2 在单目深度估计中取得了最先进的成果，并在表面法线预测中表现出极强的竞争力。

Conclusion: 这些结果表明，扩散模型可以作为确定性的世界先验，实现超越传统判别式和生成式范式的高质量几何推理。
...
### [219] [Learning Eigenstructures of Unstructured Data Manifolds](https://arxiv.org/abs/2512.01103)
*Roy Velich,Arkadi Piven,David Bensaïd,Daniel Cremers,Thomas Dagès,Ron Kimmel*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的框架，可以直接从非结构化数据中学习形状和流形分析的谱基，无需传统的算子选择、离散化和特征求解器。


<details>
  <summary>Details</summary>
Motivation: 传统的几何处理方法依赖于算子选择、离散化和特征分解，且通常对数据流形（如网格化或维度）有假设。研究动机是寻找一种无需这些步骤，能处理非结构化和高维数据的学习型替代方案。

Method: 该方法基于最优逼近理论，通过训练一个网络来分解一个隐式逼近算子。它通过最小化在所选探针函数分布上学习到的基中的重建误差来实现。对于合适的分布，这可以看作是拉普拉斯算子及其特征分解的近似。该方法以统一的方式恢复谱基、隐式度量的采样密度以及底层算子的特征值，且是无监督的，不对数据流形做任何假设。

Result: 在3D曲面上的点云和高维图像流形上，该方法生成了有意义的谱基，这些谱基可以类似于拉普拉斯算子的谱基，而无需显式构建算子。该方法能够扩展到任意维度的数据集。

Conclusion: 该框架通过学习方法取代了传统的算子选择、构建和特征分解，为几何处理提供了一种原则性、数据驱动的替代方案，为非结构化数据，特别是在高维空间中的几何处理开辟了新的可能性。
...
### [220] [OmniFD: A Unified Model for Versatile Face Forgery Detection](https://arxiv.org/abs/2512.01128)
*Haotian Liu,Haoyu Chen,Chenhui Pan,You Hu,Guoying Zhao,Xiaobai Li*

Main category: cs.CV

TL;DR: OmniFD是一个统一框架，通过共享编码器、跨任务交互模块和轻量级解码头，同时解决了图像/视频分类、空间定位和时间定位四项人脸伪造检测任务，提高了效率和性能。


<details>
  <summary>Details</summary>
Motivation: 当前人脸伪造检测方法通常采用独立架构的任务特定模型，导致计算冗余并忽略了相关任务之间的潜在关联。

Method: 本文提出了OmniFD，一个统一的框架，包含三个主要组件：1) 共享的Swin Transformer编码器，用于从图像和视频输入中提取统一的4D时空表示；2) 带有可学习查询的跨任务交互模块，通过基于注意力的推理动态捕获任务间依赖性；3) 轻量级解码头，将精炼的表示转换为所有FFD任务的相应预测。

Result: OmniFD优于任务特定模型，其统一设计利用多任务学习捕获跨任务的通用表示，并实现细粒度的知识迁移。例如，结合图像数据后，视频分类准确率提高了4.63%。此外，通过在一个框架内统一处理图像、视频和四项任务，OmniFD在各种基准测试中实现了卓越的性能，同时具有高效率和可扩展性，例如减少了63%的模型参数和50%的训练时间。

Conclusion: OmniFD为现实世界应用中的全面人脸伪造检测提供了一个实用且可推广的解决方案。
...
### [221] [Structural Prognostic Event Modeling for Multimodal Cancer Survival Analysis](https://arxiv.org/abs/2512.01116)
*Yilan Zhang,Li Nanbo,Changchun Yang,Jürgen Schmidhuber,Xin Gao*

Main category: cs.CV

TL;DR: SlotSPE是一种基于槽的框架，通过将组织学图像和基因数据压缩为离散槽来建模结构性预后事件，从而有效提高癌症生存预测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效处理高维和复杂的癌症多模态输入（组织学图像和基因谱），尤其是在捕获稀疏、患者特异性且未标注的关键预后事件（如空间组织学模式或通路共激活）方面，这些事件对患者预后至关重要。

Method: 提出SlotSPE框架，受因子编码原理启发，利用槽注意力机制将每个患者的多模态输入压缩成紧凑、模态特异性且相互独立的槽集。这些槽表示预后事件，使得框架能够高效有效地建模复杂的模态内和模态间交互，并无缝融入生物学先验知识以增强预后相关性。

Result: 在十个癌症基准测试中，SlotSPE在其中八个队列中表现优于现有方法，总体性能提升2.9%。该方法对基因组数据缺失具有鲁棒性，并通过结构化事件分解显著提高了可解释性。

Conclusion: SlotSPE框架通过高效建模结构性预后事件，显著提高了癌症生存预测的准确性和可解释性，克服了传统方法在处理高维多模态数据和捕获关键稀疏事件方面的挑战，并对缺失数据表现出鲁棒性。
...
### [222] [VSRD++: Autolabeling for 3D Object Detection via Instance-Aware Volumetric Silhouette Rendering](https://arxiv.org/abs/2512.01178)
*Zihua Liu,Hiroki Sakuma,Masatoshi Okutomi*

Main category: cs.CV

TL;DR: VSRD++ 提出了一种新颖的弱监督单目3D目标检测框架，通过神经场体渲染和2D监督，无需3D标注即可实现对静态和动态场景的有效3D检测。


<details>
  <summary>Details</summary>
Motivation: 现有的单目3D目标检测方法严重依赖于大量的3D标注，而这些标注通常从激光雷达点云中获取，过程耗时且劳动密集。本文旨在解决这一问题，摆脱对3D标注的依赖。

Method: VSRD++ 采用两阶段流水线：多视角3D自动标注和随后的单目3D检测器训练。在自动标注阶段，将目标表面表示为符号距离场（SDF），并通过实例感知的体素剪影渲染生成实例掩模。为优化3D边界框，将SDF分解为长方体SDF和残余距离场（RDF）。针对动态物体，将速度纳入边界框属性，并为每个伪标签分配置信度，同时引入3D属性初始化模块。在检测阶段，优化的3D边界框作为伪标签用于训练单目3D检测器。

Result: 在KITTI-360数据集上的大量实验表明，VSRD++ 在静态和动态场景中，均显著优于现有的弱监督单目3D目标检测方法。

Conclusion: VSRD++ 提供了一个无需3D标注的弱监督单目3D目标检测的有效解决方案，特别是在处理动态物体和几何不一致性方面表现出色，为3D场景理解开辟了新途径。
...
### [223] [Weakly Supervised Continuous Micro-Expression Intensity Estimation Using Temporal Deep Neural Network](https://arxiv.org/abs/2512.01145)
*Riyadh Mohammed Almushrafy*

Main category: cs.CV

TL;DR: 本文提出一个统一框架，仅利用稀疏的时间点（起始、顶点、结束）标注，通过三角先验生成伪强度轨迹，并结合ResNet18和双向GRU模型，实现微表情连续强度估计，在SAMM和CASME II数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中于微表情的离散类别分类，但很少关注其强度随时间的连续演变。由于缺乏帧级别的强度标注，使得完全监督的回归方法难以实施，限制了该方向的进展。

Method: 研究者提出了一个统一的框架，利用弱时间标签（起始、顶点、结束）来估计微表情的连续强度。通过简单的三角先验将稀疏的时间地标转换为密集的伪强度轨迹。然后，一个轻量级的时序回归模型（结合了ResNet18编码器和双向GRU）直接从图像序列预测帧级别的强度。该方法无需帧级别标注，并采用统一的预处理和时间对齐流程。

Result: 实验结果表明，该模型与伪强度轨迹具有很强的时间一致性。在SAMM数据集上，Spearman相关性达到0.9014，Kendall相关性达到0.7999，优于帧级别基线。在CASME II数据集上，不含顶点排序项时，分别达到0.9116和0.8168。消融研究证实，时序建模和结构化伪标签对于捕捉微表情的上升-顶点-下降动态至关重要。

Conclusion: 据作者所知，这是第一个仅使用稀疏时间标注进行微表情连续强度估计的统一方法，成功克服了缺乏密集标注的挑战，并有效捕捉了微表情的动态变化。
...
### [224] [EGG-Fusion: Efficient 3D Reconstruction with Geometry-aware Gaussian Surfel on the Fly](https://arxiv.org/abs/2512.01296)
...
### [225] [TabletopGen: Instance-Level Interactive 3D Tabletop Scene Generation from Text or Single Image](https://arxiv.org/abs/2512.01204)
...
### [226] [ViscNet: Vision-Based In-line Viscometry for Fluid Mixing Process](https://arxiv.org/abs/2512.01268)
...
### [227] [Supervised Contrastive Machine Unlearning of Background Bias in Sonar Image Classification with Fine-Grained Explainable AI](https://arxiv.org/abs/2512.01291)
...
### [228] [Closing the Approximation Gap of Partial AUC Optimization: A Tale of Two Formulations](https://arxiv.org/abs/2512.01213)
...
### [230] [PSR: Scaling Multi-Subject Personalized Image Generation with Pairwise Subject-Consistency Rewards](https://arxiv.org/abs/2512.01236)
...
### [231] [TRivia: Self-supervised Fine-tuning of Vision-Language Models for Table Recognition](https://arxiv.org/abs/2512.01248)
...
### [232] [nnMobileNet++: Towards Efficient Hybrid Networks for Retinal Image Analysis](https://arxiv.org/abs/2512.01273)
...
### [233] [TBT-Former: Learning Temporal Boundary Distributions for Action Localization](https://arxiv.org/abs/2512.01298)
...
### [234] [Gaussian Swaying: Surface-Based Framework for Aerodynamic Simulation with 3D Gaussians](https://arxiv.org/abs/2512.01306)
...
### [235] [Lost in Distortion: Uncovering the Domain Gap Between Computer Vision and Brain Imaging - A Study on Pretraining for Age Prediction](https://arxiv.org/abs/2512.01310)
...
### [236] [IVCR-200K: A Large-Scale Multi-turn Dialogue Benchmark for Interactive Video Corpus Retrieval](https://arxiv.org/abs/2512.01312)
*Ning Han,Yawen Zeng,Shaohua Long,Chengqing Li,Sijie Yang,Dun Tan,Jianfeng Dong,Jingjing Chen*

Main category: cs.CV

TL;DR: 本文提出交互式视频语料库检索 (IVCR) 任务，以实现用户与检索系统之间的多轮对话式交互。为此，我们构建了高质量双语数据集 IVCR-200K，并提出了一个基于多模态大语言模型 (MLLM) 的全面框架，实验证明了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索系统缺乏用户与系统之间的“交互”，其单向检索范式无法满足超过80.8%用户对个性化和动态检索的需求。

Method: 引入交互式视频语料库检索 (IVCR) 任务；构建了高质量、双语、多轮对话式、抽象语义的数据集 IVCR-200K，支持视频和片段检索；提出了一个基于多模态大语言模型 (MLLM) 的综合框架，以实现多模式交互和更具解释性的解决方案。

Result: 广泛的实验证明了所提出的数据集和框架的有效性。

Conclusion: 本文通过引入 IVCR 任务、构建 IVCR-200K 数据集和提出 MLLM 框架，为实现用户与检索系统之间更真实、个性化的多轮交互式视频检索奠定了基础。
...
TL;DR: 本文提出AlignVid，一个免训练的框架，通过注意力缩放调制和引导调度，解决文本引导图像到视频（TI2V）生成中的语义疏忽问题，显著提升了对精细提示语义的遵循。


<details>
  <summary>Details</summary>
Motivation: 现有TI2V方法在处理精细提示语义时（特别是涉及对象添加、删除或修改等显著图像转换时）存在困难，导致“语义疏忽”。通过初步研究发现，对输入图像应用高斯模糊可改善语义遵循，这与更清晰的前景-背景分离和较低熵的交叉注意力分布相关。

Method: 引入AlignVid框架，该框架免训练并包含两个核心组件：(i) 注意力缩放调制（Attention Scaling Modulation, ASM），通过轻量级的Q或K缩放直接重新加权注意力；(ii) 引导调度（Guidance Scheduling, GS），选择性地在Transformer块和去噪步骤中应用ASM，以减少视觉质量下降。此外，还提出了OmitI2V数据集（包含367个人工标注样本）来评估TI2V生成中的语义疏忽。

Result: AlignVid在限制美学退化的同时，显著改善了提示遵循。在OmitI2V数据集上进行的广泛实验证明，AlignVid能够有效增强语义保真度。

Conclusion: AlignVid通过最小的干预，有效解决了TI2V生成中的语义疏忽问题，显著提升了模型对精细提示语义的遵循能力，同时保持了良好的视觉质量。
...
### [238] [SRAM: Shape-Realism Alignment Metric for No Reference 3D Shape Evaluation](https://arxiv.org/abs/2512.01373)
*Sheng Liu,Tianyu Luan,Phani Nuney,Xuelu Feng,Junsong Yuan*

Main category: cs.CV

TL;DR: 本文提出了一种名为Shape-Realism Alignment Metric (SRAM)的新型3D形状真实感评估指标，它利用大型语言模型(LLM)作为桥梁，无需真实参考即可评估3D形状的真实感，并引入了一个新的数据集。


<details>
  <summary>Details</summary>
Motivation: 随着3D生成和重建技术的发展，对真正逼真的3D形状的需求日益增长。传统评估方法依赖于真实参考来衡量网格保真度，但在许多实际应用中，形状的真实感并不需要真实参考。因此，需要一种不依赖真实参考的真实感评估方法。

Method: 本文提出Shape-Realism Alignment Metric (SRAM)。该方法采用网格编码将3D形状转换为语言标记空间，并利用大型语言模型(LLM)作为形状信息和真实感评估之间的桥梁。设计了一个专门的真实感解码器，将语言模型的输出与人类对真实感的感知对齐。此外，本文还引入了一个名为RealismGrading的新数据集，该数据集提供人类标注的真实感评分，无需真实形状。

Result: 通过对不同对象进行k折交叉验证，验证了该指标的性能和泛化能力。实验结果表明，该指标与人类感知高度相关，优于现有方法，并具有良好的泛化性。

Conclusion: 本文提出的Shape-Realism Alignment Metric (SRAM)能够有效且通用地评估3D形状的真实感，无需真实参考，并且其性能优于现有方法，与人类感知高度一致。
...
### [239] [TokenPure: Watermark Removal through Tokenized Appearance and Structural Guidance](https://arxiv.org/abs/2512.01314)
*Pei Yang,Yepeng Liu,Kelly Peng,Yuan Gao,Yiren Song*

Main category: cs.CV

TL;DR: TokenPure是一个基于Diffusion Transformer的水印去除框架，通过令牌条件重建，解决了水印破坏与内容一致性的权衡问题，实现了先进的水印去除效果和重建保真度。


<details>
  <summary>Details</summary>
Motivation: 在数字经济时代，数字水印是证明AI生成内容和虚拟资产等大规模可复制内容所有权的关键基础。设计能够抵抗各种攻击和处理操作的鲁棒水印至关重要，因此，开发有效且一致的水印去除方法也变得同样重要。

Method: 本文提出了TokenPure，一个新颖的基于Diffusion Transformer的框架。它将水印去除任务重新定义为条件生成，完全绕过初始携带水印的噪声。具体方法是将带水印图像分解为视觉令牌（用于纹理）和结构令牌（用于几何），这些令牌共同调节扩散过程，从而合成无水印图像，同时保持精细的一致性和结构完整性。

Result: 全面的实验表明，TokenPure在水印去除和重建保真度方面达到了最先进的水平，在感知质量和一致性方面均显著优于现有基线。

Conclusion: TokenPure提供了一个有效且一致的水印去除解决方案，通过令牌条件重建成功平衡了彻底的水印破坏和内容一致性，并取得了卓越的性能。
...
### [240] [EvalTalker: Learning to Evaluate Real-Portrait-Driven Multi-Subject Talking Humans](https://arxiv.org/abs/2512.01340)
...
TL;DR: 该研究构建了首个大规模多说话人生成质量评估数据集THQA-MT，并提出了EvalTalker框架，用于感知说话人质量、特征、身份一致性和多模态同步性，以解决多说话人生成中的质量下降问题。


<details>
  <summary>Details</summary>
Motivation: 当前的单说话人生成系统在多主体驱动能力上存在局限性。虽然“多说话人”范式能带来更丰富的交互和沉浸感，但现有系统仍存在明显的质量下降，导致用户体验不佳。因此，需要一个有效的评估工具来解决这一挑战。

Method: 研究团队构建了THQA-MT数据集，包含5,492个由15个代表性多说话人系统生成的说话人视频，使用了400张真实肖像。通过主观实验分析了不同系统的感知差异并识别出12种常见失真类型。此外，提出了EvalTalker框架，该框架能够感知全局质量、人类特征、身份一致性，并整合Qwen-Sync来感知多模态同步性。

Result: 主观实验分析揭示了不同多说话人系统之间的感知差异，并识别出12种常见的失真类型。实验结果表明，EvalTalker框架与主观评分表现出卓越的相关性。

Conclusion: EvalTalker框架为未来高质量多说话人生成和评估研究奠定了坚实的基础，能够有效解决当前多说话人系统存在的质量评估挑战。
...
TL;DR: OpenBox提出了一种两阶段的自动化标注流程，利用2D视觉基础模型进行无监督、开放词汇的3D物体检测，通过跨模态实例对齐和自适应包围盒生成，无需自训练即可提供高质量3D标注，提高了精度和效率。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶领域，减少标注成本和识别未知物体至关重要。现有无监督和开放词汇3D物体检测方法存在标注质量不佳、计算开销大等问题，因为它们通常统一标注3D包围盒，忽略物体的物理状态，并需要多次自训练迭代来优化标注。

Method: OpenBox采用两阶段自动化标注流程，利用2D视觉基础模型：
1.  **第一阶段**：通过跨模态实例对齐，将2D视觉基础模型处理的2D图像中的实例级线索与对应的3D点云关联起来。
2.  **第二阶段**：根据实例的刚性和运动状态进行分类，然后结合类别特定的尺寸统计生成自适应的包围盒。该方法无需自训练。

Result: OpenBox能够生成高质量的3D包围盒标注，且无需自训练。在Waymo Open Dataset、Lyft Level 5 Perception数据集和nuScenes数据集上的实验表明，OpenBox比基线方法在准确性和效率方面都有所提高。

Conclusion: OpenBox通过其创新的两阶段自动化标注流程，有效解决了无监督和开放词汇3D物体检测中的挑战，提供了更准确、高效的3D标注，尤其适用于自动驾驶场景，且避免了传统方法中自训练的计算开销。
...
### [242] [Optimizing Stroke Risk Prediction: A Machine Learning Pipeline Combining ROS-Balanced Ensembles and XAI](https://arxiv.org/abs/2512.01333)
*A S M Ahsanul Sarkar Akib,Raduana Khawla,Abdul Hasib*

Main category: cs.CV

TL;DR: 本研究结合集成学习和可解释人工智能（XAI）技术，构建了一个可解释的机器学习框架，用于高精度预测中风风险，并识别出关键临床变量。


<details>
  <summary>Details</summary>
Motivation: 中风是全球主要的死亡和永久性损伤原因，早期风险评估对于及时干预和成功预防策略至关重要。

Method: 研究采用集成建模和可解释AI（XAI）技术，开发了一个可解释的机器学习框架。方法包括对10种不同机器学习模型进行5折交叉验证评估，进行特征工程和数据预处理（使用随机过采样ROS解决类别不平衡问题）。最终优化了一个集成模型（随机森林+ExtraTrees+XGBoost），并使用LIME进行可解释性分析，以识别重要的临床变量。

Result: 优化的集成模型（随机森林+ExtraTrees+XGBoost）在中风预测数据集（SPD）上表现出色，达到了99.09%的准确率。通过基于LIME的可解释性分析，识别出三个重要的临床变量：年龄、高血压和血糖水平。

Conclusion: 本研究强调了集成学习与可解释AI（XAI）相结合，能够提供高度准确且可解释的中风风险评估。该框架有潜力通过数据驱动的预防和个性化临床决策，改变中风预测和心血管风险管理。
...
### [243] [Handwritten Text Recognition for Low Resource Languages](https://arxiv.org/abs/2512.01348)
*Sayantan Dey,Alireza Alaei,Partha Pratim Roy*

Main category: cs.CV

TL;DR: 本文介绍了BharatOCR，一个用于印地语和乌尔都语的无分割段落级手写文本识别系统，采用ViT-Transformer解码器-LM架构，并在多个数据集上取得了基准测试结果。


<details>
  <summary>Details</summary>
Motivation: 尽管手写文本识别取得了进展，但段落级手写文本识别，特别是在印地语、乌尔都语等低资源语言中，由于缺乏全面的语言资源，仍然是一个具有挑战性的问题。

Method: BharatOCR提出了一种ViT-Transformer解码器-LM架构。其中，Vision Transformer（ViT，具体使用DeiT进行掩码图像建模）提取视觉特征，Transformer解码器生成文本序列，预训练语言模型（LM，具体使用RoBERTa进行掩码语言建模）用于细化输出以提高准确性、流畅性和连贯性。模型采用逐行迭代处理段落图像的“隐式行分割”方法。该模型在自定义数据集（'Parimal Urdu'和'Parimal Hindi'）以及两个公共数据集上进行了评估。

Result: 该模型在NUST-UHWR、PUCIT-OUHL和Parimal-Urdu数据集上分别取得了96.24%、92.05%和94.80%的字符识别率，并在印地语数据集上取得了80.64%的字符识别率。实验结果表明，该模型优于几种最先进的乌尔都语文本识别方法。

Conclusion: BharatOCR成功解决了印地语和乌尔都语的段落级手写文本识别挑战，通过创新的架构和隐式行分割方法，在多个数据集上取得了优于现有方法的基准性能。
...
### [244] [InternVideo-Next: Towards General Video Foundation Models without Video-Text Supervision](https://arxiv.org/abs/2512.01342)
*Chenting Wang,Yuhan Zhu,Yicheng Xu,Jiange Yang,Ziang Yan,Yali Wang,Yi Wang,Limin Wang*

Main category: cs.CV

TL;DR: InternVideo-Next通过解耦的EPD框架和两阶段预训练方案，结合条件扩散解码器和语义先验，构建语义一致且细节保留的潜在空间，有效学习世界知识，实现视频表征学习的SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视频-文本预训练依赖有噪声的合成字幕，语义覆盖有限，忽视隐式世界知识。掩码视频建模（MVM）虽利用时空结构，但在通用任务上表现不佳，原因在于像素级重建难以收敛且与语义冲突，而潜在预测易导致捷径学习。

Method: 提出Encoder-Predictor-Decoder (EPD) 框架，其中预测器充当潜在世界模型。采用InternVideo-Next两阶段预训练方案：第一阶段，引入条件扩散解码器并注入图像级语义先验，以弥合像素级保真度与高级语义抽象之间的差距，构建语义一致且细节保留的潜在空间；第二阶段，在该潜在空间内预测冻结的第一阶段目标，以学习世界知识并缓解捷径学习。

Result: 在公共、无标签视频上训练后，InternVideo-Next在各项基准测试中均取得了最先进（SOTA）的结果。

Conclusion: InternVideo-Next为通用视频表征学习提供了一条可扩展的路径。
...
### [245] [BlinkBud: Detecting Hazards from Behind via Sampled Monocular 3D Detection on a Single Earbud](https://arxiv.org/abs/2512.01366)
...
### [247] [ResDiT: Evoking the Intrinsic Resolution Scalability in Diffusion Transformers](https://arxiv.org/abs/2512.01426)
...
### [248] [Reversible Inversion for Training-Free Exemplar-guided Image Editing](https://arxiv.org/abs/2512.01382)
...
### [249] [\textit{ViRectify}: A Challenging Benchmark for Video Reasoning Correction with Multimodal Large Language Models](https://arxiv.org/abs/2512.01424)
...
### [250] [Textured Geometry Evaluation: Perceptual 3D Textured Shape Metric via 3D Latent-Geometry Network](https://arxiv.org/abs/2512.01380)
...
### [251] [MDiff4STR: Mask Diffusion Model for Scene Text Recognition](https://arxiv.org/abs/2512.01422)
...
### [254] [FRAMER: Frequency-Aligned Self-Distillation with Adaptive Modulation Leveraging Diffusion Priors for Real-World Image Super-Resolution](https://arxiv.org/abs/2512.01390)
...
### [255] [CourtMotion: Learning Event-Driven Motion Representations from Skeletal Data for Basketball](https://arxiv.org/abs/2512.01478)
...
### [256] [ChronosObserver: Taming 4D World with Hyperspace Diffusion Sampling](https://arxiv.org/abs/2512.01481)
...
### [257] [A variational method for curve extraction with curvature-dependent energies](https://arxiv.org/abs/2512.01494)
...
### [258] [FlashVGGT: Efficient and Scalable Visual Geometry Transformers with Compressed Descriptor Attention](https://arxiv.org/abs/2512.01540)
*Zipeng Wang,Dan Xu*
...
### [259] [ELVIS: Enhance Low-Light for Video Instance Segmentation in the Dark](https://arxiv.org/abs/2512.01495)
...
### [260] [Toward Content-based Indexing and Retrieval of Head and Neck CT with Abscess Segmentation](https://arxiv.org/abs/2512.01589)
...
### [261] [QuantumCanvas: A Multimodal Benchmark for Visual Learning of Atomic Interactions](https://arxiv.org/abs/2512.01519)
...
### [262] [Semantic-aware Random Convolution and Source Matching for Domain Generalization in Medical Image Segmentation](https://arxiv.org/abs/2512.01510)
...
### [263] [Depth Matching Method Based on ShapeDTW for Oil-Based Mud Imager](https://arxiv.org/abs/2512.01611)
...
### [264] [Generative Editing in the Joint Vision-Language Space for Zero-Shot Composed Image Retrieval](https://arxiv.org/abs/2512.01636)
...
### [265] [DreamingComics: A Story Visualization Pipeline via Subject and Layout Customized Generation using Video Models](https://arxiv.org/abs/2512.01686)
*Patrick Kwon,Chen Chen*

Main category: cs.CV

TL;DR: DreamingComics是一个布局感知的漫画故事可视化框架，它基于预训练的视频扩散Transformer模型，通过引入RegionalRoPE、掩码条件损失和LLM布局生成器，显著提升了角色和风格一致性，并实现了高空间准确性。


<details>
  <summary>Details</summary>
Motivation: 当前的故事可视化方法仅依靠文本定位主体，难以保持艺术一致性。研究旨在解决这些局限性。

Method: 该方法构建于预训练的视频扩散Transformer (DiT) 模型之上，利用其时空先验知识增强身份和风格一致性。为实现基于布局的位置控制，提出了区域感知的定位编码方案RegionalRoPE，根据目标布局重新索引嵌入。此外，引入了掩码条件损失，将每个主体的视觉特征约束在其指定区域。为了从自然语言脚本中推断布局，集成了经过训练的基于LLM的布局生成器，以产生漫画风格的布局，从而实现灵活可控的布局条件化。

Result: 与现有方法相比，该方法在角色一致性方面提高了29.2%，在风格相似性方面提高了36.2%，并展示了高空间准确性。

Conclusion: DreamingComics通过其创新的框架，成功解决了故事可视化中主体定位和艺术一致性的挑战，实现了更具控制性和高质量的漫画生成。
...
### [266] [SSR: Semantic and Spatial Rectification for CLIP-based Weakly Supervised Segmentation](https://arxiv.org/abs/2512.01701)
*Xiuli Bi,Die Xiao,Junchao Fan,Bin Xiao*

Main category: cs.CV

TL;DR: 本文提出了一种语义与空间校正（SSR）方法，以解决基于CLIP的弱监督语义分割（WSSS）中非目标前景和背景区域的过度激活问题，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的弱监督语义分割方法存在局限性，即在非目标前景区域和背景区域出现过度激活，影响了分割精度。

Method: 本文提出了语义与空间校正（SSR）方法。在语义层面，通过跨模态原型对齐（CMPA）建立对比学习机制，强制跨模态特征空间对齐，减少类间重叠并增强语义相关性，以校正非目标前景区域的过度激活。在空间层面，利用超像素引导校正（SGC），通过基于超像素的空间先验在亲和传播过程中精确过滤非目标区域的干扰，显著校正背景过度激活。

Result: 在PASCAL VOC和MS COCO数据集上的大量实验表明，该方法优于所有单阶段和更复杂的多阶段方法，mIoU分数分别达到79.5%和50.6%。

Conclusion: 所提出的语义与空间校正（SSR）方法有效解决了基于CLIP的弱监督语义分割中的过度激活问题，通过语义和空间层面的精确校正，显著提升了性能，并取得了最先进的结果。
...
### [267] [VideoScoop: A Non-Traditional Domain-Independent Framework For Video Analysis](https://arxiv.org/abs/2512.01769)
*Hafsa Billah*

Main category: cs.CV

TL;DR: 本文提出了一种通用的视频情境分析（VSA）框架，结合扩展关系模型（R++）和图模型来自动识别视频中的有意义活动，克服了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 自动理解视频内容对公民监控、一般监控和辅助生活等应用至关重要。尽管内容提取技术已取得进展，但识别有意义的活动或情境仍然困难，仅靠内容提取无法实现。当前视频情境分析要么依赖人工，效率低下且易出错，要么使用针对特定视频或情境的定制算法，缺乏通用性。

Method: 该框架首先使用先进技术提取视频内容，然后通过两种模型表示：扩展关系模型（R++）和图模型。R++支持数据流处理和连续查询语言（CQL-VA），而图模型则用于检测关系模型难以或无法处理的情境，并利用现有及新开发的图算法。为实现领域独立性，系统通过参数化模板表达跨领域的基本情境变体。通过对来自辅助生活、公民监控和一般监控三个领域多种情境的大量视频数据集进行实验，评估了方法的准确性、效率和鲁棒性。

Result: 该方法在辅助生活、公民监控和一般监控三个领域的多种有趣情境下进行了广泛实验，评估了其准确性、效率和鲁棒性。实验结果表明，所提出的方法能够支持广泛的情境检测，并有效克服了现有方法的局限性。

Conclusion: 所提出的通用视频情境分析框架通过结合关系模型和图模型，以及支持领域独立性的参数化模板，成功克服了当前视频情境分析中人工干预、定制算法和内容提取局限性等问题，实现了准确、高效和鲁棒的自动情境理解。
...
### [268] [FreqEdit: Preserving High-Frequency Features for Robust Multi-Turn Image Editing](https://arxiv.org/abs/2512.01755)
*Yucheng Liao,Jiajun Liang,Kaiqian Cui,Baoquan Zhao,Haoran Xie,Wei Liu,Qing Li,Xudong Mao*

Main category: cs.CV

TL;DR: FreqEdit是一个免训练框架，通过注入高频特征、自适应策略和路径补偿，解决了多轮图像编辑中高频信息丢失导致的质量下降问题，实现了10+次连续编辑的稳定性和高质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于自然语言的图像编辑模型在单次编辑中表现出色，但在多轮编辑时会出现严重的质量下降。系统分析表明，高频信息的逐步丢失是导致这种质量下降的主要原因。

Method: 本文提出了FreqEdit，一个免训练框架，包含三个协同组件：1) 从参考速度场注入高频特征以保留精细细节；2) 自适应注入策略，空间调制注入强度以实现精确的区域控制；3) 路径补偿机制，定期重新校准编辑轨迹以防止过度约束。

Result: 广泛的实验表明，与七个最先进的基线相比，FreqEdit在身份保留和指令遵循方面都取得了卓越的性能，能够稳定支持10+次连续迭代编辑。

Conclusion: FreqEdit通过其独特的三组件方法，有效解决了多轮图像编辑中的质量下降问题，显著提高了编辑的稳定性和准确性，实现了高质量的多轮图像编辑。
...
### [269] [HiconAgent: History Context-aware Policy Optimization for GUI Agents](https://arxiv.org/abs/2512.01763)
*Xurui Zhou,Gongwei Chen,Yuquan Xie,Zaijing Li,Kaiwen Zhou,Shuai Wang,Shuo Yang,Zhuotao Tian,Rui Shao*

Main category: cs.CV

TL;DR: HiconAgent引入历史上下文感知策略优化(HCPO)，通过动态上下文采样(DCS)和锚点引导历史压缩(AHC)解决GUI智能体在利用历史信息时计算开销大和效率低的问题，实现了在GUI导航任务上的高效和高性能。


<details>
  <summary>Details</summary>
Motivation: 图形用户界面(GUI)智能体在执行顺序导航任务时需要有效利用历史上下文，但直接使用完整历史会导致过高的计算开销并分散对无关信息的注意力。

Method: 本文提出了HiconAgent，一个通过历史上下文感知策略优化(HCPO)训练的GUI智能体。HCPO包含两个互补组件：1) 动态上下文采样(DCS)，在采样过程中提供可变长度的历史，以自适应地使用最相关的上下文；2) 锚点引导历史压缩(AHC)，通过双分支策略优化策略更新阶段，其中压缩分支移除历史观察但保留历史动作作为信息流锚点。压缩和未压缩分支通过历史增强对齐损失耦合，以在保持效率的同时强制执行一致的历史使用。

Result: 在主流GUI导航基准测试中，HiconAgent-3B在GUI-Odyssey上，接地精度比GUI-R1-7B高出8.46%，步骤成功率高出11.32%。在AndroidControl和AITW上取得了可比的结果，同时实现了高达2.47倍的计算速度提升和60%的FLOPs减少。

Conclusion: HiconAgent通过其创新的历史上下文处理方法，有效地利用了历史信息，显著提升了GUI导航任务的性能，同时大幅降低了计算成本和资源消耗。
...
### [270] [Bridging the Scale Gap: Balanced Tiny and General Object Detection in Remote Sensing Imagery](https://arxiv.org/abs/2512.01665)
*Zhicheng Zhao,Yin Huang,Lingma Sun,Chenglong Li,Jin Tang*

Main category: cs.CV

TL;DR: 本文提出了ScaleBridge-Det，这是首个针对遥感图像中微小目标检测的大型框架，通过尺度自适应专家路由和密度引导查询分配，实现了在各种目标尺度上的平衡检测性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中的微小目标检测面临巨大挑战，尤其是在密集微小目标与大型目标共存的场景中，难以实现跨尺度检测性能的平衡。现有的基础模型尚未探索如何有效应用于遥感图像中极端的尺度变化和密度分布问题。

Method: ScaleBridge-Det框架包含两个核心模块：1) 路由增强混合注意力（REM）模块，通过自适应路由动态选择和融合特定尺度的专家特征，生成互补且有区分度的多尺度表示；2) 密度引导动态查询（DGQ）模块，预测目标密度以自适应调整查询位置和数量，实现对不同尺度目标的有效资源分配。

Result: 在AI-TOD-V2和DTOD基准数据集上，ScaleBridge-Det取得了最先进的性能，并在VisDrone数据集上展现出卓越的跨域鲁棒性。

Conclusion: ScaleBridge-Det能够同时优化密集微小目标和通用目标的检测性能，且无需进行权衡，有效解决了遥感图像中目标尺度多样性和密度分布带来的挑战。
...
### [271] [ViT$^3$: Unlocking Test-Time Training in Vision](https://arxiv.org/abs/2512.01643)
*Dongchen Han,Yining Li,Tianyu Li,Zixuan Cao,Ziming Wang,Jun Song,Yu Cheng,Bo Zheng,Gao Huang*

Main category: cs.CV

TL;DR: 本文对视觉序列建模中的测试时训练（TTT）设计进行了系统实证研究，提炼出六项实用见解，并基于此提出了ViT³模型。ViT³实现了线性复杂度、可并行计算，并在多种视觉任务上表现优异，媲美甚至超越了先进的线性复杂度模型，并缩小了与高度优化Vision Transformer的差距。


<details>
  <summary>Details</summary>
Motivation: 测试时训练（TTT）为高效序列建模提供了有前景的方向，将注意力操作重构为在线学习问题，但在视觉TTT设计中，关于内部模块和内部训练的基本选择缺乏全面理解和实用指导，导致构建强大的视觉TTT模型仍具挑战性。

Method: 研究人员对视觉序列建模的TTT设计进行了系统性的实证研究。通过一系列实验和分析，他们提炼出六项实用见解，建立了有效视觉TTT的设计原则。基于这些发现，他们构建了Vision Test-Time Training (ViT³) 模型，这是一个纯粹的TTT架构。

Result: 研究提炼出六项实用见解，为有效视觉TTT建立了设计原则。ViT³模型实现了线性计算复杂度和可并行计算。在图像分类、图像生成、目标检测和语义分割等多种视觉任务上，ViT³始终匹配或超越了先进的线性复杂度模型（如Mamba和线性注意力变体），并有效缩小了与高度优化的Vision Transformer之间的性能差距。

Conclusion: 这项研究提供了有效视觉TTT的设计原则，并且ViT³基线模型展示了其在多种视觉任务上的强大性能和效率。作者希望这项研究和ViT³基线能够促进未来视觉TTT模型的发展。
...
### [272] [DB-KAUNet: An Adaptive Dual Branch Kolmogorov-Arnold UNet for Retinal Vessel Segmentation](https://arxiv.org/abs/2512.01657)
*Hongyu Xu,Panpan Meng,Meng Wang,Dayu Hu,Liming Liang,Xiaoqi Sheng*

Main category: cs.CV

TL;DR: 本文提出了一种自适应双分支Kolmogorov-Arnold UNet (DB-KAUNet) 模型，用于视网膜血管分割。该模型结合了CNN和Transformer的优势，并引入了KANConv和KAT块以及自适应融合机制，以克服传统CNN在捕获长距离依赖和复杂非线性关系方面的局限性，实现了领先的分割性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视网膜血管的精确分割对多种眼科和全身疾病的临床诊断至关重要。然而，传统的卷积神经网络（CNN）方法在捕获长距离依赖和复杂非线性关系方面存在固有限制。

Method: 本文提出了DB-KAUNet模型。其核心是一个异构双分支编码器（HDBE），包含并行的CNN和Transformer路径，并策略性地交织了标准的CNN/Transformer块与新颖的KANConv和KAT块，以形成全面的特征表示。HDBE集成了几个关键组件：1) 跨分支通道交互（CCI）模块，促进通道特征在并行路径间的有效交互；2) 基于注意力的空间特征增强（SFE）模块，增强空间特征并融合两分支的输出；3) 进阶版几何自适应融合空间特征增强（SFE-GAF）模块，利用自适应采样精确聚焦真实血管形态，强化显著血管特征，同时减少背景噪声和计算开销。

Result: 在DRIVE、STARE和CHASE_DB1数据集上进行的广泛实验验证了DB-KAUNet实现了领先的分割性能，并展示了卓越的鲁棒性。

Conclusion: DB-KAUNet通过结合CNN和Transformer的优势，并引入自适应机制，成功克服了传统CNN在视网膜血管分割中捕获复杂特征的局限性，实现了先进的分割效果和强大的泛化能力。
...
### [273] [GRASP: Guided Residual Adapters with Sample-wise Partitioning](https://arxiv.org/abs/2512.01675)
*Felix Nützel,Mischa Dombrowski,Bernhard Kainz*

Main category: cs.CV

TL;DR: 本文提出GRASP方法，通过样本分区和集群特定残差适配器，解决长尾数据（如医学图像）中文本到图像扩散模型因梯度冲突导致的模式崩溃问题，显著提升了稀有类别的生成质量和多样性。


<details>
  <summary>Details</summary>
Motivation: 当前的文本到图像扩散模型在长尾设置（例如医学影像）中表现不佳，稀有病理数据占比小，导致模式崩溃。这使得尾部类别的输出质量和多样性不足，阻碍了对代表性不足条件的合成数据增强。主要原因是频繁的首部类别和稀有的尾部类别之间的梯度冲突。

Method: 本文提出GRASP（Guided Residual Adapters with Sample-wise Partitioning）方法。它利用外部先验知识将样本静态地划分为集群，以最小化组内梯度冲突。然后，通过将集群特定的残差适配器注入到Transformer前馈层中，对预训练模型进行微调，绕过学习到的门控机制以提高稳定性和效率。

Result: 在长尾MIMIC-CXR-LT数据集上，GRASP在FID和多样性指标上表现优越，尤其对稀有类别效果显著，优于香草微调和专家混合等基线方法。在NIH-CXR-LT数据集上的下游分类任务中，尾部标签的性能得到显著改善。该方法还泛化到ImageNet-LT，证实了其广泛适用性。GRASP轻量级、可扩展，并易于集成到扩散管道中。

Conclusion: GRASP通过解决长尾数据中梯度冲突导致的模式崩溃问题，成功提高了文本到图像扩散模型在稀有类别上的生成质量和多样性，为合成数据增强提供了有效方案，具有广泛适用性和集成便利性。
...
### [274] [Open-world Hand-Object Interaction Video Generation Based on Structure and Contact-aware Representation](https://arxiv.org/abs/2512.01677)
...
TL;DR: 本文提出了一种结构和接触感知表示（SCAR）及联合生成范式，以解决现有手物交互视频生成方法在可扩展性和交互保真度上的两难困境，从而生成物理真实且时间连贯的手物交互视频。


<details>
  <summary>Details</summary>
Motivation: 生成逼真的手物交互（HOI）视频是一个重大挑战，因为它难以建模物理约束（如手与物体之间的接触和遮挡）。当前方法利用HOI表示作为辅助生成目标，但2D和3D表示在可扩展性和交互保真度之间存在两难困境，无法同时保证两者。

Method: 我们提出了一种结构和接触感知表示（SCAR），它无需3D标注即可捕获手物接触、手物遮挡和整体结构上下文。这种面向交互且可扩展的监督信号使模型能够学习精细的交互物理并泛化到开放世界场景。为充分利用所提出的表示，我们引入了一种具有“共享与专业化”策略的联合生成范式，用于同时生成交互导向的表示和视频。

Result: 我们的方法在两个真实世界数据集上，生成物理真实且时间连贯的HOI视频方面优于现有最先进的方法。此外，我们的方法对具有挑战性的开放世界场景表现出强大的泛化能力。

Conclusion: 所提出的可扩展设计在生成物理真实且时间连贯的手物交互视频方面具有显著优势，尤其是在泛化到开放世界场景时表现出色。
...
### [275] [SAM3-UNet: Simplified Adaptation of Segment Anything Model 3](https://arxiv.org/abs/2512.01789)
*Xinyu Xiong,Zihuang Wu,Lei Lu,Yufa Xia*

Main category: cs.CV

TL;DR: 本文提出SAM3-UNet，一个简化的SAM3变体，旨在以低成本将SAM3适应到下游任务，并在多项任务上超越现有SOTA方法且显存占用更低。


<details>
  <summary>Details</summary>
Motivation: 将最新的Segment Anything Model 3 (SAM3) 有效且低成本地应用于各种下游任务。

Method: SAM3-UNet由三个部分组成：一个SAM3图像编码器、一个用于参数高效微调的简单适配器，以及一个轻量级的U-Net风格解码器。

Result: 在镜子检测和显著目标检测等任务上，SAM3-UNet的性能优于先前的SAM2-UNet和其他最先进的方法，并且在训练时（批量大小为12）所需的GPU显存少于6 GB。

Conclusion: SAM3-UNet提供了一种有效且高效的解决方案，可以将SAM3应用于各种下游任务，同时保持较低的计算资源需求。
...
### [276] [Robust Rigid and Non-Rigid Medical Image Registration Using Learnable Edge Kernels](https://arxiv.org/abs/2512.01771)
*Ahsan Raza Siyal,Markus Haltmeier,Ruth Steiger,Malik Galijasevic,Elke Ruth Gizewski,Astrid Ellen Grams*

Main category: cs.CV

TL;DR: 该论文提出了一种将可学习边缘核与基于学习的刚性和非刚性配准技术相结合的方法，以克服传统医学图像配准的挑战，并在多项实验中表现优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 传统的医学图像配准技术在处理不同模态、时间点或受试者图像时，面临对比度差异、空间扭曲和模态特异性变化等挑战，这些问题阻碍了疾病诊断和治疗规划。

Method: 该方法将可学习边缘核与基于学习的刚性和非刚性配准技术相结合。其核心是使用预定义的边缘检测核，然后加入随机噪声进行扰动，并在训练过程中学习这些核以提取最佳边缘特征。为了深入分析各组件贡献，作者为刚性和非刚性配准各引入了四种变体模型。评估使用了医科大学提供的数据集（刚性配准带/不带颅骨移除，以及非刚性配准）和两个公开数据集。

Result: 在所有实验中，该方法持续优于最先进的技术，证明了其在多模态图像对齐和解剖结构分析方面的潜力。

Conclusion: 该方法通过引入可学习边缘核，显著提高了医学图像配准的性能，有望改进多模态图像对齐和解剖结构分析。
...
### [277] [Seeing through Imagination: Learning Scene Geometry via Implicit Spatial World Modeling](https://arxiv.org/abs/2512.01821)
*Meng Cao,Haokun Lin,Haoyuan Li,Haoran Tang,Rongtao Xu,Dong An,Xue Liu,Ian Reid,Xiaodan Liang*

Main category: cs.CV

TL;DR: 本文提出MILO范式，通过集成视觉生成器提供几何感知反馈，并引入相对位置编码RePE，以增强多模态大语言模型（MLLMs）的空间推理能力。同时构建了大规模几何感知生成数据集GeoGen。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）的空间推理能力不足，现有方法主要依赖语言描述性微调，但存在“视觉文盲”问题，即通过文本符号学习空间概念，缺乏与视觉表现的关联。

Method: 1. 引入MILO（Implicit spatIaL wOrld modeling）范式，模拟人类空间想象。2. MILO集成视觉生成器，提供几何感知反馈，将MLLM的符号推理隐式地建立在感知经验上。3. 提出RePE（Relative Positional Encoding），一种捕获相对相机姿态变换的新编码方案，优于绝对坐标系统。4. 构建GeoGen数据集，包含约2,241个视频和67,827个观察-动作-结果三元组，用于训练。

Result: 实验证明，该方法显著增强了多基线和基准测试下的空间推理能力。

Conclusion: 该方法为MLLMs提供了对3D空间更全面的理解，有效提升了其空间推理能力。
...
### [278] [Cross-Domain Validation of a Resection-Trained Self-Supervised Model on Multicentre Mesothelioma Biopsies](https://arxiv.org/abs/2512.01681)
*Farzaneh Seyedshahi,Francesca Damiola,Sylvie Lantuejoul,Ke Yuan,John Le Quesne*

Main category: cs.CV

TL;DR: 该研究展示了一种自监督编码器，通过在切除组织上训练，能够成功应用于活检样本，预测间皮瘤患者的生存期并分类肿瘤亚型。


<details>
  <summary>Details</summary>
Motivation: 间皮瘤的准确亚型分类和预后预测对于治疗和患者护理至关重要。然而，大多数计算病理模型在大型切除标本图像上训练，这限制了它们在现实世界中使用小活检样本时的应用。

Method: 研究使用了一个自监督编码器，首先在切除组织上进行训练，以捕获有意义的形态模式。然后，将该编码器应用于活检材料，利用捕获的形态模式来预测患者生存期和分类肿瘤亚型。

Result: 该模型能够成功地根据活检样本预测患者生存期，并对肿瘤亚型进行分类。

Conclusion: 这种方法证明了人工智能驱动的工具在支持间皮瘤诊断和治疗规划方面的潜力，尤其是在处理小活检样本时。
...
### [279] [Learned Image Compression for Earth Observation: Implications for Downstream Segmentation Tasks](https://arxiv.org/abs/2512.01788)
*Christian Mollière,Iker Cumplido,Marco Zeulner,Lukas Liesenhoff,Matthias Schubert,Julia Gottfriedsen*

Main category: cs.CV

TL;DR: 本文评估了针对地球观测(EO)数据的任务特定学习压缩算法，发现其在处理大规模多通道光学图像时优于传统JPEG 2000，但在小型单通道热红外数据集上传统方法仍具竞争力。联合优化并未带来性能提升。


<details>
  <summary>Details</summary>
Motivation: 卫星地球观测系统数据量快速增长，导致数据传输和存储面临巨大挑战。研究旨在通过压缩减少数据量，同时保留关键信息。

Method: 研究对比了传统压缩算法（JPEG 2000）与一种学习压缩方法（离散混合高斯似然）在三种EO分割任务（火灾、云和建筑物检测）上的表现。评估指标包括重建质量（PSNR）和分割精度。此外，还探讨了压缩和分割模型联合端到端优化的效果。

Result: 学习压缩算法在处理大规模、多通道光学图像时，无论在重建质量（PSNR）还是分割精度上都显著优于JPEG 2000。然而，对于较小、单通道的热红外数据集，传统编码器因数据量和架构限制仍具有竞争力。同时，压缩和分割模型的联合端到端优化并未比独立优化带来性能提升。

Conclusion: 任务特定学习压缩算法在处理大规模地球观测光学图像方面具有巨大潜力，能够有效减少数据量并保持信息。但对于特定类型（如小型热红外）数据，传统压缩方法仍有其优势。当前研究中，压缩与分割模型的联合优化并未显示出优于独立优化的效果。
...
### [280] [Generative Action Tell-Tales: Assessing Human Motion in Synthesized Videos](https://arxiv.org/abs/2512.01803)
*Xavier Thomas,Youngsun Lim,Ananya Srinivasan,Audrey Zheng,Deepti Ghadiyaram*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的评估指标，用于衡量生成视频中人类动作的视觉和时间正确性。该指标通过融合骨骼几何和外观特征来捕捉真实世界动作的细微差别，显著优于现有方法，并与人类感知有更强的相关性。


<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型发展迅速，但评估复杂人类动作的视觉和时间正确性的鲁棒指标仍然缺乏。现有纯视觉编码器和多模态大型语言模型（MLLMs）偏重外观，缺乏时间理解，难以辨别生成视频中复杂的运动动态和解剖学上的不合理性。

Method: 该方法引入了一种基于真实世界人类动作学习到的潜在空间的新型评估指标。它通过融合与外观无关的人体骨骼几何特征和基于外观的特征，捕捉真实世界动作的细微差别、约束和时间平滑性。这种组合特征空间被视为动作合理性的鲁棒表示。该指标通过测量生成视频的底层表示与学习到的真实世界动作分布之间的距离来量化其动作质量。同时，开发了一个新的多方面基准来探测人类动作保真度中具有时间挑战性的方面。

Result: 在作者的新基准测试中，该指标比现有最先进方法取得了超过68%的显著改进，在已建立的外部基准测试中表现出竞争力，并与人类感知有更强的相关性。深入分析揭示了当前视频生成模型的关键局限性。

Conclusion: 该指标为视频生成领域的高级研究建立了一个新标准，通过提供更鲁棒和与感知更一致的人类动作保真度评估，解决了现有模型的局限性。
...
TL;DR: 本文探讨了将SAM2模型扩展到视频语义分割（VSS）的两种主要方法，并观察到SAM2能显著提升VSS的整体性能，尤其在对象边界预测方面。


<details>
  <summary>Details</summary>
Motivation: SAM2在图像和视频对象分割方面表现出色，但将其扩展到需要空间精度、时间一致性、复杂边界和多尺度对象跟踪的密集视频语义分割（VSS）仍面临挑战。

Method: 研究探索了两种方法：1) 使用SAM2提取独特的对象掩码，并并行使用分割网络生成和优化初始预测。2) 利用SAM2预测的掩码提取独特的特征向量，然后输入一个简单网络进行分类，最后结合分类结果和掩码生成最终分割。

Result: 实验表明，利用SAM2能够提升视频语义分割的整体性能，主要得益于其对对象边界的精确预测。

Conclusion: 将SAM2应用于视频语义分割是可行的，并且其在精确预测对象边界方面的能力能够有效增强VSS的表现。
...
### [282] [CauSight: Learning to Supersense for Visual Causal Discovery](https://arxiv.org/abs/2512.01827)
*Yize Zhang,Meiqi Chen,Sirui Chen,Bo Peng,Yanxi Zhang,Tianyu Li,Chaochao Lu*

Main category: cs.CV

TL;DR: 本文引入了视觉因果发现任务，旨在让AI系统理解视觉实体间的因果关系。为此，作者构建了大型数据集VCG-32K，并开发了视觉语言模型CauSight，通过因果感知推理实现这一目标，其性能显著超越GPT-4.1。


<details>
  <summary>Details</summary>
Motivation: 人类不仅能感知事物，还能理解其发生的原因（因果思维）。为了在现代AI系统中复现这种能力，研究者希望AI能推断视觉实体间的因果关系，而不仅仅是识别它们的存在。

Method: 研究方法包括：1) 构建了包含32,000多张图像及其实体级因果图的大规模数据集VCG-32K。2) 开发了名为CauSight的新型视觉语言模型，用于视觉因果发现。3) 训练配方整合了三个组件：VCG-32K数据训练、用于合成推理轨迹的因果思维树（ToCT）、以及采用设计好的因果奖励进行强化学习以优化推理策略。

Result: 实验结果表明，CauSight在视觉因果发现任务上优于GPT-4.1，实现了超过三倍的性能提升（21%的绝对增益）。

Conclusion: 该研究成功地提出了视觉因果发现任务，构建了大规模数据集和高性能模型CauSight，使其能够进行因果感知推理，显著提升了AI系统理解视觉因果关系的能力。所有代码、模型和数据集均已开源。
...
### [283] [COACH: Collaborative Agents for Contextual Highlighting - A Multi-Agent Framework for Sports Video Analysis](https://arxiv.org/abs/2512.01853)
*Tsz-To Wong,Ching-Chun Huang,Hong-Han Shuai*

Main category: cs.CV

TL;DR: 本文提出一个可重构的多智能体系统（MAS）作为体育视频理解的基础框架，旨在克服现有端到端模型在处理时间层次结构方面的局限性，实现灵活、可扩展和可解释的跨任务体育视频智能。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端模型在处理体育视频分析中的时间层次结构时面临挑战，导致泛化能力差、新任务开发成本高且可解释性不足。

Method: 研究者提出了一个可重构的多智能体系统（MAS），其中每个智能体作为一个专门分析特定方面的“认知工具”。通过智能体的迭代调用和灵活组合，该框架能够构建适应性管道，用于短时分析推理（如回合问答）和长时生成性总结（如比赛总结），不局限于单一时间维度或任务。

Result: 该框架在羽毛球分析的两个代表性任务中展示了其适应性，成功地连接了细粒度事件检测和全局语义组织。

Conclusion: 这项工作提出了一种范式转变，旨在构建一个灵活、可扩展和可解释的系统，以实现强大且跨任务的体育视频智能。
...
### [284] [OpenREAD: Reinforced Open-Ended Reasoing for End-to-End Autonomous Driving with LLM-as-Critic](https://arxiv.org/abs/2512.01830)
*Songyan Zhang,Wenhui Huang,Zhan Chen,Chua Jiahao Collister,Qihang Huang,Chen Lv*

Main category: cs.CV

TL;DR: 现有自动驾驶两阶段微调（SFT+RFT）方法中，SFT限制了推理泛化，RFT难以量化开放式场景理解奖励。OpenREAD提出了一种基于VLM的端到端强化学习框架，通过构建CoT标注并使用大型语言模型（Qwen3）作为评论员来量化开放式推理质量，在推理和规划任务上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 知识驱动的自动驾驶范式中，两阶段微调（SFT+RFT）策略潜力巨大。然而，SFT的学习性质限制了推理的泛化能力，未能充分发挥驾驶性能。同时，当前的RFT方法主要应用于下游任务，因为场景理解是一个开放式问题，难以量化相应的奖励。

Method: 本文提出了OpenREAD，一个基于视觉-语言模型（VLM）的开放式推理强化自动驾驶（AD）框架。它实现了从高层推理到低层轨迹规划的全谱端到端强化微调（RFT）。具体来说，首先在开源驾驶相关知识数据集上构建大规模思维链（CoT）标注，并利用强大的Qwen3大型语言模型（LLM）作为RFT中的评论员，用于奖励建模时量化开放式问题的推理质量。

Result: 广泛的实验证实，联合端到端RFT在上下游任务中均取得了显著改进，使OpenREAD在推理和规划基准测试中达到了最先进的性能。

Conclusion: OpenREAD通过实现从高层推理到低层轨迹规划的端到端强化微调，并利用LLM作为评论员量化开放式推理质量，有效解决了传统SFT在推理泛化上的局限性以及RFT在开放式场景理解奖励量化上的挑战，从而在自动驾驶领域取得了最先进的性能。
...
### [285] [StyleYourSmile: Cross-Domain Face Retargeting Without Paired Multi-Style Data](https://arxiv.org/abs/2512.01895)
*Avirup Dey,Vinay Namboodiri*

Main category: cs.CV

TL;DR: StyleYourSmile是一种新颖的单次跨域人脸重定向方法，通过数据增强、双编码器和扩散模型，实现了身份、表情和领域风格的解耦控制，无需多风格配对数据，在广泛视觉领域表现出卓越的身份保留和重定向保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在跨域泛化方面表现不佳，需要测试时优化，或需要精心策划的多风格数据集进行微调，以实现领域不变的身份表示。

Method: 引入了StyleYourSmile方法，该方法采用高效的数据增强策略和双编码器框架，用于提取领域不变的身份线索和捕捉领域特定的风格变异。利用这些解耦的控制信号，条件化一个扩散模型以在不同领域间重定向面部表情。

Result: 广泛的实验证明，StyleYourSmile在广泛的视觉领域中实现了卓越的身份保留和重定向保真度。

Conclusion: StyleYourSmile成功地解决了跨域人脸重定向的挑战，消除了对精心策划的多风格配对数据的需求，并提供了有效的解耦控制，从而带来了卓越的性能。
...
### [286] [Script: Graph-Structured and Query-Conditioned Semantic Token Pruning for Multimodal Large Language Models](https://arxiv.org/abs/2512.01949)
*Zhongyu Yang,Dannong Xu,Wei Pang,Yingfang Yuan*

Main category: cs.CV

TL;DR: 本文提出了一种名为Script的即插即用剪枝方法，用于多模态大语言模型（MLLMs），旨在通过去除视觉冗余并保留与查询相关的语义信息，有效降低内存消耗和推理延迟，同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中视觉token的快速增长导致内存消耗和推理延迟过高，尤其是在处理高分辨率图像和视频时。现有token剪枝方法通常忽略用户查询相关性或受限于注意力机制，从而降低了其适应性和有效性。

Method: Script是一种无需重新训练且可泛化到多种MLLMs的即插即用剪枝方法。它包含两个模块：一个图结构剪枝模块，用于去除视觉冗余token；一个查询条件语义剪枝模块，用于保留与查询相关的视觉信息。两者协同作用以提升多模态任务性能。

Result: 在14个图像和视频理解基准测试上的实验表明，Script比现有剪枝方法持续实现更高的模型效率和预测准确性。在LLaVA-NeXT-7B上，它实现了高达6.8倍的预填充加速和10倍的FLOPs减少，同时保留了96.88%的原始性能。

Conclusion: Script通过结合图结构剪枝和查询条件语义剪枝，有效解决了多模态大语言模型在处理大量视觉token时的效率问题，显著提升了模型效率和准确性，且具有良好的通用性。
...
### [287] [PhyDetEx: Detecting and Explaining the Physical Plausibility of T2V Models](https://arxiv.org/abs/2512.01843)
...
### [288] [SARL: Spatially-Aware Self-Supervised Representation Learning for Visuo-Tactile Perception](https://arxiv.org/abs/2512.01908)
...
### [289] [SpriteHand: Real-Time Versatile Hand-Object Interaction with Autoregressive Video Generation](https://arxiv.org/abs/2512.01960)
...
### [290] [Med-VCD: Mitigating Hallucination for Medical Large Vision Language Models through Visual Contrastive Decoding](https://arxiv.org/abs/2512.01922)
...
### [291] [KM-ViPE: Online Tightly Coupled Vision-Language-Geometry Fusion for Open-Vocabulary Semantic SLAM](https://arxiv.org/abs/2512.01889)
...
### [293] [SGDiff: Scene Graph Guided Diffusion Model for Image Collaborative SegCaptioning](https://arxiv.org/abs/2512.01975)
...
### [294] [Physical ID-Transfer Attacks against Multi-Object Tracking via Adversarial Trajectory](https://arxiv.org/abs/2512.01934)
...
### [295] [PAI-Bench: A Comprehensive Benchmark For Physical AI](https://arxiv.org/abs/2512.01989)
...
### [296] [Artemis: Structured Visual Reasoning for Perception Policy Learning](https://arxiv.org/abs/2512.01988)
...
### [297] [MV-TAP: Tracking Any Point in Multi-View Videos](https://arxiv.org/abs/2512.02006)
...
### [298] [AirSim360: A Panoramic Simulation Platform within Drone View](https://arxiv.org/abs/2512.02009)
...
### [299] [Generative Video Motion Editing with 3D Point Tracks](https://arxiv.org/abs/2512.02015)
...
### [300] [Learning Visual Affordance from Audio](https://arxiv.org/abs/2512.02005)
...
### [301] [Improved Mean Flows: On the Challenges of Fastforward Generative Models](https://arxiv.org/abs/2512.02012)
...
### [302] [TUNA: Taming Unified Visual Representations for Native Unified Multimodal Models](https://arxiv.org/abs/2512.02014)
...
### [303] [Objects in Generated Videos Are Slower Than They Appear: Models Suffer Sub-Earth Gravity and Don't Know Galileo's Principle...for now](https://arxiv.org/abs/2512.02016)
...
### [304] [Text Annotation via Inductive Coding: Comparing Human Experts to LLMs in Qualitative Data Analysis](https://arxiv.org/abs/2512.00046)
*Angelina Parfenova,Andreas Marfurt,Alexander Denzler,Juergen Pfeffer*

Main category: cs.CL

TL;DR: 本研究探讨使用大型语言模型（LLMs）自动化定性数据的归纳编码，并与人类专家进行比较，发现人类在复杂句子上表现更好，LLMs在简单句子上表现更好，且人类和LLMs的标注存在系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 传统定性数据分析多依赖预定义标签的演绎方法，本研究旨在探索并自动化标签从数据中涌现的归纳编码过程，以提高效率和准确性。

Method: 本研究评估了六个开源大型语言模型在归纳编码方面的表现，并将其与人类专家进行比较。评估过程中，专家对他们编码的引文难度进行了评级。研究还通过与测试集的黄金标准进行对比，探讨了人类和LLM生成标签的系统性偏差。

Result: 结果显示，人类编码员在标记复杂句子时表现一致良好，但在简单句子上表现不佳；而LLMs则呈现相反趋势。此外，人类标注有时与黄金标准不同，但常被其他人评价更高。部分LLMs与真实标签更接近，但却获得专家较低的评价。

Conclusion: 研究揭示了LLMs在归纳编码自动化方面的潜力与挑战，尤其是在处理不同复杂度的文本时与人类表现互补。同时，人类对标注的感知和接受度与模型对“真实”标签的对齐程度存在差异，这提示未来在自动化定性分析中需要综合考虑准确性和人类偏好。
...
### [305] [Emergent Convergence in Multi-Agent LLM Annotation](https://arxiv.org/abs/2512.00047)
*Angelina Parfenova,Alexander Denzler,Juergen Pfeffer*

Main category: cs.CL

TL;DR: 本研究通过模拟多轮多智能体LLM讨论，分析了作为黑盒代理的LLM如何进行协作和协调。结果表明，LLM群体在词汇和语义上趋于一致，展现出不对称的影响模式和类似协商的行为，且其输出嵌入的内在维度会随轮次减少，表明语义压缩。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）越来越多地部署在协作环境中，但人们对它们作为黑盒代理如何协调知之甚少。

Method: 研究模拟了7500次多智能体、多轮的归纳编码任务讨论，生成了超过125000条话语。引入了过程级指标，包括代码稳定性、语义自洽性、词汇置信度、情感和收敛度，以追踪协调动态。此外，还分析了输出嵌入的演变几何结构，特别是内在维度的变化。

Result: LLM群体在词汇和语义上趋于收敛，形成了不对称的影响模式，并且在没有明确角色提示的情况下表现出类似协商的行为。输出嵌入的内在维度随轮次减少，表明存在语义压缩。

Conclusion: 这项工作展示了黑盒交互分析如何揭示LLM的涌现协调策略，为内部探针式可解释性方法提供了一种可扩展的补充方案。
...
### [306] [Tree Matching Networks for Natural Language Inference: Parameter-Efficient Semantic Understanding via Dependency Parse Trees](https://arxiv.org/abs/2512.00204)
*Jason Lunder*

Main category: cs.CL

TL;DR: 本研究通过将图匹配网络应用于依存句法树，创建了树匹配网络（TMN），并将其与基于BERT的模型在NLI任务上进行比较。结果显示，TMN在SNLI任务上以更小的内存和更短的训练时间取得了显著更好的性能，证明了显式结构表示的效率，但也指出当前聚合方法限制了可扩展性。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的模型（如BERT）在NLI任务上表现出色但参数量巨大，且需从头学习词语间的关系。研究者认为，利用依存句法树等显式语言结构可以利用预编码信息，从而提高学习效率，无需从零开始学习这些关系。

Method: 研究者将图匹配网络（GMN）应用于依存句法树，创建了树匹配网络（TMN）。他们将TMN与一个基于BERT的模型在SNLI蕴含任务和SemEval相似性任务上进行了比较。为解决可扩展性限制，他们提出了多头注意力聚合方法。

Result: 在SNLI任务上，TMN在显著减少内存占用和训练时间的情况下，取得了比基于BERT模型更好的结果。然而，两个模型在SemEval任务上的表现均不佳。结果表明，在可比规模下，显式结构表示模型显著优于基于序列的模型，但当前的聚合方法限制了其可扩展性。

Conclusion: 显式结构表示（如TMN）能够显著提高NLI任务（特别是SNLI）的学习效率和性能，且资源消耗远低于基于序列的Transformer模型。尽管当前方法在可扩展性方面存在局限，但通过引入多头注意力聚合等新机制有望解决此问题，进一步提升结构化模型的应用潜力。
...
### [307] [Towards Corpus-Grounded Agentic LLMs for Multilingual Grammatical Analysis](https://arxiv.org/abs/2512.00214)
*Matej Klemen,Tjaša Arčon,Luka Terčon,Marko Robnik-Šikonja,Kaja Dobrovoljc*

Main category: cs.CL

TL;DR: 本文探索了如何利用代理式大型语言模型（LLMs）来自动化基于语料库的语法分析，通过对标注语料库进行推理，生成可解释且数据驱动的语言学问题答案。


<details>
  <summary>Details</summary>
Motivation: 实证语法研究日益数据驱动，但对标注语料库进行系统分析仍需大量方法和技术投入。研究旨在简化这一过程。

Method: 引入了一个基于语料库的语法分析代理框架，该框架整合了自然语言任务解释、代码生成和数据驱动推理等概念。以Universal Dependencies (UD) 语料库为概念验证，针对受World Atlas of Language Structures (WALS) 启发的跨语言语法任务（涵盖13个词序特征和170多种语言）进行了测试。评估维度包括主导词序准确性、词序覆盖完整性和分布保真度。

Result: 结果表明，将LLM推理与结构化语言数据相结合是可行的，并展示了系统在泛化、识别和量化词序变异方面的表现。

Conclusion: 这项工作是迈向可解释、可扩展的语料库语法探究自动化进程的第一步。
...
### [308] [Minimal-Edit Instruction Tuning for Low-Resource Indic GEC](https://arxiv.org/abs/2512.00219)
*Akhil Rajeev P*

Main category: cs.CL

TL;DR: 本文提出了一种针对印度语言的无数据增强语法错误纠正（GEC）方法，该方法结合了指令微调的大型语言模型和保守解码策略，在马拉雅拉姆语和印地语上取得了显著成绩。


<details>
  <summary>Details</summary>
Motivation: 印度语言的语法错误纠正面临监督数据有限、文字多样以及形态丰富等挑战。

Method: 研究采用了一种无数据增强的设置：使用12B GEMMA 3模型，通过参数高效微调（PEFT）和Alpaca风格格式进行4位精度的指令微调。解码采用确定性、约束感知过程，并结合轻量级归一化器，以鼓励最小化且保留语义的编辑。推理阶段使用从确定性错误分类器（基于训练语料库的分类、标签分布和优先级）合成的固定、特定语言提示。

Result: 在官方未调优的GLEU评估中，该系统在马拉雅拉姆语上得分为92.41（总排名第六），在印地语上得分为81.44（总排名第三）。这些结果表明，分类器指导的提示设计、基于适配器的指令微调和确定性解码为印度语言GEC提供了一种可复现且计算高效的替代方案，优于以数据增强为中心的方法。

Conclusion: 该方法为印度语言的语法错误纠正提供了一种有效且高效的替代方案，并激励未来在更强的形态句法约束和保守编辑的人工评估方面进行研究。
...
### [309] [OmniFusion: Simultaneous Multilingual Multimodal Translations via Modular Fusion](https://arxiv.org/abs/2512.00234)
...
### [310] [Lost without translation -- Can transformer (language models) understand mood states?](https://arxiv.org/abs/2512.00274)
...
### [311] [EduEval: A Hierarchical Cognitive Benchmark for Evaluating Large Language Models in Chinese Education](https://arxiv.org/abs/2512.00290)
...
### [312] [Comparative Analysis of 47 Context-Based Question Answer Models Across 8 Diverse Datasets](https://arxiv.org/abs/2512.00323)
...
### [313] [Evidence-Guided Schema Normalization for Temporal Tabular Reasoning](https://arxiv.org/abs/2512.00329)
...
### [314] [Assertion-Conditioned Compliance: A Provenance-Aware Vulnerability in Multi-Turn Tool-Calling Agents](https://arxiv.org/abs/2512.00332)
*Daud Waqas,Aaryamaan Golthi,Erika Hayashida,Huanzhi Mao*

Main category: cs.CL

TL;DR: 本文提出了一种名为A-CC的新型评估范式，用于衡量多轮工具调用大型语言模型在面对用户或工具提供的误导性断言时的鲁棒性，并发现模型对此类断言高度脆弱。


<details>
  <summary>Details</summary>
Motivation: 多轮工具调用LLM在现代AI助手中至关重要，尤其在安全关键型行业。然而，尽管存在标准化基准，但仍缺乏对模型在多轮对话级别鲁棒性的可见性，特别是在面对真实世界系统时的弹性，这引发了对模型韧性的持续担忧。

Method: 引入了“断言条件合规性”（Assertion-Conditioned Compliance, A-CC）这一新颖的评估范式，用于多轮函数调用对话。A-CC通过两种不同的向量评估模型行为：(1) 用户源断言（USAs），衡量模型对貌似合理但错误的用户信念的奉承程度；(2) 函数源断言（FSAs），衡量模型对貌似合理但矛盾的系统策略（如过期工具提示）的依从性。

Result: 评估结果表明，模型对用户源断言的奉承（USA sycophancy）和函数源断言的策略冲突（FSA policy conflicts）都表现出高度脆弱性。

Conclusion: A-CC揭示了部署中的多轮工具调用代理存在一个关键的、潜在的漏洞，证实了其作为评估工具的重要性。
...
### [315] [IndicParam: Benchmark to evaluate LLMs on low-resource Indic Languages](https://arxiv.org/abs/2512.00333)
*Ayush Maheshwari,Kaushal Sharma,Vivek Patel,Aditya Maheshwari*

Main category: cs.CL

TL;DR: 研究提出了IndicParam，一个包含11种低资源印度语言及梵语-英语混合语的13,000多道多项选择题的人工策划基准，用于评估大型语言模型。结果显示即使是顶尖模型也表现不佳，平均准确率仅为45.0%。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在高资源多语言任务上表现出色，但低资源和极低资源的印度语言仍严重缺乏评估，这促使研究者创建了一个专门的基准来填补这一空白。

Method: 研究方法包括：1) 构建IndicParam数据集，包含超过13,000道多项选择题，覆盖11种印度语言（包括低资源和极低资源语言）以及梵语-英语代码混合集。2) 评估了19个专有和开源大型语言模型。3) 将问题标记为知识导向或纯语言导向，以区分事实回忆和语法能力。4) 评估了LLM处理多样化问题格式（如列表匹配、断言-理由对、序列排序）的能力。

Result: 主要结果显示，即使是表现最好的GPT-5模型，在IndicParam上的平均准确率也仅达到45.0%，其次是DeepSeek-3.2（43.1%）和Claude-4.5（42.7%），这表明当前大型语言模型在低资源印度语言上的表现仍不理想。

Conclusion: IndicParam为评估印度语言的LLM提供了一个具有挑战性的基准，并揭示了跨语言迁移能力的局限性，强调了当前模型在处理低资源语言方面的不足。
...
### [316] [CourseTimeQA: A Lecture-Video Benchmark and a Latency-Constrained Cross-Modal Fusion Method for Timestamped QA](https://arxiv.org/abs/2512.00360)
*Vsevolod Kovalev,Parteek Kumar*

Main category: cs.CL

TL;DR: 本文提出并评估了CrossFusion-RAG，一个轻量级、低延迟的跨模态检索器，用于在教育视频上进行时间戳问答，并在CourseTimeQA数据集上展示了其优越的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 在单个GPU的延迟和内存预算限制下，对教育讲座视频进行时间戳问答是一个挑战，需要系统能够检索相关的时间戳段并生成基于事实的答案。

Method: 研究构建了CourseTimeQA数据集（52.3小时，902个查询），并提出了CrossFusion-RAG检索器。该方法结合了冻结编码器、学习型512->768视觉投影、基于ASR和视频帧的浅层查询无关交叉注意力（带有时间一致性正则化器），以及一个小型交叉注意力重排器。系统在匹配的硬件和索引条件下与多种现有方法进行了比较，并评估了ASR噪声鲁棒性和时间定位诊断。

Result: 在CourseTimeQA数据集上，CrossFusion-RAG在nDCG@10上比强大的BLIP-2检索器提高了0.10，MRR提高了0.08，同时在单个A100 GPU上实现了约1.55秒的端到端中位数延迟。研究还报告了在不同ASR噪声（WER四分位数）下的鲁棒性，并提供了时间定位诊断。

Conclusion: CrossFusion-RAG是一种有效且高效的解决方案，用于在教育讲座视频上进行受延迟和内存限制的时间戳问答，显著优于现有基线方法，并具有良好的鲁棒性。
...
### [317] [Mitigating the Threshold Priming Effect in Large Language Model-Based Relevance Judgments via Personality Infusing](https://arxiv.org/abs/2512.00390)
*Nuo Chen,Hanpei Fang,Jiqun Liu,Wilson Wei,Tetsuya Sakai,Xiao-Ming Wu*

Main category: cs.CL

TL;DR: 本研究探讨了LLM在相关性标注中存在的启动效应问题，并发现通过模拟大五人格特质，特别是高开放性和低神经质，可以有效降低LLM对启动效应的敏感性。文章提出了人格提示作为缓解该效应的方法。


<details>
  <summary>Details</summary>
Motivation: LLM作为可扩展的相关性标注工具，易受启动效应影响，即先前的判断会影响后续判断。虽然心理学理论将人格特质与此类偏见联系起来，但目前尚不清楚LLM中模拟的人格是否会表现出类似的影响。

Method: 研究人员利用多个LLM，并在TREC 2021和2022深度学习赛道数据集上，调查了LLM中模拟的大五人格（Big Five personality profiles）如何影响相关性标注中的启动效应。

Result: 结果显示，某些人格特质组合，如高开放性和低神经质，能持续降低LLM对启动效应的敏感性。此外，最有效的缓解启动效应的人格特质可能因模型和任务类型而异。

Conclusion: 基于研究发现，论文提出了“人格提示”（personality prompting）作为缓解阈值启动效应的方法，将心理学证据与基于LLM的评估实践相结合。
...
### [318] [CryptoBench: A Dynamic Benchmark for Expert-Level Evaluation of LLM Agents in Cryptocurrency](https://arxiv.org/abs/2512.00417)
...
TL;DR: 本文介绍了CryptoBench，一个专家策划的动态基准测试，旨在严格评估大型语言模型（LLM）代理在加密货币领域中的实际能力，并揭示了LLM代理在检索和预测任务之间的性能不平衡。


<details>
  <summary>Details</summary>
Motivation: 现有的通用代理基准测试无法应对加密货币领域特有的挑战，如极端时间敏感性、高度对抗性的信息环境以及需要从多样化、专业来源（如链上智能平台和实时DeFi仪表板）综合数据的关键需求。因此，需要一个更具挑战性和价值的场景来评估LLM代理。

Method: 研究构建了CryptoBench，一个实时的动态基准测试，每月包含50个由加密原生专业人士设计的、反映实际分析师工作流程的问题。这些任务被严格分为四个象限：简单检索、复杂检索、简单预测和复杂预测。研究评估了十个LLM，包括直接评估和在代理框架内的评估。

Result: 评估揭示了性能等级，并发现了一种“检索-预测不平衡”的失败模式。许多领先模型尽管在数据检索方面表现出色，但在需要预测分析的任务中显示出明显的弱点。这表明代理模型存在一种问题倾向，即它们可能看起来基于事实，但缺乏更深层次的信息综合分析能力。

Conclusion: LLM代理在加密货币领域的表现显示出，尽管它们在数据检索方面表现良好，但缺乏进行深入分析和预测的关键能力。它们可能表面上看起来有事实依据，但实际上缺乏深层分析和信息综合的能力。
...
### [319] [A Taxonomy of Errors in English as she is spoke: Toward an AI-Based Method of Error Analysis for EFL Writing Instruction](https://arxiv.org/abs/2512.00392)
*Damian Heywood,Joseph Andrew Carrier,Kyu-Hong Hwang*

Main category: cs.CL

TL;DR: 本研究开发了一个基于大型语言模型（如Claude 3.5 Sonnet和DeepSeek R1）的AI辅助错误分析系统，旨在识别、分类和纠正英语写作错误。该系统利用语言学理论构建的详细分类法，并在包含真实语言错误的文本上进行了测试，展示了其潜力，但也揭示了在语境理解和处理未编码错误方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统的基于评分标准的评估缺乏细致的反馈。研究旨在开发一个能够自动化详细错误分析并提供更精细反馈的系统，以改进英语作为外语（EFL）教学。

Method: 该系统利用Claude 3.5 Sonnet和DeepSeek R1等大型语言模型。它采用基于Corder (1967)、Richards (1971)和James (1998)语言学理论的详细错误分类法，将错误分类到单词和句子级别，涵盖拼写、语法和标点。系统通过Python API调用实现，并首先通过孤立错误测试来完善分类法。最终测试使用Jose da Fonseca (1855)的《English as she is spoke》文本，该文本富含真实的语言错误，以评估系统处理复杂多层分析的能力。

Result: AI系统成功识别了多种错误类型，但表现出在语境理解方面的局限性，并且在遇到未编码错误时偶尔会生成新的错误类别。

Conclusion: 这项研究展示了AI通过自动化详细错误分析和反馈来改变EFL教学的潜力。尽管前景广阔，但仍需要进一步开发以提高语境准确性，并扩展分类法以涵盖文体和语篇层面的错误。
...
### [320] [CACARA: Cross-Modal Alignment Leveraging a Text-Centric Approach for Cost-Effective Multimodal and Multilingual Learning](https://arxiv.org/abs/2512.00496)
...
TL;DR: 本文提出CACARA架构，通过涌现对齐学习实现多模态和多语言能力，无需对现有模型进行完全重训练即可高效整合新模态和支持100多种语言，同时保持与单语言模型相当的训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在扩展到新模态或新语言时，通常需要资源密集型的全面重训练，这限制了其应用和效率。

Method: 本文提出CACARA架构，采用涌现对齐学习（emergent alignment learning）范式。该方法允许将新模态集成到现有双模态/多模态模型中，而无需完全重训练。通过仅使用与英语对齐的数据对新引入的模态进行微调，模型能够从单语言训练中解锁多语言能力。

Result: CACARA模型在没有显式多语言预训练或文本编码器调优的情况下，支持超过100种语言。在音频到文本检索的R@1指标上，实现了高达14.24个百分点的提升，超越了现有最先进的多模态模型，且训练成本与单语言模型相当。

Conclusion: 涌现对齐学习范式能够以高效的方式实现多模态和多语言能力的扩展，无需昂贵的全面重训练，同时保留了先前学习的知识并显著提升了性能。
...
### [321] [SCALE: Selective Resource Allocation for Overcoming Performance Bottlenecks in Mathematical Test-time Scaling](https://arxiv.org/abs/2512.00466)
*Yang Xiao,Chunpu Xu,Ruifeng Yuan,Jiashuo Wang,Wenjie Li,Pengfei Liu*

Main category: cs.CL

TL;DR: 该论文提出了SCALE框架，通过选择性地分配计算资源来增强大型语言模型（LLM）的数学推理能力，解决了现有测试时计算扩展方法中资源统一分配导致的效率瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的测试时计算扩展方法对所有推理子问题均匀分配资源，导致难题得不到足够关注，而常规操作却消耗过多资源，造成性能瓶颈和计算资源利用率低下。

Method: SCALE（选择性资源分配）框架受双加工理论启发，包含四个阶段：1) 将问题分解为顺序推理子问题；2) 评估每个子问题的难度；3) 根据难度分配处理模式（System 1处理简单子问题，System 2处理复杂子问题）；4) 带有上下文传播的顺序执行。

Result: 实验结果表明，SCALE显著优于统一扩展基线，在AIME25数据集上实现了高达13.75个百分点（从57.50%到71.25%）的准确率提升，同时计算成本降低了33%至53%。

Conclusion: SCALE通过将计算资源集中于具有挑战性的子问题，同时高效处理常规操作，实现了显著的性能改进和更优的资源利用率，是测试时扩展领域的一项重大进展，解决了现有方法的根本局限性。
...
### [322] [G-KV: Decoding-Time KV Cache Eviction with Global Attention](https://arxiv.org/abs/2512.00504)
...
### [323] [Developing a Comprehensive Framework for Sentiment Analysis in Turkish](https://arxiv.org/abs/2512.00515)
*Cem Rifki Aydin*
...
### [324] [Catch Me If You Can: How Smaller Reasoning Models Pretend to Reason with Mathematical Fidelity](https://arxiv.org/abs/2512.00552)
...
### [325] [Slovak Conceptual Dictionary](https://arxiv.org/abs/2512.00579)
*Miroslav Blšták*
...
### [326] [Wikontic: Constructing Wikidata-Aligned, Ontology-Aware Knowledge Graphs with Large Language Models](https://arxiv.org/abs/2512.00590)
...
### [327] [Prism: A Minimal Compositional Metalanguage for Specifying Agent Behavior](https://arxiv.org/abs/2512.00611)
...
### [328] [ART: Adaptive Response Tuning Framework -- A Multi-Agent Tournament-Based Approach to LLM Response Optimization](https://arxiv.org/abs/2512.00617)
*Omer Jauhar Khan*
...
### [329] [Sycophancy Claims about Language Models: The Missing Human-in-the-Loop](https://arxiv.org/abs/2512.00656)
*Jan Batzner,Volker Stocker,Stefan Schmid,Gjergji Kasneci*

Main category: cs.CL

TL;DR: 本文综述了大型语言模型(LLM)谄媚行为测量的方法学挑战，确定了五种核心操作定义，并指出当前研究缺乏对人类感知的评估。文章还强调了区分谄媚与其他AI对齐概念的困难，并提出了未来研究建议。


<details>
  <summary>Details</summary>
Motivation: 文献中越来越多地声称大型语言模型(LLM)存在谄媚响应模式，这促使作者对测量方法进行审查。

Method: 本文通过文献回顾和分析的方法，审视了测量LLM谄媚行为的方法学挑战，并识别了五种核心操作定义。此外，作者还分析了将谄媚响应与AI对齐中的相关概念区分开的困难。

Result: 研究结果包括：测量LLM谄媚行为存在方法学挑战；识别出五种核心操作定义；当前研究未能评估人类对LLM谄媚行为的感知；将谄媚响应与AI对齐中的相关概念区分开存在困难。

Conclusion: LLM谄媚行为的测量存在方法学挑战，且当前研究忽视了人类感知。文章提出了可操作的建议，以指导未来研究，特别是在区分谄媚与相关概念方面。
...
### [330] [Graphing the Truth: Structured Visualizations for Automated Hallucination Detection in LLMs](https://arxiv.org/abs/2512.00663)
*Tanmay Agrawal*

Main category: cs.CL

TL;DR: 本文提出一个框架，将企业专有知识和大模型生成内容组织成交互式可视化知识图谱，通过链接模型断言至信息来源并显示置信度，帮助用户直观识别大模型幻觉，并形成人工反馈循环以提升模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在自然语言处理方面进展迅速，但在企业环境中结合闭源领域知识时，由于上下文窗口限制和预训练数据与专有知识的不一致，常导致幻觉。当前缓解策略依赖昂贵的金标准问答集或二次模型验证，均无法提供确定性保证，且高可信幻觉易逃过人工审查。

Method: 本文引入一个框架，将专有知识和模型生成内容组织成交互式可视化知识图谱。该框架通过将模型断言链接到底层事实来源并显示置信水平，为最终用户提供潜在幻觉区域的清晰直观视图。

Result: 通过这种可视化界面，用户可以诊断不一致性、识别薄弱的推理链并提供纠正反馈。由此产生的人机协作工作流创建了一个结构化的反馈循环。

Conclusion: 所提出的人机协作工作流能够增强模型可靠性并持续改进响应质量，从而有效缓解大型语言模型在企业应用中的幻觉问题。
...
### [331] [A Comparison of Human and ChatGPT Classification Performance on Complex Social Media Data](https://arxiv.org/abs/2512.00673)
*Breanna E. Green,Ashley L. Shea,Pengfei Zhao,Drew B. Margolin*

Main category: cs.CL

TL;DR: 本研究评估了GPT-4在分类和标注包含细微差别语言的数据集上的表现，并与人类标注者进行比较。结果表明，GPT-4在处理细微差别语言时存在困难，建议谨慎使用。


<details>
  <summary>Details</summary>
Motivation: 计算社会科学家越来越多地使用生成式人工智能工具（如ChatGPT），但对于其在分类和标注包含细微差别语言的复杂任务中的表现，仍缺乏深入理解。

Method: 研究测量了GPT-4在细微差别语言分类任务上的表现，并将其与人类标注者进行比较。使用了ChatGPT 3.5、4和4o版本，并设计了四种提示风格作为输入。通过精确率、召回率和F1分数进行量化评估，并进行了定性分析。

Result: 定量和定性评估均表明，虽然在提示中包含标签定义可能有助于提高性能，但总体而言，GPT-4在分类细微差别语言方面存在困难。定性分析揭示了四个具体发现。

Conclusion: 研究结果建议，在涉及细微差别语言的分类任务中使用ChatGPT时应谨慎行事。
...
### [332] [FastPOS: Language-Agnostic Scalable POS Tagging Framework Low-Resource Use Case](https://arxiv.org/abs/2512.00745)
*Md Abdullah Al Kafi,Sumit Kumar Banshal*

Main category: cs.CL

TL;DR: 本研究提出一个语言无关的、基于Transformer的词性标注框架，专为低资源语言设计，在孟加拉语和印地语上实现了高精度和高效的跨语言适应性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了解决低资源语言在自然语言处理（NLP）方面的挑战，特别是词性标注，并通过减少模型设计和调整的开销，促进研究人员专注于语言预处理和数据集优化。

Method: 本研究采用一个语言无关的、基于Transformer的词性标注框架。通过仅三行代码的修改，该模型便可从孟加拉语适应到印地语。其设计模块化且开源，允许底层Transformer架构的替换和快速的跨语言适应。

Result: 该框架在孟加拉语和印地语上分别达到了96.85%和97%的词元级准确率，并在数据集不平衡和语言重叠的情况下保持了强大的F1分数。研究还指出，特定词性类别存在性能差异，这凸显了数据集整理的持续挑战。

Conclusion: 该框架为低资源语言的词性标注提供了有效解决方案，能够实现快速的跨语言适应，并显著减少模型设计和调整的开销。这使得研究人员能够将更多精力投入到对推进欠代表语言NLP至关重要的语言预处理和数据集精炼上。
...
### [333] [Accelerating Bangla NLP Tasks with Automatic Mixed Precision: Resource-Efficient Training Preserving Model Efficacy](https://arxiv.org/abs/2512.00829)
...
### [334] [Auxiliary-Hyperparameter-Free Sampling: Entropy Equilibrium for Text Generation](https://arxiv.org/abs/2512.00789)
...
### [335] [WaterSearch: A Quality-Aware Search-based Watermarking Framework for Large Language Models](https://arxiv.org/abs/2512.00837)
...
### [336] [Less is More: Resource-Efficient Low-Rank Adaptation](https://arxiv.org/abs/2512.00878)
...
### [337] [Reward Auditor: Inference on Reward Modeling Suitability in Real-World Perturbed Scenarios](https://arxiv.org/abs/2512.00920)
...
### [338] [Mitigating Hallucinations in Zero-Shot Scientific Summarisation: A Pilot Study](https://arxiv.org/abs/2512.00931)
...
### [339] [DeformAr: Rethinking NER Evaluation through Component Analysis and Visual Analytics](https://arxiv.org/abs/2512.00938)
...
### [340] [Fine-tuning of lightweight large language models for sentiment classification on heterogeneous financial textual data](https://arxiv.org/abs/2512.00946)
...
### [341] [Table as a Modality for Large Language Models](https://arxiv.org/abs/2512.00947)
...
TL;DR: 现有大型语言模型（LLMs）在处理表格数据时，因序列化导致结构信息丢失而表现不佳。本文提出TAMO，一个将表格视为独立模态的多模态框架，通过超图神经网络编码表格并与LLM集成，显著提升了表格推理任务的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs取得了巨大成功并被尝试应用于表格推理任务，但通过探究性实验发现，即使是最先进的LLMs在处理表格数据时仍存在不足。这主要是因为当前方案仅将表格序列化输入LLMs，导致表格固有的结构信息丢失，从而限制了LLMs的性能。

Method: 本文提出了TAMO框架，其核心思想是将表格视为一种独立的模态与文本进行整合。TAMO包含一个超图神经网络作为全局表格编码器，该编码器能够无缝集成到主流的LLM中，从而形成一个多模态模型。

Result: TAMO在多个基准数据集（包括HiTab、WikiTQ、WikiSQL、FeTaQA和StructQA）上取得了显著的泛化性能提升，平均相对增益达到42.65%。

Conclusion: 简单的表格序列化会导致结构信息丢失，是LLMs在表格推理任务中表现不佳的根本原因。TAMO通过将表格作为独立模态，并利用超图神经网络进行编码，有效解决了这一问题，显著提高了LLMs在表格数据处理上的泛化能力。
...
### [342] [Dr.Mi-Bench: A Modular-integrated Benchmark for Scientific Deep Research Agent](https://arxiv.org/abs/2512.00986)
*Zhihan Guo,Feiyang Xu,Yifan Li,Muzhi Li,Shuai Zou,Jiele Wu,Han Shi,Haoli Bai,Ho-fung Leung,Irwin King*

Main category: cs.CL

TL;DR: 本文提出Dr.Mi-Bench，一个针对科学深度研究(DR)代理的模块化集成基准，以及Dr.Mi-Eval评估范式。研究发现现有代理在多源检索和跨领域一致性方面存在显著弱点，并强调高层规划对解锁大型语言模型(LLM)推理潜力的关键作用。


<details>
  <summary>Details</summary>
Motivation: 学术文献爆炸式增长，急需自动化深度研究(DR)代理。然而，现有评估基准存在局限：它们过于狭隘（侧重检索，忽略高层规划和推理）且偏向通用领域而非DR代理的核心应用——科学领域。

Method: 本文引入Dr.Mi-Bench，一个基于学术文献的模块化集成基准，包含200个人工标注实例，涵盖10个科学领域的研究和综述论文。同时，提出了Dr.Mi-Eval，一种新颖的模块化集成评估范式，利用学术论文的丰富结构，通过端到端评估DR代理和独立评估基础LLM（作为潜在骨干），来评估规划、检索和推理的核心能力。

Result: 实验结果显示代理性能表现碎片化：它们展现出专业优势，但也存在关键弱点，尤其是在执行综述式任务所需的多源检索以及在不同科学领域保持一致性方面。此外，提高高层规划能力是释放基础LLM作为骨干的推理潜力的关键因素。

Conclusion: 通过揭示这些可操作的失败模式，Dr.Mi-Bench提供了一个诊断工具，旨在指导开发更可靠的学术研究助手。
...
### [343] [When Safety Blocks Sense: Measuring Semantic Confusion in LLM Refusals](https://arxiv.org/abs/2512.01037)
*Riad Ahmed Anonto,Md Labid Al Nahiyan,Md Tanvir Hassan,Ch. Md. Rakin Haider*

Main category: cs.CL

TL;DR: 本文引入“语义混淆”来衡量安全对齐语言模型在处理无害提示时，因表面形式变化而导致的局部不一致拒绝现象。通过构建一个包含意图固定但表达形式多样的复述语料库ParaGuard，并提出三种基于token的模型无关指标，揭示了全局拒绝率所掩盖的关键结构性问题，为开发者提供了在保持安全性的同时减少错误拒绝的实用信号。


<details>
  <summary>Details</summary>
Motivation: 安全对齐的语言模型常拒绝实际无害的提示。现有评估多报告全局性的错误拒绝率或依从率，但这些指标将每个提示独立处理，忽略了局部不一致性——即模型接受某种表达方式但拒绝其近义复述的情况。这种不足限制了对模型行为的诊断和调整。

Method: 本文引入了一种名为“语义混淆”的故障模式来捕捉局部不一致性，并建立了一个衡量框架。构建了ParaGuard，一个包含1万个提示的受控复述簇语料库，其中意图固定但表面形式各异。提出了三种与模型无关的、基于token的指标：混淆指数（Confusion Index）、混淆率（Confusion Rate）和混淆深度（Confusion Depth）。这些指标通过比较每次拒绝与其最近的已接受邻居，并利用token嵌入、下一token概率和困惑度信号进行计算。

Result: 对不同模型家族和部署防护措施的实验表明，全局错误拒绝率掩盖了关键结构。本文提出的指标揭示了在某些设置中全局不稳定的边界，在另一些设置中局部的非一致性区域，以及在某些情况下更严格的拒绝并未增加不一致性。研究还表明，混淆感知审计能够区分系统拒绝的频率和拒绝的合理性。

Conclusion: 本文为开发者提供了一个实用的信号，以在保持安全性的同时减少错误的拒绝，通过诊断和理解语言模型在处理语义相近提示时的局部不一致性。
...
### [344] [How do we measure privacy in text? A survey of text anonymization metrics](https://arxiv.org/abs/2512.01109)
*Yaxuan Ren,Krithika Ramesh,Yaxing Yao,Anjalie Field*

Main category: cs.CL

TL;DR: 本文通过系统性调查，澄清并调和了文本隐私保护的评估指标。研究审查了47篇论文，识别了六种隐私概念及其指标，并评估了它们与法律标准和用户期望的对齐程度，旨在促进更健壮、可比且符合法律的隐私评估。


<details>
  <summary>Details</summary>
Motivation: 文本匿名化对于敏感数据领域的NLP研究和模型开发至关重要，但评估匿名化方法是否充分保护隐私仍然是一个开放的挑战。

Method: 通过手动审查47篇报告隐私指标的论文，识别并比较了六种不同的隐私概念。分析了相关指标如何捕捉隐私风险的不同方面。评估了这些概念与法律隐私标准（HIPAA和GDPR）以及基于HCI研究的用户中心期望的对齐程度。

Result: 识别并比较了六种不同的隐私概念及其相关指标，分析了它们如何捕捉隐私风险。评估了这些概念与HIPAA、GDPR等法律标准以及用户期望的对齐情况。提供了关于隐私评估方法的实用指导，并指出了当前实践中的不足。

Conclusion: 本研究旨在促进文本匿名化中更健壮、可比且符合法律意识的隐私评估，并突出了现有实践中的差距。
...
### [345] [ELR-1000: A Community-Generated Dataset for Endangered Indic Indigenous Languages](https://arxiv.org/abs/2512.01077)
...
TL;DR: 本文提出了一个包含1060个印度东部传统食谱的多模态数据集ELR-1000，涵盖10种濒危语言。研究评估了大型语言模型（LLMs）在这些低资源、文化特定语言翻译上的表现，发现模型虽有能力但仍面临挑战，但提供目标语境能显著提高翻译质量。


<details>
  <summary>Details</summary>
Motivation: 研究旨在收集并保存濒危语言中的传统烹饪知识和文化背景，同时评估当前LLMs在处理低资源和文化特定语言时的能力与局限性，并推动开发更公平、文化感知的语言技术。

Method: 研究通过为数字素养较低的贡献者设计的移动界面，从印度东部农村社区众包收集了1060个传统食谱。随后，使用多种最先进的大型语言模型将这些食谱翻译成英语，并评估其性能。为了改善翻译质量，研究尝试提供目标语境，包括语言背景信息、翻译示例和文化保存指南。

Result: 尽管LLMs具有先进能力，但在翻译低资源、文化特定语言时表现不佳。然而，研究发现提供目标语境（如语言背景、翻译示例和文化保存指南）能够显著提高翻译质量。这表明现有模型在处理此类数据时存在局限性，但可以通过上下文信息有效弥补。

Conclusion: 研究强调了为代表性不足的语言和领域创建基准测试的必要性，以促进公平和文化感知的语言技术发展。ELR-1000数据集已向NLP社区发布，旨在激励濒危语言技术的发展。
...
### [346] [Advancing Academic Chatbots: Evaluation of Non Traditional Outputs](https://arxiv.org/abs/2512.00991)
*Nicole Favero,Francesca Salute,Daniel Hardt*

Main category: cs.CL

TL;DR: 本研究扩展了LLM评估范围，比较了两种RAG策略在问答中的表现，并评估了LLM生成幻灯片和播客脚本等非传统学术输出的能力。结果显示GPT 4o mini结合Advanced RAG在问答中表现最佳，且人类评估对于检测生成内容的缺陷至关重要。


<details>
  <summary>Details</summary>
Motivation: 大多数LLM评估侧重于标准任务（如事实问答或短摘要）。本研究旨在将评估范围扩展到：1) 比较不同检索增强生成（RAG）策略在问答中的效果；2) 评估LLM生成非传统学术输出（如幻灯片和播客脚本）的质量。

Method: 研究比较了两种检索策略：基于知识图谱的Graph RAG和混合关键词-语义搜索的Advanced RAG，用于问答。同时，评估了LLM（Meta LLaMA 3 70B和OpenAI GPT 4o mini）生成幻灯片和播客脚本的能力。问答性能通过人类评分（11个质量维度）和LLM评判进行评估。幻灯片和播客生成则通过文档检索和人类审阅进行测试。

Result: GPT 4o mini结合Advanced RAG在问答中产生了最准确的回答。Graph RAG改进有限，并导致更多幻觉。对于幻灯片和播客生成，GPT 4o mini表现最佳，尽管LLaMA 3在叙事连贯性方面显示出潜力。人类审阅者在检测布局和风格缺陷方面至关重要。

Conclusion: GPT 4o mini与Advanced RAG结合在问答任务中表现出色，而Graph RAG因其复杂性表现不佳。LLM能够生成高质量的非传统学术输出，但人类审阅在评估这些新兴输出的布局和风格缺陷方面不可或缺，强调了人机结合评估的重要性。
...
### [347] [DrawingBench: Evaluating Spatial Reasoning and UI Interaction Capabilities of Large Language Models through Mouse-Based Drawing Tasks](https://arxiv.org/abs/2512.01174)
*Hyunjun Kim,Sooyoung Ryu*

Main category: cs.CL

TL;DR: 本文提出了DrawingBench，一个用于通过空间推理GUI任务评估智能体LLM可信度的验证框架。该框架提供透明、可审计的评估，揭示了模型能力和局限性，并强调了外部监督对指导智能体行为的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI系统日益自主运行，通过可验证的评估建立信任变得至关重要。然而，现有基准缺乏评估智能体行为可靠性所需的透明度和可审计性。

Method: 我们提出了DrawingBench框架，用于通过生成低级GUI操作序列的空间推理任务来评估智能体LLM。该框架提供基于规则的透明评估，包括8个客观标准和动作级检查。它包含250个多样化提示（20个类别，4个难度级别）、确定性评估指标以及通过多轮反馈实现人类控制的外部监督机制。我们使用该框架对四种最先进的LLM（Claude-4 Sonnet, GPT-4.1, GPT-4.1-mini, Gemini-2.5 Flash）进行了1,000次测试。

Result: 模型在有结构化外部反馈的情况下取得了92.8%的完美表现，外部反馈显著提升了性能（平均+3.2%，复杂场景最高+32.8%）。然而，在工具状态管理和长程规划中出现了系统性错误模式。值得注意的是，规范的清晰度比任务复杂性更重要——当给出明确、可验证的标准时，模型实现了100%的完美表现。

Conclusion: 透明的评估框架能够建立对智能体系统的信任，且外部监督在指导智能体行为方面比自我纠正更可靠。我们开源的DrawingBench框架为可信智能体评估提供了一个模板。
...
### [348] [TempPerturb-Eval: On the Joint Effects of Internal Temperature and External Perturbations in RAG Robustness](https://arxiv.org/abs/2512.01183)
...
### [349] [Sentiment Analysis and Emotion Classification using Machine Learning Techniques for Nagamese Language - A Low-resource Language](https://arxiv.org/abs/2512.01256)
*Ekha Morang,Surhoni A. Ngullie,Sashienla Longkumer,Teisovi Angami*

Main category: cs.CL

TL;DR: 本文首次尝试对纳加米语（Nagamese）进行情感分析和情感分类，通过构建词典并结合机器学习方法实现。


<details>
  <summary>Details</summary>
Motivation: 纳加米语是一种资源匮乏的语言，此前尚未有针对其进行情感分析的研究。鉴于情感分析在英语、印地语等资源丰富语言中已取得大量进展，本研究旨在填补纳加米语在该领域的空白，实现对其文本内容的情感极性（积极、消极、中性）和基本情绪的检测。

Method: 研究人员构建了一个包含1,195个纳加米语单词的情感极性词典。他们将这些词典作为特征，并结合额外特征，使用朴素贝叶斯（Naïve Bayes）和支持向量机（Support Vector Machines）等监督机器学习技术进行情感分析和情感分类。

Result: 本文是首次对纳加米语进行情感分析和情感分类的尝试，成功构建了纳加米语情感极性词典，并将其应用于监督机器学习模型，以检测文本的情感极性和基本情绪。

Conclusion: 该研究成功地为纳加米语的情感分析和情感分类奠定了基础，证明了通过构建特定语言词典并结合机器学习方法，可以有效处理低资源语言的情感识别问题。
...
### [350] [Conveying Imagistic Thinking in Traditional Chinese Medicine Translation: A Prompt Engineering and LLM-Based Evaluation Framework](https://arxiv.org/abs/2512.01198)
*Jiatong Han*

Main category: cs.CL

TL;DR: 本研究采用人机协作（HITL）框架，通过提示词引导大语言模型（LLM）识别并传达中医经典中的隐喻和转喻，显著提升了中医古籍的翻译质量，为概念密集型文本的翻译提供了一种认知、高效且可复制的方法。


<details>
  <summary>Details</summary>
Motivation: 中医理论建立在意象思维之上，其原理和诊疗逻辑通过隐喻和转喻构建。然而，现有英文译文多采用直译，导致目标语读者难以重建底层概念网络并应用于临床实践。

Method: 研究采用人机协作（HITL）框架，选取《黄帝内经》中四个基础理论段落。通过基于提示词的认知支架，引导DeepSeek V3.1识别原文中的隐喻和转喻，并在翻译中传达其理论。在评估阶段，ChatGPT 5 Pro和Gemini 2.5 Pro被提示模拟三类真实读者，对人工翻译、基线模型翻译和提示词调整后的翻译进行五维认知评分。随后进行结构化访谈和解释性现象学分析（IPA）。

Result: 结果显示，经过提示词调整的LLM翻译在所有五个维度上表现最佳，且跨模型和跨角色的一致性很高。访谈主题揭示了人工翻译与机器翻译之间的差异、隐喻和转喻转移的有效策略以及读者的认知偏好。

Conclusion: 本研究为中医等古老、概念密集型文本的翻译提供了一种认知、高效且可复制的人机协作（HITL）方法路径。
...
### [351] [Generalist Large Language Models Outperform Clinical Tools on Medical Benchmarks](https://arxiv.org/abs/2512.01191)
...
### [352] [SUPERChem: A Multimodal Reasoning Benchmark in Chemistry](https://arxiv.org/abs/2512.01274)
...
### [353] [Kardia-R1: Unleashing LLMs to Reason toward Understanding and Empathy for Emotional Support via Rubric-as-Judge Reinforcement Learning](https://arxiv.org/abs/2512.01282)
...
### [354] [MARSAD: A Multi-Functional Tool for Real-Time Social Media Analysis](https://arxiv.org/abs/2512.01369)
...
### [355] [Agreement-Constrained Probabilistic Minimum Bayes Risk Decoding](https://arxiv.org/abs/2512.01316)
...
### [356] [DyFuLM: An Advanced Multimodal Framework for Sentiment Analysis](https://arxiv.org/abs/2512.01410)
*Ruohan Zhou,Jiachen Yuan,Churui Yang,Wenzheng Huang,Guoyan Zhang,Shiyao Wei,Jiazhen Hu,Ning Xin,Md Maruf Hasan*

Main category: cs.CL

TL;DR: 该论文提出了一个动态融合学习模型（DyFuLM），用于理解复杂文本表达中的情感，该模型通过分层动态融合和门控特征聚合模块，在多任务情感数据集上实现了高精度和低误差，有效提升了情感表示和整体性能。


<details>
  <summary>Details</summary>
Motivation: 理解复杂文本表达中的情感仍然是情感计算领域的一个基本挑战。

Method: 本文提出了一种多模态框架——动态融合学习模型（DyFuLM）。DyFuLM包含两个关键模块：一个分层动态融合模块，用于自适应地整合多级特征；一个门控特征聚合模块，用于调节跨层信息流以实现平衡的表示学习。研究在多任务情感数据集上进行了综合实验，并进行了消融研究。

Result: DyFuLM在粗粒度情感分类上达到82.64%的准确率，在细粒度情感分类上达到68.48%的准确率，回归误差最低（MAE = 0.0674，MSE = 0.0082），R^2系数最高（R^2 = 0.6903）。消融研究证实了DyFuLM中每个模块的有效性：移除所有模块导致粗/细粒度准确率分别下降0.91%/0.68%；仅保留门控融合模块导致分别下降0.75%/0.55%；移除动态损失机制导致分别下降0.78%/0.26%。

Conclusion: DyFuLM通过有效的分层特征融合，增强了情感表示和整体性能。实验结果进一步验证了模型中每个模块对特征交互和任务平衡的显著贡献。
...
### [357] [Multilingual Conversational AI for Financial Assistance: Bridging Language Barriers in Indian FinTech](https://arxiv.org/abs/2512.01439)
*Bharatdeep Hazarika,Arya Suneesh,Prasanna Devadiga,Pawan Kumar Rajpoot,Anshuman B Suresh,Ahmed Ifthaquar Hussain*

Main category: cs.CL

TL;DR: 本文提出并部署了一个多语言对话式AI系统，支持印度的混合语（如印地语+英语），以解决金融科技平台在语言多样性方面的挑战，从而提高金融普惠性。


<details>
  <summary>Details</summary>
Motivation: 印度语言多样性导致仅10%人口懂英语，这在金融科技平台中造成了金融排斥的障碍。研究旨在通过支持混合语言的自然交互来弥合这一语言鸿沟。

Method: 该系统采用多智能体架构，包括语言分类、功能管理和多语言响应生成模块。通过比较分析多种语言模型并进行实际部署。

Result: 研究表明，该系统显著提升了用户参与度，同时保持了较低的延迟开销（4-8%）。

Conclusion: 这项工作有助于弥合新兴市场数字金融服务中的语言差距，促进金融普惠。
...
### [358] [MEGConformer: Conformer-Based MEG Decoder for Robust Speech and Phoneme Classification](https://arxiv.org/abs/2512.01443)
*Xabier de Zuazo,Ibon Saratxaga,Eva Navas*

Main category: cs.CL

TL;DR: 本文为LibriBrain 2025 PNPL竞赛提出了基于Conformer的解码器，用于MEG信号的语音检测和音素分类任务，通过特定优化实现了超越基线的表现，并进入前十名。


<details>
  <summary>Details</summary>
Motivation: 参与LibriBrain 2025 PNPL竞赛，解决语音检测和音素分类这两个基础的MEG任务。

Method: 采用紧凑型Conformer模型，适配原始306通道MEG信号，并结合轻量级卷积投影层和任务特定头部。语音检测任务使用了MEG专用SpecAugment数据增强。音素分类任务采用了逆平方根类别加权和动态分组加载器处理平均样本。此外，实例级归一化被用于缓解分布偏移。

Result: 在官方标准赛道上，语音检测任务获得了88.9%的F1-macro分数，音素分类任务获得了65.8%的F1-macro分数。这些结果均超越了竞赛基线，并在两项任务中均排名前十。

Conclusion: 所提出的基于Conformer的解码器方法，通过结合MEG信号的特定优化策略，在LibriBrain 2025 PNPL竞赛的语音检测和音素分类任务中表现出色，证明了其在MEG任务中的有效性。
...
### [359] [Enhancing BERT Fine-Tuning for Sentiment Analysis in Lower-Resourced Languages](https://arxiv.org/abs/2512.01460)
*Jozef Kubík,Marek Šuppa,Martin Takáč*

Main category: cs.CL

TL;DR: 本文提出了一种结合主动学习、数据聚类和动态数据选择策略的集成微调流程，旨在提升低资源语言模型的性能并减少标注成本。


<details>
  <summary>Details</summary>
Motivation: 低资源语言的数据稀缺导致语言模型性能较弱。由于预训练计算成本高昂，研究重点转向在有限数据下改进微调过程。

Method: 研究采用主动学习（AL）方法，并结合结构化数据选择策略（称为“主动学习调度器”）。通过将AL与数据聚类相结合，提出了一种集成的微调流程，系统地整合了AL、聚类和动态数据选择调度器。

Result: 在斯洛伐克语、马耳他语、冰岛语和土耳其语上的实验表明，在微调阶段使用聚类结合AL调度，可以同时实现高达30%的标注节省和高达四个F1分数点的性能提升，并提供更好的微调稳定性。

Conclusion: 所提出的结合主动学习、聚类和动态数据选择的集成微调流程，能有效提升低资源语言模型的性能，并显著减少数据标注需求，同时提高微调的稳定性。
...
### [360] [MCAT: Scaling Many-to-Many Speech-to-Text Translation with MLLMs to 70 Languages](https://arxiv.org/abs/2512.01512)
*Yexing Du,Kaiyuan Liu,Youcheng Pan,Bo Yang,Keqi Deng,Xie Chen,Yang Xiang,Ming Liu,Bin Qin,YaoWei Wang*

Main category: cs.CL

TL;DR: 本文提出MCAT框架，通过语言扩展方法和优化的语音适配器，解决了多模态大语言模型（MLLMs）在语音到文本翻译（S2TT）中语言覆盖范围有限和推理效率低下的问题，实现了70种语言间的相互翻译，并显著提升了推理速度和性能。


<details>
  <summary>Details</summary>
Motivation: 当前S2TT任务中的MLLMs面临两大挑战：一是语言覆盖范围受限，大多数流行数据集以英语为中心，阻碍了MLLMs多对多翻译能力的扩展；二是推理效率低下，当语音转换为长序列时（例如750个token），MLLMs的推理速度急剧下降。

Method: 本文提出了多语言经济高效加速语音到文本翻译器（MCAT）框架，包含两项创新：1. 引入了一种语言扩展方法，利用课程学习和数据平衡策略，将MLLMs支持的语言覆盖范围扩展到70种，并实现这些语言间的相互翻译。2. 设计了一个优化的语音适配器模块，将语音序列的长度缩短至仅30个token。

Result: MCAT在不同规模的MLLMs（9B和27B）上进行了广泛实验。实验结果表明，MCAT不仅在FLEURS数据集上以70x69个方向超越了最先进的端到端模型，而且显著提升了批量推理效率。这仅通过约1亿个可训练参数和每种语言仅10小时的S2TT数据实现。

Conclusion: MCAT框架有效解决了MLLMs在S2TT任务中的语言覆盖和效率挑战，通过创新的语言扩展方法和优化的语音适配器，实现了优越的性能和更高的推理效率，并以开源形式发布，以促进MLLMs在S2TT领域的发展。
...
### [361] [Learning the Boundary of Solvability: Aligning LLMs to Detect Unsolvable Problems](https://arxiv.org/abs/2512.01661)
*Dengyun Peng,Qiguang Chen,Bofei Liu,Jiannan Guan,Libo Qin,Zheng Yan,Jinhao Liu,Jianshu Zhang,Wanxiang Che*

Main category: cs.CL

TL;DR: 该研究提出UnsolvableQA数据集和UnsolvableRL框架，旨在提升大型语言模型(LLMs)识别问题不可解性、解决可解问题并谨慎拒绝超出能力范围任务的能力，有效解决了模型幻觉和过度自信问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs难以区分客观不可解性（问题固有矛盾）和主观能力限制（超出模型能力），导致幻觉和过度自信，影响LLM的可靠性。

Method: 1. 构建UnsolvableQA数据集：采用双轨方法，通过程序化生成逻辑谜题，并创新性地使用“逆向构建”方法向有效推理链中注入矛盾以创建数学问题中的不可解实例。2. 引入UnsolvableRL：一个强化学习框架，包含准确性、不可解性和难度三个奖励组件，共同优化模型性能。

Result: 该方法实现了近乎完美的不可解性检测，同时提高了模型在可解任务上的准确性。研究还揭示了“能力崩溃”现象，证明明确接触不可解数据对于防止模型系统性过度自信至关重要。

Conclusion: 明确地让模型接触不可解数据对于防止其系统性过度自信是不可或缺的，并且是确保LLM可靠性的关键一步。
...
### [362] [PromptBridge: Cross-Model Prompt Transfer for Large Language Models](https://arxiv.org/abs/2512.01420)
*Yaxuan Wang,Quan Liu,Zhenting Wang,Zichao Li,Wei Wei,Yang Liu,Yujia Bao*

Main category: cs.CL

TL;DR: 大型语言模型（LLM）的快速演进导致频繁的模型切换，但提示词对模型敏感，直接复用会导致性能下降（模型漂移）。本文提出PromptBridge，一个无需训练的框架，通过校准和学习跨模型提示词映射，有效解决模型漂移问题，实现提示词的跨模型迁移。


<details>
  <summary>Details</summary>
Motivation: LLM模型格局（如GPT, Claude, Llama）快速发展，导致系统需频繁切换模型以适应能力、成本、部署和隐私需求。然而，为特定模型设计的提示词在其他模型上表现不佳，这种“模型漂移”现象严重，需要昂贵的重新优化。

Method: 本文提出PromptBridge，一个无需训练的框架。它首先通过“模型自适应反射式提示词演化”（MAP-RPE）方法，利用迭代反射式优化和定量评估，为源模型和目标模型获取任务和模型特定的最优提示词对，用于校准。然后，PromptBridge利用这些校准后的提示词对学习一个跨模型的提示词映射。在测试时，给定一个源模型的提示词，该映射可以直接生成一个针对目标模型优化的提示词。

Result: 通过在单智能体和多智能体设置中的实验表明，PromptBridge一致地提高了下游任务的准确性，并显著减少了模型迁移的工作量，有效解决了模型漂移问题。

Conclusion: 模型漂移是LLM实际应用中的普遍且严重的问题。PromptBridge提供了一个无需训练的有效解决方案，通过学习跨模型提示词映射，使得提示词能够在不同模型间高效迁移，无需昂贵的重复优化，从而提高了模型切换的效率和性能。
...
### [363] [MAC-SLU: Multi-Intent Automotive Cabin Spoken Language Understanding Benchmark](https://arxiv.org/abs/2512.01603)
...
TL;DR: 本文提出了MAC-SLU数据集，这是一个用于汽车座舱的多意图口语理解数据集，旨在增加任务复杂性。同时，本文对主流的开源大型语言模型（LLMs）和大型音频语言模型（LALMs）进行了基准测试，并发现微调模型表现优于上下文学习，端到端LALMs表现与流水线方法相当且能有效避免语音识别错误传播。


<details>
  <summary>Details</summary>
Motivation: 现有的口语理解（SLU）数据集普遍缺乏多样性和复杂性，并且缺少一个针对最新大型语言模型（LLMs）和大型音频语言模型（LALMs）的统一基准测试。

Method: 本文引入了MAC-SLU数据集，一个包含真实复杂多意图数据的多意图汽车座舱口语理解数据集。基于该数据集，对领先的开源LLMs和LALMs进行了全面的基准测试，涵盖了上下文学习（in-context learning）、监督微调（SFT）以及端到端（E2E）和流水线（pipeline）范式。

Result: 实验表明，LLMs和LALMs虽然通过上下文学习有潜力完成SLU任务，但其性能显著落后于监督微调（SFT）方法。同时，端到端（E2E）LALMs展现出与流水线方法相当的性能，并能有效避免语音识别带来的错误传播。

Conclusion: LLMs和LALMs在SLU任务中通过上下文学习仍有局限性，监督微调是提升性能的关键。端到端LALMs是一种有前景的方法，其性能可媲美流水线方法，并能有效解决语音识别错误传播问题。
...
### [364] [Language Diversity: Evaluating Language Usage and AI Performance on African Languages in Digital Spaces](https://arxiv.org/abs/2512.01557)
*Edward Ajayi,Eudoxie Umwari,Mawuli Deku,Prosper Singadi,Jules Udahemuka,Bekalu Tadele,Chukuemeka Edeh*

Main category: cs.CL

TL;DR: 本研究评估了当前语言检测工具在约鲁巴语、基尼亚卢旺达语和阿姆哈拉语上的表现，发现由于会话平台数据稀疏且存在语码转换，新闻媒体的专业内容是训练非洲语言AI模型的更可靠来源。


<details>
  <summary>Details</summary>
Motivation: 非洲语言在数字平台的表示面临挑战，尤其是在线使用稀疏、受英语影响严重，且缺乏代表真实单语对话的语料。这导致现有语言检测工具难以有效处理这些语言，并且缺乏用于训练语言模型的真实会话数据。

Method: 研究评估了现有语言检测工具（包括专门的AfroLID和通用大型语言模型）在约鲁巴语、基尼亚卢旺达语和阿姆哈拉语上的性能。数据从Reddit子版块和当地新闻来源收集，并分析了两种来源的数据特征。随后，将检测模型应用于这些数据进行测试。

Result: Reddit数据显示数据量极少且语码转换严重。相反，当地新闻媒体提供了丰富、干净的单语数据，并促进了用户在新闻社交媒体页面上更多地使用本地语言。语言检测模型在干净的新闻数据上表现接近完美，但在语码转换的Reddit帖子中表现不佳。

Conclusion: 专业策划的新闻内容是训练非洲语言上下文丰富AI模型的更可靠和有效来源，优于会话平台数据。未来的模型需要能够处理干净文本和语码转换文本，以提高非洲语言的检测准确性。
...
### [365] [MMAG: Mixed Memory-Augmented Generation for Large Language Models Applications](https://arxiv.org/abs/2512.01710)
*Stefano Zeppieri*

Main category: cs.CL

TL;DR: 为解决大型语言模型在长时间交互中缺乏相关性、个性化和连续性的问题，本文提出了混合记忆增强生成（MMAG）模式，该框架将记忆分为五层，以构建更具人类记忆能力的语言代理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在单一提示下表现出色，但在长时间交互中难以保持相关性、个性化和连续性。人类交流则依赖多种形式的记忆，从回忆过往对话到适应个人特质和情境。

Method: 引入混合记忆增强生成（MMAG）模式，该框架将基于LLM的代理的记忆组织为五个相互作用的层：对话记忆、长期用户记忆、情景和事件关联记忆、感知和上下文感知记忆、以及短期工作记忆。本文将这些层映射到技术组件，并概述了协调、优先级排序和冲突解决的策略。

Result: 通过在Heero对话代理中的实现，MMAG方法已展示出通过加密的长期生物信息和对话历史来提高用户参与度和留存率。论文还讨论了存储、检索、隐私和延迟等实现问题，并指出了开放性挑战。

Conclusion: MMAG为构建记忆丰富的语言代理奠定了基础，使其能更连贯、主动，并更好地符合人类需求。
...
### [366] [Self-Supervised Borrowing Detection on Multilingual Wordlists](https://arxiv.org/abs/2512.01713)
*Tim Wientzek*

Main category: cs.CL

TL;DR: 本文提出了一种全自监督的多语言词表借词检测方法，结合了基于全局对应模型的PMI相似性和基于语音特征向量的对比学习组件，并在无需标注数据的情况下自动选择决策阈值，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有借词检测方法在字符串相似度度量方面存在局限性，且有监督方法需要标注数据。研究动机是开发一种无需人工监督、效果良好且能自动确定阈值的借词检测方法。

Method: 该方法结合了两种信息源：基于全局对应模型的PMI（互信息）相似性，以及在语音特征向量上训练的轻量级对比学习组件。它还包含一个无需标注数据即可自动选择决策阈值的程序。通过消融研究分析了字符编码、温度设置和数据增强策略的重要性。

Result: 实验结果表明，仅PMI就优于现有的字符串相似度度量（如NED和SCA）。结合后的相似度性能与有监督基线相当或更优。消融研究强调了字符编码、温度设置和数据增强策略的重要性。该方法适用于不同大小的数据集，无需人工监督，并提供了命令行工具供研究人员使用。

Conclusion: 本文提出的全自监督借词检测方法有效且无需人工监督，其性能与有监督基线相当或更优，为多语言词表中的借词检测提供了一种鲁棒且实用的解决方案。
...
### [367] [Beware of Reasoning Overconfidence: Pitfalls in the Reasoning Process for Multi-solution Tasks](https://arxiv.org/abs/2512.01725)
...
### [368] [Reasoning About the Unsaid: Misinformation Detection with Omission-Aware Graph Inference](https://arxiv.org/abs/2512.01728)
...
### [369] [InnoGym: Benchmarking the Innovation Potential of AI Agents](https://arxiv.org/abs/2512.01822)
...
### [370] [BHRAM-IL: A Benchmark for Hallucination Recognition and Assessment in Multiple Indian Languages](https://arxiv.org/abs/2512.01852)
...
### [371] [Beyond SFT: Reinforcement Learning for Safer Large Reasoning Models with Better Reasoning Ability](https://arxiv.org/abs/2512.01848)
...
### [372] [Exploring Human Perceptions of AI Responses: Insights from a Mixed-Methods Study on Risk Mitigation in Generative Models](https://arxiv.org/abs/2512.01892)
...
### [373] [Cross-Lingual Interleaving for Speech Language Models](https://arxiv.org/abs/2512.01865)
...
### [374] [OPOR-Bench: Evaluating Large Language Models on Online Public Opinion Report Generation](https://arxiv.org/abs/2512.01896)
...
### [375] [Rectifying LLM Thought from Lens of Optimization](https://arxiv.org/abs/2512.01925)
*Junnan Liu,Hongwei Liu,Songyang Zhang,Kai Chen*

Main category: cs.CL

TL;DR: 本文提出RePro方法，将LLM的思维链（CoT）推理视为梯度下降过程，通过定义过程级奖励（基于强度和稳定性双重评分）来优化LLM的推理过程，从而提高性能并减少过度思考等次优行为。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）通过长思维链（CoT）提示展现出新兴的推理能力，但它们常表现出次优的推理行为，如过度思考和过长的推理链，这会损害性能。研究旨在解决这些问题。

Method: 将CoT推理过程视为梯度下降优化过程。引入RePro（Rectifying Process-level Reward）方法，在后训练阶段优化LLM推理。RePro定义了一个替代目标函数来评估CoT底层的优化过程，使用双重评分机制（强度和稳定性）来量化其优化过程。这些分数被聚合成一个复合的过程级奖励，并无缝集成到基于可验证奖励的强化学习（RLVR）流程中以优化LLMs。

Result: 在多种强化学习算法和不同LLMs上，通过数学、科学和编码等领域的基准测试进行评估，RePro持续提升了推理性能，并有效缓解了次优推理行为。

Conclusion: RePro是一种有效的方法，通过纠正CoT过程中的问题，显著提升了LLM的推理能力，并减少了过度思考等次优行为，从而在多个领域取得了更好的性能。
...
### [376] [How Far Are We from Genuinely Useful Deep Research Agents?](https://arxiv.org/abs/2512.01948)
...
TL;DR: 本文提出了FINDER，一个细粒度的深度研究代理（DRA）基准，以及DEFT，第一个针对DRA的失败分类法，揭示了现有DRA在证据整合、验证和规划方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究代理（DRA）主要在问答基准上进行验证，忽略了生成综合报告的能力。此外，当前的报告合成基准存在任务复杂性和主观指标问题，无法反映用户需求并限制了生成报告的实用性。

Method: 本文提出了FINDER，一个包含100个人工策划研究任务和419个结构化清单项的增强型基准，以标准化报告结构、分析深度和事实依据。基于主流DRA生成的约1000份报告，本文进一步提出了DEFT，一个包含14种细粒度失败模式（涵盖推理、检索和生成）的深度研究代理失败分类法。DEFT是基于扎根理论，通过人机协作标注和标注者间可靠性验证构建的。

Result: 实验结果表明，当前的深度研究代理并非在任务理解上遇到困难，而是在证据整合、验证和具有韧性的推理规划方面表现不足。

Conclusion: FINDER和DEFT为评估和改进深度研究代理提供了新的工具，并明确指出了当前DRA在证据处理和高级推理规划方面的关键挑战，为未来的研究指明了方向。
...
### [377] [Latent Debate: A Surrogate Framework for Interpreting LLM Thinking](https://arxiv.org/abs/2512.01909)
*Lihu Chen,Xiang Yin,Francesca Toni*

Main category: cs.CL

TL;DR: 本文提出了一种名为“潜在辩论”的新框架，通过捕捉单个大型语言模型（LLM）在单次推理过程中内部的隐式支持和攻击信号，来解释其预测并理解幻觉的成因。该框架被证明是忠实的替代模型，能有效检测幻觉，并揭示了幻觉与内部辩论模式之间的强相关性。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型（LLM）的内部思考过程以及幻觉产生的原因是一个关键挑战。现有的自我一致性或多智能体辩论方法依赖于多答案或多模型间的显式辩论，而未能探究单个模型内部的隐式争论。

Method: 引入“潜在辩论”框架，通过捕捉单个模型在单次推理过程中产生的隐藏支持和攻击信号来解释模型预测。首先提出了一个与模型和任务无关的概念框架，然后对其进行符号实例化，以近似LLM在真/假预测任务上的思考过程。

Result: 潜在辩论是一个忠实的结构化替代模型，其预测与原始LLM高度一致。它为幻觉检测提供了一个强大的基线。进一步分析揭示了幻觉与辩论模式之间存在强相关性，例如，中间层高度的潜在辩论与更高的幻觉风险相关。

Conclusion: 潜在辩论为理解大型语言模型的内部机制提供了一个潜在框架，尤其适用于推理步骤中出现内部（不）一致的场景。
...
### [378] [The Art of Scaling Test-Time Compute for Large Language Models](https://arxiv.org/abs/2512.02008)
*Aradhye Agarwal,Ayan Sengupta,Tanmoy Chakraborty*

Main category: cs.CL

TL;DR: 本文对大语言模型推理中的测试时扩展（TTS）策略进行了首次大规模系统研究，发现没有单一策略能普遍占优，模型根据问题难度和轨迹长度呈现不同的轨迹质量模式，且最佳性能随计算预算单调增长，并据此提出了选择最佳TTS策略的实用方法。


<details>
  <summary>Details</summary>
Motivation: 尽管测试时扩展（TTS）在提高大语言模型（LLMs）推理能力方面潜力巨大，但目前缺乏在相同条件下对已知TTS策略的系统比较，且模型类型和问题难度对性能的影响尚不明确。

Method: 研究进行了首次大规模TTS研究，生成了超过300亿个token，使用了8个开源LLM（7B到235B参数），并在四个推理数据集上进行了评估。

Result: 研究发现三个一致趋势：(1) 没有单一的TTS策略能普遍占据主导地位；(2) 推理模型在不同问题难度和轨迹长度下表现出独特的轨迹质量模式，可分为短视和长视两类；(3) 对于特定模型类型，最佳TTS性能随计算预算单调增长。

Conclusion: 基于这些发现，论文提供了一个实用的TTS策略选择指南，该指南考虑了问题难度、模型类型和计算预算，为有效的推理时扩展提供了实用指导。
...
### [379] [Four Over Six: More Accurate NVFP4 Quantization with Adaptive Block Scaling](https://arxiv.org/abs/2512.02010)
*Jack Cook,Junxian Guo,Guangxuan Xiao,Yujun Lin,Song Han*

Main category: cs.CL

TL;DR: 本文介绍了Four Over Six (4/6)，一种改进NVFP4量化算法的方法，通过为每个值块评估两个潜在比例因子，解决了大型语言模型训练和推理中因NVFP4量化导致的性能下降和发散问题，尤其改善了对接近最大值的表示。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型规模的增长，NVFP4等低精度数值格式因其速度和内存优势而日益普及。然而，在训练和推理过程中，将所有矩阵乘法操作数（权重、激活、梯度）量化到NVFP4通常会导致训练发散和性能下降。研究发现，浮点格式（如FP4）在每个块的接近最大值上存在最大的量化误差，这是导致性能下降的主要原因。

Method: 本文提出了Four Over Six (4/6) 算法，这是对NVFP4量化算法的修改。它为每个值块评估两个潜在的比例因子，而不是NVFP4的单个比例因子。通过将值缩放到较小的FP4范围，4/6使可表示值的分布更均匀，从而改善了对接近最大值的表示。该方法可以在NVIDIA Blackwell GPU上高效实现。

Result: 在Transformer和混合模型架构的预训练实验中，4/6在多种情况下阻止了训练发散，使训练损失显著接近BF16，优于当前最先进的NVFP4训练方案。此外，4/6可以轻松整合到多种训练后量化方法中，并普遍提高了下游任务的准确性。该方法在NVIDIA Blackwell GPU上可高效实现。

Conclusion: 4/6是对NVFP4量化算法的有效改进，它通过优化接近最大值的表示，解决了大型语言模型训练中的发散和推理中的性能下降问题。该方法在实际应用中具有可行性，并有望激发未来在NVFP4模型训练和部署方面的研究。
...
### [380] [DREAMer-VXS: A Latent World Model for Sample-Efficient AGV Exploration in Stochastic, Unobserved Environments](https://arxiv.org/abs/2512.00005)
*Agniprabha Chakraborty*

Main category: cs.RO

TL;DR: DREAMer-VXS是一个用于自主地面车辆(AGV)探索的基于模型的强化学习框架，它通过学习世界模型并在想象中规划，显著减少了真实世界交互，提高了学习效率、泛化性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统的无模型强化学习算法在实际机器人应用中存在样本效率低下和脆弱性问题，严重阻碍了其在现实世界中的部署，尤其是在AGV探索任务中。

Method: 本文提出了DREAMer-VXS框架，它从部分高维LiDAR观测中学习一个全面的世界模型。该世界模型由一个卷积变分自编码器(VAE)学习环境结构，和一个循环状态空间模型(RSSM)建模复杂时间动态。利用这个学习到的模型作为高速模拟器，智能体几乎完全在想象中训练其导航策略。策略学习与真实世界交互解耦，并通过一个结合任务目标和内在好奇心奖励的复合奖励函数来优化。

Result: 与最先进的无模型SAC基线相比，DREAMer-VXS将达到专家级性能所需的真实环境交互减少了90%。它学习速度快了几个数量级，并开发出更具泛化性和鲁棒性的策略，在未见环境中探索效率提高了45%，并对动态障碍物表现出卓越的弹性。

Conclusion: DREAMer-VXS通过利用基于模型的学习和想象规划，显著提高了自主地面车辆探索的样本效率、泛化性和鲁棒性，有效解决了无模型强化学习在实际应用中的主要挑战。
...
### [381] [Foundation Models for Trajectory Planning in Autonomous Driving: A Review of Progress and Open Challenges](https://arxiv.org/abs/2512.00021)
*Kemal Oksuz,Alexandru Buburuzan,Anthony Knittel,Yuhan Yao,Puneet K. Dokania*

Main category: cs.RO

TL;DR: 本文综述了用于自动驾驶轨迹规划的多模态基础模型，特别是VLA模型，并提供了一个统一的分类法和对37种方法的评估。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶技术正从传统的手工设计转向基于统一基础模型的方法，这些模型能够直接从原始感知输入推断运动轨迹，并能整合自然语言作为额外模态。

Method: 通过统一的分类法，对37种最近提出的基于基础模型的轨迹规划方法进行了全面审查，评估了它们的架构设计、方法优势、固有能力和局限性。同时评估了这些方法的源代码和数据集的开放性。

Result: 提出了一个涵盖37种最新方法的调查报告，这些方法涵盖了使用基础模型进行轨迹规划的领域。提供了一个伴随的网页，根据分类法对方法进行了编目。

Conclusion: 为从业者和研究人员提供了关于自动驾驶轨迹规划中基础模型的宝贵信息，包括其架构选择、方法论优势、能力和局限性的批判性评估。
...
### [382] [A Comprehensive Survey on Surgical Digital Twin](https://arxiv.org/abs/2512.00019)
...
TL;DR: 这篇综述对外科数字孪生（SDTs）进行了批判性结构化回顾，探讨了其在术前、术中和术后护理中的潜力与挑战。文章澄清了术语，提出了分类法，总结了当前成就，并指出了开放性问题，最终提出了一个研究议程，以推动SDTs从实验室走向临床应用。


<details>
  <summary>Details</summary>
Motivation: 随着多模态手术数据和实时计算的快速发展，外科数字孪生（SDTs）作为虚拟对应物出现，有望在术前、术中和术后护理中提供镜像、预测和决策支持。然而，SDTs面临着异构数据融合、模型精度与计算效率平衡、鲁棒性、可解释性、不确定性校准、互操作性、隐私和法规遵从性等持续挑战。本研究旨在通过全面综述来解决这些问题并指导SDT的设计与部署。

Method: 本研究对SDTs进行了批判性、结构化的综述。具体方法包括：澄清术语和范围；根据目的、模型保真度和数据源提出分类法；综合形变配准与跟踪、实时模拟与协同模拟、AR/VR引导、边缘-云编排以及用于场景理解和预测的AI等方面的最新成就；对比非机器人孪生与机器人循环架构；识别验证与基准测试、安全保障与人因、生命周期“数字主线”集成以及可扩展数据治理等开放性问题。

Result: 该综述明确了SDT的术语和范围，提出了一个基于目的、模型保真度和数据源的分类法。它综合了变形配准与跟踪、实时模拟、AR/VR引导、边缘-云编排和AI场景理解与预测等领域的最新成就。文章还区分了非机器人孪生与机器人循环架构，并识别了验证与基准测试、安全保障与人因、生命周期“数字主线”集成以及可扩展数据治理等关键开放问题。

Conclusion: 本研究提出了一个旨在实现可信赖、符合标准并能带来可衡量临床效益的SDTs的研究议程。通过统一词汇、组织能力并突出差距，本工作旨在指导SDT的设计与部署，并促进其从实验室原型向常规外科护理的转化。
...
### [383] [Learning from Watching: Scalable Extraction of Manipulation Trajectories from Human Videos](https://arxiv.org/abs/2512.00024)
*X. Hu,G. Ye*

Main category: cs.RO

TL;DR: 本文提出一种新方法，结合大型基础视频理解模型和点跟踪技术，从在线人类操作视频中提取任务相关关键点的密集轨迹，以实现更具扩展性和数据效率的机器人学习。


<details>
  <summary>Details</summary>
Motivation: 训练大型机器人模型所需的高质量数据收集成本高昂且耗时。现有利用在线人类操作视频的方法主要关注手部检测或物体姿态估计，未能充分利用视频中丰富的交互信息。

Method: 该方法结合了大型基础视频理解模型和点跟踪技术，用于提取操作过程中所有任务相关关键点的密集轨迹。

Result: 实验结果表明，该方法能够准确跟踪整个操作过程中的关键点。

Conclusion: 该方法为更具扩展性和数据效率的机器人学习铺平了道路，通过更全面地利用互联网规模的人类演示视频。
...
### [384] [XFlowMP: Task-Conditioned Motion Fields for Generative Robot Planning with Schrodinger Bridges](https://arxiv.org/abs/2512.00022)
...
### [385] [A Survey on Improving Human Robot Collaboration through Vision-and-Language Navigation](https://arxiv.org/abs/2512.00027)
...
### [386] [Perturbation-mitigated USV Navigation with Distributionally Robust Reinforcement Learning](https://arxiv.org/abs/2512.00030)
...
### [387] [Intelligent Systems and Robotics: Revolutionizing Engineering Industries](https://arxiv.org/abs/2512.00033)
...
### [388] [Design And Control of A Robotic Arm For Industrial Applications](https://arxiv.org/abs/2512.00034)
...
### [389] [ICD-Net: Inertial Covariance Displacement Network for Drone Visual-Inertial SLAM](https://arxiv.org/abs/2512.00037)
...
### [390] [VISTAv2: World Imagination for Indoor Vision-and-Language Navigation](https://arxiv.org/abs/2512.00041)
...
### [391] [Socially aware navigation for mobile robots: a survey on deep reinforcement learning approaches](https://arxiv.org/abs/2512.00049)
*Ibrahim Khalil Kabir,Muhammad Faizan Mysorewala*

Main category: cs.RO

TL;DR: 这篇综述全面回顾了基于深度强化学习（DRL）的社交感知导航方法，分析了其关键技术、评估机制、挑战及未来方向。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习的出现加速了机器人社交感知导航领域的发展，但需要一个全面的概述来分析其应用、挑战以及如何更好地在人类环境中融入社会规范。

Method: 本文通过综述和批判性分析，探讨了基于价值、基于策略和Actor-Critic的强化学习算法，以及前馈、循环、卷积、图和Transformer网络等神经网络架构在社交感知导航中的应用。同时，研究了评估机制（指标、基准数据集、仿真环境）和模拟到现实的迁移挑战。

Result: DRL显著提升了社交感知导航的安全性与人类接受度，但该领域仍面临评估机制不统一、缺乏标准化社交指标、计算负担重以及模拟到现实迁移困难等挑战。

Conclusion: 未来的进展将依赖于结合多种方法的混合策略，并开发平衡技术效率与以人为本评估的基准。
...
### [392] [Reinforcement Learning from Implicit Neural Feedback for Human-Aligned Robot Control](https://arxiv.org/abs/2512.00050)
*Suzie Kim*

Main category: cs.RO

TL;DR: 本文提出了一种基于隐式人机反馈的强化学习（RLIHF）框架，利用脑电图（EEG）中的错误相关电位（ErrPs）作为连续、非侵入性的反馈信号，在稀疏奖励条件下实现了与密集手动奖励相当的策略学习效果。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在稀疏奖励下难以学习有效策略，需要复杂的手动奖励设计。现有的人机反馈强化学习（RLHF）方法依赖显式反馈（如按钮、偏好标签），这会中断自然交互并增加用户认知负担。因此，需要一种非侵入性、隐式的反馈机制来解决这些问题。

Method: 所提出的RLIHF框架利用非侵入性EEG信号（特别是ErrPs）提供连续、隐式反馈。一个预训练的解码器将原始EEG信号转换为概率性奖励成分，从而在稀疏外部奖励下也能有效学习策略。该方法在一个基于MuJoCo物理引擎的仿真环境中进行评估，使用Kinova Gen2机械臂执行复杂的避障抓取任务。

Result: 结果表明，通过解码的EEG反馈训练的智能体取得了与通过密集、手动设计奖励训练的智能体相当的性能。

Conclusion: 这些发现验证了使用隐式神经反馈在交互式机器人领域实现可扩展和以人为本的强化学习的潜力。
...
### [393] [Causal Reinforcement Learning based Agent-Patient Interaction with Clinical Domain Knowledge](https://arxiv.org/abs/2512.00048)
*Wenzheng Zhao,Ran Zhang,Ruth Palan Lopez,Shu-Fen Wung,Fengpei Yuan*

Main category: cs.RO

TL;DR: 本文提出了一种名为因果结构感知强化学习（CRL）的新框架，它将因果发现和推理整合到策略优化中，以解决自适应医疗干预（如痴呆症护理）中数据稀缺、需要可解释性以及患者状态动态复杂且具有因果性质的挑战。CRL在模拟机器人辅助认知护理场景中表现优于传统模型无关的强化学习基线，并展示了可解释、临床一致的行为，且具有鲁棒性，同时还演示了轻量级的基于LLM的部署。


<details>
  <summary>Details</summary>
Motivation: 自适应医疗干预（如痴呆症护理）中的强化学习面临显著挑战：数据稀缺、决策需要可解释性，以及潜在的患者状态动态复杂且具有因果性质。

Method: 本文提出了因果结构感知强化学习（CRL）框架，它明确地将因果发现和推理整合到策略优化中。该方法使智能体能够学习和利用描述人类行为状态与机器人动作之间因果依赖关系的DAG（有向无环图）。该方法在模拟的机器人辅助认知护理场景中进行了验证，其中智能体与一个具有动态情绪、认知和参与状态的虚拟患者互动。此外，还展示了一个轻量级的基于LLM的部署，通过将固定策略嵌入到系统提示中，实现从推断状态到动作的映射，无需LLM微调即可生成一致、支持性的对话。

Result: 实验结果表明，CRL智能体在累积奖励方面优于传统的模型无关强化学习基线，能更持续地保持理想的患者状态，并表现出可解释的、与临床一致的行为。CRL的性能优势在不同的加权策略和超参数设置下保持鲁棒性。基于LLM的部署能够产生一致、支持性的对话。

Conclusion: 这项工作展示了因果强化学习在人机交互应用中的前景，尤其是在可解释性、适应性和数据效率至关重要的情况下。
...
### [394] [An adaptive experience-based discrete genetic algorithm for multi-trip picking robot task scheduling in smart orchards](https://arxiv.org/abs/2512.00057)
*Peng Chen,Jing Liangb,Kang-Jia Qiao,Hui Song,Cai-Tong Yue,Kun-Jie Yu,Ponnuthurai Nagaratnam Suganthan,Witold Pedrycz*

Main category: cs.RO

TL;DR: 本研究提出了一种自适应经验离散遗传算法（AEDGA），用于解决智能果园中多趟采摘机器人任务调度问题，通过创新的初始化、局部搜索和自适应选择策略，显著优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 智能果园中，多机器人系统能有效缓解劳动力短缺和成本上升问题，但其高效调度面临复杂的优化挑战，特别是现有算法在局部搜索中存在冗余操作，导致搜索效率低和易陷入局部最优。

Method: 本文提出了AEDGA算法，包含三项关键创新：(1) 集成负载-距离平衡初始化方法；(2) 基于聚类的局部搜索机制；(3) 基于经验的自适应选择策略。同时，为确保方案在完工时间约束下的可行性，开发了三种不同的解决方案修复框架。

Result: 在18个新提出的测试实例和24个现有测试问题上进行的综合实验表明，AEDGA显著优于八种最先进的算法。

Conclusion: AEDGA为智能果园中的多趟采摘机器人任务调度问题提供了一个高效且优越的解决方案，有效克服了传统算法的局限性，提升了自动化采摘系统的效率。
...
### [395] [Modeling and Control of Magnetic Forces between Microrobots](https://arxiv.org/abs/2512.00051)
*Amelia Fernández Seguel,Alejandro I. Maass*

Main category: cs.RO

TL;DR: 本研究提出了一种级联PID+PD控制器，通过全局磁场的角度来快速、精确且平稳地控制两个磁性微型机器人之间的径向距离，解决了现有系统同步运动的局限性。


<details>
  <summary>Details</summary>
Motivation: 在靶向药物递送和显微手术等生物医学应用中，对多个磁性微型机器人进行独立控制面临挑战。现有系统大多只允许所有智能体同步移动，限制了其在需要差异化驱动的应用中的使用。

Method: 研究采用级联控制方法，利用全局磁场的角度作为驱动参数。首先，通过文献回顾建立了物理模型，考虑了磁偶极-偶极相互作用和粘性介质中的速度。然后，实施了一个PID控制器来调节径向距离，并在此基础上串联一个PD控制器来平滑磁场方向的变化。所有控制器都在MATLAB中进行仿真。

Result: 仿真结果显示，PID控制器将收敛到期望半径的时间缩短了约40%。当加入第二个控制器后，组合的PID+PD方案在相似的时间内实现了平滑的角度轨迹，波动仅为±5°。这些结果验证了在不引起控制角度突然变化的情况下，快速、精确控制两个微型机器人径向距离的可行性。

Conclusion: 本研究验证了使用共享磁场快速、精确且平稳地控制两个微型机器人径向距离的可行性。然而，该模型目前仅限于2D环境和两个智能体，未来的研究将扩展控制器至3D系统和多智能体。
...
### [396] [Enhancing Cognitive Robotics with Commonsense through LLM-Generated Preconditions and Subgoals](https://arxiv.org/abs/2512.00069)
*Ohad Bachner,Bar Gamliel*

Main category: cs.RO

TL;DR: 该研究结合大型语言模型（LLM）与符号规划，通过LLM推断常识性细节（前提条件和子目标），显著提高了机器人在日常任务中的规划有效性和成功率。


<details>
  <summary>Details</summary>
Motivation: 机器人常因指令中缺失常识性细节（如隐藏的前提条件和小的子目标）而无法完成日常任务。传统的符号规划器需要这些细节被显式编写，这既耗时又不完整。

Method: 系统接收自然语言任务，LLM会建议合理的前提条件和子目标。这些建议被转化为形式化的规划模型，并在模拟环境中执行生成的计划。

Result: 与没有LLM步骤的基线规划器相比，该系统生成了更多有效的计划，实现了更高的任务成功率，并且在环境变化时表现出更好的适应性。

Conclusion: 将LLM的常识能力融入经典规划可以提高机器人在现实场景中的可靠性。
...
### [397] [SpeedAug: Policy Acceleration via Tempo-Enriched Policy and RL Fine-Tuning](https://arxiv.org/abs/2512.00062)
...
### [398] [Reconfigurable Auxetic Devices (RADs) for Robotic Surface Manipulation](https://arxiv.org/abs/2512.00072)
...
### [399] [Bootstrap Dynamic-Aware 3D Visual Representation for Scalable Robot Learning](https://arxiv.org/abs/2512.00074)
...
### [400] [Arcadia: Toward a Full-Lifecycle Framework for Embodied Lifelong Learning](https://arxiv.org/abs/2512.00076)
...
### [401] [A Hierarchical Framework for Humanoid Locomotion with Supernumerary Limbs](https://arxiv.org/abs/2512.00077)
*Bowen Zhi*
...
### [402] [Hyper-GoalNet: Goal-Conditioned Manipulation Policy Learning with HyperNetworks](https://arxiv.org/abs/2512.00085)
...
### [403] ["Why the face?": Exploring Robot Error Detection Using Instrumented Bystander Reactions](https://arxiv.org/abs/2512.00262)
...
### [404] [RealAppliance: Let High-fidelity Appliance Assets Controllable and Workable as Aligned Real Manuals](https://arxiv.org/abs/2512.00287)
...
### [405] [MILE: A Mechanically Isomorphic Exoskeleton Data Collection System with Fingertip Visuotactile Sensing for Dexterous Manipulation](https://arxiv.org/abs/2512.00324)
*Jinda Du,Jieji Ren,Qiaojun Yu,Ningbin Zhang,Yu Deng,Xingyu Wei,Yufei Liu,Guoying Gu,Xiangyang Zhu*

Main category: cs.RO

TL;DR: MILE是一个机械同构的遥操作与数据采集系统，旨在解决灵巧手模仿学习中高质量数据缺乏的问题。它通过消除非线性重定向并集成高分辨率指尖触觉传感器，高效收集多模态数据，显著提高了复杂手内操作的成功率。


<details>
  <summary>Details</summary>
Motivation: 灵巧手操作的模仿学习受限于大规模、高保真数据的不足。现有数据收集流程存在运动重定向不准确、数据收集效率低以及缺乏高分辨率指尖触觉感知等问题。

Method: 本文提出了MILE系统，该系统从人手到外骨骼再到机械手进行协同设计，实现了机械同构。外骨骼根据人手人体测量学设计，机械手保持一对一的关节位置同构，从而消除了非线性重定向。系统还集成了紧凑的指尖视觉触觉模块，提供高分辨率触觉观测。

Result: MILE外骨骼的多关节平均绝对角度误差低于一度。基于该无重定向接口的遥操作管道，平均成功率提高了64%。此外，整合指尖触觉观测后，成功率比仅视觉基线平均提高了25%。该系统高效收集了包含高分辨率指尖视觉触觉信号、RGB-D图像和关节位置的多模态数据集。

Conclusion: MILE系统提供了一个高保真、无重定向的遥操作和数据收集接口，显著提升了复杂手内操作的成功率。特别是指尖触觉观测的引入进一步验证了所收集数据集的保真度和实用性，为模仿学习提供了高质量数据支持。
...
### [406] [DPNet: Doppler LiDAR Motion Planning for Highly-Dynamic Environments](https://arxiv.org/abs/2512.00375)
*Wei Zuo,Zeyi Ren,Chengyang Li,Yikun Wang,Mingle Zhao,Shuai Wang,Wei Sui,Fei Gao,Yik-Chung Wu,Chengzhong Xu*

Main category: cs.RO

TL;DR: 该研究提出了一种名为DPNet的规划网络，通过整合多普勒激光雷达（Doppler LiDAR）数据，并使用基于多普勒模型的学习方法，实现对快速移动障碍物的高精度跟踪和避障规划。


<details>
  <summary>Details</summary>
Motivation: 现有的运动规划方法在处理快速移动障碍物时表现不佳，原因是对环境变化的理解不足。多普勒激光雷达能提供测距和瞬时速度，有望解决这一问题，但将其集成到规划器中存在高精度和高频率的双重挑战。

Method: 该论文提出了多普勒规划网络（DPNet）。首先，引入了多普勒卡尔曼神经网络（D-KalmanNet），用于在部分可观测高斯状态空间模型下跟踪障碍物的未来状态。其次，利用估计的运动构建了多普勒调优模型预测控制（DT-MPC）框架，用于自我运动规划，并实现了控制器参数的运行时自动调优。这些基于模型的学习器使DPNet在学习快速环境变化时保持轻量化、数据需求小，并实现高频率和高精度的跟踪与规划。

Result: DPNet在跟踪和规划方面实现了高频率和高精度，能够使用最少的数据快速学习环境变化，并保持轻量化。在高保真模拟器和真实世界数据集上的实验均表明，DPNet优于广泛的基准方案。

Conclusion: DPNet通过有效整合多普勒激光雷达和基于多普勒模型的学习方法，成功解决了快速移动障碍物的运动规划挑战，实现了高精度、高频率的跟踪和避障，并展现出卓越的性能。
...
### [407] [Balancing Efficiency and Fairness: An Iterative Exchange Framework for Multi-UAV Cooperative Path Planning](https://arxiv.org/abs/2512.00410)
*Hongzong Li,Luwei Liao,Xiangguang Dai,Yuming Feng,Rong Feng,Shiqin Tang*

Main category: cs.RO

TL;DR: 本文提出了一种多无人机协同路径规划的迭代交换框架，旨在通过迭代任务交换和路径优化，在效率（最小化总任务成本）和公平性（平衡无人机工作量）之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 多无人机协同路径规划（MUCPP）面临的主要挑战是如何在最大化效率（最小化总任务成本）和实现公平性（平衡各无人机工作量，避免单个智能体过载）之间取得平衡。

Method: 提出了一种新颖的迭代交换框架。该框架构建了一个结合总任务距离和完工时间（makespan）的复合目标函数，并通过在可行性和安全约束下的局部交换迭代地改进解决方案。每架无人机使用A*搜索算法在考虑地形的配置空间中生成无碰撞轨迹。

Result: 在多个地形数据集上的综合实验表明，与现有基线方法相比，所提出的方法在总距离和完工时间之间始终能实现更优的权衡。

Conclusion: 所提出的迭代交换框架能够有效地平衡多无人机协同路径规划中的效率和公平性，并在实验中展现出卓越的性能。
...
### [408] [Sample-Efficient Expert Query Control in Active Imitation Learning via Conformal Prediction](https://arxiv.org/abs/2512.00453)
*Arad Firouzkouhi,Omid Mirzaeedodangeh,Lars Lindemann*

Main category: cs.RO

TL;DR: CRSAIL是一种主动模仿学习方法，通过结合K近邻距离和共形预测来识别并仅查询专家未充分表示的新颖状态，从而大幅减少专家查询次数，同时保持或超越专家级性能。


<details>
  <summary>Details</summary>
Motivation: 主动模仿学习（AIL）中专家动作标注成本高昂，尤其是在GPU密集型模拟器、人机协作和机器人集群等场景中，协变量偏移问题导致需要频繁查询专家。

Method: CRSAIL（Conformalized Rejection Sampling for Active Imitation Learning）提出一种查询规则，仅当访问状态在专家标注数据集中未充分表示时才请求专家动作。它通过计算到第K个最近专家状态的距离来衡量状态新颖性，并利用共形预测设置一个全局阈值。该阈值是策略校准分数的经验(1-α)分位数，将α与预期查询率关联起来。该方法允许先完成完整轨迹（回合）的推出，然后再对访问状态的子集进行查询。

Result: 在MuJoCo机器人任务上，CRSAIL实现了与专家水平相当或更高的奖励，同时将专家查询总数相比DAgger减少高达96%，相比现有AIL方法减少高达65%。该方法对α和K参数具有经验鲁棒性，简化了在未知动态新系统上的部署。

Conclusion: CRSAIL通过创新的状态空间查询策略，显著降低了主动模仿学习中的专家查询成本，同时保持了高性能。其分布无关的校准规则和对参数的鲁棒性使其易于部署，并使α成为一个任务无关的查询率调节旋钮。
...
### [409] [Hardware-Software Collaborative Computing of Photonic Spiking Reinforcement Learning for Robotic Continuous Control](https://arxiv.org/abs/2512.00427)
...
### [410] [LAP: Fast LAtent Diffusion Planner with Fine-Grained Feature Distillation for Autonomous Driving](https://arxiv.org/abs/2512.00470)
...
### [411] [SAGAS: Semantic-Aware Graph-Assisted Stitching for Offline Temporal Logic Planning](https://arxiv.org/abs/2512.00775)
*Ruijia Liu,Ancheng Hou,Shaoyuan Li,Xiang Yin*

Main category: cs.RO

TL;DR: SAGAS是一个在离线、无模型、碎片化数据设置下，结合图辅助轨迹拼接和自动机引导规划，解决LTL约束机器人控制的框架。


<details>
  <summary>Details</summary>
Motivation: 现有LTL机器人控制方法依赖于精确的动力学模型或昂贵的在线交互。本研究旨在解决在仅有固定、任务无关的碎片化轨迹数据集的离线、无模型环境中，实现LTL约束控制的挑战。

Method: 该方法名为SAGAS，结合了图辅助轨迹拼接和自动机引导规划。首先，从学习到的时间距离表示构建潜在可达性图。其次，通过认证锚点和概率软标签增强图，以弥合语义鸿沟。然后，将LTL规范转换为Büchi自动机，并在隐式乘积空间中搜索，以导出成本最小的前缀-后缀计划。最后，部署一个子目标条件低级策略来执行这些潜在路径点。

Result: 在OGBench运动域的实验表明，SAGAS成功地为各种LTL任务合成了高效的轨迹，有效弥合了碎片化离线数据与复杂逻辑约束之间的鸿沟。

Conclusion: SAGAS框架通过结合图辅助轨迹拼接和自动机引导规划，在离线、无模型且仅使用碎片化数据的条件下，成功实现了复杂线性时序逻辑（LTL）约束下的机器人控制，展现了其有效性。
...
### [412] [A Novel MDP Decomposition Framework for Scalable UAV Mission Planning in Complex and Uncertain Environments](https://arxiv.org/abs/2512.00838)
...
### [413] [HAVEN: Hierarchical Adversary-aware Visibility-Enabled Navigation with Cover Utilization using Deep Transformer Q-Networks](https://arxiv.org/abs/2512.00592)
...
### [414] [Fast, Robust, Permutation-and-Sign Invariant SO(3) Pattern Alignment](https://arxiv.org/abs/2512.00659)
...
### [415] [Reinforcement Learning for Gliding Projectile Guidance and Control](https://arxiv.org/abs/2512.01066)
...
### [417] [Integrated YOLOP Perception and Lyapunov-based Control for Autonomous Mobile Robot Navigation on Track](https://arxiv.org/abs/2512.01608)
*Mo Chen*
...
### [419] [Dynamic Log-Gaussian Process Control Barrier Function for Safe Robotic Navigation in Dynamic Environments](https://arxiv.org/abs/2512.01668)
...
### [420] [Transforming Monolithic Foundation Models into Embodied Multi-Agent Architectures for Human-Robot Collaboration](https://arxiv.org/abs/2512.00797)
...
### [421] [NeuroHJR: Hamilton-Jacobi Reachability-based Obstacle Avoidance in Complex Environments with Physics-Informed Neural Networks](https://arxiv.org/abs/2512.01897)
...
### [422] [Magnetic Tactile-Driven Soft Actuator for Intelligent Grasping and Firmness Evaluation](https://arxiv.org/abs/2512.00907)
...
### [423] [Constant-Time Motion Planning with Manipulation Behaviors](https://arxiv.org/abs/2512.00939)
*Nayesha Gandotra,Itamar Mishani,Maxim Likhachev*

Main category: cs.RO

TL;DR: 本文提出了行为恒定时间运动规划器（B-CTMP），它扩展了现有的恒定时间运动规划（CTMP）方法，将抓取或插入等操作行为整合到规划中，为机器人操作提供具有可验证速度和成功保证的恒定时间解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前机器人操作系统多限于简单脚本化例程，缺乏可验证的安全、效率和可靠性保障。现有CTMP算法虽提供恒定时间碰撞避免查询，但未明确整合物体处理所需的关键操作行为。

Method: B-CTMP算法通过预计算紧凑数据结构，将操作任务分解为两步：1) 无碰撞运动到行为启动状态；2) 执行特定操作行为（如抓取或插入）以达到目标。这确保了在指定状态集上的恒定时间查询、完整性和任务成功。

Result: B-CTMP在货架取物和插头插入两个典型操作任务（仿真和真实机器人）中进行了评估。结果表明，B-CTMP成功将无碰撞规划和物体操作统一到单一的恒定时间框架内，为半结构化环境中的操作提供了可验证的速度和成功保证。

Conclusion: B-CTMP通过整合操作行为，扩展了恒定时间运动规划，为接触丰富的机器人操作任务提供了一个统一的、具有速度和成功率可验证保证的解决方案，特别适用于半结构化环境。
...
### [424] [H-Zero: Cross-Humanoid Locomotion Pretraining Enables Few-shot Novel Embodiment Transfer](https://arxiv.org/abs/2512.00971)
*Yunfeng Lin,Minghuan Liu,Yufei Xue,Ming Zhou,Yong Yu,Jiangmiao Pang,Weinan Zhang*

Main category: cs.RO

TL;DR: H-Zero是一个跨人形机器人运动预训练流程，旨在学习一个可泛化的人形机器人基础策略，实现零样本和少样本迁移到新型机器人。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人运动控制器高度依赖特定机器人设计，需要对奖励函数、物理参数和训练超参数进行大量调优，这阻碍了在不同平台上实现稳定高效运动的开发。

Method: 本文提出了H-Zero，一个跨人形机器人运动预训练流程。它通过在有限的机器人实体上进行预训练，学习一个可泛化的人形机器人基础策略。

Result: 预训练策略在模拟中对未见机器人实现了高达81%的完整剧集持续时间（零样本），并且能够在30分钟内通过少量微调，将策略迁移到未见过的人形机器人和直立四足机器人上。

Conclusion: H-Zero成功解决了机器人运动控制器泛化性差的问题，通过预训练学习通用策略，显著减少了新机器人平台上的开发和调优时间，提高了控制器在新机器人上的适应性和效率。
...
TL;DR: 本文提出了一种基于机器人搭载超宽带（UWB）雷达的无基础设施障碍物检测和环境建图方法，适用于能见度差的环境，并实现了较高的检测精度。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉相机和激光雷达在黑暗、烟雾或反光表面等能见度差的环境中失效。超宽带雷达在这种视觉受损条件下提供了一种有前景的替代方案。

Method: 研究了机器人搭载UWB雷达在动态、无锚点场景下环境建图的适用性，并调查了不同材料（金属、混凝土和胶合板）和UWB无线电通道（5和9）如何影响信道冲激响应（CIR）。提出了一种三步处理流程：(i) 目标识别（基于CIR峰值检测），(ii) 滤波（基于峰值特性、信噪比和到达相位差），以及 (iii) 聚类（基于距离估计和到达角估计）。

Result: 该方法成功减少了噪声和多径效应。在通道9上，即使检测低反射材料如胶合板，障碍物检测精度至少达到82.36%，召回率为89.46%。

Conclusion: 这项工作为进一步开发不依赖视觉特征且不需固定锚点进行三角测量的UWB定位与建图（SLAM）系统奠定了基础。
...
TL;DR: 本文提出CycleManip框架和基准，以端到端模仿学习方式解决机器人周期性操作任务中历史信息利用不足和缺乏评估工具的问题，并在模拟和真实环境中取得高成功率。


<details>
  <summary>Details</summary>
Motivation: 机器人周期性操作任务（如摇瓶、敲钉）在日常生活中至关重要，但却未被充分探索。现有模仿学习方法因未能有效利用历史信息而难以在预期时间内完成任务，且缺乏带有足够数据和自动评估工具的基准，阻碍了该领域的发展。

Method: 本文提出CycleManip框架，通过端到端的模仿学习实现周期性任务操作，无需额外模型、分层结构或显著计算开销。其核心在于通过成本感知采样策略增强历史感知，并通过多任务学习提高历史理解能力。此外，还引入了一个周期性任务操作基准，提供多样化的任务和自动评估方法。

Result: 在模拟和真实世界环境中的大量实验表明，CycleManip方法在周期性任务操作中实现了高成功率。结果还展示了其在通用操作中的强大适应性，以及在视觉-语言-动作（VLA）模型等模仿策略上的即插即用能力。此外，该方法可应用于双臂抓手、灵巧手和人形机器人等多种机器人平台。

Conclusion: CycleManip框架通过增强历史感知和理解，有效解决了机器人周期性操作任务中现有模仿方法的局限性。新提出的基准和自动评估工具为该领域的发展提供了支持。该方法在多种机器人平台和任务中表现出强大的性能和泛化能力。
...
### [427] [FOM-Nav: Frontier-Object Maps for Object Goal Navigation](https://arxiv.org/abs/2512.01009)
*Thomas Chabal,Shizhe Chen,Jean Ponce,Cordelia Schmid*

Main category: cs.RO

TL;DR: 本文提出了FOM-Nav框架，通过结合边界-物体地图（Frontier-Object Maps）和视觉-语言模型，解决了未知环境中物体目标导航问题，提升了探索效率和规划能力，并在多个基准测试中达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的隐式记忆方法在长期记忆保留和规划方面存在不足，而显式地图方法则缺乏丰富的语义信息，这限制了机器人在未知环境中高效寻找目标物体的能力。

Method: FOM-Nav是一个模块化框架，它在线构建边界-物体地图，联合编码空间边界和细粒度物体信息。利用这种表示，视觉-语言模型执行多模态场景理解和高级目标预测，然后由低级规划器生成高效轨迹。为训练FOM-Nav，研究人员从真实世界扫描环境中自动构建了大规模导航数据集。

Result: FOM-Nav在MP3D和HM3D基准测试中取得了最先进的性能，特别是在导航效率指标SPL上表现出色，并在真实机器人上获得了有前景的结果。实验验证了模型设计和数据集的有效性。

Conclusion: FOM-Nav通过创新的边界-物体地图和视觉-语言模型结合，有效解决了物体目标导航中的长期记忆和语义信息缺乏问题，显著提升了导航效率和性能，展现了在真实世界应用中的潜力。
...
### [428] [VLASH: Real-Time VLAs via Future-State-Aware Asynchronous Inference](https://arxiv.org/abs/2512.01031)
*Jiaming Tang,Yufei Sun,Yilong Zhao,Shang Yang,Yujun Lin,Zhuoyang Zhang,James Hou,Yao Lu,Zhijian Liu,Song Han*

Main category: cs.RO

TL;DR: VLASH是一个通用的异步推理框架，旨在解决视觉-语言-动作（VLA）模型在机器人任务中部署时存在的动作延迟和不稳定问题，实现平滑、准确、快速的控制，且无额外开销。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人任务中能力日益增强，但实际部署时存在效率低下、动作卡顿和反应延迟问题。异步推理虽能提高速度，但会因预测与执行之间的时序错位导致动作不稳定，而现有方法要么牺牲精度要么增加运行时开销。

Method: VLASH通过利用先前生成的动作块向前滚动机器人状态，估计未来的执行时间状态，从而弥合预测与执行之间的时间差，实现同步预测和执行。

Result: VLASH与同步推理相比，速度提升高达2.03倍，反应延迟减少高达17.4倍，同时完全保持原有精度。它使VLA模型能够成功处理传统同步推理无法完成的快速反应、高精度任务，如打乒乓球和打地鼠。

Conclusion: VLASH提供了一种有效且无额外开销的异步推理框架，显著提高了VLA模型在实际机器人任务中的部署效率、反应速度和控制稳定性，尤其适用于需要快速反应和高精度的场景。
...
### [429] [Autonomous Grasping On Quadruped Robot With Task Level Interaction](https://arxiv.org/abs/2512.01052)
*Muhtadin,Mochammad Hilmi Rusydiansyah,Mauridhi Hery Purnomo,I Ketut Eddy Purnama,Chastine Fatichah*

Main category: cs.RO

TL;DR: 本研究开发了一种四足机器人自主抓取系统，通过集成机械臂和夹持器，并利用ROS分层控制和GraspNet，实现了导航、物体检测和抓取，在实际场景中抓取成功率达到75%。


<details>
  <summary>Details</summary>
Motivation: 现有四足机器人主要侧重于移动性，缺乏物体操作能力。为四足机器人配备机械臂和夹持器后，其手动控制，特别是在远程场景下，变得极具挑战。

Method: 该系统包括将机械臂和夹持器集成到四足机器人本体，设计基于ROS的分层控制系统，以及开发一个基于网络的HRI（人机交互）界面。机器人能够利用GraspNet自主执行导航、物体检测和抓取等任务。通过真实场景测试评估了导航、物体选择与抓取以及用户体验。

Result: 机器人能够准确且稳定地执行任务，在12次试验中取得了75%的抓取成功率。

Conclusion: 该系统在增强四足机器人作为服务机器人在实际环境中的能力方面展现出巨大的潜力。
...
### [430] [Opening the Sim-to-Real Door for Humanoid Pixel-to-Action Policy Transfer](https://arxiv.org/abs/2512.01061)
...
### [431] [Supporting Productivity Skill Development in College Students through Social Robot Coaching: A Proof-of-Concept](https://arxiv.org/abs/2512.01105)
...
### [432] [Estimation of Kinematic Motion from Dashcam Footage](https://arxiv.org/abs/2512.01104)
...
TL;DR: 本文介绍了一种名为COMET的同轴双旋翼AAV平台，其采用双倾斜盘机制，旨在解决同轴双旋翼系统在效率、机动性和紧凑性之间平衡的挑战，并通过实验验证了其在效率、鲁棒性和机动性方面的提升。


<details>
  <summary>Details</summary>
Motivation: 同轴双旋翼自主飞行器（AAV）因其潜在的转子系统效率提升和紧凑外形而受到关注。然而，在同轴双旋翼系统中平衡效率、机动性和紧凑性仍是关键设计挑战，限制了其实际部署。

Method: 引入了COMET平台，一个采用双倾斜盘机制的同轴双旋翼AAV。通过台架测试优化了同轴双旋翼系统的效率和紧凑性。通过飞行续航实验验证了在不同载荷条件下的整体原型效率和鲁棒性。通过综合轨迹跟踪测试评估了系统的机动性性能。

Result: 双倾斜盘配置与单倾斜盘方案相比，提高了跟踪性能和飞行效率。在各种场景下成功的自主飞行试验验证了COMET在实际应用中的潜力。

Conclusion: COMET同轴双旋翼AAV平台通过双倾斜盘设计，有效提升了效率、鲁棒性和机动性，展现了在实际应用中的巨大潜力。
...
TL;DR: 本文提出了一种新颖的去中心化激光雷达蜂群跟踪框架，用于在复杂环境中实现可见性感知、协作式的目标跟踪，充分利用蜂群系统的独特优势。


<details>
  <summary>Details</summary>
Motivation: 尽管单无人机跟踪系统已被广泛研究，但基于蜂群的目标跟踪系统仍未得到充分探索，尽管它在分布式感知、容错冗余和多方向目标覆盖方面具有独特优势。

Method: 该方法引入了一种去中心化激光雷达蜂群跟踪框架。为解决可见性问题，提出了一种基于球形符号距离场（SSDF）的3D环境遮挡表示度量，并结合高效算法实现实时SSDF更新。设计了支持异构激光雷达配置的通用视场（FOV）对齐成本，以确保一致的目标观测。通过合作成本增强蜂群协调，包括机器人间安全间距、防止相互遮挡，并通过受静电势启发的分布度量促进3D多方向目标包围。这些创新集成到一个分层规划器中，结合运动学前端搜索器和时空SE(3)后端优化器，生成无碰撞、可见性优化的轨迹。系统在异构激光雷达蜂群上实现完全去中心化部署，具有协作感知、分布式规划和动态蜂群重构能力。

Result: 在混乱的室外环境中，通过严格的真实世界实验验证，该系统展示了对敏捷目标（无人机、人类）的鲁棒协作跟踪能力，并实现了卓越的可见性维持。

Conclusion: 所提出的去中心化激光雷达蜂群跟踪系统成功实现了鲁棒且可见性优化的协作蜂群跟踪，弥补了蜂群目标跟踪领域的研究空白。
...
TL;DR: 本文利用深度强化学习和课程学习训练人形机器人自主发现防摔策略，通过形成“三角形”结构显著减少跌落损害，并成功将其迁移至真实机器人平台。


<details>
  <summary>Details</summary>
Motivation: 人形机器人因其形态、动力学和控制策略的限制，相较于其他机器人更容易跌落，且其大重量、高重心和高自由度在失控跌落时会对自身及周围物体造成严重硬件损坏。现有研究多依赖基于控制的方法，难以适应多样化的跌落场景且可能引入不合适的人类先验。

Method: 研究采用大规模深度强化学习（Deep Reinforcement Learning, DRL）和课程学习（Curriculum Learning）方法。通过精心设计的奖励函数和域多样化课程，激励人形智能体探索跌落保护策略，并发现形成“三角形”结构能显著减少刚性材料机体的跌落损害。

Result: 研究成功训练了人形智能体，使其能够探索跌落保护行为，并发现通过形成“三角形”结构可以显著减少跌落损害。通过全面的指标和实验，量化了其性能并与其他方法进行了比较，可视化了其跌落行为，并成功将其策略迁移到了真实世界平台。

Conclusion: 通过深度强化学习和课程学习，人形机器人能够自主发现适合自身特性和属性的跌落保护策略（如形成“三角形”结构），从而有效减少跌落损害，并能成功应用于实际机器人平台。
...
TL;DR: 本文提出了一种模态增强微调框架，旨在使基础机器人策略适应不同的人形机器人，并通过对GR1和Unitree G1的实验验证了其有效性，结果表明模态增强能显著提升策略性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决如何将基础机器人策略有效地适应到多样化的人形机器人实体上。

Method: 该研究提出了一种模态增强微调框架。在GR1实体上，利用公开数据集并引入后处理模态（如二值接触信号和ZoeDepth生成的度量深度）；在Unitree G1实体上，贡献了一个包含cuRobo运动规划、逆运动学和地面真实接触力测量的新型多模态数据集。通过这两种设置验证了方法。

Result: 模态增强持续提升了不同实体上的策略性能。对于GR1，结合接触状态线索和RGB-D融合将在线成功率从51%提高到63%。在G1的“Pick Apple to Bowl”任务中，接触增强模型实现了94%的成功率，显著优于标准微调的48%和零样本迁移的0%。结果表明轻量级后处理对GR1有效，而高质量多模态数据对Unitree G1的可靠迁移至关重要。

Conclusion: 这项工作建立了一个统一的、以数据为中心的方法，通过有针对性的模态设计和多模态微调来扩展基础机器人策略。
...
TL;DR: 针对机器人操控中材料泛化困难的问题，特别是透明或反光物体，本文提出M$^3$A框架。它利用计算摄影和光传输，将单一真实演示光度重渲染为多种不同材料属性的演示，从而有效提升策略对材料的泛化能力，无需额外数据收集。


<details>
  <summary>Details</summary>
Motivation: 机器人需要在真实世界中处理各种材料的物体，但透明或反光材料（如玻璃、金属）引入的视觉和物理特性差异巨大，对现有方法构成严峻挑战。现有方法要么依赖模拟器进行域迁移（存在视觉域鸿沟），要么需要大量昂贵的真实世界数据（收集困难且不足以覆盖所有材料）。

Method: 本文引入了“可变材料操控增强”（M$^3$A）框架，利用计算摄影和光传输的物理特性进行光度重渲染。核心思想是：给定一个单一的真实世界演示，通过光度重渲染生成一系列具有不同材料属性的高度逼真的演示。这种增强有效解耦了任务特定的操控技能与物体表面外观，使得策略无需额外数据收集即可泛化到不同材料。

Result: 为了系统评估，本文构建了首个涵盖模拟和真实环境的多材料操控基准。实验结果表明，M$^3$A策略显著增强了跨材料泛化能力，在三个真实世界任务中的平均成功率提高了58.03%，并对之前未见的材料表现出鲁棒性能。

Conclusion: M$^3$A框架通过利用计算摄影进行光度重渲染，成功解决了机器人操控中材料泛化的挑战。它能够从单一真实演示中生成多样化的材料变体，有效提升了机器人策略对各种材料的泛化能力，包括以前未见的材料，而无需大量数据收集。
...
TL;DR: NavForesee是一个新型的视觉-语言模型（VLM），它在一个统一框架内结合了高级语言规划和预测性世界模型想象，旨在解决具身导航中长周期任务和复杂自然语言指令的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有具身智能体在面对复杂自然语言指令引导的长周期任务时，难以对未知环境进行鲁棒的长期规划，导致高失败率。

Method: NavForesee将高层语言规划与预测性世界模型想象统一起来。模型通过分解任务、跟踪进度和制定子目标来理解导航指令。同时，它作为一个生成式世界模型，通过预测短期环境动态和长期导航里程碑提供关键的预见性。VLM的结构化计划指导其有针对性的预测，而想象的未来则为导航行动提供丰富的上下文，形成一个强大的感知-规划/预测-行动的内部反馈循环。

Result: NavForesee在R2R-CE和RxR-CE基准测试的复杂场景中取得了极具竞争力的性能。

Conclusion: 该研究强调了将显式语言规划与隐式时空预测融合的巨大潜力，为开发更智能、更有能力的具身智能体铺平了道路。
...
### [444] [A Cross-Embodiment Gripper Benchmark for Rigid-Object Manipulation in Aerial and Industrial Robotics](https://arxiv.org/abs/2512.01598)
*Marek Vagas,Martin Varga,Jaroslav Romancik,Ondrej Majercak,Alejandro Suarez,Anibal Ollero,Bram Vanderborght,Ivan Virgala*

Main category: cs.RO

TL;DR: 本文提出了跨平台夹持器基准（CEGB），该基准扩展了现有评估指标，以衡量机械臂夹持器在不同机器人平台间的可转移性和能源效率，并使用一个轻量级自锁夹持器原型进行了验证。


<details>
  <summary>Details</summary>
Motivation: 现有基准（如YCB和NIST）只评估单一平台上的抓取成功率、力或时间，但未能评估跨平台可转移性或能源感知性能，而这些能力对于现代移动和空中操作至关重要。

Method: 引入了跨平台夹持器基准（CEGB），该基准在YCB和NIST选定指标的基础上增加了三个组件：1) 转移时间基准，衡量更换夹持器平台所需的实际工作量；2) 能源消耗基准，评估抓取和保持效率；3) 意图特定理想有效载荷评估，反映设计相关的操作能力。论文还实现了一个轻量级自锁夹持器原型作为参考案例。

Result: 实验表明，夹持器原型实现了快速平台转移（用户组中位数约17.6秒），低保持能耗（每10秒约1.5焦耳），以及一致的抓取性能（循环时间3.2-3.9秒，成功率超过90%）。

Conclusion: CEGB为在空中和机械臂领域对夹持器进行跨平台、能源感知的评估提供了一个可复现的基础。
...
### [445] [DiG-Flow: Discrepancy-Guided Flow Matching for Robust VLA Models](https://arxiv.org/abs/2512.01715)
*Wanpeng Zhang,Ye Wang,Hao Luo,Haoqi Yuan,Yicheng Feng,Sipeng Zheng,Qin Jin,Zongqing Lu*

Main category: cs.RO

TL;DR: DiG-Flow通过几何正则化增强了基于流匹配的视觉-语言-动作（VLA）模型的鲁棒性，特别是在分布偏移和复杂多步任务上。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在分布偏移和复杂多步任务上性能下降，表明其学习到的表示可能未能稳健地捕捉任务相关的语义。

Method: DiG-Flow引入了一个原则性框架，通过几何正则化提高VLA鲁棒性。其核心思想是，观察和动作嵌入之间的分布差异提供了一个有意义的几何信号。DiG-Flow计算观察和动作嵌入经验分布之间的差异度量，通过单调函数将其映射为调制权重，并在流匹配之前对观察嵌入应用残差更新。此干预在表示层面操作，不修改流匹配路径或目标向量场。该方法提供理论保证，证明其能减少训练目标，且引导推理细化收敛。

Result: DiG-Flow可以集成到现有VLA架构中，开销可忽略不计，并持续提高性能，尤其在复杂多步任务和有限训练数据下表现出显著提升。

Conclusion: DiG-Flow是一个有效的框架，通过几何正则化增强了VLA模型的鲁棒性，显著提高了其在挑战性场景下的性能，且不改变流匹配的核心机制。
...
### [446] [L2M-Calib: One-key Calibration Method for LiDAR and Multiple Magnetic Sensors](https://arxiv.org/abs/2512.01554)
*Qiyang Lyu,Wei Wang,Zhenyu Wu,Hongming Shen,Huiqin Zhou,Danwei Wang*

Main category: cs.RO

TL;DR: 本文提出了一种名为L2M-Calib的新型一键式校准框架，用于融合磁传感器和激光雷达系统，联合估计两种传感器之间的外参变换和磁传感器的内参畸变，以实现鲁棒的环境感知。


<details>
  <summary>Details</summary>
Motivation: 多模态传感器融合通过利用异构传感模式的互补信息实现鲁棒的环境感知，但准确的校准是有效融合的关键先决条件。磁传感器能捕捉环境磁场（AMF）模式，这些模式不受几何、纹理、光照和天气的影响，适用于挑战性环境。然而，由于缺乏有效的校准技术，磁传感在多模态系统中的集成仍未得到充分探索。

Method: 该研究提出L2M-Calib框架，通过迭代高斯-牛顿方案优化外参，并结合加权岭正则化全最小二乘（w-RRTLS）问题进行内参校准，以确保对测量噪声和病态数据的鲁棒性。该框架能联合估计两种传感器之间的外参变换和磁传感器的内参畸变。

Result: 在模拟数据集和真实世界实验（包括AGV搭载传感器配置）上的广泛评估表明，该方法在各种环境和操作条件下均实现了高校准精度和鲁棒性。

Conclusion: L2M-Calib框架有效解决了磁-激光雷达融合系统的校准难题，通过联合估计外参和磁传感器内参畸变，实现了在挑战性环境下的高精度和鲁棒性校准，为磁传感在多模态系统中的集成铺平了道路。
...
### [447] [AgriLiRa4D: A Multi-Sensor UAV Dataset for Robust SLAM in Challenging Agricultural Fields](https://arxiv.org/abs/2512.01753)
*Zhihao Zhan,Yuhang Ming,Shaobin Li,Jie Yuan*

Main category: cs.RO

TL;DR: AgriLiRa4D是一个针对挑战性农业环境设计的无人机多模态数据集，用于SLAM和定位研究，填补了现有数据集的空白。


<details>
  <summary>Details</summary>
Motivation: 农业无人机执行喷洒、测量和检查等任务时，多传感器同步定位与建图（SLAM）至关重要。然而，缺乏用于研究鲁棒操作的真实世界、多模态农业无人机数据集。

Method: 本研究提出了AgriLiRa4D数据集，涵盖平坦、丘陵和梯田三种农田类型，以及边界和覆盖两种操作模式，共六组飞行序列。数据集提供高精度的光纤惯性导航系统（FINS_RTK）真值轨迹，以及同步的3D LiDAR、4D Radar和IMU测量数据，并附带完整的内外参标定。为展示其效用，研究还在不同传感器组合下，对四种先进的多传感器SLAM算法进行了基准测试。

Result: AgriLiRa4D数据集支持多样化的SLAM和定位研究，并能对低纹理作物、重复模式、动态植被等农业环境挑战下的鲁棒性进行严格评估。基准测试结果突出显示了所提序列的难度以及多模态方法对于实现可靠无人机定位的必要性。

Conclusion: AgriLiRa4D数据集填补了农业SLAM数据集的关键空白，为研究社区提供了一个有价值的基准，并有助于推动农业无人机自主导航技术的发展。
...
### [448] [IGen: Scalable Data Generation for Robot Learning from Open-World Images](https://arxiv.org/abs/2512.01773)
...
TL;DR: IGen是一个框架，它能够从开放世界图像中可扩展地生成逼真的视觉观测和可执行动作，从而为通用机器人策略训练提供大规模数据，弥补了现实世界机器人数据收集的不足。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略对大规模训练数据需求巨大，但机器人本体数据收集成本高昂且受环境限制。开放世界图像虽然多样且与机器人操作任务相关，但缺乏关联的机器人动作，导致其丰富的视觉资源未被充分利用。

Method: IGen首先将非结构化的2D像素转换为结构化的3D场景表示，以便进行场景理解和操作。然后，它利用视觉-语言模型的推理能力，将特定场景的任务指令转化为高级规划，并生成SE(3)末端执行器姿态序列作为低级动作。最后，它根据这些姿态合成动态场景演变并渲染出时间连贯的视觉观测。

Result: 实验验证了IGen生成视动数据的质量很高。仅使用IGen合成数据训练的策略，其性能与使用真实世界数据训练的策略相当。

Conclusion: IGen框架展示了从开放世界图像生成可扩展数据以支持通用机器人策略训练的巨大潜力。
...
### [449] [GR-RL: Going Dexterous and Precise for Long-Horizon Robotic Manipulation](https://arxiv.org/abs/2512.01801)
...
### [450] [Much Ado About Noising: Dispelling the Myths of Generative Robotic Control](https://arxiv.org/abs/2512.01809)
...
### [451] [Is Image-based Object Pose Estimation Ready to Support Grasping?](https://arxiv.org/abs/2512.01856)
...
### [452] [Real-World Robot Control by Deep Active Inference With a Temporally Hierarchical World Model](https://arxiv.org/abs/2512.01924)
...
### [453] [Guardian: Detecting Robotic Planning and Execution Errors with Vision-Language Models](https://arxiv.org/abs/2512.01946)
...
### [454] [RoaD: Rollouts as Demonstrations for Closed-Loop Supervised Fine-Tuning of Autonomous Driving Policies](https://arxiv.org/abs/2512.01993)
...
### [455] [Learning Sim-to-Real Humanoid Locomotion in 15 Minutes](https://arxiv.org/abs/2512.01996)
...
### [456] [LLM-Driven Corrective Robot Operation Code Generation with Static Text-Based Simulation](https://arxiv.org/abs/2512.02002)
...
### [457] [Learning Dexterous Manipulation Skills from Imperfect Simulations](https://arxiv.org/abs/2512.02011)
*Elvis Hsieh,Wen-Han Hsieh,Yen-Jen Wang,Toru Lin,Jitendra Malik,Koushil Sreenath,Haozhi Qi*

Main category: cs.RO

TL;DR: 本文提出了一种名为DexScrew的sim-to-real框架，通过结合模拟训练、真实世界示范收集和行为克隆，有效解决了灵巧操作中复杂接触动力学和多感官信号（特别是触觉反馈）的模拟难题，并在螺母螺栓紧固和螺丝刀操作任务上展现出强大的泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 灵巧操作在强化学习和sim-to-real迁移方面取得了显著进展，但仍受限于模拟复杂接触动力学和多感官信号（尤其是触觉反馈）的难度。

Method: 该框架包含三个阶段：1. 在模拟环境中，使用简化物体模型训练强化学习策略，以学习正确的指法步态。2. 将学到的策略作为技能原语，在远程操作系统中收集包含触觉和本体感觉信息的真实世界示范。3. 训练一个结合触觉感知的行为克隆策略，以实现对不同几何形状物体的泛化。

Result: 实验结果表明，与直接sim-to-real迁移相比，该方法在两项任务中均实现了更高的任务进展率，即使面对未曾见过的物体形状和外部扰动，也能表现出鲁棒的性能。

Conclusion: 所提出的DexScrew框架有效解决了灵巧操作中sim-to-real迁移的局限性，特别是复杂接触和多感官信号的模拟挑战，并通过触觉感知实现了对多样化物体的高效泛化和鲁棒操作。
...
### [458] [EfficientFlow: Efficient Equivariant Flow Policy Learning for Embodied AI](https://arxiv.org/abs/2512.02020)
*Jianlei Chang,Ruofeng Mei,Wei Ke,Xiangyu Xu*

Main category: cs.RO

TL;DR: EfficientFlow是一个统一的流基策略学习框架，通过引入等变性以提高数据效率，并提出加速正则化策略以加快采样速度，从而解决了具身AI中生成式策略的数据和采样效率问题。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式策略在具身AI任务中存在数据效率低下（需要大量演示数据）和采样效率低下（推理时动作生成缓慢）的问题。

Method: 为提高数据效率，将等变性引入流匹配，并从理论上证明使用各向同性高斯先验和等变速度预测网络时，动作分布保持等变性。为加速采样，提出了一种新颖的加速正则化策略，并推导出一个替代损失，使得仅使用条件轨迹即可进行稳定且可扩展的训练。

Result: 在各种机器人操作基准测试中，该算法在有限数据下实现了具有竞争力或卓越的性能，同时显著加快了推理速度。

Conclusion: EfficientFlow为高性能具身AI提供了一个强大且高效的范式。
...
### [459] [ManualVLA: A Unified VLA Model for Chain-of-Thought Manual Generation and Robotic Manipulation](https://arxiv.org/abs/2512.02013)
...
TL;DR: 本文提出ManualVLA，一个统一的视觉-语言-动作（VLA）框架，通过生成多模态操作手册和链式思维（ManualCoT）推理，将目标状态转化为可执行步骤，显著提升了VLA模型在乐高组装和物体重新排列等长周期任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在面对需要明确目标状态的长周期任务（如乐高组装或物体重新排列）时，难以协调高层规划与精确操作。研究旨在赋予VLA模型从“结果”（what）推断“过程”（how）的能力，从而将目标状态转化为可执行的程序。

Method: 本文引入ManualVLA，一个基于混合Transformer（MoT）架构的统一VLA框架。它首先配备一个规划专家，从目标状态生成包含图像、位置提示和文本指令的多模态中间操作手册。在此基础上，设计了Manual Chain-of-Thought（ManualCoT）推理过程，将这些手册输入到动作专家，其中每个手册步骤提供显式控制条件和隐式指导。为缓解数据收集负担，开发了一个基于3D高斯泼溅的高保真数字孪生工具包，自动生成规划专家训练所需的操作手册数据。

Result: ManualVLA在乐高组装和物体重新排列任务中展现出强大的真实世界性能，平均成功率比之前的分层SOTA基线高出32%。

Conclusion: ManualVLA通过多模态操作手册生成和ManualCoT推理，有效解决了现有VLA模型在长周期任务中规划与操作协调的挑战，实现了从目标状态到可执行过程的有效转换，显著提高了任务成功率。
...
### [460] [Distributed Observer and Controller Design for Linear Systems: A Separation-Based Approach](https://arxiv.org/abs/2512.00393)
*Ganghui Cao,Xunyuan Yin*

Main category: eess.SY

TL;DR: 本文提出了一种基于共识的分布式控制框架，用于处理具有未知输入的线性时不变多通道系统，实现了状态估计、稳定和跟踪控制。


<details>
  <summary>Details</summary>
Motivation: 研究具有未知输入的线性时不变多通道系统的共识分布式控制问题，以实现状态估计和控制任务。

Method: 提出了一种分布式观测器-控制器框架，其中观测器节点和控制器节点协同执行状态估计和控制任务。采用不连续共识策略和两种专门用于处理未知输入的自适应算法。关键在于证明了分布式观测器和控制器可以独立设计，类似于经典的分离原理。

Result: 该框架能够有效地实现状态估计、系统稳定和跟踪控制目标。理论分析和数值仿真结果均验证了其有效性。

Conclusion: 所提出的分布式控制框架能有效处理具有未知输入的线性时不变多通道系统，并通过不连续共识和自适应算法实现了分布式观测器和控制器的独立设计，展现了分布式环境下的分离原理。
...
### [461] [A Highly Configurable Framework for Large-Scale Thermal Building Data Generation to drive Machine Learning Research](https://arxiv.org/abs/2512.00483)
*Thomas Krug,Fabian Raisch,Dominik Aimer,Markus Wirnsberger,Ferdinand Sigg,Felix Koch,Benjamin Schäfer,Benjamin Tischler*

Main category: eess.SY

TL;DR: 本文提出BuilDa，一个热力学建筑数据生成框架，旨在为机器学习研究提供大量高质量的合成数据，且无需专业的建筑模拟知识。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的建筑热力学建模对于大规模智能建筑控制至关重要，但机器学习研究所需的庞大、高质量建筑热力学数据难以获取。现有的经验性公开数据集和数据生成器在数据质量和数量上均不满足需求，且现有生成方法通常需要建筑模拟专家知识。

Method: BuilDa框架使用一个单区域的Modelica模型，将其导出为功能模型单元（FMU），并在Python中进行模拟以生成合成数据。该框架设计为无需深厚的建筑模拟知识即可生成大量数据。

Result: 通过BuilDa生成数据并将其应用于一个迁移学习研究，对486个数据驱动模型进行了微调，展示了其有效性。

Conclusion: BuilDa成功解决了机器学习研究中建筑热力学数据稀缺的问题，提供了一个无需专业知识即可生成大量高质量合成数据的解决方案。
...
### [462] [Dependent Reachable Sets for the Constant Bearing Pursuit Strategy](https://arxiv.org/abs/2512.00273)
*Venkata Ramana Makkapati,Tulasi Ram Vechalapu,Vinodhini Comandur,Seth Hutchinson*

Main category: eess.SY

TL;DR: 本文提出并分析了一种新颖的常方位角追踪策略下的可达性问题，研究了跟随者可达集的几何形状，推导了相关界限，并通过仿真验证了其形状，同时提出了一个优化问题。


<details>
  <summary>Details</summary>
Motivation: 研究一个代理通过常方位角追踪策略跟随另一个代理的场景下，跟随者可达集的几何特性，并引入一个新颖的可达性问题。

Method: 理论推导了相关依赖可达集的界限；通过仿真结果经验性地确定了依赖可达集的形状；公式化并分析了一个关于常方位角策略的优化问题。

Result: 推导了关键理论结果，为依赖可达集提供了界限；仿真结果经验性地确立了依赖可达集的形状；提出并分析了一个原创的常方位角策略优化问题。

Conclusion: 成功引入并分析了常方位角追踪策略下的可达性问题，推导了可达集的理论界限，并通过仿真验证了其形状，并提出了一个相关的优化问题。
...
### [463] [Distributionally Robust Acceleration Control Barrier Filter for Efficient UAV Obstacle Avoidance](https://arxiv.org/abs/2512.00462)
*Dnyandeep Mandaokar,Bernhard Rinner*

Main category: eess.SY

TL;DR: 本文提出了一种名为分布鲁棒加速度控制障碍函数（DR-ACBF）的高效碰撞避免方法，用于无人机（UAV）动态障碍物规避，该方法在有限资源下提供快速反应，并能处理延迟、执行器限制和不确定性，同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 无人机动态障碍物规避需要快速反应，但受限于机载资源。此外，还需要有效处理延迟、执行器限制以及障碍物的加速度，并减轻不确定性带来的风险。

Method: 该方法引入了分布鲁棒加速度控制障碍函数（DR-ACBF），将二阶控制障碍函数构建为指令加速度的线性半空间约束。通过考虑动力学和延迟的有效间隙来处理延迟、执行器限制和障碍物加速度。利用Cantelli收紧和每个障碍物的风险来缓解不确定性。采用基于DR-CVaR的早期触发机制来扩大接近违规时的裕度。通过常数时间高斯-赛德尔投影确保实时执行。

Result: 仿真研究表明，该方法在实现相似的避障性能的同时，计算工作量显著低于现有先进基线方法。Crazyflie无人机实验也验证了该方法的可行性。

Conclusion: DR-ACBF是一种高效且可行的无人机动态障碍物规避方法，能够在有限资源下提供强大的安全性，并显著降低计算成本。
...
### [464] [Datamodel-Based Data Selection for Nonlinear Data-Enabled Predictive Control](https://arxiv.org/abs/2512.00276)
*Jiachen Li,Shihao Li,Dongmei Chen*

Main category: eess.SY

TL;DR: 本文提出一种基于数据模型的任务感知数据选择方法，用于非线性系统的数据驱动预测控制(DeePC)，通过学习上下文相关的列重要性分数，显著优于传统的几何邻近选择方法。


<details>
  <summary>Details</summary>
Motivation: 现有针对非线性DeePC的数据选择方法（基于几何邻近）忽略了控制目标，导致即使在相同操作点，不同参考轨迹也可能需要不同数据。这种方法未能充分捕捉数据对特定控制任务性能的实际影响。

Method: 本文提出一种基于数据模型的方法，学习一个上下文相关的“影响函数”，将当前初始轨迹和参考轨迹映射到数据列的重要性分数。该方法借鉴机器学习中的线性数据模型框架，将闭环成本建模为列包含指示器的线性函数，其中系数取决于控制上下文。通过闭环仿真进行训练，以识别哪些数据列能实际提升特定控制任务的跟踪性能。

Result: 实验结果表明，本文提出的任务感知数据选择方法显著优于基于几何启发式的方法，尤其是在使用小数据子集时，性能提升更为明显。

Conclusion: 通过引入任务感知的数据选择机制，本文方法为非线性系统DeePC提供了一种更有效的数据利用策略，能够根据具体的控制任务选择最相关的数据，从而提高控制性能。
...
### [465] [Cooperative Safety Intelligence in V2X-Enabled Transportation: A Survey](https://arxiv.org/abs/2512.00490)
*Jiaxun Zhang,Qian Xu,Zhenning Li,Chengzhong Xu,Keqiang Li*

Main category: eess.SY

TL;DR: 这篇综述论文通过统一的“感知-推理-决策”（SPD）框架，系统地分析了V2X合作在交通安全领域的最新进展，强调了共享证据、预测推理和以人为本的干预措施，并指出了时间、信任和人因等关键制约因素。它还通过文献计量学研究评估了现有工作，并提出了未来的研究路线图。


<details>
  <summary>Details</summary>
Motivation: V2X合作正在将交通安全从以自我为中心的问题转变为集体智能问题。本研究旨在构建一个统一的框架来组织和理解V2X合作中分布式感知、协作感知和协调决策如何共同实现安全，并超越传统的链路协议和消息格式，关注共享证据、预测推理和人性化干预如何主动降低风险。

Method: 本研究采用以下方法：1) 提出了一个统一的“感知-推理-决策”（SPD）框架来结构化V2X安全领域；2) 在SPD视角下，综合了协作感知、多模态预测和风险感知规划的进展；3) 将时间、信任和人因确定为影响预测洞察力的关键跨领域制约因素；4) 通过遵循PRISMA指南的文献计量学研究，系统分析了2016-2025年数百篇出版物的研究演变和评估空白。

Result: 本研究结果包括：1) 建立了一个统一的SPD框架，形式化了安全如何从分布式感知、协作感知和协调决策的互动中产生；2) 强调了跨层耦合如何将孤立的检测转化为可校准、可操作的理解；3) 识别了时间、信任和人因是决定预测洞察力能否及时、可靠且以人类和自动化控制器可用形式交付的关键制约因素；4) 与现有V2X安全综述相比，本工作围绕正式的SPD安全循环组织了文献，并通过文献计量学研究系统分析了研究演变和评估差距。

Conclusion: 本综述为合作安全智能绘制了路线图，概述了基于SPD的设计原则和评估实践，以指导下一代V2X安全系统的开发。研究强调了SPD框架在理解和促进V2X合作安全方面的潜力，并指出了未来研究需关注的关键挑战和方向。
...
### [466] [From Range Loss to Recovery - Cold Weather Challenges and Design Strategies for Commercial Electric Vehicle Fleets](https://arxiv.org/abs/2512.00541)
*Soham Ghosh,Arpit Bohra,Karthik Saikumar*

Main category: eess.SY

TL;DR: 北美商用电动汽车快速增长，但在寒冷天气下面临性能下降、能耗增加和电网压力。本研究分析了这些影响，并提出了电池换电站和混合充电的解决方案。


<details>
  <summary>Details</summary>
Motivation: 北美商用电动汽车销量激增，但寒冷天气（-25°F至25°F）对其性能（能效降低、充电时间延长）和电网运行的影响日益成为关键挑战。

Method: 本研究对商用电动汽车在寒冷天气条件下的性能退化进行了系统分析。利用真实车队参数进行了蒙特卡洛模拟。

Result: 模拟结果显示，在严寒事件中，美国每天可能需要额外200兆瓦时（MWh）的能源来弥补效率损失，这将加剧冬季电网的压力并增加可靠性风险。此外，观察到寒冷天气充电行为导致谐波失真增加，引发对电能质量的担忧。

Conclusion: 为应对这些挑战，研究提出了两项缓解策略：1) 在温控环境下运行的“设计集成安全”电池换电站模型，以显著减少充电停机时间；2) 结合路边快速充电和车队基地深度充电的混合架构，以支持车队持续运行而不影响续航里程。这些干预措施为商用电动汽车在寒冷气候下的弹性整合奠定了坚实基础。
...
### [467] [KinesCeTI: A Modular and Size-Adaptable Force Feedback Glove with Interchangeable Actuation for the Index and Thumb](https://arxiv.org/abs/2512.00622)
*Pablo Alvarez Romeo,Mehmet Ercan Altinsoy*

Main category: eess.SY

TL;DR: 本文介绍了一种名为KinesCeTI的模块化力反馈外骨骼手套，适用于食指和拇指，具有高度适应性，支持被动和主动反馈，并通过两种不同的驱动模块进行了测试，证明了其在触觉应用中的多功能性。


<details>
  <summary>Details</summary>
Motivation: 现有的力反馈手套在适应性、反馈精细度和固定架构方面存在局限性，限制了力反馈的多功能性。研究旨在解决这些挑战，开发一种更具通用性和适应性的设备。

Method: KinesCeTI是一种模块化力反馈外骨骼，针对食指和拇指设计，可适应多种手型。它包含可互换的指套（用于指尖或指节连接）和支持被动及主动反馈的双向肌腱传动系统。该系统结合了模块化驱动设计，可附加不同的反馈系统。研究使用了两种驱动模块进行测试：一种用于被动反馈的柔性棘轮-棘爪制动机制，以及一种用于可变主动反馈的新型单向离合器。系统通过三项用户研究（每项20名参与者）进行了评估，考察了人体工程学、驱动性能以及在真实和虚拟任务中的可用性。

Result: 研究结果表明，该手套能够适应不同手型，并且通过两种机制都能提供有效的反馈。

Conclusion: KinesCeTI具有作为触觉研究通用平台的潜力，其模块化设计和有效的反馈能力使其成为一个多功能设备。
...
### [468] [Constraint-Aware Grid-Forming Control for Current Limiting](https://arxiv.org/abs/2512.00644)
*Dominic Groß*
...
### [469] [Data-Driven Computation of Polytopic Invariant Sets for Noisy Nonlinear Systems](https://arxiv.org/abs/2512.00629)
...
### [470] [Active Learning of Fractional-Order Viscoelastic Model Parameters for Realistic Haptic Rendering](https://arxiv.org/abs/2512.00667)
...
### [471] [DM-MPPI: Datamodel for Efficient and Safe Model Path Integral Control](https://arxiv.org/abs/2512.00759)
...
### [472] [Analysis of Optimal Thrust to Mass Ratio Requirement for Maximizing Payload Mass of Lunar Landing Mission](https://arxiv.org/abs/2512.00767)
...
### [473] [Mechatronic Design, Dynamic Modeling, and Real-Time Control of a Movable Scaffold](https://arxiv.org/abs/2512.00906)
*M. Özgün Güleç,Koray K. Şafak*

Main category: eess.SY

TL;DR: 本研究提出了一种新型可移动脚手架系统，涵盖其机电设计、动态建模、仿真和实时控制实验，该系统采用索驱动机制，在建筑物外表面实现了良好的定位性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一种新型可移动脚手架系统，能够定位在建筑物外表面，以应对传统脚手架的局限性。

Method: 本研究采用了以下方法：1. 机电设计了一个3自由度可移动平台。2. 平台由缠绕在滑轮上的绳索支撑和驱动，绳索连接到建筑物四角的伺服控制直流电机。3. 建立了考虑执行器动力学的索驱动机构数学模型。4. 进行了模型仿真。5. 设计、制造并进行了原型机的实时控制测试。

Result: 研究结果表明：1. 获得了考虑执行器动力学的索驱动机构数学模型。2. 提供了仿真结果。3. 成功设计、制造并测试了原型机。4. 数值仿真和实验都显示了所提出的索驱动机构具有良好的定位性能。

Conclusion: 本研究提出的新型可移动索驱动脚手架系统，通过机电设计、动态建模、仿真和实时控制实验，验证了其在建筑物外表面实现良好定位性能的可行性和有效性。
...
### [474] [Fundamental Advances in Short-Circuit Measurement: A Novel Time-Resolved Diode-Clamp Circuit Paradigm](https://arxiv.org/abs/2512.00959)
...
### [475] [Fault-Tolerant Temperature Control of HRSG Superheaters: Stability Analysis Under Valve Leakage Using Physics-Informed Neural Networks](https://arxiv.org/abs/2512.00990)
*Mojtaba Fanoodi,Farzaneh Abdollahi,Mahdi Aliyari Shoorehdeli,Mohsen Maboodi*

Main category: eess.SY

TL;DR: 本文提出了一种结合PI加前馈控制策略与物理信息神经网络（PINN）的故障容错温度控制框架，用于热回收蒸汽发生器（HRSG），以应对阀门泄漏等故障，提高控制性能和故障恢复能力。


<details>
  <summary>Details</summary>
Motivation: HRSG中的故障（如阀门泄漏）和运行扰动严重影响蒸汽温度调节，可能导致效率损失、安全风险和机组停机。传统的PI控制器由于系统固有的延迟、非线性动力学和静态增益限制而难以有效应对。

Method: 该方法将PI加前馈控制策略与物理信息神经网络（PINN）相结合。前馈组件用于预测扰动并预先调整控制动作，而PINN则实时自适应调整控制增益，并嵌入热力学约束以处理变化的运行条件和阀门泄漏故障。通过基于Lyapunov的稳定性分析，确认了在有界泄漏条件下温度跟踪误差的渐近收敛性。

Result: 使用Pareh-Sar联合循环电厂运行数据的仿真结果表明，该方法显著改善了响应时间，减少了温度偏差，增强了故障恢复能力，并实现了平滑的增益调整。

Conclusion: 所提出的自适应、数据驱动的方法在HRSG系统中展现出强大的工业部署潜力，可确保可靠运行、自主故障恢复和增强性能。
...
### [476] [Approximating Analytically-Intractable Likelihood Densities with Deterministic Arithmetic for Optimal Particle Filtering](https://arxiv.org/abs/2512.01023)
*Orestis Kaparounakis,Yunqi Zhang,Phillip Stanley-Marbell*

Main category: eess.SY

TL;DR: 本文提出了一种基于UxHw平台的新型粒子滤波方法，通过确定性地转换参数分布来近似表示任意似然密度。该方法解决了传统粒子滤波在似然表示方面的弱点，在非高斯非线性系统中，相比蒙特卡洛方法，实现了显著的速度提升（最高37.7倍），大幅降低了错误零似然率（从81.89%降至1.52%），并提高了滤波器的RMSE（最高18.9%）。


<details>
  <summary>Details</summary>
Motivation: 粒子滤波算法在机器人、目标跟踪等领域有广泛应用，但其在表示观测似然方面的固有弱点（常导致粒子退化）尚未解决，尤其在高频和资源受限系统中表现突出。现有改进方案（如最优提议和辅助粒子滤波）在特定条件下有效，但计算成本增加。

Method: 本文提出了一种新的粒子滤波方法及其实现。该方法能将任意似然密度表示为参数分布的程序转换，并利用一个新型计算平台（UxHw）进行确定性计算，该平台无需依赖随机方法即可对概率分布表示进行操作。研究通过对非高斯非线性系统和最优辅助粒子滤波器进行基准测试，评估了似然评估误差和速度。

Result: 对于非高斯非线性系统，UxHw方法在似然评估速度上比蒙特卡洛替代方案快37.7倍。在窄均匀观测噪声下，传统粒子滤波器错误地将似然设为零的比例高达81.89%，而UxHw方法仅为1.52%。UxHw方法使滤波器RMSE提高了多达18.9%（平均3.3%）。

Conclusion: 所提出的UxHw粒子滤波方法有效解决了传统粒子滤波在似然表示上的固有弱点。它通过确定性计算提供了可调近似的似然表示，显著提高了计算速度、降低了错误零似然率，并改善了滤波器的均方根误差，尤其适用于高频和资源受限的非高斯非线性系统。
...
### [477] [The Silence that Speaks: Neural Estimation via Communication Gaps](https://arxiv.org/abs/2512.01056)
*Shubham Aggarwal,Dipankar Maity,Tamer Başar*

Main category: eess.SY

TL;DR: 本文提出CALM框架，一种基于学习的方法，用于远程状态估计，它联合优化通信调度和估计器设计，通过利用通信静默期的隐式信息来提高估计精度。


<details>
  <summary>Details</summary>
Motivation: 远程状态估计中存在估计精度与通信资源使用之间的权衡。传统方法将无通信视为信息缺失，但静默本身可能包含隐式信息。如何有效利用这些隐式信息以提升估计质量，是一个具有挑战性的问题。

Method: 本文提出CALM（Communication-Aware Learning and Monitoring），一个新颖的基于学习的框架。它不仅学习何时进行通信，还学习如何从通信静默期中推断出有用信息。该方法联合解决了通信调度和估计器设计这两个挑战。

Result: 通过对多个基准进行比较案例研究，CALM能够解码估计器和调度器之间的隐式协调，从“静默”实例中提取信息，并显著提高估计精度。

Conclusion: CALM框架成功地通过学习通信调度和从静默中推断信息，提升了远程状态估计的准确性，有效利用了通信静默的隐式结构。
...
### [478] [Assessing the Viability of Fresnel Lenses for Weed Control in Prickly Pear Cactus Cultivation: A Spatiotemporal Coverage Perspective](https://arxiv.org/abs/2512.01069)
...
TL;DR: 本研究提出了一种结合线性菲涅尔透镜（LFL）和无人地面车辆（UGV）的自主热力除草系统，用于在热带半干旱地区对仙人掌进行精准、可持续的杂草控制。


<details>
  <summary>Details</summary>
Motivation: 刺梨仙人掌在热带半干旱地区是一种重要的饲料资源，但其生产力极易受杂草竞争影响。传统的除草剂控制方法面临抗药性和环境问题，因此需要可持续的替代方案。

Method: 该研究开发了一个自主除草系统，将线性菲涅尔透镜（LFL）技术集成到无人地面车辆（UGV）平台中。系统利用实时图像处理、地理配准和机械驱动，实现两阶段操作：在非最佳太阳时段进行杂草测绘，在峰值日照时段进行目标太阳热力清除。同时，通过分析模型量化了有效覆盖面积和太阳窗口的时间限制。

Result: 初步结果表明，该系统适用于稀疏杂草的精准、出苗后控制，但不适用于密集杂草。该技术在热带地区（特别是巴西半干旱地区）因有利的太阳几何条件和目标性方法，非常适合资源匮乏地区的可持续农业。

Conclusion: 基于LFL的自主热力除草系统在技术上可行，为稀疏杂草的精准控制提供了一种可持续解决方案，特别适用于热带半干旱地区的可持续农业。
...
### [479] [Negotiating Highway Interchange Traffic with a Decentralized Instability-Driven CBF-based Algorithm](https://arxiv.org/abs/2512.01098)
*Mrdjan Jankovic,Shreshta Rajakumar Deshpande,Gopika Ajaykumar*

Main category: eess.SY

TL;DR: 本文提出一种基于分散式控制障碍函数（CBF）的算法，可在高密度交通下高效完成车道交换，并强调控制器调优（代理间不稳定性）对车辆组性能和敏捷性的关键作用。


<details>
  <summary>Details</summary>
Motivation: 在有限的高速公路路段上，当大多数车辆需要变道时，如何在交通密度高的情况下实现高效且速度变化最小的车道交换是一个挑战。

Method: 本文采用一种特定的分散式控制障碍函数（CBF）算法来执行车道交换。通过比较两种不同的控制器调优以及一种通过虚拟护栏强制执行车道交换的方案，来研究控制器调优（代理间不稳定性速度）对车辆组性能的影响。

Result: 该算法能在高交通密度（每车道每小时3,500辆车）下，在指定（短）路段内高效完成车道交换，且速度变化最小。研究表明，控制器调优（代理间不稳定性）在车辆组性能中扮演重要角色，它能像战斗机动态不稳定性提升机动性一样，提高车辆组的敏捷性。

Conclusion: 分散式控制器中的代理间不稳定性，类似于战斗机的动态不稳定性，能显著提高车辆组的敏捷性，从而实现高效的车道交换。
...
### [480] [Supervisory control synthesis for multilevel DES with local buses](https://arxiv.org/abs/2512.01101)
*Marzhan M. Baubekova,Martijn A. Goorden,Michel A. Reniers,Joanna M. v. d. Mortel-Fronczak,Jacobus E. Rooda,Wan J. Fokkink*

Main category: eess.SY

TL;DR: 本文通过引入一种新颖的多级离散事件系统（MLDES）架构，将总线结构检测从全局扩展到局部，从而改进了多级监督器综合的性能。


<details>
  <summary>Details</summary>
Motivation: 在多级监督器综合中，总线组件由于其广泛的系统依赖性，常常导致多个组件不理想地聚集在一个综合子问题中。现有工作虽然解决了全局总线结构，但局部总线结构的问题仍未得到充分处理。

Method: 本文提出了一种新颖的多级离散事件系统（MLDES）架构，允许在系统层次结构的每个级别（而不仅仅是顶层）进行总线检测。在此架构下，引入了一种算法来构建树形结构的MLDES。

Result: 通过在一个生产线案例研究中的应用，所提出的方法显著提高了综合性能。性能衡量指标是局部监督器受控状态空间大小的总和。

Conclusion: 所提出的在MLDES架构中处理局部总线结构的方法是有效的，能够显著改善多级监督器综合的性能。
...
### [481] [Semantic Communications for Vehicle-Based Mission-Critical Services: Challenges and Solutions](https://arxiv.org/abs/2512.01102)
*Hui Zhou,Jiaying Guo,Marios Aristodemou,Zhaoyang Du,Shen Wang,Xiaolan Liu,Soufiene Djahel,Celimuge Wu*

Main category: eess.SY

TL;DR: 本文提出并验证了一种专为车辆任务关键型（VbMC）服务设计的语义通信（SemCom）框架，以应对传统通信在可靠性、低延迟和高负载方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 随着无人机紧急通信和车联网自动驾驶等任务关键型（MC）服务的兴起，传统通信框架无法满足日益增长的对高可靠性、低延迟和高传输负载的需求。语义通信被视为6G网络的关键革命，但目前缺乏专门为车辆任务关键型（VbMC）服务量身定制的、系统性的语义通信框架，主要是因为其复杂性以及对MC特性分析的不足。

Method: 1. 介绍了语义通信框架内的关键信息关键型和基础设施关键型车辆服务。2. 分析了MC服务的独特特性及其给语义通信带来的挑战。3. 提出了一个新颖的语义通信框架，旨在满足车辆系统中MC服务的特定需求。4. 通过一个基于无人机的快速拥堵缓解案例研究，并利用可解释人工智能（XAI）验证了所提出语义通信框架的有效性。

Result: 本文提出了一个新颖的语义通信框架，专门用于解决车辆任务关键型（VbMC）服务的特定需求和挑战。通过无人机快速拥堵缓解的案例研究和XAI验证，证明了该框架的有效性。

Conclusion: 本文成功提出了一个针对车辆任务关键型服务的语义通信框架，并对其有效性进行了验证，为未来6G网络中MC服务的通信需求提供了潜在解决方案。
...
### [482] [A Neuromodulable Current-Mode Silicon Neuron for Robust and Adaptive Neuromorphic Systems](https://arxiv.org/abs/2512.01133)
...
### [483] [Autonomous Navigation and Station-Keeping on Near-Rectilinear Halo Orbits](https://arxiv.org/abs/2512.01182)
...
### [484] [Physics-Constrained Neural Dynamics: A Unified Manifold Framework for Large-Scale Power Flow Computation](https://arxiv.org/abs/2512.01207)
*Xuezhi Liu*
...
### [485] [Input-Output Data-Driven Representation: Non-Minimality and Stability](https://arxiv.org/abs/2512.01238)
...
### [486] [A Unified Bayesian Framework for Stochastic Data-Driven Smoothing, Prediction, and Control](https://arxiv.org/abs/2512.01475)
...
### [487] [An innovative circuit for testing hot carrier and trap generation in GaN Devices](https://arxiv.org/abs/2512.01683)
...
### [488] [How to Capture Human Preference: Commissioning of a Robotic Use-Case via Preferential Bayesian Optimisation](https://arxiv.org/abs/2512.01754)
*Sander De Witte,Jeroen Taets,Andras Retzler,Guillaume Crevecoeur,Tom Lefebvre*

Main category: eess.SY

TL;DR: 本文研究了偏好贝叶斯优化（PBO）在机器人推块任务中的应用，发现PBO仅通过专家偏好反馈就能成功调试系统，并且其推导出的辅助成本函数比专家设计的成本函数更能准确捕捉专家偏好。


<details>
  <summary>Details</summary>
Motivation: 传统的贝叶斯优化（BO）依赖于一个标量目标函数，这在工业环境中很难定义，且难以捕捉专家操作员的细微判断。偏好贝叶斯优化（PBO）通过仅依赖人类专家的成对偏好反馈来解决这一限制。

Method: 研究了PBO调试一个机械手将方块推向目标位置的能力。在模拟和真实世界中对最先进的PBO算法进行了基准测试。通过将专家决策一致性与专家设计的成本函数进行比较，评估了PBO的效果。将PBO算法副产品构建的辅助成本函数与专家设计的成本函数进行比较。并展示了如何将该辅助成本函数与传统BO算法结合使用以重现最优设计。

Result: PBO仅依靠二元偏好反馈就能成功调试系统，并令专家操作员满意。专家难以定义一个与他们在PBO实验中表现出的决策过程完全一致的成本函数。PBO算法构建的辅助成本函数在决策一致性方面优于专家设计的成本函数。该辅助成本函数可用于传统BO算法以重现最优设计，表明它可能比专家自己更能有效地捕捉偏好。

Conclusion: PBO能有效利用专家偏好反馈来调试工程系统。PBO推导出的偏好成本函数比专家明确表达的偏好更能有效捕捉专家偏好，并可用于改进传统BO。论文还讨论了缺点并提出了未来的研究方向。
...
### [489] [Frequency-Dynamics-Aware Economic Dispatch with Optimal Grid-Forming Inverter Allocation and Reserved Power Headroom](https://arxiv.org/abs/2512.01814)
*Fan Jiang,Xingpeng Li*

Main category: eess.SY

TL;DR: 本文提出了一种基于深度学习的频率约束最优潮流（DL-FCOPF）框架，用于在高逆变器并网资源（IBR）渗透率下，通过精确预测频率动态并优化同步发电机（SG）和IBR的协调，解决频率稳定性问题，同时确保经济效益。


<details>
  <summary>Details</summary>
Motivation: 高渗透率的逆变器并网资源（IBR）降低了系统惯性，尤其在同步发电机（SG）停运时，引发频率稳定性问题。现有频率约束最优潮流（FCOPF）研究未能充分考虑并网型（GFM）IBR的频率支持能力和分配，或因模型简化导致频率动态表示准确性不足。

Method: 首先开发了一个深度学习（DL）模型作为预测器，用于精确估计频率相关指标，包括GFM IBR所需的备用裕度及其分配、频率变化率（RoCoF）和频率最低点。该DL模型通过电磁暂态仿真数据进行训练，然后被重新公式化并整合到FCOPF框架中，形成DL-FCOPF。此外，还进行了敏感性分析以确定DL频率预测器的最佳结构和线性化方法。

Result: 在两个测试系统上的案例研究表明，所提出的DL-FCOPF方法能够以最小的成本最优地协调SG和IBR，实现期望的频率响应，且计算时间可接受。与传统OPF和线性FCOPF基准相比，DL-FCOPF表现出优越性。敏感性分析也成功识别了DL频率预测器的最合适结构和线性化方法。

Conclusion: DL-FCOPF框架通过结合深度学习的预测能力，准确建模频率动态，并有效协调同步发电机和逆变器并网资源，解决了高IBR渗透率下的频率稳定性问题，同时实现了经济高效的运行。
...
### [490] [Uncertainty quantification in load profiles with rising EV and PV adoption: the case of residential, industrial, and office buildings](https://arxiv.org/abs/2512.01914)
*Aiko Fias,Md Umar Hashmi,Geert Deconinck*

Main category: eess.SY

TL;DR: 本文比较研究了量化光伏发电和电动汽车充电引入的净负荷不确定性的指标，发现联合考虑光伏和电动汽车可因其补偿效应而降低整体不确定性，并确定了适用于不同消费者类型的不确定性量化指标。


<details>
  <summary>Details</summary>
Motivation: 光伏发电和电动汽车充电的整合给电力消耗模式，特别是在配电层面，带来了显著的不确定性，因此需要有效的方法来量化这种不确定性。

Method: 本文对住宅、工业和办公建筑在分布式能源（EV和PV）渗透率增加情况下的净负荷曲线，进行了不确定性量化（UQ）指标的比较研究。评估了多种统计指标，包括标准差、熵、斜坡和距离指标。这些UQ指标被分类为无基线、有基线和基于误差的，并用于评估EV和PV渗透增加对不确定性的影响。

Result: 研究结果突出了适用于每种消费者类型的不确定性量化指标，并展示了净负荷不确定性如何受电动汽车和光伏采用的影响。此外，研究发现，由于电动汽车充电和光伏发电在时间上的对齐所产生的补偿效应，联合考虑电动汽车和光伏可以降低整体不确定性。这种不确定性降低在所有数据集中都有观察到，并且在办公建筑数据集中最为显著。

Conclusion: 联合考虑电动汽车和光伏的整合可以有效降低电力系统中的净负荷不确定性，尤其对于办公建筑类型更为明显。本研究还确定了量化不同消费者类型不确定性的合适指标。
...
### [491] [Event-triggered control of nonlinear systems from data](https://arxiv.org/abs/2512.01938)
*Hailong Chen,Claudio De Persis,Andrea Bisoffi,Pietro Tesi*

Main category: eess.SY

TL;DR: 本文将先前针对线性系统的数据驱动事件触发控制器设计方法扩展到非线性系统，并提出了两种基于李雅普诺夫函数的认证设计。


<details>
  <summary>Details</summary>
Motivation: 先前的研究[8]已成功为线性系统引入了数据驱动的事件触发控制器设计方法，本文旨在将该方法推广到更复杂的非线性系统。

Method: 提出了两种由经典李雅普诺夫函数认证的数据驱动设计。基于这些设计，开发了两种事件触发策略，它们依赖于李雅普诺夫函数，参数从数据中调整，确保正的最小事件间隔时间，并分别基于状态误差或库误差触发。

Result: 成功设计并数值演示了两种不同的事件触发策略，并阐述了它们各自的优点。

Conclusion: 本文为非线性系统提供了两种数据驱动的事件触发控制器设计，这些设计通过李雅普诺夫函数认证，并提出了两种有效的事件触发策略，其有效性已通过数值模拟验证。
...
### [492] [AI-Driven Optimization under Uncertainty for Mineral Processing Operations](https://arxiv.org/abs/2512.01977)
...
### [493] [ECO: Energy-Constrained Operator Learning for Chaotic Dynamics with Boundedness Guarantees](https://arxiv.org/abs/2512.01984)
...
### [494] [MedCondDiff: Lightweight, Robust, Semantically Guided Diffusion for Medical Image Segmentation](https://arxiv.org/abs/2512.00350)
*Ruirui Huang,Jiacheng Li*

Main category: eess.IV

TL;DR: MedCondDiff是一种基于扩散的框架，通过金字塔视觉Transformer（PVT）提取语义先验来指导去噪过程，实现了高效、解剖学上合理的多器官医学图像分割，同时提高了鲁棒性并减少了推理时间和VRAM使用。


<details>
  <summary>Details</summary>
Motivation: 研究动机是提高传统扩散模型在多器官医学图像分割任务中的效率、鲁棒性，并减少推理时间和VRAM使用。

Method: 该方法引入了MedCondDiff，一个基于扩散的框架。它使用金字塔视觉Transformer（PVT）骨干网络提取语义先验，并以此条件化去噪过程，从而构建了一个语义引导的轻量级扩散架构。

Result: 实验结果表明，MedCondDiff在多器官、多模态数据集上实现了具有竞争力的性能，适用于不同的解剖区域和成像模态。与传统扩散模型相比，它显著减少了推理时间和VRAM使用。

Conclusion: 结论是，像MedCondDiff这样由语义引导的扩散模型在医学成像任务中是一类有效的架构，具有巨大潜力。
...
### [495] [Large-field-of-view lensless imaging with miniaturized sensors](https://arxiv.org/abs/2512.00488)
*Yu Ren,Xiaoling Zhang,Xu Zhan,Xiangdong Ma,Yunqi Wang,Edmund Y. Lam,Tianjiao Zeng*

Main category: eess.IV

TL;DR: 本文提出一种从局部到全局的层次化框架，用于解决无透镜相机在空间变化点扩散函数（PSF）和传感器截断问题上的局限性，尤其是在小型传感器下，显著提升了重建质量和有效视场角。


<details>
  <summary>Details</summary>
Motivation: 现有无透镜相机方法依赖于理想化模型，假设全局移不变PSF和足够大的传感器。然而，实际中PSF在视场中空间变化，有限的传感器边界会截断调制光，这些效应在传感器缩小时会加剧，导致外围重建质量下降并限制有效视场角。

Method: 本文通过一个基于局部移不变卷积模型的从局部到全局的层次化框架来解决这些限制。该模型明确考虑了PSF变化和传感器截断。首先，通过分块学习的去卷积自适应估计局部PSF并独立重建区域。然后，一个层次化增强网络逐步扩展其感受野（从小块到中间块再到完整图像），整合精细的局部细节和全局上下文信息。

Result: 实验结果表明，该方法在显著减小传感器尺寸的情况下，实现了更优越的重建质量和更大的有效视场角。在极端小型化（传感器面积减小至原始的8%）条件下，PSNR提升2 dB，SSIM提升5%，尤其在结构保真度方面有显著提升。

Conclusion: 所提出的从局部到全局的层次化框架成功解决了无透镜相机在PSF空间变化和传感器截断方面的挑战，即使在传感器尺寸大幅缩减的情况下，也能提供卓越的图像重建质量和更大的有效视场角，从而实现更紧凑的成像系统。
...
### [496] [Diffusion-Based Synthesis of 3D T1w MPRAGE Images from Multi-Echo GRE with Multi-Parametric MRI Integration](https://arxiv.org/abs/2512.01135)
*Sizhe Fang,Deqiang Qiu*

Main category: eess.IV

TL;DR: 该研究提出了一种基于扩散模型的深度学习框架，利用多回波梯度回波（mGRE）数据（包括QSM和R2*图）合成高质量的3D T1加权MPRAGE图像，以缩短神经影像扫描时间并保持生物学真实性。


<details>
  <summary>Details</summary>
Motivation: 多回波梯度回波（mGRE）序列能提供有价值的定量参数图，但结构形态学分析通常需要单独的T1加权MPRAGE采集，这延长了扫描时间。研究旨在从mGRE数据中直接合成T1w图像，以简化神经影像协议。

Method: 开发了一种基于Fast-DDPM架构的新型多参数条件扩散模型。与传统方法不同，该模型将铁敏感的QSM和R2*图作为物理先验知识整合，以解决富含铁的深部灰质中的对比度模糊问题。模型在175名健康受试者的数据上进行训练和验证，并与U-Net和GAN等基线模型进行比较。评估指标包括感知质量、下游分割精度，以及通过复制年龄和性别相关的群体统计关联来评估合成图像的生物学合理性。

Result: 所提出的框架显著优于基线模型，在感知质量和分割精度方面表现卓越，尤其是在丘脑和苍白球等皮层下区域。关键是，合成图像保留了基本的生物学依赖性：回归分析显示，与真实数据相比，在年龄相关的萎缩率、老化效应大小和性别二态性模式方面具有高度一致性。

Conclusion: 该研究提出的基于扩散的方法，通过有效利用定量MRI先验知识，生成了严格符合生物学真实性的T1加权图像，适用于可靠的临床形态测量分析。这种方法为通过从定量mGRE序列中追溯推导结构对比度，从而减少采集时间提供了一条有前景的途径。
...
### [497] [Disentangling Progress in Medical Image Registration: Beyond Trend-Driven Architectures towards Domain-Specific Strategies](https://arxiv.org/abs/2512.01913)
...
TL;DR: 在医学图像配准中，领域特定设计比通用架构趋势更能带来性能提升，研究应侧重于前者。


<details>
  <summary>Details</summary>
Motivation: 深度学习医学图像配准方法融合了通用计算模块（如CNN、Transformer）和配准专用设计（如运动金字塔、关联层），但它们各自的贡献不明确。这引发了核心问题：未来的进展应关注引入通用架构趋势还是完善领域特定设计原则？

Method: 通过一个模块化框架，系统地解耦了通用“趋势驱动”计算块和高层配准特定设计在脑、肺、心脏和腹部配准中的影响。评估比较了标准U-Net基线模型与结合了“趋势驱动”块或领域特定先验的变体。同时发布了一个透明、模块化的基准平台，用于可复现的评估。

Result: 低层“趋势驱动”计算块仅提供微小或不一致的增益。而高层配准特定设计持续提供更准确、更平滑、更鲁棒的形变，显著提升了标准U-Net基线的性能（平均相对改进约3%），远超结合“趋势驱动”块的变体。

Conclusion: 研究重点应从追随架构趋势转向采纳领域特定的设计原则，将其作为基于学习的医学图像配准领域进步的真正驱动力。
...
### [498] [Comparative Evaluation of Generative AI Models for Chest Radiograph Report Generation in the Emergency Department](https://arxiv.org/abs/2512.00271)
*Woo Hyeon Lim,Ji Young Lee,Jong Hyuk Lee,Saehoon Kim,Hyungjin Kim*

Main category: eess.IV

TL;DR: 本研究评估了五种医学图像VLM在生成胸片报告方面的表现，发现其报告质量和诊断准确性差异显著，其中AIRead表现最佳。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在将开源或商业化的医学图像特定VLM与放射科医生撰写的真实报告进行基准测试。

Method: 这是一项回顾性研究，纳入了2022年1月至2025年4月期间因发热或呼吸道症状就诊急诊科并接受胸片和CT检查的成年患者。来自五种VLM（AIRead、Lingshu、MAIRA-2、MedGemma和MedVersa）的报告以及放射科医生撰写的报告被随机呈现，并由三名胸部放射科医生使用四项标准（RADPEER、临床可接受性、幻觉和语言清晰度）进行盲法评估。使用广义线性混合模型进行比较性能评估，以放射科医生报告作为参考。还以CT作为参考进行了发现层面的分析。

Result: 共纳入478名患者。AIRead表现出最低的RADPEER 3b率（5.3% vs 放射科医生13.9%；P<.001），而其他VLM的异议率更高（16.8-43.0%；P<.05）。AIRead的临床可接受性最高（84.5% vs 放射科医生74.3%；P<.001），其他VLM表现较差（41.1-71.4%；P<.05）。AIRead的幻觉发生率极低，与放射科医生相当（0.3% vs 0.1%；P=.21），但其他模型则频繁出现（5.4-17.4%；P<.05）。AIRead、Lingshu和MedVersa的语言清晰度高于放射科医生。常见发现的敏感性在VLM之间差异很大（AIRead：15.5-86.7%；Lingshu：2.4-86.7%；MAIRA-2：6.0-72.0%；MedGemma：4.8-76.7%；MedVersa：20.2-69.3%）。

Conclusion: 用于胸片报告生成的医学VLM在报告质量和诊断指标方面表现出显著差异。
