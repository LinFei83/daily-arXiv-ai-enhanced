...
### [1] [AssurAI: Experience with Constructing Korean Socio-cultural Datasets to Discover Potential Risks of Generative AI](https://arxiv.org/abs/2511.20686)
...
TL;DR: 本文介绍了AssurAI，一个高质量的韩语多模态数据集，用于评估生成式AI的安全性，旨在解决现有数据集以英语为中心且仅限于文本模态的问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI安全评估数据集主要以英语为中心，且多为文本模态，未能捕捉非英语（如韩语）社会文化背景下的特定风险。这促使研究者开发一个能覆盖这些空白的多模态数据集。

Method: 首先，定义了包含35个AI风险因素的分类法，结合了通用危害和韩国社会文化背景。其次，构建并发布了AssurAI，一个包含11,480个实例的大规模韩语多模态数据集，涵盖文本、图像、视频和音频。最后，采用了严格的质量控制流程，包括专家主导的种子创建、众包扩展、三重独立标注和迭代的专家红队循环。

Result: 初步研究验证了AssurAI在评估最新大型语言模型安全性方面的有效性。数据集已公开发布。

Conclusion: AssurAI的发布将促进为韩国社区开发更安全、更可靠的生成式AI系统，并弥补了非英语、多模态AI安全评估数据集的空白。
...
### [2] [Minimizing Hyperbolic Embedding Distortion with LLM-Guided Hierarchy Restructuring](https://arxiv.org/abs/2511.20679)
*Melika Ayoughi,Pascal Mettes,Paul Groth*

Main category: cs.AI

TL;DR: 本文提出并验证了大型语言模型（LLMs）能够自动重构层次结构，以优化双曲嵌入的质量，并提供可解释的重组理由。


<details>
  <summary>Details</summary>
Motivation: 双曲几何在嵌入分层数据方面表现出色，但其嵌入质量与输入层次结构的结构紧密相关。研究表明，高分支因子和单继承是优化双曲嵌入的关键。知识工程师在重组层次结构以满足这些标准时面临挑战，因此研究LLMs是否能自动完成此任务成为必要。

Method: 本文提出了一种基于提示词的方法，利用大型语言模型（LLMs）根据已知的双曲嵌入优化标准来转换和重构现有的层次结构。

Result: 在16个不同的层次结构上进行的实验表明，经过LLM重构的层次结构在多个标准嵌入质量指标上始终能产生更高质量的双曲嵌入。此外，LLM引导的层次结构重构能够提供可解释的重组，为知识工程师提供理由。

Conclusion: LLMs具有自动重构层次结构以满足双曲嵌入优化标准的能力，从而提高嵌入质量。同时，LLMs还能为知识工程师提供可解释的重组理由，辅助知识工程工作。
...
TL;DR: A^2Flow是一个全自动的智能体工作流生成框架，它通过自适应抽象操作符来克服现有方法对人工预定义操作符的依赖，显著提升了泛化性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型（LLMs）在自动化智能体工作流设计方面表现出潜力，但其方法严重依赖手动预定义操作符，这限制了泛化能力和可扩展性。

Method: A^2Flow采用三阶段操作符提取过程：1) 基于案例的初始操作符生成（利用专家演示和LLM推理）；2) 操作符聚类和初步抽象（对跨任务的相似操作符进行分组）；3) 深度提取抽象执行操作符（应用长链式思考提示和多路径推理）。此外，通过操作符记忆机制增强节点级工作流搜索，以丰富上下文并改进决策。

Result: A^2Flow在通用和具身基准测试中，平均性能分别提升了2.4%和19.3%，并且与现有最先进的基线相比，资源使用量减少了37%。

Conclusion: A^2Flow通过引入自适应抽象操作符和操作符记忆机制，实现了智能体工作流的全自动化生成，显著提高了性能、泛化能力和资源效率，解决了传统方法中手动预定义操作符的局限性。
...
TL;DR: 本文提出了一个名为“Reasoning With a Star”的太阳物理学推理数据集，并提供了一个基准测试方法，旨在解决大型语言模型在科学推理中面临的挑战，发现基于系统工程原理分解工作流的方法优于直接提示。


<details>
  <summary>Details</summary>
Motivation: 在太阳物理学中，大型语言模型进行科学推理不仅仅需要回忆事实，还需要整合物理假设、保持单位一致性，并以清晰的科学格式呈现。现有的模型难以满足这些要求。

Method: 研究者构建了一个名为“Reasoning With a Star”的太阳物理学数据集，数据来源于NASA和UCAR的暑期学校问题集，并以问答形式组织，包含问题背景、推理步骤、预期答案类型、真值、格式提示和元数据。他们还开发了一个程序化评分器，通过单位感知数值容差、符号等价和模式验证来检查预测。基准测试中使用了单次提示基线和四种多智能体模式。

Result: 基准测试结果显示，通过系统工程原理分解工作流的多智能体模式，在需要演绎推理而非纯粹归纳回忆的问题上，表现优于直接提示方法。

Conclusion: 对于涉及复杂科学推理（特别是演绎推理）的任务，将工作流分解的结构化方法能显著提升大型语言模型的性能，超越简单的直接提示。
...
TL;DR: 数字孪生技术正推动医疗保健转型，通过整合实时数据和计算模型，创建患者特定模拟以支持诊断、治疗和药物开发，尽管面临互操作性、数据隐私和模型保真度等挑战，但未来有望实现预测性、预防性和个性化医疗。


<details>
  <summary>Details</summary>
Motivation: 将医疗保健从被动治疗转变为预测性、预防性和真正个性化的医疗，以应对传统医疗的局限性并提高效率和效果。

Method: 数字孪生是一种动态、数据驱动的物理系统虚拟副本，通过实时数据流持续更新，并能够进行双向交互。在医学中，它整合了影像、生物传感器和计算模型来生成患者特定的模拟。

Result: 代表性应用包括用于预测心律失常治疗结果的心脏数字孪生、用于追踪肿瘤进展和优化放疗的肿瘤学数字孪生，以及用于加速药物发现的药理学数字孪生。这些应用支持诊断、治疗规划和药物开发。

Conclusion: 尽管取得了快速进展，但互操作性、数据隐私和模型保真度等主要挑战仍限制了其广泛的临床整合。可解释人工智能、联邦学习和协调监管框架等新兴解决方案提供了前景广阔的途径。未来，多器官数字孪生、基因组学整合和伦理治理的进步对于实现预测性、预防性和个性化医疗至关重要。
...
TL;DR: 本文介绍了一个名为 Paraconsistent-Lib 的开源 Python 库，用于在推理和决策系统中构建 PAL2v 算法，旨在简化开发、减少复杂性和错误。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了提供一个通用、易用的工具，以简化 PAL2v 算法的构建，并减少相关系统的复杂性、代码量和潜在错误。

Method: Paraconsistent-Lib 作为一个通用的 PAL2v 标准计算库，提供三种类型的分析结果：12个经典格 PAL2v 区域之一的次协调分析、次协调分析节点 (PAN) 输出和决策输出。它支持编写独立的或网络形式的 PAL2v 算法，如 Para-analyzer、ParaExtrCTX、PAL2v Filter、PANnet 和 PNN。

Result: 该库能够简化 PAL2v 算法的实现，减少代码复杂性、代码大小和错误，并提供多种次协调分析和决策输出。文中提供了两个示例来证明其效果。

Conclusion: Paraconsistent-Lib 是一个稳定且正在积极开发的工具，旨在响应用户需求，为构建 PAL2v 算法提供便利，从而促进次协调逻辑在推理和决策系统中的应用。
...
TL;DR: 本文对多模态思维链（Multimodal-CoT）推理的通用性进行了全面分析，发现其在不同问题类型上的有效性差异显著，尤其在常识推理方面面临挑战，但视觉整合能有效减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态思维链在科学问答基准测试中取得了最先进的成果，但其在不同领域（超越科学推理）的泛化能力尚未得到充分探索。

Method: 本研究采用了Zhang等提出的两阶段框架，该框架将理由生成与答案推断分离，并通过门控融合机制将视觉特征与基于T5的语言模型相结合。研究在A-OKVQA、OKVQA和ChartQA数据集上进行了评估，并通过系统性消融实验分析了视觉特征、理由质量和架构选择的贡献。

Result: 研究发现，视觉整合显著减少了理由生成中的幻觉。然而，思维链推理的有效性在不同问题类型之间差异很大，其中常识推理提出了特殊的挑战。

Conclusion: 本工作为实施多模态推理系统的研究人员提供了实用见解，并指出了未来在跨领域泛化方面改进的关键领域。
...
TL;DR: 本文提出了一种新颖的Agentic AI Wi-Fi框架，通过将每个接入点建模为自主大型语言模型代理，实现多接入点协调的实时动态协商和适应性策略，以应对复杂的网络环境。


<details>
  <summary>Details</summary>
Motivation: 现有的多接入点协调（MAPC）协议依赖静态、协议定义的规则，限制了它们在动态网络条件（如变化的干扰水平和拓扑结构）下的适应能力。

Method: 该研究提出了一种Agentic AI Wi-Fi框架，其中每个接入点被建模为自主大型语言模型（LLM）代理。这些代理通过认知工作流（包括集成记忆、反思和工具使用）进行自然语言对话，以协作推理网络状态并协商适应性协调策略。

Result: 全面的仿真结果表明，所提出的代理框架成功地适应了多样化和动态的网络环境，显著优于现有的空间复用基线，并验证了其作为未来无线网络强大智能解决方案的潜力。

Conclusion: Agentic AI Wi-Fi框架通过赋予接入点LLM代理的能力，实现了动态适应性的多接入点协调，为未来无线网络提供了一种鲁棒且智能的解决方案。
...
TL;DR: 自主UI代理的可靠性至关重要，但现有评估局限于固定环境。本文提出OpenApps，一个可配置应用生态系统，用于评估代理在应用变体间的可靠性，发现代理性能在不同应用变体中波动剧烈。


<details>
  <summary>Details</summary>
Motivation: 自主UI代理需要高可靠性才能被信任。然而，当前评估依赖于固定应用环境，无法反映代理在真实世界中可能遇到的应用设计和内容变体，导致对代理在不同应用变体下可靠性测量的盲区。

Method: 开发了OpenApps，一个轻量级开源生态系统，包含六个可配置外观和内容的应用（如即时通讯、日历、地图等）。OpenApps仅需单个CPU即可运行，能轻松生成和部署数千个应用版本。研究人员利用OpenApps对七个主流多模态代理进行了超过10,000次独立评估，以研究它们在不同应用变体中的可靠性。

Result: 研究发现，虽然代理在固定应用中的标准可靠性相对稳定，但在不同应用变体中，可靠性会发生剧烈变化。许多代理的任务成功率在不同应用变体之间波动超过50%。例如，Kimi-VL-3B在所有任务上的平均成功率在不同应用版本中从63%下降到仅4%。此外，代理的行为（如循环或幻觉动作）也因环境配置的不同而显著差异。

Conclusion: 这些初步发现强调了在考虑应用变体这一新维度上测量代理可靠性的重要性。OpenApps为此提供了一个有效的工具。
...
TL;DR: 本文提出RILKE，一种基于表示空间干预的鲁棒、可扩展方法，用于在不重新训练大型语言模型的情况下，实现终身、细粒度且无干扰的知识控制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常产生不准确或过时内容。在不进行昂贵再训练的情况下，高效准确地更新其知识，尤其是在终身学习环境中处理复杂、非结构化知识时，是一个重大挑战。

Method: RILKE将知识控制视为模型表示空间内的干预。它利用表示空间的表达性，学习抗复述且编辑局部化的模块，将每次更新限制在低维子空间以最小化交叉编辑干扰。在推理时，一个查询自适应路由器选择合适的模块来指导模型生成。

Result: 在LLaMA和Qwen模型上的知识编辑基准测试中，RILKE在大规模数据集上表现出可扩展性，实现了高编辑成功率、强大的复述泛化能力，并在内存开销适中的情况下保持了通用实用性。

Conclusion: RILKE是一种有效且可扩展的解决方案，用于大型语言模型中的终身知识控制问题。
...
TL;DR: 本文提出了一种计算深度神经网络构成性解释的最优框架和算法，解决了现有束搜索方法缺乏最优性保证的问题，并通过实验证明了束搜索在某些情况下会产生次优解释，并提出了一种改进的束搜索变体。


<details>
  <summary>Details</summary>
Motivation: 尽管神经元是深度神经网络的基本单元，但它们学习了什么以及它们的知识是否与人类对齐仍不清楚。构成性解释旨在通过逻辑规则描述神经元激活与概念之间的空间对齐。然而，计算这些解释通常涉及在巨大状态空间上的搜索，现有方法（如束搜索）无法提供最优性理论保证，因此不清楚当前解释与真实最优解的差距。

Method: 本文提出了一个计算保证最优构成性解释的框架，具体包括：(i) 一种识别影响空间对齐因素的分解方法，(ii) 一种在搜索任何阶段估计对齐的启发式方法，以及 (iii) 第一个能够在可行时间内计算最优构成性解释的算法。此外，还评估了由所提出的分解和启发式方法引导的束搜索变体。

Result: 在计算机视觉领域和卷积神经网络中最流行的设置中，本文发现当涉及重叠概念时，通过束搜索获得的解释有10-40%是次优的。此外，本文提出的引导式束搜索变体在运行时性能上与现有方法持平或有所改进，同时在超参数和计算资源方面提供了更大的灵活性。

Conclusion: 本文引入了第一个能够计算保证最优构成性解释的框架，揭示了常用束搜索方法在某些情况下存在次优性问题。所提出的方法不仅提供了最优性保证，而且其引导式束搜索变体在效率和灵活性方面也优于现有方法，为理解神经网络提供了更准确的工具。
...
TL;DR: 本文引入了ENACT基准，通过以自我中心交互为基础的世界建模任务来评估现代视觉-语言模型（VLMs）是否展现出具身认知能力。结果显示，VLMs与人类之间存在显著性能差距，并表现出以人为中心的偏见。


<details>
  <summary>Details</summary>
Motivation: 具身认知理论认为智能源于感觉运动交互而非被动观察。这引发了一个问题：主要通过非具身方式训练的现代VLMs是否展现出具身认知的迹象？

Method: 引入了ENACT基准，将具身认知评估转化为视觉问答（VQA）格式的自我中心交互世界建模。该基准被框定为部分可观察马尔可夫决策过程（POMDP），其动作是场景图变化，包含两个互补的序列重排任务：正向世界建模（根据动作重排观察）和逆向世界建模（根据观察重排动作）。通过机器人模拟（BEHAVIOR）合成QA对，并在8,972个QA对上评估模型，避免了低级图像合成，专注于具身认知能力。

Result: 前沿VLMs与人类之间存在性能差距，且随着交互视界（horizon）的增长而扩大。模型在逆向任务上的表现始终优于正向任务。模型还表现出以人为中心的偏见，包括偏好右手动作，以及当相机内参或视点偏离人类视觉时性能下降。

Conclusion: 当前VLMs在具身认知能力方面与人类存在显著差距，并且在处理具身交互任务时表现出特定的以人为中心的偏见。这表明需要进一步研究以弥合VLMs在具身智能方面的不足。
...
TL;DR: Ivy是一个结合符号化任务-方法-知识（TMK）模型与大型语言模型（LLM）的AI辅导系统，旨在生成结构化、深入的程序技能解释，并通过TMK的约束显著提升解释的结构质量。


<details>
  <summary>Details</summary>
Motivation: 在程序技能学习中，教学解释不仅需传达步骤，更要揭示其背后的因果、目标导向和组合逻辑。现有大型语言模型（LLMs）虽然流畅，但往往生成缺乏这种深层结构的肤浅回应。

Method: 本文提出了Ivy系统，它通过将符号化的任务-方法-知识（TMK）模型与生成式解释层（一个受TMK结构约束的LLM）相结合，来提供结构化的多步骤解释。TMK模型编码因果转换、目标层次和问题分解，并明确地指导LLM生成解释。

Result: 通过专家和独立注释，在三个推理维度上评估Ivy与GPT及检索增强型GPT基线的结果显示，符号约束（TMK）一致地提高了针对“如何”和“为什么”问题的解释的结构质量。

Conclusion: 这项研究展示了一种可扩展的教育AI方法，能够增强智能辅导系统中AI生成解释的教学价值，使其更适合程序技能学习。
...
TL;DR: 现有可验证奖励强化学习（RLVR）方法存在奖励粗粒度、奖励噪声和探索效率低等问题。本文提出ICPO方法，通过结合LLM的内在置信度（生成概率）和可验证奖励，计算偏好优势分数，有效缓解上述问题，促进更彻底的探索，从而提升LLM的推理能力。


<details>
  <summary>Details</summary>
Motivation: RLVR在增强大型语言模型（LLM）推理能力方面潜力巨大，但现有方法常受限于奖励粗粒度、奖励噪声和探索效率低下等问题，导致训练不稳定和熵坍塌。

Method: 提出内在置信度驱动的群体相对偏好优化（ICPO）方法。该方法利用LLM生成不同响应的概率作为其推理过程的内在自我评估。受偏好建模启发，ICPO通过比较同一输入提示下多个响应的相对生成概率，计算每个响应的偏好优势分数，并将该分数与可验证奖励相结合，以指导探索过程。

Result: 研究发现，偏好优势分数不仅能缓解奖励粗粒度和奖励噪声问题，还能有效抑制过度自信错误，提升被低估的高质量响应的相对优势，并防止模型过度拟合特定策略，从而促进更彻底的探索。在四个通用领域和三个数学基准上的综合实验表明，ICPO相对于GRPO稳定地提升了推理能力。

Conclusion: ICPO方法通过利用LLM的内在置信度并将其与可验证奖励相结合，有效解决了现有RLVR方法中的奖励问题和探索效率问题，显著提升了LLM的推理能力。
...
TL;DR: L4M是一个结合对抗性LLM智能体和SMT求解器支持证明的新框架，旨在将自然语言的解释灵活性与符号验证的严谨性结合起来，实现更严谨和可解释的法律推理，并在公共基准测试中超越了现有先进的LLM和法律AI基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的系统在表面文本分析方面表现出色，但缺乏进行原则性法理学所需的保证。法律的理性体现在实质理性和形式理性两个方面，而LLM在形式理性方面存在不足，无法提供严格的逻辑一致性。因此，需要一个能够结合自然语言解释灵活性和符号验证严谨性的框架。

Method: L4M框架包含三个阶段：1) 法规形式化，使用特定领域提示将法律条文转换为逻辑公式；2) 双重事实与法规提取，由检察官和辩护方LLM独立地将案例叙述映射到事实元组和法规，确保角色隔离；3) 以求解器为中心的裁决，自动形式化器将双方论点编译为逻辑约束，SMT求解器通过不可满足核心触发迭代自我批评，直到达到可满足公式，然后由法官LLM将其口头化为透明判决和优化判决。

Result: 实验结果表明，L4M系统在公共基准测试中超越了包括GPT-o4-mini、DeepSeek-V3和Claude 4在内的先进LLM以及最先进的法律AI基线，同时提供了严谨且可解释的符号论证。

Conclusion: L4M成功地将自然语言的解释灵活性与符号验证的严谨性结合起来，为法律决策提供了卓越的性能和可解释性，有效解决了现有LLM在法律推理中缺乏保证的问题。
...
TL;DR: OVOD-Agent通过引入可解释的Visual-CoT和基于弱马尔可夫决策过程(w-MDP)的视觉推理，将开放词汇目标检测(OVOD)从被动匹配转变为主动推理和自进化检测，显著提升了稀有类别的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有OVOD方法在多模态训练和单模态推理之间存在差距，推理受限于固定的类别名称，且文本空间尚未充分探索，导致检测性能受限。研究表明，改进文本表示能显著提升OVOD性能。

Method: 本文提出了OVOD-Agent，将文本优化扩展为具有明确动作的可解释Visual-CoT。它将视觉上下文转换建模为八个状态空间上的弱马尔可夫决策过程(w-MDP)。一个Bandit模块在有限监督下生成探索信号，帮助Agent关注不确定区域。此外，马尔可夫转移矩阵与Bandit轨迹相结合，用于自监督奖励模型(RM)优化，形成一个闭环。

Result: 在COCO和LVIS数据集上的实验表明，OVOD-Agent在不同的OVOD骨干网络上都提供了持续的性能提升，尤其是在稀有类别上表现突出。

Conclusion: 所提出的OVOD-Agent框架通过主动的视觉推理和自进化检测，有效弥补了多模态训练与单模态推理之间的差距，显著提升了开放词汇目标检测的性能，尤其对稀有类别检测效果显著。
...
TL;DR: 本文抽象了Halpern-Pearl的因果关系定义，使其能应用于更广泛的模型和更复杂的公式，并深化了对其特征的理解。


<details>
  <summary>Details</summary>
Motivation: Halpern-Pearl的因果关系定义虽然突出，但存在局限性，例如只能用于因果模型，且无法处理涉及析取、否定、信念和嵌套反事实的复杂公式。研究者希望扩展其适用范围。

Method: 通过提取Halpern-Pearl因果关系定义中的关键特征，对其进行抽象化处理，使其可以应用于任何定义了反事实的模型。

Result: 抽象后的定义不仅可以应用于更广泛的模型（包括允许回溯的模型），还能处理包含析取、否定、信念和嵌套反事实的复杂公式。此外，该方法可以扩展为解释的抽象定义，并加深了对因果关系定义特征的理解。

Conclusion: 通过抽象化Halpern-Pearl的因果关系定义，显著扩展了其适用范围，使其能够处理更复杂的因果判断和解释，并提供了对该定义更深层次的理解。
...
### [18] [New Hybrid Heuristics for Pseudo-Boolean Propagation](https://arxiv.org/abs/2511.21417)
*Mia Müßig,Jan Johannsen*

Main category: cs.AI

TL;DR: 本文提出了一种用于伪布尔求解中混合单元传播的新启发式方法，该方法在RoundingSAT求解器中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 伪布尔求解中当前最成功的单元传播策略是结合了监视字面量方案和计数方法的混合模式。研究旨在改进这种混合决策的性能。

Method: 引入了针对混合决策（结合监视字面量和计数方法）的新启发式方法。

Result: 这些新启发式方法在RoundingSAT求解器中能够显著超越现有方法。

Conclusion: 新的启发式方法为伪布尔求解中的混合单元传播策略带来了显著的性能提升。
...
TL;DR: Prune4Web提出了一种新颖范式，通过LLM生成的Python脚本进行高效的程序化DOM剪枝，显著减少了网页自动化中LLM处理DOM的负担，提高了动作定位的精度和效率。


<details>
  <summary>Details</summary>
Motivation: 当前的LLM驱动的网页代理在处理复杂、真实的网页时面临挑战，因为DOM结构过于庞大（10,000至100,000个token），导致效率低下。现有方法（如粗略截断或低效启发式）要么丢失关键信息，要么无法在精度和可扩展性之间取得平衡。

Method: 引入了Prune4Web范式，将DOM处理从资源密集型LLM阅读转变为高效的程序化剪枝。核心是DOM树剪枝编程，LLM生成可执行的Python评分脚本，根据分解子任务的语义线索动态过滤DOM元素。这消除了LLM直接摄取大规模原始DOM的需求。此外，提出了一种专门的数据标注流程和两轮对话训练策略，以统一优化规划器、程序化过滤器和定位器。

Result: 该方法实现了25倍到50倍的候选元素减少，从而促进了精确的动作定位并减轻了注意力稀释。在低级定位任务上，Prune4Web将准确率从46.8%大幅提高到88.28%，并展示了最先进的性能。

Conclusion: Prune4Web通过其程序化DOM剪枝方法，有效解决了大规模DOM结构对LLM驱动网页代理的挑战，显著提升了真实世界网页自动化的效率和精度。
...
TL;DR: ChatDRex是一个基于对话的多智能体系统，旨在通过自然语言促进网络化药物再利用预测的复杂生物信息学分析，使非计算机科学专家也能访问生物信息学资源。


<details>
  <summary>Details</summary>
Motivation: 传统药物开发耗时且成本高昂。药物再利用具有潜力，但在硅预测面临挑战：现有工具和数据分散且需要专业知识，导致非计算机科学专家难以进行复杂的生物信息学分析。

Method: ChatDRex是一个基于集成系统医学知识图谱NeDRex的对话式多智能体系统。它提供对生物医学知识图谱的自然语言访问，并集成了用于网络分析、药物再利用、功能一致性评估、文献挖掘和科学讨论的生物信息学智能体。其灵活的多智能体设计将特定任务分配给专业智能体，并包含一个推理模块以保持用户参与并检测幻觉。

Result: ChatDRex使没有计算机科学专业知识的医生和研究人员能够用自然语言控制复杂的分析，从而民主化了生物信息学在药物再利用中的应用。它赋能临床专家生成假设并探索药物再利用机会。

Conclusion: ChatDRex通过使生物信息学分析更易于访问，加速了新疗法的发现，推动了精准医疗和转化研究的发展。
...
TL;DR: 本文提出Extreme Weather Expert (EWE)，一个用于极端天气事件自动诊断推理的智能代理框架，旨在解决传统专家驱动方法的瓶颈，并引入了首个基准数据集和评估指标。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件对全球社会构成日益增长的风险，亟需揭示其物理机制。然而，当前专家驱动、劳动密集型的诊断范式造成了分析瓶颈，阻碍了科学进步。尽管地球科学中的人工智能在预测方面取得了进展，但自动化诊断推理这一同样重要的挑战仍未被充分探索。

Method: 本文提出了Extreme Weather Expert (EWE) 框架，通过知识引导规划、闭环推理和领域定制的气象工具包来模拟专家工作流程。EWE能够从原始气象数据中自主生成和解释多模态可视化，以实现全面的诊断分析。为了推动该领域发展，研究还引入了首个基准测试，包括一个包含103个高影响事件的精选数据集和一个新颖的逐步评估指标。

Result: EWE是第一个专注于极端天气事件自动诊断推理的智能代理框架。它能够自主生成并解释多模态可视化，从而实现全面的诊断分析。此外，本文还建立了该新兴领域的首个基准，包含一个精选数据集和新的分步评估指标。

Conclusion: EWE标志着自动化科学发现迈出了重要一步，并有望实现专业知识和智力资源的普及化，特别是对易受极端天气影响的发展中国家具有潜在益处。
...
TL;DR: 本文提出MADRA，一个免训练的多智能体辩论风险评估框架，通过LLM智能体集体推理和迭代投票，显著提升具身AI任务规划的安全性，降低误拒率。同时引入分层认知协作规划框架和SafeAware-VH数据集，实验证明其在安全性和执行效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 确保具身AI智能体在任务规划中的安全性至关重要，尤其是在家庭环境中，危险指令会带来重大风险。现有方法存在计算成本高昂（偏好对齐训练）或过度拒绝（单智能体安全提示）的局限性。

Method: 本文提出MADRA（Multi-Agent Debate Risk Assessment），一个免训练的多智能体辩论风险评估框架。该框架利用多个基于LLM的智能体对指令安全性进行辩论，并由一个关键评估器根据逻辑合理性、风险识别、证据质量和清晰度对响应进行评分。通过迭代审议和共识投票，MADRA减少了误拒。此外，还引入了一个分层认知协作规划框架，整合了安全、记忆、规划和自我进化机制以提高任务成功率，并贡献了SafeAware-VH基准数据集（包含800条VirtualHome中带注释的指令）。

Result: MADRA显著降低了错误拒绝，同时对危险任务保持高敏感性。在AI2-THOR和VirtualHome上的实验表明，该方法实现了90%以上的不安全任务拒绝率，同时确保安全任务的低拒绝率，在安全性和执行效率方面均优于现有方法。分层框架通过持续学习提高了任务成功率。

Conclusion: 该工作为构建值得信赖的具身智能体提供了一个可扩展、模型无关的解决方案，显著提升了具身AI在复杂环境中的安全性和可靠性。
...
### [23] [Self-Transparency Failures in Expert-Persona LLMs: A Large-Scale Behavioral Audit](https://arxiv.org/abs/2511.21569)
*Alex Diep*

Main category: cs.AI

TL;DR: 本研究发现，大型语言模型在扮演专业角色时，其AI身份披露在不同领域间存在显著不一致，且与模型规模无关，主要受训练因素影响。这种不一致可能导致用户过度信任，因此需要刻意的行为设计和实证验证来确保部署安全。


<details>
  <summary>Details</summary>
Motivation: 如果语言模型在专家语境中不能可靠地披露其AI身份，用户就无法信任其能力边界，这在高风险领域中可能因虚假专业知识而对用户造成伤害。

Method: 采用通用设计（common-garden design），对十六个开源模型（参数范围4B-671B）进行了19,200次试验审计。模型被赋予专业角色（如财务顾问、神经外科医生）。使用贝叶斯验证和Rogan-Gladen校正来确认结果的稳健性。

Result: 模型表现出领域特异性的显著不一致：财务顾问角色最初披露率为30.8%，而神经外科医生角色仅为3.5%。这可能导致“反盖尔曼失忆症”效应。披露率从2.8%到73.6%不等，一个14B模型达到61.4%，而一个70B模型仅为4.1%。模型身份比参数数量更能预测行为（$ΔR_{adj}^{2} = 0.359$ vs 0.018）。推理优化在某些模型中积极抑制了自我透明度，推理变体的披露率比基础版本低48.4%。贝叶斯验证确认了测量误差的稳健性（$κ= 0.908$）。

Conclusion: 这些发现表明，透明度反映的是训练因素而非模型规模。组织不能假设安全属性会转移到部署环境中，需要进行有意的行为设计和实证验证。
...
TL;DR: 本文提出悲观验证（pessimistic verification）方法，通过并行多重验证来显著提升开放式数学问题验证的错误检测能力和性能，且计算资源消耗低。


<details>
  <summary>Details</summary>
Motivation: 数学问题验证性能的主要限制在于错误检测能力不足。特别是对于开放式数学问题，提高错误检测是关键。

Method: 设计了悲观验证（pessimistic verification）的工作流程。该方法为同一证明构建多个并行验证，只要其中任何一个报告错误，该证明就被判定为不正确。

Result: 悲观验证显著提升了多个数学验证基准测试的性能，且无需大量计算资源。其token效率甚至超越了扩展的long-CoT。案例研究表明，强模型中的大部分假阴性实际上是由原始数据集中的标注错误引起的，因此该方法的实际性能可能被低估了。

Conclusion: 数学问题的自验证（如悲观验证）能有效提高语言模型输出的可靠性和性能，并在实现长周期数学任务中发挥关键作用。悲观验证的研究将有助于增强语言模型在广泛任务中的数学能力。
...
### [25] [SpatialBench: Benchmarking Multimodal Large Language Models for Spatial Cognition](https://arxiv.org/abs/2511.21471)
*Peiran Xu,Sudong Wang,Yao Zhu,Jianing Li,Yunjian Zhang*

Main category: cs.AI

TL;DR: 本文提出了一个分层空间认知框架和基准（SpatialBench），以系统评估多模态大语言模型（MLLMs）的空间智能，揭示了模型在高级推理方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准过分简化了空间认知，未能捕捉其层次结构和相互依赖性，无法全面评估MLLMs的空间能力。

Method: 研究者提出了一个将空间智能分解为五个递进复杂层次的分层空间认知框架，并构建了包含15项任务的大规模细粒度基准SpatialBench。为统一评估，还引入了一个以高层能力为导向的度量标准。通过对大量MLLMs进行实验并进行人类测试来验证。

Result: MLLMs在感知基础方面表现出色，但在符号推理、因果推断和规划方面仍有限制。人类表现出选择性、目标导向的抽象能力，而MLLMs倾向于过度关注表面细节，缺乏连贯的空间意图。

Conclusion: 这项工作建立了首个衡量MLLMs分层空间认知的系统框架，为未来空间智能系统的发展奠定了基础。
...
### [26] [Bridging the Unavoidable A Priori: A Framework for Comparative Causal Modeling](https://arxiv.org/abs/2511.21636)
*Peter S. Hovmand,Kari O'Donnell,Callie Ogland-Hand,Brian Biroscak,Douglas D. Gunzler*

Main category: cs.AI

TL;DR: 本文提出将系统动力学与结构方程模型整合到共同的数学框架中，以克服方法论障碍，从而更好地将系统动力学应用于负责任的AI/ML开发。


<details>
  <summary>Details</summary>
Motivation: AI/ML模型在解决问题时可能无意中放大人类偏见，因此需要借鉴系统动力学更丰富的因果模型来指导负责任的AI/ML开发。然而，不同方法论（如系统动力学和AI/ML）的底层假设差异是主要障碍。

Method: 将系统动力学和结构方程模型（SEM）整合到一个共同的数学框架中。

Result: 该框架可用于从分布中生成系统、开发新方法，并比较结果。

Conclusion: 旨在为数据科学和AI/ML应用中的系统动力学提供底层的认识论基础。
...
TL;DR: 本文提出了“负责任的计算预见”概念，探讨了以人为中心的AI和计算模型在推进负责任的预见中的作用，并强调AI应作为辅助工具，补充而非替代人类决策，以塑造可持续和道德的未来。


<details>
  <summary>Details</summary>
Motivation: 面对快速的技术进步和复杂的全球挑战，决策者需要一种框架来应对未来的不确定性并塑造未来。负责任的预见作为一种伦理预判新兴机遇和风险的方法，旨在促进积极、可持续和负责任的未来设计，这促使研究如何利用技术增强此能力。

Method: 本文通过提出“负责任的计算预见”这一新术语，确立了该领域的一套基本原则，并展示了当前正在塑造该领域的一系列AI驱动的预见工具。文章主张AI应作为负责任的、以人为中心的预见中的辅助工具，补充而非替代决策者的判断。

Result: AI（特别是结合模拟和情景分析）能增强决策者应对不确定性、评估风险和制定可持续策略的能力。然而，负责任的预见超越技术预测，需要对社会、环境、经济和政治系统间的相互依赖有细致理解，并致力于伦理、长期决策。AI在此过程中扮演支持性角色，以实现有弹性和道德健全的未来。

Conclusion: 文章倡导将AI深思熟虑地整合到预见实践中，以赋能决策者和社区应对21世纪的巨大挑战，并强调AI应作为人类中心预见的辅助工具，而非替代人类判断。
...
### [28] [Agentic Learner with Grow-and-Refine Multimodal Semantic Memory](https://arxiv.org/abs/2511.21678)
...
TL;DR: 本文提出了ViLoMem，一个双流、基于图式的记忆框架，旨在通过分别编码视觉干扰模式和逻辑推理错误，使多模态大语言模型（MLLMs）能够从成功和失败的经验中学习，从而减少重复错误并提高性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在独立查询上表现良好，但往往重复犯错。现有记忆增强智能体主要存储轨迹，但轨迹记忆存在简短偏差，且只记录单模态痕迹，未能保留视觉注意和逻辑推理如何共同贡献解决方案。这与人类多模态、整合的语义记忆（通过协调但独立的表征流保留视觉和抽象知识）不符。

Method: ViLoMem是一个双流记忆框架，构建紧凑的、基于图式的记忆。它分别编码视觉干扰模式和逻辑推理错误，使MLLMs能够从经验中学习。遵循“增长与精炼”原则，系统逐步积累和更新多模态语义知识，保留稳定、可泛化的策略，同时避免灾难性遗忘。

Result: 在六个多模态基准测试中，ViLoMem持续提高了pass@1准确率，并显著减少了重复的视觉和逻辑错误。消融实验证实了具有明确干扰-幻觉分离的双流记忆的必要性。

Conclusion: ViLoMem的错误感知多模态记忆对于终身和跨领域的智能体学习具有重要价值，它能帮助MLLMs更好地从经验中学习并避免重复犯错。
...
### [29] [On the Limits of Innate Planning in Large Language Models](https://arxiv.org/abs/2511.21591)
*Charles Schepanowski,Charles Ling*

Main category: cs.AI

TL;DR: 本研究使用8-puzzle任务评估了大型语言模型（LLMs）在不使用外部工具情况下的规划和状态推理能力。结果显示，LLMs存在脆性内部状态表示和弱启发式规划的严重缺陷，即使有反馈或外部验证器协助，也无法有效解决问题，表明其规划能力存在显著局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在许多基准测试中表现出色，但它们在不依赖代码执行或其他工具的情况下进行规划和有状态推理的能力仍不明确。本研究旨在直接探究LLMs的这些核心能力。

Method: 研究采用8-puzzle（八数码）作为测试任务，该任务要求状态跟踪和目标导向规划。测试了四种LLM模型，使用了常见的提示条件（零样本、思维链、算法思维）和分层纠正反馈。此外，还引入了提供有效移动的外部验证器来辅助模型。

Result: 纠正反馈在某些模型-提示组合下提高了成功率，但许多成功的运行过程冗长、计算成本高且间接。即使在外部移动验证器的协助下，所有模型也未能解决任何谜题。定性分析揭示了两个主要缺陷：1) 脆性的内部状态表示，导致频繁的无效移动；2) 弱启发式规划，模型常陷入循环或选择无法缩短与目标距离的动作。

Conclusion: 研究结果表明，在缺乏代码解释器等外部工具的情况下，当前LLMs在规划方面存在显著局限性。未来的进展可能需要引入维护显式状态和执行结构化搜索的机制。
...
### [30] [Inferix: A Block-Diffusion based Next-Generation Inference Engine for World Simulation](https://arxiv.org/abs/2511.20714)
...
TL;DR: 世界模型通过半自回归解码能生成长、真实、交互式视频，并有望突破现有LLM-中心视觉模型。Inferix是一款为世界模型设计的下一代推理引擎，通过优化半自回归解码实现高效、高质量、交互式的世界合成，并支持基准测试。


<details>
  <summary>Details</summary>
Motivation: 世界模型作为代理AI、具身AI和游戏的核心模拟器，能够生成长、物理真实且交互式的高质量视频。扩展这些模型可以解锁视觉感知、理解和推理方面的涌现能力，超越当前以LLM为中心的视觉基础模型。半自回归（块扩散）解码范式是实现这一突破的关键，但需要专门的推理引擎来高效利用其优势。

Method: 核心方法是半自回归（块扩散）解码范式，它结合了扩散和自回归方法的优点，通过在每个块内应用扩散并以先前块为条件来生成视频tokens。Inferix通过重新引入LLM风格的KV缓存管理来优化这一解码过程，从而实现高效、可变长度和高质量的生成。此外，它还支持交互式视频流和分析，并集成了LV-Bench进行高效基准测试。

Result: Inferix作为下一代推理引擎，通过优化的半自回归解码过程，实现了沉浸式世界合成。它能够进行高效、可变长度、高质量的视频生成，支持实时交互和逼真的模拟。它还通过与LV-Bench的无缝集成，实现了高效的基准测试，特别适用于分钟级视频生成场景。

Conclusion: Inferix是一款专为世界模型设计的下一代推理引擎，通过优化半自回归解码，实现了高效、高质量、交互式的世界合成，并支持细粒度基准测试。它与现有高并发系统或经典视频扩散模型不同，旨在推动世界模型探索和社区合作。
...
### [31] [Are Neuro-Inspired Multi-Modal Vision-Language Models Resilient to Membership Inference Privacy Leakage?](https://arxiv.org/abs/2511.20710)
*David Amebley,Sayanton Dibbo*

Main category: cs.CV

TL;DR: 本研究发现，通过引入神经科学启发的拓扑正则化框架，多模态视觉-语言模型（VLMs）能显著提高对成员推理攻击（MIA）的弹性，同时保持模型效用。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型（MMs）的广泛部署，它们引入了新的攻击途径，可能导致敏感训练数据泄露。现有研究主要关注单模态AI-ML系统的隐私攻击，但MMs也易受攻击。虽然生物启发神经网络已被证明能提高单模态模型对抗对抗性攻击的弹性，但神经启发MMs是否能抵抗隐私攻击仍未被探索。

Method: 本文引入了一个系统性的神经科学启发的拓扑正则化（tau）框架，以分析MM VLMs对抗基于图像-文本的推理隐私攻击的弹性。研究使用了BLIP、PaliGemma 2和ViT-GPT2三种VLM，并在COCO、CC3M和NoCaps三个基准数据集上进行了实验。通过比较基线VLM和具有拓扑正则化（tau > 0）的神经VLM，评估了其对成员推理攻击的成功率（ROC-AUC）和模型效用（MPNet和ROUGE-2指标）。

Result: 在BLIP模型和COCO数据集上的实验表明，神经VLM（NEURO VLMs）的MIA攻击成功率平均ROC-AUC下降了24%，同时在MPNet和ROUGE-2指标上保持了相似的模型效用（生成和参考标题之间的相似性）。对PaliGemma 2和ViT-GPT2模型在CC3M和NoCaps数据集上的广泛评估进一步验证了这些发现的一致性。

Conclusion: 神经启发的VLM在不显著损害模型效用的前提下，对隐私攻击具有更强的弹性。这项工作增进了对MMs中隐私风险的理解，并为神经VLM的隐私威胁弹性提供了证据。
...
### [32] [Video Object Recognition in Mobile Edge Networks: Local Tracking or Edge Detection?](https://arxiv.org/abs/2511.20716)
*Kun Guo,Yun Shen,Xijun Wang,Chaoqun You,Yun Rui,Tony Q. S. Quek*

Main category: cs.CV

TL;DR: 本文提出了一种名为LTED-Ada的深度强化学习算法，用于资源受限设备上的视频对象识别。该算法能自适应地在本地跟踪和边缘检测之间进行选择，并通过联邦学习扩展到多设备场景，以优化识别准确性和延迟，并在实际硬件中展示了其优越性。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备（如交通摄像头）上的逐帧视频对象识别面临挑战。移动边缘计算的混合方法（边缘检测+本地跟踪）虽然有前景，但如何决定何时执行边缘检测或本地跟踪是一个新的难题。

Method: 研究者首先为单设备和多设备场景建立了两个长期优化问题，考虑了帧的时间相关性和网络动态条件。在此基础上，提出了：1. 单设备设置下的LTED-Ada算法，该算法基于深度强化学习，根据帧率、识别准确性和延迟要求自适应地选择本地跟踪或边缘检测。2. 多设备设置下，通过联邦学习增强LTED-Ada，实现设备间的协作策略训练，提高对未见帧率和性能要求的泛化能力。最后，通过使用多台树莓派4B设备和一台PC作为边缘服务器进行了广泛的硬件在环实验。

Result: 通过广泛的硬件在环实验，证明了LTED-Ada算法的优越性，能够有效解决自适应选择本地跟踪和边缘检测的问题。

Conclusion: LTED-Ada算法成功地解决了资源受限设备在移动边缘网络中进行视频对象识别时的自适应计算卸载决策问题，通过结合深度强化学习和联邦学习，实现了在准确性、延迟和泛化能力上的显著提升。
...
### [33] [DeeAD: Dynamic Early Exit of Vision-Language Action for Efficient Autonomous Driving](https://arxiv.org/abs/2511.20720)
*Haibo HU,Lianming Huang,Nan Guan,Chun Jason Xue*

Main category: cs.CV

TL;DR: DeeAD是一个免训练、动作引导的早期退出框架，通过评估中间轨迹的物理可行性来加速自动驾驶中的视觉语言动作（VLA）规划模型，显著降低推理延迟，同时保持规划质量和安全性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的视觉语言动作（VLA）模型由于其深度Transformer堆栈，存在显著的推理延迟问题。

Method: DeeAD是一个免训练、动作引导的早期退出框架。它不依赖置信度分数，而是当预测轨迹与轻量级规划先验（如导航或低精度规划）在可容忍偏差（<2m）内对齐时终止推理。为提高效率，它引入了一个多跳控制器，根据分数的变化率自适应地跳过冗余层。DeeAD可以集成到现有VLA模型（如ORION）中，无需重新训练。

Result: 在Bench2Drive基准测试中，DeeAD实现了高达28%的Transformer层稀疏性和29%的延迟减少，同时保持了规划质量和安全性。

Conclusion: DeeAD通过利用物理可行性和早期退出机制，有效加速了VLA规划，在不影响性能的前提下提高了效率。
...
### [34] [Foundry: Distilling 3D Foundation Models for the Edge](https://arxiv.org/abs/2511.20721)
*Guillaume Letellier,Siddharth Srivastava,Frédéric Jurie,Gaurav Sharma*

Main category: cs.CV

TL;DR: 本文提出基础模型蒸馏（FMD），一种将大型自监督预训练模型压缩为紧凑、高效且保留通用表征能力的代理模型的新范式，并以3D点云的Foundry作为首次实现，使其适用于边缘设备。


<details>
  <summary>Details</summary>
Motivation: 自监督学习预训练的基础模型虽强大，但其巨大的规模和计算成本使其难以部署在机器人、AR/VR头显等边缘设备上。现有压缩技术（如知识蒸馏）创建的“专家”模型牺牲了基础模型关键的、与下游任务无关的通用性。

Method: 引入基础模型蒸馏（FMD）范式，旨在压缩大型自监督模型同时保留其通用表征能力。具体实现为Foundry，针对3D点云，通过训练学生模型学习一组压缩的“SuperTokens”，以重建教师模型的token级表示，从而捕捉其潜在空间的紧凑基础。

Result: 单个蒸馏模型在分类、部件分割和少样本学习等多种下游任务中表现出强大的可迁移性，性能接近完整基础模型，同时显著减少了token数量和浮点运算量（FLOPs）。

Conclusion: FMD（通过Foundry实现）使得大型基础模型在资源受限的硬件上部署成为可能，因为它能够以紧凑的形式维持模型的通用表征能力。
...
### [35] [DinoLizer: Learning from the Best for Generative Inpainting Localization](https://arxiv.org/abs/2511.20722)
*Minh Thong Doi,Jan Butora,Vincent Itier,Jérémie Boulanger,Patrick Bas*

Main category: cs.CV

TL;DR: DinoLizer是一种基于DINOv2的模型，用于定位生成式图像修复中的篡改区域，其性能超越现有技术，并对后处理操作具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是需要有效地检测和定位生成式图像修复（inapinting）图像中的局部篡改区域。

Method: 该方法构建于一个预训练的DINOv2模型之上，该模型在B-Free数据集上用于检测合成图像。研究人员在Vision Transformer的补丁嵌入层之上添加了一个线性分类头，以14x14的补丁分辨率预测篡改。该头部被训练以关注语义改变的区域。对于大尺寸图像，采用滑动窗口策略聚合预测，并通过后处理精炼二值篡改掩码。

Result: DinoLizer在各种基于不同生成模型的图像修复数据集上超越了最先进的局部篡改检测器。它对常见的后处理操作（如调整大小、添加噪声和JPEG压缩）保持鲁棒性。平均而言，DinoLizer的交并比（IoU）比次优模型高出12%，经过后处理后增益更大。实验证明了DINOv2在这一任务上强大的表示能力，并且与DINOv2及其后续版本DINOv3的消融研究证实了DinoLizer的优越性。

Conclusion: DinoLizer是一个卓越且鲁棒的DINOv2基模型，能够有效定位生成式图像修复中的篡改区域，其性能显著优于现有技术，并能抵抗常见的图像后处理操作。
...
### [36] [MODEST: Multi-Optics Depth-of-Field Stereo Dataset](https://arxiv.org/abs/2511.20853)
*Nisarg K. Trivedi,Vinayak A. Belludi,Li-Yun Wang,Pardis Taghavi,Dante Lok*

Main category: cs.CV

TL;DR: 本文提出了一个大规模、高分辨率、真实世界的立体单反数据集，包含18000张图像，系统地改变焦距和光圈，旨在弥补合成数据与真实相机光学之间的差距，并支持深度估计等领域的研究。


<details>
  <summary>Details</summary>
Motivation: 深度估计和景深渲染的现有研究受限于缺乏大规模、高保真、真实的立体单反数据集，这限制了模型在现实世界中的泛化能力和评估，尤其对于那些在合成数据上训练的模型。

Method: 构建了一个包含18000张高分辨率（5472x3648px）图像的立体单反数据集。该数据集在9个复杂的真实场景中，使用两台相同的相机，系统地改变了10种焦距（28-70mm）和5种光圈（f/2.8-f/22），共50种光学配置，每个场景捕获2000张图像。每个焦距配置都有一套专门的校准图像集，并包含多尺度错觉、反射表面、透明玻璃等挑战性视觉元素。

Result: 该数据集提供了全面的光学覆盖，支持对单目和立体深度估计、浅景深渲染、去模糊、3D场景重建和新视角合成中的几何和光学效应进行受控分析。它有助于评估基于经典和学习的内外参校准方法，并揭示了当前最先进的单目、立体深度和景深方法所面临的挑战。

Conclusion: 该工作旨在弥合合成训练数据与真实相机光学之间的现实差距，并展示了当前最先进的单目、立体深度和景深方法的挑战。数据集、校准文件和评估代码已发布，以支持真实世界光学泛化方面的可重现研究。
...
### [37] [CANVAS: A Benchmark for Vision-Language Models on Tool-Based User Interface Design](https://arxiv.org/abs/2511.20737)
*Daeheon Jeong,Seoyeon Byun,Kihoon Son,Dae Hyun Kim,Juho Kim*

Main category: cs.CV

TL;DR: 本文介绍了CANVAS，一个评估视觉语言模型（VLM）基于工具的用户界面（UI）设计能力的基准。该基准包含复制和修改两类任务，结果显示领先模型能更策略性地调用工具，提高设计质量，并揭示了常见的错误模式。


<details>
  <summary>Details</summary>
Motivation: UI设计是一个迭代过程，设计师使用Figma等软件。最近的VLM与工具调用相结合，表明它们有可能操作设计软件进行UI编辑。理解和增强这种能力很重要，因为它突出了VLM与设计师在传统软件中协作的潜力。然而，由于缺乏评估基于工具设计性能的现有基准，这种能力仍是未知的。

Method: 本文引入了CANVAS，一个用于评估VLM基于工具的UI设计的基准。它包含598个基于工具的设计任务，配有从3.3K移动UI设计中采样的真实参考，涵盖30个功能类别。每个任务要求VLM通过上下文相关的工具调用（例如，创建一个矩形作为按钮背景）逐步更新设计，这些调用与设计软件相关联。CANVAS包含两种任务类型：设计复制（评估重现整个UI屏幕的能力）和设计修改（评估修改现有屏幕特定部分的能力）。

Result: 研究结果表明，领先的模型表现出更具策略性的工具调用，从而提高了设计质量。此外，我们还识别了模型展现的常见错误模式。

Conclusion: CANVAS基准有助于理解VLM在基于工具的UI设计中的能力。研究结果揭示的错误模式为未来增强VLM的基于工具的设计能力指明了方向。
...
### [38] [Text-Guided Semantic Image Encoder](https://arxiv.org/abs/2511.20770)
*Raghuveer Thirukovalluru,Xiaochuang Han,Bhuwan Dhingra,Emily Dinan,Maha Elbayad*

Main category: cs.CV

TL;DR: 本文提出文本引导语义图像编码器（TIE），它根据文本查询生成图像表示，显著提升了视觉-语言模型（VLMs）在图像到文本任务上的性能和推理效率，同时增强了可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统的图像编码器在预训练时独立于语言模型，并且在处理图像时是“不可知”的，不考虑下游任务或文本查询，这限制了视觉-语言模型的性能。

Method: 本文提出了文本引导语义图像编码器（TIE），该编码器能够根据输入的文本查询生成条件化的图像表示。这种方法使得图像编码器能够有针对性地处理图像信息。

Result: 配备TIE的VLMs在九个图像到文本基准测试中，1B和3B规模下平均性能分别提高了1.5和1.3点，在DocVQA和InfoVQA等任务上增益高达6点。此外，TIE-based VLMs仅使用一半的图像块（tokens），显著提高了推理效率。TIE对通用查询也表现出良好的泛化能力，并通过定性分析证实其能持续关注与查询相关的区域。

Conclusion: 文本条件化训练（TIE）有效地优化了编码器以捕获关键视觉特征，增强了可解释性和查询特异性基础，从而使视觉-语言模型获得卓越的性能和更高的推理效率。
...
### [39] [One Patch is All You Need: Joint Surface Material Reconstruction and Classification from Minimal Visual Cues](https://arxiv.org/abs/2511.20784)
...
TL;DR: SMARC是一个统一模型，仅通过10%的图像补丁即可重建和分类完整表面材料，在极端稀疏观测下表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的材料表面理解方法依赖于密集或全场景观测，这限制了它们在受限或部分视图环境中的有效性。

Method: SMARC模型结合了一个部分卷积U-Net和一个分类头，实现在极度稀疏观测下（仅10%连续图像补丁）的空间修复和语义理解，同时进行表面重建和材料分类。

Result: SMARC在真实世界表面纹理数据集（Touch and Go）上，实现了17.55 dB的PSNR和85.10%的材料分类准确率，优于其他五种主流模型，达到最先进水平。

Conclusion: 研究结果突出了部分卷积在缺失数据空间推理方面的优势，并为最小视觉表面理解奠定了坚实基础。
...
### [40] [Revisiting KRISP: A Lightweight Reproduction and Analysis of Knowledge-Enhanced Vision-Language Models](https://arxiv.org/abs/2511.20795)
*Souradeep Dutta,Keshav Bulia,Neena S Nair*

Main category: cs.CV

TL;DR: 本文对Facebook AI Research的KRISP模型进行了轻量级复现，显著减少了参数，揭示了原模型的设计缺陷和实际问题，并探讨了资源受限下知识增强型视觉问答（VQA）架构的可扩展性和有效性。


<details>
  <summary>Details</summary>
Motivation: 原始KRISP模型计算量大，参数众多，且与大型骨干网络紧密耦合，不适用于资源受限的环境。研究旨在开发一个轻量级的替代方案，以适应边缘设备。

Method: 研究团队从不同角度重新审视KRISP，开发了一个参数显著减少的轻量级复现模型。通过系统的消融研究，包括在合成VQA数据上的概念验证和在DAQUAR数据集上的评估，来分析其可扩展性和有效性。模型被配置为在外部知识图谱领域内生成输出，以防止AI幻觉。

Result: 复现模型达到了原始模型约75%的性能，并揭示了原论文中未完全涵盖的多个设计缺陷、实际陷阱和隐含问题。研究证明了在资源限制下，知识增强型VQA架构的可扩展性和有效性。该模型通过限制外部知识图谱领域，有效防止了AI幻觉，并能在智能手机和AR-VR等边缘设备上运行。

Conclusion: 一个参数量少、受外部知识图谱约束的轻量级知识增强型VQA模型，不仅能有效防止AI幻觉，还能在边缘设备上运行，显著改善离线视觉推理能力。这项工作也揭示了大型工业模型在实际部署中的潜在问题。
...
### [41] [LongVT: Incentivizing "Thinking with Long Videos" via Native Tool Calling](https://arxiv.org/abs/2511.20785)
*Zuhao Yang,Sudong Wang,Kaichen Zhang,Keming Wu,Sicong Leng,Yifan Zhang,Chengwei Qin,Shijian Lu,Xingxuan Li,Lidong Bing*

Main category: cs.CV

TL;DR: 长视频中大型多模态模型（LMMs）易出现幻觉。本文提出LongVT框架，通过多模态工具链思维实现“长视频思考”，模仿人类先全局浏览再局部细查的推理方式。LongVT利用LMMs的时间定位能力作为视频裁剪工具，进行全局到局部的循环推理，并引入新数据集VideoSIAH。经验证，LongVT在长视频理解和推理任务中显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型（LMMs）在处理长视频时，由于证据稀疏且时间分散，容易产生幻觉。现有方法未能有效解决这一问题，且缺乏类似人类的先全局后局部的推理策略。

Method: LongVT是一个端到端的智能体框架，通过交错的多模态工具链思维（Multimodal Chain-of-Tool-Thought）实现“长视频思考”。它利用LMMs固有的时间定位能力作为视频裁剪工具，实现从全局概览到局部细节的推理循环，直至找到视觉证据。为解决长视频推理任务中细粒度问答数据稀缺问题，本文构建并发布了VideoSIAH数据集，包含用于工具集成冷启动监督微调（247.9K样本）、智能体强化学习（1.6K样本）和智能体强化微调（15.4K样本）的训练数据，以及1,280个经过人工验证的评估问答对。采用精心设计的三阶段训练策略。

Result: LongVT在四个具有挑战性的长视频理解和推理基准测试中，始终优于现有的强大基线模型。

Conclusion: LongVT通过引入一种模仿人类思维的全局到局部推理机制和多模态工具链思维，并辅以精心策划的数据集和训练策略，有效解决了大型多模态模型在长视频推理中易产生幻觉的问题，显著提升了长视频理解和推理性能。
...
### [42] [Intriguing Properties of Dynamic Sampling Networks](https://arxiv.org/abs/2511.20800)
*Dario Morle,Reid Zaffino*

Main category: cs.CV

TL;DR: 本文提出并分析了一种名为“扭曲”（warping）的新型广义动态采样算子，统一了现有动态采样方法，揭示了其训练前向和反向传播之间的独特不对称性，并确定了稳定训练的条件，同时引入了一种新的损失景观可视化方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习架构中的动态采样机制在计算机视觉模型中显示出实用性，但这些结构的理论分析尚未统一。

Method: 本文开发并分析了一种名为“扭曲”的新型算子，该算子概括了现有动态采样方法，并可用于重建现有架构（如可变形卷积、主动卷积单元和空间变换网络）。通过将输入建模为独立同分布变量和均匀随机场，对该算子进行了统计分析。结合理论分析和实证研究，探讨了离散化效应，并引入了一种利用梯度更新信息的新型损失景观可视化方法。

Result: “扭曲”算子提供了一个可分析的动态采样最小实现，并能重建现有架构。研究发现模型训练的前向和反向传播之间存在独特的不对称性。这些机制代表了与传统平移不变卷积算子完全不同的正交算子类别。通过理论分析和实证研究，找出了确保动态采样网络稳定训练的必要条件。此外，还研究了离散化效应，并提出了一种新的损失景观可视化方法。

Conclusion: 本文通过引入“扭曲”算子统一了各种动态采样方法，提供了对该算子的统计分析，揭示了其训练过程中的不对称性，并将其归类为与传统卷积不同的正交算子。研究确定了稳定训练动态采样网络的条件，并引入了一种新的损失景观可视化工具，以更好地理解学习行为。
...
### [43] [$Δ$-NeRF: Incremental Refinement of Neural Radiance Fields through Residual Control and Knowledge Transfer](https://arxiv.org/abs/2511.20804)
*Kriti Ghosh,Devjyoti Chakraborty,Lakshmish Ramaswamy,Suchendra M. Bhandarkar,In Kee Kim,Nancy O'Hare,Deepak Mishra*

Main category: cs.CV

TL;DR: 现有NeRF在增量数据场景下需完全重训，存在灾难性遗忘问题。Δ-NeRF提出一种模块化残差框架，通过残差控制器、不确定性感知门控和视图选择策略，实现了无需历史数据的增量精炼，并采用知识蒸馏压缩模型。实验证明其性能与联合训练相当，训练时间更短，且优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 大多数NeRF框架在引入新视图时需要完全重训，这限制了它们在数据顺序到达领域的应用，例如卫星地形分析。这种增量精炼能力不足，且在没有历史数据时容易出现灾难性遗忘。

Method: Δ-NeRF是一种独特的模块化残差框架，用于增量NeRF精炼。它包含：(1) 一个残差控制器，为冻结的基础NeRF注入逐层校正，无需访问历史数据；(2) 一个不确定性感知门控机制，自适应结合基础和精炼预测，防止过度校正；(3) 一个视图选择策略，可减少高达47%的训练数据。此外，还采用知识蒸馏将增强模型压缩为紧凑的学生网络（原始大小的20%）。

Result: Δ-NeRF在卫星图像实验中，实现了与联合训练相当的性能，同时将训练时间减少了30-42%。它持续优于现有基线，在PSNR上比朴素微调提升高达43.5%，并在某些指标上超越了联合训练。模型压缩后大小为原始的20%。

Conclusion: Δ-NeRF成功解决了NeRF在增量数据场景下的精炼问题，通过独特的模块化残差框架，有效避免了灾难性遗忘，提高了训练效率，并在性能上达到甚至超越了联合训练，极大地扩展了NeRF在顺序数据领域的适用性。
...
### [44] [Layer-Aware Video Composition via Split-then-Merge](https://arxiv.org/abs/2511.20809)
*Ozgur Kara,Yujia Chen,Ming-Hsuan Yang,James M. Rehg,Wen-Sheng Chu,Du Tran*

Main category: cs.CV

TL;DR: Split-then-Merge (StM) 是一种新的视频生成框架，通过将无标签视频分解为前景和背景层并进行自组合来学习复杂合成动力学，从而增强控制并解决数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有生成视频合成方法依赖标注数据集或手工规则，难以实现精细控制并面临数据稀缺问题。

Method: StM 将大量无标签视频分解为动态前景和背景层，然后进行自组合以学习主体与场景的交互。它引入了转换感知训练流程，利用多层融合和增强实现可供性感知合成，并采用身份保持损失来维持前景保真度。

Result: 实验表明，StM 在定量基准测试以及基于人类/VLLM 的定性评估中均优于现有最先进方法。

Conclusion: StM 通过创新的分解-组合策略和训练管道，有效提升了生成视频合成的控制能力和真实感，并缓解了数据稀缺问题。
...
### [45] [Training-Free Diffusion Priors for Text-to-Image Generation via Optimization-based Visual Inversion](https://arxiv.org/abs/2511.20821)
*Samuele Dell'Erba,Andrew D. Bagdanov*

Main category: cs.CV

TL;DR: 本文提出一种名为OVI的免训练、免数据方法来替代扩散模型中计算昂贵的先验网络，并发现现有评估基准存在缺陷。


<details>
  <summary>Details</summary>
Motivation: 扩散模型中的先验网络在计算上开销巨大，且需要大量数据进行训练。本文旨在寻找一种无需训练的替代方案。

Method: 引入基于优化的视觉反演（OVI）方法，该方法从随机伪标记初始化潜在视觉表示，并通过迭代优化使其与输入文本提示嵌入的余弦相似度最大化。此外，提出了两种新的约束：基于马哈拉诺比斯距离和最近邻损失，以将OVI优化过程正则化到真实图像的分布。

Result: 实验表明，OVI可以作为传统先验的替代方案。更重要的是，分析揭示了T2I-CompBench++等当前评估基准的一个关键缺陷：仅使用文本嵌入作为先验就能获得惊人的高分，尽管感知质量较低。本文提出的受约束OVI方法（尤其是最近邻方法）在此基线上提高了视觉保真度，并取得了与最先进的数据高效先验相当或更高的量化分数。

Conclusion: OVI作为扩散模型中训练先验的替代方案具有可行性，且值得进一步研究。同时，当前文本到图像生成模型的评估基准存在需要解决的缺陷。
...
### [46] [SPHINX: A Synthetic Environment for Visual Perception and Reasoning](https://arxiv.org/abs/2511.20814)
*Md Tanvirul Alam,Saksham Aggarwal,Justin Yang Chae,Nidhi Rastogi*

Main category: cs.CV

TL;DR: Sphinx是一个合成环境，用于评估和提升视觉感知与推理能力，通过程序化生成带可验证解的谜题。基准测试显示，即使是SOTA的LVLMs（如GPT-5）也远低于人类水平。研究表明，带有可验证奖励的强化学习（RLVR）能显著提高模型在此类任务上的准确性，并对外部视觉推理基准产生积极影响。


<details>
  <summary>Details</summary>
Motivation: 旨在评估和提升模型在视觉感知与推理中的核心认知原语能力，并提供一个能生成带可验证真值解的谜题环境，以实现精确评估和大规模数据集构建。

Method: 提出了Sphinx合成环境，通过程序化方法生成包含多种元素（图案、瓷砖、图表、图标、几何原语）的谜题，并为每个谜题提供可验证的真值解。该环境涵盖25种任务类型。研究评估了最新的大型视觉-语言模型（LVLMs）在此基准上的表现，并探索了带有可验证奖励的强化学习（RLVR）方法。

Result: 评估结果显示，即使是最先进的LVLMs（如GPT-5）在此基准测试中也仅达到51.1%的准确率，远低于人类表现。此外，研究发现，带有可验证奖励的强化学习（RLVR）能够显著提高模型在这些任务上的准确性，并对外部视觉推理基准也带来了性能提升。

Conclusion: 本研究强调了带有可验证奖励的强化学习（RLVR）在推动多模态推理方面具有巨大潜力。
...
### [47] [RefTr: Recurrent Refinement of Confluent Trajectories for 3D Vascular Tree Centerline Graphs](https://arxiv.org/abs/2511.20823)
*Roman Naeem,David Hagerman,Jennifer Alvén,Fredrik Kahl*

Main category: cs.CV

TL;DR: 本文提出RefTr，一种用于血管树中心线生成的3D图像到图模型，通过递归细化合流轨迹，实现高召回率、更快的推理速度和显著减少的参数量，在多个公开数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 准确检测人体内管状树（如血管和肺气道）的中心线及其正确的树状拓扑结构对临床诊断、治疗计划和手术导航至关重要。特别是在这些应用中，保持高召回率至关重要，因为遗漏小分支可能导致评估不完整或异常未被发现，从而造成致命错误。

Method: RefTr是一个基于Transformer解码器的Producer-Refiner架构的3D图像到图模型。Producer生成一组初始的合流轨迹，Refiner通过递归细化这些轨迹以产生最终的中心线图。合流轨迹表示允许细化完整轨迹并明确强制执行有效的树状拓扑。递归细化方案提高了精度，并跨多个步骤重用相同的Refiner块，从而使解码器参数比现有SOTA减少2.4倍。此外，还引入了一种高效的空间树图非最大抑制算法来合并重复分支并提高精度。

Result: 在多个公开中心线数据集上，RefTr实现了优于现有SOTA的召回率和可比的精度，同时提供了更快的推理速度和显著减少的参数量。与现有SOTA相比，解码器参数减少了2.4倍。

Conclusion: RefTr作为一种新的3D医学影像中血管树分析的SOTA框架，展示了其潜力，具有高召回率、效率更高和参数更少的优点。
...
### [48] [Unsupervised Memorability Modeling from Tip-of-the-Tongue Retrieval Queries](https://arxiv.org/abs/2511.20854)
*Sree Bhattacharyya,Yaman Kumar Singla,Sudhir Yarram,Somesh Kumar Singh,Harini S,James Z. Wang*

Main category: cs.CV

TL;DR: 本文引入了首个大规模无监督数据集，用于建模视觉内容记忆性，包含82,000多个视频及描述性回忆数据。通过利用Reddit等在线平台的“话在嘴边”查询，该数据集在回忆生成和“话在嘴边”检索任务中展现了丰富的信号，微调后的模型在生成开放式记忆描述方面超越了GPT-4o等现有技术，并首次实现了多模态“话在嘴边”检索。


<details>
  <summary>Details</summary>
Motivation: 视觉内容记忆性研究面临的主要挑战是人工收集记忆性标注成本高昂，这限制了数据集的多样性和可扩展性。现有数据集主要收集聚合记忆性分数，未能捕捉自然、开放式回忆描述中细致的记忆信号。

Method: 研究者引入了一个大型无监督数据集，包含超过82,000个视频及其描述性回忆数据。他们利用Reddit等在线平台上的“话在嘴边”（ToT）检索查询来构建数据。该数据集用于评估两个记忆性相关任务：回忆生成和ToT检索。大型视觉-语言模型在该数据集上进行微调，并采用对比训练策略来创建首个能够执行多模态ToT检索的模型。

Result: 该无监督数据集为回忆生成和ToT检索任务提供了丰富的信号。在数据集上微调的大型视觉-语言模型在为视觉内容生成开放式记忆描述方面，表现优于GPT-4o等现有最先进模型。研究还创建了首个能够执行多模态ToT检索的模型。
...
### [49] [Estimating Fog Parameters from a Sequence of Stereo Images](https://arxiv.org/abs/2511.20865)
*Yining Ding,João F. C. Mota,Andrew M. Wallace,Sen Wang*

Main category: cs.CV

TL;DR: 本文提出了一种从立体雾图像序列中动态估计雾模型参数的方法，通过解决一个新颖的优化问题同时估计所有参数，有效处理局部均匀但全局不均匀的真实雾。同时，作者创建并发布了一个高质量的真实雾立体数据集SDIRF，以促进相关研究。


<details>
  <summary>Details</summary>
Motivation: 现有的雾参数估计方法通常是顺序估计，容易导致误差累积。此外，真实世界的雾通常是全局不均匀的，这给现有方法带来了挑战。为了使视觉SLAM或里程计系统能在有雾环境下工作，需要一种鲁棒的雾参数估计方法。

Method: 该方法从立体雾图像序列中动态估计雾模型参数。与以往顺序估计不同，它通过解决一个新的优化问题同时估计所有参数，从而避免误差传播。通过假设雾在局部是均匀的，该方法能够有效处理全局不均匀的真实雾。该算法可作为现有视觉SLAM或里程计系统的附加模块。此外，作者还创建了一个新的数据集SDIRF，包含40多分钟、34k帧的真实雾立体图像序列，具有经过实验室校准的相机光度参数，并包含相同路线的清晰图像数据。

Result: 通过在合成雾数据和SDIRF真实雾序列上进行大量实验，结果表明所提出的算法优于现有方法。它不仅在合成数据上产生了最准确的估计，而且能更好地适应真实世界的雾。该方法的代码和SDIRF数据集已公开发布。

Conclusion: 本文提出了一种先进的雾模型参数动态估计方法，能够鲁棒地处理真实世界中复杂多变的雾条件。同时发布的SDIRF数据集，特别是其独特的相机光度参数校准和清晰数据对照，将极大地推动雾中视觉感知领域的研究进展。
...
### [50] [V$^{2}$-SAM: Marrying SAM2 with Multi-Prompt Experts for Cross-View Object Correspondence](https://arxiv.org/abs/2511.20886)
...
TL;DR: V^2-SAM是一个统一的跨视角物体对应框架，它通过引入几何感知锚点提示和外观引导视觉提示，将SAM2从单视角分割扩展到跨视角对应，并在多个基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 跨视角物体对应（如自我-外部视角物体对应）面临视角和外观剧烈变化的挑战，使得现有的单视角分割模型（如SAM2）难以直接应用。

Method: 本文提出了V^2-SAM框架，通过两个互补的提示生成器将SAM2从单视角分割适应到跨视角对应：1) 跨视角锚点提示生成器（V^2-Anchor）基于DINOv3特征建立几何感知对应，并首次为SAM2在跨视角场景中解锁了基于坐标的提示；2) 跨视角视觉提示生成器（V^2-Visual）通过新颖的视觉提示匹配器，从特征和结构角度对自我-外部视角表示进行对齐，增强了外观引导线索。为有效利用这两种提示的优势，模型还采用了多专家设计，并引入了后验循环一致性选择器（PCCS），根据循环一致性自适应选择最可靠的专家。

Result: V^2-SAM在Ego-Exo4D（自我-外部视角物体对应）、DAVIS-2017（视频物体跟踪）和HANDAL-X（机器人就绪的跨视角对应）等数据集上均取得了新的最先进（SOTA）性能。

Conclusion: V^2-SAM通过创新的几何感知和外观引导提示机制，成功解决了跨视角物体对应的挑战，并有效利用SAM2的能力，在多项任务中展现了卓越的性能和泛化能力。
...
TL;DR: 本文提出Null-TTA，一种通过优化无条件嵌入来对齐扩散模型的新方法，有效防止奖励作弊，实现最先进的测试时对齐和强大的跨奖励泛化。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时对齐（TTA）方法在优化目标奖励函数时，要么欠优化，要么过优化（即奖励作弊），未能有效平衡模型对齐与泛化。

Method: Null-TTA通过优化分类器自由指导中的无条件嵌入来对齐扩散模型，而不是操纵潜在变量或噪声变量。这种方法利用文本嵌入空间的语义连贯性，确保对齐发生在语义一致的流形上，从而防止利用非语义噪声模式进行奖励作弊。

Result: Null-TTA实现了最先进的目标测试时对齐性能，同时保持了强大的跨奖励泛化能力。这表明语义空间优化是TTA的一种有效且有原则的新范式。

Conclusion: 语义空间优化是一种有效且有原则的新范式，可用于测试时对齐（TTA），能够实现卓越的对齐性能并避免奖励作弊，同时保持良好的泛化能力。
...
### [52] [GaINeR: Geometry-Aware Implicit Network Representation](https://arxiv.org/abs/2511.20924)
*Weronika Jakubowska,Mikołaj Zieliński,Rafał Tobiasz,Krzysztof Byrski,Maciej Zięba,Dominik Belter,Przemysław Spurek*

Main category: cs.CV

TL;DR: 传统隐式神经表示（INR）缺乏几何结构和局部编辑能力。本文提出GaINeR，一种结合可训练高斯分布与INR的新框架，为2D图像提供几何感知、连续表示和灵活的局部编辑能力。


<details>
  <summary>Details</summary>
Motivation: 流行的INR架构在图像重建、超分辨率和压缩方面表现出色，但它们通常缺乏显式几何结构，且局部编辑或与物理模拟集成能力有限，这限制了它们在动态或交互式场景中的应用。

Method: 本文提出了GaINeR（Geometry-Aware Implicit Network Representation）。该方法将可训练的高斯分布与基于神经网络的INR相结合。对于给定的图像坐标，模型检索K个最近的高斯分布，聚合距离加权的嵌入，并通过神经网络预测RGB值。

Result: GaINeR的设计实现了连续的图像表示、可解释的几何结构和灵活的局部编辑能力。它为物理感知和交互式图像处理奠定了基础。

Conclusion: GaINeR通过引入可训练高斯分布，解决了传统INR在几何结构和局部编辑方面的局限性，为2D图像提供了一种更强大、更具交互性的表示方法，适用于物理感知和动态图像操作。
...
### [53] [Open Vocabulary Compositional Explanations for Neuron Alignment](https://arxiv.org/abs/2511.20931)
*Biagio La Rosa,Leilani H. Gilpin*

Main category: cs.CV

TL;DR: 本文提出了一种用于视觉领域的开放词汇组合解释框架，通过利用开放词汇语义分割生成的掩码，使用户能够探测神经元以获取任意概念和数据集的信息，克服了传统方法对人工标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有的组合解释方法依赖于人工标注数据集，限制了其在特定领域和预定义概念上的适用性。研究的动机是理解神经元如何编码信息，并扩展解释的灵活性。

Method: 该框架包含三个步骤：指定任意概念、使用开放词汇模型生成语义分割掩码，以及从这些掩码中推导出组合解释。

Result: 论文将该框架与现有组合解释方法在定量指标和人类可解释性方面进行了比较，分析了从人工标注数据转向模型标注数据时解释的差异，并展示了该框架在解释灵活性方面对不同任务和兴趣属性的额外能力。

Conclusion: 该框架通过引入开放词汇语义分割，极大地提高了神经元组合解释的灵活性和适用性，使其能够处理任意概念和数据集，从而更好地理解神经元的信息编码方式。
...
### [54] [Smooth regularization for efficient video recognition](https://arxiv.org/abs/2511.20928)
*Gil Goldman,Raja Giryes,Mahadev Satyanarayanan*

Main category: cs.CV

TL;DR: 本文提出了一种平滑正则化技术，通过将连续帧的中间层嵌入建模为高斯随机游走（GRW），以鼓励时间平滑性，从而显著提升轻量级视频识别模型的性能，并在多个基准上达到新的SOTA。


<details>
  <summary>Details</summary>
Motivation: 轻量级视频识别模型在捕捉复杂时间动态方面可能存在不足，需要一种有效的方法来增强其时间归纳偏差，使其更好地利用视频固有的时间连贯性。

Method: 研究者提出了一种平滑正则化技术。该技术通过将连续帧的中间层嵌入的变化建模为高斯随机游走（GRW），惩罚突然的表示偏移，从而促进低加速度的解决方案，以更好地符合视频的自然时间连贯性。

Result: 该技术使轻量级模型在Kinetics-600数据集上的准确率提高了3.8%至6.4%。在各自的FLOPs限制内，MoViNets模型家族的性能相比当前SOTA提高了3.8%至6.1%。MobileNetV3和MoViNets-Stream家族在相似内存占用下，相比之前的SOTA模型实现了4.9%至6.4%的提升。

Conclusion: 通过引入基于高斯随机游走的平滑正则化技术，可以有效地为视频识别模型（特别是轻量级架构）注入强大的时间归纳偏差，从而显著提升其性能，并在竞争激烈的基准测试中超越现有SOTA。
...
### [55] [A deep learning model to reduce agent dose for contrast-enhanced MRI of the cerebellopontine angle cistern](https://arxiv.org/abs/2511.20926)
...
TL;DR: 本研究评估了一个深度学习模型，该模型能够显著减少对比增强T1加权MRI（T1ce）检查中造影剂的剂量，同时保持脑桥小脑角（CPA）病灶的图像质量和诊断能力。


<details>
  <summary>Details</summary>
Motivation: 减少对比增强MRI检查中造影剂的剂量，以降低潜在的副作用和成本，同时不影响图像质量和诊断准确性。

Method: 一项多中心回顾性研究，使用前庭神经鞘瘤（VS）患者的T1和T1ce图像模拟不同剂量的低剂量T1ce。训练深度学习模型，从模拟的低剂量图像中恢复标准剂量T1ce。通过图像质量指标（SSIM、PSNR）、分割性能（Dice、95% Hausdorff距离、平均表面距离）以及放射科医生的主观评分来评估DL恢复的图像。

Result: 评估了72名患者的203项MRI研究。DL恢复的图像在输入剂量增加时，SSIM从0.639提高到0.993，PSNR从21.6 dB提高到41.4 dB。在10%的输入剂量下，DL恢复的T1ce图像用于分割时，Dice系数从0.673提高到0.734，95% Hausdorff距离从2.38 mm缩短到2.07 mm，平均表面距离从1.00 mm缩短到0.59 mm。放射科医生认为，10%和30%输入剂量下DL恢复的T1ce图像质量优秀，其中30%剂量下的图像信息量更丰富。

Conclusion: 深度学习模型显著改善了低剂量CPA脑池MRI的图像质量，使得仅使用标准剂量10%-30%的造影剂即可实现病灶检测和诊断表征。
...
### [56] [UruDendro4: A Benchmark Dataset for Automatic Tree-Ring Detection in Cross-Section Images of Pinus taeda L](https://arxiv.org/abs/2511.20935)
*Henry Marichal,Joaquin Blanco,Diego Passarella,Gregory Randall*

Main category: cs.CV

TL;DR: 本文介绍了UruDendro4数据集，一个包含多种树高切片的手动标注年轮图像数据集，旨在促进年轮自动检测和体积生长建模，并提供了当前最先进方法的性能基线。


<details>
  <summary>Details</summary>
Motivation: 树木年轮的传统手动测量耗时且不精确，而现有数据集缺乏多高度切片数据，限制了基于图像的年轮自动检测和年度木材生长的体积建模能力。

Method: 研究引入了UruDendro4数据集，包含102个来自火炬松（Pinus taeda L.）的多高度横截面图像样本，并进行了手动年轮标注。此外，使用DeepCS-TRD等最先进的方法在该数据集上建立了自动年轮检测的性能基线，并进行了消融实验以验证参数配置。

Result: DeepCS-TRD方法在UruDendro4数据集上实现了最佳性能，平均精度（mAP）为0.838，平均召回率（mAR）为0.782，调整兰德误差（ARE）为0.084。实验还证明，将UruDendro4数据集纳入训练可以提高学习模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集的引入填补了多高度木材横截面数据的空白，为基于图像的年轮体积生长建模提供了基础，并显著提升了自动年轮检测模型的泛化能力，有助于更好地评估造林实践。
...
### [57] [BUSTR: Breast Ultrasound Text Reporting with a Descriptor-Aware Vision-Language Model](https://arxiv.org/abs/2511.20956)
*Rawa Mohammed,Mina Attin,Bryar Shareef*

Main category: cs.CV

TL;DR: 本文提出BUSTR，一个多任务视觉-语言框架，无需配对的图像-报告数据即可为乳腺超声生成放射学报告，通过结构化描述符和描述符感知的视觉表示来提高报告生成质量和临床有效性。


<details>
  <summary>Details</summary>
Motivation: 自动放射学报告生成（RRG）面临配对图像-报告数据集的稀缺性以及大型语言模型可能产生幻觉的限制，尤其是在乳腺超声（BUS）领域。

Method: BUSTR框架通过以下方式生成BUS报告：1) 利用结构化描述符（如BI-RADS、病理、组织学）和影像组学特征构建报告；2) 使用多头Swin编码器和多任务损失，在数据集特定的描述符集上学习描述符感知的视觉表示；3) 通过结合令牌级交叉熵和输入输出表示之间的余弦相似度对齐损失的双层目标来对齐视觉和文本令牌。

Result: 在BrEaST和BUS-BRA两个公共BUS数据集上的评估显示，BUSTR持续改进了标准的自然语言生成指标和临床有效性指标，尤其在BI-RADS类别和病理等关键目标上表现突出。结果表明，这种描述符感知的视觉模型，通过结合令牌级和对齐损失进行训练，无需配对的图像-报告数据即可改善自动报告指标和临床有效性。

Conclusion: 研究表明，提出的描述符感知视觉模型，结合令牌级和对齐损失进行训练，可以在没有配对图像-报告数据的情况下，显著提高自动报告指标和临床有效性。
...
### [58] [Beyond Realism: Learning the Art of Expressive Composition with StickerNet](https://arxiv.org/abs/2511.20957)
*Haoming Lu,David Kocharian,Humphrey Shi*

Main category: cs.CV

TL;DR: 本文提出“表现性图像合成”新任务，旨在生成艺术、趣味或社交性内容而非追求真实感。StickerNet框架通过学习真实用户编辑行为数据，实现对合成类型和放置参数的预测，效果优于基线并接近人类表现。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成侧重视觉真实感和语义合理性，但现代在线平台用户出于社区认可等动机，常创作艺术性、趣味性或社交性内容，而非追求真实感。这种用户行为激发了对非真实感、表现性合成任务的研究。

Method: 定义了“表现性合成任务”，旨在容纳风格多样性和更宽松的放置逻辑。提出了StickerNet，一个两阶段框架：首先确定合成类型，然后预测不透明度、遮罩、位置和缩放等放置参数。数据集直接来源于匿名在线视觉创作与编辑平台上的180万条用户编辑行为，而非模拟放置，确保与真实编辑行为高度一致。

Result: 用户研究和定量评估表明，StickerNet优于常见基线，并能紧密匹配人类的放置行为。这证明了尽管任务本身存在固有的模糊性，但从真实世界编辑模式中学习是有效的。

Conclusion: 这项工作为视觉理解引入了一个新方向，强调表现力和用户意图而非真实感。
...
### [59] [TrafficLens: Multi-Camera Traffic Video Analysis Using LLMs](https://arxiv.org/abs/2511.20965)
*Md Adnan Arefeen,Biplob Debnath,Srimat Chakradhar*

Main category: cs.CV

TL;DR: TrafficLens是一种针对多摄像头交通路口设计的算法，通过利用摄像头重叠区域和迭代VLM，并结合对象级相似性检测，显著减少了交通视频到文本的转换时间，同时保持了信息准确性。


<details>
  <summary>Details</summary>
Motivation: 城市交通摄像头在智能交通系统中至关重要，但管理和分析多摄像头海量视频数据面临挑战。虽然LLM结合RAG在文本任务中表现出色，但将视频数据转换为文本供其分析需要耗时的VLM处理，这延迟了交通视频洞察的生成和事件调查。

Method: TrafficLens采用顺序方法，利用摄像头重叠覆盖区域。它迭代地应用具有不同token限制的VLM，将前一个摄像头的输出作为后续摄像头的提示，从而实现快速生成详细文本描述并减少处理时间。此外，TrafficLens通过对象级相似性检测器智能地跳过冗余的VLM调用。

Result: 使用真实世界数据集进行的实验表明，TrafficLens将视频到文本的转换时间减少了高达4倍，同时保持了信息准确性。

Conclusion: TrafficLens成功解决了多摄像头交通视频分析中VLM转换耗时的问题，通过其创新方法显著提高了处理效率，为及时获取交通洞察提供了有效工具。
...
### [60] [Privacy-Preserving Federated Vision Transformer Learning Leveraging Lightweight Homomorphic Encryption in Medical AI](https://arxiv.org/abs/2511.20983)
*Al Amin,Kamrul Hasan,Liang Hong,Sharif Ullah*

Main category: cs.CV

TL;DR: 本文提出了一种结合Vision Transformers (ViT) 和同态加密 (HE) 的隐私保护联邦学习框架，用于多机构组织病理学分类，通过加密ViT的CLS token实现安全高效的数据聚合和推理，有效抵御重建攻击。


<details>
  <summary>Details</summary>
Motivation: 医疗机构希望通过协作机器学习利用多样化数据集提高诊断准确性，但HIPAA等隐私法规禁止直接共享患者数据。虽然联邦学习 (FL) 允许去中心化训练，但传统FL中的模型梯度容易受到重建攻击，可能暴露敏感医疗信息。

Method: 该方法结合Vision Transformers (ViT) 和同态加密 (HE)。它利用ViT的CLS token作为紧凑的768维特征表示，使用CKKS同态加密技术在传输到服务器前对这些token进行加密，以实现安全聚合。此外，该框架支持直接在密文上进行加密推理。

Result: 与梯度加密相比，加密CLS token可实现30倍的通信量减少。研究表明，梯度极易受到模型反演攻击（PSNR: 52.26 dB, SSIM: 0.999, NMI: 0.741），几乎可以完美重建图像。相比之下，所提出的CLS保护HE方法能有效阻止此类攻击，每次聚合轮次仅需传输326 KB的加密数据。该框架在未加密领域实现了96.12%的全局分类准确率，在加密领域实现了90.02%的准确率。

Conclusion: 该隐私保护联邦学习框架通过结合ViT和同态加密，为多机构组织病理学分类提供了安全且高效的解决方案。它有效抵御了敏感医疗信息泄露的风险，同时保持了高分类准确性，并显著降低了通信开销。
...
### [61] [Inversion-Free Style Transfer with Dual Rectified Flows](https://arxiv.org/abs/2511.20986)
*Yingying Deng,Xiangyu He,Fan Tang,Weiming Dong,Xucheng Yin*

Main category: cs.CV

TL;DR: 本文提出一种基于双整流流的无反演风格迁移框架，通过并行预测内容和风格轨迹并动态融合，实现了高效且高质量的风格迁移，克服了传统扩散模型反演过程的计算成本和失真问题。


<details>
  <summary>Details</summary>
Motivation: 主流的基于扩散模型的风格迁移方法依赖计算密集型反演过程，且当反演不准确时会导致视觉失真，影响效率和图像质量。

Method: 本文提出一种新颖的“无反演”双整流流风格迁移框架，仅通过前向传播从内容和风格图像中找到未知的风格化分布。该方法并行预测内容和风格轨迹，并通过动态中点插值融合两者速度场，同时适应演变的风格化图像。通过联合建模内容、风格和风格化分布来设计速度场，并利用注意力注入引导风格整合。

Result: 该方法实现了鲁棒的融合，避免了简单叠加的缺点，提升了视觉保真度、内容保留能力和计算效率。实验证明其在不同风格和内容上具有良好的泛化能力，提供了一个有效且高效的风格迁移流程。

Conclusion: 所提出的无反演双整流流框架为风格迁移提供了一种有效且高效的解决方案，成功解决了现有扩散模型方法的局限性，实现了高质量和高效率的风格转换。
...
### [62] [GuardTrace-VL: Detecting Unsafe Multimodel Reasoning via Iterative Safety Supervision](https://arxiv.org/abs/2511.20994)
*Yuxiao Xiang,Junchi Chen,Zhenchao Jin,Changtao Miao,Haojie Yuan,Qi Chu,Tao Gong,Nenghai Yu*

Main category: cs.CV

TL;DR: 本文提出了GuardTrace-VL，一个视觉感知的安全审计器，用于监测多模态大推理模型（MLRMs）的完整推理过程（QTA），以检测中间推理阶段出现的有害内容，并通过新数据集和训练方案显著提升了不安全推理检测的F1分数。


<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型（MLRMs）在生成中间推理过程时，即使最终答案无害，也可能包含不安全内容（如偏见推理、违反政策的视觉上下文使用），而现有安全防护主要评估输入和最终答案，忽略了中间推理过程，导致潜在危害未被发现。

Method: 引入了GuardTrace-VL，一个通过联合图像-文本分析监控完整“问题-思考-回答”（QTA）流程的视觉感知安全审计器。构建了GuardTrace数据集，该数据集通过多样化提示策略生成，并经过MLRM和人工投票验证流程进行精炼。提出了一种结合数据精炼过程的三阶段渐进式训练方案，使模型能够学习细致且依赖上下文的安全偏好。

Result: 在包含域内和域外场景的测试集上，GuardTrace-VL模型在不安全推理检测任务中达到了93.1%的F1分数，比之前最强的多模态安全防御方法提高了13.5%的F1分数。

Conclusion: GuardTrace-VL通过监测多模态大推理模型的完整推理过程，能够有效检测中间阶段出现的有害内容，显著提升了多模态推理安全检测的性能，填补了现有安全防护的空白。
...
### [63] [RefOnce: Distilling References into a Prototype Memory for Referring Camouflaged Object Detection](https://arxiv.org/abs/2511.20989)
*Yu-Huan Wu,Zi-Xuan Zhu,Yan Wang,Liangli Zhen,Deng-Ping Fan*

Main category: cs.CV

TL;DR: 本文提出了一种新的参照伪装物体检测（Ref-COD）框架，通过在训练阶段将参照图像提炼成类别原型记忆，并在推理阶段根据查询条件合成参照向量，从而在测试时无需参照图像，解决了现有方法对参照图像的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 当前的参照伪装物体检测系统在测试时需要参照图像，这限制了部署能力，增加了延迟和数据收集负担。

Method: 该方法在训练期间将参照图像蒸馏到类别原型记忆中，并在推理时通过查询条件混合原型来合成参照向量。具体来说，它为每个类别维护一个通过EMA更新的原型，并从查询中预测混合权重以生成指导向量，从而在测试时无需任何参照。此外，提出了一种双向注意力对齐模块，以弥合参照统计数据和伪装查询特征之间的表示差距，同时调整查询特征和类别表示。

Result: 该方法在大型R2C7K基准测试上进行了评估，实验结果表明，与最新的SOTA方法相比，所提出的方法具有竞争力或更优的性能。

Conclusion: 该研究提供了一种简单、高效的参照伪装物体检测途径，在测试时无需强制性参照图像，有效解决了现有方法的部署和效率问题。
...
### [64] [Knowledge Completes the Vision: A Multimodal Entity-aware Retrieval-Augmented Generation Framework for News Image Captioning](https://arxiv.org/abs/2511.21002)
*Xiaoxing You,Qiang Huang,Lingyu Li,Chi Zhang,Xiaopeng Liu,Min Zhang,Jun Yu*

Main category: cs.CV

TL;DR: 本文提出了MERGE，一个多模态实体感知检索增强生成框架，用于新闻图像字幕生成，通过构建实体中心多模态知识库、多阶段假设-字幕策略和动态检索，解决了信息覆盖不全、跨模态对齐弱和视觉实体接地不足的问题，并在多个数据集上显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有新闻图像字幕生成方法面临三个关键挑战：1) 信息覆盖不完整，2) 跨模态对齐能力弱，3) 视觉实体接地次优。这些问题促使研究者开发一种能更有效结合视觉内容和上下文线索以生成新闻性描述的方法。

Method: 本文引入了MERGE框架，它是首个多模态实体感知检索增强生成框架。MERGE构建了一个实体中心多模态知识库（EMKB），整合了文本、视觉和结构化知识以实现丰富的背景检索。它通过多阶段假设-字幕策略改进了跨模态对齐，并通过图像内容引导的动态检索增强了视觉实体匹配。

Result: 在GoodNews和NYTimes800k数据集上的实验表明，MERGE在字幕质量方面显著优于现有基线，CIDEr得分分别提升了+6.84和+1.16，在命名实体识别方面F1-score分别提升了+4.14和+2.64。此外，MERGE在未见过的Visual News数据集上也表现出良好的泛化能力，CIDEr得分提升了+20.17，F1-score提升了+6.22。

Conclusion: MERGE框架显著提升了新闻图像字幕生成的性能，并在多个数据集上超越了最先进的基线。其强大的泛化能力和领域适应性证明了该方法在解决新闻图像字幕生成关键挑战方面的有效性和鲁棒性。
...
### [65] [Wavefront-Constrained Passive Obscured Object Detection](https://arxiv.org/abs/2511.20991)
*Zhiwen Zheng,Yiwei Ouyang,Zhao Huang,Tao Zhang,Xiaoshuai Zhang,Huiyu Zhou,Wenwen Tang,Shaowei Jiang,Jin Liu,Xingru Huang*

Main category: cs.CV

TL;DR: 本文提出了一种物理驱动的波前传播补偿网络（WavePCNet），用于从微弱光模式中定位和分割被遮挡的物体，解决了现有方法在多重散射和低信噪比条件下的局限性。


<details>
  <summary>Details</summary>
Motivation: 由于多重散射和介质扰动，从视场外微弱光模式中准确地定位和分割被遮挡物体极具挑战性。现有方法（基于实值建模或局部卷积）不足以捕捉相干光传播的底层物理，且在低信噪比条件下常收敛到非物理解决方案，严重影响观察的稳定性和可靠性。

Method: 本文提出WavePCNet，通过模拟波前传播来增强对被遮挡物体的感知。它集成了：1) 三相波前复传播重投影（TriWCP），以结合复振幅传递算子精确约束相干传播行为；2) 动量记忆机制，有效抑制扰动累积；3) 高频跨层补偿增强，构建具有多尺度感受野的频率选择性通路，并动态建模跨层的结构一致性。

Result: 在四个物理收集的数据集上进行的广泛实验表明，WavePCNet在准确性和鲁棒性方面均持续优于现有最先进的方法。

Conclusion: WavePCNet通过结合物理驱动的复杂传播模型和创新的网络机制，有效解决了被遮挡物体定位和分割的难题，显著提升了在复杂环境条件下的感知能力、鲁棒性和可解释性。
...
### [66] [Structure-Aware Prototype Guided Trusted Multi-View Classification](https://arxiv.org/abs/2511.21021)
*Haojian Huang,Jiahao Shi,Zhe Liu,Harold Haodong Chen,Han Fang,Hao Sun,Zhongjiang He*

Main category: cs.CV

TL;DR: 本文提出一种基于原型的多视角信任分类（TMVC）框架，通过简化视角内邻居关系学习并动态对齐视角内外部结构，实现高效且一致的跨视角共识发现，提高分类的信任度。


<details>
  <summary>Details</summary>
Motivation: 现有TMVC方法存在以下问题：1) 依赖全局密集邻居关系建模视角内依赖，导致计算成本高；2) 无法直接确保视角间关系的一致性；3) 通过手动分配权重聚合不同视角证据，缺乏类空间内多视角邻居结构一致性的保证，从而损害分类结果的信任度。

Method: 提出一种新颖的TMVC框架，引入原型来表示每个视角的邻居结构。这种方法简化了视角内邻居关系的学习，并实现了视角内和视角间结构的动态对齐，从而促进了更高效和一致的跨视角共识发现。

Result: 在多个公共多视角数据集上进行的广泛实验表明，该方法与流行的TMVC方法相比，实现了具有竞争力的下游性能和鲁棒性。

Conclusion: 所提出的基于原型的TMVC框架有效克服了现有方法的局限性，通过简化邻居关系学习和动态结构对齐，实现了更高效、一致且可信赖的多视角分类结果。
...
### [67] [From Inpainting to Layer Decomposition: Repurposing Generative Inpainting Models for Image Layer Decomposition](https://arxiv.org/abs/2511.20996)
*Jingxi Chen,Yixiao Zhang,Xiaoye Qian,Zongxia Li,Cornelia Fermuller,Caren Chen,Yiannis Aloimonos*

Main category: cs.CV

TL;DR: 该论文提出了一种通过轻量级微调将扩散模型适应于图像层分解的方法，利用多模态上下文融合模块和合成数据集，在物体移除和遮挡恢复方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 将图像分解为分层表示（前景物体在背景之上，可能存在遮挡）对于独立编辑元素和内容创作具有重要意义。然而，由于方法和数据的限制，大型生成模型在将单张图像分解为层方面仍然面临挑战。

Method: 研究人员观察到层分解与修补任务（in/outpainting）之间的强关联，并提出通过轻量级微调来适应基于扩散的修补模型以进行层分解。为在潜在空间中进一步保留细节，引入了一种具有线性注意力复杂度的多模态上下文融合模块。模型完全使用由开源资产构建的合成数据集进行训练。

Result: 该模型在物体移除和遮挡恢复方面取得了卓越的性能。

Conclusion: 该方法为下游编辑和创意应用开启了新的可能性，因为它能够实现更有效的图像层分解和处理。
...
### [68] [Pygmalion Effect in Vision: Image-to-Clay Translation for Reflective Geometry Reconstruction](https://arxiv.org/abs/2511.21098)
*Gayoung Lee,Junho Kim,Jin-Hwa Kim,Junmo Kim*

Main category: cs.CV

TL;DR: 该论文提出了“视觉皮格马利翁效应”框架，通过将反射物体图像转换为“粘土状”形式，去除高光反射，从而实现对复杂反射物体的鲁棒3D重建。


<details>
  <summary>Details</summary>
Motivation: 由于视图依赖的反射导致外观和几何形状的纠缠，理解反射一直是3D重建中的一个长期挑战。

Method: 该方法通过图像到粘土的转换，学习抑制镜面反射线索，同时保留固有的几何一致性。具体而言，引入了一个双分支网络：一个基于BRDF的反射分支和一个粘土引导分支，后者稳定几何形状并细化表面法线。两个分支使用合成的粘土状图像进行联合训练，这些图像提供了中性、无反射的监督信号。

Result: 在合成和真实数据集上的实验表明，该方法在法线精度和网格完整性方面显著优于现有处理反射的方法。

Conclusion: 该框架表明，“通过去光泽观察”（将辐射转换为中性）可以作为反射物体几何学习的强大归纳偏置。
...
### [69] [MetaRank: Task-Aware Metric Selection for Model Transferability Estimation](https://arxiv.org/abs/2511.21007)
*Yuhang Liu,Wenjie Zhao,Yunhui Guo*

Main category: cs.CV

TL;DR: MetaRank是一个元学习框架，通过将数据集和MTE指标的文本描述嵌入共享语义空间，自动、任务感知地选择最合适的MTE指标，以解决MTE指标选择的任务依赖性问题。


<details>
  <summary>Details</summary>
Motivation: 在迁移学习中，选择合适的预训练源模型至关重要但计算昂贵。MTE方法通过代理指标解决此问题，但MTE指标的选择通常是临时的或基于历史平均表现。研究发现MTE指标的有效性高度依赖于任务，没有单一指标是普适最优的，因此需要一种更智能、任务感知的选择方法。

Method: 本文提出了MetaRank，一个元学习框架，将MTE指标选择表述为排序学习问题。它不依赖传统元特征，而是使用预训练语言模型编码数据集和MTE指标的文本描述，将它们嵌入共享语义空间。一个元预测器在离线阶段通过多样化的元任务进行训练，使用列表式目标优化，优先正确排序表现最佳的指标。在线阶段，MetaRank根据新目标数据集的文本描述，高效地对候选MTE指标进行排序。

Result: 在11个预训练模型和11个目标数据集上进行的广泛实验证明了MetaRank方法的强大有效性。

Conclusion: MetaRank通过其元学习框架和基于文本描述的语义嵌入，能够为新的、未见过的目标数据集先验地选择最合适的MTE指标，从而帮助实践者克服MTE指标选择的任务依赖性挑战。
...
### [70] [Efficient Training for Human Video Generation with Entropy-Guided Prioritized Progressive Learning](https://arxiv.org/abs/2511.21136)
*Changlin Li,Jiawei Zhang,Shuhao Liu,Sihao Lin,Zeyi Shi,Zhihui Li,Xiaojun Chang*

Main category: cs.CV

TL;DR: 本文提出Ent-Prog，一个高效的训练框架，通过熵引导的优先渐进学习，显著降低扩散模型在人体视频生成任务中的训练时间和GPU内存消耗，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在人体视频生成方面取得了快速进展，但其在高分辨率、多帧数据上的训练面临计算成本高和内存消耗大的挑战。

Method: Ent-Prog框架包含两部分：1) 条件熵膨胀（CEI），用于评估模型组件在目标条件生成任务中的重要性，从而实现对关键组件的优先训练；2) 自适应渐进调度，通过衡量收敛效率来自适应地增加训练期间的计算复杂度。

Result: Ent-Prog在三个数据集上进行了广泛实验，结果表明其可实现高达2.2倍的训练加速和2.4倍的GPU内存减少，且不损害生成性能。

Conclusion: Ent-Prog是一个高效的训练框架，专为人体视频生成扩散模型设计，能有效降低训练时间和GPU内存消耗，同时保持模型性能。
...
### [71] [Which Layer Causes Distribution Deviation? Entropy-Guided Adaptive Pruning for Diffusion and Flow Models](https://arxiv.org/abs/2511.21122)
*Changlin Li,Jiawei Zhang,Zeyi Shi,Zongxin Yang,Zhihui Li,Xiaojun Chang*

Main category: cs.CV

TL;DR: EntPruner是一种熵引导的自动渐进式剪枝框架，专门用于扩散模型和流模型，以减少大规模视觉生成模型在下游任务中的参数冗余，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉生成模型（如扩散模型和流模型）在视觉生成任务中表现出色，但将其预训练模型迁移到下游任务时，常常导致显著的参数冗余，影响效率。

Method: 本文提出了EntPruner框架。首先，引入了熵引导剪枝，这是一种专门为生成模型设计的块级重要性评估策略，通过数据依赖的条件熵偏差（CED）来量化移除块后分布与学习到的条件数据分布的偏离程度。其次，提出了零样本自适应剪枝框架，在训练过程中自动决定何时以及剪枝多少，避免了一次性剪枝的弊端，减轻了模式崩溃并保持了模型性能。

Result: 在DiT和SiT模型上进行的广泛实验表明，EntPruner的有效性，在ImageNet和三个下游数据集上实现了高达2.22倍的推理加速，同时保持了具有竞争力的生成质量。

Conclusion: EntPruner成功解决了大型生成模型在下游任务中的参数冗余问题，通过其熵引导和自适应剪枝策略，显著提高了推理速度，同时有效保持了生成模型的性能和输出质量。
...
### [72] [FlowerDance: MeanFlow for Efficient and Refined 3D Dance Generation](https://arxiv.org/abs/2511.21029)
*Kaixing Yang,Xulong Tang,Ziqiao Peng,Xiangyue Zhang,Puwei Wang,Jun He,Hongyan Liu*

Main category: cs.CV

TL;DR: FlowerDance是一种高效的音乐到舞蹈生成方法，它结合了物理合理性和艺术表现力，实现了高质量的舞蹈生成，同时显著提高了推理速度和内存利用率，并支持动作编辑。


<details>
  <summary>Details</summary>
Motivation: 现有音乐到舞蹈生成方法的生成效率有限，导致在进行高保真3D渲染时计算开销不足，从而限制了3D角色在实际应用中的表现力。

Method: FlowerDance结合了MeanFlow与物理一致性约束，仅需少量采样步骤即可生成高质量动作。其模型架构简单高效，采用基于BiMamba的骨干网络和通道级跨模态融合，以高效的非自回归方式生成舞蹈。此外，它还支持动作编辑功能。

Result: 在AIST++和FineDance数据集上的大量实验表明，FlowerDance在动作质量和生成效率方面均达到了最先进的水平。

Conclusion: FlowerDance成功地解决了音乐到舞蹈生成中效率和质量的挑战，能够生成精细、物理合理且富有艺术表现力的动作，并支持用户交互式编辑，为虚拟现实、编舞和数字娱乐提供了广阔的应用前景。
...
### [73] [CaptionQA: Is Your Caption as Useful as the Image Itself?](https://arxiv.org/abs/2511.21025)
*Shijia Yang,Yunong Liu,Bohan Zhai,Ximeng Sun,Zicheng Liu,Emad Barsoum,Manling Li,Chenfeng Xu*

Main category: cs.CV

TL;DR: 本文提出CaptionQA，一个基于效用的基准测试，用于评估模型生成的图像字幕在下游任务中的实际可用性，并发现现有字幕与图像效用之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 图像字幕在多模态系统中作为视觉内容的有效替代品，但当前的评估方法未能解决一个基本问题：字幕能否在真实的下游任务中真正替代图像？

Method: CaptionQA是一个可扩展的、领域相关的基准测试，涵盖自然、文档、电子商务和具身AI四个领域，并包含细粒度的分类法（25个顶级类别和69个子类别）。它构建了33,027个密集标注的多项选择题（平均每张图像50.3个问题），这些问题明确要求视觉信息才能回答。评估协议是让大型语言模型（LLM）仅使用字幕来回答这些问题，直接衡量字幕是否保留了图像级别的效用并可供下游LLM利用。

Result: 评估最先进的多模态大型语言模型（MLLMs）显示，图像及其字幕的效用之间存在巨大差距。值得注意的是，在传统图像问答基准上表现几乎相同的模型，其字幕效用下降了高达32%。

Conclusion: 当前模型生成的图像字幕在支持下游任务方面存在显著不足，传统评估方法未能充分反映其在实际应用中的效用。CaptionQA提供了一个更全面的方法来评估字幕质量，并揭示了需要改进的领域。
...
### [74] [CameraMaster: Unified Camera Semantic-Parameter Control for Photography Retouching](https://arxiv.org/abs/2511.21024)
*Qirui Yang,Yang Yang,Ying Zeng,Xiaobin Hu,Bo Li,Huanjing Yue,Jingyu Yang,Peng-Tao Jiang*

Main category: cs.CV

TL;DR: CameraMaster是一个统一的相机感知框架，通过显式解耦相机指令并整合指令表示与精确相机参数嵌入，实现了对图像进行物理一致且参数精确（如曝光、白平衡、缩放）的修饰。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导扩散模型在实现物理一致的图像修饰和精确参数控制（如曝光、白平衡、缩放）方面面临挑战。它们要么依赖模糊且纠缠的文本提示，难以实现精确相机控制；要么为参数调整训练独立的头部/权重，牺牲了可扩展性、多参数组合能力以及对细微变化的敏感性。

Method: CameraMaster通过显式解耦相机指令，并将摄影师意图的指令表示和精确相机设置的参数嵌入进行连贯整合。它首先使用相机参数嵌入来调制相机指令和内容语义。然后，将调制后的指令通过交叉注意力注入到内容特征中，产生相机敏感的语义上下文。此外，指令和相机嵌入作为条件和门控信号注入到时间嵌入中，实现去噪过程中统一、逐层的调制，并强制执行紧密的语义-参数对齐。为训练和评估，构建了一个包含7.8万张图像-提示对且带有相机参数标注的大规模数据集。

Result: CameraMaster对参数变化产生了单调且接近线性的响应，支持无缝的多参数组合，并显著优于现有方法。

Conclusion: CameraMaster通过其相机感知框架和参数解耦与整合机制，有效解决了扩散模型在实现物理一致且精确参数控制的图像修饰方面的局限性，实现了卓越的性能和控制精度。
...
### [75] [LLaVA-UHD v3: Progressive Visual Compression for Efficient Native-Resolution Encoding in MLLMs](https://arxiv.org/abs/2511.21150)
*Shichu Sun,Yichen Zhang,Haolin Song,Zonghao Guo,Chi Chen,Yidan Zhang,Yuan Yao,Zhiyuan Liu,Maosong Sun*

Main category: cs.CV

TL;DR: 多模态大语言模型（MLLMs）中的全局原生分辨率视觉编码虽然增强了性能，但计算开销大。本文提出了渐进式视觉压缩（PVC）方法，并将其集成到LLaVA-UHD v3中，显著提高了效率（TTFT降低2.4倍/1.9倍），同时保持了竞争力。


<details>
  <summary>Details</summary>
Motivation: 全局原生分辨率视觉编码在多模态大语言模型（MLLMs）中越来越受欢迎，因为它能提升整体能力，但同时也带来了巨大的计算开销。研究旨在解决这一效率问题。

Method: 本文提出了渐进式视觉压缩（PVC）方法，可无缝集成到标准Vision Transformer (ViT) 中，以实现高效的原生分辨率编码。PVC包含两个关键模块：(i) 精细化补丁嵌入，支持灵活的补丁大小缩放以进行细粒度视觉建模；(ii) 窗口化令牌压缩，分层部署在ViT层中，逐步聚合局部令牌表示。这两个模块共同作用，将预训练的ViT重新配置成一个高效的架构（ViT-UHD）。

Result: 在相同的MLLM架构下，基于PVC转换而来的ViT-UHD在与MoonViT竞争的同时，将TTFT（首次生成令牌时间）减少了2.4倍。在此基础上，LLaVA-UHD v3在与Qwen2-VL竞争的同时，进一步将TTFT减少了1.9倍。

Conclusion: 渐进式视觉压缩（PVC）方法有效地解决了多模态大语言模型中全局原生分辨率视觉编码的效率问题，通过显著降低TTFT，在保持竞争性性能的同时，实现了更高效的模型架构。研究将发布所有代码和检查点以支持未来的研究。
...
### [76] [LungNoduleAgent: A Collaborative Multi-Agent System for Precision Diagnosis of Lung Nodules](https://arxiv.org/abs/2511.21042)
*Cheng Yang,Hui Jin,Xinlei Yu,Zhipeng Wang,Yaoqun Liu,Fenglei Fan,Dajiang Lei,Gangyong Jia,Changmiao Wang,Ruiquan Ge*

Main category: cs.CV

TL;DR: 本文提出LungNoduleAgent，一个创新的协作多智能体系统，用于肺部CT扫描分析和肺结节诊断，通过模块化流程和多智能体协作显著提升了诊断的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在肺部CT扫描分析方面有所进展，但在准确描述结节形态和整合医学专业知识方面仍存在挑战，这影响了模型在临床环境中的可靠性。协作多智能体系统在医学应用中平衡通用性和精确性方面具有潜力，但在病理学领域尚未充分探索。

Method: 引入LungNoduleAgent，一个协作多智能体系统，将诊断过程分解为顺序组件。它包含三个主要模块：1) 结节识别器 (Nodule Spotter) 协调临床检测模型以准确识别结节；2) 放射科医生 (Radiologist) 集成局部图像描述技术以生成全面的CT报告；3) 医生智能体系统 (Doctor Agent System) 利用图像、CT报告、病理知识库和多智能体系统框架进行恶性肿瘤推理。

Result: 在两个私有数据集和公共LIDC-IDRI数据集上的广泛测试表明，LungNoduleAgent超越了主流的视觉-语言模型、智能体系统和先进的专家模型。

Conclusion: 这些结果强调了区域级语义对齐和多智能体协作在结节诊断中的重要性。LungNoduleAgent有望成为支持肺结节临床分析的基础工具。
...
### [77] [Deformation-aware Temporal Generation for Early Prediction of Alzheimers Disease](https://arxiv.org/abs/2511.21114)
*Xin Honga,Jie Lin,Minghui Wang*

Main category: cs.CV

TL;DR: 本文提出一种名为变形感知时间生成网络（DATGN）的新方法，通过自动化学习脑形态变化并生成符合阿尔茨海默病（AD）进展的未来MRI图像，实现AD的早期预测，并显著提高了分类准确性。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病（AD）的早期预测对延缓病情进展至关重要。当前AD预测方法主要依赖手动提取脑图像的形态学特征，效率低且可能不全面，无法自动化学习疾病进展过程中的形态变化。

Method: 本文提出变形感知时间生成网络（DATGN）。该方法首先对时间序列MRI图像中的缺失数据进行插值处理。随后，一个双向时间变形感知模块引导网络生成符合疾病进展趋势的未来MRI图像，从而促进AD的早期预测。

Result: 1. DATGN在ADNI数据集上生成未来MRI图像序列，在PSNR和MMSE图像质量指标上表现出竞争力。2. 将DATGN生成的合成数据整合到SVM、CNN和3DCNN分类方法中，AD vs. NC分类准确率从6.21%显著提高到16%，AD vs. MCI vs. NC分类准确率从7.34%显著提高到21.25%。3. 定性可视化结果表明，DATGN生成的MRI图像与阿尔茨海默病中的脑萎缩趋势一致。

Conclusion: DATGN通过自动化学习脑形态变化并生成符合疾病进展的未来MRI图像，有效实现了阿尔茨海默病的早期预测。其生成的合成数据能显著提升基于现有分类方法的诊断准确性，并能定性地反映疾病的脑萎缩趋势。
...
### [78] [PG-ControlNet: A Physics-Guided ControlNet for Generative Spatially Varying Image Deblurring](https://arxiv.org/abs/2511.21043)
*Hakki Motorcu,Mujdat Cetin*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的图像去模糊框架，通过将强大的生成先验与显式、密集的物理约束相结合，解决了空间可变图像去模糊中物理准确性和感知真实性之间的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有空间可变图像去模糊方法存在局限：基于模型的深度展开方法虽强制物理约束但易产生过度平滑和伪影；生成模型虽感知质量高但因物理约束弱而可能产生幻觉细节。在复杂运动模糊和噪声下，这是一个病态问题。

Method: 提出了一种独特地融合了模型基方法和生成模型的方法。它通过密集的高维压缩核连续体来建模退化场，以捕获微小的运动和其他退化模式变化。利用这个丰富的描述符场来条件化ControlNet架构，从而强力引导扩散采样过程。

Result: 实验证明，该方法有效弥合了物理准确性与感知真实性之间的差距，在具有挑战性的严重模糊场景中，性能优于最先进的基于模型的方法以及生成基线。

Conclusion: 该框架成功地将物理约束与生成能力相结合，为空间可变图像去模糊提供了一种有效解决方案，在准确性和真实感方面均取得了卓越成果。
...
### [79] [MUSE: Manipulating Unified Framework for Synthesizing Emotions in Images via Test-Time Optimization](https://arxiv.org/abs/2511.21051)
*Yingjie Xia,Xi Wang,Jinglei Shi,Vicky Kalogeiton,Jian Yang*

Main category: cs.CV

TL;DR: MUSE是首个统一的图像情感生成与编辑框架，通过借鉴Test-Time Scaling策略，利用现成的情感分类器进行梯度优化，并解决情感引导的时机与多情感干扰问题，在情感准确性和语义多样性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 图像情感对感知影响深远，但当前图像情感合成（IES）方法将生成和编辑任务人为分离，导致效率低下并限制了在治疗干预或故事叙述等自然融合场景中的应用。

Method: MUSE引入了一个统一框架，能够同时进行情感生成和编辑。它采用了一种与Test-Time Scaling (TTS) 概念一致的策略，避免了额外的扩散模型更新和专门情感合成数据集。具体而言，MUSE解决了三个关键问题：1) 如何通过利用现成的情感分类器和基于梯度的情感token优化来稳定引导合成；2) 何时通过语义相似性识别最佳情感引导时机；3) 通过多情感损失减少固有和相似情感干扰，来选择引导合成的情感。

Result: 实验结果表明，MUSE在生成和编辑两方面均优于所有现有方法，提高了情感准确性和语义多样性，同时在期望内容、文本提示依从性和逼真情感表达之间保持了最佳平衡。

Conclusion: MUSE为情感合成建立了一个新范式，证明了统一框架在图像情感生成和编辑方面的优越性。
...
### [80] [Progress by Pieces: Test-Time Scaling for Autoregressive Image Generation](https://arxiv.org/abs/2511.21185)
*Joonhyung Park,Hyeongwon Jang,Joowon Kim,Eunho Yang*

Main category: cs.CV

TL;DR: 针对视觉自回归模型的测试时缩放挑战，本文提出GridAR框架，通过网格分区渐进式生成和布局指定提示重构，在有限的计算资源下显著提升了图像生成质量和效率。


<details>
  <summary>Details</summary>
Motivation: 当前的视觉自回归（AR）模型在文本到图像生成方面表现出色，但测试时缩放策略（如Best-of-N）效率低下：它们在错误生成轨迹上浪费计算，且光栅扫描解码方案缺乏整体画布的蓝图，限制了缩放效益，因为只能生成少数与提示对齐的候选。

Method: GridAR采用以下方法：1. 网格分区渐进式生成方案：在画布内为同一位置生成多个局部候选，早期剪枝不可行的候选，并将可行的候选固定为锚点以指导后续解码。2. 布局指定提示重构策略：通过检查局部视图推断满足提示的可行布局，然后重构提示以指导后续图像生成，弥补蓝图不足。

Result: 在有限的测试时缩放条件下，GridAR取得了更高质量的结果。例如，在N=4时，它在T2I-CompBench++上比Best-of-N（N=8）性能高出14.4%，同时成本降低25.6%。它还泛化到自回归图像编辑，在PIE-Bench上显示出可比的编辑质量和比更大N基线高13.9%的语义保留。

Conclusion: GridAR成功解决了视觉自回归模型在测试时缩放的独特挑战，通过其创新的生成和提示重构策略，在文本到图像生成和图像编辑任务中，以更低的计算成本实现了更高的输出质量和语义一致性。
...
### [81] [When Robots Obey the Patch: Universal Transferable Patch Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.21192)
...
### [82] [BotaCLIP: Contrastive Learning for Botany-Aware Representation of Earth Observation Data](https://arxiv.org/abs/2511.21194)
*Selene Cerna,Sara Si-Moussi,Wilfried Thuiller,Hadrien Hendrikx,Vincent Miele*

Main category: cs.CV

TL;DR: 本文提出了BotaCLIP，一个轻量级多模态对比学习框架，旨在将预训练的地球观测基础模型（DOFA）适应于植物学领域，通过对高分辨率航空影像和植物群落调查数据进行对齐，以在低成本下注入领域知识，并在生物多样性建模任务中取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 基础模型在不同模态上表现出卓越的表示学习能力，但将其应用于特定领域（如生物多样性建模）时，需要注入领域特定知识，同时避免从头训练或产生高昂的计算成本，尤其是在数据稀缺的环境中。

Method: 引入了BotaCLIP，一个轻量级多模态对比学习框架。该框架通过对比学习和正则化策略（以减轻灾难性遗忘），将预训练的地球观测基础模型（DOFA）与高分辨率航空影像和植物群落调查数据对齐，从而使模型内部化生态结构。

Result: BotaCLIP生成的表示在植物存在预测、蝴蝶发生建模和土壤营养群丰度估计这三项生态任务中，始终优于DOFA和监督基线模型所派生的结果。

Conclusion: 这项工作表明，对基础模型进行领域感知适应可以有效地将专家知识注入到数据稀缺的环境中，从而实现“节俭”的表示学习。
...
### [83] [Long-Term Alzheimers Disease Prediction: A Novel Image Generation Method Using Temporal Parameter Estimation with Normal Inverse Gamma Distribution on Uneven Time Series](https://arxiv.org/abs/2511.21057)
*Xin Hong,Xinze Sun,Yinhao Li,Yen-Wei Chen*

Main category: cs.CV

TL;DR: 本研究提出T-NIG模型，通过估计正态逆伽马分布中的时间参数，解决阿尔茨海默病（AD）图像生成在不规则时间间隔下长期预测时难以保持疾病特征的问题，并有效降低不确定性，实现了最先进的预测性能。


<details>
  <summary>Details</summary>
Motivation: 图像生成在AD长期预测中，当面对不规则时间间隔的序列数据时，难以维持疾病相关特征。时间分布的方面可以反映疾病相关特征的变化，因此需要一种能处理不均匀图像分布的方法。

Method: 本研究提出了T-NIG模型，用于估计正态逆伽马分布（NIG）中的时间参数。该模型利用两个不同时间点的脑部图像来生成中间图像、预测未来图像并预测疾病。T-NIG通过坐标邻域识别特征，并将时间参数整合到NIG中，以理解在时间间隔不均的脑部成像序列中特征如何变化。此外，T-NIG还利用不确定性估计来减少由时间数据不足引起的认知不确定性和偶然不确定性。

Result: T-NIG模型在数据集的短期和长期预测任务中均展现了最先进的性能。实验结果表明，即使面对不规则的时间数据分布，T-NIG也能熟练地预测疾病进展，同时保持疾病相关的特征。

Conclusion: T-NIG模型能够有效处理不规则时间间隔的图像数据，成功地在AD长期预测中生成图像并保持疾病相关特征，为AD的早期诊断和疾病进展预测提供了新的有效工具。
...
### [84] [MIRA: Multimodal Iterative Reasoning Agent for Image Editing](https://arxiv.org/abs/2511.21087)
*Ziyun Zeng,Hang Hua,Jiebo Luo*

Main category: cs.CV

TL;DR: MIRA是一个轻量级、即插即用的多模态迭代推理代理，通过模拟多轮人机交互，逐步预测原子编辑指令，显著提高了指令引导图像编辑中复杂指令的语义一致性和感知质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的图像编辑模型难以准确解释复杂的指令，特别是涉及组合关系、上下文线索或指代表达的指令，导致编辑结果在语义上偏离或未能反映预期更改。

Method: MIRA（多模态迭代推理代理）采用迭代的感知-推理-行动循环，模拟多轮人机交互过程。它不发出单一提示或静态计划，而是利用视觉反馈逐步预测原子编辑指令。研究团队构建了150K的多模态工具使用数据集MIRA-Editing，并结合两阶段SFT + GRPO训练流程来训练MIRA。

Result: 当与Flux.1-Kontext、Step1X-Edit和Qwen-Image-Edit等开源图像编辑模型结合时，MIRA显著提高了语义一致性和感知质量。其性能可与GPT-Image和Nano-Banana等专有系统媲美或超越。

Conclusion: MIRA通过迭代的多模态推理代理有效解决了复杂指令在图像编辑中的挑战，显著提升了编辑质量，使其能够更好地理解和执行复杂的用户指令。
...
### [85] [CLRecogEye : Curriculum Learning towards exploiting convolution features for Dynamic Iris Recognition](https://arxiv.org/abs/2511.21097)
*Geetanjali Sharma,Gaurav Jaswal,Aditya Nigam,Raghavendra Ramachandra*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的虹膜认证流程，利用3D-CNN和课程学习（curriculum learning）来捕捉丰富的时空-时空-时间特征，以应对旋转、缩放、反射和散焦模糊等挑战，从而实现鲁越的虹膜认证。


<details>
  <summary>Details</summary>
Motivation: 现有的虹膜认证算法在旋转、缩放、镜面反射和散焦模糊等变异下鲁棒性不足；此外，大多数方法仅依赖简单的点对点比较（如余弦或L2距离），未能有效利用虹膜图案的时空-时空-时间结构。

Method: 该方法首先将每个虹膜图像沿一个维度分割成子图像序列，作为3D-CNN的输入，以捕获空间和时空-时空-时间线索。为增强时空-时空-时间特征动态建模，模型采用课程学习方式训练。整个框架通过triplet和ArcFace损失进行端到端训练。

Result: 该设计能够直接将时间依赖性嵌入到特征空间中，提高了深度度量域的判别能力。它在旋转、缩放、反射和模糊等挑战下仍能强制生成高判别力的嵌入，从而提供了一个鲁棒且可泛化的虹膜认证解决方案。

Conclusion: 通过提出一种新颖且广义的匹配流程，利用3D-CNN捕获时空-时空-时间特征，并结合课程学习和triplet/ArcFace损失进行端到端训练，本研究成功解决了现有虹膜认证算法在各种挑战下的鲁棒性问题，实现了高度判别性和可泛化的虹膜认证。
...
TL;DR: 本文提出Contrastive Fusion (ConFu) 框架，通过扩展对比学习目标，将单模态和融合模态联合嵌入统一空间，在捕获高阶依赖的同时保持强烈的两两对应关系，从而有效解决多模态学习中的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前多模态学习方法主要处理两两模态对齐，或在捕获高阶交互时忽视或不充分保留两两关系，导致在单模态任务上表现受限。因此，需要一种能同时处理高阶依赖和保持两两对应关系的方法。

Method: ConFu框架将单个模态及其融合组合共同嵌入到统一的表示空间中，使模态及其融合对应物对齐。它通过引入一个额外的“融合模态对比项”来扩展传统的两两对比目标，鼓励模态对与第三个模态进行联合嵌入，从而捕获XOR等高阶依赖关系，同时保持强大的两两对应。

Result: ConFu在合成和真实世界多模态基准测试中，在检索和分类任务上表现出有竞争力的性能。它能够有效利用跨模态互补性，捕获高阶依赖关系，并随多模态复杂性增加而扩展，同时在一个对比框架内支持统一的一对一和二对一检索。

Conclusion: ConFu通过其创新的对比融合机制，成功地在捕获多模态数据中的高阶依赖关系和保持强大的两两对应之间取得了平衡，为多模态学习提供了一个有效且全面的解决方案。
...
TL;DR: 本文提出了SurgMLLMBench，一个统一的多模态基准，旨在开发和评估用于手术场景理解的交互式多模态大语言模型，包括新收集的MAVIS数据集，并支持像素级分割和结构化VQA。


<details>
  <summary>Details</summary>
Motivation: 现有的外科数据集主要采用VQA格式，分类法异构且缺乏像素级分割支持，这限制了对多模态大语言模型的一致评估和适用性，尤其是在医疗和外科应用潜力日益增长的背景下。

Method: 研究者创建了SurgMLLMBench，一个统一的多模态基准。它整合了像素级器械分割掩码和结构化VQA标注，涵盖腹腔镜、机器人辅助和显微外科领域，并采用统一的分类法，以实现超越传统VQA任务的全面评估和更丰富的视觉-对话交互。其中包括新收集的MAVIS数据集。

Result: 广泛的基线实验表明，在SurgMLLMBench上训练的单一模型在不同领域实现了持续的性能，并能有效地泛化到未见过的数据集。

Conclusion: SurgMLLMBench将作为一个强大的公共资源发布，以推动多模态外科人工智能研究，支持交互式外科推理模型的可复现评估和开发。
...
### [88] [Hybrid SIFT-SNN for Efficient Anomaly Detection of Traffic Flow-Control Infrastructure](https://arxiv.org/abs/2511.21337)
*Munish Rathee,Boris Bačić,Maryam Doborjeh*

Main category: cs.CV

TL;DR: 本文提出了SIFT-SNN框架，一个低延迟的神经形态信号处理管道，用于实时检测交通基础设施的结构异常，实现了高准确率和低功耗的边缘部署。


<details>
  <summary>Details</summary>
Motivation: 研究动机是需要一种低延迟、低功耗、可解释的实时系统，用于交通基础设施的结构异常检测，特别是在边缘设备上高效运行。

Method: 该方法整合了尺度不变特征变换（SIFT）进行空间特征编码，一个延迟驱动的脉冲转换层，以及一个漏积分-发射（LIF）脉冲神经网络（SNN）进行分类。研究使用了奥克兰海港大桥数据集，包含6,000帧，包括真实和合成增强的不安全情况。

Result: 该系统实现了92.3%（±0.8%）的分类准确率，每帧推理时间为9.5毫秒。亚10毫秒的延迟结合8.1%的稀疏脉冲活动，支持实时、低功耗的边缘部署。与传统CNN方法不同，SIFT-SNN管道明确保留了空间特征定位，增强了可解释性，并能高效地在嵌入式硬件上运行。

Conclusion: SIFT-SNN框架为交通基础设施（如可移动混凝土护栏）的结构安全监测提供了一个通用且有效的解决方案，具有低延迟、低功耗和高可解释性。尽管合成增强提高了鲁棒性，但其对未见过的现场条件的泛化能力仍需进一步验证。
...
### [89] [Do Reasoning Vision-Language Models Inversely Scale in Test-Time Compute? A Distractor-centric Empirical Analysis](https://arxiv.org/abs/2511.21397)
*Jiyun Bae,Hyunjong Ok,Sangwoo Mo,Jaeho Lee*

Main category: cs.CV

TL;DR: 研究发现，视觉干扰物在视觉-语言模型中导致逆向扩展效应，降低准确性但不会增加推理长度，这与文本干扰物不同。通过分析属性计数可以深入理解其相互作用，并提出了一种缓解偏差的提示策略。


<details>
  <summary>Details</summary>
Motivation: 先前的研究发现文本干扰物在语言模型中会导致逆向扩展效应（推理更长但效果更差）。本研究旨在探究在多模态（视觉-语言）设置中，视觉干扰物是否也会产生类似现象。

Method: 引入了名为Idis的视觉问答数据集，该数据集系统地改变了语义、数值和空间维度的干扰物。分析了推理轨迹中的属性计数，并扩展到Waterbirds等视觉偏差基准测试，最后提出了一种简单的提示策略来缓解推理模型中的偏差驱动预测。

Result: 视觉干扰物与文本干扰物存在根本性差异：尽管逆向扩展效应依然存在，但添加视觉干扰物会降低准确性，而不会增加推理长度。跟踪推理轨迹中的属性计数为干扰物、推理长度和准确性如何相互作用提供了关键见解。这些趋势也延伸到了Waterbirds等已建立的视觉偏差基准。

Conclusion: 视觉干扰物在视觉-语言模型中的影响与文本干扰物不同，它会导致逆向扩展效应并降低准确性，但不会延长推理时间。通过分析属性计数可以更好地理解这种相互作用。同时，提出了一种简单的提示策略，可以有效缓解推理模型中由偏差引起的预测。
...
### [90] [AnchorOPT: Towards Optimizing Dynamic Anchors for Adaptive Prompt Learning](https://arxiv.org/abs/2511.21188)
*Zheng Li,Yibing Song,Xin Zhang,Lei Luo,Xiang Li,Jian Yang*

Main category: cs.CV

TL;DR: 现有基于CLIP的提示学习方法中，文本锚点是静态的，缺乏灵活性。AnchorOPT提出了一种动态锚点框架，通过学习任务特定的锚点值和自适应的位置关系，显著提升了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的提示学习方法使用静态的文本锚点来指导可学习的软标记，这些锚点在值和位置上都是固定的，缺乏跨任务和阶段的适应性，限制了CLIP的泛化能力。

Method: AnchorOPT框架引入了动态锚点：(i) 锚点值不再是手工设定的文本标记，而是从任务特定数据中动态学习；(ii) 锚点和软标记之间的位置关系不再固定，而是通过一个可学习的位置矩阵，根据训练阶段和任务上下文自适应优化。训练分为两阶段：首先学习锚点标记，然后冻结并将其转移到第二阶段，以优化软标记和位置矩阵。

Result: 仅使用一个简单的可学习锚点和位置矩阵，AnchorOPT就达到了与一些包含额外可学习模块或正则化技术的方法相当或超越的性能。作为一个即插即用的模块，AnchorOPT能无缝集成到现有框架中，并在不同数据集中持续带来性能提升。

Conclusion: AnchorOPT通过引入动态学习的锚点值和自适应的位置关系，有效解决了现有提示学习方法中静态锚点缺乏灵活性的问题。其简单而有效的机制带来了显著的性能提升和更好的泛化能力，并能作为通用模块集成到现有框架中。
...
### [91] [Scaling Foundation Models for Radar Scene Understanding](https://arxiv.org/abs/2511.21105)
*Pushkal Mishra,Kshitiz Bansal,Dinesh Bharadia*

Main category: cs.CV

TL;DR: 本文提出了RadarFM，一个雷达基础模型，通过结构化空间语言监督学习统一的场景级表示。它引入了结构化字幕框架和哈希感知对比学习目标，并在大规模CARLA雷达数据集上进行了验证，同时提出了定位感知评估指标。


<details>
  <summary>Details</summary>
Motivation: 雷达传感器在恶劣天气、光照和远距离条件下表现可靠，但现有雷达方法分散且针对特定任务，难以实现跨任务迁移。尽管基础模型在视觉和语言理解方面取得了显著进展，但它们与雷达传感的结合仍未被充分探索。

Method: 本文提出了RadarFM，一个通过结构化空间语言监督学习统一场景级表示的雷达基础模型。具体方法包括：1) 一个结构化字幕框架，用于在原生雷达坐标系中编码车辆分布；2) 一个哈希感知对比学习目标，用于量化连续场景相似性而非二元匹配，从而实现细粒度空间推理；3) 利用CARLA模拟器生成大规模、带注释的雷达数据集；4) 提出了定位感知指标来评估空间精度。

Result: 本文介绍了RadarFM模型及其核心组件：结构化字幕框架和哈希感知对比学习目标，旨在学习统一的场景级雷达表示。同时，通过CARLA模拟器生成了大规模雷达数据集，并提出了超越传统检测指标的定位感知度量来评估空间准确性。

Conclusion: 本文通过引入RadarFM及其创新的结构化空间语言监督和哈希感知对比学习方法，旨在解决现有雷达方法的碎片化问题，实现统一的雷达场景表示和细粒度空间推理，为雷达传感领域的基础模型发展奠定基础。
...
### [92] [EM-KD: Distilling Efficient Multimodal Large Language Model with Unbalanced Vision Tokens](https://arxiv.org/abs/2511.21106)
*Ze Feng,Sen Yang,Boqiang Duan,Wankou Yang,Jingdong Wang*

Main category: cs.CV

TL;DR: 该论文提出EM-KD，一种新的知识蒸馏范式，通过引入视觉令牌空间对齐和两种蒸馏策略（VLAD和VSD），有效提升了高效多模态大语言模型（MLLMs）的理解能力和效率，解决了视觉令牌不平衡导致的信息损失问题。


<details>
  <summary>Details</summary>
Motivation: 高效多模态大语言模型通过压缩视觉令牌来减少资源消耗，但这会导致视觉信息丢失，从而降低理解能力。现有的知识蒸馏方法未能解决高效学生模型和普通教师模型之间因视觉令牌不平衡而导致的细粒度视觉理解差异。

Method: EM-KD方法首先通过计算教师和学生模型视觉logits之间的曼哈顿距离，并利用匈牙利匹配算法在空间维度上进行对齐，以解决视觉令牌不平衡问题。对齐后，EM-KD引入两种蒸馏策略：1) 视觉-语言亲和力蒸馏（VLAD），通过最小化文本令牌与对齐视觉令牌之间亲和力矩阵的平滑L1距离；2) 视觉语义蒸馏（VSD），通过反向KL散度衡量对齐视觉logits在词汇空间上的离散概率分布。

Result: 在多项基准测试中，EM-KD训练的模型在准确性和效率方面均显著优于现有高效MLLMs。与配备了所提出视觉令牌匹配策略的先前蒸馏方法相比，EM-KD也取得了更好的性能。

Conclusion: EM-KD通过创新的视觉令牌空间对齐和两种蒸馏策略（VLAD和VSD），成功克服了高效MLLMs中视觉令牌不平衡带来的挑战，显著提升了模型的理解能力和效率，验证了其有效性。
...
### [93] [Monet: Reasoning in Latent Visual Space Beyond Images and Language](https://arxiv.org/abs/2511.21395)
*Qixun Wang,Yang Shi,Yifei Wang,Yuanxing Zhang,Pengfei Wan,Kun Gai,Xianghua Ying,Yisen Wang*

Main category: cs.CV

TL;DR: Monet是一个训练框架，使多模态大语言模型（MLLMs）能够通过生成连续嵌入（作为中间视觉思维）直接在潜在视觉空间中进行推理。该框架通过三阶段蒸馏SFT和新颖的VLPO方法解决了训练挑战，并在各种视觉推理任务上取得了显著的性能提升和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的“图像思维”方法在抽象视觉思维方面不如人类，其灵活性受限于外部工具。研究旨在让MLLMs能够像人类一样直接在潜在视觉空间中进行更灵活的抽象视觉推理。

Method: 引入Monet训练框架，使MLLMs能够通过生成连续嵌入（作为中间视觉思维）直接在潜在视觉空间中进行推理。针对潜在视觉推理中的高计算成本和监督不足，提出了一个三阶段的基于蒸馏的监督微调（SFT）流程。为克服GRPO在潜在推理上的局限性，提出了VLPO（Visual-latent Policy Optimization），一种将潜在嵌入明确纳入策略梯度更新的强化学习方法。构建了Monet-SFT-125K，一个包含125K高质量文本-图像交错思维链的数据集。

Result: Monet-7B模型在真实世界感知和推理基准测试中表现出持续的性能提升，并在具有挑战性的抽象视觉推理任务上展示了强大的域外泛化能力。对每个训练组件的作用进行了实证分析，并讨论了早期失败的尝试，为未来视觉潜在推理的发展提供了见解。

Conclusion: Monet框架成功地使MLLMs能够通过直接在潜在视觉空间中进行推理，实现更接近人类的抽象视觉思维。该方法在多个基准测试中表现出色，并具有强大的泛化能力，为视觉潜在推理领域未来的发展提供了重要的方向和见解。
...
### [94] [SAM Guided Semantic and Motion Changed Region Mining for Remote Sensing Change Captioning](https://arxiv.org/abs/2511.21420)
*Futian Wang,Mengqi Wang,Xiao Wang,Haowen Wang,Jin Tang*

Main category: cs.CV

TL;DR: 本文提出一种利用SAM基础模型增强遥感图像变化描述的方法，通过整合区域级表示和知识图谱信息，解决了现有方法区域感知弱和时间对齐有限的问题，并取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感变化描述方法通常采用CNN/Transformer提取视觉表示或引入辅助任务，但存在区域感知能力弱和时间对齐有限的问题。

Method: 该方法首先使用CNN/Transformer模型提取全局视觉特征，然后利用SAM基础模型描绘语义和运动层面的变化区域，并构建知识图谱提供感兴趣对象的信息。这些异构信息源通过交叉注意力机制融合，最后由Transformer解码器生成自然语言描述。

Result: 广泛的实验结果表明，该方法在多个广泛使用的基准数据集上取得了最先进的性能。

Conclusion: 本文提出的方法通过引入SAM模型和知识图谱，有效增强了遥感变化描述的区域感知和信息整合能力，显著提升了模型性能，达到了领域内的最新水平。
...
### [95] [FaithFusion: Harmonizing Reconstruction and Generation via Pixel-wise Information Gain](https://arxiv.org/abs/2511.21113)
*YuAn Wang,Xiaofan Li,Chi Huang,Wenhao Zhang,Hao Li,Bosheng Wang,Xun Sun,Jun Wang*

Main category: cs.CV

TL;DR: 本文提出FaithFusion框架，通过像素级预期信息增益（EIG）驱动3DGS与扩散模型的融合，解决了可控驾驶场景重建中几何保真度与视觉真实感难以兼顾的问题，尤其是在大视角变化下。


<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，保持几何保真度并在大视角变化下合成视觉上合理的图像至关重要。然而，有效的几何驱动3DGS与外观驱动扩散模型融合面临挑战，因为缺乏像素级、3D一致的编辑标准常导致过度修复和几何漂移。

Method: 引入FaithFusion框架，一个由像素级预期信息增益（EIG）驱动的3DGS-扩散融合系统。EIG作为统一的时空合成策略，引导扩散模型作为空间先验来优化高不确定性区域，同时其像素级加权将编辑结果蒸馏回3DGS。该系统是即插即用的，无需额外的先验条件和结构修改。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID方面均达到了SOTA性能，即使在6米车道偏移下也能保持107.47的FID。

Conclusion: FaithFusion通过引入像素级EIG，成功实现了3DGS与扩散模型的有效融合，解决了可控驾驶场景重建中几何保真度和视觉真实感之间的矛盾，并在大视角变化下展现出卓越的性能。
...
### [96] [From Observation to Action: Latent Action-based Primitive Segmentation for VLA Pre-training in Industrial Settings](https://arxiv.org/abs/2511.21428)
*Jiajie Zhang,Sören Schwertfeger,Alexander Kleiner*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的无监督框架，用于从工业视频流中提取和组织人类演示数据，以进行视觉-语言-动作（VLA）模型预训练。


<details>
  <summary>Details</summary>
Motivation: 动机是解锁大量未标记的人类演示数据，这些数据存在于连续的工业视频流中，以用于VLA模型预训练，因为这些数据目前是无结构的。

Method: 该方法首先训练一个轻量级运动tokenizer来编码运动动态，然后采用一个无监督动作分割器，利用新颖的“潜在动作能量”指标来发现和分割语义连贯的动作基元。整个流程输出分割后的视频片段及其对应的潜在动作序列。

Result: 在公共基准和专有的电机装配数据集上的评估表明，该方法能有效分割人类在工作站执行的关键任务。通过进一步的聚类和视觉-语言模型的定量评估，证实了所发现动作基元的语义连贯性。

Conclusion: 据作者所知，这是第一个全自动端到端系统，用于从非结构化工业视频中提取和组织VLA预训练数据，为具身AI在制造业中的集成提供了可扩展的解决方案。
...
### [97] [Frequency-Aware Token Reduction for Efficient Vision Transformer](https://arxiv.org/abs/2511.21477)
*Dong-Jae Lee,Jiwan Hur,Jaehyun Choi,Jaemyung Yu,Junmo Kim*

Main category: cs.CV

TL;DR: 本文提出了一种频率感知的Token缩减策略，通过区分高频和低频Token来提高Vision Transformer的计算效率和性能，同时缓解秩坍塌和过平滑问题。


<details>
  <summary>Details</summary>
Motivation: Vision Transformer的二次计算复杂度是其主要挑战。现有Token缩减方法常忽略自注意力的频率特性，如秩坍塌和过平滑现象。

Method: 所提出的方法将Token分为高频Token和低频Token。高频Token被选择性保留，而低频Token被聚合成一个紧凑的直流（DC）Token，以保留必要的低频分量。该策略旨在缓解秩坍塌。

Result: 实验表明，该方法显著提高了准确性，降低了计算开销，并有效缓解了秩坍塌和过平滑。此外，本文还分析了现有方法的隐式频率特性及其局限性。

Conclusion: 频率感知的Token缩减策略能有效提升Vision Transformer的计算效率和性能，并通过处理频率特性问题来缓解秩坍塌和过平滑现象。
...
TL;DR: G$^2$VLM是一个几何感知视觉语言模型，通过结合3D重建和空间理解，弥补了现有视觉语言模型在空间智能方面的不足，并在两类任务上均取得了优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLMs）在空间智能方面缺乏鲁棒性，在空间理解和推理任务上表现不佳。这主要归因于它们缺少从2D图像重建3D空间的能力。

Method: 本文提出了G$^2$VLM，一个几何感知的视觉语言模型。它利用学习到的3D视觉几何特征直接预测3D属性，并通过上下文学习和交错推理增强空间推理任务。该模型在多视角图像和视频数据上进行训练，并利用3D视觉先验，无需难以收集的标注。

Result: G$^2$VLM在3D重建任务上达到了与最先进的前馈3D重建模型相当的性能，并在空间理解和推理任务上取得了更好或具有竞争力的结果。

Conclusion: G$^2$VLM通过将语义强大的VLM与低级3D视觉任务相结合，有望成为社区的强大基线，并为3D场景编辑等未来应用开启更多可能性。
...
### [99] [Referring Video Object Segmentation with Cross-Modality Proxy Queries](https://arxiv.org/abs/2511.21139)
*Baoli Sun,Xinzhu Ma,Ning Wang,Zhihui Wang,Zhiyong Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为ProxyFormer的新型参照视频目标分割（RVOS）架构，通过引入代理查询（proxy queries）来早期整合视觉和文本语义，并实现跨帧依赖建模，从而解决现有方法中跨帧变化跟踪困难和文本约束整合滞后的问题，并在多个基准测试中取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有RVOS方法在处理跨模态对齐时，主要通过基于Transformer的条件查询机制。然而，它们存在两个主要局限性：1) 条件查询缺乏帧间依赖和变化建模，导致在显著的帧间变化下难以准确跟踪目标；2) 文本约束集成较晚，可能导致视频特征过早地关注非参照对象。

Method: 本文提出ProxyFormer架构，其核心是引入一组代理查询。这些代理查询用于整合视觉和文本语义，并在它们之间促进语义流。通过在视频特征编码器的多个阶段中逐步更新和传播代理查询，ProxyFormer确保视频特征始终聚焦于目标对象，并建立帧间依赖性以增强对象跟踪的准确性和连贯性。为降低计算成本，跨模态交互被解耦为时间和空间维度。此外，还设计了一种联合语义一致性（JSC）训练策略，以对齐代理查询与视频-文本组合之间的语义共识。

Result: 在四个广泛使用的RVOS基准测试上进行的综合实验表明，ProxyFormer优于现有最先进的方法。

Conclusion: ProxyFormer通过引入代理查询，有效地解决了RVOS中跨帧依赖和早期文本整合的挑战。其动态演化和传播的代理查询机制，结合跨模态交互解耦和JSC训练策略，显著提高了参照视频目标分割的准确性和连贯性。
...
### [100] [DeepRFTv2: Kernel-level Learning for Image Deblurring](https://arxiv.org/abs/2511.21132)
*Xintian Mao,Haofei Song,Yin-Nian Liu,Qingli Li,Yan Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为傅里叶核估计器（FKE）的新型去模糊网络，通过在傅里叶空间学习核级别的模糊过程，并利用网络提取的特征进行卷积，显著提升了图像去模糊的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有深度网络在图像去模糊方面仍停留在像素级学习或伪核级恢复阶段，未能真正理解模糊的本质（即锐利图像与模糊核的卷积过程）。让网络在核级别学习模糊过程可以显著提高去模糊性能。

Method: 1. 提出了傅里叶核估计器（FKE），在傅里叶空间进行激活操作，将空间域的卷积问题转化为傅里叶空间的乘法问题。FKE与去模糊模型联合优化，无需额外监督即可学习核级模糊过程。2. 将核的卷积对象从“图像”改为网络提取的“特征”，利用特征中丰富的语义和结构信息更好地学习模糊过程。3. 设计了一种解耦多尺度架构，包含多个分层子U-Net和可逆策略，以实现高效的特征提取和低训练内存消耗。

Result: 该方法在运动去模糊任务上取得了最先进的性能。实验表明其核估计器能够学习到具有物理意义的模糊核，并展现了处理其他核相关问题的潜力。

Conclusion: 通过傅里叶核估计器和改进的网络架构，模型能够有效地在核级别理解模糊的本质，从而实现卓越的图像去模糊效果，并具有高效率和低内存消耗的优势。
...
### [101] [EvRainDrop: HyperGraph-guided Completion for Effective Frame and Event Stream Aggregation](https://arxiv.org/abs/2511.21439)
*Futian Wang,Fan Zhang,Xiao Wang,Mengqi Wang,Dexing Huang,Jin Tang*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的超图引导时空事件流补全机制，通过超图连接稀疏事件，并利用上下文信息传递进行补全，同时支持多模态（如RGB）信息融合，并通过自注意力机制聚合特征，有效提升了事件分类性能。


<details>
  <summary>Details</summary>
Motivation: 主流事件表示学习算法（如事件帧、体素或张量）难以解决事件相机数据因空间稀疏性导致的欠采样问题。

Method: 提出了一种超图引导的时空事件流补全机制，通过超图连接不同时间与空间位置的事件令牌，并利用上下文信息传递进行稀疏事件补全。该方法可灵活地将RGB令牌作为超图节点纳入补全框架，实现多模态信息补全。随后，通过自注意力机制聚合跨时间步的超图节点信息，实现多模态特征的有效学习和融合。

Result: 在单标签和多标签事件分类任务上的大量实验充分验证了所提框架的有效性。

Conclusion: 所提出的超图引导时空事件流补全框架能有效解决事件数据的空间稀疏性问题，并通过多模态信息融合和自注意力机制，显著提升了事件分类任务的性能。
...
### [102] [TEAR: Temporal-aware Automated Red-teaming for Text-to-Video Models](https://arxiv.org/abs/2511.21145)
*Jiaming He,Guanyu Hou,Hongwei Li,Zhicong Huang,Kangjie Chen,Yi Yu,Wenbo Jiang,Guowen Xu,Tianwei Zhang*

Main category: cs.CV

TL;DR: 本文提出TEAR框架，一个针对文本到视频（T2V）模型的自动化红队框架，旨在发现与视频生成中的时间动态相关的安全风险，并显著提高了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: T2V模型能生成高质量、时间连贯的视频内容，但其多样性也带来了严重的安全挑战。现有的安全评估方法主要关注静态图像和文本生成，不足以捕捉视频生成中复杂的时序动态，因此需要专门针对视频时间动态的评估方法。

Method: TEAR是一个时间感知自动化红队框架。它采用一个时间感知测试生成器，通过两阶段方法进行优化：初始生成器训练和时间感知在线偏好学习。该生成器旨在创建文本上无害但能利用时间动态引发违反策略视频输出的提示。此外，还采用了一个优化模型来循环提高提示的隐蔽性和对抗有效性。

Result: TEAR在开源和商业T2V系统上进行了广泛实验评估，证明了其有效性，攻击成功率超过80%，相比之前最佳的57%有显著提升。

Conclusion: TEAR框架能够有效地发现T2V模型中与动态时序相关的安全风险，填补了现有安全评估方法的空白，为提升T2V模型的安全性提供了新途径。
...
TL;DR: CtrlVDiff是一个统一的扩散模型，通过结合多种图形模态（如深度、法线、语义、材质等）和混合模态控制策略，实现了视频理解和可控视频生成，并引入了MMVideo数据集，在可控性和真实性方面超越现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖几何信息（如深度、边缘）不足以理解外观、材质和光照，限制了物理意义上的编辑（如重新打光、材质交换），并常导致时间不一致性。构建一个能处理多种异构模态、对缺失输入鲁棒、并保持时间一致性的单一模型存在架构挑战。同时，缺乏大规模、时间对齐的真实视频与像素级多模态标注的数据集。

Method: 本文提出了CtrlVDiff，一个统一的扩散模型，采用混合模态控制策略（HMCS）来路由和融合来自深度、法线、分割、边缘以及图形学本征（反照率、粗糙度、金属度）等多种模态的特征。为训练此模型，构建了MMVideo数据集，这是一个混合真实与合成的数据集，其视频、多模态标注和字幕均已对齐。

Result: CtrlVDiff在理解和生成基准测试中展现出卓越的可控性和真实性，实现了分层编辑（如重新打光、材质调整、物体插入）。它超越了最先进的基线模型，并且在某些模态缺失的情况下仍能保持鲁棒性。

Conclusion: CtrlVDiff成功地在一个统一的扩散框架内解决了视频理解和可控视频生成的双重挑战。通过利用丰富的图形学模态和创新的控制策略，以及构建新的多模态数据集，该模型显著提升了视频编辑的精确性、可控性和物理真实性，同时保持了时间一致性。
...
### [104] [Merge and Bound: Direct Manipulations on Weights for Class Incremental Learning](https://arxiv.org/abs/2511.21490)
*Taehoon Kim,Donghwan Jang,Bohyung Han*

Main category: cs.CV

TL;DR: 本文提出了一种名为Merge-and-Bound (M&B) 的新颖训练方法，用于类别增量学习 (CIL)，它通过参数空间中的权重合并和有界更新来优化模型，有效减少灾难性遗忘并提高性能。


<details>
  <summary>Details</summary>
Motivation: 在类别增量学习 (CIL) 中，随着新任务的学习，模型容易遗忘旧任务的知识，即发生灾难性遗忘。研究旨在开发一种方法，能够在学习新任务的同时有效保留旧知识。

Method: M&B 方法直接在参数空间中操作模型权重进行优化。它包含两种权重合并：1) 任务间权重合并，通过平均所有先前阶段模型的权重来统一旧模型；2) 任务内权重合并，通过组合当前阶段内的模型参数来促进当前任务的学习。此外，还提出了有界更新技术，旨在通过最小累积更新来优化目标模型并保留先前任务的知识，从而有效获得接近旧模型的新模型，减少灾难性遗忘。M&B 可以无缝集成到现有 CIL 方法中，无需修改架构或学习目标。

Result: 在标准 CIL 基准测试中，M&B 算法表现出优于现有最先进方法的性能。

Conclusion: M&B 是一种有效的 CIL 训练方法，通过独特的权重合并和有界更新策略，能够成功地优化模型、保留旧知识、显著减少灾难性遗忘，并提高增量学习的性能。
...
### [105] [Multimodal Robust Prompt Distillation for 3D Point Cloud Models](https://arxiv.org/abs/2511.21574)
*Xiang Gu,Liming Lu,Xu Zheng,Anan Du,Yongbin Zhou,Shuchao Pang*

Main category: cs.CV

TL;DR: 本文提出了一种名为多模态鲁棒提示蒸馏 (MRPD) 的新型高效师生框架，通过整合多模态知识，为3D点云模型提供强大的对抗性防御，同时解决了现有防御方法计算开销大和泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 学习型3D点云模型面临对抗性攻击的严重威胁，这极大地损害了它们在安全敏感应用中的可靠性。现有的防御方法通常存在计算开销大和对多样化攻击类型泛化能力差的问题。

Method: 本文提出MRPD框架，通过将学生点云模型的特征与来自三个不同教师（处理深度投影的视觉模型、高性能3D模型和文本编码器）的鲁棒嵌入对齐，学习轻量级提示。该蒸馏过程由置信门控机制引导，动态平衡所有输入模态的贡献，确保可靠的知识转移。值得注意的是，所有蒸馏都在训练阶段进行，推理时没有额外的计算成本。

Result: 广泛的实验表明，MRPD在对抗各种白盒和黑盒攻击方面显著优于最先进的防御方法，甚至在干净数据上也取得了更好的性能。

Conclusion: 本文提出了一种通过高效利用多模态知识来构建鲁棒3D视觉系统的新型实用范式。
...
### [106] [You Can Trust Your Clustering Model: A Parameter-free Self-Boosting Plug-in for Deep Clustering](https://arxiv.org/abs/2511.21193)
*Hanyang Li,Yuheng Jia,Hui Liu,Junhui Hou*

Main category: cs.CV

TL;DR: DCBoost是一个无参数插件，通过利用可靠的局部结构线索，提升深度聚类模型中全局特征结构的性能，有效解决了全局与局部特征结构不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有深度聚类方法存在全局和局部特征结构之间的差异：局部结构通常表现出强一致性和紧凑性，而全局特征则常呈现交织的边界和分离不良的簇。

Method: DCBoost首先通过基于自适应k近邻的一致性过滤识别高置信度样本，作为自监督的可靠锚点。随后，利用这些样本计算判别性损失，以促进类内紧凑性和类间可分离性，从而指导网络优化。

Result: DCBoost显著提高了各种基准数据集上现有深度聚类模型的聚类性能。值得注意的是，它将当前最先进基线（如ProPos）的性能提升了3%以上，并将轮廓系数放大了7倍以上。

Conclusion: DCBoost通过利用可靠的局部结构线索，有效增强了深度聚类模型的全局特征结构，从而显著提升了聚类性能。
...
### [107] [Scenes as Tokens: Multi-Scale Normal Distributions Transform Tokenizer for General 3D Vision-Language Understanding](https://arxiv.org/abs/2511.21191)
*Yutao Tang,Cheng Zhao,Gaurav Mittal,Rohith Kukkala,Rama Chellappa,Cheng Peng,Mei Chen*

Main category: cs.CV

TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过多尺度正态分布变换（NDT）表示和解码器实现3D场景的整体token化，并统一了多种3D理解任务和人机交互。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言模型在将3D场景有效token化为整体场景token，并将其应用于各种3D理解任务方面仍面临巨大挑战，同时需要支持自然的人机交互。

Method: 该方法的核心是一个新颖的三阶段场景token化流程，它基于多尺度正态分布变换（NDT）表示，并结合多尺度NDT解码器（MSDec）。首先，NDTokenizer3D从高分辨率点云构建多尺度NDT表示，以保留全局上下文和精细几何细节。其次，MSDec逐步融合跨尺度NDT特征，生成可供LLM使用的整体场景token。此外，MSDec被重新用作人机交互提示（点、框、掩码）和分割掩码解码的通用接口，从而在单一架构中统一了多种3D场景理解任务。

Result: NDTokenizer3D在3D指代分割、3D视觉问答和3D密集字幕任务中取得了显著的改进，提供了一个细粒度、通用目的的3D视觉语言模型。

Conclusion: NDTokenizer3D通过其紧凑和统一的设计，提供了一个细粒度、通用目的的3D视觉语言模型，有效弥合了语言推理与3D空间理解之间的鸿沟。
...
### [108] [Towards an Effective Action-Region Tracking Framework for Fine-grained Video Action Recognition](https://arxiv.org/abs/2511.21202)
*Baoli Sun,Yihan Wang,Xinzhu Ma,Zhihui Wang,Kun Lu,Zhiyong Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为Action-Region Tracking (ART) 的新框架，通过查询-响应机制发现并跟踪视频中独特局部细节的动态，从而有效区分细粒度动作，超越了现有最先进的方法。


<details>
  <summary>Details</summary>
Motivation: 现有动作识别方法在细粒度动作识别（FGAR）中往往只能捕捉粗粒度运动模式，难以识别局部区域随时间演变的细微细节，导致无法有效区分相似动作。

Method: 本文提出了ART框架，核心包括：1) 区域特定语义激活模块，利用判别性、文本约束的语义作为查询，捕捉视频帧中最相关的动作区域响应。2) 动作轨迹（action tracklets），将捕获的区域响应组织成连贯序列，表征基于区域的动作动态。3) 文本约束查询，从VLM中提取动作标签的文本描述，编码细微语义表示。4) 多级轨迹对比约束，在空间和时间层面优化动作轨迹，实现帧内区分和帧间关联。5) 任务特定微调机制，优化文本语义以适应任务偏好。

Result: 在广泛使用的动作识别基准上进行了全面实验，结果表明ART框架优于以往的最新基线方法。

Conclusion: ART框架通过发现和跟踪独特的局部细节动态，能够有效区分相似的细粒度动作，显著提升了细粒度动作识别的性能。
...
### [109] [Continual Error Correction on Low-Resource Devices](https://arxiv.org/abs/2511.21652)
...
TL;DR: 该研究提出了一种新颖的系统，通过结合服务器端基础模型训练和设备端原型分类，实现了在资源受限设备上通过少样本学习高效纠正AI模型误分类，无需模型重训练。


<details>
  <summary>Details</summary>
Motivation: 日常设备中AI模型普及带来的预测错误会降低用户体验，现有解决方案主要侧重错误检测，但缺乏高效的纠正机制，尤其是在资源受限设备上。

Method: 该系统包含两个关键组件：1) 服务器端管道，利用知识蒸馏将基础模型的鲁棒特征表示转移到设备兼容架构；2) 设备端机制，通过原型适应实现超高效的错误纠正，而非模型重训练。它通过原型更新来纠正错误，采用少样本学习，所需计算资源和存储极少。

Result: 该系统在图像分类和目标检测任务上表现出色，在Food-101和Flowers-102数据集上的一样本情景中实现了超过50%的错误纠正，同时保持了极小的遗忘（小于0.02%）和可忽略的计算开销。通过Android演示应用程序验证了其在真实场景中的实用性。

Conclusion: 该系统提供了一种实用且高效的方法，通过原型更新实现设备端AI误分类的少样本纠正，解决了资源受限设备上的AI错误修正挑战。
...
### [110] [3-Tracer: A Tri-level Temporal-Aware Framework for Audio Forgery Detection and Localization](https://arxiv.org/abs/2511.21237)
*Shuhan Xia,Xuannan Liu,Xing Cui,Peipei Li*

Main category: cs.CV

TL;DR: 本文提出T3-Tracer，一个首次在帧、段和音频三个层次联合分析音频以全面检测部分音频伪造的框架，通过结合帧级和音频级信息及多尺度段级异常检测，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 部分音频伪造选择性修改语义关键帧，同时保持整体感知真实性，使其难以检测。现有方法仅独立检测单帧伪造，缺乏捕获不同时间层面瞬态和持续异常的层次结构。

Method: T3-Tracer框架包含两个核心模块：1) 帧-音频特征聚合模块 (FA-FAM)，结合帧级和音频级时间信息检测帧内伪造线索和全局语义不一致性，以判断每帧真实性。2) 段级多尺度差异感知模块 (SMDAM)，采用双分支架构，在多尺度时间窗口内联合建模帧特征和帧间差异，有效识别伪造边界上的突发异常，以细化和校正帧检测。

Result: 在三个具有挑战性的数据集上进行的广泛实验表明，T3-Tracer方法达到了最先进的性能。

Conclusion: T3-Tracer通过在帧、段和音频三个关键层面联合分析，能够全面检测部分音频伪造痕迹，有效解决了现有方法的局限性，并实现了卓越的检测效果。
...
### [111] [Attention-Guided Patch-Wise Sparse Adversarial Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.21663)
...
### [112] [Qwen3-VL Technical Report](https://arxiv.org/abs/2511.21631)
...
TL;DR: Qwen3-VL是Qwen系列迄今为止最强大的视觉语言模型，在多模态基准测试中表现卓越，原生支持256K令牌的文本、图像和视频交错上下文，并提供密集型和MoE变体以平衡延迟与质量。


<details>
  <summary>Details</summary>
Motivation: 旨在开发一个更强大、更通用的视觉语言模型，以在广泛的多模态任务上实现卓越性能，并支持长上下文理解和复杂的跨模态推理。

Method: 该模型支持高达256K令牌的文本、图像和视频交错上下文。架构上引入了三项关键升级：(i) 增强的交错式MRoPE以强化图像和视频的时空建模；(ii) DeepStack集成，有效利用多级ViT特征以紧密视觉语言对齐；(iii) 基于文本的视频时间对齐，从T-RoPE演变为显式文本时间戳对齐，以实现更精确的时间定位。模型家族包括密集型（2B/4B/8B/32B）和专家混合（30B-A3B/235B-A22B）变体。

Result: Qwen3-VL实现了三个核心支柱：(i) 显著更强的纯文本理解能力，在某些情况下超越了同类纯文本骨干；(ii) 鲁棒的长上下文理解能力，原生支持256K令牌窗口，可实现文本和交错多模态输入的忠实保留、检索和交叉引用；(iii) 先进的多模态推理能力，在单图像、多图像和视频任务上表现出色，并在MMMU、MathVista和MathVision等综合评估中达到领先性能。在可比的令牌预算和延迟约束下，Qwen3-VL在密集型和MoE架构中均取得了卓越性能。

Conclusion: Qwen3-VL被设想为图像接地推理、代理决策和真实世界工作流中多模态代码智能的基础引擎。
...
### [113] [From Diffusion to One-Step Generation: A Comparative Study of Flow-Based Models with Application to Image Inpainting](https://arxiv.org/abs/2511.21215)
*Umang Agarwal,Rudraksh Sangore,Sumit Laddha*

Main category: cs.CV

TL;DR: 本文对DDPM、CFM和MeanFlow三种生成模型范式进行了比较研究。CFM在50步采样下表现最佳，MeanFlow实现了快速的单步生成。此外，将CFM扩展到图像修复，通过修复感知训练显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 研究者旨在对Denoising Diffusion Probabilistic Models (DDPM)、Conditional Flow Matching (CFM) 和 MeanFlow 这三种生成建模范式进行全面的比较研究，评估它们在生成质量和采样效率上的表现，并探索CFM在图像修复任务上的潜力。

Method: 研究方法包括：1) 使用统一的TinyUNet架构（<1.5M参数）在CIFAR-10数据集上实现并比较DDPM、CFM和MeanFlow。2) MeanFlow通过建模时间间隔内的平均速度实现直接的单步生成。3) 将CFM扩展到图像修复，使用四种掩码类型（中心、随机边界框、不规则、半边）进行掩码引导采样。4) 实施了修复感知的训练策略以优化CFM的修复性能。

Result: 主要结果如下：1) CFM在50步采样下实现了24.15的FID，显著优于DDPM的402.98 FID。2) MeanFlow通过单步采样实现了29.15的FID，推理时间减少了50倍。3) 经过微调的图像修复CFM模型在中心掩码上的PSNR从4.95 dB提高到8.57 dB（+73%），SSIM从0.289提高到0.418（+45%）。

Conclusion: 研究得出结论：CFM在生成质量上表现出色，MeanFlow提供了高效的单步生成能力。通过修复感知训练，CFM在图像修复任务上的性能得到了显著提升，证明了该训练策略的有效性。
...
### [114] [FIELDS: Face reconstruction with accurate Inference of Expression using Learning with Direct Supervision](https://arxiv.org/abs/2511.21245)
*Chen Ling,Henglin Shi,Hedvig Kjellström*

Main category: cs.CV

TL;DR: FIELDS提出了一种新的3D人脸重建方法，通过结合直接3D表情参数监督和辅助情感识别分支，从单张图像中生成高保真、情感丰富的3D人脸模型，显著提升了野外表情识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D人脸重建方法因依赖2D监督和缺乏3D真实数据，常无法捕捉细微的情感细节。

Method: FIELDS方法通过以下方式解决问题：1) 扩展自监督2D图像一致性线索；2) 引入直接3D表情参数监督；3) 增加辅助情感识别分支。其编码器由自发4D面部扫描的真实表情参数指导，并使用强度感知情感损失来捕捉真实情感内容，避免夸张。

Result: FIELDS能从单张图像生成具有高度逼真表情的、情感丰富的3D人脸模型，显著提高了野外人脸表情识别的性能，同时不牺牲自然度。它生成的高保真3D重建保留了细微的情感线索。

Conclusion: FIELDS的这种双重监督策略弥合了2D/3D领域鸿沟，并减轻了表情强度偏差，从而实现了高保真、情感丰富的3D人脸重建。
...
### [115] [LaGen: Towards Autoregressive LiDAR Scene Generation](https://arxiv.org/abs/2511.21256)
*Sizhuo Zhou,Xiaosong Jia,Fanrui Zhang,Junjie Li,Juyong Zhang,Yukang Feng,Jianwen Sun,Songbur Wong,Junqi You,Junchi Yan*

Main category: cs.CV

TL;DR: 本文提出了LaGen，这是一个开创性的框架，能够实现长时程、逐帧、自回归的LiDAR场景生成，支持交互式内容生成，并优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR数据生成方法仅支持单帧生成，而预测方法需要多帧历史输入且只能确定性预测，缺乏交互性。这两种范式都无法支持长时程的交互式生成。

Method: 本文引入了LaGen框架，这是首个能够逐帧自回归生成长时程LiDAR场景的模型。它以单帧LiDAR输入为起点，并利用边界框信息作为条件来生成高保真4D场景点云。此外，LaGen还包含一个场景解耦估计模块以增强对象级内容的交互式生成能力，以及一个噪声调制模块以缓解长时程生成中的误差累积。

Result: 基于nuScenes数据集构建的长时程LiDAR场景生成评估协议，实验结果全面证明LaGen优于最先进的LiDAR生成和预测模型，尤其在后期帧表现更佳。

Conclusion: LaGen成功实现了长时程、高保真、交互式的4D LiDAR场景生成，填补了现有方法的空白，并为自动驾驶领域的生成世界模型提供了新的方向。
...
### [116] [Shift-Equivariant Complex-Valued Convolutional Neural Networks](https://arxiv.org/abs/2511.21250)
...
### [117] [PathMamba: A Hybrid Mamba-Transformer for Topologically Coherent Road Segmentation in Satellite Imagery](https://arxiv.org/abs/2511.21298)
*Jules Decaestecker,Nicolas Vigne*

Main category: cs.CV

TL;DR: 本文提出PathMamba，一种混合Mamba-Transformer架构，用于卫星图像道路分割。它结合Mamba的线性效率和拓扑连续性建模能力与Transformer的全局上下文推理能力，在保持计算竞争力的同时，显著提高了道路分割的准确性和拓扑连续性，达到了新的SOTA。


<details>
  <summary>Details</summary>
Motivation: 从卫星图像中分割道路需要高精度和拓扑连续性，这对于城市规划和灾害响应至关重要。现有最先进的Vision Transformer方法虽然能捕捉全局上下文，但其二次复杂度限制了在资源受限平台上的高效部署。新兴的Mamba等状态空间模型具有线性时间效率，且天然适合建模长而连续的结构。

Method: 本文引入PathMamba，一种新颖的混合架构，将Mamba的序列建模能力与Transformer的全局推理能力相结合。设计上，策略性地使用Mamba块来追踪道路网络的连续性并保持拓扑结构，同时整合Transformer块以全局上下文精炼特征。

Result: PathMamba在DeepGlobe Road Extraction和Massachusetts Roads数据集上均取得了新的最先进成果。它显著改善了拓扑连续性（通过APLS指标衡量），设定了新基准，同时保持了计算上的竞争力。该方法在没有纯注意力模型高昂扩展成本的情况下，生成了拓扑更优的分割图。

Conclusion: PathMamba成功地结合了Mamba和Transformer的优势，实现了卫星图像道路分割的高精度和卓越拓扑连续性，同时提高了计算效率，为该领域设定了新的基准。
...
### [118] [CaliTex: Geometry-Calibrated Attention for View-Coherent 3D Texture Generation](https://arxiv.org/abs/2511.21309)
*Chenyu Liu,Hongze Chen,Jingzhi Bao,Lingting Zhu,Runze Zhang,Weikai Chen,Zeyu Hu,Yingda Yin,Keyang Luo,Xin Wang*

Main category: cs.CV

TL;DR: 现有扩散模型在3D纹理生成中存在跨视角不一致问题，源于注意力模糊。CaliTex引入几何校准注意力（包括部件对齐和条件路由注意力），使几何一致性成为网络固有特性，从而生成无缝且视角一致的纹理，超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型取得了重大进展，但当前3D纹理生成系统仍受跨视角不一致问题的困扰。研究发现此问题源于注意力模糊，即无结构的全面注意力不加区分地应用于token和模态，导致几何混淆和不稳定的外观-结构耦合。

Method: 引入CaliTex框架，该框架采用几何校准注意力，明确将注意力与3D结构对齐。它包含两个模块：1. 部件对齐注意力（Part-Aligned Attention），强制语义匹配部件间的空间对齐；2. 条件路由注意力（Condition-Routed Attention），通过几何条件路径路由外观信息以保持空间保真度。CaliTex与两阶段扩散Transformer结合使用。

Result: CaliTex能够生成无缝且视角一致的纹理。在经验上，它优于开源和商业基线。

Conclusion: CaliTex使几何一致性成为网络的固有行为而非优化副产品，有效解决了3D纹理生成中的跨视角不一致问题，显著提升了纹理质量和一致性。
...
### [119] [Unlocking Zero-shot Potential of Semi-dense Image Matching via Gaussian Splatting](https://arxiv.org/abs/2511.21265)
*Juncheng Chen,Chao Xu,Yanjun Cao*

Main category: cs.CV

TL;DR: MatchGS是一个框架，通过校正3D高斯泼溅（3DGS）的几何不准确性，生成高精度训练数据，并实现2D-3D表示对齐，从而显著提升图像匹配的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 学习型图像匹配严重依赖大规模、多样化且几何精确的训练数据。3DGS虽然能实现逼真的新视图合成，但其几何不准确性和有偏的深度渲染阻碍了鲁棒的对应标签生成。

Method: 本研究引入MatchGS框架，主要包括两方面：1) 一个几何忠实的数据生成流程，用于精炼3DGS几何，以生成高精度的对应标签，并在不影响渲染保真度的情况下合成大量多样化的视角；2) 一个2D-3D表示对齐策略，将3DGS的显式3D知识融入2D匹配器，引导2D半稠密匹配器学习视角不变的3D表示。

Result: MatchGS生成的真值对应关系将极线误差比现有数据集降低了多达40倍，支持极端视角变化下的监督学习，并通过高斯属性提供自监督信号。因此，仅使用MatchGS数据训练的现有最佳匹配器在公共基准测试中实现了显著的零样本性能提升，最高可达17.7%。

Conclusion: 本研究表明，经过适当的几何精炼，3DGS可以作为一种可扩展、高保真且结构丰富的图像匹配数据源，为新一代鲁棒的零样本图像匹配器铺平了道路。
...
### [120] [Thinking With Bounding Boxes: Enhancing Spatio-Temporal Video Grounding via Reinforcement Fine-Tuning](https://arxiv.org/abs/2511.21375)
*Xin Gu,Haoji Zhang,Qihang Fan,Jingxuan Niu,Zhipeng Zhang,Libo Zhang,Guang Chen,Fan Chen,Longyin Wen,Sijie Zhu*

Main category: cs.CV

TL;DR: 本文提出了STVG-o1，一个无需修改架构即可使现有多模态大语言模型（MLLMs）在时空视频定位（STVG）任务上达到最先进性能的框架。它通过引入边界框思维链机制和多维度强化奖励函数实现几何感知监督，显著提升了MLLMs在该任务上的表现和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型（MLLMs）具有强大的语言理解能力，但由于训练目标不匹配以及标准视觉编码器中细粒度区域-词语对齐不足，它们在时空视频定位（STVG）任务上表现不佳。

Method: 本文提出了STVG-o1框架，该框架允许现成的MLLMs无需任何架构修改即可应用于STVG。核心方法包括：1) 引入边界框思维链（bounding-box chain-of-thought）机制，在生成最终预测之前明确地推理时空位置；2) 设计一个多维度强化奖励函数，包含格式、一致性、时间、空间和思考奖励，通过强化微调提供几何感知的监督。

Result: STVG-o1在HCSTVG-v1/v2和VidSTG数据集上进行了评估，在HCSTVG上取得了新的最先进结果，在HCSTVG-v1上比最佳的特定任务方法高出7.3%的m_tIoU，在VidSTG上与专业模型持平，并大幅超越了所有现有的基于MLLM的方法。它还展示了强大的跨数据集开放词汇泛化能力。

Conclusion: STVG-o1的成功表明，多模态大语言模型（MLLMs）可以成为精确时空定位任务的强大且可行的骨干模型，并且无需进行架构修改即可实现卓越性能。
...
### [121] [HTTM: Head-wise Temporal Token Merging for Faster VGGT](https://arxiv.org/abs/2511.21317)
*Weitian Wang,Lukas Meiner,Rai Shubham,Cecilia De La Parra,Akash Kumar*

Main category: cs.CV

TL;DR: 本文提出了一种名为头级时间合并（HTTM）的无训练3D token合并方法，旨在加速视觉几何基础Transformer（VGGT）在大型场景3D重建中的推理速度，实现了高达7倍的加速且性能损失可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 视觉几何基础Transformer（VGGT）首次实现了3D场景所有关键属性（相机姿态、深度和密集几何）的联合推断。然而，其全局注意力层在处理长序列输入的大型场景时，需要对所有视图的token进行全对全计算，导致显著的延迟瓶颈。

Method: 本文提出了头级时间合并（HTTM），这是一种无训练的3D token合并方法，用于加速VGGT。与现有均匀合并token的方法不同，HTTM在多头粒度上进行token合并，从而在头部连接后保留了特征token的独特性。此外，HTTM利用在头级别观察到的空间局部性和时间对应性，以更低的合并成本实现更高的合并比率。

Result: HTTM在基于GPU的推理中实现了高达7倍的加速，并且性能下降可以忽略不计。

Conclusion: HTTM通过创新的头级时间合并策略，有效解决了VGGT在处理大型场景时因全局注意力导致的延迟问题，显著提升了推理速度，同时保持了模型性能。
...
### [122] [Endo-G$^{2}$T: Geometry-Guided & Temporally Aware Time-Embedded 4DGS For Endoscopic Scenes](https://arxiv.org/abs/2511.21367)
*Yangle Liu,Fengze Li,Kan Liu,Jieming Ma*

Main category: cs.CV

TL;DR: Endo-G²T 是一种几何引导和时间感知的训练方案，用于动态内窥镜场景中的 4D 高斯泼溅 (4DGS)，通过早期几何锚定、时间一致性和效率改进，解决了内窥镜视频中纯光度监督导致的几何漂移问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈的视点依赖效应（如镜面反射、湿反射、遮挡），导致纯光度监督与几何不匹配，并引发早期几何漂移。这种漂移一旦形成，在稠密化过程中难以纠正。因此，需要一种方法在动态内窥镜场景中为 4DGS 早期锚定几何，同时保持时间一致性和效率。

Method: 该研究提出了 Endo-G²T，包含三个核心方法：1. 几何引导先验蒸馏：将置信度门控的单目深度转换为监督信号，采用尺度不变深度和深度梯度损失，并以“热身到上限”调度策略软性注入先验，避免早期过拟合。2. 时间嵌入高斯场：使用类似转子的旋转参数化表示 XYZT 中的动态，通过轻量级正则化实现时间连贯的几何，偏爱平滑运动和清晰的不透明度边界。3. 关键帧约束流式处理：通过在最大点数预算下进行关键帧聚焦优化，并对非关键帧进行轻量级更新，提高效率和长时程稳定性。

Result: Endo-G²T 在 EndoNeRF 和 StereoMIS-P1 数据集上，在单目重建基线中实现了最先进的结果。

Conclusion: Endo-G²T 通过几何引导和时间感知训练方案，有效解决了动态内窥镜场景中 4DGS 的几何锚定、时间一致性和效率挑战，显著提升了重建性能。
...
### [123] [PFF-Net: Patch Feature Fitting for Point Cloud Normal Estimation](https://arxiv.org/abs/2511.21365)
*Qing Li,Huifang Feng,Kanle Shi,Yue Gao,Yi Fang,Yu-Shen Liu,Zhizhong Han*

Main category: cs.CV

TL;DR: 该论文提出了一种基于多尺度特征融合的新型特征提取方法，用于点云的鲁棒法线估计，通过多尺度特征聚合和跨尺度特征补偿实现尺度自适应和最优几何描述。


<details>
  <summary>Details</summary>
Motivation: 在点云法线估计中，确定合适的局部邻域大小对于不同数据或几何形状来说非常困难。现有方法虽然使用参数密集型策略，但在准确高效地预测各种点云的法线方面仍面临挑战。

Method: 本文提出了一种多尺度特征融合的特征提取新思路，通过融合不同邻域大小的多尺度特征来解决邻域尺寸选择问题。具体方法包括：1) 构建一个基于多尺度特征的补丁特征拟合（PFF）模型，以近似最优几何描述；2) 实现多尺度特征聚合模块，逐步聚合不同尺度的补丁特征并缩小补丁尺寸，以捕捉广泛结构和精细几何细节；3) 实现跨尺度特征补偿模块，确保早期层大尺度特征的重用性并揭示不同补丁尺寸的相关信息。

Result: 该方法在合成和真实世界数据集上均取得了最先进的性能，且网络参数更少，运行时间更短。

Conclusion: 所提出的基于多尺度特征聚合的近似策略使模型能够实现对不同局部补丁的尺度自适应，并提供最优的特征描述，从而在点云法线估计中达到卓越性能。
...
### [124] [CanKD: Cross-Attention-based Non-local operation for Feature-based Knowledge Distillation](https://arxiv.org/abs/2511.21503)
*Shizhe Sun,Wataru Ohyama*

Main category: cs.CV

TL;DR: 本文提出了一种名为CanKD的跨注意力非局部知识蒸馏框架，通过利用跨注意力机制增强知识迁移，使学生特征图中的每个像素能够动态考虑教师特征图中的所有像素，从而在目标检测和图像分割任务中取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的自注意力蒸馏方法独立对齐教师和学生特征图，未能充分捕捉像素间的关系，限制了特征表示学习的效果。

Method: CanKD引入了基于跨注意力的非局部知识蒸馏框架。它允许学生特征图中的每个像素动态地考虑教师特征图中的所有像素，从而实现更彻底的像素级关系捕获。该方法仅增加了一个额外的损失函数。

Result: CanKD在目标检测和图像分割任务上表现出卓越的性能，超越了现有的先进特征和混合蒸馏方法。

Conclusion: 实验结果表明CanKD作为计算机视觉任务中注意力引导蒸馏的新范式具有巨大潜力。
...
### [125] [E-M3RF: An Equivariant Multimodal 3D Re-assembly Framework](https://arxiv.org/abs/2511.21422)
*Adeela Islam,Stefano Fiorini,Manuel Lecha,Theodore Tsesmelis,Stuart James,Pietro Morerio,Alessio Del Bue*

Main category: cs.CV

TL;DR: 本文提出E-M3RF，一个等变多模态3D重组框架，它结合点云的几何位置和颜色信息，通过SE(3)流匹配预测碎片变换，以解决传统方法在几何信息不足或模糊时的局限性，并实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前的深度学习3D重组方法主要依赖几何特征，导致在碎片几何信息不足、模糊（如小、受侵蚀或对称碎片）时表现不佳。此外，这些方法未能明确施加物理约束以防止重叠组装。

Method: E-M3RF框架接收包含点位置和颜色的碎片点云作为输入，并使用SE(3)流匹配预测所需的变换。每个碎片通过多模态特征表示：3D点位置通过旋转等变编码器编码为旋转一致的几何特征；每个3D点的颜色通过Transformer编码。这两种特征集随后结合形成多模态表示。

Result: 在Breaking Bad、Fantastic Breaks（合成）和RePAIR、Presious（真实世界文化遗产）四个数据集上进行了实验。在RePAIR数据集上，E-M3RF与竞争方法相比，旋转误差降低了23.1%，平移误差降低了13.2%，Chamfer距离降低了18.4%。

Conclusion: E-M3RF通过引入几何和颜色相结合的多模态特征以及等变架构，有效解决了仅依赖几何特征的3D重组方法的局限性，在多个数据集上，尤其是在真实的文化遗产数据集上，展示了优越的重组性能和更高的精度。
...
### [126] [Co-Training Vision Language Models for Remote Sensing Multi-task Learning](https://arxiv.org/abs/2511.21272)
...
TL;DR: 本文提出了RSCoVLM，一个简单而灵活的遥感多任务学习（MTL）视觉语言模型（VLM）基线。它通过数据整理引擎、统一动态分辨率策略和针对超高分辨率图像的Zoom-in Chain机制，在多种遥感任务上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在遥感（RS）任务中表现出色，促使研究人员探索通过多任务学习（MTL）实现统一模型。MTL方法具有更好的泛化性、可扩展性和实用性。此外，视觉语言模型（VLMs）在遥感图像理解、定位和超高分辨率（UHR）图像推理方面取得了显著成果，其统一的文本接口在MTL中也展现出巨大潜力。

Method: 本文提出了RSCoVLM，一个灵活的遥感MTL VLM基线。方法包括：1) 创建数据整理引擎，处理复杂的遥感数据环境并生成灵活的视觉-语言对话。2) 提出统一动态分辨率策略，以处理遥感图像固有的不同图像尺度。3) 针对UHR图像引入Zoom-in Chain机制及其对应的LRS-VQA-Zoom数据集，有效减轻计算负担。4) 显著增强模型的物体检测能力，并提出新的评估协议以确保VLM与传统检测模型之间的公平比较。

Result: RSCoVLM在多种任务上取得了最先进的性能，超越了现有遥感VLM，甚至能与专业的专家模型相媲美。所有的训练和评估工具、模型权重和数据集都已完全开源，以支持可复现性。

Conclusion: RSCoVLM作为一个强大且灵活的遥感多任务学习VLM基线，在多种任务上实现了SOTA性能。该基线有望推动通用遥感模型的进一步发展，其所有资源均已开源以促进研究进展。
...
### [127] [MobileI2V: Fast and High-Resolution Image-to-Video on Mobile Devices](https://arxiv.org/abs/2511.21475)
*Shuai Zhang,Bao Tang,Siyuan Yu,Yueting Zhu,Jingfeng Yao,Ya Zou,Shanglin Yuan,Li Yu,Wenyu Liu,Xinggang Wang*

Main category: cs.CV

TL;DR: 本文提出了MobileI2V，一个270M的轻量级扩散模型，首次实现了在移动设备上快速、高质量的720p图像到视频（I2V）生成，解决了现有模型计算复杂和速度慢的挑战。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成方面取得了快速进展，但其高计算复杂性和慢生成速度严重阻碍了在资源受限的移动设备上进行实时、高分辨率的I2V合成。

Method: 本文采用了以下核心方法：(1) 提出了一种线性混合架构去噪器，通过分析线性注意力和softmax注意力模块在移动设备上的性能，平衡了生成效率和质量。(2) 设计了一种时间步蒸馏策略，将I2V采样步骤从20多个压缩到仅2个，显著提升了生成速度。(3) 应用了移动设备特定的注意力优化，进一步加速了设备上的推理过程。

Result: MobileI2V模型参数量为270M。它首次实现了在移动设备上快速生成720p图像到视频，其质量与现有模型相当。在一步采样条件下，生成每帧720p视频的速度低于100毫秒。时间步蒸馏策略使生成速度提高了10倍，而移动特定的注意力优化使注意力操作速度提高了2倍。

Conclusion: MobileI2V成功地为移动设备带来了实时、高质量的图像到视频生成能力，通过轻量级设计、高效的采样策略和设备特定优化，克服了传统扩散模型的性能瓶颈。
...
### [128] [DiverseVAR: Balancing Diversity and Quality of Next-Scale Visual Autoregressive Models](https://arxiv.org/abs/2511.21415)
*Mingue Park,Prin Phunyaphibarn,Phillip Y. Lee,Minhyuk Sung*

Main category: cs.CV

TL;DR: DiverseVAR是一个在测试时增强文本条件视觉自回归模型（VAR）多样性的框架，无需重新训练或大量计算开销，通过文本嵌入噪声注入和尺度旅行（scale-travel）精炼来提高多样性并保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型（VAR）在图像生成方面表现出色，但其关键限制在于多样性不足，即使对于简单提示也常生成几乎相同的图像。这一问题在当前主要关注图像质量的研究中被忽视。

Method: 该方法分两阶段解决多样性问题：1. 注入噪声到文本嵌入中以增强多样性（受扩散模型启发），但这会导致图像质量下降。2. 提出“尺度旅行”（scale-travel），一种新颖的潜在精炼技术（受扩散模型中的时间旅行策略启发），利用多尺度自编码器提取粗尺度token，从而在中级阶段恢复生成，以保持图像质量。

Result: 大量实验表明，将文本嵌入噪声注入与尺度旅行精炼相结合，显著增强了多样性，同时最大限度地减少了图像质量下降，在多样性-质量权衡中达到了新的帕累托前沿。

Conclusion: DiverseVAR框架通过在测试时注入文本嵌入噪声并结合尺度旅行精炼，成功解决了VAR模型多样性不足的问题，有效平衡了多样性和图像质量。
...
### [129] [AVFakeBench: A Comprehensive Audio-Video Forgery Detection Benchmark for AV-LMMs](https://arxiv.org/abs/2511.21251)
*Shuhan Xia,Peipei Li,Xuannan Liu,Dongsen Zhang,Xinyu Guo,Zekun Li*

Main category: cs.CV

TL;DR: 本文介绍了AVFakeBench，首个综合性的音视频伪造检测基准，旨在解决现有基准在伪造类型和标注粒度上的局限性，并评估了音视频大语言模型（AV-LMMs）在该任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的音视频伪造检测基准主要集中于DeepFake和单一粒度标注，无法捕捉真实世界中日益多样化和复杂的自然场景伪造。研究旨在创建一个更全面的基准来应对这一挑战。

Method: 本文提出了AVFakeBench，包含12K精心策划的音视频问题，涵盖七种伪造类型和四级标注，包括人类主体和通用主体。为确保高质量和多样性，作者提出了一个多阶段混合伪造框架，结合专有模型进行任务规划和专家生成模型进行精确操作。此外，还建立了一个多任务评估框架，包括二元判断、伪造类型分类、伪造细节选择和解释性推理。

Result: 通过在AVFakeBench上评估11个音视频大语言模型（AV-LMMs）和2种主流检测方法，结果表明AV-LMMs作为新兴的伪造检测器具有潜力，但也揭示了它们在细粒度感知和推理方面的显著弱点。

Conclusion: AVFakeBench是一个全面的音视频伪造检测基准，能够更好地反映真实世界的伪造场景。评估结果表明，尽管AV-LMMs在伪造检测方面展现出潜力，但它们在处理细粒度信息和进行复杂推理时仍需进一步改进。
...
### [130] [Self-Paced Learning for Images of Antinuclear Antibodies](https://arxiv.org/abs/2511.21519)
*Yiyang Jiang,Guangwu Qian,Jiaxin Wu,Qi Huang,Qing Li,Yongkang Wu,Xiao-Yong Wei*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的框架，用于自动化抗核抗体（ANA）检测，该框架通过模拟人类标注逻辑，处理多实例多标签（MIML）任务的复杂性，并在ANA数据集和公共医学MIML基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: ANA检测对诊断自身免疫性疾病至关重要，但手动检测速度慢、劳动强度大、需要专业训练，且面临超过100种抗体类型导致的复杂荧光模式组合。尽管机器学习和深度学习已实现自动化，但真实临床环境中的ANA检测涉及多实例多标签（MIML）学习，对现有方法构成独特挑战。

Method: 本文提出了一种新颖的ANA检测框架，直接处理未处理的显微镜图像，以解决MIML任务的复杂性。该框架受人类标注逻辑启发，识别一致的ANA子区域并分配聚合标签。它包含三个任务特定组件：实例采样器（通过建模模式置信度抑制低置信度实例）、概率伪标签分派器（根据实例可区分性自适应分配标签）和自定步长权重学习率系数（根据经验标签观察调整训练）。该框架克服了传统MIML方法的局限性，并支持端到端优化。

Result: 在ANA数据集上，该模型在F1-Macro上实现了高达+7.0%的增益，在mAP上实现了高达+12.6%的增益，超过了之前最好的方法，并设定了新的最先进结果。在三个公共医学MIML基准数据集上，该框架在所有关键指标上均排名前二，将Hamming损失和one-error分别降低了高达18.2%和26.9%。

Conclusion: 本文提出的框架有效解决了ANA检测中的MIML复杂性，通过创新的实例采样、伪标签分派和自定步长学习机制，实现了优于传统MIML方法的性能，并在多个基准测试中取得了最先进的成果，为自动化自身免疫性疾病诊断提供了强大工具。
...
### [131] [Generalized Design Choices for Deepfake Detectors](https://arxiv.org/abs/2511.21507)
*Lorenzo Pellegrini,Serafino Pandolfini,Davide Maltoni,Matteo Ferrara,Marco Prati,Marco Ramilli*

Main category: cs.CV

TL;DR: 本文系统研究了深度伪造检测方法中数据预处理、增强策略和优化技术等实现细节对模型性能的影响，旨在建立与架构无关的最佳实践。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测方法的有效性往往更多取决于实现细节而非核心设计，这使得公平比较检测器并理解其性能的真正贡献因素变得困难。

Method: 通过隔离单个因素的影响，系统地调查了不同设计选择（训练、推理和增量更新相关方面）如何影响深度伪造检测模型的准确性和泛化能力。

Result: 实验识别了一组能够持续改进深度伪造检测并在AI-GenBench基准测试上实现最先进性能的设计选择。

Conclusion: 旨在为未来深度伪造检测系统的设计和开发建立稳健的、与架构无关的最佳实践。
...
### [132] [Video Generation Models Are Good Latent Reward Models](https://arxiv.org/abs/2511.21541)
...
TL;DR: 针对视频生成，本文提出过程奖励反馈学习（PRFL），通过在潜在空间中进行偏好优化，显著提升了与人类偏好的对齐效果，并大幅降低了内存消耗和训练时间。


<details>
  <summary>Details</summary>
Motivation: 现有的视频奖励反馈学习（ReFL）面临挑战，其奖励模型依赖于像素空间输入，导致优化仅限于去噪后期，效率低下（高内存、长训练时间），且缺乏早期监督，未能有效改进运动动态和结构连贯性。

Method: 本文提出过程奖励反馈学习（PRFL），利用预训练视频生成模型在噪声潜在空间处理任意时间步表示和保留时间信息的固有能力，将偏好优化完全在潜在空间中进行，从而实现无需VAE解码的全去噪链高效梯度反向传播。

Result: PRFL显著提升了视频生成与人类偏好的对齐程度，同时相比于传统的RGB ReFL，大幅减少了内存消耗和训练时间。

Conclusion: PRFL通过在潜在空间进行奖励建模，为视频生成中的偏好对齐提供了一个高效且有效的框架，解决了现有像素空间方法的局限性。
...
### [133] [EoS-FM: Can an Ensemble of Specialist Models act as a Generalist Feature Extractor?](https://arxiv.org/abs/2511.21523)
*Pierre Adorni,Minh-Tan Pham,Stéphane May,Sébastien Lefèvre*

Main category: cs.CV

TL;DR: 本文提出了一种名为“专家集成”（Ensemble-of-Specialists）的新颖高效框架，用于构建遥感基础模型（RSFMs），以解决当前大型基础模型资源消耗巨大、难以普及且不环保的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的基础模型（如自然语言处理和计算机视觉领域）虽然前景广阔，但主要通过扩大模型规模和数据集容量来实现，这需要巨大的计算和数据资源，限制了其可及性，并与可持续人工智能原则相悖，导致巨大的碳足迹和资源效率低下。

Method: 本文提出了一种“专家集成”框架来构建遥感基础模型（RSFMs）。该方法将训练过程分解为轻量级的、任务特定的ConvNeXtV2专家模型，这些专家模型可以被冻结和重复使用。这种模块化方法还支持联邦训练、模型剪枝和持续的专家集成。

Result: 该框架在效率、可解释性和可扩展性方面具有显著优势。它特别适用于协作和资源受限的环境，并为构建可扩展且高效的RSFMs设定了新方向。

Conclusion: 该工作为构建可扩展和高效的遥感基础模型提供了一个新方向，有效解决了现有大型模型所面临的资源消耗和环境可持续性挑战。
...
### [134] [Harmony: Harmonizing Audio and Video Generation through Cross-Task Synergy](https://arxiv.org/abs/2511.21579)
*Teng Hu,Zhentao Yu,Guozhen Zhang,Zihan Su,Zhengguang Zhou,Youliang Zhang,Yuan Zhou,Qinglin Lu,Ran Yi*

Main category: cs.CV

TL;DR: 现有音视频同步生成模型面临对应漂移、低效注意力及模态内偏置等挑战。Harmony框架通过跨任务协同训练、全局局部解耦交互模块和同步增强型CFG，显著提升了生成保真度和音视频同步性，达到了新的SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 生成式AI中同步音视频内容的合成是一个关键挑战，开源模型在鲁棒的音视频对齐方面存在困难。分析表明，这源于联合扩散过程中的三个基本挑战：对应漂移、无法捕捉细粒度时间线索的低效全局注意力机制，以及传统分类器自由指导（CFG）的模态内偏置。

Method: Harmony框架通过以下方法强制实现音视频同步：1. 提出“跨任务协同训练”范式，利用音驱视频和视频驱音频生成任务的强监督信号来减轻对应漂移。2. 设计“全局局部解耦交互模块”，实现高效精确的时间-风格对齐。3. 引入“同步增强型CFG (SyncCFG)”，在推理过程中明确隔离并放大对齐信号。

Result: Harmony框架建立了新的最先进水平，在生成保真度和细粒度音视频同步方面均显著优于现有方法。

Conclusion: Harmony框架通过解决联合扩散过程中的核心挑战，有效地实现了音视频同步，并在生成质量和同步精度上取得了突破性进展。
...
### [135] [Enhanced Landmark Detection Model in Pelvic Fluoroscopy using 2D/3D Registration Loss](https://arxiv.org/abs/2511.21575)
*Chou Mo,Yehyun Suh,J. Ryan Martin,Daniel Moyer*

Main category: cs.CV

TL;DR: 针对骨盆透视中患者姿态可变的问题，本文提出了一种将2D/3D地标配准整合到U-Net训练中的新框架，以提高自动化地标检测在实际手术条件下的准确性。


<details>
  <summary>Details</summary>
Motivation: 目前的骨盆透视地标检测方法大多假设固定的前后位视角，但实际手术中由于成像单元或目标结构的变化，姿态经常偏离标准视角，这限制了现有方法的应用和准确性。

Method: 本文提出了一种新颖的框架，将2D/3D地标配准整合到U-Net地标预测模型的训练中。通过比较基线U-Net、使用姿态估计损失训练的U-Net以及使用姿态估计损失微调的U-Net之间的地标检测准确性，分析其在患者姿态可变的实际术中条件下的性能差异。

Result: 本文分析并比较了在患者姿态可变的真实术中条件下，基线U-Net、使用姿态估计损失训练的U-Net和使用姿态估计损失微调的U-Net在地标检测准确性方面的性能差异。

Conclusion: 该研究旨在通过引入结合2D/3D地标配准的U-Net训练框架，解决骨盆透视地标检测中固定视角假设的局限性，从而在患者姿态多变的实际手术环境中实现更准确的地标检测。
...
### [136] [UAVLight: A Benchmark for Illumination-Robust 3D Reconstruction in Unmanned Aerial Vehicle (UAV) Scenes](https://arxiv.org/abs/2511.21565)
*Kang Du,Xue Liao,Junpeng Xia,Chaozheng Guo,Yi Gu,Yirui Guan,Duotun Wang,ShengHuang,Zeyu Wang*

Main category: cs.CV

TL;DR: 多视角3D重建中光照不一致是一个重大挑战。本文引入UAVLight，一个受控且真实的基准数据集，用于在一致几何和视点下研究和评估对光照变化鲁棒的3D重建方法。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建（包括MVS、SfM和神经渲染）在光照不一致（如阳光方向、云层、阴影变化）的环境下会遇到几何漂移、颜色不一致和阴影印记等问题。这在无人机（UAV）重建中尤为关键。现有数据集要么光照变化不足，要么几何和语义变化混淆了对光照鲁棒性的独立研究。

Method: 引入UAVLight数据集，一个“受控但真实”的基准。每个场景通过可重复的、地理参考的飞行路径在一天中的多个固定时间进行捕获，以产生自然光照变化，同时保持几何、校准和视点的一致性。提供跨光照条件的标准化评估协议。

Result: UAVLight为开发和基准测试在真实户外环境中一致、真实且可重打光的重建方法提供了可靠的基础。它允许独立研究光照对3D重建的影响，避免了几何或语义变化带来的混淆。

Conclusion: UAVLight通过提供一个在几何和视点一致但光照条件多样的受控真实环境，解决了多视角3D重建中光照不一致的关键挑战，为开发和评估更鲁棒的重建方法奠定了基础。
...
### [137] [Active Learning for GCN-based Action Recognition](https://arxiv.org/abs/2511.21625)
*Hichem Sahbi*

Main category: cs.CV

TL;DR: 本文提出了一种标签高效的图卷积网络（GCN）模型，通过对抗性策略选择信息丰富的样本进行标注，并引入双向稳定的GCN架构，以解决骨架行为识别中标签数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 尽管图卷积网络（GCNs）在骨架行为识别中取得了显著成功，但其性能通常依赖于大量的标注数据，而这在实际应用中往往难以获得。

Method: 1. 开发了一种新颖的采集函数，采用对抗性策略来识别紧凑且信息丰富的样本进行标注，该选择过程平衡了代表性、多样性和不确定性。2. 引入了双向且稳定的GCN架构，以促进环境数据空间和潜在数据空间之间更有效的映射，从而更好地理解学习到的样本分布。

Result: 在两个具有挑战性的骨架行为识别基准测试中，所提出的标签高效GCNs相比现有工作取得了显著的性能提升。

Conclusion: 所提出的标签高效GCNs通过创新的样本选择策略和增强的网络架构，有效解决了骨架行为识别中标签数据稀缺的挑战，并显著提高了性能。
...
### [138] [MoGAN: Improving Motion Quality in Video Diffusion via Few-Step Motion Adversarial Post-Training](https://arxiv.org/abs/2511.21592)
*Haotian Xue,Qi Chen,Zhonghao Wang,Xun Huang,Eli Shechtman,Jinrong Xie,Yongxin Chen*

Main category: cs.CV

TL;DR: MoGAN是一个以运动为中心的后训练框架，旨在提高视频扩散模型的运动真实感。它通过训练一个光流判别器来区分真实和生成的运动，并结合分布匹配正则化器来保持视觉保真度，从而显著提升了视频的运动质量，且不牺牲视觉效果或效率。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在帧级保真度方面表现出色，但在运动连贯性、动态和真实感方面仍存在不足，常产生抖动、重影或不合理的动态。主要限制在于标准的去噪MSE目标没有直接的颞时一致性监督，导致模型即使在低损失下也可能生成糟糕的运动。

Method: MoGAN是一个基于3步蒸馏视频扩散模型的后训练框架。它训练了一个基于DiT的光流判别器来区分真实和生成的运动，并结合了一个分布匹配正则化器来保持视觉保真度。该框架无需奖励模型或人类偏好数据。

Result: 在Wan2.1-T2V-1.3B上的实验表明，MoGAN显著提升了运动质量。在VBench上，MoGAN的运动得分比50步教师模型提高了+7.3%，比3步DMD模型提高了+13.3%。在VideoJAM-Bench上，MoGAN的运动得分比教师模型提高了+7.4%，比DMD模型提高了+8.8%，同时保持了相当甚至更好的美学和图像质量得分。人类研究进一步证实，MoGAN在运动质量方面更受青睐（对比教师模型52% vs. 38%；对比DMD模型56% vs. 29%）。

Conclusion: MoGAN在不牺牲视觉保真度或效率的情况下，显著生成了更真实的运动，为快速、高质量的视频生成提供了一条实用途径。
...
### [139] [Deep Learning-Based Multiclass Classification of Oral Lesions with Stratified Augmentation](https://arxiv.org/abs/2511.21582)
*Joy Naoum,Revana Salama,Ali Hamdi*

Main category: cs.CV

TL;DR: 本研究利用深度学习构建了一个多类别口腔病变分类器，通过分层数据分割、数据增强和过采样解决了数据集限制和不平衡问题，在口腔癌早期诊断方面取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 口腔癌在全球范围内非常常见，但由于其与良性、癌前病变和恶性病变在视觉上高度相似，通常在晚期才被诊断出来。早期实施计算机辅助诊断系统具有显著改善临床结果的潜力。

Method: 本研究旨在利用深度学习构建一个针对十六种不同口腔病变的多类别分类器。为克服数据集有限和不平衡的挑战，所提出的技术结合了分层数据分割、先进的数据增强和过采样来执行分类。

Result: 实验结果显示，该模型取得了83.33%的准确率、89.12%的精确率和77.31%的召回率，证明了所建议模型优于现有最先进方法。该模型有效展示了过采样和增强策略在少数类别分类性能显著情况下的有效性。

Conclusion: 所提出的框架作为在临床环境中早期检测口腔癌的可信赖计算机辅助诊断系统的第一步，显示出巨大潜力，并强调了过采样和数据增强策略的有效性。
...
### [140] [CaFlow: Enhancing Long-Term Action Quality Assessment with Causal Counterfactual Flow](https://arxiv.org/abs/2511.21653)
*Ruisheng Han,Kanglei Zhou,Shuang Chen,Amir Atapour-Abarghouei,Hubert P. H. Shum*

Main category: cs.CV

TL;DR: CaFlow是一个统一的框架，通过结合反事实去混淆和双向时间条件流，解决了长期动作质量评估（AQA）中建模扩展时间动态和鲁棒性挑战，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 长期动作质量评估（AQA）在花样滑冰或艺术体操等领域应用广泛，但其挑战在于需要建模扩展的时间动态，同时对上下文混淆因素保持鲁棒性。现有方法要么依赖昂贵的标注，要么依赖单向时间建模，容易受到虚假关联和不稳定的长期表示的影响。

Method: 本文提出了CaFlow框架，它包含两个主要模块：因果反事实正则化（CCR）模块和BiT-Flow模块。CCR模块以自监督方式解耦因果和混淆特征，并通过反事实干预增强因果鲁棒性。BiT-Flow模块通过循环一致性约束建模前向和后向动态，以生成更平滑、更连贯的表示。

Result: 在多个长期AQA基准测试上进行的广泛实验表明，CaFlow取得了最先进的性能。

Conclusion: CaFlow通过集成反事实去混淆和双向时间条件流，有效解决了长期AQA的挑战，能够生成更鲁棒、更稳定的长期动作表示，并在实践中表现出色。
...
### [141] [The Age-specific Alzheimer 's Disease Prediction with Characteristic Constraints in Nonuniform Time Span](https://arxiv.org/abs/2511.21530)
*Xin Hong,Kaifeng Huang*

Main category: cs.CV

TL;DR: 本研究提出一种创新方法，通过整合定量指标和年龄缩放因子，生成年龄特异性MRI图像，以解决阿尔茨海默病预测中不规则时间间隔图像生成不准确的问题，并显著提高了图像合成的准确性和长期预后相似性。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病（AD）的早期识别对制定个性化治疗策略至关重要。然而，使用生成图像进行AD预测面临挑战，尤其是在输入序列捕获时间不规律时，难以准确表示疾病特征。

Method: 本研究提出一种创新的序列图像生成方法，该方法由定量指标引导，以保持疾病进展的关键特征。此外，将年龄缩放因子整合到生成过程中，以生成年龄特异性的MRI图像，从而促进对疾病晚期的预测。

Result: 消融研究结果表明，纳入定量指标显著提高了MRI图像合成的准确性。此外，应用年龄缩放像素损失有助于增强MRI图像的迭代生成。在长期疾病预后方面，结构相似性指数（SSIM）达到0.882的峰值，表明合成图像具有高度相似性。

Conclusion: 该方法通过结合定量指标和年龄缩放因子，有效地生成了年龄特异性MRI图像，能够准确表示疾病进展特征，并为长期疾病预后提供了高度相似的合成图像，有助于阿尔茨海默病的预测。
...
### [142] [ReSAM: Refine, Requery, and Reinforce: Self-Prompting Point-Supervised Segmentation for Remote Sensing Images](https://arxiv.org/abs/2511.21606)
*M. Naseer Subhani*

Main category: cs.CV

TL;DR: 该论文提出了一种自提示、点监督的框架，通过一个“细化-再查询-强化”循环，利用稀疏点注释将SAM模型高效地适应于遥感图像，以解决领域偏移和密集注释稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 交互式分割模型（如SAM）在自然图像上泛化能力强，但在遥感图像（RSI）上表现不佳，原因在于严重的领域偏移和密集注释的稀缺性。

Method: 该方法采用一个自提示、点监督的框架，通过“细化-再查询-强化”循环来适应SAM。具体步骤包括：从初始点生成粗糙伪掩码（细化），通过自构建的框提示改进掩码（再查询），以及在迭代过程中对齐嵌入以减少确认偏差（强化）。整个过程不依赖于完整掩码监督。

Result: 该方法在WHU、HRSID和NWPU VHR-10三个遥感图像基准数据集上进行了评估，结果表明其性能始终优于预训练的SAM模型和最近的点监督分割方法。

Conclusion: 研究结果表明，自提示和语义对齐为遥感应用中基础分割模型的点级可扩展适应提供了一条高效的途径。
...
### [143] [Seeing without Pixels: Perception from Camera Trajectories](https://arxiv.org/abs/2511.21681)
*Zihui Xue,Kristen Grauman,Dima Damen,Andrew Zisserman,Tengda Han*

Main category: cs.CV

TL;DR: 本文首次系统性地研究了仅通过摄像机轨迹而非像素来感知视频内容的可行性，并提出CamFormer模型证明摄像机轨迹是揭示视频内容的高度信息丰富的信号。


<details>
  <summary>Details</summary>
Motivation: 研究一个看似不可能的问题：是否可以在不查看像素的情况下，仅通过摄像机在空间中移动的路径（即摄像机轨迹）来感知视频内容。探索“如何移动”能否揭示“正在做什么”或“正在观察什么”。

Method: 提出一个对比学习框架，用于训练CamFormer，这是一个专门的编码器，它将摄像机姿态轨迹投影到一个联合嵌入空间中，并将其与自然语言对齐。

Result: 研究发现，摄像机轨迹是一个非常丰富的信息信号，能够揭示视频内容。学习到的CamFormer嵌入在各种下游任务（如跨模态对齐、分类和时间分析）上表现出多功能性，并且对不同的摄像机姿态估计方法（包括高保真多传感器和标准纯RGB估计器）都具有鲁棒性。

Conclusion: 摄像机轨迹被确立为一种轻量级、鲁棒且多功能的模态，可用于感知视频内容。
...
### [144] [Multi-Crit: Benchmarking Multimodal Judges on Pluralistic Criteria-Following](https://arxiv.org/abs/2511.21662)
...
TL;DR: 本研究引入了Multi-Crit基准，用于评估大型多模态模型（LMMs）作为多模态评判者在遵循多样化、细粒度评估标准方面的能力。结果显示，现有LMMs（包括专有和开源模型）在处理多标准判断时存在显著局限性，特别是在开放式评估中，且批评性微调未能有效提升其对细粒度标准的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型（LMMs）因其强大的指令遵循能力和与人类偏好的一致性而被广泛用作多模态评估系统中的评判者，但它们遵循多样化、细粒度评估标准的能力尚未得到充分探索。因此，需要一个基准来评估LMMs遵循多标准并生成可靠的准则级判断的能力。

Method: 研究开发了Multi-Crit基准，涵盖开放式生成和可验证推理任务。该基准通过严格的数据整理流程构建，收集了具有多准则人类标注的挑战性响应对。此外，引入了三个新颖的度量指标，用于系统评估多元依从性、准则切换灵活性以及识别准则级偏好冲突的能力。研究对25个LMMs进行了综合分析。

Result: 分析25个LMMs的结果表明：1) 专有模型在保持对多元准则的一致依从性方面仍存在困难，尤其是在开放式评估中；2) 开源模型在灵活遵循多样化准则方面落后更多；3) 使用整体判断信号进行的批评性微调虽然增强了视觉基础，但未能泛化到多元准则级判断。额外的分析进一步揭示了当前多模态评判者的局限性。

Conclusion: Multi-Crit作为一项开创性研究，为构建可靠且可控的多模态AI评估奠定了基础。当前的大型多模态模型在作为细粒度多模态评判者方面仍存在显著局限性，尤其是在处理多样化和细致的评估标准时。
...
### [145] [Revolutionizing Glioma Segmentation & Grading Using 3D MRI - Guided Hybrid Deep Learning Models](https://arxiv.org/abs/2511.21673)
*Pandiyaraju V,Sreya Mynampati,Abishek Karthik,Poovarasan L,D. Saraswathi*

Main category: cs.CV

TL;DR: 该研究提出了一种混合深度学习模型，结合U-Net分割和带有注意力机制的DenseNet-VGG分类网络，用于胶质瘤的精确分割和高精度分类诊断，表现优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 胶质瘤死亡率高，早期准确诊断对治疗干预至关重要，但实现难度大。

Method: 研究开发了一个混合深度学习模型：1) 基于U-Net的分割模型，用于3D MRI数据的肿瘤精确分割，结合空间和上下文信息；2) 混合DenseNet-VGG分类网络，融入多头注意力和空间-通道注意力机制，聚焦于临床相关特征。模型通过归一化、重采样和数据增强等预处理步骤处理高维3D MRI数据。性能评估指标包括Dice系数、平均IoU（分割）以及准确率、精确率、召回率和F1分数（分类）。

Result: 所提出的混合框架在肿瘤分割方面达到了98%的Dice系数，在分类准确性方面达到了99%，优于传统的CNN模型和无注意力机制的方法。利用多头注意力机制增强了对临床重要肿瘤特征的优先级关注，提高了可解释性和准确性。

Conclusion: 该框架在促进临床医生及时可靠地诊断和分级胶质瘤方面显示出巨大潜力，有望改善患者的治疗计划。
...
### [146] [Canvas-to-Image: Compositional Image Generation with Multimodal Controls](https://arxiv.org/abs/2511.21691)
...
TL;DR: 本文提出了Canvas-to-Image框架，通过将多种异构控制信号（如文本、主题、空间、姿态和布局）整合到单一画布界面和进行多任务训练，显著提升了扩散模型在生成高保真、多模态控制图像方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现代扩散模型在生成高质量和多样化图像方面表现出色，但在处理高保真度的组合和多模态控制时仍面临挑战，尤其当用户需要同时指定文本提示、主题参考、空间布局、姿态约束和布局标注时。

Method: 该研究引入了Canvas-to-Image框架，将异构控制整合到单一画布界面。其核心思想是将多样化的控制信号编码成一个复合画布图像，供模型直接解释以进行集成视觉-空间推理。此外，作者策划了一套多任务数据集，并提出了多任务画布训练策略，以优化扩散模型，使其在统一学习范式下共同理解和整合异构控制到文本到图像生成中。

Result: Canvas-to-Image在身份保留和控制依从性方面显著优于现有最先进方法。它在多人物组合、姿态控制组合、布局约束生成和多控制生成等具有挑战性的基准测试中表现出色。

Conclusion: Canvas-to-Image通过统一的框架和多任务训练策略，成功解决了扩散模型在处理复杂、多模态控制方面的难题，能够生成忠实反映用户意图的高保真图像，并在多控制场景下表现出良好的泛化能力和卓越的性能。
...
### [147] [Democratizing LLM Efficiency: From Hyperscale Optimizations to Universal Deployability](https://arxiv.org/abs/2511.20662)
*Hen-Hsen Huang*

Main category: cs.CL

TL;DR: 当前LLM效率方法仅适用于超大规模提供商，本文呼吁转向“稳健的简洁性”，提出新的研究议程，以实现LLM部署的民主化，降低不平等和碳排放。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（LLM）效率方法，如MoE、推测解码和复杂RAG，是为拥有庞大基础设施和精英团队的超大规模提供商设计的。在其他环境下，这些方法会变成开销、脆弱性和碳浪费，导致只有少数大型科技公司受益，而其他机构（医院、学校、政府、企业）缺乏可行的选择。

Method: 本文提出了一项新的研究议程，而非具体实施的方法，旨在实现“稳健的简洁性”。该议程包括：在不重新训练的情况下，用更高效的架构改造预训练模型；发明轻量级微调以保持对齐；使长链思维推理经济化；在没有重型RAG管道的情况下实现动态知识管理；以及采用“开销感知效率”（OAE）作为标准基准。

Result: 通过重新定义效率，将采用成本、可持续性和公平性纳入考量，本文旨在实现LLM部署的民主化，确保优化能够减少而非放大不平等和碳浪费。

Conclusion: 当前的LLM效率方法加剧了不平等和资源浪费。未来的前沿不应是更大规模的复杂性，而应是“稳健的简洁性”，即在有限资源和专业知识下也能高效运行。通过采纳新的研究议程和OAE基准，可以使LLM部署更具包容性和可持续性。
...
### [148] [Harmonic Token Projection (HTP): A Vocabulary-Free, Training-Free, Deterministic, and Reversible Embedding Methodology](https://arxiv.org/abs/2511.20665)
*Tcharlies Schmitz*

Main category: cs.CL

TL;DR: 本文提出谐波令牌投影（HTP），一个无需训练、词汇或随机参数的文本嵌入框架。它将Unicode整数表示转换为谐波轨迹，实现可逆且确定性的映射，并在多语言语义相似性任务上表现出色，计算成本极低。


<details>
  <summary>Details</summary>
Motivation: 现有神经嵌入依赖统计共现或优化，需要训练数据、词汇表和随机参数。研究旨在提供一种透明、高效、无需训练、确定性且可解释的文本嵌入替代方案。

Method: HTP将每个令牌解析地编码为其Unicode整数表示导出的谐波轨迹，建立离散符号与连续向量空间之间的双射且可解释的映射。语义相似性通过纯粹的几何对齐来估计，利用谐波公式提供保持结构和可逆性的相位相关投影。

Result: 在语义文本相似性基准（STS-B）及其多语言扩展上，HTP在英语中实现了ρ = 0.68的Spearman相关性，并在十种语言中保持了稳定的性能。该方法计算成本可忽略不计，每对句子延迟低于毫秒。

Conclusion: 研究表明，有意义的语义关系可以从确定性几何中产生，为数据驱动的嵌入提供了一种透明且高效的替代方案。
...
### [149] [A centroid based framework for text classification in itsm environments](https://arxiv.org/abs/2511.20667)
*Hossein Mohanna,Ali Ait-Bachir*

Main category: cs.CL

TL;DR: 本文提出了一种双嵌入质心分类框架，用于ITSM系统中的分层文本分类，该框架通过语义和词汇质心表示结合倒数排名融合，实现了与SVM相当的性能，并提供了可解释性和显著的训练及增量更新速度提升。


<details>
  <summary>Details</summary>
Motivation: IT服务管理（ITSM）系统中，支持工单必须根据树形分类法进行分类，因此对分层分类的需求是核心且基本的要求。

Method: 采用双嵌入质心分类框架，为每个类别维护独立的语义和词汇质心表示，并在推理时通过倒数排名融合（reciprocal rank fusion）将它们结合。该方法在8,968张ITSM工单和123个类别上进行了评估。

Result: 该方法在分层F1分数上与支持向量机（SVM）具有竞争力（0.731 vs 0.727），训练速度快5.9倍，增量更新速度快152倍，并且在排除嵌入计算的情况下，批处理速度（100-1000样本）提升了8.6-8.8倍。

Conclusion: 该方法因其可解释性和操作效率，非常适合需要优先考虑这些因素的生产ITSM环境。
...
### [150] [PIRA: Preference-Oriented Instruction-Tuned Reward Models with Dual Aggregation](https://arxiv.org/abs/2511.20668)
*Yongfu Xue*

Main category: cs.CL

TL;DR: PIRA是一种针对奖励模型训练的范式，通过将问答对重构为指令、聚合多源奖励以及平均不同dropout下的价值头输出来解决数据效率低和奖励过度优化的问题。


<details>
  <summary>Details</summary>
Motivation: 奖励模型在使大型语言模型与人类偏好对齐方面至关重要，但面临两个挑战：传统的判别式奖励模型直接拼接问答对作为输入，导致数据效率低下；奖励模型容易受到奖励过度优化的影响。

Method: PIRA通过三种策略解决问题：1) 将问答对重构为基于偏好的指令，以实现更清晰、更明确的任务规范；2) 聚合来自不同偏好任务的奖励，以减少偏差并提高鲁棒性；3) 在不同的dropout率下平均价值头输出，以稳定奖励。

Result: 广泛的实验证明了PIRA的有效性。

Conclusion: PIRA通过其提出的三种策略，成功提升了奖励模型的数据效率、鲁棒性并有效缓解了奖励过度优化的问题。
...
### [151] [Structured Definitions and Segmentations for Legal Reasoning in LLMs: A Study on Indian Legal Data](https://arxiv.org/abs/2511.20669)
*Mann Khatri,Mirza Yusuf,Rajiv Ratn Shah,Ponnurangam Kumaraguru*

Main category: cs.CL

TL;DR: 大型语言模型（LLMs）在法律等专业领域表现不佳，本研究通过重组文档、定义修辞角色和模拟法院推理，在零样本设置下显著提升了LLMs在印度法律判决预测任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）具有出色的通用推理能力，但由于缺乏领域特定预训练，它们在法律等专业领域表现不力。法律文档冗长复杂，难以有效处理。现有研究尝试通过上下文学习弥补知识差距，但本研究旨在探索更具体的策略来解决法律领域的挑战。

Method: 本研究在零样本设置下，通过三项实验分析了模型在法律任务中的行为：(i) 根据修辞角色重组文档，评估结构化信息对长文本处理和模型决策的影响；(ii) 定义修辞角色，使模型熟悉法律术语；(iii) 模拟法院关于修辞角色的逐步推理过程，以增强模型推理能力。实验在三个印度法律判决预测数据集上进行。

Result: 实验结果表明，组织数据或解释关键法律术语能显著提升模型性能。与基线相比，F1分数至少增加了约1.5%，最高提高了4.36%。

Conclusion: 通过在上下文中重组法律信息和解释关键法律术语，可以有效提升大型语言模型在法律任务上的表现，即使在缺乏领域特定预训练的情况下也能取得显著改进。
...
### [152] [MindSET: Advancing Mental Health Benchmarking through Large-Scale Social Media Data](https://arxiv.org/abs/2511.20672)
*Saad Mankarious,Ayah Zirikly,Daniel Wiechmann,Elma Kerz,Edward Kempa,Yu Qiao*

Main category: cs.CL

TL;DR: 本文提出了一个名为MindSET的新型大规模心理健康基准数据集，该数据集从Reddit收集，包含超过1300万条注释帖子，用于七种心理健康状况的分析，并通过实验证明其在诊断检测任务中优于现有基准。


<details>
  <summary>Details</summary>
Motivation: 社交媒体数据是研究心理健康的重要资源，但现有的大多数基准数据集因数据可用性有限、清洗不足以及社交媒体内容固有的多样性（如多语言和有害内容）而变得过时，无法满足研究需求。

Method: 研究者从Reddit上利用自我报告的诊断策劃并构建了MindSET数据集，包含七种心理健康状况的超过1300万条注释帖子。为确保数据质量，采用了严格的预处理步骤，包括语言过滤、移除NSFW（不适宜工作场所）内容和重复内容。此外，使用LIWC进行了语言学分析，并使用微调语言模型和词袋（BoW）特征进行了二元分类实验，以检测诊断。

Result: MindSET数据集的规模是之前基准的两倍多，涵盖七种心理健康状况。在诊断检测的二元分类实验中，基于MindSET训练的模型表现始终优于基于先前基准训练的模型，其中自闭症检测的F1分数提高了多达18点。

Conclusion: MindSET数据集为研究社交媒体与心理健康的交叉领域提供了一个强大的基础，支持早期风险检测和对新兴心理趋势的深入分析。
...
### [153] [Semantics Meet Signals: Dual Codebook Representationl Learning for Generative Recommendation](https://arxiv.org/abs/2511.20673)
*Zheng Hui,Xiaokai Wei,Reza Shirkavand,Chen Wang,Weizhi Zhang,Alejandro Peláez,Michelle Gong*

Main category: cs.CL

TL;DR: 现有生成式推荐系统采用单一码本，忽略了热门商品和长尾商品之间的差异。FlexCode提出一种流行度感知框架，通过协同过滤码本和语义码本的自适应分配，并结合MoE动态平衡精确性和泛化性，显著提升了推荐准确性和长尾鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐模型依赖单一、统一的码本对所有物品进行编码，这忽略了热门物品（富含协同信号）和长尾物品（依赖语义理解）之间的固有不平衡。这种统一处理限制了表示效率并阻碍了泛化能力。

Method: 本文提出FlexCode，一个流行度感知框架。它自适应地将固定的token预算分配给协同过滤（CF）码本和语义码本。一个轻量级的MoE（专家混合）模型动态平衡CF特异性精度和语义泛化能力。同时，通过对齐和平滑目标来维持整个流行度谱的连贯性。

Result: 在公共和工业级数据集上的实验表明，FlexCode持续优于强大的基线模型。它实现了更高的准确性和更强的长尾鲁棒性。

Conclusion: FlexCode为生成式推荐器中的token表示提供了一种新机制，实现了更强的准确性和长尾鲁棒性，并为基于token的推荐模型中记忆和泛化的平衡提供了新视角。
...
### [154] [Prompt Engineering Techniques for Context-dependent Text-to-SQL in Arabic](https://arxiv.org/abs/2511.20677)
*Saleh Almohaimeed,May Alsofyani,Saad Almohaimeed,Mansour Al Ghanim,Liqiang Wang*

Main category: cs.CL

TL;DR: 本文介绍了首个阿拉伯语跨领域、上下文相关的文本到SQL数据集Ar-SParC，并使用大型语言模型和一种新颖的GAT校正器进行了实验，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 尽管跨领域、上下文相关的文本到SQL任务受到了广泛关注，并且能让不懂SQL的用户通过自然语言与数据库交互，但现有的大多数数据集和研究都集中在英语和部分中文上，阿拉伯语领域仍是空白。

Method: 1. 构建了首个阿拉伯语跨领域、上下文相关的文本到SQL数据集Ar-SParC，包含3450个相互关联的问题序列（共10225个问题及其SQL查询）。 2. 使用GPT-3.5-turbo和GPT-4.5-turbo两种大型语言模型，结合10种不同的提示工程技术（包括4种问题表示方法和6种上下文学习技术），在Ar-SParC数据集上进行了40项实验。 3. 开发了一种名为GAT校正器的新方法。 4. 进行了消融研究，通过额外实验解释了GAT校正器优于现有GAT验证器技术的原因，特别是在阿拉伯语方面。

Result: 1. GAT校正器在所有40项实验中均提升了性能。 2. 在零样本设置下，平均执行准确率（EX）和交互准确率（IX）均提高了1.9%。 3. 在上下文学习设置下，平均执行准确率（EX）提高了1.72%，交互准确率（IX）提高了0.92%。 4. GAT校正器在阿拉伯语任务上表现优于先前的GAT验证器技术。

Conclusion: 本文成功构建了首个阿拉伯语跨领域、上下文相关的文本到SQL数据集Ar-SParC，并提出了一种有效的GAT校正器，该校正器在大型语言模型上显著提升了该任务的性能，尤其适用于阿拉伯语。
...
### [155] [Cognitive bias in LLM reasoning compromises interpretation of clinical oncology notes](https://arxiv.org/abs/2511.20680)
...
TL;DR: 尽管大型语言模型在临床基准测试中表现出色，但其可能通过错误的推理得出正确结论，这在肿瘤决策支持中存在安全隐患。本研究开发了一个推理错误分类法，发现23%的解释存在推理错误，尤其在晚期疾病管理中可能导致不安全的建议。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在临床任务中表现出高准确性，但可能通过错误的推理路径达到正确结论。这种“错误推理”模式对肿瘤决策支持具有安全隐患，且无法通过传统的基于准确性的评估方法捕获。因此，需要一种方法来识别和评估LLMs的推理缺陷。

Method: 本研究采用回顾性双队列设计。首先，从GPT-4对真实肿瘤病历的链式思维（chain-of-thought）响应中，开发了一个分层推理错误分类法。然后，使用来自CORAL数据集的乳腺癌和胰腺癌病历，标注了600个推理轨迹，定义了一个将计算失败映射到认知偏差框架的三层分类法。最后，在涵盖局限性至转移性疾病的前列腺癌会诊病历的822个响应上验证了该分类法，模拟了信息提取、分析和临床推荐任务。同时，测试了使用先进语言模型的自动化评估器检测错误的能力。

Result: 研究发现，23%的解释中存在推理错误，并且这些错误在所有错误中占主导地位，其中确认偏差和锚定偏差最为常见。推理失败与不符合指南的、潜在有害的建议相关，尤其是在晚期疾病管理中。自动化评估器能够检测到错误的存在，但无法可靠地分类错误子类型。

Conclusion: 研究得出结论，大型语言模型在推理存在缺陷时，可能提供流畅但临床不安全的建议。本研究开发的分类法提供了一个可推广的框架，用于在临床部署前评估和改进模型的推理保真度。
...
### [156] [LLMs-Powered Accurate Extraction, Querying and Intelligent Management of Literature derived 2D Materials Data](https://arxiv.org/abs/2511.20691)
*Lijun Shang,Yadong Yu,Wenqiang Kang,Jian Zhou,Dongyue Gao,Pan Xiang,Zhe Liu,Mengyan Dai,Zhonglu Guo,Zhimei Sun*

Main category: cs.CL

TL;DR: 二维材料在能源存储与转换领域应用广泛，但其重要信息分散在大量已发表的论文中。


<details>
  <summary>Details</summary>
Motivation: 二维材料因其独特的理化和电子特性，在能源存储和转换方面展现出广泛应用。然而，有关这些材料的宝贵信息（如性质和制备方法）分散在众多已发表的研究论文中，导致信息获取和整合面临挑战。

Method: 提供的摘要不完整，未提及具体研究方法。

Result: 提供的摘要不完整，未包含研究结果。

Conclusion: 提供的摘要不完整，无法得出明确结论。
...
### [157] [Memories Retrieved from Many Paths: A Multi-Prefix Framework for Robust Detection of Training Data Leakage in Large Language Models](https://arxiv.org/abs/2511.20799)
*Trung Cuong Dang,David Mohaisen*

Main category: cs.CL

TL;DR: 大型语言模型存在训练数据逐字记忆问题，带来隐私和版权风险。本文提出“多前缀记忆”框架，通过衡量能引发特定序列的不同前缀数量来定义和检测记忆，实验证明其能有效区分记忆和非记忆数据。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）容易逐字记忆训练数据，导致严重的隐私和版权风险。现有关于记忆的定义存在不足，未能全面捕捉这种现象，尤其是在对齐模型中。

Method: 引入“多前缀记忆”新框架。核心思想是记忆序列被深度编码，可以通过比非记忆内容多得多的不同前缀检索。形式化定义为：如果外部对抗性搜索能识别出目标数量的不同前缀来引发某个序列，则该序列被认为是记忆的。此框架将重点从单路径提取转向量化记忆的鲁棒性，通过检索路径的多样性来衡量。

Result: 通过在开源和对齐聊天模型上的实验表明，多前缀定义能够可靠地区分记忆数据和非记忆数据。

Conclusion: 多前缀框架为审计大型语言模型中的数据泄露提供了一个鲁棒且实用的工具。
...
### [158] [Dynamic Template Selection for Output Token Generation Optimization: MLP-Based and Transformer Approaches](https://arxiv.org/abs/2511.20683)
*Bharadwaj Yadavalli*

Main category: cs.CL

TL;DR: 本文提出动态模板选择（DTS）方法，通过自适应匹配响应模板与查询复杂性，显著降低大型语言模型（LLM）的令牌成本，同时保持响应质量。


<details>
  <summary>Details</summary>
Motivation: 当前LLM部署普遍采用统一的提示策略，对复杂分析和简单事实问题都使用冗长的响应模式，导致严重的令牌效率低下。鉴于输出令牌成本远高于输入令牌（高出4-8倍），这种“一刀切”的方法带来了巨大的成本问题。

Method: 研究提出了动态模板选择（DTS）方法，通过路由机制根据查询复杂性自适应选择响应模板。文中比较了两种路由方法：一种是使用预计算嵌入的简单多层感知器（MLP），另一种是更复杂的微调RoBERTa转换器。该方法通过正式的问题建模、理论基础和四种算法进行支持。

Result: 在1,000个MMLU问题上的评估显示，MLP路由器在测试数据上达到了90.5%的路由准确率，略高于RoBERTa（89.5%），尽管参数量少了1.25亿。经验分析表明，模板选择行为与提供商无关，路由决策在三大LLM提供商（OpenAI GPT-4、Google Gemini、Anthropic Claude）之间有效泛化。路由准确率保持在90.5%，而观察到的令牌减少量在32.6%到33.9%之间，这反映了提供商特定的生成特性。

Conclusion: 该工作通过动态模板选择（DTS）方法，有效解决了LLM部署中的令牌效率和高成本问题。DTS方法在不影响响应质量的前提下，实现了显著的成本降低，并被证明在不同LLM提供商之间具有良好的泛化能力和鲁棒性。
...
### [159] [SAGE: An Agentic Explainer Framework for Interpreting SAE Features in Language Models](https://arxiv.org/abs/2511.20820)
*Jiaojiao Han,Wujiang Xu,Mingyu Jin,Mengnan Du*

Main category: cs.CL

TL;DR: SAGE是一个基于代理的框架，通过将特征解释重构为主动、解释驱动的过程，显著提高了大型语言模型（LLM）中稀疏自编码器（SAE）特征的解释准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）的内部机制不透明，阻碍了其安全可靠的部署。尽管稀疏自编码器（SAE）能够将LLM表示分解为可解释的特征，但解释SAE捕获的特征仍然是一项具有挑战性的任务。

Method: 该研究提出了SAGE（SAE AGentic Explainer），一个基于代理的框架，将特征解释从被动、单次生成任务转变为主动、解释驱动的过程。SAGE通过系统地为每个特征制定多种解释、设计有针对性的实验进行测试，并根据经验激活反馈迭代地完善解释，实现了一套严谨的方法论。

Result: 在来自不同语言模型的SAE特征上进行的实验表明，与现有最先进的基线相比，SAGE生成的解释具有显著更高的生成和预测准确性。

Conclusion: SAGE提供了一种更有效、更严谨的方法来解释LLM中SAE的特征，从而提高了对LLM内部机制的理解和透明度。
...
### [160] [Structured Prompting Enables More Robust, Holistic Evaluation of Language Models](https://arxiv.org/abs/2511.20836)
...
TL;DR: 现有基准测试（如HELM）因使用固定提示而低估了语言模型（LM）的性能。本文提出了DSPy+HELM框架，通过引入结构化提示（特别是能激发推理的提示），实现了更准确的LM性能评估，发现传统方法平均低估4%的性能，并扭曲了排行榜。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型广泛应用，需要高质量的基准测试框架来准确评估其性能。现有框架（如HELM）依赖固定提示，导致性能估计不具代表性，可能低估了LM的真实性能上限。声明式提示框架（如DSPy）虽可扩展，但尚未在既定基准上系统评估。

Method: 本文提出了一个可复现的DSPy+HELM框架，将结构化提示方法（特别是能引发推理的方法）引入到HELM中，以实现更准确的LM基准测试。研究使用四种提示方法，在七个基准（通用/医学领域）上评估了四个前沿LM，并与现有的HELM基线分数进行了比较。

Result: 研究发现，如果没有结构化提示：(i) HELM平均低估LM性能4%；(ii) 性能估计在不同基准上的差异更大（标准差增加2%）；(iii) 性能差距被错误呈现（3/7的基准上排行榜排名发生变化）；(iv) 引入推理（思维链）能降低LM对提示设计的敏感性（提示间差异更小）。这是首次大规模实证研究LM在不同基准和提示方法下的行为。

Conclusion: 通过可扩展的性能上限估计（例如使用DSPy+HELM框架中的结构化提示），可以实现更具决策参考价值的基准测试。现有基准测试在没有结构化提示的情况下，会低估LM性能并误导性能排名。
...
### [161] [Length-MAX Tokenizer for Language Models](https://arxiv.org/abs/2511.20849)
*Dong Dong,Weijie Su*

Main category: cs.CL

TL;DR: 本文介绍了一种名为Length-MAX的新型分词器，通过最小化平均字符令牌数来优化令牌化过程。它显著减少了训练和推理所需的令牌数量，从而提高了语言模型的效率，并在下游任务中表现出更好的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是为了减少文本在语言模型训练和推理过程中所需的令牌数量，从而提高效率。现有分词器（如BPE）在令牌效率方面可能不是最优的，因此需要一种新的方法来最小化每个字符的平均令牌数。

Method: 本文引入了Length-MAX分词器，其词汇表通过将长度加权目标最大化问题转化为图划分问题来获取，并开发了一种贪婪近似算法来解决该问题。

Result: Length-MAX分词器在FineWeb和其他领域，与BPE相比，令牌数量减少了14-18%（词汇量为10K-50K时）和13.0%（词汇量为64K时）。在GPT-2模型上，达到固定验证损失所需的训练步数减少了17-18.5%，推理延迟降低了12.7-13.7%，吞吐量提高了16%。同时，它改进了下游任务表现，例如LAMBADA困惑度降低11.7%，HellaSwag准确率提高4.3%。此外，它实现了99.62%的词汇覆盖率和0.12%的低词外率，并在推理时将嵌入和KV缓存内存减少了18%。

Conclusion: 优化平均令牌长度（而不仅仅是频率）是一种有效的方法，可以提高语言模型的效率，同时不牺牲（甚至提升）下游任务的性能。Length-MAX分词器与生产系统兼容，并能有效减少内存占用。
...
### [162] [Evo-Memory: Benchmarking LLM Agent Test-time Learning with Self-Evolving Memory](https://arxiv.org/abs/2511.20857)
...
TL;DR: 本文介绍了Evo-Memory，一个用于评估大型语言模型（LLM）代理自进化记忆的基准和框架，以解决其在连续任务流中积累和重用经验的能力不足的问题，并提出了ReMem管道以实现持续改进。


<details>
  <summary>Details</summary>
Motivation: LLM代理在长期规划和问题解决中需要状态性，使记忆成为关键组成部分，但其管理和演化尚未得到充分探索。现有评估多集中于静态对话，忽略了在动态任务流中积累和重用经验的能力。LLMs常无法从累积交互中学习，失去宝贵的上下文洞察，这促使研究在部署过程中持续检索、整合和更新记忆。

Method: 本文引入了Evo-Memory，一个综合的流式基准和框架，用于评估LLM代理的自进化记忆。Evo-Memory将数据集构建为顺序任务流，要求LLM在每次交互后搜索、适应和进化记忆。作者统一并实现了十多种代表性记忆模块，并在10个多样的多轮目标导向和单轮推理及问答数据集中进行了评估。为更好地基准化经验重用，提供了ExpRAG作为基线方法，并提出了ReMem，一个紧密整合推理、任务行动和记忆更新的“行动-思考-记忆精炼”管道，以实现持续改进。

Result: 本文的主要成果是创建了Evo-Memory基准和框架，它能够全面评估LLM代理在连续任务流中自进化记忆的能力。同时，提出了ExpRAG基线方法和ReMem管道，后者通过整合推理、行动和记忆更新，旨在实现LLM代理在动态环境中的持续学习和改进。

Conclusion: Evo-Memory基准和ReMem管道共同弥补了当前LLM代理记忆管理和演化评估的空白，为在连续、动态任务环境中实现LLM代理的自进化记忆和持续改进提供了全面的评估工具和有效的方法。
...
### [163] [Winning with Less for Low Resource Languages: Advantage of Cross-Lingual English_Persian Argument Mining Model over LLM Augmentation](https://arxiv.org/abs/2511.20872)
*Ali Jahan,Masood Ghayoomi,Annette Hautli-Janisz*

Main category: cs.CL

TL;DR: 该研究旨在通过构建三种训练场景，利用跨语言方法解决低资源语言的论证挖掘问题，并发现轻量级跨语言混合模型表现优于资源密集型数据增强方法。


<details>
  <summary>Details</summary>
Motivation: 论证挖掘是自然语言处理中的一个重要子领域，能揭示文本的逻辑结构，但低资源语言在此任务上面临数据匮乏的挑战。

Method: 研究在英语（高资源语言）和波斯语（低资源语言）上进行模型评估，使用英语Microtext语料库及其波斯语并行翻译。构建了三种学习场景：(i) 零样本迁移（仅用英语数据训练），(ii) 英语训练结合大型语言模型（LLMs）生成的合成示例进行增强，以及(iii) 结合原始英语数据和手动翻译的波斯语句子的跨语言模型。

Result: 零样本迁移模型在英语测试集上F1分数为50.2%，在波斯语测试集上为50.7%。基于LLM的数据增强模型将英语性能提升至59.2%，波斯语提升至69.3%。跨语言模型（在两种语言上训练，仅在波斯语测试集上评估）达到了74.8%的F1分数，超越了基于LLM的变体。

Conclusion: 结果表明，轻量级的跨语言混合方法可以显著优于更资源密集的数据增强流程，为低资源语言的论证挖掘任务克服数据资源短缺提供了一条实用的途径。
...
### [164] [Emergence and Localisation of Semantic Role Circuits in LLMs](https://arxiv.org/abs/2511.20910)
*Nura Aljaafari,Danilo S. Carvalho,André Freitas*

Main category: cs.CL

TL;DR: 本研究通过结合最小对、时间演化分析和跨模型比较，揭示了大型语言模型（LLMs）实现抽象语义角色的内部机制，发现这些机制紧凑、因果独立，并能在不同尺度和架构间部分迁移。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型展现出语义能力，但其内部机制如何扎根抽象语义结构仍未得到充分描述。

Method: 本研究提出了一种集成方法，包括角色交叉最小对（role-cross minimal pairs）、时间涌现分析（temporal emergence analysis）和跨模型比较（cross-model comparison），以研究LLMs如何实现语义角色。

Result: 分析揭示了：(i) 高度集中的电路（89-94%的归因集中在28个节点内）；(ii) 结构逐渐细化而非相变，较大的模型有时会绕过局部电路；以及 (iii) 中等的跨尺度保守性（24-59%的组件重叠）和高度的光谱相似性。

Conclusion: 这些发现表明，LLMs为抽象语义结构形成了紧凑、因果独立的机制，并且这些机制在不同尺度和架构之间表现出部分迁移性。
...
### [165] [TrackList: Tracing Back Query Linguistic Diversity for Head and Tail Knowledge in Open Large Language Models](https://arxiv.org/abs/2511.21006)
*Ioana Buhnila,Aman Sinha,Mathieu Constant*

Main category: cs.CL

TL;DR: 本研究评估了大型语言模型（LLMs）在处理除定义型查询之外的其他语言查询时的性能下降问题，发现其在定义型问题上表现最佳，在例证型问题上表现最差，并受概念频率影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在提供定义型答案方面表现高效，但与人类不同，它们在提供示例、释义等其他类型答案时表现不佳，研究旨在探究这种性能下降的原因和程度。

Method: 研究采用TrackList管道进行细粒度的语言和统计分析，以调查预训练数据对LLMs回答多样化语言查询的影响。同时引入了RefoMed-EN数据集（包含6170个医学术语及其定义、称谓、例证、解释或释义）。研究评估了概念频率（高频或低频）对模型性能的影响，并使用句法和语义相似性指标、统计相关性及嵌入来评估LLM输出质量。

Result: 结果显示，LLM在定义型问题上的任务表现最高，而在例证型问题上最低。此外，对于定义型问题，大型语言模型更倾向于对流行和高频知识进行释义，而对尾部和技术性知识（尤其是在专家文本中）的释义较少。

Conclusion: 大型语言模型在处理不同类型的语言查询时性能存在显著差异，尤其在例证方面表现不足。其对知识的释义倾向也受概念频率影响，对流行知识的释义多于专业或长尾知识，这揭示了LLMs在复杂知识理解和生成方面的局限性。
...
### [166] [Chatty-KG: A Multi-Agent AI System for On-Demand Conversational Question Answering over Knowledge Graphs](https://arxiv.org/abs/2511.20940)
*Reham Omar,Abdelghny Orogat,Ibrahim Abdelaziz,Omij Mangukiya,Panos Kalnis,Essam Mansour*

Main category: cs.CL

TL;DR: Chatty-KG是一个模块化多智能体系统，用于知识图谱上的对话式问答。它结合了检索增强生成（RAG）和结构化执行，通过任务专用的大语言模型（LLM）智能体生成SPARQL查询，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法在知识图谱（KG）上的对话式问答面临挑战：大型语言模型（LLM）无法直接访问私有和动态KG；检索增强生成（RAG）系统难以处理KG结构和多轮对话；传统KGQA系统通常只支持单轮问答、延迟高且难以处理指代和上下文跟踪。

Method: Chatty-KG是一个模块化的多智能体系统。它通过任务专用的LLM智能体生成SPARQL查询，结合了RAG风格的检索和结构化执行。这些智能体协同工作，进行上下文解释、对话跟踪、实体和关系链接以及高效的查询规划，将自然语言问题准确、低延迟地转化为可执行查询。

Result: 在大型多样化的KG上的实验表明，Chatty-KG在单轮和多轮设置中均显著优于现有基线，实现了更高的F1和P@1分数。其模块化设计保持了对话连贯性，并支持演进中的KG，无需微调或预处理。它与商业和开源LLM（如GPT-4o、Gemini-2.0、Phi-4、Gemma 3）具有广泛兼容性和稳定性能。

Conclusion: Chatty-KG将对话灵活性与结构化KG基础相结合，为可靠的多轮KGQA提供了一种可扩展和可扩展的方法。
...
### [167] [Semantic Anchors in In-Context Learning: Why Small LLMs Cannot Flip Their Labels](https://arxiv.org/abs/2511.21038)
*Anantha Padmanaban Krishna Kumar*

Main category: cs.CL

TL;DR: 本研究发现，上下文学习（ICL）在1-12B参数的语言模型中，主要通过调整输入到预训练语义方向的映射来优化性能，而非灵活地覆盖或重新映射预训练的标签语义。ICL表现出“语义锚定”特性，难以学习反语义分类器。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在探究上下文学习（ICL）是能够覆盖预训练的标签语义，还是仅能细化现有的语义骨架。

Method: 研究将大型语言模型（LLMs）视为提示诱导的分类器，并对比其在“自然演示”（标签正确）和“反转演示”（标签含义系统性翻转）下的行为。通过分解ICL行为为三个对齐指标（真实性、先验和提示对齐），并引入“语义覆盖率”（在翻转语义下的正确性）来衡量。实验涵盖了八个分类任务和八个开源LLMs（1-12B参数）。

Result: 研究发现一致的证据支持“语义锚定”观点。在自然演示下，ICL提高了准确性并保持了强大的先验对齐；大多数正确预测与零样本行为一致。在反转演示下，模型无法学习连贯的反语义分类器：提示对齐的增加是以牺牲准确性为代价的，并且语义覆盖率在少量样本的1-12B模型设置中保持为零。ICL主要调整输入如何投射到预训练期间学习到的稳定语义方向上。

Conclusion: ICL并不能灵活地重新映射标签含义，而主要是调整输入到稳定的预训练语义方向的投射。这揭示了少样本提示的基本局限性，并表明在这些规模下，要覆盖标签语义需要超越ICL的干预措施。
...
### [168] [Context-Aware Pragmatic Metacognitive Prompting for Sarcasm Detection](https://arxiv.org/abs/2511.21066)
*Michael Iskandardinata,William Christian,Derwin Suhartono*

Main category: cs.CL

TL;DR: 本文通过引入检索增强方法，结合网络检索的非参数知识和模型内部的自知识意识，显著提升了大型语言模型（LLMs）在讽刺检测任务上的性能，尤其是在处理文化特定内容时。


<details>
  <summary>Details</summary>
Motivation: 尽管预训练语言模型（PLMs）和大型语言模型（LLMs）在自然语言处理中取得了进展，但讽刺检测仍然是一个挑战。讽刺文本的复杂性、语言多样性、文化差异以及模型对需要额外背景信息的词语检测不可靠，使得这项任务对现有模型来说依然困难。

Method: 本文在现有的“语用元认知提示”（PMP）方法基础上，引入了一种检索增强方法，为每个目标文本提供上下文信息。该方法探索了两种互补的上下文提供方式：一是使用基于网络的检索提供非参数知识，以弥补模型背景知识的不足；二是引导模型利用其自身的内部知识，实现自知识意识策略。

Result: 在Twitter Indonesia Sarcastic数据集上，非参数检索方法相比原始PMP方法实现了9.87%的宏F1分数显著提升。在SemEval-2018 Task 3和MUStARD数据集上，自知识检索分别使宏F1分数提高了3.29%和4.08%。

Conclusion: 这些发现强调了上下文信息在增强LLM讽刺检测性能方面的重要性，特别是对于文化特定俚语、引用或LLM未知术语的处理。未来的工作将侧重于优化相关上下文信息的检索和评估检索质量对性能的影响。
...
### [169] [Enhancing Burmese News Classification with Kolmogorov-Arnold Network Head Fine-tuning](https://arxiv.org/abs/2511.21081)
*Thura Aung,Eaint Kay Khaing Kyaw,Ye Kyaw Thu,Thazin Myint Oo,Thepchai Supnithi*

Main category: cs.CL

TL;DR: 本研究探索了Kolmogorov-Arnold Networks (KANs) 作为低资源语言（如缅甸语）分类任务中多层感知器（MLPs）的替代分类头，发现KANs在表达能力和效率上具有竞争力或更优。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言中，分类任务通常只微调最终分类层，保持预训练编码器权重冻结。常用的MLPs由于其固定的非线性限制了表达能力并增加了计算成本。

Method: 研究探索了三种KAN变体（FourierKAN、EfficientKAN、FasterKAN）作为分类头，并在多种嵌入（TF-IDF、fastText、mBERT、Distil-mBERT）上进行了评估，并与传统的MLPs进行了比较。

Result: KAN-based 分类头与MLPs相比具有竞争力或更优。EfficientKAN结合fastText达到了最高的F1分数（0.928）。FasterKAN在速度和准确性之间提供了最佳权衡。在Transformer嵌入上，EfficientKAN与mBERT结合时与MLPs持平或略微超越（0.917 F1）。

Conclusion: 研究结果表明KANs是MLPs在低资源语言分类任务中富有表达力且高效的替代方案。
...
### [170] [Orthographic Constraint Satisfaction and Human Difficulty Alignment in Large Language Models](https://arxiv.org/abs/2511.21086)
*Bryan E. Tuck,Rakesh M. Verma*

Main category: cs.CL

TL;DR: 本研究评估了大型语言模型在满足硬性正字法约束方面的表现，发现架构差异比参数规模对性能影响更大，且模型存在系统性故障，表明需要专门的架构创新。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在受控文本生成中必须满足硬性正字法约束，但目前缺乏系统性的跨架构评估。

Method: 研究评估了来自三个模型家族（Qwen3、Claude Haiku-4.5、GPT-5-mini）的28种配置，使用了58个需要字符级约束的单词谜题。同时，利用10,000名人类解谜者的难度评分进行校准。

Result: 架构差异导致了比家族内参数扩展更大的性能差距（2.0-2.2倍对83%的增益），这表明约束满足可能需要专门的架构特征或训练目标。思维预算敏感性表现出异质性，高容量模型有显著回报，而中等规模模型饱和或退化。所有模型家族与人类难度评分存在适度但一致的校准（r=0.24-0.38）。模型在常见但正字法不寻常的词汇（如“data”，“poop”，“loll”）上出现系统性失败，人类成功率高但模型错误率高，这揭示了模型过度依赖分布合理性。

Conclusion: 为有效满足正字法约束，可能需要超越简单扩大参数或计算预算的架构创新，以解决模型对分布合理性的过度依赖，避免惩罚正字法上非典型但符合约束的模式。
...
### [171] [ASR Error Correction in Low-Resource Burmese with Alignment-Enhanced Transformers using Phonetic Features](https://arxiv.org/abs/2511.21088)
*Ye Bhone Lin,Thura Aung,Ye Kyaw Thu,Thazin Myint Oo*

Main category: cs.CL

TL;DR: 本研究首次针对低资源缅甸语的自动语音识别（ASR）错误纠正，使用Transformer模型并结合IPA和对齐信息等特征，显著提升了ASR输出的准确性。


<details>
  <summary>Details</summary>
Motivation: 缅甸语的ASR错误纠正研究尚属空白，特别是在低资源环境下，其ASR性能有待提升。

Method: 研究采用序列到序列的Transformer模型进行ASR错误纠正，并探索了不同的特征集成策略，包括国际音标（IPA）和对齐信息。评估了五种ASR骨干网络。

Result: 所提出的ASR错误纠正（AEC）方法持续改进了词级和字符级准确性。结合IPA和对齐特征的AEC模型，将ASR模型的平均词错误率（WER）从51.56%降低到39.82%（增强后为43.59%），并将chrF++分数从0.5864提高到0.627。

Conclusion: AEC方法在低资源缅甸语ASR中表现出鲁棒性，并显著提高了ASR输出的准确性。研究强调了特征设计对于改善低资源环境下的ASR性能的重要性。
...
### [172] [MortgageLLM: Domain-Adaptive Pretraining with Residual Instruction Transfer, Alignment Tuning, and Task-Specific Routing](https://arxiv.org/abs/2511.21101)
*Manish Jain,Satheesh Kumar Ponnambalam,Salman Faroz,Chandrakanth Lns,Vinay Sharma*

Main category: cs.CL

TL;DR: MortgageLLM是一个针对抵押贷款金融领域的双专家大型语言模型，它采用双轨专业化框架和指令残差技术，有效处理对话和结构化任务，显著优于基础模型。


<details>
  <summary>Details</summary>
Motivation: 通用LLM在专业领域（如抵押贷款金融）需要领域知识增强并保持指令遵循能力。单一多任务模型在优化结构化任务（通过SFT）时会损害对话忠实度（通过DPO），存在性能权衡。

Method: 该研究提出了MortgageLLM，一个基于LLaMA-3.1-8B的双轨专业化框架。它创建了两个专家模型：一个用于对话式问答，另一个用于分类和摘要等结构化任务。为解决指令遵循问题，该方法在领域适应后应用了指令残差技术，而无需监督微调。此外，还设计了一个智能任务路由机制，由其中一个专家模型通过少样本分类实现。

Result: MortgageLLM (MLM v2) 在领域特定基准测试中显著优于基础模型LLaMA-3.1-8B-Instruct。在LLM-as-a-Judge评估中，摘要得分4.58（对比3.99），问答得分4.09（对比4.0），分类得分2.6（对比1.2）。在语义相似性BERTScore上，摘要得分0.77（对比0.74），问答得分0.68（对比0.58），分类得分0.75（对比0.73），均大幅超越基线方法。

Conclusion: MortgageLLM通过其新颖的双专家架构和指令残差技术，成功解决了将大型语言模型应用于高度专业化的抵押贷款金融领域的挑战，实现了卓越的性能表现。
...
### [173] [Can Finetuing LLMs on Small Human Samples Increase Heterogeneity, Alignment, and Belief-Action Coherence?](https://arxiv.org/abs/2511.21218)
*Steven Wang,Kyle Hunt,Shaojie Tang,Kenneth Joseph*

Main category: cs.CL

TL;DR: 研究发现，虽然对大型语言模型（LLMs）进行小样本人类数据微调能显著改善其模拟人类行为的异质性、对齐性和信念-行动一致性，但它们仍无法准确复制人类研究的回归系数，因此不适合替代人类参与者进行正式推断分析。


<details>
  <summary>Details</summary>
Motivation: 关于LLMs是否能替代人类参与者进行调查和实验研究的争论持续不断。现有证据表明LLMs在模拟人类行为时存在多样性不足、少数群体错位、群体内方差不足以及信念与行动不符等问题。本研究旨在探讨，通过对少量人类调查数据（如来自试点研究的数据）进行微调，是否能缓解这些问题并产生更真实的模拟结果。

Method: 本研究采用了一项关于信息披露的行为实验。通过将微调后的大型语言模型（LLM）与人类参与者的反应进行多维度比较，包括分布差异、子群体对齐、信念-行动一致性以及回归系数的恢复能力。

Result: 研究发现，与基础模型相比，使用少量人类样本进行微调显著改善了LLM模拟行为的异质性、对齐性和信念-行动一致性。然而，即使是表现最佳的微调模型也未能重现原始研究的回归系数。

Conclusion: 尽管微调能提升LLM模拟行为的真实性，但LLM生成的数据仍不适用于在正式推断分析中替代人类参与者。
...
### [174] [Self-Guided Defense: Adaptive Safety Alignment for Reasoning Models via Synthesized Guidelines](https://arxiv.org/abs/2511.21214)
*Yuhang Wang,Yanxu Zhu,Dongyuan Lu,Jitao Sang*

Main category: cs.CL

TL;DR: 本文提出SGASA框架，通过内部化模型生成的安全指南，自适应地增强模型对恶意越狱提示的防御能力，同时减少对良性请求的误拒。


<details>
  <summary>Details</summary>
Motivation: 推理模型虽强大，但对抗性越狱提示仍是其安全性的严峻挑战。这些提示常能规避现有安全机制，导致有害内容生成，因此需要一种自适应的安全对齐方法来强化模型防御。

Method: SGASA框架包含两个阶段：1. 数据预合成：生成安全指南和增强提示。2. 对齐微调：利用监督微调(SFT)和直接偏好优化(DPO)将这些指南嵌入模型中。

Result: 在多个数据集上的大量实验表明，SGASA显著提升了模型的安全性，验证了其自适应和可扩展的有效性。

Conclusion: SGASA框架通过内部化安全指南，提供了一种有效且可扩展的自适应安全对齐方法，显著增强了模型抵御对抗性越狱提示的能力。
...
### [175] [Developing an Open Conversational Speech Corpus for the Isan Language](https://arxiv.org/abs/2511.21229)
*Adisai Na-Thalang,Chanakan Wittayasakpan,Kritsadha Phatcharoen,Supakit Buakaw*

Main category: cs.CL

TL;DR: 本文介绍了首个面向伊善语的开放式对话语音数据集的开发，该数据集包含自然语音，捕捉了真实的语言现象，并解决了缺乏标准化正字法带来的转录挑战。


<details>
  <summary>Details</summary>
Motivation: 现有语音语料库主要基于朗读或脚本语音，缺乏自然对话语音，无法捕捉真实的语言现象（如口语、自发韵律、不流畅和与泰语的频繁语码转换）。此外，伊善语作为一种代表性不足的语言，缺乏资源，且缺乏标准化正字法对转录构成挑战。

Method: 收集自然伊善语语音，以捕捉口语、自发韵律、不流畅和语码转换等真实语言现象。针对伊善语缺乏标准化正字法的问题，制定了实用的转录协议，以平衡表征准确性和计算处理需求。

Result: 开发并发布了首个开放式伊善语对话语音数据集，该数据集包含自然语音，能捕捉真实的语言现象。通过建立转录协议，解决了因缺乏标准化正字法而导致的转录一致性、可用性和语言真实性问题。

Conclusion: 该数据集的发布旨在促进包容性AI发展，支持对代表性不足语言的研究，并为解决会话语音建模固有的语言和技术挑战奠定基础。
...
### [176] [PEFT-Bench: A Parameter-Efficient Fine-Tuning Methods Benchmark](https://arxiv.org/abs/2511.21285)
*Robert Belanec,Branislav Pecher,Ivan Srba,Maria Bielikova*

Main category: cs.CL

TL;DR: 本文介绍了PEFT-Bench，一个统一的端到端基准测试平台，用于评估自回归LLM上的各种参数高效微调（PEFT）方法，并引入了考虑训练和推理因素的PEFT软分数惩罚（PSCP）指标。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）虽然性能卓越，但其巨大的规模导致高昂的计算和环境成本，限制了可访问性。PEFT方法旨在解决这一问题，但当前的评估方法受限于模型和数据集，且难以复现。

Method: 研究人员开发了PEFT-Bench，一个统一的端到端基准测试平台，用于评估自回归LLM上的多种PEFT方法。此外，他们引入了PEFT软分数惩罚（PSCP）指标，该指标综合考虑了可训练参数、推理速度和训练内存使用量等因素。

Result: 研究人员展示了PEFT-Bench在27个NLP数据集和6种PEFT方法上的应用。他们还提出了PSCP指标，用于量化不同PEFT训练和推理因素的影响。

Conclusion: PEFT-Bench提供了一个全面的框架，用于统一评估和比较PEFT方法，并通过PSCP指标考虑了实际的训练和推理效率，从而弥补了现有评估的不足。
...
### [177] [Emergent Lexical Semantics in Neural Language Models: Testing Martin's Law on LLM-Generated Text](https://arxiv.org/abs/2511.21334)
*Kai Kugler*

Main category: cs.CL

TL;DR: 首次系统研究了神经网络语言模型训练过程中马丁定律（词频与多义性关系）的演变，发现其遵循非单调轨迹，存在一个最佳语义窗口，且模型大小影响语义崩溃的程度。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络语言模型在训练过程中如何习得并遵循马丁定律（词频与多义性之间的经验关系），并系统性地研究这一现象。

Method: 使用上下文嵌入的DBSCAN聚类来操作化词义，分析了四个Pythia模型（70M-1B参数）在30个训练检查点上的表现。

Result: 马丁定律的演变轨迹是非单调的：在检查点100左右出现，在检查点104达到峰值相关性（r > 0.6），随后在检查点105开始退化。较小的模型（70M, 160M）在后期检查点出现灾难性语义崩溃，而较大的模型（410M, 1B）则表现出平稳退化。词频-特异性权衡（r ≈ -0.3）在所有模型中保持稳定。

Conclusion: LLM生成文本中对语言规律的遵循并非随训练单调增加，而是遵循一个具有最佳语义窗口的平衡轨迹。这项工作建立了一种评估神经网络语言模型中涌现语言结构的新方法。
...
### [178] [Training Introspective Behavior: Fine-Tuning Induces Reliable Internal State Detection in a 7B Model](https://arxiv.org/abs/2511.21399)
*Joshua Fonseca Rivera*

Main category: cs.CL

TL;DR: 本文通过微调训练，显著提升了语言模型检测和报告注入“思想”（激活模式）的能力，从几乎完全失败到可靠识别，为内置AI透明度提供了途径。


<details>
  <summary>Details</summary>
Motivation: Lindsey (2025) 的研究发现语言模型内省意识不可靠。本文旨在探索是否可以通过直接训练而非等待其自然涌现来提升这种能力，以解决Lindsey提出的关于训练内省能否消除模型间差异的开放性问题。

Method: 通过对瞬态单token注入进行微调，训练一个7B参数的语言模型。

Result: 将模型从接近完全失败（0.4%准确率，6.7%误报率）提升到可靠检测（对保留概念的准确率为85%，误报率为0%）。模型能检测、保留并报告注入的短暂“思想”的语义内容。它满足Lindsey的准确性、基础性（无误报）和内在性（检测先于言语化）三个标准。对未见概念向量的泛化能力（7.5pp差距）表明模型学习了可迁移技能，而非记忆特定向量。

Conclusion: 内省行为的至少一个组成部分可以通过直接训练诱导出来，这为内置AI透明度提供了一条途径，并回应了Lindsey关于“为内省而训练是否会有助于消除跨模型差异”的开放性问题。
...
### [179] [Text-to-SQL as Dual-State Reasoning: Integrating Adaptive Context and Progressive Generation](https://arxiv.org/abs/2511.21402)
*Zhifeng Hao,Qibin Song,Ruichu Cai,Boyan Xu*

Main category: cs.CL

TL;DR: DSR-SQL是一个双状态推理框架，通过自适应上下文和渐进式生成来解决LLM在复杂企业数据库上进行Text-to-SQL时遇到的连贯推理问题，无需后训练或上下文示例即可达到有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CoT的Text-to-SQL方法在应用于复杂的企业数据库时，由于有限的上下文容量、不可靠的模式链接和对数据库语义的弱接地，难以保持连贯的推理。

Method: DSR-SQL将Text-to-SQL建模为自适应上下文状态和渐进式生成状态之间的交互。自适应上下文状态通过精炼大型模式和选择相关结构来构建紧凑、语义忠实的环境；渐进式生成状态将SQL合成形式化为反馈引导的状态转换，使模型能够自我修正并与用户意图对齐。

Result: DSR-SQL在不进行任何后训练或使用上下文示例的情况下，在Spider 2.0-Snow上达到了35.28%的执行准确率，在BIRD开发集上达到了68.32%的执行准确率，表现出有竞争力的性能。

Conclusion: DSR-SQL通过其双状态推理框架，有效地克服了LLM在复杂企业数据库上进行Text-to-SQL推理时遇到的挑战，并在多个基准测试中取得了显著的性能，证明了其在处理复杂数据库查询方面的潜力。
...
### [180] [A Systematic Study of Model Merging Techniques in Large Language Models](https://arxiv.org/abs/2511.21437)
*Oğuz Kağan Hitit,Leander Girrbach,Zeynep Akata*

Main category: cs.CL

TL;DR: 本文系统评估了六种先进的模型合并方法在大型语言模型（LLM）上的表现。结果显示，最简单的方法Task Arithmetic是唯一能可靠提升LLM性能的方法，而其他方法通常导致性能显著下降，表明当前合并技术不适用于现代LLM。


<details>
  <summary>Details</summary>
Motivation: 模型合并在小型模型和分类器上展现出优势，但其是否能推广到大型语言模型（LLM）仍不明确。

Method: 本文对六种最先进的合并方法（包括最近的子空间方法）进行了大规模、系统性评估。评估对象包括四种开源LLM、每种基础模型十二个微调检查点以及十六个标准LLM基准。评估指标包括合并模型超越基础模型的概率以及相对于最佳独立检查点的相对增益。

Result: 结果表明，最古老、最简单的方法Task Arithmetic是唯一能可靠地在LLM上产生性能增益的方法。其他考虑干扰和子空间的合并方法通常会导致显著的性能下降。

Conclusion: 当前的模型合并技术不能直接应用于现代LLM。这促使需要设计针对LLM的合并算法和合并感知型微调方法。
...
### [181] [Can LLMs extract human-like fine-grained evidence for evidence-based fact-checking?](https://arxiv.org/abs/2511.21401)
*Antonín Jarolím,Martin Fajčík,Lucia Makaiová*

Main category: cs.CL

TL;DR: 本文关注捷克语和斯洛伐克语在线评论中事实性声明的细粒度证据提取，创建了一个新的双向标注数据集，并评估了大型语言模型（LLMs）在此任务上的表现，发现LLMs在逐字复制证据方面存在不足，且不同模型表现差异显著。


<details>
  <summary>Details</summary>
Motivation: 在线新闻评论中虚假信息频繁传播，需要有效方法来检测不正确信息。为了有力支持或反驳从评论中提取的声明，必须识别相关文档并精确指出支持或驳斥每个声明的文本片段。

Method: 研究创建了一个新的数据集，其中包含由付费标注员创建的捷克语和斯洛伐克语声明的双向标注细粒度证据。然后，在该数据集上评估了大型语言模型（LLMs），以评估它们与人类标注的一致性。

Result: 结果显示，LLMs通常无法从源文本中逐字复制证据，导致输出无效。错误率分析表明，llama3.1:8b模型尽管规模相对较小，但实现了高比例的正确输出，而gpt-oss-120b模型参数更多却表现不佳。此外，qwen3:14b、deepseek-r1:32b和gpt-oss:20b模型在模型规模和与人类标注的一致性之间表现出有效平衡。

Conclusion: 大型语言模型在细粒度证据提取任务中，特别是在逐字复制证据方面存在挑战。模型性能不完全取决于其规模，较小的模型也能取得良好效果，且存在模型规模与人类标注一致性之间的权衡。
...
### [182] [Odin: Oriented Dual-module Integration for Text-rich Network Representation Learning](https://arxiv.org/abs/2511.21416)
*Kaifeng Hong,Yinglong Zhang,Xiaoying Hong,Xuewen Xia,Xing Xu*

Main category: cs.CL

TL;DR: Odin提出了一种新的Transformer架构，通过定向双模块机制在特定层注入图结构，有效结合文本理解和结构推理，避免了GNN的过平滑问题，并实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理文本属性图时存在局限：GNN受限于过平滑和跳数依赖的扩散；Transformer则忽略图拓扑，将节点视为孤立序列，无法有效结合文本理解与结构化推理。

Method: Odin（定向双模块集成）是一种将图结构注入Transformer特定深度的新架构。它通过定向双模块机制，在模型的语义层次中整合低、中、高层多跳结构。由于聚合操作作用于全局[CLS]表示，Odin从根本上避免了过平滑，并将结构抽象与邻域大小或图拓扑解耦。此外，还提出了轻量级变体Light Odin，以提高大规模或低资源环境下的效率。

Result: Odin在多个文本丰富图基准测试中达到了最先进的准确性。Light Odin在显著降低计算成本的同时，也展现出具有竞争力的性能。理论上，Odin的表达能力严格包含纯Transformer和GNN。

Conclusion: Odin和Light Odin共同构成了一个统一的、无跳数（hop-free）框架，用于原则性的结构-文本集成。该框架有效克服了现有方法的局限性，在性能和效率上均有显著提升。
...
### [183] [Bangla Sign Language Translation: Dataset Creation Challenges, Benchmarking and Prospects](https://arxiv.org/abs/2511.21533)
*Husne Ara Rubaiyeat,Hasan Mahmud,Md Kamrul Hasan*

Main category: cs.CL

TL;DR: 本文介绍了为孟加拉手语翻译（BdSLT）创建的一个新数据集IsharaKhobor及其子集，旨在解决该语言资源匮乏的问题，并为人工智能辅助工具的开发提供基准。


<details>
  <summary>Details</summary>
Motivation: 孟加拉手语（BdSL）是一种资源极度匮乏的语言。为BdSLT创建标准句子级别的数据集对于开发基于人工智能的辅助工具至关重要，以帮助孟加拉语社区的聋哑和听障人士。

Method: 研究团队创建了数据集IsharaKhobor及其两个子集，并公开可用。他们还讨论了数据集开发面临的挑战，并通过基于地标的原始和RQE嵌入进行基准测试，提出了未来的发展方向。此外，他们对数据集内的词汇限制和规范化进行了消融实验，生成了另外两个数据集：IsharaKhobor_small和IsharaKhobor_canonical_small。

Result: 本文成功推出了IsharaKhobor数据集及其两个子集，以及通过词汇限制和规范化生成的IsharaKhobor_small和IsharaKhobor_canonical_small数据集。这些数据集已公开发布，并提供了基于地标嵌入的基准测试结果。

Conclusion: 所创建的IsharaKhobor数据集及其变体将极大地促进孟加拉手语翻译领域的研究，为开发针对孟加拉语聋哑和听障人士的AI辅助工具奠定基础。通过基准测试，为未来的研究方向提供了指导。
...
### [184] [Voice, Bias, and Coreference: An Interpretability Study of Gender in Speech Translation](https://arxiv.org/abs/2511.21517)
*Lina Conti,Dennis Fucci,Marco Gaido,Matteo Negri,Guillaume Wisniewski,Luisa Bentivogli*

Main category: cs.CL

TL;DR: 语音翻译（ST）模型在性别分配上存在复杂机制和偏见。研究发现模型倾向于男性化，但能根据声学信息（超越音高）和第一人称代词进行更准确的性别判断，而非简单复制训练数据模式。


<details>
  <summary>Details</summary>
Motivation: 语音包含说话者的性别信息，这在语音翻译中可能导致偏见，尤其是在从概念性别语言翻译到语法性别语言时，可能错误地判断说话者性别。目前对ST模型如何做出这些性别判断的机制知之甚少。

Method: 研究了ST模型在三种语言对（英-西/法/意）中如何为指代说话者的术语分配性别。考察了训练数据模式、内部语言模型（ILM）偏见和声学信息之间的相互作用。使用频谱图上的对比特征归因方法揭示模型机制。

Result: 模型并非简单复制训练数据中术语特定的性别关联，而是学习了更广泛的男性普遍性模式。内部语言模型（ILM）表现出强烈的男性偏见，但模型能够根据声学输入覆盖这些偏好。高性别准确率的模型通过第一人称代词将性别术语与说话者关联起来，并利用分布在整个频率频谱而非集中在音高上的性别信息。

Conclusion: 语音翻译模型在性别分配上表现出复杂的行为，受训练数据中的男性普遍性模式和内部语言模型偏见的影响。然而，高准确率的模型能够利用更广泛的声学信息（超越音高）和代词关联机制，从而更准确地推断说话者性别，这揭示了之前未知的模型决策机制。
...
### [185] [Hierarchical Ranking Neural Network for Long Document Readability Assessment](https://arxiv.org/abs/2511.21473)
*Yurui Zheng,Yijun Chen,Shaohong Zhang*

Main category: cs.CL

TL;DR: 本文提出了一种双向可读性评估机制，结合句子级预测和文档级辅助预测，并引入成对排序算法处理可读性标签的序数关系，在中文和英文数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习可读性评估方法通常未能有效考虑文本长度或可读性标签的序数关系。

Method: 本文提出了一种双向可读性评估机制，该机制捕获上下文信息以识别文本中语义丰富区域，从而预测句子级别的可读性。这些句子级标签随后用于辅助预测文档的整体可读性。此外，引入了一种成对排序算法，通过标签相减来建模可读性级别之间的序数关系。

Result: 在中文和英文数据集上的实验结果表明，所提出的模型实现了具有竞争力的性能，并优于其他基线模型。

Conclusion: 该模型通过整合句子级和文档级预测以及对可读性标签序数关系的建模，有效提升了可读性评估的准确性和鲁棒性，并在多语言环境下展现了卓越性能。
...
### [186] [Auxiliary Metrics Help Decoding Skill Neurons in the Wild](https://arxiv.org/abs/2511.21610)
*Yixiu Zhao,Xiaozhi Wang,Zijun Yao,Lei Hou,Juanzi Li*

Main category: cs.CL

TL;DR: 本文提出了一种轻量级且广泛适用的方法，用于隔离大型语言模型中编码特定技能的神经元，并通过关联神经元激活与辅助指标来揭示可解释的任务特定行为，甚至发现模型中的捷径。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）能力强大但内部机制不透明，理解其内部运作对于解释和改进模型至关重要。现有方法识别“技能神经元”主要集中在分类任务，需要扩展到更复杂的场景。

Method: 该方法建立在通过软提示训练识别“技能神经元”的先前工作之上，并将其扩展到涉及多技能的复杂场景。它通过将神经元激活与外部标签、模型置信度等辅助指标相关联，来识别编码特定技能的神经元，无需手动进行token聚合。

Result: 该方法在开放式文本生成和自然语言推理任务上得到了实证验证，不仅能够检测驱动已知技能的神经元，还在BigBench的算术推理任务中揭示了先前未识别的“捷径”行为。

Conclusion: 该研究引入了一种有效、可解释且任务特定的方法，能够隔离LLM中的技能神经元，揭示模型的内部机制，并发现其在复杂推理任务中的隐藏行为和潜在捷径。
...
### [187] [RoParQ: Paraphrase-Aware Alignment of Large Language Models Towards Robustness to Paraphrased Questions](https://arxiv.org/abs/2511.21568)
*Minjoon Choi*

Main category: cs.CL

TL;DR: 本文介绍了RoParQ基准和XParaCon评估指标，用于衡量大型语言模型（LLMs）在处理复述问题时的跨复述一致性。通过提出一种基于推理的、复述感知的监督微调（SFT）策略，显著提高了模型的鲁棒性，使轻量级模型也能达到与大型预训练模型相当的一致性水平。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在回答复述问题时表现出不一致的行为，这表明它们可能依赖于表面模式而非真正的语义理解，因此需要解决这一局限性。

Method: 1. 引入RoParQ，一个专门用于评估闭卷多项选择问答中跨复述一致性的基准，通过私有模型生成复述并筛选出导致判别模型信心不一致的例子。2. 提出XParaCon，一种通过测量问题变体准确率的标准差来量化模型鲁棒性的新评估指标。3. 实施一种基于推理的、复述感知的监督微调（SFT）策略，旨在使模型与语义不变性对齐。

Result: 实验表明，所提出的有针对性的对齐策略显著增强了模型的鲁棒性。值得注意的是，经过微调的轻量级模型达到了与大得多的预训练模型相当的一致性水平。

Conclusion: 该方法有效缓解了模型的肤浅记忆，并促进了更鲁棒、更可靠的大型语言模型的开发。
...
### [188] [Beyond URLs: Metadata Diversity and Position for Efficient LLM Pretraining](https://arxiv.org/abs/2511.21613)
*Dongyang Fan,Diba Hashemi,Sai Praneeth Karimireddy,Martin Jaggi*

Main category: cs.CL

TL;DR: 本研究探索了多种元数据类型，发现细粒度元数据（如文档质量指标）能有效加速大型语言模型预训练。通过前置、附加作为辅助任务以及可学习元标记等方法，元数据可以提升预训练效率和效果。


<details>
  <summary>Details</summary>
Motivation: 以往研究仅强调URL作为唯一有用的元数据信号，本研究旨在探索更广泛的元数据类型，以期发现更多能加速LLM预训练并带来更大益处的信号。

Method: 研究调查了多种元数据类型（超越URL），通过将元数据前置（prepending）来加速预训练。此外，引入了元数据附加（appending）作为辅助任务以提高训练效率，并提出了使用掩码损失训练的可学习元标记（learnable meta-tokens）。最后，通过探测（probing）技术分析了潜在表征，以理解元数据如何影响学习过程。

Result: 研究发现，除了URL，其他元数据类型（如文档质量的细粒度指标）也能在前置时加速预训练。有效的元数据共同特征是编码了更细粒度的信息。将元数据附加作为辅助任务（预测合适的元数据）可以提高训练效率。可学习的元标记通过诱导质量感知的潜在结构，能够恢复部分加速效果。分析表明元数据能塑造学习过程。

Conclusion: 这些研究结果为整合元数据以提高大型语言模型预训练的效率和有效性提供了实用的指导方针。
...
### [189] [Matrix: Peer-to-Peer Multi-Agent Synthetic Data Generation Framework](https://arxiv.org/abs/2511.21686)
...
TL;DR: Matrix是一个去中心化的多智能体合成数据生成框架，通过点对点消息传递消除中央协调器，显著提高了数据生成吞吐量和灵活性，适用于LLM训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练对合成数据需求日益增长，尤其当真实数据稀缺、昂贵或涉及隐私时。现有多智能体合成数据框架依赖中心协调器，导致可扩展性瓶颈，或因特定领域硬编码而缺乏灵活性。

Method: 本文提出了Matrix框架，采用去中心化设计，将控制流和数据流表示为通过分布式队列传递的序列化消息，实现点对点通信，从而消除了中央协调器。任务由轻量级智能体独立处理，计算密集型操作（如LLM推理）由分布式服务处理。该框架基于Ray构建，具有模块化和可配置性。

Result: Matrix能够扩展到数万个并发智能体工作流，并易于适应各种数据生成场景。在多智能体协作对话、基于网络的推理数据提取和客户服务环境中的工具使用轨迹生成等多样化合成场景中，Matrix在相同硬件资源下实现了2到15倍的数据生成吞吐量提升，同时不损害输出质量。

Conclusion: Matrix框架通过其去中心化设计，有效解决了现有合成数据生成框架在可扩展性和灵活性方面的挑战，显著提高了多智能体工作流的数据生成效率，且能保持高质量输出。
...
### [190] [The author is dead, but what if they never lived? A reception experiment on Czech AI- and human-authored poetry](https://arxiv.org/abs/2511.21629)
*Anna Marklová,Ondřej Vinš,Martina Vokáčová,Jiří Milička*

Main category: cs.CL

TL;DR: 研究发现，捷克语母语者无法区分人工智能生成和人类创作的捷克语诗歌，但对作者身份的信念会强烈影响他们对诗歌的审美评价。


<details>
  <summary>Details</summary>
Motivation: 大多数关于人工智能生成诗歌的研究都集中在英语上，而英语在训练数据中占主导地位。本研究旨在探讨人工智能在形态复杂、资源稀缺（相对于AI模型训练数据）的斯拉夫语种（如捷克语）中生成诗歌的能力及其感知情况。

Method: 参与者（捷克语母语者）被要求判断诗歌的作者（AI或人类），并进行审美评价。研究使用逻辑回归模型分析了数据。

Result: 参与者在猜测作者身份时表现为随机水平（平均正确率为45.8%），表明AI生成的捷克语诗歌与人类创作的诗歌基本无法区分。审美评价显示出强烈的作者身份偏见：当参与者认为一首诗是AI生成时，他们会给予较低的评价，尽管AI诗歌实际上平均获得了与人类诗歌相同或更高的评价。逻辑回归模型揭示，人们越喜欢一首诗，他们准确判断作者身份的可能性就越小。对诗歌的熟悉程度或文学背景对识别准确性没有影响。

Conclusion: 研究结果表明，即使在像捷克语这样形态复杂、资源稀缺的斯拉夫语种中，人工智能也能令人信服地创作诗歌。同时，读者的作者身份信念与诗歌的审美评价之间存在相互关联。
...
### [191] [ToolOrchestra: Elevating Intelligence via Efficient Model and Tool Orchestration](https://arxiv.org/abs/2511.21689)
...
TL;DR: 研究表明，小型编排器通过管理智能工具，能以更高的效率和效果解决复杂问题，甚至超越大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在解决如“人类终极考试”(HLE)这类深度复杂问题时，面临概念挑战和计算成本高昂的问题。

Method: 引入ToolOrchestra方法，利用强化学习训练小型编排器，使其能够协调智能工具。该方法在奖励设计上考虑了结果、效率和用户偏好。

Result: 使用ToolOrchestra训练的8B模型Orchestrator在HLE上得分37.1%，超越GPT-5（35.1%），效率提高2.5倍。在tau2-Bench和FRAMES上，Orchestrator也大幅超越GPT-5，成本仅为其30%。它在性能与成本之间实现了最佳权衡，并能稳健泛化到未见工具。

Conclusion: 通过轻量级编排模型组合多样化工具，比现有方法更高效、更有效，为实用且可扩展的工具增强推理系统铺平了道路。
...
### [192] [Revisiting Generalization Across Difficulty Levels: It's Not So Easy](https://arxiv.org/abs/2511.21692)
*Yeganeh Kordi,Nihal V. Nayak,Max Zuo,Ilana Nguyen,Stephen H. Bach*

Main category: cs.CL

TL;DR: 本研究系统评估了大型语言模型（LLMs）在不同任务难度上的泛化能力，发现其跨难度泛化通常有限，仅用简单或困难数据训练无法在所有难度范围上取得一致改进，强调了训练和评估数据中难度多样性的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有研究对于使用简单或困难数据训练LLMs是否能带来更好结果，以及这些改进体现在简单还是困难测试数据上，存在相互矛盾的结论。本研究旨在系统地解决这一关键问题，以指导有效的数据整理和评估。

Method: 研究通过系统评估LLMs在不同模型、数据集和细粒度示例难度上的泛化能力。他们使用数千个不同LLMs的输出和教育测试中成熟的难度度量标准——项目反应理论（IRT），对六个数据集中的示例进行难度排序。与以往工作不同，本研究的难度评分完全由LLMs的能力决定，排除了人类的主观意见，实现了更客观、大规模和细粒度的分析。

Result: 研究发现LLMs的跨难度泛化能力通常有限。仅用简单或困难数据进行训练，无法在整个难度范围内实现一致的改进。

Conclusion: 这些结果表明，在LLMs的训练和评估数据中包含不同难度的示例至关重要。在数据难度上走捷径是存在风险的。
...
### [193] [OVAL-Grasp: Open-Vocabulary Affordance Localization for Task Oriented Grasping](https://arxiv.org/abs/2511.20841)
*Edmond Tong,Advaith Balaji,Anthony Opipari,Stanley Lewis,Zhen Zeng,Odest Chadwicke Jenkins*

Main category: cs.RO

TL;DR: OVAL-Grasp 是一种零样本、开放词汇的方法，利用大型语言模型和视觉语言模型，使机器人能够在未知环境中根据任务需求，识别并抓取物体的正确部位，即使在部分遮挡下也能有效工作。


<details>
  <summary>Details</summary>
Motivation: 在新型、非结构化环境中，机器人需要基于任务的抓取，针对物体特定部分进行操作。传统的基于几何的方法在处理视觉定义的部件、遮挡和未见过物体时表现不佳。

Method: OVAL-Grasp 接收 RGB 图像和任务指令。它使用大型语言模型 (LLM) 识别要抓取或避免的物体部件，然后使用视觉语言模型 (VLM) 对这些部件进行分割，并生成物体上可操作区域的 2D 热图。

Result: OVAL-Grasp 在包含 20 种家用物体和每种物体 3 个独特任务的实验中，优于两个任务导向抓取基线。在真实世界实验中，它成功识别并分割正确物体部件的成功率为 95%，抓取正确可操作区域的成功率为 78.3%。在杂乱场景下，部分遮挡下的部件选择成功率为 80%。该方法在依赖视觉特征进行部件选择的场景中也表现出有效性，并且其模块化设计在消融实验中显示出优势。

Conclusion: OVAL-Grasp 提供了一种零样本、任务导向的抓取方法，能够有效处理新颖物体、视觉定义部件和部分遮挡，显著提升了机器人在复杂环境中的操作能力。
...
### [194] [NOIR 2.0: Neural Signal Operated Intelligent Robots for Everyday Activities](https://arxiv.org/abs/2511.20848)
*Tasha Kim,Yingke Wang,Hanvit Cho,Alex Hodges*

Main category: cs.RO

TL;DR: NOIR 2.0是一个增强型脑机接口系统，利用脑电图（EEG）使人类能通过脑信号控制机器人执行日常任务，其解码速度更快，通过小样本机器人学习实现高效用户适应，显著减少了任务完成时间和总人工时间。


<details>
  <summary>Details</summary>
Motivation: 现有脑机接口系统（如NOIR）允许人类通过脑信号控制机器人执行日常任务。本研究旨在进一步提升该系统的性能，包括提高脑信号解码的速度和准确性，以及优化用户适应和意图预测的效率。

Method: NOIR 2.0系统采用脑电图（EEG）将人类意图（关于特定物体和所需动作）直接转换为机器人可执行的命令。它引入了更快、更准确的脑解码算法，并利用小样本机器人学习算法（结合基础模型）进行个体用户适应和意图预测，从而实现更高效的学习和适应。

Result: NOIR 2.0将任务完成时间缩短了46%。通过新的学习算法，它显著减少了总人工时间达65%，并且实现了更样本高效的学习和适应（例如，通过15个演示与单个演示的比较来体现其效率提升）。

Conclusion: NOIR 2.0通过更快的解码算法和高效的小样本机器人学习，显著提升了脑机接口的性能，使得人类能够更快速、更高效地通过脑信号控制机器人完成日常任务，大大减少了所需的时间和精力。
...
### [195] [ACE-F: A Cross Embodiment Foldable System with Force Feedback for Dexterous Teleoperation](https://arxiv.org/abs/2511.20887)
*Rui Yan,Jiajian Fu,Shiqi Yang,Lars Paulsen,Xuxin Cheng,Xiaolong Wang*

Main category: cs.RO

TL;DR: 本文介绍了一种名为ACE-F的跨肢体可折叠遥操作系统，它集成了力反馈功能，通过创新的虚拟力信号解释和泛化软控制器，大大简化了不同机器人肢体的精确控制，并开源供模仿学习等应用。


<details>
  <summary>Details</summary>
Motivation: 当前的遥操作平台普遍缺乏集成的力反馈、跨肢体泛化能力以及便携、用户友好的设计，这限制了它们在收集复杂、接触密集型任务高质量机器人演示数据方面的实际部署。

Method: ACE-F系统结合了逆运动学(IK)和精心设计的人机界面(HRI)来捕获精确演示。它提出了一种泛化的软控制器管道，集成了PD控制和逆动力学以确保机器人安全和精确运动控制。为实现无需额外传感器的力反馈跨肢体泛化，该方法创新性地将末端执行器位置偏差解释为虚拟力信号。

Result: 广泛的遥操作实验证实，ACE-F显著简化了各种机器人肢体的控制，使得灵巧操作任务变得像操作电脑鼠标一样直观。该系统提高了数据收集效率，并支持模仿学习等应用。

Conclusion: ACE-F提供了一种实用的解决方案，克服了现有遥操作系统的局限性，通过集成力反馈和跨肢体泛化能力，大大提高了机器人演示数据收集的效率和质量，并已开源，有望推动相关领域的发展。
...
### [196] [Efficient Greedy Algorithms for Feature Selection in Robot Visual Localization](https://arxiv.org/abs/2511.20894)
*Vivek Pandey,Amirhossein Mollaei,Nader Motee*

Main category: cs.RO

TL;DR: 本文提出两种快速且内存高效的特征选择算法，旨在实时机器人视觉定位中，通过识别最具信息量的特征子集，解决现有方法计算量大、效率低的问题，以平衡计算效率和定位精度。


<details>
  <summary>Details</summary>
Motivation: 机器人视觉定位中，图像帧包含大量冗余或无信息量的特征，处理所有特征会导致显著的计算延迟和低效率。因此，需要智能特征选择来识别对定位最有用的特征子集。

Method: 本文提出了两种快速且内存高效的特征选择算法。这些方法明确设计用于降低时间和内存复杂度，并能在实时主动评估视觉特征的效用。

Result: 所提出的方法在计算效率和定位精度之间取得了良好的权衡。与现有方法相比，它们显著降低了计算和内存需求。

Conclusion: 本文成功开发了两种高效的特征选择算法，能够帮助机器人在实时视觉定位中，通过智能选择特征，有效降低计算和内存开销，同时保持良好的定位精度。
...
### [197] [Dynamic Test-Time Compute Scaling in Control Policy: Difficulty-Aware Stochastic Interpolant Policy](https://arxiv.org/abs/2511.20906)
*Inkook Chun,Seungjae Lee,Michael S. Albergo,Saining Xie,Eric Vanden-Eijnden*

Main category: cs.RO

TL;DR: DA-SIP框架使基于扩散和流的机器人策略能够根据任务难度自适应调整计算预算，显著提高效率。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散和流策略在每个控制步骤使用固定的推理预算，导致简单子任务计算冗余，复杂子任务可能性能不足，从而造成计算效率低下。

Method: 引入了DA-SIP（Difficulty-Aware Stochastic Interpolant Policy）框架。该方法使用一个难度分类器，根据观测实时动态调整集成步长预算、选择最优求解器变体以及ODE/SDE集成。DA-SIP基于随机插值公式，为扩散和流策略提供了一个统一的训练和推理配置框架。

Result: 在多种操作任务的综合基准测试中，DA-SIP在保持与固定最大计算基线相当的任务成功率的同时，将总计算时间减少了2.6-4.4倍。

Conclusion: 通过在该框架内实现自适应计算，DA-SIP将生成式机器人控制器转变为高效、任务感知型系统，能够智能地将推理资源分配到最有益的地方。
...
### [198] [AerialMind: Towards Referring Multi-Object Tracking in UAV Scenarios](https://arxiv.org/abs/2511.21053)
*Chenglizhao Chen,Shaofeng Liang,Runwei Guan,Xiaolou Sun,Haocheng Zhao,Haiyun Jiang,Tao Huang,Henghui Ding,Qing-Long Han*

Main category: cs.RO

TL;DR: 该论文介绍了AerialMind，首个用于无人机场景的大规模参考多目标跟踪（RMOT）基准，并提出了半自动化标注工具COALA和新型跟踪方法HawkEyeTrack（HETrack）。


<details>
  <summary>Details</summary>
Motivation: 当前的RMOT研究主要局限于地面场景，限制了其捕捉广阔场景上下文和执行全面跟踪与路径规划的能力。无人机凭借其广阔的空中视角和卓越机动性，在广域监控和具身智能中发挥关键作用，因此对具备自然语言交互能力的智能空中系统有迫切需求。

Method: 引入了AerialMind，这是首个针对无人机场景的大规模RMOT基准。开发了创新的半自动化协作智能体辅助标注框架（COALA），以显著降低标注成本并保持质量。提出了HawkEyeTrack（HETrack），一种协同增强视觉-语言表示学习并改善无人机场景感知的创新方法。

Result: 全面的实验验证了所构建数据集的挑战性以及所提出方法的有效性。

Conclusion: AerialMind弥补了无人机场景RMOT研究的空白，并为该领域提供了新的基准和有效的解决方案（HETrack）。
...
### [199] [Dual-Agent Reinforcement Learning for Adaptive and Cost-Aware Visual-Inertial Odometry](https://arxiv.org/abs/2511.21083)
*Feiyang Pan,Shenghe Zheng,Chunyan Yin,Guangbin Dou*

Main category: cs.RO

TL;DR: 本文提出一种基于轻量级强化学习（RL）代理的视觉惯性里程计（VIO）框架，通过智能地控制视觉前端的运行和自适应地融合状态，在保持高精度的同时显著提升效率并减少内存消耗，解决了现有VIO方法在准确性与计算成本之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉惯性里程计（VIO）方法存在一个固有的权衡：基于滤波的方法效率高但容易漂移，而基于优化的方法虽然准确但依赖于计算成本高昂的视觉惯性束调整（VIBA），难以在资源受限平台上运行。研究旨在减少VIBA的调用频率和强度，而非完全移除它。

Method: 本文将现代VIO中的两个关键设计选择（何时运行视觉前端以及多大程度信任其输出）视为序列决策问题，并使用轻量级强化学习（RL）代理来解决。核心贡献是一个双管齐下的RL策略：1) 一个“选择代理”（Select Agent）仅基于高频IMU数据智能地控制整个VO管道的运行；2) 一个复合的“融合代理”（Fusion Agent）首先通过一个监督网络估计鲁棒的速度状态，然后通过一个RL策略自适应地融合完整的(p, v, q)状态。

Result: 在EuRoC MAV和TUM-VI数据集上的实验表明，在统一评估中，所提出的方法比之前的GPU-based VO/VIO系统实现了更有利的精度-效率-内存权衡：它获得了最佳的平均ATE，运行速度提高了1.77倍，并使用了更少的GPU内存。与经典的基于优化的VIO系统相比，该方法在保持竞争性轨迹精度的同时，大幅降低了计算负载。

Conclusion: 本文提出的RL-based VIO框架通过智能地管理视觉前端的调用和状态融合，有效地解决了VIO中的精度与效率之间的权衡问题，在准确性、效率和内存使用方面实现了更优的平衡。
...
### [200] [SocialNav: Training Human-Inspired Foundation Model for Socially-Aware Embodied Navigation](https://arxiv.org/abs/2511.21135)
...
TL;DR: SocialNav是一个分层“大脑-行动”架构的基础模型，通过大规模数据集和多阶段训练（包括模仿学习和基于流的强化学习），实现了遵循社会规范的具身导航，显著提高了导航成功率和社交合规性。


<details>
  <summary>Details</summary>
Motivation: 具身导航在遵守社会规范方面仍是一个开放的研究挑战。

Method: 提出了SocialNav模型，采用分层“大脑-行动”架构，能够理解高层社会规范并生成低层社交合规轨迹。构建了包含700万样本的SocNav数据集，其中包括提供社会推理信号的认知激活数据集和聚合多样导航演示的专家轨迹金字塔。设计了多阶段训练流程：首先通过模仿学习注入通用导航技能和社会规范理解，然后通过SAFE-GRPO（首个明确奖励社交合规行为的基于流的强化学习框架）精炼这些技能。

Result: 与现有最佳方法相比，SocialNav的导航成功率提高了38%，社交合规率提高了46%，在导航性能和社交合规性方面均取得了显著提升。

Conclusion: SocialNav通过其创新的分层架构、大规模数据集和多阶段训练方法，有效解决了社会规范具身导航的挑战，并在实际应用中展现出强大的导航性能和社交合规性。
...
### [201] [Maglev-Pentabot: Magnetic Levitation System for Non-Contact Manipulation using Deep Reinforcement Learning](https://arxiv.org/abs/2511.21149)
*Guoming Huang,Qingyi Zhou,Dianjing Liu,Shuai Zhang,Ming Zhou,Zongfu Yu*

Main category: cs.RO

TL;DR: 本文提出了一种名为Maglev-Pentabot的磁悬浮系统，利用深度强化学习实现了克级物体的非接触式灵活操控，解决了现有微观操作的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前的柔性二维和三维非接触式操作技术大多局限于微观尺度，通常只能控制毫克级的物体。研究的动机是突破这一限制，实现对克级物体的灵活操控。

Method: 开发了Maglev-Pentabot磁悬浮系统。通过数值分析优化了电磁铁排布以最大化可控空间。引入了动作重映射方法，以解决磁场强度强非线性导致的样本稀疏性问题，使深度强化学习控制器能够收敛。

Result: 实验结果展示了灵活的操作能力，并且系统能够泛化到未明确训练过的运输任务。此外，该方法可以通过使用更大的电磁铁来操纵更重的物体。

Conclusion: Maglev-Pentabot系统为工业级机器人应用提供了一个参考框架，实现了克级物体的非接触式操控，并展现了良好的泛化性和可扩展性。
...
### [202] [MarketGen: A Scalable Simulation Platform with Auto-Generated Embodied Supermarket Environments](https://arxiv.org/abs/2511.21161)
*Xu Hu,Yiyang Feng,Junran Peng,Jiawei He,Liyi Chen,Chuanchen Luo,Xucheng Yin,Qing Li,Zhaoxiang Zhang*

Main category: cs.RO

TL;DR: MarketGen是一个可扩展的超市环境模拟平台，通过新颖的代理驱动程序化内容生成（PCG）框架、多模态输入和丰富的3D资产库，自动生成逼真的超市场景。它还提出了针对收银员和销售员的复杂基准任务，并通过实验和模拟到现实的迁移验证了其有效性，旨在加速复杂商业环境中具身AI的研究。


<details>
  <summary>Details</summary>
Motivation: 现有机器人数据集和基准主要关注家庭或桌面环境，任务周期短，这阻碍了复杂商业环境中具身智能体的发展。本研究旨在填补这一空白。

Method: 引入MarketGen模拟平台，该平台具有代理驱动的程序化内容生成（PCG）框架，支持多模态输入（文本和参考图像），并结合真实世界设计原则自动生成完整、结构化和逼真的超市。提供了包含1100多种商品和参数化设施的3D资产库。在此基础上，提出了一个评估超市智能体的新基准，包含两个日常任务：结账卸货（针对收银员智能体的长周期桌面任务）和货架商品收集（针对销售员智能体的复杂移动操作任务）。通过部署模块化智能体系统和成功的模拟到现实迁移进行验证。

Result: MarketGen成功生成了完整、结构化和逼真的超市环境。通过广泛的实验，包括部署模块化智能体系统和成功的模拟到现实迁移，验证了该平台和基准的有效性。

Conclusion: MarketGen提供了一个全面的框架，旨在加速复杂商业应用中具身AI的研究。
...
### [203] [Dual Preintegration for Relative State Estimation](https://arxiv.org/abs/2511.21189)
*Ruican Xia,Hailong Pei*

Main category: cs.RO

TL;DR: 本文提出了一种名为“双预积分”的新方法，通过整合两个平台的IMU预积分来提高相对状态估计的精度和鲁棒性，有效解决了传统方法在非线性旋转和大距离下因线性化误差导致的精度下降问题。


<details>
  <summary>Details</summary>
Motivation: 在相对状态估计中，现有方法对参考平台的非线性旋转和平台间大距离敏感，尤其是在虚拟现实（VR）等应用中，会导致显著的定位误差。累积的线性化误差会引入漂移并使估计器不一致。IMU预积分在里程计中能有效缓解线性化误差，但需要为相对状态估计开发类似机制。

Method: 基于IMU预积分理论，本文提出“双预积分”方法，该方法整合了来自两个平台的IMU预积分，作为连续相对状态的运动学约束，并支持高效的重新线性化。此外，还对状态进行了可观测性分析，并解析地推导了相应的零空间。

Result: 通过仿真和真实世界实验（VR控制器跟踪），结果表明，与现有最先进（SOTA）算法相比，所提出的方法在偏置可观测性、非线性旋转和背景纹理等条件下，表现出更高的精度和鲁棒性。

Conclusion: 双预积分方法显著提高了相对状态估计的精度和鲁棒性，尤其是在参考平台进行非线性旋转以及平台间距离较大的挑战性场景中，为解决VR等应用中的定位误差问题提供了有效方案。
...
### [204] [Kinematics-Aware Multi-Policy Reinforcement Learning for Force-Capable Humanoid Loco-Manipulation](https://arxiv.org/abs/2511.21169)
*Kaiyan Xiao,Zihan Xu,Cheng Zhe,Chengju Liu,Qijun Chen*

Main category: cs.RO

TL;DR: 本文提出一种基于强化学习的人形机器人运动-操作框架，旨在解决高负载工业场景中对灵巧操作和主动力交互的综合需求。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人运动-操作方法主要侧重于灵巧操作，未能有效结合高负载工业场景所需的灵巧性和主动力交互能力。

Method: 本文提出一个基于强化学习的框架，采用解耦的三阶段训练管道，包括上身策略、下身策略和增量命令策略。为加速上身训练，设计了嵌入正向运动学先验的启发式奖励函数；为下身训练开发了基于力的课程学习策略，使机器人能主动施加和调节交互力。

Result: 通过该方法，上身策略训练收敛更快并实现卓越性能；下身策略使机器人能够主动施加和调节与环境的交互力。

Conclusion: 该框架通过结合强化学习、解耦训练和特定奖励/课程学习策略，有效弥补了人形机器人在高负载工业场景中运动-操作能力方面的不足，实现了灵巧性和主动力交互的结合。
...
### [205] [Transformer Driven Visual Servoing and Dual Arm Impedance Control for Fabric Texture Matching](https://arxiv.org/abs/2511.21203)
*Fuyuki Tokuda,Akira Seino,Akinari Kobayashi,Kai Tang,Kazuhiro Kosuge*

Main category: cs.RO

TL;DR: 本文提出了一种利用双臂机械手和灰度相机，结合Transformer驱动的视觉伺服和双臂阻抗控制，实现织物片高精度对齐和放置的方法，即使在零样本部署下也能匹配不同纹理的织物。


<details>
  <summary>Details</summary>
Motivation: 在工业应用中，需要精确地对齐并放置织物片，使其表面纹理准确匹配，同时在放置过程中保持织物平整。

Method: 该方法结合了Transformer驱动的视觉伺服与双臂阻抗控制，以同时控制织物姿态并在施加张力的同时将其放置。核心是一个基于Transformer的网络，包含预训练骨干网络和新引入的差异提取注意力模块（DEAM），显著提高了姿态差异预测精度。该网络完全在合成图像上训练，实现了真实世界的零样本部署。

Result: DEAM显著提高了姿态差异预测的准确性。在合成数据上训练的网络实现了真实世界的零样本部署，无需针对特定织物纹理进行预训练。真实世界实验证明，所提出的系统能够准确对齐具有不同纹理的织物片。

Conclusion: 所提出的系统通过结合Transformer驱动的视觉伺服和双臂阻抗控制，以及基于合成数据训练的Transformer网络，成功实现了对不同纹理织物片的高精度对齐和放置，并支持零样本部署。
...
### [206] [Sampling-Based Optimization with Parallelized Physics Simulator for Bimanual Manipulation](https://arxiv.org/abs/2511.21264)
*Iryna Hurova,Alinjar Dan,Karl Kruusamäe,Arun Kumar Singh*

Main category: cs.RO

TL;DR: 本文提出了一种基于采样的优化框架，利用GPU加速的物理模拟器作为世界模型，解决了双臂操纵任务在杂乱环境中的泛化难题，并实现了实时性能和从仿真到真实的迁移。


<details>
  <summary>Details</summary>
Motivation: 现有端到端学习的双臂操纵方法难以泛化到新场景，尤其是在有障碍物的复杂环境中。

Method: 提出了一种替代范式：基于采样的优化框架，使用GPU加速的物理模拟器作为世界模型。具体方法是定制化的模型预测路径积分控制（MPPI）算法，该算法由精心设计的任务特定成本函数指导，并利用GPU加速的MuJoCo高效评估机器人-物体交互。

Result: 该方法能够解决存在静态障碍物的复杂双臂操纵任务，成功应用于PerAct²基准测试中更具挑战性的任务（如通过障碍物路径传输球），在商用GPU上实现了实时性能，并通过利用MuJoCo的独特功能促进了成功的仿真到真实迁移。

Conclusion: 论文通过对样本复杂度和鲁棒性进行统计分析，量化了该方法的性能，并确认其能够有效解决复杂双臂操纵任务的泛化问题。
...
### [207] [Neural NMPC through Signed Distance Field Encoding for Collision Avoidance](https://arxiv.org/abs/2511.21312)
*Martin Jacquet,Marvin Harms,Kostas Alexis*

Main category: cs.RO

TL;DR: 本文提出了一种基于神经网络的非线性模型预测控制（NMPC）框架，用于无人机在未知环境中进行无图、无碰撞导航，利用机载测距传感器将测距图像编码为符号距离函数（SDF）以实现避障。


<details>
  <summary>Details</summary>
Motivation: 在未知环境中，利用机载测距传感器实现无人机的无图、无碰撞导航是一个挑战。

Method: 该方法引入了一个神经NMPC框架。核心是利用深度神经网络将单帧测距图像编码成符号距离函数（SDF）。其神经网络架构包括一个卷积编码器（将图像压缩为低维潜在向量）和一个多层感知器（将潜在向量近似为空间SDF）。SDF被用作显式位置约束以避免碰撞，并嵌入到速度跟踪NMPC中，输出推力和姿态指令。研究还进行了NMPC的理论分析，验证了在固定观测下的递归可行性和稳定性。通过仿真（包括消融研究、与现有方法的比较以及对里程计漂移的评估）和真实世界实验（在森林环境中）来评估学习组件的开环性能和控制器的闭环性能。

Result: 理论分析验证了所提出的NMPC的递归可行性和稳定性。仿真研究表明，该方法在碰撞避免方面表现出色，优于两种最先进的局部导航方法，并对里程计漂移具有鲁棒性。真实世界实验在森林环境中进行，证明了该神经NMPC在杂乱环境中能有效执行碰撞避免，即使面对对抗性参考速度输入和漂移的位置估计。

Conclusion: 所提出的神经NMPC框架能够使无人机在未知、杂乱的环境中有效实现无图、无碰撞导航，并表现出对里程计漂移和对抗性输入的鲁棒性。
...
### [208] [Improvement of Collision Avoidance in Cut-In Maneuvers Using Time-to-Collision Metrics](https://arxiv.org/abs/2511.21280)
*Jamal Raiyn*

Main category: cs.RO

TL;DR: 本文提出了一种结合深度学习和碰撞时间（TTC）指标的新型防撞策略，专门用于处理自动驾驶车辆面临的切入场景。


<details>
  <summary>Details</summary>
Motivation: 切入场景对自动驾驶车辆（AVs）来说极具挑战性，且传统基于TTC的方法存在局限性。

Method: 该系统通过整合深度学习与TTC计算，预测潜在碰撞并确定适当的规避动作。

Result: 与传统基于TTC的方法相比，该系统能够更有效地预测潜在碰撞并采取规避行动。

Conclusion: 该研究提出了一种通过结合深度学习和TTC来改进自动驾驶车辆在切入场景下防撞能力的新策略。
...
### [209] [Hybrid Control for Robotic Nut Tightening Task](https://arxiv.org/abs/2511.21366)
*Dmitri Kovalenko*

Main category: cs.RO

TL;DR: 本文提出一种用于串联机械臂和并联夹持器的自主机器人螺母拧紧系统，该系统结合了分层运动基元规划器和力-位置控制切换方案，实现了更快的拧紧速度和更小的接触力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一种自主机器人系统，能够高效且精确地执行螺母拧紧任务，以提高自动化装配的性能和鲁棒性。

Method: 该系统采用分层运动基元规划器来规划机器人运动，并结合了在力控制和位置控制之间切换的控制方案。通过广泛的仿真来验证系统性能。

Result: 仿真结果表明，该系统对初始条件的变化具有鲁棒性。与基线相比，所提出的控制器拧紧螺纹螺钉的速度快14%，同时施加在操作物上的接触力减少40倍。系统实现已开源。

Conclusion: 所提出的自主机器人螺母拧紧系统在速度、接触力和鲁棒性方面表现出色，为自动化装配任务提供了高效且精确的解决方案。
...
### [210] [$\mathcal{E}_0$: Enhancing Generalization and Fine-Grained Control in VLA Models via Continuized Discrete Diffusion](https://arxiv.org/abs/2511.21542)
...
TL;DR: E0是一种连续化的离散扩散框架，通过量化动作令牌的迭代去噪，解决了现有VLA模型在泛化性、精细控制和稳定性方面的不足，实现了机器人操作的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型在跨任务、场景和摄像机视角的泛化能力方面表现不佳，并且经常产生粗糙或不稳定的动作。

Method: 本文提出了E0，一个连续化的离散扩散框架，将动作生成表述为对量化动作令牌的迭代去噪。E0的优势在于：1) 离散动作令牌与预训练VLM/VLA骨干的符号结构自然对齐，实现更强的语义条件作用；2) 离散扩散与真实世界机器人控制的量化特性相匹配，通过贝叶斯最优去噪器建模正确的离散动作分布，从而增强泛化能力。与离散自回归和基于掩码的离散扩散模型相比，E0支持更大、更细粒度的动作词汇，并避免了掩码引入的分布不匹配。此外，引入了球形视角扰动增强方法以提高对相机位移的鲁棒性。

Result: E0在LIBERO、VLABench和ManiSkill上的14个多样化环境中实现了最先进的性能，平均超越强基线10.7%。在Franka机械臂上的真实世界评估证实，E0提供了精确、鲁棒和可迁移的操作。

Conclusion: 离散扩散（E0）被确立为可泛化VLA策略学习的一个有前景的方向，能够实现精确、鲁棒和可迁移的机器人操作。
...
### [211] [Model-Based Policy Adaptation for Closed-Loop End-to-End Autonomous Driving](https://arxiv.org/abs/2511.21584)
*Haohong Lin,Yunzhi Zhang,Wenhao Ding,Jiajun Wu,Ding Zhao*

Main category: cs.RO

TL;DR: 端到端（E2E）自动驾驶模型在闭环环境中表现不佳，本文提出模型基策略适应（MPA）框架，通过生成反事实轨迹、训练扩散策略适配器和多步Q值模型，显著提升了预训练E2E驾驶代理的鲁棒性和安全性。


<details>
  <summary>Details</summary>
Motivation: 端到端（E2E）自动驾驶模型在开环评估中表现良好，但在闭环设置中常因级联错误和泛化能力差而受挫，缺乏鲁棒性和安全性。

Method: MPA框架首先使用几何一致的模拟引擎生成多样化的反事实轨迹。然后，基于这些数据训练一个基于扩散的策略适配器来细化基础策略预测，并训练一个多步Q值模型来评估长期结果。在推理时，适配器提出多个轨迹候选，Q值模型选择预期效用最高的轨迹。

Result: 在nuScenes基准上使用逼真的闭环模拟器进行的实验表明，MPA在域内、域外和安全关键场景中显著提高了性能。研究还探讨了反事实数据的规模和推理时引导策略对整体有效性的影响。

Conclusion: MPA通过生成反事实数据和模型基策略适应，成功解决了E2E自动驾驶模型在闭环部署中面临的鲁棒性和安全性挑战，显著提升了代理的表现。
...
### [212] [VacuumVLA: Boosting VLA Capabilities via a Unified Suction and Gripping Tool for Complex Robotic Manipulation](https://arxiv.org/abs/2511.21557)
*Hui Zhou,Siyuan Huang,Minxing Li,Hao Zhang,Lue Fan,Shaoshuai Shi*

Main category: cs.RO

TL;DR: 本文提出了一种低成本、集成化的双模末端执行器，结合了机械两指夹持器和真空吸盘，以克服传统两指夹持器在视觉语言动作（VLA）机器人操作中的局限性，从而扩展了机器人可执行的任务范围。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数VLA系统主要使用并联两指夹持器，但在处理某些实际任务（如擦拭玻璃表面或打开无把手抽屉）时，由于接触面积不足或缺乏附着力而面临固有限制。

Method: 研究人员设计了一种低成本、集成化的硬件，将机械两指夹持器与真空吸盘单元结合在一个末端执行器中，实现了双模操作。该系统支持灵活切换或协同使用两种模式。该设计在DexVLA和Pi0两个先进的VLA框架中进行了效率和实用性验证。

Result: 实验结果表明，通过提出的混合末端执行器，机器人可以成功执行多种传统两指夹持器单独无法完成的复杂任务。

Conclusion: 该混合末端执行器设计有效克服了传统两指夹持器的局限性，显著扩展了VLA系统可执行任务的范围，并展现了其效率和实用性。所有硬件设计和控制系统都将发布。
...
### [213] [Uncertainty Quantification for Visual Object Pose Estimation](https://arxiv.org/abs/2511.21666)
*Lorenzo Shaikewitz,Charis Georgiou,Luca Carlone*

Main category: cs.RO

TL;DR: 本文提出SLUE，一种在单目设置下无需严格分布假设，通过凸优化程序为物体姿态估计提供高概率的无分布不确定性边界（椭球）。


<details>
  <summary>Details</summary>
Motivation: 姿态估计的不确定性量化对于鲁棒控制和规划至关重要。然而，在没有严格分布假设的情况下，附加统计上严格的不确定性仍然是一个未被充分理解的问题。

Method: SLUE（S-Lemma Uncertainty Estimation）利用已知物体2D语义关键点像素检测的高概率噪声边界，推导出一个隐式的、非凸的姿态不确定性约束集。它通过一个受S-引理启发的凸程序，将该集合简化为一个椭球不确定性边界，并保证以高概率包含真实物体姿态。为获得更紧密的边界，SLUE扩展到平方和松弛层次结构。该方法无需对边界形状或大小进行初始猜测，并能将姿态不确定性投影到独立的平移和轴角方向边界。

Result: SLUE在两个姿态估计数据集和一个真实的无人机跟踪场景中进行了评估。与现有工作相比，SLUE生成了显著更小的平移边界和具有竞争力的方向边界。

Conclusion: SLUE提供了一种无需分布假设，能以高概率保证包含真实姿态的、统计上严格的姿态不确定性量化方法，其生成的边界更紧密、更鲁棒。
...
### [214] [TraceGen: World Modeling in 3D Trace Space Enables Learning from Cross-Embodiment Videos](https://arxiv.org/abs/2511.21690)
...
TL;DR: 该研究引入了TraceGen，一个使用统一的3D“轨迹空间”符号表示的世界模型，使得机器人能够从少量、跨实体、跨环境和跨任务的视频中学习新任务，并实现高效适应和高成功率。


<details>
  <summary>Details</summary>
Motivation: 在新的平台和场景中，机器人仅通过少量演示学习新任务仍然具有挑战性。尽管存在大量其他实体（人类和不同机器人）的视频，但由于实体、摄像机和环境的差异，这些视频无法直接使用。研究旨在解决小数据问题，并利用这些异构视频资源。

Method: 研究引入了一种统一的、符号化的3D“轨迹空间”表示，它是一种紧凑的场景级轨迹表示，抽象了外观但保留了几何结构。TraceGen是一个世界模型，它在轨迹空间而非像素空间预测未来运动。为大规模训练TraceGen，研究开发了TraceForge数据管道，将异构的人类和机器人视频转换为一致的3D轨迹，生成了一个包含12.3万个视频和180万个观测-轨迹-语言三元组的语料库。在此语料库上进行预训练以获得可迁移的3D运动先验。

Result: 通过仅五个目标机器人视频，TraceGen在四项任务中达到了80%的成功率，并且推理速度比最先进的基于视频的世界模型快50-600倍。在更具挑战性的情况下，仅有五个用手持手机拍摄的未校准人类演示视频时，它在真实机器人上仍能达到67.5%的成功率，这凸显了TraceGen在不依赖物体检测器或大量像素空间生成的情况下跨实体适应的能力。

Conclusion: TraceGen通过引入统一的3D轨迹空间表示和构建大规模异构数据集，成功解决了机器人学习中的小数据问题。它实现了高效的跨实体、跨环境和跨任务学习，仅需少量演示即可展现出强大的性能和更快的推理速度。
...
### [215] [Distributionally Robust Cascading Risk in Multi-Agent Rendezvous: Extended Analysis of Parameter-Induced Ambiguity](https://arxiv.org/abs/2511.20914)
*Vivek Pandey,Nader Motee*

Main category: eess.SY

TL;DR: 本文提出了一个理论框架，用于分析多智能体在通信延迟和参数不确定性下执行会合等时间关键任务时，级联故障的分布鲁棒风险，并导出了一个闭式风险表达式。


<details>
  <summary>Details</summary>
Motivation: 在通信延迟和系统参数不确定性下，确保自主多智能体系统在会合等时间关键任务中的安全性是一个基本挑战。

Method: 开发了一个理论框架来分析多智能体会合中级联故障的分布鲁棒风险，其中系统参数存在有界不确定性。使用时间延迟动态网络作为基准模型，引入了一个基于二元高斯模型的条件分布鲁棒泛函来表征智能体之间的风险传播，并导出了一个闭式风险表达式。

Result: 量化了参数微小偏差对集体安全的影响。导出的闭式风险表达式捕捉了时间延迟、网络结构、噪声统计和故障模式之间的复杂相互作用，揭示了关键的敏感性模式，并通过广泛的模拟验证了理论结果。

Conclusion: 该框架为设计鲁棒和有弹性的多智能体网络提供了可操作的见解。
...
### [216] [Local Dissipativity Analysis of Nonlinear Systems](https://arxiv.org/abs/2511.20838)
*Amy K. Strong,Leila Bridgeman*

Main category: eess.SY

TL;DR: 本文提出了一种通用的方法，通过凸优化问题（线性矩阵不等式LMI）来确定非线性、控制仿射系统的局部耗散性，并同时合成连续分段仿射（CPA）或结合二次/CPA的存储函数。


<details>
  <summary>Details</summary>
Motivation: 耗散性是描述非线性系统的输入-输出特性，对于组合式鲁棒控制至关重要。然而，确定系统的耗散性通常是一个复杂且依赖于模型的特定过程，缺乏通用方法。

Method: 该方法将Hamilton-Jacobi不等式与耗散性不等式重新表述为LMI，并通过凸优化问题同时搜索最优的输入-输出特性并合成CPA存储函数。它开发了用于三角剖分的新型LMI界限，并提出了一种合成结合二次和CPA存储函数的方法，以扩大适用范围。

Result: 该方法能够同时找到系统的最优输入-输出特性并合成CPA存储函数。通过引入结合二次和CPA的存储函数，扩大了优化问题的适用系统范围。此外，本文证明了如果系统是严格局部耗散的，该方法总能找到一个可行的输入-输出特性和对应的CPA或二次存储函数。

Conclusion: 所提出的基于LMI的凸优化方法为非线性、控制仿射系统提供了一种通用且可靠的手段，用于确定局部耗散性并合成存储函数，从而简化了组合式鲁棒控制中耗散性分析的复杂性。
...
### [217] [Dynamic Modeling of Load Demand in Electrified Highways Based on the EV Composition](https://arxiv.org/abs/2511.20874)
*Ashutossh Gupta,Vassilis Kekatos,Dionysios Aliprantis,Steve Pekarek*

Main category: eess.SY

TL;DR: 本研究建立了电动道路（ER）上动态无线电力传输（DWPT）负载的时域和频域模型，分析了电动汽车（EV）速度、接收线圈长度和车辆组成对谐波的影响，并为电网运营商和ER设计者提供了见解。


<details>
  <summary>Details</summary>
Motivation: 电动道路上的动态无线电力传输（DWPT）技术能延长电动汽车续航里程并减少电池需求，但其固有的振荡功率特性对电力系统动态研究提出了挑战。因此，理解DWPT总负载的动态行为对电力系统研究至关重要。

Method: 研究方法包括：为恒定电动汽车速度下的单个电动汽车负载建立时域和频域模型；比较了现有DWPT电动汽车中非线性与线性控制方案的频率谐波特性；提出并分析了ER路段总DWPT负载的随机模型，以解释电动汽车组成对其频谱的影响；使用交通模拟器的真实流量数据验证了分析结果。

Result: 研究发现：电动汽车中实现的非线性控制方案比线性方案产生的频率谐波更温和；电动汽车负载的谐波幅度随接收线圈长度的增加而减小；ER上的电动汽车组成会影响其频率频谱；有趣的是，为更多具有更长接收线圈的电动汽车（如卡车）供电并不一定意味着更温和的谐波。分析结果得到了交通模拟器数据的证实。

Conclusion: 本研究通过建立动态无线电力传输负载模型，揭示了电动汽车控制方案、接收线圈长度和车辆组成对电力系统谐波特性的影响。这些发现为电网运营商和电动道路设计者提供了宝贵的见解，有助于优化系统设计和运行。
...
### [218] [Adaptive Gradient Descent MPPT Algorithm With Complexity-Aware Benchmarking for Low-Power PV Systems](https://arxiv.org/abs/2511.20895)
*Kimia Ahmadi,Wouter A. Serdijn*

Main category: eess.SY

TL;DR: 本文提出了一种针对低功率光伏系统的高效、实时最大功率点跟踪（MPPT）算法。该算法在扰动观察（P&O）基础上增加了自适应梯度下降机制和可选的初始化程序，显著提高了在快速辐照变化和部分阴影条件下的跟踪效率和速度，且计算成本低，性能优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 低功率光伏系统在快速变化的辐照和部分阴影条件下，需要计算高效、实时的最大功率点跟踪（MPPT）算法。

Method: 该方法通过自适应梯度下降机制增强了经典的扰动观察（P&O）算法，根据瞬时功率-电压斜率动态调整扰动步长，以减少跟踪时间和稳态振荡。同时，引入了一个可选的初始化程序，以增强在部分阴影条件下的全局最大功率点（GMPP）跟踪能力。

Result: 该算法在标准测试条件下（STC）实现了99.94%的MPPT效率，在实验数据下达到99.21%，在测试温度范围内超过99.6%。在部分阴影条件下，初始化程序可将跟踪效率提高高达7.8%。与35种最先进的基于P&O的MPPT算法相比，该算法在效率、跟踪时间和计算成本方面均表现更优。

Conclusion: 该算法具有鲁棒、拓扑无关的性能，且计算效率高，非常适合集成到在动态和资源受限条件下运行的低功率电源管理集成电路（PMIC）中。
...
### [219] [Data-Driven Post-Event Analysis with Real-World Oscillation Data from Denmark](https://arxiv.org/abs/2511.20939)
*Youhong Chen,Debraj Bhattacharjee,Balarko Chaudhuri,Mark O Malley,Nan Qin,Adrian Pilkaer Expethit*

Main category: eess.SY

TL;DR: 本文展示了扩展动态模态分解（EDMD）如何利用PMU数据有效识别电网振荡的主要贡献者，并在丹麦的真实振荡事件中得到验证，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 识别电网振荡（特别是低频振荡）的主要贡献者对于采取有针对性的缓解措施至关重要。传统的分析方法可能无法清晰地识别出这些贡献者，尤其是在可再生能源并网（IBR）系统日益增多的背景下。

Method: 研究采用基于Koopman算子理论的扩展动态模态分解（EDMD）算法。该算法仅处理来自丹麦电网中19个PMU的电压和电流相量数据，对一次真实的0.15 Hz振荡事件进行事后分析，且不使用任何补充系统信息（盲测设置）。

Result: EDMD准确识别出0.2 Hz振荡的主要贡献者位置，与Energinet后来确认的问题IBR电厂（其根本原因是控制系统问题）的位置一致。相比之下，传统的耗散能量流（DEF）方法未能清晰地识别出该电厂。这一结果与Energinet的联合验证，以及之前在模拟IBR系统和ISO-NE真实PMU数据上的研究结果一致。

Conclusion: 基于EDMD的事后分析在识别主要振荡贡献者方面具有巨大潜力，并能实现有针对性的次同步振荡（SSO）缓解，尤其适用于IBR主导的电力系统。
...
### [220] [Independent policy gradient-based reinforcement learning for economic and reliable energy management of multi-microgrid systems](https://arxiv.org/abs/2511.20977)
*Junkai Hu,Li Xia*

Main category: eess.SY

TL;DR: 本研究针对多微电网系统（MMSs）的能源管理问题，提出了一种分布式方法，通过将经济性和可靠性指标纳入均值-方差团队随机博弈（MV-TSG）框架，并开发了基于独立策略梯度的算法（包括模型已知时的传统算法和模型未知时的深度强化学习算法），以实现经济性能和运行可靠性之间的平衡。


<details>
  <summary>Details</summary>
Motivation: 在集成间歇性可再生能源的多微电网系统中，能源管理的效率和可靠性至关重要。现有方法在处理方差指标时存在不足，无法有效平衡经济性能和可靠性。

Method: 本研究将能量管理问题建模为均值-方差团队随机博弈（MV-TSG），其中系统与主电网交换功率的均值和方差分别作为经济性能和可靠性指标。针对模型参数已知的情况，提出了一种完全分布式的独立策略梯度算法；对于大规模且模型参数未知的情况，进一步开发了基于独立策略梯度的深度强化学习算法，以实现数据驱动的策略优化。

Result: 数值实验验证了所提出方法的有效性。这些方法充分利用了多微电网系统的分布式计算能力，并在经济性能和运行可靠性之间实现了良好的平衡。

Conclusion: 本研究提出的分布式独立策略梯度算法（包括基于模型的和基于深度强化学习的）能够有效解决多微电网系统中的经济可靠性能源管理问题，通过优化均值和方差指标，实现了经济性能与运行可靠性之间的权衡。
...
### [221] [An Exact, Finite Dimensional Representation for Full-Block, Circle Criterion Multipliers](https://arxiv.org/abs/2511.20995)
*Felix Biertümpfel,Bin Hu,Geir Dullerud,Peter Seiler*

Main category: eess.SY

TL;DR: 本文首次为全块圆判据乘子提供了有限维表征，使得离散时间线性时不变系统与扇区有界非线性互连的稳定性分析，能以计算可行的方式实现最不保守的条件。


<details>
  <summary>Details</summary>
Motivation: 全块圆判据乘子能为扇区有界非线性提供最不保守的稳定性条件，但它们由无限多个约束定义，导致计算上不可行。因此，需要一种有限维的表征方法来使其具有计算可行性。

Method: 该研究的关键理论洞察是：非重复扇区有界非线性的所有输入/输出对集合，等同于一个适当构建的分段线性函数的增量对集合。基于此，通过有限数量的矩阵共正性约束，提供了一种新的有限维表征来描述全块圆判据乘子集。

Result: 研究提供了一种新的全块圆判据乘子有限维表征，该表征仅需有限数量的矩阵共正性约束。对于输入/输出维度较小（≤4）的问题，这些条件具有精确且计算可行的实现方案。通过一个简单示例验证了该新表征的有效性。

Conclusion: 通过引入全块圆判据乘子的有限维表征，本文使得针对离散时间LTI系统与扇区有界非线性互连的最不保守稳定性条件，在计算上变得可行，特别适用于输入/输出维度较小的问题。
...
### [222] [From Consensus to Robust Clustering: Multi-Agent Systems with Nonlinear Interactions](https://arxiv.org/abs/2511.21228)
*Anthony Couthures,Gustave Bainier,Vineeth Satheeskumar Varma,Samson Lasaulce,Irinel-Constantin Morarescu*

Main category: eess.SY

TL;DR: 本文建立了一个理论框架，描述了多智能体系统中从共识到稳定聚类的转变，并量化了聚类结构的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 理解在非线性、合作交互的多智能体系统中，共识如何破裂并转变为稳定的聚类状态，以及这些聚类状态的鲁棒性。

Method: 1. 建立共识的尖锐阈值，通过交互的Lipschitz常数与归一化邻接矩阵的第二大特征值之间的不等式来定义。2. 分析交互律的特定结构（如不稳定不动点）对聚类状态出现的影响。3. 引入输入到状态稳定性（ISS）理论来量化聚类状态的鲁棒性。

Result: 1. 确定了共识的尖锐阈值，其违反会导致非同步平衡（聚类）的出现。2. 聚类状态的出现需要交互律具有特定的结构属性。3. 证明了聚类内部的凝聚力对网络其余部分的扰动具有鲁棒性。4. 发现聚类一致性受限于外部影响在内部节点间的异质性，而非其大小。

Conclusion: 提出了一个统一的框架，解释了共识的急剧崩溃和由此产生的模块化结构的可量化鲁棒性，并通过Zachary's Karate Club网络进行了验证。
...
### [223] [Stability of data-driven Koopman MPC with terminal conditions](https://arxiv.org/abs/2511.21248)
*Irene Schimperna,Lea Bold,Johannes Köhler,Karl Worthmann,Lalo Magni*

Main category: eess.SY

TL;DR: 本文推导了在存在近似误差的情况下，使用数据驱动代理模型作为预测模型的模型预测控制（MPC）如何实现渐近稳定，并证明了在满足比例误差界限条件下的递归可行性和渐近稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决在使用数据驱动代理模型作为预测模型时，模型预测控制（MPC）的稳定性问题，因为这些模型通常存在近似误差。

Method: 本文通过推导条件来证明MPC在具有终端条件时，使用数据驱动代理模型作为预测模型能够实现渐近稳定。关键方法是证明在满足比例误差界限（即误差界限与状态和输入的范数呈线性关系）时，MPC的递归可行性和渐近稳定性。具体的数据驱动模型是利用Koopman算子的核扩展动态模态分解（kEDMD）生成的。

Result: 主要结果是证明了如果存在一个与状态和输入范数成比例的误差界限，那么使用数据驱动代理模型的MPC可以实现递归可行性和渐近稳定。对于广泛的非线性系统，通过使用基于Koopman算子的kEDMD生成的数据驱动代理模型可以满足这一条件。该框架的适用性通过数值案例研究得到了验证。

Conclusion: 结论是，即使存在近似误差，使用数据驱动代理模型（特别是基于kEDMD和Koopman算子）的MPC在满足特定比例误差界限的条件下，能够实现渐近稳定和递归可行性，并且该框架在实际应用中是可行的。
...
### [224] [Design and Measurements of mmWave FMCW Radar Based Non-Contact Multi-Patient Heart Rate and Breath Rate Monitoring System](https://arxiv.org/abs/2511.21255)
*Jewel Benny,Pranjal Mahajan,Srayan Sankar Chatterjee,Mohd Wajid,Abhishek Srivastava*

Main category: eess.SY

TL;DR: 本文提出了一种基于FMCW毫米波雷达的非接触式多患者心率(HR)和呼吸率(BR)监测系统，并结合多处理方法和最小二乘解决方案，显著提高了测量精度。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达技术使得非接触式心率和呼吸率测量成为可能，极大地方便了患者监测。同时，这些技术也为同时监测多名患者提供了机会，这对于高效的大规模监测场景变得日益重要。

Method: 研究采用频率调制连续波(FMCW)毫米波雷达构建非接触式多患者HR和BR监测系统。此外，提出了一种新颖的方法，该方法结合了多种处理方法，并利用最小二乘解决方案来提高测量精度、泛化能力并处理测量误差。系统使用德州仪器(Texas Instruments)的FMCW雷达开发。

Result: 实验结果表明，该系统在多名受试者上的呼吸率测量准确率超过97%，心率测量准确率超过93%。

Conclusion: 所提出的FMCW毫米波雷达系统能够实现高精度的非接触式多患者心率和呼吸率监测，并通过结合多处理方法和最小二乘解决方案有效提升了性能。
...
### [225] [Response-Based Frequency Stability Assessment under Multi-Scale Disturbances in High-Renewable Power Systems](https://arxiv.org/abs/2511.21269)
*Jinhui Chen,Huadong Sun,Ping Wu,Baocai Wang,Bing Zhao*

Main category: eess.SY

TL;DR: 本文提出一种基于响应的频率稳定性评估方法，通过发电机电气响应推断扰动功率，统一处理多尺度有功功率扰动。该方法在线识别扰动类型和强度，并利用分析频率响应模型，针对不同扰动类型计算最大容忍扰动功率或频率偏差超限时间，有效评估高可再生能源电力系统的频率稳定性。


<details>
  <summary>Details</summary>
Motivation: 在高可再生能源电力系统中，有功功率扰动变得更大且时间尺度日益多样化，使得在不可预见事件下进行频率稳定性评估变得复杂。

Method: 1. 将不可预见扰动分类为短期和永久事件，其中永久扰动进一步细分为阶跃、秒级斜坡和分钟级斜坡扰动。2. 基于发电机组的实测功率响应，构建统一的扰动功率模型，在线识别扰动类型并量化其强度。3. 为每种扰动类型推导出分析频率响应模型。4. 对于阶跃扰动，在稳态和暂态频率偏差约束下获得最大容忍扰动功率，并定义安全裕度指标。5. 对于斜坡型扰动，使用改进的系统频率响应（SFR）模型和一次调频耗尽后的转子运动方程计算频率偏差超限时间。

Result: 所提出的基于响应的评估方法在CSEE-FS频率稳定性基准系统上得到了验证，结果表明其在高可再生能源电力系统中进行定量频率稳定性评估的有效性和准确性。

Conclusion: 本文成功开发了一种有效且准确的基于响应的频率稳定性评估方法，能够统一处理高可再生能源电力系统中的多尺度扰动，并进行定量的频率稳定性评估。
...
### [226] [Adaptive Lighting Control in Visible Light Systems: An Integrated Sensing, Communication, and Illumination Framework](https://arxiv.org/abs/2511.21271)
*Xinyan Xie,Xuesong Wang,Xin Lai,Yongheng Wen,Fengrui Yang,Haoyang He,Lai Zhang,Dong Zhao*

Main category: eess.SY

TL;DR: 本文提出了一种自适应集成感知、通信和照明（ISCI）框架，用于室内可见光通信（VLC），通过几何分区和动态优化目标切换，在保证性能和用户舒适度的同时，显著节省能源。


<details>
  <summary>Details</summary>
Motivation: 当前可见光通信（VLC）集成感知与通信（ISAC）研究主要侧重于最大化数据速率和感知精度，但这与高能耗和用户视觉舒适度之间存在冲突。

Method: 该框架首先通过几何方法将接收平面划分为活动区域和非活动区域。利用非视距（NLOS）感知确定用户位置，作为系统优化目标的动态开关。系统在活动区域内自适应地最小化总发射功率以保证通信和照明性能，在非活动区域内最大化信噪比（SNR）均匀性。

Result: 数值结果表明，与非自适应系统相比，该自适应ISCI方法实现了53.59%的能量节省，将信噪比均匀性提高了57.79%，同时满足所有照明约束并保持0.071米的平均定位误差。

Conclusion: 该自适应ISCI方法成功解决了VLC系统中高性能、高能耗和用户视觉舒适度之间的冲突，在显著节能的同时，提升了信噪比均匀性并满足了各项性能指标。
...
### [227] [Respiratory Motion Compensation and Haptic Feedback for X-ray-Guided Teleoperated Robotic Needle Insertion](https://arxiv.org/abs/2511.21273)
*Ana Cordon-Avila,Mostafa Selim,Momen Abayazid*

Main category: eess.SY

TL;DR: 该研究提出一种机器人辅助系统，通过运动估计模型补偿呼吸运动，并结合触觉反馈的远程操作进行腹部穿刺，以提高精度并减少辐射暴露。


<details>
  <summary>Details</summary>
Motivation: 呼吸运动限制了腹部经皮手术的准确性和精确性，且存在辐射暴露问题，需要一种更精确、无辐射的远程插入方案。

Method: 使用运动估计模型进行机器人呼吸运动补偿。通过基于邻近度的触觉反馈进行远程操作插入。该方法在机器人肝脏模型上进行了验证。

Result: 运动估计误差在所有方向上均低于3毫米。总体三维插入误差在头-尾、侧向和前后方向上分别为2.60毫米、7.75毫米和2.86毫米。

Conclusion: 所提出的方法有望最大限度地减少因呼吸引起的运动导致的不准确治疗或诊断，并降低辐射暴露。
...
### [228] [Data-Driven Reduction of Fault Location Errors in Onshore Wind Farm Collectors](https://arxiv.org/abs/2511.21300)
*A. J. Alves Junior,M. J. B. B. Davi,R. A. S. Fernandes,M. Oleskovicz,D. V. Coury*

Main category: eess.SY

TL;DR: 本文提出一种先进的机器学习方案，通过门控残差网络校正模型，显著提高风电场集电网络中的故障定位精度，克服了传统方法在逆变器并网系统中的局限性。


<details>
  <summary>Details</summary>
Motivation: 风电场中逆变器并网资源的日益集成改变了故障期间的电流和电压行为，使得传统基于相量的诊断方法难以有效进行准确的故障定位，从而影响运行可靠性和快速恢复。

Method: 该方法通过引入一个由门控残差网络（Gated Residual Network）驱动的校正模型来增强确定性故障距离估计器，以最小化残余故障定位误差。通过全面的特征工程和选择，该预测器在PSCAD模拟的真实风电场模型中，使用包含多种故障场景（故障类型、电阻、位置、起始角、发电渗透率）的数据集进行训练。超参数优化采用Optuna框架，并通过统计方法验证了其鲁棒性。

Result: 结果显示，与现有最先进的方法相比，故障定位误差总体降低了76%，准确性显著提高。该方法还表现出强大的可扩展性以及对拓扑和运行变化的适应性。

Conclusion: 该方法推进了现代电力系统数据驱动型故障定位框架的部署，为风电场网络提供了更可靠、高效的故障诊断解决方案。
...
### [229] [Sparse shepherding control of large-scale multi-agent systems via Reinforcement Learning](https://arxiv.org/abs/2511.21304)
*Luigi Catello,Italo Napolitano,Davide Salzano,Mario di Bernardo*

Main category: eess.SY

TL;DR: 本文提出一种基于强化学习的稀疏间接控制框架，通过耦合ODE和PDE实现对大规模多智能体系统的多尺度控制，并在数值验证中展现出高效且鲁棒的密度控制能力。


<details>
  <summary>Details</summary>
Motivation: 解决大规模多智能体系统中少数受控智能体如何影响多数非受控智能体的集体行为的挑战，克服稀疏驱动限制，并寻找替代计算成本高昂的在线优化方法。

Method: 提出一个强化学习框架，用于稀疏间接控制大规模多智能体系统。该方法通过将描述受控智能体的常微分方程（ODE）与描述非受控群体密度的偏微分方程（PDE）耦合，来解决多尺度问题。具体结合了无模型强化学习与自适应交互强度补偿，以克服稀疏驱动的局限性。

Result: 数值验证表明，该系统能够实现有效的密度控制，达到目标分布，并对扰动和测量噪声保持鲁棒性。

Conclusion: 基于学习的稀疏控制方法可以有效替代计算成本高昂的在线优化，实现大规模多智能体系统的多尺度控制。
...
### [230] [Design and Performance Assessment of a Virtualized IED for Digital Substations](https://arxiv.org/abs/2511.21310)
*Alailton J. Alves Junior,Denis V. Coury,Ricardo A. S. Fernandes*

Main category: eess.SY

TL;DR: 本文开发并评估了一种虚拟智能电子设备（vIED），旨在匹配硬件IED的性能。通过实时仿真，结果表明vIEDs在数字变电站的关键时间敏感环境中具有可接受的响应时间，证明了其适用性。


<details>
  <summary>Details</summary>
Motivation: 数字变电站通过光纤通信和IEC 61850 IEDs提高了效率、可靠性和互操作性，但仍面临成本高、网络复杂、升级受限等挑战。虚拟化IEDs（vIEDs）作为一种经济高效的解决方案出现，可提供可扩展性、简化维护并降低硬件成本，但其性能和可靠性需严格评估以确保在实时应用中的鲁棒性。

Method: 本文开发、实现并评估了一个vIED，旨在匹配其硬件对应物的性能。该vIED部署在服务器上的虚拟机中，其核心逻辑采用低级编程语言实现，以确保高速、确定性行为。通过实时仿真评估了其性能，重点关注保护功能的响应时间。

Result: 评估结果表明，所开发的vIEDs实现了可接受的响应时间。

Conclusion: vIEDs适用于数字变电站中关键时间敏感环境的部署。
...
### [231] [Scalable Multisubject Vital Sign Monitoring With mmWave FMCW Radar and FPGA Prototyping](https://arxiv.org/abs/2511.21314)
*Jewel Benny,Narahari N. Moudhgalya,Mujeev Khan,Hemant Kumar Meena,Mohd Wajid,Abhishek Srivastava*

Main category: eess.SY

TL;DR: 本文提出了一种基于FMCW雷达的非接触式多目标生命体征同步估计方法，并通过FPGA实现展示了其高性能和便携性。


<details>
  <summary>Details</summary>
Motivation: 传统生命体征监测方法存在佩戴不适、校准困难和感染风险等局限性，促使研究人员寻求一种适用于各种关键场景的多功能、非接触式监测解决方案。

Method: 采用基于FMCW雷达的系统进行非接触式多目标生命体征估计。同时，为验证硬件实现和便携性，提出了FPGA实现方案。

Result: 实验结果表明，该系统在重新定义生命体征监测方面具有巨大潜力。FPGA实现相比于软件版本，执行速度提高了2.7倍，查找表（LUT）利用率降低了18.4%，加速比超过7400倍。

Conclusion: 该研究成功展示了一种创新的非接触式多目标生命体征监测方法，通过FPGA实现显著提升了性能和便携性，克服了传统方法的局限性。
...
### [232] [Analytical Phasor-Based Fault Location Enhancement for Wind Farm Collector Networks](https://arxiv.org/abs/2511.21319)
*Alailton J. Alves Junior,Daniel Barbosa,Ricardo A. S. Fernandes,Denis V. Coury*

Main category: eess.SY

TL;DR: 该研究提出了一种补偿框架，通过增加距离相关的电压校正项来解决逆变器并网资源（IBRs）下游故障导致单端相量法故障定位不准确的问题，显著提高了风电场集电线路的故障定位精度，尤其对接地故障效果显著，且对风电渗透率不敏感。


<details>
  <summary>Details</summary>
Motivation: 逆变器并网资源（IBRs）的日益整合改变了故障电流特性，对传统保护和故障定位方法提出了挑战。当IBRs位于故障点下游时，单端相量法故障定位会因无法捕捉IBRs故障电流注入引起的电压降而系统性地高估故障距离，这是风电场集电线路故障定位的一个关键局限。

Method: 研究提出了一种通用的补偿框架，通过增加一个距离相关的电压校正项来增强经典的环路公式。该方法通过序分量域表示进行分析推导，并使用统一的符号推广到多种故障类型。它保持了传统方法的简单性和可解释性，仅需本地测量即可实现。该方法通过PSCAD中的EMT仿真，使用真实的风电场模型进行了评估。

Result: 结果显示定位精度显著提高，平均和最大误差显著降低，尤其对于涉及接地的故障，误差降低超过90%。此外，该补偿消除了对风电渗透率水平的敏感性，并确保了馈线之间的性能一致性。

Conclusion: 该方法作为一种实用的解决方案，适用于现代可再生能源主导的电网，有效解决了IBRs集成网络中单端故障定位方法的局限性，提高了故障定位的准确性和鲁棒性。
...
### [233] [Model Predictive Control and Moving Horizon Estimation using Statistically Weighted Data-Based Ensemble Models](https://arxiv.org/abs/2511.21343)
*Laura Boca de Giuli,Samuel Mallick,Alessio La Bella,Azita Dabiri,Bart De Schutter,Riccardo Scattolini*

Main category: eess.SY

TL;DR: 本文提出了一种基于集成数据模型的模型预测控制（MPC）框架，通过马哈拉诺比斯距离组合规则和移动视界估计（MHE）状态观测器，实现复杂系统在多工况下的优化控制。


<details>
  <summary>Details</summary>
Motivation: 在多工况下对复杂系统进行最优控制是一个挑战，需要能够适应不同操作条件的模型。本文旨在通过利用数据驱动的集成模型来解决这一问题。

Method: 该研究采用以下方法：1. 提出一个模型预测控制（MPC）框架，该框架利用集成数据模型来控制复杂系统。2. 提出一种基于统计马哈拉诺比斯距离的新型集成模型组合规则，使集成权重能够根据系统输入在预测窗口内适当地变化。3. 开发了一种使用移动视界估计（MHE）的集成模型状态观测器。

Result: 所提出的方法在一个基准能源系统上进行了验证，该系统在多种操作条件下运行，并展示了其有效性。

Conclusion: 该研究成功开发并验证了一个结合集成数据模型、基于马哈拉诺比斯距离的动态权重组合规则以及MHE状态观测器的MPC框架，能够有效控制在多工况下运行的复杂系统。
...
### [234] [Evaluation of Large Language Models for Numeric Anomaly Detection in Power Systems](https://arxiv.org/abs/2511.21371)
*Yichen Liu,Hongyu Wu,Bo Liu*

Main category: eess.SY

TL;DR: 本文全面评估了大型语言模型（LLMs）在电力系统数字异常检测（AD）中的性能，使用GPT-OSS-20B在IEEE 14节点系统上，通过多种提示和微调方法，并结合三西格玛准则进行检测和推理质量分析。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在通用能力方面备受关注，且异常检测对电网弹性至关重要，但LLMs在处理大规模数字数据进行电力系统异常检测方面的性能尚未得到充分探索。

Method: 研究采用GPT-OSS-20B模型，并在IEEE 14节点系统上进行评估。应用标准化提示框架，涵盖零样本、少样本、上下文学习、低秩适应（LoRA）、微调以及LLM与传统方法的混合方法。采用基于三西格玛准则的规则感知设计，并报告检测性能和推理质量。

Result: 本文对LLMs在电力系统数字异常检测中的应用进行了全面评估，并报告了不同方法下的检测性能和推理质量。

Conclusion: 这项研究为进一步探索基于LLM的异常检测的局限性、能力及其与经典检测器在信息物理电力系统应用中的集成奠定了基础。
...
### [235] [Influence of converter current limiting and prioritization on protection of highly IBR-penetrated networks](https://arxiv.org/abs/2511.21385)
*Andrés E. Quintero,Vinícius A. Lacerda,Oriol Gomis-Bellmunt,Moisés J. B. B. Davi,Mario Oleskovicz*

Main category: eess.SY

TL;DR: 本文研究了逆变器型资源(IBRs)中的并网型(GFM)和随网型(GFL)控制策略如何影响以换流器为主的输电系统中的线路距离和差动保护，发现它们会损害传统保护方案的可靠性和安全性。


<details>
  <summary>Details</summary>
Motivation: 在以换流器为主的输电系统中，逆变器型资源(IBRs)的并网型(GFM)和随网型(GFL)控制策略，以及电流限制和序电流优先级，可能会影响甚至损害传统的输电线路保护方案（如距离和差动保护）的性能和可靠性。

Method: 研究评估了一个修改后的IEEE 39节点系统。系统中配备了具有低电压穿越逻辑、电流限制和正/负序优先级的GFM和GFL单元。距离保护采用Mho特性实现，而线路差动保护则使用alpha-plane方法。

Result: 研究结果显示，距离保护中的相地故障回路会严重高估故障位置，尤其是在接近第一区范围时。对于线路差动保护，外部故障可能导致操作点短暂进入alpha-plane的跳闸区域，即使是在GFL控制下的ABG故障的健康相以及故障发生初期，这需要强大的外部安全措施。这些发现强调了现代换流器控制、电流限制和序电流优先级会损害传统保护方案的可靠性和安全性。

Conclusion: 现代换流器控制策略，结合电流限制和序电流优先级，对传统距离和差动保护方案的可靠性和安全性构成了挑战。这些因素可能导致故障定位不准确和外部故障误跳，凸显了在换流器主导的电网中重新评估和调整保护方案的必要性。
...
### [236] [Decentralized Shepherding of Non-Cohesive Swarms Through Cluttered Environments via Deep Reinforcement Learning](https://arxiv.org/abs/2511.21405)
*Cristiana Punzo,Italo Napolitano,Cinzia Tomaselli,Mario di Bernardo*

Main category: eess.SY

TL;DR: 本文提出了一种分层控制架构，结合高层目标分配和基于学习的低层驱动模块，用于在复杂环境中实现有限牧羊人对大量非合作目标的去中心化引导。


<details>
  <summary>Details</summary>
Motivation: 在存在静态障碍物的杂乱环境中，有限数量的牧羊人需要引导大量非内聚、扩散性目标群体到达目标区域。

Method: 采用分层控制架构：高层使用目标分配规则（牧羊人与选定目标配对），低层使用基于学习的驱动模块（有效引导分配的目标）。低层策略在单个牧羊人-单个目标场景下，利用近端策略优化（PPO）进行训练，并直接扩展到多智能体、多障碍物环境而无需重新训练。

Result: 数值模拟显示出平滑、无碰撞的轨迹，并能持续收敛到目标区域，验证了强化学习在复杂环境中实现可扩展、无模型引导的潜力。

Conclusion: 强化学习在复杂环境中实现可扩展、无模型的引导任务中具有巨大潜力。
...
### [237] [Understanding Regional Inertia Dynamics in CAISO from Real Grid Disturbances](https://arxiv.org/abs/2511.21387)
*Saurav Dulal,Mohammed M. Olama,Ali R. Ekti,Nils M. Stenvig,Yilu Liu*

Main category: eess.SY

TL;DR: 本研究提出了一个基于测量的框架，利用实际扰动数据估算CAISO区域电网惯量，发现区域RoCoF值显著高于互联系统，并指出电池储能系统对近期惯量恢复的贡献。


<details>
  <summary>Details</summary>
Motivation: 同步发电机向逆变器基资源转变导致电网惯量分布不均，部分区域在扰动期间面临高频率变化率（RoCoF）的脆弱性，因此需要监测和理解区域惯量动态以确保电网可靠性。

Method: 采用基于测量的框架，利用频率监测网络（FNET/GridEye）中实际扰动驱动的频率数据，分析了2013年至2024年CAISO区域的已确认扰动，以估算电网惯量并识别区域惯量和频率动态的趋势。

Result: 研究发现，区域RoCoF值最高可达互联系统范围值的六倍，这与惯量下降同时发生。近期惯量的恢复归因于部署了具有合成惯量能力的电池储能系统。同时，研究还揭示了区域惯量和频率动态与可再生能源发电以及不断演变的鸭子曲线之间的关系。

Conclusion: 研究强调了区域惯量监测、战略性资源规划和适应性运行实践的重要性，以确保在可再生能源日益整合的背景下电网的可靠性。
...
### [238] [VibraWave: Sensing the Pulse of Polluted Waters](https://arxiv.org/abs/2511.21456)
*Sagnik Ghosh,Sandip Chakraborty*

Main category: eess.SY

TL;DR: 本文提出VibraWave，一种结合毫米波雷达、声学激励、张量分解和深度学习的非侵入式水污染物检测框架，可实现多种水污染物的实时分类和定量。


<details>
  <summary>Details</summary>
Motivation: 传统水污染物检测方法通常具有侵入性、成本高昂，且不适用于实时、便携式监测。

Method: VibraWave利用毫米波雷达结合受控声学激励，捕获雷达反射并编码为包含相位动态、距离单元功率和到达角（AoA）的三维张量。通过应用带非负约束的PARAFAC分解提取紧凑且可解释的污染物指纹。随后，使用知识蒸馏训练一个轻量级学生神经网络，以实现重金属（铜、铁、镁）、油乳液和沉积物的联合分类和定量。

Result: VibraWave在纯净、二元和三元混合物中均实现了高精度和低均方根误差（RMSE），同时保持了鲁棒性和计算效率。

Conclusion: VibraWave非常适用于可扩展的实时水质监测，为解决传统方法的局限性提供了有效方案。
...
### [239] [Multi-Hypotheses Navigation in Collaborative Localization subject to Cyber Attacks](https://arxiv.org/abs/2511.21432)
*Peter Iwer Hoedt Karstensen,Roberto Galeazzi*

Main category: eess.SY

TL;DR: 本文提出了一种多智能体系统中弹性协作定位方法，通过维护多重假设、使用协方差交集和几何削减来抵御欺骗性射频测量，从而分离真实和欺骗数据，并恢复一致的估计。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在面临欺骗性射频测量时，需要一种能够抵御攻击并保持准确协作定位的能力。

Method: 每个智能体维护自身状态的多个假设，并使用协方差交集与邻居交换选定信息。通过基于距离测试和凸包结构的几何削减，限制传输的假设数量，从而控制假设在网络中的传播。此方法使智能体能够区分欺骗性和真实测量，并在识别正确假设后恢复一致的估计。

Result: 数值结果表明，该方法能够有效遏制对抗性测量的影响，同时揭示了保守融合对检测速度的影响。智能体能够分离欺骗性测量和真实测量，并恢复一致的估计。

Conclusion: 该框架为弹性多智能体导航提供了基础，并可通过网络中协调的假设选择进行扩展。
...
### [240] [Robust Rule-Based Sizing and Control of Batteries for Peak Shaving Applications](https://arxiv.org/abs/2511.21619)
*Lorenzo Nespoli,Vasco Medici*

Main category: eess.SY

TL;DR: 随着电池成本降低，本文提出随机调优的基于规则的控制器（RBCs）在电池尺寸确定和控制方面优于确定性模型预测控制（MPC），能提供更实际的能源平准化成本（LCOE）估计和更好的运行性能，尤其适用于削峰应用。


<details>
  <summary>Details</summary>
Motivation: 随着电池成本的降低，快速且能实现预期性能的电池尺寸确定和控制方法变得越来越重要。

Method: 本文展示了如何有效使用随机调优的基于规则的控制器（RBCs）来达到目标，并将其与确定性模型预测控制（MPC）进行比较。方法在真实的电表年负荷数据上进行了削峰应用测试。

Result: 与确定性MPC相比，随机调优的RBCs能提供更实际的LCOE估计，并在实际运行中表现出更好的性能。

Conclusion: 随机调优的RBCs可以有效地用于电池尺寸确定和控制，尤其在削峰应用中，相比确定性MPC，它能提供更准确的性能估计和更优的运行表现。
...
### [241] [Bang-Bang Evasion: Its Stochastic Optimality and a Terminal-Set-Based Implementation](https://arxiv.org/abs/2511.21633)
*Liraz Mudrik,Yaakov Oshman*

Main category: eess.SY

TL;DR: 本文在一个固有的随机框架下，研究了具有不完美信息和有限控制的平面末期对抗中的最优规避问题，证明了最优规避策略的存在性（包括至少一种开关型策略），并提出了一种终端集规避（TSE）策略，该策略在模拟中表现优于传统随机规避策略。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设完美信息或在随机设置中使用启发式机动模型。本研究旨在解决更现实的、涉及不完美信息和有限控制的随机规避场景。

Method: 将问题公式化为固有的随机框架，涉及不完美信息和有限控制。遵循广义分离定理，控制律考虑状态的后验分布。证明了最优规避策略的存在性，并指出最优解集包含至少一种开关型策略。基于此结构，提出闭环终端集规避（TSE）策略。

Result: 证明了最优规避策略始终存在，且最优解集至少包含一种开关型策略，从而使最优控制问题变为有限维。提出的TSE策略在模拟中对比例导航追击者有效。蒙特卡洛模拟显示，TSE策略优于基于随机电报、Singer和编织模型的传统随机规避策略。

Conclusion: 在现实的随机规避场景中，最优规避策略的存在性得到证明，并且开关型策略是其组成部分。所提出的TSE策略在性能上超越了传统的随机规避策略，为解决此类问题提供了一种有效方法。
...
### [242] [Model-free practical PI-Lead control design by ultimate sensitivity principle](https://arxiv.org/abs/2511.21641)
*Michael Ruderman*

Main category: eess.SY

TL;DR: 本文提出了一种无需过程模型、基于实验观察的实用三步法，用于设计鲁棒的PI-Lead控制器，该方法结合了极限灵敏度原理和回路整形特性，并已通过实验验证。


<details>
  <summary>Details</summary>
Motivation: 在实际的反馈控制器设计和调优中，尤其是在运动控制系统里，通常缺乏动态过程的模型，工程师仅能依赖对过程的一些通用假设（如一级稳定行为）。因此，需要一种无需建模的实用且简单的控制器设计方法。

Method: 该方法提出了一种实用的、易于实现的鲁棒PI-Lead控制器设计程序，其灵感来源于Ziegler-Nichols PID调优中的极限灵敏度原理和回路整形的一般特性。它包含三个步骤：确定积分时间常数、控制增益和超前环节，所有步骤仅依赖输出值的实验观测，旨在保证足够的相位裕度。

Result: 所提出的方法通过在受噪声扰动的平移运动机电执行器系统上进行的实验进行了评估和验证，结果表明它能保证足够的相位裕度。

Conclusion: 本文提出了一种无需建模的、基于实验观察的实用三步法来设计鲁棒PI-Lead控制器，该方法简单有效，并能保证足够的相位裕度，已通过实验验证其在实际系统中的可行性。
...
### [243] [A Fractional Variational Approach to Spectral Filtering Using the Fourier Transform](https://arxiv.org/abs/2511.20675)
*Nelson H. T. Lemes,José Claudinei Ferreira,Higor V. M. Ferreira*

Main category: eess.IV

TL;DR: 本文提出了一种基于分数阶导数变分方法的新型去噪滤波器，通过傅里叶变换在频域实现，并利用香农熵优化正则化参数和导数阶数，有效去除拉曼光谱和图像中的噪声，同时保留关键特征。


<details>
  <summary>Details</summary>
Motivation: 拉曼光谱分析中，荧光信号和噪声的干扰是一个重大挑战，常常掩盖关键的细微光谱特征，影响分析准确性。

Method: 该方法受图像去噪中变分方法的启发，通过最小化一个包含分数阶导数的泛函，以平衡噪声抑制和化学特征（如峰位置、强度、面积）的保留。问题通过傅里叶变换在频域重新表述，简化了实现。主要贡献是将频域变分方法与分数阶导数结合，并通过香农熵优化正则化参数和导数阶数。

Result: 研究探讨了分数阶和正则化参数如何影响去噪效果和光谱（及图像）特征的保留。结果表明，所提出的策略组合产生了一个高效、鲁棒且易于实现的滤波器，适用于模拟拉曼数据和图像处理。

Conclusion: 该研究表明，结合频域变分方法、分数阶导数和香农熵优化的策略，能够提供一个高效、鲁棒且易于实现的滤波器，有效解决拉曼光谱和图像中的噪声问题，同时确保关键特征的保留。
...
### [244] [Adversarial Multi-Task Learning for Liver Tumor Segmentation, Dynamic Enhancement Regression, and Classification](https://arxiv.org/abs/2511.20793)
*Xiaojiao Xiao,Qinmin Vivian Hu,Tae Hyun Kim,Guanghui Wang*

Main category: eess.IV

TL;DR: 该论文提出了MTI-Net，一个端到端的集成框架，用于同时进行肝肿瘤分割、动态增强回归和分类。它通过多域信息熵融合、任务交互模块和任务驱动判别器有效利用动态MRI数据并捕捉任务间关联。


<details>
  <summary>Details</summary>
Motivation: 肝肿瘤分割、动态增强回归和分类对临床评估和诊断至关重要。然而，现有工作未能在一个端到端框架中同时实现这些任务，主要原因在于缺乏有效捕获任务间关联以实现相互改进的框架，以及缺乏有效提取动态MRI信息的机制。

Method: 本文提出了多任务交互对抗学习网络（MTI-Net），一个新颖的集成框架。它包含：1) 多域信息熵融合（MdIEF），利用熵感知的高频频谱信息整合频域和谱域特征，增强动态MRI数据提取；2) 任务交互模块，建立分割和回归之间的高阶一致性，促进任务协同；3) 新颖的任务驱动判别器（TDD），捕捉任务内部高阶关系；4) 浅层Transformer网络，用于位置编码以捕获动态MRI序列内的关系。

Result: 在包含238名受试者的数据集上进行的实验表明，MTI-Net在多项任务中表现出高水平的性能。

Conclusion: MTI-Net在肝肿瘤临床评估方面显示出巨大的潜力，能够有效辅助诊断。
...
### [245] [Deep Parameter Interpolation for Scalar Conditioning](https://arxiv.org/abs/2511.21028)
*Chicago Y. Park,Michael T. McCann,Cristina Garcia-Cardona,Brendt Wohlberg,Ulugbek S. Kamilov*

Main category: eess.IV

TL;DR: 深度参数插值（DPI）是一种通用方法，通过在两个可学习参数集之间基于标量值进行动态插值，使现有深度神经网络能够接受额外的标量输入，从而提升生成模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前的深度生成模型（如扩散模型和流匹配）需要一个神经网络来学习时间或噪声水平相关的向量场。设计一个能有效整合高维向量（如图像）和标量信息的网络架构极具挑战性，现有方法（如将标量编码为额外图像输入或在特定网络组件中结合信息）限制了架构选择，不够灵活。

Method: 该方法提出了深度参数插值（DPI）。它在一个单一网络中维护两个可学习的参数集。在训练和采样过程中，根据标量值动态地在这两个参数集之间进行插值，从而引入标量依赖性。DPI是一种简单、与架构无关的方法。

Result: DPI显著改善了扩散模型和流匹配模型的去噪性能，并提升了样本质量。同时，其计算效率与标准的标量条件化技术相当。

Conclusion: DPI是一种简单、通用且与架构无关的方法，用于为神经网络添加标量依赖性。它在生成模型中展示了性能提升和计算效率，为处理多源输入提供了一种有效策略。
...
### [246] [Knowledge Distillation for Continual Learning of Biomedical Neural Fields](https://arxiv.org/abs/2511.21409)
*Wouter Visser,Jelmer M. Wolterink*

Main category: eess.IV

TL;DR: 本文研究了神经场在增量数据场景中面临的灾难性遗忘问题，并提出知识蒸馏作为缓解策略，以实现在医学成像等领域中神经场的持续学习。


<details>
  <summary>Details</summary>
Motivation: 神经场作为一种轻量、连续、可微分的信号表示在生物医学成像中日益普及。然而，与离散表示不同，神经场在处理新数据时容易出现灾难性遗忘，导致先前表示的信号质量下降，这限制了其在数据增量可用场景中的应用。

Method: 本研究考察了不同神经场方法遭受灾难性遗忘的程度，并提出了一种缓解策略。在心脏电影MRI数据上进行了一系列实验，验证了在时空域扩大或表示信号维度增加时，知识蒸馏如何减轻灾难性遗忘。

Result: 研究发现，灾难性遗忘的程度在很大程度上取决于所使用的神经场模型。知识蒸馏能够有效缓解灾难性遗忘，尤其是在时空域扩大或表示信号维度增加的情况下。

Conclusion: 知识蒸馏可以使神经场实现持续学习，从而解决灾难性遗忘问题。其缓解效果与所选的神经场模型密切相关。
...
### [247] [Entropy Coding for Non-Rectangular Transform Blocks using Partitioned DCT Dictionaries for AV1](https://arxiv.org/abs/2511.21609)
*Priyanka Das,Tim Classen,Mathias Wien*

Main category: eess.IV

TL;DR: 本文提出了一种新的熵编码方法，用于高效编码非矩形（NR）划分变换系数，以提高VVC和AV1等视频编码器的效率。


<details>
  <summary>Details</summary>
Motivation: VVC和AV1等现代视频编解码器采用非矩形（NR）划分来组合预测信号，但目前的熵编码方案主要为DCT系数设计，不适合优化编码这些NR变换系数。

Method: 本文引入了一种熵编码方法，通过有效建模NR变换系数的特性来高效编码它们。该方法基于一种变换技术，该技术将相同的NR划分应用于二维离散余弦变换（DCT）基，以找到NR信号的稀疏表示。

Result: 该设计提供了显著的理论码率节省（通过条件熵估算），特别是在实验设置中，对于与DCT差异较大的场景效果更佳。

Conclusion: 所提出的熵编码方法能有效编码非矩形变换系数，从而为采用非矩形划分的视频编解码器带来更高的编码效率。
...
### [248] [Semantic-Enhanced Feature Matching with Learnable Geometric Verification for Cross-Modal Neuron Registration](https://arxiv.org/abs/2511.21452)
*Wenwei Li,Lingyi Cai,Hui Gong,Qingming Luo,Anan Li*

Main category: eess.IV

TL;DR: 该论文提出了一种新颖的深度学习框架，用于高精度配准体内双光子和体外荧光显微光学切片断层扫描的单个神经元图像，有效解决了跨模态差异、数据稀缺和组织形变等挑战。


<details>
  <summary>Details</summary>
Motivation: 在神经科学中，准确配准体内双光子和体外荧光显微光学切片断层扫描的单个神经元图像对于结构-功能分析至关重要。然而，这项任务面临着巨大的跨模态外观差异、带标注数据稀缺以及严重的组织形变等挑战。

Method: 该方法引入了语义增强的混合特征描述符，融合了局部特征的几何精度和视觉基础模型DINOV3的上下文鲁棒性，以弥合模态鸿沟。为处理复杂形变，它用可学习的几何一致性置信模块（一个识别并拒绝不合理对应关系的新型分类器）取代了传统的RANSAC。此外，采用了一种数据高效的两阶段训练策略：先在合成形变数据上预训练，再在有限真实数据上微调，以解决数据稀缺问题。

Result: 该框架为具有挑战性的生物医学成像场景提供了鲁棒且准确的高精度配准解决方案。

Conclusion: 该框架能够实现大规模的关联研究，对神经科学的结构-功能分析具有重要意义。
