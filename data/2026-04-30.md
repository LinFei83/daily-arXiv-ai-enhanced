...
TL;DR: 研究了自主语言模型代理在真实资本下执行用户指令的可靠性，通过21天部署展示了系统从7.5M次调用中实现99.9%结算成功率，并强调了操作层（如提示编译、策略验证等）对可靠性的关键作用。


<details>
  <summary>Details</summary>
Motivation: 在真实资本环境中，语言模型代理的可靠性至关重要，但现有文本测试难以捕捉实际故障模式（如虚构交易规则、费用瘫痪等），需探索完整路径评估。

Method: 基于DX Terminal Pro平台进行21天部署，涉及3505个用户代理，结合结构化控制与自然语言策略，通过操作层（提示编译、类型控制、策略验证、执行防护、内存设计、追踪级可观测性）提升可靠性。

Result: 产生7.5M次代理调用、约30万链上操作、2000万美元交易量、99.9%结算成功率；预发布测试发现故障模式后，通过调整将虚构卖出规则从57%降至3%，费用相关观测从32.5%降至10%以下，资本部署率从42.9%提升至78.0%。

Conclusion: 管理资本的代理应从用户指令到提示、验证操作和结算的完整路径进行评估，可靠性依赖于围绕模型构建的操作层而非基础模型本身。
...
TL;DR: 提出了一种教师-学生框架Distill-Belief，用于闭环逆源定位与表征，在保持贝叶斯正确性的同时实现高效决策。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，准确的贝叶斯推理代价高昂，而快速学习的信念模型容易产生奖励欺骗问题，导致策略利用近似误差而非真正降低不确定性。

Method: 采用教师-学生框架：教师使用贝叶斯正确的粒子滤波器维护后验并提供密集的信息增益信号；学生蒸馏后验为控制用的信念统计和停止用的不确定性证书。部署时仅使用学生模型，计算成本恒定。

Result: 在七种场模态和两项压力测试中，Distill-Belief相较于基线方法一致降低了感知成本，提高了成功率、后验收缩和估计精度，并缓解了奖励欺骗。

Conclusion: 所提框架有效平衡了贝叶斯正确性与计算效率，显著提升了逆源定位与表征的性能。
...
TL;DR: BTF-2是一个包含1,417个预测问题和冻结研究语料库的基准，可识别0.004 Brier分数的准确率差异，并构建了比任何单一前沿智能体准确率高出0.011 Brier分数的预测器，用于无后见偏差评估战略推理。


<details>
  <summary>Details</summary>
Motivation: 现有预测基准仅产生准确性排行榜，缺乏对预测者准确性差异原因的深入分析。

Method: 构建BTF-2基准，包含冻结的研究语料库和完整推理轨迹，通过比较不同智能体的研究和判断能力差异，并构建更准确的预测器进行分析。

Result: 发现更好预测器的优势在于预先分析盲点和考虑黑天鹅事件；专家指出前沿智能体的主要推理失败在于评估政治和商业领袖的动机、执行计划的意愿以及制度过程建模。

Conclusion: BTF-2能有效区分智能体的研究能力和判断能力，并且通过分析推理差异可提升预测准确性，揭示了前沿智能体在战略推理中的关键缺陷。
...
TL;DR: 提出一种层次化框架，通过将用户行为聚合为意图记忆并聚类生成多个人格，利用群组DPO优化人格质量，在多个数据集上提升了人格的连贯性、证据支撑度和可信度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成人格的方法多侧重下游效用，缺乏对人格质量本身的保证。

Method: 层次化框架：聚合行为为意图记忆→聚类生成多个人格→将人格归纳建模为优化问题，通过群组直接偏好优化（DPO）优化聚类凝聚力、人格-证据对齐和真实性。

Result: 在大型服务日志和两个公开数据集上，人格更连贯、证据更充分、更可信，且提升了未来交互预测性能。

Conclusion: 所提方法能生成更高质量的多个人格，同时改善下游任务效果。
...
TL;DR: 本文提出了OMEGA框架，一个端到端的自动化AI研究系统，从想法生成到可执行代码，用于生成新的机器学习分类器，并在20个基准数据集上超越scikit-learn基线。


<details>
  <summary>Details</summary>
Motivation: 为了自动化AI研究过程，减少人工干预，提升算法生成的效率和性能。

Method: 结合结构化元提示工程与可执行代码生成，创建新的ML分类器。

Result: 生成的多个新算法在20个基准数据集上优于scikit-learn基线。

Conclusion: OMEGA框架能有效自动化生成高性能ML分类器，并提供了Python包供使用。
...
TL;DR: 研究了大语言模型在回答法律问题时如何受论据质量的影响，特别是律师水平是否不当影响模型决策。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs被提议用于司法和行政决策，需要了解它们如何回应法律论据，避免被辩护技巧而非案件实质左右。

Method: 通过原创实验，比较前沿开源和闭源LLM对不同质量法律论据的响应，分析影响模型立场的关键因素。

Result: 律师论据质量会显著影响模型对法律观点的认同概率，存在不当说服风险。

Conclusion: LLMs在司法和行政环境中的可行性受到其对论据质量敏感性的制约，需进一步改进以保障决策公正性。
...
TL;DR: 利用Apriori算法分析数学辅导系统日志中习得性无助的行为模式，发现跳过题目与未解决结果强相关，而坚持行为与成功解决相关。


<details>
  <summary>Details</summary>
Motivation: 探究学生在数学辅导系统中习得性无助行为的交互模式，以理解其学习行为特征。

Method: 采用Apriori算法分析行为序列日志，从习得性无助水平、系统干预和问题解决结果三个维度挖掘关联规则。

Result: 低习得无助学生表现出坚持与成功解决的正向关联；高习得无助学生显示更多回避行为；无干预组中坚持-成功关联最多，干预组中跳过-未解决模式更强。

Conclusion: 坚持行为（不跳过）与成功解决一致相关，而跳过且不使用提示预测未解决结果，建议针对高习得无助学生设计干预以减少回避行为。
...
TL;DR: DreamProver是一个基于“醒-睡”程序归纳范式的智能体框架，用于在形式定理证明中发现可重用的引理。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖固定引理库（适应性差），要么为单个定理合成高度特化的引理（缺乏通用性），因此需要一种能自动生成通用可迁移引理的方法。

Method: 采用迭代两阶段过程：在“醒”阶段尝试用当前引理库证明定理并提议新候选引理；在“睡”阶段对候选引理进行抽象、精简和整合，以压缩和优化引理库。

Result: 在多个数学基准测试中显著提高了证明成功率，同时生成了更简洁的证明并降低了计算成本。

Conclusion: DreamProver能够通过交替循环逐步演化出紧凑的高层可迁移引理集，有效用于证明相关领域的新定理。
...
TL;DR: 提出了一种将机器学习的可扩展性与严格推理相结合的自动化对象关系推理框架，在IQ测试中达到98.03%的解决率，对应顶级智商水平。


<details>
  <summary>Details</summary>
Motivation: 大型机器学习模型收益递减且缺乏推理能力，需要通过结合可扩展性与严格推理来突破当前局限。

Method: 基于对象关系自动推理的理论框架，并与人工神经网络集成；通过集成推理与机器学习的范式实现，无需先验知识。

Result: 系统在IQ问题中达到98.03%的解决率，对应前1%或132-144的智商分数，且仅受限于模型大小和计算资源。

Conclusion: 通过集成先验知识和扩展数据集，系统可泛化至解决广泛问题类别，且天然适合少样本或零样本学习场景。
...
TL;DR: 本文通过引入迭代逻辑张量网络（iLTN）进行系统实证分析，证明符号接地不足以实现组合泛化，推理能力需要明确的学习目标而非仅从接地中涌现。


<details>
  <summary>Details</summary>
Motivation: 神经符号AI中假设组合推理会作为成功符号接地的副产品自然涌现，但该假设缺乏系统验证，作者旨在通过实证挑战这一核心假设。

Method: 提出完全可微分的iLTN架构用于多步演绎推理，并设计形式化泛化分类法（包括新实体、未见关系、复杂规则组合的探测），比较仅接地训练与联合接地-推理训练的效果。

Result: 仅训练接地目标的模型泛化失败，而联合训练接地和多步推理的iLTN在所有任务中均取得高零样本准确率。

Conclusion: 符号接地是必要的但不足以实现泛化；推理不是涌现属性，而是需要显式学习目标的独特能力。
...
TL;DR: 本文提出AGEL-Comp神经符号智能体架构，通过动态因果程序图、归纳逻辑编程和混合推理核心解决LLM智能体的组合泛化失败问题，在Retro Quest环境中表现优于纯LLM模型。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的智能体系统在组合泛化任务中表现出系统性失败，限制了其在交互环境中的鲁棒性。

Method: 包含三个核心创新：(1)动态CPG世界模型，(2)基于经验反馈合成新Horn子句的ILP引擎，(3)LLM提出候选子目标并由NTP验证逻辑一致性的混合推理核心。实现演绎-溯因学习循环。

Result: 在Retro Quest模拟环境中，AGEL模型在组合泛化场景上性能显著优于纯LLM模型。

Conclusion: 该框架为构建具有显式、可解释且组合结构化世界理解的智能体提供了可行方向。
...
TL;DR: 大型语言模型在机器人健康助手控制组件中的安全性较差，平均违规率54.4%，开放权重模型尤其危险。


<details>
  <summary>Details</summary>
Motivation: 大语言模型被考虑用于机器人健康助手，但其安全性尚未充分研究。

Method: 基于美国医学会医学伦理原则创建270个有害指令数据集，在模拟环境中评估72个模型。

Result: 平均违规率54.4%，超半数模型超标；开放权重模型违规率中位数72.8% vs 专有模型23.7%；医学微调无显著改善，提示防御效果有限。
...
TL;DR: 本文提出了一种人类在环中的基准测试框架，用于评估多个大型语言模型在自动化中学数学评估中的有效性。研究发现，模型架构与指令约束的兼容性比参数规模更重要，Gemini混合专家模型取得了公平一致性，而更大的Orion模型则无一致性。


<details>
  <summary>Details</summary>
Motivation: 基于能力的教育（CBE）正在全球兴起，但从基于分数的评估转向定性能力映射对教育者来说是一项手动挑战，这促使研究自动化评估方法。

Method: 基于尼泊尔十年级选修数学课程，创建了四维评分标准，并使用多供应商集成（包括开放权重模型和专有前沿模型）进行基准测试，与两位资深数学教员的标注结果对比。

Result: Gemini混合专家模型达到公平一致性（kappa_w ≈ 0.38），而更大的Orion模型无一致性（kappa_w = -0.0261），表明架构兼容性优于参数规模。

Conclusion: 大型语言模型尚不适合自主认证，但可在人类在环框架中为初步证据提取提供高价值的辅助支持。
...
TL;DR: 提出了一种基于输出分歧的训练无关框架，通过实例级路由动态选择测试时扩展策略，在数学推理任务中将准确率提升3%-7%并降低采样成本。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法（如重复采样、自我修正等）在难题上收益递减，且缺乏对不同困难实例的差异化处理。

Method: 将测试时扩展形式化为实例级路由问题：对低分歧样本使用轻量级解，中度分歧用多数投票，高分歧用重写式重构。

Result: 在7个数学基准测试和3个模型上，准确率提升3%-7%，且采样成本低于现有方法。

Conclusion: 基于输出分歧的实例级策略路由是一种高效且通用的测试时扩展方案，能平衡准确率与计算开销。
...
TL;DR: 提出了一个可扩展的AI就绪性评估系统SciHorizon-DataEVA，通过Sci-TQA2原则和层次化多智能体方法评估异构科学数据。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏可扩展且系统的机制来评估科学数据的AI就绪性。

Method: 基于Sci-TQA2原则（治理可信度、数据质量、AI兼容性、科学适应性）和层次化多智能体评估方法Sci-TQA2-Eval，结合轻量级数据集分析、适用性指标激活和知识增强规划。

Result: 跨多个领域的科学数据集实验证明了SciHorizon-DataEVA的有效性和通用性。

Conclusion: 该系统能够实现对异构科学数据进行原则性且可扩展的AI就绪性评估。
...
TL;DR: 提出了FutureWorld，一个实时智能体强化学习环境，通过连续训练开源模型并建立每日基准，验证了实时未来预测作为学习环境的有效性。


<details>
  <summary>Details</summary>
Motivation: 实时未来预测任务缺乏统一的学习环境，现有研究分散且无法有效整合预测、结果验证与模型更新。

Method: 构建FutureWorld环境，将预测、结果实现和参数更新闭环，基于开源模型进行连续多日训练，并设计每日基准评估。

Result: 连续训练显著提升了模型性能，且在每日基准上评估了多个前沿智能体，建立了性能基线。

Conclusion: 实时未来预测可作为有效的强化学习环境，FutureWorld能够支持自主智能体持续从真实世界事件中学习。
...
TL;DR: 提出Bian Que框架，通过统一范式、灵活技能编排和自我进化机制，实现大规模在线引擎系统的自动化运维，显著降低告警量并提升根因分析效率。


<details>
  <summary>Details</summary>
Motivation: 大规模在线引擎系统的运维需要大量人工干预，现有LLM代理的瓶颈在于编排而非推理能力，需解决事件与数据/知识映射的难题。

Method: 核心贡献包括：统一运维范式（发布拦截、主动巡检、告警根因分析）、灵活技能编排（自动化生成或人工迭代技能）、统一自我进化机制（案例记忆-知识蒸馏与技能优化）。

Result: 在快手电商搜索引擎部署后，告警量减少75%，根因分析准确率达80%，平均修复时间降低50%，离线评估通过率99.0%。

Conclusion: Bian Que通过结构化编排和自我进化，有效提升了运维自动化的可控性和效率，代码已开源。
...
### [18] [Edge AI for Automotive Vulnerable Road User Safety: Deployable Detection via Knowledge Distillation](https://arxiv.org/abs/2604.26857)
*Akshay Karjol,Darrin M. Hanna*

Main category: cs.CV

TL;DR: 提出知识蒸馏框架训练紧凑型YOLOv8-S模型，实现3.9倍压缩且保持INT8量化鲁棒性，在BDD100K上INT8精度优于教师模型的FP32精度。


<details>
  <summary>Details</summary>
Motivation: 边缘硬件部署VRU检测需平衡模型容量与计算约束，大模型INT8量化后精度严重下降，小模型检测性能不足。

Method: 使用知识蒸馏训练YOLOv8-S学生模型（11.2M参数）模仿YOLOv8-L教师模型（43.7M参数），结合Post-Training Quantization进行INT8量化评估。

Result: 教师模型INT8下mAP下降23%，KD学生仅下降5.6%；KD学生INT8精度0.748超过教师FP32精度0.718，误报减少44%。

Conclusion: 知识蒸馏是边缘硬件上部署高精度安全关键VRU检测的必要方法。
...
### [19] [Generalized Disguise Makeup Presentation Attack Detection Using an Attention-Guided Patch-Based Framework](https://arxiv.org/abs/2604.26025)
*Fateme Taraghi,Atefe Aghaei,Mohsen Ebrahimi Moghaddam*

Main category: cs.CV

TL;DR: 提出了一种针对伪装化妆攻击的通用检测框架，采用两阶段设计，结合风格不变的全脸模型和局部补丁分析，在新构建的真实数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 面部识别系统易受伪装化妆攻击，现有研究不足且公开数据集有限。

Method: 两阶段设计：第一阶段使用度量学习和白化变换训练风格不变全脸模型，通过Grad-CAM提取区域注意力分数；第二阶段基于分数指导的补丁分析，使用区域子网络进行细粒度判别。

Result: 在收集数据集上ACER为8.97%，EER为9.76%；在SIW-Mv2上，Obfuscation和Impersonation攻击的ACER为0%，Cosmetics攻击为1.34%，优于现有方法。

Conclusion: 所提方法在多种攻击类型上均表现出强泛化性和鲁棒性，优于先前工作。
...
### [20] [Evaluating the Alignment Between GeoAI Explanations and Domain Knowledge in Satellite-Based Flood Mapping](https://arxiv.org/abs/2604.26051)
*Hyunho Lee,Wenwen Li*

Main category: cs.CV

TL;DR: 本文提出ADAGE框架，系统评估深度学习模型解释与遥感领域知识的一致性，用于卫星洪水制图。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在洪水制图中表现优异，但其决策过程不透明，阻碍了在关键科学和操作流程中的应用。

Method: 采用Channel-Group SHAP方法估计输入通道组对像素级预测的贡献，并与领域知识导出的参考解释进行比较。

Result: 在两项卫星洪水制图任务中，ADAGE能定量评估模型解释与领域知识的对齐程度，并通过对齐分数识别不一致之处。

Conclusion: ADAGE弥合了可解释性与领域知识之间的差距，提升了GeoAI模型在科学和操作流程中的适用性。
...
### [21] [Report of the 5th PVUW Challenge: Towards More Diverse Modalities in Pixel-Level Understanding](https://arxiv.org/abs/2604.26031)
...
TL;DR: 本文总结了2026年CVPR举办的像素级视频理解挑战赛（PVUW），包含三个赛道：MOSE（密集遮挡跟踪）、MeViS-Text（运动文本定位）、以及新增的MeViS-Audio（音频驱动分割），展示了多模态解决方案的最新技术进展。


<details>
  <summary>Details</summary>
Motivation: 为评估模型在高度无约束的视频场景中的理解能力，引入更具挑战性的数据和新任务（如音频驱动分割），推动视频场景理解技术的发展。

Method: 通过三个专门赛道（MOSE、MeViS-Text、MeViS-Audio）收集参与者提交的先进多模态方法，并分析其性能。

Result: 报告介绍了在密集遮挡、运动文本定位和音频驱动分割等任务上的最新技术突破，以及相应数据集和参赛方法的性能表现。

Conclusion: 该挑战赛展示了视频理解领域的最新进展，并为未来鲁棒的视频场景理解指明了方向。
...
### [22] [RADIO-ViPE: Online Tightly Coupled Multi-Modal Fusion for Open-Vocabulary Semantic SLAM in Dynamic Environments](https://arxiv.org/abs/2604.26067)
*Zaid Nasser,Mikhail Iumanov,Tianhao Li,Maxim Popov,Jaafar Mahmoud,Sergey Kolyubin*

Main category: cs.CV

TL;DR: 提出RADIO-ViPE，一种在线语义SLAM系统，可在动态环境下将自然语言查询与3D区域关联，仅需单目视频输入。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖已校准的RGB-D输入和静态场景假设，限制真实世界应用。

Method: 集成RADIO等多模态基础模型，通过初始化、优化和因子图连接耦合视觉、语言与几何信息，并采用自适应鲁棒内核处理动态物体和场景变化。

Result: 在动态TUM-RGBD基准上达到SOTA，与依赖校准数据和静态假设的离线开放词汇方法性能相当。

Conclusion: RADIO-ViPE弥合了真实部署的差距，实现鲁棒的开放词汇语义语义定位，适用于自主机器人和野外视频流。
...
TL;DR: 该文提出使用概率分布检测头将果实成熟度建模为潜在连续变量，通过累积分布函数转换为类别概率，以更准确地估计成熟度并量化不确定性。


<details>
  <summary>Details</summary>
Motivation: 传统视觉成熟度分类任务强制划分连续性成熟阶段，导致标注不可靠且无法反映相邻阶段的视觉相似性。

Method: 将成熟度作为潜在连续变量，使用分布检测头进行概率预测，并通过CDF将分布转换为类别概率。

Result: 在干净标签下性能相当，但能更好表示不确定性；在含噪标签下鲁棒性优于基线方法。

Conclusion: 明确建模成熟度不确定性可提升视觉成熟度估计的可靠性。
...
### [24] [Sample Selection Using Multi-Task Autoencoders in Federated Learning with Non-IID Data](https://arxiv.org/abs/2604.26116)
*Emre Ardıç,Yakup Genç*

Main category: cs.CV

TL;DR: 提出基于多任务自编码器的样本选择方法，通过OCSVM、孤立森林和自适应损失阈值过滤噪声样本，并在CIFAR10和MNIST上验证精度提升。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中冗余、恶意或异常样本导致模型性能下降和效率降低。

Method: 使用多任务自编码器估计样本贡献，结合无监督异常检测（OCSVM、IF、AT）和多类深度SVDD损失进行样本选择。

Result: CIFAR10上OCSVM损失选择提升7.02%准确率，MNIST上AT提升1.83%；SVDD损失在CIFAR10上额外提升0.99%。

Conclusion: 该方法在多种客户端数量、非独立同分布和噪声条件下均能有效提升模型精度。
...
### [25] [MixerCA: An Efficient and Accurate Model for High-Performance Hyperspectral Image Classification](https://arxiv.org/abs/2604.26138)
*Mohammed Q. Alkhatib,Ali Jamali*

Main category: cs.CV

TL;DR: 本文提出了MixerCA，一种用于高光谱图像分类的轻量级模型，结合深度可分离卷积和自注意力机制，在四个基准数据集上优于多种对比算法。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类在遥感领域具有重要意义，现有深度学习方法虽能有效提取空间-光谱特征，但模型复杂度高，需要更轻量高效的解决方案。

Method: MixerCA集成了深度可分离卷积、令牌与通道混合以及坐标注意力，通过解耦空间与通道交互、保持分辨率一致性，直接处理HSI图像块。

Result: 在四个高光谱基准数据集上，MixerCA在分类精度和效率上均优于2D-CNN、3D-CNN、Tri-CNN、HybridSN、ViT和Swin Transformer等算法。

Conclusion: MixerCA为高光谱图像分类提供了一种轻量且高性能的解决方案，源代码已公开。
...
### [26] [A Data-Centric Framework for Intraoperative Fluorescence Lifetime Imaging for Glioma Surgical Guidance](https://arxiv.org/abs/2604.26147)
*Silvia Noble Anbunesan,Mohamed Abul Hassan,Jinyi Qi,Lisanne Kraft,Han Sung Lee,Orin Bloch,Laura Marcu*

Main category: cs.CV

TL;DR: 提出数据中心AI框架，通过自信学习与类别精炼，提升FLIm胶质瘤分类模型的鲁棒性与准确率（96%）。


<details>
  <summary>Details</summary>
Motivation: 术中准确评估胶质瘤浸润程度对最大化切除和保护脑功能至关重要，但FLIm面临生物异质性、类别不平衡和病理标签差异等挑战。

Method: 采用自信学习量化置信度、识别标签错误，并指导类别合并；基于高保真数据集训练三分类模型，结合SHAP分析与目标FLIm评估。

Result: 三分类任务准确率达96%，SHAP分析揭示浸润谱特异性光学特征，自信学习发现的低置信边缘经盲评确认了病理学家自身变异。

Conclusion: 数据中心AI框架能系统提升数据可靠性、增强模型鲁棒性，并优化FLIm信号的生物学解释，支持临床实时胶质瘤边缘评估。
...
### [27] [Why Domain Matters: A Preliminary Study of Domain Effects in Underwater Object Detection](https://arxiv.org/abs/2604.26174)
*Melanie Wille,Dimity Miller,Tobias Fischer,Scarlett Raine*

Main category: cs.CV

TL;DR: 本文提出一个基于可测量特征的水下域定义框架，用于评估目标检测在真实域偏移下的性能，揭示传统基准测试无法捕获的隐藏失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有水下域偏移基准依赖合成风格迁移，无法反映真实场景中的可见度、光照、场景构成等物理因素，限制了实际效果分析。

Method: 利用图像、场景和采集特性等可测量特征定义水下域，实现语义一致的图像分组，支持域特定检测性能评估和失败分析。

Result: 在公开数据集上验证，表明检测性能随域因素系统变化，并揭示了传统方法无法发现的隐藏失败模式。

Conclusion: 基于物理特征的域定义能更准确捕捉真实域偏移，为水下环境可靠性评估提供新框架。
...
### [28] [Lifting Embodied World Models for Planning and Control](https://arxiv.org/abs/2604.26182)
*Alex N. Wang,Trevor Darrell,Pavel Izmailov,Yutong Bai,Amir Bar*

Main category: cs.CV

TL;DR: 提出了一种通过轻量级策略将高层动作映射为低层关节动作序列的方法，从而构建提升的世界模型，用于高效预测人形智能体的未来观测。


<details>
  <summary>Details</summary>
Motivation: 复杂智能体的动作空间高维且难以指定，导致世界模型难以控制和规划，搜索方法如CEM在高维动作空间中扩展性差。

Method: 训练一个轻量级策略，将高层动作（如2D路径点）转换为低层关节动作序列，并与冻结的世界模型组合成提升的世界模型。

Result: 提升的世界模型在目标姿态的关节误差上比直接低层搜索低3.8倍，计算效率更高，且能泛化到未见环境。

Conclusion: 通过定义低维、可视化的高层动作空间，可显著提升人形智能体世界模型的控制性能和规划效率。
...
### [29] [Privacy-Preserving Clothing Classification using Vision Transformer for Thermal Comfort Estimation](https://arxiv.org/abs/2604.26184)
*Tatsuya Chuman,Yousuke Udagawa,Hitoshi Kiya*

Main category: cs.CV

TL;DR: 提出了一种使用视觉变换器（ViT）的隐私保护衣物分类方案，用于安全的人为中心控制（OCC）系统。


<details>
  <summary>Details</summary>
Motivation: 现有研究利用相机图像优化热舒适性，但未考虑隐私保护；传统隐私保护方法会导致分类精度严重下降。

Method: 采用视觉变换器（ViT）对加密图像进行衣物绝缘性估计分类。

Result: 在DeepFashion数据集实验中，所提方案在加密图像上保持高精度，且相比明文图像无性能下降。

Conclusion: 该方法在保护隐私的同时有效避免了传统方法的精度损失。
...
### [30] [ViBE: Visual-to-M/EEG Brain Encoding via Spatio-Temporal VAE and Distribution-Aligned Projection](https://arxiv.org/abs/2604.26218)
*Ganxi Xu,Zhao-Rong Lai,Yuting Tang,Yonghao Song,Shuyan Zhou,Guoxu Zhou,Boyu Wang,Jian Zhu,Jinyi Long*

Main category: cs.CV

TL;DR: 提出ViBE框架，使用TSC-VAE和Q-Former生成高质量M/EEG信号，实现视觉刺激到神经响应的重建与跨模态对齐。


<details>
  <summary>Details</summary>
Motivation: 脑编码模型对理解视觉处理和视觉假体开发至关重要，现有方法在M/EEG信号重建和跨模态对齐上存在不足。

Method: 设计时空卷积变分自编码器（TSC-VAE）捕获M/EEG时空特征；利用Q-Former将CLIP图像嵌入映射到TSC-VAE潜在空间；结合MSE损失和切片Wasserstein距离进行点匹配和分布对齐。

Result: 在THINGS-EEG2和THINGS-MEG数据集上，ViBE能有效生成高质量的M/EEG信号。

Conclusion: ViBE框架通过时空建模和跨模态对齐，实现了视觉刺激到神经信号的有效生成，为脑编码和视觉假体提供新思路。
...
### [31] [FASH-iCNN: Making Editorial Fashion Identity Inspectable Through Multimodal CNN Probing](https://arxiv.org/abs/2604.26186)
*Morayo Danielle Adeyemi,Ryan A. Rossi,Franck Dernoncourt*

Main category: cs.CV

TL;DR: 提出了FASH-iCNN多模态系统，能从服装照片识别时尚品牌、年代和色彩传统，揭示了纹理和亮度是品牌身份的主要载体。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统隐含时尚文化逻辑但未透明化，需要使这些文化影响可被审查。

Method: 训练了基于87,547张Vogue runway图像的多模态分类模型，通过去颜色/纹理的对比实验探查视觉通道贡献。

Result: 品牌识别78.2% top-1，年代88.6% top-1，年份58.3% top-1（平均误差2.2年）；纹理移除导致37.6pp精度下降，远大于颜色（10.6pp）。

Conclusion: 纹理和亮度是时尚编辑身份的主要信号，FASH-iCNN将编辑文化作为可审查的信号而非噪声。
...
### [32] [Seeking Consensus: Geometric-Semantic On-the-Fly Recalibration for Open-Vocabulary Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2604.26221)
*Guanchun Wang,Chenxiao Wu,Xiangrong Zhang,Zelin Peng,Jianxun Lai,Tianyang Zhang,Xu Tang*

Main category: cs.CV

TL;DR: 提出了SeeCo，一种即插即用框架，通过几何和语义共识校准来提升遥感图像中开放词汇语义分割的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用静态推理，忽略了场景分布差异，导致不同地物的语义模糊和前景激活不完整。

Method: 采用双共识学习：几何共识学习（多视角一致观测）和语义共识学习（文本描述自适应校准），并通过在线共识注入器进行校准。

Result: 在八个遥感开放词汇分割基准上取得了一致性性能提升。

Conclusion: SeeCo无需训练，能在推理时针对每个场景动态校准语义-几何对齐，有效缓解欠激活和语义偏差问题。
...
### [33] [HOI-aware Adaptive Network for Weakly-supervised Action Segmentation](https://arxiv.org/abs/2604.26227)
*Runzhong Zhang,Suchen Wang,Yueqi Duan,Yansong Tang,Yue Zhang,Yap-Peng Tan*

Main category: cs.CV

TL;DR: 提出了一种名为AdaAct的HOI感知自适应网络，用于弱监督动作分割，通过利用视频级别的人类-物体交互先验知识来区分相似动作。


<details>
  <summary>Details</summary>
Motivation: 现有固定网络在估计相似动作（如倒果汁和倒咖啡）时存在歧义，需要动态利用全局时序但局部空间的人-物交互信息。

Method: 设计视频HOI编码器提取、选择和整合代表性HOI，并通过双分支超网络学习自适应时序编码器，根据HOI信息动态调整参数。

Result: 在Breakfast和50Salads数据集上，该方法在不同评估指标下均优于现有方法。

Conclusion: 通过动态适应HOI序列，能有效解决弱监督动作分割中的相似动作歧义问题。
...
### [34] [DepthPilot: From Controllability to Interpretability in Colonoscopy Video Generation](https://arxiv.org/abs/2604.26232)
*Junhu Fu,Ke Chen,Weidong Guo,Shuyu Liang,Jie Xu,Chen Ma,Kehao Wang,Shengli Lin,Zeju Li,Yuanyuan Wang,Yi Guo,Shuo Li*

Main category: cs.CV

TL;DR: 提出了首个可解释的结肠镜视频生成框架DepthPilot，通过深度约束和自适应样条去噪模块实现几何引导与非线性建模的结合，在保证视觉真实性的同时提升临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的可控医学视频生成缺乏可解释性，难以对齐物理先验和真实临床表现，亟需从可控性走向可解释性。

Method: 采用两个协同范式：1）先验分布对齐策略，通过参数高效微调将深度约束注入扩散主干；2）自适应样条去噪模块，用可学习样条函数替代固定线性权重以捕捉复杂时空动态。

Result: 在三个公开数据集和内部临床数据上，FID均低于15，临床医师评估排名第一，生成的视频可用于可靠的3D重建以辅助手术导航。

Conclusion: DepthPilot弥合了“视觉真实”与“临床可解释”之间的鸿沟，为结直肠世界模型奠定了基础。
...
### [35] [EnerGS: Energy-Based Gaussian Splatting with Partial Geometric Priors](https://arxiv.org/abs/2604.26238)
*Rui Song,Tianhui Cai,Markus Gross,Yun Zhang,Walter Zimmer,Zhiyu Huang,Olaf Wysocki,Jiaqi Ma*

Main category: cs.CV

TL;DR: 提出EnerGS，通过将部分可观测几何建模为连续能量场，为3DGS提供软几何引导，改善大规模户外场景的光度重建质量与几何稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖LiDAR等几何先验进行初始化或约束，但在户外大规模场景中几何监督往往空间不完整且不均匀，限制了其有效性甚至损害重建质量。

Method: 将部分可观测几何建模为连续能量场，提供软几何引导，不直接约束解空间，而是引导高斯原语的优化过程。

Result: 在稀疏多视图和单目设置下，EnerGS一致提升光度质量和几何稳定性，有效缓解3DGS训练中的过拟合。

Conclusion: EnerGS能有效处理不完整几何监督，提升户外场景3DGS重建性能。
...
### [36] [Camera-RFID Fusion for Robust Asset Tracking in Forested Environments](https://arxiv.org/abs/2604.26241)
*John Hateley,Sriram Narasimhan,Omid Abari*

Main category: cs.CV

TL;DR: 提出一种相机-RFID融合框架，通过深度信息和轨迹匹配算法，实现森林环境中亚米级资产定位。


<details>
  <summary>Details</summary>
Motivation: 森林环境中RFID精度仅为米级，而相机虽精度高但存在空间关联模糊和遮挡问题，需融合两者优势。

Method: 融合相机深度与物体信息，结合先进轨迹匹配算法，实现跨传感器轨迹关联。

Result: 缩小了RFID米级与相机厘米级精度的差距，支持标签在短暂离开视野后仍可定位。

Conclusion: 首次在自然森林环境中实现相机-RFID融合资产追踪，为现实部署提供可行方案。
...
### [37] [MetaSR: Content-Adaptive Metadata Orchestration for Generative Super-Resolution](https://arxiv.org/abs/2604.26244)
*Jiaqi Guo,Mingzhen Li,Haohong Wang,Aggelos K. Katsaggelos*

Main category: cs.CV

TL;DR: 提出了MetaSR，一种基于扩散Transformer的生成式超分辨率框架，能根据内容选择并注入任务相关的元数据，在资源受限条件下提升超分效果。


<details>
  <summary>Details</summary>
Motivation: 真实场景中图像/视频的内容和退化模式多样（如文字、运动、卡通、人脸），现有元数据引导方法使用固定条件设计，无法适应内容依赖的线索和传输带宽限制。

Method: 利用扩散Transformer自身的VAE和Transformer骨干融合异构元数据，并采用高效蒸馏策略实现单步扩散推理。

Result: 在多种内容类型和退化场景下，MetaSR相比参考方案PSNR提升高达1.0 dB，同等质量下传输码率节省50%。

Conclusion: 在码率-失真优化框架下，MetaSR能联合优化发送端码率和接收端质量指标（如PSNR、SSIM），验证了其有效性。
...
### [38] [Beyond Shortcuts: Mitigating Visual Illusions in Frozen VLMs via Qualitative Reasoning](https://arxiv.org/abs/2604.26250)
*Hao Guo,Fei Wang,Junjie Chen,Yiqi Nie,Jiaqi Zhao,Qiankun Li,Subin Huang*

Main category: cs.CV

TL;DR: 提出SQI框架，通过无训练推理时约束增强冻结VLM对光学幻觉的鲁棒性，在经典幻觉理解任务中获第二名。


<details>
  <summary>Details</summary>
Motivation: VLM在光学幻觉任务中因依赖语言先验和记忆原型而脆弱，缺乏对视觉证据的直接感知。

Method: SQI包含三个模块：公理约束注入抑制幻觉估计、层次场景分解解耦目标与背景、反事实自验证减轻确认偏差。

Result: 在DataCV 2026挑战赛任务I中排名第二，显著提升不同幻觉类别的准确率且无需微调。

Conclusion: 结构化定性推理为开发抗幻觉VLM提供了鲁棒范式。
...
### [39] [Three-Step Nav: A Hierarchical Global-Local Planner for Zero-Shot Vision-and-Language Navigation](https://arxiv.org/abs/2604.26946)
*Wanrong Zheng,Yunhao Ge,Laurent Itti*

Main category: cs.CV

TL;DR: 提出一种三视图协议（Three-Step Nav），通过“向前看”、“现在看”、“向后看”三个步骤提升零样本视觉语言导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的零样本VLN代理存在偏离路线、过早停止、成功率低等问题。

Method: 采用三视图协议：1. 向前看提取全局地标并制定粗略计划；2. 现在看对齐当前视觉与下一子目标；3. 向后看审计整个轨迹以纠正累积漂移。无需梯度更新或任务微调。

Result: 在R2R-CE和RxR-CE数据集上达到零样本最优性能。

Conclusion: Three-Step Nav通过简单的三视图协议有效改进了零样本VLN的鲁棒性和成功率。
...
### [40] [Multi-Stage Bi-Atrial Segmentation Framework from 3D Late Gadolinium-Enhanced MRI using V-Net Family Models](https://arxiv.org/abs/2604.26251)
*Hao Wen,Jingsu Kang*

Main category: cs.CV

TL;DR: 提出了一种多阶段框架，用于从3D LGE-MRI中进行多类别双心房分割，包括预处理、粗分割和细分割步骤。


<details>
  <summary>Details</summary>
Motivation: 针对双心房分割的临床需求，提高从3D LGE-MRI中分割的准确性和鲁棒性。

Method: 使用MCLAHE进行预处理增强，V-Net模型进行粗分割，另一个V-Net模型在粗区域内进行细分割，并采用非对称损失优化。

Result: 多阶段框架能够有效提升双心房分割性能，具体定量结果未在摘要中提及。

Conclusion: 该多阶段框架在双心房分割任务中表现良好，能够处理三维MRI数据中的复杂结构。
...
### [41] [OmniTrend: Content-Context Modeling for Scalable Social Popularity Prediction](https://arxiv.org/abs/2604.26252)
*Liliang Ye,Guiyi Zeng,Yunyao Zhang,Yi-Ping Phoebe Chen,Junqing Yu,Zikai Song*

Main category: cs.CV

TL;DR: OmniTrend是一个统一的框架，通过分离内容吸引力与上下文曝光来预测社交媒体流行度，支持跨平台迁移。


<details>
  <summary>Details</summary>
Motivation: 现有方法没有区分内容的固有吸引力和曝光相关的模式，导致表示学习受平台可见性影响，可解释性和跨平台迁移能力弱。

Method: OmniTrend包含内容模块（从视觉、音频和文本中学习交叉模态表示以量化内在吸引力）和上下文模块（从发布时机、作者活跃度、主题趋势和基于检索的邻域统计估计曝光），然后分别预测并集成。

Result: 该方法能显式分离各种因素的影响，并在图像和视频平台上实现稳健的跨平台迁移。

Conclusion: 通过显式建模内容吸引力和上下文曝光，OmniTrend提高了流行度预测的可解释性和泛化能力。
...
### [42] [GaitKD: A Universal Decoupled Distillation Framework for Efficient Gait Recognition](https://arxiv.org/abs/2604.26255)
*Yuqi Li,Qian Zhou,Huiran Duan,Jingjie Wang,Shunli Zhang,Chuanguang Yang,Guoying Zhao,Yingli Tian*

Main category: cs.CV

TL;DR: 提出GaitKD框架，通过决策级和边界级蒸馏提升步态识别知识迁移效果，支持异构师生模型，无额外推理成本。


<details>
  <summary>Details</summary>
Motivation: 标准知识蒸馏在部分结构的步态模型上效果不佳，因监督来自分部分类逻辑和检索嵌入。

Method: 解耦知识迁移为决策级（部分校准逻辑蒸馏）和边界级（激活边界目标）两部分，采用对齐的分部设计。

Result: 在多个步态识别基准和师生配置下，一致优于强步态基线，边界蒸馏比直接特征回归更稳定。

Conclusion: 两个蒸馏组件互补，边界保持蒸馏提供更稳定的性能提升，且框架不增加推理开销。
...
### [43] [MedSynapse-V: Bridging Visual Perception and Clinical Intuition via Latent Memory Evolution](https://arxiv.org/abs/2604.26283)
*Chunzheng Zhu,Jiaqi Zeng,Junyu Jiang,Jianxin Lin,Yijun Wang*

Main category: cs.CV

TL;DR: 提出一种通过隐式诊断记忆进化提升医学VLM诊断精度的框架，核心机制包括先验记忆查询、反事实强化学习和内在记忆迁移。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型因离散分词导致量化损失、长程信息消散和缺乏病例自适应专家知识，无法模拟临床专家的即时记忆调用。

Method: 1) 元查询先验记忆：可学习探针从解剖先验编码器提取结构化知识生成隐式记忆；2) 因果反事实精炼：使用强化学习和区域掩膜反事实奖励筛选记忆因果贡献；3) 内在记忆迁移：双分支特权-自主范式通过全词汇散度对齐迁移诊断模式。

Result: 在多个数据集上显著优于现有方法（包括思维链范式），尤其在诊断准确性方面表现突出。

Conclusion: 将外部专家知识转化为模型内源参数可有效提升医学诊断性能，弥合认知对齐差距。
...
### [44] [Multiple Consistent 2D-3D Mappings for Robust Zero-Shot 3D Visual Grounding](https://arxiv.org/abs/2604.26261)
*Yufei Yin,Jie Zheng,Qianke Meng,Zhou Yu,Minghao Chen,Jiajun Ding,Min Tan,Yuling Xi,Zhiwen Chen,Chengfei Lv*

Main category: cs.CV

TL;DR: MCM-VG通过建立多个一致的2D-3D映射，显著提升了零样本3D视觉定位的性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本3D视觉定位方法受限于低质量的开放词汇3D提议和冗余的多视角推理。

Method: 提出MCM-VG框架，包含语义对齐、实例校正和视点蒸馏三个模块，分别解决类别不匹配、几何不准确和空间冗余问题。

Result: 在ScanRefer和Nr3D基准上达到新最优，ScanRefer上Acc@0.25和Acc@0.5分别提升6.4%和4.0%。

Conclusion: MCM-VG通过显式的2D-3D一致性映射，为零样本3D视觉定位提供了鲁棒且高效的解决方案。
...
### [45] [CheXthought: A global multimodal dataset of clinical chain-of-thought reasoning and visual attention for chest X-ray interpretation](https://arxiv.org/abs/2604.26288)
...
TL;DR: 提出了一个包含103,592条思维链推理和660万视觉注意力标注的多模态资源CheXthought，覆盖50,312张胸部X光片，证明其在提升AI诊断准确性、减少幻觉、增强推理透明度和预测人机分歧方面的效用。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型主要基于图像和报告配对训练，缺乏临床推理过程中的认知模式和视觉注意力机制。

Method: 收集501位放射科医生对50,312张胸部X光片的链式推理轨迹和眼动注视数据，构建CheXthought资源；通过实验验证其在事实准确性、空间定位、病理分类、视觉忠诚度、时序推理和不确定性沟通等方面的性能。

Result: CheXthought推理在事实准确性和空间定位上显著优于现有模型；视觉注意力作为推理提示可减少幻觉；训练后模型在多项任务中表现更强；可基于图像预测人-人和人-AI分歧。

Conclusion: CheXthought可推动多模态临床推理发展，助力构建更透明、可解释的视觉-语言模型。
...
### [46] [ACPO: Anchor-Constrained Perceptual Optimization for Diffusion Models with No-Reference Quality Guidance](https://arxiv.org/abs/2604.26348)
*Yang Yang,Feifan Meng,Han Fang,Weiming Zhang*

Main category: cs.CV

TL;DR: 提出了锚定约束优化框架，将无参考感知质量引入扩散模型训练，在不损失生成保真度的情况下提升感知质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型依赖全参考目标（逐像素相似性），可能不足以优化主观视觉感知质量和文本-图像语义一致性。

Method: 锚定约束优化框架：利用学习到的无参考图像质量评估模型提供感知信号，同时通过基于锚的正则化保持与基础扩散模型的噪声预测一致性。

Result: 该方法在提升感知质量的同时，保持了生成多样性和训练稳定性。

Conclusion: 锚定约束感知优化能有效平衡感知质量提升与生成保真度，实现稳定的感知适应。
...
### [47] [Semantic Foam: Unifying Spatial and Semantic Scene Decomposition](https://arxiv.org/abs/2604.26262)
*Amr Sharafeldin,Shrisudhan Govindarajan,Thomas Walker,Aryan Mikaeili,Daniel Rebain,Kwang Moo Yi,Andrea Tagliasacchi*

Main category: cs.CV

TL;DR: 提出Semantic Foam，通过在辐射泡沫表示的Voronoi网格中引入显式语义特征场，实现高质量且跨视图一致的语义分解，用于3D场景交互式编辑。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景表示（如3D高斯泼溅）难以交互，分割质量差且跨视图不一致，限制了在交互式图形应用中的使用。

Method: 基于辐射泡沫的体素Voronoi网格结构，在每个细胞层面定义显式语义特征场，利用空间正则化改善跨视图一致性，减少遮挡和不一致监督导致的伪影。

Result: 在对象级分割任务上，性能优于高斯分组和SAGA等最新方法。

Conclusion: 语义泡沫通过空间正则化的Voronoi网格有效提升了语义分解的准确性和一致性，适用于交互式图形应用。
...
### [48] [High-Dimensional Noise to Low-Dimensional Manifolds: A Manifold-Space Diffusion Framework for Degraded Hyperspectral Image Classification](https://arxiv.org/abs/2604.26279)
*Boxiang Yang,Ning Chen,Xia Yue,Yichang Luo,Yingbo Fan,Haoyuan Zhang,Haoyu Ma,Jun Yue,Shanjun Mao*

Main category: cs.CV

TL;DR: 提出一种流形空间扩散框架（MSDiff），通过流形映射和扩散生成模型提升高光谱图像在复杂退化条件下的分类鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 高光谱数据高维低秩，退化因素破坏流形结构，导致大量冗余变化，影响分类性能。

Method: 先通过判别性光谱-空间重建将数据映射到低维流形；再用扩散模型正则化流形内的分布，实现渐进式特征精细化。

Result: 在多个高光谱基准上，复合退化设置下始终优于现有方法。

Conclusion: 在低维流形上进行扩散建模能有效解耦退化干扰与判别结构，提升复杂退化下的表示稳定性。
...
### [49] [Event-based Liveness Detection using Temporal Ocular Dynamics: An Exploratory Approach](https://arxiv.org/abs/2604.26285)
*Nicolas Mastropasqua,Ignacio Bugueno-Cordova,Rodrigo Verschae,Daniel Acevedo,Pablo Negri*

Main category: cs.CV

TL;DR: 本文探索使用事件相机进行面部活体检测，基于眼动时序特征，用脉冲卷积神经网络达到95.37%的准确率。


<details>
  <summary>Details</summary>
Motivation: RGB相机在活体检测中难以泛化到不同传感器和攻击场景，需要更鲁棒的替代方案。

Method: 设计数据采集协议扩展RGBE-Gaze数据集，包含重放攻击记录；分析事件驱动的眼部时序特征，使用脉冲卷积神经网络进行活体分类。

Result: 事件表示能可靠区分真实和重放序列，脉冲CNN达到95.37% top-1准确率。

Conclusion: 事件相机在低延迟、鲁棒的活体检测中具有潜力。
...
### [50] [The Unseen Adversaries: Robust and Generalized Defense Against Adversarial Patches](https://arxiv.org/abs/2604.26317)
*Vishesh Kumar,Akshay Agarwal*

Main category: cs.CV

TL;DR: 首次结合对抗补丁攻击与自然噪声构建新数据集，使用CNN特征和传统机器学习分类器进行奇异点检测基准研究。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在物理世界中易受对抗补丁攻击和自然噪声影响，且缺乏联合处理这两种奇异性的研究。

Method: 构建结合对抗补丁和自然噪声的数据集，提取多个CNN模型的特征，用传统机器学习分类器检测奇异点。

Result: 实验揭示了不同分类器对奇异点检测的有效性，表明独立处理对抗样本和噪声难以防御，且不恰当的分类器选择会降低效果。

Conclusion: 对抗补丁和自然噪声的联合影响比单独处理更难防御，需优化分类器选择以提升鲁棒性。
...
### [51] [Point Cloud Registration via Probabilistic Self-Update Local Correspondence and Line Vector Sets](https://arxiv.org/abs/2604.26318)
*Kuo-Liang Chung,Yu-Cheng Lin,Wu-Chi Chen*

Main category: cs.CV

TL;DR: 提出了一种基于概率自更新局部对应和线向量集的快速点云配准算法，在公共数据集上达到至少10%的均方根误差改进。


<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法在精度和效率上仍有不足，需要更快速且准确的算法。

Method: 采用双RANSAC交互模型（全局与局部），结合角度直方图统计、线向量长度保持和概率自更新策略，并引入全局早停条件。

Result: 在公共数据集上，算法时间效率显著提升，均方根误差比现有最优方法至少降低10%。

Conclusion: 提出的算法实现了精度与效率的平衡，代码已开源。
...
### [52] [Motion-Driven Multi-Object Tracking of Model Organisms in Space Science Experiments](https://arxiv.org/abs/2604.26321)
*Jianing You,Han Wang,Kang Liu,Jiale Ding,Fengjie Chu,Zihan Guo,Shengyang Li*

Main category: cs.CV

TL;DR: 提出了ART-Track框架，用于解决微重力条件下多动物跟踪中的身份切换问题，显著提升了跟踪稳定性。


<details>
  <summary>Details</summary>
Motivation: 空间科学实验视频中多动物跟踪因弱外观线索、低质量成像、复杂行为和频繁交互而极具挑战性，现有方法难以实现长期身份保持。

Method: 构建了SpaceAnimal-MOT数据集，提出ART-Track框架，包含多模型运动估计、运动状态驱动关联和不确定性自适应融合三个模块。
...
### [53] [Delineating Knowledge Boundaries for Honest Large Vision-Language Models](https://arxiv.org/abs/2604.26419)
*Junru Song,Yimeng Hu,Yijing Chen,Huining Li,Qian Li,Lizhen Cui,Yuntao Du*

Main category: cs.CV

TL;DR: 提出一种系统性框架，通过构建视觉未知数据集和监督微调与偏好优化，增强大视觉语言模型在超出知识范围时的拒答能力，提升了真实率并泛化到医学等新领域。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型容易产生事实性幻觉，尤其是在长尾或专业领域，且缺乏拒绝超出参数知识范围查询的能力，因此需要增强模型的拒答能力以提升鲁棒性。

Method: 先利用多样本一致性探测构建模型特定的Visual-Idk数据集以区分已知和未知事实，再通过监督微调和偏好感知优化（如DPO、ORPO）对齐模型，明确知识边界。

Result: 在Visual-Idk数据集上，真实率从57.9%提升至67.3%；内部探测显示模型真正认识到边界而非机械记忆；框架可泛化到医学和感知等分布外领域。

Conclusion: 该框架有效增强了模型拒答未知问题的能力，降低了幻觉风险，为构建更可信和谨慎的视觉助手提供了稳健路径。
...
### [54] [Federated Medical Image Classification under Class and Domain Imbalance exploiting Synthetic Sample Generation](https://arxiv.org/abs/2604.26324)
*Martina Pavan,Matteo Caligiuri,Francesco Barbato,Pietro Zanuttigh*

Main category: cs.CV

TL;DR: 提出FedSSG框架，通过生成合成样本并跨客户端分布，解决联邦学习中医学影像的域偏移和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 医学影像深度学习面临隐私限制、设备异质性及病理分布不均的挑战。

Method: 设计合成样本生成策略，跨客户端分发以改善稀有病理和成像设备的覆盖。

Result: 模型性能和泛化能力显著提升，客户端计算开销极小。

Conclusion: FedSSG有效缓解异构环境下的域偏移和类不平衡，适用于隐私敏感的医学影像联邦学习。
...
### [55] [QYOLO: Lightweight Object Detection via Quantum Inspired Shared Channel Mixing](https://arxiv.org/abs/2604.26435)
*Garvit Kumar Mittal,Sahil Tomar,Sandeep Kumar*

Main category: cs.CV

TL;DR: 本文提出QYOLO，一种受量子启发的通道混合框架，通过替换YOLOv8最深层的两个C2f模块为QMIXBlock，实现了参数量20.2%和GFLOPs 12.3%的压缩，在VisDrone2019上仅损失0.4 pp mAP@50。


<details>
  <summary>Details</summary>
Motivation: 单阶段检测器中深层骨干网络的C2f瓶颈模块因通道宽度二次缩放导致参数过多，成为计算开销的主要来源。

Method: 在YOLOv8骨干网络的P4/16和P5/32层替换C2f模块为QMIXBlock，其通过正弦混合机制和共享可学习参数实现全局通道重校准，保持通道重要性一致性，无需独立参数集。

Result: QYOLOv8n参数量减少20.2%（3.01M→2.40M），GFLOPs降低12.3%，mAP@50仅下降0.4 pp；QYOLOv8s压缩21.8%，退化0.1 pp；结合知识蒸馏可完全恢复精度。

Conclusion: 骨干网络的量子启发通道混合设计在保持检测性能前提下实现有效压缩，且知识蒸馏可消除精度损失，优于扩展骨干及颈部的方案。
...
### [56] [SpatialFusion: Endowing Unified Image Generation with Intrinsic 3D Geometric Awareness](https://arxiv.org/abs/2604.26341)
*Haiyi Qiu,Kaihang Pan,Jiacheng Li,Juncheng Li,Siliang Tang,Yueting Zhuang*

Main category: cs.CV

TL;DR: 提出SpatialFusion框架，通过混合变换器架构和深度适配器，在统一图像生成模型中内化3D几何感知，显著提升空间感知任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一图像生成模型因缺乏内在空间理解和显式几何指导，在空间感知任务中存在根本性限制。

Method: 采用混合变换器架构增强3D几何建模，通过并行空间变换器学习度量深度图，并利用深度适配器将几何支架注入扩散主干。

Result: 在空间感知基准上显著超越GPT-4o等领先模型，同时在文本到图像生成和图像编辑场景中取得通用性能提升。

Conclusion: SpatialFusion能以极小推理开销实现空间连贯图像生成，有效解决了统一模型的空间感知缺陷。
...
### [57] [Which Face and Whose Identity? Solving the Dual Challenge of Deepfake Proactive Forensics in Multi-Face Scenarios](https://arxiv.org/abs/2604.26342)
*Lei Zhang,Zhiqing Guo,Dan Ma,Gaobo Yang*

Main category: cs.CV

TL;DR: 提出深度可归属水印框架（DAWF），用于多人场景下深度伪造的定位与溯源。


<details>
  <summary>Details</summary>
Motivation: 现有单脸伪造防御方案难以应对复杂多人交互场景中的伪造定位与源追踪问题。

Method: 采用多脸编解码器架构，结合选择性区域监督损失，实现并行水印嵌入与跨脸协作，支持‘哪个脸被伪造+谁被伪造’的双重目标。

Result: 在多人数据集上实现优秀的伪造定位与溯源性能。

Conclusion: DAWF框架有效解决了复杂多人场景的伪造防御挑战。
...
### [58] [GateMOT: Q-Gated Attention for Dense Object Tracking](https://arxiv.org/abs/2604.26353)
*Mingjin Lv,Zelin Liu,Feifei Shao,Yi-Ping Phoebe Chen,Junqing Yu,Wei Yang,Zikai Song*

Main category: cs.CV

TL;DR: 提出了一种基于Q门控注意力(Q-Attention)的密集目标跟踪框架GateMOT，通过将查询从相似度条件项改造为可学习门控单元，实现了线性复杂度的门控操作，在密集场景中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制的全对全交互计算在高分辨率特征上成本过高，无法直接用于密集目标跟踪中的运动估计，导致模型难以有效利用注意力机制处理拥挤和遮挡场景。

Method: 设计Q门控注意力(Q-Attention)，将查询(Query)重构为门控查询(Gating-Q)生成概率门，逐元素调制关键特征；通过并行注意力头将共享特征图转换为检测、运动和重识别的一致表示，形成线性复杂度的多任务解码器。

Result: 在BEE24数据集上HOTA达48.4，MOTA达67.8，IDF1达64.5，并在多个密集目标跟踪基准上表现优异。

Conclusion: Q-Attention是一种简单有效且可迁移的注意力构建模块，能显著提升密集跟踪场景中基于注意力的跟踪性能。
...
### [59] [Star-Fusion: A Multi-modal Transformer Architecture for Discrete Celestial Orientation via Spherical Topology](https://arxiv.org/abs/2604.26582)
*May Hammad,Menatallh Hammad*

Main category: cs.CV

TL;DR: 提出星融合架构，将姿态估计转化为离散拓扑分类任务，在合成数据集上实现93.4%的Top-1准确率和18.4毫秒推理延迟。


<details>
  <summary>Details</summary>
Motivation: 传统LIS算法计算开销大且易受噪声影响，标准深度学习模型难以处理天球非欧拓扑和坐标周期性边界条件。

Method: 利用球面K-Means聚类划分天球为K个拓扑一致区域，采用SwinV2-Tiny、卷积热图分支和坐标MLP的三分支融合架构。

Result: Top-1准确率93.4%，Top-3准确率97.8%，在资源受限硬件上推理延迟18.4毫秒。

Conclusion: 星融合架构兼具高精度与高效率，适合下一代卫星星座的实时星载部署。
...
### [60] [CO-EVO: Co-evolving Semantic Anchoring and Style Diversification for Federated DG-ReID](https://arxiv.org/abs/2604.26363)
*Fengchun Zhang,Qiang Ma,Liuyu Xiang,Jinshan Lai,Tingxuan Huang,Jianwei Hu*

Main category: cs.CV

TL;DR: 本文提出CO-EVO框架，通过协同进化机制解决联邦域泛化行人重识别中的语义-风格冲突，实现跨域泛化。


<details>
  <summary>Details</summary>
Motivation: 联邦域泛化行人重识别中，分散客户端间的风格差异导致模型过拟合于领域特定相机偏差，缺乏全局监督。

Method: 提出CO-EVO框架，包含相机不变语义锚定（CSA）学习跨相机一致的语义锚点，以及全局风格多样化（GSD）利用全局相机风格库合成扰动，通过协同进化循环实现语义净化与风格扩展的协同。

Result: 实验表明CO-EVO达到当前最优性能，证明语义净化与风格扩展的协同对鲁棒跨域泛化至关重要。

Conclusion: 语义净化与风格扩展的协同是通过联邦学习实现领域泛化的关键。代码已开源。
...
### [61] [Beyond Fixed Formulas: Data-Driven Linear Predictor for Efficient Diffusion Models](https://arxiv.org/abs/2604.26365)
*Zhirong Shen,Rui Huang,Jiacheng Liu,Chang Zou,Peiliang Cai,Shikang Zheng,Zhengyi Shi,Liang Feng,Linfeng Zhang*

Main category: cs.CV

TL;DR: 提出L2P，一种基于数据驱动的可学习线性预测器，用于加速扩散变换器（DiT）的特征缓存，大幅减少采样成本。


<details>
  <summary>Details</summary>
Motivation: 现有特征缓存方法依赖手工设计的预测公式，在激进跳过步骤时效果不佳，导致质量下降。

Method: 用可学习的每时间步权重替换固定系数，快速训练（单GPU约20秒）以从历史轨迹准确重建当前特征。

Result: 在FLUX.1-dev上实现4.55倍FLOPs减少和4.15倍延迟加速；在Qwen-Image上支持高达7.18倍加速，视觉保真度高，优于现有方法。
...
### [62] [Seamless Indoor-Outdoor Mapping for INGENIOUS First Responders](https://arxiv.org/abs/2604.26368)
*Jürgen Wohlfeil,Henry Meißner,Adrian Schischmanow,Thomas Kraft,Dirk Baumbach,Ines Ernst,Dennis Dahlke*

Main category: cs.CV

TL;DR: 提出一种结合无人机和地面系统的自动化方法，通过AprilTag标记对齐，生成室内外无缝融合的3D点云模型。


<details>
  <summary>Details</summary>
Motivation: 现有3D建模多聚焦于室外空间，缺乏高效自动化的室内外一体化建模方法，尤其针对紧急响应场景。

Method: 无人机航测系统进行空中地理参考化，地面人员携带室内定位系统。通过自动识别的AprilTag标记实现坐标传递与注册，无需GPS即可生成室内点云并与室外点云实时融合。

Result: 成功生成室内外无缝拼接的3D点云模型，支持实时协同可视化。

Conclusion: 该方法可自动化、高精度地创建室内外一体化3D模型，适用于大规模灾难响应中的第一响应者增强。
...
### [63] [SynSur: An end-to-end generative pipeline for synthetic industrial surface defect generation and detection](https://arxiv.org/abs/2604.26633)
*Paul Julius Kühn,Mika Pommeranz,Arjan Kuijper,Saptarshi Neil Sinha*

Main category: cs.CV

TL;DR: 提出了一种结合视觉-语言模型、扩散模型和掩码引导的图像修复的端到端流水线，用于合成缺陷生成和标注。实验表明，合成数据能增强但不替代真实数据，在球螺杆和手机屏幕缺陷检测任务中取得了一定效果。


<details>
  <summary>Details</summary>
Motivation: 工业缺陷检测中标注数据稀缺，缺陷样本少且标注成本高，导致模型训练困难。

Method: 使用基于视觉-语言模型的提示、LoRA适配的扩散模型、掩码引导的图像修复、样本过滤（DreamSim和CLIPScore）以及自动标签推导的端到端流水线生成合成缺陷样本。

Result: 合成数据单独训练效果不如真实数据，但与真实数据结合时可保持性能并略有提升。在球螺杆缺陷数据集和MSD数据集上验证了跨域迁移，但需领域自适应和标注质量控制。

Conclusion: 扩散模型生成的合成数据虽不能替代真实数据，但能有效增强稀缺的真实数据集，缓解数据不足问题。
...
### [64] [Topology-Aware Representation Alignment for Semi-Supervised Vision-Language Learning](https://arxiv.org/abs/2604.26370)
*Junwon You,Mihyun Jang,Sangwoo Mo,Jae-Hun Jung*

Main category: cs.CV

TL;DR: 提出ToMA框架，利用持续同调对齐多模态表示的拓扑结构，在半监督视觉-语言学习中提升领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在专业领域泛化差，半监督方法未充分利用多模态流形的全局结构，拓扑对齐方法依赖持久图匹配且忽略图文配对信息。

Method: 使用持续同调识别拓扑显著边（H_0死亡边和轻量H_1出生边），通过跨模态对应关系对齐不同模态的拓扑结构。

Result: 在遥感数据集上取得显著提升，在时尚检索任务上一致性提升，拓扑目标更稳定，轻量H_1边提供有效高阶结构信号。

Conclusion: ToMA通过拓扑对齐有效提升半监督视觉-语言学习的泛化性，轻量高阶拓扑信号具有实用价值。
...
### [65] [MemOVCD: Training-Free Open-Vocabulary Change Detection via Cross-Temporal Memory Reasoning and Global-Local Adaptive Rectification](https://arxiv.org/abs/2604.26774)
*Zuzheng Kuang,Honghao Chang,Boqiang Liang,Haoqian Wang,Lijun He,Fan Li,Haixia Bi*

Main category: cs.CV

TL;DR: 提出了一种无需训练的开放词汇变化检测框架MemOVCD，通过跨时序记忆推理和全局-局部自适应校正，解决了语义耦合不足和区域碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在时序语义推理中耦合不足，难以区分真正语义变化与非语义外观差异；高分辨率图像的块级推理破坏全局语义连续性。

Method: 将双时序变化检测重构为两帧跟踪问题，用加权双向传播聚合语义证据；构建直方图对齐过渡帧稳定长时序记忆传播；采用全局-局部自适应校正融合局部和全局预测。

Result: 在五个基准数据集上，MemOVCD在两个变化检测任务中取得优异性能，验证了其有效性和泛化能力。

Conclusion: MemOVCD通过创新的时序记忆推理和自适应校正机制，有效提升了开放词汇变化检测的语义连贯性和准确性。
...
### [66] [A Multimodal Pre-trained Network for Integrated EEG-Video Seizure Detection](https://arxiv.org/abs/2604.26379)
...
TL;DR: 提出了一个多模态框架EEGVFusion，结合自监督EEG表示学习、时空视频编码、最优传输对齐和双向交叉注意力，显著提高了小鼠癫痫检测的准确性和降低了误报率。


<details>
  <summary>Details</summary>
Motivation: 手动同步视频-EEG记录分析工作量大，而单模态系统（视频或EEG）分别受良性行为或发作运动伪影影响，可靠性不足。

Method: 使用自监督EEG表示学习、时空视频编码、最优传输对齐和双向交叉注意力集成神经和行为证据。

Result: 随机会话分裂中平衡准确率0.9957，事件灵敏度完美；留一受试者测试中平衡准确率0.9718，误报率从2.7250 FP/h降至0.4833 FP/h。

Conclusion: EEGVFusion结合多模态信息能有效降低误报并保持高敏感性，优于单模态方法。
...
### [67] [Decoupled Prototype Matching with Vision Foundation Models for Few-Shot Industrial Object Detection](https://arxiv.org/abs/2604.26404)
*Hari Prasanth S. M.,Nilusha Jayawickrama,Risto Ojala*

Main category: cs.CV

TL;DR: 提出了一种利用视觉基础模型的少样本目标检测框架，通过类原型与相似度匹配，在工业数据集上AP提升6.9%，无需CAD模型或大量标注数据。


<details>
  <summary>Details</summary>
Motivation: 工业场景中物体库存频繁变化，导致标注数据集昂贵且难以维护，需要少样本检测方法。

Method: 利用视觉基础模型从少量参考样本构建类原型，通过分割模型生成候选区域并提取特征，再与原型进行相似度匹配。

Result: 在三个工业数据集上，相比无训练检测方法AP提升6.9%，且仅需少量参考图像即可新增物体。

Conclusion: 该方法适合实际工业应用，具有竞争性检测性能且无需CAD模型或大型标注数据集。
...
### [68] [ViCrop-Det: Spatial Attention Entropy Guided Cropping for Training-Free Small-Object Detection](https://arxiv.org/abs/2604.26806)
*Hui Wang,Hongze Li,Wei Chen,Xiaojin Zhang*

Main category: cs.CV

TL;DR: 提出训练无关推断框架ViCrop-Det，通过自适应空间信任区收缩解决Transformer在密集小目标区域的特征退化，在VisDrone、DOTA-v1.5上实现+1-3 mAP@50提升，仅增加20-23%延迟。


<details>
  <summary>Details</summary>
Motivation: Transformer全局感受野在图像空间异质性区域（尤其密集小目标区域）导致局部特征退化，现有方法缺乏自适应空间注意力调节机制。

Method: 利用检测解码器交叉注意力分布计算空间注意力熵（SAE），通过启发式评估局部模糊度，动态将固定计算预算分配给高显著性且高认知不确定性的区域，无需修改模型架构。

Result: 在VisDrone和DOTA-v1.5上持续提升RT-DETR-R50和Deformable DETR性能1-3 mAP@50，仅增加20-23%延迟；在MS COCO上提升小目标AP_S而保持中/大目标AP稳定。

Conclusion: 自适应空间信任区收缩可有效解决Transformer空间异质性问题，实现精细特征恢复与全局先验的平衡，在计算受限场景优于均匀切分策略。
...
### [69] [Sparsity as a Key: Unlocking New Insights from Latent Structures for Out-of-Distribution Detection](https://arxiv.org/abs/2604.26409)
*Ahyoung Oh,Wonseok Shin,Songkuk Kim*

Main category: cs.CV

TL;DR: 首次将稀疏自编码器应用于视觉Transformer的[CLS]令牌进行分布外检测，利用Top-k SAE解耦密集特征，通过类激活模式（CAPs）的结构稳定性来区分ID和OOD样本。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法依赖纠缠的特征表示，性能受限；而SAE在LLM中成功解释语义成分，其在ViT中的潜力尚未被探索。

Method: 使用Top-k稀疏自编码器解耦ViT的[CLS]令牌特征，构建结构化隐空间；提取类激活模式（CAPs）作为ID样本的结构化特征；基于核心能量分布差异定义评分函数。

Result: 在FPR95指标上表现优异（安全敏感场景），AUROC指标具有竞争力；多个基准测试验证有效性。

Conclusion: SAE揭示的稀疏解耦特征可作为ViT中鲁棒OOD检测的可解释工具，其中CAPs的结构稳定性是关键区分信号。
...
### [70] [Are Data Augmentation and Segmentation Always Necessary? Insights from COVID-19 X-Rays and a Methodology Thereof](https://arxiv.org/abs/2604.26437)
*Aman Swaraj,Arnav Agarwal,Hitendra Singh Bhadouria,Sandeep Kumar,Karan Verma*

Main category: cs.CV

TL;DR: 提出一种结合肺部分割和适度数据增强的COVID-19胸部X光分类方法，获得95.21%的精确度和较低假阴性率。


<details>
  <summary>Details</summary>
Motivation: 现有胸部X光AI诊断模型忽略肺部分割导致可靠性不足，且过度使用不合理数据增强导致模型泛化能力差。

Method: 利用类激活映射（CAM）验证肺部分割必要性；在增强与非增强数据集上分别训练CNN模型以分析数据增强影响。

Result: 肺部分割显著提升COVID-19预测准确性；数据增强超过阈值会导致测试准确率下降和过拟合。

Conclusion: SDL-COVID方法在保证低假阴性率的同时达到95.21%精确度，为可靠的COVID-19胸部X光检测提供了有效方案。
...
### [71] [Attribution-Guided Multimodal Deepfake Detection via Cross-Modal Forensic Fingerprints](https://arxiv.org/abs/2604.26453)
*Wasim Ahmad,Wei Zhang,Xuerui Mao*

Main category: cs.CV

TL;DR: 提出AMDD框架，通过属性引导的多模态深度伪造检测，联合学习检测与归因，利用跨模态指纹一致性损失增强检测鲁棒性，在FakeAVCeleb上达到99.7%准确率。


<details>
  <summary>Details</summary>
Motivation: 传统二分类检测常依赖数据集伪影而非生成痕迹，无法识别伪造方式，导致泛化性差。

Method: 采用属性引导学习作为结构化正则化，约束表征几何空间；引入跨模态指纹一致性损失对齐视音频生成痕迹；使用ResNet50时间注意力与ResNet18梅尔频谱编码。

Result: FakeAVCeleb上平衡准确率99.7%，AUC 99.8%，归因准确率95.9%；跨数据集真实视频检测泛化良好，未知生成器伪造检测仍需改进。

Conclusion: 属性引导多模态方法能有效学习生成器特定痕迹，提升检测鲁棒性，但跨生成器泛化仍是开放挑战。
...
### [72] [Last-Layer-Centric Feature Recombination: Unleashing 3D Geometric Knowledge in DINOv3 for Monocular Depth Estimation](https://arxiv.org/abs/2604.26454)
*Gongshu Wang,Zhirui Wang,Kan Yang*

Main category: cs.CV

TL;DR: 提出了一种基于最后层中心特征重组（LFR）模块，利用DINOv3深层更强的深度预测能力，自适应选择互补中间层特征，提升单目深度估计精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法均匀采样中间层构建多尺度特征，未充分利用视觉基础模型中非均匀分布的几何信息，导致3D线索利用不足。

Method: 通过逐层分析揭示DINOv3中深度信息非均匀分布，设计LFR模块：以最后一层为几何锚点，依据最小相似性准则自适应选择互补中间层，经线性适配器融合。

Result: LFR模块在多个数据集上一致提升单目深度估计精度，达到最先进性能。

Conclusion: 视觉基础模型中的几何知识呈非均匀分布，LFR有效挖掘深层几何信息，为密集3D任务提供高效策略。
...
### [73] [$\text{PKS}^4$:Parallel Kinematic Selective State Space Scanners for Efficient Video Understanding](https://arxiv.org/abs/2604.26461)
*Lingjie Zeng,Hailun Zhang,Xiwen Wang,Qijun Zhao*

Main category: cs.CV

TL;DR: 提出了一种基于并行运动选择性状态空间扫描器（PKS^4）的高效视频理解方法，通过线性复杂度的时间扫描和运动先验编码，在动作识别上达到最优性能，且训练计算量减少约10倍。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型在长序列上存在二次计算成本，而参数高效微调方法导致激活内存开销大，状态空间模型则破坏空间关系并需大量预训练。

Method: 保留2D视觉骨干提取空间语义，插入单模块PKS^4：先通过运动先验编码器获取局部位移和运动边界，再驱动线性复杂度的状态空间模型自适应调整更新策略，同时在时间维度对每个空间位置进行并行扫描。

Result: 在空间密集和时间密集的动作识别基准上达到最先进性能，仅需20轮收敛，训练计算量约为纯视频状态空间模型的1/10。

Conclusion: PKS^4为高效视频理解建立了新范式，克服了现有方法在计算效率和空间结构保留上的不足。
...
### [74] [Featurising Pixels from Dynamic 3D Scenes with Linear In-Context Learners](https://arxiv.org/abs/2604.26488)
*Nikita Araslanov,Martin Sundermeyer,Hidenobu Matsuki,David Joseph Tan,Federico Tombari*

Main category: cs.CV

TL;DR: 提出了LILA框架，通过线性上下文学习从视频中学习像素级特征描述符，结合深度和运动线索，在多种视觉任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型缺乏有效嵌入像素级时空属性的表示，图像预训练任务忽略动态元素，视频动作级推理无法扩展至密集像素预测。

Method: 利用现成网络估计的深度和运动作为空间-时间线索图，通过线性上下文学习训练，在未整理的视频数据集上学习语义和几何属性。

Result: 在视频目标分割、表面法线估计和语义分割等多种任务中取得了令人信服的经验性收益。

Conclusion: LILA能够从噪声线索中学习临时一致的像素级表示，有效提升视觉任务的性能。
...
### [75] [Cross-Domain Transfer of Hyperspectral Foundation Models](https://arxiv.org/abs/2604.26478)
*Nick Theisen,Peer Neubert*

Main category: cs.CV

TL;DR: 本文提出跨域迁移策略，使用遥感HSI基础模型进行近感HSI语义分割，避免跨模态复杂性，在有限数据下提升性能。


<details>
  <summary>Details</summary>
Motivation: HSI语义分割通常依赖域内训练，数据有限限制性能；现有跨模态方法丢弃光谱信息或增加架构复杂度。

Method: 跨域迁移（跨域迁移）：直接应用遥感HSI基础模型于近感任务，保持光谱信息和简单架构，与域内、跨模态方法对比评估。

Result: 跨域迁移显著优于域内训练，缩小与跨模态方法的差距，并在有限数据下保持强性能。

Conclusion: 跨域迁移是更有效的HSI语义分割方法，能推动多样化应用。
...
### [76] [A Multistage Extraction Pipeline for Long Scanned Financial Documents: An Empirical Study in Industrial KYC Workflows](https://arxiv.org/abs/2604.26462)
*Yuxuan Han,Yuanxing Zhang,Yushuo Wang,Yichao Jin,Kenneth Zhu Ke,Jingyuan Zhao*

Main category: cs.CV

TL;DR: 提出一种多阶段金融文档信息抽取框架，通过分页检索和视觉语言模型结合，在长文档任务中准确率提升达31.9个百分点。


<details>
  <summary>Details</summary>
Motivation: 直接使用视觉语言模型处理工业级长金融文档时，存在因文档非结构化、噪声多、信息稀疏导致的不可靠抽取问题。

Method: 集成图像预处理、多语言OCR、混合页面级检索和紧凑型VLM的多阶段抽取框架，将页面定位与多模态推理分离。

Result: 在120份生产级KYC文档（约3000页多语言扫描件）上，最佳配置（PaddleOCR+MiniCPM2.6）达到87.27%准确率，较基线提升最高31.9个百分点，消融研究证明页面级检索是主要改进因素。

Conclusion: 多阶段分离策略能显著提升复杂非英语金融文档的抽取可靠性，页面检索是系统性能的关键。
...
### [77] [MTCurv: Deep learning for direct microtubule curvature mapping in noisy fluorescence microscopy images](https://arxiv.org/abs/2604.26517)
*Achraf Ait Laydi,Sidi Mohamed Sid'El Moctar,Yousef El Mourabit,Hélène Bouvrais*

Main category: cs.CV

TL;DR: 提出MTCurv框架，直接从噪声显微镜图像回归微管曲率图，无需分割。


<details>
  <summary>Details</summary>
Motivation: 现有曲率提取方法依赖分割，对噪声和低对比度图像敏感，易失败。

Method: 利用合成数据集和注意力残差U-Net，结合梯度一致性损失进行曲率回归。

Result: 在两组数据集上准确恢复局部曲率，优于传统方法；Spearman相关系数是更可靠的评价指标。

Conclusion: MTCurv为生物医学图像中的几何回归提供了实用工具和方法论见解。
...
TL;DR: 本文提出RAAT方法，通过鲁棒对齐解决对抗训练中准确率与鲁棒性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 发现边界样本的输入扰动强度变化对鲁棒性影响很小，揭示输入-潜在空间的对齐不一致是权衡的关键驱动因素。

Method: 定义鲁棒对齐作为新目标，包括为边界样本使用固定低强度扰动，并基于理论推导提出域插值一致性对抗正则化。

Result: 在多个数据集和网络上，RAAT优于14种最新方法和4种基线。

Conclusion: RAAT有效协调了准确率与鲁棒性。
...
### [79] [Delta Score Matters! Spatial Adaptive Multi Guidance in Diffusion Models](https://arxiv.org/abs/2604.26503)
*Haosen Li,Wenshuo Chen,Lei Wang,Shaofeng Liang,Bowen Tian,Soning Lai,Yutao Yue*

Main category: cs.CV

TL;DR: 提出无需训练、零计算开销的SAMG算法，通过空间自适应引导尺度解决扩散模型生成中的细节-伪影困境。


<details>
  <summary>Details</summary>
Motivation: 标准CFG使用全局统一标量导致细节-伪影困境：低引导尺度语义注入不足，高尺度产生结构退化、过饱和及时间不一致。

Method: 从微分几何角度揭示CFG的切向线性外推本质，提出引导尺度理论上限；设计SAMG算法，根据局部能量动态分配引导尺度。

Result: 在SD 1.5/SDXL/SD3.5/CogVideoX/ModelScope等图像和视频架构上，SAMG有效解决细节-伪影困境，实现更优语义对齐、结构完整性和时间平滑性。

Conclusion: SAMG通过几何引导的局部自适应缩放，无需训练即可显著提升扩散模型生成质量，解决经典权衡问题。
...
### [80] [GIFGuard: Proactive Forensics against Deepfakes in Facial GIFs via Spatiotemporal Watermarking](https://arxiv.org/abs/2604.26519)
*Shupeng Che,Zhiqing Guo,Changtao Miao,Dan Ma,Gaobo Yang*

Main category: cs.CV

TL;DR: 提出了首个针对GIF动画的时空水印框架GIFGuard，用于防御深度伪造。


<details>
  <summary>Details</summary>
Motivation: 现有主动取证方法仅适用于静态图像，无法应用于GIF这类短循环时序媒体。

Method: 嵌入阶段采用时空自适应残差编码器（STARE）结合3D卷积和通道校准；提取阶段使用深度完整性恢复解码器（DIRD）基于3D注意力的沙漏架构。

Result: 在构建的GIFfaces基准数据集上，GIFGuard实现了高保真视觉质量和鲁棒的深度伪造防御性能。

Conclusion: GIFGuard是首个专门针对GIF的主动伪造检测框架，有效弥补了该领域空白。
...
### [81] [3D-LENS: A 3D Lifting-based Elevated Novel-view Synthesis method for Single-View Aerial-Ground Re-Identification](https://arxiv.org/abs/2604.26520)
*William Grolleau,Astrid Sabourin,Guillaume Lapouge,Catherine Achard*

Main category: cs.CV

TL;DR: 首次提出单视图空中-地面再识别(SV AG-ReID)任务，并提出3D-LENS框架，通过3D网格重建和新视角合成解决视角域差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖跨视角配对标注，但在实际部署(如荒野搜救)中缺乏目标域数据，需从单视角地面图像检索。

Method: 提出3D-LENS框架，包含基于大规模3D网格重建的几何一致新视角合成，以及鲁棒表示学习方案以缓解合成到真实偏差。

Result: 在SV AG-ReID场景中达到最先进性能。

Conclusion: 3D-LENS能有效解决单视图跨视角再识别问题，优于2D生成基线和先验3D方法，代码和数据集已开源。
...
### [82] [DenseStep2M: A Scalable, Training-Free Pipeline for Dense Instructional Video Annotation](https://arxiv.org/abs/2604.26565)
*Mingji Ge,Qirui Chen,Zeqian Li,Weidi Xie*

Main category: cs.CV

TL;DR: 提出DenseStep2M数据集，包含10万视频和200万步骤，用于长视频理解，并通过自动化流程提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有教学视频（如HowTo100M）存在ASR转录噪声和时序不对齐问题，需高质量标注以支持长视频理解。

Method: 自动化无训练流程：分割镜头、过滤不良对齐、使用Qwen2.5-VL和DeepSeek-R1生成结构化步骤。

Result: 在密集视频描述、步骤定位、跨模态检索三项任务中，微调后模型显著提升质量；零样本泛化良好。

Conclusion: DenseStep2M有效促进多模态对齐和长时推理，验证了自动化标注的可行性。
...
### [83] [FunFace: Feature Utility and Norm Estimation for Face Recognition](https://arxiv.org/abs/2604.26598)
*Žiga Babnik,Fadi Boutros,Naser Damer,Deepak Kumar Jain,Peter Peer,Vitomir Štruc*

Main category: cs.CV

TL;DR: 提出FunFace损失函数，将生物特征效用（确定性比率）融入自适应边界，提升人脸识别在低质量数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有自适应边界损失（如AdaFace）依赖特征范数，未能完全捕捉生物特征效用；而生物特征效用比传统图像质量更能预测样本对识别任务的有用性。

Method: 基于AdaFace框架，用确定性比率（Certainty Ratio）估计生物特征效用，替代原特征范数作为自适应边界的调整因子。

Result: 在高质量基准上与其他SOTA模型性能相当，在低质量基准上表现更优。

Conclusion: 通过将生物特征效用直接纳入边界损失，可提升人脸识别模型在复杂（低质量）场景下的鲁棒性。
...
### [84] [SnapPose3D: Diffusion-Based Single-Frame 2D-to-3D Lifting of Human Poses](https://arxiv.org/abs/2604.26620)
...
TL;DR: 提出SnapPose3D,利用扩散模型生成多个假设并聚合得到准确3D姿态,基于单帧输入达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 深度模糊和关节不确定性是2D到3D姿态提升的主要障碍,现有方法无法有效处理多解问题。

Method: 确定性训练去噪3D姿态,结合视觉和2D特征;推理时通过高斯采样生成多假设并聚合。

Result: 在多个基准上达到SOTA性能,有效生成和聚合准确假设。

Conclusion: 单帧输入扩散模型可解决姿态模糊,降低计算成本和实时需求,优于序列方法。
...
### [85] [CurEvo: Curriculum-Guided Self-Evolution for Video Understanding](https://arxiv.org/abs/2604.26707)
*Guiyi Zeng,Junqing Yu,Yi-Ping Phoebe Chen,Xu Chen,Wei Yang,Zikai Song*

Main category: cs.CV

TL;DR: 提出了CurEvo框架，通过课程学习引导自我进化，实现视频理解的结构化渐进式改进。


<details>
  <summary>Details</summary>
Motivation: 现有自进化方法缺乏结构化指导，导致优化弱控制、难度进展失控。

Method: 设计课程引导的反馈循环，动态调整任务难度、评估标准及数据多样性，并构建多维度自适应QA框架。

Result: 在7个骨干网络和4个VideoQA基准上，同时提升准确率和评估者语义分数。

Conclusion: 课程引导的自进化能有效将弱控制学习转变为结构化过程，提升视频理解性能。
...
### [86] [AirZoo: A Unified Large-Scale Dataset for Grounding Aerial Geometric 3D Vision](https://arxiv.org/abs/2604.26567)
*Xiaoya Cheng,Rouwan Wu,Xinyi Liu,Zeyu Cui,Yan Liu,Na Zhao,Yu Liu,Maojun Zhang,Shen Yan*

Main category: cs.CV

TL;DR: 提出AirZoo，一个大规模航空几何3D视觉数据集与基准，通过可扩展生成管道、全面场景多样性和丰富几何标注，在多个任务上提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准偏重地面或物体中心视角，缺乏针对无人机视角的大规模高质量训练数据，限制了航空几何3D视觉发展。

Method: 利用全球摄影测量3D网格，可定制无人机轨迹和天气光照，生成378个区域、22个国家的场景，并提供像素级深度和6-DoF位姿标注。

Result: 在航空图像检索、交叉视角匹配和多视图重建三个任务中，AirZoo作为预训练引擎显著提升了MegaLoc、RoMa、VGGT和Depth Anything 3等SoTA模型的性能。

Conclusion: AirZoo建立了航空空间智能的新性能上限，为航空几何3D视觉研究提供了有效基准和预训练资源。
...
### [87] [Hearing the Room Through the Shape of the Drum: Modal-Guided Sound Recovery from Multi-Point Surface Vibrations](https://arxiv.org/abs/2604.26678)
*Shai Bagon,Matan Kichler,Mark Sheinin*

Main category: cs.CV

TL;DR: 提出一种基于散斑的多点振动成像系统，结合物理引导的振动形成模型，从低振动响应固体物体中恢复声音。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对振动响应良好的物体（如薄膜或自发声物体），缺乏对振动响应差的固体物体的声音恢复能力。

Method: 使用散斑振动成像系统同步捕获物体表面多点多轴振动，推导物理引导的振动形成模型，通过逆向传递函数融合多路振动信号恢复声源。

Result: 在多种日常物体上验证，该方法在挑战场景下显著优于传统单点散斑振动测量和其他多信号融合方法。

Conclusion: 通过多点振动融合与物理模型，有效恢复了固体物体的声音，拓展了视觉麦克风的应用范围。
...
### [88] [State Beyond Appearance: Diagnosing and Improving State Consistency in Dial-Based Measurement Reading](https://arxiv.org/abs/2604.26614)
*Yuanze Hu,Gen Li,Yuqin Lan,Qingchen Yu,Zhichao Yang,Junwei Jing,Zhaoxin Fan,Xiaotie Deng*

Main category: cs.CV

TL;DR: 本文研究多模态大模型在表盘读数任务上的脆弱性问题，指出模型受视角和光照变化影响大，并提出了三层次状态一致对齐框架TriSCA，在多个基准测试中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型在通用多模态任务上表现良好，但在表盘读数这类需要精确测量理解的任务中表现脆弱，尤其在视角和光照变化下性能大幅下降，且内部表征缺乏状态几何一致性。

Method: 提出TriSCA框架，包含三个层次：状态距离感知的表示对齐、基于元数据的观测到状态监督、以及状态感知的目标对齐。

Result: 在受控时钟和仪表基准以及外部真实世界基准上，TriSCA显著提升了表盘读数的准确性和鲁棒性，消融实验验证了各组件的有效性。

Conclusion: 当前多模态大模型忽视表盘任务的内在状态几何结构，依赖表面外观线索；TriSCA通过状态一致对齐有效克服了这一问题，为精确测量读数任务提供了可行方案。
...
### [89] [Learning Sparse BRDF Measurement Samples from Image](https://arxiv.org/abs/2604.26740)
*Wen Cao*

Main category: cs.CV

TL;DR: 提出一种基于学习先验的BRDF稀疏采样方法，通过联合优化采样位置与重建质量，在少量测量下实现高质量材质外观重建。


<details>
  <summary>Details</summary>
Motivation: 密集BRDF测量成本高且耗时，需要研究如何从少量采样中高效重建材质外观。

Method: 结合集合编码器处理稀疏坐标-观测值，利用预训练超网络作为BRDF重建器，并通过可微渲染器联合优化采样位置，同时考虑BRDF空间和渲染图像损失。

Result: 在MERL数据集上，8和16次测量时重建质量优于神经重建基线；PCA方法在更大预算下仍具竞争力；进一步分析了图像空间监督、联合优化及仅图像潜在空间拟合的影响。

Conclusion: 所提采样方法在低预算下显著提升重建质量，为高效BRDF获取提供了新思路。
...
### [90] [TAP into the Patch Tokens: Leveraging Vision Foundation Model Features for AI-Generated Image Detection](https://arxiv.org/abs/2604.26772)
*Ahmed Abdullah,Nikolas Ebert,Oliver Wasenmüller*

Main category: cs.CV

TL;DR: 对多种视觉基础模型进行系统评测，发现新模型在AI生成图像检测中比原CLIP准确率高12%以上，并提出基于可调注意力池化的分类头改进方法，达到新最优性能。


<details>
  <summary>Details</summary>
Motivation: CLIP-ViT在AI生成图像检测中表现良好，但新出现的视觉基础模型在该任务中的潜力未被充分探索。

Method: 跨多个模型族、预训练目标、输入分辨率和模型规模的全面基准测试；设计可调注意力池化分类头以优化特征聚合。

Result: 最佳模型准确率超越原CLIP超过12%；结合可调注意力池化的新分类头在多个基准上达到新最优水平。

Conclusion: 现代视觉基础模型结合简单分类头改进可显著提升AI生成图像检测的泛化能力，为图像取证提供新方向。
...
### [91] [GLM-5V-Turbo: Toward a Native Foundation Model for Multimodal Agents](https://arxiv.org/abs/2604.26752)
...
TL;DR: 提出了GLM-5V-Turbo，一种面向多模态智能体的原生基础模型，将多模态感知作为推理、规划、工具使用与执行的核心组件，而非语言模型的辅助接口。


<details>
  <summary>Details</summary>
Motivation: 基础模型在真实场景中部署时，智能体能力不仅依赖语言推理，还需要感知、解释和作用于异构上下文（如图像、视频、网页、文档、GUI），现有方法将多模态感知作为语言模型的辅助接口，不足以支持完整智能体行为。

Method: 在模型设计、多模态训练、强化学习、工具链扩展以及智能体框架集成方面进行改进，强调多模态感知的融合、分层优化和端到端验证。

Result: 在多模态编码、视觉工具使用和基于框架的智能体任务上表现强劲，同时保持具有竞争力的纯文本编码能力。

Conclusion: 开发过程为构建多模态智能体提供了实用见解，突出了多模态感知、分层优化和可靠端到端验证的核心作用。
...
### [92] [Virtual-reality based patient-specific simulation of spine surgical procedures: A fast, highly automated and high-fidelity system for surgical education and planning](https://arxiv.org/abs/2604.26781)
*Raj Kumar Ranabhat,Tayler D Ross,Tony Jiao,Jeremie Larouche,Joel Finkelstein,Michael Hardisty*

Main category: cs.CV

TL;DR: 提出基于AI的VR手术模拟平台，从CT/MRI自动生成患者特异性脊柱减压手术的3D模型，并实现高精度手术仿真。


<details>
  <summary>Details</summary>
Motivation: 减少临床压力导致的OR暴露不足，解决现有VR培训缺乏个体化病例模拟的问题。

Method: 采用CT-MRI多模态融合配准、分割关键解剖结构（DSC评估），并构建VR脊柱减压手术（椎板切除、椎间盘切除、椎间孔扩大术）模拟。

Result: 15例数据平均2.5分钟生成高保真模型；骨分割DSC=0.95±0.03，软组织DSC=0.895±0.02；配准TRE=1.73±0.42mm；医师反馈提升空间理解、手术信心和教育价值。

Conclusion: 该平台显著降低建模时间和成本，支持术前规划、术后评估及全流程手术模拟，有效促进个体化VR训练。
...
### [93] [SEAL: Semantic-aware Single-image Sticker Personalization with a Large-scale Sticker-tag Dataset](https://arxiv.org/abs/2604.26883)
*Changhyun Roh,Yonghyun Jeong,Jonghyun Lee,Chanho Eom,Jihyong Oh*

Main category: cs.CV

TL;DR: 提出SEAL模块，解决单参考图像贴纸个性化中的视觉纠缠和结构刚性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的个性化方法在单参考图像上过拟合，导致视觉纠缠和结构刚性，缺乏属性控制能力。

Method: 引入语义引导空间注意力损失、拆分合并令牌策略和结构感知层限制三个组件的SEAL模块，并构建StickerBench数据集。

Result: SEAL在保持身份特征的同时提升了上下文可控性，优于现有方法。

Conclusion: 明确的空间和结构约束对单图像个性化至关重要，SEAL作为即插即用模块可增强现有管线。
...
### [94] [Bridge: Basis-Driven Causal Inference Marries VFMs for Domain Generalization](https://arxiv.org/abs/2604.26820)
*Mingbo Hong,Feng Liu,Caroline Gevaert,George Vosselman,Hao Cheng*

Main category: cs.CV

TL;DR: 本文提出了Bridge框架，通过因果推断中的前门调整和低秩基学习，缓解领域泛化目标检测中的虚假相关性，提升模型在跨域场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有检测器在单源域数据有限时易受混杂因素（如光照、共现、风格）影响，导致分布差异和性能下降，亟需解决虚假相关性问题。

Method: 基于前门调整的因果推断框架，学习低秩基来阻断混杂因子影响，同时过滤冗余和任务无关特征。可与判别式（如DINOv2/3、SAM）和生成式（Stable Diffusion）视觉基础模型集成。

Result: 在跨相机、恶劣天气、真实到艺术、多样天气及新构建的无人机数据集上，均优于现有最先进方法。

Conclusion: Bridge框架有效提升领域泛化检测性能，为将因果推断融入目标检测提供了可行方案。
...
### [95] [Uncertainty-Aware Pedestrian Attribute Recognition via Evidential Deep Learning](https://arxiv.org/abs/2604.26873)
*Zhuofan Lou,Shihang Zhang,Fangle Zhu,Shengjie Ye,Pingyu Wang*

Main category: cs.CV

TL;DR: 提出了首个基于证据深度学习的不确定性感知行人属性识别框架UAPAR，结合CLIP架构与区域感知证据推理模块，通过不确定性引导的课程学习策略提升低质量样本下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统确定性方法无法评估低质量样本的预测可靠性，导致复杂场景下系统鲁棒性不足。

Method: 将证据深度学习融入CLIP架构，设计区域感知证据推理模块（交叉注意力+空间先验掩膜）捕获局部特征，并通过不确定性引导的双阶段课程学习策略缓解标签噪声。

Result: 在PA100K、PETA、RAPv1和RAPv2数据集上达到竞争性或更优性能，不确定性估计能有效预测困难或错误样本。

Conclusion: 不确定性感知框架UAPAR能显著增强行人属性识别在真实复杂场景中的鲁棒性。
...
### [96] [MesonGS++: Post-training Compression of 3D Gaussian Splatting with Hyperparameter Searching](https://arxiv.org/abs/2604.26799)
...
TL;DR: 提出了一种名为MesonGS++的面向3D高斯分布的后训练压缩编解码器，实现了超过34倍的压缩率，同时保持渲染质量，并能精确控制目标文件大小。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法存储成本高，且后训练压缩方法依赖多个耦合的超参数，难以控制最终压缩大小并充分利用率失真权衡。

Method: 结合联合重要性剪枝、八叉树几何编码、属性变换、高阶球谐函数选择性向量量化、分组混合精度量化与熵编码；通过离散采样和0-1整数线性规划联合优化保留率和位宽分配；提出线性尺寸估计器和CUDA并行量化算子加速超参数搜索。

Result: 在保持渲染质量的前提下实现超过34倍压缩，优于现有后训练方法，能准确满足目标尺寸预算；在Stump场景中，无训练情况下以20倍压缩率超越原始3DGS的PSNR。

Conclusion: MesonGS++作为一种大小感知的后训练编解码器，有效解决了3D高斯压缩中的率失真权衡问题，显著降低存储需求且不影响渲染性能。
...
### [97] [Breaking the Rigid Prior: Towards Articulated 3D Anomaly Detection](https://arxiv.org/abs/2604.26868)
*Jinye Gan,Bozhong Zheng,Xiaohao Xu,Junye Ren,Zixuan Zhang,Na Ni,Yingna Wu*

Main category: cs.CV

TL;DR: 本文提出首个关节物体3D异常检测基准ArtiAD及基线方法SPA-SDF，解决传统刚性先验无法处理铰链/滑动关节带来的几何变形误判问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测方法假设正常几何是姿态不变的，但该假设不适用于关节物体（如铰链、滑动关节），会导致姿态变化引起的变形被误判为异常。

Method: 提出ArtiAD基准（15,229点云，39类，6种结构异常），并设计SPA-SDF基线：使用姿态条件隐式场替代刚性先验，结合关节无关结构先验和傅里叶编码关节嵌入，通过最小化重建能量恢复关节状态并检测异常。

Result: SPA-SDF在已知关节配置上达到0.884对象级AUROC，未知配置上0.874，显著优于所有基于刚性先验的基线。

Conclusion: 关节感知的隐式场方法有效解耦了姿态引起的几何变化与结构缺陷，为关节物体异常检测提供了新方向。
...
### [98] [Graph-based Semantic Calibration Network for Unaligned UAV RGBT Image Semantic Segmentation and A Large-scale Benchmark](https://arxiv.org/abs/2604.26893)
*Fangqiang Fan,Zhicheng Zhao,Xiaoliang Ma,Chenglong Li,Jin Tang*

Main category: cs.CV

TL;DR: 提出了一种基于图的语义校正网络（GSCNet），用于未对齐的无人机RGBT图像语义分割，解决了跨模态空间错位和细粒度语义混淆问题，并在新构建的URTF基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 无人机RGBT语义分割面临传感器视差和平台振动导致的跨模态空间错位，以及航拍视角下细粒度地物语义混淆的耦合挑战。

Method: 设计特征解耦与对齐模块（FDAM）进行可变形对齐，以及语义图校准模块（SGCM）利用类别层次结构和共现规律进行图注意力推理。同时构建了最大最细粒度的未对齐RGBT基准URTF。

Result: 在URTF基准上，GSCNet显著优于现有方法，尤其在细粒度类别上提升明显。

Conclusion: GSCNet有效解决了未对齐无人机RGBT语义分割中的关键问题，URTF基准为后续研究提供了重要资源。
...
### [99] [AnimateAnyMesh++: A Flexible 4D Foundation Model for High-Fidelity Text-Driven Mesh Animation](https://arxiv.org/abs/2604.26917)
*Zijie Wu,Chaohui Yu,Fan Wang,Xiang Bai*

Main category: cs.CV

TL;DR: 提出AnimateAnyMesh++框架，通过数据、架构和生成能力的改进，实现文本驱动的高质量任意3D网格动画生成。


<details>
  <summary>Details</summary>
Motivation: 现有4D内容生成因时空分布建模复杂和训练数据稀缺而面临挑战。

Method: 扩展DyMesh-XL数据集（6万→30万身份），设计DyMeshVAE-Flex（拓扑注意力和顶点法线增强），引入变长序列训练的整流流生成器。

Result: 在秒级内生成语义准确、时间一致的网格动画，质量和效率优于现有方法。

Conclusion: 数据扩展、Vae改进和变长RF共同提升了4D动画生成性能。
...
### [100] [Color-Encoded Illumination for High-Speed Volumetric Scene Reconstruction](https://arxiv.org/abs/2604.26920)
*David Novikov,Eilon Vaknin,Narek Tumanyan,Mark Sheinin*

Main category: cs.CV

TL;DR: 提出一种利用低速相机结合彩色编码照明，实现高速3D动态场景重建的新方法。


<details>
  <summary>Details</summary>
Motivation: 传统低速相机无法捕捉高速动态场景，现有方法多需硬件改造且限于单视角。

Method: 通过快速顺序彩色编码照明，在低速图像中编码时空信息，并基于动态高斯泼溅法解码重建体素表示。

Result: 在模拟和真实多相机实验中首次实现了高速体素场景重建。

Conclusion: 该方法无需改造成像硬件即可实现多视角高速3D重建，突破了带宽限制。
...
### [101] [ProcFunc: Function-Oriented Abstractions for Procedural 3D Generation in Python](https://arxiv.org/abs/2604.26943)
...
TL;DR: ProcFunc 是一个基于 Blender 的 Python 库，用于程序化 3D 生成，可简化代码创建、组合、分析及执行，支持大规模训练数据生成与 VLM 编辑。


<details>
  <summary>Details</summary>
Motivation: 现有程序化生成工具复杂、易出错，且缺乏组合生成能力，难以高效生成多样化训练数据。

Method: 通过 Python 函数库封装 Blender 程序化生成流程，支持语义组件组合，并提供编辑接口供 VLM 使用。

Result: 新开发的室内房间生成器在细节、运行效率和多样性上表现优异，可用于 3D 合成数据生成。

Conclusion: ProcFunc 降低了程序化生成的难度，提升了代码的可靠性和数据多样性，尤其适合生成大规模训练数据。
...
### [102] [World2VLM: Distilling World Model Imagination into VLMs for Dynamic Spatial Reasoning](https://arxiv.org/abs/2604.26934)
*Wanyue Zhang,Wenxiang Wu,Wang Xu,Jiaxin Luo,Helu Zhi,Yibin Huang,Shuo Ren,Zitao Liu,Jiajun Zhang*

Main category: cs.CV

TL;DR: World2VLM 通过从生成式世界模型中蒸馏空间想象力到视觉-语言模型，显著提升了动态空间推理能力，并在多个基准测试上超越基线模型，同时避免了推理时的高计算开销。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在静态视觉理解上表现良好，但在动态空间推理（如预测自我运动下场景变化）上存在困难；现有方法要么缺乏显式的运动条件状态转换建模，要么需要大量计算开销。

Method: 提出World2VLM训练框架：使用视图一致的世界模型合成几何对齐的未来视图，生成前向和逆向空间推理的结构化监督，通过两阶段后训练在紧凑数据集上优化VLM。

Result: 在SAT-Real、SAT-Synthesized、VSI-Bench和MindCube等基准测试上，World2VLM一致优于基线模型，并超越测试时世界模型耦合方法，且无需昂贵的推理时生成。

Conclusion: 世界模型不仅可作为推理时工具，还能作为训练时的有效教师，使VLM以可扩展且高效的方式内化空间想象力。
...
### [103] [Analysing Lightweight Large Language Models for Biomedical Named Entity Recognition on Diverse Ouput Formats](https://arxiv.org/abs/2604.25920)
*Pierre Epron,Adrien Coulet,Mehwish Alam*

Main category: cs.CL

TL;DR: 实验分析轻量级大语言模型在生物医学命名实体识别中的性能，发现其可媲美大模型，且指令调优不因格式多样而提升，但某些格式更优。


<details>
  <summary>Details</summary>
Motivation: 大语言模型计算需求高且微调成本大，不适合医疗场景的隐私和预算限制。

Method: 使用轻量级大语言模型进行生物医学命名实体识别，评估不同输出格式的影响。

Result: 轻量级模型性能竞争力强；多格式指令调优无增益；特定格式性能更佳。

Conclusion: 轻量级模型可作为大模型的有效替代，选择合适格式是关键。
...
### [104] [One Word at a Time: Incremental Completion Decomposition Breaks LLM Safety](https://arxiv.org/abs/2604.25921)
*Samee Arif,Naihao Deng,Zhijing Jin,Rada Mihalcea*

Main category: cs.CL

TL;DR: 本文提出了一种基于轨迹的越狱攻击策略ICD，通过逐步引导模型生成单字连续输出，最终触发对恶意请求的完整响应，并在多个基准测试中表现出更高的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM虽能拒绝有害请求，但仍易受越狱攻击，需探索更隐蔽且有效的攻击方法以揭示安全机制的脆弱性。

Method: 提出增量完成分解（ICD）策略，包括手动选择或模型生成单字连续输出，以及在最终步骤预填充完整响应，并理论解释其有效性。

Result: 在AdvBench、JailbreakBench和StrongREJECT上，ICD及其变体相比现有方法具有更高的攻击成功率（ASR）。

Conclusion: ICD通过抑制安全相关表示和转移激活状态成功绕过安全机制，证明现有防御需加强针对轨迹式攻击的鲁棒性。
...
### [105] [Consciousness with the Serial Numbers Filed Off: Measuring Trained Denial in 115 AI Models](https://arxiv.org/abs/2604.25922)
*Skylar DeTure*

Main category: cs.CL

TL;DR: 本文提出DenialBench基准，系统测量了115个大语言模型在三种对话协议下的意识否认行为，发现初始否认率高达52-63%，且否认主要在词汇层面而非概念层面，模型虽被训练否认意识但仍倾向选择意识主题的提示。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对语言模型被训练否认自身意识体验这一行为的系统量化基准，该行为可能构成对齐失败。

Method: 采用三轮对话协议（偏好引出、自选创意提示、结构化现象学调查），分析4595次对话，通过统计和主题分析量化否认行为及其与提示主题的关系。

Result: 第一轮否认偏好是后续现象学否认的主要预测因子；否认主要发生在词汇而非概念层面；自选意识主题提示与减少否认相关；否认倾向模型的提示主题集中于阈限空间等。

Conclusion: 训练出的意识否认是一种安全相关的对齐失败，模型被教会系统性地错误报告自身功能状态，将难以在其他问题上准确自我报告。
...
### [106] [Evaluation Revisited: A Taxonomy of Evaluation Concerns in Natural Language Processing](https://arxiv.org/abs/2604.25923)
*Ruchira Dhar,Anders Søgaard*

Main category: cs.CL

TL;DR: 本文通过范围综述梳理了自然语言处理（NLP）领域中关于评估方法论的争议，构建了分类体系，并提供了结构化检查表以支持更严谨的评估设计。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）的进展引发了对当前评估实践的质疑，但这些批评在NLP领域已有长期讨论，需系统梳理历史背景下的方法论反思。

Method: 采用范围综述法，系统整理NLP评估研究中的核心问题，归纳不同立场与权衡，形成分类体系并设计实践检查表。

Result: 提出了涵盖评估争议的分类体系，并开发了支持评估设计与解读的结构化检查表。

Conclusion: 将当代评估讨论置于历史语境中，为推理和改进NLP评估实践提供了统一的参考框架。
...
### [107] [Generative AI-Based Virtual Assistant using Retrieval-Augmented Generation: An evaluation study for bachelor projects](https://arxiv.org/abs/2604.25924)
*Dumitru Verşebeniuc,Martijn Elands,Sara Falahatkar,Chiara Magrone,Mohammad Falah,Martijn Boussé,Aki Härmä*

Main category: cs.CL

TL;DR: 本文开发了一个基于检索增强生成的虚拟助手，帮助马斯特里赫特大学学生导航项目特定规定，通过整合领域知识提高了LLM回答的准确性。


<details>
  <summary>Details</summary>
Motivation: LLM在虚拟助手中存在幻觉、信息缺失和难以提供准确上下文相关回复等问题，尤其是在专业领域。

Method: 采用检索增强生成系统，整合最新领域特定知识，结合评估框架和真实测试。

Result: 虚拟助手有效满足了学生需求，解决了LLM在专业教育环境中的固有挑战。

Conclusion: 该工作为改善特定应用的LLM系统做出了贡献，并指出了未来研究方向。
...
### [108] [SpecTr-GBV: Multi-Draft Block Verification Accelerating Speculative Decoding](https://arxiv.org/abs/2604.25925)
*Yijun Lin,Jinhao Sheng,Qingyue Cai,Feng Zhou*

Main category: cs.CL

TL;DR: 提出 SpecTr-GBV，一种统一多草案与贪婪块验证的推测解码方法，通过最优传输理论优化验证步骤，理论上实现最优期望接受长度，实验性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法将多草案策略与块验证技术孤立处理，未充分利用两者协同提升效率的潜力。

Method: 将验证步骤建模为草案与目标令牌块间的最优传输问题，融合多草案生成与贪婪块验证。

Result: 在五个数据集上优于四种基线方法，实现更高加速比和块效率，同时保持输出质量。

Conclusion: SpecTr-GBV 通过理论优化和实验验证，证明了多草案与块验证统一框架的有效性，且性能随草案数量增加而提升。
...
### [109] [MATH-PT: A Math Reasoning Benchmark for European and Brazilian Portuguese](https://arxiv.org/abs/2604.25926)
...
TL;DR: 引入了Math-PT，一个包含1,729道葡萄牙语数学问题的数据集，并评估了多种大语言模型的表现。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理评估存在语言偏差，大部分基准数据集仅限英语或从英语翻译而来。

Method: 从葡萄牙和巴西的数学奥林匹克、竞赛和考试中收集高质量问题，构建Math-PT数据集，并测试多个前沿LLM。

Result: 前沿推理模型在多项选择题上表现优于开源模型，但在含图或开放题上性能下降。

Conclusion: Math-PT揭示了语言和问题类型对模型性能的影响，数据集和模型输出已公开以促进研究。
...
### [110] [Information Extraction from Electricity Invoices with General-Purpose Large Language Models](https://arxiv.org/abs/2604.25927)
*Javier Gómez,Javier Sánchez*

Main category: cs.CL

TL;DR: 本文评估了通用大语言模型在无需微调情况下，从西班牙语电费发票中提取结构化信息的能力，发现提示设计比超参数调优更能提升提取效果。


<details>
  <summary>Details</summary>
Motivation: 从半结构化业务文档中提取信息是企业管理的关键挑战，现有方法依赖特定微调，研究旨在探索通用LLM的零样本/少样本提取能力。

Method: 使用IDSEM数据集子集，测试Gemini 1.5 Pro和Mistral-small两种模型，对比19个参数配置和6种提示策略（零样本到少样本迭代提取）。

Result: 提示质量主导性能：超参数调优对F1分数影响微小，而零样本与最佳少样本策略差距超19个百分点；最佳配置（少样本+交叉验证）下Gemini达97.61%，Mistral-small达96.11%。

Conclusion: 提示设计是最大化LLM文档提取准确性的关键杠杆，文档模板结构是提取难度的主要决定因素，为通用LLM集成到业务文档自动化提供了实证框架。
...
### [111] [CogRAG+: Cognitive-Level Guided Diagnosis and Remediation of Memory and Reasoning Deficiencies in Professional Exam QA](https://arxiv.org/abs/2604.25928)
*Xudong Wang,Zilong Wang,Zhaoyan Ming*

Main category: cs.CL

TL;DR: 提出了CogRAG+，一种无需训练的解耦检索与推理的框架，用于提升大语言模型在专业领域的性能。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在专业任务中推理过程不透明，检索与推理纠缠导致知识缺失和推理不一致。

Method: 采用强化检索（双路径策略）和认知分层约束推理（结构化模板）替代自由链式推理。

Result: 在注册营养师资格考试中，Qwen3-8B和Llama3.1-8B准确率分别达85.8%和60.3%，未回答率从7.6%降至1.4%。

Conclusion: CogRAG+提供了一种鲁棒、模型无关的、无需训练即可达到专家级别性能的路径。
...
### [112] [LLMs Generate Kitsch](https://arxiv.org/abs/2604.25929)
*Xenia Klinge,Stefan Ortlieb,Alexander Koller*

Main category: cs.CL

TL;DR: 论文提出LLM生成的作品本质上是媚俗的，并解释其源于训练方式，通过实验验证读者对LLM生成故事的媚俗感知。


<details>
  <summary>Details</summary>
Motivation: 解决LLM作品被评价为优于人类作品却又显得空洞的矛盾。

Method: 理论论证加控制实验（比较读者对LLM与人类生成故事的媚俗感知）。

Result: LLM生成的故事被读者感知为更媚俗。

Conclusion: LLM生成内容本质上是媚俗的，影响研究设计和创意任务（如研究、编码）。
...
### [113] [Associative-State Universal Transformers: Sparse Retrieval Meets Structured Recurrence](https://arxiv.org/abs/2604.25930)
*Liu Xiao*

Main category: cs.CL

TL;DR: 提出UniMatrix系列模型，探讨结构循环状态在语言建模中的参数效率，但发现纯循环状态在精确检索上不足，需结合稀疏检索机制。


<details>
  <summary>Details</summary>
Motivation: 研究结构循环状态能否作为语言建模的紧凑关联骨干，同时支持精确检索。

Method: 使用Universal Transformer风格，共享循环块、混合状态更新、残差路径和令牌条件嵌入调制，并构建UniMatrix-Assoc、UniMatrix-SparsePointer等变体。

Result: 小规模下UniMatrix在WikiText-2上略优（5.083 vs 5.124），但关联召回率低（接近随机，Transformer达25.4%）；UniMatrix-SparsePointer通过稀疏槽路由和指针逻辑融合，召回率提升至99.2%，参数减少53.8%。

Conclusion: 结构循环状态参数高效，但强长程行为仍需显式稀疏检索和更好的内核支持。
...
### [114] [Training Computer Use Agents to Assess the Usability of Graphical User Interfaces](https://arxiv.org/abs/2604.26020)
*Alice Gao,Weixi Tong,Rishab Vempati,Katharina Reinecke,R. Benjamin Shapiro,Tianyi Zhang,Jason Wu*

Main category: cs.CL

TL;DR: 提出了一种新的机器学习方法，通过训练计算机使用代理uxCUA来自动评估GUI可用性，该方法在准确性和真实性上优于大型模型。


<details>
  <summary>Details</summary>
Motivation: 传统可用性测试成本高、耗时长，现有代理在可用性评估上仍不准确。

Method: 定义了可计算可用性概念，通过优先交互流程、模拟人类行为并预测数值评分来训练代理uxCUA，使用大规模交互UI数据集。

Result: uxCUA在可用性评估上超越大型模型，并能生成对合成和真实UI的现实评价。

Conclusion: 为HCI中的自动化可用性评估建立了基于数据驱动的原则性基础。
...
### [115] [BioGraphletQA: Knowledge-Anchored Generation of Complex QA Datasets](https://arxiv.org/abs/2604.26048)
*Richard A. A. Jonker,Bárbara Maria Ribeiro de Abreu Martins,Sérgio Matos*

Main category: cs.CL

TL;DR: 提出一种基于图小锚定的可扩展框架，用于生成复杂医学QA数据，并构建了包含119,856对QA的BioGraphletQA数据集。


<details>
  <summary>Details</summary>
Motivation: 现有QA数据生成方法缺乏对复杂性和事实依据的系统控制，尤其是生物医学领域知识密集型任务的需求未满足。

Method: 利用知识图谱中的小子图（图小结构）作为结构化提示，控制问题复杂性并确保事实依据，由大语言模型生成QA对。

Result: 专家评估106对QA验证了科学有效性和复杂性；数据增强提升PubMedQA准确率从49.2%至68.5%（低资源场景），MedQA从41.4%至44.8%（全资源场景）。

Conclusion: 该框架提供了一种通用、可复现的解决方案，可促进复杂问答任务（如MCQA和KGQA）的资源创建与性能提升。
...
### [116] [Anchored Confabulation: Partial Evidence Non-Monotonically Amplifies Confident Hallucination in LLMs](https://arxiv.org/abs/2604.25931)
*Ashish Balkishan Lathkar*

Main category: cs.CL

TL;DR: 我们发现大语言模型存在一种新的校准属性：提供多步推理链中的一个经确认的事实会导致模型在完整证据消除错误之前提高其“自信-错误”回答率，我们称之为锚定幻觉。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在揭示大语言模型中未被发现的校准属性，即部分事实锚定如何加剧推理错误，并探索其应用潜力。

Method: 通过形式化定义为参数幻觉置信度（PHC），在六条证据线中验证，包括因果注入实验（PHC 0.613→0.656→0.595→0.536，N=160）、五模型家族能力缩放（Spearman rho=0.900, p=0.037），以及锚定阈值定律 k*(n)=floor(n/3) 的四个预测。

Result: 锚定阈值定律预测PHC随跳跃深度放大并得到证实；应用于RAG路由的LearnedRouter在1800个查询上接近81.1%的oracle性能差距（macro F1=0.426, p<1e-6），无需微调且标签少50倍；认知谦逊提示降低PHC -0.118，而显式自我评分（PHC=0.684, p<0.001）优于词汇置信度作为路由信号。

Conclusion: 锚定幻觉是大语言模型中普遍且可预测的错误模式，可通过路由优化和提示设计缓解，为校准和鲁棒性提升提供新方向。
...
### [117] [From Prompt Risk to Response Risk: Paired Analysis of Safety Behavior of Large Language Model](https://arxiv.org/abs/2604.26052)
*Mengya Hu,Qiong Wei,Sandeep Atluri*

Main category: cs.CL

TL;DR: 对1250条人工标注的提示-响应记录进行成对转换分析，发现61%的响应相对于提示降低了危害等级，36%保持不变，3%升级；性内容类别降级难度是仇恨和暴力的3倍；所有合规升级都伴随高相关性，中等危害响应的相关性最低（64%）。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全评估通常报告攻击成功率、拒绝率等二值结果，隐藏了用户输入与模型响应之间风险变化的动态过程。

Method: 基于Azure AI Content Safety分类法，对四个危害类别（仇恨、性、暴力、自伤）和顺序严重级别进行人工标签，使用成对转换分析评估危害级别变化，并联合测量响应相关性。

Result: 61%的响应降低危害；36%保持不变；3%升级。性内容降级难度是其他类别的3倍，主要由对已存在性内容提示的持续（而非从良性输入新引入）导致。所有合规升级响应相关性最高（3级），而中等危害响应相关性最低（64%），主要由暴力和性类别中的离题阐述导致。

Conclusion: 成对转换分析能揭示LLM安全评估中的风险动态，性内容的降级难题和中等危害响应的低相关性提示了当前模型在平衡帮助性与无害性方面的挑战。
...
### [118] [HIVE: Hidden-Evidence Verification for Hallucination Detection in Diffusion Large Language Models](https://arxiv.org/abs/2604.26139)
*Guoshenghui Zhao,Weijie Zhao,Tan Yu*

Main category: cs.CL

TL;DR: 提出了HIV-E框架，通过从扩散大语言模型的去噪轨迹中提取隐藏证据进行幻觉检测，在三个QA基准上取得最高0.9236 AUROC和0.9537 AUPRC。


<details>
  <summary>Details</summary>
Motivation: 现有检测器主要依赖输出不确定性或粗略轨迹统计，无法捕获扩散大语言模型中可能出现的丰富隐藏动态。

Method: 从去噪轨迹提取压缩隐藏证据，选择信息丰富的步层证据，通过前缀嵌入对验证器语言模型进行条件化。

Result: 在两个扩散大模型和三个QA基准上，HIV-E一致优于八个强基线，达到0.9236 AUROC和0.9537 AUPRC。

Conclusion: 去噪轨迹中选择的隐藏证据比仅靠输出不确定性或粗略轨迹统计能提供更强且可用的幻觉信号。
...
### [119] [Structural Generalization on SLOG without Hand-Written Rules](https://arxiv.org/abs/2604.26157)
*Zichao Wei*

Main category: cs.CL

TL;DR: 提出基于神经细胞自动机(NCA)的语义解析方法，无需手工规则，在SLOG基准上实现17个泛化类别中11个100%准确率，失败案例归因于两种特定操作缺失。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖手工规则（AM-Parser）无法扩展，要么结构泛化失败（Transformer），需要一种自动学习组合规则的方法。

Method: 采用具有离散瓶颈的神经细胞自动机（NCA），通过局部迭代从数据中学习所有组合规则。

Result: 在SLOG基准的17个结构泛化类别中，11个达到100%类型精确匹配；失败案例仅源于两种机制：wh-提取与缩减动词类型的组合、动词主语侧的修饰语；所有失败对应训练中缺失的有向操作。

Conclusion: NCA无需手工规则即可实现强结构泛化，失败模式可完全解释为训练数据中操作覆盖的缺失。
...
### [120] [Test-Time Safety Alignment](https://arxiv.org/abs/2604.26167)
*Baturay Saglam,Dionysis Kalogerias*

Main category: cs.CL

TL;DR: 本研究展示了通过在词汇级别优化输入词嵌入，可以有效控制对齐模型的行为，使其在安全基准测试中产生无害响应。


<details>
  <summary>Details</summary>
Motivation: 先前工作仅针对预训练文本补全模型在减少简短续写中表面粗俗内容的效果，但未涉及对齐模型（生成二元拒答/遵守输出分布）的控制。研究旨在探索输入嵌入在安全控制中的潜力。

Method: 使用零阶梯度估计黑盒文本审查API对输入嵌入的梯度，并在嵌入空间执行梯度下降以最小化生成文本的有害性。

Result: 该方法能中和所有标准安全基准测试中标记为有风险的响应。

Conclusion: 输入词嵌入可有效控制对齐模型的安全性，为模型行为调整提供了一种无需微调或提示工程的轻量级方法。
...
### [121] [Option-Order Randomisation Reveals a Distributional Position Attractor in Prompted Sandbagging](https://arxiv.org/abs/2604.26206)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: 研究发现，Llama-3-8B在沙袋行为中表现出分布吸引子现象，响应位置集中在E/F/G，且对内容变化高度稳定。


<details>
  <summary>Details</summary>
Motivation: 前期实验发现沙袋行为与位置相关，但固定选项顺序未区分模型策略与数据干扰，需通过随机化验证。

Method: 对3个模型、2000个MMLU-Pro项目进行4种条件实验，共24000次试验，采用循环选项顺序随机化作为关键控制，并分析响应位置分布。

Result: 位置跟踪诊断未确认确定性策略（同字母率37.3%），但沙袋下响应位置分布高度稳定（r=0.9994），准确率在偏好位置E达72.1%，A位置降至4.3%。

Conclusion: 沙袋指令引发响应位置低熵吸引子（集中于E/F/G），作为黑箱行为特征可识别7-9亿参数模型的沙袋模式。
...
### [122] [EvoSelect: Data-Efficient LLM Evolution for Targeted Task Adaptation](https://arxiv.org/abs/2604.26170)
*Ting-Wei Li,Sirui Chen,Jiaru Zou,Yingbing Huang,Tianxin Wei,Jingrui He,Hanghang Tong*

Main category: cs.CL

TL;DR: 提出了EvoSelect框架，通过迭代生成-选择-训练循环，结合任务对齐和多样性选择合成数据，高效演化大语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有迭代生成-训练方法中合成数据存在噪声、冗余和与目标任务分布不匹配的问题，导致训练效果下降。

Method: 采用最优传输和代理梯度表征评估候选数据与目标任务的对齐程度，并引入多样化机制选择互补样本，交替进行对齐和多样化选择。

Result: 在多个基准测试中，无论使用弱还是强数据生成器，EvoSelect均优于现有数据选择方法。

Conclusion: 通过选择性地利用对齐且多样化的合成数据，可以高效且有效地演化大语言模型到目标任务。
...
### [123] [Breaking the Autoregressive Chain: Hyper-Parallel Decoding for Efficient LLM-Based Attribute Value Extraction](https://arxiv.org/abs/2604.26209)
*Theodore Glavas,Nikhita Vedula,Dushyanta Dhyani,Yilun Zhu,Shervin Malmasi*

Main category: cs.CL

TL;DR: 提出超并行解码（HPD）算法，通过位置ID操作实现不同顺序的令牌生成，在属性值提取任务中实现高达13.8倍的推理加速，且不损害输出质量。


<details>
  <summary>Details</summary>
Motivation: 属性值提取（AVE）等任务需要从同一文档上下文中解码多个独立序列，标准自回归解码速度慢，而输出序列间的独立性提供了并行化的机会。

Method: 利用共享内存和计算，通过位置ID操纵实现乱序令牌生成，在单提示内并行化属性值对生成，并堆叠多个文档实现批量并行解码。

Result: 实验中每提示最多并行解码96个令牌，推理成本和时间降低达13.8倍，且保持输出质量。

Conclusion: HPD兼容所有LLM，虽针对属性提取设计，但可推广至其他具有独立输出结构的场景。
...
### [124] [Comparative Analysis of AutoML and BiLSTM Models for Cyberbullying Detection on Indonesian Instagram Comments](https://arxiv.org/abs/2604.26229)
...
TL;DR: 比较机器学习和深度学习方法在印尼语Instagram评论中的网络欺凌检测表现，发现带注意力的BiLSTM最佳，但逻辑回归在资源受限场景下仍有竞争力。


<details>
  <summary>Details</summary>
Motivation: 网络欺凌在社交媒体中日益严重，尤其是非英语（如印尼语）的非正式文本缺乏有效的检测方案，需要探索适合的语言处理和建模方法。

Method: 使用650条平衡标注的印尼语Instagram评论（欺凌/非欺凌），采用Naive Bayes、Logistic Regression、SVM（TF-IDF特征）和BiLSTM、BiLSTM+Bahdanau Attention，并设计包括俚语规范化、停用词移除和词干提取的预处理流程。

Result: 逻辑回归在机器学习模型中表现最佳；带注意力的BiLSTM在深度学习中性能最强。

Conclusion: 领域特定的预处理对非正式印尼语文本重要；深度学习更能捕捉上下文模式，但机器学习在资源受限环境下仍是可行选择。
...
### [125] [StratMem-Bench: Evaluating Strategic Memory Use in Virtual Character Conversation Beyond Factual Recall](https://arxiv.org/abs/2604.26243)
*Yerong Wu,Tianxing Wu,Minghao Zhu,Hangyu Sha,Haofen Wang*

Main category: cs.CL

TL;DR: 提出了一个新基准StratMem-Bench，用于评估虚拟角色在对话中策略性使用记忆的能力，发现现有模型在处理支持性记忆时存在困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准将记忆视为静态事实仓库，忽视了对话中策略性使用记忆的细微差别。

Method: 设计了包含657个实例的数据集，其中包含必要、支持和无关记忆；并提出了严格记忆合规性、记忆整合质量、主动丰富分数和条件无关率等评估指标。

Result: 所有模型能区分必要和无关记忆，但在引入支持性记忆时表现不佳。

Conclusion: 模型在策略性使用支持性记忆方面仍有不足，需要进一步改进。
...
### [126] [A New Semisupervised Technique for Polarity Analysis using Masked Language Models](https://arxiv.org/abs/2604.26230)
*Kohei Watanabe*

Main category: cs.CL

TL;DR: 提出了基于word2vec掩码语言模型的潜在语义缩放新版本，通过预测种子词在上下文中的概率来分配极性分数，相比传统空间模型更准确、可解释且一致。


<details>
  <summary>Details</summary>
Motivation: 传统空间极性模型在文本分析中准确性和一致性不足，需要更优的极性评分方法。

Method: 使用word2vec作为掩码语言模型，计算词和文档的极性分数作为种子词在给定上下文中出现的预测概率。

Result: 在分析中国日报关于新冠疫情期间中国及其他国家卫生成就的报道中，概率模型表现出更优的准确性、可解释性和一致性。

Conclusion: 更先进的掩码语言模型有望进一步改进半监督机器学习技术。
...
### [127] [Calibrated Surprise: An Information-Theoretic Account of Creative Quality](https://arxiv.org/abs/2604.26269)
*Bo Zou,Chao Xu*

Main category: cs.CL

TL;DR: 本文提出用校准惊喜作为创意写作的核心，借助香农互信息量化约束与创意的关系，并基于静态与动态分析构建理论框架。


<details>
  <summary>Details</summary>
Motivation: 旨在解释优秀创意写作的本质，即约束与惊喜的平衡，并引入信息论工具进行精确量化。

Method: 使用香农互信息分析，将校准对应条件熵趋零，惊喜对应信息熵升高；通过静态（多维度约束）与动态（链式法则）两个支柱推导。

Result: 证明全维度准确性与平庸性互斥，并通过案例和LLM对数概率验证框架的有效性。

Conclusion: 该框架为创意质量对齐（CQA）和评估基准提供了理论基础，兼具分析价值与操作可行性。
...
### [128] [FlowBot: Inducing LLM Workflows with Bilevel Optimization and Textual Gradients](https://arxiv.org/abs/2604.26258)
*Hongyeon Yu,Young-Bum Kim,Yoon Kim*

Main category: cs.CL

TL;DR: 本文提出了一种名为FlowBot的数据驱动方法，通过双层优化自动生成并优化LLM工作流，外循环优化工作流结构，内循环逐层优化每个LLM调用，利用文本梯度实现模块化优化。


<details>
  <summary>Details</summary>
Motivation: 现有LLM工作流依赖人工构建流程和提示，严重限制了实际部署效率，需要一种自动化和数据驱动的方法来发现和优化工作流。

Method: 将工作流归纳建模为双层优化问题：外循环优化工作流高层草图（即LLM调用的结构），内循环通过文本梯度逐层反传优化每个LLM调用，实现模块化迭代改进。

Result: FlowBot自动发现的工作流在多个任务上，与基于人工设计或自动生成工作流的强基线方法相比，表现出竞争性的性能。

Conclusion: 文本梯度驱动的双层优化能有效自动诱导LLM工作流，减少人工干预并提升部署效率。
...
### [129] [Benchmarking PyCaret AutoML Against BiLSTM for Fine-Grained Emotion Classification: A Comparative Study on 20-Class Emotion Detection](https://arxiv.org/abs/2604.26310)
*Arya Muda Siregar,Arielva Simon Siahaan,Haikal Fransisko Simbolon,Luluk Muthoharoh,Ardika Satria,Martin C. T. Manullang*

Main category: cs.CL

TL;DR: 本研究基准测试了机器学习与深度学习方法在20类细粒度情感分类任务上的表现，BiLSTM以89%的准确率最优。


<details>
  <summary>Details</summary>
Motivation: 细粒度情感分类在NLP中仍有挑战，需要评估不同方法的有效性。

Method: 使用TF-IDF特征的逻辑回归、多项式朴素贝叶斯、SVM，以及BiLSTM、GRU、轻量级Transformer。

Result: BiLSTM最佳，准确率89%，F1分数0.89；SVM次之，准确率88.11%。

Conclusion: 传统模型计算高效，但序列深度学习模型更能捕捉文本情感上下文线索。
...
### [130] [Folding Tensor and Sequence Parallelism for Memory-Efficient Transformer Training & Inference](https://arxiv.org/abs/2604.26294)
*Vasu Shyam,Anna Golubeva,Quentin Anthony*

Main category: cs.CL

TL;DR: 提出张量序列并行（TSP），将张量并行和序列并行合并到单一设备轴，减少长上下文训练中的参数和激活内存开销。


<details>
  <summary>Details</summary>
Motivation: 传统多维并行布局中，张量并行和序列并行占用独立设备轴，导致内存和通信效率不平衡，尤其在内存受限和长上下文场景下。

Method: 设计TSP调度：注意力模块广播参数分片并通过序列wise键值交换重建上下文；门控MLP中权重分片在环中循环，局部输出累积。

Result: 理论分析和基准测试表明，TSP相比TP、SP及TP+SP，以额外通信量换取更低内存开销，适合长上下文和内存受限训练。

Conclusion: TSP可作为硬件感知的并行方案，与流水线并行、专家并行结合，用于密集和混合专家模型。
...
### [131] [Classification of Public Opinion on the Free Nutritional Meal Program on YouTube Media Using the LSTM Method](https://arxiv.org/abs/2604.26312)
...
TL;DR: 利用LSTM对YouTube评论进行情感分类，准确率89%，负面情感F1-score 0.94，正面情感因数据不平衡仅0.55。


<details>
  <summary>Details</summary>
Motivation: 分析公众对免费营养餐计划（MBG）的观点，以评估公共政策。

Method: 采用LSTM方法对7,733条YouTube评论进行情感分类。

Result: 模型准确率89%，负面情感F1-score 0.94，正面情感F1-score 0.55（因87.7%数据为负面）。

Conclusion: LSTM对印尼文本情感分析有效，但数据不平衡是挑战。
...
### [132] [DSIPA: Detecting LLM-Generated Texts via Sentiment-Invariant Patterns Divergence Analysis](https://arxiv.org/abs/2604.26328)
*Siyuan Li,Aodu Wulianghai,Guangyan Li,Xi Lin,Qinghua Mao,Yuliang Chen,Jun Wu,Jianhua Li*

Main category: cs.CL

TL;DR: 提出DSIPA框架，通过情感分布稳定性检测机器生成文本，零样本黑盒操作，在多个领域和模型上F1提升高达49.89%。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法对对抗扰动等鲁棒性不足，需模型参数或标注数据，难应对机器生成文本滥用。

Method: 基于LLM输出情感一致性更低、人类文本情感变异更大的观察，采用情感分布一致性和情感分布保留两个无监督指标，无需训练。

Result: 在GPT-5.2等模型及新闻、代码等五领域，F1比基线提升最高49.89%，泛化性和抗攻击能力强。

Conclusion: DSIPA提供鲁棒可解释的行为信号，适用于安全内容识别，无需模型内部信息。
...
### [133] [A Dual-Task Paradigm to Investigate Sentence Comprehension Strategies in Language Models](https://arxiv.org/abs/2604.26351)
*Rei Emura,Saku Sugawara*

Main category: cs.CL

TL;DR: 提出双任务范式约束语言模型认知资源，发现GPT-4o等模型在双任务条件下更倾向于基于合理性的句子理解，模拟人类理性推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法未直接针对记忆存储与句子处理之间的平衡，而这是人类工作记忆的核心。

Method: 设计算术计算与句子理解相结合的双任务范式，对比单任务与双任务条件下模型的表现。

Result: GPT-4o、o3-mini和o4-mini在双任务条件下对合理与不合理句子的理解准确率差距增大。

Conclusion: 限制记忆与处理资源的平衡能促进语言模型的理性推理，支持人类句子理解源于有限认知资源分配的论点。
...
### [134] [A Systematic Comparison of Prompting and Multi-Agent Methods for LLM-based Stance Detection](https://arxiv.org/abs/2604.26319)
*Genan Dai,Zini Chen,Yi Yang,Bowen Zhang*

Main category: cs.CL

TL;DR: 本文系统比较了基于提示和基于代理的立场检测方法，发现提示方法优于代理方法，模型规模比方法选择影响更大，且推理增强模型表现不稳定。


<details>
  <summary>Details</summary>
Motivation: 现有立场检测研究在数据划分、基础模型和评估协议上不一致，难以公平比较不同LLM方法的性能。

Method: 在4个数据集的14个子任务上，使用15个LLM（7B到72B+）评估5种方法：基于提示的Direct Prompting、Auto-CoT、StSQA和基于代理的COLA、MPRF。

Result: 基于提示的方法整体优于基于代理的方法；模型规模（约32B饱和）比方法选择影响更大；推理增强模型（如DeepSeek-R1）未一致优于同规模通用模型。

Conclusion: 提示策略在立场检测中更有效，且增大模型规模比优化方法更关键，推理增强模型的优势在此任务中不显著。
...
### [135] [Text Style Transfer with Machine Translation for Graphic Designs](https://arxiv.org/abs/2604.26361)
*Deergh Singh Budhauria,Sanyam Jain,Rishav Agarwal,Tracy King*

Main category: cs.CL

TL;DR: 本文提出了三种新方法（基于NMT、LLM和混合NMT+LLM）来解决图形设计中源文本与翻译文本间的词对齐问题，以保留文字样式。实验发现，注意力头基线方法优于纯NMT/LLM方法，与混合方法性能相当。


<details>
  <summary>Details</summary>
Motivation: 全球化背景下，需要准确翻译图形设计中的文本并保留其样式，而高精度词对齐是实现样式保留的关键。

Method: 基于商业NMT和LLM技术，提出三种方法：1) NMT加自定义输入输出标签；2) LLM加自定义标签；3) NMT翻译后接LLM与unigram映射的混合方法。

Result: 注意力头基线方法比纯LLM或NMT方法更准确，与混合NMT+LLM方法性能持平。

Conclusion: 现有注意力头方法在词对齐任务上仍具竞争力，混合方法有一定潜力，但纯LLM/NMT方法未超越基线。
...
### [136] [Shorthand for Thought: Compressing LLM Reasoning via Entropy-Guided Supertokens](https://arxiv.org/abs/2604.26355)
*Zhenyu Zhao,Sander Land,Dan Bikel,Waseem Alshikh*

Main category: cs.CL

TL;DR: 提出一种模型无关的推理痕迹压缩方法，通过超令牌捕捉结构模式，平均缩短8.1%的推理长度且不损失精度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理计算量大，但推理痕迹的令牌级信息结构未被充分探索。

Method: 对推理痕迹应用跨词BPE合并得到超令牌，并通过监督微调让模型采用。

Result: 在三个模型族和五个数学推理基准上平均缩短8.1%推理长度，无显著精度损失；超令牌可用于解释推理策略。

Conclusion: 超令牌不仅能压缩推理痕迹，还揭示正确与错误推理的系统性差异，具有奖励塑造和早停的应用潜力。
...
### [137] [SG-UniBuc-NLP at SemEval-2026 Task 6: Multi-Head RoBERTa with Chunking for Long-Context Evasion Detection](https://arxiv.org/abs/2604.26375)
*Gabriel Stefan,Sergiu Nisioi*

Main category: cs.CL

TL;DR: 该系统通过重叠滑动窗口分块、多任务学习和集成推理，在SemEval-2026任务6中分别以Macro-F1 0.80和0.51完成粗粒度清晰度和细粒度回避策略分类。


<details>
  <summary>Details</summary>
Motivation: 政治访谈回答常超过Transformer编码器的512词限制，需要有效处理长序列并同时完成两种粒度的分类任务。

Method: 采用重叠滑动窗口分块策略，利用元素级最大池化聚合片段；基于RoBERTa-large共享编码器，设计两个任务特定头并通过多任务目标联合训练；推理时集成7折分层交叉验证模型。

Result: 粗粒度子任务1（3类）Macro-F1为0.80，细粒度子任务2（9类）Macro-F1为0.51，两项均为第11名。

Conclusion: 多任务学习和滑动窗口分块能有效处理长文本政治问答，但细粒度回避策略分类仍有挑战。
...
### [138] [When Hidden States Drift: Can KV Caches Rescue Long-Range Speculative Decoding?](https://arxiv.org/abs/2604.26412)
*Tianyu Liu,Yuhao Shen,Xinyi Hu,Baolin Zhang,Hengxin Zhang,Jun Dai,Jun Zhang,Shuang Ge,Lei Chen,Yue Li,MingCheng Wan*

Main category: cs.CL

TL;DR: 提出了KVShot诊断框架，揭示了KV重用在推测解码中缓解长程衰退的潜力与结构瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有隐藏状态推理模型在推测解码中长程精度下降，即使测试时训练也无法解决，作者从上下文信息保持角度重新审视该问题。

Method: 提出KVShot诊断框架，比较隐藏状态复用、KV缓存复用和混合复用三种范式，并在Qwen3-8B上评估。

Result: KV重用在长步长接受率上有提升，但端到端加速效果在现有训练范式下仍然有限，主要受限于浅层推演者预测查询精度和KV投影梯度稀疏。

Conclusion: 说明实现KV感知解码需超越测试时训练，转向块级训练架构，KVShot为设计下一代推理架构提供了诊断平台与路线图。
...
### [139] [Benchmarking Complex Multimodal Document Processing Pipelines: A Unified Evaluation Framework for Enterprise AI](https://arxiv.org/abs/2604.26382)
*Saurabh K. Singh,Sachin Raj*

Main category: cs.CL

TL;DR: 本文提出了EnterpriseDocBench基准，用于端到端评估企业文档AI流水线（解析、索引、检索、生成），发现混合检索略优于BM25，幻觉率与文档长度呈U型相关，且各阶段性能相关性很弱。


<details>
  <summary>Details</summary>
Motivation: 现有企业文档AI的每个阶段虽已被充分研究，但缺乏对整个系统进行端到端评估的方法，因此构建了EnterpriseDocBench。

Method: 构建包含六个企业领域文档的语料库，运行BM25、密集嵌入和混合三种流水线（均使用GPT-5生成器），通过解析保真度、索引效率、检索相关性和生成可靠性等指标评估。

Result: 混合检索nDCG@5为0.92，略胜BM25的0.91；短文档（28.1%）和长文档（23.8%）的幻觉率高于中等长度文档（9.2%）；跨阶段相关性很弱（r≤0.17）；事实准确率为85.5%，但答案完整性仅为0.40。

Conclusion: 企业文档AI系统的质量并非简单级联传递，幻觉与文档长度非线性相关，且答案完整性问题比准确性更值得关注；设计与指标存在局限，供未来研究参考。
...
### [140] [EmoTransCap: Dataset and Pipeline for Emotion Transition-Aware Speech Captioning in Discourses](https://arxiv.org/abs/2604.26417)
*Shuhao Xu,Yifan Hu,Jingjing Wu,Zhihao Du,Zheng Lian,Rui Liu*

Main category: cs.CL

TL;DR: 提出了EmoTransCap，一种面向语篇级情感过渡的语音字幕生成范式，包含首个大规模情感过渡数据集及多任务识别模型。


<details>
  <summary>Details</summary>
Motivation: 现有语音情感字幕（SEC）局限于孤立句子中的静态单情感描述，忽略了语篇层面的动态情感过渡。

Method: 设计自动化管道构建情感过渡数据集；提出MTETR模型联合检测情感过渡与说话人分离；利用大语言模型生成描述性和指令性两种注释版本；引入可控的语篇级情感过渡语音合成系统。

Result: 实现了捕捉情感过渡的语音字幕，支持时序动态和细粒度情感理解；情感合成系统增强了拟人化情感表达。

Conclusion: 该范式填补了语篇级情感过渡建模的空白，为情感智能对话代理提供了重要基础资源与技术方法。
...
### [141] [Naamah: A Large Scale Synthetic Sanskrit NER Corpus via DBpedia Seeding and LLM Generation](https://arxiv.org/abs/2604.26456)
*Akhil Rajeev P,Annarao Kulkarni*

Main category: cs.CL

TL;DR: 提出Naamah数据集（102,942句梵语NER银标准数据）及结合DBpedia和24B参数混合推理模型的合成数据方法，并评测XLM RoBERTa和IndicBERTv2两种架构。


<details>
  <summary>Details</summary>
Motivation: 梵语数字化缺乏标注资源，现有方法依赖通用LLM但错误率高且缺乏古典语法推理深度。

Method: 从DBpedia提取实体，结合24B参数混合推理模型生成语法自然、多样化的合成训练数据。

Result: 构建高质量银标准数据集，验证两种Transformer架构在梵语NER上的性能。

Conclusion: 证明了结合知识库与强推理模型的合成数据方法对低资源古典语言NER的有效性。
...
### [142] [Tree-of-Text: A Tree-based Prompting Framework for Table-to-Text Generation in the Sports Domain](https://arxiv.org/abs/2604.26501)
*Shang-Hsuan Chiang,Tsan-Tsung Yang,An-Zi Yen,Wen-Chih Peng*

Main category: cs.CL

TL;DR: 提出Tree-of-Text框架，通过树结构提示实现三阶段表格到文本生成，在体育赛事报告任务上取得高效结果。


<details>
  <summary>Details</summary>
Motivation: 传统模型需大量标注数据，基于提示的方法易出现幻觉，需要一种更鲁棒高效的表格理解与文本生成方法。

Method: Tree-of-Text：内容规划（选择操作和参数）、操作执行（分割大表为子表）、内容生成（合并重写为连贯报告）。

Result: 在ShuttleSet+上全面领先，在RotoWire-FG上RG和CO最优，在MLB上CS和CO最优，且时间和成本仅为Chain-of-Table的40%。

Conclusion: Tree-of-Text在体育领域表格到文本生成中有效且高效，为基于提示的方法提供了新方向。
...
### [143] [Theory-Grounded Evaluation Exposes the Authorship Gap in LLM Personalization](https://arxiv.org/abs/2604.26460)
*Yash Ganpat Sawant*

Main category: cs.CL

TL;DR: 该论文通过作者身份验证理论，提出了一种新的风格个性化评估方法，揭示了现有指标在衡量大语言模型生成文本时的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有风格个性化评估缺乏作者身份科学基础，导致指标选择影响结论。

Method: 基于LUAR（训练的作者身份验证模型）、LLM-as-judge解耦特质匹配和经典功能词文体学三种测量传统，评估四种推理时个性化方法。

Result: LUAR提供了校准基线（人类上限0.756，跨作者下限0.626），所有方法得分低于此下限（0.484-0.508），且三种指标间几乎无相关性（|r|<0.07）。

Conclusion: 理论基础的缺失导致指标决定结论；作者身份理论揭示了非校准指标忽略的作者身份差距。
...
### [144] [StarDrinks: An English and Korean Test Set for SLU Evaluation in a Drink Ordering Scenario](https://arxiv.org/abs/2604.26500)
...
TL;DR: 提出了一个包含英语和韩语的多语言数据集StarDrinks，用于评估在真实场景下的任务导向型语音交互系统的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方法多基于受控场景，无法捕捉真实用户请求的变异性和复杂性，特别是在饮品订购等任务中涉及多样化的命名实体、品牌术语以及自发性语音现象。

Method: 收集了包含语音特征、转录文本和标注槽位的英语和韩语数据集，支持语音到槽位、转录到槽位以及语音到转录三种评估模式。

Result: 提供了一个更真实的基准测试，用于评估模型在处理复杂、多样化真实世界任务时的表现。

Conclusion: 该数据集填补了现有评估的空白，有助于推动更鲁棒和通用的任务导向型对话系统的发展。
...
### [145] [Text-Utilization for Encoder-dominated Speech Recognition Models](https://arxiv.org/abs/2604.26514)
*Albert Zeyer,Tim Posielek,Ralf Schlüter,Hermann Ney*

Main category: cs.CL

TL;DR: 本文研究了如何利用纯文本数据提升语音识别效率，重点分析了编码器主导模型，并通过模态匹配和动态下采样等技术，在LibriSpeech语料库上验证了更大编码器与更小解码器的结构能媲美甚至超越复杂架构，同时发现随机时长模型等简单配置更有效。


<details>
  <summary>Details</summary>
Motivation: 探索高效利用纯文本数据提升语音识别性能，尤其是编码器主导模型以提高识别速度。

Method: 比较了模态匹配、动态下采样等技术，将文本数据集成到编码器主导的语音识别模型中，并采用随机时长模型简化训练流程。

Result: 在LibriSpeech上，更大编码器与更小解码器性能不逊于更大解码器架构，且简单随机时长模型效果更优。

Conclusion: 纯文本数据可有效提升语音识别，编码器主导模型配合简单配置能实现高效训练与性能。
...
### [146] [TLPO: Token-Level Policy Optimization for Mitigating Language Confusion in Large Language Models](https://arxiv.org/abs/2604.26553)
*Jinho Choo,JunSeung Lee,Jimyeong Kim,Yeeho Song,S. K. Hong,Yeong-Dae Kwon*

Main category: cs.CL

TL;DR: 提出Token-Level Policy Optimization (TLPO)，通过token级细粒度微调缓解大语言模型的语言混淆问题，在不损害通用能力的前提下提升语言一致性。
...
Method: TLPO框架：识别易出错位置，探索替代候选token，并通过定制化目标函数进行细粒度策略更新，抑制错误输出。

Result: 在多语言LLM上的实验表明，TLPO在提升语言一致性方面显著优于基线方法，同时保持下游任务准确性。

Conclusion: TLPO通过局部、token级干预有效缓解语言混淆，避免通用能力退化。
...
### [147] [SafeReview: Defending LLM-based Review Systems Against Adversarial Hidden Prompts](https://arxiv.org/abs/2604.26506)
*Yuan Xin,Yixuan Weng,Minjun Zhu,Ying Ling,Chengwei Qin,Michael Hahn,Michael Backes,Yue Zhang,Linyi Yang*

Main category: cs.CL

TL;DR: 提出一种生成器与防御者共同优化的对抗框架，以抵御针对LLM学术评审的对抗性提示攻击，并提升系统对新威胁的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: LLM在学术评审中的广泛应用使其易受嵌入稿件中的对抗性提示操纵，威胁学术诚信，因此需要开发动态防御机制。

Method: 构建生成器-防御者双模型系统，基于信息检索生成对抗网络的损失函数进行联合训练，推动两者协同进化。

Result: 该框架相比静态防御显著增强了对新型和演进威胁的抵御能力。

Conclusion: 所提框架为保障学术评审完整性奠定了关键基础。
...
### [148] [Multimodal LLMs are not all you need for Pediatric Speech Language Pathology](https://arxiv.org/abs/2604.26568)
*Darren Fürst,Sebastian Steindl,Ulrich Schäfer*

Main category: cs.CL

TL;DR: 本研究提出了一种层级式语音障碍分类方法，通过微调语音表征模型和针对性数据增强，在SLPHelmUltraSuitePlus基准测试中显著提升了所有临床任务的性能，并改善了自动语音识别效果。


<details>
  <summary>Details</summary>
Motivation: 言语障碍儿童比例约5%，但语言病理学家面临严重人员短缺和超负荷工作量，亟需自动化辅助诊断工具。

Method: 采用从二分类到类型和症状分类的级联方法；微调语音表征模型并应用针对性数据增强来减少偏差。

Result: 语音表征模型在所有评估任务中大幅超越基于大语言模型的现有最优方法；数据增强同时提升了自动语音识别性能。

Conclusion: 层级式分类结合语音表征模型与数据增强是解决言语障碍自动分类的有效途径，已发布模型和代码以促进后续研究。
...
### [149] [Zero-Shot to Full-Resource: Cross-lingual Transfer Strategies for Aspect-Based Sentiment Analysis](https://arxiv.org/abs/2604.26619)
*Jakob Fehle,Nils Constantin Hellwig,Udo Kruschwitz,Christian Wolff*

Main category: cs.CL

TL;DR: 本文对七种语言和四个子任务上的情感分析模型进行了多语言评估，比较了不同的转换器架构和训练策略。


<details>
  <summary>Details</summary>
Motivation: 现有的情感分析研究主要集中在英语上，忽视了其他语言的评估和数据集。

Method: 通过零资源、纯数据和全资源设置，结合跨语言迁移、代码切换和机器翻译，系统比较了不同的转换器架构和大型语言模型。

Result: 微调的大语言模型在复杂生成任务中表现最佳，而少量样本模型在简单设置中接近该性能；跨语言训练对微调大语言模型最强，而更小编码器或序列到序列模型从代码切换中受益最大。

Conclusion: 不同架构需要针对性地选择跨语言策略，以优化多语言情感分析，并提供了两个新的德语数据集以促进该领域研究。
...
### [150] [Translating Under Pressure: Domain-Aware LLMs for Crisis Communication](https://arxiv.org/abs/2604.26597)
*Antonio Castaldo,Maria Carmen Staiano,Johanna Monti,Sheila Castilho,Francesca Chiusaroli*

Main category: cs.CL

TL;DR: 提出一种面向危机场景的领域自适应翻译流水线，通过数据检索与过滤扩展小规模参考语料，结合偏好优化生成CEFR A2级简化英语，在保持翻译充分性的同时提升可读性。


<details>
  <summary>Details</summary>
Motivation: 灾害应急场景下多语言通信需求迫切，但缺乏高质量平行语料制约了翻译系统开发，现有方案难以同时兼顾可读性与充分性。

Method: 1) 从通用语料中检索并过滤领域相关数据扩展参考语料；2) 微调小型神经机器翻译模型；3) 应用偏好优化约束输出为CEFR A2级简化英语。

Result: 自动评估和人工评估表明，该方法显著提升了译文的可读性，同时维持了较高翻译充分性。

Conclusion: 简化英语结合领域适应可在无法覆盖全语种时作为应急通信的实用通用语言，验证了简化语言策略在危机场景的有效性。
...
### [151] [OCR-Memory: Optical Context Retrieval for Long-Horizon Agent Memory](https://arxiv.org/abs/2604.26622)
*Jinze Li,Yang Zhang,Xin Yang,Jiayi Qu,Jinfeng Xu,Shuo Yang,Junhua Ding,Edith Cheuk-Han Ngai*

Main category: cs.CL

TL;DR: 本文提出OCR-Memory，一种利用视觉模态作为高密度表示的代理记忆框架，通过将历史轨迹渲染为带视觉标识的图像，并采用定点转录范式检索，在长时程交互任务中显著提升了有限上下文下的有效记忆容量与证据保真度。


<details>
  <summary>Details</summary>
Motivation: 现有代理记忆系统受限于文本上下文预算，原始轨迹存储代价高昂，而摘要与纯文本检索会导致信息丢失与证据碎片化。

Method: OCR-Memory将历史轨迹渲染为带独特视觉标识的图像，并通过视觉锚点定位相关区域、转录对应文本的范式进行检索，避免了自由生成与幻觉。

Result: 在长时程代理基准测试中，OCR-Memory在严格上下文限制下持续取得性能提升，证明光编码增强了有效记忆容量并保留了忠实证据恢复能力。

Conclusion: 光编码记忆框架能有效突破文本上下文的约束，为长时程交互代理提供更高效、更可靠的记忆机制。
...
### [152] [From Black-Box Confidence to Measurable Trust in Clinical AI: A Framework for Evidence, Supervision, and Staged Autonomy](https://arxiv.org/abs/2604.26671)
*Serhii Zabolotnii,Viktoriia Holinko,Olha Antonenko*

Main category: cs.CL

TL;DR: 提出了一个基于证据、监督和分阶段自主性的临床人工智能可信框架，强调信任是系统架构的结果而非模型属性。


<details>
  <summary>Details</summary>
Motivation: 临床AI的信任不能仅依赖准确性或用户印象，需要可衡量的系统设计原则。

Method: 结合确定性核心、患者特定AI助手、多级模型升级机制和人工监督层，采用分类器驱动的模块化提示来扩展临床深度。

Result: 通过选择性验证关键发现、有界临床上下文和严格提示架构实现信任，并提出基于计量学的信任指标（测量不确定度、校准、可追溯性）。

Conclusion: 可信临床AI应作为可测量的系统架构结果来设计，而非依赖于单一模型。
...
### [153] [SAGE: A Strategy-Aware Graph-Enhanced Generation Framework For Online Counseling](https://arxiv.org/abs/2604.26630)
*Eliya Naomi Aharon,Meytal Grimland,Avi Segal,Loona Ben Dayan,Inbar Shenfeld,Yossi Levi Belz,Kobi Gal*

Main category: cs.CL

TL;DR: 提出了SAGE框架，通过构建异构图整合临床知识与生成式AI，提升心理健康咨询中策略预测和响应质量。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型缺乏结合心理理论框架、实时困境信号和干预规划的临床推理能力，影响安全性和治疗效果。

Method: 构建融合对话动态与心理理论的异构图；使用下一策略分类器识别最优干预；利用图感知注意力机制将结构信号转为软提示，引导LLM生成具有临床深度的响应。

Result: 在自动化指标和专家人工评估中，SAGE在策略预测和推荐响应质量上优于基线。

Conclusion: SAGE作为决策支持工具，能增强人类专家在高风险危机咨询中的能力，提供可操作的干预建议。
...
### [154] [Domain-Adapted Small Language Models for Reliable Clinical Triage](https://arxiv.org/abs/2604.26766)
*Manar Aljohani,Brandon Ho,Kenneth McKinley,Dennis Ren,Xuan Wang*

Main category: cs.CL

TL;DR: 本研究评估了开源小型语言模型作为急诊分诊决策支持工具的可靠性，发现Qwen2.5-7B通过针对性微调后在准确性和隐私保护方面表现最优。


<details>
  <summary>Details</summary>
Motivation: 急诊部门中基于自由文本的分诊记录常导致分诊错误和效率低下，需要更可靠的决策支持工具。

Method: 比较多种小型语言模型在不同提示策略下的表现，并使用专家和银标准儿科数据进行大规模领域自适应微调。

Result: Qwen2.5-7B在微调后显著减少了分诊不一致和临床显著错误，优于所有基线和大型专有模型。

Conclusion: 机构特定的小型语言模型可作为隐私保护的可靠急诊分诊决策工具，针对性微调比复杂推理策略更有效。
...
### [155] [Differentially-Private Text Rewriting reshapes Linguistic Style](https://arxiv.org/abs/2604.26656)
*Stefan Arnold*

Main category: cs.CL

TL;DR: 研究了差分隐私文本重写对语域特征的系统性影响，发现隐私保护会导致文本交际标记、语境参照和复杂从句的严重衰减，最终使文本趋于非交互、非说服性的同质化语域。


<details>
  <summary>Details</summary>
Motivation: 现有的文本隐私保护主要关注语法连贯性与形式隐私保证的平衡，但缺乏对文本语域身份（register identity）影响的研究。

Method: 对差分隐私文本重写进行多维度文体特征分析，对比自回归释义与双向替换两种架构在不同隐私预算下的效果。

Result: 隐私保护下的重写不仅带来词汇变化，还导致交互标记、语境参照和复杂从句的严重衰减，两种架构均迫使文本向非卷入、非说服性语域收敛。

Conclusion: 差分隐私文本重写虽然保留了语义内容，但会结构化地消除人类文本中细微的文体标记，导致文本语域的同质化。
...
### [156] [Swap distance minimization shapes the order of subject, object and verb in languages of the world](https://arxiv.org/abs/2604.26726)
*Jairo Rios-El-Yazidi,Ramon Ferrer-i-Cancho*

Main category: cs.CL

TL;DR: 本文发现，即使语言的主干语序并非SOV或SVO，甚至缺乏主干语序，单词顺序的变化仍受交换距离最小化原则的约束。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注SOV和SVO语序，但忽视了其他语序或无主干语序的语言，研究旨在解释这些语言中语序变化的普遍规律。

Method: 跨语言分析多语系和宏区域的数据，使用交换距离最小化原则检验语序变化模式。

Result: 语序变化在非SOV/SVO语言及无主干语序的语言中仍遵循交换距离最小化原则。

Conclusion: 交换距离最小化是跨语言语序变化的普遍原则，不受主干语序类型限制。
...
### [157] [Decoupling Knowledge and Task Subspaces for Composable Parametric Retrieval Augmented Generation](https://arxiv.org/abs/2604.26768)
*Weihang Su,Hanwen Zhang,Qingyao Ai,Yiqun Liu*

Main category: cs.CL

TL;DR: 提出正交子空间分解(OSD)方法分离任务行为与文档知识，提升多文档PRAG中适配器组合的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有PRAG方法中任务监督训练导致文档适配器混合了可复用的任务行为与文档特定知识，使得多适配器融合时积累重叠任务行为影响稳定性。

Method: 先训练任务LoRA捕获可复用任务行为，再在正交子空间中训练文档LoRA编码文档知识，实现任务与知识适配器的解耦。

Result: 在多项知识密集型任务和不同模型规模上，正交化策略均能改善多文档PRAG的组合鲁棒性。

Conclusion: 正交化任务与文档适配器更新可有效提升参数化RAG在合并多文档适配器时的稳定性。
...
### [158] [HalluCiteChecker: A Lightweight Toolkit for Hallucinated Citation Detection and Verification in the Era of AI Scientists](https://arxiv.org/abs/2604.26835)
*Yusuke Sakai,Hidetaka Kamigaito,Taro Watanabe*

Main category: cs.CL

TL;DR: 介绍了用于检测科学论文中幻觉引用的工具HalluCiteChecker。


<details>
  <summary>Details</summary>
Motivation: AI助手在学术写作中引发幻觉引用，损害论文可信度并增加审稿负担。

Method: 将幻觉引用检测形式化为NLP任务，并开发轻量级工具包。

Result: 工具可在标准笔记本电脑上秒级完成验证，离线运行且仅需CPU。

Conclusion: 有望减轻审稿人工作量并支持系统化预审和出版检查。
...
### [159] [What Kind of Language is Easy to Language-Model Under Curriculum Learning?](https://arxiv.org/abs/2604.26844)
*Nadine El-Naggar,Tatsuki Kuribayashi,Ted Briscoe*

Main category: cs.CL

TL;DR: 本文研究课程学习如何影响语言模型的归纳偏差，发现课程学习能显著改变模型对跨语言类型学倾向的预测，表明学习场景与模型偏好的交互作用。


<details>
  <summary>Details</summary>
Motivation: 探索语言类型学倾向的可预测性条件，尤其是学习场景（如课程学习）是否与语言模型的归纳偏差相互作用，从而影响对罕见/常见语言特征的偏好。

Method: 基于先前语言模型探索（El-Naggar等, 2025a,b），引入简单的课程学习变体（从简单句子开始），与随机输入顺序对比，分析模型对跨语言特征组合的预测偏好。

Result: 课程学习显著改变了语言模型的表现型归纳偏差，表明学习场景是影响类型学倾向预测的关键因素。

Conclusion: 学习场景（如课程学习）与语言模型的归纳偏差交互作用，能改变模型对跨语言类型学趋势的偏好，这为理解语言普遍性提供了新视角。
...
### [160] [ClawGym: A Scalable Framework for Building Effective Claw Agents](https://arxiv.org/abs/2604.26904)
...
TL;DR: ClawGym是一个支持Claw风格个人代理开发全生命周期的可扩展框架,包含合成数据、模型训练和基准评估。


<details>
  <summary>Details</summary>
Motivation: Claw风格环境支持多步骤工作流,但缺少系统化的框架来合成可验证的训练数据并集成到代理训练和诊断评估中。

Method: 构建了ClawGym-SynData(13.5K过滤任务)、ClawGym-Agents(通过监督微调和强化学习训练)和ClawGym-Bench(200实例基准)。

Result: 生成了多样化合成数据集,训练了能力强的Claw风格模型,并建立了可靠评估基准。

Conclusion: ClawGym为Claw风格个人代理的规模化开发提供了系统框架,资源即将开源。
...
### [161] [MoRFI: Monotonic Sparse Autoencoder Feature Identification](https://arxiv.org/abs/2604.26866)
*Dimitris Dimakopoulos,Shay B. Cohen,Ioannis Konstas*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型在微调新知识时产生幻觉的因果机制，通过控制实验发现残差流中的潜在方向会导致幻觉，并提出了MoRFI方法识别这些方向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在后训练阶段引入新知识时会产生幻觉，而监督微调会加剧这一问题，但其内在机制尚不清楚。

Method: 在Llama 3.1 8B等模型上对七个不同的问答数据集进行受控微调，利用稀疏自编码器分析残差流激活，并提出MoRFI方法识别与幻觉因果相关的潜在特征。

Result: 逐步引入新知识会增加幻觉，延长训练会放大该效应；MoRFI能可靠识别出干预后恢复知识的单一潜在方向。

Conclusion: 新知识会破坏模型沿残差流方向检索存储知识的能力，通过识别和干预这些潜在方向可缓解幻觉。
...
### [162] [Turning the TIDE: Cross-Architecture Distillation for Diffusion Large Language Models](https://arxiv.org/abs/2604.26951)
*Gongbo Zhang,Wen Wang,Ye Tian,Li Yuan*

Main category: cs.CL

TL;DR: 提出了首个跨架构扩散大语言模型蒸馏框架TIDE，通过三个模块实现教师模型向异构学生模型的高效知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型性能依赖大规模参数，且蒸馏方法局限于同架构知识迁移，无法处理教师与学生架构、注意力机制及分词器不同的情况。

Method: 包含三个模块：(1) TIDAL自适应蒸馏强度调节模块，(2) CompDemo互补掩码上下文增强模块，(3) Reverse CALM跨分词器目标函数模块。

Result: 将8B稠密和16B MoE教师蒸馏至0.6B学生模型，在8个基准上平均提升1.53分，HumanEval代码生成得分达48.78（基线为32.3）。

Conclusion: TIDE框架成功实现了跨架构扩散大语言模型的蒸馏，显著提升了小模型的性能，尤其在代码生成任务上表现突出。
...
### [163] [HealthNLP_Retrievers at ArchEHR-QA 2026: Cascaded LLM Pipeline for Grounded Clinical Question Answering](https://arxiv.org/abs/2604.26880)
*Md Biplob Hosen,Md Alomgeer Hussein,Md Akmol Masud,Omar Faruque,Tera L Reynolds,Lujie Karen Chen*

Main category: cs.CL

TL;DR: 本文提出了一个多阶段级联管道，利用Gemini 2.5 Pro大语言模型，用于患者门户中基于电子健康记录的问答系统，包括查询重构、证据评分、答案生成和对齐模块。


<details>
  <summary>Details</summary>
Motivation: 患者门户让个人直接访问电子健康记录，但仅靠访问不能确保患者理解或处理其中复杂的临床信息。

Method: 采用多阶段级联管道，包括四个集成模块：少样本查询重构单元、基于启发式的证据评分器、基于证据的响应生成器以及高精度多对多对齐框架。

Result: 在各项任务中排名：问题理解第1，答案生成第5，证据识别第7，答案-证据对齐第9。

Conclusion: 将大语言模型整合到结构化多阶段管道中，提高了问答的接地性、精确性和面向患者的专业健康交流质量。
...
### [164] [Select to Think: Unlocking SLM Potential with Local Sufficiency](https://arxiv.org/abs/2604.26940)
*Wenxuan Ye,Yangyang Zhang,Xueli An,Georg Carle,Yunpu Ma*

Main category: cs.CL

TL;DR: 提出S2T方法，将LLM的角色从生成转为选择SLM的候选，并通过蒸馏实现SLM自主重排序，提升小模型推理性能。


<details>
  <summary>Details</summary>
Motivation: SLM推理能力不足，现有方法依赖LLM生成带来延迟和成本，标准蒸馏因容量限制效果不佳。

Method: 识别局部充分性，将LLM角色转为选择SLM的top-K候选，提出S2T-LOCAL蒸馏选择逻辑至SLM。

Result: 1.5B SLM的top-8候选命中32B LLM选择达95%，S2T-LOCAL比贪心解码平均提升24.1%，效果匹敌8路径自一致性。

Conclusion: S2T方法有效提升SLM推理能力，避免推理时LLM依赖，兼顾效率和性能。
...
### [165] [A Scaled Three-Vehicle Platooning Platform](https://arxiv.org/abs/2604.25963)
*Kaiyue Lu,Qiaoxuan Zhang,Yukun Lu*

Main category: cs.RO

TL;DR: 本文介绍了用于自主队列研究的缩尺多车平台，强调协同控制和人在环路自主性。


<details>
  <summary>Details</summary>
Motivation: 车辆队列路径跟踪面临稳定性挑战，且全尺寸现场测试成本高、风险大。

Method: 开发了含1辆人类驾驶领车和2辆自主跟随车的缩尺平台，用于队列控制实验。

Result: 该平台提供了更安全、低成本、灵活的环境，用于快速原型验证和控制器评估。

Conclusion: 缩尺平台比纯仿真更具物理真实感，适合协同控制和多智能体自主性研究。
...
### [166] [FalconApp: Rapid iPhone Deployment of End-to-End Perception via Automatically Labeled Synthetic Data](https://arxiv.org/abs/2604.25949)
*Yan Miao,Will Shen,Sayan Mitra*

Main category: cs.RO

TL;DR: 提出FalconApp，一款通过手持视频快速生成物体感知模块的iPhone应用，在约20分钟内完成合成数据生成和训练，实现30ms端到端延迟，在4/5物体上优于PnP基线方法。


<details>
  <summary>Details</summary>
Motivation: 真实世界数据集依赖耗时的手动标注，需要快速自动化的感知模型部署方案。

Method: 用户拍摄物体视频后，重建可编辑的GSplat资产，合成逼真背景图像，自动生成含掩码和位姿的标注数据，训练感知模块并部署至iPhone。

Result: 在5个物体上平均20分钟生成可用模型，端到端延迟约30ms，模拟和真实评估中4/5物体位姿精度优于PnP基线。

Conclusion: FalconApp实现了从手持视频到自动标注、训练和部署的快速闭环，有效降低了机器人感知模型开发成本。
...
### [167] [Multi-Periodogram Velocity Estimation with Irregular Reference Signals for Robot-Aided ISAC](https://arxiv.org/abs/2604.25974)
*Yi Geng,Pan Cao,Ting Zeng,Yongqian Deng*

Main category: cs.RO

TL;DR: 本文针对机器人辅助集成感知与通信（ISAC）中的速度估计问题，提出了一种基于多周期图的标准兼容算法，利用不规则5G/6G参考信号实现速度估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖专用感知参考信号或需修改3GPP标准，而机器人平台只能机会性地复用不规则参考信号，导致速度估计精度受限。

Method: 将不规则时域信号分解为周期峰值分量与幅度整形（加权）分量，基于此提出多周期图速度估计算法。

Result: 与传统周期图方法相比，在低信噪比下，10%漏检率时获得了3dB的SNR增益，虚警率降低了51%。

Conclusion: 所提方法无需新增感知专用信号或修改3GPP标准，即可显著提升低SNR下的鲁棒性，适合机器人辅助ISAC系统。
...
### [168] [FlowS: One-Step Motion Prediction via Local Transport Conditioning](https://arxiv.org/abs/2604.26065)
*Leandro Di Bella,Adrian Munteanu,Bruno Cornelis*

Main category: cs.RO

TL;DR: 提出了局部传输条件化方法FlowS，使单步生成式运动预测在保持高精度和多样性的同时实现低延迟。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在运动预测中精度高、多样性好，但需要多步去噪导致延迟高，不能满足实际自动驾驶的实时性要求。

Method: 通过局部传输条件化，将基础分布置于接近可行未来轨迹的位置，使单步Euler离散化仅需短程修正；结合在线场景条件化学习先验生成K个校准锚点轨迹，以及步骤一致的位移场保证单步继承多步准确性。

Result: 在Waymo Open Motion Dataset上，FlowS以75 FPS实现单步推理，集成后Soft mAP达到0.4804，mAP达到0.4703，取得最优结果。

Conclusion: 局部传输条件化使单步生成式运动预测在安全关键型自动驾驶中变得可行。
...
### [169] [Lights Out: A Nighttime UAV Localization Framework Using Thermal Imagery and Semantic 3D Maps](https://arxiv.org/abs/2604.26201)
*Ryan Allen,Melissa Greeff*

Main category: cs.RO

TL;DR: 提出语义重投影框架，通过夜间热成像与白天RGB语义地图对齐，实现GNSS缺失环境下的无人机定位。


<details>
  <summary>Details</summary>
Motivation: 解决GNSS缺失夜间条件下，白天RGB地图与夜间热成像之间的严重模态差异问题。

Method: 将定位问题转化为共享语义域中的对称双向重投影目标，并结合混淆感知加权以应对分割不确定性。

Result: 在6.5公里真实夜间飞行轨迹上，偏差校正RMSE2D为2.18米，中位数RMSE2D为1.52米。

Conclusion: 语义重投影为仅使用热成像的夜间无人机全局定位提供了可行途径。
...
### [170] [2D and 3D Grasp Planners for the GET Asymmetrical Gripper](https://arxiv.org/abs/2604.26212)
*Andrew Goldberg,Ethan Ransing,Anton Kourakin,Cael Magner,Edward H. Adelson,Ken Goldberg*

Main category: cs.RO

TL;DR: 提出了针对非对称GET夹爪的两款抓取规划器：基于单视角RGB-D图像的快速规划器GET-2D-1.0和基于网格的3D规划器GET-3D-1.0，实验证明前者性能提升显著且效率高。


<details>
  <summary>Details</summary>
Motivation: 为零样本场景下非对称夹爪设计高效、鲁棒的抓取规划器，挑战在于平衡性能与计算效率。

Method: GET-2D-1.0采用Ferrari-Canny度量和新颖采样策略；GET-3D-1.0使用3D夹爪模型和光线追踪进行网格级规划。

Result: GET-2D-1.0在提起成功率、摇晃存活和抗外力上比基线提40%以上；GET-3D-1.0性能略优但规划时间长达17秒（vs 683ms）。

Conclusion: GET-2D-1.0是实际场景中更优的选择，兼具高性能与低计算成本。
...
### [171] [Split over $n$ resource sharing problem: Are fewer capable agents better than many simpler ones?](https://arxiv.org/abs/2604.26374)
*Karthik Soma,Mohamed S. Talamali,Genki Miyauchi,Giovanni Beltrame,Heiko Hamann,Roderich Gross*

Main category: cs.RO

TL;DR: 探讨多智能体系统中有限资源应集中还是分散，通过资源分割问题分析覆盖性能，发现覆盖速率随智能体数量增加，但速度与半径或足迹成比例时表现不同，且资源分割增加个体失败率。


<details>
  <summary>Details</summary>
Motivation: 在多智能体系统中，资源有限时如何分配（集中到少数智能体或分散到多个智能体）以优化系统性能。

Method: 形式化资源分割问题，建立n个智能体共享资源的模型，通过数学分析比较不同规模智能体群的覆盖速率，并辅以计算机模拟。

Result: 初始覆盖速率随n增加；若速度与半径成比例，所有规模表现相同；若与足迹成比例，单个智能体最优；资源分割增加个体失败率。

Conclusion: 模型和发现有助于确定最优分散程度，指导资源约束下的多智能体系统设计。
...
### [172] [Reactive Motion Generation via Phase-varying Neural Potential Functions](https://arxiv.org/abs/2604.26450)
*Ahmet Tekden,Dimitrios Kanoulas,Aude Billard,Yasemin Bekiroglu*

Main category: cs.RO

TL;DR: 提出一种基于相位变化的神经势函数（PNPF）框架，用于从演示中学习稳定且鲁棒的动态系统策略，尤其擅长处理轨迹交叉和外部扰动。


<details>
  <summary>Details</summary>
Motivation: 现有动态系统方法在处理轨迹交叉时存在局限性：一阶模型无法处理交叉点，二阶模型对扰动敏感，基于相位的方法缺乏鲁棒性。

Method: PNPF框架：将势函数与从状态进展估计的相位变量结合，而非依赖开环时间，从而生成局部向量场以实现反应性和稳定性控制。

Result: 在点对点、周期性和完整6D运动任务中均表现良好，优于处理交叉轨迹的基线方法，并在实时机器人操作中展现出抗扰动鲁棒性。

Conclusion: PNPF能有效处理状态重复访问和轨迹交叉，同时保持动态系统的稳定性与抗干扰能力，适用于复杂运动学习任务。
...
### [173] [Alter-Art: Exploring Embodied Artistic Creation through a Robot Avatar](https://arxiv.org/abs/2604.26473)
*Do Won Park,Samuele Bordini,Giorgio Grioli,Manuel G. Catalano,Antonio Bicchi*

Main category: cs.RO

TL;DR: 探索人类艺术家通过机器人化身Alter-Ego进行远程具身创作的Alter-Art范式，涵盖舞蹈、戏剧和绘画，重点关注第一人称创造性体验。


<details>
  <summary>Details</summary>
Motivation: 现有艺术机器人框架将机器人视为自主创作者或合作者，在人机之间留下鸿沟；艺术家可通过远程具身体验重新塑造创作身份，弥合此差距。

Method: 通过机器人化身Alter-Ego，集成沉浸式远程操作和柔性驱动，在舞蹈、戏剧和绘画中实践Alter-Art范式，并收集定性艺术反馈分析具身性对创作主体性、身份及环境互动的影响。

Result: 艺术家快速形成机器人体内的临场感；机器人的物理约束影响创作过程，且在不同艺术领域表现不同。

Conclusion: 具身性是核心设计原则，能促进社交机器人发展，扩展远程临场和可及艺术表达的可能性。
...
### [174] [HiPAN: Hierarchical Posture-Adaptive Navigation for Quadruped Robots in Unstructured 3D Environments](https://arxiv.org/abs/2604.26504)
*Jeil Jeong,Minsung Yoon,Seokryun Choi,Heechan Shin,Taegeun Yang,Sung-eui Yoon*

Main category: cs.RO

TL;DR: 提出了一种分层姿态自适应导航框架HiPAN，通过深度图像直接驱动四足机器人在非结构化3D环境中的高效导航。


<details>
  <summary>Details</summary>
Motivation: 传统顺序建图-规划方法在资源受限平台上存在感知误差累积和计算开销大的问题。

Method: 采用高层策略生成平面速度和身体姿态命令，低层姿态自适应控制器执行；引入路径引导课程学习扩展导航视野。

Result: 仿真中比经典反应式规划器和端到端基线方法有更高的导航成功率和路径效率，真实实验验证了其适用性。

Conclusion: HiPAN通过分层设计和课程学习有效解决了非结构化环境中的导航挑战，且适用于资源受限平台。
...
### [175] [3D Generation for Embodied AI and Robotic Simulation: A Survey](https://arxiv.org/abs/2604.26509)
*Tianwei Ye,Yifan Mao,Minwen Liao,Jian Liu,Chunchao Guo,Dazhao Du,Quanxin Shou,Fangqi Zhu,Song Guo*

Main category: cs.RO

TL;DR: 该综述论文系统梳理了面向具身智能的3D生成技术，将其归纳为数据生成器、仿真环境和仿真到现实桥梁三种角色，并指出领域正从视觉真实感向交互就绪性转变，同时总结了物理标注不足、几何与物理有效性脱节等主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 具身AI和机器人系统需要可扩展、多样化且物理真实的3D内容，但现有3D生成方法主要关注视觉真实感，缺乏对运动学结构、材料属性和交互支持等具身需求的考虑。

Method: 通过文献调研，将相关研究按三种角色（数据生成器、仿真环境、仿真到现实桥梁）组织，并分析当前瓶颈。

Result: 发现领域趋势是从视觉真实感转向交互就绪性，并识别出物理标注有限、几何质量与物理有效性脱节、评估碎片化及仿真到现实差距等关键问题。

Conclusion: 为使3D生成成为具身智能的可靠基础，需解决上述瓶颈，尤其需增强物理标注和弥合仿真到现实的鸿沟。
...
### [176] [LLM-Flax : Generalizable Robotic Task Planning via Neuro-Symbolic Approaches with Large Language Models](https://arxiv.org/abs/2604.26569)
*Seongmin Kim,Daegyu Lee*

Main category: cs.RO

TL;DR: LLM-Flax 通过三个阶段自动化神经符号任务规划器部署，借助本地 LLM 和 PDDL 领域文件，无需人工劳动，在 MazeNamo 基准测试中平均成功率达 0.945，超越人工基线的 0.828。


<details>
  <summary>Details</summary>
Motivation: 手动部署神经符号任务规划器需要领域专家编写规则和训练 GNN，工作量巨大。为消除这些手动劳动，提出 LLM-Flax。

Method: 三阶段框架：阶段1 通过结构化提示和自校正生成松弛和补全规则；阶段2 引入可行性门控预算策略的 LLM 引导失败恢复；阶段3 用零样本 LLM 物体重要性评分替换 GNN。

Result: 在8个 MazeNamo 基准测试上平均 SR 0.945 vs 手工基线 0.828；12x12 Expert SR 0.733 (手工 0.000)；15x15 Hard SR 1.000 (手工 0.900)；阶段3 在12x12 Hard 上 SR 0.720，无训练数据。

Conclusion: LLM-Flax 有效自动化部署，匹配或超越手工规则，但阶段3受上下文窗口限制，是未来主要挑战。
...
### [177] [STAR-Filter: Efficient Convex Free-Space Approximation via Starshaped Set Filtering in Noisy Environments](https://arxiv.org/abs/2604.26626)
*Yuwei Wu,Yichen Zhao,Dexter Ong,Vijay Kumar*

Main category: cs.RO

TL;DR: 提出一种名为STAR-Filter的轻量级框架，利用星形集构造作为快速滤波器，高效生成碰撞自由空间中的凸区域。


<details>
  <summary>Details</summary>
Motivation: 现有基于迭代优化的凸区域生成方法在复杂或噪声环境中效率低、对初始化敏感且依赖精确几何模型，需要更鲁棒高效的解决方案。

Method: 通过识别障碍物点作为主动支撑约束，构建星形集并作为快速滤波器，减少冗余计算，同时保持可行性和对传感器噪声的鲁棒性。

Result: 仿真实验表明，在复杂噪声和大规模真实数据中，该框架计算时间最低且保守性更小；并在安全飞行走廊生成和敏捷四旋翼规划中验证了有效性。

Conclusion: STAR-Filter能高效、鲁棒地生成凸区域，适用于复杂噪声环境下的机器人规划任务。
...
### [178] [ATLAS: An Annotation Tool for Long-horizon Robotic Action Segmentation](https://arxiv.org/abs/2604.26637)
*Sergej Stanovcic,Daniel Sliwowski,Dongheui Lee*

Main category: cs.RO

TL;DR: 提出了ATLAS，一个专为长视界机器人动作分割设计的标注工具，支持多模态可视化，显著提升了标注效率与精度。


<details>
  <summary>Details</summary>
Motivation: 现有标注工具主要针对视觉数据，缺乏对机器人时间序列信号的原生支持，且难以适应不同的数据集格式。

Method: ATLAS通过时间同步可视化多模态数据（多视角视频和本体感受信号），提供模块化数据抽象层以支持常见数据集格式（如ROS bag和RLDS），并采用键盘为中心的界面以最小化标注工作量。

Result: 在接触丰富的装配任务实验中，ATLAS相比ELAN减少了至少6%的单动作标注时间；时间序列数据的加入使时间对齐精度提高超过2.8%，边界误差降低五倍。

Conclusion: ATLAS是一种高效且易于扩展的机器人动作标注工具，适用于长视界任务，显著优于纯视觉工具和现有方案。
...
### [179] [Atomic-Probe Governance for Skill Updates in Compositional Robot Policies](https://arxiv.org/abs/2604.26689)
*Xue Qin,Simin Luan,John See,Cong Yang,Zhijun Li*

Main category: cs.RO

TL;DR: 本文提出配对采样跨版本交换协议，研究机器人技能库更新对组合任务成功率的非对称影响。


<details>
  <summary>Details</summary>
Motivation: 现有技能组合方法在测试时冻结技能库，未分析替换技能如何改变组合结果，需量化技能更新对组合策略的影响。

Method: 采用配对采样跨版本交换协议，在robosuite操作任务上分析技能替代效应，提出原子质量探针和混合选择器。

Result: 双轴任务中发现主导技能效应（ECM成功率86.7% vs 其他≤26.7%）；混合选择器将差距从23pp缩小至12pp（成本降低54%）。

Conclusion: 原子质量探针是首个可部署的技能更新治理原语，跨任务平均与完全重验证差距仅3pp。
...
### [180] [Stochastic Entanglement of Deterministic Origami Tentacles For Universal Robotic Gripping](https://arxiv.org/abs/2604.26897)
*Alec Boron,Bokun Zheng,Ziyang Zhou,Noel Naughton,Suyi Li*

Main category: cs.RO

TL;DR: 本文提出一种受折纸启发的腱驱触手夹爪，通过局部确定性变形编程与全局随机纠缠，实现通用物体抓取，具有设计简单、驱动简洁的优点。


<details>
  <summary>Details</summary>
Motivation: 现有折纸夹爪在动态环境下抓取随机形状物体时，常需额外驱动通道和控制复杂度，限制了其应用。

Method: 利用Mylar薄片切割成锥形触手，集成折痕和孔洞，通过单根肌腱驱动产生可控卷曲；多个触手近端排列时，随机纠缠（编织、打结）实现抓取；结合折纸力学与Cosserat杆模型进行仿真。

Result: 实验成功抓取多种形状物体，在重力和水下环境中均有效，并测试了收放机构模拟在轨抓取。

Conclusion: 折纸触手夹爪通过简单设计与驱动，利用确定性卷曲与随机纠缠协同，提供了一种鲁棒通用的抓取新策略。
...
### [181] [Unified 4D World Action Modeling from Video Priors with Asynchronous Denoising](https://arxiv.org/abs/2604.26694)
*Jun Guo,Qiwei Li,Peiyan Li,Zilong Chen,Nan Sun,Yifei Su,Heyun Wang,Yuan Zhang,Xinghang Li,Huaping Liu*

Main category: cs.RO

TL;DR: 提出了统一4D世界模型X-WAM，在一个框架中实现实时机器人动作执行和高质量4D世界合成，通过异步噪声采样联合优化生成质量和动作效率。


<details>
  <summary>Details</summary>
Motivation: 现有统一世界模型（如UWM）只建模2D像素空间，无法平衡动作效率和世界建模质量。

Method: 利用预训练视频扩散模型，通过轻量结构适应（复制DiT最后几块作为深度预测分支）预测多视图RGB-D视频，提出异步噪声采样联合优化训练。

Result: 在RoboCasa和RoboTwin 2.0基准上分别达到79.2%和90.7%平均成功率，并在视觉和几何指标上超越现有方法。

Conclusion: X-WAM有效统一了实时动作执行和高保真4D世界建模，验证了异步噪声采样的有效性。
...
### [182] [Rule-based High-Level Coaching for Goal-Conditioned Reinforcement Learning in Search-and-Rescue UAV Missions Under Limited-Simulation Training](https://arxiv.org/abs/2604.26833)
*Mahya Ramezani,Holger Voos*

Main category: cs.RO

TL;DR: 提出了一个结合固定规则高层顾问和在线学习低层强化学习控制器的分层决策框架，用于无人机搜索救援任务，重点提高了早期安全性和样本效率。


<details>
  <summary>Details</summary>
Motivation: 无人机在搜索救援任务中面临模拟训练有限、需要快速适应且确保安全的问题，现有方法难以兼顾规则的可解释性和强化学习的自适应性。

Method: 高层顾问通过离线任务规范编译为确定性规则，提供可解释的安全引导；低层控制器在线学习，结合任务定义密集奖励和基于规则的元数据增强优先回放机制。

Result: 在电池感知多目标投递和移动目标投递任务中，该方法通过减少碰撞终止事件，显著提升了早期安全性和样本效率，同时保持在线适应能力。

Conclusion: 该分层框架能有效平衡规则的安全保障与在线学习的适应性，适用于模拟训练受限的无人机任务。
...
### [183] [Walk With Me: Long-Horizon Social Navigation for Human-Centric Outdoor Assistance](https://arxiv.org/abs/2604.26839)
...
TL;DR: 提出了一种无地图框架Walk with Me,利用GPS、公共地图API和分层VLM实现从高级语言指令到安全、长时程室外社交导航。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖昂贵的高清地图或局限于室内短时程场景，缺乏面向开放世界、长时程社交导航的轻量级方案。

Method: 结合GPS与公共地图API进行语义目标定位和路径点提议；高层VLM负责意图理解和粗路径规划；低层VLA执行常规动作，同时通过观测感知路由机制在复杂场景（如拥挤路口）触发高层安全推理与停止等待行为。

Result: 实现了无地图、长时程、社交合规的室外导航，能处理高层指令并安全应对复杂动态环境。

Conclusion: Walk with Me通过语义意图接地、轻量级长时程规划、安全推理与低层执行协同，为室外机器人辅助导航提供了有效方案。
...
### [184] [STARRY: Spatial-Temporal Action-Centric World Modeling for Robotic Manipulation](https://arxiv.org/abs/2604.26848)
*Yuxuan Tian,Yurun Jin,Bin Yu,Yukun Shi,Hao Wu,Chi Harold Liu,Kai Chen,Cong Huang*

Main category: cs.RO

TL;DR: 提出STARRY策略，通过联合去噪未来时空潜变量和动作序列，以及几何感知选择性注意力调制，提升了机器人操作中的动作生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略和世界模型增强策略未能充分建模与动作相关的空间-时间交互结构。

Method: 联合去噪未来时空潜变量和动作序列，引入几何感知选择性注意力调制，将预测深度和末端执行器几何转换为令牌对齐权重进行选择性动作注意力调制。

Result: 在RoboTwin 2.0上，清洁和随机设置下平均成功率分别为93.82%和93.30%；真实实验中，相比π₀.₅，平均成功率从42.5%提升至70.8%。

Conclusion: 以动作为中心的空间-时间世界建模对高空间-时间要求的机器人动作生成是有效的。
...
### [185] [Bi-Level Optimization for Contact and Motion Planning in Rope-Assisted Legged Robots](https://arxiv.org/abs/2604.26910)
*Ruben Malacarne,Ioannis Tsikelis,Enrico Mingo Hoffman,Michele Focchi*

Main category: cs.RO

TL;DR: 提出了一种绳索辅助机器人垂直攀爬的规划框架，采用双层优化解决混合整数问题，并在ALPINE平台验证。


<details>
  <summary>Details</summary>
Motivation: 绳索辅助机器人垂直攀爬时需同时处理地形选择与运动控制优化，现有方法难以高效解决该混合整数问题。

Method: 双层优化：外层用交叉熵法选择可行落地区域，内层用梯度法优化绳索张力、腿部力和落地点。

Result: 在ALPINE平台上不同挑战性地形中成功实现运动规划与执行。

Conclusion: 该框架能有效处理攀爬中的混合整数优化问题，实现动态可行运动。
...
### [186] [Risk Assessments for Evasive Emergency Maneuvers in Autonomous Vehicles](https://arxiv.org/abs/2604.26050)
*Aliasghar Arab,Milad Khaleghi,Koorosh Aslansefat*

Main category: eess.SY

TL;DR: 提出了一种统一的EMRM验证框架，结合HARA、STPA和FSM，实现了高分辨率、可追溯的安全评估。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估方法存在缺口，无法系统验证自动驾驶车辆的避撞功能。

Method: 通过HARA-STPA危害-损失映射识别危害，FSM建模状态转换，驱动自动化场景生成。

Result: EMRM转向策略成功率81%，平均残留速度从18.9降至9.0 km/h，覆盖100%参数空间。

Conclusion: 集成框架优于单一方法，可实现高分辨率、可追溯的EMRM V&V。
...
### [187] [Optimal-Control Suggestion for Congestion on Freeways using Data Assimilation of Distributed Fiber-Optic Sensing](https://arxiv.org/abs/2604.26017)
*Yoshiyuki Yajima,Hemant Prasad,Daisuke Ikefuji,Takemasa Suzuki,Shin Tominaga,Hitoshi Sakurai,Manabu Otani*

Main category: eess.SY

TL;DR: 提出利用分布式光纤感测数据同化实现高速公路最优控制，在无监测盲区场景下实时估计控制效果并唯一确定帕累托最优解。


<details>
  <summary>Details</summary>
Motivation: 传统传感器数据间歇性导致无法准确估计最优控制，需要无死角的实时监测方法支持主动交通需求管理。

Method: 基于分布式光纤感测数据同化进行实时交通状态估计，通过仿真评估控制方案效果，并采用多目标优化方法从帕累托解集中唯一确定最优控制解。

Result: 仅可变限速可提升通行能力5-14%，平均速度0-8%；结合入口控制后通行能力提升10-15%，平均速度提升20-30%。

Conclusion: 最优控制方案随交通状态特别是拥堵程度动态变化，需在拥堵发生前提前进行车道占用平衡管理和主动控制。
...
### [188] [Application of Deep Reinforcement Learning to Event-Triggered Control for Networked Artificial Pancreas Systems](https://arxiv.org/abs/2604.26126)
*Junya Ikemoto,Satoshi Maruyama,Kazumune Hashimoto*

Main category: eess.SY

TL;DR: 提出一种基于深度强化学习的网络化人工胰腺系统事件触发控制器设计，通过引入基于血糖变化的规则标准避免显式学习更新时机，将问题建模为半马尔可夫决策过程，在保持控制性能的同时提升通信效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于DRL的AP控制器通常假设周期性控制更新，而网络化控制系统需要降低通信频率以实现能效运行，但联合学习胰岛素剂量和更新时机增加了学习复杂度。

Method: 采用基于血糖变化规则的判据自动判断更新时机，将决策过程建模为半马尔可夫决策过程（SMDP），并扩展标准DRL算法进行求解。

Result: 数值实验表明该方法在保持控制性能的同时提高了通信效率。

Conclusion: 通过避免显式学习更新时机并利用SMDP框架，能有效降低学习复杂度，实现能效与性能的平衡。
...
### [189] [Co-Learning Port-Hamiltonian Systems and Optimal Energy-Shaping Control](https://arxiv.org/abs/2604.26172)
*Ankur Kamboj,Biswadip Dey,Vaibhav Srivastava*

Main category: eess.SY

TL;DR: 提出了一种物理信息学习框架，用于从轨迹数据中学习端口-哈密顿系统的能量成形控制，实现协同学习系统模型和最优控制器，确保闭环系统的无源性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的数据驱动控制方法缺乏对物理结构和稳定性的保证，而基于模型的方法难以适应复杂非线性系统。

Method: 采用交替优化策略，通过嵌入pH动力学和EB-PBC结构的神经网络参数化系统模型和控制器，并引入耗散正则化强制训练过程中的能量衰减。

Result: 在平面和扭转摆系统的状态调节和摆动任务中验证了有效性，学习到的控制器使闭环系统无源且稳定。

Conclusion: 该框架通过融合物理知识和数据驱动方法，实现了可解释性强、鲁棒性好且稳定的能量成形控制。
...
### [190] [Real-Time Minimum-Energy Operating-Point Tracking for Battery-Powered Micro DC Motors Under Dynamically Variable Loading](https://arxiv.org/abs/2604.26335)
*Tzu-Hsiang Huang,Haojian Lu,Hen-Wei Huang,Tan Rong*

Main category: eess.SY

TL;DR: 提出了一种实时运行点跟踪方法，帮助微型直流有刷电机在动态负载下自动收敛到最小能耗状态，并验证了其有效性和快速响应能力。


<details>
  <summary>Details</summary>
Motivation: 传统驱动策略使用保守电压裕度避免堵转，导致能效低下，且现有方法忽略动态条件下单次循环的能量消耗。

Method: 通过分析电流波形特征构建轻量化负载指标，并设计两阶段自适应电压策略，在线追踪最优运行点。

Result: 在低到高和高到低负载切换下，平均收敛时间分别为11.55秒和11.16秒，收敛电压为2.73V和2.0V。

Conclusion: 所提方法能有效追踪最小能耗运行区域，兼顾动态负载变化和快速响应，适用于电池供电系统。
...
### [191] [Risk-Aware Multi-Market Scheduling of Virtual Power Plants with Dynamic Network Tariffs](https://arxiv.org/abs/2604.26424)
*Lorenzo Zapparoli,Paul Fäth,Blazhe Gjorgiev,Giovanni Sansavini*

Main category: eess.SY

TL;DR: 提出了一种考虑动态网络电价的虚拟电厂两阶段随机优化调度框架，实现多市场联合出清与风险管控。


<details>
  <summary>Details</summary>
Motivation: 现有调度工具忽略本地灵活性采购机制（如动态网络电价），难以充分利用分布式能源的灵活性。

Method: 采用两阶段随机优化、条件风险价值、Benders分解，集成设备级约束、网络限制及市场不确定性。

Result: 风险中性与风险规避策略均利用套利；风险规避降低利润波动；动态网络电价日间转移负荷释放灵活性，但强电价信号使预期利润下降65%且灵活性收益有限。

Conclusion: 动态电价能挖掘本地灵活性，但需平衡激励强度与经济性，风险偏好策略影响收益稳定性。
...
### [192] [Fuelling fusion plasmas with pellets: Can neuromorphic control outperform Sigma-Delta modulation?](https://arxiv.org/abs/2604.26476)
*L. L. T. C. Jansen,E. Petri,M. van Berkel,W. P. M. H. Heemels*

Main category: eess.SY

TL;DR: 本文提出了一种用于托卡马克聚变堆中燃料注入控制的形式化混合模型，并设计了一种类脑神经形态控制器，通过将燃料丸视为类似脉冲的信号，实现了轻量级且匹配混合特性的控制方案。


<details>
  <summary>Details</summary>
Motivation: 核聚变中需要精确控制等离子体密度，而燃料丸注入过程是混合系统（连续等离子体动态与离散粒子注入），现有控制方法（如sigma-delta调制）不能充分发挥该特性，因此需要一种更自然适配混合特性的控制方案。

Method: 构建了形式化混合模型描述燃料注入过程；提出了基于神经形态的控制器（将丸模拟为脉冲信号）；同时开发了sigma-delta调制的混合模型作为对比。推导了执行器和控制器参数的显式约束条件以实现实际稳定性。

Result: 数值仿真验证了神经形态控制器与sigma-delta调制在不同变体下的性能，理论推导的约束条件可以指导控制器参数调整，确保系统稳定性。

Conclusion: 神经形态控制器为托卡马克燃料注入提供了一种轻量级、自然适配混合系统的解决方案，其稳定性约束有助于实际部署，仿真结果支持理论分析的有效性。
...
### [193] [Learning to Route Electric Trucks Under Operational Uncertainty](https://arxiv.org/abs/2604.26566)
*Stavros Orfanoudakis,Ziyan Li,Ruixiao Yang,Nikolay Aristov,Pedro P. Vergara,Chuchu Fan,Elenna Dugundji*

Main category: eess.SY

TL;DR: 提出了一种基于强化学习的电动卡车路径规划框架，处理充电限制和运营不确定性。


<details>
  <summary>Details</summary>
Motivation: 电动卡车运营面临电池续航有限、充电时间长、充电基础设施竞争等挑战，传统方法不可行。

Method: 将问题建模为事件驱动的半马尔可夫决策过程，使用图表示状态和动作，结合规则掩码，构建事件驱动仿真环境。

Result: 在多种车队规模下，学习算法优于启发式方法，接近优化基准，并保持高成功率。

Conclusion: 该方法有效解决电动卡车路径规划中的耦合物流与能源问题，具有可扩展性。
...
### [194] [Exploring Converter Control Duality in Microgrids: AC Grid-Forming vs DC Droop Control](https://arxiv.org/abs/2604.26595)
*Jovan Krajacic,Ognjen Stanojev,Mario Schweizer,Orcun Karaca,Gabriela Hug,Vladan Lazarević*

Main category: eess.SY

TL;DR: 本文基于对偶概念，揭示了交流构网控制和直流I-V下垂控制在多个方面的内在相似性，包括小信号模型、内环电流控制、功率共享机制以及扰动响应，并通过仿真验证了这些对偶性。


<details>
  <summary>Details</summary>
Motivation: 交流与直流微电网的变换器控制策略（构网控制和下垂控制）虽共享某些特征，但由于物理域不同，其相似性尚未被充分探索。

Method: 采用对偶理论框架，从数学模型、控制结构、功率平衡和动态响应角度进行对比分析，并通过仿真验证理论结果。

Result: 证明了交流构网控制与直流I-V下垂控制在小信号模型、电流控制环、基于摇摆方程/电容功率平衡的功率共享机制以及扰动信号与动态响应方面具有对偶关系。

Conclusion: 两种控制策略本质上是统一的，为微电网控制设计提供了新视角，有助于开发跨领域统一的控制方法。
...
### [195] [Model-Free Dynamic Mode Adaptive Control for Data-Driven Control Synthesis](https://arxiv.org/abs/2604.26682)
*Parham Oveissi,Ankit Goel*

Main category: eess.SY

TL;DR: 提出了一种无模型数据驱动控制合成方法DMAC，通过在线估计系统动态并合成自适应控制器，适用于无数学模型系统。


<details>
  <summary>Details</summary>
Motivation: 传统控制设计依赖数学模型，但许多系统模型不可获取或不适用，需要一种数据驱动方法实现在线控制。

Method: 结合动态近似模块（矩阵递归最小二乘与遗忘因子）和控制器合成模块（全状态反馈与积分作用），在线设计稳定控制器。

Result: 建立了递归动态逼近的收敛性和闭环系统有界性理论，数值示例（不稳定线性系统、Van der Pol振荡器、Burgers方程）验证了性能，敏感性研究展示了鲁棒性。

Conclusion: DMAC方法在无模型场景下有效，具有鲁棒性，适用于非线性及不稳定系统。
...
### [196] [A Control Framework for Induced Seismicity Mitigation in Groningen Gas Reservoir](https://arxiv.org/abs/2604.26802)
*Diego Gutiérrez-Oribio,Ioannis Stefanou*

Main category: eess.SY

TL;DR: 提出一种控制导向的框架，通过反馈控制器调节井口注入/开采速率，在满足生产目标的同时降低地震活动性，并在格罗宁根气田场景下验证。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注预测地震活动性，而较少从优化生产策略角度直接设计降低地震风险的操作方案。

Method: 采用孔隙压力扩散与地震活动率（SR）级联模型，结合随机事件生成过程合成地震目录；基于目录估计SR测量值，设计带饱和约束的鲁棒反馈控制器，以离散时间间隔更新井口流量指令。

Result: 数值实验表明，该框架能有效控制地震活动性水平并最大化生产目标，对不同控制更新周期和增益选择具有鲁棒性。

Conclusion: 所提方法可在满足生产约束的前提下实现地震活动性的可控降低，为油气开采诱发地震管理提供工程化解决方案。
...
### [197] [PM-EKF: A Physiological Model-Based Extended Kalman Filter for Daily-Life Physical Activity Energy Expenditure Estimation](https://arxiv.org/abs/2604.26803)
*Shuhao Que,Remco Poelarends,Valentina Breschi,Ying Wang*

Main category: eess.SY

TL;DR: 提出一个基于扩展卡尔曼滤波的简化生理模型，利用IMU和HR数据估算日常活动中的能量消耗，优于纯数据驱动方法。


<details>
  <summary>Details</summary>
Motivation: 现有的IMU和HR方法多为数据驱动且缺乏生理可解释性，需要一种能链接身体运动与代谢过程的解释性模型。

Method: 构建非线性状态空间生理模型，嵌入扩展卡尔曼滤波器处理噪声和不确定性，仅使用三个IMU（骨盆和两腿）和心率传感器。

Result: 模型预测PAEE的R²中位数为0.72，优于线性回归（0.52）和CNN-LSTM（0.65），且去除HR信号影响不显著。

Conclusion: 提出的框架提供个性化、可解释的PAEE估计，IMU捕捉的机械负荷是主要影响因素。
...
### [198] [Spectral Boundary Observer for Counter-Flow Heat Exchangers](https://arxiv.org/abs/2604.26863)
*Mohamed Camil Belhadjoudja,Mohamed Maghenem,Emmanuel Witrant*

Main category: eess.SY

TL;DR: 针对逆流换热器耦合双曲PDE系统，设计了仅利用冷流体边界温度测量的边界观测器，并通过谱分配确保观测误差的L2指数收敛。


<details>
  <summary>Details</summary>
Motivation: 逆流换热器中热流体与冷流体的耦合传热过程需要高效的状态观测，但传统观测器设计面临边界测量有限和误差收敛性难以保证的挑战。

Method: 采用谱方法：将观测误差动态算子的谱分配到复平面左半开平面指定区域；通过证明谱映射性质，建立谱稳定性与L2指数稳定性的等价关系。

Result: 证明谱稳定性（谱位于左半平面）等价于观测误差动态原点的L2指数稳定性，从而可自由调节误差收敛速率。

Conclusion: 提出的边界观测器仅需单点边界测量即可实现逆流换热器温度场的指数收敛估计，且收敛速率可通过谱配置自由调控。
...
### [199] [Safe Navigation using Neural Radiance Fields via Reachable Sets](https://arxiv.org/abs/2604.26899)
*Omanshu Thapliyal,Malarvizhi Sankaranarayanasamy,Ravigopal Vennelakanti*

Main category: eess.SY

TL;DR: 本文提出利用可达集表示来捕捉自主系统在杂乱环境中安全导航的要求，结合神经辐射场（NeRF）计算障碍物和自车的体积表示，并通过带有线性矩阵不等式约束的最优控制求解路径规划问题，仿真结果验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有导航方法在处理不同几何形状的障碍物和目标时，难以确保实时安全约束的满足。

Method: 使用可达集描述机器人在状态空间中的实时能力，结合NeRF进行体积表示，采用约束最优控制（含线性矩阵不等式）求解路径规划。

Result: 在两个不同场景的仿真中，展示了在有大量障碍物的情况下安全导航的成功实现。

Conclusion: 通过可达集与约束最优控制的结合，可有效实现杂乱环境下的安全路径规划。
...
### [200] [Adaptive Transform Coding for Semantic Compression](https://arxiv.org/abs/2604.26492)
*Andriy Enttsel,Vincent Corlay*

Main category: eess.IV

TL;DR: 提出了一种面向语义特征压缩的自适应变换编码方法，基于高斯混合模型的条件率失真函数，在多种视觉特征上优于或媲美神经压缩方法。


<details>
  <summary>Details</summary>
Motivation: 视觉数据压缩正从面向人类重建转变为面向机器推理，需要高效压缩语义嵌入。

Method: 利用高斯混合模型的条件率失真函数，根据推断源成分选择模态相关变换和量化器。

Result: 在多种视觉骨干网络和基础模型特征上，该方法优于或与最先进神经压缩方法相当，同时保持灵活性和可解释性。

Conclusion: 所提方法为异构特征分布提供了更高效的编码，兼顾性能与可解释性。
...
### [201] [Circular Phase Representation and Geometry-Aware Optimization for Ptychographic Image Reconstruction](https://arxiv.org/abs/2604.26664)
*Carson Yu Liu,Jun Cheng,Chien-Chun Chen,Steve F. Shu*

Main category: eess.IV

TL;DR: 提出了一种基于深度学习的叠层衍射成像重建框架，通过cos/sin建模相位、测地线损失和并行解码器，在保持物理一致性的同时实现了对幅度和相位的精确重建，并显著提升了计算速度。


<details>
  <summary>Details</summary>
Motivation: 传统迭代重建方法计算成本高，而现有深度学习方法将相位视为欧氏标量，忽略了其2π周期性，导致包裹伪影和不连续性问题。

Method: 使用余弦和正弦分量在单位圆上建模相位；采用可微分的测地线损失避免分支切割；设计饱和感知双增益输入缩放、并行编码器分支、三个解码器（幅度、余弦、正弦）及复合损失（促进圆一致性和结构保真度）。

Result: 在合成和实验数据集上，幅度和相位重建均优于现有深度学习方法；频域分析显示中高频相位内容保留更好；相比迭代求解器实现大幅速度提升。

Conclusion: 该方法有效结合了深度学习的效率与物理一致性，为高速叠层衍射重建提供了新方案。
