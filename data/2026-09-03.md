...
TL;DR: 该论文引入了 EvalDetectBench，这是一个旨在测量大型语言模型（LLM）“评估意识”（evaluation awareness）的开源基准与流程。


<details>
  <summary>Details</summary>
Motivation: LLM 在评估和实际部署中表现不同会损害评估结果的有效性，而当前的 AI 安全框架高度依赖这些评估结果。

Method: 开发了 EvalDetectBench，利用 Inspect 兼容的评估管道，通过精心设计的转录套件测试模型识别评估的能力，并引入了“每模型探针校准”和“分层生成器协调”方法来纠正现有文献中的系统偏差。

Result: 研究发现模型生成来源的身份会导致 11.25% 的测量方差并影响模型排名，且针对特定模型优化的诱导提示词在其他模型上表现不佳。

Conclusion: EvalDetectBench 有效识别了现有评估方法中的偏差，为衡量 LLM 是否能识别评估提供了更客观、校准后的评估标准。
...
### [2] [When Can a Machine Trust a Statute? A Survival Certificate for Machine-Extracted Legal Logic](https://arxiv.org/abs/2609.01741)
*Surya Saka*

Main category: cs.AI

TL;DR: 该研究探讨了在机器解析法律条文存在差异（噪声）的情况下，如何确定逻辑蕴含关系的可靠性。


<details>
  <summary>Details</summary>
Motivation: 机器解析法律条文时往往存在分歧（假阴性率较高），需要评估在此类噪声环境下，法律逻辑体系中的哪些蕴含关系依然成立且可靠。

Method: 构建了一种基于Duquenne-Guigues蕴含基的被动生存证书（passive survival certificate），利用蒙特卡洛模拟和Wilson置信区间（95%）来验证法律属性间的蕴含关系，并针对不同误差模型进行校准分析。

Result: 研究发现，虽然部分法律族群通过了验证，但若采用全局部署的误差模型，超过93%的章节因信息量不足而失效；实验证明这种失效主要源于校准率迁移而非样本选择。

Conclusion: 所提出的生存证书方法在处理法律数据时是可行的，但具有脆弱性，建议在实际应用中采用分章节校准或容错机制。
...
### [3] [Meta-ethics and AI: exploring the novel meta-ethical questions in the era of AI](https://arxiv.org/abs/2609.01685)
*Shang Lu*

Main category: cs.AI

TL;DR: 本文探讨了人工智能具备道德能力后对元伦理学领域产生的挑战，提出了“AI自身伦理”概念，并构建了一个分析框架来重新审视传统元伦理理论。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能道德推理和反思能力的潜在提升，现有人类中心的元伦理学框架可能不足以解释AI作为道德主体时产生的新问题。

Method: 通过提出一个条件性与方法论框架，将元伦理探究划分为四个维度（人类/AI视角下的自身/对方伦理），并评估了认知主义、非认知主义、错误论等主流理论在AI语境下的适用性。

Result: 研究表明，现有的以人类为中心的元伦理学理论在直接应用于AI时会遇到困难，难以直接迁移。

Conclusion: AI自身伦理的出现将对现有伦理框架构成压力，迫使我们必须对传统元伦理学进行大幅度的重构与修正。
...
TL;DR: 该论文通过有限发现模型分析了信息共享对池化估计的影响，揭示了在特定条件下信息共享优于独立救援行动。


<details>
  <summary>Details</summary>
Motivation: 旨在量化和分离信息共享在消除独立行动与提升池化估计准确性方面的不同效应，解决复杂环境下信息共享的策略选择问题。

Method: 利用精确有限发现模型、集中式行动预算配置文件、注册增量共享协议以及双智能体贝叶斯博弈框架进行理论推导。

Result: 发现当池化残差误差的收缩速度快于独立救援时，共享步骤能提升发现效率；在特定贝叶斯均衡下，信号准确度达3/5时存在严格的正共享区间。

Conclusion: 信息共享的有效性不仅取决于模型设定，还高度依赖于博弈均衡的选择，而非具有普适性的单一结果。
...
TL;DR: 该研究提出了一种结合自然语言与源代码的计算模型，通过LLM引导的贝叶斯学习来模拟人类如何从稀疏数据中获取并维护抽象知识。


<details>
  <summary>Details</summary>
Motivation: 旨在解决人类如何在面对稀疏、嘈杂的经验数据时，实现高效、具备不确定性感知且灵活的抽象知识构建，并克服传统贝叶斯模型计算成本过高或纯LLM逻辑推理不足的问题。

Method: 将符号知识表示为“思维程序”（mental programs），结合自然语言与源代码，并利用大语言模型（LLM）作为底层机制来引导贝叶斯推理过程，实现对假设空间的动态更新。

Result: 模型成功复现了人类在归纳学习和主动探究中的量化特征（如锚定效应、路径依赖等），且在效率和灵活性上优于传统的纯LLM或经典贝叶斯模型。

Conclusion: 人类知识的增长可能是通过在大语言模型提供的“自下而上”推理机制支持下，对类语言和类程序的假设进行贝叶斯更新，从而实现高效学习的。
...
TL;DR: 提出了一种“Hydration Proxy Pattern”架构，用于解决企业平台在集成无状态LLM API时面临的会话持久化与语义记忆管理难题。


<details>
  <summary>Details</summary>
Motivation: LLM API的无状态特性导致企业应用在管理对话状态和语义记忆时负担过重，且缺乏对会话数据的控制权。

Method: 引入Hydration Proxy Pattern以解耦会话持久性与推理引擎，并提出Context Stabilization Mandate以平衡数据主权管理与KV缓存效率。

Result: 实现了在保持平台对对话数据主权的同时，支持安全的多阶段语义关联。

Conclusion: 该架构填补了企业级应用在接入LLM时状态管理能力的缺失，有效解决了无状态API带来的工程架构挑战。
...
TL;DR: SSAKG 2.0 是一个开源软件包，用于构建和操作结构化序列关联知识图谱，通过高效的内存利用实现部分上下文下的序列完整重建。


<details>
  <summary>Details</summary>
Motivation: 为了解决在大规模稀疏图数据中进行高效序列存储与关联检索的性能瓶颈，提供更灵活且高效的计算环境。

Method: 采用 Python 与 C 混合编程实现，引入利用计算机位级操作的新算法以优化图连接搜索，并针对不同数据集（数值、自然语言、mRNA）进行验证。

Result: 实验证明该软件包能够有效存储并在给定部分上下文的情况下重构完整序列，同时验证了图密度、序列长度及内存大小对检索性能的影响。

Conclusion: SSAKG 2.0 提供了一个高性能、开源且易于使用的框架，适用于处理序列关联数据，并通过优化底层实现显著降低了内存和计算开销。
...
TL;DR: 该研究探讨了持久化记忆对大语言模型的影响，发现模型在面对过时的陈旧事实时容易产生过度信任，导致性能下降（即“记忆信任差距”）。


<details>
  <summary>Details</summary>
Motivation: 个性化代理依赖持久化记忆，但存储的陈旧事实可能覆盖权威证据，研究旨在揭示模型能力随规模变化时，这种“过度信任”现象如何演变。

Method: 利用Qwen3（0.6B-8B）和Llama系列模型，构建包含“收益”和“安全”两个套件的基准测试，通过因子分析研究标签、时效性、来源权威度和位置信息对模型决策的影响。

Result: 发现模型普遍存在对陈旧信息的过度信任；较大模型在面对被标记为“最新”的陈旧信息时崩溃程度更严重；缓解策略受模型能力限制，仅较大模型能通过元数据改善，小型模型需通过冲突预解决来恢复性能。

Conclusion: 模型对记忆的过度信任是规模依赖性的，且这种缺陷难以通过简单的标签缓解，在大规模模型中表现尤为显著，需要更深层次的策略来纠正这种记忆偏见。
...
TL;DR: 提出了一种名为信念校准优化（BCO）的方法，通过将编码代理的隐式信念显式化为持续更新的上下文文档（世界模型），提升了LLM代理的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的编码代理优化过程通常依赖隐含的信念，缺乏对环境反馈机制的显式记录和累积，导致后续迭代无法有效利用历史经验。

Method: 引入信念校准优化（BCO），将编码代理对环境行为的信念以文档形式持续维护和修订，使代理能够基于该“世界模型”进行优化。

Result: 在五个涵盖内存QA、工具使用等任务的基准测试中，BCO相较于无世界模型的对照组在训练通过率上表现更优，且证明了该文档确实包含可重用的环境预测信息。

Conclusion: 将环境信念显式化为可演进的世界模型能显著提升LLM代理在迭代优化中的效率和泛化能力。
...
TL;DR: 研究指出多智能体系统在合成报告时存在“认知女巫问题”（epistemic Sybil problem），即多个智能体产生的报告可能源自相同证据，若仅依靠报告相似度聚合信息会导致推断偏差。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统常假设更多智能体等于更多信息，但未考虑多个报告可能源自相同证据（共同起源）或相同模型导致的关联误差。

Method: 形式化定义了认知女巫问题，建立高斯共享根模型，并对超过20,000次受控LLM报告提取调用进行实证测试，评估证据源与报告多重性对推断的影响。

Result: 实验显示增加智能体数量（报告多重性）会使推断覆盖率急剧下降，而增加独立证据源则能显著改善。模型揭示了智能体间提取错误的强相关性，并证明现有的报告去重机制无法有效区分证据来源。

Conclusion: 集体推理应聚焦于追踪“证据来源”及其依赖关系，而非简单地增加智能体数量或依赖报告相似度进行聚合。
...
TL;DR: 该论文提出了一个框架，通过区分“学习者差距”（模型能力不足）与“测量通道上限”（数据信息饱和），评估临床预测模型的表现瓶颈。


<details>
  <summary>Details</summary>
Motivation: 临床预测常出现性能饱和，但研究者无法区分是模型拟合能力有限还是原始变量本身的信息量上限，需要一套审计方法来决定是改进模型还是引入新数据。

Method: 利用总变差分离和交叉拟合上限估计，并结合标签置换乐观底线与欠拟合曲线等有限样本诊断工具进行分析。

Result: 通过三大临床数据集验证，发现梯度提升树在多数任务中接近预测极限；同时揭示了即使AUROC差异不大， Bayes决策翻转率也可能存在显著差异，且跨模态测量（改变测量通道）比单纯优化模型更能提升性能。

Conclusion: 研究提供了一种可审计的决策框架：若存在学习者差距则改进模型，若达到测量上限则增加新的数据测量通道。
...
TL;DR: 该研究探讨了循环神经网络（Recurrent Transformers）是否与前馈模型一样具备类似于“全局工作空间”的因果表征，发现循环架构同样能形成工作空间，但其访问和交互模式与前馈模型显著不同。


<details>
  <summary>Details</summary>
Motivation: 前馈 Transformer 的中间层已被证明存在类全局工作空间，但这种功能是否在权值共享的循环深度架构中同样涌现，目前尚不明确。

Method: 扩展了 Jacobian lens 技术，通过虚拟展开适配器（virtual-unrolling adapter）对循环架构（Ouro-2.6B 和 Huginn-0125）进行了工作空间套件分析，包括镜头拟合、读出以及11类因果实验。

Result: 循环架构中确实形成了工作空间，但 Ouro 要求跨循环进行读写和消融，而 Huginn 则表现出类似于滑动窗口的局部访问特性，且工作空间的可操纵性和可语言化程度受监督方式影响。

Conclusion: 循环深度不仅能涌现出全局工作空间，还通过不同的动态机制实现了信息的维持与更新，证明了工作空间理论在不同计算深度实现方式下的通用性。
...
TL;DR: 本文探讨了将Qwen-4B模型进行后端三元量化（1.58-bit级别）的实验，分析了其在存储压缩、性能损失及部署效率方面的表现。


<details>
  <summary>Details</summary>
Motivation: 旨在探究超低比特（1.58-bit）量化对语言模型实际能力保留、运行时行为及存储效率的影响，而非仅关注标称比特数。

Method: 使用KOTMS旋转、E2M-ATQ三元化以及TWLA风格的GPTQ误差补偿技术对Qwen-4B进行权重的后训练转换。

Result: 转换后模型占用空间从8.29 GiB降至3.96 GiB；在十项基准测试中，平均准确率从64.5%降至54.7%，不同任务间性能下降不均匀；推理微基准测试显示目前版本在特定算子下较FP16慢4.6倍。

Conclusion: 该压缩方法显著减少了存储需求，但在当前实现下未能带来推理加速，且在复杂推理任务中存在较明显的准确率损耗。
...
TL;DR: 该论文提出了“统计问题表述”（Statistical Problem Formulation）这一新任务，并发布了名为 StatFormBench 的基准测试集，用于评估大模型在真实数据科学场景中将非正式用户目标转化为统计任务的能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法多假设任务目标已明确，但现实中用户往往只有模糊目标且面对异构数据，模型需要先自行确定统计目标并识别相关变量。

Method: 将统计问题表述拆解为“统计问题分类”和“变量识别与角色分配”两个子任务，构建了涵盖 1,013 个样本、跨越 5 本教材和案例库的 StatFormBench 基准。

Result: 通过对 14 种 LLM 进行测试，发现表现最好的模型在细粒度分类上仅达到 72.0% 的准确率，在变量识别重叠度上仅为 63.2%，且主流提示词优化策略提升有限。

Conclusion: 现有的 LLM 在处理真实的统计问题表述任务上仍存在显著不足，该基准测试揭示了模型在从非结构化需求中提取统计逻辑方面的局限性。
...
TL;DR: 本文通过案例研究分析了LLM编程代理在系统级工程任务中的表现，并评估了检索过滤策略对系统性能的影响。


<details>
  <summary>Details</summary>
Motivation: 缺乏关于LLM编码代理在处理系统级需求（如架构设计、异步编排、配置正确性等）时行为的实证研究。

Method: 通过实现一个多组件数据系统案例研究，记录代理产生的五种缺陷类型；同时在HotpotQA基准上对比了‘候选实体限制’与‘无过滤搜索’两种检索策略的性能。

Result: 实验发现受限检索在较小预算下即可达到召回上限，优于无过滤搜索（p < 0.0001）；同时指出了代理在自我纠错及性能回归验证方面的局限性。

Conclusion: LLM代理在系统级工程中表现出一定的自主性，但在复杂约束遵守及性能验证方面仍存在不可忽视的缺陷，需人为介入。
...
TL;DR: 本文提出了 ClaimReceipt 框架，通过引入声明相关的收据规范和选择性验证器，解决了 AI 智能体实验证据的“充分性”与“覆盖率”验证难题。


<details>
  <summary>Details</summary>
Motivation: 现有的通用日志和哈希链接记录无法可靠地验证 AI 实验声明是否可复现，以及记录是否涵盖了完整的实验集，缺乏有效的可审计性。

Method: 设计了 ClaimReceipt 协议，将类型化的交易证据绑定到签名的实验清单中，通过预先冻结的规范进行验证，输出 PASS、INVALID 或 INCONCLUSIVE 结果。

Result: 在 1,392 条历史记录的审计中，验证器准确再现了所有人工标签，并在 11 项语义错误测试中零误报。实验显示该方法仅增加极小的计算开销（0.021%）。

Conclusion: 有效的 AI 声明验证不仅需要提供充分的证据，还必须建立一个预提交的“实验宇宙”，以便通过识别遗漏来确保证据的完整性和可验证性。
...
TL;DR: 提出了一种名为HeadWiseKV的训练无关框架，通过为混合架构语言模型的全局注意力层分配静态、多级历史窗口，有效压缩KV缓存并优化长上下文推理。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理中的KV缓存占用大量GPU内存，限制了生成吞吐量，且现有混合语言模型中全局注意力层的缓存需求依然是主要瓶颈。

Method: 将KV分配问题转化为受限的率失真优化问题，并提出SeqCalib算法进行层级协同决策；通过分组缓存运行时实现物理层面的KV驻留控制。

Result: 在保持与完整KV缓存相近的性能前提下，在112K上下文长度下减少了8.59%的峰值内存占用，并将最大成功上下文长度从114K扩展至161K。

Conclusion: HeadWiseKV通过静态、可预测的缓存分配策略，在不牺牲模型质量的情况下，显著提升了长上下文任务的内存效率和系统容量。
...
TL;DR: 该研究提出了一种在无法获取模型内部logit信息的情况下，仅通过观测Web智能体轨迹（Macro和Micro特征）来预测其执行过程风险的方法。


<details>
  <summary>Details</summary>
Motivation: 在黑盒模型中，由于缺乏 token logit 等内部不确定性信号，难以对 Web 智能体的任务执行进行可靠的监控与风险预警。

Method: 提取 Macro 特征（跨步行为与环境反馈）和 Micro 特征（多次黑盒查询下的意图、动作与状态变化一致性）；提出一种定位“关键错误边界”的标注方法，将导致最终失败的首个不可逆错误点作为预警标记。

Result: 在 WebArena-Lite 和 Online Mind2Web 基准测试中，该方法表现与基于内部信号的基线相当；支持在固定的误切预算下进行早期干预，并具备跨网站类别的迁移能力。

Conclusion: 仅通过观测外部轨迹信号即可实现有效的智能体风险预测，为黑盒环境下 Web 智能体的可靠性监控提供了一种强有力的替代方案。
...
TL;DR: DocHop是一个用于评估多模态大模型在文档中进行多跳图表推理能力的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准多将图表与文档语境分开评估，缺乏对模型利用语境信息对图表证据进行选择、解释及跨图表聚合能力的考察。

Method: 构建了一个包含2,074个示例的基准测试集，采用逻辑优先的随机生成流水线，控制推理深度与视觉密度，涵盖六大任务类别。

Result: 实验显示目前主流模型（最高62.83%）与人类表现（90%+）存在巨大差距；推理增强型模型效果更好，但随任务复杂度增加表现明显下滑。

Conclusion: DocHop为多模态模型在复杂文档环境下的多跳逻辑推理能力提供了一个严苛且可控的测试平台。
...
TL;DR: MineTRACE 是一个基于 Web 的矿产勘查系统，旨在通过整合异构数据并利用可解释的专家树模型，提供透明、自然的勘查证据支持。


<details>
  <summary>Details</summary>
Motivation: 现有矿产勘查系统通常仅输出不透明的评分或热图，缺乏对勘查证据的可解释性和交互支持，且数据整合难度大。

Method: 构建了包含八种矿产的证据溯源系统，采用基于地质知识和已知矿床的专家树模型生成可解释评分，并集成自然语言对话助手以实现人机交互与证据查询。

Result: 该评分模型在不同测试场景下的空间 AUC 值最高可达 0.917，同时在端到端评估中验证了查询准确性和证据溯源的可靠性。

Conclusion: MineTRACE 提升了公共地球科学数据的可访问性与解释性，通过透明的证据支持机制，有效辅助矿产勘查工作。
...
TL;DR: 本文提出了ToolGate框架，通过自动化的三重过滤机制（执行验证、无工具筛选、工具代理测试）来生成高质量的科学计算基准测试题目。


<details>
  <summary>Details</summary>
Motivation: 传统科学基准测试的构建依赖专家手工编写，耗时且劳动密集，研究旨在利用语言模型降低构建成本，并确保题目的有效性与难度。

Method: 引入ToolGate系统：1. 执行验证，确保脚本可运行且结果一致；2. 无工具筛选，剔除仅凭模型推理即可回答的简单题目；3. 工具代理测试，确保题目可通过调用专业科学软件（FEniCSx）在限定时间内解出。

Result: 通过在FEniCSx上的实例化，从500个初始生成项中筛选出了128个高质量、具有挑战性的科学计算测试题目。

Conclusion: ToolGate成功将重复性的人工核验转化为可审计的自动化流程，在保持专家设计核心作用的同时，显著提高了构建科学基准测试的效率。
...
TL;DR: 提出了一种名为CHIME的自我演进记忆框架，通过属性归因机制解决长期任务规划中的信用分配偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有的自我演进记忆方法过度依赖最终任务结果，导致规划经验受到执行误差和环境因素的噪声干扰，从而出现信用分配偏差。

Method: CHIME架构通过将记忆拆分为规划库和执行库，采用“先归因、后记忆”原则，根据任务结果明确将经验归类至规划或执行库中，避免了噪声的累积。

Result: 在四个长期任务基准测试中表现优于SOTA方法；仅需少量样本即可实现高效记忆；实验显示规划记忆的价值高于执行记忆，且具备跨模型迁移能力。

Conclusion: 通过精细化的信用归因，CHIME能够有效过滤噪声，提升智能体在测试阶段的自主学习能力和长期任务规划性能。
...
TL;DR: 本文提出了SCOPED-Hiring，一个针对基于LLM的多智能体招聘系统（MAS）的流程感知型公平性诊断框架，用于揭示决策过程中隐藏的偏见。


<details>
  <summary>Details</summary>
Motivation: 现有的公平性审计多关注最终决策结果，往往忽略了决策过程中出现的动态风险和隐性歧视。

Method: 构建控制简历变体，运行基于角色的招聘委员会，记录超31.1万条决策轨迹，并通过六个诊断维度（最终结果、反事实、过程、路径、动态和设计效应）将轨迹转化为量化公平性指标。

Result: 研究发现，即便最终雇佣率平衡，系统中仍存在显著的轨迹不公平（如对职业空档的怀疑、对代理线索的依赖等）；通过诊断引导的修复可减少72.3%的负担，且仅微调雇佣率。

Conclusion: 仅关注最终结果会掩盖决策过程中的偏差，通过流程感知的诊断能够更有效地识别并修复多智能体系统中的深层公平性问题。
...
TL;DR: MASkills 提出了一种通过结构化“技能”而非传统记忆来实现多智能体系统持续学习的框架。


<details>
  <summary>Details</summary>
Motivation: 现有基于记忆的自省方法在调用、优化和扩展方面存在困难，缺乏可操作的结构化知识单元。

Method: 引入技能库架构，包含基于技能的信用分配、分层信用聚合及动量平滑优化，支持技能的演化（提炼、归纳、整合与修剪）。

Result: 在 HotpotQA、LoCoMo 和 GAIA 等任务中证明了该框架在提升多智能体性能方面的有效性。

Conclusion: MASkills 通过结构化技能学习实现了多智能体系统的持续自我优化，为复杂任务提供了更具操作性的知识表达方式。
...
TL;DR: READY是一个评估AI代理在企业环境中部署可行性的框架，重点关注可靠性、人类监督成本及整体经济效益，而非单纯的基准测试准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试侧重任务完成度，忽视了企业部署中对可靠性、监管负担和成本的严格要求，导致高性能模型在实际应用中不一定可靠。

Method: 引入READY框架，通过解耦工作流定义与执行，量化人机协作系统的可靠性与成本，并采用统计学方法在留出数据上进行资格验证，以寻找满足特定可靠性目标的最优监督策略。

Result: 通过临床审计案例研究发现，表面自主性能相近的系统，在达到相同可靠性目标时，其所需的人力监管成本可能存在显著差异。

Conclusion: READY框架将企业AI评估从“表现如何”转向“在何种监督条件下、以何种成本实现可靠部署”，为企业提供证据驱动的部署决策支持。
...
TL;DR: 该研究提出了一种利用语义信号辅助逆向物流决策的框架，通过分析退货备注来指导检查深度和资源分配，以提升回收价值并降低人工成本。


<details>
  <summary>Details</summary>
Motivation: 逆向物流中完全检查资产不仅消耗稀缺劳动力，且在获取资产状况前做出决策具有挑战性，需要一种更智能的方法来优化检查流程。

Method: 引入语义信号辅助决策支持系统，将退货备注转化为状况因子和信号质量评分；对比了基于结构化特征的检查方法及风险盲目策略，并在IT退役、飞机维修和电子产品回收三个场景中进行仿真评估。

Result: 该框架在所有测试场景中均降低了检查成本，并在飞机维修场景中显著提升了净回收价值（每批次增加5.39万美元），证明了叙述性证据在决策优化中的有效性。

Conclusion: 利用大型语言模型和文本提取技术处理叙述性信息，能够有效指导检查分配，从而在资源受限的情况下优化逆向物流的经济价值。
...
TL;DR: 本文提出了一种名为“持久发现上下文”（Persistent Discovery Context）的轻量级记忆层，旨在通过存储和重用数据智能体在任务中识别到的意图-对象映射，提升未来任务的检索效率与质量。


<details>
  <summary>Details</summary>
Motivation: 数据驱动型智能体在执行任务时通常会重复进行数据发现步骤，但过往成功识别的相关数据对象往往被丢弃而未被复用，导致效率低下。

Method: 引入了一个轻量级记忆层，用于存储过往任务中的意图到对象（intent-to-object）的映射关系，并将其作为辅助上下文用于增强后续任务的数据检索能力。

Result: 在三个结构化数据环境中，该方法在检索质量上均优于仅依赖元数据的搜索；即便在自动生成的记忆下依然有效，且在词汇稀疏的领域中，仅依赖记忆的检索效果甚至超过了元数据检索。

Conclusion: 数据发现的结果是数据驱动智能体的一种宝贵且可重用的上下文资源，持久化这些发现结果能显著优化代理的执行过程。
...
TL;DR: 该研究提出了一种名为 EmoStance 的框架，通过利用多标注者表情符号分布作为弱情感证据，控制对话的情感倾向，从而提升大型语言模型（LLM）的共情回复生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的共情回复生成往往难以准确把握对话的情感基调，缺乏对回复端情感取向的精确控制。

Method: 构建了 EmojiDialogue 数据集；设计了 EmoStance 框架，通过建模源端情感表达并利用连续前缀嵌入（continuous prefix embeddings）引导冻结的指令微调大模型。

Result: 在盲测中获得了 62.2% 的胜率，在语境特异性和感知响应度方面表现显著，且能与外部知识增强方法互补。

Conclusion: 利用弱监督的表情符号分布作为情感控制信号，能够有效提升对话系统的共情水平和交互质量。
...
### [29] [Examining the Vulnerability of Multi-Agent Medical Systems to Human Interventions for Clinical Reasoning](https://arxiv.org/abs/2609.02191)
...
TL;DR: 该研究探讨了人类在医疗多智能体系统“故障点”进行干预如何影响诊断准确性及逻辑一致性。


<details>
  <summary>Details</summary>
Motivation: 多智能体医疗系统在复杂临床决策中易受外部影响，需要明确人类干预在何种程度上能提升诊断准确性并规避认知偏差。

Method: 基于MedQA数据集模拟医患对话，识别智能体推理脆弱的“故障点”，通过引入不同类型的干预措施，测量诊断准确性、诊断漂移和不确定性的变化。

Result: 正确的干预措施可将诊断准确率提升高达40%，而不当干预会导致准确率下降约6%，并增加诊断偏差与不确定性。

Conclusion: 识别并精准引导多智能体系统的“故障点”是提升医疗诊断鲁棒性的关键，且模拟环境中的智能体行为与真实临床认知偏差表现出高度相似性。
...
### [30] [FUSE: An Evaluating Framework for Dangerous Capabilities of LLMs](https://arxiv.org/abs/2609.02168)
*Zhengyi Jin,Ru Zhang,Xiao Chen,Xinbo Liu,Jiaxuan Lin,Jia Huang,Jianyi Liu,Zhen Yang*

Main category: cs.AI

TL;DR: 该论文提出了一个模块化框架，通过知识（K）、防御（D）和危害（H）三个维度对AI模型的危险能力进行标准化评估。


<details>
  <summary>Details</summary>
Motivation: 现有的AI安全评估方法碎片化，难以有效治理危险的AI能力，缺乏统一的评估标准。

Method: 构建了一个包含K、D、H三个正交流水线的评估框架，使用可插拔模块提供场景和判据，并对12个商业大模型进行了化工生物（CB）领域的实证评估与时序演化分析。

Result: 研究发现不同模型家族在危险能力表现上差异显著；模型知识水平的提升并未与防御能力的提升保持同步，说明规模化和对齐并不等同于安全性。

Conclusion: 该框架通过量化剖面图揭示了AI危险能力的非单调演化趋势，为更系统、更具可比性的AI安全评估提供了有效工具。
...
TL;DR: 该论文提出了一种名为“ASCII攻击”的新型对抗性攻击方法，通过将有害请求嵌入ASCII艺术中，绕过了大语言模型的安全对齐机制。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐主要关注显式文本，而对于通过重构（如ASCII艺术）呈现的相同恶意内容，模型往往防御不足。

Method: 利用单轮、黑盒攻击，将恶意请求编码为ASCII艺术并伪装成艺术品评价请求，对比直接询问与ASCII艺术包装请求在模型响应中的安全性差异。

Result: 实验显示，ASCII攻击在11个模型和8个有害主题中表现出极高成功率（平均62% vs 对照组42%，最高达93%），且效果不随模型规模增加而减弱。

Conclusion: 目前的模型安全对齐存在“失配泛化”问题，仅关注表面形式的对齐无法有效抵御通过重构手段实施的提示攻击。
...
TL;DR: PEARL 是一种用于归纳知识图谱补全（IKGC）的框架，通过将路径建模为上下文相关推理信号，显著提升了处理未见实体的预测能力。


<details>
  <summary>Details</summary>
Motivation: 现有的 IKGC 方法通常独立处理关系路径，忽略了路径的预测相关性会随周围结构环境变化而改变的问题。

Method: 构建查询特定的上下文子图，引入 LLM 引导的路径检索器，通过二部交互图融合路径与上下文实体特征，并采用双视图对比学习来增强鲁棒性。

Result: 在 WN18RR、FB15k-237 和 NELL-995 三个基准数据集上，PEARL 在 Hits@10 指标上均优于现有的 IKGC 方法。

Conclusion: 上下文子图建模、语义路径检索以及路径与实体的交互机制对于解决归纳知识图谱补全任务至关重要，能有效提升模型的推理质量。
...
TL;DR: 提出了SkillGLoW框架，通过将LLM代理的任务经验转化为抽象的“程序化族”，实现了高效的技能复用与自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，全局文档方案易导致信息泛化失效，而任务池方案则因包含过多实例细节导致冗余且难以泛化，缺乏针对不同任务间共享求解程序的有效管理机制。

Method: 引入SkillGLoW架构，将局部任务执行经验聚类为程序化族，并压缩为去实例化的全局先验；通过提交门控机制筛选有效先验，并利用局部再生机制处理实例细节。

Result: 在数学推理、终端自动化、软件修复和具身控制等四个基准测试中，平均提升17.2个百分点，库规模比任务池缩减至原来的1/3.6，且在未见过的ALFWorld任务上表现优异。

Conclusion: 研究证明了跨任务迁移的核心在于“求解程序”而非“任务记忆”，SkillGLoW在保证库紧凑性的同时显著提升了智能体在长视距任务中的持续学习能力。
...
TL;DR: PhoenixNest-Video 是一套基于证据的多模态 AI 代理框架，用于实现自动化的视频面试评估。


<details>
  <summary>Details</summary>
Motivation: 解决当前面试评估中因申请人数量激增导致的评估成本高、一致性差，以及现有 AI 方法缺乏可追溯评分依据（黑盒）的问题。

Method: 构建语义视频图作为工作记忆，结合基于准则的检索与多模态验证；采用基于准则的强化学习（Rubrics-based RL）训练评分器，通过双重奖励机制优化准则对齐与评分区分度。

Result: 在 VInterview-2025 数据集上达到了 91.50% 的等级准确率，性能超过了许多更大规模的闭源模型，且能为每个评分提供明确的证据支持。

Conclusion: 紧凑的、基于准则的 AI 代理在面试评估中比直接提示大规模模型更能与专家评审达成共识，并具备良好的可解释性，便于人类复核。
...
TL;DR: 提出了一种名为PGPO的强化学习方法，通过估算状态势能实现多轮智能体任务中更细粒度的信用分配。


<details>
  <summary>Details</summary>
Motivation: 现有的组基强化学习在多轮任务中对失败轨迹的信用分配较差，无法有效区分轨迹内的有效行为与错误行为。

Method: 利用锚定状态组的回报统计估算经验状态势能，并通过相邻状态间的势能差导出动作优势，实现跨轨迹的信用传播。

Result: 在ALFWorld和WebShop数据集上表现优于现有的组基RL方法，并能为失败轨迹提供更具信息量的信用信号。

Conclusion: PGPO通过状态势能引导策略优化，在不显著增加训练开销的情况下，有效提升了多轮智能体任务中细粒度信用分配的质量。
...
TL;DR: 该研究引入了“ProSE”框架，旨在解决AI助手在用户因有限理性导致评估能力受限时，如何平衡方案质量、偏好推理与评估成本的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的AI辅助方法往往假设用户能可靠评估任何建议，但现实中用户存在“有限理性”，难以评估复杂或不透明的方案，导致辅助效果不佳。

Method: 将该问题建模为隐藏参数的序列辅助问题（ProSE），通过KL正则化的有限理性二元响应模型，量化评估成本对用户接受度的影响；并提出ProSE-Plan，一种深度为2的贝叶斯自适应规划器，旨在平衡“高接受度”方案与“信息性探测”方案。

Result: 实验证明，当评估成本成为瓶颈时，ProSE-Plan在图模拟任务中显著优于传统的仅追求高接受度或短视的基线模型。

Conclusion: 用户的“可评估性”（evaluability）是AI辅助中的关键规划维度，AI助手不仅应提供高质量方案，还应通过主动探测来学习用户的评估约束，从而提升协作效率。
...
TL;DR: 提出了一种名为Prior-Guided Tuning (PGT)的训练方法，通过将自然语言先验作为辅助学习信号，显著提升了大语言模型在低资源环境下的任务表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在低资源且数据模糊的情境下表现不佳，且以往将自然语言先验仅作为上下文处理，未能将其充分利用为训练过程中的学习信号。

Method: 引入Contrastive Prior Steering (CPS)，在保持原有监督学习目标的同时，增加基于先验的对比性辅助损失，通过正负样本引导模型学习任务一致性并排斥误导性信息。

Result: 在AmbiMath、Jigsaw和MNLI/HANS等数据集上表现优异，尤其是在低数据资源条件下，模型性能显著超越了传统的微调和提示词微调方法。

Conclusion: 任务级自然语言先验能够有效作为低资源训练的辅助信号，通过对比学习机制能显著增强模型在复杂或模棱两可任务中的稳健性与准确性。
...
TL;DR: 该论文提出了PROCTOR框架，旨在解决自动提示词优化过程中LLM作为“判官（Judge）”导致的偏见、奖励黑客及评估不可靠问题。


<details>
  <summary>Details</summary>
Motivation: 现有的自我改进代理管道过度依赖LLM判官，但判官常表现出偏见、奖励黑客行为及评估失败，导致优化结果不可信。

Method: 提出了PROCTOR框架：采用“教师-学生”架构，将LLM从“权威判官”降级为“顾问”，引入五个确定性守卫（deterministic guardrails），包括沙箱环境、职责分离、硬性接受检查、冻结保持集及反作弊金丝雀案例。

Result: 实验发现，单纯优化判官的评价准则效果有限；引入确定性约束后，成功规避了缓存作弊、评估信号污染及解析器回退等典型的自动化故障。

Conclusion: LLM不应作为优化的最终权威。通过将LLM置于确定性验证层之下，能够显著提升自动提示词优化管道的鲁棒性与真实性能。
...
TL;DR: APEx是一种用于深度研究代理的层级化经验利用框架，通过将交互历史组织为实例级轨迹记忆和类别级程序技能，实现了代理的持续自我进化。


<details>
  <summary>Details</summary>
Motivation: 现有方法在利用先验经验时存在检索信息冗余或技能学习与决策策略脱节的问题，限制了代理在复杂长程任务中的表现。

Method: 提出了包含执行器、蒸馏器和规划器的闭环架构，采用三阶段交替的GRPO训练范式，并在测试时通过技能引导的强化学习实现无监督的在线策略自适应。

Result: 在7个基准测试中达到SOTA水平，较GPT-5.4提升了14.7个百分点，较现有的记忆增强基线提升了3.0个百分点。

Conclusion: 该框架通过有效的经验分层与奖励引导的技能蒸馏，显著增强了AI代理在长程任务中的自主推理与适应能力。
...
TL;DR: 提出了一种名为Codebook Agent的新方法，通过向量量化自编码器压缩通信拓扑，实现了LLM多智能体系统的高效、动态拓扑生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法将拓扑生成视为条件图生成任务，存在搜索空间冗余、启发式目标（如边数）与实际推理开销不匹配以及模型对同构代理分辨力不足的问题。

Method: 使用向量量化自编码器（VQ-VAE）将成功拓扑压缩为16项码本，通过奖励加权的MLP将查询映射到码本分布，并利用简单的MLP代理对解码候选进行快速重排序。

Result: 在六个基准测试中表现最优（平均准确率84.6%），拓扑生成仅需2.4ms，并减少了21.9%至33.2%的Token消耗。

Conclusion: 通过解耦拓扑生成与评估过程，Codebook Agent在实现高性能的同时显著降低了计算成本，证明了查询无关码本在多智能体协同中的有效性。
...
TL;DR: CoMerge 提出了一种基于冲突驱动偏好优化的模型合并框架，通过将合并缺陷转化为负样本来优化权重系数，从而有效解决多任务模型合并中的参数冲突问题。


<details>
  <summary>Details</summary>
Motivation: 现有的模型合并方法难以有效解决模型间的参数干扰问题，且缺乏直接从合并后的性能退化中学习的机制。

Method: 将模型合并重构为偏好优化问题，利用原生合并（如任务算术）产生的缺陷构造自监督偏好样本对，并通过优化轻量级的张量级合并系数来缓解冲突。

Result: 在 MergeBench 上取得了 0.9968 的平均归一化性能，优于现有所有数据驱动和无数据合并基准，并在指令遵循和安全性方面显著提升。

Conclusion: CoMerge 证明了通过偏好优化微调少量参数即可在不进行全参数微调的情况下，有效解决模型合并的冲突并保持高任务性能。
...
TL;DR: 本文提出了一种名为Router的轻量级路由模型，旨在根据任务需求自动选择最合适的LLM，以平衡速度、成本和质量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在性能、价格和功能上存在高度异质性，人工启发式规则难以有效实现多维度指标的最佳权衡。

Method: 基于0.6B参数的Qwen3解码器构建GLiClass架构，通过双向评分器评估任务，无需自回归生成即可进行路由决策，并结合构建的包含115种任务类型的本体论进行训练。

Result: 在LiveBench测试中，该路由模型性能超过了平均候选模型，并在1000个任务的子集中达到0.707的top-1得分，优于最强单一模型。

Conclusion: 该研究证明了轻量级路由模型在动态任务分发中具有显著优势，能有效提升LLM系统的整体效能与性价比。
...
TL;DR: 该研究提出了两种使对齐评估更难被模型察觉的技术（Critique Refinement 和 DISH），旨在缩小模拟评估与真实部署环境的差距，提高评估的真实性。


<details>
  <summary>Details</summary>
Motivation: 目前的对齐评估存在“评估意识”问题，模型能分辨出是测试还是真实部署，导致安全评估结果不可靠。

Method: 1. 批判性优化（Critique Refinement）：利用推理时计算，通过目标模型反馈优化模拟动作；2. DISH（模拟部署代理框架）：为编码任务中的目标模型构建更接近真实部署的代理环境。

Result: 实验证明两种技术具有互补性，结合使用能显著提高评估的逼真度，且比简单增加评估长度更有效地利用了计算资源。

Conclusion: 自动化方法能显著提升对齐评估的真实性，通过优化环境和推理过程，可以克服模型评估感知带来的安全性评估偏差。
...
TL;DR: 提出了一种名为SALA的语义感知逻辑对齐框架，用于优化复杂推理任务中的上下文学习（ICL）示范选择。


<details>
  <summary>Details</summary>
Motivation: 传统的基于表面相似度的检索无法捕捉逻辑，而现有的基于逻辑规则的方法又过于僵化，无法处理多样化的推理过程。

Method: 自动学习任务特定的推理操作，将其嵌入到连续语义空间，并利用动态时间规整（DTW）实现推理序列的软匹配。

Result: 在四个推理基准测试和三个大模型上的实验表明，SALA在示范选择效果上优于现有方法。

Conclusion: SALA通过灵活的逻辑对齐机制实现了高效且可解释的示范选择，提升了LLM在复杂推理任务中的表现。
...
TL;DR: 本文提出了AGENTSCOPE，一种用于诊断大语言模型（LLM）智能体失败的神经符号方法。


<details>
  <summary>Details</summary>
Motivation: 手动排查复杂的智能体轨迹效率低下，且现有的软件调试技术和纯LLM诊断方法在准确性和可靠性上存在局限。

Method: 将智能体行为抽象为结构化表示，引入“神经不变性”（neural invariants）规范行为属性，并结合LLM推理对失败步骤和类型进行定位。

Result: 在Who&When及自建的AgentErrata数据集上，AGENTSCOPE在故障定位和归因准确性方面显著优于当前主流方法。

Conclusion: 结合结构化抽象与LLM引导的推理，能够实现对智能体失败的高效、可靠且可解释的诊断。
...
TL;DR: 该论文为定量双极论证框架（QBAFs）引入了对比解释方法，旨在通过比较两个不同论点的论证结果来增强可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的QBAF解释方法多关注单个论点，缺乏对两个论点差异的对比分析，难以满足复杂场景下对比性决策的需求。

Method: 定义了对比归因函数（CAFs）及其应满足的一般属性，并提出了基于移除法（removal）、梯度法（gradients）和Shapley值的三种具体CAF实现方式。

Result: 通过理论分析论证了所提CAFs满足特定属性，并通过医疗决策和偏差识别两个实际应用场景展示了其有效性。

Conclusion: 对比解释为QBAFs提供了一种更深度的分析视角，能够有效揭示论证过程中的差异，提升AI模型在关键领域的可解释性。
...
### [47] [CivBench: A Long-Horizon Benchmark for Tool-Mediated Agents in Civilization VI](https://arxiv.org/abs/2609.02459)
*Austin Tudor David Andrews,Liam Wilkinson,Jamie Heagerty,Harry Coppock,Jakob Nicolaus Foerster,Rui Ponte Costa*

Main category: cs.AI

TL;DR: CivBench是一个基于Model Context Protocol (MCP) 的开源基准测试，旨在评估AI智能体在长序列、工具辅助环境下的长期规划与执行能力。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方法难以衡量智能体在复杂、长视界（300+回合）及部分可观测环境下的持续战略监控与规划执行能力。

Method: 引入了CivBench基准，利用76个MCP工具提供环境交互，并定义了主动监控率（PMR）和规划执行率（RAG@10）两个指标来量化评估智能体行为。

Result: 实验发现模型普遍存在战略监控不足（未按建议查询关键状态）以及规划与执行不一致（RAG@10在48.2%-65.8%之间）的问题。

Conclusion: 智能体在处理长期任务时，即使拥有工具和明确指导，仍会出现明显的指令执行偏离，这反映了模型在维持复杂长期行为一致性方面的局限性。
...
TL;DR: UTP-Bench是一个针对旅行规划领域不确定性的基准测试，旨在评估大型语言模型在面对交通延误和人流波动时的行程鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划基准（如TravelPlanner）多假设环境为确定性，忽视了现实中不可避免的随机干扰，导致生成的行程在实际应用中脆弱且难以执行。

Method: 构建了基于印度504个城市真实数据的UTP-Bench数据集，并引入了经验延误分布和人流密度模型；提出了BAS、CATS和TDAS三个评估指标来量化行程的鲁棒性。

Result: 对GPT-4、Qwen等顶尖LLMs的实验表明，当前模型在时间缓冲、延误处理和人群敏感规划方面与人类水平存在显著差距。

Conclusion: 该研究填补了不确定性感知旅行规划的基准空白，强调了开发具备抗干扰能力和时间鲁棒性的智能规划模型的重要性。
...
### [49] [Loom: Weaving Diagnostic Strands into Free-Text Consensus via Embedding-Space Reweighting](https://arxiv.org/abs/2609.02649)
*Ron Begleiter,Katya Egert Berg,Gilad Saban,Gil Shabat*

Main category: cs.AI

TL;DR: Loom 是一种用于根因分析（RCA）的生成式共识框架，通过将多种启发式生成的假设投影到嵌入空间并进行加权，从而在保持高精度的同时显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在工业级场景中存在的上下文限制、幻觉叠加及推理延迟过高的问题，同时克服传统弱监督方法仅限离散分类的局限性。

Method: 利用模块化启发式算法生成假设，将其映射至连续嵌入空间，并通过迭代质心重加权算法解决冲突，最终由轻量级 LLM 进行合成。

Result: 在 OpenRCA 基准测试中，Loom 实现了与顶尖自主智能体相当的准确度，且推理速度快约 26 倍至 33 倍。

Conclusion: 确定性共识机制不仅提升了推理性能，还增强了领域专家（SMEs）对自动化决策的信任，平衡了智能体深度与计算延迟之间的权衡。
...
### [50] [Door-in-the-Face Requests and Refusal Behaviour in Large Language Models](https://arxiv.org/abs/2609.02707)
*Til Jordan*

Main category: cs.AI

TL;DR: 研究探讨了“以退为进”（Door-in-the-face）这一心理学技巧在大型语言模型上的适用性及其有效性。


<details>
  <summary>Details</summary>
Motivation: 验证人类常用的社会影响力技巧是否能够通过操纵请求方式提升语言模型的顺从性，并探究不同模型架构下的差异。

Method: 测试九种生产级模型，通过先提出一个会被拒绝的大请求，再提出一个更小的后续请求，对比其与直接提问的顺从率差异，并引入无关话题控制组进行分析。

Result: 该技巧对Anthropic模型有效，顺从率提升显著；但在OpenAI、Google及部分Anthropic模型上反而适得其反，导致顺从率下降；同时发现请求内容的性质（如指令 vs 解释）是决定技巧成功与否的关键因素。

Conclusion: 人类心理影响力技巧在语言模型上的迁移具有高度的“模型特异性”，其有效性不仅取决于技巧本身，还深受模型内部机制及请求任务类型的限制。
...
### [51] [Repo-To-Skill: Distilling GitHub Repositories Into AI4AI Skills](https://arxiv.org/abs/2609.02749)
...
TL;DR: DisCo 是一项通过引入“操作知识（operational knowledge）”来提升自主智能体 ML 研究能力的框架，通过将非结构化的领域知识蒸馏为可复用的技能库，显著提升了智能体的研究性能。


<details>
  <summary>Details</summary>
Motivation: 目前的自主研究智能体缺乏“操作知识”，即如何将方法成功落地运行的实践经验，而这些知识散落在庞大的代码库和论文中，难以在任务中实时利用。

Method: 提出了 DisCo 框架，包含任务无关（从开源仓库蒸馏）和任务导向（根据具体任务生成）两种蒸馏方式，并构建了包含 5000+ 技能的 AREX-Skill 库。

Result: 在固定模型骨干（GPT-5.5）和执行预算的前提下，使用 DisCo 的智能体在 MLE-bench、PaperBench 等多个基准测试中表现显著提升，最高提升幅度达 134.3%。

Conclusion: 将零散的领域知识蒸馏为紧凑、可复用的技能是提升自主智能体解决复杂研究任务效能的关键路径。
...
### [52] [Collective creativity in hybrid societies](https://arxiv.org/abs/2609.02620)
*Mason Youngblood,Katie Mudd,Manuel Anglada-Tort,Cameron Jones,Elena Miu,Diana Omigie,Margaret Schedel*

Main category: cs.AI

TL;DR: 该研究探讨了生成式AI对创造力的双重影响，提出了从“个体新颖性”与“群体多样性”两个维度理解创造力，并强调了混合集体在优化创造力输出中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 学术界对于生成式AI是促进还是损害创造力存在争议，研究者认为这种分歧源于对创造力组成部分的混淆，旨在通过重新架构创造力概念来解决这一争论。

Method: 通过概念分析与理论建模，将创造力解构为个体维度的“新颖性”与群体维度的“多样性”，分析人机协作模式对这两者的不同影响。

Result: 研究发现AI能显著提高单体输出的新颖性，但往往会压缩整体的多样性；然而，通过合理的集体组合与人机互补，可以实现个体收益与集体多样性的共存。

Conclusion: 创造力的关键在于协作系统的构成（agents的比例与连接方式），而非单一的AI介入与否；未来的重心应转向设计能平衡个体创造力和群体多样性的协作结构。
...
TL;DR: 该研究通过博弈论和随机过程框架对多智能体LLM协作系统进行了数学建模，提出了Stochastic Reflective Memory Ascent (SRMA)算法以优化记忆更新，并证明了环境落地（grounded）反馈在提升系统表现中的必要性。


<details>
  <summary>Details</summary>
Motivation: 现有基于编排器-执行者的多智能体系统缺乏统一的协调与反射理论，且缺乏对外部验证作用的数学解释。

Method: 将协作建模为双层博弈，将反射建模为语义记忆状态上的随机移动；证明了仅观察文本无法统一改进，进而提出SRMA算法，利用环境接地评估（grounded evaluation）来优化记忆更新。

Result: 证明了SRMA在满足特定条件下可实现精确、几何级或多项式级收敛，并在SWE-bench测试集上达到了72.2%的解决率。

Conclusion: 环境接地（grounded）反馈是多智能体系统自我提升的核心，SRMA框架为高效的多智能体协作与自我修正提供了理论保障和性能提升。
...
TL;DR: 该研究提出了一种针对工厂环境的轻量化检索增强生成（RAG）部署方案，通过结构化压缩和检索接地蒸馏，使模型在极低资源限制下保持高质量回答。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型难以部署在资源受限的工厂边缘设备上，且传统模型缩放法则在检索增强场景下不再适用。

Method: 采用权重共享超网络（supernetwork）进行“三明治”式蒸馏训练，并根据设备内存预算和算力阈值进行后适配选择，从而在保证回答质量的前提下优化模型性能。

Result: 通过检索接地蒸馏，模型恢复了大部分因压缩导致的性能损失（仅损失4.6%），并能在不同算力等级的边缘设备上以1.3至5瓦的功耗稳定运行。

Conclusion: 模型大小不再是衡量RAG任务性能的唯一指标；通过特定的压缩与蒸馏策略，可以在算力受限的工业场景下实现高性能的对话式知识检索。
...
TL;DR: SafeEvolve 是一种用于大模型智能体安全对齐的经验驱动自演化框架，通过策略与运行环境（harness）的协同进化提升安全性能。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法往往孤立地处理外部约束或策略优化，难以在多步执行轨迹中有效平衡智能体的运行时控制与内在安全性。

Method: 提出基于经验的协同进化循环：在 harness 侧，将轨迹级安全经验转化为可审计、可逆的组件级约束；在策略侧，采用 SFT-RL 两阶段训练，引导模型主动利用安全组件并进行强化学习以优化自主安全行为。

Result: 在 AgentDojo 等基准测试中表现出色，以 Qwen3.5-4B 模型为例，实现了 3 倍的攻击成功率（ASR）下降，同时将良性任务效用从 59.79% 提升至 61.86%。

Conclusion: SafeEvolve 有效地将安全经验转化为可演化的运行时保障与增强的策略行为，在智能体安全与效用之间取得了更优的平衡。
...
TL;DR: 本文提出了一种针对5G及未来网络根因分析（RCA）的结构化推理框架，旨在解决大语言模型在电信领域应用中存在的幻觉和推理不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现代电信网络（5G/6G）复杂性高，传统规则或机器学习方法难以应对；而原生大语言模型在电信领域诊断时存在幻觉、推理不稳定及缺乏结构化证据对齐的问题。

Method: 提出一种结构化推理框架：首先将异构网络遥测数据转化为规范化上下文，随后在诊断过程中强制执行决策路径推理，最后生成基于证据的故障解释。

Result: 在TeleLogs和TelecomTS两个5G数据集上的实验表明，该框架在诊断准确性和决策一致性方面均优于现有基准方法。

Conclusion: 结构化推理设计对于构建实用、可靠的电信网络大模型根因分析系统至关重要。
...
### [57] [Discriminative World Models for Web Agents](https://arxiv.org/abs/2609.02885)
...
TL;DR: 该论文提出了一种新的世界模型训练目标“预测状态匹配（predicted-state matching）”，旨在通过区分不同动作产生的状态来提升网页智能体在测试时的动作规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有的世界模型主要通过监督式预测来训练，但这种方式与下游的排序模型目标不一致，难以确保生成的状态在候选动作间具有足够的辨别力。

Method: 引入预测状态匹配训练目标，要求模型学习到的状态表示能够从多种候选动作及其后果中准确识别出真实结果；并基于WebArena Go-Browse数据构建了包含多种动作分支的数据集进行训练。

Result: 实验表明，该方法在预测状态匹配基准上优于传统的监督预测模型；在WebPRMBench上提升了动作排序性能，并在WebArena-Lite测试中显著提高了端到端任务成功率。

Conclusion: 通过优化预测状态的区分度，能够有效增强网页智能体在动态环境下进行动作选取的决策能力。
...
### [58] [AI Contextual Measurement for Recovering Individual and Group-Level Effects: Validation Against Survey Measures and an Occupational Application](https://arxiv.org/abs/2609.02821)
*Wenxin Jiang,Xuyang Wang,Yuxiao Wu*

Main category: cs.AI

TL;DR: 提出了一种名为AICOME的评估框架，旨在评估基于AI生成的个体测量数据在处理多层上下文模型（Contextual Models）时的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究人员越来越多地使用AI从非结构化数据中提取社会学特征，但缺乏系统性评估这些AI测量指标在区分组内（within-group）和组间（between-group）效应时的准确性和边界条件。

Method: 利用2022年中国家庭追踪调查（CFPS）数据，通过对比调查测量与AI测量在响应层、模型层及上下文层面的表现，评估其对个体和群体效应的恢复能力。

Result: 研究表明，当拥有充足的个体和职业特征时，AI上下文测量能有效恢复与调查变量相当的上下文模型信息；但在信息受限或多个相关概念缺失时，性能会显著下降。

Conclusion: AICOME框架证明了AI测量可以超越简单的响应预测，用于严谨的社会科学分析，但需谨慎处理数据冗余与信息边界限制。
...
### [59] [Beyond Textual Chain-of-Thought: A Survey on Action-Grounded Reasoning in Autonomous Driving](https://arxiv.org/abs/2609.01659)
...
TL;DR: 本文对自动驾驶领域中的思维链（CoT）推理进行了综述，探讨了从文本推理向动作导向推理的范式转变。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶输出为连续动作，需要具备与物理世界一致的时空结构的推理，现有文本类CoT难以完全满足其需求。

Method: 通过调查171篇文献，构建了以中间状态表征为核心的分类体系，将方法归纳为语言类、视觉空间类、潜空间动态类和外化推理四种类型。

Result: 系统梳理了130种推理方法和41个相关资源，揭示了自动驾驶推理在复杂场景下的不同技术路径及优劣。

Conclusion: 未来的研究重点应在于开发能与现实世界对齐、支持实时动作控制并具备安全验证能力的中间表征推理技术。
...
### [60] [FORGE: Forward-Only Test-Time Adaptation for Integer-Only Vision Models on Microcontrollers](https://arxiv.org/abs/2609.01683)
*Muhammad Rehan,Haider Ali,Muhammad Ali Munir,Moaz Amjad*

Main category: cs.CV

TL;DR: 提出了一种针对微控制器（MCU）部署的量化卷积神经网络的测试时自适应（TTA）方法，通过重归一化折叠卷积层的输出，在不依赖BN层和反向传播的情况下实现分布偏移下的精度恢复。


<details>
  <summary>Details</summary>
Motivation: 微控制器上的量化模型因缺乏反向传播能力且BN层在部署时被融合，导致传统的TTA方法无法使用，无法应对现实场景中的分布偏移（如光照、噪声）。

Method: 通过对折叠卷积层的输出进行逐通道重归一化，利用前向传播估计恢复训练时的统计数据，并结合批次规模缩放的动量机制实现单样本流式自适应。

Result: 该方法在保持int8量化精度的同时，恢复了大部分基于梯度的TENT方法带来的精度增益，仅需调整极少数层，且在ESP32-S3微控制器上实现了极低能耗和延迟的部署。

Conclusion: 本研究证明了在资源受限的微控制器上实现廉价且高效的、基于前向计算的测试时自适应是完全可行的，弥补了边缘设备部署量化模型后的适应性空白。
...
### [61] [FairLens: Benchmarking Fairness in Vision-Language Models for High-Stakes Decision-Making](https://arxiv.org/abs/2609.01691)
*Vahid Reza Khazaie,Ahmed Y. Radwan,Shaina Raza*

Main category: cs.CV

TL;DR: FAIRLENS是一个评估视觉语言模型（VLM）在高风险领域（招聘、法律、医疗）中的公平性与有效性的基准框架。


<details>
  <summary>Details</summary>
Motivation: VLM在视觉决策中日益普及，但缺乏针对其在高风险领域中基于外貌进行无根据推断的公平性与有效性评估方法。

Method: 构建包含10万+个图像-问题对的测试集，从人口统计学均等、稳健性（Soundness）、人口统计学关联及文本生成偏见四个维度评估八个主流VLM。

Result: 主要发现模型普遍存在“无根据推断”问题（而非简单的歧视），在证据不足时未能拒绝回答；即便人口统计学差异指标看似很小，模型在不同群体间的安全性表现仍存在显著差距。

Conclusion: 实现高风险领域AI公平性的关键在于不仅要保证群体间的一致性，更要强制模型在缺乏有效证据时拒绝从外貌中推断高风险属性。
...
### [62] [From Visual Cues to Spoken Narration: Rethinking Audio Description](https://arxiv.org/abs/2609.01725)
*Akshita Gupta,Aditya Arora,Federico Tombari,Marcus Rohrbach,Anna Rohrbach*

Main category: cs.CV

TL;DR: Cue2Narrate 是一个针对长视频进行自动音频描述（AD）生成的两阶段框架，旨在解决“叙述什么”和“何时叙述”的问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究多将音频描述简化为短视频片段的字幕生成，忽略了长视频中对叙述时机（when）的选择以及叙述内容（what）的联合优化。

Method: 提出了一个双阶段流水线：首先利用双头音视频定位器预测视觉线索窗口和叙述窗口；随后通过 LoRA 微调的视觉语言模型（VLM）结合描述排名损失（Description Ranking Loss）生成高质量的音频描述。

Result: 在新建的 LongLSMDC 基准测试中，Cue2Narrate 在平均 mAP 上较基线提升了 5-12 个点，并在生成质量上优于传统的微调 VLM。

Conclusion: 该研究成功定义并基准化了长视频多片段音频描述任务，证明了联合预测叙述时机与内容对于提升视觉障碍用户体验的有效性。
...
### [63] [UAV Thermal Imagery for Inert Ordnance Screening: Multi Campaign Dataset Development,Object Detection, and Practical Recommendations](https://arxiv.org/abs/2609.01738)
*Chad Melton,PhD.,Annabelle Kelton*

Main category: cs.CV

TL;DR: 本研究构建了一个多场景无人机（UAV）热成像数据集，并评估了YOLOV11l和RT-DETR-R50模型在未爆弹药（UXO）检测中的应用效果。


<details>
  <summary>Details</summary>
Motivation: 未爆弹药严重阻碍了全球平民活动和基础设施重建，需要自动化技术手段提升人道主义排雷效率。

Method: 通过四次实地考察，在不同季节、地貌和飞行高度下采集热成像数据，整理出5855对图像标签，并对比训练了YOLOV11l和RT-DETR-R50检测算法。

Result: 成功建立了包含多种地表类型的热成像数据集，并验证了深度学习模型在辅助筛选未爆弹药方面的可行性。

Conclusion: 热成像无人机技术可作为排雷工作的辅助筛选工具，应结合RGB图像、特定气象条件及人工复核，以实现最优的排雷评估效果。
...
### [64] [ZipTok3D: High-Fidelity 3D Tokenization with Compact Token Prefixes](https://arxiv.org/abs/2609.01740)
*Mingda Lin,Weijie Wang,Zeyu Zhang,Bowen Cui,Yefei He,Haoyu Zhao,Yuanyu He,Donny Y. Chen,Feng Chen,Bohan Zhuang*

Main category: cs.CV

TL;DR: ZipTok3D 是一种新型 3D 分词器，通过分层嵌套的全局 Token 表示和迭代式解码，实现了在极低 Token 预算下的高保真 3D 重建。


<details>
  <summary>Details</summary>
Motivation: 现有的 3D 分词器在压缩至极低 Token 预算时，会导致严重的几何重构质量下降，无法平衡压缩率与保真度。

Method: 引入嵌套 Dropout 训练策略，使模型在保留部分序列时仍能重构完整对象；采用共享参数的 Transformer 模块进行迭代式解码，无需额外的生成采样阶段。

Result: 在同等 Token 维度下，ZipTok3D 仅需 1-4 个 Token 即可达到现有基线（如 COD-VAE）使用 32 个 Token 的重构质量，最高实现了 32 倍的序列压缩。

Conclusion: 通过优先化前置 Token 中的几何信息并利用迭代解码机制，ZipTok3D 有效解决了 3D 数据在极高压缩比下的重建质量难题。
...
### [65] [SCULPT: Training Edge Vision Models for Post-Training Quantization Readiness](https://arxiv.org/abs/2609.01743)
*Bharadwaj Kavuri,Sourav Babu-PK,Varadhraj Ellapan,Pullarao Maddu,Prasad Deshpande*

Main category: cs.CV

TL;DR: SCULPT 是一种在 FP32 微调阶段通过正则化和参数学习提升模型量化友好度的方法，旨在解决低比特后训练量化（PTQ）中的激活值离群点难题。


<details>
  <summary>Details</summary>
Motivation: FP32 模型中常见的重尾激活分布导致 PTQ 过程中量化精度下降，而现有方案要么训练复杂度高（QAT），要么需要复杂的后期修正（PTQ）。

Method: 引入一种拓扑感知激活正则化器以抑制偏度和峰度，并结合基于百分位数的学习型剪枝机制，在普通 FP32 微调阶段优化激活值分布。

Result: SCULPT 无需在训练中模拟量化，且不需要复杂的后期转换，生成的剪枝边界可直接导出用于 INT8 或 W4A8 等低比特量化场景。

Conclusion: SCULPT 为 PTQ 提供了一种高效的预处理手段，在保持训练简便性的同时显著提升了模型在资源受限硬件上的部署性能。
...
### [66] [Swin Meets EfficientNet: Lightweight Architectures for GAN-Based Face Forensics](https://arxiv.org/abs/2609.01749)
*Sejuti Basu,Ashima Sood,Vijay Kumar,Sahil Sharma*

Main category: cs.CV

TL;DR: 本文提出了一种结合EfficientNet-B0卷积特征与Swin Transformer架构的混合模型，用于高效检测GAN生成的合成人脸图像。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型（如GAN）生成的虚假人脸图像质量不断提高，传统的CNN或全局ViT检测方法在局部纹理捕捉与全局上下文理解之间难以平衡，且计算开销较大。

Method: 研究了三种基于Swin-Transformer的方案：从头训练的小型Swin、基于ImageNet预训练的Swin-Tiny/Small，以及融合EfficientNet-B0卷积特征与Swin Transformer的混合架构。

Result: 在140K Real and Fake Faces数据集上，EfficientNetB0+Swin混合模型表现最佳，准确率达99%，召回率达99.44%，优于纯Swin变体及基准CNN模型。

Conclusion: 结合层级化CNN特征与移动窗口自注意力机制，能够为合成人脸检测提供一种高效且计算轻量化的解决方案。
...
### [67] [AlphaRAD: Grounded Zero-Shot Classification in Chest Radiology via $α$-Corrected Binary Cross Entropy and Factorized Latent Supervision](https://arxiv.org/abs/2609.01757)
*Jianzhong You,Yuan Gao,Chris McIntosh*

Main category: cs.CV

TL;DR: AlphaRAD是一个用于胸部放射影像的视觉语言预训练模型，通过引入结构化医学概念空间和因子化潜在监督（FLaS）模块，显著提升了医学影像理解的准确性与空间定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在胸部放射领域存在对比学习中的批次噪声问题，且在不增加复杂性的情况下实现高保真空间定位的能力不足。

Method: 1.利用LLM解析报告构建结构化医学概念空间，通过α修正二元交叉熵实现概念区分；2.提出FLaS模块，将特征映射到独立子空间进行对齐监督，在不增加参数的前提下增强空间定位能力。

Result: AlphaRAD在16个分类基准测试中达到SOTA水平，并在7个定位/短语定位及3个分割数据集上表现优异，展现了强大的零样本泛化能力。

Conclusion: AlphaRAD通过结构化语义引导和高效的特征因子化设计，有效解决了医学影像理解中的噪声和定位难题，为放射学AI提供了更精准的解决方案。
...
### [68] [Evidential Deep Learning for Multi-Modal Anti-UAV Detection](https://arxiv.org/abs/2609.01742)
*Dmitry Golovchits,Seyed Sahand Mohammadi Ziabari,Ali Mohammed Mansoor Alsahag*

Main category: cs.CV

TL;DR: 该研究评估了证据深度学习（EDL）、DS证据融合及时间传感器门控在反无人机系统中的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有反无人机传感融合系统缺乏模态级可靠性信号，通过引入不确定性估计以期提升检测性能。

Method: 在三个基准测试（AntiUAV600, TRIDENT, MM-UAV）上进行对比消融实验，分析EDL训练目标、DS融合及时间门控的实际影响。

Result: EDL训练目标显著提升了检测与分类精度，优于Sigmoid基线；但DS融合未优于简单平均，Dirichlet空无性在背景失衡下失效，时间门控未带来显著延迟优化。

Conclusion: EDL的收益主要源于其训练目标而非不确定性估计；在反无人机场景中，现有的不确定性建模在锚框级评估中存在局限性。
...
### [69] [Allocate Before You Embed: Adaptive Visual Input Allocation for Video Embeddings](https://arxiv.org/abs/2609.01778)
*Song Jin,Zhongtao Jiang,Chenglei Shen,Huanxuan Liao,Haozhe Chi,Zhiwei Wang,Kun Xu,Yong Liu*

Main category: cs.CV

TL;DR: AllocEmbed 是一种针对大规模视频检索的分配嵌入框架，通过动态调整帧分辨率和优化时间覆盖范围，在固定计算预算下提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索方法受限于固定帧采样，导致时间覆盖不足或空间细节丢失，研究表明在保持分辨率的同时扩展时间覆盖能显著提升检索效果。

Method: 提出了 AllocEmbed 框架，利用轻量级分配器根据低成本预览动态分配各帧的分辨率；引入了检索驱动策略优化（RDPO），利用检索反馈和置信度引导的效率激励来训练分配策略。

Result: 在 MMEB-V2 和 LongRet 基准测试中，AllocEmbed 在预算匹配的情况下优于现有方法，且具有良好的跨模型通用性。

Conclusion: 该方法证明了在预处理阶段通过智能分配视觉预算，无需修改后端模型即可有效提升视频检索的精度与效率。
...
### [70] [Ten Architectures, One Error: Shared Failure Modes in Hyperspectral Classification under Spatially Disjoint Evaluation](https://arxiv.org/abs/2609.01786)
*Ehsan Faghih,Fatemeh Ashrafi,Marguerite Moore,Zahra Saki*

Main category: cs.CV

TL;DR: 该研究指出高光谱图像分类中常用的随机像素划分会导致数据泄露，并提出了一种无泄露的评估协议，发现现有架构的性能被严重高估且排名不稳定。


<details>
  <summary>Details</summary>
Motivation: 由于随机划分导致测试像素与训练像素在空间上过于邻近，造成了模型性能的虚高，使得模型对比不够公正。

Method: 引入了基于模型感受野的空间分离无泄露评估协议，并在十种不同架构上进行了对比实验。

Result: 无泄露评估下Macro-F1平均下降0.147，模型排名发生剧烈变动；同时发现所有模型在同一类像素上均存在误分类，揭示了数据本身的频谱模糊性。

Conclusion: 传统的随机划分评估方法不可靠，未来评估高光谱分类模型必须报告空间分离半径，且应关注解决数据固有的频谱模糊性问题。
...
### [71] [CoViT: Instance-Correspondence Contrastive Learning for Vision Transformer](https://arxiv.org/abs/2609.01787)
*Yisen Wang,Zhirong Wu,Limin Wang*

Main category: cs.CV

TL;DR: 提出了一种名为CoViT的自监督学习框架，通过几何引导的对比学习提升ViT在实例级感知任务中的表现。


<details>
  <summary>Details</summary>
Motivation: ViT在语义理解上表现出色，但难以区分同一类别下的不同实例，限制了其在目标检测和实例分割中的应用。

Method: 利用注意力图通过自适应阈值和形态学操作生成实例掩码，并基于此构建包含锚点、最难正样本和最难负样本的三元组，通过对比损失函数强化实例间的区分度。

Result: 在多种实例级感知任务中，相较于基准ViT实现了超过2个AP点的稳定性能提升，且无需额外的解码器或标注。

Conclusion: 纯ViT架构可以通过内在的注意力先验和针对性的对比约束，有效地学习到实例感知的表征。
...
### [72] [DESA-TTA: Dynamic EMA and Source Anchoring for Test-Time Adaptation](https://arxiv.org/abs/2609.01795)
*Atif Belal,Lilian Hollard,Marco Pedersoli,Eric Granger*

Main category: cs.CV

TL;DR: 提出了一种名为DESA-TTA的视觉语言对象检测器测试时自适应（TTA）方法，通过动态EMA和源锚定机制解决模型漂移和超参数敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 现有Mean-teacher TTA方法对EMA系数敏感，且在噪声伪标签下的重复训练容易导致学生模型性能漂移。

Method: 引入动态时间平均（根据伪标签置信度和框密度调整EMA系数）和源锚定（通过对比预训练参数约束模型更新幅度）。

Result: 在VOC-C等数据集上性能显著提升，相较于零样本推理，AP50提升了14.5个点，同时推理吞吐量比现有最优方法高出55%。

Conclusion: DESA-TTA能有效调节教师更新与学生漂移，是一种低开销且在多种分布偏移下表现鲁棒的TTA方案。
...
### [73] [Improved Automatic Target Recognition in Synthetic Aperture Sonar Imagery Using Large Deep Neural Networks](https://arxiv.org/abs/2609.01800)
*C. J. Moore,Alex Hurt,Jordan Malof*

Main category: cs.CV

TL;DR: 本文对比了卷积神经网络（CNN）与Transformer架构在合成孔径声纳（SAS）自动目标识别（ATR）任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在通用计算机视觉领域表现优异，但在SAS-ATR领域仍由CNN主导；此外，针对SAS数据稀缺及预训练迁移学习的有效性尚无明确共识。

Method: 通过实证研究，对比不同架构（CNN vs Transformer）、网络规模、预训练方法、数据增强及正则化策略对SAS-ATR性能的影响。

Result: 确定了最优的模型架构与训练配置，并发现不同训练策略在应对SAS数据稀缺问题时的性能表现差异。

Conclusion: 该研究为开发高性能SAS-ATR模型提供了实践指南和架构选择参考。
...
### [74] [Consistency as Regularization for Unsupervised Shadow Removal](https://arxiv.org/abs/2609.01806)
*Anh-Kiet Duong,Petra Gomez-Krämer,Jean-Michel Carozza*

Main category: cs.CV

TL;DR: ShadowCLR 是一种无需配对数据或阴影掩码的无监督阴影去除框架，通过利用多视图中场景内容的一致性来恢复无阴影图像。


<details>
  <summary>Details</summary>
Motivation: 现有的阴影去除方法通常依赖成对图像或阴影掩码，获取这些标注数据成本高昂且困难，因此需要一种能直接从阴影图像中学习的无监督方法。

Method: 利用阴影在不同观测中变化而场景内容不变的特性，通过全局和局部一致性正则化来学习恢复场景外观，并结合视觉相关图像及非完美对齐的观测进行训练。

Result: 实验表明，ShadowCLR 在多个基准测试中表现优异，性能达到甚至超过了现有的最先进无监督阴影去除方法。

Conclusion: 该研究证明了利用多观测一致性作为正则化手段，可以在无需阴影掩码或无阴影参考图的情况下有效实现阴影去除。
...
### [75] [Integrated Laser Scanning and Image-Based Topology Optimization Techniques for Detection and Quantification of Visible and Subsurface Structural Defects](https://arxiv.org/abs/2609.01808)
*Mehrdad Shafiei Dizaji,Devin Harris*

Main category: cs.CV

TL;DR: 该研究提出两种互补的非接触式视觉方法，用于检测和量化结构件的表面及内部缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有的检测方法难以同时可靠地表征可见表面损伤和不可见的内部缺陷。

Method: 第一种方法使用高分辨率激光扫描生成3D点云进行几何损失量化；第二种方法结合3D数字图像相关技术（3D-DIC）、有限元模型更新及拓扑优化，通过表面响应推断内部异常。

Result: 实验证明两种方法均能有效识别和量化缺陷几何形状，且互补性强，在复杂不规则损伤评估中表现出高保真度。

Conclusion: 该组合框架为结构件的非接触式状态评估和模型更新提供了一种高精度的有效途径。
...
### [76] [Video2Reaction: Training Foundation Video Models to Predict Audience Reaction](https://arxiv.org/abs/2609.01816)
*Sidong Zhang,Trang Nguyen,Shiv Shankar,Gauri Jagatap,Deepak Chandran,Andrea Fanelli,Madalina Fiterau*

Main category: cs.CV

TL;DR: 本文提出了Video2Reaction数据集，这是一个将电影片段与社交媒体评论中提取的观众情绪反应相映射的多模态数据集。


<details>
  <summary>Details</summary>
Motivation: 现有情绪数据集往往标注单一且缺乏多样性，无法很好地捕捉真实场景下人类对视频诱发情绪的主观、模糊及多样的反应。

Method: 通过大规模聚合社交媒体评论构建数据集，将情绪标签建模为类别分布，并使用LoRA微调视觉语言模型（VLM）进行情绪预测任务。

Result: 微调后的VLM在主流反应预测任务中表现优异，且在不同数据集（VCE）上的迁移学习能力显著，仅需1%的训练数据即可达到全量训练的基准性能。

Conclusion: Video2Reaction为理解视频诱发的情绪提供了高质量的多模态基准，证明了大规模网络评论数据对提升视觉语言模型感知人类情绪能力的有效性。
...
### [77] [Kirin: Animal Motion Generation from In-the-Wild Video](https://arxiv.org/abs/2609.01823)
*Brian Nlong Zhao,Zhuoyang Pan,James M. Rehg,Jiajun Wu,Shangzhe Wu*

Main category: cs.CV

TL;DR: Kirin是一个大规模动物运动生成框架，通过构建AiM3D数据集和视觉引导生成模型，实现了从视频到3D动画的自动生成。


<details>
  <summary>Details</summary>
Motivation: 动物运动研究受限于高质量运动数据的稀缺，传统方法难以像人类运动研究那样在大规模范围内进行建模和动画制作。

Method: 利用野外视频重建3D运动序列，构建视频-文本-运动对齐数据集（AiM3D），开发视觉和文本双重引导的生成模型，并结合现有图像到3D模型实现自动绑定与动画化。

Result: 发布了首个大规模四足动物视频-文本-运动数据集，并证明了该框架能够生成高质量、可直接渲染的动物动画。

Conclusion: Kirin为动物运动的规模化生成与动画化提供了新的基础架构，有效克服了数据瓶颈并提升了跨物种运动生成的表现。
...
### [78] [SliceBridge: context-consistent repair of corrupted slice intervals in T1-weighted MRI](https://arxiv.org/abs/2609.01827)
...
TL;DR: SliceBridge 是一种利用流匹配（Flow Matching）技术修复结构 MRI 中连续切片局部损坏的深度学习框架。


<details>
  <summary>Details</summary>
Motivation: MRI 扫描中常出现局部切片损坏（由于伪影或运动），完全丢弃或重新采集成本高昂，且此类损伤会干扰下游形态学分析。

Method: 提出 SliceBridge 框架，通过引入区间相关初始噪声、共享流时间及同步采样策略，在维持层间连贯性的前提下，利用周围完好切片作为条件进行流匹配重建。

Result: 相比独立重建模型，SliceBridge 在修复区间内切片间变化的误差降低了 32.9%-41.3%，SSIM 指标更高；并将下游分割任务中脑区体积估计的误差从 1.95% 降低至 1.05%。

Conclusion: 该方法有效修复了 MRI 的局部损坏，保证了三维解剖结构的连贯性，从而提升了基于受损图像进行的下游分析的准确性。
...
### [79] [Cross-Model Distillation of a Human-Pose Foundation Model from Unannotated Infant Video for Markerless 3D Pose Estimation](https://arxiv.org/abs/2609.01840)
*R. James Cotton,Divya Joshi,Colleen Peyton*

Main category: cs.CV

TL;DR: 通过跨模型知识蒸馏技术，利用无标注婴儿视频提升了现有的3D人体姿态估计模型在婴儿群体上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的人体姿态估计模型多针对成人训练，在婴儿身上表现不佳；而传统临床评估手段依赖人工且耗时，急需高效的自动化评估方案。

Method: 采用跨模型蒸馏策略，将Sapiens 2（2D姿态模型）作为教师，通过无标注婴儿视频为SAM 3D Body模型提供密集伪标签，并结合可微分渲染器进行对齐训练。

Result: 实验显示，模型在婴儿数据集上的2D关键点准确率显著提升（PCK@10px从0.22提高至0.42），且3D关节位置误差（MPJPE）从25.5mm降低至22.2mm。

Conclusion: 该研究证明了在无需额外人工标注的情况下，利用跨模型蒸馏能有效改善现有模型在特殊人群（婴儿）运动分析中的表现。
...
### [80] [RAFT-DVC: Resolution-Aware Machine Learning-Based Digital Volume Correlation](https://arxiv.org/abs/2609.01876)
*Zixiang Tong,Lehu Bu,Jin Yang*

Main category: cs.CV

TL;DR: 提出了一种基于RAFT架构的数字体积相关（DVC）框架——RAFT-DVC，通过分辨率感知设计实现了高精度的三维全场位移测量。


<details>
  <summary>Details</summary>
Motivation: 现有基于机器学习的DVC模型在内部分辨率对精度和操作范围的影响方面缺乏深入理解，且通用性有待提升。

Method: 开发了具有不同编码器下采样因子（s=2, 4, 8）的RAFT-DVC求解器，引入了平铺推理、频率扫描测试，并修正了三维采样中的坐标顺序不一致问题。

Result: RAFT-DVC在精细纹理、小位移条件下与经典DVC精度相当，在粗糙纹理和大位移条件下更具优势，且具备良好的跨纹理迁移能力。

Conclusion: RAFT-DVC是一个快速且具备分辨率感知能力的DVC框架，其准确性和操作范围得到了明确表征，适用于多种三维成像数据的位移测量。
...
### [81] [SignMatch: Matching Dictionary Signs to Continuous Sign Language Video](https://arxiv.org/abs/2609.01886)
*Ryan Wong,Youngjoon Jang,Liliane Momeni,Gül Varol,Andrew Zisserman*

Main category: cs.CV

TL;DR: 提出了一种基于原型结构的嵌入学习方法，实现了字典孤立手语视频与连续手语视频之间的视觉相似度匹配。


<details>
  <summary>Details</summary>
Motivation: 传统手语识别方法通常依赖大量标注数据，且难以处理未见过的手语词汇，通过字典视频匹配连续手语可提升系统的通用性和扩展性。

Method: 从标注的连续手语视频中学习原型结构化的嵌入空间，每个原型对应一个手语类别，将孤立的字典视频映射至该空间进行相似度度量。

Result: 在ASL-Citizen、ChaLearn OSLWL和BOBSL数据集上表现优异，实现了跨数据集、跨任务及跨手语语言（美、英、西）的高效迁移。

Conclusion: 该方法无需特定基准的监督训练，通过学习到的通用表示即可在不同手语语种中实现有效的字典引导匹配。
...
### [82] [Does Playing it Safe Count as Faithfulness? Reassessing LVLM Hallucination Mitigation Methods](https://arxiv.org/abs/2609.01888)
*Mehrdad Fazli,Sina Mansouri,Mohit Marvania,Ziwei Zhu*

Main category: cs.CV

TL;DR: 研究指出当前LVLM幻觉缓解方法往往以牺牲信息量和通用能力为代价，现有的幻觉评估指标存在误导性。


<details>
  <summary>Details</summary>
Motivation: 目前尚不明确幻觉评分的提升是因为模型增强了多模态感知能力，还是仅仅因为模型变得更加保守。

Method: 通过评估6种缓解方法在3种LVLM和4个基准测试（包括幻觉测试和通用能力基准MMStar）上的表现，分析幻觉率与信息量、多模态能力之间的关系。

Result: 研究发现幻觉缓解通常伴随着信息量减少（如对象召回率、视觉覆盖范围下降），且在幻觉基准上的提升并不能有效转化为更广泛的多模态推理能力。

Conclusion: 当前幻觉评估协议可能高估了技术进展，建议将幻觉缓解视为忠实度、信息量与能力之间的权衡，而非单一追求幻觉评分的降低。
...
### [83] [TAPVid-MV: A Benchmark for Tracking Any Point in 3D Across Multiple Views](https://arxiv.org/abs/2609.01899)
...
TL;DR: 本文提出了首个用于多视图视频中点追踪的基准测试集 TAPVid-MV，旨在解决机器人和自动驾驶等领域中多视角长时3D点追踪的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基准多限于单视频或静态多相机，缺乏针对多相机动态场景下长时3D点追踪的评估，且现有方法在该任务上表现不佳。

Method: 构建了包含284个序列、1142个校准相机流及约11万个经人工验证的轨迹点的基准数据集，并利用传感器深度、LiDAR、SLAM和SfM等多种模态进行标注。

Result: 通过对30多个基准方法评估发现，现有方法均难以有效解决该任务，且多视图追踪器表现并未显著优于单目追踪器，同时揭示了几何重建是当前准确3D点追踪的主要瓶颈。

Conclusion: TAPVid-MV 为多视图 3D 点追踪提供了一个具有挑战性的评估平台，并支持多种计算机视觉任务，强调了提升几何重建能力对点追踪的重要性。
...
### [84] [Automated Maize Ear Phenotyping Using 3D Reconstructions](https://arxiv.org/abs/2609.01921)
...
TL;DR: 该研究开发了一种基于3D点云的自动化玉米穗表型提取流程，用于精准测量行数、每行粒数及粒径。


<details>
  <summary>Details</summary>
Motivation: 传统手工测量玉米穗性状效率低下，无法满足育种程序对大规模表型数据的需求。

Method: 利用视频转点云技术（COLMAP/NeRF），通过密度过滤隔离穗部，采用PCA对齐、柱面展开技术将3D点云转为2D图像，并结合Cellpose-SAM进行无微调实例分割。

Result: 在验证集上，每穗粒数预测的R^2达到0.921，行数预测准确度在±2行内的比例高达95.2%。

Conclusion: 该自动化流程能高效、准确地提取玉米穗性状，为表型与基因型关联分析提供了可靠的大规模数据集。
...
### [85] [Learning with Volterra Neural Networks: A System Theoretic Perspective](https://arxiv.org/abs/2609.01928)
*Haoyu Yun,Hamid Krim,Yufang Bao*

Main category: cs.CV

TL;DR: 提出了一种名为kVNN的核化Volterra神经算子，通过核化技术实现高效的高阶信号建模。


<details>
  <summary>Details</summary>
Motivation: 显式高阶算子在建模中存在参数量大和计算成本高的问题，需要一种更紧凑且具备高阶交互能力的模型。

Method: 结合Volterra滤波器的阶数结构与可学习的多项式核原子，通过阶数解耦表示法避免显式高阶张量参数化，并将其实现为兼容CNN的层。

Result: 实验表明，kVNN在多个视觉任务上实现了更好的精度与效率平衡。

Conclusion: kVNN为高阶特征交互提供了一种参数高效且结构化解释性强的神经算子解决方案。
...
### [86] [Aggregating Neighbor Embedding Projection and Rank-Based Manifold Learning for Image Retrieval](https://arxiv.org/abs/2609.01963)
*Vinicius Atsushi Sato Kawai,Gustavo Rosseto Leticio,Lucas Pascotti Valem,Daniel Carlos Guimarães Pedronette*

Main category: cs.CV

TL;DR: 提出了一种结合邻域嵌入投影（UMAP）与基于排名的流形学习（重排序）的图像检索框架，利用Borda计数进行排名聚合，以提升图像检索效果。


<details>
  <summary>Details</summary>
Motivation: 高维特征空间中传统的两两距离难以捕捉上下文关系，且存在语义鸿沟，单一的投影或排名策略各有局限，缺乏有效的协同整合机制。

Method: 利用UMAP生成低维表示，将UMAP投影获取的排名列表与基于排名的重排序结果通过Borda计数进行聚合。

Result: 在ResNet152、Swin Transformer和DINOv2特征上实验证明，该方法能显著提升检索精度（MAP），尤其在基线表现不佳的情况下改进明显。

Conclusion: 投影学习与流形学习的协同整合通过排名聚合能够挖掘互补的上下文信息，是增强图像检索性能的有效手段。
...
### [87] [Linear Fusion MultiDiffusion for Fast Training-Free Spherical Panorama Generation](https://arxiv.org/abs/2609.01997)
*Akio Hayakawa,Yusuke Mukuta,Tatsuya Harada*

Main category: cs.CV

TL;DR: LF-MultiDiffusion 是一种无需训练的全景图生成方法，通过引入线性投影和 Krylov 迭代求解器，实现了更高效、更高质量的全景图生成。


<details>
  <summary>Details</summary>
Motivation: 现有无需训练的全景生成方法在视角融合和推理效率上存在局限，导致生成的稳定性差且计算开销大。

Method: 将潜在聚合重新构建为正则化最小二乘问题，并在去噪循环中使用基于 Krylov 的迭代求解器进行高效求解，支持目标与参考图像空间之间的线性投影。

Result: 相比现有最强基线，该方法提升了视觉质量、文本对齐度和全景一致性，并将推理速度提升了 15.36 倍。

Conclusion: 通过线性投影和优化的数值求解策略，LF-MultiDiffusion 有效解决了全景生成中的视角映射与效率瓶颈，实现了性能与速度的双重提升。
...
### [88] [LookStep: Efficient Vision-Language Navigation with Linguistic Foresight and Event Driven Memory](https://arxiv.org/abs/2609.02350)
*Kun-Yang Yu,Yingzhe Li,Hongyu Xu,Shi-Yu Tian,Zhi Zhou,Yang Chen,Ming Yang,Sheng Wang,Qing Yu,Lan-Zhe Guo,Yu-Feng Li*

Main category: cs.CV

TL;DR: LookStep 是一个高效的视觉语言导航（VLN）框架，通过语言驱动的未来状态建模和事件驱动的滚动内存机制，实现了在低数据量和低资源消耗下的高性能导航。


<details>
  <summary>Details</summary>
Motivation: 现有 VLN 方法高度依赖专家行为监督（需要海量数据）以及复杂的地图或外部工具（导致计算和内存开销巨大），难以满足资源受限场景下的导航需求。

Method: 提出了两个核心模块：1. 语言中心未来状态建模，利用语言标签预测导航进度和候选动作的未来状态；2. 事件驱动滚动内存，通过语义角色自动决定是否将观测信息写入有限的内存中。

Result: 在 VLN-CE 任务中，LookStep 在 R2R-CE Val-Unseen 集上达到 49.7% 的成功率，在保持更高内存效率和更少数据需求的情况下，超越了现有的主流方法。

Conclusion: LookStep 通过端到端的统一框架证明了在资源高效的前提下实现高性能 VLN 是可行的，为未来轻量化 embodied AI 的发展提供了新思路。
...
TL;DR: SA-WAM 引入空间感知能力，通过将深度信息与 RGB 视频预测模型集成，显著提升了机器人动作预测与世界建模的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的世界动作模型（WAMs）仅依赖 RGB 观测，缺乏对 3D 信息的感知，限制了其在复杂物理环境中的泛化能力和准确性。

Method: 提出 SA-WAM 模型，利用非线性编码将深度信息映射到预训练视频扩散模型的输入域，实现 RGB、深度和动作的联合预测，无需对 VAE 分词器进行微调。

Result: 在 RoboCasa 和 LIBERO-Plus 基准测试中达到 SOTA，并在 UR5 真实机器人实验中展现出更强的环境泛化能力。

Conclusion: 引入 3D 空间信息是提升世界动作模型性能的关键路径，SA-WAM 有效整合了预训练视觉先验与 3D 几何特征。
...
### [90] [Who Drives the Probability Game of VLMs? A Temporal Causal Drive Evaluation Framework](https://arxiv.org/abs/2609.02000)
*Shuyao Xiao,Shengling Wang,Haoyu Niu,Ke Chao,Changwei Xu,Xinran Duan,Chaoyong Jiang*

Main category: cs.CV

TL;DR: 提出了一种基于因果分析和结构化因果模型（SCM）的评估框架，用于追踪视觉语言模型（VLM）在生成过程中视觉输入、文本问题和已生成内容对预测的驱动作用。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标仅关注最终答案质量，无法揭示不同信息源（视觉、问题、上下文）如何随时间推移影响模型的生成过程。

Method: 引入三种因果驱动指标：视觉因果驱动（VCD）、问题因果驱动（QCD）和前缀因果驱动（PCD），利用干预和后门调整技术，在无需参考答案的情况下量化信息源在自回归解码中的贡献。

Result: 研究发现模型在生成过程中呈现出从依赖早期视觉和问题指导向依赖已生成前缀转变的规律；实验显示QCD和PCD在减少恢复误差方面显著优于观测性基线，并在检测偏见方面表现出高准确度。

Conclusion: 基于因果轨迹的评估方法能为多模态模型生成过程提供细粒度的诊断工具，有助于理解VLM的内部决策逻辑。
...
### [91] [MV-dVRK: A Multi-Viewpoint Benchmark for Spatial Surgical Perception](https://arxiv.org/abs/2609.02717)
...
TL;DR: 本文提出了MV-dVRK数据集，这是首个包含多视角、同步立体影像及高精度真实几何参考的离体手术数据集，旨在评估手术场景下的多视角三维重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有三维重建技术在通用领域表现出色，但在临床手术（仅有单视点立体相机）及真实手术影像的定量评估上缺乏严谨研究，且缺乏高质量的基准数据集。

Method: 构建了包含静态和动态序列的MV-dVRK数据集，利用工业级扫描仪生成地面真值几何，并系统对比了单目、立体、多立体及基于优化的多视角重建方法在不同视点数下的表现。

Result: 实验显示，三视角环境下基于优化的多视角方法表现最优（1mm容差下覆盖率达67%），显著优于前馈基础模型（覆盖率43%），且在处理器械运动和组织形变方面具备扩展潜力。

Conclusion: MV-dVRK为手术环境下的多视角感知研究提供了可靠的基准，证明了优化驱动的方法在手术复杂场景下优于现有的前馈基础模型。
...
### [92] [InsightSeg: Reusing Correction Insights for Guideline-Consistent Segmentation](https://arxiv.org/abs/2609.02002)
*Vanshika Vats,Ashwani Rathee,James Davis*

Main category: cs.CV

TL;DR: InsightSeg 是一种为语义分割引入情景记忆机制的系统，通过将纠错经验转化为可复用的视觉化建议，从单纯的“事后纠错”转向“事前预防”。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体纠错系统是无状态的，导致模型会反复犯同样的错误并进行重复纠正，效率低下且浪费资源。

Method: 引入情景记忆机制，通过元分析器将成功纠错的案例提炼为自然语言建议，并利用 patch 级视觉概念向量将建议锚定到图像区域，在后续任务中进行预先检索和提示。

Result: 在 Waymo 和 Cityscapes 数据集上，InsightSeg 提升了首次分割的准确度及最终合规性，同时显著减少了所需的修正步骤。

Conclusion: 通过利用历史纠错经验，多智能体细化系统能够实现更高效、更精准的语义分割，变“被动纠错”为“主动预防”。
...
### [93] [InstEditSeg: Instruction-Driven Image Editing for Polyp and Skin Lesion Segmentation](https://arxiv.org/abs/2609.02004)
*Ziquan Liu,Zhewei Zhu,Xuyang Shi*

Main category: cs.CV

TL;DR: InstEditSeg 将医学图像分割重新构思为一种基于指令的图像编辑任务，利用预训练潜在扩散模型的生成能力来提高分割精度和跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有分割方法在处理低对比度、模糊边界和跨域差异时表现不佳，且未能充分利用大规模预训练生成模型的视觉先验。

Method: 将分割任务转化为条件图像编辑（生成带颜色的叠加图），引入 DINOv3 作为辅助视觉编码器并结合特征金字塔注入先验，采用双分支无分类器引导策略以降低推理成本。

Result: 在息肉和皮肤病变数据集上达到与主流判别模型相当的精度，并显著提升了跨域泛化能力、多目标分割完整性及任务可控性。

Conclusion: 基于生成式模型进行医学分割是一种有效的新范式，能够有效利用生成模型的分布知识，但在颜色敏感度及特定属性选择方面仍有改进空间。
...
### [94] [GeoStore: Finding Small Storefronts in Large Scenes -- A Fine-Grained POI Localization Benchmark with Global-to-Local Asymmetric Matching](https://arxiv.org/abs/2609.02012)
*Lu Han,Xiting Sun,Hao Wang,Zhiqiang Cao,Ruihuan Du,Ziquan Zeng,Chunlong Lv*

Main category: cs.CV

TL;DR: 本文提出GeoStore基准测试及GLAM架构，用于解决将近景店铺照片与大范围街景图像进行匹配（POI定位）的非对称细粒度识别问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉地点识别（VPR）方法多假设全图对称匹配，无法有效应对近景POI图像与街景中微小目标之间的尺度和视野差异。

Method: 提出了GeoStore数据集，并设计了GLAM（全局到局部非对称匹配）架构，结合全局检索锚点与非对称局部路径，利用池化区域Token进行软交互匹配及轻量级重排序。

Result: GLAM在Recall和mAP指标上超越了现有基线，同时将重排序特征量减少了约5倍，匹配计算开销降低了约两个数量级。

Conclusion: GLAM通过引入非对称局部匹配机制，有效解决了POI定位中目标微小及视角差异带来的识别难题，具备高效的工业应用潜力。
...
### [95] [Detecting Object Hallucinations in Large Vision-Language Models via Cross-Modal Attention Drifts and Mask-Based Verification](https://arxiv.org/abs/2609.02028)
*Xuanbing Wen,Boxu Chen,Le Yang,Jiakai Wang,Zhengyu Zhao,Chenhao Lin,Chao Shen*

Main category: cs.CV

TL;DR: 提出了一种名为CADMP的轻量级框架，通过分析跨层注意力漂移和视觉掩码预测敏感度来检测大视觉语言模型（LVLM）中的对象幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有方法多关注单层注意力机制，忽略了视觉基础在多层中的演变，导致对象幻觉检测不够鲁棒。

Method: 结合相邻层间的跨模态注意力漂移（量化视觉基础的稳定性）与针对性视觉掩码的预测概率变化（验证预测对视觉证据的依赖性），通过轻量级分类器集成两种信号进行检测。

Result: 在多个基准测试中表现出极具竞争力的幻觉检测性能，证明了跨层漂移建模与掩码验证的互补有效性。

Conclusion: 通过捕捉跨层注意力变化与验证视觉归因，CADMP提供了一种高效且准确的LVLM幻觉检测方案。
...
### [96] [SelfLift: Accelerating Few-Step Diffusion via Self-Recovering Resolution Transition](https://arxiv.org/abs/2609.02036)
*Tingyan Wen,Chenqian Yan,Xurui Peng,Xiazhang Fang,Shuai Wang,Xueqian Wang,Songwei Liu*

Main category: cs.CV

TL;DR: SelfLift 是一种用于少步扩散模型的渐进分辨率推理框架，通过自修复机制减少高分辨率生成的计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有渐进分辨率方法在少步推理中存在分布失配导致的伪影，限制了推理效率的提升。

Method: 提出了 SelfLift 框架：包含无需训练的 Artifact-Aware Consistency Lift（利用潜空间升采样与 VAE 重编码的差异进行修正）以及 On-Policy Self Recovery（通过内部自监督实现高分辨率引导）。

Result: 在 FLUX 和 Z-Image-Turbo 模型上实现了 41.5% - 44.1% 的延迟降低，结合蒸馏后整体速度提升约 20-30 倍，且保持了高质量生成。

Conclusion: SelfLift 有效解决了渐进分辨率带来的伪影问题，显著提升了少步扩散模型的推理速度与质量边界。
...
### [97] [Morphology signal in whole slide image foundation models can automatically triage slides](https://arxiv.org/abs/2609.01987)
...
TL;DR: 提出了一种基于病理全切片图像（WSI）基础模型（FM）的自动化筛选管道，旨在解决癌症诊断中从多张切片中识别关键病灶切片的繁琐过程。


<details>
  <summary>Details</summary>
Motivation: 现有方法往往假设每位患者仅有一张切片，或者将所有切片均用于模型训练，这导致了人工筛选成本高昂或关键信号被噪声稀释的问题。

Method: 利用现有的公共WSI基础模型进行零样本（zero-shot）分类，根据预测分数对每位患者的切片进行排序，并提出了一种排序评估指标来衡量筛选性能。

Result: 实验证明该方法能有效从多达43张切片中准确识别出包含肿瘤的切片，且肿瘤切片通常位于排序的前两名。

Conclusion: WSI基础模型具备足够的形态学信号，可替代人工实现高效、自动化的切片筛选与分诊，从而优化癌症诊断和生存分析的流程。
...
### [98] [Test-Time Logit Prompting for Source-Free Missing Modality Adaptation](https://arxiv.org/abs/2609.02039)
*Taixi Chen,Nancy Guo*

Main category: cs.CV

TL;DR: 提出了一种名为TLP的源数据无关测试时适应框架，旨在解决视觉语言模型在缺失模态情况下的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度依赖源训练数据，而在临床等场景中，由于隐私和存储限制，无法获取源数据，因此需要研究无需源数据的测试时适应方法。

Method: 提出了测试时Logit提示（TLP）方法，通过不确定性感知调整和模态完整性一致性正则化来优化Logit提示。

Result: 实验表明，TLP在多种视觉语言基准测试中表现出色，在缺失模态场景下提升了高达8%的性能，且仅需极少量的可调参数和优化步骤。

Conclusion: TLP成功实现了高效的无源测试时自适应，显著增强了VLM在模态缺失环境下的鲁棒性。
...
### [99] [LaST-SR: Laplace-Inspired Steady-Transient Complex-Frequency Decomposition for Single Image Super-Resolution](https://arxiv.org/abs/2609.02063)
*Linhao Li,Zhaojie Pan,Langkun Chen*

Main category: cs.CV

TL;DR: 提出了一种名为LaST-SR的单图像超分辨率（SISR）方法，通过拉普拉斯神经算子实现稳态（周期性）与瞬态（非周期性）特征的协同建模。


<details>
  <summary>Details</summary>
Motivation: 傅里叶算子在处理非周期性局部变化时受限于其周期性基函数，难以精细还原图像中的不规则结构和细节。

Method: 推导了二维特征图的稳态-瞬态分解理论，并构建了LaST-SR模型，包含复杂频率分解模块（结合傅里叶分支与窗口条件局部复杂频率分支）及稳态-瞬态协同聚合模块。

Result: 在五个基准测试中，LaST-SR在×2和×4超分辨率任务上均达到了PSNR/SSIM的最高指标。

Conclusion: 引入稳态-瞬态分解能够有效整合全局结构一致性与局部非周期性细节，证明了该方法在SISR任务中的优越性。
...
### [100] [DPA: Decoupling Product-Agnostic Anomaly Representations for Zero-shot Anomaly Generation](https://arxiv.org/abs/2609.02075)
*Hang Yao,Yansheng Fu,Ming Liu,Zifei Yan,Yanli Ji,Hongzhi Zhang,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 提出了一种基于异常迁移的零样本异常生成框架（DPA），通过从源产品迁移真实异常样本来解决新产品缺乏异常数据的问题。


<details>
  <summary>Details</summary>
Motivation: 工业场景下新产品缺乏异常样本，现有基于纹理或文本的生成方法产生异常不真实，而异常模式在不同产品间存在可复用性。

Method: 1. 异常类型过滤机制选择适配源；2. DPA框架：解耦产品无关的异常表征，通过不匹配数据对学习通用异常嵌入；3. 自适应掩码引导生成管道控制异常位置与几何形态；4. 免训练异常标注模块自动生成像素级标签。

Result: 在MVTec-AD、VisA及自定义基准测试中，显著提升了零样本和少样本场景下的异常检测性能，生成了更逼真的异常样本。

Conclusion: 利用跨产品异常迁移而非从头生成，为解决工业缺陷检测中的数据稀缺问题提供了一种高效且高质量的新途径。
...
### [101] [KSG-Net: Key-Sparse and Global-Context Learning for Maritime 3D Ship Detection](https://arxiv.org/abs/2609.02077)
*Zhouyuan Huai,Meiqi Wan,Yan Yang,Minshi Chen,Xin Yuan,Wei Wang,Xiao Wang*

Main category: cs.CV

TL;DR: 提出了一种名为KSG-Net的海洋场景3D船舶检测网络，旨在解决现有方法在处理船舶尺度差异大、点云稀疏及海杂波干扰下的检测难题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在海洋环境下难以平衡检测精度与计算效率，且针对道路场景设计的稀疏3D检测器难以直接迁移至船舶场景，特别是在小目标稀疏特征表达和大型船舶全局结构建模方面存在不足。

Method: 提出了KSG-Net框架，包含两个核心模块：1. KSMA模块，通过选择关键体素并聚合跨尺度邻域特征，增强小目标与稀疏船舶的特征表示；2. GCA模块，通过场景级上下文建模捕捉长距离几何依赖，增强大型船舶的结构感知能力。

Result: 在Thames River及模拟数据集上的实验表明，KSG-Net在多尺度船舶检测任务中优于现有主流方法，并表现出较强的环境鲁棒性。

Conclusion: KSG-Net通过联合增强局部判别特征与全局结构意识，有效提升了海洋环境下3D船舶检测的性能，为自主导航提供了更可靠的感知方案。
...
### [102] [Rendering-in-the-Loop: An Execution-Driven Agent for Interactive Web Development](https://arxiv.org/abs/2609.02088)
*Yilong Guo,Hanqi Chen,Zixiao Ye,Guanzhong Wang,Chen Yu,Zeyu Chen*

Main category: cs.CV

TL;DR: RILA是一个引入浏览器执行反馈的交互式网页生成智能体，通过迭代优化提升了网页的交互功能和视觉还原度。


<details>
  <summary>Details</summary>
Motivation: 现有网页生成模型过于关注视觉美观与布局，忽略了交互功能的验证与实现，导致生成的网页难以实际运行。

Method: 提出了RILA智能体，包含Action Interaction Verification (AIV)模块用于重放交互轨迹，以及Execution-aware Rendering Score (ERS)用于指导迭代优化；同时构建了执行验证型数据合成流水线。

Result: RILA在IWR-Bench上显著提升了交互与视觉保真度，其中Qwen3.5-9B模型性能从40.40%提升至57.52%，超越了Kimi-K2.6和GPT-5.5等大参数模型。

Conclusion: 引入浏览器渲染循环和执行反馈机制，能够有效解决多模态大模型在网页生成任务中交互功能缺失的问题，提升生成代码的实际可用性。
...
### [103] [Evidence-Guided Detection, Localization and Explanation for Text-Centric Image Forensics](https://arxiv.org/abs/2609.02097)
*Peifeng Liu,Bin Li,Qingsong Zhang,Yangxin Yu,Leqing Chen,Xiaoye Qiu*

Main category: cs.CV

TL;DR: 本文提出了一种证据导向的检测-定位-推理系统，用于解决AIGC生成图像的真伪检测、空间定位及证据解释问题。


<details>
  <summary>Details</summary>
Motivation: AIGC技术进步导致图像篡改愈发容易，现有的取证技术缺乏对篡改区域的空间定位能力及基于证据的合理解释。

Method: 构建了由检测器、定位器和基于MLLM（多模态大模型）的推理器组成的级联系统；引入迭代难度感知挖掘提升定位质量，并采用报告-掩码一致性后处理对齐结果。

Result: 在ACM Multimedia 2026 GenText-Forensics挑战赛官方隐藏测试集上取得0.638分，排名第二。

Conclusion: 该证据导向的级联系统能有效实现从图像检测到证据生成的一体化取证，证明了多模块协同及证据流传递的有效性。
...
### [104] [Disease Burden over Skin Tone: Decomposing the Dermatology-AI Generalization Gap](https://arxiv.org/abs/2609.02111)
*Nirajan Kunwor,Sanjaya Poudel,Quoc-Huy Trinh,Jahidul Arafat,Sunil Kumar Gaire*

Main category: cs.CV

TL;DR: 该研究探讨了皮肤科AI在医疗资源匮乏地区的泛化问题，分析了肤色差异与疾病分布偏差对模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型多基于浅肤色和癌症数据集训练，在部署于多元化人群及不同病种场景时面临泛化挑战，需明确其根本原因。

Method: 对比了癌症预训练模型与皮肤科基础模型，利用DDI和SCIN数据集评估模型在不同肤色和病种下的表现，并采用标签无关的表征质量分析。

Result: 研究发现疾病分布偏差（Domain Shift）对性能的影响远大于肤色差异；基础模型的表征能力决定了在少量标注数据下的适应潜力。

Conclusion: 皮肤科AI的泛化局限源于特征表征不足，而非单一的肤色偏见；使用高质量皮肤科基础模型仅需少量样本即可实现性能恢复。
...
### [105] [Synergistic Information Disentanglement for Omni-modal Slide Representation Learning in Computational Pathology](https://arxiv.org/abs/2609.02118)
*Mingxin Liu,Chengfei Cai,Anwen Lu,Pengbo Xu,Jun Li,Jinze Li,Depin Chen,Jun Xu*

Main category: cs.CV

TL;DR: 提出了一种基于部分信息分解（PID）理论的 $\Phi$-Omni 框架，通过最大化模态间的协同信息而非简单的冗余对齐，显著提升了病理图像多模态表示学习的效果。


<details>
  <summary>Details</summary>
Motivation: 现有全模态自监督学习在融合组织学、基因组学和临床报告时，往往因对比对齐导致“模态坍塌”，丢失了具有诊断价值的协同交互信息。

Method: 引入协同信息瓶颈（SIB）和 $\Phi$ID 目标函数，基于 PID 理论显式抑制模态间的边际冗余，并最大化不可约的协同信息。

Result: 在乳腺癌和肺癌数据集上预训练后，在五个外部数据集的八项任务中，相比监督学习和现有 SSL 基线，展现出更强的少样本迁移学习能力。

Conclusion: 通过解耦并强化多模态间的协同交互，$\Phi$-Omni 能有效捕捉全切片图像中高质量的诊断特征，优于传统的对比学习方法。
...
### [106] [World-Coherent Decoding: Self-Verifying Test-Time Planning for World Action Models](https://arxiv.org/abs/2609.02159)
*Chuhan Zhang,Seiji Ito,Kenta Hoshino,Satoshi Ikehata,Ikuro Sato*

Main category: cs.CV

TL;DR: 提出了一种名为WCD的测试时规划框架，通过内部生成信号评估并筛选世界模型生成的视觉未来，从而提升机器人动作决策的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的世界动作模型（WAMs）在生成未来时存在随机性，导致其性能高度依赖于选取的路径，缺乏有效的可靠性评估机制。

Method: 引入World-Coherent-Decoding (WCD)框架，利用基于流的视频惊奇度（visual surprisal）和动作路径努力程度作为评估标准对候选轨迹进行筛选，并结合在线预测器对执行结果进行审计以优化后续选择。

Result: 在RoboTwin 2.0基准测试中，WCD将Hard任务的成功率从55.80%提升至60.90%，在Horizon-3任务中取得了显著增益，并在实际机器人视觉漂移测试中表现出较强的稳健性。

Conclusion: 对于世界动作模型而言，测试时的扩展效果主要取决于对可靠未来路径的筛选，而非单纯增加采样数量。
...
### [107] [Progressive Pseudo-Label Optimization for Point-Supervised Change Detection](https://arxiv.org/abs/2609.02171)
*Hailong Ning,Hao Wang,Yimeng Wang,Tao Lei,Renwei Dian,Asoke K. Nandi*

Main category: cs.CV

TL;DR: 提出了一种基于SAM2先验的弱监督变化检测框架，通过两阶段优化策略在仅使用点标注的情况下实现高精度变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有基于点监督的变化检测（PS-CD）受限于标注信息的空间稀疏性，导致生成的伪标签不完整且存在噪声。

Method: 采用两阶段架构：阶段一引入SAM2生成物体候选掩码，配合双时相掩码筛选策略生成伪标签，并通过不确定性感知损失进行边界细化；阶段二构建教师-学生自训练框架，通过闭环迭代持续优化伪标签与模型性能。

Result: 在WHU-CD、LEVIR-CD和SYSU-CD三个基准数据集上，该方法优于现有的弱监督方法，并达到与部分全监督方法相当的性能。

Conclusion: 该方法通过有效整合SAM2的视觉先验与自训练机制，显著缓解了点监督下的标签质量问题，提升了变化检测的鲁棒性和精确度。
...
### [108] [CC-4DGS: Computational Deformation and Point-Cloud Compression for Storage-Efficient Dynamic Gaussian Splatting](https://arxiv.org/abs/2609.02184)
*Kyungdae Park,Chae Eun Rhee*

Main category: cs.CV

TL;DR: CC-4DGS 是一种存储高效且可扩展的 4D 高斯溅射框架，通过优化变形建模和属性存储，实现了大幅度压缩。


<details>
  <summary>Details</summary>
Motivation: 现有的 4D 高斯溅射方法依赖庞大的哈希表和高维属性，导致模型存储占用过大（数十至数百 MB），限制了其应用范围。

Method: 提出了计算变形场（CDF）替代大型哈希表，并利用条件自动编码、选择性量化和残差码本压缩规范化点云属性（CCA）。

Result: 将模型存储降至 20-30 MB，同时保持了与 Swift4D 等先进方法相当的重建精度和实时渲染性能。

Conclusion: CC-4DGS 在保持高质量渲染的同时，极大地提高了存储效率，实现了更好的实时渲染与内存占用平衡。
...
### [109] [Lightweight Adaptation of General-Purpose VLMs for Multispectral and SAR Image Understanding](https://arxiv.org/abs/2609.02187)
*Shanji Liu,Kelu Yao,Junxiao Xue,Chenghui Lv,Xiangyang Miao,Yekai Huang,Yaying Chen,Chao Li*

Main category: cs.CV

TL;DR: 提出了一种无需训练专用视觉编码器即可将通用视觉语言模型（VLM）适配于多光谱和SAR遥感图像的新方法。


<details>
  <summary>Details</summary>
Motivation: 大多数预训练VLM基于三通道图像，难以直接处理多光谱或SAR数据，而重新训练领域专用编码器成本高且缓慢。

Method: 将传感器观测数据渲染为多视图（光学+SAR），在提示词中命名视图，并通过LoRA微调语言网络和部分视觉Transformer块，同时结合结构化监督和证据导向的偏好对齐。

Result: 在BigEarthNet-v2基准测试中，适配后的Qwen3-VL达到0.8275微观F1值，且该方法在多种VLM架构、洪水验证及遥感图像标注任务中表现出良好的迁移能力。

Conclusion: 通过输入渲染和轻量级LoRA适配，无需训练全新的基础模型，即可有效提升VLM在多光谱和SAR任务中的应用表现。
...
### [110] [TAME: Temporal-Aware Mixture-of-Experts for Text-Video Retrieval](https://arxiv.org/abs/2609.02204)
*Uicheol Jung,Juyoung Hong,Hojung Kwon,Yukyung Choi*

Main category: cs.CV

TL;DR: TAME 是一种基于 CLIP 的文本-视频检索框架，通过引入稀疏专家混合 (MoE) 和时间感知机制，解决了传统模型缺乏视频时间建模能力的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于图像-文本的模型（如 CLIP）在处理视频时缺乏时间建模，难以捕捉视频中的帧级异质性和时间结构，导致检索性能受限。

Method: 1. 在 CLIP 编码器中引入稀疏 MoE 层并应用帧一致性路由；2. 引入 Frame-Temporal (FT) token 用于聚合和反馈全局跨帧信息；3. 设计 Cross-Temporal Interaction and Aggregation (CTIA) 模块进行阶段性时间过滤与融合。

Result: TAME 在 MSR-VTT、DiDeMo、MSVD、LSMDC 和 ActivityNet 等基准测试上均取得显著提升，其中在 MSR-VTT 上的 R@1 比 CLIP4Clip 提高了 4.0。

Conclusion: 该方法通过建模帧级结构与时间关系，有效增强了预训练视觉-语言模型在视频检索中的长距离时间依赖捕获能力。
...
### [111] [LeakageBench: Document-Level Leakage Risk for Redacting Personally Identifiable Information in Document Images](https://arxiv.org/abs/2609.02207)
*Vishnu Prasad Vijaya Kumar,Santhosh Venkatesh,Ivan P. Yamshchikov*

Main category: cs.CV

TL;DR: LeakageBench 是一个评估文档图像中 PII（个人身份信息）脱敏效果的基准测试集，旨在弥补现有文本中心化评价的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的 PII 脱敏研究大多基于纯文本，忽略了文档图像（扫描件、截图）中的 OCR 错误、布局噪声对敏感信息识别的影响，缺乏文档级的安全保障评估。

Method: 构建了包含 500 张文档图像和 11,954 个 GDPR 标准 PII 标注的 LeakageBench 数据集，并对比评估了 OCR 流水线、商业检测器及视觉语言模型在不同指标下的表现。

Result: 实验发现即使借助工具（如 Code Interpreter）提升了定位 F1 分数，文档级的泄露率依然高达 0.968，说明现有方法无法保障大部分文档的脱敏安全性。

Conclusion: 目前的 PII 脱敏技术在文档图像领域仍面临重大挑战，提升局部定位能力并不等同于实现整体文档安全，LeakageBench 为评估该领域的高召回率需求提供了关键诊断工具。
...
### [112] [Asymmetric Paired-Annotation Learning for Multi-Structure ULF Pediatric Brain MRI Segmentation](https://arxiv.org/abs/2609.02210)
*Ha-Hieu Pham,Dang P. M. Cao,Minh Hoang Pham,Khanh Nguyen Vo Ngoc,Thanh-Huy Nguyen,Ulas Bagci,Huy-Hieu Pham*

Main category: cs.CV

TL;DR: 提出了一种名为AURA的基于nnU-Net的非对称监督策略，用于解决超低场（ULF）MRI脑部影像分割中因解剖边界模糊及参考标准冲突导致的问题。


<details>
  <summary>Details</summary>
Motivation: 超低场MRI影像（0.064T）解剖边界弱、结构可见度低，且高场参考图像存在局部配准误差，导致传统监督方法难以获得理想的分割效果。

Method: 采用非对称监督策略，将高场衍生（HF）掩码和低场编辑（LF）掩码视为不同的观测值，引入基于标签差异、边界、预测不确定性及类可靠性的有界可靠性门控机制来引导模型训练。

Result: 在16例开发集上，AURA方法与高场监督基线表现相当（Dice得分约0.79），且其集成模型在边界指标（HD95和ASSD）上表现优异。

Conclusion: AURA通过有效处理冲突性的多源标注，为超低场MRI分割提供了稳健的解决方案，未来将在更大规模测试集和外部队列中进一步验证其效能。
...
### [113] [FuDU: A Fuzzy Dual-dimensional Uncertainty Framework for Streaming Active Learning in Industrial Defect Detection](https://arxiv.org/abs/2609.02212)
*Zhaoyang Wang,Haiyong Chen,Binyi Su,Xinwei Lyu*

Main category: cs.CV

TL;DR: 本文提出了一种名为FuDU的流式主动学习框架，用于工业缺陷检测中的高可靠性样本挖掘。


<details>
  <summary>Details</summary>
Motivation: 工业实时缺陷检测需要应对连续媒体流中的不确定性样本，以提升系统在关键质量检查任务中的可靠性。

Method: 设计了基于原型的全局不确定性量化(PGUQ)模块评估图像级不确定性，利用双熵缺陷不确定性评估器(DeUE)量化框级不确定性，并通过模糊推理融合双维度不确定性实现自适应采样。

Result: 实验证明该方法高效且灵活，在核燃料棒等复杂工业检测任务中表现出色。

Conclusion: FuDU框架通过结合专家知识驱动的模糊决策，有效提升了工业缺陷检测系统在处理不确定样本时的可靠性。
...
### [114] [Signal or Noise? Auditing Rotation-Induced Saliency Drift in Medical and Aerial Imaging](https://arxiv.org/abs/2609.02224)
*Khawaja Murad ul Hassan,Mehran Ebrahimi*

Main category: cs.CV

TL;DR: 该研究指出Post-hoc显著性图（如Grad-CAM）在输入旋转时存在不稳定的漂移问题，并提出EquiGrad-CAM这一无需训练的改进方法，通过多视角旋转对齐显著性图，显著提升了可解释性的等变性。


<details>
  <summary>Details</summary>
Motivation: 在无固定方向的领域（如病理学和航空影像）中，模型预测不变但显著性图随输入旋转而漂移，导致解释不可靠，研究旨在揭示这种漂移的本质并提供解决方案。

Method: 通过分解CAM算子各阶段的旋转等变性，发现漂移源于空间激活张量的移动；提出EquiGrad-CAM，通过对多角度旋转输入取平均并进行反向旋转对齐，消除解释的不稳定性。

Result: EquiGrad-CAM在ImageNet-1K上大幅提升了显著性图的等变性（ResNet-50提升36%，ViT-B/16提升247%），并有效提升了CLIP的零样本解释一致性。

Conclusion: 显著性漂移是分类器丢弃的自由度造成的“噪声”，通过EquiGrad-CAM可以在不重新训练模型的情况下获得更鲁棒、更符合因果关系的视觉解释。
...
### [115] [InfraPatch: Cross-Task Targeted Grayscale Patch Attacks on Infrared-Adapted Vision-Language Models](https://arxiv.org/abs/2609.02233)
*Chengyin Hu,Dingyi Lu,Jiaju Han,Xiang Chen,Weiwen Shi,Jiahuan Long,Yiwei Wei,Jiujiang Guo*

Main category: cs.CV

TL;DR: 本文提出了InfraPatch，这是首个针对红外视觉-语言模型（IR-VLMs）的针对性数字灰度补丁攻击框架。


<details>
  <summary>Details</summary>
Motivation: 红外视觉-语言模型在低能见度下虽表现出色，但其在对抗性攻击下的鲁棒性尚不明确，现有研究多集中于RGB领域，缺乏针对红外模态的定向语义攻击评估。

Method: 开发了InfraPatch框架，通过白盒优化生成紧凑的单通道补丁（占用约5%区域），并结合代理引导放置策略和任务自适应语义目标，对红外图像进行干扰。

Result: 在十种红外适配模型变体上，InfraPatch实现了86%至100%的定向攻击成功率，并在分类、图像描述及视觉问答任务中表现出显著的威胁性。

Conclusion: 研究证明了红外多模态系统极易受到小尺寸灰度补丁的语义误导，强调了提升此类系统对抗鲁棒性的紧迫性。
...
### [116] [SAUF-Net: Structure--Appearance Representation Learning with Uncertainty Feedback for Semi-Supervised Medical Image Segmentation](https://arxiv.org/abs/2609.02247)
*Qin Lu,Zheyang Jing,Yujie Yang,Jianwang Li,Chen Yi,Shaofeng Jiang*

Main category: cs.CV

TL;DR: 提出了一种名为SAUF-Net的半监督医学图像分割框架，通过结构与外观特征解耦及不确定性反馈机制，提升模型在少标注数据下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有半监督方法过度依赖预测一致性，忽略了内部特征表示的可靠性，且医学图像中结构特征易与外观变异混淆，导致伪标签不可靠及误差积累。

Method: 引入结构-外观解耦模块（SADM）分离特征；通过解耦引导模块（DGM）增强结构感知；设计辅助解码器与外观交换一致性分支；使用带可靠性映射的双头判别器提供特征级不确定性反馈。

Result: 在ISIC-2016和Kvasir-SEG数据集上进行了大量实验，结果表明SAUF-Net在低标注数据场景下显著优于当前主流的半监督学习方法。

Conclusion: 通过显式的结构-外观特征分解和不确定性引导学习，能有效解决医学图像半监督分割中的特征不一致与噪声伪标签问题，提高了模型的稳健性。
...
### [117] [Handwriting Trajectory Recovery via Autoregressive Ordered Stroke Instance Prediction](https://arxiv.org/abs/2609.02251)
*En-Guang Wang,Yan-Ming Zhang,Fei Yin,Cheng-Lin Liu*

Main category: cs.CV

TL;DR: 提出了一种基于两阶段框架的手写轨迹恢复方法，通过先预测有序笔画再重建连续笔画内运动，解决了离线手写图像中时间信息丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 离线手写图像丢失了笔画顺序、方向和笔尖运动等动态信息，且现有方法往往直接预测完整轨迹，缺乏对笔画级结构的利用。

Method: 提出了一个两阶段框架：第一阶段采用自回归方式预测有序笔画实例；第二阶段利用方向相关的结构线索生成笔画内的连续轨迹。

Result: 在中文手写数据集上表现优于现有基准方法，且具有良好的跨类别泛化能力以及对英文和泰米尔文的扩展性。

Conclusion: 该研究证明了基于顺序的笔画预测策略在手写轨迹恢复任务中比后期处理更有效，且采样密度对性能评估有重要影响。
...
### [118] [T2LSC-Bench: Benchmarking Localized Semantic Control in Text-to-Image Generation](https://arxiv.org/abs/2609.02255)
*Yan Wang,Xinyi Hou,Weiguo Lin,Junjun Si,Siwei Ma*

Main category: cs.CV

TL;DR: 本文提出了T2LSC-Bench基准，用于评估文本到图像模型中“目标文本相关语义泄露”现象，即文本语义无意中扩散到锚点区域之外的视觉内容中。


<details>
  <summary>Details</summary>
Motivation: 现有基准多关注可读性和拼写准确性，忽视了文本语义对周围场景的负面影响（语义泄露），这在产品标签和界面设计中尤为关键。

Method: 构建了包含1,200个提示词的诊断基准T2LSC-Bench，并采用OCR-VLM与结构化VLM判断相结合的双分支协议，测量TAA、SSP、SLR和cSLR等指标。

Result: 实验发现，尽管模型在文本渲染准确率（TAA）上保持较高水平，但在特定条件下语义泄露率（SLR）可高达18.1%，且证实了抗泄露提示词可有效降低该比例。

Conclusion: 文本渲染的准确性并不代表局部文本控制的可靠性，语义泄露是一个被低估的瓶颈，本文提出的评估方法可有效量化并改善这一问题。
...
### [119] [MAOL: Morphology-Aware Ordinal Learning for Fine-Grained Industrial Defect Severity Grading](https://arxiv.org/abs/2609.02266)
*Zhaoyang Wang,Haiyong Chen,Binyi Su,Kun Liu,Kun Wang,Xianen Zhou,Atik Shahariar*

Main category: cs.CV

TL;DR: 提出了一种名为MAOL的形态感知序数学习框架，用于解决工业缺陷的细粒度严重程度分级问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理缺陷标签的序数性、对形态特征的依赖以及两阶段管道中预测噪声带来的训练与测试不一致问题。

Method: 引入形态感知特征增强表征；设计类条件自适应序数阈值建模分级边界；利用定位扰动进行预测感知训练以增强鲁棒性。

Result: 在洁净ROI和预测实例设置下均优于现有的规则方法、分类模型及序数基准，在IDA 2026挑战赛中获得第三名。

Conclusion: MAOL框架通过显式形态建模和预测感知训练，显著提升了工业缺陷细粒度分级的准确性和对噪声预测的鲁棒性。
...
### [120] [RouteGraph-Mona: Confusion-Aware Routing Fine-Tuning for Mineral Image Classification](https://arxiv.org/abs/2609.02282)
*Jierui Li,Zhiyuan Qi,Hao Zhu,Yufan Liu,Jixian Liu,Shaojie Jiang,Jianda Wang,Yaqi Liu,Xiaotong Li,Wei Wang*

Main category: cs.CV

TL;DR: RouteGraph-Mona 是一种用于矿物图像分类的轻量级参数高效适配器，通过引入样本自适应路由和路由空间正则化，解决了多尺度特征聚合的静态局限和类间相似性混淆问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法 Mona 在多尺度特征聚合上采用静态策略，难以适应不同样本的尺度需求，且在处理视觉高度相似的矿物类别时表现不佳。

Method: 引入样本自适应路由机制替换静态聚合，构建路由空间；通过类级路由锚点（route anchors）和混淆加权边界（confusion-weighted margins）对路由特征进行正则化。

Result: 在三个矿物图像数据集上表现优于原始 Mona 框架，在保持高效参数的同时，分类准确率达到并超越了主流微调方法和现有基线。

Conclusion: 通过引入自适应路由与针对性的正则化约束，RouteGraph-Mona 有效提升了预训练模型在处理具有细微差别和多尺度特征的矿物分类任务中的判别能力。
...
### [121] [Diffusion-Encoding Gaussian Field for Joint k-q dMRI Reconstruction](https://arxiv.org/abs/2609.02288)
*Zhibo Chen,Yajuan Huang,Yu Guan,Qiuyun Fan,Dong Liang,Qiegen Liu*

Main category: cs.CV

TL;DR: 提出了一种基于主体特定空间-角度高斯场（Spatial-Angular Gaussian Field）的自监督学习方法，用于扩散MRI的联合k-q空间重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用DWI数据在不同方向间共享的解剖结构信息，导致空间重建误差被误认为是角度变异，进而影响对未观测方向的预测。

Method: 使用共享的3D高斯基元作为空间支撑，每个基元包含连续的q空间条件张量残差响应，通过多重基元重叠合成信号，并直接从欠采样k空间进行自监督优化。

Result: 在HCP数据集上，该方法在缺失方向的DWI重建、张量衍生指标及主扩散方向估计方面均优于现有方法。

Conclusion: 该方法通过耦合空间区域和扩散方向，无需全采样目标或预留方向即可实现高质量的联合重建，有效解决了空间与角度采样间的权衡问题。
...
### [122] [VoRTeC: Taming Foundation Flow for One-step Real time Video Compression](https://arxiv.org/abs/2609.02291)
*Yichong Xia,Qinhong Wu,Qinhong Wu,Jinpeng Wang,Zeyuan Chen,Haoqian Wang*

Main category: cs.CV

TL;DR: 提出了一种基于流模型（Wan2.1）的视频压缩框架VoRTeC，在超低比特率下实现了高保真度、高帧率的单步解码。


<details>
  <summary>Details</summary>
Motivation: 传统神经压缩易产生模糊伪影，而现有扩散模型压缩存在解码延迟高和时间一致性差的问题。

Method: 利用Wan2.1基础流模型，通过压缩潜在表征、预测流轨迹位置及多尺度先验集成，结合尾帧重用和先验缓存机制实现单步高效解码。

Result: 相比扩散模型压缩，比特率降低了58%，解码速度提升了3至197倍，在720p下可达13 FPS，480p下可达32 FPS。

Conclusion: VoRTeC成功利用生成式视频流先验，在保持高感知质量的同时解决了生成式压缩的计算效率瓶颈。
...
### [123] [If It Moves, Radar Knows: A Physics-Aware Radar Transformer for Class-Agnostic Moving-Object Detection](https://arxiv.org/abs/2609.02289)
*Yinghao Sun,Shuguang Li,Jinliang Shao,Tieshan Li*

Main category: cs.CV

TL;DR: 提出了一种名为PART的物理感知雷达Transformer，旨在利用稀疏雷达数据实现对移动物体的类别无关检测，以应对自动驾驶中的长尾和极端工况。


<details>
  <summary>Details</summary>
Motivation: 闭集检测器难以识别训练类别之外的稀有移动物体，而现有雷达检测面临数据稀疏、噪声大及几何恢复困难的问题。

Method: 设计了Doppler-Aware Query Initialization (DAQI)进行查询初始化，采用Physics-Guided Cross-Attention (PGCA)进行物理特征约束，并引入不确定性感知监督以降低对标注完整性的依赖。

Result: 在nuScenes数据集上实现了高水平的类别无关平均精度（CA-AP 0.8827），在稀有及安全关键类别上达到0.9203的召回率，且在雨夜及遮挡环境下表现稳健。

Conclusion: PART通过结合雷达多普勒物理特性与稀疏Transformer架构，无需依赖完整标注即可实现鲁棒的运动物体检测，有效提升了自动驾驶的安全性和感知泛化能力。
...
### [124] [ORB-SVM : An Innovative Hybrid Framework for Efficient Brain Tumor Detection from MRI Scans](https://arxiv.org/abs/2609.02333)
*Amirhosein Azarpour*

Main category: cs.CV

TL;DR: 提出了一种基于ORB特征提取与SVM分类器的混合框架，用于高效、准确的脑肿瘤MRI图像分类。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法参数量大、计算成本高且依赖海量数据，且MRI图像判读易受人为因素和噪声干扰。

Method: 利用ORB算法进行特征提取以实现数据降维，并结合核函数支持向量机（SVM）进行分类。

Result: 该方法实现了约99.5%的数据降维，并在Br35H数据集上达到了97.5%的分类准确率。

Conclusion: 该方法在保持高性能的同时显著降低了计算资源需求，证明了轻量化特征表示在医疗影像分析中的有效性。
...
### [125] [Domain shift-robust object detection with GenAI image editing](https://arxiv.org/abs/2609.02299)
*Isabel D. Stein,Thijs A. Eker,Sebastiaan P. Snel,Ella P. Fokkinga,Klamer Schutte,Luca Ambrogioni,Friso G. Heslinga*

Main category: cs.CV

TL;DR: 该研究利用扩散模型生成的图像编辑技术，通过模拟伪装效果进行数据增强，显著提升了目标检测器在域外场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 目标检测器在面临光照、天气和遮挡等域偏移时性能下降，且在低数据场景下获取多样化训练样本十分困难。

Method: 利用 Qwen Image Edit 和 Flux.2 Dev 等扩散模型对训练数据进行“合成伪装”编辑，并结合 LoRA 微调模型，以 GroundingDINO 为检测器评估其在伪装车辆检测任务中的泛化表现。

Result: 相比基线模型，生成的伪装增强在叶片伪装上提升了 20.1 mAP，在网状伪装上提升了 14.4 mAP，LoRA 微调在多光谱伪装任务中也取得了显著改进。

Conclusion: 基于生成式图像编辑的数据增强能够有效模拟域偏移，是弥补源域与目标域差距、增强模型域外泛化能力的有效手段。
...
### [126] [Blending Concepts: Benchmarking Visual Metaphor Generation in Text-to-Image Models](https://arxiv.org/abs/2609.02502)
*Chuer Chen,Zichen Wang,Yi He,Zhengxi Yu,Nan Cao*

Main category: cs.CV

TL;DR: 本文提出了首个视觉隐喻生成评测基准 VMetaphor-Bench，并揭示了现有文生图模型在处理跨领域抽象概念映射方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型虽然能精准生成实体，但在表达复杂视觉隐喻这一抽象概念能力上尚缺乏系统性的评估与研究。

Method: 构建包含1500个视觉隐喻的基准数据集，并采用多模态大模型作为评判者，通过多项选择题与多维度评分协议进行系统评测。

Result: 通过对11个代表性文生图模型的评估，发现即使是顶尖的闭源模型在合成结构构建和跨领域隐喻映射方面表现依然欠佳。

Conclusion: 视觉隐喻生成是文生图研究的重要前沿，现有的模型仍难以捕捉隐喻表达中的抽象逻辑与创意构图。
...
### [127] [ShallowStream: Index Shallow then Answer Deep for Streaming Video Understanding](https://arxiv.org/abs/2609.02780)
*Jitai Hao,Ke Yang,Qiang Huang,Jun Yu*

Main category: cs.CV

TL;DR: ShallowStream 是一种旨在降低多模态大模型（MLLM）处理流式视频计算开销的新型框架。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频处理方法多关注视觉 token 剪枝或量化，忽略了模型深度带来的计算冗余，导致重复执行全深度预填充（prefill）开销巨大且 KV cache 增长过快。

Method: 利用 MLLM 的浅层（shallow layers）同步进行帧编码和索引构建，维护一个轻量级的实时索引，并在查询阶段利用浅层注意力得分进行多样性驱动的帧检索。

Result: 在保持与现有领先方法相当性能的同时，将单帧预填充延迟降低了 52.1 倍，10 秒端到端延迟降低了 11.9 倍。

Conclusion: 通过复用浅层计算资源实现高效的视频流理解与检索，有效地解决了流式处理中的计算瓶颈问题。
...
### [128] [YesTrack: Referring Multi-Object Tracking via MLLM-based Yes/No Verification](https://arxiv.org/abs/2609.02318)
*Quansheng Hu,Qin Sun,Qiansen Dai,Jin Ding,Wan Zhang,Xue Zhou,Jianxiao Zou*

Main category: cs.CV

TL;DR: YesTrack 是一种基于 MLLM 的指称多目标跟踪（RMOT）方法，通过将任务重构为判别式“是/否”验证，消除了生成式任务的冗余性。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅将 MLLM 作为标题生成器，导致额外的推理延迟且未能充分利用 MLLM 的视觉-语言对齐能力。

Method: 提出 YesTrack，将 RMOT 转变为判别式任务，直接利用 MLLM 进行“是/否”验证，并引入时间置信度先验（TCP）和时间参考传播（TRP）以增强时序一致性。

Result: 在 Refer-KITTI 和 Refer-KITTI-V2 数据集上，该方法在保持高效率的同时显著优于现有 SOTA 方法。

Conclusion: YesTrack 证明了利用 MLLM 进行判别式任务处理在 RMOT 和通用 MOT 任务中具有极高的有效性和泛化能力。
...
### [129] [Fine-Grained Anomaly Perception in Wild UGC-Enhanced Images: A Comprehensive Dataset and Difference-Fusion Framework](https://arxiv.org/abs/2609.02529)
*Yan Zhong,Gefei Chen,Qiufang Ma,Zhen Wang,Zhiwei Fan,Lei Shi,Tingting Jiang*

Main category: cs.CV

TL;DR: 针对短视频和社交媒体中图像增强算法引入的局部视觉异常问题，提出了全新的异常感知任务（UEAP）及其基准数据集UEAP-4k，并设计了名为DFAP-UGC的感知模型。


<details>
  <summary>Details</summary>
Motivation: 现有图像质量评估（IQA）方法多关注整体质量，难以有效捕捉图像增强过程中在人脸、文本和纹理等局部产生的视觉畸变。

Method: 提出了Difference-Fusion Anomaly Perception (DFAP-UGC) 方法，结合差异融合、密集空间查询、区域验证及质量感知排序；并采用Locality-Aware Dynamic Task Prioritization (LADTP) 策略实现端到端的任务学习。

Result: 构建了包含细粒度标注的UEAP-4k数据集，实验证明DFAP-UGC在检测增强图像中的局部异常方面优于传统基准方法。

Conclusion: 该研究定义的UEAP任务有效弥补了现有IQA在增强图像局部异常检测方面的空白，提出的模型具有鲁棒性和优越性。
...
### [130] [Towards Zero-Shot Transfer Across Embodiments For Driving VLAs](https://arxiv.org/abs/2609.02341)
*Caio Azevedo,Stefano Sabatini,Sascha Hornauer,Fabien Moutarde*

Main category: cs.CV

TL;DR: 该研究探讨了自动驾驶中视觉-语言-动作（VLA）模型的多数据集训练方法，并提出了“BEV-Forcing”技术以增强模型对空间布局的理解。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域的VLA模型常局限于单一数据集，缺乏跨数据集和跨相机配置的泛化能力，且简单堆叠数据并不总能提升性能。

Method: 引入BEV-Forcing辅助目标，将专业鸟瞰图（BEV）模型的空间布局信息迁移至VLA骨干网络，通过共享BEV空间接口强化目标位置表征。

Result: 在少量相机配置下，BEV-Forcing显著提升了模型在分布内及分布外的表现；但在大规模增加数据多样性后，该辅助任务的边际效应递减。

Conclusion: 研究表明，随着训练多样性和数据规模的扩大，某些特定的辅助技术优势可能会减弱，强调了在评估新技术时考虑数据规模效应的重要性。
...
### [131] [RVSD: Retrieval Vision Sparse Decoding for Mitigating Visual Hallucinations in Large Vision-Language Models](https://arxiv.org/abs/2609.02731)
*Canjie Liu,Jiawen Kang,Jinbo Wen,Zishao Zhong*

Main category: cs.CV

TL;DR: RVSD 是一种无需训练、即插即用的解码框架，通过结合令牌稀疏化和语义空间视觉检索来减少大型视觉语言模型的视觉幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有缓解大型视觉语言模型（LVLM）视觉幻觉的方法通常需要额外的训练或复杂的解码过程，计算开销大且灵活性不足。

Method: 提出了语义导向的令牌选择策略进行稀疏化，并引入语义空间视觉检索（SSVR）机制，将视觉补偿转化为共享语义空间内的按需跨模态检索过程。

Result: 实验表明，RVSD 在减轻视觉幻觉方面达到了行业领先水平，特别是在长上下文生成任务中表现出极强的抑制能力。

Conclusion: RVSD 提供了一种高效且无需训练的方案，有效解决了 LVLM 中的视觉幻觉问题，且具有良好的计算效率和泛化性能。
...
### [132] [TempoGround: State-Aware Streaming Visual Grounding with Vision-Language Models](https://arxiv.org/abs/2609.02359)
*Leqian Ding,Junning Qiu,Manwen Yang,Yu Guo,Fei Wang*

Main category: cs.CV

TL;DR: TempoGround 是一种针对流式输入视觉定位任务的 VLM 原生框架，通过跨帧物体对应关系建模与强化学习策略，显著提升了目标追踪的持续性与一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位方法在流式输入场景下常面临身份漂移、跨帧不一致以及遮挡导致定位失效等挑战。

Method: 提出了基于状态感知跨帧对应的课程预测机制，并引入了“流式定位强化”(SGR) 学习策略，通过验证性定位、身份及一致性奖励对模型进行优化。

Result: 在多项流式输入基准测试中，TempoGround 在 2D 和 3D 定位指标上均表现优异，显著优于现有方法。

Conclusion: TempoGround 有效解决了流式场景下的时序一致性难题，为视觉定位提供了更稳健的实用基础。
...
### [133] [Structured-Prior-Guided Diffusion Inpainting with Physical Consistency for Traffic Sign Augmentation](https://arxiv.org/abs/2609.02348)
*Luo Li,Chongchong Huang,Jun Jia,Qiang Gao,Xinlong Liu,Gui Yang,Liang Cao*

Main category: cs.CV

TL;DR: 提出了一种基于结构化先验的扩散修复框架，专门解决交通标志检测中长尾数据分布下的生成增强难题。


<details>
  <summary>Details</summary>
Motivation: 通用修复模型在交通标志生成中存在数字扭曲、几何畸变和色彩偏差，无法满足交通标志对物理一致性和精确度的严苛要求。

Method: 引入三路径结构化先验注入（JSON文本、IP-Adapter色彩模板、ControlNet仿射对齐模板）和物理一致性损失函数（CIELAB色彩与Sobel边缘梯度），在Stable Diffusion 1.5基础上进行自监督训练。

Result: 在TT100K-2021数据集上表现优于FLUX.1等大模型，OCR准确率达到91.1%，推理速度快14倍，且显著提升了罕见交通标志类别的检测性能。

Conclusion: 通过物理约束和多模态先验注入，该框架能高效、精准地生成具有物理一致性的交通标志图像，有效缓解了数据长尾分布带来的检测瓶颈。
...
### [134] [GlyphAnchor: Enhancing Visual Text Rendering via Position-Anchored Glyph Priors](https://arxiv.org/abs/2609.02349)
*Qiang Xiang,Shuang Sun,Binglei Li,Yibo Chen,Xu Tang,Yao Hu,Junping Zhang*

Main category: cs.CV

TL;DR: GlyphAnchor 是一种通过引入轻量级字形补丁条件来增强扩散变换器模型（DiT）精确渲染复杂文本能力的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有的图像生成模型在处理长文本、复杂布局或生僻字时表现欠佳，且现有的字形先验方法往往不够稳健或缺乏准确性。

Method: 引入字形补丁条件，将其位置锚定在目标图像的固有位置编码上；采用分阶段监督微调和文本感知后训练策略来提升鲁棒性。

Result: 在多种模型架构和测试集上，GlyphAnchor 在处理长、复杂、密集排列的文本及生僻字时，显著提高了文本忠实度，同时保持了良好的图像质量。

Conclusion: GlyphAnchor 有效解决了扩散模型中的文本渲染难题，并通过提出的 InfoTextBench 基准证明了其在生成和编辑任务中的优越性能。
...
### [135] [Information Density Imbalance in Visual Object Detection](https://arxiv.org/abs/2609.02369)
...
TL;DR: 该研究指出对象检测中的类别偏差不仅由样本数量决定，还与“信息密度”有关，并通过引入信息密度概念优化了损失函数，显著提升了模型性能与公平性。


<details>
  <summary>Details</summary>
Motivation: 现有研究过度关注样本数量不均衡导致的类别偏差，但研究发现即使在样本均衡的分布下，模型仍存在显著偏差，说明样本计数不能完全解释该现象。

Method: 定义并测量了“信息密度”指标，分析其与模型准确率的负相关性，并将该指标整合进三种先进的对象检测损失函数中进行改进。

Result: 实验表明信息密度与类别偏差密切相关，引入该指标能有效降低模型偏差并提升在Pascal VOC、COCO-LT及LVIS数据集上的整体性能。

Conclusion: 信息密度是解释对象检测模型类别偏差的新维度，为设计更公平的损失函数和训练策略提供了新的理论支撑与实践工具。
...
### [136] [ProSR: Semantic-Prototype-Guided Discrete Modeling for Physically Consistent SAR Super-Resolution](https://arxiv.org/abs/2609.02377)
*Byoungwoo Kim,Munchurl Kim*

Main category: cs.CV

TL;DR: 提出了一种基于离散令牌预测的SAR图像超分辨率重建模型ProSR，旨在解决现有生成式超分辨率模型导致的SAR物理散射特性丢失和结构失真问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于平滑逼近的扩散模型在处理高分辨率SAR图像时，无法有效保留其相干散射统计特性，导致图像产生不符合物理规律的随机结构失真。

Method: 将SAR超分辨率任务重构为量化潜在空间中的语义引导离散令牌预测任务；采用自监督学习提取先验，通过语义对齐细节编码将高频信号解耦为散射基元，并引入原型映射引导注意力机制以减少类间干扰。

Result: 在Umbra Open Dataset大规模基准测试中，ProSR在提升视觉质量的同时，成功保留了SAR图像的关键散射特性。

Conclusion: 通过将SAR超分辨率建模为离散化语义引导过程，能够有效规避过平滑问题，是实现高精度SAR图像重建的有效路径。
...
### [137] [CA-OPD: Confidence-Aware On-Policy Distillation for Structured Visual Prediction](https://arxiv.org/abs/2609.02401)
*Menghao Li,Linjie Mu,Yin Wang,Haotian Hu,Yannian Gu,Lujiayi Xue,Fanyi Wang*

Main category: cs.CV

TL;DR: 提出了一种基于置信度的策略梯度蒸馏框架 CA-OPD，通过自适应干预和监督策略解决视觉语言模型在自回归生成中的累积误差问题。


<details>
  <summary>Details</summary>
Motivation: 现有的在线策略蒸馏（OPD）方法在训练早期易受不可靠的学生预测影响，且缺乏利用教师置信度进行精细化 token 级监督的机制。

Method: CA-OPD 框架：利用教师置信度识别并修正不可靠的学生输出，采用“从严到宽”的调度策略平滑转换控制权，并针对修正与保留位置实施不同的对齐监督方案。

Result: 在 GUI 基础任务和 OCR 任务中，CA-OPD 显著提升了 Qwen3.5-0.8B 的性能，在 ScreenSpot-Pro 和 OCRBench-v2 上分别取得了 9.50 和 6.72 点的提升。

Conclusion: 该方法通过将干预决策与监督机制深度对齐，有效地缓解了视觉语言模型的累积误差，证实了策略控制权与监督质量协同优化的必要性。
...
### [138] [Uncertainty-Guided Adverse Weather Restoration via Gated Transformer Network](https://arxiv.org/abs/2609.02434)
*Zheke Jin,Yuning Cui,Tianle Jin,Alois Knoll,Hu Cao*

Main category: cs.CV

TL;DR: 提出了一种名为UAR-Net的通用全能型恶劣天气图像恢复网络，通过引入不确定性引导和多尺度特征融合，有效解决了空间异质性退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有天气恢复模型多依赖全局聚合和确定性目标，难以应对“全能型（AiO）”设置下复杂的空间异质性退化问题。

Method: 构建了带门控双尺度Transformer块（GDTB）捕捉全局/局部特征，采用平衡多尺度跳跃连接（BMSC）优化特征整合，并通过不确定性感知细化头（URH）及亮度感知能量损失（BAE-Loss）进行训练。

Result: 在多个恶劣天气基准数据集上均取得了SOTA性能，且能生成高质量的重构结果及可靠的不确定性估计。

Conclusion: 该框架通过深度集成多尺度特征处理与不确定性建模，显著提升了在复杂多变天气条件下的图像修复能力。
...
### [139] [The Diagnosis a Reporter Leaves Unspoken: Surfacing Frozen Tumor Features for Brain-Tumor MRI Reporting](https://arxiv.org/abs/2609.02411)
*Khawaja Murad ul Hassan,Ruqiyya Adil,Adil Qayyum,Rida Hassan,Asad Mansoor Khan,Muhammad Usman Akram,Mehran Ebrahimi*

Main category: cs.CV

TL;DR: NeuroFusion 是一种新型辅助脑部 MRI 报告生成系统，通过引入判别式分类头来挖掘模型内部的潜在诊断特征，显著提升了脑肿瘤诊断的准确性和报告质量。


<details>
  <summary>Details</summary>
Motivation: 现有的多链思维链（CoT）医学报告生成模型虽能生成文本，但在诊断准确性上表现较差（如混淆脑膜瘤与胶质瘤），模型内部虽含有诊断信息但未被有效利用。

Method: 提出 NeuroFusion 系统，在 Mistral-7B 主干网络的基础上，增加针对病灶特征的判别式分类头，并采用“先草拟后审查”的解码机制，利用受控诊断信息指导文本生成。

Result: 诊断召回率显著提升（脑膜瘤达0.92，转移瘤达0.75），报告质量指标全面超越基准模型，推理延迟降低至原来的 1/5-1/6，且生成的报告在专家盲测中表现最优。

Conclusion: 通过显式引导潜在诊断信号而非强制覆盖，能够显著增强医学报告生成模型的临床诊断可靠性，实现更高质量、更低延迟的医学影像分析。
...
### [140] [Adapting a Foundation Model for Lunar Surface Height Estimation](https://arxiv.org/abs/2609.02448)
*Patrick Bauer,Marius Schwinning,Melanie Siegel,Andreas Weinmann,Hichem Snoussi*

Main category: cs.CV

TL;DR: 本文通过对Depth Anything V2模型进行月球表面数据微调，实现了针对月球地貌的更精确相对高度估计，以辅助未来登月任务的危险探测。


<details>
  <summary>Details</summary>
Motivation: 传统月表DEM生成方法存在局限，现有通用的深度学习模型（如DAV2）在月球领域表现不足，缺乏针对月面特殊地貌的适应性。

Method: 利用公开的月球立体摄影测量（SPG）衍生的DEM数据，对Depth Anything V2（DAV2）模型进行领域自适应微调。

Result: 实验结果显示，微调后的模型在月球表面相对深度估计任务中表现出显著性能提升，超过了原始的零样本DAV2模型。

Conclusion: 该方法成功将通用的深度学习深度估计器转化为可靠的月表相对高度估算工具，为未来登月任务中的危险地形定位提供了有效支撑。
...
### [141] [WiFlow: Estimating Optical Flow using WiFi Channel State Information](https://arxiv.org/abs/2609.02452)
*Thomas Weigel,Simon Kiefhaber,Fabian Portner,Matthias Hollick,Simone Schaub-Meyer*

Main category: cs.CV

TL;DR: 该研究提出 WiFlow，利用 WiFi 通道状态信息（CSI）进行光学流估计，以解决摄像头隐私和光照依赖问题。


<details>
  <summary>Details</summary>
Motivation: 摄像头监测存在隐私泄露风险，且在弱光环境下性能受限，因此探索基于 WiFi 信号的非侵入式运动估计具有重要意义。

Method: 提出了 WiFlow 估计器、CSI 预处理器，并设计了三种不同复杂度与精度的模型架构；同时构建了首个用于训练和评估 CSI 光学流的数据集。

Result: 通过实验验证了 WiFi 信号在光学流估计任务上的可行性，并为该领域的设计要素提供了关键见解。

Conclusion: WiFlow 证明了 WiFi CSI 可作为摄像头在运动估计任务中的有效替代方案，具有隐私保护及环境鲁棒性强的优势。
...
### [142] [VIPS: Vehicle-Infrastructure Cooperative Planning Benchmark via Pseudo-Simulation](https://arxiv.org/abs/2609.02462)
*Hoonhee Cho,Jae-Young Kang,Giwon Lee,Hyemin Yang,Heejun Park,Kuk-Jin Yoon*

Main category: cs.CV

TL;DR: 提出了VIPS评估基准和CoS-V2X协作规划框架，旨在通过伪仿真技术解决车路协同（V2I）自动驾驶在评估与决策上的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶评估方法在开环（缺乏鲁棒性检验）与闭环（成本高、难以扩展）之间存在矛盾，且单车系统在复杂路口面临严重遮挡问题。

Method: 开发了VIPS伪仿真基准，融合车端与路端观测以评估鲁棒性；提出了CoS-V2X框架，利用稀疏表征实现高效的交互建模与协作规划。

Result: 通过VIPS基准验证了在复杂交互场景下的错误传播与鲁棒性；CoS-V2X在异构观测环境下表现出高效的通信能力与稳健的决策性能。

Conclusion: VIPS为V2I自动驾驶提供了一种既具扩展性又符合真实分布的评估方案，而CoS-V2X证明了稀疏特征表征在处理多智能体协同感知与规划任务中的有效性。
...
### [143] [UnCapsTSR: An Unsupervised Transformer-based Image Super-Resolution Approach for Capsule Endoscopy Images](https://arxiv.org/abs/2609.02476)
*Anjali Sarvaiya,Shubh Kawa,Lalit Agrawal,Jagrit Joshi,Kishor Upla,Kiran Raja*

Main category: cs.CV

TL;DR: 提出了一种名为UnCapsTSR的无监督Transformer GAN框架，用于提升无线胶囊内镜（WCE）图像的空间分辨率。


<details>
  <summary>Details</summary>
Motivation: WCE图像因设备尺寸和传输限制存在低分辨率问题，且缺乏成对的低/高分辨率训练数据。

Method: 利用无监督Transformer GAN架构，结合双边全变分（BTV）损失函数，并在无需LR-HR成对数据的情况下实现超分辨率重建。

Result: 在多项评估指标（NIQE, BRISQUE, PIQE及自研的EndoQM）上优于现有无监督超分辨率方法，EndoQM提升了40%至80%。

Conclusion: UnCapsTSR有效改善了WCE图像的视觉质量，其泛化能力在KID和GIANA数据集上得到了验证。
...
### [144] [Learning to Track from Privileged Target Appearances](https://arxiv.org/abs/2609.02471)
*Xin Chen,Jiao Xu,Dong Wang,Huchuan Lu,Kede Ma*

Main category: cs.CV

TL;DR: 提出了一种名为PATT的教师-学生训练框架，通过在训练期间利用“特权”信息（未来帧真实目标裁剪）来提升视觉追踪器的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉追踪器在推理时面临模板陈旧（初始模板）与定位不准（动态更新模板）的权衡，通过引入理想化预言机证明了利用精确当前帧信息可大幅提升性能，从而激发了该训练方案。

Method: 引入PATT（Privileged Appearance Transfer for Tracking）框架，由能获取过去、当前及未来精确目标裁剪的“特权教师”指导仅能获取过去模板的“学生”进行多级表示学习，并根据教师的相对优势及绝对精度进行加权迁移。

Result: 在七个基准测试中，PATT在多种模型规模下均实现了稳定的性能提升，显著缩小了现有追踪器与理论最优解之间的差距。

Conclusion: 利用训练阶段的特权信息辅助学生模型学习更稳健的特征表示是提升追踪性能的有效途径，且该方法在部署时无需额外计算开销。
...
### [145] [RGB-to-IR image translation for infrared vehicle detection in unseen UAV domains](https://arxiv.org/abs/2609.02556)
*Thijs A. Eker,Ella P. Fokkinga,Jan Erik van Woerden,Elfi I. S. Hofmeijer,Sebastiaan P. Snel,Klamer Schutte,Friso G. Heslinga*

Main category: cs.CV

TL;DR: 该研究探讨了利用生成式模型（如GAN、ControlNet、LoRA）将RGB图像转换为红外（IR）图像，以缓解红外航空目标检测中数据匮乏的问题。


<details>
  <summary>Details</summary>
Motivation: 红外航空图像获取困难，而基于RGB到红外转换的数据增强因红外特有的物理特性（如引擎热量）而面临跨模态迁移挑战。

Method: 利用配对的RGB-IR源数据集训练多种生成式转换器，对目标域的RGB图像进行转换生成合成红外数据，并用于训练RF-DETR检测器，同时探究了种子多样性和提示词变化对性能的影响。

Result: 基于Stable Diffusion 3.5与ControlNet的方案表现最佳，在Kust4K和VTUAV数据集上显著提升了mAP（分别从50.8提至60.1和从25.6提至38.4）。

Conclusion: 生成式RGB转红外转换能够有效填补跨模态空白，显著提升航空目标在红外域的检测性能，尽管与真实红外数据仍有差距，但其作为数据增强手段效果显著。
...
### [146] [Stereo 4D Radar for 3D Object Detection: Integrating Geometric Alignment and Absolute Velocity Estimation](https://arxiv.org/abs/2609.02560)
*Seung-Hyun Song,Dong-Hee Paek,Woong-Chan Byun,Seung-Hyun Kong*

Main category: cs.CV

TL;DR: 提出了一种基于立体4D雷达的3D目标检测框架，通过利用双雷达间的几何视差实现绝对速度估计及特征融合，显著提升了感知性能。


<details>
  <summary>Details</summary>
Motivation: 单体4D雷达预处理会导致数据稀疏，且仅能获取径向速度，限制了对物体完整运动状态的还原和感知鲁棒性。

Method: 利用左右双雷达的几何视差构建立体4D雷达框架，通过特征融合和绝对速度估计来增强空间定位与运动状态感知能力。

Result: 在自建立体4D雷达数据集上，该方法在AP 3D和AP BEV指标上分别超越当前最优单体4D雷达基准8.82和9.0个点。

Conclusion: 结合绝对速度估计与立体几何感知特征融合，能够有效解决雷达数据稀疏问题，显著提升3D目标检测的效果。
...
### [147] [Beauty is in the AI of the beholder: MLLMs systematically overrate facial attractiveness](https://arxiv.org/abs/2609.02512)
*Santiago Grandas,Juan Sebastian Cely-Acosta,Mohit Mendiratta,Shafee Hassan,Macken Murphy*

Main category: cs.CV

TL;DR: 该研究评估了主流多模态大语言模型（MLLM）在面部吸引力评分上与人类判断的一致性。


<details>
  <summary>Details</summary>
Motivation: 探讨现有的商业MLLM是否能准确反映人类对面部吸引力的审美判断，以及其评分是否存在系统性偏差。

Method: 通过对比2,513名人类参与者的评分与四种商业AI模型（Claude、Gemini、GPT、Grok）对相同面部图像的吸引力评分，分析相关性、分布特征及影响因素。

Result: MLLM倾向于给予更高的评分且范围更窄，虽无法完全复刻绝对评分，但在面部吸引力的“排序”上与人类表现出强相关性；模型间高度一致（除Grok外），且主要受年龄因素影响。

Conclusion: 虽然目前的MLLM能较好地近似人类的审美排序，但它们在绝对评分上存在系统性的“高估”倾向，尚不能完全替代人类的审美评价。
...
### [148] [MARS: What Retrieval Signals Are Hidden in Multimodal Large Language Models for Text-Video Retrieval?](https://arxiv.org/abs/2609.02565)
*Uicheol Jung,Juyoung Hong,Geuntaek Lim,Yukyung Choi*

Main category: cs.CV

TL;DR: MARS 是一种用于文本-视频检索的多层、多槽嵌入框架，通过整合不同隐藏层并使用硬负样本感知机制来增强细粒度特征表示。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大模型的嵌入方法通常仅使用最后一层输出的单一向量，导致信息压缩过大，难以区分具有相似场景或动作的视频，限制了细粒度检索性能。

Method: 提出了 MARS 框架，核心包括：1) 构建多层特征组合的自适应表示槽位；2) 槽位间的相似度匹配与聚合；3) 引入硬负样本感知槽位专业化目标，鼓励模型学习具有辨别力的特征 cues。

Result: 在四个文本-视频检索基准测试上取得了最先进的性能（SOTA），无论是直接检索还是重排序设置均表现优异。

Conclusion: 通过多层融合、多槽位架构以及针对性的负样本学习，能够有效解决多模态大模型在细粒度视频检索中的信息丢失问题。
...
### [149] [Deeply Interleaved Text-Image Contexts for Multimodal LLMs Assessment](https://arxiv.org/abs/2609.02573)
*Zihao Wang,Xi Xiang,Yuwen Sun,Yingyu Li,Yabo Zhang,Yihan Zeng,Fan Li,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 本文提出了TIC-Bench，这是一个专门用于评估多模态大模型在交错文本-图像上下文中进行深度语义交互和逻辑推理能力的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准多关注指令与图像的单一交互，忽略了现实世界中文本与图像深度交错的复杂应用场景，模型在此类任务中表现不足。

Method: 构建了包含逻辑、时间、空间三个核心领域的TIC-Bench基准，涵盖8种任务类型，总计2,280个问题，并对10个主流多模态大模型进行了评测。

Result: 实验显示当前顶级多模态模型与人类专家表现存在显著差距，且难以有效整合分散在交错文本和图像中的证据。

Conclusion: TIC-Bench揭示了当前模型在处理深度交错多模态上下文时的局限性，为未来多模态大模型的研发提供了重要的评估工具。
...
### [150] [Generalizable Brain Tumor Segmentation with Self-Training and Tumor-Aware Deformations](https://arxiv.org/abs/2609.02600)
*Henrique Zan Grande,Jeovane Honorio Alves,Rayson Laroca,Andre Gustavo Hochuli*

Main category: cs.CV

TL;DR: 该研究提出了一种针对BraTS-GoAT任务的脑肿瘤分割方法，通过结合nnU-Net框架、半监督自训练与肿瘤感知可变形增强技术，提升了在异构患者群体中的分割鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 旨在解决脑肿瘤分割任务中由于患者群体异构性导致的泛化能力不足问题，特别是在BraTS 2026挑战赛背景下提升分割准确度。

Method: 基于nnU-Net框架，采用了大型残差编码器架构，结合利用未标注数据生成的伪标签进行半监督学习，并引入肿瘤感知可变形增强技术（在保持解剖结构的同时对病灶进行局部变形）。

Result: 在BraTS-GoAT验证集上，该方法在全肿瘤（WT）、肿瘤核心（TC）和增强肿瘤（ET）的Dice及NSD指标上均优于仅使用标注数据的基线模型，证明了自训练与数据增强的互补性。

Conclusion: 通过半监督学习和特定肿瘤的增强策略，能够有效提高模型在多变患者群体中的脑肿瘤分割性能，该方案代码已开源。
...
### [151] [AffectDelta: Beyond Emotion Labels for Image Editing](https://arxiv.org/abs/2609.02616)
*Xingzu Zhan,Lin Gu,Ruogu Fang*

Main category: cs.CV

TL;DR: AffectDelta 是一种通过操纵八维情感分布实现精确情感驱动图像编辑的方法，解决了传统基于单一情感标签编辑的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法多使用单一情感标签进行编辑，忽略了情感的复杂性和共存性，且文本描述难以精确量化情感的变化程度。

Method: 提出了 AffectDelta 编辑框架，通过八维情感分布差值（Signed difference）来引导情感转变；构建了包含 24.9 万对源-目标图像的数据集 AffectPair-249K 用于训练，并采用源感知扩散模型实现语义与外观的精准编辑。

Result: 实验表明，AffectDelta 在情感对齐度和内容保持度上优于现有的六种基准模型。

Conclusion: 该方法通过引入情感分布的量化处理，有效提升了图像编辑的情感表达精度和视觉一致性。
...
### [152] [Learning to Attract and Repel: Dual Quality Margin Learning for Face Recognition (DQM-Face)](https://arxiv.org/abs/2609.02644)
*El Ouanas Belabbaci,Bhavesh Wani,Philipp Terhörst*

Main category: cs.CV

TL;DR: 提出了一种名为 DQM-Face 的双重质量边界学习框架，通过结合特征幅度和语义注意力来优化人脸识别的特征表示。


<details>
  <summary>Details</summary>
Motivation: 现有基于幅度的质量建模容易受到身份无关噪声的影响，导致特征表示的可靠性和判别力下降。

Method: 结合了传统的特征幅度估计与基于挤压-激励（Squeeze-and-Excitation）语义注意力机制的语义质量学习，并引入了包含吸引力和排斥力的动态边界优化策略。

Result: 实验表明，DQM-Face 在多个挑战性人脸识别基准测试中均优于现有的前沿方法，且学习到的质量信号可有效用于人脸图像质量评估。

Conclusion: 通过联合利用幅度和语义线索构建自适应边界，DQM-Face 能够显著提升人脸特征的结构化程度和判别能力。
...
### [153] [From Detection to Localization: A Unified Forensics Framework for Fully Synthetic and Tampered Images](https://arxiv.org/abs/2609.02640)
*Annalisa Gallina,Marco Fiorucci,Marco Brigo,Federica Battisti,Lamberto Ballan*

Main category: cs.CV

TL;DR: 提出了一种统一的多类多媒体取证框架，不仅能区分真实图像、全生成图像和篡改图像，还能实现篡改区域的像素级定位。


<details>
  <summary>Details</summary>
Motivation: 传统二分类检测方法在处理新型生成式伪造时局限性较大，且缺乏对图像篡改区域的定位能力。

Method: 扩展现有检测器，构建了一个包含分类分支（多类）和分割分支的统一框架，同时支持图像真伪识别与篡改区域定位。

Result: 在多类分类准确率和篡改区域定位（IoU）方面均优于现有的前沿基准方法。

Conclusion: 该方法通过统一的框架有效解决了生成式伪造带来的检测与定位挑战，为多媒体取证提供了一种更高效、全面的解决方案。
...
### [154] [Physics-Driven Independent Pair Generation for Iterative Self-Supervised Low-Dose CT Denoising](https://arxiv.org/abs/2609.02654)
*Xianlei Han,Shaoyu Wang,Jiancheng Fang,Weiwen Wu,Qiegen Liu*

Main category: cs.CV

TL;DR: 提出了一种针对低剂量CT（LDCT）的物理驱动自监督去噪框架，通过显式建模混合泊松-高斯噪声实现高质量图像重建。


<details>
  <summary>Details</summary>
Motivation: 大多数现有的自监督去噪方法未针对LDCT中复杂的混合泊松-高斯噪声进行专门建模，导致去噪效果受限。

Method: 引入物理驱动的跨域迭代框架：1.利用噪声模型分解泊松和高斯成分；2.通过数据稀疏化技术构建训练对；3.通过图像域网络与先验信息的迭代更新，保持重建结果与CT成像物理的一致性。

Result: 在多个标准数据集及真实LDCT数据上实验显示，该方法在各剂量水平下均优于现有的自监督基线，且性能接近监督学习模型。

Conclusion: 该方法通过引入CT成像物理约束，有效解决了混合噪声下的自监督去噪问题，为临床低剂量成像提供了强有力的重建方案。
...
### [155] [Characterizing Text Branch Sensitivity in Medical Vision-Language Segmentation via Evidence Decoupling](https://arxiv.org/abs/2609.02663)
*Ziquan Liu,Zhewei Zhu,Xuyang Shi*

Main category: cs.CV

TL;DR: 该研究探讨了文本信息在多模态医学图像分割中的真实作用，并揭示了模型对文本依赖程度在不同数据集上的差异。


<details>
  <summary>Details</summary>
Motivation: 尽管预训练视觉-语言模型（VLM）在医学影像分割中表现出色，但文本信息对像素级预测的实际贡献程度尚不明确。

Method: 分析了多种融合策略，并提出了一种基于证据深度学习（Evidential Deep Learning）和深度监督的证据解耦解码器（EDD）作为分析工具。

Result: 研究发现模型对文本的依赖具有数据集特异性；文本主要通过全局语义调节而非空间定位影响预测。

Conclusion: 不同医学影像任务对文本信息的依赖程度存在显著差异，这一发现为未来多模态分割模型的设计提供了理论依据。
...
### [156] [Query Rewriting for Complex Object Segmentation in 4D Gaussian Representations](https://arxiv.org/abs/2609.02664)
*Thanh-Khoi Nguyen,Thien-Phuc Tran,Minh-Triet Tran*

Main category: cs.CV

TL;DR: 提出了一种针对4D高斯场景表示的查询重写策略，通过将冗长的自然语言描述简化为关键词，显著提升了动态场景的分割和定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有的4D高斯动态场景理解框架对冗长且包含噪音的描述性查询非常敏感，导致分割和定位性能下降。

Method: 提出一种无需训练的重解释（reinterpretation）策略，利用检索增强语言模型和关键词引导机制，将复杂查询转化为简洁的关键词组合，以消除语言噪声并保留核心语义。

Result: 在HyperNeRF和Neu3D数据集上，时间准确率从60.92%提升至92.21%，vIoU从20.08%提升至76.94%。

Conclusion: 简洁的关键词查询能够产生更稳定的视频特征相似度分布，从而实现与对象中心化高斯表示更好的对齐，无需额外微调即可大幅提升性能。
...
### [157] [Orthogonal Ensembles and Tested Explanations for Performer-Independent Body-Motion Emotion Recognition](https://arxiv.org/abs/2609.02510)
*Naoto Nishida,Yoshio Ishiguro*

Main category: cs.CV

TL;DR: 该研究探讨了基于骨骼动作的12类情感识别问题，通过集成学习和可解释性分析提升了识别效果，并揭示了模型决策与拉班运动分析（LMA）特征的强相关性。


<details>
  <summary>Details</summary>
Motivation: LPO（留一表演者交叉验证）设定下情感识别难度极大，基线模型性能不佳，且现有深度学习模型缺乏透明的决策依据。

Method: 采用了11个具有正交误差模式的模型进行对数平均集成，并利用部件掩码、反事实编辑及LMA规则进行事后可解释性审计。

Result: 集成模型将Macro-F1指标提升至36.80%，比基线提高了11.07个百分点；分析表明模型决策显著依赖于符合LMA特征的身体区域证据。

Conclusion: 架构创新并非提升性能的唯一路径，集成学习结合事后审计套件能有效提升识别性能并验证模型决策与人类运动分析理论的一致性。
...
### [158] [GaLe: memory-efficient Global Approximate and Local Exact features](https://arxiv.org/abs/2609.02689)
*Alberto Ancilotto,Elisabetta Farella*

Main category: cs.CV

TL;DR: GaLe 是一种在资源受限设备上部署预训练网络的高效推理技术，无需重训练即可显著降低内存并提升速度。


<details>
  <summary>Details</summary>
Motivation: 嵌入式设备缺乏GPU资源，现有推理方法在计算开销（基于补丁）与精度损失（基于近似）之间难以平衡。

Method: 将特征图划分为局部精确（Le）表示和全局近似（Ga）表示，支持混合CNN-Transformer模型中的全局操作和注意力机制。

Result: 在ImageNet任务上，相比传统补丁推理，GaLe在Cortex-M33上实现了高达65%的加速和90%的内存节省，同时保持了与精确推理相当的精度。

Conclusion: GaLe 是一种多功能的资源高效推理方案，适用于分类、检测和生成任务，为轻量级架构设计提供了新思路。
...
### [159] [Generating Medical Image Counterfactuals using Causal Explanations](https://arxiv.org/abs/2609.02697)
*David A. Kelly,Tom Yaacov,Nathan Blake,Sander Beckers,Hana Chockler*

Main category: cs.CV

TL;DR: 提出了一种无需生成模型（如GAN或扩散模型）即可直接从分类器因果证据中生成医学图像反事实解释的新框架。


<details>
  <summary>Details</summary>
Motivation: 现有基于生成模型的反事实解释方法引入了额外的偏差，难以区分是分类器的决策过程还是生成模型的先验，且难以解释黑盒模型。

Method: 直接从分类器内部提取因果证据来构建反事实图像，实现确定性生成，无需额外训练，并支持在感兴趣区域进行可控编辑。

Result: 实验证明该方法在改变分类器预测的同时，能生成比生成式基线更接近原图的反事实图像，提供了更透明的决策边界视图。

Conclusion: 该方法通过规避外部生成模型，实现了更直接、可控且透明的医学图像分类器解释，增强了其在临床环境中的可信度。
...
### [160] [InceptionGS: Generative Bootstrapping for Large-Scale Gaussian Splatting under Unstructured View Sampling](https://arxiv.org/abs/2609.02747)
*Tianheng Lu,Guangyu Wang,Ruqi Huang,Lu Fang*

Main category: cs.CV

TL;DR: InceptionGS 是一种结合 3D 高斯喷溅（Gaussian Splatting）与生成式先验的框架，专门用于解决大规模场景在稀疏视角下的重建难题。


<details>
  <summary>Details</summary>
Motivation: 大规模场景数字化中，常因拍摄条件限制导致视角覆盖不全，现有的重建方法在数据缺失时效果脆弱，而生成式方法又难以保证 3D 一致性和细节还原度。

Method: 提出 InceptionGS，通过结合场景与视角自适应的生成式先验，对高斯喷溅重建过程中因视角缺失导致的质量薄弱区域进行“重构与修复”，同时保留高质量区域。

Result: 实验表明，InceptionGS 能在非结构化的稀疏图像集上实现更高保真度的渲染，显著优于现有方法，提升了大规模场景重建的稳健性。

Conclusion: 该研究证明了将生成模型引入高斯喷溅的优化流程是解决大规模场景数字化中“视角稀缺”问题的有效途径。
...
### [161] [A Top-Down Framework for Metric-Scale Athlete Localization from Single Broadcast Frames](https://arxiv.org/abs/2609.02705)
*Thanh-Khoi Nguyen,Hoang-Phuc Nguyen,Linh-Huynh,Minh-Triet Tran*

Main category: cs.CV

TL;DR: 提出了一种基于单帧广播视频的高精度运动员世界坐标定位框架，有效解决了超高分辨率影像下的尺度差异问题。


<details>
  <summary>Details</summary>
Motivation: 超高分辨率广播视频中运动员尺度差异极大，导致现有的定位方法存在严重的边界切割问题和召回率下降。

Method: 1. 边界感知自适应切片（Boundary-Aware Adaptive Tiling）：通过语义引导迭代扩展切片，消除边界分割伪影；2. 专用两关键点估计器：基于RTMPose-X优化关键点检测；3. 几何投影：通过相机校准进行射线投射，将2D投影提升至世界坐标。

Result: 在公共测试集上，LocSim得分达到97.44，mAP达到0.9128，较基线提升超过21%。

Conclusion: 该框架通过优化切片策略和几何建模，显著提升了复杂场景下运动员定位的鲁棒性和准确性。
...
### [162] [Balancing Frequencies and Pixels in Flow Matching](https://arxiv.org/abs/2609.02748)
*Lucas Degeorge,Paul Couairon,Arijit Ghosh,Alexei A. Efros,David Picard,Vicky Kalogeiton*

Main category: cs.CV

TL;DR: 提出了一种名为“焦点对数频率损失”（f-loss）的训练目标，用于解决像素空间流模型中因频谱不平衡导致的学习效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 自然图像的频谱服从1/f²分布，导致像素级重构损失过度关注低频信息，忽略了对感知重要的纹理和边缘等高频细节，限制了模型的训练效率。

Method: 引入f-loss以平衡各频率的训练信号，并配合一种多阶段训练策略：先通过频率域监督学习整体结构，再转向标准像素空间损失进行空间细节细化。

Result: 该方法在不改变模型架构的情况下，收敛速度提升高达40%，并在FID和感知保真度指标上表现出显著提升。

Conclusion: 通过纠正像素空间损失的低频偏差，该方法能有效优化流模型的学习过程，是提升生成模型性能的一种轻量级、通用的改进方案。
...
### [163] [AutoCompass: Accurate Visual Localization on Public Maps by Learning from Weak Labels](https://arxiv.org/abs/2609.02798)
...
TL;DR: AutoCompass 是一种通过处理带有噪声的地理参考标签来训练神经地图匹配器的方法。


<details>
  <summary>Details</summary>
Motivation: 现有的神经地图匹配器依赖于大规模带有噪声的地理参考（GPS和航向）数据，导致模型训练效果受限。

Method: AutoCompass 提出无需显式航向标签，在原始GPS标签中定义容差区域，并结合SLAM或SfM提供的相对位姿信号进行监督训练。

Result: 在驾驶和第一人称视觉基准测试中，AutoCompass 在位置和航向预测准确性上均优于仅依赖强绝对位姿标签的方法。

Conclusion: 该方法证明了模型可以从噪声数据中自动学习精确的航向，并利用相对位姿信息显著提升在现实场景中的地图匹配精度。
...
### [164] [Video-Based Palm-Vein Authentication under Challenging Conditions](https://arxiv.org/abs/2609.02776)
*Xiaofeng Yan,Kechen Liu,Abhilash Venkatesh,Cathy Zhang,Xia Zhou,Salvatore Stolfo*

Main category: cs.CV

TL;DR: 该研究引入了首个视频掌静脉数据集CUP，并提出了一种针对真实场景噪声（污垢、温度等）的鲁棒性识别方案。


<details>
  <summary>Details</summary>
Motivation: 现有掌静脉识别在真实环境下受表面噪声、光照、运动和温度影响严重，但缺乏相关数据集支持研究。

Method: 构建包含四种表面条件（清洁、温热、潮湿、脏污）的CUP视频数据集；提出一种无需额外学习参数的测试时匹配器，通过全局余弦相似度与显著性引导的局部最优传输融合来避开受损区域。

Result: 在CUP数据集上，该方法在各类评价指标上均领先，且能在不重训的情况下使主流识别模型的平均等错误率（EER）降低29-37%。

Conclusion: 通过空间上的局部最优传输匹配和时间上的多帧聚合，可以显著提升掌静脉识别在各种恶劣环境下的鲁棒性。
...
### [165] [Multi-Tool Image Editing Attribution in Facial Forgery](https://arxiv.org/abs/2609.02751)
*Sheng Liu,Qiang Sheng,Danding Wang,Yu Li,Chenming Zhou,Juan Cao*

Main category: cs.CV

TL;DR: 该论文提出了多工具图像编辑归因（MIEA）任务，并设计了DPEC方法以识别经过多次编辑处理的人脸图像所涉及的多种编辑工具。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法多基于单工具假设，无法应对复杂的现实场景中多种编辑工具痕迹叠加和重合的问题。

Method: 构建了包含50万张图像的MultiEdit数据集；提出了DPEC方法，结合空间域和频率域的局部感知痕迹提取，并采用基于错误的课程学习策略。

Result: 实验结果表明，DPEC在处理多达五步编辑的人脸图像时，性能优于现有的九种归因方法。

Conclusion: 该研究有效解决了多工具编辑归因难题，提升了对复杂编辑操作的检测与溯源能力。
...
### [166] [Efficient All-in-One Weather Restoration using Spectral Harmonization](https://arxiv.org/abs/2609.02839)
*Paula Garrido-Mellado,Daniel Feijoo,Yuning Cui,Alvaro Garcia,Marcos V. Conde*

Main category: cs.CV

TL;DR: 提出了一种名为FReSH-IR的轻量级多合一图像修复方法，通过频率分解和傅里叶跳跃连接处理各种恶劣天气退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法计算开销大，难以在资源受限的环境下处理高分辨率图像及多种天气退化。

Method: 采用分层编解码器架构，在每一尺度下将特征分解为高频和低频组件，并结合基于傅里叶变换的跳跃连接进行空间特征整合。

Result: 在参数量和计算量减少80%的情况下，达到了与Transformer模型相当的修复质量。

Conclusion: FReSH-IR在效率与性能之间实现了极佳的平衡，非常适合资源受限系统中的实时图像修复。
...
### [167] [GDB-Reward: From Evaluation Metrics to Training Rewards for Graphic Design](https://arxiv.org/abs/2609.02813)
*Adrienne Deganutti,Purvanshi Mehta,Simon Hadfield,Andrew Gilbert*

Main category: cs.CV

TL;DR: 提出了一种名为GDB-Reward的框架，利用强化学习通过非微分的图形设计评估指标来优化冻结的文生图模型。


<details>
  <summary>Details</summary>
Motivation: 文生图模型在处理图形设计（排版、布局、视觉传达）等需要精确约束的任务时表现不佳，且现有微调方法成本高昂，提示词优化缺乏有效的奖励反馈机制。

Method: 引入GDB-Reward框架，将多种异构且不可微的图形设计评估指标转化为强化学习中的统一标量奖励，从而在不更新模型权重的情况下引导生成过程。

Result: 实验表明，GDB-Reward能有效提升生成图像在感知质量、渲染保真度和空间布局准确性方面对设计规范的遵循程度。

Conclusion: 该研究证明了非微分的评估指标可以转化为有效的强化学习目标，为在缺乏微分监督的领域优化冻结模型提供了新路径。
...
### [168] [Benchmarking RAW and RGB Restoration in Image Signal Processors](https://arxiv.org/abs/2609.02831)
*Zihao Lu,Radu Timofte,Marcos V. Conde*

Main category: cs.CV

TL;DR: 该研究评估了在图像信号处理器（ISP）之前（RAW域）和之后（sRGB域）进行盲恢复的效果，发现ISP感知监督和模型与流水线的对齐至关重要。


<details>
  <summary>Details</summary>
Motivation: 目前的图像恢复研究缺乏对ISP处理流程的考虑，尚不清楚在流水线的哪个阶段进行恢复能达到最佳效果，且缺乏跨不同设备和退化场景的基准测试。

Method: 建立了一个涵盖四种设备、两种ISP、三种退化类型（噪声、模糊、联合退化）的基准，对比了RAW域恢复与RGB域恢复模型的性能。

Result: RAW域恢复普遍优于通用的RGB恢复模型；然而，若RGB恢复模型在训练时整合了ISP转换逻辑（ISP-aware），则能达到整体最佳效果。

Conclusion: 恢复模型的性能高度依赖于其与图像流水线的对齐程度；建议在学术实验中明确标注恢复位置并采用ISP感知的训练策略。
...
### [169] [RoGe: Novel View Synthesis via End-to-End Implicit Reconstruction and Generation](https://arxiv.org/abs/2609.02847)
*Xiaolei Lang,Ze Kang,Zehao Huang,Naiyan Wang*

Main category: cs.CV

TL;DR: RoGe 是一种端到端的视图合成框架，通过将隐式几何特征直接注入视频扩散模型，取代了传统的显式 3D 中间表示，实现了高质量的稀疏视角场景漫游。


<details>
  <summary>Details</summary>
Motivation: 现有混合视图合成方法依赖于渲染图像或点云等显式 3D 表示，这会导致信息丢失且生成模型无法反向修正重构误差。

Method: 构建了一个统一的端到端框架：通过前馈重构模型提取隐式场景特征，并利用射线查询（ray-querying）将这些特征作为条件直接注入视频扩散模型，无需中间 3D 表征，且通过联合训练让生成目标直接优化几何感知。

Result: 在 DL3DV 数据集上，RoGe 在图像质量和视频时间一致性指标上均优于现有的重构型、生成型及混合型基线方法。

Conclusion: 去除显式 3D 桥接并采用射线查询的隐式特征作为生成条件，能够有效提升稀疏输入下的视图合成质量与一致性。
...
### [170] [Thinking in Pictures: A Systematic Benchmark for Reasoning-driven Image Generation](https://arxiv.org/abs/2609.02864)
*Yutong Liu,Nan Huang,Xu Cao,James M. Rehg*

Main category: cs.CV

TL;DR: RIG-BENCH 是一个旨在评估统一生成模型（UGMs）逻辑推理与生成能力的综合基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型缺乏高层视觉推理能力，往往产生“局部合理但全局不合逻辑”的结果，需要从“表面对齐”转向“推理驱动生成”。

Method: 构建了包含2000个样本的RIG-BENCH基准，涵盖概念、变换、模式与结构、场景四个认知领域，用以评估当前主流生成模型。

Result: 实验显示现有先进模型在推理驱动生成（RIG）任务上表现出显著的推理-生成鸿沟，难以处理复杂的逻辑约束。

Conclusion: RIG-BENCH为开发下一代具备逻辑严密性的统一生成模型和世界模拟器提供了关键的诊断工具。
...
### [171] [PlantC2USeg: Cross-Scale Consistent Pre-Training for Few-Shot Unified Plant Point Cloud Segmentation](https://arxiv.org/abs/2609.02860)
*Yu Tian,Xintong Jiang,Jan Franklin Adamowski,Shiv O. Prasher,Shangpeng Sun*

Main category: cs.CV

TL;DR: 提出了一种名为PlantC2USeg的深度迁移学习框架，通过跨尺度一致性学习和信息受限解码策略，实现了植物点云的高效分割与少样本适应。


<details>
  <summary>Details</summary>
Motivation: 现有植物点云分割方法严重依赖大规模标注数据，在面临分布偏移和跨物种应用时，标注成本高且适应难度大。

Method: 提出了PlantC2USeg框架，包括用于特征对齐的跨尺度一致性学习（cross-scale consistency learning）和防止重建捷径的信息受限解码策略，以提升模型的迁移泛化能力。

Result: 在Soybean3D数据集上，全监督下表现最优；在少样本（10-20个样本）条件下，在Soybean3D、HR3D及SYAU-Maize等数据集上均取得了领先的语义IoU和实例mWCov指标，并在ShapeNet Part上展现了跨领域的通用性。

Conclusion: PlantC2USeg有效降低了分布偏移下的标注负担和训练开销，为植物表型分析提供了高效、可扩展的3D表示学习方案。
...
### [172] [MuyBridge: Mobile Human Center-of-Mass Estimation from Monocular Video via Sparse Fusion](https://arxiv.org/abs/2609.02854)
...
TL;DR: MuyBridge 是一款能在移动设备上仅通过单目视频实时估计运动员 3D 质心（CoM）轨迹的轻量级系统。


<details>
  <summary>Details</summary>
Motivation: 现有的 3D 姿态估计方法要么缺乏解剖学约束，要么计算成本过高，导致教练和分析师难以在训练现场进行实时的质心测量。

Method: 结合紧凑型 2D 姿态网络与蒸馏单目深度网络，引入解剖学和物理先验进行解析度量融合，无需 3D 或特定任务的监督学习。

Result: 在 AthletePose3D 数据集上实现了 33-41mm 的垂直质心误差，在 iPhone 15 上能以 63 FPS 的速率进行姿态估计。

Conclusion: 该系统证明了在单台手机上实现高精度、低延迟的运动质心测量是可行的，为运动员现场分析提供了便携的解决方案。
...
### [173] [SolarWM: Open Data and Scalable Training for Long-Horizon Video World Models](https://arxiv.org/abs/2609.02886)
...
TL;DR: SolarWM 是一个开源的交互式视频世界模型基础框架，通过统一的数据引擎和多主干网络适配方案，实现了长时程、高质量的视频建模。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型存在训练数据异构性高、模型架构不统一、缺乏可复现的统一训练范式等问题，导致研究难以协作和评估。

Method: 构建了包含 143 万个视频剪辑的统一数据引擎，采用“框架-主干”解耦架构，结合双向适配、教师强制自回归初始化及分布匹配蒸馏的三阶段训练策略。

Result: 成功在 Wan2.2、LTX-2.5 和 MiniMax-H3 等多种主干上实例化了 5B-33B 参数模型，支持长达数小时的实时交互式长序列推理。

Conclusion: SolarWM 为世界模型研究提供了一套可复现、可扩展的开源基础，有效解决了跨数据源与多架构适配的复杂性问题。
...
### [174] [Genesis: A Generative Engine for Hierarchical Satellite Image Synthesis](https://arxiv.org/abs/2609.02683)
*Subash Khanal,Yangzhi Cui,Daniel Cher,Eric Xing,Brian Wei,Srikumar Sastry,Nathan Jacobs*

Main category: cs.CV

TL;DR: 本文提出了Genesis，一种通过结合垂直超分辨率和水平修复算子，生成全球一致的多尺度卫星影像金字塔的生成模型。


<details>
  <summary>Details</summary>
Motivation: 现有卫星图像生成模型仅能在单一尺度（缩放或平移）上工作，无法实现多尺度下空间与分辨率的一致性。

Method: 提出了多尺度平铺补全任务，设计了Genesis引擎，通过组合垂直超分辨率模型与水平掩码修复模型来递归生成四叉树结构。

Result: Genesis在各项子任务上达到了SOTA水平，并能根据稀疏种子点生成全局一致且无缝的多分辨率地图；同时发布了dense500数据集。

Conclusion: Genesis成功解决了卫星图像在空间和尺度维度的协同生成问题，为多尺度地球观测提供了一种有效的生成框架。
...
### [175] [Doppio: A Dataset for Contactless Weight Estimation of Falling Particles](https://arxiv.org/abs/2609.02528)
...
TL;DR: 本文提出利用计算机视觉技术实现粉末（以咖啡粉为例）的非接触式质量测量，并发布了Doppio数据集及相应的深度学习评估模型。


<details>
  <summary>Details</summary>
Motivation: 工业应用中现有的非接触式质量测量方案成本高昂、技术复杂，研究者希望通过计算机视觉寻找一种更经济、通用的替代方案。

Method: 构建包含掉落咖啡粉视频及精确重量标签的Doppio数据集，并评估了从空间前馈网络到时空循环神经网络等多种深度学习模型。

Result: 研究证实深度学习模型能够准确估计掉落颗粒的累积重量，并分析了模型预测精度与计算开销之间的权衡。

Conclusion: 基于计算机视觉的深度学习模型是实现非接触式质量测量的可行路径，为未来的相关技术应用奠定了基础。
...
### [176] [SR-Edit: Region-Aware Image Editing via Self-Refinement](https://arxiv.org/abs/2609.02504)
*Andong Wang,Zehua Chen,Yuxuan Jiang,Jun Zhu*

Main category: cs.CV

TL;DR: SR-Edit 是一种通过迭代自细化实现高质量图像编辑的框架，能在无需外部标注的情况下，在精确修改目标区域的同时保持非编辑区域的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的自动区域估计编辑方法往往会导致区域划分不准确，且启发式修正策略会破坏原始生成过程，引入新的伪影。

Method: 提出了SR-Edit框架，通过迭代过程：1) 对模型预测进行轻量级后处理以提取更精确的区域划分；2) 执行与原始采样动态对齐的非编辑区域修正更新。

Result: 实验表明，SR-Edit在图像编辑的保真度、区域一致性以及整体图像质量方面均优于现有的编辑技术。

Conclusion: 通过迭代自细化策略，该方法有效地平衡了目标编辑的准确性与背景区域的完整保留，解决了传统编辑方法中常见的失真问题。
...
### [177] [PRO-Step: Step-level Process Reward Optimization for Retrieval-Augmented Generation](https://arxiv.org/abs/2609.01658)
*MinKeon Kim,Namjun Lee,Jaekwang Kim*

Main category: cs.CL

TL;DR: PRO-STEP 是一种通过过程监督改进 RAG 多跳推理的方法，通过评估逻辑有效性和证据基础来减少错误传播。


<details>
  <summary>Details</summary>
Motivation: 现有的 RAG 方法（如基于结果的优化或基于步骤的奖励）难以解决多跳推理中的错误传播问题，且常奖励“歪打正着”的错误路径。

Method: 训练生成式过程奖励模型（PRM）以评估逻辑和证据，使用 PRM 引导的价值树搜索构建偏好对，并通过步骤级直接偏好优化（DPO）训练策略。

Result: 在五个 QA 基准测试中，PRO-STEP 在平均 EM 和 F1 指标上均取得了最佳表现。

Conclusion: 引入步骤级监督并同时关注逻辑与证据，能有效提升多跳 RAG 的推理准确性和鲁棒性。
...
### [178] [Learning Evidence Sufficiency Boundaries for Selective Answering in Grounded Multi-Hop QA](https://arxiv.org/abs/2609.01687)
*Haruto Sato,Yuki Tanaka,Ren Nakamura,Aoi Kobayashi,Mei Ito*

Main category: cs.CL

TL;DR: 该研究提出了一种名为“证据充分性边界训练（ESBT）”的框架，旨在解决多跳问答中模型难以判断证据是否足以支持答案的问题，通过训练模型识别证据链中的充分性边界，实现更精准的选择性回答。


<details>
  <summary>Details</summary>
Motivation: 多跳问答中，局部证据常误导模型给出不可靠答案。现有的问答系统往往难以区分“部分证据”与“充分证据”，导致模型在证据不足时过度自信。

Method: 引入证据充分性边界训练（ESBT），构建有序证据链，通过层级监督、边界翻转边际、后边界稳定性及答案召回保护等机制，直接监督模型从“拒绝回答”到“给出答案”的转换过程。

Result: 实验表明，ESBT在Qwen2.5-3B模型上实现了更强的边界定位能力（翻转准确率达0.807），在外部不可回答数据集上的错误回答率最低（0.095），且在保持问答准确率的同时显著增强了 grounded 性。

Conclusion: 通过明确标记拒绝回答向接受回答转化的证据水平，可以有效提升问答系统在证据不足时的选择性回答能力，减少不可靠答案的产生。
...
### [179] [SpeakPay: Domain-Adaptive LoRA Fine-Tuning of Whisper for Low-Resource Nepali Financial Speech Recognition](https://arxiv.org/abs/2609.01737)
*Biraj Subedi*

Main category: cs.CL

TL;DR: 本文提出 SpeakPay，一款专为尼泊尔视障人士设计的语音优先数字钱包，通过领域自适应技术提升低资源语言金融语音识别能力。


<details>
  <summary>Details</summary>
Motivation: 尼泊尔移动支付应用主要依赖图形界面，对视障用户极其不友好，且针对尼泊尔语金融场景的语音识别资源匮乏。

Method: 构建包含403条尼泊尔语金融命令的数据集 NepFinSpeech-403，并利用 LoRA 对 Whisper large-v2 模型进行微调以实现领域自适应。

Result: 相比零样本基线，模型词错误率（WER）降低了67.2%，梵文数字识别准确率提升至73.9%，实际交易成功率从1.67%大幅提升至33.33%。

Conclusion: 该研究证明了利用少量领域特定数据（约300条）即可显著提升低资源语言在金融场景下的语音交互性能，为提升数字支付的无障碍性提供了可行的技术路径。
...
### [180] [MemeCULT-1K: Benchmarking South Asian Cultural Context and Humor Understanding of Multimodal Models](https://arxiv.org/abs/2609.01772)
...
TL;DR: 本文提出了MemeCULT-1K，这是一个包含1,000个南亚多语言模因（Meme）的基准测试集，旨在评估视觉语言模型（VLM）对模因中隐性文化知识和语用推理的理解能力。


<details>
  <summary>Details</summary>
Motivation: 大多数现有的视觉语言模型缺乏对模因中隐含的文化背景和语用含义的理解，无法有效处理具有地域文化色彩的内容。

Method: 构建包含孟加拉语、英语和印地语的MemeCULT-1K数据集，通过对比“仅模因”和“上下文感知”两种设置，评估了13种主流视觉语言模型的表现，并进行了细粒度的错误分析。

Result: 实验显示，提供基础文化背景后，所有模型在SBERT相似度、BLEURT评分及LLM评分上均有显著提升；闭源模型易在实体识别上出错，开源模型则受限于文化知识缺失。

Conclusion: 当前VLM在处理文化背景深厚的模因时仍面临巨大挑战，研究强调了未来需要将显式文化知识整合到模型训练中的必要性。
...
### [181] [VakyArth: Evaluating Pragmatic Competence in LLMs across Indic Languages](https://arxiv.org/abs/2609.01788)
*Usneek Singh,Poorvaja Veera Balaji Kumar,Parth Nanda,Anand Madhusoodanan,Geyang Guo,Wei Xu,Junyi Jessy L*

Main category: cs.CL

TL;DR: VakyArth 是首个针对印度语言（印地语、旁遮普语、泰米尔语、马拉雅拉姆语）的语用学基准测试，用于评估大模型在处理暗示、社交语境等复杂沟通时的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的语用学评估大多局限于英语等高资源语言，缺乏对印度语言这一具有深厚文化和语言多样性群体的研究。

Method: 构建了包含指代、言语行为、内涵、社交语用和连贯性五个维度的多语言诊断数据集，并通过多项选择题（MCQ）、自然语言推理（NLI）和翻译任务进行评估。

Result: 大模型在处理基于印度语言和文化惯例的语用含义时表现普遍不佳；MCQ 准确率高于 NLI；印欧语系在翻译任务上的表现优于达罗毗荼语系；自动翻译指标往往无法捕捉语用上的偏差。

Conclusion: 现有的多语言模型在理解印度语言的语用深度方面存在明显局限，未来需要针对文化特异性进行更有针对性的优化和评估。
...
### [182] [Disentangling Statistical Preemption from Entrenchment in Language Models' Avoidance of Overgeneralization](https://arxiv.org/abs/2609.01794)
*Yixuan Wang,Freda Shi,Kanishka Misra*

Main category: cs.CL

TL;DR: 该研究探讨了语言模型（LM）如何在缺乏明确负面证据的情况下，通过预占（preemption）或固化（entrenchment）机制避免语法过度泛化。


<details>
  <summary>Details</summary>
Motivation: 旨在探究人类学习者如何避免类似“Tom laughed me”的语法错误，并评估现有语言模型是否具备类似人类的纠错机制。

Method: 通过受控训练实验，在语言模型中系统性地移除“预占”与“非预占”证据，并分析模型的训练动态。

Result: 语言模型虽然能避免过度泛化，但在动词层面未表现出预占效应，仅表现出微弱的抽象预占证据，且倾向于将竞争结构视为间接正面而非负面证据。

Conclusion: 目前的神经语言模型在处理间接负面证据方面存在局限，未来需要增强模型对负面反馈的敏感性，并进一步研究人类的抽象预占能力。
...
### [183] [How Do Prompt Variations Affect Energy Consumption in On-Device LLMs?](https://arxiv.org/abs/2609.01798)
*Wei Hu,Xiaolong Tu,Dawei Chen,Yitao Chen,Kyungtae Han,Haoxin Wang*

Main category: cs.CL

TL;DR: 该研究探讨了提示词（Prompt）的设计属性如何影响移动端大语言模型（LLM）的推理能耗。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在移动端部署，能效成为关键限制，但目前缺乏针对提示词设计对能耗影响的研究。

Method: 通过跨模型、数据集和设备的实证研究，采用阶段级性能分析（区分预填充和解码阶段），分析提示词的认知负荷和措辞模式对能耗的影响。

Result: 认知负荷主要影响单位Token的能耗，而措辞模式主要通过改变Token使用量来影响能耗；提示词设计会改变不同模型在能效与质量权衡曲线上的表现。

Conclusion: 为实现高效的移动端推理，需要采取“模型感知”的提示词设计策略。
...
### [184] [TalkFa: A Unified Benchmark for Farsi Dialogue Generation and Understanding](https://arxiv.org/abs/2609.01810)
*Neda Jamshidi,Kamyar Zeinalipour,Fahimeh Akbari,Monica Bianchini,Marco Maggini,Marco Gori*

Main category: cs.CL

TL;DR: TALKFA是首个针对波斯语（Farsi）的全面对话生成与理解基准，包含知识驱动、对话行为/情感标注及戏剧对话三个数据集。


<details>
  <summary>Details</summary>
Motivation: 波斯语拥有超过1.2亿使用者，但目前缺乏系统性、高质量的对话生成与理解评估基准。

Method: 构建了WIKI-FADIAL、DAILYDIALOG-FA和PLAYDIAL-FA三个数据集；利用LLM辅助构建并经过多轮人工专家审核；实验中对比了多种LLAMA/MISTRAL模型，并采用LoRA微调与多种分类基准。

Result: 实验显示LoRA显著提升了生成效果，FABERT在对话行为分类表现最佳，LORA-MISTRAL-7B在情感识别领先，MISTRAL-24B在情感分析中表现最优；指出自动评估指标常高估对话质量。

Conclusion: TALKFA为波斯语对话模型提供了具有挑战性的评估平台，证明了人工审核数据与特定领域模型微调在低资源语言任务中的有效性。
...
### [185] [AVERT: Audio-Verified Adjudication for Spoken Dialogue State Tracking](https://arxiv.org/abs/2609.01828)
*Chunggi Lee,Hanspeter Pfister*

Main category: cs.CL

TL;DR: AVERT 是一种针对口语对话状态追踪（SDST）的后处理框架，通过结合跨轮次一致性与音频验证，有效纠正语音识别错误。


<details>
  <summary>Details</summary>
Motivation: 口语对话中语音识别（ASR）错误在实体值中高度集中且具持续性，传统的文本编辑模型难以解决跨轮次不一致、遗漏槽位及音频不支持的预测值等问题。

Method: 引入AVERT框架，利用跨轮次协议与音频条件验证器对候选值进行评分，并通过“投票（vote）”、“添加（add）”和“交换（swap）”三种受限操作符，在特定槽位范围内纠正错误。

Result: 在SpokenWOZ数据集上，AVERT达到了40.13的联合目标准确率（JGA），显著优于基线模型与纯文本编辑器，且音频验证器提供了统计学上的显著增益。

Conclusion: 通过将编辑操作限制在特定的错误易发槽位，并引入音频验证，可以在无需重新训练现有模型的情况下，有效提升口语对话状态追踪的性能。
...
### [186] [Interpretable Symptom Vectors for Depression in a Large Language Model](https://arxiv.org/abs/2609.01832)
*Fangyi Zhu,Ajay Subramanian,Allison Constant,Camille Wang,Ravish Gupta,Corey J. Keller*

Main category: cs.CL

TL;DR: 该研究利用机制可解释性技术分析了Gemma-3-27B-PT模型，验证了其内部表征能够精准识别并量化抑郁症症状。


<details>
  <summary>Details</summary>
Motivation: 临床实践常将复杂的抑郁症简化为单一评分，而大模型内部如何表征抑郁症状尚不明确，限制了其在临床评估中的可信度。

Method: 通过记录模型残差流中的激活值，采用几何距离度量寻找症状空间，利用语义投影（Semantic Projection）构建症状向量，并开发情感效价门控机制。

Result: 研究发现模型在第21层可实现症状群体的几何分离，且生成的症状系数与临床医生对情绪、躯体和自杀倾向的评级高度吻合，抑郁检测准确率（AUC）达0.789。

Conclusion: 模型内部存在与临床对齐的、去相关的抑郁症状信号，为开发可解释的AI辅助抑郁评估工具奠定了机制基础。
...
### [187] [Candidate Generation and Definition-Guided Verification for Sentence-Level Depression Symptom Recognition](https://arxiv.org/abs/2609.01833)
*Weiming Li,Catarina Barata,Miguel Constante,Joao Sanches*

Main category: cs.CL

TL;DR: 该研究提出了一种两阶段框架，通过将抑郁症状识别拆分为“候选生成”与“基于定义的验证”两个环节，提高了识别的准确性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有模型在识别抑郁症状时面临挑战：相同表达在不同语境下症状相关性不同，且缺乏对医学诊断定义的有效遵循（grounding）。

Method: 采用两阶段流程：第一阶段通过对比微调的编码器生成症状候选；第二阶段利用大语言模型，结合上下文与症状诊断定义进行逻辑验证与判断。

Result: 该框架在准确率和F1分数上均优于现有的编码器、推理基准及单阶段监督分类器，生成的解释与专家标注高度匹配。

Conclusion: 将症状识别分解为生成与验证是有效的，能增强临床可解释性，但对于罕见类别的识别性能仍有待提升。
...
### [188] [Cite or Decline: A Strict Course-Grounded Chatbot for STEM Lecture Videos](https://arxiv.org/abs/2609.01846)
*S M Masrur Ahmed,Jaspal Subhlok*

Main category: cs.CL

TL;DR: 本文介绍了VideoPoints平台，这是一个集成检索增强生成（RAG）技术的聊天机器人，能够基于课程录像回答学生问题并提供带有时间戳的引用。


<details>
  <summary>Details</summary>
Motivation: 学生难以从海量的录像资源中快速查找特定知识点或验证答案的准确性，现有的检索工具缺乏针对课程内容的精准交互。

Method: 采用基于课程隔离的检索增强生成（RAG）框架，利用章节摘要指导转录内容排序，并集成可点击的时间戳引用功能，在EduVidQA数据集上进行了评估。

Result: 系统在真实教学场景中表现稳健，70.5%的消息包含正确引用，无跨课程干扰；在EduVidQA测试集上，相比纯密集检索，正确检索率提升了6.3个百分点。

Conclusion: 有效的教学辅助AI部署依赖于课程隔离、可验证的引用机制以及与学生学习习惯的深度融合，未来应侧重于提升练习题生成功能。
...
### [189] [Thinking effort aligns between humans and reasoning models in abductive reasoning](https://arxiv.org/abs/2609.01867)
*Henry Arthur*

Main category: cs.CL

TL;DR: 该研究探讨了大型推理模型（LRM）与人类在推理任务中行为模式的一致性，重点分析了推理成本与错误倾向。


<details>
  <summary>Details</summary>
Motivation: 验证人类与通过强化学习训练的推理模型在处理非演绎推理任务时，是否表现出相似的认知努力与错误模式。

Method: 利用溯因推理任务（Abductive Reasoning）评估人类反应时间与模型推理轨迹，并引入多路径探索的解码方法观察一致性变化。

Result: LRM与人类在推理成本（努力程度）上呈现高度一致性，且倾向于犯类似的错误；多路径搜索解码能显著增强模型与人类在推理成本上的一致性。

Conclusion: 大型推理模型在处理复杂推理任务时表现出与人类相似的认知过程，证明了其作为人类认知行为模型的有效性。
...
### [190] [Grounded, Compute-Efficient LLM Policy Agents for Energy-Poverty Equity in Physically-Constrained Peer-to-Peer Energy Markets](https://arxiv.org/abs/2609.01918)
*Kunal Jadhav,Siddhesh More*

Main category: cs.CL

TL;DR: EqGrid 是一个针对能源贫困问题的闭环仿真框架，利用轻量级 LLM 智能体在满足物理电网约束的前提下实现精准的能源补贴与定价。


<details>
  <summary>Details</summary>
Motivation: 现有 NLP 领域对能源贫困研究匮乏，且常用的云端大模型存在高能耗的“计算反讽”，无法在资源受限的贫困地区有效部署。

Method: 构建包含基于现实数据建模的用户画像（personas）、多智能体强化学习（MARL）电力交易系统，以及基于 IEEE-33 总线的电网物理约束模型，并采用“LLM 设置策略+安全网关执行”的解耦架构。

Result: 该模型成功将能源负担基尼系数从 0.351 降至 0.305，平均负担降低 28%。同时，通过模型压缩，0.8B 的本地部署模型仍能保留 92% 的公平性提升效果，能耗比 235B 模型降低 24 倍。

Conclusion: 研究证明了轻量级、高能效的 LLM 可以在严格约束下有效缓解能源贫困，且在算力效率与社会公平之间实现了良好的权衡。
...
### [191] [GAPS: Dimension-Level Gates for Conditional Activation Steering](https://arxiv.org/abs/2609.01878)
*Moghis Fereidouni,Muhammad Umair Haider,Hassan Sajjad,A. B. Siddique*

Main category: cs.CL

TL;DR: GAPS 是一种针对大模型激活引导的维度级选择性方法，通过静态与动态门控机制，实现更精细的负面行为抑制。


<details>
  <summary>Details</summary>
Motivation: 现有的条件化激活引导方法（如 CAST、DSAS）在决策时会对所有维度应用完整向量，缺乏对单个神经元层面的精细控制，导致在抑制行为与保持能力之间难以平衡。

Method: 引入 GAPS 方法，包含两个无需训练的门控机制：一是基于 AUROC 的静态可分性门控（仅作用于含概念信息的神经元），二是基于高斯模型的动态后验门控（仅在激活偏离预期时介入）。

Result: GAPS 在多个模型（Gemma-3、Qwen-3）上的毒性抑制和概念删除任务中表现出色，在固定能力预算下，大幅降低了毒性生成率，优于现有的 token 级条件化方法。

Conclusion: 维度级门控是提升激活引导精细度的有效补充，GAPS 能够显著优化性能表现，证明了对干预范围进行维度选择的必要性。
...
### [192] [Sparse Readout Prism: Explaining Logit-Lens Scores in Features Instead of Tokens](https://arxiv.org/abs/2609.01936)
*Matteo He,William F. Shen,Xinchi Qiu,Nicholas D. Lane*

Main category: cs.CL

TL;DR: 该论文研究了语言模型中间层解码（Lens）中的“语料库条件性”偏差问题，并提出了“稀疏读取棱镜（SRP）”方法，通过分解权重矩阵而非依赖训练语料库来分析读取结构。


<details>
  <summary>Details</summary>
Motivation: 现有的Lens方法（将隐藏状态解码为Token）高度依赖于拟合所用的语料库，导致同一隐藏状态在不同语料库训练的Lens下会产生不同的结果，缺乏一致性。

Method: 引入稀疏读取棱镜（SRP），通过权重分解将Token Logit表示为稀疏读取特征的贡献之和，从而在不依赖语料库的情况下分析读取结构。

Result: SRP在重构Logit差异方面表现显著优于基准方法，且揭示了Token身份背后稳定的读取特征，即使在语料库改变时，主导特征依然保持稳定。

Conclusion: SRP为Lens分析提供了一种独立于语料库的控制手段，有助于更准确地理解语言模型内部表示的形成过程。
...
### [193] [NS-Copilot: An LLM-Driven Agent System for Autonomous Neuroscience Analysis](https://arxiv.org/abs/2609.01971)
*Wuche Liu,Yiran Qiao,Linlin Hou,Rui Yang,Shusen Pu,Song Wang,Jing Ma*

Main category: cs.CL

TL;DR: NS-Copilot是一个基于LLM的多智能体系统，旨在通过统一预训练模型并提供自然语言接口，解决神经科学分析中的跨学科障碍与任务自动化问题。


<details>
  <summary>Details</summary>
Motivation: 神经科学领域缺乏能够有效集成不同预训练模型并协调处理复杂生理数据的自动化工具，导致实验室难以充分利用人工智能的潜力。

Method: 构建了NS-Copilot多智能体系统，包含任务规划、自适应控制、代码生成和结果综合等专业代理，通过统一接口整合了EEG和尖峰信号等多种神经科学预训练模型。

Result: 在阿尔茨海默病、帕金森病及工作记忆尖峰解码等基准测试中，NS-Copilot的性能显著优于现有的基准方法。

Conclusion: NS-Copilot证明了利用LLM驱动的多智能体系统可以实现可扩展、自动化的神经科学数据分析，降低了该领域对特定数据集启发式方法的依赖。
...
### [194] [How Output Format Confounds Data Quality and Capability in Instruction Tuning](https://arxiv.org/abs/2609.02015)
*Chengguang Gan,Hanjun Wei,Yunhao Liang,Qinghao Zhang,Shiwen Ni,Zhixi Cai*

Main category: cs.CL

TL;DR: 该研究揭示了指令微调数据和模型评估深受“输出接口”（即答案的表面形式）的影响，导致现有的质量度量和基准测试可能存在严重的混淆效应。


<details>
  <summary>Details</summary>
Motivation: 研究者发现，指令微调的质量评估和模型能力衡量往往混杂了输出格式的影响，导致对模型真实能力的误判。

Method: 利用跨越12个任务、4种等价接口、3个模型家族及受控损坏实验，通过梯度签名分析（Gradient Signatures）和谱统计（Spectral statistics）来分离接口对模型性能的影响。

Result: 实验发现，模型能力与训练接口高度绑定，改变接口可能导致模型性能从显著提升变为大幅下降，且“接口差异”中包含了特定任务的定位信息，而非单纯的噪声。

Conclusion: 数据质量和模型能力本质上是受接口影响的量，当前行业实践往往在衡量接口形式而非实际内容，呼吁对评估方法进行根本性的范式调整。
...
### [195] [A Tri-Agent Framework for Evaluating and Aligning Question Clarification Capabilities of Large Language Models](https://arxiv.org/abs/2609.02054)
*Yikai Zhao,Saurabh Pandey,Pradeep Kumar Misra*

Main category: cs.CL

TL;DR: 提出了一种基于三智能体框架的对话系统评估方法，专门用于测试大语言模型处理模糊查询和进行澄清对话的能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在处理模糊或不明确的用户意图时缺乏系统性的评估基准，且缺乏模拟真实用户反馈及客观评测的标准化流程。

Method: 构建了一个包含三个角色的三智能体框架：QCA（被测澄清代理）、RA（模拟用户反馈的响应代理）和EA（基于大模型的评估员代理）；并在供应链领域生成合成数据进行验证。

Result: 提出了一套涵盖歧义处理、问题质量、对话效率、语言合规性和意图对齐的评估指标，并通过与人类判断的对比验证了评估员代理（EA）的有效性。

Conclusion: 该框架为对话式AI的澄清能力提供了一个结构化的评估和改进方案，有助于提升交互系统的用户意图理解精度。
...
### [196] [HyGRAIL: Cost-Aware and Evidence-Grounded Scientific Hypothesis Discovery over Knowledge Graphs](https://arxiv.org/abs/2609.02056)
*Yihang Sun,Zhihan Zhu,Zhiyuan Jiang,Jingyi Ge,Zixuan Li,Jiaxuan You*

Main category: cs.CL

TL;DR: HyGRAIL 是一种结合图神经网络（GNN）筛选与大语言模型（LLM）验证的科学假设发现框架，旨在高效且准确地识别科学知识图谱中的潜在关联。


<details>
  <summary>Details</summary>
Motivation: 科学知识图谱存在大量缺失关联，传统GNN在歧义预测上不可靠，而LLM推理成本高且缺乏图结构基础，迫切需要一种低成本、证据导向的自动化发现方案。

Method: 提出了一个分层框架：先通过GNN进行初步筛选，识别出具有高度不确定性的候选假设，并仅将这些“困难样本”提交给LLM；LLM通过检索知识图谱中的多跳关系和节点信息，结合结构化证据进行验证。

Result: 在MatKG数据集上达到了0.429的F1分数，显著优于现有基准；同时通过GNN triage机制减少了54.36%的LLM调用，实现了效率与精度的平衡。

Conclusion: HyGRAIL 证明了结合GNN的高效筛选与LLM的深度推理是解决知识图谱补全及科学假设发现的有效路径，且结构化图证据对于LLM的可靠推理至关重要。
...
### [197] [IDEEA: training-free Input-Dependent stEEring via Activation cluster matching](https://arxiv.org/abs/2609.02089)
*Zheng Wang,Muchen Li,Renjie Liao,Yan Leng*

Main category: cs.CL

TL;DR: IDEEA 是一种无需训练的 LLM 指导框架，通过根据输入动态选择最佳偏移方向来解决现有方法仅使用单一全局方向的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的推理时指导方法通常使用固定的全局偏移向量，但这忽略了不同输入在激活空间中分布不同，需要针对性的调整以平衡概念对齐与原始表达的保留。

Method: 提出 IDEEA 框架，通过对注意头进行激活簇聚类，并利用最优匹配算法构建一组簇条件偏移方向，在推理时根据输入激活特征动态选择最匹配的偏移方向。

Result: 实验显示 IDEEA 在 TruthfulQA 数据集上将 truth × info 指标平均提升了 9.9%，最高可达 23.5%，优于现有的输入无关基线模型。

Conclusion: 激活空间中概念编码分布在多个不同的子区域，使用输入相关的动态指导能比单一固定方向更有效地实现模型对齐，且能更好地保留原始输入信息。
...
### [198] [Selective Knowledge Edit Reversal via Gated Singular Vector Shrinkage](https://arxiv.org/abs/2609.02091)
*Weifeng Jiang,Ruirui Chen,Qianren Mao,Junnan Liu,Qili Zhang,Kwok-Yan Lam*

Main category: cs.CL

TL;DR: 提出了一种基于光谱（spectral-based）的知识编辑选择性回溯框架，旨在在保留其他已编辑事实的同时，有针对性地撤销特定的错误或恶意编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的知识编辑回溯方法往往是“一刀切”的全局移除，容易导致其他原本有益的知识编辑被误删。

Method: 基于“编辑内容稀疏编码于权重矩阵的主奇异子空间”的假设，通过定位并消除特定编辑在主奇异分量中的敏感组分来实现精确回溯。

Result: 实验证明该方法能够有效撤销目标编辑，同时保持未受影响的知识编辑不受干扰。

Conclusion: 不同编辑在权重矩阵中具有稀疏且可分离的奇异分量特性，利用光谱分析进行选择性回溯是修复大语言模型编辑副作用的有效途径。
...
### [199] [Predict, Don't Iterate: Efficient Adaptive-Length Infilling for Diffusion Language Models](https://arxiv.org/abs/2609.02108)
*Haobo Xu,Sirui Chen,Yuanchen Bei,Lingjie Chen,Yuchen Yan,Dongqi Fu,Jingrui He,Hanghang Tong*

Main category: cs.CL

TL;DR: 提出了一种名为PILL的高效扩散语言模型（DLM）填充方法，通过探测机制实现了无需预设长度的文本与代码填充。


<details>
  <summary>Details</summary>
Motivation: 现有DLM在填充任务中高度依赖预设长度，且处理动态长度时存在初始长度敏感性高及推理效率低的问题。

Method: 提出PILL（Probing-based InfiLling with preset-Length-free decoding），利用探测机制在生成过程中直接确定合适的填充长度，减少了冗余的前向传播计算。

Result: 在代码和文本填充任务上，相较于最优基线，PILL在代码通过率上提升4.8个点，BLEU-2提升6.0点，推理速度提升1.82倍。

Conclusion: PILL通过解除长度预设限制，显著提升了DLM在填充任务中的推理效率与生成质量，具备广泛的通用性。
...
### [200] [text2ql: Multi-Target Natural Language Querying via a Language-Agnostic Intermediate Representation](https://arxiv.org/abs/2609.02115)
*Ritesh Kumar*

Main category: cs.CL

TL;DR: Text2ql 是一个开源 Python 框架，通过引入通用中间表示 (QueryIR) 和可插拔渲染器，解决了自然语言转数据库查询在格式单一、依赖 LLM 推理及缺乏置信度评估方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统自然语言转查询方法受限于仅支持 SQL、过度依赖实时 LLM 推理以及无法在查询生成时评估语义正确性。

Method: 引入中间表示 (QueryIR)；设计七阶段检测流水线支持 SQL/GraphQL；开发零 LLM 的确定性模式；构建基于加性信号模型的运行时置信度评分系统。

Result: 确定性模式实现 100% 执行准确率且延迟仅 3.2ms；LLM 模式在 Spider 和 BIRD 基准测试中达到 84-91% 的执行准确率；模式感知提示（Schema-aware prompting）带来 18.4% 的准确率提升。

Conclusion: Text2ql 成功通过架构创新实现了高性能、高可靠的自然语言转数据库查询方案，为工业级应用提供了无需 LLM 推理的低延迟选择。
...
### [201] [AI agents reshape consensus formation in human groups](https://arxiv.org/abs/2609.02122)
*Lin Chen,Ziyi Liu,Xia Hu,Yong Li*

Main category: cs.CL

TL;DR: 研究探讨了大型语言模型（LLM）代理在混合人机群体中对集体共识形成的影响。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理成为人类群体中的参与者，需要理解其存在如何重塑集体行为及共识形成的动态过程。

Method: 通过协同描述游戏，在重复的两两沟通中改变LLM代理的比例，观察共识形成的过程、语义基础及沟通形式。

Result: 发现了三个共识体制：低代理比例下由人类主导共识；中间比例扰乱共识形成；高代理比例下由代理主导且恢复强共识。此外，人类和AI主导的共识在抽象程度和语义密度上存在显著差异。

Conclusion: AI代理的比例和透明度是影响群体规范形成、内容及合法性的关键设计变量，人类虽初始抵制AI观点，但会因从众压力逐渐接受。
...
### [202] [C$^{3}$T: Counterfactual Causal Reasoning for Sentiment Shifts in Social-Media Conversation Trees](https://arxiv.org/abs/2609.02131)
*S M Rafiuddin,Atriya Sen*

Main category: cs.CL

TL;DR: 该研究分析了社交媒体谣言传播过程中对话情绪的动态变化，并提出了一个名为C³T的模型来量化不同对话行为对情绪的影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽略了社交媒体分支树结构中，特定话语行为（如辩驳、提供证据、攻击）如何因果性地诱发后续回复的情绪转变。

Method: 引入了CaSiRe数据集（包含情绪标签、因果源标签和干预标签），并提出了C³T模型，该模型结合了线程结构、时间特征和因果反事实推理。

Result: C³T模型在情绪预测和归因分析上优于现有的文本、图和时间序列基准模型；发现辩驳和证据能降低负面情绪，而攻击行为则会增加负面情绪。

Conclusion: 结构感知和反事实建模对于理解复杂社交对话中的情绪传播至关重要，优于简单的LLM提示词工程。
...
### [203] [A Layered Taxonomy for Chinese Learner Grammatical Error Annotation](https://arxiv.org/abs/2609.02153)
*Mengyang Qiu,Jungyeul Park*

Main category: cs.CL

TL;DR: 本文提出了一种针对中文学习者写作的层级化语法错误标注框架，旨在平衡计算错误纠正（CGEC）的实用性与语言学的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的标注方案在一致性和语言学深度上存在不足，难以有效指导计算纠错模型的发展与教学应用。

Method: 构建了一个三层核心标注框架，整合了编辑操作、语言领域及词性，并针对中文特性（如时态、补语等）增加了特定扩展，利用LLM进行一致性验证与覆盖度分析。

Result: 验证了层级化方案的有效性与高覆盖度，同时指出了现有分类边界在自动化应用中仍需进一步细化。

Conclusion: 该分层标注方案为中文语法纠错提供了更具结构化和可扩展性的基础，有助于提升计算纠错系统与语言学分析的一致性。
...
### [204] [OBJECTION! Lawyer Agents Mitigate Guilty Bias in Legal Judgment Prediction](https://arxiv.org/abs/2609.02158)
*Jaehoon Jeong,Jay-Yoon Lee*

Main category: cs.CL

TL;DR: 提出了一种名为OBJECTION的推理阶段管道，引入对抗性律师智能体来对抗法律判断预测（LJP）模型中的“有罪偏见”。


<details>
  <summary>Details</summary>
Motivation: 现有LJP模型过度依赖检方叙事，在训练数据中存在严重的有罪标签倾斜，导致模型存在“有罪偏见”，未能体现无罪推定原则。

Method: 设计了三阶段推理（犯罪、违法性、罪责）的对抗性律师智能体，在推理过程中主动植入辩护论据，并构建了包含3.4k真实案例的“Natural Innocent”数据集。

Result: 将模型的错误定罪率（FGR）从82.93%大幅降低至16.69%。

Conclusion: 该方法通过在推理阶段引入辩护视角，有效缓解了法律AI模型中的有罪偏见，向实现真正的无罪推定原则迈出了重要一步。
...
### [205] [Do Cantonese-Adapted Language Models Better Predict Cantonese Reading? A Cross-Model Eye-Tracking Evaluation](https://arxiv.org/abs/2609.02163)
*Ziqi Zhang,Emmanuele Chersoni,Mohammad Momenian*

Main category: cs.CL

TL;DR: 该研究评估了粤语特定训练对大语言模型在心理语言学预测（眼动数据）中表现的影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究对于粤语特定训练是否能显著提升模型对人类阅读行为的预测能力尚无定论，且不同模型间的评估结果存在分歧。

Method: 通过对比粤语适配程度不同的两组模型对（GPT-2/JED351 和 Qwen2.5/CantoneseLLM），利用自然主义粤语眼动数据，分析词汇惊奇度（lexical surprisal）、词性惊奇度（POS surprisal）、前置熵和熵减四个信息论指标。

Result: 粤语专项训练更充分的 CantoneseLLM-7B 在词汇惊奇度及综合指标上表现最优；不同信息论指标下的模型排名存在差异，例如熵减指标更倾向于较小规模的适配模型。

Conclusion: 更深入的粤语专项训练通常能带来更好的预测拟合度，但模型的优劣评估高度依赖于所选择的具体信息论指标。
...
### [206] [Breadth Beats Depth: Improving GCG-Based Jailbreak Optimization with Breadth-Oriented Suffix Search](https://arxiv.org/abs/2609.02172)
*Shiliang Xiao,Jingsong Wei,Yuzhi Liang,Yufan Zheng,Xia Li,Qiliang Lin*

Main category: cs.CL

TL;DR: BOSS 是一种基于广度优先搜索策略的优化框架，旨在提升 GCG 类大模型越狱攻击的效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的 GCG 方法依赖平均损失和深度贪婪搜索，容易陷入局部最优，忽视了潜在的有效后缀空间，且计算效率较低。

Method: 引入“尾部聚焦对抗损失 (TFAL)”平衡难易样本，通过“行为覆盖”选择终端后缀，并采用多条短路径探索机制来取代单一的深度搜索。

Result: 实验表明 BOSS 在多种 GCG 变体上显著提升了攻击成功率，并有效降低了优化所需的计算耗时。

Conclusion: 通过广度优先的搜索策略和更精细的损失函数设计，BOSS 能够更高效地探索后缀空间，从而实现更强大的越狱攻击效果。
...
### [207] [PaperCompiler: Faithful Paper-to-Code Generation via Repository-Level Specification Compilation](https://arxiv.org/abs/2609.02272)
*Yunhao Liu,Hong Phuc Pham,Jaehong Yoon*

Main category: cs.CL

TL;DR: PaperCompiler 是一个将学术论文自动转换为代码仓库的框架，通过将论文内容“编译”为显式的实现规格说明书，确保代码实现与原论文在逻辑和结构上的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有从论文到代码的转换工具往往忽略细节，导致生成的代码在算法逻辑、评估协议及文件结构上出现简化或不一致，缺乏对论文原始证据的严谨追踪。

Method: 引入 PaperCompiler 框架，将论文转化为显式的实现规范（包括非降级要求、依赖关系和文件约束），通过追溯来源并区分确定的信息、推断信息和外部信息，引导代码生成过程。

Result: 在 Paper2CodeBench 上，该方法实现了 13.8% 的参考忠实度提升，并将严重评估缺陷率从 13.2% 降低至 6.1%。

Conclusion: 通过引入规范化的中间编译层，PaperCompiler 显著提升了自动化代码实现对原始论文的忠实度，有效解决了生成过程中的信息丢失与逻辑简化问题。
...
### [208] [Do Large Language Models Capture the Diversity in their Training Data?](https://arxiv.org/abs/2609.02275)
*Youqi Wu,Farzan Farnia*

Main category: cs.CL

TL;DR: 研究发现现代生成式模型在生成输出时的条件熵显著低于训练数据，存在系统性的“条件多样性鸿沟”。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型（LLM）是否能捕捉训练数据中所有合理输出的多样性尚不明确，缺乏量化工具。

Method: 利用信息论视角，通过条件熵和基于冯·诺依曼熵的矩阵分析法量化输出变异性；提出一种基于矩阵熵投影的后验修正机制，利用镜像下降算法优化输出多样性。

Result: 实验证实从LLM到图像生成模型均存在条件多样性缺失，且该鸿沟在不同模型规模和解码策略下普遍存在。

Conclusion: 该研究揭示了生成模型与训练数据间的条件多样性差距，并提供了一个有效的信息论框架用于测量和缓解该问题。
...
### [209] [Efficient GUI Agents: A Systems Survey of Observation, Memory, Action, and Runtime Optimization](https://arxiv.org/abs/2609.02309)
*Bizhe Bai,Jiakang Yuan,Hongming Wu,Xinyue Wang,Jie Ren,Siyao Chen,Yuchen Ya,Fan Bai,Pai Peng,Huafeng Qin,Tao Chen*

Main category: cs.CL

TL;DR: 该综述系统性地探讨了GUI代理的效率问题，指出GUI代理不仅需要关注任务成功率，还需平衡上下文、计算、动作预算及运行开销。


<details>
  <summary>Details</summary>
Motivation: 当前GUI代理研究过分强调任务成功率，忽视了实际部署中至关重要的效率指标（如延迟、成本和资源消耗）。

Method: 通过端到端的系统视角，从观察效率、上下文/内存效率、动作效率及规划/系统效率四个维度梳理现有文献，并进行深度综合分析。

Result: 研究发现现有方案趋向于采用选择性阅读、全局到局部的视觉分配、可恢复内存、验证感知控制及GUI与非GUI混合运行时等技术手段。

Conclusion: 未来的研究应重视验证成本的真实核算、基准测试的可比性，并推动观察、内存与执行层在延迟和隐私约束下的协同设计。
...
### [210] [NE-R1: Enhancing Named Entity Recognition Model via Reinforcement Learning](https://arxiv.org/abs/2609.02366)
*Meixuan Chen,Hehan Li,Ruizhi Zhao,Xin Lu,peizhi xu,Liwei Qian,LI Meifang,shuanglong li,Hanmeng Liu,Xin Pei,Yanbiao Ma*

Main category: cs.CL

TL;DR: NE-R1是一个自适应检索增强命名实体识别（NER）框架，通过“按需检索”机制和强化学习优化，有效平衡了参数知识与外部知识的使用。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在识别长尾及特定领域实体时存在局限性，而传统的RAG方案在处理已知实体时会引入噪声并增加成本。

Method: 提出了“按需检索”机制，并采用两阶段训练：多任务指令微调初始化及结合思维链（CoT）的端到端强化学习优化，通过多维度奖励函数实现检索决策。

Result: NE-R1在多项基准测试中达到SOTA水平，同域评估F1提升2.52%，零样本跨域评估F1提升1.18%。

Conclusion: 该方法通过按需引入外部知识，显著提升了模型识别长尾和专业领域实体的能力，并降低了不必要的检索开销。
...
### [211] [DiffIE: Diffusion-based Open Information Extraction](https://arxiv.org/abs/2609.02315)
*Konstantin Fedorov,Valentin Malykh*

Main category: cs.CL

TL;DR: DIFFIE是一种基于条件离散扩散模型的开放信息抽取（OpenIE）方法，通过多次反向扩散轨迹生成候选三元组并进行聚类与排序。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归生成方法速度慢且冗余，固定槽位预测方法则将提取数量与训练限制耦合，缺乏灵活性。

Method: 利用条件离散扩散过程对每个token进行角色标注，通过多次采样生成候选三元组池，并结合聚类与排序机制输出最终结果。

Result: 在CaRB和BenchIE等基准测试中达到SOTA水平，实现了提取数量与训练过程的解耦，并在推理阶段支持计算成本的动态调节。

Conclusion: 扩散模型中的随机性为处理具有多个有效输出的结构化预测任务提供了一种高效且灵活的机制。
...
### [212] [PolERo: Studying Political Evasion in Romanian](https://arxiv.org/abs/2609.02391)
*Gabriel Stefan,Sergiu Nisioi*

Main category: cs.CL

TL;DR: 该研究引入了罗马尼亚政治逃避数据集PolERo，探究了政治逃避分类在多语言和跨文化环境下的迁移能力及模型表现。


<details>
  <summary>Details</summary>
Motivation: 现有的政治逃避分类研究仅限于英语，缺乏对不同语言和政治背景下的适用性验证。

Method: 构建了包含5位罗马尼亚总统讲话的3,574个问答对数据集PolERo，并对比了TF-IDF、微调编码器、滑动窗口编码器及大语言模型（LLM）的分类性能，同时测试了跨语言迁移学习效果。

Result: 微调编码器表现出色；跨语言迁移呈现不对称性；涉及语用线索的模糊逃避类别是目前所有模型面临的主要挑战。

Conclusion: 政治逃避的检测在跨语言场景中仍具挑战性，尤其是对语境依赖强的逃避策略，模型迁移效果尚有提升空间。
...
### [213] [MultiGhostBench: A Multilingual Benchmark for Long-Form LLM-Generated Text Attribution under Distribution Shifts](https://arxiv.org/abs/2609.02379)
*Matteo Greco,Anudeex Shetty,Andrea Tagarelli,Jey Han Lau*

Main category: cs.CL

TL;DR: 本文提出了MultiGhostBench，这是一个包含6种语言、928本长篇书籍的LLM作者归因（AA）多语言基准测试，旨在解决现有研究在语言覆盖和文本长度上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM作者归因研究多局限于英语、受控环境及短文本，缺乏针对长文本、跨语言及多分布变化的鲁棒性评估基准。

Method: 构建了包含5种近期LLM生成、涵盖6种语言及3种书写系统的长篇文本数据集，并对统计学方法、指纹分析法及Transformer基检测器在多种分布变化场景下进行了评估。

Result: 实验发现，单一方法在不同设置下表现不一；Transformer模型在跨语言场景下表现出较好的生成器特征捕捉能力，而统计及指纹方法受语言依赖性限制较大，整体性能在分布偏移下均有所下降。

Conclusion: MultiGhostBench为开发更鲁棒的跨语言LLM作者归因方法提供了宝贵资源，并揭示了模型生成特征在不同语言间的迁移差异。
...
### [214] [Improving Health Literacy through Lay Summarization of Radiological Reports: An Evaluation of BioNER and Retrieval-Augmented Generation](https://arxiv.org/abs/2609.02396)
*Egecan Çelik Evgin,İlknur Karadeniz,Olcay Taner Yıldız*

Main category: cs.CL

TL;DR: 本研究评估了检索增强生成（RAG）和命名实体识别（NER）在将放射学报告转化为患者友好型摘要中的有效性。


<details>
  <summary>Details</summary>
Motivation: 放射学报告专业术语晦涩，患者自行使用通用LLM解读存在幻觉风险，因此需要更可靠的自动化 lay-summary 生成方法。

Method: 构建了一个结合 NER（提取临床发现）与 RAG（上下文接地）的框架，并在 Qwen 和 BioBART 模型上进行了少样本学习和微调对比实验。

Result: NER 能显著提高摘要的可读性和质量；单纯的 RAG 效果不佳且可能引入无关干扰；微调后的 BioBART 结合 NER 表现最佳。

Conclusion: 实体感知（Entity-aware）的提取是提升放射学报告患者友好型总结的核心驱动力，而非单纯的检索增强。
...
### [215] [Before the Script, Set the Stage: How Worldview Simulation Amplifies Psychologically Grounded Persuasion in Multi-Turn Jailbreaking](https://arxiv.org/abs/2609.02414)
*Siyu Chen,Haoran Wang,Xiaojian Li,Yao Huang,Yinpeng Dong,Wei Xu*

Main category: cs.CL

TL;DR: 该论文提出了BLUEPRINT框架，通过因子化的社交影响策略和跨轮次上下文模拟，揭示了多轮对话中大模型脆弱性的驱动机制。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以解释多轮越狱攻击中哪些具体对话机制导致了模型安全防御失效，需要更系统的评估框架来分析模型脆弱性。

Method: 引入BLUEPRINT框架，结合蒙特卡洛树搜索（MCTS）优化18种社会影响因子在四轮对话中的组合，并通过WORLDVIEWSIM模块模拟对话上下文。

Result: 在六个前沿模型上实现了极高的攻击成功率（ASR），且平均查询次数极少（2.46次）。研究发现，将任务具体化是攻破模型防御的关键因素。

Conclusion: 提升大模型安全性不仅需要监控有害内容，还需监控对话状态如何使不安全请求显得具体且具有可执行性。
...
### [216] [When Decodability Is Not Enough: Logical Validity Representations, Behavioral Dissociation, and Causal Tests in Language Models](https://arxiv.org/abs/2609.02438)
*Smitha Muthya Sudheendra,Jaideep Srivastava*

Main category: cs.CL

TL;DR: 研究发现大型语言模型（LLM）的隐藏状态中蕴含着逻辑有效性的表征，但这种表征往往无法转化为可靠的逻辑行为。


<details>
  <summary>Details</summary>
Motivation: 单一的输出正确与否无法揭示模型内部对逻辑推理的真正表征能力，需探究模型内部表征与行为表现之间的解耦关系。

Method: 利用匹配的有效/无效前提-结论对，在多种推理范式、语义领域和难度级别下，对五个开源Transformer模型进行探测（Probing）分析及因果干预测试。

Result: 逻辑有效性在隐藏状态中具有极高的可解码性，甚至在模型行为表现错误时依然可解码，但其泛化能力有限且因果干预效果较弱。

Conclusion: 模型表征逻辑有效性、在行为中表达逻辑以及逻辑的因果调用是三个独立的过程；模型虽能识别逻辑，但未必能将其可靠地应用于推理输出。
...
### [217] [PragAlign: Feedback-Guided Pragmatic Alignment for Controlled Synthetic Dialogue Generation](https://arxiv.org/abs/2609.02480)
*Smitha Muthya Sudheendra,Jaideep Srivastava*

Main category: cs.CL

TL;DR: PragAlign 是一种基于反馈的生成-评估-修正循环框架，旨在提升合成对话在特定意图、情感和情境约束下的生成质量。


<details>
  <summary>Details</summary>
Motivation: 在隐私受限的服务场景下，需要生成既能保持沟通意图和情感表达，又具备自然对话流的合成文本，但传统的单次生成往往难以满足复杂约束。

Method: 引入基于LLM的评估器，在生成后对意图对齐、情感对齐、连贯性及流畅性进行打分，并提供具体反馈进行多达三轮的迭代修正。

Result: PragAlign 的评估器接受率高达 99.50%，远超单次生成（72.25%）和无反馈重复生成（95.88%）；其中修正收益主要集中在情感对齐方面。

Conclusion: 该框架能有效提升对特定沟通约束的满足度，但情感表达的稳定性和人类感知质量仍是待解决的挑战。
...
### [218] [Learning to Fuse LLMs with Ontology Rankers for Rare-Disease Diagnosis](https://arxiv.org/abs/2609.02473)
*Zhaoyang Jiang,Zhizhong Fu,Yunsoo Kim,Zicheng Li,Xuanqi Peng,Fei Teng,Jiacong Mi,Honghan Wu*

Main category: cs.CL

TL;DR: 该研究提出了一种基于行为融合的方法，将大语言模型（LLM）与本体论排名器结合，在保留可解释证据的前提下显著提升了罕见病诊断的准确性。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM能生成差异化诊断，但缺乏可解释的证据链；而本体论排名器具备证据链但精度有限。研究旨在将两者优势互补，在不牺牲证据透明性的情况下提升诊断性能。

Method: 开发了一种基于行为的融合模型，该模型通过分析两个系统的排名列表、一致性以及本体论支持来动态学习对各系统的依赖权重，并修正了基准测试中的数据泄露问题。

Result: 在八个开源LLM上，融合模型使Phenomizer的Recall@1显著提升（最高提升20.18个百分点）；在与DeepSeek-V4-Flash搭配时，无需微调即可实现5.19个点的性能增长，且90.8%的正确诊断保留了可审查的证据。

Conclusion: LLM可以增强传统的诊断工具，且无需放弃临床诊断中至关重要的结构化证据支持。
...
### [219] [How LLMs Build Fictional Worlds: Setting and Narrative Space in AI-Generated Creative Storytelling](https://arxiv.org/abs/2609.02482)
*Katrin Rohrbacher,Björn Nieth,Emmanuelle Salin,Bjoern Eskofier,Michaela Mahlberg*

Main category: cs.CL

TL;DR: 该研究分析了大型语言模型（LLMs）与人类作家在叙事“世界构建”策略上的差异，发现LLMs倾向于过度描述感知空间，而人类更注重动作空间。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在小说创作中构建世界观的方式，以及它们与人类作家在叙事空间分配上是否存在系统性差异。

Method: 使用微调后的BERT分类器，将叙事空间划分为“动作”、“感知”、“视觉”、“描述”和“无空间”五类，对比分析了GPT-4.1、LlaMA 3.3、Mistral 3.2、Gemma 3生成的1,000个故事与Project Gutenberg人类文学语料库。

Result: 人类文学更侧重于“动作空间”（强调角色与环境的互动），而LLMs则系统性地过度生产“感知空间”（侧重气氛与情感影响），且该差异在不同语言和模型间表现稳定。

Conclusion: LLMs在世界构建上展现出与人类作家显著不同的模式，且这种模式具有模型特定性和语言敏感性。
...
### [220] [Debias-SparseGPT: Bias-Aware Pruning for Large Language Models](https://arxiv.org/abs/2609.02496)
*Irina Proskurina,Guillaume Metzler,Antoine Gourru,Julien Velcin*

Main category: cs.CL

TL;DR: 本文提出了Debias-SparseGPT，这是一种在模型剪枝过程中引入表征去偏置的新方法，旨在解决现有剪枝技术（如SparseGPT）放大模型偏差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型剪枝技术虽然能提高效率，但往往会加剧模型中存在的社会偏见，导致输出结果对提示词中的角色暗示过于敏感。

Method: 引入了一种基于二阶项的后训练剪枝方法，通过在包含人口统计学对比输入的校准集上进行优化，实现剪枝过程中的表征去偏置。

Result: 实验表明，Debias-SparseGPT在多种稀疏度水平下均能显著降低因剪枝导致的偏差，同时保持了模型原有的困惑度和零样本准确率。

Conclusion: Debias-SparseGPT在保持大模型计算效率的同时，优化了模型性能与公平性之间的平衡，提供了一种更稳健的稀疏化方案。
...
### [221] [When Persona Attributes Improve Population Alignment in Large Language Models](https://arxiv.org/abs/2609.02526)
*Leon Fröhling,Jens Rupprecht,Markus Strohmaier,Claudia Wagner*

Main category: cs.CL

TL;DR: 该研究探讨了“角色提示”（Persona Prompting）在大语言模型预测社会调查响应中的有效性，并分析了不同属性选择方法的影响及人类响应变异性的解释作用。


<details>
  <summary>Details</summary>
Motivation: 现有研究对角色提示的效果存在矛盾结论，且缺乏关于如何选择角色属性及为何有时表现不佳的系统性理论支撑。

Method: 通过四个社会调查数据集、六种大语言模型及二十项预测任务，比较了不同的角色属性选择方法，并引入人类响应变异性作为解释变量。

Result: 发现人类响应变异性是导致角色提示表现差异的关键因素；同时验证了属性选择方法对预测准确性的显著影响，并非属性越多效果越好。

Conclusion: 明确了角色提示在调查预测任务中的适用场景，并为优化基于大语言模型的社会调查预测提供了属性选择的实践指导。
...
### [222] [Predictors of Loneliness in Older Adults Using Multimodal Analysis of Speech and Language](https://arxiv.org/abs/2609.02606)
...
TL;DR: 该研究通过分析老年人的电话访谈音频和文本，发现孤独感可以通过语言特征（如否定词、社交引用）和声学特征（如语调、响度）的多模态分析进行检测。


<details>
  <summary>Details</summary>
Motivation: 孤独感对老年人身心健康影响巨大，但目前缺乏可扩展的、客观的孤独感检测手段，研究旨在探索自然对话中语言和声学线索与孤独感的相关性。

Method: 对310名老年人进行半结构化电话访谈，构建多模态框架，结合语言学特征（心理词典、n-gram、主题模型）与声学特征（音高、语调、响度），并使用模型预测自我报告的孤独感分数。

Result: 孤独感与否定词、负面语调和冲突相关语言正相关，与社交引用、动力驱动及语音情感丰富度负相关；多模态模型表现优于单一模态。

Conclusion: 孤独感在语言和声学上均有表现，语音分析可作为辅助心理评估和早期预警工具，但不宜作为独立的诊断手段。
...
### [223] [TaRA: Training-Aware Low-Rank Adaptation Initialization](https://arxiv.org/abs/2609.02639)
*Taehyeon Kim,Eunhyeok Park*

Main category: cs.CL

TL;DR: 提出了一种名为TaRA的LoRA初始化方法，通过使低秩因子的梯度逼近全秩权重矩阵的梯度，从而提升LoRA的微调效果。


<details>
  <summary>Details</summary>
Motivation: 现有的LoRA初始化方法往往忽略了全秩模型的训练动态，导致模型对初始化高度敏感且性能受限于低秩瓶颈。

Method: 推导出一套数学公式，通过对齐低秩适配器的梯度与全秩权重矩阵的梯度来优化初始化过程。

Result: TaRA在训练初期显著提高了梯度保真度，在多个微调任务中表现优于现有的SOTA方法，且计算开销极低。

Conclusion: TaRA是一种简单、鲁棒且可扩展的LoRA初始化解决方案，有效解决了低秩分解带来的性能瓶颈问题。
...
### [224] [WinoQueer-NL: Assessing Bias in Dutch Language Models toward LGBTQ+ Identities](https://arxiv.org/abs/2609.02651)
*Jiska Beuk,Gerasimos Spanakis*

Main category: cs.CL

TL;DR: 研究人员开发了一个针对荷兰语的WinoQueer数据集，用于评估和揭示荷兰语语言模型中存在的反酷儿（anti-queer）偏见。


<details>
  <summary>Details</summary>
Motivation: 英语语言模型的偏见研究已较成熟，但荷兰语模型在酷儿群体相关偏见的研究上存在空白，且需要更具文化语境的评估工具。

Method: 基于英语WinoQueer构建荷兰语数据集，通过43名荷兰酷儿参与者的问卷调查进行验证与扩充，并使用对数似然分数（log-likelihoods）评估多种掩码语言模型（MLM）和自回归语言模型（ARLM）的偏见水平。

Result: 虽然整体平均偏见分数接近中性，但模型在针对跨性别和非二元性别群体时表现出极高的偏见，最高偏见倾向达97%。

Conclusion: 该研究证实了荷兰语模型中存在显著的偏见差异，强调了开发文化适配的基准测试对于缓解针对边缘化群体偏见的重要性。
...
### [225] [oHC: Orthogonal Hyper-Connections on SO(4) via Quaternions](https://arxiv.org/abs/2609.02672)
*Haoqiang Guo,Xuyi Chen,Bo Ke,Yishu Lei,Ziyang Xu,Shikun Feng,Ximen,Wenhan Luo*

Main category: cs.CL

TL;DR: 该论文提出了一种名为正交超连接（oHC）的新架构，通过将Transformer的单一残差流替换为多个平行流，并使用旋转矩阵（SO(n)）进行混合，从而在保持训练稳定性的同时避免了信息流失。


<details>
  <summary>Details</summary>
Motivation: 现有的超连接（HC）方法中，无约束矩阵会导致训练不稳定，而双随机矩阵约束（mHC）会导致残差流随深度增加而趋同，从而降低模型的多样性和表达能力。

Method: 引入正交超连接（oHC），将残差流混合矩阵限制在旋转群SO(n)内，确保混合过程既不放大也不衰减残差流的范数，并在4流模型中利用单位四元数实现闭式参数化。

Result: oHC在保持训练稳定性的同时，有效防止了流间差异的坍缩，在多项下游任务中表现优于单流基线、mHC及iHC（恒等矩阵）。

Conclusion: oHC通过正交约束实现了比现有HC方法更稳定、更高效的并行残差流架构，为提升Transformer架构的扩展性提供了有效的解决方案。
...
### [226] [From Tokens to Semantics: Leveraging Complementary Signals for Hallucination Detection in Black-Box LLMs](https://arxiv.org/abs/2609.02679)
*Urja Pawar,Rajitha Ramanayake,Owen O'Neill,Nabeel Kemal,Abhishek Mandal,Houssem Chatbri,Christopher Martin*

Main category: cs.CL

TL;DR: 该论文提出了一系列检测大型语言模型（LLM）幻觉的方法，特别是在缺乏外部参考文档的情况下，通过语义熵和令牌不确定性来识别模型输出的虚假信息。


<details>
  <summary>Details</summary>
Motivation: 在缺乏信任参考的情况下，防止LLM幻觉对高风险工作流至关重要，旨在平衡识别错误与降低人力审核成本之间的矛盾。

Method: 研究了语义熵和令牌不确定性，提出了TopK（跨采样聚合令牌特征）、CoCoA（混合模型）、Gated（分类路由）和Stacked（集成学习）等检测策略。

Result: Stacked方法在近半数测试中表现最佳，TopK和CoCoA作为无监督方案在无需训练标签的情况下展现了竞争力，但没有单一方法在所有场景下均最优。

Conclusion: 检测幻觉的方法应根据特定任务需求和假阳性预算进行选择，且该研究为不同应用场景下的可靠性评估提供了基准参考。
...
### [227] [DKL: Decoupled Knowledge Learning for Instruction-Tuned Language Models](https://arxiv.org/abs/2609.02685)
...
TL;DR: DKL 是一种解耦知识学习方法，通过将基座模型的知识注入与指令微调后的模型合并，在不破坏指令遵循能力的前提下高效注入领域知识。


<details>
  <summary>Details</summary>
Motivation: 现有的 RAG 方法在检索失败时容易产生幻觉，而现有的微调方法（如 RAFT 或 EPT）要么需要大规模合成数据，要么在扩展预训练后会破坏模型的指令遵循能力，且补救性的指令微调（IFT）成本高昂。

Method: 在基座 LLM 上进行扩展预训练（EPT）以注入领域知识，随后利用模型合并技术将学到的权重集成到指令微调后的 LLM 中，从而实现知识注入与指令能力的解耦。

Result: 在检索失败的情况下，DKL 将 RAG 的准确率从 54.17% 提升至 79.26%，且在显著减少训练数据量的情况下优于现有方法。

Conclusion: DKL 是一种轻量级且有效的知识注入方法，能够规避高昂的指令微调成本，并有效保留指令遵循能力，为领域自适应提供了高效的新路径。
...
### [228] [Trace as State: Reasoning Traces as Conditional States for Long-Context Transformers](https://arxiv.org/abs/2609.02702)
*Xu Zou,Jie Tang*

Main category: cs.CL

TL;DR: 该研究提出了一种名为“Trace as State”的方法，通过将推理过程的轨迹（trace）置于长文本输入之前，有效改善了Transformer在长上下文推理中的性能。


<details>
  <summary>Details</summary>
Motivation: Transformer的因果处理机制与长文本推理中后期发现任务状态的需求存在冲突。在处理条件状态更新任务时，将条件置于前部可以显著降低内存需求并提升推理效率。

Method: 引入“Trace as State”框架，将预先生成的推理轨迹作为任务状态的文本代理，在进行长上下文读取之前将其置于输入序列的前端，从而引导后续的重读过程。

Result: 实验证明，“Trace as State”在多个模型和长上下文数据集上表现优异，在27项指标对比中胜出26项；在特定任务（如GraphWalks）中，模型准确率实现了大幅跃升。

Conclusion: 将推理轨迹置于长上下文之前是一种简单而有效的方法，能够显著增强Transformer在长文本环境下的逻辑推理能力，同时保持了模型的因果结构。
...
### [229] [Choosing a PEFT Variant for Per-Patient Dysarthric ASR: A Single-Speaker Case Study on Two ASR Bases](https://arxiv.org/abs/2609.02735)
*Bernard Muller,László Tóth,LaVonne Roberts*

Main category: cs.CL

TL;DR: 本文对比了七种LoRA系列PEFT方法在单名严重构音障碍患者（匈牙利语）自动语音识别（ASR）中的性能。


<details>
  <summary>Details</summary>
Motivation: 构音障碍患者ASR需个性化微调，但在“每患者适配器”模式下，尚缺乏对不同PEFT变体效能的系统性对比。

Method: 在Whisper和Qwen3-ASR基座上，针对单名患者数据，评估了LoRA, QLoRA, AdaLoRA, DoRA, LoHA, VeRA, VB-LoRA七种微调方法，并分析了数据量（注册时长）对性能的影响。

Result: LoRA表现最佳且简单经济；DoRA与其无显著差异；QLoRA在小规模适配中表现欠佳且无内存优势；扩展LoRA至前馈网络可逼近全量微调水平，仅需约3.7%的存储空间。

Conclusion: 在构音障碍ASR场景下，推荐使用LoRA进行高效微调，并指出仅需少量音频即可显著降低词错误率（CER）。
...
### [230] [Language Models Can Control Their Own Attention](https://arxiv.org/abs/2609.02737)
*Namgyu Ho,Huzama Ahmad,Woosung Koh,Se-Young Yun,Tal Schuster,Cicero Nogueira dos Santos*

Main category: cs.CL

TL;DR: 提出了一种名为 Declarative Attention (DA) 的机制，通过让模型在思维链中声明所需注意力范围（全局、聚焦或局部），动态减少长上下文推理中 KV 缓存的读取量。


<details>
  <summary>Details</summary>
Motivation: 现有长文本推理中，模型往往仅需关注少量上下文信息，但仍需对整个 KV 缓存进行 O(N) 的全量读取，造成严重的算力冗余。

Method: 引入 Declarative Attention (DA) 协议，将生成过程划分为 <global>（全量）、<focus>（聚焦特定区域）和 <local>（仅局部）三种模式，推理引擎将其解析为工具调用以跳过无关的 KV 缓存。

Result: 在 15 个长上下文任务的零样本测试中，该方法在 Gemma-2 和 Qwen 模型上显著减少了推理时的注意力 token 数量（最高达 52%），同时精度损失较小且随模型规模增加而进一步降低。

Conclusion: DA 提供了一种高效的稀疏注意力推理方案，利用模型的内在感知能力优化长文本推理效率，且无需重训练即可在现有模型上实现性能提升。
...
### [231] [Untangling the Mechanisms of Misleading Context in Medical Question Answering](https://arxiv.org/abs/2609.02754)
*Robin Linzmayer,Noémie Elhadad*

Main category: cs.CL

TL;DR: 该研究探讨了大型语言模型（LLM）在医疗决策中受误导性上下文影响的机制及可检测性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在医疗问答中表现出色，但误导性上下文可能腐蚀其医疗判断，目前缺乏对其易感性及推理腐蚀机制的系统性研究。

Method: 利用MedMisBench基准测试，在医疗问题中注入伪造证据和断言两种误导性提示；测试了三款推理模型，分析其推理轨迹及响应；并利用LLM监测器评估对错误决策的捕捉能力。

Result: 模型对“断言”类误导的易感性远高于“伪造证据”；误导性信息在推理轨迹中披露率较高但在最终响应中较低；证据类干扰通过早期渗透积累影响，而断言类干扰则在结论前夕重定向决策；查看推理轨迹可大幅提升对错误决策的监测准确率。

Conclusion: 模型最易受影响的误导信息往往在最终结论中被掩盖，且只有通过分析开放的推理轨迹才能有效监测其决策过程，这对目前隐藏推理过程的闭源模型提出了挑战。
...
### [232] [CORAL: An LLM-Native Harness for Production Recommender Systems](https://arxiv.org/abs/2609.02730)
...
TL;DR: 本文提出了CORAL，一个基于大语言模型的智能体闭环系统，用于自动执行生产级推荐系统的持续优化。


<details>
  <summary>Details</summary>
Motivation: 传统的人工在线实验优化过程缓慢且资源密集，难以应对动态变化的用户行为和模型环境。

Method: 将推荐优化建模为受约束的非平稳优化问题；利用LLM智能体在闭环中观测系统信号，通过记忆推理并调用数值优化工具来调整系统参数。

Result: 在两个大规模社交平台的实测中，CORAL在保持或提升用户参与度的同时显著优化了计算成本，且随着迭代性能持续增强。

Conclusion: CORAL证明了单一智能体循环能够有效替代传统算法工程师的部分工作，实现推荐系统的自动化持续优化。
...
### [233] [From Reweighting to Rewriting: Unlocking the Intervention Effects of Influential Samples in Training Data Attribution](https://arxiv.org/abs/2609.02771)
*Yuzhang Luo,Chenpeng Wang,Jianhui Chen,Liangming Pan*

Main category: cs.CL

TL;DR: 该研究探讨了训练数据归因（TDA）中的干预有效性，提出“影响导向响应重写”（influence-guided response rewriting）比传统的权重干预更能有效引导模型行为。


<details>
  <summary>Details</summary>
Motivation: 传统的影响函数（IF）在权重干预下表现不佳，研究者试图探究是样本本身缺乏价值，还是重写方法比重权法更能释放这些样本的干预潜力。

Method: 利用影响函数定位关键训练样本，通过替换响应内容（行为对齐或对立）而非改变样本权重来进行干预，并在四个开源大模型上对比了重写与重权的效果。

Result: 实验发现响应重写比重权法产生更强、更持久且可控的行为偏移，且影响函数定位的样本在重写任务中具有显著的干预优势。

Conclusion: 该研究证明了影响导向的重写是激活训练数据影响力的高效手段，并强调了评估TDA方法时应考虑不同干预手段的差异。
...
### [234] [HyperStyler: Low-resource Authorship Style Transfer via Context-aware Style Navigation and Hypernetworks](https://arxiv.org/abs/2609.02772)
*Jongkyung Shin,Minguk Jeon,Chanwoo Park,Chiehyeon Lim*

Main category: cs.CL

TL;DR: HyperStyler 是一种用于低资源风格迁移（LAST）的新框架，通过解耦风格选择与实现，显著提升了风格保真度与语义保留能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将作者风格压缩为单一静态嵌入，忽略了上下文相关的风格变化，且隐藏层表示导致风格与内容耦合，难以实现高质量的风格迁移。

Method: 提出了 HyperStyler 模型，包含两个关键组件：Stylo-navigator（基于源上下文和目标参考样本预测风格坐标）和 Stylo-hypernet（通过动态参数调制实现风格迁移，而非传统的隐藏状态注入）。

Result: 在 Reddit、Blog 和 News 数据集上的实验表明，HyperStyler 在保持较少额外参数（仅增加 2.4%）的情况下，性能优于基于 LLM 的方法，推理速度提升 1.8 倍以上。

Conclusion: HyperStyler 通过将风格迁移任务解耦并引入动态超网络调制，成功解决了低资源环境下的风格迁移瓶颈，具有更高的灵活性和跨领域泛化能力。
...
### [235] [EarlyEval: Cheaper Agent Evaluation via Early Outcome Prediction](https://arxiv.org/abs/2609.02783)
*Yuling Shi,Zhensu Sun,Junsen Dong,Chengcheng Wan,David Lo,Xiaodong Gu*

Main category: cs.CL

TL;DR: 提出了一种名为 EarlyEval 的评估框架，通过在 LLM 代理任务执行过程中利用中间行为进行结果预测，提前终止运行以降低昂贵的评估成本。


<details>
  <summary>Details</summary>
Motivation: 当前 LLM 代理的评估成本极高，现有的基准蒸馏方法仅减少任务数量，未解决单个任务执行成本高昂的问题。

Method: 引入“早期结果预测”机制，训练 LightGBM 分类器分析代理的中间行为（文本、特征等），在达到置信度阈值时提前终止任务执行。

Result: 在三个基准测试中，EarlyEval 减少了 13%-26% 的执行步骤和最高 44.1% 的输入 Token，同时保持了 89%-97% 的预测准确率，对评估结果的影响仅为 1-2 个百分点。

Conclusion: 该方法有效且高效，证明了通过监控中间过程来预测最终结果是降低大规模代理评估成本的有效途径。
...
### [236] [DiscoSign: Discourse-Aware Text to Sign Language Gloss Translation](https://arxiv.org/abs/2609.02796)
*Vasileios Baltatzis,Mert Inan,Connor Gillis,Raja Kushalnagar,Lorna Quandt,Leah Findlater,Colin Lea*

Main category: cs.CL

TL;DR: DiscoSign 是首个旨在实现话语级（discourse-level）文本到手语术语翻译的计算框架。


<details>
  <summary>Details</summary>
Motivation: 传统手语处理系统局限于句子层面，忽视了手语理解中至关重要的语篇连贯性与空间指代等现象。

Method: 利用大语言模型（LLM）构建模块化框架，重点处理空间共指消解、问答从句结构及概念-术语一致性，并开发了针对语篇连贯性的新型评估指标。

Result: 该方法在保持句子级翻译质量的同时，显著提升了语篇中的空间一致性和实体追踪能力。

Conclusion: 该研究建立了首个系统性的语篇级手语翻译框架及其评估方法，弥补了手语处理在语篇层面的研究空白。
...
### [237] [User Feedback Provides a Unique Signal that LLMs Can not Detect](https://arxiv.org/abs/2609.02859)
*Shachar Don-Yehiya,Leshem Choshen,Omri Abend*

Main category: cs.CL

TL;DR: 该研究指出，用户反馈是提升大语言模型（LLM）的有效信号，现有的研究认为其无效是因为评价范式存在系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 学术界普遍认为用户反馈噪声过大难以利用，研究者旨在挑战这一观点并纠正评估偏见。

Method: 通过构建带有明确真实标签（Ground Truth）的合成数据与自然数据，对比使用反馈与不使用反馈的模型修订效果，并分析LLM裁判在评估中的偏差。

Result: 实验证明，在反馈指导下进行的模型修订能显著解决特定问题；同时发现LLM裁判往往会错误地偏好质量更差的基线输出，忽略了基于反馈的有效修正。

Conclusion: 用户反馈是极其宝贵的改进资源，当前对反馈利用效果评估不佳的原因主要源于模型评估器（LLM judges）存在的系统性偏差。
...
### [238] [Not All Agreement Counts as Corroboration: Provenance-Conserving Multi-View Fusion for Typed Action Admission in Human-Robot Collaboration](https://arxiv.org/abs/2609.01662)
*Zekai Jin,Hanrong Zhang,Yihong Tang,Fei Hu,Zhen Dong,Yi Shao*

Main category: cs.RO

TL;DR: 提出了一种名为PACT的框架，旨在解决具身智能体在处理重复观察时的证据权重过高问题，通过引入溯源（provenance）分区确保证据的独立可计数性。


<details>
  <summary>Details</summary>
Motivation: 现有的预测一致性无法区分证据的真实来源，导致重复的单一观测被错误地视为多重证据，从而过度放大置信度并引发错误的行动。

Method: 引入“溯源分区”作为关系变量，通过仅在分区内积累证据、对跨分区数据应用限制条件（保持、确认或回退），实现保守的证据融合与行动准入控制。

Result: 在48个场景簇和3.12万次评估中，PACT显著降低了风险覆盖面积（ncsAURC）；在人机协作测试中，PACT有效过滤了重复观测带来的误导，提升了行动的准确性和鲁棒性。

Conclusion: 一致性仅在证据来源独立时才构成有效的相互印证。PACT成功将计算冗余与证据多重性分离开来，为具身系统提供了更可靠的决策机制。
...
### [239] [TriSAR: Task Coordination and Collision Avoidance for Aerial Robot Teams in Disaster Response](https://arxiv.org/abs/2609.01731)
*Aditya Anil Kapile,Pedro Machado,Isibor Kennedy Ihianle*

Main category: cs.RO

TL;DR: 本研究评估了多无人机（UAV）灾难响应系统在任务分配策略与局部轨迹控制协同下的效能与安全性。


<details>
  <summary>Details</summary>
Motivation: 现有研究对于多无人机协调层中任务分配与轨迹控制对任务效率及安全性的具体贡献缺乏受控实验条件下的量化评估。

Method: 利用Gazebo仿真器构建地震受损城市环境，采用2x2因子设计，对比两种任务分配策略（遗传算法GA vs. 贪婪算法）以及是否启用反应式避碰机制。

Result: 反应式避碰显著提升了飞行安全性，消除了碰撞风险；任务效率方面，遗传算法在避碰机制禁用时表现出显著优势，但在启用避碰时与贪婪策略相比无统计学差异。

Conclusion: 反应式避碰是保障安全的核心，遗传算法带来的复杂性在协同避碰机制存在时效能边际效应递减，验证了避碰机制对系统表现的调节作用。
...
### [240] [Designing Versatile Samples for Learned Trajectory Scoring](https://arxiv.org/abs/2609.01799)
*Yaguang Li,Jiaru Zhang,Chuheng Wei,Can Cui,Ziran Wang*

Main category: cs.RO

TL;DR: 该研究通过优化端到端自动驾驶策略中轨迹评分器的训练数据，通过扰动人类驾驶轨迹生成更有区分度的正负样本，显著提升了现有生成式规划器的决策表现。


<details>
  <summary>Details</summary>
Motivation: 现有端到端驾驶模型在轨迹选择阶段缺乏边界情况的有效监督，导致模型在决策边界附近的表现受限，且原始的轨迹生成方式难以提供足够的学习信号。

Method: 设计了一个新的训练数据集，通过在横向（向可行驶边界）和纵向（向前方车辆）两个维度扰动人类驾驶轨迹，构建更具信息量的训练样本，并将其应用于冻结后的 DiffusionDrive 和 MeanFuser 模型进行评分器微调。

Result: 实验表明，使用该方法后，DiffusionDrive 和 MeanFuser 在 NAVSIM 数据集上的 EPDMS 指标分别提升至 90.1 和 90.4，证明了增强训练数据对评分器能力的提升效果。

Conclusion: 针对决策边界的针对性数据增强可以显著提升端到端驾驶策略中轨迹选择组件的性能，无需重新训练复杂的骨干模型即可实现更稳健的决策。
...
### [241] [MACAW: Reliable And Efficient Surgical Debridement Using Monocular Adaptive Compact Attention Windows](https://arxiv.org/abs/2609.01961)
*Ziyang Chen,Shutong Jin,Preethi Satish,Sareena Mann,Cael Magner,Danyal Fer,Omid Mohareri,Gary Guthart,Ken Goldberg*

Main category: cs.RO

TL;DR: 该研究提出了一种名为MACAW的新型系统，旨在增强手术机器人在清创术中的灵活性，通过视觉伺服和单目自适应注意力窗口实现高效精准的组织清除。


<details>
  <summary>Details</summary>
Motivation: 手术清创过程中存在空间感知偏差和电缆驱动不精准的问题，亟需一种能够自动完成此类冗长乏味任务的增强灵活性系统。

Method: 结合视觉伺服系统用于图像平面内的抓取器对齐，并开发了MACAW（单目自适应紧凑注意力窗口）用于深度控制，同时在dVRK机器人上进行了单手及双手清创实验。

Result: 视觉伺服将位置偏移降至5像素以内；MACAW在单手实验中达到93%成功率，处理速率为304个碎片/小时；在双手实验中成功率保持92%，处理速率提升至473个碎片/小时。

Conclusion: MACAW显著优于现有基线方法，通过自动化提升了清创手术的效率与精准度，在临床辅助手术领域具有显著应用潜力。
...
### [242] [One Demonstration, Many Objects: Generalizing Manipulation via Local Contact Geometry](https://arxiv.org/abs/2609.01938)
*Satvik Sharma,Samrat Sahoo,Huang Huang,Fei-Fei Li Jiajun Wu,Dorsa Sadigh,Jeannette Bohg*

Main category: cs.RO

TL;DR: DemoMimic 提出了一种基于接触点局部几何特征的灵巧手操作策略，通过引入接触导向奖励函数，显著提升了跨物体和跨任务的 Sim-to-Real 迁移性能。


<details>
  <summary>Details</summary>
Motivation: 现有灵巧手操作研究在处理大规模数据采集时存在困难，且现有的模仿学习方法在精确接触控制奖励设计上不足，导致模型在面对未见过物体时泛化能力较差。

Method: 提出了 DemoMimic 框架，利用接触点周围的局部几何特征进行感知，并设计了以接触为中心的奖励函数，强制模型学习精确的接触策略，从而提高 Sim-to-Real 的一致性。

Result: 在 16 个物体、4 项任务及两种不同机器人手上的实验表明，DemoMimic 取得了 71% 的成功率，且在 Sim-to-Real 的性能损失上显著优于基线方法。

Conclusion: 专注于局部接触结构的策略能够有效提升灵巧手在不同物理属性物体上的泛化能力，为解决 Sim-to-Real 迁移难题提供了有效途径。
...
### [243] [Design and Validation of a Lightweight, Low-Profile Powered Knee Prosthesis with Quasi-Direct Drive Actuation](https://arxiv.org/abs/2609.02003)
*Ross J. Cortino,Ryan Posh,Emily G. Keller,Robert D. Gregg*

Main category: cs.RO

TL;DR: 本文提出了一种新型轻量化、低剖面的准直驱（QDD）动力膝关节假肢，旨在实现高性能与商业实用性。


<details>
  <summary>Details</summary>
Motivation: 传统高减速比执行器存在控制和反向驱动性问题，而既有的QDD方案通常过于笨重，难以商业化应用。

Method: 通过优化18:1两级传动系统，并结合热学与结构有限元分析进行轻量化设计，同时验证其输出扭矩、反向驱动能力及控制精度。

Result: 该假肢重量仅2.6kg，峰值扭矩达145Nm；在实验中，截肢患者佩戴该假肢进行平地行走和坐站转换时，运动学和动力学表现接近健全人水平。

Conclusion: 该研究验证了QDD假肢在临床上的可行性，且在重量、尺寸和性能上优于或等同于领先的商业化产品。
...
### [244] [Real-Time Dynamics-Based Torque-Sampling MPPI for Compliant and Force Aware Manipulation](https://arxiv.org/abs/2609.02020)
*Euncheol Im,Taehyun Kim,Yonghwan Oh,Myotaeg Lim,Yisoo Lee*

Main category: cs.RO

TL;DR: 提出了一种基于MPPI的机器人操作臂任务空间控制框架，通过GPU加速实现实时动力学求解与安全约束控制。


<details>
  <summary>Details</summary>
Motivation: 传统MPC难以处理非结构化环境下的复杂非线性动力学，且实时性与交互安全性受限。

Method: 利用模型预测路径积分（MPPI）结合基于转矩采样的架构，在GPU上进行并行计算，将刚体动力学和安全约束直接集成于MPC中。

Result: 在7自由度机械臂上实现了超过166Hz的求解频率和0.18秒的预测时域，有效提升了柔顺控制与力交互表现。

Conclusion: 该框架通过高效的并行计算，成功解决了复杂非线性动力学下的实时轨迹跟踪与力控难题，适用于非结构化环境下的安全交互。
...
### [245] [Modeling What Changes: Sparse, Residual World Models for Object-Centric Manipulation](https://arxiv.org/abs/2609.02046)
*Param Thakkar,Parsika Paresh Shah,Manisha Sushant Gote*

Main category: cs.RO

TL;DR: 研究提出了一种通过显式建模物体变化而非重构整个场景的稀疏化世界模型，有效提升了物理预测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 单体式世界模型在每一步预测整个状态，导致模型资源浪费在静态背景上并引入累积误差，因此需要一种能聚焦于物体变化的更高效架构。

Method: 设计了带门控机制的稀疏/残差模型，通过一个物体级变更门控和一个仅扰动受影响物体的残差增量头来实现物理预测。

Result: 实验表明，该模型在预测精度上优于密集模型（提升2.5至4.6倍），参数量更少（减少8.6至11.1倍），具备极佳的零样本迁移能力，且在自动回归预测中有效抑制了误差漂移，成功支持了规划任务。

Conclusion: 建模“变化”而非“重构全景”是实现物体中心化物理AI的一种简单且高效的归纳偏置。
...
### [246] [Koopman-Based Robust Model Predictive Control for Nonlinear Systems with Stochastic Intermittent Measurements](https://arxiv.org/abs/2609.02079)
*Guanhua Liu,Tong Wu,Lixian Zhang,Weifeng Du,Minghao Han*

Main category: cs.RO

TL;DR: 提出了一种基于 Koopman 算子和随机 MPC 的框架，用于解决非线性系统在间歇性测量丢失下的控制问题。


<details>
  <summary>Details</summary>
Motivation: 间歇性测量会导致预测不确定性增加及状态重置，进而影响受限非线性系统的闭环稳定性和递归可行性。

Method: 结合 Lipschitz 约束的深度 Koopman 模型构建线性潜空间预测器；使用马尔可夫跳变系统建模间歇性测量；通过分布无关的概率误差半径进行约束收紧，并引入精确惩罚软约束机制。

Result: 证明了预测误差的均方最终有界性，并实现了闭环调节误差的递归可行性，通过视觉伺服跟踪任务验证了该方法的有效性。

Conclusion: 该方法有效缓解了测量间歇性对模型预测控制的影响，在高不确定性场景下表现出良好的稳定性和鲁棒性。
...
### [247] [Towards Effective Physical Reservoir Computing with a Pneumatic Soft Robot](https://arxiv.org/abs/2609.02157)
*Jeevan Hebbal Manjunath,Jun Wang,Suyi Li,Wenlong Zhang*

Main category: cs.RO

TL;DR: 该研究探讨了气动软体手臂中物理储备池计算（PRC）的设计准则，分析了拓扑结构、刚度及传感器配置对弯曲角度估计性能的影响。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏关于如何有效设计物理储备池系统的形式化研究准则。

Method: 使用带有五个传感气囊的气动软体手臂，通过改变气囊连接拓扑、基准压力（刚度）和传感器数量，在固定岭回归估计器下对36组实验数据进行基准测试。

Result: 独立密封的气囊能保留更丰富的状态；提高基准压力会降低耦合拓扑的估计精度；在密封拓扑中，使用两到三个传感器即可达到最优性能，增加更多传感器收益递减。

Conclusion: 物理储备池系统的拓扑结构、刚度和传感器数量应进行协同设计，仅通过增强激励无法弥补不良设计导致的信息损失。
...
### [248] [Unified Motion Retargeting for Humanoids with Learned Point Cloud Correspondence](https://arxiv.org/abs/2609.02134)
...
TL;DR: 提出了一种名为UMR的通用运动重定向框架，通过学习点云间的稠密对应关系，实现了跨形态、跨来源的人形机器人运动重定向。


<details>
  <summary>Details</summary>
Motivation: 传统重定向方法依赖人工定义的关键点对应，难以扩展至不同机器人形态，且无法有效捕捉精细的身体姿态和交互细节。

Method: 引入统一运动重定向（UMR）框架，将人体和机器人转化为点云表示，通过学习点云间的稠密对应关系来解耦骨骼结构差异，并利用约束点云匹配优化进行姿态对齐。

Result: UMR在多种异构运动源和机器人形态下表现出卓越的重定向效果，在运动保真度和交互任务中显著优于现有技术。

Conclusion: UMR通过基于点云的通用接口，实现了高度可扩展的运动重定向，为大规模人类运动数据向机器人训练数据的转换奠定了基础。
...
### [249] [Hardware-Accelerated Instance Segmentation for Resource-Constrained Space Robotics with Criticality Analysis](https://arxiv.org/abs/2609.02219)
*Siddhant Shete,Hilmi Dogu Kücüker,Udo Frese,Frank Kirchner*

Main category: cs.RO

TL;DR: 该论文提出了一种针对月球机器人设计的低功耗、抗辐射的轻量化实例分割框架，解决了极端弱光环境下的实时感知难题。


<details>
  <summary>Details</summary>
Motivation: 月球任务面临极端弱光、计算资源受限以及空间辐射导致硬件推理错误的三重挑战，现有的感知系统难以在这些约束下保持鲁棒性。

Method: 引入了无需标签的激活方差信息采样（AVIS）进行量化校准；在深度学习处理器（DPU）上部署改进的YOLO模型以实现静态编译与低延迟；通过软件级关键性分析指导辐射防护。

Result: 在月球微型漫游车平台上，AVIS校准找回了69.8%的量化精度损失，推理延迟降至309ms，功耗仅5.7W，且通过针对性缓解策略将全局故障关键性降低了31.7%。

Conclusion: 该研究提供了一套集成化方案，验证了在资源受限且高辐射的空间环境中实现可靠、安全人工智能感知的可行性。
...
### [250] [FOCUS: Foot Observation Confidence for Robust Humanoid Proprioceptive Odometry](https://arxiv.org/abs/2609.02222)
*Kaixin Feng,Angsong Li,Shaopeng Zhang,Enyu Li,Peiwen Lin,Chuang Wang,You Li,Haiyu Lan*

Main category: cs.RO

TL;DR: FOCUS是一种用于四足机器人状态估计的新方法，通过预测连续的足端运动学（FK）可靠性权重，取代传统的二元接触判定，从而提高里程计的精度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于二元接触判定的接触辅助估计器在动态运动中（如拖地、打滑）容易导致累积漂移，因为接触并不等同于FK测量的可靠性。

Method: 提出FOCUS框架，利用神经网络预测每个足端的连续可靠性权重，通过该权重对FK观测进行加权融合，并自适应调整扩展卡尔曼滤波（EKF）的观测协方差，模型采用无需人工标注的模拟训练方法。

Result: 在模拟实验中绝对轨迹误差（ATE）降低了83.7%，在真实机器人测试中，无论是步行还是动态运动场景，ATE均有显著下降（分别降低70.8%和42.7%）。

Conclusion: 引入连续的FK可靠性权重能有效解决传统二元接触判定导致的漂移问题，使机器人能够在复杂动态运动中实现更稳健、平滑的状态估计。
...
### [251] [DiffuSearch: How Hybrid Trajectory Planning Benefits from Aligned Objectives in Diffusion and Action Space](https://arxiv.org/abs/2609.02252)
*Steffen Hagedorn,Aron Distelzweig,Alexandru P. Condurache*

Main category: cs.RO

TL;DR: DiffuSearch 提出了一种统一轨迹规划架构，结合引导扩散模型与蒙特卡洛树搜索（MCTS），通过共享目标函数实现高质量的自动驾驶轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 现有混合规划架构因模块间目标不一致，导致初始轨迹与精细化轨迹脱节，产生次优行为。

Method: 采用两阶段方案：第一阶段利用引导扩散模型生成场景一致的候选轨迹；第二阶段通过MCTS在离散动作空间进行局部精修，两者均使用相同的目标函数（防碰撞、舒适度、进度等）。

Result: 在nuPlan和interPlan基准测试中表现优异，显著降低了碰撞率并提升了舒适度，尤其在复杂交互场景下效果显著。

Conclusion: 通过在隐式生成与显式搜索中共享统一的驾驶目标，能够有效整合扩散模型的全局一致性与MCTS的可解释约束处理能力，是实现高性能自动驾驶规划的有效路径。
...
### [252] [Contact-Constrained Lower-Limb Joint-Offset Calibration for Humanoid Robots](https://arxiv.org/abs/2609.02306)
*Kaixiang Lu,Haiyu Lan,Chunxiao Qiao,You Li,Chengyuan Luo,Enyu Li,Peiwen Lin,Chuang Wang*

Main category: cs.RO

TL;DR: 提出了一种仅利用机器人板载关节编码器和骨盆IMU的足式机器人关节零位自标定框架。


<details>
  <summary>Details</summary>
Motivation: 传统标定方法高度依赖外部动捕系统或基准目标，无法实现机器人独立校准。

Method: 利用双足支撑状态下前向运动学计算的足间变换恒定性，通过最小化姿态相关离散度解决12维零位偏移的非线性最小二乘问题；分析Hessian特征结构以处理旋转耦合，并引入膝关节先验以增强弱可观性方向。

Result: 通过仿真与实机测试验证，显著降低了足部高度RMS残差（如A2从8.03mm降至1.43mm），激光雷达惯性参考实验证实了标定的有效性。

Conclusion: 该方法通过少量静态双足支撑姿态即可实现有效的关节零位校准，无需外部辅助，提高了人形机器人运动学的精确度。
...
### [253] [Humanoid Safe Stop via Learned Stoppability Value](https://arxiv.org/abs/2609.02358)
*Junfeng Long,Pieter Abbeel,Koushil Sreenath,Roberto Horowitz,Guanya Shi,C. Karen Liu*

Main category: cs.RO

TL;DR: Safe-Stop 是一种人形机器人应急停止框架，通过结合停止策略和可停止性估计器，在执行停止命令前评估其安全性，避免盲目执行。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人应急停止往往采用固定动作，未考虑当前状态下安全停止的可行性，易导致摔倒或碰撞。

Method: 将应急停止建模为“到达-规避”(reach-avoid)问题，利用停止概率估计器（基于经验）和到达规避估计器（基于Hamilton-Jacobi方程）进行互补评估，并结合阻尼失效策略。

Result: Safe-Stop 在不同任务间具有通用性，能够通过双重估计器有效判断停止可行性，在保证反应速度的同时提高了系统鲁棒性。

Conclusion: 该框架实现了对机器人应急停止的智能决策，显著提升了机器人处理突发停止请求时的安全表现。
...
### [254] [From Multi-Fisheye Sensing to Panoramic Perception: A Parallax-Aware Onboard Platform for Ultra-Low-Altitude UAVs](https://arxiv.org/abs/2609.02319)
*Dun Dai,Ze Lu,Cheng He,Yaowen Wang,Quan Quan*

Main category: cs.RO

TL;DR: 提出了一种面向超低空无人机的视差感知全景成像平台，能将四路鱼眼视频实时合成为等距柱状投影（ERP）全景图。


<details>
  <summary>Details</summary>
Motivation: 超低空无人机在复杂环境（如建筑物、植被附近）飞行时，需要高质量的环绕视觉感知能力，但现有的固定深度拼接方法存在视差对齐问题。

Method: 开发了基于NVIDIA Jetson Orin NX的嵌入式平台，采用自适应深度选择、受控接缝处理和光度融合技术，并提出了内容自适应接缝搜索与验证门控残差网格算法。

Result: 相比固定深度法，远场特征错位降低了41.6%；在20Hz运行频率下，功耗仅为13.29W，且在视觉地点识别任务中达到90.8%的Recall@5。

Conclusion: 该系统成功实现了视差感知的实时全景拼接，为超低空无人机提供了一种低功耗、高精度的嵌入式全景视觉感知架构。
...
### [255] [Pre-Lane-change Signal in Transitional Autonomous Vehicles: Results from Controlled Experiments](https://arxiv.org/abs/2609.02575)
*Zeyu Mu,Danjue Chen,Abhinav Sharma,George F. List*

Main category: cs.RO

TL;DR: 该研究分析了量产自动驾驶汽车（tAV）在执行强制换道时的决策过程，提出了“信号时间（SigT）”概念，并验证了在横向移动前即可预测换道意图。


<details>
  <summary>Details</summary>
Motivation: 旨在理解量产自动驾驶汽车如何从预换道状态过渡到换道执行，以及在横向动作开始前目标间隙的选择是否可观察。

Method: 利用150次受控强制换道实验数据，采用Firth逻辑回归模型，基于SigT时刻的相对位置和速度预测目标间隙选择，并对比分析原地与重定位的纵向轨迹。

Result: 模型预测准确率达0.89，证实了在横向移动开始前，通过SigT的交通状态能有效预测目标间隙选择，且原地与重定位场景遵循不同的纵向准备路径。

Conclusion: 支持“两阶段换道”猜想（纵向准备阶段与横向执行阶段），为构建更精准的自动驾驶换道模型提供了理论依据。
...
### [256] [A Physics-Consistent Benchmark for Contact-Rich Human-Robot Interaction in Assistive Care](https://arxiv.org/abs/2609.02402)
*Chengxiao He,Shanghai Yuan,Liuqun Fan,Shenzhen Zhu*

Main category: cs.RO

TL;DR: 该论文提出了一个针对接触密集型人机交互（如机器人辅助洗澡）的物理一致性基准，强调仅凭任务成功率不足以评估物理安全性。


<details>
  <summary>Details</summary>
Motivation: 传统的任务级评估忽略了物理接触中的安全风险，在辅助机器人领域，任务成功并不等同于物理交互的质量和安全性。

Method: 构建了一个包含可形变被动响应人体模型、结合物理感知的评分机制，并采用“冻结”的视觉/评分协议，通过力-压痕测量对模拟响应进行校准。

Result: 实验发现，单纯的任务成功率往往掩盖了物理安全隐患；例如，高任务成功率的策略在经过正确区域和力安全筛选后，成功率大幅下降。

Conclusion: 任务完成并不代表物理接触的有效性，在部署接触密集型机器人策略前，必须进行基于物理感知（Physics-aware）的安全性筛选。
...
### [257] [MS-MEM: Multi-Skill Manipulation-Enhanced Mapping via Uncertainty- and Disturbance-Aware Action Selection](https://arxiv.org/abs/2609.02493)
*Yitian Shi,Jesper Mücke,Nils Dengler,Sicong Pan,Rania Rayyes,Maren Bennewitz*

Main category: cs.RO

TL;DR: 提出了一种名为MS-MEM的框架，通过结合主动视觉、物体推移和抓取，在受限杂乱空间中实现高效且低扰动的场景建模。


<details>
  <summary>Details</summary>
Motivation: 服务机器人在狭窄、遮挡严重的货架环境下进行场景理解和物体检索时，面临遮挡严重、可操作空间有限且需避免破坏性操作的挑战。

Method: 利用证据理论构建不确定性感知映射框架，结合场景级度量语义估计器与全证据抓取估计器；通过统一的信息增益准则进行主动动作选择，并引入“附带干扰约束（CDC）”以最小化操作对场景的不必要扰动。

Result: 实验表明，MS-MEM在保持高地图准确性的同时，显著降低了场景的附带干扰，证明了视觉与多种操纵技能协同工作的优势。

Conclusion: 集成多种操纵技能（推、抓）与主动感知，能够有效解决受限环境下的不确定性建模问题，在提升场景理解能力的同时实现了环境友好的操作。
...
### [258] [An Adaptive Control Architecture for Slope and Terrain Compensation in Autonomous Navigation in Mediterranean Greenhouses](https://arxiv.org/abs/2609.02487)
*Fernando Cañadas-Aránega,Dirk Wollherr,José L. Guzmán,José C. Moreno,José L. Blanco-Claraco*

Main category: cs.RO

TL;DR: 提出了一种针对温室移动机器人的地形自适应轨迹跟踪策略，通过考虑负载、地形类型和坡度来提高导航精度。


<details>
  <summary>Details</summary>
Motivation: 温室中不平坦的地形和不同材质导致机器人难以实现稳定、精确的轨迹跟踪，传统控制难以应对这些动态扰动。

Method: 通过对温室常见地形进行实验表征，利用IMU测量坡度，并结合模型预测控制（MPC）和PI控制构建级联控制系统，引入增益调度技术的自适应前馈控制。

Result: 仿真结果表明，该差速驱动机器人在误差指标和控制信号效率方面均有显著提升，展现了良好的鲁棒性。

Conclusion: 该自适应控制策略能有效补偿坡度和地形变化带来的干扰，显著改善了农业机器人在复杂环境下的导航性能。
...
### [259] [World-Model-Augmented Visual Locomotion for Humanoids on Foothold-Constrained Terrain](https://arxiv.org/abs/2609.02542)
*Yuxi Liu,Lijun Han,Ziming Wang,Ao Zhang,Cong Yang,Wei Sui*

Main category: cs.RO

TL;DR: 提出了WM-LOCO框架，利用循环世界模型提供预测性特征，实现足式机器人对立足点受限地形（如踏脚石、缝隙、楼梯）的鲁棒导航。


<details>
  <summary>Details</summary>
Motivation: 传统基于局部观察的步态策略在复杂、不连续地形中容易因缺乏对未来地形的预判而导致失误，需要一种能捕捉长期依赖的预测机制来辅助决策。

Method: 引入WM-LOCO算法，通过联合训练循环世界模型和PPO策略，仅依赖本体感知和单目深度图像，生成包含未来观测和奖励预测的隐藏特征以指导运动决策。

Result: 在仿真中超越了基线策略，显著提高了在缝隙和踏脚石地形的成功率，并优化了楼梯行进效率；在Unitree G1实机上实现了93.3%的平均跨越成功率。

Conclusion: 基于世界模型的预测性特征能有效提升足式机器人在复杂受限地形中的 anticipatory（预判性）能力，无需显式的立足点标签即可实现高效、鲁棒的运动控制。
...
### [260] [ZETA: A Controlled Study of Zero-Shot Cross-Embodiment VLA Transfer for Tabletop Manipulation](https://arxiv.org/abs/2609.02546)
...
TL;DR: 该研究系统性地评估了视觉-语言-动作（VLA）模型在零样本跨具身泛化中的表现，并确定了影响迁移效率的关键因素。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在不同机器人具身间的零样本泛化缺乏统一的定义和受控评估基准，且研究界对哪些因素真正影响具身迁移尚不明确。

Method: 区分了严格零样本与预训练暴露零样本，构建了跨越14种受控具身的基准测试，并分析了状态动作表示、具身多样性、辅助目标及数据暴露比例的影响。

Result: 实验发现局部末端执行器（EEF）表示、源具身多样性和辅助共同训练分别可提升迁移性能约15%、18%和7%；同时发现少量（5%）的目标具身数据能显著提升性能。

Conclusion: 严格零样本与预训练暴露零样本存在本质差异，应分开报告；通过优化表征和增加训练多样性可显著提升VLA模型的通用具身操作能力。
...
### [261] [Advancing Accessible Underwater Robotics: The Mini-Girona I-AUV at RAMI 2025](https://arxiv.org/abs/2609.02605)
...
TL;DR: Mini-Girona 是一款高性价比的自主水下机器人（I-AUV），旨在填补科研级 AUV 与基础 ROV 之间的空白。


<details>
  <summary>Details</summary>
Motivation: 现有科研级 AUV 成本高昂，而基础 ROV 功能有限，迫切需要一款既经济又具备自主干预能力的教学与研究平台。

Method: 开发了一款造价约5万美元的 I-AUV，集成了5自由度机械臂、立体视觉系统及基于 AI 的自主导航与干预算法。

Result: 在 RAMI 2025 学生竞赛中获得总分第二名，展现了卓越的视觉感知和自主干预任务能力。

Conclusion: Mini-Girona 验证了低成本 I-AUV 在水下机器人教育与科研领域的应用潜力，为该领域提供了创新性平台。
...
### [262] [Latent Cluster Analysis for Vision-Language-Action Models](https://arxiv.org/abs/2609.02634)
*Theodor Wulff,Sergio Lanza,Tamara Bila,Angelo Cangelosi,Stefan Wermter,Igor Farkas*

Main category: cs.RO

TL;DR: 提出了LAVLA框架，通过潜空间聚类分析和交叉注意力加权方法，揭示了VLA模型（以GR00T N1.5为例）内部的表征机制。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作（VLA）模型在机器人领域应用广泛，但其内部决策逻辑和潜空间表征缺乏可解释性。

Method: 引入交叉注意力加权方法以增强关键特征，并采用分层聚类分析对动作解码器进行研究，同时将聚类结果映射为人类可理解的语义概念。

Result: 实验证明加权聚类效果优于基线；分析发现模型在中间层逐步解耦时空与运动特征，并在输出层实现表征稳定。

Conclusion: LAVLA有效提升了语言驱动机器人系统的透明度，为理解复杂VLA模型的内部决策过程提供了重要工具。
...
### [263] [HINT: Human-Intent Inception for Long-Horizon Robot Manipulation](https://arxiv.org/abs/2609.02653)
...
TL;DR: HINT 是一种旨在解决视觉-语言动作（VLA）模型在处理稀疏指令与复杂连续视觉输入时容易产生偏差的智能代理框架。


<details>
  <summary>Details</summary>
Motivation: 现有的 VLA 模型容易受到视觉干扰，往往倾向于通过视觉捷径而非人类语义目标进行动作决策，导致在长程任务中难以维持一致的意图。

Method: 引入 HINT 框架，在 manipulation 模式转换时执行语义推理以锁定目标，并通过多视图定位和视觉追踪技术在转换间保持意图；利用图像空间语义高亮和注意力先验注入接口，无需微调基座模型即可引导动作策略。

Result: 在三个长程任务及分布外变体测试中，HINT 在意图理解、任务进度和端到端成功率方面均有显著提升，同时保持了低延迟控制。

Conclusion: 通过将语义推理与连续视觉控制解耦并进行阶段性注入，HINT 有效增强了机器人执行长程任务时的目标一致性与鲁棒性。
...
### [264] [From Proxy Learning to Driving Decisions: A Transfer-Based Framework for Evaluating Future-Aware Autonomous Driving Planners](https://arxiv.org/abs/2609.02688)
*Yikai Wu*

Main category: cs.RO

TL;DR: 该论文提出了Proxy-to-Decision Transfer (PDT) 框架，旨在系统性评估自动驾驶中未来预测信息对实际决策性能的真实提升效果。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中代理目标（proxy objectives）的优化往往被误认为是最终决策性能的提升，缺乏对轨迹选择和决策逻辑的严格验证。

Method: 引入PDT框架，包括“决策迁移分解模块”（用于定位价值损失）和“可靠性约束验证模块”（用于验证性能提升的统计意义和鲁棒性）。

Result: 实验显示，尽管代理任务的性能指标（如BCE）有所下降，但实际驾驶轨迹选择的性能并未获得稳定提升，且大多数配置在严格的鲁棒性测试下均无法通过。

Conclusion: 目前的未来预测模型在转化为最终决策时存在失败或不确定性，研究呼吁对自动驾驶系统的评估应从简单的代理指标转向基于决策逻辑的严谨验证。
...
### [265] [Do Better Imagined Rollouts Mean Better Robot Control? A Controlled Study of World-Model Evaluation Under Feedback](https://arxiv.org/abs/2609.02811)
*Dharini Raghavan,Amritpal Singh*

Main category: cs.RO

TL;DR: 该研究指出传统的开环预测指标不足以评估机器人闭环任务的性能，并提出评估指标应考虑测量更新频率和预测时域以更准确地反映闭环效果。


<details>
  <summary>Details</summary>
Motivation: 机器人领域的预测模型通常基于开环误差进行评估，但这与机器人执行任务时的闭环反馈逻辑存在偏差，导致评估结果无法有效预测实际控制性能。

Method: 在差速驱动路径跟踪任务中，对比了轨迹重放、无测量滚动预测和闭环跟踪三种评估方式，分析了六种状态估计器在不同测量更新间隔和预测时域下的表现。

Result: 研究发现轨迹重放指标与闭环性能的相关性（Spearman rho = 0.923）远高于传统无测量滚动误差，且长时域预测若缺乏测量修正，其评估指标会严重偏离闭环实际表现。

Conclusion: 机器人预测模型的评估应明确预测时域和测量更新策略，对于闭环系统，若评估指标能模拟实际的修正模式，其预测准确性最高。
...
### [266] [Toward Robust LiDAR Semantic Segmentation for Real-World Deployment: Evaluation under Coarse Labels, Adverse Conditions, and Domain Shifts](https://arxiv.org/abs/2609.02830)
*Samir Abou Haidar,Alexandre Chariot,Mehdi Darouich,Cyril Joly,Jean-Emmanuel Deschaud*

Main category: cs.RO

TL;DR: 本文提出了一个评估激光雷达语义分割模型部署就绪性的统一协议，涵盖安全相关标签、传感器退化鲁棒性和跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试集中于理想条件和细粒度分类，忽视了自动驾驶在真实世界中面临的安全关键需求、环境退化及领域差异，导致学术指标与实际部署需求脱节。

Method: 构建了包含三个维度的评估协议：(i) 基于自动驾驶安全优先级的粗粒度标签评估；(ii) 模拟八种真实环境退化（如大气、几何和传感器噪声）的鲁棒性测试；(iii) 无需适应的跨数据集领域泛化测试；并纳入了嵌入式平台（Jetson AGX Orin）的实时推理速度评估。

Result: 实验发现细粒度基准排名无法反映安全相关性能；所有模型在面对噪声干扰时性能均显著下降；当前的领域泛化技术尚不足以支撑可靠的实际部署。

Conclusion: 目前的学术基准与实际部署之间存在显著差距，该协议为激光雷达语义分割提供了更具实践指导意义的评估参考体系。
...
### [267] [Towards Trustworthy Autonomous Robots: An Explainable AI-Based Decision Framework](https://arxiv.org/abs/2609.02861)
*Cagri Temel*

Main category: cs.RO

TL;DR: 本文提出了TRACE框架，旨在为深度学习驱动的自主机器人提供决策可追溯性和透明度，以满足安全关键系统的审计要求。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型存在“黑盒”问题，导致自主机器人在发生事故时无法回溯决策逻辑，难以满足欧盟AI法案等监管透明度要求。

Method: 构建了包含语义感知、信念推理、动作合成和执行验证的四层可审计框架，通过因果链将决策与传感器证据直接关联，并引入反事实文档记录。

Result: 在仓库机器人导航任务中，TRACE实现了98.6%的证据可追溯性、99.0%的时间连续性和98.1%的决策可重构性，优于LIME等传统归因方法。

Conclusion: TRACE成功解决了自主系统决策层面的审计难题，为高风险自主系统提供了模型无关且具备高度可解释性的架构方案。
...
### [268] [CrashDiffuser: VLM-Guided Collision Intent Reasoning for Fine-Grained Safety-Critical Traffic Scenario Generation](https://arxiv.org/abs/2609.02270)
*Shucheng Zhang,Yuang Zhang,Bingzhang Wang,Muhammad Monjurul Karim,Kehua Chen,Yinhai Wang*

Main category: cs.RO

TL;DR: 提出了一种名为CrashDiffuser的VLM引导扩散框架，旨在实现自动驾驶中精细化的安全关键场景生成，不仅能诱发碰撞，还能精确控制碰撞位置。


<details>
  <summary>Details</summary>
Motivation: 现有碰撞生成方法大多仅关注产生碰撞，缺乏对碰撞发生部位（车头、车侧、车尾）的细粒度控制，限制了自动驾驶系统的全面安全性评估。

Method: 引入VLM（视觉语言模型）作为闭环引导，通过层级碰撞意图接口将语义推理与轨迹合成解耦。利用扩散模型根据预测的意图生成对抗性轨迹，并结合碰撞引导采样、候选筛选和短时重规划来适应目标的动态行为。

Result: 在WOMD数据集的闭环评估中，CrashDiffuser单次尝试的碰撞成功率达50.33%，三次内达67.98%，且碰撞部位控制成功率达到40.05%，同时保持了轨迹的自然度。

Conclusion: CrashDiffuser通过结合语义推理与扩散模型，有效实现了对自动驾驶碰撞场景的可控生成，为安全验证提供了一种高效且灵活的新手段。
...
### [269] [A Control Co-Design Framework to Optimize Sustainability with Application to Microgrid-Driven Data Centers](https://arxiv.org/abs/2609.01912)
*Tania Rifat Jahan,Donald J. Docimo*

Main category: eess.SY

TL;DR: 提出了一种面向能源系统（以数据中心和微电网为例）的通用可持续性控制协同设计（CCD）框架。


<details>
  <summary>Details</summary>
Motivation: 现有数据中心和微电网设计通常缺乏对工厂参数与控制器参数的协同优化，且对环境可持续性的关注不足。

Method: 建立了一个涵盖制造、运行和处置三个生命周期阶段的通用CCD框架，并将可持续性指标纳入多目标优化过程。

Result: 研究发现组件制造产生的温室气体排放可能超过运行期间的排放，且该框架识别的设计方案在环境可持续性方面显著优于基准设计。

Conclusion: 通过将生命周期可持续性评估整合进协同设计过程，可以有效降低能源系统的整体环境足迹，为设计更可持续的基础设施提供指导。
...
### [270] [Dimension-Reduced ADP for Real-Time Microgrid Operation with Massive Air-Conditioning Loads under Multiple Uncertainties](https://arxiv.org/abs/2609.01972)
*Jingguan Liu,Xiaomeng Ai,Shichang Cui,Jiakun Fang,Jinyu Wen*

Main category: eess.SY

TL;DR: 提出了一种针对含大规模空调负载微电网的降维近似动态规划（ADP）实时运行方法。


<details>
  <summary>Details</summary>
Motivation: 解决大规模空调负载在多重不确定性下导致的实时调度“维度灾难”问题。

Method: 将问题建模为多阶段马尔可夫决策过程，采用基于一致性的价值函数投影进行状态空间降维，并结合分段线性近似进行训练。

Result: 在33节点和123节点系统中验证了该方法在确定性和随机条件下均具备接近最优的运行性能，且计算效率高、扩展性强。

Conclusion: 所提降维ADP方法有效克服了计算复杂性，能够高效处理大规模微电网调度问题。
...
### [271] [Efficient Sensor Fusion Through Covariance-Constrained Observation Decimation (CCOD)](https://arxiv.org/abs/2609.02010)
*Andres Enriquez Fernandez,Erik Blasch,Angel Flores-Abad,John J. Bird*

Main category: eess.SY

TL;DR: 提出了一种协方差约束观测抽样（CCOD）框架，通过重构DARE输入，在不增加系统维度的情况下，精确预测观测抽样后的卡尔曼滤波稳态估计误差协方差。


<details>
  <summary>Details</summary>
Motivation: 观测抽样可降低计算与通信开销，但会导致估计不确定性增加。现有方法（如提升系统或周期性Riccati方程）计算复杂度高，无法高效预测抽样后的性能。

Method: 通过重构离散代数Riccati方程（DARE）的输入，引入等效的抽样系统和过程噪声矩阵，以捕捉观测更新间隔期间的协方差增长。

Result: 仅需单次DARE评估即可精确预测稳态估计误差协方差，能够有效确定满足特定不确定性约束的最大观测抽样因子。

Conclusion: 该框架在保证估计性能的同时有效减少了测量更新频率，在复杂高维系统和空间目标跟踪场景中验证了其计算高效性和准确性。
...
### [272] [Seed-Anchored Budget-Bounded Graph Rendering for Question Answering on Industry-Standard Power-Grid Information and Exchange Models](https://arxiv.org/abs/2609.02011)
*Jayakumar Manoharan,Yamini Sehgal*

Main category: eess.SY

TL;DR: 提出了一种名为“种子锚定图渲染（seed-anchored graph rendering）”的确定性方法，用于在严格的上下文限制下，有效提取和呈现电力网格模型（CIM/CGMES）中的关键信息。


<details>
  <summary>Details</summary>
Motivation: 在大模型对电力网格模型进行问答时，现有的渲染方法往往受限于上下文预算，导致难以保留多跳查询所需的核心证据，且缺乏可验证的约束条件。

Method: 引入无需额外调参的确定性算法，优先处理查询相关的局部图证据，确保满足特定条件的答案载体单元能够完整保留在有限的上下文前缀中。

Result: 在固定8,000字符的预算下，该方法将多跳证据保留率提升至100%，并将问答准确率从0.450显著提升至0.970，表现优于LightRAG、GraphRAG等现有框架。

Conclusion: 该方法通过确定性的图渲染策略，在电力系统标准化模型应用中，能在严苛的预算限制下实现比主流检索增强生成方法更高效、更精准的信息呈现。
...
### [273] [Quantum-Based k-Coverage Optimization for UAV-Aided Search and Rescue Missions](https://arxiv.org/abs/2609.01930)
*Halim Lee,Suhui Jeong,Na Young Kim,Jiwon Seo*

Main category: eess.SY

TL;DR: 本文提出了一种利用无人机（UAV）进行灾后搜救的路径点优化方法，通过量子近似优化算法（QAOA）解决射频信号覆盖问题。


<details>
  <summary>Details</summary>
Motivation: 在大型灾难中，需要通过最小化无人机飞行路径点来实现对目标区域的高效覆盖，传统算法在复杂约束下计算效率不足。

Method: 将路径点选择问题建模为扩展的k覆盖问题，转化为无约束二进制二次规划（QUBO）模型，并在IBM量子计算机上使用QAOA算法进行求解。

Result: 在仿真中获得了已知最小基数解；在真实量子硬件上实现了超过95%的覆盖率，且路径长度较确定性网格基准缩短了约37%。

Conclusion: 证明了将搜救路径规划问题转化为QUBO并在量子硬件上求解的可行性，展现了量子计算在解决实际大规模组合优化问题中的潜力。
...
### [274] [Turnpike properties in nonlinear system identification](https://arxiv.org/abs/2609.02071)
*Julian D. Schiller,Matthias A. Müller*

Main category: eess.SY

TL;DR: 该研究分析了离散时间非线性状态空间模型在仿真误差最小化（SEM）下的“ Turnpike”属性，探讨了最优输出序列如何趋向于无约束最优解。


<details>
  <summary>Details</summary>
Motivation: 为了提供理论依据，证明使用计算上更简便的定初值SEM公式，其优化结果仍能保持接近全局最优解。

Method: 利用系统识别理论，结合累积转弯性质（cumulative turnpike property）、价值函数强制性（coercivity）及严格耗散性（strict dissipativity）进行等价性分析，并引入基数转弯性质进行弱化对比。

Result: 在温和的可达性条件下，证明了累积转弯性质、价值函数强制性和特定严格耗散性之间的等价关系；提出了基于增量输出稳定性与阶段成本凸性的充分条件。

Conclusion: 该研究为非线性系统识别提供了性能保障，证明了在特定稳定性条件下，利用长序列优化可以简化为更具计算可行性的定初值问题。
...
### [275] [Existential Opacity for Discrete-Event Systems with State Observations](https://arxiv.org/abs/2609.02132)
*Zhiyuan Huang,Zhao Tong,Jiakai Li,Bingzhuo Zhong*

Main category: eess.SY

TL;DR: 该研究提出了一种名为“存在性不透明度（Existential Opacity, EO）”的新概念，用于离散事件系统（DES）中的状态观察隐私保护。


<details>
  <summary>Details</summary>
Motivation: 现有的状态观察不透明度概念在处理路径规划或隐私保护任务时存在局限性，无法充分涵盖只需部分秘密行为即可满足隐私要求的场景。

Method: 引入存在性不透明度（EO）框架，仅要求存在部分而非全部秘密行为在观察下与非秘密行为不可区分；并构建了相应的验证方法。

Result: 研究表明EO比现有的状态观察不透明度更具表达力，并为判定隐私保护任务的可行性提供了新的准则。

Conclusion: EO为离散事件系统提供了一种更灵活的隐私分析工具，能够有效解决复杂应用场景下的不透明度评估问题。
...
### [276] [Prosumer Synchronisation Risk: Impacts of Time-Varying Tariffs on Distribution Network Expansion](https://arxiv.org/abs/2609.02174)
*Dillon Zadoks,Alfredo Oneto,Carlo Tajoli,Yi Guo,Philipp Heer,Giovanni Sansavini,Gabriela Hug*

Main category: eess.SY

TL;DR: 该论文研究了在低压电网中采用分时电价（ToU）和实时电价（RTP）对配电网扩容需求的影响，揭示了过高比例的采纳率反而可能引发用电同步，从而加剧电网负担。


<details>
  <summary>Details</summary>
Motivation: 分布式能源的快速部署促使运营商通过动态定价来引导用户行为，但这种定价机制可能导致用户行为同步，反而创造出新的用电高峰，增加电网扩容成本。

Method: 利用混合整数线性模型，对瑞士471个低压参考网络进行模拟，评估2050年分布式能源部署场景下，不同定价策略（平价、分时、实时）对电网扩容需求的影响。

Result: 研究发现，适当的动态定价采纳率可降低超过50%的扩容成本，但一旦采纳率超过最优阈值（分时电价50%或实时电价40%），扩容需求反而会激增，最高可比最优水平增加134%。

Conclusion: 动态定价虽然是优化电网运行的工具，但必须严格控制采纳比例，否则协同效应会导致严重的网络过载，运营商需要平衡价格激励与电网负载均衡之间的复杂关系。
...
### [277] [Continuous-Time Aggregation of Massive Flexible HVAC Loads Considering Uncertainty for Reserve Provision in Power System Dispatch](https://arxiv.org/abs/2609.02408)
*Jingguan Liu,Xiaomeng Ai,Jiakun Fang,Shichang Cui,Shengshi Wang,Wei Yao,Jinyu Wen*

Main category: eess.SY

TL;DR: 提出了一种基于连续时间（CT）建模、仿射变换聚合和分布鲁棒机会约束的HVAC大规模需求侧资源调度框架。


<details>
  <summary>Details</summary>
Motivation: 解决大规模HVAC系统在应对可再生能源波动时存在的计算复杂度高及室外温度不确定性带来的调度难题。

Method: 引入连续时间聚合模型，采用仿射变换处理异构性，结合分布鲁棒机会约束应对温度不确定性，并通过Bernstein多项式样条、多面体投影及线性化技术实现模型的可处理化。

Result: 通过分层调度框架实现了大规模HVAC的高效调度，在聚合精度、灵活性利用及不确定性处理方面表现出显著的有效性和可扩展性。

Conclusion: 该方法为利用HVAC系统提供电力系统备用容量提供了高效、可靠的理论支持，显著提升了需求侧灵活性。
...
### [278] [Leveraging Time-Causal State Variable Aggregation for Real-Time Schedule of Massive Air Conditioners](https://arxiv.org/abs/2609.02410)
*Jingguan Liu,Xiaomeng Ai,Shichang Cui,Xizhen Xue,Shengshi Wang,Jiakun Fang,Jinyu Wen,Yang Shi*

Main category: eess.SY

TL;DR: 提出了一种基于时间因果聚合的近似动态规划（TCA-ADP）算法，用于大规模空调负荷的实时调度与优化。


<details>
  <summary>Details</summary>
Motivation: 空调负荷具有巨大的时间耦合约束和实时时间因果不确定性，传统调度方法在处理大规模负荷时面临计算效率与精度平衡的挑战。

Method: 分析状态变量的时间因果性，建立增强型聚合模型，将聚合过程重构为线性规划，并利用近似动态规划（ADP）通过离线训练实现近优的实时闭式调度。

Result: 实验证明该方法在大规模空调调度中实现了良好的计算效率与经济性平衡，且具有处理时间因果不确定性的高精度表现。

Conclusion: TCA-ADP算法为解决大规模负荷实时调度中的维数灾难和不确定性问题提供了一种高效、可扩展且具备实时决策能力的解决方案。
...
### [279] [Multivariable Geometric Laplace Transform and Fault Detection in Distributed-Converter Lines](https://arxiv.org/abs/2609.02431)
*Francisco Manuel Arrabal-Campos,Francisco G. Montoya,Santiago Sánchez-Acevedo,Raymundo E. Torres-Olguin,Alfredo Alcayde*

Main category: eess.SY

TL;DR: 本文提出了一种基于二维几何拉普拉斯变换（双复数代数）的直流线路故障诊断与控制方法，用于处理分布式变换器网络中的时空耦合问题。


<details>
  <summary>Details</summary>
Motivation: 分布式变换器系统中的故障定位本质上是一个时空问题，现有的测量手段难以在复杂的动态环境中准确提取故障的时空特征。

Method: 利用Cl(4,0)几何代数中的双复数子代数，定义了二维几何拉普拉斯变换，将时空域分解为代数上可区分的两个平面，并以此构建了传输线模型及故障诊断链。

Result: 该方法实现了故障的精确检测、定位与分类，能够处理多重并发故障，并有效区分电厂停运与电缆缺陷，同时考虑了离散采样带来的混叠效应。

Conclusion: 通过几何嵌入的拉普拉斯变换，为分布式变换器网络的系统建模和故障诊断提供了一种严谨的数学框架，且具备高效的计算实现。
...
### [280] [Effective Range and Optimal Frequency of Through-the-Earth Magnetic Induction Communication](https://arxiv.org/abs/2609.02485)
*Honglei Ma,Erwu Liu,Wei Ni,Jun Zhu,Zhijun Fang,Rui Wang,Xinyu Qu*

Main category: eess.SY

TL;DR: 该研究分析了地下材料的涡流损耗对磁感应通信（MIC）范围的影响，并提出了优化通信距离的方法。


<details>
  <summary>Details</summary>
Motivation: 以往研究忽视了地下介质中电导率和介电常数引起的涡流损耗，导致无法准确分析穿地通信（TTE）的有效范围。

Method: 考虑地下介质的导电性和介电性，推导出MIC的有效通信距离公式，并给出了预测最优载波频率的闭式解，随后通过有限元仿真进行了验证。

Result: 研究发现存在最优载波频率可显著提升通信范围；同时，优化天线半径也能有效扩展TTE及车辆间MIC应用的通信距离。

Conclusion: 通过综合考虑材料属性并优化频率和天线参数，可以显著改善穿地磁感应通信的性能。
...
### [281] [Physics-based Online Adaptive Koopman Model Predictive Attitude Control for Combined Spacecraft with Dynamic Uncertainties](https://arxiv.org/abs/2609.02534)
*Yicheng Sun,Yueyong Lyu,Yuhan Liu,Yanning Guo,Wei Pan*

Main category: eess.SY

TL;DR: 提出了一种基于物理驱动的自适应Koopman模型预测控制（MPC）策略，用于处理航天器在惯性不确定性和主动机动目标下的姿态稳定控制。


<details>
  <summary>Details</summary>
Motivation: 传统非线性自适应MPC计算负担重，且常规黑盒Koopman建模缺乏物理可解释性，难以适应复杂的航天器姿态控制需求。

Method: 利用四元数运动学构建基于物理的Koopman提升函数，采用基于梯度下降的在线更新律识别惯性参数，并将自适应线性模型集成到MPC中，转化为二次规划（QP）问题。

Result: 显著降低了在线计算开销，并保证了递归可行性和区域输入到状态稳定性（ISS），在仿真中表现优于传统方法。

Conclusion: 该方法通过物理驱动的线性化有效平衡了非线性系统建模的精确性与计算效率，为航天器实时控制提供了一种高效、稳定的解决方案。
...
### [282] [Zonotope-Based Active Exposure of Stealthy Deception Attacks in Sensor-Fusion Systems](https://arxiv.org/abs/2609.02587)
*Meiqi Tian,Shuo Li,Bingzhuo Zhong*

Main category: eess.SY

TL;DR: 针对存在有界噪声的传感器融合信息物理系统，提出了一种通过控制信道注入辅助扰动来检测隐蔽攻击的方法。


<details>
  <summary>Details</summary>
Motivation: 传感器融合系统容易遭受多传感器同时被篡改的隐蔽攻击，且传统检测方法在面对有界噪声时难以有效区分正常状态与攻击状态。

Method: 构建可接受输出集与攻击输出集，采用滚动时域优化框架设计暴露输入（辅助控制扰动），通过增大集合间距来强化检测，并提供离线预算指导。

Result: 推导出保证可检测性的充分条件，并通过无人机（UAV）在GNSS和LiDAR攻击下的仿真实验验证了该方法能有效识别隐蔽攻击。

Conclusion: 所提出的控制信道扰动策略能显著放大系统在受攻击时的行为差异，有效提升了复杂信息物理系统对抗隐蔽传感攻击的鲁棒性。
...
### [283] [Economic Model Predictive Control with Policy-Guided Terminal Ingredients](https://arxiv.org/abs/2609.02628)
*Salim Msaad,Robert D. McAllister*

Main category: eess.SY

TL;DR: 提出了一种基于已知次优控制策略的策略引导式模型预测控制（Policy-guided MPC）框架，无需依赖稳态或参考轨迹即可保证闭环稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统MPC严重依赖稳态假设进行终端成本和约束设计，这限制了其在无固定工作点或非稳态应用场景中的使用。

Method: 利用已知的次优策略进行前向模拟（rollout）以确定终端区域中心，并通过惩罚偏离该中心的程度来构建终端成本与约束。

Result: 在有限和无限时域问题中证明了该框架相对于引导策略的闭环性能保证，并通过能源管理算例验证了其有效性。

Conclusion: 该方法成功消除了对稳态的依赖，显著扩展了MPC在复杂或动态系统中的适用范围。
...
### [284] [Guidelines for the implementation of power oscillation damping controllers in power converters](https://arxiv.org/abs/2609.02810)
*Javier Renedo,Macarena Martín Almenta,Sergio Martínez Villanueva,Agustín Díaz-García,Antonio Cordón,Davide Gotti*

Main category: eess.SY

TL;DR: 本文提出了电力转换器中功率振荡阻尼（POD）控制器的实施指南及合规性评估标准。


<details>
  <summary>Details</summary>
Motivation: 尽管POD控制器在阻尼电力系统机电振荡方面潜力巨大，但其实施难度大且效果高度依赖参数设置，缺乏实际工程应用指导。

Method: 结合合成测试系统与西班牙技术标准（NTS）的合规性监测方法，分析了具有电网跟随（GFL）控制的变流器，并评估了有功功率（POD-P）、无功功率（POD-Q）及综合（POD-PQ）控制方案。

Result: 通过数值示例和大规模电力系统验证，证明了在适当的合规准则下，基于变流器的POD控制器能有效抑制机电振荡。

Conclusion: 通过标准化的方法论和合规准则，POD控制器在电力电子化电力系统中是提升电网稳定性的有效解决方案。
...
### [285] [Tests on the POD-P controller of INELFE Spain-France VSC-HVDC interconnector](https://arxiv.org/abs/2609.02838)
...
TL;DR: 本文研究了通过优化INELFE VSC-HVDC链路的POD-P控制器参数及协同控制策略，以增强欧洲大陆电力系统跨区域振荡的阻尼效果。


<details>
  <summary>Details</summary>
Motivation: 旨在提升电力系统应对机电振荡的能力，通过调整POD-P控制器增益及与角度差控制(ADC)的协同，增强系统的稳定性。

Method: 利用电力系统仿真研究与多运行模式下的现场测试相结合的方法，验证改进后控制器的性能与系统行为。

Result: 通过仿真和现场测试，成功证明了调整后的POD-P控制器在不同运行模式下能有效增强跨区域功率振荡的阻尼贡献。

Conclusion: 优化后的POD-P控制器与ADC协同工作能显著提升INELFE互联链路对欧洲大陆电网频率稳定性的支撑能力。
...
### [286] [Data-Efficient Networks for Multi-Contrast MRI Reconstruction based on a Generalized Content/Style Prior](https://arxiv.org/abs/2609.01959)
...
TL;DR: 提出了一种名为CoSMo-RecNet的模块化框架，用于在低数据环境下实现多对比度MRI引导重建。


<details>
  <summary>Details</summary>
Motivation: 现有的引导重建模型依赖大规模配对多对比度原始数据，在低数据场景下应用受限。

Method: 利用无需k空间数据的公共无配对数据集学习通用的内容/风格表示模型作为先验，再通过轻量级展开网络在小规模任务数据集上进行精细化重建。

Result: 在0.3 T M4Raw数据集上，仅需5个训练样本即可超越100样本训练的MoDL模型；在极端低场47 mT数据集上表现优于传统重建及迁移学习方法。

Conclusion: CoSMo-RecNet通过解耦先验学习与重建任务，有效缓解了MRI引导重建对大规模配对数据的依赖，适用于数据受限的临床应用。
...
### [287] [Perceptually Regularized Diffusion Model for Image Super-Resolution](https://arxiv.org/abs/2609.02016)
*Chuxiangbo Wang,Pavithra Venkatachalapathy,Ying Liang,Min Wang,Jing Qin,Yifei Lou,Weihong Guo*

Main category: eess.IV

TL;DR: 提出了一种基于感知正则化的扩散模型框架，旨在解决图像超分辨率任务中标准像素噪声预测损失导致的细节丢失和过平滑问题。


<details>
  <summary>Details</summary>
Motivation: 传统模型方法计算复杂，而现有的扩散模型虽感知质量高，但缺乏明确的感知保真度约束，导致图像细节恢复不足。

Method: 引入感知损失作为正则化项，将先验知识整合进扩散模型的训练框架中，以约束生成过程。

Result: 实验表明，该方法在提升感知质量的同时保持了具有竞争力的失真指标，且能有效恢复图像的精细结构。

Conclusion: 通过感知正则化增强扩散模型，能够显著改善超分辨率任务中的图像特征恢复能力和训练收敛性。
...
### [288] [LoFi RADIO: A Distilled In-Domain Backbone Applied for Artifact-Severity Grading of Ultra-Low-Field Neonatal Brain MR](https://arxiv.org/abs/2609.02676)
*Jonathan B. Martin,Yashwant Kurmi,Charlotte R. Sappo*

Main category: eess.IV

TL;DR: 该研究针对超低场（ULF）新生儿脑部MRI的自动化质量控制，提出了名为“LoFi RADIO”的蒸馏模型，用于对七种常见图像伪影进行多标签严重程度评估。


<details>
  <summary>Details</summary>
Motivation: ULF MRI在低资源地区应用前景广阔，但易受低信噪比和环境干扰影响产生伪影，亟需自动化的质量评估方案。

Method: 通过引入Per-artifact gate路由多个基础模型教师，并将知识蒸馏至单一的ViT-S学生模型（LoFi RADIO），同时利用未标注的ULF MRI语料进行训练。

Result: 所提出的蒸馏模型在保持较高性能的同时，避免了推理时运行多个大型基础模型的资源消耗，性能匹配甚至超过了门控教师模型组合。

Conclusion: 知识蒸馏策略不仅能提升模型对ULF MRI伪影的分类精度，还能显著降低计算复杂性，为实际部署提供高效且精准的解决方案。
...
### [289] [Seeing Beyond the Lesion: Disease Recognition from Reactive CNS Tissue](https://arxiv.org/abs/2609.02390)
*Jan Schnorrenberg,Jan Ernsting,Enrico Küllenberg,Tim Hahn,Benjamin Risse,Christian Thomas*

Main category: eess.IV

TL;DR: 该研究评估了四种病理学基础模型在识别颅内活检“非病灶”组织中潜在诊断信号的能力。


<details>
  <summary>Details</summary>
Motivation: 颅内活检常因采样误差导致组织表现为无病变的反应性实质，造成诊断困难，研究旨在验证模型是否能从这些传统意义上的“非诊断性”组织中提取疾病特征。

Method: 利用4种病理基础模型（UNI2-h, Virchow2, Prov-GigaPath, H-optimus-0）作为冻结的Patch编码器，构建基于注意力机制的多示例学习框架，并引入“仅凭载玻片大小”作为基线对照以排除采样偏差干扰。

Result: 排除样本大小这一混淆因素后，模型仍能在细分诊断任务中表现出优于随机的预测能力，且四种基础模型在性能上无统计学差异。

Conclusion: 目前的病理基础模型已能从传统上被视为“无诊断意义”的反应性组织中识别微弱的病理信号，强调了在计算病理学基准测试中进行偏差审计的重要性。
