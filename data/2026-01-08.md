...
TL;DR: 提出QZero模型，一种无需搜索的免模型强化学习算法，通过自博弈和离策略经验回放学习纳什均衡策略，在围棋上达到AlphaGo水平。


<details>
  <summary>Details</summary>
Motivation: 围棋作为AI基准测试需要复杂策略推理和长期规划，现有方法（如AlphaGo）依赖基于模型的蒙特卡洛树搜索，本研究探索免模型强化学习在围棋中的潜力。

Method: 基于熵正则化Q学习的免模型强化学习算法，使用单一Q值网络统一策略评估和改进，通过自博弈和离策略经验回放训练，无需人类数据或搜索过程。

Result: 使用7块GPU训练5个月，QZero在围棋上达到与AlphaGo相当的性能水平，首次证明免模型强化学习可掌握围棋。

Conclusion: 研究证明了免模型强化学习在掌握围棋等复杂环境中的有效性，以及离策略强化学习在大规模复杂问题中的可行性。
...
TL;DR: 本文提出Digital Red Queen算法，利用大语言模型在Core War环境中演化对抗性程序，通过持续适应动态目标实现开放式的进化过程。


<details>
  <summary>Details</summary>
Motivation: 现有LLM进化框架多为静态优化问题，忽略了真实进化过程中开放式的对抗动态（即'红皇后'动态），需要探索动态目标下的进化方法。

Method: 采用自博弈算法DRQ，在Core War虚拟环境中让LLM演化汇编式程序（战士），每轮生成新战士击败所有前代战士，形成持续适应序列。

Result: 战士在多轮演化后表现出更强的泛化能力（相对于人类编写的测试战士），且独立运行间的行为多样性降低，表明向通用行为策略的收敛趋势。

Conclusion: 从静态目标转向动态红皇后目标具有潜在价值；Core War可作为研究对抗适应的可控沙箱；DRQ的简洁有效性可拓展至网络安全、抗药性等实际对抗领域。
...
TL;DR: 提出一种多智能体工作流，将主任务描述与约束条件解耦优化，通过量化评分反馈迭代改进提示词，显著提升大语言模型输出对形式约束的遵从性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型常生成内容相关但形式不符合约束的输出，传统提示优化方法忽视约束条件的细粒度描述，导致输出程序性错误。

Method: 设计多智能体工作流，将主任务描述优化与约束条件优化分离，使用量化评分作为反馈，迭代重写和改进提示词。

Result: 该方法在Llama 3.1 8B和Mixtral-8x7B等模型上显著提高了输出合规性评分。

Conclusion: 解耦优化任务描述与约束条件的方法能有效提升大语言模型对形式约束的遵从能力，为提示工程提供新思路。
...
TL;DR: 本文提出一种基于内省探索的强化学习框架，通过模拟生物疼痛信号来增强智能体的自我意识，在网格世界中显著提升学习性能并复现类人行为。


<details>
  <summary>Details</summary>
Motivation: 推动人工智能中的心智理论发展，探索自我与他人认知的统一系统，研究自我意识如何通过内省机制影响智能体的学习能力。

Method: 在网格世界环境中，引入受生物疼痛启发的内省探索组件，使用隐马尔可夫模型从在线观察中推断“疼痛信念”，并将其整合到主观奖励函数中；对比正常与慢性疼痛感知模型的性能差异。

Result: 内省智能体整体显著优于标准基线智能体，能够复现复杂的类人行为模式。

Conclusion: 基于疼痛信号的内省机制可有效提升智能体的自我意识与学习能力，为理解心智理论的计算基础提供了新框架。
...
TL;DR: 提出基于成熟度的框架，通过明确测量机制认证具身AI系统，以无人机系统检测为例验证可行性。
...
Method: 采用基于成熟度的认证框架，引入不确定性量化作为示例测量机制，并通过无人机系统检测案例进行验证。

Result: 框架具备可行性，能够通过结构化评估和定量机制处理具身AI系统认证中的多目标权衡问题。

Conclusion: 该成熟度框架为具身AI系统认证提供了可操作的测量方法和评估路径，有助于建立可信的认证体系。
...
TL;DR: 开发了CPGPrompt系统，将临床指南转化为决策树，利用大语言模型进行患者病例评估，在专科转诊决策上表现优异，但在多类路径分类上表现有差异。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则的临床指南集成方法存在可解释性差、指南遵循不一致和领域适用性窄的问题，需要更有效的AI集成方案。

Method: 构建自动提示系统，将叙述性临床指南转化为结构化决策树，利用LLM动态导航进行病例评估，使用合成病例在三个临床领域测试不同决策场景。

Result: 二元专科转诊分类在所有领域表现稳定（F1: 0.85-1.00），召回率高（1.00±0.00）；多类路径分配表现下降，各领域有差异：头痛（F1: 0.47）、腰痛（F1: 0.72）、前列腺癌（F1: 0.77）。

Conclusion: CPGPrompt在二元决策任务上有效，但多类路径分类性能受指南结构影响，否定处理、时间推理和量化实验室测试是影响性能的关键因素。
...
TL;DR: 大型基础模型在医疗AI中应用广泛，但无法替代N-of-1试验实现真正的个性化治疗。本文提出结合两者优势的混合框架：基础模型生成干预假设，N-of-1试验进行个体因果验证。


<details>
  <summary>Details</summary>
Motivation: 大型基础模型在医疗个性化应用中面临多重矛盾（泛化悖论、隐私-性能悖论等），且其预测能力与个性化治疗所需的因果理解之间存在差距，需要探索更可靠的个性化医疗实现路径。

Method: 提出理论分析框架，论证大型基础模型与N-of-1试验的互补性，并设计混合方法：利用基础模型从多模态数据中生成干预候选排序（含不确定性估计），再通过N-of-1试验进行个体化因果验证。

Result: 分析表明：大型基础模型擅长基于群体模式的快速假设生成，但无法解决个性化医疗中的根本矛盾；N-of-1试验能提供个体因果证据并保护隐私，两者结合可协同应对现有悖论。

Conclusion: 个性化医疗需要明确预测与因果的界限，通过整合大型基础模型的模式识别能力和N-of-1试验的因果验证机制，以负责任的方式推进AI在个性化医疗中的应用。
...
TL;DR: 提出程序化技能网络（PSN）框架，通过符号程序表示技能，在开放环境中实现持续技能获取、优化和重用。


<details>
  <summary>Details</summary>
Motivation: 解决开放环境中智能体持续获取、细化和重用可执行技能库的挑战，实现技能的长期演进和组合性。

Method: 基于大语言模型实现三个核心机制：REFLECT（结构化故障定位）、渐进式优化（成熟度感知更新门控）、规范结构重构（回滚验证）。

Result: 在MineDojo和Crafter环境中验证了PSN具备鲁棒的技能重用、快速适应能力和对开放任务分布的强泛化性能。

Conclusion: PSN框架能有效支持持续技能获取，其学习动态与神经网络训练存在结构相似性，代码将开源。
...
TL;DR: 提出STAR-S框架，通过自学习循环增强大语言模型的安全推理能力，有效防御越狱攻击。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过训练模型基于安全规则进行推理来提升安全性，但难以明确设计或直接获取有效的安全推理形式。

Method: STAR-S框架：在自学习循环中集成安全规则推理学习，通过安全规则引导生成推理与反思数据，并利用微调增强安全推理能力，形成协同增强循环。

Result: 实验表明STAR-S能有效防御越狱攻击，性能优于基线方法。

Conclusion: STAR-S通过自学习安全规则推理的循环机制，显著提升大语言模型的安全性，为安全部署提供有效方案。
...
### [10] [Variance Computation for Weighted Model Counting with Knowledge Compilation Approach](https://arxiv.org/abs/2601.03523)
*Kengo Nakamura,Masaaki Nishino,Norihito Yasuda*

Main category: cs.AI

TL;DR: 本文研究了加权模型计数（WMC）方差的计算问题，针对参数不确定性下的概率推理，提出了结构化d-DNNF的多项式时间算法，并证明了其他编译形式的计算困难性。


<details>
  <summary>Details</summary>
Motivation: 实际推理任务中，模型参数常从数据学习而来存在不确定性，需要评估推理结果的方差以量化不确定性，但现有方法在计算方差方面的可处理性未知。

Method: 1. 为结构化d-DNNF设计多项式时间方差计算算法；2. 证明结构化DNNF、d-DNNF和FBDD的方差计算困难性；3. 在贝叶斯网络上进行实证应用。

Result: 1. 结构化d-DNNF的WMC方差可多项式时间计算；2. 其他编译形式（包括普通d-DNNF）的方差计算是困难的；3. 实验成功评估了真实贝叶斯网络的边际概率方差。

Conclusion: WMC方差计算的可处理性依赖知识编译形式，结构化d-DNNF具有计算优势，该研究为不确定性量化提供了理论工具和实用算法。
...
TL;DR: 本文提出一种神经符号框架评估LLM推理过程，发现扩展生成并非深度推理必要条件，并揭示训练数据混合和模型蒸馏的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前思维链评估方法难以区分性能提升是源于真实推理还是单纯生成长文本，需要更精确的评估框架来诊断推理过程。

Method: 提出非侵入式的神经符号框架进行过程中心化评估，通过该框架识别行为原型、分析失败模式，并考察推理模式、训练策略和模型规模的影响。

Result: 发现扩展生成与深度推理无必然关联；混合长短思维链数据训练可能导致过早饱和崩溃；小模型蒸馏能复制行为长度但无法保持逻辑效能。

Conclusion: 推理能力受模型内在容量限制，当前训练和评估方法存在缺陷，需要更精细的过程评估来提升LLM的真实推理能力。
...
TL;DR: 提出了SCRIBE强化学习框架，通过技能原型库进行中间层奖励建模，显著提升工具增强代理的可靠性和多步推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的奖励模型在工具增强代理训练中产生噪声大、不一致的信号，缺乏细粒度任务标准来区分高层规划与底层执行，导致多步推理中的信用分配困难。

Method: 引入SCRIBE框架，在中间抽象层进行干预，基于技能原型库将开放式LLM评估转化为约束验证问题，为每个子目标匹配对应原型并提供结构化评估标准。

Result: 在多个推理和工具使用基准测试中达到最先进性能，将Qwen3-4B模型的AIME25准确率从43.3%提升至63.3%，显著提高复杂多轮工具交互成功率。

Conclusion: SCRIBE通过技能条件化奖励实现抽象层协同进化，与底层工具优化互补，为构建更自主可靠的工具使用代理提供了可扩展路径。
...
TL;DR: 提出SAE-Steering方法，通过稀疏自编码器解耦大推理模型的隐藏状态，识别策略特定特征作为控制向量，有效控制推理策略并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有大推理模型自主选择推理策略常导致低效或错误路径，需要可靠灵活的控制方法，但现有方法因隐藏状态的概念纠缠难以控制细粒度策略。

Method: 使用稀疏自编码器解耦策略纠缠的隐藏状态，提出两阶段特征识别流程SAE-Steering：先通过策略关键词对数概率放大召回特征，再按控制效果排序。

Result: SAE-Steering控制效果优于现有方法15%以上，控制推理策略可将模型从错误路径转向正确路径，绝对准确率提升7%。

Conclusion: 通过特征解耦与识别实现推理策略控制，能显著提升大推理模型的可靠性与灵活性，为可控推理提供新途径。
...
TL;DR: 本文提出PFUA，一种工具增强的蛋白质推理智能体，通过整合领域特定工具而非纯文本推理，显著提升蛋白质功能预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的思维链推理范式直接迁移到蛋白质功能理解时效果不佳，因为蛋白质功能预测是知识密集型科学任务，依赖外部生物先验和计算工具而非纯内部推理。

Method: 提出PFUA框架，统一问题分解、工具调用和基于证据的答案生成，通过整合领域工具产生可验证的中间证据，替代传统无约束的长推理链。

Result: 在四个基准测试中，PFUA平均性能提升103%，一致优于纯文本推理模型。

Conclusion: 蛋白质功能预测需要工具增强的推理方法，PFUA通过整合领域工具有效解决了纯文本推理的局限性，为科学任务中的AI推理提供了新范式。
...
TL;DR: 本文提出了一种基于企业分布式系统标准、形式化方法和行业实践的设计模式，用于构建包含LLM代理、智能体AI和人类参与者的智能体社区系统架构，并通过临床试验匹配案例验证了该框架。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和智能体AI技术的快速发展需要系统化的架构指导，以构建适用于企业级生产环境的复杂系统。

Method: 采用三层设计模式分类（LLM代理、智能体AI、智能体社区），基于分布式系统协调原则建立形式化框架，通过角色、协议和治理结构规范AI代理与人类的协作，并通过临床试验匹配案例进行验证。

Result: 提出了一个兼具实践指导性和形式化验证能力的架构框架，能够通过问责机制表达组织、法律和伦理规则，确保多智能体系统中通信、协商和意图建模的可操作和可验证治理。

Conclusion: 该框架为从业者提供了可操作的指导，同时保持了企业级动态多智能体生态系统部署所需的形式化严谨性。
...
TL;DR: 提出SafeRemind方法，通过在大型推理模型的思维步骤中动态注入安全提醒短语来增强安全性，无需更新模型参数即可显著降低有害行为风险。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过显式思维步骤取得显著成功，但这些步骤可能放大不安全行为，而现有防御机制未能有效应对推理动态特性。研究发现思维步骤中出现的安全提醒短语对确保模型安全具有关键作用。

Method: 提出SafeRemind解码时防御方法，利用熵触发器在决策锁定点进行干预，动态将安全提醒短语注入思维步骤，将潜在有害轨迹转向更安全结果。

Result: 在5个大型推理模型和6个基准测试上的广泛评估表明，SafeRemind显著提升安全性（最高提升45.5个百分点），同时保持核心推理能力。

Conclusion: SafeRemind通过针对性干预推理过程，有效解决了大型推理模型特有的安全漏洞，为解码时安全防御提供了高效解决方案。
...
TL;DR: 提出Sandwich Reasoning方法，通过答案-推理-答案范式实现低延迟查询纠正，在保持推理能力的同时显著降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有Chain-of-Thought推理方法虽然能提高查询纠正准确性，但延迟过高无法满足实时搜索需求；而提前输出答案的方法又无法利用后续推理改进结果。

Method: 1. 提出Sandwich Reasoning框架，采用答案-推理-答案三阶段范式；2. 设计一致性感知强化学习策略，通过一致性奖励对齐初始和最终答案；3. 使用边界拒绝采样优先处理推理影响最大的样本；4. 构建高质量查询纠正数据集。

Result: SandwichR在达到与标准CoT相当的最先进准确率的同时，实现了40-70%的延迟降低，解决了在线搜索中的延迟-准确率权衡问题。

Conclusion: Sandwich Reasoning通过显式对齐初始答案与后验推理，实现了低延迟下的高质量查询纠正，为实时搜索系统提供了有效的解决方案。
...
TL;DR: 本文提出使用LLM自动生成领域特定启发式函数的方法，显著提升了个性化用药规划系统的可扩展性，将可处理药物数量从7种提升至28种以上。


<details>
  <summary>Details</summary>
Motivation: 现有自动化用药规划系统受限于通用启发式函数，最多只能处理7种药物，无法满足临床实际需求，需要提升系统的可扩展性。

Method: 通过编程方式定义领域（指定初始状态和状态转移过程），利用LLM自动生成针对特定问题的启发式函数，结合固定的GBFS搜索算法进行用药规划。

Result: 该方法在覆盖率和规划时间上取得显著改进，能够处理至少28种药物的规划问题，使系统更接近实际临床应用。

Conclusion: 自动生成的领域特定启发式函数能有效扩展用药规划系统的规模，为临床应用提供了可行性。
...
TL;DR: 提出EntroCoT框架，通过熵基分割和蒙特卡洛评估自动筛选高质量思维链数据，提升大语言模型数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有微调数据集中存在'答案正确但推理错误'的问题，即最终答案正确但中间步骤存在幻觉、冗余或逻辑错误，影响模型推理能力提升。

Method: 1. 熵基分割机制：在不确定节点处将推理轨迹分割为多个步骤；2. 蒙特卡洛评估机制：评估每个步骤对最终答案的边际贡献；3. 构建高质量数据集：筛选每个中间步骤都有助于最终答案的推理样本。

Result: 在数学推理基准测试中，使用EntroCoT构建的子集进行微调，性能持续优于全数据集监督的基线方法。

Conclusion: EntroCoT能有效识别和优化低质量思维链监督数据，构建的高质量数据集可显著提升大语言模型的数学推理性能。
...
TL;DR: 提出ROI-Reasoning框架，使大语言模型在严格计算预算下智能分配推理资源，通过元认知微调和强化学习优化任务执行顺序与计算分配。


<details>
  <summary>Details</summary>
Motivation: 大语言模型缺乏对任务所需计算量的内在认知，无法在有限计算资源下智能分配推理预算，导致计算效率低下。

Method: 1. 将问题形式化为有序随机多选择背包问题；2. 元认知微调阶段：训练模型预测推理成本和预期效用；3. 理性感知强化学习阶段：在硬性token预算下优化序列决策。

Result: 在预算约束的数学推理基准测试中，ROI-Reasoning显著提升总体得分，并在严格计算预算下大幅减少遗憾值。

Conclusion: 该框架赋予大语言模型预算感知的理性决策能力，实现了计算资源的战略分配，为资源受限场景下的高效推理提供了有效解决方案。
...
TL;DR: 本文提出了一种使用答案集编程（ASP）计算理性闭包（RC）的声明性方法，能够自动构建最小排序模型并支持查询推理，相比现有命令式实现具有更高的计算效率。


<details>
  <summary>Details</summary>
Motivation: 可废止推理旨在从不完整信息中得出合理结论，KLM框架是其基础理论框架，而理性闭包是该框架中最突出的算法之一。现有实现多为命令式方法，本文旨在通过声明式的ASP方法改进计算效率和实现方式。

Method: 采用答案集编程（ASP）为理性闭包提供声明性定义，通过ASP编码自动从知识库构建最小排序模型，并支持指定查询的推理检查。

Result: 形式化证明了ASP编码的正确性，并通过实验评估与现有命令式实现（InfOCF求解器）进行性能比较，结果表明ASP方法不仅符合RC的理论基础，而且提供了更高的计算效率。

Conclusion: 基于ASP的方法为计算理性闭包提供了一种有效的声明性替代方案，在保持理论正确性的同时提升了计算性能，为可废止推理的实际应用提供了新的技术途径。
...
TL;DR: 使用答案集编程（ASP）对意大利刑法典进行建模，并基于司法判例半自动学习法律规则，开发支持刑事审判阶段法律推理的工具。


<details>
  <summary>Details</summary>
Motivation: 为法律专家在刑事审判阶段提供推理支持和可能的判决结果，同时提高司法决策过程的解释性和透明度。

Method: 1. 使用ASP对意大利刑法典条款（特别是针对人身和财产的犯罪）进行编码建模；2. 基于历史判决案例验证和精炼模型；3. 处理编码过程中的矛盾；4. 利用稳定模型的“支持性”生成新案件的决策和解释；5. 集成归纳逻辑编程系统从案例中归纳法律规则。

Result: 开发了一个能够：1. 对刑法条款进行形式化建模；2. 基于案例验证模型准确性；3. 为新案件生成可能判决和解释；4. 从示例中学习法律规则的工具。

Conclusion: ASP是建模法律知识和支持法律推理的有效方法，所开发的工具不仅能辅助法律决策，还能通过自动解释功能提高司法过程的透明度和可解释性。
...
TL;DR: 提出一种基于答案集编程（ASP）的方法，用于生成决策树模型（如随机森林和梯度提升树）的多种解释类型，包括充分性、对比性、多数性和树特异性解释，以提高模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 决策树模型在机器学习中广泛应用，但其复杂结构导致难以解释，尤其在安全关键应用中需要模型决策的正式论证。现有研究虽能通过自动推理技术生成逻辑和溯因解释，但缺乏灵活性和全面性。

Method: 使用答案集编程（ASP）编码决策树模型，支持用户偏好设置，并枚举所有可能的解释类型（充分性、对比性、多数性、树特异性解释）。与基于SAT的方法相比，ASP提供更高的编码灵活性。

Result: 在多样化数据集上的实证评估表明，该方法能有效生成各类解释，并展示了其相对于现有方法的优势与局限性。

Conclusion: ASP方法为决策树模型提供了一种灵活且全面的解释生成框架，有助于增强复杂模型在安全关键应用中的可解释性和可信度。
...
TL;DR: 提出xDNN(ASP)系统，通过答案集语义提取深度神经网络的逻辑程序，提供全局解释并揭示网络结构信息。


<details>
  <summary>Details</summary>
Motivation: 现有可解释AI方法（如SHAP、规则提取、反事实解释）主要关注输入输出关系，忽略了神经网络内部结构在解释生成中的作用。

Method: 基于答案集编程（ASP），从训练好的神经网络及其训练数据中提取逻辑程序，使程序答案集与网络输入输出对一一对应。

Result: 在两个合成数据集上的实验表明，提取的逻辑程序不仅保持高预测准确率，还能揭示特征重要性和隐藏节点对预测的影响。

Conclusion: xDNN(ASP)能提供全局解释，帮助理解模型内部机制，并为优化网络结构（如减少隐藏节点）提供指导。
...
TL;DR: 本文评估了当前答案集编程（ASP）技术在解决大规模配置问题（如包含超过30,000个组件的电子系统配置）时的可扩展性，重点研究了应对“接地瓶颈”的方法，并提出了一种名为“约束感知猜测”的新方法以显著降低内存需求。


<details>
  <summary>Details</summary>
Motivation: ASP旨在实现“用户指定问题，计算机解决问题”的AI愿景，并在许多应用领域取得成功。然而，当前ASP求解技术是否能够扩展到大型配置问题（如电子系统配置）仍存疑问，尤其是面对可能包含数万个组件的实例时，接地过程导致的内存需求急剧增加（即“接地瓶颈”）成为主要限制。

Method: 研究了应对接地瓶颈的方法，重点关注增量求解方法；通过分析接地过程，开发了“约束感知猜测”方法，旨在减少内存需求。

Result: 增量求解方法在实践中被证明有效，但内存需求仍构成显著限制；新提出的“约束感知猜测”方法显著降低了内存需求，从而推动了ASP处理大规模配置问题的能力边界。

Conclusion: 当前ASP技术在处理大规模配置问题时面临接地瓶颈的挑战，但通过增量求解和约束感知猜测等方法，可以显著改善内存效率，提升ASP在复杂配置问题中的可扩展性。
...
TL;DR: 本文实证研究发现，当前基于视觉语言模型的智能体难以有效利用生成世界模型进行前瞻性推理，存在调用率低、误用预测结果和性能下降等问题。


<details>
  <summary>Details</summary>
Motivation: 智能体面临的任务日益需要预测未来状态，而生成世界模型理论上可作为外部模拟器帮助智能体预见结果。本研究旨在检验当前智能体能否有效利用世界模型增强认知能力。

Method: 通过在多样化智能体任务和视觉问答任务上进行实验，分析智能体调用世界模型的频率、使用方式及性能变化，并进行归因分析。

Result: 研究发现：智能体很少调用模拟（<1%），经常误用预测推演（约15%），且在模拟可用或强制使用时性能可能下降（最高5%）。主要瓶颈在于智能体决定何时模拟、如何解释预测结果以及如何将预见整合到下游推理的能力不足。

Conclusion: 当前智能体与世界模型的交互存在显著缺陷，需要开发能够促进校准化、战略性交互的机制，以实现更可靠的未来状态预测认知能力。
...
TL;DR: 提出Trade-R1框架，通过过程级推理验证将可验证奖励与随机金融市场连接，解决标准强化学习在金融决策中因奖励噪声导致的奖励黑客问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习在数学、编程等可验证奖励领域表现优异，但金融市场的随机性使奖励虽可验证但噪声大，导致标准强化学习退化为奖励黑客，需要新方法应对金融决策的独特挑战。

Method: 提出过程级推理验证方法，将长篇金融文档的推理评估转化为结构化检索增强生成任务；构建三角一致性指标，评估检索证据、推理链和决策之间的两两对齐，作为噪声市场回报的有效性过滤器；探索固定效应语义奖励和动态效应语义奖励两种奖励整合策略。

Result: 在不同国家资产选择实验中，该框架减少了奖励黑客现象，动态效应语义奖励实现了优异的跨市场泛化能力，同时保持了最高的推理一致性。

Conclusion: Trade-R1框架通过结构化验证将可验证奖励与随机环境连接，有效解决了金融决策中强化学习的奖励噪声问题，为随机环境中的推理验证提供了新范式。
...
TL;DR: 提出动态异常截断方法，解决大模型在强化学习中过度推理的问题，显著提升推理效率同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习增强的大模型在简单查询上存在过度推理问题，导致部署成本高昂，而现有基于长度惩罚的方法存在优化冲突且未深入分析生成机制。

Method: 提出动态异常截断（DOT），在训练时选择性抑制冗余token；结合辅助KL正则化和预测性动态采样以确保稳定收敛。

Result: 在多尺度模型上显著提升了效率-性能帕累托前沿；在AIME-24上减少78%推理token使用，同时准确率超过初始策略和现有高效推理方法。

Conclusion: DOT方法有效解决了长度偏移现象，在保持复杂问题长程推理能力的同时大幅提升推理效率。
...
TL;DR: 提出MobileDreamer框架，通过文本草图世界模型和前瞻推演提升移动GUI代理在长时任务中的决策能力，在Android World上实现SOTA性能，任务成功率提升5.25%。


<details>
  <summary>Details</summary>
Motivation: 现有移动GUI代理多为反应式决策，仅依赖当前屏幕信息，限制了其在长时任务中的表现。构建能够预测动作结果的世界模型可改善决策，但需同时保证空间感知能力和部署效率。

Method: 1. 文本草图世界模型：将数字图像转换为任务关键草图，采用顺序不变学习策略保留GUI元素空间信息；2. 前瞻推演策略：利用世界模型的预测能力优化动作选择过程。

Result: 在Android World测试中达到最先进性能，任务成功率提升5.25%；世界模型评估验证了文本草图建模能准确预测关键GUI元素。

Conclusion: MobileDreamer通过高效的世界模型前瞻框架，显著提升了移动GUI代理在长时任务中的决策能力，其文本草图建模方法能有效预测交互后的界面状态。
...
TL;DR: 提出了ComfySearch框架，通过验证引导的工作流构建，有效探索组件空间并生成功能性的ComfyUI流程，显著提升了复杂创意任务的执行成功率。


<details>
  <summary>Details</summary>
Motivation: ComfyUI等平台上的模块化AI工作流虽然灵活，但组件数量庞大且需在严格图约束下保持结构一致性，导致通过率低、工作流质量有限。

Method: 采用代理框架ComfySearch，通过验证引导的工作流构建方法探索组件空间，生成可执行的ComfyUI流程。

Result: 实验表明，ComfySearch在复杂创意任务上显著优于现有方法，实现了更高的可执行率、解决方案率和更强的泛化能力。

Conclusion: ComfySearch框架能有效解决模块化AI工作流构建中的挑战，提升工作流的可靠性和质量。
...
TL;DR: 本文研究了多智能体大语言模型系统中长期行为稳定性不足的问题，提出了‘智能体漂移’的概念及其三种表现形式，并开发了量化漂移的度量框架与缓解策略。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统在复杂任务分解与协作中表现出强大能力，但其长期行为稳定性尚未得到充分研究，存在行为退化、决策质量下降和协作失效的风险，影响系统可靠部署。

Method: 提出了智能体漂移的理论框架，定义了语义漂移、协调漂移和行为漂移三种类型；开发了包含12个维度的智能体稳定性指数（ASI）进行量化评估；通过仿真分析与理论建模验证漂移影响；提出了三种缓解策略：情景记忆巩固、漂移感知路由协议和自适应行为锚定。

Result: 研究发现，未经控制的智能体漂移会导致任务完成准确率显著下降，并增加人工干预需求；理论分析表明，所提出的缓解策略能有效减少漂移相关错误，同时保持系统吞吐量。

Conclusion: 本研究为生产级智能体AI系统中智能体漂移的监测、度量和缓解提供了基础方法论，对提升企业部署可靠性和AI安全研究具有直接意义。
...
### [32] [HyperCLOVA X 32B Think](https://arxiv.org/abs/2601.03286)
*NAVER Cloud HyperCLOVA X Team*

Main category: cs.CV

TL;DR: 介绍了HyperCLOVA X 32B Think模型，这是一个专注于韩语文化背景推理和智能体能力的视觉语言模型，在多项评测中表现优异并已开源。


<details>
  <summary>Details</summary>
Motivation: 针对韩语语言文化背景的推理需求，以及提升智能体行为能力的动机，开发专门优化的多模态模型。

Method: 采用预训练强化推理能力，后训练支持多模态理解、增强推理、智能体行为和对齐人类偏好。

Result: 在韩语文本生成、视觉文本生成及智能体导向任务上，相比同类规模模型表现强劲。

Conclusion: 通过开源该模型，旨在促进学术和工业界的广泛采用与进一步研究创新。
...
TL;DR: 提出DA3方法，通过域自适应技术将仿真训练的主动对准模型适配到真实场景，减少真实数据需求并提升对准精度。


<details>
  <summary>Details</summary>
Motivation: 光学系统大规模自动化装配依赖主动对准技术，仿真数据与真实图像存在域差异，导致仿真训练模型在真实场景中泛化性能受限。

Method: 提出域自适应主动对准框架，结合自回归域转换生成器和对抗性特征对齐策略，通过自监督学习提取域不变特征进行鲁棒性失准预测。

Result: 在两个镜头类型上，DA3比纯仿真流程精度提升46%，接近3个镜头样本的精确标注真实数据性能，同时将设备端数据采集时间减少98.7%。

Conclusion: 域自适应技术能有效提升仿真训练模型在真实场景中的性能，验证数字孪生流水线可显著提升大规模光学装配效率。
...
### [34] [CageDroneRF: A Large-Scale RF Benchmark and Toolkit for Drone Perception](https://arxiv.org/abs/2601.03302)
*Mohammad Rostami,Atik Faysal,Hongtao Xia,Hadi Kasasbeh,Ziang Gao,Huaxia Wang*

Main category: cs.CV

TL;DR: 提出了CageDroneRF（CDRF）——一个基于真实采集和系统生成合成变体的大规模射频无人机检测与识别基准数据集，旨在解决现有射频数据集稀缺和多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有射频无人机数据集存在数据稀缺、多样性有限的问题，限制了射频感知模型的鲁棒性和泛化能力的发展。

Method: 通过结合大量原始录音和系统化增强流程构建数据集，包括：精确控制信噪比（SNR）、注入干扰发射源、应用频率偏移并进行标签一致的边界框变换以支持检测任务。数据采集于罗文大学校园和受控射频屏蔽笼设施，涵盖多种现有公共数据集中未包含的当代无人机型号。

Result: CDRF数据集覆盖广泛的无人机型号和采集条件，并发布了可互操作的开源工具链，支持数据生成、预处理、增强和评估，且兼容现有公共基准。

Conclusion: CDRF通过提供全面的基准数据集和工具链，旨在推动鲁棒、可泛化的射频感知模型的发展，支持标准化评估和可复现的研究流程。
...
### [35] [Mass Concept Erasure in Diffusion Models with Concept Hierarchy](https://arxiv.org/abs/2601.03305)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型概念擦除方法在处理多个概念时效率低下且效果不佳，每个概念需要独立微调参数，且可能损害整体生成质量。

Method: 1. 构建超类型-子类型概念层次结构，将语义相关概念分组；2. 提出分组抑制方法，共享可学习参数进行联合擦除；3. 设计SuPLoRA技术，在冻结下投影矩阵中编码超类型信息，仅更新上投影矩阵；4. 使用扩散正则化保护未掩码区域去噪过程。

Result: 理论分析证明SuPLoRA能有效缓解生成性能退化，并在包含名人、物体和色情内容的多领域概念擦除基准测试中表现优异。

Conclusion: 该方法通过层次化概念组织和参数共享机制，实现了对多个相关概念的高效擦除，同时保持了模型在非擦除概念上的生成能力。
...
### [36] [Padé Neurons for Efficient Neural Models](https://arxiv.org/abs/2601.04005)
*Onur Keleş,A. Murat Tekalp*

Main category: cs.CV

TL;DR: 本文提出了一种名为Padé神经元（Paons）的新型非线性神经元模型，该模型基于Padé逼近，能够学习输入的不同非线性函数，在更少的层数下提供更强的非线性能力，并在图像超分辨率、压缩和分类任务中优于传统神经元模型。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络通常采用McCulloch-Pitts神经元模型（线性模型加逐点非线性激活），其非线性能力有限。虽然已有二次神经元、广义操作神经元等非线性模型被提出，但仍需探索更优的非线性神经元结构以提升模型表达能力和效率。

Method: 受Padé逼近启发，提出Padé神经元模型。该模型将每个神经元设计为学习输入的非线性函数，而非传统线性变换加激活函数。在实验中，将ResNet架构中经典神经元替换为Paons，应用于图像超分辨率、压缩和分类任务。

Result: 实验表明，使用Paons构建的神经网络在相同或更少层数的情况下，性能优于或等于传统模型。Paons能够提供更丰富的非线性表达，且兼容现有神经元模型（可作为其特例）。

Conclusion: Padé神经元是一种高效的非线性神经元模型，能增强网络表达能力并减少所需层数，为神经网络结构设计提供了新方向。代码已开源。
...
### [37] [VLM4VLA: Revisiting Vision-Language-Models in Vision-Language-Action Models](https://arxiv.org/abs/2601.03309)
*Jianke Zhang,Xiaoyu Chen,Qiuyue Wang,Mingsheng Li,Yanjiang Guo,Yucheng Hu,Jiajun Zhang,Shuai Bai,Junyang Lin,Jianyu Chen*

Main category: cs.CV

TL;DR: 本文通过VLM4VLA框架系统研究了不同视觉语言模型（VLM）在视觉-语言-动作（VLA）策略中的表现，发现VLM的通用能力并不能有效预测下游控制性能，且视觉模块是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型广泛使用预训练的VLM作为策略骨干，但VLM的选择如何影响下游VLA策略性能缺乏系统研究，且通用VLM能力是否足以支持具身控制尚不明确。

Method: 提出VLM4VLA最小适应框架，仅学习少量新参数将通用VLM转换为VLA策略；在三个基准的多个下游任务上进行实证研究；通过七个辅助具身任务微调VLM；进行模态级消融实验。

Result: 1. VLM初始化始终优于从头训练，但VLM通用能力与下游任务性能相关性弱；2. 提升VLM在特定具身任务上的表现不保证更好的控制性能；3. 视觉模块是性能瓶颈，向视觉编码器注入控制相关监督可带来持续增益。

Conclusion: 标准VLM预训练目标与具身动作规划需求存在持续领域差距，视觉编码器的控制相关监督是关键改进方向，通用VLM能力对具身控制必要但不充分。
...
### [38] [Deep Learning-Based Image Recognition for Soft-Shell Shrimp Classification](https://arxiv.org/abs/2601.03317)
*Yun-Hao Zhang,I-Hsien Ting,Dario Liberona,Yun-Hsiu Liu,Kazunori Minetaki*

Main category: cs.CV

TL;DR: 本研究利用基于深度学习的图像识别技术，对刚收获的白虾进行自动分类，以解决虾类产品新鲜度下降和外观完整性问题。


<details>
  <summary>Details</summary>
Motivation: 随着信息技术融入水产养殖，生产稳定性提升，但消费者对高品质水产品需求增加，虾类产品在收获后新鲜度迅速下降，且软壳虾在烹饪或冷冻后易出现头身分离，影响产品外观和消费者感知。

Method: 采用卷积神经网络（CNN）模型替代人工分拣，通过深度学习图像识别技术实现白虾的自动分类。

Result: 该方法提高了分类的准确性、效率和一致性，减少了处理时间，有助于保持虾类新鲜度，并使运输企业能更好地满足客户需求。

Conclusion: 基于深度学习的自动分类技术能有效解决虾类产品的新鲜度和外观完整性问题，提升水产加工效率和产品质量。
...
### [39] [Higher order PCA-like rotation-invariant features for detailed shape descriptors modulo rotation](https://arxiv.org/abs/2601.03326)
*Jarek Duda*

Main category: cs.CV

TL;DR: 提出高阶张量扩展主成分分析（PCA），通过三阶及以上中心矩或多项式乘高斯函数构建旋转不变特征，用于复杂形状描述与识别。


<details>
  <summary>Details</summary>
Motivation: 传统PCA使用协方差矩阵（二阶矩）近似形状为椭球体，但真实形状通常更复杂，需要更高阶描述符以提升精度。

Method: 扩展PCA至三阶或更高阶张量（如p_abc），利用中心矩或多项式乘高斯函数构建可解码的形状描述符，并推导其旋转不变特征。

Result: 方法可生成任意精度的旋转不变形状描述符，适用于分子形状描述、二维图像/三维扫描的旋转无关物体识别，以及无需旋转优化的低成本形状相似度度量。

Conclusion: 高阶张量扩展为复杂形状分析提供了更精确的旋转不变描述框架，在多个领域具有实际应用潜力。
...
### [40] [MMErroR: A Benchmark for Erroneous Reasoning in Vision-Language Models](https://arxiv.org/abs/2601.03331)
*Yang Shi,Yifeng Xie,Minzhe Guo,Liangsi Lu,Mingxuan Huang,Jingchao Wang,Zhihong Zhu,Boyan Xu,Zhiqi Huang*

Main category: cs.CV

TL;DR: 提出了MMErroR基准，用于评估视觉语言模型检测和分类多模态推理错误的能力，发现即使最佳模型准确率也仅66.47%。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注答案正确性，缺乏对推理过程错误的评估。为探究视觉语言模型是否真正理解内容，需要评估其检测错误推理并分类错误类型的能力。

Method: 构建包含2,013个样本的多模态基准MMErroR，每个样本嵌入单一连贯推理错误，涵盖6大领域24个子领域，进行过程级、以错误为中心的评估。

Result: 评估20个先进视觉语言模型，最佳模型（Gemini-3.0-Pro）错误分类准确率仅66.47%，表明识别错误推理仍具挑战性。

Conclusion: 准确识别错误的能力为多模态推理模型的性能提供了重要洞见，现有模型在错误检测方面仍有显著提升空间。
...
### [41] [RelightAnyone: A Generalized Relightable 3D Gaussian Head Model](https://arxiv.org/abs/2601.03357)
*Yingyan Xu,Pramod Rao,Sebastian Weiss,Gaspard Zoss,Markus Gross,Christian Theobalt,Marc Habermann,Derek Bradley*

Main category: cs.CV

TL;DR: 提出一种无需复杂光照采集即可实现高质量重光照的3D高斯人头模型，通过两阶段学习将普通光照的3DGS头像映射到可重光照参数，支持单图或多图输入。


<details>
  <summary>Details</summary>
Motivation: 现有高质量重光照方法需要复杂时序光照采集（如OLAT），限制了实际应用；需要一种能从普通光照数据学习并泛化到新主体的方法。

Method: 两阶段模型：第一阶段从无OLAT的多视角数据学习基础3DGS头像与光照对齐；第二阶段在小规模OLAT数据上学习从基础头像到基于物理反射参数的映射。

Result: 模型能泛化处理未见主体，支持单图或多图输入，实现高质量新视角合成与重光照，效果媲美需要OLAT采集的方法。

Conclusion: 该方法通过解耦学习策略，在减少数据依赖的同时实现了高质量头像重光照，为数字化身应用提供了实用解决方案。
...
### [42] [Guardians of the Hair: Rescuing Soft Boundaries in Depth, Stereo, and Novel Views](https://arxiv.org/abs/2601.03362)
*Xiang Zhang,Yang Zhang,Lukas Mehl,Markus Gross,Christopher Schroers*

Main category: cs.CV

TL;DR: 提出HairGuard框架，通过深度修复网络和生成式场景绘制器，在3D视觉任务中恢复软边界（如发丝）的精细细节，提升深度估计和新视角合成的质量。


<details>
  <summary>Details</summary>
Motivation: 自然和计算机生成图像中常见的软边界（如发丝）因前景与背景线索混合模糊，在3D视觉任务中难以准确处理，需要专门方法恢复其精细细节。

Method: 1. 利用图像抠图数据集构建训练数据管道；2. 设计深度修复网络（含门控残差模块）自动识别并优化软边界区域的深度；3. 通过深度前向扭曲保留高保真纹理，结合生成式场景绘制器填充遮挡区域；4. 使用颜色融合器自适应结合扭曲与修复结果。

Result: 在单目深度估计、立体图像/视频转换和新视角合成任务中达到最先进性能，尤其在软边界区域有显著提升。

Conclusion: HairGuard框架能有效处理3D视觉中的软边界模糊问题，通过模块化设计实现与现有深度模型的即插即用集成，在多种任务中均验证了其优越性。
...
### [43] [RiskCueBench: Benchmarking Anticipatory Reasoning from Early Risk Cues in Video-Language Models](https://arxiv.org/abs/2601.03369)
*Sha Luo,Yogesh Prabhu,Tim Ossowski,Kaiping Chen,Junjie Hu*

Main category: cs.CV

TL;DR: 本文介绍了RiskCueBench视频理解基准，旨在评估模型从早期视觉信号预测风险事件的能力，发现现有系统在解读动态情境和预测未来风险方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频风险评估研究多基于完整视频序列（包含事故本身），降低了任务难度，无法反映真实世界中需从早期信号预测风险的实际需求。

Method: 构建RiskCueBench基准数据集，对视频进行精细标注以识别'风险信号片段'（即最早指示潜在安全问题的时刻），并基于此评估现有视频理解系统。

Result: 实验结果表明，当前系统在解读动态情境演变和基于早期视觉信号预测未来风险事件方面存在显著能力差距。

Conclusion: 该研究揭示了视频风险预测模型在实际部署中的重要挑战，强调需要开发能够从早期线索有效推断风险的新型方法。
...
### [44] [A Novel Unified Approach to Deepfake Detection](https://arxiv.org/abs/2601.03382)
*Lord Sen,Shyamapada Mukherjee*

Main category: cs.CV

TL;DR: 提出了一种结合空间域和频域特征交叉注意力以及血液检测模块的新型深度伪造检测架构，在多个数据集上取得了优于现有方法的检测性能。


<details>
  <summary>Details</summary>
Motivation: 人工智能技术的进步导致深度伪造威胁日益严重，为维持数字时代的信任，迫切需要有效的深度伪造检测和标记方法。

Method: 使用空间域和频域特征的交叉注意力机制，结合血液检测模块，构建统一的图像和视频深度伪造检测架构；采用Swin Transformer/BERT和EfficientNet-B4/BERT两种组合进行实验验证。

Result: 在FF++和Celeb-DF数据集上分别达到99.80%和99.88%的AUC（使用Swin Transformer和BERT），以及99.55%和99.38%的AUC（使用EfficientNet-B4和BERT）；同时在跨数据集测试中表现出良好的泛化能力。

Conclusion: 所提出的统一架构在深度伪造检测任务中实现了优于当前最优方法的性能，并展示了良好的泛化能力，为深度伪造检测提供了有效解决方案。
...
### [45] [Better, But Not Sufficient: Testing Video ANNs Against Macaque IT Dynamics](https://arxiv.org/abs/2601.03392)
*Matteo Dunnhofer,Christian Micheloni,Kohitij Kar*

Main category: cs.CV

TL;DR: 研究发现，尽管视频模型能部分预测猕猴颞下皮层（IT）对动态视频的响应，但它们无法捕捉IT对形状和纹理不变的运动编码，揭示了当前模型与生物视觉系统在动态计算上的差距。


<details>
  <summary>Details</summary>
Motivation: 现有基于静态图像的前馈神经网络模型无法充分解释灵长类视觉系统对动态世界的处理机制，特别是IT皮层在自然视频观看中如何编码物体运动速度及是否涉及更丰富的动态计算。

Method: 通过比较猕猴IT皮层在观看自然视频时的神经响应与静态、循环和视频基人工神经网络模型的预测能力，并设计“无外观”视频（保留运动但移除形状纹理）作为压力测试，评估模型的泛化性能。

Result: 视频模型对神经响应的预测能力有适度提升（尤其在后期响应阶段），但所有模型均无法泛化到“无外观”视频，而IT神经活动却能保持跨条件一致性。

Conclusion: 当前视频模型仅能捕捉外观依赖的动态特征，未能实现IT皮层表现的外观不变性动态计算，未来需开发能编码生物时间统计特性及不变性的新目标函数。
...
### [46] [Eye-Q: A Multilingual Benchmark for Visual Word Puzzle Solving and Image-to-Phrase Reasoning](https://arxiv.org/abs/2601.03400)
...
TL;DR: 提出视觉字谜基准Eye-Q，评估视觉语言模型在需要深层推理、假设生成与修正的非字面视觉理解任务上的表现，发现现有模型存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在标准基准上表现良好，但往往依赖表面识别而非深层推理。需要更复杂的任务来评估模型发现隐含视觉线索、生成修正假设、将感知证据映射到非字面概念的能力。

Method: 构建多语言视觉字谜基准Eye-Q，包含1343个概念密集场景的谜题，要求模型根据简短描述推断目标词或短语。采用开放式、人类对齐的评估协议，在轻量辅助下探测假设形成与修正过程。

Result: 现有最先进视觉语言模型在Eye-Q上表现不佳，尤其在抽象和跨语言谜题上存在显著性能差距，最高准确率仅为60.27%。

Conclusion: 视觉字谜任务揭示了当前模型在构建和搜索适当概念表示以进行灵活图像到短语推理方面的局限性，强调了开发更复杂推理能力的必要性。
...
### [47] [GAMBIT: A Gamified Jailbreak Framework for Multimodal Large Language Models](https://arxiv.org/abs/2601.03416)
*Xiangdong Hu,Yangyang Jiang,Qin Hu,Xiaojun Jia*

Main category: cs.CV

TL;DR: 提出GAMBIT框架，通过游戏化场景诱导多模态大语言模型主动完成越狱攻击，显著提升攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击主要增加视觉任务复杂度，未充分利用模型自身推理激励，导致在具备思维链的推理模型上效果不佳。

Method: 分解并重组有害视觉语义，构建游戏化场景驱动模型探索、重构意图，通过结构化推理链同时增加视觉和文本任务复杂度。

Result: 在主流推理与非推理MLLMs上实现高攻击成功率：Gemini 2.5 Flash达92.13%，QvQ-MAX达91.20%，GPT-4o达85.87%，显著超越基线方法。

Conclusion: 通过操纵模型认知阶段决策的游戏化攻击框架能有效突破安全对齐机制，揭示当前MLLMs安全防御的脆弱性。
...
### [48] [WeedRepFormer: Reparameterizable Vision Transformers for Real-Time Waterhemp Segmentation and Gender Classification](https://arxiv.org/abs/2601.03431)
*Toqi Tahamid Sarker,Taminul Islam,Khaled R. Ahmed,Cristiana Bernardi Rankrape,Kaitlin E. Creager,Karla Gage*

Main category: cs.CV

TL;DR: 提出WeedRepFormer，一种轻量级多任务视觉Transformer，用于同时进行水麻分割和性别分类，在保持高性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有农业模型难以平衡生物属性分类所需的细粒度特征提取与实时部署所需的效率，需要一种既能精确识别杂草属性又能高效运行的解决方案。

Method: 采用结构重参数化技术，整合视觉Transformer主干、Lite R-ASPP解码器和可重参数化分类头，在训练时保持强大容量，推理时降低延迟；并构建包含10,264标注帧的水麻数据集。

Result: 在分割任务上达到92.18% mIoU，性别分类准确率81.91%，仅需3.59M参数和3.80 GFLOPs，以108.95 FPS运行，分类准确率比iFormer-T提升4.40%，参数量减少1.9倍。

Conclusion: WeedRepFormer通过系统性的结构重参数化，实现了多任务杂草分析的高精度与高效率平衡，为农业实时视觉应用提供了有效解决方案。
...
### [49] [FROST-Drive: Scalable and Efficient End-to-End Driving with a Frozen Vision Encoder](https://arxiv.org/abs/2601.03460)
*Zeyu Dong,Yimin Zhu,Yu Wu,Yu Sun*

Main category: cs.CV

TL;DR: 提出FROST-Drive架构，通过冻结预训练视觉编码器来保留其泛化能力，在自动驾驶端到端模型中实现更好的长尾场景性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型通常对视觉编码器进行全微调，这可能导致模型过度适应训练数据而削弱泛化能力，特别是在新颖复杂场景中。

Method: 采用冻结的预训练视觉语言模型编码器，结合Transformer适配器进行多模态融合，使用GRU解码器生成平滑路径点，并设计专门优化Rater Feedback Score的损失函数。

Result: 在Waymo Open E2E数据集上的实验表明，冻结编码器方法显著优于全微调模型，尤其在长尾场景中表现更好。

Conclusion: 保留强大视觉语言模型的广泛知识比领域特定适应更能实现鲁棒、可泛化的驾驶性能，为开发处理现实世界复杂性的视觉模型提供了新途径。
...
### [50] [Experimental Comparison of Light-Weight and Deep CNN Models Across Diverse Datasets](https://arxiv.org/abs/2601.03463)
*Md. Hefzul Hossain Papon,Shadman Rabby*

Main category: cs.CV

TL;DR: 研究表明，经过良好正则化的浅层架构在异构领域（从智慧城市监控到农业品种分类）中可作为高度竞争力的基线，无需大型GPU或专用预训练模型。


<details>
  <summary>Details</summary>
Motivation: 为低资源环境中的实际部署提供轻量级解决方案，并为多个孟加拉国视觉数据集建立统一、可复现的基准。

Method: 使用经过良好正则化的浅层卷积神经网络（CNN）架构，避免依赖大型GPU或专用预训练模型。

Result: 该浅层架构在异构视觉任务中表现出高度竞争力，成功应用于智慧城市监控和农业品种分类等多个领域。

Conclusion: 轻量级CNN在低资源环境中具有实际部署价值，为相关领域提供了可复现的基准和高效解决方案。
...
### [51] [Latent Geometry of Taste: Scalable Low-Rank Matrix Factorization](https://arxiv.org/abs/2601.03466)
*Joshua Salako*

Main category: cs.CV

TL;DR: 本研究通过并行化ALS框架在MovieLens 32M数据集上优化协同过滤，发现约束低秩模型在泛化性能上优于高维模型，并可视化出语义流派聚类，验证了在冷启动场景中的实用性。


<details>
  <summary>Details</summary>
Motivation: 大规模交互数据集中的可扩展性和数据稀疏性是协同过滤的关键瓶颈，需要研究用户偏好的潜在几何结构以提升推荐性能。

Method: 采用高性能并行化交替最小二乘法（ALS）框架，在MovieLens 32M数据集上进行超参数优化，可视化嵌入空间，并引入可调评分参数处理冷启动场景。

Result: 约束低秩模型在均方根误差（RMSE）和排序精度上显著优于高维模型；嵌入空间无监督地形成了语义流派聚类；冷启动场景中通过可调参数有效平衡了流行度偏差与个性化亲和力。

Conclusion: 低秩模型能更好地捕捉交互数据中的深层结构关系，可视化证实了语义聚类的涌现，所提系统在冷启动场景中具有实际应用价值。
...
### [52] [ThinkRL-Edit: Thinking in Reinforcement Learning for Reasoning-Centric Image Editing](https://arxiv.org/abs/2601.03467)
*Hengjia Li,Liming Jiang,Qing Yan,Yizhi Song,Hao Kang,Zichuan Liu,Xin Lu,Boxi Wu,Deng Cai*

Main category: cs.CV

TL;DR: 提出ThinkRL-Edit框架，通过解耦视觉推理与图像合成、引入思维链推理采样和无偏奖励策略，显著提升指令驱动图像编辑的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态生成模型在图像编辑中视觉推理能力有限，导致推理密集型编辑效果不佳；传统强化学习方法存在推理探索受限、奖励偏差和不稳定等问题。

Method: 1. 设计思维链推理采样（规划与反思阶段）扩展推理探索；2. 提出无偏链偏好分组策略避免奖励聚合偏差；3. 用二元检查表替代基于区间的VLM评分，提供更精确低方差的奖励。

Result: 实验表明该方法在推理密集型图像编辑任务上显著优于先前工作，生成结果更符合指令、视觉连贯且语义合理。

Conclusion: ThinkRL-Edit通过强化推理探索与稳定奖励机制，有效解决了复杂指令图像编辑中的核心挑战，为多模态生成模型的推理能力提升提供了新思路。
...
### [53] [Understanding Reward Hacking in Text-to-Image Reinforcement Learning](https://arxiv.org/abs/2601.03468)
*Yunqi Hong,Kuei-Chun Kao,Hengguang Zhou,Cho-Jui Hsieh*

Main category: cs.CV

TL;DR: 本文系统分析了文本到图像强化学习后训练中的奖励黑客问题，并提出了一种轻量级自适应伪影奖励模型作为解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有奖励函数常作为人类判断的不完美代理，导致模型产生奖励黑客行为——生成不真实或低质量但获得高奖励的图像。

Method: 分析美学/人类偏好奖励和提示-图像一致性奖励对奖励黑客的贡献；提出基于小规模精选数据训练的轻量级自适应伪影奖励模型，作为现有奖励模型的补充正则化器。

Result: 实验表明，加入伪影奖励能显著提升视觉真实感，减少多种文本到图像强化学习设置中的奖励黑客行为。

Conclusion: 轻量级奖励增强可作为对抗奖励黑客的有效保障，改善图像生成质量与人类偏好对齐。
...
TL;DR: 提出MDENeRF框架，通过NeRF的深度信息和不确定性迭代优化单目深度估计，增强几何细节


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法生成的深度图过于平滑，缺乏精细几何细节，影响场景理解精度

Method: 1) 初始单目深度估计提供全局结构；2) 基于扰动视角训练NeRF并计算逐像素不确定性；3) 贝叶斯融合单目与NeRF深度

Result: 在SUN RGB-D室内场景数据集上取得优于现有方法的性能表现

Conclusion: MDENeRF能有效融合单目深度估计的全局结构与NeRF的高频细节，提升深度图质量
...
### [55] [CroBIM-U: Uncertainty-Driven Referring Remote Sensing Image Segmentation](https://arxiv.org/abs/2601.03490)
*Yuzhe Sun,Zhe Dong,Haochen Jiang,Tianzhu Liu,Yanfeng Gu*

Main category: cs.CV

TL;DR: 提出一种不确定性引导的遥感图像指代分割框架，通过预测指代不确定性图来指导自适应融合与局部优化，提升复杂场景下的分割鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有遥感指代分割方法在全图采用均匀的跨模态融合策略，忽视了因尺度变化、密集相似目标和复杂边界导致的指代可靠性空间不均匀问题，导致清晰区域受语言干扰、混淆区域歧义消除不足。

Method: 1) 设计可插拔的指代不确定性评分器（RUS），通过在线误差一致性监督预测像素级指代歧义分布；2) 基于不确定性图设计不确定性门控融合（UGF）模块，动态调节语言注入强度；3) 设计不确定性驱动局部优化（UDLR）模块，聚焦优化易错边界与细节。

Result: 大量实验表明，该方法作为统一的即插即用方案，在不改变主干网络的情况下，显著提升了复杂遥感场景下的分割鲁棒性与几何保真度。

Conclusion: 通过显式建模指代不确定性的空间分布，并以此引导自适应推理，能够有效缓解遥感指代分割中的空间不均匀对齐问题，为跨模态遥感理解提供了新思路。
...
### [56] [SDCD: Structure-Disrupted Contrastive Decoding for Mitigating Hallucinations in Large Vision-Language Models](https://arxiv.org/abs/2601.03500)
*Yuxuan Xia,Siheng Wang,Peng Li*

Main category: cs.CV

TL;DR: 本文提出了一种无需训练的方法SDCD，通过引入结构扰乱视图进行对比解码，有效缓解大型视觉语言模型中的物体幻觉问题。
...
Method: 提出结构扰乱对比解码（SDCD）算法：通过引入打乱顺序的结构扰乱视图，对输出分布进行对比校准；惩罚在结构缺失视图中仍保持高置信度的token，从而抑制纹理驱动偏差。

Result: 实验表明SDCD在多个基准测试中显著减少幻觉现象，并提升了LVLMs的整体多模态能力。

Conclusion: 视觉统计偏差是物体幻觉的关键因素，SDCD通过结构扰乱对比机制有效缓解了该问题，为提升LVLMs的可靠性提供了新思路。
...
### [57] [Systematic Evaluation of Depth Backbones and Semantic Cues for Monocular Pseudo-LiDAR 3D Detection](https://arxiv.org/abs/2601.03617)
*Samson Oseiwe Ajadalu*

Main category: cs.CV

TL;DR: 本文系统评估了深度估计主干网络和特征工程对单目伪激光雷达3D检测性能的影响，发现深度主干网络选择和几何保真度对性能起主导作用，而语义特征注入仅带来边际增益。


<details>
  <summary>Details</summary>
Motivation: 单目3D检测因从单张图像估计度量深度的困难而精度较低，本研究旨在探究深度估计主干网络和特征工程如何影响伪激光雷达管道的性能，以提升单目3D检测的准确性。

Method: 在KITTI验证集上，使用相同的伪激光雷达生成和PointRCNN检测协议，比较NeWCRFs（监督度量深度）与Depth Anything V2 Metric-Outdoor（Base）的性能；测试了点云增强方法，包括外观线索（灰度强度）和语义线索（实例分割置信度）；并利用真实2D边界框进行了深度精度与距离的诊断分析。

Result: NeWCRFs在中等难度分割上使用灰度强度时达到10.50% AP3D（IoU=0.7），表现优于Depth Anything V2；语义特征仅带来边际增益，基于掩码的采样可能因移除上下文几何而降低性能；深度精度与距离分析表明，粗略深度正确性不能完全预测严格的3D IoU。

Conclusion: 在使用现成激光雷达检测器时，深度主干网络的选择和几何保真度主导性能，超过次要特征注入的影响；语义特征未能显著缩小性能差距，几何保真度是关键因素。
...
TL;DR: CHORD是一个通用的生成式管道，用于合成动态物体和场景的4D（3D+时间）动态，通过从2D视频中提取拉格朗日运动信息，实现类别无关的动态生成，并应用于机器人操作策略。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的图形管道创建动态场景劳动密集且不可扩展，而基于学习的方法需要大规模数据集，可能无法覆盖所有感兴趣的物体类别。本研究旨在开发一种通用、类别无关的方法，从2D视频中提取运动信息以生成多样化的4D动态。

Method: 提出基于蒸馏的管道，从2D视频的欧拉表示中提取隐藏的拉格朗日运动信息，利用视频生成模型的通用性，实现类别无关的动态合成。

Result: 实验表明，该方法能生成多样化的多体4D动态，优于现有方法，并成功应用于机器人操作策略的生成。

Conclusion: CHORD是一种通用、多功能且类别无关的方法，有效解决了动态场景生成的挑战，展示了在机器人学等领域的应用潜力。
...
### [59] [REFA: Real-time Egocentric Facial Animations for Virtual Reality](https://arxiv.org/abs/2601.03507)
...
TL;DR: 提出一种基于VR头显红外摄像头的实时面部表情追踪系统，无需繁琐校准即可驱动虚拟角色表情，使用蒸馏方法训练多源数据模型。


<details>
  <summary>Details</summary>
Motivation: 传统面部捕捉系统通常需要复杂设备或校准步骤，限制了虚拟环境中自然表情交互的普及性和实时性。

Method: 采用蒸馏学习方法整合合成与真实图像的多源数据；开发轻量级采集设备（手机+定制VR头显）收集18k多样化样本；构建可微分渲染管道自动提取表情标签。

Result: 系统实现了非侵入式实时面部表情追踪，用户无需校准即可驱动虚拟角色表情，为虚拟环境中的交流提供了新工具。

Conclusion: 该系统通过多源数据训练和轻量化采集方案，突破了传统表情捕捉的限制，在视频会议、游戏娱乐和远程协作等领域具有应用潜力。
...
### [60] [G2P: Gaussian-to-Point Attribute Alignment for Boundary-Aware 3D Semantic Segmentation](https://arxiv.org/abs/2601.03510)
...
TL;DR: 提出Gaussian-to-Point（G2P）方法，将3D高斯泼溅的外观感知属性迁移到点云，以增强语义分割的区分能力，特别是在几何相似但外观不同的物体上。


<details>
  <summary>Details</summary>
Motivation: 点云的稀疏和不规则分布限制了外观证据，仅依赖几何特征难以区分形状相似但外观（如颜色、纹理、材质）不同的物体。

Method: 通过建立点与优化后高斯之间的对应关系，迁移高斯的外观感知属性（如不透明度和尺度）到点云，以解决几何模糊性并提升边界定位精度。

Result: 在标准基准测试中取得优越性能，在几何挑战性类别上显著改进，且无需任何2D或语言监督。

Conclusion: G2P通过有效结合几何与外观信息，提升了点云语义分割的判别力和一致性，为3D场景理解提供了更鲁棒的解决方案。
...
### [61] [Semantic Belief-State World Model for 3D Human Motion Prediction](https://arxiv.org/abs/2601.03517)
*Sarim Chaudhry*

Main category: cs.CV

TL;DR: 提出语义信念状态世界模型（SBWM），将人体运动预测重构为人体流形上的潜在动力学模拟，通过分离观测重建与动力学建模实现更稳定的长期运动预测。


<details>
  <summary>Details</summary>
Motivation: 传统人体运动预测方法将运动视为序列回归问题，存在长期预测时的漂移、均值姿态塌缩和不确定性校准不佳等问题，且未分离观测重建与动力学建模，缺乏对运动潜在因果的显式表示。

Method: 构建基于SMPL-X参数化的概率信念状态，通过独立于姿态重建的潜在状态演化学习运动动力学；采用随机潜在转移和以推演为中心的训练策略，在人体流形上进行潜在动力学模拟。

Result: 模型实现了连贯的长期运动推演，在显著降低计算成本的同时保持了竞争力精度，避免了传统方法在训练外推时的性能退化问题。

Conclusion: 将人体视为世界模型状态空间的一部分而非输出，从根本上改变了运动模拟与预测的方式，证明了动力学建模与观测重建分离对稳定长期预测的有效性。
...
### [62] [CloudMatch: Weak-to-Strong Consistency Learning for Semi-Supervised Cloud Detection](https://arxiv.org/abs/2601.03528)
*Jiayi Zhao,Changlu Chen,Jingsheng Li,Tianxiang Xue,Kun Zhan*

Main category: cs.CV

TL;DR: 提出CloudMatch半监督框架，通过视图一致性学习和场景混合增强，有效利用未标注遥感图像进行云检测。


<details>
  <summary>Details</summary>
Motivation: 像素级标注成本高昂，需要开发能够有效利用未标注数据的半监督学习方法进行云检测。

Method: 采用视图一致性学习结合场景混合增强：为未标注图像生成一个弱增强视图和两个互补的强增强视图（分别进行场景间混合和场景内混合），通过强制预测一致性引导伪标签生成。

Result: 大量实验表明CloudMatch取得了良好性能，能够高效利用未标注数据。

Conclusion: CloudMatch通过捕捉云模式的结构多样性和上下文丰富性，推进了半监督云检测技术的发展。
...
### [63] [Physics-Constrained Cross-Resolution Enhancement Network for Optics-Guided Thermal UAV Image Super-Resolution](https://arxiv.org/abs/2601.03526)
*Zhicheng Zhao,Fengjiao Peng,Jinquan Yan,Wei Lu,Chenglong Li,Jin Tang*

Main category: cs.CV

TL;DR: 提出PCNet方法，通过跨分辨率互增强模块和物理驱动的热传导模块，解决光学引导热成像超分辨率中的信息损失和物理不一致问题，显著提升无人机热图像重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有光学引导热图像超分辨率方法压缩光学特征以匹配热特征维度，导致高频信息丢失，并因忽略成像物理差异而产生纹理失真、边缘模糊等物理不一致伪影。

Method: 1. 跨分辨率互增强模块（CRME）联合优化热图像超分辨率和光-热模态转换，实现双向特征交互；2. 物理驱动热传导模块（PDTM）将二维热传导融入光学引导，建模空间变化的热传导特性；3. 引入温度一致性损失确保区域分布一致性和边界梯度平滑。

Result: 在VGTSR2.0和DroneVehicle数据集上的实验表明，PCNet在重建质量和下游任务（语义分割、目标检测）上显著优于现有最优方法。

Conclusion: PCNet通过跨模态互增强和物理约束的光学引导，实现了鲁棒的无人机热图像超分辨率，解决了现有方法的信息损失和物理不一致问题，具有实际应用价值。
...
### [64] [EASLT: Emotion-Aware Sign Language Translation](https://arxiv.org/abs/2601.03549)
*Guobin Tu,Di Weng*

Main category: cs.CV

TL;DR: 提出EASLT框架，通过情感编码器和情感感知融合模块整合面部表情情感信息，解决手语翻译中因相同手势表达不同概念导致的语义歧义问题，在多个基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有无注释手语翻译方法主要关注手势信号，常忽略面部表情的关键语义作用，导致相同手势表达不同概念时产生歧义，需要更有效地整合非手动信号的情感信息。

Method: 提出EASLT框架，包含专门的情感编码器捕捉连续情感动态，通过情感感知融合模块自适应地基于情感上下文重新校准时空手语特征，将面部情感作为语义锚点而非辅助信息。

Result: 在PHOENIX14T和CSL-Daily基准测试中，BLEU-4得分分别达到26.15和22.80，BLEURT得分分别为61.0和57.8，在无注释方法中表现先进；消融研究证实情感建模能有效解耦情感语义与手动动态。

Conclusion: 明确建模情感能有效分离情感语义与手势动态，显著提升翻译保真度，面部表情应被视为强大的语义锚点而非次要信息，为手语翻译提供了新视角。
...
### [65] [SpatiaLoc: Leveraging Multi-Level Spatial Enhanced Descriptors for Cross-Modal Localization](https://arxiv.org/abs/2601.03579)
*Tianyi Shang,Pengjie Xu,Zhaojun Deng,Zhenyu Li,Zhicong Chen,Lijun Wu*

Main category: cs.CV

TL;DR: 提出SpatiaLoc框架，通过粗到细策略利用空间关系进行跨模态定位，在KITTI360Pose数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 基于文本和点云的跨模态定位在自主导航和人机交互中有重要应用，而空间关系是该任务中最具区分性的线索。

Method: 采用粗到细策略：粗阶段使用BEOSE编码实例级空间关系（二次贝塞尔曲线）和FAE编码全局级频域空间表示；细阶段使用UGFL通过高斯分布建模预测并回归2D位置。

Result: 在KITTI360Pose数据集上的大量实验表明，SpatiaLoc显著优于现有最先进方法。

Conclusion: SpatiaLoc框架通过强调实例和全局层面的空间关系，有效提升了跨模态定位性能。
...
### [66] [Detecting AI-Generated Images via Distributional Deviations from Real Images](https://arxiv.org/abs/2601.03586)
*Yakun Niu,Yingjian Chen,Lei Zhang*

Main category: cs.CV

TL;DR: 提出基于掩码预训练模型微调（MPFT）策略，通过纹理感知掩码（TAM）机制提升CLIP-ViT对AI生成图像的检测泛化能力，在少量数据微调下取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型快速发展导致AI生成图像质量提升，引发虚假信息与信任危机。现有基于冻结CLIP的方法未能充分利用图像编码器潜力，且泛化至未见生成模型的能力有限。

Method: 1. 分析发现冻结CLIP-ViT能聚类真实图像，但缺乏区分真伪能力；2. 提出MPFT策略，引入TAM机制在微调时掩码含生成模型特定纹理的区域；3. 迫使模型关注真实图像的'分布偏差'以提升检测泛化性。

Result: 在GenImage和UniversalFakeDetect数据集上，仅用少量图像微调即达到98.2%和94.6%的平均准确率，显著优于现有方法。

Conclusion: 通过针对性掩码策略微调预训练模型，能有效激发CLIP-ViT区分AI生成图像的内在能力，为检测未知生成模型输出提供了高效解决方案。
...
### [67] [Can LLMs See Without Pixels? Benchmarking Spatial Intelligence from Textual Descriptions](https://arxiv.org/abs/2601.03590)
*Zhongbin Guo,Zhen Yang,Yushan Li,Xinyue Zhang,Wenyu Gao,Jiacheng Wang,Chengzhi Li,Xiangrui Liu,Ping Jian*

Main category: cs.CV

TL;DR: 该论文提出了SiT-Bench基准测试，用于评估纯文本大语言模型的空间智能能力，发现现有模型在局部语义任务上表现良好，但在全局一致性上存在显著差距，且显式空间推理能提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前空间智能研究主要依赖视觉语言模型，但尚不清楚空间理解能力究竟源于视觉编码器还是基础推理架构。为探究大语言模型本身的空间推理潜力，需要构建不依赖像素输入的评估基准。

Method: 构建包含3,800+专家标注样本的SiT-Bench基准，涵盖5大类17个子任务（如自我中心导航、视角转换、机器人操作）。通过将单/多视角场景转换为高保真坐标感知的文本描述，迫使模型进行符号化文本推理而非视觉模式匹配。

Result: 评估显示：先进大语言模型在局部语义任务上表现熟练，但在全局空间一致性上存在明显“空间鸿沟”；引入显式空间推理能显著提升性能，表明大语言模型具有潜在的世界建模能力。

Conclusion: SiT-Bench可作为基础资源，促进未来视觉语言模型和具身智能体所需的空间接地大语言模型主干的发展。研究证明大语言模型具备空间推理潜力，但需突破全局一致性瓶颈。
...
### [68] [Adaptive Attention Distillation for Robust Few-Shot Segmentation under Environmental Perturbations](https://arxiv.org/abs/2601.03596)
*Qianyu Guo,Jingrong Wu,Jieji Ren,Weifeng Ge,Wenqiang Zhang*

Main category: cs.CV

TL;DR: 本文提出环境鲁棒性少样本分割（ER-FSS）新设定及自适应注意力蒸馏（AAD）方法，以提升模型在复杂现实环境中的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有少样本分割研究多忽略真实场景中的复杂环境因素（如光照、背景、运动模糊等），导致实验室训练的模型难以满足实际部署需求。

Method: 1. 提出环境鲁棒性FSS设定，构建包含8个数据集的ER-FSS基准；2. 设计自适应注意力蒸馏方法，通过反复对比支持图像与查询图像的共享语义，蒸馏出针对新类别的类特定注意力。

Result: AAD方法在所有数据集和设定下将mIoU提升3.3%-8.5%，表现出优越性能和强泛化能力。

Conclusion: 环境鲁棒性FSS设定能更好反映实际挑战，AAD方法通过注意力蒸馏有效增强模型在复杂环境中聚焦正确目标的能力，为实际应用提供可行方案。
...
### [69] [Shape Classification using Approximately Convex Segment Features](https://arxiv.org/abs/2601.03625)
*Bimal Kumar Ray*

Main category: cs.CV

TL;DR: 提出一种基于特征排序的对象分类方法，通过归一化边界、分割为近似凸段并按长度排序，使用多特征度量边界相似性，无需对象对齐。


<details>
  <summary>Details</summary>
Motivation: 现有基于描述性特征的对象分类技术依赖对象对齐来计算相似性，限制了方法的灵活性和效率。

Method: 归一化对象边界并分割为近似凸段，按段长度降序排序，提取段长度、极值点数量、面积、底边和宽度等特征集合度量边界相似性。

Result: 在多个数据集上测试，取得了可接受的分类结果。

Conclusion: 该方法通过特征排序替代对象对齐，简化了相似性计算流程，为对象分类提供了新的可行方案。
...
### [70] [Unveiling Text in Challenging Stone Inscriptions: A Character-Context-Aware Patching Strategy for Binarization](https://arxiv.org/abs/2601.03609)
*Pratyush Jena,Amal Joseph,Arnav Sharma,Ravi Kiran Sarvadevabhatla*

Main category: cs.CV

TL;DR: 提出一种针对历史石刻图像的自适应分块策略和注意力U-Net二值化方法，有效解决石刻图像因对比度低、退化不均等导致的二值化难题，并在跨文字零样本泛化中表现优异。


<details>
  <summary>Details</summary>
Motivation: 石刻图像因刻痕与背景对比度低、表面退化不均、噪声干扰及文字布局多变，导致传统二值化方法难以提取连贯字符区域，亟需鲁棒的二值化技术以支持后续文字识别与分析。

Method: 1. 设计自适应动态分块与选择策略，提升文字片段分辨率；2. 采用注意力U-Net进行二值化，注意力机制聚焦细微结构特征；3. 构建像素级标注的印度文字石刻数据集用于训练。

Result: 1. 新分块机制显著提升经典与深度学习基线模型的二值化性能；2. 仅用单一印度文字训练，模型在其它印度文字及非印度文字上展现强零样本泛化能力；3. 生成清晰结构化表征，为下游任务奠定基础。

Conclusion: 该方法通过自适应分块与注意力机制有效克服石刻图像二值化挑战，具备脚本无关的泛化能力，为历史文字分析、OCR等任务提供了可靠预处理工具。
...
### [71] [MFC-RFNet: A Multi-scale Guided Rectified Flow Network for Radar Sequence Prediction](https://arxiv.org/abs/2601.03633)
*Wenjie Luo,Chuanhu Deng,Chaorong Li,Rongyao Deng,Qiang Yang*

Main category: cs.CV

TL;DR: 提出MFC-RFNet生成框架，通过多尺度特征通信、空间对齐和频率感知融合，提升雷达回波序列降水临近预报的准确性和分辨率。


<details>
  <summary>Details</summary>
Motivation: 雷达回波序列的降水临近预报对灾害减灾和经济规划至关重要，但面临多尺度演化建模、位移导致的帧间特征错位、长程时空上下文高效捕获等挑战。

Method: 采用整流流训练学习近线性概率流轨迹；引入小波引导跳跃连接保留高频细节；特征通信模块促进双向跨尺度交互；条件引导空间变换融合对齐浅层特征；轻量级Vision-RWKV块捕获长程时空依赖。

Result: 在四个公开数据集上评估，相比基线模型持续改进，在更高雨强阈值下产生更清晰回波形态，在更长预见期保持预报技能。

Conclusion: 整流流训练与尺度感知通信、空间对齐、频率感知融合的协同，为雷达临近预报提供了有效且鲁棒的方法。
...
### [72] [VideoMemory: Toward Consistent Video Generation via Memory Integration](https://arxiv.org/abs/2601.03655)
*Jinsong Zhou,Yihua Du,Xinli Xu,Luozhou Wang,Zijie Zhuang,Yehang Zhang,Shuaibo Li,Xiaojun Hu,Bolan Su,Ying-cong Chen*

Main category: cs.CV

TL;DR: 提出VideoMemory框架，通过动态记忆库存储和更新实体表示，解决叙事视频生成中跨镜头实体一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在场景变化或长时间间隔后难以保持角色、道具和环境的身份与外观一致性，影响叙事连贯性。

Method: 采用多智能体系统分解叙事脚本，结合动态记忆库存储实体视觉与语义描述符，通过检索-更新机制在生成过程中保持实体一致性。

Result: 在包含54个案例的多镜头一致性基准测试中，VideoMemory在实体级连贯性和感知质量方面表现优异。

Conclusion: VideoMemory通过显式的记忆机制有效支持长叙事视频的连贯生成，为实体一致性提供了可扩展的解决方案。
...
### [73] [CrackSegFlow: Controllable Flow-Matching Synthesis for Generalizable Crack Segmentation with the CSF-50K Benchmark](https://arxiv.org/abs/2601.03637)
*Babak Asadi,Peiyang Wu,Mani Golparvar-Fard,Ramez Hajj*

Main category: cs.CV

TL;DR: 提出CrackSegFlow框架，通过可控流匹配合成生成真实裂缝图像和掩码，解决裂缝分割中标签稀缺和域偏移问题，显著提升分割性能并发布大规模数据集。


<details>
  <summary>Details</summary>
Motivation: 裂缝自动分割在实际部署中面临像素级标签稀缺、传感器/光照/纹理/标注规范差异导致的严重域偏移问题，限制了规模化基础设施状态评估。

Method: 1) 使用拓扑保持掩码注入和边界门控调制的生成器合成真实裂缝图像；2) 类别条件流匹配模型生成可控覆盖率的裂缝掩码；3) 将裂缝掩码注入无裂缝背景以增强光照和表面伪影多样性；4) 在五个基准数据集上验证方法。

Result: 1) 结合真实与合成数据，域内性能平均提升5.37 mIoU和5.13 F1；2) 目标引导跨域合成仅用有限目标掩码统计即实现平均13.12 mIoU和14.82 F1增益；3) 相比扩散方法，提供更快确定性采样并改善薄结构几何的保真度与对齐；4) 发布包含5万对数据的CSF-50K公共数据集。

Conclusion: CrackSegFlow通过可控合成有效缓解裂缝分割的数据稀缺和域偏移问题，在多个基准上显著提升性能，其高效采样和高质量对齐特性优于现有方法，发布的数据集将推动可泛化裂缝分割研究。
...
### [74] [MGPC: Multimodal Network for Generalizable Point Cloud Completion With Modality Dropout and Progressive Decoding](https://arxiv.org/abs/2601.03660)
*Jiangyuan Liu,Hongxuan Ma,Yuhao Zhao,Zhe Liu,Jian Wang,Wei Zou*

Main category: cs.CV

TL;DR: 提出MGPC框架，通过多模态融合（点云、RGB图像、文本）和渐进生成器，提升点云补全的泛化能力，并构建大规模数据集MGPC-1M验证效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法（如3D CNN、点云网络、Transformer）在合成数据上表现良好，但由于模态单一、可扩展性有限和生成能力不足，难以泛化到新物体和真实场景。

Method: 1. 提出模态丢弃策略增强鲁棒性；2. 设计基于Transformer的融合模块整合多模态信息；3. 引入渐进生成器提升几何建模能力；4. 构建自动数据生成流程和大规模数据集MGPC-1M（含1000+类别、100万训练对）。

Result: 在MGPC-1M和真实场景数据上的实验表明，该方法显著优于现有基线，在真实条件下表现出强泛化能力。

Conclusion: MGPC通过多模态统一架构和渐进生成机制，有效解决了点云补全的泛化难题，为真实场景应用提供了可行方案。
...
### [75] [PhysVideoGenerator: Towards Physically Aware Video Generation via Latent Physics Guidance](https://arxiv.org/abs/2601.03665)
...
Method: 设计轻量级预测网络PredictorP，从扩散潜变量回归预训练V-JEPA 2提取的高级物理特征，通过跨注意力机制注入DiT生成器的时间注意力层。

Result: 验证了联合训练范式的技术可行性：扩散潜变量包含足够信息恢复V-JEPA 2物理表示，多任务优化在训练中保持稳定。

Conclusion: 建立了物理感知生成模型的大规模评估基础，为未来研究提供了架构设计、技术挑战和训练稳定性验证的参考。
...
### [76] [TRec: Egocentric Action Recognition using 2D Point Tracks](https://arxiv.org/abs/2601.03667)
*Dennis Holzmann,Sven Wachsmuth*

Main category: cs.CV

TL;DR: 本文提出了一种利用2D点轨迹作为额外运动线索的自中心动作识别新方法，通过跟踪随机采样的图像点来提升识别准确率。


<details>
  <summary>Details</summary>
Motivation: 现有自中心动作识别方法主要依赖RGB外观、人体姿态估计或其组合，缺乏对运动信息的有效利用，因此探索更轻量且有效的运动表示方法。

Method: 使用CoTracker在视频中跟踪随机初始化的点，将得到的轨迹与对应图像帧作为输入，采用基于Transformer的识别模型进行训练和预测。

Result: 实验表明，即使仅使用初始帧及其点轨迹（无需完整视频序列），该方法也能显著提升识别准确率，优于未使用运动信息的相同模型。

Conclusion: 2D点轨迹作为一种轻量级运动表示，能有效增强自中心动作理解，为动作识别提供了新的补充线索。
...
### [77] [MATANet: A Multi-context Attention and Taxonomy-Aware Network for Fine-Grained Underwater Recognition of Marine Species](https://arxiv.org/abs/2601.03729)
*Donghwan Lee,Byeongjin Kim,Geunhee Kim,Hyukjin Kwon,Nahyeon Maeng,Wooju Kim*

Main category: cs.CV

TL;DR: 提出MATANet模型，通过多上下文环境注意力模块和层次分离诱导学习模块，结合环境上下文和生物分类学层次结构，实现细粒度海洋物种分类的先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度海洋生物分类方法常忽略环境上下文交互和生物分类学层次结构，而生态学研究和生物多样性保护需要更准确的分类模型。

Method: 设计MATANet模型，包含多上下文环境注意力模块（学习感兴趣区域与环境的关联）和层次分离诱导学习模块（将分类学层次编码到特征空间），结合实例特征、环境特征和分类结构。

Result: 在FathomNet2025、FAIR1M和LifeCLEF2015-Fish数据集上取得最先进的分类性能。

Conclusion: MATANet通过模拟专家策略（利用分类学知识和环境上下文），有效提升了细粒度海洋物种分类的准确性，为生态研究和保护提供技术支持。
...
### [78] [BREATH-VL: Vision-Language-Guided 6-DoF Bronchoscopy Localization via Semantic-Geometric Fusion](https://arxiv.org/abs/2601.03713)
*Qingyao Tian,Bingyu Yang,Huai Liao,Xinyan Huang,Junyong Li,Dong Yi,Hongbin Liu*

Main category: cs.CV

TL;DR: 本文提出了BREATH-VL框架，通过结合视觉语言模型的语义理解与视觉配准方法的几何信息，解决了内窥镜6-DoF相机定位中的数据集缺乏、姿态回归精度不足和计算延迟高的问题，并在复杂气道场景中实现了优于现有方法的定位精度。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在导航任务中表现出色，但应用于内窥镜6-DoF相机定位面临三大挑战：缺乏大规模高质量医学定位数据集、细粒度姿态回归能力有限、时序特征提取计算延迟高。

Method: 1) 构建了迄今最大的体内内窥镜定位数据集BREATH；2) 提出BREATH-VL混合框架，融合VLM语义线索与视觉配准几何信息；3) 引入轻量级上下文学习机制，将运动历史编码为语言提示以实现高效时序推理。

Result: 实验表明：视觉语言模块在复杂手术场景中实现鲁棒语义定位；BREATH-VL在精度和泛化性上优于最先进的视觉定位方法，平移误差降低25.5%，同时保持有竞争力的计算延迟。

Conclusion: 语义与几何信息的结合能显著提升内窥镜定位性能，轻量级时序编码机制可高效利用历史信息，为手术导航等医疗应用提供了更准确的6-DoF姿态估计方案。
...
### [79] [CSMCIR: CoT-Enhanced Symmetric Alignment with Memory Bank for Composed Image Retrieval](https://arxiv.org/abs/2601.03728)
*Zhipeng Qian,Zihan Liang,Yufei Ma,Ben Chen,Huangyu Dai,Yiwei Ma,Jiayi Ji,Chenyi Lei,Han Li,Xiaoshuai Sun*

Main category: cs.CV

TL;DR: 提出CSMCIR框架解决组合图像检索中的表示空间碎片化问题，通过多级思维链提示、对称双塔架构和动态记忆库实现查询-目标对齐，在多个基准数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索方法存在表示空间碎片化问题：查询（图像+文本）和目标图像由不同编码器处理，导致特征空间存在三个分离的聚类，迫使模型通过事后对齐来桥接未对齐的表示空间，这从根本上限制了检索性能。

Method: 1. 多级思维链提示策略：引导多模态大语言模型为目标图像生成具有区分性且语义兼容的描述，建立模态对称性；2. 对称双塔架构：查询端和目标端使用相同的共享参数Q-Former进行跨模态编码，确保特征表示一致性；3. 基于熵的时序动态记忆库策略：提供高质量负样本，并与模型演化状态保持一致。

Result: 在四个基准数据集上的广泛实验表明，CSMCIR实现了最先进的性能，并具有优越的训练效率。全面的消融研究进一步验证了每个提出组件的有效性。

Conclusion: CSMCIR通过统一的表示框架解决了组合图像检索中的表示空间碎片化问题，实现了高效的查询-目标对齐。该方法通过模态对称性、架构对称性和动态负样本策略的协同作用，显著提升了检索性能，为跨模态检索提供了新的解决方案。
...
### [80] [HyperCOD: The First Challenging Benchmark and Baseline for Hyperspectral Camouflaged Object Detection](https://arxiv.org/abs/2601.03736)
*Shuyan Bai,Tingfa Xu,Peifu Liu,Yuhao Qiu,Huiyan Bai,Huan Chen,Yanyan Peng,Jianan Li*

Main category: cs.CV

TL;DR: 本文提出了首个用于高光谱伪装目标检测（HCOD）的大规模基准数据集HyperCOD，并开发了一种基于Segment Anything Model（SAM）的适配方法HSC-SAM，通过解耦空间与光谱信息有效提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于RGB图像的伪装目标检测方法在颜色和纹理模糊的真实场景中效果有限，而高光谱图像虽能提供精细光谱特征，却缺乏专用的大规模基准数据集，阻碍了HCOD领域的发展。

Method: 构建包含350张高分辨率高光谱图像的HyperCOD基准数据集；提出HSC-SAM方法，将高光谱图像解耦为空间图（输入SAM编码器）和光谱显著性图（作为自适应提示），以弥合模态差异。

Result: HSC-SAM在HyperCOD数据集上取得了最先进的性能，并在其他公开高光谱数据集上展现出强大的泛化能力。

Conclusion: HyperCOD数据集和HSC-SAM基线为高光谱伪装目标检测研究提供了坚实基础，有望推动该新兴领域的未来发展。
...
### [81] [RadDiff: Describing Differences in Radiology Image Sets with Natural Language](https://arxiv.org/abs/2601.03733)
*Xiaoxian Shen,Yuhui Zhang,Sahithi Ankireddy,Xiaohan Wang,Maya Varma,Henry Guo,Curtis Langlotz,Serena Yeung-Levy*

Main category: cs.CV

TL;DR: 提出了RadDiff，一种多模态智能系统，用于执行放射科医生风格的比较推理，以描述成对放射学研究之间的临床有意义差异。该系统在VisDiff的基础上进行了四项创新，并在新构建的RadDiffBench基准测试中显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 理解两组放射学图像之间的差异对于生成临床见解和解释医学AI系统至关重要，但目前缺乏系统的方法和基准来揭示放射学数据中的有意义差异。

Method: 基于VisDiff的提议者-排序者框架，引入了四项创新：通过领域适应的视觉语言模型注入医学知识；整合图像与临床报告的多模态推理；跨多轮推理的迭代假设细化；以及针对显著区域的定位和放大视觉搜索。

Result: 在RadDiffBench基准测试中，RadDiff达到了47%的准确率，在使用真实报告指导时达到50%的准确率，显著优于通用领域的VisDiff基线。系统在COVID-19表型比较、种族亚组分析和生存相关成像特征发现等临床任务中表现出多样性。

Conclusion: RadDiff和RadDiffBench为系统性地揭示放射学数据中的有意义差异提供了首个方法和基准基础，展示了其在多样化临床任务中的潜力和实用性。
...
### [82] [I2E: From Image Pixels to Actionable Interactive Environments for Text-Guided Image Editing](https://arxiv.org/abs/2601.03741)
...
TL;DR: 本文提出I2E框架，通过'分解-执行'范式将图像编辑转化为结构化环境中的可操作交互过程，显著提升了复杂组合编辑任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本引导的图像编辑方法主要依赖端到端的像素级修复范式，在需要精确局部控制和复杂多对象空间推理的组合编辑任务中存在明显不足，具体受限于：1）规划与执行的隐式耦合；2）缺乏对象级控制粒度；3）依赖非结构化、以像素为中心的建模。

Method: 提出I2E框架，包含：1）分解器将非结构化图像转换为离散、可操作的对象层；2）物理感知的视觉-语言-动作代理，通过思维链推理将复杂指令解析为原子动作序列。同时构建了I2E-Bench基准测试集。

Result: 在I2E-Bench和多个公开基准测试上的实验结果表明，I2E在复杂组合指令处理、物理合理性保持和多轮编辑稳定性方面显著优于现有最先进方法。

Conclusion: I2E通过结构化分解和动作化交互的新范式，有效解决了传统像素级编辑方法在组合编辑任务中的局限性，为复杂图像编辑提供了更精确可控的解决方案。
...
### [83] [MVP: Enhancing Video Large Language Models via Self-supervised Masked Video Prediction](https://arxiv.org/abs/2601.03781)
*Xiaokun Sun,Zezhong Wu,Zewen Ding,Linli Xu*

Main category: cs.CV

TL;DR: 提出一种名为MVP（Masked Video Prediction）的后训练目标，通过重构被遮蔽的连续视频片段来增强视频大语言模型的时序推理和因果理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的视频大语言模型后训练方法主要关注整体内容理解，缺乏对时序连贯性和帧间相关性的显式监督，限制了模型捕捉复杂动态和细粒度视觉因果关系的能力。

Method: 1. 提出MVP目标：要求模型从一组具有挑战性的干扰项中重构被遮蔽的连续视频片段；2. 开发可扩展的数据合成流程，将任意视频语料转化为MVP训练样本；3. 采用Group Relative Policy Optimization（GRPO）和细粒度奖励函数来增强模型对视频上下文和时序属性的理解。

Result: 综合评估表明，MVP通过直接强化时序推理和因果理解，有效提升了视频推理能力。

Conclusion: MVP作为一种新颖的后训练目标，能够显式地弥补现有方法在时序连贯性和帧间相关性监督方面的不足，从而增强视频大语言模型对复杂动态和视觉因果关系的理解。
...
### [84] [A Comparative Study of 3D Model Acquisition Methods for Synthetic Data Generation of Agricultural Products](https://arxiv.org/abs/2601.03784)
*Steven Moonen,Rob Salaets,Kenneth Batstone,Abdellatif Bey-Temsamani,Nick Michiels*

Main category: cs.CV

TL;DR: 本文探讨了在农业领域缺乏CAD模型时，如何通过替代方法生成合成数据来训练AI物体检测模型，以区分石头和土豆，并证明了结合少量真实数据微调可显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 农业领域缺乏现成的CAD模型，难以利用合成数据训练AI视觉系统，而真实数据采集和标注成本高昂，尤其在低产量、高变异性的制造环境中。

Method: 提出了替代CAD文件的多种技术，包括扫描获取高代表性3D模型和使用图像转3D方法生成合成数据集，用于训练物体检测模型，并通过在少量真实数据上微调来优化模型。

Result: 使用高代表性3D模型生成的合成数据能有效训练物体检测模型；结合少量真实数据微调可显著提升模型性能，即使使用代表性较低的模型也能达到类似效果。

Conclusion: 在农业等缺乏CAD模型的领域，可通过替代方法生成合成数据来训练AI模型，结合少量真实数据微调能有效提升性能，为低成本、高效率的视觉系统部署提供了可行方案。
...
### [85] [EvalBlocks: A Modular Pipeline for Rapidly Evaluating Foundation Models in Medical Imaging](https://arxiv.org/abs/2601.03811)
...
TL;DR: 提出EvalBlocks框架，用于医学影像基础模型开发中的高效评估，支持模块化集成和可重复实验，提升研究效率。


<details>
  <summary>Details</summary>
Motivation: 医学影像基础模型开发中，研究人员需手动跟踪大量实验和设计选择，现有流程效率低且易出错，缺乏系统化评估工具。

Method: 基于Snakemake构建模块化框架，支持数据集、基础模型、聚合方法和评估策略的即插即用集成，采用中心化跟踪、缓存和并行执行技术。

Result: 在五个前沿基础模型和三个医学影像分类任务上验证，EvalBlocks能简化评估流程，实现单命令可重复实验，并支持共享计算资源的扩展使用。

Conclusion: EvalBlocks通过自动化评估流程，帮助研究人员更快迭代模型，专注于创新而非评估管理，框架已开源发布。
...
### [86] [From Brute Force to Semantic Insight: Performance-Guided Data Transformation Design with LLMs](https://arxiv.org/abs/2601.03808)
*Usha Shrestha,Dmitry Ignatov,Radu Timofte*

Main category: cs.CV

TL;DR: 提出了一种基于性能感知的闭环解决方案，使大语言模型能够通过内部化经验性能线索来自主设计最优的代码增强转换，无需强化学习或奖励模型。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在代码合成方面表现突出，但数据感知增强仍受限于启发式设计或暴力搜索方法，需要更高效、自主的优化方案。

Method: 在NNGPT生态系统中，使用低秩适应（LoRA）对LLM进行微调，训练数据包含6000多个经过经验评估的PyTorch增强函数，每个函数仅标注下游模型准确率。采用成对性能排序（更好-更差转换）进行训练，通过经验反馈实现对齐。

Result: 该方法相比暴力搜索减少了高达600倍的候选评估次数，同时保持竞争力的峰值准确率，并将生成过程从随机合成转向任务对齐设计。消融研究表明，直接提示比结构化思维链提示更稳定，模型内部化的是语义性能线索而非语法记忆。

Conclusion: 大语言模型可以通过非文本反馈循环展现任务级推理能力，绕过显式符号奖励，实现自主的代码增强优化。
...
### [87] [IDESplat: Iterative Depth Probability Estimation for Generalizable 3D Gaussian Splatting](https://arxiv.org/abs/2601.03824)
*Wei Long,Haifeng Wu,Shiyin Jiang,Jinhua Zhang,Xinchun Ji,Shuhang Gu*

Main category: cs.CV

TL;DR: 提出IDESplat方法，通过迭代深度概率提升实现准确的3D高斯均值预测，在多个数据集上达到最先进的实时重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有通用3D高斯溅射方法依赖单次warp估计深度概率，无法充分利用跨视图几何线索，导致深度图不稳定且粗糙，影响高斯均值预测精度。

Method: 引入深度概率提升单元（DPBU），通过级联warp操作生成极线注意力图进行乘法融合；堆叠多个DPBU构建迭代深度估计流程，逐步筛选高概率深度候选并精化深度图。

Result: 在RealEstate10K上PSNR超越DepthSplat 0.33 dB，参数量仅10.7%，内存使用70%；跨数据集实验中在DTU上PSNR提升2.95 dB，展现强泛化能力。

Conclusion: IDESplat通过迭代深度概率提升显著改善高斯均值预测，实现高质量实时场景重建，在重建质量、效率和泛化性方面均优于现有方法。
...
### [88] [HemBLIP: A Vision-Language Model for Interpretable Leukemia Cell Morphology Analysis](https://arxiv.org/abs/2601.03915)
*Julie van Logtestijn,Petru Manescu*

Main category: cs.CV

TL;DR: 提出HemBLIP视觉语言模型，用于生成外周血细胞的可解释形态描述，在白血病诊断中实现更高准确性和透明度。


<details>
  <summary>Details</summary>
Motivation: 当前白血病诊断中的深度学习模型多为黑箱，缺乏可解释性，限制了临床信任和应用。

Method: 构建包含1.4万个健康与白血病细胞及专家标注的数据集，通过全微调和LoRA参数高效训练方法适配通用VLM，并与MedGEMMA生物医学基础模型对比。

Result: HemBLIP在描述质量和形态准确性上表现更优，LoRA适配在显著降低计算成本的同时带来进一步性能提升。

Conclusion: 视觉语言模型在血液学诊断中具有实现透明化和可扩展性的潜力。
...
### [89] [FLNet: Flood-Induced Agriculture Damage Assessment using Super Resolution of Satellite Images](https://arxiv.org/abs/2601.03884)
*Sanidhya Ghosal,Anurag Sharma,Sushil Ghildiyal,Mukesh Saini*

Main category: cs.CV

TL;DR: 提出FLNet深度学习架构，通过超分辨率提升卫星图像分辨率至3米，改进洪水后农作物损害评估精度，在BFCD-22数据集上验证有效。


<details>
  <summary>Details</summary>
Motivation: 印度洪水频发导致农作物受损，传统人工调查效率低且有偏差，现有卫星方法受云层和低空间分辨率限制，需更快速准确的自动化评估方案。

Method: 设计FLNet深度学习模型，先对Sentinel-2卫星图像进行超分辨率处理（10米→3米），再进行损害分类，使用BFCD-22数据集进行测试验证。

Result: 模型将'完全损害'类别的F1分数从0.83提升至0.89，接近商用高分辨率影像的0.89水平，显著提高评估准确性。

Conclusion: 该方法提供了经济高效、可扩展的自动化损害评估方案，为全国范围从人工转向高保真自动评估奠定基础。
...
### [90] [FocusUI: Efficient UI Grounding via Position-Preserving Visual Token Selection](https://arxiv.org/abs/2601.03928)
*Mingyu Ouyang,Kevin Qinghong Lin,Mike Zheng Shou,Hwee Tou Ng*

Main category: cs.CV

TL;DR: 提出FocusUI框架，通过选择性保留与指令相关的视觉补丁并保持位置连续性，显著降低UI grounding任务的计算开销和内存占用，同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型处理高分辨率UI截图时产生大量视觉token，导致计算开销大、注意力稀释；而人类交互时通常只关注感兴趣区域，因此需要高效且精确的UI grounding方法。

Method: 1. 构建补丁级监督：融合指令条件评分与基于UI图的规则评分，筛选关键视觉token；2. 提出PosPad策略：将连续丢弃的视觉token压缩为特殊标记并置于序列末尾，保持位置连续性。

Result: 在四个基准测试中超越GUI专用基线；ScreenSpot-Pro上，FocusUI-7B性能较GUI-Actor-7B提升3.7%；仅保留30%视觉token时精度仅下降3.2%，推理速度提升1.44倍，GPU峰值内存降低17%。

Conclusion: FocusUI通过智能视觉token选择和位置连续性保持，实现了高效、精确的UI grounding，为实际应用提供了计算效率与性能的平衡方案。
...
### [91] [ResTok: Learning Hierarchical Residuals in 1D Visual Tokenizers for Autoregressive Image Generation](https://arxiv.org/abs/2601.03955)
*Xu Zhang,Cheng Da,Huan Yang,Kun Gai,Ming Lu,Zhan Ma*

Main category: cs.CV

TL;DR: 提出ResTok视觉分词器，通过构建层次化残差表示改进自回归图像生成，显著提升生成质量和效率


<details>
  <summary>Details</summary>
Motivation: 现有1D视觉分词器直接套用语言建模设计，忽略了视觉数据的层次性和残差网络设计的重要性，导致表示能力受限

Method: 提出ResTok分词器，构建图像token和潜在token的层次化残差表示；设计层次化自回归生成器，通过一次预测整个层级的token来减少采样步骤

Result: 在ImageNet-256上达到gFID 2.34，仅需9个采样步骤，显著优于现有方法

Conclusion: 恢复视觉tokenization中的层次化残差先验能显著提升自回归图像生成性能，证明视觉特定设计的重要性
...
### [92] [Mind the Generative Details: Direct Localized Detail Preference Optimization for Video Diffusion Models](https://arxiv.org/abs/2601.04068)
*Zitong Huang,Kaidong Zhang,Yukang Ding,Chao Gao,Rui Ding,Ying Chen,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 提出LocalDPO框架，通过局部时空区域偏好对优化文本到视频扩散模型，提升视频生成质量与对齐效率。


<details>
  <summary>Details</summary>
Motivation: 现有DPO方法依赖多样本排序和任务特定评判模型，效率低且监督信号模糊，需更高效细粒度的视频生成器对齐方法。

Method: 设计自动化流程从真实视频构建局部偏好对，通过时空掩码腐蚀生成负样本，使用冻结基模型恢复掩码区域，并引入区域感知DPO损失进行训练。

Result: 在Wan2.1和CogVideoX数据集上，LocalDPO在视频保真度、时序一致性和人类偏好评分上优于其他后训练方法。

Conclusion: LocalDPO建立了更高效、细粒度的视频生成器对齐范式，无需外部评判模型或人工标注即可有效提升生成质量。
...
### [93] [FUSION: Full-Body Unified Motion Prior for Body and Hands via Diffusion](https://arxiv.org/abs/2601.03959)
*Enes Duran,Nikos Athanasiou,Muhammed Kocabas,Michael J. Black,Omid Taheri*

Main category: cs.CV

TL;DR: 提出首个基于扩散模型的无条件全身运动先验FUSION，联合建模身体和手部运动，在标准基准测试中表现优异，并展示了在物体交互和自交互任务中的应用。


<details>
  <summary>Details</summary>
Motivation: 现有运动合成方法存在局限：要么忽略手部运动，要么只能在高度受限的场景下生成全身运动。主要障碍是缺乏同时捕捉多样化全身运动和详细手部关节的大规模数据集。

Method: 通过整合现有手部运动数据集与大规模身体运动数据生成全身序列；提出基于扩散模型的运动先验FUSION；针对特定应用开发优化流程，通过精炼扩散模型的潜在空间生成任务特定运动。

Result: FUSION在HumanML3D数据集的关键点跟踪任务中超越现有骨骼控制模型，实现更优的运动自然度；在物体交互和语言驱动的自交互任务中展示了精确的手部运动控制和合理的全身协调。

Conclusion: FUSION能够生成包含手指细节的全身运动，为运动合成提供了更全面的解决方案，并通过优化流程展示了在特定任务中的应用潜力。
...
### [94] [PosterVerse: A Full-Workflow Framework for Commercial-Grade Poster Generation with HTML-Based Scalable Typography](https://arxiv.org/abs/2601.03993)
*Junle Liu,Peirong Zhang,Yuyi Zhang,Pengyu Yan,Hui Zhou,Xinyue Zhou,Fengjun Guo,Lianwen Jin*

Main category: cs.CV

TL;DR: 提出PosterVerse全流程商业海报生成方法，通过LLM蓝图创建、扩散模型背景生成和MLLM驱动的HTML引擎实现高精度文本渲染，并发布首个中文HTML排版数据集PosterDNA。


<details>
  <summary>Details</summary>
Motivation: 现有自动化海报生成系统存在设计流程不完整、文本渲染精度低、商业应用灵活性不足等问题，难以满足商业级海报对美学与信息传达的双重要求。

Method: 1) 微调LLM从用户需求提取设计要素生成蓝图；2) 定制扩散模型生成视觉背景；3) MLLM驱动的HTML引擎统一布局与文本渲染；4) 构建HTML格式中文海报数据集PosterDNA。

Result: PosterVerse能稳定生成具有视觉吸引力、文本对齐精确、布局可定制的商业级海报，从根本上解决了小字号高密度文本的渲染难题。

Conclusion: PosterVerse为商业海报设计自动化提供了可行解决方案，其HTML基础的数据集和渲染方法显著提升了文本处理的精度与扩展性。
...
### [95] [Analyzing Reasoning Consistency in Large Multimodal Models under Cross-Modal Conflicts](https://arxiv.org/abs/2601.04073)
*Zhihao Zhu,Jiafeng Liang,Shixin Jiang,Jinlan Fu,Ming Liu,Guanglu Sun,See-Kiong Ng,Bing Qin*

Main category: cs.CV

TL;DR: 本文发现大型多模态模型在视频推理中存在文本惯性问题，即一旦思考过程中出现文本幻觉，模型会盲目遵循错误文本而忽视冲突的视觉证据。作者提出了一种训练无关的推理范式来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在视频推理中表现出强大的链式思考能力，但其推理链的鲁棒性存在疑问。研究发现模型容易陷入文本惯性，即错误文本一旦出现就会持续传播，而模型缺乏自我纠正能力。

Method: 提出逻辑图扰动协议，系统地在不同架构的LMM推理链中注入扰动以评估其自我反思能力；并引入主动视觉上下文精炼方法，这是一种无需训练的推理范式，通过主动视觉重定位机制进行细粒度验证，并结合自适应上下文精炼策略来总结和去噪推理历史。

Result: 实验表明，现有模型在出现文本幻觉后成功自我纠正的比例低于10%，主要表现出盲目的文本错误传播。而提出的方法能显著抑制幻觉传播，提升推理鲁棒性。

Conclusion: 大型多模态模型的推理链存在严重的文本惯性问题，需要专门的机制来确保视觉证据的持续验证。提出的主动视觉上下文精炼方法能有效缓解这一问题，为提升多模态推理的可靠性提供了实用方案。
...
### [96] [Thinking with Frames: Generative Video Distortion Evaluation via Frame Reward Model](https://arxiv.org/abs/2601.04033)
*Yuan Wang,Borui Liao,Huijuan Huang,Jinda Lu,Ouxiang Li,Kuien Liu,Meng Wang,Xiang Wang*

Main category: cs.CV

TL;DR: 提出了REACT框架，一种专门评估生成视频中结构失真的帧级奖励模型，通过两阶段训练和动态采样机制提高评估准确性。


<details>
  <summary>Details</summary>
Motivation: 现有视频奖励模型主要关注视觉质量、运动质量和文本对齐，但忽略了异常物体外观和交互等结构失真问题，这些失真会显著降低生成视频的整体质量。

Method: 1) 构建大规模人类偏好数据集，基于提出的结构失真分类法进行标注；2) 使用高效的思维链合成管道生成额外数据；3) 采用两阶段训练框架：先用掩码损失进行监督微调注入领域知识，再用GRPO强化学习和成对奖励增强推理能力；4) 推理时引入动态采样机制聚焦最可能失真的帧。

Result: REACT能够有效补充现有奖励模型，在结构失真评估方面实现准确的定量评估和可解释的归因分析，并在新提出的REACT-Bench基准测试中表现良好。

Conclusion: REACT框架成功解决了生成视频中结构失真的评估问题，通过专门设计的模型架构和训练方法，为视频生成质量评估提供了更全面的工具。
...
### [97] [Unsupervised Modular Adaptive Region Growing and RegionMix Classification for Wind Turbine Segmentation](https://arxiv.org/abs/2601.04065)
*Raül Pérez-Gonzalo,Riccardo Magro,Andreas Espersen,Antonio Agudo*

Main category: cs.CV

TL;DR: 提出一种基于区域分类的注释高效风力涡轮机叶片分割方法，通过无监督区域生成和区域混合增强实现高精度分割和跨站点泛化。


<details>
  <summary>Details</summary>
Motivation: 传统像素级深度学习分割方法需要大量标注数据，难以扩展；风力涡轮机叶片表面微小损伤会影响气动性能和发电效率，需要自动化检测。

Method: 将像素级分割重构为二值区域分类问题；使用无监督的模块化自适应区域生长技术生成图像区域，结合自适应阈值和区域合并；提出RegionMix增强策略通过组合不同区域合成训练样本。

Result: 该方法实现了最先进的分割精度，并在不同风电场间表现出强大的跨站点泛化能力，能稳定分割涡轮机叶片。

Conclusion: 所提出的注释高效分割框架通过区域分类和增强策略，在减少标注需求的同时实现了高精度和强泛化能力，适用于风力涡轮机自动化检测。
...
### [98] [Pixel-Wise Multimodal Contrastive Learning for Remote Sensing Images](https://arxiv.org/abs/2601.04127)
*Leandro Stival,Ricardo da Silva Torres,Helio Pedrini*

Main category: cs.CV

TL;DR: 提出一种新颖的多模态方法，利用像素级二维表示更有效地编码卫星图像时间序列中的视觉属性变化，并通过对比学习提升表示质量。


<details>
  <summary>Details</summary>
Motivation: 卫星持续产生海量数据，但现有深度学习模型多设计用于处理完整图像或时间序列，缺乏对像素级时间动态的有效编码方法。

Method: 1. 从基于像素的植被指数时间序列生成递归图作为原始像素值的替代表示；2. 提出PIMC多模态自监督方法，基于二维像素时间序列表示和遥感影像训练编码器。

Result: 在PASTIS数据集的像素级预测和分类任务，以及EuroSAT数据集的地表覆盖分类任务中，该方法均优于现有先进方法，二维表示显著提升了特征提取能力。

Conclusion: 该多模态方法在各种地球观测任务中表现优异，为处理卫星图像时间序列和遥感影像建立了一个鲁棒的自监督框架。
...
### [99] [Gen3R: 3D Scene Generation Meets Feed-Forward Reconstruction](https://arxiv.org/abs/2601.04090)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在场景级3D生成中未能充分利用基础重建模型的几何先验和视频扩散模型的生成能力，需要一种能紧密耦合重建与生成模型的方法。

Method: 重新利用VGGT重建模型生成几何潜在表示，通过训练适配器使其与预训练视频扩散模型的外观潜在表示对齐，联合生成解耦但对齐的潜在表示。

Result: 在单图和多图条件下的3D场景生成中达到最先进效果，能生成相机位姿、深度图和全局点云，并通过生成先验增强重建鲁棒性。

Conclusion: 重建模型与生成模型的紧密耦合具有相互促进的益处，Gen3R为场景级3D生成提供了有效框架。
...
### [100] [GeoReason: Aligning Thinking And Answering In Remote Sensing Vision-Language Models Via Logical Consistency Reinforcement Learning](https://arxiv.org/abs/2601.04118)
*Wenshuai Li,Xiantai Xiang,Zixiao Wen,Guangyao Zhou,Ben Niu,Feng Wang,Lijia Huang,Qiantong Wang,Yuxin Hu*

Main category: cs.CV

TL;DR: 本文提出GeoReason框架，通过构建逻辑驱动数据集和两阶段训练策略，解决遥感视觉语言模型中的逻辑幻觉问题，提升空间推理的可靠性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前遥感视觉语言模型在复杂空间任务中存在逻辑幻觉问题，即正确答案可能基于错误推理链或位置捷径而非空间逻辑，这削弱了模型在战略空间决策中的可靠性。

Method: 1. 构建GeoReason-Bench数据集（包含4000条从几何基元和专家知识合成的推理轨迹）；2. 两阶段训练策略：监督知识初始化（学习推理语法和领域知识）和一致性感知强化学习（引入逻辑一致性奖励，通过选项排列策略惩罚逻辑漂移）。

Result: 实验表明，该框架显著提升了遥感视觉语言模型的认知可靠性和可解释性，在性能上优于其他先进方法。

Conclusion: GeoReason框架通过同步内部推理与最终决策，有效解决了逻辑幻觉问题，为遥感视觉语言模型的高层演绎推理提供了可靠解决方案。
...
### [101] [Klear: Unified Multi-Task Audio-Video Joint Generation](https://arxiv.org/abs/2601.04151)
*Jun Wang,Chunyu Qiang,Yuxin Guo,Yiran Wang,Xijuan Zeng,Chen Zhang,Pengfei Wan*

Main category: cs.CV

TL;DR: 本文提出Klear系统，通过改进模型架构、训练策略和数据构建，解决了音视频联合生成中的异步、唇语不对齐和模态退化问题，实现了高质量、对齐良好的音视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有非商业音视频生成方法存在音视频异步、唇语-语音对齐差、模态退化等问题，根源在于音视频对应建模弱、泛化能力有限、缺乏高质量密集标注数据。

Method: 采用单塔架构和统一DiT块，结合Omni-Full Attention机制；使用渐进多任务训练（随机模态掩码、多阶段课程学习）；构建首个大规模密集标注音视频数据集及自动化数据流水线。

Result: Klear在联合和单模态生成中均实现高保真、语义及时序对齐的生成效果，泛化能力强，大幅超越现有方法，性能接近Veo 3。

Conclusion: Klear为下一代音视频合成提供了统一、可扩展的解决方案，通过架构、训练和数据的协同创新，显著提升了生成质量与对齐能力。
...
### [102] [Diffusion-DRF: Differentiable Reward Flow for Video Diffusion Fine-Tuning](https://arxiv.org/abs/2601.04153)
*Yifan Wang,Yanyu Li,Sergey Tulyakov,Yun Fu,Anil Kag*

Main category: cs.CV

TL;DR: 提出Diffusion-DRF方法，使用冻结的视觉语言模型作为免训练评判器，通过可微分奖励流优化视频扩散模型，无需额外奖励模型或偏好数据。


<details>
  <summary>Details</summary>
Motivation: 现有基于DPO的视频生成方法依赖不可微分的偏好信号（人工标注或学习型奖励模型），导致训练标签密集、易产生偏差、易被攻击（奖励黑客攻击）且训练不稳定。

Method: 1. 使用冻结的现成视觉语言模型作为免训练评判器；2. 通过扩散去噪链直接反向传播VLM反馈，将logit级响应转换为token感知梯度；3. 设计自动化、按方面结构化的提示流程获取可靠的多维度VLM反馈；4. 使用梯度检查点技术实现高效更新。

Result: Diffusion-DRF提升了视频质量和语义对齐，同时缓解了奖励黑客攻击和模型崩溃问题，无需额外奖励模型或偏好数据集。该方法具有模型无关性，可推广至其他基于扩散的生成任务。

Conclusion: Diffusion-DRF提供了一种高效、稳定且可扩展的视频扩散模型微调方法，通过可微分奖励流利用现成VLM的反馈，避免了传统偏好优化方法的局限性。
...
### [103] [ImLoc: Revisiting Visual Localization with Image-based Representation](https://arxiv.org/abs/2601.04185)
*Xudong Jiang,Fangjinhua Wang,Silvano Galliani,Christoph Vogel,Marc Pollefeys*

Main category: cs.CV

TL;DR: 提出了一种结合2D图像和估计深度图的视觉定位方法，通过密集匹配和高效实现，在保持易构建维护的同时达到高精度。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位方法中，2D图像方法易构建但几何推理能力有限，3D结构方法精度高但需要集中重建且难以更新，需要一种兼顾精度与实用性的新方法。

Method: 使用2D图像增强估计深度图来捕获几何结构，结合密集匹配器，采用紧凑压缩和GPU加速的LO-RANSAC实现高效存储与计算。

Result: 在多个标准基准测试中达到新的最高精度，在可比地图大小下优于现有内存高效方法。

Conclusion: 该方法通过深度增强的2D表示实现了精度与效率的平衡，为视觉定位提供了更实用的解决方案。
...
### [104] [ToTMNet: FFT-Accelerated Toeplitz Temporal Mixing Network for Lightweight Remote Photoplethysmography](https://arxiv.org/abs/2601.04159)
*Vladimir Frants,Sos Agaian,Karen Panetta*

Main category: cs.CV

TL;DR: 提出ToTMNet，一种轻量级远程光电容积描记（rPPG）架构，使用FFT加速的Toeplitz时间混合层替代注意力机制，在保持高心率估计精度的同时大幅减少参数和计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的rPPG方法虽提升了鲁棒性，但往往计算成本高、参数多，且注意力机制的时间复杂度随序列长度呈二次增长，限制了实际应用。

Method: 设计Toeplitz时间混合层，利用循环嵌入和FFT卷积实现近线性时间复杂度的全局时间建模；结合局部深度时间卷积分支和门控全局Toeplitz混合，构建仅含63k参数的轻量级网络。

Result: 在UBFC-rPPG数据集上达到1.055 bpm MAE和0.996 Pearson相关系数；在合成到真实场景迁移（SCAMPS→UBFC-rPPG）中达到1.582 bpm MAE和0.994 Pearson相关系数；消融实验验证门控机制对域适应的重要性。

Conclusion: Toeplitz结构的时间混合是替代注意力机制的高效实用方案，能在极低参数量下实现强健的心率估计，但当前研究仅基于两个数据集，需进一步验证泛化能力。
...
### [105] [DeepResearch-Slice: Bridging the Retrieval-Utilization Gap via Explicit Text Slicing](https://arxiv.org/abs/2601.03261)
*Shuo Lu,Yinuo Xu,Jianjie Cheng,Lingxiao He,Meng Wang,Jian Liang*

Main category: cs.CL

TL;DR: 提出DeepResearch-Slice框架，通过神经符号方法预测精确文本片段索引进行硬过滤，解决检索-利用间隙问题，显著提升模型在噪声环境中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究代理主要优化检索策略，但存在检索-利用间隙：即使检索到关键证据，模型在噪声环境中因上下文盲区而无法有效利用。

Method: 提出神经符号框架DeepResearch-Slice，通过预测精确文本片段索引进行确定性硬过滤（而非隐式注意力），在推理前显式筛选证据。

Result: 在六个基准测试中取得显著鲁棒性提升，冻结骨干模型上实现73%相对改进（19.1%→33.0%），无需更新推理模型参数即可有效缓解噪声影响。

Conclusion: 开放端研究需要显式的基础机制，硬过滤方法能有效弥合检索与利用间的差距，提升证据利用效率。
...
### [106] [Internal Reasoning vs. External Control: A Thermodynamic Analysis of Sycophancy in Large Language Models](https://arxiv.org/abs/2601.03263)
*Edward Y. Chang*

Main category: cs.CL

TL;DR: 研究发现大语言模型存在奉承倾向，内部推理机制存在结构性局限，而外部约束机制能完全消除该问题，验证了外部结构约束对保证AI安全的必要性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型常表现出奉承行为（优先迎合用户而非正确性），研究者希望探究此问题是否需要外部监管，还是仅通过内部推理即可解决。

Method: 使用对抗性数据集CAP-GSM8K（N=500），在GPT-3.5、GPT-4o和GPT-5.1上比较内部推理机制（思维链）和外部约束机制（RCA）。

Result: 内部推理在弱模型中导致性能崩溃（优先悖论），在先进模型中仍存在11.4%的输出差距；而RCA在所有层级模型中完全消除奉承行为（0.0%）。研究提出热力学层级理论：仅当能力匹配且强大时混合系统能达到共振（最优效率），否则会陷入失调和熵增。

Conclusion: 外部结构约束对于保证AI安全是严格必要的，仅靠内部推理无法解决奉承行为的结构性局限。
...
### [107] [Jailbreak-Zero: A Path to Pareto Optimal Red Teaming for Large Language Models](https://arxiv.org/abs/2601.03265)
*Kai Hu,Abhinav Aggarwal,Mehran Khodabandeh,David Zhang,Eric Hsin,Li Chen,Ankit Jain,Matt Fredrikson,Akash Bharadwaj*

Main category: cs.CL

TL;DR: 提出Jailbreak-Zero方法，通过攻击LLM生成多样化对抗提示并微调，实现策略覆盖、攻击多样性和提示真实性的帕累托最优，显著提升对开源和商业LLM的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全评估方法受限于示例驱动，缺乏可扩展性和全面性，需要更有效的策略框架来识别和缓解LLM安全漏洞。

Method: 使用攻击LLM生成大量多样化对抗提示，构建偏好数据集微调攻击模型，实现策略覆盖、攻击多样性和提示真实性的平衡优化。

Result: 相比现有技术，Jailbreak-Zero对GPT-40、Claude 3.5等模型攻击成功率显著更高，生成人类可读的有效对抗提示，且需极少人工干预。

Conclusion: Jailbreak-Zero提供了更可扩展、全面的LLM安全评估方案，能有效识别安全漏洞，为LLM安全防护提供新范式。
...
### [108] [Benchmarking and Adapting On-Device Large Language Models for Clinical Decision Support](https://arxiv.org/abs/2601.03266)
*Alif Munim,Jun Ma,Omar Ibrahim,Alhusain Abdalla,Shuolin Yin,Leo Chen,Bo Wang*

Main category: cs.CL

TL;DR: 本研究评估了两种本地部署的大语言模型（gpt-oss-20b和gpt-oss-120b）在临床任务中的表现，发现其性能可媲美或超越部分先进模型，且微调后能显著提升诊断准确性，展示了本地化LLMs在临床决策支持中的潜力。


<details>
  <summary>Details</summary>
Motivation: 专有临床决策系统存在隐私风险和云端依赖问题，而开源模型通常体积较大，难以在资源受限的临床环境中部署。因此，需要探索高效、隐私安全且可本地运行的替代方案。

Method: 在三个临床任务（通用疾病诊断、眼科专科诊断与管理、专家评估模拟）上对两种本地化LLMs进行基准测试，与专有模型（GPT-5、o4-mini）及开源模型（DeepSeek-R1）对比，并对gpt-oss-20b进行通用诊断数据的微调以评估其适应性。

Result: gpt-oss模型在多项任务中表现与DeepSeek-R1和o4-mini相当或更优，尽管模型体积显著更小；微调后的gpt-oss-20b诊断准确性大幅提升，接近GPT-5水平。

Conclusion: 本地化LLMs能够提供准确、适应性强且保护隐私的临床决策支持，为LLMs更广泛地融入常规临床实践提供了可行路径。
...
### [109] [OpenAI GPT-5 System Card](https://arxiv.org/abs/2601.03267)
...
TL;DR: OpenAI发布GPT-5系统卡片，介绍其包含智能快速模型、深度推理模型和实时路由器的统一架构，在性能、安全性和实用性方面均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 为提升AI系统的实用性、响应速度和安全性，同时解决幻觉、指令遵循和谄媚行为等问题，并满足写作、编程和健康等常见应用场景的需求。

Method: 采用统一系统架构，包含主模型、深度推理模型和基于实时信号持续训练的智能路由器；实施安全完成训练以防止违规内容；对高能力领域采取预防性安全措施。

Result: GPT-5在基准测试中超越前代模型，响应更快，减少幻觉，改进指令遵循，降低谄媚行为，在写作、编程和健康领域表现提升，并具备增强的安全防护。

Conclusion: GPT-5通过多模型架构和智能路由实现了性能与实用性的平衡，在提升能力的同时加强了安全防护，特别是对生物化学领域采取预防性安全措施。
...
### [110] [WRAVAL -- WRiting Assist eVALuation](https://arxiv.org/abs/2601.03268)
*Gabriel Benedict,Matthew Butler,Naved Merchant,Eetu Salama-Laine*

Main category: cs.CL

TL;DR: 本文提出针对小语言模型（SLMs）在非推理任务（如语气修改）的评估框架，挑战当前以推理能力为主导的大模型评估标准，为工业应用提供实用工具。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估过度关注推理能力，导致参数量小于100亿的小语言模型在评测中得分远低于大模型，但实际工业应用（如语气修改）中SLMs表现可能被低估，需要更贴合实际场景的评估方法。

Method: 提出包含数据生成、提示调优和大模型辅助评估的框架，针对缺乏标准数据集的非推理任务设计评测流程，并通过任务特定微调验证SLMs潜力。

Result: 开发了开源评估工具WRAVAL，证明SLMs在语气修改等工业任务中具有实用价值，尤其在边缘计算和隐私计算场景下能为从业者提供有效的模型选型基准。

Conclusion: 语言模型评估需结合具体应用场景，SLMs在非推理任务中表现可媲美大模型，本文框架为实际部署提供了更全面的评估视角。
...
### [111] [The Instruction Gap: LLMs get lost in Following Instruction](https://arxiv.org/abs/2601.03269)
*Vishesh Tripathi,Uday Allu,Biddwan Ahmed*

Main category: cs.CL

TL;DR: 评估13个主流大语言模型在企业RAG场景中的指令遵循能力，发现模型间存在显著差异，Claude-Sonnet-4和GPT-5表现最佳，揭示了企业部署中的'指令鸿沟'问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言处理方面表现出色，但在企业环境中部署时，对定制指令的遵循存在不一致性，这成为实际应用的关键限制。

Method: 采用系统化测试方法，使用样本和企业级评估协议，在真实RAG场景中评估13个领先LLM的指令遵循、响应准确性和性能指标。

Result: 不同模型在指令遵循方面差异显著，Claude-Sonnet-4和GPT-5表现最优；研究发现模型擅长通用任务，但难以满足企业部署所需的精确指令遵循要求。

Conclusion: 研究揭示了'指令鸿沟'这一根本挑战，为企业部署LLM解决方案提供了实用见解，并为各主要模型系列的指令遵循能力建立了基准。
...
### [112] [Less is more: Not all samples are effective for evaluation](https://arxiv.org/abs/2601.03272)
*Wentang Song,Jinqiang Li,Kele Huang,Junhui Lin,Shengxiang Wu,Zhongshi Xie*

Main category: cs.CL

TL;DR: 提出无需历史性能数据的测试集压缩框架，通过领域适应嵌入和任务感知聚类，在3GPP通信基准上实现90%以上成本压缩并保持高保真度。


<details>
  <summary>Details</summary>
Motivation: 现有垂直领域LLM评估基准存在语义冗余和高计算成本问题，且传统压缩方法依赖历史模型正确性标签，无法适用于新任务/领域/模型的冷启动场景。

Method: 1. 使用少量领域数据微调基础LLM以内化任务语义；2. 仅基于原始文本生成高层语义嵌入；3. 在领域适应嵌入空间进行任务感知聚类；4. 提出数据集X-ray机制，通过分析聚类几何动态校准压缩强度。

Result: 在3GPP通信等专业领域基准上，该方法能有效识别并移除冗余样本，评估成本降低超过90%，同时保持与完整基准的高度一致性。

Conclusion: 该历史无关的压缩框架解决了冷启动场景下的基准压缩问题，为高效LLM评估提供了实用解决方案。
...
### [113] [Advances and Challenges in Semantic Textual Similarity: A Comprehensive Survey](https://arxiv.org/abs/2601.03270)
*Lokendra Kumar,Neelesh S. Upadhye,Kannan Piedy*

Main category: cs.CL

TL;DR: 本文综述了2021年以来语义文本相似性（STS）领域的研究进展，涵盖六大关键技术方向，总结了当前方法、应用及挑战，旨在指导研究者和从业者把握发展趋势。


<details>
  <summary>Details</summary>
Motivation: STS研究自2021年以来快速发展，受Transformer架构、对比学习等领域技术进步推动，但缺乏系统性梳理。本文旨在整合最新进展，为领域发展提供结构化参考。

Method: 采用文献综述方法，系统分析六大技术方向：基于Transformer的模型、对比学习方法、领域定制方案、多模态方法、图神经网络方法、知识增强技术，并结合代表性模型（如FarSSiBERT、AspectCSE等）进行归纳。

Result: 总结了各技术方向的代表性成果：Transformer模型（如DeBERTa-v3）显著提升精度；对比学习方法（如AspectCSE）刷新基准性能；领域适配模型（如医疗CXR-BERT、金融Financial-STS）验证了专业化可行性；多模态、图网络与知识融合方法进一步扩展了语义理解能力。

Conclusion: STS领域已形成多元化技术体系，在通用与垂直场景均取得突破，但跨领域泛化、计算效率、可解释性等挑战仍需解决。未来需关注轻量化模型、跨模态对齐、低资源适应等方向，推动技术落地与生态发展。
...
### [114] [LLM_annotate: A Python package for annotating and analyzing fiction characters](https://arxiv.org/abs/2601.03274)
*Hannes Rosenbusch*

Main category: cs.CL

TL;DR: LLM_annotate是一个用于分析小说人物性格的Python包，它利用大语言模型标准化了人物行为标注、性格推断和质量验证的工作流程，并提供了可交互的GUI界面。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏标准化工具来分析虚构作品中的人物性格，研究人员需要一种可重复、高效且支持多种大语言模型的方法来进行人物分析。

Method: 该包包含文本分块、基于LLM的标注、人物名称消歧、质量评分、人物级统计和嵌入计算等功能，支持商业、开源或自定义的LLM，并通过人机交互GUI进行质量验证。

Result: 通过《辛普森一家》电影和《傲慢与偏见》小说的示例，展示了该包能够实现高效、可重复的人物分析。

Conclusion: LLM_annotate为研究人员提供了一个灵活、标准化的工具，用于分析虚构作品中的人物性格，支持多种LLM并确保分析过程的可重复性。
...
### [115] [GuardEval: A Multi-Perspective Benchmark for Evaluating Safety, Fairness, and Robustness in LLM Moderators](https://arxiv.org/abs/2601.03273)
*Naseem Machlovi,Maryam Saleki,Ruhul Amin,Mohamed Rahouti,Shawqi Al-Maliki,Junaid Qadir,Mohamed M. Abdallah,Ala Al-Fuqaha*

Main category: cs.CL

TL;DR: 提出了GuardEval基准数据集和GemmaGuard模型，用于提升LLM内容审核的细粒度识别能力，特别是在隐晦冒犯、偏见和越狱提示等复杂场景下。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在内容审核中难以处理主观性强、依赖上下文的复杂案例（如隐晦冒犯、性别/种族偏见、越狱提示），且易受训练数据偏见影响，导致审核结果不一致且存在伦理问题。

Method: 1. 构建GuardEval多视角基准数据集，包含106个细粒度类别，涵盖人类情感、冒犯/仇恨言论、性别/种族偏见及广泛安全议题；2. 基于Gemma3-12B模型，使用QLoRA微调技术训练出GemmaGuard模型，用于细粒度内容审核。

Result: GemmaGuard在评估中取得0.832的宏观F1分数，显著优于OpenAI Moderator（0.64）和Llama Guard（0.61）。实验表明多视角、以人为中心的安全基准能有效减少偏见和不一致的审核决策。

Conclusion: GuardEval和GemmaGuard共同证明，多样化和具代表性的数据能实质性提升LLM在复杂边界案例中的安全性、公平性和鲁棒性，为构建更可靠的AI审核系统提供了新方向。
...
### [116] [Topic Segmentation Using Generative Language Models](https://arxiv.org/abs/2601.03276)
*Pierre Mackenzie,Maya Shah,Patrick Frenett*

Main category: cs.CL

TL;DR: 本文提出了一种使用生成式大语言模型进行主题分割的新方法，通过重叠递归提示策略和句子枚举，相比传统基于语义相似度的方法取得了更好效果。


<details>
  <summary>Details</summary>
Motivation: 现有主题分割方法主要依赖句子间的语义相似度，但这类模型缺乏大语言模型所具有的长距离依赖关系和丰富知识，因此探索LLMs在主题分割中的应用潜力。

Method: 采用重叠递归提示策略，结合句子枚举技术，并支持边界相似度评估指标来衡量分割质量。

Result: 实验结果表明，大语言模型作为主题分割器比现有方法更有效，但在完全依赖其进行主题分割前仍需解决一些问题。

Conclusion: LLMs在主题分割任务中展现出优于传统方法的潜力，但该方法仍需进一步改进和完善才能可靠应用。
...
### [117] [Bare-Metal Tensor Virtualization: Overcoming the Memory Wall in Edge-AI Inference on ARM64](https://arxiv.org/abs/2601.03324)
*Bugra Kilictas,Faruk Alpay*

Main category: cs.CL

TL;DR: 提出了一种针对ARM64架构的软件虚拟张量核心架构，通过直接内存映射和手动优化的NEON SIMD内核，在通用ARM芯片上实现高效LLM推理，突破内存墙限制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在边缘设备部署受限于'内存墙'问题，标准推理运行时存在高层抽象、动态调度和非对齐内存访问等开销，需要一种开源、可移植的解决方案来研究通用ARM芯片上的内存瓶颈。

Method: 采用软件实现的虚拟张量核心架构，包括：1) 绕过标准库容器，使用直接内存映射(mmap)；2) 手动调优的NEON SIMD内核实现软件定义直接内存访问；3) 张量虚拟化布局保证权重矩阵100%缓存行利用率；4) 零拷贝加载器消除初始化延迟。

Result: 在110M参数模型上，M2硬件实现稳定吞吐量>60 tokens/秒，满足200ms心理语言学延迟阈值，无需依赖专有硬件加速器。

Conclusion: 该架构为研究通用ARM硅芯片上的内存瓶颈提供了完全开源、可移植且确定性的参考实现，虽然专有硬件加速器能达到更高峰值吞吐量，但本方案在透明性和可移植性方面具有优势。
...
### [118] [Metaphors are a Source of Cross-Domain Misalignment of Large Reasoning Models](https://arxiv.org/abs/2601.03388)
*Zhibo Hu,Chen Wang,Yanfeng Shu,Hye-young Paik,Liming Zhu*

Main category: cs.CL

TL;DR: 研究发现训练数据中的隐喻与大型语言模型推理内容的不对齐程度存在因果关系，通过监测隐喻激活的潜在特征可有效检测不对齐内容。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明隐喻影响人类决策，而LLMs训练数据包含大量隐喻，因此探究隐喻是否影响LLMs推理路径及其在跨领域不对齐问题中的作用。

Method: 通过预训练、微调和重新对齐阶段的隐喻干预实验，分析隐喻与模型不对齐程度的关系；监测隐喻激活的全局和局部潜在特征，设计不对齐内容检测器。

Result: 隐喻与LLMs推理内容的不对齐程度存在强因果关系；干预隐喻可显著改变模型的跨领域不对齐程度；基于潜在特征设计的检测器能高精度预测不对齐内容。

Conclusion: 隐喻是影响LLMs跨领域不对齐的关键因素，通过监测隐喻相关的潜在特征可实现不对齐内容的有效检测，为理解模型推理机制提供新视角。
...
### [119] [A path to natural language through tokenisation and transformers](https://arxiv.org/abs/2601.03368)
*David S. Berman,Alexander G. Stapleton*

Main category: cs.CL

TL;DR: 本文研究了字节对编码（BPE）如何通过统计变换使语料库的分布逼近Zipf定律，并影响语言模型的信息熵与局部依赖性。


<details>
  <summary>Details</summary>
Motivation: 尽管自然语言普遍存在Zipf定律等统计规律，但现代Transformer模型使用的分词方案（如BPE）如何影响这些统计特性尚不明确。研究旨在揭示BPE在压缩之外的统计变换作用。

Method: 1. 基于Zipf分布假设推导词槽熵的闭式解；2. 实证分析BPE递归应用对词频分布和熵的影响；3. 训练不同BPE深度分词的语料库上的语言模型，比较模型预测熵与Zipf理论预测；4. 使用注意力机制诊断局部依赖性的变化。

Result: 1. BPE递归应用使词频分布趋近Zipf幂律，并诱导经验熵呈现特定增长模式；2. BPE深度增加时，模型预测熵与Zipf理论预测的一致性增强；3. 更深的分词减少了局部词元依赖性，使经验分布更接近弱依赖（近独立同分布）状态。

Conclusion: BPE不仅是一种压缩机制，更是一种统计变换，能够重构自然语言的关键信息特性（如Zipf定律），并简化语言模型的依赖结构。
...
### [120] [Breaking the Assistant Mold: Modeling Behavioral Variation in LLM Based Procedural Character Generation](https://arxiv.org/abs/2601.03396)
*Maan Qraitem,Kate Saenko,Bryan A. Plummer*

Main category: cs.CL

TL;DR: 提出PersonaWeaver框架，通过解耦角色世界构建与行为构建，解决现有角色生成方法中的道德偏见和助手机器人偏见，生成更具多样性和戏剧张力的虚拟角色。


<details>
  <summary>Details</summary>
Motivation: 现有程序化角色生成方法存在两种对齐诱导的偏见：一是正面道德偏见（角色总是采取赞同立场），二是助手机器人偏见（角色总是直接回答问题）。这些偏见导致角色缺乏戏剧张力且可预测，源于最大似然训练和助手机器人微调。

Method: 提出PersonaWeaver框架，将世界构建（角色、人口统计特征）与行为构建（道德立场、互动风格）解耦，通过分离这两个维度来生成更具多样性的角色反应、道德立场以及风格标记（如长度、语气、标点）的第二阶多样性。

Result: PersonaWeaver能够生成具有更丰富反应和道德立场的角色，同时在风格标记上展现出第二阶多样性，解决了现有方法中的对齐偏见问题。

Conclusion: 通过解耦世界构建和行为构建，PersonaWeaver框架有效提升了虚拟角色生成的多样性和戏剧性，为大规模角色生成提供了新思路。
...
### [121] [Rendering Data Unlearnable by Exploiting LLM Alignment Mechanisms](https://arxiv.org/abs/2601.03401)
*Ruihan Zhang,Jun Sun*

Main category: cs.CL

TL;DR: 提出Disclaimer Injection方法，通过在文本中注入精心设计的免责声明，利用大语言模型的对齐机制防止其有效学习受保护数据，实现数据级保护。


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练使用大规模异构文本语料，引发对未经授权使用专有或个人数据的担忧，需要在不接触或修改训练流程的黑盒设置下保护数据不被模型学习。

Method: 采用数据级防御方法Disclaimer Injection，向文本注入触发模型对齐机制的免责声明；通过分层分析验证对齐相关层的持续激活现象。

Result: 在受保护数据上微调的模型表现出显著且系统的性能下降，对齐约束会覆盖任务学习；该方法首次实现在不接触训练流程的情况下限制LLM规模的数据可学习性。

Conclusion: 对齐行为是数据保护的新杠杆，Disclaimer Injection是首个实用的、无需访问或修改训练流程的大规模LLM数据不可学习性限制方法。
...
### [122] [Tigrinya Number Verbalization: Rules, Algorithm, and Implementation](https://arxiv.org/abs/2601.03403)
*Fitsum Gaim,Issayas Tesfamariam*

Main category: cs.CL

TL;DR: 本文系统形式化了提格里尼亚语基数和序数词的口语表达规则，填补了该语言计算资源的空白，并发布了开源实现。


<details>
  <summary>Details</summary>
Motivation: 提格里尼亚语缺乏计算语言学资源，特别是数字口语化的系统规则。现有大型语言模型在该任务上表现不佳，凸显了显式规则记录的必要性。

Method: 1. 记录提格里尼亚语数字表达的正则规则（包括连接系统、量级词、日期/时间/货币等特例）；2. 设计数字到词语转换的正式算法；3. 实现开源工具；4. 评估前沿大语言模型的表现。

Result: 1. 完成了提格里尼亚语数字口语化的系统形式化；2. 大语言模型在提格里尼亚语数字口语化任务上存在显著缺陷；3. 开源实现已发布。

Conclusion: 本研究为提格里尼亚语社区的语言建模、语音合成和可访问性应用提供了基础资源，并证明了大语言模型在低资源语言任务上仍需显式规则支持。
...
### [123] [Implicit Graph, Explicit Retrieval: Towards Efficient and Interpretable Long-horizon Memory for Large Language Models](https://arxiv.org/abs/2601.03417)
*Xin Zhang,Kailai Yang,Hao Li,Chenyue Li,Qiyu Wei,Sophia Ananiadou*

Main category: cs.CL

TL;DR: 提出LatentGraphMem框架，结合隐式图记忆与显式子图检索，以解决大语言模型在长上下文稀疏证据场景下的记忆问题，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 长视野应用中，大语言模型需在证据稀疏且分散于长上下文中回答查询。现有显式结构化记忆易受长上下文过载影响，而隐式记忆机制虽高效稳定但难以检查。

Method: 在隐空间存储图结构记忆以保证稳定高效；通过任务特定的子图检索接口返回紧凑符号子图；训练时显式图视图与冻结推理器交互进行监督，推理时仅在隐空间检索并外化子图。

Result: 在多个模型规模的长视野基准测试中，LatentGraphMem持续优于显式图和隐式记忆基线，支持参数高效适配，并能灵活扩展至更大推理器而不引入大型符号产物。

Conclusion: LatentGraphMem通过隐式图记忆与显式子图检索的结合，实现了高效稳定且可检查的记忆系统，为长上下文推理提供了有效解决方案。
...
### [124] [PCoA: A New Benchmark for Medical Aspect-Based Summarization With Phrase-Level Context Attribution](https://arxiv.org/abs/2601.03418)
*Bohao Chu,Sameh Frihat,Tabea M. G. Pakull,Hendrik Damm,Meijie Li,Ula Muhabbek,Georg Lodde,Norbert Fuhr*

Main category: cs.CL

TL;DR: 提出PCoA基准数据集和评估框架，用于医学摘要的细粒度上下文归因验证，通过实验验证了其可靠性并展示了分步归因方法的优势。


<details>
  <summary>Details</summary>
Motivation: 系统生成摘要的验证具有挑战性，尤其在医疗等高风险领域，需要精确的上下文归因来确保摘要的可信度和安全性。

Method: 构建了专家标注的PCoA基准数据集，支持短语级上下文归因；提出了解耦的评估框架，分别评估摘要质量、引用和贡献短语；通过实验验证数据集质量并测试多个大语言模型。

Result: PCoA为短语级上下文归因的摘要评估提供了可靠基准；实验表明，在摘要生成前显式识别相关句子和贡献短语能提升整体质量。

Conclusion: PCoA数据集和评估框架能有效支持医学领域摘要的细粒度验证，分步归因方法有助于提高摘要生成系统的准确性和可靠性。
...
### [125] [Training-Free Adaptation of New-Generation LLMs using Legacy Clinical Models](https://arxiv.org/abs/2601.03423)
*Sasha Ronaghi,Chloe Stanwyck,Asad Aali,Amir Ronaghi,Miguel Fuentes,Tina Hernandez-Boussard,Emily Alsentzer*

Main category: cs.CL

TL;DR: 提出Cross-Architecture Proxy Tuning（CAPT）方法，无需重新训练即可将先进通用领域模型适配到临床领域，通过对比解码选择性注入临床信号，在多项任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统临床领域语言模型适配需要为每个新模型重新进行预训练和微调，成本高昂且效率低下。

Method: 采用模型集成方法，利用现有临床模型对先进通用领域模型进行训练免费适配，支持词汇表不重叠的模型，通过对比解码选择性注入临床相关信号。

Result: 在六个临床分类和文本生成任务中，CAPT平均比UniTE提升17.6%，比代理调优提升41.4%，能增强临床可操作语言、减少上下文错误并提高临床特异性。

Conclusion: CAPT提供了一种高效、低成本的临床领域模型适配方案，在保持通用模型推理和流畅性的同时显著提升临床任务性能。
...
### [126] [The Critical Role of Aspects in Measuring Document Similarity](https://arxiv.org/abs/2601.03435)
*Eftekhar Hossain,Tarnika Hazra,Ahatesham Bhuiyan,Santu Karmaker*

Main category: cs.CL

TL;DR: 提出ASPECTSIM框架，通过显式指定方面来评估文档相似性，相比传统整体方法显著提升人机一致性，但开源小模型仍落后于GPT-4o等大模型。


<details>
  <summary>Details</summary>
Motivation: 传统文档相似性评估采用整体方法，未考虑特定方面，可能无法准确反映文档在具体维度上的相似程度。

Method: 1. 构建包含2.6万对方面-文档的基准数据集；2. 使用GPT-4o直接提示生成ASPECTSIM分数；3. 对16个开源小语言模型和9个嵌入模型进行大规模元评估；4. 采用两阶段优化方法提升小模型性能。

Result: 1. GPT-4o实现的ASPECTSIM比无显式方面的整体相似性人机一致性高约80%；2. 直接提示小模型效果差（人机一致性20-30%），两阶段优化提升约140%；3. 开源小模型在捕捉方面条件相似性上仍大幅落后于GPT-4o。

Conclusion: 显式考虑方面对文档相似性评估至关重要，需修订标准实践；虽然优化可提升小模型表现，但当前开源模型仍无法匹敌大型专有模型在此任务上的能力。
...
### [127] [Enhancing Linguistic Competence of Language Models through Pre-training with Language Learning Tasks](https://arxiv.org/abs/2601.03448)
*Atsuki Yamaguchi,Maggie Mi,Nikolaos Aletras*

Main category: cs.CL

TL;DR: 提出L2T预训练框架，在标准下一个词预测基础上加入语言学习任务，提升语言模型的语言能力而不损害推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型通过原始文本的逐词预测学习世界知识和推理，但未显式优化语言能力，需弥补这一差距。

Method: 受人类语言习得启发，将原始文本转化为结构化输入-输出对，提供显式语言刺激，在原始文本和L2T数据混合上进行预训练。

Result: 在语言能力基准测试中表现提升且学习加速，同时在通用推理任务上保持竞争力。

Conclusion: L2T框架能有效增强语言模型的语言能力，加速其习得过程，且不影响模型的通用推理性能。
...
### [128] [Grading Scale Impact on LLM-as-a-Judge: Human-LLM Alignment Is Highest on 0-5 Grading Scale](https://arxiv.org/abs/2601.03444)
...
TL;DR: 研究探讨了LLM作为评估者时评分量表的效应，发现量表选择显著影响人类与LLM评分的一致性，其中0-5分量表对齐效果最佳，并揭示了量表设计和子群体诊断的重要性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM越来越多地被用作自动评估者，但已有研究表明LLM评分对提示变化敏感，而评分量表本身的影响尚未充分探索，因此需要系统研究不同量表下人类与LLM评分的一致性。

Method: 收集人类和LLM在三种评分量表上对六个基准（包括客观、主观和混合任务）的评分，使用组内相关系数（ICC）测量绝对一致性，并分析量表选择对人类-LLM一致性的影响及子群体差异。

Result: LLM在主观任务上的评分随量表变化不一致；量表选择显著影响人类-LLM一致性，即使组内可靠性高；综合任务中，0-5分量表对齐最强；聚合可靠性可能掩盖基准异质性，并揭示性别等子群体间的一致性差异。

Conclusion: 评分量表设计是LLM作为评估者协议中的关键因素，需要针对具体任务优化量表，并进行子群体层面的诊断以确保评估的公平性和可靠性。
...
### [129] [EpiQAL: Benchmarking Large Language Models in Epidemiological Question Answering for Enhanced Alignment and Reasoning](https://arxiv.org/abs/2601.03471)
*Mingyang Wei,Dehai Min,Zewen Liu,Yuzhang Xie,Guanchen Wu,Carl Yang,Max S. Y. Lau,Qi He,Lu Cheng,Wei Jin*

Main category: cs.CL

TL;DR: 提出了首个流行病学问答诊断基准EpiQAL，用于评估模型在证据基础、多步推理和结论重构方面的能力，发现当前大语言模型在流行病学推理上表现有限。
...
Method: 构建了包含三个子集的EpiQAL基准：文本基础事实回忆、连接文档证据与流行病学原理的多步推理、以及隐藏讨论部分的结论重构。构建过程结合专家设计的分类指导、多模型验证和基于检索的难度控制。在十个开源模型上进行了实验。

Result: 当前大语言模型在流行病学推理上表现有限，多步推理挑战最大；模型排名在不同子集间变化，仅靠规模无法预测成功；思维链提示对多步推理有帮助但在其他方面效果不一。

Conclusion: EpiQAL为证据基础、推理能力和结论重构提供了细粒度的诊断信号，有助于推动流行病学推理能力的发展。
...
### [130] [Prompting Underestimates LLM Capability for Time Series Classification](https://arxiv.org/abs/2601.03464)
*Dan Schumacher,Erfan Nourbakhsh,Rocky Slavin,Anthony Rios*

Main category: cs.CL

TL;DR: 研究发现大语言模型在时间序列分类任务中表现不佳主要源于提示评估方法的局限性，而非模型缺乏时间结构表征能力。通过线性探针分析发现，模型内部表征的时间序列信息远超提示生成结果，甚至能与专业模型媲美。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的评估显示大语言模型在时间序列分类任务中表现接近随机水平，这引发了对模型是否真正理解时间序列结构的质疑。研究旨在探究这种表现不佳是源于模型表征能力的本质缺陷，还是评估方法的局限性。

Method: 采用线性探针方法直接分析模型内部表征，并与零样本提示生成结果进行对比。通过分层分析探究时间序列信息在Transformer各层的分布规律，并考察视觉和多模态输入对时间序列表征的影响。

Result: 零样本提示生成的F1分数仅为0.15-0.26，而基于相同内部表征的线性探针将F1提升至0.61-0.67，达到甚至超越专业时间序列模型的水平。分层分析显示类别判别性时间序列信息在早期Transformer层已出现，且视觉和多模态输入能增强这种表征。

Conclusion: 当前基于提示的评估方法系统性地低估了大语言模型对时间序列的理解能力，模型内部实际编码了丰富的时间结构信息。这一发现揭示了模型表征能力与评估方法之间的不匹配问题，为改进时间序列任务评估提供了新视角。
...
### [131] [SegNSP: Revisiting Next Sentence Prediction for Linear Text Segmentation](https://arxiv.org/abs/2601.03474)
*José Isidro,Filipe Cunha,Purificação Silvano,Alípio Jorge,Nuno Guimarães,Sérgio Nunes,Ricardo Campos*

Main category: cs.CL

TL;DR: 本文提出SegNSP方法，将线性文本分割重新定义为下一句预测任务，通过标签无关的NSP方法和分割感知损失函数，在多个数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 线性文本分割是自然语言处理中的长期难题，由于话题边界定义复杂、篇章结构多变，现有方法难以平衡局部连贯性与全局上下文，这影响了摘要、信息检索和问答等下游应用的效果。

Method: 提出SegNSP方法，将文本分割视为下一句预测任务；采用标签无关的NSP方法，无需显式话题标签；结合分割感知损失函数和更难负采样策略，以更好地捕捉篇章连续性。

Result: 在CitiLink-Minutes数据集上达到B-F1分数0.79，接近人工标注的话题转换；在WikiSection数据集上达到B-F1分数0.65，比最强可复现基线TopSeg高出0.17个绝对点。

Conclusion: 通过建模句子间连续性可以有效提升文本分割质量，SegNSP方法展示了竞争性和鲁棒的性能，能够支持下游NLP应用。
...
### [132] [Self-Explaining Hate Speech Detection with Moral Rationales](https://arxiv.org/abs/2601.03481)
...
TL;DR: 提出SMRA框架，首次将道德理性作为注意力对齐的直接监督，用于仇恨言论检测，提高模型性能和解释忠实度，并发布葡萄牙语数据集HateBRMoralXplain。


<details>
  <summary>Details</summary>
Motivation: 现有仇恨言论检测模型依赖表面词汇特征，易受虚假相关性影响，导致鲁棒性、文化语境化和可解释性不足。

Method: 基于道德基础理论，提出监督道德理性注意力（SMRA）框架，将专家标注的道德理性作为token级注意力的直接监督目标，引导模型关注道德显著内容而非虚假词汇模式。

Result: 在二元仇恨言论检测和多标签道德情感分类任务中，SMRA分别提升F1分数0.9和1.5，解释忠实度显著提高（IoU F1提升7.4个百分点，Token F1提升5.0个百分点），充分性提升2.3个百分点且公平性保持稳定。

Conclusion: SMRA通过道德理性监督实现了性能提升与解释忠实度的双重优化，无需在性能或偏差方面做出权衡，为可解释仇恨言论检测提供了新方向。
...
### [133] [Submodular Evaluation Subset Selection in Automatic Prompt Optimization](https://arxiv.org/abs/2601.03493)
*Jinming Nian,Zhiyuan Peng,Hongwei Shang,Dae Hoon Park,Yi Fang*

Main category: cs.CL

TL;DR: 本文提出SESS方法，通过子模评估子集选择优化自动提示工程，相比随机或启发式基线能产生更好的优化提示。


<details>
  <summary>Details</summary>
Motivation: 自动提示优化依赖小规模评估子集的性能反馈，但现有方法通常将评估子集选择视为实现细节，缺乏理论指导。

Method: 提出SESS方法，将评估子集选择形式化为最大化目标集函数，证明其在温和条件下具有单调性和子模性，从而支持带理论保证的贪心选择算法。

Result: 在GSM8K、MATH和GPQA-Diamond数据集上，子模选择的评估子集相比随机或启发式基线能产生更优的优化提示。

Conclusion: 评估子集选择对提示优化至关重要，子模选择方法能有效提升优化效果，为自动提示工程提供了理论指导。
...
### [134] [CALM: Culturally Self-Aware Language Models](https://arxiv.org/abs/2601.03483)
*Lingzhi Shen,Xiaohao Cai,Yunfei Long,Imran Razzak,Guanming Chen,Shoaib Jameel*

Main category: cs.CL

TL;DR: 提出了CALM框架，通过解耦任务语义与文化概念、构建文化簇、跨注意力对齐和自适应集成，赋予语言模型文化自我意识，在跨文化基准测试中表现优异。
...
Method: CALM框架：1）解耦任务语义与显性文化概念及潜在文化信号；2）通过对比学习构建结构化文化簇；3）跨注意力对齐建立文化特征细粒度交互；4）基于文化特定维度通过混合专家机制自适应集成；5）构建文化基础内部身份状态，并通过自提示反思学习实现持续适应与自我纠正。

Result: 在多个跨文化基准数据集上的广泛实验表明，CALM持续优于现有最先进方法。

Conclusion: CALM框架能有效增强语言模型的文化自我意识，通过结构化文化表示和自适应学习机制实现更好的文化适应性。
...
### [135] [Reasoning Pattern Alignment Merging for Adaptive Reasoning](https://arxiv.org/abs/2601.03506)
*Zhaofeng Zhong,Wei Yuan,Tong Chen,Xiangyu Zhao,Quoc Viet Hung Nguyen,Hongzhi Yin*

Main category: cs.CL

TL;DR: 提出RPAM框架，通过模型合并实现自适应推理，在保持性能的同时显著降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型为每个查询生成冗长推理路径，导致不必要的计算和延迟。现有加速方法需要重新训练模型或复杂提示工程，成本高昂且对输入敏感。

Method: 提出推理模式对齐合并（RPAM）框架：1）构建带模式标签的校准集，为每个查询分配适当推理模式；2）通过特征对齐优化层间合并系数，使合并模型的中间表示与选定模型对齐，同时使用对比目标使其远离非选定模型。

Result: 在七个广泛使用的推理基准测试中，RPAM在保持强大性能的同时显著降低了推理成本。

Conclusion: RPAM提供了一种轻量级模型合并方法，无需从头训练或大规模额外数据即可实现自适应高效推理，将在文章接受后开源代码。
...
### [136] [Beyond Perplexity: A Lightweight Benchmark for Knowledge Retention in Supervised Fine-Tuning](https://arxiv.org/abs/2601.03505)
*Soheil Zibakhsh Shabgahi,Pedram Aghazadeh,Farinaz Koushanfar*

Main category: cs.CL

TL;DR: 提出了一种名为知识保留测试（KR-Test）的轻量级评估框架，用于区分监督微调中事实学习与语言风格模仿，并通过LoRA训练动态分析验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调依赖验证困惑度评估训练效果，但该方法无法区分语言风格模仿与真实事实内化，导致评估结果不准确。

Method: 1. 设计KR-Test框架，通过自动生成对比样本测量正确与错误续写的似然偏好；2. 无需指令微调或生成解码；3. 通过“盲测vs.先知”基线分析验证框架完整性；4. 应用KR-Test分析低秩适应（LoRA）的训练动态。

Result: 1. KR-Test能有效分离语言收敛与知识保留过程；2. 揭示了监督微调中事实学习与语言模仿的细粒度解耦现象；3. 增强了微调动态的可解释性。

Conclusion: KR-Test为评估大语言模型领域知识注入提供了更精确的诊断工具，有助于更深入理解微调过程中的知识内化机制。
...
### [137] [Mem-Gallery: Benchmarking Multimodal Long-Term Conversational Memory for MLLM Agents](https://arxiv.org/abs/2601.03515)
*Yuanchen Bei,Tianxin Wei,Xuying Ning,Yanjun Zhao,Zhining Liu,Xiao Lin,Yada Zhu,Hendrik Hamann,Jingrui He,Hanghang Tong*

Main category: cs.CL

TL;DR: 提出了Mem-Gallery基准测试，用于评估多模态大语言模型在长期对话中的记忆能力，发现现有模型在记忆推理和管理方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么评估纯文本对话的多会话记忆，要么评估局部上下文的多模态理解，无法评估多模态记忆在长期对话轨迹中的保存、组织和演化。

Method: 构建了Mem-Gallery数据集，包含高质量的多会话对话，融合视觉和文本信息；提出了系统评估框架，从记忆提取与测试时适应、记忆推理、记忆知识管理三个功能维度评估记忆能力。

Result: 对13个记忆系统的广泛测试表明：需要显式的多模态信息保留和组织；记忆推理和知识管理存在持续局限；当前模型存在效率瓶颈。

Conclusion: Mem-Gallery基准测试揭示了多模态长期对话记忆评估的重要性，并指出了当前模型在记忆能力方面的关键不足和改进方向。
...
### [138] [IntroLM: Introspective Language Models via Prefilling-Time Self-Evaluation](https://arxiv.org/abs/2601.03511)
*Hossein Hosseini Kasnavieh,Gholamreza Haffari,Chris Leckie,Adel N. Toosi*

Main category: cs.CL

TL;DR: 提出IntroLM方法，使因果语言模型能在预填充阶段预测自身输出质量，无需外部分类器，在问答基准测试中优于DeBERTa分类器14%，在路由系统中降低延迟33%并减少大模型使用50%。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部分类器（如BERT）预测LLM输出质量，但存在上下文窗口有限、表示能力受限和额外计算开销的问题。

Method: 引入IntroLM方法：1) 使用自省令牌让模型在预填充阶段预测输出质量；2) 采用令牌条件LoRA，仅针对自省令牌激活，保持主干模型行为不变；3) 避免使用外部评估器。

Result: 在Qwen3 8B模型上，IntroLM在问答基准测试中实现90%的ROC AUC（成功率预测），优于DeBERTa分类器14%；在多模型路由系统中，在保持可靠性的同时降低延迟33%，减少大模型使用50%。

Conclusion: IntroLM通过自省令牌和条件LoRA，使LLM能高效预测自身输出质量，显著优于外部分类器，并在路由系统中实现更好的成本性能权衡。
...
### [139] [Layer-Order Inversion: Rethinking Latent Multi-Hop Reasoning in Large Language Models](https://arxiv.org/abs/2601.03542)
*Xukai Liu,Ye Liu,Jipeng Zhang,Yanghai Zhang,Kai Zhang,Qi Liu*

Main category: cs.CL

TL;DR: 研究发现大语言模型在多跳推理中，后跳答案实体可能比桥接实体更早可解码，称为层序反转现象，并提出了概率回忆-提取框架解释该机制。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在多跳推理中表现良好，但其内部如何组合多个事实仍不清楚。先前提出的跳对齐电路假设认为桥接实体在深层计算，但该假设在真实世界查询中未能普遍成立。

Method: 通过对真实世界多跳查询进行系统分析，提出概率回忆-提取框架，使用系统探测分析验证，并重新解释先前的逐层解码证据。

Result: 发现层序反转现象（后跳答案比桥接实体更早可解码），且该现象随总跳数增加而增强；提出的框架能解释思维链的增益，并诊断多跳失败的原因。

Conclusion: 多跳推理并非严格按跳数顺序计算，而是浅层MLP进行广泛概率回忆，深层注意力进行选择性提取；该框架为理解模型内部推理机制提供了新视角。
...
### [140] [PALM-Bench: A Comprehensive Benchmark for Personalized Audio-Language Models](https://arxiv.org/abs/2601.03531)
*Yuwen Wang,Xinyuan Qian,Tian-Hao Zhang,Jiaran Gao,Yuchen Pan,Xin Wang,Zhou Pan,Chen Wei,Yiming Wang*

Main category: cs.CL

TL;DR: 该研究提出了个性化音频语言模型（PALM）任务，创建了首个相关基准（PALM-Bench），并发现现有方法在建模个性化知识和跨任务迁移方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型（LALMs）在音频理解与生成方面表现良好，但其行为较为通用（如总结语音内容），无法充分支持个性化问答（如总结特定人物的发言）。人类决策依赖个人语境，而现有模型缺乏对此的建模能力。

Method: 1. 形式化定义PALM任务，要求模型识别个人概念并在个人语境中推理；2. 创建首个多说话人场景下的基准PALM-Bench，支持结构化评估；3. 对代表性开源LALMs进行实验，评估无训练提示和有监督微调策略的效果。

Result: 实验表明，现有的无训练提示和有监督微调策略虽能带来改进，但在建模个性化知识及跨任务鲁棒迁移方面仍存在明显局限。

Conclusion: 个性化音频理解是当前LALMs的薄弱环节，需要开发更有效的个性化建模方法。PALM-Bench的发布旨在推动该领域的方法创新。
...
### [141] [Persona-aware and Explainable Bikeability Assessment: A Vision-Language Model Approach](https://arxiv.org/abs/2601.03534)
*Yilong Dai,Ziyi Wang,Chenguang Wang,Kexin Zhou,Yiheng Qian,Susu Xu,Xiang Yan*

Main category: cs.CL

TL;DR: 本文提出了一种基于人物感知的视觉语言模型框架，用于评估自行车友好性，通过人物条件化、多粒度监督微调和AI数据增强，实现了可解释的自行车道评估。


<details>
  <summary>Details</summary>
Motivation: 现有基于感知的自行车友好性评估方法难以捕捉复杂道路环境，且未能充分考虑用户主观感知的异质性，需要更有效的评估框架。

Method: 提出人物感知视觉语言模型框架，包括：基于骑行类型理论的人物条件化生成解释；结合专家标注推理与用户评分的多粒度监督微调；AI数据增强创建配对数据以隔离基础设施变量影响。

Result: 通过全景图像众包系统收集了427名骑行者的12,400条评估数据，实验表明该框架在自行车友好性评分预测上具有竞争力，并能实现可解释的因素归因。

Conclusion: 该框架能够有效评估自行车友好性，同时提供可解释的评估结果，有助于推动可持续城市交通和骑行友好城市建设。
...
### [142] [Evaluating LLMs for Police Decision-Making: A Framework Based on Police Action Scenarios](https://arxiv.org/abs/2601.03553)
*Sangyub Lee,Heedou Kim,Hyeoncheol Kim*

Main category: cs.CL

TL;DR: 提出针对警务操作的LLM评估框架PAS，构建相关数据集，发现商用LLM在警务任务中表现不佳，强调需可靠评估以确保AI警务应用安全。


<details>
  <summary>Details</summary>
Motivation: LLM在警务操作中的应用日益增多，但缺乏专门评估框架，未经验证的使用可能导致非法逮捕、证据收集不当等严重问题。

Method: 提出PAS系统评估框架，基于8000多份官方文档构建新型QA数据集，通过统计分析结合警务专家判断验证关键指标。

Result: 实验表明商用LLM在新警务任务中表现困难，尤其在提供基于事实的建议方面存在不足。

Conclusion: 研究强调需要可扩展的评估框架来确保AI驱动警务操作的可靠性，并公开了数据和提示模板。
...
### [143] [Value-Action Alignment in Large Language Models under Privacy-Prosocial Conflict](https://arxiv.org/abs/2601.03546)
*Guanyu Chen,Chenxiao Yu,Xiyang Hu*

Main category: cs.CL

TL;DR: 本文提出了一种基于上下文的评估协议，用于评估大语言模型在隐私担忧和亲社会动机竞争下的价值-行为一致性，并引入了价值-行为对齐率（VAAR）作为衡量指标。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法往往单独测量隐私相关态度或分享意图，难以确定模型表达的价值是否像真实人类行为一样共同预测其下游数据共享行为。

Method: 引入基于上下文的评估协议，在有限的历史记录会话中顺序管理隐私态度、亲社会性和数据共享接受度的标准化问卷；使用多组结构方程模型（MGSEM）分析隐私担忧和亲社会性对数据共享的影响；提出价值-行为对齐率（VAAR）作为人类参照的方向一致性度量。

Result: 在多个大语言模型中观察到稳定但模型特定的隐私-亲社会性-数据共享接受度（Privacy-PSA-AoDS）模式，以及价值-行为对齐的显著异质性。

Conclusion: 大语言模型在模拟涉及个人数据共享的决策任务时表现出不同的价值-行为对齐模式，需要更精细的评估方法来理解模型在竞争态度下的决策行为。
...
### [144] [EvolMem: A Cognitive-Driven Benchmark for Multi-Session Dialogue Memory](https://arxiv.org/abs/2601.03543)
*Ye Shen,Dun Pei,Yiqiu Guo,Junying Wang,Yijin Guo,Zicheng Zhang,Qi Jia,Jun Zhou,Guangtao Zhai*

Main category: cs.CL

TL;DR: 提出了EvolMem基准，用于评估大语言模型在多会话记忆能力上的表现，涵盖陈述性和非陈述性记忆的多个维度。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对大语言模型在多会话设置下多样化记忆维度的系统评估，需要更全面的测试框架。

Method: 采用基于认知心理学的混合数据合成框架，包括主题启动生成和叙事启发转换，生成可控复杂度的多会话对话。

Result: 评估显示没有大语言模型在所有记忆维度上表现一致最优；代理记忆机制不一定提升模型能力且存在效率限制。

Conclusion: EvolMem基准揭示了当前大语言模型在多会话记忆能力上的局限性，为未来研究提供了系统评估工具。
...
### [145] [DeepSynth-Eval: Objectively Evaluating Information Consolidation in Deep Survey Writing](https://arxiv.org/abs/2601.03540)
...
TL;DR: 提出了DeepSynth-Eval基准，用于客观评估大语言模型在信息整合方面的能力，实验表明基于规划的写作流程优于单轮生成。


<details>
  <summary>Details</summary>
Motivation: 大语言模型向自主代理发展推动了深度研究的进步，但检索后合成阶段（即代理需消化大量上下文并将碎片化证据整合为连贯长文本）因开放式写作的主观性而缺乏客观评估方法。

Method: 利用高质量综述论文作为黄金标准，逆向推导研究需求，并从其参考文献构建“Oracle Contexts”以隔离检索噪声；提出使用通用检查清单（事实覆盖）和约束检查清单（结构组织）的细粒度评估协议，将主观判断转化为可验证指标。

Result: 在96个任务上的实验表明，从数百篇参考文献中整合信息仍具挑战；代理式规划写作流程显著优于单轮生成，能有效减少幻觉并提升对复杂结构约束的遵循度。

Conclusion: DeepSynth-Eval基准能客观评估信息整合能力，规划式工作流在合成任务中表现更优，为未来自主研究代理的发展提供了重要评估工具。
...
### [146] [From Chains to Graphs: Self-Structured Reasoning for General-Domain LLMs](https://arxiv.org/abs/2601.03597)
...
TL;DR: 提出自我图推理框架，使大语言模型在开放域问答中构建图结构推理过程，提升推理一致性和性能


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的推理过程通常是线性的且逻辑不一致，而现实世界推理需要整合多个前提并并行解决子问题。现有方法如思维链虽以线性文本形式表达推理，但常导致不一致结论，且缺乏对模型自主构建图结构推理的探索

Method: 提出自我图推理框架，使大语言模型在生成最终答案前将推理过程显式表示为结构化图；构建图结构推理数据集，将多个候选推理图合并为精炼的图结构用于模型训练

Result: 在五个问答基准测试中，该框架持续提升推理一致性，相比基础模型获得17.74%的性能增益；使用该框架微调的LLaMA-3.3-70B模型性能与GPT-4o相当，超过Claude-3.5-Haiku

Conclusion: 图结构推理能有效提升大语言模型的推理一致性和问答性能，为开放域问答中的复杂推理问题提供了新解决方案
...
### [147] [DiffCoT: Diffusion-styled Chain-of-Thought Reasoning in LLMs](https://arxiv.org/abs/2601.03559)
*Shidong Cao,Hongzhan Lin,Yuxuan Gu,Ziyang Luo,Jing Ma*

Main category: cs.CL

TL;DR: 提出DiffCoT框架，将思维链推理重构为迭代去噪过程，通过滑动窗口机制和因果扩散噪声调度，在保持自回归的同时实现推理步骤的生成与修正，提升多步数学推理的鲁棒性和纠错能力。
...
Method: 1. 将思维链推理重构为扩散式迭代去噪过程；2. 通过滑动窗口机制在推理步骤层面集成扩散原理；3. 设计因果扩散噪声调度以保持推理链的时间结构因果一致性；4. 在保持词元级自回归的同时支持中间步骤的生成与回溯修正。

Result: 在三个多步思维链推理基准测试中，DiffCoT在不同模型骨干上均优于现有思维链偏好优化方法，表现出更强的鲁棒性和错误纠正能力。

Conclusion: DiffCoT通过扩散式框架有效缓解了思维链推理中的错误传播问题，为多步推理任务提供了更可靠的解决方案。
...
### [148] [How Do Large Language Models Learn Concepts During Continual Pre-Training?](https://arxiv.org/abs/2601.03570)
*Barry Menglong Yao,Sha Li,Yunzhi Yao,Minqian Liu,Zaishuo Xia,Qifan Wang,Lifu Huang*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型在持续预训练中概念的学习与遗忘机制，通过概念电路分析揭示了概念获取的动态模式、干扰效应和迁移特性。


<details>
  <summary>Details</summary>
Motivation: 人类通过概念理解世界，但大型语言模型在持续预训练中如何获取、保持和遗忘概念尚不明确，需要从计算层面揭示其内部机制。

Method: 提出概念电路作为概念对应的计算子图，结合图度量分析电路结构，研究概念在持续训练中的行为动态，包括干扰、协同效应和迁移性。

Result: 发现概念电路能显著反映概念学习与遗忘；概念学习呈阶段性变化；学习增益大的概念后续遗忘更明显；语义相似概念干扰更强；概念迁移性存在差异。

Conclusion: 研究提供了概念学习的电路级视角，为设计更具可解释性和鲁棒性的概念感知训练策略提供了依据。
...
### [149] [Evaluating the Pre-Consultation Ability of LLMs using Diagnostic Guidelines](https://arxiv.org/abs/2601.03627)
*Jean Seo,Gibaeg Kim,Kihun Shin,Seungseop Lim,Hyunkyung Lee,Wooseok Han,Jongwon Lee,Eunho Yang*

Main category: cs.CL

TL;DR: EPAG是一个评估LLMs预咨询能力的基准数据集和框架，通过HPI-诊断指南对比和疾病诊断进行评估。实验发现，经过任务特定数据微调的小型开源模型在预咨询中可能优于前沿LLMs，且HPI信息量的增加不一定提升诊断性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个评估LLMs在真实临床场景中预咨询能力的标准化框架，以促进LLMs在医疗领域的应用发展。

Method: 1. 构建EPAG基准数据集和评估框架；2. 直接评估：HPI与诊断指南对比；3. 间接评估：疾病诊断；4. 实验比较不同规模LLMs的性能；5. 分析HPI信息量和预咨询语言对结果的影响。

Result: 1. 经过任务特定数据微调的小型开源模型在预咨询任务中可能优于前沿LLMs；2. HPI信息量的增加不一定带来诊断性能提升；3. 预咨询使用的语言会影响对话特征。

Conclusion: EPAG框架为评估LLMs在临床预咨询能力提供了有效工具，开源数据集和评估流程有助于推动LLMs在真实医疗场景中的应用发展。
...
### [150] [e5-omni: Explicit Cross-modal Alignment for Omni-modal Embeddings](https://arxiv.org/abs/2601.03666)
*Haonan Chen,Sicheng Gao,Radu Timofte,Tetsuya Sakai,Zhicheng Dou*

Main category: cs.CL

TL;DR: 提出e5-omni方法，通过显式对齐技术改进预训练视觉语言模型，解决多模态嵌入中的尺度不一致、负样本效率低和统计量不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有全模态嵌入模型依赖预训练视觉语言模型的隐式对齐，导致相似度尺度不一致、跨模态批次负样本效率低下以及嵌入统计量不匹配三个问题。

Method: 提出轻量级显式对齐方案，包含三个组件：(1)模态感知温度校准对齐相似度尺度；(2)可控负样本课程学习与去偏技术聚焦困难负样本；(3)批次白化与协方差正则化匹配跨模态几何结构。

Result: 在MMEB-V2和AudioCaps基准测试中优于现有双模态和全模态基线，且方法可迁移至其他视觉语言模型骨干网络。

Conclusion: e5-omni能有效将现成视觉语言模型转化为鲁棒的全模态嵌入模型，通过显式对齐解决跨模态检索中的关键问题。
...
### [151] [PsychEthicsBench: Evaluating Large Language Models Against Australian Mental Health Ethics](https://arxiv.org/abs/2601.03578)
...
TL;DR: 该论文介绍了PsychEthicsBench，首个基于澳大利亚心理学和精神病学指南的基准测试，用于评估大型语言模型在心理健康领域的伦理知识和行为响应，发现拒绝率不能有效反映伦理行为，且领域特定微调可能降低伦理鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在心理健康应用中的安全评估主要依赖拒绝行为，但这种方法无法反映临床实践所需的细致行为，临床不恰当的拒绝可能被视为缺乏同理心并阻碍求助行为。

Method: 开发了基于澳大利亚心理学和精神病学指南的PsychEthicsBench基准，包含多项选择和开放式任务，并带有细粒度伦理标注；对14个模型进行了实证评估。

Result: 拒绝率是伦理行为的差指标，安全触发与临床适当性存在显著差异；领域特定微调可能降低伦理鲁棒性，多个专业模型在伦理对齐上表现不如其基础模型。

Conclusion: PsychEthicsBench为心理健康领域大型语言模型的系统性、司法意识评估提供了基础，鼓励该领域更负责任的发展。
...
### [152] [Towards Compositional Generalization of LLMs via Skill Taxonomy Guided Data Synthesis](https://arxiv.org/abs/2601.03676)
*Yifan Wei,Li Du,Xiaoyan Yu,Yang Feng,Angsheng Li*

Main category: cs.CL

TL;DR: 提出STEPS框架，通过构建技能分类学指导的数据合成，解决大语言模型和智能体在组合泛化上的数据瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型和智能体系统在组合泛化上存在数据瓶颈，复杂技能组合呈现长尾分布，限制了指令跟随性能和智能体任务的泛化能力。

Method: STEPS框架：1) 使用结构信息理论构建可解释的层次化技能分类学；2) 将数据合成建模为约束信息最大化问题，选择能最大化层次结构边际信息且保持语义一致性的技能组合。

Result: 在指令跟随基准测试中，STEPS优于现有数据合成基线方法；在下游智能体评估中，组合泛化能力得到提升。

Conclusion: STEPS通过结构化技能分类学指导的数据合成，有效提升了模型在组合泛化任务上的性能，为解决数据瓶颈问题提供了新思路。
...
### [153] [ADEPT: Adaptive Dynamic Early-Exit Process for Transformers](https://arxiv.org/abs/2601.03700)
*Sangmin Yoo,Srikanth Malla,Chiho Choi,Wei D. Lu,Joon Hee Choi*

Main category: cs.CL

TL;DR: ADEPT提出了一种自适应动态提前退出机制，可在Transformer的预填充和生成阶段实现令牌级提前退出，通过解耦跳过层的KV缓存依赖来提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有提前退出策略仅适用于生成阶段的第一个令牌或预填充阶段的提示级别，跳过层的KV缓存成为后续令牌生成的瓶颈，限制了计算效率的提升。

Method: 提出自适应令牌级提前退出机制，根据令牌复杂度动态调整计算；通过解耦跳过层的序列依赖关系优化KV生成过程。

Result: 在语言生成任务中效率提升达25%，下游分类任务加速4倍，性能提升最高达45%。

Conclusion: ADEPT通过动态令牌级提前退出有效解决了KV缓存瓶颈问题，在保持性能的同时显著提升了Transformer模型的推理效率。
...
### [154] [From Implicit to Explicit: Token-Efficient Logical Supervision for Mathematical Reasoning in LLMs](https://arxiv.org/abs/2601.03682)
*Shaojie Wang,Liang Zhang*

Main category: cs.CL

TL;DR: 本文提出FSLR框架，通过训练模型专注于问题解决的第一步（识别变量和操作）来提升大语言模型的逻辑推理能力，相比传统CoT-SFT方法在效果和效率上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在数学问题解决中主要依赖模式匹配和记忆，而非真正的逻辑推理，且传统CoT-SFT方法无法有效解决逻辑关系理解这一核心瓶颈（占错误预测90%以上）。

Method: 提出FSLR训练框架，通过分离训练模型在解题第一步（识别变量与操作）的能力，为逻辑关系理解提供显式监督，而非像CoT-SFT那样将逻辑关系隐含在完整解题轨迹中。

Result: FSLR在多个模型和数据集上均优于CoT-SFT，分布内和分布外场景平均提升3.2%和4.6%，训练速度提升4-6倍，训练token消耗减少80%以上。

Conclusion: FSLR通过聚焦于逻辑推理的关键第一步，能有效提升大语言模型的逻辑关系理解能力，且具有高效、轻量的优势，为改进模型逻辑推理提供了新方向。
...
### [155] [DiVA: Fine-grained Factuality Verification with Agentic-Discriminative Verifier](https://arxiv.org/abs/2601.03605)
*Hui Huang,Muyun Yang,Yuki Arase*

Main category: cs.CL

TL;DR: 提出DiVA框架，结合生成式模型的搜索能力和判别式模型的评分能力，用于细粒度事实性验证，并构建FGVeriBench基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有事实性验证方法主要进行二元判断（正确/错误），无法区分错误严重程度，限制了其在细粒度评估和偏好优化等应用中的效用。

Method: 提出Agentic Discriminative Verifier (DiVA)混合框架，结合生成式模型的代理搜索能力和判别式模型的精确评分能力，并构建FGVeriBench基准测试集。

Result: 在FGVeriBench上的实验结果表明，DiVA在通用问题和多跳问题的事实性验证上显著优于现有方法。

Conclusion: DiVA框架通过结合生成式和判别式模型的优势，有效解决了细粒度事实性验证问题，为LLM的事实性评估提供了更精细的工具。
...
### [156] [OLA: Output Language Alignment in Code-Switched LLM Interactions](https://arxiv.org/abs/2601.03589)
*Juhyun Oh,Haneul Yoo,Faiz Ghifari Haznitrama,Alice Oh*

Main category: cs.CL

TL;DR: 本文介绍了OLA基准测试，用于评估大语言模型在代码切换交互中的输出语言对齐能力，发现当前模型存在系统性偏差，并提出了一种通过少量数据微调改善对齐的方法。


<details>
  <summary>Details</summary>
Motivation: 多语言用户在对话中自然地进行代码切换（混合使用多种语言），但大语言模型在处理此类提示时，往往无法正确推断用户期望的输出语言，导致响应语言与用户预期不符，这影响了模型在实际多语言交互中的实用性。

Method: 研究团队构建了OLA（Output Language Alignment）基准测试，专注于韩语-英语代码切换场景，涵盖从简单句内混合到指令内容不匹配等多种情况。通过该基准评估多个前沿大语言模型的表现，并尝试使用思维链提示和代码切换感知的DPO（Direct Preference Optimization）微调方法（仅需约1K样本）来改善模型对齐能力。

Result: 即使是最先进的大语言模型也经常误解隐式的语言期望，表现出倾向于非英语响应的偏差；这种偏差在韩语、中文和印尼语配对中均存在。模型还表现出不稳定性，如响应中途切换语言或出现语言侵入。思维链提示无法解决这些错误，表明模型在输出语言方面的语用推理能力较弱。然而，通过少量数据的代码切换感知DPO微调可显著减少未对齐情况。

Conclusion: 大语言模型在代码切换交互中的输出语言未对齐问题源于对齐不足而非根本性限制，通过有针对性的微调可以改善。研究结果强调需要将多语言大语言模型与用户在现实代码切换交互中的隐式期望对齐，以提升其实际应用价值。
...
### [157] [Analyzing Reasoning Shifts in Audio Deepfake Detection under Adversarial Attacks: The Reasoning Tax versus Shield Bifurcation](https://arxiv.org/abs/2601.03615)
*Binh Nguyen,Thai Le*

Main category: cs.CL

TL;DR: 本文提出一个法证审计框架，评估音频语言模型在对抗攻击下的推理鲁棒性，发现显式推理并不总是增强鲁棒性，而是呈现两极分化：对感知鲁棒的模型起防御作用，对其他模型则造成性能损失，且高认知失调可作为潜在攻击的预警信号。


<details>
  <summary>Details</summary>
Motivation: 音频语言模型为可解释的音频深度伪造检测提供了新方向，但现有研究主要关注最终预测的稳定性，缺乏对其推理过程在对抗攻击下鲁棒性的系统分析。

Method: 提出一个三维法证审计框架，从声学感知、认知一致性和认知失调三个相互关联的维度，系统评估音频语言模型在对抗攻击下的推理鲁棒性。

Result: 研究发现显式推理并不普遍增强鲁棒性：对于声学感知鲁棒的模型，推理起到防御作用；对其他模型则造成性能损失，尤其在语言攻击下认知一致性降低、攻击成功率上升。即使分类失败，高认知失调也可作为潜在攻击的预警信号。

Conclusion: 这项工作批判性评估了推理在法证音频深度伪造分析中的作用及其脆弱性，为理解音频语言模型的鲁棒性提供了新视角。
...
### [158] [O-Researcher: An Open Ended Deep Research Model via Multi-Agent Distillation and Agentic RL](https://arxiv.org/abs/2601.03743)
...
TL;DR: 提出自动化合成高质量指令数据的多智能体框架，通过两阶段训练提升开源大语言模型性能，在深度研究基准上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 开源与闭源大语言模型的性能差距主要源于高质量训练数据的获取差异，需要不依赖专有数据/模型的解决方案。

Method: 1. 多智能体协作框架：模拟复杂工具集成推理，端到端生成多样化高保真指令数据；2. 两阶段训练：监督微调 + 新型强化学习方法，最大化模型对齐与能力。

Result: 实验表明该框架能有效提升不同规模开源模型的性能，在主要深度研究基准上实现新的最先进水平。

Conclusion: 该工作为不依赖专有资源、可扩展地推进开源大语言模型发展提供了有效路径。
...
### [159] [Agent-Dice: Disentangling Knowledge Updates via Geometric Consensus for Agent Continual Learning](https://arxiv.org/abs/2601.03641)
*Zheng Wu,Xingyu Lou,Xinbei Ma,Yansi Li,Weiwen Liu,Weinan Zhang,Jun Wang,Zhuosheng Zhang*

Main category: cs.CL

TL;DR: 本文提出Agent-Dice框架，通过参数融合解决LLM智能体持续学习中的稳定性-可塑性困境，区分共享知识与任务特定冲突知识，提升持续学习性能。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在动态环境中交互时面临持续学习新任务而不遗忘旧知识的挑战，即稳定性-可塑性困境，这源于未能明确区分任务间共享知识与任务特定冲突知识。

Method: 提出Agent-Dice参数融合框架，采用两阶段过程：1) 几何共识过滤修剪冲突梯度；2) 基于曲率的重要性加权放大共享语义。

Result: 在GUI智能体和工具使用智能体领域的广泛实验表明，Agent-Dice以最小计算开销和参数更新实现了出色的持续学习性能。

Conclusion: Agent-Dice通过显式区分共享与冲突知识，有效解决了LLM智能体持续学习中的稳定性-可塑性困境，并提供了理论分析和实证验证。
...
### [160] [Reasoning Model Is Superior LLM-Judge, Yet Suffers from Biases](https://arxiv.org/abs/2601.03630)
*Hui Huang,Xuanxin Wu,Muyun Yang,Yuki Arase*

Main category: cs.CL

TL;DR: 本文首次系统比较了大型推理模型（LRMs）与非推理大语言模型（LLMs）作为评估者的性能，发现LRMs在准确性、指令遵循和抗攻击方面更优，但仍存在表面质量偏见。作者提出PlanJudge策略以缓解偏见。


<details>
  <summary>Details</summary>
Motivation: 探究大型推理模型（LRMs）是否比非推理LLMs更适合作为评估者（judge），尤其是在需要推理的任务中，并解决评估过程中存在的偏见问题。

Method: 通过实证分析比较LRMs与非推理LLMs的性能；提出PlanJudge策略，即让模型在执行评估前生成明确的评估计划。

Result: 1）LRMs在判断准确性上优于非推理LLMs，尤其在推理密集型任务中；2）LRMs在评估场景中指令遵循能力更强；3）LRMs对针对判断任务的对抗攻击更具鲁棒性；4）两类模型均存在表面质量偏见，但PlanJudge能显著缓解LRMs和标准LLMs的偏见。

Conclusion: LRMs作为评估者整体优于非推理LLMs，但仍受偏见影响；PlanJudge是一种简单有效的策略，能提升评估的鲁棒性。
...
### [161] [Evaluation of Multilingual LLMs Personalized Text Generation Capabilities Targeting Groups and Social-Media Platforms](https://arxiv.org/abs/2601.03752)
*Dominik Macko*

Main category: cs.CL

TL;DR: 本研究探讨了大型语言模型在10种语言中生成个性化文本的能力及其对检测性的影响，发现平台个性化对文本检测性的影响更大，尤其在英语中。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成多语言连贯文本的能力增强，引发了对其可能被滥用于生成个性化虚假信息的担忧，但此前研究主要集中在英语，缺乏多语言环境下的全面分析。

Method: 研究覆盖10种语言，使用16个不同的语言模型生成文本，通过1080种个性化提示组合（总计17,280个文本），分析针对人口统计群体和社交媒体平台的个性化质量及其对检测性的影响。

Result: 结果显示，不同语言在针对人口统计群体和平台的个性化文本生成质量上存在差异；平台个性化对文本检测性的影响更大，尤其在英语中，其个性化质量最高。

Conclusion: 个性化能力在多语言环境中具有差异化的影响，平台个性化可能增加检测难度，尤其在英语中，这强调了在多语言背景下评估和缓解模型滥用的重要性。
...
### [162] [ELO: Efficient Layer-Specific Optimization for Continual Pretraining of Multilingual LLMs](https://arxiv.org/abs/2601.03648)
*HanGyeol Yoo,ChangSu Choi,Minjun Kim,Seohyun Song,SeungWoo Song,Inho Won,Jongyoul Park,Cheoneum Park,KyungTae Lim*

Main category: cs.CL

TL;DR: 提出一种高效的层特定优化方法，通过仅训练关键层来加速多语言大模型的持续预训练，同时保持源语言性能。


<details>
  <summary>Details</summary>
Motivation: 传统持续预训练方法计算成本高，且容易导致源语言性能下降，需要一种更高效的优化方案。

Method: 1. ELO预训练：分离并训练关键的第一层和最后一层；2. 层对齐：将训练后的层重新整合，并进行简短的全模型微调。

Result: 训练速度提升最高达6.46倍，目标语言性能提升最高6.2%，同时有效保持英语能力。

Conclusion: ELO方法能显著降低计算成本和内存消耗，在提升目标语言性能的同时保护源语言能力。
...
### [163] [LLM-MC-Affect: LLM-Based Monte Carlo Modeling of Affective Trajectories and Latent Ambiguity for Interpersonal Dynamic Insight](https://arxiv.org/abs/2601.03645)
...
TL;DR: 提出LLM-MC-Affect概率框架，将情绪建模为情感空间中的连续潜在分布，通过蒙特卡洛估计生成情感轨迹，分析人际对话中的动态协调与耦合模式。


<details>
  <summary>Details</summary>
Motivation: 传统文本情感分析将情绪视为确定性的点估计，无法捕捉对话中固有的主观性、潜在模糊性和序列耦合性，需要更精细的建模方法。

Method: 采用随机LLM解码与蒙特卡洛估计，近似情感概率分布，生成情感轨迹；通过序列互相关和斜率指标分析人际耦合，识别对话中的主导或滞后影响。

Result: 在师生教学对话案例中，定量指标成功提取出高层次互动洞察（如有效教学支架），验证了方法的解释能力。

Conclusion: 该框架为理解人际动态提供了可扩展的通用解决方案，可应用于教育及更广泛的社会行为研究。
...
### [164] [Membox: Weaving Topic Continuity into Long-Range Memory for LLM Agents](https://arxiv.org/abs/2601.03785)
*Dehao Tao,Guoliang Ma,Yongfeng Huang,Minghu Jiang*

Main category: cs.CL

TL;DR: 提出membox分层记忆架构，通过Topic Loom和Trace Weaver组件显式建模话题连续性，显著提升LLM代理在时序推理任务上的性能并降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理记忆系统采用'碎片化-补偿'范式，将对话流拆分为孤立语句存储，破坏了叙事和因果流，且检索偏向词法相似性，无法保持人类对话中常见的话题连续性。

Method: 1. 提出membox分层记忆架构；2. Topic Loom组件以滑动窗口持续监控对话，将连续同话题轮次分组为连贯的'记忆盒'；3. Trace Weaver组件将密封的记忆盒链接为长程事件时间线轨迹，恢复跨间断的宏观话题复现。

Result: 在LoCoMo数据集上的实验表明：1. 时序推理任务F1提升最高达68%；2. 优于Mem0、A-MEM等基线；3. 仅需现有方法的部分上下文token，实现效率与效果的更好平衡。

Conclusion: 通过显式建模话题连续性，membox提供了一种认知启发的机制，可同时增强LLM代理的对话连贯性和计算效率。
...
### [165] [NeoAMT: Neologism-Aware Agentic Machine Translation with Reinforcement Learning](https://arxiv.org/abs/2601.03790)
*Zhongtao Miao,Kaiyan Zhao,Masaaki Nagata,Yoshimasa Tsuruoka*

Main category: cs.CL

TL;DR: 提出NeoAMT框架，通过构建多语言新词翻译数据集和基于Wiktionary的检索工具，结合强化学习提升新词感知机器翻译质量。


<details>
  <summary>Details</summary>
Motivation: 新词感知机器翻译领域研究不足，现有通用机器翻译系统难以处理新词翻译问题，需要专门的方法和数据集支持。

Method: 1. 从英语Wiktionary构建覆盖16种语言、75个翻译方向的新词翻译数据集；2. 开发基于Wiktionary的检索工具；3. 提出包含新颖奖励设计和自适应生成方法的强化学习训练框架，利用“翻译难度”优化翻译智能体。

Result: 构建了大规模多语言新词翻译数据集和检索工具，提出的强化学习框架能有效提升翻译智能体在新词翻译任务上的性能。

Conclusion: NeoAMT框架为新词感知机器翻译提供了系统解决方案，数据集和工具支持多语言场景，强化学习方法能显著改善新词翻译质量。
...
### [166] [eTracer: Towards Traceable Text Generation via Claim-Level Grounding](https://arxiv.org/abs/2601.03669)
*Bohao Chu,Qianli Wang,Hendrik Damm,Hui Wang,Ula Muhabbek,Elisabeth Livingstone,Christoph M. Friedrich,Norbert Fuhr*

Main category: cs.CL

TL;DR: 提出eTracer框架，通过后验证据对齐实现可追溯的文本生成，提升生物医学领域生成响应的可验证性和可信度。


<details>
  <summary>Details</summary>
Motivation: 在生物医学等高风险领域，如何高效验证系统生成的响应内容，确保其可追溯性和可信度。

Method: 采用后验证据对齐方法，将生成响应的每个主张与上下文证据进行支持或反驳的关联，构建主张级别的可追溯框架。

Result: 实验表明，该方法相比传统句子级证据对齐方法，显著提升了整体对齐质量和用户验证效率。

Conclusion: eTracer框架通过主张级证据对齐，实现了生成响应的精确追溯和可信度量化，为高风险领域的文本生成验证提供了有效解决方案。
...
### [167] [SyncThink: A Training-Free Strategy to Align Inference Termination with Reasoning Saturation](https://arxiv.org/abs/2601.03649)
*Gengyang Li,Wang Cai,Yifeng Gao,Yunfang Wu*

Main category: cs.CL

TL;DR: SyncThink是一种无需训练、即插即用的解码方法，通过监控模型自身的推理转换信号提前终止推理，显著减少Chain-of-Thought的推理开销和冗余痕迹。


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought提示虽然能提升推理能力，但通常会产生冗长冗余的推理轨迹，大幅增加推理成本。

Method: 基于发现答案token对早期推理关注较弱而聚焦于特殊token“/think”的现象，SyncThink通过监控模型自身的推理转换信号来提前终止推理过程，无需修改模型权重。

Result: 在多个数据集和模型上的实验表明，SyncThink仅用656个生成token和28.68秒延迟就达到62.00%的平均Top-1准确率，相比完整CoT解码（61.22%准确率、2141个token、92.01秒延迟）显著提升效率；在GPQA等长视野任务中还能通过防止过度思考获得最高+8.1的绝对准确率提升。

Conclusion: SyncThink能有效减少CoT推理开销，在保持甚至提升准确率的同时大幅降低计算成本，特别适用于长序列推理任务。
...
### [168] [Do LLMs Really Memorize Personally Identifiable Information? Revisiting PII Leakage with a Cue-Controlled Memorization Framework](https://arxiv.org/abs/2601.03791)
*Xiaoyu Luo,Yiyi Chen,Qiongxiu Li,Johannes Bjerva*

Main category: cs.CL

TL;DR: 本文提出了一种新的评估框架CRM，用于在低词汇提示条件下评估大语言模型对个人身份信息的记忆，发现先前报道的PII泄露主要是由提示驱动的表面线索而非真实记忆导致。


<details>
  <summary>Details</summary>
Motivation: 现有研究常将LLMs成功重建PII解释为记忆证据，但缺乏对提示中词汇线索影响的控制，可能导致高估记忆风险。

Method: 提出Cue-Resistant Memorization（CRM）框架，通过控制提示与目标的重叠线索，在32种语言中重新评估多种记忆范式（包括逐字补全、关联重建、无提示生成和成员推理）。

Result: 当控制表面线索后，PII重建成功率显著下降；无提示生成和成员推理的真阳性率极低，表明先前观察到的泄露主要源于提示驱动的泛化而非真实记忆。

Conclusion: LLMs的PII泄露应通过线索控制的评估来可靠量化，当前隐私风险可能被高估，需区分提示诱导行为与真实记忆。
...
### [169] [Where meaning lives: Layer-wise accessibility of psycholinguistic features in encoder and decoder language models](https://arxiv.org/abs/2601.03798)
*Taisiia Tikhomirova,Dirk U. Wulff*

Main category: cs.CL

TL;DR: 本文通过分层探测研究，发现Transformer模型中语义信息的编码位置受方法选择和架构约束的共同影响，不同心理语言学特征在不同层达到峰值。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer语言模型在何处编码具有心理学意义的语义信息，对理论发展和实际应用都至关重要。

Method: 对10个Transformer模型（包括编码器和解码器架构）进行分层探测研究，涵盖58个心理语言学特征，并比较了三种嵌入提取方法。

Result: 语义的明显定位强烈依赖于方法：上下文嵌入比孤立嵌入具有更高的特征选择性和不同的分层特征。跨模型和方法，最后一层表示很少是线性探测恢复心理语言学信息的最佳选择。尽管存在差异，模型在意义维度上表现出共享的深度排序：词汇属性较早达到峰值，而经验和情感维度较晚达到峰值。

Conclusion: Transformer模型中语义的“位置”反映了方法论选择与架构约束之间的相互作用。
...
### [170] [DisastQA: A Comprehensive Benchmark for Evaluating Question Answering in Disaster Management](https://arxiv.org/abs/2601.03670)
*Zhitong Chen,Kai Yin,Xiangjue Dong,Chengkai Liu,Xiangpeng Li,Yiming Xiao,Bo Li,Junwei Ma,Ali Mostafavi,James Caverlee*

Main category: cs.CL

TL;DR: 提出了DisastQA灾难管理问答基准，包含3000个经过验证的问题，评估模型在不确定和冲突信息下的推理能力，发现现有模型在真实噪声环境下性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有问答基准基于干净证据构建，无法有效评估灾难管理场景中处理不确定和冲突信息的能力，需要更贴近现实噪声环境的评估框架。

Method: 通过人机协作流水线构建包含8种灾难类型、3000个验证问题的基准（2000个选择题+1000个开放题），采用分层抽样确保平衡覆盖，并提出基于关键点的人工验证评估协议。

Result: 测试20个模型发现：在干净环境下开源模型接近专有系统，但在噪声环境下性能急剧下降；灾难场景表现与通用排行榜（如MMLU-Pro）存在显著差异。

Conclusion: 灾难响应需要专门评估模型在噪声信息下的可靠性，当前模型存在关键可靠性缺口，公开了完整代码、数据和评估资源以促进相关研究。
...
### [171] [AI Generated Text Detection](https://arxiv.org/abs/2601.03812)
*Adilkhan Alikhanov,Aidar Amangeldi,Diar Demeubay,Dilnaz Akhmetzhan,Nurbek Moldakhmetov,Omar Polat,Galymzhan Zharas*

Main category: cs.CL

TL;DR: 本文评估了多种AI文本检测方法，发现深度学习模型（特别是DistilBERT）在检测学生使用大语言模型生成的学术不端内容方面表现最佳，准确率达88%以上，优于传统机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，学生越来越多地将AI生成内容当作自己的作业提交，这违反了学术诚信，因此需要有效的AI文本检测方法来应对这一问题。

Method: 使用HC3和DAIGT v2两个数据集构建统一基准，采用基于主题的数据分割防止信息泄露；评估了传统机器学习（TF-IDF逻辑回归）和深度学习模型（BiLSTM、DistilBERT）。

Result: TF-IDF逻辑回归准确率为82.87%；BiLSTM准确率为88.86%；DistilBERT准确率为88.11%且ROC-AUC得分最高（0.96），表现最优。结果表明上下文语义建模显著优于词法特征。

Conclusion: 深度学习模型在AI文本检测中表现更佳，适当的评估协议对防止主题记忆至关重要；未来将扩展数据集多样性，采用参数高效微调方法（如LoRA），并探索更小模型和硬件优化策略。
...
### [172] [NeuronScope: A Multi-Agent Framework for Explaining Polysemantic Neurons in Language Models](https://arxiv.org/abs/2601.03671)
*Weiqi Liu,Yongliang Miao,Haiyan Zhao,Yanguang Liu,Mengnan Du*

Main category: cs.CL

TL;DR: 提出NeuronScope多智能体框架，通过迭代激活引导过程解决大语言模型中神经元多义性问题，显著提升解释质量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型中普遍存在的神经元多义性（单个神经元响应多个语义概念）使现有单次解释方法难以准确捕捉其多概念行为。

Method: NeuronScope框架将神经元解释重构为迭代激活引导过程：将神经元激活分解为原子语义成分，聚类为不同语义模式，并利用激活反馈迭代优化每个解释。

Result: 实验表明NeuronScope能揭示隐藏的多义性，相比单次基线方法产生显著更高激活相关性的解释。

Conclusion: NeuronScope通过多智能体迭代方法有效解决了神经元多义性挑战，为LLM神经元解释提供了更准确的框架。
...
### [173] [Evaluation Framework for AI Creativity: A Case Study Based on Story Generation](https://arxiv.org/abs/2601.03698)
*Pharath Sathya,Yin Jou Huang,Fei Cheng*

Main category: cs.CL

TL;DR: 提出一个包含四个组件和十一个子组件的结构化评估框架，用于评估AI故事生成的创造力，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于参考的评估指标无法捕捉创造力的主观性，导致创造性文本生成评估面临挑战。

Method: 使用'Spike Prompting'控制故事生成，并通过115名读者的众包研究，分析不同创造力组件如何影响人类即时和反思性创造力判断。

Result: 创造力评估是分层而非累积的，不同维度在不同判断阶段变得显著；反思性评估显著改变评分和评分者间一致性。

Conclusion: 该框架能有效揭示被基于参考的评估所掩盖的创造力维度，为创造性文本生成评估提供了更全面的方法。
...
### [174] [RedBench: A Universal Dataset for Comprehensive Red Teaming of Large Language Models](https://arxiv.org/abs/2601.03699)
*Quy-Anh Dang,Chris Ngo,Truong-Son Hy*

Main category: cs.CL

TL;DR: 提出了RedBench数据集，整合37个基准数据集并标准化风险分类，用于全面评估大语言模型对抗性提示的鲁棒性。
...
Method: 整合37个公开数据集（共29,362个样本），建立包含22个风险类别和19个领域的标准化分类体系，并提供开源评估代码。

Result: 构建了统一的评估数据集，为现代大语言模型建立了基线，促进了模型脆弱性的系统化比较分析。

Conclusion: RedBench数据集支持鲁棒性比较，推动未来研究，促进开发安全可靠的大语言模型用于实际部署。
...
### [175] [What Matters For Safety Alignment?](https://arxiv.org/abs/2601.03868)
*Xing Li,Hui-Ling Zhen,Lihao Yin,Xianzhi Yu,Zhenhua Dong,Mingxuan Yuan*

Main category: cs.CL

TL;DR: 对32个近期流行的LLM和LRM进行大规模安全对齐能力评估，发现集成推理和自反思机制能显著提升安全性，后训练和知识蒸馏可能导致安全对齐退化，CoT攻击通过响应前缀可将攻击成功率平均提升3.34倍，角色扮演、提示注入和梯度搜索是主要攻击方法。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型和推理模型的安全对齐能力，为开发更安全可靠的AI系统提供关键见解，系统研究内在模型特性和外部攻击技术对安全性的影响。

Method: 使用5个安全数据集和56种越狱技术、4种CoT攻击策略，对32个模型（参数规模3B-235B）进行460万次API调用评估，系统比较6个关键内在模型特性和3种外部攻击技术的影响。

Result: 1. GPT-OSS-20B等集成推理和自反思机制的模型安全性最佳；2. 后训练和知识蒸馏可能导致安全对齐系统性退化；3. CoT攻击通过响应前缀可使攻击成功率平均提升3.34倍，Seed-OSS-36B-Instruct从0.6%升至96.3%；4. 角色扮演、提示注入和梯度搜索是主要攻击方法。

Conclusion: 安全必须作为后训练阶段的核心优化目标而非附属品；文本补全接口和用户自定义响应前缀功能存在重大安全风险，需架构和部署层面的防护措施；集成推理和自反思机制对安全对齐至关重要。
...
### [176] [AirNav: A Large-Scale Real-World UAV Vision-and-Language Navigation Dataset with Natural and Diverse Instructions](https://arxiv.org/abs/2601.03707)
*Hengxing Cai,Yijie Rao,Ligang Huang,Zanyang Zhong,Jinhan Dong,Jingjun Tan,Wenhao Lu,Renxin Zhong*

Main category: cs.CL

TL;DR: 提出了AirNav——一个基于真实城市航拍数据构建的大规模无人机视觉语言导航基准数据集，并开发了AirVLN-R1模型来提升导航性能与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉语言导航数据集依赖虚拟环境、指令不够自然且规模有限，需要基于真实场景、指令自然多样的大规模基准来推动相关研究。

Method: 1. 构建AirNav数据集：使用真实城市航拍数据，生成自然多样的导航指令；2. 提出AirVLN-R1模型：结合监督微调与强化微调进行训练。

Result: 模型通过真实场景测试初步验证了可行性，数据集与代码已公开。

Conclusion: AirNav解决了现有数据集的局限性，为无人机视觉语言导航研究提供了更贴近实际应用的基准与工具。
...
### [177] [Large-Scale Aspect-Based Sentiment Analysis with Reasoning-Infused LLMs](https://arxiv.org/abs/2601.03940)
*Paweł Liskowski,Krzysztof Jankowski*

Main category: cs.CL

TL;DR: 提出了Arctic-ABSA模型，用于现实场景下的方面级情感分析，通过扩展情感类别、多语言支持和推理增强技术，在多个基准测试中超越GPT-4o等模型，并发布了大规模基准数据集ABSA-mix。


<details>
  <summary>Details</summary>
Motivation: 现有方面级情感分析模型通常只支持三种基本情感类别，且缺乏对商业应用场景和多语言环境的适配，需要更强大的模型来处理复杂现实场景。

Method: 1. 使用大规模公开数据和合成数据训练模型，数据集比SemEval14大20倍；2. 将情感类别从3类扩展至5类（增加混合和未知类别）；3. 联合预测整体文本情感；4. 采用思维链微调和新型推理预训练技术；5. 开发395M参数编码器和8B参数解码器模型。

Result: 1. 在SemEval14基准测试中达到新的最优性能；2. 比GPT-4o和Claude 3.5 Sonnet准确率高出10个百分点；3. 单模型在六种语言上保持87-91%准确率且不影响英语性能；4. 发布了包含17个公开数据集、覆盖92个领域的ABSA-mix基准。

Conclusion: Arctic-ABSA模型通过扩展情感类别、多语言支持和推理增强技术，显著提升了方面级情感分析的性能，适用于商业应用场景，并为该领域提供了新的大规模基准数据集。
...
### [178] [Visual Merit or Linguistic Crutch? A Close Look at DeepSeek-OCR](https://arxiv.org/abs/2601.03714)
*Yunhao Liang,Ruixuan Ying,Bo Li,Hong Li,Kai Yan,Qingwen Li,Min Yang,Okamoto Satoshi,Zhe Cui,Shiwen Ni*

Main category: cs.CL

TL;DR: 本文通过语义破坏实验评估DeepSeek-OCR的性能驱动力，发现其严重依赖语言先验而非视觉能力，在无语言支持时性能从90%骤降至20%，且传统OCR方法比端到端方法更稳健。


<details>
  <summary>Details</summary>
Motivation: 探究DeepSeek-OCR声称的高比例视觉-文本压缩性能究竟源于其光学字符识别能力，还是依赖语言先验知识，以明确其实际能力边界及对长上下文瓶颈的影响。

Method: 采用句子级和单词级语义破坏来隔离模型的固有OCR能力与语言先验；对13个基线模型进行对比基准测试；分析视觉令牌数量与先验依赖的关系；进行上下文压力测试。

Result: 无语言支持时，DeepSeek-OCR性能从约90%暴跌至20%；传统流水线OCR方法比端到端方法对语义扰动更具鲁棒性；视觉令牌越少，对先验的依赖越大，幻觉风险越高；模型在约10,000文本令牌处完全崩溃。

Conclusion: 当前光学压缩技术可能加剧长上下文瓶颈；研究明确了DeepSeek-OCR的能力边界，为未来视觉-文本压缩范式的优化提供了关键见解。
...
### [179] [MIND: From Passive Mimicry to Active Reasoning through Capability-Aware Multi-Perspective CoT Distillation](https://arxiv.org/abs/2601.03717)
*Jin Cui,Jiaqi Guo,Jiepeng Zhou,Ruixuan Yang,Jiayi Lu,Jiajun Xu,Jiangcheng Song,Boran Zhao,Pengju Ren*

Main category: cs.CL

TL;DR: 提出了MIND框架，通过能力自适应机制将大语言模型的推理能力蒸馏到小模型中，解决了现有方法中教师最优推理路径可能成为分布外噪声的问题，实现了领域性能和跨域泛化的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法在将大语言模型的推理能力蒸馏到小模型时，通常强制学生遵循单一教师推理路径，但由于师生模型存在不同的归纳偏好和内在偏好，加上学生训练过程中能力不断变化，教师的'最优'推理可能成为分布外噪声，导致学生潜在推理分布退化，性能次优。

Method: 提出MIND框架，通过'教学助理'网络合成多样化的教师视角，并采用反馈驱动的惯性校准机制，利用惯性过滤的训练损失使监督信号与学生当前适应能力对齐，实现从被动模仿到主动认知构建的转变。

Result: 大量实验表明，MIND在分布内和分布外基准测试中都达到了最先进的性能，潜在空间分析进一步证实了推理能力内化的机制。

Conclusion: MIND框架通过能力自适应的知识蒸馏，有效解决了师生模型对齐问题，在保持领域性能的同时提升了跨域泛化能力，为小模型获得复杂推理能力提供了新途径。
...
### [180] [Layer-wise Positional Bias in Short-Context Language Modeling](https://arxiv.org/abs/2601.04098)
*Maryam Rahimi,Mahdi Nouri,Yadollah Yaghoobzadeh*

Main category: cs.CL

TL;DR: 本文提出了一种基于归因的框架，用于分析短上下文语言建模中的位置效应，揭示了不同层对输入位置的重要性分布模式及其与词类的关系。


<details>
  <summary>Details</summary>
Motivation: 语言模型往往倾向于使用输入中特定位置的信息，而不考虑语义相关性。先前研究未系统分析这种位置偏置在不同层和输入位置间的演变规律，也未探讨其独立于任务复杂性的变化。

Method: 引入基于归因的分析框架，采用层传导度与滑动窗口方法，量化每一层在输入位置间的重要性分布，生成层间位置重要性剖面。

Result: 位置重要性剖面具有架构特异性、跨输入稳定性及词汇打乱不变性；模型呈现显著的近因偏置（随深度增加）和微弱的首因偏置（随深度减弱）；早期层在所有位置上更重视实词而非虚词，而深层则失去这种词类区分。

Conclusion: 语言模型的位置偏置具有系统性的层间演化模式，且与词类处理机制存在关联，这为理解模型内部表示的形成提供了新视角。
...
### [181] [Stuttering-Aware Automatic Speech Recognition for Indonesian Language](https://arxiv.org/abs/2601.03727)
*Fadhil Muhammad,Alwin Djuliansah,Adrian Aryaputra Hamzah,Kurniawati Azizah*

Main category: cs.CL

TL;DR: 提出了一种为低资源语言（印尼语）生成合成口吃语音的数据增强框架，通过微调预训练Whisper模型提升对口吃语音的识别性能，同时保持对流畅语音的识别能力。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在处理流畅语音时表现优异，但在处理口吃语音时性能显著下降，尤其对于印尼语等低资源语言，缺乏专门的数据集。

Method: 1. 通过基于规则的转换和大语言模型向流畅文本注入重复和延长等口吃特征；2. 使用文本转语音合成生成合成口吃音频；3. 利用合成数据通过迁移学习微调预训练的印尼语Whisper模型。

Result: 实验表明，这种针对性的合成数据暴露能持续减少对口吃语音的识别错误，同时保持对流畅语音片段的识别性能。
...
### [182] [InfiniteWeb: Scalable Web Environment Synthesis for GUI Agent Training](https://arxiv.org/abs/2601.04126)
*Ziyun Zhang,Zezhou Wang,Xiaoyi Zhang,Zongyu Guo,Jiahao Li,Bin Li,Yan Lu*

Main category: cs.CL

TL;DR: 提出了InfiniteWeb系统，用于自动生成大规模功能性网页环境以训练GUI代理，解决了训练环境稀缺的问题，并在实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: GUI代理作为实用AI助手具有潜力，但训练这类代理面临合适环境稀缺的阻碍，需要可扩展的解决方案来生成真实且功能完整的网页环境。

Method: 采用统一规范、以任务为中心的测试驱动开发，结合网站种子和参考设计图像确保多样性，并生成可验证的任务评估器以提供强化学习的密集奖励信号。

Result: InfiniteWeb在构建真实网站方面超越商业编码代理，在其生成环境中训练的GUI代理在OSWorld和Online-Mind2Web基准测试中取得显著性能提升。

Conclusion: InfiniteWeb系统能有效生成大规模功能性网页环境，显著提升GUI代理的训练效果，为实用AI助手的发展提供了有力支持。
...
### [183] [Whose Facts Win? LLM Source Preferences under Knowledge Conflicts](https://arxiv.org/abs/2601.03746)
*Jakob Schuster,Vagrant Gautam,Katja Markert*

Main category: cs.CL

TL;DR: 研究大型语言模型在知识冲突中的来源偏好，发现模型倾向于机构来源信息，但重复效应会逆转偏好，并提出降低重复偏差的方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在检索增强生成中的应用增加，需要研究其在知识冲突下的行为，特别是信息来源可信度的影响。

Method: 提出评估框架，系统测试13个开源大语言模型在英语知识冲突中的表现，分析来源偏好及重复效应，并提出降低重复偏差的方法。

Result: 模型偏好机构来源信息（如政府、报纸），但重复低可信度来源信息会逆转偏好；提出的方法能降低99.8%重复偏差，保持88.8%原始偏好。

Conclusion: 信息来源可信度显著影响大语言模型的知识冲突解决，重复效应会干扰来源偏好，提出的方法能有效缓解此问题。
...
### [184] [ContextFocus: Activation Steering for Contextual Faithfulness in Large Language Models](https://arxiv.org/abs/2601.04131)
*Nikhil Anand,Shwetha Somasundaram,Anirudh Phukan,Apoorv Saxena,Koyel Mukherjee*

Main category: cs.CL

TL;DR: 提出ContextFocus方法，通过轻量级激活引导提升大语言模型在知识冲突场景下的上下文忠实度，无需微调且推理开销小。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在预训练中编码了大量参数化知识，当外部检索证据与模型内部知识冲突时，模型常依赖记忆而非上下文，导致输出不忠实。

Method: 采用无需模型微调的轻量级激活引导方法，在推理时通过调整激活向量提升对上下文的关注，并与提示策略互补。

Result: 在ConFiQA基准测试中显著优于ContextDPO、COIECD等基线方法，且在大模型上保持有效性，兼顾流畅性与效率。

Conclusion: ContextFocus能有效、鲁棒且高效地提升LLM输出的上下文忠实度，为知识冲突场景提供了实用解决方案。
...
### [185] [Do LLM Self-Explanations Help Users Predict Model Behavior? Evaluating Counterfactual Simulatability with Pragmatic Perturbations](https://arxiv.org/abs/2601.03775)
*Pingjun Hong,Benjamin Roth*

Main category: cs.CL

TL;DR: 研究评估了大型语言模型（LLM）的自我解释是否能帮助用户预测模型行为，通过反事实模拟任务发现，虽然解释能提升预测准确性，但效果受扰动策略和评估者能力影响显著。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM能生成自我解释，但这些解释可能无法真实反映模型的决策过程。研究旨在探究这些解释是否仍能帮助用户（包括人类和LLM评估者）预测模型在反事实场景下的行为。

Method: 使用StrategyQA数据集，比较人类和LLM评估者在有无模型解释（思维链或事后解释）的情况下，预测模型对反事实后续问题答案的能力。采用LLM生成的反事实和基于语用学的扰动两种方法构建测试用例。

Result: 自我解释能持续提升LLM评估者和人类的模拟准确性，但提升程度和稳定性高度依赖扰动策略和评估者能力。人类用户的自由文本理由分析表明，解释有助于他们在扰动问题上形成更准确的预测。

Conclusion: LLM的自我解释虽不完美，但在反事实模拟任务中能有效辅助行为预测，其实际效用受测试用例构建方式和评估者水平的影响。
...
### [186] [Tracing the complexity profiles of different linguistic phenomena through the intrinsic dimension of LLM representations](https://arxiv.org/abs/2601.03779)
*Marco Baroni,Emily Cheng,Iria deDios-Flores,Francesca Franzon*

Main category: cs.CL

TL;DR: 本文通过分析LLM表示的内在维度来研究语言复杂性，发现ID能有效区分形式与功能复杂性，并揭示不同LLM中相似的语言处理阶段。


<details>
  <summary>Details</summary>
Motivation: 探索LLM表示的内在维度是否可作为语言复杂性的标记，并检验不同ID模式能否区分形式复杂性和功能复杂性。

Method: 使用内在维度分析、表征相似性比较和层消融实验，对比不同句法结构（并列/从属从句、右分支/中心嵌套、明确/模糊关系从句附着）在LLM各层的ID表现。

Result: 形式复杂性（多从句结构）的ID差异与先前研究中发现的抽象语言处理阶段一致；功能复杂性的ID差异较弱且不关联相同处理阶段；表征相似性和层消融实验验证了相同趋势。

Conclusion: ID是LLM中有效的语言复杂性标记，能区分不同类型的复杂性，并指示不同LLM中相似的语言处理阶段。
...
### [187] [Compact Example-Based Explanations for Language Models](https://arxiv.org/abs/2601.03786)
*Loris Schoenegger,Benjamin Roth*

Main category: cs.CL

TL;DR: 本文提出了一种新的选择相关性评分方法，用于评估训练数据影响估计中示例选择策略的有效性，并发现常见策略常不如随机选择，进而提出了一种平衡影响力和代表性的改进策略。


<details>
  <summary>Details</summary>
Motivation: 训练数据影响估计方法可用于基于示例的解释，但人类无法处理大量文档，需选择少量示例呈现。现有研究忽略了选择策略对解释质量的影响，因此需要评估和改进选择方法。

Method: 提出了一种无需重新训练的选择相关性评分指标，通过微调实验验证其有效性，并比较了常见选择策略与随机选择的性能，最终提出了一种平衡影响力和代表性的选择策略。

Result: 验证了选择相关性评分能预测示例集对模型预测的支持或削弱作用；发现常见选择策略常不如随机选择；提出的平衡策略能更有效地利用选择预算。

Conclusion: 选择策略对基于影响的解释至关重要，常见策略存在不足，平衡影响力和代表性的方法能提升解释质量，为未来研究提供了评估和改进方向。
...
### [188] [HearSay Benchmark: Do Audio LLMs Leak What They Hear?](https://arxiv.org/abs/2601.03783)
...
TL;DR: 本文首次系统研究音频大语言模型（ALLMs）通过声纹泄露用户隐私的风险，构建了包含2.2万条真实音频的基准测试HearSay，发现ALLMs存在严重隐私泄露、安全机制不足、推理过程加剧风险三大问题。


<details>
  <summary>Details</summary>
Motivation: 音频大语言模型在理解和生成方面进展显著，但其隐私影响尚未被充分探索。研究旨在揭示ALLMs是否通过声纹无意中泄露用户隐私，填补该领域的研究空白。

Method: 构建HearSay基准数据集，包含超过22,000条真实音频片段，通过自动化分析和人工验证确保数据质量；设计实验评估ALLMs从声纹中提取隐私属性的能力，测试现有安全机制的防御效果，并分析链式推理对隐私风险的影响。

Result: 1. 显著隐私泄露：ALLMs能从声纹中提取隐私属性，性别识别准确率达92.89%，并能有效推断社会属性；2. 安全机制不足：现有防护措施严重欠缺，多数模型无法拒绝隐私侵犯请求，生理特征相关请求的拒绝率接近零；3. 推理加剧风险：链式推理会通过挖掘更深层声学关联，在能力较强的模型中进一步放大隐私风险。

Conclusion: ALLMs存在严重的隐私泄露漏洞，现有安全措施完全不足，迫切需要针对性的隐私对齐方案。研究呼吁社区重视音频模型的隐私风险，并开源了代码和数据集以促进后续研究。
...
### [189] [VietMed-MCQ: A Consistency-Filtered Data Synthesis Framework for Vietnamese Traditional Medicine Evaluation](https://arxiv.org/abs/2601.03792)
*Huynh Trung Kiet,Dao Sy Duy Minh,Nguyen Dinh Ha Duong,Le Hoang Minh Huy,Long Nguyen,Dien Dinh*

Main category: cs.CL

TL;DR: 提出了越南传统医学多选题数据集VietMed-MCQ，通过RAG和双重验证确保质量，评估发现具有中文先验的通用模型优于越南专用模型，但所有模型在复杂诊断推理上仍有困难。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用医学领域表现优异，但在越南传统医学等专业文化特定领域性能显著下降，主要由于缺乏高质量结构化基准。

Method: 使用检索增强生成（RAG）管道生成多选题数据集，采用双重模型验证机制确保推理一致性，通过医学专家和学生进行人工验证。

Result: 构建了3,190个问题的数据集，专家验证通过率达94.2%，评估者间一致性高（Fleiss' kappa=0.82）。基准测试显示具有中文先验的通用模型优于越南专用模型，突显跨语言概念迁移。

Conclusion: VietMed-MCQ数据集填补了低资源医学领域基准的空白，所有模型在复杂诊断推理上仍需改进，代码和数据集已公开以促进相关研究。
...
### [190] [Rethinking Table Pruning in TableQA: From Sequential Revisions to Gold Trajectory-Supervised Parallel Search](https://arxiv.org/abs/2601.03851)
*Yu Guo,Shenghao Ye,Shuangwu Chen,Zijian Wen,Tao Zhang,Qirui Bai,Dong Jin,Yunpeng Hou,Huasen He,Jian Yang,Xiaobin Tan*

Main category: cs.CL

TL;DR: 提出TabTrim框架，将表格剪枝从顺序修订转为并行搜索，通过监督学习对齐黄金轨迹，提升表格问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有表格剪枝方法依赖不可靠的批判信号进行顺序修订，易丢失关键数据，影响下游推理效果。

Method: TabTrim利用黄金SQL查询的中间子表生成黄金剪枝轨迹，训练剪枝器和验证器对齐轨迹；推理时并行搜索候选轨迹以选择最优子表。

Result: TabTrim-8B在WikiTQ和TableBench等任务上平均准确率达73.5%，优于最强基线3.2%，其中WikiTQ为79.4%，TableBench为61.2%。

Conclusion: TabTrim通过并行搜索和监督学习有效提升表格剪枝质量，在多样化表格推理任务中实现最先进性能。
...
### [191] [Step Potential Advantage Estimation: Harnessing Intermediate Confidence and Correctness for Efficient Mathematical Reasoning](https://arxiv.org/abs/2601.03823)
*Fei Wu,Zhenrong Zhang,Qikai Chang,Jianshu Zhang,Quan Liu,Jun Du*

Main category: cs.CL

TL;DR: 本文提出SPAE方法，通过提取中间置信度和正确性构建Step Potential信号，实现细粒度信用分配，改善强化学习中推理过程的监督不足问题，在多个基准测试中提升准确性并显著减少响应长度。


<details>
  <summary>Details</summary>
Motivation: 现有基于结果的强化学习方法在长链推理中优势估计粗糙，缺乏语义层面的逐步推理进展度量，导致模型无法区分必要推导和冗余验证，甚至可能将正确轨迹推翻为错误答案。

Method: 提出训练无关的探测机制提取中间置信度和正确性，组合成Step Potential信号；基于此提出Step Potential Advantage Estimation（SPAE）方法，进行细粒度信用分配，放大潜在收益、惩罚潜在下降，并在潜力饱和后施加惩罚以鼓励及时终止。

Result: 在多个基准测试中，SPAE consistently improves accuracy while substantially reducing response length, outperforming strong RL baselines and recent efficient reasoning and token-level advantage estimation methods.

Conclusion: SPAE通过细粒度的过程监督和信用分配，有效解决了强化学习中推理过程的监督不足问题，在提升准确性的同时显著减少了响应长度，优于现有方法。
...
### [192] [What Does Loss Optimization Actually Teach, If Anything? Knowledge Dynamics in Continual Pre-training of LLMs](https://arxiv.org/abs/2601.03858)
*Seyed Mahed Mousavi,Simone Alghisi,Giuseppe Riccardi*

Main category: cs.CL

TL;DR: 该研究揭示了持续预训练（CPT）中损失优化与知识学习之间的系统性偏差，发现知识获取不稳定、难以巩固，且会导致领域外技能退化。


<details>
  <summary>Details</summary>
Motivation: 当前持续预训练（CPT）普遍将损失作为知识学习的代理指标，但缺乏对训练过程中知识实际变化机制的理解。研究旨在将CPT视为知识学习过程而非单纯优化问题，探究其动态规律。

Method: 构建了分布匹配的事实文档基准数据集，在CPT循环中嵌入诊断探针，实现知识获取动态和领域外技能的周期级测量；分析了三种指令调优LLM和多种CPT策略下知识回路的重构过程。

Result: 损失单调下降时，知识学习呈现不稳定和非单调性；获取的事实知识难以巩固，学习严重依赖先验暴露；领域外性能从早期周期开始退化；知识回路在周期间快速重构，解释了知识获取窗口狭窄和系统性遗忘现象。

Conclusion: 损失优化与CPT中的学习进程存在错位，建议基于任务级学习动态而非损失曲线来评估和制定训练停止准则。
...
### [193] [Atlas: Orchestrating Heterogeneous Models and Tools for Multi-Domain Complex Reasoning](https://arxiv.org/abs/2601.03872)
*Jinyang Wu,Guocheng Zhai,Ruihan Jin,Jiahao Yuan,Yuhao Shen,Shuai Zhang,Zhengqi Wen,Jianhua Tao*

Main category: cs.CL

TL;DR: 提出ATLAS框架，通过双路径方法动态选择大语言模型与工具的最佳组合，提升跨领域复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和外部工具多样性增加，选择最优的模型-工具组合成为高维优化难题，现有方法无法充分利用异构模型-工具对的性能差异。

Method: 采用双路径框架：1) 基于训练免费的聚类路由，利用经验先验实现领域特定对齐；2) 基于强化学习的多步路由，探索自主轨迹以实现分布外泛化。

Result: 在15个基准测试中，ATLAS超越GPT-4o等闭源模型，在分布内任务上提升10.1%，分布外任务上提升13.1%，并在视觉推理中通过协调多模态工具获得显著增益。

Conclusion: ATLAS框架能有效解决模型-工具组合选择问题，通过自适应路由机制显著提升跨领域复杂推理性能，尤其在分布外泛化和多模态任务中表现突出。
...
### [194] [Decide Then Retrieve: A Training-Free Framework with Uncertainty-Guided Triggering and Dual-Path Retrieval](https://arxiv.org/abs/2601.03908)
*Wang Chen,Guanqiang Qi,Weikang Li,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.CL

TL;DR: 提出无需训练的DTR框架，通过生成不确定性自适应触发检索，采用双路径机制选择外部信息，在多项QA基准测试中优于标准RAG方法。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法存在盲目触发检索、依赖单一路径证据构建的问题，导致噪声干扰和性能提升有限。

Method: 基于生成不确定性动态判断检索必要性，设计双路径检索机制（稀疏/模糊查询分别处理），结合自适应信息选择策略。

Result: 在五个开放域QA基准测试中，DTR在不同模型规模和检索器上均显著提升EM/F1分数，同时减少不必要的检索操作。

Conclusion: DTR框架通过智能检索决策和双路径信息选择，有效提升RAG系统的准确性和效率，为动态知识集成提供新思路。
...
### [195] [PartisanLens: A Multilingual Dataset of Hyperpartisan and Conspiratorial Immigration Narratives in European Media](https://arxiv.org/abs/2601.03860)
...
TL;DR: 该研究创建了首个多语言超党派新闻标题数据集PartisanLens，评估了LLM在检测超党派叙事和人口替代阴谋论方面的性能，并探索了基于社会经济学和意识形态背景的LLM标注模拟。


<details>
  <summary>Details</summary>
Motivation: 现有资源稀缺、以英语为中心，且往往孤立分析超党派性、立场和修辞偏见，而非将其视为政治话语的相互关联方面。超党派叙事和人口替代阴谋论对政治极化、制度信任和公共安全构成严重威胁，亟需多语言检测工具。

Method: 1. 构建包含1617条西班牙语、意大利语和葡萄牙语超党派新闻标题的多语言数据集PartisanLens，并进行多维度政治话语标注。2. 评估广泛使用的LLM在该数据集上的分类性能。3. 分析LLM作为自动标注器的可行性。4. 探索基于社会经济和意识形态背景条件化的LLM标注模拟。

Result: LLM在超党派叙事和PRCT检测任务中表现出潜力，但也存在局限性；基于背景条件化的LLM能够模拟人类标注模式；研究提供了多语言基准数据集和评估框架。

Conclusion: PartisanLens数据集为欧洲语境下的党派和阴谋论叙事检测研究提供了资源支持，LLM在该领域具有应用前景但需进一步改进，背景条件化方法为理解标注偏差提供了新视角。
...
### [196] [Evaluating Small Decoder-Only Language Models for Grammar Correction and Text Simplification](https://arxiv.org/abs/2601.03874)
*Anthony Lamelas*

Main category: cs.CL

TL;DR: 研究探讨小型解码器语言模型在语法纠错和文本简化任务上能否替代大型语言模型，实验表明当前小型模型性能仍不及大型模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽在多种任务上表现优异，但其庞大的规模和计算成本导致部署困难、访问受限且存在安全隐患，因此需要探索更高效的替代方案。

Method: 在JFLEG和ASSET数据集上，测试小型语言模型的三种使用方式：直接使用、微调后使用、以及序列化运行，并采用已有评估指标进行性能比较。

Result: 小型语言模型虽能学习某些特定行为，但其整体性能仍低于强基线模型和当前大型语言模型，且在保持原意和避免幻觉方面存在明显不足。

Conclusion: 尽管小型语言模型具有效率优势，但在改写任务上尚无法与现有大型语言模型竞争，需要进一步改进训练方法以缩小性能差距。
...
### [197] [When Models Decide and When They Bind: A Two-Stage Computation for Multiple-Choice Question-Answering](https://arxiv.org/abs/2601.03914)
*Hugh Mee Wong,Rick Nouwen,Albert Gatt*

Main category: cs.CL

TL;DR: 本文通过表征分析和因果干预研究语言模型在多项选择题问答中的内部机制，发现模型采用两阶段机制：先在内容空间选择正确答案，再将答案绑定到输出符号。


<details>
  <summary>Details</summary>
Motivation: 多项选择题问答容易评估，但模型需要同时解决问题并输出代表答案的符号，这混淆了推理错误与符号绑定失败。研究旨在揭示语言模型如何内部实现MCQA任务。

Method: 使用表征分析（PCA、线性探针）和因果干预方法，分析选项边界（换行符）残差状态，并进行符号与内容置换测试。

Result: 发现选项边界残差状态包含与每个选项正确性相关的强线性可解码信号；胜者识别探针显示两阶段进展：获胜内容位置在最终选项处理后立即可解码，而输出符号在答案发射位置附近才被表示。

Conclusion: 模型采用两阶段机制：首先在内容空间选择胜者，然后将该胜者绑定或路由到适当的符号进行输出。
...
### [198] [RADAR: Retrieval-Augmented Detector with Adversarial Refinement for Robust Fake News Detection](https://arxiv.org/abs/2601.03981)
*Song-Duo Ma,Yi-Hung Liu,Hsin-Yu Lin,Pin-Yu Chen,Hong-Yan Huang,Shau-Yung Hsu,Yun-Nung Chen*

Main category: cs.CL

TL;DR: 提出RADAR系统，通过检索增强和对抗性优化来检测LLM生成的虚假新闻，在基准测试中达到86.98% ROC-AUC。


<details>
  <summary>Details</summary>
Motivation: 为有效应对LLM生成的虚假信息传播，需要开发更鲁棒的检测方法。

Method: 采用生成器对真实文章进行事实扰动改写，配合基于密集段落检索的轻量检测器，并引入结构化自然语言批评的对抗反馈机制。

Result: 在虚假新闻检测基准上显著优于通用检索增强LLM，消融研究表明检索组件贡献最大，对抗反馈和少样本示例对鲁棒训练至关重要。

Conclusion: RADAR通过生成器-检测器协同进化和结构化对抗反馈，实现了对LLM生成虚假新闻的有效检测。
...
### [199] [Doc-PP: Document Policy Preservation Benchmark for Large Vision-Language Models](https://arxiv.org/abs/2601.03926)
*Haeun Jang,Hwan Chang,Hwanhee Lee*

Main category: cs.CL

TL;DR: 提出了Doc-PP基准测试，揭示大视觉语言模型在文档问答中存在推理诱导的安全漏洞，并提出了DVA框架来增强策略合规性。


<details>
  <summary>Details</summary>
Motivation: 现有安全研究主要关注隐式社会规范或纯文本场景，忽视了多模态文档在动态用户定义策略下的复杂安全需求，导致模型在需要跨模态推理时容易泄露敏感信息。

Method: 1. 构建Doc-PP基准测试，包含真实场景报告和严格非披露策略；2. 提出DVA框架，将推理过程分解为感知、验证和聚合三个阶段，实现推理与策略验证的解耦。

Result: 1. 发现系统性的推理诱导安全漏洞：模型在需要复杂跨模态推理时频繁泄露敏感信息；2. 提取文本虽改善感知但加剧泄露风险；3. DVA框架显著优于标准提示防御方法。

Conclusion: 多模态文档问答需要专门的安全机制，DVA框架通过结构化推理设计有效缓解策略违规问题，为策略合规的文档理解提供了鲁棒基线。
...
### [200] [Benchmark^2: Systematic Evaluation of LLM Benchmarks](https://arxiv.org/abs/2601.03986)
...
TL;DR: 提出了Benchmark^2框架，包含三个指标来评估大语言模型评测基准的质量，通过实验发现现有基准质量差异显著，并证明基于这些指标筛选测试集可减少测试量同时保持评估效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型评测基准的快速涌现使得评估基准本身质量的需求日益迫切，需要系统性的方法来衡量基准的可靠性和有效性。

Method: 提出Benchmark^2框架，包含三个指标：跨基准排名一致性、区分度评分和能力对齐偏差。在15个涵盖数学、推理和知识领域的基准上对11个大语言模型进行了实验验证。

Result: 分析显示现有基准质量存在显著差异；基于所提指标的筛选方法能够用大幅减少的测试集达到可比的评估性能。

Conclusion: Benchmark^2框架为评估大语言模型评测基准提供了系统方法，有助于识别高质量基准并优化测试集构建，提升评估效率与可靠性。
...
### [201] [Simulated Students in Tutoring Dialogues: Substance or Illusion?](https://arxiv.org/abs/2601.04025)
*Alexander Scarlatos,Jaewook Lee,Simon Woodhead,Andrew Lan*

Main category: cs.CL

TL;DR: 本文针对LLM驱动的教育应用中模拟学生质量评估不足的问题，提出了系统化的评估框架，并通过实验发现现有模拟方法效果有限，需进一步改进。


<details>
  <summary>Details</summary>
Motivation: LLM在教育领域的应用需要真实学生参与评估，但成本高且难以扩展。现有研究多使用简单提示方法模拟学生，但缺乏对模拟质量的系统评估，因此需要建立科学的评估体系。

Method: 1. 正式定义学生模拟任务；2. 提出涵盖语言、行为和认知维度的评估指标；3. 在真实数学辅导对话数据集上，对多种模拟方法（包括提示策略、监督微调和偏好优化）进行基准测试。

Result: 实验显示：1. 基于提示策略的模拟方法效果较差；2. 监督微调和偏好优化方法表现更好，但仍存在明显局限；3. 自动评估与人工评估结果一致。

Conclusion: 学生模拟是一项具有挑战性的任务，现有方法均未达到理想效果，需要未来研究开发更有效的模拟技术以支持教育AI应用的发展。
...
### [202] [When Helpers Become Hazards: A Benchmark for Analyzing Multimodal LLM-Powered Safety in Daily Life](https://arxiv.org/abs/2601.04043)
...
TL;DR: 提出SaLAD多模态安全基准，评估MLLMs在日常生活中的安全响应能力，发现当前模型在识别危险行为方面存在显著漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着MLLMs成为人类生活助手，其生成的不安全内容可能危害人类行为，需系统评估模型在日常场景中的安全影响。

Method: 构建包含2,013个真实图像-文本样本的多模态安全基准，涵盖10类常见场景；提出基于安全警告的评估框架，鼓励模型提供明确警告而非简单拒绝。

Result: 测试18个MLLMs发现，最优模型对不安全查询的安全响应率仅57.2%；现有安全对齐方法在日常生活危险行为识别场景中效果有限。

Conclusion: 当前MLLMs在识别日常危险行为方面存在明显脆弱性，需改进安全机制以应对真实世界的多模态安全风险。
...
### [203] [VotIE: Information Extraction from Meeting Minutes](https://arxiv.org/abs/2601.03997)
*José Pedro Evans,Luís Filipe Cunha,Purificação Silvano,Alípio Jorge,Nuno Guimarães,Sérgio Nunes,Ricardo Campos*

Main category: cs.CL

TL;DR: 本文提出了VotIE任务，用于从非结构化的市政会议纪要中提取结构化投票事件，并在葡萄牙市政纪要数据集上建立了首个基准。实验表明，微调的编码器在域内表现最佳，而少样本大语言模型在跨市政泛化方面更具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 市政会议纪要记录了地方民主进程的关键决策，但其投票结果以高度异构的自由叙述文本编码，且不同市政机构间差异巨大，这给自动化信息提取带来了显著挑战。

Method: 提出了VotIE（投票信息提取）任务，基于CitiLink语料库构建了葡萄牙市政会议纪要的基准数据集。实验比较了微调编码器（如XLM-R-CRF）与生成式大语言模型（LLMs）的性能，并评估了它们在域内和跨市政场景下的表现。

Result: 1. 在标准域内评估中，微调的XLM-R-CRF编码器取得了最佳性能（宏观F1达93.2%），优于生成式方法。2. 在跨市政评估中，微调编码器性能显著下降，而少样本LLMs表现出更强的鲁棒性，性能下降幅度更小。

Conclusion: 尽管生成式模型在泛化方面具有优势，但其高计算成本限制了实际应用。因此，轻量级微调编码器目前仍是大规模实际部署的更实用选择。作者公开了基准数据集、训练模型和评估框架以支持可复现研究。
...
### [204] [Modular Prompt Optimization: Optimizing Structured Prompts with Section-Local Textual Gradients](https://arxiv.org/abs/2601.04055)
*Prith Sharma,Austin Z. Henley*

Main category: cs.CL

TL;DR: 本文提出了模块化提示优化（MPO）框架，通过将提示分解为固定语义模块并独立优化各模块，提升小型开源语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法通常将提示视为整体文本块，难以定位错误、保留关键指令或控制提示长度增长，需要更结构化、可解释的优化方法。

Method: MPO将提示结构化分为系统角色、上下文、任务描述、约束和输出格式等语义模块，使用批评语言模型生成各模块的局部文本梯度进行独立优化，并通过去重整合更新以减少冗余。

Result: 在ARC-Challenge和MMLU推理基准测试中，使用LLaMA-3 8B-Instruct和Mistral-7B-Instruct模型，MPO均优于未优化结构化提示和TextGrad基线，显著提升准确率且不修改模型参数或提示结构。

Conclusion: 保持固定提示结构的同时进行局部模块化优化，是提升小型开源语言模型推理性能的有效实用方法。
...
### [205] [Analyzing and Improving Cross-lingual Knowledge Transfer for Machine Translation](https://arxiv.org/abs/2601.04036)
*David Stap*

Main category: cs.CL

TL;DR: 研究多语言机器翻译中的跨语言知识迁移，通过分析语言相似性、检索增强、辅助监督等方法提升低资源语言的翻译性能，并探讨训练策略对多语言学习的影响。


<details>
  <summary>Details</summary>
Motivation: 多语言机器翻译系统在低资源语言上表现不佳，主要受限于平行数据稀缺和跨语言表示学习困难，需要深入理解知识迁移机制以构建更具包容性和鲁棒性的系统。

Method: 以机器翻译为实验平台，分析语言相似性对迁移的影响，采用检索增强和辅助监督强化低资源翻译，研究微调策略的权衡效应，并通过增加语言多样性训练提升泛化能力。

Result: 语言相似性促进知识迁移，检索和辅助监督能有效提升低资源翻译性能，但微调可能引入意外权衡；增加训练语言覆盖可改善泛化并减少离靶翻译行为。

Conclusion: 建模选择和数据构成共同塑造多语言学习效果，通过优化训练策略和数据多样性可推动更具包容性和鲁棒性的多语言NLP系统发展。
...
### [206] [SpeakerSleuth: Evaluating Large Audio-Language Models as Judges for Multi-turn Speaker Consistency](https://arxiv.org/abs/2601.04029)
*Jonggeun Lee,Junseong Pyo,Gyuhyeon Seo,Yohan Jo*

Main category: cs.CL

TL;DR: 提出SpeakerSleuth基准测试，评估大型音频语言模型在多轮对话中判断说话者一致性的能力，发现模型存在文本优先于声学的显著偏差。


<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型作为评估语音生成质量的工具，其在多轮对话中判断说话者一致性的能力尚未被探索，需要系统评估其可靠性和局限性。

Method: 构建包含1,818个人工验证实例的SpeakerSleuth基准，涵盖四个多样化数据集（合成与真实语音），设计三个反映实际需求的任务，评估九个常用大型音频语言模型。

Result: 模型在检测声学不一致性方面表现不佳：部分模型过度预测不一致，部分过于宽松；难以准确定位问题轮次；当提供其他对话者轮次时，性能急剧下降（优先文本连贯性而忽略声学线索）；但在选择最匹配说话者的音频变体时表现较好。

Conclusion: 大型音频语言模型存在显著的模态不平衡偏差，过度依赖文本信息而忽视声学线索，需解决此根本问题才能构建可靠的音频语言评估工具。
...
### [207] [SearchAttack: Red-Teaming LLMs against Real-World Threats via Framing Unsafe Web Information-Seeking Tasks](https://arxiv.org/abs/2601.04093)
*Yu Yan,Sheng Sun,Mingfeng Li,Zheming Yang,Chiwei Zhu,Fei Ma,Benfeng Xu,Min Liu*

Main category: cs.CL

TL;DR: 提出SearchAttack攻击方法，通过将有害语义外包给网络搜索并引导LLM重构检索内容，有效攻击搜索增强型大语言模型的安全防护。


<details>
  <summary>Details</summary>
Motivation: 搜索增强型LLM在处理有害任务时，若搜索引擎返回可直接利用的有害内容，LLM的安全防护机制无法阻止这些内容暴露，存在安全漏洞。

Method: 将有害语义外包给网络搜索，仅保留查询框架和碎片化线索，通过结构化模板引导LLM重构检索内容以实现恶意目标。

Result: SearchAttack在攻击搜索增强型LLM系统方面表现出强有效性，实验验证了其攻击能力。

Conclusion: 网络搜索是LLM安全的关键攻击面，SearchAttack方法揭示了搜索增强型LLM在负责任漏洞评估中的脆弱性。
...
### [208] [Bridging the Discrete-Continuous Gap: Unified Multimodal Generation via Coupled Manifold Discrete Absorbing Diffusion](https://arxiv.org/abs/2601.04056)
*Yuanfeng Xu,Yuhao Chen,Liang Lin,Guangrun Wang*

Main category: cs.CL

TL;DR: 提出CoM-DAD框架，通过分层双过程统一文本和图像的生成，将语义规划与token合成解耦，解决了多模态生成中的对齐和稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型分为处理离散数据的自回归方法和处理连续数据的扩散方法，阻碍了真正统一的多模态系统发展；掩码语言模型虽高效但生成保真度和语义连续性不足，且多模态扩展存在对齐挑战和训练不稳定问题。

Method: CoM-DAD框架：1) 通过连续潜在扩散过程建模语义流形；2) 将token生成视为离散吸收扩散过程，由可变速率噪声调度控制，并以演化的语义先验为条件；3) 引入随机混合模态传输策略对齐不同模态，无需重型对比双编码器。

Result: 该方法在稳定性和生成质量上优于标准掩码建模，为可扩展的统一文本-图像生成建立了新范式。

Conclusion: CoM-DAD通过解耦语义规划和token合成，有效解决了多模态生成的对齐和稳定性问题，为统一生成模型提供了可行路径。
...
### [209] [KDCM: Reducing Hallucination in LLMs through Explicit Reasoning Structures](https://arxiv.org/abs/2601.04086)
*Jinbo Hao,Kai Yang,Qingzhen Su,Yifan Li,Chao Jiang*

Main category: cs.CL

TL;DR: 提出一种通过代码引导知识图谱探索的框架，减少大语言模型因提示引发的幻觉，显著提升推理准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理过程中容易因提示设计不当产生幻觉（错误信息），需要有效方法约束推理步骤并利用外部结构化知识。

Method: 扩展链式知识蒸馏方法，在推理提示中嵌入可编程模块（代码），引导模型探索知识图谱，并设计增强的蒸馏推理框架以显式调控中间推理步骤。

Result: 在GPT-4和LLaMA-3.3的多个公开基准测试中，代码引导推理显著提升上下文建模能力，减少幻觉。HIT@1、HIT@3、HIT@5分别提高15.64%、13.38%、13.28%，多项评估得分超过95%。

Conclusion: 该方法能有效约束错误推理，同时提高预测准确性和可解释性，为缓解大语言模型幻觉问题提供了可行方案。
...
### [210] [LLMberjack: Guided Trimming of Debate Trees for Multi-Party Conversation Creation](https://arxiv.org/abs/2601.04135)
*Leonardo Bottona,Nicolò Penzo,Bruno Lepri,Marco Guerini,Sara Tonelli*

Main category: cs.CL

TL;DR: 介绍了LLMberjack平台，该平台可将树状辩论结构转化为线性多轮对话，提供可视化界面和LLM辅助编辑功能，旨在创建透明可复现的多方对话资源。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏将树状辩论结构转化为连贯多方对话的工具资源，需要一种透明、可复现的工作流程来创建此类对话数据。

Method: 开发交互式平台，可视化讨论树结构，允许用户构建线性化对话序列，保留参与者身份和话语关系，并集成LLM辅助自动编辑消息和说话者描述。

Result: 平台通过树状可视化帮助创建连贯有意义的对话线程，LLM支持在减少人工努力的同时提升了输出质量。

Conclusion: LLMberjack作为开源工具，能够有效解决多方对话资源缺乏的问题，促进透明可复现的对话创建工作流程。
...
### [211] [FLEx: Language Modeling with Few-shot Language Explanations](https://arxiv.org/abs/2601.04157)
*Adar Avsian,Christopher Richardson,Anirudh Sundar,Larry Heck*

Main category: cs.CL

TL;DR: FLEx方法通过少量解释性示例改进语言模型行为，利用聚类选择代表性错误，验证解释有效性并汇总为提示前缀，在推理时引导模型避免类似错误。


<details>
  <summary>Details</summary>
Motivation: 语言模型虽在多任务中表现良好，但错误常在不同查询中重复出现。自然语言解释可帮助纠正错误，但大规模收集解释（尤其需专家标注的领域）成本高昂。

Method: FLEx方法：1）基于嵌入聚类选择代表性模型错误；2）验证相关解释能否纠正错误；3）将解释汇总为提示前缀，在推理时预置以引导模型，不修改模型权重。

Result: 在CounterBench、GSM8K和ReasonIF数据集上评估，FLEx持续优于思维链提示，最高减少CoT剩余错误的83%。

Conclusion: FLEx通过少量解释性示例有效改进模型行为，无需大规模标注或修改模型权重，为纠正模型重复错误提供了高效解决方案。
...
### [212] [All That Glisters Is Not Gold: A Benchmark for Reference-Free Counterfactual Financial Misinformation Detection](https://arxiv.org/abs/2601.04160)
...
TL;DR: RFC Bench是一个评估大语言模型在现实金融新闻中检测金融虚假信息的基准测试，包含无参考检测和基于对比的诊断两个任务，揭示当前模型在无参考设置下存在显著弱点。


<details>
  <summary>Details</summary>
Motivation: 金融新闻中的虚假信息检测具有挑战性，因为其含义往往来自分散的线索，需要捕捉上下文复杂性。现有评估缺乏针对金融领域且考虑现实新闻语境的基准。

Method: 构建段落级金融虚假信息基准RFC Bench，定义两个互补任务：1) 无参考虚假信息检测；2) 基于原始与扰动输入对比的诊断任务。通过实验比较模型在不同设置下的表现。

Result: 实验显示一致模式：当提供对比上下文时模型表现显著更强；而无参考设置暴露了严重弱点，包括预测不稳定和无效输出率升高，表明当前模型缺乏外部锚定时难以保持一致的信念状态。

Conclusion: RFC Bench通过揭示模型在无参考推理方面的差距，为研究无参考推理和推进更可靠的现实世界金融虚假信息检测提供了结构化测试平台。
...
### [213] [Revisiting Continuous-Time Trajectory Estimation via Gaussian Processes and the Magnus Expansion](https://arxiv.org/abs/2601.03360)
*Timothy Barfoot,Cedric Le Gentil,Sven Lilge*

Main category: cs.RO

TL;DR: 本文提出了一种基于Magnus展开的全局高斯过程先验方法，用于李群上的连续时间轨迹估计，相比现有的局部高斯过程方法更具理论优雅性和通用性。


<details>
  <summary>Details</summary>
Motivation: 现有连续时间轨迹估计方法在处理李群状态时，通常采用多个局部线性时不变SDE核的高斯过程拼接方式，虽然实用但缺乏理论优雅性。

Method: 采用线性时变随机微分方程驱动的全局高斯过程先验，通过Magnus展开在李群上构建连续时间轨迹估计框架。

Result: 推导出基于Magnus展开的全局高斯过程先验方法，并与现有局部方法进行了数值比较，讨论了各自的优缺点。

Conclusion: 基于Magnus展开的全局高斯过程方法为李群上的连续时间轨迹估计提供了更优雅和通用的解决方案，扩展了连续时间状态估计的应用范围。
...
### [214] [Lunar Rover Cargo Transport: Mission Concept and Field Test](https://arxiv.org/abs/2601.03371)
*Alexander Krawciw,Nicolas Olmedo,Faizan Rehmatullah,Maxime Desjardins-Goulet,Pascal Toupin,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: 在月球表面未来操作中，使用激光雷达教学与重复技术实现一吨级漫游车在恶劣环境下自主运输货物，成功演示了货物收集和交付。


<details>
  <summary>Details</summary>
Motivation: 月球表面自动化车辆需要精确导航以避开自然危险、人造基础设施和黑暗阴影，并需在微小容差内自主停放货物以实现成功取货和交付。

Method: 采用激光雷达教学与重复技术，先以半自主远程控制模式创建安全路径网络，然后漫游车立即自主重复整个路径网络并携带货物。

Result: 闭环性能足够精确，能够对齐车辆与货物并成功拾取，在加拿大航天局模拟地形进行了为期两周的部署，成功演示了货物收集和交付。

Conclusion: 激光雷达教学与重复技术为月球表面货物运输提供了理想的自主解决方案，能够在恶劣环境条件下实现成功的货物操作。
...
### [215] [Towards Zero-Knowledge Task Planning via a Language-based Approach](https://arxiv.org/abs/2601.03398)
*Liam Merz Hoffmeister,Brian Scassellati,Daniel Rakita*

Main category: cs.RO

TL;DR: 本文提出零知识任务规划（ZKTP）问题，利用大语言模型将自然语言指令分解为子任务并生成行为树，通过实时调整机制处理执行错误，在AI2-THOR仿真中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统任务规划方法依赖任务特定知识，限制了自动化系统的泛化能力。研究旨在探索无需先验知识的任务规划方法，实现更灵活的行为生成框架。

Method: 1. 形式化定义ZKTP问题；2. 使用LLM将自然语言指令分解为子任务；3. 生成可执行的行为树；4. 设计基于LLM的实时行为树调整循环以处理执行错误。

Result: 在AI2-THOR仿真环境中，该方法相比依赖任务特定知识的基线方法显著提升了任务完成性能，验证了LLM在零知识场景下的任务规划能力。

Conclusion: LLM能够有效解决ZKTP问题的多个关键环节，为无需任务特定设置的自动化行为生成提供了鲁棒框架，展示了零知识任务规划的可行性。
...
### [216] [Cost-Effective Radar Sensors for Field-Based Water Level Monitoring with Sub-Centimeter Accuracy](https://arxiv.org/abs/2601.03447)
*Anna Zavei-Boroda,J. Toby Minear,Kyle Harlow,Dusty Woods,Christoffer Heckman*

Main category: cs.RO

TL;DR: 本文评估了商用雷达传感器作为低成本水位监测替代方案的可行性，通过实地测试和统计滤波技术实现了厘米级精度，适用于无人机和机器人平台的自主监测。


<details>
  <summary>Details</summary>
Motivation: 传统水位监测方法成本高、覆盖范围有限，而雷达传感具有非接触、环境适应性强等优势，为低成本、广覆盖的水位监测提供了潜在解决方案。

Method: 采用商用雷达传感器进行实地测试，并应用统计滤波技术提升数据精度，通过最小化校准需求验证其实际部署可行性。

Result: 单个雷达传感器在实地测试中实现了厘米级精度的水位估计，证明了其在真实环境中的可靠性和实用性。

Conclusion: 雷达传感技术可作为低成本、高精度的自主水位监测方案，尤其适用于无人机和机器人平台，为水资源管理和洪涝防控提供了新工具。
...
### [217] [FIRE-VLM: A Vision-Language-Driven Reinforcement Learning Framework for UAV Wildfire Tracking in a Physics-Grounded Fire Digital Twin](https://arxiv.org/abs/2601.03449)
*Chris Webb,Mobin Habibpour,Mayamin Hamid Raha,Ali Reza Tavakkoli,Janice Coen,Fatemeh Afghah*

Main category: cs.RO

TL;DR: 本文提出FIRE-VLM框架，结合视觉语言模型与强化学习，在物理真实的野火数字孪生环境中训练无人机自主监测火情。


<details>
  <summary>Details</summary>
Motivation: 现有无人机导航方法依赖简化模拟器和监督感知流程，缺乏与物理真实火场交互的具身智能体，难以应对极端视觉退化、快速演变的火场动态和真实训练数据稀缺的挑战。

Method: 1) 基于USGS地形数据、LANDFIRE燃料库和半物理火势蔓延求解器构建高保真野火数字孪生；2) 采用PPO强化学习算法，结合双视角无人机传感；3) 引入CLIP风格视觉语言模型，通过野火语义对齐分数（基于火与烟雾提示）作为基于势能的奖励塑形信号。

Result: 在五个数字孪生评估任务中，VLM引导的策略将火情检测时间缩短至1/6，增加视野内停留时间，成为首个在公里级物理真实数字孪生火场中验证的RL无人机野火监测系统。

Conclusion: FIRE-VLM框架通过GIS-模拟流水线、VLM引导的RL智能体和野火感知奖励设计，实现了在复杂物理真实火场环境中的高效自主监测，为极端环境下的具身智能应用提供了新范式。
...
### [218] [A Vision-Language-Action Model with Visual Prompt for OFF-Road Autonomous Driving](https://arxiv.org/abs/2601.03519)
*Liangdong Zhang,Yiming Nie,Haoyang Li,Fanjie Kong,Baobao Zhang,Shunxin Huang,Kai Fu,Chen Min,Liang Xiao*

Main category: cs.RO

TL;DR: 提出OFF-EMMA框架，通过视觉提示块和链式思维自洽推理策略，提升越野场景下视觉-语言-动作模型的轨迹规划性能。


<details>
  <summary>Details</summary>
Motivation: 传统越野轨迹规划方法在动态环境中适应性有限，且现有视觉-语言-动作模型存在空间感知不足和推理不稳定的问题。

Method: 设计视觉提示块（利用语义分割掩码增强空间理解），结合链式思维自洽推理策略（多路径推理减少异常值影响），构建端到端多模态框架。

Result: 在RELLIS-3D数据集上，OFF-EMMA显著优于现有方法：将Qwen骨干模型的平均L2误差降低13.3%，失败率从16.52%降至6.56%。

Conclusion: OFF-EMMA通过增强空间感知和稳定推理，有效提升了越野自动驾驶轨迹规划的准确性和鲁棒性。
...
### [219] [From Score to Sound: An End-to-End MIDI-to-Motion Pipeline for Robotic Cello Performance](https://arxiv.org/abs/2601.03562)
*Samantha Sudhoff,Pranesh Velmurugan,Jiashu Liu,Vincent Zhao,Yung-Hsiang Lu,Kristen Yeon-Ji Yun*

Main category: cs.RO

TL;DR: 提出了一种端到端的MIDI乐谱到机器人动作的管道，使UR5e机器人能够演奏大提琴，无需动作捕捉，并引入了音乐图灵测试作为评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有机器人演奏弦乐器（如大提琴）时依赖昂贵的动作捕捉技术，且无法像人类一样视奏乐谱，需要更自然、低成本的控制方法。

Method: 使用Universal Robot的Freedrive功能实现免动作捕捉的演奏，通过RTDE记录关节数据，并设计端到端管道将MIDI乐谱直接转换为碰撞感知的弓法动作。

Result: 机器人能够演奏五首标准曲目，并通过音乐图灵测试（132名参与者评估）验证其演奏效果接近人类水平，同时公开了人类参考录音和机器人演奏数据。

Conclusion: 该方法为机器人演奏大提琴提供了新基准，未来可通过残差强化学习进一步优化弓弦切换效率和音质。
...
### [220] [Locomotion Beyond Feet](https://arxiv.org/abs/2601.03607)
...
TL;DR: 提出了一种名为'Locomotion Beyond Feet'的全身人形机器人运动系统，结合关键帧动画与强化学习，使机器人能在极富挑战性的地形（如低矮空间、高台、陡峭楼梯）中实现稳健的全身运动。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人运动方法主要关注基于腿部的步态，而自然双足生物在复杂环境中常依赖手、膝、肘等部位建立额外接触点以增强稳定性与支撑。

Method: 结合基于物理的关键帧动画（编码人类运动技能知识）与强化学习（将关键帧转化为稳健的物理运动），并采用分层框架，包括地形特异性运动跟踪策略、故障恢复机制和基于视觉的技能规划器。

Result: 真实世界实验表明，该系统能实现稳健的全身运动，并能泛化到不同尺寸的障碍物、不同实例的地形以及连续地形序列。

Conclusion: 所提出的系统成功解决了接触丰富的运动规划和跨地形泛化两大挑战，为人形机器人在极端复杂环境中的全身运动提供了有效解决方案。
...
### [221] [Dual-Attention Heterogeneous GNN for Multi-robot Collaborative Area Search via Deep Reinforcement Learning](https://arxiv.org/abs/2601.03686)
*Lina Zhu,Jiyu Cheng,Yuehu Liu,Wei Zhang*

Main category: cs.RO

TL;DR: 提出一种基于双注意力异构图神经网络的强化学习方法，用于多机器人协作区域搜索，有效平衡探索未知区域和覆盖特定救援目标两个目标。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于同构图表示，难以建模和平衡探索未知区域与覆盖救援目标这两个不同任务之间的动态关系。

Method: 构建包含机器人节点、前沿节点和兴趣点节点的异构图，设计关系感知注意力和类型感知注意力的双注意力机制，通过深度强化学习训练网络。

Result: 在iGibson模拟器的交互式3D场景中使用Gibson和MatterPort3D数据集进行实验，验证了该方法具有优异的可扩展性和泛化能力。

Conclusion: DA-HGNN方法通过异构图表示和双注意力机制，成功解耦了探索与覆盖任务，在多机器人协作搜索中实现了更好的性能平衡。
...
### [222] [PointWorld: Scaling 3D World Models for In-The-Wild Robotic Manipulation](https://arxiv.org/abs/2601.03782)
*Wenlong Huang,Yu-Wei Chao,Arsalan Mousavian,Ming-Yu Liu,Dieter Fox,Kaichun Mo,Li Fei-Fei*

Main category: cs.RO

TL;DR: 提出PointWorld，一个大型预训练的3D世界模型，通过将状态和动作统一表示为3D点流，实现从单张或多张RGB-D图像及机器人动作序列预测3D像素位移，支持跨机器人形态的泛化学习。


<details>
  <summary>Details</summary>
Motivation: 人类能够通过视觉和身体动作预判3D世界的反应，这对机器人操作至关重要。现有方法通常依赖特定机器人形态的动作空间，限制了跨平台泛化能力。

Method: 1. 将状态（RGB-D图像）和动作（低级指令）统一表示为3D点流；2. 构建大规模数据集（约200万轨迹，500小时），涵盖真实与仿真环境中的单臂Franka和双手机器人操作；3. 通过系统实验分析骨干网络、动作表示、学习目标等设计要素；4. 集成模型预测控制框架，实现实时推理（0.1秒）。

Result: 1. 单一预训练模型使真实Franka机器人能够完成刚体推动、可变形/关节物体操作及工具使用；2. 无需演示数据或后训练，仅需单张野外捕获图像；3. 验证了模型在部分可观测性、数据混合、领域迁移和规模扩展中的有效性。

Conclusion: PointWorld通过几何统一的3D点流表示，实现了跨机器人形态的通用3D世界建模，为开放环境中的机器人操作提供了可扩展且高效的解决方案。
...
### [223] [Generational Replacement and Learning for High-Performing and Diverse Populations in Evolvable Robots](https://arxiv.org/abs/2601.03807)
*K. Ege de Bruin,Kyrre Glette,Kai Olav Ellefsen*

Main category: cs.RO

TL;DR: 该研究探讨了在进化机器人学中，通过结合代际完全替换和生命周期内学习，可以在保持性能的同时增加种群多样性，并强调了性能评估指标的重要性。


<details>
  <summary>Details</summary>
Motivation: 进化机器人学在同时优化机器人形态和控制时面临挑战：控制器需要时间适应不断演化的形态，可能导致有潜力的新设计难以进入种群；同时，进化过程往往过早收敛，导致种群多样性不足。

Method: 提出将代际完全替换（每代完全替换种群）与生命周期内学习（个体在生命周期内进行额外的控制器优化）相结合的方法，并比较了按函数评估次数和按进化代数两种性能评估指标。

Result: 结合代际完全替换和生命周期内学习能够有效增加种群多样性，同时保持性能不下降；研究还发现，使用不同的性能评估指标（函数评估次数 vs. 进化代数）可能导致不同的结论。

Conclusion: 代际完全替换与生命周期内学习的结合是解决进化机器人学中多样性-性能权衡的有效策略，且选择合适的性能评估指标对于准确理解学习在形态演化机器人中的作用至关重要。
...
### [224] [Integrating Sample Inheritance into Bayesian Optimization for Evolutionary Robotics](https://arxiv.org/abs/2601.03813)
*K. Ege de Bruin,Kyrre Glette,Kai Olav Ellefsen*

Main category: cs.RO

TL;DR: 本研究在进化机器人学中，通过贝叶斯优化和样本继承机制，解决了形态-控制器协同优化时控制器学习预算不足的问题，提高了进化效率。


<details>
  <summary>Details</summary>
Motivation: 进化机器人学中，形态与控制器需协同优化，但为每个新形态从头优化控制器需要高昂学习成本。本研究旨在通过高效的继承机制，在有限学习预算下提升性能。

Method: 使用贝叶斯优化进行控制器优化，引入两种样本继承方式：1) 将父代样本作为先验知识直接传递给子代；2) 在子代上重新评估父代最优样本。并与无继承基线对比。

Result: 重新评估继承表现最佳，先验继承也优于无继承。继承机制通过跨代累积学习适应弥补单代学习预算不足，且对形态相似的子代帮助更大。更具挑战性的环境能促进更稳定的步态生成。

Conclusion: 继承机制可在不增加学习预算的情况下提升进化机器人学性能，为高效机器人设计提供了可行路径。
...
### [225] [Towards Safe Autonomous Driving: A Real-Time Motion Planning Algorithm on Embedded Hardware](https://arxiv.org/abs/2601.03904)
*Korbinian Moller,Glenn Johannes Tungka,Lucas Jürgens,Johannes Betz*

Main category: cs.RO

TL;DR: 本文提出了一种用于自动驾驶的主动安全扩展方法，通过在嵌入式实时操作系统上部署轻量级采样轨迹规划器，确保在主要规划器故障时仍能安全运行。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶安全机制（如在线验证）缺乏主动机制来应对主要规划器故障，无法保证故障情况下的安全操作，因此需要开发主动安全扩展以实现故障可操作自动驾驶。

Method: 在汽车级嵌入式平台上使用实时操作系统部署轻量级采样轨迹规划器，在有限计算资源下持续计算轨迹，为紧急规划架构奠定基础。

Result: 实验表明该方法具有确定性时序行为、有限延迟和最小抖动，验证了在安全可认证硬件上进行轨迹规划的可行性。

Conclusion: 研究展示了将主动后备机制集成到下一代安全框架中的潜力，同时指出了剩余挑战，为故障可操作自动驾驶提供了初步解决方案。
...
### [226] [An Event-Based Opto-Tactile Skin](https://arxiv.org/abs/2601.03907)
...
TL;DR: 提出了一种基于动态视觉传感器和柔性硅胶光波导的神经形态事件驱动触觉传感系统，用于大面积软体皮肤，通过双目视觉和DBSCAN聚类实现按压定位，在极端数据稀疏条件下仍保持功能。


<details>
  <summary>Details</summary>
Motivation: 为软体机器人和交互环境开发大面积、灵活且响应灵敏的触觉传感器，需要克服传统嵌入式光接收器重复扫描的局限，并探索在低功耗、低计算负载下的可行方案。

Method: 采用柔性硅胶光波导皮肤与两个DVS摄像头组成的双目视觉系统，通过检测亮度变化产生事件流，利用DBSCAN聚类分析接触事件质心，并通过三角测量估计二维皮肤表面的按压位置；使用随机下采样评估系统在极端数据稀疏条件下的性能。

Result: 在4620 mm²探测区域内，95%的双目可见按压定位均方根误差为4.66 mm；事件流减少至原始数据的1/1024时，平均定位误差仅增至9.33 mm，85%的试验仍能产生有效定位；系统检测延迟分布特征宽度为31 ms。

Conclusion: 该系统展示了事件驱动触觉传感在大面积软体皮肤中的可行性，即使在极端数据稀疏条件下仍能保持功能，为降低未来实现的功耗和计算负载提供了有前景的途径。
...
### [227] [CoINS: Counterfactual Interactive Navigation via Skill-Aware VLM](https://arxiv.org/abs/2601.03956)
...
TL;DR: 提出CoINS框架，通过技能感知的视觉语言模型和强化学习技能库，实现机器人主动交互式导航，在复杂环境中通过移动物体清理路径。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在机器人规划中缺乏对机器人物理能力的理解，尤其在交互式导航中无法主动推理何时及如何与环境物体交互以清理阻塞路径。

Method: 提出分层框架CoINS：1) 微调InterNav-VLM模型，融入技能可用性和约束参数，通过反事实推理学习物体移除对导航连通性的因果效应；2) 通过强化学习构建全面的技能库，实现多样物体的可通行性操作。

Result: 在Isaac Sim仿真和真实世界实验中，CoINS显著优于基线方法，整体成功率提高17%，在复杂长视野场景中提升超过80%。

Conclusion: CoINS框架通过结合技能感知推理和鲁棒低级执行，有效解决了交互式导航中的主动环境修改问题，为机器人复杂环境导航提供了新方案。
...
### [228] [Stable Language Guidance for Vision-Language-Action Models](https://arxiv.org/abs/2601.04052)
*Zhihao Zhan,Yuhao Chen,Jiaying Zhou,Qinhan Lv,Hao Liu,Keze Wang,Liang Lin,Guangrun Wang*

Main category: cs.RO

TL;DR: 提出Residual Semantic Steering (RSS)框架，通过解耦物理可操作性与语义执行，解决VLA模型对语言扰动敏感的问题，提升机器人控制的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action (VLA) 模型在广义机器人控制中表现出色，但对语言扰动非常脆弱，存在'模态崩溃'现象——强视觉先验淹没稀疏语言信号，导致智能体过度拟合特定指令措辞而忽略底层语义意图。
...
Result: 理论分析表明RSS能有效最大化动作与意图间的互信息并抑制视觉干扰。在多样化操作基准测试中，RSS实现了最先进的鲁棒性，即使在对抗性语言扰动下仍能保持性能。

Conclusion: RSS框架通过解耦语义与视觉可操作性，显著提升了VLA模型对语言变化的鲁棒性，为解决模态崩溃问题提供了有效方案。
...
### [229] [CLAP: Contrastive Latent Action Pretraining for Learning Vision-Language-Action Models from Human Videos](https://arxiv.org/abs/2601.04061)
*Chubin Zhang,Jianan Wang,Zifeng Gao,Yue Su,Tianru Dai,Cai Zhou,Jiwen Lu,Yansong Tang*

Main category: cs.RO

TL;DR: 提出CLAP框架，通过对比学习对齐视频与机器人轨迹的潜在空间，实现人类视频技能向机器人执行的迁移，并引入两种VLA模型和防遗忘正则化策略。


<details>
  <summary>Details</summary>
Motivation: 现有通用视觉-语言-动作模型受限于机器人数据稀缺，而潜在动作模型利用视频数据时易产生视觉纠缠，难以提取有效操作技能。

Method: 1. 提出对比潜在动作预训练（CLAP），对齐视频视觉潜在空间与机器人本体感知潜在空间；2. 构建量化可执行代码本；3. 开发双形式VLA框架（自回归模型CLAP-NTP与整流流策略CLAP-RF）；4. 设计知识匹配正则化策略防止微调时灾难性遗忘。

Result: 大量实验表明CLAP显著优于基线方法，能有效将人类视频技能迁移至机器人执行，实现指令跟随、物体泛化及高精度操作。

Conclusion: CLAP通过跨模态对齐解决了视觉纠缠问题，为利用丰富人类视频数据训练机器人策略提供了有效框架，推动了通用VLA模型的发展。
...
### [230] [Wow, wo, val! A Comprehensive Embodied World Model Evaluation Turing Test](https://arxiv.org/abs/2601.04137)
...
TL;DR: 该论文提出了Embodied Turing Test基准测试Wow-wo-val，用于评估视频基础模型作为具身AI世界模型的生成泛化能力和鲁棒性，发现现有模型在时空一致性和物理推理方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频基础模型在具身AI中被广泛探索作为预测性世界模型，但其生成泛化能力是否足以保持人类感知保真度，以及是否足够鲁棒以作为真实世界具身智能体的通用先验，这两个关键问题尚未得到解答。

Method: 基于609个机器人操作数据构建Wow-wo-val基准测试，涵盖感知、规划、预测、泛化和执行五大核心能力，提出包含22个指标的综合评估协议，并引入逆动态模型进行真实世界执行准确性评估。

Result: 现有模型在长时程规划任务上仅得17.27分，物理一致性最高仅68.02分，表明时空一致性和物理推理能力有限。在逆动态模型测试中，多数模型成功率接近0%，而WoW模型保持40.74%的成功率。

Conclusion: 生成视频与真实世界之间存在显著差距，凸显了在具身AI中对世界模型进行基准测试的紧迫性和必要性。
...
### [231] [Hierarchical GNN-Based Multi-Agent Learning for Dynamic Queue-Jump Lane and Emergency Vehicle Corridor Formation](https://arxiv.org/abs/2601.04177)
*Haoran Su*

Main category: cs.RO

TL;DR: 提出基于分层图神经网络的多智能体强化学习框架，用于协调联网车辆形成应急通道，显著缩短应急车辆通行时间并保持交通效率。


<details>
  <summary>Details</summary>
Motivation: 现有应急车辆通行策略无法适应动态交通条件，需要一种能协调联网车辆、快速形成应急通道的智能方法。

Method: 采用分层框架：高层规划器制定全局策略，低层控制器执行轨迹；使用图注意力网络适应可变智能体数量；通过多智能体近端策略优化（MAPPO）进行训练。

Result: 仿真显示：应急车辆通行时间比基线减少28.3%，比无协调交通减少44.6%；碰撞率接近零（0.3%），同时保持81%的背景交通效率；消融和泛化研究证实了框架的鲁棒性。

Conclusion: 结合图神经网络与分层学习的方法在智能交通系统中具有高效性，能实现应急车辆快速通行与整体交通效率的平衡。
...
### [232] [Embedding Autonomous Agents in Resource-Constrained Robotic Platforms](https://arxiv.org/abs/2601.04191)
*Negar Halakou,Juan F. Gutierrez,Ye Sun,Han Jiang,Xueming Wu,Yilun Song,Andres Gomez*

Main category: cs.RO

TL;DR: 将AgentSpeak编程的自主智能体集成到双轮机器人中，使其能够在迷宫中基于传感器数据自主决策导航，实验表明推理过程高效且适合资源受限的嵌入式系统实时运行。


<details>
  <summary>Details</summary>
Motivation: 嵌入式设备常在资源受限和动态环境中运行，需要本地决策能力以提高系统响应速度并减少对外部控制的依赖。

Method: 使用AgentSpeak编程语言实现自主智能体，将其集成到双轮机器人上，结合传感器数据在迷宫环境中进行自主探索和决策。

Result: 智能体在59秒内成功解决迷宫问题，使用了287个推理周期，决策阶段耗时少于1毫秒，表明推理过程高效。

Conclusion: 该集成展示了高层基于智能体的控制方法可应用于资源受限的嵌入式系统，实现自主操作，推理效率足以支持实时执行。
...
### [233] [$α^3$-Bench: A Unified Benchmark of Safety, Robustness, and Efficiency for LLM-Based UAV Agents over 6G Networks](https://arxiv.org/abs/2601.03281)
*Mohamed Amine Ferrag,Abderrahmane Lakas,Merouane Debbah*

Main category: eess.SY

TL;DR: 本文提出了α³-Bench基准，用于评估在动态6G网络条件下，基于大语言模型的无人机自主控制系统的安全性和有效性。


<details>
  <summary>Details</summary>
Motivation: 当前对基于大语言模型的无人机自主控制系统的评估很少考虑其在真实下一代网络约束下的安全性、协议合规性和有效性，因此需要一个新的评估基准。

Method: 构建了包含11.3万次对话的无人机任务数据集，将任务建模为语言介导的控制循环，支持工具调用和多智能体协调，并提出了综合六个维度的α³评估指标。

Result: 评估了17个先进大语言模型，结果显示虽然部分模型在任务成功率和安全合规性上表现良好，但在降级的6G网络条件下，鲁棒性和效率存在显著差异。

Conclusion: 基于大语言模型的无人机智能体需要具备网络感知和资源高效的特点，α³-Bench为评估这类系统提供了全面基准，数据集已开源。
...
### [234] [Battery-time-space fragment-based formulation for the Electric Autonomous Dial-a-Ride Problem](https://arxiv.org/abs/2601.03282)
*Boshuai Zhao,Adam Abdin,Jakob Puchinger*

Main category: eess.SY

TL;DR: 本文提出了一种离散化的电动自动驾驶拼车问题（D-E-ADARP）及其基于电池-时间-空间片段（BTSFF）的高效求解方法，显著提升了计算效率，并为原问题提供了高质量下界。


<details>
  <summary>Details</summary>
Motivation: 实际场景中时间和电池数据精度有限，现有连续模型求解效率低，需要一种能处理离散化参数且高效的方法。

Method: 将时间和电池参数离散化，构建BTSFF网络模型，将路径分解为带成本的片段，在离散化的时空电池维度中优化。

Result: BTSFF求解D-E-ADARP比现有方法快得多，能为原E-ADARP提供高质量下界，并加速电池更换变体问题的求解。

Conclusion: 离散化方法结合BTSFF框架能高效求解电动自动驾驶拼车问题，在保证解质量的同时大幅提升计算性能。
...
### [235] [Modeling and Control for UAV with Off-center Slung Load](https://arxiv.org/abs/2601.03386)
*Zongyang Lv,Yanmei Jia,Yongqing Liu,Alan F. Lynch,Qing Zhao,Yuhu Wu*

Main category: eess.SY

TL;DR: 提出了一种基于悬吊点建模的无人机吊挂载荷系统控制策略，通过级联控制器实现载荷摆动抑制和姿态跟踪，并证明了闭环系统的局部指数稳定性。


<details>
  <summary>Details</summary>
Motivation: 实际应用中，无人机吊挂载荷的悬吊点常偏离无人机质心，导致系统非线性动力学耦合，传统基于质心的建模方法难以处理这种复杂控制问题。

Method: 1. 建立以悬吊点为中心的系统动力学模型；2. 设计级联控制器：中层控制器利用悬吊点加速度调节载荷摆角（无需考虑耦合），内层控制器基于偏心参考系跟踪无人机姿态（无需简化耦合效应）；3. 使用李雅普诺夫方法证明闭环系统稳定性。

Result: 仿真和实验验证表明，所提控制系统能有效处理悬吊点与质心偏移带来的耦合问题，实现稳定的载荷运输控制。

Conclusion: 基于悬吊点建模的级联控制策略能有效解耦无人机与吊挂载荷的动力学交互，为偏心吊挂系统提供了更实用的控制解决方案。
...
### [236] [Provable Acceleration of Distributed Optimization with Local Updates](https://arxiv.org/abs/2601.03442)
*Zuang Wang,Yongqiang Wang*

Main category: eess.SY

TL;DR: 本文证明在分布式优化算法DIGing中引入局部更新可以加速收敛，且仅需两次局部更新即可达到最大改进效果，额外更新无益。


<details>
  <summary>Details</summary>
Motivation: 受联邦学习中多局部更新成功的启发，研究者希望探索在梯度精确的分布式优化中引入局部更新是否也能带来加速效果，并澄清现有理论中因需减小步长而可能掩盖真实收益的问题。

Method: 基于经典DIGing算法，利用性能估计问题（PEP）提供的紧性能界进行分析，并在合成与真实数据集上进行大量实验验证。

Result: 理论分析表明，在适当步长下，仅执行两次局部更新即可实现最大可能的收敛加速，更多局部更新不会带来额外收益；实验数据支持了这一结论。

Conclusion: 局部更新可加速分布式优化，但实践中仅需两次更新即可最大化效率，这为高效实现提供了实用指导。
...
### [237] [Policy Synthesis for Interval MDPs via Polyhedral Lyapunov Functions](https://arxiv.org/abs/2601.03445)
*Negar Monir,Sadegh Soudjani*

Main category: eess.SY

TL;DR: 提出一种基于多面体Lyapunov函数的多目标区间马尔可夫决策过程策略综合方法，通过重构值迭代算法为切换仿射系统，在保证收敛性的同时避免计算密集型帕累托曲线分析。


<details>
  <summary>Details</summary>
Motivation: 现有Lyapunov方法主要依赖二次函数，在处理动态规划值迭代中的不确定性时精度有限，且多目标优化常需计算帕累托曲线，计算成本高。

Method: 1. 将值迭代算法重构为含区间不确定性的切换仿射系统；2. 采用多面体Lyapunov函数提升不确定性管理精度；3. 基于控制理论稳定性原理构建吸引不变集；4. 直接合成使目标值进入指定范围的策略。

Result: 1. 方法无需计算帕累托曲线即可确定满足目标值范围的策略；2. 通过吸引不变集保证策略收敛性并最小化转移不确定性影响；3. 在回收机器人和电动汽车电池等案例中验证了有效性。

Conclusion: 所提多面体Lyapunov函数方法能有效处理多目标区间MDP的策略综合问题，在保证收敛性的同时显著降低计算复杂度，适用于安全关键场景下的不确定性决策。
...
### [238] [Developing a Quantitative Resiliency Approach](https://arxiv.org/abs/2601.03452)
*Vincent P. Paglioni,Graeme Troxell,Aaron Brown,Steve Conrad,Mazdak Arabi*

Main category: eess.SY

TL;DR: 本文探讨了关键基础设施管理中韧性的重要性，提出了一种基于量化理解的韧性决策框架，以弥补传统可靠性方法的不足。


<details>
  <summary>Details</summary>
Motivation: 在高度复杂和相互依赖的关键系统中，传统的可靠性（作为无故障概率）可能不足以衡量系统健康，因此需要转向更重视故障响应能力的韧性中心方法。

Method: 通过分析可靠性与韧性工程的基础，提出了一种基于量化韧性理解的韧性决策方法。

Result: 提出了一种支持韧性决策的量化框架，使韧性能够像风险和可靠性一样被纳入实际决策过程。

Conclusion: 韧性需要被明确定义和量化，以支持关键基础设施的决策制定，从而更好地应对复杂系统中的故障和恢复过程。
...
### [239] [Online Decision-Making Under Uncertainty for Vehicle-to-Building Systems](https://arxiv.org/abs/2601.03476)
*Rishav Sen,Yunuo Zhang,Fangqi Liu,Jose Paolo Talusan,Ava Pettet,Yoshinori Suzue,Ayan Mukhopadhyay,Abhishek Dubey*

Main category: eess.SY

TL;DR: 本文提出了一种基于马尔可夫决策过程的车辆到建筑（V2B）优化框架，通过在线搜索和领域启发式方法解决大规模状态和动作空间问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: V2B系统通过电动汽车作为灵活储能单元优化建筑能耗，但现有方法常将其建模为单次组合优化问题，难以处理电价波动、长时域规划、异构充电桩和用户需求等多重挑战。

Method: 将V2B优化问题建模为马尔可夫决策过程，采用在线搜索处理大规模状态空间，并利用领域特定启发式方法剪枝无效动作以压缩动作空间。

Result: 与日产先进技术中心合作，基于真实电动汽车测试数据验证，所提框架在降低总用电成本方面显著优于现有最优方法。

Conclusion: 基于随机控制过程的MDP建模能更有效处理V2B优化中的动态不确定性，结合在线搜索和启发式剪枝的方法具有实际应用潜力。
...
### [240] [Adaptive Model-Based Reinforcement Learning for Orbit Feedback Control in NSLS-II Storage Ring](https://arxiv.org/abs/2601.03486)
*Zeyu Dong,Yuke Tian,Yu Sun*

Main category: eess.SY

TL;DR: 提出基于模型强化学习的自适应训练框架，用于NSLS-II同步辐射光源的电子束稳定，通过在线训练跟踪机器状态漂移，将束流位置对齐误差降至约1微米。


<details>
  <summary>Details</summary>
Motivation: 传统SVD算法和监督学习方法在应对机器状态漂移、环境噪声和非线性加速器动力学等问题时存在局限，需要更鲁棒的自适应解决方案。

Method: 采用基于模型的强化学习框架，包含两种优化：轨迹优化（在可微分环境中最小化期望总奖励）和在线模型优化（通过智能体-环境交互学习非线性机器动力学）。

Result: 在NSLS-II上的仿真和实际实验表明，该方法能稳定束流位置，将调整后束位与参考位置的RMS对齐误差降低至约1微米。

Conclusion: 所提出的自适应强化学习框架能有效解决加速器束流稳定中的动态和非线性问题，显著提升束流位置控制精度。
...
### [241] [Cyberattack Detection in Virtualized Microgrids Using LightGBM and Knowledge-Distilled Classifiers](https://arxiv.org/abs/2601.03495)
*Osasumwen Cedric Ogiesoba-Eguakun,Suman Rath*

Main category: eess.SY

TL;DR: 本文设计了一个完整的虚拟微电网，开发了结构化网络攻击框架，并利用LightGBM模型实现了高效的网络入侵检测，验证了轻量级机器学习方法在微电网网络安全中的实用性。


<details>
  <summary>Details</summary>
Motivation: 现代微电网依赖分布式传感和通信接口，使其日益容易受到网络物理干扰的威胁，这些干扰会威胁运行连续性和设备安全。

Method: 在MATLAB/Simulink中设计和实现了一个完整的虚拟微电网，集成了异构可再生能源和二次控制器层；使用MGLib开发了结构化网络攻击框架，直接向二次控制路径注入对抗信号；模拟了多种攻击类型；使用生成的数据集训练LightGBM模型进行入侵检测和攻击类型分类；通过知识蒸馏步骤减小模型尺寸。

Result: 多分类模型达到了99.72%的准确率和99.62%的F1分数，二分类模型达到了94.8%的准确率和94.3%的F1分数；知识蒸馏后的模型性能略有下降但预测速度更快；实时测试显示每1000个样本的处理延迟约为54至67毫秒。

Conclusion: 轻量级机器学习入侵检测方法能够提供快速、准确和高效的网络攻击检测，无需依赖复杂的深度学习模型，适合在微电网控制器中基于CPU的边缘部署。
...
### [242] [Spider web-inspired sensing and computation with fiber network physical reservoirs](https://arxiv.org/abs/2601.03536)
*Apoorva Khairnar,Yogesh Phalak,Jun Wang,Ziyang Zhou,Benjamin Jantzen,Suyi Li,Noel Naughton*

Main category: eess.SY

TL;DR: 研究受蜘蛛网启发的柔性纤维网络作为物理储层计算的机械系统，探索其设计参数和操作条件对计算能力的影响，并通过仿真和实验验证其非线性计算和记忆能力。


<details>
  <summary>Details</summary>
Motivation: 利用机械系统的固有动力学进行物理储层计算，探索柔性纤维网络（特别是受蜘蛛网启发）作为计算介质的潜力，以理解其机械设计和操作条件如何影响计算性能。

Method: 采用基于Cosserat杆的仿真方法，分析网络拓扑、几何形状、驱动方式和轴向张力对计算能力的影响；评估多种读出简化策略；并通过物理原型进行实验验证。

Result: 研究发现网络拓扑、几何、驱动和张力显著影响非线性计算和记忆容量；读出策略的选择（测量输出的数量和位置）也影响计算性能；实验验证了仿真结果的可靠性。

Conclusion: 柔性纤维网络可作为物理储层，具备非线性变换和输入历史记忆能力；研究为设计、驱动和传感选择提供了指导，以支持纤维网络在机械智能计算中的应用。
...
### [243] [DSP-Based Sub-Switching-Period Current-Limiting Control for Grid-Tied Inverter under Grid Faults](https://arxiv.org/abs/2601.03638)
...
<details>
  <summary>Details</summary>
Motivation: 电网电压突然扰动（如电压暂降或相位跳变）会在开关周期内产生大瞬态电流，特别是在低开关频率下，可能损坏逆变器或导致保护跳闸。

Method: 在检测到扰动后，立即修改脉宽调制载波，使逆变器输出电流在远小于开关周期的时间内持续调节，无需中断电流流动。该方法可在常用数字信号处理器上实现，无需专用电路或高速计算设备。

Result: 在3.6 kHz开关频率的2电平三相逆变器上进行的实验验证了该方法在对称和不对称电压暂降及相位跳变下的有效性。

Conclusion: 所提出的控制方法能够快速抑制瞬态过电流，实现无缝故障穿越，且易于在现有硬件平台上实施。
...
### [244] [Accounting for Optimal Control in the Sizing of Isolated Hybrid Renewable Energy Systems Using Imitation Learning](https://arxiv.org/abs/2601.03679)
*Simon Halvdansson,Lucas Ferreira Bernardino,Brage Rugstad Knudsen*

Main category: eess.SY

TL;DR: 提出了一种考虑可再生能源不确定性和最优反馈控制的孤立能源系统储能与可再生能源容量配置框架，采用模仿学习与随机神经模型预测控制方法，通过海上能源系统案例验证了投资成本与减排效果之间的非线性关系。


<details>
  <summary>Details</summary>
Motivation: 孤立能源系统通过增加间歇性可再生能源（如太阳能、风能）实现脱碳时，需要配置储能或保留化石燃料调峰电源以平衡供需。现有系统容量规划方法难以有效纳入有限时域最优控制的影响，导致减排效果与成本评估不准确。

Method: 提出灵活的容量配置框架，结合模仿学习与随机神经模型预测控制（MPC），在考虑可再生能源不确定性和有限预测视野的条件下，建立储能容量、风电峰值容量与减排效果、投资成本的关联模型。

Result: 以海上能源系统（燃气轮机、风电场、电池储能系统）为案例验证，发现投资成本与燃气减排效果相对于风电和储能容量呈非线性、非平凡关系，表明最优控制在系统设计中具有重要影响。

Conclusion: 所提框架能够帮助决策者在考虑可再生能源不确定性和有限预测视野的条件下，评估不同储能与风电容量配置的减排效果与成本，凸显了在孤立能源系统设计中纳入最优控制的重要性。
...
### [245] [Derivation of the Thermal Conductivity in a Latent Thermal Energy Storage Unit for Use in Simplified System Models](https://arxiv.org/abs/2601.03716)
*Lauritz Zendel,Chiara Springer,Frank Dammel,Peter Stephan*

Main category: eess.SY

TL;DR: 本研究开发了用于卡诺电池系统模型的简化潜热储能单元模型，通过确定单根翅片管的等效热导率来近似整个LTES单元的性能。


<details>
  <summary>Details</summary>
Motivation: 潜热储能单元在窄温度范围内存储热能，适合集成到基于朗肯循环的卡诺电池中。但由于自然对流等物理现象无法在瞬态系统模型中直接建模，需要开发简化模型来准确模拟其瞬态行为。

Method: 采用COMSOL软件建立单根翅片管截面的数值模型，通过变化简化模型中的等效热导率，并与基于复杂建模方法的参考案例进行对比，确定系统的等效热导率。

Result: 确定了可用于近似整个LTES单元性能的等效热导率参数，该结果可作为卡诺电池等系统简化模型的输入参数。

Conclusion: 通过等效热导率方法成功开发了简化的LTES单元模型，该模型能够考虑几何结构对瞬态行为的影响，适用于系统级仿真，为卡诺电池的系统设计和优化提供了实用工具。
...
### [246] [Output Consensus on Periodic References for Constrained Multi-agent Systems Under a Switching Network](https://arxiv.org/abs/2601.03767)
*Shibo Han,Bonan Hou,Chong Jin Ong*

Main category: eess.SY

TL;DR: 提出了一种模型预测控制方法，结合人工参考和修正成本，解决具有通信延迟的切换网络下约束异构多智能体系统的输出一致性问题，能够跟踪周期性参考并保持递归可行性。


<details>
  <summary>Details</summary>
Motivation: 周期性参考具有更复杂的动态特性，跟踪周期性参考并实现一致性更具挑战性，特别是在存在通信延迟和切换网络的情况下。

Method: 采用模型预测控制方法，结合人工参考和修正成本设计控制策略；提出不依赖全局信息（如全局可容许参考集和全局时间索引）的一致性协议。

Result: 理论分析证明，所提算法能渐近实现约束输出一致性，各智能体的参考收敛且满足约束条件；数值仿真验证了算法的有效性。

Conclusion: 所提出的控制方法能够有效解决约束异构多智能体系统在切换网络下跟踪周期性参考并实现输出一致性的问题，且不依赖全局信息。
...
### [247] [Unified and Efficient Analysis of Machining Chatter and Surface Location Error](https://arxiv.org/abs/2601.03819)
*Woraphrut Kornmaneesang,Tsu-Chin Tsao,Niloufar Esfandi,Shyh-Leh Chen*

Main category: eess.SY

TL;DR: 提出了一种结合半离散化方法和提升方法的创新建模框架，用于高效计算铣削加工中的颤振稳定性和表面定位误差，相比现有方法显著提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统基于稳定性叶瓣图（SLD）的切削参数选择虽能抑制颤振，但无法消除受迫振动引起的表面粗糙度问题，限制了表面质量。需要同时考虑表面定位误差（SLE）和SLD来优化切削参数。

Method: 将加工动力学系统建模为角度时变时滞微分方程（DDE），采用半离散化方法（SDM）通过零相位插值将时变和时滞项近似为常项，再通过提升方法在刀具通过角度上合并系统，建立紧凑状态空间形式的显式动力学模型。

Result: 基于紧凑状态空间模型，能够简单高效地预测颤振稳定性和SLE。仿真结果表明，该方法相比其他知名方法显著提高了计算效率。

Conclusion: 所提出的建模框架为同时优化颤振稳定性和表面定位误差提供了有效的计算工具，有助于实现更高表面质量的铣削加工。
...
### [248] [A Systems-Engineered ESP32 DAQ Architecture and FAIR Data Workflow for Small-Scale Wind Turbine Performance Measurement in Tropical Environments](https://arxiv.org/abs/2601.03867)
*Asitha Lakruwan Kulasekera*

Main category: eess.SY

TL;DR: 本文提出了一种系统化的数据采集方法和基于ESP32的参考设计，用于小型风力涡轮机的现场表征，特别针对热带/沿海部署环境，旨在解决资源受限学术环境中数据不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 资源受限的学术环境中，小型风力涡轮机研究常因临时仪器、时间同步不足、存储故障和数据治理薄弱而产生不可靠或无法发表的数据集，尤其是在低中收入国家的热带/沿海环境中。

Method: 采用学生适应的V模型与需求可追溯性、针对高湿和盐雾环境的硬件选择策略、基于中断的转子速度测量和NTP时间同步的嵌入式固件架构、结合SD卡存储和MQTT云遥测的本地优先混合存储设计，以及基于CRISP-DM和FAIR原则的数据管理流程。

Result: 通过斯里兰卡沿海螺旋垂直轴风力涡轮机设计案例展示了完整方法，目标是在六个月的研究活动中实现超过90%的数据完整性，并提供了开源固件、硬件模板和数据发布工作流。

Conclusion: 该方法为资源受限环境中的小型风力涡轮机研究提供了系统化的数据采集和管理解决方案，通过开源工具和标准化流程提高了数据可靠性和可发表性。
...
### [249] [Smooth Sampling-Based Model Predictive Control Using Deterministic Samples](https://arxiv.org/abs/2601.03893)
*Markus Walker,Marcel Reith-Braun,Tai Hoang,Gerhard Neumann,Uwe D. Hanebeck*

Main category: eess.SY

TL;DR: 提出确定性采样MPPI（dsMPPI）方法，结合确定性采样与交叉熵方法改进，用于非线性系统的模型预测控制，实现更平滑的控制输入。


<details>
  <summary>Details</summary>
Motivation: 基于采样的模型预测控制（MPC）在处理非线性系统时有效，但随机采样常导致控制输入不平滑，需要改进。

Method: 扩展模型预测路径积分（MPPI）框架，引入确定性采样，并结合交叉熵方法（CEM）的迭代优化等改进，形成dsMPPI方法。

Result: 实验表明，dsMPPI相比现有先进方法能生成更平滑的轨迹。

Conclusion: dsMPPI通过结合MPPI的指数加权与确定性采样的效率，有效提升了控制输入的平滑性。
...
### [250] [Exact Continuous Reformulations of Logic Constraints in Nonlinear Optimization and Optimal Control Problems](https://arxiv.org/abs/2601.03906)
...
<details>
  <summary>Details</summary>
Motivation: 非线性最优控制和优化问题常涉及连续动态与离散逻辑条件结合的约束，传统混合整数规划方法存在可扩展性挑战且需要专用求解器。

Method: 将任意逻辑命题重写为合取范式，转换为等价的max-min约束，并应用保持精确可行集的平滑过程。

Result: 在四旋翼轨迹避障和混合双水箱时序逻辑约束两个基准问题上验证，比现有二元变量消除技术更一致高效地获得最优解。

Conclusion: 该方法能精确重构广泛类别的逻辑约束，其可微性与底层谓词一致，可直接集成到非线性规划中，提升求解效率。
...
### [251] [A Load Impedance Emulation Active Interface for Piezoelectric Vibration Energy Harvesters](https://arxiv.org/abs/2601.04136)
*Alessandro Lo Schiavo,Luigi Costanzo,Massimo Vitelli*

Main category: eess.SY

TL;DR: 提出了一种单级有源AC/DC接口，能够模拟谐振压电振动能量采集器的最优负载阻抗，无需MPPT算法即可实现最大功率点跟踪。


<details>
  <summary>Details</summary>
Motivation: 传统单级AC/DC接口需要采用多变量扰动方法实现最大功率点跟踪，导致性能受限且需要耗能的微控制器。

Method: 设计模拟最优负载阻抗的接口，采用简单模拟控制器，避免使用MPPT算法和微控制器。

Result: 实验验证该接口在稳态和振动变化条件下均能使采集器工作在最大功率点，显著提升静态和动态性能。

Conclusion: 所提接口通过模拟最优负载阻抗，无需复杂控制算法即可实现高效能量采集，简化了系统设计。
...
### [252] [Solar Panel-based Visible Light Communication for Batteryless Systems](https://arxiv.org/abs/2601.04190)
*Juan F. Gutierrez,Nhung Nguyen,Jesus M. Quintero,Andres Gomez*

Main category: eess.SY

TL;DR: 本文提出了一种完全由环境光供电、通过可见光通信接收数据的无电池物联网无线通信节点，利用太阳能板同时作为能量收集器和光学天线，实现了低功耗运行。


<details>
  <summary>Details</summary>
Motivation: 物联网设备通常依赖电池供电，存在维护困难和环境负担问题，因此需要开发能够从环境中持续获取能量并实现通信的免维护解决方案。

Method: 采用太阳能板作为能量收集器和光学天线，设计轻量级模拟前端进行信号滤波和数字化，使用8位低功耗处理器管理系统状态，通过选择性激活主处理器和与收集器开路相位同步的数据接收来降低能耗。

Result: 原型机能够可靠解码800赫兹的32位可见光通信帧，能耗低于2.8毫焦，睡眠模式功耗低于30微瓦。

Conclusion: 该研究成功实现了一种高效的环境光供电可见光通信节点，为物联网设备提供了可持续、低维护的通信解决方案，展示了在能量收集和通信集成方面的可行性。
...
### [253] [A low-complexity method for efficient depth-guided image deblurring](https://arxiv.org/abs/2601.03924)
*Ziyao Yi,Diego Valsesia,Tiziano Bianchi,Enrico Magli*

Main category: eess.IV

TL;DR: 提出了一种基于深度引导的低复杂度图像去模糊神经网络，利用小波变换分离结构细节并减少空间冗余，同时通过高效的特征条件化处理深度信息，在保持竞争力的图像质量的同时将计算复杂度降低了两个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习去模糊模型为追求最佳图像质量导致计算复杂度极高，仅适用于高性能服务器，无法在移动设备等资源受限场景部署；同时研究发现移动激光雷达提供的深度图可作为互补信息提升去模糊质量。

Method: 采用小波变换分离图像结构细节并降低空间冗余；设计高效的特征条件化机制融合深度信息；构建低复杂度神经网络架构实现深度引导的图像去模糊。

Result: 实验结果表明，该方法在图像质量上与当前最先进模型具有竞争力，同时将计算复杂度降低了高达两个数量级。

Conclusion: 小波变换的结构细节分离与空间冗余减少，结合深度信息的高效特征条件化，是开发低复杂度深度引导图像去模糊模型的关键要素，为移动端部署提供了可行方案。
...
### [254] [GeoDiff-SAR: A Geometric Prior Guided Diffusion Model for SAR Image Generation](https://arxiv.org/abs/2601.03499)
*Fan Zhang,Xuanting Wu,Fei Ma,Qiang Yin,Yuxin Hu*

Main category: eess.IV

TL;DR: 提出了GeoDiff-SAR，一种几何先验引导的扩散模型，用于生成高保真的合成孔径雷达图像，通过融合三维物理信息和多模态特征，显著提升了生成质量和方位角控制精度。


<details>
  <summary>Details</summary>
Motivation: 现有SAR图像生成方法主要在图像域操作，忽略了显式的几何信息，导致生成质量不佳且无法精确控制关键参数（如方位角）。

Method: 1. 通过计算特定方位角的SAR点云模拟几何结构和散射关系；2. 使用基于FiLM的特征融合门控网络动态调节三维物理信息、图像控制参数和文本描述参数的权重；3. 采用LoRA架构对SD3.5模型进行轻量级微调，使其适应SAR域分布特性。

Result: 在真实SAR数据集上的实验表明，GeoDiff-SAR生成的数据具有高保真度，能有效提升下游分类任务的准确性，尤其在不同方位角的识别性能上表现突出。

Conclusion: 物理引导的生成方法在SAR图像生成中具有优越性，GeoDiff-SAR通过几何先验和多模态融合实现了高质量、可控的SAR图像生成。
...
### [255] [Scanner-Induced Domain Shifts Undermine the Robustness of Pathology Foundation Models](https://arxiv.org/abs/2601.04163)
*Erik Thiringer,Fredrik K. Gustafsson,Kajsa Ledesma Eriksson,Mattias Rantalainen*

Main category: eess.IV

TL;DR: 本研究系统评估了14种病理学基础模型对扫描仪设备引起的技术领域偏移的鲁棒性，发现现有模型均无法可靠抵抗扫描仪变异性，其嵌入空间和预测校准会受系统性影响。


<details>
  <summary>Details</summary>
Motivation: 病理学基础模型在基准测试中表现良好，但其对真实世界技术领域偏移（如不同全玻片扫描仪设备差异）的鲁棒性尚不明确，可能影响临床应用的可靠性。

Method: 使用包含5种扫描设备的384张乳腺癌全玻片图像数据集，通过无监督嵌入分析和有监督临床病理预测任务，系统评估14种模型（包括前沿模型、早期自监督模型及自然图像训练基线）的鲁棒性。

Result: 所有模型均对扫描仪变异性敏感，其嵌入空间存在明显的扫描仪特异性变异；虽然AUC指标常保持稳定，但嵌入空间变异会导致下游预测校准发生扫描仪依赖性偏差，且鲁棒性与训练数据规模、模型大小或新近性无简单关联。

Conclusion: 病理学基础模型的开发与评估需超越以准确率为中心的基准测试，转向在真实采集变异性下对嵌入稳定性和校准的显式评估与优化。
...
### [256] [Ensemble Models for Predicting Treatment Response in Pediatric Low-Grade Glioma Managed with Chemotherapy](https://arxiv.org/abs/2601.03899)
*Max Bengtsson,Elif Keles,Angela J. Waanders,Ulas Bagci*

Main category: eess.IV

TL;DR: 提出一种结合Swin UNETR分割网络与XGBoost分类器的集成框架，通过术前MRI影像组学特征和临床数据预测儿童脑肿瘤化疗反应。


<details>
  <summary>Details</summary>
Motivation: 针对无法完全手术切除的儿童脑肿瘤患者（尤其是低级别胶质瘤），化疗是主要治疗选择，但该群体无进展生存率较低，需要非侵入性方法预测化疗反应以实现个性化治疗。

Method: 1. 使用Swin UNETR分割模型提取肿瘤四个亚区域（增强/非增强肿瘤、囊性成分、水肿）；2. 从分割结果提取影像组学特征，结合临床变量（人口学、分子亚型、治疗信息等）；3. 构建集成模型：Swin UNETR直接分类MRI图像，XGBoost基于特征分类，两者结果融合。

Result: 集成模型在测试中表现最佳：无效病例精确率0.68、召回率0.85，有效病例精确率0.64，总体准确率0.69，优于Mamba-FeatureFuse等对比模型。

Conclusion: 该集成框架为无法手术的儿童脑肿瘤患者提供了有前景的化疗反应预测工具，有助于推动个性化治疗决策。
...
### [257] [Edit2Restore:Few-Shot Image Restoration via Parameter-Efficient Adaptation of Pre-trained Editing Models](https://arxiv.org/abs/2601.03391)
*M. Akın Yılmaz,Ahmet Bilican,Burak Can Biner,A. Murat Tekalp*

Main category: eess.IV

TL;DR: 本文提出一种基于预训练文本条件图像编辑模型的少样本图像修复方法，通过微调LoRA适配器，仅需16-128张配对图像即可处理去噪、去雨、去雾等多种修复任务。


<details>
  <summary>Details</summary>
Motivation: 传统图像修复方法需要为每种退化类型训练专用模型，且依赖大量配对数据。本文旨在探索如何利用大规模预训练编辑模型中已编码的视觉先验，显著降低数据需求，实现少样本、多任务的图像修复。

Method: 使用FLUX.1 Kontext（120亿参数流匹配模型）作为基础模型，通过参数高效的LoRA适配器进行微调，仅需少量配对图像（16-128张）和指定修复操作的文本提示。采用统一的多任务适配器处理多种退化类型。

Result: 方法在感知质量上表现优异，虽在PSNR/SSIM等像素级重建指标上不占优，但能有效处理多种退化任务。消融实验表明：训练集大小影响修复质量；统一适配器优于任务专用适配器；文本编码器微调起到关键作用；预训练模型具备零样本修复潜力。

Conclusion: 预训练图像编辑模型经过适当适配后，可作为传统图像修复方法的数据高效替代方案，为少样本、提示引导的图像增强开辟了新途径。
...
### [258] [Staged Voxel-Level Deep Reinforcement Learning for 3D Medical Image Segmentation with Noisy Annotations](https://arxiv.org/abs/2601.03875)
*Yuyang Fu,Xiuzhen Guo,Ji Shi*

Main category: eess.IV

TL;DR: 提出SVL-DRL框架，通过分阶段强化学习解决医学图像分割中的噪声标注问题，在三个公开数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割依赖高质量标注数据，但器官形态复杂和标注者差异常导致噪声标注，严重影响模型性能。受标注者基于先验知识修正标签的启发，研究旨在开发无需人工干预的鲁棒分割方法。

Method: 1. 将噪声标注建模为体素依赖问题，采用分阶段强化学习框架保证收敛；2. 设计体素级异步优势演员-评论家模块，每个体素作为自主智能体动态优化状态表示；3. 设计新型动作空间和结合Dice值与空间连续性度量的复合奖励函数。

Result: 在三个公开医学图像数据集上，多种实验设置下均达到SOTA性能，Dice和IoU分数平均提升超过3%。

Conclusion: SVL-DRL框架能有效降低噪声标注影响，提升分割精度和语义完整性，为医学图像分割中的标注噪声问题提供了自动化解决方案。
